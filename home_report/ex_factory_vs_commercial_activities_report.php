...
{ 
	...
	$realize_arr=return_library_array( "select a.po_breakdown_id, sum(a.current_invoice_value) as realize_value  from com_export_invoice_ship_dtls a, com_export_proceed_realization c where a.mst_id=c.INVOICE_BILL_ID   and a.status_active=1 and a.is_deleted=0  and c.is_invoice_bill=2 and c.status_active=1 and c.is_deleted=0  group by a.po_breakdown_id
	union all
	select a.po_breakdown_id, sum(a.current_invoice_value) as realize_value from com_export_invoice_ship_dtls a, com_export_doc_submission_invo b , com_export_proceed_realization c where a.mst_id=b.invoice_id and b.doc_submission_mst_id=c.invoice_bill_id and c.is_invoice_bill=1 and  a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by a.po_breakdown_id", "po_breakdown_id", "realize_value");
	
	
	if($location!="" and $location!=0) $location_cond= "and a.location_name=$location "; else $location_cond="";
 
	$sql="select a.id, a.buyer_name, a.job_no_prefix_num, a.job_no, a.gmts_item_id, a.total_set_qnty as ratio, b.id as po_id, b.po_number, b.pub_shipment_date, b.unit_price,b.po_total_price from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.company_name='$company_name' and b.pub_shipment_date between '$firstDate' and '$lastDate' and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $location_cond order by b.pub_shipment_date, a.job_no_prefix_num, b.id";	
	//echo $sql;die;
	...
	foreach($nameArray as $row)
	{
		...
		//echo $pro_ex_factory_mst_sql;die;
        ...
        <div style="width:1068px; overflow-y:scroll; max-height:400px" id="scroll_body">
            <table width="1050px" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all" id="table_body">
                ...
                foreach($nameArray as $row)
                {
					...
					$ex_factory_sql = sql_select("SELECT ENTRY_FORM, PO_BREAK_DOWN_ID, EX_FACTORY_QNTY from pro_ex_factory_mst where PO_BREAK_DOWN_ID=$po_id and status_active=1 and is_deleted=0");

					$ex_factory_total_entry = 0;
					$ex_factory_total_return = 0;
					foreach($ex_factory_sql as $ex_factory)
					{
						// entry
						if($ex_factory['ENTRY_FORM']==0){
							$ex_factory_total_entry += $ex_factory['EX_FACTORY_QNTY'];
						}
						// return
						elseif($ex_factory['ENTRY_FORM']==85){
							$ex_factory_total_return += $ex_factory['EX_FACTORY_QNTY'];
						}
					}

					$total_factory_total = $ex_factory_total_entry-$ex_factory_total_return;

					$ex_factory_values = $total_factory_total*$unit_price;
					...
					if(round($ex_factory_value)>0 || round($invoice_value)>0 || round($submission_value)>0 || round($realize_value)>0)
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
							...
							<td width="100" align="right"><p><? echo number_format($total_factory_total, 2); ?></p></td>
							<td width="100" align="right"><p><? echo number_format($ex_factory_values,2); ?></p></td>
