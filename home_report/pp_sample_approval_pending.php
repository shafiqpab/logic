...
{ 
	...
	if($db_type==0)
	{
		$buyer_con=return_field_value("group_concat(b.buyer_id) as buyer_id","lib_sample a,lib_buyer_tag_sample b","a.id=b.tag_sample and a.is_deleted=0 and a.sample_type=2 and b.sequ<>''","buyer_id");
		$buyer_con=implode(",",array_unique(explode(",",$buyer_con)));
	
		$sql = sql_select("SELECT po_break_down_id,color_number_id from wo_po_sample_approval_info where status_active=1 and is_deleted=0  and (entry_form_id is null or entry_form_id=0) group by po_break_down_id,color_number_id order by color_number_id,po_break_down_id");
		$m_sql="select distinct a.style_ref_no, a.buyer_name, a.job_no_prefix_num, a.dealing_marchant, b.id, b.po_number, b.shipment_date from wo_po_details_master a, wo_po_break_down b, wo_po_sample_approval_info c
		where  
		...
		a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.status_active=1 and c.is_deleted=0  and c.sample_type_id = '7'  and (c.entry_form_id is null or c.entry_form_id=0) $company_name_con 
	...
	else if($db_type==2)
	{
		...
		$sql = sql_select("SELECT po_break_down_id,color_number_id,sample_type_id from wo_po_sample_approval_info where  status_active=1 and is_deleted=0 and (entry_form_id is null or entry_form_id=0) group by po_break_down_id,color_number_id,sample_type_id order by color_number_id,po_break_down_id");

		$m_sql="select distinct a.style_ref_no, a.buyer_name, a.job_no_prefix_num, a.dealing_marchant, b.id, b.po_number, b.shipment_date,b.po_received_date
    ...
    <div align="center"> <h3 style="top:1px;width:98%; text-align:center;" id="accordion_h1" class="accordion_h" align="center"> PP Sample Approval Pending </h3></div>
    <div align="center">
...
Highcharts.setOptions(Highcharts.theme);
    ...
    <fieldset style="width:1200px;"  id="month_wise_rpt" >
...
<script>
	...
	//const month_wise_rpt=document.querySelector('#month_wise_rpt');
	document.getElementById('month_summery_div').style.marginBottom='50px';
	document.getElementById('month_wise_rpt').style.marginTop ='70px';
</script>
