...
$fabric_booking_type = array( 0 =>'-- Select --',1 => "Multiple Job Wise Trims Booking", 2 => "Short Trims Booking");
...
<script>
	...
	function trims_order_report(bookingType, bookingNo, company){
		//bookingType==Short, main, woven etc.
		//txt_booking_no,cbo_company_name,txt_order_no_id,cbo_fabric_natu,cbo_fabric_source,txt_job_no,id_approved_id,print_id,entry_form,type,i,fabric_nature,cbouom
		//$fabric_booking_type = array(87 => "Main", 178 => "Short" , 272 => "Woven Main");
		var action = return_global_ajax_value( bookingType+"__"+company+"__", 'report_setting_variables', '', 'requires/date_wise_trims_booking_report_controller');
		action = trim(action);
		if(action == ""){
			alert("NO default print Setting!");return;
		}
		console.log(action);
		///alert("Function");return;
		var report_title = "";
		var data="action="+action+
		'&txt_booking_no='+"'"+bookingNo+"'"+
		...
		if(action == "show_trim_booking_report13"){
			...
			http.open("POST","../woven_order/requires/trims_booking_multi_job_controllerurmi.php",true);
		}else if(action == "show_trim_booking_report"){
			http.open("POST","../woven_order/requires/short_trims_booking_controller.php",true);
		}else{
			alert("NO default print Setting2!");return;
		}
	...
	function fn_report_generated(type)
	{
			...
			if(type !=4){
				if(booking_no!="")
					{
						if(form_validation('cbo_company_id*txt_booking_no','Company Name*Booking No')==false)
						{
							return;
						}
					}
			...
			var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_buyer_id*cbo_fab_booking_type*txt_date_from*txt_date_to*txt_booking_no_id*txt_booking_no*cbo_based_on',"../../")+'&report_title='+report_title+'&type='+type;
			
			http.open("POST","requires/date_wise_trims_booking_report_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
		...
		if(http.readyState == 4)
		{
			...
			else if(trim(reponse[3])==1)
			{
				if(tot_rows*1>1){
					var tableFilters = {
						col_operation: {
						   id: ["td_tot_booking_qty","td_tot_fin_qnty","td_tot_plan_qnty"],
						   col: [16,17,18],
	...
	function openmypage_fabricBooking()
	{
		...
		if (form_validation('cbo_company_id*cbo_fab_booking_type','Company*Booking Type')==false)
		{
			return;
		}
		else
		{ 	
			...
			var page_link = 'requires/date_wise_trims_booking_report_controller.php?cbo_company_id='+cbo_company_id+'&cbo_fab_booking_type='+cbo_fab_booking_type+'&txt_booking_no='+txt_booking_no+'&txt_booking_no_id='+txt_booking_no_id+'&action=fabricBooking_popup';
			...
			emailwindow.onclose=function()
			{
				...
				if(theemail!="")
				{
					...
					$('#cbo_fab_booking_type').attr('disabled', 'disabled');
					release_freezing();
				} 
			}
		}
        ...
        <div id="content_search_panel" style="width:850px" >
            ...
            <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                <thead>
                    ...
                    <th width="140" class="must_entry_caption" colspan="2" id="date_caption">Wo Date Range</th>
                ...
                <tbody>
                    <tr class="general">
                        <td><? echo create_drop_down( "cbo_company_id", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down('requires/date_wise_trims_booking_report_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" ); ?></td>
