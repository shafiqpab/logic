...
<script>
	...
	function fn_report_generated()
	{
		...
		{	
			var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_team_name*cbo_team_member*txt_date_from*txt_date_to*txt_job_no*txt_style_ref*txt_order_no*cbo_year_selection*cbo_category_by',"../../");
			//alert(data);
			freeze_window(3);
			http.open("POST","requires/pp_approval_status_report_controller.php",true);
	...
	{ //alert(book_num);
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/pp_approval_status_report_controller.php?po_id='+po_id+'&item_name='+item_name+'&job_no='+job_no+'&book_num='+book_num+'&trim_dtla_id='+trim_dtla_id+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../');
	}
	function openmypage_inhouse(po_id,item_name,action)
	{
		var popup_width='900px';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/pp_approval_status_report_controller.php?po_id='+po_id+'&item_name='+item_name+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../');
	}
	...
	function order_qty_popup(company,job_no,po_id,buyer,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/pp_approval_status_report_controller.php?company='+company+'&job_no='+job_no+'&po_id='+po_id+'&buyer='+buyer+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../');
	}*/
	...
	function ppsample_popup(po_id,color_id,send_to_factory_date,recv_from_factory_date,submitted_to_buyer,approval_status_date,pp_submitted_to_buyer,pp_approval_status_date)
	{
		//alert(country_id);
		var popup_width='320px';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/pp_approval_status_report_controller.php?action=ppsample_veiw&po_id='+po_id+'&color_id='+color_id+'&send_to_factory_date='+send_to_factory_date+'&recv_from_factory_date='+recv_from_factory_date+'&submitted_to_buyer='+submitted_to_buyer+'&approval_status_date='+approval_status_date+'&pp_submitted_to_buyer='+pp_submitted_to_buyer+'&pp_approval_status_date='+pp_approval_status_date, 'Details Veiw', 'width='+popup_width+', height=200px,center=1,resize=0,scrolling=0','../');
	}
	...
	function booking_date_popup(po_id,color_id)
	{
		//alert(country_id);
		var popup_width='320px';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/pp_approval_status_report_controller.php?action=booking_date_view&po_id='+po_id+'&color_id='+color_id, 'Details Veiw', 'width='+popup_width+', height=200px,center=1,resize=0,scrolling=0','../');
	}
	...
	function fin_receive_date_popup(po_id,color_id,booking_id)
	{
		//alert(country_id);
		var popup_width='320px';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/pp_approval_status_report_controller.php?action=fin_receive_date_view&po_id='+po_id+'&color_id='+color_id+'&booking_id='+booking_id, 'Details Veiw', 'width='+popup_width+', height=200px,center=1,resize=0,scrolling=0','../');
	}
            ...
            <fieldset style="width:1200px;">
                <table class="rpt_table" width="1200" border="1" rules="all" cellpadding="1" cellspacing="2" align="center">
                    ...
                    <tr class="general">
                        <td> 
                            <?
								echo create_drop_down( "cbo_company_name", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/pp_approval_status_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>
                        </td>
                        <td id="buyer_td">
                            <? 
                         ...
                         <td>
                             <?
                                    echo create_drop_down( "cbo_team_name", 120, "select id,team_name from lib_marketing_team  where status_active =1 and is_deleted=0  order by team_name","id,team_name", 1, "-- Select Team --", $selected, " load_drop_down( 'requires/pp_approval_status_report_controller', this.value, 'load_drop_down_team_member', 'team_td' )" );
                              ?>
                         </td>
                         <td id="team_td">
							 <? 
                       ...
                       <td>
                        <select name="cbo_category_by" id="cbo_category_by"  style="width:100px" class="combo_boxes">
                            ...
                            <option value="3">Lab Submission </option>
                            <option value="4">Lab Approval</option>
                            <option value="5">PP Submission</option>
                            <option value="6">PP Approval</option>
                            <option value="7">Pub Ship Date Wise</option>
                        </select>
                    </td>
