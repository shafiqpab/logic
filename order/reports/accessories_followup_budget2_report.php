<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Accessories Followup Report[Budget 2].
Functionality	:	
JS Functions	:
Created by		:	Kausar 
...
<script>
	...
	function fn_report_generated(action)
	{
		...
		{	
			...
			//alert(data);
			freeze_window(3);
			http.open("POST","requires/accessories_followup_budget2_report_controller.php",true);
	...
	function generate_woven_report(company,job_no,buyer_name,style_ref_no,costing_date,po_id,costing_per,type)
	{
	...
	{
		var popup_width='900px';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/accessories_followup_budget2_report_controller.php?po_id='+po_id+'&item_name='+item_name+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../');
	}
	...
	function order_qty_popup(company,job_no,po_id,buyer,from_date,to_date,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/accessories_followup_budget2_report_controller.php?company='+company+'&job_no='+job_no+'&po_id='+po_id+'&buyer='+buyer+'&from_date='+from_date+'&to_date='+to_date+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../');
	}
...
function openmypage_style(type_id)
	{		
		if( form_validation('cbo_company_name','Company Name')==false )
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/accessories_followup_budget2_report_controller.php?data='+data+'&action=style_popup', 'style Search', 'width=480px,height=420px,center=1,resize=0,scrolling=0','../')
	...
	//alert(1);
		...
		var data = $("#cbo_company_name").val()+"_"+$("#cbo_buyer_name").val()+"_"+$("#txt_style_ref").val()+"_"+$("#cbo_year_selection").val();
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/accessories_followup_budget2_report_controller.php?data='+data+'&action=order_no_popup', 'Order No Search', 'width=450px,height=420px,center=1,resize=0,scrolling=0','../')
			emailwindow.onclose=function()
			{
				var theemailid=this.contentDoc.getElementById("txt_po_id");
				var theemailval=this.contentDoc.getElementById("txt_po_val");
        ...
        <div id="content_search_panel" > 
            <fieldset style="width:1650px;">
                <table class="rpt_table" width="1650" border="1" rules="all" cellpadding="1" cellspacing="2" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            <td><? echo create_drop_down( "cbo_company_name", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/accessories_followup_budget2_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );get_php_form_data(this.value,'print_button_variable_setting','requires/accessories_followup_budget2_report_controller' );" ); ?></td>
                            <td id="buyer_td"> <? echo create_drop_down( "cbo_buyer_name", 130, $blank_array,"", 1, "-- All Buyer --", $selected, "",0,"" ); ?></td>
							<td><?
							
							echo 	create_drop_down( "cbo_team_name", 120, "select id,team_leader_name from lib_marketing_team where   status_active =1 and is_deleted=0 order by team_leader_name","id,team_leader_name", 1, "-- Select Team --", $selected, "load_drop_down( 'requires/accessories_followup_budget2_report_controller', this.value, 'load_drop_down_team_member', 'team_td' );" );
                         ...
                         <td id="team_td">
                            ...
                            <td>
                                <input type="button" id="show_button" class="formbutton" style="width:80px;display: none;" value="Show" onClick="fn_report_generated('report_generate')" />
                                 <input type="button" id="show_item" class="formbutton" style="width:80px;display: none;" value="Item Wise" onClick="fn_report_generated('item_report_generate')" />
                            </td>
                        </tr>
                        <tr>
                            <td colspan="13" align="center"><? echo load_month_buttons(1); ?></td>
                            <td align="center"><input type="button" id="show_button1" class="formbutton" style="width:80px;display: none;" value="With Html" onClick="fn_report_generated('report_generate2')" /></td>
							<td align="center"><input type="button" id="show_button2" class="formbutton" style="width:80px;display: none;" value="Without Value" onClick="fn_report_generated('report_generate3')" /></td>
                        </tr>
                    </tbody>
                </table>
            </fieldset>
