...
{
	$print_report_format=return_field_value("format_id","lib_report_template","template_name ='$data' and module_id=2 and report_id=242 and is_deleted=0 and status_active=1");
...
if($action=="report_generate2")
{
    ...
    if(count($main_data_sql) > 0) {
       ...
       /* $receive_qty_data = sql_select("SELECT d.detarmination_id, d.color, c.quantity as order_qnty, b.order_amount, e.job_id,  b.order_rate	from inv_receive_master a join inv_transaction b on a.id = b.mst_id join order_wise_pro_details c on b.id = c.trans_id join product_details_master d on d.id=c.prod_id join wo_po_break_down e 	on e.id=c.po_breakdown_id where a.entry_form=17 and a.receive_basis=1 and a.status_active=1 and a.is_deleted=0 and b.receive_basis=1 and b.transaction_type=1 and b.status_active=1 and b.is_deleted=0 and c.trans_type=1 and c.entry_form=17 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 $order_cond	group by d.detarmination_id, d.color, c.quantity, b.order_amount, e.job_id, b.order_rate");*/
        ...
        /*$pi_number_data=sql_select("SELECT f.style_ref_no,d.pi_number,g.lc_number,f.job_no,i.job_id, i.lib_yarn_count_deter_id, c.amount, c.color_id as color_id
        from wo_booking_dtls b left join  wo_pre_cost_fabric_cost_dtls i on   b.job_no = i.job_no,com_pi_item_details c , com_pi_master_details d ,wo_po_details_master f,com_btb_lc_master_details g,com_btb_lc_pi h 	where b.booking_no=c.work_order_no and g.id=h.com_btb_lc_master_details_id and h.pi_id=d.id and b.job_no = f.job_no and d.id=c.pi_id and b.status_active=1 and b.is_deleted=0
        and c.status_active=1 and c.is_deleted=0 and g.item_basis_id in (1,2) and d.status_active=1 and d.is_deleted=0 and g.importer_id in (1)  $jobid_cond4 and d.entry_form=166
        group by f.style_ref_no,d.pi_number,g.lc_number,f.job_no ,i.job_id, i.lib_yarn_count_deter_id, c.amount, c.color_id ");
				...
				foreach ($main_data_arr as $job_id => $job_data) {
					...
					foreach ($job_data['color_data'] as $lib_yarn_id=>$color_data) {
						...
						$blc_nos=implode(", ",array_unique(explode(",",$blc_no)));
							...
							<td align="right"><?= implode(",",$wo_data_arr[$job_id][$lib_yarn_id][$color_id][$fabric_color_id]['booking_no'] )  ?></td>
				...
				<?php foreach ($trims_main_data as $job_id=>$value){
				 ...
				 ?>
					...
					<a href='#report_details' onClick="order_summery_popup('<? echo $job_id; ?>' ,'order_summery_data');"><?= $value['style_ref_no']; ?></a></td>
		                ...
		                <td align="right"><?= fn_number_format($wo_amount,2);  ?></td>
		                <td width="80" align="right"><a href='#report_details' onClick="openmypage_trim_inhouse('<? echo $trims_poid; ?>','<? echo $row['trim_group'].'__'.$row['description']; ?>','trim_booking_inhouse_info');"><?= fn_number_format($inhouse_qty,2);  ?></a></td>
		                <td align="right"><?= fn_number_format($inhouse_amount,2); ?></td>
		                <td width="80" align="right"><?= fn_number_format($trimrcv_balance,2); ?></td>
		                <td width="70" align="right"><a href='#report_details' onClick="openmypage_trim_issue('<? echo $trims_poid; ?>','<? echo $row['trim_group'].'__'.$row['description']; ?>','trim_booking_issue_info');"><?= fn_number_format($issue_qty,2);  ?></a></td>
						...
						<td width="140" align="left"><p><?=$trim_pi_lc_data_arr[$trim_pi_nos]['lc_number'];//$trim_blc_nos;?></p></td>
						<? $z++;
...
$main_data_sql=sql_select("SELECT a.id, a.job_no, a.buyer_name, a.job_no_prefix_num, a.season_buyer_wise, a.style_ref_no, a.job_quantity, a.order_uom, a.company_name,  b.id as po_id, b.po_number,  c.color_number_id, c.order_quantity, e.id as fabric_cost_id, e.lib_yarn_count_deter_id, e.body_part_id,e.fabric_description, e.uom as fabric_uom, avg(f.requirment) as avg_cons, e.color_size_sensitive, e.color, e.color_break_down, g.contrast_color_id, h.stripe_color,e.rate from wo_po_details_master a join wo_po_break_down b on a.id=b.job_id join wo_po_color_size_breakdown c on a.id=c.job_id and b.id=c.po_break_down_id join wo_pre_cost_mst d on a.id=d.job_id join wo_pre_cost_fabric_cost_dtls e on a.id=e.job_id join wo_pre_cos_fab_co_avg_con_dtls f on e.id=f.pre_cost_fabric_cost_dtls_id and f.color_size_table_id=c.id  and b.id=f.po_break_down_id  and c.po_break_down_id=f.po_break_down_id  left join wo_pre_cos_fab_co_color_dtls g on a.id=g.job_id and e.id=g.pre_cost_fabric_cost_dtls_id and c.color_number_id =g.gmts_color_id and g.status_active=1 and g.is_deleted=0 left join wo_pre_stripe_color h on a.id=h.job_id and  c.item_number_id= h.item_number_id and e.id=h.pre_cost_fabric_cost_dtls_id and f.color_number_id =h.color_number_id and f.po_break_down_id=h.po_break_down_id and f.gmts_sizes=h.size_number_id where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and a.company_name=$company_name $year_cond $date_cond $style_ref_cond $jobcond $buyer_id_cond group by a.id, a.job_no, a.buyer_name, a.job_no_prefix_num, a.season_buyer_wise, a.style_ref_no, a.job_quantity, a.order_uom, b.id,b.po_number, c.color_number_id, c.order_quantity, e.lib_yarn_count_deter_id, e.fabric_description, e.uom, e.id, e.color_size_sensitive, e.color, e.body_part_id, e.color_break_down,g.contrast_color_id, h.stripe_color, a.company_name,e.rate  order by e.id asc");
    ...
    if(count($main_data_sql) > 0) {
      ...
      /*  $po_id = array_chunk($po_id_arr, 1000, true);
		...
		$con = connect();
		execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ref_from in (1,2,3) and ENTRY_FORM=2003");
		oci_commit($con);
		fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 2003, 1, $po_id_arr, $empty_arr);//PO ID Ref from=1
		fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 2003, 2, $job_id_arr, $empty_arr);//PO ID Ref from=2
		fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 2003, 3, $fabric_id_arr, $empty_arr);//PO ID Ref from=3
// A.JOB_ID, A.LIB_YARN_COUNT_DETER_ID, B.ID, B.BOOKING_DATE,B.BOOKING_NO, C.GMTS_COLOR_ID, C.FIN_FAB_QNTY, C.AMOUNT,	B.SUPPLIER_ID, C.FABRIC_COLOR_ID
        $wo_data_sql = sql_select("SELECT a.job_id as JOB_ID, a.lib_yarn_count_deter_id as DETER_ID, b.id as ID, b.booking_date as BOOKING_DATE,b.booking_no as BOOKING_NO, c.gmts_color_id as GMTS_COLOR_ID, c.fin_fab_qnty as FIN_FAB_QNTY, c.amount as AMOUNT,	b.supplier_id as SUPPLIER_ID, c.fabric_color_id as FABRIC_COLOR_ID FROM wo_pre_cost_fabric_cost_dtls a , wo_booking_dtls c  , wo_booking_mst b,gbl_temp_engine g  where a.id=c.pre_cost_fabric_cost_dtls_id and b.booking_no=c.booking_no  and a.id=g.ref_val and c.pre_cost_fabric_cost_dtls_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in (3) and g.entry_form=2003 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  and c.fin_fab_qnty is not null ");
	
		
        foreach ($wo_data_sql as $row) {
            $wo_data_arr[$row['JOB_ID']][$row['DETER_ID']][$row['GMTS_COLOR_ID']][$row['FABRIC_COLOR_ID']]['fin_fab_qnty'] += $row['FIN_FAB_QNTY'];
            $wo_data_arr[$row['JOB_ID']][$row['DETER_ID']][$row['GMTS_COLOR_ID']][$row['FABRIC_COLOR_ID']]['amount'] += $row['AMOUNT'];
            $wo_data_arr[$row['JOB_ID']][$row['DETER_ID']][$row['GMTS_COLOR_ID']][$row['FABRIC_COLOR_ID']]['wo_date'][$row['ID']] = $row['BOOKING_DATE'];
            $wo_data_arr[$row['JOB_ID']][$row['DETER_ID']][$row['GMTS_COLOR_ID']][$row['FABRIC_COLOR_ID']]['booking_no'][$row['ID']] = $row['BOOKING_NO'];
            $wo_data_arr[$row['JOB_ID']][$row['DETER_ID']][$row['GMTS_COLOR_ID']][$row['FABRIC_COLOR_ID']]['supplier'][$row['ID']] = $lib_supplier_arr[$row['SUPPLIER_ID']];
        }
		unset($wo_data_sql);
        ...
        $receive_qty_data = sql_select("SELECT c.po_breakdown_id as PO_ID,d.detarmination_id as DETER_ID, d.color as COLOR, c.quantity as ORDER_QNTY, b.order_amount as ORDER_AMOUNT,   b.order_rate as ORDER_RATE	from inv_receive_master a , inv_transaction b , order_wise_pro_details c , product_details_master d,gbl_temp_engine g   where  a.id = b.mst_id and  b.id = c.trans_id  and  d.id=c.prod_id  and  d.id=b.prod_id and c.po_breakdown_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in (1) and g.entry_form=2003  and a.entry_form=17 and a.receive_basis in (1,2) and a.status_active=1 and a.is_deleted=0 and b.receive_basis in (1,2) and b.transaction_type=1 and b.status_active=1 and b.is_deleted=0 and c.trans_type=1 and c.entry_form=17 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0");
		
        $receive_qty_arr = array();
        foreach ($receive_qty_data as $row) {
			$job_id=$po_idArr[$row['PO_ID']];
            $receive_qty_arr[$job_id][$row['DETER_ID']][$row['COLOR']]['qty'] += $row['ORDER_QNTY'];
            $receive_qty_arr[$job_id][$row['DETER_ID']][$row['COLOR']]['amount'] += $row['ORDER_AMOUNT'];
            $receive_qty_arr[$job_id][$row['DETER_ID']][$row['COLOR']]['rate'] = $row['ORDER_RATE'];
            $receive_qty_arr[$job_id][$row['DETER_ID']][$row['COLOR']]['po_id'] = $row['PO_ID'];
        }
		//D.DETARMINATION_ID, D.COLOR, B.CONS_QUANTITY, B.CONS_AMOUNT, E.JOB_ID, E.ID AS PO_ID,D.SUPPLIER_ID
        /*$issue_qty_data = sql_select("SELECT d.detarmination_id as DETER_ID, d.color as COLOR, b.cons_quantity as CONS_QUANTITY, b.cons_amount as CONS_AMOUNT, e.job_id as JOB_ID, e.id as PO_ID,d.supplier_id as SUPPLIER_ID from inv_issue_master a , inv_transaction b  , order_wise_pro_details c  ,product_details_master d , wo_po_break_down e,gbl_temp_engine g  where  a.id = b.mst_id and b.id = c.trans_id and d.id=c.prod_id and e.id=c.po_breakdown_id and c.po_breakdown_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in (1) and g.entry_form=2003  and a.entry_form=19 and a.issue_basis=1 and a.status_active=1 and a.is_deleted=0 and b.transaction_type=2 and b.status_active=1 and b.is_deleted=0 and c.trans_type=2 and c.entry_form=19 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 $order_cond");*/
		 $issue_qty_data = sql_select("SELECT d.detarmination_id as DETER_ID, d.color as COLOR, b.cons_quantity as CONS_QUANTITY, b.cons_amount as CONS_AMOUNT, e.job_id as JOB_ID, e.id as PO_ID,d.supplier_id as SUPPLIER_ID from inv_issue_master a , inv_transaction b  , order_wise_pro_details c  ,product_details_master d , wo_po_break_down e,gbl_temp_engine g  where  a.id = b.mst_id and b.id = c.trans_id and d.id=c.prod_id and e.id=c.po_breakdown_id and c.po_breakdown_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in (1) and g.entry_form=2003  and a.entry_form=19 and a.issue_basis=1 and a.status_active=1 and a.is_deleted=0 and b.transaction_type=2 and b.status_active=1 and b.is_deleted=0 and c.trans_type=2 and c.entry_form=19 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 ");
		

        $issue_qty_arr = array();
        foreach ($issue_qty_data as $row) {
            $rate = $receive_qty_arr[$row['JOB_ID']][$row['DETER_ID']][$row['COLOR']]['rate'];
            $issue_qty_arr[$row['JOB_ID']][$row['PO_ID']][$row['DETER_ID']][$row['COLOR']]['qty'] += $row['CONS_QUANTITY'];
            $issue_qty_arr[$row['JOB_ID']][$row['PO_ID']][$row['DETER_ID']][$row['COLOR']]['amount'] += $row['CONS_QUANTITY'] * $rate;
            $issue_qty_arr[$row['JOB_ID']][$row['PO_ID']][$row['DETER_ID']][$row['COLOR']]['po_id'] = $row['PO_ID'];
            $issue_qty_arr[$row['JOB_ID']][$row['PO_ID']][$row['DETER_ID']][$row['COLOR']]['supplier'] = $lib_supplier_arr[$row['SUPPLIER_ID']];
        }
//A.JOB_ID, A.SUPPLIER_ID, B.LIB_YARN_COUNT_DETER_ID
        $fabric_supplier_data = sql_select("SELECT a.job_id as JOB_ID, a.supplier_id as SUPPLIER_ID, b.lib_yarn_count_deter_id as DETER_ID from wo_pre_cost_fabric_supplier a , wo_pre_cost_fabric_cost_dtls b ,gbl_temp_engine g where  a.JOB_ID=b.JOB_ID and a.JOB_ID=g.ref_val and b.JOB_ID=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in (2) and g.entry_form=2003 and b.id=a.fabric_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 ");

        $fabric_supplier_arr = array();
        foreach ($fabric_supplier_data as $row) {
            $fabric_supplier_arr[$row['JOB_ID']][$row['DETER_ID']][$row['SUPPLIER_ID']] = $lib_supplier_arr[$row['SUPPLIER_ID']];
        } 

        // $pi_number_data=sql_select("SELECT a.job_id, a.lib_yarn_count_deter_id, c.color_id as color_id,d.pi_number, c.amount, e.lc_number from wo_pre_cost_fabric_cost_dtls a join wo_booking_dtls b on a.job_no=b.job_no join com_pi_item_details c on b.booking_no=c.work_order_no and c.determination_id=a.lib_yarn_count_deter_id join com_pi_master_details d on d.id=c.pi_id left join com_btb_lc_master_details e on TO_CHAR(c.pi_id)=e.pi_id where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and d.entry_form=166 $jobid_cond1  group by  a.job_id, a.lib_yarn_count_deter_id,c.color_id,d.pi_number, c.amount, e.lc_number"); //c.color_id

        /*	$pi_number_data=sql_select("SELECT f.style_ref_no,d.pi_number,g.lc_number,f.job_no,i.job_id, i.lib_yarn_count_deter_id, c.amount, c.color_id as color_id from wo_booking_dtls b left join  wo_pre_cost_fabric_cost_dtls i on   b.job_no = i.job_no,com_pi_item_details c , com_pi_master_details d ,wo_po_details_master f,com_btb_lc_master_details g,com_btb_lc_pi h 	where b.booking_no=c.work_order_no and g.id=h.com_btb_lc_master_details_id and h.pi_id=d.id and b.job_no = f.job_no and d.id=c.pi_id and b.status_active=1 and b.is_deleted=0 	and c.status_active=1 and c.is_deleted=0 and g.item_basis_id in (1,2) and d.status_active=1 and d.is_deleted=0 and g.importer_id in (1)  $jobid_cond4 and d.entry_form=166	group by f.style_ref_no,d.pi_number,g.lc_number,f.job_no ,i.job_id, i.lib_yarn_count_deter_id, c.amount, c.color_id");*/
//F.STYLE_REF_NO,D.PI_NUMBER,F.JOB_NO,I.JOB_ID,C.DETERMINATION_ID, C.AMOUNT, C.COLOR_ID AS COLOR_ID
        $pi_number_data = sql_select("SELECT f.style_ref_no as STYLE_REF_NO,d.pi_number as PI_NUMBER,f.job_no as JOB_NO,i.job_id as JOB_ID,c.determination_id as DETER_ID, c.amount as AMOUNT, c.color_id as COLOR_ID from wo_booking_dtls b left join  wo_pre_cost_fabric_cost_dtls i on   b.job_no = i.job_no,com_pi_item_details c , com_pi_master_details d ,wo_po_details_master f,gbl_temp_engine g	where b.booking_no=c.work_order_no   and b.job_no = f.job_no and d.id=c.pi_id and f.JOB_ID=g.ref_val and i.JOB_ID=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in (2) and g.entry_form=2003 and b.status_active=1 and b.is_deleted=0 	and c.status_active=1 and c.is_deleted=0 and d.pi_basis_id in (1,2) and d.status_active=1 and d.is_deleted=0 and d.importer_id in ($company_name)  and d.entry_form=166	group by f.style_ref_no,c.determination_id,d.pi_number,f.job_no ,i.job_id, c.amount, c.color_id");

        $pi_data_arr = array();
        foreach ($pi_number_data as $key => $row) {
            $pi_data_arr[$row['JOB_ID']][$row['DETER_ID']][$row['COLOR_ID']]['pi_no'].= $row['PI_NUMBER'].',';
            $pi_data_arr[$row['JOB_ID']][$row['DETER_ID']][$row['COLOR_ID']]['amount'] += $row['AMOUNT'];
            //$pi_data_arr[$row[csf('job_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('color_id')]]['blc_no']=$row[csf('lc_number')];
        }
		unset($pi_number_data);
// F.STYLE_REF_NO,D.PI_NUMBER,G.LC_NUMBER,F.JOB_NO,I.JOB_ID, C.DETERMINATION_ID, C.AMOUNT, C.COLOR_ID AS COLOR_ID 
        $pi_number_data_lc = sql_select("SELECT f.style_ref_no as STYLE_REF_NO,d.pi_number as PI_NUMBER,g.lc_number as LC_NUMBER,f.job_no as JOB_NO,i.job_id as JOB_ID, c.determination_id as DETER_ID, c.amount as AMOUNT, c.color_id as COLOR_ID from wo_booking_dtls b left join  wo_pre_cost_fabric_cost_dtls i on   b.job_no = i.job_no,com_pi_item_details c , com_pi_master_details d ,wo_po_details_master f,com_btb_lc_master_details g,com_btb_lc_pi h,gbl_temp_engine g 	where b.booking_no=c.work_order_no and g.id=h.com_btb_lc_master_details_id and h.pi_id=d.id and b.job_no = f.job_no and d.id=c.pi_id and f.JOB_ID=g.ref_val and i.JOB_ID=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in (2) and g.entry_form=2003 and b.status_active=1 and b.is_deleted=0 	and c.status_active=1 and c.is_deleted=0 and g.item_basis_id in (1,2) and d.status_active=1 and d.is_deleted=0 and g.importer_id in ($company_name)  $jobid_cond4 and d.entry_form=167");
	

        foreach ($pi_number_data_lc as $key => $row) {

            $pi_data_arr[$row['JOB_ID']][$row['DETER_ID']][$row['COLOR_ID']]['blc_no'].= $row['LC_NUMBER'].',';
        }
        unset($pi_number_data_lc);
        ...
        /*echo '<pre>';
        print_r($pi_data_arr); die;*/
		// MAX(B.PUB_SHIPMENT_DATE) AS PUB_SHIPMENT_DATE ,JOB_ID
        $max_shipment_date_sql = sql_select("SELECT MAX(b.pub_shipment_date) as PUB_SHIPMENT_DATE ,job_id as JOB_ID from wo_po_break_down b,gbl_temp_engine g where   b.JOB_ID=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in (2) and g.entry_form=2003  and b.status_active=1 and b.is_deleted=0  group by b.job_id");
        foreach ($max_shipment_date_sql as $row) {
            $max_ship_arr[$row['JOB_ID']] = $row['PUB_SHIPMENT_DATE'];
        }
    ...
    if(count($trims_sql_data) > 1) {
      ...
      /*  $trimjob_id_chunk = array_chunk($trimjob_id_arr, 1000, true);
       ...
       /* $trim_id_chunk = array_chunk($trim_id_arr, 1000, true);
		...
		//A.ID AS TRIM_ID, A.JOB_ID, B.ID AS BOOKING_ID, B.BOOKING_DATE, B.BOOKING_NO, C.WO_QNTY,C.AMOUNT,B.SUPPLIER_ID 
        $trim_wo_data_sql = sql_select("SELECT a.id as TRIM_ID, a.job_id as JOB_ID, b.id as BOOKING_ID, b.booking_date as BOOKING_DATE, b.booking_no as BOOKING_NO, c.wo_qnty as WO_QNTY,c.amount as AMOUNT,b.supplier_id as SUPPLIER_ID FROM wo_pre_cost_trim_cost_dtls a , wo_booking_dtls c , wo_booking_mst b,gbl_temp_engine g where   a.id=c.pre_cost_fabric_cost_dtls_id and  b.booking_no=c.booking_no and  b.JOB_ID=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in (2) and g.entry_form=2003  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and c.booking_type=2  ");
        foreach ($trim_wo_data_sql as $row) {
            $trim_wo_data_arr[$row['JOB_ID']][$row['TRIM_ID']]['wo_qnty'] += $row['WO_QNTY'];
            $trim_wo_data_arr[$row['JOB_ID']][$row['TRIM_ID']]['wo_amount'] += $row['AMOUNT'];
            $trim_wo_data_arr[$row['JOB_ID']][$row['TRIM_ID']]['wo_date'][$row['BOOKING_ID']] = $row['BOOKING_DATE'];
            $trim_wo_data_arr[$row['JOB_ID']][$row['TRIM_ID']]['booking_no'][$row['BOOKING_ID']] = $row['BOOKING_NO'];
            $trim_wo_data_arr[$row['JOB_ID']][$row['TRIM_ID']]['supplier'][$row['BOOKING_ID']] = $lib_supplier_arr[$row['SUPPLIER_ID']];
        }
...
//B.PO_BREAKDOWN_ID, A.ITEM_GROUP_ID,A.ITEM_DESCRIPTION, SUM(B.QUANTITY) AS QUANTITY, A.RATE, E.JOB_ID
        $receive_qty_data = sql_select("SELECT b.po_breakdown_id as POID, a.item_group_id as ITEM_GROUP_ID,a.item_description as ITEM_DESCRIPTION, (b.quantity) as QUANTITY, a.rate as RATE, e.job_id as JOB_ID from inv_receive_master c,product_details_master d,inv_trims_entry_dtls a , order_wise_pro_details b, wo_po_break_down e,gbl_temp_engine g where a.mst_id=c.id and a.trans_id=b.trans_id and a.id=b.dtls_id and a.prod_id=d.id  and b.po_breakdown_id=g.ref_val and e.id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in (1) and g.entry_form=2003 and b.trans_type=1 and b.entry_form=24 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and c.status_active=1 and e.id=b.po_breakdown_id  ");
	//	echo "SELECT b.po_breakdown_id, a.item_group_id,a.item_description, sum(b.quantity) as quantity, a.rate, e.job_id from inv_receive_master c,product_details_master d,inv_trims_entry_dtls a , order_wise_pro_details b, wo_po_break_down e where a.mst_id=c.id and a.trans_id=b.trans_id and a.id=b.dtls_id and a.prod_id=d.id and b.trans_type=1 and b.entry_form=24 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and c.status_active=1 and e.id=b.po_breakdown_id $order_cond group by b.po_breakdown_id, a.item_group_id,a.item_description,a.rate, e.job_id order by a.item_group_id ";
		
        $trim_inhouse_qty = array();
        foreach ($receive_qty_data as $row) { 
            $trim_inhouse_qty[$row['JOB_ID']][$row['ITEM_GROUP_ID']][$row['ITEM_DESCRIPTION']]['qty'] += $row['QUANTITY'];
            $trim_inhouse_qty[$row['JOB_ID']][$row['ITEM_GROUP_ID']][$row['ITEM_DESCRIPTION']]['amount'] += $row['quantity'] * $row['RATE'];
            $trim_inhouse_qty[$row['JOB_ID']][$row['ITEM_GROUP_ID']][$row['ITEM_DESCRIPTION']]['rate'] = $row['RATE'];
            $trim_inhouse_qty[$row['JOB_ID']][$row['ITEM_GROUP_ID']][$row['ITEM_DESCRIPTION']]['po_id'] = $row['POID'];
            $trims_po_id_arr[$row['JOB_ID']][$row['ITEM_GROUP_ID']][$row['POID']] = $row['POID'];
        }
//B.PO_BREAKDOWN_ID, A.ITEM_GROUP_ID,P.ITEM_DESCRIPTION AS ITEM_DESCRIPTION, (B.QUANTITY) AS QUANTITY, A.RATE, E.JOB_ID
        $trim_issue_qty_data = sql_select("SELECT b.po_breakdown_id as POID, a.item_group_id as ITEM_GROUP_ID,p.item_description as ITEM_DESCRIPTION, (b.quantity) as QUANTITY, a.RATE, e.job_id  as JOB_ID from  inv_issue_master d,product_details_master p,inv_trims_issue_dtls a , order_wise_pro_details b, wo_po_break_down e,gbl_temp_engine g where a.mst_id=d.id and a.prod_id=p.id and a.trans_id=b.trans_id  and b.po_breakdown_id=g.ref_val and e.id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in (1) and g.entry_form=2003 and b.trans_type=2 and b.entry_form=25 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and e.id=b.po_breakdown_id ");
        ...
        $trim_issue_qty = array();
        foreach ($trim_issue_qty_data as $row) {
            $rate = $trim_inhouse_qty[$row['JOB_ID']][$row['ITEM_GROUP_ID']][$row['ITEM_DESCRIPTION']]['rate'];
            $trim_issue_qty[$row['JOB_ID']][$row['ITEM_GROUP_ID']][$row['ITEM_DESCRIPTION']]['qty'] += $row['QUANTITY'];
            $trim_issue_qty[$row['JOB_ID']][$row['ITEM_GROUP_ID']][$row['ITEM_DESCRIPTION']]['amount'] += $row['QUANTITY'] * $rate;
            $trim_issue_qty[$row['JOB_ID']][$row['ITEM_GROUP_ID']][$row['ITEM_DESCRIPTION']]['po_id'] = $row['POID'];
        }

        $trim_supplier_data = sql_select("SELECT a.job_id as JOB_ID, a.supplier_id as SUP_ID, b.id as TRIM_COSTID from wo_pre_cost_trim_supplier a, wo_pre_cost_trim_cost_dtls b,gbl_temp_engine g  where   a.job_id=b.job_id and b.id=a.trimid and  b.JOB_ID=g.ref_val and  a.JOB_ID=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in (2) and g.entry_form=2003  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $jobid_cond");

        $fabric_supplier_arr = array();
        foreach ($fabric_supplier_data as $row) {
            $trim_supplier_arr[$row['JOB_ID']][$row['TRIM_COSTID']][$row['SUP_ID']] = $lib_supplier_arr[$row['SUP_ID']];
        }
//A.ID, A.LC_NUMBER,A.LC_TYPE_ID,A.PAYTERM_ID,A.TENOR, C.PI_NUMBER
		...
		FROM com_btb_lc_master_details a, com_btb_lc_pi b, com_pi_master_details c WHERE a.id=b.com_btb_lc_master_details_id and b.pi_id=c.id and a.is_deleted = 0 
		 and a.item_basis_id=1 and a.importer_id=$company_name group by a.id, a.lc_number,a.lc_type_id,a.payterm_id,a.tenor,c.pi_number
		  UNION ALL 
		 ...
		 group by a.id, a.lc_number,a.lc_type_id,a.payterm_id,a.tenor, c.pi_number order by id desc ");
		 
			$trim_pi_lc_data_arr = array();$fab_pi_lc_data_arr = array();
			foreach ($trims_pi_data as $key => $row) {
				$trim_pi_lc_data_arr[$row['PI_NUMBER']]['lc_number']= $row['LC_NUMBER'];
				$fab_pi_lc_data_arr[$row['PI_NUMBER']]['lc_number']= $row['LC_NUMBER'];
			}
...
//A.JOB_ID,D.PI_NUMBER, C.AMOUNT, E.LC_NUMBER, C.ITEM_GROUP, B.PRE_COST_FABRIC_COST_DTLS_ID, C.ITEM_COLOR, C.ITEM_SIZE
        $trims_pi_number_data = sql_select("SELECT a.job_id as JOB_ID,d.pi_number as PI_NUMBER, c.amount as AMOUNT, e.lc_number as LC_NUMBER, c.item_group as ITEM_GROUP, b.pre_cost_fabric_cost_dtls_id as FAB_COSTID, c.item_color, c.item_size from wo_pre_cost_trim_cost_dtls a , wo_booking_dtls b , com_pi_item_details c , com_pi_master_details d  ,  com_btb_lc_master_details e ,gbl_temp_engine g  where a.id=b.pre_cost_fabric_cost_dtls_id  and  b.booking_no=c.work_order_no and  d.id=c.pi_id  and TO_CHAR(c.pi_id)=e.pi_id and  a.JOB_ID=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in (2) and g.entry_form=2003  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and d.entry_form=167  group by a.job_id, d.pi_number, c.amount, e.lc_number, c.item_group, b.pre_cost_fabric_cost_dtls_id, c.item_color, c.item_size");
 ...
 //    echo "SELECT a.job_id,d.pi_number, c.amount, e.lc_number, c.item_group, c.item_color, c.item_size from wo_pre_cost_trim_cost_dtls a join wo_booking_dtls b on a.job_no=b.job_no join com_pi_item_details c on b.booking_no=c.work_order_no  join com_pi_master_details d on d.id=c.pi_id left join com_btb_lc_master_details e on TO_CHAR(c.pi_id)=e.pi_id where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and d.entry_form=167 $jobid_cond1 group by a.job_id, d.pi_number, c.amount, e.lc_number, c.item_group, c.item_color, c.item_size";
        $trim_pi_data_arr = array();
        foreach ($trims_pi_number_data as $key => $row) {
            $trim_pi_data_arr[$row['FAB_COSTID']][$row['JOB_ID']][$row['ITEM_GROUP']]['pi_no'].= $row['PI_NUMBER'].',';
            $trim_pi_data_arr[$row['FAB_COSTID']][$row['JOB_ID']][$row['ITEM_GROUP']]['amount'] += $row['AMOUNT'];
            $trim_pi_data_arr[$row['FAB_COSTID']][$row['JOB_ID']][$row['ITEM_GROUP']]['blc_no'].= $row['LC_NUMBER'].',';
        }
	...
	$con = connect();
	execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ENTRY_FORM=2003");
	oci_commit($con);
	disconnect($con);
				...
				foreach ($main_data_arr as $job_id => $job_data) {
					...
					foreach ($job_data['color_data'] as $lib_yarn_id=>$color_data) {
						...
						foreach ($color_data['fabric_color'] as $fcolor_id=>$fcolor_data) {
							...
							$gmts_color=array();
							$gmts_color_id=array();
							$bom_qty=0;	$wo_qty=0; $bom_value=0; $wo_amount=0; $pi_amount=0;$issueqty=$inhouseqty=0;
							foreach ($fcolor_data as $gcolor_id => $row) {
								$fabric_color_id=$row['color'];
								$color_id = $gcolor_id;
								$gmts_color[$gcolor_id]=$color_arr[$gcolor_id];
				...
				<?php foreach ($trims_main_data as $job_id=>$value){
				 ...
				 ?>
					...
					<a href='#report_details' onClick="order_summery_popup('<? echo $job_id; ?>' ,'order_summery_data');"><?= $value['style_ref_no']; ?></a></td>
						...
						<td width="50" align="left"><?= implode(",", $trim_wo_data_arr[$job_id][$trims_id]['booking_no'])?></td>
		                <td width="50" align="right"><?= $unit_of_measurement[$row['cons_uom']]; ?></td>	                
		                <td width="80" align="right" title="Des=<? echo $row['description'];?>"><a href='#report_details' onClick="openmypage_trim_inhouse('<? echo $trims_poid;  ?>','<? echo $row['trim_group'].'__'.$row['description']; ?>','trim_booking_inhouse_info');"><?= fn_number_format($inhouse_qty,2);  ?></a></td>
		                <td width="80" align="right"><?= fn_number_format($trimrcv_balance,2); ?></td>
		                <td width="70" align="right"><a href='#report_details' onClick="openmypage_trim_issue('<? echo $trims_poid; ?>','<? echo $row['trim_group'].'__'.$row['description']; ?>','trim_booking_issue_info');"><?= fn_number_format($issue_qty,2);  ?></a></td>
						<td width="90" align="right"><?= fn_number_format($inhouse_qty-$issue_qty,2); ?></td>
...
if($action=="report_generate3") // copy show/md mamun ahmed sagor/crm-24699/26-10-2022
{
	$process = array( &$_POST );
	extract(check_magic_quote_gpc( $process ));
    ...
    if(count($trims_sql_data) > 1) {
        $trims_main_attribute = array('job_no', 'buyer_name', 'job_no_prefix_num', 'style_ref_no', 'season_buyer_wise', 'order_uom', 'job_quantity');
        $trims_dtls_attribute = array('trim_dtla_id', 'trim_group', 'description', 'brand_sup_ref', 'cons_uom', 'cons_dzn_gmts', 'po_number', 'sourcing_rate','rate');
        foreach ($trims_sql_data as $row) {
	...
	//	echo "SELECT b.po_breakdown_id, a.item_group_id,a.item_description, sum(b.quantity) as quantity, a.rate, e.job_id from inv_receive_master c,product_details_master d,inv_trims_entry_dtls a , order_wise_pro_details b, wo_po_break_down e where a.mst_id=c.id and a.trans_id=b.trans_id and a.id=b.dtls_id and a.prod_id=d.id and b.trans_type=1 and b.entry_form=24 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and c.status_active=1 and e.id=b.po_breakdown_id $order_cond group by b.po_breakdown_id, a.item_group_id,a.item_description,a.rate, e.job_id order by a.item_group_id ";
		...
		$trims_pi_data=sql_select("SELECT a.id, a.lc_number,a.lc_type_id,a.payterm_id,a.tenor, c.pi_number 
		FROM com_btb_lc_master_details a, com_btb_lc_pi b, com_pi_master_details c WHERE a.id=b.com_btb_lc_master_details_id and b.pi_id=c.id and a.is_deleted = 0 
		 and a.item_basis_id=1 and a.importer_id=$company_name group by a.id, a.lc_number,a.lc_type_id,a.payterm_id,a.tenor,c.pi_number UNION ALL SELECT a.id, a.lc_number,a.lc_type_id,a.payterm_id,a.tenor,c.pi_number
		  FROM com_btb_lc_master_details a, com_btb_lc_pi b, com_pi_master_details c
		 WHERE a.id=b.com_btb_lc_master_details_id and b.pi_id=c.id and a.is_deleted = 0 and a.item_basis_id=2 and a.importer_id=$company_name  
		 group by a.id, a.lc_number,a.lc_type_id,a.payterm_id,a.tenor, c.pi_number order by id desc ");
			$trim_pi_lc_data_arr = array();$fab_pi_lc_data_arr = array();
...
if($action=="order_wo_qty_data")
{
	...
	$wo_data_sql=sql_select("SELECT a.id as fabric_id, a.fabric_description, a.body_part_id, a.uom,  b.booking_date, c.booking_no,c.rate, c.fin_fab_qnty,b.supplier_id FROM wo_pre_cost_fabric_cost_dtls a join wo_booking_dtls c on a.id=c.pre_cost_fabric_cost_dtls_id join wo_booking_mst b on b.booking_no=c.booking_no where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.lib_yarn_count_deter_id=$yarn_id and a.job_id=$job_id and c.fin_fab_qnty is not null and c.fabric_color_id=$fcolor_id and c.gmts_color_id in ($color_id)");
	$i=1;
	?>
                ...
                <tbody>
                	<? foreach ($wo_data_sql as $row) {
            			...
            			<tr>
            				...
            				<td align="right"><? $total_po+= $row[csf('fin_fab_qnty')]; echo fn_number_format($row[csf('fin_fab_qnty')],2) ?></td>
							<td><?= $row[csf('rate')]?></td>
			...
			<table border="1" class="rpt_table" rules="all" width="850" cellpadding="0" cellspacing="0" align="center">
                ...
                <?
				 ...
				 //echo "SELECT b.id,b.po_number,a.style_ref_no from wo_po_break_down b,wo_po_details_master a where a.id=b.job_id and  b.job_id=$job_id and b.status_active=1";
					...
					/*$receive_qty_data=sql_select("SELECT a.recv_number, d.id as prod_id, a.receive_date, a.challan_no, a.supplier_id, b.body_part_id, a.inserted_by, d.detarmination_id, d.color, d.product_name_details,c.quantity as order_qnty,b.order_rate, b.order_amount,b.cons_uom, a.booking_no 	from inv_receive_master a join inv_transaction b on a.id = b.mst_id join order_wise_pro_details c on b.id = c.trans_id join product_details_master d on d.id=c.prod_id where a.entry_form=17 and a.receive_basis=1 and a.status_active=1 and a.is_deleted=0 and b.receive_basis=1  and b.transaction_type=1 and b.status_active=1 and b.is_deleted=0 and c.trans_type=1 and c.entry_form=17 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and c.po_breakdown_id in(".implode(",",$poIdArr).") and d.detarmination_id=$yarn_id and d.color=$color  group by a.recv_number, d.id , a.receive_date, a.challan_no, a.supplier_id, b.body_part_id, a.inserted_by, d.detarmination_id, d.color, d.product_name_details,c.quantity,b.order_rate, b.order_amount, b.cons_uom, a.booking_no ");*/
					$receive_qty_data=sql_select("SELECT a.recv_number, d.id as prod_id, a.receive_date, a.challan_no, a.supplier_id, b.body_part_id, a.inserted_by, d.detarmination_id, d.color, d.product_name_details,sum(c.quantity) as order_qnty,sum(c.quantity*b.order_rate) as order_amount,b.cons_uom, a.booking_no 	from inv_receive_master a join inv_transaction b on a.id = b.mst_id join order_wise_pro_details c on b.id = c.trans_id join product_details_master d on d.id=c.prod_id where a.entry_form=17 and a.receive_basis in (1,2) and a.status_active=1 and a.is_deleted=0 and b.receive_basis in (1,2)  and b.transaction_type=1 and b.status_active=1 and b.is_deleted=0 and c.trans_type=1 and c.entry_form=17 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and c.po_breakdown_id in(".implode(",",$poIdArr).") and d.detarmination_id=$yarn_id and d.color=$color group by a.recv_number, d.id , a.receive_date, a.challan_no, a.supplier_id, b.body_part_id, a.inserted_by, d.detarmination_id, d.color, d.product_name_details,b.cons_uom, a.booking_no ");
		...
		<div id="scroll_body" align="center">
			<table border="1" class="rpt_table" rules="all" width="850" cellpadding="0" cellspacing="0" align="center">
                ...
                <?
					...
					$i=1;
					if($item_desc=='') $item_desc_cond="";else $item_desc_cond="and  b.item_description='$item_desc'";
		...
		<div id="scroll_body" align="center">
			<table border="1" class="rpt_table" rules="all" width="650" cellpadding="0" cellspacing="0" align="left">
                ...
                <?
					...
					$supplier_arr=return_library_array( "select id, supplier_name from lib_supplier", "id", "supplier_name"  );
					$i=1;
					//and b.item_description='$item_desc'
					//if($item_desc!="") $item_desc_cond="and b.item_description='$item_desc'";else $item_desc_cond="";
				 ...
				 $mrr_sql=("SELECT a.id, a.issue_number,a.challan_no,b.prod_id,p.item_group_id, a.issue_date,p.item_description,SUM(c.quantity) as quantity
					...
					b.status_active=1 and b.is_deleted=0 and  c.po_breakdown_id in($po_id) and p.item_group_id='$item_name' $item_desc_cond  group by c.po_breakdown_id,p.item_group_id,p.item_description,a.issue_number,a.id,a.issue_date,b.prod_id,a.challan_no ");
