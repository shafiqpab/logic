...
if($action=="report_generate2")
{
    ...
    if(count($main_data_sql) > 0) {
        ...
        foreach ($receive_qty_data as $row) {
            ...
            $receive_qty_arr[$row[csf('job_id')]][$row[csf('detarmination_id')]][$row[csf('color')]]['amount'] +=$row[csf('order_qnty')]*$row[csf('order_rate')];
				...
				<?php foreach ($trims_main_data as $job_id=>$value){
				 ...
				 ?>
					...
					<a href='#report_details' onClick="order_summery_popup('<? echo $job_id; ?>' ,'order_summery_data');"><?= $value['style_ref_no']; ?></a></td>
		                ...
		                <td align="right"><?= fn_number_format($wo_amount,2);  ?></td>
		                <td width="80" align="right"><a href='#report_details' onClick="openmypage_trim_inhouse('<? echo $trims_poid; ?>','<? echo $row['trim_group']; ?>','trim_booking_inhouse_info');"><?= fn_number_format($inhouse_qty,2);  ?></a></td>
		                <td align="right"><?= fn_number_format($inhouse_amount,2); ?></td>
		                <td width="80" align="right"><?= fn_number_format($trimrcv_balance,2); ?></td>
		                <td width="70" align="right"><a href='#report_details' onClick="openmypage_trim_issue('<? echo $trims_poid; ?>','<? echo $row['trim_group']; ?>','trim_booking_issue_info');"><?= fn_number_format($issue_qty,2);  ?></a></td>
						...
						<td width="140" align="left"><p><?=$trim_blc_nos;?></p></td>
						<? $z++;
				...
				foreach ($main_data_arr as $job_id => $job_data) {
					...
					foreach ($job_data['color_data'] as $lib_yarn_id=>$color_data) {
						...
						foreach ($color_data['fabric_color'] as $fcolor_id=>$fcolor_data) {
							...
							<td align="left"><?= implode(",",$wo_data_arr[$job_id][$lib_yarn_id][$color_id][$fabric_color_id]['wo_date'] )  ?></td>
							<td align="right"><?= fn_number_format($row['avg_cons'],4) ?></td>
							<td align="right" title="Avg Rate"><?= fn_number_format($bom_value/$bom_qty,4) ?></td>
							<td align="right"><a href='#report_details' onClick="order_req_qty_popup('<? echo $job_id; ?>','<? echo $lib_yarn_id; ?>','<? echo $fabric_color_id; ?>' ,'order_req_qty_data');"><?= fn_number_format($bom_qty,2)?></a></td>
							<td align="right"><a href='#report_details' onClick="order_wo_qty_popup('<? echo $job_id; ?>','<? echo $lib_yarn_id; ?>','<? echo $fabric_color_id; ?>','<? echo implode(",", $gmts_color_id) ?>' ,'order_wo_qty_data');"><?= fn_number_format($wo_qty,2)  ?></a></td>
				...
				<?php foreach ($trims_main_data as $job_id=>$value){
				 ...
				 ?>
					...
					<a href='#report_details' onClick="order_summery_popup('<? echo $job_id; ?>' ,'order_summery_data');"><?= $value['style_ref_no']; ?></a></td>
						...
						<td width="90" align="left"><p>&nbsp;<?=$trim_blc_nos;?></p></td>
						<? $z++;
						 } ?>
					</tr>
				<?php 
		...
		<div id="scroll_body" align="center">
			<table border="1" class="rpt_table" rules="all" width="850" cellpadding="0" cellspacing="0" align="center">
                ...
                <?
					...
					$receive_qty_data=sql_select("SELECT a.recv_number, d.id as prod_id, a.receive_date, a.challan_no, a.supplier_id, b.body_part_id, a.inserted_by, d.detarmination_id, d.color, d.product_name_details,c.quantity as order_qnty,b.order_rate, b.order_amount, e.job_id, b.cons_uom, a.booking_no, f.style_ref_no 	from inv_receive_master a join inv_transaction b on a.id = b.mst_id join order_wise_pro_details c on b.id = c.trans_id join product_details_master d on d.id=c.prod_id join wo_po_break_down e on e.id=c.po_breakdown_id join wo_po_details_master f on e.job_id=f.id where a.entry_form=17 and a.receive_basis=1 and a.status_active=1 and a.is_deleted=0 and b.receive_basis=1  and b.transaction_type=1 and b.status_active=1 and b.is_deleted=0 and c.trans_type=1 and c.entry_form=17 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.job_id=$job_id and d.detarmination_id=$yarn_id and d.color=$color  group by a.recv_number, d.id , a.receive_date, a.challan_no, a.supplier_id, b.body_part_id, a.inserted_by, d.detarmination_id, d.color, d.product_name_details,c.quantity,b.order_rate, b.order_amount, e.job_id,  b.cons_uom, a.booking_no, f.style_ref_no ");
					...
					foreach($receive_qty_data as $row)
					{
						...
						<tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
                            ...
                            <td><? echo number_format($qty*$row[csf('order_rate')],2); ?></td>
