...
if($action=="report_generate2")
{
    ...
    if(count($trims_sql_data) > 1) {
        ...
        b.supplier_id,c.id as booking_dtls_id,d.description FROM wo_pre_cost_trim_cost_dtls a join wo_booking_dtls c on a.id=c.pre_cost_fabric_cost_dtls_id join wo_trim_book_con_dtls d on d.wo_trim_booking_dtls_id=c.id join wo_booking_mst b on b.booking_no=d.booking_no  where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and c.booking_type=2 $trimid_cond ");*/
...
if($action=="report_generate3") // copy show/md mamun ahmed sagor/crm-24699/26-10-2022
{
    ...
    if(count($main_data_sql) > 0) {
      ...
      /*  $issue_qty_data = sql_select("SELECT d.detarmination_id, d.color, c.quantity as cons_quantity, b.cons_amount, e.job_id, e.id as po_id,d.supplier_id from inv_issue_master a join inv_transaction b on a.id = b.mst_id join order_wise_pro_details c on b.id = c.trans_id join product_details_master d on d.id=c.prod_id join wo_po_break_down e on e.id=c.po_breakdown_id where a.entry_form=19 and a.issue_basis=1 and a.status_active=1 and a.is_deleted=0 and b.transaction_type=2 and b.status_active=1 and b.is_deleted=0 and c.trans_type=2 and c.entry_form=19 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 $order_cond");*/
					...
					foreach ($job_data['color_data'] as $lib_yarn_id=>$color_data) {
						...
						foreach ($color_data['fabric_color'] as $fcolor_id=>$fcolor_data) {
							...
							foreach ($fcolor_data as $gcolor_id => $row) {
								...
								{
								$issueqty+= $issue_qty_arr[$job_id][$pid][$lib_yarn_id][$fabric_color_id]['qty'];
				...
				<?php foreach ($trims_main_data as $job_id=>$value){
				 ...
				 ?>
					...
					<a href='#report_details' onClick="order_summery_popup('<? echo $job_id; ?>' ,'order_summery_data');"><?= $value['style_ref_no']; ?></a></td>
						...
						<td width="50" align="left"><?= implode(",", $trim_wo_data_arr[$job_id][$trims_id]['booking_no'])?></td>
		                <td width="50" align="right"><?= $unit_of_measurement[$row['cons_uom']]; ?></td>	                
		                <td width="80" align="right" title="Des=<? echo $descriptions;?>"><a href='#report_details' onClick="openmypage_trim_inhouse('<? echo $trims_poid;  ?>','<? echo $row['trim_group'].'__'.$bDesc; ?>','trim_booking_inhouse_info');"><?= fn_number_format($inhouse_qty,2);  ?></a></td>
		                <td width="80" align="right"><?= fn_number_format($trimrcv_balance,2); ?></td>
		                <td width="70" align="right"><a href='#report_details' onClick="openmypage_trim_issue('<? echo $trims_poid; ?>','<? echo $row['trim_group'].'__'.$descriptions; ?>','trim_booking_issue_info');"><?= fn_number_format($issue_qty,2);  ?></a></td>
						<td width="90" align="right"><?= fn_number_format($inhouse_qty-$issue_qty,2); ?></td>
		                <td width="90" align="left"><?=implode(",", $trim_wo_data_arr[$job_id][$trims_id]['supplier'])?></td>
...
if($action=="report_generate4")
{
	...
	b.supplier_id FROM wo_pre_cost_trim_cost_dtls a ,wo_booking_dtls c , wo_booking_mst b  where   b.booking_no=c.booking_no and a.id=c.pre_cost_fabric_cost_dtls_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and c.booking_type=2 $trimid_cond ");*/
	 $trim_wo_data_sql = sql_select("SELECT a.id as trim_id, a.job_id, b.id as booking_id, b.booking_date, b.booking_no, d.requirment as wo_qnty,d.amount, d.description,
		...
		$trim_trnsf_in_qty_data = sql_select("SELECT b.po_breakdown_id,p.item_description as item_description, (b.quantity) as quantity, a.rate, e.job_id from inv_item_transfer_mst d,inv_item_transfer_dtls a,inv_transaction f, order_wise_pro_details b,product_details_master p, wo_po_break_down e where d.id=a.mst_id and a.mst_id=f.mst_id and f.id=b.trans_id and a.id=b.dtls_id and b.prod_id=p.id and f.prod_id=p.id and a.to_prod_id=p.id and b.po_breakdown_id=e.id and a.to_order_id=b.po_breakdown_id and b.trans_type=5 and b.entry_form=112 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and f.item_category=4 $order_cond $order_cond2");
		$trim_transf_in_qty = array();
        foreach ($trim_trnsf_in_qty_data as $row) {
        	$trim_transf_in_qty[$row[csf('job_id')]][$row[csf('item_description')]]['qty'] += $row[csf('quantity')];
        }
        unset($trim_trnsf_in_qty_data);


		$trim_trnsf_out_qty_data = sql_select("SELECT b.po_breakdown_id,p.item_description as item_description, (b.quantity) as quantity, a.rate, e.job_id from inv_item_transfer_mst d,inv_item_transfer_dtls a,inv_transaction f, order_wise_pro_details b,product_details_master p, wo_po_break_down e where d.id=a.mst_id and a.mst_id=f.mst_id and f.id=b.trans_id and a.id=b.dtls_id and b.prod_id=p.id and f.prod_id=p.id and a.to_prod_id=p.id and b.po_breakdown_id=e.id and a.to_order_id=b.po_breakdown_id  and b.trans_type=6 and b.entry_form=112 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and f.item_category=4 $order_cond $order_cond3");
		$trim_transf_out_qty = array();
        foreach ($trim_trnsf_out_qty_data as $row) {
        	$trim_transf_out_qty[$row[csf('job_id')]][$row[csf('item_description')]]['qty'] += $row[csf('quantity')];
        }
				...
				foreach ($main_data_arr as $job_id => $job_data) {
					...
					foreach ($job_data['color_data'] as $lib_yarn_id=>$color_data) {
						...
						foreach ($color_data['fabric_color'] as $fcolor_id=>$fcolor_data) {
							...
							$gmts_color=array();
							$gmts_color_id=array();
							$bom_qty=0;	$wo_qty=0; $bom_value=0; $wo_amount=0; $pi_amount=0;$issueqty=$inhouseqty=$issue_rtn_qty=$recv_rtn_qty=$transIn_qnty=$transOut_qnty=0;
							foreach ($fcolor_data as $gcolor_id => $row) {
								$fabric_color_id=$row['color'];
								$color_id = $gcolor_id;
								$gmts_color[$gcolor_id]=$color_arr[$gcolor_id];
								...
								$transOut_qnty+= $transOut_qty_arr[$job_id][$row['po_id']][$lib_yarn_id][$fabric_color_id]['qty'];
							...
							//$inhouseqty= $receive_qty_arr[$job_id][$lib_yarn_id][$fcolor_id]['qty'];
								//echo $inhouseqty.',';
							//$issue_balance=$inhouseqty-$issueqty;
							$issue_balance=($transIn_qnty+$inhouseqty+$issue_rtn_qty)-($transOut_qnty+$recv_rtn_qty+$issueqty);
							...
							<td align="right"><a href='#report_details' onClick="openmypage_inhouse('<? echo $job_id; ?>','<? echo $lib_yarn_id; ?>','<? echo $transIn_po ; ?>','<? echo $fabric_color_id ?>' ,'booking_inhouse_info_trans_in');"><?= fn_number_format($transIn_qnty,2) ?></a></td>
							...
							<td align="right"><? if($issueqty>0){ ?><a href='#report_details' onClick="openmypage_issue('<? echo $job_id; ?>','<? echo $lib_yarn_id; ?>','<? echo $transOut_po; ?>','<? echo $fabric_color_id?>', '<?echo $rcv_rate?>' ,'booking_inhouse_info_trans_out');"><?= fn_number_format($transOut_qnty,2)  ?></a><? } else echo '0.00'; ?></td>
							<td align="right"><? echo fn_number_format($transOut_qnty+$recv_rtn_qty+$issueqty,2); ?></td>
					...
					<a href='#report_details' onClick="order_summery_popup('<? echo $job_id; ?>' ,'order_summery_data');"><?= $value['style_ref_no']; ?></a></td>
						...
						foreach ($value['trims_data'] as $trims_id=>$row) {
							...
							foreach($descriptionsArr as $desc)
							{
								...
								$trims_transf_out_qty+= $trim_transf_out_qty[$job_id][$desc]['qty'];
								$trims_transf_in_qty+= $trim_transf_in_qty[$job_id][$desc]['qty'];

							}

							$total_trims_recvQnty=$inhouse_qty+$trims_transf_in_qty;
		                ...
		                <td width="80" align="right" title="Des=<? echo $descriptions;?>"><a href='#report_details' onClick="openmypage_trim_inhouse('<? echo $trims_poid;  ?>','<? echo $row['trim_group'].'__'.$row['description']; ?>','trim_booking_inhouse_info');"><?= fn_number_format($inhouse_qty,2);  ?></a></td>
		                <td width="80" align="right" title="Des=<? echo $descriptions;?>"><a href='#report_details' onClick="openmypage_trim_inhouse('<? echo $trims_poid;  ?>','<? echo $row['trim_group'].'__'.$row['description']; ?>','trim_booking_inhouse_info');"><? ?></a></td>
		                <td width="80" align="right" title="Des=<? echo $descriptions;?>"><a href='#report_details' onClick="openmypage_trim_inhouse('<? echo $trims_poid;  ?>','<? echo $row['trim_group'].'__'.$row['description']; ?>','trim_booking_inhouse_info');"><?= fn_number_format($trims_transf_in_qty,2);  ?></a></td>
		                <td width="80" align="right" title="Des=<? echo $descriptions;?>"><a href='#report_details' onClick="openmypage_trim_inhouse('<? echo $trims_poid;  ?>','<? echo $row['trim_group'].'__'.$row['description']; ?>','trim_booking_inhouse_info');"><?= fn_number_format($total_trims_recvQnty,2);  ?></a></td>

		                <td width="80" align="right"><?= fn_number_format($trimrcv_balance,2); ?></td>
		                
		                <td width="70" align="right"><a href='#report_details' onClick="openmypage_trim_issue('<? echo $trims_poid; ?>','<? echo $row['trim_group'].'__'.$descriptions; ?>','trim_booking_issue_info');"><?= fn_number_format($issue_qty,2);  ?></a></td>
		                <td width="70" align="right"><a href='#report_details' onClick="openmypage_trim_issue('<? echo $trims_poid; ?>','<? echo $row['trim_group'].'__'.$descriptions; ?>','trim_booking_issue_info');"><?= fn_number_format($trims_recv_rtn_qty,2);  ?></a></td>
		                <td width="70" align="right"><a href='#report_details' onClick="openmypage_trim_issue('<? echo $trims_poid; ?>','<? echo $row['trim_group'].'__'.$descriptions; ?>','trim_booking_issue_info');"><?= fn_number_format($trims_transf_out_qty,2);  ?></a></td>
		                <td width="70" align="right"><a href='#report_details' onClick="openmypage_trim_issue('<? echo $trims_poid; ?>','<? echo $row['trim_group'].'__'.$descriptions; ?>','trim_booking_issue_info');"><?= fn_number_format($total_trims_issueQnty,2);  ?></a></td>
			...
			<table border="1" class="rpt_table" rules="all" width="850" cellpadding="0" cellspacing="0" align="center">
                ...
                <?
				 ...
				 //echo "SELECT b.id,b.po_number,a.style_ref_no from wo_po_break_down b,wo_po_details_master a where a.id=b.job_id and  b.job_id=$job_id and b.status_active=1";
					...
					/*echo "SELECT a.recv_number, d.id as prod_id, a.receive_date, a.challan_no, a.supplier_id, b.body_part_id, a.inserted_by, d.detarmination_id, d.color, d.product_name_details,sum(c.quantity) as order_qnty,sum(c.quantity*b.order_rate) as order_amount,b.cons_uom, a.booking_no 	from inv_receive_master a join inv_transaction b on a.id = b.mst_id join order_wise_pro_details c on b.id = c.trans_id join product_details_master d on d.id=c.prod_id where a.entry_form=17 and a.receive_basis in (1,2) and a.status_active=1 and a.is_deleted=0 and b.receive_basis in (1,2)  and b.transaction_type=1 and b.status_active=1 and b.is_deleted=0 and c.trans_type=1 and c.entry_form=17 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and c.po_breakdown_id in(".implode(",",$poIdArr).") and d.detarmination_id=$yarn_id and d.color=$color group by a.recv_number, d.id , a.receive_date, a.challan_no, a.supplier_id, b.body_part_id, a.inserted_by, d.detarmination_id, d.color, d.product_name_details,b.cons_uom, a.booking_no ";
					*/
					//  $receive_qty_data = sql_select("SELECT c.po_breakdown_id as po_id,d.detarmination_id, d.color, c.quantity as order_qnty, b.order_amount,   b.order_rate	from inv_receive_master a , inv_transaction b , order_wise_pro_details c , product_details_master d   where  a.id = b.mst_id and  b.id = c.trans_id  and  d.id=c.prod_id  and  d.id=b.prod_id  and a.entry_form=17 and a.receive_basis in (1,2) and a.status_active=1 and a.is_deleted=0 and b.receive_basis in (1,2) and b.transaction_type=1 and b.status_active=1 and b.is_deleted=0 and c.trans_type=1 and c.entry_form=17 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 $order_cond");
