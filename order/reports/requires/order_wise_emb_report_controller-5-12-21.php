...
{ 
	...
	/ 										main query										/
	/ ===================================================================================  */
	$sql = "SELECT a.job_no,a.style_ref_no,a.buyer_name,a.client_id as buyer_client,a.company_name,b.pub_shipment_date, b.id as po_id,b.po_number,b.po_quantity,c.embel_name,c.serving_company,c.production_type,c.production_quantity,c.production_source
			from wo_po_details_master a,wo_po_break_down b, pro_garments_production_mst c	
			where a.id=b.job_id and b.id=c.po_break_down_id and a.status_active=1 and b.status_active=1 and c.status_active=1  $sql_cond and c.production_type in(2,3)";				
	 //echo $sql;die();		
	...
	/ 										costing data									/
	/ ===================================================================================  */
	$sql = "SELECT b.po_break_down_id as PO_ID,b.RATE, b.AMOUNT from WO_PRE_COST_EMBE_COST_DTLS a, WO_PRE_COS_EMB_CO_AVG_CON_DTLS b where a.id=b.pre_cost_emb_cost_dtls_id and a.status_active=1 and b.status_active=1 and a.is_deleted=0 and b.is_deleted=0 and b.po_break_down_id in($poIds)";
	// echo $sql;die();
	$res = sql_select($sql);
	$costing_array = array();
	foreach ($res as $val) 
	...
	/ 										EB WO Data										/
	/ ===================================================================================  */
	$sql = "SELECT a.ID,a.SUPPLIER_ID,b.ID as DTLS_ID,b.po_break_down_id as PO_ID,a.BOOKING_NO_PREFIX_NUM, b.AMOUNT from WO_BOOKING_MST a, WO_BOOKING_DTLS b where a.booking_no=b.booking_no and a.status_active=1 and b.status_active=1 and a.is_deleted=0 and b.is_deleted=0 and b.po_break_down_id in($poIds) and b.booking_type=6 and b.is_short=2";
	// echo $sql;die();
	$res = sql_select($sql);
	...
	/ 										PI Data 										/
	/ ===================================================================================  */
	$sql = "SELECT a.SUPPLIER_ID,a.id as PI_ID,a.PI_NUMBER, b.AMOUNT,b.WORK_ORDER_DTLS_ID from COM_PI_MASTER_DETAILS a, COM_PI_ITEM_DETAILS b where a.id=b.pi_id and a.status_active=1 and b.status_active=1 and a.is_deleted=0 and b.is_deleted=0 and b.work_order_id in($eb_wo_id)";
	// echo $sql;die();
	$res = sql_select($sql);
	$pi_data_array = array();
	$pi_id_array = array();
	...
	?>
		...
		<fieldset style="width:1730px;">	
			...
			<div style="width:1730px; max-height:400px; overflow-y:scroll" id="scroll_body">
				<table class="rpt_table" width="1710" cellpadding="0" cellspacing="0" border="1" id="table_body">
					<?
					$sl=1;
					$gr_order_qnty 	= 0;
					$gr_issue_qty 	= 0;
							...
							foreach ($style_data as $po_id => $po_data) 
							{
								...
								{														
									...
									<tr  id="tr_<? echo $sl; ?>" onClick="change_color('tr_<? echo $sl; ?>','<? echo $bgcolor; ?>')" bgcolor="<? echo $bgcolor; ?>">	
					                    ...
					                    <td align="left" width="100"><? echo $style_no; ?></td>
										...
										<td align="left" width="100"><? echo $supplier_library[$wo_com_id]; ?></td>
					                    ...
					                    <td align="right" width="100"><? echo $eb_rate;?></td>
