...
if ($action=="report_generate")
{
	...
	$sql_non="select a.id, a.wo_number, b.po_breakdown_id, b.item_category_id, b.supplier_order_quantity, b.amount from wo_non_order_info_mst a, wo_non_order_info_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $poid_cond ";
	//echo $sql_non;//die;
	...
	from wo_booking_dtls where status_active=1 and is_deleted=0 and wo_qnty>0 and process<>30 $jobNos_cond
	union all
	...
	foreach($sql_rec_res as $rrow)
	{
		...
		{
			$rec_trim_arr[$jobn][$trim_id_arr[$rrow[csf("prod_id")]]]['qty']+=$rrow[csf("order_qnty")];
			$rec_trim_arr[$jobn][$trim_id_arr[$rrow[csf("prod_id")]]]['amt']+=$rrow[csf("order_amount")];
			$rec_trim_arr[$jobn][$trim_id_arr[$rrow[csf("prod_id")]]]['booking_id'].=$rrow[csf("booking_id")].",";
			$rec_trim_arr[$jobn][$trim_id_arr[$rrow[csf("prod_id")]]]['prod_id'].=$rrow[csf("prod_id")].",";
		}
			...
			foreach($job_arr as $jobval)
			{
				...
				foreach($trimData[$job_no]['qty'] as $id=>$iddata)
				{
					foreach($iddata as $trim_group=>$trim_qty)
					{
						...
						$rec_trim_qty=$rec_trim_arr[$job_no][$trim_group]['qty'];
						$rec_trim_amt=$rec_trim_arr[$job_no][$trim_group]['amt'];
						...
						$inhouse_data="inhouse_trims_dtls**".$job_no."**".$all_book_nos."**".$trim_uom."**".$all_prod_ids;
...
if($action=="pop_up_details")
{
	...
	if($pop_up_type=="wo_purchase_fabric_dtls" || $pop_up_type=="wo_embellisment_dtls" || $pop_up_type=="wo_yarn_cost_dtls" || $pop_up_type=="wo_gmts_wash_dtls" || $pop_up_type=="wo_knitting_dtls" || $pop_up_type=="wo_trims_dtls")
	{
		$tot_qty=0;
		?>
        ...
        <div style="width:100%; max-height:340px; overflow-y:scroll" align="left">
            <table cellpadding="0" width="100%" class="rpt_table" rules="all" border="1" >
                <?
                $i=0;
               ...
               ?>
                <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                    ...
                    <td align="center" width="100">
                    	<?
                    		if($pop_up_type=="wo_purchase_fabric_dtls" || $pop_up_type=="wo_knitting_dtls" || $pop_up_type=="wo_trims_dtls")
                    		{
                    			echo $unit_of_measurement[$row[csf("uom")]];//$unit_of_measurement[$uom];//echo $unit_of_measurement[$uom];
                    		}
	        ...
	        <div style="width:100%; max-height:320px; overflow-y:scroll" align="left">
	            <table cellpadding="0" width="100%" class="rpt_table" rules="all" border="1" >
	               ...
	               // $sql="select a.id, a.pi_number, a.pi_date, a.supplier_id, a.item_category_id, b.work_order_no, b.uom, b.service_type, b.embell_name, b.item_group, b.quantity, b.amount from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.item_category_id=4 and b.item_group='$process' and b.work_order_no in ($mydata)";
					 ...
					 $sql="select a.id, a.pi_number, a.pi_date, a.supplier_id, a.item_category_id, b.work_order_no, b.uom, b.service_type, b.embell_name, b.item_group, b.quantity, b.amount from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.item_category_id=4  and b.work_order_no in ($mydata)";
