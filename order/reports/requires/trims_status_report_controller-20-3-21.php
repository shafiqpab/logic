...
if($action=="report_generate")
{
	...
	if(str_replace("'","",$cbo_search_by)==1) //Order Wise
	{
		if($template==1)
		{
			...
			if($txt_po_id!="")
			{
				if($txt_po_id!="" || $txt_po_id!=0) $jobcond="and b.id in($txt_po_id) "; else $jobcond="";
			}
			...
			$budget_arr=array();
			$sqlbom="select a.costing_per, a.costing_date as costing_date, b.id as trim_dtla_id, b.trim_group, b.description, b.cons_uom, b.cons_dzn_gmts, b.rate, b.amount, b.nominated_supp, c.cons, c.country_id, c.cons as cons_cal, c.po_break_down_id
			...
			foreach($sqlBookingWithoutBomData as $worow)
			{
				...
				$po_data_arr[$worow[csf('po_break_down_id')]][$trim_dtla_id]['supplier_id']=$worow[csf('supplier_id')];
			...
			foreach($sqlRecData as $row)
			{
				...
				$recDataArr[$row[csf('po_breakdown_id')]][$row[csf('item_description')]][$row[csf('item_group_id')]][$row[csf('color')]][$row[csf('item_color')]][$row[csf('item_size')]]['inhouse_qnty']+=$row[csf('quantity')];
				$amount=0; $amount=($row[csf('quantity')]*$row[csf('rate')]);
				$recDataArr[$row[csf('po_breakdown_id')]][$row[csf('item_description')]][$row[csf('item_group_id')]][$row[csf('color')]][$row[csf('item_color')]][$row[csf('item_size')]]['inhouse_value']+=$amount;
				$recDataArr[$row[csf('po_breakdown_id')]][$row[csf('item_description')]][$row[csf('item_group_id')]][$row[csf('color')]][$row[csf('item_color')]][$row[csf('item_size')]]['basis_piwono'].=$row[csf('receive_basis')].'_'.$row[csf('booking_id')].',';
			}
			unset($sqlRecData);
			...
			*/
			...
			$sqlIssue="select b.po_breakdown_id, p.item_group_id, (b.quantity) as quantity, (b.quantity*b.order_rate) as issue_amount, p.color, p.item_color, p.item_size
			...
			foreach($sqlIssueData as $row)
			{
				...
				if($row[csf('item_color')]==0) $row[csf('item_color')]="";
				if($row[csf('item_size')]=='0') $row[csf('item_size')]="";
				$issueDataArr[$row[csf('po_breakdown_id')]][$row[csf('item_group_id')]][$row[csf('color')]][$row[csf('item_color')]][$row[csf('item_size')]]['issue_qty']+=$row[csf('quantity')];
				$issueDataArr[$row[csf('po_breakdown_id')]][$row[csf('item_group_id')]][$row[csf('color')]][$row[csf('item_color')]][$row[csf('item_size')]]['issue_amount']+=$row[csf('issue_amount')];
			}
			unset($sqlIssueData);
				...
				foreach($podata as $bomtrimid=>$bomData)
				{
								...
								foreach($itemcolorData as $size=>$sizeData)
								{
                                    ...
                                    <tr bgcolor="<?=$bgcolor;?>" onclick="change_color('tr_<?=$q; ?>','<?=$bgcolor;?>')" id="tr_<?=$q; ?>">
                                        ...
                                        if($i==1)
                                        {
                                            ...
                                            <td width="90" rowspan="<?=$rowspan; ?>" style="word-break:break-all" valign="middle"><?=$po_arr[$poid]['job_no']; ?></td>
                                            
                                            <td width="100" rowspan="<?=$rowspan; ?>" style="word-break: break-all;" valign="middle"><?=$po_arr[$poid]['style_ref']; ?></td>
                                        ...
                                        $inHouseQty=$recDataArr[$poid][$bomData['description']][$bomData['trim_group']][$gmtscolorid][$itemcolorid][$size]['inhouse_qnty'];
                                        $inHouseAmt=$recDataArr[$poid][$bomData['description']][$bomData['trim_group']][$gmtscolorid][$itemcolorid][$size]['inhouse_value'];
                                        $issueQty=$issueDataArr[$poid][$bomData['trim_group']][$gmtscolorid][$itemcolorid][$size]['issue_qty'];
                                        $issueAmt=$issueDataArr[$poid][$bomData['trim_group']][$gmtscolorid][$itemcolorid][$size]['issue_amount'];
					...
					else
					{
						...
						<tr bgcolor="<?=$bgcolor;?>" onclick="change_color('tr_<?=$q; ?>','<?=$bgcolor;?>')" id="tr_<?=$q; ?>">
                            ...
                            $inHouseQty=$inHouseAmt=$issueQty=$issueAmt=$leftOverQty=$leftOverAmt=0;
                            $inHouseQty=$recDataArr[$poid][$bomData['description']][$bomData['trim_group']]['']['']['']['inhouse_qnty'];
                            $inHouseAmt=$recDataArr[$poid][$bomData['description']][$bomData['trim_group']]['']['']['']['inhouse_value'];
                            $issueQty=$issueDataArr[$poid][$bomData['trim_group']]['']['']['']['issue_qty'];
                            $issueAmt=$issueDataArr[$poid][$bomData['trim_group']]['']['']['']['issue_amount'];
                            $leftOverQty=$inHouseQty-$issueQty;
                            $leftOverAmt=$inHouseAmt-$issueAmt;
								...
								foreach($itemcolorData as $size=>$sizeData)
								{
                                    ...
                                    <tr bgcolor="<?=$bgcolor;?>" onclick="change_color('tr_<?=$i; ?>','<?=$bgcolor;?>')" id="tr_<?=$i; ?>">
										...
										foreach($expoid as $pid)
										{
											...
											$inHouseQty+=$recDataArr[$pid][$bomData['description']][$bomData['trim_group']][$gmtscolorid][$itemcolorid][$size]['inhouse_qnty'];
											$inHouseAmt+=$recDataArr[$pid][$bomData['description']][$bomData['trim_group']][$gmtscolorid][$itemcolorid][$size]['inhouse_value'];
											$issueQty+=$issueDataArr[$pid][$bomData['trim_group']][$gmtscolorid][$itemcolorid][$size]['issue_qty'];
											$issueAmt+=$issueDataArr[$pid][$bomData['trim_group']][$gmtscolorid][$itemcolorid][$size]['issue_amount'];
										}
                                        $leftOverQty=$inHouseQty-$issueQty;
                                        $leftOverAmt=$inHouseAmt-$issueAmt;
										//echo $poid.'='.$bomData['trim_group'].'='.$gmtscolorid.'='.$itemcolorid.'='.$size.'<br>';
