	...
	if(str_replace("'","",$cbo_search_by)==1) //Order Wise
	{
		if($template==1)
		{
			...
			else
			{
			 ...
			 $bookingConsCond=where_con_using_array($exbookingid,0,'po_break_down_id');
							...
							foreach($gmtscolorData as $itemcolorid=>$itemcolorData)
							{
								...
								foreach($itemcolorData as $size=>$sizeData)
								{
                                    ...
                                    <tr bgcolor="<?=$bgcolor;?>" onclick="change_color('tr_<?=$q; ?>','<?=$bgcolor;?>')" id="tr_<?=$q; ?>">
                                        ...
                                        $inHouseAmt=$recDataArr[$poid][trim($bomData['description'])][$bomData['trim_group']][$gmtscolorid][$itemcolorid][$size]['inhouse_value'];
                                        $issueQty=$issueDataArr[$poid][trim($bomData['description'])][$bomData['trim_group']][$gmtscolorid][$itemcolorid][$size]['issue_qty'];
                                        $issueAmt=$issueDataArr[$poid][trim($bomData['description'])][$bomData['trim_group']][$gmtscolorid][$itemcolorid][$size]['issue_amount'];
