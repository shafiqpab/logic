...
if ($action=="report_generate_bk")
{
	...
	if($db_type==2 && $tot_rows>1000)
	{
		...
		$jobNos_cond2=chop($jobNos_cond2,'or ');
		$jobNos_cond.=")";
		$jobNos_cond2.=")";
	...
	$booking_job_arr=array(); $yarn_ord_arr=array(); $woNos="";$woNo_ids=""; $wo_rows=0;
	foreach($sql_non_res as $nrow)
	{
		$wo_rows++;
		$woNos.="'".$nrow[csf("wo_number")]."',";
		if($nrow[csf("id")]){$woNo_ids.="'".$nrow[csf("id")]."',";}
	...
	where a.id=b.mst_id and b.job_no=c.job_no and c.id=d.fabric_description  and a.company_id in($cbo_company_id) $jobNos_cond2 and a.item_category_id  in(1, 24)  and a.entry_form in(1,41) and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and b.yarn_wo_qty >0 and d.cons_process=30";
	...
	foreach($sql_book_res as $brow)
	{
		...
		//if($cbo_search_type==24){
			...
			if($brow[csf("booking_id")]){$woNo_ids.="'".$brow[csf("booking_id")]."',";}
		//}else{
			//$woNos.="'".$brow[csf("booking_no")]."',";
	...
	$woNos=implode(",",array_unique(explode(",",chop($woNos,','))));
	$woNo_ids=implode(",",array_unique(explode(",",chop($woNo_ids,','))));
	$woNos_cond=$woNo_ids_cond="";

	if($db_type==2 && $wo_rows>1000)
	{
		$woNos_cond=" and (";
	...
	foreach($sql_pi_res as $prow)
	{
		...
		else if($prow[csf("item_category_id")]==12)
		{
			...
			$pi_conv_arr[$jobn][$prow[csf("service_type")]][$prow[csf("uom")]]['wo_ids'].=$prow[csf("work_order_id")].",";
	...
	unset($sql_pi_res);
	// echo "<pre>";var_dump($pi_conv_arr);echo "</pre>";
    ...
    $trims_rcv_arr=array();    $trims_rcv_id_arr=array();
			...
			foreach($job_arr as $jobval)
			{
                ...
                foreach($knitData[$job_no]['qty'] as $process=>$uomdata)
				{
					...
					foreach($uomdata as $kuom=>$processqty)
					{
						...
						$all_wo_no_pi=implode(",", array_unique(explode(",", chop($pi_conv_arr[$job_no][$process][$kuom]['wo_no'],","))));
						$all_wo_no_ids_pi=implode(",", array_unique(explode(",", chop($pi_conv_arr[$job_no][$process][$kuom]['wo_ids'],","))));
                        ...
                        <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_1nd<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_1nd<? echo $i; ?>">
                            ...
                            <td width="80" align="right">
                            	<a onClick="show_popup_report_details('pi_pop_up_details','<? echo $pi_data; ?>','850px');" href="##">
                            		<? echo number_format($pi_conv_qty,2); ?>
                            	</a>
                            </td>
                    		<td width="90" align="right"><? echo number_format($pi_conv_amt,2); ?></td>
                            <td width="80" align="right">
                            	<a onClick="show_popup_report_details('inhouse_pop_up_details','<? echo $inhouse_data; ?>','850px');" href="##">
                            		<?
										if($cbo_search_type == 24){
				...
				foreach($trimData[$job_no]['qty'] as $id=>$iddata)
				{
					foreach($iddata as $trim_group=>$trim_qty)
					{
						...
						$rec_trim_po_id=implode(",", array_unique(explode(",", chop($trims_rcv_arr[$trim_group]['po_id'],","))));
						...
						$inhouse_data="inhouse_trims_dtls**".$all_trans_ids."**".$rec_trim_po_id;
...
if ($action=="report_generate")
{
	...
	$job_sql="select a.id, a.job_no, a.job_no_prefix_num, a.buyer_name, $insert_year as job_year, a.dealing_marchant, a.style_ref_no, a.job_quantity, a.avg_unit_price, a.total_price, b.id as po_id, c.exchange_rate from wo_po_details_master a, wo_po_break_down b , wo_pre_cost_mst c where a.job_no=b.job_no_mst and a.job_no=c.job_no and a.company_name='$cbo_company_id' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $buyer_id_cond $year_id_cond $job_cond $style_cond $ship_date_cond $hidd_job_no_cond order by a.id DESC";
	//echo $job_sql; die;
	$job_sql_res=sql_select($job_sql);
	...
	foreach($job_sql_res as $row)
	{
		...
		$job_arr[$row[csf("job_no")]]["avg_price"]=$row[csf("avg_unit_price")];
		$job_arr[$row[csf("job_no")]]["job_amount"]=$row[csf("total_price")];
		$job_arr[$row[csf("job_no")]]["exchange_rate"]=$row[csf("exchange_rate")];
	...
	echo '<pre>';print_r($fabPurArr);die;
	unset($data_fabPur); */
	...
	$sql_book="SELECT 0 as booking_id, b.id, b.booking_no, b.job_no, b.po_break_down_id, b.pre_cost_fabric_cost_dtls_id, b.booking_type, b.process, b.emblishment_name, b.uom, b.wo_qnty, b.grey_fab_qnty, b.amount, 1 as type, a.currency_id
	from wo_booking_mst a join wo_booking_dtls b on a.booking_no=b.booking_no where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.wo_qnty>0 $jobNos_cond2
	union all
	select distinct a.id as booking_id, b.id, a.ydw_no as booking_no, b.job_no as job_no, 0 as po_break_down_id, 0 as pre_cost_fabric_cost_dtls_id, 3 as booking_type, d.cons_process as process, 0 as emblishment_name, c.uom as uom,  b.yarn_wo_qty as wo_qnty,  0 as grey_fab_qnty,  b.amount as amount, 2 as type, 2 as currency_id
	from wo_yarn_dyeing_mst a,  wo_yarn_dyeing_dtls b,  wo_pre_cost_fabric_cost_dtls c, wo_pre_cost_fab_conv_cost_dtls d
	where a.id=b.mst_id and b.job_no=c.job_no and c.id=d.fabric_description  and a.company_id in($cbo_company_id) $jobNos_cond2 and a.item_category_id  in(1, 24)  and a.entry_form in(1,41) and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and b.yarn_wo_qty >0 and d.cons_process=30";
	...
	foreach($sql_book_res as $brow)
	{
		...
		if($brow[csf("booking_type")]==1)
		{
			$exchange_rate=$job_arr[$job_no]['exchange_rate'];
			$jobno=$po_job_arr[$brow[csf("po_break_down_id")]];
			$fab_booking_arr[$jobno]['qty']+=$brow[csf("grey_fab_qnty")];
			if($brow[csf("currency_id")]==1){
			$fab_booking_arr[$jobno]['amt']+=$brow[csf("amount")]/$exchange_rate;
			}else{
				$fab_booking_arr[$jobno]['amt']+=$brow[csf("amount")];
			}
			$fab_booking_arr[$jobno]['po'].=$brow[csf("po_break_down_id")].",";
		...
		else if($brow[csf("booking_type")]==2)
		{
			...
			$trim_booking_arr[$job_no][$brow[csf("pre_cost_fabric_cost_dtls_id")]]['qty']+=$brow[csf("wo_qnty")];
			if($brow[csf("currency_id")]==1){
			$trim_booking_arr[$job_no][$brow[csf("pre_cost_fabric_cost_dtls_id")]]['amt']+=$brow[csf("amount")]/$exchange_rate;
			}else{
				$trim_booking_arr[$job_no][$brow[csf("pre_cost_fabric_cost_dtls_id")]]['amt']+=$brow[csf("amount")];
			}
			$trim_booking_arr[$job_no][$brow[csf("pre_cost_fabric_cost_dtls_id")]]['po'].=$brow[csf("po_break_down_id")].",";
		...
		else if($brow[csf("booking_type")]==3)
		{
			if($cbo_search_type==24)
			{
				if($dtls_id_check[$brow[csf("id")]]=="")
				{
					...
					$ydw_cost_arr[$job_no][$brow[csf("process")]][$brow[csf("uom")]]['qty']+=$brow[csf("wo_qnty")];
					if($brow[csf("currency_id")]==1){
					$ydw_cost_arr[$job_no][$brow[csf("process")]][$brow[csf("uom")]]['amt']+=$brow[csf("amount")]/$exchange_rate;
					}else{
						$ydw_cost_arr[$job_no][$brow[csf("process")]][$brow[csf("uom")]]['amt']+=$brow[csf("amount")];
					}
					$ydw_cost_arr[$job_no][$brow[csf("process")]][$brow[csf("uom")]]['po'].=$brow[csf("po_break_down_id")].",";
				...
				if($dtls_id_check[$brow[csf("id")]]=="")
				{
					...
					$conv_cost_arr[$job_no][$brow[csf("process")]][$brow[csf("uom")]]['qty']+=$brow[csf("wo_qnty")];
					if($brow[csf("currency_id")]==1){
						$conv_cost_arr[$job_no][$brow[csf("process")]][$brow[csf("uom")]]['amt']+=$brow[csf("amount")]/$exchange_rate;
					}
					else{
						$conv_cost_arr[$job_no][$brow[csf("process")]][$brow[csf("uom")]]['amt']+=$brow[csf("amount")];
					}					
	...
	$emb_booking_sql=sql_select("SELECT a.currency_id, b.job_no, b.po_break_down_id, b.booking_no, b.amount, b.wo_qnty, c.emb_name as emblishment_name,b.uom from wo_booking_mst a join wo_booking_dtls b on a.booking_no=b.booking_no join wo_pre_cost_embe_cost_dtls c on c.id=b.pre_cost_fabric_cost_dtls_id where a.status_active=1 and a.is_deleted=0 and  b.status_active=1 and b.is_deleted=0 and  b.status_active=1 and b.is_deleted=0 and b.booking_type=6 $jobNos_cond2 ");
	foreach($emb_booking_sql as $row){
		$job_no=$row[csf('job_no')];
		$exchange_rate=$job_arr[$job_no]['exchange_rate'];
		$emb_booking_arr[$job_no][$row[csf("emblishment_name")]]['qty']+=$row[csf("wo_qnty")];
		if($row[csf("currency_id")]==1){
			$emb_booking_arr[$job_no][$row[csf("emblishment_name")]]['amt']+=$row[csf("amount")]/$exchange_rate;
		}else{$emb_booking_arr[$job_no][$row[csf("emblishment_name")]]['amt']+=$row[csf("amount")];}
		$emb_booking_arr[$job_no][$row[csf("emblishment_name")]]['uom']=$unit_of_measurement[$row[csf("uom")]];
		$emb_booking_arr[$job_no]['po'].=$row[csf("po_break_down_id")].",";

		$booking_job_arr[$row[csf("booking_no")]]=$job_no;
		$booking_job_ids_arr[$row[csf("booking_id")]]=$job_no;
	}

	unset($sql_book_res);
	
	/* echo "<pre>";
	print_r($conv_cost_arr); die; */
	...
	if($cbo_search_type==24){
		$sql_pi="select a.id, a.item_category_id, b.work_order_no,b.work_order_id, b.uom, b.service_type, b.embell_name, b.item_group, sum(b.quantity) as quantity, sum(b.amount) as amount from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $woNos_cond $woNo_ids_cond and a.item_category_id = 24  and a.importer_id = $cbo_company_id and b.embell_name in(1,2) group by a.id, a.item_category_id, b.work_order_no,  b.uom, b.service_type,  b.embell_name,  b.item_group,b.work_order_id";
	}else{
		$sql_pi="select a.id, a.item_category_id, b.work_order_no, b.uom, b.service_type, b.embell_name, b.item_group, sum(b.quantity) as quantity, sum(b.amount) as amount from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $woNos_cond $woNo_ids_cond  and a.importer_id = $cbo_company_id and b.embell_name in(1,2) group by a.id, a.item_category_id, b.work_order_no,  b.uom, b.service_type,  b.embell_name,  b.item_group";
	}
	...
	foreach($sql_pi_res as $prow)
	{
		...
		else if($prow[csf("item_category_id")]==25)
		{
			...
			$pi_emb_arr[$jobn][$prow[csf("embell_name")]]['amt']+=$prow[csf("amount")];
			$pi_emb_arr[$jobn][$prow[csf("embell_name")]]['wo_no'].=$prow[csf("work_order_no")].",";
			$pi_emb_array[$jobn]['wo_no'].=$prow[csf("work_order_no")].",";
		}
		$pi_job_arr[$prow[csf("id")]]=$jobn;
	}
	unset($sql_pi_res);
	...
	$sql_conv="SELECT b.job_no, b.process_id, b.batch_issue_qty, b.amount from inv_receive_mas_batchroll a join pro_grey_batch_dtls b on a.id=b.mst_id where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $jobNos_cond and a.entry_form=92";
	//echo $sql_conv;
	$sql_conv_res=sql_select($sql_conv);
	$rec_conv_arr=array();
	foreach($sql_conv_res as $srow)
    ...
    <fieldset>
        ...
        <table width="1800" cellspacing="0" border="1" class="rpt_table" rules="all">
            <thead>
                <tr>
                    <th width="50" rowspan="2">SL</th>
                    <th colspan="8">Style Info</th>
        ...
        <div style="max-height:350px; overflow-y:scroll; width:1817px" id="scroll_body" >
            <table cellspacing="0" cellpadding="0" border="1" class="rpt_table"  width="1800" rules="all" id="" >
            <?
			//print_r($job_arr); //"jahid";die;
            foreach($job_arr as $jobval)
			{
			...
			foreach($job_arr as $jobval)
			{
				...
				<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_1nd<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_1nd<? echo $i; ?>">
                    ...
                    <td width="90" align="right"><? echo number_format($yarn_wo_amt,2); ?></td>
					<td width="100" align="right"><? echo number_format(($yarn_amount-$yarn_wo_amt),2); ?></td>
                    <td width="80" align="right">
                    	 <a onClick="show_popup_report_details('pi_pop_up_details','<? echo $pi_data; ?>','850px');" href="##">
                    		<? echo number_format($pi_yarn_qty,2); ?>
                    	</a>
                    ...
                    <td width="90" align="right"><? echo number_format($pi_yarn_amt,2); ?></td>
					<td width="100" align="right"><? echo number_format(($yarn_wo_amt-$pi_yarn_amt),2); ?></td>
                    <td width="80" align="right">
                    	<a onClick="show_popup_report_details('inhouse_pop_up_details','<? echo $inhouse_data; ?>','850px');" href="##">
                    		<? echo number_format($rec_yarn_qty,2); ?>
                    	</a>
                ...
                foreach($fabPurArr[$job_no]['qty'] as $fuom=>$fabdata)
				{
                    ...
                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_1nd<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_1nd<? echo $i; ?>">
                    	...
                    	<td width="90" align="right"><? echo number_format($fab_booking_amt,2); ?></td>
						<td width="100" align="right"><? echo number_format(($fab_amount-$fab_booking_amt),2); ?></td>
                        <td width="80" align="right">
                        	<a onClick="show_popup_report_details('pi_pop_up_details','<? echo $pi_data; ?>','850px');" href="##">
                        		<? echo number_format($pi_fab_qty,2); ?>
                        	</a>
                    	...
                    	<td width="90" align="right"><? echo number_format($pi_fab_amt,2); ?></td>
						<td width="100" align="right"><? echo number_format(($fab_booking_amt-$pi_fab_amt),2); ?></td>
                        <td width="80" align="right">
                        	<a onClick="show_popup_report_details('inhouse_pop_up_details','<? echo $inhouse_data; ?>','850px');" href="##">
                        		<? echo number_format($rec_fab_qty,2); ?> </a></td>
                ...
                foreach($knitData[$job_no]['qty'] as $process=>$uomdata)
				{
					...
					foreach($uomdata as $kuom=>$processqty)
					{
                        ...
                        <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_1nd<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_1nd<? echo $i; ?>">
                    		...
                    		<td width="90" align="right" title="<?= $process ?>"><? echo number_format($conv_booking_amt,2); ?></td>
							<td width="100" align="right" ><? echo number_format(($con_amount-$conv_booking_amt),2); ?></td>
                            <td width="80" align="right">
                            	<a onClick="show_popup_report_details('pi_pop_up_details','<? echo $pi_data; ?>','850px');" href="##">
                            		<? echo number_format($pi_conv_qty,2); ?>
                            	</a>
                    		...
                    		<td width="90" align="right"><? echo number_format($pi_conv_amt,2); ?></td>
							<td width="100" align="right"><? echo number_format(($conv_booking_amt-$pi_conv_amt),2); ?></td>
                            <td width="80" align="right">
                            	<a onClick="show_popup_report_details('inhouse_pop_up_details','<? echo $inhouse_data; ?>','850px');" href="##">
                            		<?
										if($cbo_search_type == 24){
				...
				$emb_qty=$embData[$job_no]['qty'];
				$emb_amount=$embData[$job_no]['amount'];
				$emb_booking_qty=$emb_booking_arr[$job_no][1]['qty']+$emb_booking_arr[$job_no][2]['qty'];
				$emb_booking_amt=$emb_booking_arr[$job_no][1]['amt']+$emb_booking_arr[$job_no][2]['amt'];
				$emb_booking_uom=$emb_booking_arr[$job_no][1]['uom'];

				$all_po_id=implode(",", array_unique(explode(",", chop($emb_booking_arr[$job_no]['po'],","))));

				$pi_emb_qty=$pi_emb_arr[$job_no][1]['qty']+$pi_emb_arr[$job_no][2]['qty'];
				$pi_emb_amt=$pi_emb_arr[$job_no][1]['amt']+$pi_emb_arr[$job_no][2]['amt'];
				 foreach($pi_emb_array[$job_no] as $row)
				{
					$all_wo_no=implode(",", array_unique(explode(",", chop($pi_emb_array[$job_no]['wo_no'],","))));
				}
				//$all_wo_no=implode(",", array_unique(explode(",", chop($pi_emb_arr[$job_no][1]['wo_no'],","))));

				$rec_emb_qty=$rec_emb_arr[$job_no][1]['qty']+$rec_emb_arr[$job_no][2]['qty'];
				$rec_emb_amt=$rec_emb_arr[$job_no][1]['amt']+$rec_emb_arr[$job_no][2]['amt'];
				$all_po_nos=implode(",", array_unique(explode(",", chop($rec_emb_arr[$job_no][1]['po_id'],","))));
                ...
                <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_1nd<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_1nd<? echo $i; ?>">
                    <td width="160" colspan="<? echo $colspan_num; ?>"><? echo "Embel. Cost"; ?></td>
                    <td width="80" align="right"><? echo number_format($emb_qty,2); ?></td>
                    <td width="50"><? echo $emb_booking_uom; ?></td>
                    <td width="90" align="right"><? echo number_format($emb_amount,2); ?></td>
                    <td width="80" align="right">
                    	<a onClick="show_popup_report_details('pop_up_details','<? echo $data; ?>','850px');" href="##">
                    		<? echo number_format($emb_booking_qty,2); ?>
                    ...
                    <td width="90" align="right"><? echo number_format($emb_booking_amt,2); ?></td>
					<td width="100" align="right"><? echo number_format(($emb_amount-$emb_booking_amt),2); ?></td>
                    <td width="80" align="right">
                    	<a onClick="show_popup_report_details('pi_pop_up_details','<? echo $pi_data; ?>','850px');" href="##">
                    		<? echo number_format($pi_emb_qty,2); ?>
                    	</a>
                    ...
                    <td width="90" align="right"><? echo number_format($pi_emb_amt,2); ?></td>
					<td width="100" align="right"><? echo number_format(($emb_booking_amt-$pi_emb_amt),2); ?></td>
                    <td width="80" align="right">
                    	<a onClick="show_popup_report_details('inhouse_pop_up_details','<? echo $inhouse_data; ?>','850px');" href="##">
                    		<? echo number_format($rec_emb_qty,2); ?>
                    	</a>
                ...
                <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_1nd<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_1nd<? echo $i; ?>">
                    ...
                    <td width="90" align="right"><? echo number_format($wash_booking_amt,2); ?></td>
					<td width="100" align="right"><? echo number_format(($wash_amount-$wash_booking_amt),2); ?></td>
                    <td width="80" align="right">
                    	<a onClick="show_popup_report_details('pi_pop_up_details','<? echo $pi_data; ?>','850px');" href="##">
                    		<? echo number_format($pi_wash_qty,2); ?>
                    	</a>
                    ...
                    <td width="90" align="right"><? echo number_format($pi_wash_amt,2); ?></td>
					<td width="100" align="right"><? echo number_format(($wash_booking_amt-$pi_wash_amt),2); ?></td>
                    <td width="80" align="right">
                    	<a onClick="show_popup_report_details('inhouse_pop_up_details','<? echo $inhouse_data; ?>','850px');" href="##">
                    		<? echo number_format($rec_wash_qty,2); ?>
                    	</a>
				...
				foreach($trimData[$job_no]['qty'] as $id=>$iddata)
				{
					foreach($iddata as $trim_group=>$trim_qty)
					{
                        ...
                        <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_1nd<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_1nd<? echo $i; ?>">
                            ...
                            <? $x++; } ?>
                            <td width="140"><? echo $trim_name_arr[$trim_group]; ?></td>
                            <td width="80" align="right"><? echo number_format($trim_qty,2);$tot_trim_qty+=$trim_qty; ?></td>
                            <td width="50"><? echo $unit_of_measurement[$trim_uom]; ?></td>
                            <td width="90" align="right"><? echo number_format($trim_amount,2);$tot_trim_amount+=$trim_amount; ?></td>
                            <td width="80" align="right">
                            	<a onClick="show_popup_report_details('pop_up_details','<? echo $data; ?>','850px');" href="##">
                            		<? echo number_format($trim_booking_qty,2);$tot_trim_booking_qty+=$trim_booking_qty; ?>
                            	</a>
                            </td>
                    		<td width="90" align="right"><? echo number_format($trim_booking_amt,2);$tot_trim_booking_amt+=$trim_booking_amt; ?></td>
							<td width="100" align="right"><? echo number_format(($trim_amount-$trim_booking_amt),2); ?></td>
                            <td width="80" align="right">
                            	<a onClick="show_popup_report_details('pi_pop_up_details','<? echo $pi_data; ?>','850px');" href="##">
                            		<? echo number_format($pi_trim_qty,2);$tot_pi_trim_qty+=$pi_trim_qty ?>
                            	</a>
                            </td>
                    		<td width="90" align="right"><? echo number_format($pi_trim_amt,2);$tot_pi_trim_amt+=$pi_trim_amt; ?></td>
							<td width="100" align="right"><? echo number_format(($trim_booking_amt-$pi_trim_amt),2); ?></td>
                            <td width="80" align="right">
                            	<a onClick="show_popup_report_details('inhouse_pop_up_details','<? echo $inhouse_data; ?>','850px');" href="##">
                            		<? echo number_format($rec_trim_qty,2);$tot_rec_trim_qty+=$rec_trim_qty; ?>
                            	</a>
                            </td>
                    		<td align="right"><? echo number_format($rec_trim_amt,2);$tot_rec_trim_amt+=$rec_trim_amt ?></td>
                        </tr>
						<?
					}
				}
		 ...
		 <td width="50" align="center" rowspan="<? echo $row_spn; ?>" title="<? echo $trim_count;  ?>"></td>
                    <td width="100" rowspan="<? echo $row_spn; ?>"></td>
                    <td width="60" align="center" rowspan="<? echo $row_spn; ?>"></td>
                    <td width="60" align="center" rowspan="<? echo $row_spn; ?>"></td>
                    <td width="110" rowspan="<? echo $row_spn; ?>" style="word-break:break-all; word-wrap:break-word;"><p></p></td>
                    <td width="100" rowspan="<? echo $row_spn; ?>"></td>
                    <td width="80" align="right" rowspan="<? echo $row_spn; ?>"></td>
                    <td width="60" align="right" rowspan="<? echo $row_spn; ?>"></td>
                    <td width="90" align="right" rowspan="<? echo $row_spn; ?>"></td>

                    <td width="160" align="right" colspan="<? echo $colspan_num; ?>"><? echo "Subtotal"; ?></td>
                    <td width="80" align="right"><? echo number_format($tot_trim_qty,2); ?></td>
                    <td width="50"></td>
                    <td width="90" align="right"><? echo number_format($tot_trim_amount,2); ?></td>

                    <td width="80" align="right">
                    	
                    		<? echo number_format($tot_trim_booking_qty,2); ?>
                    	
                    </td>
                    <td width="90" align="right"><? echo number_format($tot_trim_booking_amt,2); ?></td>
					<td width="100" align="right"><? echo number_format(($tot_trim_amount-$tot_trim_booking_amt),2); ?></td>
                    <td width="80" align="right">
                    	
                    		<? echo number_format($tot_pi_trim_qty,2); ?>
                    	
                    </td>
                    <td width="90" align="right"><? echo number_format($tot_pi_trim_amt,2); ?></td>
					<td width="100" align="right"><? echo number_format(($tot_trim_booking_amt-$tot_pi_trim_amt),2); ?></td>

                    <td width="80" align="right">
                    	
                    		<? echo number_format($tot_rec_trim_qty,2); ?>
                    	
                    </td>
                   
					<td align="right">
                    	
						<? echo number_format($tot_rec_trim_amt,2); ?>
	...
	if($pop_up_type=="wo_purchase_fabric_dtls" || $pop_up_type=="wo_embellisment_dtls" || $pop_up_type=="wo_yarn_cost_dtls" || $pop_up_type=="wo_gmts_wash_dtls" || $pop_up_type=="wo_knitting_dtls" || $pop_up_type=="wo_trims_dtls" || $pop_up_type=="wo_yarn_cost_dtls_sweater")
	{
        ...
        <div style="width:100%; max-height:460px; overflow-y:scroll" align="left">
            <table cellpadding="0" width="100%" class="rpt_table" rules="all" border="1" >
                ...
                {
                	$sql="SELECT a.booking_date as wo_date, a.supplier_id, b.booking_no as wo_number, b.job_no, b.po_break_down_id, b.pre_cost_fabric_cost_dtls_id, b.booking_type, b.process, b.emblishment_name, b.uom, b.wo_qnty as quantity, b.grey_fab_qnty, b.amount from wo_booking_mst a, wo_booking_dtls b, wo_pre_cost_embe_cost_dtls c where c.id=b.pre_cost_fabric_cost_dtls_id and  b.status_active=1 and b.is_deleted=0 and b.job_no='$job_no' and b.po_break_down_id in ($mydata) and a.booking_type=6 and a.entry_form=201 and a.booking_no=b.booking_no and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 and c.emb_name in(1,2)";
					//echo $sql; die;
                }
                else if($pop_up_type=="wo_gmts_wash_dtls")
                {
...
if($action=="pi_pop_up_details")
{
	...
	$cbo_search_type = $expData[6];
		...
		if($wo_no_ids!='')
		{
			$wo_dtls_idCond="and b.work_order_dtls_id in($wo_no_ids)";	
		}
	        ...
	        <div style="width:100%; max-height:320px; overflow-y:scroll" align="left">
	            <table cellpadding="0" width="100%" class="rpt_table" rules="all" border="1" >
	               ...
	               // $sql="select a.id, a.pi_number, a.pi_date, a.supplier_id, a.item_category_id, b.work_order_no, b.uom, b.service_type, b.embell_name, b.item_group, b.quantity, b.amount from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.item_category_id=4 and b.item_group='$process' and b.work_order_no in ($mydata)";
					...
					from com_pi_master_details a, com_pi_item_details b
					where a.id=b.pi_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.item_group='$pi_id' and b.work_order_no in ($mydata) $wo_dtls_idCond";
	...
	if($pop_up_type=="inhouse_yarn_cost_dtls" || $pop_up_type=="inhouse_fabric_purchase_dtls" || $pop_up_type=="inhouse_knitting_dtls" || $pop_up_type=="inhouse_embl_dtls" || $pop_up_type=="inhouse_gmts_wash_dtls" || $pop_up_type=="inhouse_trims_dtls")
	{
        ...
        <div style="width:100%; max-height:320px; overflow-y:scroll" align="left">
            <table cellpadding="0" width="100%" class="rpt_table" rules="all" border="1" >
                ...
                else if($pop_up_type=="inhouse_knitting_dtls")
                {
                	...
                	if($process==1) // for knitting only
                	{
						if($db_type==2)
						{
							...
							union all
							select CAST(a.challan_no AS VARCHAR(30)) as challan_no, CAST(a.recv_number AS VARCHAR(30)) as recv_number, a.receive_date, b.prod_id, ' ' as job_no, 1 as process_id, b.grey_receive_qnty as quantity, 0 as amount from inv_receive_master a, pro_grey_prod_entry_dtls b where a.id=b.mst_id and a.entry_form=2 and a.receive_basis=2 and a.knitting_source=3 and a.exchange_rate>0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.booking_id in ($search_data)
							union all
							select CAST(a.challan_no AS VARCHAR(30)) as challan_no, CAST(a.recv_number AS VARCHAR(30)) as recv_number, a.receive_date, b.prod_id, ' ' as job_no, 1 as process_id, b.order_qnty as quantity, 0 as amount from inv_receive_master a, inv_transaction b where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and a.is_deleted=0 and b.is_deleted=0 and a.receive_basis in (11) and a.exchange_rate>0 and a.entry_form=22 and a.knitting_source=3 and a.booking_id in ($search_data)";
						}else{
							$sql="SELECT a.challan_no, a.recv_number, a.receive_date, b.prod_id, b.job_no, b.process_id, b.batch_issue_qty as quantity, b.amount from inv_receive_mas_batchroll a, pro_grey_batch_dtls b where b.status_active=1 and b.is_deleted=0 and b.job_no='$job_no' and b.process_id=$process and a.id=b.mst_id and a.status_active=1 and a.is_deleted=0
							union all
                ...
                {
                	$sql="SELECT challan_no, po_break_down_id, embel_name,production_date as receive_date,  production_quantity as quantity from pro_garments_production_mst where production_type=3 and status_active=1 and is_deleted=0 and embel_name In(1,2) and po_break_down_id in ($mydata)";
                }
                ...
                {
					if($mydata){$po_cond=" and c.po_breakdown_id in($mydata)";}
                	$sql="SELECT a.recv_number,a.receive_date,  b.prod_id, sum(c.quantity) as quantity
					from inv_receive_master a, inv_transaction b , order_wise_pro_details c
					where a.id=b.mst_id and a.receive_basis in (1,2) and a.item_category=4 and b.transaction_type=1 AND b.id = c.trans_id AND c.entry_form = 24 AND c.trans_type = 1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1  and b.id in ($job_no) $po_cond
					group by a.recv_number,a.receive_date,  b.prod_id";
                }
...
if ($action=="report_generate1")
{
	...
	$sql_book="SELECT 0 as booking_id, id, booking_no,booking_type, job_no, po_break_down_id, pre_cost_fabric_cost_dtls_id, booking_type, process, emblishment_name, uom, wo_qnty, grey_fab_qnty, amount, 1 as type from wo_booking_dtls where status_active=1 and is_deleted=0 and wo_qnty>0 $jobNos_cond 
	union all 
	select distinct a.id as booking_id, b.id, a.ydw_no as booking_no,0 as booking_type, b.job_no as job_no, 0 as po_break_down_id, 0 as pre_cost_fabric_cost_dtls_id, 3 as booking_type, d.cons_process as process, 0 as emblishment_name, c.uom as uom,  b.yarn_wo_qty as wo_qnty,  0 as grey_fab_qnty,  b.amount as amount, 2 as type from wo_yarn_dyeing_mst a,  wo_yarn_dyeing_dtls b,  wo_pre_cost_fabric_cost_dtls c, wo_pre_cost_fab_conv_cost_dtls d where a.id=b.mst_id and b.job_no=c.job_no and c.id=d.fabric_description  and a.company_id in($cbo_company_id) and a.item_category_id  in(1, 24)  and a.entry_form in(1,41) and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and b.yarn_wo_qty >0 and d.cons_process=30";
	//echo $sql_book; die;
	...
	foreach($sql_book_res as $brow)
	{
		...
		$booking_job_arr[$brow[csf("booking_no")]]=$job_no;
		$booking_job_ids_arr[$brow[csf("booking_id")]]=$job_no;
		if($brow[csf("booking_type")]==2 || $brow[csf("booking_type")]==0)
		{
		$booking_dtls_ids_arr[$brow[csf("id")]]=$brow[csf("id")];
		}
	}
//print_r($booking_dtls_ids_arr);
	unset($sql_book_res);
	...
	foreach ($sql_pi_yarn as $row) {
	 ...
	 if(isset($booking_dtls_ids_arr))
	 {
		$woDtlsId_cond_for_in=where_con_using_array($booking_dtls_ids_arr,0,"b.work_order_dtls_id"); 
	 }
	if($cbo_search_type==24){
		$sql_pi="SELECT a.id, a.item_category_id, b.work_order_no,b.work_order_id,b.work_order_dtls_id, b.uom, b.service_type, b.embell_name, b.item_group, sum(b.quantity) as quantity, sum(b.amount) as amount from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $woDtlsId_cond_for_in $woNos_cond $woNo_ids_cond and a.item_category_id = 24  and a.importer_id = $cbo_company_id group by a.id, a.item_category_id, b.work_order_no,  b.uom, b.service_type,  b.embell_name, b.b.work_order_dtls_id, b.item_group,b.work_order_id";
	}else{
		$sql_pi="SELECT a.id, a.item_category_id, b.work_order_no,b.work_order_dtls_id, b.uom, b.service_type, b.embell_name, b.item_group, sum(b.quantity) as quantity, sum(b.amount) as amount from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $woDtlsId_cond_for_in $woNos_cond $woNo_ids_cond  and a.importer_id = $cbo_company_id group by a.id, a.item_category_id, b.work_order_no,b.work_order_dtls_id,  b.uom, b.service_type,  b.embell_name,  b.item_group";
	}
	//echo $sql_pi;//die;
	...
	foreach($sql_pi_res as $prow)
	{
		...
		$pi_trim_arr[$jobn][$prow[csf("item_group")]]['amt']+=$prow[csf("amount")];
		$pi_trim_arr[$jobn][$prow[csf("item_group")]]['wo_no'].=$prow[csf("work_order_no")].",";
		$pi_trim_arr[$jobn][$prow[csf("item_group")]]['work_order_dtls_id'].=$prow[csf("work_order_dtls_id")].",";
		$pi_conv_arr[$jobn][$prow[csf("service_type")]][$prow[csf("uom")]]['qty']+=$prow[csf("quantity")];
		$pi_conv_arr[$jobn][$prow[csf("service_type")]][$prow[csf("uom")]]['amt']+=$prow[csf("amount")];
		$pi_conv_arr[$jobn][$prow[csf("service_type")]][$prow[csf("uom")]]['wo_no'].=$prow[csf("work_order_no")].",";
		$pi_trim_arr[$jobn][$prow[csf("item_group")]]['qty']=$prow[csf("quantity")];
			...
			foreach($job_arr as $jobval)
			{
				...
				foreach($trimData[$job_no]['qty'] as $id=>$iddata)
				{
					foreach($iddata as $trim_group=>$trim_qty)
					{
						...
						$pi_trim_amt=$pi_trim_arr[$job_no][$trim_group]['amt'];
						$all_wo_no=implode(",", array_unique(explode(",", chop($pi_trim_arr[$job_no][$trim_group]['wo_no'],","))));
						$work_order_dtls_id=implode(",", array_unique(explode(",", chop($pi_trim_arr[$job_no][$trim_group]['work_order_dtls_id'],","))));
//work_order_dtls_id
						$rec_trim_qty=$trims_rcv_arr[$trim_group]['quantity'];
						//$rec_id=$trims_rcv_arr[$trim_group]['mst_id'];
						$trans_ids=$trims_rcv_id_arr[$trim_group]['trans_id'];
						...
						$pi_data="pi_trims_dtls**".$job_no."**".$all_wo_no."**".$trim_uom."**".$trim_group."**".$work_order_dtls_id;
						// 'pi_trims_dtls**MF-20-00228**MF-TB-20-00092**1**154'
                        ...
                        <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_1nd<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_1nd<? echo $i; ?>">
                    		...
                    		<td width="90" align="right"><? echo number_format($trim_booking_amt,2); ?></td>
                            <td width="80" align="right" title="ItemId=<? echo $trim_group;?>">
                            	<a onClick="show_popup_report_details('pi_pop_up_details','<? echo $pi_data; ?>','850px');" href="##">
                            		<? echo number_format($pi_trim_qty,2); ?>
                            	</a>
                            </td>
