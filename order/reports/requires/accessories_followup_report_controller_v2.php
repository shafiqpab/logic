...
if ($action=="po_no_popup")
{
	...
	echo  create_list_view("list_view", "PO No.,Job No.,Pub Shipment Date", "100,100,80","400","330",0, $sql , "js_set_value", "id,po_number", "", 1, "0,0,0", $arr , "po_number,job_no_mst,pub_shipment_date", "",'setFilterGrid("list_view",-1);','0,0,3','',1) ;
	...
	if(str_replace("'","",$cbo_search_by)==1)
	{
		...
		<fieldset style="width:100%;">
          ...
          //  $receive_rtn_qty_data=sql_select("select d.po_breakdown_id, c.item_group_id,sum(d.quantity) as quantity   from  inv_issue_master a,inv_transaction b, product_details_master c, order_wise_pro_details d,inv_receive_master e  where a.id=b.mst_id and b.prod_id=c.id and b.id=d.trans_id and e.id=a.received_id   and b.transaction_type=3 and a.entry_form=49 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  $order_cond2  group by d.po_breakdown_id, c.item_group_id order by c.item_group_id");
            ...
            where c.entry_form=73 and p.id=b.prod_id  and b.id=c.trans_id and b.transaction_type=4 and c.is_deleted=0 and c.status_active=1 and b.status_active=1 and b.is_deleted=0 and p.status_active=1 and p.is_deleted=0 group by c.po_breakdown_id,p.item_group_id");
	...
	if(str_replace("'","",$cbo_search_by)==1)
	{
		...
		<fieldset style="width:100%;">
			...
			<table class="rpt_table" width="2560" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body">-->
				...
				$receive_qty_data=array();
				$receive_rtn_qty_data=sql_select("select d.po_breakdown_id, c.item_group_id,sum(d.quantity) as quantity   from  inv_issue_master a,inv_transaction b, product_details_master c, order_wise_pro_details d,inv_receive_master e  where a.id=b.mst_id and b.prod_id=c.id and b.id=d.trans_id and e.id=a.received_id   and b.transaction_type=3 and a.entry_form=49 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  $order_cond2  group by d.po_breakdown_id, c.item_group_id order by c.item_group_id");
...
where a.id=b.mst_id and a.entry_form=24 and a.item_category=4 and b.id=c.dtls_id and b.prod_id=d.id and b.trans_id=c.trans_id and c.trans_type=1 $order_cond3  and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0 group by c.po_breakdown_id, b.item_group_id,b.rate order by b.item_group_id");
			...
			unset($receive_qty_data);
				$receive_rtn_qty_data=sql_select("select d.po_breakdown_id, c.item_group_id, d.quantity as quantity, c.avg_rate_per_unit as rate from product_details_master c, order_wise_pro_details d where d.prod_id=c.id and d.trans_type=3 and d.entry_form=49 and d.status_active=1 and d.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $order_cond2 order by c.item_group_id ASC");
				foreach($receive_rtn_qty_data as $row)
				{
				...
				$sql_issue_ret=("select c.po_breakdown_id as po_id, p.item_group_id,SUM(c.quantity) as quantity
					from   inv_transaction b, order_wise_pro_details c,product_details_master p
					where  c.entry_form=73 and p.id=b.prod_id  and b.id=c.trans_id and b.transaction_type=4 and c.is_deleted=0 and c.status_active=1 and b.status_active=1 and b.is_deleted=0 and p.status_active=1 and p.is_deleted=0 $order_cond3 group by c.po_breakdown_id,p.item_group_id");
				$issue_result=sql_select($sql_issue_ret);
				foreach($issue_result as $row)
				{
				$issue_qty_data_arr[$row[csf('po_id')]][issue_ret_qty][$row[csf('item_group_id')]]+=$row[csf('quantity')];
	...
	<div style="width:1320px">
        ...
        foreach($sql_query as $row)
        {
			...
			if($po_check[$row[csf('po_id')]]=="")
			{
				...
				$po_data[$buy_key]["po_id"].=$row[csf('po_id')].",";
		...
		where a.id=b.mst_id and b.id=c.trans_id and b.prod_id=d.id and a.is_deleted=0  and c.prod_id=d.id and c.entry_form in(24,25,49,73) and d.entry_form=24 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 $order_cond";
		//$order_cond2=str_replace("c.po_break_down_id","b.po_breakdown_id",$order_cond);
		//$order_cond222 = str_replace("c.po_break_down_id", "b.po_breakdown_id", $order_cond);
		...
		foreach($trims_qty_data as $row)
		{
			...
			//echo $po_check[$row[csf("po_breakdown_id")]].'<br>';
		...
		$issue_qty_data=sql_select("select c.id as issue_trans_id, c.po_breakdown_id, a.item_group_id,(c.quantity) as quantity  from  inv_issue_master d,product_details_master p,inv_trims_issue_dtls a , order_wise_pro_details c where a.mst_id=d.id and a.prod_id=p.id and a.trans_id=c.trans_id and c.trans_type=2 and c.entry_form=25 and a.status_active=1 and c.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and p.status_active=1 and p.is_deleted=0 $order_cond  ");
		
		foreach($issue_qty_data as $row)
		{
			$trims_data[$row[csf("po_breakdown_id")]][$row[csf("item_group_id")]]["issue_qnty"]+=$row[csf("quantity")];
			if($row[csf("issue_trans_id")]>0)
			{
				$trims_data[$row[csf("po_breakdown_id")]][$row[csf("item_group_id")]]["issue_trans_id"].=$row[csf("issue_trans_id")].",";
			}
		}
		unset($issue_qty_data);
		...
		{
			$po_id=chop($po_data[$key]["po_id"],",");
			$po_ids=implode(",",array_unique(explode(",",$po_id)));
			$po_idsArr=array_unique(explode(",",$po_id));
			?>
			...
			{
				$issue_qnty=0;
				$issue_rtn_qnty=$trims_data[$key][$group_id]["issue_rtn_qnty"];
				foreach($po_idsArr as $poId)
				{
					$issue_qnty+=$trims_data[$poId][$group_id]["issue_qnty"]-$issue_rtn_qnty;
				}
				
				$rcv_qnty=$trims_data[$key][$group_id]["rcv_qnty"]-$trims_data[$key][$group_id]["rcv_rtn_qnty"];
				
				//echo $trims_data[$key][$group_id]["issue_qnty"].'='.$trims_data[$key][$group_id]["issue_rtn_qnty"].'<br>';
				...
				if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
				$popupData=$po_ids.'_'.$group_id.'_'.$iss_issRtn_trns_id;
				?>
				<tr bgcolor="<? echo $bgcolor;?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>">
					...
					<td width="80" align="right" title="<? echo "issue issuertn trans id:".$iss_issRtn_trns_id; ?>"><a href='#report_details' onclick="openmypage_issue_info('<? echo $popupData; ?>','booking_issue_info_dtls');"><? echo number_format($issue_qnty,2,'.',''); $tot_issue_qnty+=$issue_qnty; ?></a></td>
	 ...
	 <div style="width:width:1070px;" align="center">
		...
		<div id="report_div"  align="center">
			...
			$issue_rtn_qty_data=sql_select("select a.recv_number, a.receive_date, c.id as prod_id, c.product_name_details, sum(d.quantity) as quantity
					...
					where a.id=b.mst_id and b.prod_id=c.id and b.id=d.trans_id and b.transaction_type=4 and d.trans_type=4 and a.entry_form=73 and d.entry_form=73 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.id in($tr_id)
			...
			<table border="1" class="rpt_table" rules="all" width="850" cellpadding="0" cellspacing="0" align="center">
                ...
                <?
					...
					if($boking_cond!="")$boking_cond.=")";
					 $wo_sql="select max(a.is_short)as is_short,max(a.is_approved) as is_approved,a.booking_no,a.entry_form, a.booking_date, a.supplier_id, a.pay_mode, b.job_no,b.country_id_string, b.po_break_down_id,sum(b.wo_qnty) as wo_qnty,b.uom from wo_booking_mst a, wo_booking_dtls b
					...
					a.booking_no,a.entry_form, a.booking_date, a.supplier_id, a.pay_mode, b.uom,b.country_id_string"; /*As Item from precost No,so 0 is added after $trim_dtla_id and it is set to in() function */
					$dtlsArray=sql_select($wo_sql);
					foreach($dtlsArray as $row)
					{
						if ($i%2==0)
				...
				//echo $report.'DDD';
						...
						<tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
                            ...
                            <td width="100" title="Multiple Order Wise Or Country and Order Wise Page"><p><a href="#" onClick="generate_trim_report('<? echo $reporAction; ?>','<? echo $row[csf('booking_no')]; ?>',<? echo $cbo_company_name; ?>,<? echo $row[csf('is_approved')]; ?>,<? echo $row[csf('is_short')]; ?>,<? echo $entry_form; ?>)"><? echo $row[csf('booking_no')]; ?></a></p></td>
			...
			<table border="1" class="rpt_table" rules="all" width="650" cellpadding="0" cellspacing="0" align="left">
                ...
                <?
				 ...
				 $mrr_sql=("select a.id, a.issue_number,a.challan_no,b.prod_id,p.item_group_id, a.issue_date,b.item_description,b.issue_qnty,SUM(c.quantity) as quantity
					...
					b.status_active=1 and b.is_deleted=0 and p.status_active=1 and p.is_deleted=0 and c.po_breakdown_id in($po_id) and p.item_group_id='$item_name' group by c.po_breakdown_id,p.item_group_id,b.item_description,a.issue_number,a.id,a.issue_date,b.prod_id,a.challan_no,b.issue_qnty ");
				//echo $mrr_sql;
					$dtlsArray=sql_select($mrr_sql);
            ...
            <caption> Return Details</caption>
                ...
                <?
				 ...
				 $mrr_sql_ret="SELECT a.id, a.recv_number,a.challan_no,b.prod_id, p.item_group_id,a.receive_date,p.product_name_details,SUM(c.quantity) as quantity
					...
					where a.id=b.mst_id  and a.entry_form=73 and c.entry_form=73 and p.id=b.prod_id  and b.id=c.trans_id and b.transaction_type=4 and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and p.status_active=1 and p.is_deleted=0 and c.po_breakdown_id in($po_id) and p.item_group_id='$item_name' 
					group by a.id, a.recv_number, a.challan_no, b.prod_id, p.item_group_id, a.receive_date, p.product_name_details ";
				 //echo $mrr_sql_ret;
