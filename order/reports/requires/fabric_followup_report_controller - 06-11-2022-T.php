...
if($action=='fabric_report_generate'){
	...
	(CASE WHEN a.entry_form in(18) THEN a.quantity ELSE 0 END) AS knit_qnty_issue
	from order_wise_pro_details a, inv_transaction b, inv_issue_master c,product_details_master d where a.trans_id=b.id and b.mst_id=c.id and d.id=b.prod_id and a.prod_id=d.id and c.item_category=2 and a.entry_form in(18) and c.entry_form in(18) and b.transaction_type=2 and a.status_active=1 and a.is_deleted=0 and b.is_deleted=0 and b.status_active=1  and c.is_deleted=0 and c.status_active=1 $po_cond_for_in2 ");// and c.receive_basis<>9
        ...
        <div style="width:100%">
            ...
            <table width="<? echo $tbl_width;?>" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body">
						...
						foreach($body_data as $deter_id=>$deter_data)
						{
							...
							foreach($deter_data as $gmt_color=>$val)
						    {
									...
									foreach($po_ids as $pId)
									{
										...
										$kniting_prod_issue_amt+=$issue_kniting_prod_arr[$pId][$bprat_id][$deter_id][$gmt_color]["knit_qnty_issue_amt"];
									...
									?>
								 	<tr bgcolor = "<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" style="font-size:13px">
				                       ...
				                       <?
				                        ...
				                        <td align="center" style="word-wrap:break-word; word-break: break-all;" width="100" style="word-break:break-all" title="sensitive=<? echo $sensitive; ?>"><div style="word-wrap:break-word:100px;"><? echo $fab_gmt_color; ?></div></td>
				                        ...
				                        <td align="right" style="word-wrap:break-word; word-break: break-all;" width="80" title="Grey Qty"><a href="javascript:open_wo_popup('<? echo implode(',',array_unique(explode(",",$po_id)));?>','<? echo $bprat_id;?>','<? echo $deter_id;?>','<? echo $gmt_color;?>','<? echo $job_no;?>','WO Info','wo_popup')"><? echo number_format($booking_req,2); ?></a>
				                        </td>
				                        ...
				                        <td align="right" style="word-wrap:break-word; word-break: break-all;" width="80" title="Woven Fin Recv"><a href="javascript:open_wo_popup('<? echo implode(',',array_unique(explode(",",$po_id)));?>','<? echo $bprat_id;?>','<? echo $deter_id;?>','<? echo $gmt_color;?>','<? echo $job_no;?>','Fin Recv Info','fin_recv_popup')"><? echo number_format($kniting_prod_recv,2); ?></a></td>
				                        ...
				                        <td align="right" style="word-wrap:break-word; word-break: break-all;"  width="80" title="Fin Issue"><a href="javascript:open_wo_popup('<? echo implode(',',array_unique(explode(",",$po_id)));?>','<? echo $bprat_id;?>','<? echo $deter_id;?>','<? echo $gmt_color;?>','<? echo $job_no;?>','Fin Issue Info','fin_issue_popup')"><?   echo number_format($kniting_prod_issue,2); ?></a></td>
                ...
                <tr style="font-size:13px">
                    ...
                    <td width="70" align="right" id="td_order_qty"><? echo number_format($grand_tot_po_qnty,0); ?></td>
                    
                    <td width="40" align="right" id=""><? //echo number_format($tot_po_qnty_pcs,2); ?></td>
                    <td width="70" align="right" id="td_order_qty_pcs"><? echo number_format($grand_po_qnty_pcs,0); ?></td>
                    ...
                    <td width="80" align="right" id="td_total_fabric_req"><? echo number_format($total_fabric_req,2); ?></td>
                    <td width="50" align="right" id=""><? //echo number_format($tot_finish_left_over,2); ?></td> 
                    <td width="80" align="right" id="td_total_fabric_req_cost" bgcolor="#FFFFCC"><? echo number_format($total_fabric_req_cost,2); ?></td>
                    <td width="80" align="right" id="td_total_booking_req"><? echo number_format($total_booking_req,2); ?></td>
                    <td width="80" align="right" id="td_total_fabric_amount"><? echo number_format($total_fabric_amount,2); ?></td>
                    ...
                    <td width="80" align="right" id="td_total_kniting_prod_recv"><? echo number_format($total_kniting_prod_recv,2); ?></td>
                    <td width="80" align="right" id="td_total_kniting_prod_recv_amt"><? echo number_format($total_kniting_prod_recv_amt,2); ?></td>
                    <td width="80" align="right" id="td_total_fab_recv_balance"><? echo number_format($tot_recv_balance,2); ?></td>
					<td width="80" align="right" id=""><? //echo number_format($tot_recv_balance,2); ?></td>
                    <td width="80" align="right" id="td_total_kniting_prod_issue"><? echo number_format($total_kniting_prod_issue,2); ?></td>
                    <td width="80" align="right" id="td_total_kniting_prod_issue_amt"><? echo number_format($total_kniting_prod_issue_amt,2); ?></td>
                    <td width="80" align="right" id="td_total_left_over_bal"><? echo number_format($total_left_over_bal,2); ?></td>
                    <td align="right" width="" id="td_total_left_over_bal_amount"><? echo number_format($total_left_over_bal_amount,2); ?></td>
                </tr>
           </table>
            </div>
...
<script>
	...
	function generate_fabric_report(type,report_type,booking,company,fabric_nature,fabric_source,job_no,entry_form){
		  ...
		  $i=1;$tot_wo_qnty=0;
			...
			?>
            	<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trpo_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="trpo_<? echo $i; ?>">
                    <td><p><? echo $i; ?>&nbsp;</p></td>
                    <td><a href='#' onClick="generate_fabric_report('<?=$type;?>',1,'<?=$booking;?>','<?=$row[('company_id')];?>','<?=$row[('item_category')];?>','<?=$row[('fabric_source')];?>','<?=$row[('job_no')];?>','<?=$row['entry_form'];?>')"><?=$booking?></a></td>
...
<script>
 ...
 <div  style="width:1150px;" align="center">
	...
	$prodKnitDataArr=sql_select("select c.recv_number,c.challan_no,c.supplier_id,c.booking_no,c.booking_id,c.receive_date,c.receive_basis,c.inserted_by,a.po_breakdown_id as po_id,b.uom,b.prod_id,b.fabric_description_id as deter_id,b.rate,b.body_part_id,a.color_id,
	(CASE WHEN a.entry_form in(37,17) THEN a.quantity ELSE 0 END) AS knit_qnty_rec
