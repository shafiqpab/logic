...
if($action=="report_generate")
{
	...
	else if(str_replace("'","",$cbo_search_by)==2) //Style Wise
	{
		...
		if($template==1)
		{
			...
			foreach($conversion_factor as $row_f)
			{
				$conversion_factor_array[$row_f['ID']]['con_factor']=$row_f['CONVERSION_FACTOR'];
				$conversion_factor_array[$row_f['ID']]['cons_uom']=$row_f['TRIM_UOM'];
				$item_arr[$row_f['ID']]['order_uom']=$row_f['ORDER_UOM'];
			}
			unset($conversion_factor);
			...
			$po_job_arr=array(); $style_po_qty_arr=array();
 			/*$sql_po_qty_country_wise=sql_select("select  b.id as ID, b.job_no_mst as JOB_NO_MST, c.country_id as COUNTRY_ID, (c.order_quantity/a.total_set_qnty) as ORDER_QUANTITY_SET, (c.order_quantity) as ORDER_QUANTITY  from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c,wo_pre_cost_dtls d where a.id=b.job_id and a.id=c.job_id and a.id=d.job_id   and b.job_id=d.job_id  and b.job_id=c.job_id  and b.id=c.po_break_down_id  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1  and a.company_name=$company_name $buyer_id_cond $date_cond $style_ref_cond $jobcond $ordercond $year_cond 	$team_cond $team_member_cond ");
			 
			foreach( $sql_po_qty_country_wise as $row)
			{
				$sql_po_qty_country_wise_arr[$row['ID']][$row['COUNTRY_ID']]=$row['ORDER_QUANTITY_SET'];
				$po_job_arr[$row['ID']]=$row['JOB_NO_MST'];
				$style_po_qty_arr[$row['JOB_NO_MST']]['order_qty_set']+=$row['ORDER_QUANTITY_SET'];
				$style_po_qty_arr[$row['JOB_NO_MST']]['po_qty']+=$row['ORDER_QUANTITY'];
			}
			...
			$po_id_string="";
			$today=date("Y-m-d");
			//A.BUYER_NAME, A.JOB_NO, A.JOB_NO_PREFIX_NUM, A.STYLE_REF_NO, A.ORDER_UOM, B.FILE_NO, B.GROUPING, B.ID, B.PO_NUMBER, SUM(C.ORDER_QUANTITY) AS ORDER_QUANTITY, SUM(C.ORDER_QUANTITY/A.TOTAL_SET_QNTY) AS ORDER_QUANTITY_SET, B.PUB_SHIPMENT_DATE , A.SEASON_BUYER_WISE
			 /* $sql_pos=("select a.buyer_name as BUYER_NAME, a.job_no as JOB_NO, a.job_no_prefix_num as JOB_NO_PREFIX_NUM, a.style_ref_no as STYLE_REF_NO, a.order_uom as ORDER_UOM, b.file_no as FILE_NO, b.grouping as GROUPING, b.id as ID, b.po_number as PO_NUMBER,b.shiping_status as SHIPING_STATUS, sum(c.order_quantity) as ORDER_QUANTITY, sum(c.order_quantity/a.total_set_qnty) as ORDER_QUANTITY_SET, b.pub_shipment_date as PUB_SHIPMENT_DATE , a.season_buyer_wise as SEASON_BUYER_WISE
			from wo_po_details_master a, wo_po_break_down b left join pro_ex_factory_mst d on b.id=d.po_break_down_id, wo_po_color_size_breakdown c
			where
			a.id=b.job_id and a.id=c.job_id and b.id=c.po_break_down_id and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and a.company_name=$company_name $buyer_id_cond  $shipment_status_cond ".set_user_lavel_filtering(' and a.buyer_name','buyer_id')." $date_cond $style_ref_cond $jobcond  $ordercond $file_no_cond $internal_ref_cond  $year_cond $season_con 	$team_cond $team_member_cond
			group by a.buyer_name, a.job_no, a.job_no_prefix_num, a.style_ref_no, a.order_uom, a.total_set_qnty,b.shiping_status, b.file_no, b.grouping, b.id, b.po_number, b.pub_shipment_date  , a.season_buyer_wise order by b.id");*/ 
			  $sql_pos=("select a.buyer_name as BUYER_NAME, a.job_no as JOB_NO, a.job_no_prefix_num as JOB_NO_PREFIX_NUM, a.style_ref_no as STYLE_REF_NO, a.order_uom as ORDER_UOM, b.file_no as FILE_NO, b.grouping as GROUPING, b.id as ID, b.po_number as PO_NUMBER,b.shiping_status as SHIPING_STATUS, (c.order_quantity) as ORDER_QUANTITY, (c.order_quantity/a.total_set_qnty) as ORDER_QUANTITY_SET, b.pub_shipment_date as PUB_SHIPMENT_DATE , a.season_buyer_wise as SEASON_BUYER_WISE,c.COUNTRY_ID 
			from wo_po_details_master a, wo_po_break_down b , wo_po_color_size_breakdown c
			where
			a.id=b.job_id and a.id=c.job_id and b.id=c.po_break_down_id and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and a.company_name=$company_name  and a.id not in(11521,11531) $buyer_id_cond  $shipment_status_cond  $date_cond $style_ref_cond $jobcond  $ordercond $file_no_cond $internal_ref_cond  $year_cond $season_con 	$team_cond $team_member_cond
			 order by b.id"); 
			...
			foreach($sql_po as $row)
			{
				...
				$po_job_arr[$row['ID']]=$row['JOB_NO'];
				$style_po_qty_arr[$row['JOB_NO']]['order_qty_set']+=$row['ORDER_QUANTITY_SET'];
				$style_po_qty_arr[$row['JOB_NO']]['po_qty']+=$row['ORDER_QUANTITY'];
				

				$style_data[$row['JOB_NO']]['job_data']=$row["BUYER_NAME"]."##".$row["JOB_NO_PREFIX_NUM"]."##".$row["STYLE_REF_NO"]."##".$row["ORDER_UOM"]."##".$row['SEASON_BUYER_WISE'];

				$style_data_all[$row['JOB_NO']].=$row["FILE_NO"]."__".$row["GROUPING"]."__".$row["PO_NUMBER"]."__".$row["PUB_SHIPMENT_DATE"]."__".$row["SHIPING_STATUS"]."__".$row["ID"]."***";

				$po_arr[$row['JOB_NO']]['order_quantity']+=$row['ORDER_QUANTITY'];
				$po_arr[$row['JOB_NO']]['order_quantity_set']+=$row['ORDER_QUANTITY_SET'];
				$po_id_string.=$row['ID'].",";
			...
			unset($sql_po);
			$con = connect();
			execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ref_from in (1,2) and ENTRY_FORM=2002");
			//execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ref_from in (8) and ENTRY_FORM=2002");
			oci_commit($con);
			fnc_tempenginepo("GBL_TEMP_ENGINE", $user_id, 2002, 1, $poididarr, $empty_arr);//PO ID 
		//	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 2002, 8, $trimidarr, $empty_arr);//trim_dtla_id
			...
			$app_sql=sql_select("select a.job_no_mst as JOB_NO_MST,a.accessories_type_id as ACCESSORIES_TYPE_ID,a.approval_status as APPROVAL_STATUS from wo_po_trims_approval_info a,gbl_temp_engine g where   a.po_break_down_id=g.ref_val and g.user_id = ".$user_id." and g.ref_from=1 and g.entry_form=2002 and a.status_active=1");
			$app_status_arr=array();
			foreach($app_sql as $row)
			{
				$app_status_arr[$row["JOB_NO_MST"]][$row["ACCESSORIES_TYPE_ID"]]=$row["APPROVAL_STATUS"];
			}
			unset($app_sql);
...
//echo "BB-";  die;
			$costing_arr=array();
		      /* $sql_pre="select a.costing_per as COSTING_PER, a.costing_date as COSTING_DATE, b.id as TRIM_DTLA_ID, b.trim_group as TRIM_GROUP, b.remark as REMARK, b.description as DESCRIPTION, b.brand_sup_ref as BRAND_SUP_REF, b.cons_uom as CONS_UOM, b.cons_dzn_gmts as CONS_DZN_GMTS, b.rate as RATE, b.amount as AMOUNT, b.apvl_req as APVL_REQ, b.nominated_supp as NOMINATED_SUPP, b.insert_date as INSERT_DATE, c.cons as CONS, c.country_id as COUNTRY_ID, c.cons as CONS_CAL, c.po_break_down_id as PO_BREAK_DOWN_ID, b.job_no as JOB_NO
			from wo_pre_cost_mst a, wo_pre_cost_trim_cost_dtls b, wo_pre_cost_trim_co_cons_dtls c,gbl_temp_engine g
			where c.job_id=b.job_id and b.id=c.wo_pre_cost_trim_cost_dtls_id and a.job_id=b.job_id and c.po_break_down_id=g.ref_val and g.user_id = ".$user_id." and g.ref_from=1 and g.entry_form=2002 and c.cons>0 and b.status_active=1 and a.status_active=1 and c.status_active=1 $trm_group_pre_cost_cond 
			...
			if(count($sql_pre_cost)>0)
			{
				foreach($sql_pre_cost as $rowp)
				{
					...
					if($rowp['COSTING_PER']==1) $dzn_qnty=12;
					else if($rowp['COSTING_PER']==3) $dzn_qnty=12*2;
					else if($rowp['COSTING_PER']==4) $dzn_qnty=12*3;
					else if($rowp['COSTING_PER']==5) $dzn_qnty=12*4;
					else $dzn_qnty=1;

					$po_qty=0;  $req_value=0;//$req_qnty=0;
					if($rowp['COUNTRY_ID']==0)
					{
						$po_qty=$po_arr[$rowp['JOB_NO']]['order_quantity'];
						//$req_qnty+=$trim_qty[$rowp['PO_BREAK_DOWN_ID']][$rowp[csf('country_id')]][$rowp['TRIM_DTLA_ID']];
					}
					else
					{
						$country_id= explode(",",$rowp['COUNTRY_ID']);
						for($cou=0;$cou<=count($country_id); $cou++)
						{
							$po_qty+=$sql_po_qty_country_wise_arr[$rowp['PO_BREAK_DOWN_ID']][$country_id[$cou]];
							//$req_qnty+=$trim_qty[$rowp['PO_BREAK_DOWN_ID']][$country_id[$cou]][$rowp['TRIM_DTLA_ID']];
						}

					}
					$req_qnty=$trim_qty[$rowp['JOB_NO']][$rowp['TRIM_DTLA_ID']];
					$req_value=$trim_amount[$rowp['JOB_NO']][$rowp['TRIM_DTLA_ID']];
					//echo $req_qnty.'D';

					$style_data_arr[$rowp['JOB_NO']]['trim_dtla_id'][$rowp['TRIM_DTLA_ID']]=$rowp['TRIM_DTLA_ID'];// for rowspan
					$style_data_arr[$rowp['JOB_NO']]['trim_group'][$rowp['TRIM_GROUP']]=$rowp['TRIM_GROUP'];
					$style_data_arr[$rowp['JOB_NO']][$rowp['TRIM_GROUP']][$rowp['TRIM_DTLA_ID']]=$rowp['TRIM_DTLA_ID']; // for rowspannn
					$style_data_arr[$rowp['JOB_NO']]['trim_group_dtls'][$rowp['TRIM_DTLA_ID']]=$rowp['TRIM_GROUP'];
					$style_data_arr[$rowp['JOB_NO']]['remark'][$rowp['TRIM_DTLA_ID']]=$rowp['REMARK'];
					$style_data_arr[$rowp['JOB_NO']]['brand_sup_ref'][$rowp['TRIM_DTLA_ID']]=$rowp['BRAND_SUP_REF'];
					$style_data_arr[$rowp['JOB_NO']]['apvl_req'][$rowp['TRIM_DTLA_ID']]=$rowp['APVL_REQ'];
					$style_data_arr[$rowp['JOB_NO']]['insert_date'][$rowp['TRIM_DTLA_ID']]=$rowp['INSERT_DATE'];
					$style_data_arr[$rowp['JOB_NO']]['req_qnty'][$rowp['TRIM_DTLA_ID']]+=$req_qnty;
					$style_data_arr[$rowp['JOB_NO']]['req_value'][$rowp['TRIM_DTLA_ID']]+=$req_value;
					$style_data_arr[$rowp['JOB_NO']]['cons_uom'][$rowp['TRIM_DTLA_ID']]=$rowp['CONS_UOM'];
					$style_data_arr[$rowp['JOB_NO']]['trim_group_from'][$rowp['TRIM_DTLA_ID']]="Pre_cost";
					$style_data_arr[$rowp['JOB_NO']]['rate'][$rowp['TRIM_DTLA_ID']]=$rowp['RATE'];
					$style_data_arr[$rowp['JOB_NO']]['description'][$rowp['TRIM_DTLA_ID']]=$rowp['DESCRIPTION'];
					$style_data_arr[$rowp['JOB_NO']]['country_id'][$rowp['TRIM_DTLA_ID']].=$rowp['COUNTRY_ID'].',';
					$style_data_arr[$rowp['JOB_NO']]['avg_cons'][$rowp['TRIM_DTLA_ID']]=$rowp['CONS_DZN_GMTS'];

					$costing_arr[$rowp['JOB_NO']]['costing_per']=$rowp['COSTING_PER'];
					$costing_arr[$rowp['JOB_NO']]['costing_date']=$rowp['COSTING_DATE'];
				}
			}
			else
			{
			...
			if($db_type==2)
			{
// LISTAGG(CAST(a.booking_no AS VARCHAR(4000)), ',') WITHIN GROUP (ORDER BY a.booking_no) as booking_no, LISTAGG(CAST(a.supplier_id || '**' || a.pay_mode AS VARCHAR(4000)), ',') WITHIN GROUP (ORDER BY a.booking_no) as supplier_id, LISTAGG(CAST(b.id AS VARCHAR(4000)), ',') WITHIN GROUP (ORDER BY b.id) as booking_dtls_id
				/*$sql_without_precost=sql_select("select min(a.booking_date) as booking_date, b.job_no b.po_break_down_id, b.trim_group, b.pre_cost_fabric_cost_dtls_id, sum(b.wo_qnty) as wo_qnty, sum(b.amount/b.exchange_rate) as amount, sum(b.rate) as rate
				from wo_booking_mst a, wo_booking_dtls b,gbl_temp_engine g
				where a.item_category=4 and a.booking_no=b.booking_no and b.po_break_down_id=g.ref_val and g.user_id = ".$user_id." and g.ref_from=1 and g.entry_form=2002 and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.company_id=$company_name  $item_group_cond2
				group by b.po_break_down_id,b.trim_group,b.job_no,b.pre_cost_fabric_cost_dtls_id");*///and item_from_precost=2
				//A.ID AS MST_ID,B.ID AS BOOKING_DTLS_ID, A.BOOKING_NO,A.SUPPLIER_ID,A.PAY_MODE,(A.BOOKING_DATE) AS BOOKING_DATE, B.JOB_NO, B.PO_BREAK_DOWN_ID, B.TRIM_GROUP, B.PRE_COST_FABRIC_COST_DTLS_ID, (B.WO_QNTY) AS WO_QNTY, (B.AMOUNT/B.EXCHANGE_RATE) AS AMOUNT, (B.RATE) AS RATE
				$sql_without_precost=sql_select("select a.id as MST_ID,b.id as BOOKING_DTLS_ID, a.BOOKING_NO,a.supplier_id as SUPPLIER_ID,a.pay_mode as PAY_MODE,(a.booking_date) as BOOKING_DATE, b.job_no as JOB_NO, b.po_break_down_id as PO_BREAK_DOWN_ID, b.trim_group as TRIM_GROUP, b.pre_cost_fabric_cost_dtls_id as PRE_COST_FABRIC_COST_DTLS_ID, (b.wo_qnty) as WO_QNTY, (b.amount/b.exchange_rate) as AMOUNT, (b.rate) as RATE,a.fabric_source as FABRIC_SOURCE
				from wo_booking_mst a, wo_booking_dtls b,gbl_temp_engine g
				where a.item_category=4 and a.booking_no=b.booking_no and b.po_break_down_id=g.ref_val and g.user_id = ".$user_id." and g.ref_from=1 and g.entry_form=2002 and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.company_id=$company_name  $item_group_cond2");
			...
			else if($db_type==0)
			{
				...
				from wo_booking_mst a, wo_booking_dtls b
				where a.item_category=4 and a.booking_no=b.booking_no  and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.company_id=$company_name   $item_group_cond2
				group by b.po_break_down_id,b.trim_group,b.job_no,b.pre_cost_fabric_cost_dtls_id");//and item_from_precost=2
			}
			...
			{
				$conversion_factor_rate=$conversion_factor_array[$wo_row_without_precost['TRIM_GROUP']]['con_factor'];
				//$cons_uom=$item_arr[$wo_row_without_precost[csf('trim_group')]]['order_uom'];
				$cons_uom=$conversion_factor_array[$wo_row_without_precost['TRIM_GROUP']]['cons_uom'];
				//$booking_no=implode(",",array_unique(explode(",",$wo_row_without_precost[csf('booking_no')])));
				//$supplier_id=implode(",",array_unique(explode(",",$wo_row_without_precost[csf('supplier_id')])));
				$wo_qnty=$wo_row_without_precost['WO_QNTY'];//*$conversion_factor_rate;
				$amount=$wo_row_without_precost['AMOUNT'];
				$wo_date=$wo_row_without_precost['BOOKING_DATE'];
				$book_id=$wo_row_without_precost['BOOKING_DTLS_ID'];
				$mst_book_id=$wo_row_without_precost['MST_ID'];
				$mst_book_id_Arr[$mst_book_id]=$mst_book_id;
				...
				foreach($booking_id_arr as $book_id)
				{
					...
					$booking_precost_idArr[$wo_row_without_precost['PRE_COST_FABRIC_COST_DTLS_ID']]=$wo_row_without_precost['PRE_COST_FABRIC_COST_DTLS_ID']; 

				if($wo_row_without_precost['PRE_COST_FABRIC_COST_DTLS_ID'] =="" || $wo_row_without_precost['PRE_COST_FABRIC_COST_DTLS_ID'] ==0)
				{
					//echo $wo_row_without_precost[csf('trim_group')];
					//echo "V,";
					$trim_dtla_id=max($style_data_arr[$wo_row_without_precost['JOB_NO']]['trim_dtla_id'])+1;
					$style_data_arr[$wo_row_without_precost['JOB_NO']]['trim_dtla_id'][$trim_dtla_id]=$trim_dtla_id;// for rowspan
					$style_data_arr[$wo_row_without_precost['JOB_NO']]['trim_group'][$wo_row_without_precost['TRIM_GROUP']]=$wo_row_without_precost['TRIM_GROUP'];
					$style_data_arr[$wo_row_without_precost['JOB_NO']][$wo_row_without_precost['TRIM_GROUP']][$trim_dtla_id]=$trim_dtla_id;// for rowspannn
					$style_data_arr[$wo_row_without_precost['JOB_NO']]['trim_group_dtls'][$trim_dtla_id]=$wo_row_without_precost['TRIM_GROUP'];
					$style_data_arr[$wo_row_without_precost['JOB_NO']]['cons_uom'][$trim_dtla_id]=$cons_uom;

					$style_data_arr[$wo_row_without_precost['JOB_NO']]['trim_group_from'][$trim_dtla_id]="Booking Without Pre_cost";
				}
				else
				{
					$trim_dtla_id=$wo_row_without_precost['PRE_COST_FABRIC_COST_DTLS_ID'];
					...
					$style_data_arr[$wo_row_without_precost['JOB_NO']]['trim_group_from'][$trim_dtla_id]="Booking With Pre_cost";
				}
				$style_data_arr[$wo_row_without_precost['JOB_NO']]['wo_qnty'][$trim_dtla_id]+=$wo_qnty;
				$style_data_arr[$wo_row_without_precost['JOB_NO']]['amount'][$trim_dtla_id]+=$amount;
				$style_data_arr[$wo_row_without_precost['JOB_NO']]['wo_date'][$trim_dtla_id]=$wo_date;
				$style_data_arr[$wo_row_without_precost['JOB_NO']]['wo_qnty_trim_group'][$wo_row_without_precost['TRIM_GROUP']]+=$wo_qnty;


				$style_data_arr2[$wo_row_without_precost['JOB_NO']]['booking_no'][$trim_dtla_id].=$wo_row_without_precost['BOOKING_NO'].",";
				$style_data_arr[$wo_row_without_precost['JOB_NO']]['booking_no'][$trim_dtla_id].=$wo_row_without_precost['BOOKING_NO'].",";
				$style_data_arr[$wo_row_without_precost['JOB_NO']]['supplier_id'][$trim_dtla_id].=$wo_row_without_precost['SUPPLIER_ID'].'**'.$wo_row_without_precost['PAY_MODE'].",";
				$style_data_arr[$wo_row_without_precost['JOB_NO']]['conversion_factor_rate'][$trim_dtla_id]=$conversion_factor_rate;
			}
			unset($sql_without_precost);
			//echo implode(",",$booking_precost_idArr);die;
			//die;
			//$con = connect();
			//execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ref_from in (2) and ENTRY_FORM=2002");
			//execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ref_from in (8) and ENTRY_FORM=2002");
			oci_commit($con);
			fnc_tempenginepo("GBL_TEMP_ENGINE", $user_id, 2002, 2, $mst_book_id_Arr, $empty_arr);//PO ID 
			
			$receive_qty_data=sql_select("select b.po_breakdown_id as PO_BREAKDOWN_ID, a.item_group_id as ITEM_GROUP_ID, b.quantity as QUANTITY, b.order_rate as RATE, c.exchange_rate as EXCHANGE_RATE
			from  inv_receive_master c,product_details_master d, inv_trims_entry_dtls a, order_wise_pro_details b,gbl_temp_engine g
			where a.mst_id=c.id and a.trans_id=b.trans_id and a.prod_id=d.id and b.po_breakdown_id=g.ref_val and g.user_id = ".$user_id." and g.ref_from=1 and g.entry_form=2002   and b.trans_type=1 and b.entry_form=24 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0   $item_group_cond3  "); 
		 
			foreach($receive_qty_data as $row)
			{
				if($style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['trim_group'][$row['ITEM_GROUP_ID']]=="" || $style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['trim_group'][$row['ITEM_GROUP_ID']]==0)
				{
					//echo $row[csf('item_group_id')];
					$cons_uom=$conversion_factor_array[$row['ITEM_GROUP_ID']]['cons_uom'];

					$trim_dtla_id=max($style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['trim_dtla_id'])+1;
					$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['trim_dtla_id'][$trim_dtla_id]=$trim_dtla_id;// for rowspan
					$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['trim_group'][$row['ITEM_GROUP_ID']]=$row['ITEM_GROUP_ID'];
					$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]][$row['ITEM_GROUP_ID']][$trim_dtla_id]=$trim_dtla_id;// for rowspannn
					$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['trim_group_dtls'][$trim_dtla_id]=$row['ITEM_GROUP_ID'];
					$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['cons_uom'][$trim_dtla_id]=$cons_uom;

					$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['trim_group_from'][$trim_dtla_id]="Trim Receive";
				}
				$amount=0;  $amount=($row['QUANTITY']*$row['RATE']);
				$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['inhouse_qnty'][$row['ITEM_GROUP_ID']]+=$row['QUANTITY'];
				$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['inhouse_amount'][$row['ITEM_GROUP_ID']]+=$amount;//$row[csf('amount')];
			}
			...
			//D.PO_BREAKDOWN_ID, C.ITEM_GROUP_ID, D.QUANTITY AS QUANTITY, D.ORDER_RATE AS RATE
			$receive_rtn_qty_data=sql_select("select d.po_breakdown_id as PO_BREAKDOWN_ID, c.item_group_id as ITEM_GROUP_ID, d.quantity as QUANTITY, d.order_rate as RATE
			from product_details_master c, order_wise_pro_details d,gbl_temp_engine g
			where d.prod_id=c.id and d.po_breakdown_id=g.ref_val and g.user_id = ".$user_id." and g.ref_from=1 and g.entry_form=2002  and d.trans_type=3 and d.entry_form=49 and d.status_active=1 and d.is_deleted=0 ");
			foreach($receive_rtn_qty_data as $row)
			{
				$receive_rtn_amount=0;
				//$conv_quantity=$row[csf('quantity')]/$conversion_factor_array[$row[csf('item_group_id')]]['con_factor'];
				$conv_quantity=$row['QUANTITY'];
				$receive_rtn_amount=$conv_quantity*$row['RATE'];
				$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['receive_rtn_qty'][$row['ITEM_GROUP_ID']]+=$conv_quantity;
				$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['receive_rtn_amount'][$row['ITEM_GROUP_ID']]+=$receive_rtn_amount;
			}
			//echo "<pre>";print_r($style_data_arr);
			unset($receive_rtn_qty_data);

			$transfer_qty_data=sql_select("select d.po_breakdown_id as PO_BREAKDOWN_ID, c.item_group_id as ITEM_GROUP_ID, 
			(case when d.trans_type=5 then d.quantity else 0 end)-(case when d.trans_type=6 then d.quantity else 0 end) as QUANTITY ,
			(case when d.trans_type=5 then d.quantity else 0 end) as IN_QTY,
			(case when d.trans_type=6 then d.quantity else 0 end) as OUT_QTY,
			(case when d.trans_type=5 then (d.quantity*d.order_rate) else 0 end) as IN_AMOUNT,
			(case when d.trans_type=6 then (d.quantity*d.order_rate) else 0 end) as OUT_AMOUNT
			from product_details_master c,order_wise_pro_details d,gbl_temp_engine g
			where d.prod_id=c.id and d.po_breakdown_id=g.ref_val and g.user_id = ".$user_id." and g.ref_from=1 and g.entry_form=2002 and d.trans_type in(5,6) and d.entry_form=78 and d.status_active=1 and d.is_deleted=0 ");
			foreach($transfer_qty_data as $row)
			{
				$transfe_amount=0;
				$transfe_amount=$row['IN_AMOUNT']-$row['OUT_AMOUNT'];
				$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['transfe_qty'][$row['ITEM_GROUP_ID']]+=$row['QUANTITY'];
				$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['transfe_in'][$row['ITEM_GROUP_ID']]+=$row['QUANTITY'];
				$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['transfe_out'][$row['ITEM_GROUP_ID']]+=$row['OUT_QTY'];

				$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['transfe_amount'][$row['ITEM_GROUP_ID']]+=$transfe_amount;
				$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['transfe_in_amount'][$row['ITEM_GROUP_ID']]+=$row['IN_AMOUNT'];
				$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['transfe_out_amount'][$row['ITEM_GROUP_ID']]+=$row['OUT_AMOUNT'];
			}
			unset($transfer_qty_data);
//B.PO_BREAKDOWN_ID, P.ITEM_GROUP_ID, SUM(B.QUANTITY) AS QUANTITY, SUM(B.QUANTITY*B.ORDER_RATE) AS AMOUN
			$issue_qty_data=sql_select("select b.po_breakdown_id as PO_BREAKDOWN_ID, p.item_group_id as ITEM_GROUP_ID, (b.quantity) as QUANTITY, (b.quantity*b.order_rate) as AMOUNT
			from  inv_issue_master d,product_details_master p,inv_trims_issue_dtls a , order_wise_pro_details b,gbl_temp_engine g
			where a.mst_id=d.id and a.prod_id=p.id and a.trans_id=b.trans_id and b.po_breakdown_id=g.ref_val and g.user_id = ".$user_id." and g.ref_from=1 and g.entry_form=2002 and b.trans_type=2 and d.entry_form=25 and b.entry_form=25 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 ");

			foreach($issue_qty_data as $row)
			{
				$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['issue_qty'][$row['ITEM_GROUP_ID']]+=$row['QUANTITY'];
				$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['issue_amount'][$row['ITEM_GROUP_ID']]+=$row['AMOUNT'];
			}

			unset($issue_qty_data);
//D.PO_BREAKDOWN_ID, C.ITEM_GROUP_ID, D.QUANTITY AS QUANTITY, (D.QUANTITY*D.ORDER_RATE) AS AMOUNT
			$issue_rtn_qty_data=sql_select("select d.po_breakdown_id as PO_BREAKDOWN_ID, c.item_group_id as ITEM_GROUP_ID, d.quantity as QUANTITY, 
			(d.quantity*d.order_rate) as AMOUNT
			from product_details_master c,order_wise_pro_details d,gbl_temp_engine g
			where d.prod_id=c.id and d.po_breakdown_id=g.ref_val and g.user_id = ".$user_id." and g.ref_from=1 and g.entry_form=2002 and d.trans_type=4 and d.entry_form=73 and d.status_active=1 and d.is_deleted=0 ");
			foreach($issue_rtn_qty_data as $row)
			{
				$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['issue_rtn_qty'][$row['ITEM_GROUP_ID']]+=$row['QUANTITY'];
				$style_data_arr[$po_job_arr[$row['PO_BREAKDOWN_ID']]]['issue_rtn_amount'][$row['ITEM_GROUP_ID']]+=$row['QUANTITY'];
			}
			unset($issue_rtn_qty_data);
//A.PI_NUMBER, B.WORK_ORDER_NO, B.WORK_ORDER_DTLS_ID
			$sql_wo_pi=sql_select("select a.pi_number as PI_NUMBER, b.work_order_no as WORK_ORDER_NO, b.work_order_dtls_id as WORK_ORDER_DTLS_ID from com_pi_master_details a, com_pi_item_details b,gbl_temp_engine g where a.id=b.pi_id and a.item_category_id=4 and a.status_active=1 and a.importer_id=$company_name and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.work_order_dtls_id>0
			and b.work_order_id=g.ref_val and g.user_id = ".$user_id." and g.ref_from=2 and g.entry_form=2002");
			$pi_arr=array();
			foreach($sql_wo_pi as $rowPi)
			{
				if($tem_pi[$rowPi['WORK_ORDER_NO']]=="")
				{
					$tem_pi[$rowPi['WORK_ORDER_NO']]=$rowPi['PI_NUMBER'];
					$pi_arr[$rowPi['WORK_ORDER_NO']].=$rowPi['PI_NUMBER'].'**';
				}
		...
		disconnect($con);
			...
			foreach($style_data_arr as $key=>$value)
			{
				...
				<tr bgcolor="<? echo $bgcolor;?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>">
                    ...
                    <a href='#report_details' onclick="generate_woven_report('<? echo $company_name; ?>','<? echo $job; ?>', '<? echo $job_data[0]; ?>','<? echo $job_data[2]; ?>','<? echo change_date_format($costing_arr[$job]['costing_date']); ?>','<? echo rtrim($po_id_all,','); ?>','<? echo $costing_arr[$job]['costing_per']; ?>','<? echo $woven_print_button_action;?>');"> <? $po_number=$po_no; $po_id=$poId_all; echo $po_number;
                    ...
                    foreach($value['trim_group'] as $key_trim=>$value_trim)
                    {
						...
						foreach($value[$key_trim] as $key_trim1=>$value_trim1)
						{
							...
							if($gg==1)
							{
                                ...
                                <td width="140"><p style="word-wrap:break-word"><? echo $value['description'][$key_trim1]; ?></p></td>
							...
							else
							{
                                ...
                                <td width="100" align="right" ><p><? echo number_format($trim_req_value-$value['amount'][$key_trim1],2,'.',''); ?></p></td>
	...
	<fieldset style="width:870px; margin-left:3px">
        ...
        <td align="center" colspan="9"><strong>WO Summary</strong> </td>
			...
			<table border="1" class="rpt_table" rules="all" width="850" cellpadding="0" cellspacing="0" align="center">
				...
				$woven_print_report_format=return_field_value("format_id","lib_report_template","template_name ='".$cbo_company_name."' and module_id=2 and report_id=219 and is_deleted=0 and status_active=1");
				//echo $print_report_format;
					$wvnreportArr= explode(',',$woven_print_report_format);
					$wvnreport=$wvnreportArr[0];

					if($wvnreport==183){$reporAction="show_trim_booking_report2";}
					elseif($wvnreport==67){$reporAction="show_trim_booking_report";}
					elseif($wvnreport==177){$reporAction="show_trim_booking_report4";}
					elseif($wvnreport==235){$reporAction="show_trim_booking_report9";}
					elseif($wvnreport==774){$reporAction="show_trim_booking_report_wg";}
					elseif($wvnreport==85){$reporAction="print_t";}
					elseif($wvnreport==746){$reporAction="print_t7";}
					elseif($wvnreport==72){$reporAction="show_trim_booking_report6";}
