...
{
	echo create_drop_down( "cbo_brand_id", 130, "select id, brand_name from lib_buyer_brand brand where buyer_id='$data' and status_active =1 and is_deleted=0 $brand_cond order by brand_name ASC","id,brand_name", 1, "-- All --", $selected, "" );	 
	exit();
}
if($action=="load_drop_down_buyer")
{
	echo create_drop_down( "cbo_buyer_name", 130, "select buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond and buy.id in (select buyer_id from lib_buyer_party_type where party_type in (1,3,21,90)) order by buy.buyer_name","id,buyer_name", 1, "-- All Buyer --", $selected, "load_drop_down( 'requires/order_history_report_controller',this.value, 'load_drop_season', 'season_td' ); load_drop_down( 'requires/order_history_report_controller',this.value, 'load_drop_brand', 'brand_td' );" );   	 
	exit();
}
...
if($action=="report_generate_2")//MD. SAKIBUL ISLAM 
{ 
	$process = array( &$_POST );
	...
	$buyer_id_cond="";
		...
		if(str_replace("'","",$cbo_category_by)==3)
		{
				...
				$txt_job_no=trim($txt_job_no);
					...
					if(str_replace("'","",$txt_date_from)!="" && str_replace("'","",$txt_date_to)!="")
					{
						...
						//$date_cond="and trunc(b.update_date) >= '$start_date' and trunc(b.update_date) <= '$end_date'";
						$date_diff = diff_in_weeks_and_days($start_date,$end_date,'d');
						if($date_diff ==1)
						{
					...
					// $sql_data=sql_select("SELECT to_char(b.insert_date,'YYYY') as year,a.job_no,a.job_no_prefix_num,a.company_name,a.buyer_name,a.style_ref_no,a.team_leader,a.dealing_marchant,a.season_buyer_wise, b.grouping, a.gmts_item_id,a.job_quantity, b.id,b.is_confirmed,b.po_number,b.po_received_date,b.pub_shipment_date,b.shipment_date,b.factory_received_date,b.po_quantity,b.unit_price,b.excess_cut,b.plan_cut,b.pack_handover_date,b.status_active,b.projected_po_id,b.packing,b.details_remarks,b.file_no,b.updated_by,b.update_date,b.insert_date, a.style_description, a.set_break_down from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst   and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1   and a.company_name=$company_name $buyer_id_cond $date_cond $style_ref_cond $season_cond $internal_ref_cond $jobcond $ordercond $team_cond");
		...
		$season_cond="";
		if(str_replace("'","",$cbo_season_id)!=0) $season_cond=" and a.season_buyer_wise=".str_replace("'","",$cbo_season_id)."  ";
		$team_cond="";
		...
		if($template==1)
		{
				...
				?>
					<div style="width:1102px" >
						...
						<div style="width:1120px; max-height:400px; overflow-y:scroll" id="scroll_body">
						<table class="rpt_table" width="1100" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body">
				<?
				...
				?>
							...
							<tr align="center" bgcolor="<? echo $pre_bgcolor;?>">
								...
								<td width="65" style="word-wrap:break-word; word-break: break-all;">
								<?
								...
								<td width="65" style="word-wrap:break-word; word-break: break-all; color:"><? echo $company_library[$row_data[csf('company_name')]];?></td>
								
								<td width="100" style="word-wrap:break-word; word-break: break-all; color:"><? echo $team_name_library[$row_data[csf('team_leader')]]; ?></td>
								
								<td width="50" style="word-wrap:break-word; word-break: break-all; color:"><? echo $buyer_short_name_library[$row_data[csf('buyer_name')]]; ?></td>
								
								<td width="45" style="word-wrap:break-word; word-break: break-all; color:" title="<? echo $row_data[csf('job_no')];  ?>"><? echo $row_data[csf('job_no_prefix_num')]; ?></td>
								
								<td width="45" style="word-wrap:break-word; word-break: break-all;"  bgcolor="<? echo $pre_bgcolor; ?>"><? echo $row_data[csf('year')]; ?></td>
								...
								<td width="90" style="word-wrap:break-word; word-break: break-all;" ><? echo $value['po_no']; ?></td>
								...
								///}
								//else{
								//$prev_pack_handover_date= change_date_format($prive_data_array[$row_data[csf('id')]]['pack_handover_date'],'dd-mm-yyyy','-');
								//	echo $prev_pack_handover_date;
								//}
								?>
								</td>
				...
				?>
					<tr align="center" bgcolor="#EFEF00" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>">
						...
						<td width="40"><? echo $i."."."1"; ?></td>
					
						<td width="65" style="word-wrap:break-word; word-break: break-all;"><? echo "Current Data" ?></td>
						
						<td width="65" style="word-wrap:break-word; word-break: break-all;"><? echo $company_library[$row_data[csf('company_name')]];?></td>
						...
						<td width="45" style="word-wrap:break-word; word-break: break-all;" title=""><? echo $row_data[csf('year')]; ?></td>
						<? 		
										$current=$style_ref_no_arr[$row_data[csf('job_id')]]['current'];
										$previous=$style_ref_no_arr[$row_data[csf('job_id')]]['previous']; 
						?>
					
						<td width="100" style="word-wrap:break-word; word-break: break-all;"  bgcolor="<? echo $style_ref_color;//$style_ref_colorCHangeArr[$row_data[csf('id')]];?>"><? echo $current; //$row_data[csf('style_ref_no')]; ?></td>
						
						<td width="90" style="word-wrap:break-word; word-break: break-all;" bgcolor="<? echo $po_number_colorCHangeArr[$row_data[csf('id')]]; ?>"><? echo $row_data[csf('po_number')]; ?></td>
					
						<td width="60" style="word-wrap:break-word; word-break: break-all; text-align:right" title="<?  if($rowSpan>0){echo $row_data[csf('po_quantity')]-$previous_po_qty_net;}  ?>" bgcolor="<? echo $previous_po_qty_colorCHangeArr[$row_data[csf('id')]]; ?>">
						<? echo $row_data[csf('po_quantity')]; ?>
						</td>
					
						<td width="70" style="word-wrap:break-word; word-break: break-all;" bgcolor="<? echo $po_received_date_colorCHangeArr[$row_data[csf('id')]]; ?>">
						<?
						...
						<td width="70" style="word-wrap:break-word; word-break: break-all;" bgcolor="<? echo $pubshipdate_colorCHangeArr[$row_data[csf('id')]]; ?>">
						<?
						...
						<td width="70" style="word-wrap:break-word; word-break: break-all;" bgcolor="<? echo $shipment_date_colorCHangeArr[$row_data[csf('id')]]; ?>">
						<?
						...
						<td width="70" style="word-wrap:break-word; word-break: break-all;" bgcolor="<? echo $pack_handover_date_colorCHangeArr[$row_data[csf('id')]]; ?>">
						<?
						...
						<td width="90" style="word-wrap:break-word; word-break: break-all;" ><? echo $order_status[$row_data[csf('is_confirmed')]]; ?></td>
					
						<td width="70" style="word-wrap:break-word; word-break: break-all;" bgcolor="<? //echo $order_status_colorCHangeArr[$row_data[csf('id')]]; ?>">
...
{ 
	...
	if(str_replace("'","",$cbo_category_by)==3)
	{
		if(str_replace("'","",$txt_date_from)!="" && str_replace("'","",$txt_date_to)!="")
		{
			$start_date=(str_replace("'","",$txt_date_from));
			$end_date=(str_replace("'","",$txt_date_to));
			//$date_cond="and b.update_date between '$start_date' and '$end_date'";
			$date_cond="and trunc(b.update_date) >= '$start_date' and trunc(b.update_date) <= '$end_date'";
			$date_diff = diff_in_weeks_and_days($start_date,$end_date,'d');
			if($date_diff ==1)
			{
	...
	//if(str_replace("'","",$txt_internal_ref)!="") $internal_ref_cond=" and a.style_ref_no like '%".str_replace("'","",$txt_internal_ref)."%'"; else $internal_ref_cond="";
	...
	$po_id_arr=array();
	$report_data_arr=array();
	$sql_data=sql_select("SELECT a.job_no,a.job_no_prefix_num,a.company_name,a.buyer_name,a.brand_id,a.style_ref_no,a.team_leader,a.dealing_marchant,a.season_buyer_wise, b.grouping, a.gmts_item_id,a.job_quantity, b.id,b.is_confirmed,b.po_number,b.po_received_date,b.pub_shipment_date,b.shipment_date,b.factory_received_date,b.po_quantity,b.unit_price,b.excess_cut,b.plan_cut,b.status_active,b.projected_po_id,b.packing,b.details_remarks,b.file_no,b.updated_by,b.update_date,b.insert_date, a.style_description, a.set_break_down from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst   and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1   and a.company_name=$company_name $brand_id_cond $buyer_id_cond $date_cond $style_ref_cond $season_cond $internal_ref_cond $jobcond $ordercond $team_cond");
	foreach( $sql_data as $row_data)
	{
	$po_id_arr[$row_data[csf('id')]]=$row_data[csf('id')];
	$po_wise_buyer[$row_data[csf('id')]]=$row_data[csf('buyer_name')];
        ...
        <table class="rpt_table" width="850" cellpadding="0" cellspacing="0" border="1" rules="all">
			...
			?>
             <tr bgcolor="<? echo $bgcolor;?>">
                    ...
                    <td width="65" style="word-break:break-all"><p><? echo $buyer_short_name_library[$key]; ?></p></td>
					...
					<?
					$rate_of_original=fn_number_format($original_array_buyer_wise[$key]['previous_po_amount']/$original_array_buyer_wise[$key]['previous_po_qty'],2);
					?>
                    <td width="50" align="right"><? echo $rate_of_original; ?></td>
					<td width="100" align="right"><? echo number_format($original_array_buyer_wise[$key]['previous_po_amount'],2); $original_po_amount_tot+=$original_array_buyer_wise[$key]['previous_po_amount']?></td>
                    <td width="80" align="right"><? echo number_format($buyer_wise_data[$key]['po_quantity'],2); $current_po_qty_tot+=$buyer_wise_data[$key]['po_quantity'];?></td>
                    <td width="50" align="right"><? echo number_format($buyer_wise_data[$key]['po_price']/$buyer_wise_data[$key]['po_quantity'],2); ?></td>
					<td width="100" align="right"><? echo number_format($buyer_wise_data[$key]['po_price'],2); $current_po_amount_tot+=$buyer_wise_data[$key]['po_price'];?></td>
                    <td width="80" align="right"><? echo number_format($buyer_wise_data[$key]['po_quantity']-$original_array_buyer_wise[$key]['previous_po_qty'],2); ?></td>
                    <td width="50" align="right"> <? echo fn_number_format(($buyer_wise_data[$key]['po_price']/$buyer_wise_data[$key]['po_quantity'])-($rate_of_original),2);?></td>
					<td width="100" align="right"><? echo number_format($buyer_wise_data[$key]['po_price']-$original_array_buyer_wise[$key]['previous_po_amount'],2); ?></td>
                 </tr>
            <?
			$bi++;
			}
			...
			<table width="2430">
				<tr class="form_caption">
					<td colspan="28" align="center">Order Edit History</td>
				</tr>
				<tr class="form_caption">
			...
			<table class="rpt_table" width="2430" cellpadding="0" cellspacing="0" border="1" rules="all">
				<thead>
					<th width="40">SL</th>
					...
					<th width="70">Fac. Recv. Date</th>
					<th width="60">PO Qty.</th>
                    <th width="60">Change Po  Qty</th>
					<th width="60">Avg. Price</th>
                    <th width="50">Change  Avg. Price</th>
					<th width="60">Exce. Cut %</th>
					<th width="60">PlanCut</th>
                    
                    
					<th width="60">Proj. Po</th>
					<th width="55">Packing</th>
			...
			<div style="width:2430px; max-height:400px; overflow-y:scroll" id="scroll_body">
			<table class="rpt_table" width="2430" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body">
	<?
                ...
                <tr align="center" bgcolor="#FFCCCC" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>">
					<td width="40"><? echo $i; ?></td>
                    <td width="70" style="word-wrap:break-word; word-break: break-all;"><? echo $company_library[$row_data[csf('company_name')]];?></td>
					<td width="45" style="word-wrap:break-word; word-break: break-all;" title="<? echo $row_data[csf('job_no')]; ?>"><? echo $row_data[csf('job_no_prefix_num')]; ?></td> 
                    <td width="50" style="word-wrap:break-word; word-break: break-all;"><? echo $buyer_short_name_library[$row_data[csf('buyer_name')]]; ?></td>
					<td width="40" style="word-wrap:break-word; word-break: break-all;"><? echo $brand_arr[$row_data[csf('brand_id')]]; ?></td>
					<td width="100" style="word-wrap:break-word; word-break: break-all;"><? echo $row_data[csf('grouping')]; ?></td>
					<td width="100" style="word-wrap:break-word; word-break: break-all;"><? echo $row_data[csf('style_ref_no')]; ?></td>
					<td width="100" style="word-wrap:break-word; word-break: break-all;"><? echo $season_arr[$row_data[csf('season_buyer_wise')]]; ?></td>
					<td width="100" style="word-wrap:break-word; word-break: break-all;"><? echo $row_data[csf('style_description')]; ?></td>
					<td width="100" style="word-wrap:break-word; word-break: break-all;"><? echo $item_str; ?></td>
					...
					<td width="60" style="word-wrap:break-word; word-break: break-all; text-align:right" title="<?  if($rowSpan>0){echo $row_data[csf('unit_price')]-$avg_price_net;}  ?>">
					<? echo number_format($row_data[csf('unit_price')],2); ?>
                    </td>
                    <td width="50" style="word-wrap:break-word; word-break: break-all; text-align:right" >
                   ...
                   <? echo $row_data[csf('projected_po_id')]; ?>
					...
					<td width="40"><? echo $i.".".$ii; ?></td>
					
					
                     <td width="70" style="word-wrap:break-word; word-break: break-all; color:<? echo $bgcolor;?>"><? echo $company_library[$row_data[csf('company_name')]];?></td>
					...
					<td width="40" style="word-wrap:break-word; word-break: break-all; color:<? echo $bgcolor;?>"><? echo $brand_arr[$row_data[csf('brand_id')]]; ?></td>
                    <td width="100" style="word-wrap:break-word; word-break: break-all;"><? echo $row_data[csf('grouping')]; ?></td>
                   ...
                   <td width="100" style="word-wrap:break-word; word-break: break-all;"><? echo $row_data[csf('style_description')]; ?></td>
                    ...
                    <td width="50" style="word-wrap:break-word; word-break: break-all; text-align:right" bgcolor="<? echo $avg_price_color; ?>">
                   ...
                   <? echo $value['projected_po']; ?>
					...
					<td width="55" style="word-wrap:break-word; word-break: break-all;" bgcolor="<? echo $packing_color; ?>">
					...
					<? echo $row_status[$value['status']]; ?>
                      ...
                      <td width="70" style="word-wrap:break-word; word-break: break-all;" bgcolor="<? echo $status_color; ?>">
					<?  ?>
                    </td>
                    ...
                    <td width="" style="word-break: break-all;" bgcolor="<? echo $remarks_color; ?>">
                    <p>
                      <? echo $value['remarks']; ?>
                      </p>
                    </td>
