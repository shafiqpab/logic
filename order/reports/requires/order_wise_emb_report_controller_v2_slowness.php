...
{ 
	...
	{		 
		...
		$sql = "SELECT a.job_no,a.style_ref_no,a.buyer_name,a.client_id as buyer_client,a.company_name, b.id as po_id,b.po_number,c.color_number_id,c.country_ship_date,c.country_id,c.item_number_id,f.location,f.embel_name,f.serving_company, h.working_company_id,f.production_source,h.working_location_id,g.cut_no as cutting_no,h.production_type,h.manual_challan_no,h.delivery_date
				...
				where a.id=b.job_id  and b.id=c.po_break_down_id and b.id=f.po_break_down_id and f.id=g.mst_id and h.id=g.delivery_mst_id and g.color_size_break_down_id=c.id and h.status_active=1 and a.status_active=1 and b.status_active=1 and c.status_active=1 and f.status_active=1  and g.status_active=1  $sql_cond and f.production_type in(2,3) and g.production_type in(2,3) and h.production_type in(2,3)";		
		// echo $sql;die();		
		...
		foreach ($sql_res as $row) 
		{
			...
			$main_array[$row[csf('buyer_name')]][$row[csf('style_ref_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$row[csf('item_number_id')]][$row[csf('color_number_id')]][$row[csf('cutting_no')]][$row[csf('serving_company')]][$row[csf('embel_name')]]['production_source'] = $row[csf('production_source')];
				...
				if($c==1 && $row[csf('manual_challan_no')] !=""){
					...
					$c++;
				}else if($row[csf('manual_challan_no')] !=""){
					$main_array[$row[csf('buyer_name')]][$row[csf('style_ref_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$row[csf('item_number_id')]][$row[csf('color_number_id')]][$row[csf('cutting_no')]][$row[csf('serving_company')]][$row[csf('embel_name')]]['manual_challan_no'] .=",".$row[csf('manual_challan_no')];
				}
		...
		// echo "</pre>";
		// =================================== FOR ORDER QNTY ============================
		$sql_order = "SELECT a.style_ref_no,a.buyer_name,b.id as po_id,c.color_number_id,c.country_id,c.item_number_id,sum(c.order_quantity) as order_qnty
				from wo_po_details_master a,wo_po_break_down b,wo_po_color_size_breakdown c
				where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id and a.status_active=1 and b.status_active=1 and c.status_active=1 and b.id in($poId)
				GROUP BY a.style_ref_no,a.buyer_name,b.id,c.color_number_id,c.country_id,c.item_number_id";
		$sql_order_res = sql_select($sql_order);
		$order_qnty_array = array();
		foreach ($sql_order_res as $row) 
		{
			$order_qnty_array[$row[csf('buyer_name')]][$row[csf('style_ref_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$row[csf('item_number_id')]][$row[csf('color_number_id')]] = $row[csf('order_qnty')];
		}
		...
		$prod_sql= "SELECT c.po_break_down_id as po_id, a.color_number_id,a.item_number_id, a.country_id,e.style_ref_no,e.buyer_name,d.cut_no,c.embel_name,
			NVL(sum(CASE WHEN d.production_type ='1'  and c.production_type ='1' $prod_date THEN d.production_qnty ELSE 0 END),0) AS cutting_qnty,
				
			NVL(sum(CASE WHEN d.production_type ='2' $emb_type_cond $source_cond $prod_date THEN d.production_qnty ELSE 0 END),0) AS issue_qnty,
			NVL(sum(CASE WHEN d.production_type ='3' $emb_type_cond $source_cond $prod_date THEN d.production_qnty ELSE 0 END),0) AS recive_qnty,	

			NVL(sum(CASE WHEN d.production_type in (2,3) and c.production_type in (2,3)  and d.is_rescan =0 $prod_date THEN d.reject_qty ELSE 0 END),0) AS reject_qnty,
			NVL(sum(CASE WHEN d.production_type in (2,3) and c.production_type in (2,3)  and d.is_rescan =1 $prod_date THEN d.production_qnty ELSE 0 END),0) AS resc_qcpass			
				  
			from 
				pro_garments_production_mst c,pro_garments_production_dtls d,wo_po_color_size_breakdown a,wo_po_details_master e 
			where  c.id=d.mst_id and a.job_no_mst=e.job_no  and a.id=d.color_size_break_down_id and c.po_break_down_id in($poId) and
				c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0  and e.status_active=1 and e.is_deleted=0  
			group by c.po_break_down_id, a.color_number_id,a.item_number_id, a.country_id,e.style_ref_no,e.buyer_name,d.cut_no,c.embel_name";

			$prod_sql_res = sql_select($prod_sql);
			$prod_qnty_array = array();
			foreach ($prod_sql_res as $row) 
			{
				$prod_qnty_array[$row[csf('buyer_name')]][$row[csf('style_ref_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$row[csf('item_number_id')]][$row[csf('color_number_id')]][$row[csf('cut_no')]][$row[csf('embel_name')]]['cutting_qnty'] = $row[csf('cutting_qnty')];

	        	$prod_qnty_array[$row[csf('buyer_name')]][$row[csf('style_ref_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$row[csf('item_number_id')]][$row[csf('color_number_id')]][$row[csf('cut_no')]][$row[csf('embel_name')]]['issue_qnty'] = $row[csf('issue_qnty')];

	        	$prod_qnty_array[$row[csf('buyer_name')]][$row[csf('style_ref_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$row[csf('item_number_id')]][$row[csf('color_number_id')]][$row[csf('cut_no')]][$row[csf('embel_name')]]['recive_qnty'] = $row[csf('recive_qnty')];

	        	$prod_qnty_array[$row[csf('buyer_name')]][$row[csf('style_ref_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$row[csf('item_number_id')]][$row[csf('color_number_id')]][$row[csf('cut_no')]][$row[csf('embel_name')]]['reject_qty'] = $row[csf('reject_qnty')];

	        	$prod_qnty_array[$row[csf('buyer_name')]][$row[csf('style_ref_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$row[csf('item_number_id')]][$row[csf('color_number_id')]][$row[csf('cut_no')]][$row[csf('embel_name')]]['resc_qcpass'] = $row[csf('resc_qcpass')];
			}
			// echo "<pre>";
			// print_r($prod_qnty_array);
			// echo "</pre>";

		// =========================================== FOR CUT AND LAY ANTY ====================================
		$cutting_sql="SELECT e.country_id,d.color_id,d.gmt_item_id, e.order_id AS po_id,SUM (e.size_qty) AS cutting_qnty, c.cutting_no
        from ppl_cut_lay_mst c, ppl_cut_lay_dtls d,ppl_cut_lay_bundle e
        where c.id=d.mst_id and c.id=e.mst_id and d.id=e.dtls_id and e.order_id in($poId) and c.status_active=1 and d.status_active=1 and e.status_active=1
        group by e.country_id,d.color_id,d.gmt_item_id, e.order_id, c.cutting_no";
											...
											{ 
												foreach ($color_data as $cutting_no => $cutting_data) 
												{
													...
													{	
														...
														$order_cut_no = $cutting_info_array2[$cutting_no][$po_id][$item_id][$color_id]['order_cut_no'];

														$orderQnty = $order_qnty_array[$buyer_id][$style_no][$po_id][$country_id][$item_id][$color_id];
														...
														$issueQnty = $prod_qnty_array[$buyer_id][$style_no][$po_id][$country_id][$item_id][$color_id][$cutting_no][$embel_name]['issue_qnty'];
														...
														$receiveQnty = $prod_qnty_array[$buyer_id][$style_no][$po_id][$country_id][$item_id][$color_id][$cutting_no][$embel_name]['recive_qnty'];
														...
														$reScQcpass = $prod_qnty_array[$buyer_id][$style_no][$po_id][$country_id][$item_id][$color_id][$cutting_no][$embel_name]['resc_qcpass'];

														$rejectQnty = $prod_qnty_array[$buyer_id][$style_no][$po_id][$country_id][$item_id][$color_id][$cutting_no][$embel_name]['reject_qty'];
