...
<script>
	...
	function fn_report_generated(type)
	{
		...
		{	
			...
			freeze_window(3);
			http.open("POST","requires/qc_statement_report_controller.php",true);
    ...
    {
        //alert(buyer)
        emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/qc_statement_report_controller.php?qc_no='+qc_no+'&buyer='+buyer+'&costing_date='+costing_date+'&ex_rate='+ex_rate+'&offer_qty='+offer_qty+'&action='+action,'Costing Popup', 'width=958px,height=450px,center=1,resize=0','../');
	...
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/qc_statement_report_controller.php?cost_sheet_no='+cost_sheet_no+'&action='+action,'Revise Popup', 'width=650px,height=400px,center=1,resize=0','../');
        emailwindow.onclose=function()
        {
    ...
    <h3 style="width:1020px;" align="center" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3> 
        <div id="content_search_panel" style="width:1020px" > 		 
            ...
            <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr class="general">
                        <td><?=create_drop_down( "cbo_buyer_id", 150, "SELECT buy.id as id ,buy.buyer_name as buyer_name  from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) group by buy.id,buy.buyer_name order by buyer_name","id,buyer_name", 1, "-Select Buyer-", $selected, "load_drop_down( 'requires/qc_statement_report_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'requires/qc_statement_report_controller',this.value, 'load_drop_down_sub_dep', 'sub_td' );"); ?></td>
                        ...
                        <td align="center"><input type="button" id="show_button" class="formbutton" style="width:70px" value="Rpt1" onClick="fn_report_generated(1);" /></td>
                    </tr>
                    <tr>
                        <td align="center" colspan="8"><?=load_month_buttons(1); ?></td>
                        <td align="center"><input type="button" id="show_button" class="formbutton" style="width:70px;" value="Rpt2" onClick="fn_report_generated(2);" /></td>
                        <td align="center"><input type="button" id="show_button" class="formbutton" style="width:70px" value="Margin" onClick="fn_report_generated(3);" /></td>
                    </tr>
                </tbody>
            </table> 
        	</fieldset>
