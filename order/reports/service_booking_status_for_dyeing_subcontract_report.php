...
<script>
	...
	function openmypage_job()
	{
		...
		{	
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/service_booking_status_for_dyeing_subcontract_report_controller.php?data='+data+'&action=job_no_popup', 'Job No Search', 'width=700px,height=420px,center=1,resize=0,scrolling=0','../')
	...
	function openmypage_po()
	{
		...
		{	
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/service_booking_status_for_dyeing_subcontract_report_controller.php?data='+data+'&action=po_no_popup', 'PO No Search', 'width=700px,height=420px,center=1,resize=0,scrolling=0','../')
			emailwindow.onclose=function()
			{
	...
	function fn_report_generated(operation)
	{
		...
		{	
			...
			var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_buyer_id*cbo_supplier_id*cbo_year_id*txt_wo_no*hidd_wo_id*cbo_job_year_id*txt_job_no*hidd_job_id*hidd_po_id*txt_date_from*txt_date_to',"../../")+'&report_title='+report_title;
			freeze_window(3);
			http.open("POST","requires/service_booking_status_for_dyeing_subcontract_report_controller.php",true);
...
function generate_print(bookingdata)
{
	...
	var data="action=show_trim_booking_report&txt_booking_no='"+booking_no+"'&cbo_company_name="+cbo_company_name+'&show_comments='+show_comments;
        ...
        <div id="content_search_panel" >
            ...
            <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                <thead>
                 ...
                 <tbody>
                    <tr class="general">
                        <td><? echo create_drop_down( "cbo_company_id", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down('requires/service_booking_status_for_dyeing_subcontract_report_controller', this.value, 'load_drop_down_buyer', 'buyer_td'); load_drop_down('requires/service_booking_status_for_dyeing_subcontract_report_controller', this.value, 'load_drop_down_supplier', 'supplier_td' );" ); ?></td>
                        <td id="buyer_td"><? echo create_drop_down( "cbo_buyer_id", 120, $blank_array,"", 1, "--Select Buyer--", $selected, "",1,"" ); ?></td>
                        <td id="supplier_td"><? echo create_drop_down( "cbo_supplier_id", 120, $blank_array,"", 1, "--Select Party--", $selected, "",1,"" ); ?></td>
                        <td>
							<? $selected_year=date("Y"); echo create_drop_down( "cbo_year_id", 80, $year,"", 1, "--All Year--", $selected_year, "",0,"","" ); ?>
                        </td>
                        <td>
                            <input type="text" name="txt_wo_no" id="txt_wo_no" class="text_boxes" style="width:80px" placeholder="write" readonly />
                            <input type="hidden" id="hidd_wo_id" name="hidd_wo_id" style="width:50px" />
                        </td>
                        <td>
							<? $selected_year=date("Y"); echo create_drop_down( "cbo_job_year_id", 80, $year,"", 1, "--All Year--", $selected_year, "",0,"","" ); ?>
                        </td>
                        <td>
                        ...
                        <td><input type="text" name="txt_date_to" id="txt_date_to" class="datepicker" style="width:50px"  placeholder="To Date"  value="<? echo date("d-m-Y");?>"></td>
