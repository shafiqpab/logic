...
var permission = '<? echo $permission; ?>';	
	...
	function generate_fabric_report(type,booking_no,company_id,order_id,update_id,fabric_nature,fabric_source,job_no,approved,category,is_short,emb_name,item_number_id,action_type,i,supplier_id,cbo_level,revised_no)
	{
		...
		if(category==2 || category==3)//Knit Finish Fabrics/Woven
		{
			...
			{ 
				...
				if(type==139 || type==89 || type==3 || type==4)
				{
				...
				else if((type==73) || (type==93 || type==269 || type==28 || type==45 || type==53 || type==93  || type==193 || type==719 || type==723 || type==383 || type==304 || type==426 || type==786)) //
				{
				...
				else if(type==140) //Sample Requisition With Booking
				{
					//alert(type);
					report_title = "Sample Requisition With Booking";
				}
				// else if(type==203) //Sample Requisition With Booking
				// {
				// 	//alert(type);
				// 	report_title = "Sample Requisition With Booking";
				// }
				...
				else if((type==73) || (type==93 || type==269 || type==28 || type==45 || type==53 || type==93  || type==193 || type==719 || type==723 || type==383  || type==304 || type==426 || type==786 )) //main fabric booking v2
				{
				...
				{
					//alert(type);
					if(action_type=='sample_requisition_print6')
					{
						freeze_window(5);
						var user_id = "<? echo $user_id; ?>";
						$.ajax({
							url: '../woven_order/requires/sample_requisition_with_booking_controller.php',
							type: 'POST',
							data: data,
						...
						http.open("POST","../woven_order/requires/sample_requisition_with_booking_controller.php",true);
					}
		...
		{
			if(type==87 || type==272 || type==43 || type==262 || type==719  || type==723  || type==178 || type==304 || type==260 || type==492) // trims_booking_multi_job_controllerurmi; entry_form=87
			{
				var show_comment='';
				...
				{
					http.open("POST","../woven_gmts/requires/trims_booking_urmi_controller.php",true);
				}			
			}
		...
		http.onreadystatechange = function(){
			if(http.readyState == 4) 
			{
				...
				if((type==272 || type==492) && category==4 && (file_data[0].length==0 || file_data[0].includes(".xls")))
				{
                ...
                <thead>   
                 ...
                 <tbody>
                    <tr class="general">
                        ...
                        <td><? echo create_drop_down( "cbo_company_id", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-Select Company-", $selected, "load_drop_down( 'requires/wo_or_fabric_booking_report_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );get_php_form_data(this.value,'report_formate_setting','requires/wo_or_fabric_booking_report_controller')" ); ?> </td>
