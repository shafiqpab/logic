...
$based_on_arr = array(1 => "Booking Date",2 => "Shipment Date ",3 => "Last Approved Date" ,4 => "Last UN Approved Date");
...
<script>
	...
	function openmypage_fabricBooking()
	{
		...
		{ 	
			...
			var page_link = 'requires/date_wise_fabric_booking_report_controller.php?cbo_company_id='+cbo_company_id+'&cbo_fab_booking_type='+cbo_fab_booking_type+'&txt_booking_no='+txt_booking_no+'&txt_booking_no_id='+txt_booking_no_id+'&action=fabricBooking_popup';
	...
	function fn_on_change(type)
	{
		...
		if(type==1)
		{
		load_drop_down( 'requires/date_wise_fabric_booking_report_controller', cbo_company_name, 'load_drop_down_buyer', 'buyer_td' );
		set_multiselect('cbo_buyer_id','0','0','','0','');
		}
		if(type==2)
		{
		load_drop_down( 'requires/date_wise_fabric_booking_report_controller', cbo_pay_mode, 'load_drop_down_suplier', 'sup_td' );
	...
	const fn_generate_report_exel_only = (excl_no) => {
		if(excl_no==1)
		{
			...
			var data="action=report_generate_exel_only"+get_submitted_data_string('cbo_company_id*cbo_buyer_id*cbo_fab_booking_type*cbo_fabric_source*cbo_party_id*cbo_pay_mode*txt_date_from*txt_date_to*txt_booking_no_id*txt_booking_no*cbofabricnature*cbo_based_on',"../../")+'&report_title='+report_title;
			freeze_window(2);
			http.open("POST","requires/date_wise_fabric_booking_report_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
	...
	const generate_report_reponse_exel_only = () => {
		if(http.readyState == 4) 
		{  
			var reponse=trim(http.responseText).split("####");
...
<div style="width:100%; margin:1px auto;" align="left">
            ...
            <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr class="general">
                        ...
                        <td><input type="button" id="show_button" class="formbutton" style="width:60px" value="Show" onClick="fn_report_generated(0);" /><input type="button" id="show_button" class="formbutton" style="width:60px" value="Show 2" onClick="fn_report_generated(4);" /></td>
                    </tr>
                    <tr>
						<a href="" id="aa1"></a>
                        ...
                        <td><input type="button" id="show_button" class="formbutton" style="width:60px" value="Color Wise" onClick="fn_report_generated(3);" /></td>
						<td><input type="button" id="show_button" class="formbutton" style="width:60px" value="Excel Only" onClick="fn_generate_report_exel_only(1)" /></td>
                    </tr>
                </tbody>
            </table>
        	</fieldset>
...
<script type="text/javascript">
	...
	//set_multiselect('cbo_pay_mode','0','0','','0','fn_on_change(2)');
