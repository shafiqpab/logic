...
//echo $user_level;
//echo '<pre>';
//print_r($_SESSION['logic_erp']['data_arr'][469]); die;
//echo count($_SESSION['logic_erp']['data_arr'][469]);
/*if(count($_SESSION['logic_erp']['data_arr'][469])>0)
{
	$field_lavel_access = $_SESSION['logic_erp']['data_arr'][469];
	//echo count($_SESSION['logic_erp']['data_arr'][469]);
...
function print_report_button_setting(report_ids) 
{
    ...
    report_id.forEach(function(items){
        if(items==313){$('#report_btn_1').show();}
        else if(items==323){$('#report_btn_2').show();}
    });
}
...
//Fabric Cost-------------------------------------
	...
	function open_consumption_popup(page_link,title,body_part_id,cbofabricnature_id,txtgsmweight_id,trorder,updateid_fc)
	{
		...
		var csappsupprate=document.getElementById('csappsupprate_'+trorder).value;
...
function fnc_fabric_cost_dtls( operation )
{
	...
	if ( form_validation('txt_sourcing_date','Sourcing Date')==false )
	{
		release_freezing();
		return;
	}
...
function fnc_fabric_syn(i,type)
{
	 
	if(type=='cal_amount')
	{
		var dtls_update_id=document.getElementById('updateid_'+i).value;
	}
	else if(type=='trims_cal_amount')
	{
	var dtls_update_id=document.getElementById('updateidtrim_'+i).value;
	}
	else if(type=='embl_cal_amount')
	{
	var dtls_update_id=document.getElementById('embupdateid_'+i).value;
	}
	else if(type=='wash_cal_amount')
	{
	var dtls_update_id=document.getElementById('embupdateid_'+i).value;
	//alert(dtls_update_id);
	}
	 var dtls_update_data=dtls_update_id+'**'+type;
	var sum_fab_rate=return_global_ajax_value(dtls_update_data, 'sum_synchronize_rate_value', '', 'requires/pre_cost_entry_controller_v2');
		//var amount=number_format_common(sum_fab_yarn_trim, 1, 0, currency);
		var fab_amount_rate=sum_fab_rate.split('**');
		var fab_rate=fab_amount_rate[0];
		var fab_amount=fab_amount_rate[1];
		//var fab_cons=fab_amount_rate[2];
		var cons_break_down=fab_amount_rate[2];
		
	if(type=='cal_amount')
	{
	    document.getElementById('txtrate_'+i).value='';
		document.getElementById('txtamount_'+i).value='';
		document.getElementById('txtrate_'+i).value=fab_rate;
		document.getElementById('txtamount_'+i).value=fab_amount;
		document.getElementById('lastsynchronizeid_'+i).value=1;
		
	}
	else if(type=='trims_cal_amount')
	{
		document.getElementById('txttrimrate_'+i).value='';
		document.getElementById('txttrimamount_'+i).value='';
		document.getElementById('consbreckdown_'+i).value='';
		 
		document.getElementById('txttrimrate_'+i).value=fab_rate;
		document.getElementById('txttrimamount_'+i).value=fab_amount;
		document.getElementById('consbreckdown_'+i).value=cons_break_down;
		document.getElementById('lastsynchronizeid_'+i).value=1;
		calculate_trim_cost(i);
		//var synId=document.getElementById('lastsynchronizeid_'+i).value;
		//alert(synId);
		
	}
	else if(type=='embl_cal_amount')
	{
		document.getElementById('txtembrate_'+i).value='';
		document.getElementById('txtembamount_'+i).value='';
		document.getElementById('consbreckdownemb_'+i).value='';
		// alert(cons_break_down);
		document.getElementById('txtembrate_'+i).value=fab_rate;
		document.getElementById('txtembamount_'+i).value=fab_amount;
		document.getElementById('consbreckdownemb_'+i).value=cons_break_down;
		document.getElementById('lastsynchronizeid_'+i).value=1;
		calculate_emb_cost(i);
	}
	else if(type=='wash_cal_amount')
	{
		document.getElementById('txtembrate_'+i).value='';
		document.getElementById('txtembamount_'+i).value='';
		document.getElementById('consbreckdownwash_'+i).value='';
		// alert(cons_break_down);
		document.getElementById('txtembrate_'+i).value=fab_rate;
		document.getElementById('txtembamount_'+i).value=fab_amount;
		document.getElementById('consbreckdownwash_'+i).value=cons_break_down;
		document.getElementById('lastsynchronizeid_'+i).value=1;
...
function fncopenpopup_trimsupplier(inc)
{
	...
	var page_link="requires/pre_cost_entry_controller_v2.php?trim_rate_variable="+trim(trim_rate_variable)+"&action=openpopup_trimsupplier&cbogroup="+cbogroup+"&buyer="+buyer+"&nominasupplier="+nominasupplier+"&updateidtrim="+updateidtrim;
...
function fncopenpopup_emblsupplier(inc)
{
	...
	if(updateidfabric*1>0){
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, 'Nominated Supplier PopUp', 'width=470px,height=400px,center=1,resize=1,scrolling=0','../');
		emailwindow.onclose=function()
		{
			var suppdata=this.contentDoc.getElementById("suppdata").value;
...
function fncopenpopup_emblsupplier_old(inc)
{
	...
	var page_link="requires/pre_cost_entry_controller_v2.php?action=openpopup_emblsupplier&&buyer="+buyer+"&nominasupplier="+nominasupplier+"&updateidfabric="+updateidfabric;
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, 'Nominated Supplier PopUp', 'width=450px,height=400px,center=1,resize=1,scrolling=0','../');
	emailwindow.onclose=function()
	{
		var suppdata=this.contentDoc.getElementById("suppdata").value;
...
function fncopenpopup_fabricsupplier(inc)
{
	...
	//var page_link="requires/pre_cost_entry_controller_v2.php?action=openpopup_trimsupplier&cbogroup="+cbogroup+"&buyer="+buyer+"&nominasupplier="+nominasupplier;
	var page_link="requires/pre_cost_entry_controller_v2.php?action=openpopup_fabricsupplier&&buyer="+buyer+"&nominasupplier="+nominasupplier+"&updateidfabric="+updateidfabric;
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, 'Nominated Supplier PopUp', 'width=450px,height=400px,center=1,resize=1,scrolling=0','../');
	emailwindow.onclose=function()
	{
		var suppdata=this.contentDoc.getElementById("suppdata").value;
...
//Master form---------------------------------------------------------------------------
	function openmypage(page_link,title)
	{
		...
		emailwindow.onclose=function()
		{
			...
			if (theemail.value!="")
			{
				...
				get_php_form_data( theemail.value, action, "requires/pre_cost_entry_controller_v2" );
				get_php_form_data( document.getElementById("cbo_company_name").value ,'print_button_variable_setting', 'requires/pre_cost_entry_controller_v2');
				...
				var txt_fabric_pre_cost=$("#txt_fabric_pre_cost").val();
				...
				{	
					var today = new Date();
					var date = String(today.getDate()).padStart(2,'0')+'-'+(today.getMonth()+1)+'-'+today.getFullYear();
					$("#txt_sourcing_date").val(date);
					set_button_status(0, permission, 'fnc_precosting_entry',1);
				}
				$('#txt_sourcing_date').removeAttr('disabled','disabled');
				setFieldLevelAccess(document.getElementById("cbo_company_name").value);
				$('#cbo_ready_to_approved').removeAttr('disabled','disabled');
				//	alert(txt_fabric_pre_cost);
	...
	function fnc_precosting_entry( operation )
	{
		...
		//alert('DDD');return;
		if (form_validation('txt_job_no*txt_sourcing_date*cbo_costing_per*cbo_main_fabric_co','Job No*Sourcing Costing Date*Costing Per*Main Fabric CO')==false)
		{
			release_freezing();
			return;
		}
		...
		{
			var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_job_no*hidd_job_id*txt_sourcing_date*cbo_costing_per*txt_remarks*update_id*copy_quatation_id*cbo_approved_status*txt_exchange_rate*txt_sew_smv*cbo_ready_to_approved*cbo_company_name*txt_quotation_id*cbo_main_fabric_co*txt_refusing',"../../");
...
// Dtls Form 1--------------------------------------------------------
	...
	function generate_report(type,mail_data)
	{
		...
		//	if (r==true) zero_val="1"; else zero_val="0";
			...
			var data="action="+type+'&mail_data='+mail_data+"&zero_value="+zero_val+"&rate_amt="+rate_amt+"&"+get_submitted_data_string('txt_job_no*cbo_company_name*cbo_buyer_name*txt_style_ref*txt_sourcing_date*txt_po_breack_down_id*cbo_costing_per*cbo_template_id',"../../");
		//	alert(data);
			http.open("POST","requires/pre_cost_entry_controller_v2.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
	...
	'<html><head><link rel="stylesheet" media = "print" href="css/style_common.css" type="text/css" /><title></title></head><body>'+document.getElementById('data_panel').innerHTML+'</body</html>');
			d.close();
		}
	}
	...
	function openmypage_unapprove_request(entry_form)
	{
		...
		var txt_job_no=document.getElementById('txt_job_no').value;
		if(entry_form == 86){
			var txt_un_appv_request=document.getElementById('txt_higher_auth_un_appv_request').value;
			var title = 'Higher Auth Un Approval Request';
		}
		else{
		...
		var page_link = 'requires/pre_cost_entry_controller_v2.php?data='+data+'&action=unapp_request_popup'+'&entry_form='+entry_form;
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=500px,height=250px,center=1,resize=1,scrolling=0','../../');
		emailwindow.onclose=function()
		{
			var unappv_request=this.contentDoc.getElementById("hidden_appv_cause");
			if(entry_form == 86){
				$('#txt_higher_auth_un_appv_request').val(unappv_request.value);
			}
			else{
	...
	function call_print_button_for_mail(mail){
		generate_report('mkt_source_cost',mail+'**1');
		//generate_report('app_final_cost',mail+'**1');
	}
	function check_synchronize_cost(type)
	{
		...
		if(job_id!="")
		{
			if(type==1)
			{ 
			action='check_data_synchronize_action';
			get_php_form_data( job_id, action, "requires/pre_cost_entry_controller_v2" );
			//$("#cbo_ready_to_approved").val();//
			}
		}
    ...
    <fieldset style="width:1070px;">
        ...
        <form name="precosting_1" id="precosting_1" autocomplete="off">
            ...
            <table  width="1070" cellspacing="2" cellpadding=""  border="0">
                ...
                <tr>
                    <td class="must_entry_caption">Main Fabric CO.</td>
                    <td><? 
					
					echo create_drop_down( "cbo_main_fabric_co", 120, $main_fabric_co_arr,"", 1, "-- Select--", 0, "",0,"" ); ?></td>
                    
                    <td>Ready To Approved</td>
                    <td><? echo create_drop_down( "cbo_ready_to_approved", 120, $yes_no,"", 1, "-- Select--", 2, "check_synchronize_cost(this.value)",0,"" ); ?></td>
                     <td>Remarks</td>
                    <td colspan="3">
                    <input class="text_boxes" type="text" style="width:415px;" name="txt_remarks" id="txt_remarks" maxlength="500" title="Maximum 500 Character" placeholder="Remarks" /></td>
                   
                </tr>
                <tr>
                	<td>Un-approve request</td>
                    <td>
                        <Input name="txt_un_appv_request" class="text_boxes" readonly placeholder="Double Click" ID="txt_un_appv_request" style="width:110px;" onClick="openmypage_unapprove_request(47);">
                    </td>

					<td>Higher Auth Un-approve Req.</td>
                    <td>
                        <Input name="txt_higher_auth_un_appv_request" class="text_boxes" readonly placeholder="Double Click" ID="txt_higher_auth_un_appv_request" style="width:110px;" onClick="openmypage_unapprove_request(86);">
                    </td>
                    
                     <td>Refusing Cause</td>
                      <td colspan="3"><input class="text_boxes" type="text" style="width:415px;" name="txt_refusing" id="txt_refusing" maxlength="500" title="Maximum 500 Character" placeholder="Refusing Cause"/></td>
                </tr>
                ...
                <tr>
                    <td align="center" valign="middle" class="button_container" colspan="8">
                        ...
                        <? echo load_submit_buttons( $permission, "fnc_precosting_entry", 0,0 ,"ResetForm()",1,0) ; ?>
                    ...
                    <table width="100%" cellspacing="3" cellpadding="0" class="rpt_table" rules="all">
                        ...
                        <tr>
                            <td colspan="2" align="center">
                                ...
                                <input type="button" id="report_btn_1" class="formbutton" value="MKT Vs Source" onClick="generate_report('mkt_source_cost');"/> &nbsp;
                                 <input type="button" id="report_btn_2" class="formbutton" value="Final App" onClick="generate_report('app_final_cost');"/>                                
                            </td>
                        </tr>
                    </table>
                    </form>
