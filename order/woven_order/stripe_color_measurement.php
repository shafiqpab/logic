...
function openmypage(page_link,title)
{
	...
	emailwindow.onclose=function()
	{
		...
		if (theemail.value!="")
		{
			...
			get_php_form_data( theemail.value, action, "requires/stripe_color_measurement_controller" );
			show_list_view(theemail.value,'show_fabric_cost_listview','cost_container','requires/stripe_color_measurement_controller','');
			show_list_view(theemail.value+'_'+fabric_cost_id+'_'+cbo_color_name,'stripe_color_list_view','stripe_color_list_view_container','requires/stripe_color_measurement_controller','');
...
{
	get_php_form_data( fabric_cost_id, 'set_data', "requires/stripe_color_measurement_controller" );
	var txt_job_no=document.getElementById('txt_job_no').value;
	var cbogmtsitem=document.getElementById('cbogmtsitem').value;
    load_drop_down( 'requires/stripe_color_measurement_controller', txt_job_no+'_'+cbogmtsitem+'_'+fabric_cost_id, 'load_drop_down_color', 'color_td' )
...
function open_color_popup()
{
		...
		var page_link="requires/stripe_color_measurement_controller.php?action=open_color_list_view&txt_job_no="+trim(txt_job_no)+"&cbogmtsitem="+cbogmtsitem+"&fabric_cost_id="+fabric_cost_id+"&cbo_color_name="+cbo_color_name+'&cbo_company_name='+cbo_company_name+'&cbo_buyer_name='+cbo_buyer_name;
...
function fn_deletebreak_down_tr(fabric_cost_id,cbo_color_name){
	...
	{
		var response=return_global_ajax_value(fabric_cost_id+"_"+cbo_color_name+"_"+txt_job_no, 'delete_row', '', 'requires/stripe_color_measurement_controller');
		if(response*1==11){
                            ...
                            <div style="width:1070px;">  
                                <table  width="100%" cellspacing="2" cellpadding=""  border="0">
                                    <tr>
                                        ...
                                        <td  width="150">
                                        <input  style="width:150px;" type="text" title="Double Click to Search" onDblClick="openmypage('requires/stripe_color_measurement_controller.php?action=order_popup','Job/Order Selection Form')" class="text_boxes" placeholder="New Job No" name="txt_job_no" id="txt_job_no" readonly /> <!--<input type="hidden" id="hdd_jod_id" value="" /> -->                 
                                        </td>
                                        <td  align="right"  width="150">Company</td>
                                        <td  width="150">
                                        <?
                                        echo create_drop_down("cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/stripe_color_measurement_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );");
                                        ?>
                                        </td>                                        
