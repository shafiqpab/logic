...
<script>
	...
	{
		freeze_window(5);
		get_php_form_data( po_id, "populate_data_from_search_popup", "requires/bh_size_color_breakdown_controller" );
		show_list_view(po_id,'populate_size_color_breakdown','size_color_breakdown','../woven_order/requires/bh_size_color_breakdown_controller','');
		load_drop_down( 'requires/bh_size_color_breakdown_controller',document.getElementById('item_id').value, 'load_drop_down_item', 'item_td' )
	...
	function openmypage(page_link,title)
	{
		...
		emailwindow.onclose=function()
		{
			...
			{
				freeze_window(5);
				var job_no = return_global_ajax_value( theemail.value, 'get_jobno_by_poid', '', 'requires/bh_size_color_breakdown_controller');
				...
				show_list_view(job_no+'*'+theemail.value,'order_listview','job_po_list_view','requires/bh_size_color_breakdown_controller','setFilterGrid(\'tbl_po_list\',-1)');
				load_drop_down( 'requires/bh_size_color_breakdown_controller',document.getElementById('item_id').value, 'load_drop_down_item', 'item_td' )
	...
	function fnc_size_color_breakdown( operation )
	{
		...
		if(operation==2)
		{
			var cutting_qty=return_global_ajax_value(order_id+"_"+cbo_po_country, 'get_cutting_qty_country', '', 'requires/bh_size_color_breakdown_controller');
			if(cutting_qty>0){
		...
		if(operation==0)
		{
			var inserted_po_qnty=return_global_ajax_value(order_id+'_'+cbo_po_country, 'inserted_po_qnty', '', 'requires/bh_size_color_breakdown_controller');
		...
		if(operation==1)
		{
			var inserted_po_qnty=return_global_ajax_value(order_id+'_'+hid_old_country, 'inserted_po_qnty', '', 'requires/bh_size_color_breakdown_controller');
		    var inserted_po_qnty_arr=inserted_po_qnty.split("_");
		...
		http.open("POST","requires/bh_size_color_breakdown_controller.php",true);
	...
	function fnc_on_submit_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			show_list_view(document.getElementById('order_id').value,'populate_size_color_breakdown','size_color_breakdown','requires/bh_size_color_breakdown_controller','');
		    show_list_view(document.getElementById('order_id').value+"_"+document.getElementById('cbo_po_country').value,'populate_size_color_breakdown_with_data','data_form','requires/bh_size_color_breakdown_controller','');
	...
	{   
		if(table_id=='size_color_break_down_list')
		{
			...
			if(rowNo!=1)
			{
				...
				{
				var booking=return_global_ajax_value(updateid, 'delete_row_color_size', '', 'requires/bh_size_color_breakdown_controller');
				}
				var index=rowNo-1
	...
	function populate_size_color_breakdown_with_data(data)
	{
		...
		document.getElementById('txt_click_range').value="";
		document.getElementById('txt_copy_color').value="";
		show_list_view(data[0]+"_"+data[1],'populate_size_color_breakdown_with_data','data_form','requires/bh_size_color_breakdown_controller','');
	...
	function check_country(country_id)
	{
		...
		var country=return_global_ajax_value(po_id+"_"+country_id, 'check_country', '', 'requires/bh_size_color_breakdown_controller');
		if(country>0)
		{
	...
	function color_select_popup(buyer_name,texbox_id)
	{
		...
		//alert(texbox_id)
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/bh_size_color_breakdown_controller.php?action=color_popup&buyer_name='+buyer_name, 'Color Select Pop Up', 'width=250px,height=450px,center=1,resize=1,scrolling=0','../../')
		emailwindow.onclose=function()
		{
	...
	function copy_avg_price()
	{
		...
		var tot_row=return_global_ajax_value(order_id+'_'+avg_price+'_'+txt_job_no, 'update_avg_rate', '', 'requires/bh_size_color_breakdown_controller');
		show_list_view(document.getElementById('order_id').value,'populate_size_color_breakdown','size_color_breakdown','requires/bh_size_color_breakdown_controller','');
	    show_list_view(document.getElementById('order_id').value+"_"+document.getElementById('cbo_po_country').value,'populate_size_color_breakdown_with_data','data_form','requires/bh_size_color_breakdown_controller','');
		document.getElementById('cbo_po_country').value=0;
	...
	function copy_excess_cut()
	{
		...
		var tot_row=return_global_ajax_value(order_id+'_'+txt_excess_cut+'_'+txt_job_no, 'update_txt_excess_cut', '', 'requires/bh_size_color_breakdown_controller');
		show_list_view(document.getElementById('order_id').value,'populate_size_color_breakdown','size_color_breakdown','requires/bh_size_color_breakdown_controller','');
	    show_list_view(document.getElementById('order_id').value+"_"+document.getElementById('cbo_po_country').value,'populate_size_color_breakdown_with_data','data_form','requires/bh_size_color_breakdown_controller','');
		document.getElementById('cbo_po_country').value=0;
		document.getElementById('hid_old_country').value=0;
	...
	function set_ship_date() 
	{
		...
		var set_ship_date=return_global_ajax_value(txt_cutup_date+'_'+cbo_cut_up, 'set_ship_date', '', 'requires/bh_size_color_breakdown_controller');
	...
	{
		var txt_job_no=document.getElementById('txt_job_no').value;
	  emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/bh_size_color_breakdown_controller.php?action=reorder_size_color&txt_job_no='+txt_job_no, 'Color Size Ordering', 'width=600px,height=400px,center=1,resize=1,scrolling=0','../../')	
	}
	function validate_po_qty_co_si(i)
	{
		...
		var cutting_qty=return_global_ajax_value(hiddenid, 'get_cutting_qty', '', 'requires/bh_size_color_breakdown_controller');
		}
		//alert(cutting_qty)
