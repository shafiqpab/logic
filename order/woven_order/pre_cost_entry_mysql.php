...
{   
	...
	if(table_id=='tbl_fabric_cost')
	{
		...
		if(numRow==rowNo && rowNo!=1)
		{
			...
			//var booking=return_global_ajax_value(updateid, 'check_booking', '', 'requires/pre_cost_entry_controller');
			//if(trim(booking) !="")
			//{
			//alert("You have fabric booking no"+trim(booking))
			//return;
			//}
			//else
			//{
	...
	if(table_id=='tbl_yarn_cost')
	{
		...
		if(numRow==rowNo && rowNo!=1)
		{
			$('#tbl_yarn_cost tbody tr:last').remove();
		}*/
		if(rowNo!=1)
		{
			var permission_array=permission.split("_");
...
function show_sub_form(update_id, action, extra_str)
{
	...
	else
	{
		...
		{
			show_list_view(update_id+'*'+extra_str+'*'+document.getElementById('txt_quotation_id').value+'*'+document.getElementById('copy_quatation_id').value,action,'cost_container','../woven_order/requires/pre_cost_entry_controller','');
			set_sum_value( 'txtconsdzntrim_sum', 'txtconsdzngmts_', 'tbl_trim_cost' );
	        set_sum_value( 'txtratetrim_sum', 'txttrimrate_', 'tbl_trim_cost' );
	        set_sum_value( 'txttrimamount_sum', 'txttrimamount_', 'tbl_trim_cost' );
		}
...
function open_consumption_popup(page_link,title,body_part_id,cbofabricnature_id,txtgsmweight_id,trorder,updateid_fc)
{
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			document.getElementById('isclickedconsinput_'+trorder).value=0;
...
function open_stripe_color_popup()
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, "Set Details", 'width=1200px,height=400px,center=1,resize=1,scrolling=0','../')
	emailwindow.onclose=function()
	{
	}
}
...
function calculate_yarn_consumption_ratio(i,type)
{
	...
	var rowCount = $('#tbl_yarn_cost tr').length-1;
		...
		for (var k=i; k<=rowCount; k++)
		{
			...
			if(cbocount==cbocountk && cbocompone==cbocomponek && percentone==percentonek && cbocomptwo==cbocomptwok && percenttwo==percenttwok && cbotype==cbotypek)
			{
...
function fnc_fabric_yarn_cost_dtls( operation )
{
		...
		for (var i=1; i<=row_num; i++)
		{
			...
			data_all=data_all+get_submitted_data_string('cbo_company_name*update_id*cbocount_'+i+'*cbocompone_'+i+'*percentone_'+i+'*cbocomptwo_'+i+'*percenttwo_'+i+'*cbotype_'+i+'*consqnty_'+i+'*avgconsqnty_'+i+'*txtrateyarn_'+i+'*txtamountyarn_'+i+'*supplier_'+i+'*cbostatusyarn_'+i+'*updateidyarncost_'+i+'*txtconsumptionyarn_sum*txtamountyarn_sum',"../../");
		}
...
function fnc_trim_cost_dtls( operation )
{
		...
		for (var i=1; i<=row_num; i++)
		{
			...
			eval(get_submitted_variables('cbo_company_name*update_id*cbogroup_'+i+'*txtdescription_'+i+'*txtsupref_'+i+'*cboconsuom_'+i+'*txtconsdzngmts_'+i+'*txttrimrate_'+i+'*txttrimamount_'+i+'*cboapbrequired_'+i+'*cbonominasupplier_'+i+'*cbotrimstatus_'+i+'*updateidtrim_'+i+'*consbreckdown_'+i+'*txtremark_'+i+'*txtconsdzntrim_sum*txtratetrim_sum*txttrimamount_sum'));
			data_all=data_all+get_submitted_data_string('cbo_company_name*update_id*cbogroup_'+i+'*txtdescription_'+i+'*txtsupref_'+i+'*cboconsuom_'+i+'*txtconsdzngmts_'+i+'*txttrimrate_'+i+'*txttrimamount_'+i+'*cboapbrequired_'+i+'*cbonominasupplier_'+i+'*cbotrimstatus_'+i+'*updateidtrim_'+i+'*consbreckdown_'+i+'*txtremark_'+i+'*txtconsdzntrim_sum*txtratetrim_sum*txttrimamount_sum',"../../");
		}
...
function openmypage(page_link,title)
{
	...
	emailwindow.onclose=function()
	{
		...
		{
			freeze_window(5);
			reset_form('precosting_1','cost_container','','cbo_currercy,2*txt_exchange_rate,80*cbo_costing_per,1*cbo_approved_status,2*is_click_cons_box,1','')
...
function fnc_precosting_entry( operation )
{
	...
	if (form_validation('txt_job_no*txt_costing_date*cbo_inco_term*txt_incoterm_place*txt_machine_line*txt_prod_line_hr*cbo_costing_per','Job No*Costing Date*Incoterm*Incoterm Palce*Machime/Line*Prod/Line/Hr*Costing Per')==false)
	{
		return;
	}
	else
	{
		...
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('garments_nature*txt_job_no*txt_costing_date*cbo_inco_term*txt_incoterm_place*txt_machine_line*txt_prod_line_hr*cbo_costing_per*txt_remarks*update_id*update_id*copy_quatation_id*cbo_approved_status*cm_cost_predefined_method_id*txt_exchange_rate*txt_sew_smv*txt_cut_smv*txt_sew_efficiency_per*txt_cut_efficiency_per*txt_efficiency_wastage*cbo_ready_to_approved',"../../");
...
//3. CM Cost = {(MCE/26)/NFM)*MPL)}/[{(PHL)*WH}]*Costing Per/Exchange Rate
	...
	var cpm=return_global_ajax_value(cbo_company_name+'_'+txt_costing_date, 'cost_per_minute', '', 'requires/pre_cost_entry_controller');
...
function  change_caption_cost_dtls( value, type )
{
	...
	if(type=="change_caption_dzn")
	{
		...
		if(value!=trim(is_used_costing_per) && trim(is_used_costing_per)!="")
		{
				alert("Changing Costing Per Value, Below Costing Info. and CM Would Be Mismatched.\n Be Sure About Change!");
		}
		if(value==1)
		{
	...
	if(type=="change_caption_pcs")
	{
		...
		if(value==58)
		{
			document.getElementById('price_pcs_td').innerHTML="Margin/Set ";
...
function generate_report(type)
{
	...
	else
	{
		...
		var data="action="+type+"&zero_value="+zero_val+"&"+get_submitted_data_string('txt_job_no*cbo_company_name*cbo_buyer_name*txt_style_ref*txt_costing_date*txt_po_breack_down_id',"../../");
		http.open("POST","requires/pre_cost_entry_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
                            ...
                            <div style="width:1070px;">  
                                <table  width="100%" cellspacing="2" cellpadding=""  border="0">
                                    <tr>
                                        ...
                                        <td  width="150">
                                        <?
                                        echo create_drop_down("cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/pre_cost_entry_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );  load_drop_down( 'requires/pre_cost_entry_controller', this.value, 'load_drop_down_agent', 'agent_td' );");
                                        ?>
                                        </td>                                        
                                    ...
                                    <tr>
                                        ...
                                        <td><input class="datepicker" type="text" style="width:150px;" name="txt_costing_date" id="txt_costing_date" onChange="calculate_confirm_price_dzn();check_exchange_rate();" value="<? //echo date('d-m-Y'); ?>"/></td>
                                       ...
                                       <td><input class="text_boxes" type="text" style="width:150px;" name="txt_incoterm_place" id="txt_incoterm_place" maxlength="100" title="Maximum 100 Character"/>
                                       </td>
                                       <td align="right" class="must_entry_caption">Machine/Line</td>
                                    ...
                                    <tr>
                                        ...
                                        <td align="right" class="must_entry_caption">Prod/Line/Hr</td>
                                        ...
                                        <td>
                                        <? 
                                        echo create_drop_down( "cbo_costing_per", 160, $costing_per, "",0, "0", 1, "change_caption_cost_dtls( this.value, 'change_caption_dzn' )","","" );
                                        ?>
                                        </td>
                                   ...
                                   <td align="right">Images</td>
                                    ...
                                    <tr> 
										...
										echo load_submit_buttons( $permission, "fnc_precosting_entry", 0,0 ,"reset_form('precosting_1*quotationdtls_2','cost_container','','cbo_currercy,2*cbo_costing_per,1*cbo_approved_status,2*is_click_cons_box,1',$dd)",1,1) ;
										?>  
                                        </td>
                                    </tr>
                                </table>
                                    ...
                                    <form id="quotationdtls_2" autocomplete="off">
                                        <table width="100%" cellspacing="2" cellpadding="0" class="rpt_table" rules="all">
                                            ...
                                            <tr>
                                                ...
                                                <? echo load_submit_buttons( $permission, "fnc_quotation_entry_dtls", 0,1 ,"reset_form('quotationdtls_2','','')",2) ; ?>  
                                                 </td>                                                
                                            </tr>
                                            <tr>
                                            ...
                                            <tr>
                                            	<td colspan="6" align="center">
                                                	<input type="button" id="report_btn" class="formbutton" value="Pre Cost Rpt" onClick="generate_report('preCostRpt')" />
                                                    <input type="button" id="report_btn" class="formbutton" value="Pre Cost Rpt2" onClick="generate_report('preCostRpt2')" />

                                                    <input type="button" id="report_btn" class="formbutton" value="BOM Rpt" onClick="generate_report('bomRpt')" />
                                                </td>
                                            </tr>
                                        </table>
                                    </form>
