...
function openmypage(page_link,title)
{
	...
	emailwindow.onclose=function()
	{
		...
		if (theemail.value!="")
		{
			...
			get_php_form_data( theemail.value, action, "requires/stripe_color_measurement_controller_urmi" );
			show_list_view(theemail.value,'show_fabric_cost_listview','cost_container','requires/stripe_color_measurement_controller_urmi','');
			show_list_view(theemail.value+'_'+fabric_cost_id+'_'+cbo_color_name,'stripe_color_list_view','stripe_color_list_view_container','requires/stripe_color_measurement_controller_urmi','');
...
{
	get_php_form_data( fabric_cost_id, 'set_data', "requires/stripe_color_measurement_controller_urmi" );
	var txt_job_no=document.getElementById('txt_job_no').value;
	var cbogmtsitem=document.getElementById('cbogmtsitem').value;
    load_drop_down( 'requires/stripe_color_measurement_controller_urmi', txt_job_no+'_'+cbogmtsitem+'_'+fabric_cost_id, 'load_drop_down_color', 'color_td' )
}
...
function open_color_popup()
{
	...
	var page_link="requires/stripe_color_measurement_controller_urmi.php?action=open_color_list_view&txt_job_no="+trim(txt_job_no)+"&hidd_job_id="+hidd_job_id+"&cbogmtsitem="+cbogmtsitem+"&fabric_cost_id="+fabric_cost_id+"&cbo_color_name="+cbo_color_name+'&cbo_company_name='+cbo_company_name+'&cbo_buyer_name='+cbo_buyer_name;
                ...
                <div style="width:1050px;">  
                    <table width="100%" cellspacing="2" cellpadding="" border="0">
                        <tr>
                            ...
                            <td width="150">
                                <input  style="width:150px;" type="text" title="Double Click to Search" onDblClick="openmypage('requires/stripe_color_measurement_controller_urmi.php?action=order_popup','Job/Order Selection Form')" class="text_boxes" placeholder="New Job No" name="txt_job_no" id="txt_job_no" readonly />  
                                <input type="hidden" name="hidd_job_id" id="hidd_job_id" style="width:30px;" class="text_boxes" />                   
                            </td>
                            <td align="right" width="150">Company</td>
                            <td width="150"><? echo create_drop_down("cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/stripe_color_measurement_controller_urmi',this.value, 'load_drop_down_buyer', 'buyer_td' );"); ?></td>                                        
...
if($txt_job_no !="")
{
	...
	get_php_form_data(txt_job_no, 'populate_data_from_job_table', "requires/stripe_color_measurement_controller_urmi" );
	show_list_view(txt_job_no,'show_fabric_cost_listview','cost_container','requires/stripe_color_measurement_controller_urmi','');
	show_list_view(txt_job_no+'_'+fabric_cost_id+'_'+cbo_color_name,'stripe_color_list_view','stripe_color_list_view_container','requires/stripe_color_measurement_controller_urmi','');
	<?
}
?>
