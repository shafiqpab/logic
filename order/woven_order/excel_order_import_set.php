...
{	
	...
	if (move_uploaded_file($source, $targetzip)) 
	{
		...
		$card_colum=0; $m=1; $styleItemData_arr=array(); $po_data_array=array(); $country_data_array=array(); $style_all_data_arr=array();
		for ($i = 1; $i <= $excel->sheets[0]['numRows']; $i++) 
		{
			...
			{ 
				...
				$countryCode=str_replace("(","[",$excel->sheets[0]['cells'][$i][2]);
				$code=str_replace("(","[",$excel->sheets[0]['cells'][$i][5]);
				...
				$setItem=str_replace($str_rep,' ',$excel->sheets[0]['cells'][$i][24]);
				...
				if($code==0 || $code=="") $code=$countryCode;
				...
				{
					$styleItemData_arr[$style_ref][$setItem]+=$country_po_qty;
					$style_all_data_arr[$style_ref][$po_number][$setItem][$color_name][$size_name][$all_data]+=$country_po_qty;
				}
			}
		}
		...
		if(count($styleItemData_arr)>0)
		{
			foreach($styleItemData_arr as $styleName=>$itemData)
			{
				...
				{
					if($style_itemQty=="") $style_itemQty=$excelitem.'__'.$poQty; else $style_itemQty.=','.$excelitem.'__'.$poQty;
				}
				$styleWiseRatio_arr[$styleName]=$style_itemQty;
				$style_itemQty="";
			}
		}
		
		foreach($styleWiseRatio_arr as $style_name=>$styleData)
		{
        ...
        <script>
			...
			function fnc_excel_import(operation)
			{
				...
				http.open("POST","requires/excel_order_import_set_controller.php",true);
        ...
        <!-- Important Field outside Form -->  
            ...
            <fieldset style="width:1400px;">
              ...
              <tbody>
              	<tr>
                	<td><? echo create_drop_down( "cbo_company_id", 130, "select id, company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-Select Company-", $selected, "load_drop_down( 'requires/excel_order_import_set_controller', this.value, 'load_drop_down_location', 'location_td'); load_drop_down( 'requires/excel_order_import_set_controller', this.value, 'load_drop_down_buyer', 'buyer_td'); fnc_style_owner(this.value);"); ?></td>
                    <td id="location_td"><? echo create_drop_down( "cbo_location_id", 130, $blank_array,"", 1, "-Location-", $selected, ""); ?></td>
                    <td id="buyer_td"><? echo create_drop_down( "cbo_buyer_id", 130, $blank_array,"", 1, "-Buyer-", $selected, ""); ?></td>
                    <td id="owner_td"><? echo create_drop_down( "cbo_style_owner_id", 130, "select id, company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-Style Owner-", $selected, ""); ?></td>
                    <td><? echo create_drop_down( "cbo_product_department", 100, $product_dept, "", 1, "-Select-", $selected, "", "", "" ); ?></td>
                    ...
                    <td><? echo create_drop_down( "cbo_prod_catgory", 80, $product_category,"", 1, "-Category-", 1, "","","" ); ?></td>
                    <td><? echo create_drop_down( "cbo_team_leader", 100, "select id, team_leader_name from lib_marketing_team where status_active =1 and is_deleted=0 order by team_leader_name","id,team_leader_name", 1, "-Select Team-", $selected, "load_drop_down( 'requires/excel_order_import_set_controller', this.value, 'load_drop_down_dealing_merchant', 'div_marchant'); load_drop_down( 'requires/excel_order_import_set_controller', this.value, 'load_drop_down_factory_merchant', 'div_marchant_factory')" ); ?></td>
                    <td id="div_marchant"><? echo create_drop_down( "cbo_dealing_merchant", 100, $blank_array,"", 1, "-Team Member-", $selected, "" ); ?></td>
                    <td id="div_marchant_factory"><? echo create_drop_down( "cbo_factory_merchant", 100, $blank_array,"", 1, "-Fac Merchent-", $selected, "" ); ?></td>
                    <td><? echo create_drop_down( "cbo_order_uom",60, $unit_of_measurement, "",0, "", 1, "","","58" ); ?></td>
            ...
            <table width="600" cellspacing="0" class="rpt_table" border="0" id="tbl_set_details" rules="all">
					...
					foreach($styleItemData_arr as $styleName=>$itemData)
					{
						foreach($itemData as $excelitem=>$poQty)
						{
							...
							<tr id="settr_<?=$k; ?>" align="center">
								...
								<td><?  echo create_drop_down( "cboitem_".$k, 150, $garments_item, "",1,"--Select--", 0, "check_duplicate($k,this.id);",'','' ); ?></td>
						...
						if($nullItem==0)
						{
                            ...
                            <tr id="settr_1" align="center">
                                ...
                                <td><?  echo create_drop_down( "cboitem_".$k, 150, $garments_item, "",1,"--Select--", 0, "check_duplicate($k,this.id ); check_smv_set($k); check_smv_set_popup($k);",'','' ); ?></td>
                                <td><input type="text" id="txtsetitemratio_<?=$k; ?>" name="txtsetitemratio_<?=$k; ?>" style="width:30px" class="text_boxes_numeric" onChange="calculate_set_smv(<?=$k; ?>);" value="" /></td>
                                <td><input type="text" id="smv_<?=$k; ?>" name="smv_<?=$k; ?>" style="width:30px" class="text_boxes_numeric" onChange="calculate_set_smv(<?=$k; ?>);" value="0" />
                                    <input type="hidden" id="smvset_<?=$k; ?>" name="smvset_<?=$k; ?>" style="width:30px" class="text_boxes_numeric" value="0"  />
                                </td>
				...
				else
				{
                    ...
                    <tr id="settr_1" align="center">
                        ...
                        <td><?  echo create_drop_down( "cboitem_1", 150, $garments_item, "",1,"--Select--", 0, "check_duplicate(1,this.id ); check_smv_set(1); check_smv_set_popup(1);",'','' ); ?></td>
		...
		<div style="width:1390px; max-height:320px; overflow-y:scroll" id="scroll_body" > 
		<table width="1372" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all" id="scanning_tbl"><!--table_body-->
        	<?php
					...
					foreach($itemcolor_data as $setItem=>$color_size_data)
					{
						foreach($color_size_data as $color_val=>$size_data)
						{
							...
							foreach($size_data as $size_val=>$extra_data)
							{
								...
								$style_des=$ex_data[0]; $po_receive_date=$ex_data[1]; $po_shiment_date=$ex_data[2]; $po_avg_rate=number_format($ex_data[3],4); $po_remark=$ex_data[4]; $country_qty=$sizeqty; $country_amt=number_format($sizeqty*$po_avg_rate,6,'.','');
								...
								<tr bgcolor="<?php echo $bgcolor; ?>" onClick="change_color('tr_1nd<?php echo $i; ?>','<?php echo $bgcolor; ?>')" id="tr_1nd<?php echo $i; ?>">
                                    ...
                                    <td width="80" id="setItem_<?php echo $ciid; ?>"><?php echo trim($setItem); ?></td>
		...
		<div>
            <table>
            	<tr style="border:none">
                    <td align="center" colspan="16" class="button_container">
    ...
    <script> //if ($('#txt_job_no').val()!="") get_php_form_data( $('#txt_job_no').val(), 'populate_job_data_form','requires/excel_order_import_set_controller');</script>
