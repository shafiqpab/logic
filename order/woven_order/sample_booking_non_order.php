...
<?
	...
	// }
	// else if($_SESSION['logic_erp']['mandatory_field'][90][43]!="")
	// {
	// 	echo "var mst_mandatory_field = '". ($_SESSION['logic_erp']['mandatory_field'][90][43]) . "';\n";
	// 	echo "var mst_mandatory_message = '". ($_SESSION['logic_erp']['mandatory_field'][90][43]) . "';\n";
	// }
...
if(count($_SESSION['logic_erp']['mandatory_field'][90])>0)
{
	foreach ($_SESSION['logic_erp']['mandatory_field'][90] as $key => $field) {
		if($key==1 || $key==2 || $key==3 || $key==4 || $key==5 || $key==6  || $key==7 || $key==8  || $key==9  || $key==10  || $key==11 || $key==12 || $key==13 || $key==14 || $key==15 || $key==16 || $key==43)
		{			
	...
	foreach ($_SESSION['logic_erp']['mandatory_message'][90] as $key => $field) {
		if($key==1 || $key==2 || $key==3 || $key==4 || $key==5 || $key==6  || $key==7 || $key==8  || $key==9  || $key==10  || $key==11 || $key==12 || $key==13 || $key==14 || $key==15 || $key==16 || $key==43 )
		{			
...
foreach($_SESSION['logic_erp']['mandatory_field'][90] as $key=>$value){
	...
	if($key==19 || $key==20 || $key==29 || $key==33 || $key==39 || $key==41 || $key==42 || $key==23 || $key==18){
		if($i==0){
				$field.=$value;
				$message.=$value;
...
function fnc_fabric_booking( operation )
{
	...
	{
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_brand_name*txt_booking_no*cbo_fabric_natu*cbo_fabric_source*cbo_currency*txt_exchange_rate*cbo_pay_mode*txt_booking_date*cbo_supplier_name*txt_attention*txt_delivery_date*cbo_source*cbo_ready_to_approved*cbo_team_leader*cbo_dealing_merchant*cbouom_id*update_id',"../../");
...
function open_sample_popup()
{
	...
	//mst_mandatory_field
		...
		var data="action=save_update_delete_dtls&operation="+operation+"&row_num="+row_num+get_submitted_data_string('txt_booking_no*update_id*cbo_company_name*cbo_buyer_name*txt_delivery_date*cbo_team_leader*cbo_fabric_natu*cbo_fabric_source*cbo_body_part*cbo_color_type*txt_style*txt_style_des*cbo_sample_type*libyarncountdeterminationid*oldlibyarncountdeterminationid*construction*composition*yarnbreackdown*txt_fabricdescription*txt_gsm*txt_gmt_color*txt_color*txt_gmts_size*txt_size*txt_dia_width*txt_finish_qnty*txt_process_loss*txt_grey_qnty*txt_rate*txt_amount*update_id_details*process_loss_method*txt_article_no*txt_yarn_details*txt_remarks*cbo_body_type*txt_item_qty*txt_knitting_charge*txt_bh_qty*txt_rf_qty*cbo_fabric_source_dtls*txt_delivery_dates*txt_style_dtls_id*cbo_pay_mode*cbouom*cbo_dia_width_type*hidden_collarCuff_data*hidden_collar_cuff_ids',"../../")+data_all;
		//alert(data);
		//release_freezing();
		//return;
	...
	function generate_fabric_report(type)
	{
		...
		else
		{
			...
			{
				var data="action=show_fabric_booking_report8"+get_submitted_data_string('txt_booking_no*cbo_company_name*id_approved_id*cbo_fabric_natu',"../../")+'&report_title='+$report_title;
			}
			//freeze_window(5);
			http.open("POST","requires/sample_booking_non_order_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
...
/*	function validate_suplier(){
	...
	function print_report_button_setting(report_ids)
	{
		...
		for (var k=0; k<report_id.length; k++)
		{
			...
			if(report_id[k]==174)
			{
				$('#button_panel').append( '<input type="button" value="Print 8" onClick="generate_fabric_report(8)"  style="width:100px" name="print8" id="print8" class="formbutton" />&nbsp;&nbsp;&nbsp;' );
			}
		}
	}
...
<div style="width:100%;" align="center">
        ...
        <legend>Sample Fab.Booking [Without Order]</legend>
            <table  width="960" cellspacing="2" cellpadding="0" border="0">
                <tr>
                    ...
                    <td width="150"><input class="text_boxes" type="text" style="width:120px" onDblClick="openmypage_booking('requires/sample_booking_non_order_controller.php?action=fabric_booking_popup','fabric Booking Search')" readonly placeholder="Double Click for Booking" name="txt_booking_no" id="txt_booking_no"/>
                    </td>
                    <td width="100" class="must_entry_caption">Company Name</td>
                    <td width="150"><?=create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected, "load_drop_down( 'requires/sample_booking_non_order_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );color_from_library( this.value );check_kniting_charge();print_button_setting();check_exchange_rate();","","" ); // new condtion
                    ?>
                    ...
                    <td width="150" id="buyer_td"><? echo create_drop_down( "cbo_buyer_name", 130, $blank_array,"", 1, "-- Select Buyer --", $selected, "","","" ); ?></td>
					<td width="100" class="">Brand Name</td>
                    <td width="150" id="brand_td"><? echo create_drop_down( "cbo_brand_name", 130, $blank_array,"", 1, "-- Select Brand --", $selected, "","","" ); ?></td>
                </tr>
                <tr>
					<td width="100" class="must_entry_caption">Booking Date</td>
                    <td><input class="datepicker" type="text" style="width:120px" name="txt_booking_date" id="txt_booking_date" onChange="check_exchange_rate();" value="<? echo date("d-m-Y")?>" disabled />
                    </td>
                    <td class="must_entry_caption">Fabric Nature</td>
                    <td><? echo create_drop_down( "cbo_fabric_natu", 130, $item_category,"", 1, "-- Select --", 1,$onchange_func, $is_disabled, "2,3");	?></td>
                    <td>Fabric Source</td>
					<td><? echo create_drop_down( "cbo_fabric_source", 130, $fabric_source,"", 1, "-- Select --", $selected,"", "", "2,3");	?></td>
                    <td>Currency</td>
                ...
                <tr>
                    ...
                    <td><?=create_drop_down( "cbo_pay_mode", 130, $pay_mode,"", 1, "-- Select Pay Mode --", 3, "load_drop_down( 'requires/sample_booking_non_order_controller', this.value, 'load_drop_down_suplier', 'sup_td' )","","1,2,3,5" ); ?></td>
                </tr>
                <tr>
					<td class="must_entry_caption"> Supplier Name </td>              <!-- 11-00030  -->
                    ...
                    <td>Attention</td>
                    <td colspan="3"><input class="text_boxes" type="text" style="width:370px;"  name="txt_attention" id="txt_attention"/>
                    	<input type="hidden" class="image_uploader" style="width:62px" value="Lab DIP No" onClick="openmypage('requires/sample_booking_non_order_controller.php?action=lapdip_no_popup','Lapdip No','lapdip')">
                    	<input type="hidden" id="id_approved_id">
                    </td>
                    <td>Team Leader</td>
                    <td><?=create_drop_down( "cbo_team_leader", 130, "select id,team_leader_name from lib_marketing_team where status_active =1 and is_deleted=0 order by team_leader_name","id,team_leader_name", 1, "-- Select Team --", $selected, "load_drop_down( 'requires/sample_booking_non_order_controller', this.value, 'cbo_dealing_merchant', 'div_marchant' ) " ); ?></td>
                </tr>
				<tr>
				...
				<td>Dealing Merchant</td>
                    ...
                    <td>Copy From</td>
                    <td><input class="text_boxes" type="text" style="width:120px" name="txt_copy_from_booking" id="txt_copy_from_booking" readonly /></td>
                    <td>Ready To Approved</td>
                    <td><? echo create_drop_down( "cbo_ready_to_approved", 130, $yes_no,"", 1, "-- Select--", 2, "get_php_form_data( this.value+'_'+document.getElementById('txt_booking_no').value, 'check_dtls_part', 'requires/sample_booking_non_order_controller');","","" ); ?>
                    	<input type="hidden" name="is_found_dtls_part" id="is_found_dtls_part" value="1"/>
                    </td>
                    <td style="display: none">UOM</td>
                	<td style="display: none"><? echo create_drop_down( "cbouom_id", 130, $unit_of_measurement,'', 1, '-UOM-',"", "",0,"1,12,23,27" ); ?></td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                </tr>
                <tr>
                    <td colspan="4">
                    	<input type="button" class="image_uploader" style="width:100px" value="ADD/VIEW IMAGE" onClick="file_uploader ( '../../', document.getElementById('txt_booking_no').value,'', 'sample_booking_non', 0 ,1)">&nbsp;&nbsp;&nbsp;&nbsp;
                        ...
                        <input type="button" id="set_button" class="image_uploader" style="width:100px;" value="Accessories" onClick="open_trims_acc_popup('Accessories Dtls')" /></td>
    ...
    <form name="orderdetailsentry_2"  autocomplete="off" id="orderdetailsentry_2">
        ...
        <legend>Sample Fab. Booking Dtls. [Without Order] </legend>
            <table  width="960" cellspacing="2" cellpadding="0" border="0">
                <tr>
                    <td width="100">Style Ref</td>              <!-- 11-00030  -->
                    <td width="150">
                        <input type="text" id="txt_style_no" name="txt_style_no" class="text_boxes" style="width:120px" onDblClick="open_sample_popup()" placeholder="Double Click to Search" readonly/>
                    ...
                    <td width="100">Style Des.</td>
                    <td width="150"><input type="text" id="txt_style_des" name="txt_style_des"  class="text_boxes" style="width:120px"/></td>
                    <td width="100" class="must_entry_caption">Sample type</td>
                    <td width="150" id="sampletd"><?=create_drop_down( "cbo_sample_type", 130, "select sample_name,id from lib_sample where is_deleted=0 and status_active=1 order by sample_name","id,sample_name", '1', "--Select--", '', "","",'' ); ?></td>
                    <td width="100" class="must_entry_caption">Body Part</td>
                    <td><?=create_drop_down( "cbo_body_part", 130, $body_part,"", 1, "--Select--", $selected, "" ); ?></td>
                </tr>
                <tr>
                    <td>Color Type</td>
                    <td><?=create_drop_down( "cbo_color_type", 130, $color_type,"", 1, "--Select--", $selected, "" ); ?></td>
                    ...
                    <td id="fabricdescription_id_td">
                        ...
                        <input type="text" id="txt_fabricdescription" name="txt_fabricdescription" class="text_boxes" style="width:120px" onDblClick="open_fabric_decription_popup();" placeholder="Browse" readonly title="" />
                    </td>
                    <td>GSM</td>
                    <td><input name="txt_gsm" id="txt_gsm" class="text_boxes" type="text" value=""  style="width:120px "/></td>
                    <td>Dia/ Width</td>
                    <td>
                        <input name="txt_dia_width" id="txt_dia_width" class="text_boxes" type="text" value=""  style="width:45px "/>
                        <?=create_drop_down('cbo_dia_width_type', 70, $fabric_typee,'', 1, '-- Select --', ''); ?>
                    </td>
                </tr>
                <tr>
                    <td>Gmts Color</td>
                    <td><input name="txt_gmt_color" id="txt_gmt_color" class="text_boxes" type="text" value=""  style="width:120px "/></td>
                    <td class="must_entry_caption">Fabric Color</td>
                    <td id="fabriccolor_id_id_td" ><input name="txt_color" id="txt_color" class="text_boxes" type="text" value=""  style="width:120px "/></td>
                    <td>Gmts Size</td>
                    <td><input name="txt_gmts_size" id="txt_gmts_size" class="text_boxes" type="text" value="" style="width:120px "/></td>
                    <td>Item Size</td>
                    <td id="itemsize_id_td"><input name="txt_size" id="txt_size" class="text_boxes" type="text" value=""  style="width:120px "/></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td>Rate</td>
                    <td><input name="txt_rate" id="txt_rate" class="text_boxes_numeric" type="text" value=""  style="width:120px " onChange="calculate_requirement();" /></td>
                    <td>Amount</td>
                    <td><input name="txt_amount" id="txt_amount" class="text_boxes_numeric" type="text" value=""  style="width:120px " readonly/></td>
                    <td>Body Type</td>
                    <td><? echo create_drop_down( "cbo_body_type", 130, $body_type_arr,"", 1, "--Select--", $selected, "" ); ?></td>
                    <td>Item Qty.</td>
                    <td><input name="txt_item_qty" id="txt_item_qty" class="text_boxes_numeric" type="text" style="width:120px" maxlength="200" title="Maximum 200 Character" /></td>
                </tr>
                <tr>
                    <td>Yarn Details</td>
                    <td><input name="txt_yarn_details" class="text_boxes" ID="txt_yarn_details" style="width:120px" maxlength="200" title="Maximum 200 Character"></td>
                    <td id="td_knit_caption" >Knitting Charge/KG</td>
                    <td id="td_knit_input"><input name="txt_knitting_charge" id="txt_knitting_charge" class="text_boxes_numeric" type="text"  style="width:120px"  /></td>
                    <td>GMT Qty </td>
                    <td>
                        <input name="txt_bh_qty" id="txt_bh_qty" class="text_boxes_numeric" type="text" style="width:50px " placeholder="BH Qty"/>
                        <input name="txt_rf_qty" id="txt_rf_qty" class="text_boxes_numeric" type="text" style="width:50px;" placeholder="RF Qty"/>
                    </td>
                    <td>Delivery Date</td>
                    <td><input id="txt_delivery_dates" class="datepicker" style="width:120px" name="txt_delivery_dates" type="text"></td>
                </tr>
                <tr>
                	<td class="must_entry_caption">UOM</td>
                	<td><? echo create_drop_down( "cbouom", 130, $unit_of_measurement,"", 1, "-UOM-",$selected, "", "","1,12,23,27" ); ?></td>

                    <td id="cbo_fabric_source_dtls_id">Fabric Source</td>
                    <td><? echo create_drop_down( "cbo_fabric_source_dtls", 130, $fabric_source,"", 1, "-- Select --", $selected,"", "", "1,4"); ?></td>
                    <td>Remarks</td>
                    <td colspan="3">
                        <input name="txt_remarks" id="txt_remarks" class="text_boxes" type="text" value=""  style="width:370px" maxlength="200" title="Maximum 200 Character"  />
