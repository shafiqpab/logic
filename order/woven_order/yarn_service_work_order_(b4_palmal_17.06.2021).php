...
function fnc_yarn_service_wo(operation)
{
	...
	else
	{
		...
		else
		{
			...
			var mstDataString = "cbo_company_name*cbo_service_type*cbo_supplier_name*txt_booking_date*txt_attention*txt_tenor*cbo_currency*txt_exchange_rate*cbo_pay_mode*cbo_source*txt_delivery_date*cbo_is_sales_order*update_id*txt_booking_no*cbo_with_order*update_dtls_ids*original_pro_data*txt_ref_no"+twistingDataStr;
...
function openmypage_job(thisVal, title)
{
	...
	else
	{
		...
		else
		{
			...
			width = "930px";
			page_link='requires/yarn_service_work_order_controller.php?action=booking_search_popup&company='+company+'&txt_booking_date='+txt_booking_date+'&txt_job_no='+txt_job_no;
		}
			...
			if(product_info.length > 1)
			{
				...
				for( var i = 1; i < product_info.length; i++ )
				{
					...
					$('#dtls_container tr').each(function(i)
					{
						...
						textinput.eq(13).attr('id', 'txt_allocated_qty_' + i).val(data[4]);
				...
				for( var i = 1; i < product_info.length; i++ )
				{
					...
					$('#dtls_container tr').each(function(i)
					{
						...
						if(i===0)
						{
							textinput.eq(0).attr('id', 'txt_job_no_' + i).val(job_no);
							textinput.eq(1).attr('id', 'txt_job_id_' + i).val(job_id);
							textinput.eq(2).attr('id', 'txt_fab_booking_no_' + i).val(job_id);
						}
						else
						{
							textinput.eq(0).attr('id', 'txt_job_no_' + i).val(job_no).attr('disabled',true);
							textinput.eq(1).attr('id', 'txt_job_id_' + i).val(job_id).attr('disabled',true);
							textinput.eq(2).attr('id', 'txt_fab_booking_no_' + i).val(job_id).attr('disabled',true);
						}
			...
			<fieldset style="width:800px; margin-bottom:5px;">
				...
				<table cellspacing="4" cellpadding="8" border="0">
					...
					<tr>
						...
						<td>
							<?
							echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected,"load_drop_down( 'requires/yarn_service_work_order_controller',this.value, 'load_drop_down_supplier', 'supplier_td' );get_php_form_data('94'+'_'+this.value, 'populate_field_level_access_data', 'requires/yarn_service_work_order_controller' );change_job_title(document.getElementById('cbo_with_order').value);",0);
							?>
						</td>
						<td  align="right" class="must_entry_caption" width="80">Service Type</td>
						<td>
							<?
							echo create_drop_down( "cbo_service_type", 160, $yarn_issue_purpose,"", 1, "-- Select --", $selected, "load_drop_down( 'requires/yarn_service_work_order_controller',$('#cbo_company_name').val()+'_'+this.value, 'load_drop_down_supplier', 'supplier_td' );set_fin_visibility(this.value);",0,'12,15,38,46,7,50,51');
							?>
						</td>
