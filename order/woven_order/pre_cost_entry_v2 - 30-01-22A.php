...
// echo "<pre>";
// echo ($_SESSION['logic_erp']['data_arr'][158][17]['cbo_costing_per']['defalt_value']);
// echo "</pre>";
...
var cmValidation='<? echo $qcCons_from; ?>';
	//alert(cmValidation);
var israte_popup=2;
<?
...
// Trim Cost-----------------------------------------------------
	...
	function open_consumption_popup_trim(page_link,title,trorder)
	{
		...
		emailwindow.onclose=function()
		{
			...
			/*var pcsgmts = Array();
			var cons_breck_down_data = cons_breck_down.value.split('__');
			var total_pcsgmts = 0;
			for(var i=0; i< cons_breck_down_data.length; i++){
				var data = cons_breck_down_data[i].split('_');
				if(data[2] != 0){
					var qty = data[15]*1;
					total_pcsgmts += qty;
				}
			...
			var job_qty = document.getElementById('txt_offer_qnty').value;
			console.log('country_gmts: '+total_pcsgmts+'job_qty: '+job_qty+'cons_dzn: '+cons_dzn);
			var country_percent = (total_pcsgmts/job_qty)*100;*/


			//var avg_cons=this.contentDoc.getElementById("avg_cons");
			//document.getElementById('txtconsdzngmts_'+trorder).value=avg_cons.value;
			//var dzn_country_percent = cons_dzn*(country_percent/100);
...
//Master form---------------------------------------------------------------------------
	...
	function generate_report(type)
	{
		...
		else
		{
			...
			else
			{
				...
				if(type!='mo_sheet' && type != 'budgetsheet' && type != 'mo_sheet_1' && type != 'mo_sheet_3')
				{
				...
				var data="action="+type+"&zero_value="+zero_val+"&rate_amt="+rate_amt+"&excess_per_val="+excess_per_val+"&supplier_check="+supplier_check+"&"+get_submitted_data_string('txt_job_no*cbo_company_name*cbo_buyer_name*txt_style_ref*txt_costing_date*txt_po_breack_down_id*cbo_costing_per*print_option_id',"../../");
	...
	function fnc_budgete_cost_validate(type)
	{
		...
		var fab_cons_kg=0; var fab_cons_mtr=0; var fab_cons_yds=0; var fab_amount=0; var sp_oparation_amount=0; var acc_amount=0; var fright_amount=0; var lab_amount=0; var misce_amount=0; var other_amount=0; var comm_amount=0; var fob_amount=0; var cm_amount=0; var commercial_cost=0; var rmg_ratio=0; var is_approved=0;
	...
	function fnc_po_pop()
	{
		...
		{
			var hidd_job_id=$("#hidd_job_id").val();
			var page_link='requires/pre_cost_entry_controller_v2.php?action=order_no_popup&hidd_job_id='+hidd_job_id;
        ...
        <form name="precosting_1" id="precosting_1" autocomplete="off" enctype="multipart/form-data">
            ...
            <table  width="1070" cellspacing="2" cellpadding=""  border="0">
                <tr>
                    ...
                    <td width="130"><? echo create_drop_down("cbo_company_name", 120, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "","1"); //load_drop_down( 'requires/pre_cost_entry_controller_v2',this.value, 'load_drop_down_buyer', 'buyer_td' );  load_drop_down( 'requires/pre_cost_entry_controller_v2', this.value, 'load_drop_down_agent', 'agent_td' );set_currier_cost_method_variable(this.value);check_exchange_rate();?></td>
                ...
                <tr>
                    ...
                    <td><? echo create_drop_down( "cbo_costing_per", 120, $costing_per, "",0, "0", 1, "change_cost_per(this.value)","","" ); //change_cost_per(this.value) ?></td>
                    <td>Region</td>
                ...
                <tr>
                    ...
                    <td>Images</td>
                    <td><input type="button" class="image_uploader" style="width:120px" value="CLICK TO ADD/VIEW IMAGE" onClick="file_uploader ( '../../', document.getElementById('update_id').value,'', 'knit_order_entry', 0 ,1,2)" /></td>
                </tr>
                <tr>
                    <td>File no</td>
                ...
                <tr>
                     <td>Refusing Cause</td>
                      <td colspan="5"><input class="text_boxes" type="text" style="width:615px;" name="txt_refusing" id="txt_refusing" maxlength="500" title="Maximum 500 Character" readonly placeholder="Double Click for Browse" onClick="openmypage_refusing_cause();"/></td>
                </tr>
                ...
                <tr>
                    <td align="center" height="10" colspan="8" valign="top" id="check_sms2" style="font-size:18px; color:#F00">&nbsp;</td>
                </tr>
                <tr>
