...
function calculate_requirement(i,type)
{
	...
	var process_loss_method_id=return_global_ajax_value(cbo_company_name+'_'+cbo_fabric_natu, 'process_loss_method_id', '', 'requires/multi_requisition_wise_sample_fabric_booking_without_order_controller');
	if(type==2)
	{
		var WastageQty='';
...
function add_rf_tr(i)
   {
		...
		else
		{
		   ...
		   var k=i-1;
			...
			$('#txtRfColor_'+i).removeAttr("onDblClick").attr("onDblClick","openmypage_rf_color('requires/multi_requisition_wise_sample_fabric_booking_without_order_controller.php?action=color_popup_rf','Color Search','"+i+"');");
...
function openmypage_requisition()
{
		...
		var page_link = 'requires/multi_requisition_wise_sample_fabric_booking_without_order_controller.php?&action=requisition_id_popup&company='+company+'&buyer='+buyer+'&int_ref='+int_ref;
		...
		emailwindow.onclose=function()
		{
			...
			{
				freeze_window(5);
				get_php_form_data(mst_tbl_id, "populate_data_from_requisition_search_popup", "requires/multi_requisition_wise_sample_fabric_booking_without_order_controller" );
...
function fnc_load_tr(data,type)//('data1,data2',1)
{
		     	//type =1 for requisition browse and type=2 for booking save or browse
				//alert(data);
 		     	var list_view_tr = return_global_ajax_value( data+'___'+type, 'load_php_dtls_form', '', 'requires/multi_requisition_wise_sample_fabric_booking_without_order_controller');
				if(list_view_tr!='')
				{
...
function openmypage_booking(page_link,title)
{
	...
	emailwindow.onclose=function()
	{
		...
		{
			freeze_window(5);
 			get_php_form_data( theemail.value, "populate_data_from_search_popup", "requires/multi_requisition_wise_sample_fabric_booking_without_order_controller" );
...
{
	var color_from_library=return_global_ajax_value(company_id, 'color_from_library', '', 'requires/multi_requisition_wise_sample_fabric_booking_without_order_controller');
...
{
	//var page_link='requires/multi_requisition_wise_sample_fabric_booking_without_order_controller.php?action=color_popup'
	//alert(page_link)
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/multi_requisition_wise_sample_fabric_booking_without_order_controller.php?action=color_popup&buyer_name='+buyer_name, 'Color Select Pop Up', 'width=250px,height=450px,center=1,resize=1,scrolling=0','../')
	emailwindow.onclose=function()
	{
		var theform=this.contentDoc.forms[0];
...
function fnc_fabric_booking( operation )
{
	if(operation==1)
	{
	  ...
	  var issue_number=return_global_ajax_value(txt_booking_no, 'booking_no_check', '', 'requires/multi_requisition_wise_sample_fabric_booking_without_order_controller');
	 ...
	 //alert(issue_number);
	  if(trim(issue_number)!='')
	  {
		...
		get_php_form_data( txt_booking_no, "populate_data_from_search_popup", "requires/multi_requisition_wise_sample_fabric_booking_without_order_controller" );
	 	return;
	  }
	}
	...
	if (form_validation('cbo_company_name*txt_booking_date*txt_delivery_date*cbo_pay_mode*cbo_supplier_name*cbo_payterm*cbo_fabric_source','Company Name*Booking Date*Delivery Date*Pay Mode*Supplier Name*Pay term*Fabric Source')==false)
	{
		return;
	}
	else
	{
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_booking_no*cbo_company_name*cbo_buyer_name*cbo_brand_id*txt_booking_date*txt_delivery_date*cbo_pay_mode*cbo_supplier_name*cbo_fabric_source*cbo_basis*cbo_currency*txt_exchange_rate*cbo_payterm*cbo_team_leader*cbo_dealing_merchant*txt_fabriccomposition*txt_remarks*txt_tenor*txt_attention*cbo_ready_to_approved*update_id',"../../");
		//alert(data);
		freeze_window(operation);
		http.open("POST","requires/multi_requisition_wise_sample_fabric_booking_without_order_controller.php",true);
...
function open_fabric_decription_popup()
{
	...
	var page_link='requires/multi_requisition_wise_sample_fabric_booking_without_order_controller.php?action=fabric_description_popup&fabric_nature='+cbofabricnature
...
function generate_fabric_report(type)
{
	...
	else
	{
		$report_title=$( "div.form_caption" ).html();
		 if(type==1)
		{
		var data="action=show_fabric_booking_report"+get_submitted_data_string('txt_booking_no*cbo_company_name*id_approved_id*cbo_fabric_source*hidden_requisition_id',"../../")+'&report_title='+$report_title;
		}
		//freeze_window(5);
		http.open("POST","requires/multi_requisition_wise_sample_fabric_booking_without_order_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
...
function check_exchange_rate()
	{
		...
		var response=return_global_ajax_value( cbo_currercy+"**"+booking_date, 'check_conversion_rate', '', 'requires/multi_requisition_wise_sample_fabric_booking_without_order_controller');
	...
	// 	var response=return_global_ajax_value( cbo_company_name, 'check_kniting_charge', '', 'requires/multi_requisition_wise_sample_fabric_booking_without_order_controller');
...
function print_button_setting()
	{
		$('#button_panel').html('');
		get_php_form_data($('#cbo_company_name').val(),'print_button_variable_setting','requires/multi_requisition_wise_sample_fabric_booking_without_order_controller' );
	}
...
function fabic_srce_con_fnc()
{
   ...
   else
   {
	...
	// 	var page_link = 'requires/multi_requisition_wise_sample_fabric_booking_without_order_controller.php?supplier_id='+supplier_id+'&pay_mode='+pay_mode+'&action=supplier_popup';
...
function remark_popup(i)
{
	var txtremark = $("#txtremark_"+i).val();
		//alert(txtremark);
		var page_link='requires/multi_requisition_wise_sample_fabric_booking_without_order_controller.php?action=remark_popup&txtremark='+txtremark;
		var title="Remark";
...
<!-- check_kniting_charge();  color_from_library( this.value );check_kniting_charge();print_button_setting();-->
<body onLoad="set_hotkey(); check_exchange_rate(); print_button_setting();">
<div style="width:100%;" align="center">
     <? echo load_freeze_divs ("../../",$permission);  ?>
            	<form name="fabricbooking_1"  autocomplete="off" id="fabricbooking_1">
            	<fieldset style="width:1140px;">
                <legend>Multi Requisition Sample fabric Booking [Without Order]</legend>
            		<table  width="1140" cellspacing="2" cellpadding="0" border="0">
                        <tr>
                            <td colspan="5" align="right" class="must_entry_caption"> Booking No</td>
                            <td colspan="5"><input class="text_boxes" type="text" style="width:110px" onDblClick="openmypage_booking('requires/multi_requisition_wise_sample_fabric_booking_without_order_controller.php?action=fabric_booking_popup','fabric Booking Search')" readonly placeholder="Double Click Here" name="txt_booking_no" id="txt_booking_no"/></td>
						</tr>
						<tr>
                            <td width="100" class="must_entry_caption">Company Name</td>
                            <td width="130"><?=create_drop_down( "cbo_company_name", 120, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3)  $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected, "load_drop_down( 'requires/multi_requisition_wise_sample_fabric_booking_without_order_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/multi_requisition_wise_sample_fabric_booking_without_order_controller', document.getElementById('cbo_pay_mode').value, 'load_drop_down_suplier', 'sup_td' ); print_button_setting();","","" ); ?>
                                <input type="hidden" id="update_id" name="update_id"/>
                                <input type="hidden" id="report_ids" name="report_ids"/>
                            </td>
                            <td width="100" class="must_entry_caption">Buyer Name</td>
                            <td width="130" id="buyer_td">
                            <?
								$sql="select id,buyer_name from lib_buyer  where status_active=1 and is_deleted=0   order by buyer_name";
								echo create_drop_down( "cbo_buyer_name", 120, $sql,"id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down( 'requires/multi_requisition_wise_sample_fabric_booking_without_order_controller', this.value, 'load_drop_down_brand', 'brand_td' );","","" );
                            ?>
                            </td>
							<td width="100">Brand</td>
                        	<td width="130" id="brand_td"><?=create_drop_down("cbo_brand_id", 120, $blank_array,"",1, "-Brand-", $selected,""); ?></td>
                            <td width="100" class="must_entry_caption">Booking Date</td>
                            <td><input class="datepicker" type="text" style="width:110px" name="txt_booking_date" id="txt_booking_date" onChange="check_exchange_rate();" value="<? echo date("d-m-Y")?>" disabled /></td>
                        ...
                        <tr>
                            ...
                            <td><?=create_drop_down( "cbo_pay_mode", 120, $pay_mode,"", 1, "-- Select Pay Mode --",0, "fn_clear_supplier_data();load_drop_down( 'requires/multi_requisition_wise_sample_fabric_booking_without_order_controller', this.value, 'load_drop_down_suplier', 'sup_td' )","","1,2,3,5" ); ?></td>
							<td class="must_entry_caption">Supplier Name</td>
                            <td id="sup_td"><?=create_drop_down( "cbo_supplier_name", 120, "select id,supplier_name from lib_supplier where status_active =1 and is_deleted=0 order by supplier_name","id,supplier_name", 1, "-- Select Supplier --", $selected, "get_php_form_data( this.value+'_'+document.getElementById('cbo_pay_mode').value, 'load_drop_down_attention', 'requires/multi_requisition_wise_sample_fabric_booking_without_order_controller');",0 ); ?></td>
                            <td class="must_entry_caption">Fabric Source</td>
                            <td><?=create_drop_down( "cbo_fabric_source", 120, $fabric_source,"", 1, "-- Select --", "","enable_disable(this.value);fabic_srce_con_fnc()", "", "1,2,3"); ?></td>
							<td>Basis</td>
                        	<td><?$basis= array(1=>"Requisition"); echo create_drop_down( "cbo_basis", 120, $basis,"", 1, "-- Select Basis --", 1, "",1 );  ?></td>
                            <td>Currency</td>
                        ...
                        <tr>
                        	...
                        	<td><?$pay_term= array(1 => "At Sight", 2 => "Usance", 3 => "Cash In Advance", 7=>"Cash on Delivery");  echo create_drop_down( "cbo_payterm", 120, $pay_term,"", 1, "--Select--", 0, "","","" ); ?></td>
							<td>Team Leader</td>
    						<td><?=create_drop_down( "cbo_team_leader", 120, "select id,team_leader_name from lib_marketing_team where status_active =1 and is_deleted=0 order by team_leader_name","id,team_leader_name", 1, "-- Select Team --", $selected, "load_drop_down( 'requires/multi_requisition_wise_sample_fabric_booking_without_order_controller', this.value, 'cbo_dealing_merchant', 'div_marchant' ) " ); ?></td>
							<td>Dealing Merchant</td>
    						<td id="div_marchant"><?=create_drop_down( "cbo_dealing_merchant", 120, $blank_array,"", 1, "-- Select Team Member --", $selected, "" ); ?></td>
							<td>Fabric Comp.</td>
                        	<td><input class="text_boxes" type="text" style="width:110px;" name="txt_fabriccomposition" id="txt_fabriccomposition"/></td>
                        </tr>
                        <tr>
							<td>Remarks</td>
                        	<td colspan="3">
                            <input class="text_boxes" type="text" style="width:340px;"  name="txt_remarks" id="txt_remarks"/>
							<td>Tenor</td>
                    		<td><input style="width:110px;" type="text" class="text_boxes_numeric" name="txt_tenor" id="txt_tenor" /></td>
                        	<td>Attention</td>
                        	<td colspan="3">
                            	<input class="text_boxes" type="text" style="width:340px;"  name="txt_attention" id="txt_attention"/>
                            	<input type="hidden" class="image_uploader" style="width:130px" value="Lab DIP No" onClick="openmypage('requires/multi_requisition_wise_sample_fabric_booking_without_order_controller.php?action=lapdip_no_popup','Lapdip No','lapdip')">
                                <input type="hidden" id="id_approved_id">
                            </td>
                        </tr>
                        <tr>
                            <td>Ready To Approved</td>
                        	<td><?=create_drop_down( "cbo_ready_to_approved", 120, $yes_no,"", 1, "-- Select--", 0, "get_php_form_data( this.value+'_'+document.getElementById('txt_booking_no').value, 'check_dtls_part', 'requires/multi_requisition_wise_sample_fabric_booking_without_order_controller');","","" ); ?>
                            	<input type="hidden" name="is_found_dtls_part" id="is_found_dtls_part" value="1"/>
                            </td>
						</tr>
						<tr>
                            <td></td>
                            <td><input type="button" class="image_uploader" style="width:120px" value="ADD/VIEW IMAGE" onClick="file_uploader( '../../', document.getElementById('txt_booking_no').value,'', 'multi_requisition_wise_sample_fabric_booking_without_order', 0 ,1)"></td>
                        	<td colspan="2"><input type="button" class="image_uploader" style="width:95px" value="ADD FILE" onClick="file_uploader( '../../', document.getElementById('txt_booking_no').value,'', 'multi_requisition_wise_sample_fabric_booking_without_order', 2 ,1)"></td>
							<td>&nbsp;</td>
							<td >
                            	<? include("../../terms_condition/terms_condition.php");
								   terms_condition(694,'txt_booking_no','../../');
								?>
                            </td>
                        </tr>
                        <tr>
...
<fieldset style="width:1760px;" id="required_fab_dtls">
<table width="100%" cellpadding="0" cellspacing="2" align="center" >
<tr>
	<td align="right" ><strong> Sample Requisition &nbsp;</strong></td>
     <td colspan="3"> <input type="text" name="txt_requisition" id="txt_requisition" class="text_boxes" style="width: 145px;margin-right: 38px;" placeholder="Browse Requisition" readonly onDblClick="openmypage_requisition();" value="" >
...
<tr>
             <td align="center" valign="top" id="po_list_view">
                ...
                <table cellpadding="0" cellspacing="0" width="1760" class="rpt_table" border="1" rules="all" id="tbl_required_fabric">
                    <thead>
					<th class="must_entry_caption">Requisition No. </th>
                    ...
                    <tbody id="required_fabric_container">
                        ...
                        <tr id="tr_1" style="height:10px;" class="general">
							<td align="center" id="rf_req_id_1"> 
                                 <input style="width:95px;" type="text" class="text_boxes"  name="txt_req_num_1" id="txt_req_num_1" readonly disabled=""/>
								 <input type="hidden" name="txt_req_hidden_id_1" id="txt_req_hidden_id_1" value="">
                            </td>
							<td align="center" id="rf_style_1">
                                 <input style="width:95px;" type="text" class="text_boxes"  name="txt_style_1" id="txt_style_1" readonly disabled=""/>
                            </td>
...
//load_drop_down( 'requires/multi_requisition_wise_sample_fabric_booking_without_order_controller', document.getElementById('cbo_pay_mode').value, 'load_drop_down_suplier', 'sup_td' )
});
