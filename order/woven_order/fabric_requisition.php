...
echo load_html_head_contents("Fabric Requisition", "../../", 1, 1,$unicode,1,'');
//--------------------------------------------------------------------------------------------------------------------
$date                  = date('d-m-Y');
...
$cbo_company_name      = create_drop_down( "cbo_company_name", 140, "select id,company_name from lib_company  comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name", "id,company_name",1, "-- Select Company --", "", "load_drop_down( 'requires/fabric_requisition_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/fabric_requisition_controller', $('#cbo_pay_mode').val()+'_'+this.value, 'load_drop_down_suplier', 'sup_td' ); check_month_setting(); validate_suplier(); get_php_form_data( this.value, 'company_wise_report_button_setting','requires/fabric_requisition_controller' );check_exchange_rate();",0,"" );
...
$cbo_pay_mode          = create_drop_down( "cbo_pay_mode", 140, $pay_mode,"", 1, "-- Select Pay Mode --", 0, "load_drop_down( 'requires/fabric_requisition_controller', this.value+'_'+$('#cbo_company_name').val(), 'load_drop_down_suplier', 'sup_td'); fnc_greyFabPurchase(this.value);","","1,2,3,4,5" );
...
<?
if(isset($_SESSION['logic_erp']['data_arr'][611]));
{
$data_arr= json_encode( $_SESSION['logic_erp']['data_arr'][611] );
}
//print_r($_SESSION['logic_erp']['mandatory_field'][611]);
echo "var mandatory_field = '". implode('*',$_SESSION['logic_erp']['mandatory_field'][611]) . "';\n";
echo "var mandatory_message = '". implode('*',$_SESSION['logic_erp']['mandatory_message'][611]) . "';\n";
...
?>
	...
	function openmypage_booking(page_link,title)
	{
		...
		emailwindow.onclose=function()
		{
			...
			if (theemail.value!="")
			{
				...
				get_php_form_data( theemail.value, "populate_data_from_search_popup", "requires/fabric_requisition_controller" );
				set_button_status(1, permission, 'fnc_fabric_booking',1);
				fnc_show_booking_list();
				get_php_form_data( document.getElementById('cbo_company_name').value, 'company_wise_report_button_setting','requires/fabric_requisition_controller' );
				check_month_setting();
			}
	...
	function openmypage_order(page_link,title)
	{
		...
		emailwindow.onclose=function(){
			...
			if (id.value!=""){
				...
				get_php_form_data( id.value+"_"+cbo_fabric_natu+"_"+cbouom+"_"+cbo_fabric_source, "populate_order_data_from_search_popup", "requires/fabric_requisition_controller" );
				check_month_setting();
				release_freezing();
			}
		}
	...
	function fnc_generate_booking(){ 
		...
		if (form_validation('txt_booking_no*txt_order_no_id*cbo_fabric_natu*cbo_fabric_source','Booking No*Order No*Fabric Nature*Fabric Source')==false){
			...
			var data="action=generate_fabric_booking"+get_submitted_data_string('txt_booking_no*txt_order_no_id*cbo_fabric_natu*cbo_fabric_source*cbo_company_name*cbo_buyer_name*cbouom*cbo_fabric_description*cbo_level',"../../");
			http.open("POST","requires/fabric_requisition_controller.php",true);
	...
	function fnc_fabric_booking_reponse(){
		if(http.readyState == 4){
			...
			if(trim(reponse[0])=='booking'){
				alert("Booking Number Found :"+trim(reponse[2])+"\n So Update/Delete Not Possible")
				release_freezing();
				return;
			}
	...
	function generate_fabric_report_gr(type){
		...
		var page_link='requires/fabric_requisition_controller.php?action=booking_surch_option&booking_option='+booking_option+'&booking_option_id='+booking_option_id+'&booking_option_no='+booking_option_no;
	...
	function openmypage_unapprove_request(){
		...
		var page_link = 'requires/fabric_requisition_controller.php?data='+data+'&action=unapp_request_popup';
	...
	function check_exchange_rate(){
		...
		var response=return_global_ajax_value( cbo_currercy+"**"+booking_date+"**"+cbo_company_name, 'check_conversion_rate', '', 'requires/fabric_requisition_controller');
	...
	function check_month_setting(){
		var cbo_company_name=$('#cbo_company_name').val();
		var response=return_global_ajax_value( cbo_company_name, 'check_month_maintain', '', 'requires/fabric_requisition_controller');
		var response=response.split("_");
		if(response[0]==1){
			$('#month_id').val(1);
	...
	function fill_attention(supplier_id){
		...
		var cbo_pay_mode=document.getElementById('cbo_pay_mode').value;
		var attention=return_global_ajax_value(supplier_id+"_"+cbo_pay_mode, 'get_attention_name', '', 'requires/fabric_requisition_controller');
	...
	function set_data(po_id,po_number,precost_id,booking_id){
		...
		var cbouom=document.getElementById('cbouom').value;
		var cbo_fabric_source=document.getElementById('cbo_fabric_source').value;
		get_php_form_data( po_id+"_"+cbo_fabric_natu+"_"+cbouom+"_"+cbo_fabric_source, "populate_order_data_from_search_popup", "requires/fabric_requisition_controller" );
	...
	function deletedata(booking_id){
		...
		var txt_booking_no=document.getElementById('txt_booking_no').value;
		var check_is_booking_used_id=return_global_ajax_value(txt_booking_no, 'check_is_booking_used', '', 'requires/fabric_requisition_controller');
		...
		if(cbo_level==1){
			...
			var data="action=save_update_delete_dtls_job_level&operation="+operation+'&total_row='+z+dltsid+data_all+"&delete_cause="+delete_cause+"&update_id="+update_id;
		}
	...
	function call_print_button_for_mail(mail_address,mail_body,type){
		var response=return_global_ajax_value(document.getElementById('cbo_company_name').value, 'send_mail_report_setting_first_select', '', 'requires/fabric_requisition_controller');
...
<body onLoad="set_hotkey(); check_exchange_rate(); check_month_setting(); ">
    <div style="width:100%;" align="center">
        ...
        <form name="fabricbooking_1"  autocomplete="off" id="fabricbooking_1">
            ...
            <legend>Fabric Requisition </legend>
                <table  width="1000" cellspacing="2" cellpadding="0" border="0">
                    <tr>
                        <td colspan="4" align="right" class="must_entry_caption"> Requisition No </td>
                        <td colspan="4">
                            <input class="text_boxes" type="text" style="width:130px" onDblClick="openmypage_booking('requires/fabric_requisition_controller.php?action=fabric_booking_popup','fabric Requisition Search')" readonly placeholder="Double Click for Booking"  name="txt_booking_no" id="txt_booking_no"/>
                    ...
                    <tr>
                        <td width="100" id="booking_td">Requisition Month</td>
                    ...
                    <tr>
                        ...
                        <td><?=$cbo_currency; ?></td>
                        <td>Exchange Rate</td>
                        <td><input style="width:130px;" type="text" class="text_boxes"  name="txt_exchange_rate" id="txt_exchange_rate" readonly  /> <input type="hidden" id="cbo_level" value="2"></td>
                        <td>Grey Fab. Pur.</td>
                    ...
                    <tr>
                    	<td>
                            <input type="button" id="btnshippingmark" class="image_uploader" style="width:95px;" value="Shipping Mark" onClick="open_shipping_mark_popup('requires/fabric_requisition_controller.php?action=shipping_mark_popup','Shipping Mark');" />
                            <input style="width:45px;" type="hidden" class="text_boxes" name="hiddshippingmark_breck_down" id="hiddshippingmark_breck_down" />
                        </td>
                    	<td align="center">
                            <input type="button" id="set_button2" class="image_uploader" style="width:95px;" value="Process Loss %" onClick="open_rmg_process_loss_popup('requires/fabric_requisition_controller.php?action=rmg_process_loss_popup','Process Loss %');" />
                        ...
                        <td>
                            ...
                            terms_condition(611,'txt_booking_no','../../');
                            ?>
                        </td>
                        <td><input type="button" id="set_button" class="image_uploader" style="width:120px;" value="Trims Dye To Match" onClick="dtm_popup('requires/fabric_requisition_controller.php?action=dtm_popup','DTM');"  /></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td align="center" colspan="8" valign="middle" class="button_container">
                            ...
                            <input type="button" id="set_button3" class="image_uploader" style="width:130px;" value="Collar & Cuff" onClick="open_colur_cuff_popup('requires/fabric_requisition_controller.php?action=colur_cuff_popup&permissions=<? echo $permission?>','Collar & Cuff')" />
        ...
        <form name="servicebookingknitting_2"  autocomplete="off" id="servicebookingknitting_2">
            ...
            <legend title="V3">Requisition Item Form &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
...
function fnc_process_data(){
	if (form_validation('cbo_company_name*txt_booking_no','Company*Booking No')==false){
		...
		var cbo_fabric_source=document.getElementById('cbo_fabric_source').value;
		var txt_booking_no=document.getElementById('txt_booking_no').value;
	    var page_link='requires/fabric_requisition_controller.php?action=fabric_search_popup';
		var title='Fabric Requisition Job Search';
...
function fnc_tna_date(){
	...
	var fabric_natu=document.getElementById('cbo_fabric_natu').value;
		...
		get_php_form_data( order_no_id+'_'+fabric_source+'_'+fabric_natu, "populate_order_data_from_search_popup", "requires/fabric_requisition_controller" );
...
$( document ).ready(function() {
	...
	load_drop_down( 'requires/fabric_requisition_controller', document.getElementById('cbo_pay_mode').value+'_'+$('#cbo_company_name').val(), 'load_drop_down_suplier', 'sup_td' )
});
</script>
</html>