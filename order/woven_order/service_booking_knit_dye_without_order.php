...
<script>
	...
	function check_exchange_rate()
	{
		...
		var response=return_global_ajax_value( cbo_currercy+"**"+booking_date+"**"+cbo_company_name, 'check_conversion_rate', '', 'requires/service_booking_knit_dye_without_order_controller');
	...
	function get_related_data(fabric_id)
	{
		...
		$('#txt_fin_dia').attr('disabled', 'disabled');
	...
	function fnc_knit_dye_non_ord_booking( operation )
	{
		...
		if (form_validation('cbo_company_name*txt_fabric_booking*txt_booking_date*txt_dev_start_date*txt_dev_end_date*cbo_pay_mode*cbo_knitdye_type*cbo_uom*txt_wo_qty*txt_rate*txt_amount','Company Name*Fab.Booking No*Booking Date*Delivery Start Date*Delivery End Date*Pay Mode*Booking Type*UOM*WO Qnty*Rate*Amount')==false)
		{
			return;
		}
		else
	...
	function fnc_knit_dye_non_ord_booking_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			if(reponse[0]==0 || reponse[0]==1)
			{
				...
				make_enable_disable(1);
				reset_form('','','cbo_fabric_source*cbo_fabric_description*txt_gsm*txt_fin_dia*txt_mc_dia*txt_mc_gg*txt_stitchLength*cbo_diaType*cbo_uom*txt_art_work*txt_wo_qty*txt_rate*txt_amount*txt_dev_start_date*txt_dev_end_date*txt_remarks*dtls_id*cbo_knitdye_type','txt_booking_date,<? echo date("d-m-Y"); ?>');
			}
			if(reponse[0]==2)
			{
				set_button_status(0, permission, 'fnc_knit_dye_non_ord_booking',1,1);
    ...
    <form name="servicebooking_1"  autocomplete="off" id="servicebooking_1">
        ...
        <legend>Service Booking</legend>
            <table width="1000" cellspacing="2" cellpadding="0" border="0">
                <tr>
                    ...
                    <td colspan="4">
                        <input class="text_boxes" type="text" style="width:130px" onDblClick="openmypage_booking('requires/service_booking_knit_dye_without_order_controller.php?action=knit_dye_without_order_booking_search','Knitting and Dyeing Without Order Booking Search')" readonly placeholder="Browse" name="txt_booking_no" id="txt_booking_no"/>
                        <input class="text_boxes" type="hidden" style="width:130px"  readonly  name="txt_mst_id" id="txt_mst_id" />
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td><?=create_drop_down( "cbo_knitdye_source", 130, $knitting_source,"", 1, "-- Select Source --", "", "load_drop_down( 'requires/service_booking_knit_dye_without_order_controller', this.value+'_'+document.getElementById('cbo_company_name').value, 'load_drop_down_supplier', 'suplier_td' )","","1,3" ); ?></td>
                    <td>Supplier Name</td>
                    <td id="suplier_td"><?=create_drop_down( "cbo_supplier_name", 130, $blank_array,"", 1, "-- Select Supplier --", $selected, "get_php_form_data( this.value, 'load_drop_down_attention', 'requires/service_booking_knit_dye_without_order_controller');",0 ); ?></td> 
                      <td>Tenor</td>
                ...
                <tr>
                    ...
                    <td colspan="3">
                        ...
                        <input type="hidden" class="image_uploader" style="width:62px" value="Lab DIP No" onClick="openmypage( 'requires/service_booking_knit_dye_without_order_controller.php?action=lapdip_no_popup', 'Lapdip No','lapdip')">
                    </td>
                    <td>&nbsp;</td>
                	<td align="center">
                		<? 
							include("../../terms_condition/terms_condition.php");
							terms_condition(194,'txt_booking_no','../../');
						?>
                	</td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
        ...
        <legend>Service Booking Details</legend>
            <table border="0" width="1000" cellpadding="0" cellspacing="2">
                <tr>
                    <td width="110px" class="must_entry_caption">Fabric Source</td>
                    <td width="140px"><? echo create_drop_down( "cbo_fabric_source", 130, $aop_nonor_fabric_source, "",0, "", 1, "load_fabric_dropdown(this.value)","","1" ); ?></td>
                    <td width="110px" class="must_entry_caption">Fabric Description</td>
                    <td width="140px" id="fabric_description_td"><? echo create_drop_down( "cbo_fabric_description", 130, $blank_array, "",1, "-- Select Fabric --", $selected, "","","" ); ?></td>
                    <td width="110px">GSM</td>   
                    <td width="140px"><input class="text_boxes" type="text" style="width:120px;" name="txt_gsm" id="txt_gsm"/></td>
                    <td width="110px">F. Dia</td>   
                    <td><input class="text_boxes" type="text" style="width:30px;" name="txt_fin_dia" id="txt_fin_dia"/>
                   M.Dia <input class="text_boxes" type="text" style="width:30px;" name="txt_mc_dia" id="txt_mc_dia"/>
                    </td>
                </tr>
                <tr>
                    <td class="must_entry_caption">UOM</td>   
                    <td id="uom_td"><? echo create_drop_down( "cbo_uom", 130, $unit_of_measurement,"", 1, "-- Select --", $selected, "","","" ); ?></td>
                    <td>Artwork No</td>   
                    <td><input class="text_boxes" type="text" style="width:120px;" name="txt_art_work" id="txt_art_work"/></td>   
                    <td>Gmts. Color</td>   
                    <td id="gmtcolor_td"><? echo create_drop_down( "cbo_gmtcolor", 130, $blank_array,"", 1, "-- Select --", $selected, "",1,"" ); ?></td>
                    <td class="must_entry_caption">WO. Qty.</td>   
                    <td><input class="text_boxes_numeric" type="text" style="width:120px;" onChange="calculate_amount()" name="txt_wo_qty" id="txt_wo_qty"/></td>
                </tr>
                <tr>
                 ...
                 <tr>
                    <td>MC Gauge</td>   
                    <td><input class="text_boxes" type="text" style="width:30px;" name="txt_mc_gg" id="txt_mc_gg"/> &nbsp;&nbsp; S/L &nbsp;&nbsp;<input class="text_boxes" type="text" style="width:30px;" name="txt_stitchLength" id="txt_stitchLength"/></td>
                     <td class="must_entry_caption">Process</td>
					<td>
					<?
						$filtered_ids="1,25,26,31,33,35,36,37,60,62,63,64,65,66,67,68,69,70,71,73,82,83,84,85,89,90,91,93,94,127,129,135,136,145,156,170,227,205,257,258,259,260,261,263,287,412,413,429,456,462,475,478,484,458";
						echo create_drop_down( "cbo_knitdye_type", 130, $conversion_cost_head_array,"", 1, "--Select Type--", "", "","",$filtered_ids );
					?>
					</td>
                    <td>Dia/ W. Type</td>
                    <td><? echo create_drop_down( "cbo_diaType", 130, $fabric_typee,"", 1, "-- Select --", $selected, "",0,"" ); ?></td>
                    <td>&nbsp;</td> 
                    <td>&nbsp;</td> 
                </tr>
                <tr>
                    <td>Remarks</td>   
                    <td colspan="3"><input spellcheck="true"  class="text_boxes" type="text" style="width:370px;" name="txt_remarks" id="txt_remarks"/></td>
                    <td>&nbsp;</td> 
                    <td valign="middle">
                    	<input type="button" class="image_uploader" style="width:130px" value="ADD/VIEW IMAGE" onClick="file_uploader ( '../../', document.getElementById('txt_mst_id').value,document.getElementById('dtls_id').value, 'knit_dye_non_order_booking', 0 ,1)">
                    </td>
                    <td>&nbsp;</td> 
                    <td>&nbsp;</td> 
                </tr>
