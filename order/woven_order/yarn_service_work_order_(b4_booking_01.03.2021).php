...
function fnc_yarn_service_wo(operation)
{
	...
	else
	{
		...
		$("#dtls_container tr").each(function()
		{
			...
			dtlsDataString+='&txt_job_no_' + j + '=' + txt_job_no + '&txt_job_id_' + j + '=' + txt_job_id + '&txt_lot_' + j + '=' + txt_lot + '&txt_pro_id_' + j + '=' + txt_pro_id + '&txt_item_des_' + j + '=' + txt_item_des + '&cbo_uom_' + j + '=' + cbo_uom + '&txt_wo_qty_' + j + '=' + txt_wo_qty + '&cbo_count_' + j + '=' + cbo_count + '&txt_rate_' + j + '=' + txt_rate + '&txt_amount_' + j + '=' + txt_amount + '&txt_bag_' + j + '=' + txt_bag + '&txt_cone_' + j + '=' + txt_cone + '&txt_min_req_cone_' + j + '=' + txt_min_req_cone + '&txt_remarks_' + j + '=' + txt_remarks + '&dtls_update_id_' + j + '=' + dtls_update_id + '&txt_allocated_qty_' + j + '=' + allocated_qty ;
			j++;
		});
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			if(product_info.length > 1){
				...
				textinput.eq(13).attr('id', 'txt_allocated_qty_0').val(data[4]);
	...
	if(product_info.length > 1){
		...
		for( var i = 1; i < product_info.length; i++ ) {
			...
			$('#dtls_container tr').each(function(i) {
				...
				//alert(555);
				//textinput.eq(12).attr('id', 'dtls_update_id_' + i).val('');
				textinput.eq(13).attr('id', 'txt_allocated_qty_' + i).val('');
		...
		textinput.eq(11).attr('id', 'txt_remarks_0').val('');		
		//textinput.eq(12).attr('id', 'dtls_update_id_0').val('');
		textinput.eq(13).attr('id', 'txt_allocated_qty_0').val('');
			...
			<fieldset style="width:800px; margin-bottom:5px;">
				...
				<table cellspacing="4" cellpadding="8" border="0">
					...
					<tr>
						...
						<td>
							<?
							echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected,"load_drop_down( 'requires/yarn_service_work_order_controller',this.value, 'load_drop_down_supplier', 'supplier_td' );get_php_form_data('94'+'_'+this.value, 'populate_field_level_access_data', 'requires/yarn_service_work_order_controller' );",0);
							?>
						</td>
