...
<script>
		...
		else
		{
            ...
            for (var i=0; i<12; i++)
            {
                if (k<13)
                {
					...
					//month_data+="*month_"+String(month)+String(yy)+"*qty_"+String(month)+String(yy)+"*val_"+String(month)+String(yy)+"*mint_"+String(month)+String(yy) +"*cm_"+String(month)+String(yy)+"*cmval_"+String(month)+String(yy)+"*rmval_"+String(month)+String(yy)+"*actualmargin_"+String(month)+String(yy);

					month_data+="*month_"+String(month)+String(yy)+"*qty_"+String(month)+String(yy)+"*val_"+String(month)+String(yy);
                    if (month==12)
					{ 
						target_month=0; 
						i=0; 
			...
			var data="action=save_update_delete_mst&operation="+operation+get_submitted_data_string('update_id*cbo_company_id*cbo_buyer_name*cbo_agent*cbo_team_leader*cbo_starting_month*cbo_starting_year*txt_total_qty*txt_total_val'+month_data,"../../");
			//alert(data);
			freeze_window(operation);
			 
			http.open("POST","requires/sales_target_fabric_controller.php",true);
	...
	{
		get_php_form_data(comp_id, "select_month_from_variable", "requires/sales_target_fabric_controller");
	}
                    ...
                    <fieldset style="width:850px;">
                        <table  width="800" cellspacing="2" cellpadding="0" border="0">
                            <tr>
                                ...
                                <td width="80" id="buyer_td"><?  echo create_drop_down( "cbo_buyer_name", 172, "select buy.buyer_name, buy.id from lib_buyer buy where status_active =1 and is_deleted=0 order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "fnc_load_sales_target_data();" ); ?> </td>
