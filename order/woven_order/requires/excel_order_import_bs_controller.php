...
{
	echo create_drop_down( "cbo_location_id", 130, "select id,location_name from lib_location where company_id='$data' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-Location-", $selected, "" );
	exit();	 
}

if ($action=="load_drop_down_buyer")
{
	echo create_drop_down( "cbo_buyer_id", 130, "select buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond  and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name", 1, "-Buyer-", $selected, "load_drop_down( 'requires/excel_order_import_set_controller', this.value, 'load_drop_down_season', 'season_td'); " );   
	exit();	 
} 
...
{
	echo create_drop_down( "cbo_dealing_merchant", 100, "select id,team_member_name from lib_mkt_team_member_info where team_id='$data' and status_active =1 and is_deleted=0 order by team_member_name","id,team_member_name", 1, "-Team Member-", $selected, "" );
	exit();
}

if ($action=="load_drop_down_factory_merchant")
{
	echo create_drop_down( "cbo_factory_merchant", 100, "select id,team_member_name from lib_mkt_team_member_info where team_id='$data' and status_active =1 and is_deleted=0 order by team_member_name","id,team_member_name", 1, "-Fac Merchent-", $selected, "" );
	exit();
}
...
if($action=="populate_job_data_form")
{
	...
	if($data!="")
	{
		...
		echo "load_drop_down( 'requires/excel_order_import_set_controller', ".$sql_job[0][csf('company_name')].", 'load_drop_down_location', 'location_td');\n";
		echo "load_drop_down( 'requires/excel_order_import_set_controller', ".$sql_job[0][csf('company_name')].", 'load_drop_down_buyer', 'buyer_td');\n";
		echo "load_drop_down( 'requires/excel_order_import_set_controller', ".$sql_job[0][csf('buyer_name')].", 'load_drop_down_season', 'season_td');\n";
		
		echo "load_drop_down( 'requires/excel_order_import_set_controller', ".$sql_job[0][csf('team_leader')].", 'load_drop_down_dealing_merchant', 'div_marchant');\n";
		echo "load_drop_down( 'requires/excel_order_import_set_controller', ".$sql_job[0][csf('team_leader')].", 'load_drop_down_factory_merchant', 'div_marchant_factory');\n";
...
if ($action=="save_update_delete")
{
	...
	{	
		...
		//table lock here 	 
		if( check_table_status( $_SESSION['menu_id'], 1 )==0 ) { echo "15**0"; disconnect($con); check_table_status( $_SESSION['menu_id'],0); die;}
		...
		{
			$country_arr[strtoupper(trim($row[csf('country_name')]))]=$row[csf('id')];
		}
		unset($countrysql);
		...
		//echo "10"
		$set_breck_down=str_replace("'","",$set_breck_down);
		$exset_breck_down=explode("__",$set_breck_down); 
		if(count($exset_breck_down)>0)
		{
			$k=1; $itemSetbreak_arr=array(); $styleSetbreak_arr=array(); $excelStyleItemRationArr=array(); $q=0;
			foreach($exset_breck_down as $excelItem_data)
			{
				//echo $excelItem_data.'<br>';
				$exitemVal=explode("_",$excelItem_data);
				$excelstyle=$excelitem=$cboitem=$setitemratio=$smv=$smvset=$cutsmv=$cutsmvset=$finsmv=$finsmvset=$quotid=0;
				
				$excelstyle=$exitemVal[0];
				$excelitem=$exitemVal[1];
				$cboitem=$exitemVal[2];
				$setitemratio=$exitemVal[3];
				$smv=$exitemVal[4];
				$smvset=$exitemVal[5];
				$cutsmv=$exitemVal[6];
				$cutsmvset=$exitemVal[7];
				$finsmv=$exitemVal[8];
				$finsmvset=$exitemVal[9];
				$quotid=$exitemVal[10];
				
				$styleSetbreak_arr[$excelstyle][$excelitem]=$cboitem.'_'.$setitemratio.'_'.$smv.'_'.$smvset.'_0_0_'.$cutsmv.'_'.$cutsmvset.'_'.$finsmv.'_'.$finsmvset.'_0_0_0_0_0_0_0_0_0_'.$quotid;
				
				$itemSetbreak_arr[$excelstyle][$excelitem]=$cboitem;
				$excelStyleItemRationArr[$excelstyle][$excelitem]=$setitemratio;
				
				$tot_smv_qty[$excelstyle]+=$setitemratio;
				$smv_set[$excelstyle]+=$smvset;
			}
		}
		/*echo "10**<pre>";
		print_r($tot_smv_qty); check_table_status( $_SESSION['menu_id'],0); die;*/
		...
		$field_set="id, job_id, job_no, gmts_item_id, set_item_ratio, smv_pcs, smv_set, smv_pcs_precost, smv_set_precost, complexity, embelishment, cutsmv_pcs, cutsmv_set, finsmv_pcs, finsmv_set, printseq, embro, embroseq, wash, washseq, spworks, spworksseq, gmtsdying, gmtsdyingseq, ws_id";
		...
		{
			$setbreak_down_data=''; $gmtsItem="";
			//echo "10**".implode("__",$styleSetbreak_arr[$style_ref]); check_table_status( $_SESSION['menu_id'],0); die;
			$setbreak_down_data=implode("__",$styleSetbreak_arr[$style_ref]); //check_table_status( $_SESSION['menu_id'],0); die;
			$gmtsItem=implode(",",$itemSetbreak_arr[$style_ref]);
			$st=1; $pn=1; $count=1;
			foreach($order_data as $order_no=>$item_data)
			{
				$p=1;  
				foreach($item_data as $excelItem=>$color_size_data)
				{
					$ctpn=1;
					foreach($color_size_data as $color_val=>$size_data)
					{
						...
						foreach($size_data as $size_val=>$extra_data)
						{
							...
							{
								$poRemarks=''; $recDate=$shipDate="";
								$ex_data=explode('__',$ex_val);
								$style_des=$ex_data[0];
								$recDate=$ex_data[1];
								$shipDate=$ex_data[2];
								
								$poRemarks=''; $sizeQtyRatio=$sizeItemQtyRatio=$countryRate=$countryQty=$countryAmt=$cboDeliveryCountry=$noOfItem=0; 
								$sizeQtyRatio=$tot_smv_qty[$style_ref];//$excelStyleItemRationArr[$style_ref][$excelItem];/
								$sizeItemQtyRatio=$excelStyleItemRationArr[$style_ref][$excelItem];
								
								$noOfItem=count($excelStyleItemRationArr[$style_ref]);
								//$countryRate=number_format((($ex_data[3]/$tot_smv_qty[$style_ref])*$sizeQtyRatio),2,'.','');
								//$countryRate=number_format(($ex_data[3]/$sizeQtyRatio),4,'.','');
								$countryRate=($ex_data[3]/$sizeQtyRatio);
								//$poRemarks='';
								//echo $countryRate.'='.$sizeQtyRatio.'='.$ex_data[3].'='.$sizeQtyRatio.'='.$tot_smv_qty[$style_ref].'<br>';
								//1.35=3=1.80=3=4
								$countryQty=number_format(($sizeqty['poqty']*$sizeItemQtyRatio),0,'.','');
								$cboCodename=$ex_data[7];
								//$countryAmt=$countryQty*number_format($countryRate,6,'.','');
								if($noOfItem==1) $countryAmt=($sizeqty['poqty']*1)*($ex_data[3]*1);
								else $countryAmt=($sizeqty['poqty']*1)*($countryRate*1);
								
								$cboDeliveryCountry=$country_arr[trim($cboCodename)];//"cboDeliveryCountry_".$i.'_'.$j.'_'.$k;
								...
								if(str_replace("'","",$txt_job_no)=="")
								{
									...
									{ 
										...
										$field_job="id, garments_nature, job_no, job_no_prefix, job_no_prefix_num, company_name, buyer_name, style_owner, location_name, style_ref_no, style_description, product_dept, currency_id, season_buyer_wise, order_repeat_no, product_category, team_leader, dealing_marchant, factory_marchant, order_uom, gmts_item_id, set_smv, packing, set_break_down, total_set_qnty, is_excel, is_deleted, status_active, inserted_by, insert_date";
										$data_job_arr[$id]="(".$id.",2,'".$new_job_no_arr[$style_ref][0]."','".$new_job_no_arr[$style_ref][1]."','".$new_job_no_arr[$style_ref][2]."',".$cbo_company_id.",".$cbo_buyer_id.",".$cbo_style_owner_id.",".$cbo_location_id.",'".$style_ref."','".$style_des."',".$cbo_product_department.",".$cbo_currercy_id.",".$cbo_season_id.",'".$repeat_no."',".$cbo_prod_catgory.",".$cbo_team_leader.",".$cbo_dealing_merchant.",".$cbo_factory_merchant.",".$cbo_order_uom.",'".$gmtsItem."','".$smv_set[$style_ref]."',".$cbo_packing.",'".$setbreak_down_data."','".$tot_smv_qty[$style_ref]."',1,0,1,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
										
										$job_id=$id;
										
										$set_breck_down_array=explode('__',str_replace("'",'',$setbreak_down_data));
								...
								if($p==1) 
								{
									...
									if($prev_po_arr[$order_no]!="")
									{
										...
										echo "11**".$msg_dup."**".$order_no;disconnect($con); check_table_status( $_SESSION['menu_id'],0); die;
									}
									...
									$job_style_order_arr[$new_job_no_arr[$style_ref][0]][$idPo]=$setbreak_down_data;
									$p++; 
								} 
								
								if(str_replace("'","",$color_val)!="")
								{ 
									if (!in_array(str_replace("'","",$color_val),$new_array_color,TRUE))
									{
									...
									else $color_id =  array_search(str_replace("'","",$color_val), $new_array_color,TRUE); 
								}
								else $color_id=0;
								//
								...
								//echo $countryShipDate.'='.$shipDate.'<br>';
								//if($add_colSiz!=0) //$data_colSiz.=",";
								$break_down[$idColSiz]="(".$idColSiz.",'".$job_id."',".$idPo.",'".$new_job_no_arr[$style_ref][0]."',0,0,0,'".$itemSetbreak_arr[$style_ref][$excelItem]."','".$cboDeliveryCountry."','".$codeId."','".$cboCountryId."','".$countryCode."','".$countryShipDate."','".$color_id."','".$size_id."','".$countryQty."','".$countryRate."',0,'".$article_number."','".$countryAmt."','".$countryQty."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		if($ready_to_save==0) { echo "5**Please check Country and Upload the file again."; check_table_status( $_SESSION['menu_id'],0);disconnect($con); die; }
		//check_table_status( $_SESSION['menu_id'],0);
		//die;
		//print_r($data_po_arr); check_table_status( $_SESSION['menu_id'],0);die;
		//echo "10**INSERT INTO wo_po_details_mas_set_details (".$field_set.") VALUES ".implode(",",$data_set_arr); die;
		...
		foreach( $data_job as $jobRows)
		{
			...
			else if($rID==0) 
			{
				$flag=0;
				oci_rollback($con); 
				echo "10**"; disconnect($con);check_table_status( $_SESSION['menu_id'],0); die;
			}
		}//die;
		//if($rID==1) $flag=1; else $flag=0;
		...
		foreach( $data_set as $setRows)
		{
			...
			else if($rID1==0) 
			{
				$flag=0;
				oci_rollback($con); 
				echo "10**".$roll_back_msg;disconnect($con); check_table_status( $_SESSION['menu_id'],0); die;
			}
		}
		...
		foreach ( $job_style_order_arr as $job_no=>$job_data )
		{
			...
			foreach($job_data as $poId=>$setData)
			{
				...
				job_order_qty_update("'".$job_no."'",$poId,$exSetData,1,1);
...
function job_order_qty_update($job_no,$po_id,$set_data,$order_status)
{
	...
	$poavgprice=number_format($job_amt/$job_qty,8);
	...
	$poavgprice_po=number_format($po_data_arr[str_replace("'","",$po_id)]['amt']/$po_qty,8);
