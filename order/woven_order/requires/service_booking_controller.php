...
if($action=="create_po_search_list_view")
{
	...
	if ($data[2]==0)
	{
		 $sql= "select a.job_no_prefix_num,$insert_year, a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity,b.id, b.po_number,b.po_quantity,b.shipment_date,b.grouping,b.file_no from wo_po_details_master a, wo_po_break_down b, wo_pre_cost_mst c where a.job_no=b.job_no_mst and a.job_no=c.job_no and a.status_active=1 and b.status_active=1  and b.shiping_status not in(3) $approval_cond $shipment_date $company $buyer $job_cond $order_cond $ref_cond $style_ref_cond $file_no_cond order by a.insert_date DESC";
		//echo $sql;
	...
	{
		$sql= "select a.job_no_prefix_num, a.job_no,a.company_name,a.buyer_name,a.style_ref_no from wo_po_details_master a where a.status_active=1  and a.is_deleted=0 $company $buyer order by a.insert_date DESC";
...
if ($action=="load_drop_down_fabric_description")
{
	...
	if($data[1] =="")// and cons_process not in (1,30,31,35)   ISD-22-17815
	{
      ...
      //  echo "select id,fabric_description,cons_process from wo_pre_cost_fab_conv_cost_dtls  where job_no='$data[0]' and status_active=1 and is_deleted=0 and cons_process not in (1,30,31,35)";
	}
	else
	{
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		{
			$new_booking_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'FSB', date("Y",time()), 5, "select id, booking_no_prefix, booking_no_prefix_num from wo_booking_mst where company_id=$cbo_company_name and booking_type=3 and entry_form=176 and to_char(insert_date,'YYYY')=".date('Y',time())." order by id desc ", "booking_no_prefix", "booking_no_prefix_num" ));
			...
			$field_array="id, booking_type, booking_month, booking_year, booking_no_prefix, booking_no_prefix_num, booking_no, company_id, buyer_id, item_category, entry_form, supplier_id, currency_id, exchange_rate, booking_date, delivery_date, pay_mode, source, quality_level, tenor, attention, ready_to_approved, inserted_by, insert_date";
			 $data_array ="(".$id.",3,".$cbo_booking_month.",".$cbo_booking_year.",'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',".$cbo_company_name.",".$cbo_buyer_name.",12,176,".$hidden_supplier_id.",".$cbo_currency.",".$txt_exchange_rate.",".$txt_booking_date.",".$txt_delivery_date.",".$cbo_pay_mode.",".$cbo_source.",".$vari_fab_source_id.",".$txt_tenor.",".$txt_attention.",".$cbo_ready_to_approved.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
	...
	else if ($operation==1)   // Update Here
	{
		...
		$pi_number=return_field_value( "pi_number", "com_pi_master_details a,com_pi_item_details b,wo_booking_mst c"," a.id=b.pi_id and c.id=b.WORK_ORDER_ID and b.work_order_no=$txt_booking_no  and c.entry_form=162 and a.status_active=1 and  a.is_deleted=0  and c.status_active=1 and  c.is_deleted=0  and b.status_active=1 and  b.is_deleted=0");
		if($pi_number){
...
if ($action=="save_update_delete_dtls")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		//echo "select excut_source from variable_order_tracking where company_name=$cbo_company_name  and variable_list=66  and status_active=1 and is_deleted=0";
		 ...
		 if($fab_source>0) $fab_source=$fab_source;else $fab_source=1;
		...
		if  ( check_table_status( $_SESSION['menu_id'], 1 )==0 ) { echo "15**0";disconnect($con); die;}
		 ...
		 for ($i=1;$i<=$row_num;$i++)
		 {
			 ...
			 if ($i!=1) $data_array1 .=",";
			 $data_array1 .="(".$id_dtls.",".$booking_mst_id.",".$$fabric_description_id.",176,".$$artworkno.",".$$po_id.",".$$color_size_table_id.",".$$txt_job_no.",'".$response_booking_no."',3,".$color_id.",".$$gmts_color_id.",".$$item_size.",".$$gmts_size_id.",".$$fabric_description_id.",".$$uom.",".$cbo_process.",".$cbo_colorsizesensitive.",".$$txt_woqnty.",".$$txt_rate.",".$$txt_amount.",".$$startdate.",".$$enddate.",".$$findia.",".$$findia.",".$$labdipno.",".$$mcdia.",".$$fingsm.",".$$slength.",".$$yarncount.",".$$lotno.",".$$brand.",".$$lib_composition.",".$$lib_supplier_rateId.",".$$txtremaks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		   $id_dtls=$id_dtls+1;
...
if ($action=="fabric_detls_list_view")
{
	...
	sum(b.wo_qnty) as wo_qnty,b.insert_date from wo_booking_dtls b, wo_booking_mst a
  	where b.booking_no=a.booking_no and a.booking_no='$data[1]'  and a.entry_form=176 and b.process!=35  and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0
...
if($action=="update_detail_booking_list_view")
{
	...
	{*/
		$wo_pre_cost_fab_conv_cost_dtls_id=sql_select("select b.id as po_id,b.po_number,c.job_no,c.id,c.fabric_description,c.cons_process from wo_pre_cost_fab_conv_cost_dtls c,wo_po_break_down b where  c.job_id=b.job_id and b.id in($job_po_id)  and c.status_active=1 and c.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.id, b.po_number,c.job_no,  c.id,c.fabric_description,c.cons_process");

        $sql_fabric_description=sql_select("select id,body_part_id,color_type_id,fabric_description from  wo_pre_cost_fabric_cost_dtls
			where  job_no='".$job_po."' and status_active=1 and is_deleted=0");
            $fabric_description_string="";
            foreach( $sql_fabric_description as $row)
            {
                $fabric_desc_array[$row[csf("id")]]['desc']=$row[csf("fabric_description")];
                $fabric_desc_array[$row[csf("id")]]['color_type_id']=$row[csf("color_type_id")];
                $fabric_desc_array[$row[csf("id")]]['body_part_id']=$row[csf("body_part_id")];
                $fabric_description_string.=$body_part[$row[csf("body_part_id")]].', '.$color_type[$row[csf("color_type_id")]].', '.$row[csf("fabric_description")]." and ";
            }
	...
	foreach( $wo_pre_cost_fab_conv_cost_dtls_id as $row_wo_pre_cost_fab_conv_cost_dtls_id)
	{
		...
		if($row_wo_pre_cost_fab_conv_cost_dtls_id[csf("fabric_description")]!=0)
		{
			...
			// where  id='".$row_wo_pre_cost_fab_conv_cost_dtls_id[csf("fabric_description")]."'");
			// list($fabric_description_row)=$fabric_description;
            $pre_fab_id=$row_wo_pre_cost_fab_conv_cost_dtls_id[csf("fabric_description")];
            $fab_desc=$fabric_desc_array[$pre_fab_id]['desc'];
            $color_type_id=$fabric_desc_array[$pre_fab_id]['color_type_id'];
            $body_part_id=$fabric_desc_array[$pre_fab_id]['body_part_id'];
            //echo "A";

			$fabric_description_array[$row_wo_pre_cost_fab_conv_cost_dtls_id[csf("id")]]=$body_part[$body_part_id].', '.$color_type[$color_type_id].', '.$fab_desc;
		}
		if($row_wo_pre_cost_fab_conv_cost_dtls_id[csf("fabric_description")]==0)
		{
			//$fabric_description_string="";
...
if ($action=="create_booking_search_list_view")
{
	...
	$sql_po_number=sql_select("select a.booking_no, b.po_break_down_id, d.id, d.job_no_prefix_num, d.job_no, c.grouping, c.po_number,a.pay_mode,a.supplier_id  from  wo_po_details_master d, wo_po_break_down c, wo_booking_mst a, wo_booking_dtls b where d.job_no=c.job_no_mst and d.job_no=b.job_no and b.po_break_down_id=c.id and a.booking_no=b.booking_no and a.booking_type=3 and (a.entry_form  is null or a.entry_form=176) and a.status_active=1 and a.is_deleted=0 $company $buyer $booking_date $booking_cond $job_cond $style_cond $file_no_cond $ref_no_cond ");
	
	//echo "select a.booking_no, b.po_break_down_id, d.id, d.job_no_prefix_num, d.job_no, c.grouping, c.po_number,a.pay_mode,a.supplier_id  from  wo_po_details_master d, wo_po_break_down c, wo_booking_mst a, wo_booking_dtls b where d.job_no=c.job_no_mst and d.job_no=b.job_no and b.po_break_down_id=c.id and a.booking_no=b.booking_no and a.booking_type=3 and a.entry_form=176  and (a.entry_form  is null) and a.status_active=1 and a.is_deleted=0 $company $buyer $booking_date $booking_cond $job_cond $style_cond $file_no_cond $ref_no_cond ";die;
	$poId="";
	...
	if($check_type==0)
	{
		 $sql= "select a.id,a.booking_no_prefix_num, b.job_no as job_no_prefix_num, a.booking_no, a.booking_date, a.company_id, a.buyer_id, a.item_category, a.fabric_source, a.supplier_id, a.pay_mode,a.source from wo_booking_mst a, wo_booking_dtls b,wo_po_break_down c where a.id=b.booking_mst_id and b.po_break_down_id=c.id and a.booking_type=3 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0  and (a.entry_form  is null or a.entry_form=176) $company $buyer $booking_date $booking_cond $job_cond $job_cond_for_in $file_no_cond $ref_no_cond 
	...
	{
		$sql= "select a.id, a.booking_no_prefix_num, a.booking_no, a.booking_date, a.company_id, a.buyer_id, a.job_no as job_no_prefix_num, a.item_category, a.source, a.fabric_source, a.supplier_id, a.pay_mode from wo_booking_mst a where a.booking_type=3 and a.status_active=1 and a.is_deleted=0  and (a.entry_form  is null or a.entry_form=176) and a.booking_no not in( select distinct booking_no from wo_booking_dtls where status_active=1 and is_deleted=0 and booking_no is not null) $company $buyer $booking_date $booking_cond $job_cond $style_cond  order by a.id DESC";
	}
	//echo $sql;
...
if($action=="show_trim_booking_report")
{
	...
	<div style="width:1333px" align="center">
        ...
        if($show_rate==1){$show_colspan="0";}else{$show_colspan="2";}
        //print_r($fabric_description_array);
        //=================================================
       ...
       <tr style="border:1px solid black;">
        ...
        ?>
		 ...
		 <?
            echo signature_table(53, $cbo_company_name, "1313px",$cbo_template_id);
...
?>
		...
		$job_no="";
		$currency_id="";
		$nameArray_job=sql_select( "select distinct b.job_no,a.buyer_id from wo_booking_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and a.booking_no=$txt_booking_no  and a.status_active=1 and b.status_active=1");
		...
		$nameArray_job=sql_select( "select b.id, b.po_number from wo_booking_dtls a, wo_po_break_down b where a.po_break_down_id=b.id and a.booking_no=$txt_booking_no and a.status_active=1 and b.status_active=1 group by b.id, b.po_number");
        ...
        $nameArray=sql_select( "select a.booking_no,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.attention,a.delivery_date,a.source,a.pay_mode  from wo_booking_mst a where  a.booking_no=$txt_booking_no and a.status_active=1");
		...
		$article_number_arr=return_library_array( "select po_break_down_id,article_number from wo_po_color_size_breakdown where status_active=1 and po_break_down_id in(".$po_id.")", "po_break_down_id", "article_number");
       ...
       if(count($nameArray_color)>0)
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            <tr>
                <td style="border:1px solid black"><strong>GMT Color</strong> </td>
                <td style="border:1px solid black"><strong>Item Color</strong> </td>
                <td style="border:1px solid black" align="center"><strong>Wo Qty (<?echo $unit_of_measurement[$fabric_description_row[csf("uom")]];?>)</strong></td>
                <td style="border:1px solid black" align="center"><strong>Fin Dia</strong></td>
                <? if($show_comment==1) {?>
                <td style="border:1px solid black" align="center"><strong>Rate</strong></td>
                <td style="border:1px solid black" align="center"><strong>Amount</strong></td>
...
if($action=="show_trim_booking_report2" || $action=="show_trim_booking_report3")
{
	...
	<div style="width:1333px" align="center">
        ...
        $nameArray=sql_select( "select a.booking_no,a.is_approved,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.attention,a.delivery_date,a.source,a.pay_mode  from wo_booking_mst a where  a.booking_no=$txt_booking_no and a.supplier_id=$hidden_supplier_id and a.id=$booking_mst_id");
        ...
        foreach ($nameArray as $result)
        {
			...
			<table width="100%" style="border:1px solid black">
                ...
                <tr>
                    ...
                    <td width="110" style="font-size:12px"><b>&nbsp;</b> </td>
                    <td width="100" style="font-size:12px">&nbsp;</td>
                    <td width="160" style="color: red; font-size: 25px;"><? if ($result[csf('is_approved')] == 0) echo "Un-Approved"; else if($result[csf('is_approved')] == 1) echo "Approved"; else echo "Partial-Approved"; ?></td>
	...
	//=================================================
        ...
        <table border="0" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" >
            <tr>
                <td colspan="<?=count($nameArray_color)+10-$show_colspan; ?>" align="">
                <strong>As Per Garments Color</strong>
                </td>
		...
		group_concat(brand) AS brand";
            ...
            ?>
            <tr>
                <td align="right" style="border:1px solid black"  colspan="<? echo count($nameArray_color)+(14-$col_span)-$show_colspan; ?>"><strong><? if($show_rate==1) echo '';else echo 'Total'; ?></strong></td>
                <td  style="border:1px solid black;  text-align:right"><?
				if($show_rate==1) echo '';
				else echo number_format($grand_total_as_per_gmts_color,4); $booking_grand_total+=$grand_total_as_per_gmts_color; ?></td>
            </tr>
        ...
        &nbsp;
		  ...
		  <td width="2%"></td>
         <?
	    $data_array=sql_select("select b.approved_by,b.approved_no, b.approved_date, c.user_full_name from  wo_booking_mst a , approval_history b, user_passwd c where a.id=b.mst_id and b.approved_by=c.id and a.booking_no=$txt_booking_no and a.status_active =1 and a.is_deleted=0");
    ...
    <td width="49%" valign="top">
	 ...
	 <br></br>
       ...
       // $nameArray_color=sql_select( "select distinct fabric_color_id from wo_booking_dtls   where  booking_no=$txt_booking_no and sensitivity=1");
        ...
        <table border="0" align="left" cellpadding="0" width="100%" cellspacing="0" >
            <tr>
                <td colspan="<? echo count($nameArray_size)+11-$show_colspan; ?>" align="">
                <strong>As Per Garments Size </strong>
                </td>
	...
	listagg(cast(brand as varchar2(4000)),',') within group (order by brand) AS brand";
		...
		group_concat(brand) AS brand";
            ...
            ?>
            <tr>
                <td align="right" style="border:1px solid black"  colspan="<? echo count($nameArray_size)+(15-$col_span-$show_colspan); ?>"><strong><? if($show_rate==1) echo '';else echo 'Total'; ?></strong></td>
                <td  style="border:1px solid black;  text-align:right"><?
				if($show_rate==1) echo ' ';
				else echo number_format($grand_total_as_per_gmts_color,4); $booking_grand_total+=$grand_total_as_per_gmts_color; ?></td>
            </tr>
	...
	*/
       $only_color_size_qnty=sql_select( "select fabric_color_id, gmts_color_id, slength, dia_width, fin_gsm, rate, description, sum(wo_qnty) as cons from wo_booking_dtls where booking_no=$txt_booking_no and sensitivity=3 and status_active=1 and is_deleted =0 and wo_qnty>0 group by fabric_color_id, gmts_color_id, slength, dia_width, fin_gsm, rate, description");
        ...
        <table border="0" align="left" cellpadding="0" width="100%" cellspacing="0" >
            <tr>
                <td colspan="<? echo 10+count($nameArray_color)-$show_colspan; ?>" align="">
                <strong>Contrast Color</strong>
                </td>
	   ...
	   //$nameArray_color=sql_select( "select distinct fabric_color_id as color_number_id from wo_booking_dtls   where booking_no=$txt_booking_no and sensitivity=3");
        ...
        <table border="0" align="left" cellpadding="0" width="100%" cellspacing="0" >
            <tr>
                <td colspan="<? echo count($nameArray_size)+12-$show_colspan; ?>" align="">
                <strong>Color & size sensitive </strong>
                </td>
        ...
        <table border="0" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" >
            <tr>
                <td align="" colspan="<?=11-$show_colspan;?>" >
                <strong>No Sensitivity</strong>
                </td>
		...
		group_concat(brand) AS brand";
            ...
            ?>
            <tr>
                <td align="right" style="border:1px solid black"  colspan="<? echo 15-$col_span-$show_colspan?>"><strong><? if($show_rate==1) echo '';else echo 'Total'; ?></strong></td>
                <td  style="border:1px solid black;  text-align:right"><?
				if($show_rate==1) echo '';else echo number_format($grand_total_as_per_gmts_color,4); $booking_grand_total+=$grand_total_as_per_gmts_color; ?></td>
            </tr>
        </table>
...
{
	 $sql= "select is_approved,booking_no,booking_date,company_id,buyer_id,job_no,po_break_down_id,item_category,fabric_source,currency_id,exchange_rate,pay_mode,booking_month,supplier_id,attention,delivery_date,source,booking_year,quality_level,tenor,ready_to_approved,id from wo_booking_mst  where booking_no='$data'";
	 ...
	 foreach ($data_array as $row)
	 {
        ...
        echo "document.getElementById('approval_status_id').value      = '".$row[csf("is_approved")]."';\n";  
        if($row[csf("is_approved")]==1){$app_sta ="This Booking Is Approved";}
        else if($row[csf("is_approved")]==2){$app_sta ="Deny Approve";}
        else if($row[csf("is_approved")]==3){$app_sta ="This Booking Is Partially Approved";}
       
        echo "document.getElementById('approval_status').innerHTML = '<b style=\"font-size: 20px;\">".$app_sta."</b>';\n";
		$po_no="";
	...
	//	echo "document.getElementById('txt_order_no').value = '".substr($po_no, 0, -1)."';\n";
        ...
        echo "document.getElementById('txt_app_status').value = '".$row[csf("is_approved")]."';\n";
        if($row[csf("is_approved")]==1)
		{
			echo "document.getElementById('display_app_status').innerHTML = 'This booking is approved.';\n";
		}
		else if($row[csf("is_approved")]==2)
		{
			echo "document.getElementById('display_app_status').innerHTML = 'This booking is deny.';\n";
		}
		else if($row[csf("is_approved")]==3)
		{
			echo "document.getElementById('display_app_status').innerHTML = 'This booking is partial approved.';\n";
		}
		else
		{
			echo "document.getElementById('display_app_status').innerHTML = '';\n";
		}
