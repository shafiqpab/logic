...
if ($action=="open_actual_order_popup")
{
  ...
  ?>
	...
	within group (order by b.id) dtls_id  	
		...
		foreach($data_result as $row){
			...
			$po_qty_arr[$row[csf('actual_id')]][$row[csf('country_id')]]['dtls_id']=$row[csf('dtls_id')];
					...
					$actual_ids.=$row[csf('actual_id')].",";
		}

		$actual_ids=rtrim($actual_ids,",");
		$fin_fab_data=sql_select("SELECT a.actual_po_dtls_id as id, sum(a.prod_qty) as qty,a.actual_po_id,b.country_id
		...
		where a.mst_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
		 and b.production_type=8  and a.actual_po_id in ($actual_ids)  group by a.actual_po_dtls_id,a.actual_po_id,b.country_id ");


		foreach($fin_fab_data as $row){

			$po_wise_data[$row[csf('actual_po_id')]]['fin_qnty']+=$row[csf('qty')];
			$po_country_wise_data[$row[csf('actual_po_id')]][$row[csf('country_id')]]['fin_qnty']+=$row[csf('qty')];
			// $po_wise_data[$row[csf('actual_po_id')]][$row[csf('country_id')]]['dtls_id']=$row[csf('actual_po_id')];
			$po_wise_data[$row[csf('actual_po_id')]]['dtls_id']=$row[csf('actual_po_id')];
		...
		foreach($cuml_data as $row){
			...
			$po_qty=$po_wise_data[$row[csf('actual_po_id')]]['fin_qnty'];
			// echo $row[csf('cuml_insp')]."==".$po_qty."<br>";
			if($row[csf('cuml_insp')]==$po_qty){
				...
				// 	$dtls_id .=",".$po_qty_arr[$row[csf('actual_po_id')]][$row[csf('country_id')]]['dtls_id'];
				// }
				$mst_id .=$po_wise_data[$row[csf('actual_po_id')]]['dtls_id'].",";
			}
	...
	if($mst_id !=""){
		$dtls_id_cond=" and a.id not in ($mst_id)";
...
if ($action=="actual_po_list_view")
{
	...
	from pro_garments_prod_actual_po_details a, pro_garments_production_mst b 
	where a.mst_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
	 and b.production_type=8 and  a.actual_po_dtls_id in ($po_id)   group by a.actual_po_dtls_id,a.actual_po_id,b.country_id ");
	...
	?>
			...
			<table cellpadding="0" cellspacing="1" width="100%" class="rpt_table" rules="all" id="table_list_view">
						...
						foreach($sql_data as $row){
							...
							//$fin_qnty=$po_country_wise_data[$row[csf('actual_id')]][$row[csf('country_id')]]['fin_qnty'];
							$fin_qnty=$po_wise_data[$row[csf('actual_id')]]['fin_qnty'];
							// if($cuml_qnty > $row[csf('po_qty')])
							if($cuml_qnty > $fin_qnty){
...
if ($action=="actual_po_update_list_view")
{
		...
		$fin_fab_data=sql_select("SELECT a.actual_po_dtls_id as id, sum(a.prod_qty) as qty,a.actual_po_id
		from pro_garments_prod_actual_po_details a, pro_garments_production_mst b 
		where a.mst_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
		 and b.production_type=8 and  a.actual_po_id in ($actual_po_id)   group by a.actual_po_dtls_id,a.actual_po_id ");
