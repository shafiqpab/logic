...
$subprocess_for_aop = "35,37,452,272,235,393,223,273,275,274,222,202,270,213,423,269,447,448,446,449,450,236,237,214,217,368,226";
...
if ($action == "fabric_search_list_view") {
	...
	if ($cbo_currency != "") $currency_cond = " and a.currency_id='$cbo_currency'";
	else {
		echo "Please Select Currency First.";
		die;
	}
	if ($db_type == 0) {
		if ($date_from != "" &&  $date_to != "") $shipment_date = "and b.pub_shipment_date between '" . change_date_format($date_from, "yyyy-mm-dd", "-") . "' and '" . change_date_format($date_to, "yyyy-mm-dd", "-") . "'";
		else $shipment_date = "";
	} else if ($db_type == 2) {
		if ($date_from != "" &&  $date_to != "") $shipment_date = "and b.pub_shipment_date between '" . change_date_format($date_from, "yyyy-mm-dd", "-", 1) . "' and '" . change_date_format($date_to, "yyyy-mm-dd", "-", 1) . "'";
		else $shipment_date = "";
	}
	...
	if (str_replace("'", "", $job) == "" && str_replace("'", "", $order_search) == "" && str_replace("'", "", $file_no) == "" && str_replace("'", "", $internal_ref) == "" && str_replace("'", "", $style) == "" && $date_from == "" &&  $date_to == "") {
		echo "Please Insert Job, Internal Ref, File No, Style Ref, Order No or Date Range First.";
		die;
	}
...
if ($action == "generate_aop_booking") {
	...
	$sql_vari_aop = sql_select("select b.variable_list,b.excut_source,b.fabric_source_aop_id from variable_order_tracking b, wo_booking_mst a  where b.company_name=a.company_id and a.booking_type=3 and a.process=35 and a.booking_no='$txt_booking_no' and b.variable_list in(59,90) group by b.variable_list,b.excut_source,b.fabric_source_aop_id");
	//echo "select b.excut_source,b.fabric_source_aop_id from variable_order_tracking b, wo_booking_mst a  where b.company_name=a.company_id and a.booking_type=3 and a.process=35 and a.booking_no='$txt_booking_no' and b.variable_list in(59,90) group by b.excut_source,b.fabric_source_aop_id";

	$fabric_source_aop_id = '';
	$fab_req_source_for_aop_from = 1;
	foreach ($sql_vari_aop as $row) {
		$variable_listId = $row[csf("variable_list")];
		if ($variable_listId == 90) {
			$fab_req_source_for_aop_from = $row[csf("excut_source")];
		} else {
			$fabric_source_aop_id = $row[csf("fabric_source_aop_id")];
		}
	}
	if ($fabric_source_aop_id == 0 || $fabric_source_aop_id == '') $fabric_source_aop_id = 0;
	else $fabric_source_aop_id = $fabric_source_aop_id;
	//echo $excut_source.'='.$fabric_source_aop_id.'DDD';
	$fabric_description_array = array();
	$str_job = "";
	...
	$sql = "select a.id, a.pre_cost_fabric_cost_dtls_id, a.artwork_no, a.po_break_down_id, a.color_size_table_id, a.fabric_color_id, a.item_size, a.process, sensitivity, a.job_no, booking_no, a.booking_type, a.description, a.uom, a.delivery_date, a.delivery_end_date, a.sensitivity, a.wo_qnty, a.rate, a.amount, a.gmts_size, a.gmts_color_id, a.dia_width,a.aop_mc_type,a.aop_type from wo_booking_dtls a  where  a.booking_type=3 and a.process in($subprocess_for_aop) and a.booking_no='$txt_booking_no' and a.id in ($dtls_id)  and a.pre_cost_fabric_cost_dtls_id in($data[4]) and  a.status_active=1 and a.is_deleted=0 ";
	$dataArray = sql_select($sql);
	$z = 1;
	$i = 1;
	foreach ($dataArray as $row) {
	...
	if ($fab_req_source_for_aop_from == 1) //Budget Fabric
	{
   ...
   a.id=c.job_id and a.id=d.job_id and a.id=e.job_id and a.id=f.job_id and a.id=g.job_id and b.id=c.po_break_down_id and b.id=g.po_break_down_id and c.color_number_id=g.color_number_id and c.size_number_id=g.gmts_sizes and c.item_number_id=f.item_number_id and f.id=g.pre_cost_fabric_cost_dtls_id and e.fabric_description=f.id and e.id in($conversion_cost_id) and b.id in($txt_order_no_id) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0  and g.requirment >0
	...
	foreach ($dataArray as $row) {
		...
		$amt                         = $row[csf("wo_req_qnty")]*$row[csf("charge_unit")];

		$pre_cost_fabric_cost_dtls_id = $row[csf("fid")];
		$cbo_fabric_natu             = $row[csf("fab_nature_id")];
		$budget_rate = 0;
		if($color_breakdown=='') $color_breakdown=0;
		//echo $fabric_source_aop_id.'DDDD';
		...
		//echo $fab_req_source_for_aop_from.'=='.$cbo_fabric_natu.'=='.$fabric_source_aop_id.'<br>';
		/* echo '<pre>';
		print_r($job_wise_currency_exrate_arr); die; */
		if ($fab_req_source_for_aop_from == 1) //Budget Fabric
		{
			...
			if ($job_wise_currency_exrate_arr[$job_no]['currency'] != $currency_id) {
				...
				//echo $job_wise_currency_exrate_arr[$job_no]['currency'].'--'.$currency_id.'--'.$charge_unit.'='.$job_wise_currency_exrate_arr[$job_no]['exrate'].'--'.$budget_rate.'<br>';
		...
		else //************==Budget Conversation==********
		{
			$req_qty = $req_qty_arr[$pre_cost_conversion_cost_id][$po_break_down_id][$color_number_id][$dia_width][$uom];
			$req_amt = $req_amount_arr[$pre_cost_conversion_cost_id][$po_break_down_id][$color_number_id][$dia_width][$uom];
			...
			if ($job_wise_currency_exrate_arr[$job_no]['currency'] != $currency_id) {
				...
				//echo $charge_unit.'=Samchuuuuuuuu='.$currency_id.'=Samchuuuuuuuu='.$job_wise_currency_exrate_arr[$job_no]['exrate'].',';
			} else $budget_rate = $req_avg_rate;
		...
		$cu_wo_amount      = $cu_booking_data_arr[$pre_cost_conversion_cost_id][$po_break_down_id][$color_number_id][$dia_width]['amount'];
			$cu_wo_qnty    = fn_number_format($cu_wo_qnty,4,'.','');
		$cu_wo_amount    = fn_number_format($cu_wo_amount,4,'.','');
		$woqnty      = 0;
		if ($body_part_id == 3) {
		...
		// echo $charge_unit.'**'.$woqnty.'**'.$budget_amt.'**'.$fab_req_source_for_aop_from.'<br>';
		if($woqnty>0 && $cu_wo_qnty>0)
		{
		$blaqnty = $woqnty - $cu_wo_qnty;
		}
		else 	$blaqnty = $woqnty;
		$blaqnty    = fn_number_format($blaqnty,4,'.','');

		if($amount>0 && $cu_wo_amount>0)
		{
			$blaamount = $amount - $cu_wo_amount;
		}
		else $blaamount =$amount;
		$rate    = $rate;
		$amount  = $amount;
		$blaamount    = fn_number_format($blaamount,4,'.','');
		$rate    = fn_number_format($rate,4,'.','');
		$amount    = fn_number_format($amount,4,'.','');
		...
		$po_color_level_data_arr[$pre_cost_conversion_cost_id][$color_number_id][$dia_width]['cons_process'][$po_break_down_id]       = $cons_process;
			...
			<tbody>
						...
						foreach ($pre_cost_cost_conversion_cost_val as $color_id => $color_val) {
							foreach ($color_val as $dia_width_id => $dia_width_val) {
								foreach ($dia_width_val['po_id'] as $po_id) {
									...
									$cons_process               = $dia_width_val['cons_process'][$po_id];
									...
									$req_qnty                    = def_number_format($dia_width_val['req_qnty'][$po_id], 5, "");
									$blaqnty                     = def_number_format($dia_width_val['blaqnty'][$po_id], 5, "");
									$rate                        = def_number_format($dia_width_val['rate'][$po_id], 5, "");
									$amount                      = def_number_format($dia_width_val['amount'][$po_id], 5, "");
									$budget_amt                  = def_number_format($dia_width_val['budget_amt'][$po_id], 5, "");
									$wo_cu_wo_qnty               = def_number_format($dia_width_val['cu_wo_qnty'][$po_id], 5, "");
				...
				?>
											<tr align="center">
												...
												<td style="word-break:break-all"><? echo $body_part[$body_part_id]; ?>
													...
													<input type="hidden" name="subprocess_id_<? echo $i; ?>" id="subprocess_id_<? echo $i; ?>" value="<? echo $cons_process; ?>" style="width:60px;" class="text_boxes" disabled="disabled">
												</td>
												<td style="word-break:break-all"><? echo $color_type[$color_type_id]; ?></td>
												<td style="word-break:break-all"><? echo $construction; ?></td>
												<td style="word-break:break-all"><? echo $composition; ?></td>
												<td><? echo $gsm_weight; ?></td>
												<td style="word-break:break-all"><? echo $dia_width; ?>
													<input type="hidden" name="dia_<? echo $i; ?>" id="dia_<? echo $i; ?>" value="<? echo $dia_width; ?>" class="text_boxes" style="width:50px;" />
												</td>
												<td style="word-break:break-all"><? echo $color_library[$color_number_id] ?>
												...
												<td><input type="text" name="findia_<? echo $i; ?>" id="findia_<? echo $i; ?>" value="<? echo $dia_width; ?>" class="text_boxes" style="width:50px;" /></td>
												<td>
												...
												<td><input type="text" name="artworkno_<? echo $i; ?>" id="artworkno_<? echo $i; ?>" value="<? //echo $artwork_no;?>" style="width:50px;" class="text_boxes"></td>
												...
												<td><input type="text" name="startdate_<? echo $i; ?>" id="startdate_<? echo $i; ?>" value="<? //echo $delivery_date;?>" style="width:50px;" class="datepicker"></td>
												<td><input type="text" name="enddate_<? echo $i; ?>" id="enddate_<? echo $i; ?>" value="<?  //echo $delivery_end_date;?>" style="width:50px;" class="datepicker"></td>
												<td><input type="text" name="txtreqnty_<? echo $i; ?>" id="txtreqnty_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric" value="<? echo $req_qnty; ?>" readonly disabled /> </td>
												<td>
												...
												<td><? echo create_drop_down("uom_" . $i, 40, $unit_of_measurement, "", 1, "--Select--", $uom, "copy_value(" . $i . ",'uom')", 1, "$uom_item"); ?></td>
												<td>
												<? if($pay_mode==3 || $pay_mode==5){?>
													<input type="text" name="txt_rate_<? echo $i; ?>" id="txt_rate_<? echo $i; ?>" style="width:50px;" class="text_boxes_numeric" onChange="copy_value(<? echo $i; ?>,'txt_rate'); calculate_amount(<? echo $i; ?>)" value="<? echo $rate; ?>" disabled="disabled">
												<?}else{?>
													<input type="text" name="txt_rate_<? echo $i; ?>" id="txt_rate_<? echo $i; ?>" style="width:50px;" class="text_boxes_numeric" onChange="copy_value(<? echo $i; ?>,'txt_rate'); calculate_amount(<? echo $i; ?>)" value="<? echo $rate; ?>">
												<?}?>
												</td>
										...
										<tr align="center">
											...
											<td style="word-break:break-all"><? echo $construction; ?></td>
											<td style="word-break:break-all"><? echo $composition; ?></td>
											<td style="word-break:break-all"><? echo $gsm_weight; ?></td>
											<td style="word-break:break-all"><? echo $dia_width; ?>
												<input type="hidden" name="dia_<? echo $i; ?>" id="dia_<? echo $i; ?>" value="<? echo $dia_width; ?>" class="text_boxes" style="width:50px;" />
											</td>
											...
											<td><input type="text" name="txt_amount_<? echo $i; ?>" id="txt_amount_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric" value="<? //echo $amount; 
					...
					foreach ($po_color_level_data_arr as $precost_conversion_cost_id => $pre_cost_cost_conversion_cost_val) {
						foreach ($pre_cost_cost_conversion_cost_val as $color_id => $color_val) {
							foreach ($color_val as $dia_width_id => $dia_width_val) {
								...
								$cons_process               = implode(",", array_unique($dia_width_val['cons_process']));
								...
								$req_qnty                    = def_number_format(array_sum($dia_width_val['req_qnty']), 5, "");
								$blaqnty                     = def_number_format(array_sum($dia_width_val['blaqnty']), 5, "");
								//  $rate                        = def_number_format(array_sum($dia_width_val['rate']),5,"");
								$amount                      = def_number_format(array_sum($dia_width_val['amount']), 5, "");
								$budget_amt                  = def_number_format(array_sum($dia_width_val['budget_amt']), 5, "");
								$wo_cu_wo_qnty               = def_number_format(array_sum($dia_width_val['cu_wo_qnty']), 5, "");
								$wo_cu_wo_amt              = def_number_format(array_sum($dia_width_val['cu_wo_amt']), 5, "");
								...
								{
									$namt = $blaqnty * $budget_rate;
								} else {
									$namt = $blaqnty * $budget_rate;
									$budget_amt = $req_qnty * $budget_rate;
									$charge_unit = $budget_rate;
								}
								$namt= def_number_format($namt, 5, "");
								// echo $color_type_id.'D='.$item_color_id.'<br>';
								if ($is_short == 2) {
									...
									?>
										<tr align="center">
											...
											<td><input type="text" name="startdate_<? echo $i; ?>" id="startdate_<? echo $i; ?>" value="<? //echo $delivery_date;?>" style="width:50px;" class="datepicker"></td>
											<td><input type="text" name="enddate_<? echo $i; ?>" id="enddate_<? echo $i; ?>" value="<?  //echo $delivery_end_date;?>" style="width:50px;" class="datepicker"></td>
											<td><input type="text" name="txtreqnty_<? echo $i; ?>" id="txtreqnty_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric" value="<? echo number_format($req_qnty,4,'.',''); ?>" readonly disabled /> </td>
											<td>
												<input type="text" name="txt_blanty_<? echo $i; ?>" id="txt_blanty_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric" onChange="copy_value(<? echo $i; ?>,'txt_woqnty');calculate_amount(<? echo $i; ?>)" value="<? echo number_format($blaqnty,4,'.',''); ?>" />
											...
											<td title="<?=$blaqnty;?>"><input type="text" name="txt_woqnty_<? echo $i; ?>" id="txt_woqnty_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric" onChange="copy_value(<? echo $i; ?>,'txt_woqnty');calculate_amount(<? echo $i; ?>)" value="<? echo number_format($blaqnty,4,'.',''); ?>" /></td>
											<td><? echo create_drop_down("uom_" . $i, 40, $unit_of_measurement, "", 1, "--Select--", $uom, "copy_value(" . $i . ",'uom')", 1, "$uom_item"); ?></td>
											<td>
											<? if($pay_mode==3 || $pay_mode==5){?>		
												<input type="text" name="txt_rate_<? echo $i; ?>" id="txt_rate_<? echo $i; ?>" style="width:50px;" class="text_boxes_numeric" onChange="copy_value(<? echo $i; ?>,'txt_rate'); calculate_amount(<? echo $i; ?>)" value="<? echo $charge_unit; ?>" disabled="disabled">
											<?}else{?>
												<input type="text" name="txt_rate_<? echo $i; ?>" id="txt_rate_<? echo $i; ?>" style="width:50px;" class="text_boxes_numeric" onChange="copy_value(<? echo $i; ?>,'txt_rate'); calculate_amount(<? echo $i; ?>)" value="<? echo $charge_unit; ?>">
											<?}?>
											</td>
...
if ($action == "show_aop_booking1") {
	...
	from wo_booking_dtls a  where  a.booking_type=3 and a.process in($subprocess_for_aop) and a.is_short=2  and a.pre_cost_fabric_cost_dtls_id in($data[4]) and a.status_active=1 and a.is_deleted=0 ";
	$dataArray = sql_select($sql);
	$z = 1;
	$i = 1;
	foreach ($dataArray as $row) {
			...
			<tbody>
				...
				?>
										<tr align="center">
											...
											<td>
												<input type="text" name="txt_blanty_<? echo $i; ?>" id="txt_blanty_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric" onChange="copy_value(<? echo $i; ?>,'txt_woqnty');calculate_amount(<? echo $i; ?>)" value="<? if ($is_shrot == 2) echo $blaqnty;
																																																																				else echo ""; ?>" />
												<input type="hidden" name="txtreqnty_<? echo $i; ?>" id="txtreqnty_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric" value="<? if ($is_shrot == 2) echo $req_qnty;
																																															else echo ""; ?>" />
												<input type="hidden" name="txt_prev_woqnty_<? echo $i; ?>" id="txt_prev_woqnty_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric" value="<? if ($is_shrot == 2) echo $wo_cu_wo_qnty;
																																																		else echo ""; ?>" />
											</td>
...
if ($action == "show_aop_booking") {
	...
	$sql_vari_aop = sql_select("select b.variable_list,b.excut_source,b.fabric_source_aop_id from variable_order_tracking b, wo_booking_mst a  where b.company_name=a.company_id and a.booking_type=3 and a.process=35 and a.booking_no='$txt_booking_no' and b.variable_list in(59,90) group by b.variable_list,b.excut_source,b.fabric_source_aop_id");
	//echo "select b.variable_list,b.excut_source,b.fabric_source_aop_id from variable_order_tracking b, wo_booking_mst a  where b.company_name=a.company_id and a.booking_type=3 and a.process=35 and a.booking_no='$txt_booking_no' and b.variable_list in(59,90) group by b.variable_list,b.excut_source,b.fabric_source_aop_id";
	...
	//echo $fab_req_source_for_aop_from.'TTT';
	if ($fabric_source_aop_id == 0 || $fabric_source_aop_id == '') $fabric_source_aop_id = 0;
	else $fabric_source_aop_id = $fabric_source_aop_id;
	...
	unset($currency_exrate_sql_res);
	$grey_roll_issue_sql = "SELECT b.id,b.knitting_company,b.knitting_source,b.booking_no,b.prod_id,b.body_part_id,b.width_dia_type,b.febric_description_id,
	b.gsm,b.width,b.roll_wgt AS roll_wgt_curr,b.roll_id,b.order_id,b.color_id,a.challan_no,
	c.id  AS update_roll_id,c.qnty AS roll_wgt,c.roll_no,c.booking_without_order,c.qc_pass_qnty_pcs,b.job_no,a.wo_no,d.id as pre_cost_dtls_id
	FROM pro_grey_batch_dtls b, inv_receive_mas_batchroll a, pro_roll_details c,wo_pre_cost_fabric_cost_dtls d
	WHERE  a.id = b.mst_id  AND b.id = c.dtls_id  AND a.id = c.mst_id   AND c.entry_form = 539 and b.job_no=d.job_no
	 and b.febric_description_id=d.lib_yarn_count_deter_id  and b.booking_no='$txt_booking_no' AND a.is_deleted = 0
				   AND a.status_active = 1  AND b.status_active = 1 AND b.is_deleted = 0 AND c.status_active = 1  AND c.is_deleted = 0 ";
	$grey_roll_issue_data = sql_select($grey_roll_issue_sql);
	foreach ($grey_roll_issue_data as $row) {
		$grey_roll_issue_return_arr[$row[csf("pre_cost_dtls_id")]][$row[csf("order_id")]][$row[csf("color_id")]][$row[csf("width")]]['return_qnty'] += $row[csf("roll_wgt")];
	}

	// echo "<pre>";
	// print_r($grey_roll_issue_return_arr);
	$cu_booking_data_arr = array();
	$sql = "select a.id, a.pre_cost_fabric_cost_dtls_id, a.artwork_no, a.po_break_down_id, a.color_size_table_id, a.fabric_color_id, a.item_size, a.process, sensitivity, a.job_no, booking_no, a.booking_type, a.description, a.uom, a.delivery_date, a.delivery_end_date, a.sensitivity, a.wo_qnty, a.rate, a.amount, a.gmts_size, a.gmts_color_id, a.dia_width, a.printing_color_id,a.aop_mc_type,a.aop_type from wo_booking_dtls a  where  a.booking_type=3 and a.process in($subprocess_for_aop) and a.is_short=2  and a.pre_cost_fabric_cost_dtls_id in($data[4]) and a.status_active=1 and a.is_deleted=0 ";
	...
	foreach ($dataArray as $row) {
		...
		$fin_dia = $row[csf("fin_dia")];
		$dia_width = $row[csf("dia_width")];
		// echo $pre_cost_conversion_cost_dtls_id.'=='.$po_break_down_id.'=='.$color_number_id.'=='.$rate.'<br>';
		$booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['id'] = $id;
		$booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['fabric_description_id'] = $pre_cost_conversion_cost_dtls_id;
		$booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['artwork_no'] = $artwork_no;
		$booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['aop_mc_type'] = $aop_mc_type;
		$booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['aop_type'] = $aop_type;
		$booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['process'] = $process;
		$booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['fabric_color_id'] = $fabric_color_id;
		$booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['printing_color_id'] = $printing_color_id;
		$booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['item_size'] = $item_size;
		$booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['description'] = $description;
	...
	$sql = "select a.job_no, b.id as po_break_down_id, b.po_number, min(c.id)as color_size_table_id, c.color_number_id, sum(c.plan_cut_qnty) as plan_cut_qnty, d.costing_per, e.id, e.fabric_description, e.cons_process, e.charge_unit, e.amount, (a.job_quantity*e.avg_req_qnty)*e.charge_unit as amt, e.color_break_down, e.process_loss, f.id as fid, f.body_part_id, f.color_type_id, f.construction, f.composition, f.gsm_weight, f.costing_per, f.uom, f.fab_nature_id, g.dia_width, f.fabric_source,
	...
	group by
	a.job_no, b.id, b.po_number, c.color_number_id, d.costing_per, e.id, e.fabric_description, e.cons_process, e.charge_unit, e.amount, e.color_break_down, e.process_loss, f.id, f.body_part_id, f.color_type_id, f.construction, f.composition, f.gsm_weight, f.costing_per, f.uom, f.fab_nature_id, g.dia_width, f.fabric_source, a.job_quantity,e.avg_req_qnty,e.charge_unit
	order by b.id";
	//echo $sql;
	$dataArray = sql_select($sql);
	foreach ($dataArray as $row) {
		...
		$pre_cost_fabric_cost_dtls_id = $row[csf("fid")];
		$cbo_fabric_natu             = $row[csf("fab_nature_id")];
		$retun_qnty = $grey_roll_issue_return_arr[$row[csf("fid")]][$row[csf("po_break_down_id")]][$row[csf("color_number_id")]][$row[csf("dia_width")]]['return_qnty'];
		//echo $charge_unit.'d';
		...
		$cu_wo_qnty        = $cu_booking_data_arr[$pre_cost_conversion_cost_id][$po_break_down_id][$color_number_id][$dia_width]['wo_qnty'];
		$cu_wo_amount      = $cu_booking_data_arr[$pre_cost_conversion_cost_id][$po_break_down_id][$color_number_id][$dia_width]['amount'];
		$rate_wo      = $cu_booking_data_arr[$pre_cost_conversion_cost_id][$po_break_down_id][$color_number_id][$dia_width]['rate'];
		// echo $rate_wo.'='.$rate.'<br>';
		...
		$po_color_level_data_arr[$pre_cost_conversion_cost_id][$color_number_id][$dia_width]['cu_wo_amt'][$po_break_down_id]          	= $cu_wo_amount;
		$po_color_level_data_arr[$pre_cost_conversion_cost_id][$color_number_id][$dia_width]['blaqnty'][$po_break_down_id]             = $blaqnty;
		$po_color_level_data_arr[$pre_cost_conversion_cost_id][$color_number_id][$dia_width]['retun_qnty'][$po_break_down_id]             = $retun_qnty;
			...
			<tbody>
						...
						foreach ($pre_cost_cost_conversion_cost_val as $color_id => $color_val) {
							foreach ($color_val as $dia_width_id => $dia_width_val) {
								foreach ($dia_width_val['po_id'] as $po_id) {
									...
									$woqnty                      = def_number_format($dia_width_val['woqnty'][$po_id], 5, "");
									$rate                        = def_number_format($dia_width_val['rate'][$po_id], 5, "");
									$amount                      = def_number_format($dia_width_val['amount'][$po_id], 5, "");
									$budget_amt                  = def_number_format($dia_width_val['budget_amt'][$po_id], 5, "");
									$wo_cu_wo_qnty               = def_number_format($dia_width_val['cu_wo_qnty'][$po_id], 5, "");
									$cu_wo_amt               	 = def_number_format($dia_width_val['cu_wo_amt'][$po_id], 5, "");
				...
				?>
					<tr align="center">
						...
						<td><input type="text" name="findia_<? echo $i; ?>" id="findia_<? echo $i; ?>" value="<? echo $fin_dia; ?>" class="text_boxes" style="width:50px;" /></td>
						<td>
							<input type="text" name="printing_color_<? echo $i; ?>" id="printing_color_<? echo $i; ?>" style="width:60px;" class="text_boxes" onChange="copy_value()" value="<? echo $color_library[$printing_color_id] ?>" />
							<input type="hidden" name="printing_color_id_<? echo $i; ?>" id="printing_color_id_<? echo $i; ?>" style="width:60px;" class="text_boxes" value="<? echo $printing_color_id; ?>" disabled="disabled" />
						</td>
						...
						<td><input type="text" name="startdate_<? echo $i; ?>" id="startdate_<? echo $i; ?>" value="<? echo change_date_format($delivery_date, "dd-mm-yyyy", "-"); ?>" style="width:50px;" class="datepicker"></td>
						<td><input type="text" name="enddate_<? echo $i; ?>" id="enddate_<? echo $i; ?>" value="<? echo change_date_format($delivery_end_date, "dd-mm-yyyy", "-"); ?>" style="width:50px;" class="datepicker"></td>
						<td><input type="text" name="txtreqnty_<?= $i; ?>" id="txtreqnty_<?= $i; ?>" style="width:60px;" class="text_boxes_numeric" value="<? if ($is_shrot == 2) echo $req_qnty;
																																							else echo ""; ?>" readonly disabled /></td>
						<td>
							<input type="text" name="txt_blanty_<? echo $i; ?>" id="txt_blanty_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric" onChange="copy_value(<? echo $i; ?>,'txt_woqnty');calculate_amount(<? echo $i; ?>)" value="<? if ($is_shrot == 2) echo $blaqnty;
																																																															else echo ""; ?>" />
						...
						<td><input type="text" name="txt_amount_<? echo $i; ?>" id="txt_amount_<? echo $i; ?>" reqamount="<? echo $blaamount; ?>" curamt="<? echo $amount; ?>" totamt="<? echo $cu_wo_amt; ?>" style="width:60px;" class="text_boxes_numeric" value="<? echo $amount; ?>" disabled="disabled" examt="<? echo $budget_amt; ?>" />
						</td>
						<td>
							<input type="text" name="txt_paln_cut_<? echo $i; ?>" id="txt_paln_cut_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric" value="<? echo  $row[csf("plan_cut_qnty")]; ?>" disabled>
							<input type="hidden" name="updateid_<? echo $i; ?>" id="updateid_<? echo $i; ?>" value="<? echo $booking_id; ?>">
					...
					foreach ($po_color_level_data_arr as $precost_conversion_cost_id => $pre_cost_cost_conversion_cost_val) {
						foreach ($pre_cost_cost_conversion_cost_val as $color_id => $color_val) {
							foreach ($color_val as $dia_width_id => $dia_width_val) {
								...
								$req_qnty                    = def_number_format(array_sum($dia_width_val['req_qnty']), 5, "");
								$blaqnty                     = def_number_format(array_sum($dia_width_val['blaqnty']), 5, "");
								$retun_qnty                  = def_number_format(array_sum($dia_width_val['retun_qnty']), 5, "");
								...
								$woqnty                      = def_number_format(array_sum($dia_width_val['woqnty']), 5, "");
								$rate                        = def_number_format(array_sum($dia_width_val['rate']), 5, "");
								$amount                      = def_number_format(array_sum($dia_width_val['amount']), 5, "");
								$budget_amt                  = def_number_format(array_sum($dia_width_val['budget_amt']), 5, "");
								$wo_cu_wo_qnty               = def_number_format(array_sum($dia_width_val['cu_wo_qnty']), 5, "");
								$cu_wo_amt               	 = def_number_format(array_sum($dia_width_val['cu_wo_amt']), 5, "");
								...
								if ($woqnty > 0) {
									...
									<tr align="center">
										...
										<td><input type="text" name="startdate_<? echo $i; ?>" id="startdate_<? echo $i; ?>" value="<? echo change_date_format($delivery_date, "dd-mm-yyyy", "-"); ?>" style="width:50px;" class="datepicker"></td>
										<td><input type="text" name="enddate_<? echo $i; ?>" id="enddate_<? echo $i; ?>" value="<? echo change_date_format($delivery_end_date, "dd-mm-yyyy", "-"); ?>" style="width:50px;" class="datepicker"></td>
										<td><input type="text" name="txtreqnty_<?= $i; ?>" id="txtreqnty_<?= $i; ?>" style="width:60px;" class="text_boxes_numeric" value="<? echo $req_qnty; ?>" readonly disabled /></td>
										<td>
											<input type="text" name="txt_blanty_<? echo $i; ?>" id="txt_blanty_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric" onChange="copy_value(<? echo $i; ?>,'txt_woqnty');calculate_amount(<? echo $i; ?>)" value="<? if ($is_shrot == 2) echo $blaqnty;
																																																																			else echo ""; ?>" />
											<input type="hidden" name="txt_prev_woqnty_<? echo $i; ?>" id="txt_prev_woqnty_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric" value="<? if ($is_shrot == 2) echo $wo_cu_wo_qnty;
																																																	else echo ""; ?>" />
											<input type="hidden" name="txtreturnqnty_<? echo $i; ?>" id="txtreturnqnty_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric" value="<? echo $retun_qnty; ?>" />
										</td>
										<td title="Return Qnty=<?= $retun_qnty; ?>"><input type="text" name="txt_woqnty_<? echo $i; ?>" id="txt_woqnty_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric" onChange="copy_value(<? echo $i; ?>,'txt_woqnty');calculate_amount(<? echo $i; ?>)" value="<? echo $woqnty; ?>" />
											<!-- <small style="font-size:12px">R.Qnty=<?= $retun_qnty; ?></small> -->
										</td>
										<td><? echo create_drop_down("uom_" . $i, 40, $unit_of_measurement, "", 1, "--Select--", $uom, "copy_value(" . $i . ",'uom')", 1, "$uom_item"); ?></td>
										<td>
										<? if($pay_mode==3 || $pay_mode==5){?>											
		...
		<table class="rpt_table" border="1" width="1100" cellpadding="0" cellspacing="0" rules="all" id="">
			...
			<tbody>
				...
				foreach ($job_level_arr as $key => $precost_id) {
					...
					$wo_qnty = number_format(array_sum($precost_id['wo_qnty']), 2, '.', '');
					$booking_id = implode(",", array_unique($precost_id['booking_id']));
					$amount = number_format(array_sum($precost_id['amount']), 2, '.', '');
					$rate = number_format($amount / $grey_fab_qnty, 2, '.', '');
				?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="cursor:pointer">
						...
						<td align="right"><? echo  number_format($wo_qnty, 1); ?></td>
...
if ($action=="save_update_delete"){
	...
	if ($operation==0){
		...
		$sales_order=0;
		$sqls=sql_select("select a.job_no from fabric_sales_order_mst a,wo_booking_mst c where a.sales_booking_no=$txt_booking_no and c.id=a.booking_id and c.process=35 and a.status_active=1 and  a.is_deleted=0 and c.status_active=1 and  c.is_deleted=0");
		...
		$pi_number=return_field_value( "pi_number", "com_pi_master_details a,com_pi_item_details b,wo_booking_mst c"," a.id=b.pi_id and c.id=b.WORK_ORDER_ID and b.work_order_no=$txt_booking_no  and c.process=35 and a.status_active=1 and  a.is_deleted=0  and c.status_active=1 and  c.is_deleted=0  and b.status_active=1 and  b.is_deleted=0");
		if($pi_number){
		...
		$receive_mrr=0;
		$sqlre=sql_select("select a.recv_number from inv_receive_master a, wo_booking_mst c where a.booking_no=$txt_booking_no and c.id=b.booking_id and c.process=35 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and  c.is_deleted=0");
		...
		$issue_mrr=0;
		$sqlis=sql_select("select a.issue_number from inv_issue_master a, wo_booking_mst c where a.booking_no=$txt_booking_no and c.id=b.booking_id and c.process=35 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and  c.is_deleted=0");
		...
		$recv_number=return_field_value( "a.recv_number as recv_number", "inv_receive_mas_batchroll a,pro_grey_batch_dtls b,wo_booking_mst c"," a.id=b.mst_id  and b.booking_no=$txt_booking_no and a.entry_form in(91,92) and b.booking_no=c.booking_no and c.process=35 and a.status_active=1 and  a.is_deleted=0  and b.status_active=1 and  b.is_deleted=0","recv_number");
		if($recv_number){
...
if ($action=="save_update_delete_dtls_job_level"){
	...
	if ($operation==0)  // Insert Here
	{
		...
		$grey_roll_issue_sql="SELECT sum(c.qnty) AS roll_wgt	FROM pro_grey_batch_dtls b, inv_receive_mas_batchroll a, pro_roll_details c,wo_pre_cost_fabric_cost_dtls d	WHERE  a.id = b.mst_id  AND b.id = c.dtls_id  AND a.id = c.mst_id   AND c.entry_form = 539 and b.job_no=d.job_no and b.febric_description_id=d.lib_yarn_count_deter_id  and b.booking_no=$txt_booking_no AND a.is_deleted = 0
					   AND a.status_active = 1  AND b.status_active = 1 AND b.is_deleted = 0 AND c.status_active = 1  AND c.is_deleted = 0 ";
  $grey_roll_issue_return_data=sql_select($grey_roll_issue_sql);
...
//========Issue to Fin Process Found====================
	...
	else if ($operation==2)   // Delete Here
	{
		...
		$booking_dtls_ids=str_replace("'","",$updateid_1); 
		if($is_approved==1){
			echo "approved**".str_replace("'","",$txt_booking_no);
			disconnect($con);die;
		}
		...
		if($sales_order){
			echo "sal1**".str_replace("'","",$txt_booking_no)."**".$sales_order;
			disconnect($con);die;//updateid_1
		}
		//$pi_number=return_field_value( "pi_number", "com_pi_master_details a,com_pi_item_details b"," a.id=b.pi_id  and b.work_order_no=$txt_booking_no  and a.status_active=1 and  a.is_deleted=0  and b.status_active=1 and  b.is_deleted=0");
			$pi_number=return_field_value( "pi_number", "com_pi_master_details a,com_pi_item_details b,wo_booking_mst c"," a.id=b.pi_id and c.id=b.WORK_ORDER_ID and b.work_order_no=$txt_booking_no and b.work_order_dtls_id in($booking_dtls_ids) and c.process=35 and a.status_active=1 and  a.is_deleted=0  and c.status_active=1 and  c.is_deleted=0  and b.status_active=1 and  b.is_deleted=0");
		if($pi_number){
			echo "pi1**".str_replace("'","",$txt_booking_no)."**".$pi_number;
			disconnect($con);die;
		}
...
if ($action == "create_booking_search_list_view") {
	...
	if ($db_type == 0) {
		$booking_year_cond = " and year(a.booking_date)=" . $data[7] . "";
	} else {
		$booking_year_cond = " and to_char(a.booking_date,'YYYY')=" . $data[7] . "";
	}
	...
	if ($data[6] == 1) {
		...
		if (str_replace("'", "", $data[4]) != "") $job_cond = " and c.job_no_prefix_num='$data[4]'";
		else $job_cond = "";
		if (trim($data[10]) != "") $style_cond = " and c.style_ref_no ='$data[10]'";
		else $style_cond = "";
		if (str_replace("'", "", $data[11]) != "") $order_cond = " and d.po_number = '$data[11]' ";
		else $order_cond = "";
	} else if ($data[6] == 4 || $data[6] == 0) {
		if (str_replace("'", "", $data[5]) != "") $booking_cond = " and a.booking_no_prefix_num like '%$data[5]%'  ";
		else  $booking_cond = "";
		if (str_replace("'", "", $data[4]) != "") $job_cond = " and c.job_no_prefix_num like '%$data[4]%' $year_cond ";
		else  $job_cond = "";
	...
	else $file_no_cond = " and d.file_no='" . trim($file_no) . "' ";
	if ($internal_ref == "") $internal_ref_cond = "";
	else $internal_ref_cond = " and d.grouping='" . trim($internal_ref) . "' ";
...
?>
	<div style="width:1150px" align="left">
			...
			<tr>
				...
				<td width="">
					<table width="100%" cellpadding="0" cellspacing="0">
						...
						<tr>
							<?
							$styleownerArray = sql_select("select MIN(b.id),b.style_owner from wo_booking_dtls a,wo_po_details_master b where a.booking_no=$txt_booking_no and a.job_no=b.job_no and a.status_active=1 and b.status_active=1 group by b.style_owner");
							foreach ($styleownerArray as $result) {
								$style_owner = $result[csf('style_owner')];
							}
							?>
							<td align="center" style="font-size:20px">
								<strong>Style Owner:<? echo $company_library[$style_owner]; ?></strong>
							</td>
						</tr>
						<tr>
							<td align="center" style="font-size:20px">
		...
		foreach ($nameArray_job as $row) {
			...
			$main_data_arr[$row[csf('job_id')]][$row[csf('body_part_id')]][$row[csf('fabric_description')]][$row[csf('dia_width')]][$row[csf('fabric_color_id')]][$row[csf('printing_color_id')]]['wo_qnty'] += $row[csf('wo_qnty')];
			$main_data_arr[$row[csf('job_id')]][$row[csf('body_part_id')]][$row[csf('fabric_description')]][$row[csf('dia_width')]][$row[csf('fabric_color_id')]][$row[csf('printing_color_id')]]['artwork_no'] = $row[csf('artwork_no')];
			$main_data_arr[$row[csf('job_id')]][$row[csf('body_part_id')]][$row[csf('fabric_description')]][$row[csf('dia_width')]][$row[csf('fabric_color_id')]][$row[csf('printing_color_id')]]['printing_color_id'] = $row[csf('printing_color_id')];
			$main_data_arr[$row[csf('job_id')]][$row[csf('body_part_id')]][$row[csf('fabric_description')]][$row[csf('dia_width')]][$row[csf('fabric_color_id')]][$row[csf('printing_color_id')]]['uom'] = $row[csf('uom')];
			$main_data_arr[$row[csf('job_id')]][$row[csf('body_part_id')]][$row[csf('fabric_description')]][$row[csf('dia_width')]][$row[csf('fabric_color_id')]][$row[csf('printing_color_id')]]['rate'] = $row[csf('rate')];
			$main_data_arr[$row[csf('job_id')]][$row[csf('body_part_id')]][$row[csf('fabric_description')]][$row[csf('dia_width')]][$row[csf('fabric_color_id')]][$row[csf('printing_color_id')]]['id'] = $row[csf('id')];

			$main_data_arr[$row[csf('job_id')]][$row[csf('body_part_id')]][$row[csf('fabric_description')]][$row[csf('dia_width')]][$row[csf('fabric_color_id')]][$row[csf('printing_color_id')]]['color_type_id'] = $row[csf('color_type_id')];
			$main_data_arr[$row[csf('job_id')]][$row[csf('body_part_id')]][$row[csf('fabric_description')]][$row[csf('dia_width')]][$row[csf('fabric_color_id')]][$row[csf('printing_color_id')]]['gsm_weight'] = $row[csf('gsm_weight')];
			$main_data_arr[$row[csf('job_id')]][$row[csf('body_part_id')]][$row[csf('fabric_description')]][$row[csf('dia_width')]][$row[csf('fabric_color_id')]][$row[csf('printing_color_id')]]['job_no'] = $row[csf('job_no')];
			$main_data_arr[$row[csf('job_id')]][$row[csf('body_part_id')]][$row[csf('fabric_description')]][$row[csf('dia_width')]][$row[csf('fabric_color_id')]][$row[csf('printing_color_id')]]['style_ref'] = $row[csf('style_ref_no')];
			$main_data_arr[$row[csf('job_id')]][$row[csf('body_part_id')]][$row[csf('fabric_description')]][$row[csf('dia_width')]][$row[csf('fabric_color_id')]][$row[csf('printing_color_id')]]['amount'] += $row[csf('amount')];

			$fabric_color_summary[$row[csf('fabric_description')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('fabric_color_id')]][$row[csf('printing_color_id')]]['color_type_id'] = $row[csf('color_type_id')];
			$fabric_color_summary[$row[csf('fabric_description')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('fabric_color_id')]][$row[csf('printing_color_id')]]['wo_qnty'] += $row[csf('wo_qnty')];
			$fabric_color_summary[$row[csf('fabric_description')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('fabric_color_id')]][$row[csf('printing_color_id')]]['printing_color_id'] = $color_library[$row[csf('printing_color_id')]];
		}
		...
		$nameArray = sql_select("SELECT a.booking_no,a.booking_date,a.pay_mode,a.remarks,a.buyer_id,a.supplier_id,a.currency_id,a.exchange_rate,a.attention,a.delivery_date,a.source, a.delivery_to, a.attention, a.tenor, a.inserted_by from wo_booking_mst a where  a.booking_no=$txt_booking_no");
		...
		?>
						...
						foreach ($body_part_data as $desc_id => $gsm_data) {
							...
							foreach ($gsm_data as $dia_id => $dia_data) {
								...
								foreach ($dia_data as $color_id => $color_data) {
									foreach ($color_data as $print_id => $row) {
				...
				?>
										<tr>
											...
											<!-- <td title="<?//= $fabric_ima_arr[$row['id']] . "/master id=" . $row['id']; ?>"><img src='<? //echo $path . $fabric_ima_arr[$row['id']]; ?>' height='50' width='110' /></td> -->
												...
												<?
												$nameArray_imge = sql_select("SELECT image_location,real_file_name FROM common_photo_library where master_tble_id=" . $txt_booking_no . " and file_type=1");
												//$img_counter = 0;
												foreach ($nameArray_imge as $result_imge) {
													if ($path == "") {
														$path = '../../';
													}
													$image=$result_imge[csf('image_location')];}

												?>
												<td>
												<img src="<? echo $path . $image; ?>" width="90" height="100" border="2" />
			...
			<table class="rpt_table" width="100%" border="1" cellpadding="0" cellspacing="0" rules="all">
				<tr>
					<th colspan="7" align="center">Fab Description & Color Wise Summary</th>
				</tr>
				<tr>
					<th>Fabrication</th>
					<th>Color Type</th>
				...
				foreach ($fabric_color_summary as $desc_id => $desc_data) {
					...
					foreach ($desc_data as $gsm_id => $gsm_data) {
						...
						foreach ($gsm_data as $dia_id => $dia_data) {
							...
							foreach ($dia_data as $color_id => $color_data) {
								foreach ($color_data as $print_id => $print_data) {
							...
							$gsm_id_arr[$desc_id][$gsm_id] = $gsm_row;
						}
					}
				}
				...
				foreach ($fabric_color_summary as $desc_id => $desc_data) {
					...
					foreach ($desc_data as $gsm_id => $gsm_data) {
						...
						foreach ($gsm_data as $dia_id => $dia_data) {
							...
							foreach ($dia_data as $color_id => $color_data) {
								foreach ($color_data as $print_id => $color_summ) {
				?>
									<tr>
										<?
										if ($tr == 1) { ?>
											<td rowspan="<?= $desc_id_arr[$desc_id]; ?>"><?= $desc_id;  ?></td>
											<td align="center" rowspan="<?= $desc_id_arr[$desc_id]; ?>"><?= $color_type[$color_summ['color_type_id']]  ?></td>
										...
										if ($dtr == 1) {
										?>
											<td align="center" rowspan="<?= $gsm_id_arr[$desc_id][$gsm_id]; ?>"><?= $gsm_id  ?></td>
										...
										?>
										<td align="center" rowspan="<?= $dia_id_arr[$desc_id]; ?>"><?= $dia_id  ?></td>
										<td align="center"><?= $color_library[$color_id]  ?></td>
										<td align="center"><?= $color_library[$print_id]  ?></td>
			...
			echo signature_table(79, $cbo_company_name, "1113px", $template_id, 10, $user_lib_name[$inserted_by]);
			?>
			</div>
	...
	if ($action == "show_trim_booking_report5") // md mamun =>12-06-2022=>ISD:11977
	{
			...
			<div style="width:1150px" align="left">
				<table border="1" align="left" class="rpt_table container" cellpadding="0" width="100%" cellspacing="0" rules="all" style="padding-bottom: 10px">
					<tr>
						...
						<td width="200px" colspan="2" align="center"><b><?= $group_lib[0][csf('group_name')]; ?></b></td>
						<td colspan="2" align="center"><b>M&M DEPARTMENT</b></td>
						<td colspan="2" align="center"><b>Service Booking For AOP Work Order
		...
		a.booking_no=$txt_booking_no  and a.status_active=1 group by b.id ,b.po_number, a.dia_width, a.fabric_color_id, a.fin_gsm, a.gmts_color_id, a.fabric_color_id, a.job_no, a.description, a.program_no, a.lib_composition, a.delivery_date, a.delivery_end_date, a.pre_cost_fabric_cost_dtls_id , a.sensitivity, a.rate, a.uom, a.amount, a.wo_qnty,b.grouping, d.body_part_id, e.style_ref_no,e.id,d.fabric_description, d.color_type_id, d.id, d.lib_yarn_count_deter_id,d.gsm_weight,a.artwork_no,a.printing_color_id,a.id,a.aop_mc_type,a.aop_type ");
				...
				$unapprove_data_array = sql_select("select b.approved_by,b.approved_date,b.un_approved_reason,b.un_approved_date,b.approved_no from   approval_history b where b.mst_id=$mst_id and b.entry_form=162  order by b.approved_date,b.approved_by");
				...
				if (count($unapprove_data_array) > 0) {
					$sql_unapproved = sql_select("select booking_id,approval_cause from fabric_booking_approval_cause where  entry_form=162 and approval_type=2 and is_deleted=0 and status_active=1 and booking_id=$mst_id");
	...
	if ($action == "show_trim_booking_report7") // Md. Sakibul Islam =>21-12-2023=>ISD:27718
	{
		...
		?>
			<div style="width:1333px" align="left">
			<table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid black;">
				...
				$sql = "SELECT a.pay_mode, a.company_id,a.tagged_booking_no, a.booking_no, a.booking_date, a.supplier_id, a.currency_id, a.exchange_rate, a.attention, a.delivery_date, a.source, a.is_short,a.entry_form,a.booking_type,a.is_approved,a.remarks  from wo_booking_mst a where  a.booking_no=$txt_booking_no and a.status_active=1 and a.is_deleted=0";
					...
					<table width="100%" style="border:1px solid black;table-layout: fixed; margin-top:1px">
						...
						<tr>
							...
							<td width="110">:&nbsp;<? $currency_id = $result[csf('currency_id')];
													echo $currency[$result[csf('currency_id')]]; ?></td>
		...
		?>
			<div style="width:1333px" align="center">
				...
				?>
					<table border="1" align="left" class="rpt_table" cellpadding="0" width="90%" cellspacing="0" rules="all">
						...
						?>
							<tr>
								...
								?>
									...
									<td><? //echo $result_itemdescription['brand_supplier']; 
										?> Booking Qnty </td>
									<?
									foreach ($nameArray_color  as $result_color) {
						...
						<tr>
							...
							<td style="text-align:right"><? //echo number_format(array_sum($color_tatal),2);  
															?></td>
							<td style="text-align:right"><? //echo number_format(round($color_total),0); 
															?></td>
							<td style="text-align:right">
								<?
					...
					<tr>
						...
						<td style="text-align:right"><? echo number_format($grand_total_as_per_gmts_color, 4);
														$booking_grand_total += $grand_total_as_per_gmts_color; ?></td>
					</tr>
					</table>
					<br />
				<?
				...
				?>
					...
					<table border="0" align="left" cellpadding="0" width="90%" cellspacing="0">
						...
						?>
							<tr>
								...
								?>
									...
									<td style="border:1px solid black"><? //echo $result_itemdescription['brand_supplier']; 
																		?> Booking Qnty </td>
									<?
									foreach ($nameArray_size  as $result_size) {
						...
						<tr>
							...
							<td style="border:1px solid black;  text-align:right"><? //echo number_format(array_sum($color_tatal),2);  
																					?></td>
							<td style="border:1px solid black; text-align:right"><? //echo number_format(round($color_total),0); 
																					?></td>
							<td style="border:1px solid black; text-align:right">
								<?
								echo number_format($total_amount_as_per_gmts_color, 4);
					...
					<tr>
						...
						<td style="border:1px solid black;  text-align:right"><? echo number_format($grand_total_as_per_gmts_color, 4);
																				$booking_grand_total += $grand_total_as_per_gmts_color; ?></td>
					</tr>
					</table>
					<br />
						...
						?>
							<tr>
								...
								?>
									...
									<td style="border:1px solid black" rowspan="<? echo count($nameArray_color); ?>"><? //echo $result_itemdescription['brand_supplier']; 
																														?>Booking Qnty </td>
									<?
									//$item_desctiption_total=0;
									foreach ($nameArray_color as $result_color) {
				...
				<table width="90%" class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all">
					<tr>
						<th width="70%" style="text-align:right">Total Booking Amount</th>
						<td width="30%" style="text-align:right"><? echo number_format($booking_grand_total, 4); ?></td>
					</tr>
					<tr>
						<th width="70%" style="text-align:right">Total Booking Amount (in word)</th>
						<td width="30%"><? echo number_to_words(def_number_format($booking_grand_total, 2, ""), $mcurrency, $dcurrency); //echo number_to_words(number_format($booking_grand_total,2),"USD", "CENTS");
										?></td>
					</tr>
				</table>
				&nbsp;
				<table width="90%" class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all">
					<thead>
						<tr>
							<th width="3%">Sl</th>
							<th width="97%">Spacial Instruction</th>
						</tr>
					</thead>
					<tbody>
						<?
	...
	if ($action == "show_trim_booking_report1") {
		...
		?>
			<div style="width:1333px" align="center">
				...
				$fab_booking_arr = sql_select($sql_2);
				$sql = "SELECT a.pay_mode, a.company_id,a.tagged_booking_no, a.booking_no, a.booking_date, a.supplier_id, a.currency_id, a.exchange_rate, a.attention, a.delivery_date, a.source, a.is_short,a.entry_form,a.booking_type,a.is_approved  from wo_booking_mst a where  a.booking_no=$txt_booking_no and a.status_active=1 and a.is_deleted=0";
				...
				$currency_sql = "SELECT a.currency_id from wo_booking_mst a where  a.booking_no=$txt_booking_no and a.status_active=1 and a.is_deleted=0";
				$currency_sql_res = sql_select($currency_sql);
				foreach ($currency_sql_res as $row) {
					$currency_show = $row[csf('currency_id')];
				}
				...
				?>
						...
						<table border="1" align="left" class="rpt_table" cellpadding="0" width="100%" cellspacing="0" rules="all">
							...
							$k = 1;
							$tot_row = count($nameArray_item_description);
							foreach ($nameArray_item_description as $result_itemdescription) {
							...
							?>
								<tr>
									...
									<td style="border:1px solid black; text-align:center"><? echo $currency[$currency_show]; ?> </td>
									<td style="border:1px solid black; text-align:right">
							...
							?>
							<tr>
								<td colspan="5" align="right"> <strong> Total Qty (<? echo $fab_uom; ?>) </strong></td>
				...
				<table width="100%" class="rpt_table" style="border:1px solid black;" border="1" cellpadding="0" cellspacing="0" rules="all">
					<tr style="border:1px solid black;">
						...
						<th width="70%" style="border:1px solid black; text-align:right">Total Wo Qty(<? echo $fab_uom; ?>) &nbsp;</th>
						<td width="30%" style="border:1px solid black; text-align:left"><? echo number_format($booking_grand_qty, 2); ?></td>
					</tr>

					<tr style="border:1px solid black;">

						<th width="70%" style="border:1px solid black; text-align:right">Total Booking Amount</th>
						<td width="30%" style="border:1px solid black; text-align:left" align="left"><? echo number_format($booking_grand_total, 2); ?></td>
					</tr>
				</table>
				<table width="100%" class="rpt_table" style="border:1px solid black;" border="1" cellpadding="0" cellspacing="0" rules="all">
					<tr style="border:1px solid black;">
						<th width="20%" style="border:1px solid black; text-align:left">Total Booking Amount (in word)</th>
						<td width="80%" style="border:1px solid black;" align="left"><b><? echo number_to_words(def_number_format($booking_grand_total, 2, ""), $mcurrency, $dcurrency); ?></b></td>
					</tr>
				</table>
				&nbsp;
				<?
				...
				?>
					<table border="0" cellpadding="0" cellspacing="0" width="100%" class="rpt_table" style="border:1px solid black;">
						<tr>
							<td style="border:1px solid black;" colspan="9" align="center"><b> Comments</b> </td>
						</tr>
						<tr style="border:1px solid black;" align="center">
							...
							<th style="border:1px solid black;" width=""> Commedddnts </th>
						</tr>
						<tbody>
							<?
							...
							$po_qty_arr = array();
							$aop_data_arr = array();
							$aop_booking_array = array();
							$aop_booking_data = array();
							...
							foreach ($nameArray as $selectResult) {
								...
								//echo $pre_cost_currence_id.'='.$wo_currence_id;
								...
								/*else if($wo_currence_id==2 && $pre_cost_currence_id==1 ) 
					{
						 $aop_charge=$selectResult[csf("amount")]*$result[csf('exchange_rate')];
								...
								//echo $selectResult[csf("amount")].'='.$pre_cost_exchange_rate.'<br>';				
							...
							?>
								<tr>
									...
									<td title="Exchange rate=<? echo $pre_cost_exchange_rate; ?>" style="border:1px solid black;" width="10%" align="right"><? echo number_format($aop_charge, 2); ?></td>
									<td style="border:1px solid black;" width="" align="right">
										<? $tot_balance = $pre_cost_aop - $aop_charge;
	...
	if ($action == "show_trim_booking_report2") {
        ...
        foreach ($nameArray_job as $result_job){
			...
			$po_number_arr[$result_job[csf('id')]]=$result_job[csf('po_number')];
				...
				$season = array();
				$season_names = '';
				$nameArray_buyer = sql_select("select  a.style_ref_no, a.job_no, a.style_owner, a.buyer_name, a.dealing_marchant,a.season,a.season_matrix,a.season_buyer_wise   from wo_po_details_master a, wo_booking_dtls b where a.job_no=b.job_no and b.booking_no=$txt_booking_no and b.status_active=1 and b.PROCESS=35");
				foreach ($nameArray_buyer as $result_buy) {
					$buyer_string[$result_buy[csf('buyer_name')]] = $buyer_name_arr[$result_buy[csf('buyer_name')]];
					$style_owner[$result_buy[csf('job_no')]] = $company_library[$result_buy[csf('style_owner')]];
					$job_no[$result_buy[csf('job_no')]] = $result_buy[csf('job_no')];
				...
				$nameArray_job = sql_select("select b.id, b.po_number, b.job_no_mst as job_no, b.grouping, b.file_no, b.po_quantity  from wo_booking_dtls a, wo_po_break_down b where a.po_break_down_id=b.id and a.booking_no=$txt_booking_no and b.status_active=1  and a.status_active=1 and a.PROCESS=35");
				foreach ($nameArray_job as $result_job) {
					$po_no[$result_job[csf('id')]] = $result_job[csf('po_number')];
					$job_no_arr[$result_job[csf('id')]] = $result_job[csf('job_no')];
					$file_no[$result_job[csf('id')]] = $result_job[csf('file_no')];
			...
			group by a.pay_mode, a.company_id,a.tagged_booking_no, a.booking_no, a.booking_date, a.supplier_id, a.process, a.currency_id, a.exchange_rate, a.attention, a.delivery_date, a.source, a.is_short,a.entry_form,a.booking_type";
				...
				$fab_booking_arr = sql_select($sql_2);
				$sql = "SELECT a.pay_mode, a.company_id,a.tagged_booking_no, a.booking_no, a.booking_date, a.supplier_id, a.currency_id, a.exchange_rate, a.attention, a.delivery_date, a.source, a.is_short,a.entry_form,a.booking_type  from wo_booking_mst a where  a.booking_no=$txt_booking_no and a.status_active=1 and a.status_active=1 and a.entry_form=162 and a.is_deleted=0";

				//cho $sql;die();


				$nameArray = sql_select("select a.pay_mode,a.inserted_by, a.company_id,a.tagged_booking_no, a.booking_no,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.attention,a.delivery_date,a.source, a.is_short  from wo_booking_mst a where  a.booking_no=$txt_booking_no and a.status_active=1 and a.is_deleted=0  and a.entry_form=162");
				...
				?>
					<table width="100%" style="border:1px solid black;table-layout: fixed;">
						...
						<tr>
							...
							<td>Booking Type : <? if ($result[csf('is_short')] == 2) echo "Main";
												else echo "Short"; ?></td>
						</tr>
						<tr>
							<td style="font-size:12px"><b>PO No</b> </td>
							...
							<td>Pay Mode </td>
							<td style="overflow:hidden;text-overflow: ellipsis;white-space: nowrap;">:&nbsp;<?= $pay_mode[$result[csf("pay_mode")]]; ?></td>
							<td>Fabric Booking : <?= $result[csf('tagged_booking_no')]; ?></td>
						</tr>
					</table>
					<br />
				...
				?>
						...
						<table border="1" align="left" class="rpt_table" cellpadding="0" width="100%" cellspacing="0" rules="all">
							...
							?>
								<tr>
									...
									<td style="border:1px solid black; text-align:right">
										...
										echo number_format($amount_as_per_gmts_color, 2);
										$total_amount_as_per_gmts_color += $amount_as_per_gmts_color;
										$total_qty_as_per_gmts_color += number_format($wo_qnty_sum, 2, '.', '');
										?>
									</td>
							...
							<tr>
								...
								<td align="right"><? echo fn_number_format($total_qty_as_per_gmts_color, 2); ?> </td>
				...
				<table width="100%" class="rpt_table" style="border:1px solid black;" border="1" cellpadding="0" cellspacing="0" rules="all">
					<tr style="border:1px solid black;">
						...
						<th width="70%" style="border:1px solid black; text-align:right">Total WO Qty &nbsp;</th>
						<td width="30%" style="border:1px solid black; text-align:left"><? echo number_format($booking_grand_qty, 2); ?></td>
					</tr>
					...
					<tr style="border:1px solid black;">
						...
						<th width="70%" style="border:1px solid black; text-align:right">Total Booking Amount</th>
						<td width="30%" style="border:1px solid black; text-align:left"><? echo number_format($booking_grand_total, 2); ?></td>
					</tr>
					<tr style="border:1px solid black;">
						<th width="70%" style="border:1px solid black; text-align:right">Total Booking Amount (in word)</th>
						<td width="30%" style="border:1px solid black;"><? echo number_to_words(def_number_format($booking_grand_total, 2, ""), $mcurrency, $dcurrency); //number_to_words(number_format($booking_grand_total,2),$mcurrency, $dcurrency);
																		?></td>
					</tr>
				</table>
				<br />
						...
						?>
							<td valign="top" width="90" style="word-wrap:break-word;width:90px;">
								<p style="word-wrap:break-word; width:90px">
									<!--<img src="../../<? //echo $result_imge[csf('image_location')]; 
														?>" width="90" height="100" border="2" />-->
				...
				?>
					<table border="0" cellpadding="0" cellspacing="0" width="100%" class="rpt_table" style="border:1px solid black;">
						...
						<tr style="border:1px solid black;" align="center">
							...
							<th style="border:1px solid black;" width="10%">Balance</th>
							<th style="border:1px solid black;" width=""> Commeaaants </th>
						</tr>
						<tbody>
							<?
							$pre_cost_currency_arr = return_library_array("select job_no,currency_id from  wo_po_details_master", "job_no", "currency_id");
	...
	union all
	select b.id as APPROVED_BY, a.INSERT_DATE as APPROVED_DATE,b.USER_FULL_NAME,c.CUSTOM_DESIGNATION from wo_booking_mst a,USER_PASSWD b,LIB_DESIGNATION c where a.id =$update_id and a.INSERTED_BY=b.id and b.DESIGNATION=c.id and c.STATUS_ACTIVE=1 and c.IS_DELETED=0 and b.STATUS_ACTIVE=1 and b.IS_DELETED=0";
				//echo $appSql;die;
				...
				echo get_app_signature(310, $cbo_company_name, "900px", $template_id, 50, $inserted_by, $userDtlsArr);
				echo "****" . custom_file_name($txt_booking_no, $style_sting, $job_no);
				?>
			</div>
		<?
	...
	if ($action == "show_trim_booking_report3") {
		...
		?>
			<div style="width:1333px" align="center">
				...
				$nameArray = sql_select("select a.pay_mode, a.company_id,a.tagged_booking_no, a.booking_no,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.attention,a.delivery_date,a.source, a.is_short  from wo_booking_mst a where  a.id=$booking_id and a.status_active=1 and a.is_deleted=0");
				foreach ($nameArray as $result) {
					if ($result[csf("pay_mode")] == 1 || $result[csf("pay_mode")] == 2) {
						$supp = $supplier_name_arr[$result[csf("supplier_id")]];
						$supp_adds = $supplier_address_arr[$result[csf("supplier_id")]];
...
and b.booking_no=$txt_booking_no and b.wo_qnty !=0 and b.status_active=1 and b.is_deleted=0 and b.process=35 and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0 ");
				...
				<table width="100%" class="rpt_table" style="border:1px solid black;" border="1" cellpadding="0" cellspacing="0" rules="all">
					...
					<tr style="border:1px solid black;">
						...
						<th width="70%" style="border:1px solid black; text-align:right">Total Booking Amount</th>
						<td width="30%" style="border:1px solid black; text-align:right"><? echo number_format($booking_grand_total, 2); ?></td>
					</tr>
					<tr style="border:1px solid black;">
						<th width="70%" style="border:1px solid black; text-align:right">Total Booking Amount (in word)</th>
						<td width="30%" style="border:1px solid black;"><? echo number_to_words(def_number_format($booking_grand_total, 2, ""), $mcurrency, $dcurrency); ?></td>
					</tr>
				</table>
				&nbsp;
				<?
	...
	if ($action == "show_trim_booking_report6") {
		...
		?>
			<div style="width:1333px" align="center">
				...
				?>
					<table width="100%" style="border:1px solid black;table-layout: fixed; margin-top:1px">
						...
						<tr>
							...
							<td width="100" style="font-size:12px"><b>Booking Type : <? if ($result[csf('is_short')] == 2) echo "Main";
																						else echo "Short"; ?></b> </td>
						</tr>
						<tr>
				...
				$nameArray_item = sql_select("select distinct description from wo_booking_dtls b, WO_PRE_COST_FAB_CONV_COST_DTLS a where b.pre_cost_fabric_cost_dtls_id=a.id and b.booking_no=$txt_booking_no and b.job_no in(" . implode(",", $job_no_in) . ") and b.wo_qnty !=0 and b.status_active=1 and b.is_deleted=0 and b.process=35 and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0 ");


				$nameArray_color = sql_select("select distinct fabric_color_id,printing_color_id from wo_booking_dtls   where  booking_no=$txt_booking_no and wo_qnty !=0  and status_active=1 and is_deleted=0 and process=35 and status_active=1 and is_deleted=0 ");
				$nameArray_job_po = sql_select("select description,job_no from wo_booking_dtls  where booking_no=$txt_booking_no and status_active =1 and is_deleted=0");


				foreach ($nameArray_job_po as $nameArray_job_po_row) {
					$job_noArr[$nameArray_job_po_row[csf('description')]] .= $nameArray_job_po_row[csf('job_no')] . ',';
				}

				if (count($nameArray_color) > 0) {
					foreach ($nameArray_item as $result_item) {
						$nameArray_job_No = rtrim($job_noArr[$result_item[csf('description')]], ',');
						$job_nos = implode(",", array_unique(explode(",", $nameArray_job_No)));
				?>
						...
						<table border="1" align="left" class="rpt_table" cellpadding="0" width="100%" cellspacing="0" rules="all">
							<tr>
								<td colspan="10" align="">
									<strong><? echo "Fabrication:" . rtrim($fabric_description_array[$result_item[csf('description')]], ", ");
											$fab_uom = $fabric_description_uom_array[$result_item[csf('description')]];
											?>&nbsp;<? echo "Job NO:" . $job_nos; ?> <? echo "&nbsp;Style No:" . $style_ref[$job_nos]; ?> &nbsp; PO No:<? echo implode(",", $po_no[$job_nos]); ?> </strong><br />
								</td>
							</tr>
							<tr>
								<td style="border:1px solid black"><strong>GMT Color</strong> </td>
	...
	if ($action == "create_booking_search_list_view2") {
		...
		if ($data[7] == 1) {
			if (str_replace("'", "", $data[6]) != "") $booking_cond = " and a.booking_no_prefix_num='$data[6]'    ";
			else  $booking_cond = "";
			if (str_replace("'", "", $data[4]) != "") $job_cond = " and b.job_no_prefix_num='$data[4]'  ";
			else  $job_cond = "";
			if (trim($data[10]) != "") $style_cond = " and b.style_ref_no ='$data[10]'";
			if (str_replace("'", "", $data[11]) != "") $order_cond = " and c.po_number = '$data[11]'  ";
		}
		if ($data[7] == 2) {
			if (str_replace("'", "", $data[6]) != "") $booking_cond = " and a.booking_no_prefix_num like '$data[6]%'  $booking_year_cond  ";
			else  $booking_cond = "";
			if (str_replace("'", "", $data[4]) != "") $job_cond = " and b.job_no_prefix_num like '$data[4]%'  $year_cond  ";
			else  $job_cond = "";
		...
		$internal_ref = str_replace("'", "", $data[9]);
		if ($file_no == "") $file_no_cond = "";
		else $file_no_cond = " and c.file_no='" . trim($file_no) . "' ";
		if ($internal_ref == "") $internal_ref_cond = "";
		else $internal_ref_cond = " and c.grouping='" . trim($internal_ref) . "' ";
