...
{
	$data=explode("**",$data);
	if($db_type==0) $conversion_date=change_date_format($data[1], "yyyy-mm-dd", "-",1); else $conversion_date=change_date_format($data[1], "d-M-y", "-",1);
...
if ($action=="order_popup")
{
    ...
    <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
        <table width="960" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
            <thead>
                <tr>
                    <th colspan="9" align="center"><? echo create_drop_down( "cbo_string_search_type", 100, $string_search_type,'', 1, "-- Searching Type --" ); ?></th>
                </tr>
                <tr>
            ...
            <tr class="general">
                <td>
                    ...
                    <?
                    echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", '',"load_drop_down( 'print_booking_urmi_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" ); ?>
                </td>
                ...
                <td align="center">
                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_job_prifix').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_order_search').value+'_'+document.getElementById('txt_style').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+document.getElementById('txt_ref_no').value, 'create_po_search_list_view', 'search_div', 'print_booking_urmi_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" /></td>
            </tr>
            <tr>
...
if($action=="create_po_search_list_view")
{
	...
	if($db_type==0)
	{
		...
		$year_cond=" and YEAR(a.insert_date)=$data[5]";
	}
	else if($db_type==2)
	{
...
//echo "select b.id, b.page_id, b.approval_need, b.allow_partial, b.validate_page,a.setup_date from approval_setup_mst a,approval_setup_dtls b 
//where a.id=b.mst_id and a.company_id='$data[0]' and a.status_active=1 and b.page_id=25 and b.status_active=1 and b.is_deleted=0 order by b.id desc ";
		...
		//echo $approval_cond.'dd';
		

	 $sql= "select $yearCond as year, a.job_no_prefix_num, a.company_name, a.buyer_name, a.style_ref_no, a.job_quantity, b.po_number, b.po_quantity, b.shipment_date, a.job_no, c.id as pre_id, b.grouping from wo_po_details_master a, wo_po_break_down b, wo_pre_cost_mst c, wo_pre_cost_embe_cost_dtls d where a.job_no=b.job_no_mst and a.job_no=c.job_no and a.job_no=d.job_no and b.shiping_status not in(3) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 $shipment_date $company $buyer $job_cond $style_cond $ref_cond $order_cond $approval_cond order by a.job_no";

	echo  create_list_view("list_view", "Year,Job No,Company,Buyer Name,Internal Ref. No,Style Ref. No,Job Qty.,PO number,PO Quantity,Shipment Date, Precost id", "60,60,100,100,80,100,80,90,80,70,60","950","320",0, $sql , "js_set_value", "job_no", "", 1, "0,0,company_name,buyer_name,0,0,0,0,0,0", $arr , "year,job_no_prefix_num,company_name,buyer_name,grouping,style_ref_no,job_quantity,po_number,po_quantity,shipment_date,pre_id", "",'','0,0,0,0,0,0,1,0,1,3,0') ;
	exit();
}
...
{
	$emb_name="";
	$data_array=sql_select("select a.job_no, b.emb_name from wo_po_details_master a, wo_pre_cost_embe_cost_dtls b where a.job_no ='".$data."' and b.cons_dzn_gmts !=0 and b.emb_name in(1,2,3,4,5) and a.job_no=b.job_no and b.status_active=1 and a.status_active=1");
	...
	if($emb_name !="")
	{
		$cbo_booking_natu= create_drop_down( "cbo_booking_natu", 130, $emblishment_name_array,"", 1, "-- Select --", "","fnc_generate_booking()", "", $emb_name);
		echo "document.getElementById('booking_natu_td').innerHTML ='".$cbo_booking_natu."';\n";
	}
	else echo "document.getElementById('cbo_booking_natu').value = 0;";
	exit();
...
if ($action=="populate_order_data_from_search_popup")
{
	...
	$data_array=sql_select("select a.job_no,a.company_name,a.season_buyer_wise,a.currency_id,a.buyer_name,a.gmts_item_id,b.emb_name from wo_po_details_master a, wo_pre_cost_embe_cost_dtls b where a.job_no ='".$data."' and b.cons_dzn_gmts !=0 and b.emb_name in(1,2,3,4,5) and a.job_no=b.job_no and b.status_active=1 and a.status_active=1");
...
if ($action=="generate_print_booking")
{
	...
	$is_short=str_replace("'","",$cbo_is_short);
	 ...
	 $currency_id=str_replace("'","",$cbo_currency);
	...
	//echo $costPerQty.'DD';
	if($costPerQty>1) $costPerUom=($costPerQty/12)." Dzn"; else $costPerUom=($costPerQty/1)." Pcs";
//getQtyArray_by_job

	if($is_short==1)//Yes
	{
		$emblishment_short_qty_arr=$emblishment->getQtyArray_by_job();
		$wash_short_qty_arr=$wash->getQtyArray_by_job();
		//txt_job_no
		//$emblishment_req_qty=$emblishment_short_qty_arr[$txt_job_no];
		//$wash_req_qty=$wash_short_qty_arr[$txt_job_no];
		//echo $emblishment_req_qty.'='.$wash_req_qty.',';
	}

	if($cbo_level==1)
	{
		$emblishmentqty_arr=$emblishment->getQtyArray_by_OrderCountryGmtsitemGmtscolorAndEmblishmentid();
		$emblishmentamount_arr=$emblishment->getAmountArray_by_OrderCountryGmtsitemGmtscolorAndEmblishmentid();
		//$wash= new wash($condition);
		$washqty_arr=$wash->getQtyArray_by_OrderCountryGmtsitemGmtscolorAndEmblishmentid();
		$washamount_arr=$wash->getAmountArray_by_OrderCountryGmtsitemGmtscolorAndEmblishmentid();
	}
	else
	{
	...
	//print_r($emblishmentqty_arr);
	?>
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1347" class="rpt_table" >
        <thead>
            <th width="40">SL</th>
            ...
            <th width="60">Order Qty (Pcs)<? //if($calculation_basis==1){echo "Order Qty (Pcs)";} else{echo "Plan Cut(Pcs)";} ?></th>
            <th width="200">Description</th>
            <th width="60">Req Qty /<? echo $costPerUom; ?></th>
            <th width="60">CU WOQ/ <? echo $costPerUom; ?></th>
            <th width="60">Bal Qty/ <? echo $costPerUom; ?></th>
            <th width="60">WOQ / <? echo $costPerUom; ?></th>
            <th width="50">Rate/ <? echo $costPerUom; ?></th>
            <th width="60">Amount/ <? //echo $costPerUom; ?></th>
            <th>Delv. Date</th>
        </thead>
    </table>
    <div style="width:1347px; overflow-y:scroll; max-height:350px;" id="buyer_list_view" align="center">
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1330" class="rpt_table" id="tbl_list_search" >
       <tbody>
       <?
	   $sql_embl="select a.country_id, a.color_number_id from wo_pre_cos_emb_co_avg_con_dtls a, wo_pre_cost_embe_cost_dtls b where b.id=a.pre_cost_emb_cost_dtls_id and b.job_no='$txt_job_no' and b.status_active=1 and b.is_deleted=0 and b.emb_name=$cbo_booking_natu and a.item_number_id=$cbo_gmt_item group by a.country_id, a.color_number_id";

		$result_embl=sql_select($sql_embl);
		$country_ids='';
		foreach($result_embl as $row )
		{

			if($row[csf('country_id')])
			{
			$country_arr[$row[csf('country_id')]]=$row[csf('country_id')];
			}
			$job_country_arr[$row[csf('color_number_id')]]['country'].=$country_name_library[$row[csf('country_id')]].',';
			$job_country_arr[$row[csf('color_number_id')]]['country_id'].=$row[csf('country_id')].',';
		}
		$country_ids=implode (",", $country_arr);
		unset($result_embl);
		if($country_ids!='') $country_cond="and e.country_id in($country_ids)";else $country_cond="";
		$poCond="";$poCond2="";
		if($txt_order_no_id)
		{
			$poCond="and d.id in($txt_order_no_id)";
			$poCond2="and b.po_break_down_id in($txt_order_no_id)";
		}
		$booking_data_array_cu=array();
		$sql_booking_cu= sql_select( "select c.job_no,b.po_break_down_id, b.country_id_string, b.gmt_item, b.pre_cost_fabric_cost_dtls_id as cost_dtls_id, b.gmts_color_id, b.wo_qnty as cu_woq, b.amount as cu_woamt, c.emb_name, c.emb_type from wo_booking_mst a, wo_booking_dtls b, wo_pre_cost_embe_cost_dtls c where a.job_no=b.job_no and a.job_no=c.job_no and a.booking_no=b.booking_no and b.pre_cost_fabric_cost_dtls_id=c.id and a.job_no='$txt_job_no' and a.booking_type=6 and a.is_short=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.wo_qnty!=0 $poCond2");

		foreach($sql_booking_cu as $row_cu)
		{
			$country_id_string=array_unique(explode(",",$row_cu[csf('country_id_string')]));
			$country_idss=$country_id_string[0];
			if($cbo_level==1)
			{
				$booking_data_array_cu[$row_cu[csf('po_break_down_id')]][$row_cu[csf('country_id_string')]][$row_cu[csf('gmt_item')]][$row_cu[csf('cost_dtls_id')]][$row_cu[csf('emb_name')]][$row_cu[csf('emb_type')]][$row_cu[csf('gmts_color_id')]]['wo_qty']+=$row_cu[csf('cu_woq')];

				$booking_data_array_cu[$row_cu[csf('po_break_down_id')]][$row_cu[csf('country_id_string')]][$row_cu[csf('gmt_item')]][$row_cu[csf('cost_dtls_id')]][$row_cu[csf('emb_name')]][$row_cu[csf('emb_type')]][$row_cu[csf('gmts_color_id')]]['wo_amt']+=$row_cu[csf('cu_woamt')];
			}
			else
			{
				$booking_data_array_cu[$row_cu[csf('po_break_down_id')]][$row_cu[csf('gmt_item')]][$row_cu[csf('cost_dtls_id')]][$row_cu[csf('emb_name')]][$row_cu[csf('emb_type')]][$row_cu[csf('gmts_color_id')]]['wo_qty']+=$row_cu[csf('cu_woq')];
				$booking_data_array_cu[$row_cu[csf('po_break_down_id')]][$row_cu[csf('gmt_item')]][$row_cu[csf('cost_dtls_id')]][$row_cu[csf('emb_name')]][$row_cu[csf('emb_type')]][$row_cu[csf('gmts_color_id')]]['wo_amt']+=$row_cu[csf('cu_woamt')];
				$booking_previ_QtyArr[$row_cu[csf('job_no')]]+=$row_cu[csf('cu_woq')];
			}
		}
		unset($sql_booking_cu);
		//print_r($booking_data_array_cu);
		
		$booking_data_array=array();
		$updateid="";//and b.wo_qnty!=0
		$sql_booking= sql_select( "select b.id, b.job_no, b.po_break_down_id, b.pre_cost_fabric_cost_dtls_id as pre_cost_dtls_id, b.country_id_string, b.booking_no, b.booking_type, b.gmts_color_id, b.gmt_item, b.wo_qnty, b.rate, b.amount, b.delivery_date, b.description from wo_booking_dtls b, wo_pre_cost_embe_cost_dtls c where c.id=b.pre_cost_fabric_cost_dtls_id and b.job_no=c.job_no and b.booking_no='$txt_booking_no' and b.booking_type=6 and b.emblishment_name=c.emb_name and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 $poCond2");// and b.po_break_down_id=32936
		...
		foreach($sql_booking as $row)
		{
			...
			if($cbo_level==1)
			{
				$country_id_string=$row[csf('country_id_string')];

				$booking_data_array[$po_id][$country_id_string][$row[csf('gmt_item')]][$row[csf('pre_cost_dtls_id')]][$row[csf('gmts_color_id')]][id]=$row[csf('id')];
				$booking_data_array[$po_id][$country_id_string][$row[csf('gmt_item')]][$row[csf('pre_cost_dtls_id')]][$row[csf('gmts_color_id')]][wo_qnty]+=$row[csf('wo_qnty')];
				$booking_data_array[$po_id][$country_id_string][$row[csf('gmt_item')]][$row[csf('pre_cost_dtls_id')]][$row[csf('gmts_color_id')]][rate]=$row[csf('rate')];
				$booking_data_array[$po_id][$country_id_string][$row[csf('gmt_item')]][$row[csf('pre_cost_dtls_id')]][$row[csf('gmts_color_id')]][amount]+=$row[csf('amount')];
				$booking_data_array[$po_id][$country_id_string][$row[csf('gmt_item')]][$row[csf('pre_cost_dtls_id')]][$row[csf('gmts_color_id')]][delivery_date]=$row[csf('delivery_date')];
				$booking_data_array[$po_id][$country_id_string][$row[csf('gmt_item')]][$row[csf('pre_cost_dtls_id')]][$row[csf('gmts_color_id')]][description]=$row[csf('description')];
			}
			else
			{
				$booking_data_array[$po_id][$row[csf('gmt_item')]][$row[csf('pre_cost_dtls_id')]][$row[csf('gmts_color_id')]][id]=$row[csf('id')];
				$booking_data_array[$po_id][$row[csf('gmt_item')]][$row[csf('pre_cost_dtls_id')]][$row[csf('gmts_color_id')]][wo_qnty]+=$row[csf('wo_qnty')];
				$booking_data_array[$po_id][$row[csf('gmt_item')]][$row[csf('pre_cost_dtls_id')]][$row[csf('gmts_color_id')]][rate]=$row[csf('rate')];
				$booking_data_array[$po_id][$row[csf('gmt_item')]][$row[csf('pre_cost_dtls_id')]][$row[csf('gmts_color_id')]][amount]+=$row[csf('amount')];

				$booking_data_array[$po_id][$row[csf('gmt_item')]][$row[csf('pre_cost_dtls_id')]][$row[csf('gmts_color_id')]][delivery_date]=$row[csf('delivery_date')];
				$booking_data_array[$po_id][$row[csf('gmt_item')]][$row[csf('pre_cost_dtls_id')]][$row[csf('gmts_color_id')]][description]=$row[csf('description')];
			}
		}
		unset($sql_booking);
		//print_r($booking_data_array);
		$country_group_concat="";
		if($cbo_level==2)
		{
			if ($db_type == 2)
			{
				$country_group_concat = "listagg(e.country_id ,',') within group (order by e.country_id) as country_id "; $country_group_filed="";
			}
			else
			{
				$country_group_concat = "group_concat(e.country_id) as country_id "; $country_group_filed="";
			}
		}
		else
		{
			$country_group_concat="e.country_id"; $country_group_filed=",e.country_id";
		}
//and b.po_break_down_id in($txt_order_no_id)
		
	 	 $sql="select a.total_set_qnty, b.exchange_rate,b.costing_per, c.id as embe_cost_dtls_id, c.job_no, c.emb_name, c.emb_type, c.cons_dzn_gmts, c.rate, c.amount, d.id as po_id, d.po_number, d.pub_shipment_date, d.grouping, d.plan_cut, e.item_number_id, e.color_number_id, $country_group_concat, sum(e.plan_cut_qnty) as plan_cut_qnty, sum(e.order_quantity) as order_quantity
		from wo_po_details_master a, wo_pre_cost_mst b, wo_pre_cost_embe_cost_dtls c, wo_po_break_down d, wo_po_color_size_breakdown e, wo_pre_cos_emb_co_avg_con_dtls f
		where
		a.id=b.job_id and a.id=c.job_id and a.id=d.job_id and a.id=e.job_id and d.id=e.po_break_down_id and
		
		d.id=f.po_break_down_id and e.item_number_id= f.item_number_id and e.color_number_id=f.color_number_id and e.size_number_id=f.size_number_id and c.id=f.pre_cost_emb_cost_dtls_id and f.requirment>0
		$cbo_company_name_cond $txt_job_no_cond $cbo_buyer_name_cond $country_cond and c.emb_name=$cbo_booking_natu and e.item_number_id=$cbo_gmt_item
		and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1 and e.is_deleted=0 and e.status_active=1 $poCond
		group by
		a.total_set_qnty, b.exchange_rate,b.costing_per, c.id, c.job_no, c.emb_name, c.emb_type, c.cons_dzn_gmts, c.rate, c.amount, d.id, d.po_number, d.pub_shipment_date, d.grouping, d.plan_cut, e.item_number_id, e.color_number_id $country_group_filed order by d.id ASC";// and d.id=32936
		//echo $sql;
		$job_level=array();
		...
		foreach ($nameArray as $selectResult)
		{
			...
			if($cbo_level==1)
			{
				...
				$country_id=$selectResult[csf('country_id')];
				$booking_dtls_id=$booking_data_array[$po_id][$country_id][$selectResult[csf('item_number_id')]][$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]][id];
				$updateid.=$booking_dtls_id;
				//echo $wo_qnty_pre.'='.$po_id.'<br>';
				$wo_qnty_pre=$booking_data_array[$po_id][$country_id][$selectResult[csf('item_number_id')]][$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]][wo_qnty];
				//echo $wo_qnty_pre.'='.$po_id.'<br>';
				$rate_pre=$booking_data_array[$po_id][$country_id][$selectResult[csf('item_number_id')]][$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]][rate];
				$amount_pre=$booking_data_array[$po_id][$country_id][$selectResult[csf('item_number_id')]][$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]][amount];
				$delivery_date=$booking_data_array[$po_id][$country_id][$selectResult[csf('item_number_id')]][$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]][delivery_date];
				$description=$booking_data_array[$po_id][$country_id][$selectResult[csf('item_number_id')]][$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]][description];

				$cu_woq=$booking_data_array_cu[$po_id][$country_id][$selectResult[csf('item_number_id')]][$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('emb_name')]][$selectResult[csf('emb_type')]][$selectResult[csf('color_number_id')]]['wo_qty'];

				$cu_amount=$booking_data_array_cu[$po_id][$country_id][$selectResult[csf('item_number_id')]][$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('emb_name')]][$selectResult[csf('emb_type')]][$selectResult[csf('color_number_id')]]['cu_woamt'];
			}
			else
			{
				$booking_dtls_id=$booking_data_array[$po_id][$selectResult[csf('item_number_id')]][$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]][id];
				$updateid.=$booking_dtls_id;
				//echo $wo_qnty_pre.'='.$po_id.'<br>';
				$wo_qnty_pre=$booking_data_array[$po_id][$selectResult[csf('item_number_id')]][$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]][wo_qnty];
				$rate_pre=$booking_data_array[$po_id][$selectResult[csf('item_number_id')]][$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]][rate];
				$amount_pre=$booking_data_array[$po_id][$selectResult[csf('item_number_id')]][$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]][amount];
				//echo $amount_pre.'DD';
				$delivery_date=$booking_data_array[$po_id][$selectResult[csf('item_number_id')]][$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]][delivery_date];
				$description=$booking_data_array[$po_id][$selectResult[csf('item_number_id')]][$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]][description];

				$cu_woq=$booking_data_array_cu[$po_id][$selectResult[csf('item_number_id')]][$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('emb_name')]][$selectResult[csf('emb_type')]][$selectResult[csf('color_number_id')]]['wo_qty'];
				$cu_amount=$booking_data_array_cu[$po_id][$selectResult[csf('item_number_id')]][$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('emb_name')]][$selectResult[csf('emb_type')]][$selectResult[csf('color_number_id')]]['cu_woamt'];
				//echo $po_id."=".$cu_woq.'<br/>';
			}
			...
			$basis_plan_qty=0; $basis_order_qty=0;
			//echo $calculation_basis.'D';
			if($calculation_basis==1)
			{
				$basis_plan_qty=$selectResult[csf('plan_cut_qnty')]; $basis_order_qty=$selectResult[csf('order_quantity')];
			}
			else
			{
				$basis_plan_qty=$selectResult[csf('order_quantity')]; $basis_order_qty=$selectResult[csf('plan_cut_qnty')];
			}

			$class_arr_qty=0; $class_arr_amt=0;

			if($selectResult[csf('emb_name')]==3)
			{
				$class_arr_qty=$washqty_arr; $class_arr_amt=$washamount_arr;
			}
			else
			{
				$class_arr_qty=$emblishmentqty_arr; $class_arr_amt=$emblishmentamount_arr;
			}

			if($cbo_level==1)
			{
				//Off As per COO Sir
				/*if($basis_plan_qty!=0 && $basis_order_qty!=0){ //Off As per COO Sir
					$req_qnty=($class_arr_qty[$po_id][$selectResult[csf('country_id')]][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$selectResult[csf('embe_cost_dtls_id')]]/$basis_plan_qty)*$basis_order_qty;
					$req_amount=($class_arr_amt[$po_id][$selectResult[csf('country_id')]][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$selectResult[csf('embe_cost_dtls_id')]]/$basis_plan_qty)*$basis_order_qty;
				}*/
				$req_qnty=$class_arr_qty[$po_id][$selectResult[csf('country_id')]][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$selectResult[csf('embe_cost_dtls_id')]];
				$req_amount=$class_arr_amt[$po_id][$selectResult[csf('country_id')]][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$selectResult[csf('embe_cost_dtls_id')]];
			}
			else
			{
				//Off As per COO Sir****
				
				/*if($basis_plan_qty!=0 && $basis_order_qty!=0){
					$req_qnty=($class_arr_qty[$po_id][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$selectResult[csf('embe_cost_dtls_id')]]/$basis_plan_qty)*$basis_order_qty;
					$req_amount=($class_arr_amt[$po_id][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$selectResult[csf('embe_cost_dtls_id')]]/$basis_plan_qty)*$basis_order_qty;
				}*/
				$req_qnty=$class_arr_qty[$po_id][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$selectResult[csf('embe_cost_dtls_id')]];
				$req_amount=$class_arr_amt[$po_id][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$selectResult[csf('embe_cost_dtls_id')]];
			}

			//echo $req_qnty.'=='.$cu_woq.'<br/>';
			$bal_woq=def_number_format($req_qnty-$cu_woq,2,"");
			$blaamount=def_number_format($req_amount-$cu_amount,2,"");
			...
			$preCostAmount=def_number_format($req_amount,5,"");//country_id
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['booking_dtls_id'][$po_id]=$booking_dtls_id;
			//$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['exchange_rate'][$po_id]=$selectResult[csf('exchange_rate')];
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['country_id'][$po_id]=$selectResult[csf('country_id')];
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['po_id'][$po_id]=$po_id;
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['po_number'][$po_id]=$selectResult[csf('po_number')];
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['item_number_id'][$po_id]=$selectResult[csf('item_number_id')];
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['embe_cost_dtls_id'][$po_id]=$selectResult[csf('embe_cost_dtls_id')];
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['emb_name'][$po_id]=$selectResult[csf('emb_name')];
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['emb_type'][$po_id]=$selectResult[csf('emb_type')];
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['color_number_id'][$po_id]=$selectResult[csf('color_number_id')];
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['order_quantity'][$po_id]=$selectResult[csf('order_quantity')];
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['plan_cut_qnty'][$po_id]=$selectResult[csf('plan_cut_qnty')];
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['description'][$po_id]=$description;
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['req_qnty'][$po_id]=$req_qnty;
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['cu_woq'][$po_id]=$cu_woq;
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['bal_woq'][$po_id]=$bal_woq;
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['bal_amount'][$po_id]=$blaamount;
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['wo_qnty_pre'][$po_id]=$wo_qnty_pre;
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['wo_qnty_new'][$po_id]=$wo_qnty_new;
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['rate'][$po_id]=$rate;
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['amount'][$po_id]=$amount;
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['preCostRate'][$po_id]=$preCostRate;
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['preCostAmount'][$po_id]=$preCostAmount;
			$job_level[$selectResult[csf('embe_cost_dtls_id')]][$selectResult[csf('color_number_id')]]['delv_date'][$po_id]=$delv_date;
			$job_exchane_rate_arr[$selectResult[csf('job_no')]]=$selectResult[csf('exchange_rate')];
		}
		//print_r($job_level);
		$i=1;
		if($cbo_level==1)
		{
			foreach ($nameArray as $selectResult)
			{
				...
				$embe_cost_dtls_id=$selectResult[csf('embe_cost_dtls_id')];
				//echo $po_id.'='.$country_id.'='.$selectResult[csf('item_number_id')].'='.$embe_cost_dtls_id.'='.$selectResult[csf('color_number_id')];
				$booking_dtls_id=$booking_data_array[$po_id][$country_id][$selectResult[csf('item_number_id')]][$embe_cost_dtls_id][$selectResult[csf('color_number_id')]][id];
				$updateid.=$booking_dtls_id;

				$wo_qnty_pre=$booking_data_array[$po_id][$country_id][$selectResult[csf('item_number_id')]][$embe_cost_dtls_id][$selectResult[csf('color_number_id')]][wo_qnty];
				//echo $wo_qnty_pre.'='.$po_id.'<br>';
				$wo_qnty_pre=def_number_format($wo_qnty_pre,2,"");

				$rate_pre=$booking_data_array[$po_id][$country_id][$selectResult[csf('item_number_id')]][$embe_cost_dtls_id][$selectResult[csf('color_number_id')]][rate];
				$rate_pre=def_number_format($rate_pre,5,"");

				$amount_pre=$booking_data_array[$po_id][$country_id][$selectResult[csf('item_number_id')]][$embe_cost_dtls_id][$selectResult[csf('color_number_id')]][amount];
				$amount_pre=def_number_format($amount_pre,5,"");

				$delivery_date=$booking_data_array[$po_id][$country_id][$selectResult[csf('item_number_id')]][$embe_cost_dtls_id][$selectResult[csf('color_number_id')]][delivery_date];
				$delivery_date=change_date_format($delivery_date,"dd-mm-yyyy","-");

				$description=$booking_data_array[$po_id][$country_id][$selectResult[csf('item_number_id')]][$embe_cost_dtls_id][$selectResult[csf('color_number_id')]][description];

				$cu_woq=$booking_data_array_cu[$po_id][$country_id][$selectResult[csf('item_number_id')]][$embe_cost_dtls_id][$selectResult[csf('emb_name')]][$selectResult[csf('emb_type')]][$selectResult[csf('color_number_id')]]['wo_qty'];
				//echo $cu_woq.'='.$po_id.'<br>';

				$cu_amt=$booking_data_array_cu[$po_id][$country_id][$selectResult[csf('item_number_id')]][$embe_cost_dtls_id][$selectResult[csf('emb_name')]][$selectResult[csf('emb_type')]][$selectResult[csf('color_number_id')]]['wo_amt'];
				$cu_woq=def_number_format($cu_woq-$wo_qnty_pre,2,"");
				//echo $cu_woq.'='.$wo_qnty_pre.'<br>';
				...
				/*$req_qnty=($class_arr_qty[$po_id][$selectResult[csf('country_id')]][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$embe_cost_dtls_id]/$basis_plan_qty)*$basis_order_qty;
				$req_amount=($class_arr_amt[$po_id][$selectResult[csf('country_id')]][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$embe_cost_dtls_id]/$basis_plan_qty)*$basis_order_qty;*/
				
				$req_qnty=$class_arr_qty[$po_id][$selectResult[csf('country_id')]][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$embe_cost_dtls_id];
				$req_amount=$class_arr_amt[$po_id][$selectResult[csf('country_id')]][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$embe_cost_dtls_id];


				$bal_woq=def_number_format($req_qnty-$cu_woq,2,"");
				$blaamount=def_number_format($req_amount-$cu_amt,2,"");
				//echo $req_qnty.'='.$req_amount.'<br>';
				...
				//if($calculation_basis==1) $calculation_cond="Pre Req Qty/Plan Cut*Order Qty"; else $calculation_cond="Pre Req Qty/Order Qty*Plan Cut";
				$job_exchane_rate=$job_exchane_rate_arr[$txt_job_no];
				//echo $currency_id.'='.$job_exchane_rate;
				if($booking_dtls_id=="")
				{
					if($currency_id==1) //Tk
					{
						$rate=$rate*$job_exchane_rate;
						$preCostRate=$preCostRate*$job_exchane_rate;
						$amount=$wo_qnty_new*$rate;
						$preCostAmount=$req_qnty*$rate;
					}
				}
				else
				{
				...
				$booking_previQty=number_format($booking_previ_QtyArr[$txt_job_no],2,'.','');
					$emblishment_req_qty=$emblishment_short_qty_arr[$txt_job_no];
					$wash_req_qty=$wash_short_qty_arr[$txt_job_no];
					$embl_req_qty=number_format(($emblishment_req_qty+$wash_req_qty),2,'.','');
					//echo $req_qnty.'='.$cu_woq.'<br> ';
				$tot_bal=$req_qnty-$cu_woq;
				if($req_qnty>0 && $is_short==2 && $tot_bal>0)
				{
					$fullbook++;
					?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="change_color('search<? echo $i; ?>','<? echo $bgcolor; ?>')">
                        <td width="40">T<? echo $i;?><input type="hidden" id="txtbookingdtlasid_<? echo $i;?>" value="<? echo $booking_dtls_id ;?>" style="width:40px" /></td>
                        <td width="90"><? echo $selectResult[csf('po_number')]; ?><input type="hidden" id="txtpoid_<? echo $i;?>" value="<? echo $selectResult[csf('po_id')];?>" /></td>
                        <td width="100"><? echo $country_name_library[$selectResult[csf('country_id')]];?><input type="hidden" id="txtcountryid_<? echo $i;?>" value="<? echo $selectResult[csf('country_id')];?>" /></td>
                        <td width="100"><? echo $garments_item[$selectResult[csf('item_number_id')]]; ?><input type="hidden" id="txtitemnumberid_<? echo $i;?>" value="<? echo $selectResult[csf('item_number_id')];?>" /></td>
                        <td width="90">
                        <input type="hidden" id="txtembcostid_<? echo $i;?>" value="<? echo $selectResult[csf('embe_cost_dtls_id')];?>" />
                        <input type="hidden" id="txtuomid_<? echo $i;?>" value="<? echo $uom_id;?>" />
                        <? echo $emblishment_name_array[$selectResult[csf('emb_name')]]; ?></td>
                        <td width="100"><? echo $emb_type; ?><input type="hidden" id="txtembtype_<? echo $i;?>" value="<? echo $selectResult[csf('emb_type')];?>" /></td>
                        <td width="90"><? echo $color_library[$selectResult[csf('color_number_id')]]; ?><input type="hidden" id="txtcolorid_<? echo $i;?>" value="<? echo $selectResult[csf('color_number_id')];?>"/></td>
                        <td width="60"><input type="text" style="width:100%; font-family:Verdana, Geneva, sans-serif; text-align:right; font-size:11px; background-color:<? echo $bgcolor; ?>" id="txtplancut_<? echo $i;?>" value="<? //if($calculation_basis==1){echo $selectResult[csf('order_quantity')];}else{ echo $selectResult[csf('plan_cut_qnty')];}
						echo $selectResult[csf('order_quantity')];
						?>" readonly/></td>
                        <td width="200"><input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; background-color:#FFC" id="description_<? echo $i;?>" value="<? echo $description;?>" onChange="copy_value(<? echo $i ?>,'description_')" /></td>
                        <td width="60" title="<? echo $calculation_cond;?>" ><input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; text-align:right; font-size:11px; background-color:<? echo $bgcolor; ?>" id="txtreqqty_<? echo $i;?>" value="<? echo def_number_format($req_qnty,2,"");?>" readonly/></td>
                        <td width="60" align="right"><input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtcuwoq_<? echo $i;?>" value="<? echo $cu_woq;?>" readonly /></td>
                        <td width="60" align="right"><input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtbalqty_<? echo $i;?>" value="<? echo $bal_woq; ?>" readonly /></td>
                        <td width="60" align="right">
                            <input type="hidden" id="txt_bal_qty_precost_<? echo $i;?>" value="<? echo def_number_format($wo_qnty_pre,5,"");?>" />
                        ...
                        <td width="50" align="right">
                            ...
                            <input type="hidden" id="txtrate_precost_<? echo $i;?>" value="<? echo def_number_format($preCostRate,5,"");?>" />
                        </td>
                        <td width="60" align="right">

                            <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtamount_<? echo $i;?>" reqamount="<? echo $blaamount; ?>"  value="<? echo $amount; ?>" readonly  />
                            <input type="hidden" id="txtamount_precost_<? echo $i;?>" value="<? echo def_number_format($preCostAmount,5,"");?>" />
                        </td>
                        <td align="right"><input type="text"   style="width:70%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtddate_<? echo $i;?>"  class="datepicker" value="<? echo $delv_date; ?>" readonly />
                        </td>
					</tr>
					<?
					$i++;
				}
				else if($req_qnty>0 && ($booking_previQty>=$embl_req_qty)  && $is_short==1) //for Is short Yes
				{
					$fullbook++;
					?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="change_color('search<? echo $i; ?>','<? echo $bgcolor; ?>')">
                        <td width="40">A<? echo $i;?><input type="hidden" id="txtbookingdtlasid_<? echo $i;?>" value="<? echo $booking_dtls_id ;?>" style="width:40px" /></td>
                        <td width="90"><? echo $selectResult[csf('po_number')]; ?><input type="hidden" id="txtpoid_<? echo $i;?>" value="<? echo $selectResult[csf('po_id')];?>" /></td>
                        <td width="100"><? echo $country_name_library[$selectResult[csf('country_id')]];?><input type="hidden" id="txtcountryid_<? echo $i;?>" value="<? echo $selectResult[csf('country_id')];?>" /></td>
                        <td width="100"><? echo $garments_item[$selectResult[csf('item_number_id')]]; ?><input type="hidden" id="txtitemnumberid_<? echo $i;?>" value="<? echo $selectResult[csf('item_number_id')];?>" /></td>
                        <td width="90">
				...
				else if($booking_dtls_id!="")
				{
					$fullbook=1;
					?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="change_color('search<? echo $i; ?>','<? echo $bgcolor; ?>')">
                        <td width="40">B<? echo $i;?><input type="hidden" id="txtbookingdtlasid_<? echo $i;?>" value=" <? echo $booking_dtls_id ;?>" style="width:40px" /></td>
                        <td width="90"><? echo $selectResult[csf('po_number')];?><input type="hidden" id="txtpoid_<? echo $i;?>" value="<? echo $selectResult[csf('po_id')];?>" /></td>
                        <td width="100"><? echo $country_name_library[$selectResult[csf('country_id')]];?><input type="hidden" id="txtcountryid_<? echo $i;?>" value="<? echo $selectResult[csf('country_id')];?>" /></td>
                        <td width="100"><p><? echo $garments_item[$selectResult[csf('item_number_id')]];?><input type="hidden" id="txtitemnumberid_<? echo $i;?>" value="<? echo $selectResult[csf('item_number_id')];?>" /></p></td>
                        <td width="90"><input type="hidden" id="txtembcostid_<? echo $i;?>" value="<? echo $selectResult[csf('embe_cost_dtls_id')];?>" />
                        <input type="hidden" id="txtuomid_<? echo $i;?>" value="<? echo $uom_id;?>" />
                        <? echo $emblishment_name_array[$selectResult[csf('emb_name')]];?></td>
                        <td width="100"><? echo $emb_type; ?><input type="hidden" id="txtembtype_<? echo $i;?>" value="<? echo $selectResult[csf('emb_type')];?>" /></td>
                        <td width="90"><p><? echo $color_library[$selectResult[csf('color_number_id')]];?><input type="hidden" id="txtcolorid_<? echo $i;?>" value="<? echo $selectResult[csf('color_number_id')];?>"/></p></td>
                        <td width="60"><input type="text" style="width:100%; font-family:Verdana, Geneva, sans-serif; text-align:right; font-size:11px; background-color:<? echo $bgcolor; ?>" id="txtplancut_<? echo $i;?>" value="<? 
						//if($calculation_basis==1){echo $selectResult[csf('order_quantity')];}else{ echo $selectResult[csf('plan_cut_qnty')];}
						echo  $selectResult[csf('order_quantity')];
						?>" readonly/></td>
                        <td width="200"><input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; background-color:#FFC" id="description_<? echo $i;?>" value="<? echo $description;?>" onChange="copy_value(<? echo $i ?>,'description_')" /></td>
                        <td width="60" title="<? echo $calculation_cond;?>"><input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; text-align:right; font-size:11px; background-color:<? echo $bgcolor; ?>" id="txtreqqty_<? echo $i;?>" value="<? echo def_number_format($req_qnty,2,"");?>" readonly/></td>
                        ...
                        <td width="" align="right">
                            <input type="text" style="width:70%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtddate_<? echo $i;?>"  class="datepicker" value="<? echo $delv_date; ?>" readonly />
                        </td>
					</tr>
					<?
					$i++;
				...
				/*else
				{
					echo "<b style='color:#F00'> Full Book Found</b>";die;
				}*/	
			}
		}
		$i=1;
		if($cbo_level==2)
		{
			...
			foreach($job_level as $color_idarray)
			{
				foreach($color_idarray as $colorid)
				{
					...
					$embe_cost_dtls_id=implode(",",array_unique($colorid['embe_cost_dtls_id']));
					...
					$country_id=rtrim($job_country_arr[$color_number_id]['country_id'],',');
					$country_id=implode(",",array_unique(explode(",",$country_id)));
					$country_ids=array_unique(explode(",",$country_id));
					$country_name="";
					...
					$blaamount=def_number_format(array_sum($colorid['bal_amount']),2,"");
					...
					$amount=def_number_format(array_sum($colorid['amount']),2,"");
					...
					if($cbo_booking_natu==1) $emb_type=$emblishment_print_type[$emb_typeid];
					if($cbo_booking_natu==2) $emb_type=$emblishment_embroy_type[$emb_typeid];
					if($cbo_booking_natu==3) $emb_type=$emblishment_wash_type[$emb_typeid];
					if($cbo_booking_natu==4) $emb_type=$emblishment_spwork_type[$emb_typeid];
					...
					//echo $req_qnty.'='.$cu_woq.'<br> ';
					$tot_bal=$req_qnty-$cu_woq;
				// echo $booking_previQty.'='.$embl_req_qty.',';
				//echo $req_qnty.'='.$booking_previQty.'='.$embl_req_qty.'<br>';
				//if($req_qnty>0 && ($booking_previQty>=$embl_req_qty)  && $is_short==1)
					...
					if($req_qnty>0 && $is_short==2 && $tot_bal>0)
					{
						...
						?>
						<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="change_color('search<? echo $i; ?>','<? echo $bgcolor; ?>')">
                            <td width="40"><? echo $i;?><input type="hidden" id="txtbookingdtlasid_<? echo $i;?>" value=" <? echo $booking_dtls_id; ?>" style="width:40px"/></td>

                            <td width="90"><A href="#" onClick="setdata('<? echo $po_number;?>' )">View</A><input type="hidden" id="txtpoid_<? echo $i;?>" value="<? echo $po_id;?>" /></td>
                            <td width="100"><p><? echo $country_name;?> </p><input type="hidden" id="txtcountryid_<? echo $i;?>" value="<? echo $country_id;?>" /></td>
                            <td width="100"><? echo $garments_item[$item_number_id];?><input type="hidden" id="txtitemnumberid_<? echo $i;?>" value="<? echo $item_number_id;?>" /></td>
                            <td width="90"><input type="hidden" id="txtembcostid_<? echo $i;?>" value="<? echo $embe_cost_dtls_id;?>" />
                            <input type="hidden" id="txtuomid_<? echo $i;?>" value="<? echo $uom_id;?>" />
                            <? echo $emblishment_name_array[$emb_name];?></td>
                            <td width="100"><? echo $emb_type; ?><input type="hidden" id="txtembtype_<? echo $i;?>" value="<? echo $emb_typeid;?>" /></td>

                            <td width="90"><? echo $color_library[$color_number_id];?><input type="hidden" id="txtcolorid_<? echo $i;?>" value="<? echo $color_number_id;?>"/></td>
                            <td width="60"><input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; text-align:right; font-size:11px; background-color:<? echo $bgcolor; ?>" id="txtplancut_<? echo $i;?>" value="<? //if($calculation_basis==1){echo $order_quantity;}else{ echo $plan_cut_qnty;}
							echo $order_quantity;
							?>" readonly/></td>
                            <td width="200"><input type="text" style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; background-color:#FFC" id="description_<? echo $i;?>" value="<? echo $description;?>" onChange="copy_value(<? echo $i; ?>,'description_')" /></td>
                            <td width="60" title="<? echo $calculation_cond;?>"><input type="text" style="width:100%; font-family:Verdana, Geneva, sans-serif; text-align:right; font-size:11px; background-color:<? echo $bgcolor; ?>" id="txtreqqty_<? echo $i;?>" value="<? echo def_number_format($req_qnty,2,"")?>" readonly/></td>
                            <td width="60" align="right"><input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtcuwoq_<? echo $i;?>" value="<? echo $cu_woq;?>" readonly /></td>
                            <td width="60" align="right"><input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtbalqty_<? echo $i;?>" value="<? echo $bal_woq; ?>" readonly /></td>
                            <td width="60" align="right"><input type="hidden"id="txt_bal_qty_precost_<? echo $i;?>" value="<? echo $wo_qnty_pre;?>" />
                                <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtwoq_<? echo $i;?>" value="<? echo $wo_qnty_new;?>" onChange="calculate_amount(<? echo $i; ?>);calculate_amount2(<? echo $i; ?>);copy_value(<? echo $i; ?>,'txtwoq_')" />
                            </td>
                            <td width="50" align="right">
                                <input type="text" style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtrate_<? echo $i;?>" value="<? if($rate>0){ echo $rate;} else {echo 0;}?>" onChange="calculate_amount(<? echo $i; ?>)" />
                                <input type="hidden" id="txtrate_precost_<? echo $i;?>" value="<? echo $preCostRate;?>" />
                            </td>
                            <td width="60" align="right">
                                <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtamount_<? echo $i;?>" reqamount="<? echo $blaamount; ?>"  value="<? echo $amount; ?>"  readonly  />
                                <input type="hidden" id="txtamount_precost_<? echo $i;?>" value="<? echo $preCostAmount;?>" />
                            </td>
                            <td width="" align="right">
                                <input type="text" style="width:70%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtddate_<? echo $i;?>"  class="datepicker" value="<? echo $delv_date; ?>"  readonly  />
                            </td>
					...
					else if($req_qnty>0 && ($booking_previQty>=$embl_req_qty)  && $is_short==1) //for Is short Yes
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="change_color('search<? echo $i; ?>','<? echo $bgcolor; ?>')">
                            ...
                            <td width="90"><A href="#" onClick="setdata('<? echo $po_number;?>' )">View</A><input type="hidden" id="txtpoid_<? echo $i;?>" value="<? echo $po_id;?>" /></td>
                            <td width="100"><p><? echo $country_name;?> </p><input type="hidden" id="txtcountryid_<? echo $i;?>" value="<? echo $country_id;?>" /></td>
                            <td width="100"><p><? echo $garments_item[$item_number_id];?><input type="hidden" id="txtitemnumberid_<? echo $i;?>" value="<? echo $item_number_id;?>" /></p></td>
                            <td width="90"><input type="hidden" id="txtembcostid_<? echo $i;?>" value="<? echo $embe_cost_dtls_id;?>" />
                            <input type="hidden" id="txtuomid_<? echo $i;?>" value="<? echo $uom_id;?>" />
                            <? echo $emblishment_name_array[$emb_name];?></td>
                            <td width="100"><? echo $emb_type; ?><input type="hidden" id="txtembtype_<? echo $i;?>" value="<? echo $emb_typeid;?>" /></td>

                            <td width="90"><p><? echo $color_library[$color_number_id];?><input type="hidden" id="txtcolorid_<? echo $i;?>" value="<? echo $color_number_id;?>"/></p></td>
                            <td width="60"><input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; text-align:right; font-size:11px; background-color:<? echo $bgcolor; ?>" id="txtplancut_<? echo $i;?>" value="<? //if($calculation_basis==1){echo $order_quantity;}else{ echo $plan_cut_qnty;}
							echo $order_quantity;
							?>" readonly/></td>
                            <td width="200"><input type="text" style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; background-color:#FFC" id="description_<? echo $i;?>" value="<? echo $description;?>" onChange="copy_value(<? echo $i; ?>,'description_')" /></td>
					...
					else if ($booking_dtls_id!="")
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="change_color('search<? echo $i; ?>','<? echo $bgcolor; ?>')">
                            ...
                            <td width="90"><A href="#" onClick="setdata('<? echo $po_number;?>' )">View</A><input type="hidden" id="txtpoid_<? echo $i;?>" value="<? echo $po_id;?>" /></td>
                            <td width="100"><p><? echo $country_name;?></p><input type="hidden" id="txtcountryid_<? echo $i;?>" value="<? echo $country_id;?>" /></td>
                            <td width="100"><p><? echo $garments_item[$item_number_id];?><input type="hidden" id="txtitemnumberid_<? echo $i;?>" value="<? echo $item_number_id;?>" readonly /></p></td>
                            <td width="90" ><input type="hidden" id="txtembcostid_<? echo $i;?>" value="<? echo $embe_cost_dtls_id;?>" />
                            <input type="hidden" id="txtuomid_<? echo $i;?>" value="<? echo $uom_id;?>" />
                             <? echo $emblishment_name_array[$emb_name];?></td>
                            <td width="100"><? echo $emb_type; ?><input type="hidden" id="txtembtype_<? echo $i;?>" value="<? echo $emb_typeid;?>" /></td>
                            ...
                            <td width="90"><p><? echo $color_library[$color_number_id];?><input type="hidden" id="txtcolorid_<? echo $i;?>" value="<? echo $color_number_id;?>"/></p></td>
                            <td width="60"><input type="text" style="width:100%; font-family:Verdana, Geneva, sans-serif; text-align:right; font-size:11px; background-color:<? echo $bgcolor; ?>" id="txtplancut_<? echo $i;?>" value="<? echo $order_quantity;//if($calculation_basis==1){echo $order_quantity;}else{ echo $plan_cut_qnty;}?>" readonly/></td>
                            <td width="200"><input type="text" style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; background-color:#FFC" id="description_<? echo $i;?>" value="<? echo $description;?>" onChange="copy_value(<? echo $i; ?>,'description_')" /></td>
                            <td width="60" title="<? echo $calculation_cond.',OrdQty='.$colorid['order_quantity'];?>"><input type="text" style="width:100%; font-family:Verdana, Geneva, sans-serif; text-align:right; font-size:11px; background-color:<? echo $bgcolor; ?>" id="txtreqqty_<? echo $i;?>" value="<? echo def_number_format($req_qnty,2,"")?>" readonly/></td>
                            <td width="60" align="right"><input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtcuwoq_<? echo $i;?>" value="<? echo $cu_woq;?>" readonly /></td>
                            <td width="60" align="right"><input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtbalqty_<? echo $i;?>" value="<? echo $bal_woq; ?>" readonly /></td>
                            <td width="60" align="right">
                                <input type="hidden" id="txt_bal_qty_precost_<? echo $i;?>" value="<? echo $wo_qnty_pre;?>" />
                                <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtwoq_<? echo $i;?>" value="<? echo $wo_qnty_new;?>" onChange="calculate_amount(<? echo $i; ?>);calculate_amount2(<? echo $i; ?>);copy_value(<? echo $i; ?>,'txtwoq_')" />
                            </td>
                            <td width="50" align="right">
                                <input type="text" style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtrate_<? echo $i;?>" value="<? if($rate>0){ echo $rate;} else {echo 0;}?>" onChange="calculate_amount(<? echo $i; ?>)" />
		 ...
		 if($fullbook==0)
		{
			$value ="Full Book Found/Balance Qty Not found"; $display="";
			echo "<b style='color:#F00'> $value</b>";die;
		}
		else
		{
			$value =""; $display="none";
    ...
    <table width="1347" class="rpt_table" border="0" rules="all">
        <tfoot>
            <tr>
                <th  width="40">&nbsp;</th>
                <th width="90">&nbsp;</th>
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$new_booking_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'ESB', date("Y",time()), 5, "select booking_no_prefix, booking_no_prefix_num from wo_booking_mst where company_id=$cbo_company_name and booking_type=6 and entry_form=161 $yearCond=".date('Y',time())." order by id desc ", "booking_no_prefix", "booking_no_prefix_num" ));

		$id=return_next_id( "id", "wo_booking_mst", 1 ) ;
		$field_array="id, booking_type, is_short, booking_no_prefix, booking_no_prefix_num, booking_no, entry_form, company_id, buyer_id, job_no, currency_id, item_category, exchange_rate, pay_mode, source, booking_date, delivery_date, supplier_id, attention, tenor, ready_to_approved, inserted_by, insert_date, calculation_basis, cbo_level, status_active, is_deleted";
		 $data_array ="(".$id.",6,".$cbo_is_short.",'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',161,".$cbo_company_name.",".$cbo_buyer_name.",".$txt_job_no.",".$cbo_currency.",25,".$txt_exchange_rate.",".$cbo_pay_mode.",".$cbo_source.",".$txt_booking_date.",".$txt_delivery_date.",".$hidden_supplier_id.",".$txt_attention.",".$txt_tenor.",".$cbo_ready_to_approved.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$calculation_basis.",".$cbo_level.",1,0)";
	...
	else if ($operation==1)   // Update Here
	{
		...
		$field_array="buyer_id*job_no*currency_id*item_category*exchange_rate*pay_mode*source*booking_date*delivery_date*supplier_id*attention*tenor*ready_to_approved*updated_by*update_date*calculation_basis*cbo_level";
		 $data_array ="".$cbo_buyer_name."*".$txt_job_no."*".$cbo_currency."*25*".$txt_exchange_rate."*".$cbo_pay_mode."*".$cbo_source."*".$txt_booking_date."*".$txt_delivery_date."*".$hidden_supplier_id."*".$txt_attention."*".$txt_tenor."*".$cbo_ready_to_approved."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$calculation_basis."*".$cbo_level."";
		...
		if($db_type==0)
		{
			if($rID ){
				mysql_query("COMMIT");
				echo "1**".str_replace("'","",$txt_booking_no)."**".$id;
			}
			else{
				mysql_query("ROLLBACK");
...
if($action=="save_update_delete_dtls")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		if($row_msg>0)
		{
		 ...
		 $field_array1="id, booking_mst_id, pre_cost_fabric_cost_dtls_id, entry_form_id, po_break_down_id, job_no, booking_no, booking_type, is_short, gmt_item, emblishment_name, gmts_color_id, wo_qnty, rate, amount, delivery_date, description, country_id_string, uom, inserted_by, insert_date, status_active, is_deleted";
		 for ($i=1;$i<=$total_row;$i++)
		 {
			 ...
			 $txtcountryid="txtcountryid_".$i;
			 $uom_id="txtuomid_".$i;
			 if ($i!=1) $data_array1 .=",";
			 $data_array1 .="(".$id_dtls.",".$update_id.",".$$txtembcostid.",161,".$$txtpoid.",".$txt_job_no.",".$txt_booking_no.",6,".$cbo_is_short.",".$cbo_gmt_item.",".$cbo_booking_natu.",".$$txtcolorid.",".$$txtwoq.",".$$txtrate.",".$$txtamount.",".$$txtddate.",".$$description.",".$$txtcountryid.",".$$uom_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			 $id_dtls=$id_dtls+1;
		 }
	...
	else if ($operation==1)  // Insert Here
	{
		...
		$field_array_up1="pre_cost_fabric_cost_dtls_id*po_break_down_id*job_no*booking_type*gmt_item*emblishment_name*gmts_color_id*wo_qnty*rate*amount*delivery_date*description*country_id_string*uom*updated_by*update_date*status_active*is_deleted";
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			...
			if(str_replace("'",'',$$txtbookingdtlasid)!="")
			{
				...
				$data_array_up1[str_replace("'",'',$$txtbookingdtlasid)] =explode("*",("".$$txtembcostid."*".$$txtpoid."*".$txt_job_no."*6*".$cbo_gmt_item."*".$cbo_booking_natu."*".$$txtcolorid."*".$$txtwoq."*".$$txtrate."*".$$txtamount."*".$$txtddate."*".$$description."*".$$txtcountryid."*".$$uom_id."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*1*0"));
			}
			if(str_replace("'",'',$$txtbookingdtlasid)=="")
			{
...
if($action=="save_update_delete_dtls_job_level")
{
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			 ...
			 foreach($jason_data->$embcostid->$colorid->po_id as $poId){
				 ...
				 if(str_replace("'","",$cbo_is_short)==2)
				 {
					 ...
					 if($woq>0) $wqQty=($jason_data->$embcostid->$colorid->req_qnty->$poId/$reqqnty)*$woq; else $wqQty=0;
				 }
				 else
				 {
					$wqQty=$woq;
				 ...
				 $data_array1 .="(".$id_dtls.",".$update_id.",".$$txtembcostid.",161,".$poId.",".$txt_job_no.",".$txt_booking_no.",6,".$cbo_is_short.",".$cbo_gmt_item.",".$cbo_booking_natu.",".$$txtcolorid.",".$wqQty.",".$rate.",".$amount.",".$$txtddate.",".$$description.",".$$txtcountryid.",".$$uom_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
	...
	else if ($operation==1)  // Insert Here
	{
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			...
			if(str_replace("'",'',$$txtbookingdtlasid)!="")
			{
				foreach($jason_data->$embcostid->$colorid->po_id as $poId){
					...
					$data_array_up1[str_replace("'",'',$jason_data->$embcostid->$colorid->booking_dtls_id->$poId)] =explode("*",("".$$txtembcostid."*".$poId."*".$txt_job_no."*6*".$cbo_gmt_item."*".$cbo_booking_natu."*".$$txtcolorid."*".$wqQty."*".$rate."*".$amount."*".$$txtddate."*".$$description."*".$$txtcountryid."*".$$uom_id."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*1*0"));
					}
			}
		...
		if($data_array1 !="")
		 ...
		 //echo "10**";check_table_status( $_SESSION['menu_id'],0);

		// print_r($data_array_up1);
	...
	else if ($operation==2)  // Insert Here
	{
		...
		for ($i=1;$i<=$total_row;$i++){
			 ...
			 $txtbookingdtlasid="txtbookingdtlasid_".$i;
			// $rID=execute_query( "delete from wo_booking_dtls where booking_no =$txt_booking_no and gmt_item=$cbo_gmt_item and emblishment_name=$cbo_booking_natu  and pre_cost_fabric_cost_dtls_id in (".str_replace("'","",$$txtembcostid).")",0);
			  $rID=execute_query( "update wo_booking_dtls  set status_active=0,is_deleted=1,delete_cause='$delete_cause',updated_by=".$_SESSION['logic_erp']['user_id'].",update_date='".$pc_date_time."'  where booking_no =$txt_booking_no and gmt_item=$cbo_gmt_item and emblishment_name=$cbo_booking_natu and id in((".str_replace("'","",$$txtbookingdtlasid)."))  and pre_cost_fabric_cost_dtls_id in (".str_replace("'","",$$txtembcostid).")",0);
...
<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            	<table width="970" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
        			...
        			<tr class="general">
                        ...
                        <?
                        echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", '', "load_drop_down( 'print_booking_urmi_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
                        ?>
                        </td>
...
if ($action=="create_booking_search_list_view")
{
   ...
   if($data[6]==2)
	...
	if($data[7]==0)
		  $sql= "select a.id,a.pay_mode, a.booking_no_prefix_num, a.booking_no,a.booking_date,a.pay_mode,company_id,a.buyer_id,a.job_no,b.po_break_down_id,b.gmt_item,c.emb_name,a.supplier_id,a.is_approved,a.ready_to_approved,d.grouping from wo_booking_mst a, wo_booking_dtls b,  wo_pre_cost_embe_cost_dtls c,wo_po_break_down d where $company $buyer $booking_date $booking_cond and d.job_no_mst=b.job_no and d.id=b.po_break_down_id and  a.job_no=b.job_no and  a.job_no=c.job_no and a.booking_no=b.booking_no and b.pre_cost_fabric_cost_dtls_id=c.id  and a.booking_type=6 and  a.status_active=1  and 	a.is_deleted=0 and  b.status_active=1  and 	b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  $job_cond $ref_cond group by a.id, a.pay_mode,a.booking_no_prefix_num, a.booking_no,a.booking_date,company_id,a.buyer_id,a.job_no,b.po_break_down_id,b.gmt_item,c.emb_name,a.supplier_id,a.is_approved,a.ready_to_approved,d.grouping order by a.id DESC";
	else
		$sql= "select a.id,a.pay_mode, a.booking_no_prefix_num, a.booking_no,a.booking_date,company_id,a.buyer_id,a.job_no,c.emb_name,a.supplier_id,a.is_approved,a.ready_to_approved,1 as po_break_down_id,2 as gmt_item,'' as grouping  from wo_booking_mst a,  wo_pre_cost_embe_cost_dtls c where $company $buyer $booking_date $booking_cond and NOT EXISTS (SELECT booking_no FROM wo_booking_dtls b WHERE a.booking_no=b.booking_no and b.status_active=1) and  a.job_no=c.job_no  and a.booking_type=6  and  a.status_active=1  and 	a.is_deleted=0  and c.status_active=1 and c.is_deleted=0 $job_cond group by a.id,a.pay_mode, a.booking_no_prefix_num, a.booking_no,a.booking_date,company_id,a.buyer_id,a.job_no, c.emb_name,a.supplier_id,a.is_approved,a.ready_to_approved order by a.id DESC";
	//	echo $data[7].'='.$sql;
...
if($action=="show_trim_booking_report"){
   ...
   $sql_booking=sql_select("select a.company_id, a.supplier_id, a.job_no, a.booking_date, a.delivery_date, a.buyer_id, a.currency_id, a.pay_mode, a.source, a.exchange_rate from wo_booking_mst a where a.is_deleted=0 and a.status_active=1 and a.booking_no='$txt_booking_no' and a.booking_type=6 and entry_form=161");
   ...
   $color_id=return_library_array("select id,color_name from lib_color where status_active =1 and is_deleted=0", "id", "color_name");
   ...
   <div style="width:1100px;" align="center">
	...
	<table width="1100" cellspacing="0" align="center" border="0" >
	   ...
	   <tr>
		...
		<tr>
		   <td style="font-size:20" width="130" >Supplier Name </td> <td style="font-size:20"  width="170">: &nbsp;<?
		   if($cbo_pay_mode==3 || $cbo_pay_mode==5)
		   {
			echo $company_library[$cbo_supplier_name];
		   }
		   else echo $supplier_name[$cbo_supplier_name];
		   ?></td>
...
<table width="1100" cellspacing="0" align="center" border="1" rules="all" class="rpt_table" >
	 <thead bgcolor="#dddddd" align="center">
		...
		<th width="80">Color</th>
		<th width="80"> Order Qty Pcs<?  //if($calculation_basis==1) echo "Order Qty Pcs";else "Plan Cut(PCS)";?></th>
	 ...
	 <?
				...
				$sql="select
				   a.total_set_qnty,b.costing_per,c.id as wo_pre_cost_embe_cost_dtls,c.job_no,c.emb_name,c.emb_type,c.cons_dzn_gmts,c.rate,d.id as po_id,d.po_number,d.plan_cut,e.item_number_id,e.color_number_id,e.country_id,sum(e.plan_cut_qnty) as plan_cut_qnty,sum(e.order_quantity) as order_quantity,f.wo_qnty as wo_qnty,f.rate as frate,f.amount as famount,f.description
				   ...
				   group by   f.wo_qnty,f.rate,f.amount,e.item_number_id,e.color_number_id,e.country_id,d.id,d.po_number,d.plan_cut,c.id,c.job_no,c.emb_name,c.emb_type,c.cons_dzn_gmts,c.rate,b.costing_per,a.total_set_qnty,f.description order by d.id,e.color_number_id";
				   //echo $sql;
				   $condition= new condition();
				   $condition->company_name("=$cbo_company_name");
				   if(str_replace("'","",$txt_job_no) !=''){
				   ...
				   foreach ($nameArray as $selectResult)
				   {
					   ...
					   //echo $calculation_basis.'sdd'.$selectResult[csf('order_quantity')];
					   //$selectResult[csf('plan_cut_qnty')])*$selectResult[csf('order_quantity')];

					   if($selectResult[csf('emb_name')]==3) {
						   //$wash_qty=$washqty_arr[$selectResult[csf('po_id')]][$selectResult[csf('wo_pre_cost_embe_cost_dtls')]];
							if($cbo_level==1)
						   {
							   $wash_qty=$washqty_arr[$selectResult[csf('po_id')]][$selectResult[csf('country_id')]][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$selectResult[csf('wo_pre_cost_embe_cost_dtls')]];
						   }
						   else
						   {
							   $wash_qty=$washqty_arr[$selectResult[csf('po_id')]][$selectResult[csf('country_id')]][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$selectResult[csf('wo_pre_cost_embe_cost_dtls')]];
						...
						if($cbo_level==1)
						   {
						   $emb_req_qty=$emblishmentqty_arr[$selectResult[csf('po_id')]][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$selectResult[csf('wo_pre_cost_embe_cost_dtls')]];

						   $req_qnty=$emb_req_qty+$wash_qty;
						   }
						   else
						   {
							   $emb_req_qty=$emblishmentqty_arr[$selectResult[csf('po_id')]][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$selectResult[csf('wo_pre_cost_embe_cost_dtls')]];
							   $req_qnty=($emblishmentqty_arr[$selectResult[csf('po_id')]][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$selectResult[csf('wo_pre_cost_embe_cost_dtls')]]+$wash_qty);
						   }
					   ...
					   $amount=$selectResult[csf('famount')];
					   $amount=def_number_format($amount,5,"");
					   $plan_cut_qnty=$selectResult[csf('plan_cut_qnty')];
					   $order_quantity=$selectResult[csf('order_quantity')];
					   ...
					   <tr <? echo $bgcolor; ?>>
						   ...
						   <td width="80" align="right" title="PlanCut=<? echo $selectResult[csf('plan_cut_qnty')];?>"><p><? echo $selectResult[csf('order_quantity')]; ?> </p></td>
						   <td width="100" align="right" title="<? echo $calculation_cond;?>"><p><? echo number_format($plan_cut_qnty/12,2); ?></p> </td>
						   ...
						   <td width="70" align="right" title="Rate=<? echo $rate;?>"><p> <? echo number_format($rate,5); ?> </p></td>
...
$order_arr = return_library_array( "select id, po_number from wo_po_break_down",'id','po_number');
$job_no_qty_arr = return_library_array( "select job_no, job_quantity from wo_po_details_master",'job_no','job_quantity');
$internal_ref_arr=return_library_array( "select job_no_mst, grouping from wo_po_break_down",'job_no_mst','grouping');
 //$season=return_field_value("season","wo_po_details_master","company_name='$cbo_company_name' and job_no='$txt_job_no' and is_deleted=0 and status_active=1");
...
<div style="width:1100px;" align="center">
 ...
 <table width="100%" cellpadding="0" cellspacing="0"  >
        ...
        <tr>
         ...
         <tr>
			...
			if($cbo_pay_mode==3 || $cbo_pay_mode==5)
			{
			 ...
			 //echo $supplier_name[$cbo_supplier_name]; ?></td>
            <td style="font-size:20">Work Order No</td> <td style="font-size:20">: &nbsp;<? echo $txt_booking_no; //change_date_format($txt_booking_date); ?></td>
            <td style="font-size:20">Work Order Date  </td> <td style="font-size:20">: &nbsp;<? echo change_date_format($txt_booking_date); ?></td>
        </tr>
         <tr>
        ...
        <tr>
			...
			<td style="font-size:20">: &nbsp;<? echo $txt_season;?> </td>			
			<td colspan="2"></td>
			<td style="font-size:20">Internal Ref</td>
			<td style="font-size:20">: &nbsp;<? echo $internal_ref_arr[$txt_job_no];?> </td>
		</tr>
	 ...
	 <table width="1100" cellspacing="0" align="center" border="1" rules="all" class="rpt_table" >
		  <thead bgcolor="#dddddd" align="center">
			...
			<!-- <th width="80">Order Qty (PCS)</th>-->
			 ...
			 <th width="80">WOQ/<? echo $costing_per_dzn;?></th>
			 <th width="70">Rate/<? echo $costing_per_dzn;?></th>
...
<div style="width:1100px;" align="center">
 ...
 <table width="100%" cellpadding="0" cellspacing="0" style="table-layout: fixed"  >
            ...
            <td colspan="6" align="center">
				<?
					...
					?>
						...
						<? if($country_arr[$result[csf('country_id')]]!=""){ echo "Country: ".$country_name_arr[$result[csf('country_id')]]; }?><br>
        ...
        <tr>
         ...
         <tr>
            <td style="font-size:24" width="130" ><b>Supplier Name </b></td> <td style="font-size:24"  width="170">: &nbsp;<b><? 
			if($cbo_pay_mode==3 || $cbo_pay_mode==5)
				{
				 echo $company_library[$cbo_supplier_name];
				}
				else echo $supplier_name[$cbo_supplier_name];
			//echo $supplier_name[$cbo_supplier_name]; ?></b></td>
...
/* $costing_per=return_field_value("costing_per", "wo_pre_cost_mst", "job_no='$txt_job_no'");
	...
	//echo $costPerQty.'DD';
	if($costPerQty>1){
			$costing_per_dzn=($costPerQty/12)." Dzn";
		}else{
			$costing_per_dzn=($costPerQty/1)." Pcs";
		}
		//echo $costing_per_dzn.'DD';
...
<?php */?>
      ...
      <?
					 $sql="select
					a.costing_per,b.id as wo_pre_cost_embe_cost_dtls,b.job_no,b.emb_name,b.emb_type,b.cons_dzn_gmts,b.rate,c.country_id_string,
					...
					c.gmt_item,c.gmts_color_id,c.description ,c.country_id_string order by b.emb_name";
					...
					foreach ($nameArray as $selectResult)
					{
						if ($i%2==0)
							...
							$bgcolor="#FFFFFF";
						$country_strings=array_unique(explode(",",$selectResult[csf('country_id_string')]));
						$country_names='';
						foreach($country_strings as $c_id)
						{
							if($c_id!='')
							{
							if($country_names=='') $country_names=$country_name_arr[$c_id];else $country_names.=','.$country_name_arr[$c_id];
							}
						}
						$rate=$selectResult[csf('frate')];
						$description=$selectResult[csf('description')];
						...
						if (!in_array($selectResult[csf('emb_name')],$order_id_array))
						{
							if($k!=1)
							{
								...
								<tr>
                                    <td colspan="7" align="right"><b>Sub Total</b></td>
                                   <?php /*?> <td   align="right"><b><? echo number_format($po_qty_tot,0); ?></b> </td> <?php */?>
                                    <td   align="right"><b><? echo number_format($wo_qnty,2); ?></b> </td>
                                    <td  align="right">&nbsp;</td>
                                    <td  align="right"><b><? echo number_format($amount_tot,2);?> </b></td>
								</tr>
...
?><?php */?>
 ...
 ?>
                      ...
                      <tr>
                          <td  align="right" colspan="7"><strong>Grand Total</strong></td>
                         <?php /*?> <td  align="right"><b><? echo number_format($po_qty_tot_t,0); ?> </b></td><?php */?>
                          <td  align="right"><b><? echo number_format($wo_qnty_r,2);//]$emblishment_name_array[; ?> </b></td>
                          <td  align="right">&nbsp;</td>
                          <td  align="right"><b><? echo number_format($amount_tot_t,2);?> </b></td>
                      </tr>
