            	...
            	<table  cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
        			...
        			<tr>
                        ...
                        <input type="hidden" id="garments_nature" value="<? echo $garments_nature; ?>">
							<?
								echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", '',"load_drop_down( 'short_trims_booking_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
							?>
                    </td>
                   	<td id="buyer_td">
                     <?
...
?>
        ...
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1220" class="rpt_table" >
            ...
            <div style="width:1220px; overflow-y:scroll; max-height:350px;" id="buyer_list_view" align="center">
...
CASE e.costing_per WHEN 1 THEN round(((d.cons/12)*b.po_quantity)/cc.conversion_factor,4) WHEN 2 THEN round(((d.cons/1)*b.po_quantity)/cc.conversion_factor,4)  WHEN 3 THEN round(((d.cons/24)*b.po_quantity)/cc.conversion_factor,4) WHEN 4 THEN round(((d.cons/36)*b.po_quantity)/cc.conversion_factor,4) WHEN 5 THEN round(((d.cons/48)*b.po_quantity)/cc.conversion_factor,4) ELSE 0 END as req_qnty,cc.order_uom as cons_uom,IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0)  as cu_woq, CASE e.costing_per WHEN 1 THEN round((((d.cons/12)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 2 THEN round((((d.cons/1)*b.po_quantity)/cc.conversion_factor) - IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4)  WHEN 3 THEN round((((d.cons/24)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 4 THEN round((((d.cons/36)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 5 THEN round((((d.cons/48)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) ELSE 0 END as bal_woq, round((c.rate*cc.conversion_factor),8) as rate from wo_po_details_master a, wo_po_break_down b ,wo_pre_cost_mst e,wo_pre_cost_trim_cost_dtls c ,lib_item_group cc, wo_pre_cost_trim_co_cons_dtls d left join wo_booking_dtls f on f.job_no=d.job_no and f.pre_cost_fabric_cost_dtls_id=d.wo_pre_cost_trim_cost_dtls_id and f.po_break_down_id=d.po_break_down_id and f.booking_type=2   where a.job_no=b.job_no_mst and  a.job_no=c.job_no and a.job_no=e.job_no and  a.job_no=d.job_no   and c.id=d.wo_pre_cost_trim_cost_dtls_id and b.id=d.po_break_down_id and cc.id=c.trim_group   and a.company_name=$company_id $txt_job_no_cond $cbo_buyer_name_cond and a.garments_nature=$garments_nature  group by d.id order by b.id) m  where  company_name=$company_id $txt_job_no_cond1 $cbo_buyer_name_cond1  and garments_nature=$garments_nature   and bal_woq >0 order by trim_group";//and b.pub_shipment_date between '$start_date' and '$end_date'*/
					/* $sql_po="select d.id,d.po_number,d.file_no,d.grouping, d.plan_cut from wo_po_details_master a,wo_pre_cost_mst b,wo_po_break_down d where a.job_no=d.job_no_mst and				a.company_name=$company_id $txt_job_no_cond $cbo_buyer_name_cond ";
					 $po_nameArray=sql_select( $sql_po );
					foreach($po_nameArray as $row)
					{
						$po_data_arr[$row[csf('id')]]['po_number']=$row[csf('po_number')];
						$po_data_arr[$row[csf('id')]]['file_no']=$row[csf('file_no')];
						$po_data_arr[$row[csf('id')]]['ref']=$row[csf('grouping')];
						$po_data_arr[$row[csf('id')]]['plan_cut']=$row[csf('plan_cut')];
					}*/
					//print_r($po_data_arr);
					...
					if($db_type==0) $group_po="group_concat( distinct d.id ) as po_id";
					if($db_type==2) $group_po="listagg(d.id ,',') within group (order by d.id) AS po_id";

					/*$sql="select a.job_no_prefix_num,a.job_no,a.company_name,a.buyer_name,a.style_ref_no,b.costing_per,c.id as wo_pre_cost_trim_cost_dtls,c.trim_group,	c.description,c.brand_sup_ref,c.rate,d.id as po_id,d.po_number,d.file_no,d.grouping, d.plan_cut,min(e.id) as id,sum(f.wo_qnty) as cu_woq from
					wo_po_details_master a,	wo_pre_cost_mst b,wo_pre_cost_trim_cost_dtls c,wo_po_break_down d,wo_pre_cost_trim_co_cons_dtls e
					left join wo_booking_dtls f on f.job_no=e.job_no and  f.wo_qnty>0 and  f.pre_cost_fabric_cost_dtls_id=e.wo_pre_cost_trim_cost_dtls_id and
					f.po_break_down_id=e.po_break_down_id and f.booking_type=2 where a.job_no=b.job_no and a.job_no=c.job_no and a.job_no=d.job_no_mst and
					a.job_no=e.job_no and c.id=e.wo_pre_cost_trim_cost_dtls_id and d.id=e.po_break_down_id and
					a.company_name=$company_id $txt_job_no_cond $cbo_buyer_name_cond and (c.nominated_supp = $cbo_supplier_name or c.nominated_supp= 0) and
					a.garments_nature=$garments_nature and d.is_deleted=0  and d.status_active=1 group by a.job_no_prefix_num,a.job_no,a.company_name,
					a.buyer_name,a.style_ref_no,b.costing_per,c.id,c.trim_group,c.description,c.brand_sup_ref,c.rate,d.id,d.po_number,d.file_no,d.grouping,
					d.plan_cut order by d.id,c.id";*/
					$sql_booking="select b.description,b.brand_supplier,b.po_break_down_id as po_id,(b.wo_qnty) as cu_woq ,b.pre_cost_fabric_cost_dtls_id as trim_pre_id from wo_po_details_master a,wo_booking_dtls b where  a.job_no=b.job_no  and b. booking_type=2 and b.wo_qnty>0 $txt_job_no_cond $cbo_buyer_name_cond ";
					$booking_nameArray=sql_select( $sql_booking );
					foreach ($booking_nameArray as $row)
                    {
						$booking_prev_wo_qty_arr[$row[csf('po_id')]][$row[csf('trim_pre_id')]][$row[csf('description')]][$row[csf('brand_supplier')]]+=$row[csf('cu_woq')];
					}

					 $sql="select a.job_no_prefix_num,a.job_no,a.company_name,a.buyer_name,a.style_ref_no,b.costing_per,c.id as wo_pre_cost_trim_cost_dtls,c.trim_group,	c.description,c.brand_sup_ref,c.rate,d.id as po_id,d.po_number,d.file_no,d.grouping, d.plan_cut,min(e.id) as id  from
					wo_po_details_master a,	wo_pre_cost_mst b,wo_pre_cost_trim_cost_dtls c,wo_po_break_down d,wo_pre_cost_trim_co_cons_dtls e where a.job_no=b.job_no and a.job_no=c.job_no and a.job_no=d.job_no_mst and
					a.job_no=e.job_no and c.id=e.wo_pre_cost_trim_cost_dtls_id and d.id=e.po_break_down_id and
					a.company_name=$company_id $txt_job_no_cond $cbo_buyer_name_cond and (c.nominated_supp = $cbo_supplier_name or c.nominated_supp= 0) and
					a.garments_nature=$garments_nature and d.is_deleted=0  and d.status_active=1 group by a.job_no_prefix_num,a.job_no,a.company_name,
					a.buyer_name,a.style_ref_no,b.costing_per,c.id,c.trim_group,c.description,c.brand_sup_ref,c.rate,d.id,d.po_number,d.file_no,d.grouping,
					...
					on
					f.job_no=e.job_no and  f.wo_qnty>0 and
					...
					order by c.id";*/
					 $condition= new condition();
						...
						$trim_arr_qty=$trims->getQtyArray_by_orderAndPrecostdtlsid();
                    ...
                    foreach ($nameArray as $selectResult)
                    {
						...
						$booking_prev_wo_qty=$booking_prev_wo_qty_arr[$po_ids][$selectResult[csf('wo_pre_cost_trim_cost_dtls')]][$selectResult[csf('description')]][$selectResult[csf('brand_sup_ref')]];

						$req_qnty=$trim_arr_qty[$po_ids][$selectResult[csf('wo_pre_cost_trim_cost_dtls')]];
						if($selectResult[csf('cu_woq')]==0 || $selectResult[csf('cu_woq')]=='') $selectResult[csf('cu_woq')]=0;
						else $selectResult[csf('cu_woq')]=$selectResult[csf('cu_woq')];
						$bal_woq=def_number_format($req_qnty-$selectResult[csf('cu_woq')],5,"");

						$po_no_cond='';$file_no_cond='';$ref_no_cond='';
						foreach($po_no_ids as $pid)
						{
							//echo $pid;
						//	$po_no=$po_data_arr[$pid]['po_number'];$file_no=$po_data_arr[$pid]['file_no'];
							if($po_no_cond=='') $po_no_cond=$po_data_arr[$pid]['po_number'];else  $po_no_cond.=",".$po_data_arr[$pid]['po_number'];
							if($file_no_cond=='') $file_no_cond=$po_data_arr[$pid]['file_no'];else  $file_no_cond.=",".$po_data_arr[$pid]['file_no'];
							if($ref_no_cond=='') $ref_no_cond=$po_data_arr[$pid]['ref'];else  $ref_no_cond.=",".$po_data_arr[$pid]['ref'];
							//echo $po_no_cond.'='.$pid.'<br/>';
						}

						//echo $bal_woq.'=='.$req_qnty.'='.$selectResult[csf('cu_woq')].'<br>';
                    ...
                    <td width="80" align="right"><? echo def_number_format($booking_prev_wo_qty,5,"");?></td>
...
CASE e.costing_per WHEN 1 THEN round(((d.cons/12)*b.po_quantity)/cc.conversion_factor,4) WHEN 2 THEN round(((d.cons/1)*b.po_quantity)/cc.conversion_factor,4)  WHEN 3 THEN round(((d.cons/24)*b.po_quantity)/cc.conversion_factor,4) WHEN 4 THEN round(((d.cons/36)*b.po_quantity)/cc.conversion_factor,4) WHEN 5 THEN round(((d.cons/48)*b.po_quantity)/cc.conversion_factor,4) ELSE 0 END as req_qnty,cc.order_uom as cons_uom,IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0)  as cu_woq, CASE e.costing_per WHEN 1 THEN round((((d.cons/12)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 2 THEN round((((d.cons/1)*b.po_quantity)/cc.conversion_factor) - IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4)  WHEN 3 THEN round((((d.cons/24)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 4 THEN round((((d.cons/36)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 5 THEN round((((d.cons/48)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) ELSE 0 END as bal_woq, round((c.rate*cc.conversion_factor),8) as rate from wo_po_details_master a, wo_po_break_down b ,wo_pre_cost_mst e,wo_pre_cost_trim_cost_dtls c ,lib_item_group cc, wo_pre_cost_trim_co_cons_dtls d left join wo_booking_dtls f on f.job_no=d.job_no and f.pre_cost_fabric_cost_dtls_id=d.wo_pre_cost_trim_cost_dtls_id and f.po_break_down_id=d.po_break_down_id and f.booking_type=2   where a.job_no=b.job_no_mst and  a.job_no=c.job_no and a.job_no=e.job_no and  a.job_no=d.job_no   and c.id=d.wo_pre_cost_trim_cost_dtls_id and b.id=d.po_break_down_id and cc.id=c.trim_group  and a.company_name=$cbo_company_name $txt_job_no_cond $cbo_buyer_name_cond and a.garments_nature=$garments_nature group by d.id order by b.id) m   where id in($data) $txt_job_no_cond1  $cbo_buyer_name_cond1 and garments_nature=$garments_nature order by trim_group";//and b.pub_shipment_date between '$start_date' and '$end_date'*/
					...
					$sql_pre="select c.id as wo_pre_cost_trim_cost_dtls,e.id as trim_cons_dtls_id,d.id as po_id,sum(f.wo_qnty) as wo_qnty,f.trim_group,f.id as booking_dtl_id,f.is_short,f.cons_break_down,f.description,f.brand_supplier from wo_po_details_master a,wo_pre_cost_mst b,wo_po_break_down d,wo_booking_dtls f,wo_pre_cost_trim_cost_dtls c,wo_pre_cost_trim_co_cons_dtls e
					where
					a.job_no=b.job_no and
					a.job_no=c.job_no and
					a.job_no=d.job_no_mst and
					...
					f.pre_cost_fabric_cost_dtls_id=e.wo_pre_cost_trim_cost_dtls_id and
					f.po_break_down_id=e.po_break_down_id and
					f.booking_type=2 and f.is_short=1 and

					a.company_name=$cbo_company_name $txt_job_no_cond $cbo_buyer_name_cond and
					a.garments_nature=$garments_nature and
					  d.is_deleted=0 and d.status_active=1 and f.is_deleted=0 and f.status_active=1  and d.is_deleted=0 and d.status_active=1 group by c.id,d.id,f.trim_group,e.id,f.id,f.is_short,f.cons_break_down,f.description,f.brand_supplier";
					 $pre_nameArray=sql_select( $sql_pre );
					 $trim_cons_dtls_id='';
					foreach($pre_nameArray as $row)
					{
						if($row[csf('trim_cons_dtls_id')]!='' && $row[csf('is_short')]==1)
						{
							if($trim_cons_dtls_id=='') $trim_cons_dtls_id=$row[csf('trim_cons_dtls_id')];else $trim_cons_dtls_id.=",".$row[csf('trim_cons_dtls_id')];

							 $trim_booking_dtls_arr[$row[csf('wo_pre_cost_trim_cost_dtls')]][$row[csf('po_id')]]['booking_dtl_id']=$row[csf('booking_dtl_id')];
							 $trim_booking_dtls_arr[$row[csf('wo_pre_cost_trim_cost_dtls')]][$row[csf('po_id')]]['cons_break_down']=$row[csf('cons_break_down')];
							 $trim_booking_dtls_arr[$row[csf('wo_pre_cost_trim_cost_dtls')]][$row[csf('po_id')]]['wo_qnty']=$row[csf('wo_qnty')];
							 $trim_booking_dtls_arr[$row[csf('wo_pre_cost_trim_cost_dtls')]][$row[csf('po_id')]]['description']=$row[csf('description')];
							 $trim_booking_dtls_arr[$row[csf('wo_pre_cost_trim_cost_dtls')]][$row[csf('po_id')]]['brand_supplier']=$row[csf('brand_supplier')];

						}

					}
					//echo $trim_cons_dtls_id;
					if($trim_cons_dtls_id!='') $trim_cons_dtls_id=$data.",".$trim_cons_dtls_id;
					else $trim_cons_dtls_id=$data;
					//echo $data.'=='.$trim_cons_dtls_id.'<br/>';
					...
					*/
					...
					left join wo_booking_dtls f
					on
					f.job_no=e.job_no and f.is_deleted=0 and f.status_active=1 and
					f.pre_cost_fabric_cost_dtls_id=e.wo_pre_cost_trim_cost_dtls_id and
					f.po_break_down_id=e.po_break_down_id and
					f.booking_type=2
					where
                    ...
                    foreach ($nameArray as $selectResult)
                    {
						...
						//$cu_woq= $trim_booking_cu_arr[$selectResult[csf('wo_pre_cost_trim_cost_dtls')]][$po_ids]['cu_woq'];

						$req_qnty=$trim_arr_qty[$po_ids][$selectResult[csf('wo_pre_cost_trim_cost_dtls')]];
						$bal_woq=def_number_format($req_qnty-$selectResult[csf('cu_woq')],5,"");
						$prev_wo_qnty=$trim_booking_dtls_arr[$selectResult[csf('wo_pre_cost_trim_cost_dtls')]][$po_ids]['wo_qnty'];
						if($prev_wo_qnty>0)
						{
							$bal_woq=$prev_wo_qnty;
							$selectResult[csf('cu_woq')]=$selectResult[csf('cu_woq')]-$prev_wo_qnty;
							$selectResult[csf('description')]=$trim_booking_dtls_arr[$selectResult[csf('wo_pre_cost_trim_cost_dtls')]][$po_ids]['description'];
							$selectResult[csf('brand_sup_ref')]=$trim_booking_dtls_arr[$selectResult[csf('wo_pre_cost_trim_cost_dtls')]][$po_ids]['brand_supplier'];

						}
						else
						{
							$bal_woq=$bal_woq;
							$selectResult[csf('description')]=$selectResult[csf('description')];
							$selectResult[csf('brand_sup_ref')]=$selectResult[csf('brand_sup_ref')];

						}
						$rate=def_number_format(($selectResult[csf('rate')]*$sql_lib_item_group_array[$selectResult[csf('trim_group')]][conversion_factor]),8,"");
						$booking_dtl_id=$trim_booking_dtls_arr[$selectResult[csf('wo_pre_cost_trim_cost_dtls')]][$po_ids]['booking_dtl_id'];
						$cons_break_down=$trim_booking_dtls_arr[$selectResult[csf('wo_pre_cost_trim_cost_dtls')]][$po_ids]['cons_break_down'];
	   ?>
       <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>">
                    <td width="40"><? echo $i;?></td>
                    <td width="50"><? echo $buyer_arr [$selectResult[csf('buyer_name')]];?></td>
                    <td width="100"> <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; background-color:<? echo $bgcolor; ?>" id="txtjob_<? echo $i;?>" value="<? echo $selectResult[csf('job_no')];?>" readonly/>
					  <input type="hidden" id="txtbookingid_<? echo $i;?>" value="<? echo $booking_dtl_id;?>"  style="width:50%;" readonly/></td>
                    <td width="80"><? echo $selectResult[csf('style_ref_no')];?></td>
                    <td width="100"><p><? echo $selectResult[csf('po_number')];?> <input type="hidden" id="txtpoid_<? echo $i;?>" value="<? echo $selectResult[csf('po_id')];?>" readonly/></p></td>
                    <td width="100"><p><? echo $trim_group[$selectResult[csf('trim_group')]];?> <input type="hidden" id="txttrimcostid_<? echo $i;?>" value="<? echo $selectResult[csf('wo_pre_cost_trim_cost_dtls')];?>" readonly/> <input type="hidden" id="txttrimgroup_<? echo $i;?>" value="<? echo $selectResult[csf('trim_group')];?>" readonly/></p></td>
                    <td width="150" ><p><input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; background-color:<? echo $bgcolor; ?>" id="txtdescription_<? echo $i;?>" value="<? echo $selectResult[csf('description')];?>"  onBlur="copy_value(this.value,'txtdescription_',<? echo $i;?>)"/></p></td>
                    <td width="100">
                  <p>  <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; background-color:<? echo $bgcolor; ?>" id="txtbrandsupref_<? echo $i;?>" value="<? echo $selectResult[csf('brand_sup_ref')];?>"  onBlur="copy_value(this.value,'txtbrandsupref_',<? echo $i;?>)"  />
                   </p> </td>
                    <td width="80" align="right">
					<?
					...
					?>
                    <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; text-align:right; background-color:<? echo $bgcolor; ?>" id="txtreqqnty_<? echo $i;?>" value="<? echo number_format($req_qnty,2,'.','');?>"  readonly  />
                    </td>
                    <td width="80">
                    ...
                    <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtcuwoq_<? echo $i;?>" value="<? echo number_format($selectResult[csf('cu_woq')],2,'.','');?>"  readonly  />
                    </td>
                    <td width="80" align="right">
					...
					if($cons_break_down!='')
					{
						$cons_break_down=$cons_break_down;
					}
					else
					{
						$cons_break_down='';
...
if ($action=="show_trim_booking")
{
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1687" class="rpt_table" >
       ...
       <?
					...
					$sql="select m.job_no_prefix_num, m.job_no, m.company_name, m.buyer_name, m.style_ref_no, m.costing_per, m.wo_pre_cost_trim_cost_dtls, m.trim_group, m.rate as precost_rate, m.po_id, m.po_number, m.plan_cut, m.cu_woq, n.description, n.brand_supplier, n.id as booking_id, n.booking_no, n.sensitivity, n.wo_qnty, n.rate, n.amount, n.delivery_date, n.cons_break_down from (select a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.style_ref_no, b.costing_per, c.id as wo_pre_cost_trim_cost_dtls, c.trim_group, c.description, c.brand_sup_ref, c.rate, d.id as po_id, d.po_number, d.plan_cut, sum(f.wo_qnty) as cu_woq from wo_po_details_master a, wo_pre_cost_mst b, wo_pre_cost_trim_cost_dtls c, wo_po_break_down d, wo_pre_cost_trim_co_cons_dtls e left join wo_booking_dtls f on f.job_no=e.job_no and f.is_deleted=0 and f.status_active=1  and f.pre_cost_fabric_cost_dtls_id=e.wo_pre_cost_trim_cost_dtls_id and f.po_break_down_id=e.po_break_down_id and f.booking_type=2 where a.job_no=b.job_no and a.job_no=c.job_no and a.job_no=d.job_no_mst and a.job_no=e.job_no and c.id=e.wo_pre_cost_trim_cost_dtls_id and d.id=e.po_break_down_id and a.company_name=$cbo_company_name $txt_job_no_cond $cbo_buyer_name_cond and a.garments_nature=$garments_nature and d.is_deleted=0 and d.status_active=1 group by a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.style_ref_no, b.costing_per, c.id, c.trim_group, c.description, c.brand_sup_ref, c.rate, d.id, d.po_number, d.plan_cut order by d.id,c.id) m, wo_booking_dtls n where m.job_no=n.job_no and m.po_id=n.po_break_down_id and m.wo_pre_cost_trim_cost_dtls=n.pre_cost_fabric_cost_dtls_id  and n.booking_no=$txt_booking_no order by m.po_id,m.wo_pre_cost_trim_cost_dtls ";
					$i=1;
					$total_amount=0;
                    $nameArray=sql_select( $sql );
                    foreach ($nameArray as $selectResult)
       ...
       <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>">
                    ...
                    <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtcuwoq_<? echo $i;?>" value="<? echo number_format($cu_woq,2,'.','');?>"  readonly  />
                    </td>
                    <td width="80" align="right">
...
function validate_sum(i)
{
	...
	var txtwoq= (document.getElementById('txtwoq_qty').value)*1;
	var copy_val=document.getElementById('copy_val').checked;
	var rowCount = $('#tbl_consmption_cost tr').length-1;
	if(copy_val==true)
	{
		for(var j=i; j<=rowCount; j++)
		{
			...
			if(cons_sum > txtwoq)
			{
				if(j==i){
					...
					document.getElementById('cons_'+j).value="";
				}
				else{
					document.getElementById('cons_'+j).value="";
				}
...
<fieldset>
            ...
            <input type="hidden" id="txtwoq" value="<? echo $txtwoq;?>"/>
            ...
            $gmt_size_edb="";
            $item_size_edb="";
            if($cbocolorsizesensitive ==1 || $cbocolorsizesensitive ==3)
            {
            	$gmt_size_edb=1;
				$item_size_edb=1;
            }
            else if($cbocolorsizesensitive==2)
            {
            	$gmt_color_edb=1;
				$item_color_edb=1;
            }
            else if($cbocolorsizesensitive==4)
            {
            	$gmt_color_edb="";
	            $item_color_edb="";
	            $gmt_size_edb="";
           ...
           <input type="hidden" id="process_loss_method_id" name="process_loss_method_id" value="<? echo $process_loss_method; ?>"/>
            	...
            	<table width="800" cellspacing="0" class="rpt_table" border="0" id="tbl_consmption_cost" rules="all">
                	<thead>
                    	<tr>
                        	<th width="50">SL</th><th  width="100">PO NO</th><th  width="100">Gmts. Color</th><th  width="100">Gmts. Size</th><th  width="110">Item Color</th><th width="90">Item Size</th><th width="110"> WOQ </th><th width="110" style="display:none">Process Loss %</th><th width="105" style="display:none">Requirment </th><th width="90">Pln.Cut Qnty</th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
					...
					if($cbocolorsizesensitive == 1)
					{
						...
						foreach ($data_array as $cons_data) {
							$data=explode('_',$cons_data);
							$data_arr[$data[0]][$data[1]]['gmt_size'] = 0;
							$data_arr[$data[0]][$data[1]]['dia'] = $data[3];
							$data_arr[$data[0]][$data[1]]['item_size'] = 0;
							$data_arr[$data[0]][$data[1]]['cons'] += $data[5];
							$data_arr[$data[0]][$data[1]]['process_loss'] += $data[6];
							$data_arr[$data[0]][$data[1]]['req'] += $data[7];
							$data_arr[$data[0]][$data[1]]['pcs'] += $data[8];
							$data_arr[$data[0]][$data[1]]['color_size'] += $data[9];
						}
						$i = 0;
						foreach ($data_arr as $po=>$po_data) {
							foreach ($po_data as $color_id=>$data) {
								if($i !=0) $comma ='__'; else $comma = '';
								$new_cons_breck_downn .= $comma.$po.'_'.$color_id.'_'.$data['gmt_size'].'_'.$data['dia'].'_'.$data['item_size'].'_'.$data['cons'].'_'.$data['process_loss'].'_'.$data['req'].'_'.$data['pcs'].'_'.$data['color_size'];
								$i ++;
							}
						}
						$data_array=explode("__",$new_cons_breck_downn);
					}
					else{
						$data_array=explode("__",$cons_breck_downn);
					}
					...
					if ( count($data_array)>0)
					{
						...
						foreach( $data_array as $row )
						{
							$data=explode('_',$row);
							$i++;
							if($cbocolorsizesensitive == 2 || $cbocolorsizesensitive == 4)
							{
								if($data[2] == '' || $data[2] == 0){
									$data[2]=$size_library[$data[4]];
								}
								if($data[4] == '' || $data[4] == 0){
									$data[4]=$size_library[$data[2]];
								}
							...
							?>
                            	<tr id="break_1" align="center">
                                    ...
                                    <td>
                                    <input type="text" id="pocolor_<? echo $i;?>"  name="pocolor_<? echo $i;?>" class="text_boxes" style="width:85px" value="<? if($cbocolorsizesensitive == 2) { echo ''; } else {echo $color_library[$data[1]];} ?>" <? if($gmt_color_edb){ echo  "disabled";}else { echo "";} ?> readonly />
                                    ...
                                    <td>
                                    <input type="text" id="gmtssizes_<? echo $i;?>"  name="gmtssizes_<? echo $i;?>" class="text_boxes" style="width:85px" value="<? if($cbocolorsizesensitive==3){ echo '';} else{ echo $size_library[$data[2]];} ?>" <? if($gmt_size_edb){ echo  "disabled";}else { echo "";} ?> readonly />
                                    ...
                                    <td>
                                    <input type="text" id="diawidth_<? echo $i;?>"    name="diawidth_<? echo $i;?>"  class=" text_boxes" style="width:95px" onChange="copy_value(this.value,'diawidth_',<? echo $i;?>)" value="<? if($cbocolorsizesensitive == 2) { echo ''; } else { echo $data[3]; } ?>" <? if($item_color_edb){ echo  "disabled";}else { echo "";} ?>  />
                                    </td>
                                    <td>
                                    <input type="text" id="itemsizes_<? echo $i;?>"  name="itemsizes_<? echo $i;?>"    class="text_boxes" style="width:75px" onChange="copy_value(this.value,'itemsizes_',<? echo $i;?>)" value="<? if($cbocolorsizesensitive==3 || $cbocolorsizesensitive==1){ echo '';} else{ echo $data[4]; } ?>" <? if($item_size_edb){ echo  "disabled";}else { echo "";} ?>   />
                                    </td>
                                    <td>
                                    <input type="text" id="cons_<? echo $i;?>"  onChange="set_sum_value( 'cons_sum', 'cons_' );set_sum_value( 'requirement_sum', 'requirement_' );calculate_requirement(<? echo $i;?>);copy_value(this.value,'cons_',<? echo $i;?>);validate_sum( <? echo $i; ?> )"  name="cons_<? echo $i;?>" class="text_boxes_numeric" style="width:95px" value="<? echo $data[5]; ?>"   />
                                    </td>
					...
					else
					{
                    	...
                    	<tr id="break_1" align="center">
                                    ...
                                    <td>
                                    <input type="text" id="pocolor_<? echo $i;?>"  name="pocolor_<? echo $i;?>" class="text_boxes" style="width:85px" value="<? if($cbocolorsizesensitive==2){echo '';} else {echo $color_library[$row[csf('color_number_id')]];} ?>" <? if($gmt_color_edb){ echo  "disabled";}else { echo "";} ?>/>
                                    <input type="hidden" id="pocolorid_<? echo $i;?>"  name="pocolorid_<? echo $i;?>" class="text_boxes" style="width:85px" value="<? echo $row[csf('color_number_id')]; ?>" />
                                     ...
                                     <td>
                                    <input type="text" id="gmtssizes_<? echo $i;?>"  name="gmtssizes_<? echo $i;?>" class="text_boxes" style="width:85px" value="<? if($cbocolorsizesensitive==3){echo '';} else { echo $size_library[$row[csf('size_number_id')]];} ?>" <? if($gmt_size_edb){ echo  "disabled";}else { echo "";} ?> >
                                    ...
                                    <td>
                                    <input type="text" id="diawidth_<? echo $i;?>"  value="<? if($cbocolorsizesensitive==2){echo '';} else  { echo $color_library[$row[csf('color_number_id')]];} ?>"  name="diawidth_<? echo $i;?>"  class="text_boxes" style="width:95px" onChange="copy_value(this.value,'diawidth_',<? echo $i;?>)" <? if($item_color_edb){ echo  "disabled";}else { echo "";} ?> />
                                    </td>
                                    <td>
                                    <input type="text" id="itemsizes_<? echo $i;?>"  name="itemsizes_<? echo $i;?>"    class="text_boxes" style="width:75px" onChange="copy_value(this.value,'itemsizes_',<? echo $i;?>)" value="<? if($cbocolorsizesensitive==3){echo '';} else { if($data[4]==''){echo $size_library[$row[csf('size_number_id')]];} else {echo $data[4];} }?>"  <? if($item_size_edb){ echo  "disabled";}else { echo "";} ?>/>
                                    </td>
                                    <td>
                                    <input type="text" id="cons_<? echo $i;?>" onChange="set_sum_value( 'cons_sum', 'cons_' );set_sum_value( 'requirement_sum', 'requirement_' );calculate_requirement(<? echo $i;?>);copy_value(this.value,'cons_',<? echo $i;?>);validate_sum( <? echo $i; ?> )"  name="cons_<? echo $i;?>" class="text_boxes_numeric" style="width:95px"  value="<? echo $txtwoq_cal; ?>"/>
                                    </td>
                                    <td  style="display:none">
                                    <input type="text" id="processloss_<? echo $i;?>" onBlur="set_sum_value( 'processloss_sum', 'processloss_' ) "  name="processloss_<? echo $i;?>" class="text_boxes_numeric" style="width:95px" onChange="calculate_requirement(<? echo $i;?>);set_sum_value( 'processloss_sum', 'processloss_' );set_sum_value( 'requirement_sum', 'requirement_' );copy_value(this.value,'processloss_',<? echo $i;?>) " value="<? echo $data[6]; ?>" />
                                    </td>
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$field_array="id, booking_type, is_short, booking_month, booking_year, booking_no_prefix, booking_no_prefix_num, booking_no, company_id, buyer_id, job_no, 	item_category, supplier_id, currency_id, exchange_rate, booking_date, delivery_date, pay_mode, source, attention, fabric_source, tenor, entry_form, ready_to_approved, inserted_by, insert_date";
		//cbo_ready_to_approved
		 $data_array ="(".$id.",2,1,".$cbo_booking_month.",".$cbo_booking_year.",'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',".$cbo_company_name.",".$cbo_buyer_name.",".$txt_job_no.",4,".$cbo_supplier_name.",".$cbo_currency.",".$txt_exchange_rate.",".$txt_booking_date.",".$txt_delivery_date.",".$cbo_pay_mode.",".$cbo_source.",".$txt_attention.",".$cbo_material_source.",".$txt_tenor.",178,".$cbo_ready_to_approved.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";


		 if  ( check_table_status( $_SESSION['menu_id'], 1 )==0 ) { echo "15**0"; disconnect($con); die;}
		 $id_dtls=return_next_id( "id", "wo_booking_dtls", 1 ) ;
		 $field_array1="id, pre_cost_fabric_cost_dtls_id, po_break_down_id, job_no, booking_no, booking_mst_id, booking_type, is_short, trim_group, description, brand_supplier, uom, sensitivity, wo_qnty, rate, amount, delivery_date, cons_break_down, inserted_by, insert_date";
		 ...
		 $field_array2="id, wo_trim_booking_dtls_id, booking_no, job_no, po_break_down_id, color_number_id, gmts_sizes, item_color, item_size, cons, process_loss_percent, requirment, rate, amount, pcs, color_size_table_id";
		 $add_comma=0;
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			 ...
			 if ($i!=1) $data_array1 .=",";
			 $data_array1 .="(".$id_dtls.",".$$txttrimcostid.",".$$txtpoid.",".$$txtjob.",'".$new_booking_no[0]."',".$id.",2,1,".$$txttrimgroup.",".$$txtdescription.",".$$txtbrandsupref.",".$$txtuom.",".$$cbocolorsizesensitive.",".$$txtwoq.",".$$txtrate.",".$$txtamount.",".$$txtddate.",".$$consbreckdown.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				...
				if(str_replace("'",'',$$consbreckdown) !='')
				{
					...
					for($c=0;$c < count($consbreckdown_array);$c++)
					{
						...
						if(str_replace("'","",$consbreckdownarr[3])!="")
						{
						 ...
						 $woAmt=0;
						 $woAmt=str_replace("'","",$consbreckdownarr[5])*str_replace("'","",$$txtrate);

						if ($add_comma!=0) $data_array2 .=",";
						$data_array2 .="(".$id1.",".$id_dtls.",'".$new_booking_no[0]."',".$$txtjob.",'".$consbreckdownarr[0]."','".$consbreckdownarr[1]."','".$consbreckdownarr[2]."','".$color_id."','".$consbreckdownarr[4]."','".$consbreckdownarr[5]."','".$consbreckdownarr[6]."','".$consbreckdownarr[7]."',".$$txtrate.",'".$woAmt."','".$consbreckdownarr[8]."','".$consbreckdownarr[9]."')";
						$id1=$id1+1;
						$add_comma++;
					}
				}
	...
	/*	}else{*/
		 ...
		 $field_array_up="booking_month*booking_year*buyer_id*job_no*item_category*supplier_id*currency_id*exchange_rate*booking_date*delivery_date*pay_mode*source*attention*fabric_source*tenor*ready_to_approved*updated_by*update_date";
		  $field_array_dtls="id,pre_cost_fabric_cost_dtls_id,po_break_down_id,job_no,booking_mst_id,booking_no,booking_type,is_short,trim_group,description,brand_supplier,uom,sensitivity,wo_qnty,rate,amount,delivery_date,cons_break_down,inserted_by,insert_date";

		 $data_array_up ="".$cbo_booking_month."*".$cbo_booking_year."*".$cbo_buyer_name."*".$txt_job_no."*4*".$cbo_supplier_name."*".$cbo_currency."*".$txt_exchange_rate."*".$txt_booking_date."*".$txt_delivery_date."*".$cbo_pay_mode."*".$cbo_source."*".$txt_attention."*".$cbo_material_source."*".$txt_tenor."*".$cbo_ready_to_approved."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		 ...
		 $field_array_up2="id, wo_trim_booking_dtls_id, booking_no, job_no, po_break_down_id, color_number_id, gmts_sizes, item_color, item_size, cons, process_loss_percent, requirment, rate, amount, pcs, color_size_table_id";

		  $add_comma=0;  $add_comma2=0;
		  $id1=return_next_id( "id", "wo_trim_book_con_dtls", 1 );
		  $book_dtls_id=return_next_id( "id", "wo_booking_dtls", 1 );
		  $new_array_color=array();
		 for ($i=1;$i<=$total_row;$i++)
		 {
			 $txttrimcostid="txttrimcostid_".$i;
			...
			if(str_replace("'",'',$$txtbookingid)!="")
			{
				...
				if(str_replace("'",'',$$consbreckdown) !='')
				{
					...
					for($c=0;$c < count($consbreckdown_array);$c++)
					{
						...
						$data_array_up2 .="(".$id1.",".$$txtbookingid.",".$txt_booking_no.",".$$txtjob.",'".$consbreckdownarr[0]."','".$consbreckdownarr[1]."','".$consbreckdownarr[2]."','".$color_id."','".$consbreckdownarr[4]."','".$consbreckdownarr[5]."','".$consbreckdownarr[6]."','".$consbreckdownarr[7]."',".$$txtrate.",'".$woAmt."','".$consbreckdownarr[8]."','".$consbreckdownarr[9]."')";
						$id1=$id1+1;
						$add_comma++;
						//$msg=$id1;
					}
          ...
          //CONS break down end===============================================================================================
			...
			else ////New Insert/ Booking Dtls
			{
				 	if ($add_comma2!=0) $data_array_dtls .=",";
			 $data_array_dtls .="(".$book_dtls_id.",".$$txttrimcostid.",".$$txtpoid.",".$$txtjob.",".$txt_booking_no.",".$booking_id.",2,1,".$$txttrimgroup.",".$$txtdescription.",".$$txtbrandsupref.",".$$txtuom.",".$$cbocolorsizesensitive.",".$$txtwoq.",".$$txtrate.",".$$txtamount.",".$$txtddate.",".$$consbreckdown.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			    $book_dtls_id=$book_dtls_id+1;
				$add_comma2++;
				...
				if(str_replace("'",'',$$consbreckdown) !='')
				{
					...
					for($c=0;$c < count($consbreckdown_array);$c++)
					{
						...
						if ($add_comma!=0) $data_array_up2 .=",";
						$data_array_up2 .="(".$id1.",".$book_dtls_id.",".$txt_booking_no.",".$$txtjob.",'".$consbreckdownarr[0]."','".$consbreckdownarr[1]."','".$consbreckdownarr[2]."','".$color_id."','".$consbreckdownarr[4]."','".$consbreckdownarr[5]."','".$consbreckdownarr[6]."','".$consbreckdownarr[7]."',".$$txtrate.",'".$woAmt."','".$consbreckdownarr[8]."','".$consbreckdownarr[9]."')";
						$id1=$id1+1;
						$add_comma++;
						//$msg=$id1.'='.'DDD';
					}
		...
		$rID1=execute_query(bulk_update_sql_statement( "wo_booking_dtls", "id", $field_array_up1, $data_array_up1, $id_arr ));
		 ...
		 if($data_array_dtls!="")
		 {
		  $rID3=sql_insert("wo_booking_dtls",$field_array_dtls,$data_array_dtls,0);
		...
		else if($db_type==2 || $db_type==1 )
		{
			if($rID && $rID1 &&  $rID2 &&  $rID3){
				oci_commit($con);
				echo "1**".str_replace("'","",$txt_booking_no).'**'.$msg;
			}
			else{
				oci_rollback($con);
				echo "10**".str_replace("'","",$txt_booking_no);
...
?>
    ...
    <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
		<table width="750" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
            <thead>
            	<tr>
                    <th colspan="9" align="center"><? echo create_drop_down( "cbo_search_category", 130, $string_search_type,'', 1, "-- Search Catagory --" ); ?> </th>
                </tr>
                <tr>
            ...
            <tr class="general">
                <td><? echo create_drop_down( "cbo_company_mst", 140, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", '', ""); ?></td>
                <td id="buyer_td"><? echo create_drop_down( "cbo_supplier_name", 140, "select id,supplier_name from lib_supplier where find_in_set(4,party_type) and status_active =1 and is_deleted=0 order by supplier_name","id,supplier_name", 1, "-- Select Supplier --", $selected, "",0 ); ?></td>
                <td><input name="txt_booking_prifix" id="txt_booking_prifix" class="text_boxes" style="width:70px" placeholder="Booking"></td>
                <td><input name="txt_job_no" id="txt_job_no" class="text_boxes" style="width:70px" placeholder="Job"></td>
                <td><input name="txt_style_ref" id="txt_style_ref" class="text_boxes" style="width:70px" placeholder="Style"></td>
                <td><input name="txt_order_no" id="txt_order_no" class="text_boxes" style="width:70px" placeholder="Order"></td>

                <td><input type="text" name="txt_date_from" id="txt_date_from" class="datepicker" style="width:70px" placeholder="From"></td>
                <td><input type="text" name="txt_date_to" id="txt_date_to" class="datepicker" style="width:70px" placeholder="To"></td>
                <td align="center">
                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_supplier_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_booking_prifix').value+'_'+document.getElementById('cbo_search_category').value+'_'+document.getElementById('txt_job_no').value+'_'+document.getElementById('txt_style_ref').value+'_'+document.getElementById('txt_order_no').value, 'create_booking_search_list_view', 'search_div', 'short_trims_booking_controller','setFilterGrid(\'list_view\',-1)')" style="width:70px;" /></td>
            </tr>
            <tr class="general">
...
if ($action=="create_booking_search_list_view")
{
	...
	if ($data[1]!=0) $supplier_id=" and a.supplier_id='$data[1]'"; else $supplier_id ="";
	...
	if($data[6]==4 || $data[6]==0)
	{
	   	...
	   	if (str_replace("'","",$data[7])!="") $job_cond=" and a.booking_no_prefix_num like '%$data[5]%'  $booking_year_cond  "; else  $job_cond="";
    ...
    else if($data[6]==1)
	{
		...
		if (trim($data[8])!="") $style_cond=" and c.style_ref_no ='$data[8]'"; else $style_cond="";
	...
	if (str_replace("'","",$data[7])!="") $job_cond=" and c.job_no_prefix_num='$data[7]'"; else $job_cond="";
	...
	$sql="select a.id, a.booking_no_prefix_num, a.booking_no, a.supplier_id, a.booking_date, a.delivery_date, c.style_ref_no, d.po_number from wo_booking_mst a, wo_booking_dtls b, wo_po_details_master c, wo_po_break_down d where a.booking_no=b.booking_no and b.job_no=c.job_no and b.job_no=d.job_no_mst and a.booking_type=2 and a.is_short=1 and a.entry_form=178 and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 $company $buyer $supplier_id $booking_date $booking_cond $job_cond $style_cond $order_cond group by a.id, a.booking_no_prefix_num, a.booking_no, a.supplier_id, a.booking_date, a.delivery_date, c.style_ref_no, d.po_number order by a.id DESC";
	//echo $sql;
	echo  create_list_view("list_view", "Booking No, Supplier, Booking Date, Delivery Date", "120,100,100,100","600","320",0, $sql , "js_set_value", "booking_no", "", 1, "0,supplier_id,0,0", $arr , "booking_no_prefix_num,supplier_id,booking_date,delivery_date", '','','0,0,3,3','','');
	exit();
}
...
?>
        ...
        foreach ($nameArray as $result)
        {
			...
			foreach ($data_array_po as $rows)
			{
				...
				if($rows[csf('shiping_status')]==1) $shiping_status.= "FP".",";
				else if($rows[csf('shiping_status')]==2) $shiping_status.= "PS".",";
				else if($rows[csf('shiping_status')]==3) $shiping_status.= "FS".",";
			}
       ...
       <table width="100%" style="border:1px solid black">
		...
		//$sizeColorSql=sql_select( "select b.color_number_id, b.gmts_sizes, b.item_color, b.item_size from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=$txt_booking_no and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.color_number_id, b.gmts_sizes, b.item_color, b.item_size"); $q=1;
        ?>
        <!--<table border="1" align="left" cellpadding="0" width="440" cellspacing="0" class="rpt_table" rules="all">
        	<thead>
            ...
            <tbody>
            	<?php
/*foreach($sizeColorSql as $row)
{
if($q%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
?>
<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>" style="cursor:pointer">
<td style="word-break:break-all"><?=$color_library[$row[csf('color_number_id')]]; ?></td>
<td style="word-break:break-all"><?=$size_library[$row[csf('gmts_sizes')]]; ?></td>
<td style="word-break:break-all"><?=$color_library[$row[csf('item_color')]]; ?></td>
<td style="word-break:break-all"><?=$row[csf('item_size')]; ?></td>
</tr>
<?
$q++;
}*/
?>
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            {
				$i++;
            $nameArray_item_description=sql_select( "select a.trim_group, a.description, a.brand_supplier, a.rate, a.uom, b.color_number_id, b.gmts_sizes, b.item_color, b.item_size, sum(b.cons) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=$txt_booking_no and a.sensitivity=0 and a.trim_group=".$result_item[csf('trim_group')]." group by a.trim_group, a.description, a.brand_supplier, a.rate, a.uom, b.color_number_id, b.gmts_sizes, b.item_color, b.item_size order by a.trim_group ");
			//echo "select a.trim_group, a.description, a.brand_supplier, a.rate, a.uom, b.color_number_id, b.gmts_sizes, b.item_color, b.item_size, sum(b.cons) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=$txt_booking_no and a.sensitivity=0 and a.trim_group=".$result_item[csf('trim_group')]." group by a.description, a.brand_supplier, a.rate, a.uom, b.color_number_id, b.gmts_sizes, b.item_color, b.item_size order by a.trim_group "; die;
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>"><? echo $i; ?></td>
                <td align="center" style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>"><? echo $trim_group[$result_item[csf('trim_group')]]; ?></td>
                <?
                $color_tatal=0;
                $total_amount_as_per_gmts_color=0;
                foreach($nameArray_item_description as $result_itemdescription)
				{
                    ...
                    <td style="border:1px solid black"><? echo $color_library[$result_itemdescription[csf('color_number_id')]]; ?> </td>
                    <td style="border:1px solid black"><? echo $size_library[$result_itemdescription[csf('gmts_sizes')]]; ?> </td>
                    ...
                    <td style="border:1px solid black" align="right"><? echo number_format($result_itemdescription[csf('cons')],2); ?> </td>
					<?
	...
	if($show_comment==1)
	{
	   ...
	   ?>
                    <tr>
                        <td width="40"><? echo $i;?></td>
                        <td width="150"><? echo $trim_group[$selectResult[csf('trim_group')]];?></td>
                        <td width="150"><? echo $po_number[$selectResult[csf('po_break_down_id')]];?></td>
                        <td width="150" align="right"><? echo $pre_amount; ?></td>
                        <td width="" align="right"><? echo number_format($cu_woq_amount/$exchange_rate,5);?></td>
                    </tr>
	   <?
	   $i++;
					}
...
if ($action=="populate_data_from_search_popup")
{
	 $sql= "select booking_no,booking_date,company_id,buyer_id, 	job_no,currency_id,exchange_rate,pay_mode,booking_month,ready_to_approved,supplier_id,attention,delivery_date,source,booking_year,is_approved,fabric_source,tenor from wo_booking_mst  where booking_no='$data'";
