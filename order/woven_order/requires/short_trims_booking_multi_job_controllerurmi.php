...
if ($action=="generate_fabric_booking"){
	...
	foreach($sql_cu_booking as $row_cu_booking){
		if($row_cu_booking['ENTRY_FORM_ID']==716)
		{
...
if ($action=="show_trim_booking"){
	...
	$cu_booking_arr=array();
	$sql_cu_booking=sql_select("select C.JOB_NO, C.PRE_COST_FABRIC_COST_DTLS_ID as BOMID, C.PO_BREAK_DOWN_ID, SUM(C.WO_QNTY) AS CU_WO_QNTY, SUM(C.AMOUNT) AS CU_AMOUNT from wo_po_details_master a, wo_po_break_down  d, wo_booking_dtls c where a.job_no=d.job_no_mst and a.job_no=c.job_no and d.id=c.po_break_down_id and a.company_name=$cbo_company_name and c.pre_cost_fabric_cost_dtls_id=$pre_cost_id and c.entry_form_id!=716 and c.status_active=1 and c.is_deleted=0 group by c.job_no, c.pre_cost_fabric_cost_dtls_id, c.po_break_down_id");
	foreach($sql_cu_booking as $row_cu_booking){
		$cu_booking_arr[$row_cu_booking['JOB_NO']][$row_cu_booking['BOMID']]['cu_woq'][$row_cu_booking['PO_BREAK_DOWN_ID']]=$row_cu_booking['CU_WO_QNTY'];
		$cu_booking_arr[$row_cu_booking['JOB_NO']][$row_cu_booking['BOMID']]['cu_amount'][$row_cu_booking['PO_BREAK_DOWN_ID']]=$row_cu_booking['CU_AMOUNT'];
	}
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0){
		...
		$data_array ="(".$id.",2,1,4,262,1,'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',".$cbo_company_name.",".$cbo_buyer_name.",".$txt_booking_date.",".$txt_delivery_date.",".$cbo_basis_id.",".$txt_reqsn_id.",".$txt_reqsn_no.",".$cbo_source.",".$cbo_currency.",".$cbo_pay_mode.",".$cbo_supplier_name.",".$txt_tenor.",".$cbo_division_id.",".$cbo_responsible_dept.",".$txt_responsible_person.",".$cbo_ready_to_approved.",".$cbo_level.",".$txt_reason.",".$txt_attention.",".$txtdelivery_address.",".$txt_remark.",'".$variable_short_booking."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
				...
				for($c=0;$c < count($consbreckdown_array);$c++){
					...
					if(str_replace("'","",$consbreckdownarr[4])!="")
					{
						if (!in_array(str_replace("'","",$consbreckdownarr[4]),$new_array_color)){
							$color_id = return_id( str_replace("'","",$consbreckdownarr[4]), $color_library, "lib_color", "id,color_name","262");
	...
	if ($operation==0){
		...
		for ($i=1; $i<=$total_row; $i++){
			...
			foreach($strdata->$job->$trimcostid->po_id as $poId){
				...
				$amount=$wqQty*$rate;
				$data_array1 ="(".$id_dtls.",".$$txttrimcostid.",".$poId.",".$$txtjob_id.",".$txt_booking_no.",".$booking_mst_id.",2,1,262,".$$txttrimgroup.",".$$txtuom.",".$$cbocolorsizesensitive.",".$wqQty.",".$$txtexchrate.",".$$txtrate.",".$amount.",'".$txtdlvdate."',".$$txtdescid.",".$$txtcountry.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
...
if ($action=="create_booking_search_list_view")
{
	...
	if($data[11]==0)
	{
		...
		if(!empty($data[12]))
		{
		 ...
		 $sql="select a.id, a.booking_no_prefix_num, a.booking_no, a.company_id, a.supplier_id, a.booking_date, a.delivery_date,c.style_ref_no,d.po_number,a.pay_mode from wo_booking_mst a, wo_booking_dtls b, wo_po_details_master c, wo_po_break_down d  where a.id=b.booking_mst_id and b.job_no=c.job_no and b.po_break_down_id=d.id and c.id=d.job_id and a.booking_type=2 and a.entry_form=262 and a.is_short=1
	...
	else
	{
		...
		$sql="select a.id, a.job_no, a.booking_no_prefix_num, a.booking_no, a.company_id, a.supplier_id, a.booking_date, a.delivery_date, a.pay_mode from wo_booking_mst a where a.id not in (select a.id from wo_booking_mst a, wo_booking_dtls b, wo_po_break_down c where a.id=b.booking_mst_id and b.po_break_down_id=c.id and a.booking_type=2 and a.is_short=1 and a.entry_form=262 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $internal_ref_con $company ".set_user_lavel_filtering(' and a.buyer_id','buyer_id')." $supplier_id $booking_date $booking_cond $job_cond $file_cond $ref_cond $booking_year_cond group by a.id) and a.booking_type=2 and a.is_short=1 and a.entry_form=262 and a.status_active=1 and a.is_deleted=0 $company ".set_user_lavel_filtering(' and a.buyer_id','buyer_id')." $supplier_id $booking_date $booking_cond $booking_year_cond group by a.id, a.booking_no_prefix_num, a.booking_no, a.job_no, a.company_id, a.supplier_id, a.booking_date, a.delivery_date, a.pay_mode order by a.id DESC";
	}
	 //echo $sql; die;
	?>
    <div width="900">
...
if($action=="show_trim_booking_report2"){
	...
	<div style="width:1333px" align="center">
       ...
       <tr style="border:1px solid black;">
        ...
        <td width="49%">
	        <?
	        	echo get_spacial_instruction($txt_booking_no,'');//,262
	        ?>
    	</td>
