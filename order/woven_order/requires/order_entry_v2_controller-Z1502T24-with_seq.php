...
if ($action=="populate_data_from_search_popup")
{
	...
	foreach ($data_array as $row)
	{
		...
		$sub_dept= create_drop_down( "cbo_sub_dept", 150, "select id,sub_department_name from lib_pro_sub_deparatment where buyer_id=$buyer_id and	department_id=$department_id and status_active =1 and is_deleted=0 order by sub_department_name","id,sub_department_name", 1, "-- Select Sub Dep --", $selected, "" );
		...
		echo "document.getElementById('sub_td').innerHTML = '".$sub_dept."';\n";
...
if ($action=="show_po_active_listview")
{
	...
	{
		$countryQty_arr[$datastr]+=$val['poqty'];
		$countryAmount_arr[$datastr]+=$val['amt'];
	}
     ...
     <div style="width:900px; max-height:270px;overflow-y:scroll;" >
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="882" class="rpt_table" id="tbl_po_list">
            ...
            foreach( $countryListArr as $datastr=>$val )
            {
				...
				$packQty=0;
				if(count($countryPackArr[$datastr])>0){
					foreach($countryPackArr[$datastr] as $sizeid=>$sdata)
					{
						if($sdata['poqtypcs']>0 && $sdata['pcsqtypack']>0){
							$packQty+=$sdata['poqtypcs']/$sdata['pcsqtypack'];
						}						
					}
				}				
                if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
				$avgRate=$countryAmount_arr[$datastr]/$countryQty_arr[$datastr];
				?>
					<tr bgcolor="<?=$bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="get_php_form_data('<?=$exdata[0].'_'.$exdata[1].'_'.$exdata[3].'_'.$data.'_'.$exdata[4].'_'.$exdata[5].'_'.$exdata[6].'_'.$exdata[7].'_'.$exdata[8]; ?>','populate_country_details_form_data','requires/order_entry_v2_controller');">
                        ...
                        <td width="40" align="right" style="word-break:break-all"><?=number_format($avgRate,4); ?></td>
...
if ($action=="save_update_delete_mst")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$id= return_next_id_by_sequence("po_details_master_seq","wo_po_details_master",$con );
		...
		for($c=0;$c < count($set_breck_down_array);$c++)
		{
			...
			$id1= return_next_id_by_sequence("po_details_mas_set_details_seq", "wo_po_details_mas_set_details", $con );
		...
		if($cost_control_source==1 && $flag==1)
		{
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		for($c=0;$c < count($set_breck_down_array);$c++)
		{
			...
			$id1= return_next_id_by_sequence("po_details_master_seq","wo_po_details_master",$con );
...
if ($action=="save_update_delete_dtls")
{
	...
	$con = connect();
	$size_id_arr=array();//Lib Size Insert
	for($k=1; $k<=$size_table; $k++)
	{
		...
		$size_id_arr[trim(str_replace("'","",$txtSizeName))]=$size_id_val;
	}
	
	$gmtsColorIdArr=array();//Lib Color Insert
	for($i=1; $i<=$color_table; $i++)
	{
		...
		if(str_replace("'","",$txtColorName)!="")
		{
			...
			$gmtsColorIdArr[trim(str_replace("'","",$txtColorName))]=$color_id;
		}
	}
	//oci_commit($con);
	...
	if ($operation==0)  // Insert Here
	{
		...
		if (str_replace("'","",$copy_id)==2)
		{
			...
			$id= return_next_id_by_sequence(  "po_break_down_seq",   "wo_po_break_down", $con );
			...
			if($breakdown_type==4)
			{
				$field_array1="id, job_id, po_break_down_id, job_no_mst, color_mst_id, size_mst_id, item_mst_id, item_number_id, country_id, code_id, ultimate_country_id, ul_country_code, cutup_date, cutup, country_ship_date, color_number_id, size_number_id, order_quantity, order_rate, excess_cut_perc, article_number, order_total, plan_cut_qnty, pack_qty, pcs_per_pack, pack_type, pcs_pack, country_avg_rate, assort_qty, solid_qty, color_order, size_order, inserted_by, insert_date, is_deleted, status_active";
			}
			else
			{
				$field_array1="id, job_id, po_break_down_id, job_no_mst, color_mst_id, size_mst_id, item_mst_id, item_number_id, country_id, code_id, ultimate_country_id, ul_country_code, cutup_date, cutup, country_ship_date, color_number_id, size_number_id, order_quantity, order_rate, excess_cut_perc, article_number, order_total, plan_cut_qnty, country_avg_rate, assort_qty, solid_qty, color_order, size_order, inserted_by, insert_date, is_deleted, status_active";
			}

			//echo $cbo_gmtsItem_id.'kausar';die;$excess_cut_perc=($data[1]*$row[csf('order_quantity')]/100)+$row[csf('order_quantity')];
			$item_mst=$color_mst=$size_mst=array();
			$sqlMstid=sql_select("select ITEM_MST_ID, ITEM_NUMBER_ID, COLOR_MST_ID, COLOR_NUMBER_ID, SIZE_MST_ID, SIZE_NUMBER_ID from wo_po_color_size_breakdown where po_break_down_id=$po_id and status_active=1 and is_deleted=0");
			
			foreach($sqlMstid as $mstrow)
			{
				if($mstrow['ITEM_MST_ID']!=0)
				{
					$item_mst[$mstrow['ITEM_NUMBER_ID']]=$mstrow['ITEM_MST_ID'];
				}
				if($mstrow['COLOR_MST_ID']!=0)
				{
					$color_mst[$mstrow['COLOR_NUMBER_ID']]=$mstrow['COLOR_MST_ID'];
				}
				if($mstrow['SIZE_MST_ID']!=0)
				{
					$size_mst[$mstrow['SIZE_NUMBER_ID']]=$mstrow['SIZE_MST_ID'];
				}
			}
			unset($sqlMstid);			
			...
			if($breakdown_type==1)
			{
				for($i=1; $i<=$color_table; $i++)
				{
					...
					if(str_replace("'","",$txtColorName)!="")
					{
						...
						$color_id=$gmtsColorIdArr[trim(str_replace("'","",$txtColorName))];
						...
						for($m=1; $m<=$size_table; $m++)
						{
							...
							if(str_replace("'","",$txtSizeName)!="")
							{
								$id1= return_next_id_by_sequence(  "po_color_size_breakdown_seq",   "wo_po_color_size_breakdown", $con );
								$size_id = $size_id_arr[trim(str_replace("'","",$txtSizeName))];
								
								if($sizeSeqArr[$size_id]=="")
								{
									$sizeSeqArr[$size_id]=$sizeseq;
									$sizeseq++;
								}
								//$txt_colorSizeQty="";
								...
								{
									$data_array1.="INTO wo_po_color_size_breakdown (".$field_array1.") VALUES(".$id1.",".$hidd_job_id.",".$po_id.",".$update_id.",'".$color_mst_id."','".$size_mst_id."','".$item_mst_id."',".$cbo_gmtsItem_id.",".$cbo_deliveryCountry_id.",".$cbo_code_id.",".$cbo_country_id.",".$cbo_countryCode_id.",".$txt_cutup_date.",".$cbo_cutOff_id.",".$txt_countryShip_date.",".$color_id.",'".$size_id."','".$color_size_poQty."','".$color_size_rate."',".$$txt_colorSizeExCut.",'".$txt_colorSizeArticleNo."','".$order_total_amt."','".ceil($color_size_planCutQty)."',".$txtCaAvgRatePack.",'".$assort_qty."','".$solid_qty."','".$colorSeqArr[$color_id]."','".$sizeSeqArr[$size_id]."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',0,".$cbo_status.")";
								}
							}
							//echo $data_array1;
						}
						...
						for($m=1; $m<=$size_table; $m++)
						{
							...
							if(str_replace("'","",$txtSizeName)!="")
							{
								...
								if($color_size_poQty>0)
								{
									...
									$data_array1.="INTO wo_po_color_size_breakdown (".$field_array1.") VALUES(".$id1.",".$hidd_job_id.",".$po_id.",".$update_id.",'".$color_mst_id."','".$size_mst_id."','".$item_mst_id."',".$cbo_gmtsItem_id.",".$cbo_deliveryCountry_id.",".$cbo_code_id.",".$cbo_country_id.",".$cbo_countryCode_id.",".$txt_cutup_date.",".$cbo_cutOff_id.",".$txt_countryShip_date.",".$color_id.",'".$size_id."','".$color_size_poQty."','".$color_size_rate."',".$$txt_colorSizeExCut.",'".$txt_colorSizeArticleNo."','".$order_total_amt."','".ceil($color_size_planCutQty)."',".$$txt_colorSizePackQty.",".$$txt_colorSizePcsQty.",".$txt_breakdownGrouping.",".$txt_pcsQty.",".$txtCaAvgRatePack.",'".$assort_qty."','".$solid_qty."','".$colorSeqArr[$color_id]."','".$sizeSeqArr[$size_id]."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',0,".$cbo_status.")";
				...
				for($i=1; $i<=$color_table; $i++)
				{
					...
					if(str_replace("'","",$txtColorName)!="")
					{
						...
						for($m=1; $m<=$size_table; $m++)
						{
							...
							{
								$id1= return_next_id_by_sequence(  "po_color_size_breakdown_seq",   "wo_po_color_size_breakdown", $con );
								$idratio= return_next_id_by_sequence(  "po_ratio_breakdown_seq",   "wo_po_ratio_breakdown", $con );
								$size_id = $size_id_arr[trim(str_replace("'","",$txtSizeName))];
								if($sizeSeqArr[$size_id]=="")
								{
									$sizeSeqArr[$size_id]=$sizeseq;
								...
								if($color_size_poQty>0)
								{
									...
									$data_array_ratio.="INTO wo_po_ratio_breakdown (".$field_array_ratio.") VALUES(".$idratio.",".$hidd_job_id.",".$po_id.",".$cbo_deliveryCountry_id.",".$cbo_gmtsItem_id.",".$cbo_code_id.",".$cbo_country_id.",".$cbo_countryCode_id.",".$txt_countryShip_date.",'".$color_id."','".$size_id."',".$$txt_colorSizeRatioQty.",".$$txt_colorSizeRatioRate.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
								}
							}
							//echo $data_array1;
						}
			...
			if (str_replace("'","",$update_id_details)=="")
			{
				...
				if($rID==1 && $flag==1) $flag=1; else { $flag=0; echo "10**PO"; oci_rollback($con); disconnect($con); die; }
			}
			else
			{
				$rID=sql_update("wo_po_break_down",$field_array,$data_array,"id","".$update_id_details."",0);
				if($rID==1 && $flag==1) $flag=1; else { $flag=0; echo "10**POUP"; oci_rollback($con); disconnect($con); die; }
			}
			
			if($flag==1)
			{
				$querycolorSizeBreakDown="INSERT ALL ".$data_array1." SELECT * FROM dual";
				$rID1=execute_query($querycolorSizeBreakDown);
				if($rID1==1 && $flag==1) $flag=1; else { $flag=0; echo "10**COLOR"; oci_rollback($con); disconnect($con); die; }
			}
			if($flag==1)
			{
				$rIDratio=1;
				...
				{
					$queryRatio="INSERT ALL ".$data_array_ratio." SELECT * FROM dual";
					$rIDratio=execute_query($queryRatio);
					if($rIDratio==1 && $flag==1) $flag=1; else { $flag=0; echo "10**RATIO"; oci_rollback($con); disconnect($con); die; }
				}
			}
			//============================================================================================
			...
			if($flag==1)
			{
				...
				foreach($sample_tag as $sample_tag_row)
				{
					foreach ( $data_array_sample as $row_sam1 )
					{
						...
						if( $idsm[csf('id')] =='')
						{
							$id_sm= return_next_id_by_sequence("po_sample_approval_info_seq","wo_po_sample_approval_info", $con );
				...
				if($data_array_sm !='')
				{
					...
					if($rID3==1 && $flag==1) $flag=1; else { $flag=0; echo "10**SAMPLE"; oci_rollback($con); disconnect($con); die; }
				}
			}
			//============================================================================================
			if($flag==1)
			{
				...
				foreach ( $data_array_lapdip as $row_lap1 )
				{
					...
					if( $idlap[csf('id')] =='')
					{
						$id_lap= return_next_id_by_sequence(  "po_lapdip_approval_info_seq",   "wo_po_lapdip_approval_info", $con );
				...
				if($data_array_lap !='')
				{
					...
					if($rID4==1 && $flag==1) $flag=1; else { $flag=0; echo "10**LAB"; oci_rollback($con); disconnect($con); die; }
				}
			}
			if($flag==1)
			{
			...
			if (str_replace("'","",$update_id_details)!="")
			{
				...
				if(str_replace("'","",$cbo_breakdown_type)==2 || str_replace("'","",$cbo_breakdown_type)==3)
				{
					...
					foreach($sql_destination as $row)
					{
						...
						$idDest= return_next_id_by_sequence(  "po_destination_info_seq",   "wo_po_destination_info", $con );
						if ($add_comma!=0) $data_array_ratio .=",";
	...
	else if ($operation==1) // Update Here
	{
		...
		if($data_array1!="" && $flag==1)
		{
			...
			if($rID4==1 && $flag==1) $flag=1; else { $flag=0; echo "10**COLORINSERT"; oci_rollback($con); disconnect($con); die; }
		}
		...
		if($data_array_up!="" && $flag==1)
		{
			...
			if($rID1==1 && $flag==1) $flag=1; else { $flag=0; echo "10**COLORUP"; oci_rollback($con); disconnect($con); die; }
		}
		...
		if($flag==1)
		{
			...
			foreach($colSizeUpdate_id_array as $id_val)
			{
				...
				if($rID5==1 && $flag==1) $flag=1; else { $flag=0; echo "10**COLORDEL"; oci_rollback($con); disconnect($con); die; }
			}
			...
			if(count($deleted_size_id)>0){
				...
				foreach($deleted_size_id as $key=>$id_val)
				{
					if($id_val>0){
						...
						if($rID90==1 && $flag==1) $flag=1; else { $flag=0; echo "10**COLORDEL2"; oci_rollback($con); disconnect($con); die; }
					}				
				}
			}
		}
		...
		if($flag==13)
		{
			...
			{
				$rID4=sql_insert("wo_po_lapdip_approval_info",$field_array_lap,$data_array_lap,1);
				if($rID4==1 && $flag==1) $flag=1; else { $flag=0; echo "10**LABDIP"; oci_rollback($con); disconnect($con); die; }
			}
			...
			if($breakdown_type==2 || $breakdown_type==3)
			{
				...
				if(implode(',',$distance_ratio_delete_id)!="")
				{
					foreach($distance_ratio_delete_id as $id_val)
					{
						...
						if($rIDdelratio==1 && $flag==1) $flag=1; else { $flag=0; echo "10**RATIODEL"; oci_rollback($con); disconnect($con); die; }
					}
				}
				...
				{
					$rIDratio=sql_insert("wo_po_ratio_breakdown",$field_array_ratio,$data_array_ratio,0);
					if($rIDratio==1 && $flag==1) $flag=1; else { $flag=0; echo "10**RATIOINSERT"; oci_rollback($con); disconnect($con); die; }
				}
				...
				if($rIDratioUp==1 && $flag==1) $flag=1; else { $flag=0; echo "10**RATIOUP"; oci_rollback($con); disconnect($con); die; }
			}
		}
...
if($action=="save_update_delete_accpoinfo")// zakaria joy 28-05-22(8883)
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$mst_id= return_next_id_by_sequence(  "po_acc_po_info_seq",   "wo_po_acc_po_info", $con );
		...
		for ($i=1; $i<=$total_row; $i++)
		{
			...
			$povalue="povalue_".$i;
			$rowid="rowid_".$i;
			$id= return_next_id_by_sequence("po_acc_po_info_dtls_seq","wo_po_acc_po_info_dtls",$con );
			if ($i!=1) $data_array_dtls .=",";
...
if($action=="save_update_delete_destination_info")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		for ($i=1;$i<=$total_row;$i++)
		{
			...
			$txt_qty="txtQty_".$i;
			$id= return_next_id_by_sequence("po_destination_info_seq","wo_po_destination_info",$con );
			if ($i!=1) $data_array .=",";
...
if($action=="get_size_template"){
	$data=explode("_",$data);
	$get_size_data=sql_select("SELECT size_name, sequence from buyer_wise_size where buyer_id=$data[0] and brand_id=$data[1] and product_dept=$data[2] and pro_sub_dep=$data[3] and status_active=1 group by size_name, sequence order by sequence ASC");
	$size_arr=array();
	foreach($get_size_data as $row){
		$size_arr[$row[csf('size_name')]]=$row[csf('size_name')];
	}
