...
?>
	<script>
		...
		function check() {
			...
			show_list_view(cbo_company_name + '_' + cbo_buyer_name + '_' + cbo_supplier_name + '_' + cbo_year_selection + '_' + cbo_currency + '_' + txt_style + '_' + txt_order_search + '_' + txt_job + '_' + cbo_item + '_' + txt_ref_no + '_' + '<? echo $txt_booking_no; ?>' + '_' + '<? echo $cbo_level; ?>' + '_' + cbo_isshort, 'create_fnc_process_data', 'search_div', 'print_booking_multijob_controller', setFilterGrid('tbl_list_search', -1))
		}
	</script>
...
?>
	...
	<body>
		<div style="width:1220px;">
			...
			<caption> <b style=" color:#F00"> Fullship Po not available.</b> </caption>
				<thead>
					<th width="20">SL</th>
			...
			<div style="width:1320px; overflow-y:scroll; max-height:350px;" id="buyer_list_view">
				<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1300" class="rpt_table" id="tbl_list_search">
					...
					$sql_cu_booking = sql_select("select c.pre_cost_fabric_cost_dtls_id,c.po_break_down_id,c.gmt_item, sum(c.wo_qnty) as cu_wo_qnty, sum(c.amount) as cu_amount from wo_po_details_master a, wo_po_break_down  d, wo_booking_mst b,wo_booking_dtls c where a.id=d.job_id and a.job_no=c.job_no and b.booking_no=c.booking_no and  d.id=c.po_break_down_id and a.company_name=$company_id and a.buyer_name=$cbo_buyer_name  and b.booking_type=6  and c.booking_type=6 and b.entry_form=201 and d.is_deleted=0 and d.status_active=1  and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  $job_cond $order_cond $ref_cond $style_cond $year_cond group by a.job_no, c.pre_cost_fabric_cost_dtls_id,c.po_break_down_id,c.gmt_item");
...
if($action=="color_popup") 
{
    ...
    <div align="center" style="width:100%;">
    <form>
		<? $sql_data=sql_select("select color_number_id  from wo_po_color_size_breakdown   WHERE po_break_down_id in($txt_selected_trim_id) and  status_active=1 and is_deleted=0 group by color_number_id"); 
		//echo "select color_number_id  from wo_po_color_size_breakdown   WHERE po_break_down_id in($txt_selected_trim_id) and  status_active=1 and is_deleted=0 group by color_number_id";
		?>
		...
		<thead>
         ...
         <div style="width:420px; overflow-y:scroll; max-height:340px;" >
			...
			<?
            $couArr=array();
            $i=1;$color_row_id = ''; //$nIdcouSArr=array(); $nNamecouSArr=array();
            foreach($sql_data as $row)
            {
                $couArr['id'][trim($row[csf('color_number_id')])]=trim($row[csf('color_number_id')]);
                $couArr['name'][trim($row[csf('color_number_id')])]= $color_library[trim($row[csf('color_number_id')])];
                if ($i%2==0)  $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
					$countrySArr=explode(",",trim($color_ids));
					if (in_array($row[csf('color_number_id')], $countrySArr)) 
					{
                ...
                <tr bgcolor="<? echo $bgcolor;  ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value(<? echo $i;?>)"> 
                    ...
                    <td width="200">
                    <input type="hidden" name="txt_individual_id<?php $i; ?>" id="txt_individual_id<?php echo $i; ?>" value="<? echo trim($row[csf('color_number_id')]); ?>"/>
                    <input type="hidden" name="txt_individual_name<?php echo $i; ?>" id="txt_individual_name<?php echo $i; ?>" value="<? echo $color_library[trim($row[csf('color_number_id')])]; ?>"/>
                    <? echo $color_library[$row[csf('color_number_id')]]; ?>
                    </td>
                </tr>
                <?
                $i++;
           ...
           $nIdcouSArr=array(); $nNamecouSArr=array();
            $couSArr=explode(",",trim($color_ids));
        ...
        <table width="400" cellspacing="0" cellpadding="0" style="border:none" align="center">
            <tr>
                <td align="center" height="30" valign="bottom">
                    <div style="width:100%">
                        <div style="width:50%; float:left" align="left">
                         <input type="hidden" name="txt_country_row_id" id="txt_country_row_id" value="<?php echo $color_row_id; ?>"/>
...
if ($action == "generate_fabric_booking") {
	...
	$cu_booking_arr = array();
	$sql_cu_booking = sql_select("select c.job_no,c.pre_cost_fabric_cost_dtls_id,c.po_break_down_id,c.gmt_item, sum(c.wo_qnty) as cu_wo_qnty, sum(c.amount) as cu_amount from wo_po_details_master a, wo_po_break_down  d , wo_booking_mst b, wo_booking_dtls c where a.id=d.job_id and a.job_no=c.job_no and  b.booking_no=c.booking_no and b.entry_form=201 and  d.id=c.po_break_down_id and a.company_name=$cbo_company_name  and c.status_active=1 and c.is_deleted=0 and c.booking_type=6   group by c.job_no, c.pre_cost_fabric_cost_dtls_id,c.po_break_down_id,c.gmt_item");
	foreach ($sql_cu_booking as $row_cu_booking) {
	...
	a.buyer_name,
	a.currency_id,
	a.style_ref_no,a.order_uom, b.entry_from,
	b.costing_per,
	b.exchange_rate,
	c.id as pre_cost_emb_cost_dtls_id,
	c.emb_name,
	c.emb_type,
	c.body_part_id,
	c.country,
	...
	wo_po_break_down d,
	wo_pre_cos_emb_co_avg_con_dtls e
	...
	c.id=e.pre_cost_emb_cost_dtls_id and
	...
	d.po_quantity,
	e.po_break_down_id,
	e.item_number_id, a.order_uom, b.entry_from
	order by d.id,c.id";
	//echo $sql; die;
	...
	foreach ($nameArray as $selectResult) {
		...
		//echo $req_qnty_cons_uom.'='.$cu_woq.'<br>';
				...
				?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i; ?>">
						...
						<td width="80" align="right">
							...
							<input type="text" style="width:100%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtwoq_<? echo $i; ?>" value="<? echo number_format($bal_woq, 4, '.', ''); ?>" onClick="open_consumption_popup('requires/print_booking_multijob_controller.php?action=consumption_popup', 'Consumtion Entry Form','txtpoid_<? echo $i; ?>',<? echo $i; ?>)" readonly />
						</td>
...
?>
	<script>
		...
		function poportionate_qty(qty) {
			...
			for (var i = 1; i <= rowCount; i++) {
				...
				//var checkcolorid = $('#checkcolorid_' + i).val();
				//alert(checkcolorid);
		...
		function poportionate_qty_check_color(qty) {
			var txtwoq = document.getElementById('txtwoq').value;
			var txtwoq_qty = document.getElementById('txtwoq_qty').value * 1;
			var rowCount = $('#tbl_consmption_cost tbody tr').length;
			for (var i = 1; i <= rowCount; i++) {
	...
	<body>
		<?
		extract($_REQUEST);
		$chk_color_idArr=$selected_color_id;
			// echo $color_id.'fff';;
			if($chk_color_idArr)
			{
				$check_color=1;	
	 ...
	 // echo $check_color.'T';
	 //print_r($color_idArr);
	 if($selected_color_id) $color_id_cond="and c.color_number_id in($selected_color_id)";
	 else $color_id_cond="";
		...
		if($check_color==1) //Select color from main page popup
		{
			
			$po_ids=str_replace("'", "", $txt_po_id);
			$po_idArr=array_unique(explode(",",$po_ids));
			//print_r($txt_po_idArr);
			$selected_color_idArr=array_unique(explode(",",str_replace("'", "", $selected_color_id)));
			if ($cbo_colorsizesensitive == 4) 
			{
				$tot_color_req_qty=0;
				$pre_embcostid=str_replace("'", "", $txtembcostid);
				$gmtitemid=str_replace("'", "", $txtgmtitemid);
				if ($emb_name == 3) {
					$req_qty_arr_wash2 = $wash->getQtyArray_by_OrderEmblishmentidGmtscolorAndGmtsitem();
					$req_amount_arr_wash2 = $wash->getAmountArray_by_OrderEmblishmentidGmtscolorAndGmtsitem();
					foreach ($po_idArr as $pid) 
					{
						foreach ($selected_color_idArr as $cid) 
						{
						$tot_color_req_qty+=$req_qty_arr_wash2[$pid][$pre_embcostid][$cid][$gmtitemid];
						}
					}
				}
				else
				{
					$req_qty_arr2 = $emblishment->getQtyArray_by_OrderEmblishmentidGmtscolorAndGmtsitem();
					$req_amount_arr2 = $emblishment->getAmountArray_by_OrderEmblishmentidGmtscolorAndGmtsitem();
					//print_r($po_idArr);
					...
					foreach ($po_idArr as $pid) 
					{
						foreach ($selected_color_idArr as $cid) 
						{
						$tot_color_req_qty+= $req_qty_arr2[$pid][$pre_embcostid][$cid][$gmtitemid];
						//$tot_color_req_qty+=$req_qty_arr2[$pid][$txtembcostid][$cid][$gmtitemid];
						}
						
					}
				}
				// echo $tot_color_req_qty.'D';
				$txt_req_quantity='';
				if ($txt_update_dtls_id!="") 
				{
					//$txtwoq='';
						...
						$txtwoq='';
						$txtwoq=$tot_color_req_qty;
					}
				$txt_req_quantity=$tot_color_req_qty;
			}
		}
						...
						<tbody>
							...
							if (count($data_array) > 0 && $cbo_level == 1) {
								...
								foreach ($data_array as $row) {
									...
									if ($txtwoq_cal > 0) {
										$i++;
										$color_IdArr=array_unique(explode(",",$chk_color_idArr));
										//print_r($color_IdArr);
										$color_checkId=1;
										if(in_array($row[csf('color_number_id')],$color_IdArr))
										{
											$color_checkId=0;
										}
										//echo $color_checkId;
							...
							?>
										<tr id="break_1" align="center">
											...
											<td>
												...
												<input type="hidden" id="checkcolorid_<? echo $i; ?>" name="checkcolorid_<? echo $i; ?>" class="text_boxes" style="width:100px" value="<? echo $color_checkId; ?>" readonly />
											...
											<td>
									<input type="text" id="rate_<? echo $i; ?>" name="rate_<? echo $i; ?>" class="text_boxes_numeric" style="width:120px" onChange="calculate_amount(<? echo $i; ?>);set_sum_value( 'amount_sum', 'amount_' );copy_value(this.value,'rate_',<? echo $i; ?>) " value="<? echo fn_number_format($rate,4); ?>" <? if ($piNumber || $recvNumber) {
																																																																																	echo  "disabled";
																																																																																} else {
																																																																																	echo "";
																																																																																} ?> />
								...
								foreach ($data_array as $row) {
									...
									?>
										<tr id="break_1" align="center">
											...
											<td><input type="text" id="rate_<? echo $i; ?>" name="rate_<? echo $i; ?>" class="text_boxes_numeric" style="width:120px" onChange="calculate_amount(<? echo $i; ?>);set_sum_value( 'amount_sum', 'amount_' );copy_value(this.value,'rate_',<? echo $i; ?>) " value="<? echo fn_number_format($rate,4); ?>" <? if ($piNumber || $recvNumber) {
																																																																																	echo  "disabled";
																																																																																} else {
																																																																																	echo "";
																																																																																} ?> />
...
if ($action == "save_update_delete") {
	$process = array(&$_POST);
	extract(check_magic_quote_gpc($process));
	$lock_another_process = '';$service_lock_another_process = '';
	...
	if (str_replace("'", "", $txt_booking_no) != '') {
		...
		if ($operation == 2) //Delete Check.......
		{
			//echo "10**=select system_no from wo_service_acknowledgement_mst where wo_booking_no=$txt_booking_no and status_active=1 and is_deleted=0";die;
			$sql_service_ack = sql_select("select system_no from wo_service_acknowledgement_mst where wo_booking_no=$txt_booking_no and status_active=1 and is_deleted=0");
			foreach ($sql_service_ack as $row) {
	
				if ($row[csf('system_no')] == "") $row[csf('system_no')] = $row[csf('system_no')];
				$service_lock_another_process = $row[csf('system_no')];
			}
			if ($service_lock_another_process != '') {
				echo "serviceacklockAnotherProcess**" . $service_lock_another_process;
				disconnect($con);
				die;
			}
	    }
	}
	//echo $lock_another_process; die;service_ack_lockAnotherProcess
	...
	if ($operation == 0)  // Insert Here
	{
		...
		if ($db_type == 0) {
			$new_booking_no = explode("*", return_mrr_number(str_replace("'", "", $cbo_company_name), '', 'EB', date("Y", time()), 5, "select booking_no_prefix, booking_no_prefix_num from wo_booking_mst where company_id=$cbo_company_name and booking_type=6 and entry_form=201 and YEAR(insert_date)=" . date('Y', time()) . " order by id desc ", "booking_no_prefix", "booking_no_prefix_num"));
		} else if ($db_type == 2) {
			$new_booking_no = explode("*", return_mrr_number(str_replace("'", "", $cbo_company_name), '', 'EB', date("Y", time()), 5, "select booking_no_prefix, booking_no_prefix_num from wo_booking_mst where company_id=$cbo_company_name and booking_type=6 and entry_form=201 and to_char(insert_date,'YYYY')=" . date('Y', time()) . " order by id desc ", "booking_no_prefix", "booking_no_prefix_num"));
		}
		...
		$data_array = "(" . $id . ",6," . $cbo_isshort . ",'" . $new_booking_no[1] . "'," . $new_booking_no[2] . ",'" . $new_booking_no[0] . "'," . $cbo_company_name . "," . $cbo_buyer_name . "," . $cbo_currency . ",25," . $cbo_pay_mode . "," . $cbo_source . "," . $txt_booking_date . "," . $txt_delivery_date . "," . $hidden_supplier_id . "," . $txt_attention . "," . $txt_tenor . "," . $cbo_ready_to_approved.",".$txt_delivery_to . ",201," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "'," . $cbo_level . "," . $remarks . ")";
	...
	} else if ($operation == 1)   // Update Here
	{
		...
		$data_array = "" . $cbo_buyer_name . "*" . $cbo_currency . "*25*" . $cbo_pay_mode . "*" . $cbo_source . "*" . $txt_booking_date . "*" . $txt_delivery_date . "*" . $hidden_supplier_id . "*" . $txt_attention . "*" . $txt_tenor . "*" . $txt_delivery_to. "*" . $cbo_ready_to_approved  . "*201*" . $cbo_isshort . "*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'*" . $cbo_level . "*" . $remarks . "";
	...
	} else if ($operation == 2)   // Delete Here
	{
		...
		$sql_res_emb_recv = sql_select("select b.po_break_down_id from wo_booking_dtls b,pro_garments_production_mst c where   b.po_break_down_id=c.po_break_down_id and  b.booking_no=$txt_booking_no  and c.production_type=3 and b.status_active=1 and c.status_active=1");
		if(count($sql_res_emb_recv)>0)
		{
			echo "embRecv**Emblish Recv Found,Deleted not allowed";
			disconnect($con);
			die;
		}
	 
		$sql = "SELECT c.po_break_down_id as po_id,c.item_number_id,c.color_number_id,b.cost_of_fab_per_pcs,b.cut_oh_per_pcs,b.cost_per_pcs,b.bundle_no,b.production_qnty,(b.cost_per_pcs*b.production_qnty) as amount from pro_garments_production_mst a,pro_garments_production_dtls b,wo_po_color_size_breakdown c where a.id=b.mst_id and b.color_size_break_down_id=c.id and b.status_active=1 and b.is_deleted=0 and b.production_type=3   and c.po_break_down_id=$hidden_po_break_down_id and c.item_number_id=$cbo_item_name order by a.production_type asc";
...
if ($action == "save_update_delete_dtls") {
	...
	if ($operation == 0) {
		...
		$field_array1 = "id,booking_mst_id, pre_cost_fabric_cost_dtls_id, po_break_down_id, job_no,gmt_item, booking_no, booking_type, is_short, uom, sensitivity, wo_qnty, exchange_rate, rate, amount, delivery_date,country_id_string,selected_colorid, inserted_by, insert_date";
		...
		for ($i = 1; $i <= $total_row; $i++) {
			...
			$data_array1 = "(" . $id_dtls . "," . $booking_mst_id . "," . $$txtembcostid . "," . $$txtpoid . "," . $$txtjob_id . "," . $$txtgmtitemid . "," . $txt_booking_no . ",6,2," . $$txtuom . "," . $$cbocolorsizesensitive . "," . $$txtwoq . "," . $$txtexchrate . "," . $$txtrate . "," . $$txtamount . "," . $$txtddate . "," . $$txtcountry . ",'" . $selected_color_id . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
		...
		$field_array_up1 = "pre_cost_fabric_cost_dtls_id*po_break_down_id*job_no*gmt_item*uom*sensitivity*wo_qnty*exchange_rate*rate*amount*delivery_date*country_id_string*selected_colorid*updated_by*update_date";
		...
		for ($i = 1; $i <= $total_row; $i++) {
			...
			if (str_replace("'", '', $$txtbookingid) != "") {
				...
				$data_array_up1[str_replace("'", '', $$txtbookingid)] = explode("*", ("" . $$txtembcostid . "*" . $$txtpoid . "*" . $$txtjob_id . "*" . $$txtgmtitemid . "*" . $$txtuom . "*" . $$cbocolorsizesensitive . "*" . $$txtwoq . "*" . $$txtexchrate . "*" . $$txtrate . "*" . $$txtamount . "*" . $$txtddate . "*" . $$txtcountry . "*'" . $selected_color_id . "'*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'"));
...
if ($action == "save_update_delete_dtls_job_level") {
	...
	$process = array(&$_POST);
	extract(check_magic_quote_gpc($process));
	$selected_color_id=str_replace("'", "", $selected_color_id);
	$is_approved = return_field_value("is_approved", "wo_booking_mst", "booking_no=$txt_booking_no");
	if ($is_approved == 1) {
		echo "app1**" . str_replace("'", "", $txt_booking_no);
		disconnect($con);
	...
	if ($operation == 0) {
		...
		for ($i = 1; $i <= $total_row; $i++) {
			...
			foreach ($strdata->$job->$embcostid->$gmtitemid->po_id as $poId) {
				...
				$amount = $wqQty * $rate;
				$data_array1 = "(" . $id_dtls . "," . $booking_mst_id . "," . $$txtembcostid . "," . $poId . "," . $$txtjob_id . "," . $$txtgmtitemid . "," . $txt_booking_no . ",6,2," . $$txtuom . "," . $$cbocolorsizesensitive . "," . $wqQty . "," . $$txtexchrate . "," . $$txtrate . "," . $amount . "," . $$txtddate . "," . $$txtcountry . ",'" . $selected_color_id . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
				...
				if (str_replace("'", '', $$consbreckdown) != '') {
					...
					for ($c = 0; $c < count($consbreckdown_array); $c++) {
						...
						//"id,wo_booking_dtls_id,booking_no,job_no,po_break_down_id,color_number_id,gmts_sizes,description,item_color,item_size,cons, process_loss_percent, requirment, rate, amount, pcs,color_size_table_id,article_number"
						//=pocolorid+'_'+gmtssizesid+'_'+des+'_'+itemcolor+'_'+itemsizes+'_'+qty+'_'+excess+'_'+woqny+'_'+rate+'_'+amount+'_'+pcs+'_'+colorsizetableid+'_'+reqqty+'_'+poarticle;
						...
						//echo "10** insert into wo_emb_book_con_dtls (".$field_array2.") values ".$data_array2;check_table_status($_SESSION['menu_id'], 0); die;
					}
				} //CONS break down end==============================================================================================
				$id_dtls = $id_dtls + 1;
			}
...
//selected_color_id
			if (str_replace("'", '', $$txtbookingid) != "") {
				foreach ($strdata->$job->$embcostid->$gmtitemid->po_id as $poId) {
					...
					$data_array_up1[str_replace("'", '', $strdata->$job->$embcostid->$gmtitemid->booking_id->$poId)] = explode("*", ("" . $$txtembcostid . "*" . $poId . "*" . $$txtjob_id . "*" . $gmtitemid . "*" . $$txtuom . "*" . $$cbocolorsizesensitive . "*" . $wqQty . "*" . $$txtexchrate . "*" . $$txtrate . "*" . $amount . "*" . $$txtddate . "*" . $$txtcountry . "*'" . $selected_color_id . "'*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'"));
...
if ($action == "show_trim_booking") {
	...
	order by d.id,c.id";
	$job_and_trimgroup_level = array();$selected_colorIdArrr=array();$selected_colorArrr=array();
	$i = 1;
	$nameArray = sql_select($sql);
	foreach ($nameArray as $infr) {
		...
		if($infr[csf('body_part_id')]=="" ) $infr[csf('body_part_id')]="";
		if($infr[csf('uom')]=="" ) $infr[csf('uom')]="";
		if($infr[csf('country')]=="" ) $infr[csf('country')]="";
		if($infr[csf('sensitivity')]=="" ) $infr[csf('sensitivity')]="";
		if($infr[csf('booking_id')]=="" ) $infr[csf('booking_id')]="";
		if($infr[csf('delivery_date')]=="" ) $infr[csf('delivery_date')]="";
		if($body_part[$infr[csf('body_part_id')]]=="" ) $body_part[$infr[csf('body_part_id')]]="";
		...
		<tbody>
			...
			if ($cbo_level == 1) {
				foreach ($nameArray as $selectResult) {
					...
					// if (!empty($selectResult[csf('uom')])) {
					// 	$uom_id = $selectResult[csf('uom')];
					// } else {
					// 	$uom_id = 2;
					// }
					$uom_id = $selectResult[csf('uom')];
				...
				foreach ($job_and_trimgroup_level as $job_no) {
					...
					?>
							<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i; ?>">
								...
								<td width="80" align="right">
									<input type="text" style="width:100%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtwoq_<? echo $i; ?>" value="<? echo number_format($woq, 4, '.', '');$tot_woq+=$woq; ?>" onClick="open_consumption_popup('requires/print_booking_multijob_controller.php?action=consumption_popup', 'Consumtion Entry Form','txtpoid_<? echo $i; ?>',<? echo $i; ?>)" readonly />
								</td>
								<td width="55" align="right">
									<input type="text" style="width:100%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtexchrate_<? echo $i; ?>" value="<? echo $exchange_rate; ?>" readonly />
								</td>
...
?>
	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1350" class="rpt_table">
		...
		<tbody id="save_list">
			...
			if ($cbo_level == 1) {
				foreach ($nameArray as $selectResult) {
					...
					$total_amount += $amount;
					$uom_name =$unit_of_measurement[$selectResult[csf('uom')]];
					// $uom_id = 0;
		...
		$selected_coloridArr=array_unique(explode(",",$selectResult[csf('selected_colorid')]));
		foreach($selected_coloridArr as $cid)
		{
			$selected_colorArrr[$color_library[$cid]]=$color_library[$cid];
			$selected_colorIdArrr[$cid]=$cid;
		}
			?>
					<tr bgcolor="<?= $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<?= $i; ?>" onClick="fnc_show_booking(<?= $selectResult[csf('wo_pre_cost_embe_cost_dtls')]; ?>,'<?= $selectResult[csf('po_id')]; ?>','<?= $selectResult[csf('booking_id')]; ?>','<?= $selectResult[csf('job_no')]; ?>','<?= implode(",",$selected_colorArrr); ?>','<?= implode(",",$selected_colorIdArrr); ?>')">
						<td width="40"><? echo $i; ?></td>
				...
				<table width="950" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
					...
					<tr class="general">
						<td> <input type="hidden" id="selected_booking">
							<?
							echo create_drop_down("cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 and core_business not in(3) $company_cond order by company_name", "id,company_name", 1, "-- Select Company --", $cbo_company_name, "load_drop_down( 'print_booking_multijob_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
							?>
						</td>
...
if ($action == "populate_data_from_search_popup") {
	...
	foreach ($data_array as $row) {
		...
		echo " $('#cbo_isshort').attr('disabled',true);\n";
...
if ($action == "show_trim_booking_report2") {
	...
	$sql="select id from electronic_approval_setup where company_id=$cbo_company_name and page_id in(1162) and is_deleted=0";
	...
	$inserted_by=$nameArray_buyer[0][csf('inserted_by')];
	...
	$nameArray = sql_select("SELECT a.booking_no, a.pay_mode,a.buyer_id, a.booking_date, a.supplier_id, a.currency_id, a.exchange_rate, a.attention, a.delivery_date, a.source, a.remarks, a.revised_no, a.inserted_by from wo_booking_mst a where a.booking_no='$txt_booking_no' and a.status_active =1 and a.is_deleted=0");
...
?>
						...
						?>
							<tr>
								...
								foreach ($nameArray_item_description as $result_itemdescription) {
									...
									$rate=$result_itemdescription[csf('amount')]/$result_itemdescription[csf('cons')];
								?>
									...
									<td style="border:1px solid black; text-align:right"><? echo number_format($rate, 4); ?> </td>
									<td style="border:1px solid black; text-align:right">
										<?
										$amount_as_per_gmts_color = $result_itemdescription[csf('cons')] *$rate;
			...
			?>
				<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1100" class="rpt_table">
					...
					b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and b.po_break_down_id in($po_idss) group by  b.po_break_down_id, b.booking_no,b.pre_cost_fabric_cost_dtls_id,a.exchange_rate"); //and b.is_short=2
					 ...
					 booking_type=6  and status_active=1 and is_deleted=0 and po_break_down_id in($po_idss) group by po_break_down_id,pre_cost_fabric_cost_dtls_id"); //and is_short=2
	...
	<table width="780" align="center">
			<tr>
				<div style="text-align:center;font-size:xx-large; font-style:italic; margin-top:20px; color:#FF0000;">
						...
						{				
							if($id_approved_id == 0){echo "Draft";}else{}
						}		
						?>
				</div>
			</tr>
	...
	<div style="margin-top:-5px;">
		<?
			 $appSql = "select a.APPROVED_BY, a.APPROVED_DATE,b.USER_FULL_NAME,c.CUSTOM_DESIGNATION from approval_mst a,USER_PASSWD b,LIB_DESIGNATION c where a.mst_id in($po_idss) and a.APPROVED_BY=b.id and b.DESIGNATION=c.id and c.STATUS_ACTIVE=1 and c.IS_DELETED=0 and b.STATUS_ACTIVE=1 and b.IS_DELETED=0
			 union all
			 select b.id as APPROVED_BY, a.INSERT_DATE as APPROVED_DATE,b.USER_FULL_NAME,c.CUSTOM_DESIGNATION from wo_po_break_down a,USER_PASSWD b,LIB_DESIGNATION c where a.id in($po_idss) and a.INSERTED_BY=b.id and b.DESIGNATION=c.id and c.STATUS_ACTIVE=1 and c.IS_DELETED=0 and b.STATUS_ACTIVE=1 and b.IS_DELETED=0";
			 //echo $appSql;die;
			...
			//    echo "</pre>";die();
			 ...
			 echo get_app_signature(133, $cbo_company_name, "1300px",$cbo_template_id, 50,$inserted_by,$userDtlsArr); 
			// echo signature_table(133,$cbo_company_name,"1300px",$cbo_template_id,10,$user_lib_name[$inserted_by]);
		?>
	</div>
	<br>
...
if ($action == "show_trim_booking_report8") {
	...
	$nameArray_buyer = sql_select("select  a.id as job_id,a.style_ref_no, a.job_no,a.order_uom, a.style_owner, a.buyer_name, a.dealing_marchant, a.season, a.season_matrix, a.season_buyer_wise, a.order_repeat_no, b.po_break_down_id from wo_po_details_master a, wo_booking_dtls b where a.job_no=b.job_no and b.booking_no='$txt_booking_no' and b.status_active =1 and b.is_deleted=0");
	foreach ($nameArray_buyer as $result_buy) {
		$buyer_string[$result_buy[csf('buyer_name')]] = $buyer_name_arr[$result_buy[csf('buyer_name')]];
		$style_owner[$result_buy[csf('job_no')]] = $company_library[$result_buy[csf('style_owner')]];
		$job_no[$result_buy[csf('job_no')]] = $result_buy[csf('job_no')];
		$job_id[$result_buy[csf('job_no')]] = $result_buy[csf('job_id')];
		$job_uom_no[$result_buy[csf('job_no')]] = $unit_of_measurement[$result_buy[csf('order_uom')]];
		$style_ref[$result_buy[csf('job_no')]] = $result_buy[csf('style_ref_no')];
		$all_dealing_marcent[$result_buy[csf('job_no')]] = $deling_marcent_arr[$result_buy[csf('dealing_marchant')]];
		$season_matrix = $result_buy[csf('season_matrix')];
	...
	$job_nos = rtrim($job_nos, ",");
	$sql_job = sql_select("select b.job_no_mst,b.id as po_id, b.po_quantity as po_quantity,a.style_owner  from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and  a.status_active =1 and a.is_deleted=0 and  b.status_active =1 and b.is_deleted=0 and b.id in(" . $po_idss . ") ");
	// echo "select b.job_no_mst,b.id as po_id, b.po_quantity as po_quantity,a.style_owner  from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and  a.status_active =1 and a.is_deleted=0 and  b.status_active =1 and b.is_deleted=0 and b.id in(" . $po_idss . ") ";
	foreach ($sql_job as $row) {
		$job_po_qty_arr[$row[csf('job_no_mst')]][$row[csf('po_id')]] += $row[csf('po_quantity')];
		$tot_po_quantity += $row[csf('po_quantity')];
 ...
 ?>
	...
	<div style="width:1333px" align="center">
		<table width="1333px" cellpadding="0" cellspacing="0" style="border:0px solid black">
					...
					<td width="20px">
						<table width="100%" cellpadding="0" cellspacing="0" style="border:0px solid black">
							<tr>
								...
								<td width="30px" align="center">
								<b style="font-size:25px;">Style Owner:<?
																echo $company_library[$style_owner]; ?>
									</b>
								<br>
									<b style="font-size:22px;">Owner Company:<?
																echo $company_library[$cbo_company_name]; ?>
									</b>
									<br>
									<label>
			...
			$booking_country_v = array();
			$colSize_sql_country=sql_select(" Select job_no_mst, po_break_down_id,country_id from wo_po_color_size_breakdown where job_id in ($job_id) and po_break_down_id in ($po_idss) and status_active=1 and is_deleted=0 group by job_no_mst, po_break_down_id,country_id");
			$country_ids_arr=array();
			foreach ($colSize_sql_country as $row) {
				
				$country_ids_arr[$row[csf('job_no_mst')]][$row[csf('po_break_down_id')]]['country_id'] .= $country_arr[$row[csf('country_id')]].",";

				// $country_id_string = explode(",", $row[csf('country_id')]);
				// $tocu = count($country_id_string);
				// for ($cu = 0; $cu < $tocu; $cu++) {
				// 	$country_ids_arr[$row[csf('job_no_mst')]][$row[csf('po_break_down_id')]][$country_id_string[$cu]] = $country_arr[$country_id_string[$cu]];
				// }

			}
			//echo "<pre>";print_r($country_ids_arr);die;
			$country_id=implode(",", array_unique($country_id_arr));
			$country_id_res=rtrim($country_id,',');
			$nameArray_job_po = sql_select("select job_no from wo_booking_dtls  where booking_no='$txt_booking_no' and status_active =1 and is_deleted=0 group by job_no order by job_no ");

			foreach ($nameArray_job_po as $nameArray_job_po_row) {
				$nameArray_item = sql_select("select  a.pre_cost_fabric_cost_dtls_id,c.emb_name, a.po_break_down_id from wo_booking_dtls a, wo_pre_cost_embe_cost_dtls c  where a.pre_cost_fabric_cost_dtls_id=c.id and  a.booking_no='$txt_booking_no' and  a.status_active =1 and a.is_deleted=0 and a.job_no='" . $nameArray_job_po_row[csf('job_no')] . "'   and a.sensitivity=1 group by a.pre_cost_fabric_cost_dtls_id,c.emb_name, a.po_break_down_id  order by c.emb_name ");
				if (count($nameArray_item) > 0) {
					$po_ids = rtrim($po_no_arr[$nameArray_job_po_row[csf('job_no')]]['po_id']);
					$po_ids = array_unique(explode(",", $po_ids));
					$ref_nos = rtrim($job_ref_no[$nameArray_job_po_row[csf('job_no')]], ',');
			...
			?>
					...
					<table border="1" align="left" class="rpt_table" cellpadding="0" width="100%" cellspacing="0" rules="all">
								...
								<table width="100%" style="table-layout: fixed;">
									<tr>
										<td width="60%" align="left"><strong>As Per Garments Color (<? echo "Job NO:" . $nameArray_job_po_row[csf('job_no')]; ?>) <? echo "Style NO:" . $style_ref[$nameArray_job_po_row[csf('job_no')]];
																																								...
																																								echo " &nbsp;  Total PO Qty.:&nbsp;" . $po_no_qty; ?></strong></td>
										<td width="40%" style="overflow:hidden;text-overflow: ellipsis;white-space: nowrap;font-weight:bold; display: none">Po No: <? echo implode(",", $po_no[$nameArray_job_po_row[csf('job_no')]]); ?></td>
									</tr>
								</table>
							</td>
						</tr>
						<tr>
							...
							<td style="border:1px solid black"><strong>Description</strong> </td>
							<td align="center" style="border:1px solid black"><strong>Item Color</strong></td>
							<td style="border:1px solid black" align="center"><strong>WO Qty[Dzn]</strong></td>
							<td style="border:1px solid black" align="center"><strong>WO Qty[Pcs]</strong></td>
							<? if($show_comment==1) {?>
							<td style="border:1px solid black" align="center"><strong>Rate[Dzn]</strong></td>
							<td style="border:1px solid black" align="center"><strong>Amount</strong></td>
							<? } ?>
						</tr>
						<?
						$i = 0;
						$grand_total_as_per_gmts_color = 0;
						$grand_total_item_desctiption = 0;
						$grand_wo_qnty_pcs_total=0;
						foreach ($nameArray_item as $result_item) { 
							$i++;
							$nameArray_item_description = sql_select("SELECT a.pre_cost_fabric_cost_dtls_id,a.gmt_item,min(b.id) as bid, b.description,b.item_color,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount, c.emb_name,c.emb_type,c.body_part_id,a.job_no from wo_booking_dtls a,  wo_emb_book_con_dtls b, wo_pre_cost_embe_cost_dtls c where a.id= b.wo_booking_dtls_id and a.booking_no=b.booking_no  and a.pre_cost_fabric_cost_dtls_id=c.id and  a.booking_no='$txt_booking_no' and a.job_no='" . $nameArray_job_po_row[csf('job_no')] . "'  and a.sensitivity=1  and c.emb_name=" . $result_item[csf('emb_name')] . " and a.pre_cost_fabric_cost_dtls_id=" . $result_item[csf('pre_cost_fabric_cost_dtls_id')] . " and b.requirment !=0 and  a.status_active =1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.po_break_down_id=" . $result_item[csf('po_break_down_id')] . " group by a.pre_cost_fabric_cost_dtls_id,a.gmt_item, b.description,b.item_color,c.emb_name,c.emb_type,c.body_part_id,a.job_no order by bid ");
						...
						?>
							<tr>
								<td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description) + 1; ?>"> <? echo $i; ?></td>
								<td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description) + 1; ?>"> <? echo $po_number_arr[$result_item[csf('po_break_down_id')]]; ?></td>
								<td align="center" style="word-break:break-all" style="border:1px solid black" rowspan="<? echo count($nameArray_item_description) + 1; ?>">
									<?
									echo $emblishment_name_array[$result_item[csf('emb_name')]] . "/<br>";
									$country_value= implode(",",array_unique(explode(",",$country_ids_arr[$result_item[csf('job_no')]][$result_item[csf('po_break_down_id')]]['country_id'])));
									echo chop($country_value,",");
									?>
								</td>
								<?
								$item_desctiption_total = 0;
								...
								?>
									...
									<td style="border:1px solid black; text-align:right"><? echo number_format($result_itemdescription[csf('cons')], 4);
																							$item_desctiption_total += $result_itemdescription[csf('cons')]; ?></td>
									<td style="border:1px solid black; text-align:right"><? echo number_format($result_itemdescription[csf('cons')]*12, 4);
																							$wo_qnty_pcs_total += $result_itemdescription[csf('cons')]*12; ?></td>
																							<? if($show_comment==1) {?>
									<td style="border:1px solid black; text-align:right"><? echo number_format($result_itemdescription[csf('rate')], 4); ?> </td>
									<td style="border:1px solid black; text-align:right">
										<?
						...
						<tr>
							...
							<td style="border:1px solid black;  text-align:right; font-weight:bold;">
								<?
								echo number_format($item_desctiption_total, 4);
								$grand_total_item_desctiption += $item_desctiption_total;
								?></td>
								<td style="border:1px solid black;  text-align:right; font-weight:bold;">
								<?
								echo number_format($wo_qnty_pcs_total, 4);
								$grand_wo_qnty_pcs_total += $wo_qnty_pcs_total;
								?></td>
								<? if($show_comment==1) {?>
							<td style="border:1px solid black; text-align:right"></td>
							<td style="border:1px solid black; text-align:right">
					...
					<tr>
						...
						<td align="right" style="border:1px solid black"><? echo number_format($grand_total_item_desctiption, 2) ?></td>
						<td align="right" style="border:1px solid black"><? echo number_format($grand_wo_qnty_pcs_total, 2) ?></td>
						<? if($show_comment==1) {?>
						<td align="right" style="border:1px solid black"></td>
						<td style="border:1px solid black; text-align:right"><? echo number_format($grand_total_as_per_gmts_color, 2);
				...
				?>
					...
					<table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all">
								...
								<table width="100%" style="table-layout: fixed;">
									<tr>
										<td width="60%"><strong>Size Sensitive (<? echo "Job NO:" . $nameArray_job_po_row[csf('job_no')]; ?>) <? echo "Style NO:" . $style_ref[$nameArray_job_po_row[csf('job_no')]];
																																			echo "&nbsp;&nbsp;Int Ref.:" . $ref_nos;
																																			echo "&nbsp;&nbsp; Total PO Qty..:" . $po_no_qty; ?></strong></td>
										<td width="40%" style="overflow:hidden;text-overflow: ellipsis;white-space: nowrap;margin-left:210px; font-weight:bold; display: none;">Po No: <? echo implode(",", $po_no[$nameArray_job_po_row[csf('job_no')]]); ?></td>
									</tr>
								</table>
							</td>
						</tr>
						...
						?>
							<tr>
								<td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description) + 1; ?>"><? echo $i; ?></td>
								<td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description) + 1; ?>">
									<? echo $po_number_arr[$result_item[csf('po_break_down_id')]]; ?>
								</td>
								...
								?>
									...
									<td style="border:1px solid black; text-align:right">
										<?
										echo number_format($result_itemdescription[csf('cons')]*12, 4);
										$wo_qnty_pcs_total += $result_itemdescription[csf('cons')]*12;
										?>
									</td>
									<? if($show_comment==1) {?>
									<td style="border:1px solid black; text-align:right"><? echo number_format($result_itemdescription[csf('rate')], 4); ?> </td>
					...
					<tr>
						...
						<? if($show_comment==1) {?>
						<td align="right" style="border:1px solid black"></td>
						<td style="border:1px solid black; text-align:right"><? echo number_format($grand_total_as_per_gmts_size, 2);
																				$booking_grand_total += $grand_total_as_per_gmts_size; ?></td>
																				<? } ?>
					</tr>
					</table>
					<br />
				...
				<!--==============================================AS PER GMTS Color & SIZE START=========================================  -->
				<?
				$nameArray_item = sql_select("select  a.pre_cost_fabric_cost_dtls_id,c.emb_name, a.po_break_down_id,a.job_no from wo_booking_dtls a, wo_pre_cost_embe_cost_dtls c  where a.pre_cost_fabric_cost_dtls_id=c.id and  a.booking_no='$txt_booking_no' and  a.status_active =1 and a.is_deleted=0 and a.job_no='" . $nameArray_job_po_row[csf('job_no')] . "'   and a.sensitivity=4 group by a.pre_cost_fabric_cost_dtls_id,c.emb_name, a.po_break_down_id,a.job_no  order by c.emb_name ");
				if (count($nameArray_item) > 0) {
					$po_ids = rtrim($po_no_arr[$nameArray_job_po_row[csf('job_no')]]['po_id']);
					$po_ids = array_unique(explode(",", $po_ids));
					$ref_nos = rtrim($job_ref_no[$nameArray_job_po_row[csf('job_no')]], ',');
		...
		<div align="left" style="margin:5px 2px;float:left;width:100%">
			<? foreach ($data_array as $inf) { ?>
				<img src='../../<? echo $inf[csf("image_location")]; ?>' height='130' width='140' />
			<? } ?>
		</div>
	</div>
	<!--class="footer_signature"-->
...
if ($action == "show_trim_booking_report9") {
	extract($_REQUEST);
	$process = array(&$_POST);
	extract(check_magic_quote_gpc($process));
	$txt_booking_no = str_replace("'", "", $txt_booking_no);
	...
	$supplier_name_arr = return_library_array("select id,supplier_name from   lib_supplier", 'id', 'supplier_name');
	$supplier_address_arr = return_library_array("select id,address_1 from   lib_supplier", 'id', 'address_1');
	$supplier_contract_per_arr = return_library_array("select id,contact_person from   lib_supplier", 'id', 'contact_person');
	$supplier_contract_no_arr = return_library_array("select id,contact_no from   lib_supplier", 'id', 'contact_no');
	$buyer_name_arr = return_library_array("select id,buyer_name from lib_buyer", 'id', 'buyer_name');
	$order_uom_arr = return_library_array("select id,order_uom  from lib_item_group", "id", "order_uom");
	$deling_marcent_arr = return_library_array("select id,team_member_name from lib_mkt_team_member_info", "id", "team_member_name");
	$deling_marcent_desg_arr = return_library_array("select id,designation from lib_mkt_team_member_info", "id", "designation");
	$deling_marcent_num_arr = return_library_array("select id,member_contact_no from lib_mkt_team_member_info", "id", "member_contact_no");
	$deling_marcent_email_arr = return_library_array("select id,team_member_email from lib_mkt_team_member_info", "id", "team_member_email");
	$season_arr = return_library_array("select id,season_name from lib_buyer_season", "id", "season_name");
	$nameArray_approved = sql_select("select max(b.approved_no) as approved_no from wo_booking_mst a, approval_history b where a.id=b.mst_id and booking_no='$txt_booking_no' and b.entry_form=8 and  a.status_active =1 and a.is_deleted=0");
	list($nameArray_approved_row) = $nameArray_approved;
	$booking_grand_total = 0;
	...
	$style_ref = array();
	$all_dealing_marcent = array();
	$all_deling_marcent_desg_arr = array();
	$all_deling_marcent_num_arr = array();
	$all_deling_marcent_email_arr = array();
	$season = array();
	$order_repeat_no = array();
	$po_id_arr = array();
	...
	foreach ($nameArray_buyer as $result_buy) {
		...
		$style_ref[$result_buy[csf('job_no')]] = $result_buy[csf('style_ref_no')];
		$all_dealing_marcent[$result_buy[csf('job_no')]] = $deling_marcent_arr[$result_buy[csf('dealing_marchant')]];
		$all_deling_marcent_desg_arr[$result_buy[csf('job_no')]] = $deling_marcent_desg_arr[$result_buy[csf('dealing_marchant')]];
		$all_deling_marcent_num_arr[$result_buy[csf('job_no')]] = $deling_marcent_num_arr[$result_buy[csf('dealing_marchant')]];
		$all_deling_marcent_email_arr[$result_buy[csf('job_no')]] = $deling_marcent_email_arr[$result_buy[csf('dealing_marchant')]];
		$season_matrix = $result_buy[csf('season_matrix')];
		$season_buyer_wise = $result_buy[csf('season_buyer_wise')];
		if ($season_matrix != 0 && $season_buyer_wise == 0) {
			$season_matrix_con = $season_matrix;
 ...
 ?>
		...
		<table width="1333px" cellpadding="0" cellspacing="0" style="border:0px solid black">
			...
			<table width="100%"  style="border:0px solid black;table-layout: fixed;">
                    <td align="right"  style="width: 30%">
                        <table class="rpt_table" border="1" cellpadding="1" cellspacing="1" rules="all" width="400">
							<tr>
							<td width="130" align="center"><b>Supplier</b></td>
							</tr>
							<tr>								
								<td><b>Name:</b><?
												if ($pay_mode_id == 5 || $pay_mode_id == 3) echo $company_library[$supplier_id];
							...
							<tr>
								<td><b>Address:</b><?php echo $supplier_address_arr[$supplier_id]; ?></td>
							</tr>
							<tr>
								<td><b>Person:</b><?php echo $supplier_contract_per_arr[$supplier_id]; ?></td>
							</tr>
                            <tr>
                                <td><b>Cell:</b><?  echo $supplier_contract_no_arr[$supplier_id]; ?></td>
                            </tr>
                        </table>
                    </td>
					<td align="right"  style="width: 30%">
                        <table class="rpt_table" border="1" cellpadding="1" cellspacing="1" rules="all" width="400">
							<tr>
								<td width="130" align="center"><b>Dealing Merchant</b></td>
							</tr>
							<tr>
								<td><b>Name:</b><? echo implode(",", array_unique($all_dealing_marcent)); ?></td>
							</tr>
							<tr>
							<td><b>Designation:</b><? echo implode(",", array_unique($all_deling_marcent_desg_arr)); ?></td>
							</tr>
							<tr>
								<td><b>Cell:</b><? echo implode(",", array_unique($all_deling_marcent_num_arr)); ?></td>
							</tr>
							<tr>
								<td><b>Email:</b><? echo implode(",", array_unique($all_deling_marcent_email_arr)); ?></td>
							</tr>
                        </table>
                    </td>
					<td align="right"  style="width: 30%">
                        <table class="rpt_table" border="1" cellpadding="1" cellspacing="1" rules="all" width="400">
							<tr>
								<td width="130" align="center"><b>Delivery Address</b></td>
							</tr>
							
							<tr>
								<td><b>Address:</b><? echo $company_library[$cbo_company_name]; ?></td>
							</tr>
							<tr>
								<td><?
										$nameArray = sql_select("select id,plot_no,level_no,road_no,block_no,bin_no,country_id,province,city,zip_code,email,website from lib_company where id=$cbo_company_name");
										foreach ($nameArray as $result) {
										 ...
										 <? echo $result[csf('zip_code')]; ?> &nbsp;
												<!--<?php echo $result[csf('province')]; ?> &nbsp;
											<? echo $country_arr[$result[csf('country_id')]]; ?></td><? } ?> -->
							</tr>
                        </table>
                    </td>
            </table>
			...
			<table width="100%" style="border:0px solid black;table-layout: fixed;">
				...
				<tr>
					...
					<td width="110">:&nbsp;<?  echo $varcode_booking_no; ?></td>
			...
			?>
					...
					<table border="1" align="left" class="rpt_table" cellpadding="0" width="100%" cellspacing="0" rules="all">
						...
						<tr>
							<td style="border:1px solid black"><strong>Sl</strong> </td>
							<td style="border:1px solid black"><strong>Embellishment Name</strong> </td>
							<td style="border:1px solid black"><strong>Embl. Type</strong> </td>
							<td style="border:1px solid black"><strong>Gmts Item</strong> </td>
							<td style="border:1px solid black"><strong>Body Part</strong> </td>
							<td style="border:1px solid black"><strong>Description</strong> </td>
				...
				?>
					...
					<table border="1" align="left" class="rpt_table" cellpadding="0" width="100%" cellspacing="0" rules="all">
						...
						<tr>
							<td align="center" width="30" style="border:1px solid black"><strong>Sl</strong> </td>
							<td align="center" style="border:1px solid black"><strong>Embellishment Name</strong> </td>
							<td align="center" style="border:1px solid black"><strong>Embl. Type</strong> </td>
						...
						foreach ($nameArray_item as $result_item) {
							$i++;
							$nameArray_item_description = sql_select("select a.pre_cost_fabric_cost_dtls_id,a.gmt_item,min(b.id) as bid, b.description,b.item_color,b.color_number_id,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount,c.emb_type,c.body_part_id,a.uom as uom from wo_booking_dtls a,  wo_emb_book_con_dtls b, wo_pre_cost_embe_cost_dtls c where a.id= b.wo_booking_dtls_id and a.booking_no=b.booking_no  and a.pre_cost_fabric_cost_dtls_id=c.id and  a.booking_no='$txt_booking_no' and a.job_no='" . $nameArray_job_po_row[csf('job_no')] . "'  and a.sensitivity=4  and c.emb_name=" . $result_item[csf('emb_name')] . " and  a.pre_cost_fabric_cost_dtls_id=" . $result_item[csf('pre_cost_fabric_cost_dtls_id')] . " and b.requirment !=0 and  a.status_active =1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.pre_cost_fabric_cost_dtls_id,a.gmt_item, b.description,b.item_color,b.color_number_id,c.emb_type,c.body_part_id,a.uom order by bid ");

						?>
							<tr>
							<td  align="center" width="30" style="border:1px solid black" rowspan="<? echo count($nameArray_item_description) + 1; ?>">
									<? echo $i; ?>
								</td>
								<td align="center" style="border:1px solid black" rowspan="<? echo count($nameArray_item_description) + 1; ?>">
									<?
			...
			$mst_id=$txt_booking_no; $width="100%"; $entry_form=403;
	...
	if (count($data_array) > 0) {
		...
		<div align="left" style="margin:5px 2px;float:left;width:100%">
			<? foreach ($data_array as $inf) { ?>
				<img src='../../<? echo $inf[csf("image_location")]; ?>' height='140' width='120' />
			<? } ?>
		</div>
	</div>
...
if ($action == "show_trim_booking_report4") //md mamun 27-12-2021
{
	extract($_REQUEST);
	$process = array(&$_POST);
	extract(check_magic_quote_gpc($process));
...
if ($action == "show_trim_booking_report5") //ISD-7730-md mamun 14-05-2022
{
	extract($_REQUEST);
	$process = array(&$_POST);
	extract(check_magic_quote_gpc($process));
	...
	<div style="width:1333px" align="center">
		<table width="1333px" cellpadding="0" cellspacing="0" style="border:0px solid black">
				...
				<table border="1" align="left" class="rpt_table container" cellpadding="0" width="100%" cellspacing="0" rules="all" style="padding-bottom: 10px">
					...
					<td  colspan="2" align="center"><b>Multiple Job Wise Embellishment Work Order<hr>(CODE: MMD/M&M/DMF-09)</b></td>
				...
				<tr>
				   ...
				   <!-- <td  colspan="2" align="left"><b>SUB: <? echo $sub_embl;;?> Work Order</b></td>-->
				<td  colspan="2" align="left"><b>SUB:<? echo implode(",",array_unique(explode(",",$sub_embl))); ?> Work Order</b></td>	 
				...
				<tr>
				   <td width="100" align="left"><b>Delivery Date :</b></td>
				   <td width="350" colspan="3" align="left"><b><?= change_date_format($delivery_date,'dd-mm-yyyy','-') ;?></b></td>
			...
			?>
					...
					<table border="1" align="left" class="rpt_table" cellpadding="0" width="100%" cellspacing="0" rules="all">
						...
						<tr>
							...
							<td style="border:1px solid black" align="center"><strong>UOM</strong></td>
							<td style="border:1px solid black" align="center"><strong>Req. Qty</strong></td>
							<td style="border:1px solid black" align="center"><strong>Rate</strong></td>
							<td style="border:1px solid black" align="center"><strong>Amount</strong></td>
						...
						?>
							<tr>
								...
								?>
									...
									<td style="border:1px solid black; text-align:center"><? echo $mcurrency; ?> </td>
									<td style="border:1px solid black; text-align:left"><? echo  $remarks; ?> </td>
							</tr>
						<?
								}
						?>
...
if ($action == "show_trim_booking_report6") //Shariar 30-7
{
	extract($_REQUEST);
	$process = array(&$_POST);
	extract(check_magic_quote_gpc($process));
	...
	$nameArray_item = sql_select("select  a.pre_cost_fabric_cost_dtls_id,c.emb_name,c.emb_type,a.gmt_item,c.body_part_id from wo_booking_dtls a, wo_pre_cost_embe_cost_dtls c  where a.pre_cost_fabric_cost_dtls_id=c.id and  a.booking_no='$txt_booking_no' and  a.status_active =1 and a.is_deleted=0 and a.job_no in ( $jobNos )    group by a.pre_cost_fabric_cost_dtls_id,c.emb_name,c.emb_type,a.gmt_item,c.body_part_id  order by c.emb_name ");
	// echo "select  a.pre_cost_fabric_cost_dtls_id,c.emb_name,c.emb_type,a.gmt_item,c.body_part_id from wo_booking_dtls a, wo_pre_cost_embe_cost_dtls c  where a.pre_cost_fabric_cost_dtls_id=c.id and  a.booking_no='$txt_booking_no' and  a.status_active =1 and a.is_deleted=0 and a.job_no in ( $jobNos )    group by a.pre_cost_fabric_cost_dtls_id,c.emb_name,c.emb_type,a.gmt_item,c.body_part_id  order by c.emb_name ";
			...
			foreach ($nameArray_item as $result_item) {
				...
				$gmt_arr[$result_item[csf('pre_cost_fabric_cost_dtls_id')]]['gmt_item']=$result_item[csf('gmt_item')];
				$body_arr[$result_item[csf('pre_cost_fabric_cost_dtls_id')]]['body_part_id']=$result_item[csf('body_part_id')];
			}
 ...
 ?>
	...
	<div style="width:1333px" align="center">
		<table width="1333px" cellpadding="0" cellspacing="0" style="border:0px solid black">
			...
			$nameArray_job_po = sql_select("select job_no from wo_booking_dtls  where booking_no='$txt_booking_no' and status_active =1 and is_deleted=0 group by job_no order by job_no ");
			foreach ($nameArray_job_po as $nameArray_job_po_row) {
				$nameArray_item = sql_select("select  a.pre_cost_fabric_cost_dtls_id,c.emb_name,c.emb_type from wo_booking_dtls a, wo_pre_cost_embe_cost_dtls c  where a.pre_cost_fabric_cost_dtls_id=c.id and  a.booking_no='$txt_booking_no' and  a.status_active =1 and a.is_deleted=0 and a.job_no='" . $nameArray_job_po_row[csf('job_no')] . "'   and a.sensitivity=1 group by a.pre_cost_fabric_cost_dtls_id,c.emb_name,c.emb_type  order by c.emb_name ");
				if (count($nameArray_item) > 0) {
					$po_ids = rtrim($po_no_arr[$nameArray_job_po_row[csf('job_no')]]['po_id']);
					$po_ids = array_unique(explode(",", $po_ids));
					$ref_nos = rtrim($job_ref_no[$nameArray_job_po_row[csf('job_no')]], ',');
			...
			?>
					...
					<table border="1" align="left" class="rpt_table" cellpadding="0" width="100%" cellspacing="0" rules="all">
						...
						foreach ($nameArray_item as $result_item) {
							...
							if ($result_item[csf('emb_name')] == 1) {
								$emb_type_name = $emblishment_print_type[$result_item[csf('emb_type')]];
							}
							 if ($result_item[csf('emb_name')] == 2) {
								$emb_type_name = $emblishment_embroy_type[$result_item[csf('emb_type')]];
							}
							 if ($result_item[csf('emb_name')] == 3) {
								$emb_type_name = $emblishment_wash_type[$result_item[csf('emb_type')]];
							}
							 if ($result_item[csf('emb_name')] == 4) {
								$emb_type_name = $emblishment_spwork_type[$result_item[csf('emb_type')]];
							}
							if ($result_item[csf('emb_name')] == 5) {
							$emb_type_name = $emblishment_gmts_type[$result_item[csf('emb_type')]];
							}
						...
						?>
							<tr>
								...
								<td align="center" style="border:1px solid black" rowspan="<? echo count($nameArray_item_description) + 1; ?>">
								<? echo $emb_type_name; ?></td>
								
								<?
								$emb_gmt=$gmt_arr[$result_item[csf('pre_cost_fabric_cost_dtls_id')]]['gmt_item'];
								?>
								<td align="center" style="border:1px solid black" rowspan="<? echo count($nameArray_item_description) + 1; ?>"><? echo $garments_item[$emb_gmt]; ?></td>
								<?
								$emb_body=$body_arr[$result_item[csf('pre_cost_fabric_cost_dtls_id')]]['body_part_id'];
								?>
								<td align="center" style="border:1px solid black; text-align:left" rowspan="<? echo count($nameArray_item_description) + 1; ?>"><? echo $body_part[$emb_body]; ?></td>
								<?
								$item_desctiption_total = 0;
								$total_amount_as_per_gmts_color = 0;
								foreach ($nameArray_item_description as $result_itemdescription) {
...
if ($action == "show_trim_booking_report7") { //md mamun /issue-21307/copy print booking button/21-09-2022
	extract($_REQUEST);
	$process = array(&$_POST);
	extract(check_magic_quote_gpc($process));
	$txt_booking_no = str_replace("'", "", $txt_booking_no);
		...
		<table width="1333px" cellpadding="0" cellspacing="0" style="border:0px solid black">
			...
			?>
				...
				if (count($nameArray_item) > 0) {
					...
					$color_size_wise_data=array();$color_sise_qnty=array();$sizeArr=array();
					foreach ($nameArray_item as $result_item) {
						$job_no=$nameArray_job_po_row[csf('job_no')];
						$i++;
						$nameArray_item_description = sql_select("select a.pre_cost_fabric_cost_dtls_id,a.gmt_item,min(b.id) as bid, b.description,b.item_color,b.color_number_id,b.item_size,b.gmts_sizes,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount, c.emb_name,c.emb_type,c.body_part_id,a.uom as uom from wo_booking_dtls a,  wo_emb_book_con_dtls b, wo_pre_cost_embe_cost_dtls c where a.id= b.wo_booking_dtls_id and a.booking_no=b.booking_no  and a.pre_cost_fabric_cost_dtls_id=c.id and  a.booking_no='$txt_booking_no' and a.job_no='" . $nameArray_job_po_row[csf('job_no')] . "'  and a.sensitivity=4  and c.emb_name=" . $result_item[csf('emb_name')] . " and a.pre_cost_fabric_cost_dtls_id=" . $result_item[csf('pre_cost_fabric_cost_dtls_id')] . " and b.requirment !=0 and  a.status_active =1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.pre_cost_fabric_cost_dtls_id,a.gmt_item, b.description,b.item_color,b.color_number_id,b.item_size,b.gmts_sizes,c.emb_name,c.emb_type,c.body_part_id,a.uom order by bid ");
						 ...
						 foreach($nameArray_item_description as $row){
						 	$sizeArr[$row[csf('gmts_sizes')]]=$row[csf('gmts_sizes')];
						...
						if ($result_item[csf('emb_name')] == 5) {
							...
							$string2=$row[csf('emb_type')]."*".$row[csf('color_number_id')];
							$string=$job_no."*".$row[csf('gmt_item')]."*".$result_item[csf('emb_name')];
							
							$color_size_wise_data[$string][$row[csf('emb_type')]][$row[csf('color_number_id')]]['emb_type']=$emb_type_name;
							$color_size_wise_data[$string][$row[csf('emb_type')]][$row[csf('color_number_id')]]['gmt_item']=$row[csf('gmt_item')];
							$color_size_wise_data[$string][$row[csf('emb_type')]][$row[csf('color_number_id')]]['cons']+=$row[csf('cons')];
							$color_size_wise_data[$string][$row[csf('emb_type')]][$row[csf('color_number_id')]]['body_part_id']=$row[csf('body_part_id')];
							$color_size_wise_data[$string][$row[csf('emb_type')]][$row[csf('color_number_id')]]['description']=$row[csf('description')];
							$color_size_wise_data[$string][$row[csf('emb_type')]][$row[csf('color_number_id')]]['item_color']=$row[csf('item_color')];
							$color_size_wise_data[$string][$row[csf('emb_type')]][$row[csf('color_number_id')]]['uom']=$row[csf('uom')];
							$color_size_wise_data[$string][$row[csf('emb_type')]][$row[csf('color_number_id')]]['rate']=$row[csf('rate')];
							$color_size_wise_data[$string][$row[csf('emb_type')]][$row[csf('color_number_id')]]['amount']+=$row[csf('amount')];						
							$color_sise_qnty[$string][$row[csf('emb_type')]][$row[csf('color_number_id')]][$row[csf('gmts_sizes')]]['size_cons']+=$row[csf('cons')];
							// $color_sise_qnty[$string2][$row[csf('gmts_sizes')]]['size_cons']+=$row[csf('cons')];
				...
				?>
					...
					<table border="1" align="left" class="rpt_table" cellpadding="0" width="100%" cellspacing="0" rules="all">
						...
						<tr>
							...
							<td style="border:1px solid black" rowspan="2"><strong>Name/Country</strong> </td>
							<td style="border:1px solid black" rowspan="2"><strong>Embl. Type</strong> </td>
							<td style="border:1px solid black" rowspan="2"><strong>Gmts Item</strong> </td>
							<td style="border:1px solid black" rowspan="2"><strong>Body Part</strong> </td>
							<td style="border:1px solid black" rowspan="2"><strong>Description</strong> </td>
							<td align="center" style="border:1px solid black" rowspan="2"><strong>Item Color</strong></td>
							<td align="center" style="border:1px solid black" rowspan="2"><strong>Gmts Color</strong></td>
					
							<td align="center" style="border:1px solid black" colspan="<?=count($sizeArr);?>"><strong>Gmts Size</strong></td>
							<td align="center" style="border:1px solid black" rowspan="2"><strong>UOM</strong></td>
						...
						<tr>
							<?
							foreach($sizeArr as $sid){?>							
							<td align="center" style="border:1px solid black"><strong><?=$size_library[$sid];?></strong></td>
						...
						<?
						$embl_rowspan=array();
						foreach ($color_size_wise_data as $key=>$embl_type_data) {
							list($job_no,$gmt_item,$emb_name)=explode("*",$key);
							foreach ($embl_type_data as $embl_type=>$color_data) {
								foreach ($color_data as $color_id=>$value) {
							
							$embl_rowspan[$job_no][$gmt_item][$emb_name][$embl_type]+=1;
						}}}
					
						// echo "<pre>";
						// print_r($color_size_wise_data);
						$i = 0;
						$grand_total_as_per_gmts_color = 0;$string2="";
						$gmts_size_arr=array();$item_desctiption_total = 0;$embltypeid="";
						...
						foreach ($color_size_wise_data as $key=>$embl_type_data) {
							...
							<tr>
									...
									<?if($e==1){?>
									<td style="border:1px solid black"  rowspan="<?=$embl_rowspan[$job_no][$gmt_item][$emb_name][$embl_type];?>">
										<? echo $i; ?>
									</td>
									<td align="center" style="border:1px solid black"  rowspan="<?=$embl_rowspan[$job_no][$gmt_item][$emb_name][$embl_type];?>">
										<?
										echo $emblishment_name_array[$emb_name] . "<br/>";
										echo implode(", ", $booking_country_arr[$pre_cost_id][4]);
										?>
									</td>
									<td style="border:1px solid black" rowspan="<?=$embl_rowspan[$job_no][$gmt_item][$emb_name][$embl_type];?>"><? echo $value['emb_type']; ?></td>
									<td style="border:1px solid black" rowspan="<?=$embl_rowspan[$job_no][$gmt_item][$emb_name][$embl_type];?>"><? echo $garments_item[$gmt_item];  ?></td>
									<td style="border:1px solid black; text-align:left" rowspan="<?=$embl_rowspan[$job_no][$gmt_item][$emb_name][$embl_type];?>"><? echo $body_part[$value['body_part_id']];	?></td>
									<?}?>
									...
									<td style="border:1px solid black; text-align:right">
										<?
										echo $color_library[$value['item_color']];
										?>
									</td>
									<td style="border:1px solid black; text-align:right">
										<?
									...
									foreach($sizeArr as $sid){?>
									<td style="border:1px solid black; text-align:right"><?	
									echo number_format($color_sise_qnty[$key][$embl_type][$color_id][$sid]['size_cons'],4);
									$item_wise_qnty[$key][$embl_type]+=$color_sise_qnty[$key][$embl_type][$color_id][$sid]['size_cons'];
									$item_size_wise_qnty[$key][$embl_type][$sid]+=$color_sise_qnty[$key][$embl_type][$color_id][$sid]['size_cons'];
									?>	</td>
									<?
									...
									<td style="border:1px solid black; text-align:right">
										...
										//echo $unit_of_measurement[$result_itemdescription[csf('uom')]];
											if (empty($value['uom'])) {
												...
												echo $unit_of_measurement[$value['uom']];
											}
									...
									<td style="border:1px solid black; text-align:right">
										<?
										echo number_format($value['cons'], 4);
										$item_desctiption_total += $value[csf('cons')];
										?>
									</td>
									<td style="border:1px solid black; text-align:right"><? echo number_format($value['rate'], 4); ?> </td>
									<td style="border:1px solid black; text-align:right">
										<?
										$amount_as_per_gmts_color = $value['cons']*$value['rate'];
										echo number_format($amount_as_per_gmts_color, 4);
										$total_amount_as_per_gmts_color += $amount_as_per_gmts_color;
										?>
									</td>
							...
							?>
									<tr>
										...
										<?
									foreach($sizeArr as $sid){?>
										<td style="border:1px solid black;  text-align:right"><strong><?=number_format($item_size_wise_qnty[$key][$embl_type][$sid],4);?></strong></td>
										<?}?>
										<td style="border:1px solid black;  text-align:right" ><strong></strong></td>
										<td style="border:1px solid black;  text-align:right; font-weight:bold;"><?	echo number_format($item_wise_qnty[$key][$embl_type], 4);	?></td>
										<td style="border:1px solid black; text-align:right"></td>
										<td style="border:1px solid black; text-align:right">
											<?
											echo number_format($total_amount_as_per_gmts_color, 2);
					...
					?>
					<tr>
						<td align="right" style="border:1px solid black" colspan="<?=count($sizeArr)+11;?>"><strong>Total</strong></td>
						<td style="border:1px solid black;  text-align:right"><? echo number_format($grand_total_as_per_gmts_color, 2);  ?></td>
					</tr>
					</table>
