	...
	<div id="mstDiv" style="float:left;">
        <table width="1100" cellspacing="0" border="0"  style="font-family: Arial Narrow;margin-left: 3px;" >
            ...
            <tr>
                <td colspan="5">
                    ...
                    $sql="SELECT id, requisition_number, requisition_number_prefix_num, style_ref_no, buyer_name, season, product_dept, dealing_marchant, agent_name, buyer_ref, bh_merchant, estimated_shipdate, team_leader, season_buyer_wise, remarks, quotation_id, sample_stage_id, requisition_date, material_delivery_date,qrr_date from sample_development_mst where  id='$data[1]' and entry_form_id=203 and  is_deleted=0  and status_active=1";
                    $dataArray=sql_select($sql);
                    $barcode_no=$dataArray[0][csf('requisition_number')];
                    if($dataArray[0][csf("sample_stage_id")]==1)
                    {
        ...
        <table width="1100" cellspacing="0" border="0" class="rpt_table" style="font-family: Arial Narrow;margin-left: 3px;" >
            ...
            <tr>
                ...
                <td><?=$dealing_merchant_library[$dataArray_book[0][csf('dealing_marchant')]];?></td>
					...
					<table  class="rpt_table"  border="1" style="border:1px solid black;"   cellpadding="0" width="100%" cellspacing="0" rules="all" >
						<tr>
							...
							<td align="center" width="100" colspan="2"> <?
										 ...
										 {
										 ?>
										 Revised No: <? echo $nameArray_approved_row[csf('approved_no')]-1; ?>
										  <?
										 }
										  ?></td>
					 ...
					 <table  class="rpt_table"  border="1" style="border:1px solid black;"   cellpadding="0" width="100%" cellspacing="0" rules="all" >
						<tr>
							...
							<td width="150" style="font-size:16px;"><b>Requisition No.</b></td>
							<td width="40" style="font-size:16px;" align="center"><b>:</b></td>
					...
					foreach( $yarn_data_arr as $key=>$row )
					{
						...
						?>
                        	<tr>
                                <td> <? echo $l;?> </td>
                                <td> <? echo $key; ?> </td>
                                <td align="right"> <?=$row['cons_qnty']; ?> </td>
                            </tr>
                        <?
                        $l++;
	...
	<form name="searchsampledevelopmentfrm_1"  id="searchsampledevelopmentfrm_1" autocomplete="off">
        <table width="950" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
            ...
            <tr class="general">
                ...
                <td>
                    <input type="button" name="btn_show" id="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_search_category').value+'_'+document.getElementById('txt_style_id').value+'_'+document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_style_name1').value+'_'+document.getElementById('txt_requisition_num').value+'_'+document.getElementById('cbo_sample_stage').value+'_'+document.getElementById('txt_booking_num').value, 'create_requisition_id_search_list_view', 'search_div', 'sample_requisition_with_booking_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:80px;" />
                </td>
            </tr>
            <tr>
...
if($action == "yarn_dtls_popup") 
{
	...
	//echo "select id, construction from lib_yarn_count_determina_mst where id in($yarn_deter_min_id)";
	...
	//echo "SELECT id,samp_fab_dtls_id,determin_id, mst_id, count_id,copm_one_id, cons_ratio, type_id, cons_qnty from sample_development_yarn_dtls where is_deleted=0 and status_active=1 and mst_id =".$req_id."";
...
{
	$res = sql_select("select id,company_id,location_id,buyer_name,style_ref_no,product_dept,agent_name,dealing_marchant,bh_merchant,season,buyer_ref,estimated_shipdate,remarks,requisition_number,sample_stage_id,requisition_date,material_delivery_date,quotation_id,is_approved,req_ready_to_approved, copy_from, fabric_material_id, sustainability_std_id, order_nature_id, quality_level_id, design_source_id, factory_merchant, team_leader, client_id, item_catgory, season_year, brand_id, sub_dept_id, style_desc,qrr_date,fit_id,internal_ref from sample_development_mst where id=$data and entry_form_id=203 and is_deleted=0 and status_active=1");
 	...
 	foreach($res as $result) 
	{
		...
		//$buyer_td=create_drop_down( "cbo_buyer_name", 150, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='".$result[csf("company_id")]."' $buyer_cond  and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,30,90)) order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down( 'requires/sample_requisition_with_booking_controller', this.value, 'load_drop_down_season_buyer', 'season_td');load_drop_down( 'requires/sample_requisition_with_booking_controller', this.value, 'load_drop_down_sample_for_buyer', 'sample_td');get_buyer_config(this.value);" );
		//echo "document.getElementById('buyer_td').innerHTML = '".$buyer_td."';\n";
		

		//echo "load_drop_down( 'requires/sample_requisition_with_booking_controller', '".$result[csf("company_id")]."', 'load_drop_down_location', 'location_td' );\n";
		$location_td='';
		...
		if(count(sql_select($sql))==1)
		{
			$location_td=create_drop_down( "cbo_location_name", 150, $sql,'id,location_name', 0, '--- Select Location ---', 0, ""  );
		}
		else
		{
			$location_td=create_drop_down( "cbo_location_name", 150, $sql,'id,location_name', 1, '--- Select Location ---', 0, ""  );
		}
		echo "document.getElementById('location_td').innerHTML = '".$location_td."';\n";


		//echo "load_drop_down( 'requires/sample_requisition_with_booking_controller', '".$result[csf("company_id")]."', 'load_drop_down_agent', 'agent_td' );\n";
		$agent_td=create_drop_down( "cbo_agent", 150, "SELECT a.id,a.buyer_name from lib_buyer a, lib_buyer_tag_company b,lib_buyer_party_type c where b.buyer_id=c.buyer_id and  a.status_active =1 and a.is_deleted=0 and c.party_type in(20,21) and b.buyer_id=a.id and b.tag_company='".$result[csf("company_id")]."' group by  a.id,a.buyer_name order by buyer_name","id,buyer_name", 1, "-- Select Agent --", $selected, "" );
		echo "document.getElementById('agent_td').innerHTML = '".$agent_td."';\n";

		//echo "load_drop_down( 'requires/sample_requisition_with_booking_controller','".$result[csf("buyer_name")]."', 'load_drop_down_season_buyer', 'season_td');\n";
		...
		if(count(sql_select($sql))==1)
		{
			$season_td=create_drop_down( "cbo_season_name", 150, $sql,'id,season_name', 0, '--- Select Season ---', 1, ""  );
		}
		else
		{
			$season_td=create_drop_down( "cbo_season_name", 150, $sql,'id,season_name', 1, '--- Select Season ---', 0, ""  );
		}
		echo "document.getElementById('season_td').innerHTML = '".$season_td."';\n";

		//echo "load_drop_down( 'requires/sample_requisition_with_booking_controller','".$result[csf("buyer_name")]."', 'load_drop_down_sample_for_buyer', 'sample_td');\n";
		//$sample_td=create_drop_down( "cboSampleName_1", 100, "select a.id,a.sample_name,b.sequ from lib_sample a,lib_buyer_tag_sample b where a.id=b.tag_sample and  b.buyer_id='".$result[csf("buyer_name")]."' and b.sequ  is not null and a.status_active=1 and a.is_deleted=0  group by  a.id,a.sample_name,b.sequ order by b.sequ ","id,sample_name", 1, "-- Select Buyer --", $selected, "" );
		//echo "document.getElementById('sample_td').innerHTML = '".$sample_td."';\n";
		
		//echo "load_drop_down( 'requires/sample_requisition_with_booking_controller', '".$result[csf("id")]._1."', 'load_drop_down_required_fabric_sample_name', 'rfSampleId_1');\n";
		//echo "load_drop_down( 'requires/sample_requisition_with_booking_controller', '".$result[csf("id")]._2."', 'load_drop_down_required_fabric_sample_name','raSampleId_1');\n";
		//echo "load_drop_down( 'requires/sample_requisition_with_booking_controller', '".$result[csf("id")]._3."', 'load_drop_down_required_fabric_sample_name','reSampleId_1');\n";
		...
		/*$rfSampleId_1=create_drop_down( "cboRfSampleName_1", 95, $samp_array,"", 1, "select Sample", $selected,"sample_wise_item(".$result[csf("id")].",this.value,1,1);");
		echo "document.getElementById('rfSampleId_1').innerHTML = '".$rfSampleId_1."';\n";

		$raSampleId_1=create_drop_down( "cboRaSampleName_1", 100, $samp_array,"", 1, "select Sample", $selected,"sample_wise_item(".$result[csf("id")].",this.value,1,2);");
		echo "document.getElementById('raSampleId_1').innerHTML = '".$raSampleId_1."';\n";

		$reSampleId_1=create_drop_down( "cboReSampleName_1", 140, $samp_array,"", 1, "select Sample", $selected,"sample_wise_item(".$result[csf("id")].",this.value,1,3);");
		echo "document.getElementById('reSampleId_1').innerHTML = '".$reSampleId_1."';\n";

		$sql=sql_select("select id,gmts_item_id from sample_development_dtls where is_deleted=0  and status_active=1 and entry_form_id=203 and sample_mst_id='".$result[csf("id")]."'");
		$gmts="";
		foreach ($sql as $row)
		{
			$gmts.=$row[csf("gmts_item_id")].",";
		...
		$rfItemId_1=create_drop_down( "cboRfGarmentItem_1", 95, $garments_item,"", 0, "-- Select Item --", $selected, "",0,$gmts);;
		echo "document.getElementById('rfItemId_1').innerHTML = '".$rfItemId_1."';\n";

	
		$raItemId_1=create_drop_down( "cboRaGarmentItem_1", 100, $garments_item,"", 0, "-- Select Item --", $selected, "",0,$gmts);
		echo "document.getElementById('raItemId_1').innerHTML = '".$raItemId_1."';\n";

		
		$reItemIid_1=create_drop_down( "cboReGarmentItem_1", 95, $garments_item,"", 0, "-- Select Item --", $selected, "",0,$gmts);
		echo "document.getElementById('reItemIid_1').innerHTML = '".$reItemIid_1."';\n";*/

		

		$div_marchant= create_drop_down( "cbo_dealing_merchant", 150, "select id,team_member_name from lib_mkt_team_member_info where team_id='".$result[csf("team_leader")]."' and status_active =1 and is_deleted=0 order by team_member_name","id,team_member_name", 1, "-- Select Team Member --", $selected, "" );
		echo "document.getElementById('div_marchant').innerHTML = '".$div_marchant."';\n";

		
		$div_marchant_factory=create_drop_down( "cbo_factory_merchant", 150, "select a.id, a.team_member_name from lib_mkt_team_member_info a, lib_marketing_team b where a.team_id=b.id and a.team_id='".$result[csf("team_leader")]."' and b.team_type in (2) and a.status_active =1 and a.is_deleted=0 order by a.team_member_name","id,team_member_name", 1, "-- Select Team Member --", $selected, "" );
...
if ($action=="save_update_delete_sample_details")
{
	...
	if ($operation==0)  // Insert Here
	{
			...
			for ($i=1;$i<=$total_row;$i++)
		    {
				...
				if ($i!=1) $data_array .=",";
				$data_array .="(".$id_dtls.",".$update_id.",".$$cboSampleName.",".$$cboGarmentItem.",".$$txtSmv.",".$$txtArticle.",'".$color_id."',".$$txtSampleProdQty.",".$$txtSubmissionQty.",".$$txtDelvStartDate.",".$$txtDelvEndDate.",".$$txtBuyerSubDate.",".$$txtRemarks.",".$$txtChargeUnit.",".$$cboCurrency.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','1',0,203,".$$txtAllData.",0,0,0,0,0,0)";
	...
	else if ($operation==1)  // Update Here
	{
			...
			for ($i=1;$i<=$total_row;$i++)
		    {
				...
				//	$color_delete_arr[$val[csf('id')]]['sample_name']=$key_val[csf('sample_name')];
					//$color_delete_arr[$val[csf('id')]]['gmts_item_id']=$key_val[csf('gmts_item_id')];
				}
				...
				if (str_replace("'",'',$$updateIdDtls)!="")
				{
					...
					$data_array_up[str_replace("'",'',$$updateIdDtls)] =explode("*",("".$$cboSampleName."*".$$cboGarmentItem."*".$$txtSmv."*".$$txtArticle."*'".$color_id."'*".$$txtSampleProdQty."*".$$txtSubmissionQty."*".$$txtDelvStartDate."*".$$txtDelvEndDate."*".$$txtBuyerSubDate."*".$$txtRemarks."*".$$txtChargeUnit."*".$$cboCurrency."*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'*".$$txtAllData.""));
...
if ($action=="save_update_delete_required_accessories")
{
	...
	if ($operation==0)  // Insert Here
	{
			...
			for ($i=1;$i<=$total_row;$i++)
		    {
				...
				if ($i!=1) $data_array .=",";
				$data_array .="(".$id_dtls.",".$update_id.",".$$cboRaSampleName.",".$$cboRaGarmentItem.",".$$cboRaSupplierName.",".$$accDate.",".$$cboRaFabricSource.",".$$cboRaTrimsGroup.",".$$txtRaDescription.",".$$txtRaBrandSupp.",".$$cboRaUom.",".$$txtRaReqDzn.",".$$txtRaReqQty.",".$$txtRaRemarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','1',0,2)";
	...
	else if ($operation==1)  // Update Here
	{
			...
			for ($i=1;$i<=$total_row;$i++)
		    {
				...
				if (str_replace("'",'',$$updateIdAccDtls)!="")
				{
					...
					$data_array_up[str_replace("'",'',$$updateIdAccDtls)] =explode("*",("".$$cboRaSampleName."*".$$cboRaGarmentItem."*".$$cboRaSupplierName."*".$$accDate."*".$$cboRaFabricSource."*".$$cboRaTrimsGroup."*".$$txtRaDescription."*".$$txtRaBrandSupp."*".$$cboRaUom."*".$$txtRaReqDzn."*".$$txtRaReqQty."*".$$txtRaRemarks."*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'"));
				...
				<tr id="tr_<?=$i; ?>" style="height:10px;" class="general">
					...
					<td>
						...
						else
						{
							if($value=="" || $value==0)
							{
								echo create_drop_down( "cboGarmentItem_$i", 100, $garments_item,"", 1, "Select Item",$row[csf("gmts_item_id")], "",0,"");
							}
							else
							{
								echo create_drop_down( "cboGarmentItem_$i", 100, $garments_item,"", 1, "Select Item",$row[csf("gmts_item_id")], "",0,$row[csf("gmts_item_id")]);
							}
						}
						?>
=					</td>
					...
					<td><input style="width:80px;" type="text" class="text_boxes"  name="txtColor_<? echo $i; ?>" id="txtColor_<? echo $i; ?>" placeholder="write/browse" onDblClick="openmypage_color_size('requires/sample_requisition_with_booking_controller.php?action=color_popup','Color Search','1','<? echo $i; ?>');" value="<? echo $color_arr[$row[csf("sample_color")]]; ?>"/>
	...
	else if($type==2)//Fabric
	{
		$sample_color_sql=sql_select("select sample_mst_id,sample_name,gmts_item_id,sample_color from sample_development_dtls where entry_form_id=203 and sample_mst_id=$up_id  and is_deleted=0  and status_active=1 order by id ASC");
				//echo "select sample_mst_id,sample_color from sample_development_dtls where entry_form_id=203 and sample_mst_id=$up_id  and is_deleted=0  and status_active=1";

		foreach($sample_color_sql as $row)
		{
			$sample_color_arr[$row[csf("sample_name")]][$row[csf("gmts_item_id")]].=$color_arr[$row[csf("sample_color")]].'***';
			//$sample_color_id_arr[$row[csf("sample_name")]]=$row[csf("sample_color")];
		}
		...
		if(count($sql_resultf)>0)
		{
			foreach($sql_resultf as $row)
			{
				...
				<tr id="tr_<?=$i; ?>" style="height:10px;" class="general">
					...
					<td id="rf_fab_<?=$i; ?>"><?=create_drop_down( "cboRfFabricSource_$i", 70, $fabric_source,'', '', "",$row[csf("fabric_source")],"","","1,4" ); ?></td>
	...
	if ($operation==0)  // Insert Here  update here
	{
		...
		if($booking_no)
		{
			$field_array_up="fabric_source*currency_id*source*buyer_req_no*revised_no*style_desc*exchange_rate*pay_mode*supplier_id*attention*ready_to_approved*team_leader*dealing_marchant*remarks*rmg_process_breakdown*updated_by*update_date";
			 $data_array_up ="".$cbo_fabric_source."*".$cbo_currency."*".$cbo_sources."*".$txt_buyer_req_no."*".$txt_revise_no."*'".$txt_style_desc."'*".$txt_exchange_rate."*".$cbo_pay_mode."*".$cbo_supplier_name."*".$txt_attention."*".$cbo_ready_to_approved_book."*".$cbo_team_leader_book."*".$cbo_dealing_merchant_book."*".$txt_booking_remarks."*".$txt_processloss_breck_down."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		else
		{
			...
			$data_array ="(".$id.",4,'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',".$cbo_company_name.",".$cbo_buyer_name.",'2',".$cbo_fabric_source.",".$cbo_currency.",".$cbo_sources.",".$txt_buyer_req_no.",".$txt_revise_no.",".$txt_exchange_rate.",".$cbo_pay_mode.",".$txt_booking_date.",".$cbo_supplier_name.",".$txt_attention.",".$cbo_ready_to_approved_book.",".$cbo_team_leader_book.",".$cbo_dealing_merchant_book.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','140','".$txt_style_desc."',".$txt_processloss_breck_down.",".$txt_booking_remarks.")";
...
<fieldset>
           ...
           <input type="text" id="txt_booking_no" name="txt_booking_no" value="<? echo str_replace("'","",$txt_booking_no) ?>"/>
            <table width="650" cellspacing="0" class="rpt_table" border="0" id="tbl_termcondi_details" rules="all">
					...
					{
					$data_array2=sql_select("select id, terms from  lib_terms_condition  where is_default=1 and page_id in(203,140) ");// quotation_id='$data'
