...
Affected Code            :
DB Script                :
Updated by 		         : Zakaria joy (Country wise Quantity Validation)
...
{
	if($data==5 || $data==3){
	   echo create_drop_down( "cbo_supplier_name", 172, "select id,company_name from lib_company where status_active=1 and is_deleted=0 order by company_name", "id,company_name",1, "-- Select Company --", "", "get_php_form_data( this.value, 'load_drop_down_attention', 'requires/trims_booking_urmi_controller');",0,"" );
	}
	else
	{
	echo create_drop_down( "cbo_supplier_name", 172, "select c.supplier_name,c.id from lib_supplier_tag_company a, lib_supplier_party_type b, lib_supplier c where c.id=b.supplier_id and a.supplier_id = b.supplier_id  and b.party_type=4 and c.status_active=1 and c.is_deleted=0 group by c.id, c.supplier_name order by c.supplier_name","id,supplier_name", 1, "--Select Supplier--",$selected,"get_php_form_data( this.value, 'load_drop_down_attention', 'requires/trims_booking_urmi_controller');","");
	}
	exit();
}
...
function load_drop_down_trim_precost_id($data)
{
	...
	$sql_data=sql_select("select a.id,a.trim_group,a.cons_dzn_gmts,a.description from wo_pre_cost_trim_cost_dtls a where a.job_no='$data[0]' $trim_idCond  and a.is_deleted=0  and a.status_active=1");
        ...
        <table width="1160" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
            ...
            <tr class="general">
                ...
                <input type="hidden" id="garments_nature" value="<? echo $garments_nature; ?>">
                    <? echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", $cbo_company_name,"load_drop_down( 'trims_booking_urmi_controller', this.value+'_'+document.getElementById('buyer_id').value, 'load_drop_down_buyer_pop', 'buyer_td' );",1,"" ); ?>
            </td>
...
if($action=="create_po_search_list_view")
{
   ...
   // echo $booking_no.'D';;
	...
	$arr=array (4=>$comp,5=>$buyer_arr,11=>$item_category,12=>$yes_no);
     $sqlPi=sql_select("select a.pi_number, b.quantity,b.amount, a.approved from  com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id  and b.work_order_no='$booking_no'   and a.item_category_id=4 and a.status_active=1 and  a.is_deleted=0  and b.status_active=1 and  b.is_deleted=0");
     
			foreach($sqlPi as $rowPi){
				$pi_numberArr[$rowPi[csf('pi_number')]]=$rowPi[csf('pi_number')]; 
			}
            if(count($pi_numberArr)>0) //issue id-25497, Hams-2023
            {
                echo "<div style='color:red; font-size:20px'><b>Pi Found against the booking no=".implode(",",$pi_numberArr)."</b></div>";die;
            }

	echo "<div style='width:1000px;font-size:16px'; align='center'><font color='#FF0000'>Only Pre Cost V2 Job Allowed.</font></div>";
	...
	if ($data[2]==0)
	{
		if($db_type==0)
		{
			$sql= "select a.job_no_prefix_num, a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity, b.po_number,b.file_no,b.grouping,b.po_quantity,b.shipment_date,a.garments_nature,SUBSTRING_INDEX(a.insert_date, '-', 1) as year,b.id,c.approved from wo_po_details_master a, wo_po_break_down b,wo_pre_cost_mst c where a.id=b.job_id and a.id=c.job_id and c.approved=1  and b.shiping_status not in(3)  and c.entry_from=158 and a.status_active=1 and b.status_active=1 $shipment_date $company $buyer $job_cond $file_cond $ref_cond $order_cond $style_cond $gmt_nature_cond order  by a.job_no";
		}
		if($db_type==2)
		{
			 $sql= "select a.job_no_prefix_num, a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity, b.po_number,b.file_no,b.grouping,b.po_quantity,b.shipment_date,a.garments_nature,to_char(a.insert_date,'YYYY') as year,b.id,c.approved from wo_po_details_master a, wo_po_break_down b,wo_pre_cost_mst c where a.id=b.job_id and a.id=c.job_id and c.entry_from=158 and c.approved=1   and b.shiping_status not in(3) and a.status_active=1 and b.status_active=1 $shipment_date $company $buyer $job_cond $file_cond $ref_cond $order_cond $style_cond $gmt_nature_cond order by a.job_no";
		}
		//echo $sql;
	...
	else
	{
		...
		if($db_type==0)
		{
			$sql= "select a.job_no_prefix_num,a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.garments_nature,SUBSTRING_INDEX(a.`insert_date`, '-', 1) as year from wo_po_details_master a,wo_pre_cost_mst c where  a.id=c.job_id and c.entry_from=158 and a.job_no not in( select distinct job_no_mst from wo_po_break_down where status_active=1 and is_deleted=0 ) and a.status_active=1   and a.is_deleted=0 $gmt_nature_cond $company $buyer order by a.job_no";
		}
		if($db_type==2)
		{
...
/*if ($action=="populate_data_from_search_popup")
{
	...
	foreach ($data_array as $row)
	{
		echo "load_drop_down( 'requires/trims_booking_urmi_controller', '".$row[csf("company_name")]."', 'load_drop_down_buyer', 'buyer_td' );\n";
...
function set_precost_data($data)
{
	$data=explode("_",$data);
	 ...
	 // With Gmt Item
      ...
      $sql_job=sql_select("select b.exchange_rate,b.costing_per,a.job_no,a.currency_id from wo_po_details_master a,wo_pre_cost_mst b,wo_po_break_down c where a.id=b.job_id and a.id=c.job_id and b.job_id=c.job_id  and  c.id=$data[1] and b.status_active=1 and c.status_active=1");
      foreach($sql_job as $row)
      {
        $exchange_rate=$row[csf('exchange_rate')];
        $costing_perId=$row[csf('costing_per')];
        $currency_id_pre_cost=$row[csf('currency_id')];
      }
	...
	foreach($sql_data as $row)
	{
		...
		else if($costing_perId==2)
		{
			$costing_per_qty=1;
		}
		else if($costing_perId==3)
		{
			$costing_per_qty=24;
		}
		else if($costing_perId==4)
		{
			$costing_per_qty=36;
		}
		else if($costing_perId==5)
		{
			$costing_per_qty=48;
		}
		...
		if($data[5]==1)
		{
			...
			$req_amount_cons_uom=def_number_format($req_qnty_cons_uom*$row[csf("rate")],6,"");
...
if($action=="booking_qnty_and_amount_trim_group")
{
   ...
   $woqty="";
   $amount="";
   $data_sql=sql_select("select sum(b.wo_qnty) as wo_qnty, sum(b.amount) as amount from wo_booking_dtls b,wo_booking_mst a 
   where a.booking_no=b.booking_no and b.po_break_down_id=$txt_po_id and b.trim_group=$txt_trim_group_id AND a.ENTRY_FORM = 260  and b.pre_cost_fabric_cost_dtls_id=$cbo_trim_precost_id and b.booking_type=2 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0");
	...
	<div style="width:1360px; overflow-y:scroll; max-height:350px;" id="buyer_list_view" align="center">
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1343" class="rpt_table" id="tbl_list_search" >
            <tbody>
                ...
                $po_number=return_library_array("select id,po_number from wo_po_break_down where status_active=1 and id in($po_ids)", "id", "po_number");
                ...
                $sql_cu_woq=sql_select("select pre_cost_fabric_cost_dtls_id as fab_dtlid,po_break_down_id as poid,country_id_string,gmt_item,trim_group,(wo_qnty) as cu_woq  from wo_booking_dtls where    po_break_down_id in(".$po_ids.")  and  booking_type=2 and status_active=1 and is_deleted=0");
                foreach($sql_cu_woq as $row)
                {
                $trims_cu_woq_consArr[$row[csf('fab_dtlid')]][$row[csf('poid')]][$row[csf('country_id_string')]][$row[csf('gmt_item')]][$row[csf('trim_group')]]+=$row[csf('cu_woq')];	
                }
                unset($sql_cu_woq);
            ...
            $cu_woq=$trims_cu_woq_consArr[$selectResult[csf('pre_cost_fabric_cost_dtls_id')]][$selectResult[csf('po_break_down_id')]][$selectResult[csf('country_id_string')]][$selectResult[csf('gmt_item')]][$selectResult[csf('trim_group')]];
				//echo  $cu_woq.'=';
				if($is_short_id==1)
				{
					$cu_woq=0;
                ...
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="load_dtls_data(<? echo $selectResult[csf('booking_id')]; ?>,<? echo $req_qnty;  ?>,<? echo $bal_woq;  ?>)">
                    ...
                    <td width=""   align="right"><? echo def_number_format($selectResult[csf('amount')],2,""); $total_amount+=$selectResult[csf('amount')];?></td>
                </tr>
                <?
                $i++;
                }
	...
	<table width="1360" class="rpt_table" border="0" rules="all">
        <tfoot>
            <tr>
                ...
                <th width="50"></th>
                <th width="60"></th>
                <th width=""><? echo number_format($total_amount,2);?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
            </tr>
        </tfoot>
	</table>
<?
...
$sql="select id,job_no,po_break_down_id,pre_cost_fabric_cost_dtls_id,trim_group,sensitivity,uom,delivery_date,wo_qnty,rate,amount,exchange_rate,country_id_string,gmt_item from wo_booking_dtls  where id='$data'";
	 $data_array=sql_select($sql);
     foreach ($data_array as $row)
	 {
        $poIdArr[$row[csf("po_break_down_id")]]=$row[csf("po_break_down_id")];
     }
     $ProdId_cond=implode(",",$poIdArr);

     $po_number=return_library_array("select id,po_number from wo_po_break_down where status_active=1 and id in($ProdId_cond)", "id", "po_number");
	 foreach ($data_array as $row)
	 {
		//echo "load_drop_down( 'requires/trims_booking_urmi_controller', '".$row[csf("job_no")]."', 'load_drop_down_trim_precost_id', 'trim_group_td' );\n";
		...
		//echo "load_drop_down( 'requires/trims_booking_urmi_controller', '".$row[csf("job_no")]."', 'load_drop_down_gmt_item', 'gmt_item_td' );\n";
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$field_array="id, booking_type, is_short, booking_no_prefix, booking_no_prefix_num, booking_no, company_id, buyer_id, job_no, 	item_category, supplier_id, currency_id, exchange_rate, booking_date, delivery_date, pay_mode, source, attention, ready_to_approved, tenor, remarks, entry_form, item_from_precost,inserted_by,insert_date";
		$data_array ="(".$id.",2,".$cbo_isshort.",'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',".$cbo_company_name.",".$cbo_buyer_name.",".$txt_job_no.",4,".$cbo_supplier_name.",".$cbo_currency.",".$txt_exchange_rate.",".$txt_booking_date.",".$txt_delivery_date.",".$cbo_pay_mode.",".$cbo_source.",".$txt_attention.",".$cbo_ready_to_approved.",".$txt_tenor.",".$txt_remarks.",260,1,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
	...
	else if ($operation==1)   // Update Here
	{
		...
		//}
		 $field_array_up="is_short*buyer_id*job_no*item_category*supplier_id*currency_id*exchange_rate*booking_date*delivery_date*pay_mode*source*attention*ready_to_approved*tenor*remarks*entry_form*item_from_precost*updated_by*update_date";

		 $data_array_up ="".$cbo_isshort."*".$cbo_buyer_name."*".$txt_job_no."*4*".$cbo_supplier_name."*".$cbo_currency."*".$txt_exchange_rate."*".$txt_booking_date."*".$txt_delivery_date."*".$cbo_pay_mode."*".$cbo_source."*".$txt_attention."*".$cbo_ready_to_approved."*".$txt_tenor."*".$txt_remarks."*260*1*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
...
if ($action=="save_update_delete_dtls")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		if($db_type==0)
		{
		 ...
		 $field_array1="id,pre_cost_fabric_cost_dtls_id,po_break_down_id,job_no,booking_no,booking_mst_id,booking_type,is_short,trim_group,uom,sensitivity,wo_qnty,rate,exchange_rate,amount,delivery_date,country_id_string,gmt_item,inserted_by,insert_date";
		...
		$data_array1 .="(".$id_dtls.",".$cbo_trim_precost_id.",".$txt_po_id.",".$txt_job_no.",".$txt_booking_no.",".$booking_mst_id.",2,".$cbo_isshort.",".$txt_trim_group_id.",".$cbo_uom.",".$cbo_colorsizesensitive.",".$txt_quantity.",".$txt_avg_price.",".$txt_exchange_rate_dtls.",".$txt_amount.",".$txt_delevary_date.",".$txt_country.",".$cbo_gmt_item_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		//echo $data_array1;
		//==============================
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			...
			if(str_replace("'","",$$itemcolor)!="")
			{
				 ...
				 {
					  $color_id = return_id( str_replace("'","",$$itemcolor), $color_library, "lib_color", "id,color_name","260");
	...
	else if ($operation==1)   // Update Here
	{
		...
		//==============================
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			...
			if($curAmt>$reqAmt){
					echo "vad1**".str_replace("'","",$txt_booking_no)."**".$i."**".$curAmt."**".$reqAmt;
					//check_table_status( $_SESSION['menu_id'],0);
					 disconnect($con);die;
				}
			}
    ...
    <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            <table width="1120" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
                ...
                <tr class="general">
                    <td>
                        <?
                            echo create_drop_down( "cbo_company_mst", 120, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", '', "load_drop_down( 'trims_booking_urmi_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
                        ?>
                    </td>
                     <td id="buyer_td">
                          <?
                ...
                <td><input name="txt_date_from" id="txt_date_from" class="datepicker" style="width:60px"></td>
                 ...
                 <td align="center">
                    <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('cbo_supplier_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_booking_prifix').value+'_'+document.getElementById('cbo_search_category').value+'_'+document.getElementById('txt_file_no').value+'_'+document.getElementById('chk_job_wo_po').value+'_'+document.getElementById('txt_ref').value+'_'+document.getElementById('txt_job').value, 'create_booking_search_list_view', 'search_div', 'trims_booking_urmi_controller','setFilterGrid(\'list_view\',-1)')" style="width:100px;" /></td>
            </tr>
            <tr>
...
$booking_prefix=trim($data[6]);
   ...
   if($data[7]==2)
	...
	if($data[9]==0)
	{
		$sql="select a.id, a.booking_no_prefix_num, a.booking_no, company_id, a.job_no, a.supplier_id, a.booking_date, a.delivery_date, c.file_no, c.grouping, a.inserted_by from wo_booking_mst a, wo_booking_dtls b, wo_po_break_down c where a.booking_no=b.booking_no and b.po_break_down_id=c.id and a.booking_type=2 and a.entry_form=260 and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 $company $buyer ".set_user_lavel_filtering(' and a.buyer_id','buyer_id')." $supplier_id $booking_date $booking_cond $job_cond $file_cond $ref_cond group by a.id, a.booking_no_prefix_num, a.job_no, a.booking_no, company_id, a.supplier_id, a.booking_date, a.delivery_date, c.file_no, c.grouping, a.inserted_by order by a.id DESC";

		echo  create_list_view("list_view", "Booking No,Company,Supplier,Insert User,Booking Date,Delivery Date,Job No,File No,Ref. No", "60,100,100,100,70,70,100,100","900","300",0, $sql , "js_set_value", "booking_no", "", 1, "0,company_id,supplier_id,inserted_by,0,0,0,0,0", $arr , "booking_no_prefix_num,company_id,supplier_id,inserted_by,booking_date,delivery_date,job_no,file_no,grouping", '','','0,0,0,0,3,3,0','','');
	}
	else
	{
		$sql="select a.id, a.job_no, a.booking_no_prefix_num, a.booking_no, company_id, a.supplier_id, a.booking_date, a.delivery_date, a.inserted_by from wo_booking_mst a where  a.booking_no not in ( select a.booking_no from wo_booking_mst a, wo_booking_dtls b, wo_po_break_down c where a.booking_no=b.booking_no and b.po_break_down_id=c.id and  a.booking_type=2 and a.entry_form=260  and b.status_active =1  $company $buyer $supplier_id $booking_date $booking_cond $job_cond $file_cond $ref_cond group by a.booking_no_prefix_num, a.booking_no, company_id, a.supplier_id, a.booking_date, a.delivery_date) and a.status_active =1 and a.is_deleted=0 and a.booking_type=2 and a.entry_form=260 $company $buyer ".set_user_lavel_filtering(' and a.buyer_id','buyer_id')."  $supplier_id $booking_date $booking_cond group by a.id, a.booking_no_prefix_num, a.booking_no, a.job_no, company_id, a.supplier_id, a.booking_date, a.delivery_date, a.inserted_by order by a.id DESC";

		echo  create_list_view("list_view", "Booking No,Company,Supplier,Insert User,Booking Date,Delivery Date,Job No", "60,100,100,100,70,70","700","300",0, $sql , "js_set_value", "booking_no", "", 1, "0,company_id,supplier_id,inserted_by,0,0,0,0", $arr , "booking_no_prefix_num,company_id,supplier_id,inserted_by,booking_date,delivery_date,job_no", '','','0,0,0,0,3,3,0,0','','');
	}
	exit();
}
...
function fnc_fabric_booking_terms_condition( operation )
{
	...
	http.open("POST","trims_booking_urmi_controller.php",true);
...
if ($action=="populate_data_from_search_popup")
{
	 $sql= "select id, booking_no,booking_date,company_id,ready_to_approved,tenor,buyer_id,job_no,currency_id,exchange_rate,pay_mode,supplier_id,attention,delivery_date,source,is_approved,is_short,remarks,item_from_precost from wo_booking_mst  where booking_no='$data'";
	 //echo $sql;die;
	 $data_array=sql_select($sql);
	 foreach ($data_array as $row)
	 {
		...
		echo "load_drop_down( 'requires/trims_booking_urmi_controller', '".$row[csf("pay_mode")]."', 'load_drop_down_supplier', 'supplier_td' );\n";
...
if($action=="show_trim_booking_report")
{
    ...
    $imge_arr=return_library_array( "select c.master_tble_id,c.image_location from   common_photo_library c where   c.form_name='company_details'  and c.file_type=1",'master_tble_id','image_location');
	...
	<div style="width:1333px" align="center">
       <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid black">
		...
		$job_no="";
		$currency_id="";
		$currency_id="";$order_repeat_no="";
		$style_ref="";$buyer_string="";
		$nameArray_job=sql_select( "select distinct b.job_no ,c.buyer_name,c.style_ref_no,c.order_repeat_no from wo_booking_mst a, wo_booking_dtls b,wo_po_details_master c where a.booking_no=b.booking_no and c.job_no=b.job_no and a.booking_no=$txt_booking_no"); 
        foreach ($nameArray_job as $result_job)
        {
            ...
            $order_repeat_no.=$result_job[csf('order_repeat_no')].", ";
		}
        ...
        foreach ($nameArray_job as $result_job)
        {
            ...
            if($result_job[csf('file_no')])
            {
                $order_file_no_arr[$result_job[csf('file_no')]]=$result_job[csf('file_no')];
            }
            if($result_job[csf('grouping')])
            {
                $order_ref_no_arr[$result_job[csf('grouping')]]=$result_job[csf('grouping')];
            }
		}
		...
		$style_ref="";
		$nameArray_style=sql_select( "select  a.style_ref_no,a.order_repeat_no,a.season_buyer_wise  from wo_po_details_master a, wo_booking_dtls b where a.job_no=b.job_no and b.booking_no=$txt_booking_no and a.status_active=1 and b.status_active=1 group by a.style_ref_no,a.order_repeat_no,a.season_buyer_wise");
        foreach ($nameArray_style as $result_style)
        {
            ...
            $season=$result_style[csf('season_buyer_wise')];
            $seasons_name.=$season_arr[$season].',';
		}
       ...
       <table width="100%" style="border:1px solid black">
            ...
            <tr>
              ...
              <tr>
                ...
                <td style="font-size:12px">:&nbsp;<? echo rtrim($po_qty,", "); ?> </td>
                <td width="110" style="font-size:12px"><b>Season</b> </td>
                <td style="font-size:12px">:&nbsp;<? $seasons_names=rtrim($seasons_name,',');
            $seasons_names_res=implode(",",array_unique(explode(",",$seasons_names))); echo $seasons_names_res; ?> </td>
            ...
            <tr>
             ...
             <tr>
				...
				// //$order_file_no_arr
				// echo rtrim($file_no,',');
                echo implode(",",$order_file_no_arr);
				?>
                </td>
...
if($action=="show_trim_booking_report1")
{
	...
	<div style="width:1333px" align="center">
       <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid black">
		...
		$po_no="";$po_id="";
		$nameArray_job=sql_select( "select distinct b.id as po_id, b.po_number,b.file_no,b.grouping  from wo_booking_dtls a, wo_po_break_down b where a.po_break_down_id=b.id and a.booking_no=$txt_booking_no");
        foreach ($nameArray_job as $result_job)
        {
			$po_no.=$result_job[csf('po_number')].", ";
			$po_id.=$result_job[csf('po_id')].",";
...
?>
		...
		$style_ref="";
		$deling_marchent="";
		$nameArray_style=sql_select( "select distinct a.style_ref_no,a.dealing_marchant,a.season_buyer_wise  from wo_po_details_master a, wo_booking_dtls b where a.job_no=b.job_no and b.booking_no=$txt_booking_no and  b.status_active=1");
        foreach ($nameArray_style as $result_style)
        {
			$style_ref.=$result_style[csf('style_ref_no')].", ";
			$deling_marchent.=$team_member_arr[$result_style[csf('dealing_marchant')]].", ";
       ...
       <table width="100%" style="border:1px solid black">
            ...
            <tr>
                <td width="100"><span style="font-size:12px"><b>Season</b></span> </td>
                <td width="110">:&nbsp;<? $seasons_names=rtrim($seasons_name,','); $seasons_names_res=implode(",",array_unique(explode(",",$seasons_names))); echo $seasons_names_res; ?></td>
            </tr>
...
if($action=="show_trim_booking_report5")
{
	...
	$buyer_name_arr=return_library_array( "select id,buyer_name from lib_buyer",'id','buyer_name');
	$order_uom_arr=return_library_array("select id,order_uom  from lib_item_group","id","order_uom");
    $po_number=return_library_array( "select b.id,b.po_number from wo_po_break_down b,wo_booking_dtls d where d.po_break_down_id=b.id and d.booking_no =$txt_booking_no  and d.status_active=1 and b.status_active=1 ", "id", "po_number");
    $sql="select id from electronic_approval_setup where company_id=$cbo_company_name and page_id in(336) and is_deleted=0";
	 // echo $sql;die;
	...
	<div style="width:1333px" align="center">
       <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid black">
        ...
        $nameArray=sql_select( "select a.booking_no, a.booking_date, a.supplier_id, a.currency_id, a.exchange_rate, a.attention, a.delivery_date, a.source, a.remarks, a.pay_mode,a.is_approved  from wo_booking_mst a where a.booking_no=$txt_booking_no and booking_type=2 and a.status_active=1");
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item)
            {
				...
				$nameArray_item_description=sql_select( "select  b.description, b.brand_supplier, b.item_color,  a.gmt_item, avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.trim_group=".$result_item[csf('trim_group')]." and a.delivery_date='".$result_item[csf('delivery_date')]."' and a.sensitivity=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.description, b.brand_supplier,b.item_color,  a.gmt_item");
            ...
            <tr>
				...
				if($db_type==0)
						...
						if($db_type==2)
				        {
						$nameArray_color_size_qnty=sql_select("select sum(b.requirment) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=0 and a.gmt_item='".$result_itemdescription[csf('gmt_item')]."' and a.trim_group=". $result_item[csf('trim_group')]." and nvl( b.description,0)=nvl('". $result_itemdescription[csf('description')]."',0) and nvl(b.brand_supplier,0)=nvl('".$result_itemdescription[csf('brand_supplier')]."',0) and nvl(b.item_color,0)=nvl('".$result_itemdescription[csf('item_color')]."',0) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
						}
   ...
   foreach($res_result_arr as $row){
	...
	<div style="width:1333px" align="center">
       <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid black">
        ...
        $nameArray=sql_select( "select a.booking_no,a.booking_date,a.pay_mode,a.supplier_id,a.currency_id,a.exchange_rate,a.attention,a.delivery_date,a.source,a.remarks,a.is_approved  from wo_booking_mst a where  a.booking_no=$txt_booking_no and a.status_active=1");
        foreach ($nameArray as $result)
        {
			$varcode_booking_no=$result[csf('booking_no')];
            $is_approved=$row[csf('is_approved')];
       ...
       //$nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=0 order by trim_group ");
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item)
            {
				...
				$nameArray_item_description=sql_select( "select a.pre_cost_fabric_cost_dtls_id, b.description, b.brand_supplier,b.item_color, a.gmt_item,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id='".$nameArray_job_po_row[csf('po_id')]."' and a.trim_group=".$result_item[csf('trim_group')]." and a.delivery_date='".$result_item[csf('delivery_date')]."' and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and a.sensitivity=0  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.pre_cost_fabric_cost_dtls_id, b.description, b.brand_supplier,b.item_color,a.gmt_item");
            ...
            <tr>
                ...
                <td align="center" style="border:1px solid black; word-break:break-all" rowspan="<? echo count($nameArray_item_description)+1; ?>">
			   ...
			   ?>
				...
				if($db_type==0)
						...
						if($db_type==2)
				        {
						$nameArray_color_size_qnty=sql_select("select sum(b.requirment) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id='".$nameArray_job_po_row[csf('po_id')]."'  and a.sensitivity=0 and a.gmt_item='".$result_itemdescription[csf('gmt_item')]."' and a.trim_group=". $result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and nvl( b.description,0)=nvl('". $result_itemdescription[csf('description')]."',0) and nvl(b.brand_supplier,0)=nvl('".$result_itemdescription[csf('brand_supplier')]."',0) and nvl(b.item_color,0)=nvl('".$result_itemdescription[csf('item_color')]."',0) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
						}
