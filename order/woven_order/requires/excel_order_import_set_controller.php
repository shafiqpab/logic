...
if ($action=="save_update_delete")
{
	...
	{	
		...
		{
			$k=1; $itemSetbreak_arr=array(); $styleSetbreak_arr=array(); 
					...
					foreach($color_size_data as $color_val=>$size_data)
					{
						...
						foreach($size_data as $size_val=>$extra_data)
						{
							foreach($extra_data as $ex_val=>$sizeqty)
							{
								...
								$recDate=$ex_data[1];
								$shipDate=$ex_data[2];
								$countryRate=number_format($ex_data[3],4,'.','');
								$poRemarks=$ex_data[4];
								//$poRemarks='';
								...
								$countryAmt=number_format($countryQty*$countryRate,6,'.','');
								...
								if(str_replace("'","",$txt_job_no)=="")
								{
									...
									{ 
										...
										$field_job="id, garments_nature, job_no, job_no_prefix, job_no_prefix_num, company_name, buyer_name, style_owner, location_name, style_ref_no, style_description, product_dept, currency_id, season_buyer_wise, order_repeat_no, product_category, team_leader, dealing_marchant, factory_marchant, order_uom, gmts_item_id, set_smv, packing, set_break_down, total_set_qnty, is_excel, is_deleted, status_active, inserted_by, insert_date";
										$data_job_arr[$id]="(".$id.",2,'".$new_job_no_arr[$style_ref][0]."','".$new_job_no_arr[$style_ref][1]."','".$new_job_no_arr[$style_ref][2]."',".$cbo_company_id.",".$cbo_buyer_id.",".$cbo_style_owner_id.",".$cbo_location_id.",'".$style_ref."','".$style_des."',".$cbo_product_department.",".$cbo_currercy_id.",".$cbo_season_id.",'".$repeat_no."',".$cbo_prod_catgory.",".$cbo_team_leader.",".$cbo_dealing_merchant.",".$cbo_factory_merchant.",".$cbo_order_uom.",'".$gmtsItem."','".$smv_set[$excelstyle]."',".$cbo_packing.",'".$setbreak_down_data."','".$tot_smv_qty[$excelstyle]."',1,0,1,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
								...
								if($p==1) 
								{
									...
									if($prev_po_arr[$order_no]!="")
									{
										...
										echo "11**".$msg_dup."**".$order_no;disconnect($con); die;
									}
