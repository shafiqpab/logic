...
if ($action=="save_update_delete_mst")
{
	...
	if ($operation==0) 
	{
		...
		$new_sys_number=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'LWO', date("Y"), 5, "select labtest_prefix, labtest_prefix_num from wo_labtest_mst where company_id=$cbo_company_name and entry_form=179 and TO_CHAR(insert_date,'YYYY')=".date('Y',time())." order by id DESC ", "labtest_prefix", "labtest_prefix_num",""));
		...
		$data_array="(".$id.", '".$new_sys_number[1]."', '".$new_sys_number[2]."', '".$new_sys_number[0]."', 179, ".$cbo_company_name.", ".$cbo_supplier.", ".$txt_workorder_date.", ".$txt_delivery_date.", ".$cbo_currency.", ".$txt_exchange_rate.", ".$cbo_pay_mode.", ".$txt_attention.",".$txt_tenor.", ".$txt_address.", ".$cbo_ready_to_approved.", ".$txt_vat_per.", '".$user_id."', '".$pc_date_time."',1,0)";
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		/*$is_approved=0;
		$sql=sql_select("select is_approved from wo_labtest_mst where booking_no=$txt_booking_no");
		foreach($sql as $row){
	...
	else if ($operation==2) // Delete Update Here----------------------------------------------------------
	{
		...
		$flag=1;
		$rID=sql_delete("wo_labtest_mst",$field_array,$data_array,"id","".$update_id."",0);
		if($flag==1 && $rID==1) $flag=1; else $flag=0;
		$rID1=sql_delete("wo_labtest_dtls",$field_array,$data_array,"mst_id","".$update_id."",0);
...
if($action=="save_update_delete_dtls")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$field_array="id, mst_id, entry_form, test_for, test_item_id, test_item_value, color, amount, discount, labtest_charge, wo_value, vat_amount, wo_with_vat_value, remarks,referance_no,style_no, inserted_by, insert_date, status_active, is_deleted, qty_breakdown";
		//$field_array1="id, mst_id, dtls_id, wo_value, order_qty, inserted_by, insert_date, status_active, is_deleted";
		$data_array="(".$id_dtls.",".$update_id.",179,".$cbo_test_for.",".$txt_party_type_id.",".$txt_party_type_name.",'".$color_id."',".$txt_amount.",".$txt_discount.",".$txt_delivery_charge.",".$txt_wo_value.",".$txt_vat_amount.",".$txt_wo_value_with_vat.",".$txt_remarks.",".$txt_transaction_ref.",".$txt_style_no.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,".$save_qty_break_data.")";
	...
	else if ($operation==1)  // Insert Here
	{
		...
		$field_array="test_for*test_item_id*test_item_value*color*amount*discount*labtest_charge*wo_value*vat_amount*wo_with_vat_value*remarks*referance_no*style_no*updated_by*update_date*qty_breakdown";
		$data_array="".$cbo_test_for."*".$txt_party_type_id."*".$txt_party_type_name."*'".$color_id."'*".$txt_amount."*".$txt_discount."*".$txt_delivery_charge."*".$txt_wo_value."*".$txt_vat_amount."*".str_replace(",","",$txt_wo_value_with_vat)."*".$txt_remarks."*".$txt_transaction_ref."*".$txt_style_no."*'".$user_id."'*'".$pc_date_time."'*".$save_qty_break_data."";//,".$txt_vat_amount.",".$txt_wo_value_with_vat."
		//print_r($data_array);
	...
	else if ($operation==2)  // Insert Here
	{
		...
		$data_array="".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*'0'*'1'";
		$flag=1;
		$rID1=sql_delete("wo_labtest_dtls",$field_array,$data_array,"id","".$update_dtls_id."",0);
		if($flag==1 && $rID1==1) $flag=1; else $flag=0;
            ...
            <tbody>
                <tr class="general">
                    <td><input type="hidden" id="selected_booking">
                    	<? echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", $cbo_company_name, ""); ?>
                    </td>
...
if ($action=="create_wo_search_list_view")
{
	...
	if($db_type==0)
	{
		...
		$year_id=" SUBSTRING_INDEX(insert_date, '-', 1) as year";
	...
	$sql= "SELECT id, labtest_prefix, labtest_prefix_num, labtest_no, entry_form, company_id, supplier_id, wo_date, delivery_date, currency, ecchange_rate, pay_mode, attention,address, ready_to_approved, inserted_by, insert_date, $year_id 
	from wo_labtest_mst 
	where status_active=1 and is_deleted=0 and entry_form=179 $company $buyer $booking_date $booking_cond order by id DESC";
	
	echo  create_list_view("list_view", "WO No,Year,Test Company,Currency,Exchange Rate,Wo Date,Delivery Date,Pay Mode,Attention,Ready To Approved", "60,60,130,70,60,70,70,80,140,70","860","250",0, $sql , "js_set_value", "id,labtest_no", "", 1, "0,0,supplier_id,currency,0,0,0,pay_mode,0,ready_to_approved", $arr , "labtest_prefix_num,year,supplier_id,currency,ecchange_rate,wo_date,delivery_date,pay_mode,attention,ready_to_approved", '','','0,0,0,0,1,3,3,0,0,0','','');
	exit();
}
if($action=="load_drop_down_Test_Supplier_new")
{
	$sql = "SELECT DISTINCT a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=26  and a.status_active=1    UNION ALL SELECT DISTINCT a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b,wo_labtest_mst c where a.id=b.supplier_id and b.party_type=26  and c.supplier_id = a.id  and a.status_active IN(1,3) order by supplier_name ";
...
{
	$data=explode("_",$data);
	$sql= "select id, labtest_prefix, labtest_prefix_num, labtest_no, entry_form, company_id, supplier_id, wo_date, delivery_date, currency, ecchange_rate, pay_mode, attention, tenor, address, ready_to_approved, vat_percent, inserted_by, insert_date from wo_labtest_mst where labtest_no='$data[1]'"; 
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_drop_down( 'requires/labtest_wo_without_po_controller', ".$row[csf("company_id")].", 'load_drop_down_Test_Supplier_new', 'test_supplier' );\n";
...
{
	$sql= "select id, mst_id, test_for, test_item_id, test_item_value, color, amount, discount, labtest_charge, wo_value, vat_amount, wo_with_vat_value, remarks,referance_no,style_no, qty_breakdown from wo_labtest_dtls where id=$data and entry_form=179";
	//echo $sql;die; 
	$data_array=sql_select($sql);
	foreach ($data_array as $row)
	{

		echo "load_drop_down( 'requires/labtest_wo_without_po_controller', document.getElementById('cbo_company_name').value, 'load_drop_down_Test_Supplier_new', 'test_supplier' );\n";
		...
		echo "document.getElementById('txt_style_no').value = '".$row[csf("style_no")]."';\n";
...
$varcode_booking_no=$dataArray[0][csf('labtest_no')];
$wo_date=$dataArray[0][csf('wo_date')];
...
<div style="margin-top: 20px"></div>
	<table align="center">
		...
		<tr>
            ...
            <td><strong>  WO Date</strong></td>
			<td><b> : </b><?  
			echo change_date_format($wo_date);  ?></td>
