...
if ($action=="set_precost_data")
{
	...
	if($data[6]==1)
	{
		...
		if(str_replace("'","",$data[1]) !=''){
			$condition->po_id("=$data[1]");
		}
		$condition->init();
		...
		$trims_req_qty_arr=$trims->getQtyArray_by_orderCountryGmtsitemAndPrecostdtlsid();
		$trims_req_amt_arr=$trims->getAmountArray_by_orderCountryGmtsitemAndPrecostdtlsid();
		//print_r($trims_req_qty_arr);
		...
		list($sql_po_qty_row)=$sql_po_qty;
		$po_qty=$sql_po_qty_row[csf('order_quantity_set')];
		//echo "select a.id, a.job_no, a.trim_group, a.cons_dzn_gmts, a.rate, a.amount, a.description, a.brand_sup_ref, b.cons, b.country_id from wo_pre_cost_trim_cost_dtls a , wo_pre_cost_trim_co_cons_dtls b where a.id=b.wo_pre_cost_trim_cost_dtls_id and  a.id='$data[0]' and b.po_break_down_id='$data[1]' and a.is_deleted=0  and a.status_active=1"; die;
		$sql_data=sql_select("select a.id, a.job_no, a.trim_group, a.cons_dzn_gmts, a.rate, a.amount, a.description, a.brand_sup_ref, b.cons, b.country_id from wo_pre_cost_trim_cost_dtls a , wo_pre_cost_trim_co_cons_dtls b where a.id=b.wo_pre_cost_trim_cost_dtls_id and  a.id='$data[0]' and b.po_break_down_id='$data[1]' and a.is_deleted=0  and a.status_active=1");
		foreach($sql_data as $row)
		{
			if($row[csf('country_id')]==0) $txt_country_cond1="";
		...
		$po_id=$data[1];$item_id=$data[3];$trims_id=$data[0];
		$trims_req_qty=0;$trims_req_amt=0;//getQtyArray_by_orderCountryGmtsitemAndPrecostdtlsid
		foreach($selected_country_array as $cid)
		{
			$trims_req_qty+=$trims_req_qty_arr[$po_id][$cid][$item_id][$trims_id];
			$trims_req_amt+=$trims_req_amt_arr[$po_id][$cid][$item_id][$trims_id];
		}
		
		if($data[5]==1) //add new
		{
			$sql_trim=sql_select("select trim_group, job_no, brand_sup_ref, description, amount from wo_pre_cost_trim_cost_dtls where id=".$data[0]."");
			$trim_group_id=0; $amount=0;
			foreach($sql_trim as $row)
			{
				$trim_group_id=$row[csf('trim_group')];
			...
			$cons_uom=return_field_value("order_uom", "lib_item_group", "id=".$trim_group_id."");
			$conversion_factor=return_field_value("conversion_factor", "lib_item_group", "id=".$trim_group_id."");
			//$costing_per=return_field_value("costing_per", "wo_pre_cost_mst", "job_no='".$row[csf('job_no')]."'");
			...
			$req_qnty_cons_uom=$trims_req_qty;//def_number_format(($row[csf('cons')]*($po_qty1/$costing_per_qty)),5,"");
			$req_amount_cons_uom=$trims_req_amt;
			
			//def_number_format($req_qnty_cons_uom*$row[csf("rate")],5,"");
			$avg_rate=$req_amount_cons_uom/$req_qnty_cons_uom;

			$req_qnty=def_number_format($req_qnty_cons_uom/$conversion_factor,5,"");
			$txt_avg_price=def_number_format(($avg_rate*$conversion_factor)*$exchange_rate,5,"");

			echo "document.getElementById('txt_trim_group_id').value = '".$trim_group_id."';\n";
			echo "document.getElementById('cbo_uom').value = '".$cons_uom."';\n";
			echo "document.getElementById('txt_req_quantity').value = '".$req_qnty."';\n";
			echo "document.getElementById('txt_req_amt').value = '".$amount."';\n";
			echo "document.getElementById('txt_avg_price').value = '".$txt_avg_price."';\n";
		...
		if($data[5]==2)
		{
			...
			$req_qnty_cons_uom=$trims_req_qty;//def_number_format(($row[csf('cons')]*($po_qty1/$costing_per_qty)),5,"");
			$req_amount_cons_uom=$trims_req_amt;//def_number_format($req_qnty_cons_uom*$row[csf("rate")],5,"");
			$avg_rate=$req_amount_cons_uom/$req_qnty_cons_uom;
		...
		/*	$req_qnty_cons_uom=def_number_format(($row[csf('cons')]*($po_qty1/$costing_per_qty)),5,"");
			...
			$req_qnty_cons_uom=def_number_format($req_qnty_cons_uom,5,"");
			$req_amount_cons_uom=def_number_format($req_amount_cons_uom,5,"");
			$req_qnty=def_number_format($req_qnty_cons_uom/$conversion_factor,5,"");
			$avg_rate=$req_amount_cons_uom/$req_qnty_cons_uom;
			$txt_avg_price=def_number_format(($avg_rate*$conversion_factor)*$exchange_rate,5,"");
...
if($action=="booking_qnty_and_amount_trim_group")
{
   ...
   $woqty="";
   $amount="";
   $data_sql=sql_select("select sum(wo_qnty) as wo_qnty, sum(amount) as amount from wo_booking_dtls where po_break_down_id=$txt_po_id and trim_group=$txt_trim_group_id and pre_cost_fabric_cost_dtls_id=$cbo_trim_precost_id and b.gmt_item=$gmt_item_id and booking_type=2 and status_active=1 and is_deleted=0");
...
if ($action=="generate_fabric_booking")
{
	...
	<div align="center" style="width:1050px;" >
        <fieldset>
            ...
            $sql_po_qty=sql_select("select sum(c.order_quantity) as order_quantity,(sum(c.order_quantity)/a.total_set_qnty) as order_quantity_set  from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id and  b.id='$txt_po_id'  $txt_country_cond  and a.status_active=1 and b.status_active=1 and c.is_deleted=0  and c.status_active=1 group by b.id,a.total_set_qnty");//and c.item_number_id='$cbo_gmt_item_id'
            ...
            <table width="1050" cellspacing="0" class="rpt_table" border="0" id="tbl_consmption_cost" rules="all">
                ...
                if($cbo_colorsizesensitive==1)
                {
					...
					$sql="select b.id, b.po_number,b.po_quantity,min(c.id) as color_size_table_id,c.color_number_id,sum(c.order_quantity) as order_quantity,(sum(c.order_quantity)/a.total_set_qnty) as order_quantity_set  from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id and  b.id='$txt_po_id'  $txt_country_cond and a.status_active=1 and b.status_active=1 and c.status_active=1 and c.is_deleted=0  group by  b.id, b.po_number,b.po_quantity,a.total_set_qnty,c.color_number_id order by b.id, c.color_number_id";//and c.item_number_id='$cbo_gmt_item_id'
...
if ($action=="create_booking_search_list_view")
{
   ...
   if($data[7]==2)
	...
	echo $sql_Data="select a.id,a.booking_no,a.pay_mode,a.supplier_id from  wo_booking_mst a , wo_booking_dtls b, wo_po_break_down c where a.booking_no=b.booking_no  and b.po_break_down_id=c.id and  a.booking_type=2 and a.entry_form=43   $company  $buyer  $supplier_id $booking_date $booking_cond $job_cond $file_cond $ref_cond group by a.id,a.booking_no_prefix_num, a.booking_no,company_id,a.supplier_id,a.booking_date,a.delivery_date";
...
?>
		...
		$job_no="";
		$currency_id="";
		$nameArray_job=sql_select( "select distinct b.job_no,min(b.delivery_date) as  delivery_date from wo_booking_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and a.booking_no=$txt_booking_no group by b.job_no");
        foreach ($nameArray_job as $result_job)
        {

			$job_no.=$result_job[csf('job_no')].", ";
			$min_delivery_date=$result_job[csf('delivery_date')];
		}
		$buyer_string="";
		...
		$style_ref="";
		$deling_marchent="";
		$nameArray_style=sql_select( "select distinct a.style_ref_no,a.dealing_marchant,b.delivery_date,b.po_break_down_id as po_id  from wo_po_details_master a, wo_booking_dtls b where a.job_no=b.job_no and b.booking_no=$txt_booking_no");
		
        foreach ($nameArray_style as $result_style)
        {
			$style_ref.=$result_style[csf('style_ref_no')].", ";
			$deling_marchent.=$team_member_arr[$result_style[csf('dealing_marchant')]].", ";
			$po_delivery_date_arr[$result_style[csf('po_id')]]=$result_style[csf('delivery_date')];
		}
		//print_r($po_delivery_date_arr);
        $nameArray=sql_select("select a.booking_no,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.attention,a.delivery_date,a.source,a.remarks,a.pay_mode  from wo_booking_mst a where  a.booking_no=$txt_booking_no");
        foreach ($nameArray as $result)
        {
			$varcode_booking_no=$result[csf('booking_no')];
            ...
            <tr>
             ...
             <tr>
              ...
              <tr>
                ...
                <td width="110">:&nbsp;<? echo change_date_format($min_delivery_date,'dd-mm-yyyy','-');?> </td>
	   ...
	   if(count($nameArray_item)>0)
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
           <tr>
                <td colspan="9" align="">
                <strong><? echo "Job NO:".$nameArray_job_po_row[csf('job_no')]." &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;PO No:" .$po_number[$nameArray_job_po_row[csf('po_break_down_id')]]." &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;PO Qty:" .$nameArray_job_po_row[csf('po_quantity')]."&nbsp; &nbsp; &nbsp; &nbsp;Delivery Date:".change_date_format($po_delivery_date_arr[$nameArray_job_po_row[csf('po_break_down_id')]]);;?></strong>
                </td>
            </tr>
            <tr>
                <td colspan="9" align="">
...
?>
		...
		$nameArray_buyer=sql_select( "select  b.job_no,b.delivery_date,a.buyer_name,a.agent_name,a.style_ref_no,a.dealing_marchant,c.id  as po_id,c.po_number,c.shiping_status,c.shipment_date,c.pub_shipment_date,d.costing_per   from wo_po_details_master a, wo_booking_dtls b, wo_po_break_down c ,wo_pre_cost_mst d where a.job_no=b.job_no and a.job_no=c.job_no_mst and a.job_no=d.job_no and b.po_break_down_id=c.id and b.booking_no=$txt_booking_no and b.booking_type=2 order by c.shipment_date desc");
        foreach ($nameArray_buyer as $result_buy)
		{
			$agent_name_id=$result_buy[csf('agent_name')];
			if($agent_name_id) $agent_name=' ('.$buyer_name_arr[$agent_name_id].')';else $agent_name="";
			$job_no_string_arr[$result_buy[csf('job_no')]]=$result_buy[csf('job_no')];
			$buyer_string_arr[$result_buy[csf('buyer_name')]]=$buyer_name_arr[$result_buy[csf('buyer_name')]].$agent_name;
			...
			$deliver_date_arr[$result_buy[csf('po_id')]]=$result_buy[csf('delivery_date')];
		}
		...
		$sql_order=sql_select("select order_quantity,color_number_id,size_number_id,po_break_down_id,country_id from wo_po_color_size_breakdown  where po_break_down_id in($po_id ) and status_active=1 and is_deleted=0");
		...
		$nameArray=sql_select("select a.booking_no,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.attention,min(b.delivery_date) as delivery_date,a.pay_mode,a.source,a.remarks, $select_update_date
	   ...
	   if(count($nameArray_item)>0){
        ...
        <table style="border:1px solid black" align="left" class=""  cellpadding="0" width="100%" cellspacing="0" rules="all" >
           <tr>
				...
				echo "Job NO:".$nameArray_job_po_row[csf('job_no')]." &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp; Style Ref:".$style_ref." &nbsp;&nbsp; &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; PO No:" .$po_number[$nameArray_job_po_row[csf('po_break_down_id')]]." &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;PO Qty:" .$po_qty_po_level[$nameArray_job_po_row[csf('po_break_down_id')]]." &nbsp; &nbsp;Delivery_date:".change_date_format($deliver_date_arr[$nameArray_job_po_row[csf('po_break_down_id')]]);?></strong>
                </td>
            </tr>
            <tr>
                <td colspan="11" align="">
			...
			$sql_trim_amount=sql_select("select  calculatorstring,cons_uom,trim_group,job_no,description, cons_dzn_gmts, rate, amount from  wo_pre_cost_trim_cost_dtls where job_no='".$nameArray_job_po_row[csf('job_no')]."' and  status_active=1 and is_deleted=0 ");
			foreach($sql_trim_amount as $item){
				$calculatorstring=explode("_",$item[csf('calculatorstring')]);
				$trims_cons_gmts_per_arr[$item[csf("job_no")]][$item[csf("trim_group")]]['gmt_per']=$calculatorstring[0];
				$trims_cons_gmts_per_arr[$item[csf("job_no")]][$item[csf("trim_group")]]['cone_len']=$calculatorstring[2];
            ...
            foreach($nameArray_item as $result_item){
				$i++;
				$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,b.item_color, b.color_number_id ,min(b.color_size_table_id) as color_size_table_id,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and  a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=1 and   a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and  b.status_active=1 and b.requirment !=0 and b.requirment>0 group by b.description, b.brand_supplier,b.item_color, b.color_number_id order by color_size_table_id ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
               ...
               <? echo $color_library[$result_itemdescription[csf('color_number_id')]]; ?>
                ...
                <td style="border:1px solid black; text-align:right">
                <? 
                	//echo $nameArray_job_po_row[csf('job_no')].'--'.$result_item[csf("trim_group")].'--'.$result_itemdescription[csf('description')];
                	$cone_length = $trims_cons_gmts_per_arr[$nameArray_job_po_row[csf('job_no')]][$result_item[csf("trim_group")]]['cone_len'];
                	echo $cone_length;
                 ?> 
                </td>
        ...
        <table style="border:1px solid black" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            <tr>
                <td colspan="12" align="">
                <strong><? echo "Job NO:".$nameArray_job_po_row[csf('job_no')]." &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; Style Ref:&nbsp; ".$style_ref."  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;PO No:" .$po_number[$nameArray_job_po_row[csf('po_break_down_id')]]." &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;PO Qty:" .$po_qty_po_level[$nameArray_job_po_row[csf('po_break_down_id')]]." &nbsp; &nbsp; Delivery Date:".change_date_format($deliver_date_arr[$nameArray_job_po_row[csf('po_break_down_id')]]);?></strong>
                </td>
            </tr>
            <tr>
                <td colspan="12" align="">
            ...
            foreach($nameArray_item as $result_item){
			$i++;
			$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,b.item_size,b.gmts_sizes,min(b.color_size_table_id) as color_size_table_id, sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id=b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=2 and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and  b.status_active=1 and b.requirment>0 group by b.description, b.brand_supplier,b.item_size, b.gmts_sizes order by color_size_table_id");
            ...
            <tr>
              ...
              <? echo $size_library[$result_itemdescription[csf('gmts_sizes')]];?>
                ...
                <td style="border:1px solid black;text-align:center"><? echo $trims_cons_gmts_per_arr[$nameArray_job_po_row[csf('job_no')]][$result_item[csf("trim_group")]]['cone_len']; ?></td>
        ...
        <table style="border:1px solid black" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
           <tr>
            ...
            foreach($nameArray_item as $result_item){
				$i++;
				$gmtcolor_library=return_library_array( "select b.item_color,b.color_number_id from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=3 and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and  b.status_active=1 and b.requirment !=0  order by trim_group ", "item_color", "color_number_id"  );
				$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,b.item_color,b.color_number_id,sum(b.requirment) as cons,min(b.color_size_table_id) as color_size_table_id,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=3 and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and  b.status_active=1 and b.requirment !=0 group by b.description, b.brand_supplier,b.item_color,b.color_number_id order by color_size_table_id ");
				$sql_trim_amount=sql_select("select  cons_uom, cons_dzn_gmts, rate, amount from  wo_pre_cost_trim_cost_dtls where job_no='".$nameArray_job_po_row[csf('job_no')]."'  and trim_group=".$result_item[csf('trim_group')]." ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
               ...
               <? echo $color_library[$result_itemdescription[csf('item_color')]]; ?>
				...
				?>
                 ...
                 <td style="border:1px solid black; text-align:right"><? echo $trims_cons_gmts_per_arr[$nameArray_job_po_row[csf('job_no')]][$result_item[csf("trim_group")]]['cone_len']; ?> </td>
	   ...
	   if(count($nameArray_item)>0)
        ...
        <table style="border:1px solid black" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            {
			$i++;
			$nameArray_color=sql_select( "select  b.item_color,b.color_number_id,b.item_size,b.gmts_sizes,min(b.color_size_table_id) as color_size_table_id,b.description, b.brand_supplier,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id   and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and  b.status_active=1 and a.sensitivity=4 and b.requirment !=0 group by b.item_color,b.color_number_id,b.item_size,b.gmts_sizes,b.description, b.brand_supplier order by color_size_table_id ");
            $nameArray_item_description=sql_select( "select distinct uom from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=4   and trim_group=".$result_item[csf('trim_group')]." order by trim_group ");
			$sql_trim_amount=sql_select("select  cons_uom, cons_dzn_gmts, rate, amount from  wo_pre_cost_trim_cost_dtls where job_no='".$nameArray_job_po_row[csf('job_no')]."'  and trim_group=".$result_item[csf('trim_group')]." ");
            ?>
            <tr>
				...
				foreach($nameArray_color as $result_color)
                {
					...
					<td style="border:1px solid black;text-align:center"><? echo $unit_of_measurement[$order_uom_arr[$result_item[csf('trim_group')]]]; $uom_arr_check[$order_uom_arr[$result_item[csf('trim_group')]]]= $order_uom_arr[$result_item[csf('trim_group')]];?>
						
					</td>

					<td style="border:1px solid black; text-align:right"><? echo $trims_cons_gmts_per_arr[$nameArray_job_po_row[csf('job_no')]][$result_item[csf("trim_group")]]['cone_len'];  ?>
                    ...
                    <td style="border:1px solid black; text-align:right"><? $trim_amount= $trim_amount_arr[$nameArray_job_po_row[csf('job_no')]][$result_item[csf("trim_group")]][$result_color[csf('description')]]['amount']; echo ($trim_amount*$costing_per_qty)/12;  ?>
	   ...
	   ?>
        ...
        <table  style="border:1px solid black" align="left" class=""  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item)
            {
				...
				$nameArray_item_description=sql_select( "select  b.description, b.brand_supplier,b.item_color,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and  a.id= ".$result_item[csf('id')]." and  b.status_active=1 and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=0 group by b.description, b.brand_supplier,b.item_color");
            ...
            <tr>
				...
				if($db_type==0)
				{
				$nameArray_color_size_qnty=sql_select("select sum(b.requirment) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=0 and  a.id= ".$result_item[csf('id')]." and a.trim_group=". $result_item[csf('trim_group')]." and  b.description='". $result_itemdescription[csf('description')]."' and b.brand_supplier='".$result_itemdescription[csf('brand_supplier')]."' and  b.status_active=1 and b.item_color='".$result_itemdescription[csf('item_color')]."'");
				}
				if($db_type==2)
				{
				$nameArray_color_size_qnty=sql_select("select sum(b.requirment) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=0 and  a.id= ".$result_item[csf('id')]." and a.trim_group=". $result_item[csf('trim_group')]." and nvl( b.description,0)=nvl('". $result_itemdescription[csf('description')]."',0) and nvl(b.brand_supplier,0)=nvl('".$result_itemdescription[csf('brand_supplier')]."',0) and  b.status_active=1  and nvl(b.item_color,0)=nvl('".$result_itemdescription[csf('item_color')]."',0)");
				}
    ...
    <table cellspacing="0" cellpadding="0" style="border:1px solid black" rules="all" width="100%" class=""  align="left">
           ...
           <?
					...
					$trims_costing_arr=$trim->getAmountArray_by_orderAndPrecostdtlsid();
                    ...
                    foreach ($nameArray as $selectResult){
						...
						$sql_po_qty=sql_select("select sum(c.order_quantity) as order_quantity,(sum(c.order_quantity)/a.total_set_qnty) as order_quantity_set 
						 from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c where a.id=b.job_id and a.id=c.job_id 
						 and b.id=c.po_break_down_id and  b.id='".$selectResult[csf('po_break_down_id')]."' $contry_cond  and a.status_active=1 and b.status_active=1 and c.status_active=1 group by b.id,a.total_set_qnty");
						list($sql_po_qty_row)=$sql_po_qty;
						$po_qty=$sql_po_qty_row[csf('order_quantity_set')];
					...
					//	$pre_amount=$pre_req_qnty*$pre_rate;
						$pre_amount=$trims_costing_arr[$selectResult[csf('po_break_down_id')]][$selectResult[csf('pre_cost_fabric_cost_dtls_id')]];
...
?>
		...
		$job_style_arr=array();
		$costing_per_arr=array();
		$nameArray_buyer=sql_select( "select  b.job_no,a.buyer_name,a.style_ref_no,a.agent_name,a.dealing_marchant,c.id  as po_id,c.po_number,c.shiping_status,min(c.pub_shipment_date) as pub_shipment_date,d.costing_per,b.delivery_date   from wo_po_details_master a, wo_booking_dtls b, wo_po_break_down c ,wo_pre_cost_mst d where a.job_no=b.job_no and a.job_no=c.job_no_mst and a.job_no=d.job_no and b.po_break_down_id=c.id and b.booking_no=$txt_booking_no and b.booking_type=2 group by  b.job_no,a.buyer_name,a.style_ref_no,a.agent_name,a.dealing_marchant,c.id,c.po_number,c.shiping_status,d.costing_per,b.delivery_date  order by c.id desc");
        foreach ($nameArray_buyer as $result_buy)
		{
			$agent_name_id=$result_buy[csf('agent_name')];
			if($agent_name_id) $agent_name=' ('.$buyer_name_arr[$agent_name_id].')';else $agent_name="";
			...
			$po_num_arr[$result_buy[csf('po_id')]]=$result_buy[csf('po_number')];
			$po_id_arr[$result_buy[csf('po_id')]]=$result_buy[csf('po_id')];
			$po_delivery_date_arr[$result_buy[csf('po_id')]]=$result_buy[csf('delivery_date')];
			$po_status_arr[]=$result_buy[csf('shiping_status')];
		...
		$nameArray=sql_select("select a.booking_no,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.attention,a.pay_mode,a.source,a.remarks, $select_update_date
		from wo_booking_mst a , wo_booking_dtls b
		where a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.booking_type=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
		group by a.booking_no,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.attention,a.pay_mode,a.source,a.remarks");
	   ...
	   if(count($nameArray_item)>0){
        ...
        <table style="border:1px solid black" align="left" class=""  cellpadding="0" width="100%" cellspacing="0" rules="all" >
           <tr>
                <td colspan="10" align="">
                <strong><?
				$po_delivery_date=$po_delivery_date_arr[$nameArray_job_po_row[csf('po_break_down_id')]];
				$style_sting=$style_ref;
				$tot_po_qty_po_level+=$po_qty_po_level[$nameArray_job_po_row[csf('po_break_down_id')]];
				echo "Job NO:".$nameArray_job_po_row[csf('job_no')]." &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp; Style Ref:".$style_ref." &nbsp;&nbsp; &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; PO No:" .$po_number[$nameArray_job_po_row[csf('po_break_down_id')]]." &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;PO Qty:" .$po_qty_po_level[$nameArray_job_po_row[csf('po_break_down_id')]]."&nbsp; &nbsp;Delivery Date:".change_date_format($po_delivery_date_arr[$nameArray_job_po_row[csf('po_break_down_id')]]);?></strong>
                </td>
            </tr>
            <tr>
                <td colspan="10" align="">
        ...
        <table style="border:1px solid black" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            <tr>
                <td colspan="11" align="">
                <strong><? echo "Job NO:".$nameArray_job_po_row[csf('job_no')]." &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; Style Ref:&nbsp; ".$style_ref."  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;PO No:" .$po_number[$nameArray_job_po_row[csf('po_break_down_id')]]." &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;PO Qty:" .$po_qty_po_level[$nameArray_job_po_row[csf('po_break_down_id')]]."&nbsp; &nbsp;Delivery Date:".change_date_format($po_delivery_date_arr[$nameArray_job_po_row[csf('po_break_down_id')]]);?></strong>
                </td>
            </tr>
            <tr>
                <td colspan="11" align="">
	   ...
	   ?>
        ...
        <table  style="border:1px solid black" align="left" class=""  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            <tr>
				...
				if($db_type==2)
				{
				$nameArray_color_size_qnty=sql_select("select sum(b.requirment) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and  b.status_active=1 and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=0 and  a.id= ".$result_item[csf('id')]." and a.trim_group=". $result_item[csf('trim_group')]." and nvl( b.description,0)=nvl('". $result_itemdescription[csf('description')]."',0) and nvl(b.brand_supplier,0)=nvl('".$result_itemdescription[csf('brand_supplier')]."',0) and nvl(b.item_color,0)=nvl('".$result_itemdescription[csf('item_color')]."',0)");
				}
       ...
       <table  width="100%" class=""  style="border:1px solid black" cellpadding="0" cellspacing="0" rules="all" align="left">
            <tr style="border:1px solid black;">
                <td width="14%"  style="border:1px solid black; text-align:right; font-weight:bold; ">Total Order Qty:</td>
				<td width="17%"  style="border:1px solid black; text-align:right; font-weight:bold; "><? echo number_format($tot_po_qty_po_level,2);?></td>
                <td width="22%" style="border:1px solid black;  text-align:right; font-weight:bold; "> Grand Total:</td>
                <td width="9%" style="border:1px solid black;  text-align:right; font-weight:bold; ">
				<?
				if(count($uom_arr_check)<2 && count($group_arr_check)<2){
    ...
    <table cellspacing="0" cellpadding="0" style="border:1px solid black" rules="all" width="100%" class=""  align="left">
           ...
           <?
					...
					$booking_no=str_replace("'","",$txt_booking_no);
						...
						$prev_wo_amt=sql_select("select sum(b.amount) as prev_amount,b.po_break_down_id, a.exchange_rate from wo_booking_mst a,wo_booking_dtls b where a.booking_no=b.booking_no and b.booking_no!=$txt_booking_no and b.po_break_down_id in($po_id)   $prev_book_cond and  b.booking_type=2 and b.is_short=2 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and a.entry_form=43  group by b.po_break_down_id,a.exchange_rate");
						//echo "select sum(b.amount) as prev_amount, a.exchange_rate from wo_booking_mst a,wo_booking_dtls b where a.booking_no=b.booking_no and b.booking_no!=$txt_booking_no and b.po_break_down_id in($po_id)   $prev_book_cond and  b.booking_type=2 and b.is_short=2 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 group by a.exchange_rate";
						//$prev_wo_amount=0;
						foreach($prev_wo_amt as $row)
						{
							if($row[csf("prev_amount")]>0 && ($row[csf("exchange_rate")]!="" || $row[csf("exchange_rate")]!=0) )
							{
							$prev_wo_amount_arr[$row[csf("po_break_down_id")]]+=$row[csf("prev_amount")]/$row[csf("exchange_rate")];
							}
						}
				...
				//echo	$sql="select job_no,po_break_down_id from wo_booking_dtls where booking_no=$txt_booking_no and status_active=1 and is_deleted=0 group by job_no,po_break_down_id";
                       $nameArray=sql_select( $sql );
						...
						list($sql_cu_woq_row_short)=$sql_cu_woq_short;
						$cu_woq_amount_short=$sql_cu_woq_row_short[csf('amount')];*/
						$sql_cu_woq_short=sql_select("select sum(amount) as amount,exchange_rate  from wo_booking_dtls where po_break_down_id='".$selectResult[csf('po_break_down_id')]."'  and  booking_type=2 and booking_no!=$txt_booking_no and is_short=1 and status_active=1 and is_deleted=0 and amount>0 group by exchange_rate");

						$cu_woq_amount_short=0;
						foreach($sql_cu_woq_short as $row)
						{
							if($row[csf("exchange_rate")]!="" || $row[csf("exchange_rate")]!=0 )
							{
						...
						foreach ($poWiseArr as $job=>$po){
							...
							$value['prev_amount']=$prev_wo_amount_arr[$key];
						        ...
						        $value['pre_amount']=$trims_costing_arr[$key];
...
if ($action=="populate_data_from_search_popup")
{
	 $sql= "select booking_no,booking_date,company_id,ready_to_approved,buyer_id,job_no,currency_id,exchange_rate,pay_mode,supplier_id,attention,delivery_date,source,is_approved,is_short,trime_type,remarks,item_from_precost,budget_version from wo_booking_mst  where booking_no='$data'";
	 //echo $sql;die;
	 $data_array=sql_select($sql);
	 foreach ($data_array as $row)
	 {
