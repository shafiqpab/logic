...
if ($action=="load_drop_down_suplier"){
	...
	echo create_drop_down( "cbo_supplier_name", 130, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=9 and   a.status_active =1 and a.is_deleted=0 order by supplier_name","id,supplier_name", 1, "-- Select Supplier --", $selected, "",0 );
...
if($action=="create_po_search_list_view"){
	...
	$sql= "select a.job_no_prefix_num,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity,b.id, b.po_number,b.grouping,b.file_no,b.po_quantity,b.shipment_date,a.job_no from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst".set_user_lavel_filtering(' and a.buyer_name','buyer_id')." and a.status_active=1 and b.status_active=1  and b.shiping_status not in(3)  $shipment_date $company $buyer $job_cond $order_cond $style_cond $file_no_cond $internal_ref_cond order by a.job_no"; 
...
if ($action=="save_update_delete"){
	...
	if ($operation==0){
		...
		$issue_mrr=0;
		$sqlis=sql_select("select issue_number from inv_issue_master where booking_no=$txt_booking_no and status_active=1 and is_deleted=0 and entry_form!=3");
...
if($action=="save_update_delete_dtls"){
	...
	if ($operation==0){
		...
		$field_array="id,job_no,booking_mst_id,po_break_down_id,pre_cost_fabric_cost_dtls_id,color_size_table_id,booking_no,booking_type, is_short,fabric_color_id,fin_fab_qnty,grey_fab_qnty,process_loss_percent,color_type,construction,copmposition,gsm_weight,dia_width,rmg_qty,new_cons,grey_cons_dzn,inserted_by,insert_date";
				...
				for($m=1; $m<=$total_col; $m++){
					...
					if(str_replace("'","",$$gmtssize)!=""){
						...
						if(str_replace("'","",$$colorsizetableid) !=""){
							...
							$data_array .="(".$id.",".$txt_job_no.",".$update_id.",".$cbo_order_id.",".$cbo_fabricdescription_id.",".$$colorsizetableid.",".$txt_booking_no.",1,3,".$$fabcolor.",".$$totfincons.",".$$totcons.",".$$processloss.",".$colortype.",".$construction.",".$composition.",".$gsm_weight.",".$newdia.",".$$gmtsqty.",".$$newcons.",".$$greyconsdzn.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		if($issue_mrr){
		 ...
		 $field_array="id,job_no,booking_mst_id,po_break_down_id,pre_cost_fabric_cost_dtls_id,color_size_table_id,booking_no,booking_type, is_short,fabric_color_id,fin_fab_qnty,grey_fab_qnty,process_loss_percent,color_type,construction,copmposition,gsm_weight,dia_width,rmg_qty,new_cons,grey_cons_dzn,updated_by,update_date";
		 for($i=1; $i<=$total_row; $i++){
				$gmtscolor="gmtscolor_".$i;
				$fabcolor="fabcolor_".$i;
				if(str_replace("'","",$$gmtscolor)!=""){
		...
		//$rID=execute_query( "delete  from  wo_booking_dtls  where booking_no=$txt_booking_no and po_break_down_id=$cbo_order_id and pre_cost_fabric_cost_dtls_id =$cbo_fabricdescription_id and dia_width=$saveddia",0);	
		$rID=execute_query( "update wo_booking_dtls set status_active=0,is_deleted =1 where booking_no =$txt_booking_no and pre_cost_fabric_cost_dtls_id=$cbo_fabricdescription_id and status_active=1 and is_deleted =0",0);	
...
if($action=="show_fabric_booking_report"){
	...
	d.booking_no =$txt_booking_no and d.rmg_qty>0 and
						...
						foreach($gsm_weightValue as $dia_widthId=>$dia_widthValue){
							...
							<tbody>
                                <tr>
                                    ...
                                    <td align="right" title="(Finis Qty <? echo $FinFabQnty; ?>/ Rmg Qty <? echo $RmgQty; ?>)*12">
                                    <?
									$NewCons=0;
									$NewCons=($FinFabQnty/$RmgQty)*12;
