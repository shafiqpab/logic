	...
	<script type="text/javascript">
      ...
      { 	//alert(id);
		...
		function js_set_value( strCon ) 
		{
			...
			toggle( document.getElementById( 'tr_' + strCon ), '#FFFFCC' );
			
			if( jQuery.inArray( strCon, selected_data ) == -1 )
			{
				selected_data.push( strCon );
			...
			else
			{
				...
				{
					if( selected_data[i] == strCon ) break;
				}
				selected_data.splice( i, 1 );
				//selected_name.splice( i, 1 ); 
			}
			var strdata = ''; var name = ''; var job = '';
			for( var i = 0; i < selected_data.length; i++ )
			{
				strdata += selected_data[i] + ',';
...
if($action=="create_item_smv_search_list_view")
{
    ...
    <input type="hidden" id="txt_selected" name="txt_selected" />
	<?
	...
	foreach($sql_result as $row)
	{
		...
		$smv_dtls_arr[$row[csf('style_ref')]][$row[csf('extention_no')]]['style_ref']=$row[csf('style_ref')];
		$smv_dtls_arr[$row[csf('style_ref')]][$row[csf('extention_no')]]['operation_count']=$row[csf('operation_count')];
		$smv_dtls_arr[$row[csf('style_ref')]][$row[csf('extention_no')]]['id'].=$row[csf('id')].',';
		$smv_dtls_arr[$row[csf('style_ref')]][$row[csf('extention_no')]]['system_no'].=$row[csf('system_no')].',';
		//$smv_dtls_arr[$row[csf('id')]]['gmts_item_id']=$row[csf('gmts_item_id')];
		$smv_dtls_arr[$row[csf('style_ref')]][$row[csf('extention_no')]]['lib_sewing_id'].=$row[csf('lib_sewing_id')].',';
		...
		$smv_sewing_arr[$row[csf('style_ref')]][$row[csf('id')]][$row[csf('department_code')]][$row[csf('lib_sewing_id')]]['operator_smv']+=$smv;
	}
	...
	<table width="600" cellspacing="0" cellpadding="0" border="0" rules="all" class="rpt_table " >
        ...
        <?
        $i=1;
		foreach($smv_dtls_arr as $styleref=>$styledata)
		{
			foreach($styledata as $ext_no=>$arrdata)
			{
				...
				foreach($ids as $idstr)
				{
					...
					{
						$finish_smv+=$smv_sewing_arr[$styleref][$idstr][4][$lsid]['operator_smv'];
						$cut_smv+=$smv_sewing_arr[$styleref][$idstr][7][$lsid]['operator_smv'];
						$sewing_smv+=$smv_sewing_arr[$styleref][$idstr][8][$lsid]['operator_smv'];
					}
					$k++;
				}
				...
				$data=$sewing_smv."_".$id_str."_".$arrdata['style_ref'];
				?>
				<tr bgcolor="<?=$bgcolor; ?>" id="tr_<?=$data; ?>" style="cursor:pointer" onClick="js_set_value('<?=$data; ?>');">
					<td width="30"><?=$i;//.'='.$k ?></td>
        ...
        <tfoot>
            <tr>
                <td colspan="8" align="center"><input type="button" class="formbutton" value="Close" onClick="parent.emailwindow.hide()" style="width:70px;"/></th>
            </tr>
        </tfoot>
	</table>
	<?
...
if ($action=="save_update_delete")
{
	...
	{	
		...
		$field_set="id, job_id, job_no, gmts_item_id, set_item_ratio, smv_pcs, smv_set, smv_pcs_precost, smv_set_precost, ws_id";
		...
		$set_breck_downarr=array(); $styleSmv=array(); $set_breck_down="";
		if(str_replace("'","",$hid_ws_data)!="")
		{
			$exwsdata=explode(",",str_replace("'","",$hid_ws_data));
			
			foreach($exwsdata as $wsrow)
			{
				$exbreakdata=explode("_",$wsrow);
				if($exbreakdata[0]=="" || $exbreakdata[0]==0) $exbreakdata[0]=str_replace("'","",$tot_smv_qty);
				$set_breck_downarr[$exbreakdata[2]]=str_replace("'","",$cbo_gmtsItem_id).'_'.'1'.'_'.($exbreakdata[0]*1).'_'.($exbreakdata[0]*1).'_'.'0'.'_'.'2'.'_'.($exbreakdata[1]*1);
				$styleSmv[$exbreakdata[2]]+=($exbreakdata[0]*1);
			}
		}
		else
		{
		...
		foreach($_SESSION['excel'] as $style_ref=>$order_data)
		{
			...
			{
				$p=1; $ctpn=1; 
				$idPo=return_next_id_by_sequence("po_break_down_seq","wo_po_break_down",$con );
						...
						foreach($extra_data as $ex_val=>$sizeqty)
						{
							...
							if(str_replace("'","",$txt_job_no)=="")
							{
								...
								{ 
									...
									$itembreakdown=""; $smvperstyle=0;
									if(str_replace("'","",$hid_ws_data)!="")
									{
										$set_breck_down_array=explode('__',str_replace("'",'',$set_breck_downarr[$style_ref]));
										$itembreakdown=str_replace("'",'',$set_breck_downarr[$style_ref]);
										$smvperstyle=$styleSmv[$style_ref];
									}
									else
									{
										$set_breck_down_array=explode('__',str_replace("'",'',$set_breck_down));
										$itembreakdown=str_replace("'",'',$set_breck_down);
										$smvperstyle=str_replace("'","",$tot_smv_qty);
									}
									
									$data_job_arr[$id]="(".$id.",2,'".$new_job_no_arr[$style_ref][0]."','".$new_job_no_arr[$style_ref][1]."','".$new_job_no_arr[$style_ref][2]."',".$cbo_company_id.",".$cbo_buyer_id.",".$cbo_style_owner_id.",".$cbo_location_id.",'".$style_ref."','".$style_des."',".$cbo_product_department.",".$cbo_currercy_id.",".$cbo_season_year.",".$cbo_season_id.",".$cbo_brand_id.",'".$repeat_no."',".$cbo_prod_catgory.",".$cbo_team_leader.",".$cbo_dealing_merchant.",".$cbo_factory_merchant.",".$cbo_order_uom.",".$cbo_gmtsItem_id.",'".$smvperstyle."',".$cbo_packing.",".$cbo_ship_mode.",'".$itembreakdown."',1,1,0,1,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
									...
									for($c=0; $c < count($set_breck_down_array);$c++)
									{
										...
										if(($set_breck_down_arr[2]*1)==0 || ($set_breck_down_arr[2]*1)==''){
											echo "SMV**"; check_table_status( $_SESSION['menu_id'],0);
											die;
										}
										$idSet= return_next_id_by_sequence("po_details_mas_set_details_seq", "wo_po_details_mas_set_details", $con );
										if ($add_set!=0) $data_set .=",";
										$data_set_arr[$idSet] ="(".$idSet.",'".$job_id."','".$new_job_no_arr[$style_ref][0]."','".$set_breck_down_arr[0]."','".$set_breck_down_arr[1]."','".$set_breck_down_arr[2]."','".$set_breck_down_arr[3]."','".$set_breck_down_arr[2]."','".$set_breck_down_arr[3]."','".$set_breck_down_arr[6]."')";
										$add_set++;
										//$idSet=$idSet+1;
									}
									$st++;
							...
							if($p==1) 
							{
								...
								if($postatus=="" || $postatus==0) $postatus=1;
								...
								if(str_replace("'","",$hid_ws_data)!="")
									$job_style_order_arr[$new_job_no_arr[$style_ref][0]][$idPo]=$set_breck_downarr[$style_ref];
								else
							...
							$idColSiz= return_next_id_by_sequence("po_color_size_breakdown_seq","wo_po_color_size_breakdown",$con );
