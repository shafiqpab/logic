...
where a.id=b.mst_id and a.company_id='$company_id' and a.status_active=1 and b.page_id=25 and b.status_active=1 and b.is_deleted=0 order by b.id desc ");
        ...
        a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.currency_id, a.style_ref_no, b.costing_per, b.exchange_rate, c.id, c.emb_name,c.emb_type,c.body_part_id, c.rate, a.insert_date, d.id, d.po_number, d.file_no, d.grouping, d.po_quantity, e.po_break_down_id, e.item_number_id, a.order_uom,b.entry_from  order by d.id, c.id";
					...
					{
					echo "<div style='color:red'>Data not found </div>";	die;
					}
...
// 			mysql_query("BEGIN");
// 		}
// 		$field_array = "buyer_id*currency_id*item_category*pay_mode*source*booking_date*delivery_date*supplier_id*attention*tenor*delivery_to*ready_to_approved*is_short*updated_by*update_date*remarks";
// 		$data_array = "" . $cbo_buyer_name . "*" . $cbo_currency . "*25*" . $cbo_pay_mode . "*" . $cbo_source . "*" . $txt_booking_date . "*" . $txt_delivery_date . "*" . $hidden_supplier_id . "*" . $txt_attention . "*" . $txt_tenor . "*" . $txt_delivery_to. "*" . $cbo_ready_to_approved  . "*" . $cbo_isshort . "*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "*" . $remarks . "";
...
if ($action == "save_update_delete") {
	...
	if ($operation == 0)  // Insert Here
	{
		...
		if ($db_type == 0) {
			$new_booking_no = explode("*", return_mrr_number(str_replace("'", "", $cbo_company_name), '', 'EB', date("Y", time()), 5, "select booking_no_prefix, booking_no_prefix_num from wo_booking_mst where company_id=$cbo_company_name and booking_type=6 and YEAR(insert_date)=" . date('Y', time()) . " order by id desc ", "booking_no_prefix", "booking_no_prefix_num"));
		} else if ($db_type == 2) {
			$new_booking_no = explode("*", return_mrr_number(str_replace("'", "", $cbo_company_name), '', 'EB', date("Y", time()), 5, "select booking_no_prefix, booking_no_prefix_num from wo_booking_mst where company_id=$cbo_company_name and booking_type=6 and to_char(insert_date,'YYYY')=" . date('Y', time()) . " order by id desc ", "booking_no_prefix", "booking_no_prefix_num"));
		}
...
if ($action == "save_update_delete_dtls") {
	...
	if ($operation == 0) {
		...
		$field_array1 = "id, booking_mst_id, pre_cost_fabric_cost_dtls_id, entry_form_id, po_break_down_id, job_no, gmt_item, booking_no, booking_type, is_short, uom, sensitivity, wo_qnty, exchange_rate, rate, amount, delivery_date, country_id_string, inserted_by, insert_date, status_active, is_deleted";
		...
		$field_array2 = "id, booking_mst_id, wo_booking_dtls_id, booking_no, job_no, po_break_down_id, color_number_id, gmts_sizes, description, item_color, item_size, cons, process_loss_percent, requirment, rate, amount, pcs, color_size_table_id, article_number, status_active, is_deleted";
		...
		for ($i = 1; $i <= $total_row; $i++) {
			...
			$data_array1 = "(" . $id_dtls . "," . $booking_mst_id . "," . $$txtembcostid . ",201," . $$txtpoid . "," . $$txtjob_id . "," . $$txtgmtitemid . "," . $txt_booking_no . ",6,2," . $$txtuom . "," . $$cbocolorsizesensitive . "," . $$txtwoq . "," . $$txtexchrate . "," . $$txtrate . "," . $$txtamount . "," . $$txtddate . "," . $$txtcountry . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "',1,0)";
		...
		$field_array_up2 = "id, booking_mst_id, wo_booking_dtls_id, booking_no, job_no, po_break_down_id, color_number_id, gmts_sizes, description, item_color, item_size, cons, process_loss_percent, requirment, rate, amount, pcs, color_size_table_id, article_number, status_active, is_deleted";
	...
	if ($operation == 0) {
		...
		for ($i = 1; $i <= $total_row; $i++) {
			...
			foreach ($strdata->$job->$embcostid->$gmtitemid->po_id as $poId) {
				...
				$amount = $wqQty * $rate;
				$data_array1 = "(" . $id_dtls . "," . $booking_mst_id . "," . $$txtembcostid . ",201," . $poId . "," . $$txtjob_id . "," . $$txtgmtitemid . "," . $txt_booking_no . ",6,2," . $$txtuom . "," . $$cbocolorsizesensitive . "," . $wqQty . "," . $$txtexchrate . "," . $$txtrate . "," . $amount . "," . $$txtddate . "," . $$txtcountry . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "',1,0)";
				...
				if (str_replace("'", '', $$consbreckdown) != '') {
					...
					for ($c = 0; $c < count($consbreckdown_array); $c++) {
						...
						$data_array2 = "(" . $id1 . "," . $booking_mst_id . "," . $id_dtls . "," . $txt_booking_no . "," . $$txtjob_id . "," . $poId . ",'" . $consbreckdownarr[0] . "','" . $consbreckdownarr[1] . "','" . $consbreckdownarr[2] . "','" . $color_id . "','" . $consbreckdownarr[4] . "','" . $bQty . "','" . $consbreckdownarr[6] . "','" . $bwqQty . "','" . $consbreckdownarr[8] . "','" . $bamount . "','" . $order_qty . "','" . $colorSizeTableId . "','" . $consbreckdownarr[13] . "',1,0)";
						$id1 = $id1 + 1;
						$add_comma++;
						$d++;
				...
				foreach ($strdata->$job->$embcostid->$gmtitemid->po_id as $poId) {
					...
					if (str_replace("'", '', $$consbreckdown) != '') {
						...
						for ($c = 0; $c < count($consbreckdown_array); $c++) {
							...
							$bamount = $bwqQty * $consbreckdownarr[8];
							if ($d != 0) $data_array2 .= ",";
							$data_array2 = "(" . $id1 . "," . $booking_mst_id . "," . $strdata->$job->$embcostid->$gmtitemid->booking_id->$poId . "," . $txt_booking_no . "," . $$txtjob_id . "," . $poId . ",'" . $consbreckdownarr[0] . "','" . $consbreckdownarr[1] . "','" . $consbreckdownarr[2] . "','" . $color_id . "','" . $consbreckdownarr[4] . "','" . $bQty . "','" . $consbreckdownarr[6] . "','" . $bwqQty . "','" . $consbreckdownarr[8] . "','" . $bamount . "','" . $order_qty . "','" . $colorSizeTableId . "','" . $consbreckdownarr[13] . "',1,0)";
							$id1 = $id1 + 1;
							$add_comma++;
							$d++;
							check_table_status($_SESSION['menu_id'], 0);
...
if ($action == "show_trim_booking_report9") {
 ...
 ?>
			...
			?>
				...
				?>
					...
					<table border="1" align="left" class="rpt_table" cellpadding="0" width="100%" cellspacing="0" rules="all">
						...
						foreach ($nameArray_item as $result_item) {
							$i++;
							$nameArray_item_description = sql_select("select a.pre_cost_fabric_cost_dtls_id,a.gmt_item,min(b.id) as bid, b.description,b.item_color,b.color_number_id,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount,c.emb_type,c.body_part_id,a.uom as uom from wo_booking_dtls a,  wo_emb_book_con_dtls b, wo_pre_cost_embe_cost_dtls c where a.id= b.wo_booking_dtls_id and a.booking_no=b.booking_no  and a.pre_cost_fabric_cost_dtls_id=c.id and  a.booking_no='$txt_booking_no' and a.job_no='" . $nameArray_job_po_row[csf('job_no')] . "'  and a.sensitivity=4  and  a.pre_cost_fabric_cost_dtls_id=" . $result_item[csf('pre_cost_fabric_cost_dtls_id')] . " and b.requirment !=0 and  a.status_active =1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.pre_cost_fabric_cost_dtls_id,a.gmt_item, b.description,b.item_color,b.color_number_id,c.emb_type,c.body_part_id,a.uom order by bid ");
	...
	<table id="signatureTblId" width="100%" style="padding-top:70px;">
		<tr>
			<td style="text-align: center; font-size: 18px;" width="330">
