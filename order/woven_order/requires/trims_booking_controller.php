...
if ($action=="load_drop_down_supplier")
{
	...
	if($data[0]==5 || $data[0]==3){
	   ...
	   echo create_drop_down( "cbo_supplier_name", 172, "select c.supplier_name,c.id from lib_supplier_tag_company a, lib_supplier_party_type b, lib_supplier c where c.id=b.supplier_id and a.supplier_id = b.supplier_id  and b.party_type in (4,5) and a.tag_company=".$data[1]." and c.status_active=1 and c.is_deleted=0 group by c.id, c.supplier_name order by c.supplier_name","id,supplier_name", 1, "--Select Supplier--",$selected,"get_php_form_data( this.value, 'load_drop_down_attention', 'requires/trims_booking_controller');","");
	}
	exit();
}
...
{
	$data_array="select field_id,field_name,is_disable from field_level_access where company_id='$data' and user_id=$user_id and page_id=43 and status_active=1 and is_deleted=0";
	$data_array_res = sql_select($data_array);
	if($data_array_res[0][csf('is_disable')]==2)
	{
		// echo "$('#".$data_array_res[0][csf('field_name')]."').attr('disabled','false');\n";
		echo "$('select[name=".$data_array_res[0][csf('field_name')]."]').removeAttr('disabled');\n";
	}
	else
	{
		echo "$('select[name=".$data_array_res[0][csf('field_name')]."]').attr('disabled','disabled');\n";
	}
}
...
function load_drop_down_trim_precost_id($data)
{
	...
	if($data[1]>0) $nominated_sup_cond=" and (a.nominated_supp='$data[1]' or a.nominated_supp = 0)"; else $nominated_sup_cond="";
	...
	$sql_data=sql_select("select a.id, a.trim_group, a.cons_dzn_gmts from wo_pre_cost_trim_cost_dtls a, wo_pre_cost_trim_co_cons_dtls b where a.id=b.wo_pre_cost_trim_cost_dtls_id and a.job_no='$data[0]' $nominated_sup_cond and b.po_break_down_id='$data[2]' and b.cons>0 and a.is_deleted=0 and a.status_active=1 group by a.id, a.trim_group, a.cons_dzn_gmts");
...
<table width="1180" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
    <thead>
        <tr>
...
if($action=="create_po_search_list_view")
{
	...
	$budget_version=str_replace("'","",$data[13]);
	if($data[10]==1)
	{
	...
	if ($data[2]==0)
	{
		if($db_type==0)
		{
	 	$sql= "select a.job_no_prefix_num, a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity, b.po_number,b.file_no,b.grouping,b.po_quantity,b.shipment_date,a.garments_nature,SUBSTRING_INDEX(a.insert_date, '-', 1) as year,b.id,c.approved from wo_po_details_master a, wo_po_break_down b,wo_pre_cost_mst c where a.job_no=b.job_no_mst and a.job_no=c.job_no and c.approved=1 and b.shiping_status not in(3) and a.garments_nature=$data[5] and a.status_active=1 and b.status_active=1 $shipment_date $company $buyer $job_cond $file_cond $ref_cond $order_cond $style_cond $entry_form_cond order  by a.id DESC";
		}
		if($db_type==2)
		{
	...
	else
	{
		...
		if($db_type==0)
		{
			$sql= "select a.job_no_prefix_num,a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.garments_nature,SUBSTRING_INDEX(a.`insert_date`, '-', 1) as year from wo_po_details_master a,wo_pre_cost_mst c where  a.job_no=c.job_no and a.job_no not in( select distinct job_no_mst from wo_po_break_down where status_active=1 and is_deleted=0 ) and a.garments_nature=$data[5] and a.status_active=1   and a.is_deleted=0 $company $buyer $entry_form_cond order by a.id DESC";
		}
		if($db_type==2)
		{
...
function set_precost_data($data)
{
    ...
    $sql_select_job=sql_select("select b.costing_per,b.exchange_rate,a.job_no,a.currency_id from wo_pre_cost_mst b,wo_po_details_master a,wo_po_break_down c where  b.job_id=a.id and  b.job_id=c.job_id and b.status_active=1  and c.status_active=1");
    foreach($sql_select_job as $row)
      {
          $jobArr[$row[csf('job_no')]]['currency_id']=$row[csf('currency_id')];
          $jobArr[$row[csf('job_no')]]['exchange_rate']=$row[csf('exchange_rate')];
          $jobArr[$row[csf('job_no')]]['costing_per']=$row[csf('costing_per')];
      }
      unset( $sql_select_job);

      $sql_trim=sql_select("select id,order_uom,conversion_factor from lib_item_group  where status_active=1");
      foreach($sql_trim as $row)
      {
          $TrimGrpArr[$row[csf('id')]]['order_uom']=$row[csf('order_uom')];
          $TrimGrpArr[$row[csf('id')]]['conversion_factor']=$row[csf('conversion_factor')];
	...
	foreach($sql_data as $row)
	{
        ...
        $cons_uom=$TrimGrpArr[$row[csf('trim_group')]]['order_uom'];
        $conversion_factor=$TrimGrpArr[$row[csf('trim_group')]]['conversion_factor'];
        $currency_id_pre_cost=$jobArr[$row[csf('job_no')]]['currency_id'];
        $exchange_rate= $jobArr[$row[csf('job_no')]]['exchange_rate'];
        $costing_per=$jobArr[$row[csf('job_no')]]['costing_per'];

		//$currency_id_pre_cost=$sql_po_qty_row1[csf('currency_id')];//return_field_value("currency_id", "wo_po_details_master", "job_no='".$row[csf('job_no')]."'");
...
if ($action=="set_precost_data")
{
	...
	if($data[6]==1)
	{
		...
		$sql_data=sql_select("select a.id, a.job_no, a.trim_group, a.cons_dzn_gmts, a.rate, a.amount, a.description, a.brand_sup_ref, b.cons, b.country_id from wo_pre_cost_trim_cost_dtls a , wo_pre_cost_trim_co_cons_dtls b where a.id=b.wo_pre_cost_trim_cost_dtls_id and  a.id='$data[0]' and b.po_break_down_id='$data[1]' and b.cons>0 and a.is_deleted=0  and a.status_active=1");
		$consCountry_id='';
		foreach($sql_data as $row)
		{
			if($consCountry_id=="") $consCountry_id=$row[csf('country_id')]; else $consCountry_id.=','.$row[csf('country_id')];
		}
		$countryId_arr=array_unique(explode(",",$consCountry_id));
		$country_diff=array();
		foreach($sql_data as $row)
		{
			if($row[csf('country_id')]==0) $txt_country_cond1="";
			...
			// $costing_per=return_field_value("costing_per", "wo_pre_cost_mst", "job_no='".$row[csf('job_no')]."'");
			// $exchange_rate=return_field_value("exchange_rate", "wo_pre_cost_mst", "job_no='".$row[csf('job_no')]."'");
			// $currency_id_pre_cost=$sql_po_qty_row1[csf('currency_id')];
            //return_field_value("currency_id", "wo_po_details_master", "job_no='".$row[csf('job_no')]."'");
			$currency_id_this_booking=$data[4];
			if($currency_id_pre_cost==$currency_id_this_booking) //$exchange_rate=1;
			...
			if($costing_perId==1) $costing_per_qty=12;
			else if($costing_perId==2) $costing_per_qty=1;
			else if($costing_perId==3) $costing_per_qty=24;
			else if($costing_perId==4) $costing_per_qty=36;
			else if($costing_perId==5) $costing_per_qty=48;
			...
			if($data[5]==1)
			{
				if($row[csf('country_id')] !='0')
				{
					...
					$country_array=$countryId_arr;//explode(",",$row[csf('country_id')]);
					$country_diff=array_diff($selected_country_array,$country_array);
...
if($action=="booking_qnty_and_amount_trim_group")
{
   ...
   //$data_sql=sql_select("select sum(wo_qnty) as wo_qnty, sum(amount) as amount from wo_booking_dtls where po_break_down_id=$txt_po_id and trim_group=$txt_trim_group_id and pre_cost_fabric_cost_dtls_id=$cbo_trim_precost_id and booking_type=2 and status_active=1 and is_deleted=0");
    $data_sql=sql_select("select sum(b.wo_qnty) as wo_qnty, sum(b.amount) as amount from wo_booking_mst a,wo_booking_dtls b where a.booking_no=b.booking_no and  b.po_break_down_id=$txt_po_id and b.trim_group=$txt_trim_group_id and b.pre_cost_fabric_cost_dtls_id=$cbo_trim_precost_id and b.gmt_item=$gmt_item_id and b.booking_type=2 and b.status_active=1 and b.is_deleted=0 and a.entry_form=43");
...
if ($action=="generate_fabric_booking")
{
	...
	$item_cond="";
	if(!empty($cbo_gmt_item_id))
	{
		$item_cond=" and c.item_number_id=$cbo_gmt_item_id";
	}
	...
	<div align="center" style="width:1050px;" >
        <fieldset>
            ...
            $sql_po_qty=sql_select("select sum(c.order_quantity) as order_quantity,(sum(c.order_quantity)/a.total_set_qnty) as order_quantity_set  from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c where a.id=b.job_id and a.id=c.job_id and b.id=c.po_break_down_id and  b.id='$txt_po_id'  $txt_country_cond $item_cond  and a.status_active=1 and b.status_active=1 and c.status_active=1 and c.is_deleted=0  group by b.id,a.total_set_qnty");//and c.item_number_id='$cbo_gmt_item_id'
            ...
            <table width="1050" cellspacing="0" class="rpt_table" border="0" id="tbl_consmption_cost" rules="all">
                ...
                else if($cbo_colorsizesensitive==4)
                {
					// With Gmts Item
					/*$sql="select b.id, b.po_number,b.po_quantity,min(c.id) as color_size_table_id,c.color_number_id,c.size_number_id,sum(c.order_quantity) as order_quantity,(sum(c.order_quantity)/a.total_set_qnty) as order_quantity_set  from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id and  b.id='$txt_po_id' and c.item_number_id='$cbo_gmt_item_id' $txt_country_cond and a.status_active=1 and b.status_active=1 and c.status_active=1 group by  b.id, b.po_number,b.po_quantity,a.total_set_qnty,c.color_number_id,c.size_number_id,c.item_number_id  order by b.id, c.color_number_id,c.size_number_id";*/
					$sql="select b.id, b.po_number,b.po_quantity,min(c.id) as color_size_table_id,c.color_number_id,c.size_number_id,sum(c.order_quantity) as order_quantity,(sum(c.order_quantity)/a.total_set_qnty) as order_quantity_set  from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c where a.id=b.job_id and a.id=c.job_id and b.id=c.po_break_down_id and  b.id='$txt_po_id'  $txt_country_cond $item_cond and a.status_active=1 and b.status_active=1 and c.is_deleted=0  and c.status_active=1 group by  b.id, b.po_number,b.po_quantity,a.total_set_qnty,c.color_number_id,c.size_number_id  order by b.id, c.color_number_id,c.size_number_id";//and c.item_number_id='$cbo_gmt_item_id'
                }
                else
                {
					// With Gmts Item
...
if ($action=="show_trim_booking")
{
	...
	<div style="width:1200px; overflow-y:scroll; max-height:350px;" id="buyer_list_view" align="center">
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1180" class="rpt_table" id="tbl_list_search" >
            ...
            // }
            $array_po_qty=array();
            $sql_po_qty=sql_select("select b.id,b.file_no,b.grouping, c.country_id, sum(c.order_quantity) as order_quantity,(sum(c.order_quantity)/a.total_set_qnty) as order_quantity_set  from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id and  b.id in(".$po_ids.")  $txt_country_cond  and a.status_active=1 and b.status_active=1 and c.status_active=1 group by b.id,a.total_set_qnty,c.country_id,b.file_no,b.grouping");
            
            foreach($sql_po_qty as $row_po_qty)
            {
            $array_po_qty[$row_po_qty[csf('id')]]=$row_po_qty[csf('order_quantity_set')];
            $array_po_DataArr[$row_po_qty[csf('id')]]['grouping']=$row_po_qty[csf('grouping')];	
            $array_po_DataArr[$row_po_qty[csf('id')]]['file_no']=$row_po_qty[csf('file_no')];	
            }
            unset($sql_po_qty);
            $sql_cons_data=sql_select("select a.id as trim_id,b.cons,b.po_break_down_id as poid from wo_pre_cost_trim_cost_dtls a , wo_pre_cost_trim_co_cons_dtls b where a.id=b.wo_pre_cost_trim_cost_dtls_id   and b.po_break_down_id in(".$po_ids.") and a.is_deleted=0  and a.status_active=1");
            foreach($sql_cons_data as $row)
            {
            $trims_consArr[$row[csf('trim_id')]][$row[csf('poid')]]+=$row[csf('cons')];	
            }
            unset($sql_cons_data);
            ...
            foreach ($list_view_data_arr as $selectResult)
            {
				...
				// $sql_po_qty=sql_select("select sum(c.order_quantity) as order_quantity,(sum(c.order_quantity)/a.total_set_qnty) as order_quantity_set  from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id and  b.id='".$selectResult[csf('po_break_down_id')]."'  $txt_country_cond  and a.status_active=1 and b.status_active=1 and c.status_active=1 and c.is_deleted=0 group by b.id,a.total_set_qnty");

				// list($sql_po_qty_row)=$sql_po_qty;
				// $po_qty=$sql_po_qty_row[csf('order_quantity_set')];

				// $sql_cons_data=sql_select("select b.cons from wo_pre_cost_trim_cost_dtls a, wo_pre_cost_trim_co_cons_dtls b where a.id=b.wo_pre_cost_trim_cost_dtls_id and  a.id='".$selectResult[csf('pre_cost_fabric_cost_dtls_id')]."' and b.po_break_down_id='".$selectResult[csf('po_break_down_id')]."' and a.is_deleted=0  and a.status_active=1 and b.cons <>0");
				// list($sql_cons_data_row)=$sql_cons_data;
				// $cons=$sql_cons_data_row[csf('cons')];
                $po_qty= $array_po_qty[$selectResult[csf('po_break_down_id')]];
                $cons=$trims_consArr[$selectResult[csf('pre_cost_fabric_cost_dtls_id')]][$selectResult[csf('po_break_down_id')]];
                ...
                $file_no=$array_po_DataArr[$selectResult[csf('po_break_down_id')]]['grouping'];	
                $ref_no=$array_po_DataArr[$selectResult[csf('po_break_down_id')]]['file_no'];
...
$sql="select a.supplier_id, b.id, b.job_no, b.po_break_down_id, b.pre_cost_fabric_cost_dtls_id, b.trim_group, b.sensitivity, b.uom, b.delivery_date, b.wo_qnty, b.rate, b.amount, b.pre_req_amt, b.exchange_rate, b.country_id_string, b.gmt_item, a.company_id, a.pay_mode, a.trime_type,a.item_from_precost from wo_booking_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and b.id='$data'";
	 ...
	 foreach ($data_array as $row)
	 {
		...
		if($row[csf("item_from_precost")]==1){
			$drop_down_trim_precost_id=load_drop_down_trim_precost_id($row[csf("job_no")].'_'.$row[csf("supplier_id")].'_'.$row[csf("po_break_down_id")]);
...
if ($action=="save_update_delete")
{
	...
	if(str_replace("'","",$txt_booking_no)!='')
	{
		...
		$book_qty=return_field_value( "sum(b.wo_qnty) as wo_qnty", "wo_booking_mst a,wo_booking_dtls b","a.booking_no=b.booking_no and a.company_id=$cbo_company_name and a.booking_type=2 and a.booking_no=$txt_booking_no","wo_qnty");
	...
	else if ($operation==1)   // Update Here
	{
		...
		$sub_order_no = return_field_value("subcon_job", "subcon_ord_mst", "order_no=$txt_booking_no and status_active = 1", "subcon_job");		
		if($sub_order_no!="")
		{
				echo "20**This Booking No Already Receive. Update Not Allow."."Receive No:".$sub_order_no;
				disconnect($con);
				die;
		}
		...
		$field_array_up="is_short*job_no*item_category*supplier_id*currency_id*exchange_rate*booking_date*delivery_date*pay_mode*source*attention*trime_type*ready_to_approved*remarks*entry_form*item_from_precost*budget_version*updated_by*update_date";
		//cbo_trim_type //
		$data_array_up ="".$cbo_isshort."*".$txt_job_no."*4*".$cbo_supplier_name."*".$cbo_currency."*".$txt_exchange_rate."*".$txt_booking_date."*".$txt_delivery_date."*".$cbo_pay_mode."*".$cbo_source."*".$txt_attention."*".$cbo_trim_type."*".$cbo_ready_to_approved."*".$txt_remarks."*43*".$cbo_item_from_precost."*".$cbo_budget_version."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
...
if ($action=="save_update_delete_dtls")
{
	...
	if(str_replace("'","",$txt_booking_no)!='' && str_replace("'","",$txt_update_dtls_id)!='' )
	{
		$sql=sql_select("select a.trims_del,b.delevery_qty  from trims_delivery_mst a, trims_delivery_dtls b where a.id=b.mst_id and b.booking_dtls_id=$txt_update_dtls_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
		foreach($sql as $row)
		{
			$trims_del_no.=$row[csf('trims_del')].",";
			$trims_del_qty +=$row[csf('delevery_qty')];
		...
		if( str_replace("'","",$txt_quantity) <= $trims_del_qty)
		{
			$trims_del_no=implode(", ",array_unique(explode(",",chop($trims_del_no,','))));
			echo "delQtyExeed**".$trims_del_no;
			 disconnect($con);die;
	...
	if ($operation!=0)
	{
		$sql_rec="select b.booking_id, b.booking_no, b.item_group_id, b.item_description, b.order_id, b.receive_qnty, b.reject_receive_qnty, b.cons_qnty, b.gmts_color_id, b.item_color, b.gmts_size_id, b.item_size from inv_receive_master a,inv_trims_entry_dtls b where a.booking_no=b.booking_no and a.id=b.mst_id and a. booking_no=$txt_booking_no and b.status_active=1 and a.status_active=1";
	...
	if ($operation==0)  // Insert Here
	{
		...
		if($db_type==0)
		{
		 ...
		 $field_array1="id, pre_cost_fabric_cost_dtls_id, po_break_down_id, job_no, booking_no,booking_mst_id, booking_type, is_short, trim_group, uom, sensitivity, wo_qnty, rate, exchange_rate, amount, pre_req_amt, delivery_date, country_id_string, gmt_item, inserted_by, insert_date";
		...
		$data_array1 .="(".$id_dtls.",".$cbo_trim_precost_id.",".$txt_po_id.",".$txt_job_no.",".$txt_booking_no.",".$update_id.",2,".$cbo_isshort.",".$txt_trim_group_id.",".$cbo_uom.",".$cbo_colorsizesensitive.",".$txt_quantity.",".$txt_avg_price.",".$txt_exchange_rate_dtls.",".$txt_amount.",".$txt_req_amt.",".$txt_delevary_date.",".$txt_country.",".$cbo_gmt_item_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		//echo $data_array1;
		//==============================
		...
		//echo "10**$rID==$rID_1==$rID1==$flag"; die;
		if($db_type==0)
		{
			if ($flag == 1) {
				mysql_query("COMMIT");
	...
	else if ($operation==1)   // Update Here
	{
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			...
			if(str_replace("'",'',$$updateid)=="")
			{
				if ($k!=1) $data_array_dtls .=",";
				 $data_array_dtls .="(".$id1.",".$txt_update_dtls_id.",".$txt_booking_no.",".$txt_job_no.",".$txt_po_id.",'".$pocolorid."','".$gmtssizesid."',".$$des.",".$$brndsup.",'".$color_id."','".$itemsizes."',".$$qty.",".$$excess.",".$$woqny.",".$$rate.",".$$amount.",".$$pcs.",".$$colorsizetableid.")";
			 //echo $data_array2;
				 $id1=$id1+1;
			}
		}
		...
		$txt_delivery_date=return_field_value( "max(delivery_date) as ", "wo_booking_dtls","booking_no = $txt_booking_no","delivery_date");
	   	$is_received_id=return_field_value( "id", "subcon_ord_mst","order_no=$txt_booking_no and order_id is not null and entry_form=255 and status_active=1 and is_deleted=0");

		$rID=sql_update("wo_booking_dtls",$field_array_up1,$data_array_up1,"id","".$txt_update_dtls_id."",0);
		if($rID) $flag = 1;else $flag = 0;
		
		$rID_11=execute_query("update  wo_booking_mst set supplier_id=$cbo_supplier_name,delivery_date='$txt_delivery_date' where booking_no=$txt_booking_no");
		if ($flag == 1)
		{
			if($rID_11) $flag = 1;else $flag = 0;
		}
		//rID_1=execute_query("update  wo_booking_mst set supplier_id=$cbo_supplier_name,delivery_date='$txt_delivery_date' where booking_no=$txt_booking_no");
		if($data_array_up2!='')
		{
		...
		if($data_array_dtls!='')
		{
			$field_array_dtls="id, wo_trim_booking_dtls_id, booking_no, job_no, po_break_down_id, color_number_id, gmts_sizes, description, brand_supplier, item_color, item_size, cons, process_loss_percent, requirment, rate, amount, pcs, color_size_table_id";
			$rID1Up=sql_insert("wo_trim_book_con_dtls",$field_array_dtls,$data_array_dtls,0);
			if ($flag == 1)
			{
				if($rID1Up) $flag = 1;else $flag = 0;
			}
		}
	...
	else if ($operation==2)   // Delete Here
	{
		...
		$rID=execute_query("delete from wo_trim_book_con_dtls where wo_trim_booking_dtls_id=$txt_update_dtls_id and booking_no=$txt_booking_no");
		$rID_rec=1;
		if($is_received_id!='')
		{
			$field_array_rec_up="is_apply_last_update*updated_by*update_date";
...
<body>
    <div align="center" style="width:100%;" >
    <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
        <table width="1120" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all" align="center">
             <thead>
                    <th colspan="10">
                      <?
            ...
            <tr class="general">
                <td>
                    <?
                        echo create_drop_down( "cbo_company_mst", 120, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", '', "load_drop_down( 'trims_booking_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
                    ?>
                </td>
...
if ($action=="create_booking_search_list_view")
{
   ...
   if($data[7]==2)
	...
	$sql_Data="select a.id,a.booking_no,a.pay_mode,a.supplier_id from  wo_booking_mst a , wo_booking_dtls b, wo_po_break_down c where a.booking_no=b.booking_no  and b.po_break_down_id=c.id and  a.booking_type=2 and a.entry_form=43   $company  $buyer  $supplier_id $booking_date $booking_cond $job_cond $file_cond $ref_cond group by a.id,a.booking_no,a.pay_mode,a.supplier_id";
	...
	foreach($ResultData as $row)
	{
		...
		{
			$comp_sup_arr[$row[csf("booking_no")]]=$comp[$row[csf("supplier_id")]];
		}
		else
		{
			$comp_sup_arr[$row[csf("booking_no")]]=$suplier[$row[csf("supplier_id")]];
		}
		if($all_book_id=="") $all_book_id=$row[csf("id")]; else  $all_book_id.=",".$row[csf("id")];
	}
	$arr=array (1=>$comp,2=>$comp_sup_arr);
	if($all_book_id!="")
	{
	...
	if($data[9]==0)
	{
		$job_cond=str_replace("a.job_no", "c.job_no_mst", $job_cond);
		$sql="select a.id, a.booking_no_prefix_num, a.booking_no, company_id, c.job_no_mst as job_no, a.supplier_id, a.booking_date, b.delivery_date, c.file_no, c.grouping from wo_booking_mst a, wo_booking_dtls b, wo_po_break_down c where a.booking_no=b.booking_no and b.po_break_down_id=c.id and a.booking_type=2 and a.entry_form=43 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $company $buyer ".set_user_lavel_filtering(' and a.buyer_id','buyer_id')." $supplier_id $booking_date $booking_cond $job_cond $file_cond $ref_cond group by a.id, a.booking_no_prefix_num, c.job_no_mst, a.booking_no, company_id, a.supplier_id, a.booking_date, b.delivery_date, c.file_no, c.grouping order by a.id DESC";
		//echo $sql;
		echo  create_list_view("list_view", "Booking No,Company,Supplier,Booking Date,Delivery Date,Job No,File No,Ref. No", "120,100,100,100,100,100,100","900","280",0, $sql , "js_set_value", "booking_no", "", 1, "0,company_id,booking_no,0,0,0,0,0", $arr , "booking_no_prefix_num,company_id,booking_no,booking_date,delivery_date,job_no,file_no,grouping", '','','0,0,0,3,3,0','','');
	}
	else
	{
		 $sql="select a.id, a.job_no, a.booking_no_prefix_num, a.booking_no, company_id, a.supplier_id, a.booking_date, a.delivery_date from wo_booking_mst a where   a.booking_type=2 and a.entry_form=43 and a.status_active=1 and a.is_deleted=0  $company  $buyer ".set_user_lavel_filtering(' and a.buyer_id','buyer_id')."  $supplier_id $booking_date $booking_cond $bo_cond_for_in group by a.id, a.booking_no_prefix_num, a.booking_no,a.job_no,company_id,a.supplier_id,a.booking_date,a.delivery_date order by a.id DESC";
		 //echo $sql;

		echo  create_list_view("list_view", "Booking No,Company,Supplier,Booking Date,Delivery Date,Job No", "120,100,100,100,100","700","280",0, $sql , "js_set_value", "booking_no", "", 1, "0,company_id,booking_no,0,0,0,0", $arr , "booking_no_prefix_num,company_id,booking_no,booking_date,delivery_date,job_no", '','','0,0,3,3,0,0','','');
	}
	exit();
}
...
?>
		...
		$job_no="";
		$currency_id="";	$style_ref="";
		$nameArray_job=sql_select( "select distinct b.job_no ,c.buyer_name,c.style_ref_no from wo_booking_mst a, wo_booking_dtls b,wo_po_details_master c where a.booking_no=b.booking_no and c.job_no=b.job_no and a.booking_no=$txt_booking_no"); 
...
if($action=="show_trim_booking_report2")
{
	extract($_REQUEST);
	$cbo_company_name=str_replace("'","",$cbo_company_name);
    // echo "select c.master_tble_id,c.image_location from   common_photo_library c,wo_po_details_master a,wo_booking_dtls b where a.id=c.master_tble_id and a.job_no=b.job_no and b.status_active=1 and c.form_name='company_details' and b.booking_no=$txt_booking_no  and c.file_type=1";
    $imge_arr=return_library_array( "select c.master_tble_id,c.image_location from   common_photo_library c where   c.form_name='company_details'  and c.file_type=1",'master_tble_id','image_location');
	$country_arr=return_library_array( "select id,country_name from   lib_country",'id','country_name');
	$supplier_name_arr=return_library_array( "select id,supplier_name from   lib_supplier",'id','supplier_name');
	$supplier_address_arr=return_library_array( "select id,address_1 from   lib_supplier",'id','address_1');
...
?>
		...
		$job_no="";
		$currency_id="";
		$buyer_string="";  $style_ref="";
		  $deling_marchent="";
		$nameArray_job=sql_select( "select distinct b.job_no,c.buyer_name,c.style_ref_no,c.dealing_marchant  from wo_booking_mst a, wo_booking_dtls b,wo_po_details_master c where a.booking_no=b.booking_no and c.job_no=b.job_no and a.booking_no=$txt_booking_no and b.statsu_active=1"); 
        foreach ($nameArray_job as $result_job)
        {
			$job_no.=$result_job[csf('job_no')].", ";
            $buyer_string.=$buyer_name_arr[$result_job[csf('buyer_name')]].",";
            $style_ref.=$result_job[csf('style_ref_no')].", ";
			$deling_marchent.=$team_member_arr[$result_job[csf('dealing_marchant')]].", ";
		}
		// $buyer_string="";
       ...
       <table width="100%" style="border:1px solid black">
        ...
        ?>
          <!--==============================================AS PER GMTS COLOR START=========================================  -->
		<?
		$nameArray_job_po=sql_select( "select a.job_no,a.po_break_down_id, sum(b.po_quantity) as po_quantity from wo_booking_dtls a,wo_po_break_down b  where a.booking_no=$txt_booking_no and a.po_break_down_id=b.id and b.status_active=1 and b.is_deleted=0  group by a.job_no,a.po_break_down_id order by a.job_no,a.po_break_down_id "); 
		foreach($nameArray_job_po as $nameArray_job_po_row)
		{
...
?>
    ...
    <table cellspacing="0" cellpadding="0" style="border:1px solid black" rules="all" width="100%" class=""  align="left">
            <thead>
            	<tr>
                	<th colspan="10" style="font-size:18px; font-weight:bold;">Comments</th>
                </tr>
                <tr style="border:1px solid black">
        ...
        <tfoot>
         <tr align="right">
                    ...
                    <th style="border:1px solid black"><? echo number_format($cu_woq_amount_short_total,2); ?></th>
                    <th style="border:1px solid black"><? echo number_format($cu_woq_amount_sample_total,2); ?></th>
                    <th style="border:1px solid black"><? echo number_format($total_value_total,2); ?></th>
                    <th style="border:1px solid black"><? echo number_format($trims_balance_total,2); ?></th>
...
?>
		...
		$job_style_arr=array();
		$costing_per_arr=array();
		$nameArray_buyer=sql_select( "select  b.job_no,a.buyer_name,a.style_ref_no,a.dealing_marchant,c.id  as po_id,c.po_number,c.shiping_status,min(c.pub_shipment_date) as pub_shipment_date,d.costing_per   from wo_po_details_master a, wo_booking_dtls b, wo_po_break_down c ,wo_pre_cost_mst d where a.job_no=b.job_no and a.job_no=c.job_no_mst and a.job_no=d.job_no and b.po_break_down_id=c.id and b.booking_no=$txt_booking_no and b.booking_type=2 group by b.job_no,a.buyer_name,a.style_ref_no,a.dealing_marchant,c.id ,c.po_number,c.shiping_status,d.costing_per order by c.id desc");
        ...
        foreach ($nameArray_buyer as $result_buy)
		{
			...
			$po_id_arr[$result_buy[csf('po_id')]]=$result_buy[csf('po_id')];
			$po_status_arr[]=$result_buy[csf('shiping_status')];
			$min_shipdate=$result_buy[csf('pub_shipment_date')];
			$costing_per_arr[$result_buy[csf('job_no')]]=$result_buy[csf('costing_per')];
		}
		$job_no=implode(",",$job_no_string_arr);
		$buyer_string=implode(",",$buyer_string_arr);
		...
		//'MM-DD-YYYY'
		if($db_type==0) $select_update_date="max(date( b.update_date)) as update_date,min(date(b.delivery_date)) as delivery_date_min"; else $select_update_date="max(to_char( b.update_date,'DD-MM-YYYY')) as update_date,min(to_char( b.delivery_date,'DD-MM-YYYY')) as delivery_date_min";
		$nameArray=sql_select("select a.booking_no,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.attention,a.delivery_date,a.pay_mode,a.source,a.remarks, $select_update_date
		from wo_booking_mst a , wo_booking_dtls b
		where a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.booking_type=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
		group by a.booking_no,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.attention,a.pay_mode,a.source,a.remarks");
           ...
           <table width="100%" style="border:1px solid black; margin:20px 0px 20px 0px" align="left">
            <tr>
             ...
             <tr>
              ...
              <tr>
                <td style="font-size:16px"><b>Delivery Date </b>   </td>
                <td>:&nbsp;<? echo change_date_format($result[csf('delivery_date_min')],'dd-mm-yyyy','-');?> </td>
                <td style="font-size:16px"><b>Shipment Date</b></td>
                <td>:&nbsp;<? if($min_shipdate!="") echo change_date_format($min_shipdate,'dd-mm-yyyy','-'); ?>&nbsp;&nbsp;&nbsp;</td>
                <td><span style="font-size:16px"><b>Ship.days in Hand</b></span></td>
                <td>:&nbsp;<? echo datediff( "d",date("d-m-Y"),$min_shipdate);?></td>
    ...
    <table cellspacing="0" cellpadding="0" style="border:1px solid black" rules="all" width="100%" class=""  align="left">
           ...
           <?
                    ...
                    foreach ($nameArray as $selectResult){
						...
						$trims_costing_arr=$trims->getAmountArray_by_order();
						$pre_amount=$trims_costing_arr[$selectResult[csf('po_break_down_id')]];
...
if ($action=="populate_data_from_search_popup")
{
	 $sql= "select id,booking_no,booking_date,company_id,ready_to_approved,buyer_id,job_no,currency_id,exchange_rate,pay_mode,supplier_id,attention,delivery_date,source,is_approved,is_short,trime_type,remarks,item_from_precost,budget_version from wo_booking_mst  where booking_no='$data'";
	 //echo $sql;die;
	 $data_array=sql_select($sql);
	 foreach ($data_array as $row)
	 {
		...
		echo "load_drop_down( 'requires/trims_booking_controller', '".$row[csf("pay_mode")].'_'.$row[csf("company_id")]."', 'load_drop_down_supplier', 'supplier_td' );\n";
		...
		echo "document.getElementById('txt_attention').value = '".str_replace(array("&", "*", "(", ")", "=","'","_","\r", "\n",'"','#'), "", $row[csf("attention")])."';\n";
		...
		echo "document.getElementById('txt_remarks').value = '".str_replace(array("&", "*", "(", ")", "=","'","_","\r", "\n",'"','#'), "", $row[csf("remarks")])."';\n";
