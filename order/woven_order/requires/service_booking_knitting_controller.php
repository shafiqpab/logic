...
if ($action=="load_drop_down_fabric_description")
{
	...
	if($data[1] =="")
	{
		$wo_pre_cost_fab_conv_cost_dtls_id=sql_select("select id,fabric_description,cons_process from wo_pre_cost_fab_conv_cost_dtls where job_no='$data[0]' and cons_process in(1,3) ");
	}
	else
	{
		$wo_pre_cost_fab_conv_cost_dtls_id=sql_select("select id,fabric_description,cons_process from wo_pre_cost_fab_conv_cost_dtls where job_no='$data[0]' and status_active=1 and is_deleted=0 and cons_process in(1,3)  ");
	}
...
if($action=="lapdip_approval_list_view_edit")
{
	...
	if($type==0)
	{
        $sql="select a.id,a.pre_cost_fabric_cost_dtls_id,a.artwork_no,a.po_break_down_id,a.color_size_table_id,a.fabric_color_id,a.item_size,a.process,
		   ...
		   a.po_break_down_id=b.po_break_down_id and a.color_size_table_id=b.id and a.job_no='$job_no' and a.booking_type=3 and a.process=$process and
	...
	if($type==1)
	{
        ...
        {
			 $sql1 = "SELECT a.id, a.mst_id as plan_id,a.dtls_id as program_no,a.booking_no,a.body_part_id,a.color_type_id,a.fabric_desc,a.gsm_weight,a.dia,a.program_qnty,a.yarn_desc,a.po_id as po_break_down_id, b.id as fabric_des_id,b.amount,b.charge_unit,min(d.id)as color_size_table_id,(e.color_prog_qty) as color_prog_qty,sum(d.plan_cut_qnty) as plan_cut_qnty,d.color_number_id as color_number_id, e.id as ppl_color_dtls_id FROM ppl_planning_entry_plan_dtls a,wo_pre_cost_fab_conv_cost_dtls b,ppl_planning_info_entry_dtls c,ppl_color_wise_break_down e, wo_po_color_size_breakdown d WHERE a.yarn_desc = b.fabric_description and a.dtls_id = e.program_no and c.id = e.program_no and d.color_number_id=e.color_id  and a.dtls_id = c.id and c.knitting_source = $kinitting_source and b.cons_process=$process and a.dtls_id=$programNo and a.po_id=d.po_break_down_id and a.status_active=1 and b.status_active=1 and c.status_active=1 and d.status_active=1 and e.status_active=1  group by a.id,a.mst_id,a.dtls_id,a.booking_no,a.body_part_id,a.color_type_id,a.fabric_desc,a.gsm_weight,a.dia,e.color_prog_qty,a.program_qnty,a.yarn_desc,a.po_id , b.id,b.amount,b.charge_unit,e.id,d.color_number_id ";
            //echo $sql1; die;
        } 
					...
					if ($sensitivity == 1 && $programNo!='')
                    {
                        ...
                        foreach($dataArray as $row)
                        {
                           ...
                           if($sensitivity==1 || $sensitivity==3) // AS Per Garments/Contrast Color
							{
								$pre_req_qnty=$row[csf("color_prog_qty")];
                                ...
                                $wo_bal_qnty=$row[csf("color_prog_qty")]-$wo_prev_qnty;
							}
                        ...
                        ?>
                            <tr align="center">
                                ...
                                <td title="<? echo 'Prev Qty='.$wo_prev_qnty?>">
                                    <input type="text"  name="txt_woqnty_<? echo $fabric_description_id.'_'.$i; ?>" id="txt_woqnty_<? echo $fabric_description_id.'_'.$i; ?>" style="width:60px;" class="text_boxes_numeric" onChange="copy_value(<? echo $fabric_description_id; ?>,<? echo $i; ?>,'txt_woqnty');calculate_amount(<? echo $fabric_description_id; ?>,<? echo $i; ?>);qunatity_chk(<? echo $fabric_description_id; ?>,<? echo $i; ?>)"  value="<? echo $wo_bal_qnty; ?>" />
									 <input type="hidden" name="txt_reqqty_<? echo $fabric_description_id.'_'.$i; ?>" id="txt_reqqty_<? echo $fabric_description_id.'_'.$i; ?>" style="width:70px;" class="text_boxes_numeric" value="<? echo $pre_req_qnty;?>" />
                                </td>
                                <td>
...
if ($action=="fabric_detls_list_view")
{
	...
	sum(b.wo_qnty) as wo_qnty, b.insert_date,b.program_no from wo_booking_dtls b, wo_booking_mst a
  	where b.booking_no=a.booking_no and a.booking_no='$data[1]' and b.job_no='$data[0]' and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0
	
  	group by b.job_no, a.id, b.dia_width, b.pre_cost_fabric_cost_dtls_id, b.process, b.sensitivity, b.booking_no, b.insert_date, b.program_no"; //and b.process=1
	//echo $sql;
		?>
    <div id="" style="" class="accord_close">
        ...
        <table class="rpt_table" border="1" width="1100" cellpadding="0" cellspacing="0" rules="all" id="">
            ...
            ?>
                <tr bgcolor="<?=$bgcolor; ?>" onClick='update_booking_data("<?=$row[csf("dtls_id")]."_".$row[csf("job_no")]."_".$row[csf("pre_cost_fabric_cost_dtls_id")]."_".$row[csf("process")]."_".$row[csf("sensitivity")]."_".$row[csf("order_id")]."_".$row[csf("booking_no")]."_".$row[csf("dia_width")]."_".$row[csf("program_no")];?>","child_form_input_data","requires/chemical_dyes_receive_controller")' style="cursor:pointer" >
                    <td><?=$i; ?>
                        <input type="hidden" name="po_id_<?=$fabric_description_id.'_'.$i; ?>" id="po_id_<?=$fabric_description_id.'_'.$i; ?>" value="<?=$row[csf("po_break_down_id")]; ?>" style="width:110px;" class="text_boxes" disabled="disabled">
                    </td>
                    <td style="word-break:break-all"><?=$fabric_description_array[$row[csf('pre_cost_fabric_cost_dtls_id')]]; ?></td>
                    <td style="word-break:break-all"><?=$row[csf('job_no')]; ?></td>
                    <td style="word-break:break-all"><?=$row[csf('booking_no')]; ?></td>
                    <td style="word-break:break-all"><?=implode(",",array_unique(explode(",",$allorder))); ?></td>
                    <td style="word-break:break-all"><?=$conversion_cost_head_array[$row[csf('process')]]; ?></td>
                    <td style="word-break:break-all"><?=$size_color_sensitive[$row[csf('sensitivity')]]; ?></td>
                    <td style="word-break:break-all" align="right"><?=number_format($row[csf('wo_qnty')],4); ?></td>
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$new_booking_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'KSB', date("Y",time()), 5,"select id, booking_no_prefix, booking_no_prefix_num from wo_booking_mst where company_id=$cbo_company_name and booking_type=3 and entry_form=182 and to_char(insert_date,'YYYY')=".date('Y',time())." order by id desc ", "booking_no_prefix", "booking_no_prefix_num" ));
		

		$id=return_next_id( "id", "wo_booking_mst", 1 ) ;
		$field_array="id, booking_type, booking_month, booking_year, booking_no_prefix, booking_no_prefix_num, booking_no, entry_form, company_id, buyer_id, job_no, po_break_down_id, item_category, supplier_id, currency_id, exchange_rate, booking_date, delivery_date, pay_mode, source, attention, ready_to_approved, process, tenor, tagged_booking_no, inserted_by, insert_date, status_active, is_deleted";//
		$data_array ="(".$id.",3,".$cbo_booking_month.",".$cbo_booking_year.",'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',182,".$cbo_company_name.",".$cbo_buyer_name.",".$txt_job_no.",".$txt_order_no_id.",12,".$cbo_supplier_name.",".$cbo_currency.",".$txt_exchange_rate.",".$txt_booking_date.",".$txt_delivery_date.",".$cbo_pay_mode.",".$cbo_source.",".$txt_attention.",".$cbo_ready_to_approved.",".$cbo_process.",".$txt_tenor.",".$txt_fab_booking.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
	...
	else if ($operation==1)   // Update Here
	{
		...
		if($is_approved==1){
		 ...
		 $field_array_up="booking_month*booking_year*buyer_id*job_no*po_break_down_id*item_category*supplier_id*currency_id*exchange_rate*booking_date*delivery_date*pay_mode*source*attention*ready_to_approved*tenor*tagged_booking_no*updated_by*update_date";
		 $data_array_up ="".$cbo_booking_month."*".$cbo_booking_year."*".$cbo_buyer_name."*".$txt_job_no."*".$txt_order_no_id."*12*".$cbo_supplier_name."*".$cbo_currency."*".$txt_exchange_rate."*".$txt_booking_date."*".$txt_delivery_date."*".$cbo_pay_mode."*".$cbo_source."*".$txt_attention."*".$cbo_ready_to_approved."*".$txt_tenor."*".$txt_fab_booking."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		 //=======================================================================================================
		...
		// echo "10**jahid##$row_num";die;
		 for ($i=1;$i<=$row_num;$i++)
		 {
			 ...
			 */
			 ...
			 if ($i!=1) $data_array1 .=",";
			 $data_array1 .="(".$id_dtls.",".$booking_mst_id.",".$$fabric_description_id.",182,".$$artworkno.",".$$po_id.",".$$color_size_table_id.",".$txt_job_no.",".$txt_booking_no.",3,".$$item_color_id.",".$$gmts_color_id.",".$$item_size.",".$$gmts_size_id.",".$$fabric_description_id.",".$cbo_dia.",".$$uom.",".$cbo_process.",".$cbo_colorsizesensitive.",".$$txt_woqnty.",".$$txt_rate.",".$$txt_amount.",".$$startdate.",".$$enddate.",".$$lib_composition.",".$$lib_supplier_rateId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$program_no."',1,0)";
		     $id_dtls=$id_dtls+1;
		 }
...
if ($action=="create_booking_search_list_view")
{
    ...
    where a.job_no = b.job_no_mst and $company $buyer $booking_date $jobNoCond $bookingNoCond $source_cond and a.booking_type=3 and a.status_active=1 and a.is_deleted=0 and a.process=1 and b.status_active=1 and b.is_deleted=0   order by a.id DESC";
	$result = sql_select($sql);
	?>
    ...
    <table class="rpt_table" id="list_view" rules="all" width="963" height="" cellspacing="0" cellpadding="0" border="0">
        <tbody>
            ...
            ?>
            <tr onClick="js_set_value('<? echo $row[csf('mst_id')]; ?>')" style="cursor:pointer" id="tr_<? echo $i; ?>" height="20" bgcolor="<? echo $bgcolor; ?>">
...
if($action=="show_trim_booking_report")
{
    ...
    <table border="0" cellpadding="0" cellspacing="0"  width="90%" class="rpt_table"  style="border:1px solid black;" >
	   ...
	   $po_number=return_library_array( "select id,po_number from wo_po_break_down", "id", "po_number"  );
					...
					$pre_cost=sql_select("select job_no,sum(amount) AS knit_cost from wo_pre_cost_fab_conv_cost_dtls where cons_process in(1,3) and status_active=1 and is_deleted=0 group by job_no");
	   ...
	   ?>
                    ...
                    <?
					$pre_cost_knit=number_format($pre_cost_knit,2,'.','');
					$knit_charge=number_format($knit_charge,2,'.','');
...
if($action=="show_trim_booking_report6")
{
    ...
    ?>
                        ...
                        <tr>
                            ...
                            <tr>
                            <td align="center" style="font-size:20px">
                            <strong>Knitting Work Order</strong>
                             </td>
                            </tr>
                      </table>
                </td>
        ...
        $nameArray=sql_select( "select a.booking_no,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.attention,a.delivery_date,a.source, a.pay_mode,a.tagged_booking_no,a.remarks,b.job_no, b.buyer_name, b.style_ref_no  from wo_booking_mst a, wo_po_details_master b where a.job_no=b.job_no and a.booking_no=$txt_booking_no");
       ...
       <table width="90%" style="border:1px solid black">
            ...
            <tr>
             ...
             <tr>
                <td width="100" style="font-size:12px"><b>Supplier Address</b></td>
               	<td width="110">:&nbsp;<? echo $supplier_address_arr[$result[csf('supplier_id')]];?></td>
                   <td width="100" style="font-size:12px"><b>Fabric Booking No</b></td>
				<td width="110">:&nbsp;<? echo $result[csf('tagged_booking_no')];?></td>

            </tr>
            <tr>
                <td width="100" style="font-size:12px"><b>Job No</b>   </td>
                <td width="110">:&nbsp;<? echo $txt_job_no=rtrim($job_no,','); ?>
                <td width="100" style="font-size:12px"><b>Buyer</b></td>
				<td width="110">:&nbsp;<? echo $buyer_name_arr[$result[csf('buyer_name')]]; ?></td>
                </td>
            </tr>
            <tr>
            <td width="100" style="font-size:12px"><b>PO No</b> </td>
                <td  width="100">:&nbsp;<? echo rtrim(str_replace("'","",$po_no),','); ?> </td>
                <td width="100" style="font-size:12px;"><b>Style</b></td>
				<td width="110">:&nbsp;<? echo $result[csf('style_ref_no')]; ?></td>
            </tr>
            <tr>
                <td width="100" style="font-size:12px"><b>Remarks</b></span></td>
				<td width="110" colsapn="3">:&nbsp;<? echo $result[csf('remarks')];?></td>
            </tr>
        </table>
        <br/>
		<?
	...
	//=================================================
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="90%" cellspacing="0" rules="all" >              
            <tr>               
                <td style="border:1px solid black"><strong>Program No & Date</strong> </td>
                <td style="border:1px solid black"><strong>Fabrication</strong> </td>
                <td style="border:1px solid black"><strong>GMT Color</strong> </td>
                <td style="border:1px solid black"><strong>M/C Dia</strong> </td>
                <td style="border:1px solid black"><strong>Gauge</strong> </td>
                <td style="border:1px solid black"><strong>GSM</strong> </td>
                <td style="border:1px solid black"><strong>S/Len</strong> </td>
                <td style="border:1px solid black"><strong>Spandex S/L</strong> </td>
                <td style="border:1px solid black"><strong>Dia Type</strong> </td>
                <td style="border:1px solid black"><strong>Fin/Dia</strong> </td>
                <td style="border:1px solid black" align="center"><strong>Program Qty</strong></td>
                <td style="border:1px solid black" align="center"><strong>Wo Qty (Kg)</strong></td>
                <td style="border:1px solid black" align="center"><strong>Rate</strong></td>
                <td style="border:1px solid black" align="center"><strong>Amount</strong></td>
            </tr>
			...
			$po_number=return_library_array( "select id,po_number from wo_po_break_down", "id", "po_number"  );
			 $total_amount_as_per_gmts_color=0;
             $nameArray_item_description=sql_select( "select a.job_no,a.po_break_down_id,a.fabric_color_id,a.description,a.rate,a.mc_dia,a.mc_gauge,b.gsm_weight,sum(a.wo_qnty) as cons,a.program_no,a.fin_dia,b.job_no,b.width_dia_type,b.construction,b.composition from wo_booking_dtls a, wo_pre_cost_fabric_cost_dtls b where a.job_no=b.job_no and booking_no=$txt_booking_no  and process=".$result_item[csf('process')]." and wo_qnty>0 group by a.job_no,a.po_break_down_id,a.fabric_color_id,a.description,a.rate,a.mc_dia,a.mc_gauge,b.gsm_weight,a.program_no,a.fin_dia,b.job_no,b.width_dia_type,b.construction,b.composition");
            ...
            <tr>
                ...
                <td style="border:1px solid black"><? echo $result_itemdescription[csf('program_no')]; ?> </td>              
                <td style="border:1px solid black"><? echo ($result_itemdescription[csf('construction')].",".$result_itemdescription[csf('composition')]); ?> </td>
                <td style="border:1px solid black"><? echo $color_library[$result_itemdescription[csf('fabric_color_id')]]; ?>  </td>
                <td style="border:1px solid black"><? echo $result_itemdescription[csf('mc_dia')]; ?> </td>
                <td style="border:1px solid black"><? echo $result_itemdescription[csf('mc_gauge')]; ?> </td>
                <td style="border:1px solid black"><? echo $result_itemdescription[csf('gsm_weight')]; ?> </td>
                <td style="border:1px solid black">&nbsp;</td>
                <td style="border:1px solid black">&nbsp;</td>
                <td style="border:1px solid black"><? echo $result_itemdescription[csf('width_dia_type')]; ?> </td>
                <td style="border:1px solid black"><? echo $result_itemdescription[csf('fin_dia')]; ?> </td>
                <td style="border:1px solid black; text-align:right"><? echo number_format($result_itemdescription[csf('cons')],2);$total+= $result_itemdescription[csf('cons')];?></td>
                <td style="border:1px solid black; text-align:right"><? echo number_format($result_itemdescription[csf('cons')],2);$atotal+= $result_itemdescription[csf('cons')]; ?></td>
                <td style="border:1px solid black; text-align:right"><? echo number_format($result_itemdescription[csf('rate')],4); ?> </td>
                <td style="border:1px solid black; text-align:right">
                <?
                $amount_as_per_gmts_color = $result_itemdescription[csf('cons')]*$result_itemdescription[csf('rate')];
      ...
      <?
      $nameArray_ite=sql_select( "select distinct description from wo_booking_dtls  where job_no=$txt_job_no  ");//and sensitivity=1


      foreach($nameArray_ite as $result_ite)
      {
      ?>

       <table border="0" cellpadding="0" cellspacing="0"  width="90%" class="rpt_table"  style="border:1px solid black;" >
                <tr> <td style="border:1px solid black;" colspan="9" align="center"><b> Yarn Details</b> </td></tr>
                <tr style="border:1px solid black;" align="center">
                    <th style="border:1px solid black;" width="200">Count</th>
                    <th style="border:1px solid black;" width="200">Composition</th>
                    <th style="border:1px solid black;" width="80">Type</th>
                    <th style="border:1px solid black;" width="80">Yarn Color</th>
                    <th style="border:1px solid black;" width="80">Lot</th>
                    <th style="border:1px solid black;" width="80">Qty</th>
                </tr>
                <?
                $nameArray_item_desc=sql_select("select a.id,a.job_no,a.count_id,a.copm_one_id,a.color, a.type_id, sum(a.cons_qnty) as cons_qnty,b.job_no from wo_pre_cost_fab_yarn_cost_dtls a,wo_booking_dtls b  where a.job_no=b.job_no and a.job_no=$txt_job_no and  a.status_active=1 and a.is_deleted=0 group by a.id,a.job_no,a.count_id,a.copm_one_id,a.color, a.type_id,b.job_no");

             foreach($nameArray_item_desc as $result_itemdesc)
			 {	  	
			 ?>
                <tr>
                <td style="border:1px solid black;" width="200"><? echo $result_itemdesc[csf('count_id')]; ?></td>
                <td style="border:1px solid black;" width="200"><? echo $result_itemdesc[csf('copm_one_id')]; ?></td>
                <td style="border:1px solid black;" width="200"><? echo $result_itemdesc[csf('type_id')]; ?></td>
                <td style="border:1px solid black;" width="200"><? echo $result_itemdesc[csf('color')]; ?></td>
                <td style="border:1px solid black;" width="200">&nbsp;</td>
                <td style="border:1px solid black;" width="200"><? echo $result_itemdesc[csf('cons_qnty')]; ?></td>
...
if($action=="show_trim_booking_report2" || $action=="show_trim_booking_report3")
{
	...
	$buyer_name_arr=return_library_array( "select id,buyer_name from lib_buyer",'id','buyer_name');
	$job_no="";
	$nameArray_job=sql_select( "select distinct b.job_no,a.is_approved  from wo_booking_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.status_active=1");
	foreach ($nameArray_job as $result_job)
	{
		$job_no.=$result_job[csf('job_no')].",";
	...
	<div style="width:1150px" align="left">
       <table width="90%" cellpadding="0" cellspacing="0" style="border:1px solid black">
		...
		$po_no="";
		$nameArray_job=sql_select( "select distinct b.po_number  from wo_booking_dtls a, wo_po_break_down b where a.po_break_down_id=b.id and a.booking_no=$txt_booking_no and a.status_active=1 and ab.status_active=1");
        foreach ($nameArray_job as $result_job)
        {
			$po_no.=$result_job[csf('po_number')].",";
		}
        $nameArray=sql_select( "select a.booking_no,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.attention,a.delivery_date,a.source, a.pay_mode,a.is_approved  from wo_booking_mst a where  a.booking_no=$txt_booking_no and a.status_active=1");
       ...
       // $nameArray_size=sql_select( "select distinct  item_size  as gmts_sizes from wo_booking_dtls   where booking_no=$txt_booking_no and sensitivity=2");
        ...
        <table border="0" align="left" cellpadding="0" width="90%" cellspacing="0" >
            ...
            <tr>
                ...
                foreach($nameArray_color  as $result_color)
                {
                $nameArray_color_size_qnty=sql_select( "select sum(wo_qnty) as cons from wo_booking_dtls    where   booking_no=$txt_booking_no and sensitivity=3 and process=". $result_item[csf('process')]." and description='". $result_itemdescription[csf('description')]."' and nvl( fabric_color_id,0)=nvl('". $result_color[csf('color_number_id')]."',0)  ");
                foreach($nameArray_color_size_qnty as $result_color_size_qnty)
                {
                ?>
                <td style="border:1px solid black; text-align:right">
       ...
       <table  width="100%" class="rpt_table"   border="1" cellpadding="0" cellspacing="0" rules="all">
	    ...
	    <table width="780" align="center">
                ...
                <div style="text-align:center;font-size:xx-large; font-style:italic; margin-top:20px; color:#FF0000;">
                    <?
                    if($is_approved == 0 || $is_approved == 2){echo "Draft";}else{}
                    ?>
                </div>
                </tr>
        </table>
		<br>
		<?
    echo signature_table(81, $cbo_company_name, "1113px","",1);
	echo "****".custom_file_name($txt_booking_no,$style_sting,$txt_job_no);
    ?>
    </div>
	<script type="text/javascript" src="../../js/jquery.js"></script>
...
{
	  $sql= "select booking_no,booking_date,company_id,buyer_id,is_approved,job_no,tagged_booking_no,po_break_down_id,item_category,fabric_source,currency_id,exchange_rate,pay_mode,booking_month,supplier_id,attention,delivery_date,source,booking_year,ready_to_approved,tenor,id from wo_booking_mst  where id='$data' and status_active=1 and is_deleted=0 ";
	 $data_array=sql_select($sql);
	 foreach ($data_array as $row)
	 {
...
if($action=="show_trim_booking_report7")
{
    ...
    $style_arr=return_library_array( "select job_no, style_ref_no from wo_po_details_master",'job_no','style_ref_no');
	...
	<div style="width:1150px" align="left">
       <table width="90%" cellpadding="0" cellspacing="0" style="border:1px solid black">
		...
		$nameArray_dtls=sql_select("SELECT b.job_no, b.po_break_down_id, b.program_no, c.po_number, b.pre_cost_fabric_cost_dtls_id from wo_booking_mst a join  wo_booking_dtls b on a.id=b.booking_mst_id join wo_po_break_down c on c.id=b.po_break_down_id  where a.booking_no=$txt_booking_no and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0");
        foreach ($nameArray_dtls as $row)
        {
			$job_no_arr[$row[csf('job_no')]]=$row[csf('job_no')];
			$po_no_arr[$row[csf('po_break_down_id')]]=$row[csf('po_number')];
			$program_no_arr[$row[csf('program_no')]]=$row[csf('program_no')];
			$fabric_cost_arr[$row[csf('pre_cost_fabric_cost_dtls_id')]]=$row[csf('pre_cost_fabric_cost_dtls_id')];
		}
        $program_cond=where_con_using_array($program_no_arr,0,"b.program_no");
        $program_cond1=where_con_using_array($program_no_arr,0,"b.id");
        $fabric_cost_cond=where_con_using_array($fabric_cost_arr,0,"b.id");

        $nameArray=sql_select( "select a.booking_no, a.booking_date, a.supplier_id, a.currency_id, a.exchange_rate, a.attention, a.delivery_date, a.source, a.pay_mode, a.remarks, a.buyer_id, a.job_no from wo_booking_mst a where  a.booking_no=$txt_booking_no and status_active=1 and is_deleted=0 and a.entry_form=182");
        ...
        $fabric_booking = sql_select("SELECT a.booking_no from wo_booking_mst a join wo_booking_dtls b on a.id=b.booking_mst_id where a.status_active=1 and a.is_deleted=0 and b.job_no=$txt_job_no and b.status_active=1 and b.is_deleted=0 and b.booking_type=1 and b.is_short=2 group by a.booking_no");
        foreach($fabric_booking as $row){
            $fabric_booking_arr[$row[csf('booking_no')]]=$row[csf('booking_no')];
        }

        $yarn_count_info_sql=sql_select("SELECT a.copmposition_id , a.percent, c.id as fabric_id from lib_yarn_count_determina_dtls a join wo_pre_cost_fabric_cost_dtls c on a.mst_id=c.lib_yarn_count_deter_id join wo_pre_cost_fab_conv_cost_dtls b on c.id=b.fabric_description where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $fabric_cost_cond");
        $yarn_count_info_arr=array();
        foreach($yarn_count_info_sql as $row){
            $yarn_count_info_arr[$row[csf('copmposition_id')]][$row[csf('fabric_id')]]=$row[csf('percent')];
        }
        ...
        ?>
       <table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:90%; margin-top:5px" rules="all">
            <tr>
                ...
                <th width="120" rowspan="8">IMAGE</th>
            </tr>
            <tr>
                <th>Delivery Date</th>
            ...
            <tr>                
                ...
                <td><? echo implode(",",$fabric_booking_arr) ?></td>               
            </tr>
            <tr>
                <th>Job No</th>
                <td><?	echo implode(",", $job_no_arr); ?></td>
                <th>Buyer Name</th>
                <td><? echo $buyer_arr[$result[csf('buyer_id')]]; ?> </td>               	
            </tr>
            <tr>
                <th>PO No</th>
                <td><? echo implode(",", $po_no_arr); ?> </td>
                <th>Style</th>
                <td><?	echo $style_arr[$result[csf('job_no')]]; ?></td>               	
            </tr>
            <tr>
                <th>Remarks</th>
        ...
        $booking_dtls_data=sql_select("SELECT e.id as plan_id, d.id as program_no, d.program_date, c.body_part_id, c.color_type_id, c.fabric_description, e.gsm_weight, b.gmts_color_id, d.machine_dia, d.machine_gg, d.stitch_length, d.spandex_stitch_length, d.width_dia_type, d.fabric_dia, b.wo_qnty, b.rate, b.amount  from wo_booking_mst a join wo_booking_dtls b on a.id=b.booking_mst_id join wo_pre_cost_fabric_cost_dtls c on c.id=b.pre_cost_fabric_cost_dtls_id left join ppl_planning_info_entry_dtls d on to_char(d.id)=b.program_no and d.status_active=1 and d.is_deleted=0  left join ppl_planning_info_entry_mst e on e.id=d.mst_id and e.status_active=1 and e.is_deleted=0  where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.booking_no=$txt_booking_no and b.wo_qnty>0");

        ?>
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="90%" cellspacing="0" rules="all" style="margin-top:5px;">              
            <tr>               
                ...
                <th width="50">GSM</th>
                <th width="50">S/Len</th>
                <th width="50">Spandex S/L</th>
                <th width="50">Dia Type</th>
                <th width="50">Fin/Dia</th>
                <th width="50">Wo Qty (Kg)</th>
                <th width="50">Rate</th>
                <th width="50">Amount</th>
            </tr>
            <?
                foreach($booking_dtls_data as $row){
                    $fabrication=$body_part[$row[csf("body_part_id")]].', '.$color_type[$row[csf("color_type_id")]].', '.$row[csf("fabric_description")];
                     ?>
                    <tr>
                        <td><? echo $row[csf('program_no')].'<br>'.change_date_format($row[csf('program_date')],'dd-mm-yyyy','-') ?></td>
                        <td><?= $fabric_description_array[$row[csf('description')]];?></td>
                        <td align="right"><?= $color_library[$row[csf('gmts_color_id')]] ?></td>
                        <td align="right"><?= $row[csf('machine_dia')] ?></td>
                        <td align="right"><?= $row[csf('machine_gg')] ?></td>
                        <td align="right"><?= $row[csf('gsm_weight')] ?></td>
                        <td align="right"><?= $row[csf('stitch_length')] ?></td>
                        <td align="right"><?= $row[csf('spandex_stitch_length')] ?></td>
                        <td align="right"><?= $fabric_typee[$row[csf('width_dia_type')]] ?></td>
                        <td align="right"><?= $row[csf('fabric_dia')] ?></td>
                        <td align="right"><?= fn_number_format($row[csf('wo_qnty')],2);  ?></td>
                        <td align="right"><?= fn_number_format($row[csf('rate')],2) ?></td>
                        <td align="right"><?= fn_number_format($row[csf('amount')],2) ?></td>
                    </tr>
                <? 
                    //totla_pro_qty+=$program_qty;
                    $totla_wo_qnty+=$row[csf('wo_qnty')];
                    $totla_amount_qnty+=$row[csf('amount')];
                    if($row[csf('program_no')]!=''){
                        $planning_id_arr[$row[csf('program_no')]]=$row[csf('program_no')];
                    }
            ...
            <tr>
                ...
                <th align="right"><?= fn_number_format($totla_wo_qnty,2) ?></th>
                <th align="right"></th>
                <th align="right"><?= fn_number_format($totla_amount_qnty,2) ?></th>
            </tr>
        </table>
        <?
            if(count($planning_id_arr)>0){
                $prog_id_str=implode(",",$planning_id_arr);
                $budget_yarn_dtls=sql_select("SELECT a.supplier_id, a.lot, a.yarn_comp_type1st, a.yarn_comp_percent1st, a.yarn_comp_type2nd, a.yarn_comp_percent2nd, a.yarn_count_id, a.yarn_type, a.color, a.is_within_group,a.brand, b.yarn_qnty from product_details_master a join ppl_yarn_requisition_entry b on a.id=b.prod_id where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.knit_id in ($prog_id_str)");
               
            }
            
        ?>
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="90%" cellspacing="0" rules="all" style="margin-top:5px; margin-bottom:5px">
            <tr>
                <th>Count</th>
                <th>Compossition</th>
                <th>Type</th>
            ...
            <?
            foreach($budget_yarn_dtls as $row){ 
                $composition_str = '';
				if ($row[csf('yarn_comp_percent2nd')] != 0)
				{
					$composition_str = $composition[$row[csf('yarn_comp_type1st')]] . " " . $row[csf('yarn_comp_percent1st')] . "%" . " " . $composition[$row[csf('yarn_comp_type2nd')]] . " " . $row[csf('yarn_comp_percent2nd')] . "%";
				}
				else
				{
					$composition_str = $composition[$row[csf('yarn_comp_type1st')]] . " " . $row[csf('yarn_comp_percent1st')] . "%" . " " . $composition[$row[csf('yarn_comp_type2nd')]];
				}
                ?>
                <tr>
                    <td align="center"><? echo $yarn_count_arr[$row[csf('yarn_count_id')]];?></td>                    
                    <td align="center"><? echo $composition_str ?></td>
                    <td align="center"><? echo $yarn_type[$row[csf('yarn_type')]] ?></td>
                    <td align="center"><? echo $color_library[$row[csf('color')]] ?></td>
                    <td align="center"><? echo $row[csf('lot')] ?></td>
                    <td align="center"><? echo $brand_name_arr[$row[csf('brand')]]; ?></td>
                    <td align="right"><? echo fn_number_format($row[csf('yarn_qnty')],2) ?></td>
                </tr>
            <? } ?>
        </table>
        <?
