...
{ 
	if($data==5 || $data==3){
	   ...
	   echo create_drop_down( "cbo_supplier_name", 172, "select c.supplier_name,c.id from lib_supplier_tag_company a, lib_supplier_party_type b, lib_supplier c where c.id=b.supplier_id and a.supplier_id = b.supplier_id  and b.party_type=25 and c.status_active=1 and c.is_deleted=0 group by c.id, c.supplier_name order by c.supplier_name","id,supplier_name", 1, "--Select Supplier--",$selected,"get_php_form_data( this.value+'_'+document.getElementById('cbo_pay_mode').value, 'load_drop_down_attention', 'requires/service_booking_dyeing_controller');","");
...
//	echo $approval_allow[0][csf("approval_need")].'=TTTTT';
	
	if($approval_allow[0][csf("approval_need")]==1 && $approval_allow[0][csf("allow_partial")]==1)
	{
		$approval_cond="and c.approved in (1,2,3)";
	}
	else{
	...
	if ($data[2]==0)
	{
		 $sql= "select a.job_no_prefix_num,$insert_year, a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity,b.id, b.po_number,b.po_quantity,b.shipment_date,b.grouping,b.file_no from wo_po_details_master a, wo_po_break_down b, wo_pre_cost_mst c where a.job_no=b.job_no_mst and a.job_no=c.job_no and a.status_active=1 and b.status_active=1 and c.approved=1  and b.shiping_status not in(3) $approval_cond $shipment_date $company $buyer $job_cond $order_cond $ref_cond $style_ref_cond $file_no_cond order by a.job_no";  
		// echo $sql;
...
if($action=="lapdip_approval_list_view_edit")
{
	...
	$fabric_description_array=array();
	$wo_pre_cost_fab_conv_cost_dtls_id=sql_select("SELECT id,fabric_description,cons_process from wo_pre_cost_fab_conv_cost_dtls  where job_no='$job_no' and status_active=1 and is_deleted=0");
	foreach( $wo_pre_cost_fab_conv_cost_dtls_id as $row_wo_pre_cost_fab_conv_cost_dtls_id)
	{
		if($row_wo_pre_cost_fab_conv_cost_dtls_id[csf("fabric_description")]!=0)
		{
				...
				<table class="rpt_table" border="1" width="1200" cellpadding="0" cellspacing="0" rules="all" id="table_<? echo $fabric_description_id; ?>">
					...
					foreach($dataArray as $row)
					{
						...
						if($sensitivity==1 || $sensitivity==3)
						{
							...
							//$priv_wo_qty=$po_fab_prev_color_booking_arr[$row[csf('po_break_down_id')]][$fabric_description_id][$row[csf('color_number_id')]]['wo_qnty'];
						}
						else if($sensitivity==4)
						{
							...
							//$priv_wo_qty=$po_fab_prev_color_size_booking_arr[$row[csf('po_break_down_id')]][$fabric_description_id][$row[csf('color_number_id')]][$row[csf("size_number_id")]]['wo_qnty'];
						}
						else if($sensitivity==2 || $sensitivity==0)
						{
							...
							//$priv_wo_qty=$po_fab_prev_size_booking_arr[$row[csf('po_break_down_id')]][$fabric_description_id][$row[csf("size_number_id")]]['wo_qnty'];
						}
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		if($db_type==0)
		{
		    ...
		    $response_booking_no="";
			$new_booking_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'ASB', date("Y",time()), 5,"select id, booking_no_prefix, booking_no_prefix_num from wo_booking_mst where company_id=$cbo_company_name and booking_type=3 and entry_form=361 and to_char(insert_date,'YYYY')=".date('Y',time())." order by id desc ", "booking_no_prefix", "booking_no_prefix_num" ));
			
			$id=return_next_id( "id", "wo_booking_mst", 1 ) ;
			$field_array="id, booking_type, booking_month, booking_year, booking_no_prefix, booking_no_prefix_num, booking_no, entry_form, company_id, buyer_id, job_no, po_break_down_id, item_category, supplier_id, currency_id, exchange_rate, booking_date, delivery_date, pay_mode, source, attention, tenor, process, inserted_by, insert_date, status_active, is_deleted";
			$data_array ="(".$id.",3,".$cbo_booking_month.",".$cbo_booking_year.",'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',361,".$cbo_company_name.",".$cbo_buyer_name.",".$txt_job_no.",".$txt_order_no_id.",12,".$cbo_supplier_name.",".$cbo_currency.",".$txt_exchange_rate.",".$txt_booking_date.",".$txt_delivery_date.",".$cbo_pay_mode.",".$cbo_source.",".$txt_attention.",".$txt_tenor.",".$cbo_process.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
	...
	else if ($operation==1)   // Update Here
	{
		...
		if($recv_number){
		 ...
		 $field_array_up="booking_type*booking_month*booking_year*buyer_id*job_no*po_break_down_id* item_category*supplier_id*currency_id*exchange_rate*booking_date*delivery_date*pay_mode*source*attention*tenor*updated_by*update_date";
		 $data_array_up ="3*".$cbo_booking_month."*".$cbo_booking_year."*".$cbo_buyer_name."*".$txt_job_no."*".$txt_order_no_id."*12*".$cbo_supplier_name."*".$cbo_currency."*".$txt_exchange_rate."*".$txt_booking_date."*".$txt_delivery_date."*".$cbo_pay_mode."*".$cbo_source."*".$txt_attention."*".$txt_tenor."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		 //=======================================================================================================
		 $rID=sql_update("wo_booking_mst",$field_array_up,$data_array_up,"id",$booking_mst_id,0);
		if($db_type==0)
		{
			if($rID){
				mysql_query("COMMIT");  
...
if ($action=="save_update_delete_dtls")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		if($db_type==0)
		{
		 ...
		 for ($i=1;$i<=$row_num;$i++)
		 {
			 ...
			 if ($i!=1) $data_array1 .=",";
			 $data_array1 .="(".$id_dtls.",".$booking_mst_id.",".$$fabric_description_id.",361,".$$artworkno.",".$$po_id.",".$$color_size_table_id.",".$txt_job_no.",".$txt_booking_no.",3,".$color_id.",".$$gmts_color_id.",".$$item_size.",".$$gmts_size_id.",".$$fabric_description_id.",".$$uom.",".$cbo_process.",".$cbo_colorsizesensitive.",".$$txt_woqnty.",".$$txt_rate.",".$$txt_amount.",".$$startdate.",".$$enddate.",".$$findia.",".$$lib_composition.",".$$lib_supplier_rateId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		     $id_dtls=$id_dtls+1;
		 }
	...
	else if ($operation==1)   // Update Here
	{
		...
		$iss_to_finish_arr=array();
		$iss_to_finish=sql_select("select booking_dtls_id, sum(batch_issue_qty) as issue_qty from pro_grey_batch_dtls where booking_no=$txt_booking_no and status_active=1 and  is_deleted=0 group by booking_dtls_id");
		foreach($iss_to_finish as $row)
		{
			$iss_to_finish_arr[$row[csf("booking_dtls_id")]]=$row[csf("issue_qty")];
		}
		unset($iss_to_finish);
		
		 $field_array_up1="pre_cost_fabric_cost_dtls_id*artwork_no*po_break_down_id*color_size_table_id*job_no*booking_no*fabric_color_id
*gmts_color_id*item_size*gmts_size*description*uom*process*sensitivity*wo_qnty*rate*amount*delivery_date*delivery_end_date*dia_width* lib_composition* lib_supplier_rate_id*updated_by*update_date";
		 ...
		 for ($i=1;$i<=$row_num;$i++)
		 {
			 ...
			 if( str_replace("'","",$$txt_woqnty) < $iss_to_finish_arr[str_replace("'",'',$$updateid)] )
			 {
				echo "issueQty**".str_replace("'","",$txt_booking_no);
				check_table_status( $_SESSION['menu_id'],0);
				die;
			 }
			...
			if(str_replace("'",'',$$updateid)!="")
			{
				...
				$data_array_up1[str_replace("'",'',$$updateid)] =explode("*",("".$$fabric_description_id."*".$$artworkno."*".$$po_id."*".$$color_size_table_id."*".$txt_job_no."*".$txt_booking_no."*".$color_id."*".$$gmts_color_id."*".$$item_size."*".$$gmts_size_id."*".$$fabric_description_id."*".$$uom."*".$cbo_process."*".$cbo_colorsizesensitive."*".$$txt_woqnty."*".$$txt_rate."*".$$txt_amount."*".$$startdate."*".$$enddate."*".$$findia."*".$$lib_composition."*".$$lib_supplier_rateId."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
			}
		 }
    ...
    <table width="970" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
        ...
        <tr class="general">
            <td> <input type="hidden" id="selected_booking">
                <? 
                    echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", '', "load_drop_down( 'service_booking_aop_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
                ?>
            </td>
        <td id="buyer_td">
            <? 
        ...
        <td><input name="txt_date_from" id="txt_date_from" class="datepicker" style="width:60px"></td>
         ...
         <td align="center">
            <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_job_prifix').value+'_'+document.getElementById('txt_booking_prifix').value+'_'+document.getElementById('cbo_search_category').value+'_'+document.getElementById('txt_ref_no').value+'_'+document.getElementById('txt_file_no').value, 'create_booking_search_list_view', 'search_div', 'service_booking_aop_controller','setFilterGrid(\'list_view\',-1)')" style="width:60px;" /></td>
    </tr>
    <tr>
...
if ($action=="create_booking_search_list_view")
{
	...
	if($data[6]==1)
	{
		...
		if (str_replace("'","",$data[7])!="") $ref_no_cond=" and d.grouping='$data[7]'  "; else  $ref_no_cond=""; 
		if (str_replace("'","",$data[8])!="") $file_no_cond=" and d.file_no=$data[8]  "; else  $file_no_cond=""; 
	}
	if($data[6]==4 || $data[6]==0)
	{
		...
		if (str_replace("'","",$data[7])!="") $ref_no_cond=" and d.grouping like '%$data[7]%' "; else  $ref_no_cond=""; 
		if (str_replace("'","",$data[8])!="") $file_no_cond=" and d.file_no like '%$data[8]%' "; else  $file_no_cond=""; 
	}
...
//	if($db_type==0) $group_concat="group_concat( distinct d.grouping,',') AS grouping,group_concat( distinct d.file_no,',') AS file_no";
//	else if($db_type==2)  $group_concat="listagg(cast(d.grouping as varchar2(4000)),',') within group (order by d.grouping) AS grouping,listagg(cast(d.file_no as varchar2(4000)),',') within group (order by d.file_no) AS file_no";
					
					
	$arr=array (2=>$comp,3=>$buyer_arr,5=>$po_no,6=>$item_category,7=>$fabric_source,8=>$suplier);
	  $sql= "select a.id, a.booking_no_prefix_num, a.booking_no, a.booking_date, a.company_id, a.buyer_id, c.job_no_prefix_num, a.job_no, a.po_break_down_id, a.item_category, a.fabric_source, a.supplier_id from wo_booking_mst a,wo_po_details_master c,wo_po_break_down d
		where $company $buyer $booking_date and  a.booking_type=3 and  a.status_active=1 and a.is_deleted=0 and a.process=35 and a.job_no=c.job_no  and a.job_no=d.job_no_mst and d.job_no_mst=c.job_no $booking_cond $job_cond $ref_no_cond $file_no_cond group by a.id, a.booking_no_prefix_num, a.booking_no, a.booking_date, a.company_id, a.buyer_id, a.job_no, a.po_break_down_id, a.item_category, a.fabric_source, c.job_no_prefix_num, a.supplier_id order by a.id desc"; 
		
	//echo $sql;
	echo  create_list_view("list_view", "Booking No,Booking Date,Company,Buyer,Job No.,PO number,Fabric Nature,Fabric Source,Supplier", "60,70,80,100,80,180,100,80","950","320",0, $sql , "js_set_value", "booking_no", "", 1, "0,0,company_id,buyer_id,0,po_break_down_id,item_category,fabric_source,supplier_id", $arr , "booking_no_prefix_num,booking_date,company_id,buyer_id,job_no_prefix_num,po_break_down_id,item_category,fabric_source,supplier_id", '','','0,3,0,0,0,0,0,0,0','','');
	exit();
}
...
if($action=="show_trim_booking_report")
{
	...
	<div style="width:1150px" align="left">       
       <table width="90%" cellpadding="0" cellspacing="0" style="border:1px solid black">
        ...
        $nameArray=sql_select( "select a.booking_no, a.booking_date, a.supplier_id, a.currency_id, a.exchange_rate, a.attention, a.delivery_date, a.source, a.buyer_id from wo_booking_mst a where  a.booking_no=$txt_booking_no"); 
        foreach ($nameArray as $result)
        {
       ...
       <table width="90%" style="border:1px solid black">                    	
            ...
            <tr>
             ...
             <tr>
                ...
                <td width="150">:&nbsp;<? echo $supplier_name_arr[$result[csf('supplier_id')]];?></td>
            ...
            <tr>
                <td width="90" style="font-size:12px"><b>Style No</b></td>
                <td width="150">:&nbsp;<? echo chop($style_ref_no,','); ?></td>
                <td width="90" style="font-size:12px"><b>Job No</b></td>
                <td width="150">:&nbsp;<? echo $all_job_arr=rtrim($job_no,','); ?></td> 
               	<td width="90" style="font-size:12px"><b>PO No</b> </td>
                <td style="font-size:12px">:&nbsp;<? echo rtrim($po_no,','); ?></td>
            </tr>
            <tr>
            	<td width="90" style="font-size:12px"><b>Buyer</b></td>
                <td width="150">:&nbsp;<? echo $buyer_name_arr[$result[csf('buyer_id')]]; ?></td>
    ...
    <br>     
        <? echo get_spacial_instruction($txt_booking_no, "",361);?>
    <br>
    <?
    if($show_comment==1)
	{
...
if($action=="show_trim_booking_report1")
{
	...
	<div style="width:1333px" align="center">       
       <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid black">
        ...
        {
			$job_no.=$result_job[csf('job_no')].",";
			array_push($job_arr, "'".$result_job[csf('job_no')]."'");
		}
		$job_arr=array_unique($job_arr);
		$style_ref="";
		$style_arr=return_library_array( "select job_no,style_ref_no from wo_po_details_master where job_no in(".implode(",", $job_arr).") group by job_no,style_ref_no", "job_no", "style_ref_no");
		...
		$po_no=""; $po_id=''; $internal_ref=""; $fileNo="";
		
		$nameArray_job=sql_select( "select b.id, b.po_number, b.grouping, b.file_no from wo_booking_dtls a, wo_po_break_down b where a.po_break_down_id=b.id and a.booking_no=$txt_booking_no group by b.id, b.po_number, b.grouping, b.file_no"); 
		
        foreach ($nameArray_job as $result_job)
        {
			$po_no.=$result_job[csf('po_number')].",";
			$po_id.=$result_job[csf('id')].",";
			$internal_ref.=$result_job[csf('grouping')].",";
			$fileNo.=$result_job[csf('file_no')].",";
		}
       ...
       <table width="100%" style="border:1px solid black">                    	
            ...
            <tr>
                ...
                <td>:&nbsp;<? echo $buyer_name_arr[$buyer_name];?></td>
            </tr> 
            <tr>
               	<td style="font-size:12px"><b>PO No</b> </td>
                <td style="font-size:12px" >:&nbsp;<? echo rtrim($po_no,','); ?> </td>

                <td style="font-size:12px"><b>Style Number</b> </td>
                <td style="font-size:12px" >:&nbsp;<? echo $style_ref; ?> </td>
            </tr> 
            <tr>
                <td style="font-size:12px"><b>File No</b></td>
                <td>:&nbsp;<?   $fvalue=explode(",",$fileNo); echo $fvalue[0];  //    echo rtrim($fileNo,',');  ?></td>
                <td style="font-size:12px"><b>Ref. No</b></td>
                <td>:&nbsp;<?  $refValue=explode(",",$internal_ref); echo $refValue[0];  //  echo rtrim($internal_ref,','); ?></td>
            </tr>
        </table> 
        <br/> 
		<?
       ...
       if(count($nameArray_color)>0)
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            <tr>
                ...
                <td style="border:1px solid black; text-align:right"><? echo number_format($result_itemdescription[csf('cons')],4);$woqty_sum +=$result_itemdescription[csf('cons')]; ?></td>
                ...
                ?>
            <tr>
                <td style="border:1px solid black;  text-align:right" colspan="4"><strong> Wo Qty Total</strong></td>
	    	    <td style="border:1px solid black;  text-align:right"><strong><?php echo number_format($woqty_sum,4); ?></strong></td>
...
if($action=="show_sb_booking_report2")
{
	extract($_REQUEST);
	$cbo_company_name=str_replace("'","",$cbo_company_name);
	$imge_arr=return_library_array( "select master_tble_id,image_location from common_photo_library where form_name='company_details' and file_type=1",'master_tble_id','image_location');
...
{
	 $sql= "select booking_no,booking_date,company_id,buyer_id,job_no,po_break_down_id,item_category,fabric_source,currency_id,exchange_rate,pay_mode,booking_month,supplier_id,attention,delivery_date,source,booking_year,tenor,id from wo_booking_mst  where booking_no='$data'"; 
