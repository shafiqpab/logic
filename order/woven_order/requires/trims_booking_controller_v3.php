            	...
            	<table  cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
        			...
        			<tr>
                        ...
                        <input type="hidden" id="garments_nature" value="<? echo $garments_nature; ?>">
							<? 
								echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", $cbo_company_name,"load_drop_down( 'trims_booking_controller_v3', this.value+'_'+document.getElementById('buyer_id').value, 'load_drop_down_buyer_pop', 'buyer_td' );",1,"" );
							?>
                    </td>
                   	<td id="buyer_td">
                     <? 
...
$sql="select b.id, b.job_no, b.po_break_down_id, b.pre_cost_fabric_cost_dtls_id, b.trim_group, b.sensitivity, b.uom, b.delivery_date, b.wo_qnty, b.rate, b.amount, b.pre_req_amt, b.exchange_rate, b.country_id_string, b.gmt_item, a.company_id, a.pay_mode, a.trime_type,a.item_from_precost from wo_booking_mst a, wo_booking_dtls b  where a.booking_no=b.booking_no and b.id='$data'";     
	 ...
	 foreach ($data_array as $row)
	 {
		//echo "load_drop_down( 'requires/trims_booking_controller_v3', '".$row[csf("job_no")]."', 'load_drop_down_trim_precost_id', 'trim_group_td' );\n";
		
		//echo "load_drop_down( 'requires/trims_booking_controller_v3', '".$row[csf("job_no")]."', 'load_drop_down_gmt_item', 'gmt_item_td' );\n";
...
if ($action=="save_update_delete")
{
	...
	/*	}else{*/
		...
		//}
		 ...
		 $field_array_up="is_short*job_no*item_category*supplier_id*currency_id*exchange_rate*booking_date*delivery_date*pay_mode*source*attention*trime_type*ready_to_approved*remarks*item_from_precost*updated_by*update_date"; 
		 //cbo_trim_type // 
		 $data_array_up ="".$cbo_isshort."*".$txt_job_no."*4*".$cbo_supplier_name."*".$cbo_currency."*".$txt_exchange_rate."*".$txt_booking_date."*".$txt_delivery_date."*".$cbo_pay_mode."*".$cbo_source."*".$txt_attention."*".$cbo_trim_type."*".$cbo_ready_to_approved."*".$txt_remarks."*".$cbo_item_from_precost."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		//==============================
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			 ...
			 if(str_replace("'","",$$itemcolor)!="")
			 {
				 ...
				 {
					  $color_id = return_id( str_replace("'","",$$itemcolor), $color_library, "lib_color", "id,color_name","215");  
        	...
        	<td align="center" width="100%">
            	<table  cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
        			...
        			<tr>
                    	<td> 
							<? 
								echo create_drop_down( "cbo_company_mst", 120, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", '', "load_drop_down( 'trims_booking_controller_v3', this.value, 'load_drop_down_buyer', 'buyer_td' );");
							?>
                        </td>
                         <td id="buyer_td">
                              <? 
...
if ($action=="create_booking_search_list_view")
{
   ...
   if($data[7]==2)
	...
	$sql_Data="select a.id,a.booking_no from  wo_booking_mst a , wo_booking_dtls b, wo_po_break_down c where a.booking_no=b.booking_no  and b.po_break_down_id=c.id and  a.booking_type=2 and a.entry_form=215   $company  $buyer  $supplier_id $booking_date $booking_cond $job_cond $file_cond $ref_cond group by a.id,a.booking_no_prefix_num, a.booking_no,company_id,a.supplier_id,a.booking_date,a.delivery_date";
	...
	if($data[9]==0)
	{
		$sql="select min(a.id) as id,a.booking_no_prefix_num, a.booking_no,company_id,a.job_no,a.supplier_id,a.booking_date,a.delivery_date,c.file_no,c.grouping from wo_booking_mst a, wo_booking_dtls b, wo_po_break_down c where a.booking_no=b.booking_no  and b.po_break_down_id=c.id and  a.booking_type=2 and a.entry_form=215 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  $company  $buyer ".set_user_lavel_filtering(' and a.buyer_id','buyer_id')."  $supplier_id $booking_date $booking_cond $job_cond $file_cond $ref_cond group by a.booking_no_prefix_num,a.job_no, a.booking_no,company_id,a.supplier_id,a.booking_date,a.delivery_date,c.file_no,c.grouping order by id";
	...
	else
	{
		 ...
		 $sql="select min(a.id) as id,a.job_no,a.booking_no_prefix_num, a.booking_no,company_id,a.supplier_id,a.booking_date,a.delivery_date from wo_booking_mst a  where   a.booking_type=2 and a.entry_form=215 and a.status_active=1 and a.is_deleted=0  $company  $buyer ".set_user_lavel_filtering(' and a.buyer_id','buyer_id')."  $supplier_id $booking_date $booking_cond $bo_cond_for_in group by a.booking_no_prefix_num, a.booking_no,a.job_no,company_id,a.supplier_id,a.booking_date,a.delivery_date order by id";
...
?>
	...
	function fnc_fabric_booking_terms_condition( operation )
	{
		...
		http.open("POST","trims_booking_controller_v3.php",true);
...
if ($action=="populate_data_from_search_popup")
{
	 ...
	 foreach ($data_array as $row)
	 {
		...
		//echo "document.getElementById('buyer_td').innerHTML = '".$buyer_dropdown."';\n";
		//echo "get_php_form_data(".$row[csf("company_id")].", 'populate_variable_setting_data', 'requires/trims_booking_controller_v3' );\n";
		...
		echo "load_drop_down( 'requires/trims_booking_controller_v3', '".$row[csf("pay_mode")]."', 'load_drop_down_supplier', 'supplier_td' );\n";
