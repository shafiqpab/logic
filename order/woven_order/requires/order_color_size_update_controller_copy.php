            ...
            <tbody id="color_size_data">
            	...
            	foreach ($color_size_data as $row) {
            	 ...
            	 ?>
            		<tr>
            			<td><?= $i;?></td>
            			<td><?= $buyerArr[$row[csf('buyer_name')]];?></td>
            			<td><?= $row[csf('job_no_prefix_num')]; ?></td>
            			<td><?= date("Y", strtotime($row[csf('insert_date')]));?></td>
            			<td><?= $row[csf('style_ref_no')];?></td>
            			<td><?= $row[csf('po_number')];?>
						...
						<td><input type="text" class="datepicker" id="txt_org_shipment_date_<?= $i;?>" value="<?= change_date_format($row[csf('shipment_date')], "yyyy-mm-dd", "-");;?>" style="width:60px;" readonly ></td>
            			<td><input type="text" class="datepicker" id="txt_rfi_date_<?= $i;?>" value="<?= change_date_format($row[csf('rfi_date')], "yyyy-mm-dd", "-");?>" style="width:60px;" onChange="copy_value(this.value,'txt_rfi_date_',<?= $i ?>)"></td>
						<td><input type="text" class="text_boxes_numeric" id="cartoon_qty_<?= $i;?>" value="<?= $row[csf('cartoon_qty')];?>" style="width:60px;" onChange="copy_value(this.value,'cartoon_qty_',<?= $i ?>)"></td>
						<td><input type="text" class="text_boxes_numeric" id="approx_cbm_<?= $i;?>" value="<?= $row[csf('approx_cbm')];?>" style="width:60px;" onChange="copy_value(this.value,'approx_cbm_',<?= $i ?>)"></td>
						
						<td><input type="text" class="text_boxes_numeric" id="gross_weight_<?= $i;?>" value="<?= $row[csf('gross_weight')];?>" style="width:60px;" onChange="copy_value(this.value,'gross_weight_',<?= $i ?>)"></td>

						<td><input type="text" class="text_boxes_numeric" id="net_weight_<?= $i;?>" value="<?= $row[csf('net_weight')];?>" style="width:60px;" onChange="copy_value(this.value,'net_weight_',<?= $i ?>)"></td>

						<td><? echo create_drop_down( "approx_ship_mode_".$i,80,$shipment_mode,"", 1, "-- Select --",$row[csf('approx_ship_mode')],""); ?>
            		</tr>
            	<? 
