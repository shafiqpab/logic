...
{
	if($data==5 || $data==3){
	   echo create_drop_down( "cbo_supplier_name", 172, "select id,company_name from lib_company where status_active=1 and is_deleted=0 order by company_name", "id,company_name",1, "-- Select Company --", "", "get_php_form_data( this.value, 'load_drop_down_attention', 'requires/print_booking_urmi_controller');",0,"" );
	}
	else
	{
...
{
	$emb_name="";
	$data_array=sql_select("select a.job_no, b.emb_name from wo_po_details_master a, wo_pre_cost_embe_cost_dtls b where a.job_no ='".$data."' and b.cons_dzn_gmts !=0 and b.emb_name in(1,2,3,4,5) and a.job_no=b.job_no");
	   ...
	   $sql_embl="select a.country_id, a.color_number_id from wo_pre_cos_emb_co_avg_con_dtls a, wo_pre_cost_embe_cost_dtls b where b.id=a.pre_cost_emb_cost_dtls_id and b.job_no='$txt_job_no' and b.status_active=1 and b.is_deleted=0 and b.emb_name=$cbo_booking_natu and a.item_number_id=$cbo_gmt_item group by a.country_id, a.color_number_id";
		...
		foreach($result_embl as $row )
		{
			...
			if($row[csf('country_id')]!=0)
			{
				...
				{
					if($country_ids=='') $country_ids=$cid; else $country_ids.=",".$cid;
				}
			}*/
		...
		from wo_po_details_master a, wo_pre_cost_mst b, wo_pre_cost_embe_cost_dtls c, wo_po_break_down d, wo_po_color_size_breakdown e
		where
		a.job_no=b.job_no and a.job_no=c.job_no and a.job_no=d.job_no_mst and a.job_no=e.job_no_mst and d.id=e.po_break_down_id $cbo_company_name_cond $txt_job_no_cond $cbo_buyer_name_cond $country_cond and c.emb_name=$cbo_booking_natu and e.item_number_id=$cbo_gmt_item
		and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1 and e.is_deleted=0 and e.status_active=1
		group by
			...
			foreach ($nameArray as $selectResult)
			{
				...
				if($req_qnty>0 && $is_short==2)
				{
					...
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="change_color('search<? echo $i; ?>','<? echo $bgcolor; ?>')">
                        ...
                        <td width="90"><input type="hidden" id="txtembcostid_<? echo $i;?>" value="<? echo $selectResult[csf('embe_cost_dtls_id')];?>" /><? echo $emblishment_name_array[$selectResult[csf('emb_name')]]; ?></td>
                        ...
                        <td width="60"><input type="text" style="width:100%; font-family:Verdana, Geneva, sans-serif; text-align:right; font-size:11px; background-color:<? echo $bgcolor; ?>" id="txtplancut_<? echo $i;?>" value="<? if($calculation_basis==1){echo $selectResult[csf('order_quantity')];}else{ echo $selectResult[csf('plan_cut_qnty')];}?>" readonly/></td>
				...
				foreach($color_idarray as $colorid)
				{
					...
					if($req_qnty>0 && $is_short==2)
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="change_color('search<? echo $i; ?>','<? echo $bgcolor; ?>')">
                            ...
                            <td width="90"><input type="hidden" id="txtembcostid_<? echo $i;?>" value="<? echo $embe_cost_dtls_id;?>" /><? echo $emblishment_name_array[$emb_name];?></td>
                            ...
                            <td width="60"><input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; text-align:right; font-size:11px; background-color:<? echo $bgcolor; ?>" id="txtplancut_<? echo $i;?>" value="<? if($calculation_basis==1){echo $order_quantity;}else{ echo $plan_cut_qnty;}?>" readonly/></td>
                            <td width="200"><input type="text" style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; background-color:#FFC" id="description_<? echo $i;?>" value="<? echo $description;?>" onChange="copy_value(<? echo $i; ?>,'description_')" /></td>
					...
					else if ($booking_dtls_id!="")
					{
						?>
						<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="change_color('search<? echo $i; ?>','<? echo $bgcolor; ?>')">
                            <td width="40"><? echo $i;?><input type="hidden" id="txtbookingdtlasid_<? echo $i;?>" value=" <? echo $booking_dtls_id ;?>" style="width:40px"/>X</td>
                            <td width="90"><a href="#" onClick="setdata('<? echo $po_number;?>' )">View</a><input type="hidden" id="txtpoid_<? echo $i;?>" value="<? echo $po_id;?>" /></td>
                            <td width="100"><p><? echo $country_name;?></p><input type="hidden" id="txtcountryid_<? echo $i;?>" value="<? echo $country_id;?>" /></td>
                            <td width="100"><? echo $garments_item[$item_number_id];?><input type="hidden" id="txtitemnumberid_<? echo $i;?>" value="<? echo $item_number_id;?>" readonly /></td>
                            <td width="90" ><input type="hidden" id="txtembcostid_<? echo $i;?>" value="<? echo $embe_cost_dtls_id;?>" /> <? echo $emblishment_name_array[$emb_name];?></td>
                            <td width="100"><? echo $emb_type; ?><input type="hidden" id="txtembtype_<? echo $i;?>" value="<? echo $emb_typeid;?>" /></td>
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$new_booking_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'EB', date("Y",time()), 5, "select booking_no_prefix, booking_no_prefix_num from wo_booking_mst where company_id=$cbo_company_name and booking_type=6 $yearCond=".date('Y',time())." order by id desc ", "booking_no_prefix", "booking_no_prefix_num" ));

		$id=return_next_id( "id", "wo_booking_mst", 1 ) ;
		$field_array="id,booking_type,is_short,booking_no_prefix,booking_no_prefix_num,booking_no,company_id,buyer_id,job_no,currency_id,item_category,exchange_rate,pay_mode,source,booking_date,delivery_date,supplier_id,attention,tenor,ready_to_approved,inserted_by,insert_date,calculation_basis,cbo_level";
		 $data_array ="(".$id.",6,".$cbo_is_short.",'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',".$cbo_company_name.",".$cbo_buyer_name.",".$txt_job_no.",".$cbo_currency.",25,".$txt_exchange_rate.",".$cbo_pay_mode.",".$cbo_source.",".$txt_booking_date.",".$txt_delivery_date.",".$hidden_supplier_id.",".$txt_attention.",".$txt_tenor.",".$cbo_ready_to_approved.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$calculation_basis.",".$cbo_level.")";
...
if($action=="save_update_delete_dtls")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		if($row_msg>0)
		{
		 ...
		 $field_array1="id,pre_cost_fabric_cost_dtls_id,po_break_down_id,job_no,booking_no,booking_type,is_short,gmt_item,emblishment_name,gmts_color_id,wo_qnty,rate,amount,delivery_date,description,country_id_string,inserted_by,insert_date";
		 for ($i=1;$i<=$total_row;$i++)
		 {
			 ...
			 if ($i!=1) $data_array1 .=",";
			 $data_array1 .="(".$id_dtls.",".$$txtembcostid.",".$$txtpoid.",".$txt_job_no.",".$txt_booking_no.",6,".$cbo_is_short.",".$cbo_gmt_item.",".$cbo_booking_natu.",".$$txtcolorid.",".$$txtwoq.",".$$txtrate.",".$$txtamount.",".$$txtddate.",".$$description.",".$$txtcountryid.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			 $id_dtls=$id_dtls+1;
		 }
	...
	else if ($operation==1)  // Insert Here
	{
		...
		$field_array_up1="pre_cost_fabric_cost_dtls_id*po_break_down_id*job_no*booking_no*booking_type*gmt_item*emblishment_name*gmts_color_id*wo_qnty*rate*amount*delivery_date*description*country_id_string*updated_by*update_date*status_active*is_deleted";
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			...
			if(str_replace("'",'',$$txtbookingdtlasid)!="")
			{
				...
				$data_array_up1[str_replace("'",'',$$txtbookingdtlasid)] =explode("*",("".$$txtembcostid."*".$$txtpoid."*".$txt_job_no."*".$txt_booking_no."*6*".$cbo_gmt_item."*".$cbo_booking_natu."*".$$txtcolorid."*".$$txtwoq."*".$$txtrate."*".$$txtamount."*".$$txtddate."*".$$description."*".$$txtcountryid."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*1*0"));
			}
			if(str_replace("'",'',$$txtbookingdtlasid)=="")
			{
		...
		if($row_msg>0)
		{
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			 ...
			 foreach($jason_data->$embcostid->$colorid->po_id as $poId){
				 ...
				 $data_array1 .="(".$id_dtls.",".$$txtembcostid.",".$poId.",".$txt_job_no.",".$txt_booking_no.",6,".$cbo_is_short.",".$cbo_gmt_item.",".$cbo_booking_natu.",".$$txtcolorid.",".$wqQty.",".$rate.",".$amount.",".$$txtddate.",".$$description.",".$$txtcountryid.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			...
			if(str_replace("'",'',$$txtbookingdtlasid)!="")
			{
				foreach($jason_data->$embcostid->$colorid->po_id as $poId){
					...
					$data_array_up1[str_replace("'",'',$jason_data->$embcostid->$colorid->booking_dtls_id->$poId)] =explode("*",("".$$txtembcostid."*".$poId."*".$txt_job_no."*".$txt_booking_no."*6*".$cbo_gmt_item."*".$cbo_booking_natu."*".$$txtcolorid."*".$wqQty."*".$rate."*".$amount."*".$$txtddate."*".$$description."*".$$txtcountryid."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*1*0"));
					}
			}
...
if ($action=="create_booking_search_list_view")
{
   ...
   if($data[6]==2)
	...
	if($data[7]==0)
		  $sql= "select a.id, a.booking_no_prefix_num, a.booking_no,a.booking_date,company_id,a.buyer_id,a.job_no,b.po_break_down_id,b.gmt_item,c.emb_name,a.supplier_id,a.is_approved,a.ready_to_approved,d.grouping from wo_booking_mst a, wo_booking_dtls b,  wo_pre_cost_embe_cost_dtls c,wo_po_break_down d where $company $buyer $booking_date $booking_cond and d.job_no_mst=b.job_no and d.id=b.po_break_down_id and  a.job_no=b.job_no and  a.job_no=c.job_no and a.booking_no=b.booking_no and b.pre_cost_fabric_cost_dtls_id=c.id  and a.booking_type=6 and  a.status_active=1  and 	a.is_deleted=0 and  b.status_active=1  and 	b.is_deleted=0 $job_cond $ref_cond group by a.id, a.booking_no_prefix_num, a.booking_no,a.booking_date,company_id,a.buyer_id,a.job_no,b.po_break_down_id,b.gmt_item,c.emb_name,a.supplier_id,a.is_approved,a.ready_to_approved,d.grouping order by a.id DESC";
	else
		$sql= "select a.id, a.booking_no_prefix_num, a.booking_no,a.booking_date,company_id,a.buyer_id,a.job_no,c.emb_name,a.supplier_id,a.is_approved,a.ready_to_approved,1 as po_break_down_id,2 as gmt_item,'' as grouping  from wo_booking_mst a,  wo_pre_cost_embe_cost_dtls c where $company $buyer $booking_date $booking_cond  and  a.job_no=c.job_no  and a.booking_type=6  and  a.status_active=1  and 	a.is_deleted=0 $job_cond group by a.id, a.booking_no_prefix_num, a.booking_no,a.booking_date,company_id,a.buyer_id,a.job_no, c.emb_name,a.supplier_id,a.is_approved,a.ready_to_approved order by a.id DESC";
...
if ($action=="populate_data_from_search_popup")
{
	 ...
	 $sql= "select booking_no,booking_date,company_id,buyer_id,job_no,currency_id,exchange_rate,pay_mode,booking_month,supplier_id,attention,delivery_date,source,booking_year,is_approved,ready_to_approved,calculation_basis,cbo_level,is_short from wo_booking_mst  where booking_no='$data' and  status_active=1 and is_deleted=0";
...
<div style="width:1100px;" align="center">
 ...
 <table width="1100" cellspacing="0" align="center" border="1" rules="all" class="rpt_table" >
      <thead bgcolor="#dddddd" align="center">
         ...
         <th width="80">Color</th>
         <th width="80"><?  if($calculation_basis==1) echo "Order Qty Pcs";else "Plan Cut(PCS)";?></th>
				 ...
				 $sql="select
					...
					foreach ($nameArray as $selectResult)
					{
						...
						if($selectResult[csf('emb_name')]==3) {
							...
							{
								$wash_qty=$washqty_arr[$selectResult[csf('po_id')]][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$selectResult[csf('wo_pre_cost_embe_cost_dtls')]];
							}
						}
						if($calculation_basis==1)
						{
							...
							//echo $emb_req_qty.'fd';;
							$req_qnty=(($emb_req_qty+$wash_qty)/$selectResult[csf('plan_cut_qnty')])*$selectResult[csf('order_quantity')];
							}
							else
							{
							...
							//$emblishmentqty_arr[$selectResult[csf('po_id')]][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$selectResult[csf('wo_pre_cost_embe_cost_dtls')]]
								$req_qnty=(($emblishmentqty_arr[$selectResult[csf('po_id')]][$selectResult[csf('item_number_id')]][$selectResult[csf('color_number_id')]][$selectResult[csf('wo_pre_cost_embe_cost_dtls')]]+$wash_qty)/$selectResult[csf('plan_cut_qnty')])*$selectResult[csf('order_quantity')];
							}
						...
						else
						{
							if($cbo_level==1)
							{
								...
								$req_qnty=(($emb_req_qty+$wash_qty)/$selectResult[csf('order_quantity')])*$selectResult[csf('plan_cut_qnty')];
							}
							else
							{
								$emb_req_qty=$emblishmentqty_arr[$selectResult[csf('po_id')]][$selectResult[csf('wo_pre_cost_embe_cost_dtls')]];
								$req_qnty=(($emblishmentqty_arr[$selectResult[csf('po_id')]][$selectResult[csf('wo_pre_cost_embe_cost_dtls')]]+$wash_qty)/$selectResult[csf('order_quantity')])*$selectResult[csf('plan_cut_qnty')];
							}
						}
						...
						if($calculation_basis==1) $calculation_cond="Pre Req Qty($emb_req_qty)/Plan Cut($plan_cut_qnty)*Order Qty($order_quantity)"; else $calculation_cond="Pre Req Qty($emb_req_qty)/Order Qty($order_quantity)*Plan Cut($plan_cut_qnty)";
						...
						<tr <? echo $bgcolor; ?>>
                            ...
                            <td width="100" align="right" title="ReqQty Calculation=<? echo $calculation_cond;?>"><p><? echo number_format($req_qnty,2); ?></p> </td>
