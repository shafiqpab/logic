...
if ($action=="fabric_search_list_view"){
	...
	if (str_replace("'","",$job)==""){echo "Please Insert Job First."; die;}
...
if($action=="create_po_search_list_view"){
	...
	if ($data[2]==0){
		 $sql= "select a.job_no_prefix_num,$insert_year, a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity,b.id, b.po_number,b.po_quantity,b.shipment_date from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.status_active=1 and b.status_active=1  and b.shiping_status not in(3) $shipment_date $company $buyer $job_cond $order_cond $year_cond order by a.job_no";  
...
if($action=="generate_aop_booking"){
	...
	from wo_booking_dtls a  where  a.booking_type=3 and a.process=35 and a.is_short=2 and a.status_active=1 and a.pre_cost_fabric_cost_dtls_id in($data[4]) and a.is_deleted=0 ";
	...
	foreach($dataArray as $row){
		...
		/*$cu_booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['id']=$id;
		$cu_booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['fabric_description_id']=$pre_cost_conversion_cost_dtls_id;
		$cu_booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['artwork_no']=$artwork_no;
		$cu_booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['fabric_color_id']=$fabric_color_id;
		$cu_booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['item_size']=$item_size;
		$cu_booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['description']=$description;
		$cu_booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['uom']=$uom;
		$cu_booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['delivery_date']=$delivery_date;
		$cu_booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['delivery_end_date']=$delivery_end_date;*/
		$cu_booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['wo_qnty']+=$wo_qnty;
		/*$cu_booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['rate']=$rate;
		$cu_booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['amount']=$amount;
		$cu_booking_data_arr[$pre_cost_conversion_cost_dtls_id][$po_break_down_id][$color_number_id][$dia_width]['dia_width']=$dia_width;*/
	}
	...
	a.booking_no='$txt_booking_no' and a.id in ($dtls_id) and   a.status_active=1 and a.pre_cost_fabric_cost_dtls_id=$data[4] and a.is_deleted=0 ";
	$dataArray=sql_select($sql);
	$z=1; $i=1;
	foreach($dataArray as $row){
		$id=$row[csf("id")];
	...
	//$req_qty_arr=$fabric->getQtyArray_by_orderFabriccostidAndGmtscolor_knitAndwoven_greyAndfinish();
	//$req_amount_arr=$fabric->getAmountArray_by_orderFabriccostidAndGmtscolor_knitAndwoven_greyAndfinish();
	...
	foreach($dataArray as $row){
		...
		$dia_width         = $booking_data_arr[$pre_cost_conversion_cost_id][$po_break_down_id][$color_number_id][$dia_width]['dia_width'];
		...
		$po_color_level_data_arr[$pre_cost_conversion_cost_id][$color_number_id][$dia_width]['fabric_color_id'][$po_break_down_id]     = $item_color_id;
						...
						foreach($po_color_level_data_arr as $precost_conversion_cost_id=>$pre_cost_cost_conversion_cost_val){
							foreach($pre_cost_cost_conversion_cost_val as $color_id=>$color_val){
								foreach($color_val as $dia_width_id=>$dia_width_val){
									...
									//echo $blaqnty."==".$is_short."==".jahid;die;
...
if($action=="show_aop_booking"){
	...
	a.amount,a.gmts_size,a.gmts_color_id,a.fin_dia,a.dia_width
	from wo_booking_dtls a  where  a.booking_type=3 and a.process=35 and 
	a.booking_no='$txt_booking_no'  and   a.status_active=1 and a.pre_cost_fabric_cost_dtls_id=$data[4] and a.is_deleted=0 ";
	$dataArray=sql_select($sql);
	$z=1; $i=1;
	foreach($dataArray as $row){
		$id=$row[csf("id")];
...
if ($action=="fabric_detls_list_view"){
    ...
    <div id="" style="" class="accord_close">
        ...
        <table class="rpt_table" border="1" width="1100" cellpadding="0" cellspacing="0" rules="all" id="">
            ...
            ?>
                <tr bgcolor="<? echo $bgcolor; ?>" onClick="set_data('<? echo $po_break_down_id;  ?>','<? echo $fabric_cost_id; ?>','<? echo $key; ?>','<? echo $booking_id?>')"  style="cursor:pointer" >
                    <td> <? echo $i; ?></td>
                    <td><? echo  $job_no; ?></td>
                    <td><? //echo  $po_number; ?><a href="#" onClick="setdata('<? echo $po_number;?>' )">View</a></td>
...
if ($action=="save_update_delete"){
	...
	if ($operation==0){
		...
		$new_booking_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'SASB', date("Y",time()), 5,"select id, booking_no_prefix, booking_no_prefix_num from wo_booking_mst where company_id=$cbo_company_name and booking_type=3 and entry_form=404 and to_char(insert_date,'YYYY')=".date('Y',time())." order by id desc ", "booking_no_prefix", "booking_no_prefix_num" ));
		
		$id=return_next_id( "id", "wo_booking_mst", 1 ) ;
		$field_array="id, booking_type, is_short, booking_month, booking_year, booking_no_prefix, booking_no_prefix_num, booking_no, entry_form, company_id, buyer_id, item_category, supplier_id, currency_id, exchange_rate, booking_date, delivery_date, pay_mode, source, attention, tenor, process, cbo_level, inserted_by, insert_date, status_active, is_deleted";
		
		$data_array ="(".$id.",3,".$cbo_is_short.",".$cbo_booking_month.",".$cbo_booking_year.",'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',404,".$cbo_company_name.",".$cbo_buyer_name.",12,".$cbo_supplier_name.",".$cbo_currency.",".$txt_exchange_rate.",".$txt_booking_date.",".$txt_delivery_date.",".$cbo_pay_mode.",".$cbo_source.",".$txt_attention.",".$txt_tenor.",35,".$cbo_level.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		...
		if($issue_mrr){
		 ...
		 $field_array_up="booking_month*booking_year*buyer_id*item_category*supplier_id*currency_id*exchange_rate*booking_date*delivery_date*pay_mode*source*attention*tenor*cbo_level*updated_by*update_date";
		 $data_array_up ="".$cbo_booking_month."*".$cbo_booking_year."*".$cbo_buyer_name."*12*".$cbo_supplier_name."*".$cbo_currency."*".$txt_exchange_rate."*".$txt_booking_date."*".$txt_delivery_date."*".$cbo_pay_mode."*".$cbo_source."*".$txt_attention."*".$txt_tenor."*".$cbo_level."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
...
if ($action=="save_update_delete_dtls")
{
		...
		if($db_type==0)
		{
		 ...
		 for ($i=1;$i<=$row_num;$i++)
		 {
			 ...
			 if(str_replace("'","",$$item_color)!="")
			 {
				 ...
				 {
					  $color_id = return_id( str_replace("'","",$$item_color), $color_library, "lib_color", "id,color_name","404");  
			 ...
			 if ($i!=1) $data_array1 .=",";
			 $data_array1 .="(".$id_dtls.",".$update_id.",".$cbo_is_short.",".$$job_no.",".$$fabric_description_id.",404,".$$color_size_table_id.",".$$artworkno.",".$$po_id.",".$txt_booking_no.",3,".$color_id.",".$$gmts_color_id.",".$$fabric_description_id.",".$$uom.",35,".$$txt_woqnty.",".$$txt_rate.",".$$txt_amount.",".$$startdate.",".$$enddate.",".$$dia.",".$$findia.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		     $id_dtls=$id_dtls+1;
		 }
	...
	else if ($operation==1)   // Update Here
	{
		...
		if($issue_mrr){
		 ...
		 $field_array_up1="artwork_no*color_size_table_id*fabric_color_id*gmts_color_id*description*uom*wo_qnty*rate*amount*delivery_date*delivery_end_date*dia_width*fin_dia*updated_by*update_date";
		 $new_array_color=array();
		 for ($i=1;$i<=$row_num;$i++){
			 $job_no="job_no_".$i;
			 $po_id="po_id_".$i;
			...
			$data_array_up1[str_replace("'",'',$$updateid)] =explode("*",("".$$artworkno."*".$$color_size_table_id."*".$color_id."*".$$gmts_color_id."*".$$fabric_description_id."*".$$uom."*".$$txt_woqnty."*".$$txt_rate."*".$$txt_amount."*".$$startdate."*".$$enddate."*".$$dia."*".$$findia."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
			}
		 }
...
if ($action=="save_update_delete_dtls_job_level"){
	...
	if ($operation==0)  // Insert Here
	{
		...
		if($db_type==0)
		{
		 ...
		 if  ( check_table_status( $_SESSION['menu_id'], 1 )==0 ) { echo "15**0";disconnect($con); die;}		
		 $id_dtls=return_next_id( "id", "wo_booking_dtls", 1 ) ;
		 $field_array1="id, booking_mst_id, is_short, job_no, pre_cost_fabric_cost_dtls_id, entry_form_id, artwork_no, po_break_down_id, booking_no, booking_type, fabric_color_id, gmts_color_id, description, uom, process, wo_qnty, rate, amount, delivery_date, delivery_end_date, dia_width, fin_dia, inserted_by, insert_date, status_active, is_deleted";
		 $j=1;
		 $new_array_color=array();
		 for ($i=1;$i<=$row_num;$i++)
		 {
			 ...
			 foreach($json_data->$precostid->$colorid->$dia->po_id as $poId)
			 {
				 if($woq>0)
				 {
					 ...
					 $amount=$wQty*$rate;
					 if ($j!=1) $data_array1 .=",";
					$data_array1 .="(".$id_dtls.",".$update_id.",".$cbo_is_short.",".$$job_no.",".$$fabric_description_id.",404,".$$artworkno.",".$poId.",".$txt_booking_no.",3,".$color_id.",".$colorid.",".$$fabric_description_id.",".$$uom.",35,".$wQty.",".$$txt_rate.",".$amount.",".$$startdate.",".$$enddate.",'".$dia."',".$$findia.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
					$id_dtls=$id_dtls+1;
					$j++;
				 }
			 }
		...
		if($issue_mrr){
		 ...
		 $field_array_up1="artwork_no*fabric_color_id*gmts_color_id*description*uom*wo_qnty*rate*amount*delivery_date*delivery_end_date*dia_width*fin_dia*updated_by*update_date";
		 $new_array_color=array();
		 for ($i=1;$i<=$row_num;$i++){
			 $job_no="job_no_".$i;
			 $po_id="po_id_".$i;
			 ...
			 //echo "10**";
			 //print_r($json_data->$precostid->$colorid);
			 foreach($json_data->$precostid->$colorid->$dia->po_id as $poId){
				 if(str_replace("'","",$cbo_is_short)==2)
				 {
					 $wQty=($json_data->$precostid->$colorid->$dia->req_qnty->$poId/$reqqnty)*$woq;
				 ...
				 if(str_replace("'",'',$$updateid)!=""){
					...
					$data_array_up1[str_replace("'",'',$json_data->$precostid->$colorid->$dia->id->$poId)] =explode("*",("".$$artworkno."*".$color_id."*".$colorid."*".$$fabric_description_id."*".$$uom."*".$wQty."*".$rate."*".$amount."*".$$startdate."*".$$enddate."*'".$dia."'*".$$findia."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
				 }
			 }
		 }
		 //check_table_status( $_SESSION['menu_id'],0);
...
if ($action=="create_booking_search_list_view"){
	...
	if($data[7]==1){
	 $sql= "select a.id,a.booking_no_prefix_num,a.booking_no,a.booking_date,a.company_id,a.buyer_id,
	...
	where $company $buyer $booking_date and  a.booking_no=b.booking_no and b.job_no=c.job_no and a.booking_type=3 and  a.status_active=1 and a.is_deleted=0 and a.process=35
	...
	c.job_no_prefix_num,b.po_break_down_id,a.item_category,a.fabric_source,a.supplier_id ) and  a.booking_type=3 and  a.status_active=1 and a.is_deleted=0 and a.process=35
	    $booking_cond 
	group by a.id,a.booking_no_prefix_num,a.booking_no,a.booking_date,a.company_id,a.buyer_id,
...
if($action=="show_trim_booking_report1"){
	...
	<div style="width:1333px" align="center">       
       <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid black;">
		...
		$nameArray_buyer=sql_select( "select  a.style_ref_no, a.job_no, a.style_owner, a.buyer_name, a.dealing_marchant,a.season,a.season_matrix   from wo_po_details_master a, wo_booking_dtls b where a.job_no=b.job_no and b.booking_no=$txt_booking_no"); 
	...
	//=================================================
	   ...
	   foreach($nameArray_item as $result_item){
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            $nameArray_item_description=sql_select( "select  fabric_color_id,gmts_color_id,description,rate,dia_width,sum(wo_qnty) as cons from wo_booking_dtls  where booking_no=$txt_booking_no  and process=35 and description='".$result_item[csf('description')]."' and wo_qnty !=0 and status_active=1 and is_deleted=0  group by fabric_color_id,gmts_color_id,description,rate,dia_width");//and sensitivity=1 
                foreach($nameArray_item_description as $result_itemdescription)
                {
            ...
            <tr>
                ...
                ?>
            <tr>
            <td colspan="2" align="right"> <strong> Total Qty (kg) </strong></td>
...
if ($action=="populate_data_from_search_popup"){
	 $sql= "select booking_no,booking_date,company_id,buyer_id,job_no,po_break_down_id,item_category,fabric_source,currency_id,exchange_rate,pay_mode,booking_month,supplier_id,attention,tenor,delivery_date,source,booking_year,cbo_level,is_short from wo_booking_mst  where booking_no='$data'"; 
	...
	/*	$po_no="";
		...
		//echo "load_drop_down( 'requires/service_booking_aop_urmi_controller', '".$row[csf("job_no")]."_".$row[csf("booking_no")]."', 'load_drop_down_fabric_description', 'fabric_description_td' )\n";
