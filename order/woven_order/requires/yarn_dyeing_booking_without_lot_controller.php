...
if ($action=="load_drop_down_supplier")
{
	...
	{
		echo create_drop_down( "cbo_supplier_name", 140, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_tag_company c where a.id=c.supplier_id and c.tag_company=$data_exp[1] and a.status_active =1 and a.id in(select supplier_id from lib_supplier_party_type where party_type=2) and a.id in(select supplier_id from lib_supplier_party_type where party_type=21) group by a.id,a.supplier_name order by supplier_name","id,supplier_name", 1, "-- Select Supplier --", $selected, "",0 );
	}
	exit;
}

if ($action=="check_issue_and_return")
{
	$sql_issue=sql_select("SELECT
		         a.issue_number,
		         a.booking_no,
		         SUM (b.cons_quantity) issue_quantity
	...
	foreach ($sql_issue as $row) {
		$issue_qnt+=$row[csf('issue_quantity')];
	...
	$sql_return=sql_select("SELECT 
						   ...
						   WHERE     a.id = b.mst_id
						         ...
						         and b.status_active = 1
						         and b.is_deleted = 0
						         and a.receive_basis=1
						ORDER BY a.id DESC");

	$return_qnt=0;
	$return_number='';
	foreach ($sql_return as $row) {
		$return_qnt+=$row[csf('cons_quantity')];
		if(!empty($return_number))
		{
			$return_number.=",".$row[csf('recv_number')];
		}
		else{
			$return_number=$row[csf('recv_number')];
		}
	...
	$status=0;
	if($issue_qnt-$return_qnt>0)
	{
		echo $status."***".$issue_number."***".$issue_qnt."***".$return_number."***".$return_qnt;
	}
	else{
		$status=1;
		echo $status."***".$issue_number."***".$issue_qnt."***".$return_number."***".$return_qnt;
...
if($action=="check_recieve_found")
{
	...
	$sql_return=sql_select("  SELECT 
								       ...
								       a.dyeing_charge,
								       c.recv_number
							  FROM wo_yarn_dyeing_dtls a,wo_yarn_dyeing_mst b,inv_receive_master c,inv_transaction d,product_details_master e
							  where   b.id=a.mst_id 
									  and C.BOOKING_NO=B.YDW_NO
									  and c.id=d.mst_id
									  and c.receive_basis=2
									  and C.RECEIVE_PURPOSE=2
									  and D.PROD_ID=e.id
									  and A.COUNT=E.YARN_COUNT_ID
									  and A.YARN_COMP_TYPE1ST=E.YARN_COMP_TYPE1ST
									  and A.YARN_TYPE=E.YARN_TYPE
									  and A.YARN_COLOR=E.COLOR
									  and a.id='$dtls_id'
  					");
...
if($action=="populate_budge_req_data")
{
	...
	echo "$('#txt_budget_wo_qty').val('".number_format($cum_bal,4,'.','')."');\n";
	exit;
}
			...
			<table width="900" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
				...
				<tbody>
					<tr>
						...
						<td align="center">
							<input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_job_no').value+'_'+document.getElementById('txt_style_no').value+'_'+document.getElementById('txt_order_no').value+'_'+document.getElementById('txt_fab_booking_no').value+'_'+document.getElementById('cbo_budget_version').value, 'create_booking_search_list_view', 'search_div', 'yarn_dyeing_booking_without_lot_controller', 'setFilterGrid(\'tbl_list_search\',-1)')" style="width:70px;" />
						</td>
					</tr>
				</tbody>
			</table>
...
if ($action=="create_booking_search_list_view")
{
	...
	$po_ids=count(explode(",",$poidstr)); $poidCond="";
		...
		<div style="width:790px; overflow-y:scroll; max-height:250px;" id="buyer_list_view">
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="770" class="rpt_table" id="tbl_list_search" >
				...
				foreach ($booking_arr as $bookino_no=>$selectResult)
				{
					...
					{
						if($poNo=="") $poNo=$poDataArr[$pid]['po']; else $poNo.=','.$poDataArr[$pid]['po'];
						if($fileNo=="") $fileNo=$poDataArr[$pid]['file']; else $fileNo.=','.$poDataArr[$pid]['file'];
						if($refNo=="") $refNo=$poDataArr[$pid]['ref']; else $refNo.=','.$poDataArr[$pid]['ref'];
					}
					...
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value('<? echo $bookino_no; ?>'+'_'+'<? echo $budget_version; ?>'+'_'+'<? echo $is_short; ?>'); ">
						...
						<td width="40"><p> <? echo $i; ?></p></td>
						<td width="50"  align="center"> <p><? echo $selectResult[('year')]; ?></p></td>
						<td width="120"  align="center"> <p><? echo $bookino_no; ?></p></td>
						<td width="130"><p><?  echo  $buyer_arr[$selectResult[('buyer_name')]]; ?></p></td>
						<td width="120"> <p><?  echo $selectResult[('job_no')]; ?></p></td>
						<td width="150"> <p><?  echo $selectResult[('style_ref_no')]; ?></p></td>
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)
	{
		...
		$sql_select="select sum(yarn_wo_qty) as yarn_wo_qty  from wo_yarn_dyeing_mst a,wo_yarn_dyeing_dtls b where a.id=b.mst_id and a.ydw_no=$txt_booking_no and a.entry_form=125 and b.job_no='$txt_job_no' and b.fab_booking_no='$txt_fab_booking_no' and b.count=$cbo_count and b.yarn_comp_type1st=$cbo_composition and b.yarn_type=$cbo_yarn_type and b.yarn_color=$txt_yern_color and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
		$sql_result=sql_select($sql_select);
		$yarn_wo_qty_found=0;
		$yarn_wo_qty_found=$sql_result[0][csf('yarn_wo_qty')];
		if($yarn_wo_qty_found>0)
		{
			echo "40**Work Order Entry Does Not Allow Same As following Fabric\nWO No,Job No,Booking No,Count,composition,Type,Color"; disconnect($con);die;
		}
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		$wo_qnty=str_replace("'","",$txt_wo_qty);
		$supplier_id=str_replace("'","",$cbo_supplier_name);
		$yarn_color_id=str_replace("'","",$txt_yern_color);
		$company_id=str_replace("'","",$cbo_company_name);

		$result_wo=sql_select("select a.company_id,a.supplier_id,sum(case when b.id<>$dtls_update_id then b.yarn_wo_qty else 0 end) as yarn_wo_qty from wo_yarn_dyeing_mst a,wo_yarn_dyeing_dtls b where a.id=b.mst_id and a.ydw_no=$txt_booking_no and b.entry_form=125 and b.job_no='$txt_job_no' and b.fab_booking_no='$txt_fab_booking_no' and b.count=$cbo_count and b.yarn_comp_type1st=$cbo_composition and b.yarn_type=$cbo_yarn_type and b.yarn_color=$yarn_color_id and b.status_active=1 and b.is_deleted=0 group by a.company_id,a.supplier_id");
		...
		$tot_wo_qty=$tot_previous_wo_qnty+$wo_qnty;

		$result_yarn=sql_select("select a.issue_number,b.cons_quantity from inv_issue_master a, inv_transaction b, product_details_master c where a.id=b.mst_id and b.prod_id=c.id and a.booking_no=$txt_booking_no and a.booking_id=$update_id and b.transaction_type=2 and a.company_id=$cbo_company_name  and c.yarn_comp_type1st=$cbo_composition and a.entry_form=3 and b.dyeing_color_id=$yarn_color_id and a.issue_purpose=2 and a.status_active=1 and b.status_active=1 and c.status_active=1 and c.is_deleted=0");// omit and c.yarn_count_id=$cbo_count and c.yarn_type=$cbo_yarn_type 
		...
		$result_yarn_rtn=sql_select("select a.recv_number,b.cons_quantity from inv_receive_master a, inv_transaction b,product_details_master c where a.id=b.mst_id and b.prod_id=c.id and a.booking_no=$txt_booking_no and a.booking_id=$update_id and b.transaction_type=4 and a.company_id=$cbo_company_name and c.yarn_comp_type1st=$cbo_composition and b.dyeing_color_id=$yarn_color_id and a.entry_form=9 and a.receive_basis=1 and a.status_active=1 and b.status_active=1 and c.status_active=1 and c.is_deleted=0");//omit only consider composition : and c.yarn_count_id=$cbo_count and c.yarn_type=$cbo_yarn_type

		foreach($result_yarn_rtn as $row)
		{
			$tot_issue_rtn_qty+=$row[csf("cons_quantity")];
			$yarn_issue_rtn_no.=$row[csf("recv_number")].",";
		}

		//echo "10**".$tot_issue_qty.'='.$tot_issue_rtn_qty ."==".$tot_wo_qty;die;

		if( ($tot_issue_qty-$tot_issue_rtn_qty ) > $tot_wo_qty )
		{
			if($yarn_issue_no!="")
			{
				$yarn_issue_no = chop($yarn_issue_no,",");
				$yarn_issue_rtn_no = chop($yarn_issue_rtn_no,",");
				$actual_issue_qty = ($tot_issue_qty-$tot_issue_rtn_qty);

				if($yarn_issue_rtn_no!="")
				{
					$issueRtnMsg = "\nIssue Return Found\nReturn Mrr No=$yarn_issue_rtn_no";
				}

				$msg_txt = "Yarn Wo. quantity can not be less than issue quantity.\nIssue No=$yarn_issue_no".$issueRtnMsg."\nUpto reducible quantity = $actual_issue_qty";
				echo "14**".$msg_txt; disconnect($con);die;
			}
		}

		if($company_id!=$prev_company_id && $tot_issue_qty>0)
		{
			$msg_txt = "Issue Found=".$yarn_issue_no."";
			$msg_txt .= "\nCompany Change Not allowed";
			echo "14**".$msg_txt; disconnect($con);die;
		}
		else if($supplier_id!=$prev_supplier_id && $tot_issue_qty>0)
		{
			$msg_txt = "Issue Found=".$yarn_issue_no."";
		...
		$field_array="company_id*supplier_id*booking_date*delivery_date*delivery_date_end*dy_delivery_date_start*dy_delivery_date_end*currency*ecchange_rate*pay_mode*source*attention*tenor*is_short*budget_version*updated_by*update_date*status_active*is_deleted";
		$data_array="".$cbo_company_name."*".$cbo_supplier_name."*".$txt_booking_date."*".$txt_delivery_date."*".$txt_delivery_end."*".$dy_delevery_start."*".$dy_delevery_end."*".$cbo_currency."*".$txt_exchange_rate."*".$cbo_pay_mode."*".$cbo_source."*".$txt_attention."*".$txt_tenor."*".$cbo_is_short."*'".$cbo_budget_version."'*'".$user_id."'*'".$pc_date_time."'*'".$cbo_wo_status."'*0";

		$field_array_dtls = "job_no*fab_booking_no*product_id*job_no_id*count*yarn_description*yarn_comp_type1st*yarn_comp_percent1st*yarn_type*yarn_color*color_range*uom*yarn_wo_qty*dyeing_charge*amount*no_of_bag*no_of_cone*min_require_cone*file_no*internal_ref_no*remarks*referance_no*app_batch_no*status_active";
		$data_array_dtls = "'".$txt_job_no."'*'".$txt_fab_booking_no."'*'".$txt_pro_id."'*".$txt_job_id."*".$cbo_count."*'".$txt_item_desc."'*".$cbo_composition."*".$txt_pacent."*".$cbo_yarn_type."*".$txt_yern_color."*".$cbo_color_range."*".$cbo_uom."*".$txt_wo_qty."*".$txt_dyeing_charge."*".$txt_amount."
		*".$txt_bag."*".$txt_cone."*".$txt_min_req_cone."*".$txt_file_no."*".$txt_int_ref_no."*".$txt_remarks."*'".$txt_ref_no."'*'".$txt_app_batch_no."'*'".$cbo_wo_status."'";
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("ROLLBACK");
				echo "10**".str_replace("'",'',$return_no)."**".str_replace("'",'',$update_id)."**".$rID."**".$dtlsrID;
			}
		}
		if($db_type==2)
		{
...
{	
	if($db_type==0)
	{
		$sql = "select a.id, a.job_no, a.fab_booking_no, a.count, a.yarn_description, a.app_batch_no, a.yarn_color, a.color_range, a.uom, a.yarn_wo_qty, a.dyeing_charge, a.amount, a.no_of_bag, a.no_of_cone, a.min_require_cone, a.referance_no, a.yarn_comp_type1st, a.yarn_comp_percent1st, group_concat(b.file_no) as file_no, group_concat(b.grouping) as internal_ref_no from wo_yarn_dyeing_dtls a, wo_po_break_down b where a.job_no=b.job_no_mst and a.status_active=1 and a.is_deleted=0 and a.mst_id='$data' group by a.id, a.job_no, a.fab_booking_no, a.count, a.yarn_description, a.app_batch_no, a.yarn_color, a.color_range, a.uom, a.yarn_wo_qty, a.dyeing_charge, a.amount, a.no_of_bag, a.no_of_cone, a.min_require_cone, a.referance_no, a.yarn_comp_type1st, a.yarn_comp_percent1st";
	}
	else
	{
		$sql = "select a.id, a.job_no, a.fab_booking_no, a.count, a.yarn_description, a.app_batch_no, a.yarn_color, a.color_range, a.uom, a.yarn_wo_qty, a.dyeing_charge, a.amount, a.no_of_bag, a.no_of_cone, a.min_require_cone, a.referance_no, a.yarn_comp_type1st, a.yarn_comp_percent1st, listagg(cast(b.file_no as varchar(4000)), ',') within group(order by b.file_no) as file_no, listagg(cast(b.grouping as varchar(4000)), ',') within group(order by b.grouping) as internal_ref_no from wo_yarn_dyeing_dtls a, wo_po_break_down b where a.job_no=b.job_no_mst and a.status_active=1 and a.is_deleted=0 and a.mst_id='$data' group by a.id, a.job_no,a.fab_booking_no,a.count, a.yarn_description, a.app_batch_no, a.yarn_color, a.color_range, a.uom, a.yarn_wo_qty, a.dyeing_charge, a.amount, a.no_of_bag, a.no_of_cone, a.min_require_cone, a.referance_no, a.yarn_comp_type1st, a.yarn_comp_percent1st";
	}
	...
	{
		?>
		<table width="1380" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
		<thead>
			<tr>
				<th width="40">SL</th>
				<th width="80">Job No</th>
...
if($action=="child_form_input_data")
{
	if($db_type==0)
	{
		$sql = "select a.id, a.mst_id, a.job_no,a.fab_booking_no,a.app_batch_no, a.product_id, a.job_no_id, a.count, a.yarn_description, a.yarn_color, a.color_range, a.uom, a.yarn_wo_qty, a.dyeing_charge, a.amount, a.no_of_bag, a.no_of_cone, a.min_require_cone, a.remarks, a.referance_no, a.yarn_comp_type1st, a.yarn_comp_percent1st, a.yarn_type, group_concat(b.file_no) as file_no, group_concat(b.grouping) as internal_ref_no
		from wo_yarn_dyeing_dtls a, wo_po_break_down b
		where a.job_no=b.job_no_mst and a.id='$data'
		group by a.id, a.mst_id, a.job_no,a.fab_booking_no,a.app_batch_no, a.product_id, a.job_no_id, a.count, a.yarn_description, a.yarn_color, a.color_range, a.uom, a.yarn_wo_qty, a.dyeing_charge, a.amount, a.no_of_bag, a.no_of_cone, a.min_require_cone, a.remarks, a.referance_no, a.yarn_comp_type1st, a.yarn_comp_percent1st, a.yarn_type";
	}
	else
	{
		$sql = "select a.id, a.mst_id, a.job_no,a.fab_booking_no, a.app_batch_no, a.product_id, a.job_no_id, a.count, a.yarn_description, a.yarn_color, a.color_range, a.uom, a.yarn_wo_qty, a.dyeing_charge, a.amount, a.no_of_bag, a.no_of_cone, a.min_require_cone, a.remarks, a.referance_no, a.yarn_comp_type1st, a.yarn_comp_percent1st, a.yarn_type, listagg(cast(b.file_no as varchar(4000)), ',') within group(order by b.file_no) as file_no, listagg(cast(b.grouping as varchar(4000)), ',') within group(order by b.grouping) as internal_ref_no
		from wo_yarn_dyeing_dtls a, wo_po_break_down b
		where a.job_no=b.job_no_mst and a.id='$data'
		group by a.id, a.mst_id, a.job_no,a.fab_booking_no, a.app_batch_no, a.product_id, a.job_no_id, a.count, a.yarn_description, a.yarn_color, a.color_range, a.uom, a.yarn_wo_qty, a.dyeing_charge, a.amount, a.no_of_bag, a.no_of_cone, a.min_require_cone, a.remarks, a.referance_no, a.yarn_comp_type1st, a.yarn_comp_percent1st, a.yarn_type";
	}
...
if($action=="create_sys_search_list_view")
{
	...
	if($chk_job_wo_po==1)
	{
		...
		where a.status_active=1 and a.is_deleted=0 and a.entry_form=125 and a.id not in(select mst_id from wo_yarn_dyeing_dtls where job_no_id>0 and entry_form=125  and status_active=1 and  is_deleted=0) $company $supplier  $sql_cond  $booking_cond order by a.id DESC";
	}
	else
	{
		if($db_type==0)
		{
			...
			where a.id=b.mst_id and b.job_no_id=d.id and a.status_active in(1,2) and a.is_deleted=0 and b.status_active in (1,2) and a.entry_form=125 and b.entry_form=125 $company $supplier  $sql_cond  $buyer_cond $job_cond $booking_cond
				...
				foreach ($nameArray as $selectResult)
				{
					...
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value(<? echo $selectResult[csf('id')]; ?>+'_'+'<? echo $selectResult[csf('ydw_no')]; ?>'); ">
						...
						<td width="200" style="word-break:break-all">
							...
							<td width="130" style="word-break:break-all"><? echo $supplier_arr[$selectResult[csf('supplier_id')]]; ?></td>
...
{
	$sql="select  a.id, a.yarn_dyeing_prefix_num, a.ydw_no, a.company_id, a.supplier_id, a.booking_date, a.delivery_date, a.delivery_date_end, a.dy_delivery_date_start, a.dy_delivery_date_end, a.currency, a.ecchange_rate, a.pay_mode,a.source, a.attention, a.tenor, a.is_short,budget_version,status_active from wo_yarn_dyeing_mst a where  a.id=$data";
	//echo $sql;die;
	$contact_person=return_library_array( "select id, contact_person from lib_supplier",'id','contact_person');
	$res = sql_select($sql);
	foreach($res as $row)
	{
		...
		$data_comp=$row[csf("pay_mode")].'_'.$row[csf("company_id")];
		echo "load_drop_down( 'requires/yarn_dyeing_booking_without_lot_controller','".$data_comp."', 'load_drop_down_supplier', 'supplier_td' );\n";
		...
		echo "$('#cbo_wo_status').val(".$row[csf("status_active")].");\n";
	}
	exit();
}
...
if($action=="show_with_multiple_job")
{
	...
	$nameArray=sql_select( "select a.id, a.ydw_no,a.booking_date,a.supplier_id,a.attention,a.delivery_date,a.pay_mode,a.delivery_date_end,a.dy_delivery_date_start,a.dy_delivery_date_end,a.currency,a.ecchange_rate, a.is_short from wo_yarn_dyeing_mst a where a.id=$update_id");
				...
				<td width="350"  style="font-size:12px">
					<table width="350" style="" align="left">
						<tr  >
							...
							<td width="230">:&nbsp;&nbsp;<? if($pay_mode==3 || $pay_mode==5)  echo $company_library[$supplier_id];else echo $supplier_arr[$supplier_id];;?></td>
						</tr>
...
if($action=="show_with_multiple_job_without_rate")
{
	...
	<div style="width:1140px" align="center">
		...
		foreach ($nameArray as $result)
		{
			...
			$currency_id=$result[csf('currency')];
			$attention=$result[csf('attention')];
			$delivery_date=$result[csf('delivery_date')];$pay_mode=$result[csf('pay_mode')];
			$delivery_date_end=$result[csf('delivery_date_end')];
			$dy_delivery_start=$result[csf('dy_delivery_date_start')];
			$dy_delivery_end=$result[csf('dy_delivery_date_end')];
		}
...
if($action=="print2")
{
	...
	<div style="width:1140px" align="center">
		...
		<?
		//echo "select a.id, a.ydw_no,a.booking_date,a.supplier_id,a.attention  from wo_yarn_dyeing_mst a where a.id=$update_id";die;
		$nameArray=sql_select( "select a.id, a.ydw_no,a.booking_date,a.supplier_id,a.attention,a.pay_mode,a.delivery_date,a.delivery_date_end,a.dy_delivery_date_start,a.dy_delivery_date_end,a.currency,a.booking_date from wo_yarn_dyeing_mst a where a.id=$update_id");
		...
		<td width="350"  style="font-size:12px">
			<table width="350" style="" align="left">
				<tr  >
					...
					<td width="230" style="font-size:18px">:&nbsp;&nbsp;<b><? if($pay_mode==3 || $pay_mode==5)  echo $company_library[$supplier_id];else echo $supplier_arr[$supplier_id];;?></b></td>
				</tr>
        ...
        $job_cond_for_booking=where_con_using_array(array_unique(array_filter($job_no_arrs)),1,"job_no");

         $booking_arr=sql_select("select job_no,booking_no from  wo_booking_dtls  where booking_type=1 and status_active=1 and color_type in (2,3,4,6,32,33,44,47,48,63) $job_cond_for_booking  group by job_no,booking_no");
        $job_arr=array();
        ...
        <tr>
        	<td colspan="10" align="right"><strong>Total:</strong>&nbsp;&nbsp;</td>
        	<td align="right" ><b><? echo number_format($total_qtys,2); ?></b></td>
        	<td>&nbsp;</td>
        	<td align="right"><b><? echo number_format($total_amount,2); ?></b></td>
        	<td>&nbsp;</td>
        	<td>&nbsp;</td>
...
if($action=="show_print2_booking_report")
{
	...
	foreach ($nameArray as $result)
	{
		...
		$currency_id=$result[csf('currency')];
		$exchange_rate=$result[csf('ecchange_rate')];
		$is_short=$result[csf('is_short')];$pay_mode=$result[csf('pay_mode')];
	}
			...
			<tr>
				<td width="350"  style="font-size:12px">
					<table width="350" style="" align="left">
						...
						<tr>
							<td colspan="2"><? if($pay_mode==3 || $pay_mode==5)  echo $company_library[$supplier_id];else echo $supplier_arr[$supplier_id];;?></td>
						</tr>
