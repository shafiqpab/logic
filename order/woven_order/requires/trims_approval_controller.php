...
$sql="select c.id, c.po_break_down_id,a.job_no, c.accessories_type_id, c.target_approval_date, c.sent_to_supplier, c.submitted_to_buyer, c.approval_status, c.approval_status_date, c.supplier_name, c.accessories_comments, c.status_active from wo_pre_cost_trim_cost_dtls a,wo_po_trims_approval_info c  where a.job_no=c.job_no_mst  and c.accessories_type_id=a.trim_group  and a.apvl_req=1 and c.job_no_mst='$job_no' and c.is_deleted=0 order by c.accessories_type_id,c.po_break_down_id,c.id";
	
	$dataArray=sql_select($sql);
	foreach($dataArray as $row)
	{
		$trims_po_approv_arr_chk[$row[csf("po_break_down_id")]]['po_ids']=$row[csf("po_break_down_id")];
	}
	
	$z=1; $i=1;
	$sql="select b.job_no_mst,b.id as po_break_down_id from  wo_po_break_down b where b.job_no_mst='$job_no' and b.status_active=1 and b.is_deleted=0 group by b.job_no_mst,b.id order by b.id";
	$dataArray2=sql_select($sql);
	foreach($dataArray2 as $row)
	{
	$po_ids=$trims_po_approv_arr_chk[$row[csf("po_break_down_id")]]['po_ids'];
	if($po_ids=="")
	{
	//$trims_approv_arr[$row[csf("job_no_mst")]]['po_ids'].=$row[csf("po_break_down_id")].',';
	//echo $row[csf("po_break_down_id")].'DD';
	$trims_new_po_approv_arr[$row[csf("po_break_down_id")]]=$row[csf("po_break_down_id")];
	}
	}
	//print_r($trims_approv_arr);
	$po_check=array();
	foreach($dataArray as $row)
	{
		...
		//$item_group_id_po_id=$accessories_type_id;
        ...
        ?>
				...
				<?
				//$po_id_chk=$trims_po_approv_arr[$row[csf("po_break_down_id")]]['po_ids'];
				//$po_id=rtrim($trims_approv_arr[$row[csf("job_no")]]['po_ids'],',');
				//$po_ids=array_unique(explode(",",$po_id));
				//print_r($po_ids);
				if(count($trims_new_po_approv_arr)>0)
				{
				$kk=1;$i=$i+1;
			
				foreach($trims_new_po_approv_arr as $pid)
				{
				//echo $pid.'=='.$row[csf("po_break_down_id")].'<br>';
				//$group_dtls_value=$pid;
				if (!in_array($pid,$po_check) )
					{
				...
				?>
                    <tr align="center">
                        <td>
                          <?
                                echo create_drop_down("po_no_".$item_group_id."_".$i, 110, $po_number_arr,"", 1,'', $pid,"",1);
                            ?>
                            <input type="hidden" name="po_id_<? echo $item_group_id.'_'.$i; ?>" id="po_id_<? echo $item_group_id.'_'.$i; ?>" value="<? echo $pid; ?>" style="width:110px;" class="text_boxes" disabled="disabled">
                        </td>
                        <td>
                            <?
                                echo create_drop_down("item_group_".$item_group_id."_".$i, 110, $item_arr,"", 1,'', $item_group_id,"",1);
                        ...
                        <td>
                            <?  
                               // echo create_drop_down( "cbo_supplier_".$item_group_id."_".$i, 100, "select a.id, a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type in(4,5) and status_active =1 and is_deleted=0 group by a.id, a.supplier_name order by a.supplier_name","id,supplier_name", 1, "--   --", $row[csf("supplier_name")], "copy_value(this.value,'cbo_supplier_',".$i.")", $disable_status );
								 echo create_drop_down( "cbo_supplier_".$item_group_id."_".$i, 100, $supplier_arr,"", 1, "--   --", "", "copy_value(this.value,'cbo_supplier_',".$i.")","" );
                            ?>
                        </td>
                        <td>
                            <input type="text" name="txt_comments_<? echo $item_group_id.'_'.$i; ?>" id="txt_comments_<? echo $item_group_id.'_'.$i; ?>" style="width:120px;" class="text_boxes" onChange="copy_value(this.value,'txt_comments_',<? echo $i; ?>)" value="<? // echo $row[csf("accessories_comments")]; ?>" <? //echo $disable; ?> >
                        ...
                        <td>
                            <?
                                echo create_drop_down("cbo_status_".$item_group_id."_".$i, 80, $row_status,"", 0, "",$row[csf("status_active")],"copy_value(this.value,'cbo_status_',".$i.")","");
                            ?>
                            <input type="hidden" name="updateid_<? echo $item_group_id.'_'.$i;?>" id="updateid_<? echo $item_group_id.'_'.$i; ?>" value="<? //echo $row[csf("id")];?>">
                        </td>
                        <td>
