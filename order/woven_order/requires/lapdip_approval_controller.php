...
if($action=="save_update_delete")
{
	...
	if($operation==0)  // Insert Here
	{
		...
		$field_array="id, job_no_mst, po_break_down_id, color_name_id, lapdip_target_approval_date, send_to_factory_date, recv_from_factory_date, submitted_to_buyer, approval_status, approval_status_date, lapdip_no,shade_per,pantone_no, lapdip_comments, is_master_color, is_deleted, status_active,entry_form, inserted_by, insert_date, garments_nature"; 
		...
		for($i=1; $i<=$tot_row; $i++)
		{
			...
			$txt_shade_per_no="txt_shade_per_no_".$color_id."_".$i;
			$txt_pantone_no="txt_pantone_no_".$color_id."_".$i;
			...
			if ($i!=1) $data_array .=",";
			$data_array .="(".$id.",'".$txt_job_no."',".$$po_id.",".$color_lib_id.",".$$target_app_date.",".$$send_to_factory_date.",".$$recv_from_factory_date.",".$$sent_to_buyer.",".$$action.",".$$action_date.",".$$txt_lapdip_no.",".$$txt_shade_per_no.",".$$txt_pantone_no.",".$$txt_comments.",".$is_master_color.",0,".$$cbo_status.",583,".$user_id.",'".$pc_date_time."',$garments_nature)";
	...
	else if ($operation==1)  // Update Here
	{
		...
		$field_array="id, job_no_mst, po_break_down_id, color_name_id, lapdip_target_approval_date, send_to_factory_date, recv_from_factory_date, submitted_to_buyer, approval_status, approval_status_date, lapdip_no, shade_per,pantone_no, lapdip_comments, is_master_color, is_deleted, status_active, inserted_by, insert_date, garments_nature";
		$field_array_update="job_no_mst*po_break_down_id* 	color_name_id*lapdip_target_approval_date*send_to_factory_date*recv_from_factory_date*submitted_to_buyer*approval_status*approval_status_date*lapdip_no*shade_per*pantone_no*lapdip_comments*is_deleted*status_active*updated_by*update_date*garments_nature";
		...
		for($i=1; $i<=$tot_row; $i++)
		{
			...
			$updateid="updateid_".$color_id."_".$i;
			 ...
			 if(str_replace("'",'',$$updateid)!="")
			 {
				...
				$data_array_update[str_replace("'",'',$$updateid)] = explode(",",("'".$txt_job_no."',".$$po_id.",".$$color_name_id.",".$$target_app_date.",".$$send_to_factory_date.",".$$recv_from_factory_date.",".$$sent_to_buyer.",".$$action.",".$$action_date.",".$$txt_lapdip_no.",".$$txt_shade_per_no.",".$$txt_pantone_no.",".$$txt_comments.",0,".$$cbo_status.",".$user_id.",'".$pc_date_time."',$garments_nature"));
			 }
			 else
			 {
				if($data_array!="") $data_array.=",";
			$data_array .="(".$id.",'".$txt_job_no."',".$$po_id.",".$$color_name_id.",".$$target_app_date.",".$$send_to_factory_date.",".$$recv_from_factory_date.",".$$sent_to_buyer.",".$$action.",".$$action_date.",".$$txt_lapdip_no.",".$$txt_shade_per_no.",".$$txt_pantone_no.",".$$txt_comments.",1,0,".$$cbo_status.",".$user_id.",'".$pc_date_time."',$garments_nature)";
...
if($action=="lapdip_approval_list_view_edit")
{
	...
	$BookData=sql_select("select a.is_approved from wo_booking_mst a,wo_booking_dtls b where a.booking_no=b.booking_no and  b.job_no='$job_no' and b.status_active=1 group by a.is_approved");
	$booking_is_approved=0;
	foreach($BookData as $row)
	{
		if($row[csf('is_approved')]==1)
		{
			$booking_is_approved=$row[csf('is_approved')];
		}
		
	}
	$booking_app="";$booking_is=0;
	if($booking_is_approved==1)
	{
		 
		$booking_app="disabled='disabled'";
		$booking_is=1;
	}
	?>
    <script>
	...
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'lapdip_approval_controller.php?action=color_popup&buyer_name='+buyer_name, 'Color Select Pop Up', 'width=250px,height=450px,center=1,resize=1,scrolling=0','../../')
	...
	$sql="SELECT id, po_break_down_id, color_name_id, lapdip_target_approval_date, send_to_factory_date, recv_from_factory_date, submitted_to_buyer, approval_status, approval_status_date,lapdip_no,shade_per,pantone_no,lapdip_comments,status_active from wo_po_lapdip_approval_info where job_no_mst='$job_no' and is_deleted=0 and status_active=1 order by color_name_id,po_break_down_id,id";
	...
	foreach($dataArray as $row)
	{
		...
		if($print_cond_footer==1)
		{
					...
					?>
						<tr align="center">
							...
							<td>
								A <input type="text" name="target_app_date_<? echo $prev_color_id.'_'.$i; ?>" id="target_app_date_<? echo $prev_color_id.'_'.$i; ?>" style="width:70px;" class="datepicker" onChange="copy_value(this.value,'target_app_date_',<? echo $i; ?>)">
							</td>
							<td>
							...
							<td>
								<input type="text" name="txt_pantone_no_<? echo $prev_color_id.'_'.$i; ?>" id="txt_pantone_no_<? echo $prev_color_id.'_'.$i; ?>" style="width:100px;" class="text_boxes" onChange="copy_value(this.value,'txt_pantone_no_',<? echo $i; ?>)">
							</td>
							<td>
		...
		{
		?>
            <h3 align="left" id="accordion_h<? echo $color_id; ?>" style="width:1175px" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel_<? echo $color_id; ?>', 'fnc_color_id(<? echo $color_id; ?>,1,0)',1)">  +<? echo $color_arr[$color_id]; ?> <span style="color: red">[<? echo $color_approved_status[$color_id] ?>]</span></h3>
        ...
        ?>
                <tr align="center" title="<?=$po_number_arr[$row[csf("po_break_down_id")]];?>">
                    <td>
                        ...
                        ?>
                        <input type="hidden" name="po_id_<? echo $color_id.'_'.$i; ?>" id="po_id_<? echo $color_id.'_'.$i; ?>" value="<? echo $row[csf("po_break_down_id")]; ?>" style="width:100px;" class="text_boxes"  >
                    </td>
                    <td>
                        <?
                    ...
                    <td>
                        <input type="text" name="target_app_date_<? echo $color_id.'_'.$i; ?>" id="target_app_date_<? echo $color_id.'_'.$i; ?>" style="width:70px;" Bclass="datepicker" onChange="copy_value(this.value,'target_app_date_',<? echo $i;?>)" value="<? if($row[csf("lapdip_target_approval_date")]!="0000-00-00") echo change_date_format($row[csf("lapdip_target_approval_date")]);?>" <? echo $disable; ?> >
                    </td>
                    <td>
					...
					<td>
                        <input type="text" name="txt_pantone_no_<? echo $color_id.'_'.$i; ?>" id="txt_pantone_no_<? echo $color_id.'_'.$i; ?>" style="width:100px;" class="text_boxes" onChange="copy_value(this.value,'txt_pantone_no_',<? echo $i; ?>)" value="<? echo $row[csf("pantone_no")]; ?>"  <? echo $disable; ?>  >
                    </td>
					<td>
	...
	if($z>1)
	{
					...
					?>
						<tr align="center">
							...
							<td>
								<input type="text" name="txt_pantone_no_<? echo $color_id.'_'.$i; ?>" id="txt_pantone_no_<? echo $color_id.'_'.$i; ?>" style="width:100px;" class="text_boxes" onChange="copy_value(this.value,'txt_pantone_no_',<? echo $i; ?>)">
							</td>
							<td>
		...
		?>
			...
			<div id="content_search_panel_<? echo $color_id; ?>" style="display:none" class="accord_close">
				<table class="rpt_table" border="1" width="1175" cellpadding="0" cellspacing="0" rules="all" id="table_<? echo $color_id; ?>">
					...
					<tbody>
					<?
					$sql_color=sql_select("select a.buyer_name,b.color_from_library from variable_order_tracking b,wo_po_details_master a where a.company_name=b.company_name and a.job_no='$job_no'  and b.variable_list=23  and b.status_active=1 and b.is_deleted=0  and a.status_active=1 and a.is_deleted=0");
					...
					//$color_from_library=return_field_value("b.color_from_library as color_from_library", "variable_order_tracking b,wo_po_details_master a", "a.company_name=b.company_name and a.job_no='$job_no'  and b.variable_list=23  and b.status_active=1 and b.is_deleted=0  and a.status_active=1 and a.is_deleted=0","color_from_library");
					//echo $color_from_library.'D';
					...
					?>
						<tr align="center" title="<?=$po_number_arr[$poId];?>">
							...
							<td>
							<input type="text" name="color_id_<? echo $color_id.'_'.$i; ?>" id="color_id_<? echo $color_id.'_'.$i; ?>" value="" style="width:90px;" class="text_boxes" onBlur="check_color_name(this.value,'color_id_',<? echo $i; ?>)" <? echo $onClick.' '.$plachoder.' '.$readonly; //echo $disable; ?>>
							...
							<td>
								<input type="text" name="txt_pantone_no_<? echo $color_id.'_'.$i; ?>" id="txt_pantone_no_<? echo $color_id.'_'.$i; ?>" style="width:100px;" class="text_boxes" onChange="copy_value(this.value,'txt_pantone_no_',<? echo $i; ?>)" <? echo $disable; ?>>
							</td>
							<td>
