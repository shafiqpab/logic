...
{
	echo create_drop_down( "cbo_new_buyer_id", 150, "select buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond  and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down( 'requires/job_copy_full_controller', this.value, 'load_drop_down_season', 'season_td'); " );
	exit();
}
    ...
    <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
        <table width="840" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
            ...
            <tr class="general">
                ...
                <td>
                    <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_id').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_job_prifix').value+'_'+document.getElementById('txt_order_search').value+'_'+document.getElementById('txt_internal_ref').value+'_'+document.getElementById('cbo_year_selection').value+'_'+<? echo $garments_nature; ?>+'_'+document.getElementById('cbo_string_search_type').value, 'create_po_search_list_view', 'search_div', 'job_copy_full_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:80px;" /></td>
            </tr>
            <tr>
...
if($action=="create_po_search_list_view")
{
	...
	$gmt_nature=str_replace("'","",$data[8]);
	$internal_ref = str_replace("'","",$data[6]);
	$string_search_type=str_replace("'","",$data[9]);
	...
	$job_cond=""; $order_cond=""; $internal_ref_cond="";
	...
	$sql= "select $year_slt as year, a.job_no, a.job_no_prefix_num, a.buyer_name, a.style_ref_no, a.job_quantity, b.po_number, b.po_quantity, b.shipment_date, c.id as pre_id from wo_po_details_master a, wo_po_break_down b left join wo_pre_cost_mst c on b.job_no_mst=c.job_no and c.status_active=1 and c.is_deleted=0 where a.job_no=b.job_no_mst and a.status_active=1 and b.status_active=1 $gmt_nature_cond $shipment_date $company $buyer $job_cond $order_cond $internal_ref_cond order by a.id DESC";
...
if($action=="save_update_delete_copy_job")
{
	...
	if(str_replace("'","",$hiddn_wsdata)!="")
	{
		...
		$gmts_item_id_file=str_replace("'","",$cbo_gmtsItem_id); 
		else $gmts_item_id_file='gmts_item_id';
	...
	$sql_jobInst="insert into wo_po_details_master( id, garments_nature, job_no_prefix, job_no_prefix_num, job_no, quotation_id, copy_from, order_repeat_no, company_name, buyer_name, style_ref_no, product_dept, product_code, location_name, style_description, ship_mode, region, team_leader, dealing_marchant, remarks, job_quantity, avg_unit_price, currency_id, total_price, packing, agent_name, product_category, order_uom,  gmts_item_id, set_break_down, total_set_qnty, set_smv, pro_sub_dep, client_id, item_number_id, factory_marchant, season_buyer_wise, qlty_label, is_excel, style_owner, booking_meeting_date, bh_merchant, ready_for_budget, working_location_id, gauge, fabric_composition, design_source_id, yarn_quality, season_year, brand_id, inquiry_id, body_wash_color, sustainability_standard, fab_material, quality_level, requisition_no, is_deleted, status_active, inserted_by, insert_date)
	select
	$id, garments_nature, '".$new_job_no[1]."', '".$new_job_no[2]."', '".$new_job_no[0]."', 0 as quotation_id, job_no, order_repeat_no, $company_name_file, $buyer_name_file, $style_ref_no_file, product_dept, product_code, location_name, style_description, ship_mode, region, team_leader, dealing_marchant, remarks, job_quantity, avg_unit_price, currency_id, total_price, packing,  agent_name, product_category, order_uom,  $gmts_item_id_file, $set_break_down_file, total_set_qnty, $set_smv_file, pro_sub_dep, client_id, item_number_id, factory_marchant, $season_buyer_wise_file, qlty_label, is_excel, style_owner, booking_meeting_date, bh_merchant, ready_for_budget, working_location_id, gauge, fabric_composition, design_source_id, yarn_quality, season_year, brand_id, inquiry_id, body_wash_color, sustainability_standard, fab_material, quality_level, requisition_no, is_deleted, status_active, ".$_SESSION['logic_erp']['user_id'].", '".$pc_date_time."' from wo_po_details_master where job_no=$txt_job_no"; //Don't hide 0 as quotation_id issue id ISD-21-01768 comments by kausar
	...
	foreach($sql_se_po as $row_se_po)
	{
		...
		if(str_replace("'","",$copy_int_ref)==1){			
			$sql_poInst="insert into  wo_po_break_down(id, job_id, job_no_mst, po_number, pub_shipment_date, excess_cut, po_received_date, po_quantity, unit_price, plan_cut, country_name, po_total_price, shipment_date, t_year, t_month, is_confirmed, details_remarks, delay_for, packing, projected_po_id, tna_task_from_upto, shiping_status, original_po_qty, factory_received_date, original_avg_price, pp_meeting_date, file_no_prev, file_no, matrix_type, no_of_carton, actual_po_no, round_type, doc_sheet_qty, up_charge, pack_price, sc_lc, extended_ship_date, sewing_company_id, sewing_location_id, extend_ship_mode, sea_discount, air_discount, pack_handover_date, txt_etd_ldd, file_year, status_active, is_deleted, inserted_by, insert_date)
			select $id_po, $job_id, '".$new_job_no[0]."', po_number, pub_shipment_date, excess_cut, po_received_date, po_quantity, unit_price, plan_cut, country_name, po_total_price, shipment_date, t_year, t_month, is_confirmed, details_remarks, delay_for, packing, projected_po_id, tna_task_from_upto, 1, original_po_qty, factory_received_date, original_avg_price, pp_meeting_date, file_no_prev, file_no, matrix_type, no_of_carton, actual_po_no, round_type, doc_sheet_qty, up_charge, pack_price, sc_lc, extended_ship_date, sewing_company_id, sewing_location_id, extend_ship_mode, sea_discount, air_discount, pack_handover_date, txt_etd_ldd, file_year, status_active, is_deleted, ".$_SESSION['logic_erp']['user_id'].", '".$pc_date_time."' from wo_po_break_down where job_no_mst=$txt_job_no and id=".$row_se_po[csf('id')]." and is_deleted=0 and status_active=1";
		}
		else{
	...
	else if($db_type==2 || $db_type==1 )
	{
		...
		// echo "10**select a.id as po_id, b.color_number_id, min(b.id) as color_size_table_id from  wo_po_break_down a, wo_po_color_size_breakdown b where a.job_no_mst=b.job_no_mst and a.job_no_mst='$txt_job_no' and b.color_mst_id !=0 and a.id=b.po_break_down_id and  b.po_break_down_id='$order_id' and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 group by a.id,b.color_number_id order by a.id";die;
		...
		// {
			...
			//echo "10**"."insert into wo_po_lapdip_approval_info (".$field_array_lap.") values ".$data_array_lap; die;
		// }
	...
	//txt_costing_date
	if (is_duplicate_field( "job_no", "wo_pre_cost_mst", "job_no='".$new_job_no[0]."' and is_deleted=0 and status_active=1" ) == 0)
	{
		...
		$id_pre_dtls, $job_id, '".$new_job_no[0]."', costing_per_id, order_uom_id, fabric_cost, fabric_cost_percent, trims_cost, trims_cost_percent, embel_cost, embel_cost_percent, wash_cost, wash_cost_percent, comm_cost, comm_cost_percent, commission, commission_percent, lab_test, lab_test_percent, inspection, inspection_percent, cm_cost, cm_cost_percent, freight, freight_percent, common_oh, common_oh_percent, total_cost, total_cost_percent, price_dzn, price_dzn_percent, margin_dzn, margin_dzn_percent, cost_pcs_set, cost_pcs_set_percent, price_pcs_or_set, price_pcs_or_set_percent, margin_pcs_set, margin_pcs_set_percent, cm_for_sipment_sche, currier_pre_cost, currier_percent, certificate_pre_cost, certificate_percent, depr_amor_pre_cost, depr_amor_po_price, interest_cost, interest_percent, incometax_cost, incometax_percent, deffdlc_cost, deffdlc_percent, design_cost, design_percent, studio_cost, studio_percent, ".$_SESSION['logic_erp']['user_id'].", '".$pc_date_time."', status_active, is_deleted from wo_pre_cost_dtls where job_no=$txt_job_no and is_deleted=0";
			...
			foreach($sql_se_fabric as $row_se_fabric)
			{
				...
				$sql_insert_fabric="insert into  wo_pre_cost_fabric_cost_dtls (id, job_id, job_no, item_number_id, body_part_id, fab_nature_id, color_type_id, lib_yarn_count_deter_id, construction, composition, fabric_description, gsm_weight, color_size_sensitive, color, avg_cons, fabric_source, rate, amount, avg_finish_cons, avg_process_loss, company_id, costing_per, consumption_basis, process_loss_method, cons_breack_down, msmnt_break_down, color_break_down, yarn_breack_down, marker_break_down, width_dia_type, avg_cons_yarn, gsm_weight_yarn, plan_cut_qty, job_plan_cut_qty, is_apply_last_update, uom, body_part_type, sample_id, gsm_weight_type, nominated_supp_multi, quotdtlsid, budget_on, source_id,seq, inserted_by, insert_date, status_active, is_deleted)
				select $wo_pre_cost_fabric_cost_dtls_id, $job_id, '".$new_job_no[0]."', $item_number_id_fill, body_part_id, fab_nature_id, color_type_id, lib_yarn_count_deter_id, construction, composition, fabric_description, gsm_weight, color_size_sensitive, color, avg_cons, fabric_source, rate, amount, avg_finish_cons, avg_process_loss, company_id, costing_per, consumption_basis, process_loss_method, cons_breack_down, msmnt_break_down, color_break_down, yarn_breack_down, marker_break_down, width_dia_type, avg_cons_yarn, gsm_weight_yarn, plan_cut_qty, job_plan_cut_qty, is_apply_last_update, uom, body_part_type, sample_id, gsm_weight_type, nominated_supp_multi, quotdtlsid, budget_on, source_id,seq, ".$_SESSION['logic_erp']['user_id'].", '".$pc_date_time."', status_active, is_deleted from wo_pre_cost_fabric_cost_dtls where job_no=$txt_job_no and id=".$row_se_fabric[csf('id')]." and is_deleted=0";
			...
			foreach($sql_color_size_table_id as $row_color_size_table_id)
			{
				...
				//echo '10**'.$row_color_size_table_id[csf('color_size_table_id')].'='.$row_color_size_table_id[csf('id')].'='.$row_color_size_table_id[csf('item_number_id')].'='.$row_color_size_table_id[csf('color_number_id')].'='.$row_color_size_table_id[csf('size_number_id')].'=AA<br>';
			}
		  ...
		  $sql_insert_fabric_color="insert into wo_pre_cos_fab_co_color_dtls(id, job_id, pre_cost_fabric_cost_dtls_id, job_no, gmts_color_id, gmts_color, contrast_color_id)
			...
			foreach($sql_stripe_color as $row_stripe_color)
			{
				$wo_pre_cos_stripe_id=return_next_id( "id", "wo_pre_stripe_color", 1) ;
		  		$sql_insert_stripe_color="insert into wo_pre_stripe_color (id, job_id, job_no, item_number_id, pre_cost_fabric_cost_dtls_id, color_number_id, stripe_color, measurement, uom, inserted_by, insert_date, status_active, is_deleted, totfidder, fabreq, fabreqtotkg, yarn_dyed, sales_dtls_id, size_number_id, po_break_down_id, lenth, width, sample_color, sample_per, cons, excess_per, stripe_type)
				select $wo_pre_cos_stripe_id, $job_id, '".$new_job_no[0]."', $item_number_id_fill, ".$fabric_cost_id_maping[$row_stripe_color[csf('pre_cost_fabric_cost_dtls_id')]].",  color_number_id, stripe_color, measurement, uom, ".$_SESSION['logic_erp']['user_id'].", '".$pc_date_time."', status_active, is_deleted, totfidder, fabreq, fabreqtotkg, yarn_dyed, sales_dtls_id, size_number_id, po_break_down_id, lenth, width, sample_color, sample_per, cons, excess_per, stripe_type from  wo_pre_stripe_color where job_no=$txt_job_no and id=".$row_stripe_color[csf('id')]." and status_active=1 and is_deleted=0";
			...
			foreach($sql_se_fabric_conver as $row_se_fabric_conver)
			{
				...
				$conv_id_maping[$row_se_fabric_conver[csf('id')]]=$wo_pre_cost_fab_conv_cost_dtls_id;
			}
		    if($db_type==0)
			{
				if($rID12){
			...
			$sql_conv_color=sql_select("select id, conv_cost_dtls_id, fabric_cost_dtls_id from wo_pre_cos_conv_color_dtls where job_no=$txt_job_no order by id ASC");
			foreach($sql_conv_color as $row_conv_color)
			{
				$bomconvcolor_id=return_next_id( "id", "wo_pre_cos_conv_color_dtls", 1) ;
		  		$sql_insert_conv_color="insert into wo_pre_cos_conv_color_dtls (id, fabric_cost_dtls_id, conv_cost_dtls_id, job_no, job_id, convchargelibraryid, gmts_color_id, fabric_color_id, cons, unit_charge, inserted_by, insert_date, status_active, is_deleted)
				select $bomconvcolor_id, '".$fabric_cost_id_maping[$row_conv_color[csf('fabric_cost_dtls_id')]]."','".$conv_id_maping[$row_conv_color[csf('conv_cost_dtls_id')]]."','".$new_job_no[0]."', $job_id,  convchargelibraryid, gmts_color_id, fabric_color_id, cons, unit_charge, ".$_SESSION['logic_erp']['user_id'].", '".$pc_date_time."', status_active, is_deleted from  wo_pre_cos_conv_color_dtls where job_no=$txt_job_no and id=".$row_conv_color[csf('id')]." and status_active=1 and is_deleted=0";
				//echo "10**".$sql_insert_conv_color; die;
				$rIDconv=execute_query($sql_insert_conv_color,0);
			}
		    if($db_type==0)
			{
				if($rIDconv){
			...
			foreach($sql_se_trim_cons as $row_se_trim_cons)
			{
		        ...
		        $sql_insert_fabric_trim_avg="insert into wo_pre_cost_trim_co_cons_dtls (id, job_id, wo_pre_cost_trim_cost_dtls_id, job_no, po_break_down_id, item_size, cons, place, pcs, country_id, excess_per, tot_cons, ex_cons, cons_pcs, item_number_id, color_number_id, item_color_number_id, size_number_id, rate, amount, gmts_pcs, color_size_table_id)
				select $wo_pre_cost_trim_co_cons_dtls_id, $job_id, ".$trim_table_id_maping[$row_se_trim_cons[csf('wo_pre_cost_trim_cost_dtls_id')]].", '".$new_job_no[0]."', ".$po_id_maping_array[$row_se_trim_cons[csf('po_break_down_id')]].", item_size, cons, place, pcs, country_id, excess_per, tot_cons, ex_cons, cons_pcs, $item_number_id_fill, color_number_id, item_color_number_id, size_number_id, rate, amount, gmts_pcs, '".$color_size_table_id."' from wo_pre_cost_trim_co_cons_dtls where job_no=$txt_job_no and id=".$row_se_trim_cons[csf('id')]."";
			...
			foreach($sql_se_embe_wash as $row_se_embe_wash)
			{
		        ...
		        $sql_insert_embe_wash="insert into wo_pre_cost_embe_cost_dtls (id, job_id, job_no, emb_name, emb_type, cons_dzn_gmts, rate, amount, charge_lib_id, budget_on, nominated_supp_multi, country, body_part_id, inserted_by, insert_date, status_active, is_deleted)
				select $embe_cost_dtls_id, $job_id, '".$new_job_no[0]."', emb_name, emb_type, cons_dzn_gmts, rate, amount, charge_lib_id, budget_on, nominated_supp_multi, country, body_part_id, ".$_SESSION['logic_erp']['user_id'].", '".$pc_date_time."', status_active, is_deleted from wo_pre_cost_embe_cost_dtls where job_no=$txt_job_no and id=".$row_se_embe_wash[csf('id')]." and is_deleted=0";
...
if($action=="create_item_smv_search_list_view")
{
	...
	$sql_result = sql_select("select variable_list, season_mandatory, publish_shipment_date from variable_order_tracking where company_name=$company and variable_list in (47) and status_active=1 and is_deleted=0 order by variable_list ASC");
	$set_smv_id=0;
 	foreach($sql_result as $result)
	{
		if($result[csf('variable_list')]==47) $set_smv_id=$result[csf('publish_shipment_date')];
	}
	$bulletin_type_cond="";
	if($set_smv_id>0 && $set_smv_id!=''){
		if($set_smv_id==3 || $set_smv_id==4){
			$bulletin_type_cond="and a.bulletin_type=3";
		}
		if($set_smv_id==6 || $set_smv_id==8 || $set_smv_id==9){
			$bulletin_type_cond="and a.bulletin_type=2";
		}		
	}
	if($db_type==0)
	{
		$itemCond="group_concat(gmts_item_id) as gmts_item_id";
	}
	else
	{
		$itemCond="listagg(CAST(gmts_item_id as VARCHAR(4000)),',') within group (order by gmts_item_id) as gmts_item_id";
	}
	
	if($job_no!="" && $item_id!=0) { 
	//For Urmi 
//$gmtsItem=return_field_value("gmts_item_id","wo_po_details_master","style_ref_no='$style' and is_deleted=0 and status_active=1 order by id desc");
$sql_item=sql_select("select $itemCond from wo_po_details_master where style_ref_no='$style'  and is_deleted=0 and status_active=1 order by id desc");
//echo "select $itemCond from wo_po_details_master where style_ref_no='$style'  and is_deleted=0 and status_active=1 order by id desc"; 
	foreach($sql_item as $row)
	{
		$gmtsItem=$row[csf('gmts_item_id')];
	}
	...
	$sql="select a.id, a.system_no, a.color_type, a.extention_no, a.style_ref, a.operation_count, a.gmts_item_id, b.operator_smv, b.helper_smv, b.body_part_id, b.lib_sewing_id, c.department_code, a.bulletin_type from ppl_gsd_entry_mst a, ppl_gsd_entry_dtls b, lib_sewing_operation_entry c where a.id=b.mst_id and b.lib_sewing_id=c.id and a.approved=1 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $gmts_item_con $style_con $buyer_id_con $bulletin_type_cond and rownum = 1
	order by a.id DESC";
	//echo $sql; die;
	...
	foreach($sql_result as $row)
	{
		...
		$smv_dtls_arr[$row[csf('gmts_item_id')]][$row[csf('system_no')]][$row[csf('extention_no')]]['bulletin_type']=$row[csf('bulletin_type')];
		...
		foreach($smv_dtls_arr as $gmtsitem=>$gmtsitemdata)
		{
			foreach ($gmtsitemdata as $sysno => $systemdata)
			{
				foreach($systemdata as $ext_no=>$arrdata)
				{
					...
					?>
					<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>" style="cursor:pointer" onClick="js_set_value('<? echo $i; ?>')">
	                    ...
	                    <td width="40" style="word-break:break-all"><? echo $bulletin_type_arr[$arrdata['bulletin_type']]; ?></td>
