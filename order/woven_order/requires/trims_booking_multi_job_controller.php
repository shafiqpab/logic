...
function load_drop_down_supplier($data){
	if($data==5 || $data==3){
	   echo create_drop_down( "cbo_supplier_name", 172, "select id,company_name from lib_company where status_active=1 and is_deleted=0 order by company_name", "id,company_name",1, "-- Select Company --", "", "get_php_form_data( this.value, 'load_drop_down_attention', 'requires/trims_booking_multi_job_controller');",0,"" );
	}
	else
	{
	$cbo_supplier_name= create_drop_down( "cbo_supplier_name", 172, "select c.supplier_name,c.id from lib_supplier_tag_company a, lib_supplier_party_type b, lib_supplier c where c.id=b.supplier_id and a.supplier_id = b.supplier_id  and b.party_type=4 and c.status_active=1 and c.is_deleted=0 group by c.id, c.supplier_name order by c.supplier_name","id,supplier_name", 1, "--Select Supplier--",$selected,"get_php_form_data( this.value, 'load_drop_down_attention', 'requires/trims_booking_multi_job_controller');","");
	}
...
if ($action=="populate_variable_setting_data")
{
	...
	$print_report_format=return_field_value("format_id","lib_report_template","template_name ='".$data."' and module_id=2 and report_id in(25) and is_deleted=0 and status_active=1");
            	...
            	<table  cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
        			...
        			<tr>
                        ...
                        <input type="hidden" id="garments_nature" value="<? echo $garments_nature; ?>">
							<? 
								echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", '',"load_drop_down( 'trims_booking_multi_job_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
							?>
                    </td>
                   	<td id="buyer_td">
                     <? 
...
{
	$data_job=explode(",",$data);
	$job_id='';
	foreach($data_job as $job)
	{
	if($job_id=='')	$job_id="'".$job."'"; else $job_id.=","."'".$job."'";
	}
	$data_array=sql_select("select job_no,company_name,buyer_name,currency_id from wo_po_details_master where job_no in($job_id)");
	foreach ($data_array as $row)
	{
		$data_req='';
		foreach($data_job as $job_no)
		{
			$data_req=total_cu_data($job_no);	
		}
		...
		//echo "load_drop_down( 'requires/trims_booking_multi_job_controller', '".$row[csf("company_name")]."', 'load_drop_down_buyer', 'buyer_td' );\n";
		...
		//echo "get_php_form_data(".$row[csf("company_name")].", 'populate_variable_setting_data', 'requires/trims_booking_multi_job_controller' );\n";
...
if ($action=="fnc_process_data") //1st Item
{
        ...
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1295" class="rpt_table"  >
            ...
            <?
				   ...
				   $sql="select
					...
					c.id=e.wo_pre_cost_trim_cost_dtls_id and
					d.id=e.po_break_down_id and 
					a.company_name=$company_id $shipment_date and
					(c.nominated_supp = $cbo_supplier_name or c.nominated_supp= 0)  
					and b.approved=1
					and
					d.is_deleted=0 and 
					d.status_active=1
                    ...
                    <td width="100"><div style="width:100px; word-wrap:break-word;"><? echo $trim_group[$selectResult[csf('trim_group')]];?></div></td>
                    ...
                    <td width="80" align="right"><? echo def_number_format($selectResult[csf('cu_woq')],2,"");?></td>
                    <td width="80" align="right">
					<?
					echo number_format($bal_woq,2);
					?>
                    </td>
                    ...
                    <?
					$amount=def_number_format($rate*$bal_woq,2,"");
...
//if($data[4]==2) $cbo_order_status="%%"; else $cbo_order_status= "$data[4]";
	 ...
	 {
		 $garments_nature_cond="";
		
	 }
	 else
	 {
		 $garments_nature_cond=" and a.garments_nature=$garments_nature"; 
	 }
...
<input type="hidden" name="txt_job_id" id="txt_job_id" value="" />
        ...
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="550" class="rpt_table" >
            ...
            <?
					...
					where
					a.job_no=d.job_no_mst and 
					a.company_name=$company_id $garments_nature_cond $cbo_buyer_name_cond $shipment_date and
                    ...
                    <input type="hidden" name="txt_individual_id" id="txt_individual_id<?php echo $i ?>" value="<? echo $selectResult[csf('po_id')]; ?>"/>
                     ...
                     <input type="hidden" name="txt_individual_job" id="txt_individual_job<?php echo $i ?>" value="<? echo $selectResult[csf('job_no')]; ?>"/>
                    </td>
                    <td width="100">
...
if ($action=="generate_fabric_booking")
{
   ...
   <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1230" class="rpt_table" >
            ...
            <div style="width:1230px; overflow-y:scroll; max-height:350px;" id="buyer_list_view" align="center">
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1213" class="rpt_table" id="tbl_list_search" >
       <tbody>
       <?
...
CASE e.costing_per WHEN 1 THEN round(((d.cons/12)*b.po_quantity)/cc.conversion_factor,4) WHEN 2 THEN round(((d.cons/1)*b.po_quantity)/cc.conversion_factor,4)  WHEN 3 THEN round(((d.cons/24)*b.po_quantity)/cc.conversion_factor,4) WHEN 4 THEN round(((d.cons/36)*b.po_quantity)/cc.conversion_factor,4) WHEN 5 THEN round(((d.cons/48)*b.po_quantity)/cc.conversion_factor,4) ELSE 0 END as req_qnty,cc.order_uom as cons_uom,IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0)  as cu_woq, CASE e.costing_per WHEN 1 THEN round((((d.cons/12)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 2 THEN round((((d.cons/1)*b.po_quantity)/cc.conversion_factor) - IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4)  WHEN 3 THEN round((((d.cons/24)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 4 THEN round((((d.cons/36)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 5 THEN round((((d.cons/48)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) ELSE 0 END as bal_woq, round((c.rate*cc.conversion_factor),8) as rate from wo_po_details_master a, wo_po_break_down b ,wo_pre_cost_mst e,wo_pre_cost_trim_cost_dtls c ,lib_item_group cc, wo_pre_cost_trim_co_cons_dtls d left join wo_booking_dtls f on f.job_no=d.job_no and f.pre_cost_fabric_cost_dtls_id=d.wo_pre_cost_trim_cost_dtls_id and f.po_break_down_id=d.po_break_down_id and f.booking_type=2   where a.job_no=b.job_no_mst and  a.job_no=c.job_no and a.job_no=e.job_no and  a.job_no=d.job_no   and c.id=d.wo_pre_cost_trim_cost_dtls_id and b.id=d.po_break_down_id and cc.id=c.trim_group  and a.company_name=$cbo_company_name $txt_job_no_cond $cbo_buyer_name_cond and a.garments_nature=$garments_nature group by d.id order by b.id) m   where id in($data) $txt_job_no_cond1  $cbo_buyer_name_cond1 and garments_nature=$garments_nature order by trim_group";//and b.pub_shipment_date between '$start_date' and '$end_date'*/ 
					...
					c.id=e.wo_pre_cost_trim_cost_dtls_id and
					d.id=e.po_break_down_id and 
					a.company_name=$cbo_company_name  $cbo_buyer_name_cond $garment_nature_cond and
                    ...
                    foreach ($nameArray as $selectResult)
                    {
						...
						$job_and_trimgroup_level[$selectResult[csf('job_no')]][$selectResult[csf('wo_pre_cost_trim_cost_dtls')]]['country'][$selectResult[csf('po_id')]]=$country_id_string;
						...
						$job_and_trimgroup_level[$selectResult[csf('job_no')]][$selectResult[csf('wo_pre_cost_trim_cost_dtls')]]['req_qnty'][$selectResult[csf('po_id')]]=$req_qnty;
						$job_and_trimgroup_level[$selectResult[csf('job_no')]][$selectResult[csf('wo_pre_cost_trim_cost_dtls')]]['req_amount'][$selectResult[csf('po_id')]]=$req_amount;
                        $job_and_trimgroup_level[$selectResult[csf('job_no')]][$selectResult[csf('wo_pre_cost_trim_cost_dtls')]]['cu_amount'][$selectResult[csf('po_id')]]=$req_amount;						
						$job_and_trimgroup_level[$selectResult[csf('job_no')]][$selectResult[csf('wo_pre_cost_trim_cost_dtls')]]['cu_woq'][$selectResult[csf('po_id')]]=$selectResult[csf('cu_woq')];
						 $job_and_trimgroup_level[$selectResult[csf('job_no')]][$selectResult[csf('wo_pre_cost_trim_cost_dtls')]]['cu_amount'][$selectResult[csf('po_id')]]=$selectResult[csf('cu_amount')];				
						...
						$job_and_trimgroup_level[$selectResult[csf('job_no')]][$selectResult[csf('wo_pre_cost_trim_cost_dtls')]]['rate'][$selectResult[csf('po_id')]]=$rate;
       ...
       <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>">
                    ...
                    <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtwoq_<? echo $i;?>" value="<? echo $bal_woq;?>" onClick="open_consumption_popup('requires/trims_booking_multi_job_controller.php?action=consumption_popup', 'Consumtion Entry Form','txtpoid_<? echo $i;?>',<? echo $i;?>)" />
                    </td>
                    <td width="80" align="right">
    ...
    <table width="1230" class="rpt_table" border="0" rules="all">
    <tfoot>
    <tr>
            <th width="40">SL</th>
            <th width="100"></th>
...
CASE e.costing_per WHEN 1 THEN round(((d.cons/12)*b.po_quantity)/cc.conversion_factor,4) WHEN 2 THEN round(((d.cons/1)*b.po_quantity)/cc.conversion_factor,4)  WHEN 3 THEN round(((d.cons/24)*b.po_quantity)/cc.conversion_factor,4) WHEN 4 THEN round(((d.cons/36)*b.po_quantity)/cc.conversion_factor,4) WHEN 5 THEN round(((d.cons/48)*b.po_quantity)/cc.conversion_factor,4) ELSE 0 END as req_qnty,cc.order_uom as cons_uom,IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0)  as cu_woq, CASE e.costing_per WHEN 1 THEN round((((d.cons/12)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 2 THEN round((((d.cons/1)*b.po_quantity)/cc.conversion_factor) - IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4)  WHEN 3 THEN round((((d.cons/24)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 4 THEN round((((d.cons/36)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 5 THEN round((((d.cons/48)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) ELSE 0 END as bal_woq, round((c.rate*cc.conversion_factor),8) as rate from wo_po_details_master a, wo_po_break_down b ,wo_pre_cost_mst e,wo_pre_cost_trim_cost_dtls c ,lib_item_group cc, wo_pre_cost_trim_co_cons_dtls d left join wo_booking_dtls f on f.job_no=d.job_no and f.pre_cost_fabric_cost_dtls_id=d.wo_pre_cost_trim_cost_dtls_id and f.po_break_down_id=d.po_break_down_id and f.booking_type=2   where a.job_no=b.job_no_mst and  a.job_no=c.job_no and a.job_no=e.job_no and  a.job_no=d.job_no   and c.id=d.wo_pre_cost_trim_cost_dtls_id and b.id=d.po_break_down_id and cc.id=c.trim_group  and a.company_name=$cbo_company_name $txt_job_no_cond $cbo_buyer_name_cond and a.garments_nature=$garments_nature group by d.id order by b.id) m   where id in($data) $txt_job_no_cond1  $cbo_buyer_name_cond1 and garments_nature=$garments_nature order by trim_group";//and b.pub_shipment_date between '$start_date' and '$end_date'*/ 
				...
				/*	$sql_lib_item_group_array=array();
					...
					a.job_no=d.job_no_mst and 
					a.job_no=e.job_no and 
					a.company_name=$cbo_company_name  $cbo_buyer_name_cond $po_id_cond $garments_nature_cond
					and 
					d.is_deleted=0 and 
					d.status_active=1
					order by d.id";		 
       ...
       <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>">
                    ...
                    <input type="hidden" id="txtbookingid_<? echo $i;?>" value="" readonly/>
                    <input type="hidden" id="txtpoid_<? echo $i;?>" value="<? echo $selectResult[csf('po_id')];?>" readonly/>
                     <input type="hidden" id="txtjob_<? echo $i;?>" value="<? echo $selectResult[csf('job_no')];?>" readonly/>
                    <input type="hidden" id="txtcountry_<? echo $i;?>"  value="<? echo $country_id_string ?>" readonly />
                    ...
                    <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtwoq_<? echo $i;?>" value="" onClick="open_consumption_popup('requires/trims_booking_multi_job_controller.php?action=consumption_popup', 'Consumtion Entry Form','txtpoid_<? echo $i;?>',<? echo $i;?>)"     />
                    </td>
                    <td width="80" align="right">
                     <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtexchrate_<? echo $i;?>" value="<? echo $exchange_rate;?>" readonly />
...
if ($action=="show_trim_booking")
{
	...
	if($data_job=="")
	 ...
	 {
		 $txt_job_no_cond ="and a.job_no in($job_id)";
		 //$txt_job_no_cond1 ="and job_no='$txt_job_no'";
	 }
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1290" class="rpt_table" >
       ...
       <?
					...
					c.id=e.wo_pre_cost_trim_cost_dtls_id and
					d.id=e.po_break_down_id and 
					a.company_name=$cbo_company_name $txt_job_no_cond $cbo_buyer_name_cond $garment_nature_cond and
       ...
       <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>">
                    ...
                    <td width="80" align="right">
                    <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtwoq_<? echo $i;?>" value="<? echo $selectResult[csf('wo_qnty')];?>" onClick="open_consumption_popup('requires/trims_booking_multi_job_controller.php?action=consumption_popup', 'Consumtion Entry Form','txtpoid_<? echo $i;?>',<? echo $i;?>)"     />
                    </td>
                     <td width="80" align="right">
...
if ($action=="show_trim_booking_without_precost")
{
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="880" class="rpt_table" >
       ...
       <tbody>
       <?
					$sql="select id, job_no, po_break_down_id, booking_no, booking_type, trim_group, uom, sensitivity, wo_qnty, exchange_rate, rate, amount, cons_break_down, delivery_date from  wo_booking_dtls where booking_no=$txt_booking_no";
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$field_array="id, booking_type, is_short, booking_month, booking_year, booking_no_prefix, booking_no_prefix_num, booking_no, company_id, buyer_id, job_no, 	item_category, supplier_id, currency_id, exchange_rate, booking_date, delivery_date, pay_mode, source, attention, tenor, remarks, item_from_precost, entry_form, inserted_by, insert_date";
		
		 $data_array ="(".$id.",2,2,".$cbo_booking_month.",".$cbo_booking_year.",'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',".$cbo_company_name.",".$cbo_buyer_name.",".$txt_job_no.",4,".$cbo_supplier_name.",".$cbo_currency.",".$txt_exchange_rate.",".$txt_booking_date.",".$txt_delivery_date.",".$cbo_pay_mode.",".$cbo_source.",".$txt_attention.",".$txt_tenor.",".$txt_remarks.",".$cbo_item_from_precost.",261,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		 ...
		 $field_array1="id, pre_cost_fabric_cost_dtls_id, po_break_down_id, job_no, booking_no,booking_mst_id, booking_type, is_short, trim_group, uom, sensitivity, wo_qnty, exchange_rate, rate, amount, delivery_date, cons_break_down, country_id_string, inserted_by, insert_date";
		...
		$field_array2="id, wo_trim_booking_dtls_id, booking_no, job_no, po_break_down_id, color_number_id, gmts_sizes, description, brand_supplier, item_color, item_size, cons, process_loss_percent, requirment, rate, amount, pcs, color_size_table_id";
		 ...
		 $new_array_color=array(); $data_array2="";
		 for ($i=1;$i<=$total_row;$i++)
		 {
			...
			// $txtcuwoq="txtcuwoq_".$i;
			 ...
			 $txtjob_id="txtjob_".$i;
			 //if ($i!=1) $data_array1 .=",";
			 $data_array1 ="(".$id_dtls.",".$$txttrimcostid.",".$$txtpoid.",".$$txtjob_id.",'".$new_booking_no[0]."',".$id.",2,2,".$$txttrimgroup.",".$$txtuom.",".$$cbocolorsizesensitive.",".$$txtwoq.",".$$txtexchrate.",".$$txtrate.",".$$txtamount.",".$$txtddate.",".$$consbreckdown.",".$$txtcountry.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			 ...
			 //	CONS break down===============================================================================================	
				if(str_replace("'",'',$$consbreckdown) !='')
				{
					...
					for($c=0;$c < count($consbreckdown_array);$c++)
					{
						...
						if(str_replace("'","",$consbreckdownarr[4])!="")
			 			{
							 ...
							 {
								  $color_id = return_id( str_replace("'","",$consbreckdownarr[4]), $color_library, "lib_color", "id,color_name","261");  
						...
						if ($c!=0) $data_array2 .=",";
						$data_array2 .="(".$id1.",".$id_dtls.",'".$new_booking_no[0]."',".$$txtjob_id.",".$$txtpoid.",'".$consbreckdownarr[0]."','".$consbreckdownarr[1]."','".$consbreckdownarr[2]."','".$consbreckdownarr[3]."','".$color_id."','".$consbreckdownarr[5]."','".$consbreckdownarr[6]."','".$consbreckdownarr[7]."','".$consbreckdownarr[8]."','".$consbreckdownarr[9]."','".$consbreckdownarr[10]."','".$consbreckdownarr[11]."','".$consbreckdownarr[12]."')";
						$id1=$id1+1;
						$add_comma++;
					}
				}
	...
	/*	}else{*/
		 ...
		 $field_array_up="booking_month*booking_year*buyer_id*job_no*item_category*supplier_id*currency_id*exchange_rate*booking_date*delivery_date*pay_mode*source*attention*tenor*remarks*item_from_precost*entry_form*updated_by*update_date"; 
		 
		 $data_array_up ="".$cbo_booking_month."*".$cbo_booking_year."*".$cbo_buyer_name."*".$txt_job_no."*4*".$cbo_supplier_name."*".$cbo_currency."*".$txt_exchange_rate."*".$txt_booking_date."*".$txt_delivery_date."*".$cbo_pay_mode."*".$cbo_source."*".$txt_attention."*".$txt_tenor."*".$txt_remarks."*".$cbo_item_from_precost."*261*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		// if  ( check_table_status( $_SESSION['menu_id'], 1 )==0 ) { echo "15**1"; die;}		
		 $field_array_up1="pre_cost_fabric_cost_dtls_id*po_break_down_id*job_no*booking_no*trim_group*uom*sensitivity*wo_qnty*exchange_rate*rate*amount*delivery_date*cons_break_down*country_id_string*updated_by*update_date";
		  $id_dtls=return_next_id( "id", "wo_booking_dtls", 1 ) ;
		  $field_array1="id,pre_cost_fabric_cost_dtls_id,po_break_down_id,job_no,booking_no,booking_mst_id,booking_type,is_short,trim_group,uom,sensitivity,wo_qnty,exchange_rate,rate,amount,delivery_date,cons_break_down,country_id_string,inserted_by,insert_date";
		 //$field_array2="id,wo_trim_booking_dtls_id,booking_no,job_no,po_break_down_id,color_number_id,gmts_sizes,description,brand_supplier,item_color,item_size,cons, process_loss_percent,requirment,rate,amount,pcs,color_size_table_id";
		 $field_array_up2="id,wo_trim_booking_dtls_id,color_number_id,gmts_sizes,description,brand_supplier,item_color,item_size,cons, process_loss_percent,requirment,rate,amount,pcs,color_size_table_id,booking_no,job_no,po_break_down_id";
		 
		  $add_comma=0; //echo '10**';
		  $id1=return_next_id( "id", "wo_trim_book_con_dtls", 1 );
		  $new_array_color=array(); $rID1=1; $rID2=1; $data_array_up2="";
		 for ($i=1;$i<=$total_row;$i++)
		 {
			 $txttrimcostid="txttrimcostid_".$i;
			 $txtpoid="txtpoid_".$i;
			...
			if(str_replace("'",'',$$txtbookingid)!="")
			{
				...
				$data_array_up1[str_replace("'",'',$$txtbookingid)] =explode("*",("".$$txttrimcostid."*".$$txtpoid."*".$$txtjob_id."*".$txt_booking_no."*".$$txttrimgroup."*".$$txtuom."*".$$cbocolorsizesensitive."*".$$txtwoq."*".$$txtexchrate."*".$$txtrate."*".$$txtamount."*".$$txtddate."*".$$consbreckdown."*".$$txtcountry."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
				...
				if(str_replace("'",'',$$consbreckdown) !='')
				{
					...
					for($c=0;$c < count($consbreckdown_array);$c++)
					{
						...
						$data_array_up2 .="(".$id1.",".$$txtbookingid.",'".$consbreckdownarr[0]."','".$consbreckdownarr[1]."','".$consbreckdownarr[2]."','".$consbreckdownarr[3]."','".$color_id."','".$consbreckdownarr[5]."','".$consbreckdownarr[6]."','".$consbreckdownarr[7]."','".$consbreckdownarr[8]."','".$consbreckdownarr[9]."','".$consbreckdownarr[10]."','".$consbreckdownarr[11]."','".$consbreckdownarr[12]."',".$txt_booking_no.",".$$txtjob_id.",".$$txtpoid.")";
						$id1=$id1+1;
						$add_comma++;
					}
				}
			...
			else if(str_replace("'",'',$$txtbookingid)=="")
			{
				 //if ($i!=1) $data_array1 .=",";
				 $data_array1 ="(".$id_dtls.",".$$txttrimcostid.",".$$txtpoid.",".$$txtjob_id.",".$txt_booking_no.",".$booking_id.",2,2,".$$txttrimgroup.",".$$txtuom.",".$$cbocolorsizesensitive.",".$$txtwoq.",".$$txtexchrate.",".$$txtrate.",".$$txtamount.",".$$txtddate.",".$$consbreckdown.",".$$txtcountry.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				//	CONS break down===============================================================================================	
					if(str_replace("'",'',$$consbreckdown) !='')
					{
						//$data_array_up2="";
						...
						for($c=0;$c < count($consbreckdown_array);$c++)
						{
							...
							if ($c!=0) $data_array_up2 .=",";
							$data_array_up2 .="(".$id1.", ".$id_dtls.", '".$consbreckdownarr[0]."', '".$consbreckdownarr[1]."', '".$consbreckdownarr[2]."', '".$consbreckdownarr[3]."', '".$color_id."', '".$consbreckdownarr[5]."', '".$consbreckdownarr[6]."', '".$consbreckdownarr[7]."', '".$consbreckdownarr[8]."', '".$consbreckdownarr[9]."', '".$consbreckdownarr[10]."', '".$consbreckdownarr[11]."', '".$consbreckdownarr[12]."', ".$txt_booking_no.", ".$$txtjob_id.", ".$$txtpoid.")";
							$id1=$id1+1;
							$add_comma++;
						}
					}
		 ...
		 if($data_array_up2 !="")
			{
				$rID2=sql_insert("wo_trim_book_con_dtls",$field_array_up2,$data_array_up2,0);
			}
            ...
            <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
                <table width="1130" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
                    ...
                    <tr class="general">
                        <td> 
                        <? 
                        echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", '', "load_drop_down( 'trims_booking_multi_job_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
                        ?>
                        </td>
                        <td id="buyer_td">
                        <? 
                        ...
                        <td>
                        <?
                        echo create_drop_down( "cbo_supplier_name", 172, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=4 and   a.status_active =1 and a.is_deleted=0 order by supplier_name","id,supplier_name", 1, "-- Select Supplier --", $selected, "",0 );
                        ?>	</td>
                        ...
                        <td align="center">
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('cbo_supplier_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_booking_prifix').value+'_'+document.getElementById('cbo_search_category').value+'_'+document.getElementById('txt_style').value+'_'+document.getElementById('txt_order_search').value, 'create_booking_search_list_view', 'search_div', 'trims_booking_multi_job_controller','setFilterGrid(\'list_view\',-1)')" style="width:100px;" />
                        </td>
                    </tr>
                    <tr>
...
if ($action=="create_booking_search_list_view")
{
	...
	$sql="select a.id, a.booking_no_prefix_num, a.booking_no, a.company_id, a.supplier_id, a.booking_date, a.delivery_date, c.style_ref_no, d.po_number from wo_booking_mst a, wo_booking_dtls b, wo_po_details_master c, wo_po_break_down d where a.booking_no=b.booking_no and b.job_no=c.job_no and b.job_no=d.job_no_mst and a.booking_type=2 and a.entry_form=261 
	and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 $company  $buyer  $supplier_id $booking_date $booking_cond $style_cond $order_cond group by a.id, a.booking_no_prefix_num, a.booking_no, a.company_id, a.supplier_id, a.booking_date, a.delivery_date,c.style_ref_no,d.po_number order by a.id DESC";
...
function fnc_fabric_booking_terms_condition( operation )
{
		...
		http.open("POST","trims_booking_multi_job_controller.php",true);
...
if($action=="show_trim_booking_report")
{
	...
	<div style="width:1333px" align="center">       
       ...
       <table width="100%" style="border:1px solid black">                    	
        ...
        $nameArray_color=sql_select( "select distinct b.color_number_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and b.requirment!=0 and a.sensitivity=1"); 
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            {
				$i++;
            $nameArray_item_description=sql_select( "select b.description, b.brand_supplier,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and b.requirment!=0  and a.sensitivity=1 and a.trim_group=".$result_item[csf('trim_group')]." group by b.description, b.brand_supplier order by trim_group "); 
            ?>
            <tr>
                ...
                $nameArray_color_size_qnty=sql_select( "select sum(b.requirment) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and b.requirment!=0  and a.sensitivity=1 and a.trim_group=". $result_item[csf('trim_group')]." and b.description='". $result_itemdescription[csf('description')]."' and b.brand_supplier='". $result_itemdescription[csf('brand_supplier')]."' and b.color_number_id=".$result_color[csf('color_number_id')].""); 
					}
					if($db_type==2)
					{
					
                $nameArray_color_size_qnty=sql_select( "select sum(b.requirment) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and b.requirment!=0  and a.sensitivity=1 and a.trim_group=". $result_item[csf('trim_group')]." and nvl(b.description,0)=nvl('". $result_itemdescription[csf('description')]."',0) and nvl(b.brand_supplier,0)=nvl('". $result_itemdescription[csf('brand_supplier')]."',0) and nvl(b.color_number_id,0)=nvl(".$result_color[csf('color_number_id')].",0)"); 
					}
        ...
        $nameArray_size=sql_select( "select distinct b.item_size  as gmts_sizes from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.sensitivity=2");
        ...
        $nameArray_color=sql_select( "select distinct b.item_color as color_number_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.sensitivity=3"); 
		if(count($nameArray_color)>0)
		{
        ?>
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
        ...
        $nameArray_size=sql_select( "select distinct b.item_size  as gmts_sizes from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.sensitivity=4");
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
			...
			$nameArray_color=sql_select( "select  b.item_color,b.color_number_id,b.description, b.brand_supplier,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=4 group by b.item_color,b.color_number_id,b.description, b.brand_supplier"); 
            $nameArray_item_description=sql_select( "select distinct uom from wo_booking_dtls  where booking_no=$txt_booking_no and sensitivity=4 and trim_group=".$result_item[csf('trim_group')]." order by trim_group "); 
            ?>
            <tr>
	...
	<div style="width:1333px" align="center">       
       ...
       <table width="100%" style="border:1px solid black">                    	
        ...
        ?>
          <!--==============================================AS PER GMTS COLOR START=========================================  -->
		<?
		$nameArray_job_po=sql_select( "select job_no,po_break_down_id from wo_booking_dtls  where booking_no=$txt_booking_no group by job_no,po_break_down_id order by job_no,po_break_down_id "); 
        ...
        $nameArray_color=sql_select( "select distinct b.color_number_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=1"); 
		if(count($nameArray_color)>0)
		{
        ?>
        &nbsp;
        ...
        <!--==============================================AS PER SIZE  END=========================================  -->
         ...
         <!--==============================================AS PER CONTRAST COLOR START=========================================  -->
		<?
		$gmtcolor_library=return_library_array( "select b.item_color,b.color_number_id from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=$txt_booking_no and a.sensitivity=3", "item_color", "color_number_id"  );
        ...
        $nameArray_color=sql_select( "select distinct b.item_color as color_number_id,b.color_number_id as gmts_color from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=3"); 
		if(count($nameArray_color)>0)
		{
        ?>
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
        ...
        $nameArray_size=sql_select( "select distinct b.item_size  as gmts_sizes from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=4");
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
			...
			$gmtcolor_library=return_library_array( "select b.item_color,b.color_number_id from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=4", "item_color", "color_number_id"  );
			 ...
			 $nameArray_color=sql_select( "select  b.item_color as color_number_id,b.description, b.brand_supplier,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=4 group by b.item_color,b.description, b.brand_supplier"); 
...
if($action=="show_trim_booking_report2")
{
	...
	<div style="width:1333px" align="center">       
       ...
       <table width="100%" style="border:1px solid black">                    	
            ...
            <tr>
                <td width="100" style="font-size:12px"><b>File</b>   </td>
                <td width="110">:&nbsp;
				<? 
				echo rtrim($file_no,", ");
				?> 
                </td>
                <td width="100" style="font-size:12px"><b>Ref.</b>   </td>
                <td width="110">:&nbsp;
				<? 
				echo rtrim($ref_no,", ");
				?> 
	   ...
	   if(count($nameArray_item)>0)
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
           <tr>
            ...
            {
				$i++;
            $nameArray_item_description=sql_select( "select b.description, b.brand_supplier,b.item_color,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=1 and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and b.requirment !=0 group by b.description, b.brand_supplier,b.item_color order by trim_group "); 
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
			...
			$i++;
            //$nameArray_item_description=sql_select( "select distinct description,brand_supplier,rate,uom from wo_booking_dtls  where booking_no=$txt_booking_no and sensitivity=2 and trim_group=".$result_item[csf('trim_group')]." order by trim_group "); 
			$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,b.item_size,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and   a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=2 and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and b.requirment !=0 group by b.description, b.brand_supplier,b.item_size order by b.item_size"); 
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i ; ?>
       ...
       // $nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=3 order by trim_group "); 
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
           <tr>
            ...
            {
				$i++;
				$gmtcolor_library=return_library_array( "select b.item_color,b.color_number_id from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=3 and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and b.requirment !=0  order by trim_group ", "item_color", "color_number_id"  );

			$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,b.item_color,b.color_number_id,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=3 and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and b.requirment !=0 group by b.description, b.brand_supplier,b.item_color,b.color_number_id order by trim_group "); 
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
	   ...
	   if(count($nameArray_item)>0)
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item)
            {
			 ...
			 $nameArray_color=sql_select( "select  b.item_color as color_number_id,b.item_size,b.description, b.brand_supplier,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=4 and b.requirment !=0 group by b.item_color,b.item_size,b.description, b.brand_supplier order by b.item_color "); 
...
{
	$job_no="";
	 $sql= "select booking_no,booking_date,company_id,buyer_id, 	job_no,currency_id,exchange_rate,pay_mode,booking_month,supplier_id,attention,tenor,remarks,item_from_precost,delivery_date,source,booking_year,is_approved from wo_booking_mst  where booking_no='$data'";     
	 $data_array=sql_select($sql);
	 foreach ($data_array as $row)
	 {
		...
		echo "document.getElementById('txt_tot_req_amount').value = '".$data_req_arr[0]."';\n";
		echo "document.getElementById('txt_tot_cu_amount').value = '".$data_req_arr[1]."';\n";
		echo "load_drop_down( 'requires/trims_booking_multi_job_controller', '".$row[csf("company_id")]."', 'load_drop_down_buyer', 'buyer_td' );\n";
		...
		echo "load_drop_down( 'requires/trims_booking_multi_job_controller', '".$row[csf("pay_mode")]."', 'load_drop_down_supplier', 'supplier_td' );\n";
		...
		echo "get_php_form_data(".$row[csf("company_id")].", 'populate_variable_setting_data', 'requires/trims_booking_multi_job_controller' );\n";
