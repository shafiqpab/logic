	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="rpt_table" rules="all">
        ...
        <tr>
            <td align="center" colspan="11" id="country_po_active_listview11">
				...
				?>
                <tr onClick="populate_size_color_breakdown_with_data('<? echo $row[csf("id")]."_".$row[csf("country_id")]."_".change_date_format($row[csf("country_ship_date")],"dd-mm-yyyy","-")."_".change_date_format($row[csf("cutup_date")],"dd-mm-yyyy","-")."_".$row[csf("cutup")]."_".$row[csf("country_remarks")]."_".change_date_format($row[csf("extended_ship_date")],"dd-mm-yyyy","-");?>')" style="cursor:pointer">
...
$po_break_down_data=sql_select("select b.job_no, a.excess_cut,a.po_quantity,a.po_number,a.unit_price,b.company_name,b.buyer_name,b.gmts_item_id,b.order_uom,total_set_qnty from wo_po_break_down a, wo_po_details_master b where a.job_no_mst=b.job_no and a.id='$data[0]'");
list($po_break_down_data_array )=$po_break_down_data;
$gmt_item_id=$po_break_down_data_array[csf(gmts_item_id)];
$company_name=$po_break_down_data_array[csf(company_name)];
	$buyer_name=$po_break_down_data_array[csf(buyer_name)];
	$job_no=$po_break_down_data_array[csf(job_no)];
...
<tr id="break_1" onClick="tr_index(this)" >
    ...
    <td>
    <input type="text" id="txtorderrate_<?=$k; ?>" onBlur="calculate_total_amnt(<?=$k; ?>);" value="<?=$order_rate; ?>" name="txtorderrate_<?=$k; ?>" class="text_boxes_numeric" style="width:75px" > 
    </td>
    <td>
...
if($action=="delete_row_color_size")
{
	...
	if(trim($exData[1])!="")
	{
		$sql_data=sql_select("select approved from wo_pre_cost_mst where job_no='$exData[1]' and is_deleted=0 and status_active=1");
	...
	$rID_de1=execute_query( "update wo_po_color_size_breakdown set status_active=0, is_deleted=1, updated_by=".$_SESSION['logic_erp']['user_id'].", update_date='".$pc_date_time."' where id =".$exData[0]."",0);
	if($db_type==0)
	{
		if($rID_de1 ){
		mysql_query("COMMIT");  
...
?>
	...
	function fnc_size_color_breakdown( operation )
	{
		...
		for (var i=1; i<=row_num; i++)
		{
			...
			data_all=data_all+get_submitted_data_string('txt_country_ship_date*txt_extended_ship_date*txt_remark*txt_cutup_date*cbo_cut_up*cbogmtsitem_'+i+'*hiddenid_'+i+'*txtarticleno_'+i+'*txtcolor_'+i+'*txtsize_'+i+'*txtorderquantity_'+i+'*txtorderrate_'+i+'*txtorderamount_'+i+'*txtorderexcesscut_'+i+'*txtorderplancut_'+i+'*cbostatus_'+i,"../../../",i);
		}
	...
	{   
		if(table_id=='size_color_break_down_list')
		{
			...
			if(rowNo!=1)
			{
				...
				{
					var booking=return_global_ajax_value(updateid+'_'+job_no, 'delete_row_color_size', '', 'size_color_breakdown_controller_update');
					if(booking==50)
					{
						alert("BOM Approved. Update or Delete Restricted.");
						release_freezing();
	...
	function populate_size_color_breakdown_with_data(data)
	{
		...
		document.getElementById('txt_extended_ship_date').value=data[6];
		if(data[3] !="")
		{
...
function copy_avg_price()
{
	...
	var order_id=document.getElementById('order_id').value;
	var job_no=$('#txt_job_no').val();
	var tot_row=return_global_ajax_value(order_id+'_'+avg_price+'_'+job_no, 'update_avg_rate', '', 'size_color_breakdown_controller_update');
	if(tot_row==50){
		alert("BOM Approved. Update or Delete Restricted.");
		release_freezing();
		return;
    ...
    <div style="width:100%;" align="center">
       ...
       <fieldset style="width:1180px">
        ...
        <tr>
            ...
            <td colspan="2" align="center">
            <input type="button" id="copyset3" style="width:100px" class="formbutton" value="Copy Ex. Cut" onClick="copy_excess_cut();" /> 
            </td>
        </tr>
        </table>
        </fieldset>
        ...
        <table>
            <tr>
                ...
                <td>
                <?php
                echo create_drop_down( "cbo_po_country", 170,"select id,country_name from lib_country where status_active=1 and is_deleted=0 order by country_name", "id,country_name", 1, "Select", "","check_country(this.value);","1" ); 
                ?>
                ...
                <input class="datepicker" type="text" style="width:70px;"  name="txt_country_ship_date" id="txt_country_ship_date" />
                 ...
                 <td  align="right">Extended  Shipment Date</td>
                <td>
                <input class="datepicker" type="text" style="width:70px;"  name="txt_extended_ship_date" id="txt_extended_ship_date" />
                </td>
...
if($action=="update_avg_rate")
{
	$i=0;
	$data=explode("_",$data);
	if(trim(str_replace("'","",$data[2]))!="")
	{
		$sql_data=sql_select("select approved from wo_pre_cost_mst where job_no='$data[2]' and is_deleted=0 and status_active=1");
		$isapproved=$sql_data[0][csf("approved")];
...
if ($action=="save_update_delete")
{
	...
	$sql_attach="select sum(attached_value) as order_attach_value from com_sales_contract_order_info where status_active=1 and is_deleted=0 and wo_po_break_down_id=$order_id
	union all 
	select sum(attached_value) as order_attach_value from com_export_lc_order_info where status_active=1 and is_deleted=0 and wo_po_break_down_id=$order_id";
	...
	{
		echo "16**Order Value Not Allowed Less Then Attach Value. $tot_country_amount = $tot_attach_value";disconnect($con);die;
	}
	...
	if ($operation==0)  //Insert Here
	{
		 ...
		 $field_array="id,po_break_down_id,job_no_mst,color_mst_id, size_mst_id, item_mst_id, article_number, item_number_id,country_id,cutup_date,cutup,country_remarks,country_ship_date,extended_ship_date, size_number_id, color_number_id, order_quantity, order_rate, order_total,excess_cut_perc,plan_cut_qnty,is_deleted,status_active,inserted_by,insert_date";
		 for ($i=1;$i<=$total_row;$i++)
		 {
			...
			if ($i!=1) $data_array .=",";
			$data_array .="(".$id.",".$order_id.",'".$txt_job_no."','".$color_mst_id."','".$size_mst_id."','".$item_mst_id."',".$$txtarticleno.",".$$cbogmtsitem.",".$cbo_po_country.",".$txt_cutup_date.",".$cbo_cut_up.",".$txt_remark.",".$txt_country_ship_date.",".$txt_extended_ship_date.",".$size_id.",".$color_id.",".$$txtorderquantity.",".$$txtorderrate.",".$$txtorderamount.",".$$txtorderexcesscut.",".$$txtorderplancut.",0,".$$cbostatus.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			$id=$id+1;
		 }
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			...
			/* $PrevData=sql_select("select color_number_id,country_ship_date from wo_po_color_size_breakdown where id=".$$hiddenid);
			 ...
			 $PrevCountryShipDate=$PrevData[0][csf('country_ship_date')];*/
			  ...
			  $PrevData=sql_select("select color_number_id, country_ship_date, cutup_date, country_ship_date_prev, color_number_id_prev from wo_po_color_size_breakdown where id=".$$hiddenid);
				$PrevColorNumberId=$PrevData[0][csf('color_number_id')];
				$PrevCountryShipDate=$PrevData[0][csf('country_ship_date')];
				$PrevCutupDate=$PrevData[0][csf('cutup_date')];
				...
				if(change_date_format($PrevCountryShipDate)==change_date_format(str_replace("'","",$txt_cutup_date)))
				{
					$pre_countryship_date=$pre_country_date;
				}
				else $pre_countryship_date=$PrevCountryShipDate;
			 
				$field_array="job_no_mst* 	color_mst_id*size_mst_id*item_mst_id*article_number*item_number_id*country_id*cutup_date*cutup*country_remarks*country_ship_date*extended_ship_date*size_number_id*color_number_id*order_quantity*order_rate*order_total*excess_cut_perc*plan_cut_qnty*color_number_id_prev*country_ship_date_prev*is_deleted*status_active*updated_by";
				$data_array="'".$txt_job_no."'*'".$color_mst_id."'*'".$size_mst_id."'*'".$item_mst_id."'*".$$txtarticleno."*".$$cbogmtsitem."*".$cbo_po_country."*".$txt_cutup_date."*".$cbo_cut_up."*".$txt_remark."*".$txt_country_ship_date."*".$txt_extended_ship_date."*".$size_id."*".$color_id."*".$$txtorderquantity."*".$$txtorderrate."*".$$txtorderamount."*".$$txtorderexcesscut."*".$$txtorderplancut."*'".$pre_colorid."'*'".$pre_countryship_date."'*0*".$$cbostatus."*".$_SESSION['logic_erp']['user_id']."";
			...
			if(str_replace("'",'',$$hiddenid)=="")
			{
		     ...
		     else
			 {
				...
				$field_array="id,po_break_down_id,job_no_mst,color_mst_id, size_mst_id, item_mst_id, article_number, item_number_id,country_id,cutup_date,cutup,country_ship_date,extended_ship_date, size_number_id, color_number_id, order_quantity, order_rate,order_total,excess_cut_perc,plan_cut_qnty,is_deleted,status_active,inserted_by,insert_date";
				$data_array="(".$id.",".$order_id.",'".$txt_job_no."','".$color_mst_id."','".$size_mst_id."','".$item_mst_id."',".$$txtarticleno.",".$$cbogmtsitem.",".$cbo_po_country.",".$txt_cutup_date.",".$cbo_cut_up.",".$txt_country_ship_date.",".$txt_extended_ship_date.",".$size_id.",".$color_id.",".$$txtorderquantity.",".$$txtorderrate.",".$$txtorderamount.",".$$txtorderexcesscut.",".$$txtorderplancut.",0,".$$cbostatus.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
