...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$field_array="id,booking_type,booking_no_prefix,booking_no_prefix_num,booking_no,company_id,buyer_id,item_category,fabric_source,currency_id,exchange_rate,pay_mode,source,booking_date,delivery_date,supplier_id,attention,ready_to_approved,team_leader,dealing_marchant,uom_id,inserted_by,insert_date";
	...
	else if ($operation==1)   // Update Here
	{
		...
		$field_array="buyer_id*item_category*fabric_source*currency_id*exchange_rate*pay_mode*source*booking_date*delivery_date*supplier_id*attention*ready_to_approved*team_leader*dealing_marchant*uom_id*updated_by*update_date";
		 $data_array ="".$cbo_buyer_name."*".$cbo_fabric_natu."*".$cbo_fabric_source."*".$cbo_currency."*".$txt_exchange_rate."*".$cbo_pay_mode."*".$cbo_source."*".$txt_booking_date."*".$txt_delivery_date."*".$cbo_supplier_name."*".$txt_attention."*".$cbo_ready_to_approved."*".$cbo_team_leader."*".$cbo_dealing_merchant."*".$cbouom_id."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
...
if($action=="save_update_delete_dtls")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		if(str_replace("'","", trim($txt_gmts_size))!="")
		{
			...
			{
			$gmts_size_id = return_id( str_replace("'","", trim($txt_gmts_size)), $size_library, "lib_size", "id,size_name","90");
