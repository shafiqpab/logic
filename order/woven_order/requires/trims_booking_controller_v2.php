            	...
            	<table  cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
        			...
        			<tr>
                        ...
                        <input type="hidden" id="garments_nature" value="<? echo $garments_nature; ?>">
							<? 
								echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", '',"load_drop_down( 'trims_booking_controller_v2', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
							?>
                    </td>
                   	<td id="buyer_td">
                     <? 
...
CASE e.costing_per WHEN 1 THEN round(((d.cons/12)*b.po_quantity)/cc.conversion_factor,4) WHEN 2 THEN round(((d.cons/1)*b.po_quantity)/cc.conversion_factor,4)  WHEN 3 THEN round(((d.cons/24)*b.po_quantity)/cc.conversion_factor,4) WHEN 4 THEN round(((d.cons/36)*b.po_quantity)/cc.conversion_factor,4) WHEN 5 THEN round(((d.cons/48)*b.po_quantity)/cc.conversion_factor,4) ELSE 0 END as req_qnty,cc.order_uom as cons_uom,IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0)  as cu_woq, CASE e.costing_per WHEN 1 THEN round((((d.cons/12)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 2 THEN round((((d.cons/1)*b.po_quantity)/cc.conversion_factor) - IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4)  WHEN 3 THEN round((((d.cons/24)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 4 THEN round((((d.cons/36)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 5 THEN round((((d.cons/48)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) ELSE 0 END as bal_woq, round((c.rate*cc.conversion_factor),8) as rate from wo_po_details_master a, wo_po_break_down b ,wo_pre_cost_mst e,wo_pre_cost_trim_cost_dtls c ,lib_item_group cc, wo_pre_cost_trim_co_cons_dtls d left join wo_booking_dtls f on f.job_no=d.job_no and f.pre_cost_fabric_cost_dtls_id=d.wo_pre_cost_trim_cost_dtls_id and f.po_break_down_id=d.po_break_down_id and f.booking_type=2   where a.job_no=b.job_no_mst and  a.job_no=c.job_no and a.job_no=e.job_no and  a.job_no=d.job_no   and c.id=d.wo_pre_cost_trim_cost_dtls_id and b.id=d.po_break_down_id and cc.id=c.trim_group   and a.company_name=$company_id $txt_job_no_cond $cbo_buyer_name_cond and a.garments_nature=$garments_nature  group by d.id order by b.id) m  where  company_name=$company_id $txt_job_no_cond1 $cbo_buyer_name_cond1  and garments_nature=$garments_nature   and bal_woq >0 order by trim_group";//and b.pub_shipment_date between '$start_date' and '$end_date'*/
					...
					f.pre_cost_fabric_cost_dtls_id=e.wo_pre_cost_trim_cost_dtls_id and 
					f.po_break_down_id=e.po_break_down_id and 
					f.booking_type=2 and f.status_active=1 and f.is_deleted=0
					where
					a.job_no=b.job_no and 
					a.job_no=c.job_no and 
					a.job_no=d.job_no_mst and 
...
CASE e.costing_per WHEN 1 THEN round(((d.cons/12)*b.po_quantity)/cc.conversion_factor,4) WHEN 2 THEN round(((d.cons/1)*b.po_quantity)/cc.conversion_factor,4)  WHEN 3 THEN round(((d.cons/24)*b.po_quantity)/cc.conversion_factor,4) WHEN 4 THEN round(((d.cons/36)*b.po_quantity)/cc.conversion_factor,4) WHEN 5 THEN round(((d.cons/48)*b.po_quantity)/cc.conversion_factor,4) ELSE 0 END as req_qnty,cc.order_uom as cons_uom,IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0)  as cu_woq, CASE e.costing_per WHEN 1 THEN round((((d.cons/12)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 2 THEN round((((d.cons/1)*b.po_quantity)/cc.conversion_factor) - IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4)  WHEN 3 THEN round((((d.cons/24)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 4 THEN round((((d.cons/36)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 5 THEN round((((d.cons/48)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) ELSE 0 END as bal_woq, round((c.rate*cc.conversion_factor),8) as rate from wo_po_details_master a, wo_po_break_down b ,wo_pre_cost_mst e,wo_pre_cost_trim_cost_dtls c ,lib_item_group cc, wo_pre_cost_trim_co_cons_dtls d left join wo_booking_dtls f on f.job_no=d.job_no and f.pre_cost_fabric_cost_dtls_id=d.wo_pre_cost_trim_cost_dtls_id and f.po_break_down_id=d.po_break_down_id and f.booking_type=2   where a.job_no=b.job_no_mst and  a.job_no=c.job_no and a.job_no=e.job_no and  a.job_no=d.job_no   and c.id=d.wo_pre_cost_trim_cost_dtls_id and b.id=d.po_break_down_id and cc.id=c.trim_group  and a.company_name=$cbo_company_name $txt_job_no_cond $cbo_buyer_name_cond and a.garments_nature=$garments_nature group by d.id order by b.id) m   where id in($data) $txt_job_no_cond1  $cbo_buyer_name_cond1 and garments_nature=$garments_nature order by trim_group";//and b.pub_shipment_date between '$start_date' and '$end_date'*/ 
					...
					f.pre_cost_fabric_cost_dtls_id=e.wo_pre_cost_trim_cost_dtls_id and 
					f.po_break_down_id=e.po_break_down_id and 
					f.booking_type=2 and f.status_active=1
					where
					a.job_no=b.job_no and 
					a.job_no=c.job_no and 
					a.job_no=d.job_no_mst and 
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1290" class="rpt_table" >
       ...
       <?
					...
					$sql_lib_item_group_array[$row_sql_lib_item_group[csf('id')]][cons_uom]=$row_sql_lib_item_group[csf('cons_uom')];
					 ...
					 //$sql="select m.job_no_prefix_num, m.job_no, m.company_name, m.buyer_name, m.style_ref_no, m.costing_per, m.wo_pre_cost_trim_cost_dtls, m.trim_group, m.rate as precost_rate, m.po_id, m.po_number, m.plan_cut, m.po_break_down_id, m.cons, m.cu_woq, m.cu_amount, m.description, m.brand_sup_ref, n.id as booking_id, n.booking_no, n.sensitivity, n.wo_qnty, n.rate, n.exchange_rate, n.amount, n.delivery_date, n.cons_break_down from (select a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.style_ref_no, b.costing_per, c.id as wo_pre_cost_trim_cost_dtls, c.trim_group, c.description, c.brand_sup_ref, c.rate, c.amount as pre_amt, d.id as po_id, d.po_number, d.file_no, d.grouping, d.po_quantity as plan_cut, e.po_break_down_id, e.cons, sum(f.wo_qnty) as cu_woq, sum(f.amount) as cu_amount from wo_po_details_master a, wo_pre_cost_mst b, wo_pre_cost_trim_cost_dtls c, wo_po_break_down d, wo_pre_cost_trim_co_cons_dtls e left join wo_booking_dtls f on f.job_no=e.job_no and f.pre_cost_fabric_cost_dtls_id=e.wo_pre_cost_trim_cost_dtls_id and f.po_break_down_id=e.po_break_down_id and f.booking_type=2 where a.job_no=b.job_no and a.job_no=c.job_no and a.job_no=d.job_no_mst and a.job_no=e.job_no and c.id=e.wo_pre_cost_trim_cost_dtls_id and d.id=e.po_break_down_id and a.company_name=$cbo_company_name $txt_job_no_cond $cbo_buyer_name_cond and a.garments_nature=$garments_nature and d.is_deleted=0 and d.status_active=1 group by a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.style_ref_no, b.costing_per, c.id, c.trim_group, c.description, c.brand_sup_ref, c.rate, c.amount, d.id, d.po_number, d.file_no, d.grouping, d.po_quantity, e.po_break_down_id, e.cons order by d.id,c.id) m, wo_booking_dtls n where m.job_no=n.job_no and m.po_id=n.po_break_down_id and m.wo_pre_cost_trim_cost_dtls=n.pre_cost_fabric_cost_dtls_id  and n.booking_no=$txt_booking_no and n.is_deleted=0 and n.status_active=1 order by m.po_id,m.wo_pre_cost_trim_cost_dtls ";
					 $sql="SELECT m.job_no_prefix_num, m.job_no, m.company_name, m.buyer_name, m.style_ref_no, m.costing_per, m.wo_pre_cost_trim_cost_dtls, m.trim_group, m.rate as precost_rate, m.po_id, m.po_number, m.plan_cut, m.id, m.po_break_down_id, m.cons, m.cu_woq, m.cu_amount, m.description, m.brand_sup_ref, n.id as booking_id, n.booking_no, n.sensitivity, n.wo_qnty, n.rate, n.exchange_rate, n.amount, n.delivery_date, n.cons_break_down from (SELECT a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.style_ref_no, b.costing_per, c.id as wo_pre_cost_trim_cost_dtls, c.trim_group, c.description, c.brand_sup_ref, c.rate, d.id as po_id, d.po_number, d.file_no, d.grouping, d.po_quantity as plan_cut, e.id as id, e.po_break_down_id, e.cons, sum(f.wo_qnty) as cu_woq, sum(f.amount) as cu_amount from wo_po_details_master a join wo_pre_cost_mst b on a.id=b.job_id join wo_pre_cost_trim_cost_dtls c on c.job_id=b.job_id join wo_po_break_down d on a.id=d.job_id join wo_pre_cost_trim_co_cons_dtls e on c.id=e.wo_pre_cost_trim_cost_dtls_id and d.id=e.po_break_down_id left join wo_booking_dtls f on f.pre_cost_fabric_cost_dtls_id=e.wo_pre_cost_trim_cost_dtls_id and f.po_break_down_id=e.po_break_down_id and f.booking_type=2 where a.company_name=$cbo_company_name $txt_job_no_cond $cbo_buyer_name_cond and a.garments_nature=$garments_nature and d.is_deleted=0 and d.status_active=1 group by e.id, a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.style_ref_no, b.costing_per, c.id, c.trim_group, c.description, c.brand_sup_ref, c.rate, d.id, d.po_number, d.file_no, d.grouping, d.po_quantity, e.po_break_down_id, e.cons order by d.id,c.id) m, wo_booking_dtls n where m.job_no=n.job_no and m.po_id=n.po_break_down_id and m.wo_pre_cost_trim_cost_dtls=n.pre_cost_fabric_cost_dtls_id  and n.booking_no=$txt_booking_no and n.is_deleted=0 and n.status_active=1 order by m.po_id,m.wo_pre_cost_trim_cost_dtls";
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$field_array="id,booking_type,is_short,booking_month,booking_year,booking_no_prefix,booking_no_prefix_num,booking_no,company_id,buyer_id,job_no, 	item_category,supplier_id,currency_id,exchange_rate,booking_date,delivery_date,pay_mode,source,attention,ready_to_approved,tenor,remarks,item_from_precost,entry_form,inserted_by,insert_date";
		
		 $data_array ="(".$id.",2,2,".$cbo_booking_month.",".$cbo_booking_year.",'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',".$cbo_company_name.",".$cbo_buyer_name.",".$txt_job_no.",4,".$cbo_supplier_name.",".$cbo_currency.",".$txt_exchange_rate.",".$txt_booking_date.",".$txt_delivery_date.",".$cbo_pay_mode.",".$cbo_source.",".$txt_attention.",".$cbo_ready_to_approved.",".$txt_tenor.",".$txt_remarks.",".$cbo_item_from_precost.",44,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		 
		 $rID=sql_insert("wo_booking_mst",$field_array,$data_array,0);
		 	
		 $id_dtls=return_next_id( "id", "wo_booking_dtls", 1 ) ;
		 $field_array1="id, pre_cost_fabric_cost_dtls_id, po_break_down_id, job_no, booking_mst_id,booking_no, booking_type, is_short, trim_group, uom, sensitivity, wo_qnty, exchange_rate, pre_req_amt, rate, amount, delivery_date, country_id_string, inserted_by, insert_date";
		...
		$field_array2="id, wo_trim_booking_dtls_id, booking_no, job_no, po_break_down_id, color_number_id, gmts_sizes, description, brand_supplier, item_color, item_size, cons, process_loss_percent, requirment, rate, amount, pcs, color_size_table_id";
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			 ...
			 //if ($i!=1) $data_array1 .=",";
			 $data_array1 ="(".$id_dtls.",".$$txttrimcostid.",".$$txtpoid.",".$txt_job_no.",".$id.",'".$new_booking_no[0]."',2,2,".$$txttrimgroup.",".$$txtuom.",".$$cbocolorsizesensitive.",".$$txtwoq.",".$$txtexchrate.",".$$preconsamt.",".$$txtrate.",".$$txtamount.",".$$txtddate.",".$$txtcountry.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
...
//"id,wo_trim_booking_dtls_id,booking_no,job_no,po_break_down_id,color_number_id,gmts_sizes,description,brand_supplier,item_color,item_size,cons, process_loss_percent,requirment,rate,amount,pcs,color_size_table_id"
	...
	/*	}else{*/
		 ...
		 $field_array_up="booking_month*booking_year*buyer_id*job_no*item_category*supplier_id*currency_id*exchange_rate*booking_date*delivery_date*pay_mode*source*attention*ready_to_approved*tenor*remarks*item_from_precost*entry_form*updated_by*update_date"; 
		 
		 $data_array_up ="".$cbo_booking_month."*".$cbo_booking_year."*".$cbo_buyer_name."*".$txt_job_no."*4*".$cbo_supplier_name."*".$cbo_currency."*".$txt_exchange_rate."*".$txt_booking_date."*".$txt_delivery_date."*".$cbo_pay_mode."*".$cbo_source."*".$txt_attention."*".$cbo_ready_to_approved."*".$txt_tenor."*".$txt_remarks."*".$cbo_item_from_precost."*44*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		
		
		// if  ( check_table_status( $_SESSION['menu_id'], 1 )==0 ) { echo "15**1"; die;}		
		 $field_array_up1="pre_cost_fabric_cost_dtls_id*po_break_down_id*job_no*booking_no*trim_group*uom*sensitivity*wo_qnty*exchange_rate*pre_req_amt*rate*amount*delivery_date*country_id_string*updated_by*update_date";
		  $id_dtls=return_next_id( "id", "wo_booking_dtls", 1 ) ;
		  $field_array1="id, pre_cost_fabric_cost_dtls_id, po_break_down_id, job_no, booking_no, booking_type, is_short, trim_group, uom, sensitivity, wo_qnty, exchange_rate, pre_req_amt, rate, amount, delivery_date,country_id_string, inserted_by, insert_date";
		 //$field_array2="id,wo_trim_booking_dtls_id,booking_no,job_no,po_break_down_id,color_number_id,gmts_sizes,description,brand_supplier,item_color,item_size,cons, process_loss_percent,requirment,rate,amount,pcs,color_size_table_id";
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			...
			if(str_replace("'",'',$$txtbookingid)!="")
			{
				...
				$data_array_up1[str_replace("'",'',$$txtbookingid)] =explode("*",("".$$txttrimcostid."*".$$txtpoid."*".$txt_job_no."*".$txt_booking_no."*".$$txttrimgroup."*".$$txtuom."*".$$cbocolorsizesensitive."*".$$txtwoq."*".$$txtexchrate."*".$$preconsamt."*".$$txtrate."*".$$txtamount."*".$$txtddate."*".$$txtcountry."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
			...
			{
				 //if ($i!=1) $data_array1 .=",";
				 $data_array1 ="(".$id_dtls.",".$$txttrimcostid.",".$$txtpoid.",".$txt_job_no.",".$txt_booking_no.",2,2,".$$txttrimgroup.",".$$txtuom.",".$$cbocolorsizesensitive.",".$$txtwoq.",".$$txtexchrate.",".$$preconsamt.",".$$txtrate.",".$$txtamount.",".$$txtddate.",".$$txtcountry.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				//	CONS break down===============================================================================================	
					if(str_replace("'",'',$$consbreckdown) !='')
					{
						$data_array_up2="";
...
if ($action=="create_booking_search_list_view")
{
	...
	$arr=array (1=>$comp,2=>$suplier);
	  ...
	  $sql="select a.id, a.booking_no_prefix_num, a.booking_no, a.company_id, a.supplier_id, a.booking_date, a.delivery_date, c.style_ref_no, c.job_no_prefix_num, d.po_number 
	 from wo_booking_mst a,wo_booking_dtls b, wo_po_details_master c,wo_po_break_down d  
	 where a.booking_no=b.booking_no and b.job_no=c.job_no and b.job_no=d.job_no_mst and a.booking_type=2 and a.entry_form=44 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1  and d.is_deleted=0 and d.status_active=1 $company  $buyer $job_cond  $supplier_id $booking_date $booking_cond $style_cond $order_cond group by a.id, a.booking_no_prefix_num, a.booking_no, a.company_id, a.supplier_id, a.booking_date, a.delivery_date, c.style_ref_no, c.job_no_prefix_num, d.po_number order by a.id DESC";
...
{
	$job_no="";
	 $sql= "select booking_no,booking_date,company_id,buyer_id, 	job_no,currency_id,exchange_rate,pay_mode,ready_to_approved,tenor,booking_month,supplier_id,attention,remarks,item_from_precost,delivery_date,source,booking_year,is_approved from wo_booking_mst  where booking_no='$data'";     
	 $data_array=sql_select($sql);
	 foreach ($data_array as $row)
	 {
		...
		echo "load_drop_down( 'requires/trims_booking_controller_v2', '".$row[csf("pay_mode")].'_'.$row[csf("company_id")]."', 'load_drop_down_supplier', 'supplier_td' );\n";
