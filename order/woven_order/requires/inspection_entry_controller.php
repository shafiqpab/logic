...
{ 
	...
	echo create_drop_down( "cbo_order_id",100, "select id,po_number from wo_po_break_down where job_no_mst='$data' and status_active in(1,2,3) and is_deleted=0","id,po_number", 1, "--Select--", "", "load_drop_down( 'requires/inspection_entry_controller', '__'+this.value, 'load_drop_down_country_id', 'country_drop_down_td' );get_php_form_data( this.value+','+document.getElementById('cbo_week_no').value+','+document.getElementById('cbo_country_id').value+','+document.getElementById('cbo_inspection_level').value, 'set_po_qnty_ship_date', 'requires/inspection_entry_controller');","","","","","","" );
}
...
{ 
	...
	//echo create_drop_down( "cbo_country_id",100, "select a.id,a.country_name from lib_country a,wo_po_color_size_breakdown b where $con and a.id=b.country_id group by a.id,a.country_name order by a.country_name","id,country_name", 1, "--Select--", "", "get_php_form_data( document.getElementById('cbo_order_id').value+','+document.getElementById('cbo_week_no').value+','+this.value, 'set_po_qnty_ship_date', 'requires/inspection_entry_controller')","","","","","","" );
	echo create_drop_down( "cbo_country_id",100, "SELECT a.id,a.country_name from lib_country a inner join wo_po_color_size_breakdown b on a.id=b.country_id left join week_of_year c on c.week_date=b.country_ship_date $con_week  where $con  and b.status_active=1 and b.is_deleted=0 group by a.id,a.country_name order by a.country_name","id,country_name", 1, "--Select--", "", "get_php_form_data( document.getElementById('cbo_order_id').value+','+document.getElementById('cbo_week_no').value+','+this.value+','+document.getElementById('cbo_inspection_level').value, 'set_po_qnty_ship_date', 'requires/inspection_entry_controller')","","","","","","" );
}
	            	...
	            	<table  cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
	        			...
	        			<tr>
	                    	<td> <input type="hidden" id="selected_job">
								<? 
									echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", '',"load_drop_down( 'inspection_entry_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
								?>
	                    </td>
	                   	<td id="buyer_td">
	                     <? 
...
if($action=="open_order_popup")
{
	...
	<? 
		...
		// echo $sql;
		echo  create_list_view("list_view", "Order No.,Order Qty,Insp Qty,Balance", "150,140,140,150","590","350",0, $sql , "js_set_value", "id,po_number", "", 1, "0,0,0,0", $arr , "po_number,po_quantity,insp_qty,balance", "requires/inspection_entry_controller",'setFilterGrid("list_view",-1);','','');
...
if ($action=="open_actual_order_popup")
{
  ...
  ?>
	...
	$sql= "SELECT a.id as actual_id,a.acc_po_no,a.acc_ship_date ,a.acc_po_qty,b.country_id,sum (b.po_qty) as po_qty,listagg( b.id,', ' on overflow truncate with count)     
	within group (order by b.id) dtls_id  	
		from wo_po_acc_po_info a,wo_po_acc_po_info_dtls b where a.id=b.mst_id and   a.status_active=1  and a.is_deleted=0 and a.job_no='$job_no'  $dtls_id_cond group by a.id,a.acc_po_no,a.acc_ship_date ,a.acc_po_qty,b.country_id";
		//echo $sql;
...
if ($action=="actual_po_list_view")
{
	...
	$country_arr=return_library_array( "select id,country_name from lib_country where status_active=1 order by id", "id", "country_name"  );
		...
		from wo_po_acc_po_info a,wo_po_acc_po_info_dtls b where a.id=b.mst_id and   a.status_active=1  and a.is_deleted=0 and b.id in ($po_id) group by a.id,a.acc_po_no,a.acc_ship_date ,a.acc_po_qty,b.country_id,b.po_qty";
	...
	?>
			...
			<table cellpadding="0" cellspacing="1" width="100%" class="rpt_table" rules="all" id="table_list_view">
						...
						foreach($sql_data as $row){
							...
							$bal_qnty=$row[csf('po_qty')]-$cuml_qnty;
                        ...
                        <tr id="tr_<?=$i;?>">
                            ...
                            <td><?  //echo create_drop_down( "cbo_order_id",100, $blank_array, 0, "", $selected, "" ); ?>
							...
							<? echo create_drop_down( "cbo_country_id_".$i, 100, $country_arr,"", 1, "--- Select ---", $row[csf('country_id')], "" ,1)
							?></td> 
							...
							<td ><input name="txt_finishing_qnty_<?=$i;?>" id="txt_finishing_qnty_<?=$i;?>"  class="text_boxes_numeric" type="text"  style="width:80px" value="0" readonly/></td>
							...
							<td><?  echo create_drop_down( "cbo_insp_status_".$i,80, $inspection_status,"", 1, "-- Select --", $selected, "" ); //change_cause_validation( this.value ) ?></td>
                            <td ><input name="txt_comment_<?=$i;?>" id="txt_comment_<?=$i;?>"  class="text_boxes" type="text"  style="width:150px"  />
...
if ($action=="actual_po_update_list_view")
{
		...
		b.actual_po_id=$actual_po_id and b.country_id=$country_id and b.inspection_status=1 and a.inspection_level=$inspection_level");
	...
	?>
			...
			<table cellpadding="0" cellspacing="1" width="100%" class="rpt_table" rules="all" id="table_list_view">
						...
						foreach($sql_data as $row){
							...
							$bal_qnty=$row[csf('actual_po_qnty')]-$cuml_insp;
							?>
                        <tr id="tr_<?=$i;?>">
							...
							<td><?   echo create_drop_down( "cbo_country_id_".$i, 100, $country_arr,"", 1, "--- Select ---", $row[csf('country_id')], "" )
							 ?></td> 
							...
							<td ><input name="txt_finishing_qnty_<?=$i;?>" id="txt_finishing_qnty_<?=$i;?>"  class="text_boxes_numeric" type="text"  style="width:80px" value="<?=$row[csf('finishing_qnty')];?>" /></td>
							...
							<td><?  echo create_drop_down( "cbo_insp_status_".$i,80, $inspection_status,"", 1, "-- Select --", $row[csf('inspection_status')], "" ); //change_cause_validation( this.value ) ?></td>
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$data_array="(".$id.",".$txt_job_no.",".$cbo_inspection_company.",".$cbo_source.",".$cbo_working_company.",".$cbo_working_location.",".$cbo_working_floor.",".$cbo_inspection_by.",".$txt_inp_date.",".$cbo_inspection_level.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
...
if($action=="show_active_listview")
{
	...
	?>
			...
			<tr bgcolor="<? echo $bgcolor;?>" id="tr_<? echo $i;?>" onClick="get_php_form_data('<? echo $row[csf('id')]."_".$row[csf('dtls_id')]."_".$row[csf('country_id')]."_".$row[csf('actual_po_id')]; ?>','populate_inspection_details_form_data','requires/inspection_entry_controller')" style="cursor:pointer;">
				<td width="35"><? echo $i;?></td>
				<td width="100"><? echo $row[csf('actual_po_no')];?></td>	
				...
				<td width="100" align="right"><? echo $row[csf('current_inspection_qnty')];?></td>
				<td width="80" align="right"><? echo $row[csf('minor_qnty')];?></td>
				<td width="80" align="right"><? echo $row[csf('major_qnty')];?></td>
				<td width="80" align="right"><? echo $row[csf('critical_qnty')];?></td>
				<td width="80" align="right"><? echo $row[csf('acceptable_qnty')];?></td>
				...
				<td width="150"><? echo $row[csf('comments')];?></td>
			</tr>
			<?
			$i++;
...
if($action=="populate_inspection_details_form_data")
{
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_drop_down( 'requires/inspection_entry_controller', '__".$row[csf("po_break_down_id")]."', 'load_drop_down_country_id', 'country_drop_down_td' );";
		echo "show_list_view('".$dtls_data."','actual_po_update_list_view','po_list_view','requires/inspection_entry_controller');\n"; 
		 
		 echo "load_drop_down( 'requires/inspection_entry_controller', $source, 'load_drop_working_company', 'working_company_td' );";
		 echo "load_drop_down( 'requires/inspection_entry_controller', $working_company, 'load_drop_down_working_location', 'working_location_td' );";
		 echo "load_drop_down( 'requires/inspection_entry_controller', $working_location, 'load_drop_down_working_floor', 'working_floor_td' );";
		...
		echo "document.getElementById('cbo_working_floor').value = '".$row[csf("working_floor")]."';\n"; 
		 ...
		 echo "load_drop_down( 'requires/inspection_entry_controller',".$row[csf("inspected_by")]."+','+document.getElementById('cbo_buyer_name').value+','+document.getElementById('cbo_company_name').value, 'load_drop_down_buyer_party_company', 'cutt_company_td' );";
		
	 	echo "get_php_form_data('".$row[csf("po_break_down_id")].",".$row[csf("week_id")].",".$row[csf("country_id")].",".$row[csf("inspection_level")].",".$row[csf("id")]."','set_po_qnty_ship_date', 'requires/inspection_entry_controller');\n";
