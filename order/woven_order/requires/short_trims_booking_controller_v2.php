...
if($action=="create_po_search_list_view")
{
	...
	$arr=array (2=>$comp,3=>$buyer_arr,9=>$item_category,10=>$yes_no);
	if ($data[2]==0)
	{
		if($db_type==0)
		{
	 	$sql= "select a.job_no_prefix_num, a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity, b.po_number,b.po_quantity,b.shipment_date,a.garments_nature,SUBSTRING_INDEX(a.insert_date, '-', 1) as year,c.approved from wo_po_details_master a, wo_po_break_down b,wo_pre_cost_mst c where a.job_no=b.job_no_mst and a.job_no=c.job_no and  b.shiping_status not in(3) and c.approved=1 and a.garments_nature=$data[5] and a.status_active=1 and b.status_active=1 $shipment_date $company $buyer $job_cond $style_cond $order_cond order by a.job_no";
		}
		if($db_type==2)
		{
	 	echo $sql= "select a.job_no_prefix_num, a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity, b.po_number,b.po_quantity,b.shipment_date,a.garments_nature,to_char(a.insert_date,'YYYY') as year,c.approved from wo_po_details_master a, wo_po_break_down b, wo_pre_cost_mst c where a.job_no=b.job_no_mst and a.job_no=c.job_no and c.approved=1 and  b.shiping_status not in(3) and a.garments_nature=$data[5] and a.status_active=1 and b.status_active=1 $shipment_date $company $buyer $job_cond $style_cond $order_cond order by a.job_no";
		}
		 echo  create_list_view("list_view", "Job No,Year,Company,Buyer Name,Style Ref. No,Job Qty.,PO number,PO Quantity,Shipment Date,Gmts Nature,Approved", "50,60,120,100,100,90,90,90,80,80,50","1000","320",0, $sql , "js_set_value", "job_no", "", 1, "0,0,company_name,buyer_name,0,0,0,0,0,garments_nature,approved", $arr , "job_no_prefix_num,year,company_name,buyer_name,style_ref_no,job_quantity,po_number,po_quantity,shipment_date,garments_nature,approved", "",'','0,0,0,0,0,1,0,1,3,0,0');
	}
	else
	{
...
if ($action=="populate_data_from_search_popup")
{
	...
	foreach ($data_array as $row)
	{
		...
		echo "get_php_form_data(".$row[csf("company_name")].", 'populate_variable_setting_data', 'requires/trims_booking_controller' );\n";
...
CASE e.costing_per WHEN 1 THEN round(((d.cons/12)*b.po_quantity)/cc.conversion_factor,4) WHEN 2 THEN round(((d.cons/1)*b.po_quantity)/cc.conversion_factor,4)  WHEN 3 THEN round(((d.cons/24)*b.po_quantity)/cc.conversion_factor,4) WHEN 4 THEN round(((d.cons/36)*b.po_quantity)/cc.conversion_factor,4) WHEN 5 THEN round(((d.cons/48)*b.po_quantity)/cc.conversion_factor,4) ELSE 0 END as req_qnty,cc.order_uom as cons_uom,IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0)  as cu_woq, CASE e.costing_per WHEN 1 THEN round((((d.cons/12)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 2 THEN round((((d.cons/1)*b.po_quantity)/cc.conversion_factor) - IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4)  WHEN 3 THEN round((((d.cons/24)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 4 THEN round((((d.cons/36)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 5 THEN round((((d.cons/48)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) ELSE 0 END as bal_woq, round((c.rate*cc.conversion_factor),8) as rate from wo_po_details_master a, wo_po_break_down b ,wo_pre_cost_mst e,wo_pre_cost_trim_cost_dtls c ,lib_item_group cc, wo_pre_cost_trim_co_cons_dtls d left join wo_booking_dtls f on f.job_no=d.job_no and f.pre_cost_fabric_cost_dtls_id=d.wo_pre_cost_trim_cost_dtls_id and f.po_break_down_id=d.po_break_down_id and f.booking_type=2   where a.job_no=b.job_no_mst and  a.job_no=c.job_no and a.job_no=e.job_no and  a.job_no=d.job_no   and c.id=d.wo_pre_cost_trim_cost_dtls_id and b.id=d.po_break_down_id and cc.id=c.trim_group  and a.company_name=$cbo_company_name $txt_job_no_cond $cbo_buyer_name_cond and a.garments_nature=$garments_nature group by d.id order by b.id) m   where id in($data) $txt_job_no_cond1  $cbo_buyer_name_cond1 and garments_nature=$garments_nature order by trim_group";//and b.pub_shipment_date between '$start_date' and '$end_date'*/ 
       ...
       <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>">
                    ...
                    <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtwoq_<? echo $i;?>" value="" onClick="open_consumption_popup('requires/short_trims_booking_controller.php?action=consumption_popup', 'Consumtion Entry Form','txtpoid_<? echo $i;?>',<? echo $i;?>)"     />
                    </td>
                    <td width="80" align="right">
                    <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtrate_<? echo $i;?>" value="<? echo $rate;?>" onChange="calculate_amount(<? echo $i; ?>)" />
...
if ($action=="set_cons_break_down")
{
	...
	if ( count($data_array)>0)
	{
		...
		foreach( $data_array as $row )
		{
			...
			$brand_supplier=$booking_data_arr[$booking_data_row[csf('color_size_table_id')]][cons];
			if($brand_supplier=="")
			{
				$brand_supplier=$txt_pre_brand_sup;
			}*/
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$field_array="id, booking_type, is_short, booking_month, booking_year, booking_no_prefix, booking_no_prefix_num, booking_no, company_id, buyer_id, job_no, 	item_category, supplier_id, currency_id, exchange_rate, booking_date, delivery_date, pay_mode, source, attention, remarks, item_from_precost, entry_form, inserted_by, insert_date";
		 ...
		 $data_array ="(".$id.",2,1,".$cbo_booking_month.",".$cbo_booking_year.",'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',".$cbo_company_name.",".$cbo_buyer_name.",".$txt_job_no.",4,".$cbo_supplier_name.",".$cbo_currency.",".$txt_exchange_rate.",".$txt_booking_date.",".$txt_delivery_date.",".$cbo_pay_mode.",".$cbo_source.",".$txt_attention.",".$txt_remarks.",".$cbo_item_from_precost.",45,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		$field_array2="id,wo_trim_booking_dtls_id,booking_no,job_no,po_break_down_id,color_number_id,gmts_sizes,description,brand_supplier,item_color,item_size,cons, process_loss_percent,requirment,rate,amount,pcs,color_size_table_id";
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			...
			// $txtcuwoq="txtcuwoq_".$i;
			 ...
			 //if ($i!=1) $data_array1 .=",";
			 $data_array1 ="(".$id_dtls.",".$$txttrimcostid.",".$$txtpoid.",".$txt_job_no.",'".$new_booking_no[0]."',2,1,".$$txttrimgroup.",".$$txtuom.",".$$cbocolorsizesensitive.",".$$txtwoq.",".$$txtexchrate.",".$$txtrate.",".$$txtamount.",".$$txtddate.",".$$consbreckdown.",".$$txtcountry.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				...
				if(str_replace("'",'',$$consbreckdown) !='')
				{
					...
					for($c=0;$c < count($consbreckdown_array);$c++)
					{
						...
						if(str_replace("'","",$consbreckdownarr[4])!="")
						{
							 ...
							 {
								  $color_id = return_id( str_replace("'","",$consbreckdownarr[4]), $color_library, "lib_color", "id,color_name","45");  
	...
	else if ($operation==1)   // Update Here
	{
		 ...
		 $data_array_up ="".$cbo_booking_month."*".$cbo_booking_year."*".$cbo_buyer_name."*".$txt_job_no."*4*".$cbo_supplier_name."*".$cbo_currency."*".$txt_exchange_rate."*".$txt_booking_date."*".$txt_delivery_date."*".$cbo_pay_mode."*".$cbo_source."*".$txt_attention."*".$txt_remarks."*".$cbo_item_from_precost."*45*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		// if  ( check_table_status( $_SESSION['menu_id'], 1 )==0 ) { echo "15**1"; die;}		
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			...
			if(str_replace("'",'',$$txtbookingid)!="")
			{
				...
				$data_array_up1[str_replace("'",'',$$txtbookingid)] =explode("*",("".$$txttrimcostid."*".$$txtpoid."*".$txt_job_no."*".$txt_booking_no."*".$$txttrimgroup."*".$$txtuom."*".$$cbocolorsizesensitive."*".$$txtwoq."*".$$txtexchrate."*".$$txtrate."*".$$txtamount."*".$$txtddate."*".$$consbreckdown."*".$$txtcountry."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
				...
				if($data_array_up1 !="")
				{
				//echo bulk_update_sql_statement( "wo_booking_dtls", "id", $field_array_up1, $data_array_up1, $id_arr ); die;
			...
			{
				 //if ($i!=1) $data_array1 .=",";
				 $data_array1 ="(".$id_dtls.",".$$txttrimcostid.",".$$txtpoid.",".$txt_job_no.",".$txt_booking_no.",2,1,".$$txttrimgroup.",".$$txtuom.",".$$cbocolorsizesensitive.",".$$txtwoq.",".$$txtexchrate.",".$$txtrate.",".$$txtamount.",".$$txtddate.",".$$consbreckdown.",".$$txtcountry.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				//	CONS break down===============================================================================================	
					if(str_replace("'",'',$$consbreckdown) !='')
					{
						$data_array_up2="";
...
if ($action=="create_booking_search_list_view")
{
	...
	if ($data[2]!=0) $supplier_id=" and supplier_id='$data[2]'"; else $supplier_id ="";
	if($db_type==0)
	{
	$booking_year_cond=" and SUBSTRING_INDEX(`insert_date`, '-', 1)=$data[5]";	
	...
	if($db_type==2)
	{
	$booking_year_cond=" and to_char(insert_date,'YYYY')=$data[5]";
	...
	if($data[7]==4 || $data[7]==0)
		{
			if (str_replace("'","",$data[6])!="") $booking_cond=" and booking_no_prefix_num like '%$data[6]%'  $booking_year_cond  "; else $booking_cond="";
		}
    if($data[7]==1)
		{
			if (str_replace("'","",$data[6])!="") $booking_cond=" and booking_no_prefix_num ='$data[6]'   "; else $booking_cond="";
		}
   if($data[7]==2)
		{
			if (str_replace("'","",$data[6])!="") $booking_cond=" and booking_no_prefix_num like '$data[6]%'  $booking_year_cond  "; else $booking_cond="";
	...
	$arr=array (1=>$comp,2=>$suplier);
	 $sql=" select booking_no_prefix_num, booking_no,company_id,supplier_id,booking_date,delivery_date from wo_booking_mst where  booking_type=2 and entry_form=45 and is_short=1     $company  $buyer  $supplier_id $booking_date $booking_cond order by id";
...
if($action=="show_trim_booking_report")
{
	...
	<div style="width:1333px" align="center">       
       <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid black">
        ...
        $nameArray=sql_select( "select a.booking_no,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.attention,a.delivery_date,a.source,a.remarks  from wo_booking_mst a where  a.booking_no=$txt_booking_no"); 
        foreach ($nameArray as $result)
        {
        ?>
       <table width="100%" style="border:1px solid black">                    	
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
			...
			$nameArray_color=sql_select( "select  b.item_color as color_number_id,b.description, b.brand_supplier,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=4 group by b.item_color,b.description, b.brand_supplier"); 
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item)
            {
				...
				$nameArray_item_description=sql_select( "select  b.description, b.brand_supplier,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=0 group by b.description, b.brand_supplier"); 
       ...
       <table width="100%" style="border:1px solid black">                    	
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item)
            {
				...
				$nameArray_item_description=sql_select( "select  b.description, b.brand_supplier,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=0 group by b.description, b.brand_supplier"); 
       ...
       //$nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=0 order by trim_group ");
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item)
            {
				...
				$nameArray_item_description=sql_select( "select  b.description, b.brand_supplier,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=0 group by b.description, b.brand_supplier"); 
