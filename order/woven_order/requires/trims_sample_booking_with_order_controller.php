...
//$trim_group_conversion_factor= return_library_array("select id,conversion_factor from lib_item_group",'id','conversion_factor');
...
function load_drop_down_supplier($data){
	if($data==5 || $data==3){
	   echo create_drop_down( "cbo_supplier_name", 172, "select id,company_name from lib_company where status_active=1 and is_deleted=0 order by company_name", "id,company_name",1, "-- Select Company --", "", "get_php_form_data( this.value, 'load_drop_down_attention', 'requires/trims_sample_booking_with_order_controller');",0,"" );
	}
	else
	{
    ...
    <table width="900" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
        ...
        <tr class="general">
            <td> 
                ...
                <? 
                echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", '',"load_drop_down( 'trims_sample_booking_with_order_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" ); ?>
            </td>
            ...
            <td align="center">
            <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('chk_job_wo_po').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('garments_nature').value+'_'+document.getElementById('txt_job_prifix').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_style').value+'_'+document.getElementById('txt_order_search').value+'_'+document.getElementById('cbo_string_search_type').value, 'create_po_search_list_view', 'search_div', 'trims_sample_booking_with_order_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" /></td>
        </tr>
        <tr>
...
if($action=="create_po_search_list_view")
{
	...
	$arr=array (2=>$comp,3=>$buyer_arr,9=>$item_category);
		...
		if (str_replace("'","",$data[5])!=0)  $fab_nature_cond="and a.garments_nature=$data[5] ";else $fab_nature_cond="";
		
	if ($data[2]==0)
	{
		if($db_type==0)
		{
	 	$sql= "select a.job_no_prefix_num, a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity, b.po_number,b.po_quantity,b.shipment_date,a.garments_nature,SUBSTRING_INDEX(a.`insert_date`, '-', 1) as year from wo_po_details_master a, wo_po_break_down b, wo_pre_cost_mst c where a.job_no=b.job_no_mst and a.job_no=c.job_no and a.status_active=1 and b.status_active=1 and c.approved=1  and b.shiping_status not in(3) $shipment_date $company $buyer ".set_user_lavel_filtering(' and a.buyer_name','buyer_id')." $job_cond $style_cond $order_cond $fab_nature_cond order by a.job_no";
		}
		if($db_type==2)
		{
	 	 $sql= "select a.job_no_prefix_num, a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity, b.po_number,b.po_quantity,b.shipment_date,a.garments_nature,to_char(a.insert_date,'YYYY') as year from wo_po_details_master a, wo_po_break_down b, wo_pre_cost_mst c where a.job_no=b.job_no_mst and a.job_no=c.job_no  and c.approved=1  and b.shiping_status not in(3) and a.status_active=1 and b.status_active=1  ".set_user_lavel_filtering(' and a.buyer_name','buyer_id')." $shipment_date $company $buyer $job_cond $style_cond $order_cond $fab_nature_cond order by a.job_no";
		}
	//echo $sql;	
	...
	else
	{
		...
		if($db_type==0)
		{
		$sql= "select a.job_no_prefix_num,a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.garments_nature,SUBSTRING_INDEX(a.`insert_date`, '-', 1) as year from wo_po_details_master a where a.job_no not in( select distinct job_no_mst from wo_po_break_down where status_active=1 and is_deleted=0 ) and a.status_active=1 ".set_user_lavel_filtering(' and a.buyer_name','buyer_id')."   and a.is_deleted=0 $company $buyer $fab_nature_cond order by a.job_no";
		}
		if($db_type==2)
		{
		$sql= "select a.job_no_prefix_num,a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.garments_nature,to_char(a.insert_date,'YYYY') as year from wo_po_details_master a where a.job_no not in( select distinct job_no_mst from wo_po_break_down where status_active=1 and is_deleted=0 ) and a.status_active=1 ".set_user_lavel_filtering(' and a.buyer_name','buyer_id')."   and a.is_deleted=0 $company $buyer $fab_nature_cond order by a.job_no";
...
{
	$data_array=sql_select("select id,garments_nature,job_no,job_no_prefix,job_no_prefix_num,company_name,buyer_name,location_name,style_ref_no,style_description,product_dept,product_code,pro_sub_dep,currency_id,agent_name,order_repeat_no,region,product_category,team_leader,dealing_marchant,packing,remarks,ship_mode,order_uom,set_break_down,gmts_item_id,total_set_qnty,set_smv,season,quotation_id,job_quantity,order_uom,avg_unit_price,currency_id,total_price from wo_po_details_master where job_no='$data'");
	foreach ($data_array as $row)
	{
		echo "load_drop_down( 'requires/trims_sample_booking_with_order_controller', '".$row[csf("company_name")]."', 'load_drop_down_buyer', 'buyer_td' );\n";
		//echo "load_drop_down( 'requires/trims_sample_booking_with_order_controller', '".$row[csf("company_name")]."', 'load_drop_down_supplier', 'supplier_td' );\n";
...
<input type="hidden" name="txt_selected_id" id="txt_selected_id" value="" />
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1267" class="rpt_table" >
                <thead>
                    <th width="40">SL</th>
            ...
            <div style="width:1267px; overflow-y:scroll; max-height:350px;" id="buyer_list_view" align="center">
			...
			CASE e.costing_per WHEN 1 THEN round(((d.cons/12)*b.po_quantity)/cc.conversion_factor,4) WHEN 2 THEN round(((d.cons/1)*b.po_quantity)/cc.conversion_factor,4)  WHEN 3 THEN round(((d.cons/24)*b.po_quantity)/cc.conversion_factor,4) WHEN 4 THEN round(((d.cons/36)*b.po_quantity)/cc.conversion_factor,4) WHEN 5 THEN round(((d.cons/48)*b.po_quantity)/cc.conversion_factor,4) ELSE 0 END as req_qnty,cc.order_uom as cons_uom,IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0)  as cu_woq, CASE e.costing_per WHEN 1 THEN round((((d.cons/12)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 2 THEN round((((d.cons/1)*b.po_quantity)/cc.conversion_factor) - IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4)  WHEN 3 THEN round((((d.cons/24)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 4 THEN round((((d.cons/36)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 5 THEN round((((d.cons/48)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) ELSE 0 END as bal_woq, round((c.rate*cc.conversion_factor),8) as rate from wo_po_details_master a, wo_po_break_down b ,wo_pre_cost_mst e,wo_pre_cost_trim_cost_dtls c ,lib_item_group cc, wo_pre_cost_trim_co_cons_dtls d left join wo_booking_dtls f on f.job_no=d.job_no and f.pre_cost_fabric_cost_dtls_id=d.wo_pre_cost_trim_cost_dtls_id and f.po_break_down_id=d.po_break_down_id and f.booking_type=2   where a.job_no=b.job_no_mst and  a.job_no=c.job_no and a.job_no=e.job_no and  a.job_no=d.job_no   and c.id=d.wo_pre_cost_trim_cost_dtls_id and b.id=d.po_break_down_id and cc.id=c.trim_group   and a.company_name=$company_id $txt_job_no_cond $cbo_buyer_name_cond and a.garments_nature=$garments_nature  group by d.id order by b.id) m  where  company_name=$company_id $txt_job_no_cond1 $cbo_buyer_name_cond1  and garments_nature=$garments_nature   and bal_woq >0 order by trim_group";//and b.pub_shipment_date between '$start_date' and '$end_date'*/
					...
					$sql_lib_item_group_array[$row_sql_lib_item_group[csf('id')]][cons_uom]=$row_sql_lib_item_group[csf('cons_uom')];
					 ...
					 if(str_replace("'","",$txt_job_no) !=''){
						...
						$trims_qty_arr=$trims->getQtyArray_by_orderCountryGmtsitemGmtscolorAndPrecostdtlsid();
   ...
   <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1567" class="rpt_table" >
       ...
       <?
			...
			CASE e.costing_per WHEN 1 THEN round(((d.cons/12)*b.po_quantity)/cc.conversion_factor,4) WHEN 2 THEN round(((d.cons/1)*b.po_quantity)/cc.conversion_factor,4)  WHEN 3 THEN round(((d.cons/24)*b.po_quantity)/cc.conversion_factor,4) WHEN 4 THEN round(((d.cons/36)*b.po_quantity)/cc.conversion_factor,4) WHEN 5 THEN round(((d.cons/48)*b.po_quantity)/cc.conversion_factor,4) ELSE 0 END as req_qnty,cc.order_uom as cons_uom,IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0)  as cu_woq, CASE e.costing_per WHEN 1 THEN round((((d.cons/12)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 2 THEN round((((d.cons/1)*b.po_quantity)/cc.conversion_factor) - IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4)  WHEN 3 THEN round((((d.cons/24)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 4 THEN round((((d.cons/36)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 5 THEN round((((d.cons/48)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) ELSE 0 END as bal_woq, round((c.rate*cc.conversion_factor),8) as rate from wo_po_details_master a, wo_po_break_down b ,wo_pre_cost_mst e,wo_pre_cost_trim_cost_dtls c ,lib_item_group cc, wo_pre_cost_trim_co_cons_dtls d left join wo_booking_dtls f on f.job_no=d.job_no and f.pre_cost_fabric_cost_dtls_id=d.wo_pre_cost_trim_cost_dtls_id and f.po_break_down_id=d.po_break_down_id and f.booking_type=2   where a.job_no=b.job_no_mst and  a.job_no=c.job_no and a.job_no=e.job_no and  a.job_no=d.job_no   and c.id=d.wo_pre_cost_trim_cost_dtls_id and b.id=d.po_break_down_id and cc.id=c.trim_group and b.pub_shipment_date between '$start_date' and '$end_date'  and a.company_name=$cbo_company_name group by d.id order by b.id) m   where id in($data) order by trim_group";*/
					...
					f.pre_cost_fabric_cost_dtls_id=e.wo_pre_cost_trim_cost_dtls_id and 
					f.po_break_down_id=e.po_break_down_id and 
					f.booking_type=5 and f.is_deleted=0
					where
					a.job_no=b.job_no and 
					a.job_no=c.job_no and 
					a.job_no=d.job_no_mst and 
                    ...
                    foreach ($nameArray as $selectResult)
                    {
						...
						if($cbo_currency*1==1)
						{
							...
							$currency_rate=set_conversion_rate( 2, $conversion_date, $cbo_company_name );
						}
						$pre_cost_rate=$rate;
						$rate=$rate*$currency_rate;
	   ?>
       <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>">
                    <td width="40" title="cbo_currency=<?=$cbo_currency?> ; conversion_date=<?=$conversion_date?> ; company_id=<?=$company_id?> ; currency_rate=<?=$currency_rate?>"><? echo $i;?></td>
                    <td width="50"><? echo $buyer_arr [$selectResult[csf('buyer_name')]];?></td>
                    ...
                    <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtwoq_<? echo $i;?>" value="<? echo $bal_woq;?>" onClick="open_consumption_popup('requires/trims_sample_booking_with_order_controller.php?action=consumption_popup', 'Consumtion Entry Form','txtpoid_<? echo $i;?>',<? echo $i;?>)"     />
                    </td>
                    ...
                    <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtrate_<? echo $i;?>" value="<? echo $rate;?>" onChange="calculate_amount(<? echo $i; ?>)" />
                     ...
                     <input type="hidden"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtrate_precost_<? echo $i;?>" value="<? echo $pre_cost_rate;?>" readonly />
...
{
	//var rowCount = $('#tbl_consmption_cost tr').length-1;
	var rowCount = $('#tbl_consmption_cost tbody tr').length*1;
...
function poportionate_qty(qty)
{
	...
	// var txtwoq_cal =number_format_common((txtwoq_qty/po_qty) * (pcs),5,0);
	  var txtwoq_cal =number_format_common(txtwoq_qty/rowCount,5,0);
	// alert(txtwoq_qty+'='+po_qty+'='+pcs);
...
<fieldset>
           ...
           <input type="hidden" id="process_loss_method_id" name="process_loss_method_id" value="<? echo $process_loss_method; ?>"/>
            	...
            	<table width="800" cellspacing="0" class="rpt_table" border="0" id="tbl_consmption_cost" rules="all">
                	<thead>
                    	<tr>
                            <th colspan="11" id="td_sync_msg" style="color:#FF0000"></th>
                        </tr>
                    	<tr>
...
CASE e.costing_per WHEN 1 THEN round(((d.cons/12)*b.po_quantity)/cc.conversion_factor,4) WHEN 2 THEN round(((d.cons/1)*b.po_quantity)/cc.conversion_factor,4)  WHEN 3 THEN round(((d.cons/24)*b.po_quantity)/cc.conversion_factor,4) WHEN 4 THEN round(((d.cons/36)*b.po_quantity)/cc.conversion_factor,4) WHEN 5 THEN round(((d.cons/48)*b.po_quantity)/cc.conversion_factor,4) ELSE 0 END as req_qnty,cc.order_uom as cons_uom,IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0)  as cu_woq, CASE e.costing_per WHEN 1 THEN round((((d.cons/12)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 2 THEN round((((d.cons/1)*b.po_quantity)/cc.conversion_factor) - IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4)  WHEN 3 THEN round((((d.cons/24)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 4 THEN round((((d.cons/36)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) WHEN 5 THEN round((((d.cons/48)*b.po_quantity)/cc.conversion_factor)- IF(f.wo_qnty != '', round(sum(f.wo_qnty),4), 0),4) ELSE 0 END as bal_woq, round((c.rate*cc.conversion_factor),8) as rate from wo_po_details_master a, wo_po_break_down b ,wo_pre_cost_mst e,wo_pre_cost_trim_cost_dtls c ,lib_item_group cc, wo_pre_cost_trim_co_cons_dtls d left join wo_booking_dtls f on f.job_no=d.job_no and f.pre_cost_fabric_cost_dtls_id=d.wo_pre_cost_trim_cost_dtls_id and f.po_break_down_id=d.po_break_down_id and f.booking_type=2   where a.job_no=b.job_no_mst and  a.job_no=c.job_no and a.job_no=e.job_no and  a.job_no=d.job_no   and c.id=d.wo_pre_cost_trim_cost_dtls_id and b.id=d.po_break_down_id and cc.id=c.trim_group  and a.company_name=$cbo_company_name $txt_job_no_cond $cbo_buyer_name_cond and a.garments_nature=$garments_nature  group by d.id order by b.id) m, wo_booking_dtls n where m.job_no=n.job_no and m.po_id=n.po_break_down_id and m.wo_pre_cost_trim_cost_dtls=n.pre_cost_fabric_cost_dtls_id  and n.booking_no=$txt_booking_no order by trim_group"; //and b.pub_shipment_date between '$start_date' and '$end_date' */
					...
					e.po_break_down_id,
					e.cons
					order by d.id,c.id) m, wo_booking_dtls n where m.job_no=n.job_no and m.po_id=n.po_break_down_id and m.wo_pre_cost_trim_cost_dtls=n.pre_cost_fabric_cost_dtls_id  and n.booking_no=$txt_booking_no and n.booking_type=5 order by m.po_id,m.wo_pre_cost_trim_cost_dtls ";
					//echo  $sql;
				   ...
				   <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>">
                    ...
                    <td width="80" align="right">
					<?// echo number_format($selectResult[csf('bal_woq')],4);?>
                    <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtwoq_<? echo $i;?>" value="<? echo $selectResult[csf('wo_qnty')];?>" onClick="open_consumption_popup('requires/trims_sample_booking_with_order_controller.php?action=consumption_popup', 'Consumtion Entry Form','txtpoid_<? echo $i;?>',<? echo $i;?>)"     />
                    </td>
                    <td width="80" align="right"><? //echo $selectResult[csf('rate')];?> 
                    <input type="text"  style="width:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtrate_<? echo $i;?>" value="<? echo number_format($selectResult[csf('rate')],4);?>" onChange="calculate_amount(<? echo $i; ?>)" readonly />
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		if($db_type==0)
		{
			...
			$new_booking_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'TSB', date("Y",time()), 5, "select booking_no_prefix, booking_no_prefix_num from wo_booking_mst where company_id=$cbo_company_name and booking_type in(5) and YEAR(insert_date)=".date('Y',time())." order by id desc ", "booking_no_prefix", "booking_no_prefix_num" ));
		}
		else if($db_type==2)
		{
			...
			$new_booking_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'TSB', date("Y",time()), 5, "select booking_no_prefix, booking_no_prefix_num from wo_booking_mst where company_id=$cbo_company_name and booking_type in(5)  and to_char(insert_date,'YYYY')=".date('Y',time())." order by id desc ", "booking_no_prefix", "booking_no_prefix_num" ));
		}
		
		$id=return_next_id( "id", "wo_booking_mst", 1 ) ;
		$field_array="id, booking_type, is_short, booking_month, booking_year, booking_no_prefix, booking_no_prefix_num, booking_no, company_id, buyer_id, job_no, item_category, supplier_id, currency_id, exchange_rate, booking_date, delivery_date, pay_mode, source, attention, tenor, fabric_source, ready_to_approved, inserted_by, insert_date";
		//print_r($new_booking_no);
	//	echo "10**=";
		//die;
		 $data_array ="(".$id.",5,2,".$cbo_booking_month.",".$cbo_booking_year.",'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',".$cbo_company_name.",".$cbo_buyer_name.",".$txt_job_no.",4,".$cbo_supplier_name.",".$cbo_currency.",".$txt_exchange_rate.",".$txt_booking_date.",".$txt_delivery_date.",".$cbo_pay_mode.",".$cbo_source.",".$txt_attention.",".$txt_tenor.",".$cbo_material_source.",".$cbo_ready_to_approved.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		 ...
		 $field_array2="id,wo_trim_booking_dtls_id,booking_no,job_no,description,po_break_down_id,color_number_id,gmts_sizes,item_color,item_size,cons,process_loss_percent,requirment,rate,amount,pcs,color_size_table_id";
		 $add_comma=0;
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			 ...
			 $txtbookingid="txtbookingid_".$i;
			  $cons_rate= str_replace("'",'',$$txtrate);
			 if ($i!=1) $data_array1 .=",";
			 $data_array1 .="(".$id_dtls.",".$$txttrimcostid.",".$$txtpoid.",".$$txtjob.",'".$new_booking_no[0]."',".$id.",5,2,".$$txttrimgroup.",".$$txtdescription.",".$$txtbrandsupref.",".$$txtuom.",".$$cbocolorsizesensitive.",".$$txtwoq.",".$$txtrate.",".$$txtamount.",".$$txtddate.",".$$consbreckdown.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				...
				if(str_replace("'",'',$$consbreckdown) !='')
				{
					...
					for($c=0;$c < count($consbreckdown_array);$c++)
					{
						...
						if(str_replace("'","",$consbreckdownarr[3])!="")
						{
							 ...
							 {
								  $color_id = return_id( str_replace("'","",$consbreckdownarr[3]), $color_library, "lib_color", "id,color_name","142");  
						...
						$req_cons= str_replace("'",'',$consbreckdownarr[5]);
						$req_amount=$cons_rate*$req_cons;
						
						if ($add_comma!=0) $data_array2 .=",";
						$data_array2 .="(".$id1.",".$id_dtls.",'".$new_booking_no[0]."',".$$txtjob.",".$$txtdescription.",'".$consbreckdownarr[0]."','".$consbreckdownarr[1]."','".$consbreckdownarr[2]."','".$color_id."','".$consbreckdownarr[4]."','".$consbreckdownarr[5]."','".$consbreckdownarr[6]."','".$consbreckdownarr[7]."','".$cons_rate."','".$req_amount."','".$consbreckdownarr[8]."','".$consbreckdownarr[9]."')";
						$id1=$id1+1;
						$add_comma++;
					}
				}
       ...
       // echo "5**insert into wo_booking_mst (".$field_array.") values ".$data_array;die;
		 ...
		 //echo "10** insert into wo_trim_book_con_dtls ($field_array2) values $data_array2";oci_rollback($con);die;
	...
	/*	}else{*/
		 ...
		 $field_array_up="booking_month*booking_year*buyer_id*job_no*item_category*supplier_id*currency_id*exchange_rate*booking_date*delivery_date*pay_mode*source*attention*tenor*fabric_source*ready_to_approved*updated_by*update_date"; 
		 
		 $data_array_up ="".$cbo_booking_month."*".$cbo_booking_year."*".$cbo_buyer_name."*".$txt_job_no."*4*".$cbo_supplier_name."*".$cbo_currency."*".$txt_exchange_rate."*".$txt_booking_date."*".$txt_delivery_date."*".$cbo_pay_mode."*".$cbo_source."*".$txt_attention."*".$txt_tenor."*".$cbo_material_source."*".$cbo_ready_to_approved."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		 ...
		 $field_array_up2="id,wo_trim_booking_dtls_id,booking_no,job_no,description,po_break_down_id,color_number_id,gmts_sizes,item_color,item_size,cons,process_loss_percent,requirment,rate,amount,pcs,color_size_table_id";
		...
		$pi_number=return_field_value( "pi_number", "com_pi_master_details a,com_pi_item_details b"," a.id=b.pi_id  and b.work_order_no=$txt_booking_no and a.item_category_id=4 and a.status_active=1 and  a.is_deleted=0 and b.order_id in($poid) and b.item_group=".$$txttrimgroup."   and b.status_active=1 and  b.is_deleted=0");
			if($pi_number){
				echo "pi1**".str_replace("'","",$txt_booking_no)."**".$pi_number;
				 disconnect($con);die;
			}
			...
			if(str_replace("'",'',$$txtbookingid)!="")
			{
				...
				if(str_replace("'",'',$$consbreckdown) !='')
				{
					...
					for($c=0;$c < count($consbreckdown_array);$c++)
					{
						...
						$data_array_up2 .="(".$id1.",".$$txtbookingid.",".$txt_booking_no.",".$$txtjob.",".$$txtdescription.",'".$consbreckdownarr[0]."','".$consbreckdownarr[1]."','".$consbreckdownarr[2]."','".$color_id."','".$consbreckdownarr[4]."','".$consbreckdownarr[5]."','".$consbreckdownarr[6]."','".$consbreckdownarr[7]."','".$cons_rate."','".$req_amount."','".$consbreckdownarr[8]."','".$consbreckdownarr[9]."')";
						$id1=$id1+1;
						$add_comma++;
					}
				}
          ...
          //CONS break down end===============================================================================================
			...
			if(str_replace("'",'',$$txtbookingid)=="")
			{
			 $data_array1 ="(".$id_dtls.",".$$txttrimcostid.",".$$txtpoid.",".$$txtjob.",".$txt_booking_no.",".$booking_id.",5,2,".$$txttrimgroup.",".$$txtdescription.",".$$txtbrandsupref.",".$$txtuom.",".$$cbocolorsizesensitive.",".$$txtwoq.",".$$txtrate.",".$$txtamount.",".$$txtddate.",".$$consbreckdown.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				//	CONS break down===============================================================================================	
				if(str_replace("'",'',$$consbreckdown) !='')
				{
					$rID_de1=execute_query( "delete from wo_trim_book_con_dtls where  wo_trim_booking_dtls_id =".$$txtbookingid."",0);
					...
					for($c=0;$c < count($consbreckdown_array);$c++)
					{
						...
						if ($add_comma!=0) $data_array_up2 .=",";
						$data_array_up2 .="(".$id1.",".$id_dtls.",".$txt_booking_no.",".$$txtjob.",".$$txtdescription.",'".$consbreckdownarr[0]."','".$consbreckdownarr[1]."','".$consbreckdownarr[2]."','".$color_id."','".$consbreckdownarr[4]."','".$consbreckdownarr[5]."','".$consbreckdownarr[6]."','".$consbreckdownarr[7]."','".$cons_rate."','".$req_amount."','".$consbreckdownarr[8]."','".$consbreckdownarr[9]."')";
						$id1=$id1+1;
						$add_comma++;
					}
				}
           ...
           <thead>
            ...
            <thead>                	 
                ...
                <th width="150">Supplier Name</th>
                 <th width="100">Booking No</th>
                <th width="200" colspan="2"> Booking Date Range</th>
                <th>&nbsp;</th>           
            </thead>
            <tr class="general">
                <td> 
                    <? 
                        echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", '', "");
                    ?>
                </td>
            <td id="buyer_td">
             <? 
            ...
            <td><input name="txt_date_to" id="txt_date_to" class="datepicker" style="width:70px"></td> 
             <td align="center">
                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_supplier_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_booking_prifix').value+'_'+document.getElementById('cbo_search_category').value, 'create_booking_search_list_view', 'search_div', 'trims_sample_booking_with_order_controller','setFilterGrid(\'list_view\',-1)')" style="width:100px;" /></td>
        </tr>
         <tr>
...
if ($action=="create_booking_search_list_view")
{
   ...
   if($data[6]==2)
	...
	$sql=" select booking_no_prefix_num, booking_no,company_id,supplier_id,booking_date,delivery_date from wo_booking_mst where booking_type=5 and is_short=2".set_user_lavel_filtering(' and buyer_id','buyer_id')." $company  $supplier_id $booking_date $booking_cond order by id desc";
	
	 echo  create_list_view("list_view", "Booking No,Company,Supplier,Booking Date,Delivery Date", "120,100,100,100","600","300",0, $sql , "js_set_value", "booking_no", "", 1, "0,company_id,supplier_id,0,0", $arr , "booking_no_prefix_num,company_id,supplier_id,booking_date,delivery_date", '','','0,0,0,3,3','','');
	exit();
}
...
function fnc_fabric_booking_terms_condition( operation )
{
		...
		http.open("POST","trims_sample_booking_with_order_controller.php",true);
...
if($action=="show_trim_booking_report")
{
	...
	<div style="width:1333px" align="center">       
       <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid black">
		...
		$job_no="";
		$currency_id="";
		//echo "select distinct b.job_no  from wo_booking_mst a, wo_booking_dtls b where a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.booking_type=5";
		$nameArray_job=sql_select( "select distinct b.job_no  from wo_booking_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.booking_type=5 and a.booking_type=b.booking_type"); 
		...
		$nameArray_buyer=sql_select( "select distinct a.buyer_name  from wo_po_details_master a, wo_booking_dtls b where a.job_no=b.job_no and b.booking_no=$txt_booking_no and b.booking_type=5"); 
		...
		$po_no="";
		$nameArray_job=sql_select( "select distinct b.po_number  from wo_booking_dtls a, wo_po_break_down b where a.po_break_down_id=b.id and a.booking_no=$txt_booking_no and a.booking_type=5"); 
        foreach ($nameArray_job as $result_job)
        {
			$po_no.=$result_job[csf('po_number')].", ";
		}
		$style_ref="";
		$nameArray_style=sql_select( "select distinct a.style_ref_no  from wo_po_details_master a, wo_booking_dtls b where a.job_no=b.job_no and b.booking_no=$txt_booking_no and b.booking_type=5"); 
        foreach ($nameArray_style as $result_style)
        {
			$style_ref.=$result_style[csf('style_ref_no')].", ";
		}
		
        $nameArray=sql_select( "select a.booking_no,a.booking_date,a.supplier_id,a.pay_mode,a.currency_id,a.exchange_rate,a.attention,a.delivery_date,a.source,a.insert_date,a.update_date  from wo_booking_mst a where  a.booking_no=$txt_booking_no and a.booking_type=5"); 
       
	    foreach ($nameArray as $result)
        {
				
			$sql_po= "select b.po_number,MIN(b.pub_shipment_date) pub_shipment_date, MIN(b.insert_date) as insert_date,b.shiping_status from wo_booking_dtls a,wo_po_break_down b  where a.po_break_down_id=b.id and a.booking_no=$txt_booking_no and a.booking_type=5 group by b.po_number, b.shiping_status"; 
       ...
       <table width="100%" style="border:1px solid black">                    	
		...
		<?
        $nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and booking_type=5 and sensitivity=1 order by trim_group "); 
        $nameArray_color=sql_select( "select distinct b.color_number_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=$txt_booking_no and a.booking_type=5 and a.sensitivity=1"); 
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            {
				$i++;
            $nameArray_item_description=sql_select( "select distinct description,brand_supplier,rate,uom from wo_booking_dtls  where booking_no=$txt_booking_no and booking_type=5 and sensitivity=1 and trim_group=".$result_item[csf('trim_group')]." order by trim_group "); 
            ?>
            <tr>
                ...
                foreach($nameArray_color  as $result_color)
                {
                $nameArray_color_size_qnty=sql_select( "select sum(b.cons) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=$txt_booking_no and a.booking_type=5 and a.sensitivity=1 and a.trim_group=". $result_item[csf('trim_group')]." and a.description='". $result_itemdescription[csf('description')]."' and b.color_number_id=".$result_color[csf('color_number_id')]."");                          
		...
		<?
        $nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and booking_type=5 and sensitivity=2 order by trim_group "); 
        $nameArray_size=sql_select( "select distinct b.item_size  as gmts_sizes from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=$txt_booking_no and a.booking_type=5 and a.sensitivity=2");
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            {
			$i++;
            $nameArray_item_description=sql_select( "select distinct description,brand_supplier,rate,uom from wo_booking_dtls  where booking_no=$txt_booking_no and booking_type=5 and sensitivity=2 and trim_group=".$result_item[csf('trim_group')]." order by trim_group "); 
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i ; ?>
                ...
                foreach($nameArray_size  as $result_size)
                {
                $nameArray_size_size_qnty=sql_select( "select sum(b.cons) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=$txt_booking_no and booking_type=5 and a.sensitivity=2 and a.trim_group=". $result_item[csf('trim_group')]." and a.description='". $result_itemdescription[csf('description')]."' and b.item_size='".$result_size[csf('gmts_sizes')]."'");                          
		...
		<?
        $nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and booking_type=5 and sensitivity=3 order by trim_group "); 
        $nameArray_color=sql_select( "select distinct b.item_color as color_number_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=$txt_booking_no and a.booking_type=5 and a.sensitivity=3"); 
		if(count($nameArray_color)>0)
		{
        ?>
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            {
				$i++;
            $nameArray_item_description=sql_select( "select distinct description,brand_supplier,rate,uom from wo_booking_dtls  where booking_no=$txt_booking_no and booking_type=5 and sensitivity=3 and trim_group=".$result_item[csf('trim_group')]." order by trim_group "); 
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
                ...
                foreach($nameArray_color  as $result_color)
                {
                $nameArray_color_size_qnty=sql_select( "select sum(b.cons) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=$txt_booking_no and a.booking_type=5 and a.sensitivity=3 and a.trim_group=". $result_item[csf('trim_group')]." and a.description='". $result_itemdescription[csf('description')]."' and b.item_color=".$result_color[csf('color_number_id')]."");                          
                foreach($nameArray_color_size_qnty as $result_color_size_qnty)
                {
                ?>
                <td style="border:1px solid black; text-align:right">
		...
		<?
        $nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and booking_type=5 and sensitivity=4 order by trim_group "); 
        $nameArray_size=sql_select( "select distinct b.item_size  as gmts_sizes from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=$txt_booking_no and a.booking_type=5 and a.sensitivity=4");
	    $nameArray_color=sql_select( "select distinct b.item_color as color_number_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=$txt_booking_no and a.booking_type=5 and a.sensitivity=4"); 
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            {
			$i++;
            $nameArray_item_description=sql_select( "select distinct description,brand_supplier,rate,uom from wo_booking_dtls  where booking_no=$txt_booking_no and booking_type=5 and sensitivity=4 and trim_group=".$result_item[csf('trim_group')]." order by trim_group "); 
            ?>
            <tr>
                ...
                foreach($nameArray_size  as $result_size)
                {
                $nameArray_size_size_qnty=sql_select( "select sum(b.cons) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=$txt_booking_no and a.booking_type=5 and a.sensitivity=4 and a.trim_group=". $result_item[csf('trim_group')]." and a.description='". $result_itemdescription[csf('description')]."' and b.item_size='".$result_size[csf('gmts_sizes')]."' and b.item_color=".$result_color[csf('color_number_id')]."");                          
                foreach($nameArray_size_size_qnty as $result_size_size_qnty)
                {
                ?>
                <td style="border:1px solid black; text-align:right">
		...
		<?
        $nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and booking_type=5 and sensitivity=0 order by trim_group "); 
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            {
				$i++;
            $nameArray_item_description=sql_select( "select distinct description,brand_supplier,rate,uom from wo_booking_dtls  where booking_no=$txt_booking_no and booking_type=5 and sensitivity=0 and trim_group=".$result_item[csf('trim_group')]." order by trim_group "); 
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
				...
				$nameArray_color_size_qnty=sql_select( "select sum(wo_qnty) as cons from wo_booking_dtls     where    booking_no=$txt_booking_no and booking_type=5 and sensitivity=0 and trim_group=". $result_item[csf('trim_group')]." and description='". $result_itemdescription[csf('description')]."'");                          
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="100%" class="rpt_table" >
       ...
       <?
                    ...
                    foreach ($nameArray as $selectResult)
                    {
						...
						//echo "select sum(amount) as amount  from wo_booking_dtls where po_break_down_id='".$selectResult[csf('po_break_down_id')]."' and pre_cost_fabric_cost_dtls_id='".$selectResult[csf('pre_cost_fabric_cost_dtls_id')]."'  and  booking_type=5 and status_active=1 and is_deleted=0";
						$sql_cu_woq=sql_select("select sum(amount) as amount  from wo_booking_dtls where po_break_down_id='".$selectResult[csf('po_break_down_id')]."' and pre_cost_fabric_cost_dtls_id='".$selectResult[csf('pre_cost_fabric_cost_dtls_id')]."'  and  booking_type=5 and status_active=1 and is_deleted=0");
...
if ($action=="populate_data_from_search_popup")
{
	 $sql= "select booking_no, booking_date, company_id, buyer_id, job_no, currency_id, exchange_rate, pay_mode, booking_month, supplier_id, attention, tenor, delivery_date, source, booking_year, ready_to_approved, is_approved, fabric_source from wo_booking_mst where booking_no='$data'";     
	 $data_array=sql_select($sql);
	 foreach ($data_array as $row)
	 {
		echo "document.getElementById('cbo_company_name').value = '".$row[csf("company_id")]."';\n";  
		echo "document.getElementById('txt_booking_no').value = '".$row[csf("booking_no")]."';\n";
		echo "load_drop_down( 'requires/trims_sample_booking_with_order_controller', '".$row[csf("company_id")]."', 'load_drop_down_buyer', 'buyer_td' );\n";
		...
		echo "load_drop_down( 'requires/trims_sample_booking_with_order_controller', '".$row[csf("pay_mode")]."', 'load_drop_down_supplier', 'supplier_td' );\n";
