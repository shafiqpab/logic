<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Knit Garments Price Quotation Entry
Functionality	:
JS Functions	:
Created by		:	Rashed
...
/*$seq="select id, quotation_id from wo_pri_quo_fabric_cost_dtls where status_active=1 and is_deleted=0 order by id asc";
$seqRes=sql_select($seq);
foreach($seqRes as $srow)
{
	$sqArr[$srow[csf("quotation_id")]][$srow[csf("id")]]=$srow[csf("seq")];
}
$i=1; $con=connect();
foreach($sqArr as $qid=>$rdata)
{
	$q=1;
	foreach($rdata as $rid=>$sdata)
	{
		$query="UPDATE wo_pri_quo_fabric_cost_dtls SET seq='$q' WHERE id='$rid' and status_active=1 and is_deleted=0";
		//echo $query."<br>";
		$rIDt=execute_query("UPDATE wo_pri_quo_fabric_cost_dtls SET seq='$q' WHERE id='$rid' and status_active=1 and is_deleted=0",1);
		$q++; $i++;
		if($rIDt==0) echo $i."=".$query.'<br>';
	}
}
...
// print_r($_SESSION['logic_erp']['mandatory_field'][314]) ;die;
...
var field_level_data="";
	<?
		if(isset($_SESSION['logic_erp']['data_arr'][314])){
	    ...
	    echo "var field_message = '". implode('*',$_SESSION['logic_erp']['field_message'][314]) . "';\n";
	?>

	<?php
	if($_SESSION['logic_erp']['mandatory_field'][314]!="")
	{
		$mandatory_field_arr= json_encode( $_SESSION['logic_erp']['mandatory_field'][314] );
		echo "var mandatory_field_arr= ". $mandatory_field_arr . ";\n";

		// condition for txt internal File No
		$temp_mandatory_field_arr = $_SESSION['logic_erp']['mandatory_field'][314];
		unset($temp_mandatory_field_arr[3]);
		$temp_mandatory_message_arr = $_SESSION['logic_erp']['mandatory_message'][314];
		unset($temp_mandatory_message_arr[3]);
	}
	?>


	//alert(mandatory_field+'=='+field_message)
	...
	var str_factory = [<?=substr(return_library_autocomplete( "select factory from  wo_price_quotation group by factory", "factory"), 0, -1); ?>];
	// Common For All----------------------------------------------------
	...
	function show_sub_form(update_id, action, extra_str)
	{
		...
		else
		{
			...
			var yarn_needed=0; var txtconsumptionyarn_sum=0; var txtknit_sum=0; var txtconreqnty_sum=0;
			var yarn_needed=$("#tot_yarn_needed").val()*1; var txtconsumptionyarn_sum=$("#txtconsumptionyarn_sum").attr('pre_commis_cost')*1;
			var txtknit_sum=$("#txtknit_sum").val()*1; var txtconreqnty_sum=$("#txtconreqnty_sum").val()*1;
			console.log(comms_cost+'--'+pre_comms_cost+'--'+action);
			...
			//alert((yarn_needed*1)+'_'+(txtconsumptionyarn_sum*1));
			/*if((yarn_needed!=txtconsumptionyarn_sum) && action=="show_fabric_cost_listview")
			{
				alert("Fabric Cost Change Found, Please Save or Update.");
				release_freezing();
				return;
			}
			if((txtknit_sum!=txtconreqnty_sum) && action=="show_fabric_cost_listview")
			{
				alert("Fabric Cost Change Found, Please Save or Update.");
				release_freezing();
				return;
			...
			if((comml_cost!=pre_comml_cost) && action!="show_comarcial_cost_listview")
			{
			...
			{
				show_list_view(update_id+'_'+document.getElementById('cbo_company_name').value,action,'cost_container','requires/quotation_entry_controller','');
				sum_yarn_required();
			...
			if(action=="show_trim_cost_listview"){
				...
				extra_str=2;//ISD-23-11387
				show_list_view(update_id+'*'+extra_str+'*'+document.getElementById('cbo_company_name').value+'*'+document.getElementById('cbo_costing_per').value,action,'cost_container','requires/quotation_entry_controller','');
			...
			if(action=="show_embellishment_cost_listview"){
				show_list_view(update_id,action,'cost_container','requires/quotation_entry_controller','');
	...
	function fn_deletebreak_down_tr(rowNo,table_id,tr)
	{
		...
		var is_job_pre_cost=return_global_ajax_value(update_id, 'validate_is_job_pre_cost', '', 'requires/quotation_entry_controller');
				...
				if(r==true)
				{
					...
					for(i = rowNo;i <= numRow;i++){
						$("#tbl_trim_cost tr:eq("+i+")").find("input,select").each(function() {
							...
							$('#txttrimexper_'+i).removeAttr("onChange").attr("onChange","calculate_trim_cost( "+i+" )");
							$('#txttotCons_'+i).removeAttr("onChange").attr("onChange","calculate_trim_cost( "+i+" )");
	...
	function add_break_down_tr(i,tr){
		var row_num=$('#tbl_fabric_cost tbody tr').length;
		if (i==0){
			i=1;
			$("#txtconstruction_"+i).autocomplete({
				source: str_construction
	...
	function open_consumption_popup(page_link,title,body_part_id,cbofabricnature_id,txtgsmweight_id,trorder,updateid_fc){
		...
		var cbouom=document.getElementById('cbouom_'+trorder).value;
		...
		var cbowidthdiatype = document.getElementById('cbowidthdiatype_'+trorder).value;
	...
	function fnc_fabric_cost_dtls( operation )
	{
		...
		var data_component=fnc_quotation_entry_component();
		//alert(data_component); release_freezing(); return;
		for (var i=1; i<=row_num; i++){
			...
			var composition=encodeURIComponent($('#txtcomposition_'+i).val());
			var txtconstruction=encodeURIComponent($('#txtconstruction_'+i).val());
			var markerbreackdown=encodeURIComponent($('#markerbreackdown_'+i).val());
			
			data_all+="&seq_" + z + "='" + $('#seq_'+i).val()+"'"+"&consumptionbasis_" + z + "='" + $('#consumptionbasis_'+i).val()+"'"+"&cbogmtsitem_" + z + "='" + $('#cbogmtsitem_'+i).val()+"'"+"&cbouom_" + z + "='" + $('#cbouom_'+i).val()+"'"+"&txtbodypart_" + z + "='" + $('#txtbodypart_'+i).val()+"'"+"&txtbodyparttype_" + z + "='" + $('#txtbodyparttype_'+i).val()+"'"+"&cbofabricnature_" + z + "='" + $('#cbofabricnature_'+i).val()+"'"+"&cbocolortype_" + z + "='" + $('#cbocolortype_'+i).val()+"'"+"&libyarncountdeterminationid_" + z + "='" + $('#libyarncountdeterminationid_'+i).val()+"'"+"&oldlibyarncountdeterminationid_" + z + "='" + $('#oldlibyarncountdeterminationid_'+i).val()+"'"+"&fabricdescription_" + z + "='" + fabricdescription+"'"+"&txtcomposition_" + z + "='" + composition+"'"+"&txtconstruction_" + z + "='" + txtconstruction+"'"+"&txtgsmweight_" + z + "='" + $('#txtgsmweight_'+i).val()+"'"+"&txtconsumption_" + z + "='" + $('#txtconsumption_'+i).val()+"'"+"&cbofabricsource_" + z + "='" + $('#cbofabricsource_'+i).val()+"'"+"&txtrate_" + z + "='" + $('#txtrate_'+i).val()+"'"+"&txtamount_" + z + "='" + $('#txtamount_'+i).val()+"'"+"&txtfinishconsumption_" + z + "='" + $('#txtfinishconsumption_'+i).val()+"'"+"&txtavgprocessloss_" + z + "='" + $('#txtavgprocessloss_'+i).val()+"'"+"&cbostatus_" + z + "='" + $('#cbostatus_'+i).val()+"'"+"&consbreckdown_" + z + "='" + $('#consbreckdown_'+i).val()+"'"+"&msmntbreackdown_" + z + "='" + $('#msmntbreackdown_'+i).val()+"'"+"&yarnbreackdown_" + z + "='" + $('#yarnbreackdown_'+i).val()+"'"+"&updateid_" + z + "='" + $('#updateid_'+i).val()+"'"+"&cbowidthdiatype_" + z + "='" + $('#cbowidthdiatype_'+i).val()+"'"+"&markerbreackdown_" + z + "='" + markerbreackdown+"'"+"&processlossmethod_" + z + "='" + $('#processlossmethod_'+i).val()+"'";
			//data_all2+="&fabricdescription_" + z + "=" + fabricdescription+"&txtcomposition_" + z + "=" +composition+"&txtconstruction" + z + "=" + txtconstruction+"";
			z++;
		}
		
		var data="action=save_update_delet_fabric_cost_dtls&operation="+operation+'&total_row='+row_num+get_submitted_data_string('cbo_company_name*cbo_costing_per*tot_yarn_needed*txtwoven_sum*txtknit_sum*txtamount_sum*update_id',"../../")+data_all+"&data_component="+data_component;
	...
	function fnc_fabric_cost_dtls_reponse(){		
		if(http.readyState == 4){
			...
			if(reponse[0]==0 || reponse[0]==1)
			{
				...
				if(update_id!=0)
				{
					var sum_fab_cost=return_global_ajax_value(update_id+'_0', 'sum_fabric_for_incentives_value', '', 'requires/quotation_entry_controller');
					var amount=number_format_common(sum_fab_cost, 1, 0, currency);
					$("#txt_fabric_pre_cost").attr('pre_fab_cost',amount);
					$('#txt_fabric_pre_cost').val(amount);
				}
				calculate_main_total();
			}
			...
			if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2){
				update_related_data(trim(reponse[0]))
	...
	function fnc_fabric_yarn_cost_dtls( operation )
	{
		...
		for (var i=1; i<=row_num; i++){
			...
			//data_all=data_all+get_submitted_data_string('update_id*cbocount_'+i+'*cbocompone_'+i+'*percentone_'+i+'*cbotype_'+i+'*cboyarnfinish_'+i+'*cboyarnsnippingsystem_'+i+'*consratio_'+i+'*consqnty_'+i+'*supplier_'+i+'*txtrateyarn_'+i+'*txtamountyarn_'+i+'*cbostatusyarn_'+i+'*updateidyarncost_'+i+'*txtconsumptionyarn_sum*txtamountyarn_sum',"../../");
			
			data_all+="&cbocount_" + z + "='" + $('#cbocount_'+i).val()+"'"+"&cbocompone_" + z + "='" + $('#cbocompone_'+i).val()+"'"+"&percentone_" + z + "='" + $('#percentone_'+i).val()+"'"+"&cbotype_" + z + "='" + $('#cbotype_'+i).val()+"'"+"&cboyarnfinish_" + z + "='" + $('#cboyarnfinish_'+i).val()+"'"+"&cboyarnsnippingsystem_" + z + "='" + $('#cboyarnsnippingsystem_'+i).val()+"'"+"&consratio_" + z + "='" + $('#consratio_'+i).val()+"'"+"&consqnty_" + z + "='" + $('#consqnty_'+i).val()+"'"+"&supplier_" + z + "='" + $('#supplier_'+i).val()+"'"+"&txtrateyarn_" + z + "='" + $('#txtrateyarn_'+i).val()+"'"+"&txtamountyarn_" + z + "='" + $('#txtamountyarn_'+i).val()+"'"+"&cbostatusyarn_" + z + "='" + $('#cbostatusyarn_'+i).val()+"'"+"&updateidyarncost_" + z + "='" + $('#updateidyarncost_'+i).val()+"'";
			z++;
		}
		
		var data="action=save_update_delet_fabric_yarn_cost_dtls&operation="+operation+'&total_row='+row_num+get_submitted_data_string('cbo_company_name*update_id*txtconsumptionyarn_sum*txtamountyarn_sum',"../../")+data_all;
	...
	function add_break_down_tr_conversion_cost( i,conversion_from_chart,tr ){
		...
		/*if (row_num!=i){
			...
			//$('#fabtd_'+i).removeAttr("title","");
			//$('#fabtd_'+i).attr( 'title','' );
			$('#fabtd_'+i).css({ 'background': 'white' });
			set_sum_value( 'txtconreqnty_sum', 'txtreqqnty_', 'tbl_conversion_cost' );
			set_sum_value( 'txtconchargeunit_sum', 'txtchargeunit_', 'tbl_conversion_cost' );
			set_sum_value( 'txtconamount_sum', 'txtamountconversion_', 'tbl_conversion_cost' );
		//}
	...
	function set_conversion_qnty(i)
	{
		...
		if(cbocosthead !=0){
			var conversion_qnty=return_global_ajax_value(cbocosthead+"_"+cbotypeconversion+"_"+txtprocessloss, 'set_conversion_qnty', '', 'requires/quotation_entry_controller');
			conversion_qnty=conversion_qnty.split("_");
			document.getElementById('covseq_'+i).value=conversion_qnty[2];
		}
		...
		if(conversion_qnty[1]=="") conversion_qnty[1]=txtprocessloss;

		if(conversion_qnty[1]==0 && cbotypeconversion>0){
	...
	function charge_unit_color_popup(i,conversion_from_chart)
	{
		...
		var coversionchargelibraryid=document.getElementById('coversionchargelibraryid_'+i).value;
		var page_link="requires/quotation_entry_controller.php?action=conversion_color_popup&cbocosthead="+cbocosthead+'&job_no='+job_no+'&colorbreakdown='+colorbreakdown+'&conversion_from_chart='+conversion_from_chart+'&cbo_company_name='+cbo_company_name+'&cbotypeconversion='+cbotypeconversion+'&txt_exchange_rate='+txt_exchange_rate+'&cbo_currercy='+cbo_currercy+'&coversionchargelibraryid='+coversionchargelibraryid;
	...
	function fnc_fabric_conversion_cost_dtls( operation )
	{
		...
		for (var i=1; i<=row_num; i++){
			...
			data_all+="&cbocosthead_" + z + "='" + $('#cbocosthead_'+i).val()+"'"+"&cbotypeconversion_" + z + "='" + $('#cbotypeconversion_'+i).val()+"'"+"&txtprocessloss_" + z + "='" + $('#txtprocessloss_'+i).val()+"'"+"&txtreqqnty_" + z + "='" + $('#txtreqqnty_'+i).val()+"'"+"&txtchargeunit_" + z + "='" + $('#txtchargeunit_'+i).val()+"'"+"&txtamountconversion_" + z + "='" + $('#txtamountconversion_'+i).val()+"'"+"&cbostatusconversion_" + z + "='" + $('#cbostatusconversion_'+i).val()+"'"+"&updateidcoversion_" + z + "='" + $('#updateidcoversion_'+i).val()+"'"+"&coversionchargelibraryid_" + z + "='" + $('#coversionchargelibraryid_'+i).val()+"'";
			z++;
		}
		
		var data="action=save_update_delet_fabric_conversion_cost_dtls&operation="+operation+'&total_row='+row_num+get_submitted_data_string('cbo_company_name*update_id*txtconreqnty_sum*txtconchargeunit_sum*txtconamount_sum',"../../")+data_all;
	...
	function calculate_trim_cost(i){
		...
		var trimcons=$('#txtconsdzngmts_'+i).val()*1;
		var trimexper=$('#txttrimexper_'+i).val()*1;
		
		if(trimexper>0) 
		{
			var totcons=((trimexper/100)*trimcons)+trimcons;
			$('#txttotCons_'+i).val( number_format_common(totcons, 1, 0,document.getElementById('cbo_currercy').value) );
		}
		else $('#txttotCons_'+i).val( number_format_common(trimcons, 1, 0,document.getElementById('cbo_currercy').value) );
		
		math_operation( 'txttrimamount_'+i, 'txttotCons_'+i+'*txttrimrate_'+i, '*','',{dec_type:1,comma:0,currency:document.getElementById('cbo_currercy').value} );
		set_sum_value( 'txtconsdzntrim_sum', 'txttotCons_', 'tbl_trim_cost' );
		set_sum_value( 'txtratetrim_sum', 'txttrimrate_', 'tbl_trim_cost' );
		set_sum_value( 'txttrimamount_sum', 'txttrimamount_', 'tbl_trim_cost' );
		//calculate_cofirm_price_commision()
	}
	...
	function trim_rate_popup(i){
		var cbogroup=document.getElementById('cbogroup_'+i).value;
		var page_link="requires/quotation_entry_controller.php?cbogroup="+trim(cbogroup)+"&action=trim_rate_popup_page";
...
function fnc_trim_cost_dtls( operation )
{
	...
	for (var i=1; i<=row_num; i++)
	{
		...
		data_all+="&cbogroup_" + z + "='" + $('#cbogroup_'+i).val()+"'"+"&txtdescription_" + z + "='" + $('#txtdescription_'+i).val()+"'"+"&cboconsuom_" + z + "='" + $('#cboconsuom_'+i).val()+"'"+"&txtconsdzngmts_" + z + "='" + $('#txtconsdzngmts_'+i).val()+"'"+"&txttrimexper_" + z + "='" + $('#txttrimexper_'+i).val()+"'"+"&txttotCons_" + z + "='" + $('#txttotCons_'+i).val()+"'"+"&txttrimrate_" + z + "='" + $('#txttrimrate_'+i).val()+"'"+"&txttrimamount_" + z + "='" + $('#txttrimamount_'+i).val()+"'"+"&cboapbrequired_" + z + "='" + $('#cboapbrequired_'+i).val()+"'"+"&cbonominasupplier_" + z + "='" + $('#cbonominasupplier_'+i).val()+"'"+"&cbotrimstatus_" + z + "='" + $('#cbotrimstatus_'+i).val()+"'"+"&updateidtrim_" + z + "='" + $('#updateidtrim_'+i).val()+"'"+"&seq_" + z + "='" + $('#seq_'+i).val()+"'";
		z++;
	}
	
	var data="action=save_update_delet_trim_cost_dtls&operation="+operation+'&total_row='+row_num+get_submitted_data_string('cbo_company_name*update_id*txtconsdzntrim_sum*txtratetrim_sum*txttrimamount_sum',"../../")+data_all;
...
function fnc_embellishment_cost_dtls( operation )
{
	...
	for (var i=1; i<=row_num; i++){
		...
		// 	return;
		// }
		//alert("Select r "+row_num+ " Type");
		var cboembname=document.getElementById('cboembname_'+i).value;
		//alert("Select t "+cboembname+ " Type");
		var cboembtype=document.getElementById('cboembtype_'+i).value;
		//alert("Select u "+cboembtype+ " Type");
		var txtembconsdzngmts=document.getElementById('txtembconsdzngmts_'+i).value*1;
		//alert("Select y "+cboembname+ " Type "+txtembconsdzngmts+"="+cboembname);
		
		if(cboembname >0 && txtembconsdzngmts >0 && cboembtype==0){
			//alert("Select "+cboembname+ " Type");
			if (form_validation('cboembtype_'+i,'Type')==false){
				release_freezing();
				return;
			}
			release_freezing();
		...
		data_all+="&cboembname_" + z + "='" + $('#cboembname_'+i).val()+"'"+"&cboembtype_" + z + "='" + $('#cboembtype_'+i).val()+"'"+"&txtembconsdzngmts_" + z + "='" + $('#txtembconsdzngmts_'+i).val()+"'"+"&txtembrate_" + z + "='" + $('#txtembrate_'+i).val()+"'"+"&txtembamount_" + z + "='" + $('#txtembamount_'+i).val()+"'"+"&cboembstatus_" + z + "='" + $('#cboembstatus_'+i).val()+"'"+"&embupdateid_" + z + "='" + $('#embupdateid_'+i).val()+"'";
		z++;
	}
	//alert(data_all);
	
	var data="action=save_update_delet_embellishment_cost_dtls&operation="+operation+'&total_row='+row_num+get_submitted_data_string('cbo_company_name*update_id*txtamountemb_sum',"../../")+data_all;
...
function fnc_wash_cost_dtls( operation )
{
	...
	for (var i=1; i<=row_num; i++){
		...
		data_all+="&cboembname_" + z + "='" + $('#cboembname_'+i).val()+"'"+"&cboembtype_" + z + "='" + $('#cboembtype_'+i).val()+"'"+"&txtembconsdzngmts_" + z + "='" + $('#txtembconsdzngmts_'+i).val()+"'"+"&txtembrate_" + z + "='" + $('#txtembrate_'+i).val()+"'"+"&txtembamount_" + z + "='" + $('#txtembamount_'+i).val()+"'"+"&cboembstatus_" + z + "='" + $('#cboembstatus_'+i).val()+"'"+"&embupdateid_" + z + "='" + $('#embupdateid_'+i).val()+"'"+"&embratelibid_" + z + "='" + $('#embratelibid_'+i).val()+"'";
		z++;
	}
	
	var data="action=save_update_delet_wash_cost_dtls&operation="+operation+'&total_row='+row_num+get_submitted_data_string('cbo_company_name*update_id*txtamountemb_sum',"../../")+data_all+"&data_component="+data_component;
...
function fnc_commission_cost_dtls( operation )
{
	...
	var data="action=save_update_delet_commission_cost_dtls&operation="+operation+'&total_row='+row_num+get_submitted_data_string('cbo_company_name*update_id*txtratecommission_sum*txtamountcommission_sum',"../../")+data_all;
...
function fnc_commission_cost_dtls1( operation )
{
	...
	for (var i=1; i<=row_num; i++){
		...
		data_all+="&cboparticulars_" + z + "='" + $('#cboparticulars_'+i).val()+"'"+"&cbocommissionbase_" + z + "='" + $('#cbocommissionbase_'+i).val()+"'"+"&txtcommissionrate_" + z + "='" + $('#txtcommissionrate_'+i).val()+"'"+"&txtcommissionamount_" + z + "='" + $('#txtcommissionamount_'+i).val()+"'"+"&cbocommissionstatus_" + z + "='" + $('#cbocommissionstatus_'+i).val()+"'"+"&commissionupdateid_commissionupdateid_" + z + "='" + $('#commissionupdateid_'+i).val()+"'";
		z++;
	}
...
function calculate_insentives_cost(i,type){
	//var txt_commercial_cost_method=document.getElementById('txt_commercial_cost_method').value*1;
	var update_id=document.getElementById('update_id').value;
	var currency=(document.getElementById('cbo_currercy').value)*1;
	var incentivesrate=(document.getElementById('txt_incentives_rate').value)*1;
	var incentives_preamount=(document.getElementById('txt_incentives_pre_cost').value)*1;
	var txt_commercial_cost_method=1;

	var amount=0;
	if(update_id!=0)
	{
		var sum_fab_cost=return_global_ajax_value(update_id+'_'+txt_commercial_cost_method, 'sum_fabric_for_incentives_value', '', 'requires/quotation_entry_controller');
		var amount=number_format_common(sum_fab_cost, 1, 0, currency);
	}
	
	
	if(type=='rate'){
		var incentive_amount=amount*(incentivesrate/100);
		document.getElementById('txt_incentives_pre_cost').value=number_format_common(incentive_amount, 1, 0, currency);
	}
	if(type=='amount'){
		var incentive_rate=(incentives_preamount*100)/amount;
		//alert(amount);
		document.getElementById('txt_incentives_rate').value=number_format_common(incentive_rate, 1, 0, currency);
	}
...
function calculate_comarcial_cost(i,type){
	...
	var amount=0;
	if(txt_commercial_cost_method!=0)
	{
...
function fnc_quotation_entry_dtls2( operation )
{
	...
	if(txt_confirm_price_pre_cost =="" && txt_confirm_date_pre_cost!=""){
		...
		var data="action=save_update_delete_quotation_entry_dtls&operation="+operation+get_submitted_data_string('update_id*cbo_currercy*cbo_company_name*cbo_costing_per*cbo_order_uom*update_id_dtls*txt_fabric_pre_cost*txt_fabric_po_price*txt_trim_pre_cost*txt_trim_po_price*txt_embel_pre_cost*txt_embel_po_price*txt_wash_pre_cost*txt_wash_po_price*txt_comml_pre_cost*txt_comml_po_price*txt_lab_test_pre_cost*txt_lab_test_po_price*txt_inspection_pre_cost*txt_inspection_po_price*txt_cm_pre_cost*txt_cm_po_price*txt_freight_pre_cost*txt_freight_po_price*txt_currier_pre_cost*txt_currier_po_price*txt_certificate_pre_cost*txt_certificate_po_price*txt_common_oh_pre_cost*txt_common_oh_po_price*txt_depr_amor_pre_cost*txt_depr_amor_po_price*txt_interest_pre_cost*txt_interest_po_price*txt_income_tax_pre_cost*txt_income_tax_po_price*txt_total_pre_cost*txt_total_po_price*txt_commission_pre_cost*txt_commission_po_price*txt_final_cost_dzn_pre_cost*txt_final_cost_dzn_po_price*txt_final_cost_pcs_po_price*txt_final_cost_set_pcs_rate*txt_1st_quoted_price_pre_cost*txt_1st_quoted_po_price*txt_first_quoted_price_date*txt_revised_price_pre_cost*txt_revised_price_date*txt_confirm_price_pre_cost*txt_confirm_price_set_pcs_rate*txt_confirm_price_pre_cost_dzn*txt_confirm_price_po_price_dzn*txt_margin_dzn_pre_cost*txt_margin_dzn_po_price*txt_confirm_date_pre_cost*txt_asking_quoted_price*txt_asking_quoted_po_price*txt_with_commission_pre_cost_dzn*txt_with_commission_po_price_dzn*txt_with_commission_pre_cost_pcs*txt_with_commission_po_price_pcs*txt_terget_qty*txt_studio_pre_cost*txt_design_pre_cost*txt_studio_po_price*txt_design_po_price*txt_incentives_rate*txt_incentives_pre_cost*txt_incentives_po_price*interest_breack_down',"../../");
		...
		http.onreadystatechange = function(){
			if(http.readyState == 4 && http.status == 200) {
				...
				show_msg(reponse[0]);
				var txt_commission_pre_cost=document.getElementById('txt_commission_pre_cost').value*1;
				if(txt_commission_pre_cost != reponse[3]){
					var commission_cost_new=reponse[3]*1;
					document.getElementById('txt_commission_pre_cost').value=number_format (commission_cost_new, 6,'.' , "") ;
				}
...
function fnc_comarcial_cost_dtls( operation )
{
	...
	var data="action=save_update_delet_comarcial_cost_dtls&operation="+operation+'&total_row='+row_num+get_submitted_data_string('cbo_company_name*update_id*txtratecomarcial_sum*txtamountcomarcial_sum',"../../")+data_all;
...
function openmypage_inquery(){
	...
	emailwindow.onclose=function(){
		...
		if (theemail!=""){
			...
			document.getElementById('cbo_location_id').value=theemail[6];
...
function open_set_popup(unit_id){
	...
	var txt_sew_smv=document.getElementById('txt_sew_smv').value;
	var txt_ws_id=document.getElementById('txt_ws_id').value;

	var page_link="requires/quotation_entry_controller.php?txt_quotation_id="+trim(txt_quotation_id)+"&action=open_set_list_view&set_breck_down="+set_breck_down+"&tot_set_qnty="+tot_set_qnty+'&unit_id='+unit_id+'&txt_inquery_id='+txt_inquery_id+'&set_smv_id='+set_smv_id+'&txt_style_ref='+txt_style_ref+'&cbo_company_name='+cbo_company_name+'&cbo_buyer_name='+cbo_buyer_name+'&item_id='+item_id+'&txt_sew_smv='+txt_sew_smv+'&txt_ws_id='+txt_ws_id;
...
{
	/*if(($("#hidd_is_dtls_open").val())==2)// hide by ISD-23-28454
	{
	...
	var cm_editable=$("#cm_cost_editable").val()*1;
	 txt_ws_id=$("#txt_ws_id").val();
	
	var set_breck_down=$("#set_breck_down").val()*1;
	if(set_breck_down=='')
	{
		alert('Please Item Details popup click and close.');
		release_freezing();
		return;	
	}
	//alert(cm_editable);
	...
	if(txt_comml_pre_cost_4disit != 0 && ready_to_approved==1){
		var txt_comml_pre_cost=document.getElementById('txt_comml_pre_cost').value*1;
		var page_comml_pre_cost = number_format (txt_comml_pre_cost, 4,'.' , "") ;	
		var fab_cost=$("#txt_fabric_pre_cost").val()*1;     var pre_fab_cost=$("#txt_fabric_pre_cost").attr('pre_fab_cost')*1;
		if(fab_cost==pre_fab_cost){
			console.log(txt_comml_pre_cost_4disit+'--'+page_comml_pre_cost);
			if( txt_comml_pre_cost_4disit != 0 && txt_comml_pre_cost_4disit != page_comml_pre_cost){
				alert("Comml. Cost Change Found, Please Save or Update.");
	...
	if('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][314]);?>'){
		if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][314]);?>','<? echo implode('*',$_SESSION['logic_erp']['field_message'][314]);?>')==false)
		{
			return;
		}
	}
	...
	if(operation==0 || operation==1)
	{
		if(ready_to_approved==1)
		{
			...
			var response=detail_part.split('**');
			//alert(response[0]);
...
function fnc_quotation_entry_reponse(){
	if(http.readyState == 4) {
		...
		if(reponse[0]=='jobfound')
		{
			alert("Order Entry Done. So Delete Not Allowed."+reponse[1]);
			release_freezing();
			return;
		}
		if(reponse[0]=='approved')
		...
		if(reponse[0]==0 || reponse[0]==1){
			...
			//show_list_view(reponse[1],'show_fabric_cost_listview','cost_container','../woven_order/requires/quotation_entry_controller','');// hide by ISD-23-28454
			release_freezing();
			uploadFile(reponse[1]);
		}
...
function uploadFile(mst_id){
	...
	$(document).ready(function() {
		...
		$.ajax({ 
			url: 'requires/quotation_entry_controller.php?action=file_upload&mst_id='+ mst_id, 
			type: 'post', 
			data: fd, 
			contentType: false, 
			processData: false, 
...
function copy_quatation(operation){
	...
	function style_from_library(data,smv_source_id){
		//alert(smv_source_id);
		if(data == 1 && smv_source_id==2){
			$('#txt_style_ref').removeAttr("onDblClick").attr("onDblClick","open_style_ref_popup(2)");
			$('#txt_style_ref').prop('readonly', true);
			$('#txt_style_ref').val("");
			$('#txt_style_ref_id').val("");
			$('#txt_style_ref').attr('placeholder','Browse');
		}
		else if(smv_source_id==4){
			$('#txt_style_ref').removeAttr("onDblClick").attr("onDblClick","open_style_ref_popup(4)");
			$('#txt_style_ref').prop('readonly', true);
			$('#txt_style_ref').val("");
			$('#txt_style_ref_id').val("");
			$('#txt_style_ref').attr('placeholder','Browse');
	...
	function open_style_ref_popup(smv_source_id)
	{
		...
		if(smv_source_id==4)
		{
			var page_link='requires/quotation_entry_controller.php?action=ws_style_ref_popup&txt_style_ref='+txt_style_ref+'&txt_style_ref_id='+txt_style_ref_id+'&buyer_name='+buyer_name+'&cbo_company_name='+cbo_company_name;
			//alert(page_link);
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, 'Style Description', 'width=680px,height=320px,center=1,resize=1,scrolling=0','../');
		}
		else
		{
		...
		emailwindow.onclose=function()
		{
			...
			else
			{
				...
				$('#cbo_order_uom').attr('disabled',false);
				$('#cbo_pord_dept').attr('disabled',false);
	...
	function fnc_disable_filed(type)
	{
		//alert(type);
		$('#cbo_buyer_name').attr('disabled',true);
		$('#cbo_order_uom').attr('disabled',true);
	...
	function calculate_final_cost_pcs()
	{
		...
		var txt_final_cost_dzn_po_price=document.getElementById('txt_final_cost_dzn_po_price').value;
		 if(txt_final_cost_dzn_po_price==0){
			asking_profit_percent=return_global_ajax_value(company_id+"_"+txt_quotation_date, 'asking_profit_percent', '', 'requires/quotation_entry_controller');
			document.getElementById('txt_final_cost_dzn_po_price').value=asking_profit_percent;
			//Some one was off this code-Live problem found for it.
		}
		else{
...
function calculate_confirm_price_dzn()
{
	...
	//For Ontime calculate intarest cost
	var interest_break_down=$('#interest_breack_down').val();
	if(interest_break_down !='' && txt_confirm_price_pre_cost_dzn>0){
		var ibreak_down=interest_break_down.split('_');
		if(ibreak_down[0]>0 && txt_confirm_price_pre_cost_dzn!=ibreak_down[1]){
			var interest_amt=txt_confirm_price_pre_cost_dzn*(ibreak_down[0]/100);
			var new_break_down=ibreak_down[0]+'_'+txt_confirm_price_pre_cost_dzn+'_'+interest_amt;
			$('#txt_interest_pre_cost').val(interest_amt);
			$('#interest_breack_down').val(new_break_down);
			calculate_main_total();
		}
	}
	//For Ontime calculate Income tex cost
	var cost_per_break_down=$('#cost_per_minute').val();
	var income_tax_curr=$("#txt_income_tax_pre_cost").val();
	if(cost_per_break_down !='' && txt_confirm_price_pre_cost_dzn>0){
		var incometax_value =0;
		var income_text_arr=cost_per_break_down.split('_');
		var incometexvalue=income_text_arr[7];
		if(incometexvalue=="") incometexvalue=0;
		if(incometexvalue>0)
		{
			incometax_value=(txt_confirm_price_pre_cost_dzn*incometexvalue)/100;
			if(incometexvalue>0)
				$('#txt_income_tax_pre_cost').attr('disabled','true');
			else
				$('#txt_income_tax_pre_cost').removeAttr('disabled');
				
			$("#txt_income_tax_pre_cost").val( number_format_common(incometax_value,1,0,currency) );
			if(number_format_common(income_tax_curr,1,0,currency)!=number_format_common(incometax_value,1,0,currency)){
				calculate_main_total();
			}
...
function calculate_percent_on_po_price(){
	...
	var txt_confirm_price_pre_cost_dzn=(document.getElementById('txt_confirm_price_pre_cost_dzn').value)*1;
	var total_cost=(document.getElementById('txt_total_pre_cost').value)*1-(document.getElementById('txt_comml_pre_cost').value)*1;
	var percentage_calculation=(document.getElementById('percentage_calculation').value)*1;
	...
	if(percentage_calculation==1){
		var comml_per=number_format_common(((((document.getElementById('txt_comml_pre_cost').value)*1)/total_cost)*100), 7, 0);
		document.getElementById('txt_comml_po_price').value=Math.round(comml_per);
	}
	else{
	...
	document.getElementById('txt_incentives_po_price').value=number_format_common(((((document.getElementById('txt_incentives_pre_cost').value)*1)/txt_confirm_price_pre_cost_dzn)*100), 7, 0);
...
function fnc_quotation_entry_dtls( operation )
{
	...
	var wash_cost=$("#txt_wash_pre_cost").val()*1; var pre_wash_cost=$("#txt_wash_pre_cost").attr('pre_wash_cost')*1;
	var comml_cost=$("#txt_comml_pre_cost").val()*1; var pre_comml_cost=$("#txt_comml_pre_cost").attr('pre_comml_cost')*1;
	//var lab_cost=$("#txt_lab_test_pre_cost").val()*1; var pre_lab_cost=$("#txt_lab_test_pre_cost").attr('pri_lab_test_pre_cost')*1;
	...
	if (txt_confirm_date_pre_cost !="" && form_validation('txt_confirm_price_pre_cost','Price Before Comn')==false){
		//alert(2)
		alert('Fill Up Price Before Comn');
		return;
	}
	if(txt_confirm_price_pre_cost =="" && txt_confirm_date_pre_cost!=""){
		alert("Insert Confirm Price");
		...
		var data="action=save_update_delete_quotation_entry_dtls&operation="+operation+get_submitted_data_string('cbo_company_name*txt_quotation_date*update_id*cbo_costing_per*cbo_order_uom*tot_set_qnty*update_id_dtls*txt_fabric_pre_cost*txt_fabric_po_price*txt_trim_pre_cost*txt_trim_po_price*txt_embel_pre_cost*txt_embel_po_price*txt_wash_pre_cost*txt_wash_po_price*txt_comml_pre_cost*txt_comml_po_price*txt_lab_test_pre_cost*txt_lab_test_po_price*txt_inspection_pre_cost*txt_inspection_po_price*txt_cm_pre_cost*txt_cm_po_price*txt_freight_pre_cost*txt_freight_po_price*txt_currier_pre_cost*txt_currier_po_price*txt_certificate_pre_cost*txt_certificate_po_price*txt_common_oh_pre_cost*txt_common_oh_po_price*txt_depr_amor_pre_cost*txt_depr_amor_po_price*txt_interest_pre_cost*txt_interest_po_price*txt_income_tax_pre_cost*txt_income_tax_po_price*txt_incentives_rate*txt_incentives_pre_cost*txt_incentives_po_price*txt_total_pre_cost*txt_total_po_price*txt_commission_pre_cost*txt_commission_po_price*txt_final_cost_dzn_pre_cost*txt_final_cost_dzn_po_price*txt_final_cost_pcs_po_price*txt_final_cost_set_pcs_rate*txt_1st_quoted_price_pre_cost*txt_1st_quoted_po_price*txt_first_quoted_price_date*txt_revised_price_pre_cost*txt_revised_price_date*txt_confirm_price_pre_cost*txt_confirm_price_set_pcs_rate*txt_confirm_price_pre_cost_dzn*txt_confirm_price_po_price_dzn*txt_margin_dzn_pre_cost*txt_margin_dzn_po_price*txt_confirm_date_pre_cost*txt_asking_quoted_price*txt_asking_quoted_po_price*txt_with_commission_pre_cost_dzn*txt_with_commission_po_price_dzn*txt_with_commission_pre_cost_pcs*txt_with_commission_po_price_pcs*txt_terget_qty*txt_studio_pre_cost*txt_design_pre_cost*txt_studio_po_price*txt_design_po_price*interest_breack_down',"../../");
		freeze_window(operation);
		http.open("POST","requires/quotation_entry_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
...
function fnc_quotation_entry_dtls_reponse(){
	if(http.readyState == 4){
		...
		if(reponse[0]==0 || reponse[0]==1)
		{
			...
			if(txt_commission_pre_cost != (reponse[3]*1)){
				var commission_cost_new=reponse[3]*1;
				document.getElementById('txt_commission_pre_cost').value=number_format (commission_cost_new, 6,'.' , "") ;
			}
			if(txt_comml_pre_cost_4disit != 0 && ready_to_approved==1){
...
function fnc_quotation_entry_dtls1(operation)
{
	...
	if(txt_confirm_price_pre_cost =="" && txt_confirm_date_pre_cost!=""){
		...
		var data="action=save_update_delete_quotation_entry_dtls&operation="+operation+get_submitted_data_string('cbo_company_name*update_id*cbo_currercy*cbo_costing_per*cbo_order_uom*update_id_dtls*txt_fabric_pre_cost*txt_fabric_po_price*txt_trim_pre_cost*txt_trim_po_price*txt_embel_pre_cost*txt_embel_po_price*txt_wash_pre_cost*txt_wash_po_price*txt_comml_pre_cost*txt_comml_po_price*txt_lab_test_pre_cost*txt_lab_test_po_price*txt_inspection_pre_cost*txt_inspection_po_price*txt_cm_pre_cost*txt_cm_po_price*txt_freight_pre_cost*txt_freight_po_price*txt_currier_pre_cost*txt_currier_po_price*txt_certificate_pre_cost*txt_certificate_po_price*txt_common_oh_pre_cost*txt_common_oh_po_price*txt_depr_amor_pre_cost*txt_depr_amor_po_price*txt_interest_pre_cost*txt_interest_po_price*txt_income_tax_pre_cost*txt_income_tax_po_price*txt_total_pre_cost*txt_total_po_price*txt_commission_pre_cost*txt_commission_po_price*txt_final_cost_dzn_pre_cost*txt_final_cost_dzn_po_price*txt_final_cost_pcs_po_price*txt_final_cost_set_pcs_rate*txt_1st_quoted_price_pre_cost*txt_1st_quoted_po_price*txt_first_quoted_price_date*txt_revised_price_pre_cost*txt_revised_price_date*txt_confirm_price_pre_cost*txt_confirm_price_set_pcs_rate*txt_confirm_price_pre_cost_dzn*txt_confirm_price_po_price_dzn*txt_margin_dzn_pre_cost*txt_margin_dzn_po_price*txt_confirm_date_pre_cost*txt_asking_quoted_price*txt_asking_quoted_po_price*txt_with_commission_pre_cost_dzn*txt_with_commission_po_price_dzn*txt_with_commission_pre_cost_pcs*txt_with_commission_po_price_pcs*txt_terget_qty*txt_studio_pre_cost*txt_design_pre_cost*txt_studio_po_price*txt_design_po_price*txt_incentives_rate*txt_incentives_pre_cost*txt_incentives_po_price*interest_breack_down',"../../");
		http.onreadystatechange = function(){
			if(http.readyState == 4 && http.status == 200) {
				var reponse=trim(http.responseText).split('**');
				if (reponse[0].length>2) reponse[0]=10;
				...
				var txt_comml_pre_cost=document.getElementById('txt_comml_pre_cost').value*1;
				var txt_commission_pre_cost=document.getElementById('txt_commission_pre_cost').value*1;
				var ready_to_approved=document.getElementById('cbo_ready_to_approved').value*1;
				if(txt_commission_pre_cost != reponse[3]*1){
					var commission_cost_new=reponse[3]*1;
					document.getElementById('txt_commission_pre_cost').value=number_format (commission_cost_new, 6,'.' , "") ;
				}
...
function summary(){
	...
	var txt_asking_quoted_price_commision=return_global_ajax_value(update_id+"_"+cbo_costing_per+"_"+txt_confirm_price_pre_cost_dzn, 'cofirm_price_commision', '', 'requires/quotation_entry_controller');
	...
	document.getElementById('txt_deviation_1').value=number_format_common(txt_with_commission_pre_cost_pcs-txt_expected_1,1,0,currency);
	...
	var txt_deviation_2=txt_final_cost_pcs_po_price*1-txt_final_cost_pcs_po_price*1;
	
	document.getElementById('txt_deviation_2').value=txt_deviation_2*1;
	//console.log(txt_commission_pre_cost); return;
	var txt_confirmed_3=number_format_common((txt_commission_pre_cost/cbo_costing_per_value),1,0,currency);
	
	var confirmed_3=number_format_common((txt_commission_pre_cost/cbo_costing_per_value),1,0,currency);
	document.getElementById('txt_confirmed_3').value=confirmed_3;
	...
	//alert(txt_asking_quoted_price+'=='+txt_asking_quoted_price_commision_4+'=='+txt_commission_po_price);
	
	document.getElementById('txt_expected_3').value=number_format_common(txt_expected_3,1,0,currency);
	//var deviation_3=number_format_common((txt_expected_3*1-txt_confirmed_3*1),1,0,currency);
	var deviation_3=number_format_common((txt_confirmed_3*1-txt_expected_3*1),1,0,currency);
	document.getElementById('txt_deviation_3').value=deviation_3*1;
	//console.log(deviation_3); return;
	...
	var deviation_4=number_format_common((confirm_4*1-txt_expected_4*1),1,0,currency);
	document.getElementById('txt_deviation_4').value=deviation_4;
	
	document.getElementById('txt_expected_5').value=number_format_common(txt_expected_4*cbo_costing_per_value,1,0,currency);
	var confirmed_5=number_format_common((confirm_4*cbo_costing_per_value),1,0,currency);
	document.getElementById('txt_confirmed_5').value=confirmed_5;
	var deviation_5=number_format_common((number_format_common((confirm_4*cbo_costing_per_value),1,0,currency)*1-number_format_common(txt_expected_4*cbo_costing_per_value,1,0,currency)*1),1,0,currency);
	document.getElementById('txt_deviation_5').value=deviation_5;
	document.getElementById('txt_expected_6').value=number_format_common(txt_expected_4*txt_offer_qnty,1,0,currency);
	var confirmed_6=number_format_common((confirm_4*txt_offer_qnty),1,0,currency);
	document.getElementById('txt_confirmed_6').value=confirmed_6;
	var deviation_6=number_format_common((number_format_common((confirm_4*txt_offer_qnty),1,0,currency)*1-number_format_common( txt_expected_4*txt_offer_qnty,1,0,currency)*1),1,0,currency);
	document.getElementById('txt_deviation_6').value=deviation_6;
	var devition_data=txt_with_commission_pre_cost_pcs-txt_expected_1;
	var profitlossper_1=(devition_data/txt_with_commission_pre_cost_pcs)*100;
	var profitlossper_2=((txt_final_cost_pcs_po_price-txt_final_cost_pcs_po_price)/txt_final_cost_pcs_po_price)*100;
	var profitlossper_3=(deviation_3/confirmed_3)*100;
	var profitlossper_4=(deviation_4/confirm_4)*100;
	var profitlossper_5=(deviation_5/confirmed_5)*100;
	var profitlossper_6=(deviation_6/confirmed_6)*100;
	
	$('#txtprofitloss_1').val( number_format_common((profitlossper_1),1,0,currency) );
	$('#txtprofitloss_2').val( number_format_common((profitlossper_2),1,0,currency) );
	$('#txtprofitloss_3').val( number_format_common((profitlossper_3),1,0,currency) );
	$('#txtprofitloss_4').val( number_format_common((profitlossper_4),1,0,currency) );
	$('#txtprofitloss_5').val( number_format_common((profitlossper_5),1,0,currency) );
	$('#txtprofitloss_6').val( number_format_common((profitlossper_6),1,0,currency) );
}
...
function generate_report(type,mail_data,mail_status)
{
	...
	else
	{
		if(type=='summary2' || type=='lc_cost_details' )
		{
			...
			var txt_final_cost_dzn_pre_cost=$('#txt_final_cost_dzn_pre_cost').val();
			...
			'&txt_final_cost_dzn_pre_cost='+"'"+txt_final_cost_dzn_pre_cost+"'"+
		...
		{
			var zero_val=0;
			if(type=="preCostRpt" || type=="preCostRpt2" || type=="preCostRpt6" || type=="preCostRpt12")
			{
				if(mail_status!==1){
			...
			var data="action=generate_report&type="+type+"&mail_data="+mail_data+"&zero_value="+zero_val+"&"+get_submitted_data_string('txt_quotation_id*cbo_company_name*cbo_buyer_name*txt_style_ref*txt_quotation_date*txt_final_cost_dzn_pre_cost',"../../")+'&path=../../';
			//alert(data);
			http.open("POST","requires/quotation_entry_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
...
function check_exchange_rate(){
	...
	var cbo_company_name = $('#cbo_company_name').val();
	var response=return_global_ajax_value( cbo_currercy+"**"+quotation_date+"**"+cbo_company_name, 'check_exchange_rate', '', 'requires/quotation_entry_controller');
...
function print_report_button_setting(report_ids)
{
	...
	for (var k=0; k<report_id.length; k++)
	{
		...
		if(report_id[k]==406) $("#report_btn_6").show();
		if(report_id[k]==217) $("#report_btn_8").show();
		if(report_id[k]==275) $("#report_btn_9").show();
		if(report_id[k]==414) $("#report_btn_10").show();
		if(report_id[k]==137) $("#report_btn_11").show();
		if(report_id[k]==191) $("#report_btn_12").show();
		if(report_id[k]==220) $("#report_btn_13").show();
	}
}
...
function check_quatation(){
	...
	function open_body_part_popup(i)
	{
		var cbofabricnature=document.getElementById('cbofabricnature_'+i).value;
		var libyarncountdeterminationid =document.getElementById('libyarncountdeterminationid_'+i).value
		var page_link='requires/quotation_entry_controller.php?action=body_part_popup&fabric_nature='+cbofabricnature+'&libyarncountdeterminationid='+libyarncountdeterminationid;
		...
		emailwindow.onclose=function()
		{
			...
			document.getElementById('txtbodyparttext_'+i).title=name.value;
	...
	{
		var update_id=$("#txt_quotation_id").val();
		var buyer=$("#cbo_buyer_name").val();
		//var check_is_master_part_saved=return_global_ajax_value(update_id, 'check_is_master_part_saved', '', 'requires/quotation_entry_controller');
		if(trim(update_id)=="")
		{
			$('#messagebox_main', window.parent.document).fadeTo(100,1,function(){
		...
		var page_link='requires/quotation_entry_controller.php?action=trims_cost_template_name_popup&company=' + document.getElementById('cbo_company_name').value + '&buyer_name=' + document.getElementById('cbo_buyer_name').value+ '&update_id=' + document.getElementById('txt_quotation_id').value;
	...
	function load_template_data(data)
	{
		...
		for(var b=1; b<=itemdata.length; b++)
		{
			...
			if(row_count == 1 && document.getElementById('cboconsuom_1').value == 0)
			{
				...
				document.getElementById('txttrimexper_1').value=exdata[11];
				document.getElementById('txttotCons_1').value=exdata[9];
			...
			else
			{
				...
				document.getElementById('txttrimexper_'+row_count).value=exdata[11];
				document.getElementById('txttotCons_'+row_count).value=exdata[9];
	...
	function call_print_button_for_mail(mail){		
		...
		if(data==1){
			...
			generate_report('costingSheetRpt',mail+'**1',1);
		}else if(data==6){			
			generate_report('buyerSubmitSummery',mail+'**1',1);
		}else if(data==7){			
			generate_report('summary2',mail+'**1',1);
		}else if(data==8){			
			generate_report('lc_cost_details',mail+'**1',1);
		}else if(data==9){			
			generate_report('preCostRpt5',mail+'**1',1);
		}else if(data==10){			
			generate_report('preCostRpt6',mail+'**1',1);     
		}else if(data==11){			
			generate_report('preCostRpt13',mail+'**1',1);     
		}
	}

	function interest_popup(){
		var total_cost=$('#txt_confirm_price_pre_cost_dzn').val();
		var breack_down=$('#interest_breack_down').val();
		var interest_cost=$('#txt_interest_pre_cost').val();
		var cbo_costing_per=document.getElementById('cbo_costing_per').value;
		var page_link='requires/quotation_entry_controller.php?action=interest_calculation_popup&total_cost='+total_cost+'&breack_down='+breack_down+'&interest_cost='+interest_cost+'&cbo_costing_per='+cbo_costing_per;
		var title='Interest Calculation Form' ;
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=300px,height=150px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function(){
			var theemail=this.contentDoc.getElementById("interest_breack_down").value;
			var interest_amt=this.contentDoc.getElementById("interest_amt_data").value;
			if (theemail!=""){
				document.getElementById('interest_breack_down').value=theemail;
				document.getElementById('txt_interest_pre_cost').value=interest_amt;
				calculate_main_total();
			}
		}
	}
...
<body onLoad="set_hotkey();set_auto_complete('price_quation_mst')" >
	...
	<div id="content_master_form">
        <table width="1250px" cellpadding="0" cellspacing="2" align="center">
            <tr>
                <td width="72%" align="left" valign="top">  <!--   Form Left Container -->
                            ...
                            <div style="width:900px;">
                                <table width="100%" cellspacing="2" cellpadding="" border="0">
                                    <tr>
                                        ...
                                        <td colspan="4">
                                        	<input type="text" id="txt_quotation_id" name="txt_quotation_id" class="text_boxes" style="width:130px;" readonly placeholder="Browse Quotation" onDblClick="openmypage('requires/quotation_entry_controller.php?action=quotation_id_popup','Quotation ID Selection Form')"/>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td width="70">Inquiry ID</td>
                                        <td width="145">
                                    ...
                                    <tr>
                                        ...
                                        <td><? echo create_drop_down( "cbo_order_uom",55, $unit_of_measurement, "",0, "", 1, "change_caption_cost_dtls( this.value, 'change_caption_pcs' )","","1,58" ); ?>
                                            ...
                                            <input type="hidden" id="is_season_must" />
                                        </td>
                                        <td>Factory</td>
                                    ...
                                    <tr>
                                        ...
                                        <td>Season Year</td>
                                        <td><? echo create_drop_down( "cbo_season_year", 130, create_year_array(),"", 1,"-Year-", 1, "",0,"" );; ?></td>
                                    ...
                                    <tr>
                                        ...
                                        <td>Remarks</td>
                                        <td><input class="text_boxes" type="text" style="width:120px;" name="txt_remarks" id="txt_remarks" maxlength="500"/></td>
                                    </tr>
                                    <tr>
                                    	<td>&nbsp;</td> 
                                        <td><input type="button" id="image_button" class="image_uploader" style="width:65px" value="ADD IMAGE" onClick="file_uploader ( '../../', document.getElementById('update_id').value,'', 'quotation_entry', 0 ,1)" />
										<input type="button" id="image_button" class="image_uploader" style="width:65px" value="ADD FILE" onClick="file_uploader ( '../../', document.getElementById('update_id').value,'', 'quotation_entry', 2 ,1)">
                                        </td>
                                        <td>&nbsp;</td>
										<td align="left">
											<input type="file" multiple id="file_uploaded" class="image_uploader" style="width:130px" onChange="document.getElementById('txt_file').value=1">
											<input type="hidden" multiple id="file_text">
										</td>
                                        <td>&nbsp;</td>
                                    ...
                                    <tr>
                                        <td align="right" valign="top" class="button_container" colspan="5">
                                            ...
                                            <input type="hidden" id="percentage_calculation" value=""/>
                    ...
                    <fieldset>
                        ...
                        <table width="100%" cellspacing="2" cellpadding="0" class="rpt_table" rules="all">
                            <thead>
                                ...
                                <th title="(Deviation/Confirm)*100">Prof/ Loss %</th>
                            </thead>
                            <tr>
                                <td>Price With Commn /Pcs</td>
                                <td ><input class="text_boxes_numeric" type="text" name="txt_expected_1" id="txt_expected_1" title="(Commision/12)+Asking Quoted Price" style="width:37px;" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txt_confirmed_1" id="txt_confirmed_1" style="width:37px;" title="Price with Commn /Pcs" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txt_deviation_1" id="txt_deviation_1" style="width:37px;" title="Price With Commn /Pcs(Confirm)- Price With Commn /Pcs(Asking)" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txtprofitloss_1" id="txtprofitloss_1" style="width:37px;" title="(Price With Commn /Pcs(Deviation)/Price with Commn /Pcs)*100" readonly/></td>
                            </tr>
                            <tr>
                                <td>Prod. Cost/Pcs</td>
                                <td><input class="text_boxes_numeric" type="text" name="txt_expected_2" id="txt_expected_2" style="width:37px;" title="Final Cost" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txt_confirmed_2" id="txt_confirmed_2" style="width:37px;" title="Final Cost" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txt_deviation_2" id="txt_deviation_2" style="width:37px;" title="Prod. Cost/Pcs(Confirm)- Prod. Cost/Pcs(Asking)" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txtprofitloss_2" id="txtprofitloss_2" style="width:37px;" title="((Final Cost /Pcs-Final Cost /Pcs)/Final Cost /Pcs)*100" readonly/></td>
                            </tr>
                            <tr>
                                <td>Commn/Pcs</td>
                                <td><input class="text_boxes_numeric" type="text" name="txt_expected_3" id="txt_expected_3" style="width:37px;" title="Commn/Pcs(Asking)=(Asking Quoted Price /Pcs+Commision)*Commi./ Dzn %" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txt_confirmed_3" id="txt_confirmed_3" style="width:37px;" title="Commision/Costing Per" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txt_deviation_3" id="txt_deviation_3" style="width:37px;" title="Commn/Pcs(Confirm)- Commn/Pcs(Asking)" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txtprofitloss_3" id="txtprofitloss_3" style="width:37px;" title="(Commn/Pcs(Deviation)/Commn/Pcs(Confirm))*100" readonly/></td>
                            </tr>
                            <tr>
                                <td>Margin/Pcs</td>
                                <td><input class="text_boxes_numeric" type="text" name="txt_expected_4" id="txt_expected_4" style="width:37px;" title="Margin/Pcs(Asking)=Price With Commn /Pcs(Asking)-Final Cost /Pcs-Commn/Pcs(Asking)" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txt_confirmed_4" id="txt_confirmed_4" style="width:37px;" title="Price with Commn /Pcs-Final Cost /Pcs(Commision/Costing Per)" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txt_deviation_4" id="txt_deviation_4" style="width:37px;" title="Margin/Pcs(Confirm)- Margin/Pcs(Asking)" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txtprofitloss_4" id="txtprofitloss_4" style="width:37px;" title="(Margin/Pcs(Deviation)/Margin/Pcs(Confirm))*100" readonly/></td>
                            </tr>
                            <tr>
                                <td>Margin/Dzn</td>
                                <td><input class="text_boxes_numeric" type="text" name="txt_expected_5" id="txt_expected_5" style="width:37px;" title="Margin/Pcs(Asking)*Costing Per" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txt_confirmed_5" id="txt_confirmed_5" style="width:37px;" title="Margin/Pcs(Confirm)*Costing Per" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txt_deviation_5" id="txt_deviation_5" style="width:37px;" title="Margin/Dzn(Confirm)- Margin/Dzn(Asking)" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txtprofitloss_5" id="txtprofitloss_5" title="(Margin/Dzn(Deviation)/Margin/Dzn(Confirm))*100" style="width:37px;" readonly/></td>
                            </tr>
                            <tr>
                                <td>Margin for Offer Qty</td>
                                <td><input class="text_boxes_numeric" type="text" name="txt_expected_6" id="txt_expected_6" style="width:37px;" title="Margin/Pcs(Asking)*Offer Qty" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txt_confirmed_6" id="txt_confirmed_6" style="width:37px;" title="Margin/Pcs(Confirm)*Offer Qty" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txt_deviation_6" id="txt_deviation_6" style="width:37px;" title="Margin for Offer Qty(Confirm)- Margin for Offer Qty(Asking)" readonly/></td>
                                <td><input class="text_boxes_numeric" type="text" name="txtprofitloss_6" id="txtprofitloss_6" style="width:37px;" title="(Margin for Offer Qty(Deviation)/Margin for Offer Qty(Confirm))*100" readonly/></td>
                            </tr>
                        </table>
                    </fieldset>
                </td>
    ...
    <fieldset>
        <form id="quotationdtls_2" autocomplete="off">
            <table width="1780px" cellspacing="2" cellpadding="0" class="rpt_table" rules="all">
                <tr>
                    <thead>
                        <th width="100">Cost Components</th>
                        <th width="50">Fabric Cost</th>
                        <th width="50"><div><input type="checkbox" id="is_tmplete" name="is_tmplete" onClick="show_sub_form(document.getElementById('update_id').value,'show_trim_cost_listview',document.getElementById('cbo_buyer_name').value+'*'+0);" style="display:none"/></div>Trims Cost<div> <span id="load_temp" style=" width:10px; font-weight: bold;background-color: white;color:black; border: 1px white solid; cursor: pointer;" onClick="openmypage_template_name('Template Search')">...</span></div></th>
                        ...
                        <th width="80">Incentives <br>Missing Cost</th>
                ...
                <tr bgcolor="#CCFFFF">
                    ...
                    <td>
						<input class="text_boxes_numeric" type="text" name="txt_interest_pre_cost" id="txt_interest_pre_cost" style="width:40px;" onChange="calculate_main_total()" onDblClick="interest_popup()" readonly/>
						<input type="hidden" id="interest_breack_down" value="" />
					</td>
                    <td><input class="text_boxes_numeric" type="text" name="txt_income_tax_pre_cost" id="txt_income_tax_pre_cost" style="width:40px;" onChange="calculate_main_total()"/></td>
                     <td><input class="text_boxes_numeric" type="text" name="txt_incentives_rate" id="txt_incentives_rate" style="width:20px;"  onChange="calculate_insentives_cost(1,'rate' );calculate_main_total()" placeholder="Rate"/><input class="text_boxes_numeric" type="text" name="txt_incentives_pre_cost" id="txt_incentives_pre_cost" style="width:45px;" onChange="calculate_insentives_cost(1,'amount');calculate_main_total()"/></td>
                ...
                <tr>
                    ...
                    <td><input class="text_boxes_numeric" type="text"  name="txt_incentives_po_price" id="txt_incentives_po_price" style="width:80px;" readonly/></td>
                ...
                <tr>
                    <td align="center" colspan="33" valign="top">
                        ...
                        <input type="button" id="report_btn_6" class="formbutton" value="Buyer Sub. Summary" onClick="generate_report('buyerSubmitSummery');" style="display:none;" />
						...
						<input type="button" id="report_btn_10" class="formbutton" value="Quot. Rpt6" onClick="generate_report('preCostRpt6');" style="display:none;" />
						<input type="button" id="report_btn_11" class="formbutton" value="Quot. Rpt4" onClick="generate_report('preCostRpt11');" style="display:none;" />

						<input type="button" id="report_btn_12" class="formbutton" value="Quot. Rpt7" onClick="generate_report('preCostRpt12');" style="display:none;" />
						<input type="button" id="report_btn_13" class="formbutton" value="Quot. Rpt8" onClick="generate_report('preCostRpt13');" style="display:none;" /> 
                    </td>
                </tr>
            </table>
        </form>
