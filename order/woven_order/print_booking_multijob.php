...
function openmypage_emb_item(page_link,title){
	...
	emailwindow.onclose=function()
	{
		...
		if (theemail.value!="")
		{
			...
			//release_freezing();
				$("#color_name").attr("disabled",false);
		}
		else{
			//release_freezing();
		}
...
function fnc_trims_booking_dtls( operation ){
	...
	var selected_color_id=document.getElementById('selected_color_id').value;
	if(cbo_level==1){
		var data="action=save_update_delete_dtls&operation="+operation+'&total_row='+row_num+data_all+'&delete_cause='+delete_cause+'&cbo_company_name='+cbo_company_name+'&selected_color_id='+selected_color_id;
	}
	if(cbo_level==2){
		var data="action=save_update_delete_dtls_job_level&operation="+operation+'&total_row='+row_num+data_all+'&delete_cause='+delete_cause+'&cbo_company_name='+cbo_company_name+'&selected_color_id='+selected_color_id;
	}
	http.open("POST","requires/print_booking_multijob_controller.php",true);
	http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	http.send(data);
...
function fnc_fabric_booking_reponse()
{
	if(http.readyState == 4)
	{
		...
		if(trim(response[0])=='serviceacklockAnotherProcess'){
			alert("This booking is Attached In Service Acknowledgement. Ref :"+trim(response[1])+" \n So Delete Not Allowed.")
		    release_freezing();
		    return;
		}
...
function fnc_color_popup(page_link,title)
{
	//txt_selected_trim_id
	var txt_selected_trim_id=$("#txt_selected_po").val();
	if (form_validation('cbo_company_name*txt_booking_no*txt_select_item','Company*Booking No*Select Embellishment')==false){
		return;
	}
	var txt_booking_no=document.getElementById('txt_booking_no').value;
	var garments_nature=document.getElementById('garments_nature').value;
	...
	var color_ids=document.getElementById('selected_color_id').value;
	

	page_link=page_link+'&company_id='+cbo_company_name+'&garments_nature='+garments_nature+'&txt_booking_no='+txt_booking_no+'&color_name='+color_name+'&color_ids='+color_ids+'&txt_selected_trim_id='+txt_selected_trim_id;
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=540px,height=450px,center=1,resize=1,scrolling=0','../')
	emailwindow.onclose=function()
	{
		//freeze_window(5);
		...
		var theform=this.contentDoc.forms[0];
		var theemail=this.contentDoc.getElementById("txt_selected_id");
		var theemail3=this.contentDoc.getElementById("txt_selected_name");
		if (theemail.value!="")
		{
			document.getElementById('selected_color_id').value=theemail.value;
			document.getElementById('color_name').value=theemail3.value;
			$("#color_name").attr("disabled",true);
		}
		else{
			//release_freezing();
