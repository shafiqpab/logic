...
var mandatory = '<? echo count($_SESSION['logic_erp']['mandatory_field'][89]) ?>';
...
function fnc_fabric_booking( operation )
{
	...
	if (form_validation('txt_order_no_id*txt_booking_date*txt_delivery_date*cbo_pay_mode*cbo_supplier_name','Order No*Booking Date*Delivery Date*Pay mode*Supplier')==false)
	{
		return;
	}
	...
	{
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_order_no_id*cbo_company_name*update_id*cbo_buyer_name*txt_job_no*txt_booking_no*cbo_fabric_natu*cbo_fabric_source*cbo_currency*txt_exchange_rate*cbo_pay_mode*txt_booking_date*cbo_booking_month*cbo_supplier_name*txt_attention*txt_delivery_date*cbo_source*cbo_booking_year*cbo_ready_to_approved*txt_fabriccomposition*txt_fabric_booking_no',"../../");
...
function fnc_fabric_booking_reponse(){
	...
	if(http.readyState == 4){
		 ...
		 if(parseInt(trim(reponse[0]))==0 || parseInt(trim(reponse[0]))==1 || parseInt(trim(reponse[0]))==2){
			 ...
			 {
				reset_form('fabricbooking_1','','booking_list_view','cbo_pay_mode,3*cbo_booking_year,2018*cbo_currency,2*cbo_ready_to_approved,2*txt_booking_date,<? echo date("d-m-Y"); ?>');
...
function fnc_fabric_booking_dtls( operation )
{
	...
	/*if (form_validation('txt_bh_qty*txt_rf_qty','BH Qty*Rf Qty')==false)
		{
			return;
		}*/
	console.log(parseInt(mandatory));
	if( parseInt(mandatory) >0)
	{
		if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][89]);?>','<? echo implode('*',$_SESSION['logic_erp']['field_message'][89]);?>')==false)
		{
			return;
		}
	}	
	var data="action=save_update_delete_dtls&operation="+operation+get_submitted_data_string('txt_booking_no*update_id*txt_job_no*cbo_order_id*cbo_fabricdescription_id*cbo_sample_type*cbo_fabriccolor_id*cbo_garmentscolor_id*cbo_itemsize_id*cbo_garmentssize_id*txt_dia_width*txt_finish_qnty*txt_process_loss*txt_grey_qnty*txt_rate*txt_amount*update_id_details*txt_bh_qty*txt_rf_qty*cbo_pay_mode*cbouom*txt_remarks*txt_additional_process_loss',"../../");
	freeze_window(operation);
	http.open("POST","requires/sample_booking_controller.php",true);
	http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	http.send(data);
...
function open_trims_acc_popup(title)
{
	...
	{
		page_link='requires/sample_booking_controller.php?action=acc_popup'+get_submitted_data_string('txt_booking_no*txt_job_no*update_id','../../');
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title,'width=720px,height=470px,center=1,resize=1,scrolling=0','../');
...
function generate_fabric_report(type)
{
	...
	else
	{
		...
		{
			var data="action=show_fabric_booking_report_print_booking_3"+get_submitted_data_string('txt_booking_no*cbo_company_name*txt_order_no_id*cbo_fabric_natu*cbo_fabric_source*id_approved_id*txt_job_no',"../../")+'&report_title='+$report_title+'&show_yarn_rate='+show_yarn_rate;
		}
		else if(type==5)
		{
			var data="action=show_fabric_booking_report4"+get_submitted_data_string('txt_booking_no*cbo_company_name*txt_order_no_id*cbo_fabric_natu*cbo_fabric_source*id_approved_id*txt_job_no',"../../")+'&report_title='+$report_title+'&show_yarn_rate='+show_yarn_rate;
		}
		//freeze_window(5);
		http.open("POST","requires/sample_booking_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
...
function check_exchange_rate()
{
		...
		var response=return_global_ajax_value( cbo_currercy+"**"+booking_date+"**"+cbo_company_name, 'check_conversion_rate', '', 'requires/sample_booking_controller');
	...
	function print_report_button_setting(report_ids) 
	{
		...
		for (var k=0; k<report_id.length; k++)
		{
			...
			if(report_id[k]==16)
			{
				...
				$('#button_panel').append( '<input type="button" value="Print Booking 3" onClick="generate_fabric_report(4)"  style="width:100px" name="print_booking_3" id="print_booking_3" class="formbutton" />&nbsp;&nbsp;&nbsp;' );
			}
			if(report_id[k]==177)
			{
				
				$('#button_panel').append( '<input type="button" value="Print Booking 4" onClick="generate_fabric_report(5)"  style="width:100px" name="print_booking_4" id="print_booking_4" class="formbutton" />&nbsp;&nbsp;&nbsp;' );
			}
		}
	}
	function open_stripe_color_popup()
	{
		if (form_validation('cbo_company_name*txt_job_no*cbo_buyer_name*cbo_garmentscolor_id*cbo_fabricdescription_id','Company Name*Job NO*Buyer Name*Garments Color*Fabric Description')==false)
		{
			return;
		}
		var cbogmtsitem='';
		...
		var cbo_color_name=document.getElementById('cbo_garmentscolor_id').value;
		var cbo_company_name=document.getElementById('cbo_company_name').value;
		var cbo_buyer_name=document.getElementById('cbo_buyer_name').value;

		var page_link="requires/sample_booking_controller.php?action=open_color_list_view&txt_job_no="+trim(txt_job_no)+"&cbogmtsitem="+cbogmtsitem+"&fabric_cost_id="+fabric_cost_id+"&cbo_color_name="+cbo_color_name+'&cbo_company_name='+cbo_company_name+'&cbo_buyer_name='+cbo_buyer_name;
    ...
    <form name="fabricbooking_1"  autocomplete="off" id="fabricbooking_1">
        ...
        <legend>Sample Booking </legend>
            <table  width="960" cellspacing="2" cellpadding="0" border="0">
                <tr>
                    <td align="right" class="must_entry_caption" colspan="4">Booking No</td><!-- 11-00030  -->
                    <td colspan="4"><input class="text_boxes" type="text" style="width:160px" onDblClick="openmypage_booking('requires/sample_booking_controller.php?action=fabric_booking_popup','fabric Booking Search')" readonly placeholder="Double Click for Booking" name="txt_booking_no" id="txt_booking_no"/>
                ...
                <tr>
                    ...
                    <td width="150"><? echo create_drop_down( "cbo_company_name", 130, "select comp.id,comp.company_name from lib_company comp where comp.status_active=1 and core_business not in(3) $company_cond and comp.is_deleted=0 order by comp.company_name", "id,company_name",1, "-- Select Company --", $selected, "load_drop_down( 'requires/sample_booking_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );check_month_setting();validate_suplier();print_button_setting();check_exchange_rate();",0,"" );
                    ?>                    
                    </td>
                    <td width="100">Buyer Name</td>   
                    <td width="150" id="buyer_td"><?  echo create_drop_down( "cbo_buyer_name", 130, "select id,buyer_name from lib_buyer where status_active =1 and is_deleted=0 order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down( 'requires/sample_booking_controller', this.value, 'load_drop_down_buyer_tag_sample', 'sample_td' );",1,"" ); ?></td>
                    <td width="110">Job No.</td>
                    <td width="150"><input style="width:120px;" type="text" class="text_boxes"  name="txt_job_no" id="txt_job_no" disabled /></td>
                    <td width="100" id="booking_td">Booking Month</td>   
                    <td><? echo create_drop_down( "cbo_booking_month", 75, $months,"", 1, "-- Select --", "", "",0 ); ?>
                    	<? echo create_drop_down( "cbo_booking_year", 50, $year,"", 1, "-- Select --", date('Y'), "",0 ); ?>
                    </td>
                </tr>
                <tr>
                    <td class="must_entry_caption">Fabric Nature</td>
                    <td><? echo create_drop_down( "cbo_fabric_natu", 130, $item_category,"", 1, "-- Select --", 1,$onchange_func, $is_disabled, "2,3");	?>	
                    </td>
                ...
                <tr>
                    ...
                    <td colspan="3"><input class="text_boxes" type="text" style="width:360px;" placeholder="Double click for Order" onDblClick="openmypage_order('requires/sample_booking_controller.php?action=order_search_popup','Order Search');" name="txt_order_no" id="txt_order_no"/>
                    <input class="text_boxes" type="hidden" style="width:130px;"  name="txt_order_no_id" id="txt_order_no_id"/>
                    </td>                               
                </tr>
                <tr>
                    ...
                    <td><? echo create_drop_down( "cbo_pay_mode", 130, $pay_mode,"", 1, "-- Select Pay Mode --",3, "load_drop_down( 'requires/sample_booking_controller', this.value, 'load_drop_down_suplier', 'sup_td' )","","1,2,3,5" ); ?> 
                    </td>
                    <td class="must_entry_caption">Supplier Name</td>
                    <td id="sup_td"><?=create_drop_down( "cbo_supplier_name", 130, "select id,supplier_name from lib_supplier where status_active =1 and is_deleted=0 order by supplier_name","id,supplier_name", 1, "-- Select Supplier --", $selected, "get_php_form_data( this.value+'_'+document.getElementById('cbo_pay_mode').value, 'load_drop_down_attention', 'requires/sample_booking_controller');",0 ); ?></td> 
                    <td >Tag Booking No</td>                            
                    <td><input class="text_boxes" type="text" style="width:120px" onDblClick="openmypage_fabric_booking('requires/sample_booking_controller.php?action=fabric_booking_no_popup','fabric Booking Search');" readonly placeholder="Double Click for Fabric Booking"  name="txt_fabric_booking_no" id="txt_fabric_booking_no"/>
                    </td>
                </tr>
                <tr>
                    <td>Attention</td>   
                    <td colspan="3">
                        <input class="text_boxes" type="text" style="width:360px;"  name="txt_attention" id="txt_attention"/>
                    ...
                    <td>File No </td>   
                    <td><input class="text_boxes" type="text"  style="width:120px;"  name="txt_file" id="txt_file"/></td>
                    <td>Ref. No </td>   
                    <td><input class="text_boxes" type="text"  style="width:120px;"  name="txt_ref" id="txt_ref"/></td>
                </tr>
                <tr>
                    <td>Fabric Compo.</td>   
                    <td colspan="3"><input class="text_boxes" type="text" maxlength="200" style="width:360px;"  name="txt_fabriccomposition" id="txt_fabriccomposition"/></td>
                    <td>Ready To App.</td>  
                    <td><?=create_drop_down( "cbo_ready_to_approved", 130, $yes_no,"", 1, "-- Select--", 2, "","","" ); ?></td>
                    <td><input type="button" id="set_button" class="image_uploader" style="width:80px;" value="Accessories" onClick="open_trims_acc_popup('Accessories Dtls');" /></td>
                    <td>&nbsp;</td>
                </tr>
                <tr>
    ...
    <legend>Sample Booking </legend>
        <table  width="900" cellspacing="2" cellpadding="0" border="0">
            <tr>
                ...
                <td id="order_drop_down_td"><? echo create_drop_down( "cbo_order_id", 172, $blank_array,"", 1, "--Select--", $selected, "" ); ?>
				<input class="text_boxes" type="text" style="width:360px;" name="txt_order_no" id="txt_order_no"/>
			    </td>
			...
			<tr>
                ...
                <td><input name="txt_remarks" id="txt_remarks"  class="text_boxes" type="text" value=""  style="width:160px"/></td>
				<td align="right">Additional Process</td>
                <td><input name="txt_additional_process_loss" id="txt_additional_process_loss"  class="text_boxes" type="text" value=""  style="width:160px"/></td>
            </tr>
            <tr>
