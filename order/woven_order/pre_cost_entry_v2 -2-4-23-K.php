...
//Conversion Cost-------------------------------------------
	...
	function set_conversion_charge_unit(i,conversion_from_chart)
	{
		...
		//alert(cost_head);
		if(cost_head==31 || cost_head==1) // Issue Id=11140 For Lariz As per Nasir
		{
...
function calculate_incentives_cost(i,type)
	...
	function fnc_quotation_entry_dtls1( operation )
	{
		...
		{
			var data="action=save_update_delete_quotation_entry_dtls&operation="+operation+get_submitted_data_string('update_id*hidd_job_id*txt_cost_control_source*cbo_costing_per*cbo_order_uom*update_id_dtls*txt_fabric_pre_cost*txt_fabric_po_price*txt_trim_pre_cost*txt_trim_po_price*txt_embel_pre_cost*txt_embel_po_price*txt_wash_pre_cost*txt_wash_po_price*txt_comml_pre_cost*txt_comml_po_price*txt_commission_pre_cost*txt_commission_po_price*txt_lab_test_pre_cost*txt_lab_test_po_price*txt_inspection_pre_cost*txt_inspection_po_price*txtcmcost*txt_cm_pre_cost*txt_cm_po_price*txt_freight_pre_cost*txt_freight_po_price*txt_currier_pre_cost*txt_currier_po_price*txt_certificate_pre_cost*txt_certificate_po_price*txt_common_oh_pre_cost*txt_common_oh_po_price*txt_depr_amor_pre_cost*txt_depr_amor_po_price*txt_total_pre_cost*txt_total_po_price*txt_final_price_dzn_pre_cost*txt_final_price_dzn_po_price*txt_margin_dzn_pre_cost*txt_margin_dzn_po_price*txt_total_pre_cost_psc_set*txt_total_pre_cost_psc_set_po_price*txt_final_price_pcs_pre_cost*txt_final_price_pcs_po_price*txt_margin_pcs_pre_cost*txt_margin_pcs_po_price*cbo_company_name*txt_quotation_id*copy_quatation_id*txt_deffdlc_pre_cost*txt_deffdlc_po_price*txt_interest_pre_cost*txt_interest_po_price*txt_incometax_pre_cost*txt_incometax_po_price*txt_incentives_pre_cost*txt_incentives_pre_cost_per*txt_incentives_pre_rate*txt_design_pre_cost*txt_design_po_price*txt_studio_pre_cost*txt_studio_po_price',"../../");
			//alert(data); return;
			http.onreadystatechange = function() {
...
//Master form---------------------------------------------------------------------------
		...
		else
		{
			...
			else
			{
				...
				if(mail_status!==1){
						...
						if(type!='mo_sheet' && type != 'budgetsheet' && type != 'budgetsheet2' && type != 'budgetsheet4' && type != 'mo_sheet_1' && type != 'mo_sheet_3' && type != 'budgetsheet3'  && type != 'preCostRpt10' && type != 'preCostRpt12')
						{
						...
						if(type == 'budgetsheet2' || type == 'budgetsheet4')
						{
							var r=confirm("Press  \"OK\" to Show Budget, \nPress  \"Cancel\"  to Show Management Budget");
						}
						if(type == 'mo_sheet_1')
	...
	function fnc_budgete_cost_validate(type)
	{
		...
		if(trim(cost_control_source_data)==5 && type==2)
		{
			var row_num=$('#tbl_fabric_cost tr').length-1;
			var z=1;
			for (var i=1; i<=row_num; i++)
		...
		if(trim(cost_control_source_data)==1 || trim(cost_control_source_data)==5)
		{
			if(costing_id!=0)
			{
				...
				if(entry_formno==458) { var sp_oparation_pre=embel_cost_pre; var wash_cost_pre=wash_cost_pre; }
	...
	function print_report_button_setting(report_ids)
	{
		...
		for (var k=0; k<report_id.length; k++)
		{
			...
			if(report_id[k]==498) $("#report_btn_35").show();
	...
	function fnc_appSubmission_withoutanyChange()
	{
		...
		else
		{
			...
			{*/
				fnc_margin_as_bom();
				var submission_withoutanyChange=return_global_ajax_value(update_id+'**'+$("#hidd_job_id").val()+'**'+cbo_ready_to_approved+'**'+$("#pre_cost_id").val()+'**'+$("#txt_fabric_pre_cost").val(), 'appSubmission_withoutanyChange', '', 'requires/pre_cost_entry_controller_v2');
                ...
                <fieldset>
                    ...
                    <table width="100%" cellspacing="2" cellpadding="0" class="rpt_table" rules="all">
                        ...
                        <tr>
                         ...
                         <tr>
                            <td>Incentives<br>  Missing Cost</td>
                            <td>
                            <input class="text_boxes_numeric" type="text" name="txt_incentives_pre_rate" id="txt_incentives_pre_rate" style="width:15px;" onChange="calculate_incentives_cost( 1,'cal_amount');calculate_main_total();" placeholder="Rate"/><input class="text_boxes_numeric" type="text" name="txt_incentives_pre_cost" id="txt_incentives_pre_cost" style="width:30px;" onChange="calculate_main_total();"/></td>
                            <td><input class="text_boxes_numeric" type="text"  name="txt_incentives_pre_cost_per" id="txt_incentives_pre_cost_per" style="width:32px;" onChange="calculate_incentives_cost( 1,'cal_rate');calculate_main_total();" disabled=""/></td>
                        </tr>
                        ...
                        <tr>
							...
							<input type="button" id="report_btn_31" class="formbutton" value="Budget Sheet 2" onClick="generate_report('budgetsheet2');" style="display:none;" />
                                ...
                                <input type="button" id="report_btn_35" class="formbutton" value="Cost Rpt 11" onClick="generate_report('preCostRpt11');"   style="display:none;"/>
