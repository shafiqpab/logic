...
function openmypage_order(page_link,title)
{
	...
	{
		page_link=page_link+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_booking_month*cbo_booking_year*txt_fab_booking','../../');
...
{	
	...
	if(type=='set_process')
	{     
		show_list_view(document.getElementById('txt_job_no').value+'**'+1+'**'+fabric_desription_id+'**'+document.getElementById('cbo_process').value+'**'+document.getElementById('cbo_colorsizesensitive').value+'**'+document.getElementById('txt_order_no_id').value+'**'+document.getElementById('txt_booking_no').value+'****'+document.getElementById('service_rate_from').value+'**'+document.getElementById('txt_program_no').value+'**'+document.getElementById('cbo_pay_mode').value, 'lapdip_approval_list_view_edit','booking_list_view','requires/service_booking_knitting_controller','$(\'#hide_fabric_description\').val(\'\')');
	}
...
function qunatity_chk(param1,param2)
{
	var txt_woqnty=(document.getElementById('txt_woqnty_'+param1+'_'+param2).value)*1;
	var txt_prevqnty=(document.getElementById('txt_reqqty_'+param1+'_'+param2).value)*1;
	//console.log(txt_woqnty+'--'+txt_prevqnty);
		if(txt_woqnty>txt_prevqnty)
		{
			alert("Service Cost Can't Be Greater Then programm cost");
			return;
		}	
}
	 ...
	 if(copy_val==true)
	  {
		  for(var j=param2; j<=rowCount; j++)
		  {
			  ...
			  calculate_amount(param1,param2);
...
function fnc_trims_booking( operation )
{
	...
	var data_all="";
	if (form_validation('cbo_booking_month*txt_order_no*cbo_company_name*cbo_pay_mode*cbo_supplier_name','Booking Month*Order No*Company Name*Pay Mode*Supplier Name')==false)
	{
		release_freezing();
		return;
	}
...
function generate_trim_report(action)
{
	...
	else
	{
		...
		var data="action="+action+get_submitted_data_string('txt_booking_no*cbo_company_name*cbo_pay_mode*id_approved_id*txt_job_no',"../../")+'&show_comments='+show_comments;
		// alert(data);return;
...
function check_exchange_rate()
{
	...
	var response=return_global_ajax_value( cbo_currercy+"**"+booking_date+"**"+cbo_company_name, 'check_conversion_rate', '', 'requires/service_booking_knitting_controller');
...
function service_supplier_popup(id)
{
	...
	var page_link = 'requires/service_booking_knitting_controller.php?cbo_company_name='+cbo_company_name+'&cbo_supplier_name='+$("#cbo_supplier_name").val()+'&txt_exchange_rate='+$("#txt_exchange_rate").val()+'&hidden_supplier_rate_id='+hidden_supplier_rate_id+'&action=Supplier_workorder_popup';
        ...
        <legend>Service Booking</legend>
            <table  width="1000" cellspacing="2" cellpadding="0" border="0" style="">
                <tr>
                    ...
                    <td colspan="4">
                    	<input class="text_boxes" type="text" style="width:150px" onDblClick="openmypage_booking('requires/service_booking_knitting_controller.php?action=service_booking_popup','Service Booking Search')" readonly placeholder="Double Click for Booking" name="txt_booking_no" id="txt_booking_no"/>
                ...
                <tr>
                    ...
                    <td width="140"><?=create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name", "id,company_name",1, "-Select Company-", $selected, "load_drop_down( 'requires/service_booking_knitting_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );get_php_form_data( this.value, 'print_report_button', 'requires/service_booking_knitting_controller');check_exchange_rate();","","" ); ?></td>
                ...
                <tr>
                    ...
                    <td colspan="3">
                    	<input class="text_boxes" type="text" style="width:370px" placeholder="Browse Order"  onDblClick="openmypage_order('requires/service_booking_knitting_controller.php?action=order_search_popup','Order Search')"   name="txt_order_no" id="txt_order_no"/>
                ...
                <tr>
                    <td>Currency</td>
                    <td><?=create_drop_down( "cbo_currency", 130, $currency,"", 1, "-- Select --", 1, "check_exchange_rate();",0 ); ?></td>
                    <td>Exchange Rate</td>
                    ...
                    <td><?=create_drop_down( "cbo_pay_mode", 130, $pay_mode,"", 1, "-- Select Pay Mode --", "", "load_drop_down( 'requires/service_booking_knitting_controller', this.value, 'load_drop_down_supplier', 'supplier_td' )","" ); ?> 
                    </td>
                ...
                <tr>
                    ...
                    <td id="supplier_td"><?=create_drop_down( "cbo_supplier_name", 130, "select a.id,a.supplier_name from lib_supplier a,lib_supplier_party_type b where a.id=b.supplier_id and b.party_type in(20) and a.status_active =1 and a.is_deleted=0 order by a.supplier_name","id,supplier_name", 1, "-- Select Supplier --", $selected, "get_php_form_data( this.value+'_'+document.getElementById('cbo_pay_mode').value, 'load_drop_down_attention', 'requires/service_booking_knitting_controller');",0 ); ?></td> 
                    <td>Tenor</td>
                    ...
                    <td colspan="3">
                    	...
                    	<input type="hidden" class="image_uploader" style="width:100px" value="Lab DIP No" onClick="openmypage( 'requires/service_booking_knitting_controller.php?action=lapdip_no_popup', 'Lapdip No', 'lapdip')">
                    </td>
                </tr>
                 <tr>
                 	<td>Fabric Booking</td>
					<td><input class="text_boxes" type="text" style="width:130px;"  name="txt_fab_booking" id="txt_fab_booking" onDblClick="fnc_fab_booking('requires/service_booking_knitting_controller.php?action=fabric_booking_popup','fabric Booking Search')" placeholder="Browser" readonly/> </td>
                    <td>Ready To Approved</td>  
                    <td><?=create_drop_down( "cbo_ready_to_approved", 130, $yes_no,"", 1, "-- Select--", 2, "","","" ); ?></td>
                    <td>&nbsp;</td> 
                    <td valign="middle"><input type="button" class="image_uploader" style="width:130px" value="ADD/VIEW IMAGE" onClick="file_uploader ( '../../', document.getElementById('txt_booking_no').value,'', 'service_booking', 0 ,1)">
                    </td>
                    <td>&nbsp;</td>
                    <td>
						<? 
                ...
                <tr>
                    <td align="center" colspan="8">
						...
						<input type="button" value="Print Booking5" onClick="generate_trim_report('show_trim_booking_report5')"  style="width:100px;display:none;" name="print_booking5" id="print_booking5" class="formbutton" />  
						<input type="button" value="Print Booking6" onClick="generate_trim_report('show_trim_booking_report6')"  style="width:100px;display:none;" name="print_booking6" id="print_booking6" class="formbutton" /> 
						<input type="button" value="Print Booking7" onClick="generate_trim_report('show_trim_booking_report7')"  style="width:100px;display:none;" name="print_booking7" id="print_booking7" class="formbutton" /> 
                    </td>
                </tr>
            </table>
        </fieldset>
        ...
        <legend>Service Booking</legend>
            <table width="900" cellspacing="2" cellpadding="0" border="0">
                <tr>
                    ...
                    <td id="process_td">
                    	<? echo create_drop_down( "cbo_process", 172, $conversion_cost_head_array,"", 1, "-- Select --", 1, "","","1,3" ); ?>
                    </td>
