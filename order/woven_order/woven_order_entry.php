...
/*$con=connect();
//die;
$job_smv_arr=sql_select( "SELECT a.id as id, a.job_no, a.set_smv, a.inserted_by, sum(b.smv_set) as item_set_smv from  wo_po_details_master a, wo_po_details_mas_set_details b where a.job_no=b.job_no  group by a.id, a.job_no, a.set_smv, a.inserted_by order by a.id DESC");
...
foreach($job_smv_arr as $val)
{
	...
	{
		//echo $val[csf("job_no")].'=='.number_format($val[csf("set_smv")],2).'=='.number_format($val[csf("item_set_smv")],2).'=='.$val[csf("inserted_by")].'<br>';
...
foreach($job_smv_arr as $val)
{
	if(number_format($val[csf("set_smv")],2)!=number_format($val[csf("sew_smv")],2))
	{
		...
		//$query=execute_query("update wo_po_details_master set set_smv='".number_format($val[csf("sew_smv")],2)."' where job_no='".$val[csf("job_no")]."'",1);
		$query=execute_query("update wo_pre_cost_mst set sew_smv='".number_format($val[csf("set_smv")],2)."' where job_no='".$val[csf("job_no")]."'",1);
		$i++;
	}
}
////echo $i; die;
...
foreach($_SESSION['logic_erp']['mandatory_field'][163] as $key=>$value){
	...
	if($key==3 || $key==4 || $key==5 || $key==6 || $key==9 || $key==10 || $key==11){
...
echo "var mst_mandatory_message = '". ($message_mst) . "';\n";
	...
	// if($_SESSION['logic_erp']['mandatory_field'][163][3]!="")
	// {
	// echo "var mst_mandatory_field = '". ($_SESSION['logic_erp']['mandatory_field'][163][3]) . "';\n";
	// echo "var mst_mandatory_message = '". ($_SESSION['logic_erp']['mandatory_field'][163][3]) . "';\n";
	// }
	// if($_SESSION['logic_erp']['mandatory_field'][163][4]!="")
	// {
	// echo "var mst_mandatory_field = '". ($_SESSION['logic_erp']['mandatory_field'][163][4]) . "';\n";
	// echo "var mst_mandatory_message = '". ($_SESSION['logic_erp']['mandatory_field'][163][4]) . "';\n";
	// }
...
echo "var dtls_mandatory_message = '". ($message_dtls) . "';\n";
	...
	// if($_SESSION['logic_erp']['mandatory_field'][163][1]!="" && $_SESSION['logic_erp']['mandatory_field'][163][2]=="")
	// {
	// 	echo "var dtls_mandatory_field = '". ($_SESSION['logic_erp']['mandatory_field'][163][1]) . "';\n";
	// 	echo "var dtls_mandatory_message = '". ($_SESSION['logic_erp']['mandatory_field'][163][1]) . "';\n";
	// }
	// else if($_SESSION['logic_erp']['mandatory_field'][163][2]!="" && $_SESSION['logic_erp']['mandatory_field'][163][1]=="")
	// {
	// 	echo "var dtls_mandatory_field = '". ($_SESSION['logic_erp']['mandatory_field'][163][2]) . "';\n";
	// 	echo "var dtls_mandatory_message = '". ($_SESSION['logic_erp']['mandatory_field'][163][2]) . "';\n";
	// }
	// else if($_SESSION['logic_erp']['mandatory_field'][163][1]!="" && $_SESSION['logic_erp']['mandatory_field'][163][2]!="")
	// {
	// 	echo "var dtls_mandatory_field = '". ($_SESSION['logic_erp']['mandatory_field'][163][1].'*'.$_SESSION['logic_erp']['mandatory_field'][163][2]) . "';\n";
	// 	echo "var dtls_mandatory_message = '". ($_SESSION['logic_erp']['mandatory_field'][163][1].'*'.$_SESSION['logic_erp']['mandatory_field'][163][2]) . "';\n";
	// }
	...
	function openmypage(page_link,title)
	{
		...
		emailwindow.onclose=function()
		{
			...
			if (theemail.value!="")
			{
				...
				show_list_view(theemail.value,'show_po_active_listview','po_list_view','requires/woven_order_entry_controller','');
	...
	function openmypage_file_no()
	{
		...
		var page_link='requires/woven_order_entry_controller.php?action=file_no_popup&cbo_company_name='+cbo_company_name+'&cbo_buyer_name='+cbo_buyer_name; 
	...
	// function MYFUN()
	// {
	// 	if($("#update_id_details").val() && $("#cbo_order_status").val()==2)
	// 	{
	// 		alert("Order Status cannot be changed from 'confirmed' to 'projected'");
	// 		$("#cbo_order_status").val(1);
	// 		return ;
	// 	}
	// }
			...
			emailwindow.onclose=function()
			{
				...
				if (theemail.value!="")
				{
					...
					else
					{
						...
						{
							get_php_form_data( theemail.value, "populate_data_from_search_popup_qc", "requires/woven_order_entry_controller" );
						}
						location_select();
					}
					release_freezing();
	...
	function fnc_order_entry( operation )
	{
		...
		//check working company/season validation
		...
		if (form_validation('cbo_company_name*cbo_location_name*cbo_buyer_name*txt_style_ref*cbo_product_department*txt_item_catgory*cbo_dealing_merchant*item_id*tot_smv_qnty','Company Name*Location Name*Buyer Name*Style Ref*Product Department*Item Catagory*Dealing Merchandiser*Item Details*SMV')==false)
		{
			return;
		}
		else
	...
	function open_color_size_popup(page_link,title)
	{
		...
		else
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1150px,height=550px,center=1,resize=1,scrolling=0','../../')
	...
	function set_excess_cut( val, excs )
	{
		...
		if ( val!="" || val!=0 )
		{
			var excs_cut=return_ajax_request_value(val+"_"+document.getElementById('cbo_company_name').value+"_"+document.getElementById('cbo_buyer_name').value+"_"+document.getElementById('hidd_job_id').value, "get_excess_cut_percent", "requires/woven_order_entry_controller") ;
	...
	function fnc_order_entry_details( operation )
	{
		...
		if(cbo_status!=1 || operation==2)
		{
			var scLc_status = return_ajax_request_value(txt_job_no+"_"+$("#update_id_details").val(), 'sc_lc_status', 'requires/woven_order_entry_controller')
		...
		if(operation==1) //Check Only Update event
		{
			...
			// var status_id = $("#cbo_order_status").val();
			// if(status_id !=1)
			// {
            //     alert('Status Cannot changed');
			// 	release_freezing();
			// 	return;
			// }
		...
		if(operation==2 || cbo_status==2 || cbo_status==3){
			var cutting_qty=return_global_ajax_value(po_id, 'get_cutting_qty_order', '', 'requires/woven_order_entry_controller');
			cutting_qty=cutting_qty.split("**");
			if(cutting_qty[0]==1){
				if(operation==2){
					alert(cutting_qty[1]);
					release_freezing();
					...
					alert(cutting_qty[1].replace('Delete','Inactive'));
					release_freezing();
					return;
				}else if(cbo_status==3 ){
					var sms=cutting_qty[1].replace('Delete not allowed bcz following','Following');
					sms+="\n If You want to Cancel Press Ok; If Not Press Cancel";
					var r=confirm(sms);
					if(r==false){
						release_freezing();
						return;
		...
		if(with_qty==1){
			...
			if (form_validation('update_id*txt_po_no*txt_po_received_date*txt_org_shipment_date','Master Info*PO Number*PO Received Date*Org. Shipment Date')==false){
				release_freezing();
				return;
			}
		}
		...
		var is_of_day=return_global_ajax_value(company_id+"_"+document.getElementById('txt_pub_shipment_date').value+"_"+document.getElementById('txt_org_shipment_date').value+"_"+document.getElementById('cbo_location_name').value, 'is_of_day', '', 'requires/woven_order_entry_controller');
	    ...
	    {
	      var shipping_status = trim(return_global_ajax_value(document.getElementById('txt_job_no').value+'***'+document.getElementById('update_id_details').value, 'check_po_shipping_status_po_id', '', 'requires/woven_order_entry_controller'));
	...
	function fnc_on_submit_reponse()
	{
		if(http.readyState == 4)
		{
			...
			if(trim(reponse[0])=='ProdFound')//ISD-23-04662
			{
				alert("Gmts Production Found. PO Inactive/Cancel Restricted for You. \n If Need Please Contract with Admin User");
				release_freezing();
				return;
			}
	...
	function check_tna_leadtime(){
		...
		var txt_pub_shipment_date=document.getElementById('txt_pub_shipment_date').value
			...
			var tna=return_global_ajax_value(buyer_id+"_"+cbo_company_name+'_'+txt_po_received_date+'_'+txt_pub_shipment_date, 'check_tna_leadtime', '', 'requires/woven_order_entry_controller');
			return tna;
	}
	...
	function pop_entry_actual_po()
	{
		...
		var page_link='requires/woven_order_entry_controller.php?action='+action+'&po_id='+po_id+'&txt_job_no='+txt_job_no+'&gmts_item='+gmts_item+'&po_quantity='+po_quantity+'&job_id='+job_id+'&rcv_date='+po_rcv_date+'&cbo_company_name='+cbo_company_name+'&act_po_id='+act_po_id;
	...
	function pop_entry_actual_po_back()//ISD-21-02141
	{
		...
		var page_link='requires/woven_order_entry_controller.php?action=actual_po_info_popup&po_id='+po_id+'&txt_job_no='+txt_job_no+'&company_id='+company_id;
		var title='Actual Po Entry Info';
	...
	function oepn_requision()
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/woven_order_entry_controller.php?action=requisition_pop_up', "Requisition Pop Up", 'width=1050px,height=470px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function()
		{
	...
	function validate_po_qty()
	{
		...
		if(po_id!=0)
		{
			var cutting_qty=return_global_ajax_value(po_id, 'get_cutting_qty', '', 'requires/woven_order_entry_controller');
	...
	function check_tna_templete(buyer_id){
		var cbo_company_name=$('#cbo_company_name').val();
		var tna=return_global_ajax_value(buyer_id+"_"+cbo_company_name, 'check_tna_templete', '', 'requires/woven_order_entry_controller');
		var data=tna.split("_");
		var temp=data[0];
		var tna=data[1];
	...
	function fnc_file_no_check(file_no_check)
	{
		...
		if(file_no_check==3)
		{
			...
			$('#txt_file_no').attr('readonly',true);
		}
		else
		{
	...
	function get_company_config(company_id)
	{
		...
		load_drop_down( 'requires/woven_order_entry_controller', company_id, 'load_drop_down_sew_location', 'sew_location' );

		get_php_form_data(company_id,'get_company_config','requires/woven_order_entry_controller' ); 
		//location_select();
		po_update_period();
	}
	...
	function get_buyer_config(buyer_id)
	{
		...
		load_drop_down( 'requires/woven_order_entry_controller', buyer_id, 'load_drop_down_season_buyer', 'season_td');
		load_drop_down( 'requires/woven_order_entry_controller', buyer_id+'*'+1, 'load_drop_down_brand', 'brand_td');
	}
	...
	function budget_exceeds_quot(data)
	{
		...
		//alert(set_smv_id)
		//var copy_quotation=return_global_ajax_value(company_id, 'copy_quotation', '', 'requires/woven_order_entry_controller');
		//open_qoutation_popup('requires/woven_order_entry_controller.php?action=quotation_id_popup','Quotation ID Selection Form')
		if(copy_quotation==1)
		{
			if((set_smv_id==2 || set_smv_id==4 || set_smv_id==5 || set_smv_id==6) && cost_control_source==2)
			{
				...
				var page_link="'requires/woven_order_entry_controller.php?action=quotation_id_popup','Quotation ID Selection Form'";// for Price Quotation
			...
			else if(set_smv_id==7 && styleBrowse==2 && cost_control_source==1)
			{
				$('#txt_style_ref').attr('readonly',true);
				$('#txt_style_ref').attr('placeholder','Browse');
				var page_link="'requires/woven_order_entry_controller.php?action=qc_id_popup','Quick Costing Style Selection'";// for Quick Costing
				$('#txt_style_ref').removeAttr("onDblClick").attr("onDblClick","open_qoutation_popup("+page_link+");");
			}
			else if(set_smv_id==7 && styleBrowse!=2 && cost_control_source==1)
			{
			...
			else if(set_smv_id==3 || set_smv_id==8 || set_smv_id==9)
			{
				$('#txt_style_ref').attr('readonly',false);
				$('#txt_style_ref').attr('placeholder','Browse/Write');
				var page_link="'requires/woven_order_entry_controller.php?action=ws_id_popup','Work Study Style Selection'";// for Work Study
				$('#txt_style_ref').removeAttr("onDblClick").attr("onDblClick","open_qoutation_popup("+page_link+");");
			}
			else
			{
...
<!-- Important Field outside Form -->
        ...
        <form name="orderentry_1" id="orderentry_1" autocomplete="off">
            <table width="960" cellspacing="2" cellpadding="0" border="0">
                <tr>
                    ...
                    <td width="140"><input style="width:120px;" type="text" title="Double Click to Search" onDblClick="openmypage('requires/woven_order_entry_controller.php?action=order_popup','Job/Order Selection Form');" class="text_boxes" placeholder="New Job No" name="txt_job_no" id="txt_job_no" readonly />
                    ...
                    <td width="140">
						<?=create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "get_company_config(this.value); location_select();" ); ?>
                    ...
                    <input style="width:65px;" class="text_boxes" name="txt_repeat_job_no" id="txt_repeat_job_no" onDblClick="repeat_openmypage( 'requires/woven_order_entry_controller.php?action=repeat_job_popup', 'Repeat Job Form');" placeholder="Browse Job" /> 
                    </td>
                </tr>
                <tr>
                    ...
                    <td><input class="text_boxes" type="text" style="width:120px" placeholder="Double Click for Quotation" name="txt_style_ref" id="txt_style_ref" onDblClick="open_qoutation_popup('requires/woven_order_entry_controller.php?action=quotation_id_popup','Quotation ID Selection Form');" readonly/></td>
                ...
                <tr>
                    ...
                    <td id="div_teamleader"><?=create_drop_down( "cbo_team_leader", 130, "select id,team_leader_name from lib_marketing_team where project_type=1 and team_type in (0,1,2) and status_active =1 and is_deleted=0 order by team_leader_name","id,team_leader_name", 1, "-- Select Team --", $selected, "load_drop_down( 'requires/woven_order_entry_controller', this.value, 'cbo_dealing_merchant', 'div_marchant' );" ); // issue id=20032; it was remove found.now active //load_drop_down( 'requires/woven_order_entry_controller', this.value, 'cbo_factory_merchant', 'div_marchant_factory' ) ?></td>
                    <td class="must_entry_caption">Dealing Merchandiser</td>
                ...
                <tr>
                    <td>Order Nature</td>
                    <td><?=create_drop_down( "cbo_order_nature", 130, $fbooking_order_nature,"", 1, "-- Select--", 0, "","","" ); ?></td>
                    <td>PO TNA Lead Time</td>
                    <td><?=create_drop_down( "cbo_po_tna_lead_time", 130, $fr_order_tna_lead_time,"", 1, "-- Select--", 0, "","","" ); ?></td>
                    <td>Remarks</td>
                    <td colspan="3"><input class="text_boxes" type="text" style="width:350px;"  name="txt_remarks" id="txt_remarks"/></td>
                </tr>
                <tr>
                    <td>&nbsp;</td>
                    <td><input type="button" class="image_uploader" style="width:130px" value="ADD/VIEW IMAGE" onClick="file_uploader ( '../../', document.getElementById('update_id').value,'', 'knit_order_entry', 0 ,1);"></td>
                    <td>&nbsp;</td>
                    <td><input type="button" class="image_uploader" style="width:130px" value="ADD FILE" onClick="file_uploader ( '../../', document.getElementById('update_id').value,'', 'knit_order_entry', 2 ,1);"></td>
                    <td>&nbsp;</td>
                    <td><input type="button" id="set_button" class="image_uploader" style="width:130px;" value="Internal Ref" onClick="open_terms_condition_popup('requires/woven_order_entry_controller.php?action=terms_condition_popup','Terms Condition');" /></td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                </tr>
                <tr>
    ...
    <legend>PO Details Entry</legend>
        <form id="orderdetailsentry_2" autocomplete="off">
            <table style="border:none" width="915px" cellpadding="0" cellspacing="2" border="0">
                <thead class="form_table_header">
                    <tr>
                        ...
                        <th width="70">Org. Shipment Date</th>
                        ...
                        <th width="60" class="must_entry_caption">Avg. Price</th>
                ...
                <tr>
                    <td><?=create_drop_down( "cbo_order_status", 80, $order_status,"", 0, "----", 1,"po_recevied_date( this.value )", "","","","" ); ?>
                    <input name="txt_order_status" id="txt_order_status" type="hidden" value=""  style="width:65px " readonly/>
                    </td>
                    <td>
                    <input class="text_boxes" name="txt_po_no" id="txt_po_no" type="text" value="" onDblClick="openmypage_for_po_copy('requires/woven_order_entry_controller.php?action=order_popup_for_copy','Order Selection Form');" style="width:90px"/></td>
                    <td><input name="txt_po_received_date" id="txt_po_received_date" class="datepicker" type="text" onChange="set_tna_task();" style="width:60px;" readonly/></td>
                    <td><input name="txt_pub_shipment_date" id="txt_pub_shipment_date" class="datepicker" type="text" onChange="set_tna_task();" style="width:60px;" readonly/></td>
                    <td><input name="txt_org_shipment_date" id="txt_org_shipment_date" class="datepicker" type="text" style="width:60px;" onChange="set_pub_ship_date();" readonly/></td>
                    <td><input  name="txt_factory_rec_date" id="txt_factory_rec_date" class="datepicker" type="text" style="width:60px;" value="<?=date('d-m-Y')?>" readonly/></td>
                    <td>
                        <input name="txt_po_quantity" id="txt_po_quantity" onDblClick="open_color_size_popup('requires/size_color_breakdown_controller.php?action=populate_size_color_breakdown_pop_up','Color Size Entry Form')" class="text_boxes_numeric" type="text" style="width:70px" onBlur="set_excess_cut(this.value,document.getElementById('txt_excess_cut').value);" onChange="validate_po_qty();" placeholder="Dbl. Click"/>
                        <input type="hidden" id="with_qty_pop" name="with_qty_pop" value="1"/>
                        <input type="hidden" id="hidden_po_qty" name="hidden_po_qty" value=""/>
                    </td>
                    <td><input name="txt_avg_price" id="txt_avg_price" onBlur="math_operation( 'txt_amount', 'txt_avg_price*txt_po_quantity', '*','',{dec_type:1,comma:0,currency:document.getElementById('cbo_currercy').value} ); fnc_avg_price_check(this.value);" quot_cost="" class="text_boxes_numeric" type="text" style="width:50px" /></td>
                    <td><input name="txt_amount" id="txt_amount" class="text_boxes_numeric" type="text" value=""  style="width:75px" readonly/></td>
                    <td><input name="txt_excess_cut" id="txt_excess_cut" class="text_boxes_numeric" onChange="calculate_plan_cut();" style="width:50px" disabled/></td>
                    <td><input name="txt_plan_cut" id="txt_plan_cut"  class="text_boxes_numeric" type="text" value=""  style="width:60px " readonly/></td>
                    <td><?=create_drop_down("cbo_status", 80, $row_status, 0, "", 1, "","check_po_shipping_status()"); ?></td>
                    <td>&nbsp;</td>
                </tr>
                <tr>
                    <td align="right"><strong>Projected Po</strong></td>
                    <td id="projected_po_td"><?=create_drop_down("cbo_projected_po", 100, $blank_array, "", 1, "--Select--", ""); ?></td>
                    <td align="right" colspan="2"><strong>TNA From /Upto</strong></td>
                    <td id="tna_task_td" colspan="2"><?=create_drop_down("cbo_tna_task", 70, $blank_array, "", 1, "--Select--", ""); ?></td>
                    <td align="right" colspan="2"><strong>Internal Ref/Grouping</strong></td>
                    <td><input type="text" id="txt_grouping" class="text_boxes" style="width:70px"></td>
                    <td align="right"><strong>Delay For</strong></td>
                    <td colspan="2"><?=create_drop_down("cbo_delay_for", 150, $delay_for, 0, "", 1, ""); ?></td>
                </tr>
                <tr>
                    ...
                    <td colspan="2">ETD/LDD<input name="txt_etd_ldd" id="txt_etd_ldd" class="datepicker" style="width:50px" placeholder="Date"></td>
