...
/*-------------------------------------------- Comments
Version          :  V1
Purpose			 : This form will create Additional print Booking
...
<script>
	...
	function fnc_generate_booking(param,po_id,pre_cost_id,cbo_company_name){
		...
		http.open("POST","requires/multi_job_additional_print_booking_controller.php",true);
	...
	function load_print_button() {
		...
		get_php_form_data( company_id, 'load_button', 'requires/multi_job_additional_print_booking_controller' );
	}
	...
	function openmypage_booking(page_link,title)
	{
		...
		emailwindow.onclose=function()
		{
			...
			if (theemail.value!="")
			{
				...
				get_php_form_data( theemail.value, "populate_data_from_search_popup", "requires/multi_job_additional_print_booking_controller" );
	...
	function openmypage_emb_item(page_link,title){
		...
		var picheck=trim(return_global_ajax_value(txt_booking_no, 'check_pi_number', '', 'requires/multi_job_additional_print_booking_controller'));
		if(picheck==1){
	...
	function fnc_trims_booking_dtls( operation ){
		...
		for (var i=1; i<=row_num; i++){
			...
			{
					var booking_msg="Wo Amount less than Budget amount not allowed.";
					alert(booking_msg);
					release_freezing();
					return;
			}			
	...
	function fnc_fabric_booking( operation )
	{
		...
		{
			if('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][612]);?>')
			{
				if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][612]);?>','<? echo implode('*',$_SESSION['logic_erp']['mandatory_message'][612]);?>')==false)
				{
				release_freezing();
				return;
				}
	...
	function auto_completesupplier2()
	{
		...
		var supplier = return_global_ajax_value( company_id+'_'+pay_mode, 'supplier_company_action', '', 'requires/multi_job_additional_print_booking_controller');
		...
		$("#cbo_supplier_name").autocomplete({
			...
			/*select: function (e, ui) {
				...
				var attention = return_global_ajax_value( ui.item.id, 'load_attention', '', 'requires/multi_job_additional_print_booking_controller');
    ...
    <div style="width:100%;" align="center">
        ...
        <form name="printbooking_1"  autocomplete="off" id="printbooking_1">
            <fieldset style="width:1000px;">
                <legend>Additional Service Booking</legend>
                <table width="1000" cellspacing="2" cellpadding="0" border="0">
                    <tr>
                        <td colspan="4" align="right" class="must_entry_caption"> Wo No </td>
                        <td colspan="4"><input class="text_boxes" type="text" style="width:130px" onDblClick="openmypage_booking('requires/multi_job_additional_print_booking_controller.php?action=fabric_booking_popup','Print Booking Search')" readonly placeholder="Double Click for Booking" name="txt_booking_no" id="txt_booking_no"/>
                    ...
                    <tr>
                        ...
                        <td width="140"><?=create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company where status_active=1 and is_deleted=0 and core_business not in(3) order by company_name", "id,company_name",1, "-- Select Company --", $selected, "load_drop_down( 'requires/multi_job_additional_print_booking_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );get_php_form_data( this.value, 'load_button', 'requires/multi_job_additional_print_booking_controller' );",'',"" );  ?></td>
                    ...
                    <tr>
                        <td align="center" valign="middle" colspan="8">
							...
							<? echo create_drop_down( "", 100, $report_template_list,'', 0, '', 0, ""); ?>
                            <input type="button" value="Print Booking" onClick="generate_trim_report('show_trim_booking_report2',1)"  style="width:100px;" name="print_booking1" id="print_booking1" class="formbutton" />
