...
function fnc_fabric_booking_reponse(){
	if(http.readyState == 4){
		...
		if(parseInt(trim(reponse[0]))==2)
		{
			...
			reset_form('fabricbooking_1*orderdetailsentry_2','','booking_list_view','cbo_pay_mode,3*cbo_booking_year,2016*cbo_currency,2*cbo_ready_to_approved,2*txt_booking_percent,100*txt_booking_date,<? echo date("d-m-Y"); ?>')
		}
...
function fnc_fabric_booking_dtls( operation )
{
	...
	var mstdata=get_submitted_data_string('txt_booking_no*update_id*txt_job_no*cbo_order_id*cbo_gmt_item_id*cbo_fabricdescription_id*newdia*saveddia*colortype*construction*composition*gsm_weight',"../../",i);
...
function fnc_fabric_booking_dtls_reponse(){
	if(http.readyState == 4){
        ...
        if(trim(reponse[0])=='iss1'){
		 ...
		 if(parseInt(trim(reponse[0]))==2)
		 {
			 reset_form('orderdetailsentry_2','breackdown_form','','','other_work()','');
		 }
...
function check_exchange_rate(){
	...
	var response=return_global_ajax_value( cbo_currercy+"**"+booking_date+"**"+cbo_company_name, 'check_conversion_rate', '', 'requires/fabric_booking_controller_v2');
        ...
        <fieldset style="width:960px;">
            ...
            <table width="960" cellspacing="2" cellpadding="0" border="0">
                ...
                <tr>
                    ...
                    <td width="150"><?=create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company  comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name", "id,company_name",1, "-- Select Company --", "", "load_drop_down( 'requires/fabric_booking_controller_v2', this.value, 'load_drop_down_buyer', 'buyer_td' );check_month_setting();validate_suplier();check_exchange_rate();",0,"" ); ?></td>
                    ...
                    <td>
						<?=create_drop_down( "cbo_booking_month", 70, $months,"", 1, "-- Select --", "", "",0 ); ?>
                ...
                <tr>
                    ...
                    <td colspan="3">
                        <input class="text_boxes" type="text" style="width:370px;" placeholder="Double click for Order"  onDblClick="openmypage_order('requires/fabric_booking_controller_v2.php?action=order_search_popup','Order Search')"   name="txt_order_no" id="txt_order_no"/>
                        <input class="text_boxes" type="hidden" style="width:172px;"  name="txt_order_no_id" id="txt_order_no_id"/>
                    </td>                                
                </tr>
                <tr>
                    <td>Pay Mode</td>
                    <td><?=create_drop_down( "cbo_pay_mode", 130, $pay_mode,"", 1, "-- Select Pay Mode --", 3, "load_drop_down( 'requires/fabric_booking_controller_v2', this.value, 'load_drop_down_suplier', 'sup_td' )","","1,2,3,5" ); ?></td>
                    <td>Supplier Name</td>
                    <td id="sup_td"><?=create_drop_down( "cbo_supplier_name", 130, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=9 and a.status_active =1 and a.is_deleted=0 order by supplier_name","id,supplier_name", 1, "-- Select Supplier --", $selected, "",0 ); ?></td> 
                    <td>Currency</td>
                ...
                <tr>
                    <td>Attention</td>   
                    <td colspan="3"><input class="text_boxes" type="text" style="width:365px;"  name="txt_attention" id="txt_attention"/></td>
                ...
                <tr>
                    ...
                    <td colspan="4">
                        <input type="button" id="set_button" class="image_uploader" style="width:100px;" value="Terms & Condition/Notes" onClick="open_terms_condition_popup('requires/fabric_booking_controller_v2.php?action=terms_condition_popup','Terms Condition')" />
                        <input type="button" id="set_button" class="image_uploader" style="width:130px;" value="Process Loss %" onClick="open_rmg_process_loss_popup('requires/fabric_booking_controller_v2.php?action=rmg_process_loss_popup','Process Loss %')" />
