...
function openmypage(page_link,title)
{
	...
	emailwindow.onclose=function()
	{
		...
		{
			freeze_window(5);
			get_php_form_data(theemail.value, "populate_data_from_search_popup", "requires/trims_booking_multi_job_controller" );
...
{
	if (form_validation('cbo_booking_month*cbo_company_name','Month*Company')==false)
	{
		return;
	}	
	else
	{
		...
		var page_link='requires/trims_booking_multi_job_controller.php?action=fnc_po_select_data';
		...
		emailwindow.onclose=function()
		{
			...
			var theemail_job=this.contentDoc.getElementById("txt_job_id");
			if (theemail.value!="")
			{
				document.getElementById('txt_job_no').value=theemail_job.value;
...
{
	if (form_validation('cbo_booking_month*cbo_booking_year*cbo_company_name','Month*Year*Company')==false)
	{
		return;
	}	
	else
	{
		...
		{
			var page_link='requires/trims_booking_multi_job_controller.php?action=fnc_process_data';
		}
		if(cbo_item_from_precost==2)
		{
			var page_link='requires/trims_booking_multi_job_controller.php?action=fnc_process_data_item_from_library';
		}
		//alert(cbo_item_from_precost);
		var title='Trim Booking Search';
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1330px,height=450px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0];
			var theemail=this.contentDoc.getElementById("txt_selected_id");
			var theemail2=this.contentDoc.getElementById("txt_job_id");
			//var theemail=split(",",theemail);
			
			//var theemail=item_job[0];
			//var job=item_job[1];
			...
			if (theemail.value!="")
			{
				//alert(theemail2.value);
				document.getElementById('txt_select_item').value=theemail.value;
				//document.getElementById('txt_job_no').value=theemail2.value;
				//get_php_form_data(theemail2.value, "populate_data_from_search_popup", "requires/trims_booking_multi_job_controller" );
...
function fnc_generate_booking(param,cbo_company_name)
{
		...
		http.open("POST","requires/trims_booking_multi_job_controller.php",true);
...
function openmypage_booking(page_link,title)
{
		...
		emailwindow.onclose=function()
		{
			...
			if (theemail.value!="")
			{
				...
				$("#cbo_currency").attr("disabled",true);
				get_php_form_data( theemail.value, "populate_data_from_search_popup_booking", "requires/trims_booking_multi_job_controller" );
...
function fnc_trims_booking( operation )
{
	...
	{
		 data_all=data_all+get_submitted_data_string('txt_booking_no*cbo_booking_month*cbo_booking_year*txt_job_no*cbo_company_name*cbo_buyer_name*cbo_supplier_name*cbo_currency*txt_exchange_rate*txt_booking_date*txt_delivery_date*cbo_pay_mode*cbo_source*txt_attention*txt_tenor*txt_remarks*cbo_item_from_precost',"../../");
	}
	...
	for (var i=1; i<=row_num; i++)
	{
		...
		//else
		//{
			data_all=data_all+get_submitted_data_string('txtbookingid_'+i+'*txtpoid_'+i+'*txtcountry_'+i+'*txttrimcostid_'+i+'*txttrimgroup_'+i+'*txtuom_'+i+'*cbocolorsizesensitive_'+i+'*txtwoq_'+i+'*txtrate_'+i+'*txtamount_'+i+'*txtddate_'+i+'*consbreckdown_'+i+'*txtexchrate_'+i+'*txtjob_'+i,"../../",2);
...
function select_po_enable_disable(value)
{
	...
	if (form_validation('txt_booking_no','Booking No')==false)
		...
		else
		{
			...
			var data="action="+action+get_submitted_data_string('txt_booking_no*cbo_company_name*id_approved_id',"../../")+'&report_title='+$report_title+'&show_comment='+show_comment+'&path=../../';
			//freeze_window(5);
			http.open("POST","requires/trims_booking_multi_job_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
...
function popolate_job_data(data)
{
  ...
  get_php_form_data($job_no, "populate_data_from_search_popup", "requires/trims_booking_multi_job_controller" );
...
function fill_attention(supplier_id){
	...
	var attention=return_global_ajax_value(supplier_id, 'get_attention_name', '', 'requires/trims_booking_multi_job_controller');
...
<div style="width:100%;" align="center">
     <? echo load_freeze_divs ("../../",$permission);  ?>
                ...
                <legend title="V3">Trims Booking &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;   <font id="app_sms" style="color:#F00"></font></legend>
                    ...
                    <td  width="170" >
                        <input class="text_boxes" type="text" style="width:160px" onDblClick="openmypage_booking('requires/trims_booking_multi_job_controller.php?action=trims_booking_popup','Trims Booking Search')" readonly placeholder="Double Click for Booking" name="txt_booking_no" id="txt_booking_no"/>
                    </td>
                    <td align="">
                    ...
                    <tr>
                            ...
                            <td>
                              <? 
							  //load_drop_down( 'requires/trims_booking_multi_job_controller', this.value, 'load_drop_down_supplier', 'supplier_td' );
							  	echo create_drop_down( "cbo_company_name", 172, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected, "load_drop_down( 'requires/trims_booking_multi_job_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );get_php_form_data( this.value, 'populate_variable_setting_data', 'requires/trims_booking_multi_job_controller' );","","" );
								?>	  
                           </td>
                           <td  width="130"  align="right">Job No</td>              
                           <td  width="170" >
                                 
                                <input style="width:160px;" type="text"  class="text_boxes" placeholder="Display No" name="txt_job_no" id="txt_job_no" readonly disabled />   
                        ...
                        <td align="right">Buyer Name</td>
                            ...
                            <td  align="right" class="must_entry_caption">Pay Mode</td>
                                <td>
                               <?
							   		echo create_drop_down( "cbo_pay_mode", 172, $pay_mode,"", 1, "-- Select Pay Mode --", "", "load_drop_down( 'requires/trims_booking_multi_job_controller', this.value, 'load_drop_down_supplier', 'supplier_td' )","" );
							   ?> 
                                 </td>
                        ...
                        <tr>
                             ...
                             <td  width="130" height="" align="right" class="must_entry_caption"> Source </td>
                                <td  width="170" >
                                	<?
                        ...
                        <tr>
                        	...
                        	<td align="left" height="10" colspan="3">
                            	<input class="text_boxes" type="text" style="width:97%;"  name="txt_remarks" id="txt_remarks"/>                            </td>
                           <td  width="130"  align="right" class="must_entry_caption"> Select Item </td> 
                           <td  width="170" >
						...
						<td><input style="width:160px;" type="text" class="text_boxes_numeric" name="txt_tenor" id="txt_tenor" /></td>
                         <td align="right"></td> 
                           <td align="center" height="10">
                            <input type="button" id="set_button" class="image_uploader" style="width:160px;" value="Terms & Condition/Notes" onClick="open_terms_condition_popup('requires/trims_booking_multi_job_controller.php?action=terms_condition_popup','Terms and Condition')" /></td>
                        </tr>
                        <tr>
