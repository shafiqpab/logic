        ...
        <legend>Service Booking</legend>
            <table  width="900" cellspacing="2" cellpadding="0" border="0" style="">
                ...
                <tr>
                    ...
                    <? 
                    	echo create_drop_down( "cbo_booking_month", 90, $months,"", 1, "-Select-", "", "",0 );		
                   		echo create_drop_down( "cbo_booking_year", 60, $year,"", 1, "-Select-", date('Y'), "",0 );		
                    ?>
                    </td>
                    <td class="must_entry_caption">Selected Order No</td>   
                    <td colspan="3">
                    	<input class="text_boxes" type="text" style="width:440px" placeholder="Browse Order"  onDblClick="openmypage_order('requires/service_booking_knitting_controller.php?action=order_search_popup','Order Search')"   name="txt_order_no" id="txt_order_no"/>
                ...
                <tr>
                    ...
                    <td width="170">
                    	<input style="width:150px;" type="text" class="text_boxes"  name="txt_job_no" id="txt_job_no" disabled  /> 
                    </td>
                    <td width="120" class="must_entry_caption">Company Name</td>
                    <td width="170">
						<? 
                            echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected, "load_drop_down( 'requires/service_booking_knitting_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );get_php_form_data( this.value, 'print_report_button', 'requires/service_booking_knitting_controller');check_exchange_rate();","","" );
                        ?>	  
                    </td>
                    <td width="120">Buyer Name</td>   
                    <td id="buyer_td"> 
						<?  
                        	echo create_drop_down( "cbo_buyer_name", 160, "select id,buyer_name from lib_buyer where status_active =1 and is_deleted=0 order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "",1,"" );
                        ?>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
						<? 
                        	echo create_drop_down( "cbo_currency", 160, $currency,"", 1, "-- Select --", 1, "check_exchange_rate();",0 );//	set_conversion_rate(this.value, $('#txt_booking_date').val(), '../../', 'txt_exchange_rate')	
                        ?>	
                    </td>
                    <td>Exchange Rate</td>
                    <td>
                    	<input style="width:150px;" type="text" class="text_boxes"  name="txt_exchange_rate" id="txt_exchange_rate"  readonly />  
                    </td>
                    <td>Booking Date</td>
                    <td>
                    	<input class="datepicker" type="text" style="width:150px" name="txt_booking_date" id="txt_booking_date" onChange="check_exchange_rate();" value="<? echo date("d-m-Y")?>" disabled />	
                    </td>
                </tr>
                <tr>
                    <td>Delivery Date</td>
                    <td>
                    	<input class="datepicker" type="text" style="width:150px" name="txt_delivery_date" id="txt_delivery_date"/>	
                    </td>
                    <td class="must_entry_caption">Pay Mode</td>
                    <td>
                    	<? echo create_drop_down( "cbo_pay_mode", 160, $pay_mode,"", 1, "-- Select Pay Mode --", "", "load_drop_down( 'requires/service_booking_knitting_controller', this.value, 'load_drop_down_supplier', 'supplier_td' )","" ); ?> 
                    </td>
                    <td>Source</td>              <!-- 11-00030  -->
                    <td > <? echo create_drop_down( "cbo_source", 160, $source,"", 1, "-- Select Source --", "", "","" ); ?></td>
                </tr>
                <tr>
                    <td>Supplier/Party Name</td>
                    <td id="supplier_td">
						<?
                            echo create_drop_down( "cbo_supplier_name", 160, "select id,supplier_name from lib_supplier where status_active =1 and is_deleted=0 order by supplier_name","id,supplier_name", 1, "-- Select Supplier --", $selected, "get_php_form_data( this.value+'_'+document.getElementById('cbo_pay_mode').value, 'load_drop_down_attention', 'requires/service_booking_knitting_controller');",0 );
                        ?> 
                    </td> 
                    <td>Attention</td>   
                    <td colspan="3">
                    	<input class="text_boxes" type="text" style="width:440px;"  name="txt_attention" id="txt_attention"/>
                 ...
                 <tr>
					...
					<td><input class="text_boxes" type="text" style="width:160px;"  name="txt_fab_booking" id="txt_fab_booking" onDblClick="fnc_fab_booking('requires/service_booking_knitting_controller.php?action=fabric_booking_popup','fabric Booking Search')" placeholder="Browser" readonly/> </td>
                    
                </tr>
                <tr>
                    <td></td> 
                    <td valign="middle">
                    	<input type="button" class="image_uploader" style="width:160px" value="CLICK TO ADD/VIEW IMAGE" onClick="file_uploader ( '../../', document.getElementById('txt_booking_no').value,'', 'service_booking', 0 ,1)">
                    </td>
                    <td></td>
