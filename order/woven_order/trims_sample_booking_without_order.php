...
function openmypage_booking(page_link,title)
{
	...
	emailwindow.onclose=function()
	{
		...
		if (theemail.value!="")
		{
			...
			get_php_form_data( theemail.value, "populate_data_from_search_popup", "requires/trims_sample_booking_without_order_controller" );
			reset_form('orderdetailsentry_2','booking_list_view','','','')
			show_list_view(theemail.value,'show_fabric_booking','booking_list_view','requires/trims_sample_booking_without_order_controller','setFilterGrid(\'list_view\',-1)');
...
{
	var color_from_library=return_global_ajax_value(company_id, 'color_from_library', '', 'requires/trims_sample_booking_without_order_controller');
...
{
	//var page_link='requires/trims_sample_booking_without_order_controller.php?action=color_popup'
	//alert(page_link)
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/trims_sample_booking_without_order_controller.php?action=color_popup&buyer_name='+buyer_name, 'Color Select Pop Up', 'width=250px,height=450px,center=1,resize=1,scrolling=0','../')
	emailwindow.onclose=function()
	{
		var theform=this.contentDoc.forms[0];
...
function fnc_fabric_booking( operation )
{
 ...
 if (form_validation('cbo_company_name*cbo_buyer_name*txt_booking_date*cbo_pay_mode','Company Name*Buyer Name*Fabric Nature*Fabric Source*Booking Date*Pay Mode')==false)
	...
	{
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_booking_no*cbo_currency*txt_exchange_rate*cbo_pay_mode*txt_booking_date*cbo_supplier_name*txt_attention*txt_tenor*delivery_address*txt_delivery_date*cbo_source*txt_remark*cbo_ready_to_approved',"../../");
		freeze_window(operation);
		http.open("POST","requires/trims_sample_booking_without_order_controller.php",true);
...
function open_sample_popup()
{
	...
	var page_link='requires/trims_sample_booking_without_order_controller.php?action=sample_description_popup&cbo_company_name='+cbo_company_name+'&cbo_buyer_name='+cbo_buyer_name;
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, 'Sample Description', 'width=1200px,height=450px,center=1,resize=1,scrolling=0','../');
	...
	emailwindow.onclose=function()
	{
		...
		load_drop_down( 'requires/trims_sample_booking_without_order_controller', cbo_buyer_name+'_'+style_id.value, 'load_drop_down_buyer_tag_sample', 'sample_td' );
		
		load_drop_down( 'requires/trims_sample_booking_without_order_controller', style_id.value+'_'+sample_id.value, 'load_drop_down_trim_group', 'tgroup_td' );
...
function fnc_fabric_booking_dtls( operation )
{
	...
	if (form_validation('txt_booking_no*txt_style_no*cbo_trim_group*txt_trim_qty*cbo_sample_type*txt_description','Booking No*Style*Trims Group*Finish Fabric*Sample type*Fabric Description')==false)
	{
		return;
	}	
	var data="action=save_update_delete_dtls&operation="+operation+get_submitted_data_string('txt_booking_no*cbo_trim_group*cbo_uom*txt_style*txt_style_des*cbo_sample_type*txt_description*txt_barnd_sup_ref*txt_gmt_color*txt_color*txt_gmts_size*txt_size*txt_trim_qty*txt_trim_req_qty*txt_rate*txt_amount*update_id_details*txt_remarks',"../../");
	//alert(data);
	//return;
	freeze_window(operation);
	http.open("POST","requires/trims_sample_booking_without_order_controller.php",true);
...
function fnc_fabric_booking_dtls_reponse()
{
	if(http.readyState == 4) 
	{
		...
		show_list_view(reponse[1],'show_fabric_booking','booking_list_view','requires/trims_sample_booking_without_order_controller','setFilterGrid(\'list_view\',-1)');
		release_freezing();
	}
}
...
function set_trim_cons_uom(trim_group_id){
	...
	var response=return_global_ajax_value(trim_group_id+'_'+txt_style_id+'_'+cbo_sample_type, 'set_cons_uom', '', 'requires/trims_sample_booking_without_order_controller');
	...
	else  { $("#txt_description").attr('disabled',false);}
}
...
function fnc_trimGroup(sam_type)
{
	...
	load_drop_down( 'requires/trims_sample_booking_without_order_controller', txt_style_id+'_'+cbo_sample_type, 'load_drop_down_trim_group', 'tgroup_td' );
}
function fnc_desc_popup()
{
	...
	var page_link='requires/trims_sample_booking_without_order_controller.php?action=trim_description_popup&cbo_trim_group='+cbo_trim_group+'&txt_style_id='+txt_style_id+'&cbo_sample_type='+cbo_sample_type;
	...
	emailwindow.onclose=function()
	{
		...
		var trim_qty_balance=(req_qty.value-previous_qty.value);
		//alert(trim_qty_balance)
		...
		document.getElementById('txt_prev_trim_req_qty').value=previous_qty.value;
     ...
     <? echo load_freeze_divs ("../../",$permission);  ?>
                ...
                <legend>Sample Booking (Without Order) </legend>
                    ...
                    <tr>
                                ...
                                <td  width="170" >
                                	<input class="text_boxes" type="text" style="width:160px" onDblClick="openmypage_booking('requires/trims_sample_booking_without_order_controller.php?action=trims_booking_popup','Trims Booking Search')" readonly placeholder="Double Click for Booking" name="txt_booking_no" id="txt_booking_no"/>
                                </td>
                              <td  align="right" class="must_entry_caption">Company Name</td>
                              <td>
                              <? 
							  	echo create_drop_down( "cbo_company_name", 172, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected, "load_drop_down( 'requires/trims_sample_booking_without_order_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/trims_sample_booking_without_order_controller', this.value, 'load_drop_down_supplier', 'supplier_td' );color_from_library( this.value )","","" );
								?>	  
                              </td>
                       ...
                       <td align="right">Delivery Date</td>
                        	<td>
                                ...
                                <td>
                               <?
							   		echo create_drop_down( "cbo_pay_mode", 172, $pay_mode,"", 1, "-- Select Pay Mode --", 3, "load_drop_down( 'requires/trims_sample_booking_without_order_controller', this.value, 'load_drop_down_supplier', 'supplier_td' )","" );
							   ?> 
                                 </td>
                        </tr>
                        <tr>
                        <td align="right">Currency</td>
                        	...
                        	<td align="right">Exchange Rate</td>
                             ...
                             <td  align="right">Supplier Name</td>
                                <td id="supplier_td">
                               <?
							   		echo create_drop_down( "cbo_supplier_name", 172, "select id,supplier_name from lib_supplier where status_active =1 and is_deleted=0 order by supplier_name","id,supplier_name", 1, "-- Select Supplier --", $selected, "get_php_form_data( this.value, 'load_drop_down_attention', 'requires/trims_sample_booking_without_order_controller');",0 );
							   ?> 
                                 </td> 
                              </tr>
                        <tr>
                        ...
                        <tr>
                    		...
                    		<td height="10"><textarea id="delivery_address" name="delivery_address" class="text_area" type="text" style="width:160px;" placeholder="Delivery Address" ></textarea></td>
                        </tr>
						<tr>
							<td align="right">Remarks</td>
                            <td colspan="5"><input class="text_boxes" type="text" maxlength="400" style="width:722px"  name="txt_remark" id="txt_remark"/></td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                        	<td align="center" colspan="6" valign="middle" class="button_container">
                              ...
                              <input type="hidden" class="image_uploader" style="width:162px" value="Lab DIP No" onClick="openmypage('requires/trims_sample_booking_without_order_controller.php?action=lapdip_no_popup','Lapdip No','lapdip')">
              ...
              <form name="orderdetailsentry_2"  autocomplete="off" id="orderdetailsentry_2">
                ...
                <legend>Sample Booking (Without Order) </legend>
            		<table  width="900" cellspacing="2" cellpadding="0" border="0">
                    <tr>
                                <td  width="130" height="" class="must_entry_caption" align="right" > Style ref </td>              <!-- 11-00030  -->
                                <td  width="170" >
                       ...
                       <td align="right" class="must_entry_caption">Trims Group</td>
                        	<td id="tgroup_td">
                               ...
                               UOM
                                ...
                                <td>	
                                     <?  echo create_drop_down( "cbo_uom", 172, $unit_of_measurement,"", 1, "--Select--", $selected, "",1 ); ?>
                                </td>
                       <td  width="130" align="right" class="must_entry_caption"> Description</td>
                                <td  width="170"> 
							  <input type="text" id="txt_description"    name="txt_description" placeholder="Browse" onClick="fnc_desc_popup();"  class="text_boxes" style="width:160px" />
                              </td>
                        ...
                        <td  align="right">Item size</td>
                              ...
                              <td  align="right" class="must_entry_caption">Qty</td>
							   ...
							   <input name="txt_prev_trim_req_qty" id="txt_prev_trim_req_qty" class="text_boxes_numeric" type="hidden"  style="width:20px"/>
                                 </td>
                        </tr>
                        <tr>
							...
							<td align="right">Remarks</td>
                    		<td height="10"><textarea id="txt_remarks" name="txt_remarks" class="text_area" type="text" style="width:160px;" placeholder="Remarks" ></textarea></td>
                        </tr>
                        ...
                        <tr>
                        	<td align="center" colspan="6" valign="middle" class="button_container">
                              <?
									...
									<input type="button" value="Print Booking 2" onClick="generate_trim_report('show_trim_booking_report',1)"  style="width:100px;" name="print_booking2" id="print_booking2" class="formbutton" />
                            </td>
                        </tr>
                    </table>
              </fieldset>
