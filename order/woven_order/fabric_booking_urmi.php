...
?>
	...
	function fnc_generate_booking()
	{
		...
		{
			var data="action=generate_fabric_booking"+get_submitted_data_string('txt_job_no*txt_order_no_id*txt_booking_no*cbo_fabric_natu*cbo_fabric_source*txt_booking_percent*cbo_company_name*cbo_buyer_name*cbouom*txt_colar_excess_percent*txt_cuff_excess_percent',"../../");
	...
	function fnc_generate_booking_reponse()
	{
		if(http.readyState == 4)
		{
			...
			//$('#print22').removeAttr('href').attr('href','requires/'+trim(file_data[0]));
	...
	{
		if(operation==2) //Delete Alert(Issue ID 29289)
			{
		...
		if(http.readyState == 4)
		{
			...
			if(trim(reponse[0])=='parb1'){
			 ...
			 if(parseInt(trim(reponse[0]))==2)
			 {
				 ...
				 reset_form('fabricbooking_1','booking_list_view','','cbo_pay_mode,5*cbo_booking_year,2019*cbo_currency,2*cbo_ready_to_approved,2*txt_booking_percent,100*txt_booking_date,<? echo date("d-m-Y"); ?>','','cbo_company_name*cbo_booking_year*cbo_booking_month*cbo_fabric_natu*cbo_fabric_source*txt_booking_date*txt_delivery_date*cbo_supplier_name*cbo_pay_mode*cbo_source*cbo_currency')
			 }
	...
	var fab_source=$("#cbo_fabric_source").val()*1;
		...
		for(var i=1; i<=row_num; i++)
		{
			  ...
			  if(fab_source==2)
			{
				if (form_validation('rate_'+i,'Rate')==false)
				{
					release_freezing();
					return;   
				}
		...
		var data="action=save_update_delete_dtls&operation="+operation+'&total_row='+row_num+get_submitted_data_string('cbo_company_name*txt_booking_no*update_id*txt_order_no_id*txt_job_no*selected_id_for_delete*cbo_pay_mode*cbo_fabric_natu*cbo_fabric_source*hidd_apply_last_update_btn',"../../")+dataAll+dataAll2;
		//alert(data);
		//freeze_window(operation);
		if(operation==2)
		{
	...
	function fnc_fabric_booking_dtls_reponse(){
		if(http.readyState == 4) {
			...
			if(parseInt(trim(reponse[0]))==0 || parseInt(trim(reponse[0]))==1){
				...
				release_freezing();
				fnc_show_booking(1);
				$('#hidd_apply_last_update_btn').val(0);
			}
			...
			if(trim(reponse[0])=='budgetOver'){
				alert("Budget Over than  Booking Qty.")
				release_freezing();
				return;
			}
			if(trim(reponse[0])=='budget_updated'){
				alert("Budget Fabric Sensitive Changed Found,\n Please go to  Selected Order No: Popup and call the Job/PO again.");
				release_freezing();
				return;
			}
	...
	function fnc_show_booking(type)
	{
		...
		if(type==2){
			document.getElementById('app_sms3').innerHTML = ''
			$('#hidd_apply_last_update_btn').val(2);
		}
		...
		{
			var data="action=show_fabric_booking"+get_submitted_data_string('txt_job_no*txt_order_no_id*cbo_fabric_natu*cbo_fabric_source*txt_booking_no*txt_booking_percent*cbouom*txt_colar_excess_percent*txt_cuff_excess_percent*hidd_apply_last_update_btn',"../../")+"&type="+type;
			http.open("POST","requires/fabric_booking_urmi_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
	...
	function validate_value( i , type)
	{
		if(type=='finish')
		{
			var real_value =document.getElementById('finscons_'+i).placeholder*1;
			var user_given =document.getElementById('finscons_'+i).value*1;
			if(user_given> real_value)
			{
				alert("Over booking than budget not allowed="+user_given+'='+real_value);
		...
		if(type=='grey')
		{
			var real_value =document.getElementById('greycons_'+i).placeholder*1;
			var user_given =document.getElementById('greycons_'+i).value*1;
	...
	function generate_fabric_report(type,is_mail_send,mail_id)
	{
		...
		else
		{
			...
			if(type=='show_fabric_booking_report_print23')
			{
				var show_yarn_rate='';
			...
			}else if(type=='show_fabric_booking_report_libas' && fabric_source==2){
				var show_yarn_rate='';
				var r=confirm("Press  \"Cancel\"  to hide  Fabric Rate\nPress  \"OK\"  to Show Fabric Rate");
				if (r==true) show_yarn_rate="1"; else show_yarn_rate="0";
				
			}
			else if(type=='show_fabric_booking_report13'){
				var show_yarn_rate='';
				var r=confirm("Press  \"Cancel\"  to hide  Fabric Rate & Amount\nPress  \"OK\"  to Show Fabric Rate & Amount");
				if (r==true) show_yarn_rate="1"; else show_yarn_rate="0";
			}
			else{
				var show_yarn_rate='';
			...
			var data="action="+type+get_submitted_data_string('txt_booking_no*cbo_company_name*txt_order_no_id*cbo_fabric_natu*cbo_fabric_source*id_approved_id*txt_job_no*txt_requision_no',"../../")+'&report_title='+$report_title+'&is_mail_send='+is_mail_send+'&mail_id='+mail_id+'&show_yarn_rate='+show_yarn_rate+'&path='+path;
				freeze_window(5);
				http.open("POST","requires/fabric_booking_urmi_controller.php",true);
				http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
				http.send(data);
	...
	function generate_fabric_report_reponse()
	{
		...
		else  if(file_data[2]==101)
        {
        $('#data_panel').html(file_data[0]);
        $('#print1').removeAttr('href').attr('href','requires/'+trim(file_data[1]));
            //$('#print_report4')[0].click();
	...
	function generate_fabric_excel_report(type,is_mail_send,mail_id)
	{
		if ( form_validation('txt_booking_no','Booking No')==false )
		{
			return;
	...
	function openmypage_req_popup()
	{
		...
		var data=txt_booking_no+"_"+cbo_company_name+"_"+cbo_buyer_name+"_"+txt_job_no+"_"+txt_booking_date;
	
		var title = 'Requisiton Search';
		var page_link = 'requires/fabric_booking_urmi_controller.php?data='+data+'&action=requisition_popup';
	
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1000px,height=250px,center=1,resize=1,scrolling=0','../');
		emailwindow.onclose=function()
		{
			//var req_number=this.contentDoc.getElementById("selected_requition");
			var theform=this.contentDoc.forms[0];
			var mst_tbl_id=this.contentDoc.getElementById("selected_requition").value;//mst id
			if (mst_tbl_id!="")
				{
					inquery_data_arr = mst_tbl_id.split("_");
					$('#txt_requision_no').val( inquery_data_arr[1] );
					$('#hid_requision_no').val( inquery_data_arr[0] );
				}
		}
	}
            ...
            <table width="1060" cellspacing="2" cellpadding="2" border="0">
                <tr>
                    ...
                    <td colspan="4">
                        ...
                        <input type="hidden" id="hidd_apply_last_update_btn">
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                	<td>Sample Requisition No/Booking No</td>
                	<td><input type="text" name="txt_requision_no" id="txt_requision_no"  class="text_boxes" style="width:130px;" placeholder="Click for Brows" onClick="openmypage_req_popup();">
					<input class="text_boxes" type="hidden" style="width:200px;" name="hid_requision_no" id="hid_requision_no"/>
					</td>
                    <td>Remarks</td>
                ...
                <tr>
                    ...
                    <td><input style="width:130px;" type="text" class="text_boxes_numeric"  name="txt_cad_eff_percent" id="txt_cad_eff_percent"/></td>
                ...
                <tr>
                    <td colspan="4"><? include("../../terms_condition/terms_condition.php"); terms_condition(118,'txt_booking_no','../../'); ?>
						...
						<input type="button" class="image_uploader" style="width:70px" value="ADD FILE" onClick="file_uploader ( '../../', document.getElementById('update_id').value,'', 'main_fabric_booking', 2 ,1)">
                        <input style="width:60px; display:none" type="hidden" class="text_boxes"  name="adjust_qty_breck_down" id="adjust_qty_breck_down" />
                    </td>
					<td>Excess Input %</td>
                    <td><input style="width:130px;" type="text" class="text_boxes_numeric"  name="txt_excess_input_per" id="txt_excess_input_per"/></td>
                ...
                <tr>
                    <td align="center" colspan="8" valign="middle" class="button_container">
						<? $date=date('d-m-Y'); echo load_submit_buttons( $permission, "fnc_fabric_booking", 0,0 ,"fnResetForm(); reset_form('fabricbooking_1','','booking_list_view', 'cbo_pay_mode,5*cbo_booking_year,2023*cbo_currency,2*cbo_ready_to_approved,2*txt_booking_percent,100*txt_booking_date, ".$date."','', 'cbo_company_name*cbo_booking_year*cbo_booking_month*cbo_fabric_natu*cbo_fabric_source*txt_booking_date*txt_delivery_date*cbo_supplier_name*cbo_pay_mode*cbo_source*cbo_currency')",1) ; ?>
