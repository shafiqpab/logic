...
<script>
	...
	function fnc_yarn_service_wo(operation) {
		if (form_validation('cbo_company_name*cbo_service_type*cbo_supplier_name*txt_booking_date*cbo_pay_mode*txt_delivery_date', 'Company Name *Service Type*Factory*Booking Date*Pay Mode*Delivery Date') == false) {
			...
			$("#dtls_container tr").each(function() {
				...
				var allocated_qty = $("#txt_allocated_qty_" + j).val();
				var fab_booking_no = $("#txt_fab_booking_no_" + j).val();

				if ((cbo_with_order == 1 && (txt_job_no == "" || fab_booking_no == "")) || txt_lot == "" || txt_wo_qty == "" || txt_rate == "") {
					validation = false;

					return;
				}

				dtlsDataString += '&txt_job_no_' + j + '=' + txt_job_no + '&txt_job_id_' + j + '=' + txt_job_id + '&txt_lot_' + j + '=' + txt_lot + '&txt_pro_id_' + j + '=' + txt_pro_id + '&txt_item_des_' + j + '=' + txt_item_des + '&cbo_uom_' + j + '=' + cbo_uom + '&txt_wo_qty_' + j + '=' + txt_wo_qty + '&cbo_count_' + j + '=' + cbo_count + '&txt_rate_' + j + '=' + txt_rate + '&txt_amount_' + j + '=' + txt_amount + '&txt_bag_' + j + '=' + txt_bag + '&txt_cone_' + j + '=' + txt_cone + '&txt_min_req_cone_' + j + '=' + txt_min_req_cone + '&txt_remarks_' + j + '=' + txt_remarks + '&dtls_update_id_' + j + '=' + dtls_update_id + '&txt_allocated_qty_' + j + '=' + allocated_qty + '&fab_booking_no_' + j + '=' + fab_booking_no;
				j++;
			});
			...
			if (validation == false) {
				...
				var mstDataString = "cbo_company_name*cbo_service_type*cbo_supplier_name*txt_booking_date*txt_attention*txt_tenor*cbo_currency*txt_exchange_rate*cbo_pay_mode*cbo_source*txt_delivery_date*cbo_is_sales_order*cbo_ready_to_approved*update_id*txt_booking_no*cbo_with_order*update_dtls_ids*original_pro_data*txt_ref_no" + twistingDataStr;
	...
	function fnc_yarn_service_wo_response() {
		if (http.readyState == 4) {
			...
			reset_form('', '', 'txt_job_no_0*txt_job_id_0*txt_fab_booking_no_0*cbo_count_0*txt_item_des_0*txt_lot_0*txt_pro_id_0*txt_wo_qty_0*txt_rate_0*txt_amount_0*txt_bag_0*txt_cone_0*txt_min_req_cone_0*txt_remarks_0*dtls_update_id_0*txt_allocated_qty_0*update_dtls_ids*original_pro_data' + $reset_fin_prod_dtls, '', '', 'update_id*txt_booking_no*cbo_is_sales_order*');
	...
	function fnc_calculate(thisVal, i) {
		...
		if (cbo_service_type == 15 || cbo_service_type == 50 || cbo_service_type == 51) {
			...
			$('#dtls_container tr').each(function(j) {
				...
				//alert(wo_qty+'<'+place_val);

				if ((operation_type != 1) && (place_val < wo_qty)) {
					$('#txt_wo_qty_' + j).val("");
					$('#txt_amount_' + j).val("");
					$('#txt_rate_' + j).val("");
					return;
	...
	function openmypage_job(thisVal, title) {
		if (form_validation('cbo_company_name', 'Company Name') == false) {
			...
			var txt_job_no = $("#txt_job_no_0").val();
			...
			if (title == 'Job Search') {
				...
				width = "930px";
				page_link = 'requires/yarn_service_work_order_controller.php?action=booking_search_popup&company=' + company + '&txt_booking_date=' + txt_booking_date + '&txt_job_no=' + txt_job_no + '&is_sales_order=' + is_sales_order;
			}
			...
			emailwindow.onclose = function() {
				...
				if (title == 'Job Search') {
					...
					if (is_sales_order == 1) {
						$('#txt_fab_booking_no_0').val(dataArr[2]).attr('disabled', 'disabled');
					} else {
						$('#txt_fab_booking_no_0').val('').removeAttr('disabled');
					}
				} else {
					var theform = this.contentDoc.forms[0];
					var data = this.contentDoc.getElementById("hdn_booking_no").value;
					var dataArr = data.split(',');

					document.getElementById('txt_fab_booking_no_0').value = dataArr[0];
					document.getElementById('txt_lot_0').value = '';
				}
				release_freezing();
			}
		}
	...
	function openmypage_lot() {
		if (form_validation('cbo_company_name', 'Company Number') == false) {
			...
			var fab_booking_no = $("#txt_fab_booking_no_0").val();
			var is_sales_order = $("#cbo_is_sales_order").val();
			var cbo_service_type = $("#cbo_service_type").val();
			var cbo_with_order = $("#cbo_with_order").val();
			...
			page_link = 'requires/yarn_service_work_order_controller.php?action=lot_search_popup&company=' + company + '&job_no=' + job_no + '&is_sales_order=' + is_sales_order + '&cbo_service_type=' + cbo_service_type + '&cbo_with_order=' + cbo_with_order + '&fab_booking_no=' + fab_booking_no;
			emailwindow = dhtmlmodal.open('EmailBox', 'iframe', page_link, 'Lot Number Search', 'width=1200px,height=380px,center=1,resize=1,scrolling=0', '../')
			emailwindow.onclose = function() {
				freeze_window(5);
				release_freezing();
				var theform = this.contentDoc.forms[0];
				...
				if (product_info.length > 1) {
					...
					for (var i = 1; i < product_info.length; i++) {
						...
						$('#dtls_container tr').each(function(i) {
							...
							if (i === 0) {
								...
								textinput.eq(2).attr('id', 'txt_fab_booking_no_' + i).val(fab_booking_no);
							} else {
								...
								textinput.eq(2).attr('id', 'txt_fab_booking_no_' + i).val(fab_booking_no).attr('disabled', true);
							}

							textinput.eq(3).attr('id', 'txt_lot_' + i).val(data[2]);
							textinput.eq(4).attr('id', 'txt_pro_id_' + i).val(data[3]);
							textinput.eq(5).attr('id', 'txt_item_des_' + i).val(data[0]).attr('disabled', true);
							textinput.eq(6).attr('id', 'txt_wo_qty_' + i).attr("placeholder", data[4]).attr("onKeyUp", "fnc_calculate(this," + i + ")").val('');
							textinput.eq(7).attr('id', 'txt_rate_' + i).val('').attr("onKeyUp", "fnc_calculate(this," + i + ")");
							textinput.eq(8).attr('id', 'txt_amount_' + i).val('');
							textinput.eq(9).attr('id', 'txt_bag_' + i).val('');
							textinput.eq(10).attr('id', 'txt_cone_' + i).val('');
							textinput.eq(11).attr('id', 'txt_min_req_cone_' + i).val('');
							textinput.eq(12).attr('id', 'txt_remarks_' + i).val('');
							//textinput.eq(13).attr('id', 'dtls_update_id_' + i).val('');
							textinput.eq(14).attr('id', 'txt_allocated_qty_' + i).val(data[4]);
							selectinput.eq(0).attr('id', 'cbo_count_' + i).val(data[1]).attr('disabled', true);
							selectinput.eq(1).attr('id', 'cbo_uom_' + i);
						});
					}
				} else {
					...
					textinput.eq(3).attr('id', 'txt_lot_0').val(data[2]);
					textinput.eq(4).attr('id', 'txt_pro_id_0').val(data[3]);
					textinput.eq(5).attr('id', 'txt_item_des_0').val(data[0]).attr('disabled', true);
					textinput.eq(6).attr('id', 'txt_wo_qty_0').attr("placeholder", data[4]).val('').attr("onKeyUp", "fnc_calculate(this,1)");;
					textinput.eq(7).attr('id', 'txt_rate_0').val('').attr("onKeyUp", "fnc_calculate(this,1)");
					textinput.eq(8).attr('id', 'txt_amount_0').val('');
					textinput.eq(9).attr('id', 'txt_bag_0').val('');
					textinput.eq(10).attr('id', 'txt_cone_0').val('');
					textinput.eq(11).attr('id', 'txt_min_req_cone_0').val('');
					textinput.eq(12).attr('id', 'txt_remarks_0').val('');
					//textinput.eq(13).attr('id', 'dtls_update_id_0').val('');
					textinput.eq(14).attr('id', 'txt_allocated_qty_0').val(data[4]);
					selectinput.eq(0).attr('id', 'cbo_count_0').val(data[1]).attr('disabled', true);
					selectinput.eq(1).attr('id', 'cbo_uom_0');
				}
				...
				var is_sales_order = $("#cbo_is_sales_order").val();
				if (is_sales_order == 1) {
					get_php_form_data(company + '*' + job_no + '*' + fab_booking_no + '*' + data[3], 'actn_available_qty', 'requires/yarn_service_work_order_controller');
				}
		...
		if (product_info.length > 1) {
			...
			for (var i = 1; i < product_info.length; i++) {
				...
				$('#dtls_container tr').each(function(i) {
					...
					textinput.eq(2).attr('id', 'txt_fab_booking_no_' + i).val('');

					textinput.eq(3).attr('id', 'txt_lot_' + i).val('');
					textinput.eq(4).attr('id', 'txt_pro_id_' + i).val('');

					textinput.eq(5).attr('id', 'txt_item_des_' + i).val('');
					textinput.eq(6).attr('id', 'txt_wo_qty_' + i).attr("onKeyUp", "fnc_calculate(this," + i + ")");
					textinput.eq(7).attr('id', 'txt_rate_' + i).val('').attr("onKeyUp", "fnc_calculate(this," + i + ")");
					textinput.eq(8).attr('id', 'txt_amount_' + i).val('');
					textinput.eq(9).attr('id', 'txt_bag_' + i).val('');
					textinput.eq(10).attr('id', 'txt_cone_' + i).val('');
					...
					textinput.eq(12).attr('id', 'txt_remarks_' + i).val('');
					//textinput.eq(13).attr('id', 'dtls_update_id_' + i).val('');
					textinput.eq(14).attr('id', 'txt_allocated_qty_' + i).val('');
			...
			textinput.eq(1).attr('id', 'txt_job_id_0').val('');
			textinput.eq(2).attr('id', 'txt_fab_booking_no_0').val('');
			textinput.eq(3).attr('id', 'txt_lot_0').val(data[2]);
			textinput.eq(4).attr('id', 'txt_pro_id_0').val(data[3]);
			textinput.eq(5).attr('id', 'txt_item_des_0').val(data[0]).attr('disabled', true);
			textinput.eq(6).attr('id', 'txt_wo_qty_0').attr("placeholder", data[4]);
			textinput.eq(6).attr('id', 'txt_wo_qty_0').attr("onKeyUp", "fnc_calculate(this,1)");
			textinput.eq(7).attr('id', 'txt_rate_0').val('').attr("onKeyUp", "fnc_calculate(this,1)");
			textinput.eq(8).attr('id', 'txt_amount_0').val('');
			textinput.eq(9).attr('id', 'txt_bag_0').val('');
			textinput.eq(10).attr('id', 'txt_cone_0').val('');
			...
			textinput.eq(12).attr('id', 'txt_remarks_0').val('');
			//textinput.eq(13).attr('id', 'dtls_update_id_0').val('');
			textinput.eq(14).attr('id', 'txt_allocated_qty_0').val('');
			selectinput.eq(0).attr('id', 'cbo_count_0').val(data[1]).attr('disabled', true);
			selectinput.eq(1).attr('id', 'cbo_uom_0');
		}
	}
	...
	function openmypage_booking() {
		...
		page_link = 'requires/yarn_service_work_order_controller.php?action=yern_service_wo_popup&company=' + company + '&is_sales=' + is_sales;
		emailwindow = dhtmlmodal.open('EmailBox', 'iframe', page_link, 'Yarn Dyeing Booking Search', 'width=1082px, height=450px, center=1, resize=0, scrolling=0', '../');
		emailwindow.onclose = function() {
			var theform = this.contentDoc.forms[0];
			...
			if (sys_number != "") {
				...
				get_php_form_data(sys_number[0] + "_" + sys_number[1], "populate_master_from_data", "requires/yarn_service_work_order_controller");
	...
	function sales_order_report_two() {
		var update_id = $('#update_id').val();
		var is_sales = $('#cbo_is_sales_order').val();
		if (update_id == "" || is_sales == 2) {
			alert("This Report Is For Sales Order Only");
		...
		var form_name = "yarn_dyeing_wo_booking_without_order";
		var data = "action=sales_order_report_two" + '&form_name=' + form_name + get_submitted_data_string('txt_booking_no*cbo_company_name*update_id*cbo_service_type', "../../") + "&show_val_column=" + show_rate_column;
		http.open("POST", "requires/yarn_service_work_order_controller.php", true);
		http.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		http.send(data);
		http.onreadystatechange = sales_order_report_two_reponse;
	}

	function sales_order_report_two_reponse() {
		if (http.readyState == 4) {
			freeze_window(5);
	...
	function change_job_title(val) {
		//txt_job_no_0
		//txt_job_id_0
		//txt_fab_booking_no_0
		//txt_lot_0
		//txt_pro_id_0
		$('#txt_job_no_0').val('');
		$('#txt_job_id_0').val('');
		$('#txt_fab_booking_no_0').val('');
		$('#txt_lot_0').val('');
		$('#txt_pro_id_0').val('');
		$('#cbo_count_0').val(0);
		$('#txt_item_des_0').val('');
		$('#cbo_uom_0').val(12);
		$('#txt_wo_qty_0').val('');
		$('#txt_wo_qty_0').removeAttr('placeholder').attr('placeholder', '');
		$('#txt_rate_0').val('');
		$('#txt_amount_0').val('');
		$('#txt_bag_0').val('');
		$('#txt_cone_0').val('');
		$('#txt_min_req_cone_0').val('');
		$('#txt_remarks_0').val('');

		if (val == 1) {
			$("#job_title").text("Sales Order No");
			//$("#cbo_with_order").attr("disabled","disabled");
			$("#txt_job_no_0").removeAttr("disabled").attr("placeholder", "Doubole Click for Job");
		} else {
			$("#job_title").text("Job No");
			//$("#cbo_with_order").removeAttr("disabled");
		}
	}

	function change_job_priority(val) {
		//alert(val);
		var val = $("#cbo_with_order").val();
		if (val == 2) {
	...
	function fnc_libdata(company_id) {
		var all_variable_settings = return_ajax_request_value(company_id, 'load_variable_settings', 'requires/yarn_service_work_order_controller');
		$('#hdn_color_form').val(all_variable_settings);
		if (all_variable_settings == 1) {
			$('#txt_fin_color').attr('readonly', true);
			$('#txt_fin_color').attr('placeholder', 'Browse');
			$('#txt_fin_color').removeAttr("onDblClick").attr("onDblClick", "color_select_popup();");
		} else {
	...
	function color_select_popup() {
		...
		//alert(texbox_id)
		//var buyer_name=$('#hdn_color_form').val();
			...
			<fieldset style="width:800px; margin-bottom:5px;">
				...
				<table cellspacing="4" cellpadding="8" border="0">
					...
					<tr>
						...
						<td>
							<?
							echo create_drop_down("cbo_company_name", 160, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name", "id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/yarn_service_work_order_controller',this.value, 'load_drop_down_supplier', 'supplier_td' );get_php_form_data('94'+'_'+this.value, 'populate_field_level_access_data', 'requires/yarn_service_work_order_controller' );change_job_title(document.getElementById('cbo_with_order').value); fnc_libdata(this.value); get_php_form_data(this.value,'button_setting', 'requires/yarn_service_work_order_controller');", 0);
							?>
						</td>
						<td align="right" class="must_entry_caption" width="80">Service Type</td>
						<td>
							<?
							echo create_drop_down("cbo_service_type", 160, $yarn_issue_purpose, "", 1, "-- Select --", $selected, "load_drop_down( 'requires/yarn_service_work_order_controller',$('#cbo_company_name').val()+'_'+this.value, 'load_drop_down_supplier', 'supplier_td' );set_fin_visibility(this.value);", 0, '12,15,38,44,46,7,50,51,63,81,82');
							?>
						</td>
						<td align="right" class="must_entry_caption">Pay Mode</td>
						<td><?
							echo create_drop_down("cbo_pay_mode", 160, $pay_mode, "", 1, "-- Select Pay Mode --", "", "load_drop_down( 'requires/yarn_service_work_order_controller',this.value+'_'+$('#cbo_company_name').val()+'_'+$('#cbo_service_type').val(), 'load_drop_down_supplier', 'supplier_td' )", "");
							?></td>
					...
					<tr>
						<td align="right" width="112">Ready To Approved</td>
						<td>
							<?
				...
				<table cellspacing="0" cellpadding="0" border="0" class="rpt_table">
					<thead id="dtls_container_head">
						<tr>
							...
							<th width="100" class="must_entry_caption">Sale Job/ Booking No</th>
					...
					<tbody id="dtls_container">
						<tr>
							<td>
								<input type="text" id="txt_job_no_0" name="txt_job_no_0" disabled="disabled" placeholder="Doubole Click for Job" readonly style="width:100px;" class="text_boxes job_field" onDblClick="openmypage_job(this,'Job Search')" />
								<input type="hidden" id="txt_job_id_0" name="txt_job_id_0" />
							</td>
							<td><input type="text" id="txt_fab_booking_no_0" name="txt_fab_booking_no_0" placeholder="Double Click for Booking" readonly style="width:100px;" class="text_boxes" onDblClick="openmypage_job(this, 'booking Search')" />
							</td>
							<td>
							...
							<td>
								<?
								echo create_drop_down("cbo_uom_0", 50, $unit_of_measurement, "", 1, "-- UOM--", 12, "", 1);
								?>
							</td>
							<td>
							...
							<td>
								...
								<input type="hidden" id="txt_allocated_qty_0">
							</td>
						</tr>
					</tbody>
				</table>
					...
					<table cellspacing="0" cellpadding="0" border="1" class="rpt_table" style="width:100%;">
						...
						<tr>
							...
							<td>
								...
								<input type="hidden" id="hdn_color_form" name="hdn_color_form" />
							</td>
						</tr>
					</table>
				</fieldset>
				<table width="100%">
					<tr>
						<td align="center" class="button_container">
							<div id="is_approved" style="float:left; font-size:24px; color:#FF0000;"></div>
							...
							<input type="hidden" id="original_pro_data">

							<div id="pdf_file_name"></div>
							<input type="button" value="Print With Rate" onClick="generate_trim_report()" style="width:160px; display:none;" name="print_booking" id="print_booking" class="formbutton" />
							<input type="button" value="Print Without Rate" onClick="generate_without_rate_report()" style="width:160px; display:none;" name="print_booking2" id="print_booking2" class="formbutton" />
							...
							<input type="button" value="Print Report Sales Two" onClick="sales_order_report_two()" style="width:170px; display:none;" name="print_booking6" id="print_booking6" class="formbutton" />
