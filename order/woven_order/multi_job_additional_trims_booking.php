...
Version (Oracle)         :
Converted by             :  Aziz
Purpose			         :  This form will create Multi Jodwise Additional Trims Booking
Functionality	         :
JS Functions	         :
Created by		         :  Aziz
...
echo load_html_head_contents("Knit Trims Booking", "../../", 1, 1,$unicode,'','');
...
?>
	...
	// <?
	// if(isset($_SESSION['logic_erp']['data_arr'][87]))
	// {
	// 	$data_arr= json_encode( $_SESSION['logic_erp']['data_arr'][87] );
	...
	function call_print_button_for_mail(mail_address,mail_body,type){
		/*var response=return_global_ajax_value(document.getElementById('cbo_company_name').value, 'send_mail_report_setting_first_select', '', 'requires/multi_job_additional_trims_booking_controller');
		...
		else if(report_id[0]==85)  generate_trim_report('show_trim_booking_report8',1,'1___'+mail_address);
	...
	else
	{
		...
		http.open("POST","requires/multi_job_additional_trims_booking_controller.php",true);
...
$cbo_company_name      = create_drop_down( "cbo_company_name", 120, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected, "get_php_form_data( this.value, 'populate_variable_setting_data', 'requires/multi_job_additional_trims_booking_controller' );","","" );
$cbo_buyer_name        = create_drop_down( "cbo_buyer_name", 120, $blank_array,"", 1, "-- Select Buyer --", $selected, "check_paymode(this.value);","" );
$cbo_supplier_name     = create_drop_down( "cbo_supplier_name", 120, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type in (4,5) and a.status_active =1 and a.is_deleted=0 order by supplier_name","id,supplier_name", 1, "-- Select Supplier --", $selected, "get_php_form_data( this.value, 'load_drop_down_attention', 'requires/multi_job_additional_trims_booking_controller');",0 );
$cbo_currency          = create_drop_down( "cbo_currency", 120, $currency,"", 1, "-- Select --", 2, "",0 );
$cbo_pay_mode          = create_drop_down( "cbo_pay_mode", 120, $pay_mode,"", 1, "-- Select Pay Mode --", "", "load_drop_down( 'requires/multi_job_additional_trims_booking_controller', this.value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('cbo_company_name').value, 'load_drop_down_supplier', 'supplier_td' )","" );
$cbo_source            = create_drop_down( "cbo_source", 120, $source,"", 1, "-- Select Source --", "", "","" );
$cbo_item_from            = create_drop_down( "cbo_item_from", 120, $item_from_arr,"", 1, "-- Select --", "", "","" );

$cbo_material_source   = create_drop_down( "cbo_material_source", 120, $fabric_source,"", 1, "-- Select Source --", "2", "","","","","","1,4" );
...
<div style="width:100%;" align="center">
    ...
    <form name="trimsbooking_1"  autocomplete="off" id="trimsbooking_1">
        <fieldset style="width:1180px;">
            ...
            <table  width="1180" cellspacing="2" cellpadding="0" border="0">
                <tr>
                    <td align="right" class="must_entry_caption" colspan="4"><b>Booking No</b></td>
                    <td colspan="5">
                        <input class="text_boxes" type="text" style="width:140px" onDblClick="openmypage_booking('requires/multi_job_additional_trims_booking_controller.php?action=trims_booking_popup','Trims Booking Search');" placeholder="Double Click for Booking" name="txt_booking_no" id="txt_booking_no" readonly/>
                ...
                <tr>
					...
					<td width="120"><?=$cbo_company_name; ?></td>
                    <td width="100" align="right" class="must_entry_caption">Buyer</td>
                    <td width="120" id="buyer_td"><?=$cbo_buyer_name; ?></td>
                    <td width="100" align="right" class="must_entry_caption">Booking Date</td>
                ...
                <tr>
                    ...
                    <td height="10"><textarea class="text_area" type="text" style="width:110px; height:20px;" name="txt_attention" id="txt_attention" placeholder="Attention" /></textarea></td>
                     <td align="right"  class="must_entry_caption"> Source </td>
                    <td><?=$cbo_source; ?> </td>
                </tr>
                <tr>
					<td align="right" class="">Trims Type</td>
                    <td><?=create_drop_down( "cbo_trim_type", 120, $trim_type,"", "1", "---- Select ----", 0, "" ); ?></td>
                    <td align="right" class="must_entry_caption">Level</td>
                    <td align="left"><?=$cbo_level; ?></td>
                     <td align="right">Delivery To</td>
                    <td height="10"><textarea id="delivery_address" name="delivery_address" class="text_area" type="text" style="width:110px; height:20px;" placeholder="Delivery Address" ></textarea></td>
                    <td align="right">Ready To App.</td>
                    <td align="left"><?=$cbo_ready_to_approved; ?></td>
                      <td align="right">Un-app.request</td>
                    <td align="left"><Input name="txt_un_appv_request" class="text_boxes" readonly placeholder="Double Click for Brows" ID="txt_un_appv_request" style="width:110px"  onClick="openmypage_unapprove_request();"></td>
                ...
                <tr>
                    ...
                    <td height="10"><textarea id="txt_remarks" name="txt_remarks" class="text_area" type="text" style="width:110px; height:20px;" placeholder="Remarks" ></textarea></td>
                    <td align="right">File</td>
                    <td> <input type="button" class="image_uploader" style="width:120px" value="CLICK TO ADD FILE" onClick="file_uploader ( '../../', document.getElementById('txt_booking_no').value,'', 'multiple_job_wise_additional_trim_booking', 2 ,1)"> </td>
					<td align="right">&nbsp;</td>
                    <td>
                      <?
                ...
                <tr>
					...
					<input type="button" value="Print Booking 1" onClick="generate_trim_report('show_trim_booking_report_2',1)"  style="width:100px;" name="print_booking_1" id="print_booking_1" class="formbutton" />
...
<script>
	...
	function fnc_process_data()
	{
		...
		//	var cbo_booking_year=document.getElementById('cbo_booking_year').value;
			...
			var page_link='requires/multi_job_additional_trims_booking_controller.php?action=fnc_process_data';
			var title='PO Search For Trim Booking';
			page_link=page_link+'&cbo_item_from='+cbo_item_from+'&company_id='+cbo_company_name+'&garments_nature='+garments_nature+'&cbo_currency='+cbo_currency+'&cbo_currency_job='+cbo_currency_job+'&cbo_supplier_name='+cbo_supplier_name+'&cbo_buyer_name='+cbo_buyer_name+'&txt_booking_no='+txt_booking_no+'&cbo_level='+cbo_level+'&txt_booking_date='+txt_booking_date+'&cbo_material_source='+cbo_material_source+'&cbo_trim_type='+cbo_trim_type+'&cbo_source='+cbo_source+'&cbo_pay_mode='+cbo_pay_mode;
			...
			emailwindow.onclose=function(){
				...
				if (theemail.value!=""){
					...
					//get_php_form_data(theemail3.value+"_save_"+cbo_company_name+"_"+cbo_pay_mode+"_"+cbo_level, "set_delivery_date_from_tna", "requires/multi_job_additional_trims_booking_controller" );
...
function fnc_generate_booking(param,po_id,pre_cost_id,cbo_company_name){
	...
	var data="action=generate_fabric_booking&data="+data+'&cbo_company_name='+cbo_company_name+'&txt_delivery_date='+txt_delivery_date+'&cbo_item_from='+cbo_item_from+'&cbo_item_from='+cbo_item_from+'&garments_nature='+garments_nature+'&cbo_currency='+cbo_currency+'&cbo_currency_job='+cbo_currency_job+'&cbo_level='+cbo_level+'&pre_cost_id='+precost_id+'&param='+param+'&txt_booking_no='+txt_booking_no+'&txt_booking_date='+txt_booking_date+'&cbo_pay_mode='+cbo_pay_mode+'&permission='+permission;
	http.open("POST","requires/multi_job_additional_trims_booking_controller.php",true);
	http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	http.send(data);
...
function copy_value(value,field_id,i){
	...
	if(txttrimgroup>0)
	{
		var uom_id=return_global_ajax_value(txttrimgroup, 'item_group_uom', '', 'requires/multi_job_additional_trims_booking_controller');
		//alert(uom_id);
		//document.getElementById('txtuom_'+i).value=uom_id;
		$('#txtuom_'+i).val(trim(uom_id));
	}
...
//	var cbo_booking_month=document.getElementById('cbo_booking_month').value;
	...
	emailwindow.onclose=function(){
		...
		if (theemail.value!=""){
			...
			$("#cbo_currency").attr("disabled",true);
			get_php_form_data( theemail.value, "populate_data_from_search_popup_booking", "requires/multi_job_additional_trims_booking_controller" );
...
function fnc_trims_booking( operation ){
	...
	if('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][555]); ?>') 
		{
			if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][555]); ?>','<? echo implode('*',$_SESSION['logic_erp']['mandatory_message'][555]); ?>')==false) {
				return;
			}
		}
	freeze_window(operation);
	var data_all="";
	if (form_validation('cbo_company_name*cbo_buyer_name*txt_booking_date*txt_delivery_date*cbo_pay_mode*cbo_supplier_name*cbo_material_source*cbo_source*cbo_item_from','Company Name*Buyer Name*Booking Date*Delivery Date*Pay Mode*Supplier*Source*Material Source*Item From')==false){
		release_freezing();
		return;
	}
	...
	data_all=data_all+get_submitted_data_string('txt_booking_no*cbo_company_name*cbo_buyer_name*cbo_supplier_name*cbo_currency*txt_booking_date*txt_delivery_date*cbo_pay_mode*cbo_source*txt_attention*txt_remarks*cbo_level*cbo_ready_to_approved*cbo_material_source*delivery_address*cbo_trim_type*txt_tenor*cbo_item_from*update_id',"../../")+"&delete_type="+delete_type;
...
function fnc_trims_booking_dtls( operation ){
	...
	for (var i=1; i<=row_num; i++){
		...
		var txttrimgroup=$('#txttrimgroup_'+i).val();
		var lib_tna_intregrate=$('#lib_tna_intregrate').val();
		
		//alert(lib_tna_intregrate);
		if(txttrimgroup==0)
		{
			alert('Trim Group is empty');
			//$('#txtddate_'+i).focus();
			release_freezing();
					return;
		}
		...
		if(cbo_item_from!=2) //No need for Item From -Library
		{
		...
		data_all=data_all+get_submitted_data_string('txtbookingid_'+i+'*txtpoid_'+i+'*txtcountry_'+i+'*txttrimcostid_'+i+'*txtReqAmt_'+i+'*txttrimgroup_'+i+'*txtuom_'+i+'*cbocolorsizesensitive_'+i+'*txtwoq_'+i+'*txtrate_'+i+'*txtamount_'+i+'*txtddate_'+i+'*consbreckdown_'+i+'*txtexchrate_'+i+'*txtjob_'+i+'*txtreqqnty_'+i+'*jsondata_'+i+'*txtdesc_'+i+'*txtbrandsup_'+i+'*txtreqamount_'+i+'*txtreqamountjoblevelconsuom_'+i+'*txtreqamountitemlevelconsuom_'+i+'*txthscode_'+i+'*txtremark_'+i+'*hiddlabeldtlsdata_'+i,"../../",i);
	}
	var cbo_level=document.getElementById('cbo_level').value;
	var cbo_company_name=document.getElementById('cbo_company_name').value;
	var cbo_item_from=document.getElementById('cbo_item_from').value;
	if(cbo_level==1){
		var data="action=save_update_delete_dtls&operation="+operation+'&total_row='+row_num+data_all+'&delete_cause='+delete_cause+'&cbo_company_name='+cbo_company_name+'&cbo_item_from='+cbo_item_from;
	}
	if(cbo_level==2){
		var data="action=save_update_delete_dtls_job_level&operation="+operation+'&total_row='+row_num+data_all+'&delete_cause='+delete_cause+'&cbo_company_name='+cbo_company_name+'&cbo_item_from='+cbo_item_from;
	}
	// alert(data);
	http.open("POST","requires/multi_job_additional_trims_booking_controller.php",true);
	http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
...
//	var cbo_booking_month=document.getElementById('cbo_booking_month').value;
	...
	var data="action=show_trim_booking_list"+get_submitted_data_string('txt_booking_no',"../../")+'&cbo_company_name='+cbo_company_name+'&garments_nature='+garments_nature+'&data='+data+'&param='+param+'&pre_cost_id='+precost_id+'&cbo_level='+cbo_level+'&cbo_currency='+cbo_currency+'&cbo_item_from='+cbo_item_from;
	http.open("POST","requires/multi_job_additional_trims_booking_controller.php",true);
	http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	http.send(data);
...
function generate_trim_report(action,report_type,mail_send_data){
	...
	else
	{
		...
		var report_title=$( "div.form_caption" ).html();
        // freeze_window();
		var data="action="+action+get_submitted_data_string('txt_booking_no*cbo_company_name*id_approved_id*cbo_item_from',"../../")+'&report_title='+report_title+'&show_comment='+show_comment+'&report_type='+report_type+'&mail_send_data='+mail_send_data;
		http.open("POST","requires/multi_job_additional_trims_booking_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
		http.onreadystatechange = generate_trim_report_reponse;
...
function generate_fabric_excel_report(action,report_type)
{
	...
	else
	{
		...
		if(action=='show_trim_booking_report20')
		{
			...
			$.ajax({
				url: 'requires/multi_job_additional_trims_booking_controller.php',
				type: 'POST',
				data: data,
...
function openmypage_unapprove_request()
{
	...
	var page_link = 'requires/multi_job_additional_trims_booking_controller.php?data='+data+'&action=unapp_request_popup';
...
function openlabeldtls_popup(trimitem,i)
{
	...
	var page_link = 'requires/multi_job_additional_trims_booking_controller.php?data='+trimitem+'&action=labeldtls_popup';
...
function deletedata(booking_id){
		...
		var txt_booking_no=document.getElementById('txt_booking_no').value;
		var check_is_booking_used_id=return_global_ajax_value(txt_booking_no, 'check_is_booking_used', '', 'requires/multi_job_additional_trims_booking_controller');
