...
$txt_department_hidden_id = $_REQUEST['txt_department_hidden_id'];
$txt_department_name = $_REQUEST['txt_department_name'];
...
// 	echo create_drop_down("cbo_department", 190, "select id,department_name from lib_department where STATUS_ACTIVE=1 and DIVISION_ID in(select id from LIB_DIVISION where STATUS_ACTIVE=1 and COMPANY_ID=$data) ", "id,department_name", 1, "-- Select Department --", $selected, 0, 1);
// 	exit();
// }
...
// {
//     list($data,$width)=explode('***',$data); 
// 	echo create_drop_down("cbo_profit_center", 190, "select id,profit_center_name from  lib_profit_center where company_id=$data", "id,profit_center_name", 1, "-- Select Profit Center --", $selected, 0, 1);
// 	exit();
// }
...
{    
	...
	<table cellpadding="0" cellspacing="0" border="0" class="rpt_table">
		...
		<tr>
			...
			<td> <input type="text" style="width:80px" class="text_boxes" name="txt_erosion_no" id="txt_erosion_no" /></td>
			<td> <input type="text" style="width:80px" class="text_boxes" name="txt_job_no" id="txt_job_no" /></td>
			<td> <input type="text" style="width:80px" class="text_boxes" name="txt_po_no" id="txt_po_no" /></td>
			...
			<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view (document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('txt_erosion_no').value+'**'+document.getElementById('txt_job_no').value+'**'+document.getElementById('txt_po_no').value+'**'+document.getElementById('txt_date_from').value+'**'+document.getElementById('txt_date_to').value+'**'+document.getElementById('cbo_shipment_type').value+'**'+document.getElementById('cbo_year').value, 'sys_id_popup_list_view', 'sys_id_popup_list_container', 'erosion_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1)');" style="width:80px;" />
			</td>
		</tr>
		<tr>
...
if ($action == "profit_center_popup")
{    
	echo load_html_head_contents("Popup Profit Center", "../../../", 1, 1, $unicode);
	extract($_REQUEST);
	$company_id    = str_replace("'", "", $company_id);
	$txt_erotion_value = str_replace("'", "", $txt_erotion_value);
	$txt_profit_center_hidden = str_replace("'", "", $txt_profit_center_hidden);
	$txt_profit_center_id_arr = explode(',', $txt_profit_center_hidden);

 
	$profit_center_sql = "SELECT ID, PROFIT_CENTER_NAME FROM  LIB_PROFIT_CENTER WHERE COMPANY_ID=$company_id";
	$profit_center_arr = sql_select($profit_center_sql);
 
  
	$percentage_distribution_arr = array();
	foreach($txt_profit_center_id_arr as $key=>$btn_user_str){
		list($btn_id, $percentage, $distribution_value) = explode('_',$btn_user_str) ;
		$percentage_distribution_arr[$btn_id]['percentage'] = $percentage;
		$percentage_distribution_arr[$btn_id]['distribution_value'] = $distribution_value;
	}
 
	?>	
	<script>
		function js_set_value(btn_id, btn_name, values)
		{
			// alert(values);
			...
			js_get_value(btn_id, btn_name, values);
		}

		
		function percentages(id,value)
		{
			var sum = 0;
			$('.price').each(function() {
				sum += Number($(this).val());
			...
			if( sum > 100){ 
				alert("Percentage Already 100% Up.");
				$("#close_id").css({"display":"none"});
				$("#percentage_"+id).val(0);
				$("#distribution_value_"+id).css({"background-color":"red"}); return;
			}
			else{
				var erotion_value = $("#erotion_value").val();
				var distribution_value = erotion_value*value/100;
				$("#distribution_value_"+id).val(distribution_value);
				$("#distribution_value_"+id).css({"background-color":""});
			}
		}

		function set_profit_center_value(){
			var selected_profit_center_name_arr = Array();
			var selected_data_arr = Array();
			var sum = 0;
			$('#erotion_form .price').each(function() {
				var [_,btn_id] = $(this).attr('id').split('_');
				if($("#percentage_"+btn_id).val() && $("#percentage_"+btn_id).val()!=0){
					selected_profit_center_name_arr.push($("#profit_center_name_"+btn_id).text());
					selected_data_arr.push(btn_id+'_'+$("#percentage_"+btn_id).val()+'_'+$("#distribution_value_"+btn_id).val());
					sum += Number($(this).val());
				}
			});
 
			if( sum < 100){ 
				alert("Please percentage 100% count");
				return;
			}
			else{
			 
				$("#txt_profit_center_name").val(selected_profit_center_name_arr.join(','));
				$("#txt_profit_center_id").val(selected_data_arr.join(','));
	...
	<form id="erotion_form">
		<input type="hidden" id="sys_id"/>
		<input type="hidden" id="erotion_value" value="<?= $txt_erotion_value;?>" />
		<table cellpadding="0" style="width:379px" cellspacing="0" border="1" rules="all" class="rpt_table">
			<thead>
				<th width="20">Sl</th>
				<th style="width:120px;">Profit Center</th>
				<th>Percentage(%)</th>
			...
			<tbody>
				...
				foreach($profit_center_arr as $key=>$row)
				{
					$bgcolor = ($i%2==0) ? "#E9F3FF" : "#FFFFFF";
				?>
                <tr bgcolor="<?= $bgcolor;?>" id="color_selected_<?= $row['ID'];?>">
					<td width="20"><?= ++$key;?></td>
					<td style="width:120px;" id="profit_center_name_<?= $row['ID'];?>"><?= $row['PROFIT_CENTER_NAME'];?></td>
					<td style="text-align: center;">
						<input type="text" class="text_boxes price" name="percentage" id="percentage_<?= $row['ID'];?>" value="<?= $percentage_distribution_arr[$row['ID']]['percentage'];?>" onkeyup="percentages('<?= $row['ID'];?>', this.value);" style="width:40px;">
					</td>
					<td style="text-align: center;">
						<input type="text" id="distribution_value_<?= $row['ID'];?>" class="text_boxes" name="distribution_value" readonly value="<?= $percentage_distribution_arr[$row['ID']]['distribution_value'];?>" style="width:40px;">
					</td>
				</tr>
				<?php
				// <?= $row['ID'].'_'.$percentage.'_'.$dstribution;? >
				}
				?>
			</tbody>
		</table>
				...
				<td align="center" height="30" valign="bottom">
					<div style="width:100%">
						<div style="width:50%;" align="center">
						    <input type="hidden" id="txt_profit_center_id" value="">
							<input type="hidden" id="txt_profit_center_name" value="">
							<input type="button" name="close" onclick="set_profit_center_value();" class="formbutton" value="Close" style="width:100px" />
						</div>
					</div>
				</td>
			</tr>
	...
	// alert(2);
		var percentage_arr = '<?= implode(',', $percentage_arr);?>';
		var dstribution_arr = '<?= implode(',', $dstribution_arr);?>';
		var btn_data_arr = '<?= implode(',', $btn_data_arr);?>';
		percentage_arr = percentage_arr.split(',');
		dstribution_arr = dstribution_arr.split(','); 
		btn_data_arr = btn_data_arr.split(',');

		txt_profit_center_ar = txt_profit_center.split(',');

		var i= 0;
		btn_data_arr.forEach((btn_id) => {

			btn_id = btn_id*1;
			var percentage = percentage_arr[i];
			var profit_center = txt_profit_center_ar[i];

			var value      = dstribution_arr[i];
			//alert(percentage);
			//alert(value);
			id = btn_id;
            
			//// id profit_center,input,distribution_value
			//js_get_value(id, btn_name, value, distribution_value);

			if(btn_id){js_get_value(id, btn_id, percentage, value);}
			i++;
		});
	</script>
	<?
...
{
	echo load_html_head_contents("Popup Department","../../../", 1, 1, $unicode); 
	$txt_report_button_wise_user_id_arr = explode(',', $txt_department_hidden_id);
	$txt_department_name_arr = explode(',', $txt_department_name);
	...
	<body>
		...
		<form action="">
			...
			<?php
			$department_sql = "SELECT ID, DEPARTMENT_NAME FROM LIB_DEPARTMENT";
			$department_res = sql_select($department_sql);
			?> 
			<div style="width:422px; overflow-y:scroll;height: 301px;" >
				<table width="420" cellspacing="0" class="rpt_table" border="0" rules="all" align="left" id="item_table" style="height:100px; overflow-y:scroll;">
					<tbody>
						<?
						$people = array("50", "52");
						$i=1;
						$btn_name_arr = array();
						foreach($department_res as $row){
							if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
							if(in_array($row["ID"], $btn_data_arr))
							{
								$bgcolor = "yellow";
							}
							?>
							<tr bgcolor="<?= $bgcolor;?>" id="color_selected_<?= $row['ID'];?>">
								<td onClick="js_set_value(<?= $row['ID'];?>,'<?= $row['DEPARTMENT_NAME'];?>')" width="25"><?= $i; ?></td>
								<td onClick="js_set_value(<?= $row['ID'];?>,'<?= $row['DEPARTMENT_NAME'];?>')" width="50"><?= $row['DEPARTMENT_NAME']; ?>
								<input type="hidden" id="selected_button_user_id_<?= $row['ID']; ?>" value="" />
							    </td>
							</tr>
							<?
							$i++;
		...
		<table width="420" cellspacing="0" cellpadding="0" style="border:none" align="center">
			<tr>
				<td align="center" height="30" valign="bottom">
					<div style="width:100%">
						<div style="width:50%;" align="center">
							<input type="hidden" id="txt_department_id" value="">
							<input type="hidden" id="txt_department_name" value="<?= implode(',', $txt_report_button_wise_user_id_arr); ?>">
	...
	<script>
		...
		var btn_name_arr = '<?= implode(',', $txt_department_name_arr);?>';
		var btn_data_arr = '<?= implode(',', $btn_data_arr);?>';
		...
		btn_data_arr.forEach((btn_id) => {
			...
			if(btn_id){js_get_value(btn_id, btn_name);}
			i++;
		});
	</script>
	</html>
...
if($action =='sys_id_popup_list_view'){
		...
		list($cbo_company_id,$cbo_buyer_name,$txt_erosion_no,$txt_job_no,$txt_po_no,$txt_date_from,$txt_date_to,$cbo_shipment_type,$cbo_year)=explode('**',$data);
		...
		if($cbo_shipment_type!=0){$where_con .=" and c.EROSION_TYPE='$cbo_shipment_type'";}
		...
		if($cbo_year!=0){$where_con .=" and to_char(c.insert_date,'YYYY')=$cbo_year";}
...
{
		$sql="SELECT a.FINAL_COMMENTS,a.PROFIT_CENTER,a.DEPARTMENT,a.APPROVED,a.ID,a.EXPECTED_SHIP_DATE,a.EROSION_DATE,a.COMPANY_ID,a.SHIP_APP_REQ_PREFIX,a.SHIP_APP_REQ_NUM,a.SHIP_APP_REQ_NO,a.PO_BREAK_DOWN_ID,a.EROSION_TYPE,a.SHIP_APP_REQ_DATE,a.TO_BE_SHIPPED_QTY,a.EROSION_VALUE,a.READY_TO_APPROVE,a.BUYER_ID,a.THE_PROBLEMS,a.ROOT_CAUSES,a.CORRECTIVE_ACTION_PLANS,a.PRECAUTIONERY_FUTURE_PLANS,b.PO_NUMBER FROM wo_po_break_down b,erosion_entry a  where b.id=a.PO_BREAK_DOWN_ID and  a.is_deleted=0 and a.status_active=1 and a.id=$data";
 
		// 5_10_7.8, 8_20_15.6, 7_10_7.8, 11_10_7.8, 2_10_7.8, 14_30_23.4
		// Garments, bashar, Marketing, l, r4ty, testtttttttttt 
 
 		$sql_dlts = "SELECT a.ID, a.MST_ID, a.PROFIT_CENTER_ID, a.PERCENTAGE,a.DISTRIBUTION_VALUE FROM erosion_entry_profits a WHERE a.MST_ID= $data and a.IS_DELETED=0 and a.STATUS_ACTIVE=1";
		$sql_dlts_res = sql_select($sql_dlts);
		if(count($sql_dlts_res)>0){
			$id_value = '';
			$name = '';
			foreach ($sql_dlts_res as $row)
			{
				$profit_center_id = $row['PROFIT_CENTER_ID'];
				$percentage = $row['PERCENTAGE'];
				$distribution_value = $row['DISTRIBUTION_VALUE'];
				
				$id_value .= $profit_center_id.'_'.$percentage.'_'.$distribution_value.',';

				$profit_center_sql = "SELECT ID, PROFIT_CENTER_NAME FROM  LIB_PROFIT_CENTER WHERE ID=$profit_center_id";
				$profit_center = sql_select($profit_center_sql);
				$coma = '';
				$name .= $profit_center[0]['PROFIT_CENTER_NAME'].',';
			}
			$id_value = rtrim($id_value, ', ');
			$profit_name = rtrim($name, ', ');
			echo "document.getElementById('txt_profit_center_hidden_id').value = '".$id_value."';\n";
			echo "document.getElementById('cbo_profit_center').value = '".$profit_name."';\n";
		} 

		$data_array=sql_select($sql);
		$department_names = '';
		$department_id = $data_array[0]['DEPARTMENT'];

		if($department_id !=''){
			$department_sql = "SELECT ID, DEPARTMENT_NAME FROM LIB_DEPARTMENT where ID IN ($department_id)";
			$department_res = sql_select($department_sql);
			$name = '';
			foreach ($department_res as $rows)
			{
				$name .= $rows['DEPARTMENT_NAME'].',';
			}
			$name = rtrim($name, ', ');
			echo "document.getElementById('cbo_department').value = '".$name."';\n";
		}
		

		foreach ($data_array as $row)
		{ 
			$root_cause = preg_replace("/[\n\r]/","", $row['ROOT_CAUSES']);
			...
			echo "$('#cbo_company_id').val('".$row['COMPANY_ID']."').change()".";\n";
			...
			echo "document.getElementById('txt_root_cause').value = '".$root_cause."';\n";
			...
			echo "document.getElementById('txt_final_comments').value = '".$row['FINAL_COMMENTS']."';\n";


			//echo "$('#txt_final_comments').val('".$row['FINAL_COMMENTS']."').attr('readonly',true)".";\n";

			//echo "document.getElementById('cbo_profit_center').value = '".$row['PROFIT_CENTER']."';\n";
			//echo "document.getElementById('txt_department_hidden_id').value = '".$row['DEPARTMENT']."';\n";
			echo "document.getElementById('txt_department_hidden_id').value = '".$row['DEPARTMENT']."';\n";
			
			//echo "$('#cbo_profit_center').val('".$row['PROFIT_CENTER']."').prop('disabled',true)".";\n";
			//echo "$('#cbo_department').val('".$row['DEPARTMENT']."').prop('disabled',true)".";\n";
			/*  echo "$('#cbo_profit_center').prop('disabled',true)".";\n";
			echo "$('#cbo_department').prop('disabled',true)".";\n";  */
			...
			if($row['APPROVED']==1){
				$app_sta ="Approved";
				//echo "$('#txt_final_comments').attr('readonly', false)".";\n";
				/* echo "$('#cbo_profit_center').prop('disabled',false)".";\n";
				echo "$('#cbo_department').prop('disabled',false)".";\n"; */
				echo "$('#txt_update_type').val(1)".";\n";
				 
				echo "$('#cbo_company_id').prop('disabled',true)".";\n";
				echo "$('#cbo_erosion_type').prop('disabled',true)".";\n";
				echo "$('#txt_shiped_qty').prop('disabled',true)".";\n";
				echo "$('#txt_problem').prop('disabled',true)".";\n";
				echo "$('#txt_precautionary_plan').prop('disabled',true)".";\n";
				 

				echo "$('#txt_erosion_date').prop('disabled',true)".";\n";
				echo "$('#txt_po_no').prop('disabled',true)".";\n";
				echo "$('#cbo_buyer_name').prop('disabled',true)".";\n";
				echo "$('#txt_erotion_value').prop('disabled',true)".";\n";
				echo "$('#txt_expected_date').prop('disabled',true)".";\n";
				echo "$('#cbo_approved').prop('disabled',true)".";\n";
				echo "$('#txt_root_cause').prop('disabled',true)".";\n";
				echo "$('#txt_corrective_action_plan').prop('disabled',true)".";\n";
				
			}
			else if($row['APPROVED']==2){$app_sta ="Denied";}
			else if($row['APPROVED']==3){
				$app_sta ="Partial Approve";
				echo "$('#txt_update_type').val(1)".";\n";
			}
...
{     
	...
	$txt_final_comments = str_replace("'", "", $txt_final_comments);
	$cbo_profit_center = str_replace("'", "", $cbo_profit_center);
	$cbo_department = str_replace("'", "", $cbo_department);
	$update_type = str_replace("'", "", $txt_update_type);

	$department_id = str_replace("'", "", $txt_department_hidden_id);
	$txt_profit_center_hidden_id = str_replace("'", "", $txt_profit_center_hidden_id);

	//echo $txt_profit_center_hidden_id;die;
	//print_r($cbo_department);exit;
	
	// txt_department_hidden_id
	// txt_profit_center_hidden_id

	//print_r($txt_profit_center_hidden_id);die;
	...
	if ($operation == 0)
	{
		...
		if (str_replace("'", "", $update_id) == "")
		{
			...
			{
				echo "11**0";
				disconnect($con); // DEPARTMENT
				die;
			}
			
			$field_array_mst="ID,COMPANY_ID,SHIP_APP_REQ_PREFIX,SHIP_APP_REQ_NUM,SHIP_APP_REQ_NO,PO_BREAK_DOWN_ID,EROSION_TYPE,BUYER_ID,EROSION_DATE,EROSION_VALUE,TO_BE_SHIPPED_QTY,EXPECTED_SHIP_DATE,READY_TO_APPROVE,THE_PROBLEMS,ROOT_CAUSES,CORRECTIVE_ACTION_PLANS,PRECAUTIONERY_FUTURE_PLANS,INSERTED_BY,INSERT_DATE,UPDATED_BY,UPDATE_DATE,STATUS_ACTIVE,IS_DELETED,DEPARTMENT,FINAL_COMMENTS";
			
			$data_array_mst = "(".$id.",".$cbo_company_id.",'".$new_system_id[1]."','".$new_system_id[2]."','".$new_system_id[0]."','".$hidden_po_id."','".$cbo_erosion_type."','".$cbo_buyer_name."','".$txt_erosion_date."','".$txt_erotion_value."','".$txt_shiped_qty."','".$txt_expected_date."','".$cbo_approved."','".$txt_problem."','".$txt_root_cause."','" . $txt_corrective_action_plan."','".$txt_precautionary_plan."',".$_SESSION['logic_erp']['user_id'].",'" .$pc_date_time. "',0,'',1,0,'".$department_id."','".$txt_final_comments."')";
			
			 $delts_flag = 0;
			 if($txt_profit_center_hidden_id){
			 	$dtls_id = return_next_id( "id", "erosion_entry_profits", 1 );
			 	$txt_profit_center = explode(",", $txt_profit_center_hidden_id);
			// 	// print_r($txt_profit_center_hidden_id);die;
			 	$field_array_dtls = "ID, MST_ID, PROFIT_CENTER_ID, PERCENTAGE, DISTRIBUTION_VALUE, INSERTED_BY, INSERT_DATE, STATUS_ACTIVE, IS_DELETED";
				foreach($txt_profit_center as $row)
			 	{
			 		list($profit_center_id, $percentage, $distribution_value) = explode("_", $row);
			 		$percentage = $percentage*1;
					$distribution_value = $distribution_value*1;
				
			 		if($data_array_dtls){$add_comma = ", ";}
			 		$data_array_dtls .= "$add_comma (".$dtls_id.", ".$id.", ".$profit_center_id.", ".$percentage." , ".$distribution_value.", ".$_SESSION['logic_erp']['user_id'].", '".$pc_date_time."', 1, 0)";
			 		$dtls_id++;
				}
			 	$rID2 = sql_insert("erosion_entry_profits", $field_array_dtls, $data_array_dtls, 0);
				$delts_flag = 1;
			 }

			// echo $delts_flag;die;

			// '5_10_5.4, 8_20_10.8,7_40_21.6,11_30_16.2,2__0'
			$flag=1;
		    $rID = sql_insert("erosion_entry",$field_array_mst,$data_array_mst,0); 
			// echo "insert into erosion_entry_profits $field_array_dtls values $data_array_dtls";die;
			 
		    if($rID==1) $flag=1; else $flag=0;
		 
		} 
		//echo "10**insert into erosion_entry_profits ($field_array_dtls) values" . $data_array_dtls;die;
	...
	{  //alert(1);
		...
		//print_r($update_type);exit;
		
		if($update_type == 1){
			$field_array_update="FINAL_COMMENTS*DEPARTMENT*UPDATED_BY*UPDATE_DATE";	
		
			$data_array_update = "'".$txt_final_comments."'*'".$department_id."'*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'";
		}
		else {
			$field_array_update="COMPANY_ID*PO_BREAK_DOWN_ID*EROSION_TYPE*BUYER_ID*EROSION_DATE*EROSION_VALUE*TO_BE_SHIPPED_QTY*EXPECTED_SHIP_DATE*READY_TO_APPROVE*THE_PROBLEMS*ROOT_CAUSES*CORRECTIVE_ACTION_PLANS*PRECAUTIONERY_FUTURE_PLANS*UPDATED_BY*UPDATE_DATE*DEPARTMENT*FINAL_COMMENTS";
			
			$data_array_update = "'".$cbo_company_id."'*'".$hidden_po_id."'*'".$cbo_erosion_type."'*'".$cbo_buyer_name."'*'".$txt_erosion_date."'*'".$txt_erotion_value."'*'".$txt_shiped_qty."'*'".$txt_expected_date."'*'".$cbo_approved."'*'".$txt_problem."'*'".$txt_root_cause."'*'".$txt_corrective_action_plan."'*'".$txt_precautionary_plan."'*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'"."*'".$department_id."'*'".$txt_final_comments."'";

			// ,'".$department_id."')";
		}

		//echo "10**insert into erosion_entry_profits ($field_array_update) values" . $data_array_update;die;

		//echo $txt_update_type;die;
		$profit_approved_flag = 1;
		$delts_flag = 0;
		if($update_type==1){
			$txt_profit_center = explode(",", $txt_profit_center_hidden_id);
			$dtls_id = return_next_id( "id", "erosion_entry_profits", 1 );
			...
			foreach($txt_profit_center as $row)
			{
				...
				if($data_array_dtls){$add_comma = ", ";}
				$data_array_dtls .= "$add_comma (".$dtls_id.", ".$update_id.", ".$profit_center_id.", ".$percentage." , ".$distribution_value.", ".$_SESSION['logic_erp']['user_id'].", '".$pc_date_time."', 1, 0)";
				$dtls_id++;
			}

			$rID_delete = sql_select("delete from erosion_entry_profits where mst_id=$update_id");
		    $rID2 = sql_insert("erosion_entry_profits", $field_array_dtls, $data_array_dtls, 0);
			if($rID2==1) $delts_flag=1; else $delts_flag=0;
		}
		 
		if($update_type==0 && $txt_profit_center_hidden_id !=''){
			$profit_approved_flag = 0;
		}
		...
		if($delts_flag==1){
			...
			{
				oci_commit($con);
				echo "1**" . $update_id . "**" . $txt_erosion_no."**".$profit_approved_flag;
			}
			else
			{
				oci_rollback($con);
  ...
  from wo_pre_cost_mst_histry a,wo_pre_cost_dtls_histry b where a.job_no=b.job_no and a.job_no=$txt_job_no  order by  a.id,b.id  asc");
	...
	//$ttl_commercial_cost=$commercial_costing_arr[$job_no];
		...
		$knit_fab .='<tr class="rpt_bottom" style="font-weight:bold">
					...
					$precostData['KnitTotal']=$knit_subtotal_avg_cons;
  		echo $knit_fab;
				...
				if (!in_array($process_id,$process_array_check) )
				{
					if($k!=1)
					{
					   ...
					   <tr>
							...
							<? 
								echo fn_number_format($total_convsion_qty,2); 
								$precostData['knitting_day'][$process_id]=$total_convsion_qty;
							?>
							</strong></td>
...
if($action=="generate_print")
{
   ...
   $sql="SELECT c.ID, c.FINAL_COMMENTS,c.PROFIT_CENTER,c.DEPARTMENT,   c.APPROVED,c.COMPANY_ID,c.SHIP_APP_REQ_PREFIX,c.SHIP_APP_REQ_NUM,c.SHIP_APP_REQ_NO,c.PO_BREAK_DOWN_ID,c.EROSION_TYPE,c.SHIP_APP_REQ_DATE,c.EROSION_DATE,
   ...
   //echo $sql;die;
	...
	$erosion_profit_sql = "SELECT a.ID, a.MST_ID, a.PROFIT_CENTER_ID, a.PERCENTAGE, a.DISTRIBUTION_VALUE FROM erosion_entry_profits a WHERE a.MST_ID= $id";
	$erosion_profit_res = sql_select($erosion_profit_sql);
	 
  
	//var_dump($data_array);die;
	$lib_profit_center=return_library_array("select ID, PROFIT_CENTER_NAME from LIB_PROFIT_CENTER", "ID", "PROFIT_CENTER_NAME");
	...
	$tna_sql="select b.TASK_SEQUENCE_NO,a.TASK_NUMBER,a.PO_NUMBER_ID,a.TASK_START_DATE,a.TASK_FINISH_DATE,a.ACTUAL_START_DATE,a.ACTUAL_FINISH_DATE,b.TASK_SHORT_NAME from TNA_PROCESS_MST a,LIB_TNA_TASK b where b.TASK_NAME=a.TASK_NUMBER and a.TASK_TYPE=1 and b.TASK_TYPE=1 and a.PO_NUMBER_ID={$row[PO_BREAK_DOWN_ID]} order by b.TASK_SEQUENCE_NO";
	...
	foreach($tna_sql_result as $tnaRow){
		...
		$tna_task_id_arr[$tnaRow['TASK_NUMBER']]=$tnaRow['TASK_SHORT_NAME'];
		$tna_task_seq_arr[$tnaRow['TASK_NUMBER']]=$tnaRow['TASK_SEQUENCE_NO'];
	}
...
//echo $insfection_qty;die;
  ...
  // print_r($precost_data_arr);
    //echo 23455;die();
	$hisSql =  "select id,MST_ID,APPROVED_BY,APPROVED_DATE, SEQUENCE_NO,APPROVED,APPROVED_NO from APPROVAL_HISTORY where ENTRY_FORM=66 and MST_ID in(".$update_id.") order by SEQUENCE_NO,APPROVED_BY,id";
	//echo $hisSql;die();
...
$hisSql ="select MST_ID,INSERTED_BY,REFUSING_REASON,APPROVED_NO,APP_TYPE,INSERT_DATE from REFUSING_CAUSE_HISTORY where ENTRY_FORM=66 and MST_ID in(".$update_id.") order by INSERTED_BY,id ASC";
   // echo $hisSql;die();
	$hisSqlRes=sql_select($hisSql);
	$refusing_res_arr = [];
	foreach($hisSqlRes as $key => $prerow){
		$refusing_res_arr[$prerow['INSERTED_BY']][]= $prerow['REFUSING_REASON'];
		//$refusing_data_arr[]= $prerow;
	  ...
	  <body>
		...
		<section>	
				...
				<table cellspacing="0" cellpadding="0" border="0">
					<tr>
						<td rowspan="2" width="300"><img  src='../../../<?=$imge_arr[$cbo_company_id]; ?>' style="height:50px;" /></td>
						<td style="font-size:20px; text-align: center;">
							<strong><?= $lib_company[$cbo_company_id]; ?></strong>
						</td>
					</tr>
					<tr>
			...
			<table border="1" rules="all" style="width:100%;">
				...
				<tr>
					 <td>Erosion Date <!--Budget Profit --></td>
					<td>
						<?=change_date_format($row['EROSION_DATE']);?>
						<!-- < ?=number_format($precost_data_arr['TotalMargin'],2); ?> -->
					</td>
					<td>Erosion Value</td>
			...
			<table>
				...
				<tr>
					<td><strong>Final comments</strong></td>
					<td>:</td>
					<td valign="top"><?=$row['FINAL_COMMENTS']; ?></td>
				</tr>

				<!-- < ?= return_field_value("profit_center_name","erosion_entry_profits","id={$row['PROFIT_CENTER']}"); ?> -->
				<!-- erosion_entry_profits  -->
				<!-- $field_array_dtls = "ID, MST_ID, PROFIT_CENTER_ID, PERCENTAGE, DISTRIBUTION_VALUE, INSERTED_BY, INSERT_DATE, STATUS_ACTIVE, IS_DELETED"; -->

				<tr>
					<td><strong>Profit Center/Percentage/Value</strong></td>
					<td>:</td>
					<td valign="top">
					<?php 
						foreach($erosion_profit_res as $row){
							$values = $lib_profit_center[$row['PROFIT_CENTER_ID']].'/'.$row['PERCENTAGE'].'/'.$row['DISTRIBUTION_VALUE'];
							?>
							<span><?= $values;?><span>
							<br>
						<?php
				...
				$dept_sql = "SELECT ID, DEPARTMENT_NAME FROM lib_department where id IN ($dept_ids)";
				$dept_res  =sql_select($dept_sql);
				?>
				...
				<tr>
					...
					<td valign="top">
					    <?php
						$department_name = '';
						foreach($dept_res as $row){
							$department_name .= $row['DEPARTMENT_NAME'].', ';
							?> 
						<?php
						}
					    ?> 
						<span><?= rtrim($department_name, ', ');?><span>
					</td>
				</tr>
			...
			<table border="1" rules="all">
				<tr><strong>Show the TNA (Task Wise)</strong></tr>
				<tr>   
					<th>Task Name</th>
				    ...
				    <? 
					foreach($tna_task_id_arr as $task_id=>$TASK_SHORT_NAME){
					...
					?>
					<tr>   
						<td title="Task Id:<?=$task_id;?>, Seq:<?= $tna_task_seq_arr[$task_id];?>"><?= $TASK_SHORT_NAME;?></td>
			...
			<table border="1" rules="all">
				...
				<tr>   
					...
					$precost_data_arr['knitting_day'][31]=$precost_data_arr['KnitTotal'];
			...
			<br>
			<div><strong>Show Garments Qty (Task Wise)</strong></div>
			<table border="1" rules="all">
				<tr>   
					<th>Task Name</th>
				...
				<tr> 
					...
					<td align="right"><?=$sewing_input;?></td> 
				...
				<tr>     
					...
					<td align="right"><?=$sewing_output;?></td> 
				...
				<tr>    
					...
					<td align="right"><?=$total_fin_gmt;?></td> 
					...
					<td align="right"><?=$total_ins_gmt;?></td> 
				...
				<tr>      
					...
					<td align="right"><?=$total_ship_out_gmt;?></td> 
			...
			<table border="1" rules="all">
				<tr><strong>Approval Authority / History</strong></tr>
				<tr>   
					<th>Department</th>
					<th>Name</th>
					<th>Designation</th>
					...
					{  $userAppSl=0;
						...
						<tr>   
							...
							<td><?=$refusing_res_arr[$row['APPROVED_BY']][$userAppSl];?></td>
							<td><?=$app_sta;?></td>
...
<footer>
	...
	?>
	<img src="<?=$waterMarkImage;?>" style="width:150px;">
</footer>
</div>
</div>
 ...
 <style>
	...
	if($is_mail_send==1){
		...
		$title = "<h2>Dear Sir,<br>
		Please log in to ERP and  check below erosion request for your electronic approval</h2><br>";

		$mailBody=$title . $mailBody;
