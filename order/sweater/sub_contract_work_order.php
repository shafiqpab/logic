...
{
	if( form_validation('cbo_company_name*cbo_supplier*txt_booking_date*txt_job_no*cbo_service_type*cbo_source*cbo_pay_mode*txt_amount*cbo_buyer_name','Company Name*Service Company*WO Date*Job No*Service Type*Source Name*Pay Mode*Amount*Buyer Name')==false )
	{
		return;
	}
	...
	var dataString = "cbo_company_name*cbo_supplier*cbo_service_type*txt_booking_date*cbo_currency*txt_exchange_rate*cbo_pay_mode*txt_tenor*txt_attention*cbo_source*cbo_buyer_name*cbo_brand_id*cbo_team_leader*cbo_dealing_merchant*txt_delivery_to*txt_closing_date*txt_mc_available*txt_mc_allocate*txt_prod*txt_job_no*txt_style_no*txt_gmts_no*txt_gauge_id*txt_wo_qty*txt_dyeing_charge*txt_amount*txt_start_date*txt_end_date*update_id*dtls_update_id*txt_job_id*txt_booking_no*txt_wo_qty_per*txt_total_wo_qty*rate_break_down";
...
function fnc_sub_contract_wo_response()
{
	if(http.readyState == 4)
	{
		...
		reset_form('','','txt_job_no*txt_style_no*txt_gmts_no*txt_gauge*txt_gauge_id*txt_wo_qty*balance_req_qty*txt_dyeing_charge*txt_amount*txt_req_qty*txt_wo_qty_per*txt_total_wo_qty*txt_start_date*txt_end_date*dtls_update_id*rate_break_down','','','');
...
function openmypage_wovalue(page_link,title)
{
	...
	emailwindow.onclose=function()
	{
		...
		var gauge_id=this.contentDoc.getElementById("gauge_id").value
		var req_qanty=this.contentDoc.getElementById("wo_qanty").value
		var job_id=this.contentDoc.getElementById("selected_job_id").value
		var balance_woqty=this.contentDoc.getElementById("bwoqty").value
		if (theemail.value!="")
		{
			...
			$("#txt_gauge_id").val(gauge_id);
			$("#txt_req_qty").val(req_qanty);
			$("#balance_req_qty").val(balance_woqty);
			$('#txt_wo_qty').attr('placeholder',balance_woqty);
			release_freezing();
		}
	}
}
...
{
	reset_form('sub_contract_wo','list_container','','txt_booking_date,<? echo date("d-m-Y"); ?>','disable_enable_fields("cbo_supplier*cbo_service_type*txt_booking_date*cbo_currency*txt_exchange_rate*cbo_pay_mode*txt_tenor*txt_attention*cbo_source*cbo_buyer_name*cbo_brand_id*cbo_team_leader*cbo_dealing_merchant*txt_delivery_to*txt_closing_date*txt_mc_available*txt_mc_allocate*txt_prod*txt_booking_no",0)','cbo_uom');
...
function fnc_calculate(qty)
{
	var balance_qty=$('#balance_req_qty').val()*1;
	if(qty>balance_qty){
		alert("Service Qty Can't Be Greater Then Plan Cut Qty.");
		$('#txt_wo_qty').val('');
		$('#txt_total_wo_qty').val('');
		return;
	}
	var percent=$('#txt_wo_qty_per').val()*1;
	var req_qty=$('#txt_wo_qty').val()*1;
	var percent_amount=0;
	if(percent>0){
		percent_amount=req_qty*(percent/100);
	}	
	var wo_qty=Math.round(req_qty+percent_amount);
	var dyeing_charge=$('#txt_dyeing_charge').val();
	var amount=(wo_qty*1)*(dyeing_charge*1);
	$('#txt_total_wo_qty').val(wo_qty);
	$('#txt_amount').val(number_format_common( amount, 2));
}
function fnc_calculate_per(value){
	if(value!='' && value>0){
		var req_qty=$('#txt_wo_qty').val()*1;
		var percent_amount=req_qty*(value/100);
		var wo_qty=Math.round(req_qty+percent_amount);
		$('#txt_total_wo_qty').val(wo_qty);
		var dyeing_charge=$('#txt_dyeing_charge').val();
		var amount=(wo_qty*1)*(dyeing_charge*1);
		...
		var req_qty=$('#txt_wo_qty').val()*1;
		$('#txt_total_wo_qty').val(req_qty);
		var dyeing_charge=$('#txt_dyeing_charge').val();
		var amount=(wo_qty*1)*(dyeing_charge*1);
		$('#txt_amount').val(number_format_common( amount, 2));
	}
}
function job_rate_pop(){

	if(form_validation('txt_job_no','Job Load First')==false)
	{
		return;
	}
	else 
	{
		...
		var rate_break_down=$('#rate_break_down').val();
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/sub_contract_work_order_controller.php?job_id='+job_id+'&rate_break_down='+rate_break_down+'&action=rate_dtls_popup','Rate Dtls Popup', 'width=360px,height=400px,center=1,resize=1,scrolling=0','../')
		release_freezing();
		emailwindow.onclose=function()
		{
			var break_down=this.contentDoc.getElementById("rate_breck_down").value
			var rate=this.contentDoc.getElementById("total_rate").value
			if (break_down!="")
			{
				$("#rate_break_down").val(break_down);
				$("#txt_dyeing_charge").val(rate);				
				var wo_qty=$('#txt_total_wo_qty').val();
				var amount=(wo_qty*1)*(rate*1);
				$('#txt_amount').val(number_format_common( amount, 2));
			}
		}
	}
				...
				<table width="1350" cellspacing="2" cellpadding="0" border="0">
					<tr>
						...
						<td colspan="5">
							...
							<input style="width:130px;" type="hidden" class="text"  name="txt_booking_no" id="txt_booking_no"   readonly/>
						</td>
					</tr>
					<tr>
						...
						<td width="120" class="must_entry_caption">Service Company</td>
				...
				<table cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                            ...
                            <table align="center" width="820" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all" id="dtls_part">
                                <thead>
                                    <tr>
										...
										<th width="80">WOQ</th>
                                        <th width="80">Extra Percent</th>
                                        <th width="80">Total WOQ</th>
                                ...
                                <tbody>
									...
									<td>
										...
										<input style="width:70px;" type="hidden" class="text_boxes"  name="txt_gauge_id" id="txt_gauge_id"   readonly/>
                    				</td>
									<td>
										<input style="width:70px;" type="text" class="text_boxes"  name="txt_req_qty" id="txt_req_qty" readonly/>
                    				</td>
									<td>
										<input style="width:70px;" type="text" class="text_boxes"  name="txt_wo_qty" id="txt_wo_qty"  onKeyUp="fnc_calculate(this.value);" />
										<input type="hidden" class="text_boxes"  name="balance_req_qty" id="balance_req_qty"/>
                    				</td>
									<td>
										<input style="width:70px;" type="text" class="text_boxes"  name="txt_wo_qty_per" id="txt_wo_qty_per"  onKeyUp="fnc_calculate_per(this.value)"/>
                    				</td>
									<td>
										<input style="width:70px;" type="text" class="text_boxes"  name="txt_total_wo_qty" id="txt_total_wo_qty"/>
                    				</td>									
									<td id="dyeing_charge_td">
										<input type="text" id="txt_dyeing_charge" name="txt_dyeing_charge" style="width:50px;" class="text_boxes_numeric"  onDblClick="job_rate_pop()" readonly/>
										<input type="hidden" id="rate_break_down" value="" />
									</td>
									<td>
                    ...
                    <tr>
                        <td align="center" valign="middle" colspan="10">
							...
							<input type="button" value="Print Booking 2" onClick="generate_trim_report('show_trim_booking_report2',1)"  style="width:100px;" name="print_booking2" id="print_booking2" class="formbutton" />
                        </td>
                    </tr>
            	</table>
        	</fieldset>
