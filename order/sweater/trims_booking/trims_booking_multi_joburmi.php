...
function fnc_generate_booking(param,po_id,pre_cost_id,cbo_company_name){
	...
	var data="action=generate_fabric_booking&data="+data+'&cbo_company_name='+cbo_company_name+'&txt_delivery_date='+txt_delivery_date+'&cbo_booking_month='+cbo_booking_month+'&cbo_booking_year='+cbo_booking_year+'&garments_nature='+garments_nature+'&cbo_currency='+cbo_currency+'&cbo_currency_job='+cbo_currency_job+'&cbo_level='+cbo_level+'&pre_cost_id='+precost_id+'&param='+param+'&txt_booking_no='+txt_booking_no+'&txt_booking_date='+txt_booking_date+'&cbo_supplier_name='+cbo_supplier_name;
...
function calculate_amount(i){
	...
	var totWoAmt=txtrate*txtwoq;
	 var totWo_Amt = totWoAmt.toFixed(5);
	//document.getElementById('txtamount_'+i).value=number_format_common((txtrate*txtwoq),5,0);
	 document.getElementById('txtamount_'+i).value=totWo_Amt;
	var tot_amount=0;var totamt=0;var tot_qty=0;
	var tot_amt=(document.getElementById('txtamount_'+i).value)*1
	//$("#tot_woqqty").val(txtwoq);
	//$("#totamt").val(tot_amt);
	...
	for (var j=1; j<=row_num; j++){
		...
		tot_amount+=amount;
			...
			woqty=$("#txtwoq_"+j).val()*1;
			tot_qty+=woqty;
			 totamt+=amount;
	}
	var totalamount=tot_amount*1;
	var totamount = totalamount.toFixed(5);
	var total_mount = totamt.toFixed(5);
	  document.getElementById('tot_amount').value=totamount;
	// document.getElementById('tot_amount').value=number_format_common(tot_amount,5,0);
		// alert(tot_qty);
		$("#tot_woqqty").val(tot_qty);
		$("#totamount").val(total_mount);
}
// function tot_woqty_set()
// 	{
// 		var tot_qty=0;var tot_amt=0;
// 		var row_num=$('#tbl_list_search tbody tr').length;
// 	//	alert(row_num);
// 		for (var i=1; i<=row_num; i++){
// 			//qty+=Number(document.getElementById("txtwoq_"+i).value);
// 			//tot_qty+=Number(document.getElementById("txtwoq_"+i).value)*1;
// 			qty=$("#txtwoq_"+i).val()*1;
// 			tot_qty+=qty;
// 			 tot_amt+=(document.getElementById("txtamount_"+i).value)*1;
			
// 		}
// 		alert(tot_qty);
// 		$("#tot_woqty").val(tot_qty);
