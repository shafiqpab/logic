...
function load_drop_down_supplier($data){
	if($data==5 || $data==3){
		$cbo_supplier_name=create_drop_down( "cbo_supplier_name", 130, "select id,company_name from lib_company where status_active=1 and is_deleted=0 order by company_name", "id,company_name",1, "-- Select Company --", "", "get_php_form_data( this.value, 'load_drop_down_attention', 'requires/short_trims_booking_multi_job_controller');",0,"" );
	}
	else
	{
		$cbo_supplier_name= create_drop_down( "cbo_supplier_name", 130, "select c.supplier_name,c.id from lib_supplier_tag_company a, lib_supplier_party_type b, lib_supplier c where c.id=b.supplier_id and a.supplier_id = b.supplier_id  and b.party_type=4 and c.status_active=1 and c.is_deleted=0 group by c.id, c.supplier_name order by c.supplier_name","id,supplier_name", 1, "--Select Supplier--",$selected,"get_php_form_data( this.value, 'load_drop_down_attention', 'requires/short_trims_booking_multi_job_controller');","");
	}
...
if ($action=="create_reqsn_search_list_view")
{
	...
	$sql="select a.id, a.booking_no_prefix_num, a.booking_no, a.company_id, a.booking_date, a.delivery_date, a.responsible_dept, c.style_ref_no, d.po_number, a.pay_mode from wo_booking_mst a, wo_booking_dtls b, wo_po_details_master c, wo_po_break_down d where a.booking_no=b.booking_no and b.job_no=c.job_no and b.job_no=d.job_no_mst and b.po_break_down_id=d.id and a.booking_type=13 and a.entry_form=718 and a.is_short=1
...
if ($action=="populate_data_from_search_popup_requisition"){
	...
	foreach ($data_array as $row){
		echo "get_php_form_data(".$row[csf("company_id")].", 'populate_variable_setting_data', 'requires/short_trims_booking_multi_job_controller' );\n";
...
if ($action=="generate_fabric_booking"){
	...
	foreach($sql_cu_booking as $row_cu_booking){
		if($row_cu_booking['ENTRY_FORM_ID']==718)
		{
        ...
        if($cbo_level==1){
			foreach ($nameArray as $selectResult){
				...
				<tr bgcolor="<?=$bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<?=$i; ?>">
                    ...
                    <td width="80" align="right"><input type="text" style="width:100%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtwoq_<?=$i; ?>" value="<?=number_format($reqsn_qty,4,'.','');?>" onClick="open_consumption_popup('requires/short_trims_booking_multi_job_controller.php?action=consumption_popup', 'Consumtion Entry Form','txtpoid_<?=$i; ?>',<?=$i; ?>);" readonly /> </td>
...
if ($action=="show_trim_booking"){
	...
	$cu_booking_arr=array();
	$sql_cu_booking=sql_select("select C.JOB_NO, C.PRE_COST_FABRIC_COST_DTLS_ID as BOMID, C.PO_BREAK_DOWN_ID, SUM(C.WO_QNTY) AS CU_WO_QNTY, SUM(C.AMOUNT) AS CU_AMOUNT from wo_po_details_master a, wo_po_break_down  d, wo_booking_dtls c where a.job_no=d.job_no_mst and a.job_no=c.job_no and d.id=c.po_break_down_id and a.company_name=$cbo_company_name and c.pre_cost_fabric_cost_dtls_id=$pre_cost_id and c.entry_form_id!=718 and c.status_active=1 and c.is_deleted=0 group by c.job_no, c.pre_cost_fabric_cost_dtls_id, c.po_break_down_id");
	foreach($sql_cu_booking as $row_cu_booking){
		$cu_booking_arr[$row_cu_booking['JOB_NO']][$row_cu_booking['BOMID']]['cu_woq'][$row_cu_booking['PO_BREAK_DOWN_ID']]=$row_cu_booking['CU_WO_QNTY'];
		$cu_booking_arr[$row_cu_booking['JOB_NO']][$row_cu_booking['BOMID']]['cu_amount'][$row_cu_booking['PO_BREAK_DOWN_ID']]=$row_cu_booking['CU_AMOUNT'];
	}
        ...
        if($cbo_level==1){
			foreach ($nameArray as $selectResult){
				...
				<tr bgcolor="<?=$bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<?=$i; ?>">
                    ...
                    <td width="80" align="right"><input type="text" style="width:100%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; text-align:right; background-color:#FFC" id="txtwoq_<?=$i; ?>" value="<?=number_format($woq,4,'.',''); ?>" onClick="open_consumption_popup( 'requires/short_trims_booking_multi_job_controller.php?action=consumption_popup', 'Consumtion Entry Form','txtpoid_<?=$i; ?>',<?=$i; ?>);" readonly />
                    </td>
                    <td width="55" align="right"><input type="text" style="width:100%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; text-align:right; background-color:<?=$bgcolor; ?>" id="txtexchrate_<?=$i; ?>" value="<?=$exchange_rate; ?>" readonly /></td>
                    <td width="80" align="right">
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0){
		...
		$data_array ="(".$id.",2,1,4,734,1,'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',".$cbo_company_name.",".$cbo_buyer_name.",".$txt_booking_date.",".$txt_delivery_date.",".$cbo_basis_id.",".$txt_reqsn_id.",".$txt_reqsn_no.",".$cbo_source.",".$cbo_currency.",".$cbo_pay_mode.",".$cbo_supplier_name.",".$txt_tenor.",".$cbo_division_id.",".$cbo_responsible_dept.",".$txt_responsible_person.",".$cbo_ready_to_approved.",".$cbo_level.",".$txt_reason.",".$txt_attention.",".$txtdelivery_address.",".$txt_remark.",'".$variable_short_booking."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
				...
				for($c=0;$c < count($consbreckdown_array);$c++){
					...
					if(str_replace("'","",$consbreckdownarr[4])!="")
					{
						if (!in_array(str_replace("'","",$consbreckdownarr[4]),$new_array_color)){
							$color_id = return_id( str_replace("'","",$consbreckdownarr[4]), $color_library, "lib_color", "id,color_name","734");
	...
	if ($operation==0){
		...
		for ($i=1; $i<=$total_row; $i++){
			...
			foreach($strdata->$job->$trimcostid->po_id as $poId){
				...
				$amount=$wqQty*$rate;
				$data_array1 ="(".$id_dtls.",".$$txttrimcostid.",".$poId.",".$$txtjob_id.",".$txt_booking_no.",".$booking_mst_id.",2,1,734,".$$txttrimgroup.",".$$txtuom.",".$$cbocolorsizesensitive.",".$wqQty.",".$$txtexchrate.",".$$txtrate.",".$amount.",'".$txtdlvdate."',".$$txtdescid.",".$$txtcountry.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
        ...
        <div align="center" style="width:100%;" >
            ...
            <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
                <table width="1000" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
                    ...
                    <tr class="general">
                        <td><?=create_drop_down( "cbo_company_mst", 130, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", '', "load_drop_down( 'short_trims_booking_multi_job_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );"); ?></td>
                        <td id="buyer_td"><?=create_drop_down( "cbo_buyer_name", 130, $blank_array,"", 1, "-- Select Buyer --", $selected, "","" ); ?></td>
                        ...
                        <td><input name="txt_date_to" id="txt_date_to" class="datepicker" style="width:50px"></td>
                        <td align="center">
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('cbo_supplier_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_booking_prifix').value+'_'+document.getElementById('cbo_search_category').value+'_'+document.getElementById('txt_style').value+'_'+document.getElementById('txt_order_search').value+'_'+document.getElementById('txt_job').value+'_'+document.getElementById('chk_job_wo_po').value+'_'+document.getElementById('internal_ref').value, 'create_booking_search_list_view', 'search_div', 'short_trims_booking_multi_job_controller','setFilterGrid(\'list_view\',-1)')" style="width:90px;" />
                        </td>
                    </tr>
                    <tr>
...
if ($action=="create_booking_search_list_view")
{
	...
	if($data[11]==0)
	{
		...
		if(!empty($data[12]))
		{
		 ...
		 $sql="select a.id, a.booking_no_prefix_num, a.booking_no, a.company_id, a.supplier_id, a.booking_date, a.delivery_date,c.style_ref_no,d.po_number,a.pay_mode from wo_booking_mst a, wo_booking_dtls b, wo_po_details_master c, wo_po_break_down d  where a.id=b.booking_mst_id and b.job_no=c.job_no and b.po_break_down_id=d.id and c.id=d.job_id and a.booking_type=2 and a.entry_form=734 and a.is_short=1
	...
	else
	{
		...
		$sql="select a.id, a.job_no, a.booking_no_prefix_num, a.booking_no, a.company_id, a.supplier_id, a.booking_date, a.delivery_date, a.pay_mode from wo_booking_mst a where a.id not in (select a.id from wo_booking_mst a, wo_booking_dtls b, wo_po_break_down c where a.id=b.booking_mst_id and b.po_break_down_id=c.id and a.booking_type=2 and a.is_short=1 and a.entry_form=734 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $internal_ref_con $company ".set_user_lavel_filtering(' and a.buyer_id','buyer_id')." $supplier_id $booking_date $booking_cond $job_cond $file_cond $ref_cond $booking_year_cond group by a.id) and a.booking_type=2 and a.is_short=1 and a.entry_form=734 and a.status_active=1 and a.is_deleted=0 $company ".set_user_lavel_filtering(' and a.buyer_id','buyer_id')." $supplier_id $booking_date $booking_cond $booking_year_cond group by a.id, a.booking_no_prefix_num, a.booking_no, a.job_no, a.company_id, a.supplier_id, a.booking_date, a.delivery_date, a.pay_mode order by a.id DESC";
	}
	 //echo $sql; die;
	?>
    <div width="900">
...
if ($action=="populate_data_from_search_popup_booking"){
	...
	foreach ($data_array as $row){
		...
		echo "load_drop_down('requires/short_trims_booking_multi_job_controller', '".$row[csf("pay_mode")]."', 'load_drop_down_supplier', 'supplier_td');\n";
...
if($action=="show_trim_booking_report2"){
	...
	<div style="width:1333px" align="center">
       ...
       <tr style="border:1px solid black;">
        ...
        <td width="49%">
	        <?
	        	echo get_spacial_instruction($txt_booking_no,'');//,734
	        ?>
    	</td>
...
if($action=="show_trim_booking_report4")
{
	...
	?>
	    <table border="1" align="left" class="rpt_table container" cellpadding="0" width="100%" cellspacing="0" rules="all" style="padding-bottom: 10px">
   			<tr>
			   <td width="150px"  style="border-right:0" align="left"><? 
			   if($report_type==1)
                   {
                       if($link == 1)
