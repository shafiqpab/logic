...
if ($action=="create_fnc_process_data"){
	...
	//echo $booking_year_cond;
	//if ($cbo_company_name!=0) $company=" and a.company_id='$data[0]'"; else $company=$cbo_company_name;
	//if ($cbo_buyer_name!=0) $buyer=" and c.buyer_name='$cbo_buyer_name'"; else $buyer="";
	//if ($cbo_supplier_name!=0) $supplier_id=" and a.supplier_id='$cbo_supplier_name'"; else $supplier_id ="";
	...
	<div style="width:1220px; overflow-y:scroll; max-height:350px;" id="buyer_list_view" >
		...
		/*$booked_item_arr=array();
		$sql_booking=sql_select("select pre_cost_fabric_cost_dtls_id, po_break_down_id, wo_qnty from wo_booking_dtls where booking_no='$booking_no' and status_active=1 and is_deleted=0");
		foreach($sql_booking as $row)
		{
			$booked_item_arr[$row[csf('po_break_down_id')]][$row[csf('pre_cost_fabric_cost_dtls_id')]]=$row[csf('wo_qnty')];
		}
		unset($sql_booking);*/
       // print_r($booked_item_arr);
        $cu_booking_arr=array();
		//echo "select c.pre_cost_fabric_cost_dtls_id, c.po_break_down_id, sum(c.wo_qnty) as cu_wo_qnty, sum(c.amount) as cu_amount from wo_po_details_master a, wo_po_break_down d, wo_booking_dtls c where a.job_no=d.job_no_mst and a.job_no=c.job_no and d.id=c.po_break_down_id and a.company_name=$company_id and a.buyer_name=$cbo_buyer_name  and c.booking_type=2 and c.status_active=1 and c.is_deleted=0 $job_cond $order_cond $ref_cond $style_cond $itemgroup_cond group by a.job_no, c.pre_cost_fabric_cost_dtls_id, c.po_break_down_id"; die;
        ...
        where a.job_no=b.job_no and a.job_no=c.job_no and a.job_no=d.job_no_mst and a.job_no=e.job_no and c.id=e.wo_pre_cost_trim_cost_dtls_id and d.id=e.po_break_down_id and a.company_name=$company_id and
        ...
        group by 
        a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.currency_id, a.style_ref_no, b.costing_per, b.exchange_rate, c.id, c.trim_group, c.description, c.brand_sup_ref, c.rate, a.insert_date, d.id, d.po_number, d.file_no, d.grouping, d.po_quantity, e.po_break_down_id order by d.id, c.id";
       ...
       //echo $sql;
        ...
        {
			//if( $booked_item_arr[$selectResult[csf('po_id')]][$selectResult[csf('wo_pre_cost_trim_cost_dtls')]]=='')
			//{
			...
			//	echo $bal_woq.'='.$cu_wo_qnty.'='.$exceed_qty_level;
                ...
                <form id="consumptionform_1" autocomplete="off">
                    <table width="1150" cellspacing="0" class="rpt_table" border="0" id="tbl_consmption_cost" rules="all">
                        <thead>
                            ...
                            <tr>
                                <th width="40">SL</th><th  width="100">Article No</th><th  width="100">Gmts. Color</th><th  width="70">Gmts. sizes</th><th  width="100">Description</th><th  width="100">Brand/Sup Ref</th><th  width="100">Item Color</th><th width="80">Item Sizes</th><th width="70"> Wo Qty</th><th width="40">Excess %</th><th width="70">WO Qty.</th><th width="100">Rate</th><th width="80">Amount</th><th width="60">RMG Qnty</th><? if($cbo_colorsizesensitive==0){ ?><th width=""></th> <? } ?>
							...
							foreach( $data_array as $row ){
								...
								if($booking_amt>0)
								{
									$rate=$booking_amt/$booking_qty;
									 //echo $booking_amt.="A".$booking_qty;
								}
								else
								{
									 $rate=$txt_avg_price;
								...
								?>
									<tr id="break_1" align="center">
                                        ...
                                        <td><input type="hidden" id="reqqty_<? echo $i;?>"  name="reqqty_<? echo $i;?>" class="text_boxes_numeric" style="width:70px"    value="<? echo $txtwoq_cal ?>" readonly/>
                                        	...
                                        	<input type="text" id="qty_<? echo $i;?>" onBlur="validate_sum( <? echo $i; ?> )" onChange="set_sum_value( 'qty_sum', 'qty_' );set_sum_value( 'woqty_sum', 'woqny_' );calculate_requirement(<? echo $i;?>);copy_value(this.value,'qty_',<? echo $i;?>)"  name="qty_<? echo $i;?>" class="text_boxes_numeric" style="width:80px"   placeholder="<? echo $txtwoq_cal; ?>" value="<? if($booking_cons>0){echo $booking_cons;} ?>"/> 
                                        </td>
                                        <td>
                                        	<input type="text" id="excess_<? echo $i;?>" onBlur="set_sum_value( 'excess_sum', 'excess_' ) "  name="excess_<? echo $i;?>" class="text_boxes_numeric" style="width:40px" onChange="calculate_requirement(<? echo $i;?>);set_sum_value( 'excess_sum', 'excess_' );set_sum_value( 'woqty_sum', 'woqny_' );copy_value(this.value,'excess_',<? echo $i;?>) " value="<? echo $booking_data_arr[$row[csf('color_size_table_id')]][process_loss_percent]; ?>" disabled/> 
                                        </td>
