...
<script>
	...
	function fnc_openmypage_requisition(page_link,title)
	{
		...
		emailwindow.onclose=function(){
			...
			if(theemail!=""){
				...
				get_php_form_data( exdata[0], "populate_data_from_search_popup_requisition", "requires/short_trims_booking_multi_job_controller" );
	...
	function fnc_process_data(){
		if (form_validation('cbo_company_name*txt_booking_no','Company*Booking No')==false){
			...
			var page_link='requires/short_trims_booking_multi_job_controller.php?action=fnc_process_data';
	...
	function fnc_generate_booking(param,po_id,pre_cost_id,cbo_company_name){
		...
		http.open("POST","requires/short_trims_booking_multi_job_controller.php",true);
	...
	function openmypage_booking(page_link,title){
		...
		emailwindow.onclose=function(){
			...
			if (theemail.value!=""){
				...
				$("#cbo_currency").attr("disabled",true);
				get_php_form_data( theemail.value, "populate_data_from_search_popup_booking", "requires/short_trims_booking_multi_job_controller" );
...
<div style="width:100%;" align="center">
    ...
    <form name="trimsbooking_1"  autocomplete="off" id="trimsbooking_1">
        <fieldset style="width:1050px;">
            <legend title="V3">Short Trims Booking[Sweater] &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;   <font id="app_sms" style="color:#F00"></font></legend>
            <table  width="1050" cellspacing="2" cellpadding="0" border="0">
                <tr>
                    <td align="right" class="must_entry_caption" colspan="4"><b>Booking No:</b></td>              
                    <td colspan="4">
                        <input class="text_boxes" type="text" style="width:160px" onDblClick="openmypage_booking('requires/short_trims_booking_multi_job_controller.php?action=trims_booking_popup','Trims Booking Search');"  placeholder="Double Click for Booking" name="txt_booking_no" id="txt_booking_no" readonly/>
                ...
                <tr>
                    ...
                    <td width="150"><?=create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected, "get_php_form_data( this.value, 'populate_variable_setting_data', 'requires/short_trims_booking_multi_job_controller' );","","");?></td>
                ...
                <tr>
                    ...
                    <td><input name="txt_reqsn_no" id="txt_reqsn_no" class="text_boxes" type="text" style="width:120px" disabled onDblClick="fnc_openmypage_requisition('requires/short_trims_booking_multi_job_controller.php?action=requisition_popup','Trims Requisition Search'); " readonly placeholder="Browse" /></td>
                ...
                <tr>   
                    ...
                    <td> <?=create_drop_down( "cbo_pay_mode", 130, $pay_mode,"", 1, "-- Select Pay Mode --", "", "load_drop_down( 'requires/short_trims_booking_multi_job_controller', this.value, 'load_drop_down_supplier', 'supplier_td' )","" ); ?></td>
                    <td class="must_entry_caption">Supplier Name</td>
                    <td id="supplier_td"><?=create_drop_down( "cbo_supplier_name", 130, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=4 and a.status_active =1 and a.is_deleted=0 order by supplier_name","id,supplier_name", 1, "-- Select Supplier --", $selected, "get_php_form_data( this.value, 'load_drop_down_attention', 'requires/short_trims_booking_multi_job_controller');",0 ); ?></td>
                	<td>Tenor</td>
