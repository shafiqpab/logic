...
if ($action=="save_update_delete_mst")
{
	...
	if ($operation==0)
	{
		...
		$new_sys_number=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'LTW', date("Y"), 5, "select labtest_prefix, labtest_prefix_num from   wo_labtest_mst where company_id=$cbo_company_name and entry_form=576 and TO_CHAR(insert_date,'YYYY')=".date('Y',time())." order by id DESC ", "labtest_prefix", "labtest_prefix_num",""));
		...
		$data_array="(".$id.",'".$new_sys_number[1]."','".$new_sys_number[2]."','".$new_sys_number[0]."',576,".$cbo_company_name.", ".$cbo_supplier.",".$txt_workorder_date.",".$txt_delivery_date.",".$cbo_currency.",".$txt_exchange_rate.",".$cbo_pay_mode.",".$txt_attention.",".$txt_tenor.",".$txt_address.",".$cbo_ready_to_approved.",".$txt_vat_per.",'".$user_id."','".$pc_date_time."',1,0)";
...
if($action=="save_update_delete_dtls")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		{ 
			...
			{
				$color_id = return_id( str_replace("'","",$txt_color), $color_library, "lib_color", "id,color_name","576");  
		 ...
		 $data_array="(".$id_dtls.",".$update_id.",".$txt_job_no.",".$txt_order_id.",576,".$cbo_test_for.",".$txt_party_type_id.",".$txt_party_type_name.",'".$color_id."',".$txt_amount.",".$txt_discount.",".$txt_delivery_charge.",".$txt_wo_value.",".$txt_vat_amount.",".$txt_wo_value_with_vat.",".$txt_remarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,".$save_qty_break_data.")";
		 $add_comma=0;
...
where mst_id=$data and  status_active = 1 and is_deleted=0 and entry_form=576";
...
if ($action=="load_php_mst_data")
{
	 ...
	 pay_mode,attention,tenor,address,ready_to_approved,vat_percent,inserted_by,insert_date,is_approved from wo_labtest_mst  where labtest_no='$data[1]' and entry_form=576";
	 $data_array=sql_select($sql);
	 foreach ($data_array as $row)
	 {
...
if($action=="load_php_dtls_data_to_form")
{
	 ...
	 where id=$data and status_active =1 and is_deleted = 0 and entry_form=576";
	 //echo $sql;die;
	 $data_array=sql_select($sql);
	 foreach ($data_array as $row)
	 {
...
if($action=="show_trim_booking_report")
{
	...
	foreach($result as $val)
	{
     ...
     <div style="800px;">
								...
								$mst_id=return_field_value("id as mst_id","wo_labtest_mst","labtest_no='$labtest_no' and entry_form=576 and status_active=1","mst_id");
