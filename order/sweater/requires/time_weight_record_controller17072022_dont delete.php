    ...
    <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
        <table width="1050" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
            ...
            <tr class="general">
				...
				<td ><?
					$date_category=array(1=>"Est. Ship Date",2=>"Insert Date");
				 echo create_drop_down( "cbo_date_category", 90, $date_category,'', 0, "-select date categoy-",$selected, "" ); ?>
...
if($action=="create_style_search_list_view")
{
	...
	if($db_type==0)
	{
		...
		if ($data[3]!="" &&  $data[4]!="") $shipment_date = "and estimated_shipdate between '".change_date_format($data[3], "yyyy-mm-dd", "-")."' and '".change_date_format($data[4], "yyyy-mm-dd", "-")."'"; else $shipment_date ="";
			...
			if ($data[3]!="" &&  $data[4]!="") $shipment_date = "and insert_date between '".change_date_format($data[3], "yyyy-mm-dd", "-")."' and '".change_date_format($data[4], "yyyy-mm-dd", "-")."'"; else $shipment_date ="";
		}
	}
	else if($db_type==2)
	{
		...
		if ($data[3]!="" &&  $data[4]!="") $shipment_date = "and estimated_shipdate between '".change_date_format($data[3], "yyyy-mm-dd", "-",1)."' and '".change_date_format($data[4], "yyyy-mm-dd", "-",1)."'"; else $shipment_date ="";
			...
			if ($data[3]!="" &&  $data[4]!="") $shipment_date = "and insert_date between '".change_date_format($data[3], "yyyy-mm-dd", "-",1)."' and '".change_date_format($data[4], "yyyy-mm-dd", "-",1)."'"; else $shipment_date ="";
		}
	}
...
if ($action=="populate_data_from_search_popup")
{
	...
	foreach ($dtls_arr as $row)
	{
		...
		echo "document.getElementById('txtknitinglbs_tot').value = '".$row[csf("knitinglbs")]."';\n";
	...
	$dtls_rarr=sql_select("select id, body_part_id, minute, second, movingsec, knitinglbs, knitinggm from sample_development_fabric_acc where sample_mst_id='$data' and is_deleted=0 and status_active=1 order by id asc");
	$i=1;
	foreach ($dtls_rarr as $row)
	{
		...
		echo "document.getElementById('txtknitinglbs_".$i."').value = '".$row[csf("knitinglbs")]."';\n";  
...
if ($action=="save_update_delete_mst")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$field_array_mst="id, garments_nature, entry_form_id, requisition_number, requisition_number_prefix, requisition_number_prefix_num, company_id, location_id, buyer_name, style_ref_no, mers_style, product_dept, product_code, item_category, article_no, item_name, region, agent_name, season_buyer_wise, team_leader, dealing_marchant, bh_merchant, remarks, estimated_shipdate, requisition_date, gauge_no_ends, efficiency, brand_id, season_year, quotation_id, inserted_by, insert_date, status_active, is_deleted";
		$txt_mers_style=str_replace("'", "", $txt_mers_style);
		$data_array_mst="(".$id.",".$garments_nature.",'245','".$new_sample_no[0]."','".$new_sample_no[1]."','".$new_sample_no[2]."',".$cbo_company_name.",".$cbo_location_name.",".$cbo_buyer_name.",".$txt_style_ref.",'".$txt_mers_style."',".$cbo_product_department.",".$txt_product_code.",".$cbo_item_catgory.",".$txt_artical_no.",".$cbo_gmts_item.",".$cbo_region.",".$cbo_agent.",".$cbo_season_id.",".$cbo_team_leader.",".$cbo_dealing_merchant.",".$txt_bh_merchant.",".$txt_remarks.",".$txt_est_ship_date.",".$txt_sample_date.",".$txt_gause.",".$txt_efficiency.",".$cbo_brand_id.",".$cbo_season_year.",".$mst_ref_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		...
		$field_array_dtls="id, sample_mst_id, sample_name, acc_status_id, entry_form_id, color_id, size_id, designer, tech_manager, programmer, yarn_quality, count_ply, minute, second, movingsec, knitinglbs, knitinggm, critical_point, knitingweight, knitting_system, machine_brand_name, inserted_by, insert_date, is_deleted, status_active";
		$data_array_dtls="(".$id_dtls.",".$id.",".$cbo_sample_type.",".$cbo_dev_no.",'245','".$color_id."','".$size_id."',".$txt_designer.",".$txt_asst_tech_manager.",".$txt_programmer.",".$txt_yarn_quality.",".$txt_count_ply.",".$txtminute_tot.",".$txtsecond_tot.",".$txtmovingsec_tot.",".$txtknitinglbs_tot.",".$txtknitinggm_tot.",".$txtcritical_point.",".$txtknitingweight_dzn.",".$txt_knitting_system.",".$txt_machine_brand_name.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',0,1)";
		//echo "10**INSERT INTO sample_development_dtls (".$field_array.") VALUES ".$data_array; die;
		$field_array1="id, sample_mst_id, sample_dtls_id, body_part_id, minute, second, movingsec, knitinglbs, knitinggm, inserted_by, insert_date, is_deleted, status_active";
		...
		for($m=1; $m<=$panel_tr; $m++)
		{
			...
			$txtknitinglbs="txtknitinglbs_".$m;
			$txtknitinggm="txtknitinggm_".$m;
			
			if ($add_comma!=0) $data_array1 .=",";
			$data_array1 .="(".$id1.",".$id.",".$id_dtls.",'".$$txtpanelid."','".$$txtminute."','".$$txtsecond."','".$$txtmovingsec."','".$$txtknitinglbs."','".$$txtknitinggm."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',0,1)";
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		$data_array_mst="".$cbo_location_name."*".$cbo_buyer_name."*".$txt_style_ref."*".$txt_mers_style."*".$cbo_product_department."*".$txt_product_code."*".$cbo_item_catgory."*".$txt_artical_no."*".$cbo_gmts_item."*".$cbo_region."*".$cbo_agent."*".$cbo_season_id."*".$cbo_team_leader."*".$cbo_dealing_merchant."*".$txt_bh_merchant."*".$txt_remarks."*".$txt_est_ship_date."*".$txt_sample_date."*".$txt_gause."*".$txt_efficiency."*".$cbo_brand_id."*".$cbo_season_year."*".$mst_ref_id."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		$field_array_dtls="sample_name*acc_status_id*color_id*size_id*designer*tech_manager*programmer*yarn_quality*count_ply*minute*second*movingsec*knitinglbs*knitinggm*critical_point*knitingweight*knitting_system*machine_brand_name*updated_by*update_date";
		...
		for($m=1; $m<=$panel_tr; $m++)
		{
			...
			if ($add_comma!=0) $data_array1 .=",";
			$data_array1 .="(".$id1.",".$update_id.",".$updatedtls_id.",'".$$txtpanelid."','".$$txtminute."','".$$txtsecond."','".$$txtmovingsec."','".$$txtknitinglbs."','".$$txtknitinggm."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',0,1)";
...
if($action=="color_body_part_dtls_list")
{
	...
	$colordtls_data=sql_select("select id, color_id, body_part_id, bodycolorlbs, bodycolor from sample_development_rf_color where mst_id='$mst_id' and dtls_id='$dtls_id' and is_deleted=0 and status_active=1 order by id asc");
	...
	{
		$save_update=1;
		$color_data_arr[$row[csf("color_id")]][$row[csf("body_part_id")]]['consgm']=$row[csf("bodycolor")];
		$color_data_arr[$row[csf("color_id")]][$row[csf("body_part_id")]]['conslbs']=$row[csf("bodycolorlbs")];
	...
	$tbl_width=100*$i;
	?>
    <h3 style="width:<? echo $tbl_width+300; ?>px;" align="left" class="accordion_h">+Sample Color & Body Part Details Entry</h3>
       <div id="color_body_part">
    	<fieldset style="width:<? echo $tbl_width+300; ?>px;">
        	<form id="timeweightentry_3" autocomplete="off">
            <table width="<? echo $tbl_width+300; ?>" cellspacing="0" class="rpt_table" border="0" id="tbl_color_body_part" rules="all">
                <thead>
                    <tr>
                        <th width="30" rowspan="2">SL<input type="hidden" id="txtbodyc" name="txtbodyc" class="text_boxes" style="width:20px" value="<? echo $i; ?>" /></th>
                        <th width="80" rowspan="2">Color</th>
                        <?
						foreach ($body_data_arr as $id=>$bid )
						{
							?>
                            <th colspan="2"><? echo $time_weight_panel[$bid]; ?><input type="hidden" id="bodypartid_<? echo $id; ?>" value="<? echo $bid; ?>"></th>
                            <?
						}
						?>
				...
				for($m=1; $m<=$j; $m++)
				{
                    ...
                    <tr id="colortr_<? echo $m; ?>" align="center">
						...
						{
							$cons=$conslbs=0;
							$cons=$color_data_arr[$color_id][$bid]['consgm'];
							$conslbs=$color_data_arr[$color_id][$bid]['conslbs'];
							?>
                        	<td><input type="text" id="txtbodycolorlbs_<?=$m.'_'.$k; ?>" name="txtbodycolorlbs_<?=$m.'_'.$k; ?>" class="text_boxes_numeric" style="width:38px" value="<?=$conslbs; ?>" onBlur="fncbodylbsgm_calculate(1,'<?=$m.'_'.$k; ?>'); fnc_tot_per_cal();" /></td>
                            <td><input type="text" id="txtbodycolor_<? echo $m.'_'.$k; ?>" name="txtbodycolor_<? echo $m.'_'.$k; ?>" class="text_boxes_numeric" style="width:38px" value="<? echo $cons; ?>" onBlur="fncbodylbsgm_calculate(2,'<?=$m.'_'.$k; ?>'); fnc_tot_per_cal();"  /></td>
                        	<?
							$k++;
						}
						?>
                        <td><input type="text" id="txtbodycolorvallbs_<? echo $m; ?>" name="txtbodycolorvallbs_<? echo $m; ?>" class="text_boxes_numeric" style="width:43px" value="" disabled /></td>
                        <td><input type="text" id="txtbodycolorval_<? echo $m; ?>" name="txtbodycolorval_<? echo $m; ?>" class="text_boxes_numeric" style="width:43px" value="" disabled /></td>
                        <td><input type="text" id="txtbodycolorper_<? echo $m; ?>" name="txtbodycolorper_<? echo $m; ?>" class="text_boxes_numeric" style="width:58px" value="" disabled /></td>
                    </tr>
				<?
                }
            	?>
            ...
            <tfoot>
            	<tr align="center" class="general">
                    ...
                    {
                        ?>
                        <td><input type="text" id="txtbodytotlbs_<? echo $k; ?>" name="txtbodytotlbs_<? echo $k; ?>" class="text_boxes_numeric" style="width:38px" value="<? echo $row[csf("gsm_weight_yarn")]; ?>" total_value="" readonly /></td>
                        <td><input type="text" id="txtbodytot_<? echo $k; ?>" name="txtbodytot_<? echo $k; ?>" class="text_boxes_numeric" style="width:38px" value="<? echo $row[csf("gsm_weight_yarn")]; ?>" total_value="" readonly /></td>
                        <?
                    }
                    ?>
                    <td><input type="text" id="txtcolortotlbs" name="txtcolortotlbs" class="text_boxes_numeric" style="width:43px" value="" readonly /></td>
                    <td><input type="text" id="txtcolortot" name="txtcolortot" class="text_boxes_numeric" style="width:43px" value="" readonly /></td>
                    <td><input type="text" id="txttotper" name="txttotper" class="text_boxes_numeric" style="width:58px" value="" readonly /></td>
                </tr>
            </tfoot>
        </table>
        <br/>
        <table width="<? echo $tbl_width+300; ?>" cellspacing="0" class="" border="0">
            <tr>
...
if ($action=="save_update_delete_colordtls")
{
	...
	{	
		...
		$field_arr="id, mst_id, dtls_id, color_id, body_part_id, bodycolorlbs, bodycolor, inserted_by, insert_date, status_active, is_deleted";

		$add_comma=0; $data_array='';
		for($k=1; $k<=$j; $k++)
		{
			$txtcolor="txtcolor_".$k;
			$txtbodycolorvallbs="txtbodycolorvallbs_".$k;
			...
			{
				$txtbodycolorlbs="txtbodycolorlbs_".$k.'_'.$m;
				$txtbodycolor="txtbodycolor_".$k.'_'.$m;
				$bodypartid="bodypartid_".$m;
				
				if ($add_comma!=0) $data_array .=",";
				$data_array .="(".$id.",".$update_id.",".$updatedtls_id.",'".$color_id."',".$$bodypartid.",".$$txtbodycolorlbs.",".$$txtbodycolor.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
...
{
  	echo load_html_head_contents("Sample Requisition popup","../../../", 1, 1, $unicode);
	extract($_REQUEST);
	?>
	<script>
    ...
    <table width="780" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
        ...
        <tr class="general">
            ...
            <td><input type="text" style="width:70px" class="text_boxes"  name="txt_requisition_no" id="txt_requisition_no" /></td>
            ...
            <td align="center">
            	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_id').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+document.getElementById('txt_requisition_no').value+'_'+document.getElementById('txt_style').value, 'create_requisition_list_view', 'search_div', 'time_weight_record_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" /></td>
        </tr>
        <tr>
    ...
    <script>
		load_drop_down('time_weight_record_controller', <? echo $cbo_company_name; ?>, 'load_drop_down_buyer_popup', 'buyer_pop_td' );
...
if($action=="create_requisition_list_view")
{
	$data=explode('_',$data);
	if ($data[0]!=0) $company=" and company_id='$data[0]'"; else 
	{ echo "<b style='color:crimson;'> Please Select Company First.</b>"; die; }	
	if ($data[1]!=0) $buyer=" and buyer_name='$data[1]'"; else $buyer="";
	if($data[4]==1)
	{
	...
	if ($data[2]!="" &&  $data[3]!="") $estimated_shipdate  = "and requisition_date  between '".change_date_format($data[2], "yyyy-mm-dd", "-",1)."' and '".change_date_format($data[3], "yyyy-mm-dd", "-",1)."'"; else $estimated_shipdate ="";

	if ($data[5]!="") $requisition_num=" and requisition_number_prefix_num like '%$data[7]' "; else $requisition_num="";
	...
	$arr=array (1=>$comp,2=>$buyer_arr,5=>$product_dept);

	$sql= "select id,requisition_number_prefix_num,company_id,buyer_name,style_ref_no,product_dept, remarks, estimated_shipdate from sample_development_mst where entry_form_id=341 and  status_active=1 and is_deleted=0 $company $buyer $style_id_cond $style_cond $estimated_shipdate $requisition_num  order by id DESC";
	//echo $sql; die;

	echo  create_list_view("list_view", "Requisition No, Company, Buyer Name,Style Ref,Style Desc.,Product Department,Est Ship Date", "70,120,90,90,90,90,70","750","240",0, $sql , "js_set_value", "id", "", 1, "0,company_id,buyer_name,0,0,product_dept,0", $arr , "requisition_number_prefix_num,company_id,buyer_name,style_ref_no,remarks,product_dept,estimated_shipdate", "",'','0,0,0,0,0,0,0') ;

	exit();
}

if($action=="populate_data_from_sample_requisition")
{
	...
	{
		/* echo "load_drop_down( 'requires/time_weight_record_controller', '".$result[csf("company_id")]."', 'load_drop_down_location', 'location' );load_drop_down( 'requires/time_weight_record_controller', '".$result[csf("company_id")]."', 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/time_weight_record_controller', '".$result[csf("company_id")]."', 'load_drop_down_agent', 'agent_td' );load_drop_down( 'requires/time_weight_record_controller','".$result[csf("buyer_name")]."', load_drop_down_brand', 'brand_td');load_drop_down( 'requires/time_weight_record_controller','".$result[csf("buyer_name")]."', load_drop_down_season', 'season_td');\n"; */
		echo "load_drop_down( 'requires/time_weight_record_controller', '".$result[csf("company_id")]."', 'load_drop_down_location', 'location');\n";
		echo "load_drop_down( 'requires/time_weight_record_controller', '".$result[csf("company_id")]."', 'load_drop_down_buyer', 'buyer_td');\n";
		echo "load_drop_down( 'requires/time_weight_record_controller', '".$result[csf("company_id")]."', 'load_drop_down_agent', 'agent_td');\n";
		echo "load_drop_down( 'requires/time_weight_record_controller', '".$result[csf("buyer_name")]."', 'load_drop_down_brand', 'brand_td');\n";
		echo "load_drop_down( 'requires/time_weight_record_controller', '".$result[csf("buyer_name")]."', 'load_drop_down_season', 'season_td');\n";
		echo "sub_dept_load('".$result[csf("buyer_name")]."','".$result[csf("product_dept")]."');\n";
 		echo "$('#mst_ref_id').val('".$result[csf('id')]."');\n";
 		...
 		echo "$('#cbo_season_id').val('".$result[csf('season')]."');\n";		
	}
   	unset($res);
 	exit();
}
