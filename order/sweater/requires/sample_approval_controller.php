...
if($action=="show_sample_approved_list")
{
	...
	$data=explode('_',$data);
	$buyer_id=$data[1];
	 //$sql="select distinct sample_type_id from wo_po_sample_approval_info where job_no_mst='$data[0]' and is_deleted=0 and status_active=1 order by id";
	  //$sql="select min(id) as id , sample_type_id from wo_po_sample_approval_info where job_no_mst='$data[0]' and is_deleted=0 and status_active=1 and (entry_form_id is null or entry_form_id=0)  group by sample_type_id order by id";
	$sql="select min(a.id) as id , a.sample_type_id from wo_po_sample_approval_info a ,lib_buyer_tag_sample b where b.tag_sample=a.sample_type_id and a.job_no_mst='$data[0]' and a.is_deleted=0 and a.status_active=1 and b.buyer_id=$buyer_id and b.business_nature=100 and b.sequ!=0 and (a.entry_form_id is null or a.entry_form_id=0)  group by a.sample_type_id order by id";
...
{
	echo create_drop_down( "cbo_po_no", 140, "select id , po_number from wo_po_break_down where job_no_mst='$data' and status_active=1 and is_deleted=0","id,po_number", 1, "-- Select Po --", '', "load_drop_down( 'requires/sample_approval_controller', this.value, 'load_drop_down_color', 'color_td' );" );
}
...
<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
    <table width="1100" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
        ...
        <tr class="general">
            ...
            <td><input name="txt_date_from" id="txt_date_from" class="datepicker" style="width:70px" placeholder="From">
            <input name="txt_date_to" id="txt_date_to" class="datepicker" style="width:70px" placeholder="To">
            </td>
            <td>
...
if ($action=="set_php_form_data")
{
	...
	foreach ($data_array as $row)
	{
		//echo "load_drop_down( 'requires/sample_approval_controller', this.value, 'load_drop_down_color', 'color_td' )"
		echo "load_drop_down( 'requires/sample_approval_controller', '".$row[csf("po_break_down_id")]."', 'load_drop_down_color', 'color_td' ) ;\n";
