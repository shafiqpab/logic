<?
/*-------------------------------------------- Comments -----------------------
Purpose			         : 	This Form Will Create Bom Of Yarn [Sweater].
Functionality	         :	
JS Functions	         :
Created by		         :	Kausar 
...
//echo $user_level;
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Bom Of Yarn [Sweater]","../../", 1, 1, $unicode,1,'');
?>
...
<script type="text/javascript">
	...
	function fn_report_generated(action)
	{
		...
		{	
			...
			show_list_view($("#txt_job_no").val()+'_'+$("#txt_quotation_id").val()+'_0_'+$("#cbo_company_name").val()+'_'+$("#cbo_buyer_name").val()+'_'+yarn_controller+'_bomofyarn',action,'report_container','requires/pre_cost_entry_controller_v2','');
		}
	}
	...
	function openmypage(title)
	{
		...
		var page_link='requires/bom_of_yarn_controller.php?action=order_popup';
		
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/bom_of_yarn_controller.php?data='+data+'&action=job_popup', title, 'width=1130px,height=450px,center=1,resize=0,scrolling=0','../')
	...
	function open_consumption_popup(page_link,title,body_part_id,cbofabricnature_id,txtgsmweight_id,trorder,updateid_fc)
	{
		...
		var bomfyarn_approval_id = document.getElementById('bomfyarn_approval_id').value;
	...
	function fnc_fabric_cost_dtls_per_row(operation,i )
	{
		...
		if (form_validation('cbogmtsitem_'+i+'*txtbodypart_'+i+'*cbofabricnature_'+i+'*cbocolortype_'+i+'*libyarncountdeterminationid_'+i+'*fabricdescription_'+i+'*cbofabricsource_'+i+'*uom_'+i,'Gmts Item *Body Part*Fabric Nature*Color Type*Construction*Fabric Source*UOM')==false)
		{
			release_freezing();
			return;
		}
		...
		http.onreadystatechange = function (){
			fnc_fabric_cost_dtls_per_row_reponse(i)
		};	
	}
	...
	function calculate_yarn_consumption_ratio(i,type)
	{
		...
		for (var k=i; k<=rowCount; k++)
		{
			...
			var consqnty=0; var avg_yarn_cons=0; var rate_dzn=0; var rate=0; var yarnamount=0;
			...
			yarnamount=consqnty*txtrateyarn;
			rate_dzn=avg_yarn_cons*txtrateyarn;
			$('#txtratedzn_'+inc).val( number_format( yarnamount,4,'.','' ) );
			
			rate=rate_dzn*rowCount;
			$('#txtratecal_'+inc).val( number_format(rate,4,'.','' ) );
			
			$('#txtamountyarn_'+inc).val( number_format_common( rate_dzn,1,0,cbo_currercy) );
		}
		...
		set_sum_value( 'txtratedzn_sum', 'txtratedzn_', 'tbl_yarn_cost' );
		set_sum_value( 'txtratecal_sum', 'txtratecal_', 'tbl_yarn_cost' );
		set_sum_value( 'txtconsumptionyarn_sum', 'consqnty_', 'tbl_yarn_cost' );
