...
<script>
  	...
  	function copy_requisition(operation)
	{
	   ...
	   if (form_validation(data1,data2)==false)
		...
		{
			var data="action=copy_requisition&operation="+operation+get_submitted_data_string('txt_requisition_id*cbo_sample_stage*cbo_company_name*txt_requisition_date*txt_quotation_id*txt_style_name*cbo_location_name*cbo_buyer_name*cbo_season_name*cbo_product_department*cbo_dealing_merchant*cbo_agent*txt_buyer_ref*txt_est_ship_date*txt_remarks*cbo_ready_to_approved*update_id*cbo_sample_team*txt_material_dlvry_date',"../../../");
			freeze_window(operation);
			http.open("POST","requires/sweater_sample_requisition_v2_controller.php", true);
  	...
  	function fnc_sweater_sample_requisition_mst_info( operation )
	{
		...
		var comp=$('#cbo_company_name').val();
 		if(operation==4)
		{
			 print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val(), "sweater_sample_requisition_print", "requires/sweater_sample_requisition_v2_controller" );
			 return;
		}
	   if(season_mandatory_arr[comp]==1)
	   {
	   	    var data1="cbo_sample_stage*txt_requisition_date*txt_style_name*cbo_company_name*cbo_location_name*cbo_buyer_name*cbo_season_name*cbo_product_department*cbo_dealing_merchant*cbo_sample_team";
	   	    var data2="Sample Stage*Requisition Date*Style Name*Company Name*Location*Buyer*Season Name*Product Department*Dealing Merchant*Team";
	   }
	   if(season_mandatory_arr[comp]!=1)
	   {
	    	var data1="cbo_sample_stage*txt_requisition_date*txt_style_name*cbo_company_name*cbo_location_name*cbo_buyer_name*cbo_product_department*cbo_dealing_merchant*cbo_sample_team";
	   	    var data2="Sample Stage*Requisition Date*Style Name*Company Name*Location*Buyer*Product Department*Dealing Merchant*Team";
	   }
	   ...
	   if (form_validation(data1,data2)==false)
		...
		else
		{
			...
			var data="action=save_update_delete_mst&operation="+operation+get_submitted_data_string('txt_requisition_id*cbo_sample_stage*cbo_company_name*txt_requisition_date*txt_quotation_id*txt_style_name*cbo_location_name*cbo_buyer_name*cbo_season_name*cbo_product_department*cbo_dealing_merchant*cbo_agent*txt_buyer_ref*txt_est_ship_date*txt_remarks*update_id*cbo_ready_to_approved*txt_material_dlvry_date*cbo_sample_team*cbo_season_year*cbo_brand_id',"../../../");
			freeze_window(operation);
			http.open("POST","requires/sweater_sample_requisition_v2_controller.php", true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
...
function mail_send(){
   ...
   if (form_validation('cbo_company_name*update_id*cbo_ready_to_approved','Company Name*System Id*Ready App Yeas')==false)
	{
		return;
	}
	else
	{
		var returnValue=return_global_ajax_value($("#cbo_company_name").val()+'*'+$("#update_id").val(), 'sweater_sample_requisition_v2_mail_notification', '', '../../../auto_mail/sweater_sample_requisition_v2_mail_notification');
		alert(returnValue);
	}
}
	...
	function fnc_sample_details_info( operation )
	{
		...
		var is_att_file=return_global_ajax_value($('#update_id').val(), 'check_attached_file', '', 'requires/sweater_sample_requisition_v2_controller');
		if(is_att_file==0){
			alert("Please Add File in Master Part");return;	
		}
		...
		else
		{
			...
			for (var i=1; i<=row_num; i++)
			{
				...
				data_all=data_all+get_submitted_data_string('cboSampleName_'+i+'*cboGarmentItem_'+i+'*txtSmv_'+i+'*txtArticle_'+i+'*txtColor_'+i+'*txtSampleProdQty_'+i+'*txtSubmissionQty_'+i+'*txtDelvStartDate_'+i+'*txtDelvEndDate_'+i+'*txtChargeUnit_'+i+'*cboCurrency_'+i+'*updateidsampledtl_'+i+'*txtAllData_'+i+'*txtcolorcombono_'+i,"../../../");
			}
	...
	function fnc_sample_details_info_reponse()
	{
		if(http.readyState == 4)
		{
			...
			if(reponse[0]==0)
			{
				...
				load_drop_down( 'requires/sweater_sample_requisition_v2_controller', upId+'_1', 'load_drop_down_required_fabric_sample_name', 'rfSampleId_1');
				load_drop_down( 'requires/sweater_sample_requisition_v2_controller', upId+'_1', 'load_drop_down_required_fabric_gmts_item', 'rfItemId_1');
				load_drop_down( 'requires/sweater_sample_requisition_v2_controller', upId+'_2', 'load_drop_down_required_fabric_sample_name','raSampleId_1');
				load_drop_down( 'requires/sweater_sample_requisition_v2_controller', upId+'_2', 'load_drop_down_required_fabric_gmts_item', 'raItemId_1');
				load_drop_down( 'requires/sweater_sample_requisition_v2_controller', upId+'_3', 'load_drop_down_required_fabric_sample_name','reSampleId_1');
				load_drop_down( 'requires/sweater_sample_requisition_v2_controller', upId+'_3', 'load_drop_down_required_fabric_gmts_item', 'reItemIid_1');
			...
			var addDays = return_global_ajax_value( $('#update_id').val()+'__1', 'update_delivery_date', '', 'requires/sweater_sample_requisition_v2_controller');
			auto_acknowledge($('#update_id').val());		

		}
	}
	function fnc_acknowledgment()
	{
		var update_id = $('#update_id').val();
		if(update_id !='')
		{
			auto_acknowledge(update_id);	
		}
		else{
			alert("Update ID can not be null");
			return;
		}
	...
	function fnc_load_tr(up_id,type) //after save of details part or browse requisition this function load all saved data from db in specific tr
	{
		...
		{
				var data=up_id+'**'+type;
 	 			var list_view_tr = return_global_ajax_value( data, 'load_php_dtls_form', '', 'requires/sweater_sample_requisition_v2_controller');
	...
	function openmypage_sizeinfo(page_link,title,inc) // this function for sample details- sample qty browse
	{
		...
		{
			get_php_form_data(update_id_dtl, "load_data_to_sizeinfo","requires/sweater_sample_requisition_v2_controller" );
	...
	function auto_sampleDtls_color_generate(inc)
	{
		...
		var result = return_global_ajax_value( data, 'auto_sd_color_generation', '', 'requires/sweater_sample_requisition_v2_controller');
	...
	function open_fabric_description_popup(i)
	{
		...
		var page_link='requires/sweater_sample_requisition_v2_controller.php?action=fabric_description_popup&fabric_nature='+cbofabricnature+'&libyarncountdeterminationid='+libyarncountdeterminationid;
	...
	function required_fab_remarks(i)
	{
		...
		//alert(existing_rem);
		var page_link='requires/sweater_sample_requisition_v2_controller.php?action=fabric_remarks_popup&existing_rem='+existing_rem;
	...
	function add_break_down_tr(i)
	{
		...
		else
		{
			...
			$('#txtColor_'+i).removeAttr("onDblClick").attr("onDblClick","openmypage_color_size('requires/sweater_sample_requisition_v2_controller.php?action=color_popup','Color Search','1','"+i+"');");
			$('#txtSampleProdQty_'+i).removeAttr("onfocus");


			$('#txtSampleProdQty_'+i).removeAttr("onDblClick").attr("onDblClick","openmypage_sizeinfo('requires/sweater_sample_requisition_v2_controller.php?action=sizeinfo_popup','Size Search','"+i+"')");
	...
	function add_rf_tr(i)
	{
		...
		else
		{
			...
			$('#txtRfColor_'+i).removeAttr("onDblClick").attr("onDblClick","openmypage_rf_color('requires/sweater_sample_requisition_v2_controller.php?action=color_popup_rf','Color Search','"+i+"');");
	...
	function cbotype_loder( i )
	{
		...
		load_drop_down( 'requires/sweater_sample_requisition_v2_controller', cboembname+'_'+i, 'load_drop_down_emb_type', 'reType_'+i );
	}
	...
	function openmypage(sample_stage)
	{
		if(sample_stage==1)
		{
			...
			var page_link = 'requires/sweater_sample_requisition_v2_controller.php?&action=style_id_popup';
			...
			emailwindow.onclose=function()
			{
				...
				{
					freeze_window(5);
					get_php_form_data(mst_tbl_id, "populate_data_from_search_popup", "requires/sweater_sample_requisition_v2_controller" );
					$("#cbo_company_name").attr('disabled','disabled');
					$("#cbo_location_name").attr('disabled','disabled');
					$("#cbo_buyer_name").attr('disabled','disabled');
					$("#cbo_dealing_merchant").attr('disabled','disabled');
					...
					load_drop_down( 'requires/sweater_sample_requisition_v2_controller', '', 'load_drop_down_trims_group_from_budget_for_after_order', 'ra_trims_group_1');
					release_freezing();
				}
			}
		}
		...
		if(sample_stage==2)
		{
			...
			var page_link='requires/sweater_sample_requisition_v2_controller.php?action=inquiry_popup&company='+company;
			var title="Search  Popup";
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1080px,height=400px,center=1,resize=0,scrolling=0','../')
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0];
				var inquiryId=this.contentDoc.getElementById("txt_inquiry_id").value; // mrr number
				get_php_form_data(inquiryId, "populate_data_from_inquiry_search", "requires/sweater_sample_requisition_v2_controller");
				$("#cbo_company_name").attr('disabled','disabled');
				$("#cbo_buyer_name").attr('disabled','disabled');
				$("#cbo_company_name").attr('disabled','disabled');
				$("#cbo_dealing_merchant").attr('disabled','disabled');
	...
	function openmypage_requisition()
	{
		...
		var page_link = 'requires/sweater_sample_requisition_v2_controller.php?&action=requisition_id_popup&company_id='+$('#cbo_company_name').val();
		...
		emailwindow.onclose=function()
		{
			...
			{
				freeze_window(5);
				get_php_form_data(mst_tbl_id, "populate_data_from_requisition_search_popup", "requires/sweater_sample_requisition_v2_controller" );
 	...
 	function calculate_required_qty(type,inc)
 	{
 		if(type==1)
 		{
 			...
 			var qty =return_global_ajax_value( data, 'populate_data_to_req_qty', '', 'requires/sweater_sample_requisition_v2_controller') ;
 			var qty=qty.trim();
 	...
 	{
 		var result = return_global_ajax_value( data, 'load_data_to_uom', '', 'requires/sweater_sample_requisition_v2_controller');
 		var res=trim(result);
 	...
 	function load_data_to_rfcolor(inc)
 	{
 		...
 		var result = return_global_ajax_value( data, 'load_data_to_colorRF', '', 'requires/sweater_sample_requisition_v2_controller');
	...
	function fnc_show_acknowledge() {
     	show_list_view('', 'show_acknowledge', 'list_acknowledge', 'requires/sweater_sample_requisition_v2_controller', 'setFilterGrid(\'tbl_list_search\',-1);');
     }
...
function open_body_part_popup(i){
		...
		var page_link='requires/sweater_sample_requisition_v2_controller.php?action=body_part_popup';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, 'Fabric Description', 'width=1060px,height=450px,center=1,resize=1,scrolling=0','../../');
		emailwindow.onclose=function()
		{
	...
	function fn_show_calender() {
		...
		if(requisition_date!='' && company_name>0 && sample_team>0){
			show_list_view(company_name+'__'+requisition_date+'__'+sample_team, 'show_calender', 'list_acknowledge', 'requires/sweater_sample_requisition_v2_controller', '');
		}
    }
    function company_config(company)
    {
    	var sample_stage=$('#cbo_sample_stage').val();
    	if(sample_stage=='' || sample_stage!=2)
    	{
    		return;
    	}
    	else{
	    	var style_source = return_global_ajax_value(company, 'get_sample_style_source', '', 'requires/sweater_sample_requisition_v2_controller');
	...
	function fn_calculate_delivery_date(i){
		...
		if(date_compare( currentDate, txtDelvStartDate)==true){
			var addDays = return_global_ajax_value( $('#update_id').val()+'__0__'+txtDelvStartDate, 'update_delivery_date', '', 'requires/sweater_sample_requisition_v2_controller');
	...
	function auto_acknowledge(requisitionId){
			var dataString = return_global_ajax_value(requisitionId, 'get_sample_requisition', '', 'requires/sweater_sample_requisition_v2_controller');
...
function uploadFile(mst_id){
	$(document).ready(function() { 
		...
		$.ajax({ 
			url: 'requires/sweater_sample_requisition_v2_controller.php?action=file_upload&mst_id='+ mst_id, 
			type: 'post', 
			data: fd, 
			contentType: false, 
			processData: false, 
...
<body onLoad="set_hotkey(); btn_load_acknowledge();">
	<div style="width:100%;">
			...
			<fieldset style="width:1100px;">
				...
				<table cellpadding="2" cellspacing="2">
					...
					<tr>
						...
						<td width="150"><?=create_drop_down( "cbo_sample_stage", 150, $sample_stage, "", 1, "-- Select Stage --", $selected, "fnc_browse_style(this.value);", "", "" ); ?>
						</td>
						<td class="must_entry_caption" align="right">Company Name</td>
						<td><?=create_drop_down( "cbo_company_name", 150, "select comp.id,comp.company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/sweater_sample_requisition_v2_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/sweater_sample_requisition_v2_controller', this.value, 'load_drop_down_agent', 'agent_td' );load_drop_down( 'requires/sweater_sample_requisition_v2_controller', this.value, 'load_drop_down_location', 'location_td' );fn_show_calender(); company_config(this.value);" ); ?></td>
					...
					<tr>
						...
						<td><?=create_drop_down( "cbo_dealing_merchant", 150, "select id,team_member_name from lib_mkt_team_member_info where status_active =1 and is_deleted=0 order by team_member_name","id,team_member_name", 1, "-- Select Team Member --", $selected, "" ); ?></td>
                    ...
                    <tr>
						...
						<td><input name="txt_bom" class="text_boxes" id="txt_bom" style="width:140px" disabled=""></td>
                    	<td align="right">File</td>
						<td align="left"><input type="file" id="sample_mst_file" class="image_uploader" style="width:150px"></td>
                    	<td></td>
                        <td> <input type="button" id="image_button" class="image_uploader" style="width:150px" value="CLICK TO ADD FILE" onClick="file_uploader( '../../../', document.getElementById('update_id').value,'', 'sweater_sample_requisition_1', 2 ,1)" title="0" />
                        </td>
                   </tr>
					<tr>
					...
					<tr>
						<td colspan="6" height="40" valign="bottom" align="center" class="button_container">
							<?
							echo load_submit_buttons( $permission, "fnc_sweater_sample_requisition_mst_info", 0,1,"button_status(1)",1);
							?>
		...
		<h3 align="left" class="accordion_h" onClick="show_hide_content('sample_details', '');fnc_load_tr(document.getElementById('update_id').value,1);hide_others_section('sd');" style="width:1250px;"> +Sample Details </h3>
		<div id="content_sample_details" style="display:none;">
			<form name="sample_details_1" id="sample_details_1">
				<fieldset style="width:1250px;" id="sample_dtls">
					<table width="100%" cellpadding="0" cellspacing="2" align="center" style="">
						<tr>
							<td align="center" valign="top" id="po_list_views">
								...
								<table cellpadding="0" cellspacing="0" width="1250" class="rpt_table" border="1" rules="all" id="tbl_sample_details">
									<thead>
									...
									<tbody id="sample_details_container">
										<tr id="tr_1" style="height:10px;" class="general">
											...
											<td align="center" id="color_1">
												<input style="width:80px;" type="text" class="text_boxes"  name="txtColor_1" id="txtColor_1" placeholder="Write/Browse" onDblClick="openmypage_color_size('requires/sweater_sample_requisition_v2_controller.php?action=color_popup','Color Search','1','1');"/>
											</td>
											<td>
											<input style="width:60px;" type="text" class="text_boxes"  name="txtcolorcombono_1" id="txtcolorcombono_1" placeholder="write" value="" />
											</td>

											<td align="center" id="sample_prod_qty_1">
												<input style="width:100px;" type="text" class="text_boxes_numeric"  name="txtSampleProdQty_1" id="txtSampleProdQty_1"  readonly placeholder="Browse" onDblClick="openmypage_sizeinfo('requires/sweater_sample_requisition_v2_controller.php?action=sizeinfo_popup','Size Search','1')" /><input type="hidden" class="text_boxes"  name="txtAllData_1" id="txtAllData_1"/>
								...
								<table style="margin-top: 5px;">
									<tr>
										<td colspan="14" height="40" valign="bottom" align="center" class="">
											...
											<input type="button" value="Acknowledgment" name="Acknowledgment" onClick="fnc_acknowledgment()" style="width:100px;" id="acknowledgment"   class="formbutton"/>
                                        ...
                                        <tr  id="tr_1" style="height:10px;" class="general">
                                            ...
                                            <td align="center">
                                             ...
                                             <td align="center" id="rf_color_1">
                                                <input style="width:60px;" type="text" class="text_boxes"  name="txtRfColor_1" id="txtRfColor_1" placeholder="Write/Browse" onDblClick="openmypage_rf_color('requires/sweater_sample_requisition_v2_controller.php?action=color_popup_rf','Color Search','1');" readonly />
                                             </td>
