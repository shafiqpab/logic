...
<script>
  	...
  	function fnc_sweater_sample_requisition_mst_info( operation )
	{
		...
		if(operation==4)
		{
			 print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val(), "sweater_sample_requisition_print1", "requires/sweater_sample_requisition_controller" );
			 return;
		}
	   if(season_mandatory_arr[comp]==1)
	   {
	...
	function fnc_sample_details_info( operation )
	{
		...
		else
		{
			...
			for (var i=1; i<=row_num; i++)
			{
				...
				data_all=data_all+get_submitted_data_string('cboSampleName_'+i+'*cboGarmentItem_'+i+'*txtSmv_'+i+'*txtweight_'+i+'*txtArticle_'+i+'*txtColor_'+i+'*txtSampleProdQty_'+i+'*txtSubmissionQty_'+i+'*txtDelvStartDate_'+i+'*txtDelvEndDate_'+i+'*txtChargeUnit_'+i+'*cboCurrency_'+i+'*updateidsampledtl_'+i+'*txtAllData_'+i,"../../../");
			}
