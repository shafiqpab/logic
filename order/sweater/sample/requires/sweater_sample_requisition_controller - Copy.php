...
if($action=="sweater_sample_requisition_print1")
{
    extract($_REQUEST);
	list($company_name,$update_id)=explode('*',$data);
	$company_library=return_library_array( "select id, company_name from lib_company where id=$company_name", "id", "company_name"  );
    ...
    <script>
		...
		function total_submission_qty(inc)
		{
			...
			for(var i=1; i<=tot_row; i++)
			{
				...
				var total= bhQty + plQty + dyQty + testQty + selfQty + targetWeight;
		...
		function calculate_total_qnty_by_type()
		{
			...
			document.getElementById('txt_total_target_weight').value=total_targetweight;
        ...
        <form name="size_1" id="size_1">
					...
					if($count_tr>0)
					{
						...
						?>
							<tr id="row_<? echo $i; ?>" >
							   ...
							   <td><input name="txttargetweight[]" class="text_boxes_numeric" ID="txttargetweight_<? echo $i; ?>" style="width:70px"  onBlur="calculate_total_qnty_by_type();" value="<? echo $target_weight; ?>"/></td>
            ...
            <table align="center" cellspacing="0" width="770" class="rpt_table" border="1" rules="all" id="" >
				<tr>
					...
					<td width="70" align="center"><Input name="txt_total_target_weight" class="text_boxes_numeric" ID="txt_total_target_weight" style="width:70px" value="<? echo $total_targetweight; ?>" readonly /></td>
...
if ($action=="save_update_delete_mst")
{
   $process = array( &$_POST );
 	...
 	if ($operation==0)  // Insert Here
	{
		...
		$data_array="(".$id_mst.",'".$new_system_id[1]."',".$new_system_id[2].",'".$new_system_id[0]."',".$cbo_sample_stage.",".$txt_requisition_date.",".$txt_quotation_id.",".$txt_style_name.",".$cbo_company_name.",".$cbo_location_name.",".$cbo_buyer_name.",".$cbo_season_name.",".$cbo_product_department.",".$cbo_dealing_merchant.",".$cbo_agent.",".$txt_buyer_ref.",".$txt_est_ship_date.",".$txt_remarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,341,0,".$cbo_ready_to_approved.",".$txt_material_dlvry_date.",".$cbo_sample_team.",".$cbo_season_year.",".$cbo_brand_id.")";
...
if ($action=="save_update_delete_sample_details")
{
	...
	if ($operation==0)  // Insert Here
	{
			...
			if($db_type==0)
			{
 			...
 			$field_array= "id,sample_mst_id,sample_name,gmts_item_id,smv,article_no,sample_color,sample_prod_qty,submission_qty,delv_start_date,delv_end_date,sample_charge,sample_curency,weight,inserted_by,insert_date,status_active,is_deleted,entry_form_id,size_data,fabric_status,acc_status,embellishment_status,fab_status_id,acc_status_id,embellishment_status_id";
			...
			for ($i=1;$i<=$total_row;$i++)
		    {
				...
				if ($i!=1) $data_array .=",";
				$data_array .="(".$id_dtls.",".$update_id.",".$$cboSampleName.",".$$cboGarmentItem.",".$$txtSmv.",".$$txtArticle.",'".$color_id."',".$$txtSampleProdQty.",".$$txtSubmissionQty.",".$$txtDelvStartDate.",".$$txtDelvEndDate.",".$$txtChargeUnit.",".$$cboCurrency.",".$$txtweight.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','1',0,341,".$$txtAllData.",0,0,0,0,0,0)";
	...
	else if ($operation==1)  // Update Here
	{
			...
			$field_array_up="sample_name*gmts_item_id*smv*article_no*sample_color*sample_prod_qty*submission_qty*delv_start_date*delv_end_date*sample_charge*sample_curency*weight*updated_by*update_date*size_data";
			...
			for ($i=1;$i<=$total_row;$i++)
		    {
				...
				if (str_replace("'",'',$$updateIdDtls)!="")
				{
					...
					$data_array_up[str_replace("'",'',$$updateIdDtls)] =explode("*",("".$$cboSampleName."*".$$cboGarmentItem."*".$$txtSmv."*".$$txtArticle."*'".$color_id."'*".$$txtSampleProdQty."*".$$txtSubmissionQty."*".$$txtDelvStartDate."*".$$txtDelvEndDate."*".$$txtChargeUnit."*".$$cboCurrency."*".$$txtweight."*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'*".$$txtAllData.""));
...
if ($action=="save_update_delete_required_fabric")
{
	...
	if ($operation==0)  // Insert Here
	{
			...
			$field_array_col="id, mst_id, dtls_id,color_id,fabric_color,qnty,count,composition,type,yarn_source,lot,store_id,comments,supplier_id,inserted_by, insert_date, status_active, is_deleted";
			...
			for ($i=1;$i<=$total_row;$i++)
		    {
				...
				$ex_data="";
 				...
 				$add_comm="";
  				foreach($ex_data as $color_data) 
				{
					list($txtSL,$txtColor,$hiddenColorId,$txtYarnColor,$txtCount,$txtComposition,$cboType,$cboYarnSource,$txtGreyQnty,$cboUom,$txtLot,$cboStore,$txtComments,$supplier_id,$yarn_color_id)=explode('__',$color_data);

 					 if($add_comm) $add_comm.=","; else $add_comm.="";
  					$data_array_col.="$add_comm(".$idColorTbl.",".$update_id.",".$id_dtls.",'".$hiddenColorId."','".$yarn_color_id."','".$txtGreyQnty."','".$txtCount."','".$txtComposition."','".$cboType."','".$cboYarnSource."','".$txtLot."',".$cboStore.",'".$txtComments."',".$supplier_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
	...
	else if ($operation==1)  // Update Here
	{
			...
			for ($i=1;$i<=$total_row;$i++)
		    {
			 	...
			 	else
				{
					...
					{
						list($txtSL,$txtColor,$hiddenColorId,$txtYarnColor,$txtCount,$txtComposition,$cboType,$cboYarnSource,$txtGreyQnty,$cboUom,$txtLot,$cboStore,$txtComments,$supplier_id,$yarn_color_id)=explode('__',$color_datas);
						if($kk==1) $add_comma=""; else $add_comma=",";
						$data_array_cols.="$add_comma(".$idColorTbl.",".$update_id.",".$id_dtls.",'".$hiddenColorId."','".$yarn_color_id."','".$txtGreyQnty."','".$txtCount."','".$txtComposition."','".$cboType."','".$cboYarnSource."','".$txtLot."',".$cboStore.",'".$txtComments."',".$supplier_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		...
		if(count($sql_result)>0)
		{
			foreach($sql_result as $row)
			{
				...
				<tr id="tr_<? echo $i; ?>" style="height:10px;" class="general">
                    ...
                    <td>
						<input style="width:70px;" type="text" class="text_boxes_numeric" name="txtweight_<? echo $i; ?>" id="txtweight_<? echo $i; ?>" value="<? echo $row[csf("weight")]; ?>"/>
	...
	$size_arr=return_library_array( "select id,size_name from lib_size", "id", "size_name" );
  	...
  	if ($operation==5)  // Insert Here
  	{
		...
		//$new_system_id=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), 'SR', date("Y",time()), 5, "select requisition_number_prefix, requisition_number_prefix_num from sample_development_mst where entry_form_id=341 and company_id=$cbo_company_name  and $year_cond=".date('Y',time())." order by requisition_number_prefix_num desc ", "requisition_number_prefix", "requisition_number_prefix_num" ));
  		...
  		$data_array="(".$id_mst.",'".$new_system_id[1]."',".$new_system_id[2].",'".$new_system_id[0]."',".$cbo_sample_stage.",".$txt_requisition_date.",".$txt_quotation_id.",".$txt_style_name.",".$cbo_company_name.",".$cbo_location_name.",".$cbo_buyer_name.",".$cbo_season_name.",".$cbo_product_department.",".$cbo_dealing_merchant.",".$cbo_agent.",".$txt_buyer_ref.",".$txt_est_ship_date.",".$txt_remarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,341,1,'2',".$txt_material_dlvry_date.",".$cbo_sample_team.")";
...
?>
    ...
    <script>
		...
		function fnc_close( )
		{
			...
			for(var i=1; i<=rowCount; i++)
			{
				...
				if(breck_down_data=="")
				{
					breck_down_data+=($('#txtSL_'+i).val()*1)+'__'+$('#txtColor_'+i).val()+'__'+$('#hiddenColorId_'+i).val()*1+'__'+$('#txtYarnColor_'+i).val()+'__'+$('#txtCount_'+i).val()+'__'+$('#txtComposition_'+i).val()+'__'+$('#cboType_'+i).val()*1+'__'+$('#cboYarnSource_'+i).val()*1+'__'+$('#txtGreyQnty_'+i).val()*1+'__'+$('#cboUom_'+i).val()*1+'__'+$('#txtLot_'+i).val()+'__'+$('#cboYarnStore_'+i).val()+'__'+$('#txtComments_'+i).val()+'__'+$('#cboSupplier_'+i).val();
					  display_col +=$('#txtColor_'+i).val() ;
				}
				else  
				{
    ...
    <body>
        ...
        <form name="size_1" id="size_1">
            ...
            <table align="center" cellspacing="0" width="1080" class="rpt_table" border="1" rules="all" id="col_tbl" >
            	<thead>
                ...
                <tbody>
                	...
                	if($data)
                	{
                		...
                		if($count_tr>0)
                		{
							...
							list($txtSL,$txtColor,$hiddenColorId,$txtYarnColor,$txtCount,$txtComposition,$cboType,$cboYarnSource,$txtGreyQnty,$cboUom,$txtLot,$cboStore,$txtComments,$supplier_id)=explode('__',$size_data);
							//echo $supplier_id.'<br>';
            ...
            <table align="center" cellspacing="0" width="1080" class="rpt_table" border="1" rules="all" id="" >
...
if ($action=="update_delivery_date")
{
	...
	else
	{
		...
		//echo bulk_update_sql_statement("sample_development_dtls", "id",$field_array_up,$data_array_up,$id_arr );die;
