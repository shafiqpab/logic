...
$req_library=return_library_array( "select id, requisition_number_prefix_num from sample_development_mst where status_active=1 and is_deleted=0 and entry_form_id in (341,459)",'id','requisition_number_prefix_num');

$delivery_sql=sql_select("select sample_dtls_part_tbl_id,sample_name,gmts_item_id,sum(ex_factory_qty) as qc_pass_qty  from  sample_ex_factory_dtls where entry_form_id=544 and status_active=1 and is_deleted=0  group by sample_dtls_part_tbl_id,sample_name,gmts_item_id ");
...
$sample_dtls_sql=sql_select("select id,sample_name,gmts_item_id,sample_prod_qty from  sample_development_dtls where entry_form_id in (341,459) and status_active=1 and is_deleted=0  group by id,sample_name,gmts_item_id,sample_prod_qty order by id ");
...
if($action=="populate_data_yet_to_cut")
{
	...
	$val=return_field_value("sum(ex_factory_qty)","sample_ex_factory_dtls","sample_ex_factory_mst_id=$ex_fac_mst and sample_dtls_part_tbl_id=$smp_tbl_id and sample_name=$sample_name and gmts_item_id=$gmts and entry_form_id=544 and sample_development_id=$req_id");
	echo $val;
	exit();
}
        ...
        <tr class="general">
            <td width="140">
                ...
                <? $is_disabled=0;
                    ...
                    echo create_drop_down( "cbo_company_mst", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", $company,"load_drop_down( 'sample_delivery_entry_buyer_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );",$is_disabled );
                ?>
            </td>
...
if($action=="create_po_search_list_view")
{
	...
	$arr=array (1=>$buyer_arr,4=>$dealing_marchant,5=>$requistion_qnty,6=>$output_qnty,7=>$bal_arr);

 	 $sql= "select id, requisition_number_prefix_num, company_id, buyer_name, style_ref_no,dealing_marchant from sample_development_mst where  entry_form_id in (341,459)  and status_active=1 and is_deleted=0  $ref_cond $company $buyer $style_id_cond $style_cond  $style_id_in_cond  $estimated_shipdate order by id asc";

	echo  create_list_view("list_view", "Req No,Buyer Name,Style Name,Dealing Merchant,Req. Qty., Delivery. Qty., Bal Qty.", "60,140,130,120,80,80,80","800","240",0, $sql , "js_set_value", "id", "", 1, "0,buyer_name,0,0,dealing_marchant,id,id,id", $arr, "requisition_number_prefix_num,buyer_name,style_ref_no,dealing_marchant,id,id,id", "",'',"0,0,0,0,0,0,0") ;
...
if($action=="populate_data_from_search_popup")
{
	...
	if($type==1)
	{
		$res = sql_select("select requisition_number_prefix_num,id,company_id,location_id,sample_stage_id,buyer_name,style_ref_no,item_name from sample_development_mst where entry_form_id in (341,459) and id=$data  and status_active=1 and is_deleted=0");

	  	foreach($res as $result)
		{
			echo "$('#cbo_company_name').val('".$result[csf('company_id')]."');\n";
		    echo "load_drop_down( 'requires/sample_delivery_entry_buyer_controller', '".$result[csf("company_id")]."', 'load_drop_down_location', 'location_td' );\n";
	...
	else
	{
	    ...
	    $smp_mst_id = sql_select("SELECT location, id,sys_number_prefix,sys_number_prefix_num,sys_number, company_id, location, delivery_to, ex_factory_date, gp_no, final_destination,received_by from sample_ex_factory_mst where id='$challan_id' and entry_form_id=544 and status_active=1 and is_deleted=0");
...
if($action=="color_and_size_level")
{
 	...
 	if(trim($is_exists_wash_dyeing)=='' && $val_req_embel=='')
	{
		...
		$total_cuml=return_field_value("sum(ex_factory_qty)","sample_ex_factory_dtls","sample_name=$smp_id and gmts_item_id=$gmts and status_active=1 and is_deleted=0 and entry_form_id=544 and sample_dtls_part_tbl_id=$sample_dtls_part_tbl_id");
	...
	foreach($colorResult_qc_pass as $row)
	{
		...
		echo "$('#txt_remark').val('');\n";
  		...
  		$value=return_field_value("gmts_item_id","sample_development_dtls","entry_form_id in (341,459) and sample_mst_id=$req_id and id=$sample_dtls_part_tbl_id and status_active=1 and is_deleted=0");
...
$i = 1;
$kk = 1;
$sqlResult = sql_select("SELECT id,sample_ex_factory_mst_id, sample_development_id, sample_name,gmts_item_id, ex_factory_qty, carton_qty,carton_per_qty, remarks, shiping_status,sample_dtls_part_tbl_id from sample_ex_factory_dtls where $sql_con and status_active=1 and is_deleted=0 and entry_form_id=544");
foreach ($sqlResult as $row) {
	$bgcolor = ($i % 2 == 0) ? "#E9F3FF" : "#FFFFFF";
	$click_var = "onclick=get_php_form_data('" . $row[csf('sample_development_id')] . '**' . $row[csf('sample_ex_factory_mst_id')] . '**' . $row[csf('id')] . '**' . $row[csf('sample_name')] . '**' . $row[csf('gmts_item_id')] . '**' . $row[csf('sample_dtls_part_tbl_id')] . "'" . ",'populate_input_form_data','requires/sample_delivery_entry_buyer_controller');";
	...
	foreach($colorResult as $row)
	{
		...
		echo "$('#cbo_item_name').val('".$dtlsArr[$dtls_id]['gmts_item_id']."');\n";
 		...
 		if($is_exist_emb=='' && $val_req_embel=='')
 		{
 			...
 			$total_cuml=return_field_value("sum(ex_factory_qty)","sample_ex_factory_dtls","sample_name=$sample_name and gmts_item_id=$gmts and status_active=1 and is_deleted=0 and entry_form_id=544 and sample_dtls_part_tbl_id='$sample_dtls_row_id' ");
 		}
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert part----------------------------------------------------------
	{
		...
		if($mst_update_id=='')
		{
			...
			$new_mrr_number=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'GDS', date("Y",time()), 5, "select a.sys_number_prefix,a.sys_number_prefix_num from sample_ex_factory_mst a where a.entry_form_id=544 and a.company_id=$cbo_company_name and $mrr_date_check =".date('Y',time())." order by a.id DESC", "sys_number_prefix", "sys_number_prefix_num" ));

			$mst_id=return_next_id("id", "sample_ex_factory_mst", 1);
			$field_array_mst="id,sys_number_prefix,sys_number_prefix_num,sys_number, company_id, location, delivery_to,  gp_no, final_destination,received_by,sent_by, inserted_by, insert_date, status_active, is_deleted,entry_form_id";
			$data_array_mst="(".$mst_id.",'".$new_mrr_number[1]."','".$new_mrr_number[2]."','".$new_mrr_number[0]."',".$cbo_company_name.",".$cbo_location_name.",".$cbo_delivery_to.",".$txt_gp_no.",".$txt_final_destination.",".$txt_received_by.",".$txt_sent_by.",".$user_id.",'".$pc_date_time."','1','0','544')";
 		 // Details part--------------------------------------------------------------;
			...
			$data_array_dtls="(".$dtls_id.",".$mst_id.",".$hidden_requisition_id.",".$hidden_sample_dtls_tbl_id.",".$cbo_sample_name.",".$cbo_item_name.",".$txt_delivery_date.",".$txt_delivery_qty.",".$txt_carton_qnty.",".$txt_remark.",".$cbo_shipping_status.",".$user_id.",'".$pc_date_time."','1','0','544')";
		 ...
		 $colorsize_brk_id=return_next_id("id", "sample_ex_factory_colorsize", 1);
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				if($j==0)$data_array_brk = "(".$colorsize_brk_id.",".$mst_id.",".$dtls_id.",'".$colorID."','".$sizeID."','".$colorSizeValue."',".$user_id.",'".$pc_date_time."','1','0','544')";
				else $data_array_brk .= ",(".$colorsize_brk_id.",".$mst_id.",".$dtls_id.",'".$colorID."','".$sizeID."','".$colorSizeValue."',".$user_id.",'".$pc_date_time."','1','0','544')";
				$colorsize_brk_id+=1;
				$j++;
			}
		...
		else
		{
			...
			if(count($previous_sampl_data)>0 && $shipping_flag==1){
 			...
 			$dtls_id=return_next_id("id", "sample_ex_factory_dtls", 1);
			$field_array_dtls="id, sample_ex_factory_mst_id, sample_development_id,sample_dtls_part_tbl_id, sample_name,gmts_item_id,delivery_date, ex_factory_qty, carton_qty, remarks, shiping_status, inserted_by, insert_date, status_active, is_deleted,entry_form_id";
			$data_array_dtls="(".$dtls_id.",".$mst_update_id.",".$hidden_requisition_id.",".$hidden_sample_dtls_tbl_id.",".$cbo_sample_name.",".$cbo_item_name.",".$txt_delivery_date.",".$txt_delivery_qty.",".$txt_carton_qnty.",".$txt_remark.",".$cbo_shipping_status.",".$user_id.",'".$pc_date_time."','1','0','544')";
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				if($j==0)$data_array_brk = "(".$colorsize_brk_id.",".$mst_update_id.",".$dtls_id.",'".$colorID."','".$sizeID."','".$colorSizeValue."',".$user_id.",'".$pc_date_time."','1','0','544')";
				else $data_array_brk .= ",(".$colorsize_brk_id.",".$mst_update_id.",".$dtls_id.",'".$colorID."','".$sizeID."','".$colorSizeValue."',".$user_id.",'".$pc_date_time."','1','0','544')";
				$colorsize_brk_id+=1;
				$j++;
			}
  	...
  	else if ($operation==1) // Update part ------------------------------------------------------
	{
		...
		$dtls_update_id=str_replace("'","",$dtls_update_id);
		$shipping_flag = 0;
		$previous_sampl_data = sql_select("SELECT a.sample_development_id, a.sample_name, a.gmts_item_id, a.ex_factory_qty ,b.color_id FROM sample_ex_factory_dtls a JOIN sample_ex_factory_colorsize b ON a.id=b.sample_ex_factory_dtls_id where a.sample_development_id = $hidden_requisition_id and sample_name = $cbo_sample_name and gmts_item_id=$cbo_item_name and a.id <> $dtls_update_id");
		$previous_sample_qty = array();
		foreach ($previous_sampl_data as $value) {
			$previous_sample_qty[$value[csf('sample_development_id')]][$value[csf('sample_name')]][$value[csf('gmts_item_id')]][$value[csf('color_id')]] += $value[csf('ex_factory_qty')];
		}
		...
		{
			// master part--------------------------------------------------------------;
		   $field_array_mst="delivery_to*gp_no*final_destination*received_by*sent_by*updated_by*update_date";
			$data_array_mst="".$cbo_delivery_to."*".$txt_gp_no."*".$txt_final_destination."*".$txt_received_by."*".$txt_sent_by."*".$user_id."*'".$pc_date_time."'";
		...
		$colorsize_brk_id=return_next_id("id", "sample_ex_factory_colorsize", 1);
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				if($j==0)$data_array_brk = "(".$colorsize_brk_id.",".$mst_update_id.",".$dtls_update_id.",'".$colorID."','".$sizeID."','".$colorSizeValue."',".$user_id.",'".$pc_date_time."','1','0',544)";
				else $data_array_brk .= ",(".$colorsize_brk_id.",".$mst_update_id.",".$dtls_update_id.",'".$colorID."','".$sizeID."','".$colorSizeValue."',".$user_id.",'".$pc_date_time."','1','0',544)";
				$colorsize_brk_id+=1;
				$j++;
			}
...
$sql_dev= sql_select("select b.sample_ex_factory_mst_id,b.sample_development_id,a.buyer_name from sample_ex_factory_dtls b,sample_development_mst a where a.id=b.sample_development_id and b.status_active=1");
	...
	$string_search_type=str_replace("'","",$ex_data[6]);
	
	$date_cond="";
 	if($txt_date_from!="" and  $txt_date_to!="")
	{
		if($db_type==0){$date_cond  = " select sample_ex_factory_mst_id from sample_ex_factory_dtls where delivery_date between '".change_date_format($txt_date_from,'yyyy-mm-dd')."' and '".change_date_format($txt_date_to,'yyyy-mm-dd')."'";}
		if($db_type==2 || $db_type==1){ $date_cond= " and id in(select sample_ex_factory_mst_id from sample_ex_factory_dtls where delivery_date between '".date("j-M-Y",strtotime($txt_date_from))."' and '".date("j-M-Y",strtotime($txt_date_to)) ."')";}

	}
	if(trim($company)!=0) {$com_cond= " and company_id= '$company'";} else {$com_cond="";}
	if(trim($cbo_delivery_to)!=0) {$delv_cond= " and delivery_to= '$cbo_delivery_to'";} else {$delv_cond="";}

	$challan_cond="";
		
	if($string_search_type==1)
	{
		if(trim($challan)!="") {$challan_cond= " and sys_number_prefix_num= '$challan'";} else {$challan_cond="";}
	}
	else if($string_search_type==2)
	{
		if(trim($challan)!="") {$challan_cond= " and sys_number_prefix_num like '$challan%'";} else {$challan_cond="";}
	}
	...
	if(trim($req_no)!="") {$req_cond= " and id in (  select sample_ex_factory_mst_id from sample_ex_factory_dtls where sample_development_id in(select id from sample_development_mst where requisition_number_prefix_num ='$req_no')) ";} else {$req_cond="";}

	$sql = "select id,sys_number, company_id, location, delivery_to,  gp_no, final_destination, received_by from  sample_ex_factory_mst where status_active=1 and is_deleted=0 $com_cond $delv_cond $challan_cond $date_cond $req_cond and entry_form_id=544 order by id desc";
	//echo $sql;die;
...
if($action=="delivery_print")
{
    ...
    $mst_data=sql_select("select * from sample_ex_factory_mst where id=$mst_id and status_active=1 and entry_form_id=544");
    $dtls_data=sql_select("select delivery_date from sample_ex_factory_dtls where sample_ex_factory_mst_id=$mst_id and status_active=1 and entry_form_id=544");
    	$req_array=array();
	$req_sql=sql_select("select * from sample_development_mst where is_deleted=0 and status_active=1 and entry_form_id in (341,459) and id in($req_id)");
 	foreach($req_sql as $row)
	{
		...
		$sql="SELECT * from sample_ex_factory_dtls where sample_ex_factory_mst_id=$mst_id  and status_active=1 and is_deleted=0 and entry_form_id=544";
		$result=sql_select($sql);
		 foreach($result as $row){
			$data_arr[]=array(
	...
	$result_smp=sql_select("SELECT b.sample_name,a.buyer_name,a.style_ref_no,b.gmts_item_id from sample_development_mst a,sample_development_dtls b where a.company_id=$company_name and a.entry_form_id in (341,459) and b.entry_form_id in (341,459) and a.id in($req_id) group by a.buyer_name,a.style_ref_no,b.gmts_item_id,b.sample_name");
