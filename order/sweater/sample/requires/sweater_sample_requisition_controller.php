...
if ($action=="sample_dealing_merchant")
{
...
if($action=="populate_data_to_req_qty")
{
	...
	if($data[4]==1)
	{
		...
		foreach($color_name as $vals)
		{
			...
			if($col_id=="")
			{
				$col_sql="select id,color_name from lib_color where status_active=1 and is_deleted=0 and  color_name='$vals' and id in(select sample_color from sample_development_dtls where status_active=1 and is_deleted=0 and entry_form_id=341 and sample_mst_id='$data[2]')";
				$col_arr=sql_select($col_sql);
				$col_id=$col_arr[0][csf("id")];
			}
			else
			{
				$col_arr=sql_select("select id,color_name from lib_color where status_active=1 and is_deleted=0 and  color_name='$vals' and id in(select sample_color from sample_development_dtls where status_active=1 and is_deleted=0 and entry_form_id=341 and sample_mst_id='$data[2]')");
				$col_id.=','.$col_arr[0][csf("id")];
			}
		}
		$value=return_field_value("sum(sample_prod_qty)","sample_development_dtls","entry_form_id=341 and sample_name=$data[0] and gmts_item_id=$data[1] and sample_mst_id=$data[2] and status_active=1 and is_deleted=0 and sample_color in($col_id) ");
	}
	else
	{
		$value=return_field_value("sum(sample_prod_qty)","sample_development_dtls","entry_form_id=341 and sample_name=$data[0] and gmts_item_id=$data[1] and sample_mst_id=$data[2] and status_active=1 and is_deleted=0 ");
	}
	echo trim($value);
	exit();
}
...
{
	echo create_drop_down( "cbo_brand_id", 150, "select id, brand_name from lib_buyer_brand where buyer_id='$data' and status_active =1 and is_deleted=0 order by brand_name ASC","id,brand_name", 1, "-Brand-", $selected, "" );
	exit();
}*/
...
if($action=="load_data_to_colorRF")
{
    ...
    $sql=sql_select("select distinct(sample_color) from sample_development_dtls where entry_form_id=341 and sample_mst_id=$data[2] and sample_name=$data[0] and gmts_item_id=$data[1] and is_deleted=0  and status_active=1");
	if(count($sql)==1)
	{
...
{
	$data=explode("***",$data);
	$sql=sql_select("select sample_color from sample_development_dtls where entry_form_id=341 and sample_name=$data[0] and gmts_item_id=$data[1] and sample_mst_id=$data[2] and status_active=1 and is_deleted=0 ");
	$color="";
	$i=1;
	foreach($sql as $row)
	{
...
if ($action=="load_drop_down_required_fabric_gmts_item")
{
	$data=explode("_", trim($data));
 	$sql=sql_select("select id,gmts_item_id from sample_development_dtls where is_deleted=0  and status_active=1 and entry_form_id=341 and sample_mst_id='$data[0]'");
...
{
	$data=explode("_", $data);
	$sql="select a.id,a.sample_name,b.id as dtls_id from  lib_sample a ,sample_development_dtls b where  a.status_active=1 and b.status_active=1 and a.is_deleted=0 and b.is_deleted=0 and a.id=b.sample_name and b.entry_form_id=341 and b.sample_mst_id='$data[0]' group by a.id,a.sample_name,b.id order by b.id";
	$samp_array=array();
...
if($action=="fabric_description_popup")
{
	...
	<script>
		function js_set_value(data)
		{
			...
			var fabric_yarn_description=return_global_ajax_value(data[0], 'fabric_yarn_description', '', 'sweater_sample_requisition_controller');
    ...
    <div align="center">
		...
		<fieldset>
            <table cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
            	<thead>
                	<tr>
                    	<th colspan="3" align="center"><?=create_drop_down( "cbo_string_search_type", 100, $string_search_type,'', 1, "-- Searching Type --",4 ); ?></th>
                    </tr>
                	<tr>
                        <th>Construction</th>
                        <th>GSM/Weight</th>
...
if($action=="fabric_description_popup_search_list_view")
{
	...
	?>
		<div align="center">
			...
			$sql="select  a.id, a.fab_nature_id, a.construction, a.gsm_weight, a.color_range_id, a.stich_length, a.process_loss, b.copmposition_id, b.percent, b.count_id, b.type_id from lib_yarn_count_determina_mst a, lib_yarn_count_determina_dtls b where a.id=b.mst_id and  a.is_deleted=0 and a.fab_nature_id=$fabric_nature order by b.id";
			...
			if (count($data_array)>0)
			{
				foreach( $data_array as $row )
				{
					...
					$other_data_arr[$row[csf('id')]]=$row[csf('fab_nature_id')]."__".$row[csf('construction')]."__".$row[csf('gsm_weight')]."__".$row[csf('process_loss')]."__".$row[csf('color_range_id')]."__".$row[csf('stich_length')];
				}
			}
			unset($data_array);
    ...
    <body>
        ...
        <form name="size_1" id="size_1">
            ...
            <table align="center" cellspacing="0" width="400" class="rpt_table" border="1" rules="all" id="col_tbl" >
                ...
                <?
				$res_ack = sql_select("SELECT  a.is_acknowledge from sample_development_mst a,sample_development_dtls b where a.id=b.sample_mst_id and sample_mst_id=$mainId and a.entry_form_id=341 and a.is_deleted=0 and a.status_active=1 group by a.is_acknowledge");
				...
				else
				{
					...
					else
					{
						...
						$sql_col="select id,sample_color from sample_development_dtls where entry_form_id=341 and sample_mst_id=$mainId and sample_name=$sampleName and gmts_item_id=$garmentItem and is_deleted=0  and status_active=1 order by id ASC";
...
if($action=="print_button_variable_setting")
{
    ...
    $print_report_format=return_field_value("format_id","lib_report_template","template_name ='".$data."' and module_id=2 and report_id=209 and is_deleted=0 and status_active=1");
   	$printButton=explode(',',$print_report_format);

	foreach($printButton as $id){
		if($id==86)$buttonHtml.='<input type="button" style="width:80px;" id="print"  onClick="fnc_sweater_sample_requisition_mst_info(4);" class="formbutton printReport" name="print" value="Print" />';
		if($id==116)$buttonHtml.='<input id="print2" class="formbutton printReport" type="button" style="width:80px" onclick="fnc_sweater_sample_requisition_mst_info(5);" name="print2" value="Print 2">';
	}
...
<div id="mstDiv">
     ...
     <tr>
        ...
        <tr>
            <td colspan="9" align="center"><strong>Sample Requisition</strong></td>
				<?
                    $is_app=return_field_value("is_approved","sample_development_mst","entry_form_id=341 and id=$update_id and status_active=1 and is_deleted=0");
    ...
    <table cellspacing="0" border="1" class="rpt_table" rules="all" width="800">
		...
		foreach($size_select as $row)
		{
		 ...
		 $sql_qry="select id,sample_mst_id,sample_name,gmts_item_id,smv,article_no,weight,sample_color,sample_prod_qty,submission_qty,delv_start_date,delv_end_date,sample_charge,sample_curency from sample_development_dtls where status_active =1 and is_deleted=0 and entry_form_id=341 and sample_mst_id=$update_id and  status_active =1 and is_deleted=0 order by id asc";
			
		$result=sql_select($sql_qry);
		$i=1;$totalSizeQty=0;$totalSubmissionQty=$totalweight=0;
		foreach($result as $row)
		{
            $rowspan=count($sizeDataArr[$row[csf('id')]]);
			$totalSubmissionQty+=$bhQtyArr[$row[csf('id')]];
			 
			$totalweight+=$row[csf('weight')];
			?>
            <tr>
                ...
                <? 
					...
					<td rowspan="<? echo $rowspan;?>" align="right"><? echo $bhQtyArr[$row[csf('id')]];?></td>
                     <td rowspan="<? echo $rowspan;?>"><? echo $row[csf('weight')];?></td>
        ...
        <tfoot>
            <tr>
                ...
                <td  align="right"><b><? echo number_format($totalweight,2);?> </b></td>
                <td colspan="3"></td>
            </tr>
        </tfoot>
   </table>
    <br>  
  	<table cellspacing="0" border="1" class="rpt_table" rules="all" width="860">
        <thead>
            <tr> 
...
ORDER BY a.id ASC";
			...
			foreach($color_data_arr as $dataArr){
				list($txtSL,$txtColor,$hiddenColorId,$txtYarnColor,$txtCount,$txtComposition,$cboType,$cboYarnSource,$txtGreyQnty,$cboUom,$txtLot,$cboStore,$text_comments,$supplier_id,$yarn_color_id)=explode('__',$dataArr);
				...
				<tr>
                    ...
                    <td align="center"><? echo $supplier_library[$supplier_id];?></td>
    ...
    <br>  
 	...
 	<? echo signature_table(341, $data[0], "810px");?>    
...
if($action=="sweater_sample_requisition_print2")
{
    extract($_REQUEST);
	list($company_name,$update_id)=explode('*',$data);
	$company_library=return_library_array( "select id, company_name from lib_company where id=$company_name", "id", "company_name"  );
	...
	<div id="mstDiv">
		...
		<table cellspacing="0" cellpadding="5" border="0" width="1000">
			...
			<tr>
				<td colspan="10" align="center" style="font-size: 24px;"><strong><? echo $company_library[$company_name]; ?></strong></td>
			</tr>
			<tr>
			...
			<tr>
				<td width="100" valign="top"><strong>Requisition No</strong>
				<td width="100" valign="top">&nbsp;:&nbsp;<? echo $dataArray[0][csf("requisition_number")]; ?></td> 
				<td width="100" valign="top"><strong>Req. Date</strong>
				<td width="100" valign="top">&nbsp;:&nbsp;<? echo change_date_format($dataArray[0][csf("requisition_date")],"dd-mm-yyyy"); ?></td>
				<td width="100" valign="top"><strong>Buyer Name</strong></td>
				<td width="100" valign="top">&nbsp;:&nbsp;<? echo $buyer_library[$dataArray[0][csf('buyer_name')]];?></td>
				<td width="100" valign="top"><strong>Brand</strong></td>
				<td width="100" valign="top">&nbsp;:&nbsp;<?=$brandArr[$dataArray[0][csf('brand_id')]]; ?></td>
				<td width="100" valign="top"><strong>Master/Style Ref</strong></td>
				<td valign="top">&nbsp;:&nbsp;<? echo $dataArray[0][csf('style_ref_no')];?></td>
				
			</tr>
			<tr>

				<td valign="top"><strong>Season</strong></td>
				<td valign="top">&nbsp;:&nbsp;<? echo $season_arr[$dataArray[0][csf('season')]];?></td>
				<td valign="top"><strong>Saeason Year</strong></td>
				<td valign="top">&nbsp;:&nbsp;<?=$dataArray[0][csf('season_year')]; ?></td>
				<td valign="top"><strong>Product Dept</strong></td>
				<td valign="top">&nbsp;:&nbsp;<? echo $product_dept[$dataArray[0][csf('product_dept')]];?></td>
				<td valign="top"><strong>Sample Name</strong></td>
				<td valign="top">&nbsp;:&nbsp;<? echo $sample_library[$sample_name];?></td>
				<td valign="top"><strong>Buyer Ref</strong></td>
				<td valign="top">&nbsp;:&nbsp;<? echo $dataArray[0][csf('buyer_ref')];?></td>
			</tr>
			<tr>
				<td valign="top"><strong>Dealing Merchant</strong></td>
				<td valign="top">&nbsp;:&nbsp;<? echo $dealing_merchant_library[$dataArray[0][csf('dealing_marchant')]];?></td>
				<td valign="top"><strong>Team Name</strong></td>
				<td valign="top">&nbsp;:&nbsp;<? echo $team_name_lib[$dataArray[0][csf('team_leader')]];?></td>
				<td valign="top"><strong>Est.Ship Date</strong></td>
				<td valign="top">&nbsp;:&nbsp;<? echo change_date_format($dataArray[0][csf('estimated_shipdate')]);?></td>
				<td valign="top"><strong>Remarks/Desc</strong></td>
				<td valign="top" colspan="3" style="word-wrap: break-word;word-break: break-all;" >&nbsp;:&nbsp;<? echo $dataArray[0][csf('remarks')];?></td>
			</tr>
		</table>
			
		<table cellspacing="0" border="1" class="rpt_table" rules="all" width="1000">
			<thead>
				<tr><td colspan="11" align="center"><strong>Sample Details</strong></td></tr>
				<tr>
					<th rowspan="2" width="30">SL</th>
					<th rowspan="2" width="120">Garment Item</th>              
					<th rowspan="2" width="80">Article No</th>
					<th rowspan="2" width="80">Gmts Color</th>
					<th colspan="2">Prod Qty</th>
					<th rowspan="2" width="80">Submission Qty</th>
					<th colspan="2">Buyer Taget Weight Per Pcs</th>
					<th rowspan="2" width="80">Start Date</th>
			...
			<tbody>
				...
				<?
				$size_select=sql_select("SELECT  BH_QTY,DTLS_ID,SIZE_ID,TOTAL_QTY,TARGET_WEIGHT  from sample_development_size where mst_id=$update_id and status_active=1 and is_deleted=0");
				foreach($size_select as $row)
				{
					$sizeDataArr[$row["DTLS_ID"]][$row["SIZE_ID"]]+=$row["TOTAL_QTY"];
					$sizeTargetWtargetArr[$row["DTLS_ID"]][$row["SIZE_ID"]]+=$row["TARGET_WEIGHT"];
					$bhQtyArr[$row["DTLS_ID"]]+=$row["BH_QTY"];
				}
				//$imge_arr=return_library_array( "select master_tble_id,image_location from common_photo_library where form_name='sample_details_1' and file_type=1",'master_tble_id','image_location');
				$img_select=sql_select("SELECT  master_tble_id,image_location  from common_photo_library where form_name='sample_details_1' and file_type=1");
				foreach($img_select as $row)
				{
						$ImgArr[$row[csf('master_tble_id')]].=$row[csf('image_location')].',';
				}
				...
				foreach($result as $row)
				{
					...
					<tr>
						...
						if($ii !=1){echo "<tr>";}
						$totalSizeQty+=$size_qty;
						$totalweight+=$sizeTargetWtargetArr[$row[csf('id')]][$size_id];
						?>
						<td align="center"><p><? echo $size_arr[$size_id];?></p></td>
						...
						?>
						<td align="right"><? echo number_format($sizeTargetWtargetArr[$row[csf('id')]][$size_id],2);?></td>
						<td align="center"><? echo "Lbs";?></td>
						<?
							if($ii==1)
							{
								$ImgArrAll=rtrim($ImgArr[$row[csf('id')]],',');
								$ImgArrAllArr=array_unique(explode(",",$ImgArrAll));
								?>
									<td rowspan="<? echo $rowspan;?>" align="center"><? echo change_date_format($row[csf('delv_start_date')]);?> </td>
									<td rowspan="<? echo $rowspan;?>" align="center"><? echo change_date_format($row[csf('delv_end_date')]);?> </td>
									<td rowspan="<? echo $rowspan;?>">
                                    <?
                                    foreach($ImgArrAllArr as $img_file)
									{
									?>
                                    <img src="../../../<? echo $img_file;?>" width="120" height='35'> <br>
                                    <?
									}
									?>
		...
		<table cellspacing="0" border="1" class="rpt_table" rules="all" width="1000">
			<thead>
				...
				<tr>
					...
					<th width="80" align="center">Guage</th>
					<th width="80">No Of Ends </th>
					<th width="80" align="center">Yarn Color</th>
					<th width="80" align="center">Yarn Req. Qty.</th>
					<th width="80" align="center">UOM</th>
					<th width="80" align="center">Lot No</th>
					<th width="80" align="center">Supplier</th>
					<th width="80" align="center">Ref No.</th>
					<th width="80" align="center">Yarn Store</th>
					<th align="center">Remarks</th>
				</tr>
			</thead>
			<tbody>
			...
			$sql_qryf="SELECT a.uom_id,a.gauge, a.id, a.gmts_item_id, a.sample_mst_id, a.sample_name, a.body_part_id, a.fabric_nature_id, a.fabric_description, a.gsm, a.dia, a.SAMPLE_COLOR, a.color_data, a.color_type_id, a.width_dia_id,a.remarks_ra,a.buyer_prov,a.no_of_ends
				...
				$color_data_arr=explode('-----',$row[csf('color_data')]);
				foreach($color_data_arr as $dataArr){
					list($txtSL,$txtColor,$hiddenColorId,$txtYarnColor,$txtCount,$txtComposition,$cboType,$cboYarnSource,$txtGreyQnty,$cboUom,$txtLot,$cboStore,$text_comments,$supplier_id,$ref_no,$yarn_color_id)=explode('__',$dataArr);
					...
					<tr>
						...
						<td align="center"><? echo $txtLot;?></td>
						<td align="center"><? echo $supplier_library[$supplier_id];?></td>
						<td align="center"><? echo $ref_no;?></td>
						<td align="center"><? echo $store_name_lib[$cboStore];?></td>
						<td align="right"><p><? echo $text_comments;?></p></td>
					</tr>
				<?
		...
		<table cellspacing="0" border="1"  class="rpt_table" rules="all" width="1000">
			<thead>
				...
				<tr>
					<th width="30" align="center">SL</th>
					<th width="150" align="center">Garment Item</th>
					<th width="120" align="center">Trims Group</th>
					<th width="120" align="center">Description</th>
					<th width="120" align="center">Brand/Supp.Ref</th>
					<th width="50" align="center">UOM</th>
					<th width="70" align="center">Req/Dzn </th>
		...
		<table cellspacing="0" border="1" class="rpt_table" rules="all" width="1000">
				<thead>
					<tr>
						<td colspan="6" align="center"><strong>Required Emebellishment</td>
					</tr>
				<tr>
					<th width="30" align="center">SL</th>
					<th width="160" align="center">Garment Item</th>
		...
		<? echo signature_table(341, $data[0], "1000px");?>    
...
if ($action=="sizeinfo_popup")
{
    ...
    <script>
		...
		function total_submission_qty(inc)
		{
			...
			for(var i=1; i<=tot_row; i++)
			{
				...
				var targetWeight=$("#txttargetweight_"+i).val()*1;
		...
		{
			var tot_row=$('#size_tbl tbody tr').length;
			var total_bhqnty=""; var total_plqnty=""; var total_dyqnty=""; var total_testqnty=""; var total_selfqnty=""; var total_targetweight=""; var total_all_qnty=""; var total='';
			for(var i=1; i<=tot_row; i++)
			{
				var bhQty=$("#txtbhqty_"+i).val()*1;
				var plQty=$("#txtplqty_"+i).val()*1;
				...
				var target_weight=$("#txttargetweight_"+i).val()*1;
				total_targetweight=total_targetweight*1+$("#txttargetweight_"+i).val()*1;
			...
			document.getElementById('txt_total_target_weight').value=total_targetweight.toFixed(2);
		...
		function fnc_close( )
		{
			...
			for(var i=1; i<=rowCount; i++)
			{
				if(breck_down_data=="")
				{
					breck_down_data+=$('#txtsizename_'+i).val()+'_'+($('#txtbhqty_'+i).val()*1)+'_'+($('#txtplqty_'+i).val()*1)+'_'+($('#txtdyqty_'+i).val()*1)+'_'+($('#txttestqty_'+i).val()*1)+'_'+($('#txtselfqty_'+i).val()*1)+'_'+($('#txttargetweight_'+i).val()*1)+'_'+($('#txttotalqty_'+i).val()*1);
				}
				else
				{
					breck_down_data+="__"+$('#txtsizename_'+i).val()+'_'+($('#txtbhqty_'+i).val()*1)+'_'+($('#txtplqty_'+i).val()*1)+'_'+($('#txtdyqty_'+i).val()*1)+'_'+($('#txttestqty_'+i).val()*1)+'_'+($('#txtselfqty_'+i).val()*1)+'_'+($('#txttargetweight_'+i).val()*1)+'_'+($('#txttotalqty_'+i).val()*1);
				}
			}
    ...
    <body onLoad="add_auto_complete(1);" >
        ...
        <form name="size_1" id="size_1">
            ...
            <table align="center" cellspacing="0" width="770" class="rpt_table" border="1" rules="all" id="size_tbl" >
                <thead>
                    ...
                    <th width="70" >Test</th>
                    <th width="70" >Self</th>
					<th width="70" >Buyer Target Weight Per Pcs [Lbs]</th>
                ...
                <?
					...
					if($count_tr>0)
					{
						...
						foreach ($data_all as $size_data)
						{
							$size_name=''; $bh_qty=0; $pl_qty=0; $dy_qty=0; $test_qty=0; $self_qty=0; $target_weight=0; $totalqty=0;
							$ex_size_data=explode('_',$size_data);
							...
							$target_weight=$ex_size_data[6];
							$totalqty=$ex_size_data[7];
						...
						?>
							<tr id="row_<? echo $i; ?>" >
							   ...
							   <td><input name="txttargetweight[]" class="text_boxes_numeric" ID="txttargetweight_<? echo $i; ?>" style="width:70px"  onBlur="calculate_total_qnty_by_type();" value="<? echo number_format($target_weight, 2, '.', ''); ?>"/></td>

							   <td><input name="txttotalqty[]" class="text_boxes_numeric" ID="txttotalqty_<? echo $i; ?>" style="width:70px"  readonly value="<? echo number_format($totalqty, 2, '.', ''); ?>" /></td>
								<td align="center">
					...
					else
					{
						...
						<tr id="row_1">
						   ...
						   <td width="70" align="center" ><Input name="txttargetweight[]" class="text_boxes_numeric" ID="txttargetweight_1" style="width:70px"  onBlur="calculate_total_qnty_by_type();"/></td>
            ...
            <table align="center" cellspacing="0" width="770" class="rpt_table" border="1" rules="all" id="" >
				<tr>
					<td width="110">&nbsp;</td>
					...
					<td width="70" align="center"><Input name="txt_total_target_weight" class="text_boxes_numeric" ID="txt_total_target_weight" style="width:70px" value="<? echo number_format($total_targetweight, 2, '.', ''); ?>" readonly /></td>
...
{
	$size_arr=return_library_array( "select id,size_name from lib_size", "id", "size_name" );
	$qry_size="select id, mst_id, dtls_id, size_id, size_qty,bh_qty,plan_qty,dyeing_qty,test_qty,self_qty,target_weight,total_qty from sample_development_size where dtls_id='$data'";
	...
	foreach ($qry_result as $row)
	{
		...
		if($target_weight=="") $target_weight=$row[csf("target_weight")]; else $target_weight.="*".$row[csf("target_weight")];
	...
	echo "document.getElementById('hidden_totalweight').value 	 				= '".$target_weight."';\n";
...
{
	echo create_drop_down( "cbo_buyer_name", 150, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond  and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,30,90)) order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", 0, "load_drop_down( 'requires/sweater_sample_requisition_controller', this.value, 'load_drop_down_season_buyer', 'season_td');load_drop_down( 'requires/sweater_sample_requisition_controller', this.value, 'load_drop_down_sample_for_buyer', 'sample_td');load_drop_down( 'requires/sweater_sample_requisition_controller', this.value+'*'+1, 'load_drop_down_brand', 'brand_td');" );
	exit();
}
...
if ($action=="save_update_delete_mst")
{
   $process = array( &$_POST );
 	...
 	if ($operation==0)  // Insert Here
	{
		...
		$new_system_id=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', '', date("Y",time()), 5, "select requisition_number_prefix,requisition_number_prefix_num from sample_development_mst where entry_form_id=341 and company_id=$cbo_company_name and to_char(insert_date,'YYYY')=".date('Y',time())." order by id desc ", "requisition_number_prefix", "requisition_number_prefix_num" ));
		

		$field_array="id, requisition_number_prefix, requisition_number_prefix_num, requisition_number, sample_stage_id, requisition_date, quotation_id, style_ref_no, company_id, location_id, buyer_name, season, product_dept, dealing_marchant, agent_name, buyer_ref, estimated_shipdate, remarks, inserted_by, insert_date, status_active, is_deleted, entry_form_id, is_copy, req_ready_to_approved, material_delivery_date, sample_team_id,team_leader, season_year, brand_id";
		$data_array="(".$id_mst.",'".$new_system_id[1]."',".$new_system_id[2].",'".$new_system_id[0]."',".$cbo_sample_stage.",".$txt_requisition_date.",".$txt_quotation_id.",".$txt_style_name.",".$cbo_company_name.",".$cbo_location_name.",".$cbo_buyer_name.",".$cbo_season_name.",".$cbo_product_department.",".$cbo_dealing_merchant.",".$cbo_agent.",".$txt_buyer_ref.",".$txt_est_ship_date.",".$txt_remarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,341,0,".$cbo_ready_to_approved.",".$txt_material_dlvry_date.",".$cbo_sample_team.",".$cbo_team_leader.",".$cbo_season_year.",".$cbo_brand_id.")";
	...
	if ($operation==1)  // Update Here
	{
		...
		$field_array="sample_stage_id*requisition_date*style_ref_no*buyer_name*season*product_dept*dealing_marchant*agent_name*buyer_ref*estimated_shipdate*remarks*updated_by*update_date*status_active*is_deleted*req_ready_to_approved*material_delivery_date*sample_team_id*season_year*brand_id*team_leader";
		//txt_bhmerchant*txt_product_code
		$data_array="".$cbo_sample_stage."*".$txt_requisition_date."*".$txt_style_name."*".$cbo_buyer_name."*".$cbo_season_name."*".$cbo_product_department."*".$cbo_dealing_merchant."*".$cbo_agent."*".$txt_buyer_ref."*".$txt_est_ship_date."*".$txt_remarks."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*1*0*".$cbo_ready_to_approved."*".$txt_material_dlvry_date."*".$cbo_sample_team."*".$cbo_season_year."*".$cbo_brand_id."*".$cbo_team_leader."";
...
if ($action=="save_update_delete_sample_details")
{
	...
	if ($operation==0)  // Insert Here
	{
			...
			$variableAckSql = "select editable as auto_acknowledge from variable_order_tracking where variable_list = 74 and status_active=1 and is_deleted=0 and company_name=$cbo_company_name";
			$variableAckSqlRes = sql_select($variableAckSql);
			if(count($variableAckSqlRes)>0)
			{
				foreach ($variableAckSqlRes as $rows)
			{
				$auto_acknowledge=$rows[csf('auto_acknowledge')];
			}
			}
 			$id_dtls=return_next_id( "id", "sample_development_dtls", 1 ) ;
 			$field_array= "id,sample_mst_id,sample_name,gmts_item_id,smv,article_no,sample_color,sample_prod_qty,submission_qty,delv_start_date,delv_end_date,sample_charge,sample_curency,acknowledge_status,inserted_by,insert_date,status_active,is_deleted,entry_form_id,size_data,fabric_status,acc_status,embellishment_status,fab_status_id,acc_status_id,embellishment_status_id";
			$ids=return_next_id( "id","sample_development_size", 1 ) ;
			$field_array_size="id, mst_id, dtls_id,size_id,bh_qty,plan_qty,dyeing_qty,test_qty,self_qty,target_weight,total_qty,inserted_by, insert_date, status_active, is_deleted";
			...
			//	$txtweight="txtweight_".$i;
				...
				if(str_replace("'","",$$txtColor)!="")
				{
					...
					{
						$color_id = return_id( str_replace("'","",$$txtColor), $color_arr, "lib_color", "id,color_name","341");
				...
				if ($i!=1) $data_array .=",";
				$data_array .="(".$id_dtls.",".$update_id.",".$$cboSampleName.",".$$cboGarmentItem.",".$$txtSmv.",".$$txtArticle.",'".$color_id."',".$$txtSampleProdQty.",".$$txtSubmissionQty.",".$$txtDelvStartDate.",".$$txtDelvEndDate.",".$$txtChargeUnit.",".$$cboCurrency.",".$auto_acknowledge.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','1',0,341,".$$txtAllData.",0,0,0,0,0,0)";
				...
				{
					$size_name=""; $bhqty=0; $dyqty=0; $testqty=0; $selfqty=0; $targetweight=0; $totalqty=0;
					$ex_size_data=explode("_",$size_data);
					$size_name=$ex_size_data[0];
					...
					if($size_name!="")
					{
						...
						{
							$size_id = return_id( $size_name, $size_arr, "lib_size", "id,size_name","341");
							//echo $$txtColorName.'='.$color_id.'<br>';
					...
					$data_array_size.="$add_comma(".$ids.",".$update_id.",".$id_dtls.",".$size_id.",".$bhqty.",".$plqty.",".$dyqty.",".$testqty.",".$selfqty.",".$targetweight.",".$totalqty.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
					$ids=$ids+1;
				}
				$id_dtls=$id_dtls+1;

		    }
			/* if($auto_acknowledge==1){
				$mstAckSql = "select id, requisition_number,requisition_date, style_ref_no, company_id, buyer_name, season, dealing_marchant,estimated_shipdate,team_leader from sample_development_mst where entry_form_id = 341 and id=$update_id and status_active=1 and is_deleted=0 and company_id=$cbo_company_name";
				$mstAckSqlRes = sql_select($mstAckSql);
				if(count($mstAckSqlRes)>0)
				{
					foreach ($mstAckSqlRes as $rows)
					{
						$requisition_number=$rows[csf('requisition_number')];
						$requisition_date=$rows[csf('requisition_date')];
						$style_ref_no=$rows[csf('style_ref_no')];
						$buyer_name=$rows[csf('buyer_name')];
						$season=$rows[csf('season')];
						$dealing_marchant=$rows[csf('dealing_marchant')];
						$estimated_shipdate=$rows[csf('estimated_shipdate')];
						$team_leader=$rows[csf('team_leader')];
					}
				}
				$id_mst=return_next_id( "id", "sample_requisition_acknowledge", 1 ) ;

				$ack_field_array="id,sample_mst_id,requisition_number,requisition_date,company_id,buyer_name,style_ref_no,dealing_marchant,team_leader,sample_qty,delv_start_date,delv_end_date,entry_form,season,inserted_by, insert_date,status_active,is_deleted";
				$ack_data_array="(".$id_mst.",".$update_id.",'".$requisition_number."','".$requisition_date."',".$cbo_company_name.",".$buyer_name.",'".$style_ref_no."',".$dealing_marchant.",".$team_leader.",".$totalqty.",".$$txtDelvStartDate.",".$$txtDelvEndDate.",345,".$season.",".$user_id.",'".$pc_date_time."',1,0)";
				$rID3=sql_insert("sample_requisition_acknowledge",$ack_field_array,$ack_data_array,1);
			} */
			
			//echo "559**INSERT INTO sample_requisition_acknowledge ($ack_field_array) values $ack_data_array"; die;
	...
	else if ($operation==1)  // Update Here
	{
			...
			$prev_ids="SELECT id from sample_development_dtls where status_active=1 and is_deleted=0 and sample_mst_id=$update_id and entry_form_id=341";
			$prev_ids_array=array();
			...
			for ($i=1;$i<=$total_row;$i++)
		    {
				...
				$data_array_size.='';
					foreach($ex_data as $size_data)
					{
						...
						$data_array_size.="$add_comma(".$ids.",".$update_id.",".$$updateIdDtls.",".$size_id.",".$bhqty.",".$plqty.",".$dyqty.",".$testqty.",".$selfqty.",".$targetweight.",".$totalqty.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
						$ids=$ids+1;
					}
				}
			 	else
				{
					...
					$data_array .="(".$id_dtls.",".$update_id.",".$$cboSampleName.",".$$cboGarmentItem.",".$$txtSmv.",".$$txtArticle.",'".$color_id."',".$$txtSampleProdQty.",".$$txtSubmissionQty.",".$$txtDelvStartDate.",".$$txtDelvEndDate.",".$$txtChargeUnit.",".$$cboCurrency.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','1',0,341,".$$txtAllData.",0,0,0,0,0,0)";
	...
	if ($operation==2)  // Delete Here
	{
		...
		$is_approved=return_field_value("is_approved","sample_development_mst","id=$update_id and entry_form_id=341 and status_active=1 and is_deleted=0");
		if($is_approved==3){
			$is_approved=1;
		}
		...
		$rID=sql_delete("sample_development_dtls",$field_array,$data_array,"sample_mst_id*entry_form_id","".$update_id."*341",0);
		if($rID)
		{
...
if ($action=="save_update_delete_required_fabric")
{
	...
	if ($operation==0)  // Insert Here
	{
			...
			$field_array_col="id, mst_id, dtls_id,color_id,fabric_color,qnty,count,composition,type,yarn_source,lot,store_id,comments,supplier_id,ref_no,inserted_by, insert_date, status_active, is_deleted";
			...
			for ($i=1;$i<=$total_row;$i++)
		    {
				...
				$ex_data="";
 				...
 				foreach($ex_data as $color_data)
 				{
					...
					if(str_replace("'","",$yarnColor)!="")
					{
						...
						{
							$yarn_color_id = return_id( str_replace("'","",$yarnColor), $color_arr, "lib_color", "id,color_name","341");
				...
				$ex_data="";
 				...
 				$add_comm="";
  				foreach($ex_data as $color_data) 
				{
					list($txtSL,$txtColor,$hiddenColorId,$txtYarnColor,$txtCount,$txtComposition,$cboType,$cboYarnSource,$txtGreyQnty,$cboUom,$txtLot,$cboStore,$txtComments,$supplier_id,$ref_no,$yarn_color_id)=explode('__',$color_data);

 					 if($add_comm) $add_comm.=","; else $add_comm.="";
  					$data_array_col.="$add_comm(".$idColorTbl.",".$update_id.",".$id_dtls.",'".$hiddenColorId."','".$yarn_color_id."','".$txtGreyQnty."','".$txtCount."','".$txtComposition."','".$cboType."','".$cboYarnSource."','".$txtLot."',".$cboStore.",'".$txtComments."',".$supplier_id.",'".$ref_no."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
	...
	else if ($operation==1)  // Update Here
	{
			...
			for ($i=1;$i<=$total_row;$i++)
		    {
			 	...
			 	else
				{
					...
					{
						list($txtSL,$txtColor,$hiddenColorId,$txtYarnColor,$txtCount,$txtComposition,$cboType,$cboYarnSource,$txtGreyQnty,$cboUom,$txtLot,$cboStore,$txtComments,$supplier_id,$ref_no,$yarn_color_id)=explode('__',$color_datas);
						if($kk==1) $add_comma=""; else $add_comma=",";
						$data_array_cols.="$add_comma(".$idColorTbl.",".$update_id.",".$id_dtls.",'".$hiddenColorId."','".$yarn_color_id."','".$txtGreyQnty."','".$txtCount."','".$txtComposition."','".$cboType."','".$cboYarnSource."','".$txtLot."',".$cboStore.",'".$txtComments."',".$supplier_id.",'".$ref_no."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
 			...
 			if($data_array_up!="")
			{
				...
				//echo "10**".$rIDs.'='.$rID1.'='.$delrfDtls.'='.$del.'='.$flag;die;
		...
		//	echo "10**".$rID.'='.$rIDs.'='.$delrfDtls.'='.$del.'='.$flag;die;
			if($db_type==0)
			{
				if($flag==1){
					mysql_query("COMMIT");
...
if ($action=="save_update_delete_required_accessories")
{
	...
	if ($operation==0)  // Insert Here
	{
			...
			$id_dtls=return_next_id( "id", "sample_development_fabric_acc", 1 ) ;//txtRaexQty_
			$field_array= "id,sample_mst_id,sample_name_ra,gmts_item_id_ra,trims_group_ra,description_ra,brand_ref_ra,uom_id_ra,req_dzn_ra,ex_qty,req_qty_ra,remarks_ra,inserted_by,insert_date,status_active,is_deleted,form_type";
			for ($i=1;$i<=$total_row;$i++)
		    {
				...
				$txtRaexQty="txtRaexQty_".$i;
				$txtRaRemarks="txtRaRemarks_".$i;


				if ($i!=1) $data_array .=",";
				$data_array .="(".$id_dtls.",".$update_id.",".$$cboRaSampleName.",".$$cboRaGarmentItem.",".$$cboRaTrimsGroup.",".$$txtRaDescription.",".$$txtRaBrandSupp.",".$$cboRaUom.",".$$txtRaReqDzn.",".$$txtRaexQty.",".$$txtRaReqQty.",".$$txtRaRemarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','1',0,2)";
	...
	else if ($operation==1)  // Update Here
	{
			...
			$id_dtls=return_next_id( "id", "sample_development_fabric_acc", 1);
			$field_array_up="sample_name_ra*gmts_item_id_ra*trims_group_ra*description_ra*brand_ref_ra*uom_id_ra*req_dzn_ra*ex_qty*req_qty_ra*remarks_ra*updated_by*update_date";
			$field_array= "id,sample_mst_id,sample_name_ra,gmts_item_id_ra,trims_group_ra,description_ra,brand_ref_ra,uom_id_ra,req_dzn_ra,ex_qty,req_qty_ra,remarks_ra,inserted_by,insert_date,status_active,is_deleted,form_type";
			$add_comma=0; $data_array=""; //echo "10**";
			for ($i=1;$i<=$total_row;$i++)
		    {
				...
				if (str_replace("'",'',$$updateIdAccDtls)!="")
				{
					...
					$data_array_up[str_replace("'",'',$$updateIdAccDtls)] =explode("*",("".$$cboRaSampleName."*".$$cboRaGarmentItem."*".$$cboRaTrimsGroup."*".$$txtRaDescription."*".$$txtRaBrandSupp."*".$$cboRaUom."*".$$txtRaReqDzn."*".$$txtRaexQty."*".$$txtRaReqQty."*".$$txtRaRemarks."*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'"));
							...
							<table  cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
								...
								<tr>
									<td width="140">
										...
										<?
										echo create_drop_down( "cbo_company_mst", 140, "select id,company_name from lib_company where status_active =1 and is_deleted=0  order by company_name","id,company_name",1, "-- Select Company --", '',"load_drop_down( 'sweater_sample_requisition_controller', this.value, 'load_drop_down_buyer_style', 'buyer_td_st' );" );
										?>
									</td>
...
<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
	<table width="500" cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
            ...
            <tbody>
                ...
                <td>
                    <?
                        echo create_drop_down( "cbo_company_name", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select Company --",'', "load_drop_down( 'sweater_sample_requisition_controller', this.value, 'load_drop_down_buyer_inq', 'buyer_td_inq' );");
                     ?>
...
if($action=="populate_data_from_inquiry_search")
{
	...
	foreach($sql as $row)
	{
		echo "load_drop_down( 'requires/sweater_sample_requisition_controller', '".$row[csf("company_id")]."', 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/sweater_sample_requisition_controller', '".$row[csf("company_id")]."', 'load_drop_down_location', 'location_td' );load_drop_down( 'requires/sweater_sample_requisition_controller', '".$row[csf("company_id")]."', 'load_drop_down_agent', 'agent_td' );load_drop_down( 'requires/sweater_sample_requisition_controller','".$row[csf("buyer_id")]."', 'load_drop_down_season_buyer', 'season_td');load_drop_down( 'requires/sweater_sample_requisition_controller','".$row[csf("buyer_id")]."', 'load_drop_down_sample_for_buyer', 'sample_td');load_drop_down( 'requires/sweater_sample_requisition_controller', '".$row[csf("gmts_item")]."', 'load_drop_down_garment_item_for_after_order', 'item_id_1')\n";
...
if($action=="populate_data_from_search_popup")
{
	...
	//echo "select * from wo_po_details_master where id=$data";
 	foreach($res as $result)
	{
		//echo "set_button_status(0, '".$_SESSION['page_permission']."', 'fnc_sweater_sample_requisition_mst_info',1);\n";
		echo "load_drop_down( 'requires/sweater_sample_requisition_controller', '".$result[csf("company_name")]."', 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/sweater_sample_requisition_controller', '".$result[csf("company_name")]."', 'load_drop_down_location', 'location_td' );load_drop_down( 'requires/sweater_sample_requisition_controller', '".$result[csf("company_name")]."', 'load_drop_down_agent', 'agent_td' );load_drop_down( 'requires/sweater_sample_requisition_controller', '".$result[csf("company_name")]."', 'load_drop_down_buyer', 'buyer_td');load_drop_down( 'requires/sweater_sample_requisition_controller','".$result[csf("buyer_name")]."', 'load_drop_down_season_buyer', 'season_td');load_drop_down( 'requires/sweater_sample_requisition_controller','".$result[csf("buyer_name")]."', 'load_drop_down_brand', 'brand_td');load_drop_down( 'requires/sweater_sample_requisition_controller','".$result[csf("buyer_name")]."', 'load_drop_down_sample_for_buyer', 'sample_td');load_drop_down( 'requires/sweater_sample_requisition_controller', '".$result[csf("gmts_item_id")]."', 'load_drop_down_garment_item_for_after_order', 'item_id_1');load_drop_down( 'requires/sweater_sample_requisition_controller', '".$result[csf("garments_nature")]."', 'load_drop_down_fabric_nature_for_after_order', 'rf_fabric_nature_1');\n";
		//load_drop_down( 'requires/sweater_sample_requisition_controller', '".$result[csf("item_number_id")]."', 'load_drop_down_trims_group_for_after_order', 'ra_trims_group_1');
...
if($action=="populate_data_from_requisition_search_popup")
{
	...
	if($sample_st==2 && $quotation_info)
	{
 	...
 	foreach($res as $result)
	{
		echo "load_drop_down( 'requires/sweater_sample_requisition_controller', '".$result[csf("company_id")]."', 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/sweater_sample_requisition_controller', '".$result[csf("company_id")]."', 'load_drop_down_location', 'location_td' );load_drop_down( 'requires/sweater_sample_requisition_controller', '".$result[csf("company_id")]."', 'load_drop_down_agent', 'agent_td' );load_drop_down( 'requires/sweater_sample_requisition_controller','".$result[csf("buyer_name")]."', 'load_drop_down_season_buyer', 'season_td');load_drop_down( 'requires/sweater_sample_requisition_controller','".$result[csf("buyer_name")]."', 'load_drop_down_brand', 'brand_td');load_drop_down( 'requires/sweater_sample_requisition_controller','".$result[csf("buyer_name")]."', 'load_drop_down_sample_for_buyer', 'sample_td');load_drop_down( 'requires/sweater_sample_requisition_controller', '".$result[csf("id")]._1."', 'load_drop_down_required_fabric_sample_name', 'rfSampleId_1');load_drop_down( 'requires/sweater_sample_requisition_controller','".$result[csf("id")]._1."', 'load_drop_down_required_fabric_gmts_item', 'rfItemId_1');load_drop_down( 'requires/sweater_sample_requisition_controller', '".$result[csf("id")]._2."', 'load_drop_down_required_fabric_sample_name','raSampleId_1');load_drop_down( 'requires/sweater_sample_requisition_controller', '".$result[csf("id")]._2."', 'load_drop_down_required_fabric_gmts_item', 'raItemId_1');load_drop_down( 'requires/sweater_sample_requisition_controller', '".$result[csf("id")]._3."', 'load_drop_down_required_fabric_sample_name','reSampleId_1'); load_drop_down( 'requires/sweater_sample_requisition_controller','".$result[csf("id")]._3."', 'load_drop_down_required_fabric_gmts_item', 'reItemIid_1');\n";
		...
		echo "$('#cbo_sample_team').val('".$result[csf('sample_team_id')]."');\n";
		echo "$('#cbo_team_leader').val('".$result[csf('team_leader')]."');\n";
		...
		if($result[csf('sample_stage_id')]==1)
		{
			...
			echo "load_drop_down( 'requires/sweater_sample_requisition_controller', '".$job_arr[$result[csf("quotation_id")]]['gmts']."', 'load_drop_down_garment_item_for_after_order', 'item_id_1');\n";
		}
		...
		else if($result[csf('sample_stage_id')]==2 && ($result[csf('quotation_id')]))
		{
			...
			echo "fnc_browse_style('".$result[csf('sample_stage_id')]."');\n";
			echo "load_drop_down( 'requires/sweater_sample_requisition_controller', '".$inq_arr[$result[csf("quotation_id")]]['gmts']."', 'load_drop_down_garment_item_for_after_order', 'item_id_1');\n";
...
if($action=="color_popup")
{
	...
	if($style_db_id!='')
	{
		...
		echo  create_list_view("list_view", "Color Name", "150","220","240",0, $sql , "js_set_value", "color_name", "", 1, "0,0", $arr , "color_name","requires/sweater_sample_requisition_controller", 'setFilterGrid("list_view",-1);' );
	}
	else
	{
		$sql= "select id, color_name from lib_color where color_name <> '' or color_name is not null";

		echo  create_list_view("list_view", "Color Name", "150","220","240",0, $sql , "js_set_value", "color_name", "", 1, "0", $arr , "color_name","requires/sweater_sample_requisition_controller", 'setFilterGrid("list_view",-1);' );
	}
	exit();
}
...
if($action=="load_php_dtls_form")
{
		...
		if(count($sql_result)>0)
		{
			foreach($sql_result as $row)
			{
				...
				<tr id="tr_<? echo $i; ?>" style="height:10px;" class="general">
					...
					<td><input style="width:80px;" type="text" class="text_boxes"  name="txtColor_<? echo $i; ?>" id="txtColor_<? echo $i; ?>" placeholder="write/browse" onDblClick="openmypage_color_size('requires/sweater_sample_requisition_controller.php?action=color_popup','Color Search','1','<? echo $i; ?>');" value="<? echo $color_arr[$row[csf("sample_color")]]; ?>"/></td>
					...
					<td>
						...
						{
							?>
							<input style="width:100px;" type="text" class="text_boxes_numeric"  name="txtSampleProdQty_<? echo $i; ?>" readonly id="txtSampleProdQty_<? echo $i; ?>" placeholder="browse"  ondblclick="openmypage_sizeinfo('requires/sweater_sample_requisition_controller.php?action=sizeinfo_popup','Size Search','<? echo $i;?>')" value="<? echo $row[csf("sample_prod_qty")]; ?>"   />

							 <!--onFocus="openmypage_sizeinfo('requires/sweater_sample_requisition_controller.php?action=sizeinfo_popup_mouseover','Size Search','< ? echo $i;?>')"-->
			...
			foreach($sql_resultf as $row)
			{
				...
				<tr id="tr_<? echo $i; ?>" style="height:10px;" class="general">
					...
					<td align="center" id="rf_color_1">
                        <input style="width:60px;" type="text" class="text_boxes"  name="txtRfColor_<? echo $i; ?>" id="txtRfColor_<? echo $i; ?>" placeholder="browse" onDblClick="openmypage_rf_color('requires/sweater_sample_requisition_controller.php?action=color_popup_rf','Color Search','<? echo $i;?>');"
	...
	{
		$sql_sam="SELECT id,sample_mst_id,sample_name_ra,gmts_item_id_ra,trims_group_ra,description_ra,brand_ref_ra,uom_id_ra,ex_qty,req_dzn_ra,req_qty_ra,remarks_ra from sample_development_fabric_acc where sample_mst_id='$up_id' and form_type=2 and  is_deleted=0  and status_active=1 order by id ASC";
		$sql_result =sql_select($sql_sam);  $i=1;
		if(count($sql_result)>0)
		{
			foreach($sql_result as $row)
			{
				...
				<tr  id="tr_<? echo $i;?>"  class="general">
                    ...
                    <td align="center" id="ra_req_dzn_1" width="100">
						<input style="width:100px;" type="text" class="text_boxes_numeric"  name="txtRaexQty_<? echo $i;?>" id="txtRaexQty_<? echo $i;?>" placeholder="write" value="<? echo $row[csf("ex_qty")]; ?>" onBlur="calculate_required_qty('3','<? echo $i ;?>');" />
					</td>
                    
                     

					<td align="center" id="ra_req_qty_1" width="80">
						<input style="width:80px;" type="text" class="text_boxes_numeric"  name="txtRaReqQty_<? echo $i;?>" id="txtRaReqQty_<? echo $i;?>" placeholder="write" value="<? echo $row[csf("req_qty_ra")]; ?>" readonly/>
					</td>
...
if ($action=="copy_requisition")
{
	...
	$size_arr=return_library_array( "select id,size_name from lib_size", "id", "size_name" );
  	...
  	if ($operation==5)  // Insert Here
  	{
  		...
  		$id_mst=return_next_id( "id", "sample_development_mst", 1 ) ;
		//$new_system_id=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), 'SR', date("Y",time()), 5, "select requisition_number_prefix, requisition_number_prefix_num from sample_development_mst where entry_form_id=341 and company_id=$cbo_company_name  and $year_cond=".date('Y',time())." order by requisition_number_prefix_num desc ", "requisition_number_prefix", "requisition_number_prefix_num" ));
  		if($db_type==0)
  		{
  			$new_system_id=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', '', date("Y",time()), 5, "select requisition_number_prefix,requisition_number_prefix_num from sample_development_mst where  entry_form_id=341 and company_id=$cbo_company_name and YEAR(insert_date)=".date('Y',time())." order by id desc ", "requisition_number_prefix", "requisition_number_prefix_num" ));
  		}
  		if($db_type==2)
  		{
  			$new_system_id=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', '', date("Y",time()), 5, "select requisition_number_prefix,requisition_number_prefix_num from sample_development_mst where entry_form_id=341 and company_id=$cbo_company_name and to_char(insert_date,'YYYY')=".date('Y',time())." order by id desc ", "requisition_number_prefix", "requisition_number_prefix_num" ));
  		...
  		$field_array="id,requisition_number_prefix,requisition_number_prefix_num,requisition_number,sample_stage_id,requisition_date,quotation_id,style_ref_no,company_id,location_id,buyer_name,season,product_dept,dealing_marchant,agent_name,buyer_ref,estimated_shipdate,remarks,inserted_by,insert_date,status_active,is_deleted,entry_form_id,is_copy,req_ready_to_approved,material_delivery_date,team_leader,sample_team_id";
  		$data_array="(".$id_mst.",'".$new_system_id[1]."',".$new_system_id[2].",'".$new_system_id[0]."',".$cbo_sample_stage.",".$txt_requisition_date.",".$txt_quotation_id.",".$txt_style_name.",".$cbo_company_name.",".$cbo_location_name.",".$cbo_buyer_name.",".$cbo_season_name.",".$cbo_product_department.",".$cbo_dealing_merchant.",".$cbo_agent.",".$txt_buyer_ref.",".$txt_est_ship_date.",".$txt_remarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,341,1,'2',".$txt_material_dlvry_date.",".$cbo_team_leader.",".$cbo_sample_team.")";
...
?>
    ...
    <script>
		...
		function fnc_close( )
		{
			...
			for(var i=1; i<=rowCount; i++)
			{
				...
				if(breck_down_data=="")
				{
					breck_down_data+=($('#txtSL_'+i).val()*1)+'__'+$('#txtColor_'+i).val()+'__'+$('#hiddenColorId_'+i).val()*1+'__'+$('#txtYarnColor_'+i).val()+'__'+$('#txtCount_'+i).val()+'__'+$('#txtComposition_'+i).val()+'__'+$('#cboType_'+i).val()*1+'__'+$('#cboYarnSource_'+i).val()*1+'__'+$('#txtGreyQnty_'+i).val()*1+'__'+$('#cboUom_'+i).val()*1+'__'+$('#txtLot_'+i).val()+'__'+$('#cboYarnStore_'+i).val()+'__'+$('#txtComments_'+i).val()+'__'+$('#cboSupplier_'+i).val()+'__'+$('#txtrefno_'+i).val();
					  display_col +=$('#txtColor_'+i).val() ;
				}
				else  
				{
    ...
    <body>
        ...
        <form name="size_1" id="size_1">
            ...
            <table align="center" cellspacing="0" width="1140" class="rpt_table" border="1" rules="all" id="col_tbl" >
            	<thead>
                ...
                <tbody>
                	...
                	if($data)
                	{
                		...
                		if($count_tr>0)
                		{
							...
							list($txtSL,$txtColor,$hiddenColorId,$txtYarnColor,$txtCount,$txtComposition,$cboType,$cboYarnSource,$txtGreyQnty,$cboUom,$txtLot,$cboStore,$txtComments,$supplier_id,$ref_no)=explode('__',$size_data);
							//echo $supplier_id.'<br>';
							...
							<tr id="row_<? echo $i; ?>">
								...
								<td>
                                	<input name="txtSL_<? echo $i; ?>" class="text_boxes" id="txtSL_<? echo $i; ?>" value="<? echo $i; ?>" style="width:30px" readonly />
                                    <input type="hidden" name="colorupid_<? echo $i; ?>" class="text_boxes" ID="colorupid_<? echo $i; ?>" value="" style="width:30px">
                                </td>
								<td>
									<input name="txtColor_<? echo $i; ?>" class="text_boxes" id="txtColor_<? echo $i; ?>" style="width:70px" value="<? echo $txtColor; ?>" /><input type="hidden" name="hiddenColorId_<? echo $i; ?>" id="hiddenColorId_<? echo $i; ?>" value="<? echo $hiddenColorId ?>">
                                ...
                                <td>
									<input name="txtComposition_<? echo $i; ?>" class="text_boxes" id="txtComposition_<? echo $i; ?>" style="width:90px" value="<? echo $txtComposition; ?>"  />
								</td>
                                ...
                                <td>
                                 ...
                                 <td>
								<?
                                	$sql_supp="select c.id,c.supplier_name from lib_supplier_tag_company a, lib_supplier_party_type b, lib_supplier c where c.id=b.supplier_id and a.supplier_id = b.supplier_id and b.party_type=2 and c.status_active=1 and c.is_deleted=0 group by c.id, c.supplier_name order by c.supplier_name";
									
									echo create_drop_down( "cboSupplier_".$i, 100, $sql_supp,"id,supplier_name", 1, "-- Select --", $supplier_id, "",0,'','','','','','' );
								?>
                            	</td>
                                 <td>
									<input name="txtrefno_<? echo $i; ?>" class="text_boxes" id="txtrefno_<? echo $i; ?>" style="width:60px" value="<? echo $ref_no; ?>"  />
								</td>
								...
								<td align="center">
										...
										<input type="button" id="decrease_<? echo $i; ?>" name="decrease_<? echo $i; ?>" style="width:20px" class="formbutton" value="-" onClick="fn_deleteRow(<? echo $i; ?>);" />
								</td>
							</tr>
								<?
								$i++;
					...
					{
						$sql_col="select id,sample_color from sample_development_dtls where entry_form_id=341 and sample_mst_id=$mainId and sample_name=$sampleName and gmts_item_id=$garmentItem and is_deleted=0 and sample_color=$gmtsColorId  and status_active=1 order by id ASC";
						$sql_result =sql_select($sql_col);
						$i=1;
						foreach($sql_result as $row)
						{
							...
							<tr id="row_<? echo $i; ?>">
								...
								<td align="center" >
                                <input name="txtColor_<? echo $i; ?>" class="text_boxes" ID="txtColor_<? echo $i; ?>" value="<? echo $color_library[$row[csf('sample_color')]];  ?>" style="width:70px"  />
								<input type="hidden" name="hiddenColorId_<? echo $i; ?>" id="hiddenColorId_<? echo $i; ?>" value="<? echo $row[csf('sample_color')];  ?>">
								</td>
								...
								<td>
									...
									<input name="txtComposition_<? echo $i; ?>" class="text_boxes" ID="txtComposition_<? echo $i; ?>" style="width:90px" value="<? echo $FabricDescription; ?>"  />
								</td>
                                ...
                                <td>
								<?
                                	 $sql="select c.id,c.supplier_name from lib_supplier_tag_company a, lib_supplier_party_type b, lib_supplier c where c.id=b.supplier_id and a.supplier_id = b.supplier_id and b.party_type=2 and c.status_active=1 and c.is_deleted=0 group by c.id, c.supplier_name order by c.supplier_name";
									
									echo create_drop_down( "cboSupplier_".$i, 100, $sql,"id,supplier_name", 1, "--Select--", 0, "" );
								?>
                            	</td>
                                <td>
									<input name="txtrefno_<? echo $i; ?>" class="text_boxes" id="txtrefno_<? echo $i; ?>" style="width:60px" value="<? //echo $txtComposition; ?>"  />
								</td>
            ...
            <table align="center" cellspacing="0" width="1140" class="rpt_table" border="1" rules="all" id="" >
...
list($company_name,$requisition_date,$sample_team)=explode("__",$data);	
	...
	$dataArr=array();$sample_data_arr=array();
	foreach ($sql_rslt as $rows)
	{
		$sample_data_arr[mst_id][$rows[csf("sample_mst_id")]]=$rows[csf("sample_mst_id")];
		$sample_data_arr[team][$rows[csf("sample_mst_id")]]=$rows[csf("team_leader")];
		$sample_data_arr[cdate][$rows[csf("sample_mst_id")]]=$rows[csf("confirm_del_end_date")];
	...
	//------------------------------------------------tna
	$sql_actual_dtls = "select PO_NUMBER_ID,ACTUAL_START_DATE from TNA_PROCESS_MST where TASK_TYPE =5 ".where_con_using_array($sample_data_arr[mst_id],0,'PO_NUMBER_ID')."";
	$sql_actual_dtls_res = sql_select($sql_actual_dtls);
	foreach ($sql_actual_dtls_res as $rows)
	{
		if($rows[ACTUAL_START_DATE]!=''){
			$sample_team=$sample_data_arr[team][$rows[PO_NUMBER_ID]];
			$key = date("d-m-Y",strtotime($rows[ACTUAL_START_DATE]));
			
			foreach ($day_arr as $date) {
				$balance = $req_capacity_arr[$sample_team]-count($dataArr[$sample_team][$date]);
				if(strtotime($key) <= strtotime($date) && $balance > 0){
					$dataArr[$sample_team][$date][$rows[PO_NUMBER_ID]]=$rows[PO_NUMBER_ID];
					break;
				}
			}
			
			$rows[csf("confirm_del_end_date")]=$sample_data_arr[cdate][$rows[PO_NUMBER_ID]];
			$sample_key= date("d-m-Y",strtotime($rows[csf("confirm_del_end_date")]));
			unset($dataArr[$sample_team][$sample_key][$rows[PO_NUMBER_ID]]);
		}
	}
	//---------------------------------------end tna;
	//echo $sql_actual_dtls;die;
...
if ($action=="update_delivery_date")
{
	...
	list($update_id,$delivery_date_update,$delvStartDate,$company_id)=explode('__',$data);
	
	

	
	$variableSql = "select  EXCUT_SOURCE as WITHOUT_EMBELLISHMENT,EXCEED_QTY_LEVEL AS WITH_EMBELLISHMENT from variable_order_tracking where VARIABLE_LIST = 74 and STATUS_ACTIVE=1 and IS_DELETED=0 and COMPANY_NAME=$company_id";
	 //echo $variableSql;die;
	$variableSqlRes = sql_select($variableSql);
	$WITHOUT_EMBELLISHMENT=7;$WITH_EMBELLISHMENT=10;
	foreach ($variableSqlRes as $rows)
	{
		$WITHOUT_EMBELLISHMENT=$rows[WITHOUT_EMBELLISHMENT]*1;
		$WITH_EMBELLISHMENT=$rows[WITH_EMBELLISHMENT]*1;
	}	
	...
	$ddDate=($is_embellishment>0)?$WITH_EMBELLISHMENT:$WITHOUT_EMBELLISHMENT;
	

	//Teamp Capacity ........start;
	$sql = "select a.ID, a.TEAM_NAME, a.STYLE_CAPACITY,b.COMPANY_ID from lib_sample_production_team a,SAMPLE_DEVELOPMENT_MST b where product_category=6 and a.status_active=1 and a.is_deleted=0 and a.id=b.sample_team_id and b.id=$update_id";
	...
	if($delivery_date_update==0)
	{
		...
		$requisition_conf_arr=array();$sample_data_arr=array();
		foreach ($sql_requisition_conf as $rows)
		{
			...
			$sample_data_arr[mst_id][$rows[SAMPLE_MST_ID]]=$rows[SAMPLE_MST_ID];
			//$sample_data_arr[team][$rows[SAMPLE_MST_ID]]=$team_id;
			$sample_data_arr[cdate][$rows[SAMPLE_MST_ID]]=$rows[CONFIRM_DEL_END_DATE];
		}	
		
	 
	//----------------------------------------Sample Followup Report TNA
    $sql_actual_dtls = "select PO_NUMBER_ID,ACTUAL_START_DATE from TNA_PROCESS_MST where TASK_TYPE =5 ".where_con_using_array($sample_data_arr[mst_id],0,'PO_NUMBER_ID')."";
	$sql_actual_dtls_res = sql_select($sql_actual_dtls);
	foreach ($sql_actual_dtls_res as $rows)
	{
		if($rows[ACTUAL_START_DATE]!=''){
			$key = date($date_format,strtotime($rows[ACTUAL_START_DATE]));
			foreach ($day_arr as $date) {
				$balance = $req_capacity_arr[$team_id]-count($requisition_conf_arr[$date])*1;
				if(strtotime($key) <= strtotime($date) && $balance > 0){
					$requisition_conf_arr[$date][$rows[PO_NUMBER_ID]]=$rows[PO_NUMBER_ID];
					break;
				}
			}
			$rows[CONFIRM_DEL_END_DATE]=$sample_data_arr[cdate][$rows[PO_NUMBER_ID]];
			$sample_key= date($date_format,strtotime($rows[CONFIRM_DEL_END_DATE]));
			unset($requisition_conf_arr[$sample_key][$rows[PO_NUMBER_ID]]);
		}
	}
	
	
	 //print_r($requisition_conf_arr);die;
	
	//------------------------------------------end;
		
		$addDate=($is_embellishment>0)?$WITH_EMBELLISHMENT:$WITHOUT_EMBELLISHMENT;
		$extraDay=0;
		...
		if(date('l', strtotime($delvStartDate. " + $addDate day"))=='Friday'){
			...
			//echo $WITH_EMBELLISHMENT.'==';die;
			
		$newDay=$addDate;
		for($i=0; $i<= count($requisition_conf_arr)+$WITH_EMBELLISHMENT; $i++){
			$newDay2 = $newDay+$i; 
			$key=date($date_format, strtotime($delvStartDate. " + $newDay2 day"));
			$blance=$req_capacity-count($requisition_conf_arr[$key])*1;
			
			    //echo $key.','.$req_capacity.','.count($requisition_conf_arr[$key])."<br>";
	...
	{
		$sql_dtls = "select ID,DELV_START_DATE from sample_development_dtls where SAMPLE_MST_ID=$update_id and status_active=1 and is_deleted=0 and entry_form_id=341";
		$sql_rslt = sql_select($sql_dtls);
		foreach ($sql_rslt as $rows)
		{
			...
			foreach ($sql_requisition_conf as $con_rows)
			{
				...
				$sample_data_arr[mst_id][$con_rows[SAMPLE_MST_ID]]=$con_rows[SAMPLE_MST_ID];
				//$sample_data_arr[team][$rows[SAMPLE_MST_ID]]=$team_id;
				$sample_data_arr[cdate][$con_rows[SAMPLE_MST_ID]]=$con_rows[CONFIRM_DEL_END_DATE];
			}	
			...
			$sql_actual_dtls = "select PO_NUMBER_ID,ACTUAL_START_DATE from TNA_PROCESS_MST where TASK_TYPE =5 ".where_con_using_array($sample_data_arr[mst_id],0,'PO_NUMBER_ID')."";
			 ...
			 //print_r($sample_data_arr[mst_id]);die;
			
			$sql_actual_dtls_res = sql_select($sql_actual_dtls);
			foreach ($sql_actual_dtls_res as $tna_rows)
			{
				if($tna_rows[ACTUAL_START_DATE]!=''){
					$key = date($date_format,strtotime($tna_rows[ACTUAL_START_DATE]));
					foreach ($day_arr as $date) {
						$balance = $req_capacity_arr[$team_id]-count($requisition_conf_arr[$date])*1;
						if(strtotime($key) <= strtotime($date) && $balance > 0){
							$requisition_conf_arr[$date][$tna_rows[PO_NUMBER_ID]]=$tna_rows[PO_NUMBER_ID];
							break;
						}
					}
					$tna_rows[CONFIRM_DEL_END_DATE]=$sample_data_arr[cdate][$tna_rows[PO_NUMBER_ID]];
					$sample_key= date($date_format,strtotime($tna_rows[CONFIRM_DEL_END_DATE]));
					unset($requisition_conf_arr[$sample_key][$tna_rows[PO_NUMBER_ID]]);
				}
			}
			//------------------------------------------end;
...
if($action=='get_sample_requisition'){
		...
		sum(c.REQUIRED_QTY) AS REQUIRED_QTY
		 FROM SAMPLE_DEVELOPMENT_MST a,SAMPLE_DEVELOPMENT_DTLS b,SAMPLE_DEVELOPMENT_FABRIC_ACC c WHERE a.id=b.SAMPLE_MST_ID and b.SAMPLE_MST_ID=c.SAMPLE_MST_ID  and a.entry_form_id=341 and c.FORM_TYPE=1 and a.ID='$data'
