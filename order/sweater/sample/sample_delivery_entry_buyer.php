<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create sample delivery entry for sweater
...
function fnc_sample_delivery_entry(operation)
{
	...
	if(operation==4)
	{
		  ...
		  if(ids=="")
		  {
 		  ...
 		  print_report( $('#mst_update_id').val()+'*'+ids+'*'+$('#cbo_company_name').val()+'*'+$('#cbo_sample_name').val()+'*'+$('#cbo_item_name').val()+'*'+req_ids+'*'+$('#hidden_sample_dtls_tbl_id').val(), "delivery_print", "requires/sample_delivery_entry_buyer_controller" )  
			 return;
	}
...
function fnc_sample_delivery_entry_Reply_info()
{
 	if(http.readyState == 4) 
	{
		...
		if(response[0]==0)//insert response;
		{
			...
			show_list_view(response[2]+'*'+response[1],'show_dtls_listview','list_view_container','requires/sample_delivery_entry_buyer_controller','');
			...
			$('#breakdown_td_id').html('');
  			var val =return_global_ajax_value( response[1]+"__"+response[4]+"__"+response[2]+'__'+$('#cbo_sample_name').val()+"__"+$('#cbo_item_name').val(), 'populate_data_yet_to_cut', '', 'requires/sample_delivery_entry_buyer_controller');
...
function openmypage(page_link,title)
{
		...
		emailwindow.onclose=function()
		{
			...
			if (smp_id!="")
			{
				...
				get_php_form_data(smp_id+"**1", "populate_data_from_search_popup", "requires/sample_delivery_entry_buyer_controller" );
				
				show_list_view(smp_id,'show_sample_item_listview','list_view_country','requires/sample_delivery_entry_buyer_controller','');		
				
				//show_list_view(smp_id,'show_dtls_listview','list_view_container','requires/sample_delivery_entry_buyer_controller','');
...
function put_sample_item_data(sample_dtls_part_tbl_id,smp_id,gmts)
{
	...
	//freeze_window(5);
 	get_php_form_data(sample_dtls_part_tbl_id+'**'+smp_id+'**'+req_id+'**'+gmts, "color_and_size_level", "requires/sample_delivery_entry_buyer_controller" ); 
    set_button_status(0, permission, 'fnc_sample_delivery_entry',1,0);
	//release_freezing();
}
...
{
	reset_form('','','txt_delivery_qty*txt_carton_qnty*txt_remark','','');
}
...
function ex_factory_sys_popup()
{
	...
	*/
	var page_link="requires/sample_delivery_entry_buyer_controller.php?action=sys_surch_popup&company="+$("#cbo_company_name").val();
	...
	emailwindow.onclose=function()
	{
		...
		show_list_view(smp_id,'show_dtls_listview','list_view_container','requires/sample_delivery_entry_buyer_controller','');
		get_php_form_data(data_format+"**2", "populate_data_from_search_popup", "requires/sample_delivery_entry_buyer_controller" );
                ...
                <fieldset>
                    ...
                    <tr>
                      ...
                      <tr>
                        ...
                        <td width="">
                            <?
                            echo create_drop_down( "cbo_company_name", 172, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", '', "load_drop_down( 'requires/sample_delivery_entry_buyer_controller', this.value, 'load_drop_down_location', 'location_td' );",0 ); ?>
                        </td>
                        ...
                        <td > 
						<?
						$delivery_to = array(1=>'Marketing & Merchandising'); 
                ...
                <table cellpadding="0" cellspacing="1" width="100%">
                    <tr>
                    	<td width="35%" valign="top">
                            ...
                            <td>
								<input name="txt_sample_requisition_id" placeholder="Double Click to Search" id="txt_sample_requisition_id" onDblClick="openmypage('requires/sample_delivery_entry_buyer_controller.php?action=sample_requisition_popup&company='+document.getElementById('cbo_company_name').value,'Sample Requisition ID')"  class="text_boxes" style="width:138px " readonly>
								...
								<input type="hidden" id="hidden_requisition_id" />	 
                              	...
                              	<input type="hidden" name="hidden_sample_dtls_tbl_id" id="hidden_sample_dtls_tbl_id" value="">
                                   ...
                                   <tr>
                                        ...
                                        <td width="" valign="top"><input name="txt_delivery_qty" id="txt_delivery_qty" class="text_boxes_numeric"  style="width:138px" >
