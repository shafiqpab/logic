...
<script>
	...
	function fnc_time_weight_record_entry( operation )
	{
		...
		if (form_validation('cbo_company_name*cbo_location_name*cbo_buyer_name*txt_style_ref*cbo_product_department*cbo_gmts_item*cbo_item_catgory*cbo_team_leader*cbo_dealing_merchant*cbo_sample_type*txt_sample_color*txt_sample_size*txtknitingweight_dzn','Company Name*Location Name*Buyer Name*Style Ref*Product Department*Garments Item*Product Catagory*Team Leader*Dealing Merchant*Style ID*Sample Type*Sample Color*Sample Size*Knitting Weight per Dzn')==false)
		{
			release_freezing();
			return;
		}	
		else
		{
			...
			for(var i=1; i<=panel_tr; i++)
			{
				...
				data_panel += '&txtpanelupid_' + i + '=' + trim($('#txtpanelupid_'+i).val()) + '&txtpanelid_' + i + '=' + trim($('#txtpanelid_'+i).val())+ '&txtminute_' + i + '=' + trim($('#txtminute_'+i).val()) + '&txtsecond_' + i + '=' + trim($('#txtsecond_'+i).val()) + '&txtmovingsec_' + i + '=' + trim($('#txtmovingsec_'+i).val()) + '&txtknitinglbs_' + i + '=' + trim($('#txtknitinglbs_'+i).val())+ '&txtknitinggm_' + i + '=' + trim($('#txtknitinggm_'+i).val()); 
			}
	...
	function openmypage_style_no(page_link,title)
	{
	...
	function fnc_color_dtls(operation)
	{
		...
		//var color_data=color_body_data="";
		for(var k=1; k<=j; k++)
		{
			var bodypartid_data="";
			//color_data+=get_submitted_data_string('txtcolor_'+k+'*txtbodycolorval_'+k+'*txtbodycolorper_'+k,"../../",3);
			color_data += '&txtcolor_' + k + '=' + trim($('#txtcolor_'+k).val())+ '&txtbodycolorvallbs_' + k + '=' + trim($('#txtbodycolorvallbs_'+k).val()) + '&txtbodycolorval_' + k + '=' + trim($('#txtbodycolorval_'+k).val())+ '&txtbodycolorper_' + k + '=' + trim($('#txtbodycolorper_'+k).val()); 
			
			for(var m=1; m<=txtbodyc; m++)
			{
				//color_body_data+=get_submitted_data_string('txtbodycolor_'+k+'_'+m,"../../",3);
				color_body_data += '&txtbodycolor_'+k+'_'+m + '=' + trim($('#txtbodycolor_'+k+'_'+m).val())+'&txtbodycolorlbs_'+k+'_'+m + '=' + trim($('#txtbodycolorlbs_'+k+'_'+m).val());
				
				//bodypartid_data+=get_submitted_data_string('bodypartid_'+m,"../../",3);
				bodypartid_data += '&bodypartid_'+m + '=' + trim($('#bodypartid_'+m).val());
			}
		}
		...
		var data="action=save_update_delete_colordtls&operation="+operation+get_submitted_data_string('txtbodyc*update_id*updatedtls_id',"../../")+color_data+bodypartid_data+color_body_data;
	...
	{
		var panel_tr =$('#tbl_panel tbody tr').length; 
		var tot_minute=0; var tot_second=0; var moving_speed=0; var knitting_weight_gm=0; var knitting_weight_lbs=0;  var k=0;
		for(var i=1; i<=panel_tr; i++)
		{
			var txtminute=0; var txtsecond=0; var txtmovingsec=0; var txtknitinggm=0; var txtknitinglbs=0;
			
			txtminute=$('#txtminute_'+i).val()*1;
			txtsecond=$('#txtsecond_'+i).val()*1;
			if(($('#txtmovingsec_'+i).val()*1)!=0) { txtmovingsec=$('#txtmovingsec_'+i).val()*1; k++; }
			txtknitinglbs=$('#txtknitinglbs_'+i).val()*1;
			txtknitinggm=$('#txtknitinggm_'+i).val()*1;
			
			tot_minute+=txtminute;
			tot_second+=txtsecond;
			moving_speed+=txtmovingsec;
			knitting_weight_lbs+=txtknitinglbs;
			knitting_weight_gm+=txtknitinggm;
		}
		//alert(knitting_weight_lbs)
		...
		$('#txtknitinglbs_tot').val( number_format(knitting_weight_lbs,4,'.','') );
	...
	function fnc_tot_per_cal()
	{
		...
		var j=14; var gtot=0; var gtotlbs=0;
		for(var k=1; k<=j; k++)
		{
			var color_totlbs=0; var color_tot=0;
			for(var m=1; m<=txtbodyc; m++)
			{
				color_totlbs+=$('#txtbodycolorlbs_'+k+'_'+m).val()*1;
				color_tot+=$('#txtbodycolor_'+k+'_'+m).val()*1;
			}
			$('#txtbodycolorvallbs_'+k).val( number_format(color_totlbs,4,'.','') );
			$('#txtbodycolorval_'+k).val( number_format(color_tot,4,'.','') );
			gtotlbs+=color_totlbs;
			gtot+=color_tot;
		}
		$('#txtcolortotlbs').val( number_format(gtotlbs,4,'.','') );
		...
		for(var m=1; m<=txtbodyc; m++)
		{
			var colorlbs_sum=0; var color_sum=0;
			for(var k=1; k<=j; k++)
			{
				colorlbs_sum+=$('#txtbodycolorlbs_'+k+'_'+m).val()*1;
				color_sum+=$('#txtbodycolor_'+k+'_'+m).val()*1;
			}
			//$('#txtbodytot_'+m).val( color_sum );
			document.getElementById('txtbodytotlbs_'+m).value = number_format( colorlbs_sum,4,'.','');
			$('#txtbodytotlbs_'+m).attr('total_value',number_format(colorlbs_sum,4,'.',''));
	...
	function fnc_variable_settings_check(company_id)
	{
		...
		if(set_smv_id==10){
			...
			var page_link="'requires/time_weight_record_controller.php?action=sample_requisition_popup','Sample Requisition Selection Form'";// for Sample Requisition
			$('#txt_style_ref').removeAttr("onDblClick").attr("onDblClick","open_smv_popup("+page_link+");");
		}
		else{
	...
	function open_smv_popup(page_link,title)
	{
		if( form_validation('cbo_company_name','Company Name')==false)
		{
			return;
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				if (theemail.value!="")
				{
					...
					if(set_smv_id==10)
					{
						get_php_form_data( theemail.value, "populate_data_from_sample_requisition", "requires/time_weight_record_controller" );
					}
					release_freezing();
				}
	...
	function fnc_lbs_gm_calculate(type,inc)
	{
		if(type==1)
		{
			var lbs=($('#txtknitinglbs_'+inc).val()*1)*453.592;
			$('#txtknitinggm_'+inc).val( number_format(lbs,4,'.','' ) );
		}
		else
		{
			var gm=($('#txtknitinggm_'+inc).val()*1)/453.592;
			$('#txtknitinglbs_'+inc).val( number_format(gm,4,'.','' ) );
		}
		//fnc_panel_sum();
	}
	
	
	function fncbodylbsgm_calculate(type,inc)
	{
		if(type==1)
		{
			var lbs=($('#txtbodycolorlbs_'+inc).val()*1)*453.592;
			$('#txtbodycolor_'+inc).val( number_format(lbs,4,'.','' ) );
		}
		else
		{
			var gm=($('#txtbodycolor_'+inc).val()*1)/453.592;
			$('#txtbodycolorlbs_'+inc).val( number_format(gm,4,'.','' ) );
		}
		//fnc_panel_sum();
	}
            ...
            <fieldset style="width:840px;">
                ...
                <table width="840" cellspacing="2" cellpadding="0" border="0">
                	<tr>
                        ...
                        <td colspan="3">
                        	<input style="width:140px;" type="text" title="Double Click to Search" onDblClick="openmypage_style_no('requires/time_weight_record_controller.php?action=style_popup','Style Selection Form')" class="text_boxes" placeholder="Browse Style" name="txt_style_no" id="txt_style_no" readonly />
                    ...
                    <tr>
                        ...
                        <td><input class="text_boxes" type="text" style="width:140px" placeholder="Write" name="txt_style_ref" id="txt_style_ref"/><input type="hidden" id="mst_ref_id"/></td>
                        ...
                        <td><? echo create_drop_down( "cbo_gmts_item", 150, $garments_item,"", 1, "--Gmts Item--", $selected, "" ); ?></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                    	<td>Remarks</td>
                        <td colspan="5"><input class="text_boxes" type="text" style="width:415px;" name="txt_remarks" id="txt_remarks"/></td>
                    </tr>
                    <tr>
                        <td>&nbsp;</td>
            ...
            <form id="timeweightentry_2" autocomplete="off">
                <table width="820" cellspacing="2" cellpadding="0" border="0">
                	 <tr>
                        ...
                        <td width="100"><? echo create_drop_down( "cbo_sample_type", 100, "select sample_name,id from lib_sample where is_deleted=0 and status_active=1 order by sample_name","id,sample_name", '1', "--Select--", '', "",'','' ); ?></td>
                 ...
                 <table width="820" cellspacing="2" cellpadding="0" border="1" rules="all" class="rpt_table" id="tbl_panel">
                	 <thead>
                    	<tr>
                            ...
                            <th width="80" rowspan="2">Knitting Weight [LBS]</th>
                            <th rowspan="2">Knitting Weight [GM]</th>
                        </tr>
                        <tr>
                    	...
                    	<?
							...
							foreach($time_weight_panel as $arrid=>$val)
							{
                                ...
                                <tr id="tr_<?=$i; ?>" bgcolor="<?=$bgcolor; ?>" style="text-decoration:none;cursor:pointer">
                                	<td align="center"><?=$i; ?><input type="hidden" id="txtpanelupid_<?=$i; ?>" value="" style="width:50px;"></td>
                                    <td><?=$val; ?><input type="hidden" id="txtpanelid_<?=$i; ?>" value="<?=$arrid; ?>" style="width:50px;"></td>
                                    <td align="center"><input type="text" id="txtminute_<?=$i; ?>" name="txtminute_<?=$i; ?>" style="width:80px;" class="text_boxes_numeric" onChange="fnc_panel_sum();"></td>
                                    <td align="center"><input type="text" id="txtsecond_<?=$i; ?>" name="txtsecond_<?=$i; ?>" style="width:80px;" class="text_boxes_numeric" onChange="fnc_panel_sum();"></td>
                                    <td align="center"><input type="text" id="txtmovingsec_<?=$i; ?>" name="txtmovingsec_<?=$i; ?>" style="width:80px;" class="text_boxes_numeric" onChange="fnc_panel_sum();"></td>
                                    <td align="center"><input type="text" id="txtknitinglbs_<?=$i; ?>" name="txtknitinglbs_<?=$i; ?>" style="width:60px;" class="text_boxes_numeric" onBlur="fnc_lbs_gm_calculate(1,<?=$i; ?>);" onChange="fnc_panel_sum();"></td>
                                    <td align="center"><input type="text" id="txtknitinggm_<?=$i; ?>" name="txtknitinggm_<?=$i; ?>" style="width:60px;" class="text_boxes_numeric"  onBlur="fnc_lbs_gm_calculate(2,<?=$i; ?>);" onChange="fnc_panel_sum();"></td>
                                </tr>
                                <?
								$i++;
							}
                    ...
                    <tfoot>
                    	<tr class="general">
                            ...
                            <td><input type="text" id="txtknitinglbs_tot" name="txtknitinglbs_tot" style="width:60px;" class="text_boxes_numeric" readonly></td>
                            <td><input type="text" id="txtknitinggm_tot" name="txtknitinggm_tot" style="width:60px;" class="text_boxes_numeric" readonly></td>
                        </tr>
                        <tr>
                            <td>&nbsp;</td>
                            ...
                            <td colspan="2" align="right">Knitting Weight per Dzn/Lbs</td>
                        ...
                        <tr>
                            <td colspan="7" valign="middle" align="center" class="button_container">
