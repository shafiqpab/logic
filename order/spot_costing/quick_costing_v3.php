...
<script>
	...
	function openmypage_temp(page_link,title)
	{
		...
		{
			load_drop_down( 'requires/quick_costing_v3_controller', '', 'load_drop_template_name', 'template_td');
		}
	}
	...
	function fnc_template_view()
	{
		...
		var item_id=return_ajax_request_value(temp_id, 'load_lib_item_id', 'requires/quick_costing_v3_controller');
		load_drop_down( 'requires/quick_costing_v3_controller', item_id, 'load_drop_down_tempItem', 'item_td');
		//alert(item_id);
	...
	function fnc_summary_dtls(val)
	{
        ...
        html += '<td id="totLbTstCst_td" align="right">&nbsp;</td>';
        	html +='<tr><td>Storage Cost</td>';

		var ac=0;
		for(var aw=1; aw<=gmtName.length; aw++)
		{
            html += '<td align="center"><input name="txtstorage_[]" id="txtstorage_'+trim(gmtId[ac])+'" onChange="fnc_amount_calculation(12,'+trim(gmtId[ac])+',0);" value="" class="text_boxes_numeric" style="width:40px;"  placeholder="Write" /></td>';
			ac++;
		}
		html += '<td id="totStorage_td" align="right">&nbsp;</td></tr>';

		html += '<tr><td>Operating Expense</td>';

		var ac=0;
		for(var aw=1; aw<=gmtName.length; aw++)
		{
            html += '<td align="center"><input name="txtoperatingexpense_[]" id="txtoperatingexpense_'+trim(gmtId[ac])+'" onChange="fnc_amount_calculation(13,'+trim(gmtId[ac])+',0);" value="" class="text_boxes_numeric" style="width:40px;"  placeholder="Write" /></td>';
			ac++;
		}
		html += '<td id="totOperatingExpense_td" align="right">&nbsp;</td></tr>';

       html +='</tr><tr><td>Mis/Offer Qty.&nbsp; <input name="txt_lumSum_cost" id="txt_lumSum_cost" value="" class="text_boxes_numeric" style="width:27px;" title="Lum Sum Cost" placeholder="L.S" onChange="fnc_amount_calculation(7,0,0);" /></td>';
		var aa=0;
		...
		html += '<td id="totOtherCst_td" align="right">&nbsp;</td></tr><tr><td title="Commercial Cost">Comml.($/DZN)<input type="text" name="txt_commlPer" id="txt_commlPer" value="" class="text_boxes_numeric" style="width:20px;" placeholder="%" onChange="fnc_amount_calculation(11,0,0);" /></td>';
        var p=0;
        ...
        html += '<td id="totCommlCst_td" align="right">&nbsp;</td></tr><tr><td title="Commission Cost">Com.(%)($/DZN)<input name="txt_commPer" id="txt_commPer" value="" class="text_boxes_numeric" style="width:20px;" title="Com(%)($/DZN)" placeholder="%" onChange="fnc_amount_calculation(9,0,0);" /></td>';
        var p=0;
		for(var y=1; y<=gmtName.length; y++)
		{
		...
		html += '<td id="totRmgQty_td" align="right">&nbsp;</td></tr>';

	

		html += '<tr><td>Remark</td>';

		var ac=0;
		for(var aw=1; aw<=gmtName.length; aw++)
		{
            html += '<td align="center" ><input name="txtremark_[]" id="txtremark_'+trim(gmtId[ac])+'" value="" class="text_boxes" style="width:60px;"  placeholder="Write" /></td>';
			ac++;
		}
		html += '<td >&nbsp;</td></tr>';
	...
	function fnc_dtls_ganerate(val)
	{
		...
		$('#tbl_wash tbody tr:not(:first)').remove();
		$('#txtWashTypetext_1').val('');
		$('#txtWbodypartid_1').val('');
		$('#txtWbodyparttext_1').val('');
		$('#txtWbodypartid_1').val('');
		$('#txtWConsumtion_1').val('');
		$('#txtWexper_1').val('');
		$('#txtTotWConsumtion_1').val('');
		$('#txtwRate_1').val('');
		$('#txtWValue_1').val('');
		$('#txtWashRemarks_1').val('');
		...
		/*for(var q=1; q<=acc; q++)
		{
			...
			$('#txtAccRate_'+q).val('');
			$('#txtAccValue_'+q).val('');
			$('#txtAccRemarks_'+q).val('');
		}*/
		
		$('#tbl_acc tbody tr:not(:first)').remove();
		
		$('#txtAcctext_1').val('');
		$('#txtAccId_1').val('');
		$('#txtAccDescription_1').val('');
		$('#txtAccBandRef_1').val('');
		$('#txtAccConsumtion_1').val('');
		$('#txtAcexper_1').val('');
		$('#txtTotAccConsumtion_1').val('');
		$('#cboconsuom_1').val(0);
		$('#txtAccRate_1').val('');
		$('#txtAccValue_1').val('');
		$('#txtAccRemarks_1').val('');
		...
		if(itemConsRateData!="")
		{
			for(var m=1; m<=f; m++)
			{
				...
				var bodypart=bodypartid=cons=exper=totCons=ratePop=rate=fabVal=fabRemarks=fabupid="";
				
				var bodypart=itemConsRate_fab[0];
				var bodypartid=itemConsRate_fab[1];
				var cons=itemConsRate_fab[2];
				var exper=itemConsRate_fab[3];
				var totCons=itemConsRate_fab[4];
				var ratePop=itemConsRate_fab[5];
				var rate=itemConsRate_fab[6];
				var fabVal=itemConsRate_fab[7];
				var fabRemarks=itemConsRate_fab[8];
				var fabupid=itemConsRate_fab[9];
				...
				else if(ratePop==1)
				{
					...
					$("#ratePop_"+m).val(1);
					$('#txt_rate'+m).removeAttr("onDblClick").attr("onDblClick","fnc_openmypage_rate("+m+",'"+totCons+"');");
					$('#txt_rate'+m).attr('readonly','readonly');
					$('#txt_rate'+m).attr("placeholder", "Browse");
					var avgRate=(fabVal*1)/(totCons*1);
					$("#txt_rate"+m).attr('fabactualrate',avgRate);
				}
				
				$('#txt_rate'+m).val(rate);
				$('#txt_value'+m).val(fabVal);
				$('#txtremarks_'+m).val(fabRemarks);
		...
		if(itemAccData!="")
		{
			for(var q=1; q<=accs; q++)
			{
				...
				if(itemAccData[adata]!=undefined)
				{
					...
					var acctext=accId=accDescription=accBandRef=accConsumtion=acexper=totAccConsumtion=acRate=acValue=acRemarks=acconsUom="";
					...
					$('#txtAccRemarks_'+q).val(acRemarks);
					adata++;
				}
			}
		}
	...
	function fnc_amount_calculation(type,inc_id,rate)
	{
		...
		if (type=='fabric')
		{
			...
			{
				rate=($("#txt_rate"+inc_id).attr('fabactualrate')*1);
			}
			else
			{
				if(rate=="" || rate==0)
			...
			$("#txt_totConsumtion"+inc_id).val( number_format(ex_fab_cons,4,'.','') );
			//alert(ex_fab_cons);
			var row_tot_value=((ex_fab_cons)*rate);
			
			$("#txt_value"+inc_id).val( number_format(row_tot_value,4,'.','') );
		...
		{
			var cons_val=''; var row_tot_value=''; var ex_acper=""; var accRate="";
			...
			$("#txtTotAccConsumtion_"+inc_id).val( number_format(ex_ac_cons,4,'.','') );
			accRate=$("#txtAccRate_"+inc_id).val()*1;
			row_tot_value=(ex_ac_cons*1)*accRate;
		...
		else if (type=='3')
		{
			...
			if($("#cmPop").val()==1)
			{
				...
				if(cc==1)
				{
					//var cpm_from_financial_parameter=return_ajax_request_value(costing_date, 'cpm_check_load', 'requires/quick_costing_v3_controller');
					cc++;
				}
		...
		else if (type=='9')
		{
			...
			for(var bb=1; bb<=all_item_id_hid.length; bb++)
			{
				...
				before_commission_cost=($("#fab_td"+trim(all_item_id_hid[aa])).text()*1)+($("#spOpe_td"+trim(all_item_id_hid[aa])).text()*1)+($("#wash_td"+trim(all_item_id_hid[aa])).text()*1)+($("#acc_td"+trim(all_item_id_hid[aa])).text()*1)+($("#txtCmCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtFriCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtLtstCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtMissCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtOtherCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtCommlCost_"+trim(all_item_id_hid[aa])).val()*1);
		...
		else if (type=='11')
		{
			...
			for(var bb=1; bb<=all_item_id_hid.length; bb++)
			{
				...
				before_commercial_cost=($("#fab_td"+trim(all_item_id_hid[aa])).text()*1)+($("#spOpe_td"+trim(all_item_id_hid[aa])).text()*1)+($("#wash_td"+trim(all_item_id_hid[aa])).text()*1)+($("#acc_td"+trim(all_item_id_hid[aa])).text()*1)+($("#txtCmCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtFriCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtLtstCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtMissCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtOtherCost_"+trim(all_item_id_hid[aa])).val()*1);
		...
		{
			var ab=0; var storage=0;
			for(var bc=1; bc<=all_item_id_hid.length; bc++)
			{
				storage=storage+($("#txtstorage_"+trim(all_item_id_hid[ab])).val()*1);
				ab++;
			}
			$("#totStorage_td").text( number_format(storage,4,'.',''));
		}
		else if (type=='13')
		{
			var ab=0; var operatingexpense=0;
			for(var bc=1; bc<=all_item_id_hid.length; bc++)
			{
				operatingexpense=operatingexpense+($("#txtoperatingexpense_"+trim(all_item_id_hid[ab])).val()*1);
				ab++;
			}
			$("#totOperatingExpense_td").text( number_format(operatingexpense,4,'.',''));
		}
		//alert(all_item_id_hid)
		var aa=0; var itemFoBarr=new Array(); var val_td_fob=0; var totCommercial_Cost=totCommiss_Cost=0;
		for(var bb=1; bb<=all_item_id_hid.length; bb++)
		{
			...
			before_commercial_cost=($("#fab_td"+trim(all_item_id_hid[aa])).text()*1)+($("#spOpe_td"+trim(all_item_id_hid[aa])).text()*1)+($("#wash_td"+trim(all_item_id_hid[aa])).text()*1)+($("#acc_td"+trim(all_item_id_hid[aa])).text()*1)+($("#txtCmCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtFriCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtLtstCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtMissCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtOtherCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtstorage_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtoperatingexpense_"+trim(all_item_id_hid[aa])).val()*1);
		...
		//alert(fob_rmg);
		var tot_cost=($("#totFab_td").text()*1)+($("#totSpc_td").text()*1)+($("#totWash_td").text()*1)+($("#totAcc_td").text()*1)+($("#totCm_td").text()*1)+($("#totFriCst_td").text()*1)+($("#totLbTstCst_td").text()*1)+($("#totMissCst_td").text()*1)+($("#totOtherCst_td").text()*1)+($("#totCommlCst_td").text()*1)+($("#totCommCst_td").text()*1)+($("#totStorage_td").text()*1)+($("#totOperatingExpense_td").text()*1);
		var tot_costPcs=tot_cost/12;
		...
		var margincalculate=tot_costPcs*(($("#txtmarignper").val()*1)/100);
		
		$("#txtmarign").val( number_format(margincalculate,4,'.',''))
	...
	function fnc_meeting_remarks_pop_up(costSheetId,styleRef)
	{
		...
		var page_link = 'requires/quick_costing_v3_controller.php?action=meeting_remarks_popup';
	...
	function fnc_openmypage_rate(inc_id,cons)
	{
		...
		var totCons=$('#txt_totConsumtion'+inc_id).val();
		
		var title = 'Rate Details PopUp';	
		var page_link = 'requires/quick_costing_v3_controller.php?action=rate_details_popup';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link+'&qc_no='+qc_no+'&bodypartid='+bodypartid+'&totCons='+totCons+'&consRateId='+consRateId, title, 'width=850px,height=380px,center=1,resize=0,scrolling=0','../')
		release_freezing();
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0];
			var cons=this.contentDoc.getElementById("txt_tot_cons").value;
			var tot_cost=this.contentDoc.getElementById("txt_tot_tot_cost").value;	 //Access form field with id="emailfield"
			var totamount=this.contentDoc.getElementById("txttotamount").value;
			
			var avgCons=(((tot_cost*1)-(cons*1))/cons)*100;
			
			var avgRate=(totamount*1)/(tot_cost*1);
			$('#txt_consumtion'+inc_id).val( number_format(cons,4,'.','') );
			$('#txt_exper'+inc_id).val( number_format(avgCons,4,'.','') );
			$('#txt_totConsumtion'+inc_id).val( number_format(tot_cost,4,'.','') );
			$("#txt_rate"+inc_id).attr('fabactualrate',avgRate);
			$('#txt_rate'+inc_id).val( number_format(avgRate,4,'.','') );
			$('#txt_value'+inc_id).val( number_format(totamount,4,'.','') );
			fnc_amount_calculation('fabric', inc_id, '');
			fnc_amount_calculation(11, inc_id, '');
		}
	}
	...
	function fnc_new_stage_popup()
	{
		...
		var page_link = 'requires/quick_costing_v3_controller.php?action=stage_saveUpdate_popup';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=600px,height=380px,center=1,resize=0,scrolling=0','../')
		release_freezing();
		emailwindow.onclose=function()
		{
			load_drop_down( 'requires/quick_costing_v3_controller', '', 'load_drop_stage_name', 'stage_td');
		}
	}
	...
	function fnc_itemwise_data_cache()
	{
		...
		for(var m=1; m<=f; m++)
		{
			fabData+= ($('#txtbodyparttext_'+m).val())+'_'+($('#txtbodypartid_'+m).val()*1)+'_'+($('#txt_consumtion'+m).val()*1)+'_'+($('#txt_exper'+m).val()*1)+'_'+($('#txt_totConsumtion'+m).val()*1)+'_'+($('#ratePop_'+m).val()*1)+'_'+($('#txt_rate'+m).val()*1)+'_'+($('#txt_value'+m).val()*1)+'_'+($('#txtremarks_'+m).val())+'_'+($('#txtfabupid_'+m).val())+'##';
		}
		...
		for(var q=1; q<=acc; q++)
		{
			accessoriesData+= ($('#txtAcctext_'+q).val())+'_'+($('#txtAccId_'+q).val()*1)+'_'+($('#txtAccDescription_'+q).val())+'_'+($('#txtAccBandRef_'+q).val())+'_'+($('#txtAccConsumtion_'+q).val()*1)+'_'+($('#txtAcexper_'+q).val()*1)+'_'+($('#txtTotAccConsumtion_'+q).val()*1)+'_'+($('#txtAccRate_'+q).val()*1)+'_'+($('#txtAccValue_'+q).val()*1)+'_'+($('#txtAccRemarks_'+q).val())+'_'+($('#cboconsuom_'+q).val())+'##';
		}
	...
	function fnc_qcosting_entry( operation )
	{
		...
		if (form_validation('cbo_company_name*cbo_temp_id*cbo_cons_basis_id*txt_exchangeRate*cbo_buyer_id*cbo_brand*txt_styleRef*txt_costingDate*txt_offerQty','Company Name*Template Name*Cons. Basis*Exchange Rate*Buyer Name*Brand*Style Ref.*Costing Date*Offer Qty.')==false)
		{
			release_freezing();
			return;
		}	
		else
		{
			if(operation==4)
			{
				...
				generate_report_file( $('#hid_qc_no').val()+'*'+$('#txt_costSheetNo').val()+'*'+report_title,'quick_costing_print','requires/quick_costing_v3_controller');
				release_freezing();
				return;
			}
			else
	...
	function fnc_qcosting_entry_response()
	{
		if(http.readyState == 4) 
		{
			...
			//{
				...
				{
					var temp_style_list=return_ajax_request_value(reponse[2]+'__'+1, 'temp_style_list_view', 'requires/quick_costing_v3_controller');
					$('#style_td').html( temp_style_list );
					var user_id='<? echo $_SESSION['logic_erp']['user_id']; ?>';
					//get_php_form_data(reponse[1]+"__"+user_id+"__"+reponse[2]+"__"+0+"***"+0, 'populate_style_details_data', 'requires/quick_costing_v3_controller');
					load_drop_down('requires/quick_costing_v3_controller', reponse[2]+'__'+reponse[4]+'__'+reponse[3], 'load_drop_down_revise_no', 'revise_td');
					load_drop_down('requires/quick_costing_v3_controller', reponse[2]+'__'+reponse[4]+'__'+reponse[4], 'load_drop_down_option_id', 'option_td');
					set_onclick_style_list(reponse[2]+'__'+user_id+'__'+reponse[2]);
				}
				if(reponse[5]==6) //Revise
				{
					load_drop_down('requires/quick_costing_v3_controller', reponse[2]+'__'+reponse[4]+'__0', 'load_drop_down_revise_no', 'revise_td');
					load_drop_down('requires/quick_costing_v3_controller', reponse[2]+'__'+reponse[4]+'__'+reponse[4], 'load_drop_down_option_id', 'option_td');
					set_onclick_style_list(reponse[2]+'__'+user_id+'__'+reponse[2]);
				}
				else if(  reponse[5]==7) //Option
				{	
					load_drop_down('requires/quick_costing_v3_controller', reponse[2]+'__'+reponse[4]+'__'+$('#cbo_revise_no').val(), 'load_drop_down_revise_no', 'revise_td');
					load_drop_down('requires/quick_costing_v3_controller', reponse[2]+'__'+reponse[4]+'__0', 'load_drop_down_option_id', 'option_td');
			...
			var temp_style_list=return_ajax_request_value(reponse[2]+'__'+0, 'temp_style_list_view', 'requires/quick_costing_v3_controller');
			$('#style_td').html( temp_style_list );
			//alert(reponse[7]);
	...
	{
		window.open("requires/quick_costing_v3_controller.php?data=" + data+'&action='+action, true );
	}
	...
	function openmypage_style(type)
	{
		if( form_validation('cbo_company_name','Company Name')==false)
		{
			...
			var page_link='requires/quick_costing_v3_controller.php?action=style_popup';
			var title="Style Search Popup";
			var data=$('#cbo_buyer_id').val()+'__'+type+'__'+$('#cbo_company_name').val();
			var user_id='<? echo $_SESSION['logic_erp']['user_id']; ?>';
			var k=1;
			...
			emailwindow.onclose=function()
			{
				...
				if(style_id!="")
				{
					var temp_style_list=return_ajax_request_value(cost_sheet_no+'__'+1, 'temp_style_list_view', 'requires/quick_costing_v3_controller');
					$('#style_td').html( temp_style_list );
					...
					for( var i=0; i< str.length; i++ ) {
						if( k==1)
						{
							...
							load_drop_down('requires/quick_costing_v3_controller', cost_sheet_no+'__0__0', 'load_drop_down_option_id', 'option_td');
							load_drop_down('requires/quick_costing_v3_controller', cost_sheet_no+'__'+$('#cbo_option_id').val()+'__0', 'load_drop_down_revise_no', 'revise_td');
							k++;
						}
					}
				}
				//alert(temp_style_list)
				//$('#style_td').html( $('#style_td').html() +""+ return_ajax_request_value(style_id, 'temp_style_list_view', 'requires/quick_costing_v3_controller') );
				//show_list_view( style_id,'temp_style_list_view','style_td','requires/quick_costing_v3_controller','',1);//setFilterGrid(\'tbl_po_list\',-1)
	...
	function set_onclick_style_list( data )
	{
		...
		if(datas[3]=="25")
		{
			...
			load_drop_down('requires/quick_costing_v3_controller', datas[2]+'__0__0', 'load_drop_down_option_id', 'option_td');
			load_drop_down('requires/quick_costing_v3_controller', datas[2]+'__'+$('#cbo_option_id').val()+'__0', 'load_drop_down_revise_no', 'revise_td');
			...
			get_php_form_data(datas[0]+"__"+datas[1]+"__"+datas[2]+"__"+val, 'populate_style_details_data', 'requires/quick_costing_v3_controller');
	...
	function fnc_delete_style_row()
	{
		...
		var temp_style_list=return_ajax_request_value(hid_qc_no+'__'+3, 'temp_style_list_view', 'requires/quick_costing_v3_controller');
		$('#style_td').html( temp_style_list );
		reset_fnc();
		/*if( (td*1)>1 )
		{
	...
	{
		var user_id='<? echo $_SESSION['logic_erp']['user_id']; ?>';
		get_php_form_data(($("#hid_qc_no").val()*1)+'__'+user_id+'__'+0, 'populate_style_details_data', 'requires/quick_costing_v3_controller');
	}
	...
	function fnc_clear_all()
	{
		...
		{
			var style_id='';
			var temp_style_list=return_ajax_request_value(style_id+'__'+2, 'temp_style_list_view', 'requires/quick_costing_v3_controller');
			$('#style_td').html('');
			reset_fnc();
		}
	}
	...
	function fnc_confirm_style()
	{
		...
		{
			var data=$('#hid_qc_no').val()+'__'+$('#txt_update_id').val()+'__'+$('#cbo_company_name').val();
			var page_link='requires/quick_costing_v3_controller.php?action=confirmStyle_popup';
			...
			emailwindow.onclose=function()
			{
				/*var theform=this.contentDoc.forms[0];
				var style_id=this.contentDoc.getElementById("hide_style_id").value; // product ID
				var temp_style_list=return_ajax_request_value(style_id+'__'+1, 'temp_style_list_view', 'requires/quick_costing_v3_controller');
				$('#style_td').html( temp_style_list );*/
				//alert(temp_style_list)
				//$('#style_td').html( $('#style_td').html() +""+ return_ajax_request_value(style_id, 'temp_style_list_view', 'requires/quick_costing_v3_controller') );
				//show_list_view( style_id,'temp_style_list_view','style_td','requires/quick_costing_v3_controller','',1);//setFilterGrid(\'tbl_po_list\',-1)
	...
	function fnc_option_rev( val )
	{
		...
		get_php_form_data(hid_qc_no+"__"+user_id+"__"+cost_no+"__"+val+"__from_option", 'populate_style_details_data', 'requires/quick_costing_v3_controller');
	}
	...
	function openmypage_agent_location(page_link,title,type)
	{
		...
		emailwindow.onclose=function()
		{
			if(type==1) load_drop_down( 'requires/quick_costing_v3_controller', type, 'load_drop_agent_location_name', 'agent_td');
			else if (type==2) load_drop_down( 'requires/quick_costing_v3_controller', type, 'load_drop_agent_location_name', 'location_td');
		}
	}
	...
	{
		var meeting_val=1;
		var max_meeting_no=return_ajax_request_value('', 'max_meeting_no', 'requires/quick_costing_v3_controller');
	...
	function fnc_fobavg_option()
	{
		...
		else
		{
			...
			var page_link='requires/quick_costing_v3_controller.php?action=fobavg_option_popup';
	...
	{
		var report_ids=return_ajax_request_value('', 'print_btn_id', 'requires/quick_costing_v3_controller');
	...
	function fnc_print_report(action)
	{
		...
		var report_title=$( "div.form_caption" ).html();
		generate_report_file( $('#hid_qc_no').val()+'*'+$('#txt_costSheetNo').val()+'*'+report_title+'*'+$('#cbo_company_name').val(), action,'requires/quick_costing_v3_controller');
		return;
	}
	
	function fnc_openmypage_inquery()
	{
		if( form_validation('cbo_company_name*cbo_temp_id','Company Name*Template Name')==false)
		{
			return;
		}
		var company_id=$('#cbo_company_name').val();
		var page_link='requires/quick_costing_v3_controller.php?action=inquery_id_popup&company_id='+company_id;
		...
		emailwindow.onclose=function()
		{
			...
			if(theval!="")
			{
				...
				load_drop_down( 'requires/quick_costing_v3_controller', theemail[1], 'load_drop_down_season', 'season_td');
				load_drop_down( 'requires/quick_costing_v3_controller', theemail[1], 'load_drop_down_brand', 'brand_td');
				...
				//get_php_form_data(theemail[8], "populate_data_from_rdnolib", "requires/quick_costing_v3_controller" );
			}
		}
	}
	...
	function fnc_bodyPart(inc)
	{
		...
		if(type==5)
		{
			var page_link='requires/quick_costing_v3_controller.php?action=openpopup_itemgroup&type='+type;
			...
			emailwindow.onclose=function()
			{
				...
				var row_count=$('#tbl_acc tr').length;
				var a=0; var n=0;
				for(var b=1; b<=itemdata.length; b++)
				{
					...
					if(a==0)
					{
						document.getElementById('txtAccId_'+i).value=exdata[0];
						document.getElementById('txtAcctext_'+i).value=exdata[1];
						document.getElementById('cboconsuom_'+i).value=exdata[2];
						$('#txtAcctext_'+i).removeAttr("title").attr( 'title',exdata[1] );
					}
					else
					{
						...
						n++;
						row_count++;
						document.getElementById('txtAccId_'+row_count).value=exdata[0];
						document.getElementById('txtAcctext_'+row_count).value=exdata[1];
						document.getElementById('cboconsuom_'+row_count).value=exdata[2];
						$('#txtAcctext_'+row_count).removeAttr("title").attr( 'title',exdata[1] );
					}
					a++;
				}
			}
		...
		{
			var page_link='requires/quick_costing_v3_controller.php?action=bodyPart_washType_popup&type='+type;
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, pageTitle, 'width=460px,height=450px,center=1,resize=1,scrolling=0','../');
			emailwindow.onclose=function()
			{
	...
	function fnc_type_loder( i )
	{
		...
		load_drop_down( 'requires/quick_costing_v3_controller', cboembname+'_'+i, 'load_drop_down_embtype', 'embtypetd_'+i );
	}
	
	function fnc_fabric_popup(inc)
	{
		var txt_fabid =document.getElementById('txt_fabid_'+inc).value
		var page_link='requires/quick_costing_v3_controller.php?action=fabric_description_popup&txt_fabid='+txt_fabid;
	...
	function append_row(inc,type)
	{
		...
		else if(type==2)
		{
			...
			else
			{
				...
				$('#txtAccConsumtion_'+i).removeAttr("onChange").attr("onChange","fnc_amount_calculation('accessories','"+i+"',0)");
	...
	{
		var page_link='requires/quick_costing_v3_controller.php?action=trims_cost_template_name_popup&buyer_name=' + document.getElementById('cbo_buyer_id').value;
				...
				if(select_template_data!='')
				{
					...
					for(var b=1; b<=itemdata.length; b++)
					{
						...
						if(row_count == 1 && document.getElementById('cboconsuom_1').value == 0)
						{
							...
							document.getElementById('txtTotAccConsumtion_1').value=exdata[4];
						...
						else
						{
							...
							document.getElementById('txtTotAccConsumtion_'+row_count).value=exdata[4];
	...
	function sub_dept_load(cbo_buyer_id,cbo_product_department,sup_dep_id=0)
	{
		if(cbo_buyer_id ==0 || cbo_product_department==0 )
		{
			return;
		}
		else
		{
			load_drop_down( 'requires/quick_costing_v3_controller',cbo_buyer_id+'_'+cbo_product_department+'_'+sup_dep_id, 'load_drop_down_sub_dep', 'sub_td');
		}
	}
...
<body onLoad="set_hotkey();">
        ...
        <form name="quickCosting_1" id="quickCosting_1" autocomplete="off">
            <fieldset style="width:1200px;">
                <table width="1200px" cellspacing="2" cellpadding="0" border="0">
                    <tr>
						<td width="110" class="must_entry_caption"><strong>Company Name</strong></td>
                        <td width="130"><? echo create_drop_down( "cbo_company_name", 130, "select id, company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in (3) $company_cond order by company_name","id,company_name", 1, "--Select Company--", $selected, "load_drop_down( 'requires/quick_costing_v3_controller', this.value, 'load_drop_down_buyer', 'buyer_td');"); 
							?> </td>
                        <td width="110" class="must_entry_caption">Template Name<input type="button" class="formbutton" style="width:20px; font-style:italic" value="N" onClick="openmypage_temp('requires/quick_costing_v3_controller.php?action=template_popup','Create Template')"/>
                    ...
                    <tr>
                        ...
                        <td id="buyer_td"><?=create_drop_down( "cbo_buyer_id", 130, $blank_array,"", 1, "Select Buyer", $selected, "" ); ?></td>
                        <td><strong>Season</strong>&nbsp;&nbsp;&nbsp;<? echo create_drop_down( "cbo_season_year", 50, create_year_array(),"", 1,"-Year-", 1, "",0,"" ); ?></td>
                        ...
                        <td class="must_entry_caption"><strong>Currency</strong></td>
                        <td>
                        	<? echo create_drop_down( "cbo_currency_id", 50, $currency,"", 0, "", 2, "set_conversion_rate(this.value, $('#txt_costingDate').val(), '../../', 'txt_exchangeRate')" ,0,""); ?>
                        	Ex<input style="width:50px;" type="text" class="text_boxes_numeric" name="txt_exchangeRate" id="txt_exchangeRate" value="80" />
                        </td>
                    </tr>
                    <tr>
                    	<td class="must_entry_caption">Prod. Dept.</td>
                        <td><? echo create_drop_down( "cbo_product_department", 75, $product_dept, "", 1, "-Select-", $selected, "sub_dept_load(document.getElementById('cbo_buyer_id').value,this.value);", "", "" ); ?>&nbsp;<input class="text_boxes" type="text" style="width:35px;" name="txt_product_code" id="txt_product_code" placeholder="P.Code" maxlength="10" title="Maximum 10 Character" />
                        </td>
                        ...
                        <td><?=create_drop_down( "cbo_team_leader", 130, "select id,team_leader_name from lib_marketing_team where project_type=1 and team_type in (0,1) and status_active =1 and is_deleted=0 order by team_leader_name","id,team_leader_name", 1, "-Select Team-", $selected, "load_drop_down( 'requires/quick_costing_v3_controller', this.value, 'load_drop_down_dealing_merchant', 'tddealmerchent');" ); ?></td>
                        
                        <td><strong>Dealing Merchant</strong></td>
                        <td id="tddealmerchent"><?=create_drop_down( "cbo_dealing_merchant", 130, $blank_array,"", 1, "-Team Member-", $selected, "" ); ?></td>
                    ...
                    <tr>
                        ...
                        <td id="sub_td"><? echo create_drop_down( "cbo_subDept_id", 130, $blank_array,'', 1, "-Sub. Dept.-",$selected, "" ); ?></td>
                    ...
                    <tr>
                        ...
                        <td><strong>Exporter Code/Ref</strong></td>
                        <td><input style="width:120px;" type="text" class="text_boxes" name="txt_exporter_code" id="txt_exporter_code" /></td>
						
                        <td width="110" class="must_entry_caption">Cons. Basis<? echo create_drop_down( "cbouom", 40, $unit_of_measurement,'', 0, '-Uom-', 12, "",$disabled,"12,23,27" ); ?></td>
                ...
                <tr>
                	<td width="600" valign="top">
                    	<table width="600" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                        	<thead>
                                <tr>
                                	<th class="must_entry_caption" id="item_td"><?=create_drop_down( "cboItemId", 90, $blank_array,"", 1, "-Select-", 1, "fnc_change_data(); fnc_itemwise_data_cache(); " ); ?></th>
                                	<th class="must_entry_caption" colspan="8">Particulars For Fabric</th>
                                </tr>
                                <tr>
                        ...
                        <div style="width:600px; max-height:110px; overflow-y:scroll" id="scroll_body" > 
                    	<table width="580" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                            <tbody>
                                <? $hd=9; $m=1;
								for($n=1; $n<=$hd; $n++) { ?>
                                <tr>
                                    <td width="110"><input style="width:98px;" type="text" class="text_boxes" name="txtbodyparttext_<?=$m; ?>" id="txtbodyparttext_<?=$m; ?>" placeholder="Browse" onFocus="add_auto_complete('<?=$m.'_1'; ?>');" onDblClick="fnc_bodyPart('<?=$m.'_1'; ?>');" onBlur="fnc_itemwise_data_cache(); fn_getIndex('<?=$m.'_1'; ?>',this.value);" readonly /><input style="width:50px;" type="hidden" name="txtbodypartid_<?=$m; ?>" id="txtbodypartid_<?=$m; ?>" readonly /><input style="width:50px;" type="hidden" name="txtfabupid_<?=$m; ?>" id="txtfabupid_<?=$m; ?>" readonly /></td>
                                    <td width="50"><input style="width:38px;" type="text" class="text_boxes_numeric" name="txt_consumtion<?=$m; ?>" id="txt_consumtion<?=$m; ?>" placeholder="Write" onChange="fnc_amount_calculation('fabric',<?=$m; ?>,document.getElementById('txt_rate<?=$m; ?>').value);" onBlur="fnc_itemwise_data_cache();" /></td>
                                    ...
                                    <td width="25" align="center"><input type="checkbox" name="ratePop_<?=$m; ?>" id="ratePop_<?=$m; ?>" onClick="fnc_rate_write_popup(<?=$m; ?>);" value="2" style="width:12px;" ><input style="width:50px;" type="hidden" class="text_boxes" name="txtRateData_<?=$m; ?>" id="txtRateData_<?=$m; ?>" /></td>
                                    <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txt_rate<?=$m; ?>" id="txt_rate<?=$m; ?>" fabactualrate="" placeholder="Write" onChange="fnc_amount_calculation('fabric',<?=$m; ?>,document.getElementById('txt_rate<?=$m; ?>').value);" /></td>
                                    ...
                                    <td><input style="width:180px;" type="text" class="text_boxes" name="txtremarks_<?=$m; ?>" id="txtremarks_<?=$m; ?>" placeholder="Write" /></td>
                                </tr>
                                <? $m++; } ?>
                                </tbody>
                            </table>
                        ...
                        <div style="width:600px; max-height:100px; overflow-y:scroll" id="scroll_body" > 
                               <table width="580" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all"> 
								...
								?>
                                <tr bgcolor="#FFCCFF">
                                    <td width="80"><?=create_drop_down( "cboSpeciaOperationId_$j", 80, $emblishment_name_array,"", 1, "--Select--", $sel, "fnc_type_loder(".$j.");","","1,2,4,5,99" ); ?></td>
                                    <td width="70" id="embtypetd_<?=$j; ?>"><?=create_drop_down( "cboSpeciaTypeId_$j", 70, $emb_typearr[$sel],"", 1, "-Select-", "", "" ); ?></td>
                                    <td width="80"><input style="width:68px;" type="text" class="text_boxes" name="txtspbodyparttext_<?=$j; ?>" id="txtspbodyparttext_<?=$j; ?>" placeholder="Browse" onFocus="add_auto_complete('<?=$j.'_2'; ?>');" onDblClick="fnc_bodyPart('<?=$j.'_2'; ?>');" onBlur="fnc_itemwise_data_cache(); fn_getIndex('<?=$j.'_2'; ?>',this.value);" readonly/><input style="width:50px;" type="hidden" name="txtspbodypartid_<?=$j; ?>" id="txtspbodypartid_<?=$j; ?>" readonly /></td>
                                    ...
                                    <td width="35"><input style="width:22px;" type="text" class="text_boxes_numeric" name="txt_spexper<?=$j; ?>" id="txt_spexper<?=$j; ?>" placeholder="Write" onChange="fnc_amount_calculation('special',<?=$j; ?>,document.getElementById('txtSpRate_<?=$j; ?>').value);" /></td>
                                    ...
                                    <td><input style="width:100px;" type="text" class="text_boxes" name="txtSpRemarks_<?=$j; ?>" id="txtSpRemarks_<?=$j; ?>" placeholder="Write" /></td>
                                </tr>
                                <? $j++; } ?>
                            </table>
                        </div>
                        ...
                        <div style="width:600px; max-height:100px; overflow-y:scroll" id="scroll_body" > 
                               <table width="580" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all" id="tbl_wash"> 
                                ...
                                <tbody>
                                    <tr bgcolor="#FFFF00" id="trwash_1">
                                        <td width="110"><input style="width:98px;" type="text" class="text_boxes" name="txtWashTypetext_1" id="txtWashTypetext_1" placeholder="Browse" onDblClick="fnc_bodyPart('1_3');" onBlur="fnc_itemwise_data_cache();" readonly /><input style="width:50px;" type="hidden" name="txtWashTypeId_1" id="txtWashTypeId_1" readonly /></td>
                                        ...
                                        <td width="50"><input style="width:38px;" type="text" class="text_boxes" name="txtWashRemarks_1" id="txtWashRemarks_1" placeholder="Write" /></td>
                        ...
                        <div style="width:600px; max-height:100px; overflow-y:scroll" id="scroll_body" > 
                           <table width="580" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all" id="tbl_acc">
                            <?
                            /*$acc=25; $k=1;
                            for($l=1; $l<=$acc; $l++){ */
                            ?>
                            <tbody>
                                <tr id="tracc_1">
                                    <td width="80"><input style="width:68px;" type="text" class="text_boxes" name="txtAcctext_1" id="txtAcctext_1" placeholder="Browse"  onDblClick="fnc_bodyPart('1_5');" onBlur="fnc_itemwise_data_cache();" readonly /><input style="width:50px;" type="hidden" name="txtAccId_1" id="txtAccId_1" readonly /></td>
                                    <td width="70"><input style="width:58px;" type="text" class="text_boxes" name="txtAccDescription_1" id="txtAccDescription_1" placeholder="Write" onBlur="fnc_itemwise_data_cache();" /></td>
                                    <td width="40"><input style="width:28px;" type="text" class="text_boxes" name="txtAccBandRef_1" id="txtAccBandRef_1" placeholder="Write" onBlur="fnc_itemwise_data_cache();" /></td>
                                    <td width="45"><input style="width:33px;" type="text" class="text_boxes_numeric" name="txtAccConsumtion_1" id="txtAccConsumtion_1" onChange="fnc_amount_calculation('accessories',1,0);" /></td>
                                    <td width="40"><input style="width:27px;" type="text" class="text_boxes_numeric" name="txtAcexper_1" id="txtAcexper_1" placeholder="Write" onChange="fnc_amount_calculation('accessories',1,0);" /></td>
                                    <td width="40"><input style="width:27px;" type="text" class="text_boxes_numeric" name="txtTotAccConsumtion_1" id="txtTotAccConsumtion_1" disabled /></td>
                                    <td width="50"><?=create_drop_down( "cboconsuom_1", 50, $unit_of_measurement,"", 1, "-Select-", 0, "",1,"" ); ?></td>
                                    <td width="40"><input style="width:27px;" type="text" class="text_boxes_numeric" name="txtAccRate_1" id="txtAccRate_1"  onChange="fnc_amount_calculation('accessories',1,this.value);" /></td>
                                    <td width="45"><input style="width:33px;" type="text" class="text_boxes_numeric" name="txtAccValue_1" id="txtAccValue_1" readonly /></td>
                                    <td><input style="width:33px;" type="text" class="text_boxes" name="txtAccRemarks_1" id="txtAccRemarks_1" placeholder="Write"/></td>
                                     <td><input type="button" id="increasetrim_1" name="increasetrim_1" style="width:30px" class="formbutton" value="+" onClick="append_row(1,2);" />
                                    <input type="button" id="decreasetrim_1" name="decreasetrim_1" style="width:30px" class="formbutton" value="-" onClick="javascript:fnc_remove_row(1,2);" /></td>
                                </tr>
                                <? //$k++; } ?>
                            </tbody>
                        </table>
                    ...
                    <td valign="top" width="280">
                    	<table width="270" cellspacing="0" cellpadding="0" border="0" class="rpt_table" rules="all">
                        	<tr valign="top">
                            	<td width="80"><strong>Buyer Agent</strong><input type="button" class="formbutton" style="width:15px; font-style:italic" value="N" onClick="openmypage_agent_location('requires/quick_costing_v3_controller.php?action=agent_location_popup','Create Buyer Agent',1)"/></td>
                            ...
                            <tr>
                            	<td><strong>B. Location
                            	    <input type="button" class="formbutton" style="width:15px; font-style:italic" value="N" onClick="openmypage_agent_location('requires/quick_costing_v3_controller.php?action=agent_location_popup','Create Location',2)"/></strong></td>
                            ...
                            <tr>
                                ...
                                <td><input type="text" name="txtmarignper" id="txtmarignper" style="width:68px" class="text_boxes_numeric" value="" onChange="fnc_amount_calculation(0,0,0);" placeholder="Write %" /></td>
                                <td><input type="text" name="txtmarign" id="txtmarign" style="width:68px" class="text_boxes_numeric" value="" onChange="fnc_amount_calculation(0,0,0);" readonly disabled /></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
								...
								<? echo load_submit_buttons($permission,"fnc_qcosting_entry",0,0,"reset_fnc();",1); ?><br><input type="button" id="report_btn_1" class="formbutton" style="width:30px;" value="PNT" onClick="fnc_print_report('quick_costing_print')" />&nbsp; <input type="button" id="report_btn_2" class="formbutton" style="width:35px;display:none;" value="PNT2" onClick="fnc_print_report('quick_costing_print2')" />&nbsp; <input type="button" id="report_btn_p_1" class="formbutton" style="width:30px;" value="P-1" onClick="fnc_print_report('quick_costing_print_p1')"> &nbsp;<input type="button" id="report_btn_p_2" class="formbutton" style="width:30px;" value="P-2" onClick="fnc_print_report('quick_costing_print_p2')"> &nbsp; <input type="button" id="set_button" class="formbutton" style="width:60px;" value="Copy" onClick="fnc_copy_cost_sheet(0);" />&nbsp; <input type="button" id="set_button" class="formbutton" style="width:45px;" value="Revise" onClick="fnc_qcosting_entry(6)" />&nbsp; <input type="button" id="set_button" class="formbutton" style="width:45px;" value="Option" onClick="fnc_qcosting_entry(7)" />
                                </td>
                            </tr>
                                ...
                                <td valign="top" id="summary_td">
                                    <table cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>SAM/SMV / EFI</td>
                                                <td align="center" ><input name="txt_sam_0" id="txt_sam_0" class="text_boxes_numeric" style="width:20px;" title="SAM" placeholder="SAM"  /><input name="txt_smv_0" id="txt_smv_0" class="text_boxes_numeric" style="width:20px;" title="SMV" placeholder="SMV" disabled /><input name="txt_eff_0" id="txt_eff_0" class="text_boxes_numeric" style="width:20px;" title="Efficiency" placeholder="EFF" disabled /></td>
                                            ...
                                            <tr>
                                                <td title="Commercial Cost">Comml.($/DZN)<input type="text" name="txt_commlPer" id="txt_commlPer" class="text_boxes_numeric" style="width:23px;" title="Commercial Cost" placeholder="%" disabled /></td>
                                            ...
                                            <tr>
                                                <td title="Commission Cost">Com(%)($/DZN)<input name="txt_commPer" id="txt_commPer" class="text_boxes_numeric" style="width:20px;" title="Commission (%)" placeholder="%" disabled /></td>
                                            ...
                                            <tr>
                                                <td>Storage Cost</td>
                                                <td><input name="txtstorage_0" id="txtstorage_0" class="text_boxes_numeric" style="width:40px;" value="0" /></td>
                                                <td>&nbsp;</td>
                                            </tr>
                                             <tr>
                                                <td>Operating Expense</td>
                                                <td><input name="txtoperatingexpense_0" id="txtoperatingexpense_0" class="text_boxes_numeric" value="0" style="width:40px;"  /></td>
                                                <td>&nbsp;</td>
                                            </tr>
                                             <tr>
                                                <td >Remark</td>
                                                
                                                <td ><input name="txtremark_0" id="txtremark_0" class="text_boxes" value="''" style="width:60px;"  /></td>
                                                <td>&nbsp;</td>
    ...
    <script> 
		var style_id='';
		var temp_style_list=return_ajax_request_value(style_id+'__'+0, 'temp_style_list_view', 'requires/quick_costing_v3_controller');
		$('#style_td').html( temp_style_list );
