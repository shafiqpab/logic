...
<script>
	...
	function fnc_dtls_ganerate(val)
	{
		...
		else if($("#txt_itemBodyData_"+gmtId).val()!='')
		{
			...
			for(var j=1; j<=hd; j++)
			{
				...
				$("#txt_head"+j).val(get_dropdown_text('cbo_head_'+j));		
	...
	function fnc_qcosting_entry( operation )
	{
		...
		else
		{
			if(operation==4)
			{
				...
				generate_report_file( $('#hid_qc_no').val()+'*'+$('#txt_costSheetNo').val()+'*'+report_title,'quick_costing_print','requires/quick_costing_controller');
				return;
			}
			else
			{
	...
	function fnc_qcosting_entry_response()
	{
		if(http.readyState == 4) 
		{
			...
			else if (reponse[0]==1 || reponse[0]==0)
			{
				...
				{
					var temp_style_list=return_ajax_request_value(reponse[2]+'__'+1, 'temp_style_list_view', 'requires/quick_costing_controller');
					...
					load_drop_down('requires/quick_costing_controller', reponse[2]+'__'+reponse[4]+'__'+reponse[4], 'load_drop_down_option_id', 'option_td');
					set_onclick_style_list(reponse[2]+'__'+user_id+'__'+reponse[2]);
				}
				if(reponse[5]==6) //Revise
				{
					load_drop_down('requires/quick_costing_controller', reponse[2]+'__'+reponse[4]+'__0', 'load_drop_down_revise_no', 'revise_td');
					load_drop_down('requires/quick_costing_controller', reponse[2]+'__'+reponse[4]+'__'+reponse[4], 'load_drop_down_option_id', 'option_td');
					set_onclick_style_list(reponse[2]+'__'+user_id+'__'+reponse[2]);
				}
				else if(  reponse[5]==7) //Option
				{	
					load_drop_down('requires/quick_costing_controller', reponse[2]+'__'+reponse[4]+'__'+$('#cbo_revise_no').val(), 'load_drop_down_revise_no', 'revise_td');
					load_drop_down('requires/quick_costing_controller', reponse[2]+'__'+reponse[4]+'__0', 'load_drop_down_option_id', 'option_td');
			...
			var temp_style_list=return_ajax_request_value(reponse[2]+'__'+0, 'temp_style_list_view', 'requires/quick_costing_controller');
			$('#style_td').html( temp_style_list );
	...
	function openmypage_style(type)
	{
		...
		emailwindow.onclose=function()
		{
			...
			if(style_id!="")
			{
				var temp_style_list=return_ajax_request_value(cost_sheet_no+'__'+1, 'temp_style_list_view', 'requires/quick_costing_controller');
				$('#style_td').html( temp_style_list );
				...
				for( var i=0; i< str.length; i++ ) {
					if( k==1)
					{
						...
						load_drop_down('requires/quick_costing_controller', cost_sheet_no+'__0__0', 'load_drop_down_option_id', 'option_td');
						load_drop_down('requires/quick_costing_controller', cost_sheet_no+'__'+$('#cbo_option_id').val()+'__0', 'load_drop_down_revise_no', 'revise_td');
						k++;
					}
				}
			}
	...
	function set_onclick_style_list( data )
	{
		...
		if(datas[3]=="25")
		{
			...
			load_drop_down('requires/quick_costing_controller', datas[2]+'__0__0', 'load_drop_down_option_id', 'option_td');
			load_drop_down('requires/quick_costing_controller', datas[2]+'__'+$('#cbo_option_id').val()+'__0', 'load_drop_down_revise_no', 'revise_td');
	...
	function fnc_delete_style_row()
	{
		...
		var temp_style_list=return_ajax_request_value(hid_qc_no+'__'+3, 'temp_style_list_view', 'requires/quick_costing_controller');
		$('#style_td').html( temp_style_list );
		reset_fnc();
		/*if( (td*1)>1 )
		{
	...
	{
		var user_id='<? echo $_SESSION['logic_erp']['user_id']; ?>';
		get_php_form_data(($("#hid_qc_no").val()*1)+'__'+user_id+'__'+0, 'populate_style_details_data', 'requires/quick_costing_controller');
	}
	...
	function fnc_confirm_style()
	{
		if($('#txt_update_id').val()!="")
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link+'&data='+data, title, 'width=970px,height=450px,center=1,resize=0,scrolling=0','../')
			emailwindow.onclose=function()
			{
				/*var theform=this.contentDoc.forms[0];
				var style_id=this.contentDoc.getElementById("hide_style_id").value; // product ID
	...
	function fnc_option_rev( val )
	{
		...
		get_php_form_data(hid_qc_no+"__"+user_id+"__"+cost_no+"__"+val+"__from_option", 'populate_style_details_data', 'requires/quick_costing_controller');
	}
	...
	function fnc_fobavg_option()
	{
		...
		else
		{
			...
			var page_link='requires/quick_costing_controller.php?action=fobavg_option_popup';
	...
	{
		var report_ids=return_ajax_request_value('', 'print_btn_id', 'requires/quick_costing_controller');
	...
	function fnc_print_report(action)
	{
		...
		var report_title=$( "div.form_caption" ).html();
		generate_report_file( $('#hid_qc_no').val()+'*'+$('#txt_costSheetNo').val()+'*'+report_title, action,'requires/quick_costing_controller');
		return;
	}
    ...
    <div style="width:100%;">
        ...
        <form name="quickCosting_1" id="quickCosting_1" autocomplete="off">
            ...
            <table width="1200px" cellspacing="2" cellpadding="0" border="0">
                <tr>
                    <td width="110" class="must_entry_caption">Template Name<input type="button" class="formbutton" style="width:20px; font-style:italic" value="N" onClick="openmypage_temp('requires/quick_costing_controller.php?action=template_popup','Create Template')"/>
                ...
                <tr>
                    ...
                    <td><? echo create_drop_down( "cbo_buyer_id", 130, $qc_buyer_arr,"", 1, "-- Select Buyer --", $selected, "load_drop_down( 'requires/quick_costing_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'requires/quick_costing_controller',this.value, 'load_drop_down_sub_dep', 'sub_td' );" ); ?></td>
                    	...
                    	<table width="930" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                        	<thead>
                            	<tr>
									...
									<? $costing_unit=array(998=>'Other',999=>'Yds',1000=>'Mtr'); ?>
                                    ...
                                    <th colspan="3"><? echo create_drop_down( "cbo_head_4", 150, $costing_unit,'', 0, "-Other-",$selected, "fnc_change_data(this.value,4);","","998,1000","","","" ); ?></th>
                                    <th colspan="3"><? echo create_drop_down( "cbo_head_5", 150, $costing_unit,'', 0, "-Yds-",$selected, "fnc_change_data(this.value,5);","","999,1000","","","" ); ?></th>
                                </tr>
                                <tr>
                    	...
                    	<table width="270" cellspacing="0" cellpadding="0" border="0" class="rpt_table" rules="all">
                            ...
                            <tr>
                            	<td><strong>B. Location
                            	    <input type="button" class="formbutton" style="width:15px; font-style:italic" value="N" onClick="openmypage_agent_location('requires/quick_costing_controller.php?action=agent_location_popup','Create Location',2)"/></strong></td>
