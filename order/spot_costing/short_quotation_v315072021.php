...
<script>
	...
	function openmypage_temp(page_link,title)
	{
		...
		{
			load_drop_down( 'requires/short_quotation_v3_controller', '', 'load_drop_template_name', 'template_td');
		}
	}
	...
	function fnc_template_view()
	{
		...
		if(temp_id!=0)
		{
			var item_data=return_ajax_request_value(temp_id, 'load_lib_item_id', 'requires/short_quotation_v3_controller');
			...
			load_drop_down( 'requires/short_quotation_v3_controller', itemNratio, 'load_drop_down_tempItem', 'item_td');
			//alert(item_id);
	...
	function fnc_summary_dtls(val)
	{
		...
		html += '<table cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all"><thead><tr><th colspan="'+count_item+2+'">Item Wise Cost Summary ($/Pack)</th></tr><tr><th width="120">Description</th>';
		var i=0;
		...
		html += '<td>&nbsp;</td></tr><tr><td>CM ($/Pack)</td>';
        var m=0;
		...
		html += '<td id="totCm_td" align="right">&nbsp;</td></tr><tr><td>Frieght Cost($/Pack)</td>';
        var n=0;
        ...
        html += '<td id="totFriCst_td" align="right">&nbsp;</td></tr><tr><td>Lab - Test($/Pack)</td>';
        ...
        html += '<td id="totMissCst_td" align="right">&nbsp;</td></tr><tr><td>Other Cost($/Pack)</td>';
		var ab=0;
		...
		html += '<td id="totOtherCst_td" align="right">&nbsp;</td></tr><tr><td title="Commercial Cost">Comml.($/Pack)<input type="text" name="txt_commlPer" id="txt_commlPer" value="" class="text_boxes_numeric" style="width:20px;" placeholder="%" onChange="fnc_amount_calculation(11,0,0);" /></td>';
        var p=0;
        ...
        html += '<td id="totCommlCst_td" align="right">&nbsp;</td></tr><tr><td title="Commission Cost">Com.(%)($/Pack)<input name="txt_commPer" id="txt_commPer" value="" class="text_boxes_numeric" style="width:20px;" title="Com(%)($/Pack)" placeholder="%" onChange="fnc_amount_calculation(9,0,0);" /></td>';
        var p=0;
		for(var y=1; y<=gmtName.length; y++)
		{
        ...
        html += '<td id="totCommCst_td" align="right">&nbsp;</td></tr><tr><td>F.O.B($/Pack)</td>';
		var q=0;
	...
	function fnc_dtls_ganerate(val)
	{
		...
		for(var m=1; m<=f; m++)
		{
			...
			$('#txtdznyds_'+m).attr("placeholder","Write");
			$('#txtdznyds_'+m).removeAttr('readonly','readonly');
			$('#txtdznkg_'+m).attr("placeholder","Write");
			$('#txtdznkg_'+m).removeAttr('readonly','readonly');
		...
		for(var q=1; q<=acc; q++)
		{
			...
			$('#txtAccConsDzn_'+q).removeAttr('readonly','readonly');
			...
			$('#txtAccConsDzn_'+q).attr("placeholder","Write");
		...
		if(itemConsRateData!="")
		{
			for(var m=1; m<=f; m++)
			{
				...
				var bodypart=bodypartid=cons=exper=totCons=ratePop=rate=fabVal=fabRemarks=fabupid=fabuom="";
				...
				var cons=itemConsRate_fab[10];
				var exper=itemConsRate_fab[11];
				var totCons=itemConsRate_fab[12];
				var ratePop=itemConsRate_fab[13];
				var rateData=itemConsRate_fab[14];
				var rate=itemConsRate_fab[15];
				var fabVal=itemConsRate_fab[16];
				var fabupid=itemConsRate_fab[17];
				var fabuom=itemConsRate_fab[18];
				...
				$('#txtdznyds_'+m).val(dznyds);
				$('#txtdznkg_'+m).val(dznkg);
				...
				else if(ratePop==1)
				{
					...
					$('#txtdznyds_'+m).attr("placeholder","Display");
					$('#txtdznyds_'+m).attr('readonly','readonly');
					$('#txtdznkg_'+m).attr("placeholder","Display");
					$('#txtdznkg_'+m).attr('readonly','readonly');
					
					$('#txt_consumtion'+m).attr('readonly','readonly');
					$('#txt_exper'+m).attr('readonly','readonly');
					$('#txt_rate'+m).attr('readonly','readonly');
					$('#txt_consumtion'+m).attr("placeholder","Display");
		...
		if(itemAccData!="")
		{
			for(var q=1; q<=accs; q++)
			{
				...
				if(itemAccData[adata]!=undefined)
				{
					...
					var acctext=accId=accDescription=accBandRef=accUseFor=accConsumtion=acexper=totAccConsumtion=acRate=acValue=acConsDzn=acconsUom=acconsCal=accRateData="";
					...
					var acConsDzn=accData[11];
					var acconsCal=accData[12];
					var accRateData=accData[13];
					...
					$('#txtAccConsDzn_'+q).val(acConsDzn);
					adata++;
				}
			}
		}
	...
	function fnc_amount_calculation(type,inc_id,rate)
	{
		...
		if (type=='fabric')
		{
			var consYds=($("#txtdznyds_"+inc_id).val()*1);
			var consKg=($("#txtdznkg_"+inc_id).val()*1);
			...
			if(rate=='yds')
			{
				if(fabUom==12)//kg
				{
					var consFab=(consYds*36*(fabWidth*1)*(fabGsm*1))/1550/1000;
				}
				else if(fabUom==23)//mtr
				{
					var consFab=consYds*1.09361*36*((fabWidth*1)*(fabGsm*1)/1550/1000);
				}
				...
				//alert(consFab);
				$("#txtdznkg_"+inc_id).val( number_format(consFab,4,'.','') );
			...
			else if (rate=='kg')
			{
				if(fabUom==12)//kg
				{
					var consFab=((consKg*1) / 36)/((fabWidth*1)*(fabGsm*1)/1550/1000);
				}
				else if(fabUom==23)//mtr
				{
					var consFab=(consKg*1) / 1.09361 /36 /((fabWidth*1)*(fabGsm*1)/1550/1000);
				}
				else if(fabUom==27)//yds
				{
					var consFab=consKg;
				...
				//alert(consFab)
				$("#txtdznyds_"+inc_id).val( number_format(consFab,4,'.','') );
				//var consPack=(consFab/12)*(itemRatio*1);
			}
			var conspackkg=(($("#txtdznkg_"+inc_id).val()*1)/12)*(fabItemRatio*1);
			$("#txt_consumtion"+inc_id).val( number_format(conspackkg,4,'.','') );
			...
			$("#txt_totConsumtion"+inc_id).val( number_format(totCons,4,'.','') );
			var fabVal=totCons*($("#txt_rate"+inc_id).val()*1);
			$("#txt_value"+inc_id).val( number_format(fabVal,4,'.','') );
		...
		else if (type=='accessories')
		{
			...
			if(rate=='dzn')
			{
				var consAccDzn=$("#txtAccConsDzn_"+inc_id).val()*1;
				var consAccPk=(consAccDzn/12)*accItemRatio;
				$("#txtAccConsumtion_"+inc_id).val( number_format(consAccPk,4,'.','') );
			}
			else if(rate=='pack')
			{
				var consAccPk=$("#txtAccConsumtion_"+inc_id).val()*1;
				var consAccDzn=(consAccPk/accItemRatio)*12;
				$("#txtAccConsDzn_"+inc_id).val( number_format(consAccDzn,4,'.','') );
			}
		...
		else if (type=='3')
		{
			...
			if($("#cmPop").val()==1)
			{
				...
				if(cc==1)
				{
					//var cpm_from_financial_parameter=return_ajax_request_value(costing_date, 'cpm_check_load', 'requires/short_quotation_v3_controller');
					cc++;
				}
	...
	function fnc_meeting_remarks_pop_up(costSheetId,styleRef)
	{
		...
		var page_link = 'requires/short_quotation_v3_controller.php?action=meeting_remarks_popup';
		...
		else
		{
			if(type==1)
			{
				if(document.getElementById('ratePop_'+inc_id).checked==true)
				{
					...
					$('#txtdznyds_'+inc_id).val('');
					$('#txtdznyds_'+inc_id).attr("placeholder","Display");
					$('#txtdznyds_'+inc_id).attr('readonly','readonly');
					$('#txtdznkg_'+inc_id).val('');
					$('#txtdznkg_'+inc_id).attr("placeholder","Display");
					$('#txtdznkg_'+inc_id).attr('readonly','readonly');
				...
				else if(document.getElementById('ratePop_'+inc_id).checked==false)
				{
					...
					$('#txtdznyds_'+inc_id).val('');
					$('#txtdznyds_'+inc_id).attr("placeholder","Write");
					$('#txtdznyds_'+inc_id).removeAttr('readonly','readonly');
					$('#txtdznkg_'+inc_id).val('');
					$('#txtdznkg_'+inc_id).attr("placeholder","Write");
					$('#txtdznkg_'+inc_id).removeAttr('readonly','readonly');
			...
			else if(type==2)
			{
				...
				{
					var consumtion=$('#txtAccConsDzn_'+inc_id).val();
					document.getElementById('accRatePop_'+inc_id).value=1;
					
					$('#txtAccConsDzn_'+inc_id).val('');
					...
					$('#txtAccConsDzn_'+inc_id).attr('readonly','readonly');
					...
					$('#txtRateData_'+inc_id).attr('readonly','readonly');
					$('#txtAccConsDzn_'+inc_id).attr("placeholder","Display");
				...
				else if(document.getElementById('accRatePop_'+inc_id).checked==false)
				{
					...
					$('#txtAccRateData_'+inc_id).val('');
					$('#txtAccValue_'+inc_id).val('');
					$('#txtAccConsDzn_'+inc_id).removeAttr('readonly','readonly');
					...
					$('#txtAccConsDzn_'+inc_id).attr("placeholder","Write");
	...
	function fnc_openmypage_rate(inc_id,popuptype)
	{
		...
		if(popuptype==1)
		{
			var txtdznkg=$('#txtdznkg_'+inc_id).val();
			...
			var page_link = 'requires/short_quotation_v3_controller.php?action=rate_details_popup';
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link+'&qc_no='+qc_no+'&bodypartid='+bodypartid+'&consRateId='+consRateId+'&txtdznkg='+txtdznkg+'&txt_fabDesc='+txt_fabDesc+'&cbocolortype='+cbocolortype+'&txtgsmweight='+txtgsmweight+'&txtfabwidth='+txtfabwidth+'&packQty='+packQty+'&itemRatio='+itemRatio+'&fabuom='+fabuom+'&process_loss_method_id='+process_loss_method_id+'&rateData='+rateData, title, 'width=850px,height=380px,center=1,resize=0,scrolling=0','../')
			release_freezing();
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0];
				var totconsyds=this.contentDoc.getElementById("txttotcons_yds").value;
				var totconskg=this.contentDoc.getElementById("txttotcons_kg").value;
				...
				$('#txtdznkg_'+inc_id).val( totconskg );
				$('#txt_consumtion'+inc_id).val( totcons );
				$('#txt_exper'+inc_id).val( totconsexper );
				$('#txt_totConsumtion'+inc_id).val( tottotcons );
				$('#txt_rate'+inc_id).val( avgRate );
		...
		else if (popuptype==2)
		{
			...
			var page_link = 'requires/short_quotation_v3_controller.php?action=accrate_details_popup';
			...
			emailwindow.onclose=function()
			{
				...
				var txtdatastr=this.contentDoc.getElementById("txtdatastr").value;
				//alert(totcons+'__'+avgRate+'__'+totamount)
				$('#txtAccConsDzn_'+inc_id).val( totconsdzn );
	...
	function fnc_new_stage_popup()
	{
		...
		var page_link = 'requires/short_quotation_v3_controller.php?action=stage_saveUpdate_popup';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=600px,height=380px,center=1,resize=0,scrolling=0','../')
		release_freezing();
		emailwindow.onclose=function()
		{
			load_drop_down( 'requires/short_quotation_v3_controller', '', 'load_drop_stage_name', 'stage_td');
		}
	}
	...
	function fnc_itemwise_data_cache()
	{
		...
		for(var m=1; m<=f; m++)
		{
			fabData+=($('#txtbodyparttext_'+m).val())+'~!~'+($('#txtbodypartid_'+m).val()*1)+'~!~'+($('#txt_fabDesc'+m).val())+'~!~'+($('#txt_fabid_'+m).val())+'~!~'+($('#cbocolortype_'+m).val())+'~!~'+($('#txtgsmweight_'+m).val()*1)+'~!~'+($('#txtfabwidth_'+m).val()*1)+'~!~'+($('#txtusefor_'+m).val())+'~!~'+($('#txtdznyds_'+m).val()*1)+'~!~'+($('#txtdznkg_'+m).val()*1)+'~!~'+($('#txt_consumtion'+m).val()*1)+'~!~'+($('#txt_exper'+m).val()*1)+'~!~'+($('#txt_totConsumtion'+m).val()*1)+'~!~'+($('#ratePop_'+m).val()*1)+'~!~'+($('#txtRateData_'+m).val())+'~!~'+($('#txt_rate'+m).val()*1)+'~!~'+($('#txt_value'+m).val()*1)+'~!~'+($('#txtfabupid_'+m).val())+'~!~'+($('#cbofabuom_'+m).val())+'#^';
		}
		...
		for(var q=1; q<=acc; q++)
		{
			accessoriesData+= ($('#txtAcctext_'+q).val())+'~!~'+($('#txtAccId_'+q).val()*1)+'~!~'+($('#txtAccDescription_'+q).val())+'~!~'+($('#txtAccBandRef_'+q).val())+'~!~'+($('#txtAccConsumtion_'+q).val()*1)+'~!~'+($('#txtAcexper_'+q).val()*1)+'~!~'+($('#txtTotAccConsumtion_'+q).val()*1)+'~!~'+($('#txtAccRate_'+q).val()*1)+'~!~'+($('#txtAccValue_'+q).val()*1)+'~!~'+($('#txtAccUseFor_'+q).val())+'~!~'+($('#cboconsuom_'+q).val())+'~!~'+($('#txtAccConsDzn_'+q).val()*1)+'~!~'+($('#accRatePop_'+q).val()*1)+'~!~'+$('#txtAccRateData_'+q).val()+'#^';
		}
	...
	function fnc_qcosting_entry( operation )
	{
		...
		else
		{
			if(operation==4)
			{
				...
				generate_report_file( $('#hid_qc_no').val()+'*'+$('#txt_costSheetNo').val()+'*'+report_title,'quick_costing_print','requires/short_quotation_v3_controller');
				release_freezing();
				return;
			}
			else
			{
				...
				http.open("POST","requires/short_quotation_v3_controller.php",true);
	...
	function fnc_qcosting_entry_response()
	{
		if(http.readyState == 4) 
		{
			...
			//{
				...
				{
					var temp_style_list=return_ajax_request_value(reponse[2]+'__'+1, 'temp_style_list_view', 'requires/short_quotation_v3_controller');
					$('#style_td').html( temp_style_list );
					var user_id='<? echo $_SESSION['logic_erp']['user_id']; ?>';
					//get_php_form_data(reponse[1]+"__"+user_id+"__"+reponse[2]+"__"+0+"***"+0, 'populate_style_details_data', 'requires/short_quotation_v3_controller');
					load_drop_down('requires/short_quotation_v3_controller', reponse[2]+'__'+reponse[4]+'__'+reponse[3], 'load_drop_down_revise_no', 'revise_td');
					load_drop_down('requires/short_quotation_v3_controller', reponse[2]+'__'+reponse[4]+'__'+reponse[4], 'load_drop_down_option_id', 'option_td');
					set_onclick_style_list(reponse[2]+'__'+user_id+'__'+reponse[2]);
				}
				if(reponse[5]==6) //Revise
				{
					load_drop_down('requires/short_quotation_v3_controller', reponse[2]+'__'+reponse[4]+'__0', 'load_drop_down_revise_no', 'revise_td');
					load_drop_down('requires/short_quotation_v3_controller', reponse[2]+'__'+reponse[4]+'__'+reponse[4], 'load_drop_down_option_id', 'option_td');
					set_onclick_style_list(reponse[2]+'__'+user_id+'__'+reponse[2]);
				}
				else if(  reponse[5]==7) //Option
				{	
					load_drop_down('requires/short_quotation_v3_controller', reponse[2]+'__'+reponse[4]+'__'+$('#cbo_revise_no').val(), 'load_drop_down_revise_no', 'revise_td');
					load_drop_down('requires/short_quotation_v3_controller', reponse[2]+'__'+reponse[4]+'__0', 'load_drop_down_option_id', 'option_td');
			...
			var temp_style_list=return_ajax_request_value(reponse[2]+'__'+0, 'temp_style_list_view', 'requires/short_quotation_v3_controller');
			$('#style_td').html( temp_style_list );
			//alert(reponse[7]);
	...
	{
		window.open("requires/short_quotation_v3_controller.php?data=" + data+'&action='+action, true );
	}
	
	function openmypage_style(type)
	{
		var page_link='requires/short_quotation_v3_controller.php?action=style_popup';
		...
		emailwindow.onclose=function()
		{
			...
			if(style_id!="")
			{
				var temp_style_list=return_ajax_request_value(cost_sheet_no+'__'+1, 'temp_style_list_view', 'requires/short_quotation_v3_controller');
				$('#style_td').html( temp_style_list );
				...
				for( var i=0; i< str.length; i++ ) {
					if( k==1)
					{
						...
						load_drop_down('requires/short_quotation_v3_controller', cost_sheet_no+'__0__0', 'load_drop_down_option_id', 'option_td');
						load_drop_down('requires/short_quotation_v3_controller', cost_sheet_no+'__'+$('#cbo_option_id').val()+'__0', 'load_drop_down_revise_no', 'revise_td');
						k++;
					}
				}
			}
			//alert(temp_style_list)
			//$('#style_td').html( $('#style_td').html() +""+ return_ajax_request_value(style_id, 'temp_style_list_view', 'requires/short_quotation_v3_controller') );
			//show_list_view( style_id,'temp_style_list_view','style_td','requires/short_quotation_v3_controller','',1);//setFilterGrid(\'tbl_po_list\',-1)
	...
	function set_onclick_style_list( data )
	{
		...
		if(datas[3]=="25")
		{
			...
			load_drop_down('requires/short_quotation_v3_controller', datas[2]+'__0__0', 'load_drop_down_option_id', 'option_td');
			load_drop_down('requires/short_quotation_v3_controller', datas[2]+'__'+$('#cbo_option_id').val()+'__0', 'load_drop_down_revise_no', 'revise_td');
			...
			get_php_form_data(datas[0]+"__"+datas[1]+"__"+datas[2]+"__"+val, 'populate_style_details_data', 'requires/short_quotation_v3_controller');
	...
	function fnc_delete_style_row()
	{
		...
		var temp_style_list=return_ajax_request_value(hid_qc_no+'__'+3, 'temp_style_list_view', 'requires/short_quotation_v3_controller');
		$('#style_td').html( temp_style_list );
		reset_fnc();
		/*if( (td*1)>1 )
		{
	...
	{
		var user_id='<? echo $_SESSION['logic_erp']['user_id']; ?>';
		get_php_form_data(($("#hid_qc_no").val()*1)+'__'+user_id+'__'+0, 'populate_style_details_data', 'requires/short_quotation_v3_controller');
	}
	...
	function fnc_clear_all()
	{
		...
		{
			var style_id='';
			var temp_style_list=return_ajax_request_value(style_id+'__'+2, 'temp_style_list_view', 'requires/short_quotation_v3_controller');
			$('#style_td').html('');
			reset_fnc();
		}
	}
	...
	function fnc_confirm_style()
	{
		if($('#txt_update_id').val()!="")
		{
			...
			var page_link='requires/short_quotation_v3_controller.php?action=confirmStyle_popup';
			...
			emailwindow.onclose=function()
			{
				/*var theform=this.contentDoc.forms[0];
				var style_id=this.contentDoc.getElementById("hide_style_id").value; // product ID
				var temp_style_list=return_ajax_request_value(style_id+'__'+1, 'temp_style_list_view', 'requires/short_quotation_v3_controller');
				$('#style_td').html( temp_style_list );*/
				//alert(temp_style_list)
				//$('#style_td').html( $('#style_td').html() +""+ return_ajax_request_value(style_id, 'temp_style_list_view', 'requires/short_quotation_v3_controller') );
				//show_list_view( style_id,'temp_style_list_view','style_td','requires/short_quotation_v3_controller','',1);//setFilterGrid(\'tbl_po_list\',-1)
	...
	function fnc_option_rev( val )
	{
		...
		get_php_form_data(hid_qc_no+"__"+user_id+"__"+cost_no+"__"+val+"__from_option", 'populate_style_details_data', 'requires/short_quotation_v3_controller');
	}
	...
	function openmypage_agent_location(page_link,title,type)
	{
		...
		emailwindow.onclose=function()
		{
			if(type==1) load_drop_down( 'requires/short_quotation_v3_controller', type, 'load_drop_agent_location_name', 'agent_td');
			else if (type==2) load_drop_down( 'requires/short_quotation_v3_controller', type, 'load_drop_agent_location_name', 'location_td');
		}
	}
	...
	{
		var meeting_val=1;
		var max_meeting_no=return_ajax_request_value('', 'max_meeting_no', 'requires/short_quotation_v3_controller');
	...
	function fnc_fobavg_option()
	{
		...
		else
		{
			...
			var page_link='requires/short_quotation_v3_controller.php?action=fobavg_option_popup';
	...
	{
		var report_ids=return_ajax_request_value('', 'print_btn_id', 'requires/short_quotation_v3_controller');
	...
	function fnc_print_report(action)
	{
		...
		var report_title=$( "div.form_caption" ).html();
		generate_report_file( $('#hid_qc_no').val()+'*'+$('#txt_costSheetNo').val()+'*'+report_title, action,'requires/short_quotation_v3_controller');
		return;
	}
	...
	function fnc_openmypage_inquery()
	{
		...
		var page_link='requires/short_quotation_v3_controller.php?action=inquery_id_popup';
		...
		emailwindow.onclose=function()
		{
			...
			if(theval!="")
			{
				...
				load_drop_down( 'requires/short_quotation_v3_controller', theemail[1], 'load_drop_down_season', 'season_td');
				load_drop_down( 'requires/short_quotation_v3_controller', theemail[1], 'load_drop_down_brand', 'brand_td');
				...
				//get_php_form_data(theemail[8], "populate_data_from_rdnolib", "requires/short_quotation_v3_controller" );
			}
		}
	}
	...
	function fnc_bodyPart(inc)
	{
		...
		var page_link='requires/short_quotation_v3_controller.php?action=bodyPart_washType_ItemGroup_popup&type='+type;
	...
	function fnc_type_loder( i )
	{
		...
		load_drop_down( 'requires/short_quotation_v3_controller', cboembname+'_'+i, 'load_drop_down_embtype', 'embtypetd_'+i );
	}
	
	function fnc_fabric_popup(inc)
	{
		var txt_fabid =document.getElementById('txt_fabid_'+inc).value
		var page_link='requires/short_quotation_v3_controller.php?action=fabric_description_popup&txt_fabid='+txt_fabid;
	...
	function append_row(inc,type)
	{
		...
		else if(type==2)
		{
			...
			else
			{
				...
				$('#txtAccConsDzn_'+i).removeAttr("onChange").attr("onChange","fnc_amount_calculation('accessories','"+i+"','dzn')");
				...
				$('#txtAccConsDzn_'+i).removeAttr('readonly','readonly');
				...
				$('#txtAccConsDzn_'+i).attr("placeholder","Write");
	...
	function fnc_remove_row(inc,type)
	{
		...
		else if(type==3)//Fabric
		{
			...
			$('#txtdznyds_'+inc).attr("placeholder","Write");
			$('#txtdznyds_'+inc).removeAttr('readonly','readonly');
			$('#txtdznkg_'+inc).attr("placeholder","Write");
			$('#txtdznkg_'+inc).removeAttr('readonly','readonly');
	...
	{
		var page_link='requires/short_quotation_v3_controller.php?action=trims_cost_template_name_popup&buyer_name=' + document.getElementById('cbo_buyer_id').value;
	...
	function check_exchange_rate_variable()
	{
		...
		var response=return_global_ajax_value( cbo_currercy+"**"+costing_date+"**"+cbo_company_name, 'check_conversion_rate_variable', '', 'requires/short_quotation_v3_controller');
...
<body onLoad="set_hotkey();">
            ...
            <fieldset style="width:1200px;">
                <table width="1200px" cellspacing="2" cellpadding="0" border="0">
                    <tr>
                    	...
                    	<td width="130"><? echo create_drop_down( "cbo_company_id", 130, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "--Company--", $selected,"load_drop_down( 'requires/short_quotation_v3_controller', this.value, 'load_drop_down_location', 'location_td'); check_exchange_rate_variable();",'' ); ?></td>
                    	...
                    	<td width="110" class="must_entry_caption">Template Name<input type="button" class="formbutton" style="width:20px; font-style:italic" value="N" onClick="openmypage_temp('requires/short_quotation_v3_controller.php?action=template_popup','Create Template')"/>
                    ...
                    <tr>
                        ...
                        <td><? echo create_drop_down( "cbo_buyer_id", 130, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down('requires/short_quotation_v3_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'requires/short_quotation_v3_controller',this.value, 'load_drop_down_sub_dep', 'sub_td' ); load_drop_down( 'requires/short_quotation_v3_controller', this.value, 'load_drop_down_brand', 'brand_td');" ); ?></td>
                	...
                	<td width="885" valign="top">
                    	<table width="885" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                        	<thead>
                                ...
                                <tr>
                                    <th width="80">Pack Qty</th>
                                    <th width="120" align="center" id="tdpackQty">&nbsp;</th>
                                ...
                                <tr>
                                    ...
                                    <th width="40">DZN [YDS /MTR]</th>
                                    <th width="40">DZN [KG]</th>
                                    <th width="40">Pack [KG]</th>
                                    <th width="30">Ex %</th>
                                    <th width="40">Tot.</th>
                    	...
                    	<table width="865" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                            <tbody>
								...
								for($n=1; $n<=$hd; $n++) { ?>
                                <tr>
                                    <td width="80"><input style="width:67px;" type="text" class="text_boxes" name="txtbodyparttext_<?=$m; ?>" id="txtbodyparttext_<?=$m; ?>" placeholder="Browse" onFocus="add_auto_complete('<?=$m.'_1'; ?>');" onDblClick="fnc_bodyPart('<?=$m.'_1'; ?>');" onBlur="fnc_itemwise_data_cache(); fn_getIndex('<?=$m.'_1'; ?>',this.value);" readonly /><input style="width:50px;" type="hidden" name="txtbodypartid_<?=$m; ?>" id="txtbodypartid_<?=$m; ?>" readonly /><input style="width:50px;" type="hidden" name="txtfabupid_<?=$m; ?>" id="txtfabupid_<?=$m; ?>" readonly /></td>
                                    <td width="120"><input style="width:107px;" type="text" class="text_boxes" name="txt_fabDesc<?=$m; ?>" id="txt_fabDesc<?=$m; ?>" readonly placeholder="Browse" onDblClick="fnc_fabric_popup(<?=$m; ?>);" /><input style="width:50px;" type="hidden" name="txt_fabid_<?=$m; ?>" id="txt_fabid_<?=$m; ?>" readonly /></td>
                                    ...
                                    <td width="40"><input style="width:27px;" type="text" class="text_boxes_numeric" name="txtdznyds_<?=$m; ?>" id="txtdznyds_<?=$m; ?>" placeholder="Write" onChange="fnc_amount_calculation('fabric',<?=$m; ?>,'yds');" onBlur="fnc_itemwise_data_cache();" /></td>
                                    <td width="40"><input style="width:27px;" type="text" class="text_boxes_numeric" name="txtdznkg_<?=$m; ?>" id="txtdznkg_<?=$m; ?>" placeholder="Write" onChange="fnc_amount_calculation('fabric',<?=$m; ?>,'kg');" /></td>
                                    <td width="40"><input style="width:27px;" type="text" class="text_boxes_numeric" name="txt_consumtion<?=$m; ?>" id="txt_consumtion<?=$m; ?>" placeholder="Display" onChange="fnc_amount_calculation('fabric',<?=$m; ?>,document.getElementById('txt_rate<?=$m; ?>').value);" onBlur="fnc_itemwise_data_cache();" readonly /></td>
                        		...
                        		<td width="620" valign="top">
                                    <table width="620" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                                        <thead>
                                            ...
                                            <tr>
                                                <th width="70">Item Group <span id="load_temp" style="float:right; width:10px; font-weight: bold;background-color: white;color:black; border: 1px white solid; cursor: pointer;" onClick="openmypage_template_name('Template Search');">...</span></th>
                                                ...
                                                <th width="40">Cons. /Pack</th>
                                                <th width="30">Ex %</th>
                                                <th width="40">Tot. Cons. /Pack</th>
                                                <th width="30">Rate ($)</th>
                                    ...
                                    <div style="width:620px; max-height:350px; overflow-y:scroll" id="scroll_body" > 
                                       <table width="600" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all" id="tbl_acc">
                                        ...
                                        <tbody>
                                            <tr id="tracc_<?=$k; ?>">
                                                <td width="70"><input style="width:58px;" type="text" class="text_boxes" name="txtAcctext_<?=$k; ?>" id="txtAcctext_<?=$k; ?>" placeholder="Browse"  onDblClick="fnc_bodyPart('<?=$k; ?>_5');" onBlur="fnc_itemwise_data_cache();" readonly /><input style="width:50px;" type="hidden" name="txtAccId_<?=$k; ?>" id="txtAccId_<?=$k; ?>" readonly /></td>
                                                <td width="70"><input style="width:58px;" type="text" class="text_boxes" name="txtAccDescription_<?=$k; ?>" id="txtAccDescription_<?=$k; ?>" placeholder="Write" onBlur="fnc_itemwise_data_cache();" /></td>
                                                ...
                                                <td width="40"><input style="width:28px;" type="text" class="text_boxes_numeric" name="txtAccConsDzn_<?=$k; ?>" id="txtAccConsDzn_<?=$k; ?>" onChange="fnc_amount_calculation('accessories',<?=$k; ?>,'dzn');" /></td>
                                                ...
                                                <td width="30"><input style="width:17px;" type="text" class="text_boxes_numeric" name="txtAccRate_<?=$k; ?>" id="txtAccRate_<?=$k; ?>"  onChange="fnc_amount_calculation('accessories',<?=$k; ?>,this.value);" /></td>
                                ...
                                <td>
                                    <table width="260" cellspacing="0" cellpadding="0" border="0" class="rpt_table" rules="all">
                                        ...
                                        <tr>
                                            ...
                                            <? echo load_submit_buttons($permission,"fnc_qcosting_entry",0,0,"reset_fnc();",1); ?><br><input type="button" id="report_btn_1" class="formbutton" style="width:30px;" value="PNT" onClick="fnc_print_report('quick_costing_print')" />&nbsp; <input type="button" id="report_btn_2" class="formbutton" style="width:35px;display:none;" value="PNT2" onClick="fnc_print_report('quick_costing_print2')" />&nbsp; <input type="button" id="set_button" class="formbutton" style="width:60px;" value="Copy" onClick="fnc_copy_cost_sheet(0);" />&nbsp; <input type="button" id="set_button" class="formbutton" style="width:45px;" value="Revise" onClick="fnc_qcosting_entry(6)" />&nbsp; <input type="button" id="set_button" class="formbutton" style="width:45px;" value="Option" onClick="fnc_qcosting_entry(7)" />
                                            </td>
                                        </tr>
                            ...
                            <tr>
                                <td valign="top" id="summary_td">
                                    <table cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                                        <thead>
                                            <tr>
                                                <th colspan="3">Item Wise Cost Summary ($/Pack)</th>
                                            </tr>
                                            <tr>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>Other Cost($/Pack)</td>
                                                <td align="center"><input name="txtOtherCost_0" id="txtOtherCost_0" class="text_boxes_numeric" style="width:40px;" disabled /></td>
                                                <td>&nbsp;</td>
                                            </tr>
                                            <tr>
                                                <td title="Commercial Cost">Comml.($/Pack)                                                  <input type="text" name="txt_commlPer" id="txt_commlPer" class="text_boxes_numeric" style="width:23px;" title="Commercial Cost" placeholder="%" disabled /></td>
                                            ...
                                            <tr>
                                                <td title="Commission Cost">Com(%)($/Pack)                                                  <input name="txt_commPer" id="txt_commPer" class="text_boxes_numeric" style="width:20px;" title="Commission (%)" placeholder="%" disabled /></td>
    ...
    <script> 
		var style_id='';
		var temp_style_list=return_ajax_request_value(style_id+'__'+0, 'temp_style_list_view', 'requires/short_quotation_v3_controller');
		$('#style_td').html( temp_style_list );
