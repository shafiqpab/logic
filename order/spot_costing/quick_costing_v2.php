<?
/*--- ----------------------------------------- Comments
Purpose			: 	This form will create Quick Costing V2
Functionality	:	
JS Functions	:
Created by		:	Kausar 
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Quick Costing V2","../../", 1, 1, $unicode,1,'');
...
<script>
	...
	function openmypage_temp(page_link,title)
	{
		...
		{
			load_drop_down( 'requires/quick_costing_v2_controller', '', 'load_drop_template_name', 'template_td');
		}
	}
	...
	function fnc_template_view()
	{
		...
		var item_id=return_ajax_request_value(temp_id, 'load_lib_item_id', 'requires/quick_costing_v2_controller');
		//alert(item_id);
			...
			for(var j=1; j<=trLen; j++)
			{
				...
				for(var n=1; n<=hd; n++)
				{
					...
					else
					{
						...
						{
							if( n!=5 )defult_des="FAB. WIDTH-INCH";
						}
					}
	...
	function fnc_summary_dtls(val)
	{
		...
		if(costingper_id==2) var costingcap="$/PCS"; else if(costingper_id==1) var costingcap="$/DZN"; else var costingcap="";
		var value=val.split("__");
		var cpmlib=$('#txt_costingDate').attr('libcpm')*1;
		//alert(cpm)
		...
		html += '<table cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all"><thead><tr><th colspan="'+count_item+2+'">Item Wise Cost Summary ('+costingcap+')</th></tr><tr><th width="120">Description</th>';
		var i=0;
		...
		{
			html += '<td align="center"><input name="txt_cpm_[]" id="txt_cpm_'+trim(gmtId[cpm])+'" value="'+cpmlib+'" class="text_boxes_numeric" style="width:40px;" title="CPM" placeholder="CPM" disabled onChange="fnc_amount_calculation(3,'+trim(gmtId[cpm])+',0);" /></td>';
			cpm++;
		}		
		...
		html += '<td>&nbsp;</td></tr><tr><td>CM ('+costingcap+')</td>';
        var m=0;
		...
		html += '<td id="totCm_td" align="right">&nbsp;</td></tr><tr><td>Frieght Cost('+costingcap+')</td>';
        var n=0;
        ...
        html += '<td id="totFriCst_td" align="right">&nbsp;</td></tr><tr><td>Lab - Test('+costingcap+')</td>';
        ...
        html += '<td id="totMissCst_td" align="right">&nbsp;</td></tr><tr><td>Other Cost('+costingcap+')</td>';
		var ab=0;
		...
		html += '<td id="totOtherCst_td" align="right">&nbsp;</td></tr><tr><td title="Commercial Cost">Comml.('+costingcap+')<input type="text" name="txt_commlPer" id="txt_commlPer" value="" class="text_boxes_numeric" style="width:20px;" placeholder="%" onChange="fnc_amount_calculation(11,0,0);" /></td>';
        var p=0;
        ...
        html += '<td id="totCommlCst_td" align="right">&nbsp;</td></tr><tr><td title="Commission Cost">Com.(%)('+costingcap+')<input name="txt_commPer" id="txt_commPer" value="" class="text_boxes_numeric" style="width:20px;" title="Com(%)('+costingcap+')" placeholder="%" onChange="fnc_amount_calculation(9,0,0);" placeholder="Display"/></td>';
        ...
        html += '<td id="totCommCst_td" align="right">&nbsp;</td></tr><tr><td>F.O.B('+costingcap+')</td>';
		var q=0;
	...
	function fnc_dtls_ganerate(val)
	{
		...
		for(var k=1; k<=fs; k++)
		{
			...
			if(k==5)
			{
				document.getElementById('ratePop_'+k).checked==true;
				$("#ratePop_"+k).val(1);
				$("#txtRateData_"+k).val('');
			}
			else
		...
		else if($("#txt_itemBodyData_"+gmtId).val()!='')
		{
			...
			for(var j=1; j<=hd; j++)
			{
				...
				if(consumption_fab!=0) $("#txt_consumtion"+j).val(number_format(consumption_fab,4,'.','' ));
				...
				if(ex_per!=0) $("#txt_exper"+j).val((number_format(ex_per,4,'.','' )));
				if(tot_cons_fab!=0) $("#txt_totConsumtion"+j).val(number_format(tot_cons_fab,4,'.','' ));
				$('#txt_value'+j).attr('disabled','disabled');
				//fnc_rate_write_popup(j);
				if(is_calculation_fab==1 || j==5)
				{
					if(j==5)
					{
						$('#txt_consumtion'+j).attr('readonly','readonly');
						$('#txt_exper'+j).attr('readonly','readonly');
					}
				...
				else
				{
					if(j==5)
					{
						$('#txt_consumtion'+j).removeAttr('readonly','readonly');
						$('#txt_exper'+j).removeAttr('readonly','readonly');
					}
			...
			if(accln>0) var accs=accln; else var accs=50; 
			...
			for(var v=1; v<=accs; v++)
			{
				...
				var trimdesc=itemAccData_ac[5];
				var trmuom=itemAccData_ac[6];
				
				$("#cbo_accessories_id"+v).val(particular_type_ac);
				$("#txt_accdesc_"+v).val(trimdesc);
				$("#cboconsuom_"+v).val(trmuom);
	...
	function fnc_formula_bilder(id)
	{
		...
		if($('#txt_head'+id).val()!="")
		{
			...
			var page_link = 'requires/quick_costing_v2_controller.php?&action=formulaBilder_popup';
	...
	function fnc_consumption_calculation(item_id, row_id, col_id, val, aw)
	{
		...
		if(col_id==1)
		{
			...
			if(itm_name.search('BTS'))
			{
				...
				var item_cons_body=(btm_item_cons_body*costingqty)*wastage_per*fabric_weight; 
				//alert(btm_item_cons_body)
			}
			else
			{
				var half_chest=(($("#txtVal_"+item_id+"_3_1").val()*1)+($("#txtAw_"+item_id+"_3_1").val()*1))*cm_inch;
				var item_cons_body=(((((body_length+sleeve_length)*(half_chest*2))*costingqty)*wastage_per)*fabric_weight);
		...
		else if(col_id==2)
		{
			...
			var item_cons_rib=(((((rib_length*rib_weight)*2)*costingqty)*wastage_per)*fabric_weight_rib);
			var rib_cons=0;
		...
		else //
		{
			...
			//(POCKET LENGTH_V+POCKET LENGTH_A)*(RIB WIDTH_V+RIB WIDTH_A)*2/10000*costingqty*1.05*(FABRIC WEIGHT_V+FABRIC WEIGHT_A)/1000
			...
			var item_cons_pocket=(((((pocket_length*pocket_weight)*2)*costingqty)*wastage_per)*fabric_weight_pocket);
			var pocket_cons=0;
	...
	function fnc_amount_calculation(type,inc_id,rate)
	{
		...
		else if (type=='3')
		{
			...
			if($("#cmPop").val()==1)
			{
				...
				if(cc==1)
				{
					//var cpm_from_financial_parameter=return_ajax_request_value(costing_date, 'cpm_check_load', 'requires/quick_costing_v2_controller');
					cc++;
				}
				...
				//var cm_cost=((smv*cpm_from_financial_parameter)*costingqty+(smv*cpm_from_financial_parameter*costingqty)*eff)/exchange_rate;
				var cm_cost=(((cpm*100)/eff)*smv)*costingqty;
		...
		else if (type=='7')
		{
			...
			if($("#txt_offerQty").val()!="")
			{
				var item_wise_miss_cost=(lum_sum_cost/offer_qty)*costingqty;
		...
		else if (type=='9')
		{
			...
			for(var bb=1; bb<=all_item_id_hid.length; bb++)
			{
				...
				mis_cost=(($("#txt_lumSum_cost").val()*1)/($("#txt_offerQty").val()*1))*costingqty;
				
				$("#txtMissCost_"+trim(all_item_id_hid[aa])).val( number_format(mis_cost,2,'.','') );
				
				before_commission_cost=($("#fab_td"+trim(all_item_id_hid[aa])).text()*1)+($("#spOpe_td"+trim(all_item_id_hid[aa])).text()*1)+($("#acc_td"+trim(all_item_id_hid[aa])).text()*1)+($("#txtCmCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtFriCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtLtstCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtMissCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtOtherCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtCommlCost_"+trim(all_item_id_hid[aa])).val()*1);
		...
		else if (type=='11')
		{
			...
			for(var bb=1; bb<=all_item_id_hid.length; bb++)
			{
				...
				$("#txtCommlCost_"+trim(all_item_id_hid[aa])).val( number_format(commercial_cost,2,'.','') );
				row_cl=row_cl+commercial_cost;
				//$("#fobT_td"+trim(all_item_id_hid[aa])).text( number_format(itemWiseTot,2,'.',''));
				aa++;
			}
			$("#totCommlCst_td").text(number_format(row_cl,2,'.',''));
		}
		...
		for(var bb=1; bb<=all_item_id_hid.length; bb++)
		{
			...
			before_commercial_cost=($("#fab_td"+trim(all_item_id_hid[aa])).text()*1)+($("#spOpe_td"+trim(all_item_id_hid[aa])).text()*1)+($("#acc_td"+trim(all_item_id_hid[aa])).text()*1)+($("#txtCmCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtFriCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtLtstCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtMissCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtOtherCost_"+trim(all_item_id_hid[aa])).val()*1);
			...
			itemFoBarr[item_id_hid]=itemWiseTot;
			$("#fobT_td"+trim(all_item_id_hid[aa])).text( number_format(itemWiseTot,2,'.',''));
			$("#fobPcsT_td"+trim(all_item_id_hid[aa])).text( number_format(itemWiseTot/costingqty,2,'.',''));
			var fob_rmg=0;
		...
		//alert(fob_rmg);
		var tot_cost=($("#totFab_td").text()*1)+($("#totSpc_td").text()*1)+($("#totAcc_td").text()*1)+($("#totCm_td").text()*1)+($("#totFriCst_td").text()*1)+($("#totLbTstCst_td").text()*1)+($("#totMissCst_td").text()*1)+($("#totOtherCst_td").text()*1)+($("#totCommlCst_td").text()*1)+($("#totCommCst_td").text()*1);
		
		$("#totCommlCst_td").text( number_format(totCommercial_Cost,2,'.',''));
		$("#totCost_td").text( number_format(tot_cost,2,'.','') );
		$("#totFOBCost_td").text( number_format(tot_cost/costingqty,2,'.','') );
	...
	function fnc_meeting_remarks_pop_up(costSheetId,styleRef)
	{
		...
		var page_link = 'requires/quick_costing_v2_controller.php?action=meeting_remarks_popup';
	...
	function fnc_rate_write_popup(inc_id)
	{
		...
		else if(inc_id=='cm')
		{
			...
			var cpm=$('#txt_costingDate').attr('libcpm')*1;
	...
	{
		if( form_validation('txt_costingDate',' Costing Date')==false)
		{
			return;
		}
		else
		{
			...
			var costingDate=$('#txt_costingDate').val();
			var rateData=$('#txtRateData_'+inc_id).val();
			var title = 'Rate Details PopUp';
			if(inc_id!=5)
			{
				var page_link = 'requires/quick_costing_v2_controller.php?action=rate_details_popup';
			}
			else
			{
				var page_link = 'requires/quick_costing_v2_controller.php?action=ydsrate_details_popup';
			}
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link+'&styleRef='+styleRef+'&cons='+cons+'&rateData='+rateData+'&costingDate='+costingDate, title, 'width=650px,height=550px,center=1,resize=0,scrolling=0','../')
			release_freezing();
			emailwindow.onclose=function()
			{
				...
				if(inc_id==5)
				{
					var txttotydscons=this.contentDoc.getElementById("txttotydscons").value;
					var txtydtotsexper=this.contentDoc.getElementById("txtydtotsexper").value;
					var txtydstottotcons=this.contentDoc.getElementById("txtydstottotcons").value;
					var pop_rate=this.contentDoc.getElementById("txtydstotrate").value;	 //Access form field with id="emailfield"
					var all_rate_data=this.contentDoc.getElementById("hidden_all_data").value;
					var txtydstotamt=this.contentDoc.getElementById("txtydstotamt").value;
					
					$('#txt_consumtion'+inc_id).val( txttotydscons );
					$('#txt_exper'+inc_id).val( txtydtotsexper );
					$('#txt_totConsumtion'+inc_id).val( txtydstottotcons );
					$('#txt_rate'+inc_id).val( pop_rate );
					$('#txtRateData_'+inc_id).val( all_rate_data );
					$('#txt_value'+inc_id).val( txtydstotamt );
					
					fnc_itemwise_data_cache(document.getElementById('txt_itemId').value);
				}
	...
	function fnc_new_stage_popup()
	{
		...
		var page_link = 'requires/quick_costing_v2_controller.php?action=stage_saveUpdate_popup';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=600px,height=380px,center=1,resize=0,scrolling=0','../')
		release_freezing();
		emailwindow.onclose=function()
		{
			load_drop_down( 'requires/quick_costing_v2_controller', '', 'load_drop_stage_name', 'stage_td');
		}
	}
	...
	function fnc_itemwise_data_cache( item_id )
	{
		...
		for(var m=1; m<=hd; m++)
		{
			...
			cons_rate_data+= ($('#txt_head_id_'+m).val()*1)+'_'+($('#txt_consumtion'+m).val()*1)+'_'+($('#ratePop_'+m).val()*1)+'_'+($('#txt_rate'+m).val()*1)+'_'+$('#txtRateData_'+m).val()+'_'+$('#txt_exper'+m).val()+'_'+$('#txt_totConsumtion'+m).val()+'##';
		...
		var acc=50; var accessories_cons_data="";
		for(var q=1; q<=acc; q++)
		{
			accessories_cons_data+= ($('#cbo_accessories_id'+q).val()*1)+'_'+($('#txt_accConsumtion'+q).val()*1)+'_'+($('#txt_acexper'+q).val()*1)+'_'+($('#txt_acRate'+q).val()*1)+'_'+($('#txt_totAccConsumtion'+q).val()*1)+'_'+($('#txt_accdesc_'+q).val())+'_'+($('#cboconsuom_'+q).val()*1)+'##';
		}
	...
	function fnc_qcosting_entry( operation )
	{
		...
		if (form_validation('cbo_company_id*cbo_temp_id*cbo_costingper_id*cbo_cons_basis_id*txt_exchangeRate*cbo_buyer_id*cbo_season_id*txt_styleRef*txt_offerQty*txt_delivery_date*txt_costingDate','Company*Costing Per*Template Name*Cons. Basis*Exchange Rate*Buyer Name*Season*Style Ref.*Offer Qty*Delivery Date*Costing Date')==false)
		{
			release_freezing();	
			return;
		}	
		else
		{
			if(operation==4)
			{
				...
				generate_report_file( $('#hid_qc_no').val()+'*'+$('#txt_costSheetNo').val()+'*'+report_title,'quick_costing_print','requires/quick_costing_v2_controller');
				release_freezing();	
				return;
			}
			else
			{
				...
				for(i=1; i<=split_tmep_id.length; i++)
				{
					...
					for(var m=1; m<=hd; m++)
					{
						...
						for(var j=1; j<=trLen; j++)
						{
							...
							description_value_alw_data+="&txtItemDes_"+itm_id+"_"+z+"_"+sq+"='" + $('#txtItemDes_'+itm_id+'_'+m+'_'+j).val()+"'"+"&txtVal_"+itm_id+"_"+z+"_"+sq+"='" + $('#txtVal_'+itm_id+'_'+m+'_'+j).val()+"'"+"&txtAw_"+itm_id+"_"+z+"_"+sq+"='" + $('#txtAw_'+itm_id+'_'+m+'_'+j).val()+"'"+"&txtdvaId_"+itm_id+"_"+z+"_"+sq+"='" + $('#txtdvaId_'+itm_id+'_'+m+'_'+j).val()+"'";
							sq++;
						}
						z++;
					}
					item_wise_tot_data+=($('#fab_td'+itm_id).text()*1)+'_'+($('#spOpe_td'+itm_id).text()*1)+'_'+($('#acc_td'+itm_id).text()*1)+'_'+($('#txt_cpm_'+itm_id).val()*1)+'_'+($('#txt_smv_'+itm_id).val()*1)+'_'+($('#txt_eff_'+itm_id).val()*1)+'_'+($('#txtCmCost_'+itm_id).val()*1)+'_'+($('#txtFriCost_'+itm_id).val()*1)+'_'+($('#txtLtstCost_'+itm_id).val()*1)+'_'+($('#txtMissCost_'+itm_id).val()*1)+'_'+($('#txtOtherCost_'+itm_id).val()*1)+'_'+($('#txtCommCost_'+itm_id).val()*1)+'_'+($('#fobT_td'+itm_id).text()*1)+'_'+($('#txtRmgQty_'+itm_id).val()*1)+'_'+($('#txtCommlCost_'+itm_id).val()*1)+'_'+itm_id+'__';
				...
				var data_mst="action=save_update_delete&operation="+operation+"&data_tot_cost_summ="+data_tot_cost_summ+"&item_wise_tot_data="+item_wise_tot_data+"&type="+type+get_submitted_data_string('cbo_temp_id*txt_temp_id*txt_styleRef*txt_costSheetNo*txt_update_id*cbo_buyer_id*cbouom*cbo_cons_basis_id*cbo_season_id*txt_styleDesc*cbo_subDept_id*txt_delivery_date*txt_exchangeRate*txt_offerQty*txt_quotedPrice*txt_tgtPrice*cbo_stage_id*txt_costingDate*txt_costing_remarks*cbo_revise_no*cbo_option_id*txt_option_remarks*txt_meeting_date*txt_meeting_time*chk_is_new_meeting*txt_meeting_remarks*txt_meeting_no*hid_qc_no*cbo_company_id*cbo_location_id*cbo_agent*cbo_costingper_id',"../../");
				
				var data=data_mst+detls_data+description_value_alw_data;
				/*alert(data_mst);
				return;*/
				
				http.open("POST","requires/quick_costing_v2_controller.php",true);
	...
	function fnc_qcosting_entry_response()
	{
		if(http.readyState == 4) 
		{
			...
			if (reponse[0]=="marginEntry")
			{
				alert("Quick Costing Margin Entry Found.");
				release_freezing();
				return;
			}
			...
			else if (reponse[0]==1 || reponse[0]==0)
			{
				...
				$('#txt_costingDate').attr('disabled','true');
			...
			//{
				...
				{
					var temp_style_list=return_ajax_request_value(reponse[2]+'__'+1, 'temp_style_list_view', 'requires/quick_costing_v2_controller');
					$('#style_td').html( temp_style_list );
					var user_id='<? echo $_SESSION['logic_erp']['user_id']; ?>';
					//get_php_form_data(reponse[1]+"__"+user_id+"__"+reponse[2]+"__"+0+"***"+0, 'populate_style_details_data', 'requires/quick_costing_v2_controller');
					load_drop_down('requires/quick_costing_v2_controller', reponse[2]+'__'+reponse[4]+'__'+reponse[3], 'load_drop_down_revise_no', 'revise_td');
					load_drop_down('requires/quick_costing_v2_controller', reponse[2]+'__'+reponse[4]+'__'+reponse[4], 'load_drop_down_option_id', 'option_td');
					set_onclick_style_list(reponse[2]+'__'+user_id+'__'+reponse[2]);
				}
				if(reponse[5]==6) //Revise
				{
					load_drop_down('requires/quick_costing_v2_controller', reponse[2]+'__'+reponse[4]+'__0', 'load_drop_down_revise_no', 'revise_td');
					load_drop_down('requires/quick_costing_v2_controller', reponse[2]+'__'+reponse[4]+'__'+reponse[4], 'load_drop_down_option_id', 'option_td');
					set_onclick_style_list(reponse[2]+'__'+user_id+'__'+reponse[2]);
				}
				else if(  reponse[5]==7) //Option
				{	
					load_drop_down('requires/quick_costing_v2_controller', reponse[2]+'__'+reponse[4]+'__'+$('#cbo_revise_no').val(), 'load_drop_down_revise_no', 'revise_td');
					load_drop_down('requires/quick_costing_v2_controller', reponse[2]+'__'+reponse[4]+'__0', 'load_drop_down_option_id', 'option_td');
			...
			var temp_style_list=return_ajax_request_value(reponse[2]+'__'+0, 'temp_style_list_view', 'requires/quick_costing_v2_controller');
			$('#style_td').html( temp_style_list );
			//alert(reponse[7]);
	...
	{
		window.open("requires/quick_costing_v2_controller.php?data=" + data+'&action='+action, true );
	}
	
	function openmypage_style(type)
	{
		var page_link='requires/quick_costing_v2_controller.php?action=style_popup';
		...
		emailwindow.onclose=function()
		{
			...
			if(style_id!="")
			{
				var temp_style_list=return_ajax_request_value(cost_sheet_no+'__'+1, 'temp_style_list_view', 'requires/quick_costing_v2_controller');
				$('#style_td').html( temp_style_list );
				...
				for( var i=0; i< str.length; i++ ) {
					if( k==1)
					{
						...
						load_drop_down('requires/quick_costing_v2_controller', cost_sheet_no+'__0__0', 'load_drop_down_option_id', 'option_td');
						load_drop_down('requires/quick_costing_v2_controller', cost_sheet_no+'__'+$('#cbo_option_id').val()+'__0', 'load_drop_down_revise_no', 'revise_td');
						k++;
					}
				}
			}
			//alert(temp_style_list)
			//$('#style_td').html( $('#style_td').html() +""+ return_ajax_request_value(style_id, 'temp_style_list_view', 'requires/quick_costing_v2_controller') );
			//show_list_view( style_id,'temp_style_list_view','style_td','requires/quick_costing_v2_controller','',1);//setFilterGrid(\'tbl_po_list\',-1)
	...
	function set_onclick_style_list( data )
	{
		...
		if(datas[3]=="25")
		{
			...
			load_drop_down('requires/quick_costing_v2_controller', datas[2]+'__0__0', 'load_drop_down_option_id', 'option_td');
			load_drop_down('requires/quick_costing_v2_controller', datas[2]+'__'+$('#cbo_option_id').val()+'__0', 'load_drop_down_revise_no', 'revise_td');
			...
			get_php_form_data(datas[0]+"__"+datas[1]+"__"+datas[2]+"__"+val, 'populate_style_details_data', 'requires/quick_costing_v2_controller');
	...
	function fnc_delete_style_row()
	{
		...
		var temp_style_list=return_ajax_request_value(hid_qc_no+'__'+3, 'temp_style_list_view', 'requires/quick_costing_v2_controller');
		$('#style_td').html( temp_style_list );
		reset_fnc();
		/*if( (td*1)>1 )
		{
	...
	{
		var user_id='<? echo $_SESSION['logic_erp']['user_id']; ?>';
		get_php_form_data(($("#hid_qc_no").val()*1)+'__'+user_id+'__'+0, 'populate_style_details_data', 'requires/quick_costing_v2_controller');
	}
	...
	function fnc_clear_all()
	{
		...
		{
			var style_id='';
			var temp_style_list=return_ajax_request_value(style_id+'__'+2, 'temp_style_list_view', 'requires/quick_costing_v2_controller');
			$('#style_td').html('');
			reset_fnc();
		}
	}
	...
	function fnc_confirm_style()
	{
		...
		{
			var data=$('#hid_qc_no').val()+'__'+$('#txt_update_id').val()+'__'+$('#cbo_costingper_id').val();
			var page_link='requires/quick_costing_v2_controller.php?action=confirmStyle_popup';
			...
			emailwindow.onclose=function()
			{
				/*var theform=this.contentDoc.forms[0];
				var style_id=this.contentDoc.getElementById("hide_style_id").value; // product ID
				var temp_style_list=return_ajax_request_value(style_id+'__'+1, 'temp_style_list_view', 'requires/quick_costing_v2_controller');
				$('#style_td').html( temp_style_list );*/
				//alert(temp_style_list)
				//$('#style_td').html( $('#style_td').html() +""+ return_ajax_request_value(style_id, 'temp_style_list_view', 'requires/quick_costing_v2_controller') );
				//show_list_view( style_id,'temp_style_list_view','style_td','requires/quick_costing_v2_controller','',1);//setFilterGrid(\'tbl_po_list\',-1)
	...
	function fnc_option_rev( val )
	{
		...
		get_php_form_data(hid_qc_no+"__"+user_id+"__"+cost_no+"__"+val+"__from_option", 'populate_style_details_data', 'requires/quick_costing_v2_controller');
	}
	...
	function openmypage_agent_location(page_link,title,type)
	{
		...
		emailwindow.onclose=function()
		{
			if(type==1) load_drop_down( 'requires/quick_costing_v2_controller', type, 'load_drop_agent_location_name', 'agent_td');
			else if (type==2) load_drop_down( 'requires/quick_costing_v2_controller', type, 'load_drop_agent_location_name', 'location_td');
		}
	}
	...
	{
		var meeting_val=1;
		var max_meeting_no=return_ajax_request_value('', 'max_meeting_no', 'requires/quick_costing_v2_controller');
	...
	function fnc_fobavg_option()
	{
		...
		else
		{
			...
			var page_link='requires/quick_costing_v2_controller.php?action=fobavg_option_popup';
	...
	{
		var report_ids=return_ajax_request_value('', 'print_btn_id', 'requires/quick_costing_v2_controller');
	...
	function fnc_print_report(action)
	{
		...
		var report_title=$( "div.form_caption" ).html();
		generate_report_file( $('#hid_qc_no').val()+'*'+$('#txt_costSheetNo').val()+'*'+report_title, action,'requires/quick_costing_v2_controller');
		return;
	}
	...
	function check_exchange_rate()
	{
		...
		var response=return_global_ajax_value( cbo_currercy+"**"+costing_date+"**"+cbo_company_name+"**"+cbo_location_id, 'check_conversion_rate', '', 'requires/quick_costing_v2_controller');
		var response=response.split("_");
		$('#txt_exchangeRate').val(response[1]);
		var cpmval=response[2]/response[1];
		$('#txt_costingDate').attr('libcpm', number_format(cpmval,4,'.','') );
	}
	
	function fnc_accuom(inc)
	{
		var accessories_id = $('#cbo_accessories_id'+inc).val();
		var accuom=return_ajax_request_value(accessories_id, 'accuom_load', 'requires/quick_costing_v2_controller');
		
		$('#cboconsuom_'+inc).val(accuom);
	}
...
<body onLoad="set_hotkey();">
        ...
        <form name="quickCosting_1" id="quickCosting_1" autocomplete="off">
            ...
            <table width="1200px" cellspacing="2" cellpadding="0" border="0">
            	<tr>
                    ...
                    <td width="130"><? echo create_drop_down( "cbo_company_id", 130, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "--Company--", $selected,"load_drop_down( 'requires/quick_costing_v2_controller', this.value, 'load_drop_down_location', 'location_td'); load_drop_down( 'requires/quick_costing_v2_controller', this.value, 'load_drop_down_agent', 'agent_td'); check_exchange_rate();",'' ); ?></td>
                    ...
                    <td width="110">Agent</td>
                    <td width="130" id="agent_td"><? echo create_drop_down( "cbo_agent", 130, $blank_array,"", 1, "-Select Agent-", $selected, "" ); ?></td>
                    <td width="110" class="must_entry_caption"><strong>Costing Per</strong></td>
                    <td width="130"><? echo create_drop_down( "cbo_costingper_id", 130, $qccosting_per,'', 1, "-Select-",$selected, "" ); ?></td>
                    
                    <td width="110" class="must_entry_caption"><strong>Costing Date</strong></td>
                    <td width="130"><input style="width:120px;" type="text" class="datepicker" name="txt_costingDate" id="txt_costingDate" libcpm="" value="<?=date('d-m-Y');?>" onBlur="check_exchange_rate();" /></td>
                </tr>
                <tr>
                    <td width="110" class="must_entry_caption">Template Name<input type="button" class="formbutton" style="width:20px; font-style:italic" value="N" onClick="openmypage_temp('requires/quick_costing_v2_controller.php?action=template_popup','Create Template')"/>
                ...
                <tr>
                    ...
                    <td><? echo create_drop_down( "cbo_buyer_id", 130, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down( 'requires/quick_costing_v2_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'requires/quick_costing_v2_controller',this.value, 'load_drop_down_sub_dep', 'sub_td' );" ); ?></td>
                    ...
                    <td><input style="width:120px;" type="text" onDblClick="openmypage_style(0);" class="text_boxes textbox1" name="txt_styleRef" id="txt_styleRef" placeholder="Write/Browse"/></td>
                    <td class="must_entry_caption"><strong>Delivery Date</strong></td>
                ...
                <tr>
                    ...
                    <td id="stage_td"><? echo create_drop_down( "cbo_stage_id", 130,"select tuid, stage_name from lib_stage_name where status_active=1 and is_deleted=0","tuid,stage_name", 1, "-- Select --", $selected, "" ); ?></td>
                    <td><input type="button" class="image_uploader" style="width:80px" value="ADD File" onClick="file_uploader ( '../../', document.getElementById('hid_qc_no').value,'', 'qcv2file', 2 ,1)"></td>
                    <td><input type="button" id="image_button" class="image_uploader" style="width:120px" value="ADD IMAGE" onClick="file_uploader( '../../', document.getElementById('hid_qc_no').value,'', 'qcv2img', 0 ,1)" /></td>
                </tr>
            </table>
        </fieldset>
                    	...
                    	<table width="930" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                        	<thead>
                            	<tr>
                                    ...
                                    <th colspan="3"><? echo create_drop_down( "cbo_head_1", 150, $body_part,'', 1, "-Body Part 1-",$selected, "fnc_change_data(this.value,1);","","1,20","","",""); ?></th>
                                    <th colspan="3"><? echo create_drop_down( "cbo_head_2", 150, $body_part,'', 1, "-Body Part 2-",$selected, "fnc_change_data(this.value,2);","","1,4,20,42","","",""); ?></th>
                                    <th colspan="3"><? echo create_drop_down( "cbo_head_3", 150, $body_part,'', 1, "-Body Part 3-",$selected, "fnc_change_data(this.value,3);","","1,4,6,7,20,42","","","" ); ?></th>
                ...
                <tr>
                	<td width="520" valign="top">
                    	...
                    	<table width="500" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                            <tbody>
                                ...
                                <? $hd=4; $m=2;
								for($n=1; $n<=$hd; $n++) { if($m==5) { $plh="BR"; $readonly="readonly"; } else {  $rateBr=""; $plh="Write";  $readonly=""; } ?>
                                <tr>
                                    ...
                                    <td width="60"><input style="width:48px;" type="text" class="text_boxes_numeric" name="txt_consumtion<?=$m; ?>" id="txt_consumtion<?=$m; ?>" <?=$readonly; ?> disabled /></td>
                                    <td width="40"><input style="width:27px;" type="text" class="text_boxes_numeric" name="txt_exper<? echo $m; ?>" id="txt_exper<? echo $m; ?>" placeholder="Write" onChange="fnc_amount_calculation('fabric',<? echo $m; ?>,document.getElementById('txt_rate<? echo $m; ?>').value);" <?=$readonly; ?>  /></td>
                                    ...
                                    <? if($m==5) { $rateBr='onDblClick="fnc_openmypage_rate(5,0);"'; $plh="BR"; $readonly="readonly"; ?>
									<td width="25"><input type="checkbox" name="ratePop_<? echo $m; ?>" id="ratePop_<? echo $m; ?>" onClick="fnc_rate_write_popup(<? echo $m; ?>)" value="2" style="width:12px; display:none" ><input style="width:50px;" type="hidden" class="text_boxes" name="txtRateData_<? echo $m; ?>" id="txtRateData_<? echo $m; ?>" /></td> 
                                    <? } else {  $rateBr=""; $plh="Write";  $readonly=""; ?>
                                    ...
                                    <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txt_rate<? echo $m; ?>" id="txt_rate<? echo $m; ?>" onChange="fnc_amount_calculation('fabric',<? echo $m; ?>,this.value);" <?=$rateBr; ?> <?=$readonly; ?> placeholder="<?=$plh; ?>" /></td>
                                ...
                                <tr>
                                    <td colspan="8">
                                        <div style="width:500px; max-height:150px; overflow-y:scroll" id="scroll_body" > 
											<table width="480" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                                            	<thead>
                                                	<tr>
                                                    	<th class="must_entry_caption" colspan="8">Particulars For Accessories</th>
                                                    </tr>
                                                    <tr>
                                                    	<th>Item</th>
                                                        <th>Desc.</th>
                                                ...
                                                for($l=1; $l<=$acc; $l++){ ?>
                                                <tr>
                                                    <td width="100"><?=create_drop_down( "cbo_accessories_id$k", 100, $accessories_arr,"", 1, "-Accessories-", $selected, "fnc_accuom($k);" ); ?></td>
                                                     <td width="60"><input style="width:48px;" type="text" class="text_boxes" name="txt_accdesc_<?=$k; ?>" id="txt_accdesc_<?=$k; ?>" placeholder="DESC" onChange="fnc_itemwise_data_cache(document.getElementById('txt_itemId').value);" /></td>
                                                     <td width="50"><?=create_drop_down( "cboconsuom_$k", 50, $unit_of_measurement,"", 1, "-Select-", 0, "fnc_itemwise_data_cache();",1,"" ); ?></td>
                                                    <td width="60"><input style="width:48px;" type="text" class="text_boxes_numeric" name="txt_accConsumtion<?=$k; ?>" id="txt_accConsumtion<?=$k; ?>" onChange="fnc_amount_calculation('accessories',<?=$k; ?>,document.getElementById('txt_acRate<?=$k; ?>').value);  fnc_itemwise_data_cache(document.getElementById('txt_itemId').value);" /></td>
                                                    <td width="40"><input style="width:27px;" type="text" class="text_boxes_numeric" name="txt_acexper<?=$k; ?>" id="txt_acexper<?=$k; ?>" placeholder="Write" onChange="fnc_amount_calculation('accessories',<?=$k; ?>,document.getElementById('txt_acRate<?=$k; ?>').value);  fnc_itemwise_data_cache(document.getElementById('txt_itemId').value);" /></td>
                                                    <td width="50"><input style="width:38px;" type="text" class="text_boxes_numeric" name="txt_totAccConsumtion<?=$k; ?>" id="txt_totAccConsumtion<?=$k; ?>" disabled /></td>
                                                    <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txt_acRate<?=$k; ?>" id="txt_acRate<?=$k; ?>"  onChange="fnc_amount_calculation('accessories',<?=$k; ?>,this.value);" /></td>
                                                    <td><input style="width:50px;" type="text" class="text_boxes_numeric" name="txt_acValue<?=$k; ?>" id="txt_acValue<?=$k; ?>" readonly /></td>
                                                </tr>
                                                <? $k++; } ?>
                                            </table>
                                        </div>
                    ...
                    <td valign="top" width="270">
                    	<table width="270" cellspacing="0" cellpadding="0" border="0" class="rpt_table" rules="all">
                        	<tr valign="top">
                            	<td width="80"><strong>Buyer Agent</strong><input type="button" class="formbutton" style="width:15px; font-style:italic" value="N" onClick="openmypage_agent_location('requires/quick_costing_v2_controller.php?action=agent_location_popup','Create Buyer Agent',1)"/></td>
                            ...
                            <tr>
                            	<td><strong>B. Location
                            	    <input type="button" class="formbutton" style="width:15px; font-style:italic" value="N" onClick="openmypage_agent_location('requires/quick_costing_v2_controller.php?action=agent_location_popup','Create Location',2)"/></strong></td>
    ...
    <script> 
		var style_id='';
		var temp_style_list=return_ajax_request_value(style_id+'__'+0, 'temp_style_list_view', 'requires/quick_costing_v2_controller');
		$('#style_td').html( temp_style_list );
		//$('#style_td').html( localStorage.getItem("temp_style_list_view") ); 
		fnc_select(); fnc_meeting_no(0); print_report_button_setting(); check_exchange_rate();
    </script>
