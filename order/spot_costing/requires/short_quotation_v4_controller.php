...
if ($action=="load_drop_down_option_id")
{
	...
	/*if($ex_data[1]!=0)
		...
		$selectopt=$selected;*/
		//load_drop_down('requires/short_quotation_v4_controller', '".$row[csf("cost_sheet_no")].'__'.$row[csf("option_id")].'__'.$row[csf("revise_no")]."', 'load_drop_down_revise_no', 'revise_td');
	...
	if($ex_data[1]!=0)
		...
		$selectopt=$max;
	//echo $sql;
	echo create_drop_down( "cbo_option_id", 45, $rvs,"", 0, "-0-", $selectopt, "load_drop_down( 'requires/short_quotation_v4_controller', document.getElementById('txt_costSheetNo').value+'__'+this.value+'__'+0, 'load_drop_down_revise_no', 'revise_td'); fnc_option_rev( document.getElementById('cbo_revise_no').value+'***'+this.value+'***2' );" );//
	exit();
}
...
if($action=="template_popup")
{
	...
	function fnc_template_entry( operation )
	{
		...
		http.open("POST","short_quotation_v4_controller.php",true);
	...
	function fnc_template_entry_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			if(reponse[0]==0 || reponse[0]==1)
			{
				...
				show_list_view( '','template_list_view','save_up_list_view','short_quotation_v4_controller','setFilterGrid(\'tbl_upListView\',-1)');
			}
	...
	{
		var list_view_grid = return_global_ajax_value( temp_id, 'load_php_dtls_form', '', 'short_quotation_v4_controller');
		if(list_view_grid!='')
		{
                ...
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="282" class="rpt_table" id="tbl_tempCreat" >
                	<tbody id="tbd_temp">
                        <tr id="tr_1">
                            ...
                            <? echo create_drop_down( "cboItemBts_1", 150, get_garments_item_array(100),"", 1, "-Select-", 0, "", "", "", "", "", "", "", "", "cboItemBts[]" ); ?></td>
...
if($action=="rate_details_popup")
{
	...
	$adata=$qc_no.'!!!'.$bodypartid.'!!!'.$consRateId.'!!!'.$packQty.'!!!'.$itemRatio.'!!!'.$costing_perid.'!!!'.$rateData;
	?>
    <script>
		var qc_no='<?=$qc_no; ?>';
		...
		var costing_perid='<?=$costing_perid; ?>';
		...
		function fnc_cons_amt()
		{
			...
			var conslbsdzn=conslbspcs*costingper;
			...
			var actuakconslbsdzn=actuakconslbspcs*costingper;
		...
		function fnc_row_generate()
		{
			...
			show_list_view(data,'populate_confirm_fabric_data','fabricDiv','short_quotation_v4_controller','');
			fnc_cons_amt("0_1");
		}
...
if($action=="accrate_details_popup")
{
	...
	$adata=$qc_no.'!!!'.$itemRatio.'!!!'.$acctext.'!!!'.$accuom.'!!!'.$costing_perid.'!!!'.$accRateData;
	?>
    <script>
		var qc_no='<?=$qc_no; ?>';
		var acctext='<?=$acctext; ?>';
		...
		function fnc_cons_amt(str)
		{
			...
			if(type==1)
			{
				...
				var consAccPk=(colorconsdzn/costingper)*colorratio;
			...
			else if(type==2)
			{
				...
				var consAccDzn=(consAccPk/colorratio)*costingper;
		...
		function fnc_accrow_generate()
		{
			...
			show_list_view(data,'populate_confirm_acc_data','accDiv','short_quotation_v4_controller','');
		}
...
if($action=="populate_confirm_acc_data")
{
    ...
    <table width="600" cellspacing="0" border="1" class="rpt_table" id="tbl_acc" rules="all">
        <thead>
            <td colspan="11" align="center" bgcolor="#99FFCC"><b><?=$acctext.', '.$unit_of_measurement[$accuom].'; Costing Per:-'.$costingpertxt; ?></b></td>
        </thead>
        <thead>
            <th width="60">Item Qty</th>
            <th width="80">Color/ Option</th>
            <th width="50">Color Ratio</th>
            <th width="50">Cons/<?=$costingpertxt; ?></th>
            <th width="50">Cons</th>
            <th width="50">Ex %</th>
...
if($action=="stage_saveUpdate_popup")
{
		...
		function fnc_stage_entry_reponse()
		{
			if(http.readyState == 4) 
			{
				...
				if(reponse[0]==0 || reponse[0]==1)
				{
					...
					show_list_view('','stage_data_list','stage_data_div','short_quotation_v4_controller','');
				}
			}
			release_freezing();
		}
    ...
    <script>show_list_view('','stage_data_list','stage_data_div','short_quotation_v4_controller','');</script>          
    <script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
    </html>
    <?
	exit();	
...
{
	echo  create_list_view ( "list_view", "Stage Name", "200","200","150",1, "select tuid, stage_name from lib_stage_name where status_active=1 and is_deleted=0 order by tuid desc", "get_php_form_data", "tuid","'load_php_data_to_form_satge'", 1, "0", $arr, "stage_name","short_quotation_v4_controller", 'setFilterGrid("list_view",-1);','0' );
	exit();
}
...
if($action=="save_update_delete")
{
	...
	where  b.team_id in (select id from lib_marketing_team a where user_tag_id='$user_id' and a.status_active=1 and a.is_deleted=0 ) and b.status_active=1 and b.is_deleted=0");
		...
		$field_array_mst="id, company_id, location_id, cost_sheet_id, cost_sheet_no, temp_id, lib_item_id, inquery_id, style_ref, buyer_id, season_id, season_year, brand_id, style_des, prod_dept, prod_code, department_id, delivery_date, exchange_rate, offer_qty, quoted_price, tgt_price, margin, stage_id, costing_date, pre_cost_sheet_id, revise_no, option_id, buyer_remarks, option_remarks, article, qc_no, entry_form, fab_process_loss_method, acc_process_loss_method, gauge, no_of_ends, sample_color, sample_size, sample_range, costing_per, inserted_by, insert_date, status_active, is_deleted";
		
		if(str_replace("'","",$txtsamplecolor)!="")
		{ 
			if (!in_array(str_replace("'","",$txtsamplecolor),$tmp_array_color))
			{
				$smpcolor_id = return_id( str_replace("'","",$txtsamplecolor), $color_library, "lib_color", "id,color_name","511");  
				$tmp_array_color[$smpcolor_id]=str_replace("'","",$txtsamplecolor);
			}
			else $smpcolor_id =  array_search(str_replace("'","",$txtsamplecolor), $tmp_array_color); 
		}
		else $smpcolor_id=0;
		
		if(str_replace("'","",$txtsamplesize)!="")
		{ 
			if (!in_array(str_replace("'","",$txtsamplesize),$tmp_array_size))
			{
				$smpsize_id = return_id( str_replace("'","",$txtsamplesize), $size_library, "lib_size", "id,size_name","511");  
				$tmp_array_size[$smpsize_id]=str_replace("'","",$txtsamplesize);
			}
			else $smpsize_id =  array_search(str_replace("'","",$txtsamplesize), $tmp_array_size); 
		}
		else $smpsize_id=0;
		
		$data_array_mst="(".$mst_id.", ".$cbo_company_id.", ".$cbo_location_id.", ".$mst_id.", '".$autoCostSheetNo."', ".$cbo_temp_id.", ".$txt_temp_id.", ".$txt_inquery_id.", '".strtoupper(str_replace("'","",$txt_styleRef))."', ".$cbo_buyer_id.", ".$cbo_season_id.", ".$cbo_season_year.", ".$cbo_brand.", '".strtoupper(str_replace("'","",$txt_styleDesc))."', ".$cbo_product_department.", ".$txt_product_code.", ".$cbo_subDept_id.", ".$txt_delivery_date.", ".$txt_exchangeRate.", ".$txt_offerQty.", ".$txt_quotedPrice.", ".$txt_tgtPrice.", ".$txtmarign.", ".$cbo_stage_id.", ".$txt_costingDate.", '".$update_id."', '".$revise_no."', '".$option_id."', '".strtoupper($txt_costing_remarks)."', '".strtoupper($txt_option_remarks)."', ".$txt_article.", '".$qcno."', 511, ".$txt_fab_process_loss_method.", ".$txt_acc_process_loss_method.", ".$cbo_gauge.", ".$txtnoofends.", '".$smpcolor_id."', '".$smpsize_id."', ".$txtsamplerange.",".$cbo_costing_per.", ".$_SESSION['logic_erp']['user_id'].", '".$pc_date_time."', 1, 0)";
		...
		$field_array_consrate="id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, value, fab_id, description, body_part_id, supp_ref, remarks, uom, inserted_by, insert_date, status_active, is_deleted, color_type_id, rate_data, use_for";
		...
		foreach($temp_id as $item_id)
		{
			...
			foreach($ex_itemConsRateData as $item_wise_consRateData)
			{
				if($item_wise_consRateData!="")
				{
					...
					$fabDesc=$rateData="";
					$bodypartid=$fabid=$colortype=$conspack=$exPer=$totCons=$ratePop=$rate=$fabAmt=$fabupid=$fabuom=0;
					...
					$conspack=$ex_itemConsRate[5];
					$exPer=$ex_itemConsRate[6];
					$totCons=$ex_itemConsRate[7];
					$ratePop=$ex_itemConsRate[8];
					$rateData=$ex_itemConsRate[9];
					$rate=$ex_itemConsRate[10];
					$fabAmt=$ex_itemConsRate[11];
					$fabupid=$ex_itemConsRate[12];
					$fabuom=$ex_itemConsRate[13];
					
					if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
					$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',1,0,'".$conspack."','".$exPer."','".$totCons."','".$ratePop."','".$rate."','".$fabAmt."','".$fabid."','".$fabDesc."','".$bodypartid."','','','".$fabuom."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,'".$colortype."','".$rateData."','')";
			...
			foreach($ex_specialData as $item_wise_specialData)
			{
				...
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',2,'".$speciaOperation_id."','".$spConsumtion."','".$spExper."','".$totSpCons."',0,'".$spRate."','".$spValue."','".$speciaTypeId."','','".$spbodypartid."','','".$spRemarks."',0,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,0,'','')";
			...
			foreach($ex_washData as $item_wise_washData)
			{
				...
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',3,'".$washTypeId."','".$wConsumtion."','".$wExper."','".$totwCons."',0,'".$wRate."','".$wValue."',0,'','".$wbodypartid."','','".$wRemarks."',0,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,0,'','')";
			...
			foreach($ex_itemAccData as $item_wise_itemAccData)
			{
				...
				$accessories_id=$accDescription=$accBandRef=$accUseFor=$acConsumtion=$acExPer=$totAccCons=$acRate=$acVal=$acconsUom=$acConsCal=$acRateData=0;
				...
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',4,'".$accessories_id."','".$acConsumtion."','".$acExPer."','".$totAccCons."','".$acConsCal."','".$acRate."','".$acVal."',0,'".$accDescription."',0,'".$accBandRef."','','".$acconsUom."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,0,'".$acRateData."','".$accUseFor."')";
		...
		{
			$fab_td=$spOpe_td=$wash_td=$acc_td=$txtCpm=$txtSmv=$txtEff=$txtCmCost=$txtFriCost=$txtLtstCost=$txtMissCost=$txtOtherCost=$txtCommCost=$fobT_td=$fobDzn_td=$tot_item_id=$txtCommlCost=$optexp=$txtoptexp=$txtknittingtime=$txtmakeuptime=$txtfinishingtime=0;
...
where b.team_id in (select id from lib_marketing_team a where user_tag_id='$user_id' and a.status_active=1 and a.is_deleted=0 ) and b.status_active=1 and b.is_deleted=0");
		...
		$data_array_mst="".$cbo_location_id."*".$cbo_temp_id."*".$txt_temp_id."*".$txt_inquery_id."*'".strtoupper(str_replace("'","",$txt_styleRef))."'*".$cbo_buyer_id."*".$cbo_season_id."*".$cbo_season_year."*".$cbo_brand."*'".strtoupper(str_replace("'","",$txt_styleDesc))."'*".$cbo_product_department."*".$txt_product_code."*".$cbo_subDept_id."*".$txt_delivery_date."*".$txt_exchangeRate."*".$txt_offerQty."*".$txt_quotedPrice."*".$txt_tgtPrice."*".$txtmarign."*".$cbo_stage_id."*".$txt_costingDate."*'".strtoupper($txt_costing_remarks)."'*".$txt_article."*'".strtoupper($txt_option_remarks)."'*".$cbo_gauge."*".$txtnoofends."*'".$smpcolor_id."'*'".$smpsize_id."'*".$txtsamplerange."*".$cbo_costing_per."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		$field_array_consrate="id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, value, fab_id, description, body_part_id, supp_ref, remarks, uom, use_for, rate_data, inserted_by, insert_date, status_active, is_deleted, color_type_id";
		
		$fieldArrConsRate="particular_type_id*consumption*ex_percent*tot_cons*is_calculation*rate*value*fab_id*description*body_part_id*supp_ref*remarks*uom*updated_by*update_date*color_type_id*rate_data*use_for";
		...
		foreach($temp_id as $item_id)
		{
			...
			foreach($ex_itemConsRateData as $item_wise_consRateData)
			{
				if($item_wise_consRateData!="")
				{
					...
					$fabDesc=$rateData="";
					$bodypartid=$fabid=$colortype=$conspack=$exPer=$totCons=$ratePop=$rate=$fabAmt=$fabupid=0;
					...
					$rate=$ex_itemConsRate[10];
					$fabAmt=$ex_itemConsRate[11];
					$fabupid=($ex_itemConsRate[12]*1);
					$fabuom=$ex_itemConsRate[13];
					
					if($fabupid>0)
					{
						$id_arr[]=$fabupid;
						$fabConsRateUpdateArr[$fabupid] =explode("*",("'0'*'".$conspack."'*'".$exPer."'*'".$totCons."'*'".$ratePop."'*'".$rate."'*'".$fabAmt."'*'".$fabid."'*'".$fabDesc."'*'".$bodypartid."'*''*''*'".$fabuom."'*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'*'".$colortype."'*'".$rateData."'*''"));
					}
					else if($fabupid==0)
					{
						if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
						$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',1,0,'".$conspack."','".$exPer."','".$totCons."','".$ratePop."','".$rate."','".$fabAmt."','".$fabid."','".$fabDesc."','".$bodypartid."','','','".$fabuom."','','".$rateData."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,'".$colortype."')";
			...
			foreach($ex_itemAccData as $item_wise_itemAccData)
			{
				...
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',4,'".$accessories_id."','".$acConsumtion."','".$acExPer."','".$totAccCons."','".$acConsCal."','".$acRate."','".$acVal."',0,'".$accDescription."',0,'".$accBandRef."','','".$acconsUom."','".$accUseFor."','".$acRateData."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,0)";
		...
		foreach($ex_item_wise_tot_data as $item_wise_tot)
		{
			$fab_td=$spOpe_td=$wash_td=$acc_td=$txtCpm=$txtSmv=$txtEff=$txtCmCost=$txtFriCost=$txtLtstCost=$txtMissCost=$txtOtherCost=$txtCommCost=$fobT_td=$fobDzn_td=$txtoptexp=$tot_item_id=$txtknittingtime=$txtmakeuptime=$txtfinishingtime=0;
...
if($action=="style_popup")
{
	...
	{
		show_list_view (document.getElementById('cbo_buyer_id').value+'**'+document.getElementById('cbo_season_id').value+'**'+document.getElementById('cbo_subDept_id').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('txt_cost_sheet_no').value+'**'+document.getElementById('hide_dataType').value+'**'+document.getElementById('cbo_approval_type').value+'**'+document.getElementById('cbo_string_search_type').value, 'style_ref_search_list_view', 'search_div', 'short_quotation_v4_controller', 'setFilterGrid(\'tbl_list_search\',-1)');
		set_all();
	}
	...
	function fnc_confirm_style(qcno,upid)
	{
		if(upid!="")
		{
			...
			var page_link='short_quotation_v4_controller.php?action=confirmStyle_popup';
    ...
    <div align="center">
		...
		<fieldset>
            <table cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr class="general">
                        <td><? echo create_drop_down( "cbo_buyer_id", 130, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name",1, "-- All Buyer--",$buyerId,"load_drop_down( 'short_quotation_v4_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'short_quotation_v4_controller',this.value, 'load_drop_down_sub_dep', 'sub_td' );",0 ); ?></td>                 
...
if($action=="style_ref_search_list_view")
{
	...
	else
	{
		...
		if($string_search_type==1)
		{
			...
			if($cost_sheet_no!='') $cost_sheet_no_cond=" and ost_sheet_no='$cost_sheet_no'"; else $cost_sheet_no_cond="";
		}
		else if($string_search_type==2)
		{
	...
	{
		$sql_style = "select min(id) as id, cost_sheet_no, max(costing_date) as costing_date, buyer_id, season_id, style_ref, department_id, temp_id, lib_item_id, stage_id, max(inserted_by) as inserted_by, max(updated_by) as updated_by, min(qc_no) as qc_no from qc_mst where status_active=1 and is_deleted=0 and entry_form=511 $buyer_id_cond $style_ref_cond $cost_sheet_no_cond $season_id_cond $department_id_cond $approvaltype_cond group by cost_sheet_no, buyer_id, season_id, style_ref, department_id, temp_id, lib_item_id, stage_id order by id Desc";
...
if($action=="temp_style_list_view")
{
	...
	if($ex_data[1]==1)
	{
		...
		$sql_style= "select min(id) as id, cost_sheet_no, style_ref, min(qc_no) as qc_no, season_id, inserted_by from qc_mst where entry_form=511 and cost_sheet_no in ($ex_data[0]) and cost_sheet_no not in (select cost_sheet_no from qc_style_temp_data where cost_sheet_no in ($ex_data[0]) and inserted_by='$user_id' and status_active=1 and is_deleted=0 ) and revise_no=0 and option_id=0 and status_active=1 and is_deleted=0 group by cost_sheet_no, style_ref, season_id, inserted_by";
		//echo  $sql_style;
	...
	$sql_temp_style=sql_select("select a.id, a.cost_sheet_no, a.style_ref, a.season_id, a.inserted_by, a.updated_by, a.qc_no, a.season_year from qc_mst a, qc_style_temp_data b where a.id=b.mst_id and b.inserted_by='$user_id' and a.entry_form=511 and b.status_active=1 and b.is_deleted=0 order by a.style_ref ASC");
...
if($action=="populate_style_details_data")
{
  ...
  //echo $data_array;die();
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_drop_down('requires/short_quotation_v4_controller', '".trim($row[csf("company_id")])."', 'load_drop_down_location', 'location_td'); \n";
		...
		echo "load_drop_down('requires/short_quotation_v4_controller', '".trim($itemNratioArr[$row[csf("temp_id")]]['ratio'])."', 'load_drop_down_tempItem', 'item_td'); \n";
		...
		echo "load_drop_down('requires/short_quotation_v4_controller', '".$row[csf("buyer_id")]."', 'load_drop_down_season', 'season_td');\n"; 
		echo "load_drop_down('requires/short_quotation_v4_controller', '".$row[csf("buyer_id")]."', 'load_drop_down_brand', 'brand_td');\n";
		echo "load_drop_down('requires/short_quotation_v4_controller', '".$row[csf("buyer_id")]."', 'load_drop_down_sub_dep', 'sub_td');\n";
		...
		echo "$('#cbo_gauge').val('".$row[csf("gauge")]."');\n";
		echo "$('#txtnoofends').val('".$row[csf("no_of_ends")]."');\n"; 
		echo "$('#txtsamplecolor').val('".$color_library[$row[csf("sample_color")]]."');\n";
		echo "$('#txtsamplesize').val('".$size_library[$row[csf("sample_size")]]."');\n";
		echo "$('#txtsamplerange').val('".$row[csf("sample_range")]."');\n";
		...
		{
			//echo "load_drop_down('requires/short_quotation_v4_controller', '".$row[csf("cost_sheet_no")].'__'.$row[csf("option_id")].'__'.$row[csf("revise_no")]."', 'load_drop_down_revise_no', 'revise_td');\n"; //.'__'.$row[csf("revise_no")]
			//echo "load_drop_down('requires/short_quotation_v4_controller', '".$row[csf("cost_sheet_no")].'__'.$row[csf("option_id")]."', 'load_drop_down_option_id', 'option_td');\n"; 
		}
	...
	$sql_cons_rate="select id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, value, fab_id, description, body_part_id, supp_ref, remarks, uom, color_type_id, dznkg, rate_data, use_for from qc_cons_rate_dtls where mst_id=$cond_data and status_active=1 and is_deleted=0 order by id asc";
	...
	foreach ($sql_result_cons_rate as $rowConsRate)
	{
		...
		{
			$cons_rate_fab_arr[$rowConsRate[csf("item_id")]].=$body_part[$rowConsRate[csf("body_part_id")]].'~!~'.$rowConsRate[csf("body_part_id")].'~!~'.$rowConsRate[csf("description")].'~!~'.$rowConsRate[csf("fab_id")].'~!~'.$rowConsRate[csf("color_type_id")].'~!~'.$rowConsRate[csf("consumption")].'~!~'.$rowConsRate[csf("ex_percent")].'~!~'.$rowConsRate[csf("tot_cons")].'~!~'.$rowConsRate[csf("is_calculation")].'~!~'.$rowConsRate[csf("rate_data")].'~!~'.$rowConsRate[csf("rate")].'~!~'.$rowConsRate[csf("value")].'~!~'.$rowConsRate[csf("id")].'~!~'.$rowConsRate[csf("uom")].'#^';
		}
		...
		{
			$cons_rate_ac_arr[$rowConsRate[csf("item_id")]].=$itemGroupArr[$rowConsRate[csf("particular_type_id")]].'~!~'.$rowConsRate[csf("particular_type_id")].'~!~'.$rowConsRate[csf("description")].'~!~'.$rowConsRate[csf("supp_ref")].'~!~'.$rowConsRate[csf("consumption")].'~!~'.$rowConsRate[csf("ex_percent")].'~!~'.$rowConsRate[csf("tot_cons")].'~!~'.$rowConsRate[csf("rate")].'~!~'.$rowConsRate[csf("value")].'~!~'.$rowConsRate[csf("use_for")].'~!~'.$rowConsRate[csf("uom")].'~!~'.$rowConsRate[csf("is_calculation")].'~!~'.$rowConsRate[csf("rate_data")].'#^';
		}
	...
	$sql_summ="select buyer_agent_id, location_id, no_of_pack, is_confirm, is_cm_calculative, mis_lumsum_cost, commision_per, tot_fab_cost, tot_sp_operation_cost, tot_wash_cost, tot_accessories_cost, tot_cm_cost, tot_fright_cost, tot_lab_test_cost, tot_miscellaneous_cost, tot_other_cost, tot_commission_cost, tot_cost, tot_fob_cost, tot_rmg_ratio, commercial_per, commercial_cost, operating_exp, knitting_time, makeup_time, finishing_time from qc_tot_cost_summary where mst_id='$cond_data' and status_active=1 and is_deleted=0";
	...
	foreach($sql_result_summ as $rowSumm)
	{
		...
		echo "$('#totFOBCost_td').text('".number_format(($rowSumm[csf("tot_cost")]/$costing_per),4)."');\n";
		echo "$('#totknittime_td').text('".$rowSumm[csf("knitting_time")]."');\n";
		echo "$('#totmakeuptime_td').text('".$rowSumm[csf("makeup_time")]."');\n";
		echo "$('#totfinishtime_td').text('".$rowSumm[csf("finishing_time")]."');\n";
	}
	unset($sql_result_summ);
	$sql_item_summ="select item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs, rmg_ratio, commercial_cost, operating_exp, knitting_time, makeup_time, finishing_time from qc_item_cost_summary where mst_id='$cond_data' and status_active=1 and is_deleted=0";
	...
	foreach($sql_result_item_summ as $rowItemSumm)
	{
		...
		echo "$('#fobPcsT_td".$rowItemSumm[csf("item_id")]."').text('".number_format(($rowItemSumm[csf("fob_pcs")]/$costing_per),4)."');\n";
		...
		echo "$('#txtknittingtime_".$rowItemSumm[csf("item_id")]."').val('".$rowItemSumm[csf("knitting_time")]."');\n";
		echo "$('#txtmakeuptime_".$rowItemSumm[csf("item_id")]."').val('".$rowItemSumm[csf("makeup_time")]."');\n";
		echo "$('#txtfinishingtime_".$rowItemSumm[csf("item_id")]."').val('".$rowItemSumm[csf("finishing_time")]."');\n";
	}
...
if($action=="copy_cost_sheet")
{
	...
	{	
		...
		if (is_duplicate_field( "cost_sheet_no", "qc_mst", "style_ref='$dup_styleRef' and season_id='$dup_season_id' and buyer_id='$dup_buyer_id' and entry_form=511 and status_active=1 and is_deleted=0")==1)
		{
			echo "11**".$msg; 
			die;
		}
		...
		$cbo_gauge=$sql_mst[0][csf('gauge')];
		$txtnoofends=$sql_mst[0][csf('no_of_ends')];
		$txtsamplecolor=$sql_mst[0][csf('sample_color')];
		$txtsamplesize=$sql_mst[0][csf('sample_size')];
		$txtsamplerange=$sql_mst[0][csf('sample_range')];
...
if($action=="confirmStyle_popup")
{
    ...
    <script>
		...
		function fnc_confirm_entry( operation )
		{
			if(operation==3)
			{
				...
				var opexp_amount=$("#txtOpExpCst_bom").val();
				...
				var qc_fab_amt=0; var qc_sp_amt=0; var qc_wash_amt=0; var qc_acc_amt=0; var qc_fri_amt=0; var qc_lab_amt=0; var qc_opexp_amt=0; var qc_other_amt=0; var qc_comml_amt=0; var qc_comm_amt=0; var qc_fob_amt=0; var qc_cm_amt=0; var qc_rmg_amt=0;
				for(j=1; j<=split_tmep_id.length; j++)
				{
					...
					qc_opexp_amt+=$("#txtOpExpCst_"+itm_id).val()*1;
				...
				if(job_no!="")
				{
					...
					else if(qc_opexp_amt<opexp_amount)
					{
			...
			for(i=1; i<=split_tmep_id.length; i++)
			{
				...
				//data_dtls+=get_submitted_data_string('txtitemid_'+itm_id+'*txtdtlsupid_'+itm_id+'*txtFabCst_'+itm_id+'*txtSpOpa_'+itm_id+'*txtAcc_'+itm_id+'*txtFrightCst_'+itm_id+'*txtLabCst_'+itm_id+'*txtMiscCst_'+itm_id+'*txtOtherCst_'+itm_id+'*txtCommCst_'+itm_id+'*txtCommlCst_'+itm_id+'*txtFobDzn_'+itm_id+'*txtCpm_'+itm_id+'*txtSmv_'+itm_id+'*txtCmCst_'+itm_id+'*txtPack_'+itm_id,"../../",2);
				//k++;
				
				data_all+="&txtitemid_" + itm_id + "='" + $('#txtitemid_'+itm_id).val()+"'"+"&txtdtlsupid_" + itm_id + "='" + $('#txtdtlsupid_'+itm_id).val()+"'"+"&txtFabCst_" + itm_id + "='" + $('#txtFabCst_'+itm_id).val()+"'"+"&txtSpOpa_" + itm_id + "='" + $('#txtSpOpa_'+itm_id).val()+"'"+"&txtWash_" + itm_id + "='" + $('#txtWash_'+itm_id).val()+"'"+"&txtAcc_" + itm_id + "='" + $('#txtAcc_'+itm_id).val()+"'"+"&txtFrightCst_" + itm_id + "='" + $('#txtFrightCst_'+itm_id).val()+"'"+"&txtLabCst_" + itm_id + "='" + $('#txtLabCst_'+itm_id).val()+"'"+"&txtOpExpCst_" + itm_id + "='" + $('#txtOpExpCst_'+itm_id).val()+"'"+"&txtOtherCst_" + itm_id + "='" + $('#txtOtherCst_'+itm_id).val()+"'"+"&txtCommCst_" + itm_id + "='" + $('#txtCommCst_'+itm_id).val()+"'"+"&txtCommlCst_" + itm_id + "='" + $('#txtCommlCst_'+itm_id).val()+"'"+"&txtFobDzn_" + itm_id + "='" + $('#txtFobDzn_'+itm_id).val()+"'"+"&txtCpm_" + itm_id + "='" + $('#txtCpm_'+itm_id).val()+"'"+"&txtSmv_" + itm_id + "='" + $('#txtSmv_'+itm_id).val()+"'"+"&txtCmCst_" + itm_id + "='" + $('#txtCmCst_'+itm_id).val()+"'"+"&txtPack_" + itm_id + "='" + $('#txtPack_'+itm_id).val()+"'";
				k++;
			}
		...
		function fnc_stage_entry_reponse()
		{
			if(http.readyState == 4) 
			{
				...
				if(trim(reponse[0])==0 || trim(reponse[0])==1 || trim(reponse[0])==2 || trim(reponse[0])==3)
				{
					...
					get_php_form_data($('#txt_costSheet_id').val()+'__'+$('#txtItem_id').val(),'populate_confirm_style_form_data','short_quotation_v4_controller');
					//show_list_view('','stage_data_list','stage_data_div','short_quotation_v4_controller','');
					js_set_value();
				}
			}
		...
		function fnc_openJobPopup()
		{
			...
			page_link='short_quotation_v4_controller.php?action=style_tag_popup&data='+data;
		...
		function fnc_bom_data_load()
		{
			...
			if(job_no!="")
			{
				var str_data=return_global_ajax_value( job_no, 'budgete_cost_validate', '', 'short_quotation_v4_controller');
				
				var spdata=str_data.split("##");
				var fab_amount=spdata[0]; var sp_oparation_amount=spdata[1]; var wash_amount=spdata[2]; var acc_amount=spdata[3]; var fright_amount=spdata[4]; var lab_amount=spdata[5]; var opexp_amount=spdata[6]; var other_amount=spdata[7]; var comm_amount=spdata[8]; var fob_amount=spdata[9]; var comml_amount=spdata[10]; var cm_amount=spdata[11]; var rmg_ratio=spdata[12];
				
				if(qcCons_from==1)
				{
					var bomTotCost=(number_format(fab_amount,4)*1)+(number_format(sp_oparation_amount,4)*1)+(number_format(wash_amount,4)*1)+(number_format(acc_amount,4)*1)+(number_format(fright_amount,4)*1)+(number_format(lab_amount,4)*1)+(number_format(opexp_amount,4)*1)+(number_format(other_amount,4)*1)+(number_format(comml_amount,4)*1)+(number_format(comm_amount,4)*1);
					
					var bomCost='Fab='+fab_amount+'+ SP='+sp_oparation_amount+'+ Wash='+wash_amount+'+ ACC='+acc_amount+'+ FR='+fright_amount+'+ Lab='+lab_amount+'+ OPEXP='+opexp_amount+'+ Others='+other_amount+'+ Comml.='+comml_amount+'+ Comm='+comm_amount+'= Total='+bomTotCost;
				...
				$("#txtOpExpCst_bom").val(opexp_amount);
		...
		function fnc_total_calculate()
		{
			...
			var split_tmep_id=temp_id.split(',');
			var ab=0;
			var qc_fab_amt=0; var qc_sp_amt=0; var qc_wash_amt=0; var qc_acc_amt=0; var qc_fri_amt=0; var qc_lab_amt=0; var qc_opexp_amt=0; var qc_other_amt=0; var qc_comml_amt=0; var qc_comm_amt=0; var qc_fob_amt=0; var qc_cpm_amt=0; var qc_smv_amt=0; var qc_cm_amt=0; var qc_rmg_amt=0;
			for(j=1; j<=split_tmep_id.length; j++)
			{
				...
				item_tot_amount=($("#txtFabCst_"+itm_id).val()*1)+($("#txtSpOpa_"+itm_id).val()*1)+($("#txtWash_"+itm_id).val()*1)+($("#txtAcc_"+itm_id).val()*1)+($("#txtFrightCst_"+itm_id).val()*1)+($("#txtLabCst_"+itm_id).val()*1)+($("#txtOpExpCst_"+itm_id).val()*1)+($("#txtOtherCst_"+itm_id).val()*1)+($("#txtCommlCst_"+itm_id).val()*1)+($("#txtCommCst_"+itm_id).val()*1);
				
				var itemCost='Fab='+$("#txtFabCst_"+itm_id).val()+'+ SP='+$("#txtSpOpa_"+itm_id).val()+'+ Wash='+$("#txtWash_"+itm_id).val()+'+ ACC='+$("#txtAcc_"+itm_id).val()+'+ FR='+$("#txtFrightCst_"+itm_id).val()+'+ Lab='+$("#txtLabCst_"+itm_id).val()+'+ OpExp='+$("#txtOpExpCst_"+itm_id).val()+'+ Others='+$("#txtOtherCst_"+itm_id).val()+'+ Comml.='+$("#txtCommlCst_"+itm_id).val()+'+ Comm='+$("#txtCommCst_"+itm_id).val()+'= Total='+item_tot_amount;
			...
			$("#txtOpExpCst_qc").val( number_format(qc_opexp_amt,4,'.','') );
			...
			var total_amount=qc_fab_amt+qc_sp_amt+qc_wash_amt+qc_acc_amt+qc_fri_amt+qc_lab_amt+qc_opexp_amt+qc_other_amt+qc_comml_amt+qc_comm_amt;
		...
		function openmypage_unapprove_request()
		{
			...
			var page_link = 'short_quotation_v4_controller.php?data='+data+'&action=unapp_request_popup';
		...
		function fncqclimit(str)
		{
			...
			var tdopexpamt=$('#tdopexpamt'+item_id).text()*1;
			var tdothamt=$('#tdothamt'+item_id).text()*1;
			var tdcomlamt=$('#tdcomlamt'+item_id).text()*1;
			...
			if(qcCons_from==2)
			{
				...
				if( validatetype==9 && tdopexpamt < ($("#txtOpExpCst_"+item_id).val()*1) )
				{
					alert("QC BOM Limit (Opt. Exp.) is Greater Than QC!!!");
					$("#txtOpExpCst_"+item_id).val(tdopexpamt);
					//return;
				}
				...
				{
					alert("QC BOM Limit (FOB ($)) is Greater Than QC!!!");
				...
				if( validatetype==17 && tdcomlamt < ($("#txtCommlCst_"+item_id).val()*1) )
				{
					alert("QC BOM Limit (Commercial Cost) is Greater Than QC!!!");
					$("#txtCommlCst_"+item_id).val(tdcomlamt);
					//return;
				}
				if( validatetype==18 && tdwashamt < ($("#txtWash_"+item_id).val()*1) )
				{
					alert("QC BOM Limit (Wash Cost) is Greater Than QC!!!");
    ...
    <div style="display:none"><? echo load_freeze_divs ("../../../",$permission);  ?></div>       
        <form name="confirmStyle_1" id="confirmStyle_1" autocomplete="off">
        	<table width="850">
                <tr>
                    ...
                    <td width="120"><? echo create_drop_down( "cbo_buyer_id", 120, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name", 1, "-Select Buyer-", $sql_data[0][csf('buyer_id')], "load_drop_down( 'short_quotation_v4_controller', this.value, 'load_drop_down_season_conf', 'season_conf_td'); load_drop_down( 'short_quotation_v4_controller',this.value, 'load_drop_down_sub_depConf', 'subConf_td' );",1 ); ?></td>
                ...
                <tr>
                	...
                	<td colspan="3"><strong>Un-App. Request</strong>&nbsp;&nbsp;
            ...
            <table width="930" cellspacing="0" border="1" rules="all" class="rpt_table">
        	<thead>
            	<th width="80">Item</th>
                <th width="50">Yarn Cost</th>
			...
			<?
            $sql_item_summ="select item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, operating_exp, other_cost, commercial_cost, commission_cost, fob_pcs from qc_item_cost_summary where mst_id='$qc_no' and status_active=1 and is_deleted=0";
            ...
            foreach($sql_result_item_summ as $rowItemSumm)
            {
                ...
                <tr id="trVal_<?=$z; ?>" bgcolor="<?=$bgcolor; ?>">
                    ...
                    <td align="right" id="tdopexpamt<?=$item_id; ?>"><? echo number_format($rowItemSumm[csf("operating_exp")],4); ?></td>
                    <td align="right" id="tdothamt<?=$item_id; ?>"><? echo number_format($rowItemSumm[csf("other_cost")],4); ?></td>
                    <td align="right" id="tdcomlamt<?=$item_id; ?>"><? echo number_format($rowItemSumm[csf("commercial_cost")],4); ?></td>
                ...
                <tr id="tr_<? echo $z; ?>" bgcolor="<? echo $bgcolorN; ?>">
                    ...
                    <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtWash_<?=$item_id; ?>" id="txtWash_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("wash_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_18'; ?>');" <?=$disable; ?> /></td>
                    ...
                    <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtFrightCst_<?=$item_id; ?>" id="txtFrightCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("frieght_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_7'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtLabCst_<?=$item_id; ?>" id="txtLabCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("lab_test_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_8'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtOpExpCst_<?=$item_id; ?>" id="txtOpExpCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("operating_exp")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_9'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtOtherCst_<?=$item_id; ?>" id="txtOtherCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("other_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_10'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtCommlCst_<?=$item_id; ?>" id="txtCommlCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("commercial_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_17'; ?>');" <?=$disable; ?> /></td>
			...
			$sql="select fab_cons_kg, fab_cons_yds, fab_amount, sp_oparation_amount, wash_amount, acc_amount, fright_amount, lab_amount, operating_exp, other_amount, commercial_cost, comm_amount, fob_amount, cm_amount, rmg_ratio from qc_confirm_dtls where cost_sheet_id='$cost_sheet_id' and status_active=1 and is_deleted=0";
        ...
        <table width="925" cellspacing="0" border="1" rules="all" class="rpt_table">
        	<tr id="tr_qc" bgcolor="#CCFFCC">
                ...
                <td width="50"><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtOpExpCst_qc" id="txtOpExpCst_qc" value="" disabled /></td>
        	...
        	<tr id="tr_bom" bgcolor="#CCCCCC">
                ...
                <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtOpExpCst_bom" id="txtOpExpCst_bom" value="" readonly /></td>
    ...
    <script>get_php_form_data($('#txt_costSheet_id').val()+'__'+$('#txtItem_id').val(),'populate_confirm_style_form_data','short_quotation_v4_controller'); fnc_bom_data_load(); fnc_total_calculate(); fnc_select();</script>          
    <script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
    </html>
    <?
	exit();	
...
if($action=="save_update_delete_confirm_style")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$dtls_field_arr="id, mst_id, cost_sheet_id, item_id, fab_amount, sp_oparation_amount, wash_amount, acc_amount, fright_amount, lab_amount, operating_exp, other_amount, commercial_cost, comm_amount, fob_amount, cppm_amount, smv_amount, cm_amount, rmg_ratio, inserted_by, insert_date";
		//echo "10**";
		...
		for($i=1; $i<=count($itm_id); $i++)
		{
			...
			$txtOpExpCst='txtOpExpCst_'.$item_id;
			...
			if ($add_comma!=0) $dtls_data_arr .=",";
			$dtls_data_arr .="(".$confirm_dtls_id.",".$confirm_mst_id.",".$txt_costSheet_id.",".$$txtitemid.",".$$txtFabCst.",".$$txtSpOpa.",".$$txtWash.",".$$txtAcc.",".$$txtFrightCst.",".$$txtLabCst.", ".$$txtOpExpCst.", ".$$txtOtherCst.", ".$$txtCommlCst.", ".$$txtCommCst.", ".trim($$txtFobDzn).",".$$txtCpm.",".$$txtSmv.",".$$txtCmCst.",".$$txtPack.",".$user_id.",'".$pc_date_time."')";
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		$dtls_field_arr="fab_amount*sp_oparation_amount*wash_amount*acc_amount*fright_amount*lab_amount*operating_exp*other_amount*commercial_cost*comm_amount*fob_amount*cppm_amount*smv_amount*cm_amount*rmg_ratio*updated_by*update_date";
		//echo "10**";
		$itm_id=explode(",",str_replace("'","",$txtItem_id));
		//print_r($itm_id);
		//echo count($itm_id);
		...
		for($i=1; $i<=count($itm_id); $i++)
		{
			...
			$dtls_data_arr[str_replace("'",'',$$txtdtlsupid)] =explode("*",("".$$txtFabCst."*".$$txtSpOpa."*".$$txtWash."*".$$txtAcc."*".$$txtFrightCst."*".$$txtLabCst."*".$$txtOpExpCst."*".$$txtOtherCst."*".$$txtCommlCst."*".$$txtCommCst."*".$$txtFobDzn."*".$$txtCpm."*".$$txtSmv."*".$$txtCmCst."*".$$txtPack."*'".$user_id."'*'".$pc_date_time."'"));
			$k++;
		}
...
if($action=="populate_confirm_style_form_data")
{
	...
	$sql_confirm_dtls=sql_select("select id, mst_id, cost_sheet_id, item_id, fab_amount, sp_oparation_amount, wash_amount, acc_amount, fright_amount, lab_amount, operating_exp, other_amount, commercial_cost, comm_amount, fob_amount, cppm_amount, smv_amount, cm_amount, rmg_ratio from qc_confirm_dtls where cost_sheet_id='$ex_data[0]'");
	foreach($sql_confirm_dtls as $row_dtls)
	{
		...
		echo "$('#txtOpExpCst_".$row_dtls[csf("item_id")]."').val('".$row_dtls[csf("operating_exp")]."');\n";
...
if($action=="budgete_cost_validate")
{
	...
	if($job_no!="")
	{
		...
		$dataArr=sql_select($sql);
		$fab_amount=$sp_oparation_amount=$wash_amount=$acc_amount=$fright_amount=$lab_amount=$opexp_amount=$other_amount=$comml_amount=$comm_amount=$fob_amount=$cm_amount=$rmg_ratio=0;
		...
		foreach($dataArr as $row)
		{
			...
			$opexp_amount+=$row[csf("common_oh")];
		...
		$str_data=$fab_amount."##".$sp_oparation_amount."##".$wash_amount."##".$acc_amount."##".$fright_amount."##".$lab_amount."##".$opexp_amount."##".$other_amount."##".$comm_amount."##".$fob_amount."##".$comml_amount."##".$cm_amount."##".$rmg_ratio;
		unset($dataArr);
	}
        ...
        <table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
            ...
            <tr class="general">
                <td> 
					...
					<? echo create_drop_down( "cbo_company_mst", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-Select Company-", $cbo_company_name,"load_drop_down( 'short_quotation_v4_controller', this.value, 'load_drop_down_buyer_pop', 'buyer_pop_td' );",'' ); ?>
                </td>
                ...
                <td align="center">
                	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_id').value+'_'+document.getElementById('chk_job_wo_po').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_job_prifix').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_style').value, 'create_po_search_list_view', 'search_div', 'short_quotation_v4_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" /></td>
        	</tr>
            <tr>
		...
		<fieldset>
            <table cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr>
                        <td align="center">
                        	 <? echo create_drop_down( "cbo_buyer_id", 120, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name",1, "-- All Buyer--",$selected,"load_drop_down( 'short_quotation_v4_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'short_quotation_v4_controller',this.value, 'load_drop_down_sub_dep', 'sub_td' );",0 ); ?>
                        </td>                 
                        <td id="season_td"><? echo create_drop_down( "cbo_season_id", 130, $blank_array,'', 1, "-- Select Season--",$selected, "" ); ?></td>
                        <td id="sub_td"><? echo create_drop_down( "cbo_subDept_id", 130, $blank_array,'', 1, "-- Select Dept--",$selected, "" ); ?></td>     
...
if($action=="agent_location_popup")
{
		...
		function fnc_agent_location_reponse()
		{
			if(http.readyState == 4) 
			{
				...
				if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
				{
					...
					show_list_view(type,'agent_location_data_list','agent_location_data_div','short_quotation_v4_controller','');
				}
    ...
    <script>show_list_view(type,'agent_location_data_list','agent_location_data_div','short_quotation_v4_controller','');</script>          
    <script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
    </html>
    <?
	exit();	
...
if($action=="quick_costing_print")
{
	...
	$mstSql = "SELECT id, company_id, cost_sheet_id, cost_sheet_no, temp_id, lib_item_id, inquery_id, style_ref, buyer_id, uom, cons_basis, season_id, season_year, brand_id, style_des, prod_dept, margin, department_id, delivery_date, exchange_rate, offer_qty, quoted_price, tgt_price, stage_id, costing_date, pre_cost_sheet_id, revise_no, option_id, buyer_remarks, option_remarks, gauge, no_of_ends, sample_color, sample_size, sample_range,approved from qc_mst where qc_no='$qc_no' and status_active=1 and is_deleted=0";
	//echo $mstSql; die;
	...
	$costing_per="";
	$costing_per_id=return_field_value( "costing_per", "qc_mst","qc_no='$qc_no'");
	if($costing_per_id==1){ $costing_per="DZN"; }
	if($costing_per_id==2){ $costing_per="PCS"; }
	if($costing_per_id==3){ $costing_per="DZN"; }
	if($costing_per_id==4){ $costing_per="DZN"; }
	if($costing_per_id==5){ $costing_per="DZN"; }
	
	$sqlSum="select id, mst_id, buyer_agent_id, location_id, no_of_pack, is_confirm, is_cm_calculative, mis_lumsum_cost, commision_per, tot_fab_cost, tot_sp_operation_cost, tot_wash_cost, tot_accessories_cost, tot_cm_cost, tot_fright_cost, tot_lab_test_cost, tot_miscellaneous_cost, tot_other_cost, commercial_cost, tot_commission_cost, tot_cost, tot_fob_cost, operating_exp, knitting_time, makeup_time, finishing_time from qc_tot_cost_summary where mst_id='$qc_no' and status_active=1 and is_deleted=0";
	$sqlSumData=sql_select($sqlSum);
	$fobTot=$sqlSumData[0][csf('tot_fob_cost')];

	$totCost=$sqlSumData[0][csf('tot_cost')];

	if($costing_per_id==1){ 
		$fobTotal=$sqlSumData[0][csf('tot_cost')]*12;
	}
	else{
		$fobTotal=$sqlSumData[0][csf('tot_cost')];
	}
        	...
        	<tr>
            	<td width="460" valign="top">
                	<table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:460px;" rules="all">
                    	<tr>
                        	<td align="center"><b>Merchandiser Remarks :<? if($master_data[0][csf('approved')]==1 || $master_data[0][csf('approved')]==3){ echo "<h3>Approved</h3>";}?></b></td>
                        </tr>
                        <?
						$excomments=explode("\n",$marchendiserRemarks);
						//print_r($excomments);
		...
		<table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:930px; margin-top: 10px" rules="all">
			<thead>
            	...
            	<tr>
                    ...
                    <th width="65">Yarn Count</th>
                    <th width="65">Color Type</th>
                    <th width="45">UOM</th>
                    <th width="50">Cons/<? echo $costing_per; ?> [LBS]</th>
                    <th width="45">Ex. %</th>
                    <th width="45">Ttl Cons/<? echo $costing_per; ?> [LBS]</th>
			...
			<? 
			$consSql="select id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, rate, is_calculation, rate_data, value, fab_id, description, use_for, body_part_id, uom, color_type_id from qc_cons_rate_dtls where mst_id='$qc_no' and status_active=1 and is_deleted=0 and type!=4 order by id ASC";
			$consSqlData=sql_select($consSql); $fabidArr=array(); $knittingWeightPcs=$knittingWeightLbs=$sizeGrading=$wastage=$actualKnittingWeightPcs=$actualKnittingWeightLbs=0;
			foreach ($consSqlData as $frow)
			{
				$fabidArr[$frow[csf('fab_id')]]=$frow[csf('fab_id')];
				//$('#txtknitgrampcs').val()+'~~'+$('#txtknitlbspcs').val()+'~~'+$('#txtknitlbsdzn').val()+'~~'+$('#txtsizegrading').val()+'~~'+$('#txtwastage').val()+'~~'+$('#txtactualconskgpcs').val()+'~~'+$('#txtactualconslbspcs').val()+'~~'+$('#txtactualconslbsdzn').val();
				if($frow[csf('rate_data')]!="" && $frow[csf('type')]==1)
				{
					$exconsdata=explode("~~",$frow[csf('rate_data')]);
					
					$knittingWeightPcs+=$exconsdata[0];
					$knittingWeightLbs+=$exconsdata[2];
					$sizeGrading+=$exconsdata[3];
					$wastage+=$exconsdata[4];
					$actualKnittingWeightPcs+=$exconsdata[5]*1000;
					$actualKnittingWeightLbs+=$exconsdata[7];
				}
			}
			
			$sqlYarnCount=sql_select("select mst_id, count_id from lib_yarn_count_determina_dtls where status_active=1 and is_deleted=0 ".where_con_using_array($fabidArr,1,'mst_id')."");
			$yCountIdArr=array();
			foreach ($sqlYarnCount as $yrow)
			{
				$yCountIdArr[$yrow[csf('mst_id')]].=$yrow[csf('count_id')].',';
			}
			...
			foreach ($consSqlData as $val)
			{
				...
				{
					$exyarnCount=array_filter(explode(',',$yCountIdArr[$val[csf('fab_id')]]));
					$yarncountstr="";
					
					foreach($exyarnCount as $ycid)
					{
						if($yarncountstr=="") $yarncountstr=$yarncountArr[$ycid]; else $yarncountstr.=', '.$yarncountArr[$ycid];
					}
					$fabPerFob=0;
					//$fabPerFob=($val[csf('value')]/$fobTotal)*100;
					$fabPerFob=($val[csf('value')]*100)/$totCost;
					?>
					<tr>
						<td width="60"><?=$garments_item[$val[csf('item_id')]]; ?></td>
						<td width="70"><?=$body_part[$val[csf('body_part_id')]]; ?></td>
						<td width="120" style="word-break:break-all"><?=$val[csf('description')]; ?></td>
						<td width="120" style="word-break:break-all"><?=$val[csf('description')]; ?></td>
						<td width="65" style="word-break:break-all"><?=$yarncountstr; ?></td>
			...
			//$gfabPerFob=($fabtot/$fobTotal)*100;$totCost
			$gfabPerFob=($fabtot*100)/$totCost;
			?>
            <tr bgcolor="#CCCCCC">
            	<td width="60">&nbsp;</td>
                <td width="70">&nbsp;</td>
                <td width="120">&nbsp;</td>
                <td width="120">Total Yarn Cost:</td>
        ...
        <table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:930px; margin-top: 10px" rules="all">
			<thead>
            	...
            	<tr>
                    ...
                    <th width="70">UOM</th>
                    <th width="60">Cons/<? echo $costing_per; ?></th>
                    <th width="60">Ex. %</th>
			...
			foreach ($trmsDataArr as $gmtitemid=>$gmtitemdata)
			{
				foreach ($gmtitemdata as $trimType=>$arrdata)
				{
					?>
					<tr>
						<td colspan="11" bgcolor="#CCCCCC" align="center"><b><?=$trim_type[$trimType]; ?><b/>&nbsp;</td>
					</tr>
					<? $ttypetotal=0;
					foreach ($arrdata as $rid=>$otdata)
					{
						...
						$accPerFob=0;
						//$accPerFob=($value/$fobTotal);
						$accPerFob=($value*100)/$totCost;
						?>
						<tr>
					...
					//$ttypeAccPerFob=($ttypetotal/$fobTotal)*100;
					$ttypeAccPerFob=($ttypetotal*100)/$totCost;
					?>
					<tr bgcolor="#CCCCCC">
						<td colspan="9" align="right"><b><?=$trim_type[$trimType]; ?> Total=<b/>&nbsp;</td>
			...
			//$gAccPerFob=($acctot/$fobTotal)*100;$totCost
			$gAccPerFob=($acctot*100)/$totCost;
			?>
            <tr bgcolor="#CCCCCC">
            	<td width="70">&nbsp;</td>
                <td width="120">&nbsp;</td>
        ...
        <table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:930px; margin-top: 10px" rules="all">
			<thead>
            	...
            	<tr>
                    ...
                    <th width="60">Ttl Cons.</th>
                    <th width="60">Avg. Rate</th>
                    <th width="60">Cost/<? echo $costing_per; ?></th>
                    <th>% On FOB</th>
                </tr>
			</thead>
			<? 
			foreach ($consSqlData as $val)
			{
				...
				{
					$embPerFob=0;
					//$embPerFob=($val[csf('value')]/$fobTotal)*100;$totCost
					$embPerFob=($val[csf('value')]*100)/$totCost;
					$embName=""; $emb_type="";
					if($val[csf('type')]==2)
					{
			...
			//$gEmbPerFob=($embtot/$fobTotal)*100;$totCost
			$gEmbPerFob=($embtot*100)/$totCost;
			?>
            <tr bgcolor="#CCCCCC">
            	<td width="100">&nbsp;</td>
                <td width="120">&nbsp;</td>
        	...
        	<tr>
            	<td width="410" valign="top">
                    <table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:410px;" rules="all">
                        <tr>
                            <td colspan="4" align="center"><b>Others</b></td>
                        </tr>
                        <tr bgcolor="#CCFFFF">
                            <td width="200"><b>Yarn Price</b></td>
                            <td width="120"><?=number_format($fabtot,4); ?> Cost/<? echo $costing_per; ?> </td>
                            <td>[LBS]</td>
                        </tr>
                        <tr bgcolor="#D4DFFF">
                            <td width="200" align="center"><b>Time</b></td>
                            <td width="120" align="center"><b>Min</b></td>
                            <td align="center"><b>PROD [<? echo $costing_per; ?>]/ DAY</b></td>
                        </tr>
                        <tr>
                            <td width="200">Knitting Time</td>
                            <td width="120" align="center"><?=$sqlSumData[0][csf('knitting_time')]; ?></td>
                            <td align="center"><?=number_format(((1440*.8)/$sqlSumData[0][csf('knitting_time')])); ?></td>
                        </tr>
                        <tr>
                            <td width="200">Make Up Time</td>
                            <td width="120" align="center"><?=$sqlSumData[0][csf('makeup_time')]; ?></td>
                            <td align="center"><?=number_format(((600*.8)/$sqlSumData[0][csf('makeup_time')])); ?></td>
                        </tr>
                        <tr>
                            <td width="200">Finishing Time</td>
                            <td width="120" align="center"><?=$sqlSumData[0][csf('finishing_time')]; ?></td>
                            <td align="center"><?=number_format(((600*.8)/$sqlSumData[0][csf('finishing_time')])); ?></td>
                        </tr>
                        <tr bgcolor="#CCCCCC">
                        	<td width="200" align="center"><b>WEIGHT</b></td>
                            <td width="120" align="center"><b>GRAM / PCS</b></td>
                            <td align="center"><b>LBS / DZN</b></td>
                        </tr>
                        <tr>
                            <td width="200">Knitting Weight</td>
                            <td width="120" align="center"><?=number_format($knittingWeightPcs,2); ?></td>
                            <td align="center"><?=number_format($knittingWeightLbs,4); ?></td>
                        </tr>
                        <tr>
                            <td width="200">Size Grading %</td>
                            <td align="center" colspan="2"><?=$sizeGrading; ?></td>
                        </tr>
                        <tr>
                            <td>Wastage %</td>
                            <td align="center" colspan="2"><?=$wastage; ?></td>
                        </tr>
                        <tr>
                            <td>Knitting Weight [Including Wastage]</td>
                            <td align="center"><?=number_format($actualKnittingWeightPcs,2); ?></td>
                            <td align="center"><?=number_format($actualKnittingWeightLbs,4); ?></td>
                        </tr>
                    </table>
                </td>
                <td width="20">&nbsp;</td>
                <td valign="top">
                	<table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:500px;" rules="all">
                        <tr>
                            <td colspan="4" align="center"><b> Cost Summary [$/<? echo $costing_per; ?>]</b></td>
                        </tr>
                        <tr bgcolor="#CCFFFF">
                            <td width="250"><b>COST BREAKDOWN</b></td>
                            <td width="90"><b>COSTING</b></td>
                            <td width="90"><b>1ST OFFER</b></td>
                            <td><b>FINAL PRICE</b></td>
                        </tr>
                        
                        <?
						foreach($costHeadSummArr as $hid=>$hval)
						{
							?>
							<tr>
								<td><?=$hval; ?></td>
								<td align="center"><?=$costValueSummArr[$hid]; ?></td>
								<td align="center">&nbsp;</td>
                                <td align="center"><?=$costValueSummArr[$hid]; ?></td>
							</tr>
							<?
...
{
	echo create_drop_down( "cbo_buyer_name", 130, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90))  order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down( 'short_quotation_v4_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'short_quotation_v4_controller', this.value, 'load_drop_down_brand', 'brand_td');" );
	exit();
}
...
<body>
    <div align="center" style="width:100%;" >
        <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            <table width="910" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
                ...
                <tr class="general">
                    <td><? echo create_drop_down( "cbo_company_mst", 140, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", $cbo_company_name,"load_drop_down( 'short_quotation_v4_controller', this.value, 'load_drop_down_buyer', 'buyer_td');" ); ?><input type="hidden" id="selected_id"></td>
    ...
    <script>
		load_drop_down('short_quotation_v4_controller', <? echo $cbo_company_name; ?>, 'load_drop_down_buyer', 'buyer_td' )
    </script>
    <script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
    </html>
    <?
...
<body>
   ...
   <table id="list_view" class="rpt_table" width="930" height="" cellspacing="0" cellpadding="0" border="1" rules="all">
        <tbody>
	<?
	$sql_data=sql_select("select a.fab_nature_id, a.construction, a.gsm_weight, a.color_range_id, a.stich_length, a.process_loss, a.id, a.sequence_no from  lib_yarn_count_determina_mst a, lib_yarn_count_determina_dtls b where a.id=b.mst_id and a.fab_nature_id=100  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $search_con group by a.id, a.fab_nature_id, a.construction, a.gsm_weight, a.color_range_id, a.stich_length, a.process_loss, a.sequence_no order by a.id");//and a.fab_nature_id=2
	$i=1;
	foreach($sql_data as $row)
	{
		if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
...
if($action=="trims_cost_template_name_popup")
{
    ...
    <script>
        ...
        {
            var template_data=return_global_ajax_value( data, 'get_template_data', '', 'short_quotation_v4_controller');
...
if($action == "get_template_data")
{
    ...
    foreach( $data_array as $row )
    {
       ...
       // $str=$lib_item_group_arr[$row[csf("trims_group")]].'***'.$row[csf('user_code')].'***'.$row[csf('trims_group')].'***'.$row[csf('cons_uom')].'***'.$row[csf('cons_dzn_gmts')].'***'.$row[csf('purchase_rate')].'***'.$row[csf('amount')].'***'.$row[csf('apvl_req')].'***'.$row[csf('supplyer')].'***'.$row[csf('sup_ref')].'***'.$row[csf('item_description')];
	    ...
	    $str=$lib_item_group_arr[$row[csf("trims_group")]].'***'.$row[csf('user_code')].'***'.$row[csf('trims_group')].'***'.$row[csf('cons_uom')].'***'.$row[csf('cons_dzn_gmts')].'***'.$row[csf('purchase_rate')].'***'.$row[csf('amount')].'***'.$row[csf('apvl_req')].'***'.$row[csf('supplyer')].'***'.$row[csf('sup_ref')].'***'.$row[csf('item_description')].'***'.$row[csf('id')].'***'.$row[csf('extra_rate')];
     ...
     ?>
        <tr bgcolor="<?=$bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<?=$i;?>" class="itemdata" onClick="js_set_value(<?=$i; ?>)" >
            ...
            <td style="word-break:break-all"><?=$lib_item_group_arr[$row[csf("trims_group")]]; ?></td>
            <td style="word-break:break-all"><?=$row[csf("item_description")];?></td>
            <td style="word-break:break-all"><?=$row[csf("sup_ref")]; ?></td>
            <td style="word-break:break-all"><?=$supplier_library[$row[csf("supplyer")]]; ?></td>
            <td align="center"><?=$unit_of_measurement[$row[csf("cons_uom")]]; ?></td>
            <td align="center"><?=fn_number_format($row[csf("cons_dzn_gmts")],6,'.','');?></td>
            <td align="center"><?=fn_number_format($rate,6,'.','');?></td>
            <td align="center"><?=fn_number_format($amount,6,'.','');?></td>
            <td align="center"><?=$yes_no[$row[csf("apvl_req")]]; ?></td>
        </tr>
    <?  $i++; 
	} 
    exit();   
