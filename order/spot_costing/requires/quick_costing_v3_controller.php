...
{
	echo create_drop_down( "cbo_dealing_merchant", 130, "select id,team_member_name from lib_mkt_team_member_info where team_id='$data' and status_active =1 and is_deleted=0 order by team_member_name","id,team_member_name", 1, "-Team Member-", $selected, "" );
	exit();	
}
...
if ($action=="load_drop_down_sub_dep")
{
	...
	//echo "select id, sub_department_name from lib_pro_sub_deparatment where buyer_id=$edata[0] and department_id='$edata[1]' and status_active =1 and is_deleted=0 order by sub_department_name";
	$id_cond='';
	if($edata[2]>0)
	{
		$selected=$edata[2];
		$id_cond=" and id=$edata[2]";
	}
	
	echo create_drop_down( "cbo_subDept_id", 130, "select id, sub_department_name from lib_pro_sub_deparatment where buyer_id=$edata[0] and department_id='$edata[1]' and status_active =1 and is_deleted=0 $id_cond order by sub_department_name","id,sub_department_name", 1, "-Sub. Dept.-", $selected, "" );
	exit();
}
...
if ($action=="load_drop_down_buyer_pop_bk")
{
...
if ($action=="load_drop_down_option_id")
{
	...
	/*if($ex_data[1]!=0)
		...
		$selectopt=$selected;*/
		//load_drop_down('requires/quick_costing_v3_controller', '".$row[csf("cost_sheet_no")].'__'.$row[csf("option_id")].'__'.$row[csf("revise_no")]."', 'load_drop_down_revise_no', 'revise_td');
	...
	if($ex_data[1]!=0)
		...
		$selectopt=$max;
	//echo $sql;
	echo create_drop_down( "cbo_option_id", 45, $rvs,"", 0, "-0-", $selectopt, "load_drop_down( 'requires/quick_costing_v3_controller', document.getElementById('txt_costSheetNo').value+'__'+this.value+'__'+0, 'load_drop_down_revise_no', 'revise_td'); fnc_option_rev( document.getElementById('cbo_revise_no').value+'***'+this.value+'***2' );" );//
	exit();
}
...
if($action=="template_popup")
{
	...
	function fnc_template_entry( operation )
	{
		...
		http.open("POST","quick_costing_v3_controller.php",true);
...
*/	}
	...
	function fnc_template_entry_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			if(reponse[0]==0 || reponse[0]==1)
			{
				...
				show_list_view( '','template_list_view','save_up_list_view','quick_costing_v3_controller','setFilterGrid(\'tbl_upListView\',-1)');
			}
		}
	}
	...
	{
		var list_view_grid = return_global_ajax_value( temp_id, 'load_php_dtls_form', '', 'quick_costing_v3_controller');
		if(list_view_grid!='')
		{
                ...
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="352" class="rpt_table" id="tbl_tempCreat" >
                	<tbody id="tbd_temp">
                        <tr id="tr_1">
                            ...
                            <? echo create_drop_down( "cboItemBts_1", 70, $garments_item,"", 0, "-Select-", 1, "", "", "", "", "", "", "", "", "cboItemBts[]" ); ?></td>
                            <td width="53"><input style="width:40px;" type="text" class="text_boxes_numeric" name="txtBtsRatio[]" id="txtBtsRatio_1" /></td>
                            <td width="70"><? echo create_drop_down( "cboItemBtm_1", 70, $garments_item,"", 0, "-Select-", 2, "", "", "", "", "", "", "", "", "cboItemBtm[]" ); ?></td>
...
if($action=='save_update_delete_tamplete')
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		for ($i=1;$i<=$tot_row;$i++)
		{
			...
			{
				if($bts_ratio=='') $bts_ratio=$garments_item[str_replace("'","",$$itemBts)].$m; else $bts_ratio.=','.$garments_item[str_replace("'","",$$itemBts)].$m;
				$m++;
			}
			
			for($k=1; $k<=str_replace("'","",$$btmRatio); $k++)
			{
				if($btm_ratio=='') $btm_ratio=$garments_item[str_replace("'","",$$itemBtm)].$n; else $btm_ratio.=','.$garments_item[str_replace("'","",$$itemBtm)].$n;
				$n++;
			}
			$item_data='';
			 ...
			 //$lib_temp_arr=return_library_array("select id, item_name from lib_qc_template","id","item_name");
				...
				foreach($mst_temp_arr as $temp_id=>$tmp_data)
				{
					...
					foreach($template_data as $temp_val)
					{
						...
						for($j=1; $j<=$ratio1; $j++)
						{
							if($bts_ratio=='') $bts_ratio=$garments_item[$item_id1].$m; else $bts_ratio.=','.$garments_item[$item_id1].$m;
							$m++;
						}
						$btm_ratio='';
						for($k=1; $k<=$ratio2; $k++)
						{
							if($btm_ratio=='') $btm_ratio=$garments_item[$item_id2].$n; else $btm_ratio.=','.$garments_item[$item_id2].$n;
							$n++;
						}
						if($template_name=='')
						{
...
if($action=="load_drop_down_tempItem")
{
	...
	echo create_drop_down( "cboItemId", 90, $sql_tmp,"id,item_name", 0, "-Select-", $selected, "fnc_change_data();" );
	exit();
}
	...
	foreach($sql_tmp_res as $row)
	{
        ...
        <tr id="tr_<? echo $i; ?>">
            ...
            <input style="width:40px;" type="hidden" id="txttmpid_<? echo $i; ?>" value="<? echo $row[csf('temp_id')]; ?>" />
                <? echo create_drop_down( "cboItemBts_$i", 70, $garments_item,"", 0, "-Select-", $row[csf('item_id1')], "", "", "", "", "", "", "", "", "cboItemBts[]" ); ?></td>
            ...
            <td width="70"><? echo create_drop_down( "cboItemBtm_$i", 70, $garments_item,"", 0, "-Select-", $row[csf('item_id2')], "", "", "", "", "", "", "", "", "cboItemBtm[]" ); ?></td>
...
if($action=="rate_details_popup")
{
    ...
    <script>
		...
		function fnc_cons_amt(str)
		{
			...
			if(valstr[1]==1)
			{
				var fabCons=($('#txtfabcons_'+id).val()*1);
				var fabex=($('#txtfabex_'+id).val()*1);
				var fabRate=($('#txtfabrate_'+id).val()*1);
				var totfabCons=fabCons;
				if(fabex>0)
				{
					var totfabCons=fabCons+(fabCons*(fabex/100));
				}
				
				var fabAmt=totfabCons*fabRate;
				$('#txtfabtotcons_'+id).val( number_format(totfabCons,4,'.','') );
				
				$('#txtfabamt_'+id).val( number_format(fabAmt,4,'.','') );
			}
			else if(valstr[1]==2)
			{
				var yarnCons=($('#txtyarncons_'+id).val()*1);
				var yarnRate=($('#txtyarnrate_'+id).val()*1);
				
				var yarnAmt=yarnCons*yarnRate;
				
				$('#txtyarnamt_'+id).val( number_format(yarnAmt,4,'.','') );
			}
			else if(valstr[1]==3)
			{
		...
		function fnc_total_rate(type)
		{
			if(type==1)
			{
				...
				var z=1; var consTot=0; var totconsTot=0; var amtTot=0;
				for(var i=1; i<=row; i++)
				{
					consTot+=($('#txtfabcons_'+i).val()*1);
					totconsTot+=($('#txtfabtotcons_'+i).val()*1);
					amtTot+=($('#txtfabamt_'+i).val()*1);
				}
				$('#txt_tot_cons').val( number_format(consTot,4,'.','') );
				$('#txt_tot_tot_cost').val( number_format(totconsTot,4,'.','') );
				$('#txt_tot_amt').val( number_format(amtTot,4,'.','') );
			}
			else if(type==2)
			{
		...
		{
			fnc_total_rate(0);
			document.getElementById('txt_tot_cons').value;
			document.getElementById('txt_tot_tot_cost').value;
        ...
        {
            //alert(row)
            emailwindow=dhtmlmodal.open('EmailBox', 'iframe','quick_costing_v3_controller.php?action='+action,'Details Popup', 'width=680px,height=300px,center=1,resize=0','../../');
            emailwindow.onclose=function()
            {
                var popupData=this.contentDoc.getElementById("popupData").value;
                var popupDataOther=popupData.split('_');
               // alert(popupData);
                if(action=='fabric_popup'){
					var popupDataFab=popupData.split('##');
                    $('#hidLibid_'+row).val(popupDataFab[0]);
					var exdata=popupDataFab[1]+', '+popupDataFab[2]+', '+popupDataFab[3];
                    $('#txtFabricDtls_'+row).val(exdata);
					$('#txtYarnDtls_'+row).val(popupDataFab[4]);
					$('#txtConvDtls_'+row).val(popupDataFab[5]);
                }
				if(action=='knit_popup')
				{
					$('#hidlibknitid_'+row).val(popupDataOther[0]);
					var exdata=popupDataOther[1]+', '+popupDataOther[2]+', '+popupDataOther[3];
                    $('#txtknitdtls'+row).val(exdata);
				}
				if(action=='dye_popup')
				{
					$('#hidlibdyeid_'+row).val(popupDataOther[0]);
					var exdata=popupDataOther[1]+', '+popupDataOther[2]+', '+popupDataOther[3];
                    $('#txtdyedtls'+row).val(exdata);
				}
				if(action=='aop_popup')
				{
					$('#hidlibaopid_'+row).val(popupDataOther[0]);
					var exdata=popupDataOther[1]+', '+popupDataOther[2]+', '+popupDataOther[3];
		...
		function append_row(str)
		{
			...
			if(strval[1]==1)
			{
				var counter =$('#tbl_fab tbody tr').length; 
				//alert(counter);
				//if(counter>=1) counter++; else if (counter<1) counter=1;
				//alert(counter);
				var z=1;
				//alert(($("#txtfabcons_"+inc).val()*1))
				for(var i=1; i<=counter; i++)
				{
					if( ($("#txtfabcons_"+i).val()*1)==0)
					{
						z++;
					}
				}
				...
				if(z==1)
				{
					...
					$('#tbl_fab tbody').append(
						'<tr id="fabtr_'+counter+'">'
						+ '<td><input style="width:188px;" type="text" class="text_boxes" name="txtFabricDtls_'+counter+'" id="txtFabricDtls_'+counter+'" placeholder="Br. Fabric" onClick="" readonly /><input type="hidden" class="text_boxes" name="hidLibid_'+counter+'" id="hidLibid_'+counter+'" value=""/><input type="hidden" class="text_boxes" name="txtYarnDtls_'+counter+'" id="txtYarnDtls_'+counter+'" value=""/><input type="hidden" class="text_boxes" name="txtConvDtls_'+counter+'" id="txtConvDtls_'+counter+'" value=""/><input type="hidden" class="text_boxes" name="hidupid_'+counter+'" id="hidupid_'+counter+'" value=""/></td><td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtgsm_'+counter+'" id="txtgsm_'+counter+'" placeholder="Write" value="" /></td><td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtgauge_'+counter+'" id="txtgauge_'+counter+'" placeholder="Write" value="" /></td><td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtdia_'+counter+'" id="txtdia_'+counter+'" placeholder="Write" value="" /></td><td><?=create_drop_down("cbowidthdiatype_'+counter+'", 60, $fabric_typee,"", 1, "-Select-", $selected, "","","" ); ?></td><td><?=create_drop_down("cbocolortype_'+counter+'", 50, $color_type,'', 1, '-Select-','', "",'',"" ); ?></td><td><?=create_drop_down("cbonominasupplier_'+counter+'", 50, $supplier_library_fabric,'', 1, '-Select-','', "",'',"" ); ?></td><td><?=create_drop_down("cbouom_'+counter+'", 50, $unit_of_measurement,'', 1, '-Select-','12', "",'',"1,12,23,27" ); ?></td><td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtfabcons_'+counter+'" id="txtfabcons_'+counter+'" placeholder="Write" onChange="fnc_cons_amt('+"'"+counter+"_1'"+');" onBlur="append_row('+"'"+counter+"_1'"+');"/></td><td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtfabex_'+counter+'" id="txtfabex_'+counter+'" placeholder="Write" onChange="fnc_cons_amt('+"'"+counter+"_1'"+');" /></td><td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtfabtotcons_'+counter+'" id="txtfabtotcons_'+counter+'" placeholder="Display" readonly /></td><td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtfabrate_'+counter+'" id="txtfabrate_'+counter+'" onChange="fnc_cons_amt('+"'"+counter+"_1'"+');" /></td><td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtfabamt_'+counter+'" id="txtfabamt_'+counter+'" placeholder="Display" /></td>'+ '</tr>'
					);
			...
			{
				var counter =$('#tbl_yarn tbody tr').length; 
				//alert(counter);
				//if(counter>=1) counter++; else if (counter<1) counter=1;
				//alert(counter);
				var y=1;
				...
				if(y==1)
				{
					$('#tbl_yarn tbody').append(
						'<tr id="yarntr_'+counter+'">'
						+ '<td><?=create_drop_down( "cbocount_'+counter+'", 60, $yarnCountArr, "",1,"-Count-", $selected, "","","" ); ?><input type="hidden" class="text_boxes" name="hidfabid_'+counter+'" id="hidfabid_'+counter+'" value=""/><input type="hidden" class="text_boxes" name="hidyarnupid_'+counter+'" id="hidyarnupid_'+counter+'" value=""/></td><td><?=create_drop_down( "cbocompone_'+counter+'", 100, $composition,"", 1, "-Select-", $selected, "","","","","" ); ?></td><td><input type="text" id="txtpercentone_'+counter+'"  name="txtpercentone_'+counter+'" class="text_boxes" style="width:38px" value="" readonly/></td><td><? echo create_drop_down( "cbotype_'+counter+'", 110, $yarn_type,"", 1, "-Select-", $selected, "","","","","","" ); ?></td><td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtyarncons_'+counter+'" id="txtyarncons_'+counter+'" placeholder="Write" onChange="fnc_cons_amt('+"'"+counter+"_2'"+');" onBlur="append_row('+"'"+counter+"_2'"+');" value="" /></td><td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtyarnrate_'+counter+'" id="txtyarnrate_'+counter+'" onChange="fnc_cons_amt('+"'"+counter+"_2'"+');" value="" /></td><td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtyarnamt_'+counter+'" id="txtyarnamt_'+counter+'" placeholder="Display" readonly value="" /></td>'+ '</tr>'
					);
				}
			}
			else if(strval[1]==3)
		...
		function fnc_remove_row(type)
		{
			if(type==1)
			{
				...
				for(var i=1; i<=counter-1; i++)
				{
					...
					{
						var index=i-1;
						$("table#tbl_fab tbody tr:eq("+index+")").remove();
					}
				}
				var numRow = $('table#tbl_fab tbody tr').length;
				for(var i=1; i<=counter; i++)
				{
					var index=i-1;
					$("#tbl_fab tbody tr:eq("+index+")").find("input,select").each(function() {
						$(this).attr({
							'id': function(_, id) { var id=id.split("_"); return id[0] +"_"+ i },
						});
					});
					
					$("#tbl_fab tbody tr:eq("+index+")").each(function(){
						$('#txtFabricDtls_'+i).removeAttr("onClick").attr("onClick","fnc_details_popup('"+i+"','fabric_popup','"+i+"')");
						$('#txtfabcons_'+i).removeAttr("onChange").attr("onChange","fnc_cons_amt('"+i+"_1')");
						$('#txtfabcons_'+i).removeAttr("onBlur").attr("onBlur","append_row('"+i+"_1')");
						$('#txtfabex_'+i).removeAttr("onChange").attr("onChange","fnc_cons_amt('"+i+"_1')");
						$('#txtfabrate_'+i).removeAttr("onChange").attr("onChange","fnc_cons_amt('"+i+"_1')");
					});
				}
			}
			else if(type==2)
			{
				...
				for(var i=1; i<=counter-1; i++)
				{
					...
					{
						var index=i-1;
						$("table#tbl_yarn tbody tr:eq("+index+")").remove();
					}
				}
				var numRow = $('table#tbl_yarn tbody tr').length;
				for(var i=1; i<=counter; i++)
				{
					var index=i-1;
					$("#tbl_yarn tbody tr:eq("+index+")").find("input,select").each(function() {
						$(this).attr({
							'id': function(_, id) { var id=id.split("_"); return id[0] +"_"+ i },
						});
					});
					
					$("#tbl_yarn tbody tr:eq("+index+")").each(function(){
						$('#txtyarncons_'+i).removeAttr("onChange").attr("onChange","fnc_cons_amt('"+i+"_2')");
						$('#txtyarncons_'+i).removeAttr("onBlur").attr("onBlur","append_row('"+i+"_2')");
						$('#txtyarnrate_'+i).removeAttr("onChange").attr("onChange","fnc_cons_amt('"+i+"_2')");
					});
				}
			}
			else if(type==3)
        ...
        function fnc_fabric_entry(operation)
        {
			var counter =$('#tbl_fab tbody tr').length; 
			var z=1; var dataAll=""; var r=0;
			for(var i=1; i<=counter; i++)
			{
				if( ($("#txtfabcons_"+i).val()*1)!=0)
				{
					 dataAll+="&txtFabricDtls_" + z + "='" + $('#txtFabricDtls_'+i).val()+"'"+"&hidLibid_" + z + "='" + $('#hidLibid_'+i).val()+"'"+"&txtYarnDtls_" + z + "='" + $('#txtYarnDtls_'+i).val()+"'"+"&txtConvDtls_" + z + "='" + $('#txtConvDtls_'+i).val()+"'"+"&txtfabcons_" + z + "='" + $('#txtfabcons_'+i).val()+"'"+"&txtfabex_" + z + "='" + $('#txtfabex_'+i).val()+"'"+"&txtfabtotcons_" + z + "='" + $('#txtfabtotcons_'+i).val()+"'"+"&txtfabrate_" + z + "='" + $('#txtfabrate_'+i).val()+"'"+"&txtfabamt_" + z + "='" + $('#txtfabamt_'+i).val()+"'"+"&txtgsm_" + z + "='" + $('#txtgsm_'+i).val()+"'"+"&txtdia_" + z + "='" + $('#txtdia_'+i).val()+"'"+"&cbowidthdiatype_" + z + "='" + $('#cbowidthdiatype_'+i).val()+"'"+"&cbouom_" + z + "='" + $('#cbouom_'+i).val()+"'"+"&hidupid_" + z + "='" + $('#hidupid_'+i).val()+"'"+"&txtgauge_" + z + "='" + $('#txtgauge_'+i).val()+"'"+"&cbonominasupplier_" + z + "='" + $('#cbonominasupplier_'+i).val()+"'"+"&cbocolortype_" + z + "='" + $('#cbocolortype_'+i).val()+"'";
					z++; r++;
				}
			}
			if(z==1)
            ...
            var data="action=save_update_delete_fabric&operation="+operation+'&numRowFabric='+r+get_submitted_data_string('hid_qc_no*hid_bodypart_id*hid_body_cons*txtamount*txtprocessloss*txttotamount*hid_cons_rate_dtls_id',"../../../")+dataAll;
			...
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
			http.onreadystatechange = fnc_fabric_entry_reponse;
        }

        function fnc_fabric_entry_reponse()
        {
            if(http.readyState == 4)
            {
                var reponse=trim(http.responseText).split('**');
		...
		function fnc_yarn_entry(operation)
        {
			...
			for(var i=1; i<=counter; i++)
			{
				if( ($("#txtyarncons_"+i).val()*1)!=0)
				{
					 dataAll+="&cbocount_" + z + "='" + $('#cbocount_'+i).val()+"'"+"&hidfabid_" + z + "='" + $('#hidfabid_'+i).val()+"'"+"&cbocompone_" + z + "='" + $('#cbocompone_'+i).val()+"'"+"&txtpercentone_" + z + "='" + $('#txtpercentone_'+i).val()+"'"+"&cbotype_" + z + "='" + $('#cbotype_'+i).val()+"'"+"&txtyarncons_" + z + "='" + $('#txtyarncons_'+i).val()+"'"+"&txtyarnrate_" + z + "='" + $('#txtyarnrate_'+i).val()+"'"+"&txtyarnamt_" + z + "='" + $('#txtyarnamt_'+i).val()+"'"+"&hidyarnupid_" + z + "='" + $('#hidyarnupid_'+i).val()+"'"+"&cbo_yarn_supplier_" + z + "='" + $('#cbo_yarn_supplier_'+i).val()+"'";
					z++;
				}
			}
            ...
            var data="action=save_update_delete_yarn&operation="+operation+'&numRowYarn='+z+get_submitted_data_string('hid_qc_no*hid_bodypart_id*hid_body_cons*txtamount*txtprocessloss*txttotamount*hid_cons_rate_dtls_id',"../../../")+dataAll;
		...
		function fnc_conv_entry(operation)
        {
            ...
            var data="action=save_update_delete_conv&operation="+operation+'&numRowConv='+z+get_submitted_data_string('hid_qc_no*hid_bodypart_id*hid_body_cons*txtamount*txtprocessloss*txttotamount*hid_cons_rate_dtls_id',"../../../")+dataAll;
		...
		{
			var data=qc_no+'__'+consRateId+'__'+bodypartid;
			
			show_list_view(data,'populate_confirm_fabric_data','fabricDiv','quick_costing_v3_controller','');
		}
		
		function fnc_fabconsload(inc)
		{
			var fabid=$('#cbofab_'+inc).val();
			var fabcons=return_ajax_request_value(fabid, 'generate_fabcons', 'quick_costing_v3_controller');
        ...
        <fieldset style="width:630px;margin-left:10px">
			...
			if (count($data_array)>0)
			{
				foreach( $data_array as $row )
				{
					...
					if($row[csf('percent')]=="") $row[csf('percent')]=0;
					if($row[csf('type_id')]=="") $row[csf('type_id')]=0;
					if($row[csf('yarn_rate')]=="") $row[csf('yarn_rate')]=0;
					...
					{
						$yarn_descriptionArr[$row[csf('id')]].="__".$row[csf('count_id')]."_".$row[csf('copmposition_id')]."_100_".$row[csf('type_id')]."_".$row[csf('percent')];
					}
					else 
					{
						$yarn_descriptionArr[$row[csf('id')]]=$row[csf('count_id')]."_".$row[csf('copmposition_id')]."_100_".$row[csf('type_id')]."_".$row[csf('percent')];
					}
				}
				unset($data_array);
			}
            ...
            <div style="width:630px; max-height:230px;overflow-y:scroll;" >  
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="610" class="rpt_table" id="list_view">
                    <tbody>
                        <? 
                        $i=1;
						$sql_data=sql_select("select a.fab_nature_id, a.construction, a.gsm_weight, a.color_range_id, a.stich_length, a.process_loss, a.id, a.sequence_no, a.fabric_composition_id from lib_yarn_count_determina_mst a, lib_yarn_count_determina_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id, a.fab_nature_id, a.construction, a.gsm_weight, a.color_range_id, a.stich_length, a.process_loss, a.sequence_no, a.fabric_composition_id order by a.id ASC");
						$i=1;
						foreach($sql_data as $row)
						{
							...
							//else $yarnBreak="";
							?>
							<tr id="tr_<?=$row[csf('id')]; ?>" bgcolor="<?=$bgcolor; ?>" style="cursor:pointer; word-break:break-all;" onClick="js_set_value('<?=$row[csf('id')]."##".$row[csf('construction')]."##".$composition_arr[$row[csf('id')]]."##".$row[csf('gsm_weight')]."##".$yarnBreak."##".$convDataArr[$row[csf('id')]]; ?>');">
                                ...
                                <td width="120" style="word-break:break-all"><?=$lib_fabric_composition[$row[csf('fabric_composition_id')]]; ?></td>
...
if($action=="save_update_delete_fabric")
{
    ...
    if ($operation==0)  // Insert Here
    {
		...
		$field_array="id, qc_no, body_part_id, color_type_id, nominated_supp_multi, gauge, type, yarn_lib_id, count, composition, percent, yarn_type, fab_id, process, fab_des, gsm, dia, diatype, uom, cons, exper, tot_cons, rate, value, yarn_break_down, conv_break_down, cons_rate_dtls_id, inserted_by, insert_date, status_active, is_deleted";
		//echo "10**";
        ...
        for($i=1; $i<=$numRowFabric; $i++) 
        {
            ...
            $txtgauge        ="txtgauge_".$i;
            $cbocolortype        ="cbocolortype_".$i;
            $cbonominasupplier        ="cbonominasupplier_".$i;

			//echo $$txtYarnDtls.'*****'.$$txtConvDtls.'<br>';
			$txtgauge=str_replace("'", "", $$txtgauge);
			$cbocolortype=str_replace("'", "", $$cbocolortype);
			$cbonominasupplier=str_replace("'", "", $$cbonominasupplier);

			if((str_replace("'",'',$$txtfabtotcons)*1)!=0)
			{
				if ($add_comma!=0) $data_array .=","; $add_comma=0;
				$data_array .="(".$id.",".$hid_qc_no.",".$hid_bodypart_id.",'".$cbocolortype."','".$cbonominasupplier."','".$txtgauge."',1,".$$hidLibid.",0,0,0,0,0,0,".$$txtFabricDtls.",".$$txtgsm.",".$$txtdia.",".$$cbowidthdiatype.",".$$cbouom.",".$$txtfabcons.",".$$txtfabex.",".$$txtfabtotcons.",".$$txtfabrate.",".$$txtfabamt.",".$$txtYarnDtls.",".$$txtConvDtls.",".$hid_cons_rate_dtls_id.",'".$user_id."','".$pc_date_time."',1,0)";
				$fabid=$id;
				$id++; $add_comma++;
				
				if(str_replace("'",'',$$txtYarnDtls)!="")//Yarn Data
				{
					$yarnbreckdown_array=explode('__',str_replace("'",'',$$txtYarnDtls));
					for($y=0; $y< count($yarnbreckdown_array); $y++)
					{
						$counter++;
						$yarnbreckdownarr=explode('_',$yarnbreckdown_array[$y]);
						...
						if ($add_comma!=0) $data_array .=","; $add_comma=0;
						$data_array .="(".$id.",".$hid_qc_no.",".$hid_bodypart_id.",'".$cbocolortype."','".$cbonominasupplier."','".$txtgauge."',2,0,'".$yarnbreckdownarr[0]."','".$yarnbreckdownarr[1]."','".$yarnbreckdownarr[4]."','".$yarnbreckdownarr[3]."','".$fabid."',0,'',0,0,0,0,'".$cons."',0,0,'".$yarnbreckdownarr[5]."','".$yarnamt."','','',".$hid_cons_rate_dtls_id.",'".$user_id."','".$pc_date_time."',1,0)";
						$id++; $add_comma++;
						$amt+=$yarnamt; 
						$totAmt+=$yarnamt;
					}
				...
				if(str_replace("'",'',$$txtConvDtls)!="")//Conv Data
				{
					...
					for($c=0; $c< count($convbreakdownArr); $c++)
					{
						...
						if ($add_comma!=0) $data_array .=","; $add_comma=0;
						$data_array .="(".$id.",".$hid_qc_no.",".$hid_bodypart_id.",'".$cbocolortype."','".$cbonominasupplier."','".$txtgauge."',3,0,0,0,0,0,'".$fabid."','".$convbreakarr[0]."','',0,0,0,0,'".$convCons."',0,0,'".$convbreakarr[1]."','".$convamt."','','',".$hid_cons_rate_dtls_id.",'".$user_id."','".$pc_date_time."',1,0)";
    ...
    else if ($operation==1)   // Update Here
    {
        ...
        if($db_type==0) mysql_query("BEGIN");
		//echo "10**";
		$yarnConvData="select cons_rate_dtls_id, type, fab_id, count, composition, yarn_type, rate, nominated_supp_multi, process from qc_fab_yarn_conv where qc_no =".$hid_qc_no." and cons_rate_dtls_id =".$hid_cons_rate_dtls_id." and type in (2,3) and status_active=1 and is_deleted=0";
		$yarnupdataArr=array(); $convupdataArr=array();
		$yarnConvDataArr=sql_select($yarnConvData);
		foreach($yarnConvDataArr as $row)
		{
			if($row[csf("type")]==2)
			{
				$yarnupdataArr[$row[csf("cons_rate_dtls_id")]][$row[csf("fab_id")]][$row[csf("count")]][$row[csf("composition")]][$row[csf("yarn_type")]]['rate']=$row[csf("rate")];
				$yarnupdataArr[$row[csf("cons_rate_dtls_id")]][$row[csf("fab_id")]][$row[csf("count")]][$row[csf("composition")]][$row[csf("yarn_type")]]['supp']=$row[csf("nominated_supp_multi")];
			}
			else
			{
				$convupdataArr[$row[csf("cons_rate_dtls_id")]][$row[csf("fab_id")]][$row[csf("process")]]['rate']=$row[csf("rate")];
			}
		}
		unset($yarnConvDataArr);
		
		/*print_r($yarnupdataArr);
		die;*/
			...
			if((str_replace("'",'',$$txtfabtotcons)*1)!=0)
			{
				...
				if(str_replace("'",'',$$txtYarnDtls)!="")//Yarn Data
				{
					...
					for($y=0; $y< count($yarnbreckdown_array); $y++)
					{
						$counter++;
						$yarnbreckdownarr=explode('_',$yarnbreckdown_array[$y]);
						$ysupp=$yarnupdataArr[str_replace("'",'',$hid_cons_rate_dtls_id)][$fabid][$yarnbreckdownarr[0]][$yarnbreckdownarr[1]][$yarnbreckdownarr[3]]['supp'];
						
						if($ysupp!=0) $cbonominasupplier=$ysupp;
						
						$yrate=$yarnupdataArr[str_replace("'",'',$hid_cons_rate_dtls_id)][$fabid][$yarnbreckdownarr[0]][$yarnbreckdownarr[1]][$yarnbreckdownarr[3]]['rate'];
						if($yrate=="") $yrate=0;
						$yarnbreckdownarr[5]=$yrate;
						
						//echo $yrate.'--'.str_replace("'",'',$hid_cons_rate_dtls_id).'--'.$fabid.'--'.$yarnbreckdownarr[0].'--'.$yarnbreckdownarr[1].'--'.$yarnbreckdownarr[3].'<br>';
				...
				if(str_replace("'",'',$$txtConvDtls)!="")//Conv Data
				{
					...
					for($c=0; $c< count($convbreakdownArr); $c++)
					{
						...
						$crate=$convupdataArr[str_replace("'",'',$hid_cons_rate_dtls_id)][$fabid][$convbreakarr[0]]['rate'];
						if($crate=="") $crate=0;
						$convbreakarr[1]=$crate;
...
if($action=="populate_confirm_fabric_data")
{
	...
	$sqlConsRate=sql_select("select fab_cost, process_loss, totfab_cost from qc_cons_rate_dtls where mst_id=".$qc_no." and id =".$consRateId." and type=1");
	//echo "select fab_cost, process_loss, totfab_cost from qc_cons_rate_dtls where mst_id=".$qc_no." and body_part_id =".$bodypartid." and type=1";

	$supplier_library_fabric=return_library_array( "select a.id, a.supplier_name from lib_supplier a where a.is_deleted=0  and a.status_active=1 group by a.id,a.supplier_name order by a.supplier_name", "id", "supplier_name");

	$fab_cost=$sqlConsRate[0][csf('fab_cost')];
	$process_loss=$sqlConsRate[0][csf('process_loss')];
	$totfab_cost=$sqlConsRate[0][csf('totfab_cost')];
	?>
    <table width="700" cellspacing="0" border="1" class="rpt_table" id="tbl_fab" rules="all">
        <thead>
            <td colspan="13" align="center" bgcolor="#99FFCC"><b>Fabric Charge Details</b></td>
        </thead>
        <thead>
            <th width="200">Fabric Details</th>
            <th width="50">GSM</th>
            <th width="50">GAUGE</th>
            <th width="50">Dia</th>
        ...
        <tbody id="fabtbody">
			<?
            $sqlFab="select id, type, yarn_lib_id, fab_des, count, composition, percent, yarn_type, process, fab_id, gsm, dia, diatype, uom, cons, exper, tot_cons, rate, value, yarn_break_down, conv_break_down,gauge,color_type_id,nominated_supp_multi from qc_fab_yarn_conv where qc_no='$qc_no' and cons_rate_dtls_id='$consRateId' and status_active=1 and is_deleted=0";
            
            $sqlFabData=sql_select($sqlFab); $i=1; $fabBtn=0; $fab_description=array();
            foreach($sqlFabData as $row)
            {
                if($row[csf("type")]==1)
                {
                    ...
                    ?>
                    <tr id="fabtr_<?=$i; ?>">
                        <td><input style="width:188px;" type="text" class="text_boxes" name="txtFabricDtls_<?=$i; ?>" id="txtFabricDtls_<?=$i; ?>" placeholder="Br. Fabric" onClick="fnc_details_popup('<?=$i; ?>','fabric_popup','<?=$i; ?>');" value="<?=$row[csf("fab_des")]; ?>" readonly />
                            <input type="hidden" class="text_boxes" name="hidLibid_<?=$i; ?>" id="hidLibid_<?=$i; ?>" value="<?=$row[csf("yarn_lib_id")]; ?>"/>
                            <input type="hidden" class="text_boxes" name="txtYarnDtls_<?=$i; ?>" id="txtYarnDtls_<?=$i; ?>" value="<?=$row[csf("yarn_break_down")]; ?>"/>
                            <input type="hidden" class="text_boxes" name="txtConvDtls_<?=$i; ?>" id="txtConvDtls_<?=$i; ?>" value="<?=$row[csf("conv_break_down")]; ?>"/>
                            <input type="hidden" class="text_boxes" name="hidupid_<?=$i; ?>" id="hidupid_<?=$i; ?>" value="<?=$row[csf("id")]; ?>"/>
                        </td>
                        <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtgsm_<?=$i; ?>" id="txtgsm_<?=$i; ?>" placeholder="Write" value="<?=$row[csf("gsm")]; ?>" /></td>
                        <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtgauge_<?=$i; ?>" id="txtgauge_<?=$i; ?>" placeholder="Write" value="<?=$row[csf("gauge")]; ?>" /></td>
                        <td><input style="width:38px;" type="text" class="text_boxes" name="txtdia_<?=$i; ?>" id="txtdia_<?=$i; ?>" placeholder="Write" value="<?=$row[csf("dia")]; ?>" /></td>
                        <td><?=create_drop_down("cbowidthdiatype_$i", 60, $fabric_typee,"", 1, "-Select-", $row[csf("diatype")], "","","" ); ?></td>
                       
                        <td><?=create_drop_down("cbocolortype_$i", 50, $color_type,'', 1, '-Select-',$row[csf("color_type_id")], "",'',"" ); ?></td>
                        <td><?=create_drop_down("cbonominasupplier_$i", 50, $supplier_library_fabric,'', 1, '-Select-',$row[csf("nominated_supp_multi")], "",'',"" ); ?></td>
                         <td><?=create_drop_down("cbouom_$i", 50, $unit_of_measurement,'', 1, '-Select-',$row[csf("uom")], "",'',"1,12,23,27" ); ?></td>
                       
                        <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtfabcons_<?=$i; ?>" id="txtfabcons_<?=$i; ?>" placeholder="Write" onChange="fnc_cons_amt('<?=$i.'_1'; ?>');" onBlur="append_row('<?=$i.'_1'; ?>');" value="<?=$row[csf("cons")]; ?>" /></td>
                        <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtfabex_<?=$i; ?>" id="txtfabex_<?=$i; ?>" placeholder="Write" onChange="fnc_cons_amt('<?=$i.'_1'; ?>');" value="<?=$row[csf("exper")]; ?>" /></td>
                        <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtfabtotcons_<?=$i; ?>" id="txtfabtotcons_<?=$i; ?>" placeholder="Display" readonly value="<?=$row[csf("tot_cons")]; ?>"/></td>
                        <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtfabrate_<?=$i; ?>" id="txtfabrate_<?=$i; ?>" onChange="fnc_cons_amt('<?=$i.'_1'; ?>');" value="<?=$row[csf("rate")]; ?>" /></td>
                        <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtfabamt_<?=$i; ?>" id="txtfabamt_<?=$i; ?>" placeholder="Display" readonly value="<?=$row[csf("value")]; ?>" /></td>
                    </tr>
                    <?
					$fabBtn=1;
                    $i++;
					$consTot+=$row[csf("cons")];
					$consTotTot+=$row[csf("tot_cons")];
					$amtTot+=$row[csf("value")];
                }
            }
            ?>
            <tr id="fabtr_<?=$i; ?>">
                <td><input style="width:188px;" type="text" class="text_boxes" name="txtFabricDtls_<?=$i; ?>" id="txtFabricDtls_<?=$i; ?>" placeholder="Br. Fabric" onClick="fnc_details_popup('<?=$i; ?>','fabric_popup','<?=$i; ?>');" value="" readonly />
                    <input type="hidden" class="text_boxes" name="hidLibid_<?=$i; ?>" id="hidLibid_<?=$i; ?>" value=""/>
                    <input type="hidden" class="text_boxes" name="txtYarnDtls_<?=$i; ?>" id="txtYarnDtls_<?=$i; ?>" value=""/>
                    <input type="hidden" class="text_boxes" name="txtConvDtls_<?=$i; ?>" id="txtConvDtls_<?=$i; ?>" value=""/>
                    <input type="hidden" class="text_boxes" name="hidupid_<?=$i; ?>" id="hidupid_<?=$i; ?>" value=""/>
                </td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtgsm_<?=$i; ?>" id="txtgsm_<?=$i; ?>" placeholder="Write" value="" /></td>
                 <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtgauge_<?=$i; ?>" id="txtgauge_<?=$i; ?>" placeholder="Write"  /></td>
                <td><input style="width:38px;" type="text" class="text_boxes" name="txtdia_<?=$i; ?>" id="txtdia_<?=$i; ?>" placeholder="Write" value="" /></td>
                <td><?=create_drop_down("cbowidthdiatype_$i", 60, $fabric_typee,"", 1, "-Select-", $selected, "","","" ); ?></td>
                <td><?=create_drop_down("cbocolortype_$i", 50, $color_type,'', 1, '-Select-',"", "",'',"" ); ?></td>
                <td><?=create_drop_down("cbonominasupplier_$i", 50, $supplier_library_fabric,'', 1, '-Select-',"", "",'',"" ); ?></td>
                <td><?=create_drop_down("cbouom_$i", 50, $unit_of_measurement,'', 1, '-Select-','12', "",'',"1,12,23,27" ); ?></td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtfabcons_<?=$i; ?>" id="txtfabcons_<?=$i; ?>" placeholder="Write" onChange="fnc_cons_amt('<?=$i.'_1'; ?>');" onBlur="append_row('<?=$i.'_1'; ?>');" value="" /></td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtfabex_<?=$i; ?>" id="txtfabex_<?=$i; ?>" placeholder="Write" onChange="fnc_cons_amt('<?=$i.'_1'; ?>');" value="" /></td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtfabtotcons_<?=$i; ?>" id="txtfabtotcons_<?=$i; ?>" placeholder="Display" readonly value=""/></td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtfabrate_<?=$i; ?>" id="txtfabrate_<?=$i; ?>" onChange="fnc_cons_amt('<?=$i.'_1'; ?>');" value="" /></td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtfabamt_<?=$i; ?>" id="txtfabamt_<?=$i; ?>" placeholder="Display" readonly value="" /></td>
           </tr>
        </tbody>
        <tfoot>
            <tr bgcolor="#CCCCAA">
                ...
                <td>&nbsp;</td>
                <td>&nbsp;</td>
                <td align="right" colspan="5">Total Fabric Cost=</td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txt_tot_cons" id="txt_tot_cons" value="<?=$consTot; ?>" disabled /></td>
                <td>&nbsp;</td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txt_tot_tot_cost" id="txt_tot_tot_cost" value="<?=$consTotTot; ?>" disabled /></td>
                <td>&nbsp;</td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txt_tot_amt" id="txt_tot_amt" value="<?=$amtTot; ?>" disabled /></td>
            </tr>
            <tr>
                <td colspan="10" valign="middle" align="center">
                    <?=load_submit_buttons( $permission, "fnc_fabric_entry", $fabBtn,0 ,"",1); ?>
                </td>
            </tr>
        </tfoot>
    </table>
    
    <table width="500" cellspacing="0" border="1" class="rpt_table" id="tbl_yarn" rules="all">
        <thead>
            <td colspan="8" align="center" bgcolor="#99FFCC"><b>Yarn Charge Details</b></td>
        </thead>
        <thead>
        ...
        <tbody id="yarntbody">
            ...
            $sqlFabData=sql_select($sqlFab); $y=1; $yarnBtn=0;
            foreach($sqlFabData as $row)
            {
                if($row[csf("type")]==2)
                {
                    ?>
                    <tr id="yarntr_<?=$y; ?>">
                        <td><? echo create_drop_down( "cbocount_".$y, 60, $yarnCountArr, "",1,"-Count-", $row[csf("count")], "","","" ); ?>
                            <input type="hidden" class="text_boxes" name="hidfabid_<?=$y; ?>" id="hidfabid_<?=$y; ?>" value="<?=$row[csf("fab_id")]; ?>"/>
                            <input type="hidden" class="text_boxes" name="hidyarnupid_<?=$y; ?>" id="hidyarnupid_<?=$y; ?>" value="<?=$row[csf("id")]; ?>"/>
                        </td>
                        <td><? echo create_drop_down( "cbocompone_".$y, 100, $composition,"", 1, "-Select-", $row[csf("composition")], "","","","","" ); ?></td>
                        <td><input type="text" id="txtpercentone_<?=$y; ?>"  name="txtpercentone_<?=$y; ?>" class="text_boxes" style="width:38px" value="<?=$row[csf("percent")]; ?>" readonly/></td>
                        <td><? echo create_drop_down( "cbotype_".$y, 110, $yarn_type,"", 1, "-Select-", $row[csf("yarn_type")], "","","","","","" ); ?></td>
                        <td><?=create_drop_down("cbo_yarn_supplier_$y", 50, $supplier_library_fabric,'', 1, '-Select-',$row[csf("nominated_supp_multi")], "",'',"" ); ?></td>
                        <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtyarncons_<?=$y; ?>" id="txtyarncons_<?=$y; ?>" placeholder="Write" onChange="fnc_cons_amt('<?=$y.'_2'; ?>');" onBlur="append_row('<?=$y.'_2'; ?>');" value="<?=$row[csf("cons")]; ?>" /></td>
                        <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtyarnrate_<?=$y; ?>" id="txtyarnrate_<?=$y; ?>" onChange="fnc_cons_amt('<?=$y.'_2'; ?>');" value="<?=$row[csf("rate")]; ?>" /></td>
                        <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtyarnamt_<?=$y; ?>" id="txtyarnamt_<?=$y; ?>" placeholder="Display" readonly value="<?=$row[csf("value")]; ?>" /></td>
                    </tr>
                    <?
					$yarnBtn=1;
                    $y++;
					$consYarnTot+=$row[csf("cons")];
					$amtYarnTot+=$row[csf("value")];
                }
            }
            ?>
            <tr id="yarntr_<?=$y; ?>">
                <td><? echo create_drop_down( "cbocount_".$y, 60, $yarnCountArr, "",1,"-Count-", $selected, "","","" ); ?>
                    <input type="hidden" class="text_boxes" name="hidfabid_<?=$y; ?>" id="hidfabid_<?=$y; ?>" value=""/>
                    <input type="hidden" class="text_boxes" name="hidyarnupid_<?=$y; ?>" id="hidyarnupid_<?=$y; ?>" value=""/>
                </td>
                <td><? echo create_drop_down( "cbocompone_".$y, 100, $composition,"", 1, "-Select-", $selected, "","","","","" ); ?></td>
                <td><input type="text" id="txtpercentone_<?=$y; ?>"  name="txtpercentone_<?=$y; ?>" class="text_boxes" style="width:38px" value="" readonly/></td>
                <td><? echo create_drop_down( "cbotype_".$y, 110, $yarn_type,"", 1, "-Select-", $selected, "","","","","","" ); ?></td>
                <td><?=create_drop_down("cbo_yarn_supplier_$y", 50, $supplier_library_fabric,'', 1, '-Select-',"", "",'',"" ); ?></td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtyarncons_<?=$y; ?>" id="txtyarncons_<?=$y; ?>" placeholder="Write" onChange="fnc_cons_amt('<?=$y.'_2'; ?>');" onBlur="append_row('<?=$y.'_2'; ?>');" value="" /></td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtyarnrate_<?=$y; ?>" id="txtyarnrate_<?=$y; ?>" onChange="fnc_cons_amt('<?=$y.'_2'; ?>');" value="" /></td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtyarnamt_<?=$y; ?>" id="txtyarnamt_<?=$y; ?>" placeholder="Display" readonly value=""/></td>
            </tr>
        </tbody>
        <tfoot>
            <tr bgcolor="#CCCCAA">
                <td colspan="5" align="right">Total Yarn Cost=</td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txt_tot_yarncons" id="txt_tot_yarncons" value="<?=$consYarnTot; ?>" disabled /></td>
                <td>&nbsp;</td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txt_tot_yarnamt" id="txt_tot_yarnamt" value="<?=$amtYarnTot; ?>" disabled /></td>
            </tr>
            <tr>
    ...
    <table width="500" cellspacing="0" border="1" class="rpt_table" id="tbl_conv" rules="all">
        ...
        <tbody id="convbody">
			<?
            $sqlFab="select id, type, yarn_lib_id, fab_des, count, composition, percent, yarn_type, process, fab_id, cons, exper, tot_cons, rate, value from qc_fab_yarn_conv where qc_no='$qc_no' and cons_rate_dtls_id='$consRateId' and status_active=1 and is_deleted=0";
            ...
            foreach($sqlFabData as $row)
            {
                if($row[csf("type")]==3)
                {
                    ?>
                    <tr id="convtr_<?=$y; ?>">
                        <td><? echo create_drop_down( "cbofab_".$c, 250, $fab_description, "",1,"-Select-", $row[csf("fab_id")], "fnc_fabconsload(".$c.");","","" ); ?>
            ...
            ?>
           <tr id="convtr_<?=$y; ?>">
                <td><?=create_drop_down( "cbofab_".$c, 250, $fab_description, "",1,"-Select-", $selected, "fnc_fabconsload(".$c.");","","" ); ?>
    ...
    <table width="300" cellspacing="0" border="1" class="rpt_table" rules="all">
        ...
        <tbody>
            <td><input style="width:88px;" type="text" class="text_boxes_numeric" name="txtamount" id="txtamount" placeholder="Dispaly" value="<?=$fab_cost; ?>" readonly /></td>
            <td><input style="width:88px;" type="text" class="text_boxes_numeric" name="txtprocessloss" id="txtprocessloss" placeholder="Write" value="<?=$process_loss; ?>" onChange="fnc_total_rate(0);" /></td>
            <td><input style="width:88px;" type="text" class="text_boxes_numeric" name="txttotamount" id="txttotamount" placeholder="Dispaly" value="<?=$totfab_cost; ?>" readonly /></td>
        </tbody>
        <tfoot>
...
if($action=="save_update_delete_yarn")
{
    ...
    if ($operation==0)  // Insert Here
    {
        ...
        $id=return_next_id( "id", "qc_fab_yarn_conv", 1);
		$field_array="id, qc_no, body_part_id,nominated_supp_multi, type, fab_id, count, composition, percent, yarn_type, cons, rate, value, cons_rate_dtls_id, inserted_by, insert_date, status_active, is_deleted";
        ...
        for($i=1; $i<=$numRowYarn; $i++) 
        {
			...
			$cbonominasupplier ="cbo_yarn_supplier_".$i;
			$cbonominasupplier=str_replace("'", "", $$cbonominasupplier);
			
			if((str_replace("'",'',$$txtyarncons)*1)!=0)
			{
				if ($add_comma!=0) $data_array .=","; $add_comma=0;
				$data_array .="(".$id.",".$hid_qc_no.",".$hid_bodypart_id.",'".$cbonominasupplier."',2,".$$hidfabid.",".$$cbocount.",".$$cbocompone.",".$$txtpercentone.",".$$cbotype.",".$$txtyarncons.",".$$txtyarnrate.",".$$txtyarnamt.",".$hid_cons_rate_dtls_id.",'".$user_id."','".$pc_date_time."',1,0)";
				$id++; $add_comma++;
			}
        }
...
if($action=="generate_fabcons")
{
	$sql_arr=sql_select( "select tot_cons from qc_fab_yarn_conv where id='$data' and type=1 and status_active=1 and is_deleted=0");
	
	if(count($sql_arr)>0)
		echo $sql_arr[0][csf('tot_cons')];
	else
		echo '0';	
	exit();
}
...
if($action=="stage_saveUpdate_popup")
{
		...
		function fnc_stage_entry_reponse()
		{
			if(http.readyState == 4) 
			{
				...
				if(reponse[0]==0 || reponse[0]==1)
				{
					...
					show_list_view('','stage_data_list','stage_data_div','quick_costing_v3_controller','');
				}
			}
			release_freezing();
		}
    ...
    <script>show_list_view('','stage_data_list','stage_data_div','quick_costing_v3_controller','');</script>          
    <script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
    </html>
    <?
	exit();	
...
{
	echo  create_list_view ( "list_view", "Stage Name", "200","200","150",1, "select tuid, stage_name from lib_stage_name where status_active=1 and is_deleted=0 order by tuid desc", "get_php_form_data", "tuid","'load_php_data_to_form_satge'", 1, "0", $arr, "stage_name","quick_costing_v3_controller", 'setFilterGrid("list_view",-1);','0' );
	exit();
}
...
if($action=="save_update_delete")
{
	...
	$txt_exporter_code=str_replace("'","",$txt_exporter_code);
	$str_rep=array("/", "&", "*", "(", ")", "=","'",",",'"','#');
	if ($operation==0)  // Insert Here
	{	
		$con = connect();
		...
		$field_array_mst=" id, company_id, cost_sheet_id, cost_sheet_no, temp_id, lib_item_id, inquery_id, style_ref, buyer_id, uom, cons_basis, season_id, season_year, brand_id, style_des, team_leader, dealing_marchant, prod_dept, prod_code, department_id, delivery_date, exchange_rate, offer_qty, quoted_price, tgt_price, margin_per, margin, stage_id, costing_date, pre_cost_sheet_id, revise_no, option_id, buyer_remarks, option_remarks, article,currency_id, exporter_code, qc_no, entry_form, inserted_by, insert_date, status_active, is_deleted";
		
		$data_array_mst="(".$mst_id.",".str_replace("'","",$cbo_company_name).",".$mst_id.",'".$autoCostSheetNo."',".$cbo_temp_id.",".$txt_temp_id.",".$txt_inquery_id.",'".strtoupper(str_replace("'","",$txt_styleRef))."',".$cbo_buyer_id.",".$cbouom.",".$cbo_cons_basis_id.",".$cbo_season_id.",".$cbo_season_year.",".$cbo_brand.",'".strtoupper(str_replace("'","",$txt_styleDesc))."',".$cbo_team_leader.",".$cbo_dealing_merchant.",".$cbo_product_department.",".$txt_product_code.",".$cbo_subDept_id.",".$txt_delivery_date.",".$txt_exchangeRate.",".$txt_offerQty.",".$txt_quotedPrice.",".$txt_tgtPrice.",".$txtmarignper.",".$txtmarign.",".$cbo_stage_id.",".$txt_costingDate.",'".$update_id."','".$revise_no."','".$option_id."','".strtoupper($txt_costing_remarks)."','".strtoupper($txt_option_remarks)."',".$txt_article.",'".$cbo_currency_id."','".$txt_exporter_code."','".$qcno."',458,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		...
		if($type==6 || $type==7)
		{
			$sql_consrate=sql_select("select id, mst_id, item_id, type, fab_cost, process_loss, totfab_cost from qc_cons_rate_dtls where mst_id=$hid_qc_no and type=1 and status_active=1 and is_deleted=0 order by id ASC");
			//echo "10**select id, mst_id, item_id, type, fab_cost, process_loss, totfab_cost from qc_cons_rate_dtls where mst_id=$hid_qc_no and type=1 and status_active=1 and is_deleted=0 order by id ASC"; die;
			foreach($sql_consrate as $row)
			{
				$fabStrArr[$row[csf('id')]]['fab_cost']=$row[csf('fab_cost')];
				$fabStrArr[$row[csf('id')]]['process_loss']=$row[csf('process_loss')];
				$fabStrArr[$row[csf('id')]]['totfab_cost']=$row[csf('totfab_cost')];
			}
			unset($sql_consrate);
			//echo "10**";
			//print_r($fabStrArr); die;
		}
		
		$cons_rate_id=return_next_id("id", "qc_cons_rate_dtls", 1);
		$field_array_consrate="id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, value, fab_id, description, body_part_id, supp_ref, remarks, uom, inserted_by, insert_date, status_active, is_deleted";
		...
		$add_comma_cons_rate=0; $data_array_cons_rate=""; $consRateIdMapArr=array();
		foreach($temp_id as $item_id)
		{
			...
			foreach($ex_itemConsRateData as $item_wise_consRateData)
			{
				if($item_wise_consRateData!="")
				{
					...
					$bodypartid=$cons=$exPer=$totCons=$ratePop=$rate=$fabAmt=$fabRemarks=$fabupid=0;
					
					$bodypartid=$ex_itemConsRate[1];
					$cons=$ex_itemConsRate[2];
					$exPer=$ex_itemConsRate[3];
					$totCons=$ex_itemConsRate[4];
					$ratePop=$ex_itemConsRate[5];
					$rate=$ex_itemConsRate[6];
					$fabAmt=$ex_itemConsRate[7];
					$fabRemarks=$ex_itemConsRate[8];
					$fabupid=$ex_itemConsRate[9]*1;
					
					$consRateIdMapArr[$fabupid]=$cons_rate_id;
					
					if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
					$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',1,0,'".$cons."','".$exPer."','".$totCons."','".$ratePop."','".$rate."','".$fabAmt."',0,'','".$bodypartid."','','".$fabRemarks."',0,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			...
			foreach($ex_specialData as $item_wise_specialData)
			{
				...
				$speciaOperation_id=$speciaTypeId=$spbodypartid=$spConsumtion=$spExper=$totSpCons=$spRate=$spValue=$spRemarks=0;
				...
				$spRemarks=$ex_itemSpConsRate[9];
				
				
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',2,'".$speciaOperation_id."','".$spConsumtion."','".$spExper."','".$totSpCons."',0,'".$spRate."','".$spValue."','".$speciaTypeId."','','".$spbodypartid."','','".$spRemarks."',0,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			...
			foreach($ex_washData as $item_wise_washData)
			{
				...
				$washTypeId=$wbodypartid=$wConsumtion=$wExper=$totwCons=$wRate=$wValue=$wRemarks=0;
				...
				$wRemarks=$ex_itemwashConsRate[9];
				
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',3,'".$washTypeId."','".$wConsumtion."','".$wExper."','".$totwCons."',0,'".$wRate."','".$wValue."',0,'','".$wbodypartid."','','".$wRemarks."',0,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			...
			foreach($ex_itemAccData as $item_wise_itemAccData)
			{
				...
				$accessories_id=$accDescription=$accBandRef=$acConsumtion=$acExPer=$totAccCons=$acRate=$acVal=$acRemarks=$acconsUom=0;
				...
				$acRemarks=$ex_itemAcConsRate[9];
				$acconsUom=$ex_itemAcConsRate[10];
				
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',4,'".$accessories_id."','".$acConsumtion."','".$acExPer."','".$totAccCons."',0,'".$acRate."','".$acVal."',0,'".$accDescription."',0,'".$accBandRef."','".$acRemarks."','".$acconsUom."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		...
		if($type==6 || $type==7)
		{
			$fabSql=sql_select("select id, qc_no, body_part_id, color_type_id, nominated_supp_multi, gauge, type, yarn_lib_id, count, composition, percent, yarn_type, fab_id, process, fab_des, gsm, dia, diatype, uom, cons, exper, tot_cons, rate, value, yarn_break_down, conv_break_down, cons_rate_dtls_id from qc_fab_yarn_conv where qc_no=$hid_qc_no and status_active=1 and is_deleted=0 order by id asc");
			//echo "10**select id, qc_no, body_part_id, color_type_id, nominated_supp_multi, gauge, type, yarn_lib_id, count, composition, percent, yarn_type, fab_id, process, fab_des, gsm, dia, diatype, uom, cons, exper, tot_cons, rate, value, yarn_break_down, conv_break_down, cons_rate_dtls_id from qc_fab_yarn_conv where qc_no=$hid_qc_no and status_active=1 and is_deleted=0 order by id asc"; die;
			$field_arrayfabup="fab_cost*process_loss*totfab_cost";
	
			$fabfield_array="id, qc_no, body_part_id, color_type_id, nominated_supp_multi, gauge, type, yarn_lib_id, count, composition, percent, yarn_type, fab_id, process, fab_des, gsm, dia, diatype, uom, cons, exper, tot_cons, rate, value, yarn_break_down, conv_break_down, cons_rate_dtls_id, inserted_by, insert_date, status_active, is_deleted";
			$fabid=return_next_id( "id", "qc_fab_yarn_conv", 1); $fabdata_array=""; $fabricidarr=array(); //echo "10**";
			foreach($fabSql as $frow)
			{
				$hid_bodypart_id=$hidLibid=$txtgsm=$cbowidthdiatype=$cbouom=$txtfabcons=$txtfabex=$txtfabtotcons=$txtfabrate=$txtfabamt=$hidupid=$cbocolortype=$cbonominasupplier=$fabtype=$ycount=$ycomposition=$ypercent=$yyarn_type=$fabricid=$hid_cons_rate_dtls_id=0;
				$txtFabricDtls=$txtYarnDtls=$txtConvDtls=$txtdia=$txtgauge="";
				
				$hid_bodypart_id=$frow[csf('body_part_id')];
				$cbocolortype=$frow[csf('color_type_id')];
				$cbonominasupplier=$frow[csf('nominated_supp_multi')];
				$txtgauge=$frow[csf('gauge')];
				$fabtype=$frow[csf('type')];
				$hidLibid=$frow[csf('yarn_lib_id')];
				$ycount=$frow[csf('count')];
				$ycomposition=$frow[csf('composition')];
				$ypercent=$frow[csf('percent')];
				$yyarn_type=$frow[csf('yarn_type')];
				$cprocess=$frow[csf('process')];
				$txtFabricDtls=$frow[csf('fab_des')];
				$txtgsm=$frow[csf('gsm')];
				$txtdia=$frow[csf('dia')];
				$cbowidthdiatype=$frow[csf('diatype')];
				$cbouom=$frow[csf('uom')];
				$txtfabcons=$frow[csf('cons')];
				$txtfabex=$frow[csf('exper')];
				
				$txtfabtotcons=$frow[csf('tot_cons')];
				$txtfabrate=$frow[csf('rate')];
				$txtfabamt=$frow[csf('value')];
				$txtYarnDtls=$frow[csf('yarn_break_down')];
				$txtConvDtls=$frow[csf('conv_break_down')];
				$hid_cons_rate_dtls_id=$consRateIdMapArr[$frow[csf('cons_rate_dtls_id')]];
				
				$fabricid=0;
				if($fabtype==1)
				{
					$fabricidarr[$frow[csf('id')]]=$fabid;
					$fabricid=0;
				}
				else
				{
					//$fabricidarr[$frow[csf('cons_rate_dtls_id')]]=0;
					$fabricid=$fabricidarr[$frow[csf('fab_id')]];
				}
				
				//$fabricid=$fabricidarr[$frow[csf('cons_rate_dtls_id')]];
				//echo $fabricid.'='.$fabid.'='.$frow[csf('fab_id')].'='.$fabtype.'<br>';
				
				if ($fabadd_comma!=0) $fabdata_array .=","; $fabadd_comma=0;
				$fabdata_array .="(".$fabid.",".$qcno.",'".$hid_bodypart_id."','".$cbocolortype."','".$cbonominasupplier."','".$txtgauge."','".$fabtype."','".$hidLibid."','".$ycount."','".$ycomposition."','".$ypercent."','".$yyarn_type."','".$fabricid."','".$cprocess."','".$txtFabricDtls."','".$txtgsm."','".$txtdia."','".$cbowidthdiatype."','".$cbouom."','".$txtfabcons."','".$txtfabex."','".$txtfabtotcons."','".$txtfabrate."','".$txtfabamt."','".$txtYarnDtls."','".$txtConvDtls."','".$hid_cons_rate_dtls_id."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
				
				$fabid++; $fabadd_comma++;
			}
			
			foreach($consRateIdMapArr as $oldfabid=>$newfabid)
			{
				$idfab_arr[]=$newfabid;
				$data_arrayfabup[$newfabid] =explode("*",("'".$fabStrArr[$oldfabid]['fab_cost']."'*'".$fabStrArr[$oldfabid]['process_loss']."'*'".$fabStrArr[$oldfabid]['totfab_cost']."'"));
			}
		}
		...
		$totLbTstCst_td=trim($ex_tot_cost_data[12]);
		$totMissCst_td=trim($ex_tot_cost_data[13]);
		$totOtherCst_td=trim($ex_tot_cost_data[14]);
		$totCommCst_td=trim($ex_tot_cost_data[15]);
		$totCost_td=trim($ex_tot_cost_data[16]);
		$totalFob_td=trim($ex_tot_cost_data[17]);
		$totRmgQty_td=trim($ex_tot_cost_data[18]);
		$totCommlCst_td=trim($ex_tot_cost_data[19]);
		$txt_commlPer=trim($ex_tot_cost_data[20]);
		
		$data_array_tot_cost="(".$tc_sum_id.",".$qcno.",'".$cbo_buyer_agent."','".$cbo_agent_location."','".$txt_noOfPack."',0,'".$cmPop."','".$txt_lumSum_cost."','".$txt_commPer."','".$totFab_td."','".$totSpc_td."','".$totWash_td."','".$totAcc_td."','".$totCm_td."','".$totFriCst_td."','".$totLbTstCst_td."','".$totMissCst_td."','".$totOtherCst_td."','".$totCommCst_td."','".$totCost_td."','".$totalFob_td."','".$totRmgQty_td."','".$txt_commlPer."','".$totCommlCst_td."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		//echo $data_array_tot_cost; die;
		$field_array_item_tot=" id, mst_id, item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs, rmg_ratio, commercial_cost, inserted_by, insert_date,operating_exp,storage_cost,sam_cost,remark";
		$item_cost_id=return_next_id("id", "qc_item_cost_summary", 1);
		$ex_item_wise_tot_data=array_filter(explode('###',str_replace("'","",$item_wise_tot_data)));
		$add_comma_item_tot=0; $data_array_item_tot="";
		foreach($ex_item_wise_tot_data as $item_wise_tot)
		{
			$fab_td=$spOpe_td=$wash_td=$acc_td=$txtCpm=$txtSmv=$txtEff=$txtCmCost=$txtFriCost=$txtLtstCost=$txtMissCost=$txtOtherCost=$txtCommCost=$fobT_td=$fobDzn_td=$tot_item_id=$txtCommlCost=0;
			...
			$txtLtstCost=trim($ex_item_tot[9]);
			$txtMissCost=trim($ex_item_tot[10]);
			$txtOtherCost=trim($ex_item_tot[11]);
			$txtCommCost=trim($ex_item_tot[12]);
			$fobT_td=trim($ex_item_tot[13]);
			$fobDzn_td=trim($ex_item_tot[14]);
			$txtRmgQty=trim($ex_item_tot[15]);
			$txtCommlCost=trim($ex_item_tot[16]);
			$tot_item_id=trim($ex_item_tot[17]);
			$txtstorage=trim($ex_item_tot[18]);
			$txtoperatingexpense=trim($ex_item_tot[19]);
			$txt_sam=trim($ex_item_tot[20]);
			$txtremark=trim($ex_item_tot[21]);
			
			if ($add_comma_item_tot!=0) $data_array_item_tot .=",";
			$data_array_item_tot .="(".$item_cost_id.",".$qcno.",'".$tot_item_id."','".$fab_td."','".$spOpe_td."','".$wash_td."','".$acc_td."','".$txtCpm."','".$txtSmv."','".$txtEff."','".$txtCmCost."','".$txtFriCost."','".$txtLtstCost."','".$txtMissCost."','".$txtOtherCost."','".$txtCommCost."','".$fobT_td."','".$txtRmgQty."','".$txtCommlCost."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$txtoperatingexpense."','".$txtstorage."','".$txt_sam."','".$txtremark."')";
		...
		//if($type==6 || $type==7) { echo "10**insert into qc_fab_yarn_conv (".$fabfield_array.") values ".$fabdata_array;die; }
		...
		if($type==6 || $type==7)
		{
			$rIDFab=sql_insert("qc_fab_yarn_conv",$fabfield_array,$fabdata_array,0);
		...
		//echo "10**insert into qc_item_cost_summary ($field_array_item_tot) values ($data_array_item_tot)"; die;
		...
		//echo '10**'.$flag.'='.$rID_consRate.'='.$rID_tot_cost.'='.$rID_item_tot.'='.$rID.'='.$rIDFab.'='.$rIDfabup; die;
		/*if(str_replace("'","",$chk_is_new_meeting)==1)
		{*/
		if(str_replace("'","",$txt_meeting_remarks)!="" && str_replace("'","",$txt_meeting_remarks)!="1. ")
		{
		...
		if($flag==1)
		{
			...
			{
				oci_commit($con);
				//echo "10**".bulk_update_sql_statement("qc_cons_rate_dtls", "id",$field_arrayfabup,$data_arrayfabup,$idfab_arr ); die;
				$rIDfabup=execute_query(bulk_update_sql_statement("qc_cons_rate_dtls", "id",$field_arrayfabup,$data_arrayfabup,$idfab_arr ));
				if($rIDfabup==1 && $flag==1) $flag=1; else $flag=0;
			}
		}
		/*}
		else
	...
	{	
		...
		$team_dtls_sql=sql_select("select a.id as team_leader from lib_marketing_team a where a.id in (select b.team_id from lib_mkt_team_member_info b where b.user_tag_id='$user_id' and b.status_active=1 and b.is_deleted=0 ) and a.status_active=1 and a.is_deleted=0");
		
		$team_leaderid=$team_dtls_sql[0][csf('team_leader')];
		
		$team_dtls_sql=sql_select("select b.user_tag_id as team_member from lib_mkt_team_member_info b where b.team_id in (select id from lib_marketing_team a where id='$team_leaderid' and a.status_active=1 and a.is_deleted=0) and b.status_active=1 and b.is_deleted=0");
		if(count($team_dtls_sql)>1)
		{
			foreach($team_dtls_sql as $row)
			{
		...
		//print_r($team_dtls_arr);
		if($team_dtls_arr[$insert_user_id]=='' && $user_level!=2)
		{
			echo "6**"."Update restricted, This Information is Update Only team Leader or member.";
			disconnect($con);die;
		}
		...
		$field_array_mst="company_id*temp_id*lib_item_id*inquery_id*style_ref*buyer_id*uom*cons_basis*season_id*season_year*brand_id*style_des*team_leader*dealing_marchant*prod_dept*prod_code*department_id*delivery_date*exchange_rate*offer_qty*quoted_price*tgt_price*margin_per*margin*stage_id*costing_date*buyer_remarks*article*currency_id*exporter_code*option_remarks*updated_by*update_date";
		
		//$txt_temp_id=explode(",",str_replace("'","",$txt_temp_id));
		//asort($txt_temp_id);
		//$txt_temp_id="'".implode(",",$txt_temp_id)."'";
		
		$data_array_mst="".$cbo_company_name."*".$cbo_temp_id."*".$txt_temp_id."*".$txt_inquery_id."*'".strtoupper(str_replace("'","",$txt_styleRef))."'*".$cbo_buyer_id."*".$cbouom."*".$cbo_cons_basis_id."*".$cbo_season_id."*".$cbo_season_year."*".$cbo_brand."*'".strtoupper(str_replace("'","",$txt_styleDesc))."'*".$cbo_team_leader."*".$cbo_dealing_merchant."*".$cbo_product_department."*".$txt_product_code."*".$cbo_subDept_id."*".$txt_delivery_date."*".$txt_exchangeRate."*".$txt_offerQty."*".$txt_quotedPrice."*".$txt_tgtPrice."*".$txtmarignper."*".$txtmarign."*".$cbo_stage_id."*".$txt_costingDate."*'".strtoupper($txt_costing_remarks)."'*".$txt_article."*'".$cbo_currency_id."'*'".$txt_exporter_code."'*'".strtoupper($txt_option_remarks)."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		$fieldArrConsRate="particular_type_id*consumption*ex_percent*tot_cons*is_calculation*rate*value*fab_id*description*body_part_id*supp_ref*remarks*uom*updated_by*update_date";
		...
		foreach($temp_id as $item_id)
		{
			...
			foreach($ex_itemConsRateData as $item_wise_consRateData)
			{
				if($item_wise_consRateData!="")
				{
					...
					{
						$id_arr[]=$fabupid;
						$fabConsRateUpdateArr[$fabupid] =explode("*",("'0'*'".$cons."'*'".$exPer."'*'".$totCons."'*'".$ratePop."'*'".$rate."'*'".$fabAmt."'*'0'*''*'".$bodypartid."'*''*'".$fabRemarks."'*'0'*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'"));
					}
					else if($fabupid==0)
					{
						if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
		...
		foreach($ex_item_wise_tot_data as $item_wise_tot)
		{
			$fab_td=$spOpe_td=$wash_td=$acc_td=$txtCpm=$txtSmv=$txtEff=$txtCmCost=$txtFriCost=$txtLtstCost=$txtMissCost=$txtOtherCost=$txtCommCost=$fobT_td=$fobDzn_td=$tot_item_id=0;
		...
		//echo "10**delete from qc_cons_rate_dtls where mst_id =".$hid_qc_no." and type<>1"; die;
...
if($action=="style_popup")
{
	...
	{
		show_list_view (document.getElementById('cbo_buyer_id').value+'**'+document.getElementById('cbo_season_id').value+'**'+document.getElementById('cbo_subDept_id').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('txt_cost_sheet_no').value+'**'+document.getElementById('hide_dataType').value+'**'+document.getElementById('cbo_approval_type').value+'**'+document.getElementById('cbo_company_name').value+'**'+document.getElementById('cbo_string_search_type').value, 'style_ref_search_list_view', 'search_div', 'quick_costing_v3_controller', 'setFilterGrid(\'tbl_list_search\',-1)');
		set_all();
	}
	...
	function fnc_confirm_style(qcno,upid)
	{
		if(upid!="")
		{
			...
			var page_link='quick_costing_v3_controller.php?action=confirmStyle_popup';
    ...
    <div align="center">
		...
		<fieldset>
            <table cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr class="general">
						<td><?=create_drop_down( "cbo_company_name", 130, "select id, company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "Select Company", $company_id, "load_drop_down( 'quick_costing_v3_controller', this.value, 'load_drop_down_buyer_pop', 'buyer_pop_td');",1); ?></td>
                        <td id="buyer_pop_td"><? echo create_drop_down( "cbo_buyer_id", 130, $blank_array,'', 1, "-Select-" ); ?></td>                 
    ...
    <script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
	<script>
		load_drop_down('quick_costing_v3_controller', <?=$company_id; ?>, 'load_drop_down_buyer_pop', 'buyer_pop_td' )
		document.getElementById('cbo_buyer_id').value=<?=$buyerId;?>
    </script>
    <script>
...
{
	echo create_drop_down( "cbo_buyer_id", 130, "select buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond  and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down( 'quick_costing_v3_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'quick_costing_v3_controller',this.value, 'load_drop_down_sub_dep', 'sub_td' );",0 );
	exit();
}
...
if($action=="style_ref_search_list_view")
{
	...
	{
		$sql_style = "select min(a.id) as id, a.cost_sheet_no, max(a.costing_date) as costing_date, a.buyer_id, a.season_id, a.style_ref, a.department_id, a.temp_id, a.lib_item_id, a.stage_id, max(a.inserted_by) as inserted_by, max(a.updated_by) as updated_by, min(a.qc_no) as qc_no from qc_mst a where a.status_active=1 and a.is_deleted=0 and a.entry_form=458 and a.company_id=$company $buyer_id_cond $style_ref_cond $cost_sheet_no_cond $season_id_cond $department_id_cond $approvaltype_cond group by a.cost_sheet_no, a.buyer_id, a.season_id, a.style_ref, a.department_id, a.temp_id, a.lib_item_id, a.stage_id order by id Desc";
...
if($action=="temp_style_list_view")
{
	...
	if($ex_data[1]==1)
	{
		...
		$sql_style= "select min(id) as id, cost_sheet_no, style_ref, min(qc_no) as qc_no, season_id, inserted_by from qc_mst where entry_form=458 and cost_sheet_no in ($ex_data[0]) and cost_sheet_no not in (select cost_sheet_no from qc_style_temp_data where cost_sheet_no in ($ex_data[0]) and inserted_by='$user_id' and status_active=1 and is_deleted=0 ) and revise_no=0 and option_id=0 and status_active=1 and is_deleted=0 group by cost_sheet_no, style_ref, season_id, inserted_by";
		//echo  $sql_style;
	...
	$sql_temp_style=sql_select("select a.id, a.cost_sheet_no, a.style_ref, a.season_id, a.inserted_by, a.updated_by, a.qc_no from qc_mst a, qc_style_temp_data b where a.id=b.mst_id and b.inserted_by='$user_id' and a.entry_form=458 and b.status_active=1 and b.is_deleted=0 order by a.style_ref ASC");
...
if($action=="populate_style_details_data")
{
	...
	$sql_tmp="select tuid, temp_id, item_id1, ratio1, item_id2, ratio2, status_active from qc_template where status_active=1 and is_deleted=0  order by id ASC";
	//$sql_tmp="select tuid, temp_id, item_id1, ratio1, status_active from qc_template where status_active=1 and is_deleted=0 order by temp_id ASC";
	$sql_tmp_res=sql_select($sql_tmp);
	$mst_temp_arr=array();
	foreach($sql_tmp_res as $row)
	{
	...
	$data_array=sql_select("select id, cost_sheet_id, cost_sheet_no, temp_id, lib_item_id, inquery_id, style_ref, buyer_id, uom, cons_basis, season_id, season_year, brand_id, style_des, team_leader, dealing_marchant, prod_dept, prod_code, department_id, delivery_date, exchange_rate, offer_qty, quoted_price, tgt_price, margin_per, margin, stage_id, costing_date, revise_no, option_id, buyer_remarks, article, option_remarks, inserted_by, meeting_no, qc_no,exporter_code, company_id from qc_mst where status_active=1 and is_deleted=0 and cost_sheet_no='$cost_sheet_no' $rev_no_cond $option_id_cond");//$starting_row_cond and inserted_by='$cond_user'  qc_no='$cost_sheet_id'   
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_drop_down('requires/quick_costing_v3_controller', '".trim($row[csf("lib_item_id")])."', 'load_drop_down_tempItem', 'item_td'); sub_dept_load('".$row[csf("buyer_id")]."','".$row[csf("prod_dept")]."','".$row[csf("department_id")]."');\n";
		...
		echo "load_drop_down('requires/quick_costing_v3_controller', '".$row[csf("buyer_id")]."', 'load_drop_down_season', 'season_td');\n"; 
		echo "load_drop_down('requires/quick_costing_v3_controller', '".$row[csf("buyer_id")]."', 'load_drop_down_brand', 'brand_td');\n";
		//echo "load_drop_down('requires/quick_costing_v3_controller', '".$row[csf("buyer_id")]."', 'load_drop_down_sub_dep', 'sub_td');\n";
		echo "load_drop_down('requires/quick_costing_v3_controller', '".$row[csf("team_leader")]."', 'load_drop_down_dealing_merchant', 'tddealmerchent');\n";
		...
		echo "$('#cbo_dealing_merchant').val('".$row[csf("dealing_marchant")]."');\n";
		...
		echo "$('#txtmarignper').val('".$row[csf("margin_per")]."');\n";
		...
		echo "$('#txt_exporter_code').val('".$row[csf("exporter_code")]."');\n"; 
		echo "$('#txt_isUpdate').val(1);\n"; 

		echo "disable_enable_fields('cbo_temp_id*cbouom*cbo_cons_basis_id',1);\n";
		if($ex_data[4]!="from_option")
		{
			//echo "load_drop_down('requires/quick_costing_v3_controller', '".$row[csf("cost_sheet_no")].'__'.$row[csf("option_id")].'__'.$row[csf("revise_no")]."', 'load_drop_down_revise_no', 'revise_td');\n"; //.'__'.$row[csf("revise_no")]
			//echo "load_drop_down('requires/quick_costing_v3_controller', '".$row[csf("cost_sheet_no")].'__'.$row[csf("option_id")]."', 'load_drop_down_option_id', 'option_td');\n"; 
		}
	...
	$sql_cons_rate="select id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, value, fab_id, description, body_part_id, supp_ref, remarks, uom from qc_cons_rate_dtls where mst_id=$cond_data and status_active=1 and is_deleted=0 order by id asc";
	...
	foreach ($sql_result_cons_rate as $rowConsRate)
	{
		...
		{
			$cons_rate_fab_arr[$rowConsRate[csf("item_id")]].=$body_part[$rowConsRate[csf("body_part_id")]].'_'.$rowConsRate[csf("body_part_id")].'_'.$rowConsRate[csf("consumption")].'_'.$rowConsRate[csf("ex_percent")].'_'.$rowConsRate[csf("tot_cons")].'_'.$rowConsRate[csf("is_calculation")].'_'.$rowConsRate[csf("rate")].'_'.$rowConsRate[csf("value")].'_'.$rowConsRate[csf("remarks")].'_'.$rowConsRate[csf("id")].'##';
		}
		if($rowConsRate[csf("type")]==2)
		{
			$cons_rate_sp_arr[$rowConsRate[csf("item_id")]].=$rowConsRate[csf("particular_type_id")].'_'.$rowConsRate[csf("fab_id")].'_'.$body_part[$rowConsRate[csf("body_part_id")]].'_'.$rowConsRate[csf("body_part_id")].'_'.$rowConsRate[csf("consumption")].'_'.$rowConsRate[csf("ex_percent")].'_'.$rowConsRate[csf("tot_cons")].'_'.$rowConsRate[csf("rate")].'_'.$rowConsRate[csf("value")].'_'.$rowConsRate[csf("remarks")].'##';
		}
		if($rowConsRate[csf("type")]==3)
		{
			$cons_rate_wash_arr[$rowConsRate[csf("item_id")]].=$emblishment_wash_type[$rowConsRate[csf("particular_type_id")]].'_'.$rowConsRate[csf("particular_type_id")].'_'.$body_part[$rowConsRate[csf("body_part_id")]].'_'.$rowConsRate[csf("body_part_id")].'_'.$rowConsRate[csf("consumption")].'_'.$rowConsRate[csf("ex_percent")].'_'.$rowConsRate[csf("tot_cons")].'_'.$rowConsRate[csf("rate")].'_'.$rowConsRate[csf("value")].'_'.$rowConsRate[csf("remarks")].'##';
		}
		if($rowConsRate[csf("type")]==4)
		{
			$cons_rate_ac_arr[$rowConsRate[csf("item_id")]].=$itemGroupArr[$rowConsRate[csf("particular_type_id")]].'_'.$rowConsRate[csf("particular_type_id")].'_'.$rowConsRate[csf("description")].'_'.$rowConsRate[csf("supp_ref")].'_'.$rowConsRate[csf("consumption")].'_'.$rowConsRate[csf("ex_percent")].'_'.$rowConsRate[csf("tot_cons")].'_'.$rowConsRate[csf("rate")].'_'.$rowConsRate[csf("value")].'_'.$rowConsRate[csf("remarks")].'_'.$rowConsRate[csf("uom")].'##';
		}
	...
	foreach($sql_result_summ as $rowSumm)
	{
		...
		echo "$('#totFOBCost_td').text('".number_format(($rowSumm[csf("tot_cost")]/12),2)."');\n";
	}

	unset($sql_result_summ);
	$sql_item_summ="select item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs, rmg_ratio, commercial_cost,operating_exp,storage_cost,sam_cost,remark from qc_item_cost_summary where mst_id='$cond_data' and status_active=1 and is_deleted=0";
	...
	foreach($sql_result_item_summ as $rowItemSumm)
	{
		...
		echo "$('#txtstorage_".$rowItemSumm[csf("item_id")]."').val('".$rowItemSumm[csf("storage_cost")]."');\n";
		echo "$('#txtoperatingexpense_".$rowItemSumm[csf("item_id")]."').val('".$rowItemSumm[csf("operating_exp")]."');\n";
		echo "$('#txt_sam_".$rowItemSumm[csf("item_id")]."').val('".$rowItemSumm[csf("sam_cost")]."');\n";
		echo "$('#txtremark_".$rowItemSumm[csf("item_id")]."').val('".$rowItemSumm[csf("remark")]."');\n";
		...
		$parameter=$rowItemSumm[csf("item_id")];
		//$parameter13="13,".$rowItemSumm[csf("item_id")].",0";
		echo "fnc_amount_calculation('12','".$parameter."','0');\n";
		echo "fnc_amount_calculation('13','".$parameter."','0');\n";
		
		//echo "fnc_amount_calculation(13,".$rowItemSumm[csf("item_id")].",0);\n";
		//echo "console.log(".$rowItemSumm[csf("item_id")].")\n";
	}
...
if($action=="copy_cost_sheet")
{
	...
	{	
		...
		if (is_duplicate_field( "cost_sheet_no", "qc_mst", "style_ref='$dup_styleRef' and season_id='$dup_season_id' and buyer_id='$dup_buyer_id' and entry_form=458 and status_active=1 and is_deleted=0")==1)
		{
			echo "11**".$msg; 
			die;
		}
		...
		$qcno=$_SESSION['logic_erp']['user_id'].'000'.$mst_id;
		$sql_mst=sql_select("select id, cost_sheet_id, cost_sheet_no, temp_id, lib_item_id, inquery_id, style_ref, buyer_id, uom, cons_basis, season_id, season_year, brand_id, style_des, team_leader, dealing_marchant, prod_dept, prod_code, margin, department_id, delivery_date, exchange_rate, offer_qty, quoted_price, tgt_price, margin_per, margin, stage_id, costing_date, buyer_remarks, option_remarks, article, currency_id, exporter_code, entry_form from qc_mst where qc_no='$hid_qc_no'");
		...
		$cbo_team_leader=$sql_mst[0][csf('team_leader')];
		$cbo_dealing_merchant=$sql_mst[0][csf('dealing_marchant')];
		$cbo_subDept_id=$sql_mst[0][csf('department_id')];
		...
		$txtmarignper=$sql_mst[0][csf('margin_per')];
		$txtmarign=$sql_mst[0][csf('margin')];
		...
		$cbo_currency_id=$sql_mst[0][csf('currency_id')];
		$txt_exporter_code=$sql_mst[0][csf('exporter_code')];
		...
		$data_array_mst="(".$mst_id.",".$mst_id.",'".$autoCostSheetNo."','".$cbo_temp_id."','".$txt_temp_id."','".$txt_inquery_id."',".$txt_styleRef.",'".$dup_buyer_id."','".$cbouom."','".$cbo_cons_basis_id."',".$cbo_season_id.",'".$cbo_season_year."','".$cbo_brand."','".$txt_styleDesc."','".$cbo_team_leader."','".$cbo_dealing_merchant."','".$cbo_product_department."','".$txt_product_code."','".$cbo_subDept_id."','".$txt_delivery_date."','".$txt_exchangeRate."','".$txt_offerQty."','".$txt_quotedPrice."','".$txt_tgtPrice."','".$txtmarignper."','".$txtmarign."','".$cbo_stage_id."','".$txt_costingDate."','".$hid_qc_no."','".$revise_no."','".$option_id."','".$txt_costing_remarks."','".$txt_option_remarks."','".$txt_article."','".$cbo_currency_id."','".$txt_exporter_code."','".$qcno."',458,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		
		$temp_id=explode(",",str_replace("'","",$txt_temp_id));
		
		$cons_rate_id=return_next_id("id", "qc_cons_rate_dtls", 1);
		$sql_consrate=sql_select("select id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, value, fab_id, description, body_part_id, supp_ref, remarks, uom, fab_cost, process_loss, totfab_cost from qc_cons_rate_dtls where mst_id='$hid_qc_no' order by id ASC");
		
		$field_array_consrate="id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, value, fab_id, description, body_part_id, supp_ref, remarks, uom, fab_cost, process_loss, totfab_cost, inserted_by, insert_date, status_active, is_deleted";
		$add_comma_cons_rate=0; $data_array_cons_rate=""; $consRateIdMapArr=array();
		foreach( $sql_consrate as $rowConsrate )
		{
			...
			$is_calculation=$rowConsrate[csf('is_calculation')];
			$remarks=$rowConsrate[csf('remarks')];
			$body_part_id=$rowConsrate[csf('body_part_id')];
			$supp_ref=$rowConsrate[csf('supp_ref')];
			$uom=$rowConsrate[csf('uom')];
			$fab_cost=$rowConsrate[csf('fab_cost')];
			$process_loss=$rowConsrate[csf('process_loss')];
			$totfab_cost=$rowConsrate[csf('totfab_cost')];
			
			if($type==1) $consRateIdMapArr[$old_consrate_id]=$cons_rate_id;
			
			if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
			$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."','".$type."','".$head_id."','".$cons."','".$ex_percent."','".$tot_cons."','".$is_calculation."','".$rate."','".$tot_val."','".$fab_id."','".$description."','".$body_part_id."','".$supp_ref."','".$remarks."','".$uom."','".$fab_cost."','".$process_loss."','".$totfab_cost."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		...
		//die;
		$tc_sum_id=return_next_id("id", "qc_tot_cost_summary", 1);
		$sql_tot_cost=sql_select("select id, mst_id, buyer_agent_id, location_id, no_of_pack, is_confirm, is_cm_calculative, mis_lumsum_cost, commision_per, tot_fab_cost, tot_sp_operation_cost, tot_wash_cost, tot_accessories_cost, tot_cm_cost, tot_fright_cost, tot_lab_test_cost, tot_miscellaneous_cost, tot_other_cost, tot_commission_cost, tot_cost, tot_fob_cost, tot_rmg_ratio, commercial_per, commercial_cost from qc_tot_cost_summary where mst_id='$hid_qc_no'");
		
		$field_array_tot_cost="id, mst_id, buyer_agent_id, location_id, no_of_pack, is_confirm, is_cm_calculative, mis_lumsum_cost, commision_per, tot_fab_cost, tot_sp_operation_cost, tot_wash_cost, tot_accessories_cost, tot_cm_cost, tot_fright_cost, tot_lab_test_cost, tot_miscellaneous_cost, tot_other_cost, tot_commission_cost, tot_cost, tot_fob_cost, tot_rmg_ratio, commercial_per, commercial_cost, inserted_by, insert_date, status_active, is_deleted";
		...
		$data_array_tot_cost="(".$tc_sum_id.",".$qcno.",'".$cbo_buyer_agent."','".$cbo_agent_location."','".$txt_noOfPack."',0,'".$cmPop."','".$txt_lumSum_cost."','".$txt_commPer."','".$totFab_td."','".$totSpc_td."','".$totWash_td."','".$totAcc_td."','".$totCm_td."','".$totFriCst_td."','".$totLbTstCst_td."','".$totMissCst_td."','".$totOtherCst_td."','".$totCommCst_td."','".$totCost_td."','".$totalFob_td."','".$totalRmg_td."','".$txt_commlPer."','".$totCommlCst_td."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		//echo $data_array_tot_cost; die;
		$sql_item_tot=sql_select("select id, mst_id, item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs, rmg_ratio, commercial_cost, operating_exp, storage_cost, sam_cost, remark from qc_item_cost_summary where mst_id='$hid_qc_no' order by id ASC");
		
		$field_array_item_tot="id, mst_id, item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs, rmg_ratio, commercial_cost, operating_exp, storage_cost, sam_cost, remark, inserted_by, insert_date, status_active, is_deleted";
		$item_cost_id=return_next_id("id", "qc_item_cost_summary", 1);
		$add_comma_item_tot=0; $data_array_item_tot="";
		foreach($sql_item_tot as $rowItem_tot)
		{
			$fab_td=$spOpe_td=$wash_td=$acc_td=$txtCpm=$txtSmv=$txtEff=$txtCmCost=$txtFriCost=$txtLtstCost=$txtMissCost=$txtOtherCost=$txtCommCost=$txtoperatingexpense=$txtstorage=$txt_sam=$txtremark=$fobT_td=$tot_item_id=$rmg_ratio=0;
			...
			$txtoperatingexpense=$rowItem_tot[csf('operating_exp')];
			$txtstorage=$rowItem_tot[csf('storage_cost')];
			$txt_sam=$rowItem_tot[csf('sam_cost')];
			$txtremark=$rowItem_tot[csf('remark')];
			...
			if ($add_comma_item_tot!=0) $data_array_item_tot .=",";
			$data_array_item_tot .="(".$item_cost_id.",".$qcno.",'".$tot_item_id."','".$fab_td."','".$spOpe_td."','".$wash_td."','".$acc_td."','".$txtCpm."','".$txtSmv."','".$txtEff."','".$txtCmCost."','".$txtFriCost."','".$txtLtstCost."','".$txtMissCost."','".$txtOtherCost."','".$txtCommCost."','".$fobT_td."','".$rmg_ratio."','".$txtCommlCost."','".$txtoperatingexpense."','".$txtstorage."','".$txt_sam."','".$txtremark."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
...
if($action=="confirmStyle_popup")
{
    ...
    <script>
		...
		function fnc_confirm_entry( operation )
		{
			...
			if(operation==3)
			{
				...
				var comml_amount=$("#txtComml_bom").val();
				...
				var qc_fab_amt=0; var qc_sp_amt=0; var qc_wash_amt=0; var qc_acc_amt=0; var qc_comml_amt=0; var qc_fri_amt=0; var qc_lab_amt=0; var qc_misce_amt=0; var qc_other_amt=0; var qc_comm_amt=0; var qc_fob_amt=0; var qc_cm_amt=0; var qc_rmg_amt=0;
				for(j=1; j<=split_tmep_id.length; j++)
				{
					...
					qc_comml_amt+=$("#txtComml_"+itm_id).val()*1;
				...
				if(job_no!="")
				{
					...
					{
						alert("BOM Commercial Amount is Greater Than QC.");
						release_freezing();	
						return;
					}
			...
			for(i=1; i<=split_tmep_id.length; i++)
			{
				...
				data_dtls+=get_submitted_data_string('txtitemid_'+itm_id+'*txtdtlsupid_'+itm_id+'*txtFabCst_'+itm_id+'*txtSpOpa_'+itm_id+'*txtWash_'+itm_id+'*txtAcc_'+itm_id+'*txtComml_'+itm_id+'*txtFrightCst_'+itm_id+'*txtLabCst_'+itm_id+'*txtMiscCst_'+itm_id+'*txtOtherCst_'+itm_id+'*txtCommCst_'+itm_id+'*txtFobDzn_'+itm_id+'*txtCpm_'+itm_id+'*txtSmv_'+itm_id+'*txtCmCst_'+itm_id+'*txtPack_'+itm_id,"../../../",2);
				k++;
			}
			var data_mst="action=save_update_delete_confirm_style&operation="+operation+get_submitted_data_string('txt_costSheet_id*txtConfirm_id*txtItem_id*txt_confirm_style*txt_order_qty*txt_confirm_fob*txt_ship_date*txt_job_id*cbo_approved_status*cbo_ready_approve*cbo_company_name',"../../../",2);
		...
		function fnc_stage_entry_reponse()
		{
			if(http.readyState == 4) 
			{
				...
				if(trim(reponse[0])==0 || trim(reponse[0])==1 || trim(reponse[0])==2 || trim(reponse[0])==3)
				{
					...
					get_php_form_data($('#txt_costSheet_id').val()+'__'+$('#txtItem_id').val(),'populate_confirm_style_form_data','quick_costing_v3_controller');
					//show_list_view('','stage_data_list','stage_data_div','quick_costing_v3_controller','');
					js_set_value();
				}
			}
		...
		function fnc_openJobPopup()
		{
			...
			page_link='quick_costing_v3_controller.php?action=style_tag_popup&data='+data;
		...
		function fnc_bom_data_load()
		{
			...
			if(job_no!="")
			{
				var str_data=return_global_ajax_value( job_no, 'budgete_cost_validate', '', 'quick_costing_v3_controller');
				
				var spdata=str_data.split("##");
				
				var fab_amount=spdata[0]; var sp_oparation_amount=spdata[1]; var wash_amount=spdata[11]; var acc_amount=spdata[2]; var comml_amount=spdata[12]; var fright_amount=spdata[3]; var lab_amount=spdata[4]; var misce_amount=spdata[5]; var other_amount=spdata[6]; var comm_amount=spdata[7]; var fob_amount=spdata[8]; var cm_amount=spdata[9]; var rmg_ratio=spdata[10];
				
				if(qcCons_from==1)
				{
					var bomTotCost=(number_format(fab_amount,2)*1)+(number_format(sp_oparation_amount,2)*1)+(number_format(wash_amount,2)*1)+(number_format(acc_amount,2)*1)+(number_format(comml_amount,2)*1)+(number_format(fright_amount,2)*1)+(number_format(lab_amount,2)*1)+(number_format(misce_amount,2)*1)+(number_format(other_amount,2)*1)+(number_format(comm_amount,2)*1);
					
					var bomCost='Fab='+fab_amount+'+ SP='+sp_oparation_amount+'+ Wash='+wash_amount+'+ ACC='+acc_amount+'+ Comml='+comml_amount+'+ FR='+fright_amount+'+ Lab='+lab_amount+'+ Mis='+misce_amount+'+ Others='+other_amount+'+ Comm='+comm_amount+'= Total='+bomTotCost;
		...
		function fnc_total_calculate()
		{
			...
			var split_tmep_id=temp_id.split(',');
			var ab=0;
			var qc_fab_amt=0; var qc_sp_amt=0; var qc_wash_amt=0; var qc_acc_amt=0; var qc_comml_amt=0; var qc_fri_amt=0; var qc_lab_amt=0; var qc_misce_amt=0; var qc_other_amt=0; var qc_comm_amt=0; var qc_fob_amt=0; var qc_cpm_amt=0; var qc_smv_amt=0; var qc_cm_amt=0; var qc_rmg_amt=0;
			for(j=1; j<=split_tmep_id.length; j++)
			{
				...
				item_tot_amount=($("#txtFabCst_"+itm_id).val()*1)+($("#txtSpOpa_"+itm_id).val()*1)+($("#txtWash_"+itm_id).val()*1)+($("#txtAcc_"+itm_id).val()*1)+($("#txtComml_"+itm_id).val()*1)+($("#txtFrightCst_"+itm_id).val()*1)+($("#txtLabCst_"+itm_id).val()*1)+($("#txtMiscCst_"+itm_id).val()*1)+($("#txtOtherCst_"+itm_id).val()*1)+($("#txtCommCst_"+itm_id).val()*1);
				
				var itemCost='Fab='+$("#txtFabCst_"+itm_id).val()+'+ SP='+$("#txtSpOpa_"+itm_id).val()+'+ Wash='+$("#txtWash_"+itm_id).val()+'+ ACC='+$("#txtAcc_"+itm_id).val()+'+ Comml='+$("#txtComml_"+itm_id).val()+'+ FR='+$("#txtFrightCst_"+itm_id).val()+'+ Lab='+$("#txtLabCst_"+itm_id).val()+'+ Mis='+$("#txtMiscCst_"+itm_id).val()+'+ Others='+$("#txtOtherCst_"+itm_id).val()+'+ Comm='+$("#txtCommCst_"+itm_id).val()+'= Total='+item_tot_amount;
			...
			$("#txtComml_qc").val( number_format(qc_comml_amt,4,'.','') );
		...
		function openmypage_unapprove_request()
		{
			...
			var page_link = 'quick_costing_v3_controller.php?data='+data+'&action=unapp_request_popup';
		...
		function fncqclimit(str)
		{
			...
			var tdwashamt=$('#tdwashamt'+item_id).text()*1;
			var tdaccamt=$('#tdaccamt'+item_id).text()*1;
			var tdcommlamt=$('#tdcomml'+item_id).text()*1;
			...
			if(qcCons_from==2)
			{
				if( validatetype==1 && tdwashamt < ($("#txtWash_"+item_id).val()*1) )
				{
					alert("QC BOM Limit (Wash Amt.) is Greater Than QC!!!");
					$("#txtWash_"+item_id).val(tdwashamt);
					//return;
				}
				if( validatetype==2 && tdcommlamt < ($("#txtComml_"+item_id).val()*1) )
				{
					alert("QC BOM Limit (Commercial Amt.) is Greater Than QC!!!");
					$("#txtComml_"+item_id).val(tdcommlamt);
					//return;
				}
    ...
    <div style="display:none"><? echo load_freeze_divs ("../../../",$permission);  ?></div>       
        <form name="confirmStyle_1" id="confirmStyle_1" autocomplete="off">
        	<table width="850">
                <tr>
                    ...
                    <td width="120"><? echo create_drop_down( "cbo_company_name", 120, "select id, company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in (3) $company_cond order by company_name","id,company_name", 1, "--Select Company--", $company_id, "",1); 
							?> </td>
                    ...
                    <td width="120"><? echo create_drop_down( "cbo_buyer_id", 120, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name", 1, "-Select Buyer-", $sql_data[0][csf('buyer_id')], "load_drop_down( 'quick_costing_v3_controller', this.value, 'load_drop_down_season_conf', 'season_conf_td'); load_drop_down( 'quick_costing_v3_controller',this.value, 'load_drop_down_sub_depConf', 'subConf_td' );",1 ); ?></td>
			...
			<?
            $sql_item_summ="select item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, commercial_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs from qc_item_cost_summary where mst_id='$qc_no' and status_active=1 and is_deleted=0";
            ...
            foreach($sql_result_item_summ as $rowItemSumm)
            {
                ...
                <tr id="trVal_<? echo $z; ?>" bgcolor="<? echo $bgcolor; ?>">
                    ...
                    <td align="right" id="tdwashamt<?=$item_id; ?>"><? echo number_format($rowItemSumm[csf("wash_cost")],4); ?></td>
                    <td align="right" id="tdaccamt<?=$item_id; ?>"><? echo number_format($rowItemSumm[csf("accessories_cost")],4); ?></td>
                    <td align="right" id="tdcomml<?=$item_id; ?>"><? echo number_format($rowItemSumm[csf("commercial_cost")],4); ?></td>
                ...
                <tr id="tr_<? echo $z; ?>" bgcolor="<? echo $bgcolorN; ?>">
                    ...
                    <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtWash_<?=$item_id; ?>" id="txtWash_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("wash_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_1'; ?>');" <?=$disable; ?> /></td>
                    ...
                    <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtComml_<?=$item_id; ?>" id="txtComml_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("commercial_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_2'; ?>');" <?=$disable; ?> /></td>
			...
			$sql="select fab_amount, sp_oparation_amount, acc_amount, fright_amount, lab_amount, misce_amount, other_amount, comm_amount, fob_amount, cm_amount, rmg_ratio from qc_confirm_dtls where cost_sheet_id='$cost_sheet_id' and status_active=1 and is_deleted=0";
        ...
        <table width="925" cellspacing="0" border="1" rules="all" class="rpt_table">
        	<tr id="tr_qc" bgcolor="#CCFFCC">
                ...
                <td width="50"><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtComml_qc" id="txtComml_qc" value="" disabled /></td>
        	...
        	<tr id="tr_bom" bgcolor="#CCCCCC">
                ...
                <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtComml_bom" id="txtComml_bom" value="" readonly /></td>
    ...
    <script>get_php_form_data($('#txt_costSheet_id').val()+'__'+$('#txtItem_id').val(),'populate_confirm_style_form_data','quick_costing_v3_controller'); fnc_bom_data_load(); fnc_total_calculate(); fnc_select();</script>          
    <script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
    </html>
    <?
	exit();	
...
if($action=="save_update_delete_confirm_style")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$dtls_field_arr="id, mst_id, cost_sheet_id, item_id, fab_amount, sp_oparation_amount, wash_amount, acc_amount, commercial_cost, fright_amount, lab_amount, misce_amount, other_amount, comm_amount, fob_amount, cppm_amount, smv_amount, cm_amount, rmg_ratio, inserted_by, insert_date";
		//echo "10**";
		...
		for($i=1; $i<=count($itm_id); $i++)
		{
			...
			if ($add_comma!=0) $dtls_data_arr .=",";
			$dtls_data_arr .="(".$confirm_dtls_id.",".$confirm_mst_id.",".$txt_costSheet_id.",".$$txtitemid.",".$$txtFabCst.",".$$txtSpOpa.",".$$txtWash.",".$$txtAcc.",".$$txtComml.",".$$txtFrightCst.",".$$txtLabCst.", ".$$txtMiscCst.", ".$$txtOtherCst.", ".$$txtCommCst.", ".trim($$txtFobDzn).",".$$txtCpm.",".$$txtSmv.",".$$txtCmCst.",".$$txtPack.",".$user_id.",'".$pc_date_time."')";
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		$dtls_field_arr="fab_amount*sp_oparation_amount*wash_amount*acc_amount*commercial_cost*fright_amount*lab_amount*misce_amount*other_amount*comm_amount*fob_amount*cppm_amount*smv_amount*cm_amount*rmg_ratio*updated_by*update_date";
		//echo "10**";
		$itm_id=explode(",",str_replace("'","",$txtItem_id));
		//print_r($itm_id);
		//echo count($itm_id);
		...
		for($i=1; $i<=count($itm_id); $i++)
		{
			...
			$dtls_data_arr[str_replace("'",'',$$txtdtlsupid)] =explode("*",("".$$txtFabCst."*".$$txtSpOpa."*".$$txtWash."*".$$txtAcc."*".$$txtComml."*".$$txtFrightCst."*".$$txtLabCst."*".$$txtMiscCst."*".$$txtOtherCst."*".$$txtCommCst."*".$$txtFobDzn."*".$$txtCpm."*".$$txtSmv."*".$$txtCmCst."*".$$txtPack."*'".$user_id."'*'".$pc_date_time."'"));
			$k++;
		}
		$flag=1;
	...
	else if ($operation==2) // Delete Here----------------------------------------------------------
	{
		...
		$joborbom=sql_select("select a.job_no from wo_po_details_master a, qc_mst b, wo_pre_cost_mst c where a.quotation_id=b.qc_no and b.qc_no=$txt_costSheet_id and a.id=c.job_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0");
...
if($action=="populate_confirm_style_form_data")
{
	...
	$sql_confirm_mst=sql_select("select id, cost_sheet_id, lib_item_id, confirm_style, confirm_order_qty, confirm_fob, deal_merchant, ship_date, ready_to_approve, job_id, approved from qc_confirm_mst where cost_sheet_id ='$ex_data[0]' and status_active=1 and is_deleted=0") ;
	...
	$sql_confirm_dtls=sql_select("select id, mst_id, cost_sheet_id, item_id, fab_amount, sp_oparation_amount, wash_amount, acc_amount, commercial_cost, fright_amount, lab_amount, misce_amount, other_amount, comm_amount, fob_amount, cppm_amount, smv_amount, cm_amount, rmg_ratio from qc_confirm_dtls where cost_sheet_id='$ex_data[0]' and status_active=1 and is_deleted=0");
	foreach($sql_confirm_dtls as $row_dtls)
	{
		...
		echo "$('#txtComml_".$row_dtls[csf("item_id")]."').val('".$row_dtls[csf("commercial_cost")]."');\n";
...
if($action=="budgete_cost_validate")
{
	...
	if($job_no!="")
	{
		...
		$dataArr=sql_select($sql);
		$fab_amount=$sp_oparation_amount=$wash_amount=$acc_amount=$comml_amount=$fright_amount=$lab_amount=$misce_amount=$other_amount=$comm_amount=$fob_amount=$cm_amount=$rmg_ratio=0;
		...
		foreach($dataArr as $row)
		{
			...
			$comml_amount+=$row[csf("comm_cost")];
		...
		$str_data=$fab_amount."##".$sp_oparation_amount."##".$acc_amount."##".$fright_amount."##".$lab_amount."##".$misce_amount."##".$other_amount."##".$comm_amount."##".$fob_amount."##".$cm_amount."##".$rmg_ratio."##".$wash_amount."##".$comml_amount;
		unset($dataArr);
	}
        ...
        <table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
            ...
            <tr class="general">
                <td> 
					...
					<? echo create_drop_down( "cbo_company_mst", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-Select Company-", $cbo_company_name,"load_drop_down( 'quick_costing_v3_controller', this.value, 'load_drop_down_buyer_pop', 'buyer_pop_td' );",'' ); ?>
                </td>
		...
		<fieldset>
            <table cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr>
                        <td align="center">
                        	 <? echo create_drop_down( "cbo_buyer_id", 120, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name",1, "-- All Buyer--",$selected,"load_drop_down( 'quick_costing_v3_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'quick_costing_v3_controller',this.value, 'load_drop_down_sub_dep', 'sub_td' );",0 ); ?>
                        </td>                 
                        <td id="season_td"><? echo create_drop_down( "cbo_season_id", 130, $blank_array,'', 1, "-- Select Season--",$selected, "" ); ?></td>
                        <td id="sub_td"><? echo create_drop_down( "cbo_subDept_id", 130, $blank_array,'', 1, "-- Select Dept--",$selected, "" ); ?></td>     
...
if($action=="agent_location_popup")
{
		...
		function fnc_agent_location_reponse()
		{
			if(http.readyState == 4) 
			{
				...
				if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
				{
					...
					show_list_view(type,'agent_location_data_list','agent_location_data_div','quick_costing_v3_controller','');
				}
    ...
    <script>show_list_view(type,'agent_location_data_list','agent_location_data_div','quick_costing_v3_controller','');</script>          
    <script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
    </html>
    <?
	exit();	
	...
	?>
		...
		<table width="1250">
			<tr>
				...
				<td width="170"><?=$season_name_arr[$master_data[0][csf('season_id')]]; ?></td>
			...
			<tr>
				...
				<td>USD</td>
                <td>Item : </td>
				<td><?=$template_name_arr[$master_data[0][csf('temp_id')]]; ?></td>
                <td>Costing Per : </td>
				<td>1 PCS</td>
                <td>&nbsp;</td>
				<td>&nbsp;</td>
		...
		<table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:1250px; margin-top: 10px" rules="all">
			<thead>
				...
				<th width="50">Mergin/ 1 PCS</th>
			</thead>
			<? 
			
			$consSql="select id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, rate, value, fab_id, description, use_for, fc_per, body_part_id, supp_ref from qc_cons_rate_dtls where mst_id='$qc_no' and status_active=1 and is_deleted=0 order by id ASC";
			$consSqlData=sql_select($consSql); $cons_rate_fab_arr=array(); $EmbData=array(); $TrimData=array();
			foreach ($consSqlData as $row)
			{
				...
				if($row[csf("type")]==3)
				{
					/*$cons_rate_wash_arr[$row[csf("item_id")]].=$emblishment_wash_type[$row[csf("particular_type_id")]].'_'.$row[csf("particular_type_id")].'_'.$body_part[$row[csf("body_part_id")]].'_'.$row[csf("body_part_id")].'_'.$row[csf("consumption")].'_'.$row[csf("ex_percent")].'_'.$row[csf("tot_cons")].'_'.$row[csf("rate")].'_'.$row[csf("value")].'##';*/
					if($row[csf("tot_cons")]>0)
					{
				...
				if($row[csf("type")]==4)
				{
					//$cons_rate_ac_arr[$row[csf("item_id")]].=$itemGroupArr[$row[csf("particular_type_id")]].'_'.$row[csf("particular_type_id")].'_'.$row[csf("description")].'_'.$row[csf("supp_ref")].'_'.$row[csf("consumption")].'_'.$row[csf("ex_percent")].'_'.$row[csf("tot_cons")].'_'.$row[csf("rate")].'_'.$row[csf("value")].'##';
					if($row[csf("tot_cons")]>0)
					{
			...
			foreach ($cons_rate_fab_arr as $fabric_des=>$useForData) 
			{
				foreach($useForData as $useFor=>$val) 
				{
					...
					<tr>
						...
						<? if($k==1){ ?>
						<td width="60" rowspan="<?=$colSpan ?>" align="right"><?=number_format($cm_cost,4); ?></td>
						<td width="60" rowspan="<?=$colSpan ?>" align="right"><?=number_format($trims_cost,4); ?></td>
						<td width="60" rowspan="<?=$colSpan ?>" align="right"><?=number_format($wash_cost,4); ?></td>
						<td width="60" rowspan="<?=$colSpan ?>" align="right"><?=number_format($embel_cost,4); ?></td>
						<td width="60" rowspan="<?=$colSpan ?>" align="right"><?=number_format($total_other_cost,4); ?></td>
						<td width="60" rowspan="<?=$colSpan ?>" align="right"><?=number_format($total_cost,4); ?></td>
						<td width="50" rowspan="<?=$colSpan ?>" align="right"><?=number_format($price_dzn,4); ?></td>
						<td width="50" rowspan="<?=$colSpan ?>" align="right"><?=number_format($price_dzn-$total_cost,4); ?></td>
						<? } ?>
					</tr><?
					$k++;
					$total_fabric_total += number_format($val['val'],4);
				}
			}
			 ?>
			 <tr>
			 	<th colspan="7" align="center">Total Fabrics Cost</th>
			 	<th align="right"><?=number_format($total_fabric_total,4); ?></th>
			 </tr>
		</table>
				...
				<table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:400px;" rules="all">
					...
					<? foreach ($EmbData as $index => $row) { ?>						
						<tr>
							...
							<td width="40"></td>
							<td width="40"></td>
							<td width="40"><?=number_format($row["amount"],4); ?></td>
						</tr>						
					<?
		...
		<table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:950px; margin-top: 10px; margin-bottom: 10px" rules="all">
			<tr>
				<th rowspan="1" width="200">Cost Summery</th>
				<th colspan="8">Cost Detials</th>
			</tr>
			<tr>
				<td>Avg Price/ Pcs</td>
				<td>Fabric Cost</td>
			...
			<tr>
				<? $total_cost = $fabric_req_amount + $cm_cost_amount + $totTrimAmount + $totWashAmount + $embel_cost + $other_cost_value; ?>
				<td align="right"><? echo number_format($price_dzn,4,'.','') ?></td>
				<td align="right"><? echo number_format($fabric_req_amount,4,'.','') ?></td>
				<td align="right"><? echo number_format($cm_cost_amount,4,'.','') ?></td>
				<td align="right"><? echo number_format($totTrimAmount,4,'.',''); ?></td>
				<td align="right"><? echo number_format($totWashAmount,4,'.','') ?></td>
				<td align="right"><? echo number_format($embel_cost,4,'.','')  ?></td>
				<td align="right"><? echo number_format($other_cost_value,4,'.',''); ?></td>
				<td align="right"><? echo number_format($total_cost,4,'.','') ?></td>
				<td align="right"><? echo number_format($total_price-$total_cost,4,'.','') ?></td>
			</tr>
		</table>
		<br>
...
if($action=="quick_costing_print_p2")
{
	extract($_REQUEST);
	echo load_html_head_contents("Quick Costing Print", "../../../", 1, 1,'','','');
	$data=explode('*',$data);
	...
	$sql_mst="select id, cost_sheet_id, cost_sheet_no, temp_id, lib_item_id, style_ref, buyer_id, uom, cons_basis, season_id, style_des, department_id, delivery_date, exchange_rate, offer_qty, quoted_price, tgt_price, stage_id, costing_date, revise_no, option_id, option_remarks, buyer_remarks, inquery_id, season_year, prod_dept, brand_id, article, currency_id, margin_per, margin, approved from qc_mst where status_active=1 and is_deleted=0 and qc_no='$data[0]'";
	$sqlMst_res=sql_select($sql_mst);
	$sql_summary="select tot_fob_cost, tot_fab_cost, tot_accessories_cost, tot_lab_test_cost, tot_cm_cost, tot_fright_cost, tot_other_cost, commercial_cost, tot_commission_cost, (tot_wash_cost+tot_sp_operation_cost) as tot_emb_cost from qc_tot_cost_summary where status_active=1 and is_deleted=0 and mst_id='$data[0]'";
	$res_summary=sql_select($sql_summary);

	$tot_fob_cost=$res_summary[0][csf('tot_fob_cost')];
	$qc_mst_id=$sqlMst_res[0][csf('id')];
	$inquery_id=$sqlMst_res[0][csf('inquery_id')];
	$costing_date=$sqlMst_res[0][csf('costing_date')];
	$margin_per=$sqlMst_res[0][csf('margin_per')];
	$margin=$sqlMst_res[0][csf('margin')];
	$approved=$sqlMst_res[0][csf('approved')];
	$approvedstr="";
	if($approved==1) $approvedstr="Approved"; else if($approved==3) $approvedstr="Partial Approved"; else $approvedstr="Un-Approved";

	$item_name='';
	foreach ($sqlMst_res[0][csf('lib_item_id')] as $key => $item_id) {
		$item_name.=$lib_temp_arr[$item_id]."***";
	}

	$sql_item_summary="select * from qc_item_cost_summary where status_active=1 and is_deleted=0 and mst_id='$data[0]'";
	$res_item_summary=sql_select($sql_item_summary);
	$total_fob=$tot_fob_cost;
	// foreach ($res_item_summary as $row) {
	// 	$total_fob+=($row[csf('FOB_PCS')]/12);
	// }
	...
	/* $company_name_str=sql_select("SELECT b.company_name,b.id from wo_quotation_inquery a,lib_company b where a.id=$inquery_id and a.company_id=b.id");
	$company_name=$company_name_str[0][csf("id")]; */
	$company_name=$data[3];
	
	$sql_item="select sum(cpm) as cpm,sum(smv) as smv,sum(efficiency) as efficiency,sum(operating_exp) as operating_exp,sum(storage_cost) as storage_cost from QC_ITEM_COST_SUMMARY where status_active=1 and is_deleted=0 and mst_id=$data[0]";
	$res_item=sql_select($sql_item);
	?>
    <style>
		.watermark {
			opacity: 0.5;
    ...
    <div style="width:1000px;">
    	<table style="width: 1000px;">
    		<tr>
    			<td width="20%">
    				<img  src='../../../<? echo $imge_arr[$company_name]; ?>' height='100' width='150' />
    			</td>
    			<td width="60%" style="justify-content: center;text-align: center;">
    				<h2 style="justify-content: center;text-align: center;"><?=$company_arr[$company_name];?></h2>
    				<h3 style="justify-content: center;text-align: center;">Quotation/Buyer Costing</h3>
    				<h4 style="justify-content: center;text-align: center;">Costing Date: <?=change_date_format($costing_date)?></h4>
    			</td>
    			<td width="20%"><b style="color:#F00; font-size:20px"><?=$approvedstr; ?></b></td>
    		</tr>
    	</table>
        <table width="1000" cellspacing="0" border="1"  rules="all" class="rpt_table">
           
            <tr>
            	<td width="100"><strong>Quot. Inq. :</strong></td><td width="145px"><? echo $inquery_id; ?></td>
                <td width="100"><strong>Buyer :</strong></td><td width="145px"><? echo $buyer_arr[$sqlMst_res[0][csf('buyer_id')]]; ?></td>
                <td width="100"><strong>Cost Sheet No :</strong></td><td width="145px"><? echo $sqlMst_res[0][csf('cost_sheet_no')]; ?></td>
                <td width="100"><strong>Delivery Date :</strong></td><td width="145px"><? echo change_date_format($sqlMst_res[0][csf('delivery_date')]); ?></td>
            </tr>
            <tr>
            	<td width="100"><strong>Garments Item :</strong></td><td width="145px"><? echo $template_name_arr[$sqlMst_res[0][csf('temp_id')]]; //implode(", ", array_unique(explode("***", chop($item_name,"***"))));; ?></td>
                <td width="100"><strong>Style Ref. :</strong></td><td width="145px"><? echo $sqlMst_res[0][csf('style_ref')]; ?></td>
                <td width="100"><strong>Season :</strong></td><td width="145px"><? echo $season_arr[$sqlMst_res[0][csf('season_id')]]; ?></td>
                <td width="100"><strong>Season Year :</strong></td><td width="145px"><? echo $sqlMst_res[0][csf('season_year')]; ?></td>
            </tr>
            <tr>
            	<td width="100"><strong>Style Description :</strong></td><td width="145px"><? echo $sqlMst_res[0][csf('style_des')]; ?></td>
                <td width="100"><strong>Product Dept. :</strong></td><td width="145px"><? echo $product_dept[$sqlMst_res[0][csf('prod_dept')]]; ?></td>
                <td width="100"><strong>Sub. Dept. :</strong></td><td width="145px"><? echo $department_arr[$sqlMst_res[0][csf('department_id')]]; ?></td>
                <td width="100"><strong>Brand :</strong></td><td width="145px"><? echo $brand_arr[$sqlMst_res[0][csf('brand_id')]]; ?></td>
            </tr>
            <tr>
            	<td width="100"><strong>Article :</strong></td><td width="145px"><? echo $sqlMst_res[0][csf('article')]; ?></td>
                <td width="100"><strong>Offer Qty :</strong></td><td width="145px"><? echo number_format($sqlMst_res[0][csf('offer_qty')],4,'.',''); ?></td>
                <td width="100"><strong>FOB/Pcs:</strong></td><td width="145px"><? echo number_format($tot_fob_cost,4,".",","); ?></td>
                <td width="100"><strong>Currency/Exc. Rate:</strong></td><td width="145px"><span style="border-right: 1px solid black;margin-left: 20px;"><? echo $currency[$sqlMst_res[0][csf('currency_id')]]; ?></span><span ><? echo number_format($sqlMst_res[0][csf('exchange_rate')],3,'.',''); ?></span></td>
            </tr>
        </table>
        <table width="1000">
        	<caption style="justify-content: left;text-align: left;"><strong>Cost Summary</strong></caption>
        	<tr>
        		<td>
        			<table width="600" cellspacing="0" border="1"  rules="all" class="rpt_table">
        				...
        				<tbody>
        					<tr>
        						...
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_fab_cost')],4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_fab_cost')]/12,4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_fab_cost')],4,".",","); ?></td>
        					</tr>
        					<tr>
        						<td>Total Accessories Cost</td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_accessories_cost')],4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_accessories_cost')]/12,4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_accessories_cost')],4,".",","); ?></td>
        					</tr>
        					<tr>
        						
        						<td>Total Embellishment Cost</td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_emb_cost')],4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_emb_cost')]/12,4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format(($res_summary[0][csf('tot_emb_cost')]*100)/($total_fob*12),4,".",","); ?></td>
        					</tr>
        					<tr>
        						<td>Lab Test</td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_lab_test_cost')],4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_lab_test_cost')]/12,4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format(($res_summary[0][csf('tot_lab_test_cost')]*100)/($total_fob*12),4,".",","); ?></td>
        					</tr>
        					<tr>
        						<td>Storage Cost</td>
        						<td align="right"><?php echo fn_number_format($res_item[0][csf('storage_cost')],4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format($res_item[0][csf('storage_cost')]/12,4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format(($res_item[0][csf('storage_cost')]*100)/($total_fob*12),4,".",","); ?></td>
        					</tr>
        					<tr>
        						<td>Operating Expense</td>
        						<td align="right"><?php echo fn_number_format($res_item[0][csf('operating_exp')],4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format($res_item[0][csf('operating_exp')]/12,4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format(($res_item[0][csf('operating_exp')]*100)/($total_fob*12),4,".",","); ?></td>
        					</tr>
        					<tr>
        						<td>CM Cost</td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_cm_cost')],4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_cm_cost')]/12,4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format(($res_summary[0][csf('tot_cm_cost')]*100)/($total_fob*12),4,".",","); ?></td>
        					</tr>
        					<tr>
        						<td>Frieght Cost</td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_fright_cost')],4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_fright_cost')]/12,4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format(($res_summary[0][csf('tot_fright_cost')]*100)/($total_fob*12),4,".",","); ?></td>
        					</tr>
        					<tr>
        						<td>Other Cost</td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_other_cost')],4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_other_cost')]/12,4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format(($res_summary[0][csf('tot_other_cost')]*100)/($total_fob*12),4,".",","); ?></td>
        					</tr>
        					<tr>
        						<td>Commercial Cost</td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('commercial_cost')],4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('commercial_cost')]/12,4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format(($res_summary[0][csf('commercial_cost')]*100)/($total_fob*12),4,".",","); ?></td>
        					</tr>
        					<tr>
        						<td>Commission</td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_commission_cost')],4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format($res_summary[0][csf('tot_commission_cost')]/12,4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format(($res_summary[0][csf('tot_commission_cost')]*100)/($total_fob*12),4,".",","); ?></td>
        					</tr>
        					<tr>
        						<td>Margin/Profit</td>
        						<td align="right"><?php echo fn_number_format($margin*12,4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format($margin,4,".",","); ?></td>
        						<td align="right" title="FOB: <?=$total_fob?> "><?php echo fn_number_format(($margin*100)/($total_fob),4,".",","); ?></td>
        					</tr>
        				</tbody>
        				<tfoot>
        					<tr>
        						<td>FOB $</td>
        						<?php 
        						$fob_summary=$res_summary[0][csf('tot_fab_cost')]+$res_summary[0][csf('tot_accessories_cost')]+$res_summary[0][csf('tot_emb_cost')]+$res_summary[0][csf('tot_lab_test_cost')]+$res_summary[0][csf('tot_cm_cost')]+$res_summary[0][csf('tot_fright_cost')]+$res_summary[0][csf('tot_other_cost')]+$res_summary[0][csf('commercial_cost')]+$res_summary[0][csf('tot_commission_cost')]+($margin*12)+$res_item[0][csf('storage_cost')]+$res_item[0][csf('operating_exp')];
        						 ?>
        						<td align="right"><?php echo fn_number_format($fob_summary,4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format(($fob_summary)/12,4,".",","); ?></td>
        						<td align="right"><?php echo fn_number_format(($fob_summary*100)/($total_fob*12),0,".",","); ?></td>
        					</tr>
        				</tfoot>
        			</table>
        		</td>
        		<td style="border: 1px solid black;display: flex;">
        			<?php $sql_img=sql_select("select image_location from common_photo_library where master_tble_id='$data[0]' and form_name='short_quotation_v2' and file_type=1");
        				foreach ($sql_img as $s_img) 
        				{
        					?><img src="../../../<?=$s_img[csf('image_location')]; ?>" height="120"><?
        				}
        			 ?>
        		</td>
        	</tr>
       	...
       	<table width="1000" cellspacing="0" border="1"  rules="all" class="rpt_table">
       		<caption  style="justify-content: left;text-align: left;"><strong>Fabric Details</strong></caption>
       		<thead>
       			<tr>
       				<th>Gmts. Item</th>
       				<th width="100">Body Part</th>
       				<th width="170">Fab. Des.</th>
       				<th>Color Type</th>
       				<th>Weight /GSM</th>
       				<th>UOM</th>
       				<th>Cons/Dzn</th>
       		...
       		<tbody>
       			<?
				$sql_f=sql_select("SELECT  id,item_id,consumption,rate,value,tot_cons,body_part_id,remarks,fab_cost,totfab_cost,ex_percent from qc_cons_rate_dtls where mst_id = '$data[0]' and  body_part_id>0 and type=1 ");
				$fabric_data=array();
				$fabric_id_arr=array();
				foreach ($sql_f as $row) 
				{
					$fabric_data[$row[csf('id')]]['item_id']=$row[csf('item_id')];
					$fabric_data[$row[csf('id')]]['consumption']=$row[csf('consumption')];
					$fabric_data[$row[csf('id')]]['rate']=$row[csf('rate')];
					$fabric_data[$row[csf('id')]]['value']=$row[csf('value')];
					$fabric_data[$row[csf('id')]]['tot_cons']=$row[csf('tot_cons')];
					$fabric_data[$row[csf('id')]]['body_part_id']=$row[csf('body_part_id')];
					$fabric_data[$row[csf('id')]]['remarks']=$row[csf('remarks')];
					$fabric_data[$row[csf('id')]]['fab_cost']=$row[csf('fab_cost')];
					$fabric_data[$row[csf('id')]]['totfab_cost']=$row[csf('totfab_cost')];
					$fabric_data[$row[csf('id')]]['ex_percent']=$row[csf('ex_percent')];
					array_push($fabric_id_arr, $row[csf('id')]);
				}
				$cons_rate_dtls_id=where_con_using_array($fabric_id_arr,0,"cons_rate_dtls_id");
				$sqlFab="SELECT cons_rate_dtls_id,yarn_lib_id,fab_des,yarn_break_down,cons,tot_cons,rate,value,conv_break_down,gsm,dia,uom,color_type_id,nominated_supp_multi,gauge from qc_fab_yarn_conv where qc_no='$data[0]'  and status_active=1 and is_deleted=0  $cons_rate_dtls_id";
								// echo $sqlFab;
				$res_comp=sql_select($sqlFab);
				$fab_det_data=array();
				foreach ($res_comp as $row) {
					$fab_det_data[$row[csf('cons_rate_dtls_id')]]['fab_des'].=$row[csf('fab_des')]."***";
					$fab_det_data[$row[csf('cons_rate_dtls_id')]]['yarn_count'].=$yarnCountArr[$row[csf('count')]]."***";
					$fab_det_data[$row[csf('cons_rate_dtls_id')]]['color_type'].=$color_type[$row[csf('color_type_id')]]."***";
					$fab_det_data[$row[csf('cons_rate_dtls_id')]]['uom'].=$unit_of_measurement[$row[csf('uom')]]."***";
					$fab_det_data[$row[csf('cons_rate_dtls_id')]]['nominated_supp_multi'].=$lib_supplier[$row[csf('nominated_supp_multi')]]."***";
					$fab_det_data[$row[csf('cons_rate_dtls_id')]]['gsm'].=$row[csf('gsm')]."***";
					$fab_det_data[$row[csf('cons_rate_dtls_id')]]['gauge'].=$row[csf('gauge')]."***";
				}

			    $fab_total_cost=0;
			    	
			    foreach ($fabric_data as $cons_rate_dtls_id => $fab)
			    {
			    	$fab_total_cost+=$fab['value'];		 	
	       			?>
	       			<tr>
	       				<td><?=$lib_temp_arr[$fab['item_id']];?></td>
	       				<td style="word-break: break-all;"><?=$body_part[$fab['body_part_id']];?></td>
	       				<td style="word-break: break-all;"><?=implode(",", array_unique(explode("***", chop($fab_det_data[$cons_rate_dtls_id]['fab_des'],"***"))));?></td>
	       				<td><?=implode(",", array_unique(explode("***", chop($fab_det_data[$cons_rate_dtls_id]['color_type'],"***"))));?></td>
	       				<td><?=implode(",", array_unique(explode("***", chop($fab_det_data[$cons_rate_dtls_id]['gsm'],"***"))));?></td>
	       				<td ><?=implode(",", array_unique(explode("***", chop($fab_det_data[$cons_rate_dtls_id]['uom'],"***"))));?></td>
	       				<td align="right"><?=fn_number_format($fab['consumption'],4,".",",")?></td>
	       				<td align="right"><?=fn_number_format($fab['ex_percent'],0,".",",")?></td>
	       				<td align="right"><?=fn_number_format($fab['tot_cons'],4,".",",")?></td>
	       				<td align="right"><?=fn_number_format($fab['rate'],4,".",",")?></td>
	       				<td align="right"><?=fn_number_format($fab['value'],4,".",",")?></td>
	       				<td align="right"><?=fn_number_format($fab['tot_cons']/12,4,".",",")?></td>
	       				<td align="right"><?=fn_number_format($fab['value']/12,4,".",",")?></td>
	       				<td style="word-break: break-all;"><?=$fab['remarks']?></td>
	       			</tr>
	       		  <?php 
       			}
       			?>
       		...
       		<tfoot>
       			<tr>
       				<td align="right" colspan="10"><strong>Total Fabric Cost:</strong></td>
       				<td align="right"><strong><?=fn_number_format($fab_total_cost,4,".",",")?></strong></td>
       				<td colspan="3"></td>
       			</tr>
       		</tfoot>
       	</table>
       	...
       	<table width="1000" cellspacing="0" border="1"  rules="all" class="rpt_table">
       		<caption style="justify-content: left;text-align: left;"><strong>Yarn Cost Details</strong></caption>
       		<thead>
       			<tr>
       				<th  width="300">Body Part</th>
       				<th width="270">Yarn Description</th>
       				<th >UOM</th>
       				<th >Cons/Dzn </th>
       				<th> Rate $ /Kg</th>
       				<th>Amount $</th>
       		...
       		<tbody>
       			<?
				$sql_wash=sql_select("SELECT particular_type_id,sum(value) as value,remarks FROM qc_cons_rate_dtls WHERE mst_id = '$data[0]' and type=3 and item_id=$item_id group by particular_type_id,remarks");
				$fabric_data=array();

				$yarnCountArr=return_library_array("select id,yarn_count from lib_yarn_count where is_deleted=0 and status_active=1 order by yarn_count", "id","yarn_count");
			   
			    $sql_yarn="SELECT b.composition, b.count, b.yarn_type, a.body_part_id, b.cons_rate_dtls_id, SUM (b.value) AS value, SUM (b.cons) AS cons
				   FROM qc_cons_rate_dtls a, qc_fab_yarn_conv b
				   WHERE a.id = b.cons_rate_dtls_id AND b.qc_no = '$data[0]' AND b.status_active = 1 AND b.is_deleted = 0 AND a.status_active = 1 AND a.is_deleted = 0 AND a.mst_id = '$data[0]' AND a.body_part_id > 0 AND a.TYPE = 1 AND b.TYPE = 2 group by b.composition, b.count, b.yarn_type, a.body_part_id, b.cons_rate_dtls_id order by  b.cons_rate_dtls_id,a.body_part_id";		
				
				$yarn_data=sql_select($sql_yarn);
       			$total_yarn_amount=0;
       				
			    foreach ($yarn_data as $yarn)
			    {
			    	$yarn_des=$yarnCountArr[$yarn[csf('count')]].",".$composition[$yarn[csf('composition')]]. ",". $yarn_type[$yarn[csf('yarn_type')]];
			    	$total_yarn_amount+=$yarn[csf('value')];	
	       			?>
	       			<tr>
	       				<td><?=$body_part[$yarn[csf('body_part_id')]];?></td>
	       				<td style="word-break: break-all;"><?=$yarn_des;?></td>
	       				<td style="word-break: break-all;">Kg</td>
	       				<td align="right"><?=fn_number_format($yarn[csf('cons')],4,".",",")?></td>
	       				<td align="right"><?=fn_number_format($yarn[csf('value')]/$yarn[csf('cons')],4,".",",");?></td>
	       				<td align="right"><?=fn_number_format($yarn[csf('value')],4,".",",")?></td>
	       			</tr>
	       		  <?php 
       			}
       			?>
       		...
       		<tfoot>
       			<tr>
       				...
       				<td align="right"><strong><?=fn_number_format($total_yarn_amount,4,".",",")?></strong></td>
       			</tr>
       		</tfoot>
       	</table>
       
        <table width="1000" cellspacing="0" border="1"  rules="all" class="rpt_table">
            <caption  style="justify-content: left;text-align: left;"><strong>Conversion Cost Details</strong></caption>
            <thead>
                <tr>
                    <th width="120">Gmts. Item</th>
                    <th width="270">Body Part</th>
                    <th width="300">Fab. Des.</th>
            ...
            <tbody>
				<?
                $sqldes="select id, type, yarn_lib_id,body_part_id, fab_des from qc_fab_yarn_conv where qc_no='$data[0]' and type=1 and status_active=1 and is_deleted=0";
                
                $sqldesData=sql_select($sqldes); $fab_description=array();
                foreach($sqldesData as $row)
                {
                    $fab_description[$row[csf("id")]]=$body_part[$body_part_id].', '.$row[csf("fab_des")];
                }
                
                $sqlFab="select body_part_id,id,cons_rate_dtls_id,id, type, yarn_lib_id, fab_des, count, composition, percent, yarn_type, process, fab_id, cons, exper, tot_cons, rate, value from qc_fab_yarn_conv where qc_no='$data[0]' and type=3 and status_active=1 and is_deleted=0";
                
                $sqlFabData=sql_select($sqlFab); $c=1; $convBtn=0;	
                
                $sql_conv="SELECT a.item_id, b.process, a.body_part_id, b.fab_id, b.cons_rate_dtls_id, SUM (b.value) AS value, SUM (b.cons) AS cons FROM qc_cons_rate_dtls a, qc_fab_yarn_conv b WHERE a.id = b.cons_rate_dtls_id AND b.qc_no = '$data[0]' AND b.status_active = 1 AND b.is_deleted = 0 AND a.status_active = 1 AND a.is_deleted = 0 AND a.mst_id = '$data[0]' AND a.body_part_id > 0 AND a.TYPE = 1 AND b.TYPE = 3 group by a.item_id, b.process, a.body_part_id, b.fab_id, b.cons_rate_dtls_id order by  b.cons_rate_dtls_id,a.body_part_id";		
                
                $res_conv=sql_select($sql_conv);
                
                $total_cons_amount=0;
                foreach ($res_conv as  $conv)
                {
                    $fab_des=$fab_description[$conv[csf("id")]] ;		 		
                    //$fab_des=$body_part[$conv[csf('body_part_id')]].', '.implode(", ", array_unique(explode("***", chop($fab_det_data[$conv[csf('cons_rate_dtls_id')]]['fab_des'],"***")))) ;		 		
                    $total_cons_amount+=$conv[csf('value')];			 	
                    ?>
                    <tr>
                        <td><?=$lib_temp_arr[$conv[csf('item_id')]];?></td>
                        <td><?=$body_part[$conv[csf('body_part_id')]];?></td>
                        <td><?=$fab_description[$conv[csf('fab_id')]];?></td>
                        <td style="word-break: break-all;"><?=$conversion_cost_head_array[$conv[csf('process')]];?></td>
                        <td align="right"><?=fn_number_format($conv[csf('cons')],4,".",",");?></td>
                        <td align="right"><?=fn_number_format($conv[csf('value')]/$conv[csf('cons')],4,".",",")?></td>
                        <td align="right"><?=fn_number_format($conv[csf('value')],4,".",",")?></td>
                    </tr>
                    <?php 
                }
                ?>
            ...
            <tfoot>
                <tr>
                    <td colspan="6" align="right"><strong>Total Conversion Cost:</strong></td>
                    <td align="right"><strong><?=fn_number_format($total_cons_amount,4,".",",")?></strong></td>
                </tr>
            </tfoot>
        </table>
        <table width="1000" cellspacing="0" border="1"  rules="all" class="rpt_table">
            <caption  style="justify-content: left;text-align: left;"><strong>Accessories Details</strong></caption>
            <thead>
                <tr>
                    <th>Gmts. Item</th>
            ...
            <tbody>
            <?
            $sql_em="SELECT  id as id,item_id as item_id,consumption as consumption,rate as rate,value as value,tot_cons as tot_cons,body_part_id as body_part_id,remarks as remarks,fab_cost as fab_cost,totfab_cost as totfab_cost,ex_percent as ex_percent,particular_type_id as particular_type_id,fab_id,description from qc_cons_rate_dtls where mst_id = '$data[0]' and  particular_type_id>0 and tot_cons>0 and type=4 ";
            $sql_emp=sql_select($sql_em);
            
            //$sql_tgroup=sql_select( "select id, item_name, trim_uom, trim_type from lib_item_group where item_category=4 and is_deleted=0 and status_active=1 order by item_name");
            $item_name=return_library_array( "select id, item_name from lib_item_group where item_category=4 and is_deleted=0 and status_active=1",'id','item_name');
            
            $accressories_cost=0; $accressories_cost_fob=0;
            
            foreach ($sql_emp as $row)
            {
				$accressories_cost+=$row[csf('value')];
				$accressories_cost_fob+=($row[csf('value')]*100)/($total_fob*12);	
				?>
				<tr>
                    <td><?=$lib_temp_arr[$row[csf('item_id')]];?></td>
                    <td style="word-break: break-all;"><?=$item_name[$row[csf('particular_type_id')]];?></td>
                    <td style="word-break: break-all;"><?=$row[csf('description')];?></td>
                    <td align="right"><?=fn_number_format($row[csf('consumption')],3,".",",")?></td>
                    <td align="right"><?=fn_number_format($row[csf('ex_percent')],0,".",",")?></td>
                    <td align="right"><?=fn_number_format($row[csf('tot_cons')],3,".",",")?></td>
                    <td align="right"><?=fn_number_format($row[csf('rate')],3,".",",")?></td>
                    <td align="right"><?=fn_number_format($row[csf('value')],3,".",",")?></td>
                    <td align="right"><?=fn_number_format($row[csf('consumption')]/12,3,".",",")?></td>
                    <td align="right"><?=fn_number_format($row[csf('value')]/12,3,".",",")?></td>
                    <td align="right"><?=fn_number_format(($row[csf('value')]*100)/($total_fob*12),3,".",",")?></td>
            ...
            <tfoot>
                <tr>
                    <td align="right" colspan="7"><strong>Total Fabric Cost:</strong></td>
                    <td align="right"><strong><?=fn_number_format($accressories_cost,4,".",",")?></strong></td>
                    <td align="right"></td>
                    <td align="right"><strong><?=fn_number_format($accressories_cost/12,4,".",",")?></strong></td>
                    <td align="right"><strong><?=fn_number_format($accressories_cost_fob,4,".",",")?></strong></td>
                    <td align="right"></td>
                </tr>
            </tfoot>
        </table>
       	<table width="1000" cellspacing="0" border="1"  rules="all" class="rpt_table">
       		<caption  style="justify-content: left;text-align: left;"><strong>Embellishment Name</strong></caption>
       		<thead>
       			<tr>
       				<th>Gmts. Item</th>
       				<th width="100">Embellishment Name</th>
       				<th width="170">Embellishment Type</th>
       				<th>Body Part</th>
       				<th>Cons/Dzn</th>
       				<th>Ex. %</th>
       				<th>Ttl Cons. [Dzn]</th>
       		...
       		<tbody>
       			<?
				$sql_em="SELECT  id as id,item_id as item_id,consumption as consumption,rate as rate,value as value,tot_cons as tot_cons,body_part_id as body_part_id,remarks as remarks,fab_cost as fab_cost,totfab_cost as totfab_cost,ex_percent as ex_percent,particular_type_id as particular_type_id,fab_id,type from qc_cons_rate_dtls where mst_id = '$data[0]' and tot_cons>0 and type in (2,3) and status_active=1 and is_deleted=0";
				$sql_emp=sql_select($sql_em);
				
				$lib_tgroup=return_library_array("select id, item_name from lib_item_group where  is_deleted=0 and status_active=1","id","item_name");
       			$emb_cost=0; $emb_cost_fob=0; $emb_cons=0;
			    ...
			    foreach ($sql_emp as $row)
			    {
			    	...
			    	if($row[csf('type')]==2)
			    	{
	    		    	if($row[csf('particular_type_id')]==1) {$emb_typearr=$emblishment_print_type;$em_name='Printing';}
	    				else if($row[csf('particular_type_id')]==2) {$emb_typearr=$emblishment_embroy_type;$em_name='Embroidery';}
	    				else if($row[csf('particular_type_id')]==4) {$emb_typearr=$emblishment_spwork_type;$em_name='Special Works';}
	    				else if($row[csf('particular_type_id')]==5) {$emb_typearr=$emblishment_gmts_type;$em_name='Gmts Dyeing';}
	    				else if($row[csf('particular_type_id')]==99) {$emb_typearr=$blank_array;$em_name='Others';}
			    	}
			    	else
			    	{
			    		$em_name=$emblishment_wash_type[$row[csf("particular_type_id")]];
			    	}
			    	$emb_cost+=$row[csf('value')];
			    	$emb_cons+=$row[csf('tot_cons')];
       				$emb_cost_fob+=($row[csf('value')]*100)/($total_fob*12);				 	
	       			?>
	       			<tr>
	       				<td><?=$lib_temp_arr[$row[csf('item_id')]];?></td>
	       				<td style="word-break: break-all;"><?=$em_name;?></td>
	       				<td style="word-break: break-all;"><?=$emb_typearr[$row[csf('fab_id')]];?></td>
	       				...
	       				<td align="right"><?=fn_number_format($row[csf('rate')],3,".",",")?></td>
	       				<td align="right"><?=fn_number_format($row[csf('value')],3,".",",")?></td>
	       				<td align="right"><?=fn_number_format($row[csf('tot_cons')]/12,3,".",",")?></td>
	       				<td align="right"><?=fn_number_format($row[csf('value')]/12,3,".",",")?></td>
	       				<td align="right"><?=fn_number_format(($row[csf('value')]*100)/($total_fob*12),3,".",",")?></td>
	       				<td style="word-break: break-all;"><?=$row[csf('remarks')]?></td>
	       			</tr>
            ...
            <tfoot>
       			<tr>
       				<td align="right" colspan="8"><strong>Total Embellishment Cost:</strong></td>
       				<td align="right"><strong><?=fn_number_format($emb_cost,4,".",",")?></strong></td>
       				<td align="right"><strong><?=fn_number_format($emb_cons/12,4,".",",")?></strong></td>
       				<td align="right"></td>
       				<td align="right"></td>
       				<td align="right"></td>
       			</tr>
		...
		<table width="800">
			<tr>
				<td>
					<table width="400" cellspacing="0" border="1"  rules="all" class="rpt_table">
						<caption style="justify-content: center;text-align: center;"><strong>Others Component</strong></caption>
						<thead>
							<tr>
								<th>Particulars</th>
								<th>Cost/Dzn</th>
						...
						<tbody>
							<tr>
								<td>Lab Test</td>
								<td align="right"><?=fn_number_format($res_summary[0][csf('tot_lab_test_cost')],4,".",",")?></td>
								<td align="right"><?=fn_number_format($res_summary[0][csf('tot_lab_test_cost')]/12,4,".",",")?></td>
								<td align="right"><?=fn_number_format(($res_summary[0][csf('tot_lab_test_cost')]*100)/(12*$total_fob),4,".",",")?></td>
							</tr>
							<tr>
								<td>Storage Cost</td>
								<td align="right"><?php echo fn_number_format($res_item[0][csf('storage_cost')],4,".",","); ?></td>
							...
							<tr>
								<td>CM Cost</td>
								<td align="right"><?=fn_number_format($res_summary[0][csf('tot_cm_cost')],4,".",",")?></td>
								<td align="right"><?=fn_number_format($res_summary[0][csf('tot_cm_cost')]/12,4,".",",")?></td>
								<td align="right"><?=fn_number_format(($res_summary[0][csf('tot_cm_cost')]*100)/(12*$total_fob),4,".",",")?></td>
							</tr>
							<tr>
								<td>Frieght Cost</td>
								<td align="right"><?=fn_number_format($res_summary[0][csf('tot_fright_cost')],4,".",",")?></td>
								<td align="right"><?=fn_number_format($res_summary[0][csf('tot_fright_cost')]/12,4,".",",")?></td>
								<td align="right"><?=fn_number_format(($res_summary[0][csf('tot_fright_cost')]*100)/(12*$total_fob),4,".",",")?></td>
							</tr>
							<tr>
								<td>Other Cost</td>
								<td align="right"><?=fn_number_format($res_summary[0][csf('tot_other_cost')],4,".",",")?></td>
								<td align="right"><?=fn_number_format($res_summary[0][csf('tot_other_cost')]/12,4,".",",")?></td>
								<td align="right"><?=fn_number_format(($res_summary[0][csf('tot_other_cost')]*100)/(12*$total_fob),4,".",",")?></td>
							</tr>
							<tr>
								<td>Commercial Cost</td>
								<td align="right"><?=fn_number_format($res_summary[0][csf('commercial_cost')],4,".",",")?></td>
								<td align="right"><?=fn_number_format($res_summary[0][csf('commercial_cost')]/12,4,".",",")?></td>
								<td align="right"><?=fn_number_format(($res_summary[0][csf('commercial_cost')]*100)/(12*$total_fob),4,".",",")?></td>
							</tr>
							<tr>
								<td>Commission</td>
								<td align="right"><?=fn_number_format($res_summary[0][csf('tot_commission_cost')],4,".",",")?></td>
								<td align="right"><?=fn_number_format($res_summary[0][csf('tot_commission_cost')]/12,4,".",",")?></td>
								<td align="right"><?=fn_number_format(($res_summary[0][csf('tot_commission_cost')]*100)/(12*$total_fob),4,".",",")?></td>
							</tr>
							<tr>
								<td>Margin/Profit</td>
								<td align="right"><?=fn_number_format($margin*12,4,".",",")?></td>
								<td align="right"><?=fn_number_format($margin,4,".",",")?></td>
								<td align="right"><?=fn_number_format(($margin*100)/($total_fob),4,".",",")?></td>
							</tr>
							<tr>
								<td>FOB $</td>
								<?php 
								$last_fob=$res_summary[0][csf('tot_lab_test_cost')]+$res_summary[0][csf('tot_cm_cost')]+$res_summary[0][csf('tot_fright_cost')]+$res_summary[0][csf('tot_other_cost')]+$res_summary[0][csf('commercial_cost')]+$res_summary[0][csf('tot_commission_cost')]+$margin*12+$emb_cost+$accressories_cost+$res_summary[0][csf('tot_fab_cost')]+$res_item[0][csf('storage_cost')]+$res_item[0][csf('operating_exp')];
								 ?>
								<td align="right"><?=fn_number_format($last_fob,4,".",",")?></td>
								<td align="right"><?=fn_number_format(($last_fob/12)+$margin ,4,".",",")?></td>
								<td align="right"><?=fn_number_format(($last_fob*100)/(12*$total_fob),0,".",",")?></td>
							</tr>
						</tbody>
					</table>
				</td>
				<td style="float: left;">
					<table width="200" cellspacing="0" border="1"  rules="all" class="rpt_table" >
						<caption style="justify-content: center;text-align: center;"><strong>SMV Details</strong></caption>
                        <tr>
                            <td>CPM</td>
                            <td align="right"><?=fn_number_format($res_item[0][csf('cpm')],4,".",",")?></td>
                        </tr>
                        <tr>
                            <td>SMV</td>
                            <td align="right"><?=fn_number_format($res_item[0][csf('smv')],4,".",",")?></td>
                        </tr>
                        <tr>
                            <td>Eff  %</td>
                            <td align="right"><?=fn_number_format($res_item[0][csf('efficiency')],4,".",",")?></td>
                        </tr>
					</table>
				</td>
			</tr>
		...
		$approve_data_array=sql_select("select b.approved_by, min(b.approved_date) as approved_date from  approval_history b where b.mst_id=$mst_id and b.entry_form=70 group by  b.approved_by ");
		$unapprove_data_array=sql_select("select b.approved_by, b.un_approved_by, b.approved_date, b.un_approved_reason, b.un_approved_date, b.approved_no from approval_history b where b.mst_id=$mst_id and b.entry_form=70 order by b.approved_date, b.approved_by");
		...
		{
			?>
			<table width="950" class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all">
				<thead>
					<tr style="border:1px solid black;">
				...
				foreach($approve_data_array as $row)
				{
					?>
					<tr style="border:1px solid black;">
					<td width="3%" style="border:1px solid black;" align="center"><?=$i;?></td>
		...
		<br/>
		<?
		$sql_unapproved=sql_select("select booking_id,approval_cause from fabric_booking_approval_cause where entry_form=70 and approval_type=2 and is_deleted=0 and status_active=1 and booking_id=$mst_id");
				...
				<tbody>
					...
					foreach($unapprove_data_array as $row)
					{
						...
						<tr style="border:1px solid black;">
							...
							<td width="5%" style="border:1px solid black; text-align:center"><?='Yes';?></td>
						...
						if($un_approved_date!="")
						{
							...
							<tr style="border:1px solid black;">
								...
								<td width="20%" style="border:1px solid black;text-align:center"><?=$unapproved_request_arr[$mst_id];?></td>
		...
		echo signature_table(458, $cbo_company_name, "860px");?>
    </div>
	<?
	exit();
}

if($action=="quick_costing_print_p1")
{
	extract($_REQUEST);
	echo load_html_head_contents("Quick Costing Print", "../../../", 1, 1,'','','');
	$data=explode('*',$data);
	...
	$sql_cost_summary=sql_select("SELECT commision_per
				  FROM qc_tot_cost_summary
				 WHERE mst_id = $data[0] ");
	$commision_per=$sql_cost_summary[0][csf('commision_per')];
	$is_set=0;
	
	$sql_mst="select id, cost_sheet_id, cost_sheet_no, temp_id, lib_item_id, style_ref, buyer_id, uom, cons_basis, season_id, style_des, department_id, delivery_date, exchange_rate, offer_qty, quoted_price, tgt_price, stage_id, costing_date, revise_no, option_id, option_remarks, margin_per,margin,buyer_remarks,exporter_code,currency_id,article,inquery_id from qc_mst where status_active=1 and is_deleted=0 and qc_no='$data[0]'";
	//echo $sql_mst; die;
	$sqlMst_res=sql_select($sql_mst);
	
	$qc_mst_id=$sqlMst_res[0][csf('id')];
	$inquery_id=$sqlMst_res[0][csf('inquery_id')];
	$quoted_price=$sqlMst_res[0][csf('quoted_price')];
	$margin_per=$sqlMst_res[0][csf('margin_per')];
	$margin=$sqlMst_res[0][csf('margin')];
	$count_temp=count(explode(',',$sqlMst_res[0][csf("lib_item_id")]));
	if($count_temp>1)
	{
		$is_set=1;
	}
	...
	$company_name_str=sql_select("SELECT b.company_name from wo_quotation_inquery a,lib_company b where a.id=$inquery_id and a.company_id=b.id");
	...
	$sql_img=sql_select("select image_location from common_photo_library where master_tble_id='$data[0]' and form_name='short_quotation_v2' and file_type=1 order by id");
	$sql_tgroup=sql_select( "select id, item_name, trim_uom, trim_type from lib_item_group where item_category=4 and is_deleted=0 and status_active=1 order by item_name");
	$lib_tgroup=return_library_array("select id, item_name from lib_item_group where item_category=4 and is_deleted=0 and status_active=1","id","item_name");
	?>
    <style>
		.watermark {
			opacity: 0.5;
    	...
    	<?
    		...
    		$item_list=sql_select("select  item_id,fob_pcs from qc_item_cost_summary where mst_id =$data[0] and status_active=1  order by id");
    		$item_wise_fob=array();
    		foreach ($item_list as $row) {
    			$item_list_arr[$row[csf('item_id')]]=$lib_temp_arr[$row[csf('item_id')]];
    			$item_wise_fob[$row[csf('item_id')]]=((($row[csf('fob_pcs')]/12)*$margin_per)/100);
    		}
    		$item_sl=0;
    		foreach ($item_list_arr as $item_id => $item_name) 
    		{
    			$without_commission=0;
    			
    			?>
    			<div style="width:1000px;margin-bottom: 10px;" class="pagebreak">
    				<table width="1000" cellspacing="0" border="1" style="margin-right:10px;" rules="all" class="rpt_table">
    					<tr>
    						<td width="300" style="border: none;background-color:#D9D9D9; "><strong>Supplier Name / Code</strong></td>
    						<td width="400" ><strong><? echo $company_name_str[0][csf('company_name')]; ?> / <? echo $sqlMst_res[0][csf('exporter_code')]; ?> &nbsp;Date: <? echo date('d-M-Y',strtotime($sqlMst_res[0][csf('costing_date')])); ?> </strong>
    						<td rowspan="7" width="300">
    							<p>&nbsp; </p>
    							<?php 

    								$image_sl=0;
    								if(count($sql_img)>=$item_sl)
    								{
    									$image_sl=$item_sl;
    								}
    							?>
    							<img src="../../../<?=$sql_img[$image_sl][csf('image_location')]?>" height="120">
    							
    						</td>
    					</tr>
    					<tr>
    						<td style="border: none;background-color:#D9D9D9; "><strong>Currency</strong></td>
    						<td><strong><?=$currency[$sqlMst_res[0][csf('currency_id')]]?></strong></td>
    					</tr>

    					<tr>
    						<td style="border: none;background-color:#D9D9D9; ">Quantity</td>
    						<td style="justify-content: right;text-align: right;">$<? echo number_format($sqlMst_res[0][csf('offer_qty')],0,'.',''); ?></td>
    					</tr>
    					<tr>
    						<td style="border: none;background-color:#D9D9D9; "><strong>Style Name</strong></td>
    						<td><strong><? echo $sqlMst_res[0][csf('style_ref')]; ?></strong></td>
    					</tr>
    					<tr>
    						<td style="border: none;background-color:#D9D9D9; ">Article Name</td>
    						<td> <? echo $sqlMst_res[0][csf('article')]; ?></td>
    					</tr>
    					<tr>
    						<td style="border: none;background-color:#D9D9D9; ">Season</td>
    						<td><? echo $season_arr[$sqlMst_res[0][csf('season_id')]]; ?></td>
    					</tr>
    					<tr>
    						<td style="border: none;background-color:#D9D9D9; ">Type</td>
    						<td></td>
    					</tr>
    				</table>
			    	<table width="1000" cellspacing="0" border="1" style="margin-right:10px;" rules="all" class="rpt_table">
			    		...
			    		$sql_f="select id, sum(value) as value,remarks from qc_cons_rate_dtls where mst_id = '$data[0]' and  body_part_id>0 and type=1 and item_id=$item_id group by  id,remarks order by id";
			    		//echo $sql_f;
			    		$sql_f_res=sql_select($sql_f);
			    		$sl=1;
			    		$main_fabric=0;
			    		$fab_value=array();
			    		$fab_remark=array();
			    		foreach ($sql_f_res as $row) {
			    			$fab_value[$sl]=$row[csf('value')];
			    			$fab_remark[$sl]=$row[csf('remarks')];
			    			if($sl<5)
			    			{
			    				$main_fabric+=$row[csf('value')];
			    			}
			    			$sl++;
			    		}
			    		$without_commission+=$main_fabric;
			    		?>
			    		
			    		<tr >
			    			<td style="background-color:#D9D9D9;justify-content: left;text-align:left; " width="300"><span style="justify-content: left;text-align:left"><strong>Fabric COST</strong></span></td>
			    			<td style="background-color:#D9D9D9; justify-content: right;text-align:right;" ><span style="justify-content: right;text-align:right">$<?=fn_number_format($main_fabric/12,4);?></span></strong>
			    			<td style="border: none;" width="300">&nbsp;</td>
			    		</tr>
			    		<tr>
			    			<td width="300" style="border: none;background-color:#D9D9D9; ">Units Pairs Per Ticket</td>
			    			<td width="400" align="right">1</strong>
			    			<td  width="300">Notes:</td>
			    		</tr>
			    		<tr>
			    			<td width="300" style="border: none;background-color:#D9D9D9; ">Secondary Fabric Cost</td>
			    			<td width="400" style="justify-content: right;text-align: right;">$<?=number_format($fab_value[2]/12,4)?></td>
			    			<td  width="300" style="word-break: break-all;"><p ><?=$fab_remark[2]?></p></td>
			    		</tr>
			    		<tr>
			    			<td width="300" style="border: none;background-color:#D9D9D9; ">Third Fabric Cost</td>
			    			<td style="justify-content: right;text-align: right;" width="400" >$<?=number_format($fab_value[3]/12,4)?></td>
			    			<td  width="300" style="word-break: break-all;"><p ><?=$fab_remark[3]?></p></td>
			    		</tr>
			    		<tr>
			    			<td width="300" style="border: none;background-color:#D9D9D9; ">Fourth Fabric Cost</td>
			    			<td width="400" align="right">$<?=number_format($fab_value[4]/12,4)?></td>
			    			<td  width="300" style="word-break: break-all;"><p ><?=$fab_remark[4]?></p></td>
			    		</tr>
			    		<tr>
			    			<td width="300" style="background-color:#D9D9D9; "><strong>Main Fabric</strong></td>
			    			<td width="400" style="background-color:#D9D9D9;justify-content: right;text-align: right; "><strong>$<?=number_format($fab_value[1]/12,4)?></strong></td>
			    			<td  width="300"style="background-color:#D9D9D9; "></td>
			    		</tr>
			    		<tr>
			    			<td width="700"  colspan="2" >
			    				<table cellspacing="0" border="1"  rules="all" class="rpt_table" width="100%">
			    					<?php

			    						$sql_main_f=sql_select("SELECT id,rate FROM qc_cons_rate_dtls WHERE     mst_id = '$data[0]' AND TYPE = 1 and  body_part_id>0 and item_id=$item_id order by id");
			    						$firsth_fb_id=$sql_main_f[0][csf('id')];
			    						$rate=$sql_main_f[0][csf('rate')];
			    						$sql_main_fab="SELECT qc_no,body_part_id,yarn_lib_id,yarn_break_down,fab_des FROM qc_fab_yarn_conv WHERE QC_NO = '$data[0]' AND status_active = 1 AND is_deleted = 0 and type=1  and cons_rate_dtls_id in ($firsth_fb_id)";
			    						//echo $sql_main_fab;
			    						$res_main_fab=sql_select($sql_main_fab);

			    						$fab_data=array();
			    						$determina_arr=array();
			    						foreach ($res_main_fab as $row) {
			    							$fab_data['fabric_name']=$row[csf('yarn_lib_id')];
			    							$fab_data['composition']=$row[csf('yarn_lib_id')];
			    							$fab_data['construction']=$row[csf('yarn_lib_id')];
			    							array_push($determina_arr, $row[csf('yarn_lib_id')]);
			    						}
			    						$determina_cond=where_con_using_array($determina_arr,0,"a.id");
										$sql_d="SELECT a.id,
											       a.construction
											 ...
											 WHERE    
											       ...
											       $determina_cond";
										//echo $sql_d;
									 $lib_determina=sql_select($sql_d);
			    					 $determina_data=array();

			    					 foreach ($lib_determina as $row) {
			    					 	$determina_data['construction'].=$row[csf('construction')]."***";
			    					 	//$determina_data['composition_name'].=$row[csf('composition_name')]."***";
			    					 	//$determina_data['yarn_count'].=$row[csf('yarn_count')]."***";
			    					 	
			    					 }
			    					 
			    					 $yarnCountArr=return_library_array("select id,yarn_count from lib_yarn_count where is_deleted=0 and status_active=1 order by yarn_count", "id","yarn_count");
			    					 $sqlFab="select id, type, yarn_lib_id, fab_des, count, composition, percent, yarn_type, process, fab_id, cons, exper, tot_cons, rate, value,nominated_supp_multi,  gsm,gauge,uom from qc_fab_yarn_conv where qc_no='$data[0]'  and status_active=1 and is_deleted=0  and cons_rate_dtls_id in ($firsth_fb_id)";
			    					// echo $sqlFab;
			    					 $res_comp=sql_select($sqlFab);
			    					 $supplier_library=return_library_array( "select a.supplier_name, a.id from lib_supplier  where a.is_deleted=0  and a.status_active=1 group by a.id,a.supplier_name order by a.supplier_name", "id", "supplier_name");
			    					
			    					 $yarn_price=0;
			    					 $cons=0;
			    					 foreach ($res_comp as $row) {
			    					 	$determina_data['construction'].=$row[csf('construction')]."***";
			    					 	$determina_data['composition_name'].=$composition[$row[csf('composition')]]."***";
			    					 	$determina_data['yarn_count'].=$yarnCountArr[$row[csf('count')]]."***";
			    					 	$determina_data['tot_cons']+=$row[csf('tot_cons')];
			    					 	$determina_data['gsm'].=$row[csf('gsm')]."***";
			    					 	$determina_data['gauge'].=$row[csf('gauge')]."***";
			    					 	$determina_data['nominated_supp_multi'].=$lib_supplier[$row[csf('nominated_supp_multi')]]."***";
			    					 	$determina_data['uom'].=$unit_of_measurement[$row[csf('uom')]]."***";
			    					 	
			    					 	if($row[csf('type')]==2)
			    					 	{
			    					 		 $yarn_price+=$row[csf('value')];
			    					 		 $cons+=$row[csf('cons')];
			    					 	}
			    					 }
			    					...
			    					?>
			    					<tr>
			    						<td width="300" style="border: none;background-color:#D9D9D9; " align="right">Fabric Name</td>
			    						<td width="400" align="left" bgcolor="white"><?=implode(",", array_unique(explode("***", chop($determina_data['construction'],"***"))))?></td>
			    					</tr>
			    					<tr>
			    						<td align="right" style="border: none;background-color:#D9D9D9; ">Composition</td>
			    						<td align="left" bgcolor="white"><?=implode(",", array_unique(explode("***", chop($determina_data['composition_name'],"***"))))?></td>
			    					</tr>
			    					<tr>
			    						<td align="right" style="border: none;background-color:#D9D9D9; ">Construction</td>
			    						<td align="left"><?=implode(",", array_unique(explode("***", chop($determina_data['construction'],"***"))))?></td>
			    					</tr>
			    					<tr>
			    						<td align="right" style="border: none;background-color:#D9D9D9; ">Fabric Weight Unit</td>
			    						<td align="left" style="background-color:#D9D9D9; ">Gr/SqM</td>
			    					</tr>
			    					<tr>
			    						<td align="right" style="border: none;background-color:#D9D9D9; ">Fabric Weight</td>
			    						<td align="left"><?=implode(",", array_unique(explode("***", chop($determina_data['gsm'],"***"))))?></td>
			    					</tr>
			    					<tr>
			    						<td align="right" style="border: none;background-color:#D9D9D9; ">Gauge</td>
			    						<td align="left"><?=implode(",", array_unique(explode("***", chop($determina_data['gauge'],"***"))))?></td>
			    					</tr>
			    					<tr>
			    						<td align="right" style="border: none;background-color:#D9D9D9; ">Consumption Unit</td>
			    						<td align="left" style="background-color:#D9D9D9; "><?=implode(",", array_unique(explode("***", chop($determina_data['uom'],"***"))))?></td>
			    					</tr>
			    					<tr>
			    						<td align="right" style="border: none;background-color:#D9D9D9; ">Consumption</td>
			    						<td align="left"><?=fn_number_format($determina_data['tot_cons']/12,4,".",",");?></td>
			    					</tr>
			    					<tr>
			    						<td align="right" style="border: none;background-color:#D9D9D9; ">Yarn Count</td>
			    						<td align="left"><?=implode(",", array_unique(explode("***", chop($determina_data['yarn_count'],"***"))))?></td>
			    					</tr>
			    					<tr>
			    						<td align="right" style="border: none;background-color:#D9D9D9; ">Yarn Fabric Mill</td>
			    						<td align="left"><?=implode(",", array_unique(explode("***", chop($determina_data['nominated_supp_multi'],"***"))))?></td>
			    					</tr>
			    					<tr>
			    						<td align="right" style="border: none;background-color:#D9D9D9; ">Yarn Price Unit</td>
			    						<td align="left" style="background-color:#D9D9D9; "><?=implode(",", array_unique(explode("***", chop($determina_data['uom'],"***"))))?></td>
			    					</tr>
			    					<tr>
			    						<td align="right" style="border: none;background-color:#D9D9D9; ">Yarn Price</td>
			    						<td align="left"><?=fn_number_format( $yarn_price/$cons,2,".",",")?></td>
			    					</tr>
			    					<tr>
			    						<td align="right" style="border: none;background-color:#D9D9D9; ">Fabric Price Unit</td>
			    						<td align="left" style="background-color:#D9D9D9; "><?=implode(",", array_unique(explode("***", chop($determina_data['uom'],"***"))))?></td>
			    					</tr>
			    					<tr>
			    						<td align="right" style="border: none;background-color:#D9D9D9; ">Fabric Price</td>
			    						<td align="right" style="background-color:#D9D9D9; ">$<?=fn_number_format($rate,4,".",",")?></td>
			    					</tr>
			    					
			    				</table>
			    			</td>
			    			<td  width="300"style="word-break: break-all;"><p ><?=$fab_remark[1]?></p></td>
			    		</tr>
			    		<tr >
			    			<td  width="300"style="border: none;background-color:#D9D9D9; "><span style="justify-content: left;text-align:left"><strong>Trim Accessories</strong></span></td>
			    			<?

			    				$sql_print_embodatory=sql_select("SELECT sum(value) as value,listagg(cast(remarks as varchar2(4000)),',') within group (order by remarks) AS remarks FROM qc_cons_rate_dtls WHERE mst_id = '$data[0]' and type=2 and particular_type_id in (1,2) and item_id=$item_id");
			    				$trims_total=0;
			    				$trims_total+=$sql_print_embodatory[0][csf('value')];
			    				 $sql_trims=sql_select("SELECT particular_type_id,sum(value) as value,remarks,id FROM qc_cons_rate_dtls WHERE mst_id = '$data[0]' and type=4 and item_id=$item_id group by particular_type_id,remarks,id order by id");

			    				foreach ($sql_trims as $row) 
			    				{
			    					$trims_total+=$row[csf('value')];
			    				}

			    				

			    			?>
			    			<td style="border: none;background-color:#D9D9D9;justify-content: right;text-align:right; " width="400" ><span style="justify-content: right;text-align:right">$<?=number_format($trims_total/12,4); $without_commission+=$trims_total;?></span></strong>
			    			<td style="border: none;background-color:#D9D9D9; " width="300">&nbsp;</td>
			    		</tr>
			    		<tr>
			    				...
			    				<table cellspacing="0" border="1"  rules="all" class="rpt_table" width="100%">
			    					...
			    					<?
			    						foreach ($sql_trims as $row) 
			    						{
			    							?>
			    							<tr>
			    								<td style="border: none;background-color:#D9D9D9; " width="300" align="right"><?=$lib_tgroup[$row[csf('particular_type_id')]]?></td>
			    								<td width="400" align="right" bgcolor="white">$<?=fn_number_format($row[csf('value')]/12,4,".",",");?></td>
			    								<td width="300" style="word-break: break-all;"><p><?=$row[csf('remarks')]?></p></td>
			    							</tr>
			    							<?
			    						}
			    					...
			    					?>
			    					<tr>
			    						<td align="right" style="border: none;background-color:#D9D9D9; ">Prints+Embroidery</td>
			    						<td align="right">$<?=fn_number_format($sql_print_embodatory[0][csf('value')]/12,4,".",",")?></td>
			    						<td align="left" style="word-break: break-all;"><p><?=$sql_print_embodatory[0][csf('remarks')]?></p></td>
			    					</tr>
			    	...
			    	<table cellspacing="0" border="1"  rules="all" class="rpt_table" width="700">
			    		<? $sql_wash=sql_select("SELECT particular_type_id,sum(value) as value,remarks FROM qc_cons_rate_dtls WHERE mst_id = '$data[0]' and type=3 and item_id=$item_id group by particular_type_id,remarks");
			    			$was_amount=0;
			    			foreach ($sql_wash as $row) {
			    				$was_amount+=$row[csf('value')];
			    			}
			    			$without_commission+=$was_amount;
			    		?>
			    		<tr>
			    			<td style="background-color:#D9D9D9;"  align="left" width="300"><strong>Garment Washing</strong></td>
			    			<td width="400" style="background-color:#D9D9D9;justify-content: right;text-align: right; font-weight: bold;" align="right">$<?=fn_number_format($was_amount/12,4,".",",")?></td>
			    		</tr>
    					...
    					<?
    						foreach ($sql_wash as $row) 
    						{
    							?>
    							<tr>
    								<td style="border: none;background-color:#D9D9D9; " width="300" align="right"><?=$emblishment_wash_type[$row[csf('particular_type_id')]]?></td>
    								<td width="400" align="right" bgcolor="white">$<?=fn_number_format($row[csf('value')]/12,4,".",",");?></td>
    					...
    					<?
    						$sql_cost="SELECT smv,
									       ...
									       other_cost,
									       commission_cost
									  FROM qc_item_cost_summary
									 WHERE mst_id = $data[0] and item_id=$item_id";
						    $res_cost=sql_select($sql_cost);

						    $without_commission+=($res_cost[0][csf('smv')]+$res_cost[0][csf('sam_cost')]+$res_cost[0][csf('cm_cost')]);

    					?>

    					<tr>
			    			<td style="background-color:#D9D9D9;"  align="left" width="300"><strong>Cut Make</strong></td>
			    			<td width="400" style="background-color:#D9D9D9;justify-content: right;text-align: right; font-weight: bold;" align="right">$<?=fn_number_format( ($res_cost[0][csf('smv')]+$res_cost[0][csf('sam_cost')]+$res_cost[0][csf('cm_cost')])/12,4,".",",")?></td>
			    		</tr>
			    		<tr>
							<td style="border: none;background-color:#D9D9D9; " width="300" align="right">Sam</td>
							<td width="400" align="right" bgcolor="white">$<?=fn_number_format($res_cost[0][csf('sam_cost')]/12,4,".",",");?></td>
							
						</tr>
						<tr>
							<td style="border: none;background-color:#D9D9D9; " width="300" align="right">Cutting</td>
							<td width="400" align="right" bgcolor="white">$<?=fn_number_format((($res_cost[0][csf('cm_cost')]/12)*20)/100,4,".",",");?></td>
							
						</tr>
						<tr>
							<td style="border: none;background-color:#D9D9D9; " width="300" align="right">Sewing</td>
							<td width="400" align="right" bgcolor="white">$<?=fn_number_format((($res_cost[0][csf('cm_cost')]/12)*60)/100,4,".",",");?></td>
							
						</tr>
    					<tr>
							<td style="border: none;background-color:#D9D9D9; " width="300" align="right">Packing</td>
							<td width="400" align="right" bgcolor="white">$<?=fn_number_format((($res_cost[0][csf('cm_cost')]/12)*20)/100,4,".",",");?></td>
						...
						<tr>
			    			<td style="background-color:#D9D9D9;"  align="left" width="300"><strong>Additional Margins</strong></td>
			    			<td width="400" style="background-color:#D9D9D9;justify-content: right;text-align: right; font-weight: bold;" align="right" >$<?=fn_number_format( (($res_cost[0][csf('storage_cost')]+$res_cost[0][csf('frieght_cost')]+$res_cost[0][csf('commercial_cost')]+$res_cost[0][csf('lab_test_cost')]+$res_cost[0][csf('other_cost')]+$res_cost[0][csf('sup_margin')])/12) +$item_wise_fob[$item_id],4,".",",")?></td>

			    			<?
			    				$without_commission+=($res_cost[0][csf('storage_cost')]+$res_cost[0][csf('frieght_cost')]+$res_cost[0][csf('commercial_cost')]+$res_cost[0][csf('lab_test_cost')]+$res_cost[0][csf('other_cost')]);
			    				$without_commission=($without_commission/12)+$margin;
			    		...
			    		<tr>
							<td style="border: none;background-color:#D9D9D9; " width="300" align="right">Storage Cost</td>
							<td width="400" align="right" bgcolor="white">$<?=fn_number_format($res_cost[0][csf('storage_cost')]/12,4,".",",");?></td>
							
						</tr>
    					<tr>
							<td style="border: none;background-color:#D9D9D9; " width="300" align="right">Transport Cost</td>
							<td width="400" align="right" bgcolor="white">$<?=fn_number_format($res_cost[0][csf('frieght_cost')]/12,4,".",",");?></td>
							
						</tr>
    					<tr>
							<td style="border: none;background-color:#D9D9D9; " width="300" align="right">Financing Cost</td>
							<td width="400" align="right" bgcolor="white">$<?=fn_number_format($res_cost[0][csf('commercial_cost')]/12,4,".",",");?></td>
							
						</tr>
    					
    					<tr>
							<td style="border: none;background-color:#D9D9D9; " width="300" align="right">Qad Test</td>
							<td width="400" align="right" bgcolor="white">$<?=fn_number_format($res_cost[0][csf('lab_test_cost')]/12,4,".",",");?></td>
							
						</tr>
    					<tr>
							<td style="border: none;background-color:#D9D9D9; " width="300" align="right">Other Costs</td>
							<td width="400" align="right" bgcolor="white">$<?=fn_number_format($res_cost[0][csf('other_cost')]/12,4,".",",");?></td>
							
						</tr>
						<tr>
							<td style="border: none;background-color:#D9D9D9; " width="300" align="right">Specify Other Costs</td>
							<td width="400" align="left" bgcolor="white" style="word-break: break-all;"><?=$res_cost[0][csf('remark')];?></td>
							
						</tr>
    					<tr>
							<td style="border: none;background-color:#D9D9D9; " width="300" align="right">Supplier Margin</td>
							<td width="400" align="right" bgcolor="white">$<?=fn_number_format($item_wise_fob[$item_id],4,".",",");?></td>
    				...
    				<table cellspacing="0" border="1"  rules="all" class="rpt_table" width="700">
    					<tr>
			    			<td style="background-color:#D9D9D9;border: none;"  align="left" width="300"><strong>Total  Scp Net</strong></td>
			    			<td width="400" style="background-color:#D9D9D9;border:none;justify-content: right;text-align: right; font-weight: bold;" align="right">$<?=fn_number_format( $without_commission,4,".",",")?></td>
			    		</tr>
    				</table>
    				<?
	    				$without_commission+( $res_cost[0][csf('commission_cost')]);

	    			?>
    				<table cellspacing="0" border="1"  rules="all" class="rpt_table" width="700">
    					<tr>
			    			<td style="background-color:#D9D9D9;border: none;"  align="left" width="300"><strong>Total  Scp (Including <?=$commision_per;?>%TPA)</strong></td>
			    			<td width="400" style="background-color:#D9D9D9;border:none;justify-content: right;text-align: right; font-weight: bold;" align="right">$<?=fn_number_format($without_commission+( $res_cost[0][csf('commission_cost')]/12),4,".",","); $total_scp+=$without_commission+($res_cost[0][csf('commission_cost')]/12);?></td>
			    		</tr>
    				</table>
    				<table cellspacing="0" border="1"  rules="all" class="rpt_table" width="700">
    					<tr>
			    			<td style="background-color:#D9D9D9;border: none;"  align="left" width="300"><strong>Negotiated  Scp</strong></td>
			    			<td width="400" style="background-color:#D9D9D9;border:none;justify-content: right;text-align: right; font-weight: bold;" align="right">$<?=fn_number_format($quoted_price,4,".",",")?></td>
			    		</tr>
    				</table>
			     </div>
    	...
    	<table cellspacing="0" border="1"  rules="all" class="rpt_table" width="700">
			<tr>
    			<td style="background-color:#D9D9D9;border: none;"  align="left" width="300"><strong>TOTAL FOB PRICE</strong></td>
    			<td width="400" style="background-color:#D9D9D9;border:none;justify-content: right;text-align: right; font-weight: bold;" align="right">$<?=fn_number_format($total_scp,4,".",",")?></td>
    		</tr>
		</table>       
		<?
		}
...
{
	echo create_drop_down( "cbo_buyer_id", 130, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90))  order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down( 'requires/quick_costing_v3_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'requires/quick_costing_v3_controller', this.value, 'load_drop_down_brand', 'brand_td');" );
	exit();
}
    ...
    <div align="center" style="width:100%;" >
        <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            <table width="910" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
                ...
                <tr class="general">
                    <td><? echo create_drop_down( "cbo_company_mst", 140, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", $company_id,"load_drop_down( 'quick_costing_v3_controller', this.value, 'load_drop_down_buyer', 'buyer_td');",1 ); ?>
					<input type="hidden" id="selected_id">
					</td>
                    <td id="buyer_td">
						<? //echo create_drop_down( "cbo_buyer_name", 130, $blank_array,'', 1, "-- Select Buyer --" );
						echo create_drop_down( "cbo_buyer_name", 130, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$company_id' $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90))  order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down( 'quick_costing_v3_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'quick_costing_v3_controller', this.value, 'load_drop_down_brand', 'brand_td');" ); ?>
					</td>
...
if($action=="bodyPart_washType_popup")
{
            ...
            <tbody>
                ...
                foreach($sql_tgroup as $row_tgroup)
				{
					...
					<tr id="search<?=$i;?>" onClick="js_set_value(<?=$i; ?>);" bgcolor="<?=$bgcolor; ?>">
						<td width="30"><?=$i; ?></td>
                        <td width="190" style="word-break:break-all"><?=$row_tgroup[csf('item_name')]; ?>
                        	<input type="hidden" name="txttrimgroupdata_<?=$i; ?>" id="txttrimgroupdata_<?=$i; ?>" value="<?=$str; ?>"/>
                        </td>
						<td width="70"><?=$unit_of_measurement[$row_tgroup[csf('trim_uom')]]; ?></td>
            ...
            <table cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                    <tr class="general">
                        ...
                        <td align="center">
                        	<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view ('<?=$txt_fabid; ?>'+'**'+document.getElementById('txt_construction').value+'**'+document.getElementById('txt_gsm_weight').value+'**'+document.getElementById('cbo_string_search_type').value+'**'+document.getElementById('txt_rdno').value, 'fabric_description_popup_search_list_view', 'search_div', 'quick_costing_v3_controller', 'setFilterGrid(\'list_view\',-1)'); toggle( 'tr_'+'<?=$txt_fabid; ?>', '#FFFFCC');" style="width:100px;" />
                        </td>
                    </tr>
            	</tbody>
           	</table>
...
?>
    <table class="rpt_table" width="1000" cellspacing="0" cellpadding="0" border="0" rules="all" style="position: sticky; top: 0;" >
        <thead>
        	<tr>
   ...
   <table id="list_view" class="rpt_table" width="1000" cellspacing="0" cellpadding="0" border="1" rules="all" style="max-height:300px; overflow-y:scroll">
        <tbody>
	<?
		$sql_data=sql_select("select a.id, a.fab_nature_id, a.type, a.construction, a.gsm_weight, a.weight_type, a.design, a.fabric_ref, a.color_range_id, a.rd_no, a.inserted_by, a.status_active, a.full_width, a.cutable_width from lib_yarn_count_determina_mst a where a.is_deleted=0 and a.entry_form=426 $search_con order by a.id");
		$i=1;
		foreach($sql_data as $row)
		{
			if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
			...
			$fabric=$row[csf('rd_no')].','.$row[csf('fabric_ref')].','.$row[csf('type')].','.$row[csf('construction')].','.$row[csf('design')].','.$row[csf('gsm_weight')].','.$fabric_weight_type[$row[csf('weight_type')]].','.$color_range[$row[csf('color_range_id')]].','.$composition_arr[$row[csf('id')]].','.$row[csf('full_width')].','.$row[csf('cutable_width')];
			?>
	            <tr id="tr_<?=$row[csf('id')] ?>" bgcolor="<?=$bgcolor; ?>" height="20" style="cursor:pointer; word-break:break-all;" onClick="js_set_value('<?=$row[csf('id')]."_".$fabric; ?>')">
...
if($action=="trims_cost_template_name_popup")
{
    ...
    <script>
        ...
        {
            var template_data=return_global_ajax_value( data, 'get_template_data', '', 'quick_costing_v3_controller');
    ...
    <div align="center" style="width:100%;">
    	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="200" class="rpt_table" align="left">
    		...
    		if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF"; ?>
    		<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="insert_template_data('<? echo $row[csf("template_name")] ?>')">
...
if($action == "get_template_data")
{
	...
	//echo "SELECT * from wo_lib_trim_cost_temp where template_name='$data' and status_active=1 and is_deleted=0";
    $i=1;
    foreach( $data_array as $row )
    {
...
if ($action=="load_drop_down_buyer")
{
	echo create_drop_down( "cbo_buyer_name", 140, "select buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond  and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) group by buy.id, buy.buyer_name order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down('requires/quick_costing_v3_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'requires/quick_costing_v3_controller',this.value, 'load_drop_down_sub_dep', 'sub_td' ); load_drop_down( 'requires/quick_costing_v3_controller', this.value, 'load_drop_down_brand', 'brand_td');" );
	exit();
}