...
if($action=="template_list_view")
{
    ...
    <legend>Update Template List</legend>
        ...
        <div style="width:370px; overflow-y:scroll; max-height:220px;" align="center">
            ...
            <?
				$sql_tmp="select tuid, temp_id, item_id1, ratio1, item_id2, ratio2, status_active from qc_template where status_active=1 and is_deleted=0 and lib_item_id!='0' order by id ASC";
...
{
	$sql_tmp="select id, item_name from lib_qc_template where status_active=1 and is_deleted=0 and id in ($data) order by item_name";
...
{
	$sql_tmp="select id, tuid, temp_id, item_id1, ratio1, item_id2, ratio2, status_active from qc_template where tuid='$data' and status_active=1 and is_deleted=0 and lib_item_id!='0' order by id ASC";
...
if($action=="save_update_delete")
{
	...
	where  b.team_id in (select id from lib_marketing_team a where user_tag_id='$user_id' and a.status_active=1 and a.is_deleted=0 ) and b.status_active=1 and b.is_deleted=0");
		...
		$field_array_mst=" id, company_id, location_id, cost_sheet_id, cost_sheet_no, temp_id, lib_item_id, inquery_id, style_ref, buyer_id, uom, cons_basis, season_id, season_year, brand_id, style_des, prod_dept, prod_code, delivery_date, exchange_rate, offer_qty, quoted_price, tgt_price, margin, stage_id, costing_date, pre_cost_sheet_id, revise_no, option_id, buyer_remarks, option_remarks, body_color_id, qc_no, commercial_cost_method, entry_form, inserted_by, insert_date, status_active, is_deleted";
		
		$data_array_mst="(".$mst_id.",".$cbo_company_id.",".$cbo_location_id.",".$mst_id.",'".$autoCostSheetNo."',".$cbo_temp_id.",".$txt_temp_id.",".$txt_inquery_id.",".$txt_styleRef.",".$cbo_buyer_id.",".$cbouom.",".$cbo_cons_basis_id.",".$cbo_season_id.",".$cbo_season_year.",".$cbo_brand.",'".strtoupper(str_replace("'","",$txt_styleDesc))."',".$cbo_product_department.",".$txt_product_code.",".$txt_delivery_date.",".$txt_exchangeRate.",".$txt_offerQty.",".$txt_quotedPrice.",".$txt_tgtPrice.",".$txtmarign.",".$cbo_stage_id.",".$txt_costingDate.",'".$update_id."','".$revise_no."','".$option_id."','".strtoupper($txt_costing_remarks)."','".strtoupper($txt_option_remarks)."','".$color_id."','".$qcno."',".$txt_commercial_cost_method.",430,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		...
		foreach($temp_id as $item_id)
		{
			...
			foreach($ex_itemConsRateData as $item_wise_consRateData)
			{
				if($item_wise_consRateData!="")
				{
					...
					$bodypartid=$fabDesc=$fabid=$usefor=$cons=$exPer=$totCons=$rate=$fcPer=$fabAmt=$fabNSupp=$fabuom=0;
					...
					$fabuom=$ex_itemConsRate[12];
					
					if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
					$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',1,0,'".$cons."','".$exPer."','".$totCons."','".$fabuom."','".$rate."','".$fabAmt."','".$fabid."','".$fabDesc."','".$usefor."','".$fcPer."','".$bodypartid."','0','0','".$fabNSupp."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		...
		{
			$fab_td=$spOpe_td=$wash_td=$acc_td=$txtCpm=$txtSmv=$txtEff=$txtCmCost=$txtFriCost=$txtCourierCost=$txtLtstCost=$txtMissCost=$txtOtherCost=$txtCommCost=$fobT_td=$fobDzn_td=$txtCommlCost=$txtInspCost=$txtdlcCost=$txtOptExp=$tot_item_id=0;
			...
			$txtOptExp=trim($ex_item_tot[20]);
			$tot_item_id=trim($ex_item_tot[21]);
			
			if ($add_comma_item_tot!=0) $data_array_item_tot .=",";
			$data_array_item_tot .="(".$item_cost_id.",".$qcno.",'".$tot_item_id."','".$fab_td."','".$spOpe_td."','".$wash_td."','".$acc_td."','".$txtCpm."','".$txtSmv."','".$txtEff."','".$txtCmCost."','".$txtFriCost."','".$txtCourierCost."','".$txtLtstCost."','".$txtMissCost."','".$txtOtherCost."','".$txtCommCost."','".$fobT_td."','".$txtRmgQty."','".$txtCommlCost."','".$txtInspCost."', '".$txtdlcCost."','".$txtOptExp."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
...
where b.team_id in (select id from lib_marketing_team a where user_tag_id='$user_id' and a.status_active=1 and a.is_deleted=0 ) and b.status_active=1 and b.is_deleted=0");
		...
		$field_array_mst="location_id*temp_id*lib_item_id*inquery_id*style_ref*buyer_id*uom*cons_basis*season_id*season_year*brand_id*style_des*prod_dept*prod_code*delivery_date*exchange_rate*offer_qty*quoted_price*tgt_price*margin*stage_id*costing_date*buyer_remarks*option_remarks*body_color_id*updated_by*update_date";
		
		//$txt_temp_id=explode(",",str_replace("'","",$txt_temp_id));
		//asort($txt_temp_id);
		//$txt_temp_id="'".implode(",",$txt_temp_id)."'";
		
		$data_array_mst="".$cbo_location_id."*".$cbo_temp_id."*".$txt_temp_id."*".$txt_inquery_id."*".$txt_styleRef."*".$cbo_buyer_id."*".$cbouom."*".$cbo_cons_basis_id."*".$cbo_season_id."*".$cbo_season_year."*".$cbo_brand."*'".strtoupper(str_replace("'","",$txt_styleDesc))."'*".$cbo_product_department."*".$txt_product_code."*".$txt_delivery_date."*".$txt_exchangeRate."*".$txt_offerQty."*".$txt_quotedPrice."*".$txt_tgtPrice."*".$txtmarign."*".$cbo_stage_id."*".$txt_costingDate."*'".strtoupper($txt_costing_remarks)."'*'".strtoupper($txt_option_remarks)."'*'".$color_id."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
...
if($action=="style_popup")
{
	...
	{
		show_list_view (document.getElementById('cbo_buyer_id').value+'**'+document.getElementById('cbo_season_id').value+'**'+document.getElementById('cbo_product_department').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('txt_cost_sheet_no').value+'**'+document.getElementById('hide_dataType').value+'**'+document.getElementById('cbo_approval_type').value+'**'+document.getElementById('cbo_season_year').value+'**'+document.getElementById('cbo_brand').value+'**'+document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_string_search_type').value, 'style_ref_search_list_view', 'search_div', 'quick_costing_woven_controller', 'setFilterGrid(\'tbl_list_search\',-1)');
		set_all();
	}
	...
	function fnc_confirm_style(qcno,upid)
	{
		if(upid!="")
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link+'&data='+data, title, 'width=1080px,height=350px,center=1,resize=0,scrolling=0','../../')
            ...
            <table cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr class="general">
                        ...
                        <td><? echo create_drop_down( "cbo_product_department", 130, $product_dept, "", 1, "-Select-", $selected, "", "", "" ); ?></td>     
...
if($action=="style_ref_search_list_view")
{
	...
	$proddepartment_id=str_replace("'","",$ex_data[2]);
	...
	$string_search_type=str_replace("'","",$ex_data[10]);
	$unappreq="";
	...
	if($type==1)
	{
		...
		if($proddepartment_id!=0) $proddepartment_id_cond="and a.prod_dept=$proddepartment_id"; else $proddepartment_id_cond="";
		...
		if(trim($brandid)==0) $brandCond=""; else if(trim($brandid)!=0) $brandCond=" and a.brand_id='$brandid' "; else if ($filterBrandId!="") $brandCond=" and a.brand_id in ($filterBrandId)"; else $brandCond="";
	...
	else
	{
		...
		if($proddepartment_id!=0) $proddepartment_id_cond="and prod_dept=$proddepartment_id"; else $proddepartment_id_cond="";
		...
		if(trim($brandid)==0) $brandCond=""; else if(trim($brandid)!=0) $brandCond=" and brand_id='$brandid' "; else if ($filterBrandId!="") $brandCond=" and brand_id in ($filterBrandId)"; else $brandCond="";
		...
		if($string_search_type==1)
		{
			if($style_ref!='') $style_ref_cond=" and a.style_ref='$style_ref'"; else $style_ref_cond="";
			if($cost_sheet_no!='') $cost_sheet_no_cond=" and a.cost_sheet_no='$cost_sheet_no'"; else $cost_sheet_no_cond="";
			if($style_ref!='') $style_ref_cond2=" and style_ref='$style_ref'"; else $style_ref_cond2="";
			if($cost_sheet_no!='') $cost_sheet_no_cond2=" and cost_sheet_no='$cost_sheet_no'"; else $cost_sheet_no_cond2="";
		}
		else if($string_search_type==2)
		{
			if($style_ref!='') $style_ref_cond=" and a.style_ref like '$style_ref%'"; else $style_ref_cond="";
			if($cost_sheet_no!='') $cost_sheet_no_cond=" and a.cost_sheet_no like '$cost_sheet_no%'"; else $cost_sheet_no_cond="";
			if($style_ref!='') $style_ref_cond2=" and style_ref like '$style_ref%'"; else $style_ref_cond2="";
			if($cost_sheet_no!='') $cost_sheet_no_cond2=" and cost_sheet_no like '$cost_sheet_no%'"; else $cost_sheet_no_cond2="";
		}
		else if($string_search_type==3)
		{
			if($style_ref!='') $style_ref_cond=" and a.style_ref like '%$style_ref'"; else $style_ref_cond="";
	...
	$sql_tmp="select tuid, $concat_cond as lib_item_id from qc_template where status_active=1 and is_deleted=0 and lib_item_id!='0' group by tuid order by tuid ASC";
	$sql_tmp_res=sql_select($sql_tmp);
	//print_r($sql_tmp_res);die;
	$template_name_arr=array();
	foreach($sql_tmp_res as $row)
	...
	if($type==1)
	{
		if($approvaltype==2)
		{
			$sql_style = "select a.id, a.cost_sheet_no, a.costing_date, a.buyer_id, a.season_id, a.season_year, a.brand_id, a.body_color_id, a.style_ref, a.prod_dept, a.temp_id, a.lib_item_id, a.stage_id, a.inserted_by, a.updated_by, a.qc_no, a.revise_no, a.option_id, a.approved, c.approval_cause from qc_mst a, qc_confirm_mst b, fabric_booking_approval_cause c where a.qc_no=b.cost_sheet_id and c.booking_id=a.qc_no and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.entry_form=28 and c.approval_type=2 and c.status_active=1 and c.is_deleted=0 and a.entry_form=430
			$buyer_id_cond $style_ref_cond $cost_sheet_no_cond $season_id_cond $proddepartment_id_cond $approvaltype_cond $unappreq $seasonYearCond $brandCond $companyid_cond
			group by a.id, a.cost_sheet_no, a.costing_date, a.buyer_id, a.season_id, a.season_year, a.brand_id, a.body_color_id, a.style_ref, a.prod_dept, a.temp_id, a.lib_item_id, a.stage_id, a.inserted_by, a.updated_by, a.qc_no, a.revise_no, a.option_id, a.approved, c.approval_cause order by a.id Desc";
		}
		else
		{
			...
			$buyer_id_cond $style_ref_cond $cost_sheet_no_cond $season_id_cond $proddepartment_id_cond $approvaltype_cond $seasonYearCond $brandCond $companyid_cond
			group by a.id, a.cost_sheet_no, a.costing_date, a.buyer_id, a.season_id, a.season_year, a.brand_id, a.body_color_id, a.style_ref, a.prod_dept, a.temp_id, a.lib_item_id, a.stage_id, a.inserted_by, a.updated_by, a.qc_no, a.revise_no, a.option_id, a.approved order by a.id Desc";
		}
	}
	else
	{
		$sql_style = "select min(id) as id, cost_sheet_no, max(costing_date) as costing_date, buyer_id, season_id, season_year, brand_id, body_color_id, style_ref, prod_dept, temp_id, lib_item_id, stage_id, max(inserted_by) as inserted_by, max(updated_by) as updated_by, approved, min(qc_no) as qc_no from qc_mst where status_active=1 and is_deleted=0 and entry_form=430 $buyer_id_cond $style_ref_cond2 $cost_sheet_no_cond2 $season_id_cond $proddepartment_id_cond $approvaltype_cond $seasonYearCond $brandCond $companyid_cond group by cost_sheet_no, buyer_id, season_id, season_year, brand_id, body_color_id, style_ref, prod_dept, temp_id, lib_item_id, stage_id, approved order by id Desc";
	...
	foreach($sql_style_result as $row)
	{
        ...
        <tr bgcolor="<?=$bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="tr_<?=$i; ?>" onClick="<?=$jsFunction; ?>"> 
            ...
            <td width="70" style="word-break:break-all"><?=$product_dept[$row[csf("prod_dept")]]; ?>&nbsp;</td>
...
if($action=="populate_style_details_data")
{
	...
	foreach ($data_array as $row)
	{
		...
		echo "get_php_form_data('".trim($row[csf("company_id")])."', 'report_button_setting','requires/quick_costing_woven_controller' ); \n";
	...
	foreach ($sql_result_cons_rate as $rowConsRate)
	{
		...
		{
			$cons_rate_fab_arr[$rowConsRate[csf("item_id")]].=$body_part[$rowConsRate[csf("body_part_id")]].'_'.$rowConsRate[csf("body_part_id")].'_'.$rowConsRate[csf("description")].'_'.$rowConsRate[csf("fab_id")].'_'.$rowConsRate[csf("use_for")].'_'.$rowConsRate[csf("consumption")].'_'.$rowConsRate[csf("ex_percent")].'_'.$rowConsRate[csf("tot_cons")].'_'.$rowConsRate[csf("rate")].'_'.$rowConsRate[csf("fc_per")].'_'.$rowConsRate[csf("value")].'_'.$yarn_count_determin_arr[$rowConsRate[csf("fab_id")]].'_'.$rowConsRate[csf("nominated_supp_multi")].'_'.$nominated_supp_multi.'_'.$rowConsRate[csf("uom")].'##';
		}
	...
	$sql_summ="select buyer_agent_id, location_id, no_of_pack, is_confirm, is_cm_calculative, mis_lumsum_cost, commision_per, tot_fab_cost, tot_sp_operation_cost, tot_wash_cost, tot_accessories_cost, tot_cm_cost, tot_fright_cost, tot_courier_cost, tot_lab_test_cost, tot_miscellaneous_cost, tot_other_cost, tot_commission_cost, tot_cost, tot_fob_cost, tot_rmg_ratio, commercial_per, commercial_cost, inspection_cost, lc_per, lc_cost, operating_exp from qc_tot_cost_summary where mst_id='$cond_data' and status_active=1 and is_deleted=0";
	...
	//qc_confirm_mst
	$qc_confirm_data=sql_select("SELECT id from qc_confirm_mst where status_active=1 and is_deleted=0 and cost_sheet_id='$cost_sheet_id'");
	//echo "SELECT id from qc_confirm_mst where status_active=1 and is_deleted=0 and cost_sheet_id='$cost_sheet_id'"; die;
	if(count($qc_confirm_data)>0){
		echo "$('#confirm_style').css('background-color','#008000');\n";
	}
	else{
		echo "$('#confirm_style').css('background-color','#CBE1FF');\n";
	}
...
{
	$sql_item_id="Select lib_item_id from qc_template where tuid='$data' and status_active=1 and is_deleted=0 and lib_item_id!='0'";
	$lib_item_id="";
...
if($action=="copy_cost_sheet")
{
	...
	{	
		...
		$data_array_mst="(".$mst_id.",'".$cbo_company_id."','".$cbo_location_id."',".$mst_id.",'".$autoCostSheetNo."','".$cbo_temp_id."','".$txt_temp_id."','".$txt_inquery_id."',".$txt_styleRef.",'".$cbo_buyer_id."','".$cbouom."','".$cbo_cons_basis_id."',".$cbo_season_id.",'".$cbo_season_year."','".$cbo_brand."','".$txt_styleDesc."','".$cbo_product_department."','".$txt_product_code."','".$txt_delivery_date."','".$txt_exchangeRate."','".$txt_offerQty."','".$txt_quotedPrice."','".$txt_tgtPrice."','".$txtmarign."','".$cbo_stage_id."','".$txt_costingDate."','".$hid_qc_no."','".$revise_no."','".$option_id."','".$txt_costing_remarks."','".$txt_option_remarks."','".$txtcommercial_cost_method."','".$qcno."',430,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
...
if($action=="confirmStyle_popup")
{
	...
	$print_report_format=return_field_value("format_id"," lib_report_template","template_name ='".$company_id."'  and module_id=2 and report_id=291 and is_deleted=0 and status_active=1");
	$print_report_format = explode(",", $print_report_format);
	$button_id = $print_report_format[0];
 

	$sql_data=sql_select("Select cost_sheet_no, buyer_id, season_id, prod_dept, temp_id, lib_item_id, style_ref, offer_qty, revise_no, option_id, delivery_date, uom from qc_mst where qc_no='$qc_no' ");
	...
	$sql_tmp="select temp_id, $concat_cond as lib_item_id from qc_template where status_active=1 and is_deleted=0 and lib_item_id!='0' group by temp_id order by temp_id ASC";
	$sql_tmp_res=sql_select($sql_tmp);
	//print_r($sql_tmp_res);die;
	$template_name_arr=array();
	foreach($sql_tmp_res as $row)
    ...
    <script>
		...
		function fnc_confirm_entry( operation )
		{
			...
			if(operation==3)
			{
				...
				var qc_fab_kg=0; var qc_fab_mtr=0; var qc_fab_yds=0; var qc_fab_amt=0; var qc_sp_amt=0; var qc_wash_amt=0; var qc_acc_amt=0; var qc_fri_amt=0; var qc_lab_amt=0; var qc_insp_amt=0; var qc_optexp_amt=0; var qc_courier_amt=0; var qc_other_amt=0; var qc_comml_amt=0; var qc_comm_amt=0; var qc_fob_amt=0; var qc_cm_amt=0; var qc_dlc_amt=0; var qc_rmg_amt=0;
				...
				if(job_no!="")
				{
					...
					{
						alert("BOM D. LC Cost is Greater Than QC.");
						release_freezing();
						return;
					}
		...
		function fnc_bom_data_load()
		{
			...
			if(job_no!="")
			{
				...
				var fab_cons_kg=spdata[0]; var fab_cons_mtr=spdata[1]; var fab_cons_yds=spdata[2]; var fab_amount=spdata[3]; var sp_oparation_amount=spdata[4]; var wash_amount=spdata[14]; var acc_amount=spdata[5]; var fright_amount=spdata[6]; var lab_amount=spdata[7]; var optexp_amount=spdata[8]; var other_amount=spdata[9]; var comm_amount=spdata[10]; var fob_amount=spdata[11]; var cm_amount=spdata[12]; var rmg_ratio=spdata[13]; var courier_amount=spdata[15]; var comml_amount=spdata[16]; var insp_amount=spdata[17]; var dlc_amount=spdata[18];
				
				if(qcCons_from==1)
				{
					var bomTotCost=(number_format(fab_amount,4)*1)+(number_format(sp_oparation_amount,4)*1)+(number_format(wash_amount,4)*1)+(number_format(acc_amount,4)*1)+(number_format(fright_amount,4)*1)+(number_format(lab_amount,4)*1)+(number_format(insp_amount,4)*1)+(number_format(optexp_amount,4)*1)+(number_format(other_amount,4)*1)+(number_format(comm_amount,4)*1)+(number_format(courier_amount,4)*1)+(number_format(comml_amount,2)*1)+(number_format(dlc_amount,2)*1);
					
					var bomCost='Fab='+fab_amount+'+ SP='+sp_oparation_amount+'+ Wash='+wash_amount+'+ ACC='+acc_amount+'+ FR='+fright_amount+'+ Lab='+lab_amount+'+ Insp='+insp_amount+'+ Opt. Exp='+optexp_amount+'+ Courier='+courier_amount+'+ Others='+other_amount+'+ Comml. Amt.='+comml_amount+'+ D. LC='+dlc_amount+'+ Comm='+comm_amount+'= Total='+bomTotCost;
		...
		function fnc_consamt_calculate()
		{
			...
			for(j=1; j<=split_tmep_id.length; j++)
			{
				...
				var qcoptexpcostper=($("#tdoptexpamt"+itm_id).text()*1)-( ($("#tdoptexpamt"+itm_id).text()*1)*(($("#txtOptExpCst_"+itm_id).val()*1)/100) );
				...
				var qcdlccostper=($("#tddlcamt"+itm_id).text()*1)-( ($("#tddlcamt"+itm_id).text()*1)*(($("#txtDlcCstPer_"+itm_id).val()*1)/100) );
				...
				$("#txtOptExpCst_"+itm_id).val( number_format(qcoptexpcostper,4,'.','') );
				...
				$("#txtDlcCst_"+itm_id).val( number_format(qcdlccostper,4,'.','') );
		...
		function fnc_total_calculate()
		{
			...
			for(j=1; j<=split_tmep_id.length; j++)
			{
				...
				item_tot_amount=($("#txtFabCst_"+itm_id).val()*1)+($("#txtSpOpa_"+itm_id).val()*1)+($("#txtWash_"+itm_id).val()*1)+($("#txtAcc_"+itm_id).val()*1)+($("#txtFrightCst_"+itm_id).val()*1)+($("#txtLabCst_"+itm_id).val()*1)+($("#txtInspCst_"+itm_id).val()*1)+($("#txtOptExpCst_"+itm_id).val()*1)+($("#txtCourierCst_"+itm_id).val()*1)+($("#txtOtherCst_"+itm_id).val()*1)+($("#txtCommlCst_"+itm_id).val()*1)+($("#txtDlcCst_"+itm_id).val()*1)+($("#txtCommCst_"+itm_id).val()*1);
				
				var itemCost='Fab='+$("#txtFabCst_"+itm_id).val()+'+ SP='+$("#txtSpOpa_"+itm_id).val()+'+ Wash='+$("#txtWash_"+itm_id).val()+'+ ACC='+$("#txtAcc_"+itm_id).val()+'+ FR='+$("#txtFrightCst_"+itm_id).val()+'+ Lab='+$("#txtLabCst_"+itm_id).val()+'+ Insp='+$("#txtInspCst_"+itm_id).val()+'+ OptExp='+$("#txtOptExpCst_"+itm_id).val()+'+ Courier='+$("#txtCourierCst_"+itm_id).val()+'+ Others='+$("#txtOtherCst_"+itm_id).val()+'+ Comml='+$("#txtCommlCst_"+itm_id).val()+'+ DLC='+$("#txtDlcCst_"+itm_id).val()+'+ Comm='+$("#txtCommCst_"+itm_id).val()+'= Total='+item_tot_amount;
			...
			$("#txtDlcCst_qc").val( number_format(qc_dlc_amt,2,'.','') );
		...
		function fncqclimit(str)
		{
			...
			if(qcCons_from==2)
			{
				...
				if( validatetype==21 && tddlcamt < ($("#txtDlcCst_"+item_id).val()*1) )
				{
					alert("QC BOM Limit (D. LC Cost) is Greater Than QC!!!");
		...
		function mail_send(){
			...
			else  
			{
				...
				if(button_id == 2){var action ="quick_costing_print";}
				else if(button_id == 6){var action ="quick_costing_print3";}
				else if(button_id == 45){var action ="quick_costing_print4";}
				else if(button_id == 53){var action ="quick_costing_print5";} 
				var report_title="Quick Costing Woven";
				var data=<?=$qc_no;?>+'*'+<?=$cost_sheet;?>+'*'+report_title;
				window.open("quick_costing_woven_controller.php?data=" + data+'&action='+action+'&is_mail_send=1', true );
			}
		}
    ...
    <div style="display:none"><? echo load_freeze_divs ("../../../",$permission);  ?></div>       
        <form name="confirmStyle_1" id="confirmStyle_1" autocomplete="off">
        	<table width="850">
                <tr>
                    ...
                    <td width="120"><? echo create_drop_down( "cbo_buyer_id", 120, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name", 1, "-Select Buyer-", $sql_data[0][csf('buyer_id')], "load_drop_down( 'quick_costing_woven_controller', this.value, 'load_drop_down_season_conf', 'season_conf_td'); ",1 ); ?></td>
                    ...
                    <td width="90">&nbsp;&nbsp;<strong>Prod. Dept.</strong></td>
                    <td width="100"><?=create_drop_down( "cbo_product_department", 100, $product_dept,"", 1, "-Prod. Dept--",$sql_data[0][csf('prod_dept')], "",1 ); ?></td>
            ...
            <table width="1070" cellspacing="0" border="1" rules="all" class="rpt_table">
        	<thead>
            	<th width="90">Item</th>
                ...
                <th width="50">Insp. Cost</th>
                <th width="50">Opt. Exp.</th>
			...
			<?
            $sql_item_summ="select item_id, fabric_cost, sp_operation_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, courier_cost, lab_test_cost, inspection_cost, operating_exp, other_cost, commercial_cost, lc_cost, commission_cost, fob_pcs, wash_cost  from qc_item_cost_summary where mst_id='$qc_no' and status_active=1 and is_deleted=0";
            ...
            foreach($sql_result_item_summ as $rowItemSumm)
            {
                ...
                <tr id="trper_<?=$z; ?>" bgcolor="<?=$bgcolorP; ?>">
                    ...
                    <td><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtFabConydsPer_<?=$item_id; ?>" id="txtFabConydsPer_<?=$item_id; ?>" onChange="fnc_consamt_calculate();" onBlur="fncqclimit('<?=$item_id.'_3'; ?>');" <?=$disable; ?>/></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtFabCstPer_<?=$item_id; ?>" id="txtFabCstPer_<?=$item_id; ?>" onChange="fnc_consamt_calculate();" onBlur="fncqclimit('<?=$item_id.'_4'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtSpOpaPer_<?=$item_id; ?>" id="txtSpOpaPer_<?=$item_id; ?>" onChange="fnc_consamt_calculate();" onBlur="fncqclimit('<?=$item_id.'_5'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtWashPer_<?=$item_id; ?>" id="txtWashPer_<?=$item_id; ?>" onChange="fnc_consamt_calculate();" onBlur="fncqclimit('<?=$item_id.'_17'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtAccPer_<?=$item_id; ?>" id="txtAccPer_<?=$item_id; ?>" onChange="fnc_consamt_calculate();" onBlur="fncqclimit('<?=$item_id.'_6'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtFrightCstPer_<?=$item_id; ?>" id="txtFrightCstPer_<?=$item_id; ?>" onChange="fnc_consamt_calculate();" onBlur="fncqclimit('<?=$item_id.'_7'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtCourierCstPer_<?=$item_id; ?>" id="txtCourierCstPer_<?=$item_id; ?>" onChange="fnc_consamt_calculate();" onBlur="fncqclimit('<?=$item_id.'_18'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtLabCstPer_<?=$item_id; ?>" id="txtLabCstPer_<?=$item_id; ?>" onChange="fnc_consamt_calculate();" onBlur="fncqclimit('<?=$item_id.'_8'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtInspCstPer_<?=$item_id; ?>" id="txtInspCstPer_<?=$item_id; ?>" onChange="fnc_consamt_calculate();" onBlur="fncqclimit('<?=$item_id.'_20'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtOptExpCstPer_<?=$item_id; ?>" id="txtOptExpCstPer_<?=$item_id; ?>" onChange="fnc_consamt_calculate();" onBlur="fncqclimit('<?=$item_id.'_9'; ?>');" <?=$disable; ?> /></td>
                    
                    
                    <td><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtOtherCstPer_<?=$item_id; ?>" id="txtOtherCstPer_<?=$item_id; ?>" onChange="fnc_consamt_calculate();" onBlur="fncqclimit('<?=$item_id.'_10'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtCommlCstPer_<?=$item_id; ?>" id="txtCommlCstPer_<?=$item_id; ?>" onChange="fnc_consamt_calculate();" onBlur="fncqclimit('<?=$item_id.'_19'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtDlcCstPer_<?=$item_id; ?>" id="txtDlcCstPer_<?=$item_id; ?>" onChange="fnc_consamt_calculate();" onBlur="fncqclimit('<?=$item_id.'_21'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtCommCstPer_<?=$item_id; ?>" id="txtCommCstPer_<?=$item_id; ?>" onChange="fnc_consamt_calculate();" onBlur="fncqclimit('<?=$item_id.'_11'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtFobDznPer_<?=$item_id; ?>" id="txtFobDznPer_<?=$item_id; ?>" onChange="fnc_consamt_calculate();" onBlur="fncqclimit('<?=$item_id.'_12'; ?>');" <?=$disable; ?> /></td>
                    
                    <td title="((CPM*100)/Efficiency)"><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtCpmPer_<?=$item_id; ?>" id="txtCpmPer_<?=$item_id; ?>" value="<? if($cppm==0) echo "0"; else echo number_format($cppm,4); ?>" onChange="fnc_consamt_calculate();" onBlur="fncqclimit('<?=$item_id.'_13'; ?>');" disabled <? //echo $disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtSmvPer_<?=$item_id; ?>" id="txtSmvPer_<?=$item_id; ?>" onChange="fnc_consamt_calculate();" onBlur="fnc_cppm_cal(<?=$item_id; ?>); fncqclimit('<?=$item_id.'_14'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:37px; background-color:<?=$bgcolorP; ?>" type="text" class="text_boxes_numeric" name="txtCmCstPer_<?=$item_id; ?>" id="txtCmCstPer_<?=$item_id; ?>" onChange="fnc_consamt_calculate();" onBlur="fncqclimit('<?=$item_id.'_15'; ?>');" <?=$disable; ?> /></td>
                ...
                <tr id="tr_<?=$z; ?>" bgcolor="<?=$bgcolorN; ?>">
                    ...
                    <td><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtFabConyds_<?=$item_id; ?>" id="txtFabConyds_<?=$item_id; ?>" value="<?=number_format($item_wise_cons_arr[$item_id]['qty_yds'],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_3'; ?>');" <?=$disable; ?>/></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtFabCst_<?=$item_id; ?>" id="txtFabCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("fabric_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_4'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtSpOpa_<?=$item_id; ?>" id="txtSpOpa_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("sp_operation_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_5'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtWash_<?=$item_id; ?>" id="txtWash_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("wash_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_17'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtAcc_<?=$item_id; ?>" id="txtAcc_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("accessories_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_6'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtFrightCst_<?=$item_id; ?>" id="txtFrightCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("frieght_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_7'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtCourierCst_<?=$item_id; ?>" id="txtCourierCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("courier_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_18'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtLabCst_<?=$item_id; ?>" id="txtLabCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("lab_test_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_8'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtInspCst_<?=$item_id; ?>" id="txtInspCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("inspection_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_20'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtOptExpCst_<?=$item_id; ?>" id="txtOptExpCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("operating_exp")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_9'; ?>');" <?=$disable; ?> /></td>
                    
                    
                    <td><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtOtherCst_<?=$item_id; ?>" id="txtOtherCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("other_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_10'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtCommlCst_<?=$item_id; ?>" id="txtCommlCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("commercial_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_19'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtDlcCst_<?=$item_id; ?>" id="txtDlcCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("lc_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_21'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtCommCst_<?=$item_id; ?>" id="txtCommCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("commission_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_11'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtFobDzn_<?=$item_id; ?>" id="txtFobDzn_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("fob_pcs")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_12'; ?>');" <?=$disable; ?> /></td>
                    
                    <td title="((CPM*100)/Efficiency)"><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtCpm_<?=$item_id; ?>" id="txtCpm_<?=$item_id; ?>" value="<? if($cppm==0) echo "0"; else echo number_format($cppm,4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_13'; ?>');" disabled <? //echo $disable; ?> /></td>
                    <td><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtSmv_<?=$item_id; ?>" id="txtSmv_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("smv")],4); ?>" onChange="fnc_total_calculate();" onBlur="fnc_cppm_cal(<?=$item_id; ?>); fncqclimit('<?=$item_id.'_14'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:37px; background-color:<?=$bgcolorN; ?>" type="text" class="text_boxes_numeric" name="txtCmCst_<?=$item_id; ?>" id="txtCmCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("cm_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_15'; ?>');" <?=$disable; ?> /></td>
			...
			$sql="select fab_cons_kg, fab_cons_yds, fab_amount, sp_oparation_amount, acc_amount, fright_amount, courier_amount, lab_amount, inspection_cost, operating_exp, other_amount, commercial_cost, lc_cost, comm_amount, fob_amount, cm_amount, wash_amount, rmg_ratio from qc_confirm_dtls where cost_sheet_id='$cost_sheet_id' and status_active=1 and is_deleted=0";
        ...
        <table width="1070" cellspacing="0" border="1" rules="all" class="rpt_table">
        	<tr id="tr_qc" bgcolor="#CCFFCC">
                <td width="90" title="QC Limit Total"><font color="#0000FF" style="font-size:11px; font-weight:90">Budget Limit Total</font></td>
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtFabConyds_qc" id="txtFabConyds_qc" value="" disabled /></td>
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtFabCst_qc" id="txtFabCst_qc" value="" disabled /></td>
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtSpOpa_qc" id="txtSpOpa_qc" value="" disabled /></td>
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtWash_qc" id="txtWash_qc" value="" disabled /></td>
                
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtAcc_qc" id="txtAcc_qc" value="" disabled /></td>
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtFrightCst_qc" id="txtFrightCst_qc" value="" disabled /></td>
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtCourierCst_qc" id="txtCourierCst_qc" value="" disabled /></td>
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtLabCst_qc" id="txtLabCst_qc" value="" disabled /></td>
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtInspCst_qc" id="txtInspCst_qc" value="" disabled /></td>
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtOptExpCst_qc" id="txtOptExpCst_qc" value="" disabled /></td>
                
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtOtherCst_qc" id="txtOtherCst_qc" value="" disabled /></td>
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtCommlCst_qc" id="txtCommlCst_qc" value="" disabled /></td>
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtDlcCst_qc" id="txtDlcCst_qc" value="" disabled /></td>
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtCommCst_qc" id="txtCommCst_qc" value="" disabled /></td>
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtFobDzn_qc" id="txtFobDzn_qc" value="" disabled /></td>
                
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtCpm_qc" id="txtCpm_qc" value="" disabled /></td>
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtSmv_qc" id="txtSmv_qc" value="" disabled /></td>
                
                <td width="50"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtCmCst_qc" id="txtCmCst_qc" value="" disabled /></td>
        	...
        	<tr id="tr_bom" bgcolor="#CCCCCC">
                <td width="90"><font color="#FF0000">Current BOM</font></td>
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtFabConyds_bom" id="txtFabConyds_bom" value="" readonly /></td>
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtFabCst_bom" id="txtFabCst_bom" value="" readonly /></td>
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtSpOpa_bom" id="txtSpOpa_bom" value="" readonly /></td>
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtWash_bom" id="txtWash_bom" value="" readonly /></td>
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtAcc_bom" id="txtAcc_bom" value="" readonly /></td>
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtFrightCst_bom" id="txtFrightCst_bom" value="" readonly /></td>
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtCourierCst_bom" id="txtCourierCst_bom" value="" readonly /></td>
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtLabCst_bom" id="txtLabCst_bom" value="" readonly /></td>
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtInspCst_bom" id="txtInspCst_bom" value="" readonly /></td>
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtOptExpCst_bom" id="txtOptExpCst_bom" value="" readonly /></td>
                
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtOtherCst_bom" id="txtOtherCst_bom" value="" readonly /></td>
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtCommlCst_bom" id="txtCommlCst_bom" value="" readonly /></td>
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtDlcCst_bom" id="txtDlcCst_bom" value="" readonly /></td>
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtCommCst_bom" id="txtCommCst_bom" value="" readonly /></td>
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtFobDzn_bom" id="txtFobDzn_bom" value="" readonly /></td>
                
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtCpm_bom" id="txtCpm_bom" value="" readonly /></td>
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtSmv_bom" id="txtSmv_bom" value="" readonly /></td>
                <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtCmCst_bom" id="txtCmCst_bom" value="" readonly /></td>
...
if($action=="save_update_delete_confirm_style")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$dtls_field_arr="id, mst_id, cost_sheet_id, item_id, fab_cons_yds, fab_amount, sp_oparation_amount, wash_amount, acc_amount, fright_amount, lab_amount, inspection_cost, operating_exp, courier_amount, other_amount, commercial_cost, lc_cost, comm_amount, fob_amount, cppm_amount, smv_amount, cm_amount, rmg_ratio, fab_cons_yds_per, fab_amt_per, sp_oparation_amt_per, wash_amt_per, acc_amt_per, fright_amt_per, lab_amt_per, inspection_amt_per, opt_exp_per, courier_amt_per, other_amt_per, commercial_amt_per, lc_cost_per, comm_amt_per, fob_amt_per, cppm_amt_per, smv_amt_per, cm_amt_per, rmg_ratio_per, inserted_by, insert_date, status_active, is_deleted";//fab_cons_kg, fab_cons_mtr,
		//echo "10**";
		...
		for($i=1; $i<=count($itm_id); $i++)
		{
			...
			if ($add_comma!=0) $dtls_data_arr .=",";
			$dtls_data_arr .="(".$confirm_dtls_id.",".$confirm_mst_id.",".$txt_costSheet_id.",".$$txtitemid.",".$$txtFabConyds.",".$$txtFabCst.",".$$txtSpOpa.",".$$txtWash.",".$$txtAcc.",".$$txtFrightCst.",".$$txtLabCst.",".$$txtInspCst.",".$$txtOptExpCst.",".$$txtCourierCst.",".$$txtOtherCst.",".$$txtCommlCst.",".$$txtDlcCst.",".$$txtCommCst.",".trim($$txtFobDzn).",".$$txtCpm.",".$$txtSmv.",".$$txtCmCst.",".$$txtPack.",".$$txtFabConydsPer.",".$$txtFabCstPer.",".$$txtSpOpaPer.",".$$txtWashPer.",".$$txtAccPer.",".$$txtFrightCstPer.",".$$txtLabCstPer.",".$$txtInspCstPer.",".$$txtOptExpCstPer.",".$$txtCourierCstPer.",".$$txtOtherCstPer.",".$$txtCommlCstPer.",".$$txtDlcCstPer.",".$$txtCommCstPer.",".trim($$txtFobDznPer).",".$$txtCpmPer.",".$$txtSmvPer.",".$$txtCmCstPer.",".$$txtPackPer.",".$user_id.",'".$pc_date_time."',1,0)";//".$$txtFabConkg.",".$$txtFabConmtr.",
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		$dtls_field_arr="fab_cons_yds*fab_amount*sp_oparation_amount*wash_amount*acc_amount*fright_amount*lab_amount*inspection_cost*operating_exp*courier_amount*other_amount*commercial_cost*lc_cost*comm_amount*fob_amount*cppm_amount*smv_amount*cm_amount*rmg_ratio*fab_cons_yds_per*fab_amt_per*sp_oparation_amt_per*wash_amt_per*acc_amt_per*fright_amt_per*lab_amt_per*inspection_amt_per*opt_exp_per*courier_amt_per*other_amt_per*commercial_amt_per*lc_cost_per*comm_amt_per*fob_amt_per*cppm_amt_per*smv_amt_per*cm_amt_per*rmg_ratio_per*updated_by*update_date";//fab_cons_kg*fab_cons_mtr*
		//echo "10**";
		$itm_id=explode(",",str_replace("'","",$txtItem_id));
		//print_r($itm_id);
		//echo count($itm_id);
		...
		for($i=1; $i<=count($itm_id); $i++)
		{
			...
			$dtls_data_arr[str_replace("'",'',$$txtdtlsupid)] =explode("*",("".$$txtFabConyds."*".$$txtFabCst."*".$$txtSpOpa."*".$$txtWash."*".$$txtAcc."*".$$txtFrightCst."*".$$txtLabCst."*".$$txtInspCst."*".$$txtOptExpCst."*".$$txtCourierCst."*".$$txtOtherCst."*".$$txtCommlCst."*".$$txtDlcCst."*".$$txtCommCst."*".$$txtFobDzn."*".$$txtCpm."*".$$txtSmv."*".$$txtCmCst."*".$$txtPack."*".$$txtFabConydsPer."*".$$txtFabCstPer."*".$$txtSpOpaPer."*".$$txtWashPer."*".$$txtAccPer."*".$$txtFrightCstPer."*".$$txtLabCstPer."*".$$txtInspCstPer."*".$$txtOptExpCstPer."*".$$txtCourierCstPer."*".$$txtOtherCstPer."*".$$txtCommlCstPer."*".$$txtDlcCstPer."*".$$txtCommCstPer."*".$$txtFobDznPer."*".$$txtCpmPer."*".$$txtSmvPer."*".$$txtCmCstPer."*".$$txtPackPer."*'".$user_id."'*'".$pc_date_time."'"));//".$$txtFabConkg."*".$$txtFabConmtr."*
			$k++;
		}
		$flag=1;
		//echo "10**<pre>".$$txtFabConyds;
		//print_r($dtls_data_arr); die;
...
if($action=="populate_confirm_style_form_data")
{
	...
	$sql_confirm_dtls=sql_select("select id, mst_id, cost_sheet_id, item_id, fab_cons_kg, fab_cons_mtr, fab_cons_yds, fab_amount, sp_oparation_amount, wash_amount, acc_amount, fright_amount, lab_amount, inspection_cost, operating_exp, courier_amount, other_amount, commercial_cost, lc_cost, comm_amount, fob_amount, cppm_amount, smv_amount, cm_amount, rmg_ratio, fab_cons_yds_per, fab_amt_per, sp_oparation_amt_per, wash_amt_per, acc_amt_per, fright_amt_per, lab_amt_per, inspection_amt_per, opt_exp_per, courier_amt_per, other_amt_per, commercial_amt_per, lc_cost_per, comm_amt_per, fob_amt_per, cppm_amt_per, smv_amt_per, cm_amt_per, rmg_ratio_per from qc_confirm_dtls where cost_sheet_id='$ex_data[0]'");
	foreach($sql_confirm_dtls as $row_dtls)
	{
		...
		echo "$('#txtOptExpCstPer_".$row_dtls[csf("item_id")]."').val('".$row_dtls[csf("opt_exp_per")]."');\n";
		...
		echo "$('#txtDlcCstPer_".$row_dtls[csf("item_id")]."').val('".$row_dtls[csf("lc_cost_per")]."');\n";
...
if($action=="budgete_cost_validate")
{
	...
	if($job_no!="")
	{
		...
		$sql="select id, job_no, costing_per_id, order_uom_id, fabric_cost, trims_cost, embel_cost, wash_cost, comm_cost, deffdlc_cost, commission, lab_test, inspection, cm_cost, freight, currier_pre_cost, certificate_pre_cost, common_oh, depr_amor_pre_cost, total_cost, price_dzn from wo_pre_cost_dtls where job_no='$job_no' and status_active=1 and is_deleted=0";
		
		$dataArr=sql_select($sql);
		$fab_amount=$sp_oparation_amount=$wash_amount=$acc_amount=$fright_amount=$lab_amount=$optexp_amount=$currier_amount=$other_amount=$comm_amount=$fob_amount=$cm_amount=$rmg_ratio=$comml_cost=$deff_lc_amt=$inspection=0;
		...
		foreach($dataArr as $row)
		{
			...
			$deff_lc_amt+=$row[csf("deffdlc_cost")];
		}
		$str_data=$fab_cons_kg."##".$fab_cons_mtr."##".$fab_cons_yds."##".$fab_amount."##".$sp_oparation_amount."##".$acc_amount."##".$fright_amount."##".$lab_amount."##".$optexp_amount."##".$other_amount."##".$comm_amount."##".$fob_amount."##".$cm_amount."##".$rmg_ratio."##".$wash_amount."##".$currier_amount."##".$comml_cost."##".$inspection."##".$deff_lc_amt;
		unset($dataArr);
	}
...
if($action=='meeting_remarks_popup')
{
	...
	function fn_report_generated(action)
	{
		...
		var data="action="+action+get_submitted_data_string('cbo_buyer_id*cbo_season_id*cbo_product_department*txt_search_common*hide_curr_cost_id' ,"../../../",4);
...
if($action=="report_generate")
{
	...
	$prodDept_id=str_replace("'","",$cbo_product_department);
	...
	if($season_id!=0) $season_id_cond="and a.season_id='$season_id'"; else $season_id_cond="";
	if($prodDept_id!=0) $prodDept_id_cond="and a.prod_dept='$prodDept_id'"; else $prodDept_id_cond="";
	...
	$sql_mst="select a.id as mst_id, a.buyer_id, a.season_id, a.prod_dept, a.style_ref, a.temp_id, a.lib_item_id, a.offer_qty, a.tgt_price, a.quoted_price, a.stage_id, a.option_id, a.revise_no, a.inserted_by, a.updated_by, b.id, b.meeting_no, b.buyer_agent_id, b.location_id, b.meeting_date, $meeting_time as meeting_time, b.remarks from qc_mst a, qc_meeting_mst b where a.qc_no=b.mst_id and a.status_active=1 and a.is_deleted=0 and (b.remarks<>' ' and b.remarks<>'1.' and b.remarks<>'1. ') $buyer_id_cond $season_id_cond $prodDept_id_cond $style_ref_cond order by a.id DESC";
	//echo $sql_mst;
	...
	foreach($sql_mst_arr as $row)
	{
		...
		<tr bgcolor="<? echo $bgcolor; ?>" onClick="jsset_value('<? echo $meeting_data; ?>'); change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>');" id="tr_<? echo $i; ?>" > 
			...
			<td width="80"><? echo $product_dept[$row[csf('prod_dept')]]; ?>&nbsp;</td>
...
if($action=="quick_costing_print")
{
 ...
 //	echo load_html_head_contents("Quick Costing Print", "../../../", 1, 1,'','','');
			...
			foreach ($cons_rate_fab_arr as $fabric_des=>$useForData) 
			{
				foreach($useForData as $useFor=>$val) 
				{
					...
					<tr>
						...
						<td width="50" align="right"><?=$val['fc_per']; ?> &#x25;</td>
	...
	if($is_mail_send==1){
 	    ...
 	    $buyerData = sql_select("select b.USER_EMAIL,a.BRAND_ID from electronic_approval_setup a,USER_PASSWD b where b.id=a.user_id and a.entry_form=45 and a.is_deleted=0 ");
		...
		foreach($sqlResult as $row){
			if($row['USER_EMAIL']){$mailAddress[$row['USER_EMAIL']]=$row['USER_EMAIL'];}
		}
    ...
    <div style="width:1000px;">
        <table width="1000" cellspacing="0" border="1" style="margin-right:-10px;" rules="all" class="rpt_table">
            ...
            <tr>
                ...
                <td width="100"><strong>Prod. Dept.:</strong></td><td width="145px"><? echo $department_arr[$sqlMst_res[0][csf('prod_dept')]]; ?></td>
...
{
	extract($_REQUEST);
	$print_report_format=return_field_value("format_id"," lib_report_template","template_name ='".$data."'  and module_id=2 and report_id=291 and is_deleted=0 and status_active=1");
	$print_report_format_arr=explode(",",$print_report_format);
	//print_r($print_report_format_arr);
	//die; 
		echo "$('#report_btn_1').hide();\n";
		echo "$('#report_btn_3').hide();\n";
		echo "$('#report_btn_4').hide();\n";
		echo "$('#report_btn_5').hide();\n";
	foreach($print_report_format_arr as $id){
		if($id==2){echo "$('#report_btn_1').show();\n";}
		if($id==6){echo "$('#report_btn_3').show();\n";}
		if($id==45){echo "$('#report_btn_4').show();\n";}
		if($id==53){echo "$('#report_btn_5').show();\n";}
	}
	exit();
}
if($action=="quick_costing_print4")
{ 
	extract(check_magic_quote_gpc($_REQUEST));
	$qc_no="";
	$data=explode('*',$data);
	...
	$trim_group=return_library_array( "select item_name,id from lib_item_group", "id", "item_name");
	$color_arr=return_library_array("select id, color_name from lib_color where status_active=1 and is_deleted=0","id","color_name"); 
	$option_arr=return_library_array("select option_id from qc_mst where qc_no='".$qc_no."' and status_active=1 and is_deleted=0 order by cost_sheet_no Desc","option_id","option_id"); 
	...
	$image_location = sql_select("SELECT image_location from common_photo_library where file_type=1 and form_name in ('quotation_inquery_front_image','quotation_inquery_back_image') and master_tble_id='$inquery_id'");
	$data='zz';
	ob_start();
	?>
		...
		<table width="930">			
				...
				<td valign="top">
					<table class="rpt_table" align="left" border="1" cellpadding="1" cellspacing="1" style="width:550px; margin:5px; font-size:14px;" rules="all">
						<tr>
							...
							<td width="100"><?= $company_arr[$company]; ?> </td>
							<td width="80"><strong>Buyer</strong></td>
							<td width="100"><?= $buyer_arr[$buyer_name]; ?></td>
						</tr>
						<tr>
							<td><strong>Cost Sheet No</strong></td>
							<td><?= $cost_sheet_no ?></td>
							<td><strong>Costing Date</strong></td>
							<td><?= change_date_format($costing_date,'yyyy-mm-dd','-'); ?></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td><?= $option_arr[$option_id] ?></td>
						</tr>
						<tr>
							<td><strong>Season</strong></td>
							<td><?= $season_name_arr[$season_id] ?></td>
							<td><strong>Season Year</strong></td>
							<td><?= $season_year ?></td>
						</tr>
						<tr>
							<td><strong>Brand</strong></td>
							<td><?= $brand_name_arr[$brand_id] ?></td>
						...
						<tr>
							...
							<td><?= $style_des ?></td>
							<td><strong>Costing Per</strong></td>
							<td>1 PCS</td>
						</tr>
						<tr>
							<td><strong>Product Item</strong></td>
							<td><?= $garments_item[$gmts_item] ?></td>
							<td><strong>Buyer TGT Price</strong></td>
							<td><?= $tgt_price ?></td>
						</tr>
						<tr>
							...
							<td><?= $color_arr[$body_color_id] ?></td>
						</tr>
						<tr>
							<td><strong>Product Dept.</strong></td>
							<td><?= $product_dept[$prod_dept] ?></td>
							<td><strong>Bulk Est. Ship Date</strong></td>
							<td><?= change_date_format($est_ship_date,'yyyy-mm-dd','-'); ?></td>
						</tr>
					</table>
				</td>
				<td valign="top">
					<table class="rpt_table" align="left" border="1" cellpadding="1" cellspacing="1" style="width:420px; margin:5px; font-size:14px;" rules="all">
						<tr>
							<td colspan="3" align="center" bgcolor="#FFFF00"><strong>Buyer Order Summary</strong></td>
						</tr>
						<tr>
							<td width="140" align="center"><strong>Header</strong></td>
							<td width="140" align="center"><strong>Cost/Pcs</strong></td>
							<td width="140" align="center"><strong>Cost/Order (Value)</strong></td>
						</tr>
						<tr>
							<td><strong>Style Offer Qty -Pcs</strong></td>
							<td><?= $offer_qty ?></td>
							<td></td>
						</tr>
						<tr>
							<td><strong>FOB -Pcs</strong></td>
							<td><?= $fob_pcs ?></td>
							<td><?= number_format($fob_pcs*$offer_qty) ?></td>
						</tr>
						<tr>
							<td><strong>SMV/Pcs [Minutes]</strong></td>
							<td><?= $smv_cost ?></td>
							<td><?= number_format($smv_cost*$offer_qty) ?></td>
						</tr>
						<tr>
							<td><strong>Material Cost</strong></td>
							<td title="without CM & Margin"><?= $buyer_cost=number_format($fob_pcs-($cm_cost+$margin),4) ?></td>
							<td><?= number_format($buyer_cost*$offer_qty) ?></td>
						</tr>						
						<tr>
							<td><strong>Calculative CM</strong></td>
							<td title="((CPM(<?=$cpm; ?>)*SMV(<?=$smv_cost; ?>))/EFI(<?=$efficiency; ?>))*100"><? $calculative_cm=(($cpm*$smv_cost)/$efficiency)*100; echo number_format($calculative_cm,4) ?></td>
							<td><?= number_format($buffer*$offer_qty) ?></td>
						</tr>
						<tr>
							<td><strong>Actual CM</strong></td>
							<td><?= $cm_cost ?></td>
							<td><?= number_format($cm_cost*$offer_qty) ?></td>
						</tr>
						<tr>
							<td width="80"><strong>Margin</strong></td>
							<td width="80"><?= $margin ?></td>
							<td width="80"><?= number_format($margin*$offer_qty) ?></td>
						</tr>
						<tr>
							<td><strong>Marketing Cost/Pcs</strong></td>
							<td title="Material Cost+Actual CM+Margin"><?= $marketing_cost=number_format($buyer_cost+$cm_cost+$margin,4) ?></td>
							<td><?= number_format($marketing_cost*$offer_qty) ?></td>
						</tr>
						<tr>
							<td><strong>E.P.M [USD]</strong></td>
							<td title="CM/SMV"><? $epm=$cm_cost/$smv_cost; echo number_format($epm,2)  ?></td>
							<td><?= number_format($epm*$offer_qty) ?></td>
						</tr>
					</table>
				</td>
			</tr>
			<tr>
				<td colspan="2" align="center" style="font-size: large; color:red"><strong>This Quick Costing is <? if($approved==1){ ?> Approved <? } else{ ?>Not Approved<? } ?></strong></td>
			</tr>
		</table>
			...
			//  print_r($spwork);
			?>
		<table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:1102px; margin-top: 10px" rules="all">
			...
			foreach ($cons_rate_fab_arr as $fabric_des=>$useForData) 
			{
				foreach($useForData as $useFor=>$val) 
				{
					?>
					<tr>
						<td width="330"><?=$fabric_des.",".$cutableArr[$val['fab_id']]['shrinkage_l'].",".$cutableArr[$val['fab_id']]['shrinkage_w']; ?></td>
						<td width="100"><?=$useFor; ?></td>
						<td width="60" align="center" title="<?=$val['fab_id']; ?>"><?=$cutableArr[$val['fab_id']]['cutable_width']; ?></td>
		...
		<div style="width: 1130px; margin-top: 10px; margin-bottom: 10px">
			<div style="width: 260px; float: left;">
				<table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:260px;" rules="all">
					<thead>
						<th colspan="2">Wash Cost Break Up</th>
					</thead>
					<tr>
					...
					?>
					<tr>
						<th align="center">Total Wash Cost</th>
						<th align="right"><?= number_format($totaldznamount,2); ?></th>
					</tr>			
				</table>
                <br>
				<table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:260px;" rules="all">
					<thead>
						<th colspan="2">Special Operation Cost</th>
					</thead>
					<tr>
						<th align="center">Stable</th>
						<th>Cost</th>
					...
					foreach ($spwork as $index => $row) { ?>						
						<tr>
							<td width="200" ><?=$emblishment_name_array[$row["spwork_name"]]; ?></td>
			...
			<div style="float: left;margin-left:3px;">
				<table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:260px;" rules="all">
					...
					<tr>
						...
						foreach ($image_location as $row) { ?>
							<td width="200"><img  src='<?//echo $path;?><? echo base_url($row[csf("image_location")]); ?>' height='200' width='190' /></td>
						<? } ?>
					</tr>			
				</table>
			</div>
...
if($action=="quick_costing_print5")
{
	extract(check_magic_quote_gpc($_REQUEST));
	$qc_no="";
	$data=explode('*',$data);
	...
	$mstSql = "SELECT a.id, a.cost_sheet_id, a.cost_sheet_no, a.temp_id, a.lib_item_id, a.inquery_id, a.style_ref, a.buyer_id, a.uom, a.cons_basis, a.season_id, a.season_year, a.brand_id, a.style_des, a.prod_dept, a.delivery_date, a.exchange_rate, a.offer_qty, a.quoted_price, a.tgt_price, a.stage_id, a.costing_date, a.pre_cost_sheet_id, a.revise_no, a.option_id, a.buyer_remarks, a.option_remarks, a.body_color_id,a.inserted_by, a.margin, c.gmts_item, c.department_name, c.est_ship_date, b.confirm_style, b.approved from qc_mst a left join wo_quotation_inquery c on a.inquery_id=c.id and c.status_active=1 and c.is_deleted=0 left join qc_confirm_mst b on a.qc_no=b.cost_sheet_id where a.cost_sheet_no='$txt_costSheetNo' and a.status_active=1 and a.is_deleted=0 order by a.costing_date";
	//echo $mstSql;  die;
	$master_attribute=array('id', 'cost_sheet_id', 'cost_sheet_no', 'temp_id', 'lib_item_id', 'inquery_id', 'style_ref', 'buyer_id', 'uom', 'cons_basis', 'season_id', 'season_year', 'brand_id', 'style_des', 'prod_dept', 'delivery_date', 'exchange_rate', 'offer_qty', 'quoted_price', 'tgt_price', 'stage_id', 'costing_date', 'pre_cost_sheet_id', 'revise_no', 'option_id', 'buyer_remarks', 'option_remarks', 'body_color_id','inserted_by', 'gmts_item', 'department_name', 'est_ship_date', 'confirm_style', 'approved','margin');
	$master_data=sql_select($mstSql);
	foreach($master_data as $row){
	...
	?>
		<table width="720">			
            <tr>
				<td valign="top">
					<table class="rpt_table" align="left" border="1" cellpadding="1" cellspacing="1" style="width:720px; margin:5px; font-size:14px;" rules="all">
						<tr>
							<td width="100"><strong>Buyer</strong></td>
							<td width="140"><?= $buyer_arr[$buyer_name]; ?></td>
							<td width="100"><strong>Style</strong></td>
							<td width="140"><?= $style_ref; ?> </td>
							<td width="100"><strong>Garments Item</strong></td>
							<td width="140"><?= $garments_item[$gmts_item]; ?></td>
						</tr>
						<tr>
							<td><strong>Cost Sheet No</strong></td>
							<td colspan="5"><?= $cost_sheet_no ?></td>
			...
			<? 
			$conditionSql = "SELECT a.qc_no,a.id, a.cost_sheet_id, a.cost_sheet_no, a.offer_qty, a.costing_date, a.revise_no, a.option_id, a.margin,c.tot_fab_cost, c.tot_sp_operation_cost, c.tot_accessories_cost, c.tot_cm_cost,c.tot_cost,c.tot_fob_cost,c.tot_wash_cost from qc_mst a left join qc_tot_cost_summary c on a.QC_NO=c.MST_ID and c.status_active=1 and c.is_deleted=0 where a.cost_sheet_no='$txt_costSheetNo' and a.status_active=1 and a.is_deleted=0 and a.revise_no=0 and a.option_id=0 order by a.costing_date";

			$codintionSqlRes=sql_select($conditionSql);
			foreach($codintionSqlRes as $row){
				$fabCondition=$row[csf('tot_fab_cost')];
				$embCondition=$row[csf('tot_sp_operation_cost')];
				$accCondition=$row[csf('tot_accessories_cost')];
				$cmCondition=$row[csf('tot_cm_cost')];
				$washCondition=$row[csf('tot_wash_cost')];
				$marginCondition=$row[csf('margin')];
				$totalCondition=$row[csf('tot_fab_cost')]+$row[csf('tot_sp_operation_cost')]+$row[csf('tot_accessories_cost')]+$row[csf('tot_cm_cost')]+$row[csf('tot_wash_cost')]+$row[csf('margin')];
				$totFabCondition=$row[csf('tot_fob_cost')];
				$totValCondition=$row[csf('tot_fob_cost')]*$row[csf('offer_qty')];
			}
			unset($codintionSqlRes);
			$dtlsSql = "SELECT a.qc_no,a.id, a.cost_sheet_id, a.cost_sheet_no, a.offer_qty, a.costing_date, a.revise_no, a.option_id, a.margin,c.tot_fab_cost, c.tot_sp_operation_cost, c.tot_accessories_cost, c.tot_cm_cost,c.tot_cost,c.tot_fob_cost,c.tot_wash_cost, b.approved,b.id as bid from qc_mst a left join qc_tot_cost_summary c on a.QC_NO=c.MST_ID and c.status_active=1 and c.is_deleted=0 left join qc_confirm_mst b on a.qc_no=b.cost_sheet_id where a.cost_sheet_no='$txt_costSheetNo' and a.status_active=1 and a.is_deleted=0 order by a.costing_date";

			$reportSqlRes=sql_select($dtlsSql);
			foreach($reportSqlRes as $row){
				$qc_number.=$row[csf('qc_no')].",";
				$cost_sheet_id.=$row[csf('cost_sheet_id')].",";
				$revise_no_arr[$row[csf('cost_sheet_id')]]['revise_no'] = $row[csf('revise_no')];
				$option_no_arr[$row[csf('cost_sheet_id')]]['option_id'] = $row[csf('option_id')];
			}
			$qc_num=implode(",",array_unique(explode(",",chop($qc_number,","))));
			$cost_sheet_ids=implode(",",array_unique(explode(",",chop($cost_sheet_id,","))));
			$smv_qurey=sql_select("SELECT smv  from qc_item_cost_summary where mst_id in ($qc_num) and status_active=1 and is_deleted=0");
			foreach($smv_qurey as $row){
				$smv_cost=$row[csf('smv')];
			}
			unset($smv_qurey);
			$app_qurey=sql_select(" select b.id, b.approved_by, b.approved_no, b.approved_date, b.un_approved_reason, b.current_approval_status, c.user_full_name, c.designation, d.approval_cause from qc_mst a join approval_history b on a.id=b.mst_id join user_passwd c on b.approved_by=c.id left join fabric_booking_approval_cause d on b.id=d.approval_history_id where a.cost_sheet_no='$txt_costSheetNo' and b.entry_form=36");
			foreach($app_qurey as $row){
				$approved_date=$row[csf('approved_date')];
			}
			unset($app_qurey);
			?>
			<table cellspacing="0" cellpadding="0" border="1" rules="all"  style="vertical-align:top;" class="rpt_table">
			<tr align="center"><th align="left">Costing Date</th>
				<? foreach($reportSqlRes  as $rows)
				{
				if( $rows[csf('costing_date')] == "")  echo "<td>&nbsp</td>";
				else echo "<td>". change_date_format($rows[csf('costing_date')])."</td>";
				} ?>
			</tr>
			
			<tr align="center"><th align="left">Approved Date</th>
				<? foreach($reportSqlRes  as $rows)
				{
				if( $approved_date == "")  echo "<td>&nbsp</td>";
				else echo "<td>". change_date_format($approved_date)."</td>";
				} ?>
			</tr>

			<tr align="center"><th align="left">Costing Status</th>
				<? foreach($reportSqlRes  as $rows)
				{
					if( $rows[csf('revise_no')]==0 && $rows[csf('option_id')]==0){
						echo "<td>Main</td>";
					}
					else if( $rows[csf('revise_no')]==1 && $rows[csf('option_id')]==0){
						echo "<td>Rev-01</td>";
					}
					else if( $rows[csf('revise_no')]==2 && $rows[csf('option_id')]==0){
						echo "<td>Rev-02</td>";
					}
					else if( $rows[csf('revise_no')]==0 && $rows[csf('option_id')]==1){
						echo "<td>OP-01</td>";
					}
					else if( $rows[csf('revise_no')]==0 &&  $rows[csf('option_id')]==2){
						echo "<td>OP-02</td>";
					}
					else if( $rows[csf('revise_no')]==1 &&  $rows[csf('option_id')]==1){
						echo "<td>Rev-01 & OP-01</td>";
					}
				} ?>
			</tr>

			<tr align="center"><th align="left">Confirm Status</th>
				<? foreach($reportSqlRes  as $rows)
				{
					if( $rows[csf('bid')]!=""){
						echo "<td>Confrim</td>";
						...
						echo "<td>Not Confrim</td>";
					}
				} ?>
			</tr>

			<tr align="center"><th align="left">Approved Status</th>
				<? foreach($reportSqlRes  as $rows)
				{
					if( $rows[csf('bid')]!="" && $rows[csf('approved')]==2){
						echo "<td>Un Approve</td>";
					}
					else if( $rows[csf('bid')]!="" && $rows[csf('approved')]==1){
						echo "<td>Un Approve</td>";
					}
					else{
						echo "<td>&nbsp</td>";
			...
			<tr align="center"><th align="left">Offer Qty -Pcs</th>
				<? 
				
				foreach($reportSqlRes  as $rows)
				{
					
				if( $rows[csf('offer_qty')] == "")  echo "<td>&nbsp</td>";
				else echo "<td>". $rows[csf('offer_qty')]."</td>";
				
				} ?>
			</tr>

			<tr align="center"><th align="left">SMV/ Pcs</th>
				<? foreach($reportSqlRes  as $rows)
				{
				if( $smv_cost == "")  echo "<td>&nbsp</td>";
				else echo "<td>". $smv_cost."</td>";
				} ?>
			</tr>

			<tr align="center"><th align="left">Fabric Cost</th>
				<? 
				foreach($reportSqlRes  as $rows)
				{	
					if( $fabCondition==$rows[csf('tot_fab_cost')]){
						echo "<td bgcolor='white'>". $rows[csf('tot_fab_cost')]."</td>";
					}
					else if($fabCondition>$rows[csf('tot_fab_cost')]){
						echo "<td bgcolor='green'>". $rows[csf('tot_fab_cost')]."</td>";
					}
					else if($fabCondition<$rows[csf('tot_fab_cost')]){
						echo "<td bgcolor='red'>". $rows[csf('tot_fab_cost')]."</td>";
					}	
				}
			...
			<tr align="center"><th align="left">Accessories Cost</th>
				<? foreach($reportSqlRes  as $rows)
				{
					if( $accCondition==$rows[csf('tot_accessories_cost')]){
						echo "<td bgcolor='white'>". $rows[csf('tot_accessories_cost')]."</td>";
					}
					else if($accCondition>$rows[csf('tot_accessories_cost')]){
						echo "<td bgcolor='green'>". $rows[csf('tot_accessories_cost')]."</td>";
					}
					else if($accCondition<$rows[csf('tot_accessories_cost')]){
						echo "<td bgcolor='red'>". $rows[csf('tot_accessories_cost')]."</td>";
					}
				} ?>
			</tr>

			<tr align="center"><th align="left">Emb Cost</th>
				<? foreach($reportSqlRes  as $rows)
				{
					if( $embCondition==$rows[csf('tot_sp_operation_cost')]){
						echo "<td bgcolor='white'>". $rows[csf('tot_sp_operation_cost')]."</td>";
					}
					else if($embCondition>$rows[csf('tot_sp_operation_cost')]){
						echo "<td bgcolor='green'>". $rows[csf('tot_sp_operation_cost')]."</td>";
					}
					else if($embCondition<$rows[csf('tot_sp_operation_cost')]){
						echo "<td bgcolor='red'>". $rows[csf('tot_sp_operation_cost')]."</td>";
					}
				} ?>
			</tr>

			<tr align="center"><th align="left">Wash Cost</th>
				<? foreach($reportSqlRes  as $rows)
				{
					if( $washCondition==$rows[csf('tot_wash_cost')]){
						echo "<td bgcolor='white'>". $rows[csf('tot_wash_cost')]."</td>";
					}
					else if($washCondition>$rows[csf('tot_wash_cost')]){
						echo "<td bgcolor='green'>". $rows[csf('tot_wash_cost')]."</td>";
					}
					else if($washCondition<$rows[csf('tot_wash_cost')]){
						echo "<td bgcolor='red'>". $rows[csf('tot_wash_cost')]."</td>";
					}
				} ?>
			</tr>

			<tr align="center"><th align="left">CM Cost</th>
				<? foreach($reportSqlRes  as $rows)
				{
					if( $cmCondition==$rows[csf('tot_cm_cost')]){
						echo "<td bgcolor='white'>". $rows[csf('tot_cm_cost')]."</td>";
					}
					else if($cmCondition>$rows[csf('tot_cm_cost')]){
						echo "<td bgcolor='green'>". $rows[csf('tot_cm_cost')]."</td>";
					}
					else if($cmCondition<$rows[csf('tot_cm_cost')]){
						echo "<td bgcolor='red'>". $rows[csf('tot_cm_cost')]."</td>";
					}
				} ?>
			</tr>

			<tr align="center"><th align="left">Margin</th>
				<? foreach($reportSqlRes  as $rows)
				{
					if( $marginCondition==$rows[csf('margin')]){
						echo "<td bgcolor='white'>". $rows[csf('margin')]."</td>";
					}
					else if($marginCondition>$rows[csf('margin')]){
						echo "<td bgcolor='green'>". $rows[csf('margin')]."</td>";
					}
					else if($marginCondition<$rows[csf('margin')]){
						echo "<td bgcolor='red'>". $rows[csf('margin')]."</td>";
					}
				} ?>
			</tr>

			<tr align="center"><th align="left">Total Cost</th>
				<? foreach($reportSqlRes  as $rows)
				{
					$total_cost=$rows[csf('tot_cm_cost')]+$rows[csf('tot_wash_cost')]+$rows[csf('tot_sp_operation_cost')]+$rows[csf('tot_accessories_cost')]+$rows[csf('tot_fab_cost')]+$rows[csf('margin')];
					if( $totalCondition==$total_cost){
						echo "<td bgcolor='white'>". $total_cost."</td>";
					}
					else if($totalCondition>$total_cost){
						echo "<td bgcolor='green'>". $total_cost."</td>";
					}
					else if($totalCondition<$total_cost){
						echo "<td bgcolor='red'>". $total_cost."</td>";
					}
				} ?>
			</tr>

			<tr align="center"><th align="left">Confirm FOB</th>
				<? foreach($reportSqlRes  as $rows)
				{
					if( $totFabCondition==$rows[csf('tot_fob_cost')]){
						echo "<td bgcolor='white'>". $rows[csf('tot_fob_cost')]."</td>";
					}
					else if($totFabCondition>$rows[csf('tot_fob_cost')]){
						echo "<td bgcolor='green'>". $rows[csf('tot_fob_cost')]."</td>";
					}
					else if($totFabCondition<$rows[csf('tot_fob_cost')]){
						echo "<td bgcolor='red'>". $rows[csf('tot_fob_cost')]."</td>";
					}
				} ?>
			</tr>

			<tr align="center"><th align="left">Offer Value</th>
				<? foreach($reportSqlRes  as $rows)
				{
				$offer_value=$rows[csf('tot_fob_cost')]*$rows[csf('offer_qty')];
				if( $totValCondition==$offer_value){
					echo "<td bgcolor='white'>". $offer_value."</td>";
				}
				else if($totValCondition>$offer_value){
					echo "<td bgcolor='green'>". $offer_value."</td>";
				}
				else if($totValCondition<$offer_value){
					echo "<td bgcolor='red'>". $offer_value."</td>";
				}
				} ?>
			</tr>
		</table>
