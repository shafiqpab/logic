...
if($action=="populate_confirm_fabric_data")
{
    ...
    <table width="700" cellspacing="0" border="1" class="rpt_table" id="tbl_fab" rules="all">
        ...
        <thead>
            ...
            <th width="50">Cons/DZN [Yds]</th>
            <th width="50">Cons/DZN [KG]</th>
        ...
        <tbody id="fabtbody">
			...
			if($ratedata!="")
			{
				...
				foreach($exrdata as $rdata)
				{
					...
					<tr id="fabtr_<?=$i; ?>">
						...
						<td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtcolorconsyds_<?=$i; ?>" id="txtcolorconsyds_<?=$i; ?>" value="<?=$xrdata[4]; ?>" onChange="fnc_cons_amt('<?=$i.'_1'; ?>');" /></td>
            ...
            <tr id="fabtr_<?=$i; ?>">
                ...
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtcolorconsyds_<?=$i; ?>" id="txtcolorconsyds_<?=$i; ?>" value="" onChange="fnc_cons_amt('<?=$i.'_1'; ?>');" /></td>
        ...
        <tfoot>
            <tr bgcolor="#CCCCAA">
                ...
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txttotcons_yds" id="txttotcons_yds" value="<?=$consTotYds; ?>" disabled /></td>
...
if($action=="save_update_delete")
{
	...
	where  b.team_id in (select id from lib_marketing_team a where user_tag_id='$user_id' and a.status_active=1 and a.is_deleted=0 ) and b.status_active=1 and b.is_deleted=0");
		...
		$field_array_consrate="id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, value, fab_id, description, body_part_id, supp_ref, remarks, uom, inserted_by, insert_date, status_active, is_deleted, color_type_id, gsmweight, fabwidth, dznyds, dznkg, rate_data, use_for";
		...
		foreach($temp_id as $item_id)
		{
			...
			foreach($ex_itemConsRateData as $item_wise_consRateData)
			{
				if($item_wise_consRateData!="")
				{
					...
					$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',1,0,'".$conspack."','".$exPer."','".$totCons."','".$ratePop."','".$rate."','".$fabAmt."','".$fabid."','".$fabDesc."','".$bodypartid."','','','".$fabuom."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,'".$colortype."','".$gsmweight."','".$fabwidth."','".$dznyds."','".$dznkg."','".$rateData."','".$usefor."')";
		...
		$data_array_tot_cost="(".$tc_sum_id.",".$qcno.",'".$cbo_buyer_agent."','".$cbo_agent_location."','".$txt_noOfPack."',0,'".$cmPop."','".$txt_lumSum_cost."','".$txt_commPer."','".$totFab_td."','".$totSpc_td."','".$totWash_td."','".$totAcc_td."','".$totCm_td."','".$totFriCst_td."','".$totLbTstCst_td."','".$totMissCst_td."','".$totOtherCst_td."','".$totCommCst_td."','".$totCost_td."','".$totalFob_td."','".$totRmgQty_td."','".$txt_commlPer."','".$totCommlCst_td."','".$txt_optexp."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		//echo $data_array_tot_cost; die;
		$field_array_item_tot=" id, mst_id, item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs, rmg_ratio, commercial_cost, operating_exp, inserted_by, insert_date";
		...
		foreach($ex_item_wise_tot_data as $item_wise_tot)
		{
			...
			$data_array_item_tot .="(".$item_cost_id.",".$qcno.",'".$tot_item_id."','".$fab_td."','".$spOpe_td."','".$wash_td."','".$acc_td."','".$txtCpm."','".$txtSmv."','".$txtEff."','".$txtCmCost."','".$txtFriCost."','".$txtLtstCost."','".$txtMissCost."','".$txtOtherCost."','".$txtCommCost."','".$fobT_td."','".$txtRmgQty."','".$txtCommlCost."','".$txtoptexp."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
...
where  b.team_id in (select id from lib_marketing_team a where user_tag_id='$user_id' and a.status_active=1 and a.is_deleted=0 ) and b.status_active=1 and b.is_deleted=0");
		...
		$field_array_consrate="id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, value, fab_id, description, body_part_id, supp_ref, remarks, uom, gsmweight, fabwidth, dznyds, dznkg, use_for, rate_data, inserted_by, insert_date, status_active, is_deleted, color_type_id";
		
		$fieldArrConsRate="particular_type_id*consumption*ex_percent*tot_cons*is_calculation*rate*value*fab_id*description*body_part_id*supp_ref*remarks*uom*updated_by*update_date*color_type_id*gsmweight*fabwidth*dznyds*dznkg*rate_data*use_for";
		...
		foreach($temp_id as $item_id)
		{
			...
			foreach($ex_itemConsRateData as $item_wise_consRateData)
			{
				if($item_wise_consRateData!="")
				{
					...
					{
						$id_arr[]=$fabupid;
						$fabConsRateUpdateArr[$fabupid] =explode("*",("'0'*'".$conspack."'*'".$exPer."'*'".$totCons."'*'".$ratePop."'*'".$rate."'*'".$fabAmt."'*'".$fabid."'*'".$fabDesc."'*'".$bodypartid."'*''*''*'".$fabuom."'*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'*'".$colortype."'*'".$gsmweight."'*'".$fabwidth."'*'".$dznyds."'*'".$dznkg."'*'".$rateData."'*'".$usefor."'"));
					}
					else if($fabupid==0)
					{
						if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
						$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',1,0,'".$conspack."','".$exPer."','".$totCons."','".$ratePop."','".$rate."','".$fabAmt."','".$fabid."','".$fabDesc."','".$bodypartid."','','','".$fabuom."','".$gsmweight."','".$fabwidth."','".$dznyds."','".$dznkg."','".$usefor."','".$rateData."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,'".$colortype."')";
...
if($action=="style_popup")
{
	...
	{
		show_list_view (document.getElementById('cbo_buyer_id').value+'**'+document.getElementById('cbo_season_id').value+'**'+document.getElementById('cbo_subDept_id').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('txt_cost_sheet_no').value+'**'+document.getElementById('hide_dataType').value+'**'+document.getElementById('cbo_approval_type').value, 'style_ref_search_list_view', 'search_div', 'short_quotation_v3_controller', 'setFilterGrid(\'tbl_list_search\',-1)');
		set_all();
	}
...
if($action=="temp_style_list_view")
{
	...
	$sql_temp_style=sql_select("select a.id, a.cost_sheet_no, a.style_ref, a.season_id, a.inserted_by, a.updated_by, a.qc_no from qc_mst a, qc_style_temp_data b where a.id=b.mst_id and b.inserted_by='$user_id' and a.entry_form=471 and b.status_active=1 and b.is_deleted=0 order by a.style_ref ASC");
	...
	{  
		...
		?>
        <tr id="tr_<?=$row[csf('qc_no')]; ?>" bgcolor="<?=$bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="change_color_tr('<?=$row[csf('qc_no')]; ?>','<?=$bgcolor; ?>'); set_onclick_style_list('<?=$row[csf('qc_no')].'__'.$row[csf('inserted_by')].'__'.$row[csf('cost_sheet_no')].'__25'; ?>')"> <!--change_color('tr_<?// echo $i; ?>','<?// echo $bgcolor;?>');-->
        	<td width="90" style="word-break:break-all"><?=$row[csf('style_ref')]; ?>&nbsp;</td>
            <td width="70" style="word-break:break-all"><?=$season_arr[$row[csf('season_id')]]; ?>&nbsp;</td>
            <td style="word-break:break-all" title="<? //echo $user_arr[$row[csf('updated_by')]]; ?>"><?=$user_name; ?>&nbsp;</td>
        <?
		$i++;
	}
...
if($action=="populate_style_details_data")
{
	...
	foreach ($sql_result_cons_rate as $rowConsRate)
	{
		...
		{
			$cons_rate_fab_arr[$rowConsRate[csf("item_id")]].=$body_part[$rowConsRate[csf("body_part_id")]].'~!~'.$rowConsRate[csf("body_part_id")].'~!~'.$rowConsRate[csf("description")].'~!~'.$rowConsRate[csf("fab_id")].'~!~'.$rowConsRate[csf("color_type_id")].'~!~'.$rowConsRate[csf("gsmweight")].'~!~'.$rowConsRate[csf("fabwidth")].'~!~'.$rowConsRate[csf("use_for")].'~!~'.$rowConsRate[csf("dznyds")].'~!~'.$rowConsRate[csf("dznkg")].'~!~'.$rowConsRate[csf("consumption")].'~!~'.$rowConsRate[csf("ex_percent")].'~!~'.$rowConsRate[csf("tot_cons")].'~!~'.$rowConsRate[csf("is_calculation")].'~!~'.$rowConsRate[csf("rate_data")].'~!~'.$rowConsRate[csf("rate")].'~!~'.$rowConsRate[csf("value")].'~!~'.$rowConsRate[csf("id")].'~!~'.$rowConsRate[csf("uom")].'#^';
		}
...
if($action=="copy_cost_sheet")
{
	...
	{	
		...
		$data_array_mst="(".$mst_id.",".$mst_id.",'".$autoCostSheetNo."','".$cbo_temp_id."','".$txt_temp_id."','".$txt_inquery_id."',".$txt_styleRef.",'".$cbo_buyer_id."','".$cbouom."','".$cbo_cons_basis_id."',".$cbo_season_id.",'".$cbo_season_year."','".$cbo_brand."','".$txt_styleDesc."','".$cbo_product_department."','".$txt_product_code."','".$txtmarign."','".$cbo_subDept_id."','".$txt_delivery_date."','".$txt_exchangeRate."','".$txt_offerQty."','".$txt_quotedPrice."','".$txt_tgtPrice."','".$cbo_stage_id."','".$txt_costingDate."','".$hid_qc_no."','".$revise_no."','".$option_id."','".$txt_costing_remarks."','".$txt_option_remarks."','".$txt_article."','".$qcno."',471,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		...
		$cons_rate_id=return_next_id("id", "qc_cons_rate_dtls", 1);
		$sql_consrate=sql_select("select id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, rate, value, fab_id, description, use_for, fc_per, body_part_id, supp_ref from qc_cons_rate_dtls where mst_id='$hid_qc_no' order by id ASC");
		...
		foreach( $sql_consrate as $rowConsrate )
		{
			...
			if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
			$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."','".$type."','".$head_id."','".$cons."','".$ex_percent."','".$tot_cons."','".$rate."','".$tot_val."','".$fab_id."','".$description."','".$use_for."','".$fc_per."','".$body_part_id."','".$supp_ref."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		...
		$data_array_tot_cost="(".$tc_sum_id.",".$qcno.",'".$cbo_buyer_agent."','".$cbo_agent_location."','".$txt_noOfPack."',0,'".$cmPop."','".$txt_lumSum_cost."','".$txt_commPer."','".$totFab_td."','".$totSpc_td."','".$totWash_td."','".$totAcc_td."','".$totCm_td."','".$totFriCst_td."','".$totLbTstCst_td."','".$totMissCst_td."','".$totOtherCst_td."','".$totCommCst_td."','".$totCost_td."','".$totalFob_td."','".$totalRmg_td."','".$txt_commlPer."','".$totCommlCst_td."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		//echo $data_array_tot_cost; die;
		$sql_item_tot=sql_select("select id, mst_id, item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs, rmg_ratio, commercial_cost from qc_item_cost_summary where mst_id='$hid_qc_no' order by id ASC");
		
		//$field_array_item_tot=" id, mst_id, item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs, rmg_ratio, commercial_cost inserted_by, insert_date";
		...
		{
			$fab_td=$spOpe_td=$wash_td=$acc_td=$txtCpm=$txtSmv=$txtEff=$txtCmCost=$txtFriCost=$txtLtstCost=$txtMissCost=$txtOtherCost=$txtCommCost=$fobT_td=$tot_item_id=$rmg_ratio=0;
			...
			if ($add_comma_item_tot!=0) $data_array_item_tot .=",";
			$data_array_item_tot .="(".$item_cost_id.",".$qcno.",'".$tot_item_id."','".$fab_td."','".$spOpe_td."','".$wash_td."','".$acc_td."','".$txtCpm."','".$txtSmv."','".$txtEff."','".$txtCmCost."','".$txtFriCost."','".$txtLtstCost."','".$txtMissCost."','".$txtOtherCost."','".$txtCommCost."','".$fobT_td."','".$rmg_ratio."','".$txtCommlCost."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
    ...
    <script>
		...
		function fnc_confirm_entry( operation )
		{
			if(operation==3)
			{
				...
				var qc_fab_kg=0; var qc_fab_mtr=0; var qc_fab_yds=0; var qc_fab_amt=0; var qc_sp_amt=0; var qc_acc_amt=0; var qc_fri_amt=0; var qc_lab_amt=0; var qc_misce_amt=0; var qc_other_amt=0; var qc_comm_amt=0; var qc_fob_amt=0; var qc_cm_amt=0; var qc_rmg_amt=0;
		...
		function fnc_bom_data_load()
		{
			...
			if(job_no!="")
			{
				...
				if(qcCons_from==1)
				{
					var bomTotCost=(number_format(fab_amount,4)*1)+(number_format(sp_oparation_amount,4)*1)+(number_format(acc_amount,4)*1)+(number_format(fright_amount,4)*1)+(number_format(lab_amount,4)*1)+(number_format(misce_amount,4)*1)+(number_format(other_amount,4)*1)+(number_format(comm_amount,4)*1);
					
					var bomCost='Fab='+fab_amount+'+ SP='+sp_oparation_amount+'+ ACC='+acc_amount+'+ FR='+fright_amount+'+ Lab='+lab_amount+'+ Mis='+misce_amount+'+ Others='+other_amount+'+ Comm='+comm_amount+'= Total='+bomTotCost;
		...
		function fnc_total_calculate()
		{
			...
			for(j=1; j<=split_tmep_id.length; j++)
			{
				...
				var itemCost='Fab='+$("#txtFabCst_"+itm_id).val()+'+ SP='+$("#txtSpOpa_"+itm_id).val()+'+ ACC='+$("#txtAcc_"+itm_id).val()+'+ FR='+$("#txtFrightCst_"+itm_id).val()+'+ Lab='+$("#txtLabCst_"+itm_id).val()+'+ Mis='+$("#txtMiscCst_"+itm_id).val()+'+ Others='+$("#txtOtherCst_"+itm_id).val()+'+ Comm='+$("#txtCommCst_"+itm_id).val()+'= Total='+item_tot_amount;
        ...
        <form name="confirmStyle_1" id="confirmStyle_1" autocomplete="off">
        	<table width="850">
                <tr>
                    ...
                    <td width="120"><? echo create_drop_down( "cbo_buyer_id", 120, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name", 1, "-Select Buyer-", $sql_data[0][csf('buyer_id')], "load_drop_down( 'short_quotation_v3_controller', this.value, 'load_drop_down_season_conf', 'season_conf_td'); load_drop_down( 'short_quotation_v3_controller',this.value, 'load_drop_down_sub_depConf', 'subConf_td' );",1 ); ?></td>
...
if($action=="save_update_delete_confirm_style")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$dtls_field_arr="id, mst_id, cost_sheet_id, item_id, fab_cons_kg, fab_cons_mtr, fab_cons_yds, fab_amount, sp_oparation_amount, acc_amount, fright_amount, lab_amount, misce_amount, other_amount, comm_amount, fob_amount, cppm_amount, smv_amount, cm_amount, rmg_ratio, inserted_by, insert_date";
		//echo "10**";
		...
		for($i=1; $i<=count($itm_id); $i++)
		{
			...
			if ($add_comma!=0) $dtls_data_arr .=",";
			$dtls_data_arr .="(".$confirm_dtls_id.",".$confirm_mst_id.",".$txt_costSheet_id.",".$$txtitemid.",".$$txtFabConkg.",".$$txtFabConmtr.",".$$txtFabConyds.",".$$txtFabCst.",".$$txtSpOpa.",".$$txtAcc.",".$$txtFrightCst.",".$$txtLabCst.", ".$$txtMiscCst.", ".$$txtOtherCst.", ".$$txtCommCst.", ".trim($$txtFobDzn).",".$$txtCpm.",".$$txtSmv.",".$$txtCmCst.",".$$txtPack.",".$user_id.",'".$pc_date_time."')";
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		$dtls_field_arr="fab_cons_kg*fab_cons_mtr*fab_cons_yds*fab_amount*sp_oparation_amount*acc_amount*fright_amount*lab_amount*misce_amount*other_amount*comm_amount*fob_amount*cppm_amount*smv_amount*cm_amount*rmg_ratio*updated_by*update_date";
		//echo "10**";
		$itm_id=explode(",",str_replace("'","",$txtItem_id));
		//print_r($itm_id);
		//echo count($itm_id);
		...
		for($i=1; $i<=count($itm_id); $i++)
		{
			...
			$dtls_data_arr[str_replace("'",'',$$txtdtlsupid)] =explode("*",("".$$txtFabConkg."*".$$txtFabConmtr."*".$$txtFabConyds."*".$$txtFabCst."*".$$txtSpOpa."*".$$txtAcc."*".$$txtFrightCst."*".$$txtLabCst."*".$$txtMiscCst."*".$$txtOtherCst."*".$$txtCommCst."*".$$txtFobDzn."*".$$txtCpm."*".$$txtSmv."*".$$txtCmCst."*".$$txtPack."*'".$user_id."'*'".$pc_date_time."'"));
			$k++;
		}
		$flag=1;
...
if($action=="budgete_cost_validate")
{
	...
	if($job_no!="")
	{
		...
		$dataArr=sql_select($sql);
		$fab_amount=$sp_oparation_amount=$acc_amount=$fright_amount=$lab_amount=$misce_amount=$other_amount=$comm_amount=$fob_amount=$cm_amount=$rmg_ratio=0;
		...
		$str_data=$fab_cons_kg."##".$fab_cons_mtr."##".$fab_cons_yds."##".$fab_amount."##".$sp_oparation_amount."##".$acc_amount."##".$fright_amount."##".$lab_amount."##".$misce_amount."##".$other_amount."##".$comm_amount."##".$fob_amount."##".$cm_amount."##".$rmg_ratio;
		unset($dataArr);
	}
		...
		<fieldset>
            <table cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr>
                        <td align="center">
                        	 <? echo create_drop_down( "cbo_buyer_id", 120, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name",1, "-- All Buyer--",$selected,"load_drop_down( 'short_quotation_v3_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'short_quotation_v3_controller',this.value, 'load_drop_down_sub_dep', 'sub_td' );",0 ); ?>
                        </td>                 
                        <td id="season_td"><? echo create_drop_down( "cbo_season_id", 130, $blank_array,'', 1, "-- Select Season--",$selected, "" ); ?></td>
                        <td id="sub_td"><? echo create_drop_down( "cbo_subDept_id", 130, $blank_array,'', 1, "-- Select Dept--",$selected, "" ); ?></td>     
            	...
            	<td width="460" valign="top">
                	<table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:460px;" rules="all">
                        ...
                        <tr>
                            ...
                            <td width="70" align="center"><?=$sqlSumData[0][csf('tot_fab_cost')]; ?></td>
                        ...
                        <tr>
                            ...
                            <td width="70" align="center"><?=$sqlSumData[0][csf('tot_accessories_cost')]; ?></td>
                        ...
                        <tr>
                            ...
                            <td width="70" align="center"><?=($sqlSumData[0][csf('tot_sp_operation_cost')]+$sqlSumData[0][csf('tot_wash_cost')]); ?></td>
		...
		<table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:930px; margin-top: 10px" rules="all">
			<thead>
            	...
            	<tr>
                    ...
                    <th width="45">UOM</th>
                    <th width="50">Cons/Dzn [Yds/MTR]</th>
                    <th width="45">Cons/Dzn [Kg]</th>
                    <th width="45">Cons/Pack [Kg]</th>
                    <th width="45">Ex. %</th>
                    <th width="45">Ttl Cons.</th>
				...
				if($val[csf('type')]==1 && ($val[csf('tot_cons')]*1>0))
				{
					...
					if($val[csf('is_calculation')]==1 && trim($val[csf('rate_data')])!="")
					{
						...
						foreach($exratestrdata as $ratestrval)
						{
							...
							<tr>
								...
								<td width="120"><?=$val[csf('description')]; ?></td>
								...
								<td width="50" align="right"><?=number_format($exrdata[4],4); ?></td>
					...
					else
					{
						...
						<tr>
							...
							<td width="60"><? //=$val[csf('use_for')]; ?>&nbsp;</td>
                            <td width="45"><?=$unit_of_measurement[$val[csf('uom')]]; ?></td>
							<td width="50" align="right"><?=number_format($val[csf('dznyds')],4); ?></td>
            ...
            <tr bgcolor="#CCCCCC">
            	<td width="60">&nbsp;</td>
                <td width="70">&nbsp;</td>
                <td width="120">Total Fabric Cost:</td>
...
{
	echo create_drop_down( "cbo_buyer_name", 130, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90))  order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down( 'short_quotation_v3_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'short_quotation_v3_controller', this.value, 'load_drop_down_brand', 'brand_td');" );
	exit();
}
