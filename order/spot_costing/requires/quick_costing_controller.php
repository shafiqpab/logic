...
if ($action=="load_drop_down_option_id")
{
	...
	if($ex_data[1]!=0)
		...
		$selectopt=$max;
	//echo $sql;
	echo create_drop_down( "cbo_option_id", 45, $rvs,"", 0, "-0-", $selectopt, "load_drop_down( 'requires/quick_costing_controller', document.getElementById('txt_costSheetNo').value+'__'+this.value+'__'+0, 'load_drop_down_revise_no', 'revise_td'); fnc_option_rev( document.getElementById('cbo_revise_no').value+'***'+this.value+'***2' );" );//
	exit();
}
...
{
	echo  create_list_view ( "list_view", "Stage Name", "200","200","150",1, "select tuid, stage_name from lib_stage_name where status_active=1 and is_deleted=0 order by tuid desc", "get_php_form_data", "tuid","'load_php_data_to_form_satge'", 1, "0", $arr, "stage_name","quick_costing_controller", 'setFilterGrid("list_view",-1);','0' );
	exit();
}
...
if($action=="save_update_delete")
{
	...
	$msgbuyer="This buyer is InActive/Cancelled, Any Change Not Allowed.";
	if (is_duplicate_field( "buyer_name", "lib_buyer", "id=$cbo_buyer_id and status_active!=1" ) == 1)
	{
		echo "88**".$msgbuyer; //ISD-23-25572
		die;
	}
	...
	where  b.team_id in (select id from lib_marketing_team a where user_tag_id='$user_id' and a.status_active=1 and a.is_deleted=0 ) and b.status_active=1 and b.is_deleted=0");
		...
		$field_array_consrate="id, mst_id, item_id, type, particular_type_id, formula, consumption, ex_percent, tot_cons, is_calculation, rate, rate_data, value, inserted_by, insert_date";
		...
		foreach($temp_id as $item_id)
		{
			...
			foreach($ex_itemConsRateData as $item_wise_consRateData)
			{
				if($item_wise_consRateData!="")
				{
					...
					$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',1,'".$head_id."',0,'".$cons."','".$exPer."','".$totCons."','".$is_rate_cal."','".$rate."','".$rateData."','".$tot_val."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			...
			foreach($ex_specialData as $item_wise_specialData)
			{
				...
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',2,'".$speciaOperation_id."',0,'".$spConsumtion."','".$spExper."','".$totSpCons."','','".$spRate."','','".$tot_spVal."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			...
			foreach($ex_itemAccData as $item_wise_itemAccData)
			{
				...
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',3,'".$accessories_id."',0,'".$acConsumtion."','".$acExPer."','".$totAccCons."',0,'".$acRate."','','".$tot_acVal."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		$field_array_item_tot=" id, mst_id, item_id, fabric_cost, sp_operation_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs, rmg_ratio, inserted_by, insert_date";
		...
		{
			$fab_td=$spOpe_td=$acc_td=$txtCpm=$txtSmv=$txtEff=$txtCmCost=$txtFriCost=$txtLtstCost=$txtMissCost=$txtOtherCost=$txtCommCost=$fobT_td=$tot_item_id=0;
			...
			$tot_item_id=trim($ex_item_tot[14]);
			
			if ($add_comma_item_tot!=0) $data_array_item_tot .=",";
			$data_array_item_tot .="(".$item_cost_id.",".$qcno.",'".$tot_item_id."','".$fab_td."','".$spOpe_td."','".$acc_td."','".$txtCpm."','".$txtSmv."','".$txtEff."','".$txtCmCost."','".$txtFriCost."','".$txtLtstCost."','".$txtMissCost."','".$txtOtherCost."','".$txtCommCost."','".$fobT_td."','".$txtRmgQty."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
...
where  b.team_id in (select id from lib_marketing_team a where user_tag_id='$user_id' and a.status_active=1 and a.is_deleted=0 ) and b.status_active=1 and b.is_deleted=0");
		...
		$field_array_mst="temp_id*lib_item_id*style_ref*buyer_id*uom*cons_basis*season_id*style_des*department_id*delivery_date*exchange_rate*offer_qty*quoted_price*tgt_price*stage_id*costing_date*buyer_remarks*option_remarks*updated_by*update_date";
		
		//$txt_temp_id=explode(",",str_replace("'","",$txt_temp_id));
		//asort($txt_temp_id);
		//$txt_temp_id="'".implode(",",$txt_temp_id)."'";
		
		$data_array_mst="".$cbo_temp_id."*".$txt_temp_id."*'".strtoupper(str_replace("'","",$txt_styleRef))."'*".$cbo_buyer_id."*".$cbouom."*".$cbo_cons_basis_id."*".$cbo_season_id."*'".strtoupper(str_replace("'","",$txt_styleDesc))."'*".$cbo_subDept_id."*".$txt_delivery_date."*".$txt_exchangeRate."*".$txt_offerQty."*".$txt_quotedPrice."*".$txt_tgtPrice."*".$cbo_stage_id."*".$txt_costingDate."*'".strtoupper($txt_costing_remarks)."'*'".strtoupper($txt_option_remarks)."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
...
if($action=="style_popup")
{
	...
	$buyerId=$ex_data[0];
	$type=$ex_data[1];
	$buyer_data_arr=sql_select("select buy.id, buy.buyer_name , buy.status_active from lib_buyer buy where 1=1 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name");
	foreach ($buyer_data_arr as $row) {
		if($row[csf('status_active')]==1)
		{
			$qc_buyer_arr[$row[csf('id')]] = $row[csf('buyer_name')];
		}
		else{
			$qc_buyer_arr[$row[csf('id')]] = $row[csf('buyer_name')].' ('.$row_status[$row[csf('status_active')]].")";
		}
	...
	{
		show_list_view (document.getElementById('cbo_buyer_id').value+'**'+document.getElementById('cbo_season_id').value+'**'+document.getElementById('cbo_subDept_id').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('txt_cost_sheet_no').value+'**'+document.getElementById('hide_dataType').value+'**'+document.getElementById('cbo_approval_type').value+'**'+document.getElementById('cbo_string_search_type').value, 'style_ref_search_list_view', 'search_div', 'quick_costing_controller', 'setFilterGrid(\'tbl_list_search\',-1)');
		set_all();
		...
		if(tbl_row_count==1)
		{
			var auto_generatepopup=$('#hiddconfirmpopupdata_1').val();
			var str=auto_generatepopup.split(",");
			var qcno=str[0];
			var upid=str[1];
			fnc_confirm_style(qcno,upid)
		}
	}
	...
	function fnc_confirm_style(qcno,upid)
	{
		if(upid!="")
		{
			...
			var page_link='quick_costing_controller.php?action=confirmStyle_popup';
    ...
    <div align="center">
		...
		<fieldset>
            <table cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr class="general">
                        <td><? echo create_drop_down( "cbo_buyer_id", 130, $qc_buyer_arr,"",1, "-- All Buyer--",$buyerId,"load_drop_down( 'quick_costing_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'quick_costing_controller',this.value, 'load_drop_down_sub_dep', 'sub_td' );",0 ); ?></td>                 
...
if($action=="style_ref_search_list_view")
{
	...
	foreach($sql_style_result as $row)
	{
        ...
        <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="tr_<? echo $i; ?>" onClick="<? echo $jsFunction; ?>"> 
            ...
            <td width="100" style="word-break:break-all"><?php echo $qc_buyer_arr[$row[csf("buyer_id")]]; ?></td>
            ...
            <td width="100" style="word-break:break-all"><?=$unapp_link; ?> <?=$row[csf("style_ref")]; ?><?=$unapp_endlink; ?>
            <input type="hidden"  id="hiddconfirmpopupdata_<?=$i; ?>" value="<?=$row[csf('qc_no')].','.$row[csf('id')]; ?>"/></td>
    ...
    <input type="hidden" name="txt_select_row_id" id="txt_select_row_id" value=""/>
    <? if($type!=1) { ?>
...
if($action=="temp_style_list_view")
{
	...
	if($ex_data[1]==1)
	{
		...
		$sql_style= "select min(id) as id, cost_sheet_no, style_ref, min(qc_no) as qc_no, season_id, inserted_by from qc_mst where cost_sheet_no in ($ex_data[0]) and cost_sheet_no not in (select cost_sheet_no from qc_style_temp_data where cost_sheet_no in ($ex_data[0]) and inserted_by='$user_id' and status_active=1 and is_deleted=0 ) and revise_no=0 and option_id=0 and status_active=1 and is_deleted=0 group by cost_sheet_no, style_ref, season_id, inserted_by";
		//echo  $sql_style;
	...
	$sql_temp_style=sql_select("select a.id, a.cost_sheet_no, a.style_ref, a.season_id, a.inserted_by, a.updated_by, a.qc_no from qc_mst a, qc_style_temp_data b where a.id=b.mst_id and b.inserted_by='$user_id' and b.status_active=1 and b.is_deleted=0 order by a.style_ref ASC");
...
if($action=="populate_style_details_data")
{
	...
	$data_array=sql_select("select id, cost_sheet_id, cost_sheet_no, temp_id, lib_item_id, style_ref, buyer_id, uom, cons_basis, season_id, style_des, department_id, delivery_date, exchange_rate, offer_qty, quoted_price, tgt_price, stage_id, costing_date, revise_no, option_id, buyer_remarks, option_remarks, inserted_by, meeting_no, qc_no from qc_mst where status_active=1 and is_deleted=0 and cost_sheet_no='$cost_sheet_no' $rev_no_cond $option_id_cond");//$starting_row_cond and inserted_by='$cond_user'  qc_no='$cost_sheet_id'   
	...
	$sql_cons_rate="select id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, rate_data, value from qc_cons_rate_dtls where mst_id=$cond_data and status_active=1 and is_deleted=0  order by id asc";
	...
	foreach ($sql_result_cons_rate as $rowConsRate)
	{
		...
		{
			$cons_rate_ac_arr[$rowConsRate[csf("item_id")]].=$rowConsRate[csf("particular_type_id")].'_'.$rowConsRate[csf("consumption")].'_'.$rowConsRate[csf("ex_percent")].'_'.$rowConsRate[csf("rate")].'_'.$rowConsRate[csf("tot_cons")].'##';
		}
	}
	...
	$sql_item_summ="select item_id, fabric_cost, sp_operation_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs, rmg_ratio from qc_item_cost_summary where mst_id='$cond_data' and status_active=1 and is_deleted=0";
...
if($action=="copy_cost_sheet")
{
	...
	{	
		...
		$qcno=$_SESSION['logic_erp']['user_id'].'000'.$mst_id;
		$sql_mst=sql_select("select id, cost_sheet_id, cost_sheet_no, temp_id, lib_item_id, style_ref, buyer_id, uom, cons_basis, season_id, style_des, department_id, delivery_date, exchange_rate, offer_qty, quoted_price, tgt_price, stage_id, costing_date, buyer_remarks, company_id from qc_mst where qc_no='$hid_qc_no'");
		...
		$txt_costingDate=$pc_date;//$sql_mst[0][csf('costing_date')];
		$txt_costing_remarks=$sql_mst[0][csf('buyer_remarks')];
		$old_id=$sql_mst[0][csf('id')];
		
		
		$field_array_mst=" id, cost_sheet_id, cost_sheet_no, temp_id, lib_item_id, style_ref, buyer_id, uom, cons_basis, season_id, style_des, department_id, delivery_date, exchange_rate, offer_qty, quoted_price, tgt_price, stage_id, costing_date, buyer_remarks, pre_cost_sheet_id, revise_no, option_id, qc_no, inserted_by, insert_date";
		...
		$data_array_tot_cost="(".$tc_sum_id.",".$qcno.",'".$cbo_buyer_agent."','".$cbo_agent_location."','".$txt_noOfPack."',0,'".$cmPop."','".$txt_lumSum_cost."','".$txt_commPer."','".$totFab_td."','".$totSpc_td."','".$totAcc_td."','".$totCm_td."','".$totFriCst_td."','".$totLbTstCst_td."','".$totMissCst_td."','".$totOtherCst_td."','".$totCommCst_td."','".$totCost_td."','".$totalFob_td."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		//echo $data_array_tot_cost; die;
		$sql_item_tot=sql_select("select id, mst_id, item_id, fabric_cost, sp_operation_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs from qc_item_cost_summary where mst_id='$hid_qc_no' order by id ASC");
		$field_array_item_tot=" id, mst_id, item_id, fabric_cost, sp_operation_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs, inserted_by, insert_date";
		$item_cost_id=return_next_id("id", "qc_item_cost_summary", 1);
		$add_comma_item_tot=0; $data_array_item_tot="";
		foreach($sql_item_tot as $rowItem_tot)
		{
			...
			if ($add_comma_item_tot!=0) $data_array_item_tot .=",";
			$data_array_item_tot .="(".$item_cost_id.",".$qcno.",'".$tot_item_id."','".$fab_td."','".$spOpe_td."','".$acc_td."','".$txtCpm."','".$txtSmv."','".$txtEff."','".$txtCmCost."','".$txtFriCost."','".$txtLtstCost."','".$txtMissCost."','".$txtOtherCost."','".$txtCommCost."','".$fobT_td."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
...
if($action=="confirmStyle_popup")
{
	...
	$sql_cons=sql_select("select item_id, sum(CASE WHEN particular_type_id in (1,20,4,6,7,998) THEN $cons_cond ELSE 0 END) as qty_kg, sum(CASE WHEN particular_type_id=999 THEN $cons_cond ELSE 0 END) as qty_yds, sum(CASE WHEN particular_type_id=1000 THEN $cons_cond ELSE 0 END) as qty_mtr from qc_cons_rate_dtls where mst_id='$qc_no' and type=1 group by item_id");//type ='1' and
	...
	foreach($sql_cons as $cRow)
	{
		if($uom==12)
		{
			...
			$item_wise_cons_arr[$cRow[csf("item_id")]]['qty_mtr']=$cRow[csf("qty_mtr")];
			$item_wise_cons_arr[$cRow[csf("item_id")]]['qty_yds']=$cRow[csf("qty_yds")];
		}
		else if($uom==23)
		{
			$item_wise_cons_arr[$cRow[csf("item_id")]]['qty_kg']=0;
			$item_wise_cons_arr[$cRow[csf("item_id")]]['qty_mtr']=$cRow[csf("qty_kg")]+$cRow[csf("qty_mtr")];
			$item_wise_cons_arr[$cRow[csf("item_id")]]['qty_yds']=$cRow[csf("qty_yds")];
		}
		else if($uom==27)
		{
    ...
    <script>
		...
		function fnc_confirm_entry( operation )
		{
			...
			if(operation==3)
			{
				var fab_cons_kg=$("#txtFabConkg_bom").val()*1;
				var fab_cons_mtr=$("#txtFabConmtr_bom").val()*1;
				var fab_cons_yds=$("#txtFabConyds_bom").val()*1;
				var fab_amount=$("#txtFabCst_bom").val()*1;
				var sp_oparation_amount=$("#txtSpOpa_bom").val()*1;
				var acc_amount=$("#txtAcc_bom").val()*1;
				var fright_amount=$("#txtFrightCst_bom").val()*1;
				var lab_amount=$("#txtLabCst_bom").val()*1;
				var misce_amount=$("#txtMiscCst_bom").val()*1;
				var other_amount=$("#txtOtherCst_bom").val()*1;
				var comm_amount=$("#txtCommCst_bom").val()*1;
				var fob_amount=$("#txtFobDzn_bom").val()*1;
				var cm_amount=$("#txtCmCst_bom").val()*1;
				var rmg_ratio=$("#txtPack_bom").val()*1;
				var set_smv=$("#txtSmv_bom").val()*1;
							
				var temp_id=$('#txtItem_id').val();
				var split_tmep_id=temp_id.split(','); var ab=0;
				var qc_fab_kg=0; var qc_fab_mtr=0; var qc_fab_yds=0; var qc_fab_amt=0; var qc_sp_amt=0; var qc_acc_amt=0; var qc_fri_amt=0; var qc_lab_amt=0; var qc_misce_amt=0; var qc_other_amt=0; var qc_comm_amt=0; var qc_fob_amt=0; var qc_cm_amt=0; var qc_rmg_amt=0; var qc_smv=0;
				...
				if(job_no!="")
				{
					...
					/*else if( qc_smv < set_smv )//ISD-23-25363
					{
						alert("BOM SMV is Greater Than QC.");
						release_freezing();	
						return;
					}*/
		...
		function fnc_stage_entry_reponse()
		{
			if(http.readyState == 4) 
			{
				...
				if (trim(reponse[0])=="88")//ISD-23-25572
				{
					alert(reponse[1]);
					release_freezing();
					return;
				...
				if(trim(reponse[0])==0 || trim(reponse[0])==1 || trim(reponse[0])==2 || trim(reponse[0])==3)
				{
					if (trim(reponse[0])==0)
					{
						...
						//document.getElementById('approve2').value = 'Approved';
					}
					...
					else if (trim(reponse[0])==2)
					{
						...
						//document.getElementById('approve2').value = 'Un-Approved';
					}
					else if (trim(reponse[0])==3)
					{
						...
						else
						{
							...
							document.getElementById('approve2').value = 'Approve';
						}
					}
		...
		function fnc_total_calculate()
		{
			...
			$("#txtEpmCst_qc").val( number_format((cal_cm/12)/qc_smv_amt,2,'.','') );
		}
		...
		function fnc_cppm_cal(item_id)
		{
			...
			var txtEpm=(txtCm/12)/txtSmv;
			if(txtEpm=="nan" || txtEpm=="Infinity") txtEpm=0;
			$("#txtEpmCst_"+item_id).val( txtEpm );
		...
		function openmypage_unapprove_request()
		{
			...
			var page_link = 'quick_costing_controller.php?data='+data+'&action=unapp_request_popup';
		...
		function fncqclimit(str)
		{
			...
			//alert(validatetype+'_'+tdsmvamt+'_'+($("#txtSmv_"+item_id).val()*1));
			...
			if(qcCons_from==2)
			{
				...
				/*if( validatetype==14 && tdsmvamt < ($("#txtSmv_"+item_id).val()*1) )//ISD-23-25363
				{
    ...
    <div style="display:none"><? echo load_freeze_divs ("../../../",$permission);  ?></div>       
        <form name="confirmStyle_1" id="confirmStyle_1" autocomplete="off">
        	<table width="850">
                <tr>
                    ...
                    <td width="120"><? echo create_drop_down( "cbo_buyer_id", 120, $qc_buyer_arr,"", 1, "-Select Buyer-", $sql_data[0][csf('buyer_id')], "load_drop_down( 'quick_costing_controller', this.value, 'load_drop_down_season_conf', 'season_conf_td'); load_drop_down( 'quick_costing_controller',this.value, 'load_drop_down_sub_depConf', 'subConf_td' );",1 ); ?></td>
            ...
            <table width="400" cellspacing="0" class="" border="0">
                <tr>
                    <td align="center" width="100%" class="button_container">
                        ...
                        <input type="button" value="Approved" name="approve" onClick="fnc_confirm_entry(3)" style="width:80px; display:<? echo $admin_or_leader; ?>" id="approve2" class="formbutton"><input type="button" class="formbutton" value="Close" style="width:80px" onClick="js_set_value();"/>
                    </td> 
                </tr>
            </table>
            <div id="confirm_data_div">
            <table width="950" cellspacing="0" border="1" rules="all" class="rpt_table">
        	<thead>
            	<th width="80">Item</th>
                ...
                <th width="50">SMV</th>
                <th width="50">CM</th>
                <th width="50" title="(CM/12)/SMV">EPM</th>
                <th>RMG Qty(Pcs)</th>
            </thead>
			<?
            ...
            foreach($sql_result_item_summ as $rowItemSumm)
            {
                ...
                <tr id="trVal_<? echo $z; ?>" bgcolor="<? echo $bgcolor; ?>">
                    ...
                    <? 
                    	$emp_amount=0;
                    	if($rowItemSumm[csf("cm_cost")]!=0 && $rowItemSumm[csf("smv")]!=0)
                    	{
                    		$emp_amount=($rowItemSumm[csf("cm_cost")]/12)/$rowItemSumm[csf("smv")];
                    	}
                    ?>
                    <td align="right" id="tdepmamt<?=$item_id; ?>"><? echo number_format($emp_amount,4); ?></td>
                ...
                <tr id="tr_<? echo $z; ?>" bgcolor="<? echo $bgcolorN; ?>">
                    ...
                    <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtEpmCst_<?=$item_id; ?>" id="txtEpmCst_<?=$item_id; ?>" value="<?=number_format($emp_amount,4); ?>" readonly /></td>
        ...
        <table width="950" cellspacing="0" border="1" rules="all" class="rpt_table">
        	<tr id="tr_qc" bgcolor="#CCFFCC">
                ...
                <td width="50"><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtEpmCst_qc" id="txtEpmCst_qc" value="" disabled /></td>
        	...
        	<tr id="tr_bom" bgcolor="#CCCCCC">
                ...
                <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtEpmCst_bom" id="txtEpmCst_bom" value="" readonly /></td>
...
if($action=="save_update_delete_confirm_style")
{
	...
	$user_id=$_SESSION['logic_erp']['user_id'];
	if(trim(str_replace("'","",$txt_job_id))!="")
	{
		$confirmjob_sql=sql_select("select a.cost_sheet_no, a.style_ref, a.revise_no, a.option_id, a.qc_no from qc_mst a, qc_confirm_mst b where a.qc_no=b.cost_sheet_id and b.cost_sheet_id!=$txt_costSheet_id and b.job_id=$txt_job_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
		$confirmjobmsg="";
		foreach($confirmjob_sql as $crow)
		{
			if($confirmjobmsg=="") $confirmjobmsg="QC: Cost Sheet No-".$crow[csf("cost_sheet_no")]."; Style Ref.-".$crow[csf("style_ref")]."; Rev. NO-".$crow[csf("revise_no")]."; Option-".$crow[csf("option_id")];
			else $confirmjobmsg.="n/ QC: Cost Sheet No-".$crow[csf("cost_sheet_no")]."; Style Ref.-".$crow[csf("style_ref")]."; Rev. NO-".$crow[csf("revise_no")]."; Option-".$crow[csf("option_id")];
		}
		if($confirmjobmsg!="")
		{
			echo "6**".$confirmjobmsg;
			die;
		}
		$jobtagmsg="";
		$sql_job=sql_select("Select job_no, style_ref_no from wo_po_details_master where quotation_id=$txt_costSheet_id and id!=$txt_job_id and status_active=1 and is_deleted=0");
		foreach($sql_job as $jrow)
		{
			if($jobtagmsg=="") $jobtagmsg="Order Entry :Job No-".$jrow[csf("job_no")]."; Style Ref.-".$jrow[csf("style_ref_no")];
			else $jobtagmsg.="n/ Order Entry :Job No-".$jrow[csf("job_no")]."; Style Ref.-".$jrow[csf("style_ref_no")];
		}
		if($jobtagmsg!="")
		{
			echo "6**".$jobtagmsg;
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$dtls_field_arr="id, mst_id, cost_sheet_id, item_id, fab_cons_kg, fab_cons_mtr, fab_cons_yds, fab_amount, sp_oparation_amount, acc_amount, fright_amount, lab_amount, misce_amount, other_amount, comm_amount, fob_amount, cppm_amount, smv_amount, cm_amount, epm_amount, rmg_ratio, inserted_by, insert_date";
		//echo "10**";
		...
		for($i=1; $i<=count($itm_id); $i++)
		{
			...
			$txtEpmCst='txtEpmCst_'.$item_id;
			$txtPack='txtPack_'.$item_id;
			
			if ($add_comma!=0) $dtls_data_arr .=",";
			$dtls_data_arr .="(".$confirm_dtls_id.",".$confirm_mst_id.",".$txt_costSheet_id.",".$$txtitemid.",".$$txtFabConkg.",".$$txtFabConmtr.",".$$txtFabConyds.",".$$txtFabCst.",".$$txtSpOpa.",".$$txtAcc.",".$$txtFrightCst.",".$$txtLabCst.", ".$$txtMiscCst.", ".$$txtOtherCst.", ".$$txtCommCst.", ".trim($$txtFobDzn).",".$$txtCpm.",".$$txtSmv.",".$$txtCmCst.",".$$txtEpmCst.",".$$txtPack.",".$user_id.",'".$pc_date_time."')";
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		$dtls_field_arr="fab_cons_kg*fab_cons_mtr*fab_cons_yds*fab_amount*sp_oparation_amount*acc_amount*fright_amount*lab_amount*misce_amount*other_amount*comm_amount*fob_amount*cppm_amount*smv_amount*cm_amount*epm_amount*rmg_ratio*updated_by*update_date";
		//echo "10**";
		$itm_id=explode(",",str_replace("'","",$txtItem_id));
		//print_r($itm_id);
		//echo count($itm_id);
		...
		for($i=1; $i<=count($itm_id); $i++)
		{
			...
			$dtls_data_arr[str_replace("'",'',$$txtdtlsupid)] =explode("*",("".$$txtFabConkg."*".$$txtFabConmtr."*".$$txtFabConyds."*".$$txtFabCst."*".$$txtSpOpa."*".$$txtAcc."*".$$txtFrightCst."*".$$txtLabCst."*".$$txtMiscCst."*".$$txtOtherCst."*".$$txtCommCst."*".$$txtFobDzn."*".$$txtCpm."*".$$txtSmv."*".$$txtCmCst."*".$$txtEpmCst."*".$$txtPack."*'".$user_id."'*'".$pc_date_time."'"));
			$k++;
		}
		$flag=1;
	...
	else if ($operation==3) // Approve Here----------------------------------------------------------
	{
		...
		{
			$data_array="'1'*'".$user_id."'*'".$pc_date_time."'";
		}
		else 
		{
			$data_array="'2'*'".$user_id."'*'".$pc_date_time."'";
			if($qcCons_from!=2)
			{
				$rID4=execute_query( "update fabric_booking_approval_cause set updated_by='".$user_id."', update_date='".$pc_date_time."', status_active=0, is_deleted=1 where booking_id =$txt_costSheet_id and entry_form=28 and approval_type=2 and status_active=1 and is_deleted=0",1);
...
if($action=="populate_confirm_style_form_data")
{
	...
	foreach($sql_confirm_mst as $row_mst)
	{
		...
		if($row_mst[csf("approved")]==1)
		{
			...
			unset($nameArray_request);
			echo "document.getElementById('approve2').value = 'Un-Approved';\n";
	...
	foreach($sql_confirm_dtls as $row_dtls)
	{
		...
		echo "$('#txtEpmCst_".$row_dtls[csf("item_id")]."').val('".$row_dtls[csf("epm_amount")]."');\n";
...
if($action=="budgete_cost_validate")
{
	...
	if($job_no!="")
	{
		...
		$set_smv=0;
		$set_smvsql=sql_select("Select set_smv from wo_po_details_master where job_no='$job_no' and status_active=1 and is_deleted=0 ");
		$set_smv=$set_smvsql[0][csf('set_smv')];
		...
		$str_data=$fab_cons_kg."##".$fab_cons_mtr."##".$fab_cons_yds."##".$fab_amount."##".$sp_oparation_amount."##".$acc_amount."##".$fright_amount."##".$lab_amount."##".$misce_amount."##".$other_amount."##".$comm_amount."##".$fob_amount."##".$cm_amount."##".$rmg_ratio."##".$set_smv;
		unset($dataArr);
	}
        ...
        <table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
            ...
            <tr class="general">
                <td> 
					...
					<? echo create_drop_down( "cbo_company_mst", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-Select Company-", $cbo_company_name,"",'' ); ?>
                </td>
                ...
                <td align="center">
                	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+<?=$buyer_id; ?>+'_'+document.getElementById('chk_job_wo_po').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_job_prifix').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_style').value, 'create_po_search_list_view', 'search_div', 'quick_costing_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" /></td>
        	</tr>
            <tr>
...
if($action=="create_po_search_list_view")
{
	...
	if (is_duplicate_field( "buyer_name", "lib_buyer", "id='$data[1]' and status_active!=1" ) == 1)
	{
		?>
		<br>
		<div style="width:80%" class="alert alert-danger"><b>Style Tag is not Allow for InActive/Cancelled Buyer.</b> </div>
		<?
		die;
	}
...
if($action=="check_buyer_status")
{
	$status=0;
	$check_buyer_status=sql_select("Select status_active from lib_buyer where id=$data");
	foreach ($check_buyer_status as $row) {
