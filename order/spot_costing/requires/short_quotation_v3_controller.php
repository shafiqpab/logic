...
if ($action=="load_drop_down_option_id")
{
	...
	/*if($ex_data[1]!=0)
		...
		$selectopt=$selected;*/
		//load_drop_down('requires/short_quotation_v3_controller', '".$row[csf("cost_sheet_no")].'__'.$row[csf("option_id")].'__'.$row[csf("revise_no")]."', 'load_drop_down_revise_no', 'revise_td');
	...
	if($ex_data[1]!=0)
		...
		$selectopt=$max;
	//echo $sql;
	echo create_drop_down( "cbo_option_id", 45, $rvs,"", 0, "-0-", $selectopt, "load_drop_down( 'requires/short_quotation_v3_controller', document.getElementById('txt_costSheetNo').value+'__'+this.value+'__'+0, 'load_drop_down_revise_no', 'revise_td'); fnc_option_rev( document.getElementById('cbo_revise_no').value+'***'+this.value+'***2' );" );//
	exit();
}
...
if($action=="template_popup")
{
	...
	function fnc_template_entry( operation )
	{
		...
		http.open("POST","short_quotation_v3_controller.php",true);
	...
	function fnc_template_entry_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			if(reponse[0]==0 || reponse[0]==1)
			{
				...
				show_list_view( '','template_list_view','save_up_list_view','short_quotation_v3_controller','setFilterGrid(\'tbl_upListView\',-1)');
			}
	...
	{
		var list_view_grid = return_global_ajax_value( temp_id, 'load_php_dtls_form', '', 'short_quotation_v3_controller');
		if(list_view_grid!='')
		{
...
if($action=="rate_details_popup")
{
	...
	$adata=$qc_no.'!!!'.$bodypartid.'!!!'.$consRateId.'!!!'.$txtdznkg.'!!!'.$txt_fabDesc.'!!!'.$cbocolortype.'!!!'.$txtgsmweight.'!!!'.$txtfabwidth.'!!!'.$packQty.'!!!'.$itemRatio.'!!!'.$rateData;
	?>
    <script>
		var qc_no='<?=$qc_no; ?>';
		...
		function fnc_cons_amt1(str)
		{
			...
			if(type==1)
			{
				var colorconsyds=$('#txtcolorconsyds_'+inc).val();
				if(fabuom==12)//kg
				{
					var consFab=(colorconsyds*36*(fabwidth*1)*(fabgsm*1))/1550/1000;
				}
				...
				$("#txtcolorconskg_"+inc).val( number_format(consFab,4,'.','') );
				var consPack=(consFab/12)*($('#txtcolorratio_'+inc).val()*1);
			}
			else if (type==2)
			{
				var colorconskg=$('#txtcolorconskg_'+inc).val();
				if(fabuom==12)//kg
				{
					var consFab=(colorconskg/36)/((fabwidth*1)*(fabgsm*1)/1550/1000);
				}
				...
				$("#txtcolorconsyds_"+inc).val( number_format(consFab,4,'.','') );
				//var consPack=(consFab/12)*(itemRatio*1);
				var consPack=(colorconskg/12)*($('#txtcolorratio_'+inc).val()*1);
			}
			
			//var cnsdznkg=((colorconsyds*1)*36*(fabwidth*1)*(fabgsm*1))/1550/1000;
			
			//$('#txtcolorconskg_'+inc).val( number_format(consPack,4,'.','') );
			
			//var conspack=(consFab*1)/12*($('#txtcolorratio_'+inc).val()*1);
			$('#txtcolorconspack_'+inc).val( number_format(conspack,4,'.','') );
			var conspackkg=conspack;//$('#txtcolorconspack_'+inc).val()*1;
			var colorexper=$("#txtcolorexper_"+inc).val()*1;
			var rowtotCons=0;
			if(process_loss_method_id==1)
			{
				rowtotCons=conspackkg+(conspackkg*(colorexper/100));
		...
		function js_set_value()
		{
			...
			for(var i=1; i<=row; i++)
			{
				if(($('#txtcolorratio_'+i).val()*1)!=0)
				{
					if(consData=="") consData+=$('#txtitemqty_'+i).val()+'_'+$('#hidupid_'+i).val()+'_'+$('#txtcoloroption_'+i).val()+'_'+$('#txtcolorratio_'+i).val()+'_'+$('#txtcolorconsyds_'+i).val()+'_'+$('#txtcolorconskg_'+i).val()+'_'+$('#txtcolorconspack_'+i).val()+'_'+$('#txtcolorexper_'+i).val()+'_'+$('#txtcolortotccons_'+i).val()+'_'+$('#txtcolorrate_'+i).val()+'_'+$('#txtcoloramt_'+i).val();
					else consData+="@@!"+$('#txtitemqty_'+i).val()+'_'+$('#hidupid_'+i).val()+'_'+$('#txtcoloroption_'+i).val()+'_'+$('#txtcolorratio_'+i).val()+'_'+$('#txtcolorconsyds_'+i).val()+'_'+$('#txtcolorconskg_'+i).val()+'_'+$('#txtcolorconspack_'+i).val()+'_'+$('#txtcolorexper_'+i).val()+'_'+$('#txtcolortotccons_'+i).val()+'_'+$('#txtcolorrate_'+i).val()+'_'+$('#txtcoloramt_'+i).val();
			...
			document.getElementById('txttotcons_yds').value;
			document.getElementById('txttotcons_kg').value;
        ...
        {
            //alert(row)
            emailwindow=dhtmlmodal.open('EmailBox', 'iframe','short_quotation_v3_controller.php?action='+action,'Details Popup', 'width=580px,height=300px,center=1,resize=0','../../');
		...
		function append_row(str)
		{
			...
			{
				i++;
				$("#tbl_fab tbody tr:last").clone().find("input").each(function() {
					$(this).attr({
						'id': function(_, id) { var id=id.split("_"); return id[0] +"_"+ i },
						'name': function(_, name) { return name + i },
						'value': function(_, value) { return value }
					});
				}).end().appendTo("#tbl_fab");
				
				$('#txtcolorratio_'+i).removeAttr("onChange").attr("onChange","fnc_cons_amt('"+i+"_1')");
				$('#txtcolorconsyds_'+i).removeAttr("onChange").attr("onChange","fnc_cons_amt('"+i+"_1')");
				$('#txtcolorconskg_'+i).removeAttr("onChange").attr("onChange","fnc_cons_amt('"+i+"_2')");
				...
				$('#txtcolorconsyds_'+i).val("");
				$('#txtcolorconskg_'+i).val("");
		...
		function fnc_remove_row(inc)
		{
			...
			if(counter!=1)
			{
				...
				{
					var index=i-1;
					$("#tbl_fab tbody tr:eq("+index+")").find("input").each(function() {
						$(this).attr({
							'id': function(_, id) { var id=id.split("_"); return id[0] +"_"+ i },
						});
					});
        ...
        function fnc_fabric_entry(operation)
        {
			...
			for(var i=1; i<=counter; i++)
			{
				...
				{
					 dataAll+="&txtFabricDtls_" + z + "='" + $('#txtFabricDtls_'+i).val()+"'"+"&hidLibid_" + z + "='" + $('#hidLibid_'+i).val()+"'"+"&txtYarnDtls_" + z + "='" + $('#txtYarnDtls_'+i).val()+"'"+"&txtConvDtls_" + z + "='" + $('#txtConvDtls_'+i).val()+"'"+"&txtfabcons_" + z + "='" + $('#txtfabcons_'+i).val()+"'"+"&txtfabex_" + z + "='" + $('#txtfabex_'+i).val()+"'"+"&txtfabtotcons_" + z + "='" + $('#txtfabtotcons_'+i).val()+"'"+"&txtfabrate_" + z + "='" + $('#txtfabrate_'+i).val()+"'"+"&txtfabamt_" + z + "='" + $('#txtfabamt_'+i).val()+"'"+"&hidupid_" + z + "='" + $('#hidupid_'+i).val()+"'";
					z++; r++;
				}
			}
		...
		function fnc_yarn_entry(operation)
        {
			...
			for(var i=1; i<=counter; i++)
			{
				...
				{
					 dataAll+="&cbocount_" + z + "='" + $('#cbocount_'+i).val()+"'"+"&hidfabid_" + z + "='" + $('#hidfabid_'+i).val()+"'"+"&cbocompone_" + z + "='" + $('#cbocompone_'+i).val()+"'"+"&txtpercentone_" + z + "='" + $('#txtpercentone_'+i).val()+"'"+"&cbotype_" + z + "='" + $('#cbotype_'+i).val()+"'"+"&txtyarncons_" + z + "='" + $('#txtyarncons_'+i).val()+"'"+"&txtyarnrate_" + z + "='" + $('#txtyarnrate_'+i).val()+"'"+"&txtyarnamt_" + z + "='" + $('#txtyarnamt_'+i).val()+"'"+"&hidyarnupid_" + z + "='" + $('#hidyarnupid_'+i).val()+"'";
					z++;
				}
			}
		...
		function fnc_row_generate()
		{
			...
			show_list_view(data,'populate_confirm_fabric_data','fabricDiv','short_quotation_v3_controller','');
			fnc_cons_amt("0_1");
		}
...
if($action=="accrate_details_popup")
{
    ...
    <script>
		...
		function fnc_accrow_generate()
		{
			...
			show_list_view(data,'populate_confirm_acc_data','accDiv','short_quotation_v3_controller','');
		}
...
if($action=="populate_confirm_fabric_data")
{
    ...
    <table width="700" cellspacing="0" border="1" class="rpt_table" id="tbl_fab" rules="all">
        <thead>
            <td colspan="11" align="center" bgcolor="#99FFCC"><b><?=$body_part[$body_part_id].','.$fabDesc.','.$color_type[$colortype].';  GSM: '.$gsmweight.';  F. Width [Inch]: '.$fabwidth; ?></b></td>
        </thead>
        <thead>
            <th width="60">Item Qty/Pack</th>
            <th width="80">Color/ Option</th>
            <th width="50">Color Ratio</th>
            <th width="50" style="display:none">Cons/DZN [Yds]</th>
        ...
        <tbody id="fabtbody">
			...
			if($ratedata!="")
			{
				...
				//print_r($exrdata);
				//$('#txtitemqty_'+i).val()+'_'+$('#hidupid_'+i).val()+'_'+$('#txtcoloroption_'+i).val()+'_'+$('#txtcolorratio_'+i).val()+'_'+$('#txtcolorconsyds_'+i).val()+'_'+$('#txtcolorconskg_'+i).val()+'_'+$('#txtcolorconspack_'+i).val()+'_'+$('#txtcolorexper_'+i).val()+'_'+$('#txtcolortotccons_'+i).val()+'_'+$('#txtcolorrate_'+i).val()+'_'+$('#txtcoloramt_'+i).val();
				foreach($exrdata as $rdata)
				{
					...
					<tr id="fabtr_<?=$i; ?>">
						...
						<td style="display:none"><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtcolorconsyds_<?=$i; ?>" id="txtcolorconsyds_<?=$i; ?>" value="<?=$xrdata[4]; ?>" onChange="fnc_cons_amt('<?=$i.'_1'; ?>');" /></td>
						<td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtcolorconskg_<?=$i; ?>" id="txtcolorconskg_<?=$i; ?>" value="<?=$xrdata[5]; ?>" onChange="fnc_cons_amt('<?=$i.'_2'; ?>');" /></td>
						<td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtcolorconspack_<?=$i; ?>" id="txtcolorconspack_<?=$i; ?>" value="<?=$xrdata[6]; ?>" readonly /></td>
                        <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtcolorexper_<?=$i; ?>" id="txtcolorexper_<?=$i; ?>" onChange="fnc_cons_amt('<?=$i.'_1'; ?>');" value="<?=$xrdata[7]; ?>" /></td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtcolortotccons_<?=$i; ?>" id="txtcolortotccons_<?=$i; ?>" onChange="fnc_cons_amt('<?=$i.'_1'; ?>');" value="<?=$xrdata[8]; ?>" readonly /></td>
						<td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtcolorrate_<?=$i; ?>" id="txtcolorrate_<?=$i; ?>" onChange="fnc_cons_amt('<?=$i.'_1'; ?>');" value="<?=$xrdata[9]; ?>" /></td>
						<td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtcoloramt_<?=$i; ?>" id="txtcoloramt_<?=$i; ?>" placeholder="Display" readonly value="<?=$xrdata[10]; ?>" /></td>
						<td>
					...
					//$consTotYds+=$xrdata[4];
					$consTotKg+=$xrdata[5];
					$consTotPack+=$xrdata[6];
					$consTotTot+=$xrdata[8];
					$amtTot+=$xrdata[10];
				}
				$rowi=$i-1;
				//$consTotYds=$consTotYds/$rowi;
				$consTotKg=$consTotKg/$rowi;
            ...
            <tr id="fabtr_<?=$i; ?>">
                ...
                <td style="display:none"><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtcolorconsyds_<?=$i; ?>" id="txtcolorconsyds_<?=$i; ?>" value="" onChange="fnc_cons_amt('<?=$i.'_1'; ?>');" /></td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtcolorconskg_<?=$i; ?>" id="txtcolorconskg_<?=$i; ?>" value="" onChange="fnc_cons_amt('<?=$i.'_2'; ?>');" /></td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtcolorconspack_<?=$i; ?>" id="txtcolorconspack_<?=$i; ?>" value="" readonly /></td>
        ...
        <tfoot>
            <tr bgcolor="#CCCCAA">
                ...
                <td style="display:none"><input style="width:38px;" type="text" class="text_boxes_numeric" name="txttotcons_yds" id="txttotcons_yds" value="<?=$consTotYds; ?>" disabled /></td>
                <td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txttotcons_kg" id="txttotcons_kg" value="<?=$consTotKg; ?>" disabled /></td>
            ...
            <tr>
            	<td colspan="11" align="center"><input type="button" class="formbutton" style="width:100px" value="Close" onClick="js_set_value();"/></td>
            </tr>
        </tfoot>
    </table>
    </table>
...
if($action=="stage_saveUpdate_popup")
{
		...
		function fnc_stage_entry_reponse()
		{
			if(http.readyState == 4) 
			{
				...
				if(reponse[0]==0 || reponse[0]==1)
				{
					...
					show_list_view('','stage_data_list','stage_data_div','short_quotation_v3_controller','');
				}
			}
			release_freezing();
		}
    ...
    <script>show_list_view('','stage_data_list','stage_data_div','short_quotation_v3_controller','');</script>          
    <script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
    </html>
    <?
	exit();	
...
{
	echo  create_list_view ( "list_view", "Stage Name", "200","200","150",1, "select tuid, stage_name from lib_stage_name where status_active=1 and is_deleted=0 order by tuid desc", "get_php_form_data", "tuid","'load_php_data_to_form_satge'", 1, "0", $arr, "stage_name","short_quotation_v3_controller", 'setFilterGrid("list_view",-1);','0' );
	exit();
}
...
if($action=="save_update_delete")
{
	...
	where  b.team_id in (select id from lib_marketing_team a where user_tag_id='$user_id' and a.status_active=1 and a.is_deleted=0 ) and b.status_active=1 and b.is_deleted=0");
		...
		$field_array_consrate="id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, value, fab_id, description, body_part_id, supp_ref, remarks, uom, inserted_by, insert_date, status_active, is_deleted, color_type_id, gsmweight, fabwidth, dznyds, dznkg, rate_data, use_for, yarn_break_down";
		...
		foreach($temp_id as $item_id)
		{
			...
			foreach($ex_itemConsRateData as $item_wise_consRateData)
			{
				if($item_wise_consRateData!="")
				{
					...
					$bodypartid=$fabid=$colortype=$gsmweight=$fabwidth=$dznyds=$dznkg=$conspack=$exPer=$totCons=$ratePop=$rate=$fabAmt=$fabupid=$fabuom=0;
					...
					$dznyds=$ex_itemConsRate[8];
					$dznkg=$ex_itemConsRate[9];
					$conspack=$ex_itemConsRate[10];
					$exPer=$ex_itemConsRate[11];
					$totCons=$ex_itemConsRate[12];
					$ratePop=$ex_itemConsRate[13];
					$rateData=$ex_itemConsRate[14];
					$rate=$ex_itemConsRate[15];
					$fabAmt=$ex_itemConsRate[16];
					$fabupid=$ex_itemConsRate[17];
					$fabuom=$ex_itemConsRate[18];
					$yarnBreakDown=$ex_itemConsRate[19];
					
					if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
					$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',1,0,'".$conspack."','".$exPer."','".$totCons."','".$ratePop."','".$rate."','".$fabAmt."','".$fabid."','".$fabDesc."','".$bodypartid."','','','".$fabuom."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,'".$colortype."','".$gsmweight."','".$fabwidth."','".$dznyds."','".$dznkg."','".$rateData."','".$usefor."','".$yarnBreakDown."')";
			...
			foreach($ex_specialData as $item_wise_specialData)
			{
				...
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',2,'".$speciaOperation_id."','".$spConsumtion."','".$spExper."','".$totSpCons."',0,'".$spRate."','".$spValue."','".$speciaTypeId."','','".$spbodypartid."','','".$spRemarks."',0,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,0,0,0,0,0,'','','')";
			...
			foreach($ex_washData as $item_wise_washData)
			{
				...
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',3,'".$washTypeId."','".$wConsumtion."','".$wExper."','".$totwCons."',0,'".$wRate."','".$wValue."',0,'','".$wbodypartid."','','".$wRemarks."',0,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,0,0,0,0,0,'','','')";
			...
			foreach($ex_itemAccData as $item_wise_itemAccData)
			{
				...
				$accessories_id=$accDescription=$accBandRef=$accUseFor=$acConsumtion=$acExPer=$totAccCons=$acRate=$acVal=$acConsDzn=$acconsUom=$acConsCal=$acRateData=0;
				...
				$acConsDzn=$ex_itemAcConsRate[11];
				$acConsCal=$ex_itemAcConsRate[12];
				$acRateData=$ex_itemAcConsRate[13];
				
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',4,'".$accessories_id."','".$acConsumtion."','".$acExPer."','".$totAccCons."','".$acConsCal."','".$acRate."','".$acVal."',0,'".$accDescription."',0,'".$accBandRef."','','".$acconsUom."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,0,0,0,0,'".$acConsDzn."','".$acRateData."','".$accUseFor."','')";
		...
		$data_array_tot_cost="(".$tc_sum_id.",".$qcno.",'".$cbo_buyer_agent."','".$cbo_agent_location."','".$txt_noOfPack."',0,'".$cmPop."','".$txt_lumSum_cost."','".$txt_commPer."','".$totFab_td."','".$totSpc_td."','".$totWash_td."','".$totAcc_td."','".$totCm_td."','".$totFriCst_td."','".$totLbTstCst_td."','".$totMissCst_td."','".$totOtherCst_td."','".$totCommCst_td."','".$totCost_td."','".$totalFob_td."','".$totRmgQty_td."','".$txt_commlPer."','".$totCommlCst_td."','".$txt_optexp."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		//echo $data_array_tot_cost; die;
		$field_array_item_tot=" id, mst_id, item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs, rmg_ratio, commercial_cost, operating_exp, inserted_by, insert_date, status_active, is_deleted";
		...
		{
			$fab_td=$spOpe_td=$wash_td=$acc_td=$txtCpm=$txtSmv=$txtEff=$txtCmCost=$txtFriCost=$txtLtstCost=$txtMissCost=$txtOtherCost=$txtCommCost=$fobT_td=$fobDzn_td=$tot_item_id=$txtCommlCost=$optexp=$txtoptexp=0;
			...
			$tot_item_id=trim($ex_item_tot[18]);
			
			if ($add_comma_item_tot!=0) $data_array_item_tot .=",";
			$data_array_item_tot .="(".$item_cost_id.",".$qcno.",'".$tot_item_id."','".$fab_td."','".$spOpe_td."','".$wash_td."','".$acc_td."','".$txtCpm."','".$txtSmv."','".$txtEff."','".$txtCmCost."','".$txtFriCost."','".$txtLtstCost."','".$txtMissCost."','".$txtOtherCost."','".$txtCommCost."','".$fobT_td."','".$txtRmgQty."','".$txtCommlCost."','".$txtoptexp."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
...
where b.team_id in (select id from lib_marketing_team a where user_tag_id='$user_id' and a.status_active=1 and a.is_deleted=0 ) and b.status_active=1 and b.is_deleted=0");
		...
		$fieldArrConsRate="particular_type_id*consumption*ex_percent*tot_cons*is_calculation*rate*value*fab_id*description*body_part_id*supp_ref*remarks*uom*updated_by*update_date*color_type_id*gsmweight*fabwidth*dznyds*dznkg*rate_data*use_for*yarn_break_down";
		...
		foreach($temp_id as $item_id)
		{
			...
			foreach($ex_itemConsRateData as $item_wise_consRateData)
			{
				if($item_wise_consRateData!="")
				{
					...
					$fabDesc=$usefor=$rateData="";
					$bodypartid=$fabid=$colortype=$gsmweight=$fabwidth=$dznyds=$dznkg=$conspack=$exPer=$totCons=$ratePop=$rate=$fabAmt=$fabupid=0;
					...
					{
						$id_arr[]=$fabupid;
						$fabConsRateUpdateArr[$fabupid] =explode("*",("'0'*'".$conspack."'*'".$exPer."'*'".$totCons."'*'".$ratePop."'*'".$rate."'*'".$fabAmt."'*'".$fabid."'*'".$fabDesc."'*'".$bodypartid."'*''*''*'".$fabuom."'*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'*'".$colortype."'*'".$gsmweight."'*'".$fabwidth."'*'".$dznyds."'*'".$dznkg."'*'".$rateData."'*'".$usefor."'*'".$yarnBreakDown."'"));
					}
					else if($fabupid==0)
					{
						if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
						$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',1,0,'".$conspack."','".$exPer."','".$totCons."','".$ratePop."','".$rate."','".$fabAmt."','".$fabid."','".$fabDesc."','".$bodypartid."','','','".$fabuom."','".$gsmweight."','".$fabwidth."','".$dznyds."','".$dznkg."','".$usefor."','".$rateData."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,'".$colortype."','".$yarnBreakDown."')";
			...
			foreach($ex_specialData as $item_wise_specialData)
			{
				...
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',2,'".$speciaOperation_id."','".$spConsumtion."','".$spExper."','".$totSpCons."',0,'".$spRate."','".$spValue."','".$speciaTypeId."','','".$spbodypartid."','','".$spRemarks."',0,0,0,0,0,'','',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,0,'')";
			...
			foreach($ex_washData as $item_wise_washData)
			{
				...
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',3,'".$washTypeId."','".$wConsumtion."','".$wExper."','".$totwCons."',0,'".$wRate."','".$wValue."',0,'','".$wbodypartid."','','".$wRemarks."',0,0,0,0,0,'','',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,0,'')";
			...
			foreach($ex_itemAccData as $item_wise_itemAccData)
			{
				...
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',4,'".$accessories_id."','".$acConsumtion."','".$acExPer."','".$totAccCons."','".$acConsCal."','".$acRate."','".$acVal."',0,'".$accDescription."',0,'".$accBandRef."','','".$acconsUom."',0,0,0,'".$acConsDzn."','".$accUseFor."','".$acRateData."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,0,'')";
		...
		foreach($ex_item_wise_tot_data as $item_wise_tot)
		{
			$fab_td=$spOpe_td=$wash_td=$acc_td=$txtCpm=$txtSmv=$txtEff=$txtCmCost=$txtFriCost=$txtLtstCost=$txtMissCost=$txtOtherCost=$txtCommCost=$fobT_td=$fobDzn_td=$txtoptexp=$tot_item_id=0;
...
if($action=="style_popup")
{
	...
	{
		show_list_view (document.getElementById('cbo_buyer_id').value+'**'+document.getElementById('cbo_season_id').value+'**'+document.getElementById('cbo_subDept_id').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('txt_cost_sheet_no').value+'**'+document.getElementById('hide_dataType').value+'**'+document.getElementById('cbo_approval_type').value+'**'+document.getElementById('cbo_string_search_type').value, 'style_ref_search_list_view', 'search_div', 'short_quotation_v3_controller', 'setFilterGrid(\'tbl_list_search\',-1)');
		set_all();
	}
	...
	function fnc_confirm_style(qcno,upid)
	{
		if(upid!="")
		{
			...
			var page_link='short_quotation_v3_controller.php?action=confirmStyle_popup';
    ...
    <div align="center">
		...
		<fieldset>
            <table cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr class="general">
                        <td><? echo create_drop_down( "cbo_buyer_id", 130, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,30,90)) order by buyer_name","id,buyer_name",1, "-- All Buyer--",$buyerId,"load_drop_down( 'short_quotation_v3_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'short_quotation_v3_controller',this.value, 'load_drop_down_sub_dep', 'sub_td' );",0 ); ?></td>                 
...
if($action=="style_ref_search_list_view")
{
	...
	else
	{
		...
		if($string_search_type==1)
		{
			if($style_ref!='') $style_ref_cond=" and style_ref='$style_ref'"; else $style_ref_cond="";
			if($cost_sheet_no!='') $cost_sheet_no_cond=" and cost_sheet_no='$cost_sheet_no'"; else $cost_sheet_no_cond="";
		}
		else if($string_search_type==2)
		{
	...
	{
		$sql_style = "select min(a.id) as id, a.cost_sheet_no, max(a.costing_date) as costing_date, a.buyer_id, a.season_id, a.style_ref, a.department_id, a.temp_id, a.lib_item_id, a.stage_id, max(a.inserted_by) as inserted_by, max(a.updated_by) as updated_by, min(a.qc_no) as qc_no from qc_mst a where a.status_active=1 and a.is_deleted=0 and a.entry_form=471 $buyer_id_cond $style_ref_cond $cost_sheet_no_cond $season_id_cond $department_id_cond $approvaltype_cond group by a.cost_sheet_no, a.buyer_id, a.season_id, a.style_ref, a.department_id, a.temp_id, a.lib_item_id, a.stage_id order by id Desc";
...
if($action=="temp_style_list_view")
{
	...
	if($ex_data[1]==1)
	{
		...
		$sql_style= "select min(id) as id, cost_sheet_no, style_ref, min(qc_no) as qc_no, season_id, inserted_by from qc_mst where entry_form=471 and cost_sheet_no in ($ex_data[0]) and cost_sheet_no not in (select cost_sheet_no from qc_style_temp_data where cost_sheet_no in ($ex_data[0]) and inserted_by='$user_id' and status_active=1 and is_deleted=0 ) and revise_no=0 and option_id=0 and status_active=1 and is_deleted=0 group by cost_sheet_no, style_ref, season_id, inserted_by";
		//echo  $sql_style;
	...
	$sql_temp_style=sql_select("select a.id, a.cost_sheet_no, a.style_ref, a.season_id, a.inserted_by, a.updated_by, a.qc_no, a.season_year from qc_mst a, qc_style_temp_data b where a.id=b.mst_id and b.inserted_by='$user_id' and a.entry_form=471 and b.status_active=1 and b.is_deleted=0 order by a.style_ref ASC");
...
if($action=="populate_style_details_data")
{
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_drop_down('requires/short_quotation_v3_controller', '".trim($row[csf("company_id")])."', 'load_drop_down_location', 'location_td'); \n";
		...
		echo "load_drop_down('requires/short_quotation_v3_controller', '".trim($itemNratioArr[$row[csf("temp_id")]]['ratio'])."', 'load_drop_down_tempItem', 'item_td'); \n";
		...
		echo "load_drop_down('requires/short_quotation_v3_controller', '".$row[csf("buyer_id")]."', 'load_drop_down_season', 'season_td');\n"; 
		echo "load_drop_down('requires/short_quotation_v3_controller', '".$row[csf("buyer_id")]."', 'load_drop_down_brand', 'brand_td');\n";
		echo "load_drop_down('requires/short_quotation_v3_controller', '".$row[csf("buyer_id")]."', 'load_drop_down_sub_dep', 'sub_td');\n";
		...
		{
			//echo "load_drop_down('requires/short_quotation_v3_controller', '".$row[csf("cost_sheet_no")].'__'.$row[csf("option_id")].'__'.$row[csf("revise_no")]."', 'load_drop_down_revise_no', 'revise_td');\n"; //.'__'.$row[csf("revise_no")]
			//echo "load_drop_down('requires/short_quotation_v3_controller', '".$row[csf("cost_sheet_no")].'__'.$row[csf("option_id")]."', 'load_drop_down_option_id', 'option_td');\n"; 
		}
		//echo "$('#cbo_option_id').val('".$row[csf("option_id")]."');\n";
		if($count_temp==1) $set_pcs='Pack';
	...
	foreach ($sql_result_cons_rate as $rowConsRate)
	{
		...
		{
			$cons_rate_fab_arr[$rowConsRate[csf("item_id")]].=$body_part[$rowConsRate[csf("body_part_id")]].'~!~'.$rowConsRate[csf("body_part_id")].'~!~'.$rowConsRate[csf("description")].'~!~'.$rowConsRate[csf("fab_id")].'~!~'.$rowConsRate[csf("color_type_id")].'~!~'.$rowConsRate[csf("gsmweight")].'~!~'.$rowConsRate[csf("fabwidth")].'~!~'.$rowConsRate[csf("use_for")].'~!~'.$rowConsRate[csf("dznyds")].'~!~'.$rowConsRate[csf("dznkg")].'~!~'.$rowConsRate[csf("consumption")].'~!~'.$rowConsRate[csf("ex_percent")].'~!~'.$rowConsRate[csf("tot_cons")].'~!~'.$rowConsRate[csf("is_calculation")].'~!~'.$rowConsRate[csf("rate_data")].'~!~'.$rowConsRate[csf("rate")].'~!~'.$rowConsRate[csf("value")].'~!~'.$rowConsRate[csf("id")].'~!~'.$rowConsRate[csf("uom")].'~!~'.$rowConsRate[csf("yarn_break_down")].'#^';
		}
		...
		{
			$cons_rate_ac_arr[$rowConsRate[csf("item_id")]].=$itemGroupArr[$rowConsRate[csf("particular_type_id")]].'~!~'.$rowConsRate[csf("particular_type_id")].'~!~'.$rowConsRate[csf("description")].'~!~'.$rowConsRate[csf("supp_ref")].'~!~'.$rowConsRate[csf("consumption")].'~!~'.$rowConsRate[csf("ex_percent")].'~!~'.$rowConsRate[csf("tot_cons")].'~!~'.$rowConsRate[csf("rate")].'~!~'.$rowConsRate[csf("value")].'~!~'.$rowConsRate[csf("use_for")].'~!~'.$rowConsRate[csf("uom")].'~!~'.$rowConsRate[csf("dznkg")].'~!~'.$rowConsRate[csf("is_calculation")].'~!~'.$rowConsRate[csf("rate_data")].'#^';
		}
	...
	$sql_summ="select buyer_agent_id, location_id, no_of_pack, is_confirm, is_cm_calculative, mis_lumsum_cost, commision_per, tot_fab_cost, tot_sp_operation_cost, tot_wash_cost, tot_accessories_cost, tot_cm_cost, tot_fright_cost, tot_lab_test_cost, tot_miscellaneous_cost, tot_other_cost, tot_commission_cost, tot_cost, tot_fob_cost, tot_rmg_ratio, commercial_per, commercial_cost, operating_exp from qc_tot_cost_summary where mst_id='$cond_data' and status_active=1 and is_deleted=0";
	...
	$sql_item_summ="select item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs, rmg_ratio, commercial_cost, operating_exp from qc_item_cost_summary where mst_id='$cond_data' and status_active=1 and is_deleted=0";
...
if($action=="copy_cost_sheet")
{
	...
	{	
		...
		if (is_duplicate_field( "cost_sheet_no", "qc_mst", "style_ref='$dup_styleRef' and season_id='$dup_season_id' and buyer_id='$dup_buyer_id' and entry_form=471 and status_active=1 and is_deleted=0")==1)
		{
			echo "11**".$msg; 
			die;
		}
		...
		$data_array_tot_cost="(".$tc_sum_id.",".$qcno.",'".$cbo_buyer_agent."','".$cbo_agent_location."','".$txt_noOfPack."',0,'".$cmPop."','".$txt_lumSum_cost."','".$txt_commPer."','".$totFab_td."','".$totSpc_td."','".$totWash_td."','".$totAcc_td."','".$totCm_td."','".$totFriCst_td."','".$totLbTstCst_td."','".$totMissCst_td."','".$totOtherCst_td."','".$totCommCst_td."','".$totCost_td."','".$totalFob_td."','".$totalRmg_td."','".$txt_commlPer."','".$totCommlCst_td."','".$txt_optexp."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		//echo $data_array_tot_cost; die;
		$sql_item_tot=sql_select("select id, mst_id, item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs, rmg_ratio, commercial_cost, operating_exp from qc_item_cost_summary where mst_id='$hid_qc_no' order by id ASC");
		...
		{
			$fab_td=$spOpe_td=$wash_td=$acc_td=$txtCpm=$txtSmv=$txtEff=$txtCmCost=$txtFriCost=$txtLtstCost=$txtMissCost=$txtOtherCost=$txtCommCost=$fobT_td=$tot_item_id=$rmg_ratio=$txtoptexp=0;
			...
			if ($add_comma_item_tot!=0) $data_array_item_tot .=",";
			$data_array_item_tot .="(".$item_cost_id.",".$qcno.",'".$tot_item_id."','".$fab_td."','".$spOpe_td."','".$wash_td."','".$acc_td."','".$txtCpm."','".$txtSmv."','".$txtEff."','".$txtCmCost."','".$txtFriCost."','".$txtLtstCost."','".$txtMissCost."','".$txtOtherCost."','".$txtCommCost."','".$fobT_td."','".$rmg_ratio."','".$txtCommlCost."','".$txtoptexp."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
...
if($action=="confirmStyle_popup")
{
    ...
    <script>
		...
		function fnc_stage_entry_reponse()
		{
			if(http.readyState == 4) 
			{
				...
				if(trim(reponse[0])==0 || trim(reponse[0])==1 || trim(reponse[0])==2 || trim(reponse[0])==3)
				{
					...
					get_php_form_data($('#txt_costSheet_id').val()+'__'+$('#txtItem_id').val(),'populate_confirm_style_form_data','short_quotation_v3_controller');
					//show_list_view('','stage_data_list','stage_data_div','short_quotation_v3_controller','');
					js_set_value();
				}
			}
		...
		function fnc_openJobPopup()
		{
			...
			page_link='short_quotation_v3_controller.php?action=style_tag_popup&data='+data;
		...
		function fnc_bom_data_load()
		{
			...
			if(job_no!="")
			{
				var str_data=return_global_ajax_value( job_no, 'budgete_cost_validate', '', 'short_quotation_v3_controller');
		...
		function fnc_total_calculate()
		{
			...
			for(j=1; j<=split_tmep_id.length; j++)
			{
				...
				item_tot_amount=($("#txtFabCst_"+itm_id).val()*1)+($("#txtSpOpa_"+itm_id).val()*1)+($("#txtWash_"+itm_id).val()*1)+($("#txtAcc_"+itm_id).val()*1)+($("#txtFrightCst_"+itm_id).val()*1)+($("#txtLabCst_"+itm_id).val()*1)+($("#txtMiscCst_"+itm_id).val()*1)+($("#txtOtherCst_"+itm_id).val()*1)+($("#txtCommCst_"+itm_id).val()*1);
				
				var itemCost='Fab='+$("#txtFabCst_"+itm_id).val()+'+ SP='+$("#txtSpOpa_"+itm_id).val()+'+ Wash='+$("#txtWash_"+itm_id).val()+'+ ACC='+$("#txtAcc_"+itm_id).val()+'+ FR='+$("#txtFrightCst_"+itm_id).val()+'+ Lab='+$("#txtLabCst_"+itm_id).val()+'+ Mis='+$("#txtMiscCst_"+itm_id).val()+'+ Others='+$("#txtOtherCst_"+itm_id).val()+'+ Comm='+$("#txtCommCst_"+itm_id).val()+'= Total='+item_tot_amount;
		...
		function openmypage_unapprove_request()
		{
			...
			var page_link = 'short_quotation_v3_controller.php?data='+data+'&action=unapp_request_popup';
    ...
    <div style="display:none"><? echo load_freeze_divs ("../../../",$permission);  ?></div>       
        <form name="confirmStyle_1" id="confirmStyle_1" autocomplete="off">
        	<table width="850">
                <tr>
                    ...
                    <td width="120"><? echo create_drop_down( "cbo_buyer_id", 120, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,30,90)) order by buyer_name","id,buyer_name", 1, "-Select Buyer-", $sql_data[0][csf('buyer_id')], "load_drop_down( 'short_quotation_v3_controller', this.value, 'load_drop_down_season_conf', 'season_conf_td'); load_drop_down( 'short_quotation_v3_controller',this.value, 'load_drop_down_sub_depConf', 'subConf_td' );",1 ); ?></td>
            ...
            <table width="940" cellspacing="0" border="1" rules="all" class="rpt_table">
        	<thead>
			...
			<?
            $sql_item_summ="select item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs from qc_item_cost_summary where mst_id='$qc_no' and status_active=1 and is_deleted=0";
            ...
            foreach($sql_result_item_summ as $rowItemSumm)
            {
                ...
                <tr id="tr_<? echo $z; ?>" bgcolor="<? echo $bgcolorN; ?>">
                    ...
                    <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtFabConkg_<?=$item_id; ?>" id="txtFabConkg_<?=$item_id; ?>" value="<?=number_format($item_wise_cons_arr[$item_id][12]['qty'],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_1'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:33px;" type="text" class="text_boxes_numeric" name="txtFabConmtr_<?=$item_id; ?>" id="txtFabConmtr_<?=$item_id; ?>" value="<?=number_format($item_wise_cons_arr[$item_id][23]['qty'],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_2'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:33px;" type="text" class="text_boxes_numeric" name="txtFabConyds_<?=$item_id; ?>" id="txtFabConyds_<?=$item_id; ?>" value="<?=number_format($item_wise_cons_arr[$item_id][27]['qty'],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_3'; ?>');" <?=$disable; ?>/></td>
                    <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtFabCst_<?=$item_id; ?>" id="txtFabCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("fabric_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_4'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtSpOpa_<?=$item_id; ?>" id="txtSpOpa_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("sp_operation_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_5'; ?>');" <?=$disable; ?> /></td>
                     <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtWash_<?=$item_id; ?>" id="txtWash_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("wash_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_17'; ?>');" <?=$disable; ?> /></td>
                     
                    <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtAcc_<?=$item_id; ?>" id="txtAcc_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("accessories_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_6'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:33px;" type="text" class="text_boxes_numeric" name="txtFrightCst_<?=$item_id; ?>" id="txtFrightCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("frieght_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_7'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:33px;" type="text" class="text_boxes_numeric" name="txtLabCst_<?=$item_id; ?>" id="txtLabCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("lab_test_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_8'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:33px;" type="text" class="text_boxes_numeric" name="txtMiscCst_<?=$item_id; ?>" id="txtMiscCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("miscellaneous_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_9'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:33px;" type="text" class="text_boxes_numeric" name="txtOtherCst_<?=$item_id; ?>" id="txtOtherCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("other_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_10'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtCommCst_<?=$item_id; ?>" id="txtCommCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("commission_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_11'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtFobDzn_<?=$item_id; ?>" id="txtFobDzn_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("fob_pcs")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_12'; ?>');" <?=$disable; ?> /></td>
                    
                    <td title="((CPM*100)/Efficiency)"><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtCpm_<?=$item_id; ?>" id="txtCpm_<?=$item_id; ?>" value="<? if($cppm==0) echo "0"; else echo number_format($cppm,4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_13'; ?>');" disabled <? //echo $disable; ?> /></td>
                    <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtSmv_<?=$item_id; ?>" id="txtSmv_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("smv")],4); ?>" onChange="fnc_total_calculate();" onBlur="fnc_cppm_cal(<?=$item_id; ?>); fncqclimit('<?=$item_id.'_14'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:37px;" type="text" class="text_boxes_numeric" name="txtCmCst_<?=$item_id; ?>" id="txtCmCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("cm_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_15'; ?>');" <?=$disable; ?> /></td>
			...
			$sql="select fab_cons_kg, fab_cons_yds, fab_amount, sp_oparation_amount, wash_amount, acc_amount, fright_amount, lab_amount, misce_amount, other_amount, comm_amount, fob_amount, cm_amount, rmg_ratio from qc_confirm_dtls where cost_sheet_id='$cost_sheet_id' and status_active=1 and is_deleted=0";
        ...
        <table width="940" cellspacing="0" border="1" rules="all" class="rpt_table">
        	<tr id="tr_qc" bgcolor="#CCFFCC">
                ...
                <td width="45"><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtMiscCst_qc" id="txtMiscCst_qc" value="" disabled /></td>
                <td width="45"><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtOtherCst_qc" id="txtOtherCst_qc" value="" disabled /></td>
        	...
        	<tr id="tr_bom" bgcolor="#CCCCCC">
                ...
                <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtMiscCst_bom" id="txtMiscCst_bom" value="" readonly /></td>
    ...
    <script>get_php_form_data($('#txt_costSheet_id').val()+'__'+$('#txtItem_id').val(),'populate_confirm_style_form_data','short_quotation_v3_controller'); fnc_bom_data_load(); fnc_total_calculate(); fnc_select();</script>          
    <script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
    </html>
    <?
	exit();	
...
if($action=="budgete_cost_validate")
{
	...
	if($job_no!="")
	{
		...
		$dataArr=sql_select($sql);
		$fab_amount=$sp_oparation_amount=$wash_amount=$acc_amount=$fright_amount=$lab_amount=$misce_amount=$other_amount=$comm_amount=$fob_amount=$cm_amount=$rmg_ratio=0;
		...
		$str_data=$fab_cons_kg."##".$fab_cons_mtr."##".$fab_cons_yds."##".$fab_amount."##".$sp_oparation_amount."##".$acc_amount."##".$fright_amount."##".$lab_amount."##".$misce_amount."##".$other_amount."##".$comm_amount."##".$fob_amount."##".$cm_amount."##".$rmg_ratio."##".$wash_amount;
		unset($dataArr);
	}
        ...
        <table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
            ...
            <tr class="general">
                <td> 
					...
					<? echo create_drop_down( "cbo_company_mst", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-Select Company-", $cbo_company_name,"load_drop_down( 'short_quotation_v3_controller', this.value, 'load_drop_down_buyer_pop', 'buyer_pop_td' );",'' ); ?>
                </td>
		...
		<fieldset>
            <table cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr>
                        <td align="center">
                        	 <? echo create_drop_down( "cbo_buyer_id", 120, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,30,90)) order by buyer_name","id,buyer_name",1, "-- All Buyer--",$selected,"load_drop_down( 'short_quotation_v3_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'short_quotation_v3_controller',this.value, 'load_drop_down_sub_dep', 'sub_td' );",0 ); ?>
                        </td>                 
                        <td id="season_td"><? echo create_drop_down( "cbo_season_id", 130, $blank_array,'', 1, "-- Select Season--",$selected, "" ); ?></td>
                        <td id="sub_td"><? echo create_drop_down( "cbo_subDept_id", 130, $blank_array,'', 1, "-- Select Dept--",$selected, "" ); ?></td>     
...
if($action=="agent_location_popup")
{
		...
		function fnc_agent_location_reponse()
		{
			if(http.readyState == 4) 
			{
				...
				if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
				{
					...
					show_list_view(type,'agent_location_data_list','agent_location_data_div','short_quotation_v3_controller','');
				}
    ...
    <script>show_list_view(type,'agent_location_data_list','agent_location_data_div','short_quotation_v3_controller','');</script>          
    <script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
    </html>
    <?
	exit();	
...
{
	echo create_drop_down( "cbo_buyer_name", 130, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,30,90))  order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down( 'short_quotation_v3_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'short_quotation_v3_controller', this.value, 'load_drop_down_brand', 'brand_td');" );
	exit();
}
    ...
    <div align="center" style="width:100%;" >
        <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            <table width="910" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
                ...
                <tr class="general">
                    <td><? echo create_drop_down( "cbo_company_mst", 140, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", '',"load_drop_down( 'short_quotation_v3_controller', this.value, 'load_drop_down_buyer', 'buyer_td');" ); ?><input type="hidden" id="selected_id"></td>
...
if($action=="trims_cost_template_name_popup")
{
    ...
    <script>
        ...
        {
            var template_data=return_global_ajax_value( data, 'get_template_data', '', 'short_quotation_v3_controller');
