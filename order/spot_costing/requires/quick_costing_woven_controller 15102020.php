...
if($action=="save_update_delete")
{
	...
	where  b.team_id in (select id from lib_marketing_team a where user_tag_id='$user_id' and a.status_active=1 and a.is_deleted=0 ) and b.status_active=1 and b.is_deleted=0");
		...
		foreach($temp_id as $item_id)
		{
			...
			if( $itemBodyData[0]=='' ) $itemBodyData=array(17,20,21,22,997);//$itemBodyData=array(1,4,9,998,999);
			$m=1;
			//echo "10**<pre>";
...
where  b.team_id in (select id from lib_marketing_team a where user_tag_id='$user_id' and a.status_active=1 and a.is_deleted=0 ) and b.status_active=1 and b.is_deleted=0");
		...
		$field_array_mst="temp_id*lib_item_id*inquery_id*style_ref*buyer_id*uom*cons_basis*season_id*season_year*brand_id*style_des*department_id*delivery_date*exchange_rate*offer_qty*quoted_price*tgt_price*stage_id*costing_date*buyer_remarks*option_remarks*updated_by*update_date";
...
if($action=="style_popup")
{
	...
	{
		show_list_view (document.getElementById('cbo_buyer_id').value+'**'+document.getElementById('cbo_season_id').value+'**'+document.getElementById('cbo_subDept_id').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('txt_cost_sheet_no').value+'**'+document.getElementById('hide_dataType').value+'**'+document.getElementById('cbo_approval_type').value, 'style_ref_search_list_view', 'search_div', 'quick_costing_woven_controller', 'setFilterGrid(\'tbl_list_search\',-1)');
		set_all();
	}
    ...
    <div align="center">
		...
		<fieldset>
            <table cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr class="general">
                        <td><? echo create_drop_down( "cbo_buyer_id", 130, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name",1, "-- All Buyer--",$buyerId,"load_drop_down( 'quick_costing_woven_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'quick_costing_woven_controller',this.value, 'load_drop_down_sub_dep', 'sub_td' );",0 ); ?></td>                 
...
if($action=="temp_style_list_view")
{
	...
	$sql_temp_style=sql_select("select a.id, a.cost_sheet_no, a.style_ref, a.season_id, a.inserted_by, a.updated_by, a.qc_no from qc_mst a, qc_style_temp_data b where a.id=b.mst_id and b.inserted_by='$user_id' and a.entry_form=430 and b.status_active=1 and b.is_deleted=0 order by a.style_ref ASC");
...
if($action=="copy_cost_sheet")
{
	...
	{	
		...
		if (is_duplicate_field( "cost_sheet_no", "qc_mst", "style_ref='$dup_styleRef' and season_id='$dup_season_id' and buyer_id='$dup_buyer_id' and entry_form=430 and status_active=1 and is_deleted=0 " ) == 1)
		{
			echo "11**".$msg; 
			die;
		}
		...
		$field_array_mst="id, cost_sheet_id, cost_sheet_no, temp_id, lib_item_id, inquery_id, style_ref, buyer_id, uom, cons_basis, season_id, season_year, brand_id, style_des, department_id, delivery_date, exchange_rate, offer_qty, quoted_price, tgt_price, stage_id, costing_date, buyer_remarks, pre_cost_sheet_id, revise_no, option_id, qc_no, entry_form, inserted_by, insert_date";
		...
		$data_array_mst="(".$mst_id.",".$mst_id.",'".$autoCostSheetNo."','".$cbo_temp_id."','".$txt_temp_id."','".$txt_inquery_id."',".$txt_styleRef.",'".$cbo_buyer_id."','".$cbouom."','".$cbo_cons_basis_id."',".$cbo_season_id.",'".$cbo_season_year."','".$cbo_brand."','".$txt_styleDesc."','".$cbo_subDept_id."','".$txt_delivery_date."','".$txt_exchangeRate."','".$txt_offerQty."','".$txt_quotedPrice."','".$txt_tgtPrice."','".$cbo_stage_id."','".$txt_costingDate."','".$txt_costing_remarks."','".$hid_qc_no."','".$revise_no."','".$option_id."','".$qcno."',430,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
    ...
    <div align="center" style="width:100%;" >
        <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            <table width="910" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
                <thead>
                    <tr>
                    	<th colspan="9"><? echo create_drop_down( "cbo_string_search_type", 140, $string_search_type,'', 1, "-- Search Catagory --" ); ?></th>
                    </tr>
                    <tr>
                ...
                <tr class="general">
                    <td><? echo create_drop_down( "cbo_company_mst", 140, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", '',"load_drop_down( 'quick_costing_woven_controller', this.value, 'load_drop_down_buyer', 'buyer_td');" ); ?><input type="hidden" id="selected_id"></td>
...
if($action=="create_inquery_id_list_view")
{
	...
	$sql= "select id, system_number_prefix_num, company_id, buyer_id, season_buyer_wise, season_year, inquery_date, buyer_request, style_refernce, brand_id from wo_quotation_inquery where status_active=1  and is_deleted=0 $company $buyer $est_ship_date $inquery_id_cond $style_cond $season_cond $brand_cond 
	order by id DESC";//and id not in (select inquery_id from qc_mst where and status_active=1 and is_deleted=0 and inquery_id!=0)
	//echo $sql;
	echo  create_list_view("list_view", "Inquiry ID,Buyer Name,Style Ref,Season,Inquiry Date, Buyer Request,Brand", "60,120,100,80,70,80","800","280",0, $sql , "js_set_value", "id,buyer_id,style_refernce,system_number_prefix_num,season_buyer_wise,season_year,brand_id", "", 1, "0,buyer_id,0,season_buyer_wise,0,0,brand_id", $arr , "system_number_prefix_num,buyer_id,style_refernce,season_buyer_wise,inquery_date,buyer_request,brand_id", "",'','0,0,0,0,3,0,0') ;
	exit();
}