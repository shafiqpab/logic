...
if ($action=="load_drop_down_option_id")
{
	...
	/*if($ex_data[1]!=0)
		...
		$selectopt=$selected;*/
		//load_drop_down('requires/short_quotation_v6_controller', '".$row[csf("cost_sheet_no")].'__'.$row[csf("option_id")].'__'.$row[csf("revise_no")]."', 'load_drop_down_revise_no', 'revise_td');
	...
	if($ex_data[1]!=0)
		...
		$selectopt=$max;
	//echo $sql;
	echo create_drop_down( "cbo_option_id", 45, $rvs,"", 0, "-0-", $selectopt, "load_drop_down( 'requires/short_quotation_v6_controller', document.getElementById('txt_costSheetNo').value+'__'+this.value+'__'+0, 'load_drop_down_revise_no', 'revise_td'); fnc_option_rev( document.getElementById('cbo_revise_no').value+'***'+this.value+'***2' );" );//
	exit();
}
...
if($action=="template_popup")
{
	...
	function fnc_template_entry( operation )
	{
		...
		http.open("POST","short_quotation_v6_controller.php",true);
	...
	function fnc_template_entry_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			if(reponse[0]==0 || reponse[0]==1)
			{
				...
				show_list_view( '','template_list_view','save_up_list_view','short_quotation_v6_controller','setFilterGrid(\'tbl_upListView\',-1)');
			}
	...
	{
		var list_view_grid = return_global_ajax_value( temp_id, 'load_php_dtls_form', '', 'short_quotation_v6_controller');
		if(list_view_grid!='')
		{
                ...
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="282" class="rpt_table" id="tbl_tempCreat" >
                	<tbody id="tbd_temp">
                        <tr id="tr_1">
                            ...
                            <? echo create_drop_down( "cboItemBts_1", 150, $garments_item,"", 1, "-Select-", 0, "", "", '92,93', "", "", "", "", "", "cboItemBts[]" ); ?></td>
...
if($action=="rate_details_popup")
{
	...
	$adata=$qc_no.'!!!'.$bodypartid.'!!!'.$consRateId.'!!!'.$txtdznkg.'!!!'.$txt_fabDesc.'!!!'.$cbocolortype.'!!!'.$txtgsmweight.'!!!'.$txtfabwidth.'!!!'.$totConsumtion.'!!!'.$itemRatio.'!!!'.$fabid.'!!!'.$rateData;
	?>
    <script>
		var qc_no='<?=$qc_no; ?>';
		...
		var adata='<?=$adata; ?>';
		var fabgsm='<?=$txtgsmweight; ?>';
		var fabwidth='<?=$txtfabwidth; ?>';
		var fabuom='<?=$fabuom; ?>';
		var fabid='<?=$fabid; ?>';
		var itemRatio='<?=$itemRatio; ?>';
		var totConsumtion='<?=$totConsumtion; ?>';
		...
		function fnc_cons_amt(str)
		{
			...
			if(valstr[1]==1)
			{
				var fabamt=(totConsumtion*1)*($('#txtfabrate').val()*1);
				$('#txtfabamt').val( number_format(fabamt,4,'.','') );
			}
			else if(valstr[1]==2)
			{
				var yarnCons=($('#txtyarncons_'+inc).val()*1);
				var yarnRate=($('#txtyarnrate_'+inc).val()*1);
				
				var yarnAmt=yarnCons*yarnRate;
				
				$('#txtyarnamt_'+inc).val( number_format(yarnAmt,4,'.','') );
			}
			else if(valstr[1]==3)
			{
				var convCons=($('#txtconvcons_'+inc).val()*1);
				var convRate=($('#txtconvrate_'+inc).val()*1);
				
				var convAmt=convCons*convRate;
				
				$('#txtconvamt_'+inc).val( number_format(convAmt,4,'.','') );
			}
		...
		function fnc_total_rate(type)
		{
			...
			var totCost=($('#txtfabamt').val()*1)+($('#txt_tot_yarnamt').val()*1)+($('#txt_tot_convamt').val()*1);
			...
			$('#txttotamount').val( number_format(totCost,4,'.','') );
		}
        ...
        {
            //alert(row)
            emailwindow=dhtmlmodal.open('EmailBox', 'iframe','short_quotation_v6_controller.php?action='+action,'Details Popup', 'width=580px,height=300px,center=1,resize=0','../../');
            emailwindow.onclose=function()
            {
               ...
               // alert(popupData);
                if(action=='fabric_popup'){
                    $('#hidLibid_'+row).val(popupData[0]);
					var exdata=popupData[1]+', '+popupData[2]+', '+popupData[3];
                    $('#txtFabricDtls_'+row).val(exdata);
					$('#txtYarnDtls_'+row).val(popupData[4]);
					$('#txtConvDtls_'+row).val(popupData[5]);
                }
			...
			if(strval[1]==3)
			{
				...
				if(c==1)
				{
					$('#tbl_conv tbody').append(
						...
						+ '<td><?=create_drop_down( "cboprocess_'+counter+'", 200, $conversion_cost_head_array,"", 1, "-Select-", $selected, "","","","","" ); ?></td><td><input style="width:58px;" type="text" class="text_boxes_numeric" name="txtconvcons_'+counter+'" id="txtconvcons_'+counter+'" placeholder="Write" onChange="fnc_cons_amt('+"'"+counter+"_3'"+');" onBlur="append_row('+"'"+counter+"_3'"+');" value="'+totConsumtion+'" /></td><td><input style="width:48px;" type="text" class="text_boxes_numeric" name="txtconvrate_'+counter+'" id="txtconvrate_'+counter+'" onChange="fnc_cons_amt('+"'"+counter+"_3'"+');" value="" /></td><td><input style="width:58px;" type="text" class="text_boxes_numeric" name="txtconvamt_'+counter+'" id="txtconvamt_'+counter+'" placeholder="Display" readonly value="" /><input type="hidden" class="text_boxes" name="hidconvupid_'+counter+'" id="hidconvupid_'+counter+'" value=""/></td>'+ '</tr>'
					);
				}
			}
        ...
        {
			freeze_window(operation);
			var counter =$('#yarntbody tr').length; 
			var z=1; var dataAll="";
			for(var i=1; i<=counter; i++)
			{
				if( ($("#txtyarncons_"+i).val()*1)!=0)
				{
					 dataAll+="&cbocount_" + z + "='" + $('#cbocount_'+i).val()+"'"+"&hidfabid_" + z + "='" + $('#hidfabid_'+i).val()+"'"+"&cbocompone_" + z + "='" + $('#cbocompone_'+i).val()+"'"+"&txtpercentone_" + z + "='" + $('#txtpercentone_'+i).val()+"'"+"&cbotype_" + z + "='" + $('#cbotype_'+i).val()+"'"+"&cbo_yarn_supplier_" + z + "='" + $('#cbo_yarn_supplier_'+i).val()+"'"+"&txtyarncons_" + z + "='" + $('#txtyarncons_'+i).val()+"'"+"&txtyarnrate_" + z + "='" + $('#txtyarnrate_'+i).val()+"'"+"&txtyarnamt_" + z + "='" + $('#txtyarnamt_'+i).val()+"'"+"&hidyarnupid_" + z + "='" + $('#hidyarnupid_'+i).val()+"'";
					z++;
				}
			}
            ...
            var data="action=save_update_delete_yarn&operation="+operation+'&numRowYarn='+z+get_submitted_data_string('hid_qc_no*hid_bodypart_id*hid_body_cons*txttotamount*hid_cons_rate_dtls_id*txtYarnDtls*txtfabrate*txtfabamt*hidLibid*hidupid',"../../../")+dataAll;
		...
		function fnc_conv_entry(operation)
        {
			...
			for(var i=1; i<=counter; i++)
			{
				if( ($("#txtconvrate_"+i).val()*1)!=0)
				{
					 dataAll+="&cboprocess_" + z + "='" + $('#cboprocess_'+i).val()+"'"+"&txtconvcons_" + z + "='" + $('#txtconvcons_'+i).val()+"'"+"&txtconvrate_" + z + "='" + $('#txtconvrate_'+i).val()+"'"+"&txtconvamt_" + z + "='" + $('#txtconvamt_'+i).val()+"'"+"&hidconvupid_" + z + "='" + $('#hidconvupid_'+i).val()+"'";
					z++;
				}
			}
			...
			var data="action=save_update_delete_conv&operation="+operation+'&numRowConv='+z+get_submitted_data_string('hid_qc_no*hid_bodypart_id*hid_body_cons*txttotamount*hid_cons_rate_dtls_id*txtYarnDtls*txtfabrate*txtfabamt*hidLibid*hidupid',"../../../")+dataAll;
		...
		function fnc_row_generate()
		{
			...
			show_list_view(data,'populate_confirm_fabric_data','fabricDiv','short_quotation_v6_controller','');
			fnc_cons_amt("0_1");
		}
	...
	<body onLoad="set_hotkey();">
        ...
        <input type="hidden" class="text_boxes" name="hid_body_cons" id="hid_body_cons" value="<?=$totConsumtion; ?>">
        <input type="hidden" class="text_boxes" name="hidLibid" id="hidLibid" value="<?=$fabid; ?>"/>
    </body>
...
if($action=="accrate_details_popup")
{
    ...
    <script>
		...
		function fnc_accrow_generate()
		{
			...
			show_list_view(data,'populate_confirm_acc_data','accDiv','short_quotation_v6_controller','');
		}
...
if($action=="populate_confirm_fabric_data")
{
	...
	$gsmweight=$edata[6];
	$fabwidth=$edata[7];
	$totConsumtion=$edata[8];
	$itemRatio=$edata[9];
	$fabid=$edata[10];
	$ratedata=$edata[11];
	
	$sqlFab="select id, type, tot_cons, rate, value from qc_fab_yarn_conv where qc_no='$qc_no' and type=1 and cons_rate_dtls_id='$consRateId' and status_active=1 and is_deleted=0";
	$sqlFabData=sql_select($sqlFab);
	$fab_upid=$sqlFabData[0][csf('id')];
	$fab_rate=$sqlFabData[0][csf('rate')];
	$fab_value=$sqlFabData[0][csf('value')];
	?>
    <table width="700" cellspacing="0" border="1" class="rpt_table" id="tbl_fab" rules="all">
        <thead>
            <td colspan="8" align="center" bgcolor=""><b style="background-color:#99FFCC"><?=$body_part[$body_part_id].','.$fabDesc.','.$color_type[$colortype].';  GSM: '.$gsmweight.';  F. Width [Inch]: '.$fabwidth; ?>; </b>&nbsp;  &nbsp; &nbsp; <span style="background-color:#FFFFFF"><b style="color:#00F">Fabric Rate :</b><input style="width:48px;" type="text" class="text_boxes_numeric" name="txtfabrate" id="txtfabrate" value="<?=$fab_rate; ?>" onBlur="fnc_cons_amt('<?='1_1'; ?>');" /></span>&nbsp;  &nbsp; &nbsp;<span style="background-color:#FFFFFF"><b style="color:#00F">Fabric Amount :</b><input style="width:48px;" type="text" class="text_boxes_numeric" name="txtfabamt" id="txtfabamt" value="<?=$fab_value; ?>" readonly /><input type="hidden" class="text_boxes" name="hidupid" id="hidupid" value="<?=$fab_upid; ?>"/></span></td>
        </thead>
        <thead>
        ...
        <thead>
            ...
            <th width="150" class="must_entry_caption">Comp</th>
        ...
        <?
			$yarnCountArr=return_library_array("select id,yarn_count from lib_yarn_count where is_deleted=0 and status_active=1 order by yarn_count", "id","yarn_count");
           	$sqlFab="select id, type, yarn_lib_id, fab_des, count, composition, percent, yarn_type, process, fab_id, cons, exper, tot_cons, rate, value, nominated_supp_multi from qc_fab_yarn_conv where qc_no='$qc_no' and type=2 and cons_rate_dtls_id='$consRateId' and status_active=1 and is_deleted=0";
            
            $sqlFabData=sql_select($sqlFab); $yarnBtn=1;
			$composition_arr=array(); $yarn_descriptionArr="";
			
			$sql="select a.id, b.id as bid, b.copmposition_id, b.percent, b.count_id, b.type_id, b.yarn_rate from lib_yarn_count_determina_mst a, lib_yarn_count_determina_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.id='$fabid' order by a.id, b.id";
			$data_array=sql_select($sql);
			if (count($data_array)>0)
			{
				foreach( $data_array as $row )
				{
					...
					if($yarn_descriptionArr!="")
					{
						$yarn_descriptionArr.="__".$row[csf('count_id')]."_".$row[csf('copmposition_id')]."_100_".$row[csf('type_id')]."_".$row[csf('percent')];
					}
					else 
					{
						$yarn_descriptionArr=$row[csf('count_id')]."_".$row[csf('copmposition_id')]."_100_".$row[csf('type_id')]."_".$row[csf('percent')];
					}
				}
				unset($data_array);
			}
			if(count($sqlFabData)<1 && $yarn_descriptionArr!="")
			{
				$sqlFabData=explode("__",$yarn_descriptionArr);
				$yarnBtn=0;
				//print_r($sqlFabData);
			}
            ...
            {
				//print_r($row);
				$datatype=$yarncount=$compositionbval=$percent=$yarn_type_id=$nominated_supp_multi=$cons=$rate=$value=0;
				$fab_id=$upyarnid="";
				if($yarnBtn==0)
				{
					$exyarndata=explode("_",$row);
					//print_r($exyarndata);
					$yarncount=$exyarndata[0];
					$fab_id=$consRateId;
					$upyarnid='';
					$compositionbval=$exyarndata[1];
					$percent=$exyarndata[4];
					$yarn_type_id=$exyarndata[3];
					$nominated_supp_multi=0;
					$cons=0;
					$rate=0;
					$value=0;
					$datatype=2;
				...
				{
					$yarncount=$row[csf("count")];
					$fab_id=$row[csf("fab_id")];
					$upyarnid=$row[csf("id")];
					$compositionbval=$row[csf("composition")];
					$percent=$row[csf("percent")];
					$yarn_type_id=$row[csf("yarn_type")];
					$nominated_supp_multi=$row[csf("nominated_supp_multi")];
					$cons=$row[csf("cons")];
					$rate=$row[csf("rate")];
					$value=$row[csf("value")];
					$datatype=$row[csf("type")];
				}
				if($datatype==2)
				{
					$consyarn=def_number_format((($totConsumtion*$percent)/100),5,"");
					?>
					<tr id="yarntr_<?=$y; ?>">
						<td><? echo create_drop_down( "cbocount_".$y, 80, $yarnCountArr, "",1,"-Count-", $yarncount, "","","" ); ?>
							<input type="hidden" class="text_boxes" name="hidfabid_<?=$y; ?>" id="hidfabid_<?=$y; ?>" value="<?=$consRateId; ?>"/>
							<input type="hidden" class="text_boxes" name="hidyarnupid_<?=$y; ?>" id="hidyarnupid_<?=$y; ?>" value="<?=$upyarnid; ?>"/>
						</td>
						<td><? echo create_drop_down( "cbocompone_".$y, 150, $composition,"", 1, "-Select-", $compositionbval, "","","","","" ); ?></td>
						<td><input type="text" id="txtpercentone_<?=$y; ?>"  name="txtpercentone_<?=$y; ?>" class="text_boxes_numeric" style="width:38px" value="<?=$percent; ?>" readonly disabled /></td>
						<td><? echo create_drop_down( "cbotype_".$y, 110, $yarn_type,"", 1, "-Select-", $yarn_type_id, "","","","","","" ); ?></td>
						<td><?=create_drop_down("cbo_yarn_supplier_$y", 120, $supplier_library_fabric,'', 1, '-Select-',$nominated_supp_multi, "",'',"" ); ?></td>
						<td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtyarncons_<?=$y; ?>" id="txtyarncons_<?=$y; ?>" placeholder="Write" onChange="fnc_cons_amt('<?=$y.'_2'; ?>');" value="<?=$consyarn; ?>" readonly /></td>
						<td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtyarnrate_<?=$y; ?>" id="txtyarnrate_<?=$y; ?>" onChange="fnc_cons_amt('<?=$y.'_2'; ?>');" value="<?=$rate; ?>" /></td>
						<td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtyarnamt_<?=$y; ?>" id="txtyarnamt_<?=$y; ?>" placeholder="Display" readonly value="<?=$value; ?>" /></td>
					</tr>
					<?
					//$yarnBtn=1;
					$y++;
					$consYarnTot+=$consyarn;
					$amtYarnTot+=$value;
				}
            }
			if (count($sqlFabData)<1)
			{
				?>
				<tr id="yarntr_<?=$y; ?>">
					<td><? echo create_drop_down( "cbocount_".$y, 80, $yarnCountArr, "",1,"-Count-", $selected, "","","" ); ?>
					...
					<td><? echo create_drop_down( "cbocompone_".$y, 150, $composition,"", 1, "-Select-", $selected, "","","","","" ); ?></td>
					...
					<td><?=create_drop_down("cbo_yarn_supplier_$y", 120, $supplier_library_fabric,'', 1, '-Select-',"", "",'',"" ); ?></td>
					<td><input style="width:38px;" type="text" class="text_boxes_numeric" name="txtyarncons_<?=$y; ?>" id="txtyarncons_<?=$y; ?>" placeholder="Write" onChange="fnc_cons_amt('<?=$y.'_2'; ?>');" readonly /></td>
        ...
        <tfoot>
            <tr bgcolor="#CCCCAA">
                <td colspan="5" align="right"><b><i>Total Yarn Cost=</i></b></td>
                ...
                <td><input type="hidden" class="text_boxes" name="txtYarnDtls" id="txtYarnDtls" value="<?=$yarn_descriptionArr; ?>"/></td>
    ...
    <table width="400" cellspacing="0" border="1" class="rpt_table" id="tbl_conv" rules="all">
        <thead>
            <td colspan="4" align="center" bgcolor="#99FFCC"><b>Conversion Charge Details</b></td>
        </thead>
        <thead>
            <th width="200" class="must_entry_caption">Process</th>
            <th width="70" class="must_entry_caption">Cons Qty</th>
            <th width="60">Rate</th>
            <th>Amount</th>
        </thead>
        <tbody id="convbody">
            ...
            foreach($sqlFabData as $row)
            {
                if($row[csf("type")]==3)
                {
                    ?>
                    <tr id="convtr_<?=$y; ?>">
                        <td><? echo create_drop_down( "cboprocess_".$c, 200, $conversion_cost_head_array,"", 1, "-Select-", $row[csf("process")], "","","","","" ); ?></td>
                        <td><input style="width:58px;" type="text" class="text_boxes_numeric" name="txtconvcons_<?=$c; ?>" id="txtconvcons_<?=$c; ?>" placeholder="Display" onChange="fnc_cons_amt('<?=$c.'_3'; ?>');" onBlur="append_row('<?=$c.'_3'; ?>');" value="<?=$totConsumtion; ?>" readonly /></td>
                        <td><input style="width:48px;" type="text" class="text_boxes_numeric" name="txtconvrate_<?=$c; ?>" id="txtconvrate_<?=$c; ?>" onChange="fnc_cons_amt('<?=$c.'_3'; ?>');" value="<?=$row[csf("rate")]; ?>" /></td>
                        <td>
                            <input style="width:58px;" type="text" class="text_boxes_numeric" name="txtconvamt_<?=$c; ?>" id="txtconvamt_<?=$c; ?>" placeholder="Display" readonly value="<?=$row[csf("value")]; ?>" />
            ...
            ?>
           <tr id="convtr_<?=$y; ?>">
                <td><?=create_drop_down( "cboprocess_".$c, 200, $conversion_cost_head_array,"", 1, "-Select-", $selected, "","","","","" ); ?></td>
                <td><input style="width:58px;" type="text" class="text_boxes_numeric" name="txtconvcons_<?=$c; ?>" id="txtconvcons_<?=$c; ?>" placeholder="Display" onChange="fnc_cons_amt('<?=$c.'_3'; ?>');" value="<?=$totConsumtion; ?>" readonly /></td>
                <td><input style="width:48px;" type="text" class="text_boxes_numeric" name="txtconvrate_<?=$c; ?>" id="txtconvrate_<?=$c; ?>" onChange="fnc_cons_amt('<?=$c.'_3'; ?>');" onBlur="append_row('<?=$c.'_3'; ?>');" /></td>
                <td>
                	<input style="width:58px;" type="text" class="text_boxes_numeric" name="txtconvamt_<?=$c; ?>" id="txtconvamt_<?=$c; ?>" placeholder="Display" readonly />
        ...
        <tfoot>
            <tr bgcolor="#CCCCAA">
                <td align="right"><b><i>Total Conversion Cost=</i></b></td>
                <td><input style="width:58px;" type="text" class="text_boxes_numeric" name="txt_tot_convcons" id="txt_tot_convcons" value="<?=$consConvTot; ?>" disabled /></td>
                <td>&nbsp;</td>
                <td><input style="width:58px;" type="text" class="text_boxes_numeric" name="txt_tot_convamt" id="txt_tot_convamt" value="<?=$amtConvTot; ?>" disabled /></td>
            </tr>
            <tr>
    ...
    <table width="300" cellspacing="0" border="1" class="rpt_table" rules="all">
        ...
        <tfoot>
        	<td colspan="2" align="center"><input type="button" class="formbutton" style="width:100px" value="Close" onClick="js_set_value();"/></td> 
        </tfoot>
    </table>
	<?
	unset($sqlFabData);
...
if($action=="save_update_delete_yarn")
{
    ...
    if ($operation==0)  // Insert Here
    {
		...
		{
			$fab_field_array="id, qc_no, body_part_id, type, yarn_lib_id, tot_cons, rate, value, yarn_break_down, cons_rate_dtls_id, inserted_by, insert_date, status_active, is_deleted";
			
			$fab_data_array .="(".$id.",".$hid_qc_no.",".$hid_bodypart_id.",1,".$hidLibid.",".$hid_body_cons.",".$txtfabrate.",".$txtfabamt.",".$txtYarnDtls.",".$hid_cons_rate_dtls_id.",'".$user_id."','".$pc_date_time."',1,0)";
			$id++;
		}
		else
		{
			$fabup_field_array="body_part_id*yarn_lib_id*tot_cons*rate*value*yarn_break_down*updated_by*update_date";
			
			$fabup_data_array="".$hid_bodypart_id."*".$hidLibid."*".$hid_body_cons."*".$txtfabrate."*".$txtfabamt."*".$txtYarnDtls."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		}
		
		$field_array="id, qc_no, body_part_id, type, yarn_lib_id, fab_id, count, composition, percent, yarn_type, cons, rate, value, cons_rate_dtls_id, inserted_by, insert_date, status_active, is_deleted";
        ...
        for($i=1; $i<=$numRowYarn; $i++) 
        {
			...
			{
				if ($add_comma!=0) $data_array .=","; $add_comma=0;
				$data_array .="(".$id.",".$hid_qc_no.",".$hid_bodypart_id.",2,".$hidLibid.",".$$hidfabid.",".$$cbocount.",".$$cbocompone.",".$$txtpercentone.",".$$cbotype.",".$$txtyarncons.",".$$txtyarnrate.",".$$txtyarnamt.",".$hid_cons_rate_dtls_id.",'".$user_id."','".$pc_date_time."',1,0)";
				$id++; $add_comma++;
			}
        }
		
		$fabricrate=str_replace("'",'',$txttotamount)/str_replace("'",'',$hid_body_cons);
		
		$fabricrate=number_format($fabricrate,4);
		
		//echo "10**insert into qc_fab_yarn_conv (".$field_array.") values ".$data_array;die;
		$flag=1;
		if(str_replace("'",'',$hidupid)=="")
		{
			$rID=sql_insert("qc_fab_yarn_conv",$fab_field_array,$fab_data_array,0);
			if($rID==1 && $flag==1) $flag=1; else $flag=0;
		}
		else
		{
			$rID=sql_update("qc_fab_yarn_conv",$fabup_field_array,$fabup_data_array,"id","".$hidupid."",1);
			if($rID==1 && $flag==1) $flag=1; else $flag=0;
		}
		
		$rID1=sql_insert("qc_fab_yarn_conv",$field_array,$data_array,0);
		if($rID1==1 && $flag==1) $flag=1; else $flag=0;
		
		$rIDMst=execute_query("update qc_cons_rate_dtls set rate='".$fabricrate."', value=".$txttotamount." where mst_id =".$hid_qc_no." and id =".$hid_cons_rate_dtls_id." and type=1");
		if($rIDMst==1 && $flag==1) $flag=1; else $flag=0;
		
		//echo "10**".$rID.'=='.$rID1.'=='.$rIDMst.'=='.$flag;  oci_rollback($con); disconnect($con); die;
    ...
    else if ($operation==1)   // Update Here
    {
		...
		//echo "10**".$rID.'=='.$fycRid.'=='.$rID1.'=='.$rIDMst.'=='.$flag;  oci_rollback($con); disconnect($con); die;
...
if($action=="save_update_delete_conv")
{
    ...
    if ($operation==0)  // Insert Here
    {
        ...
        for($i=1; $i<=$numRowConv; $i++) 
        {
			...
			{
				if ($add_comma!=0) $data_array .=","; $add_comma=0;
				$data_array .="(".$id.",".$hid_qc_no.",".$hid_bodypart_id.",3,".$hid_cons_rate_dtls_id.",".$$cboprocess.",".$$txtconvcons.",".$$txtconvrate.",".$$txtconvamt.",".$hid_cons_rate_dtls_id.",'".$user_id."','".$pc_date_time."',1,0)";
				$id++; $add_comma++;
			}
        }
...
if($action=="stage_saveUpdate_popup")
{
		...
		function fnc_stage_entry_reponse()
		{
			if(http.readyState == 4) 
			{
				...
				if(reponse[0]==0 || reponse[0]==1)
				{
					...
					show_list_view('','stage_data_list','stage_data_div','short_quotation_v6_controller','');
				}
			}
			release_freezing();
		}
    ...
    <script>show_list_view('','stage_data_list','stage_data_div','short_quotation_v6_controller','');</script>          
    <script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
    </html>
    <?
	exit();	
...
{
	echo  create_list_view ( "list_view", "Stage Name", "200","200","150",1, "select tuid, stage_name from lib_stage_name where status_active=1 and is_deleted=0 order by tuid desc", "get_php_form_data", "tuid","'load_php_data_to_form_satge'", 1, "0", $arr, "stage_name","short_quotation_v6_controller", 'setFilterGrid("list_view",-1);','0' );
	exit();
}
...
if($action=="save_update_delete")
{
	...
	where  b.team_id in (select id from lib_marketing_team a where user_tag_id='$user_id' and a.status_active=1 and a.is_deleted=0 ) and b.status_active=1 and b.is_deleted=0");
		...
		$field_array_consrate="id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, value, fab_id, description, body_part_id, supp_ref, remarks, uom, inserted_by, insert_date, status_active, is_deleted, color_type_id, gsmweight, fabwidth, rate_data, use_for, yarn_break_down";
		...
		foreach($temp_id as $item_id)
		{
			...
			foreach($ex_itemConsRateData as $item_wise_consRateData)
			{
				if($item_wise_consRateData!="")
				{
					...
					$fabDesc=$usefor=$rateData="";
					$bodypartid=$fabid=$colortype=$gsmweight=$fabwidth=$dznkg=$conspack=$exPer=$totCons=$ratePop=$rate=$fabAmt=$fabupid=$fabuom=0;
					
					$bodypartid=$ex_itemConsRate[1];
					$fabDesc=$ex_itemConsRate[2];
					$fabid=$ex_itemConsRate[3];
					$colortype=$ex_itemConsRate[4];
					$gsmweight=$ex_itemConsRate[5];
					$fabwidth=$ex_itemConsRate[6];
					$usefor=$ex_itemConsRate[7];
					$conspack=$ex_itemConsRate[8];
					$exPer=$ex_itemConsRate[9];
					$totCons=$ex_itemConsRate[10];
					$ratePop=$ex_itemConsRate[11];
					$rateData=$ex_itemConsRate[12];
					$rate=$ex_itemConsRate[13];
					$fabAmt=$ex_itemConsRate[14];
					$fabupid=$ex_itemConsRate[15];
					$fabuom=$ex_itemConsRate[16];
					$yarnBreakDown=$ex_itemConsRate[17];
					
					$consRateIdMapArr[$fabupid]=$cons_rate_id;
					
					if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
					$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',1,0,'".$conspack."','".$exPer."','".$totCons."','".$ratePop."','".$rate."','".$fabAmt."','".$fabid."','".$fabDesc."','".$bodypartid."','','','".$fabuom."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,'".$colortype."','".$gsmweight."','".$fabwidth."','".$rateData."','".$usefor."','".$yarnBreakDown."')";
			...
			foreach($ex_specialData as $item_wise_specialData)
			{
				...
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',2,'".$speciaOperation_id."','".$spConsumtion."','".$spExper."','".$totSpCons."',0,'".$spRate."','".$spValue."','".$speciaTypeId."','','".$spbodypartid."','','".$spRemarks."',0,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,0,0,0,'','','')";
			...
			foreach($ex_washData as $item_wise_washData)
			{
				...
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',3,'".$washTypeId."','".$wConsumtion."','".$wExper."','".$totwCons."',0,'".$wRate."','".$wValue."',0,'','".$wbodypartid."','','".$wRemarks."',0,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,0,0,0,'','','')";
			...
			foreach($ex_itemAccData as $item_wise_itemAccData)
			{
				...
				$accessories_id=$accDescription=$accBandRef=$accUseFor=$acConsumtion=$acExPer=$totAccCons=$acRate=$acVal=$acconsUom=$acConsCal=$acRateData=$accCalParaMeter=$accConsCalData=0;
				
				$accessories_id=$ex_itemAcConsRate[1];
				$accDescription=str_replace($str_rep,' ',str_replace("'","",$ex_itemAcConsRate[2]));
				$accBandRef=str_replace($str_rep,' ',str_replace("'","",$ex_itemAcConsRate[3]));
				...
				$accUseFor=str_replace($str_rep,' ',str_replace("'","",$ex_itemAcConsRate[9]));
				$acconsUom=$ex_itemAcConsRate[10];
				$acConsCal=$ex_itemAcConsRate[11];
				$acRateData=$ex_itemAcConsRate[12];
				$accCalParaMeter=$ex_itemAcConsRate[13];
				$accConsCalData=$ex_itemAcConsRate[14];
				
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',4,'".$accessories_id."','".$acConsumtion."','".$acExPer."','".$totAccCons."','".$acConsCal."','".$acRate."','".$acVal."',0,'".$accDescription."',0,'".$accBandRef."','','".$acconsUom."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,0,0,0,'".$accConsCalData."','".$accUseFor."','')";
		...
		if($type==6 || $type==7)
		{
			...
			$isYarnConvFound=count($fabSql);
			
			//echo "select id, qc_no, body_part_id, color_type_id, nominated_supp_multi, gauge, type, yarn_lib_id, count, composition, percent, yarn_type, fab_id, process, fab_des, gsm, dia, diatype, uom, cons, exper, tot_cons, rate, value, yarn_break_down, conv_break_down, cons_rate_dtls_id from qc_fab_yarn_conv where qc_no=$hid_qc_no and status_active=1 and is_deleted=0 order by id asc";// die;
		...
		$txt_optexp=trim($ex_tot_cost_data[21]);
		$totCourierCst=trim($ex_tot_cost_data[22]);
		$txt_lcPer=trim($ex_tot_cost_data[23]);
		$totDlcCst_td=trim($ex_tot_cost_data[24]);
		$totInspCst_td=trim($ex_tot_cost_data[25]);
		
		$data_array_tot_cost="(".$tc_sum_id.",".$qcno.",'".$cbo_buyer_agent."','".$cbo_agent_location."','".$txt_noOfPack."',0,'".$cmPop."','".$txt_lumSum_cost."','".$txt_commPer."','".$totFab_td."','".$totSpc_td."','".$totWash_td."','".$totAcc_td."','".$totCm_td."','".$totFriCst_td."','".$totCourierCst."','".$totLbTstCst_td."','".$totMissCst_td."','".$totOtherCst_td."','".$totCommCst_td."','".$totCost_td."','".$totalFob_td."','".$totRmgQty_td."','".$txt_commlPer."','".$totCommlCst_td."','".$txt_optexp."','".$txt_lcPer."','".$totDlcCst_td."','".$totInspCst_td."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		//echo $data_array_tot_cost; die;
		$field_array_item_tot=" id, mst_id, item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, courier_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs, rmg_ratio, commercial_cost, operating_exp, lc_cost, inspection_cost, inserted_by, insert_date, status_active, is_deleted";
		...
		{
			$fab_td=$spOpe_td=$wash_td=$acc_td=$txtCpm=$txtSmv=$txtEff=$txtCmCost=$txtFriCost=$txtCourierCost=$txtLtstCost=$txtMissCost=$txtOtherCost=$txtCommCost=$fobT_td=$fobDzn_td=$tot_item_id=$txtCommlCost=$optexp=$txtoptexp=$txtdlcCost=$txtInspCost=0;
			...
			$txtoptexp=trim($ex_item_tot[17]);
			$txtCourierCost=trim($ex_item_tot[18]);
			$txtdlcCost=trim($ex_item_tot[19]);
			$txtInspCost=trim($ex_item_tot[20]);
			$tot_item_id=trim($ex_item_tot[21]);
			
			if ($add_comma_item_tot!=0) $data_array_item_tot .=",";
			$data_array_item_tot .="(".$item_cost_id.",".$qcno.",'".$tot_item_id."','".$fab_td."','".$spOpe_td."','".$wash_td."','".$acc_td."','".$txtCpm."','".$txtSmv."','".$txtEff."','".$txtCmCost."','".$txtFriCost."','".$txtCourierCost."','".$txtLtstCost."','".$txtMissCost."','".$txtOtherCost."','".$txtCommCost."','".$fobT_td."','".$txtRmgQty."','".$txtCommlCost."','".$txtoptexp."','".$txtdlcCost."','".$txtInspCost."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		...
		if($isYarnConvFound>0)
		{
			if($type==6 || $type==7)
			{
				$rIDFabYarnConv=sql_insert("qc_fab_yarn_conv",$fabfield_array,$fabdata_array,0);
				if($rIDFabYarnConv==1 && $flag==1) $flag=1; else $flag=0;
			}
		}
		...
		//echo "10**".$flag."=".$rID."=".$rID_consRate."=".$rIDFabYarnConv."=".$rID_tot_cost."=".$rID_item_tot."=".$rID_item_tot."=".$rIDfabup; die;
	...
	{	
		...
		//echo "select cost_sheet_id from qc_confirm_mst where cost_sheet_id=$hid_qc_no and status_active=1 and is_deleted=0"; die;
...
where b.team_id in (select id from lib_marketing_team a where user_tag_id='$user_id' and a.status_active=1 and a.is_deleted=0 ) and b.status_active=1 and b.is_deleted=0");
		...
		$sqlYarnConv="select ID, TYPE, CONS_RATE_DTLS_ID, YARN_LIB_ID, FAB_DES, COUNT, COMPOSITION, PERCENT, YARN_TYPE, PROCESS, FAB_ID, CONS, EXPER, TOT_CONS, RATE, VALUE, NOMINATED_SUPP_MULTI from qc_fab_yarn_conv where qc_no='$qcno' and type in (1,2,3) and status_active=1 and is_deleted=0";
		//echo "10**".$sqlYarnConv; die;
		$sqlYarnConvArr=sql_select( $sqlYarnConv ); 
		
		$yarn_rateArr=array(); $conv_rateArr=array(); $isyarn=0; $isconv=0; $yarnBreakDownArr=array();
		foreach($sqlYarnConvArr as $drow)
		{
			if($drow['TYPE']==2)
			{
				$ystr="";
				$ystr=$drow['ID'].'_'.$drow['COUNT'].'_'.$drow['COMPOSITION'].'_'.$drow['YARN_TYPE'].'_'.$drow['PERCENT'];
				$yarn_rateArr[$drow['CONS_RATE_DTLS_ID']][$ystr]=$drow['RATE'];
				$isyarn=1;
			}
			if($drow['TYPE']==3)
			{
				$cstr="";
				$cstr=$drow['ID'].'_'.$drow['PROCESS'];
				$conv_rateArr[$drow['CONS_RATE_DTLS_ID']][$cstr]=$drow['RATE'];
				$isconv=1;
			}
			if($drow['TYPE']==1)
			{
				$yarnBreakDownArr[$drow['CONS_RATE_DTLS_ID']]=$drow['YARN_LIB_ID'];
			}
		}
		unset($sqlYarnConvArr);
		/*echo "10**";
		print_r($yarnBreakDownArr); die;*/
		
		$txt_option_remarks=str_replace($str_rep,' ',str_replace("'","",$txt_option_remarks));
		$txt_costing_remarks=str_replace($str_rep,' ',str_replace("'","",$txt_costing_remarks));
		$txt_meeting_remarks=str_replace($str_rep,' ',str_replace("'","",$txt_meeting_remarks));
		
		$field_array_mst="location_id*temp_id*lib_item_id*inquery_id*style_ref*buyer_id*season_id*season_year*brand_id*style_des*prod_dept*prod_code*department_id*delivery_date*exchange_rate*offer_qty*quoted_price*tgt_price*margin*stage_id*costing_date*buyer_remarks*article*option_remarks*costing_per*style_owner*updated_by*update_date";
		
		//$txt_temp_id=explode(",",str_replace("'","",$txt_temp_id));
		//asort($txt_temp_id);
		//$txt_temp_id="'".implode(",",$txt_temp_id)."'";
		
		$data_array_mst="".$cbo_location_id."*".$cbo_temp_id."*".$txt_temp_id."*".$txt_inquery_id."*'".strtoupper(str_replace("'","",$txt_styleRef))."'*".$cbo_buyer_id."*".$cbo_season_id."*".$cbo_season_year."*".$cbo_brand."*'".strtoupper(str_replace("'","",$txt_styleDesc))."'*".$cbo_product_department."*".$txt_product_code."*".$cbo_subDept_id."*".$txt_delivery_date."*".$txt_exchangeRate."*".$txt_offerQty."*".$txt_quotedPrice."*".$txt_tgtPrice."*".$txtmarign."*".$cbo_stage_id."*".$txt_costingDate."*'".strtoupper($txt_costing_remarks)."'*".$txt_article."*'".strtoupper($txt_option_remarks)."'*".$cbo_costing_per."*".$cbo_style_owner."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		
		//$template_id = return_id( $txt_temp_id, $template_library, "qc_template", "id,item_id");
		
		$temp_id=explode(",",str_replace("'","",$txt_temp_id));
		$cons_rate_id=return_next_id("id", "qc_cons_rate_dtls", 1);
		//$field_array_consrate="id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, value, fab_id, description, body_part_id, supp_ref, remarks, uom, dznkg, use_for, rate_data, inserted_by, insert_date, status_active, is_deleted";
		$field_array_consrate="id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, value, fab_id, description, body_part_id, supp_ref, remarks, uom, gsmweight, fabwidth, use_for, rate_data, inserted_by, insert_date, status_active, is_deleted, color_type_id, yarn_break_down";
		
		$fieldArrConsRate="particular_type_id*consumption*ex_percent*tot_cons*is_calculation*rate*value*fab_id*description*body_part_id*supp_ref*remarks*uom*updated_by*update_date*color_type_id*gsmweight*fabwidth*rate_data*use_for*yarn_break_down";
		
		$fieldArrYarnConv="body_part_id*yarn_lib_id*cons*tot_cons*value";
		...
		//type=4 acc;  particular_type_id=item_group
		//echo "10**";
		$add_comma_cons_rate=0; $data_array_cons_rate=""; $fabConsRateUpdateArr=array(); $yarnDataUpArr=array(); $isyarnconvfound=0; $yarnConvUpIdArr=array(); $deleteYarnIdArr=array();
		foreach($temp_id as $item_id)
		{
			$item_id=trim($item_id);
			...
			foreach($ex_itemConsRateData as $item_wise_consRateData)
			{
				if($item_wise_consRateData!="")
				{
					...
					$fabDesc=$usefor=$rateData="";
					$bodypartid=$fabid=$colortype=$gsmweight=$fabwidth=$conspack=$exPer=$totCons=$ratePop=$rate=$fabAmt=$fabupid=0;
					...
					{
						$id_arr[]=$fabupid;
						$fabConsRateUpdateArr[$fabupid] =explode("*",("'0'*'".$conspack."'*'".$exPer."'*'".$totCons."'*'".$ratePop."'*'".$rate."'*'".$fabAmt."'*'".$fabid."'*'".$fabDesc."'*'".$bodypartid."'*''*''*'".$fabuom."'*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'*'".$colortype."'*'".$gsmweight."'*'".$fabwidth."'*'0'*'".$rateData."'*'".$usefor."'*'".$yarnBreakDown."'"));
						
						if($isyarn==1)
						{
							if($yarnBreakDownArr[$fabupid]==$fabid)
							{
								foreach($yarn_rateArr[$fabupid] as $ydata=>$yrate)
								{
									$exyarncons=explode("_",$ydata);
									$yupid=$ycount=$ycomposition=$ytype=$ypercent=$consyarn=$yamt=0;
									
									$yupid=$exyarncons[0];
									$ycount=$exyarncons[1];
									$ycomposition=$exyarncons[2];
									$ytype=$exyarncons[3];
									$ypercent=$exyarncons[4];
									
									$consyarn=def_number_format((($totCons*$ypercent)/100),5,"");
									$yamt=($consyarn*$yrate)*1;
									
									$yarnConvUpIdArr[]=$yupid;
									
									$yarnDataUpArr[$yupid]=explode("*",("'".$bodypartid."'*'".$fabid."'*'".$consyarn."'*'".$consyarn."'*'".def_number_format($yamt,5,"")."'"));
									$isyarnconvfound=1;
								}
							}
							else
							{
								$deleteYarnIdArr[]=$fabupid;
							}
						}
						
						if($isconv==1)
						{
							foreach($conv_rateArr[$fabupid] as $convdata=>$convrate)
							{
								$exconvcons=explode("_",$convdata);
								$convupid=$convprocess=$convamt=0;
								
								$convupid=$exconvcons[0];
								$convprocess=$exconvcons[1];
								
								$convamt=($totCons*$convrate)*1;
								$yarnConvUpIdArr[]=$convupid;
								$yarnDataUpArr[$convupid]=explode("*",("'".$bodypartid."'*'".$fabid."'*'".$totCons."'*'".$totCons."'*'".def_number_format($convamt,5,"")."'"));
								$isyarnconvfound=1;
							}
						}
					}
					...
					{
						if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
						$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',1,0,'".$conspack."','".$exPer."','".$totCons."','".$ratePop."','".$rate."','".$fabAmt."','".$fabid."','".$fabDesc."','".$bodypartid."','','','".$fabuom."','".$gsmweight."','".$fabwidth."','".$usefor."','".$rateData."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,'".$colortype."','".$yarnBreakDown."')";
			...
			foreach($ex_specialData as $item_wise_specialData)
			{
				...
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',2,'".$speciaOperation_id."','".$spConsumtion."','".$spExper."','".$totSpCons."',0,'".$spRate."','".$spValue."','".$speciaTypeId."','','".$spbodypartid."','','".$spRemarks."',0,0,0,'','',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,0,'')";
			...
			foreach($ex_washData as $item_wise_washData)
			{
				...
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',3,'".$washTypeId."','".$wConsumtion."','".$wExper."','".$totwCons."',0,'".$wRate."','".$wValue."',0,'','".$wbodypartid."','','".$wRemarks."',0,0,0,'','',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,0,'')";
			...
			foreach($ex_itemAccData as $item_wise_itemAccData)
			{
				...
				//$field_array_consrate="id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, value, fab_id, description, body_part_id, supp_ref, remarks, uom, gsmweight, fabwidth, dznyds, dznkg, use_for, rate_data, inserted_by, insert_date, status_active, is_deleted, color_type_id, yarn_break_down";
				
				if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
				$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."',4,'".$accessories_id."','".$acConsumtion."','".$acExPer."','".$totAccCons."','".$acConsCal."','".$acRate."','".$acVal."',0,'".$accDescription."',0,'".$accBandRef."','','".$acconsUom."',0,0,'".$accUseFor."','".$accConsCalData."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,0,'')";
				
				$cons_rate_id=$cons_rate_id+1;
				$add_comma_cons_rate++;
			}
		}
		/*print_r($yarnDataUpArr);
		die;*/
		//echo "10**";
		...
		foreach($ex_item_wise_tot_data as $item_wise_tot)
		{
			$fab_td=$spOpe_td=$wash_td=$acc_td=$txtCpm=$txtSmv=$txtEff=$txtCmCost=$txtFriCost=$txtCourierCost=$txtLtstCost=$txtMissCost=$txtOtherCost=$txtCommCost=$fobT_td=$fobDzn_td=$txtoptexp=$txtdlcCost=$tot_item_id=$txtInspCost=0;
		...
		//die;
		$sqlYarndel=1;
		if(!empty($deleteYarnIdArr))
		{
			//echo "update qc_fab_yarn_conv set status_active=0, is_deleted=1, UPDATED_BY='".$_SESSION['logic_erp']['user_id']."', UPDATE_DATE='".$pc_date_time."' where CONS_RATE_DTLS_ID in (".implode(",",$deleteYarnIdArr).") and mst_id=$hid_qc_no and type=2 and status_active=1 and is_deleted=0";
			$sqlYarndel=execute_query("update qc_fab_yarn_conv set status_active=0, is_deleted=1, UPDATED_BY='".$_SESSION['logic_erp']['user_id']."', UPDATE_DATE='".$pc_date_time."' where CONS_RATE_DTLS_ID in (".implode(",",$deleteYarnIdArr).") and QC_NO=$hid_qc_no and type=2 and status_active=1 and is_deleted=0");
			if($sqlYarndel==1 && $flag==1) $flag=1; else $flag=0;
		}
		//die;
		$rIDyarnConv=1;
		if($isyarnconvfound==1 && $flag==1)
		{
			$rIDyarnConv=execute_query(bulk_update_sql_statement("qc_fab_yarn_conv", "id",$fieldArrYarnConv,$yarnDataUpArr,$yarnConvUpIdArr));
			if($rIDyarnConv==1 && $flag==1) $flag=1; else $flag=0;
		}
		...
		/*if($flag==1)
		{
			fnc_yarn_conv_cons_update(str_replace("'",'',$qcno));$rIDyarnConv
		}*/
		
		//echo "10**".$flag."=".$rID."=".$rIDfab."=".$sqlYarndel."=".$rIDyarnConv."=".$rID_consRate."=".$rID_tot_cost."=".$rID_item_tot."=".$rIDFabYarnConv."=".$rID_upmst."=".$rIDfabup; die;
...
if($action=="style_popup")
{
	...
	{
		show_list_view (document.getElementById('cbo_buyer_id').value+'**'+document.getElementById('cbo_season_id').value+'**'+document.getElementById('cbo_subDept_id').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('txt_cost_sheet_no').value+'**'+document.getElementById('hide_dataType').value+'**'+document.getElementById('cbo_approval_type').value+'**'+document.getElementById('cbo_string_search_type').value, 'style_ref_search_list_view', 'search_div', 'short_quotation_v6_controller', 'setFilterGrid(\'tbl_list_search\',-1)');
		set_all();
	}
	...
	function fnc_confirm_style(qcno,upid)
	{
		if(upid!="")
		{
			...
			var page_link='short_quotation_v6_controller.php?action=confirmStyle_popup';
			var title="Confirm Style Popup";
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link+'&data='+data, title, 'width=1150px,height=350px,center=1,resize=0,scrolling=0','../../')
			emailwindow.onclose=function()
			{
				fnc_listview();
			}
    ...
    <div align="center">
		...
		<fieldset>
            <table cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr class="general">
                        <td><? echo create_drop_down( "cbo_buyer_id", 130, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name",1, "-- All Buyer--",$buyerId,"load_drop_down( 'short_quotation_v6_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'short_quotation_v6_controller',this.value, 'load_drop_down_sub_dep', 'sub_td' );",0 ); ?></td>                 
...
if($action=="style_ref_search_list_view")
{
	...
	else
	{
		...
		$brand_cond="";
		if ($filterBrandId!="") $brand_cond=" and brand_id in ($filterBrandId)"; else $brand_cond="";
	...
	{
		$sql_style = "select min(a.id) as id, a.cost_sheet_no, max(a.costing_date) as costing_date, a.buyer_id, a.season_id, a.style_ref, a.department_id, a.temp_id, a.lib_item_id, a.stage_id, max(a.inserted_by) as inserted_by, max(a.updated_by) as updated_by, min(a.qc_no) as qc_no from qc_mst a where a.status_active=1 and a.is_deleted=0 and a.entry_form=634 $buyer_id_cond $style_ref_cond $cost_sheet_no_cond $season_id_cond $department_id_cond $approvaltype_cond $brand_cond group by a.cost_sheet_no, a.buyer_id, a.season_id, a.style_ref, a.department_id, a.temp_id, a.lib_item_id, a.stage_id order by id Desc";
...
if($action=="temp_style_list_view")
{
	...
	if($ex_data[1]==1)
	{
		...
		$sql_style= "select min(id) as id, cost_sheet_no, style_ref, min(qc_no) as qc_no, season_id, inserted_by from qc_mst where entry_form=634 and cost_sheet_no in ($ex_data[0]) and cost_sheet_no not in (select cost_sheet_no from qc_style_temp_data where cost_sheet_no in ($ex_data[0]) and inserted_by='$user_id' and status_active=1 and is_deleted=0 ) and revise_no=0 and option_id=0 and status_active=1 and is_deleted=0 group by cost_sheet_no, style_ref, season_id, inserted_by";
		//echo  $sql_style;
	...
	$sql_temp_style=sql_select("select a.id, a.cost_sheet_no, a.style_ref, a.season_id, a.inserted_by, a.updated_by, a.qc_no, a.season_year from qc_mst a, qc_style_temp_data b where a.id=b.mst_id and b.inserted_by='$user_id' and a.entry_form=634 and b.status_active=1 and b.is_deleted=0 order by a.style_ref ASC");
...
if($action=="populate_style_details_data")
{
	...
	$template_name_arr=array(); $itemNratioArr=array(); $itemWiseRatioArr=array();
	foreach($mst_temp_arr as $temp_id=>$tmp_data)
	{
		$template_data=array_filter(explode('__',$tmp_data));
		...
		foreach($template_data as $temp_val)
		{
			...
			$item_id1=$ex_tmp_val[0]; 
			$ratio1=$ex_tmp_val[1]; 
			$itemWiseRatioArr[$temp_id][$item_id1]=$ratio1;
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_drop_down('requires/short_quotation_v6_controller', '".trim($row[csf("company_id")])."', 'load_drop_down_location', 'location_td'); \n";
		...
		echo "load_drop_down('requires/short_quotation_v6_controller', '".trim($itemNratioArr[$row[csf("temp_id")]]['ratio'])."', 'load_drop_down_tempItem', 'item_td'); \n";
		...
		$itemWiseRatio=$itemWiseRatioArr[$row[csf("temp_id")]];
		...
		echo "load_drop_down('requires/short_quotation_v6_controller', '".$row[csf("buyer_id")]."', 'load_drop_down_season', 'season_td');\n"; 
		echo "load_drop_down('requires/short_quotation_v6_controller', '".$row[csf("buyer_id")]."', 'load_drop_down_brand', 'brand_td');\n";
		echo "load_drop_down('requires/short_quotation_v6_controller', '".$row[csf("buyer_id")]."', 'load_drop_down_sub_dep', 'sub_td');\n";
		...
		echo "disable_enable_fields('cbo_temp_id*cbo_costing_per',1);\n";
		if($ex_data[4]!="from_option")
		{
			//echo "load_drop_down('requires/short_quotation_v6_controller', '".$row[csf("cost_sheet_no")].'__'.$row[csf("option_id")].'__'.$row[csf("revise_no")]."', 'load_drop_down_revise_no', 'revise_td');\n"; //.'__'.$row[csf("revise_no")]
			//echo "load_drop_down('requires/short_quotation_v6_controller', '".$row[csf("cost_sheet_no")].'__'.$row[csf("option_id")]."', 'load_drop_down_option_id', 'option_td');\n"; 
		}
	...
	$sql_cons_rate="select id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, value, fab_id, description, body_part_id, supp_ref, remarks, uom, color_type_id, gsmweight, fabwidth, dznkg, rate_data, use_for, yarn_break_down from qc_cons_rate_dtls where mst_id=$cond_data and status_active=1 and is_deleted=0 order by id asc";
	...
	foreach ($sql_result_cons_rate as $rowConsRate)
	{
		...
		{
			$cons_rate_fab_arr[$rowConsRate[csf("item_id")]].=$body_part[$rowConsRate[csf("body_part_id")]].'~!~'.$rowConsRate[csf("body_part_id")].'~!~'.$rowConsRate[csf("description")].'~!~'.$rowConsRate[csf("fab_id")].'~!~'.$rowConsRate[csf("color_type_id")].'~!~'.$rowConsRate[csf("gsmweight")].'~!~'.$rowConsRate[csf("fabwidth")].'~!~'.$rowConsRate[csf("use_for")].'~!~'.$rowConsRate[csf("consumption")].'~!~'.$rowConsRate[csf("ex_percent")].'~!~'.$rowConsRate[csf("tot_cons")].'~!~'.$rowConsRate[csf("is_calculation")].'~!~'.$rowConsRate[csf("rate_data")].'~!~'.$rowConsRate[csf("rate")].'~!~'.$rowConsRate[csf("value")].'~!~'.$rowConsRate[csf("id")].'~!~'.$rowConsRate[csf("uom")].'~!~'.$rowConsRate[csf("yarn_break_down")].'#^';
		}
		...
		{
			$cons_rate_ac_arr[$rowConsRate[csf("item_id")]].=$itemGroupArr[$rowConsRate[csf("particular_type_id")]].'~!~'.$rowConsRate[csf("particular_type_id")].'~!~'.$rowConsRate[csf("description")].'~!~'.$rowConsRate[csf("supp_ref")].'~!~'.$rowConsRate[csf("consumption")].'~!~'.$rowConsRate[csf("ex_percent")].'~!~'.$rowConsRate[csf("tot_cons")].'~!~'.$rowConsRate[csf("rate")].'~!~'.$rowConsRate[csf("value")].'~!~'.$rowConsRate[csf("use_for")].'~!~'.$rowConsRate[csf("uom")].'~!~'.$rowConsRate[csf("dznkg")].'~!~'.$rowConsRate[csf("is_calculation")].'~!~'.$itemGroupCalParaArr[$rowConsRate[csf("particular_type_id")]].'~!~'.$rowConsRate[csf("rate_data")].'#^';
		}
	...
	$sql_summ="select buyer_agent_id, location_id, no_of_pack, is_confirm, is_cm_calculative, mis_lumsum_cost, commision_per, tot_fab_cost, tot_sp_operation_cost, tot_wash_cost, tot_accessories_cost, tot_cm_cost, tot_fright_cost, tot_courier_cost, tot_lab_test_cost, tot_miscellaneous_cost, tot_other_cost, tot_commission_cost, tot_cost, tot_fob_cost, tot_rmg_ratio, commercial_per, commercial_cost, inspection_cost, operating_exp, lc_per, lc_cost from qc_tot_cost_summary where mst_id='$cond_data' and status_active=1 and is_deleted=0";
	...
	foreach($sql_result_summ as $rowSumm)
	{
		...
		echo "$('#totFOBCost_td').text('".number_format(($rowSumm[csf("tot_cost")]),4)."');\n";
		
	}
	unset($sql_result_summ);
	$sql_item_summ="select item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, courier_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs, rmg_ratio, commercial_cost, inspection_cost, operating_exp, lc_cost from qc_item_cost_summary where mst_id='$cond_data' and status_active=1 and is_deleted=0";
	...
	foreach($sql_result_item_summ as $rowItemSumm)
	{
		...
		echo "$('#fobPcsT_td".$rowItemSumm[csf("item_id")]."').text('".number_format(($rowItemSumm[csf("fob_pcs")]/$itemWiseRatio[$rowItemSumm[csf("item_id")]]),4)."');\n";
		echo "fnc_change_data();\n";
	}
...
if($action=="copy_cost_sheet")
{
	...
	{	
		...
		if (is_duplicate_field( "cost_sheet_no", "qc_mst", "style_ref='$dup_styleRef' and season_id='$dup_season_id' and buyer_id='$dup_buyer_id' and entry_form=634 and status_active=1 and is_deleted=0")==1)
		{
			echo "11**".$msg; 
			die;
		}
		...
		$cbo_costing_per=$sql_mst[0][csf('costing_per')];
		$cbo_style_owner=$sql_mst[0][csf('style_owner')];
		...
		$field_array_mst="id, company_id, location_id, cost_sheet_id, cost_sheet_no, temp_id, lib_item_id, inquery_id, style_ref, buyer_id, uom, cons_basis, season_id, season_year, brand_id, style_des, prod_dept, prod_code, margin, department_id, delivery_date, exchange_rate, offer_qty, quoted_price, tgt_price, stage_id, costing_date, pre_cost_sheet_id, revise_no, option_id, buyer_remarks, option_remarks, article, qc_no, entry_form, fab_process_loss_method, acc_process_loss_method, style_owner, costing_per, inserted_by, insert_date, status_active, is_deleted";
		...
		$cons_rate_id=return_next_id("id", "qc_cons_rate_dtls", 1);
		$sql_consrate=sql_select("select id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, value, fab_id, description, use_for, fc_per, body_part_id, supp_ref, remarks, uom, color_type_id, gsmweight, fabwidth, dznyds, dznkg, rate_data from qc_cons_rate_dtls where mst_id='$hid_qc_no' order by id ASC");
		...
		$field_array_consrate="id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, is_calculation, rate, value, fab_id, description, use_for, fc_per, body_part_id, supp_ref, remarks, uom, inserted_by, insert_date, status_active, is_deleted, color_type_id, gsmweight, fabwidth, dznyds, dznkg, rate_data";
		$add_comma_cons_rate=0; $data_array_cons_rate=""; $consRateIdMapArr=array();
		foreach( $sql_consrate as $rowConsrate )
		{
			...
			if ($add_comma_cons_rate!=0) $data_array_cons_rate .=",";
			$data_array_cons_rate .="(".$cons_rate_id.",".$qcno.",'".$item_id."','".$type."','".$head_id."','".$cons."','".$ex_percent."','".$tot_cons."','".$ratePop."','".$rate."','".$tot_val."','".$fab_id."','".$description."','".$use_for."','".$fc_per."','".$body_part_id."','".$supp_ref."','".$consRemarks."','".$consuom."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,'".$fabcolortype."','".$gsmweight."','".$fabwidth."','".$dznyds."','".$dznkg."','".$rateData."')";
		...
		$txt_lc_per=$sql_tot_cost[0][csf('lc_per')];
		$txt_lc_cost=$sql_tot_cost[0][csf('lc_cost')];
		$totInspCst_td=$sql_tot_cost[0][csf('inspection_cost')];
		
		$data_array_tot_cost="(".$tc_sum_id.",".$qcno.",'".$cbo_buyer_agent."','".$cbo_agent_location."','".$txt_noOfPack."',0,'".$cmPop."','".$txt_lumSum_cost."','".$txt_commPer."','".$totFab_td."','".$totSpc_td."','".$totWash_td."','".$totAcc_td."','".$totCm_td."','".$totFriCst_td."','".$totCourierCst_td."','".$totLbTstCst_td."','".$totMissCst_td."','".$totOtherCst_td."','".$totCommCst_td."','".$totCost_td."','".$totalFob_td."','".$totalRmg_td."','".$txt_commlPer."','".$totCommlCst_td."','".$txt_optexp."','".$txt_lc_per."','".$txt_lc_cost."','".$totInspCst_td."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		//echo $data_array_tot_cost; die;
		$sql_item_tot=sql_select("select id, mst_id, item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, courier_cost, lab_test_cost, miscellaneous_cost, other_cost, commission_cost, fob_pcs, rmg_ratio, commercial_cost, inspection_cost, operating_exp, lc_cost from qc_item_cost_summary where mst_id='$hid_qc_no' order by id ASC");
		...
		{
			$fab_td=$spOpe_td=$wash_td=$acc_td=$txtCpm=$txtSmv=$txtEff=$txtCmCost=$txtFriCost=$txtCourierCost=$txtLtstCost=$txtMissCost=$txtOtherCost=$txtCommlCost=$txtDlcCost=$txtCommCost=$fobT_td=$tot_item_id=$rmg_ratio=$txtoptexp=$txtInspCost=0;
			...
			if ($add_comma_item_tot!=0) $data_array_item_tot .=",";
			$data_array_item_tot .="(".$item_cost_id.",".$qcno.",'".$tot_item_id."','".$fab_td."','".$spOpe_td."','".$wash_td."','".$acc_td."','".$txtCpm."','".$txtSmv."','".$txtEff."','".$txtCmCost."','".$txtFriCost."','".$txtCourierCost."','".$txtLtstCost."','".$txtMissCost."','".$txtOtherCost."','".$txtCommCost."','".$fobT_td."','".$rmg_ratio."','".$txtCommlCost."','".$txtoptexp."','".$txtDlcCost."','".$txtInspCost."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		...
		//echo "10**".$rID.'='.$rID_consRate.'='.$rIDFab.'='.$rID_tot_cost.'='.$rID_item_tot.'='.$flag; die;
...
if($action=="confirmStyle_popup")
{
	...
	$sql_cons=sql_select("select item_id, uom, tot_cons from qc_cons_rate_dtls where mst_id='$qc_no' and type=1 and status_active=1 and is_deleted=0");//type ='1' and
	...
	{
		$item_wise_cons_arr[$cRow[csf("item_id")]][$cRow[csf("uom")]]['qty']+=$cRow[csf("tot_cons")];
	}
	//$sql_result_summ=sql_select($sql_summ);
    ...
    <script>
		...
		function fnc_confirm_entry( operation )
		{
			if(operation==3)
			{
				...
				var qc_fab_kg=0; var qc_fab_mtr=0; var qc_fab_yds=0; var qc_fab_amt=0; var qc_sp_amt=0; var qc_wash_amt=0; var qc_acc_amt=0; var qc_fri_amt=0; var qc_courier_amt=0; var qc_lab_amt=0; var qc_insp_amt=0; var qc_optexp_amt=0; var qc_other_amt=0; var qc_comml_amt=0; var qc_dlc_amt=0; var qc_comm_amt=0; var qc_fob_amt=0; var qc_cm_amt=0; var qc_rmg_amt=0;
				...
				if(job_no!="")
				{
					...
					/*else if(qc_optexp_amt<optexp_amount)
					{
						alert("BOM Opt. Exp. Amount is Greater Than QC.");
						return;
					}
					...
					else if(qc_dlc_amt<dlc_amount)
					{
						alert("BOM D. LC Amount is Greater Than QC.");
						return;
					}
					else if(qc_insp_amt<insp_amount)
					{
						alert("BOM Insp. Cost is Greater Than QC.");
						return;
					}
			...
			var data_all=''; var k=0;
			for(i=1; i<=split_tmep_id.length; i++)
			{
				var itm_id=trim(split_tmep_id[k]);
				//data_dtls+=get_submitted_data_string('txtitemid_'+itm_id+'*txtdtlsupid_'+itm_id+'*txtFabConkg_'+itm_id+'*txtFabConmtr_'+itm_id+'*txtFabConyds_'+itm_id+'*txtFabCst_'+itm_id+'*txtSpOpa_'+itm_id+'*txtWash_'+itm_id+'*txtAcc_'+itm_id+'*txtFrightCst_'+itm_id+'*txtCourierCst_'+itm_id+'*txtLabCst_'+itm_id+'*txtMiscCst_'+itm_id+'*txtOtherCst_'+itm_id+'*txtCommlCst_'+itm_id+'*txtDlcCst_'+itm_id+'*txtCommCst_'+itm_id+'*txtFobDzn_'+itm_id+'*txtCpm_'+itm_id+'*txtSmv_'+itm_id+'*txtCmCst_'+itm_id+'*txtPack_'+itm_id,"../../../",2);
				
				data_all+="&txtitemid_" + itm_id + "='" + $('#txtitemid_'+itm_id).val()+"'"+"&txtdtlsupid_" + itm_id + "='" + $('#txtdtlsupid_'+itm_id).val()+"'"+"&txtFabConkg_" + itm_id + "='" + $('#txtFabConkg_'+itm_id).val()+"'"+"&txtFabConmtr_" + itm_id + "='" + $('#txtFabConmtr_'+itm_id).val()+"'"+"&txtFabConyds_" + itm_id + "='" + $('#txtFabConyds_'+itm_id).val()+"'"+"&txtFabCst_" + itm_id + "='" + $('#txtFabCst_'+itm_id).val()+"'"+"&txtSpOpa_" + itm_id + "='" + $('#txtSpOpa_'+itm_id).val()+"'"+"&txtWash_" + itm_id + "='" + $('#txtWash_'+itm_id).val()+"'"+"&txtAcc_" + itm_id + "='" + $('#txtAcc_'+itm_id).val()+"'"+"&txtFrightCst_" + itm_id + "='" + $('#txtFrightCst_'+itm_id).val()+"'"+"&txtCourierCst_" + itm_id + "='" + $('#txtCourierCst_'+itm_id).val()+"'"+"&txtLabCst_" + itm_id + "='" + $('#txtLabCst_'+itm_id).val()+"'"+"&txtInspCst_" + itm_id + "='" + $('#txtInspCst_'+itm_id).val()+"'"+"&txtOptExpCst_" + itm_id + "='" + $('#txtOptExpCst_'+itm_id).val()+"'"+"&txtOtherCst_" + itm_id + "='" + $('#txtOtherCst_'+itm_id).val()+"'"+"&txtCommlCst_" + itm_id + "='" + $('#txtCommlCst_'+itm_id).val()+"'"+"&txtCommCst_" + itm_id + "='" + $('#txtCommCst_'+itm_id).val()+"'"+"&txtDlcCst_" + itm_id + "='" + $('#txtDlcCst_'+itm_id).val()+"'"+"&txtFobDzn_" + itm_id + "='" + $('#txtFobDzn_'+itm_id).val()+"'"+"&txtCpm_" + itm_id + "='" + $('#txtCpm_'+itm_id).val()+"'"+"&txtSmv_" + itm_id + "='" + $('#txtSmv_'+itm_id).val()+"'"+"&txtCmCst_" + itm_id + "='" + $('#txtCmCst_'+itm_id).val()+"'"+"&txtPack_" + itm_id + "='" + $('#txtPack_'+itm_id).val()+"'";
				k++;
			}
			var data="action=save_update_delete_confirm_style&operation="+operation+get_submitted_data_string('txt_costSheet_id*txtConfirm_id*txtItem_id*txt_confirm_style*txt_order_qty*txt_confirm_fob*txt_ship_date*txt_job_id*cbo_approved_status*cbo_ready_approve',"../../../",2)+data_all;
		...
		function fnc_stage_entry_reponse()
		{
			if(http.readyState == 4) 
			{
				...
				if(trim(reponse[0])==0 || trim(reponse[0])==1 || trim(reponse[0])==2 || trim(reponse[0])==3)
				{
					...
					get_php_form_data($('#txt_costSheet_id').val()+'__'+$('#txtItem_id').val(),'populate_confirm_style_form_data','short_quotation_v6_controller');
					//show_list_view('','stage_data_list','stage_data_div','short_quotation_v6_controller','');
					js_set_value();
				}
			}
		...
		function fnc_openJobPopup()
		{
			...
			page_link='short_quotation_v6_controller.php?action=style_tag_popup&data='+data;
		...
		function fnc_bom_data_load()
		{
			...
			if(job_no!="")
			{
				var str_data=return_global_ajax_value( job_no, 'budgete_cost_validate', '', 'short_quotation_v6_controller');
				
				var spdata=str_data.split("##");
				var fab_cons_kg=spdata[0]; var fab_cons_mtr=spdata[1]; var fab_cons_yds=spdata[2]; var fab_amount=spdata[3]; var sp_oparation_amount=spdata[4]; var acc_amount=spdata[5]; var fright_amount=spdata[6]; var lab_amount=spdata[7]; var insp_amount=spdata[8]; var optexp_amount=spdata[9]; var other_amount=spdata[10]; var comm_amount=spdata[11]; var fob_amount=spdata[12]; var cm_amount=spdata[13]; var rmg_ratio=spdata[14]; var wash_amount=spdata[15];
				
				if(qcCons_from==1)
				{
					var bomTotCost=(number_format(fab_amount,4)*1)+(number_format(sp_oparation_amount,4)*1)+(number_format(wash_amount,4)*1)+(number_format(acc_amount,4)*1)+(number_format(fright_amount,4)*1)+(number_format(lab_amount,4)*1)+(number_format(insp_amount,4)*1)+(number_format(optexp_amount,4)*1)+(number_format(other_amount,4)*1)+(number_format(comm_amount,4)*1);
					
					var bomCost='Fab='+fab_amount+'+ SP='+sp_oparation_amount+'+ Wash='+wash_amount+'+ ACC='+acc_amount+'+ FR='+fright_amount+'+ Lab='+lab_amount+'+ Insp.='+insp_amount+'+ Opt.Exp.='+optexp_amount+'+ Others='+other_amount+'+ Comm='+comm_amount+'= Total='+bomTotCost;
				...
				$("#txtOptExpCst_bom").val(optexp_amount);
		...
		function fnc_total_calculate()
		{
			...
			var split_tmep_id=temp_id.split(',');
			var ab=0;
			var qc_fab_kg=0; var qc_fab_mtr=0; var qc_fab_yds=0; var qc_fab_amt=0; var qc_sp_amt=0; var qc_wash_amt=0; var qc_acc_amt=0; var qc_fri_amt=0; var qc_courier_amt=0; var qc_lab_amt=0; var qc_insp_amt=0; var qc_optexp_amt=0; var qc_other_amt=0; var qc_comml_amt=0; var qc_dlc_amt=0; var qc_comm_amt=0; var qc_fob_amt=0; var qc_cpm_amt=0; var qc_smv_amt=0; var qc_cm_amt=0; var qc_rmg_amt=0;
			for(j=1; j<=split_tmep_id.length; j++)
			{
				...
				item_tot_amount=($("#txtFabCst_"+itm_id).val()*1)+($("#txtSpOpa_"+itm_id).val()*1)+($("#txtWash_"+itm_id).val()*1)+($("#txtAcc_"+itm_id).val()*1)+($("#txtFrightCst_"+itm_id).val()*1)+($("#txtCourierCst_"+itm_id).val()*1)+($("#txtLabCst_"+itm_id).val()*1)+($("#txtInspCst_"+itm_id).val()*1)+($("#txtOptExpCst_"+itm_id).val()*1)+($("#txtOtherCst_"+itm_id).val()*1)+($("#txtCommlCst_"+itm_id).val()*1)+($("#txtDlcCst_"+itm_id).val()*1)+($("#txtCommCst_"+itm_id).val()*1);
				
				var itemCost='Fab='+$("#txtFabCst_"+itm_id).val()+'+ SP='+$("#txtSpOpa_"+itm_id).val()+'+ Wash='+$("#txtWash_"+itm_id).val()+'+ ACC='+$("#txtAcc_"+itm_id).val()+'+ FR='+$("#txtFrightCst_"+itm_id).val()+'+ Courier ='+$("#txtCourierCst_"+itm_id).val()+'+ Lab='+$("#txtLabCst_"+itm_id).val()+'+ Insp.='+$("#txtInspCst_"+itm_id).val()+'+ OptExp='+$("#txtOptExpCst_"+itm_id).val()+'+ Others='+$("#txtOtherCst_"+itm_id).val()+'+ Comml='+$("#txtCommlCst_"+itm_id).val()+'+ D. LC='+$("#txtDlcCst_"+itm_id).val()+'+ Comm='+$("#txtCommCst_"+itm_id).val()+'= Total='+item_tot_amount;
			...
			$("#txtOptExpCst_qc").val( number_format(qc_optexp_amt,4,'.','') );
			$("#txtOtherCst_qc").val( number_format(qc_other_amt,4,'.','') );
			$("#txtCommlCst_qc").val( number_format(qc_comml_amt,4,'.','') );
			$("#txtDlcCst_qc").val( number_format(qc_dlc_amt,4,'.','') );
			...
			var total_amount=qc_fab_amt+qc_sp_amt+qc_wash_amt+qc_acc_amt+qc_fri_amt+qc_courier_amt+qc_lab_amt+qc_insp_amt+qc_optexp_amt+qc_other_amt+qc_comml_amt+qc_dlc_amt+qc_comm_amt;
		...
		function openmypage_unapprove_request()
		{
			...
			var page_link = 'short_quotation_v6_controller.php?data='+data+'&action=unapp_request_popup';
		...
		function fncqclimit(str)
		{
			...
			var tdoptexpamt=$('#tdoptexpamt'+item_id).text()*1;
			var tdothamt=$('#tdothamt'+item_id).text()*1;
			var tdcommlamt=$('#tdcommlamt'+item_id).text()*1;
			var tddlcamt=$('#tddlcamt'+item_id).text()*1;
			...
			if(qcCons_from==2)
			{
				...
				if( validatetype==17 && tdwashamt < ($("#txtWash_"+item_id).val()*1) )
				{
				...
				if( validatetype==9 && tdoptexpamt < ($("#txtOptExpCst_"+item_id).val()*1) )
				{
					alert("QC BOM Limit (Opt. Exp.) is Greater Than QC!!!");
					$("#txtOptExpCst_"+item_id).val(tdoptexpamt);
					//return;
				}
				...
				{
					alert("QC BOM Limit (Courier) is Greater Than QC!!!");
					$("#txtCourierCst_"+item_id).val(tdcourieramt);
					//return;
				}
				if( validatetype==19 && tdcommlamt < ($("#txtCommlCst_"+item_id).val()*1) )
				{
					alert("QC BOM Limit (Commercial) is Greater Than QC!!!");
					$("#txtCommlCst_"+item_id).val(tdcommlamt);
					//return;
				}
				if( validatetype==20 && tddlcamt < ($("#txtDlcCst_"+item_id).val()*1) )
				{
					alert("QC BOM Limit (D. LC) is Greater Than QC!!!");
					$("#txtDlcCst_"+item_id).val(tddlcamt);
					//return;
				}
				if( validatetype==21 && tdinspamt < ($("#txtInspCst_"+item_id).val()*1) )
				{
					alert("QC BOM Limit (Insp. Cost) is Greater Than QC!!!");
    ...
    <div style="display:none"><? echo load_freeze_divs ("../../../",$permission);  ?></div>       
        <form name="confirmStyle_1" id="confirmStyle_1" autocomplete="off">
        	<table width="850">
                <tr>
                    ...
                    <td width="120"><? echo create_drop_down( "cbo_buyer_id", 120, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name", 1, "-Select Buyer-", $sql_data[0][csf('buyer_id')], "load_drop_down( 'short_quotation_v6_controller', this.value, 'load_drop_down_season_conf', 'season_conf_td'); load_drop_down( 'short_quotation_v6_controller',this.value, 'load_drop_down_sub_depConf', 'subConf_td' );",1 ); ?></td>
            ...
            <table width="1120" cellspacing="0" border="1" rules="all" class="rpt_table">
        	<thead>
                ...
                <th width="45">Frieght</th>
                <th width="45">Courier</th>
                <th width="45">Lab - Test</th>
                <th width="45">Insp. Cost</th>
                <th width="45">Opt. Exp</th>
                <th width="45">Other Cost</th>
                <th width="45">Comml. Cost</th>
                <th width="45">D. LC</th>
			...
			<?
            $sql_item_summ="select item_id, fabric_cost, sp_operation_cost, wash_cost, accessories_cost, cpm, smv, efficiency, cm_cost, frieght_cost, courier_cost, lab_test_cost, inspection_cost, operating_exp, other_cost, commercial_cost, commission_cost, fob_pcs, lc_cost from qc_item_cost_summary where mst_id='$qc_no' and status_active=1 and is_deleted=0";
            ...
            foreach($sql_result_item_summ as $rowItemSumm)
            {
                ...
                <tr id="trVal_<?=$z; ?>" bgcolor="<?=$bgcolor; ?>">
                    <td style="word-break:break-all"><?=$garments_item[$item_id]; ?></td>
                    <td align="right" id="tdfabkg<?=$item_id; ?>"><? echo number_format($item_wise_cons_arr[$item_id][12]['qty'],4); ?></td>
                    <td align="right" id="tdfabmtr<?=$item_id; ?>"><? echo number_format($item_wise_cons_arr[$item_id][23]['qty'],4); ?></td>
                    <td align="right" id="tdfabyds<?=$item_id; ?>"><? echo number_format($item_wise_cons_arr[$item_id][27]['qty'],4); ?></td>
                    ...
                    <td align="right" id="tdoptexpamt<?=$item_id; ?>"><? echo number_format($rowItemSumm[csf("operating_exp")],4); ?></td>
                    ...
                    <td align="right" id="tddlcamt<?=$item_id; ?>"><? echo number_format($rowItemSumm[csf("lc_cost")],4); ?></td>
                ...
                <tr id="tr_<? echo $z; ?>" bgcolor="<? echo $bgcolorN; ?>">
                    <td>QC BOM Limit<input style="width:20px;" type="hidden" name="txtitemid_<?=$item_id; ?>" id="txtitemid_<?=$item_id; ?>" value="<?=$item_id; ?>" /><input style="width:20px;" type="hidden" name="txtdtlsupid_<?=$item_id; ?>" id="txtdtlsupid_<?=$item_id; ?>" value="" /></td>
                    <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtFabConkg_<?=$item_id; ?>" id="txtFabConkg_<?=$item_id; ?>" value="<?=number_format($item_wise_cons_arr[$item_id][12]['qty'],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_1'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtFabConmtr_<?=$item_id; ?>" id="txtFabConmtr_<?=$item_id; ?>" value="<?=number_format($item_wise_cons_arr[$item_id][23]['qty'],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_2'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtFabConyds_<?=$item_id; ?>" id="txtFabConyds_<?=$item_id; ?>" value="<?=number_format($item_wise_cons_arr[$item_id][27]['qty'],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_3'; ?>');" <?=$disable; ?>/></td>
                    <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtFabCst_<?=$item_id; ?>" id="txtFabCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("fabric_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_4'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtSpOpa_<?=$item_id; ?>" id="txtSpOpa_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("sp_operation_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_5'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtWash_<?=$item_id; ?>" id="txtWash_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("wash_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_17'; ?>');" <?=$disable; ?> /></td>
                     
                    <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtAcc_<?=$item_id; ?>" id="txtAcc_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("accessories_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_6'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtFrightCst_<?=$item_id; ?>" id="txtFrightCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("frieght_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_7'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtCourierCst_<?=$item_id; ?>" id="txtCourierCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("courier_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_18'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtLabCst_<?=$item_id; ?>" id="txtLabCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("lab_test_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_8'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtInspCst_<?=$item_id; ?>" id="txtInspCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("inspection_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_21'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtOptExpCst_<?=$item_id; ?>" id="txtOptExpCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("operating_exp")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_9'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtOtherCst_<?=$item_id; ?>" id="txtOtherCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("other_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_10'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtCommlCst_<?=$item_id; ?>" id="txtCommlCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("commercial_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_19'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtDlcCst_<?=$item_id; ?>" id="txtDlcCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("lc_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_20'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtCommCst_<?=$item_id; ?>" id="txtCommCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("commission_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_11'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtFobDzn_<?=$item_id; ?>" id="txtFobDzn_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("fob_pcs")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_12'; ?>');" <?=$disable; ?> /></td>
                    
                    <td title="((CPM*100)/Efficiency)"><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtCpm_<?=$item_id; ?>" id="txtCpm_<?=$item_id; ?>" value="<? if($cppm==0) echo "0"; else echo number_format($cppm,4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_13'; ?>');" disabled <? //echo $disable; ?> /></td>
                    <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtSmv_<?=$item_id; ?>" id="txtSmv_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("smv")],4); ?>" onChange="fnc_total_calculate();" onBlur="fnc_cppm_cal(<?=$item_id; ?>); fncqclimit('<?=$item_id.'_14'; ?>');" <?=$disable; ?> /></td>
                    
                    <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtCmCst_<?=$item_id; ?>" id="txtCmCst_<?=$item_id; ?>" value="<?=number_format($rowItemSumm[csf("cm_cost")],4); ?>" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_15'; ?>');" <?=$disable; ?> /></td>
                    <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtPack_<?=$item_id; ?>" id="txtPack_<?=$item_id; ?>" value="" onChange="fnc_total_calculate();" onBlur="fncqclimit('<?=$item_id.'_16'; ?>');" <?=$disable; ?> />&nbsp;</td>
                </tr>
                <?
				$z++;
            }
			$sql="select fab_cons_kg, fab_cons_yds, fab_amount, sp_oparation_amount, wash_amount, acc_amount, fright_amount, lab_amount, inspection_cost, operating_exp, other_amount, comm_amount, fob_amount, cm_amount, rmg_ratio from qc_confirm_dtls where cost_sheet_id='$cost_sheet_id' and status_active=1 and is_deleted=0";
        ...
        <table width="1120" cellspacing="0" border="1" rules="all" class="rpt_table">
        	<tr id="tr_qc" bgcolor="#CCFFCC">
                ...
                <td width="45"><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtFabConmtr_qc" id="txtFabConmtr_qc" value="" disabled /></td>
                <td width="45"><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtFabConyds_qc" id="txtFabConyds_qc" value="" disabled /></td>
                ...
                <td width="45"><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtFrightCst_qc" id="txtFrightCst_qc" value="" disabled /></td>
                <td width="45"><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtCourierCst_qc" id="txtCourierCst_qc" value="" disabled /></td>
                <td width="45"><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtLabCst_qc" id="txtLabCst_qc" value="" disabled /></td>
                <td width="45"><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtInspCst_qc" id="txtInspCst_qc" value="" disabled /></td>
                <td width="45"><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtOptExpCst_qc" id="txtOptExpCst_qc" value="" disabled /></td>
                <td width="45"><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtOtherCst_qc" id="txtOtherCst_qc" value="" disabled /></td>
                <td width="45"><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtCommlCst_qc" id="txtCommlCst_qc" value="" disabled /></td>
                <td width="45"><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtDlcCst_qc" id="txtDlcCst_qc" value="" disabled /></td>
                ...
                <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtPack_qc" id="txtPack_qc" value="" disabled />&nbsp;</td>
            </tr>
        	<tr id="tr_bom" bgcolor="#CCCCCC">
                <td><font color="#FF0000">Current BOM</font></td>
                <td><input style="width:35px;" type="text" class="text_boxes_numeric" name="txtFabConkg_bom" id="txtFabConkg_bom" value="" readonly /></td>
                <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtFabConmtr_bom" id="txtFabConmtr_bom" value="" readonly /></td>
                <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtFabConyds_bom" id="txtFabConyds_bom" value="" readonly /></td>
                ...
                <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtFrightCst_bom" id="txtFrightCst_bom" value="" readonly /></td>
                <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtCourierCst_bom" id="txtCourierCst_bom" value="" readonly /></td>
                <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtLabCst_bom" id="txtLabCst_bom" value="" readonly /></td>
                <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtInspCst_bom" id="txtInspCst_bom" value="" readonly /></td>
                <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtOptExpCst_bom" id="txtOptExpCst_bom" value="" readonly /></td>
                <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtOtherCst_bom" id="txtOtherCst_bom" value="" readonly /></td>
                <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtCommlCst_bom" id="txtCommlCst_bom" value="" readonly /></td>
                <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtDlcCst_bom" id="txtDlcCst_bom" value="" readonly /></td>
                ...
                <td><input style="width:30px;" type="text" class="text_boxes_numeric" name="txtPack_bom" id="txtPack_bom" value="" readonly />&nbsp;</td>
            </tr>
        </table>
            </div>
        </form>
    ...
    <script>get_php_form_data($('#txt_costSheet_id').val()+'__'+$('#txtItem_id').val(),'populate_confirm_style_form_data','short_quotation_v6_controller'); fnc_bom_data_load(); fnc_total_calculate(); fnc_select();</script>          
    <script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
    </html>
    <?
	exit();	
...
if($action=="save_update_delete_confirm_style")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$dtls_field_arr="id, mst_id, cost_sheet_id, item_id, fab_cons_kg, fab_cons_mtr, fab_cons_yds, fab_amount, sp_oparation_amount, wash_amount, acc_amount, fright_amount, courier_amount, lab_amount, inspection_cost, operating_exp, other_amount, commercial_cost, lc_cost, comm_amount, fob_amount, cppm_amount, smv_amount, cm_amount, rmg_ratio, inserted_by, insert_date";
		//echo "10**";
		...
		for($i=1; $i<=count($itm_id); $i++)
		{
			...
			if ($add_comma!=0) $dtls_data_arr .=",";
			$dtls_data_arr .="(".$confirm_dtls_id.",".$confirm_mst_id.",".$txt_costSheet_id.",".$$txtitemid.",".$$txtFabConkg.",".$$txtFabConmtr.",".$$txtFabConyds.",".$$txtFabCst.",".$$txtSpOpa.",".$$txtWash.",".$$txtAcc.",".$$txtFrightCst.",".$$txtCourierCst.",".$$txtLabCst.",".$$txtInspCst.",".$$txtOptExpCst.", ".$$txtOtherCst.",".$$txtCommlCst.",".$$txtDlcCst.",".$$txtCommCst.", ".trim($$txtFobDzn).",".$$txtCpm.",".$$txtSmv.",".$$txtCmCst.",".$$txtPack.",".$user_id.",'".$pc_date_time."')";
		...
		/*echo "10**$confirm_mst_id INSERT INTO qc_confirm_dtls (".$dtls_field_arr.") VALUES ".$dtls_data_arr; 
		die;*//**/
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		$dtls_field_arr="fab_cons_kg*fab_cons_mtr*fab_cons_yds*fab_amount*sp_oparation_amount*wash_amount*acc_amount*fright_amount*courier_amount*lab_amount*inspection_cost*operating_exp*other_amount*commercial_cost*lc_cost*comm_amount*fob_amount*cppm_amount*smv_amount*cm_amount*rmg_ratio*updated_by*update_date";
		//echo "10**";
		$itm_id=explode(",",str_replace("'","",$txtItem_id));
		//print_r($itm_id);
		//echo count($itm_id);
		...
		for($i=1; $i<=count($itm_id); $i++)
		{
			...
			$dtls_data_arr[str_replace("'",'',$$txtdtlsupid)] =explode("*",("".$$txtFabConkg."*".$$txtFabConmtr."*".$$txtFabConyds."*".$$txtFabCst."*".$$txtSpOpa."*".$$txtWash."*".$$txtAcc."*".$$txtFrightCst."*".$$txtCourierCst."*".$$txtLabCst."*".$$txtInspCst."*".$$txtOptExpCst."*".$$txtOtherCst."*".$$txtCommlCst."*".$$txtDlcCst."*".$$txtCommCst."*".$$txtFobDzn."*".$$txtCpm."*".$$txtSmv."*".$$txtCmCst."*".$$txtPack."*'".$user_id."'*'".$pc_date_time."'"));
			$k++;
		}
	...
	else if ($operation==2) // Delete Here----------------------------------------------------------
	{
		...
		$rID=execute_query( "update qc_confirm_mst set updated_by=".$_SESSION['logic_erp']['user_id'].", update_date='".$pc_date_time."', status_active=0, is_deleted=1 where  cost_sheet_id =".$txt_costSheet_id." and status_active=1 and is_deleted=0",1);
		
		$rID1=execute_query( "update qc_confirm_dtls set updated_by=".$_SESSION['logic_erp']['user_id'].", update_date='".$pc_date_time."', status_active=0, is_deleted=1 where  cost_sheet_id =".$txt_costSheet_id." and status_active=1 and is_deleted=0",1);
		//echo "10**update qc_confirm_dtls set updated_by=".$_SESSION['logic_erp']['user_id'].", update_date='".$pc_date_time."', status_active=0, is_deleted=1 where  cost_sheet_id =".$txt_costSheet_id." and status_active=1 and is_deleted=0";
...
if($action=="populate_confirm_style_form_data")
{
	...
	$sql_confirm_dtls=sql_select("select id, mst_id, cost_sheet_id, item_id, fab_cons_kg, fab_cons_mtr, fab_cons_yds, fab_amount, sp_oparation_amount, wash_amount, acc_amount, fright_amount, courier_amount, lab_amount, inspection_cost, operating_exp, other_amount, commercial_cost, lc_cost, comm_amount, fob_amount, cppm_amount, smv_amount, cm_amount, rmg_ratio from qc_confirm_dtls where cost_sheet_id='$ex_data[0]' and status_active=1 and is_deleted=0");
	foreach($sql_confirm_dtls as $row_dtls)
	{
		...
		echo "$('#txtOptExpCst_".$row_dtls[csf("item_id")]."').val('".$row_dtls[csf("operating_exp")]."');\n";
		...
		echo "$('#txtDlcCst_".$row_dtls[csf("item_id")]."').val('".$row_dtls[csf("lc_cost")]."');\n";
...
if($action=="budgete_cost_validate")
{
	...
	if($job_no!="")
	{
		...
		$dataArr=sql_select($sql);
		$fab_amount=$sp_oparation_amount=$wash_amount=$acc_amount=$fright_amount=$lab_amount=$inspection_amt=$optexp_amount=$other_amount=$comm_amount=$fob_amount=$cm_amount=$rmg_ratio=0;
		...
		foreach($dataArr as $row)
		{
			...
			$inspection_amt+=$row[csf("inspection")];
			$optexp_amount+=$row[csf("common_oh")];
		...
		$str_data=$fab_cons_kg."##".$fab_cons_mtr."##".$fab_cons_yds."##".$fab_amount."##".$sp_oparation_amount."##".$acc_amount."##".$fright_amount."##".$lab_amount."##".$inspection_amt."##".$optexp_amount."##".$other_amount."##".$comm_amount."##".$fob_amount."##".$cm_amount."##".$rmg_ratio."##".$wash_amount;
		unset($dataArr);
	}
        ...
        <table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
            ...
            <tr class="general">
                <td> 
					...
					<? echo create_drop_down( "cbo_company_mst", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-Select Company-", $cbo_company_name,"load_drop_down( 'short_quotation_v6_controller', this.value, 'load_drop_down_buyer_pop', 'buyer_pop_td' );",'' ); ?>
                </td>
                ...
                <td align="center">
                	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_id').value+'_'+document.getElementById('chk_job_wo_po').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_job_prifix').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_style').value, 'create_po_search_list_view', 'search_div', 'short_quotation_v6_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" /></td>
        	</tr>
            <tr>
		...
		<fieldset>
            <table cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr>
                        <td align="center">
                        	 <? echo create_drop_down( "cbo_buyer_id", 120, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name",1, "-- All Buyer--",$selected,"load_drop_down( 'short_quotation_v6_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'short_quotation_v6_controller',this.value, 'load_drop_down_sub_dep', 'sub_td' );",0 ); ?>
                        </td>                 
                        <td id="season_td"><? echo create_drop_down( "cbo_season_id", 130, $blank_array,'', 1, "-- Select Season--",$selected, "" ); ?></td>
                        <td id="sub_td"><? echo create_drop_down( "cbo_subDept_id", 130, $blank_array,'', 1, "-- Select Dept--",$selected, "" ); ?></td>     
...
if($action=="agent_location_popup")
{
		...
		function fnc_agent_location_reponse()
		{
			if(http.readyState == 4) 
			{
				...
				if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
				{
					...
					show_list_view(type,'agent_location_data_list','agent_location_data_div','short_quotation_v6_controller','');
				}
    ...
    <script>show_list_view(type,'agent_location_data_list','agent_location_data_div','short_quotation_v6_controller','');</script>          
    <script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
    </html>
    <?
	exit();	
...
if($action=="quick_costing_print")
{
	...
	$noofitem=count(explode(",",$master_data[0][csf('lib_item_id')]));
	...
	?>
		...
		<table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:930px;" rules="all">
			<tr>
				<td width="110">Quot. Inq. : </td>
				<td width="130"><?=$master_data[0][csf('inquery_id')]; ?></td>
			...
			<tr>
                ...
                <td>Qty Per Pack : </td>
				<td align="center"><?=$packQty; ?></td>
                <td>FOB/Pack : </td>
				<td align="center"><?=$fobTot; ?></td>
                <td>Quoted Price/Pack ($) : </td>
        	...
        	<tr>
            	<td width="460" valign="top">
                	<table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:460px;" rules="all">
                    	<tr>
                        	<td colspan="4" align="center"><b>Cost Summary</b></td>
                        </tr>
                        <tr bgcolor="#CCFFFF">
                        	<td width="250">Particulars</td>
                            <td width="70">Cost/Pcs</td>
                            <td width="70">Cost/Pack</td>
                            <td>% On FOB</td>
                        </tr>
                        <tr>
                        	<td width="250">Total Fabric Cost</td>
                            <td width="70" align="center"><?=number_format(($sqlSumData[0][csf('tot_fab_cost')]/$packQty),4); ?></td>
                            <td width="70" align="center"><?=number_format($sqlSumData[0][csf('tot_fab_cost')],4); ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('tot_fab_cost')]/$fobTot)*100),4); ?></td>
                        </tr>
                        <tr>
                        	<td width="250">Total Accessories Cost</td>
                            <td width="70" align="center"><?=number_format(($sqlSumData[0][csf('tot_accessories_cost')]/$packQty),4); ?></td>
                            <td width="70" align="center"><?=number_format($sqlSumData[0][csf('tot_accessories_cost')],4); ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('tot_accessories_cost')]/$fobTot)*100),4); ?></td>
                        </tr>
                        <tr>
                        	<td width="250">Total Embellishment Cost</td>
                            <td width="70" align="center"><?=number_format((($sqlSumData[0][csf('tot_sp_operation_cost')]+$sqlSumData[0][csf('tot_wash_cost')])/$packQty),4); ?></td>
                            <td width="70" align="center"><?=number_format(($sqlSumData[0][csf('tot_sp_operation_cost')]+$sqlSumData[0][csf('tot_wash_cost')]),4); ?></td>
                            <td align="center"><?=number_format(((($sqlSumData[0][csf('tot_sp_operation_cost')]+$sqlSumData[0][csf('tot_wash_cost')])/$fobTot)*100),4); ?></td>
                        </tr>
                        <tr>
                        	<td width="250">Lab Test</td>
                            <td width="70" align="center"><?=number_format(($sqlSumData[0][csf('tot_lab_test_cost')]/$packQty),4); ?></td>
                            <td width="70" align="center"><?=number_format($sqlSumData[0][csf('tot_lab_test_cost')],4); ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('tot_lab_test_cost')]/$fobTot)*100),4); ?></td>
                        </tr>
                        <tr>
                        	<td width="250">CM Cost</td>
                            <td width="70" align="center"><?=number_format(($sqlSumData[0][csf('tot_cm_cost')]/$packQty),4); ?></td>
                            <td width="70" align="center"><?=number_format($sqlSumData[0][csf('tot_cm_cost')],4); ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('tot_cm_cost')]/$fobTot)*100),4); ?></td>
                        </tr>
                        <tr>
                        	<td width="250">Frieght Cost</td>
                            <td width="70" align="center"><?=number_format(($sqlSumData[0][csf('tot_fright_cost')]/$packQty),4); ?></td>
                            <td width="70" align="center"><?=number_format($sqlSumData[0][csf('tot_fright_cost')],4); ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('tot_fright_cost')]/$fobTot)*100),4); ?></td>
                        </tr>
                        <tr>
                        	<td width="250">Operating Expenses</td>
                            <td width="70" align="center"><?=number_format(($sqlSumData[0][csf('operating_exp')]/$packQty),4); ?></td>
                            <td width="70" align="center"><?=number_format($sqlSumData[0][csf('operating_exp')],4); ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('operating_exp')]/$fobTot)*100),4); ?></td>
                        </tr>
                        <tr>
                        	<td width="250">Other Cost</td>
                            <td width="70" align="center"><?=number_format(($sqlSumData[0][csf('tot_other_cost')]/$packQty),4); ?></td>
                            <td width="70" align="center"><?=number_format($sqlSumData[0][csf('tot_other_cost')],4); ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('tot_other_cost')]/$fobTot)*100),4); ?></td>
                        </tr>
                        <tr>
                        	<td width="250">Commercial Cost</td>
                            <td width="70" align="center"><?=number_format(($sqlSumData[0][csf('commercial_cost')]/$packQty),4); ?></td>
                            <td width="70" align="center"><?=number_format($sqlSumData[0][csf('commercial_cost')],4); ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('commercial_cost')]/$fobTot)*100),4); ?></td>
                        </tr>
                        <tr>
                        	<td width="250">Commission</td>
                            <td width="70" align="center"><?=number_format(($sqlSumData[0][csf('tot_commission_cost')]/$packQty),4); ?></td>
                            <td width="70" align="center"><?=number_format($sqlSumData[0][csf('tot_commission_cost')],4); ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('tot_commission_cost')]/$fobTot)*100),4); ?></td>
                        </tr>
                        <tr>
                        	<td width="250">Margin/Profit</td>
                            <td width="70" align="center"><?=number_format(($master_data[0][csf('margin')]/$packQty),4); ?></td>
                            <td width="70" align="center"><?=number_format($master_data[0][csf('margin')],4); ?></td>
                            <td align="center"><?=number_format((($master_data[0][csf('margin')]/$fobTot)*100),4); ?></td>
                        </tr>
                        <tr>
                        	<td width="250"><b>FOB $</b></td>
                            <td width="70" align="center"><?=number_format(($fobTot/$packQty),4); ?></td>
                            <td width="70" align="center"><?=number_format($fobTot,4); ?></td>
                            <td align="center"><?=number_format((($fobTot/$fobTot)*100),4); ?></td>
                        </tr>
                    </table>
            	</td>
                <td>
		...
		<table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:930px; margin-top: 10px" rules="all">
			<thead>
            	...
            	<tr>
                    ...
                    <th width="180">Fab. Des.</th>
                    <th width="65">Color Type</th>
                    <th width="45">Width /GSM</th>
                    <th width="60">Use For</th>
                    <th width="60">Color X Ratio[Pcs]</th>
                    <th width="45">UOM</th>
                    <th width="45">Cons /Dzn</th>
                    <th width="45">Cons /Pack</th>
                    <th width="45">Ex. %</th>
                    <th width="45">Ttl Cons. [Pack]</th>
                    <th width="40">Avg. Rate</th>
                    <th width="50">Cost /Pack</th>
                    <th>% On FOB</th>
                </tr>
			</thead>
			<? 
			$consSql="select id, mst_id, item_id, type, particular_type_id, consumption, ex_percent, tot_cons, rate, is_calculation, rate_data, value, fab_id, description, use_for, body_part_id, supp_ref, uom, color_type_id, gsmweight, fabwidth, dznyds, dznkg from qc_cons_rate_dtls where mst_id='$qc_no' and status_active=1 and is_deleted=0 and type!=4 order by id ASC";
			$consSqlData=sql_select($consSql);
			foreach ($consSqlData as $val)
			{
				if($val[csf('type')]==1 && ($val[csf('tot_cons')]*1>0))
				{
					//echo $val[csf('is_calculation')].'='.$val[csf('rate_data')].'<br>';
					if($val[csf('is_calculation')]==1 && trim($val[csf('rate_data')])!="")
					{
						$exratestrdata=explode("@@!",$val[csf('rate_data')]);
						//print_r($exratestrdata);
						foreach($exratestrdata as $ratestrval)
						{
							//echo $ratestrval.'<br>';
							$exrdata=explode("_",$ratestrval);
							
							$fabPerFob=0; $colorDesc="";
							$fabPerFob=($exrdata[10]/$fobTot)*100;
							$colorDesc=$exrdata[2].' X '.$exrdata[3];
							?>
							<tr>
                            	<td width="60"><?=$garments_item[$val[csf('item_id')]]; ?></td>
								<td width="70"><?=$body_part[$val[csf('body_part_id')]]; ?></td>
								<td width="180"><?=$val[csf('description')]; ?></td>
								<td width="65" align="center"><?=$color_type[$val[csf('color_type_id')]]; ?></td>
								<td width="45" align="center"><?=$val[csf('gsmweight')].'/'.$val[csf('fabwidth')]; ?></td>
								<td width="60"><?=$val[csf('use_for')]; ?></td>
								<td width="60"><?=$colorDesc; ?></td>
                                <td width="45"><?=$unit_of_measurement[$val[csf('uom')]]; ?></td>
								<td width="45" align="right"><?=number_format($exrdata[5],4); ?></td>
								<td width="45" align="right"><?=number_format($exrdata[6],4); ?></td>
								<td width="45" align="right"><?=number_format($exrdata[7],4); ?></td>
								<td width="45" align="right"><?=number_format($exrdata[8],4); ?></td>
								<td width="40" align="right"><?=number_format($exrdata[9],4); ?></td>
								<td width="50" align="right"><?=number_format($exrdata[10],4); ?></td>
								<td align="right"><?=number_format($fabPerFob,4); ?></td>
							</tr><?
						}
					}
					else
					{
						$fabPerFob=0;
						$fabPerFob=($val[csf('value')]/$fobTot)*100;
						?>
						<tr>
                        	<td width="60"><?=$garments_item[$val[csf('item_id')]]; ?></td>
							<td width="70"><?=$body_part[$val[csf('body_part_id')]]; ?></td>
							...
							<td width="60"><? //=$val[csf('use_for')]; ?>&nbsp;</td>
                            <td width="45"><?=$unit_of_measurement[$val[csf('uom')]]; ?></td>
							<td width="45" align="right"><?=number_format($val[csf('dznkg')],4); ?></td>
							<td width="45" align="right"><?=number_format($val[csf('consumption')],4); ?></td>
							<td width="45" align="right"><?=number_format($val[csf('ex_percent')],4); ?></td>
							<td width="45" align="right"><?=number_format($val[csf('tot_cons')],4); ?></td>
							<td width="40" align="right"><?=number_format($val[csf('rate')],4); ?></td>
							<td width="50" align="right"><?=number_format($val[csf('value')],4); ?></td>
							<td align="right"><?=number_format($fabPerFob,4); ?></td>
						</tr><?
					}
					$fabtot+=$val[csf('value')];
				}
			}
			$gfabPerFob=($fabtot/$fobTot)*100;
			?>
            <tr bgcolor="#CCCCCC">
            	<td width="60">&nbsp;</td>
                <td width="70">&nbsp;</td>
                <td width="180">Total Fabric Cost:</td>
                ...
                <td width="50" align="right"><?=number_format($fabtot,4); ?></td>
                <td align="right"><?=number_format($gfabPerFob,4); ?></td>
            </tr>
        </table>
        ...
        <table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:930px; margin-top: 10px" rules="all">
			<thead>
            	<tr>
                	<th colspan="12">Accessories Details</th>
                </tr>
            	<tr>
                	<th width="70">Gmts. Item</th>
                    <th width="120">Item Name</th>
                    <th width="140">Description</th>
                    <th width="70">Use For</th>
                    <th width="70">UOM</th>
                    <th width="60">Cons/Dzn</th>
                    <th width="60">Cons/Pack</th>
                    <th width="60">Ex. %</th>
                    <th width="60">Ttl Cons.</th>
                    <th width="60">Avg. Rate</th>
                    <th width="60">Cost/Pack</th>
                    <th>% On FOB</th>
                </tr>
			</thead>
			<? 
			$consAccSql="select a.id, a.mst_id, a.item_id, a.type, a.particular_type_id, a.consumption, a.ex_percent, a.tot_cons, a.rate, a.is_calculation, a.rate_data, a.value, a.fab_id, a.description, a.use_for, a.body_part_id, a.supp_ref, a.uom, a.color_type_id, a.gsmweight, a.fabwidth, a.dznyds, a.dznkg, b.item_name, b.trim_type from qc_cons_rate_dtls a, lib_item_group b where a.particular_type_id=b.id and a.mst_id='$qc_no' and a.status_active=1 and a.is_deleted=0 and a.type=4";
			$consAccSqlData=sql_select($consAccSql);
			$trmsDataArr=array();
			foreach($consAccSqlData as $valAcc)
			{
				if($valAcc[csf('type')]==4 && ($valAcc[csf('tot_cons')]*1>0))
				{
					$trmsDataArr[$valAcc[csf('item_id')]][$valAcc[csf('trim_type')]][$valAcc[csf('id')]]['exdata']=$valAcc[csf('value')].'__'.$valAcc[csf('uom')].'__'.$valAcc[csf('dznkg')].'__'.$valAcc[csf('consumption')].'__'.$valAcc[csf('ex_percent')].'__'.$valAcc[csf('tot_cons')].'__'.$valAcc[csf('rate')];
					$trmsDataArr[$valAcc[csf('item_id')]][$valAcc[csf('trim_type')]][$valAcc[csf('id')]]['tname']=$valAcc[csf('item_name')];
					$trmsDataArr[$valAcc[csf('item_id')]][$valAcc[csf('trim_type')]][$valAcc[csf('id')]]['desc']=$valAcc[csf('description')];
					$trmsDataArr[$valAcc[csf('item_id')]][$valAcc[csf('trim_type')]][$valAcc[csf('id')]]['usefor']=$valAcc[csf('use_for')];
				}
			}
			//$trim_group=return_library_array( "select item_name,id from lib_item_group", "id", "item_name");
			//$consAccSqlData=sql_select($consAccSql);
			foreach ($trmsDataArr as $gmtitemid=>$gmtitemdata)
			{
				foreach ($gmtitemdata as $trimType=>$arrdata)
				{
					?>
					<tr>
						<td colspan="12" bgcolor="#CCCCCC" align="center"><b><?=$trim_type[$trimType]; ?><b/>&nbsp;</td>
					</tr>
					<? $ttypetotal=0;
					foreach ($arrdata as $rid=>$otdata)
					{
						$exrdata=explode('__',$otdata['exdata']);
						
						$value=$exrdata[0];
						$trimuom=$exrdata[1];
						$dznkg=$exrdata[2];
						$consumption=$exrdata[3];
						$ex_percent=$exrdata[4];
						$tot_cons=$exrdata[5];
						$rate=$exrdata[6];
						
						$accPerFob=0;
						$accPerFob=($value/$fobTot)*100;
						?>
						<tr>
                        	<td width="70"><?=$garments_item[$gmtitemid]; ?></td>
							<td width="120"><?=$otdata['tname']; ?></td>
							<td width="140"><?=$otdata['desc']; ?></td>
							<td width="70"><?=$otdata['usefor']; ?></td>
							<td width="70" align="center"><?=$unit_of_measurement[$trimuom]; ?></td>
							
							<td width="60" align="right"><?=number_format($dznkg,4); ?></td>
							<td width="60" align="right"><?=number_format($consumption,4); ?></td>
							<td width="60" align="right"><?=number_format($ex_percent,4); ?></td>
							<td width="60" align="right"><?=number_format($tot_cons,4); ?></td>
							<td width="60" align="right"><?=number_format($rate,4); ?></td>
							<td width="60" align="right"><?=number_format($value,4); ?></td>
							<td align="right"><?=number_format($accPerFob,4); ?></td>
						</tr><?
						$acctot+=$value;
						$ttypetotal+=$value;
					}
					$ttypeAccPerFob=($ttypetotal/$fobTot)*100;
					?>
					<tr bgcolor="#CCCCCC">
						<td colspan="10" align="right"><b><?=$trim_type[$trimType]; ?> Total=<b/>&nbsp;</td>
						<td align="right"><b><?=number_format($ttypetotal,4); ?><b/></td>
						<td align="right"><b><?=number_format($ttypeAccPerFob,4); ?><b/></td>
					</tr>
					<?
				}
			}
			$gAccPerFob=($acctot/$fobTot)*100;
			?>
            <tr bgcolor="#CCCCCC">
            	<td width="70">&nbsp;</td>
                <td width="120">&nbsp;</td>
                <td width="140">Total Accessories Cost:</td>
                ...
                <td width="60" align="right">&nbsp;</td>
                <td width="60" align="right">&nbsp;</td>
                <td width="60" align="right"><?=number_format($acctot,4); ?></td>
                <td align="right"><?=number_format($gAccPerFob,4); ?></td>
            </tr>
        </table>
        ...
        <table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:930px; margin-top: 10px" rules="all">
			<thead>
            	...
            	<tr>
                    ...
                    <th width="150">Embellishment Type</th>
			...
			foreach ($consSqlData as $val)
			{
				if(($val[csf('type')]==2 || $val[csf('type')]==3) && ($val[csf('tot_cons')]*1>0))
				{
					$embPerFob=0;
					$embPerFob=($val[csf('value')]/$fobTot)*100;
					$embName=""; $emb_type="";
					if($val[csf('type')]==2)
					{
						$embName=$emblishment_name_array[$val[csf('particular_type_id')]];
						if($val[csf('particular_type_id')]==1) $emb_type=$emblishment_print_type[$val[csf('fab_id')]];
						else if($val[csf('particular_type_id')]==2) $emb_type=$emblishment_embroy_type[$val[csf('fab_id')]];
						else if($val[csf('particular_type_id')]==3) $emb_type=$emblishment_wash_type[$val[csf('fab_id')]];
						else if($val[csf('particular_type_id')]==4) $emb_type=$emblishment_spwork_type[$val[csf('fab_id')]];
						else if($val[csf('particular_type_id')]==5) $emb_type=$emblishment_gmts_type[$val[csf('fab_id')]];
					}
					else if($val[csf('type')]==3)
					{
						$embName="Wash";
						$emb_type=$emblishment_spwork_type[$val[csf('particular_type_id')]];
					}
					?>
					<tr>
                    	<td width="100"><?=$garments_item[$val[csf('item_id')]]; ?></td>
						<td width="120"><?=$embName; ?></td>
						<td width="150"><?=$emb_type; ?></td>
                        <td width="120"><?=$body_part[$val[csf('body_part_id')]]; ?></td>
						
						<td width="60" align="right"><?=number_format($val[csf('consumption')],4); ?></td>
						<td width="60" align="right"><?=number_format($val[csf('ex_percent')],4); ?></td>
                        <td width="60" align="right"><?=number_format($val[csf('tot_cons')],4); ?></td>
						<td width="60" align="right"><?=number_format($val[csf('rate')],4); ?></td>
						<td width="60" align="right"><?=number_format($val[csf('value')],4); ?></td>
						<td align="right"><?=number_format($embPerFob,2); ?></td>
					</tr><?
					$embtot+=$val[csf('value')];
				}
			}
			$gEmbPerFob=($embtot/$fobTot)*100;
			?>
            <tr bgcolor="#CCCCCC">
            	<td width="100">&nbsp;</td>
                <td width="120">&nbsp;</td>
                <td width="150">Total Embellishment Cost:</td>
                <td width="120" align="center">&nbsp;</td>
                <td width="60" align="right">&nbsp;</td>
                <td width="60" align="right">&nbsp;</td>
                <td width="60" align="right">&nbsp;</td>
                <td width="60" align="right">&nbsp;</td>
                <td width="60" align="right"><?=number_format($embtot,4); ?></td>
                <td align="right"><?=number_format($gEmbPerFob,2); ?></td>
            </tr>
        </table>
        <br>
        <table cellpadding="1" cellspacing="1" style="width:930px;">
        	<tr>
            	<td width="410" valign="top">
                    <table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:410px;" rules="all">
                        <tr>
                            <td colspan="4" align="center"><b>Others Component</b></td>
                        </tr>
                        <tr bgcolor="#CCFFFF">
                            <td width="200">Particulars</td>
                            <td width="70">Cost/Dzn</td>
                            <td width="70">Cost/Pack</td>
                            <td>% On FOB</td>
                        </tr>
                        <tr>
                            <td width="210">Lab Test</td>
                            <td width="70" align="center"><?=number_format((($sqlSumData[0][csf('tot_lab_test_cost')]/$packQty)*12),4); ?></td>
                            <td width="70" align="center"><?=$sqlSumData[0][csf('tot_lab_test_cost')]; ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('tot_lab_test_cost')]/$fobTot)*100),2); ?></td>
                        </tr>
                        <tr>
                            <td width="210">CM Cost</td>
                            <td width="70" align="center"><?=number_format((($sqlSumData[0][csf('tot_cm_cost')]/$packQty)*12),4); ?></td>
                            <td width="70" align="center"><?=$sqlSumData[0][csf('tot_cm_cost')]; ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('tot_cm_cost')]/$fobTot)*100),2); ?></td>
                        </tr>
                        <tr>
                            <td width="210">Frieght Cost</td>
                            <td width="70" align="center"><?=number_format((($sqlSumData[0][csf('tot_fright_cost')]/$packQty)*12),4); ?></td>
                            <td width="70" align="center"><?=$sqlSumData[0][csf('tot_fright_cost')]; ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('tot_fright_cost')]/$fobTot)*100),2); ?></td>
                        </tr>
                        <tr>
                            <td width="210">Operating Expenses</td>
                            <td width="70" align="center"><?=number_format((($sqlSumData[0][csf('operating_exp')]/$packQty)*12),4); ?></td>
                            <td width="70" align="center"><?=$sqlSumData[0][csf('operating_exp')]; ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('operating_exp')]/$fobTot)*100),2); ?></td>
                        </tr>
                        <tr>
                        	<td width="210">Other Cost</td>
                            <td width="70" align="center"><?=number_format(($sqlSumData[0][csf('tot_other_cost')]/$packQty),4); ?></td>
                            <td width="70" align="center"><?=$sqlSumData[0][csf('tot_other_cost')]; ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('tot_other_cost')]/$fobTot)*100),2); ?></td>
                        </tr>
                        <tr>
                            <td width="210">Commercial Cost</td>
                            <td width="70" align="center"><?=number_format((($sqlSumData[0][csf('commercial_cost')]/$packQty)*12),4); ?></td>
                            <td width="70" align="center"><?=$sqlSumData[0][csf('commercial_cost')]; ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('commercial_cost')]/$fobTot)*100),2); ?></td>
                        </tr>
                        <tr>
                            <td width="210">Commission</td>
                            <td width="70" align="center"><?=number_format((($sqlSumData[0][csf('tot_commission_cost')]/$packQty)*12),4); ?></td>
                            <td width="70" align="center"><?=$sqlSumData[0][csf('tot_commission_cost')]; ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('tot_commission_cost')]/$fobTot)*100),2); ?></td>
                        </tr>
                        <tr>
                            <td width="210">Margin/Profit</td>
                            <td width="70" align="center"><?=number_format((($master_data[0][csf('margin')]/$packQty)*12),4); ?></td>
                            <td width="70" align="center"><?=$master_data[0][csf('margin')]; ?></td>
                            <td align="center"><?=number_format((($master_data[0][csf('margin')]/$fobTot)*100),2); ?></td>
                        </tr>
                        <tr>
                            <td width="210"><b>FOB $</b></td>
                            <td width="70" align="center"><?=number_format((($fobTot/$packQty)*12),4); ?></td>
                            <td width="70" align="center"><?=$fobTot; ?></td>
                            <td align="center"><?=number_format((($fobTot/$fobTot)*100),2); ?></td>
                        </tr>
                    </table>
                </td>
                <td width="340" valign="top">
                	<table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:340px;" rules="all">
						<?
						$sqlSmv="select sum(smv) as smv, sum(efficiency) as efficiency, sum(cpm) as cpm from qc_item_cost_summary where mst_id='$qc_no' and status_active =1 and is_deleted=0";
						$sqlSmvRes=sql_select($sqlSmv);
						$cm_dzn=($sqlSumData[0][csf('tot_cm_cost')]/$packQty)*12;
						$margin_dzn=(($master_data[0][csf('margin')]/$packQty)*12);
						?>
                        <tr>
                            <td colspan="4" align="center"><b>Contribution Margin</b></td>
                        </tr>
                        <tr bgcolor="#CCFFFF">
                            <td width="200">Particulars</td>
                            <td width="70">Cost/Pack</td>
                        ...
                        <tr>
                            <td width="200">CM Cost</td>
                            <td width="70" align="center"><?=$sqlSumData[0][csf('tot_cm_cost')]; ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('tot_cm_cost')]/$fobTot)*100),2); ?></td>
                        </tr>
                        <tr>
                            <td width="200">Operating Expenses</td>
                            <td width="70" align="center"><?=$sqlSumData[0][csf('operating_exp')]; ?></td>
                            <td align="center"><?=number_format((($sqlSumData[0][csf('operating_exp')]/$fobTot)*100),2); ?></td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td width="70" align="center"><?
							$contributionMargin=$sqlSumData[0][csf('tot_cm_cost')]+$sqlSumData[0][csf('operating_exp')]+$sqlSumData[0][csf('commercial_cost')]+$master_data[0][csf('margin')];
							echo number_format($contributionMargin,4);
							 ?></td>
                            <td align="center"><?=number_format((($contributionMargin/$fobTot)*100),2); ?></td>
                        </tr>
						<tr>
                            <td width="200" title="CM+Mergin">CM with Margin[Dzn] :</td>
                            <td width="70" align="center" colspan="2"><?
							$cm_with_margin=$cm_dzn+$margin_dzn;
							echo number_format($cm_with_margin,4);
							 ?></td>
                        </tr>
						<tr>
                            <td width="200" title="(CM with Margin[Dzn]/12)/SMV">EPM [ Pcs] :</td>
                            <td width="70" align="center" colspan="2" title="<?='('.$cm_with_margin.'/12)/('.$sqlSmvRes[0][csf('smv')].'/'.$noofitem.')'; ?>"><?
							$epm_pcs=($cm_with_margin/12)/($sqlSmvRes[0][csf('smv')]/$noofitem);
							echo number_format($epm_pcs,4);
							 ?></td>
                        </tr>
                    </table>
                </td>
                <td valign="top">
                	<table class="rpt_table" border="1" cellpadding="1" cellspacing="1" style="width:170px;" rules="all">
                        <tr>
                            <td colspan="2" align="center"><b>SMV Details</b></td>
                        </tr>
                        <tr>
                            <td width="100">CPM</td>
                            <td align="center"><?=$sqlSmvRes[0][csf('cpm')] ?></td>
                        </tr>
                        <tr>
                            <td width="100">SMV</td>
                            <td align="center"><?=$sqlSmvRes[0][csf('smv')] ?></td>
                        </tr>
                        <tr>
                            <td width="100">Effency %</td>
                            <td align="center"><?=$sqlSmvRes[0][csf('efficiency')] ?></td>
                        </tr>
                    </table>
                </td>
            </tr>
        ...
        <?
	echo signature_table(109, $cbo_company_id, "930px");
	exit();
}
...
if($action=="quick_costing_print2")
{
    ...
    <div style="width:735px;">
        ...
        <table width="<?=$tblWeight; ?>" cellspacing="0" border="1" rules="all" class="rpt_table">
			...
			$despQcArr=array();
			$despQcArr=array(1 => "Fabric", 2 => "Special Operation", 3 => "Wash", 4 => "Accessories", 5 => "CM [$/".$uomstr."]", 6 => "Frieght Cost[$/".$uomstr."]", 7 => "Lab - Test[$/".$uomstr."]", 8 => "Opt. Exp. ", 9 => "Other Cost[$/".$uomstr."]", 10 => "Comml.[$/".$uomstr."]", 11 => "Com.(%)[$/".$uomstr."]", 12 => "Margin/Profit", 13 => "F.O.B[$/".$uomstr."]");
...
{
	echo create_drop_down( "cbo_buyer_name", 130, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90))  order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down( 'short_quotation_v6_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'short_quotation_v6_controller', this.value, 'load_drop_down_brand', 'brand_td');" );
	exit();
}
    ...
    <div align="center" style="width:100%;" >
        <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            <table width="910" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
                ...
                <tr class="general">
                    <td><? echo create_drop_down( "cbo_company_mst", 140, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", '',"load_drop_down( 'short_quotation_v6_controller', this.value, 'load_drop_down_buyer', 'buyer_td');" ); ?><input type="hidden" id="selected_id"></td>
                ...
                <tbody>
                    ...
                    foreach($accessories_arr as $row_tgroup)
                    {
                        ...
                        <tr id="search<?=$i; ?>" onClick="js_set_value(<?=$i; ?>);" bgcolor="<?=$bgcolor; ?>">
                            <td width="30" style="word-break:break-all" title="<?=$str; ?>"><?=$i; ?></td>
...
if($action=="populate_data_from_rdnolib")
{
	...
	$sqlRd="select id, type, construction, gsm_weight, weight_type, design, fabric_ref, rd_no, color_range_id, full_width, cutable_width from lib_yarn_count_determina_mst where status_active=1 and is_deleted=0 and id in ($data)";
	...
	if(count($sqlRdData)>0)
	{
		foreach($sqlRdData as $row)
		{
			...
			echo "$('#txt_fabDesc".$i."').attr('title', '".$fabric."');\n"; 
			echo "$('#txtfabwidth_".$i."').val('".$row[csf('gsm_weight')]."');\n"; 
			echo "$('#txtYarnDtls_".$i."').val('".$yarnBreak."');\n"; 
			$i++;
		}
		unset($sqlRdData);
	}	
...
if($action=="fabric_description_popup")
{
	...
	<script>
		function js_set_value(data)
		{
			...
			document.getElementById('hiddYarnDescription').value=data[6];
			parent.emailwindow.hide();
		}
...
<body>
   ...
   <div id="" style="max-height:300px; width:950px; overflow-y:scroll">
   <table id="list_view" class="rpt_table" width="930" height="" cellspacing="0" cellpadding="0" border="1" rules="all">
        <tbody>
	<?
	$sql_data=sql_select("select a.fab_nature_id, a.construction, a.gsm_weight, a.color_range_id, a.stich_length, a.process_loss, a.id, a.sequence_no from  lib_yarn_count_determina_mst a, lib_yarn_count_determina_dtls b where a.id=b.mst_id and a.fab_nature_id=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $search_con group by a.id, a.fab_nature_id, a.construction, a.gsm_weight, a.color_range_id, a.stich_length, a.process_loss, a.sequence_no order by a.id");//
	$i=1;
	foreach($sql_data as $row)
	{
		if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
		...
		$yarnBreak=$yarn_descriptionArr[$row[csf('id')]]; 
		?>
        <tr id="tr_<?=$row[csf('id')]; ?>" bgcolor="<?=$bgcolor; ?>" height="20" style="cursor:pointer; word-break:break-all;" onClick="js_set_value('<?=$row[csf('id')]."_".$row[csf('fab_nature_id')]."_".$row[csf('construction')]."_".$row[csf('gsm_weight')]."_".$row[csf('process_loss')]."_".$row[csf('construction')].', '.$composition_arr[$row[csf('id')]]."_".$yarnBreak; ?>');">
            <td width="30" title="<?=$yarnBreak; ?>"><?=$i; ?></td>
...
if($action=="trims_cost_template_name_popup")
{
    ...
    <script>
        ...
        {
            var template_data=return_global_ajax_value( data, 'get_template_data', '', 'short_quotation_v6_controller');
        ...
        function check_all_data() {
            ...
            if(document.getElementById('check_all').checked){
                ...
                var templatedata='';
                for( var i = 0; i < selected_name.length; i++ ) {
                    templatedata += selected_name[i] + '__';
                }
...
if($action == "get_template_data")
{
    ...
    foreach( $data_array as $row )
    {
        ...
        $amount=$row[csf('cons_dzn_gmts')]*$rate;
        if($rate=="" || $rate==0)
        {
     ...
     ?>
        <tr bgcolor="<?=$bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<?=$i;?>" class="itemdata" onClick="js_set_value(<?=$i; ?>)" >
            ...
            <td align="center"><?=$row[csf("cons_dzn_gmts")];?></td>
            <td align="center"><?=$rate;?></td>
            <td align="center"><?=number_format($amount,4);?></td>
