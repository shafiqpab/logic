...
<script type="text/javascript">
	<?
		$field_mst='';
		foreach($_SESSION['logic_erp']['mandatory_field'][471] as $key=>$value)
		{
			if($key==1)
			{
				$field_mst=$value;
...
<script>
	...
	function fnc_dtls_ganerate(val)
	{
		...
		if(itemConsRateData!="")
		{
			for(var m=1; m<=f; m++)
			{
				...
				var yarnDtls=itemConsRate_fab[19];
	...
	function fnc_amount_calculation(type,inc_id,rate)
	{
		...
		if (type=='fabric')
		{
			...
			var conspackkg=number_format(conspackkg,4,'.','');
			$("#txt_consumtion"+inc_id).val( conspackkg );
				...
				if(select_template_data!='')
				{
					...
					for(var b=1; b<=itemdata.length; b++)
					{
						...
						if(row_count == 1 && document.getElementById('cboconsuom_1').value == 0)
						{
							...
							document.getElementById('txtAccConsDzn_1').value=exdata[4];
						...
						else
						{
							...
							document.getElementById('txtAccConsDzn_'+row_count).value=exdata[4];
            ...
            <fieldset style="width:1200px;">
                <table width="1200px" cellspacing="2" cellpadding="0" border="0">
                    ...
                    <tr>
                        ...
                        <td><? echo create_drop_down( "cbo_buyer_id", 130, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,30,90)) order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down('requires/short_quotation_v3_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'requires/short_quotation_v3_controller',this.value, 'load_drop_down_sub_dep', 'sub_td' ); load_drop_down( 'requires/short_quotation_v3_controller', this.value, 'load_drop_down_brand', 'brand_td');" ); // Party Type 30 Added by CRM No-ISD-23-17835 ?></td>
                    	...
                    	<table width="885" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                        	<thead>
                                ...
                                <tr>
                                    ...
                                    <th width="40" style="display:none">DZN [YDS /MTR]</th>
                    	...
                    	<table width="865" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td width="120"><input style="width:107px;" type="text" class="text_boxes" name="txt_fabDesc<?=$m; ?>" id="txt_fabDesc<?=$m; ?>" readonly placeholder="Browse" onDblClick="fnc_fabric_popup(<?=$m; ?>);" />
                                    ...
                                    <td width="40" style="display:none"><input style="width:27px;" type="text" class="text_boxes_numeric" name="txtdznyds_<?=$m; ?>" id="txtdznyds_<?=$m; ?>" placeholder="Write" onChange="fnc_amount_calculation('fabric',<?=$m; ?>,'yds');" onBlur="fnc_itemwise_data_cache();" /></td>
                                       ...
                                       <table width="600" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all" id="tbl_acc">
                                        ...
                                        <tbody>
                                            <tr id="tracc_<?=$k; ?>">
                                                ...
                                                <td width="50"><input style="width:33px;" type="text" class="text_boxes" name="txtAccUseFor_<?=$k; ?>" id="txtAccUseFor_<?=$k; ?>" placeholder="Write" onBlur="fnc_itemwise_data_cache();" /></td>
