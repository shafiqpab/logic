...
<script>
	...
	function fnc_dtls_ganerate(val)
	{
		...
		for(var o=1; o<=wash_row; o++)
		{
			...
			$('#txtWbodypartid_'+o).val('');
			$('#txt_wConsumtion'+o).val('');
			$('#txt_wexper'+o).val('');
			$('#txt_totWConsumtion'+o).val('');
			$('#txtwRate_'+o).val('');
			$('#txt_wValue'+o).val('');
		...
		if(itemConsRateData!="")
		{
			for(var m=1; m<=f; m++)
			{
				...
				var bodypart=bodypartid=cons=exper=totCons=ratePop=rate=fabVal=fabRemarks="";
		...
		if(washData!="")
		{
			for(var o=1; o<=w; o++)
			{
				...
				$('#txt_wConsumtion'+o).val(wConsumtion);
				$('#txt_wexper'+o).val(wexper);
				$('#txt_totWConsumtion'+o).val(totwConsumtion);
				$('#txtwRate_'+o).val(rate);
				$('#txt_wValue'+o).val(wVal);
				$('#txtWashRemarks_'+o).val(wRemarks);
				q++;
			}
		}
				
		var accs=25; var adata=0;// itemAccData_ac='';
		if(itemAccData!="")
		{
			for(var q=1; q<=accs; q++)
			{
				...
				if(itemAccData[adata]!=undefined)
				{
					...
					var acctext=accId=accDescription=accBandRef=accConsumtion=acexper=totAccConsumtion=acRate=acValue=acRemarks="";
					...
					$('#txt_accConsumtion'+q).val(accConsumtion);
					$('#txt_acexper'+q).val(acexper);
					$('#txt_totAccConsumtion'+q).val(totAccConsumtion);
					$('#txt_acRate'+q).val(acRate);
					$('#txt_acValue'+q).val(acValue);
	...
	function fnc_amount_calculation(type,inc_id,rate)
	{
		...
		else if (type=='wash')
		{
			...
			cons_val=$("#txt_wConsumtion"+inc_id).val();
			ex_wper=$("#txt_wexper"+inc_id).val()*1;
			var ex_wash_cons=0;
			if(ex_spper!=0) ex_wash_cons=((cons_val*1)+((cons_val*ex_wper)/100));
			else ex_wash_cons=(cons_val*1);
			$("#txt_totWConsumtion"+inc_id).val( number_format(ex_wash_cons,2,'.','') );
			w_rate=$("#txtwRate_"+inc_id).val()*1;
			row_tot_value=(ex_wash_cons*1)*w_rate;
			$("#txt_wValue"+inc_id).val(number_format(row_tot_value,2,'.',''));
			
			var w_item_cost=0;
			for(var i=1; i<=5; i++)
			{
				w_item_cost=w_item_cost+($("#txt_wValue"+i).val()*1);
			}
			$("#wash_td"+item_id).text( number_format(w_item_cost,2,'.','') );
			...
			$("#totWash_td").text( number_format(row_wash_cost,2,'.',''));
		}
		...
		for(var bb=1; bb<=all_item_id_hid.length; bb++)
		{
			...
			before_commission_cost=($("#fab_td"+trim(all_item_id_hid[aa])).text()*1)+($("#spOpe_td"+trim(all_item_id_hid[aa])).text()*1)+($("#wash_td"+trim(all_item_id_hid[aa])).text()*1)+($("#acc_td"+trim(all_item_id_hid[aa])).text()*1)+($("#txtCmCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtFriCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtLtstCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtMissCost_"+trim(all_item_id_hid[aa])).val()*1)+($("#txtOtherCost_"+trim(all_item_id_hid[aa])).val()*1);
			commPer=($("#txt_commPer").val()*1)/100;
			//alert("#txtCommCost_"+trim(all_item_id_hid[aa]));
			commissson_cost=((before_commission_cost/(1-commPer))-before_commission_cost);
		...
		//alert(fob_rmg);
		var tot_cost=($("#totFab_td").text()*1)+($("#totSpc_td").text()*1)+($("#totWash_td").text()*1)+($("#totAcc_td").text()*1)+($("#totCm_td").text()*1)+($("#totFriCst_td").text()*1)+($("#totLbTstCst_td").text()*1)+($("#totMissCst_td").text()*1)+($("#totOtherCst_td").text()*1)+($("#totCommCst_td").text()*1);
		var tot_costPcs=tot_cost/12;
		
		//alert(tot_costDzn);
		$("#totCost_td").text( number_format(tot_cost,2,'.','') );
		$("#totFOBCost_td").text( number_format(tot_costPcs,2,'.','') );
	...
	function fnc_openmypage_rate(inc_id,cons)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link+'&qc_no='+qc_no+'&bodypartid='+bodypartid+'&totCons='+totCons, title, 'width=850px,height=380px,center=1,resize=0,scrolling=0','../')
		release_freezing();
		emailwindow.onclose=function()
		{
			...
			$('#txt_consumtion'+inc_id).val( number_format(cons,2,'.','') );
			$('#txt_exper'+inc_id).val( number_format(avgCons,2,'.','') );
			$('#txt_totConsumtion'+inc_id).val( number_format(tot_cost,2,'.','') );
			$('#txt_rate'+inc_id).val( number_format(avgRate,2,'.','') );
			$('#txt_value'+inc_id).val( number_format(totamount,2,'.','') );
	...
	function fnc_itemwise_data_cache( item_id )
	{
		...
		for(var m=1; m<=f; m++)
		{
			fabData+= ($('#txtbodyparttext_'+m).val())+'_'+($('#txtbodypartid_'+m).val()*1)+'_'+($('#txt_consumtion'+m).val()*1)+'_'+($('#txt_exper'+m).val()*1)+'_'+($('#txt_totConsumtion'+m).val()*1)+'_'+($('#ratePop_'+m).val()*1)+'_'+($('#txt_rate'+m).val()*1)+'_'+($('#txt_value'+m).val()*1)+'_'+($('#txtremarks_'+m).val())+'##';
		}
		...
		var wash_row=17; var washData='';
		for(var o=1; o<=wash_row; o++)
		{
			washData+= $('#txtWashTypetext_'+o).val()+'_'+($('#txtWashTypeId_'+o).val()*1)+'_'+($('#txtWbodyparttext_'+o).val())+'_'+($('#txtWbodypartid_'+o).val()*1)+'_'+($('#txt_wConsumtion'+o).val()*1)+'_'+($('#txt_wexper'+o).val()*1)+'_'+($('#txt_totWConsumtion'+o).val()*1)+'_'+($('#txtwRate_'+o).val()*1)+'_'+($('#txt_wValue'+o).val()*1)+'_'+($('#txtWashRemarks_'+o).val())+'##';
		}
		// alert( item_id_tmp+'=='+item_id );
		var acc=25; var accessoriesData="";
		for(var q=1; q<=acc; q++)
		{
			accessoriesData+= ($('#txtAcctext_'+q).val())+'_'+($('#txtAccId_'+q).val()*1)+'_'+($('#txtAccDescription_'+q).val())+'_'+($('#txtAccBandRef_'+q).val())+'_'+($('#txt_accConsumtion'+q).val()*1)+'_'+($('#txt_acexper'+q).val()*1)+'_'+($('#txt_totAccConsumtion'+q).val()*1)+'_'+($('#txt_acRate'+q).val()*1)+'_'+($('#txt_acValue'+q).val()*1)+'_'+($('#txtAccRemarks_'+q).val())+'##';
		}
	...
	function fnc_qcosting_entry( operation )
	{
		...
		if (form_validation('cbo_temp_id*cbo_cons_basis_id*txt_exchangeRate*cbo_buyer_id*cbo_season_id*txt_styleRef*txt_costingDate*txt_offerQty','Template Name*Cons. Basis*Exchange Rate*Buyer Name*Season*Style Ref.*Costing Date*Offer Qty.')==false)
		{
			return;
		}	
		else
		{
			...
			else
			{
				...
				for(i=1; i<=split_tmep_id.length; i++)
				{
					...
					item_wise_tot_data+=($('#fab_td'+itm_id).text()*1)+'_'+($('#spOpe_td'+itm_id).text()*1)+'_'+($('#wash_td'+itm_id).text()*1)+'_'+($('#acc_td'+itm_id).text()*1)+'_'+($('#txt_cpm_'+itm_id).val()*1)+'_'+($('#txt_smv_'+itm_id).val()*1)+'_'+($('#txt_eff_'+itm_id).val()*1)+'_'+($('#txtCmCost_'+itm_id).val()*1)+'_'+($('#txtFriCost_'+itm_id).val()*1)+'_'+($('#txtLtstCost_'+itm_id).val()*1)+'_'+($('#txtMissCost_'+itm_id).val()*1)+'_'+($('#txtOtherCost_'+itm_id).val()*1)+'_'+($('#txtCommCost_'+itm_id).val()*1)+'_'+($('#fobT_td'+itm_id).text()*1)+'_'+($('#fobPcsT_td'+itm_id).text()*1)+'_'+($('#txtRmgQty_'+itm_id).val()*1)+'_'+itm_id+'__';
	...
	function fnc_print_report(action)
	{
		...
		var report_title=$( "div.form_caption" ).html();
		generate_report_file( $('#hid_qc_no').val()+'*'+$('#txt_costSheetNo').val()+'*'+report_title, action,'requires/quick_costing_v3_controller');
		return;
	}
	...
	function fnc_openmypage_inquery()
	{
		...
		var page_link='requires/quick_costing_v3_controller.php?action=inquery_id_popup';
	...
	function fnc_bodyPart(inc)
	{
		...
		var page_link='requires/quick_costing_v3_controller.php?action=bodyPart_washType_ItemGroup_popup&type='+type;
                ...
                <tr>
                	<td width="520" valign="top">
                    	<table width="520" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                        	<thead>
                                <tr>
                                	<th class="must_entry_caption" id="item_td"><?=create_drop_down( "cboItemId", 90, $blank_array,"", 1, "-Select-", 1, "fnc_itemwise_data_cache(this.value); " ); ?></th>
                    	...
                    	<table width="500" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><input style="width:98px;" type="text" class="text_boxes" name="txtremarks_<?=$m; ?>" id="txtremarks_<?=$m; ?>" placeholder="Write" /></td>
                                </tr>
                                <? $m++; } ?>
                                </tbody>
                            </table>
                        ...
                        <div style="width:520px; max-height:100px; overflow-y:scroll" id="scroll_body" > 
                               <table width="500" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all"> 
                                ...
                                <tr bgcolor="#FFCCFF">
                                    ...
                                    <td><input style="width:33px;" type="text" class="text_boxes" name="txtSpRemarks_<?=$j; ?>" id="txtSpRemarks_<?=$j; ?>" placeholder="Write" /></td>
                                </tr>
                                <? $j++; } ?>
                            </table>
                        </div>
                        <table width="520" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                        	<thead>
                                <tr>
                                	<th class="must_entry_caption" colspan="8">Particulars For Wash</th>
                                </tr>
                                <tr>
                        ...
                        <div style="width:520px; max-height:100px; overflow-y:scroll" id="scroll_body" > 
                               <table width="500" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all"> 
								...
								?>
                                <tr bgcolor="#FFFF00">
                                    <td width="110"><input style="width:98px;" type="text" class="text_boxes" name="txtWashTypetext_<?=$w; ?>" id="txtWashTypetext_<?=$w; ?>" placeholder="Browse" onFocus="add_auto_complete('<?=$w.'_3'; ?>');" onDblClick="fnc_bodyPart('<?=$w.'_3'; ?>');" onBlur="fnc_itemwise_data_cache( document.getElementById('cboItemId').value ); fn_getIndex('<?=$w.'_3'; ?>',this.value);" readonly /><input style="width:50px;" type="hidden" name="txtWashTypeId_<?=$w; ?>" id="txtWashTypeId_<?=$w; ?>" readonly /></td>
                                    <td width="100"><input style="width:88px;" type="text" class="text_boxes" name="txtWbodyparttext_<?=$w; ?>" id="txtWbodyparttext_<?=$w; ?>" placeholder="Browse" onFocus="add_auto_complete('<?=$w.'_4'; ?>');" onDblClick="fnc_bodyPart('<?=$w.'_4'; ?>');" onBlur="fnc_itemwise_data_cache( document.getElementById('cboItemId').value ); fn_getIndex('<?=$w.'_4'; ?>',this.value);" readonly /><input style="width:50px;" type="hidden" name="txtWbodypartid_<?=$w; ?>" id="txtWbodypartid_<?=$w; ?>" readonly /></td>
                                    
                                    <td width="50"><input style="width:38px;" type="text" class="text_boxes_numeric" name="txt_wConsumtion<?=$w; ?>" id="txt_wConsumtion<?=$w; ?>" onChange="fnc_amount_calculation('wash',<?=$w; ?>,document.getElementById('txtwRate_<?=$w; ?>').value);" /></td>
                                    <td width="40"><input style="width:27px;" type="text" class="text_boxes_numeric" name="txt_wexper<?=$w; ?>" id="txt_wexper<?=$w; ?>" placeholder="Write" onChange="fnc_amount_calculation('wash',<?=$w; ?>,document.getElementById('txtwRate_<?=$w; ?>').value);" /></td>
                                    <td width="50"><input style="width:38px;" type="text" class="text_boxes_numeric" name="txt_totWConsumtion<?=$w; ?>" id="txt_totWConsumtion<?=$w; ?>" disabled /></td>
                                    ...
                                    <td width="50"><input style="width:38px;" type="text" class="text_boxes_numeric" name="txt_wValue<?=$w; ?>" id="txt_wValue<?=$w; ?>" readonly /></td>
                                    <td><input style="width:38px;" type="text" class="text_boxes" name="txtWashRemarks_<?=$w; ?>" id="txtWashRemarks_<?=$w; ?>" placeholder="Write" /></td>
                                </tr>
                                <? $w++; } ?>
                            </table>
                        </div>
                        <table width="520" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                        	<thead>
                                <tr>
                                	<th class="must_entry_caption" colspan="9">Particulars For Accessories</th>
                                </tr>
                                <tr>
                        ...
                        <div style="width:520px; max-height:100px; overflow-y:scroll" id="scroll_body" > 
                               <table width="500" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
								...
								?>
                                <tr>
                                    <td width="80"><input style="width:68px;" type="text" class="text_boxes" name="txtAcctext_<?=$k; ?>" id="txtAcctext_<?=$k; ?>" placeholder="Browse" onFocus="add_auto_complete('<?=$k.'_5'; ?>');" onDblClick="fnc_bodyPart('<?=$k.'_5'; ?>');" onBlur="fnc_itemwise_data_cache( document.getElementById('cboItemId').value ); fn_getIndex('<?=$k.'_5'; ?>',this.value);" readonly /><input style="width:50px;" type="hidden" name="txtAccId_<?=$k; ?>" id="txtAccId_<?=$k; ?>" readonly /></td>
                                    <td width="80"><input style="width:68px;" type="text" class="text_boxes" name="txtAccDescription_<?=$k; ?>" id="txtAccDescription_<?=$k; ?>" placeholder="Write" onBlur="fnc_itemwise_data_cache( document.getElementById('cboItemId').value );" /></td>
                                    <td width="50"><input style="width:38px;" type="text" class="text_boxes" name="txtAccBandRef_<?=$k; ?>" id="txtAccBandRef_<?=$k; ?>" placeholder="Write" onBlur="fnc_itemwise_data_cache( document.getElementById('cboItemId').value );" /></td>
                                    <td width="50"><input style="width:38px;" type="text" class="text_boxes_numeric" name="txt_accConsumtion<?=$k; ?>" id="txt_accConsumtion<?=$k; ?>" onChange="fnc_amount_calculation('accessories',<?=$k; ?>,document.getElementById('txt_acRate<?=$k; ?>').value);" /></td>
                                    <td width="40"><input style="width:27px;" type="text" class="text_boxes_numeric" name="txt_acexper<?=$k; ?>" id="txt_acexper<?=$k; ?>" placeholder="Write" onChange="fnc_amount_calculation('accessories',<?=$k; ?>,document.getElementById('txt_acRate<?=$k; ?>').value);" /></td>
                                    ...
                                    <td width="40"><input style="width:27px;" type="text" class="text_boxes_numeric" name="txt_acRate<?=$k; ?>" id="txt_acRate<?=$k; ?>"  onChange="fnc_amount_calculation('accessories',<?=$k; ?>,this.value);" /></td>
                                    <td width="50"><input style="width:38px;" type="text" class="text_boxes_numeric" name="txt_acValue<?=$k; ?>" id="txt_acValue<?=$k; ?>" readonly /></td>
                                    <td><input style="width:38px;" type="text" class="text_boxes" name="txtAccRemarks_<?=$k; ?>" id="txtAccRemarks_<?=$k; ?>" placeholder="Write" /></td>
                                </tr>
                                <? $k++; } ?>
                            </tbody>
                        </table>
