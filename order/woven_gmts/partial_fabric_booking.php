...
$cbo_company_name      = create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company  comp where status_active=1 and is_deleted=0 $company_cond order by company_name", "id,company_name",1, "-- Select Company --", "", "load_drop_down( 'requires/partial_fabric_booking_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );check_month_setting();validate_suplier();get_php_form_data( this.value, 'company_wise_report_button_setting','requires/partial_fabric_booking_controller' );",0,"" );
$cbo_buyer_name		   = create_drop_down( "cbo_buyer_name", 130, "select id,buyer_name from lib_buyer where status_active =1 and is_deleted=0 order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", "", "","","" );
$cbo_fabric_natu       = create_drop_down( "cbo_fabric_natu", 80, $item_category,"", 1, "-- Select --", 1,$onchange_func, $is_disabled, "2,3");
$cbouom                = create_drop_down( "cbouom", 50, $unit_of_measurement,'', 1, '-Uom-', $row[csf('uom')], "",$disabled,"1,12,23,27" );
$cbo_fabric_source     = create_drop_down( "cbo_fabric_source", 130, $fabric_source,"", 1, "-- Select --", "","", "", "");
$cbo_pay_mode          = create_drop_down( "cbo_pay_mode", 130, $pay_mode,"", 1, "-- Select Pay Mode --", 5, "load_drop_down( 'requires/partial_fabric_booking_controller', this.value, 'load_drop_down_suplier', 'sup_td' )","","1,2,3,5" );
...
$cbo_currency          = create_drop_down( "cbo_currency", 130, $currency,"",1, "-- Select --", 2, "check_exchange_rate()",0 );
$cbo_source            = create_drop_down( "cbo_source", 130, $source,"", 1, "-- Select Source --", "", "","" );
...
$cbo_ready_to_approved = create_drop_down( "cbo_ready_to_approved", 130, $yes_no,"", 1, "-- Select--", 2, "","","" );
$cbo_level             = create_drop_down( "cbo_level", 130, $level_arr,"", 0, "", 2, "","","" );
...
//echo $_SESSION['logic_erp']['mandatory_field'][108].'dd';
?>
<script>
...
<?
if(isset($_SESSION['logic_erp']['mandatory_field'][108]))
{
...
?>
	...
	function openmypage_adbooking(page_link,title){
		if (form_validation('cbo_company_name','Company Name')==false)
		{
			return;
		}
		var company=$("#cbo_company_name").val()*1;
		var cbo_buyer_name=$("#cbo_buyer_name").val()*1;
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link+'&company='+company+'&cbo_buyer_name='+cbo_buyer_name, title, 'width=660,height=300px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function(){
			var theform=this.contentDoc.forms[0];
			var theemail=this.contentDoc.getElementById("select_booking");
			if (theemail.value!=""){
				get_php_form_data( theemail.value, "populate_data_from_adv_search_popup", "requires/partial_fabric_booking_controller" );
			}
			$("#cbo_company_name").attr("disabled","disabled");
			$("#cbo_buyer_name").attr("disabled","disabled");
		}
	...
	function fnc_generate_booking(){
		if (form_validation('txt_booking_no*txt_order_no_id*cbo_fabric_natu*cbo_fabric_source','Booking No*Order No*Fabric Nature*Fabric Source')==false){
			...
			var data="action=generate_fabric_booking"+get_submitted_data_string('txt_booking_no*txt_order_no_id*cbo_fabric_natu*cbo_fabric_source*cbo_company_name*cbo_buyer_name*cbouom*cbo_fabric_description*cbo_supplier_name*cbo_level',"../../");
	...
	function fnc_fabric_booking_dtls( operation ){
		...
		var fab_source=$("#cbo_fabric_source").val()*1;
		//release_freezing()
			//return;
			alert(fab_source);
		var data_all="";
	...
	function generate_fabric_report(type,report_type,mail_id,is_mail_send){
		if ( form_validation('txt_booking_no','Booking No')==false ){
			...
			var data="action="+type+get_submitted_data_string('txt_booking_no*cbo_company_name*txt_order_no_id*cbo_fabric_natu*cbo_fabric_source*id_approved_id',"../../")+'&report_title='+$report_title+'&show_yarn_rate='+show_yarn_rate+'&path=../../'+'&report_type='+report_type+'&mail_id='+mail_id+'&is_mail_send='+is_mail_send;
			freeze_window(5);
			http.open("POST","requires/partial_fabric_booking_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
	...
	function check_exchange_rate(){
		...
		var response=return_global_ajax_value( cbo_currercy+"**"+booking_date, 'check_conversion_rate', '', 'requires/partial_fabric_booking_controller');
	...
	function fnResetForm(){
		...
		get_php_form_data( 0, 'company_wise_report_button_setting','requires/partial_fabric_booking_controller' );
	}
	...
	function fnc_check(type)
	{
		if(type==1)
		{
			if(document.getElementById('chk_pro_advanced').checked==true)
			{
				document.getElementById('chk_pro_advanced').value=1;
			}
			else if(document.getElementById('chk_pro_advanced').checked==false)
			{
				document.getElementById('chk_pro_advanced').value=2;
			}
		}
	}
    ...
    <div style="width:100%;" align="center">
        ...
        <form name="fabricbooking_1"  autocomplete="off" id="fabricbooking_1">
            <fieldset style="width:1020px;">
                <legend>Partial Fabric Booking </legend>
                <table  width="1010" cellspacing="2" cellpadding="0" border="0">
                    <tr>
                        <td align="right" class="must_entry_caption" colspan="4"> Booking No </td>
                        <td colspan="4">
                        <input class="text_boxes" type="text" style="width:160px" onDblClick="openmypage_booking('requires/partial_fabric_booking_controller.php?action=fabric_booking_popup','fabric Booking Search')" readonly placeholder="Double Click for Booking"  name="txt_booking_no" id="txt_booking_no"/>
                    ...
                    <tr>
                        <td width="110" id="booking_td">Booking Month</td>
                        ...
                        <td> <input class="datepicker" type="text" style="width:120px" name="txt_booking_date" id="txt_booking_date" onChange="check_exchange_rate();" value="<? echo $date?>" disabled />
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td><input class="datepicker" type="hidden" style="width:170px" name="txt_tna_date" id="txt_tna_date"/>
                        <input class="datepicker" type="text" style="width:120px" name="txt_delivery_date" id="txt_delivery_date" onChange="compare_date();"/></td>
                        <td>Currency</td>
                    ...
                    <tr>
                        ...
                        <td>Attention</td>
                        <td colspan="3"><input class="text_boxes" type="text" style="width:372px;" name="txt_attention" id="txt_attention"/></td>
                        <td>Level</td>
                    ...
                    <tr>
                        ...
                        <td colspan="3"><input class="text_boxes" type="text" maxlength="200" style="width:372px;"  name="txt_fabriccomposition" id="txt_fabriccomposition"/></td>
                        <td>Proceed for Advanced</td>
                    	<td><input type="checkbox" name="chk_pro_advanced" id="chk_pro_advanced" onClick="fnc_check(1);" value="2" ></td>
                    </tr>
                    <tr>
                        ...
                        <td>Remarks</td>
                        <td colspan="3"><input class="text_boxes" type="text" maxlength="200" style="width:372px;"  name="txt_remark" id="txt_remark"/></td>
                        <td>Advanced Booking</td>
                    	<td> <input class="text_boxes" type="text" style="width:120px" onDblClick="openmypage_adbooking('requires/partial_fabric_booking_controller.php?action=advanced_booking_popup','Advanced Booking Search')" readonly placeholder="Double Click for Booking"  name="txt_booking_num" id="txt_booking_num"/></td>
                    </tr>
                    <tr>
                    	<td>Delivery Address</td>
                    	<td colspan="3">
                    		<textarea id="delivery_address" class="text_area" style="width:372px; height:40px;" placeholder="Delivery Address" title="Allowed Characters: abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890.-,%@!/<>?+[]{};: "></textarea>
                    	</td>
                        <td>Test Requirement</td>
                    	<td colspan="3">
                    		<textarea id="txt_test_req" name="txt_test_req" class="text_area" style="width:372px; height:40px;" placeholder="Test Requirement" title="Allowed Characters: abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890.-,%@!/<>?+[]{};: "></textarea>
                    	</td>
                    </tr>
                    <tr>
                        <td>
                        <input type="button" id="set_button2" class="image_uploader" style="width:110px;" value="Process Loss %" onClick="open_rmg_process_loss_popup('requires/partial_fabric_booking_controller.php?action=rmg_process_loss_popup','Process Loss %')" />
                        ...
                        <td>
                        	<input type="button" id="set_button3" class="image_uploader" style="width:110px;" value="Collar & Cuff" onClick="open_colur_cuff_popup('requires/partial_fabric_booking_controller.php?action=colur_cuff_popup&permissions=<? echo $permission?>','Collar & Cuff')" />
                        </td>
						<td>
						<input type="button" class="image_uploader" style="width:70px" value="ADD FILE" onClick="file_uploader ( '../../', document.getElementById('update_id').value,'', 'partial_fabric_booking', 2 ,1)">
						</td>
                    </tr>
                    <tr>
            ...
            <div id="booking_list_view"><font id="save_sms" style="color:#F00">Select New Item</font></div>
              <? echo load_submit_buttons( $permission, "fnc_fabric_booking_dtls", 0,0 ,"fnResetFormDtls();",2) ; ?>
                ...
                <input type="button" value="AAL Print" onClick="generate_fabric_report('show_fabric_booking_report_advance_attire_ltd')"  style="width:100px; display:none;" name="print_booking_aal" id="print_4" class="formbutton" /><a   id="aal_report4" href="" style="text-decoration:none" download hidden>BB</a>
                  ...
                  <input type="button" value="Fabric Booking Report" onClick="generate_fabric_report('fabric_booking_report',1)"  style="width:150px;display:none;" name="fabric_booking_report" id="fabric_booking_report" class="formbutton" />

				  <input type="button" value="Print 7" onClick="generate_fabric_report('print_booking_7',1)"  style="width:100px;display:none;"name="print_7" id="print_7" class="formbutton" />
                 ...
                 <input type="button" value="Print 9" onClick="generate_fabric_report('print_9',1)"  style="width:80px;display:none;" name="print_9" id="print_9" class="formbutton" />

				   <input class="formbutton" type="button" onClick="fnSendMail('../../','auto_mail/partial_fabric_booking_auto_mail','txt_booking_no',1,1,0)" value="Mail Send" style="width:80px;">
                        </td>
...
function fnc_process_data(){
	if (form_validation('cbo_company_name*txt_booking_no','Company*Booking No')==false){
		...
		page_link=page_link+'&garments_nature='+garments_nature+'&cbo_booking_month='+cbo_booking_month+'&cbo_booking_year='+cbo_booking_year+'&cbo_company_name='+cbo_company_name+'&cbo_buyer_name='+cbo_buyer_name+'&cbo_currency='+cbo_currency+'&cbo_fabric_natu='+cbo_fabric_natu+'&cbouom='+cbouom+'&cbo_fabric_source='+cbo_fabric_source+'&cbo_supplier='+cbo_supplier+'&cbo_source='+cbo_source;
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1063px,height=450px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function(){
			var theform=this.contentDoc.forms[0];
...
function call_print_button_for_mail(mail_id){
	get_php_form_data( document.getElementById('cbo_company_name').value+'**'+mail_id, "get_first_selected_print_report", "requires/partial_fabric_booking_controller" );
	}
</script>
...
load_drop_down( 'requires/partial_fabric_booking_controller', document.getElementById('cbo_pay_mode').value, 'load_drop_down_suplier', 'sup_td' )
});
