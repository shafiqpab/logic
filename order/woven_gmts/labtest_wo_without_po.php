...
<script>
	...
	function fnc_labtest_wo( operation )
	{
		...
		{
			var data="action=save_update_delete_mst&operation="+operation+get_submitted_data_string('cbo_company_name*txt_workorder_no*cbo_supplier*txt_workorder_date*cbo_currency*txt_exchange_rate*txt_delivery_date*cbo_pay_mode*cbo_ready_to_approved*txt_attention*txt_address*txt_vat_per*update_id',"../../");
	...
	function fnc_labtest_wo_dtls( operation )
	{
		...
		data_all=get_submitted_data_string('cbo_test_for*txt_color*txt_test_item*txt_amount*txt_delivery_charge*txt_discount*txt_wo_value*txt_party_type_id*txt_party_type_name*txt_vat_amount*txt_wo_value_with_vat*save_qty_break_data*txt_remarks*cbo_pay_mode*txt_workorder_no*update_id*update_dtls_id',"../../");
...
<? echo load_freeze_divs ("../../",$permission);  ?>
    <form name="printbooking_1"  autocomplete="off" id="printbooking_1">
        ...
        <legend>Lab Test Work Order Without Order</legend>
            <table width="820" cellspacing="2" cellpadding="0" border="0">
                <tr>
                    <td align="right" colspan="3">Wo No</td>              
                    <td colspan="3"><input class="text_boxes" type="text" style="width:140px" onDblClick="openmypage_booking()" readonly placeholder="Double Click for Booking" name="txt_workorder_no" id="txt_workorder_no"/>
                ...
                <tr>
                    ...
                    <td width="150"><? echo create_drop_down( "cbo_company_name", 150, "select id,company_name from lib_company where status_active=1 and is_deleted=0 order by company_name", "id,company_name",1, "-- Select Company --", $selected, "","","" ); ?>	  
                    </td>
                    <td width="120" class="must_entry_caption">Test Company</td>
                    <td width="150"><? echo create_drop_down( "cbo_supplier", 150, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=26 order by a.supplier_name","id,supplier_name", 1, "-- Select Test Company--", 0, "","" ); ?>	  
                    </td>
                    <td width="120" class="must_entry_caption">WO Date</td>   
                    <td><input class="datepicker" type="text" style="width:140px" name="txt_workorder_date" id="txt_workorder_date" onChange="set_conversion_rate($('#cbo_currency').val(), this.value,'../../', 'txt_exchange_rate');" value="<? echo date("d-m-Y")?>" disabled />	
                    </td>
                </tr>
                <tr>
                    <td class="must_entry_caption">Currency</td>   
                    <td><? echo create_drop_down( "cbo_currency", 150, $currency,"", 1, "-- Select --",'', "set_conversion_rate(this.value,$('#txt_workorder_date').val(), '../../', 'txt_exchange_rate')",0 ); ?></td>
                    <td>Exchange Rate</td>
                    <td><input style="width:140px;" type="text" class="text_boxes_numeric" name="txt_exchange_rate" id="txt_exchange_rate" onChange="check_exchange_rate();"  readonly/>
                    </td>
                    <td>Delivery Date</td>
                ...
                <tr>
                    <td>Pay Mode</td>
                    <td><? echo create_drop_down( "cbo_pay_mode", 150, $pay_mode,"", 1, "-- Select Pay Mode --", 1, "","" ); ?></td>
                    <td>Attention</td>
                    <td><input class="text_boxes" type="text" style="width:140px;"  name="txt_attention" id="txt_attention" /></td>
                    <td>Ready To Approved</td>  
                    <td><? echo create_drop_down( "cbo_ready_to_approved", 150, $yes_no,"", 0, "", 2, "","","" ); ?></td>
                </tr>
                <tr>
                    <td class="must_entry_caption">Vat %</td>
                    <td><input class="text_boxes_numeric" type="text" style="width:140px;"  name="txt_vat_per" id="txt_vat_per" /></td>
                    <td>Address</td>
                    <td colspan="3"><input class="text_boxes" type="text" style="width:400px;"  name="txt_address" id="txt_address" /></td>
                </tr>
                <tr>
    ...
    <form id="printbooking_2" name="printbooking_2" autocomplete="off">
        ...
        <legend>Details</legend>
            <table class="rpt_table" width="880" cellspacing="0" cellpadding="0" border="1" rules="all">
                <thead>
                    <tr>
