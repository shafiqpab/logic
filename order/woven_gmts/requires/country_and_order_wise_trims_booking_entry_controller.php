...
Version                  :  V1
Converted by             :  
Purpose			         :  This form will create Country and Order Wise Trims Booking For Woven
Functionality	         :
JS Functions	         :
Created by		         :  md mamun ahmed sagor
Creation date 	         :  26-09-2022
Requirment Client        :  Windy Group
Requirment By            :
Requirment type          :
Requirment               :
Affected page            :
...
{
	if($data==5 || $data==3){
	   echo create_drop_down( "cbo_supplier_name", 172, "select id,company_name from lib_company where status_active=1 and is_deleted=0 order by company_name", "id,company_name",1, "-- Select Company --", "", "get_php_form_data( this.value, 'load_drop_down_attention', 'requires/country_and_order_wise_trims_booking_entry_controller');",0,"" );
	}
	else
	{
	echo create_drop_down( "cbo_supplier_name", 172, "select c.supplier_name,c.id from lib_supplier_tag_company a, lib_supplier_party_type b, lib_supplier c where c.id=b.supplier_id and a.supplier_id = b.supplier_id  and b.party_type=4 and c.status_active=1 and c.is_deleted=0 group by c.id, c.supplier_name order by c.supplier_name","id,supplier_name", 1, "--Select Supplier--",$selected,"get_php_form_data( this.value, 'load_drop_down_attention', 'requires/country_and_order_wise_trims_booking_entry_controller');","");
	}
	exit();
}
...
function populate_variable_setting_data($data)
{
	...
	foreach ($data_array as $row)
	{
		...
		echo "document.getElementById('qty_exceed_level').value = '".$row[csf("exceed_qty_level")]."';\n";
	}
	$buyer_dropdown=load_drop_down_buyer($data);
	echo "document.getElementById('buyer_td').innerHTML = '".$buyer_dropdown."';\n";
	$print_report_format=return_field_value("format_id","lib_report_template","template_name ='".$data."' and module_id=2 and report_id in(6) and is_deleted=0 and status_active=1");
	echo "document.getElementById('report_ids').value = '';\n";
...
function load_drop_down_trim_precost_id($data) 
{
	...
	$trim_group= return_library_array("select id, item_name from lib_item_group",'id','item_name');
     $variable_data=sql_select("select excut_source, item_category_id, id from  variable_order_tracking where company_name=$data[2] and variable_list=72 and excut_source in (2,4) and item_category_id=4 and status_active= 1 order by id");
	 
	 $sql_vari_lib="select item_category_id,variable_list, excut_source  from variable_order_tracking where company_name=".$data[2]." and item_category_id=4  and variable_list=72 and status_active=1"; 
	...
	$trim_group_array=array();$sql_suppRes=array(); $trimsId="";$supId=array();
    if(count($variable_data)>0){

        $sqldata=sql_select("select a.id,a.sourcing_nominated_supp || ',' || a.nominated_supp_multi || ',' || a.nominated_supp as supp_multi from wo_pre_cost_trim_cost_dtls a where a.job_no='$data[0]'  and a.is_deleted=0  and a.status_active=1");
	//	echo "select a.id,a.sourcing_nominated_supp || ',' || a.nominated_supp_multi || ',' || a.nominated_supp as supp_multi from wo_pre_cost_trim_cost_dtls a where a.job_no='$data[0]'  and a.is_deleted=0  and a.status_active=1";
      
        foreach($sqldata as $row)
        {
            
            $supId=explode(",",$row[csf('supp_multi')]);
            foreach($supId as $sid){
                if($data[1]==$sid){
                    $trimIdArr[$row[csf('id')]]=$row[csf('id')];
                }
            }
           
        }

        $trimsId=implode(",",$trimIdArr);
	//	echo $trimsId.'=';
		if($trimsId!="")
		{
			$trimscond="and a.id in ($trimsId)";
		}
		else  $trimscond="";
	  
    }
    if($data[1]!= 0 && $data[1] !='') $nominated_sup_cond=" and (supplier_id='$data[1]')"; else $nominated_sup_cond="";
     $sql_supp="select trimid from wo_pre_cost_trim_supplier where job_no='$data[0]' $nominated_sup_cond and is_deleted=0 and status_active=1";	
	...
	//$con = connect();
	//echo $db_type.'XCV';
	if($db_type==2) //db_type Not work it
	{
		//echo "A";
		if($source_from==2)
		{
			if($trim_ids!="") $trim_idCond="and (a.id in ($trim_ids) or a.sourcing_nominated_supp is null)"; else $trim_idCond=" and (a.sourcing_nominated_supp is null or sourcing_nominated_supp='0')";
		}
		else
		{
		if($trim_ids!="") $trim_idCond="and (a.id in ($trim_ids) or a.nominated_supp_multi is null)"; else $trim_idCond=" and (a.nominated_supp_multi is null or nominated_supp_multi='0')";
		}
	}
	else
	{
		//echo "B";
		if($trim_ids!="") $trim_idCond="and (id in ($trim_ids) or nominated_supp_multi='')"; else $trim_idCond=" and nominated_supp_multi='' or nominated_supp_multi='0'";
	}
	 //echo "select a.id,a.trim_group,a.cons_dzn_gmts,a.description from wo_pre_cost_trim_cost_dtls a where a.job_no='$data[0]' $trim_idCond  and a.is_deleted=0  and a.status_active=1";
	$sql_data=sql_select("select a.id,a.trim_group,a.cons_dzn_gmts,a.description from wo_pre_cost_trim_cost_dtls a where a.job_no='$data[0]' $trim_idCond $trimscond and a.is_deleted=0  and a.status_active=1");
	//echo "select a.id,a.trim_group,a.cons_dzn_gmts,a.description from wo_pre_cost_trim_cost_dtls a where a.job_no='$data[0]' $trim_idCond $trimscond and a.is_deleted=0  and a.status_active=1";
	foreach($sql_data as $row)
	{
		// $trim_group_array[$row[csf('id')]]=$trim_group[$row[csf('trim_group')]]." (".$row[csf('description')]." )"." (".$row[csf('cons_dzn_gmts')]." )";
        $trim_group_array[$row[csf('id')]]=$trim_group[$row[csf('trim_group')]];
	}
...
<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
        <table width="1160" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
            <thead>
                <tr>
                    <th colspan="10"><? echo create_drop_down( "cbo_string_search_type", 100, $string_search_type,'', 1, "-- Searching Type --" ); ?></th>
                </tr>
                <tr>
                    ...
                    <th><input type="hidden" value="0"  id="chk_job_wo_po"><!--Job Without PO onClick="set_checkvalue()"--></th>
                </tr>
            </thead>
            <tr class="general">
                <td>
                ...
                <input type="hidden" id="garments_nature" value="<? echo $garments_nature; ?>">
                    <? echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", $cbo_company_name,"load_drop_down( 'country_and_order_wise_trims_booking_entry_controller', this.value+'_'+document.getElementById('buyer_id').value, 'load_drop_down_buyer_pop', 'buyer_td' );",1,"" ); ?>
            </td>
            <td id="buyer_td"><? echo create_drop_down( "cbo_buyer_name", 172, $buyer_arr,'', 1, "-- Select Buyer --", $cbo_buyer_name,"",1); ?>	</td>
             <td><input name="txt_file" id="txt_file" class="text_boxes" style="width:70px"></td>
             <td><input name="txt_ref" id="txt_ref" class="text_boxes" style="width:70px"></td>
...
if($action=="create_po_search_list_view")
{
	...
	$job_cond=""; $order_cond=""; $style_cond=""; $file_cond=""; $ref_cond="";
	if($data[10]==1)
	{
	...
	echo "<div style='width:1000px;font-size:16px'; align='center'><font color='#FF0000'>Only Pre Cost V2 Woven Job Allowed.</font></div>";

	if (str_replace("'","",$data[5])!=0) $gmt_nature_cond=" and a.garments_nature=$data[5]";else $gmt_nature_cond="";

	if ($data[2]==0)
	{
		if($db_type==0)
		{
			$sql= "select a.job_no_prefix_num, a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity, b.po_number,b.file_no,b.grouping,b.po_quantity,b.shipment_date,a.garments_nature,SUBSTRING_INDEX(a.insert_date, '-', 1) as year,b.id,c.approved from wo_po_details_master a, wo_po_break_down b,wo_pre_cost_mst c where a.job_no=b.job_no_mst and a.job_no=c.job_no and c.approved=1  and b.shiping_status not in(3)  and c.entry_from=425 and a.status_active=1 and b.status_active=1 $shipment_date $company $buyer $job_cond $file_cond $ref_cond $order_cond $style_cond $gmt_nature_cond order  by a.job_no";
		}
		if($db_type==2)
		{
			 $sql= "select a.job_no_prefix_num, a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity, b.po_number,b.file_no,b.grouping,b.po_quantity,b.shipment_date,a.garments_nature,to_char(a.insert_date,'YYYY') as year,b.id,c.approved from wo_po_details_master a, wo_po_break_down b,wo_pre_cost_mst c where a.job_no=b.job_no_mst and a.job_no=c.job_no and c.entry_from=425 and c.approved=1   and b.shiping_status not in(3) and a.status_active=1 and b.status_active=1 $shipment_date $company $buyer $job_cond $file_cond $ref_cond $order_cond $style_cond $gmt_nature_cond order by a.job_no";
		}
		//echo $sql;
	...
	else
	{
		...
		if($db_type==0)
		{
			$sql= "select a.job_no_prefix_num,a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.garments_nature,SUBSTRING_INDEX(a.`insert_date`, '-', 1) as year from wo_po_details_master a,wo_pre_cost_mst c where  a.job_no=c.job_no and c.entry_from=425 and a.job_no not in( select distinct job_no_mst from wo_po_break_down where status_active=1 and is_deleted=0 ) and a.status_active=1   and a.is_deleted=0 $gmt_nature_cond $company $buyer order by a.job_no";
		}
		if($db_type==2)
		{
...
/*if ($action=="populate_data_from_search_popup")
{
	...
	foreach ($data_array as $row)
	{
		echo "load_drop_down( 'requires/country_and_order_wise_trims_booking_entry_controller', '".$row[csf("company_name")]."', 'load_drop_down_buyer', 'buyer_td' );\n";
...
function set_precost_data($data)
{
	$data=explode("_",$data);
	 ...
	 {
		 $txt_country_cond ="and c.country_id in ($data[2])";
	 }
	 ...
	 // With Gmt Item
	  ...
	  $sql_data=sql_select("select a.id,a.job_no,a.trim_group,a.cons_dzn_gmts,a.rate,a.description,a.brand_sup_ref, b.cons,a.country from wo_pre_cost_trim_cost_dtls a , wo_pre_cost_trim_co_cons_dtls b where a.id=b.wo_pre_cost_trim_cost_dtls_id and  a.id='$data[0]' and b.po_break_down_id='$data[1]' and a.is_deleted=0  and a.status_active=1"); //b.country_id
	foreach($sql_data as $row)
	{

		if($row[csf('country')]==0)
		{
			 $txt_country_cond1="";
		}
		else
		{
			 $txt_country_cond1 ="and c.country_id in (".$row[csf('country_id')].")";
		}
		...
		if($currency_id_pre_cost==$currency_id_this_booking)
		{
		 $exchange_rate=1;
		}
		...
		if($data[5]==1)
		{
			if($row[csf('country')] !=0)
			{
				$country_array=explode(",",$row[csf('country')]);
			...
			$req_amount_cons_uom=def_number_format($req_qnty_cons_uom*$row[csf("rate")],8,"");
			...
			$txt_avg_price=($row[csf("rate")]*$conversion_factor)*$exchange_rate;
		...
		if($data[5]==2)
		{
			...
			$req_amount_cons_uom=def_number_format($req_qnty_cons_uom*$row[csf("rate")],8,"");
			$req_qnty=def_number_format(($row[csf('cons')]*($po_qty/$costing_per_qty))/$conversion_factor,5,"");
			$txt_avg_price=def_number_format(($row[csf("rate")]*$conversion_factor)*$exchange_rate,8,"");
			echo "document.getElementById('txt_trim_group_id').value = '".$row[csf("trim_group")]."';\n";
			echo "$('#txt_req_quantity').attr('req_qnty_cons_uom', ".$req_qnty_cons_uom.");\n";
			echo "$('#txt_req_quantity').attr('conversion_factor', ".$conversion_factor.");\n";
			echo "$('#txt_req_quantity').attr('req_amount_cons_uom', ".$req_amount_cons_uom.");\n";
...
if($action == "check_previous_booking")
{
	
	return;
	$data=explode("_",$data);
	$po_id=$data[0];
	$item_number_id=$data[1];
	//echo "10**".$po_id.'--'.$item_number_id; die;
	$sql_data=sql_select("select booking_no,sensitivity from wo_booking_dtls  where po_break_down_id='$data[0]' and pre_cost_fabric_cost_dtls_id =  $data[1] and status_active=1 and status_active=1 group by booking_no,sensitivity");
	if(count($sql_data)>0){
		foreach ($sql_data as $row) {
			echo "document.getElementById('cbo_colorsizesensitive').value = '".$row[csf('sensitivity')]."';\n";
			echo " $('#cbo_colorsizesensitive').attr('disabled',false);\n";
			//echo "fnc_generate_booking();\n";
		}
	}
	else
	{
		echo "document.getElementById('cbo_colorsizesensitive').value = '0';\n";
		echo " $('#cbo_colorsizesensitive').attr('disabled',false);\n";
	}
...
if ($action=="set_precost_data")
{
	...
	$data=explode("_",$data);
	$trim_cost_dtls_id=$data[0];
	$po_id=$data[1];
	$country_id_string=$data[2];
	$selected_country_array=explode(',',$data[2]);
	$item_number_id=$data[3];
	$currency_id_this_booking=$data[4];
	$is_short_id=$data[7];
	$company_id=$data[8];
	//echo "10**". $company_id; die;
	...
	if($data[6]==1){
		$sql_data=sql_select("SELECT a.id,a.job_no,a.trim_group,a.cons_dzn_gmts,a.rate,a.description,a.brand_sup_ref, b.cons,a.country from wo_pre_cost_trim_cost_dtls a , wo_pre_cost_trim_co_cons_dtls b where a.id=b.wo_pre_cost_trim_cost_dtls_id and  a.id='$data[0]' and b.po_break_down_id='$data[1]' and a.is_deleted=0  and a.status_active=1 group by a.id,a.job_no,a.trim_group,a.cons_dzn_gmts,a.rate,a.description,a.brand_sup_ref, b.cons,a.country");
		foreach($sql_data as $row){
			if($row[csf('country')] !=0){
				$country_array[]=$row[csf('country')];
			}
			else{
				$country_array=explode(',',$data[2]);
			}
			$trim_group=$row[csf("trim_group")];
			$description=$row[csf("description")];
			$brand_sup_ref=$row[csf("brand_sup_ref")];
			$job_no=$row[csf("job_no")];
		}
		

		$country_array=array_unique(explode(",",implode(",",$country_array)));
		$result=array_diff($selected_country_array,$country_array);
		if(count($result)>0){
			$join_result="";
			for($i=0;$i<count($result);$i++){
       ...
       $all_booking_data=sql_select("SELECT a.trim_group, b.id, b.wo_trim_booking_dtls_id, b.description, b.brand_supplier, b.item_color, b.item_size, b.cons, b.process_loss_percent, b.requirment, b.rate, 
b.amount, b.pcs, b.color_size_table_id, b.country_id, b.po_break_down_id,a.gmt_item from wo_booking_dtls a join wo_trim_book_con_dtls b on a.id = b.wo_trim_booking_dtls_id where a.po_break_down_id=$po_id and a.trim_group = $trim_group and a.pre_cost_fabric_cost_dtls_id=$trim_cost_dtls_id and a.gmt_item = $item_number_id and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 ");
       foreach ($all_booking_data as $row) {
       		$cu_woq[$row[csf('po_break_down_id')]][$row[csf('country_id')]][$row[csf('gmt_item')]][$trim_cost_dtls_id] +=$row[csf('cons')];
       }
	  // echo $source_from.'d';die;
		...
		$tottrimQtyArray=$trims->getQtyArray_by_precostdtlsid();
		// print_r($tottrimQtyArray);die;
		if($source_from==2) //Lib Booking Rate
		{
			//echo $source_from;die;
			$tottrimAmountArray=$trims->getAmountArray_precostdtlsidSourcing();
		 // print_r($tottrimAmountArray);
		}
		else
		{
			$tottrimAmountArray=$trims->getAmountArray_precostdtlsid();
		}
		...
		if(str_replace("'","",$country_id_string) !=''){
			$condition->country_id(" in($country_id_string)");
		}
		//$condition->init();

		//$trims= new trims($condition);
		$trimQtyArray=$trims->getQtyArray_by_orderCountryGmtsitemAndPrecostdtlsid();
		/*echo '<pre>';
		print_r($trimQtyArray); die;*/
		if($source_from==2) //Lib Booking Rate
		{
			$trimAmountArray=$trims->getAmountArray_by_orderCountryGmtsitemAndPrecostdtlsidSourcing();
		}
		else
		{
			$trimAmountArray=$trims->getAmountArray_by_orderCountryGmtsitemAndPrecostdtlsid();

		}

		$cons_uom=return_field_value("order_uom", "lib_item_group", "id=".$trim_group."");
		$conversion_factor=return_field_value("conversion_factor", "lib_item_group", "id=".$trim_group."");
		$exchange_rate=return_field_value("exchange_rate", "wo_pre_cost_mst", "job_no='".$job_no."'");
		$currency_id_pre_cost=return_field_value("currency_id", "wo_po_details_master", "job_no='".$job_no."'");
		...
		$req_qnty_cons_uom=0;
		$req_amount_cons_uom=0;
		for($i=0;$i<count($selected_country_array);$i++){
			$req_qnty_cons_uom+=$trimQtyArray[$po_id][$selected_country_array[$i]][$item_number_id][$trim_cost_dtls_id];
			$req_amount_cons_uom+=$trimAmountArray[$po_id][$selected_country_array[$i]][$item_number_id][$trim_cost_dtls_id];
			$booking_qty += $cu_woq[$po_id][$selected_country_array[$i]][$item_number_id][$trim_cost_dtls_id];
		}
		//echo $booking_qty; die;
		$rate=$req_amount_cons_uom/$req_qnty_cons_uom;
		$req_qnty=def_number_format($req_qnty_cons_uom/$conversion_factor,5,"");
		//$txt_avg_price=def_number_format(($rate*$conversion_factor)*$exchange_rate,5,"");
		$txt_avg_price=($rate*$conversion_factor)*$exchange_rate;
		if($is_short_id==1) //Short
		{
			$bal_woq = $req_qnty_cons_uom/$conversion_factor;
		}
		else
		{
			$bal_woq = $req_qnty_cons_uom/$conversion_factor-$booking_qty;
		}
		
		$balwoq = def_number_format($bal_woq,5,"");
		//echo $balwoq; die;

		if($data[5]==1){
			echo "document.getElementById('txt_trim_group_id').value = '".$trim_group."';\n";
			echo "document.getElementById('cbo_uom').value = '".$cons_uom."';\n";
			echo "document.getElementById('txt_req_quantity').value = '".$req_qnty."';\n";
			echo "document.getElementById('txt_bal_woq').value = '".$balwoq."';\n";
			echo "document.getElementById('txt_avg_price').value = '".$txt_avg_price."';\n";
			echo "document.getElementById('txt_pre_des').value = '".$description."';\n";
			echo "document.getElementById('txt_pre_brand_sup').value = '".$brand_sup_ref."';\n";
			echo "document.getElementById('txt_exchange_rate_dtls').value = '".$exchange_rate."';\n";

			echo "$('#txt_req_quantity').attr('req_qnty_cons_uom', ".$tottrimQtyArray[$trim_cost_dtls_id].");\n";
			echo "$('#txt_req_quantity').attr('conversion_factor', ".$conversion_factor.");\n";
			echo "$('#txt_req_quantity').attr('req_amount_cons_uom', ".$tottrimAmountArray[$trim_cost_dtls_id].");\n";
		}
		if($data[5]==2){
			echo "$('#txt_req_quantity').attr('req_qnty_cons_uom', ".$tottrimQtyArray[$trim_cost_dtls_id].");\n";
			echo "$('#txt_req_quantity').attr('conversion_factor', ".$conversion_factor.");\n";
...
function set_delivery_date_from_tna($poid, $save_update_mode)
{
	$task_finish_date='';
	$tnasql=sql_select("select task_finish_date from tna_process_mst where task_number=71 and po_number_id=$poid and is_deleted= 0 and status_active=1");
...
if($action=="set_delivery_date_from_tna"){
	    $data=explode("_",$data);
		set_delivery_date_from_tna($data[0], $data[1]);
}
...
if($action=="booking_qnty_and_amount_trim_group")
{
   ...
   $woqty="";
   $amount="";
   $data_sql=sql_select("select sum(b.wo_qnty) as wo_qnty, sum(b.amount) as amount from wo_booking_dtls b,wo_booking_mst a where a.booking_no=b.booking_no and b.po_break_down_id=$txt_po_id and b.trim_group=$txt_trim_group_id and b.pre_cost_fabric_cost_dtls_id=$cbo_trim_precost_id and b.booking_type=2 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0");
...
if ($action=="generate_fabric_booking"){
	...
	extract($_REQUEST);
	 ...
	 $cbo_colorsizesensitive=str_replace("'","",$cbo_colorsizesensitive);
	 $txt_trim_group_id=str_replace("'","",$txt_trim_group_id);
	 $isshort_id=str_replace("'","",$cbo_isshort);
	?>
	...
	//echo $source_from.'Dx';
            ...
            ?>
            	...
            	<table width="1050" cellspacing="0" class="rpt_table" border="0" id="tbl_consmption_cost" rules="all">
                	<thead>
                         <tr>
                            ...
                            <input type="hidden" id="po_qty1" name="po_qty1" value="<? echo $po_qty; ?>"/>
                           </th>
                        </tr>
                    	<tr>
                        	<th width="40">SL</th><th  width="100">Country</th><th  width="100">Gmts. Color</th><th  width="70">Gmts. Size</th><th  width="100">Description</th><th  width="100">Brand/Sup Ref</th><th  width="100">Item Color</th><th width="80">Item Size</th><th width="70"> Req Qty</th><th width="70"> WO Qty</th><th width="40">Excess %</th><th width="70">WO Qty.</th><th width="40">Rate</th><th width="50">Amount</th><th width="50">RMG Qnty</th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
                    <?
					$gmt_item_id=str_replace("'","",$cbo_gmt_item_id);
					$update_dtls_id=str_replace("'","",$txt_update_dtls_id);
					$booking_data_arr=array(); $all_booking_data_arr =array();

					if($update_dtls_id !='')
					{
						$all_booking_data=sql_select("SELECT a.trim_group,a.gmt_item, b.id, b.wo_trim_booking_dtls_id, b.description, b.brand_supplier, b.color_number_id, b.item_size, b.cons, b.process_loss_percent, b.requirment, b.rate, b.amount, b.pcs, b.color_size_table_id, b.country_id, b.po_break_down_id, a.pre_cost_fabric_cost_dtls_id  from wo_booking_dtls a join wo_trim_book_con_dtls b on a.id = b.wo_trim_booking_dtls_id where a.trim_group = $txt_trim_group_id and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.id <> $update_dtls_id ");
					}
					else{
						$all_booking_data=sql_select("SELECT a.trim_group,a.gmt_item, b.id, b.wo_trim_booking_dtls_id, b.description, b.brand_supplier, b.color_number_id, b.item_size, b.cons, b.process_loss_percent, b.requirment, b.rate, b.amount, b.pcs, b.color_size_table_id, b.country_id, b.po_break_down_id ,a.pre_cost_fabric_cost_dtls_id from wo_booking_dtls a join wo_trim_book_con_dtls b on a.id = b.wo_trim_booking_dtls_id where a.trim_group = $txt_trim_group_id and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 ");	
					}
					...
					foreach($all_booking_data as $row)
					{
						$gmt_item=$row[csf('gmt_item')];
						if($cbo_colorsizesensitive ==1 || $cbo_colorsizesensitive ==3)
						{
							//$key =$row[csf('po_break_down_id')].'*'.$row[csf('country_id')].'*'.$row[csf('trim_group')].'*'.$row[csf('item_color')].'*'.$gmt_item.'*'.$row[csf('description')];
							$key =$row[csf('po_break_down_id')].'*'.$row[csf('country_id')].'*'.$row[csf('trim_group')].'*'.$row[csf('color_number_id')].'*'.$gmt_item.'*'.$row[csf('pre_cost_fabric_cost_dtls_id')];
						}
						else if($cbo_colorsizesensitive ==2)
						{
							//$key =$row[csf('po_break_down_id')].'*'.$row[csf('country_id')].'*'.$row[csf('trim_group')].'*'.$row[csf('item_size')].'*'.$gmt_item.'*'.$row[csf('description')];
							$key =$row[csf('po_break_down_id')].'*'.$row[csf('country_id')].'*'.$row[csf('trim_group')].'*'.$row[csf('item_size')].'*'.$gmt_item.'*'.$row[csf('pre_cost_fabric_cost_dtls_id')];
						}
						else if($cbo_colorsizesensitive ==4)
						{
							//$key =$row[csf('po_break_down_id')].'*'.$row[csf('country_id')].'*'.$row[csf('trim_group')].'*'.$row[csf('item_color')].'*'.$row[csf('item_size')].'*'.$gmt_item.'*'.$row[csf('description')];
							$key =$row[csf('po_break_down_id')].'*'.$row[csf('country_id')].'*'.$row[csf('trim_group')].'*'.$row[csf('color_number_id')].'*'.$row[csf('item_size')].'*'.$gmt_item.'*'.$row[csf('pre_cost_fabric_cost_dtls_id')];
						}
						else
						{
							//$key =$row[csf('po_break_down_id')].'*'.$row[csf('country_id')].'*'.$row[csf('trim_group')].'*'.$gmt_item.'*'.$row[csf('description')];
							$key =$row[csf('po_break_down_id')].'*'.$row[csf('country_id')].'*'.$row[csf('trim_group')].'*'.$gmt_item.'*'.$row[csf('pre_cost_fabric_cost_dtls_id')];
						}
						$all_booking_data_arr[$key][cons]+=$row[csf('cons')];
					}
					$conversion_factor=return_field_value("conversion_factor", "lib_item_group", "id=".$txt_trim_group_id."");

					$booking_data=sql_select("SELECT a.trim_group,a.gmt_item, b.id, b.wo_trim_booking_dtls_id, b.description, b.brand_supplier, b.color_number_id, b.item_size, b.cons, b.process_loss_percent, b.requirment, b.rate, b.amount, b.pcs, b.color_size_table_id, b.country_id, b.po_break_down_id, a.pre_cost_fabric_cost_dtls_id  from wo_booking_dtls a join wo_trim_book_con_dtls b on a.id = b.wo_trim_booking_dtls_id where b.wo_trim_booking_dtls_id='$txt_update_dtls_id' and a.trim_group = $txt_trim_group_id and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0");
					...
					foreach($booking_data as $row)
					{
						...
						$booking_data_arr[$key][id]=$row[csf('id')];
						$booking_data_arr[$key][description]=$row[csf('description')];
						$booking_data_arr[$key][brand_supplier]=$row[csf('brand_supplier')];
						$booking_data_arr[$key][item_color]=$row[csf('color_number_id')];
						$booking_data_arr[$key][item_size]=$row[csf('item_size')];
						$booking_data_arr[$key][cons]=$row[csf('cons')];
						$booking_data_arr[$key][process_loss_percent]=$row[csf('process_loss_percent')];
						$booking_data_arr[$key][requirment]=$row[csf('requirment')];
						$booking_data_arr[$key][rate]=$row[csf('rate')];
						$booking_data_arr[$key][amount]=$row[csf('amount')];
					}
					...
					if(str_replace("'","",$cbo_gmt_item_id) !='')
					{
						$condition->item_number_id("=$cbo_gmt_item_id");
					}
					...
					{
						$trimQtyArray=$trims->getQtyArray_by_orderCountryPrecostdtlsidAndGmtssize();
						if($source_from==2) //Sourcing from Booking Rate
						{
						$trimAmtArray=$trims->getAmountArray_by_orderCountryPrecostdtlsidAndGmtssizeSourcing();
						}
						else
						{
						$trimAmtArray=$trims->getAmountArray_by_orderCountryPrecostdtlsidAndGmtssize();
						}
						$sql="SELECT b.id, b.po_number,b.po_quantity,min(c.id) as color_size_table_id,c.size_number_id,min(e.item_size) as item_size,sum(c.order_quantity) as order_quantity ,(sum(c.order_quantity)/a.total_set_qnty) as order_quantity_set, c.country_id, d.id as trim_cost_dtls_id from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c,wo_pre_cost_trim_cost_dtls d, wo_pre_cost_trim_co_cons_dtls e where  a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and a.job_no=d.job_no and a.job_no=e.job_no and b.id=c.po_break_down_id and b.id=e.po_break_down_id  and d.id=e.wo_pre_cost_trim_cost_dtls_id and c.po_break_down_id=e.po_break_down_id and c.item_number_id= e.item_number_id and c.color_number_id=e.color_number_id and c.size_number_id=e.size_number_id  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and b.id='$txt_po_id' and c.item_number_id='$cbo_gmt_item_id' $txt_country_cond and d.id=$cbo_trim_precost_id group by  b.id, b.po_number,b.po_quantity,a.total_set_qnty,c.size_number_id,c.item_number_id, c.country_id , d.id order by b.id,c.size_number_id";
						//echo $sql; die;
					...
					else if($cbo_colorsizesensitive==3)
					{
						...
						$sql="SELECT b.id, b.po_number,b.po_quantity,min(c.id) as color_size_table_id,c.color_number_id,sum(c.order_quantity) as order_quantity ,(sum(c.order_quantity)/a.total_set_qnty) as order_quantity_set, c.country_id, d.id as trim_cost_dtls_id from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c,wo_pre_cost_trim_cost_dtls d, wo_pre_cost_trim_co_cons_dtls e where  a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and a.job_no=d.job_no and a.job_no=e.job_no and b.id=c.po_break_down_id and b.id=e.po_break_down_id  and d.id=e.wo_pre_cost_trim_cost_dtls_id and c.po_break_down_id=e.po_break_down_id and c.item_number_id= e.item_number_id and c.color_number_id=e.color_number_id and c.size_number_id=e.size_number_id  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and  b.id='$txt_po_id' and c.item_number_id='$cbo_gmt_item_id' $txt_country_cond and d.id=$cbo_trim_precost_id group by  b.id, b.po_number,b.po_quantity,a.total_set_qnty,c.color_number_id,c.item_number_id, c.country_id, d.id  order by c.country_id, c.color_number_id";
					 $gmt_size_edb="disabled";
					 $item_size_edb="disabled";
					}
					else if($cbo_colorsizesensitive==4)
					{
						$trimQtyArray=$trims->getQtyArray_by_OrderCountryPrecostdtlsidGmtscolorAndGmtssize();
						if($source_from==2) //Sourcing from Booking Rate
						{
						$trimAmtArray=$trims->getAmountArray_by_OrderCountryPrecostdtlsidGmtscolorAndGmtssizeSourcing();
						}
						else
						{
							$trimAmtArray=$trims->getAmountArray_by_OrderCountryPrecostdtlsidGmtscolorAndGmtssize();
						}
						$sql="SELECT b.id, b.po_number,b.po_quantity,min(c.id) as color_size_table_id,c.color_number_id,c.size_number_id,min(e.item_size) as item_size,sum(c.order_quantity) as order_quantity,(sum(c.order_quantity)/a.total_set_qnty) as order_quantity_set, c.country_id, d.id as trim_cost_dtls_id  from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c ,wo_pre_cost_trim_cost_dtls d, wo_pre_cost_trim_co_cons_dtls e where  a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and a.job_no=d.job_no and a.job_no=e.job_no and b.id=c.po_break_down_id and b.id=e.po_break_down_id  and d.id=e.wo_pre_cost_trim_cost_dtls_id and c.po_break_down_id=e.po_break_down_id and c.item_number_id= e.item_number_id and c.color_number_id=e.color_number_id and c.size_number_id=e.size_number_id  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and  b.id='$txt_po_id' and c.item_number_id='$cbo_gmt_item_id' $txt_country_cond and d.id=$cbo_trim_precost_id  group by  b.id, b.po_number,b.po_quantity,a.total_set_qnty,c.color_number_id,c.size_number_id,c.item_number_id, c.country_id,d.id  order by c.country_id, c.color_number_id,c.size_number_id";
					}
					else
					{
						$trimQtyArray=$trims->getQtyArray_by_precostdtlsidAndCountry();
						if($source_from==2) //Sourcing from Booking Rate
						{
						$trimAmtArray=$trims->getAmountArray_precostdtlsidAndCountrySourcing();
						//print_r($trimAmtArray);
						}
						else
						{
							$trimAmtArray=$trims->getAmountArray_precostdtlsidAndCountry();
						}
						 

						$sql="SELECT b.id, b.po_number,b.po_quantity,min(c.id) as color_size_table_id,sum(c.order_quantity) as order_quantity,(sum(c.order_quantity)/a.total_set_qnty) as order_quantity_set , c.country_id , d.id as trim_cost_dtls_id from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c,wo_pre_cost_trim_cost_dtls d, wo_pre_cost_trim_co_cons_dtls e where  a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and a.job_no=d.job_no and a.job_no=e.job_no and b.id=c.po_break_down_id and b.id=e.po_break_down_id  and d.id=e.wo_pre_cost_trim_cost_dtls_id and c.po_break_down_id=e.po_break_down_id and c.item_number_id= e.item_number_id and c.color_number_id=e.color_number_id and c.size_number_id=e.size_number_id  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and  b.id='$txt_po_id' and c.item_number_id='$cbo_gmt_item_id' $txt_country_cond and d.id=$cbo_trim_precost_id group by  b.id, b.po_number,b.po_quantity,a.total_set_qnty,c.item_number_id,c.country_id, d.id order by b.id";
					}
					...
					if ( count($data_array)>0)
					{
						...
						foreach( $data_array as $row ){
							if($cbo_colorsizesensitive ==1 || $cbo_colorsizesensitive ==3)
							{
								//$key =$row[csf('id')].'*'.$row[csf('country_id')].'*'.$txt_trim_group_id.'*'.$row[csf('color_number_id')].'*'.$gmt_item_id.'*'.$description;
								$key =$row[csf('id')].'*'.$row[csf('country_id')].'*'.$txt_trim_group_id.'*'.$row[csf('color_number_id')].'*'.$gmt_item_id.'*'.$row[csf('trim_cost_dtls_id')];
							}
							else if($cbo_colorsizesensitive ==2)
							{
								//$key =$row[csf('id')].'*'.$row[csf('country_id')].'*'.$txt_trim_group_id.'*'.$size_library[$row[csf('size_number_id')]].'*'.$gmt_item_id.'*'.$description;
								$key =$row[csf('id')].'*'.$row[csf('country_id')].'*'.$txt_trim_group_id.'*'.$size_library[$row[csf('size_number_id')]].'*'.$gmt_item_id.'*'.$row[csf('trim_cost_dtls_id')];
							}
							else if($cbo_colorsizesensitive ==4)
							{
								//$key =$row[csf('id')].'*'.$row[csf('country_id')].'*'.$txt_trim_group_id.'*'.$row[csf('color_number_id')].'*'.$size_library[$row[csf('size_number_id')]].'*'.$gmt_item_id.'*'.$description;
								$key =$row[csf('id')].'*'.$row[csf('country_id')].'*'.$txt_trim_group_id.'*'.$row[csf('color_number_id')].'*'.$size_library[$row[csf('size_number_id')]].'*'.$gmt_item_id.'*'.$row[csf('trim_cost_dtls_id')];
							}
							else
							{
								//$key =$row[csf('id')].'*'.$row[csf('country_id')].'*'.$txt_trim_group_id.'*'.$gmt_item_id.'*'.$description;
								$key =$row[csf('id')].'*'.$row[csf('country_id')].'*'.$txt_trim_group_id.'*'.$gmt_item_id.'*'.$row[csf('trim_cost_dtls_id')];
							}
							$description="";
							if(str_replace("'","",$txt_update_dtls_id) != ''){
								$description=$booking_data_arr[$key][description];
							}
							if($description==""){
								$description=$txt_pre_des;
							}
							$brand_supplier=$booking_data_arr[$key][brand_supplier];
							if($brand_supplier==""){
							...
							$item_color = $booking_data_arr[$key][item_color];
							if($item_color==0 || $item_color=="" ){
								$item_color = $row[csf('color_number_id')];
							}

							$item_size=$booking_data_arr[$key][item_size];
							if($item_size==0 || $item_size == ""){
							  $item_size=$row[csf('item_size')];
							}

							$rate=$booking_data_arr[$key][rate];
							if($rate==0 || $rate==""){
								$rate=$txt_avg_price;
							}
							//echo $rate.'=DDD';
							...
							$current_qty = $all_booking_data_arr[$key][cons];
							//echo "1990**".$current_qty; die;
							if($cbo_colorsizesensitive==1){

								$Req=$trimQtyArray[$txt_po_id][$row[csf('country_id')]][$cbo_trim_precost_id][$row[csf('color_number_id')]]/$conversion_factor;
								$Amt=$trimAmtArray[$txt_po_id][$row[csf('country_id')]][$cbo_trim_precost_id][$row[csf('color_number_id')]];
								//echo $Req.'='.$Amt.', ';
								if($rate != ''){
									//$rate=$txt_avg_price;
								}
							...
							if($cbo_colorsizesensitive==2){
								$Req=$trimQtyArray[$txt_po_id][$row[csf('country_id')]][$cbo_trim_precost_id][$row[csf('size_number_id')]]/$conversion_factor;
								$Amt=$trimAmtArray[$txt_po_id][$row[csf('country_id')]][$cbo_trim_precost_id][$row[csf('size_number_id')]];
								if($rate != ''){
									//$rate=$txt_avg_price;
								}
								else
							...
							if($cbo_colorsizesensitive==4){
								$Req=$trimQtyArray[$txt_po_id][$row[csf('country_id')]][$cbo_trim_precost_id][$row[csf('color_number_id')]][$row[csf('size_number_id')]]/$conversion_factor;
								$Amt=$trimAmtArray[$txt_po_id][$row[csf('country_id')]][$cbo_trim_precost_id][$row[csf('color_number_id')]][$row[csf('size_number_id')]];
								if($rate != ''){
									//$rate=$txt_avg_price;
								}
								else
							...
							if($cbo_colorsizesensitive==0){
								$Req = $trimQtyArray[$cbo_trim_precost_id][$row[csf('country_id')]]/$conversion_factor;
							}
							if($isshort_id==1)
							{
								$current_qty=0;
							}
							else $current_qty=$current_qty;
                    ...
                    <tr id="break_1" align="center">
                                    ...
                                    <td>
                                    <input type="text" id="pocountry_<? echo $i;?>"  name="pocountry_<? echo $i;?>" class="text_boxes" style="width:85px" value="<? echo $country_library[$row[csf('country_id')]]; ?>" disabled/>
                                    <input type="hidden" id="pocountryid_<? echo $i;?>"  name="pocountryid_<? echo $i;?>" class="text_boxes" style="width:85px" value="<? echo $row[csf('country_id')]; ?>" />
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                    	<input type="text" id="reqqty_<? echo $i;?>"  name="reqqty_<? echo $i;?>"    class="text_boxes" style="width:70px" value="<? echo number_format($Req-$current_qty,4,'.',''); ?>" disabled/>
                                    </td>
                                    <td>
                                    <input type="text" id="qty_<? echo $i;?>" onBlur="" onChange="set_sum_value( 'qty_sum', 'qty_' );set_sum_value( 'woqty_sum', 'woqny_' );calculate_requirement(<? echo $i;?>);copy_value(this.value,'qty_',<? echo $i;?>);validate_sum( <? echo $i; ?> );"  name="qty_<? echo $i;?>" class="text_boxes_numeric" style="width:70px"   placeholder="<? echo number_format($Req-$current_qty,4); ?>" value="<? if(str_replace("'","",$txt_update_dtls_id) != ''){ echo $booking_data_arr[$key][cons]; } else { } ?>"/>

                                    <input type="hidden" id="req_qty_<? echo $i;?>" name="req_qty_<? echo $i;?>" value="<? echo number_format($Req,4) ?>" />
                                    <input type="hidden" id="previous_<? echo $i;?>" name="previous_<? echo $i;?>" value="<? echo number_format($current_qty,4) ?>" />
                                    </td>
                                    <td>
                                    <input type="text" id="excess_<? echo $i;?>" onBlur="set_sum_value( 'excess_sum', 'excess_' ) "  name="excess_<? echo $i;?>" class="text_boxes_numeric" style="width:40px" onChange="calculate_requirement(<? echo $i;?>);set_sum_value( 'excess_sum', 'excess_' );set_sum_value( 'woqty_sum', 'woqny_' );copy_value(this.value,'excess_',<? echo $i;?>) " value="<? echo $booking_data_arr[$key][process_loss_percent]; ?>" disabled/>
                                    </td>
                                    <td>
                                    <input type="text" id="woqny_<? echo $i;?>" onBlur="set_sum_value( 'woqty_sum', 'woqny_' ) " onChange="set_sum_value( 'woqty_sum', 'woqny_' )"  name="woqny_<? echo $i;?>" class="text_boxes_numeric" style="width:70px" value="<? if(str_replace("'","",$txt_update_dtls_id) != ''){echo $booking_data_arr[$key][requirment];} ?>" readonly />
                                    </td>
                                    ...
                                    <td>
                                    <input type="text" id="amount_<? echo $i;?>"  name="amount_<? echo $i;?>"  onBlur="set_sum_value( 'amount_sum', 'amount_' ) " class="text_boxes_numeric" style="width:50px"  value="<? if(str_replace("'","",$txt_update_dtls_id) != ''){ echo $booking_data_arr[$key][amount];} ?>" placeholder="<? echo $Amt;//echo $txtwoq_cal; ?>" readonly>
                                    </td>
                                    <td>
                                    ...
                                    <input type="hidden" id="pcsset_<? echo $i;?>"  name="pcsset_<? echo $i;?>"  onBlur="set_sum_value( 'pcs_sum', 'pcs_' ) " class="text_boxes_numeric" style="width:50px"  value="<? echo $row[csf('order_quantity_set')]; $TOTORD+=$row[csf('order_quantity_set')]; ?>" readonly>
                                    </td>
                                    <td id="add_1">
                                   <input type="hidden" id="colorsizetableid_<? echo $i;?>"  name="colorsizetableid_<? echo $i;?>" class="text_boxes" style="width:85px" value="<? echo $row[csf('color_size_table_id')]; ?>" />
                                    <input type="hidden" id="updateid_<? echo $i;?>"  name="updateid_<? echo $i;?>" class="text_boxes" style="width:85px" value="<? echo $booking_data_arr[$key][id]; ?>" />
                    ...
                    <?
                    				$total_bal_woq +=$Req-$current_qty; 
                    				//$total_bal_woq +=number_format($Req-$current_qty,4,'.',''); 
								}
							}
						}
                ...
                <tfoot>
                    	<tr>
                        	<th colspan="8">SUM</th>
                            <th width="82" title="<?= $total_bal_woq ?>"><input type="text" id="bal_qty_sum" name="bal_qty_sum" class="text_boxes_numeric" style="width:70px" value="<? echo number_format($total_bal_woq,4,'.','') ?>"  readonly></th>
                            ...
                            <input type="hidden" id="po_qty"    name="po_qty" class="text_boxes_numeric" style="width:50px" value="<? echo $TOTORD; ?>" readonly>
                            </th>
                            <th width=""></th>
                        </tr>
...
if ($action=="show_trim_booking")
{
	...
	<div style="width:1200px; overflow-y:scroll; max-height:350px;" id="buyer_list_view" align="center">
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1180" class="rpt_table" id="tbl_list_search" >
            <tbody>
				...
				$is_short_arr=return_library_array("select booking_no,is_short from wo_booking_mst where booking_no='$txt_booking_no' and status_active=1", "booking_no", "is_short");
				 $sql_book=sql_select("select booking_no,is_short from wo_booking_mst where booking_no='$txt_booking_no' and status_active=1");
                foreach($sql_book as $row)
                {
						$is_short_id=$row[csf('is_short')];
				}
                ...
                $list_view_data_arr=array();
			    $country_id_array=array();
				$po_id_array=array();
				$job_array=array();
				$pre_cost_fabric_cost_dtls_id_array=array();
				$i=1;
                $total_amount=0;
                ...
                {
                $txt_country_cond ="and c.country_id in (".$country_ids.")";
                }
				if($po_ids=="" || $po_ids==0) $po_ids=0;else $po_ids=$po_ids;
				//$array_po_qty=array();
				$sql_po_qty=sql_select("select b.file_no,b.grouping,b.id,c.country_id, sum(c.order_quantity) as order_quantity,(sum(c.order_quantity)/a.total_set_qnty) as order_quantity_set  from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id and  b.id in(".$po_ids.")  $txt_country_cond  and a.status_active=1 and b.status_active=1 and c.status_active=1 group by b.id,b.file_no,b.grouping,a.total_set_qnty,c.country_id");
				foreach($sql_po_qty as $row)
				{
				//$array_po_qty[$row[csf('id')]][$row[csf('country_id')]]=$row[csf('order_quantity_set')];
				$po_array[$row[csf('id')]]['file_no']=$row[csf('file_no')];
				$po_array[$row[csf('id')]]['ref_no']=$row[csf('grouping')];
				}
				   /*$sql_cu_woq=sql_select("select po_break_down_id as po_id,country_id_string,pre_cost_fabric_cost_dtls_id as fab_dtls_id,trim_group,gmt_item,sum(wo_qnty) as cu_woq  from wo_booking_dtls where   booking_type=2 and status_active=1 and is_deleted=0 and  po_break_down_id in(".$po_ids.") group by po_break_down_id,country_id_string,pre_cost_fabric_cost_dtls_id,trim_group,gmt_item");
				   foreach($sql_cu_woq as $row)
					{
						$cu_woq_arr[$row[csf('po_id')]][$row[csf('fab_dtls_id')]][$row[csf('gmt_item')]][$row[csf('trim_group')]]['cu_woq']+=$row[csf('cu_woq')];
					}*/
				//print_r($cu_woq_arr);
				$condition= new condition();
                ...
                {
                $txt_country_cond ="and c.country_id in (".$selectResult[csf('country_id_string')].")";
                }
                //With Gmt Item
              ...
              /*  $sql_cons_data=sql_select("select b.cons from wo_pre_cost_trim_cost_dtls a , wo_pre_cost_trim_co_cons_dtls b where a.id=b.wo_pre_cost_trim_cost_dtls_id and  a.id='".$selectResult[csf('pre_cost_fabric_cost_dtls_id')]."' and b.po_break_down_id='".$selectResult[csf('po_break_down_id')]."' and a.is_deleted=0  and a.status_active=1");
               ...
               // $costing_per=return_field_value("costing_per", "wo_pre_cost_mst", "job_no='".$selectResult[csf('job_no')]."'");
                ...
                $file_no=$po_array[$selectResult[csf('po_break_down_id')]]['file_no'];//$sql_po_data[csf('file_no')];
				$ref_no=$po_array[$selectResult[csf('po_break_down_id')]]['ref_no'];//$sql_po_data[csf('grouping')];

			    $selected_country_array=explode(",",$selectResult[csf("country_id_string")]);
				$req_qnty_cons_uom=0;
				$req_amount_cons_uom=0;
				for($i=0;$i<count($selected_country_array);$i++){
					$req_qnty_cons_uom+=$trimQtyArray[$selectResult[csf('po_break_down_id')]][$selected_country_array[$i]][$selectResult[csf('gmt_item')]][$selectResult[csf('pre_cost_fabric_cost_dtls_id')]];
					$req_amount_cons_uom+=$trimAmountArray[$selectResult[csf('po_break_down_id')]][$selected_country_array[$i]][$selectResult[csf('gmt_item')]][$selectResult[csf('pre_cost_fabric_cost_dtls_id')]];
				}
               ...
               $cu_woq=$sql_cu_woq_row[csf('cu_woq')];
                ...
                ?>
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="load_dtls_data(<? echo $selectResult[csf('booking_id')]; ?>,<? echo $req_qnty;  ?>,<? echo $bal_woq;  ?>)">
                    <td width="30"><? echo $i;?></td>
                    <td width="90" style="word-break:break-all"><? echo $selectResult[csf('job_no')];?></td>
                    <td width="80" style="word-break:break-all"><? echo $file_no;?></td>
                    <td width="80" style="word-break:break-all"><?  echo $ref_no;?></td>
                    <td width="100" style="word-break:break-all"><? echo $po_number[$selectResult[csf('po_break_down_id')]];?></td>
                    <td width="80" style="word-break:break-all" title="<? echo $country_name_string ?>">
                    <?
                    ...
                    <td width="100" style="word-break:break-all"><? echo $sql_lib_item_group_array[$selectResult[csf('trim_group')]][item_name];?></td>
                    <td width="100" style="word-break:break-all"><? echo $size_color_sensitive[$selectResult[csf('sensitivity')]];?></td>
                    <td width="40" style="word-break:break-all"><? echo $unit_of_measurement[$sql_lib_item_group_array[$selectResult[csf('trim_group')]][cons_uom]];?></td>
                    <td width="60" style="word-break:break-all"><? echo change_date_format($selectResult[csf('delivery_date')],"dd-mm-yyyy","-"); ?></td>
                    <td width="60" style="word-break:break-all" align="right"><? echo def_number_format($req_qnty,2,""); ?></td>
                    <td width="60" style="word-break:break-all" align="right"><? echo def_number_format($cu_woq,2,"");?></td>
                    <td width="60" style="word-break:break-all" align="right"><? echo def_number_format($bal_woq,2,"");?></td>
                    <td width="60" style="word-break:break-all" align="right"><? echo $selectResult[csf('wo_qnty')];?></td>
                    <td width="50" style="word-break:break-all" align="right"><? echo number_format($selectResult[csf('rate')],8);?></td>
                    <td width="50" style="word-break:break-all" align="right"><? echo number_format($selectResult[csf('exchange_rate')],2);?></td>
                    <td style="word-break:break-all" align="right"><? echo def_number_format($selectResult[csf('amount')],2,""); $total_amount+=$selectResult[csf('amount')];?></td>
                </tr>
                <?
                $i++;
                }
...
{
	$po_number=return_library_array("select id,po_number from wo_po_break_down", "id", "po_number");
$sql="select id,job_no,po_break_down_id,pre_cost_fabric_cost_dtls_id,trim_group,sensitivity,uom,delivery_date,wo_qnty,rate,amount,exchange_rate,country_id_string,gmt_item,booking_mst_id from wo_booking_dtls  where id='$data'";
	 $data_array=sql_select($sql);
	 foreach ($data_array as $row)
	 {
        $booking_mst_id=$row[csf('booking_mst_id')];
        $company=return_field_value( "company_id", "wo_booking_mst","id=$booking_mst_id");
        $supplier_id=return_field_value( "supplier_id", "wo_booking_mst","id=$booking_mst_id");
		//echo "load_drop_down( 'requires/country_and_order_wise_trims_booking_entry_controller', '".$row[csf("job_no")]."', 'load_drop_down_trim_precost_id', 'trim_group_td' );\n";
		$drop_down_trim_precost_id=load_drop_down_trim_precost_id($row[csf("job_no")].'_'.$supplier_id.'_'.$company);
		echo "document.getElementById('trim_group_td').innerHTML = '".$drop_down_trim_precost_id."';\n";
		//echo "load_drop_down( 'requires/country_and_order_wise_trims_booking_entry_controller', '".$row[csf("job_no")]."', 'load_drop_down_gmt_item', 'gmt_item_td' );\n";
		...
		//set_precost_data($row[csf("pre_cost_fabric_cost_dtls_id")]."_".$row[csf("po_break_down_id")]."_".$row[csf("country_id_string")]."_".$row[csf("gmt_item")]."_".$row[csf("gmt_item")],2);
		echo "set_precost_data(".$row[csf("pre_cost_fabric_cost_dtls_id")].",2);\n";
		set_delivery_date_from_tna($row[csf("po_break_down_id")], "update");
	 }
}
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		if($db_type==0)
		{
			$new_booking_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'TB', date("Y",time()), 5, "select booking_no_prefix, booking_no_prefix_num from wo_booking_mst where company_id=$cbo_company_name and booking_type=2   and YEAR(insert_date)=".date('Y',time())." order by id desc ", "booking_no_prefix", "booking_no_prefix_num" ));
		}
		else if($db_type==2)
		{
		...
		$field_array="id, booking_type, is_short, booking_no_prefix, booking_no_prefix_num, booking_no, company_id, buyer_id, job_no, 	item_category, supplier_id, currency_id, exchange_rate, booking_date, delivery_date, pay_mode, source, attention, ready_to_approved, tenor, remarks,delivery_address,pay_term, entry_form, item_from_precost,inserted_by,insert_date";
		$data_array ="(".$id.",2,".$cbo_isshort.",'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',".$cbo_company_name.",".$cbo_buyer_name.",".$txt_job_no.",4,".$cbo_supplier_name.",".$cbo_currency.",".$txt_exchange_rate.",".$txt_booking_date.",".$txt_delivery_date.",".$cbo_pay_mode.",".$cbo_source.",".$txt_attention.",".$cbo_ready_to_approved.",".$txt_tenor.",".$txt_remarks.",".$delivery_address.",".$cbo_pay_term.",568,1,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
	...
	else if ($operation==1)   // Update Here
	{
		...
		//}
		 $field_array_up="is_short*buyer_id*job_no*item_category*supplier_id*currency_id*exchange_rate*booking_date*delivery_date*pay_mode*source*attention*ready_to_approved*tenor*remarks*delivery_address*pay_term*item_from_precost*updated_by*update_date";


		 $data_array_up ="".$cbo_isshort."*".$cbo_buyer_name."*".$txt_job_no."*4*".$cbo_supplier_name."*".$cbo_currency."*".$txt_exchange_rate."*".$txt_booking_date."*".$txt_delivery_date."*".$cbo_pay_mode."*".$cbo_source."*".$txt_attention."*".$cbo_ready_to_approved."*".$txt_tenor."*".$txt_remarks."*".$delivery_address."*".$cbo_pay_term."*1*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
...
if ($action=="save_update_delete_dtls")
{
	...
	if ($operation!=0)
	{
		$sql_rec="select b.booking_id, b.booking_no, b.item_group_id, b.item_description, b.order_id, b.receive_qnty, b.reject_receive_qnty, b.cons_qnty, b.gmts_color_id, b.item_color, b.gmts_size_id, b.item_size from inv_receive_master a,inv_trims_entry_dtls b where a.booking_no=b.booking_no and a.id=b.mst_id and a. booking_no=$txt_booking_no and b.item_group_id=".$txt_trim_group_id." and b.status_active=1 and  b.is_deleted=0";
	...
	if ($operation==0)  // Insert Here
	{
		$booking_data=sql_select("SELECT a.trim_group,a.gmt_item, b.id, b.wo_trim_booking_dtls_id, b.description, b.brand_supplier, b.item_color, b.item_size, b.cons, b.process_loss_percent, b.requirment, b.rate, b.amount, b.pcs, b.color_size_table_id, b.country_id, b.po_break_down_id  from wo_booking_dtls a join wo_trim_book_con_dtls b on a.id = b.wo_trim_booking_dtls_id where b.po_break_down_id=$txt_po_id and a.trim_group = $txt_trim_group_id and a.booking_type=2 and a.is_short = 2 and a.gmt_item = $cbo_gmt_item_id and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1");
	}
	if($operation==1)
	{
		$booking_data=sql_select("SELECT a.trim_group,a.gmt_item, b.id, b.wo_trim_booking_dtls_id, b.description, b.brand_supplier, b.item_color, b.item_size, b.cons, b.process_loss_percent, b.requirment, b.rate, b.amount, b.pcs, b.color_size_table_id, b.country_id, b.po_break_down_id  from wo_booking_dtls a join wo_trim_book_con_dtls b on a.id = b.wo_trim_booking_dtls_id where b.po_break_down_id=$txt_po_id and a.trim_group = $txt_trim_group_id and b.wo_trim_booking_dtls_id <> $txt_update_dtls_id and a.booking_type=2 and a.is_short = 2 and a.gmt_item = $cbo_gmt_item_id and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1");
	}
	foreach($booking_data as $row)
	{
		$booking_data_arr[$row[csf('po_break_down_id')]][$row[csf('country_id')]][$row[csf('trim_group')]][$row[csf('gmt_item')]][$row[csf('description')]][cons] = $row[csf('cons')];
		$booking_data_arr[$row[csf('po_break_down_id')]][$row[csf('country_id')]][$row[csf('trim_group')]][$row[csf('gmt_item')]][$row[csf('description')]][amount] = $row[csf('amount')];
		$booking_data_prev_arr[$row[csf('id')]][amount] = $row[csf('amount')];
		$booking_data_prev_arr[$row[csf('id')]][cons] = $row[csf('cons')];
	}

	$exeed_budge_qty=0; $exeed_budge_amount=0; $amount_exceed_level =0;
	$material_setting=sql_select("select exeed_budge_qty,exeed_budge_amount,amount_exceed_level,exceed_qty_level from variable_order_tracking where company_name=$cbo_company_name and item_category_id=4 and variable_list=26 and status_active=1 and is_deleted=0");
	foreach ($material_setting as $row) {
	...
	if ($operation==0)  // Insert Here
	{
		...
		if($db_type==0)
		{
		 ...
		 $field_array1="id,pre_cost_fabric_cost_dtls_id,po_break_down_id,job_no,booking_no,booking_mst_id,booking_type,is_short,trim_group,uom,sensitivity,wo_qnty,rate,exchange_rate,amount,delivery_date,country_id_string,gmt_item,entry_form_id,inserted_by,insert_date";
		 $field_array2="id,wo_trim_booking_dtls_id,booking_no,booking_mst_id,job_no,po_break_down_id,color_number_id,gmts_sizes,description,brand_supplier,item_color,item_size,cons, process_loss_percent,requirment,rate,amount,pcs,color_size_table_id,country_id";
         if(str_replace("'","",$cbo_item_from_precost)==2){
			 $cbo_trim_precost_id=0;
		 }

		$data_array1 .="(".$id_dtls.",".$cbo_trim_precost_id.",".$txt_po_id.",".$txt_job_no.",".$txt_booking_no.",".$booking_mst_id.",2,".$cbo_isshort.",".$txt_trim_group_id.",".$cbo_uom.",".$cbo_colorsizesensitive.",".$txt_quantity.",".$txt_avg_price.",".$txt_exchange_rate_dtls.",".$txt_amount.",".$txt_delevary_date.",".$txt_country.",".$cbo_gmt_item_id.",568,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		//echo $data_array1;
		//==============================
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			...
			if(str_replace("'","",$$itemcolor)!="")
			{
				 ...
				 {
					  $color_id = return_id( str_replace("'","",$$itemcolor), $color_library, "lib_color", "id,color_name","568");
			 ...
			 $txt_trim_group_id = str_replace("'","",$txt_trim_group_id);
			 $country_id = str_replace("'","",$$countryid);
			 $cbo_trim_precost_id = str_replace("'","",$cbo_trim_precost_id);
			 $txt_qty = str_replace("'","",$$qty); 
			 $woamount = str_replace("'","",$$amount);
			 $gmt_item_id = str_replace("'","",$$cbo_gmt_item_id);
			 $desItem = str_replace("'","",$$des);
			    
			 if($cbo_colorsizesensitive==1){ //
				$current_qty = $booking_data_arr[$txt_po_id][$country_id][$txt_trim_group_id][$gmt_item_id][$desItem][cons]+$txt_qty;
				$current_amt = $booking_data_arr[$txt_po_id][$country_id][$txt_trim_group_id][$gmt_item_id][$desItem][amount]+$woamount;
				$Req=$trimQtyArray[$txt_po_id][$country_id][$cbo_trim_precost_id][$pocolorid];
				$ReqAmt=$trimAmtArray[$txt_po_id][$country_id][$cbo_trim_precost_id][$pocolorid];

			}
			if($cbo_colorsizesensitive==2){
				$current_qty = $booking_data_arr[$txt_po_id][$country_id][$txt_trim_group_id][$gmt_item_id][$desItem][cons]+$txt_qty;
				$current_amt = $booking_data_arr[$txt_po_id][$country_id][$txt_trim_group_id][$gmt_item_id][$desItem][amount]+$woamount;
				$Req=$trimQtyArray[$txt_po_id][$country_id][$cbo_trim_precost_id][$gmtssizesid];
				$ReqAmt=$trimAmtArray[$txt_po_id][$country_id][$cbo_trim_precost_id][$gmtssizesid];
			}
			if($cbo_colorsizesensitive==3){
				$current_qty = $booking_data_arr[$txt_po_id][$country_id][$txt_trim_group_id][$gmt_item_id][$desItem][cons]+$txt_qty;
				$current_amt = $booking_data_arr[$txt_po_id][$country_id][$txt_trim_group_id][$gmt_item_id][$desItem][amount]+$woamount;
				//echo "10**".$txt_po_id.'--'.$country_id.'--'.$cbo_trim_precost_id.'--'.$pocolorid; die;
				$Req=$trimQtyArray[$txt_po_id][$country_id][$cbo_trim_precost_id][$pocolorid];
				$ReqAmt=$trimAmtArray[$txt_po_id][$country_id][$cbo_trim_precost_id][$pocolorid];
			}
			if($cbo_colorsizesensitive==4){
				$current_qty = $booking_data_arr[$txt_po_id][$country_id][$txt_trim_group_id][$gmt_item_id][$desItem][cons]+$txt_qty;
				$current_amt = $booking_data_arr[$txt_po_id][$country_id][$txt_trim_group_id][$gmt_item_id][$desItem][amount]+$woamount;
				$Req=$trimQtyArray[$txt_po_id][$country_id][$cbo_trim_precost_id][$pocolorid][$gmtssizesid];
				$ReqAmt=$trimAmtArray[$txt_po_id][$country_id][$cbo_trim_precost_id][$pocolorid][$gmtssizesid];
			}
			if($cbo_colorsizesensitive==0){
				$current_qty = $booking_data_arr[$txt_po_id][$country_id][$txt_trim_group_id][$gmt_item_id][$desItem][cons]+$txt_qty;
				$current_amt = $booking_data_arr[$txt_po_id][$country_id][$txt_trim_group_id][$gmt_item_id][$desItem][amount]+$woamount;
				$Req = $trimQtyArray[$cbo_trim_precost_id][$country_id];
				$ReqAmt = $trimAmtArray[$cbo_trim_precost_id][$country_id];
			}
			if($isshort_id==1) //Short
			{
				$current_qty =0;
			}
			else
			...
			if($amount_exceed_level==1 || $amount_exceed_level==2){ //Total Amount Level/Item
			
				$reqAmt_check=(($ReqAmt*$exeed_budge_amount)/100)+$ReqAmt;
				$curAmt=$current_amt;
				//echo "10**".$curAmt."**".$reqAmt_check; die;
				if(($curAmt-$reqAmt_check)>1){
					echo "vad1**".str_replace("'","",$txt_booking_no)."**".$i."**".$curAmt."**".$reqAmt_check;
				//	check_table_status( $_SESSION['menu_id'],0);
					 disconnect($con);die;
				}
				
			}
			if($exceed_qty_level==2){ //Total Qty Level/Item
			
			$exeed_budge_qty_percent = (($exeed_budge_qty/100)*$Req)+$Req;
			//echo "10**".$current_qty."**".$exeed_budge_qty_percent; die;
			$exeed_budge_qty_percent = number_format($exeed_budge_qty_percent,4,'.','');
			$reqQty_check=$exeed_budge_qty_percent;
			$curQty=number_format($current_qty,4,'.','');
			//if($current_qty>$exeed_budge_qty_percent)
			if(($curQty-$reqQty_check)>1)
			{
				//echo 'req_exc'.'**'.$txt_booking_no.'**'.$current_qty.'**'.$exeed_budge_qty_percent;
				echo "vad2**".str_replace("'","",$txt_booking_no)."**".$i."**".$curQty."**".$reqQty_check;
				disconnect($con);
				die;
			}
			}
			 if ($i!=1) $data_array2 .=",";
			 $data_array2 .="(".$id1.",".$id_dtls.",".$txt_booking_no.",".$booking_mst_id.",".$txt_job_no.",".$txt_po_id.",'".$pocolorid."','".$gmtssizesid."',".$$des.",".$$brndsup.",'".$color_id."','".$itemsizes."',".$$qty.",".$$excess.",".$$woqny.",".$$rate.",".$$amount.",".$$pcs.",".$$colorsizetableid.",".$$countryid.")";
			 //echo $data_array2;
			 $id1=$id1+1;
		 }
		 ...
		 //=============================
		  $txt_delivery_date=return_field_value( "max(delivery_date) as delivery_date", "wo_booking_dtls","booking_no = $txt_booking_no","delivery_date");
	...
	else if ($operation==1)   // Update Here
	{
		...
		$poid=str_replace("'","",$txt_po_id);

		$pi_number=return_field_value( "pi_number", "com_pi_master_details a,com_pi_item_details b"," a.id=b.pi_id  and b.work_order_no=$txt_booking_no  and a.item_category_id=4 and a.status_active=1 and  a.is_deleted=0  and b.status_active=1 and  b.is_deleted=0 and b.item_group=".$txt_trim_group_id." and b.order_id in($poid)  ");
				if($pi_number){
					echo "pi1**".str_replace("'","",$txt_booking_no)."**".$pi_number;
					disconnect($con);
					die;
		...
		$recv_number=return_field_value( "recv_number", "inv_receive_master a,inv_trims_entry_dtls b"," a.id=b.mst_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no  and a.item_category=4 and a.status_active=1 and  a.is_deleted=0  and b.status_active=1 and  b.is_deleted=0 and b.item_group_id=".$txt_trim_group_id."");
		 ...
		 $field_array_up1="pre_cost_fabric_cost_dtls_id*po_break_down_id*job_no*booking_no*is_short*trim_group*uom*sensitivity*wo_qnty*rate*amount*exchange_rate*delivery_date*country_id_string*gmt_item*updated_by*update_date";
		...
		$data_array_up1 .="".$cbo_trim_precost_id."*".$txt_po_id."*".$txt_job_no."*".$txt_booking_no."*".$cbo_isshort."*".$txt_trim_group_id."*".$cbo_uom."*".$cbo_colorsizesensitive."*".$txt_quantity."*".$txt_avg_price."*".$txt_amount."*".$txt_exchange_rate_dtls."*".$txt_delevary_date."*".$txt_country."*".$cbo_gmt_item_id."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			...
			if(str_replace("'","",$cbo_colorsizesensitive)==2){
			 ...
			 $gmt_item_id = str_replace("'","",$$cbo_gmt_item_id);
			   $desItem = str_replace("'","",$$des);
			 if($cbo_colorsizesensitive==1){
				$current_qty = $booking_data_arr[$txt_po_id][$country_id][$txt_trim_group_id][$gmt_item_id][$desItem][cons];
				$current_amt = $booking_data_arr[$txt_po_id][$country_id][$txt_trim_group_id][$gmt_item_id][$desItem][amount];
			...
			$dtls_id=str_replace("'",'',$$updateid);
			$amt_prev=$booking_data_prev_arr[$dtls_id][amount];
			if($amount_exceed_level==1 || $amount_exceed_level==2)
			{
				
			$reqAmt=(($ReqAmt*$exeed_budge_amount)/100)+$ReqAmt;
			$curAmt=($current_amt-$amt_prev)+($woamount);
			...
			{
			$woQty_prev=$booking_data_prev_arr[$dtls_id][cons];
			//+$txt_qty
			$curQty=($current_qty-$woQty_prev)+($txt_qty*$conversion_factor);
			//$reqQty=(($Req*$exeed_budge_qty)/100)+$Req;
			//$exeed_budge_qty_percent = (($exeed_budge_qty/100)*$Req)+$Req;
			$reqQty = (($exeed_budge_qty/100)*$Req)+$Req;
			$reqQty_check = number_format($reqQty,4,'.','');
			$curr_Qty = number_format($curQty,4,'.','');
			
			 if(($curr_Qty-$reqQty_check)>1)
			 {
				//echo 'req_exc'.'**'.$txt_booking_no.'**'.$current_qty.'**'.$exeed_budge_qty_percent;
				echo "vad2**".str_replace("'","",$txt_booking_no)."**".$i."**".$curr_Qty."**".$reqQty_check;
				disconnect($con);
				die;
			 }
			}
			...
			$rID_1=execute_query("update  wo_booking_mst set supplier_id=$cbo_supplier_name,delivery_date='$txt_delivery_date' where booking_no=$txt_booking_no");
	    	$rID_2=execute_query(bulk_update_sql_statement( "wo_trim_book_con_dtls", "id", $field_array_up2, $data_array_up2, $id_arr ));
			if($db_type==0)
			{
				if($rID && $rID_1 && $rID_2){
					mysql_query("COMMIT");
	...
	else if ($operation==2)   // Delete Here
			{
				...
				$data_array="".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*'0'*'1'";
				$data_array_dtls="'0'*'1'";
				$rID=sql_delete("wo_booking_dtls",$field_array,$data_array,"id","".$txt_update_dtls_id."",1);
				$rID2=sql_delete("wo_trim_book_con_dtls",$field_array_dtls,$data_array_dtls,"wo_trim_booking_dtls_id","".$txt_update_dtls_id."",1);
				if($db_type==0)
				{
					if($rID && $rID2){
						mysql_query("COMMIT");
...
<body>
    ...
    <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            <table width="1120" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
                 <thead>
                    <th colspan="10">
                      <?
                ...
                <tr class="general">
                    <td>
                        <?
                            echo create_drop_down( "cbo_company_mst", 120, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3)  order by company_name","id,company_name",1, "-- Select Company --", $company_id, "load_drop_down( 'country_and_order_wise_trims_booking_entry_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
                        ?>
                    </td>
                     <td id="buyer_td">
                          <?
                ...
                <td><input name="txt_date_from" id="txt_date_from" class="datepicker" style="width:60px"></td>
                 ...
                 <td align="center">
                    <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('cbo_supplier_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_booking_prifix').value+'_'+document.getElementById('cbo_search_category').value+'_'+document.getElementById('txt_file_no').value+'_'+document.getElementById('chk_job_wo_po').value+'_'+document.getElementById('txt_ref').value+'_'+document.getElementById('txt_job').value, 'create_booking_search_list_view', 'search_div', 'country_and_order_wise_trims_booking_entry_controller','setFilterGrid(\'list_view\',-1)')" style="width:100px;" /></td>
            </tr>
            <tr>
...
$booking_prefix=trim($data[6]);
	if($data[7]==4 || $data[7]==0)
		{
			if (str_replace("'","",$data[6])!="") $booking_cond=" and a.booking_no_prefix_num like '%$booking_prefix%'  $booking_year_cond  "; else $booking_cond="";
			...
			if (str_replace("'","",$data[11])!="") $job_cond=" and b.job_no like '%$data[11]%' "; else $job_cond="";
		}
    if($data[7]==1)
		{
			if (str_replace("'","",$data[6])!="") $booking_cond=" and a.booking_no_prefix_num ='$booking_prefix'   "; else $booking_cond="";
   ...
   if($data[7]==2)
	...
	$user=return_library_array( "select id, user_full_name from user_passwd",'id','user_full_name');
	$arr=array (1=>$comp,2=>$suplier,3=>$user);
	 
	if($data[9]==0)
	{
		  $sql="select a.id, a.booking_no_prefix_num, a.booking_no, company_id, c.job_no_mst, a.supplier_id, a.booking_date, a.delivery_date, c.file_no, c.grouping, a.inserted_by from wo_booking_mst a, wo_booking_dtls b, wo_po_break_down c where a.booking_no=b.booking_no and b.po_break_down_id=c.id and a.booking_type=2 and a.entry_form=568 and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 $company $buyer ".set_user_lavel_filtering(' and a.buyer_id','buyer_id')." $supplier_id $booking_date $booking_cond $job_cond $file_cond $ref_cond group by a.id, a.booking_no_prefix_num, c.job_no_mst, a.booking_no, company_id, a.supplier_id, a.booking_date, a.delivery_date, c.file_no, c.grouping, a.inserted_by order by a.id DESC";

		echo  create_list_view("list_view", "Booking No,Company,Supplier,Insert User,Booking Date,Delivery Date,Job No,File No,Ref. No", "60,100,100,100,70,70,100,100","900","300",0, $sql , "js_set_value", "booking_no", "", 1, "0,company_id,supplier_id,inserted_by,0,0,0,0,0", $arr , "booking_no_prefix_num,company_id,supplier_id,inserted_by,booking_date,delivery_date,job_no_mst,file_no,grouping", '','','0,0,0,0,3,3,0','','');
	}
	else
	{
		 $sql="select a.id, a.booking_no_prefix_num, a.booking_no, a.company_id, a.supplier_id, a.booking_date, a.delivery_date, a.inserted_by from wo_booking_mst a where  a.booking_no not in ( select a.booking_no from wo_booking_mst a, wo_booking_dtls b, wo_po_break_down c where a.booking_no=b.booking_no and b.po_break_down_id=c.id and  a.booking_type=2 and a.entry_form=568  and b.status_active =1  $company $buyer $supplier_id $booking_date $booking_cond $job_cond $file_cond $ref_cond group by a.booking_no_prefix_num, a.booking_no, company_id, a.supplier_id, a.booking_date, a.delivery_date) and a.status_active =1 and a.is_deleted=0 and a.booking_type=2 and a.entry_form=568 $company $buyer ".set_user_lavel_filtering(' and a.buyer_id','buyer_id')."  $supplier_id $booking_date $booking_cond group by a.id, a.booking_no_prefix_num, a.booking_no,a.company_id, a.supplier_id, a.booking_date, a.delivery_date, a.inserted_by order by a.id DESC";

		echo  create_list_view("list_view", "Booking No,Company,Supplier,Insert User,Booking Date,Delivery Date,Job No", "60,100,100,100,70,70","700","300",0, $sql , "js_set_value", "booking_no", "", 1, "0,company_id,supplier_id,inserted_by,0,0,0,0", $arr , "booking_no_prefix_num,company_id,supplier_id,inserted_by,booking_date,delivery_date,job_no_mst", '','','0,0,0,0,3,3,0,0','','');
	}
	exit();
}
...
function fnc_fabric_booking_terms_condition( operation )
{
	...
	http.open("POST","country_and_order_wise_trims_booking_entry_controller.php",true);
...
if ($action=="populate_data_from_search_popup")
{
	 $sql= "select id, booking_no,booking_date,company_id,ready_to_approved,tenor,buyer_id,job_no,currency_id,exchange_rate,pay_mode,supplier_id,attention,delivery_date,source,is_approved,is_short,remarks,item_from_precost,delivery_address,pay_term from wo_booking_mst  where booking_no='$data'";
	 //echo $sql;die;
	 $data_array=sql_select($sql);
	 foreach ($data_array as $row)
	 {
		...
		echo "load_drop_down( 'requires/country_and_order_wise_trims_booking_entry_controller', '".$row[csf("pay_mode")]."', 'load_drop_down_supplier', 'supplier_td' );\n";
...
if($action=="show_trim_booking_report")
{
	...
	<div style="width:1333px" align="center">
       <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid black">
		...
		$po_no="";$po_id="";
		$po_qty="";
		$nameArray_job=sql_select( "select b.id as po_id, b.po_number, sum(distinct b.po_quantity) as po_quantity   from wo_booking_dtls a, wo_po_break_down b where a.po_break_down_id=b.id and a.booking_no=$txt_booking_no and b.status_active=1 and b.is_deleted=0  group by b.id,b.po_number order by b.po_number");
        foreach ($nameArray_job as $result_job)
        {
		...
		$po_no_id=rtrim($po_id,',');
		//echo $po_no_id.'BBB';

		$style_ref="";
		$nameArray_style=sql_select( "select  a.style_ref_no,a.order_repeat_no  from wo_po_details_master a, wo_booking_dtls b where a.job_no=b.job_no and b.booking_no=$txt_booking_no and a.status_active=1 and b.status_active=1 group by a.style_ref_no,a.order_repeat_no");
        foreach ($nameArray_style as $result_style)
        {
			$style_ref.=$result_style[csf('style_ref_no')].", ";
			$order_repeat_no.=$result_style[csf('order_repeat_no')].", ";
		}


	    $nameArray=sql_select( "select a.booking_no,a.booking_date,a.supplier_id,a.pay_mode,a.currency_id,a.exchange_rate,a.attention,a.delivery_date,a.source,a.insert_date,a.update_date,a.remarks  from wo_booking_mst a where  a.booking_no=$txt_booking_no");
        foreach ($nameArray as $result)
        {


			$sql_po= "select b.po_number,MIN(b.pub_shipment_date) pub_shipment_date, MIN(b.insert_date) as insert_date,b.shiping_status from wo_booking_dtls a,wo_po_break_down b  where a.po_break_down_id=b.id and a.booking_no=$txt_booking_no and a.status_active=1 and b.status_active=1 group by b.po_number, b.shiping_status";
       ...
       <table width="100%" style="border:1px solid black">
            ...
            <tr>
              ...
              <tr>
                ...
                <td style="font-size:12px" colspan="3">:&nbsp;<? echo rtrim($po_qty,", "); ?> </td>
				...
				<?
				$order_repeat= rtrim($order_repeat_no,", ");
				if($order_repeat){
					echo $order_repeat;
				}
				?>
        ...
        $nameArray_color=sql_select( "select  b.color_number_id , min(b.color_size_table_id) as  color_size_table_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.sensitivity=1 and a.status_active=1 and b.status_active=1 group by b.color_number_id order by color_size_table_id");
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            {
				$i++;
            $nameArray_item_description=sql_select( "select b.description, b.brand_supplier,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.sensitivity=1 and a.trim_group=".$result_item[csf('trim_group')]." and a.status_active=1 group by b.description, b.brand_supplier order by trim_group ");
            ?>
            <tr>
        ...
        $nameArray_size=sql_select( "SELECT b.item_size as gmts_sizes, b.gmts_sizes as gmt_size_id , min(b.color_size_table_id) as color_size_table_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.sensitivity=2 and a.status_active=1 and b.status_active=1 group by b.item_size, b.gmts_sizes order by color_size_table_id");

        $article_number_data=sql_select( "SELECT a.job_no,c.size_number_id, c.article_number
		from wo_booking_dtls a, wo_trim_book_con_dtls b, wo_po_color_size_breakdown c
		where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.sensitivity=2 and c.job_no_mst=a.job_no  and c.job_no_mst=b.job_no and b.po_break_down_id=c.po_break_down_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
		group by a.job_no,c.size_number_id, c.article_number");
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            <tr>
                <td rowspan="2" style="border:1px solid black"><strong>Sl</strong> </td>
                <td rowspan="2" style="border:1px solid black"><strong>Item Group</strong> </td>
                <td rowspan="2" style="border:1px solid black"><strong>Item Description</strong> </td>
                <td rowspan="2" style="border:1px solid black"><strong>Brand Supplier</strong> </td>
                <?
                ...
                <td rowspan="2" style="border:1px solid black" align="center"><strong>Total</strong></td>
                <td rowspan="2" style="border:1px solid black" align="center"><strong>UOM</strong></td>
                <td rowspan="2" style="border:1px solid black" align="center"><strong>Rate</strong></td>
                <td rowspan="2" style="border:1px solid black" align="center"><strong>Amount</strong></td>
            </tr>
            <tr>
            	<?
            	// $artical = array(1=>'A',2=>'B',3=>'C',4=>'D',5=>'E');
                foreach($nameArray_size  as $result_size)
                {
                	$art_no=rtrim($article_numberArr[$result_size[csf('gmt_size_id')]]['article_number'],',');
				    ...
				    //echo $art_nos;
                	?>
                	<td align="center" style="border:1px solid black"><strong><? echo $art_nos;?></strong></td>
                	<?	
                }?>
            </tr>
            <?
			...
			$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.sensitivity=2 and a.trim_group=".$result_item[csf('trim_group')]." and a.status_active=1 and b.status_active=1 group by b.description, b.brand_supplier");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i ; ?>
		...
		<?
		$gmtcolor_library=return_library_array( "select b.item_color,b.color_number_id from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.sensitivity=3 and a.status_active=1 and b.status_active=1", "item_color", "color_number_id"  );
        ...
        $nameArray_color=sql_select( "select  b.item_color as color_number_id,b.color_number_id as gmts_color, min(b.color_size_table_id) as  color_size_table_id  from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.status_active=1 and a.booking_no=$txt_booking_no and a.sensitivity=3 and b.cons > 0 and a.status_active=1 and b.status_active=1  group by b.item_color,b.color_number_id order by color_size_table_id ");
		if(count($nameArray_color)>0)
		{
        ?>
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
			...
			$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.sensitivity=3 and a.trim_group=".$result_item[csf('trim_group')]." and a.status_active=1 and b.status_active=1 group by b.description, b.brand_supplier order by trim_group ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
        ...
        $nameArray_size=sql_select( "SELECT  b.item_size  as gmts_sizes, b.gmts_sizes as gmt_size_id, min(b.color_size_table_id) as color_size_table_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.sensitivity=4 and a.status_active=1 and b.status_active=1 group by b.item_size, b.gmts_sizes order by color_size_table_id");
		...
		from wo_booking_dtls a, wo_trim_book_con_dtls b, wo_po_color_size_breakdown c
		where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.sensitivity=4 and c.job_no_mst=a.job_no  and c.job_no_mst=b.job_no and b.po_break_down_id=c.po_break_down_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            <tr>
                ...
                <td rowspan="2" style="border:1px solid black"><strong>Item Color</strong> </td>
                <td rowspan="2" style="border:1px solid black"><strong>Gmts Color</strong> </td>
                <td rowspan="2" style="border:1px solid black"><strong>Item Description</strong> </td>
                <td rowspan="2" style="border:1px solid black"><strong>Brand Supplier</strong> </td>
                <?
                foreach($nameArray_size  as $result_size)
			...
			$nameArray_color=sql_select( "select  b.item_color,b.color_number_id,b.description, b.brand_supplier, min(b.color_size_table_id) as  color_size_table_id, avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.trim_group=".$result_item[csf('trim_group')]."  and a.sensitivity=4 and a.status_active=1 and b.status_active=1 group by b.item_color,b.color_number_id,b.description, b.brand_supplier order by color_size_table_id");

            $nameArray_item_description=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and sensitivity=4 and trim_group=".$result_item[csf('trim_group')]." and status_active=1 order by trim_group ");
            ?>
            <tr>
		...
		<?
        $nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and sensitivity=0 and status_active=1 order by trim_group ");
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item)
            {
				...
				$nameArray_item_description=sql_select( "select  b.description, b.brand_supplier,b.item_color,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=0 and a.status_active=1 group by b.description, b.brand_supplier,b.item_color");
            ...
            <tr>
				...
				if($db_type==0)
				        {
						$nameArray_color_size_qnty=sql_select("select sum(b.requirment) as cons,sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.sensitivity=0 and a.trim_group=". $result_item[csf('trim_group')]." and  b.description='". $result_itemdescription[csf('description')]."' and a.status_active=1 and b.brand_supplier='".$result_itemdescription[csf('brand_supplier')]."' and b.item_color='".$result_itemdescription[csf('item_color')]."' and a.status_active=1 and b.status_active=1");
						}
						if($db_type==2)
				        {
						$nameArray_color_size_qnty=sql_select("select sum(b.requirment) as cons,sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.sensitivity=0 and a.trim_group=". $result_item[csf('trim_group')]." and nvl( b.description,0)=nvl('". $result_itemdescription[csf('description')]."',0) and a.status_active=1 and nvl(b.brand_supplier,0)=nvl('".$result_itemdescription[csf('brand_supplier')]."',0) and nvl(b.item_color,0)=nvl('".$result_itemdescription[csf('item_color')]."',0) and a.status_active=1 and b.status_active=1");
						}
                ...
                <td style="border:1px solid black; text-align:center "><? echo $unit_of_measurement[$order_uom_arr[$result_item[csf('trim_group')]]]; ?></td>
                <td style="border:1px solid black; text-align:right"><? echo number_format($result_color_size_qnty[csf('amount')]/$result_color_size_qnty[csf('cons')],8); ?> </td>
                <td style="border:1px solid black; text-align:right">
                <?
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="100%" class="rpt_table" >
       ...
       <?
					...
					$condition= new condition();
					if(str_replace("'","",$po_no_id) !=''){
						$condition->po_id("in($po_no_id)");
					}
					$condition->init();
					$trims= new trims($condition);
					$trim_amount_arr=$trims->getAmountArray_by_orderAndItemid();
					...
					/*	$costing_per=return_field_value("costing_per", "wo_pre_cost_mst", "job_no='".$selectResult[csf('job_no')]."'");
						...
						$sql_cons_data=sql_select("select a.rate,b.cons from wo_pre_cost_trim_cost_dtls a , wo_pre_cost_trim_co_cons_dtls b where a.id=b.wo_pre_cost_trim_cost_dtls_id and  a.id='".$selectResult[csf('pre_cost_fabric_cost_dtls_id')]."' and b.po_break_down_id='".$selectResult[csf('po_break_down_id')]."' and a.is_deleted=0  and a.status_active=1  and b.cons>0");
						list($sql_cons_data_row)=$sql_cons_data;
						...
						$pre_amount=$trim_amount_arr[$selectResult[csf('po_break_down_id')]][$selectResult[csf('trim_group')]];//$pre_req_qnty*$pre_rate;
 ...
 if($action=="show_trim_booking_report7")
{
	...
	<div style="width:1333px" align="center">
       <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid black">
		...
		$style_ref="";
		$nameArray_style=sql_select( "select  a.style_ref_no,a.order_repeat_no  from wo_po_details_master a, wo_booking_dtls b where a.job_no=b.job_no and b.booking_no=$txt_booking_no group by a.style_ref_no,a.order_repeat_no");
        foreach ($nameArray_style as $result_style)
        {
			$style_ref.=$result_style[csf('style_ref_no')].", ";
			$order_repeat_no.=$result_style[csf('order_repeat_no')].", ";
       ...
       <table width="100%" style="border:1px solid black">
		...
		<?
        $nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and sensitivity=1 order by trim_group ");
        $nameArray_color=sql_select( "select  b.color_number_id , min(b.color_size_table_id) as  color_size_table_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.sensitivity=1 and a.status_active=1 group by b.color_number_id order by color_size_table_id");
		if(count($nameArray_color)>0)
		{
        ?>
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
		...
		<?
        $nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and sensitivity=2 order by trim_group ");
        $nameArray_size=sql_select( "select  b.item_size  as gmts_sizes,b.gmts_sizes as gmt_size_id, min(b.color_size_table_id) as color_size_table_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.status_active=1 and a.sensitivity=2 group by b.item_size,b.gmts_sizes order by color_size_table_id");
         $article_number_data=sql_select( "SELECT a.job_no,c.size_number_id, c.article_number
		from wo_booking_dtls a, wo_trim_book_con_dtls b, wo_po_color_size_breakdown c
		where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.sensitivity=2 and c.job_no_mst=a.job_no  and c.job_no_mst=b.job_no and b.po_break_down_id=c.po_break_down_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
		group by a.job_no,c.size_number_id, c.article_number");
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
			...
			$i++;
            //$nameArray_item_description=sql_select( "select distinct description,brand_supplier,rate,uom from wo_booking_dtls  where booking_no=$txt_booking_no and sensitivity=2 and trim_group=".$result_item[csf('trim_group')]." order by trim_group ");
			$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.sensitivity=2 and a.trim_group=".$result_item[csf('trim_group')]." and a.status_active=1 group by b.description, b.brand_supplier");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i ; ?>
                ...
                foreach($nameArray_size  as $result_size)
                {
					if($db_type==0)
					{
                $nameArray_size_size_qnty=sql_select( "select sum(b.requirment) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.sensitivity=2 and a.trim_group=". $result_item[csf('trim_group')]." and b.description='". $result_itemdescription[csf('description')]."' and b.brand_supplier='". $result_itemdescription[csf('brand_supplier')]."' and a.status_active=1 and b.item_size='".$result_size[csf('gmts_sizes')]."'");
					}
					if($db_type==2)
					{
                $nameArray_size_size_qnty=sql_select( "select sum(b.requirment) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.sensitivity=2 and a.trim_group=". $result_item[csf('trim_group')]." and nvl(b.description,0)=nvl('". $result_itemdescription[csf('description')]."',0) and a.status_active=1 and nvl(b.brand_supplier,0)=nvl('". $result_itemdescription[csf('brand_supplier')]."',0) and nvl(b.item_size,0)=nvl('".$result_size[csf('gmts_sizes')]."',0)");
        ...
        $nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and sensitivity=3 order by trim_group ");
		//echo "select  b.item_color as color_number_id,b.color_number_id as gmts_color, min(b.color_size_table_id) as  color_size_table_id  from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.sensitivity=3 group by b.item_color,b.color_number_id order by color_size_table_id ";
        $nameArray_color=sql_select( "select  b.item_color as color_number_id,b.color_number_id as gmts_color, min(b.color_size_table_id) as  color_size_table_id  from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.status_active=1 and a.booking_no=$txt_booking_no and a.sensitivity=3 and b.cons > 0 group by b.item_color,b.color_number_id order by color_size_table_id ");
		if(count($nameArray_color)>0)
		{
        ?>
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item)
            {
				$i++;
            //$nameArray_item_description=sql_select( "select distinct description,brand_supplier,rate,uom from wo_booking_dtls  where booking_no=$txt_booking_no and sensitivity=3 and trim_group=".$result_item[csf('trim_group')]." order by trim_group ");
			$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.sensitivity=3 and a.trim_group=".$result_item[csf('trim_group')]." and a.status_active=1 group by b.description, b.brand_supplier order by trim_group ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
        ...
        $nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and sensitivity=4 order by trim_group ");
        $nameArray_size=sql_select( "select  b.item_size  as gmts_sizes,b.gmts_sizes as gmt_size_id, min(b.color_size_table_id) as color_size_table_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.status_active=1 and a.sensitivity=4 group by b.item_size,b.gmts_sizes order by color_size_table_id");
        $article_number_data=sql_select( "SELECT a.job_no,c.size_number_id, c.article_number
		from wo_booking_dtls a, wo_trim_book_con_dtls b, wo_po_color_size_breakdown c
		where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.sensitivity=4 and c.job_no_mst=a.job_no  and c.job_no_mst=b.job_no and b.po_break_down_id=c.po_break_down_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
		group by a.job_no,c.size_number_id, c.article_number");
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
			...
			$nameArray_color=sql_select( "select  b.item_color,b.color_number_id,b.description, b.brand_supplier, min(b.color_size_table_id) as  color_size_table_id, avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.trim_group=".$result_item[csf('trim_group')]." and a.status_active=1 and a.sensitivity=4 group by b.item_color,b.color_number_id,b.description, b.brand_supplier order by color_size_table_id");
...
$po_number=return_library_array("select id,po_number from wo_po_break_down", "id", "po_number");
        ...
        $nameArray_color=sql_select( "select  b.color_number_id, min(b.color_size_table_id) as  color_size_table_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.status_active=1 and a.sensitivity=1 group by b.color_number_id order by color_size_table_id");
		if(count($nameArray_color)>0)
		{
        ?>
        &nbsp;
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
           <tr>
            ...
            {
				$i++;
            $nameArray_item_description=sql_select( "select b.description, b.brand_supplier,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and b.status_active=1  and a.status_active=1 and a.sensitivity=1 and a.trim_group=".$result_item[csf('trim_group')]." group by b.description, b.brand_supplier order by trim_group ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
        ...
        $nameArray_size=sql_select( "SELECT  b.item_size  as gmts_sizes, b.gmts_sizes as gmt_size_id, min(b.color_size_table_id) as color_size_table_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.sensitivity=2  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  group by b.item_size, b.gmts_sizes order by color_size_table_id");

        $article_number_data=sql_select( "SELECT c.size_number_id, c.article_number, c.job_no_mst
		from wo_booking_dtls a, wo_trim_book_con_dtls b, wo_po_color_size_breakdown c
		where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.sensitivity=2 and c.job_no_mst=a.job_no  and c.job_no_mst=b.job_no and b.po_break_down_id=c.po_break_down_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
		group by c.size_number_id, c.article_number, c.job_no_mst");
		$article_numberArr=array();
		foreach ($article_number_data as $value)
		{
		    $article_numberArr[$value[csf('job_no_mst')]][$value[csf('size_number_id')]]['article_number'].=$value[csf('article_number')].',';
		}
		if(count($nameArray_item)>0)
		{
        ?>
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            <tr>
                ...
                foreach($nameArray_size  as $result_size)
                {                	
                	$art_no=rtrim($article_numberArr[$nameArray_job_po_row[csf('job_no')]][$result_size[csf('gmt_size_id')]]['article_number'],',');
                	//$art_nos=implode(",",array_unique(explode(",",$art_no)));
                	$art_no_arr=array_unique(explode(",",$art_no));
			...
			$i++;
            //$nameArray_item_description=sql_select( "select distinct description,brand_supplier,rate,uom from wo_booking_dtls  where booking_no=$txt_booking_no and sensitivity=2 and trim_group=".$result_item[csf('trim_group')]." order by trim_group ");
			$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.status_active=1 and a.sensitivity=2 and a.trim_group=".$result_item[csf('trim_group')]." group by b.description, b.brand_supplier");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i ; ?>
               ...
               $total_amount_as_per_gmts_color=0;$total_amount_as_per_gmts_size=0;
                ...
                echo number_format($result_size_size_qnty[csf('cons')],2);
                $item_desctiption_total += $result_size_size_qnty[csf('cons')] ;
                if (array_key_exists($result_size[csf('color_number_id')], $size_tatal))
                {
                $size_tatal[$result_size[csf('gmts_sizes')]]+=$result_size_size_qnty[csf('cons')];
                }
                else
        ...
        $nameArray_color=sql_select( "select  b.item_color as color_number_id, b.color_number_id as gmts_color, min(b.color_size_table_id) as  color_size_table_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.status_active=1 and a.sensitivity=3 and b.cons >0 group by b.item_color ,b.color_number_id order by color_size_table_id ");
		if(count($nameArray_color)>0)
		{
        ?>
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
           <tr>
            ...
            foreach($nameArray_item as $result_item)
            {
				$i++;
            //$nameArray_item_description=sql_select( "select distinct description,brand_supplier,rate,uom from wo_booking_dtls  where booking_no=$txt_booking_no and sensitivity=3 and trim_group=".$result_item[csf('trim_group')]." order by trim_group ");
			$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.status_active=1 and a.sensitivity=3 and a.trim_group=".$result_item[csf('trim_group')]." group by b.description, b.brand_supplier order by trim_group ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
        ...
        $nameArray_size=sql_select( "SELECT  b.item_size  as gmts_sizes, b.gmts_sizes as gmt_size_id, min(b.color_size_table_id) as color_size_table_id  from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.status_active=1 and a.sensitivity=4 group by b.item_size, b.gmts_sizes order by color_size_table_id");

        $article_number_data=sql_select( "SELECT c.size_number_id, c.article_number, c.job_no_mst
		from wo_booking_dtls a, wo_trim_book_con_dtls b, wo_po_color_size_breakdown c
		where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.sensitivity=4 and c.job_no_mst=a.job_no  and c.job_no_mst=b.job_no and b.po_break_down_id=c.po_break_down_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.status_active=1 and b.status_active=1 and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]."
		group by c.size_number_id, c.article_number, c.job_no_mst");
		$article_numberArr=array();
		foreach ($article_number_data as $value)
		{
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            <tr>
                ...
                foreach($nameArray_size  as $result_size)
                {	     ?>
                <td rowspan="2" align="center" style="border:1px solid black"><strong><? echo $result_size[csf('gmts_sizes')];?></strong></td>
                <?	}    ?>
                <td rowspan="2" style="border:1px solid black" align="center"><strong>Total</strong></td>
                <td rowspan="2" style="border:1px solid black" align="center"><strong>UOM</strong></td>
                <td rowspan="2" style="border:1px solid black" align="center"><strong>Rate</strong></td>
			...
			$gmtcolor_library=return_library_array( "select b.item_color,b.color_number_id from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=4", "item_color", "color_number_id"  );
			 ...
			 $nameArray_color=sql_select( "select  b.item_color,b.color_number_id,b.description, b.brand_supplier, min(b.color_size_table_id) as  color_size_table_id,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=4 and a.status_active=1 group by b.item_color,b.color_number_id,b.description, b.brand_supplier order by color_size_table_id");

            $nameArray_item_description=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.status_active=1 and sensitivity=4  and trim_group=".$result_item[csf('trim_group')]." order by trim_group ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo   (count($nameArray_item_description)*count($nameArray_color)); ?>">
                <? echo $i ; ?>
        ...
        <!--==============================================AS PER Color & SIZE  END=========================================  -->
         ...
         <!--==============================================NO NENSITIBITY START=========================================  -->
		<?
        $nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and status_active=1 and sensitivity=0 order by trim_group ");
        //$nameArray_color=sql_select( "select distinct b.color_number_id from wo_trims_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=$txt_booking_no and a.sensitivity=1");
		//$nameArray_color= array();
		if(count($nameArray_item)>0)
		{
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item)
            {
				...
				$nameArray_item_description=sql_select( "select  b.description, b.brand_supplier,b.item_color,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.status_active=1 and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=0 and a.status_active=1 and b.status_active=1  group by b.description, b.brand_supplier,b.item_color");
...
?>
		...
		where a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.booking_type=2
       ...
       <table width="100%" style="border:1px solid black" align="left">
           ...
           <table width="100%" style="border:1px solid black; margin:20px 0px 20px 0px" align="left">
            ...
            <tr>
                ...
                <td style="font-weight:bold;">:&nbsp;<? echo $deling_marchent;?></td>
	   ...
	   if(count($nameArray_item)>0){
        ...
        <table style="border:1px solid black" align="left" class=""  cellpadding="0" width="100%" cellspacing="0" rules="all" >
           <tr>
            ...
            foreach($nameArray_item as $result_item){
				$i++;
				$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,b.item_color, b.color_number_id ,min(b.color_size_table_id) as color_size_table_id,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and  a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=1 and   a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and a.status_active=1 and b.requirment !=0 group by b.description, b.brand_supplier,b.item_color, b.color_number_id order by color_size_table_id ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
		...
		<?
		$nameArray_item=sql_select( "select  id,trim_group,country_id_string from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and status_active=1 and sensitivity=2 order by trim_group ");
		if(count($nameArray_item)>0){
        ?>
        ...
        <table style="border:1px solid black" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item){
			$i++;
			$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,b.item_size,b.gmts_sizes,min(b.color_size_table_id) as color_size_table_id, sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id=b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=2 and  a.id= ".$result_item[csf('id')]." and a.status_active=1 and a.trim_group=".$result_item[csf('trim_group')]." and b.requirment !=0 group by b.description, b.brand_supplier,b.item_size, b.gmts_sizes order by color_size_table_id");
        ...
        <!--==============================================Size Sensitive END=========================================  -->
         ...
         <!--==============================================AS PER CONTRAST COLOR START=========================================  -->
		<?
		 $nameArray_item=sql_select( "select  id,trim_group,country_id_string from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and status_active=1 and sensitivity=3 order by trim_group ");
		if(count($nameArray_item)>0){
        ?>
        <table style="border:1px solid black" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
           <tr>
            ...
            foreach($nameArray_item as $result_item){
				$i++;
				$gmtcolor_library=return_library_array( "select b.item_color,b.color_number_id from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=3 and  a.id= ".$result_item[csf('id')]." and a.status_active=1 and a.trim_group=".$result_item[csf('trim_group')]." and b.requirment !=0  order by trim_group ", "item_color", "color_number_id"  );
				$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,b.item_color,b.color_number_id,sum(b.requirment) as cons,min(b.color_size_table_id) as color_size_table_id,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=3 and  a.id= ".$result_item[csf('id')]." and a.status_active=1 and a.trim_group=".$result_item[csf('trim_group')]." and b.requirment !=0 group by b.description, b.brand_supplier,b.item_color,b.color_number_id order by color_size_table_id ");
				$sql_trim_amount=sql_select("select  cons_uom, cons_dzn_gmts, rate, amount from  wo_pre_cost_trim_cost_dtls where job_no='".$nameArray_job_po_row[csf('job_no')]."'  and trim_group=".$result_item[csf('trim_group')]." ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
		...
		$nameArray_item=sql_select( "select  id,trim_group,country_id_string from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and status_active=1 and sensitivity=4 order by trim_group ");
	   if(count($nameArray_item)>0)
		{
        ?>
        ...
        <table style="border:1px solid black" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            {
			$i++;
			$nameArray_color=sql_select( "select  b.item_color,b.color_number_id,b.item_size,b.gmts_sizes,min(b.color_size_table_id) as color_size_table_id,b.description, b.brand_supplier,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id   and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and  a.id= ".$result_item[csf('id')]." and a.status_active=1 and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=4 and b.requirment !=0 group by b.item_color,b.color_number_id,b.item_size,b.gmts_sizes,b.description, b.brand_supplier order by color_size_table_id ");
            $nameArray_item_description=sql_select( "select distinct uom from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and status_active=1 and sensitivity=4   and trim_group=".$result_item[csf('trim_group')]." order by trim_group ");
			$sql_trim_amount=sql_select("select  cons_uom, cons_dzn_gmts, rate, amount from  wo_pre_cost_trim_cost_dtls where job_no='".$nameArray_job_po_row[csf('job_no')]."'  and trim_group=".$result_item[csf('trim_group')]." ");
            ?>
            <tr>
	   ...
	   ?>
        <!--==============================================AS PER Color & SIZE  END=========================================  -->
         ...
         <!--==============================================NO NENSITIBITY START=========================================  -->
		<?
		$nameArray_item=sql_select( "select  id,trim_group,country_id_string from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and status_active=1 and sensitivity=0 order by trim_group ");
		if(count($nameArray_item)>0)
		{
        ?>
        <table  style="border:1px solid black" align="left" class=""  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item)
            {
				...
				$nameArray_item_description=sql_select( "select  b.description, b.brand_supplier,b.item_color,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.status_active=1 and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=0 group by b.description, b.brand_supplier,b.item_color");
       ...
       &nbsp;
        ...
        <table  width="100%" class=""   border="1" cellpadding="0" cellspacing="0" rules="all"  align="left">
        <thead>
            <tr style="border:1px solid black;">
    ...
    <table cellspacing="0" cellpadding="0" style="border:1px solid black" rules="all" width="100%" class=""  align="left">
           ...
           <?
					...
					$sql_wo=sql_select("select b.po_break_down_id as po_id,b.booking_no,a.exchange_rate,b.pre_cost_fabric_cost_dtls_id as fab_dtls_id,sum(b.amount) as amount  from wo_booking_mst a,wo_booking_dtls b where a.booking_no=b.booking_no and b.booking_type=2 and b.is_short=2 and
					...
					foreach($sql_wo as $row){
						$main_booking_array[$row[csf('booking_no')]][$row[csf('fab_dtls_id')]][$row[csf('po_id')]]['amount']=$row[csf('amount')];
						$main_booking_array[$row[csf('booking_no')]][$row[csf('fab_dtls_id')]][$row[csf('po_id')]]['exchange_rate']=$row[csf('exchange_rate')];
					}
					...
					$wo_book2=sql_select("select po_break_down_id,pre_cost_fabric_cost_dtls_id as fab_dtls_id,$group_concat,sum(amount) as amount  from wo_booking_dtls where
					 booking_type=2 and is_short=2 and status_active=1 and is_deleted=0 group by po_break_down_id,pre_cost_fabric_cost_dtls_id");
					foreach($wo_book2 as $row){
							$main_booking_array2[$row[csf('po_break_down_id')]][$row[csf('fab_dtls_id')]]['booking_no']=$row[csf('booking_no')];
					}
                    ...
                    foreach ($nameArray as $selectResult){
						...
						$booking_data=explode(",",$main_booking_array2[$selectResult[csf('po_break_down_id')]][$selectResult[csf('pre_cost_fabric_cost_dtls_id')]]['booking_no']);
						$booking_amount=0;
						$exchaned_rate=0;

						foreach($booking_data as $book_no){
							if($book_no!=str_replace("'","",$txt_booking_no)){
							$booking_amount=$main_booking_array[$book_no][$selectResult[csf('pre_cost_fabric_cost_dtls_id')]][$selectResult[csf('po_break_down_id')]]['amount'];
							$exchaned_rate=$main_booking_array[$book_no][$selectResult[csf('pre_cost_fabric_cost_dtls_id')]][$selectResult[csf('po_break_down_id')]]['exchange_rate'];
							}
						}
						$sql_cu_woq=sql_select("select sum(amount) as amount  from wo_booking_dtls where booking_no=$txt_booking_no and po_break_down_id='".$selectResult[csf('po_break_down_id')]."' and pre_cost_fabric_cost_dtls_id='".$selectResult[csf('pre_cost_fabric_cost_dtls_id')]."'  and  booking_type=2 and is_short=2 and status_active=1 and is_deleted=0");

						list($sql_cu_woq_row)=$sql_cu_woq;
						$cu_woq_amount=($sql_cu_woq_row[csf('amount')]/$exchange_rate)+($booking_amount/$exchaned_rate);
						...
						<tr style="border:1px solid black">
                            ...
                            <td style="border:1px solid black"><? echo $trim_group_library[$selectResult[csf('trim_group')]];?></td>
        ...
        <table cellspacing="0" cellpadding="0" rules="all" width="100%" class=""  align="left">
        	<tr>
				...
				<?
					...
					echo "****".custom_file_name($txt_booking_no,$style_ref,$job_no); ?>
                </td>
            </tr>
        </table>
        <!--</div>       -->
...
?>
		...
		$nameArray_buyer=sql_select( "select distinct a.buyer_name  from wo_po_details_master a, wo_booking_dtls b where a.job_no=b.job_no and b.booking_no=$txt_booking_no and  b.status_active=1");
        foreach ($nameArray_buyer as $result_buy)
        {
			$buyer_string.=$buyer_name_arr[$result_buy[csf('buyer_name')]].",";
		}
		...
		$repeat_no_string='';
		$nameArray_order_repeat_no=sql_select( "select distinct a.order_repeat_no  from wo_po_details_master a, wo_booking_dtls b where a.job_no=b.job_no and b.booking_no=$txt_booking_no and  b.status_active=1");
        foreach ($nameArray_order_repeat_no as $result_repeat_no)
        {
			$repeat_no_string.=$result_repeat_no[csf('order_repeat_no')].",";
		}

		$po_no="";$file_no="";$ref_no="";$po_id="";
		$nameArray_job=sql_select( "select distinct b.id as po_id,b.po_number,b.file_no,b.grouping  from wo_booking_dtls a, wo_po_break_down b where a.po_break_down_id=b.id and a.booking_no=$txt_booking_no and  a.status_active=1 and  b.status_active=1");
        foreach ($nameArray_job as $result_job)
        {
			$po_no.=$result_job[csf('po_number')].", ";
			$po_id.=$result_job[csf('po_id')].",";
		...
		$style_ref="";
		$deling_marchent="";
		$nameArray_style=sql_select( "select distinct a.style_ref_no,a.dealing_marchant  from wo_po_details_master a, wo_booking_dtls b where a.job_no=b.job_no and b.booking_no=$txt_booking_no and  b.status_active=1");
        foreach ($nameArray_style as $result_style)
        {
			$style_ref.=$result_style[csf('style_ref_no')].", ";
			$deling_marchent.=$team_member_arr[$result_style[csf('dealing_marchant')]].", ";
		}
        $nameArray=sql_select("select a.booking_no,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.attention,a.delivery_date,a.source,a.remarks,a.pay_mode  from wo_booking_mst a where  a.booking_no=$txt_booking_no and  a.status_active=1");
        foreach ($nameArray as $result)
        {
			$varcode_booking_no=$result[csf('booking_no')];
        ?>
       <table width="100%" style="border:1px solid black">
            <tr>
             ...
             <tr>
                ...
                <td width="100"><span style="font-size:12px"><b>Order Repeat No</b></span></td>
                <td width="110">:&nbsp;<? echo   rtrim($repeat_no_string,",");?></td>
        ...
        ?>
          <!--==============================================AS PER GMTS COLOR START=========================================  -->
		<?
		//echo "select a.id,a.job_no,a.po_break_down_id, b.po_quantity as po_quantity from wo_booking_dtls a,wo_po_break_down b  where a.booking_no=$txt_booking_no and a.po_break_down_id=b.id and b.status_active=1 and b.is_deleted=0  group by a.id,a.job_no,a.po_break_down_id, b.po_quantity order by a.job_no,a.po_break_down_id ";
		$nameArray_job_po=sql_select( "select a.job_no,a.po_break_down_id, b.po_quantity as po_quantity from wo_booking_dtls a,wo_po_break_down b  where a.booking_no=$txt_booking_no and a.po_break_down_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  group by a.job_no,a.po_break_down_id, b.po_quantity order by a.job_no,a.po_break_down_id ");
		foreach($nameArray_job_po as $nameArray_job_po_row)
		{

       // $nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]."  and sensitivity=1 order by trim_group ");
		 $nameArray_item=sql_select( "select  id,trim_group,country_id_string from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]."  and sensitivity=1 and status_active=1 order by trim_group ");
	   ...
	   if(count($nameArray_item)>0)
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
           <tr>
            ...
            {
				$i++;
            $nameArray_item_description=sql_select( "select b.description, b.brand_supplier,b.item_color,min(b.color_size_table_id) as color_size_table_id,sum(b.cons) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and  a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=1 and   a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and a.status_active=1 and b.cons>0 group by a.id,b.description, b.brand_supplier,b.item_color order by color_size_table_id ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
		...
		$nameArray_item=sql_select( "select  id,trim_group,country_id_string from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and status_active=1 and sensitivity=2 and status_active=1 and is_deleted=0 order by trim_group ");
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
			...
			$i++;
            //$nameArray_item_description=sql_select( "select distinct description,brand_supplier,rate,uom from wo_booking_dtls  where booking_no=$txt_booking_no and sensitivity=2 and trim_group=".$result_item[csf('trim_group')]." order by trim_group ");
			$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,b.item_size,b.gmts_sizes,min(b.color_size_table_id) as color_size_table_id, sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id=b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=2 and  a.id= ".$result_item[csf('id')]." and a.status_active=1 and a.trim_group=".$result_item[csf('trim_group')]." and b.requirment !=0 group by a.id,b.description, b.brand_supplier,b.item_size, b.gmts_sizes order by color_size_table_id");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i ; ?>
       ...
       // $nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=3 order by trim_group ");
		 ...
		 $nameArray_item=sql_select( "select  id,trim_group,country_id_string from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=3  and status_active=1 and is_deleted=0 order by trim_group ");
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
           <tr>
            ...
            {
				$i++;
				$gmtcolor_library=return_library_array( "select b.item_color,b.color_number_id from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=3 and  a.id= ".$result_item[csf('id')]." and a.status_active=1 and a.trim_group=".$result_item[csf('trim_group')]." and b.requirment !=0  and a.status_active=1  order by trim_group ", "item_color", "color_number_id"  );
			$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,b.item_color,b.color_number_id,sum(b.requirment) as cons,min(b.color_size_table_id) as color_size_table_id,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=3 and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and a.status_active=1 and b.requirment !=0 group by a.id,b.description, b.brand_supplier,b.item_color,b.color_number_id order by color_size_table_id ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
	   ...
	   if(count($nameArray_item)>0)
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
			...
			//$gmtcolor_library=return_library_array( "select b.item_color,b.color_number_id from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no  and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=4 and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and b.requirment !=0  order by trim_group ", "item_color", "color_number_id"  );
			 ...
			 $nameArray_color=sql_select( "select  b.item_color,b.color_number_id,b.item_size,b.gmts_sizes,min(b.color_size_table_id) as color_size_table_id,b.description, b.brand_supplier,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id   and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and a.status_active=1 and a.sensitivity=4 and b.requirment !=0  and a.status_active=1 group by a.id,b.item_color,b.color_number_id,b.item_size,b.gmts_sizes,b.description, b.brand_supplier order by color_size_table_id ");
	   ...
	   ?>
		...
		<?
       //$nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=0 order by trim_group ");
		$nameArray_item=sql_select( "select  id,trim_group,country_id_string from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=0 and status_active=1 and is_deleted=0 order by trim_group ");
        //$nameArray_color=sql_select( "select distinct b.color_number_id from wo_trims_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=$txt_booking_no and a.sensitivity=1");
		//$nameArray_color= array();
		if(count($nameArray_item)>0)
		{
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item)
            {
				...
				$nameArray_item_description=sql_select( "select  b.description, b.brand_supplier,b.item_color,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=0  and a.status_active=1 group by a.id, b.description, b.brand_supplier,b.item_color");
	...
	<?
       ...
       <table  width="850" class="rpt_table"   border="1" cellpadding="0" cellspacing="0" rules="all">
		...
		{
			$sql_unapproved=sql_select("select booking_id,approval_cause from fabric_booking_approval_cause where  entry_form=8 and approval_type=0 and is_deleted=0 and status_active=1 and booking_id=$mst_id");
			//echo "select booking_id,approval_cause from fabric_booking_approval_cause where  entry_form=8 and approval_type=0 and is_deleted=0 and status_active=1 and booking_id=$mst_id";
...
?>
                        ...
                        <tr>
                            ...
                            <td align="center" style="font-size:20px">
                                <strong>
								<?
								if(str_replace("'","",$cbo_isshort)==2) $isshort="";
								if(str_replace("'","",$cbo_isshort)==1) $isshort="[Short]";

								if ($report_title !="") echo $report_title." ".$isshort;
								else echo "Main Trims Booking ".$isshort;
								?>
                                &nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                <font style="color:#F00">
								<? if(str_replace("'","",$id_approved_id) ==1){ echo "(Approved)";}else{echo "";}; ?>
		...
		$booking_grand_total=0; $job_no=""; $currency_id="";

		$sql_dtls=sql_select("select a.job_no, a.buyer_name, a.style_ref_no,b.po_number,b.file_no,b.grouping from wo_po_details_master a, wo_po_break_down b, wo_booking_mst c, wo_booking_dtls d where a.job_no=b.job_no_mst and c.booking_no=d.booking_no and d.po_break_down_id=b.id and c.booking_no=$txt_booking_no and a.status_active=1 and d.status_active=1");

		 foreach ($sql_dtls as $result_job)
		 {
			 $job_no.=$result_job[csf('job_no')].",";
			 $buyer_string.=$buyer_name_arr[$result_job[csf('buyer_name')]].",";
			 ...
			 $po_no.=$result_job[csf('po_number')].",";
			  ...
			  $style_ref_arr[$result_job[csf('job_no')]] = $result_job[csf('style_ref_no')];
		 }
        ...
        $nameArray=sql_select( "select a.booking_no, a.booking_date, a.supplier_id, a.currency_id, a.exchange_rate, a.attention, a.delivery_date, a.source, a.remarks, a.pay_mode  from wo_booking_mst a where a.booking_no=$txt_booking_no and booking_type=2 and a.status_active=1");
        foreach ($nameArray as $result)
        {
			$varcode_booking_no=$result[csf('booking_no')];
			?>
			<table width="100%" style="border:1px solid black">
                ...
                <tr>
                    <td width="100" style="font-size:12px; font-weight: bold;"><b>Booking No </b>   </td>
                    <td width="110" style="font-weight: bold;">:&nbsp;<? echo $result[csf('booking_no')];?> </td>
                    <td width="100" style="font-size:12px"><b>Booking Date</b></td>
                    <td width="110">:&nbsp;<? echo change_date_format($result[csf('booking_date')],'dd-mm-yyyy','-');?>&nbsp;&nbsp;&nbsp;</td>
                    <td width="100" style="font-size:12px"><b>Source</b></td>
                    <td width="110" >:&nbsp;<? echo $source[$result[csf('source')]]; ?></td>
                ...
                <tr>
                    ...
                    <td style="font-size:12px" >:&nbsp;<? echo $pay_mode[$result[csf('pay_mode')]]; ?> </td>
                </tr>
                <tr>
                ...
                <tr>
                    <td>Ref. No </td><td colspan="5">:&nbsp; <?  echo rtrim($ref_no,','); ?> </td>
                </tr>
			</table>
			<br/>
			<?
		...
		foreach($nameArray_job_po as $nameArray_job_po_row)
		{
        	$nameArray_item=sql_select( "select distinct trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]."  and sensitivity=1 and status_active=1 and is_deleted=0 order by trim_group ");

        	$nameArray_color=sql_select( "select  b.color_number_id, min(b.color_size_table_id) as  color_size_table_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.color_number_id order by color_size_table_id");
		if(count($nameArray_color)>0)
		{
			?>
			&nbsp;
			<table border="1" align="left" class="rpt_table" cellpadding="0" width="100%" cellspacing="0" rules="all" >
                <tr>
                    <td colspan="<? echo count($nameArray_color)+8; ?>" align="">
                    <strong><? echo "Job NO:".$nameArray_job_po_row[csf('job_no')]." &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;PO No:" .$po_number[$nameArray_job_po_row[csf('po_break_down_id')]]." &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;PO Qty:" .$nameArray_job_po_row[csf('po_quantity')]."&nbsp; &nbsp;Style no:".$style_ref_arr[$nameArray_job_po_row[csf('job_no')]];?></strong>
                    </td>
                </tr>
                <tr>
                    <td colspan="<? echo count($nameArray_color)+8; ?>" align=""><strong>As Per Garments Color</strong></td>
                </tr>
                <tr>
                    <td style="border:1px solid black"><strong>Sl</strong> </td>
                    <td style="border:1px solid black"><strong>Item Group</strong> </td>
                ...
                {
                $i++;
                $nameArray_item_description=sql_select( "select b.description, b.brand_supplier, avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.trim_group=".$result_item[csf('trim_group')]." group by b.description, b.brand_supplier order by trim_group ");
                ?>
                <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
                ...
                $nameArray_color_size_qnty=sql_select( "select sum(b.requirment) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=1 and a.trim_group=". $result_item[csf('trim_group')]." and nvl(b.description,0)=nvl('". $result_itemdescription[csf('description')]."',0) and nvl(b.brand_supplier,0)=nvl('". $result_itemdescription[csf('brand_supplier')]."',0) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and nvl(b.color_number_id,0)=nvl(".$result_color[csf('color_number_id')].",0) ");
                }
                ...
                ?>
                <tr>
                <td style="border:1px solid black;  text-align:right" colspan="2"><strong> <b style="float:left">Delivery Date: <? echo change_date_format($result_item[csf('delivery_date')]);?></b>  Item Total</strong></td>
                <?
                foreach($nameArray_color  as $result_color)
                {
        ...
        <!--==============================================AS PER GMTS SIZE START=========================================  -->
		<?
        $nameArray_item=sql_select( "select distinct trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=2 and status_active=1 and is_deleted=0 order by trim_group ");
        $nameArray_size=sql_select( "select  b.item_size  as gmts_sizes, min(b.color_size_table_id) as color_size_table_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.sensitivity=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.item_size order by color_size_table_id");
		if(count($nameArray_item)>0)
		{
        ?>
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            {
			$i++;
			$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.trim_group=".$result_item[csf('trim_group')]." group by b.description, b.brand_supplier");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i ; ?>
               ...
               $total_amount_as_per_gmts_color=0;$total_amount_as_per_gmts_size=0;
                foreach($nameArray_item_description as $result_itemdescription)
                {
                $item_desctiption_total=0; $item_desctiption_total_amount=0;
                ?>
                <td style="border:1px solid black"><? echo $result_itemdescription[csf('description')]; ?> </td>
                <td style="border:1px solid black"><? echo $result_itemdescription[csf('brand_supplier')]; ?> </td>
                <?
                ...
                $nameArray_size_size_qnty=sql_select( "select sum(b.requirment) as cons,sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=2 and a.trim_group=". $result_item[csf('trim_group')]." and b.description='". $result_itemdescription[csf('description')]."' and b.brand_supplier='". $result_itemdescription[csf('brand_supplier')]."' and b.item_size='".$result_size[csf('gmts_sizes')]."' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
					}
					if($db_type==2)
					{
                $nameArray_size_size_qnty=sql_select( "select sum(b.requirment) as cons,sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=2 and a.trim_group=". $result_item[csf('trim_group')]." and nvl(b.description,0)=nvl('". $result_itemdescription[csf('description')]."',0) and nvl(b.brand_supplier,0)=nvl('". $result_itemdescription[csf('brand_supplier')]."',0) and nvl(b.item_size,0)=nvl('".$result_size[csf('gmts_sizes')]."',0) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
					}
                foreach($nameArray_size_size_qnty as $result_size_size_qnty)
                {
                ?>
                ...
                echo number_format($result_size_size_qnty[csf('cons')],2);
                $item_desctiption_total += $result_size_size_qnty[csf('cons')] ;
				$item_desctiption_total_amount += $result_size_size_qnty[csf('amount')] ;
                if (array_key_exists($result_size[csf('gmts_sizes')], $size_tatal))
                {
                  $size_tatal[$result_size[csf('gmts_sizes')]]+=$result_size_size_qnty[csf('cons')];
				  $size_tatal_amt[$result_item[csf('trim_group')]]+=$result_size_size_qnty[csf('amount')];
                }
                else
                {
                ...
                <?
                $amount_as_per_gmts_size =$item_desctiption_total_amount;//  $item_desctiption_total*$result_itemdescription[csf('rate')];
				echo number_format($amount_as_per_gmts_size,2);
				$tot_item_amt_arr[$result_item[csf('trim_group')]]+=$amount_as_per_gmts_size;
                $total_amount_as_per_gmts_size+=$amount_as_per_gmts_size;
                ?>
                </td>
            </tr>
            ...
            ?>
                ...
                <?
               // echo number_format($total_amount_as_per_gmts_size,2);
			    echo number_format($tot_item_amt_arr[$result_item[csf('trim_group')]],2);
                $grand_total_as_per_gmts_size+=$tot_item_amt_arr[$result_item[csf('trim_group')]];
                ?>
                </td>
            </tr>
            <?
        ...
        $nameArray_item=sql_select( "select distinct trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=3 and status_active=1 and is_deleted=0 order by trim_group ");

        $nameArray_color=sql_select( "select  b.item_color as color_number_id, b.color_number_id as gmts_color, min(b.color_size_table_id) as  color_size_table_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=3 and b.cons >0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.item_color ,b.color_number_id order by color_size_table_id ");
		if(count($nameArray_color)>0)
		{
        ?>
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
           <tr>
            ...
            {
				$i++;
			$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=3 and a.trim_group=".$result_item[csf('trim_group')]." and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.description, b.brand_supplier order by trim_group ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
                ...
                foreach($nameArray_color  as $result_color)
                {
					if($db_type==0)
				    {
                $nameArray_color_size_qnty=sql_select( "select sum(b.cons) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=3 and a.trim_group=". $result_item[csf('trim_group')]." and b.description='". $result_itemdescription[csf('description')]."' and b.brand_supplier='". $result_itemdescription[csf('brand_supplier')]."' and b.item_color=".$result_color[csf('color_number_id')]." and b.color_number_id=".$result_color[csf('gmts_color')]." and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
					}
					if($db_type==2)
				    {
                $nameArray_color_size_qnty=sql_select( "select sum(b.cons) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=3 and a.trim_group=". $result_item[csf('trim_group')]." and nvl(b.description,0)=nvl('". $result_itemdescription[csf('description')]."',0) and nvl(b.brand_supplier,0)=nvl('". $result_itemdescription[csf('brand_supplier')]."',0) and nvl(b.item_color,0)=nvl(".$result_color[csf('color_number_id')].",0) and nvl(b.color_number_id,0)=nvl(".$result_color[csf('gmts_color')].",0) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
					}
                foreach($nameArray_color_size_qnty as $result_color_size_qnty)
                {
                ?>
            ...
            ?>
            <tr>
                <td style="border:1px solid black;  text-align:right" colspan="3"><strong><b style="float:left">Delivery Date: <? echo change_date_format($result_item[csf('delivery_date')]);?></b> Item Total</strong></td>
                <?
				$item_total=0;
                foreach($nameArray_color  as $result_color)
                {
        ...
        <!--==============================================AS PER GMTS Color & SIZE START=========================================  -->
		<?
        $nameArray_item=sql_select( "select distinct trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=4 and status_active=1 and is_deleted=0 order by trim_group ");
        $nameArray_size=sql_select( "select  b.item_size  as gmts_sizes, min(b.color_size_table_id) as color_size_table_id  from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=4 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.item_size order by color_size_table_id");
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            {
			$i++;
			$gmtcolor_library=return_library_array( "select b.item_color,b.color_number_id from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=4 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0", "item_color", "color_number_id"  );
			 $nameArray_color=sql_select( "select  b.item_color,b.color_number_id,b.description, b.brand_supplier, min(b.color_size_table_id) as  color_size_table_id,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=4 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.item_color,b.color_number_id,b.description, b.brand_supplier order by color_size_table_id");

            $nameArray_item_description=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=4 and trim_group=".$result_item[csf('trim_group')]." and status_active=1 and is_deleted=0 order by trim_group ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo   (count($nameArray_item_description)*count($nameArray_color)); ?>">
                <? echo $i ; ?>
            ...
            ?>
            <tr>
                <td style="border:1px solid black;  text-align:right" colspan="6"><strong> <b style="float:left">Delivery Date: <? echo change_date_format($result_item[csf('delivery_date')]);?></b> Item Total</strong></td>
                <?
                foreach($nameArray_size  as $result_size)
                {
        ...
        <!--==============================================AS PER Color & SIZE  END=========================================  -->
         ...
         <!--==============================================NO NENSITIBITY START=========================================  -->
		<?
        $nameArray_item=sql_select( "select distinct trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=0 and status_active=1 and is_deleted=0 order by trim_group ");
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item)
            {
				...
				$nameArray_item_description=sql_select( "select  b.description, b.brand_supplier,b.item_color,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.trim_group=".$result_item[csf('trim_group')]." and a.delivery_date='".$result_item[csf('delivery_date')]."' and a.sensitivity=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.description, b.brand_supplier,b.item_color");
            ...
            <tr>
				...
				if($db_type==0)
				        {
						$nameArray_color_size_qnty=sql_select("select sum(b.requirment) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=0 and a.trim_group=". $result_item[csf('trim_group')]." and  b.description='". $result_itemdescription[csf('description')]."' and b.brand_supplier='".$result_itemdescription[csf('brand_supplier')]."' and b.item_color='".$result_itemdescription[csf('item_color')]."' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
						}
						if($db_type==2)
				        {
						$nameArray_color_size_qnty=sql_select("select sum(b.requirment) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=0 and a.trim_group=". $result_item[csf('trim_group')]." and nvl( b.description,0)=nvl('". $result_itemdescription[csf('description')]."',0) and nvl(b.brand_supplier,0)=nvl('".$result_itemdescription[csf('brand_supplier')]."',0) and nvl(b.item_color,0)=nvl('".$result_itemdescription[csf('item_color')]."',0) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
						}
...
?>
		...
		$nameArray_buyer=sql_select( "select  a.style_ref_no, a.job_no, a.style_owner, a.buyer_name, a.dealing_marchant,a.season,a.season_matrix,a.order_repeat_no   from wo_po_details_master a, wo_booking_dtls b where a.job_no=b.job_no and b.booking_no=$txt_booking_no ");
        foreach ($nameArray_buyer as $result_buy){
			$buyer_string[$result_buy[csf('buyer_name')]]=$buyer_name_arr[$result_buy[csf('buyer_name')]];
			$style_owner[$result_buy[csf('job_no')]]=$company_library[$result_buy[csf('style_owner')]];
			$job_no[$result_buy[csf('job_no')]]=$result_buy[csf('job_no')];
        ...
        $nameArray=sql_select( "select a.booking_no,a.booking_date,a.pay_mode,a.supplier_id,a.currency_id,a.exchange_rate,a.attention,a.delivery_date,a.source,a.remarks  from wo_booking_mst a where  a.booking_no=$txt_booking_no and a.status_active=1");
        foreach ($nameArray as $result)
        {
			$varcode_booking_no=$result[csf('booking_no')];
        ?>
       <table width="100%" style="border:1px solid black;table-layout: fixed;">
		...
		//$nameArray_job_po=sql_select( "select job_no from wo_booking_dtls  where booking_no=$txt_booking_no group by job_no order by job_no ");
		//$nameArray_job_po=sql_select( "SELECT a.job_no,a.style_ref_no,b.po_number from wo_po_details_master a join wo_po_break_down b on a.id=b.job_id join wo_booking_dtls c on a.job_no=c.job_no where c.booking_no=$txt_booking_no and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 group by a.job_no,a.style_ref_no,b.po_number order by a.job_no");
		$nameArray_job_po=sql_select( "SELECT a.job_no,a.style_ref_no,b.po_number,b.id as po_id from wo_po_details_master a, wo_po_break_down b ,wo_booking_dtls c   where  a.id=b.job_id  and  a.job_no=c.job_no and b.id=c.po_break_down_id  and  c.booking_no=$txt_booking_no and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 group by a.job_no,a.style_ref_no,b.po_number,b.id order by a.job_no ");
		foreach($nameArray_job_po as $nameArray_job_po_row){
		$nameArray_item=sql_select( "select  pre_cost_fabric_cost_dtls_id,trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id='".$nameArray_job_po_row[csf('po_id')]."'  and sensitivity=1 and status_active=1 and is_deleted=0 group by pre_cost_fabric_cost_dtls_id,trim_group,delivery_date  order by trim_group ");
	    if(count($nameArray_item)>0){
        ?>
        &nbsp;
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            <tr>
                <td colspan="9" align="">
                <strong>As Per Garments Color (<? echo "Job NO:".$nameArray_job_po_row[csf('job_no')]; ?>)&nbsp;&nbsp;PO no:(<? echo $nameArray_job_po_row[csf('po_number')]; ?>)&nbsp;&nbsp;Style no:(<? echo $nameArray_job_po_row[csf('style_ref_no')]; ?>)</strong>
                </td>
            </tr>
            <tr>
                <td style="border:1px solid black"><strong>Sl</strong> </td>
            ...
            foreach($nameArray_item as $result_item){
			$i++;
            $nameArray_item_description=sql_select( "select a.pre_cost_fabric_cost_dtls_id,min(b.id) as bid, b.description, b.brand_supplier,b.item_color,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id='".$nameArray_job_po_row[csf('po_id')]."'  and a.sensitivity=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.trim_group=".$result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and b.requirment !=0 group by a.pre_cost_fabric_cost_dtls_id, b.description, b.brand_supplier,b.item_color order by bid ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
            ...
            ?>
            <tr>
                <td style="border:1px solid black;  text-align:right" colspan="4"><strong><b style="float:left">Delivery Date: <? echo change_date_format($result_item[csf('delivery_date')]);?></b> Item Total</strong></td>
                <td style="border:1px solid black;  text-align:right"><? echo number_format($item_desctiption_total ,4);  ?></td>
                <td style="border:1px solid black;  text-align:right"></td>
                <td style="border:1px solid black; text-align:right"></td>
                <td style="border:1px solid black; text-align:right">
        ...
        <!--==============================================Size Sensitive START=========================================  -->
		<?
		$nameArray_item=sql_select( "select  pre_cost_fabric_cost_dtls_id,trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id='".$nameArray_job_po_row[csf('po_id')]."' and sensitivity=2 and status_active=1 and is_deleted=0 group by pre_cost_fabric_cost_dtls_id,trim_group,delivery_date  order by trim_group");
		if(count($nameArray_item)>0)
		{
        ?>
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            <tr>
                <td colspan="9" align="">
                <strong>Size Sensitive (<? echo "Job NO:".$nameArray_job_po_row[csf('job_no')];?>)&nbsp;&nbsp;PO no:(<? echo $nameArray_job_po_row[csf('po_number')]; ?>)&nbsp;&nbsp;Style no:(<? echo $nameArray_job_po_row[csf('style_ref_no')]; ?>)</strong>
                </td>
            </tr>
            <tr>
                <td style="border:1px solid black"><strong>Sl</strong> </td>
            ...
            {
			$i++;
			$nameArray_item_description=sql_select( "select a.pre_cost_fabric_cost_dtls_id,min(b.id) as bid, b.description, b.brand_supplier,b.item_size,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and   a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id='".$nameArray_job_po_row[csf('po_id')]."' and a.sensitivity=2 and a.trim_group=".$result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and b.requirment !=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.pre_cost_fabric_cost_dtls_id, b.description, b.brand_supplier,b.item_size order by bid");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i ; ?>
        ...
        <!--==============================================Size Sensitive END=========================================  -->
         ...
         <!--==============================================AS PER CONTRAST COLOR START=========================================  -->
		<?
		 $nameArray_item=sql_select( "select pre_cost_fabric_cost_dtls_id, trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id='".$nameArray_job_po_row[csf('po_id')]."'  and sensitivity=3 and status_active=1 and is_deleted=0 group by pre_cost_fabric_cost_dtls_id,trim_group,delivery_date order by trim_group ");
		if(count($nameArray_item)>0)
		{
        ?>
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            <tr>
                <td colspan="10" align="">
                <strong>Contrast Color (<? echo "Job NO:".$nameArray_job_po_row[csf('job_no')];?>)&nbsp;&nbsp;PO no:(<? echo $nameArray_job_po_row[csf('po_number')]; ?>)&nbsp;&nbsp;Style no:(<? echo $nameArray_job_po_row[csf('style_ref_no')]; ?>)</strong>
                </td>
            </tr>
            <tr>
                <td style="border:1px solid black"><strong>Sl</strong> </td>
            ...
            {
				$i++;
				$gmtcolor_library=return_library_array( "select b.item_color,b.color_number_id from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id='".$nameArray_job_po_row[csf('po_id')]."'  and a.sensitivity=3  and a.trim_group=".$result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and b.requirment !=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by trim_group ", "item_color", "color_number_id"  );

			$nameArray_item_description=sql_select( "select a.pre_cost_fabric_cost_dtls_id,min(b.id) as bid,b.description, b.brand_supplier,b.item_color,b.color_number_id,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id='".$nameArray_job_po_row[csf('po_id')]."' and a.sensitivity=3  and a.trim_group=".$result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and b.requirment !=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.pre_cost_fabric_cost_dtls_id, b.description, b.brand_supplier,b.item_color,b.color_number_id order by bid ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
            ...
            ?>
            <tr>
                <td style="border:1px solid black;  text-align:right" colspan="5"><strong><b style="float:left">Delivery Date: <? echo change_date_format($result_item[csf('delivery_date')]);?></b> Item Total</strong></td>
                <td style="border:1px solid black;  text-align:right"><? echo number_format($item_desctiption_total,4);  ?></td>
                <td style="border:1px solid black;text-align:center"></td>
                <td style="border:1px solid black; text-align:right"><? //echo number_format(round($color_total),0); ?></td>
                <td style="border:1px solid black; text-align:right">
        ...
        <!--==============================================AS PER GMTS Color & SIZE START=========================================  -->
		<?
		$nameArray_item=sql_select( "select  pre_cost_fabric_cost_dtls_id,trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id='".$nameArray_job_po_row[csf('po_id')]."'  and sensitivity=4 and status_active=1 and is_deleted=0 group by pre_cost_fabric_cost_dtls_id,trim_group,delivery_date order by trim_group");
	   if(count($nameArray_item)>0)
		{
        ?>
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            <tr>
                <td colspan="11" align="">
                <strong>Color & size sensitive (<? echo "Job NO:".$nameArray_job_po_row[csf('job_no')];?>)&nbsp;&nbsp;PO no:(<? echo $nameArray_job_po_row[csf('po_number')]; ?>)&nbsp;&nbsp;Style no:(<? echo $nameArray_job_po_row[csf('style_ref_no')]; ?>)</strong>
                </td>
            </tr>
            <tr>
                <td style="border:1px solid black"><strong>Sl</strong> </td>
            ...
            {
				$i++;
				$gmtcolor_library=return_library_array( "select b.item_color,b.color_number_id from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id='".$nameArray_job_po_row[csf('po_id')]."' and a.sensitivity=4    and a.trim_group=".$result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and b.requirment !=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by trim_group ", "item_color", "color_number_id"  );

			 $nameArray_color=sql_select( "select a.pre_cost_fabric_cost_dtls_id,min(b.id) as bid, b.item_color as color_number_id,b.item_size,b.description, b.brand_supplier,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id='".$nameArray_job_po_row[csf('po_id')]."' and a.trim_group=".$result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and a.sensitivity=4 and b.requirment !=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.pre_cost_fabric_cost_dtls_id, b.item_color,b.item_size,b.description, b.brand_supplier order by bid ");

            $nameArray_item_description=sql_select( "select distinct uom from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id='".$nameArray_job_po_row[csf('po_id')]."'  and sensitivity=4 and trim_group=".$result_item[csf('trim_group')]." and pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and status_active=1 and is_deleted=0 order by trim_group ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo   (count($nameArray_item_description)*count($nameArray_color)); ?>">
                <? echo $i ; ?>
            ...
            ?>
            <tr>
                <td style="border:1px solid black;  text-align:right" colspan="8"><strong> <b style="float:left">Delivery Date: <? echo change_date_format($result_item[csf('delivery_date')]);?></b> Item Total</strong></td>
                <td style="border:1px solid black;  text-align:right"><? echo number_format($item_desctiption_total,4);  ?></td>
                <td style="border:1px solid black;  text-align:right"><? //echo number_format(array_sum($color_tatal),2);  ?></td>
	   ...
	   ?>
		...
		<?
       //$nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=0 order by trim_group ");
		$nameArray_item=sql_select( "select  pre_cost_fabric_cost_dtls_id,trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id='".$nameArray_job_po_row[csf('po_id')]."'  and sensitivity=0 and status_active=1 and is_deleted=0 group by pre_cost_fabric_cost_dtls_id,trim_group,delivery_date order by trim_group ");
        //$nameArray_color=sql_select( "select distinct b.color_number_id from wo_trims_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=$txt_booking_no and a.sensitivity=1");
		//$nameArray_color= array();
		if(count($nameArray_item)>0)
		{
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            <tr>
                <td colspan="8" align="">
                <strong>No Sensitivity  (<? echo "Job NO:".$nameArray_job_po_row[csf('job_no')];?>) &nbsp;&nbsp;PO no:(<? echo $nameArray_job_po_row[csf('po_number')]; ?>)&nbsp;&nbsp;Style no:(<? echo $nameArray_job_po_row[csf('style_ref_no')]; ?>)</strong>
                </td>
            </tr>
            <tr>
                <td style="border:1px solid black"><strong>Sl</strong> </td>
            ...
            foreach($nameArray_item as $result_item)
            {
				...
				$nameArray_item_description=sql_select( "select a.pre_cost_fabric_cost_dtls_id, b.description, b.brand_supplier,b.item_color,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id='".$nameArray_job_po_row[csf('po_id')]."' and a.trim_group=".$result_item[csf('trim_group')]." and a.delivery_date='".$result_item[csf('delivery_date')]."' and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and a.sensitivity=0  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.pre_cost_fabric_cost_dtls_id, b.description, b.brand_supplier,b.item_color");
            ...
            <tr>
			   ...
			   ?>
				...
				if($db_type==0)
				        {
						$nameArray_color_size_qnty=sql_select("select sum(b.requirment) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id='".$nameArray_job_po_row[csf('po_id')]."'  and a.sensitivity=0  and a.trim_group=". $result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and  b.description='". $result_itemdescription[csf('description')]."' and b.brand_supplier='".$result_itemdescription[csf('brand_supplier')]."' and b.item_color='".$result_itemdescription[csf('item_color')]."' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
						}
						if($db_type==2)
				        {
						$nameArray_color_size_qnty=sql_select("select sum(b.requirment) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id='".$nameArray_job_po_row[csf('po_id')]."'  and a.sensitivity=0 and a.trim_group=". $result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and nvl( b.description,0)=nvl('". $result_itemdescription[csf('description')]."',0) and nvl(b.brand_supplier,0)=nvl('".$result_itemdescription[csf('brand_supplier')]."',0) and nvl(b.item_color,0)=nvl('".$result_itemdescription[csf('item_color')]."',0) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
...
if($action=="show_trim_booking_report_gr"){
	extract($_REQUEST);
	$cbo_company_name=str_replace("'","",$cbo_company_name);
	$imge_arr=return_library_array( "select master_tble_id,image_location from   common_photo_library where form_name='company_details' and file_type=1",'master_tble_id','image_location');
	$country_arr=return_library_array( "select id,country_name from   lib_country",'id','country_name');
	...
	<div style="width:1033px" align="center">
	   ...
	   if(count($nameArray_item)>0){
        ...
        <table style="border:1px solid black" align="left" class=""  cellpadding="0" width="100%" cellspacing="0" rules="all" >
           <tr>
            ...
            foreach($nameArray_item as $result_item){
				...
				$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,b.item_color, b.color_number_id ,min(b.color_size_table_id) as color_size_table_id,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and  a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=1 and   a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and b.requirment !=0 and b.requirment>0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.description, b.brand_supplier,b.item_color, b.color_number_id order by color_size_table_id ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
        ...
        <table style="border:1px solid black" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item){
			$i++;
			$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,b.item_size,b.gmts_sizes,min(b.color_size_table_id) as color_size_table_id, sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id=b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=2 and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and b.requirment>0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.description, b.brand_supplier,b.item_size, b.gmts_sizes order by color_size_table_id");
			$sql_trim_amount=sql_select("select  cons_uom, cons_dzn_gmts, rate, amount from  wo_pre_cost_trim_cost_dtls where job_no='".$nameArray_job_po_row[csf('job_no')]."'  and trim_group=".$result_item[csf('trim_group')]." and status_active=1 and is_deleted=0 ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i ; ?>
              ...
              <? 
              	...
              	$art_no=rtrim($article_numberArr[$result_itemdescription[csf('gmts_sizes')]]['article_number'],',');
                	//$art_nos=implode(",",array_unique(explode(",",$art_no)));
                	$art_no_arr=array_unique(explode(",",$art_no));
        ...
        <table style="border:1px solid black" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
           <tr>
            ...
            foreach($nameArray_item as $result_item){
				$i++;
				$gmtcolor_library=return_library_array( "select b.item_color,b.color_number_id from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=3 and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and b.requirment !=0  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by trim_group ", "item_color", "color_number_id"  );
				$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,b.item_color,b.color_number_id,sum(b.requirment) as cons,min(b.color_size_table_id) as color_size_table_id,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=3 and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and b.requirment !=0  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0group by b.description, b.brand_supplier,b.item_color,b.color_number_id order by color_size_table_id ");
				$sql_trim_amount=sql_select("select  cons_uom, cons_dzn_gmts, rate, amount from  wo_pre_cost_trim_cost_dtls where job_no='".$nameArray_job_po_row[csf('job_no')]."'  and trim_group=".$result_item[csf('trim_group')]." and status_active=1 and is_deleted=0");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
	   ...
	   if(count($nameArray_item)>0)
        ...
        <table style="border:1px solid black" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            {
			$i++;
			$nameArray_color=sql_select( "select  b.item_color,b.color_number_id,b.item_size,b.gmts_sizes,min(b.color_size_table_id) as color_size_table_id,b.description, b.brand_supplier,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id   and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=4 and b.requirment !=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.item_color,b.color_number_id,b.item_size,b.gmts_sizes,b.description, b.brand_supplier order by color_size_table_id ");
            $nameArray_item_description=sql_select( "select distinct uom from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=4   and trim_group=".$result_item[csf('trim_group')]." and status_active=1 and is_deleted=0 order by trim_group ");
			$sql_trim_amount=sql_select("select  cons_uom, cons_dzn_gmts, rate, amount from  wo_pre_cost_trim_cost_dtls where job_no='".$nameArray_job_po_row[csf('job_no')]."'  and trim_group=".$result_item[csf('trim_group')]." and status_active=1 and is_deleted=0");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo   (count($nameArray_color)); ?>">
				...
				foreach($nameArray_color as $result_color)
                {
					...
					<td style="border:1px solid black; text-align:right">
		              <? 
		              	...
		              	$art_no=rtrim($article_numberArr[$result_color[csf('gmts_sizes')]]['article_number'],',');
		                	//$art_nos=implode(",",array_unique(explode(",",$art_no)));
		                	$art_no_arr=array_unique(explode(",",$art_no));
	   ...
	   ?>
        ...
        <table  style="border:1px solid black" align="left" class=""  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item)
            {
				...
				$nameArray_item_description=sql_select( "select  b.description, b.brand_supplier,b.item_color,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and  a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and a.sensitivity=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.description, b.brand_supplier,b.item_color");
           ...
           <?
				...
				//echo	$sql="select job_no,po_break_down_id from wo_booking_dtls where booking_no=$txt_booking_no and status_active=1 and is_deleted=0 group by job_no,po_break_down_id";
                       $nameArray=sql_select( $sql );
						...
						list($sql_cu_woq_row_short)=$sql_cu_woq_short;
						$cu_woq_amount_short=$sql_cu_woq_row_short[csf('amount')];*/
						$sql_cu_woq_short=sql_select("select sum(amount) as amount,exchange_rate  from wo_booking_dtls where po_break_down_id='".$selectResult[csf('po_break_down_id')]."'  and  booking_type=2 and booking_no!=$txt_booking_no and is_short=1 and status_active=1 and is_deleted=0 group by exchange_rate");

						$cu_woq_amount_short=0;
						foreach($sql_cu_woq_short as $row)
						{
							$cu_woq_amount_short+=$row[csf("amount")]/$row[csf("exchange_rate")];
						}
...
if($action=="tna_variable_setting")
{
$tna_status=return_field_value("tna_integrated","variable_order_tracking","company_name ='$data' and variable_list=14  and is_deleted=0 and status_active=1");
echo trim($tna_status);
}
...
if($action=="show_trim_booking_report_wg")
{
	...
	$company_library=return_library_array("select id, company_name from lib_company", "id", "company_name");
    $country_short_name_arr=return_library_array( "select id,short_name from   lib_country",'id','short_name');
	$trim_group_library= return_library_array("select id, item_name from lib_item_group",'id','item_name');
	$imge_arr=return_library_array( "select master_tble_id,image_location from   common_photo_library where form_name='company_details' and file_type=1",'master_tble_id','image_location');
	$country_arr=return_library_array( "select id,country_name from   lib_country",'id','country_name');
	$supplier_name_arr=return_library_array( "select id,supplier_name from   lib_supplier",'id','supplier_name');
    ...
    <thead>
     ...
     <tbody>
		...
		foreach($nameArray_job_po as $nameArray_job_po_row){
			$job_po_ids=implode(",",$job_po_no_arr[$nameArray_job_po_row[csf('job_no')]]);
		$nameArray_item=sql_select( "select  pre_cost_fabric_cost_dtls_id,trim_group,country_id_string from wo_booking_dtls  where booking_no='$txt_booking_no' and  status_active =1 and is_deleted=0 and job_no='".$nameArray_job_po_row[csf('job_no')]."' and  po_break_down_id in ($job_po_ids)   and sensitivity=1 group by pre_cost_fabric_cost_dtls_id,trim_group,country_id_string  order by trim_group ");
	    if(count($nameArray_item)>0){
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item){
			$i++;
            $nameArray_item_description=sql_select( "select a.pre_cost_fabric_cost_dtls_id,min(b.id) as bid, b.description, b.brand_supplier,b.item_color,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no='$txt_booking_no' and  a.po_break_down_id in ($job_po_ids) and a.job_no='".$nameArray_job_po_row[csf('job_no')]."'  and a.sensitivity=1  and a.trim_group=".$result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and b.requirment !=0 and  a.status_active =1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.pre_cost_fabric_cost_dtls_id, b.description, b.brand_supplier,b.item_color order by bid ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
				...
				$country_name_string="";
				for($co=0; $co <count($country_arr);$co++){
				$country_name_string.=$country_short_name_arr[$country_arr[$co]].",";
				}
				echo rtrim($country_name_string,',');
				?>
                </td>
        ...
        <!--==============================================Size Sensitive START=========================================  -->
		<?
		$nameArray_item=sql_select( "select  pre_cost_fabric_cost_dtls_id,trim_group,country_id_string from wo_booking_dtls  where booking_no='$txt_booking_no' and job_no='".$nameArray_job_po_row[csf('job_no')]."' and  po_break_down_id in ($job_po_ids) and sensitivity=2 and  status_active =1 and  is_deleted=0 group by pre_cost_fabric_cost_dtls_id,trim_group,country_id_string  order by trim_group ");
		if(count($nameArray_item)>0)
		{
			$po_ids=rtrim($po_no_arr[$nameArray_job_po_row[csf('job_no')]]['po_id']);
			$po_ids=array_unique(explode(",",$po_ids));
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            {
			$i++;
			$nameArray_item_description=sql_select( "select a.pre_cost_fabric_cost_dtls_id,min(b.id) as bid, b.description, b.brand_supplier,b.item_size,b.article_number,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and   a.booking_no='$txt_booking_no' and  a.po_break_down_id in ($job_po_ids) and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.sensitivity=2 and a.trim_group=".$result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and b.requirment !=0 and  a.status_active =1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.pre_cost_fabric_cost_dtls_id, b.description, b.brand_supplier,b.item_size,b.article_number order by bid");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i ; ?>
        ...
        <!--==============================================Size Sensitive END=========================================  -->
         ...
         <!--==============================================AS PER CONTRAST COLOR START=========================================  -->
		<?
		 $nameArray_item=sql_select( "select pre_cost_fabric_cost_dtls_id, trim_group,country_id_string from wo_booking_dtls  where booking_no='$txt_booking_no' and job_no='".$nameArray_job_po_row[csf('job_no')]."'  and  po_break_down_id in ($job_po_ids) and sensitivity=3 and  status_active =1 and is_deleted=0 group by pre_cost_fabric_cost_dtls_id,trim_group,country_id_string order by trim_group ");
		if(count($nameArray_item)>0)
		{
			$po_ids=rtrim($po_no_arr[$nameArray_job_po_row[csf('job_no')]]['po_id']);
			$po_ids=array_unique(explode(",",$po_ids));
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            {
				$i++;
				$gmtcolor_library=return_library_array( "select b.item_color,b.color_number_id from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no='$txt_booking_no' and  a.po_break_down_id in ($job_po_ids) and a.job_no='".$nameArray_job_po_row[csf('job_no')]."'  and a.sensitivity=3  and a.trim_group=".$result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and b.requirment !=0 and  a.status_active =1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  order by trim_group ", "item_color", "color_number_id"  );

			$nameArray_item_description=sql_select( "select a.pre_cost_fabric_cost_dtls_id,min(b.id) as bid,b.description, b.brand_supplier,b.item_color,b.color_number_id,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no='$txt_booking_no' and  a.po_break_down_id in ($job_po_ids) and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.sensitivity=3  and a.trim_group=".$result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and b.requirment !=0 and  a.status_active =1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.pre_cost_fabric_cost_dtls_id, b.description, b.brand_supplier,b.item_color,b.color_number_id order by bid ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
        ...
        <!--==============================================AS PER GMTS Color & SIZE START=========================================  -->
		<?
		$nameArray_item=sql_select( "select  pre_cost_fabric_cost_dtls_id,trim_group,country_id_string from wo_booking_dtls  where booking_no='$txt_booking_no' and job_no='".$nameArray_job_po_row[csf('job_no')]."' and  po_break_down_id in ($job_po_ids) and sensitivity=4 and  status_active =1 and is_deleted=0 group by pre_cost_fabric_cost_dtls_id,trim_group,country_id_string order by trim_group");
	   if(count($nameArray_item)>0)
		{
			$po_ids=rtrim($po_no_arr[$nameArray_job_po_row[csf('job_no')]]['po_id']);
			$po_ids=array_unique(explode(",",$po_ids));
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            {
				$i++;
				$gmtcolor_library=return_library_array( "select b.item_color,b.color_number_id from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and  a.booking_no='$txt_booking_no' and  a.po_break_down_id in ($job_po_ids) and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.sensitivity=4    and a.trim_group=".$result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and b.requirment !=0 and  a.status_active =1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by trim_group ", "item_color", "color_number_id"  );

			 $nameArray_color=sql_select( "select a.pre_cost_fabric_cost_dtls_id,min(b.id) as bid, b.color_number_id as gmt_color, b.item_color as color_number_id,b.item_size,b.gmts_sizes,b.description, b.brand_supplier,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount, min(c.color_order) as color_order, min(c.size_order) as size_order,c.article_number from wo_booking_dtls a, wo_trim_book_con_dtls b,wo_po_color_size_breakdown c  where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and b.po_break_down_id=c.po_break_down_id and b.color_number_id=c.color_number_id and b.gmts_sizes=c.size_number_id and  c.id=b.color_size_table_id and a.booking_no='$txt_booking_no' and  a.po_break_down_id in ($job_po_ids) and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.trim_group=".$result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and a.sensitivity=4 and b.requirment !=0 and  a.status_active =1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.pre_cost_fabric_cost_dtls_id, b.color_number_id, b.item_color,b.item_size,b.gmts_sizes,b.description, b.brand_supplier,c.article_number order by c.article_number,color_order,size_order");



            $nameArray_item_description=sql_select( "select distinct uom from wo_booking_dtls  where booking_no='$txt_booking_no' and job_no='".$nameArray_job_po_row[csf('job_no')]."' and  po_break_down_id in ($job_po_ids)  and sensitivity=4 and trim_group=".$result_item[csf('trim_group')]." and pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and  status_active =1 and is_deleted=0 order by trim_group ");
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo   (count($nameArray_item_description)*count($nameArray_color)); ?>">
                <? echo $i ; ?>
	   ...
	   ?>
        <!--==============================================AS PER Color & SIZE  END=========================================  -->
         ...
         <!--==============================================NO NENSITIBITY START=========================================  -->
		<?
		$nameArray_item=sql_select( "select  pre_cost_fabric_cost_dtls_id,trim_group,country_id_string from wo_booking_dtls  where booking_no='$txt_booking_no' and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id in ($job_po_ids)  and sensitivity=0 and  status_active =1 and is_deleted=0 group by pre_cost_fabric_cost_dtls_id,trim_group,country_id_string order by trim_group ");
		if(count($nameArray_item)>0)
		{
			$po_ids=rtrim($po_no_arr[$nameArray_job_po_row[csf('job_no')]]['po_id']);
			$po_ids=array_unique(explode(",",$po_ids));
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item)
            {
				...
				$nameArray_item_description=sql_select( "select a.pre_cost_fabric_cost_dtls_id, b.description, b.brand_supplier,b.item_color,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no='$txt_booking_no' and  a.po_break_down_id in ($job_po_ids) and a.job_no='".$nameArray_job_po_row[csf('job_no')]."'  and a.trim_group=".$result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and a.sensitivity=0 and  a.status_active =1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.pre_cost_fabric_cost_dtls_id, b.description, b.brand_supplier,b.item_color");
            ...
            <tr>
			   ...
			   ?>
				...
				if($db_type==0)
				        {
						$nameArray_color_size_qnty=sql_select("select sum(b.requirment) as cons from wo_booking_dtls a, wo_trim_book_con_dtls b  where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no='$txt_booking_no' and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and  a.po_break_down_id in ($job_po_ids)  and a.sensitivity=0 and  a.status_active =1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.trim_group=". $result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and  b.description='". $result_itemdescription[csf('description')]."' and b.brand_supplier='".$result_itemdescription[csf('brand_supplier')]."' and b.item_color='".$result_itemdescription[csf('item_color')]."'");
						}
						if($db_type==2)
				        {
