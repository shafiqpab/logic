            	...
            	<table  cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
        			...
        			<tr>
                    	<td> <input type="hidden" id="selected_job">
							<? 
								echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", '',"load_drop_down( 'stripe_color_measurement_controller_urmi', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
							?>
                    </td>
                   	<td id="buyer_td">
                     <? 
    	...
    	<fieldset style="width:810px;">
            ...
            <input type="hidden" id="tr_ortder" name="tr_ortder" value="" width="500" /> 
            	...
            	<table width="810" cellspacing="0" class="rpt_table" border="0" id="tbl_fabric_cost" rules="all">
					...
					$fab_description_array=sql_select("select id, body_part_id, color_type_id, fabric_description from wo_pre_cost_fabric_cost_dtls where job_no='$data[0]' and  	 color_type_id in (2,3,4,6,31,32,33,34)");
...
<table width="680" cellspacing="0" class="rpt_table" border="0" id="tbl_set_details" rules="all">
	...
	$save_update=1;
	$sql_data=sql_select("select stripe_color, measurement, uom,totfidder,fabreq,fabreqtotkg,yarn_dyed from  wo_pre_stripe_color where pre_cost_fabric_cost_dtls_id=$fabric_cost_id and color_number_id=$cbo_color_name");
	if(count($sql_data)>0)
	{
		$i=1;
		$totmeasurement=0;
...
if($action=="save_update_delete")
{
	...
	$field_array="id,job_no,item_number_id,pre_cost_fabric_cost_dtls_id,color_number_id,stripe_color,measurement,uom,totfidder,fabreq,fabreqtotkg,yarn_dyed,inserted_by,insert_date";
	...
	/*if (!in_array(str_replace("'","",$$stcolor),$new_array_color)){
		$color_id = return_id( str_replace("'","",$$stcolor), $color_library, "lib_color", "id,color_name");  
		$new_array_color[$color_id]=str_replace("'","",$$stcolor);
	}
	else{
		$color_id =  array_search(str_replace("'","",$$stcolor), $new_array_color);
	}*/
