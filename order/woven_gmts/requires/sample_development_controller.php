...
{ 
	...
	{
	echo create_drop_down( "cbo_season_name", 158, "select id,season_name from lib_buyer_season  where status_active =1 and is_deleted=0 ","id,season_name", 1, "-- Select Season --", $selected,"" );
	}
	else
	{
		echo create_drop_down( "cbo_season_name", 158, " ","", 1, "-- Select Season --", $selected,"" );
	}
...
{
	echo create_drop_down( "cbo_season_name", 172, "select id,season_name from lib_buyer_season  where status_active =1 and is_deleted=0 ","id,season_name", 1, "-- Select Season --", $selected,"" );
}
if ($action=="load_drop_down_season_buyer")
{
	...
	echo create_drop_down( "cbo_season_name", 158, "select a.id,a.season_name from lib_buyer_season a,lib_buyer_tag_company b where a.buyer_id=b.buyer_id and a.status_active =1 and a.is_deleted=0 and a.buyer_id='$datas[0]' and b.tag_company='$datas[1]'","id,season_name", 1, "-- Select Season --", $selected, "" );
	exit();
}
if ($action=="load_drop_down_buyer")
{ //$selected
	echo create_drop_down( "cbo_buyer_name", 157, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond  and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,30,90)) order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", "", "load_drop_down( 'requires/sample_development_controller', this.value, 'cbo_sample_type', 'sampletd' );load_drop_down( 'requires/sample_development_controller', this.value+'_'+document.getElementById('cbo_company_name').value, 'load_drop_down_season_buyer', 'season_td');" ); 
}
...
else if ($action=="cbo_sample_type")
{
    ...
    echo create_drop_down( "cbo_sample_type", 157, $sample_library,"", '1', "--Select--", '', "",'','' );
	exit();
}
...
else if ($action=="load_php_data_to_form_sample_development_details_info")
{
	...
	$qry_size="select id, mst_id, dtls_id, size_id, size_qty,bh_qty from sample_development_size where dtls_id='$data'";
            		...
            		<table  cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
        				...
        				<tr>
                        	<td width="140"> 
								...
								<? 
                                    echo create_drop_down( "cbo_company_mst", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", '',"load_drop_down( 'sample_development_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                                ?>
                    		</td>
...
//LEFT JOIN pro_ex_factory_mst d on  c.po_break_down_id=d.po_break_down_id //sample_development_size
                      $sql_qry="Select a.id as dtls_id,a.sample_name,a.sample_color,b.size_id, sum(b.size_qty) as size_qty,sum(b.bh_qty) as bh_qty,a.fabrication,a.fabric_sorce,a.comments,a.comments,a.factory_dead_line,a.buyer_req_no from sample_development_dtls a LEFT JOIN sample_development_size b on  a.sample_mst_id=mst_id and a.id=b.dtls_id and b.is_deleted=0  where a.is_deleted=0 and a.sample_mst_id=$data[2] group by a.id,a.sample_name,a.fabrication,a.fabric_sorce,a.factory_dead_line,a.comments,a.buyer_req_no,a.sample_color,b.size_id order by a.id";
                ...
                <td>
                	 <table align="left" cellspacing="0" border="0" width="100%" >
                     	<tr>
                            ...
                            <td width="10">&nbsp;&nbsp;NO</td>
                            <td width="10"  style="border:solid 1px;"> </td>
                            <td  width="50">&nbsp;&nbsp;Print/Emb.</td> <td width="5">:</td>
                            <td width="10">YES</td>
                            <td width="10"  style="border:solid 1px"> </td>
                            <td width="10">&nbsp;&nbsp;NO</td>
                            <td width="10"  style="border:solid 1px"> </td>
    ...
    <script>
		...
		function fnc_size_entry(operation)
		{
			...
			for(i=1; i<=tot_row; i++)
			{
				...
				{
					j++;
					data_all+=get_submitted_data_string('txtsizename_'+i+'*txtgmtpcs_'+i+'*txtgmtbhqty_'+i+'*sizeupid_'+i,"../../../",i);
				}
			}
			if(data_all=='')
			{
...
if ($action=="save_update_delete_size")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$field_array_size="id, mst_id, dtls_id,size_id, size_qty,bh_qty, inserted_by, insert_date, status_active, is_deleted";
		$data_array_size=''; 
		for($i=1;$i<=$tot_row; $i++)
		{
			...
			$data_array_size.="$add_comma(".$sizeid.",".$mainupid.",".$dtlsupid.",".$size_id.",".$$txtgmtpcs.",".$$txtgmtbhqty.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			$sizeid=$sizeid+1;
		}
