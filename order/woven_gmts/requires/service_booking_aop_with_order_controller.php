...
if ($action=="save_update_delete_dtls")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		if($db_type==0)
		{
         ...
         gmts_color_id,description,uom,process,wo_qnty,rate,amount,delivery_date,delivery_end_date,dia_width,fin_dia,inserted_by,
		 insert_date";
		 ...
		 for ($i=1;$i<=$row_num;$i++)
		 {
			 ...
			 ".$txt_booking_no.",3,".$color_id.",".$$gmts_color_id.",".$$fabric_description_id.",".$$uom.",
		 ...
		 for ($i=1;$i<=$row_num;$i++)
		 {
			 ...
			 foreach($json_data->$precostid->$colorid->$dia->po_id as $poId)
			 {
				 if($woq>0)
				 {
					 ...
					 $amount=$wQty*$rate;
					 if ($j!=1) $data_array1 .=",";
					$data_array1 .="(".$id_dtls.",".$cbo_is_short.",".$$job_no.",".$$fabric_description_id.",".$$artworkno.",".$poId.",".$txt_booking_no.",3,".$color_id.",".$colorid.",".$$fabric_description_id.",".$$uom.",35,".$wQty.",".$$txt_rate.",".$amount.",".$$startdate.",".$$enddate.",'".$dia."',".$$findia.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
					$id_dtls=$id_dtls+1;
					$j++;
				 }
			 }
...
if ($action=="populate_data_from_search_popup"){
	 $sql= "select booking_no,booking_date,company_id,buyer_id,job_no,po_break_down_id,item_category,fabric_source,currency_id,exchange_rate,pay_mode,booking_month,supplier_id,attention,delivery_date,source,booking_year,cbo_level,is_short from wo_booking_mst  where booking_no='$data'"; 
