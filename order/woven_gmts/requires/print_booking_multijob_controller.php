...
if ($action=="create_fnc_process_data")
{
	...
	$ref_no=$data[9];
	$booking_no=$data[10];
	if($txt_style=="" && $txt_order_search=="" && $txt_job=="" && $ref_no=="" && $cbo_item==0 )
	{
		echo "<b> Please select/write search category</b>";die;
	}
	...
	else if($db_type==2 || $db_type==0) $year_cond=" and to_char(a.insert_date,'YYYY')=$cbo_year_selection";
	...
	<div style="width:1280px; overflow-y:scroll; max-height:350px;" id="buyer_list_view" >
		...
		$sql_vari_lib="select item_category_id, variable_list, excut_source from variable_order_tracking where company_name=".$company_id." and item_category_id in(16,15)  and variable_list=72 and status_active=1"; 
		...
		//=========Booking Rate and Supplier Rate====================
		// echo $source_from.'='.$woven_category_id;
		if($source_from==2 && $woven_category_id==15 || $woven_category_id==16) //Lib Booking souece of Rate and Subpplier From Sourcing Post Cost sheet
		{
			$sql_supp="select c.emb_id as emb_id,c.supplier_id from wo_po_details_master a,wo_pre_cost_emb_supp_sourcing c where   a.id=c.job_id and c.supplier_id in($cbo_supplier_name) and a.company_name=$company_id and a.buyer_name=$cbo_buyer_name  and c.is_deleted=0 and c.status_active=1 $job_cond ";

			$sql_suppRes=sql_select( $sql_supp ); $emblish_id="";
			foreach($sql_suppRes as $row)
			{
				$emblish_id.=$row[csf('emb_id')].",";
			}
			unset($sql_suppRes);
			$embl_ids=chop($emblish_id,',');
			if($emblish_id!="") $embl_idCond="and c.id in ($embl_ids)"; else $embl_idCond=" and (c.sourcing_nominated_supp is null or c.sourcing_nominated_supp='0')";
		}
		else  //Lib Booking souece of Rate and Subpplier From Budget
		{
			$sql_supp="select c.id as fabric_id,c.nominated_supp_multi from wo_po_details_master a,wo_pre_cost_embe_cost_dtls c where   a.id=c.job_id and c.nominated_supp_multi like '%$cbo_supplier_name%' and a.company_name=$company_id and a.buyer_name=$cbo_buyer_name  and c.is_deleted=0 and c.status_active=1 $job_cond ";
			...
			if($fabric_id!="") $fabric_idCond="and c.id in ($fabric_ids)"; else $fabric_idCond=" and (c.nominated_supp_multi is null or c.nominated_supp_multi='0')";
		}
		
        $sql="select a.job_no_prefix_num, $year_field,a.job_no, a.company_name, a.buyer_name, a.brand_id, a.season_buyer_wise, a.season_year, a.currency_id, a.style_ref_no, b.costing_per, b.exchange_rate, c.id as precost_emb_id, c.emb_name, c.emb_type,c.body_part_id, c.rate, d.id as po_id, d.po_number, d.file_no, d.grouping, d.po_quantity as plan_cut, min(e.id) as id, e.po_break_down_id,e.item_number_id, avg(e.requirment) AS cons from wo_po_details_master a, wo_pre_cost_mst b, wo_pre_cost_embe_cost_dtls c, wo_po_break_down d, wo_pre_cos_emb_co_avg_con_dtls e where a.job_no=b.job_no and a.job_no=c.job_no and a.job_no=d.job_no_mst and a.job_no=e.job_no and c.id=e.pre_cost_emb_cost_dtls_id and d.id=e.po_break_down_id and a.company_name=$company_id and a.buyer_name=$cbo_buyer_name  and d.shiping_status!=3 and d.is_deleted=0 and d.status_active=1 ".$buyer_cond_test." $itemgroup_cond $job_cond $order_cond $ref_cond $style_cond $sourcingAppCond $year_cond $embl_idCond group by a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.brand_id, a.season_buyer_wise, a.season_year, a.currency_id, a.style_ref_no, b.costing_per, b.exchange_rate, c.id, c.emb_name,c.emb_type,c.body_part_id, c.rate, a.insert_date, d.id, d.po_number, d.file_no, d.grouping, d.po_quantity, e.po_break_down_id, e.item_number_id order by d.id, c.id";
...
if ($action=="generate_fabric_booking")
{
	...
	$sql_vari_lib="select item_category_id,variable_list,excut_source  from variable_order_tracking where company_name=".$cbo_company_name." and item_category_id in(16,15)  and variable_list=72 and status_active=1"; 
	$result_vari_lib=sql_select($sql_vari_lib);
	...
	<script>
		...
		function calculate_avg_rate(){
			...
			//var avg_rate=number_format_common((amount_sum/woqty_sum),5,0);
				var avg_rate=number_format((amount_sum/woqty_sum),4,'.','');
							...
							foreach( $data_array as $row ){
								...
								?>
									<tr id="break_1" align="center">
                                        ...
                                        <td>
                                        	<input type="text" id="rate_<? echo $i;?>"  name="rate_<? echo $i;?>" class="text_boxes_numeric" style="width:120px" onChange="calculate_amount(<? echo $i;?>);set_sum_value( 'amount_sum', 'amount_' );copy_value(this.value,'rate_',<? echo $i;?>) " value="<? echo  fn_number_format($rate,4); ?>" <? if( $piNumber || $recvNumber ){ echo  "disabled";}else { echo "";} ?> />
                                        </td>
                                        <td>
							...
							foreach( $data_array as $row ){
								...
								?>
									<tr id="break_1" align="center">
                                        ...
                                        <td><input type="text" id="rate_<? echo $i;?>"  name="rate_<? echo $i;?>" class="text_boxes_numeric" style="width:120px" onChange="calculate_amount(<? echo $i;?>);set_sum_value( 'amount_sum', 'amount_' );copy_value(this.value,'rate_',<? echo $i;?>) " value="<? echo fn_number_format($rate,4); ?>" <? if( $piNumber || $recvNumber ){ echo  "disabled";}else { echo "";} ?> />
                                        </td>
...
/*	echo '<pre>';
	...
	if ( count($data_array)>0)
	{
		...
		foreach( $data_array as $row )
		{
			...
			if($update_dtls_id=="")  $update_dtls_id=0;
			...
			if($cbo_colorsizesensitive==1 || $cbo_colorsizesensitive==3 ){
				...
				if($balance_woq>0 && $update_dtls_id==0) 
				{
				...
				if($update_dtls_id>0)  //Update //2023-Issue-123 //For Json populate.
				{
					$req_qnty_ordUom = def_number_format((($data[14]/$data[8])*$txt_req_quantity),5,"");
					$txtwoq_cal = def_number_format($req_qnty_ordUom,5,"");
					$amount = def_number_format($txtwoq_cal*$txt_avg_price,5,"");
...
if ($action=="save_update_delete_dtls_job_level")
{
	...
	if ($operation==0){
		...
		for ($i=1;$i<=$total_row;$i++){
			...
			$pre_emb_id_arr[$txtembcostId]=$txtembcostId;
			$poArr[$poid]=$poid;					
			$gmtArr[$gmtItem]=$gmtItem;
		...
		$sensitivity_check=sql_select("SELECT sensitivity,po_break_down_id from wo_booking_dtls where po_break_down_id in (".implode(",",$poArr).") and pre_cost_fabric_cost_dtls_id in(".implode(",",$pre_emb_id_arr).")  and status_active=1 and is_deleted=0 and booking_type=6 and is_short=2 group by sensitivity,po_break_down_id");
		foreach ($sensitivity_check as $row) {
	
			$sen_chk_arr[$row[csf('po_break_down_id')]]['sensitivity'] = $row[csf('sensitivity')];
			$sen_chk_arr[$row[csf('po_break_down_id')]]['po_id'] = $row[csf('po_id')];
		}
		if(count($sen_chk_arr)>0)
		{
			foreach ($poidArr as $poId) {
				if(($sen_chk_arr[$poId]['sensitivity'] != $senArr[$poId]) && ($sen_chk_arr[$poId]['po_id']==$poId)){
					echo "samesen**=";
					 disconnect($con);
				}
			}
		...
		for ($i=1;$i<=$total_row;$i++){
			...
			foreach($strdata->$job->$embcostid->$gmtitemid->po_id as $poId){
				...
				$data_array1 ="(".$id_dtls.",".$update_id.",".$$txtembcostid.",".$poId.",".$$txtjob_id.",".$$txtgmtitemid.",".$txt_booking_no.",6,2,".$$txtuom.",".$$cbocolorsizesensitive.",".$wqQty.",".$$txtexchrate.",".$$txtrate.",".$amount.",".$$txtddate.",".$$txtcountry.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				//echo "10** insert into wo_booking_dtls (".$field_array1.") values ".$data_array1; die;
				$rID1=sql_insert("wo_booking_dtls",$field_array1,$data_array1,0);
				//	CONS break down===============================================================================================
				if(str_replace("'",'',$$consbreckdown) !=''){
...
if ($action=="show_trim_booking")
{
	...
	foreach ($nameArray as $infr)
	{
		...
		$job_and_trimgroup_level[$infr[csf('job_no')]][$pre_cost_emb_id][$infr[csf('item_number_id')]]['style_ref_no'][$infr[csf('po_id')]]=$infr[csf('style_ref_no')];
	...
	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1600" class="rpt_table" id="tbl_list_search" >
        <tbody>
        <?
        if($cbo_level==1)
        {
            foreach ($nameArray as $selectResult)
            {
                ...
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>">
                    ...
                    <input type="hidden" id="txtstyleref_<? echo $i;?>" value="<? echo $selectResult[csf('style_ref_no')];?>" readonly/>
                    </td>
            ...
            foreach ($job_and_trimgroup_level as $job_no)
            {
                ...
                foreach ($wo_pre_cost_trim_cost_dtlsArr as $gmtItem=>$wo_pre_cost_trim_cost_dtls)
                {
                    ...
                    <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>">
						...
						<td width="100">
					<? echo $style_ref_no;?>
                    <input type="hidden" id="txtstyleref_<? echo $i;?>" value="<? echo $style_ref_no;?>" readonly/>
                    </td>
	...
	<table width="1600" class="rpt_table" border="0" rules="all">
        <tfoot>
            <tr>
                <th width="40">&nbsp;</th>
                <th width="80">&nbsp;</th>
...
if ($action=="show_trim_booking_list")
{
	...
	$job_and_trimgroup_level[$selectResult[csf('job_no')]][$selectResult[csf('wo_pre_cost_embe_cost_dtls')]][$selectResult[csf('sensitivity')]][$selectResult[csf('gmt_item')]]['style_ref_no'][$selectResult[csf('po_id')]]=$selectResult[csf('style_ref_no')];
...
//$uom_id=$job_uom_id_arr[$selectResult[csf('job_no')]];
	...
	<? 
	echo $costing_per_dzn; //pls discuss if want to enable this
	 //echo $unit_of_measurement[$uom_id];?>
	</td>
...
if($cbo_level==2){
 ...
 $uom_id=$job_uom_id_arr[$job_no];
	...
	<?  
	echo $costing_per_dzn; // if you want to disable this pls discuss
	//echo $unit_of_measurement[$uom_id];?>
	</td>
	<td width="100" align="right">
...
if($action=="show_trim_booking_report2")
{
	...
	$nameArray_buyer=sql_select( "select  a.id,a.style_ref_no, a.order_uom,a.job_no, a.style_owner, a.buyer_name, a.dealing_marchant, a.season, a.season_matrix, a.season_buyer_wise, a.order_repeat_no, a.inserted_by, b.po_break_down_id,c.costing_per, d.file_no from wo_po_details_master a, wo_booking_dtls b,wo_pre_cost_mst c,wo_po_break_down d where a.job_no=b.job_no and a.job_no=c.job_no and b.job_no=c.job_no and a.job_no=d.job_no_mst and  b.booking_no='$txt_booking_no' and b.status_active =1 and b.is_deleted=0");
	...
	$nameArray=sql_select( "select a.id,a.booking_no, a.pay_mode,a.buyer_id, a.booking_date, a.supplier_id, a.currency_id, a.exchange_rate, a.attention, a.delivery_date, a.source, a.remarks, a.revised_no, a.floor_id, a.inserted_by from wo_booking_mst a where a.booking_no='$txt_booking_no' and a.status_active =1 and a.is_deleted=0");
	//print_r($nameArray);die;
	$mst_id = $nameArray[0]['ID'];
                        ...
                        <tr>
                            ...
                            <td width="10px" align="center" style="font-size:20px;">
                                <table width="80%" align="right" cellpadding="0" cellspacing="0" style="border:0px solid black">
                                    ...
                                    if(str_replace("'","",$id_approved_id) ==1 || str_replace("'","",$id_approved_id) ==3)
                                    {
                                    ?>
                                    <tr>
                                        <td>Approved Status :&nbsp;    <font style="color:#F00"><? if(str_replace("'","",$id_approved_id) ==1){ echo "Approved";}else if(str_replace("'","",$id_approved_id) ==3){ echo "Partial Approved";}else{echo "";}; ?></font> </td>
                                    </tr>
                                    <?
                                    }
                                    ?>
   ...
   // $nameArray_item_description=sql_select( "select a.pre_cost_fabric_cost_dtls_id,min(b.id) as bid, b.description, b.brand_supplier,b.item_size,b.article_number,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_emb_book_con_dtls b where a.id= b.wo_booking_dtls_id and a.booking_no=b.booking_no and   a.booking_no='$txt_booking_no' and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.sensitivity=2 and a.gmt_item=".$result_item[csf('gmt_item')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and b.requirment !=0 and  a.status_active =1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.pre_cost_fabric_cost_dtls_id, b.description, b.brand_supplier,b.item_size,b.article_number order by bid");
    ...
    <td width="2%"></td>
    <? 
    $data_array=sql_select("select b.approved_by,b.approved_no, b.approved_date, c.user_full_name from  wo_booking_mst a , approval_history b, user_passwd c where a.id=b.mst_id and b.approved_by=c.id and a.booking_no='$txt_booking_no' and b.entry_form=32 and  a.status_active =1 and a.is_deleted=0");
	//echo $data_array;die;
    ?>
    ...
    <?
	$cbo_template_id=1;
	// print_r($nameArray_buyer['ID']);die;
	// print_r($nameArray_buyer['INSERTED_BY']);die;
	// $mst_id = $nameArray[0]['ID'];
	//echo $mst_id;
	//$inserted_by = $nameArray[0]['INSERTED_BY'];
	...
	$appSql="SELECT APPROVED_BY from APPROVAL_MST where ENTRY_FORM=32 and MST_ID ={$mst_id}";
	// echo $appSql;die;
	...
	echo signature_table(133, $cbo_company_name, "1333px",$cbo_template_id,20,$inserted_by,$userSignatureArr);
...
ob_clean();
	...
	if($is_mail_send==1){
		...
		// $elcetronicSql = "SELECT a.BUYER_ID,a.SEQUENCE_NO,a.BYPASS,b.USER_EMAIL  from electronic_approval_setup a join user_passwd b on a.user_id=b.id where b.valid=1  AND a.entry_form=29 and a.company_id=$cbo_company_name and a.IS_DELETED=0 and b.STATUS_ACTIVE=1 and b.IS_DELETED=0 order by a.SEQUENCE_NO";
		...
		// {
		// 	if($row['EMAIL']){$mailArr[$row['EMAIL']]=$row['EMAIL'];}
		// }
		...
		$to=implode(',',$mailArr);
		$subject="Embellishment Work Order Notification";
...
if($action=="show_trim_booking_report3")
{
    ...
    <table width="1333px" cellpadding="0" cellspacing="0" style="border:0px solid black" >
                ...
                <td width="20px">
                    <table width="100%" cellpadding="0" cellspacing="0" style="border:0px solid black">
                        <tr>
                            ...
                            <td width="70px" align="center" style="font-size:20px;">
                                <table width="100%" align="right" cellpadding="0" cellspacing="0" style="border:0px solid black">
                                    <tr>
									<td width="40px" align="center">
                               			 &nbsp;  &nbsp;  &nbsp;
                            		</td>
        ...
        <table width="900" align="left" style="border:0px solid black;">
			...
			<tr>
            	<td width="100" style="font-size:16px"><b>TO</b></td>
                <td width="110" style="font-size:16px" colspan="3">:&nbsp; <b>
					<?
					if($pay_mode_id==5 || $pay_mode_id==3){
					echo $company_library[$supplier_id];
					}
                ...
                <td width="110" style="font-size:16px">:&nbsp;<?php echo $varcode_booking_no; ?></td>
            </tr>
			<tr>
            	<td width="100" style="font-size:16px"><b>Address</b></td>
                <td width="110" style="font-size:16px" colspan="3">:&nbsp; 
				<?
					if($pay_mode_id==5 || $pay_mode_id==3){
					$address=$company_address[$supplier_id];
					}
                ...
                <td width="110" style="font-size:16px">:&nbsp;<?php echo change_date_format($booking_date); ?></td>
            </tr>
			<tr>
            <td colspan="6" valign="top"></td>
            </tr>
			<tr>
				<td width="100" style="font-size:12px" align="right"><b>Buyer.</b></td>
				<td width="110" >:&nbsp;<? echo $buyer_name_arr[$buyer_id]; ?></td>
				<td width="100" style="font-size:12px" align="right"><b>Currency</b></td>
				<td width="110">:&nbsp;<?  echo $currency[$currency_id]; ?></td>
				<td width="100" style="font-size:12px" align="right"><b>Floor/Unit</b></td>
				<td width="110">:&nbsp;<?=$floorArr[$floor_id];?></td>
			</tr>
			<tr>
				<td width="100" style="font-size:12px" align="right"><b>Season</b> </td>
				<td width="110">:&nbsp;<? echo $seasons_names; ?></td>
				<td width="100" style="font-size:12px" align="right"><b>Dealing Merchant</b></td>
				<td width="110">:&nbsp;<? echo implode(",",array_unique($all_dealing_marcent))?></td>
				<td width="100" style="font-size:12px" align="right" ><b>Attention </b>   </td>
				<td width="110">:&nbsp;<? echo $attention; ?> </td>
			</tr>
			<tr>
				<td width="100" style="font-size:12px" align="right"><b>Pay mode</b></td>
				<td width="110">:&nbsp;<? echo $pay_mode[$pay_mode_id];?></td>
				<td width="100" style="font-size:12px" align="right"><b>Source</b></td>
                <td width="110">:&nbsp;<? echo $source[$source_id];?></td>
				<td width="100" style="font-size:12px" align="right"><b>Remarks</b>  </td>
                <td width="110">:&nbsp;<? echo $remarks; ?></td>
			</tr>
			<tr>
				<td width="100" style="font-size:12px" align="right"><b>Order Repeat </b> </td>
				<td width="110">:&nbsp;<? echo $order_rept_no; ?></td>
				<td width="100" style="font-size:12px" align="right"><b>Delivery Date</b></td>
				<td width="110">:&nbsp;<?  echo change_date_format($delivery_date); ?></td>
			</tr>
			<tr>
            <td colspan="6" valign="top"></td>
   ...
   /* $precost_arr=array();
    ...
    <tr>
	<td align="center" style="border:1px solid black"><strong>Job No</strong> </td>		
	<td align="center" style="border:1px solid black"><strong>Style Ref</strong> </td>	
    ...
    <td style="border:1px solid black" align="center"><strong>WO Rate</strong></td>
    <td style="border:1px solid black" align="center"><strong>WO Amount</strong></td>
    </tr>
    <?
    $i=0;
    $grand_total_as_per_gmts_color=0;
	...
	<td align="center"  style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
    <?
    echo $nameArray_job_po_row[csf('job_no')];
    ?>
    </td>
	<td align="center"  style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
    <?
    echo $style_ref[$nameArray_job_po_row[csf('job_no')]];
    ?>
    </td>
    <?
    $item_desctiption_total=0;
    $total_amount_as_per_gmts_color=0;
	$total_item_desctiption_total=0;
    foreach($nameArray_item_description as $result_itemdescription){
	if($result_item[csf('emb_name')]==1)
	{
		$emb_type_name=$emblishment_print_type[$result_itemdescription[csf('emb_type')]];
    ...
    <td style="border:1px solid black;text-align:center">
	<?  echo $emblishment_name_array[$result_item[csf('emb_name')]] ?>
    </td>
    <td style="border:1px solid black; text-align:center">
    <?
	echo $color_library[$result_itemdescription[csf('item_color')]];
    ...
    <td style="border:1px solid black;  text-align:right; font-weight:bold;"><?
    echo number_format($item_desctiption_total ,4);$total_item_desctiption_total+=$item_desctiption_total;
    ?></td>
    <td style="border:1px solid black; text-align:right"></td>
    <td style="border:1px solid black; text-align:right">
    <?
    ...
    <tr>
	<td></td>
    <td align="right" style="border:1px solid black"  colspan="7"><? $grand_item_desctiption_total+=$total_item_desctiption_total;?><strong>Total</strong></td>
   ...
   // $nameArray_item_description=sql_select( "select a.pre_cost_fabric_cost_dtls_id,min(b.id) as bid, b.description, b.brand_supplier,b.item_size,b.article_number,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_emb_book_con_dtls b where a.id= b.wo_booking_dtls_id and a.booking_no=b.booking_no and   a.booking_no='$txt_booking_no' and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.sensitivity=2 and a.gmt_item=".$result_item[csf('gmt_item')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and b.requirment !=0 and  a.status_active =1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.pre_cost_fabric_cost_dtls_id, b.description, b.brand_supplier,b.item_size,b.article_number order by bid");
    ...
    ?>
    <tr>
    <td align="right" style="border:1px solid black"  colspan="8"><? $grand_item_desctiption_total+=$total_item_desctiption_total;?><strong>Total</strong></td>
    ...
    <td style="border:1px solid black;  text-align:right" colspan="5"><strong> Item Total</strong></td>
    <td style="border:1px solid black;  text-align:right; font-weight:bold;"><?
    echo  $currency_sign_arr[$currency_id].'&nbsp;'.number_format($item_desctiption_total ,4);$total_item_desctiption_total+=$item_desctiption_total;
    ?></td>
    <td style="border:1px solid black; text-align:right"></td>
    <td style="border:1px solid black; text-align:right">
    <?
    ...
    ?>
    <tr>
    <td align="right" style="border:1px solid black"  colspan="9"><? $grand_item_desctiption_total+=$total_item_desctiption_total;?><strong>Total</strong></td>
    <td  style="border:1px solid black;  text-align:right"><?  echo  $currency_sign_arr[$currency_id].'&nbsp;'.number_format($grand_total_as_per_gmts_color,2); $booking_grand_total+=$grand_total_as_per_gmts_color; ?></td>
    </tr>
    </table>
    ...
    <?
				...
				?>
					...
					?>
					<tr>
						<td align="right" style="border:1px solid black" colspan="11"><? $grand_item_desctiption_total+=$total_item_desctiption_total;?><strong>Total</strong></td>
	...
	<table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
	<tr>
    <td width="40%" align="right" style="border:1px solid black"  ><strong> Grand Total</strong></td>
    <td width="38%" style="border:1px solid black;  text-align:right"><b><?  echo  number_format($grand_item_desctiption_total,2); ?></td>
	<td width="10%" align="right" style="border:1px solid black"  ></b></td>
	<td width="13%" style="border:1px solid black;  text-align:right"><b><?  echo  $currency_sign_arr[$currency_id].'&nbsp;'.number_format($booking_grand_total,2); ?></b></td>
    </tr>
	</table>
	<table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
    <tr>
    <td>
	<table  align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
    <!-- <tr style="border:1px solid black;">
    <td width="30%" style="border:1px solid black; text-align:left">Total Booking Amount</td>
    <td width="70%" style="border:1px solid black; text-align:left"><? //echo number_format($booking_grand_total,2);?></td>
    </tr> -->
...
if ($action=="get_first_selected_print_report"){
	...
	$print_report_format = return_field_value("format_id", "lib_report_template", "template_name ='" . $company_id . "' and module_id=2 and report_id=244 and is_deleted=0 and status_active=1");
	...
	foreach (explode(',', $print_report_format) as $button_id) {
		if ($button_id == 13) {
			...
			echo "generate_trim_report('show_trim_booking_report3',1,'".$mail_id."',1)";
		}
		break;
	}
	exit();
