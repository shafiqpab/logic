...
if ($action=="save_update_delete_mst")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$field_array1="id, job_no,job_id, gmts_item_id, set_item_ratio, smv_pcs, smv_set, complexity, embelishment";
		$add_comma=0;
		...
		for($c=0;$c < count($set_breck_down_array);$c++)
		{
			...
			$data_array1 .="(".$id1.",'".$new_job_no[0]."',".$id.",'".$set_breck_down_arr[0]."','".$set_breck_down_arr[1]."','".$set_breck_down_arr[2]."','".$set_breck_down_arr[3]."','".$set_breck_down_arr[4]."','".$set_breck_down_arr[5]."')";
			$add_comma++;
			$id1=$id1+1;
		}
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		for($c=0;$c < count($set_breck_down_array);$c++)
		{
			...
			if ($add_comma!=0) $data_array1 .=",";
			$data_array1 .="(".$id1.",".$txt_job_no.",".$hidd_job_id.",'".$set_breck_down_arr[0]."','".$set_breck_down_arr[1]."','".$set_breck_down_arr[2]."','".$set_breck_down_arr[3]."','".$set_breck_down_arr[4]."','".$set_breck_down_arr[5]."')";
			$add_comma++;
			$id1=$id1+1;
		}
				...
				for($m=1; $m<=$size_table; $m++)
				{
					...
					if($color_size_poQty>0)
					{
						if(str_replace("'",'',$$txt_colorSizeId)=="")
						{
							...
							if($color_size_poQty>0)
							if ($add_comma!=0) $data_array1 .=",";
							$data_array1 .="(".$id1.",".$update_id_details.",".$update_id.",'".$color_mst_id."','".$size_mst_id."','".$item_mst_id."',".$cbo_gmtsItem_id.",".$cbo_deliveryCountry_id.",".$cbo_code_id.",".$cbo_country_id.",".$cbo_countryCode_id.",".$txt_cutup_date.",".$cbo_cutOff_id.",".$txt_countryShip_date.",".$color_id.",".$size_id.",'".$color_size_poQty."','".$color_size_rate."',".$$txt_colorSizeExCut.",".$$txt_colorSizeArticleNo.",".$order_total_amt.",'".$color_size_planCutQty."','".$assort_qty."','".$solid_qty."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',0,1)";
							$id1=$id1+1;
							$add_comma++;
						}
				...
				for($m=1; $m<=$size_table; $m++)
				{
					...
					if($color_size_poQty>0)
					{
						if(str_replace("'",'',$$txt_colorSizeId)=="")
						{
							...
							if ($add_comma!=0) $data_array1 .=",";
							$data_array1 .="(".$id1.",".$update_id_details.",".$update_id.",'".$color_mst_id."','".$size_mst_id."','".$item_mst_id."',".$cbo_gmtsItem_id.",".$cbo_deliveryCountry_id.",".$cbo_code_id.",".$cbo_country_id.",".$cbo_countryCode_id.",".$txt_cutup_date.",".$cbo_cutOff_id.",".$txt_countryShip_date.",".$color_id.",".$size_id.",'".$color_size_poQty."','".$color_size_rate."',".$$txt_colorSizeExCut.",".$$txt_colorSizeArticleNo.",".$order_total_amt.",'".$color_size_planCutQty."',".$$txt_colorSizePackQty.",".$$txt_colorSizePcsQty.",".$txt_breakdownGrouping.",".$txt_pcsQty.",'".$assort_qty."','".$solid_qty."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',0,1)";
							$id1=$id1+1;
							$add_comma++;
						}
						else if (str_replace("'",'',$$txt_colorSizeId)!="")
...
if($action=="save_update_delete_destination_info")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		if(str_replace("'","",$txt_color)!="")
		{
			...
			{
				$color_id = return_id( str_replace("'","",$txt_color), $color_arr, "lib_size", "id,lib_color","351");
