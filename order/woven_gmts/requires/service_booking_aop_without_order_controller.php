...
if ($action=="load_drop_down_supplier")
{
	...
	echo create_drop_down( "cbo_supplier_name", 172, "select a.id,a.supplier_name from lib_supplier a,lib_supplier_tag_company b  where a.status_active =1 and a.is_deleted=0 and b.supplier_id=a.id and b.tag_company='$data[1]' and a.id in (select  supplier_id from  lib_supplier_party_type where party_type in (25)) order by a.supplier_name","id,supplier_name", 1, "-- Select Supplier --", $selected, "get_php_form_data( this.value, 'load_drop_down_attention', 'requires/service_booking_aop_without_order_controller');",0 );
	}
	else
	{
		echo create_drop_down( "cbo_supplier_name", 172, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Supplier --", $data[1], "get_php_form_data( this.value, 'load_drop_down_attention', 'requires/service_booking_aop_without_order_controller');",0 );
...
$data=explode("_",$data);
	...
	echo create_drop_down( "cbo_fabric_description", 172, $fabric_description,"", 1, "-- Select --", $selected,"get_related_data(this.value)" );
...
{ 
	...
	echo create_drop_down( "cbo_gmtcolor", 172, $color_array,"", 1, "-- Select --", $selected, "",1,"" );
	exit();
}
...
{ 
	...
	echo create_drop_down( "cbo_printing_color", 172, $print_color_array,"", 1, "-- Select --", $selected, "","","" );
	exit();
}
...
{ 
   $data=explode("_",$data);
	if($data[1]==1)
	{
		$sql=sql_select("select a.id,a.body_part,a.fabric_description,a.fabric_color,a.gmts_color,a.gsm_weight,a.dia_width from wo_non_ord_samp_booking_mst b,wo_non_ord_samp_booking_dtls a where a.id=$data[0] and a.booking_no=b.booking_no and a.status_active=1 and a.is_deleted=0");
		foreach($sql as $row)
		{
...
if ($action=="aop_detls_list_view")
{
	...
	$sql="select id,wo_id,wo_no,fab_booking_id,fab_booking_no,fabric_source,fabric_description,aop_gsm,aop_dia,uom,artwork_no,wo_qty,rate,amount,dev_start_date ,dev_end_date,remarks,is_deleted,status_active,inserted_by,insert_date from wo_non_ord_aop_booking_dtls where wo_id=$data[0]";
		?>
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		if($db_type==0)
		{
			...
			if(str_replace("'","",$txt_booking_no)=="")
			{
				if($db_type==0)
				{
				$new_wo_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'NAP', date("Y",time()), 5, 
				"select wo_no_prefix, wo_no_prefix_num from wo_non_ord_aop_booking_mst where company_id=$cbo_company_name 
				and YEAR(insert_date)=".date('Y',time())." order by wo_no_prefix_num desc ", "wo_no_prefix", "wo_no_prefix_num" ));
				}
				if($db_type==2)
				{
				$new_wo_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'NAP', date("Y",time()), 5,"select wo_no_prefix,
				wo_no_prefix_num from wo_non_ord_aop_booking_mst where company_id=$cbo_company_name and 
				to_char(insert_date,'YYYY')=".date('Y',time())." order by wo_no_prefix_num desc ", "wo_no_prefix", "wo_no_prefix_num" ));
				}
				...
				$data_array ="(".$id.",'".$new_wo_no[1]."',".$new_wo_no[2].",'".$new_wo_no[0]."',".$txt_fabric_booking_id.",".$txt_fabric_booking.",".$cbo_company_name.",".$cbo_buyer_name.",".$txt_booking_date.",".$cbo_currency.",".$txt_exchange_rate.",".$cbo_pay_mode.",".$cbo_source.",".$cbo_aop_source.",".$cbo_supplier_name.",".$txt_attention.",0,0,0,1,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				$response_wo_no=$new_wo_no[0];
				$mst_id=$id;
				
				$id_dtls=return_next_id( "id", "wo_non_ord_aop_booking_dtls", 1 ) ;
				$field_array_d="id,	wo_id,wo_no,fab_booking_id,fab_booking_no,fabric_source,fabric_description,aop_gsm,aop_dia,uom,artwork_no,wo_qty,rate,amount,dev_start_date ,dev_end_date,remarks,printing_color_id,is_deleted,status_active,inserted_by,insert_date";
				$data_array_d ="(".$id_dtls.",".$id.",'".$new_wo_no[0]."',".$txt_fabric_booking_id.",".$txt_fabric_booking.",".$cbo_fabric_source.",".$cbo_fabric_description.",".$txt_gsm.",".$txt_fin_dia.",".$cbo_uom.",".$txt_art_work.",".$txt_wo_qty.",".$txt_rate.",".$txt_amount.",".$txt_dev_start_date.",".$txt_dev_end_date.",".$txt_remarks.",".$cbo_printing_color.",0,1,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
	...
	else if ($operation==1)   // Update Here
	{
		 ...
		 $field_array_up="booking_date*currency_id*exchange_rate*pay_mode*source*aop_source*supplier_id*attention*is_approved*ready_to_approved*updated_by*update_date";
		 $data_array_up ="".$txt_booking_date."*".$cbo_currency."*".$txt_exchange_rate."*".$cbo_pay_mode."*".$cbo_source."*".$cbo_aop_source."*".$cbo_supplier_name."*".$txt_attention."*0*0*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		 
		 $field_array_up_d="fabric_description*aop_gsm*aop_dia*uom*artwork_no*wo_qty*rate*amount*dev_start_date *dev_end_date*remarks*printing_color_id*updated_by*update_date";
		 $data_array_up_d ="".$cbo_fabric_description."*".$txt_gsm."*".$txt_fin_dia."*".$cbo_uom."*".$txt_art_work."*".$txt_wo_qty."*".$txt_rate."*".$txt_amount."*".$txt_dev_start_date."*".$txt_dev_end_date."*".$txt_remarks."*".$cbo_printing_color."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
            	...
            	<table  cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
        			...
        			<tr>
                    	<td> <input type="hidden" id="selected_booking">
							<? 
								echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 $company_cond order by company_name","id,company_name",1, "-- Select Company --", $cbo_company_name, "load_drop_down( 'service_booking_aop_without_order_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
							?>
                        </td>
                   	<td id="buyer_td">
                     	<? 
...
if ($action=="create_booking_search_list_view")
{
	...
	$arr=array (2=>$comp,3=>$buyer_arr,5=>$knitting_source);
	 $sql= "select a.id,a.wo_no_prefix,a.wo_no_prefix_num,a.wo_no,fab_booking_id,a.fab_booking_no,a.company_id,a.buyer_id,a.booking_date,a.currency_id,a.exchange_rate,a.pay_mode,a.source,a.aop_source,a.supplier_id,a.attention,a.is_approved,a.ready_to_approved from wo_non_ord_aop_booking_mst a,  wo_non_ord_samp_booking_mst b where a.fab_booking_id=b.id and $company $buyer  $wo_cond $booking_cond $booking_date order by a.wo_no_prefix_num"; 
		//echo $sql;
	echo  create_list_view("list_view", "WO No,WO Date,Company,Buyer,Booking No,Aop source", "100,80,100,100,100","670","320",0, $sql , "js_set_value", "id", "", 1, "0,0,company_id,buyer_id,0,aop_source", $arr , "wo_no_prefix_num,booking_date,company_id,buyer_id,fab_booking_no,aop_source", '','','0,3,0,0,0,0','','');
	exit();
}
