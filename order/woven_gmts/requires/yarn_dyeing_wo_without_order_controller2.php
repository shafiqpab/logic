...
if ($action=="load_drop_down_sample_booking")
{
	...
	$sql="Select b.id, b.sample_name from  lib_sample b,  wo_non_ord_samp_booking_dtls a where a.sample_type=b.id and b.status_active=1 and a.status_active=1 and a.booking_no='$data' and a.color_type_id in(2,3,4,6,33)";
	//echo $sql;die;
    echo create_drop_down( "cbo_sample_name", 70, $sql,"id,sample_name", 1, "-select-", $selected,"","0" );
	exit();
}
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) 
	{
		...
		if($txt_yern_color !="")
		{
			...
			{
				$color_id = return_id( $txt_yern_color, $color_arr, "lib_color", "id,color_name","114");
...
if($action=="child_form_input_data")
{
	...
	$sql = "select id,job_no,product_id,job_no_id,count,yarn_description,yarn_color,color_range,yarn_comp_type1st,yarn_comp_percent1st,yarn_type,uom,yarn_wo_qty,dyeing_charge,amount,no_of_bag, no_of_cone,min_require_cone,remarks,referance_no,sample_name,booking_no from wo_yarn_dyeing_dtls where id='$data'";
	//echo $sql;
   ...
   if($ex_data[7]==2)
	...
	/*$sql = "select id, yarn_dyeing_prefix_num, ydw_no, company_id, supplier_id, booking_date, delivery_date, currency, ecchange_rate, pay_mode,source, attention from  wo_yarn_dyeing_mst where  status_active=1 and is_deleted=0 $supplier $company $sql_cond";*/
	 ...
	 if($db_type==0)
	 {
		 ...
		 where  
				a.id=b.mst_id and  a.status_active=1 and a.is_deleted=0 and b.status_active=1 and a.entry_form=114 and b.entry_form=114 $company $supplier  $sql_cond  $buyer_cond  $booking_cond
		 group by a.id";
	 }
...
if($data[6]==4 || $data[6]==0)
   ...
   if($data[6]==2)
	...
	$arr=array (2=>$comp,3=>$buyer_arr,4=>$item_category,5=>$fabric_source,6=>$suplier,7=>$style_library);
	 ...
	 $sql= "select a.booking_no_prefix_num, a.booking_no,a.booking_date,a.company_id,a.buyer_id,a.item_category,a.fabric_source,a.supplier_id,a.is_approved,a.ready_to_approved,b.style_id,b.style_des from wo_non_ord_samp_booking_mst  a, wo_non_ord_samp_booking_dtls b  where a.booking_no=b.booking_no  and b.status_active=1 and b.is_deleted=0   $company". set_user_lavel_filtering(' and a.buyer_id','buyer_id')." $buyer $booking_date $booking_cond $style_des_cond and a.booking_type=4 and a.status_active=1 and a.is_deleted=0 and b.color_type_id in(2,3,4,6,33)  order by booking_no"; 
	//echo $sql;
...
//print_r($supp_arr);die;
	...
	<div style="width:950px" align="center">      
            	...
            	<td width="350"  style="font-size:12px">
                        <table width="350" style="" align="left">
                            ...
                            <tr>
                                ...
                                <td width="230" style="font-size:16px">:&nbsp;&nbsp; <b><? echo $supplier_arr[$supplier_id];?></b></td>
                            </tr> 
       ...
       $buyer_sample=return_library_array( "select id, buyer_name from  sample_development_mst",'id','buyer_name');
	   //$buyer_season_name=return_library_array( "select id, season_name from  lib_buyer_season",'id','season_name');
	...
	<div style="width:950px" align="center">      
       ...
       <table width="950" style="" align="center">
        ...
        <table width="950"  align="center" border="1"  cellpadding="0" cellspacing="0" rules="all" class="rpt_table">
			...
			id,product_id,job_no,job_no_id,yarn_description,count,dyeing_charge,min_require_cone,referance_no, remarks order by id ";
