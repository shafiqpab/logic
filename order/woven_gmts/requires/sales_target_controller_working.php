...
if ($action=="generate_list_view")
{
	...
	//Current Data History------------------------------------------------------------------
	 $sql="select mst.id,mst.total_target_mint,dtls.year_month_name, dtls.sales_target_qty, dtls.sales_target_value,dtls.sales_target_mint FROM wo_sales_target_mst mst,wo_sales_target_dtls dtls WHERE mst.id=dtls.sales_target_mst_id AND mst.company_id='$company_id' AND mst.buyer_id='$buyer_id' AND mst.agent='$agent' AND mst.team_leader='$tem_leader' AND mst.starting_year=$t_year";
	$sql_result=sql_select($sql);
	$mst_id="";
	$target_mint="";
	foreach($sql_result as $row)
	...
	{
		$year_id=$row[csf("year")];
		$month_id=$row[csf("month_id")];
		$al_prcnt_arr_month_yr['allocation'.$month_id.$year_id]=$row[csf("allocation_percentage")];
		$capacity_min_arr_month_yr['mint'.$month_id.$year_id]=$row[csf("capacity_month_min")];
		print_r( $al_prcnt_arr_month_yr);
		print_r( $capacity_min_arr_month_yr);
		//$po_array[$row[csf("month_id")]]['month']=$row[csf("month_id")];
		//$mmyy=trim(str_replace(",","",$row[csf("year_month_name")]));	
	...
	echo '<br/>';
	print_r($arr_capacity_arr);
	echo '<br/>';
	print_r($arr_capacity_arr_yr);
	echo '<br/>';
	print_r($month_id_arr);
	echo '<br/>';
	print_r($year_id_arr);
       ...
       <?
         ...
         $tot_prv_qty=$tot_prv_val=0;
            for ($i=0; $i<12; $i++)
            {
                if ($k<13)
                {
                    ...
                    <tr>                       	 
                         ...
                         <? if($mst_id=="")
						 {
							 //echo $arr_al_prcnt_arr[$month]*$arr_capacity_arr[$month]/100; 
							 //$cur_trg_mint=$cur_data_arr['mint'.$month.$yy]; $tot_trg_mint+=$cur_trg_mint;  $tot_alo_prcnt_val+=$arr_al_prcnt_arr[$month]*$arr_capacity_arr[$month]/100;
                              $cur_trg_mint_1=$al_prcnt_arr_month_yr['allocation22010'];$tot_trg_mint_1+=$cur_trg_mint_1;
                              $cur_trg_mint=$capacity_min_arr_month_yr['mint'.$month.$yy];$tot_trg_mint+=$cur_trg_mint;
                             //$total_target_cap_mint= $cur_trg_mint_1*$cur_trg_mint_2/100;
                              ?>
            ...
            <tr>                       	 
                 ...
                 <td align="center"><input type="text" id="txt_total_alo_prcnt" value="<?  if($target_mint>0){echo $target_mint;} else {echo $tot_trg_mint;} ?>" readonly="readonly" class="text_boxes" style="text-align:right;"/> </td>
