       ...
       <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid black">
           <tr>
               ...
               <td width="1000">                                     
				...
				$nameseason=sql_select( "select a.season from  sample_development_mst a,  wo_booking_dtls c  where   a.id=c.style_id and c.booking_no=$txt_booking_no and a.entry_form_id=117 and c.entry_form_id=440");
				
				$fabric_source='';
                $nameArray=sql_select( "select id,buyer_id,pay_mode,booking_no,booking_date,supplier_id,currency_id,exchange_rate,attention,delivery_date,fabric_source,team_leader,	dealing_marchant,job_no,po_break_down_id from wo_booking_mst   where  booking_no=$txt_booking_no and entry_form=440"); 
	...
	foreach($col_data as $req_val)
	{
		...
		?>
			<tr>
				...
				<td align="right"><p><? echo $req_qty= number_format($req_qty,2); ?></p></td>
       ...
       <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid black" >
           <tr>
               ...
               <td width="1000">                                     
                ...
                $nameArray=sql_select( "SELECT buyer_id,fabric_source,booking_no,pay_mode,booking_date,internal_ref_no,supplier_id,currency_id,exchange_rate,attention,delivery_date,fabric_source,team_leader,dealing_marchant from wo_booking_mst   where  booking_no=$txt_booking_no");
    ...
    <caption><? echo $unit_of_measurement[$uom_id];?> </caption>
        			...
        			{	
        				...
        				foreach($dtm_data as $dtm_id=>$gsm_data)
        				{
        					...
        					foreach($gsm_data as $gsm_id=>$value)
        					{
                                ...
                                <td width="50"  align="right"><? echo $value["amount"]; ?></td>
                                <?
								}
								?>
        ...
        ?>
       			...
       			<tr>
					...
					?>
                    <th width="40" align="right">&nbsp;</th>					
					<th width="50" align="right"> <? echo $total_amount;?></th>
                    <?
					}
					?>
                    <th width=""> </th>
        			...
        			<tr>
                    	<td> <input type="hidden" id="selected_booking">
							<? 
								...
								echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 $company_cond order by company_name","id,company_name",1, "-- Select Company --", '', "load_drop_down( 'sample_requisition_booking_with_order_controller', this.value, 'load_drop_down_buyer_pop', 'buyer_td' );");
							?>
                        </td>
                   	<td id="buyer_td">
                     <? 
...
if($data[6]==4 || $data[6]==0)
   ...
   if($data[6]==2)
	...
	$arr=array (2=>$comp,3=>$buyer_arr,4=>$item_category,5=>$fabric_source,6=>$suplier,7=>$style_library,9=>$approved,10=>$is_ready); 
	 $sqls= "select a.booking_no_prefix_num, a.booking_no,a.booking_date,a.company_id,a.buyer_id,a.item_category,a.fabric_source,a.supplier_id,a.is_approved,a.ready_to_approved,a.pay_mode,b.style_id from wo_booking_mst  a left join wo_booking_dtls b on a.booking_no=b.booking_no  and b.status_active=1 and b.is_deleted=0  where  a.entry_form='440' and b.entry_form_id='440' $company". set_user_lavel_filtering(' and a.buyer_id','buyer_id')." $buyer $booking_date $booking_cond  and a.booking_type=4 and  a.status_active=1 and a.is_deleted=0  order by booking_no"; 
	  //echo $sqls;
	?>
   <table class="rpt_table scroll" width="980" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body">
   <thead>
        <th width="50">SL</th> 
        <th width="100">Booking No</th>  
...
{
	 $sql= "select  id,booking_no,booking_date,company_id,buyer_id,item_category,fabric_source,currency_id,exchange_rate,pay_mode,booking_month,supplier_id,attention,delivery_date,source,booking_year,is_approved,ready_to_approved,team_leader,	dealing_marchant from wo_booking_mst  where booking_no='$data' and entry_form='440' and status_active=1 and is_deleted=0"; 
	 $requisition_id=return_field_value( "style_id", "wo_booking_dtls","booking_no='$data' and status_active=1 and is_deleted=0 and entry_form_id=440");
...
if($action=="load_php_dtls_form")
{
		...
		WHERE
		a.job_no=b.job_no and b.job_no='$job_nos'  
		...
		$nameArray_fabric_color=sql_select( "
	select
	a.id as pre_cost_fabric_cost_dtls_id,
	a.job_no,
	...
	$nameArray_item_size=sql_select( "
	select
	b.item_size
	FROM
	wo_pre_cost_fabric_cost_dtls a,
	...
	b.po_break_down_id in (select id from wo_po_break_down where status_active=1 and is_deleted=0 and job_no_mst='$job_nos')  	order by b.item_size"); 
	...
	foreach ($nameArray_item_size as $result)
	{
		...
		if($data[1]=="1")
		{
			$sql_fabric="Select id,sample_mst_id,sample_name,gmts_item_id,body_part_id,fabric_nature_id,fabric_description,gsm,dia,sample_color,color_type_id,width_dia_id,uom_id,required_dzn,required_qty,color_data from sample_development_fabric_acc where sample_mst_id='$data[0]' and form_type=1 and  is_deleted=0  and status_active=1 order by id ASC";
					...
					?>
						...
						<tr id="tr_<? echo $i; ?>" style="height:10px;" class="general">
						 <td align="center" id="rfPOId_1">
                        	<?
                        		$all_po="select  b.id  ,b.po_number   from 	wo_po_details_master  a, wo_po_break_down b,sample_development_mst c where a.job_no=b.job_no_mst and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.id='$data[0]' and c.quotation_id=a.id order by b.id";
                        	...
                        	<?
                        	echo create_drop_down( "cboFabricDescPreCost_$i", 100, $fabric_description_array,"", 0, "Select Desc", $selected,"",'');	
							?>		 
	                            ...
	                            <td align="center" id="rf_color_1">
	                                 <input style="width:60px;" type="text" class="text_boxes"  name="txtRfColor_<? echo $i; ?>" id="txtRfColor_<? echo $i; ?>" placeholder="browse" onDblClick="openmypage_rf_color('requires/sample_requisition_booking_with_order_controller.php?action=color_popup_rf','Color Search','<? echo $i;?>');" 
                        	...
                        	<?
                        	echo create_drop_down( "cboGarmentsColor_$i", 100, $color_library_order,"", 0, " Garments Color", $selected,"",'');	
							?>		 
                        	...
                        	<?
                        	echo create_drop_down( "cboFabricColor_$i", 100, $fabric_color_array,"", 0, "Fabric Color", $selected,"",'');	
							?>		 
                        	...
                        	<?
                        	echo create_drop_down( "cboGarmentsSize_$i", 100, $size_library_order,"", 0, "Select Size", $selected,"",'');	
							?>		 
                        	...
                        	<?
                        	echo create_drop_down( "cboItemSize_$i", 100, $item_size_array,"", 0, "Item Size", $selected,"",'');	
							?>		 
	                           ...
	                           <td align="center" id="rf_req_qty_1">
	                            ...
	                            <td align="center" id="rf_reqs_qty_1">
                                 <input style="width:50px;" type="text" class="text_boxes_numeric"  name="txtProcessLoss_<? echo $i; ?>" id="txtProcessLoss_<? echo $i; ?>" placeholder=""  onChange="calculate_requirement('<? echo $i; ?>')" />
                            </td>

                            <td align="center" id="rf_req_qnty_1">
                                 <input style="width:50px;" type="text" class="text_boxes_numeric"  name="txtGrayFabric_<? echo $i; ?>" id="txtGrayFabric_<? echo $i; ?>" placeholder="" readonly />
                            </td>

                             <td align="center" id="rf_req_qnty_1">
                                 <input style="width:50px;" type="text" class="text_boxes_numeric"  name="txtRate_<? echo $i; ?>" id="txtRate_<? echo $i; ?>" placeholder="" />
                            </td>
		...
		$sql_fabric="SELECT id as booking_dtls_id,booking_no,po_break_down_id,style_id as sample_mst_id,sample_type as sample_name,gmt_item as gmts_item_id,body_part as body_part_id,fabric_source as fabric_nature_id, fabric_description,gsm_weight as gsm,dia,color_all_data as color_data,color_type,dia_width as width_dia_id,uom as uom_id,req_dzn as required_dzn ,fin_fab_qnty as required_qty,dtls_id as id,process_loss_percent,grey_fab_qnty,rate,amount,pre_cost_fabric_cost_dtls_id,fabric_color_id,gmts_color_id,item_size,gmts_size from wo_booking_dtls
			where status_active=1 and is_deleted=0 and style_id='$data[0]' and entry_form_id=440";
			//echo $sql_fabric;die;
		$sql_resultf =sql_select($sql_fabric);  $i=1;
		if(count($sql_resultf)>0)
			{ 
...
if ($action=="save_update_delete_required_fabric")
{
	...
	if ($operation==0)  // Insert Here
	{
		 ...
		 $field_array="id,job_no,booking_mst_id,po_break_down_id,pre_cost_fabric_cost_dtls_id,booking_no,booking_type,is_short,style_id,sample_type,gmt_item,body_part,fabric_source,fabric_description,gsm_weight,dia,color_all_data,color_type,dia_width,uom,gmts_color_id,fabric_color_id,gmts_size,item_size,req_dzn,fin_fab_qnty,dtls_id,inserted_by,insert_date,status_active,is_deleted,entry_form_id,process_loss_percent,grey_fab_qnty,rate,amount"; 
			...
			for ($i=1;$i<=$total_row;$i++)
		    {
				...
				if ($i!=1) $data_array .=",";
 				$data_array .="(".$id_dtls.",'".$job_no."','".$update_id."',".$$cboPoId.",".$$cboFabricDescPreCost.",'".$booking_no."','4','2','".$requisition_id."',".$$cboRfSampleName.",".$$cboRfGarmentItem.",".$$cboRfBodyPart.",".$$cboRfFabricNature.",".$$txtRfFabricDescription.",".$$txtRfGsm.",".$$txtRfDia.",".$$txtRfColorAllData.",".$$cboRfColorType.",".$$cboRfWidthDia.",".$$cboRfUom.",".$$cboGarmentsColor.",".$$cboFabricColor.",".$$cboGarmentsSize.",".$$cboItemSize.",".$$txtRfReqDzn.",".$$txtRfReqQty.",".$$required_fab_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','1','0','440',".$$txtProcessLoss.",".$$txtGrayFabric.",".$$txtRate.",".$$txtAmount.")";
	...
	else if ($operation==1)  // Update Here
	{
			...
			if($db_type==0)
			{
 			...
 			$id_dtls=return_next_id( "id", "wo_booking_dtls", 1 ) ;
 			$field_array_up="fin_fab_qnty*job_no*po_break_down_id*pre_cost_fabric_cost_dtls_id*process_loss_percent*grey_fab_qnty*rate*amount*gmts_color_id*fabric_color_id*gmts_size*item_size*updated_by*update_date";
			for ($i=1;$i<=$total_row;$i++)
		    {
				$txtRfReqQty="txtRfReqQty_".$i;
				$cboPoId="cboPoId_".$i;
   				...
   				if (str_replace("'",'',$$updateidRequiredDtlf)!="")
				{
					...
					$data_array_up[str_replace("'",'',$$updateidRequiredDtlf)] =explode("*",("".$$txtRfReqQty."*'".$job_no."'*".$$cboPoId."*".$$cboFabricDescPreCost."*".$$txtProcessLoss."*".$$txtGrayFabric."*".$$txtRate."*".$$txtAmount."*".$$cboGarmentsColor."*".$$cboFabricColor."*".$$cboGarmentsSize."*".$$cboItemSize."*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'"));
				}
		    }
  			if($data_array_up!="")
			{
