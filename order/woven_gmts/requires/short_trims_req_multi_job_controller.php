...
if ($action=="save_update_delete")
{
	...
	if ($operation==0){
		...
		$new_booking_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'WSTR', date("Y",time()), 5, "select booking_no_prefix, booking_no_prefix_num from wo_booking_mst where company_id=$cbo_company_name and booking_type=13 and entry_form=717 and to_char(insert_date,'YYYY')=".date('Y',time())." order by id desc", "booking_no_prefix", "booking_no_prefix_num"));
		...
		$data_array ="(".$id.",13,1,4,717,1,'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',".$cbo_company_name.",".$cbo_buyer_name.",".$txt_req_date.",".$cbo_level.",".$cbo_responsible_dept.",".$cbo_responsible_person.",".$cbo_division_id.",".$cbo_ready_to_approved.",".$txt_reason.",".$txt_remarks.",'".$variable_short_booking."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
...
if ($action=="save_update_delete_dtls")
{
	...
	if ($operation==0){
				...
				for($c=0;$c < count($consbreckdown_array);$c++){
					...
					if(str_replace("'","",$consbreckdownarr[4])!="")
					{
						if (!in_array(str_replace("'","",$consbreckdownarr[4]),$new_array_color)){
							$color_id = return_id( str_replace("'","",$consbreckdownarr[4]), $color_library, "lib_color", "id,color_name","717");
		...
		for ($i=1;$i<=$total_row;$i++){
			...
			$data_array1 ="(".$id_dtls.",".$$txttrimcostid.",".$$txtpoid.",".$$txtjob_id.",".$txt_req_no.",".$booking_mst_id.",13,1,717,".$$txttrimgroup.",".$$txtuom.",".$$cbocolorsizesensitive.",".$$txtwoq.",".$$txtexchrate.",".$$txtrate.",".$$txtamount.",'".$txtdlvdate."',".$$txtdescid.",".$$txtcountry.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
	...
	if ($operation==0){
		...
		for ($i=1;$i<=$total_row;$i++){
			...
			foreach($strdata->$job->$trimcostid->po_id as $poId){
				...
				$amount=$wqQty*$rate;
				$data_array1 ="(".$id_dtls.",".$$txttrimcostid.",".$poId.",".$$txtjob_id.",".$txt_req_no.",".$booking_mst_id.",13,1,717,".$$txttrimgroup.",".$$txtuom.",".$$cbocolorsizesensitive.",".$wqQty.",".$$txtexchrate.",".$$txtrate.",".$amount.",'".$txtdlvdate."',".$$txtdescid.",".$$txtcountry.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				$rID1=sql_insert("wo_booking_dtls",$field_array1,$data_array1,0);
...
if ($action=="create_reqsn_search_list_view")
{
	...
	else
	{
		...
		$sql="select a.id, a.job_no, a.booking_no_prefix_num, a.booking_no, company_id, a.booking_date, a.delivery_date, a.responsible_dept from wo_booking_mst a where  a.booking_no not in ( select a.booking_no from  wo_booking_mst a, wo_booking_dtls b, wo_po_break_down c where a.booking_no=b.booking_no and b.po_break_down_id=c.id and  a.booking_type=13 and a.entry_form=717 and a.is_short=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $internal_ref_con  $company  ".set_user_lavel_filtering(' and a.buyer_id','buyer_id')." $supplier_id $booking_date $booking_cond $job_cond $file_cond $ref_cond group by a.booking_no_prefix_num, a.booking_no, a.company_id, a.booking_date, a.delivery_date) and a.booking_type=13 and a.entry_form=717 and a.status_active=1 and a.is_deleted=0 $company ".set_user_lavel_filtering(' and a.buyer_id','buyer_id')." $booking_date $booking_cond and a.is_short=1 group by a.id, a.booking_no_prefix_num, a.booking_no, a.job_no, a.company_id, a.booking_date, a.delivery_date, a.responsible_dept order by a.id DESC";
	}
	//echo $sql;
	?>
	<div width="900">
