...
function load_drop_down_trim_precost_id($data)
{
		...
		$sql_data=sql_select("select id,trim_group,cons_dzn_gmts,description from wo_pre_cost_trim_cost_dtls where job_no='$data[0]' $nominated_sup_cond  and is_deleted=0  and status_active=1");
...
if ($action=="set_precost_data")
{
	...
	foreach($sql_data as $row){
		if($row[csf('country_id')] !=0){
			$country_array[]=$row[csf('country_id')];
		}
...
<fieldset>
            ...
            ?>
            	...
            	<table width="1050" cellspacing="0" class="rpt_table" border="0" id="tbl_consmption_cost" rules="all">
					...
					$trims= new trims($condition);
					//echo "monzu"; die;
					//$trimQtyArray=$trims->getAmountArray_by_orderAndPrecostdtlsidAndGmtssize();
					...
					// With Gmts Item	
					$trimQtyArray=$trims->getQtyArray_by_orderPrecostdtlsidAndGmtscolor();
					$trimAmtArray=$trims->getAmountArray_by_orderAndPrecostdtlsidAndGmtscolor();
					...
					// With Gmts Item	
					$trimQtyArray=$trims->getQtyArray_by_orderPrecostdtlsidAndGmtssize();
					$trimAmtArray=$trims->getAmountArray_by_orderAndPrecostdtlsidAndGmtssize();
					//print_r($trimQtyArray);
					$sql="select b.id, b.po_number,b.po_quantity,min(c.id) as color_size_table_id,c.size_number_id,min(e.item_size) as item_size,sum(c.order_quantity) as order_quantity ,(sum(c.order_quantity)/a.total_set_qnty) as order_quantity_set from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c,wo_pre_cost_trim_cost_dtls d, wo_pre_cost_trim_co_cons_dtls e where  a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and a.job_no=d.job_no and a.job_no=e.job_no and b.id=c.po_break_down_id and b.id=e.po_break_down_id  and d.id=e.wo_pre_cost_trim_cost_dtls_id and c.po_break_down_id=e.po_break_down_id and c.item_number_id= e.item_number_id and c.color_number_id=e.color_number_id and c.size_number_id=e.size_number_id  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and b.id='$txt_po_id' and c.item_number_id='$cbo_gmt_item_id' $txt_country_cond and d.id=$cbo_trim_precost_id group by  b.id, b.po_number,b.po_quantity,a.total_set_qnty,c.size_number_id,c.item_number_id  order by b.id,c.size_number_id";
					...
					// With Gmts Item
					$trimQtyArray=$trims->getQtyArray_by_OrderPrecostdtlsidGmtscolorAndGmtssize();
					$trimAmtArray=$trims->getAmountArray_by_OrderPrecostdtlsidGmtscolorAndGmtssize();
					$sql="select b.id, b.po_number,b.po_quantity,min(c.id) as color_size_table_id,c.color_number_id,c.size_number_id,min(e.item_size) as item_size,sum(c.order_quantity) as order_quantity,(sum(c.order_quantity)/a.total_set_qnty) as order_quantity_set  from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c ,wo_pre_cost_trim_cost_dtls d, wo_pre_cost_trim_co_cons_dtls e where  a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and a.job_no=d.job_no and a.job_no=e.job_no and b.id=c.po_break_down_id and b.id=e.po_break_down_id  and d.id=e.wo_pre_cost_trim_cost_dtls_id and c.po_break_down_id=e.po_break_down_id and c.item_number_id= e.item_number_id and c.color_number_id=e.color_number_id and c.size_number_id=e.size_number_id  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and  b.id='$txt_po_id' and c.item_number_id='$cbo_gmt_item_id' $txt_country_cond and d.id=$cbo_trim_precost_id  group by  b.id, b.po_number,b.po_quantity,a.total_set_qnty,c.color_number_id,c.size_number_id,c.item_number_id  order by b.id, c.color_number_id,c.size_number_id";
					//$sql="select b.id, b.po_number,b.po_quantity,min(c.id) as color_size_table_id,c.color_number_id,c.size_number_id,sum(c.order_quantity) as order_quantity,(sum(c.order_quantity)/a.total_set_qnty) as order_quantity_set  from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id and  b.id='$txt_po_id'  $txt_country_cond and a.status_active=1 and b.status_active=1 and c.status_active=1 group by  b.id, b.po_number,b.po_quantity,a.total_set_qnty,c.color_number_id,c.size_number_id  order by b.id, c.color_number_id,c.size_number_id";//and c.item_number_id='$cbo_gmt_item_id'
					}
					else{
						// With Gmts Item
						$trimQtyArray=$trims->getQtyArray_by_precostdtlsid();
						$trimAmtArray=$trims->getAmountArray_precostdtlsid();
						...
						if ( count($data_array)>0){
							...
							foreach( $data_array as $row ){
								...
								//echo $cbo_trim_precost_id."==".$txt_po_id."==".$row[csf('size_number_id')]."/";
								//print_r( $trimQtyArray[$cbo_trim_precost_id]);
								//print_r( $trimQtyArray[$cbo_trim_precost_id][$txt_po_id]);//[$row[csf('size_number_id')]]."==";
								if($cbo_colorsizesensitive==1){
									$Req=$trimQtyArray[$txt_po_id][$cbo_trim_precost_id][$row[csf('color_number_id')]];
									$Amt=$trimAmtArray[$txt_po_id][$cbo_trim_precost_id][$row[csf('color_number_id')]];
									$rate=$Amt/$Req;
									
								}
								if($cbo_colorsizesensitive==2){
									$Req=$trimQtyArray[$txt_po_id][$cbo_trim_precost_id][$row[csf('size_number_id')]];
									$Amt=$trimAmtArray[$txt_po_id][$cbo_trim_precost_id][$row[csf('size_number_id')]];
									$rate=$Amt/$Req;
								}
								...
								if($cbo_colorsizesensitive==4){
									$Req=$trimQtyArray[$txt_po_id][$cbo_trim_precost_id][$row[csf('color_number_id')]][$row[csf('size_number_id')]];
									$Amt=$trimAmtArray[$txt_po_id][$cbo_trim_precost_id][$row[csf('color_number_id')]][$row[csf('size_number_id')]];
									$rate=$Amt/$Req;
									//echo  $Amt."/".$Req;
								}
								if($cbo_colorsizesensitive==0){
									$Req=$txt_req_quantity;
								}
                    ...
                    <tr id="break_1" align="center">
                                    ...
                                    <td>
                                    <input type="text" id="qty_<? echo $i;?>" onBlur="validate_sum( <? echo $i; ?> )" onChange="set_sum_value( 'qty_sum', 'qty_' );set_sum_value( 'woqty_sum', 'woqny_' );calculate_requirement(<? echo $i;?>);copy_value(this.value,'qty_',<? echo $i;?>)"  name="qty_<? echo $i;?>" class="text_boxes_numeric" style="width:70px"   placeholder="<? echo $Req;//echo $txtwoq_cal; ?>" value="<? if($booking_data_arr[$row[csf('color_size_table_id')]][id]){ echo $booking_data_arr[$row[csf('color_size_table_id')]][cons];} else { /*echo $Req;*/} ?>"/> 
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                    <input type="text" id="amount_<? echo $i;?>"  name="amount_<? echo $i;?>"  onBlur="set_sum_value( 'amount_sum', 'amount_' ) " class="text_boxes_numeric" style="width:50px"  value="<? echo $booking_data_arr[$row[csf('color_size_table_id')]][amount]; ?>" placeholder="<? echo $Amt;//echo $txtwoq_cal; ?>" readonly> 
                                    </td>
                                    <td>
              ...
              /*  $sql_cons_data=sql_select("select b.cons from wo_pre_cost_trim_cost_dtls a , wo_pre_cost_trim_co_cons_dtls b where a.id=b.wo_pre_cost_trim_cost_dtls_id and  a.id='".$selectResult[csf('pre_cost_fabric_cost_dtls_id')]."' and b.po_break_down_id='".$selectResult[csf('po_break_down_id')]."' and a.is_deleted=0  and a.status_active=1");
               ...
               // $costing_per=return_field_value("costing_per", "wo_pre_cost_mst", "job_no='".$selectResult[csf('job_no')]."'");
                ...
                list($sql_cu_woq_row)=$sql_cu_woq;
				 //$cu_woq= $cu_woq_arr[$selectResult[csf('po_break_down_id')]][$selectResult[csf('pre_cost_fabric_cost_dtls_id')]][$$selectResult[csf('gmt_item')]][$selectResult[csf('trim_group')]]['cu_woq'];
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$field_array="id,booking_type,is_short,booking_no_prefix,booking_no_prefix_num,booking_no,company_id,buyer_id,job_no, 	item_category,supplier_id,currency_id,exchange_rate,booking_date,delivery_date,pay_mode,source,attention,ready_to_approved,remarks,entry_form,inserted_by,insert_date";
		$data_array ="(".$id.",2,".$cbo_isshort.",'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',".$cbo_company_name.",".$cbo_buyer_name.",".$txt_job_no.",4,".$cbo_supplier_name.",".$cbo_currency.",".$txt_exchange_rate.",".$txt_booking_date.",".$txt_delivery_date.",".$cbo_pay_mode.",".$cbo_source.",".$txt_attention.",".$cbo_ready_to_approved.",".$txt_remarks.",43,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
	...
	else if ($operation==1)   // Update Here
	{
		...
		//}
		 $field_array_up="is_short*buyer_id*job_no*item_category*supplier_id*currency_id*exchange_rate*booking_date*delivery_date*pay_mode*source*attention*ready_to_approved*remarks*entry_form*updated_by*update_date"; 
		 
		 $data_array_up ="".$cbo_isshort."*".$cbo_buyer_name."*".$txt_job_no."*4*".$cbo_supplier_name."*".$cbo_currency."*".$txt_exchange_rate."*".$txt_booking_date."*".$txt_delivery_date."*".$cbo_pay_mode."*".$cbo_source."*".$txt_attention."*".$cbo_ready_to_approved."*".$txt_remarks."*43*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
...
if ($action=="save_update_delete_dtls")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		if($db_type==0)
		{
		 ...
		 $field_array1="id,pre_cost_fabric_cost_dtls_id,po_break_down_id,job_no,booking_no,booking_type,is_short,trim_group,uom,sensitivity,wo_qnty,rate,exchange_rate,amount,delivery_date,country_id_string,gmt_item,inserted_by,insert_date";
		...
		$data_array1 .="(".$id_dtls.",".$cbo_trim_precost_id.",".$txt_po_id.",".$txt_job_no.",".$txt_booking_no.",2,".$cbo_isshort.",".$txt_trim_group_id.",".$cbo_uom.",".$cbo_colorsizesensitive.",".$txt_quantity.",".$txt_avg_price.",".$txt_exchange_rate_dtls.",".$txt_amount.",".$txt_delevary_date.",".$txt_country.",".$cbo_gmt_item_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		//echo $data_array1;
		//==============================
        	...
        	<td align="center" width="100%">
            	<table  cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
        			...
        			<tr>
                    	<td> 
							<? 
								echo create_drop_down( "cbo_company_mst", 120, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", '', "load_drop_down( 'trims_booking_urmi_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
							?>
                        </td>
                         <td id="buyer_td">
                              <? 
...
if ($action=="create_booking_search_list_view")
{
   ...
   if($data[7]==2)
	...
	if($data[9]==0)
	{
		$sql="select min(a.id) as id, a.booking_no_prefix_num, a.booking_no, company_id, a.job_no, a.supplier_id, a.booking_date, a.delivery_date, c.file_no, c.grouping, a.inserted_by from wo_booking_mst a, wo_booking_dtls b, wo_po_break_down c where a.booking_no=b.booking_no and b.po_break_down_id=c.id and a.booking_type=2 and a.entry_form=43 $company $buyer ".set_user_lavel_filtering(' and a.buyer_id','buyer_id')." $supplier_id $booking_date $booking_cond $job_cond $file_cond $ref_cond group by a.booking_no_prefix_num, a.job_no, a.booking_no, company_id, a.supplier_id, a.booking_date, a.delivery_date, c.file_no, c.grouping, a.inserted_by order by id DESC";
		
		echo  create_list_view("list_view", "Booking No,Company,Supplier,Insert User,Booking Date,Delivery Date,Job No,File No,Ref. No", "60,100,100,100,70,70,100,100","900","420",0, $sql , "js_set_value", "booking_no", "", 1, "0,company_id,supplier_id,inserted_by,0,0,0,0,0", $arr , "booking_no_prefix_num,company_id,supplier_id,inserted_by,booking_date,delivery_date,job_no,file_no,grouping", '','','0,0,0,0,3,3,0','','');

	}
	else
	{
		$sql="select min(a.id) as id, a.job_no, a.booking_no_prefix_num, a.booking_no, company_id, a.supplier_id, a.booking_date, a.delivery_date, a.inserted_by from wo_booking_mst a where  a.booking_no not in ( select a.booking_no from wo_booking_mst a, wo_booking_dtls b, wo_po_break_down c where a.booking_no=b.booking_no and b.po_break_down_id=c.id and  a.booking_type=2 and a.entry_form=43 $company $buyer $supplier_id $booking_date $booking_cond $job_cond $file_cond $ref_cond group by a.booking_no_prefix_num, a.booking_no, company_id, a.supplier_id, a.booking_date, a.delivery_date) and a.booking_type=2 and a.entry_form=43 $company $buyer ".set_user_lavel_filtering(' and a.buyer_id','buyer_id')."  $supplier_id $booking_date $booking_cond group by a.booking_no_prefix_num, a.booking_no, a.job_no, company_id, a.supplier_id, a.booking_date, a.delivery_date, a.inserted_by order by id DESC";
		
		echo  create_list_view("list_view", "Booking No,Company,Supplier,Insert User,Booking Date,Delivery Date,Job No", "60,100,100,100,70,70","700","420",0, $sql , "js_set_value", "booking_no", "", 1, "0,company_id,supplier_id,inserted_by,0,0,0,0", $arr , "booking_no_prefix_num,company_id,supplier_id,inserted_by,booking_date,delivery_date,job_no", '','','0,0,0,0,3,3,0,0','','');
	}
...
if ($action=="populate_data_from_search_popup")
{
	 $sql= "select booking_no,booking_date,company_id,ready_to_approved,buyer_id,job_no,currency_id,exchange_rate,pay_mode,supplier_id,attention,delivery_date,source,is_approved,is_short,remarks from wo_booking_mst  where booking_no='$data'";
	 //echo $sql;die;
	 $data_array=sql_select($sql);
	 foreach ($data_array as $row)
	 {
		...
		//echo "document.getElementById('buyer_td').innerHTML = '".$buyer_dropdown."';\n";
		//echo "get_php_form_data(".$row[csf("company_id")].", 'populate_variable_setting_data', 'requires/trims_booking_urmi_controller' );\n";
	   ...
	   if(count($nameArray_item)>0){
        ...
        <table style="border:1px solid black" align="left" class=""  cellpadding="0" width="100%" cellspacing="0" rules="all" >
           <tr>
            ...
            foreach($nameArray_item as $result_item){
				$i++;
				$nameArray_item_description=sql_select( "select b.description, b.brand_supplier,b.item_color, b.color_number_id ,min(b.color_size_table_id) as color_size_table_id,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a,  wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and  a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=1 and   a.id= ".$result_item[csf('id')]." and a.trim_group=".$result_item[csf('trim_group')]." and b.requirment !=0 group by b.description, b.brand_supplier,b.item_color, b.color_number_id order by color_size_table_id "); 
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo count($nameArray_item_description)+1; ?>">
                <? echo $i; ?>
...
if($action=="show_trim_booking_report2")
{
	...
	<div style="width:1333px" align="center">       
       ...
       <table width="100%" style="border:1px solid black;table-layout: fixed;">                    	
		...
		foreach($nameArray_job_po as $nameArray_job_po_row){
		$nameArray_item=sql_select( "select  pre_cost_fabric_cost_dtls_id,trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."'   and sensitivity=1 group by pre_cost_fabric_cost_dtls_id,trim_group  order by trim_group ");
	    if(count($nameArray_item)>0){
        ?>
        &nbsp;
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
        ...
        <!--==============================================AS PER GMTS Color & SIZE START=========================================  -->
		<?
		$nameArray_item=sql_select( "select  pre_cost_fabric_cost_dtls_id,trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."'  and sensitivity=4 group by pre_cost_fabric_cost_dtls_id,trim_group order by trim_group");
	   if(count($nameArray_item)>0)
		{
        ?>
        ...
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
            ...
            foreach($nameArray_item as $result_item)
            {
			 ...
			 $nameArray_color=sql_select( "select a.pre_cost_fabric_cost_dtls_id,min(b.id) as bid, b.item_color as color_number_id,b.item_size,b.description, b.brand_supplier,sum(b.requirment) as cons,avg(b.rate) as rate, sum(b.amount) as amount from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.trim_group=".$result_item[csf('trim_group')]." and a.pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." and a.sensitivity=4 and b.requirment !=0 group by a.pre_cost_fabric_cost_dtls_id, b.item_color,b.item_size,b.description, b.brand_supplier order by bid "); 
			 
            $nameArray_item_description=sql_select( "select distinct uom from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."'  and sensitivity=4 and trim_group=".$result_item[csf('trim_group')]." and pre_cost_fabric_cost_dtls_id=".$result_item[csf('pre_cost_fabric_cost_dtls_id')]." order by trim_group "); 
            ?>
            <tr>
                <td style="border:1px solid black" rowspan="<? echo   (count($nameArray_item_description)*count($nameArray_color)); ?>">
                <? echo $i ; ?>
...
if($action=="show_trim_booking_report4"){
	...
	<div style="width:1333px" align="center">       
       <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid black">
		...
		$repeat_no_string='';
		$nameArray_order_repeat_no=sql_select( "select distinct a.order_repeat_no  from wo_po_details_master a, wo_booking_dtls b where a.job_no=b.job_no and b.booking_no=$txt_booking_no"); 
...
if($action=="show_trim_booking_report5")
{
	...
	<div style="width:1333px" align="center">       
       ...
       <table width="100%" style="border:1px solid black">                    	
        ...
        $nameArray_item=sql_select( "select distinct trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]."  and sensitivity=1 order by trim_group "); 
        $nameArray_color=sql_select( "select  b.color_number_id, min(b.color_size_table_id) as  color_size_table_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=1 group by b.color_number_id order by color_size_table_id"); 
		if(count($nameArray_color)>0)
		{
        ?>
        ...
        <!--==============================================AS PER GMTS SIZE START=========================================  -->
		<?
        $nameArray_item=sql_select( "select distinct trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=2 order by trim_group "); 
        $nameArray_size=sql_select( "select  b.item_size  as gmts_sizes, min(b.color_size_table_id) as color_size_table_id from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.sensitivity=2 group by b.item_size order by color_size_table_id");
		if(count($nameArray_item)>0)
		{
        ?>
        ...
        $nameArray_item=sql_select( "select distinct trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=3 order by trim_group "); 
        ...
        <!--==============================================AS PER GMTS Color & SIZE START=========================================  -->
		<?
        $nameArray_item=sql_select( "select distinct trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=4 order by trim_group "); 
        $nameArray_size=sql_select( "select  b.item_size  as gmts_sizes, min(b.color_size_table_id) as color_size_table_id  from wo_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id and a.booking_no=b.booking_no  and a.booking_no=$txt_booking_no and a.job_no='".$nameArray_job_po_row[csf('job_no')]."' and a.po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and a.sensitivity=4 group by b.item_size order by color_size_table_id");
        ...
        <!--==============================================AS PER Color & SIZE  END=========================================  -->
         ...
         <!--==============================================NO NENSITIBITY START=========================================  -->
		<?
        $nameArray_item=sql_select( "select distinct trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=0 order by trim_group "); 
        //$nameArray_color=sql_select( "select distinct b.color_number_id from wo_trims_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=$txt_booking_no and a.sensitivity=1"); 
		//$nameArray_color= array();
		if(count($nameArray_item)>0)
		{
...
if($action=="show_trim_booking_report6")
{
	...
	<div style="width:1333px" align="center">       
       ...
       <table width="100%" style="border:1px solid black;table-layout: fixed;">                    	
		...
		$nameArray_job_po=sql_select( "select job_no from wo_booking_dtls  where booking_no=$txt_booking_no group by job_no order by job_no "); 
		foreach($nameArray_job_po as $nameArray_job_po_row){
		$nameArray_item=sql_select( "select  pre_cost_fabric_cost_dtls_id,trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."'   and sensitivity=1 group by pre_cost_fabric_cost_dtls_id,trim_group,delivery_date  order by trim_group ");
	    if(count($nameArray_item)>0){
        ?>
        &nbsp;
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
        ...
        <!--==============================================Size Sensitive START=========================================  -->
		<?
		$nameArray_item=sql_select( "select  pre_cost_fabric_cost_dtls_id,trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."'  and sensitivity=2 group by pre_cost_fabric_cost_dtls_id,trim_group,delivery_date  order by trim_group "); 
		if(count($nameArray_item)>0)
		{
        ?>
        ...
        <!--==============================================Size Sensitive END=========================================  -->
         ...
         <!--==============================================AS PER CONTRAST COLOR START=========================================  -->
		<?
		 $nameArray_item=sql_select( "select pre_cost_fabric_cost_dtls_id, trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."'  and sensitivity=3 group by pre_cost_fabric_cost_dtls_id,trim_group,delivery_date order by trim_group "); 
		if(count($nameArray_item)>0)
		{
        ?>
        <table border="1" align="left" cellpadding="0" width="100%" cellspacing="0" rules="all" >
        ...
        <!--==============================================AS PER GMTS Color & SIZE START=========================================  -->
		<?
		$nameArray_item=sql_select( "select  pre_cost_fabric_cost_dtls_id,trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."'  and sensitivity=4 group by pre_cost_fabric_cost_dtls_id,trim_group,delivery_date order by trim_group");
	   if(count($nameArray_item)>0)
		{
        ?>
	   ...
	   ?>
		...
		<?
       //$nameArray_item=sql_select( "select distinct trim_group from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."' and po_break_down_id=".$nameArray_job_po_row[csf('po_break_down_id')]." and sensitivity=0 order by trim_group ");
		$nameArray_item=sql_select( "select  pre_cost_fabric_cost_dtls_id,trim_group,delivery_date from wo_booking_dtls  where booking_no=$txt_booking_no and job_no='".$nameArray_job_po_row[csf('job_no')]."'  and sensitivity=0 group by pre_cost_fabric_cost_dtls_id,trim_group,delivery_date order by trim_group ");
        //$nameArray_color=sql_select( "select distinct b.color_number_id from wo_trims_booking_dtls a, wo_trim_book_con_dtls b where a.id= b.wo_trim_booking_dtls_id  and a.booking_no=$txt_booking_no and a.sensitivity=1"); 
		//$nameArray_color= array();
		if(count($nameArray_item)>0)
		{
