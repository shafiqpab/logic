...
if ($action=="populate_data_from_search_popup"){
	 ...
	 foreach ($data_array as $row){
		...
		$wo_id=$row[csf('id')];
		$sql_cause="select MAX(id) as id from fabric_booking_approval_cause where page_id='410' and entry_form=7  and booking_id='$wo_id' and approval_type=0 and status_active=1 and is_deleted=0"; //and user_id='$user_id'
        //echo $sql_cause; //die;
...
if ($action=="fabric_search_list_view")
{
   ...
   $sql_vari_lib="select item_category_id, variable_list, excut_source from variable_order_tracking where company_name=".$company." and item_category_id=3  and variable_list=72 and status_active=1"; 
		...
		if($source_from==4) //Booking Supplier and Rate
		{
			...
			if (str_replace("'","",$job)=="" && trim($style)=="")
			{
			 ...
			 $sql_cs_app=sql_select("SELECT a.id,a.approved, a.sys_number,  d.id as dtls_id,d.supp_id,d.neg_price,a.req_item_no, a.company_id,b.item_category_id, b.item_description, b.detarmination_id 
					...
					where a.id=b.mst_id and b.id=d.dtls_id and d.mst_id=a.id and c.style_ref_no=a.req_item_no and a.entry_form=512 and c.job_no_prefix_num='$job'   and a.ready_to_approved=1 and a.approved in(1,3) and d.approved in(1,3) and d.supp_id=$cbo_supplier_name and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and d.status_active=1 and d.is_deleted=0 $job_cond_cs $style_cond_cs  order by a.id,d.id asc");
	...
	/*$subpluer_cond="";
	if(!empty($cbo_supplier_name))
	{
		$subpluer_cond=" and ( d.nominated_supp_multi like '$cbo_supplier_name,%' or d.nominated_supp_multi like '%,$cbo_supplier_name' or d.nominated_supp_multi like '%,$cbo_supplier_name,%' or d.nominated_supp_multi = '$cbo_supplier_name' or  d.nominated_supp_multi is null) ";
		
	}*/
	
	$sql=sql_select("select b.page_id, b.approval_need, b.validate_page, b.allow_partial from approval_setup_mst a, approval_setup_dtls b where a.id=b.mst_id and a.company_id='$data[0]' and b.page_id in (2,36,37) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.setup_date");
	...
	//Dont HIde Issue id ISD-21-04458---and open ISD-22-15490
	/*if($app_nessity==1)
	{
		 if($allow_partial==1) $budgetAppCond=" and b.approved in (1,3)";
		 else $budgetAppCond=" and b.approved=1";
...
if ($action=="generate_fabric_booking")
{
	...
	if($source_from==4) //Booking Supplier and Rate
	{
		 $sql_cs_app=sql_select("SELECT a.id, a.approved, a.sys_number, d.id as dtls_id, d.supp_id, d.neg_price, d.last_approval_rate, a.req_item_no, a.company_id, b.item_category_id, b.item_description, b.detarmination_id 
				from req_comparative_mst a, req_comparative_dtls b,wo_po_details_master c,req_comparative_supp_dtls d
	...
	{	
		foreach($sql_data as $sql_row)
		{
			...
			//$cu_booking_qty=array_sum($cu_booking_data_arr[$pre_cost_fabric_cost_dtls_id][$sql_row['color_number_id']][$sql_row['dia_width']][$sql_row['remarks']]['cu_booking_qty']);
		...
		if($sql_row['rd_no']=='') $sql_row['rd_no']='';		
			...
			$bal_qty=$req_qty-$cu_booking_qty;
			$bal_amt=$bal_qty*$rate;
			//echo $pre_cost_fabric_cost_dtls_id.'='.$sql_row['color_number_id'].'='.$sql_row['dia_width'].'='.$sql_row['remarks'].'jj<br>';
			//echo $pre_cost_fabric_cost_dtls_id.'='.$req_qty.'='.$cu_booking_qty.'='.$bal_qty.'req<br>';
	...
	?>
    <table width="2460" class="rpt_table" border="0" rules="all" id="tbl_fabric_booking">
        <thead>
			...
			foreach($sql_data as $sql_row)
			{
				...
				if(number_format($bal_qty,4,'.','') >0)
				{
					...
					<tr bgcolor="<?=$bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<?=$i; ?>">
						...
						<td width="90"><input type="text" style="width:75px; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtadj_<?=$i; ?>" value="" class="text_boxes_numeric" onChange="claculate_acwoQty(<?=$i; ?>);"  /></td>
						...
						//	echo $tot_cs_supp_rate.'dSSSS';
                            ...
                            if(number_format($bal_qty,4,'.','') >0){
                                ...
                                <tr bgcolor="<?=$bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<?=$i;?>">
                                    ...
                                    <td width="90">
                                        <input type="text" style="width:75px; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtwoq_<?=$i;?>" value="<?=number_format($bal_qty,4,'.','');?>" class="text_boxes_numeric" onChange="claculate_acwoQty(<?=$i; ?>)"   />
                                    ...
                                    <td width="90"><input type="text" style="width:75px; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtadj_<?=$i; ?>" value="" class="text_boxes_numeric" onChange="claculate_acwoQty(<?=$i; ?>);"  />
                                    </td>
                                    <td width="90"><input type="text" style="width:75px; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtacwoq_<?=$i; ?>" value="<?=number_format($bal_qty,4,'.',''); ?>"  class="text_boxes_numeric" readonly /></td>
    ...
    <table width="2460" class="rpt_table" border="0" rules="all">
        <thead>
            <th width="90">&nbsp;</th>
            <th width="80">&nbsp;</th>
            <th width="100">&nbsp;</th>
				...
				if(str_replace("'","",$cbo_level)==1)
				{
					...
					foreach($sql_data as $sql_row)
					{
						...
						<tr bgcolor="<?=$bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<?=$i; ?>">
								...
								<td width="90">
									<input type="text" style="width:75px; font-family:Verdana, Geneva, sans-serif; font-size:11px; text-align:right; background-color:#FFC" id="txtadj_<?=$i; ?>" value="<?=number_format($adjust_qty,4,'.',''); ?>" class="text_boxes_numeric" onChange="claculate_acwoQty(<?=$i; ?>)"  />
								</td>
								<td width="90">
						...
						foreach($precost_id as $color_id)
						{
							foreach($color_id as $diawith)
							{
								...
								<input type="text" style="width:75px; font-family:Verdana, Geneva, sans-serif; font-size:11px; text-align:right; background-color:#FFC" id="txtshrinkagew_<?=$i; ?>" value="<?=$shrinkage_w;?>" class="text_boxes_numeric"  />
										...
										<td width="90"><input type="text" style="width:75px; font-family:Verdana, Geneva, sans-serif; font-size:11px; text-align:right; background-color:#FFC" id="txtadj_<?=$i; ?>" value="<?=number_format($adjust_qty,4,'.',''); ?>" class="text_boxes_numeric" onChange="claculate_acwoQty(<?=$i; ?>);" />
										</td>
...
if($action=="show_fabric_booking_list")
{
	...
	?>
	<table width="1550" class="rpt_table" border="0" rules="all">
		<thead>
			<th width="100"></th>
	...
	<div style=" max-height:200px; overflow-y:scroll; width:1550px"  align="left">
		<table width="1533" class="rpt_table" id="tbl_fabric_booking_list" border="0" rules="all">
			<tbody>
				<?
				$total_wo_qnt=0;
				if($cbo_level==11)
			...
			<tfoot>
				<tr>
					<th colspan="10" align="right">Total WO. Qnty:</th>
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0){
		...
		$field_array="item_category*fabric_source*currency_id*exchange_rate*pay_mode*source*booking_date*delivery_date*booking_month*booking_year*supplier_id*attention*booking_percent*colar_excess_percent*cuff_excess_percent*ready_to_approved*updated_by*update_date*rmg_process_breakdown*fabric_composition*uom*remarks*cbo_level*delivery_address*season_year*season_id*brand_id*pay_term*tenor";
		 $data_array ="".$cbo_fabric_natu."*".$cbo_fabric_source."*".$cbo_currency."*".$txt_exchange_rate."*".$cbo_pay_mode."*".$cbo_source."*".$txt_booking_date."*".$txt_delivery_date."*".$cbo_booking_month."*".$cbo_booking_year."*".$cbo_supplier_name."*".$txt_attention."*".$txt_booking_percent."*".$txt_colar_excess_percent."*".$txt_cuff_excess_percent."*".$cbo_ready_to_approved."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$processloss_breck_down."*".$txt_fabriccomposition."*".$cbouom."*".$txt_remark."*".$cbo_level."*'".$delivery_address."'*".$cbo_season_year."*".$cbo_season_id."*".$cbo_brand_id."*'".str_replace("'", "", $cbo_payterm_id)."'*'".str_replace("'", "", $txt_tenor)."'";
		...
		for ($i=1;$i<=$total_row;$i++)
		 {
			 $txtjob="txtjob_".$i;
			 $txtpoid="txtpoid_".$i;
			 $poidArr[str_replace("'","",$$txtpoid)]=str_replace("'","",$$txtpoid);
			 
		 }
	$sqlChk=sql_select("select a.cbo_level,b.po_break_down_id as po_id from wo_booking_mst a,wo_booking_dtls b where a.booking_no=b.booking_no and a.status_active=1 and b.status_active=1 and b.po_break_down_id in(".implode(',',$poidArr).")");
	 
		foreach($sqlChk as $row){
			$po_levelArr[$row[csf('po_id')]]=$row[csf('cbo_level')];
		}*/
	if ($operation==0){
		$con = connect();
		if($db_type==0){
	...
	?>
                            ...
                            <tr>
                            	<? if($currency_id==1){$paysa_sent="Paisa";} else if($currency_id==2){$paysa_sent="CENTS";} ?>
		                   		<td colspan="17" align="left"><b>In Word: <? echo number_to_words(number_format($total_amount,2),$currency[$currency_id],$paysa_sent); ?></b></td>
		               		</tr>
                            </tfoot>
                            </table>
                            <?
...
if($action=="print6booking") //Add For windy
{
	...
	$location_lib=return_library_array( "select id, location_name from lib_location where status_active=1 and is_deleted=0 and company_id='$cbo_company_name'", "id", "location_name");



	$mstSql="SELECT a.buyer_id, a.booking_no, a.booking_date, a.supplier_id, a.currency_id, a.exchange_rate, a.attention, a.delivery_date, a.fabric_source, a.delivery_address, a.remarks,a.pay_mode, a.pay_term,a.tenor,a.source, b.job_no, b.color_type, b.gsm_weight, b.dia_width, b.fabric_color_id, b.gmts_color_id, b.remark as dtlsremark, b.fin_fab_qnty, b.amount, c.style_ref_no, c.dealing_marchant, e.body_part_id, e.lib_yarn_count_deter_id, e.construction, e.composition, e.uom,e.id as pre_cost_fabric_cost_dtls_id,e.gsm_weight_type,c.location_name
    ...
    <div style="width:1310px" align="center">
        <table width="100%" cellpadding="0" cellspacing="0" style="border:0px solid black" >
            <tr>
                <td width="100">
                	<img src='<?="../../".$imge_arr[$cbo_company_name]; ?>' height='100%' width='100%' />
                </td>
                <td width="1250">
                    <table width="100%" cellpadding="0" cellspacing="0"  border="0" >
						
                        <tr>
                        	<td align="center" style="font-size:24px;"><b><?=$company_library[$cbo_company_name]; ?></b></td>
                        </tr>
                        <!-- <tr>
                            ...
                            // }
                            // else $location="";
                             $address_sql="select address from  lib_location where status_active=1 and is_deleted=0 and company_id=$cbo_company_name and id=$location_name";
                           // echo $address_sql;
                             $address_res=sql_select($address_sql);
                             $address_str="";
                             foreach ($address_res as $row) {
                             	if(!empty($address_str))
                             	{
                             		$address_str.=$row[csf('address')].",";
                             	}
                             	else{
                             		$address_str=$row[csf('address')];
                             	}
                             }
                            //  echo $location_lib[$mstSqlRes[0][csf('location_name')]];
							echo $address_str;
                            ?>
                            </td>
                        </tr> -->
                        <tr>
                            <td align="center" style="font-size:20px;">
                            	<strong>
									Purchase Order
									<!-- <? if($report_title!="") { echo $report_title.'-'.$fabric_source[$cbo_fabric_source];}?> -->
								</strong>
                            </td>
                       </tr>
					   <tr>
						<td align="right" style="font-size:20px; padding-right: 50px;"><b>PO Type:Fabric</b></td>
						</tr>
                    </table>
                </td>
            </tr>
        ...
        <table class="rpt_table" width="100%" border="1" cellpadding="0" cellspacing="0" rules="all" >
            <tr>
                <td width="120"><span><b>PO No</b></span></td>
                <td width="130"><span> &nbsp;<b><?=$mstSqlRes[0][csf('booking_no')]; ?></b></span></td>
                <td width="120"><span><b>Dealing Person</b></span></td>
                <td width="130"><span> &nbsp;<b><?=$marchentrArr[$mstSqlRes[0][csf('dealing_marchant')]]; ?></b></span></td>
                <td width="120"><span><b>Buyer</b></span></td>
                <td width="130"><span> &nbsp;<b><?=$buyer_name_arr[$mstSqlRes[0][csf('buyer_id')]]; ?></b></span></td>
                <td width="120"><span><b>PO Date</b></span></td>
                <td width="130"><span> &nbsp;<b><?=change_date_format($mstSqlRes[0][csf('booking_date')]); ?></b></span></td>
                <td width="120"><span><b>Pay Term</b></span></td>
				<td align="left">&nbsp;<b><? 
				...
				?></b></td>
                <!-- <td><span> &nbsp;<b><?=$pay_mode[$mstSqlRes[0][csf('pay_mode')]]; ?></b></span></td> -->
            </tr>
            <tr>
                <td><span><b>Supplier Name</b></span></td>
                <td><span> &nbsp;<b><?=$supplierName; ?></b></span></td>
                <td><span><b>Contract Person</b></span></td>
                <td><span> &nbsp;<b><?=$mstSqlRes[0][csf('attention')]; ?></b></span></td>
                <td><span><b>E-Mail ID</b></span></td>
                <td><span> &nbsp;<b><?=$marchentr_email[$mstSqlRes[0][csf('dealing_marchant')]]; ?></b></span></td>
                <td><span><b>Fabric ETD</b></span></td>
                <td><span> &nbsp;<b><?=change_date_format($mstSqlRes[0][csf('delivery_date')]); ?></b></span></td>
                <td><span><b>Source Type</b></span></td>
                <td><span> &nbsp;<b><?=$source[$mstSqlRes[0][csf('source')]]; ?></b></span></td>
            </tr>
            <tr>
            	<td><span><b>Delivery Address</b></span></td>
                <td colspan="9"><span> &nbsp;<b><?=$mstSqlRes[0][csf('delivery_address')]; ?></b></span></td>
               
            </tr>
            <tr>
            	<td><span><b>Remark</b></span></td>
            	<td colspan="9"><span> &nbsp;<b><?=$mstSqlRes[0][csf('remarks')]; ?></b></span></td>
            </tr>
        </table>
        <?
		...
		foreach ($mstSqlRes as $row) {
			array_push($fab_cost_dtls_id_arr, $row[csf('pre_cost_fabric_cost_dtls_id')]);
		}
		$pre_cost_fab_cond=where_con_using_array($fab_cost_dtls_id_arr,0,"pre_cost_fabric_cost_dtls_id");
		$sql_pre_cost_fab_dia=sql_select("select item_size,dia_width,pre_cost_fabric_cost_dtls_id from wo_pre_cos_fab_co_avg_con_dtls  where  status_active=1  $pre_cost_fab_cond group by item_size,dia_width,pre_cost_fabric_cost_dtls_id");
	
		$pre_cost_fab_dia_arr=array();
		foreach ($sql_pre_cost_fab_dia as $row) {
			$pre_cost_fab_dia_arr[$row[csf('pre_cost_fabric_cost_dtls_id')]]=$row[csf('dia_width')]."/".$row[csf('item_size')];
		}
		foreach($mstSqlRes as $row)
		{
			...
			$jobWiseBookingArr[$jobstyle][$row[csf('body_part_id')]][$lib_comp_id][$row[csf('color_type')]][$row[csf('gmts_color_id')]][$row[csf('fabric_color_id')]]['gsm'].=$row[csf('gsm_weight')].' /'.$fabric_weight_type[$row[csf('gsm_weight_type')]].',';
			// $jobWiseBookingArr[$jobstyle][$row[csf('body_part_id')]][$lib_comp_id][$row[csf('color_type')]][$row[csf('gmts_color_id')]][$row[csf('fabric_color_id')]]['dia'].=$row[csf('dia_width')].','.$pre_cost_fab_dia_arr[$row[csf('pre_cost_fabric_cost_dtls_id')]].',';
			...
			// $jobWiseBookingArr[$jobstyle][$row[csf('body_part_id')]][$lib_comp_id][$row[csf('color_type')]][$row[csf('gmts_color_id')]][$row[csf('fabric_color_id')]]['gsm_weight_type'].=
			// $fabric_weight_type[$row[csf('gsm_weight_type')]].',';
			
			$bodyPartWiseArr[$row[csf('body_part_id')]][$lib_comp_id][$row[csf('color_type')]][$row[csf('fabric_color_id')]]['gsm'].=$row[csf('gsm_weight')].' /'.$fabric_weight_type[$row[csf('gsm_weight_type')]].',';
			// $bodyPartWiseArr[$row[csf('body_part_id')]][$lib_comp_id][$row[csf('color_type')]][$row[csf('fabric_color_id')]]['dia'].=$row[csf('dia_width')].','.$pre_cost_fab_dia_arr[$row[csf('pre_cost_fabric_cost_dtls_id')]].',';
		...
		// echo "SELECT a.id, a.fabric_ref, a.rd_no, a.type,a.shrinkage_l,a.shrinkage_w from lib_yarn_count_determina_mst a where a.is_deleted=0 and a.id in ($imp_libcompid)";
		$feblibDataArr=array();
        ...
        <table class="rpt_table" width="100%" border="1" cellpadding="0" cellspacing="0" rules="all" >
			...
			foreach($jobWiseBookingArr as $jobstyle=>$jobdata)
			{
					...
					foreach($body_partdata as $libcomp=>$libcompdata)
					{
						foreach($libcompdata as $colortype=>$colortypedata)
						{
							foreach($colortypedata as $gmtscolorid=>$gmtscolordata)	
							{
								foreach($gmtscolordata as $fabcolorid=>$dtlsdata)
								{
									$fabweighttype=implode(",",array_filter(array_unique(explode(",",$dtlsdata['gsm']))));
									...
									<tr style="word-break:break-all;word-wrap: break-word;">
                                    <? if($p==1) { ?>
										<td width="80" rowspan="<?=$jobspan; ?>"><?=$jobno; ?></td>
										<td width="90" rowspan="<?=$jobspan; ?>"><?=$styleref; ?></td>
                                     <? } ?>
										<td width="70"><?=$feblibDataArr[$libcomp]['fabref']; ?></td>
										<td width="70"><?=$feblibDataArr[$libcomp]['type']; ?></td>
										<td width="150"><?=$dtlsdata['composition']; ?></td>
										<td width="120"><?=$dtlsdata['construction']; ?></td>
										<td width="70"><?=$color_type[$colortype]; ?></td>
										<?
										$shrinkage=($feblibDataArr[$libcomp]['shrinkagel'])." ". "to"." ". ($feblibDataArr[$libcomp]['shrinkagew']);
										?>
										<td width="70" align="center"><?=$shrinkage; ?></td>
										<td width="70"><?=$fabweighttype; ?></td>
										<td width="70"><?=$width; ?></td>
										<td width="90"><?=$color_library[$fabcolorid]; ?></td>
										<td width='90' align="right"><?=number_format($dtlsdata['finQty'], 4,'.',''); ?></td>
										<td width="50" align="center"><p><? echo $unit_of_measurement[$dtlsdata['uom']]; ?>&nbsp;</p></td>
										<td width='50' align="right"><?=number_format($avgrate, 4,'.',''); ?></td>
										<td width='60' align="right"><?=number_format($dtlsdata['finAmt'], 4,'.',''); ?></td>
										<td><?=$remarksdtls; ?></td>
									</tr>
									<?
									$p++;
                ...
                <tr >
                    ...
                    <td width="90" colspan="11" align="right"><b>Job Total:</b></td>
                    <td width='60' align="right"><b><?=number_format($jobFinQty, 4,'.',''); ?></b></td>
                    <td width='50'>&nbsp;</td>
					<td width='50'>&nbsp;</td>
                    <td width='60' align="right"><b>$<?=number_format($jobFinAmt, 4,'.',''); ?></b></td>
                    <td>&nbsp;</td>
                </tr>
                <?
            ...
            <tr >
                ...
                <td width="90" colspan="11" align="right"><b>Grand Total:</b></td>
                <td width='60' align="right"><b><?=number_format($grandFinQty, 4,'.',''); ?></b></td>
                <td width='50'>&nbsp;</td>
				<td width='50'>&nbsp;</td>
                <td width='60' align="right"><b>$<?=number_format($grandFinAmt, 4,'.',''); ?></b></td>
                <td>&nbsp;</td>
            </tr>
        </table>
		   <?
       ...
       <table width="100%" style="margin-top:1px">
           ...
           <table width="100%" class="rpt_table"  border="1" cellpadding="0" cellspacing="0" rules="all">
                <tr style="border:1px solid black;">
                    <td width="30%" style="border:1px solid black; text-align:left">Total PO Amount</td>
                    <td width="70%" style="border:1px solid black; text-align:left">$<? echo number_format($grandFinAmt,2);?></td>
                </tr>
                <tr style="border:1px solid black;">
                    <td width="30%" style="border:1px solid black; text-align:left">Total PO Amount (in word)</td>
                    <td width="70%" style="border:1px solid black;"><? echo number_to_words(def_number_format($grandFinAmt,2,""),$mcurrency, $dcurrency);?></td>
                </tr>
           </table>
           </td>
           </tr>
        ...
        <table width="100%" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all" style="font-family:Arial Narrow;">
            <thead>
                <tr>
                    <th width="3%" align="left"><strong style="font-size:16px">SL</strong></th>
                    <th width="97%" align="left"><strong style="font-size:16px">Terms & Condition</strong></th>
                </tr>
            </thead>
            <tbody>
            <?
