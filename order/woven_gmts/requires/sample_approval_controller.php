...
if($action=="show_sample_approved_list")
{
	$data=explode('_',$data);
	$buyer_id=$data[1];
	//$sql="select min(a.id) as id , a.sample_type_id from wo_po_sample_approval_info a ,lib_buyer_tag_sample b where b.tag_sample=a.sample_type_id and a.job_no_mst='$data[0]' and a.is_deleted=0 and a.status_active=1 and b.buyer_id=$buyer_id and b.sequ!=0 and (a.entry_form_id is null or a.entry_form_id=0)  group by a.sample_type_id order by id";
	$sql="select min(a.id) as id , a.sample_type_id from wo_po_sample_approval_info a ,lib_buyer_tag_sample b,wo_po_color_size_breakdown c ,lib_sample d
  where b.tag_sample=a.sample_type_id and c.id=a.color_number_id and
  a.job_no_mst = c.job_no_mst and d.id=b.tag_sample and d.id=a.sample_type_id and d.business_nature=3 and a.job_no_mst='$data[0]' and a.is_deleted=0 and a.status_active=1 and d.is_deleted=0 and d.status_active=1  and c.is_deleted=0 and c.status_active=1 and b.buyer_id=$buyer_id and b.sequ!=0 
  and (a.entry_form_id is null or a.entry_form_id=0) group by a.sample_type_id order by id ";
	// echo $sql;
	$data_array=sql_select($sql);
					...
					//	echo "select a.id as po_id,a.po_number,min(b.id) as color_mst_id, b.color_number_id,c.id as sample_table_id  from  wo_po_break_down a, wo_po_color_size_breakdown b, wo_po_sample_approval_info c where a.job_no_mst=b.job_no_mst and b.job_no_mst=c.job_no_mst and a.id=b.po_break_down_id and b.po_break_down_id=c.po_break_down_id and  b.id=c.color_number_id  and a.job_no_mst='$data[0]'  and c.sample_type_id ='$data[1]'  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and c.current_status = 1 and b.status_active=1 and (c.entry_form_id is null or c.entry_form_id=0) and c.is_deleted=0 and c.status_active=1 group by a.id,a.po_number,color_mst_id,b.color_number_id,c.id order by a.id,color_mst_id";
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
									<?
                                    echo create_drop_down( "cbosampletype_".$i, 140, "select sample_name,id from lib_sample where is_deleted=0 and status_active=1 and business_nature=3 order by sample_name","id,sample_name", '', "", $data[1], "",1,'' );
                                    ?>
                                </td>
                                <td>
                                ...
                                <td>
									<?
                                    echo create_drop_down( "cbostatus_".$i, 80, $row_status,"", '', "", $sample_table_data_array[csf("status_active")], "copy_value(this.value,'cbostatus_',". $i.")",$disabled,'' );
                                    ?>
	...
	else if ($operation==1)   // Update Here
	{
		...
		// $app_action_arr=array();
		 for ($i=1;$i<=$total_row;$i++)
		 {
			 ...
			 $txtsendtofatorydate="txtsendtofatorydate_".$i;
			 $txtsubmissiontobuyerdate="txtsubmissiontobuyerdate_".$i;
			 $cboapprovalstatus="cboapprovalstatus_".$i;//cboapprovalstatus_2
			 $txtapprovalrejectdate="txtapprovalrejectdate_".$i;
			 $txtsamplecomments="txtsamplecomments_".$i;
			 $cbostatus="cbostatus_".$i;
			 $updateid="updateid_".$i;
			 ...
			 if(str_replace("'",'',$$updateid)!="")
			 {
				...
				$appaction_arr[str_replace("'",'',$$updateid)]['status']=str_replace("'",'',$$cboapprovalstatus);
				$appaction_arr[str_replace("'",'',$$updateid)]['tar_date']=str_replace("'",'',$$txttargetapprovaldate);
				$appaction_arr[str_replace("'",'',$$updateid)]['sendto']=str_replace("'",'',$$txtsendtofatorydate);
				$appaction_arr[str_replace("'",'',$$updateid)]['subtobuyer']=str_replace("'",'',$$txtsubmissiontobuyerdate);
				$appaction_arr[str_replace("'",'',$$updateid)]['rejdate']=str_replace("'",'',$$txtapprovalrejectdate);
				$appaction_arr[str_replace("'",'',$$updateid)]['comment']=str_replace("'",'',$$txtsamplecomments);
				$appaction_arr[str_replace("'",'',$$updateid)]['status_id']=str_replace("'",'',$$cbostatus);
		 ...
		 $resubmit_Id=str_replace("'","",$resubmit_id);
		 $resubmit_Id=rtrim($resubmit_Id,",");
		...
		//	$rID=execute_query("update wo_po_sample_approval_info set current_status=0 where id in($resubmit_Id)",1);
			$resubmitid=array_unique(explode(",",$resubmit_Id));
			
			foreach($resubmitid as $resubID)
			{
				$app_status_id=$appaction_arr[$resubID]['status'];
				$tar_date=$appaction_arr[$resubID]['tar_date'];
				$sendto=$appaction_arr[$resubID]['sendto'];
				$subtobuyer=$appaction_arr[$resubID]['subtobuyer'];
				$rejdate=$appaction_arr[$resubID]['rejdate'];
				$comment=$appaction_arr[$resubID]['comment'];
				$status_id=$appaction_arr[$resubID]['status_id'];
			
				//echo "10**".$app_status_id.'='.$resubmit_Id.'='.$resubID;die;
				if($app_status_id)
				{
					$rID=execute_query("update wo_po_sample_approval_info set current_status=1,target_approval_date='".$tar_date."',send_to_factory_date='".$sendto."',submitted_to_buyer='".$subtobuyer."',approval_status='".$app_status_id."',approval_status_date='".$rejdate."',sample_comments='".$comment."',status_active='".$status_id."' where id in($resubID)",1);
				}
				//echo "10**Aupdate wo_po_sample_approval_info set current_status=1,target_approval_date='',send_to_factory_date='',submitted_to_buyer='',approval_status=0,approval_status_date='',sample_comments='' where id in($resubID)";die;
			}
			//echo "10**update wo_po_sample_approval_info set current_status=1,target_approval_date='',send_to_factory_date='',submitted_to_buyer='',approval_status=0,approval_status_date='',sample_comments='' where id in($resubmit_id)";die;
			
			// echo "10**".$sql_priv="id,garments_nature,job_no_mst,po_break_down_id, 	color_number_id,sample_type_id,target_approval_date,send_to_factory_date,submitted_to_buyer,approval_status,approval_status_date,sample_comments,is_deleted,status_active,inserted_by,insert_date from wo_po_sample_approval_info where id in($resubmit_id)";die;
			 /*$result_priv=sql_select($sql_priv);
			 foreach($result_priv as $row)
			 {
				 $data_arr[$row[csf("id")]]=$row[csf("id")];
			 }*/
        ...
        <table width="870" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
            <thead>
                ...
                <tr>
                    ...
                    <th colspan="2" width="130">Date Range</th>
            ...
            <tr class="general">
                <td> <input type="hidden" id="selected_job">
                    <? echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 $company_cond order by company_name","id,company_name",1, "-- Select Company --", '', "load_drop_down( 'sample_approval_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );"); ?>
                </td>
...
if($action=="create_po_search_list_view")
{
	...
	else
	{
		...
		echo  create_list_view("list_view", "Job No,Year,Company,Buyer Name,Style Ref. No", "90,120,100,100,90","850","320",0, $sql , "js_set_value", "job_no", "", 1, "0,0,company_name,buyer_name,0", $arr , "job_no_prefix_num,year,company_name,buyer_name,style_ref_no", "",'','0,0,0,0,0');
	}
	exit();
}
...
if ($action=="populate_data_from_search_popup")
{
	...
	$sample_array=array();
	$sample_tag=sql_select("select a.id,a.sample_name,b.tag_sample,b.sequ from lib_sample a, lib_buyer_tag_sample b where a.id=b.tag_sample and b.sequ!=0 and b.buyer_id='".$buyer_id."' and a.business_nature=3 order by b.sequ");
	//echo "select a.id,a.sample_name,b.tag_sample,b.sequ from lib_sample a, lib_buyer_tag_sample b where a.id=b.tag_sample and b.sequ!=0 and b.buyer_id='".$buyer_id."' and a.business_nature=3 order by b.sequ";
