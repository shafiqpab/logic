...
if($action=="save_update_delete")
{
	...
	if($operation==0)  // Insert Here
	{
		...
		if($color_id=='ec')
		{
			//$color_new_id=return_id( $color_id_ec_1, $color_arr, "lib_color", "id,color_name");
			...
			// color_id_ec_1=Always will be checked with the first color name for Extra color
		}
            ...
            <tbody>
				<tr class="general">
					<td><input type="hidden" id="selected_job">
                        <? echo create_drop_down( "cbo_company_name", 160, "select comp.id,comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'lapdip_approval_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" ); ?></td>
                    ...
                    <td><input name="txt_order_search" id="txt_order_search" class="text_boxes" style="width:110px"></td>
                    <td>
...
if($action=="create_po_search_list_view")
{
	...
	//echo $sql;
		...
		echo create_list_view("list_view", "Job No,Year,Company,Buyer Name,Style Ref. No,Job Qty.,PO number,PO Quantity,Shipment Date", "60,60,120,110,110,90,110,90,80","950","240",0, $sql , "js_set_value", "job_no", "", 1, "0,0,company_name,buyer_name,0,0,0,0", $arr , "job_no_prefix_num,year,company_name,buyer_name,style_ref_no,job_quantity,po_number,po_quantity,shipment_date", "",'','0,0,0,0,0,1,0,1,3');
	}
	else
	{
		$sql= "select a.job_no_prefix_num,$insert_year, a.job_no,a.company_name,a.buyer_name,a.style_ref_no from wo_po_details_master a where a.status_active=1 and a.garments_nature=$data[5] and a.is_deleted=0 $company $buyer $job_cond order by a.job_no";
...
if($action=="lapdip_approval_list_view_edit")
{
	...
	foreach($dataArray as $row)
	{
		...
		if($print_cond_footer==1)
		{
					...
					$sql2="select po_break_down_id from wo_po_color_size_breakdown where job_no_mst='$job_no' and color_number_id=$prev_color_id and po_break_down_id not in($po_id) and status_active=1 and is_deleted=0 group by po_break_down_id";
					$dataArray2=sql_select($sql2);
					foreach($dataArray2 as $row2)*/
		...
		{
		?>
            <h3 align="left" id="accordion_h<? echo $color_id; ?>" style="width:1075px" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel_<? echo $color_id; ?>', 'fnc_color_id(<? echo $color_id; ?>,1,0)',1)"> +<? echo $color_arr[$color_id]; ?></h3>
	...
	{
					/*$po_id=implode(",",array_unique(explode(",",substr($po_id,0,-1))));
					$sql3="select po_break_down_id from wo_po_color_size_breakdown where job_no_mst='$job_no' and color_number_id=$color_id and po_break_down_id not in($po_id) and status_active=1 and is_deleted=0 group by po_break_down_id";
					$dataArray3=sql_select($sql3);
					foreach($dataArray3 as $row3)*/
					$po_id_arr=array_unique(explode(",",substr($po_id,0,-1)));
	...
	if($type==1)
	{
		...
		{
		?>
			<h3 align="left" id="accordion_h<? echo $color_id; ?>" style="width:1075px" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel_<? echo $color_id; ?>', 'fnc_color_id(\'<? echo $color_id; ?>\',0,1)',1)"> +<? echo "Extra Color"; ?></h3>
			<div id="content_search_panel_<? echo $color_id; ?>" style="display:none" class="accord_close">
				<table class="rpt_table" border="1" width="1075" cellpadding="0" cellspacing="0" rules="all" id="table_<? echo $color_id; ?>">
					<thead>
						<th>Po Number</th>
					...
					<?
                    $i=1;
					/*$sql="select po_break_down_id from wo_po_color_size_breakdown where job_no_mst='$job_no' and status_active=1 and is_deleted=0 group by po_break_down_id";
					$dataArray=sql_select($sql);
					foreach($dataArray as $row)*/
		...
		{
		?>
			<h3 align="left" id="accordion_h<? echo $color_id; ?>" style="width:1075px" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel_<? echo $color_id; ?>', 'fnc_color_id(<? echo $color_id; ?>,0,0)',1)"> +<? echo $color_arr[$color_id]; ?></h3>
			<div id="content_search_panel_<? echo $color_id; ?>" style="display:none" class="accord_close">
				<table class="rpt_table" border="1" width="1075" cellpadding="0" cellspacing="0" rules="all" id="table_<? echo $color_id; ?>">
					<thead>
						<th>Po Number</th>
