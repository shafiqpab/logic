...
{
	$nameArray=sql_select( "select id,style_ref_name,set_break_down,buyer_id,order_uom,gmts_item_id,department_id, status_active,style_no,buyer_brand_id,level_type_id,design_type,product_department_id,division,short_name from  lib_style_ref where id='$data'" );
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		else
		{
			...
			$field_array="id,style_number_prefix,style_number_prefix_num,style_no,style_ref_name,short_name,buyer_id,order_uom,gmts_item_id,department_id,buyer_brand_id,level_type_id,design_type,product_department_id,division,set_break_down,total_set_qnty,set_smv,inserted_by,insert_date,status_active,is_deleted";
			$data_array="(".$id. ",'" . $style_prefix . "'," . $new_wo_number[2] . ",'" . $new_mrr_fin. "','" .$txt_style_name. "','" .$txt_style_short_name."',".$cbo_buyer_name.",".$cbo_order_uom.",'".str_replace("'", "", $cbo_gmt_item)."',".$cbo_department_id.",'".$cbo_buyer_brand_id."','".$cbo_level_type_id."','".$cbo_design_type."','".$cbo_product_department."','".$txt_division."','".str_replace("'", "", $set_breck_down)."','".str_replace("'", "", $tot_set_qnty)."','".str_replace("'", "", $txt_sew_smv)."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_status.",0)";
	...
	{
		if (is_duplicate_field( "style_ref_name", "lib_style_ref", "style_ref_name=$txt_style_name 
		                                                   and id!=$update_id and is_deleted=0 
														   and department_id=$cbo_department_id" ) == 1)
		{
			echo "11**0"; disconnect($con);die;
		}
		else
		{
			...
			$field_array="style_ref_name*short_name*buyer_id*order_uom*gmts_item_id*department_id*buyer_brand_id*level_type_id*design_type*product_department_id*division*set_break_down*total_set_qnty*set_smv*updated_by*update_date*status_active*is_deleted";
