            ...
            <tfoot>
            	<tr bgcolor="#CCCCCC">
            		...
            		<td>
                    	...
                    	<input type="hidden" id="budgetapproved" value="<?=$budget_approved; ?>">
                    </td>
            		<td>&nbsp;</td>
...
if($action=='save_update_delete_dtls'){
	...
	if ($operation==1)
	{
		...
		if ($reporttype==1)//Show
		{
			...
			if($podata_array_up!="" && $approved_id != 1){
			...
			if($flag==1 && $approved_id != 1){
				$job_field_array_up="job_quantity*total_price*avg_unit_price*updated_by*update_date";
				$total_amount=str_replace("'","",$total_amount);
				$total_qty=str_replace("'","",$total_qty);
				$avg_rate=$total_amount/$total_qty;
				$avg_rate=number_format($avg_rate,2);
				$job_data_array_up="".$total_qty."*".$total_amount."*".$avg_rate."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
				//echo "10**".$job_data_array_up; die;
				$rID3=sql_update("wo_po_details_master",$job_field_array_up,$job_data_array_up,"id","".$hidden_job_id."",1);
				if($rID3==1 && $flag==1) $flag=1; else $flag=0;	
			}
