...
<script>
<?
	// if(isset($_SESSION['logic_erp']['data_arr'][201]))
	// {
	// 	$data_arr= json_encode($_SESSION['logic_erp']['data_arr'][201]);
	// 	echo "var field_level_data= ". $data_arr . ";\n";
	// }
	if(isset($_SESSION['logic_erp']['data_arr'][403]))
	{
		$data_arr= json_encode($_SESSION['logic_erp']['data_arr'][403]);
		echo "var field_level_data= ". $data_arr . ";\n";
	}
?>
...
function openmypage_booking(page_link,title)
{
	...
	page_link+="&cbo_company_name="+cbo_company_name+"&cbo_buyer_name="+cbo_buyer_name;
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1160px,height=450px,center=1,resize=1,scrolling=0','../')
	emailwindow.onclose=function()
	{
...
function openmypage_emb_item(page_link,title){
	...
	var cbo_level=document.getElementById('cbo_level').value;
	page_link=page_link+'&company_id='+cbo_company_name+'&garments_nature='+garments_nature+'&cbo_currency='+cbo_currency+'&cbo_supplier_name='+cbo_supplier_name+'&cbo_buyer_name='+cbo_buyer_name+'&txt_booking_no='+txt_booking_no+'&cbo_level='+cbo_level;
...
function fnc_trims_booking_dtls( operation ){
	...
	if(operation==1 || operation==2)
	{
		...
		if(cbo_pay_mode==2)
		{
			console.log(cbo_pay_mode);
			var pir_exist=trim(return_global_ajax_value(document.getElementById("txt_booking_no").value, 'check_pi_exist_or_not', '', 'requires/print_booking_multijob_controller'));
			console.log(pir_exist);
			if(pir_exist)
			{
				alert('PI : '+pir_exist+ ' Exist . delete not allowed');
				return;
			}
		}
	}
...
function fnc_trims_booking_dtls_reponse(){
	if(http.readyState == 4){
		...
		if(trim(reponse[0])=='lockAnotherProcess'){
			alert("This Booking is Attached in Wash Order Entry. Ref :"+trim(reponse[1])+" \n So, Update/Delete Not Allowed.")
		    release_freezing();
		    return;
		}
		if(trim(reponse[0])=='serviceacklockAnotherProcess'){
			alert("This booking is Attached In Service Acknowledgement. Ref :"+trim(reponse[1])+" \n So Delete Not Allowed.")
		    release_freezing();
		    return;
		}
		...
		if(trim(reponse[0])=='samesen'){
			alert("This PO already booking in another sensitivity so you can not use differante.")
			//$('#search'+reponse[2]).css('background-color', 'red');
		    release_freezing();
		    return;
		}
...
function fnc_show_booking_list(){ 
	...
	var param="'"+param+"'"
	var data="'"+data+"'"
	var precost_id="'"+precost_id+"'"//update_id
	var cbo_level=document.getElementById('cbo_level').value;
	var data="action=show_trim_booking_list"+get_submitted_data_string('txt_booking_no',"../../")+'&cbo_company_name='+cbo_company_name+'&garments_nature='+garments_nature+'&data='+data+'&param='+param+'&pre_cost_id='+precost_id+'&cbo_level='+cbo_level+'&cbo_currency='+cbo_currency+'&update_id='+update_id;
	http.open("POST","requires/print_booking_multijob_controller.php",true);
	http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	http.send(data);
...
function fnc_fabric_booking( operation )
{
	...
	if (form_validation('cbo_company_name*cbo_buyer_name*txt_booking_date*txt_delivery_date*cbo_pay_mode*cbo_supplier_name','Company*Buyer*Booking Date*Delivery Date*Pay Mode*Supplier Name')==false)
	{
		release_freezing();
		return;
	}
...
function auto_completesupplier()
{
	if( form_validation('cbo_company_name','Company Name')==false )
	{
		return;
	...
	$("#cbo_supplier_name").autocomplete({
		...
		select: function (e, ui) {
			...
			load_drop_down( 'requires/print_booking_multijob_controller', ui.item.id, 'load_drop_down_party_location', 'location_td' );
			fnc_floorload();
		}
	});
	...
	//var supplier_id=document.getElementById('hidden_supplier_name').value;
...
function generate_trim_report(action,report_type,mail_id,is_mail_send){
	...
	var data="action="+action+'&is_mail_send='+is_mail_send+'&mail_id='+mail_id+get_submitted_data_string('txt_booking_no*cbo_company_name*id_approved_id*cbo_template_id',"../../")+'&report_title='+$report_title+'&report_type='+report_type;
	http.open("POST","requires/print_booking_multijob_controller.php",true);
	http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	http.send(data);
...
function fnc_floorload()
{
	var pay_mode=$("#cbo_pay_mode").val();
	var supplier_id=$("#hidden_supplier_id").val()*1;
	var party_location=$("#cbo_party_location").val();
	
	load_drop_down( 'requires/print_booking_multijob_controller', pay_mode+'__'+supplier_id+'__'+party_location, 'load_drop_down_floor', 'floor_td');
}


function call_print_button_for_mail(mail_id){
		get_php_form_data( document.getElementById('cbo_company_name').value+'**'+mail_id, "get_first_selected_print_report", "requires/print_booking_multijob_controller" );
	}
            ...
            <fieldset style="width:1000px;">
                ...
                <table  width="1000" cellspacing="2" cellpadding="0" border="0">
                    <tr>
                        ...
                        <td colspan="4">
                            <input class="text_boxes" type="text" style="width:130px" onDblClick="openmypage_booking('requires/print_booking_multijob_controller.php?action=fabric_booking_popup','Print Booking Search');" readonly placeholder="Double Click for Booking" name="txt_booking_no" id="txt_booking_no"/>
                            <input type="hidden" id="id_approved_id">
                            <input type="hidden" style="width:50px;" class="text_boxes" name="report_ids" id="report_ids" />
                        </td>
                    </tr>
                    <tr>
                        <td width="110"  class="must_entry_caption">Company Name</td>
                        <td width="140"><?=create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company where status_active=1 and is_deleted=0 order by company_name", "id,company_name",1, "-- Select Company --", $selected, "load_drop_down( 'requires/print_booking_multijob_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );get_php_form_data( this.value, 'load_button', 'requires/print_booking_multijob_controller' );",'',"" ); ?></td>
                        <td width="110" class="must_entry_caption">Buyer Name</td>
                        <td width="140" id="buyer_td"><?=create_drop_down( "cbo_buyer_name", 130, "select id,buyer_name from lib_buyer where status_active =1 and is_deleted=0 order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "",'',"" ); ?></td>
                        <td width="110" class="must_entry_caption">WO Date</td>
                        <td width="140"><input class="datepicker" type="text" style="width:120px" name="txt_booking_date" id="txt_booking_date" value="<?=date("d-m-Y");?>"  disabled/></td>
                    ...
                    <tr>
                        ...
                        <td><?=create_drop_down( "cbo_pay_mode", 130, $pay_mode,"", 1, "-- Select Pay Mode --", "", "supplier_empty_check(); fnc_floorload()","" ); ?></td>
                        <td class="must_entry_caption">Party Name
                    ...
                    <tr>
                        ...
                        <td id="location_td"><?=create_drop_down("cbo_party_location", 130, "select id, location_name from lib_location", "id,location_name", 1, "--Select--", 0, "fnc_floorload();", 0); ?></td>
                        <td>Floor/Unit</td>
                        <td id="floor_td"><?=create_drop_down("cbo_floor_id", 130, $blank_array, "", 1, "--Select--", 0, "", 0); ?></td>
                        <td>Level</td>
                        <td>
                        <?
                    ...
                    <tr>
                        ...
                        <td>Remarks</td>
                        <td><input class="text_boxes" type="text" style="width:120px;"  name="remarks" id="remarks" /></td>
                        <td>&nbsp;</td>
                        <td>
							<?
                    ...
                    <tr>
                        <td align="center"  valign="middle" colspan="8">
                            ...
                            <input type="button" value="Print Booking" onClick="generate_trim_report('show_trim_booking_report2',1)"  style="width:100px;display:none" name="print_booking" id="print_booking" class="formbutton" />
							<input type="button" value="Print Booking 1" onClick="generate_trim_report('show_trim_booking_report3',1)"  style="width:100px;display:none" name="print_booking1" id="print_booking1" class="formbutton" />
