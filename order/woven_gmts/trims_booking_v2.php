...
function fnc_trims_booking( operation )
{
	...
	{
		 data_all=data_all+get_submitted_data_string('txt_booking_no*cbo_booking_month*cbo_booking_year*txt_job_no*cbo_company_name*cbo_buyer_name*cbo_supplier_name*cbo_currency*txt_exchange_rate*txt_booking_date*txt_delivery_date*cbo_pay_mode*cbo_source*txt_attention*txt_remarks*cbo_ready_to_approved*cbo_item_from_precost',"../../");
	}
	var row_num=$('#tbl_list_search tr').length;
	if(row_num <1)
	{
                  ...
                  <!-- <ul style="position:absolute;width:55px;height:20px;border:0;padding:0;margin:0;">
                    ...
                    <tr>
                            ...
                            <td  width="130"  align="right" class="must_entry_caption">Job No</td>              
                              ...
                              <td>
                              <? 
							  	echo create_drop_down( "cbo_company_name", 172, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected, "load_drop_down( 'requires/trims_booking_controller_v2', this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/trims_booking_controller_v2', this.value, 'load_drop_down_buyer', 'buyer_td' );get_php_form_data( this.value, 'populate_variable_setting_data', 'requires/trims_booking_controller_v2' );","","" );
								?>	  
                              </td>
                        ...
                        <td align="right" class="must_entry_caption">Buyer Name</td>
                            ...
                            <td  align="right">Pay Mode</td>
                                <td>
                               <?
							   		echo create_drop_down( "cbo_pay_mode", 172, $pay_mode,"", 1, "-- Select Pay Mode --", "", "load_drop_down( 'requires/trims_booking_controller_v2', this.value, 'load_drop_down_supplier', 'supplier_td' )","" );
							   ?> 
                         </td>
