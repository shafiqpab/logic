...
{
	if('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][676]);?>')
	{
		if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][676]);?>','<? echo implode('*',$_SESSION['logic_erp']['mandatory_message'][676]);?>')==false)
		{ 
			release_freezing();
			return;
		}
...
function fnc_style_ref_info_reponse()
{
	...
	function call_print_button_for_mail(mail,mail_body,type){
		if (form_validation('txt_style_id','Style Id')==false)
		{
			return;
		}

		var dataStr = document.getElementById('update_id').value+'**'+mail+'**'+mail_body;
		var is_send = return_global_ajax_value(dataStr, 'send_auto_mail', '', 'requires/style_ref_controller');
		alert(is_send);
	}
	...
	<fieldset style="width:600px;">
		...
		<form name="styleRef_1" id="styleRef_1">
			<table cellpadding="0" cellspacing="3" width="100%">
				<tr>
					<td valign="top" colspan="4" style="justify-content: center;text-align: center;">Style ID
				...
				<tr>
					...
					<td id="brand_td">
						<?php echo create_drop_down( "cbo_buyer_brand_id", 130, "select id, brand_name from lib_buyer_brand where  status_active=1 and is_deleted=0 order by id","id,brand_name", "1", "Select Buyer Brand", 0, "" );?>
					</td>
				...
				<tr>
					...
					<td id="department_td">
						<?php echo create_drop_down( "cbo_department_id", 130, $blank_array,"", "1", "Select Department", 0, "" );?>
					</td>
				...
				<tr>
					...
					<td><? echo create_drop_down( "cbo_order_uom",100, $unit_of_measurement, "",0, "", 1, "","","1,58" ); ?></td>
                ...
                <tr>
					...
					<td width="100">Offer Qnty</td>
					<td>
						<input type="text" name="txt_offer_qnty" id="txt_offer_qnty" class="text_boxes_numeric" style="width:120px" />
					</td>
				</tr>
				<tr>
					<td width="100">Garments Item</td>
                    <td>
						<input type="button" id="set_button" class="image_uploader" style="width:130px;" value="Style Type/Item" onClick="open_set_popup(document.getElementById('cbo_order_uom').value);" />
