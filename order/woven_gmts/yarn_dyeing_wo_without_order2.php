...
function fnc_yarn_dyeing(operation)
{
	...
	if( form_validation('cbo_company_name*cbo_supplier_name*txt_booking_date*cbo_pay_mode*cbo_sample_name*cbo_count*txt_yern_color*cbo_composition*txt_pacent*cbo_yarn_type*txt_wo_qty','Company Name *Supplier Name*Booking Date*Job Number*Source Name*Color*Pay Mode*Sample Name*Count*Color*Composition*Composition Percent*Yarn Type*Order Quanty')==false )
	{
		return;
	}
	else
...
function check_exchange_rate()
	{
		...
		var response=return_global_ajax_value( cbo_currercy+"**"+booking_date, 'check_conversion_rate', '', 'requires/yarn_dyeing_wo_without_order_controller2');
    ...
    <legend>Yarn Dyeing Wo</legend>
        <table  width="1250" cellspacing="2" cellpadding="0" border="0">
                ...
                <tr>
                    ...
                    echo create_drop_down( "cbo_company_name", 140, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected,"chack_variable(this.value);load_drop_down( 'requires/yarn_dyeing_wo_without_order_controller2',this.value, 'load_drop_down_supplier', 'supplier_td' );print_button_setting(this.value);",0);
                    ?></td>
                ...
                <tr>
                    ...
                    <input type="button" id="set_button" class="image_uploader" style="width:160px;" value="Terms & Condition/Notes" onClick="open_terms_condition_popup( 'requires/yarn_dyeing_wo_without_order_controller2.php?action=terms_condition_popup','Terms Condition'
