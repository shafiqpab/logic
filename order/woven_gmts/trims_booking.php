...
function openmypage(page_link,title){
	...
	emailwindow.onclose=function(){
		...
		if (theemail.value!=""){
			...
			load_drop_down( 'requires/trims_booking_controller', theemail.value+'_'+document.getElementById('cbo_supplier_name').value, 'load_drop_down_trim_precost_id', 'trim_group_td' )
			}
...
function fnc_trims_booking( operation ){
	...
	if (form_validation('cbo_company_name*cbo_buyer_name*txt_booking_date*cbo_pay_mode*cbo_trim_type','Company Name*Buyer Name*Booking Date*Pay Mode*Trim Type')==false){
		return;
		 ...
		 data_all=data_all+get_submitted_data_string('txt_booking_no*txt_job_no*cbo_company_name*cbo_buyer_name*cbo_supplier_name*cbo_currency*txt_exchange_rate*txt_booking_date*txt_delivery_date*cbo_pay_mode*cbo_source*txt_attention*cbo_isshort*txt_remarks*cbo_ready_to_approved*cbo_trim_type*cbo_item_from_precost',"../../");
	}
	...
	if(http.readyState == 4){
		...
		if(trim(reponse[0])=='pi1'){
		 ...
		 if(trim(reponse[0])==2){
			  var d = new Date();
			  var date=d.yyyymmdd();
			  reset_form('trimsbooking_1','booking_list_view','id_approved_id','cbo_currency,2*cbo_pay_mode,1*cbo_source,3*cbo_isshort,2','disable_enable_fields("cbo_company_name*cbo_buyer_name",0,"","")','copy_val');
...
function fnc_trims_booking_dtls( operation ){
	...
	var booking_qnty_and_amount_trim_group=return_global_ajax_value(txt_po_id+"_"+txt_trim_group_id+"_"+cbo_trim_precost_id, 'booking_qnty_and_amount_trim_group', '', 'requires/trims_booking_controller');
     ...
     <? echo load_freeze_divs ("../../",$permission);  ?>
                ...
                <legend title="V3">Trims Booking &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;   <font id="app_sms" style="color:#F00"></font></legend>
                    ...
                    <tr>
                        ...
                        echo create_drop_down( "cbo_company_name", 172, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected, "get_php_form_data( this.value, 'populate_variable_setting_data', 'requires/trims_booking_controller' );","","" );
                        ?>	
                        ...
                        <td>
                        <?
                        echo create_drop_down( "cbo_pay_mode", 172, $pay_mode,"", 1, "-- Select Pay Mode --", $selected, "load_drop_down( 'requires/trims_booking_controller', this.value, 'load_drop_down_supplier', 'supplier_td' )","" );
                        ?> 
                        </td>
