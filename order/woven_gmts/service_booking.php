...
<script>
	...
	function generate_trim_report(action,mail_send_data)
	{
		...
		if(action=='show_trim_booking_bpkw_report')
		{
			var show_item='';
		...
		var data="action="+action+get_submitted_data_string('txt_booking_no*cbo_company_name*hidden_supplier_id*booking_mst_id',"../../")+'&show_rate='+show_rate+'&show_buyer='+show_buyer+'&show_comment='+show_comment+'&mail_send_data='+mail_send_data;
		//alert(data);
		http.open("POST","requires/service_booking_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
