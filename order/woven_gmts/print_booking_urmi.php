...
function fnc_generate_booking()
{
	...
	else
	{
		...
		var data="action=generate_print_booking"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_job_no*txt_order_no_id*cbo_booking_natu*cbo_gmt_item*txt_booking_no*txt_booking_date*txt_delivery_date*calculation_basis*cbo_level*cbo_is_short',"../../");
...
{
	get_php_form_data('', 'change_emb_name', 'requires/print_booking_urmi_controller')	
}
...
function fnc_fabric_booking( operation )
{
	...
	if (form_validation('txt_job_no*txt_booking_date*txt_delivery_date*cbo_pay_mode','Job No*Booking Date*Delivery Date*Pay Mode')==false)
	{
		return;
	}	
	else
	{
		var data="action=save_update_delete&operation="+operation+"&delete_cause="+delete_cause+get_submitted_data_string('txt_booking_no*id_approved_id*txt_job_no*cbo_company_name*cbo_buyer_name*txt_booking_date*txt_delivery_date*cbo_currency*txt_exchange_rate*cbo_supplier_name*hidden_supplier_id*cbo_pay_mode*cbo_source*cbo_ready_to_approved*txt_attention*calculation_basis*cbo_level*cbo_is_short*update_id',"../../");
...
function fnc_fabric_booking_dtls( operation ){
	...
	data_all=data_all+get_submitted_data_string('txtbookingdtlasid_'+i+'*txtpoid_'+i+'*txtitemnumberid_'+i+'*txtembcostid_'+i+'*txtcolorid_'+i+'*txtwoq_'+i+'*txtrate_'+i+'*txtamount_'+i+'*txtddate_'+i+'*description_'+i+'*txtreqqty_'+i+'*txtuomid_'+i,"../../",i);
	}
...
function calculate_amount(i)
{
	...
	{
		alert("Rate Exceeds Of Pre-Cost Rate");
...
{ 
	...
	else
	{
		...
		var data="action="+action+get_submitted_data_string('txt_booking_no*txt_job_no*cbo_company_name*cbo_buyer_name*txt_booking_date*txt_delivery_date*cbo_currency*cbo_supplier_name*hidden_supplier_id*cbo_pay_mode*txt_exchange_rate*cbo_source*cbo_booking_natu*calculation_basis*cbo_is_short*cbo_template_id*txt_season',"../../")+'&show_comment='+show_comment;
		http.open("POST","requires/print_booking_urmi_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
...
/*function check_exchange_rate()
	{
		...
		var response=return_global_ajax_value( cbo_currercy+"**"+booking_date, 'check_conversion_rate', '', 'requires/print_booking_urmi_controller');
...
<form name="printbooking_1"  autocomplete="off" id="printbooking_1">
    ...
    <legend>Master</legend>
        <table  width="1000" cellspacing="2" cellpadding="0" border="0">
            <tr>
                <td colspan="4" align="right" class="must_entry_caption">Work order No</td>              
                <td colspan="4">
                    <input class="text_boxes" type="text" style="width:160px" onDblClick="openmypage_booking('requires/print_booking_urmi_controller.php?action=fabric_booking_popup','Print Booking Search')" readonly placeholder="Double Click for Booking" name="txt_booking_no" id="txt_booking_no"/>
            ...
            <tr>
                ...
                <td width="140"><?=create_drop_down( "cbo_company_name",130, "select id,company_name from lib_company where status_active=1 and is_deleted=0 order by company_name", "id,company_name",1, "-- Select Company --", $selected, "load_drop_down( 'requires/print_booking_urmi_controller', this.value, 'load_drop_down_buyer', 'buyer_td' )",1,"" ); ?></td>
                ...
                <td><input class="datepicker" type="text" style="width:120px" name="txt_booking_date" id="txt_booking_date" value="<? echo date("d-m-Y");?>"  disabled/></td>
            </tr>
            <tr>
            ...
            <tr>
               ...
               <td>Supplier Name <input type="hidden" id="hidden_supplier_id" name="hidden_supplier_id" style="width:60px;" class="text_boxes" ></td>
               <td><input type="text" name="cbo_supplier_name" id="cbo_supplier_name" class="text_boxes supplier_name" onFocus="auto_completesupplier();" style="width:120px;" placeholder="Write" />
                <?
             	// echo create_drop_down( "cbo_supplier_name", 172, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=23 and   a.status_active =1 and a.is_deleted=0 order by supplier_name","id,supplier_name", 1, "-- Select Supplier --", $selected, "get_php_form_data( this.value, 'load_drop_down_attention', 'requires/print_booking_urmi_controller');",0 );
                ?> 
                </td>
                <td>Is Short</td>
                <td><? echo create_drop_down( "cbo_is_short", 130, $yes_no,'', 0, '',2,"");?></td> 
            </tr>
            <tr>
                <td>Attention</td>   
        ...
        <legend>Details</legend>
                <table style="border:none" cellpadding="0" cellspacing="2" border="0">
                    <tr>
                     ...
                     <tr align="center">
                        <td colspan="6"><b>Copy</b> :<input type="checkbox" id="copy_val" name="copy_val" checked/> </td>	
                    </tr>
                    <tr align="center">
