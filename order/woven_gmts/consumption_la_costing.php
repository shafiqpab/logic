<?
/*--- ----------------------------------------- Comments
Purpose			: 	This form will create Woven Garments Consumption Entry [CAD] For LA Costing
Functionality	:
JS Functions	:
...
echo load_html_head_contents("Cons. la Costing-WVN","../../", 1, 1, $unicode,1,'');
...
<script type="text/javascript">
    ...
    { 
        var cbo_style_source=$('#cbo_style_source').val();
        ...
        emailwindow.onclose=function(){
            ...
            if (system_id.value!=""){
                system_id_data_arr = system_id.split("_");
                if(system_id_data_arr[2]==2){
                    get_php_form_data(system_id_data_arr[1], "populate_data_from_req_consumption", "requires/consumption_la_costing_controller");
                }else if(system_id_data_arr[2]==1){
                    get_php_form_data(system_id_data_arr[0], "populate_data_from_consumption", "requires/consumption_la_costing_controller");
                } else if(system_id_data_arr[2]==3){
                    get_php_form_data(system_id_data_arr[1], "populate_data_from_job_consumption", "requires/consumption_la_costing_controller");
                }
	...
	function fnc_style_browse()
	{
        var cbo_style_source=$('#cbo_style_source').val();
        var title = "Master Style Ref.";
        var page_link='requires/consumption_la_costing_controller.php?action=generate_buyer_inquery'+'&cbo_style_source='+cbo_style_source;
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1100,height=480px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function(){
			var theform=this.contentDoc.forms[0];
			...
			if (inquery_data.value!=""){
                inquery_data_arr = inquery_data.split("_");
                 if(cbo_style_source==2){
                     get_php_form_data(inquery_data_arr[0], "populate_req_data_from_data", "requires/consumption_la_costing_controller");
                 }else if(cbo_style_source==3){
                     get_php_form_data(inquery_data_arr[0], "populate_job_data_from_data", "requires/consumption_la_costing_controller");
                 }else{
    ...
    function fnc_show_fabrication_list(){
        var data="action=show_fabrication_list"+get_submitted_data_string('txt_style_ref*inquery_id*txt_fabrication*update_id*cbo_style_source',"../../");
    ...
    function fnc_consumption_entry(operation)
    {
        ...
        if (form_validation('txt_consumption_date*txt_style_ref*inquery_id*cbo_cons_for','Consumption Date*Master Style Ref*Inquiry Data*Cons For')==false){
            ...
            data_all=data_all+get_submitted_data_string('txt_consumption_date*inquery_id*txt_fabrication*txt_merch_style*txt_style_desc*txt_pattern_master*txt_bom_no*txt_comments*cbo_company_name*update_id*txt_mclastmod_date*cbo_cons_for*cbo_style_source',"../../");
    ...
    function fnc_consumption_entry_reponse()
    {
        if(http.readyState == 4){
            ...
            if(reponse[0]==2)
            {
                ...
                $('#txt_style_ref').attr('ondblclick','fnc_style_browse()');
	...
	function fnc_fabric_decription_popup(i)
	{
		var libyarncountdeterminationid =document.getElementById('yarncountid_'+i).value
		var page_link='requires/consumption_la_costing_controller.php?action=fabric_description_popup&libyarncountdeterminationid='+libyarncountdeterminationid;
		...
		emailwindow.onclose=function()
		{
			...
			var fab_data=this.contentDoc.getElementById("fab_data").value;
			var hiddrdno=this.contentDoc.getElementById("hiddrdno");
			var hiddfabref=this.contentDoc.getElementById("hiddfabref");
			var exfabdata=fab_data.split('_');
			//var composition=this.contentDoc.getElementById("composition");
			//var weight_type=this.contentDoc.getElementById("weight_type");
			document.getElementById('yarncountid_'+i).value=fab_des_id.value;
			document.getElementById('fabricdes_'+i).value=fab_desctiption.value;
			document.getElementById('fabricdes_'+i).title=fab_desctiption_title.value;
			document.getElementById('txtfullwidth_'+i).value=exfabdata[0];
			document.getElementById('txtrdno_'+i).value=hiddrdno.value;
			document.getElementById('fabricref_'+i).value=hiddfabref.value;
			document.getElementById('txtcuttablewidth_'+i).value=exfabdata[1];
			document.getElementById('txtshrinkagel_'+i).value=exfabdata[2];
			document.getElementById('txtshrinkagew_'+i).value=exfabdata[3];
			document.getElementById('txt_fabrication').value=fab_des_id.value;
    ...
    function fnc_load_fabric_dtls(up_id){
        ...
        if(list_view_tr=="" || list_view_tr==0)
        {
            ...
            reset_form('consumptiondtlsform_1','','','','','fabric_dtls_id');
...
<body onLoad="set_hotkey();">
    <div style="width:100%;" align="center">
        ...
        <fieldset style="width:1050px;">
            ...
            <form name="consumption_form" id="consumption_form" autocomplete="off">
                <table width="1050" cellspacing="2" cellpadding="0" border="0">
                    ...
                    <tr>
                        ...
                        <td width="150"><? echo create_drop_down( "cbo_company_name", 120, "select id, company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "Select Company", $selected, "",1); ?></td> 
                        <td width="100" class="must_entry_caption">Style Source</td>
                        <td width="150"><? echo create_drop_down( "cbo_style_source", 120, $search_by_arr, "", 1, "-- Select --", 1, "", "", "" );?></td>                        
                        <td width="100" class="must_entry_caption">M.Style Ref</td>
                        <td width="150">
                            <input class="text_boxes" type="text" style="width:110px" placeholder="Browse"  name="txt_style_ref" id="txt_style_ref" onDblClick="fnc_style_browse();"/>
                            <input type="hidden" name="inquery_id" id="inquery_id" value="">
                            <input type="hidden" name="txt_fabrication" id="txt_fabrication" value="">
                        </td> 
                    	<td width="100" class="must_entry_caption">CAD Date</td>
                        <td><input name="txt_consumption_date" style="width:110px"  id="txt_consumption_date" placeholder="Select Date" class="datepicker" type="text" value="<? echo date("d-m-Y")?>" /></td> 
                    ...
                    <tr>
                        ...
                        <td>Comments</td>
                        <td>
                            <input class="text_boxes" style="width:110px" type="text" placeholder="Write" name="txt_comments" id="txt_comments"/>
            ...
            <form name="consumptiondtlsform_1" id="consumptiondtlsform_1" autocomplete="off">
                <legend>Fabric Details</legend>
                <table cellspacing="0" cellpadding="0" border="1" width="960" class="rpt_table" id="fabric_info_dtls_tbl" rules="all">
                    <thead>
                        <tr>
                            ...
                            <th width="50" rowspan="2" style="color: blue;">Bundles Cons Yds</th>
                            <th width="50" rowspan="2" style="color: blue;">Cons Yds / Dzn</th>
                            <th width="50" rowspan="2">Wastage %</th>
                            <th width="50" rowspan="2">Final Cons [Yds]</th>
                ...
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="960" class="rpt_table">       
                    <tr>
                        <td align="center" valign="middle" style="max-height:960px; min-height:15px;" id="fabric_dtls_save_update">
                            <? echo load_submit_buttons( $permission, "fnc_consumption_fabric_entry", 0,0 ,"reset_form('consumptiondtlsform_1','','')",2); ?>
