...
function openmypage_booking(page_link,title)
{
	...
	emailwindow.onclose=function()
	{
		...
		if (theemail.value!="")
		{
		    ...
		    show_list_view(document.getElementById('txt_booking_no').value, 'fabric_detls_list_view','data_panel','requires/service_booking_aop_urmi_controller','');
...
function fnc_trims_booking( operation ){
	...
	{
	data_all=data_all+get_submitted_data_string('txt_booking_no*cbo_booking_month*cbo_booking_year*cbo_company_name*cbo_supplier_name*cbo_currency*txt_exchange_rate*txt_booking_date*txt_delivery_date*cbo_pay_mode*cbo_source*txt_attention*cbo_buyer_name*cbo_level*cbo_is_short',"../../");
	}
...
function fnc_trims_booking_reponse(){
	if(http.readyState == 4) 
	{
		 ...
		 if(reponse[0]==2)
		 {
			show_msg(trim(reponse[0]));
			set_button_status(0, permission, 'fnc_trims_booking',1);
			reset_form('','data_panel*booking_list_view','txt_booking_no*cbo_company_name*cbo_buyer_name*cbo_currency*txt_exchange_rate*txt_booking_date*txt_delivery_date*cbo_pay_mode*cbo_source*cbo_supplier_name*txt_attention','txt_booking_date,<? echo date("d-m-Y"); ?>'); 
		 }
...
/*	if(operation==2){
	...
	for (var i=1; i<=row_num; i++)
	{
		...
		data_all+=get_submitted_data_string('job_no_'+i+'*po_id_'+i+'*fabric_description_id_'+i+'*dia_'+i+'*gmts_color_id_'+i+'*color_size_table_id_'+i+'*item_color_'+i+'*findia_'+i+'*artworkno_'+i+'*startdate_'+i+'*enddate_'+i+'*txt_blanty_'+i+'*txtreqnty_'+i+'*txt_woqnty_'+i+'*uom_'+i+'*txt_rate_'+i+'*txt_amount_'+i+'*txt_paln_cut_'+i+'*updateid_'+i+'*printing_color_'+i,"../../",i);	
	}
	...
	if(cbo_level==2){
	var data="action=save_update_delete_dtls_job_level&operation="+operation+data_all+'&row_num='+row_num+'&json_data='+json_data+'&delete_cause='+delete_cause;
	}
...
function check_exchange_rate()
	{
		...
		var response=return_global_ajax_value( cbo_currercy+"**"+booking_date, 'check_conversion_rate', '', 'requires/service_booking_aop_urmi_controller');
            ...
            <fieldset style="width:950px;">
                ...
                <table  width="900" cellspacing="2" cellpadding="0" border="0">
                    ...
                    <tr>
                        ...
                        <td>
                        <? 
                        echo create_drop_down( "cbo_company_name", 172, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected, "load_drop_down( 'requires/woven_order_entry_controller', this.value, 'load_drop_down_buyer', 'buyer_td' ); get_php_form_data(this.value, 'company_wise_report_button_setting','requires/service_booking_aop_urmi_controller');","","" );
                        ?>	  
                        </td>
                    ...
                    <tr>
                        ...
                        ?>
                        <?
                        echo create_drop_down( "cbo_level", 172, $level_arr,"", 0, "", "2", "","","" );
                        ?>
                        </td>
                        <td align="right"></td> 
                        ...
                        <td align="center" height="10" >
                        	<!-- <input type="button" id="set_button" class="image_uploader" style="width:160px;" value="Terms & Condition/Notes" onClick="open_terms_condition_popup('requires/service_booking_aop_urmi_controller.php?action=terms_condition_popup','Terms Condition')" /> -->
                    ...
                    <tr>
                        ...
                        <input type="button" value="Print Booking 2" onClick="generate_trim_report('show_trim_booking_report2')"  style="width:120px; display: none;" name="print_booking1" id="print_booking2" class="formbutton" />
                        </td>
                    </tr>
                </table>
            </fieldset>
...
function set_data(po_id,fabric_cost_id,precost_conver_id,booking_id){
		...
		var cbo_is_short=document.getElementById('cbo_is_short').value;
		//alert(cbo_is_short);
		show_list_view(fabric_cost_id+'**'+document.getElementById('txt_order_no_id').value+'**'+document.getElementById('txt_booking_no').value+'**'+document.getElementById('cbo_level').value+"**"+precost_conver_id+"**"+cbo_is_short, 'show_aop_booking','booking_list_view','requires/service_booking_aop_urmi_controller','$(\'#hide_fabric_description\').val(\'\')');
