...
Affected Code            :                   
DB Script                : 
Updated by 		         : 	Monir Hossain	
Update date		         : 	11/10/2016	   
QC Performed BY	         :		
QC Date			         :	
...
function openmypage_booking(page_link,title)
{
	...
	//alert(company);
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link+'&company='+company, title, 'width=900px,height=450px,center=1,resize=1,scrolling=0','../')
...
function fnc_fabric_booking( operation )
{
	...
	if(date_compare($('#txt_booking_date').val(), delivery_date)==false)
	{
 	...
 	if (form_validation('cbo_company_name*cbo_buyer_name*cbo_fabric_natu*txt_booking_date*txt_delivery_date*cbo_pay_mode','Company Name*Buyer Name*Fabric Nature*Booking Date*Delivery Date*Pay Mode')==false)
	{
		return;
	}	
	else
	{
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('cbo_company_name*update_id*cbo_buyer_name*txt_booking_no*cbo_fabric_natu*cbo_fabric_source*cbo_currency*txt_exchange_rate*cbo_pay_mode*txt_booking_date*cbo_supplier_name*txt_attention*txt_delivery_date*cbo_source*cbo_ready_to_approved*cbo_team_leader*cbo_dealing_merchant',"../../");
...
function open_sample_popup()
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, 'Sample Description', 'width=1030px,height=450px,center=1,resize=1,scrolling=0','../');
	...
	function fnc_fabric_booking_dtls( operation )
	{
		...
		{
			alert('Please , Select Fabric Source from Master part or Details part.');
			return;
		}
		...
		var data="action=save_update_delete_dtls&operation="+operation+get_submitted_data_string('txt_booking_no*update_id*cbo_fabric_natu*cbo_fabric_source*cbo_body_part*cbo_color_type*txt_style*txt_style_des*cbo_sample_type*libyarncountdeterminationid*construction*composition*yarnbreackdown*txt_fabricdescription*txt_gsm*txt_gmt_color*txt_color*txt_gmts_size*txt_size*txt_dia_width*txt_finish_qnty*txt_process_loss*txt_grey_qnty*txt_rate*txt_amount*update_id_details*process_loss_method*txt_article_no*txt_yarn_details*txt_remarks*cbo_body_type*txt_item_qty*txt_knitting_charge*txt_bh_qty*txt_rf_qty*cbo_fabric_source_dtls*txt_delivery_dates*txt_style_dtls_id*cbo_pay_mode*cbouom',"../../");
		//alert(data);
		//return;
		freeze_window(operation);
		http.open("POST","requires/sample_booking_non_order_controller.php",true);
...
function check_exchange_rate()
	{
		...
		var response=return_global_ajax_value( cbo_currercy+"**"+booking_date, 'check_conversion_rate', '', 'requires/sample_booking_non_order_controller');
...
/*	function validate_suplier(){
	...
	function print_report_button_setting(report_ids) 
	{
		...
		for (var k=0; k<report_id.length; k++)
		{
			...
			if(report_id[k]==174)
			{
				$('#button_panel').append( '<input type="button" value="Print UG" onClick="generate_fabric_report(7)"  style="width:100px" name="print7" id="print7" class="formbutton" />&nbsp;&nbsp;&nbsp;' );
			}
		}
	}
        ...
        <legend>Sample Booking (Without Order) </legend>
            <table  width="900" cellspacing="2" cellpadding="0" border="0">
                <tr>
                    ...
                    <td width="170"><? echo create_drop_down( "cbo_company_name", 172, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected, "load_drop_down( 'requires/sample_booking_non_order_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );color_from_library( this.value );check_kniting_charge();print_button_setting();","","" ); // new condtion
                    ?>	
                ...
                <tr>
                    ...
                    <td><? echo create_drop_down( "cbo_fabric_source", 172, $fabric_source,"", 1, "-- Select --", "","enable_disable(this.value);fabic_srce_con_fnc()", "", "2,3");	?></td>
                ...
                <tr>
                    <td align="right"></td> 
                    <td><input type="button" class="image_uploader" style="width:172px" value="CLICK TO ADD/VIEW IMAGE" onClick="file_uploader ( '../../', document.getElementById('txt_booking_no').value,'', 'sample_booking_non', 0 ,1)">
                    </td> 
                    <td>&nbsp;</td>
            ...
            <table  width="900" cellspacing="2" cellpadding="0" border="0">
                ...
                <tr>
                    ...
                    <td id="fabricdescription_id_td"> 
                        ...
                        <input type="text" id="txt_fabricdescription" name="txt_fabricdescription" class="text_boxes" style="width:160px" onDblClick="open_fabric_decription_popup()" placeholder="Browse" readonly/>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td><? echo create_drop_down( "cbo_fabric_source_dtls", 172, $fabric_source,"", 1, "-- Select --", "","enable_disable(this.value);", "", "1,4"); ?></td>
                ...
                <tr>
                	...
                	<td><? echo create_drop_down( "cbouom", 172, $unit_of_measurement,'', 1, '-Uom-',"", "",0,"1,12,23,27" ); ?></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td align="center" colspan="6" valign="middle" class="button_container">
                        ...
                        <input type="button" value="Print Booking2" onClick="generate_fabric_report(2)"  style="width:100px" name="print" id="print" class="formbutton" />
                        <input type="button" value="Print Amana" onClick="generate_fabric_report(3)"  style="width:100px" name="print3" id="print3" class="formbutton" />
                        <input type="button" value="Print AKH" onClick="generate_fabric_report(4)"  style="width:100px" name="print4" id="print4" class="formbutton" />-->
