<?
/*--- ----------------------------------------- Comments
Purpose			: 	This form will create Woven Garments Order Import
Functionality	:	
JS Functions	:
Created by		:	Kausar 
        ...
        <table width="900" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
            ...
            <tr class="general">
                <td> 
					...
					<? echo create_drop_down( "cbo_company_mst", 130, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "--Select Company--", $selected,"load_drop_down( 'buyer_order_import', this.value, 'load_drop_down_buyer_pop', 'buyer_pop_td' );",'' ); ?>
                </td>
...
$buyerFormat = array(1 => "PUMA", 2 => "GAP");
...
<!-- Important Field outside Form -->  
    <? echo load_freeze_divs ("../../",$permission);  ?>
            ...
            <fieldset style="width:670px;">
                <form onSubmit="check_format();" name="excelImport_1" id="excelImport_1" action="buyer_excel_order_import.php" enctype="multipart/form-data" method="post" > 
				<table width="670" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                <tr>
                	<td width="60"><b>Job</b></td>
                    <td width="100"><input style="width:90px;" type="text" title="Double Click to Search" onDblClick="openmypage_job('buyer_order_import.php?action=job_popup','Job/Order Selection Form')" class="text_boxes" placeholder="New Job No" name="txt_job_no" id="txt_job_no" readonly disabled /></td>
                    <td width="90" class="must_entry_caption"><b>Buyer Format</b></td>
                    <td width="100"><? echo create_drop_down( "cbo_buyer_format", 100, $buyerFormat,"", 1, "-Format-", 2, "fnc_jobField(this.value);"); ?></td>
                    <td width="80" class="must_entry_caption"><b>Select File</b></td>
                    <td width="170"><input type="file" id="uploadfile" name="uploadfile" class="image_uploader" required style="width:170px" /></td>
	...
	{
		var buyer_format=$('#cbo_buyer_format').val();
		if(buyer_format==0)
		{
			alert("Please Check or Select Buyer Format.");
			return;
		}
	}
	</script>
