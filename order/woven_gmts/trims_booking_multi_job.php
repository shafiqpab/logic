...
function fnc_trims_booking( operation )
{
	...
	var data_all="";
	if (form_validation('cbo_booking_month*cbo_company_name*cbo_supplier_name*txt_booking_date*cbo_source*txt_delivery_date','Booking Month*Company Name*Supplier Name*Booking Date*source*Delivery Date')==false)
	{
		return;
	}	
	else
                    ...
                    <tr>
                            ...
                            <td>
							  ...
							  //load_drop_down( 'requires/trims_booking_multi_job_controller', this.value, 'load_drop_down_supplier', 'supplier_td' );
							  	echo create_drop_down( "cbo_company_name", 172, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected, "load_drop_down( 'requires/trims_booking_multi_job_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );get_php_form_data( this.value, 'populate_variable_setting_data', 'requires/trims_booking_multi_job_controller' );","","" );
								?>	  
                           </td>
