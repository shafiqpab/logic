<?
/*--- ----------------------------------------- Comments
Purpose			: 	This form will create Woven Garments Order Entry Update
...
//--------------------------------------------------------------------------------------------------------------------
	echo load_html_head_contents("Order Info Update","../../", 1, 1, $unicode,1,'');
...
<script>
	...
	// Master Form-----------------------------------------------------------------------------
	var isRateEditableAfterBomApp=1;
	...
	function fnc_variable_settings_check(company_id)
	{
		...
		var all_variable_settings=return_ajax_request_value(company_id, 'load_variable_settings', 'requires/order_entry_update_controller');
		...
		if(copy_quotation==1)
		{
			...
			var page_link="'requires/order_entry_update_controller.php?action=quotation_id_popup','Quotation ID Selection Form'";
	...
	function sub_dept_load(cbo_buyer_name,cbo_product_department)
	{
		...
		{
			load_drop_down( 'requires/order_entry_update_controller',cbo_buyer_name+'_'+cbo_product_department, 'load_drop_down_sub_dep', 'sub_td' );
		}
	}
	...
	function check_tna_templete(buyer_id)
	{
		...
		var tnare=return_global_ajax_value(buyer_id+"_"+cbo_company_name, 'check_tna_templete', '', 'requires/order_entry_update_controller');
	...
	function open_qoutation_popup(page_link,title)
	{
		...
		else
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=980px,height=420px,center=1,resize=0,scrolling=0','../')
			emailwindow.onclose=function()
			{
				...
				if (theemail.value!="")
				{
					freeze_window(5);
					$('#tot_smv_qnty').val('');
					get_php_form_data( theemail.value+"_"+document.getElementById("cbo_company_name").value, "populate_data_from_search_popup_quotation", "requires/order_entry_update_controller" );
					if(set_smv_id==2) $('#cbo_order_uom').attr('disabled',true);
					load_drop_down('requires/order_entry_update_controller',document.getElementById("cbo_company_name").value, 'load_drop_down_location', 'location' );
					location_select();
					release_freezing();
				}
				var datas=set_breck_down.value;
				var list_view_tr = return_global_ajax_value( datas, 'load_php_dtls_form', '', 'requires/order_entry_update_controller');
				if(list_view_tr!='')
				{
	...
	function open_set_popup(unit_id,texboxid)
	{
		...
		var precost = return_ajax_request_value(txt_job_no, 'check_precost', 'requires/order_entry_update_controller');
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/order_entry_update_controller', item_id.value, 'load_drop_gmts_item', 'itm_td');
		}
	}
	...
	function fnc_order_entry( operation )
	{
		...
		{
			var txt_job_no =$("#txt_job_no").val();
			var precost = return_ajax_request_value(txt_job_no, 'check_precost_approve', 'requires/order_entry_update_controller');
			if (precost==1)
			{
	...
	function fnc_on_submit_reponse_mst()
	{
		if(http.readyState == 4)
		{
			...
			else if(parseInt(trim(reponse[0]))==0 || parseInt(trim(reponse[0]))==1)
			{
				...
				load_drop_down( 'requires/order_entry_update_controller',reponse[4], 'load_drop_gmts_item', 'itm_td');
	...
	function openmypage_job(page_link,title){
		/*if(form_validation('cbo_company_name','Company Name')==false)
		{
			...
			emailwindow.onclose=function()
			{
				...
				{
					freeze_window(5);
					var company_id=return_global_ajax_value(theemail.value, 'get_company_name', '', 'requires/order_entry_update_controller');
					fnc_variable_settings_check(company_id);
					get_php_form_data(theemail.value, "populate_data_from_search_popup", "requires/order_entry_update_controller" );
					var current_date='<? echo date('d-m-Y'); ?>';
					reset_form('orderentry_2','breakdown_div*breakdownratio_div*country_po_list_view','','','');
					load_drop_down( 'requires/order_entry_update_controller', document.getElementById('txt_job_no').value, 'load_drop_down_projected_po', 'projected_po_td' );
					load_drop_down( 'requires/order_entry_update_controller', document.getElementById('item_id').value, 'load_drop_gmts_item', 'itm_td');
					show_list_view(theemail.value,'order_listview','po_list_view','requires/order_entry_update_controller','setFilterGrid(\'tbl_po_list\',-1)');
					po_recevied_date();
					$('#breakdown_div').html('');
	...
	function fnc_order_entry_details( operation ){
		...
		else
		{
			...
			if(operation==1)
			{
				if($("#cbo_status").val()!=1)
				{
					var scLc_status = return_ajax_request_value(txt_job_no+"_"+$("#update_id_details").val(), 'sc_lc_status', 'requires/order_entry_update_controller')
			...
			if(operation==2)
			{
				var cutting_qty=return_global_ajax_value($("#update_id_details").val()+"_"+$("#cbo_deliveryCountry_id").val(), 'get_cutting_qty_country', '', 'requires/order_entry_update_controller');
				if(cutting_qty>0)
				{
			...
			if(breakdown_type==1)
			{
				...
				if(po_id!="")
				{
					previous_ord_qty = return_ajax_request_value(po_id+'***'+country_id+'***'+gmtsItem_id+'***'+operation+'***'+pre_country_id, 'full_qty_check_for_validation', 'requires/order_entry_update_controller')
				}
			...
			var data_po="action=save_update_delete_dtls&operation="+operation+"&color_table="+color_table+"&size_table="+size_table+"&delete_po="+delete_po+"&delete_country="+delete_country+get_submitted_data_string('cbo_breakdown_type*cbo_round_type*cbo_order_status*txt_po_no*txt_po_received_date*txt_pub_shipment_date*txt_avg_price*txt_upCharge*txt_docSheetQty*txt_noOf_carton*cbo_projected_po*txt_grouping*txt_file_no*cbo_packing_po_level*cbo_delay_for*update_id_details*color_size_break_down*update_id*txt_po_datedif_hour*txt_po_remarks*cbo_packing*copy_id*tot_set_qnty*cbo_status*set_breck_down*cbo_buyer_name*txt_style_ref*cbo_company_name*txt_shipment_date*hidd_job_id*txt_phd',"../../");
			...
			//var data_country=get_submitted_data_string('cbo_gmtsItem_id*cbo_deliveryCountry_id*cbo_code_id*txt_cutup_date*cbo_cutOff_id*txt_countryShip_date*txt_breakdownGrouping*txt_pcsQty',"../../",2);
			//alert(data_country);return;
			var data_color="";
			var data_size="";
			var data_breakdown="";
	...
	function fnc_on_submit_reponsedtls()
	{
		if(http.readyState == 4)
		{
			...
			else if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2  || reponse[0]==20)
			{
				...
				show_list_view( $('#update_id').val()+"*"+reponse[1],'order_listview','po_list_view','requires/order_entry_update_controller','setFilterGrid(\'tbl_po_list\',-1)');
				show_list_view(reponse[1],'show_po_active_listview','country_po_list_view','requires/order_entry_update_controller','');
	...
	function set_tna_task()
	{
		...
		load_drop_down( 'requires/order_entry_update_controller', txt_po_received_date+"_"+txt_pub_shipment_date+"_"+cbo_buyer_name, 'load_drop_down_tna_task', 'tna_task_td');
	...
	function set_pub_ship_date()
	{
		...
		var publish_shipment_date=return_global_ajax_value(company_id, 'publish_shipment_date', '', 'requires/order_entry_update_controller');
		if(publish_shipment_date==1){
			 $('#txt_pub_shipment_date').attr('disabled',false);
		}
		else{
	...
	function pop_entry_actual_po()
	{
		...
		var page_link='requires/order_entry_update_controller.php?action=actual_po_info_popup&po_id='+po_id+'&txt_job_no='+txt_job_no;
	...
	function fnc_create_qty_breakdown(is_update,type)
	{
		...
		var disabled = ''; var disabled_rate = '';
		var job_no = $('#txt_job_no').val();
		if(isRateEditableAfterBomApp==1)
		{
			var is_approved=return_global_ajax_value(job_no, 'check_precost_approve', '', 'requires/order_entry_update_controller');
			if(is_approved == 1){
				disabled_rate = "disabled";
			}
			else{
				disabled_rate = '';
			}
		...
		if(type==1 || type==4)
		{
			...
			for (var i = 1; i < color_value.length; i++)
			{
				...
				for (var m = 1; m < size_value.length; m++){
					...
					html+='<td width="55px"><input type="text" name="txt_colorSizeExCut_[]" id="txt_colorSizeExCut_'+i+'_'+m+'" value="'+excut+'" class="text_boxes_numeric" style="width:50px;" onBlur="fnc_calAmountQty_ex(this.value,3); fnc_copy_qty_excut('+i+','+m+',this.value,2);" '+disabled+' /></td>';
				}
				...
				for (var m = 1; m < size_value.length; m++){
					...
					html+='<td width="55px"><input type="text" name="txt_colorSizeArticleNo_[]" id="txt_colorSizeArticleNo_'+i+'_'+m+'" value="'+artno+'" class="text_boxes" style="width:50px;" onBlur="fnc_calAmountQty_ex(this.value,4)" '+disabled+'/></td>';
				}
			...
			for (var m = 1; m < size_table; m++)
			{
				...
				if(type==1 || type==2  || type==3)
				{
					...
					if(($('#txt_colorSizeExCut_'+i+'_'+m).val()*1)==0)
					{
						...
						//console.log($('#txt_colorSizePLanCut_'+i+'_'+m).val());
						$('#txt_colorSizePLanCut_'+i+'_'+m).val( $('#txt_colorSizeQty_'+i+'_'+m).val()*1 );
					}
					else
					{
						...
						//$('#txt_colorSizePLanCut_'+i+'_'+m).val(454);
					}

					//console.log('first:'+totPlanCutAmt);
					//console.log('#txt_colorSizePLanCut_'+i+'_'+m + ' __' +$('#txt_colorSizePLanCut_'+i+'_'+m).val()*1);
					totPlanCutAmt = totPlanCutAmt+$('#txt_colorSizePLanCut_'+i+'_'+m).val()*1;
					//console.log(''+totPlanCutAmt);
				}
				else if(type==4)
				{
	...
	function fnc_set_ship_date()
	{
		...
		var set_ship_date=return_global_ajax_value(txt_cutup_date+'_'+cbo_cut_up, 'set_ship_date', '', 'requires/order_entry_update_controller');
	...
	function fnc_copy_po(operation,old_po_id,old_po_no)
	{
		...
		if(r==true)
		{
			...
			var data="action=save_update_delete_dtls&operation="+operation+get_submitted_data_string('copy_id*update_id*txt_copypo_no*set_breck_down*update_id_details',"../../");
	...
	function reorder_size_color()
	{
		...
		{
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/order_entry_update_controller.php?action=reorder_size_color&txt_job_no='+txt_job_no, 'Color Size Ordering', 'width=600px,height=400px,center=1,resize=1,scrolling=0','../')
		}
	}
	...
	function openmypage_ultimate_pop(val,id,color)
	{
		...
		else
		{
			...
			else
			{
				...
				emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/order_entry_update_controller.php?data='+data+'&action=ultimate_dtls_popup','Utimate Dtls Pop-Up', 'width=780px,height=400px,center=1,resize=1,scrolling=0','../')
	...
	function fnc_buyer_style_po_check(value)
	{
		...
		var po_id=$('#update_id_details').val();
		var buyer_style_po_check=return_ajax_request_value(company_id+'_'+buyer_id+'_'+style_ref+'_'+value+'_'+po_id, 'load_buyer_style_po_check', 'requires/order_entry_update_controller');
	...
	function fnc_cut_off_select( country_id )
	{
		if(country_id!=0)
		{
			get_php_form_data(country_id, "load_cutOff_id_from_lib", "requires/order_entry_update_controller" );
		}
	}
	...
	function fnc_assortment_pop_up(color_n,size_n,col_in,siz_in,qty)
	{
		...
		else
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/order_entry_update_controller.php?data='+data+'&action=assortment_pop_up','Assortment Dtls Pop-Up', 'width=480px,height=200px,center=1,resize=1,scrolling=0','../');
			...
			<tr>
				<td valign="top" width="850">
					...
					<div id="content_search_panel" style="width:840px">
						<fieldset style="width:840px;">
							<form name="orderentry_1" id="orderentry_1" autocomplete="off">
								<table width="820" cellspacing="2" cellpadding="0" border="0">
									<tr>
										<td colspan="3" align="right"><span  style="color:blue; font-size:16px; font-weight:bold ">Job No</span></td>
										<td colspan="3">
											<input style="width:140px;" type="text" title="Double Click to Search" onDblClick="openmypage_job('requires/order_entry_update_controller.php?action=job_popup','Job/Order Selection Form')" class="text_boxes" placeholder="Browse Job No" name="txt_job_no" id="txt_job_no" readonly />
									...
									<tr>
										...
										<td width="150">
                        	<? echo create_drop_down( "cbo_company_name", 150, "select id, company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "Select Company", $selected, "load_drop_down( 'requires/order_entry_update_controller', this.value, 'load_drop_down_location', 'location'); location_select(); load_drop_down( 'requires/order_entry_update_controller', this.value, 'load_drop_down_buyer', 'buyer_td'); load_drop_down( 'requires/order_entry_update_controller', this.value, 'load_drop_down_agent', 'agent_td'); load_drop_down( 'requires/order_entry_update_controller', this.value, 'load_drop_down_party_type', 'party_type_td'); fnc_variable_settings_check(this.value);"); //set_smv_check(this.value);
                        	?>
                        ...
                        <td>
                        	<input class="text_boxes" type="text" style="width:82px" placeholder="Browse/Write" name="txt_style_ref" id="txt_style_ref" onDblClick="open_qoutation_popup('requires/order_entry_update_controller.php?action=quotation_id_popup','Quotation ID Selection Form')" readonly/>
                        	<input class="text_boxes" id="quotation_id" type="text" style="width:40px" placeholder="pq. id" disabled/>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                    	...
                    	<tr>
                    		...
                    		<td id="leader_td"><? echo create_drop_down( "cbo_team_leader", 150, "select id,team_leader_name from lib_marketing_team where status_active =1 and is_deleted=0 order by team_leader_name","id,team_leader_name", 1, "Select Team", $selected, "load_drop_down( 'requires/order_entry_update_controller', this.value, 'cbo_dealing_merchant', 'div_marchant' );load_drop_down( 'requires/order_entry_update_controller', this.value, 'cbo_factory_merchant', 'div_marchant_factory' ) " );
                    		?></td>
                    	...
                    	<tr>
                    		...
                    		<td><? echo create_drop_down( "cbo_order_uom",50, $unit_of_measurement, "",0, "", 1, "get_unit_id(this.value)","","1,58" ); ?>
                    			...
                    			<input class="text_boxes" type="text" style="width:80px;" name="tot_smv_qnty" id="tot_smv_qnty" placeholder="SMV" disabled/>
                    		</td>
                    		<td>Ready for Budget</td>
                    		<td ><? asort($yes_no); echo create_drop_down( "cbo_ready_for_budget", 150, $yes_no,"", 1, "-- Select--", 2, "","","" ); ?></td>
                    		<td>Remarks</td>
                    		<td><input class="text_boxes" type="text" style="width:135px;"  name="txt_remarks" id="txt_remarks"/></td>
								...
								<td><input type="text" id="smv_<? echo $i;?>"   name="smv_<? echo $i;?>" style="width:20px"  class="text_boxes_numeric" onChange="calculate_set_smv(<? echo $i;?>)"  value="<? echo $data[2]; ?>" <? echo $disabl." "; echo $readonly; ?>  />
									...
									<td><input type="text" id="cutsmv_<? echo $i;?>" name="cutsmv_<? echo $i;?>" style="width:20px" class="text_boxes_numeric" onChange="calculate_set_cutsmv(<? echo $i;?>)" value="<? echo $data[6]; ?>" <? echo $disabl." "; echo $readonly; ?> />
										...
										else
										{
											?>
											<tr id="settr_1" align="center">
												<td><?  echo create_drop_down( "cboitem_1", 120, $garments_item, "",1,"Select", 0, "check_duplicate(1,this.id ); check_smv_set(1); check_smv_set_popup(1);",'','' ); ?></td>
			...
			<!--<table width="950" cellspacing="0" class="" border="0">
				<tr>
					...
					<tr>
						...
						<td>&nbsp;</td>
						<td><input type="button" id="set_button" class="image_uploader" style="width:150px;" value="Internal Ref" onClick="open_terms_condition_popup('requires/order_entry_update_controller.php?action=terms_condition_popup','Terms Condition')" /></td>
					</tr>
					<tr>
			...
			<table cellpadding="0" cellspacing="2" border="0" >
				<thead class="form_table_header">
					<tr height="30">
						...
						<th><input type="button" id="reorder" value="Color Size Sequence" width="120" class="image_uploader" onClick="reorder_size_color()"/></th>
						<th colspan="4">&nbsp;</th>
					</tr>
					<tr height="30">
						<th>Order Status</th>
						...
						<th class="must_entry_caption">Pub. Shipment Date</th>
						<th class="must_entry_caption">Shipment Date</th>
						<th width="60" title="Pack Handover Date">PHD</th>
				...
				<tr>
					<td width="80">
						<? echo create_drop_down( "cbo_order_status", 80, $order_status, 0, "", $selected,"", "po_recevied_date( this.value );" ); ?>

					</td>
					<td width="105">
						<input class="text_boxes" name="txt_po_no" id="txt_po_no" type="text" placeholder="Write" style="width:100px" onBlur="fnc_buyer_style_po_check(this.value);"/>
					</td>
					<td width="60">
						<input name="txt_po_received_date" id="txt_po_received_date" class="datepicker" type="text" style="width:55px;" readonly/>
					</td>
					<td width="60">
						<input name="txt_pub_shipment_date" id="txt_pub_shipment_date" class="datepicker" type="text"  style="width:55px;" onChange="set_publish_shipment_date()" readonly/>
					</td>
					<td width="60">
						<input name="txt_shipment_date" id="txt_shipment_date" class="datepicker" type="text"  style="width:55px;" onChange="shipment_date_diff(); set_pub_ship_date()" readonly/>
					</td>
					<td><input name="txt_phd" id="txt_phd" class="datepicker" type="text"  style="width:50px;" onChange="set_phd_po_date(1);" readonly/></td>
					<td width="55">
						<input name="txt_avg_price" id="txt_avg_price" class="text_boxes_numeric" type="text" style="width:50px;" onBlur="fnc_calculateRate(document.getElementById('cbo_breakdown_type').value,0);" />
					</td>
					<td width="40">
						<input name="txt_upCharge" id="txt_upCharge" class="text_boxes_numeric" type="text" style="width:35px;" />
					</td>
					<td width="65">
						<input name="txt_docSheetQty" id="txt_docSheetQty" class="text_boxes_numeric" type="text" style="width:60px;" />
					</td>
					<td width="55">
						<input name="txt_noOf_carton" id="txt_noOf_carton" class="text_boxes_numeric" type="text" style="width:55px;" disabled />
					</td>
					<td id="projected_po_td" width="80"><? echo create_drop_down( "cbo_projected_po", 80,$blank_array, "", 1, "Select", "" ); ?></td>
					<td id="tna_task_td" width="80" style="display: none;"><? //echo create_drop_down( "cbo_tna_task", 80,$blank_array, "", 1, "Select", "" ); ?></td>
					...
					<td width="80"><? echo create_drop_down( "cbo_packing_po_level", 80, $packing,"", 1, "Select", "", "",1,"" ); ?></td>
				...
				<tr>
					...
					<td><? echo create_drop_down( "cbo_status", 80, $row_status, 0, "", 1, "","",1 ); ?></td>
				</tr>
			</table>
			<table width="600">
				<tr><td width="400">
					<table rules="all" width="400" cellpadding="0" cellspacing="2" border="1" class="rpt_table">
						...
						<tbody>
							<tr>
								<td id="itm_td">
									<? echo create_drop_down( "cbo_gmtsItem_id", 120, $garments_item, 0, 1, "Select Item", $selected_item,"fnc_calAmountQty_ex(0,1); fnc_calculateRate(document.getElementById('cbo_breakdown_type').value,0);",1);  ?>

								</td>
								<td>
									<?  echo create_drop_down( "cbo_deliveryCountry_id", 100,"select id, country_name from lib_country where status_active=1 and is_deleted=0 order by country_name", "id,country_name", 1, "Select Country", "","load_drop_down( 'requires/order_entry_update_controller', this.value, 'load_dorp_down_code', 'code_td' ); fnc_cut_off_select(this.value); fnc_set_ship_date(); fnc_calculateRate(document.getElementById('cbo_breakdown_type').value,0);change_country_code(this.value);" ); ?><input type="hidden" id="hid_prev_country" />
								</td>
                                <td id="cbo_countryCode_id" style=""><?  echo create_drop_down( "cbo_countryCode_id", 100,"select id, short_name from lib_country where status_active=1 and is_deleted=0 and short_name is not null  order by country_name", "id,short_name", 1, " Country Code", "","change_country(this.value)" ); ?>

								</td>
								<td id="code_td">
									<? echo create_drop_down( "cbo_code_id", 100,$blank_array, "", 1, "Select Code", "","" ); ?>

								</td>
								<td style="display: none;">
									<? echo create_drop_down( "cbo_country_id", 100,"select id, country_name from lib_country where status_active=1 and is_deleted=0 order by country_name", "id,country_name", 1, "Country", ""," load_drop_down( 'requires/order_entry_update_controller', this.value, 'load_dorp_down_countryCode', 'countryCode_td' );" ); ?></td>
