<?
/*-------------------------------------------- Comments
Purpose			:	This form will create Hand loom Requisition
Functionality	:
JS Functions	:
...
<script>
	...
	function fnc_quotation_inquery( operation )
	{
		...
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_name').val()+'**'+$('#txt_system_id').val()+'**'+$('#update_id').val()+'**'+report_title, "inquery_entry_print", "requires/hand_loom_requisition_controller" )
			release_freezing();
			return;
		}
		else if(operation==6)
		{
			...
			var report_title=$( "div.form_caption" ).html();
			print_report( $('#cbo_company_name').val()+'**'+$('#txt_system_id').val()+'**'+$('#update_id').val()+'**'+report_title+'**'+$('#txt_buyer_inquiry_id').val(), "inquery_entry_print2", "requires/hand_loom_requisition_controller" )
			release_freezing();
			return;
		}
		...
		if(cbo_basis == 2)
		{
			var form_val_ids = 'cbo_company_name*cbo_location_name';
			var form_val_mess = 'Company*Location';
		}
		else
		{
			var form_val_ids = 'txt_buyer_inquiry_no*cbo_company_name*cbo_location_name';
			var form_val_mess = 'Buyer Inquiry No*Company*Location';
		}

		if (form_validation(form_val_ids,form_val_mess)==false)
		{
			release_freezing();
			return;
		}
	...
	function open_mrrpopup()
	{
		...
		var page_link='requires/hand_loom_requisition_controller.php?action=mrr_popup&company='+company;
		...
		emailwindow.onclose=function()
		{
			...
			get_php_form_data(mrrNumber[0], "populate_data_from_data", "requires/hand_loom_requisition_controller");
	...
	function open_inquery_popup()
	{
		...
		var company = $("#cbo_company_name").val();
		var page_link='requires/hand_loom_requisition_controller.php?action=inquery_popup&company='+company;
		var title="Search  Popup";
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1180px,height=400px,center=1,resize=0,scrolling=0','../')
		emailwindow.onclose=function()
		{
			...
			get_php_form_data(mrrNumber[0], "populate_data_from_inquiry", "requires/hand_loom_requisition_controller");
	...
	function buyer_season_load()
	{
		...
		load_drop_down( 'requires/hand_loom_requisition_controller', cbo_buyer_name+"_"+company, 'load_drop_down_season_buyer', 'season_td' );
	}
	function check_quatation()
	{
		var txt_style_ref=$('#txt_style_ref').val();
		var txt_inquery_id=$('#txt_system_id').val();
		var response=return_global_ajax_value( txt_style_ref+"**"+txt_inquery_id, 'check_style_ref', '', 'requires/hand_loom_requisition_controller');
	...
	function fnc_variable_settings_check(company_id)
	{
		$('#txt_color').val('');
		$('#txt_color_id').val('');
		var color_from_lib=return_ajax_request_value(company_id, 'load_variable_settings', 'requires/hand_loom_requisition_controller');
		if(color_from_lib==1)
		{
			$('#txt_color_id').val('');
	...
	function color_select_popup()
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/hand_loom_requisition_controller.php?action=color_popup&buyer_name='+buyer_name, 'Color Select Pop Up', 'width=250px,height=450px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0];
	...
	function active_inactive() 
	{
    	...
    	load_drop_down('requires/hand_loom_requisition_controller', within_group + '_' + company_id, 'load_drop_down_buyer', 'buyer_td');

    	if(within_group == 1)
    	{
    		load_drop_down( 'requires/hand_loom_requisition_controller', ''+'_'+document.getElementById('cbo_company_name').value, 'load_drop_down_season_buyer', 'season_td');
    		load_drop_down( 'requires/hand_loom_requisition_controller', '', 'load_drop_down_brand', 'brand_td');
    	}
    }
	...
	function openmypage_fabric_cons(inc)
	{
		...
		if(document.getElementById('checkBox_'+inc).checked)
		{
			...
			var width = 'width=1280px,height=350px,center=1,resize=1,scrolling=0';
		}
		else
		{
			var action = "fabric_construction_popup";
			var title = "Material Construction Popup";
			var width = 'width=1000px,height=350px,center=1,resize=1,scrolling=0';
		}
		var page_link="requires/hand_loom_requisition_controller.php?action="+action+"&fab_construction_id="+fab_construction_id;
		...
		emailwindow.onclose=function()
		{
			if(document.getElementById('checkBox_'+inc).checked)
			{
				...
				get_php_form_data(determination_id+'_'+inc, "populate_data_from_determination", "requires/hand_loom_requisition_controller");
			}
			else
			{
	...
	{
		var page_link="requires/hand_loom_requisition_controller.php?action=composition_popup&inc="+inc;
	...
	function fnc_details_info( operation )
	{
		...
		else
		{
			...
			{
				if (form_validation('cboType_'+i+'*txtconstruction_'+i+'*cboProductType_'+i+'*txtcompone_'+i+'*cboFinishType_'+i+'*txtColor_'+i+'*txtFabricWeight_'+i+'*txtCutableWidth_'+i+'*cboWashType_'+i+'*cboUom_'+i,'Type*Construction*Product Type*Composition*Finish Type*Color*Finished Width*Cutable Width*Wash Type*Uom')==false)
				{
					return;
				}
				data_all=data_all+get_submitted_data_string('cboType_'+i+'*fabConstructionId_'+i+'*txtconstruction_'+i+'*fabConstruction_'+i+'*cboProductType_'+i+'*txtcompone_'+i+'*cbocompone_'+i+'*txtWeaveDesign_'+i+'*cboFinishType_'+i+'*txtColor_'+i+'*txtFabricWeight_'+i+'*cboweighttype_'+i+'*txtFinishedWidth_'+i+'*txtCutableWidth_'+i+'*cboWashType_'+i+'*txtOfferQty_'+i+'*cboUom_'+i+'*txtBuyerTgtPrice_'+i+'*txtAmount_'+i+'*updateDtlsId_'+i+'*cboColorId_'+i+'*txtHlNo_'+i+'*inquiryDtlsId_'+i+'*yarnCountDeterminationId_'+i+'*hiddWarpYarnTypeId_'+i+'*hiddWeftYarnTypeId_'+i+'*txtWeave_'+i+'*txtDesign_'+i+'*txtRemark_'+i,"../../");
			}
	...
	function fnc_load_tr(inquiry_id,up_id='') 
	{
		...
		var return_data = return_global_ajax_value( data, 'load_php_dtls_form', '', 'requires/hand_loom_requisition_controller');
		...
		var dtls_row = `<tr id="tr_1" style="height:10px;" class="general">
								...
								<td >
									<? 
										...
										echo create_drop_down("cboType_1", 100, $hand_loom_typeArr , "", 1, "Select", 0, "");
									?>
										
								</td>
								<td>
									<input type="text" class="text_boxes" name="txtHlNo_1" id="txtHlNo_1"   style="width:70px;" >
								...
								<td >
									...
									<input type="hidden" id="txtWeaveDesign_1"  name="txtWeaveDesign_1"  value="" />
								</td>
								<td> 
								...
								<td>
									<input type="text" class="text_boxes" name="txtRemark_1" id="txtRemark_1" placeholder="Write"  style="width:70px;" >
								</td>
								<td>
	...
	function openmypage_yarnType(type,row)
	{
		...
		var page_link='requires/hand_loom_requisition_controller.php?action=yarn_type_popup&hiddYarnTypeId='+hiddYarnTypeId+'&type='+type;
                ...
                <table  width="1020" cellspacing="2" cellpadding="0" border="0">
                    ...
                    <tr>
                        ...
                        <td width="150">
                        	...
                        	echo create_drop_down( "cbo_company_name", 140,$com_sql,"id,company_name", 1, "-- Select Company --", $selected,"load_drop_down( 'requires/hand_loom_requisition_controller', $('#cbo_within_group').val()+'_'+this.value, 'load_drop_down_buyer', 'buyer_td');load_drop_down( 'requires/hand_loom_requisition_controller', this.value, 'load_drop_down_season_com', 'season_td'); fnc_variable_settings_check(this.value);load_drop_down( 'requires/hand_loom_requisition_controller', this.value, 'load_drop_down_location', 'location_td' );" ,0); ?>
                    ...
                    <tr>
                        ...
                        <td id="buyer_td">
                        	<? echo create_drop_down( "cbo_buyer_name", 140, "select id,buyer_name from lib_buyer  where status_active =1 and is_deleted=0  order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down( 'requires/hand_loom_requisition_controller', this.value+'_'+document.getElementById('cbo_company_name').value, 'load_drop_down_season_buyer', 'season_td');load_drop_down( 'requires/hand_loom_requisition_controller', this.value, 'load_drop_down_brand', 'brand_td');" ,0); ?>
                    ...
                    <tr>
						...
						<td align="right" class="must_entry_caption">Team leader</td>
                       	<td>
							   ...
							   $team_onchange_str = "load_drop_down( 'requires/hand_loom_requisition_controller', this.value, 'load_drop_down_dealing_merchant', 'div_marchant'); load_drop_down( 'requires/hand_loom_requisition_controller', this.value, 'load_drop_down_sample_marchant', 'div_sample_marchant'); ";
                    ...
                    <tr>
                        ...
                        <td colspan="5"><input class="text_boxes" type="text" style="width:98%" placeholder="Write"  name="txt_remarks" id="txt_remarks"/></td>
                    </tr>
					<tr>
						<td align="center" colspan="8">
							<?
								include("../../terms_condition/terms_condition.php");
								terms_condition(585,'txt_system_id','../../','');
							?>
						</td>
					</tr>
                    ...
                    <tr>
                        ...
                        <?=load_submit_buttons( $permission, "fnc_quotation_inquery", 0,1 ,"reset_fnc();",1); ?>   
                       	 	<input type="button" name="button" class="formbutton" value="Print 2"   id="report_btn_2" style="display:none;width:80px;" onClick="fnc_quotation_inquery(6);"  />                    
                        </td>
                   </tr>
                </table>
					...
					<table cellpadding="0" cellspacing="0" width="2050" class="rpt_table" border="1" rules="all" id="tbl_details">
						<thead>
							<th width="35">Check</th>
							<th width="100" class="must_entry_caption">Requisition Type</th>
					...
					<table style="margin-top: 5px;">
						<tr>
							<td colspan="15"  valign="bottom" align="center" class="">
								<?=load_submit_buttons($permission, "fnc_details_info", 0, 0, "refresh_button()", 2);?>
...
<script>
	...
	if(company_id!=0){
		setTimeout(function(){
			load_drop_down( 'requires/hand_loom_requisition_controller', company_id, 'load_drop_down_buyer', 'buyer_td');
			load_drop_down( 'requires/hand_loom_requisition_controller', company_id, 'load_drop_down_season_com', 'season_td');
			...
			if(buyer_id!=0){
				setTimeout(function(){
					load_drop_down( 'requires/hand_loom_requisition_controller',document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('cbo_company_name').value, 'load_drop_down_season_buyer', 'season_td'); 
					load_drop_down( 'requires/hand_loom_requisition_controller', document.getElementById('cbo_buyer_name').value, 'load_drop_down_brand', 'brand_td');
