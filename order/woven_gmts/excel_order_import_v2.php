...
{	
	...
	if (move_uploaded_file($source, $targetzip)) 
	{
        ...
        <script>
			...
			function fnc_excel_import(operation)
			{
				...
				for(var i=1; i<=totStyleCount; i++)
				{
					...
					if($('#txt_quotation_id').val()!="" && totStyleCount>1)
					{
						alert("Multiple Style Upload Restricted when QC No Browse.");
						release_freezing();
						return;
					}
				...
				var data="action=save_update_delete&operation="+operation+get_submitted_data_string('cbo_company_id*cbo_location_id*cbo_buyer_id*cbo_style_owner_id*cbo_product_department*cbo_currercy_id*cbo_season_year*cbo_season_id*cbo_brand_id*cbo_prod_catgory*cbo_team_leader*cbo_dealing_merchant*cbo_factory_merchant*cbo_order_uom*cbo_gmtsItem_id*tot_smv_qty*cbo_packing*cbo_ship_mode*txt_sclc*txt_job_no*hid_ws_id*txt_quotation_id',"../../")+dataString;
				//alert(data);
			...
			function fnc_style_owner(val)
			{
				...
				var cost_control_source=ex_variable[2];
				$('#hidd_copyQuotdata').val(all_variable_settings);
				
				if(copy_quotation==1)
				{
					if(set_smv_id==2 || set_smv_id==4 || set_smv_id==5 || set_smv_id==6)
					{
						$("#txt_quotation_id").attr("disabled",false);
						$('#txt_quotation_id').attr('readonly',true);
						$('#txt_quotation_id').attr('placeholder','Browse');
						var page_link="'requires/excel_order_import_controller_v2.php?action=quotation_id_popup','Quotation ID Selection Form'";// for Price Quotation
						$('#txt_quotation_id').removeAttr("onDblClick").attr("onDblClick","open_qoutation_popup("+page_link+");");
					}
					else if(set_smv_id==7)
					{
						...
						var page_link="'requires/excel_order_import_controller_v2.php?action=qc_id_popup','Quick Costing Style Selection'";// for Quick Costing
						$('#txt_quotation_id').removeAttr("onDblClick").attr("onDblClick","open_qoutation_popup("+page_link+");");
					}
					else
					{
						$("#txt_quotation_id").attr("disabled",true);
						$('#txt_quotation_id').attr('readonly',true);
						$('#txt_quotation_id').attr('placeholder','Display');
						$('#txt_quotation_id').removeAttr('onDblClick','onDblClick');
					}
				}
				else
				{
			...
			{
				if( form_validation('cbo_company_id*cbo_buyer_id','Company Name*Buyer')==false)
				{
					return;
				}
				else
				{
					...
					var ex_variable=$('#hidd_copyQuotdata').val().split("_");
					...
					emailwindow.onclose=function()
					{
						...
						if (theemail.value!="")
						{
							...
							else
							{
								if(cost_source==2)
								{	
									get_php_form_data( theemail.value+"_"+document.getElementById("cbo_company_name").value, "populate_data_from_search_popup_quotation", "requires/excel_order_import_controller_v2" );
									if(set_smv_id==4) 
									{
										//$("#season_td").css('color','blue');
										$('#cbo_season_id').attr('disabled',true);
									}
								}
								else if (cost_source==1 || cost_source==4)//QC
								{
									get_php_form_data( theemail.value, "populate_data_from_search_popup_qc", "requires/excel_order_import_controller_v2" );//else 
									//$("#season_td").css('color','black');
								}
								...
								if(location_id==0)
								{
									load_drop_down('requires/excel_order_import_controller_v2',document.getElementById("cbo_company_id").value, 'load_drop_down_location', 'location_td' );
								}
								location_select();
        ...
        <!-- Important Field outside Form -->  
          ...
          <table width="1530" cellspacing="0" border="1" class="rpt_table" rules="all" id="" >      
          	<thead>
            	<tr>
              ...
              <tbody>
              	<tr>
                    ...
                    <td>
                    	<input class="text_boxes" type="text" style="width:68px;" name="txt_quotation_id" id="txt_quotation_id" readonly />
                        <input class="text_boxes" type="hidden" style="width:28px;" name="hidd_copyQuotdata" id="hidd_copyQuotdata" />
                    </td>
                    ...
                    <td><?=create_drop_down( "cbo_team_leader", 80, "select id, team_leader_name from lib_marketing_team where project_type=2 and status_active =1 and is_deleted=0 order by team_leader_name","id,team_leader_name", 1, "-Select Team-", $selected, "load_drop_down( 'requires/excel_order_import_controller_v2', this.value, 'load_drop_down_dealing_merchant', 'div_marchant'); load_drop_down('requires/excel_order_import_controller_v2', this.value, 'load_drop_down_factory_merchant', 'div_marchant_factory')" ); ?></td>
					...
					foreach($color_size_data as $color_val=>$size_data)
					{
						...
						foreach($size_data as $size_val=>$extra_data)
						{
							...
							{	
								...
								$po_avg_rate=number_format($ex_data[5],4,'.','');
								$exPer=$ex_data[6];
