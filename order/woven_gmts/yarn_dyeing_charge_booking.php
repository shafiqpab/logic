...
function openmypage_job(title)
{
	...
	else
	{
		...
		//alert(company);
		page_link='requires/yarn_dyeing_charge_booking_controller.php?action=order_search_popup&company='+company+'&txt_booking_date='+txt_booking_date;
...
function fnc_yarn_dyeing(operation)
{
	...
	if( form_validation('cbo_company_name*cbo_supplier_name*txt_booking_date*txt_job_no*txt_lot*cbo_source*txt_yern_color*txt_wo_qty*cbo_pay_mode','Company Name *Supplier Name*Booking Date*Job Number*Yarn Lot*Source Name*Color*Order Quanty*Pay Mode')==false )
	{
		return;
	}
	else
...
function check_exchange_rate()
	{
		...
		var response=return_global_ajax_value( cbo_currercy+"**"+booking_date, 'check_conversion_rate', '', 'requires/yarn_dyeing_charge_booking_controller');
    ...
    <legend>Yarn Dyeing Wo</legend>
        <table  width="1200" cellspacing="2" cellpadding="0" border="0">
                ...
                <tr>
                    ...
                    echo create_drop_down( "cbo_company_name", 140, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected,"chack_variable(this.value);load_drop_down( 'requires/yarn_dyeing_charge_booking_controller',this.value, 'load_drop_down_supplier', 'supplier_td' );get_php_form_data( this.value, 'button_setting_data', 'requires/yarn_dyeing_charge_booking_controller' );",0);
                    ?></td>
               ...
               <tr>
                ...
                <tr>
                    <td  colspan="8"> 
                        <table width="1200" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all" id="dtls_part">
                            <thead>
                                <tr>
