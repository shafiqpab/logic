...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Woven Country and Order Wise Trims Booking", "../../", 1, 1,$unicode,'','');
?>
<script>
...
function openmypage(page_link,title)
{
	...
	emailwindow.onclose=function()
	{
		...
		if (theemail.value!="")
		{
			...
			get_php_form_data(theemail_po_id.value+"_save", "set_delivery_date_from_tna", "requires/country_and_order_wise_trims_booking_entry_controller" );
			var tna_date=$('#txt_tna_date').val();
            compare_date(2);
			if(cbo_item_from_precost==1){
				load_drop_down( 'requires/country_and_order_wise_trims_booking_entry_controller', theemail.value+'_'+document.getElementById('cbo_supplier_name').value+'_'+cbo_company_name, 'load_drop_down_trim_precost_id', 'trim_group_td' );
			}
			load_drop_down( 'requires/country_and_order_wise_trims_booking_entry_controller', theemail.value, 'load_drop_down_gmt_item', 'gmt_item_td' );
...
function set_precost_data(trim_precost_id,type)
{
   ...
   reset_form('','','cbo_colorsizesensitive*cbo_uom*txt_quantity*txt_bal_woq*txt_avg_price*txt_amount*txt_update_dtls_id','','','')//txt_delevary_date
	...
	var is_booking = get_php_form_data(txt_po_id+"_"+trim_precost_id, "check_previous_booking", "requires/country_and_order_wise_trims_booking_entry_controller");

	get_php_form_data(trim_precost_id+"_"+txt_po_id+"_"+txt_country+"_"+cbo_gmt_item_id+"_"+cbo_currency+"_"+type+"_"+cbo_item_from_precost+"_"+cbo_isshort+"_"+cbo_company_name, "set_precost_data", "requires/country_and_order_wise_trims_booking_entry_controller");
}
...
function fnc_generate_booking()
{
		...
		http.open("POST","requires/country_and_order_wise_trims_booking_entry_controller.php",true);
...
function openmypage_booking(page_link,title)
{
		...
		emailwindow.onclose=function()
		{
			...
			if (theemail.value!="")
			{
				...
				get_php_form_data( theemail.value, "populate_data_from_search_popup", "requires/country_and_order_wise_trims_booking_entry_controller" );
...
function fnc_trims_booking( operation )
{
	...
	var data_all="";
	if (form_validation('cbo_company_name*cbo_buyer_name*cbo_supplier_name*txt_booking_date*cbo_pay_mode','Company Name*Buyer Name*Supplier Name*Booking Date*Pay Mode')==false)
	{
		return;
	}
	else
	{
		 data_all=data_all+get_submitted_data_string('txt_booking_no*txt_job_no*cbo_company_name*cbo_buyer_name*cbo_supplier_name*cbo_currency*txt_exchange_rate*txt_booking_date*txt_delivery_date*cbo_pay_mode*cbo_source*txt_attention*cbo_isshort*txt_remarks*cbo_ready_to_approved*txt_tenor*cbo_item_from_precost*booking_mst_id*delivery_address*cbo_pay_term',"../../")+"&delete_type="+delete_type;;
	}
...
function fnc_trims_booking_dtls( operation )
{
	...
	var booking_qnty_and_amount_trim_group=return_global_ajax_value(txt_po_id+"_"+txt_trim_group_id+"_"+cbo_trim_precost_id, 'booking_qnty_and_amount_trim_group', '', 'requires/country_and_order_wise_trims_booking_entry_controller');
	...
	if(amount_exceed_level==1){
		...
		var booking_amount_and_budget_amount_po_level=return_global_ajax_value(txt_po_id+"_"+txt_trim_group_id+"_"+cbo_trim_precost_id, 'booking_amount_and_budget_amount_po_level', '', 'requires/country_and_order_wise_trims_booking_entry_controller');
	...
	{
		if (form_validation('qty_'+i,'WO Qty')==false )
		{
			return;
		}
		else
...
{
	get_php_form_data( booking_dtls_id, "populate_data_dtls_from", "requires/country_and_order_wise_trims_booking_entry_controller" );
...
if (form_validation('txt_booking_no','Booking No')==false)
	...
	else
	{
		...
		mail_id='';is_mail_send='';
		$report_title=$( "div.form_caption" ).html();
		var data="action="+action+get_submitted_data_string('txt_booking_no*cbo_company_name*id_approved_id*cbo_isshort',"../../")+'&report_title='+$report_title+'&show_comment='+show_comment+'&report_type=1&mail_id='+mail_id+'&is_mail_send='+is_mail_send;
		//freeze_window(5);
		http.open("POST","requires/country_and_order_wise_trims_booking_entry_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
...
/*function popolate_job_data(data)
{
  ...
  get_php_form_data($job_no, "populate_data_from_search_popup", "requires/country_and_order_wise_trims_booking_entry_controller" );
...
function compare_date(str)
{
	...
	var tna_variable_setting = return_global_ajax_value( cbo_company_name, 'tna_variable_setting', '', 'requires/country_and_order_wise_trims_booking_entry_controller');
...
<div style="width:100%;" align="center">
     <? echo load_freeze_divs ("../../",$permission);  ?>
                ...
                <legend title="V3">Trims Booking &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;   <font id="app_sms" style="color:#F00"></font></legend>
                    ...
                    <td  width="170" >
                        <input class="text_boxes" type="text" style="width:160px" onDblClick="openmypage_booking('requires/country_and_order_wise_trims_booking_entry_controller.php?action=trims_booking_popup','Trims Booking Search')" readonly placeholder="Double Click for Booking" name="txt_booking_no" id="txt_booking_no"/>
                    </td>
                    <td align="">
                    ...
                    <tr>
                           ...
                           <td  width="130"  align="right" class="must_entry_caption">Company Name</td>
                                ...
                                <?
								//load_drop_down( 'requires/country_and_order_wise_trims_booking_entry_controller', this.value, 'load_drop_down_supplier', 'supplier_td' );
							  	echo create_drop_down( "cbo_company_name", 172, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected, "get_php_form_data( this.value, 'populate_variable_setting_data', 'requires/country_and_order_wise_trims_booking_entry_controller' ); ","","" );
								?>
                              ...
                              <td class="must_entry_caption" align="right">Pay Mode</td>
                                <td>
                               <?
							   		echo create_drop_down( "cbo_pay_mode", 172, $pay_mode,"", 1, "-- Select Pay Mode --", 1, "load_drop_down( 'requires/country_and_order_wise_trims_booking_entry_controller', this.value, 'load_drop_down_supplier', 'supplier_td' )","" );
							   ?>
                                 </td>
                       ...
                       <td align="right">Exchange Rate</td>
                        ...
                        <tr>
                        	 <td align="right">Delivery To</td>
                   			<td height="10"><textarea id="delivery_address" name="delivery_address" class="text_area" type="text" style="width:160px; height:40px;" placeholder="Delivery Address" ></textarea></td>
			                <td   align="right">Pay Term  </td>
			                <td align="left"  width="110"  >:<?=create_drop_down( "cbo_pay_term", 160, $pay_term,"", 1, "-- Select --", "", "","" ); ; ?></td>
                        ...
                        <tr>
                            ...
                            <!--
                            <input type="button" id="set_button" class="image_uploader" style="width:172px;" value="Terms & Condition/Notes" onClick="open_terms_condition_popup('requires/country_and_order_wise_trims_booking_entry_controller.php?action=terms_condition_popup','Terms and Condition')" />-->
                        ...
                        <tr>
							...
							<input type="button" value=" WG " onClick="generate_trim_report('show_trim_booking_report_wg');"  name="print_booking_wg" id="print_booking_wg"  style="width:100px;display:none;" class="formbutton" />
                            </td>
                        </tr>
                    </table>
              </fieldset>
              ...
              <form name="trimsbooking_2"  autocomplete="off" id="trimsbooking_2">
                ...
                <legend title="V3">Trims Booking Dtls &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; </legend>
                    ...
                    <thead class="form_table_header">
                        ...
                        <tr>
                              <td>
                              <input class="text_boxes" name="txt_po_no" id="txt_po_no" type="text" value=""  style="width:100px"  onDblClick="openmypage('requires/country_and_order_wise_trims_booking_entry_controller.php?action=order_popup','Job/Order Selection Form')" placeholder="Browse" readonly />
                               ...
                               <td>
                              <input name="txt_country_name" id="txt_country_name" onDblClick="open_country_popup('requires/country_and_order_wise_trims_booking_entry_controller.php?action=open_country_popup','Country')" class="text_boxes" type="text"  style="width:100px"  placeholder="Dbl. Click"/>
...
<script>
load_drop_down( 'requires/country_and_order_wise_trims_booking_entry_controller', document.getElementById('cbo_pay_mode').value, 'load_drop_down_supplier', 'supplier_td' )
</script>
