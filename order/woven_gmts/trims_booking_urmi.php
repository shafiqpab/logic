...
function set_precost_data(trim_precost_id,type)
{
   if(type==1)
   {
   reset_form('','','cbo_colorsizesensitive*cbo_uom*txt_quantity*txt_avg_price*txt_amount*txt_update_dtls_id','','','')//txt_delevary_date
   }
	...
	get_php_form_data(trim_precost_id+"_"+txt_po_id+"_"+txt_country+"_"+cbo_gmt_item_id+"_"+cbo_currency+"_"+type, "set_precost_data", "requires/trims_booking_urmi_controller" );
}
...
/*function fnc_process_data(page_link,title)
{
	...
	else
	{
		...
		var page_link=page_link+'&company_id='+cbo_company_name+'&txt_job_no='+txt_job_no+'&cbo_buyer_name='+cbo_buyer_name+'&garments_nature='+garments_nature;
...
function fnc_trims_booking( operation )
{
	...
	{
		 data_all=data_all+get_submitted_data_string('txt_booking_no*txt_job_no*cbo_company_name*cbo_buyer_name*cbo_supplier_name*cbo_currency*txt_exchange_rate*txt_booking_date*txt_delivery_date*cbo_pay_mode*cbo_source*txt_attention*cbo_isshort*txt_remarks*cbo_ready_to_approved',"../../");
	}
...
function fnc_show_booking()
{
	...
	else
	{
		...
		var cbo_buyer_name=document.getElementById('cbo_buyer_name').value;
		var txt_booking_no=document.getElementById('txt_booking_no').value;
		var data='action=show_trim_booking&txt_booking_no='+txt_booking_no+'&cbo_company_name='+cbo_company_name+'&txt_job_no='+txt_job_no+'&cbo_buyer_name='+cbo_buyer_name+'&garments_nature='+garments_nature;
		//freeze_window(5);
		http.open("POST","requires/trims_booking_urmi_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
                    ...
                    <tr>
                           ...
                           <td  width="130"  align="right" class="must_entry_caption">Company Name</td>              
								...
								//load_drop_down( 'requires/trims_booking_urmi_controller', this.value, 'load_drop_down_supplier', 'supplier_td' );
							  	echo create_drop_down( "cbo_company_name", 172, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected, "get_php_form_data( this.value, 'populate_variable_setting_data', 'requires/trims_booking_urmi_controller' ); ","","" );
								?>	
