...
<script>
	...
	function fnc_order_entry_details(operation){
		...
		var data="action=save_update_delete_dtls&operation="+operation+"&row_table="+rowCount+"&reporttype="+reportlevel+get_submitted_data_string('hidden_job_id*cbo_company_name*txt_job_no*total_qty*total_amount',"../../")+data_breakdown;			
		http.open("POST","requires/order_color_size_update_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
