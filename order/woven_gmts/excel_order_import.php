	...
	if (move_uploaded_file($source, $targetzip)) 
	{
		...
		for ($i = 1; $i <= $excel->sheets[0]['numRows']; $i++) 
		{
			...
			{ 
				...
				{
					$all_data=$style_description.'__'.change_date_format($pubshipdate).'__'.change_date_format($shipdate).'__'.$country.'__'.change_date_format($countryshipdate).'__'.$avg_rate.'__'.$excutPer;
        ...
        <script>
			...
			function fnc_excel_import(operation)
			{
				...
				if( form_validation('cbo_company_id*cbo_location_id*cbo_buyer_id*cbo_product_department*cbo_currercy_id*cbo_season_year*cbo_season_id*cbo_prod_catgory*cbo_team_leader*cbo_dealing_merchant*cbo_order_uom*cbo_gmtsItem_id*tot_smv_qty*cbo_ship_mode','Company*Location*Buyer*Prod. Dept.*Currency*Season Year*Season*Prod. Catgory*Team Leader*Dealing Marchant*Order Uom*Gmts Item*SMV*Ship Mode')==false)
				{
					release_freezing();
					return;
				}
				...
				var data="action=save_update_delete&operation="+operation+get_submitted_data_string('cbo_company_id*cbo_location_id*cbo_buyer_id*cbo_style_owner_id*cbo_product_department*cbo_currercy_id*cbo_season_year*cbo_season_id*cbo_brand_id*cbo_prod_catgory*cbo_team_leader*cbo_dealing_merchant*cbo_factory_merchant*cbo_order_uom*cbo_gmtsItem_id*tot_smv_qty*cbo_packing*cbo_ship_mode*txt_sclc*txt_job_no*hid_ws_id',"../../")+dataString;
				//alert(data);
			...
			function fnc_variable_settings_check()
			{
				...
				var all_variable_settings=return_ajax_request_value(company_id, 'load_variable_settings', 'requires/excel_order_import_controller');
				...
				if(set_smv_id==3 || set_smv_id==8)
				{
					...
					{
						var page_link="requires/excel_order_import_controller.php?action=open_smv_list&txt_style_ref="+txt_style_ref+"&set_smv_id="+set_smv_id+"&item_id="+item_id+"&cbo_company_name="+company_id+"&cbo_buyer_name="+cbo_buyer_name;
					}
        ...
        <!-- Important Field outside Form -->  
          ...
          <table width="1450" cellspacing="0" border="1" class="rpt_table" rules="all" id="" >      
              ...
              <tbody>
              	<tr>
                	<td><?=create_drop_down( "cbo_company_id", 100, "select id, company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-Select Company-", $selected, "load_drop_down( 'requires/excel_order_import_controller', this.value, 'load_drop_down_location', 'location_td'); load_drop_down( 'requires/excel_order_import_controller', this.value, 'load_drop_down_buyer', 'buyer_td'); fnc_style_owner(this.value); "); ?></td>
                    ...
                    <td><?=create_drop_down( "cbo_team_leader", 80, "select id, team_leader_name from lib_marketing_team where project_type=2 and status_active =1 and is_deleted=0 order by team_leader_name","id,team_leader_name", 1, "-Select Team-", $selected, "load_drop_down( 'requires/excel_order_import_controller', this.value, 'load_drop_down_dealing_merchant', 'div_marchant'); load_drop_down( 'requires/excel_order_import_controller', this.value, 'load_drop_down_factory_merchant', 'div_marchant_factory')" ); ?></td>
