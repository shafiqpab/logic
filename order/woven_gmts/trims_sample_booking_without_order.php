...
function fnc_fabric_booking( operation )
{
   ...
   if (form_validation('cbo_company_name*cbo_buyer_name*txt_booking_date*cbo_pay_mode','Company Name*Buyer Name*Fabric Nature*Fabric Source*Booking Date*Pay Mode')==false)
	...
	else
	{
		...
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_booking_no*cbo_currency*txt_exchange_rate*cbo_pay_mode*txt_booking_date*cbo_supplier_name*txt_attention*txt_delivery_date*cbo_source*cbo_ready_to_approved*booking_mst_id*cbo_trims_source',"../../");
...
function open_sample_popup()
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, 'Sample Description', 'width=1160px,height=450px,center=1,resize=1,scrolling=0','../');
	emailwindow.onclose=function()
		{
		    ...
		    load_drop_down( 'requires/trims_sample_booking_without_order_controller', style_id.value+'_'+sample_id.value+'_'+$('#cbo_supplier_name').val(), 'load_drop_down_trim_group', 'tgroup_td' );
...
function fnc_desc_popup()
{
	...
	var page_link='requires/trims_sample_booking_without_order_controller.php?action=trim_description_popup&cbo_trim_group='+cbo_trim_group+'&txt_style_id='+txt_style_id+'&cbo_sample_type='+cbo_sample_type+'&txt_booking_no='+txt_booking_no+'&update_id_details='+update_id_details;
	...
	emailwindow.onclose=function()
	{
		...
		//console.log('popup ok='+previous_qty.value);
		var prev_qty=(previous_qty.value)*1;
		var trim_qty_balance=(req_qty.value-prev_qty);
		//alert(sample_id.value)
		//console.log('description='+description.value);
		//console.log('trim_qty_balance='+trim_qty_balance);
		//console.log('uom_id='+uom_id.value);
		...
		document.getElementById('txt_trim_req_qty').value=trim_qty_balance;
...
function fnc_disable()
{
	...
	$('#txt_description').val('');
	document.getElementById('txt_trim_req_qty').value=0;
...
function fnc_fabric_booking_dtls( operation )
{
	...
	if (form_validation('cbo_trim_group*txt_trim_qty*cbo_sample_type*txt_description','Body Part*Finish Fabric*Sample type*Fabric Description')==false)
	{
		return;
	}	

	  var trims_source=$('#cbo_trims_source').val();
	...
	var data="action=save_update_delete_dtls&operation="+operation+get_submitted_data_string('txt_booking_no*cbo_trim_group*cbo_uom*txt_style*txt_style_des*cbo_sample_type*txt_description*txt_barnd_sup_ref*txt_gmt_color*txt_color*txt_gmts_size*txt_size*txt_trim_qty*txt_rate*txt_amount*update_id_details*booking_mst_id',"../../");
	//alert(data);
	//return;
	freeze_window(operation);
	http.open("POST","requires/trims_sample_booking_without_order_controller.php",true);
...
{
	var trim_qty=document.getElementById('txt_trim_qty').value*1;
	var txt_trim_req_qty=document.getElementById('txt_trim_req_qty').value*1;
	...
	if(trim_qty>txt_trim_req_qty)
	{
		alert('Trim Quantity Exceeded Sample Quantity');
		document.getElementById('txt_trim_qty').value=txt_trim_req_qty;
		return;
	}
...
function set_trim_cons_uom(trim_group_id){
	var cbo_cons_uom=return_global_ajax_value(trim_group_id, 'set_cons_uom', '', 'requires/trims_sample_booking_without_order_controller');
  	document.getElementById('cbo_uom').value = trim(cbo_cons_uom);
     ...
     <? echo load_freeze_divs ("../../",$permission);  ?>
                ...
                <legend>Sample Booking (Without Order) </legend>
                    ...
                    <tr>
                                ...
                                <td  width="170" >
                                	<input class="text_boxes" type="text" style="width:160px" onDblClick="openmypage_booking('requires/trims_sample_booking_without_order_controller.php?action=fabric_booking_popup','fabric Booking Search')" readonly placeholder="Double Click for Booking" name="txt_booking_no" id="txt_booking_no"/>
                              ...
                              <td>
                              <? 
							  	echo create_drop_down( "cbo_company_name", 172, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected, "load_drop_down( 'requires/trims_sample_booking_without_order_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/trims_sample_booking_without_order_controller', this.value, 'load_drop_down_supplier', 'supplier_td' );color_from_library( this.value )","","" );
								?>	  
                              </td>
                       ...
                       <td align="right">Delivery Date</td>
                        ...
                        <tr>
							<td align="right">Trims Source</td>
                              <td>
                              <? 
							  	echo create_drop_down( "cbo_trims_source", 172, $fabric_source,"", 1, "-- Select --", 2, "",0 );		
							  ?>	
						...
						<td align="right">Attention</td>   
							...
							?>
                            <!--<input type="button" id="set_button" class="image_uploader" style="width:160px;" value="Terms & Condition/Notes" onClick="open_terms_condition_popup('requires/trims_sample_booking_without_order_controller.php?action=terms_condition_popup','Terms Condition')" />-->
                    ...
                    <tr>
                       ...
                       <td align="right" class="must_entry_caption" >Trims Group</td>
                        	<td id="tgroup_td">
								   ...
								   echo create_drop_down( "cbo_trim_group", 172, $trim_group,"", 1, "--Select--", $selected, "fnc_disable();set_trim_cons_uom(this.value)"); 
								   ?>	
                            </td>
                               <td align="right" width="130" > UOM</td>
                                <td>	
                       ...
                       <td  width="130" align="right" class="must_entry_caption"> Description</td>
                                <td  width="170"> 
							  <input type="text" id="txt_description"    name="txt_description" placeholder="Browse"  class="text_boxes" style="width:160px" onDblClick="fnc_desc_popup()" />
                              </td>
                        ...
                        <td  align="right">Item size</td>
                              ...
                              <td  align="right" class="must_entry_caption">Qty</td>
                                <td>
                                <input type="hidden" name="txt_trim_req_qty" id="txt_trim_req_qty" >
