   ...
   { 
	...
	function fnc_fabric_booking( operation )
	{
		...
		if(date_compare($('#txt_booking_date').val(), delivery_date)==false)
		{
		 ...
		 if (form_validation('cbo_company_name*cbo_buyer_name*cbo_fabric_natu*txt_booking_date*txt_delivery_date*cbo_pay_mode*txt_requisition','Company Name*Buyer Name*Fabric Nature*Booking Date*Delivery Date*Pay Mode*Select Requisition')==false)
		 {
			return;
		 }	
		else
		{
	var data="action=save_update_delete&operation="+operation+"&requisition_id="+requisition_id+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_booking_no*update_id*cbo_fabric_natu*cbo_fabric_source*cbo_currency*txt_exchange_rate*cbo_pay_mode*txt_booking_date*cbo_supplier_name*txt_supplier_name*txt_attention*txt_delivery_date*cbo_source*cbo_ready_to_approved*cbo_team_leader*cbo_dealing_merchant',"../../");
			//alert(data);return;
...
/*	function validate_suplier(){
	...
	function openmypage_supplier()
	{
		...
		var page_link = 'requires/sample_requisition_booking_with_order_controller.php?supplier_id='+supplier_id+'&action=supplier_popup';
        ...
        <fieldset style="width:950px;">
            ...
            <table  width="900" cellspacing="2" cellpadding="0" border="0">
                <tr>
                    ...
                    <td width="170"><input class="text_boxes" type="text" style="width:160px" onDblClick="openmypage_booking('requires/sample_requisition_booking_with_order_controller.php?action=fabric_booking_popup','fabric Booking Search')" readonly placeholder="Double Click for Booking" name="txt_booking_no" id="txt_booking_no"/></td>
                    <td align="right" class="must_entry_caption">Company Name</td>
                    <td><?=create_drop_down( "cbo_company_name", 172, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected, "load_drop_down( 'requires/sample_requisition_booking_with_order_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );color_from_library( this.value );check_kniting_charge();print_button_setting();","","" ); // new condtion ?>	
                ...
                <tr>
                    ...
                    <td><?=create_drop_down( "cbo_fabric_natu", 172, $item_category,"", 1, "-- Select --", 1,$onchange_func, $is_disabled, "3"); ?></td>
                    <td align="right" width="130">Fabric Source</td>
                    <td><?=create_drop_down( "cbo_fabric_source", 172, $fabric_source,"", 1, "-- Select --", "","enable_disable(this.value);fabic_srce_con_fnc()", "", "2,3"); ?></td>
                ...
                <tr>
                    ...
                    <td><?=create_drop_down( "cbo_pay_mode", 172, $pay_mode,"", 1, "-- Select Pay Mode --", 3, "","","1,2,3,5" ); ?></td>
                    <td  width="130" height="" align="right"> Supplier Name </td>              <!-- 11-00030  -->
                ...
                <tr>
                    ...
                    <td align="center" height="10">
						<?=create_drop_down( "cbo_ready_to_approved", 172, $yes_no,"", 1, "-- Select--", 2, "get_php_form_data( this.value+'_'+document.getElementById('txt_booking_no').value, 'check_dtls_part', 'requires/sample_requisition_booking_with_order_controller');","","" ); ?>
                        <input type="hidden" name="is_found_dtls_part" id="is_found_dtls_part" value="1"/>
                    </td>
                </tr>
                <tr>
                    <td align="right" class="">Team Leader</td>   
                    <td><?=create_drop_down( "cbo_team_leader", 172, "select id,team_leader_name from lib_marketing_team where status_active =1 and is_deleted=0 order by team_leader_name","id,team_leader_name", 1, "-- Select Team --", $selected, "load_drop_down( 'requires/sample_requisition_booking_with_order_controller', this.value, 'cbo_dealing_merchant', 'div_marchant' ) " ); ?></td>
                    ...
                    <td align="right"></td>
                    <td><input type="button" id="set_button" class="image_uploader" style="width:160px;" value="Terms & Condition/Notes" onClick="open_terms_condition_popup('requires/sample_requisition_booking_with_order_controller.php?action=terms_condition_popup','Terms Condition')" /></td>
                </tr>
                <tr>
                    <td align="right"></td> 
                    <td><input type="button" class="image_uploader" style="width:192px" value="CLICK TO ADD/VIEW IMAGE" onClick="file_uploader ( '../../', document.getElementById('txt_booking_no').value,'', 'sample_booking_non', 0 ,1)"></td> 
                    <td>&nbsp;</td>
        ...
        <table width="100%" cellpadding="0" cellspacing="2" align="center" >
            <tr>
             <td align="center" valign="top" id="po_list_view">
                ...
                <table cellpadding="0" cellspacing="0" width="1750" class="rpt_table" border="1" rules="all" id="tbl_required_fabric">
                    <thead>
                    ...
                    <tbody id="required_fabric_container">
                        <tr id="tr_1" style="height:10px;" class="general">
                            <td align="center" id="rfPOId_1"><?=create_drop_down( "cboPoId_1", 100, $blank_array,"", 1, "select PO", $selected,"",''); ?></td>
                            <td align="center" id="fabricdescription_id_td"><?=create_drop_down( "cboFabricDescPreCost_1", 120, $blank_array,"", 1, "Select Desc", $selected,"",''); ?></td>
                            ...
                            <td align="center" id="rf_fabric_nature_1"><?=create_drop_down( "cboRfFabricNature_1", 95, $item_category,"", 0, "Select Fabric Nature", 0, "","1","2,3"); ?></td>
                            ...
                            <td align="center" id="rf_dia_1"><input style="width:40px;" type="text" class="text_boxes"  name="txtRfDia_1" id="txtRfDia_1" readonly disabled=""/></td>
                            ...
                            <td align="center" id="rf_width_dia_1"><?=create_drop_down( "cboRfWidthDia_1", 80, $fabric_typee,"", 1, "Select Width/Dia", 0, "",'1'); ?></td>
                            <td align="center" id="rf_uom_1"><?=create_drop_down( "cboRfUom_1", 56, $unit_of_measurement,'', '', "",12,"","1","12,27,1,23" ); ?></td>
                            <td align="center" id="rf_req_dzn_1"><input style="width:50px;" type="text" class="text_boxes_numeric"  name="txtRfReqDzn_1" id="txtRfReqDzn_1" placeholder="write" onBlur="calculate_required_qty('1','1');" readonly disabled="" /></td>
                            
                            <td align="center" id="rfgarmentsCol_1"><?=create_drop_down( "cboGarmentsColor_1", 100, $blank_array,"", 1, " Garments Color", $selected,"",'');	?></td>
                            <td align="center" id="rfFabricColor_1"><?=create_drop_down( "cboFabricColor_1", 100, $blank_array,"", 1, "Fabric Color", $selected,"",'');	?></td>
                            <td align="center" id="rfGarmentSize_1"><?=create_drop_down( "cboGarmentsSize_1", 100, $blank_array,"", 1, "Select Size", $selected,"",''); ?></td>
                            <td align="center" id="rfItemSize_1"><?=create_drop_down( "cboItemSize_1", 100, $blank_array,"", 1, "Item Size", $selected,"",''); ?></td>
                            ...
                            <td align="center" id="rf_req_qnty_1"><input style="width:50px;" type="text" class="text_boxes_numeric"  name="txtRate_1" id="txtRate_1" placeholder=""  /></td>
