...
function openmypage(page_link,title)
{
	...
	emailwindow.onclose=function()
	{
		...
		if (theemail.value!="")
		{
			...
			show_list_view(document.getElementById('txt_job_no').value+'_'+buyer_name,'show_sample_approved_list','accordion_container','requires/sample_approval_controller','');
			release_freezing();
		}
	}
}
	  ...
	  for(var j=i; j<=rowCount; j++)
		{
		  ...
		  if(field_id=='cbostatus_' &&  document.getElementById(field_id+j).disabled ==false )
		  {
...
function fnc_sample_approval_reponse()
{
	...
	if(http.readyState == 4)
	{
		...
		// show_list_view(document.getElementById('txt_job_no').value,'show_sample_approval_list','accordion_container','../woven_order/requires/sample_approval_controller','');
		 	show_list_view(document.getElementById('txt_job_no').value+'_'+document.getElementById('cbo_sample_type').value,'show_sample_approval_list_form','po_list_view','../woven_gmts/requires/sample_approval_controller','');
			show_list_view(document.getElementById('txt_job_no').value+'_'+document.getElementById('cbo_buyer_name').value,'show_sample_approved_list','accordion_container','../woven_gmts/requires/sample_approval_controller','');
...
function load_form (sample_type)
{
	...
	{
		//alert(sample_type)
		show_list_view(document.getElementById('txt_job_no').value+'_'+sample_type,'show_sample_approval_list_form','po_list_view','../woven_gmts/requires/sample_approval_controller','');
	}
...
function resubmit(i)
{
	...
	document.getElementById('cbostatus_'+i).value=1;
                       ...
                       <tr>
                        ...
                        <tr>
                            ...
                            <span id="dropdown_span">
									<?
                                    echo create_drop_down( "cbo_sample_type", 140, "select sample_name,id from lib_sample where is_deleted=0 and status_active=1 and business_nature=3 order by sample_name","id,sample_name", '1', "--Select--", '', "load_form(this.value)",1,'' );
                                    ?>
