...
function fnc_fabric_booking( operation )
{
	...
	{
		var data="action=save_update_delete_mst&operation="+operation+get_submitted_data_string('cbo_company_name*txt_workorder_no*cbo_supplier*txt_workorder_date*cbo_currency*txt_exchange_rate*txt_delivery_date*cbo_pay_mode*cbo_ready_to_approved*txt_attention*txt_address*update_id*txt_vat_per',"../../");
...
function fnc_fabric_booking_dtls_reponse()
{
	if(http.readyState == 4)
	{
		...
		if(trim(reponse[0])=='pi1'){
			alert("PI Number Found :"+trim(reponse[1])+"\n So Update/Delete Not Possible")
			release_freezing();
			return;
		}
		if(reponse[0]==14)
    ...
    <legend>Lab Test Work Order</legend>
        <table  width="1000" cellspacing="2" cellpadding="0" border="0">
            <tr>
                ...
                <td colspan="4">
                <input class="text_boxes" type="text" style="width:160px" onDblClick="openmypage_booking()" readonly placeholder="Double Click for Booking" name="txt_workorder_no" id="txt_workorder_no"/>
            ...
            <tr>
                <td>Address</td>
                <td colspan="3"><input class="text_boxes" type="text" style="width:370px;"  name="txt_address" id="txt_address" /></td>
