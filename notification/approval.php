...
<body>
	<form class="form-inline" id="messageForm">
		<?php 
		$messages = $oracleDb->sql_select( 'SELECT * FROM NOTIFICATION_BREAK_DOWN where IS_VIEWED=0' );
        // echo "<pre>";
			...
			<div class="msg_wrap">
				<div class="msg_body">
					...
					foreach( $messages as $message ):
						?>
						<div class="msg_a"> <strong><?php echo $message['INSERTED_BY']; ?></strong> : <?php echo $message['APP_NOTI_DESC']; ?> </div>
		...
		*/
		...
		$('#msg_input').keypress(function(event){
			...
			if (keycode == '13') {
				...
				if (msg != "") {
                    const dataArray = [
                        { message: "New Approval request found. Quotation ID = 3, Buyer = A.Oliver, Style Ref. = 53.404.75.7412, Quotation Date = 16-06-2023, Est. Ship Date = 16-07-2023, Inserted By = Logic." },
                        { message: "New Approval request found. Quotation ID = 4, Buyer = H and M, Style Ref. = 53.404.75.7412, Quotation Date = 16-06-2023, Est. Ship Date = 30-06-2023, Inserted By = Logic."}
                    ];

                    // Convert the array to a JSON string
                    const jsonData = JSON.stringify(dataArray);
					socket.emit('message', { menu_id: 1031,user_id:1,notification_user:4, app_noti_desc: jsonData }, function() {
