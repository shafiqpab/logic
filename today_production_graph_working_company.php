...
/*if( $location!=0 )
{
	$str_location=" and c.location_name=$location";
	$location_cond="and location_name=$location";
}
else 
{
	$str_location="";
...
?>	
	...
	<div class="wrap" style="margin-left:15px; margin-top:10px">
		...
		$lineDataArr=sql_select( "select id, line_name, company_name, location_name from lib_sewing_line where company_name in ($manufacturing_company) and status_active=1 and is_deleted=0");
		...
		$sql.=" from pro_garments_production_mst where production_type in(5,11) and serving_company in($manufacturing_company) and production_date='$today' and is_deleted=0 and status_active=1 group by sewing_line";
			...
			?>
				<div style="width:469px; height:240px; float:left; position:relative; margin-left:10px; margin-top:5px; border:solid 1px">
					<table style="margin-left:50px; font-size:12px;">
						<tr>
							<td colspan="9"><b><? echo "Company : ".$comp_arr[$company_id_arr[$line_id_for_loc]].";";// $comp_name; ?> &nbsp; <? echo $location_name; ?></b><br />Today Hourly Production Line No- <? echo $sewing_line; ?></td>
						</tr>
						<tr>
							<td bgcolor="#FF3300" width="10"></td>
							<td>Target</td>
							...
							<td>Poly</td>
							<td>
							<a href="##" style="text-decoration:none"><input type="button" value="Details" name="a" id="a" class="formbutton" style="width:60px" onclick="show_details(<? echo $line_id; ?>,'<? echo $sewing_line; ?>');"/></a></td>
						</tr>
					</table>
