		...
		?>
			<script>
				...
				function search_populate(str)
				{
					...
					{
						load_drop_down( 'emb_delivery_and_received_status_controller',document.getElementById('company_search_by').value,'load_drop_down_buyer', 'search_by_td' );
...
{ 
	...
	/ 										main query										/
	/ ===================================================================================  */
	 $sql = "SELECT a.company_name,a.buyer_name,order_uom as uom,b.id as po_id,b.po_number,c.item_number_id,c.color_number_id,d.embel_name,d.production_type,d.challan_no,d.serving_company,d.production_date,d.remarks,e.production_qnty,e.reject_qty
	...
	foreach($sql_res as $row ){
		$data_array[$row[csf('production_type')]][$row[csf('serving_company')]][$row[csf('po_id')]][$row[csf('color_number_id')]][$row[csf('challan_no')]][$row[csf('production_date')]]['buyer_name']= $row[csf('buyer_name')];

		$data_array[$row[csf('production_type')]][$row[csf('serving_company')]][$row[csf('po_id')]][$row[csf('color_number_id')]][$row[csf('challan_no')]][$row[csf('production_date')]]['company_name']= $row[csf('company_name')];

		

		$data_array[$row[csf('production_type')]][$row[csf('serving_company')]][$row[csf('po_id')]][$row[csf('color_number_id')]][$row[csf('challan_no')]][$row[csf('production_date')]]['po_number']= $row[csf('po_number')];

		$data_array[$row[csf('production_type')]][$row[csf('serving_company')]][$row[csf('po_id')]][$row[csf('color_number_id')]][$row[csf('challan_no')]][$row[csf('production_date')]]['item_number_id']= $row[csf('item_number_id')];


		$data_array[$row[csf('production_type')]][$row[csf('serving_company')]][$row[csf('po_id')]][$row[csf('color_number_id')]][$row[csf('challan_no')]][$row[csf('production_date')]]['embel_name']= $row[csf('embel_name')];

		$data_array[$row[csf('production_type')]][$row[csf('serving_company')]][$row[csf('po_id')]][$row[csf('color_number_id')]][$row[csf('challan_no')]][$row[csf('production_date')]]['uom']= $row[csf('uom')];

		$data_array[$row[csf('production_type')]][$row[csf('serving_company')]][$row[csf('po_id')]][$row[csf('color_number_id')]][$row[csf('challan_no')]][$row[csf('production_date')]]['production_qnty']+= $row[csf('production_qnty')];

		$data_array[$row[csf('production_type')]][$row[csf('serving_company')]][$row[csf('po_id')]][$row[csf('color_number_id')]][$row[csf('challan_no')]][$row[csf('production_date')]]['reject_qty']+= $row[csf('reject_qty')];

		$data_array[$row[csf('production_type')]][$row[csf('serving_company')]][$row[csf('po_id')]][$row[csf('color_number_id')]][$row[csf('challan_no')]][$row[csf('production_date')]]['remarks']= $row[csf('remarks')];
	...
	?>
		...
		<div align="center"style="width:1100px;; margin: 0 auto">
			...
			<tr class="form_caption">
				...
				<tr class="form_caption">
					<td  align="center" style="font-size:20px">Cutting Part Print Delivery Summery</td>
				</tr>
					...
					foreach ($data_array[2] as $party_key => $party_data) 
					{
						foreach ($party_data as $po_id => $po_data) 
						{
							foreach ($po_data as $color_key => $color_data) 
							{
								...
								{
								   foreach ($challan_data as $prod_date => $rows ) 
								    {
										...
										<tr bgcolor="<? echo $bgcolor;?>" id="tr_<?= $i;?>" onClick="change_color('tr_<?= $i; ?>','<?= $bgcolor; ?>')">
												...
												<td width="100"><? echo $company_library[$party_key];?></td>
												<td width="100"><? echo $row[csf('po_number')];?></td>
												<td width="100"><? echo $buyer_library[$row[csf('buyer_name')]];?></td>
												<td width="80"><? echo $color_library[ $color_key];?></td>
												<td width="80" align="center"><? echo $prod_date;?></td>
												<td width="60" align="center"><? echo $challan_no;?></td>
												<td width="60"><?echo $garments_item[$row[csf('item_number_id')]];?></td>
												<td width="40"><?echo $unit_of_measurement[$rows['uom']]?></td>
												<td width="60" align="right"><? echo $rows['production_qnty'];?></td>
												<td width="80"><?echo $rows[csf('remarks')]?></td>
											
										</tr>
									
											<?
											$total_ok_goods+=$rows['production_qnty'];
      ...
      <!-- ==================================== 1st part end and 2nd part start ========================= -->
		<div>
			...
			<tr class="form_caption">
				...
				<tr class="form_caption">
					<td  align="center" style="font-size:20px">Cutting Part Print Received Summery</td>
				</tr>
									...
									foreach ($challan_data as $prod_date => $rows ) 
								    {
									  $bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF";	
											...
											<tr bgcolor="<? echo $bgcolor;?>" id="tr_<?= $i;?>" onClick="change_color('tr_<?= $i; ?>','<?= $bgcolor; ?>')">	
													...
													<td width="40"><? echo $unit_of_measurement[$rows['uom']];?></td>
													<td width="60" align="right"><? echo $rows['production_qnty'];?></td>
													<td width="60" align="right"><? echo $rows['reject_qty'] ;?></td>
													<td width="60" align="right"><? echo $rows['production_qnty'];?></td>
												...
												<?
														...
														$total_ok_goods+=$rows['production_qnty'];
														$total_rej_goods+=$rows['reject_qty'] ;
