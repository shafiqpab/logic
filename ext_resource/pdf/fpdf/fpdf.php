...
function FPDF($orientation='P',$unit='mm',$format='A4')
{
	...
	$this->CoreFonts=array('courier'=>'Courier','courierB'=>'Courier-Bold','courierI'=>'Courier-Oblique','courierBI'=>'Courier-BoldOblique',
		...
		'symbol'=>'Symbol','zapfdingbats'=>'ZapfDingbats','calibri'=>'calibri');
	//Scale factor
	if($unit=='pt')	$this->k=1;
	...
	if(is_string($format))
	{
		...
		elseif($format=='a6')	$format=array(280.94,170.28);
		elseif($format=='a8')	$format=array(200.94,170.28);
		elseif($format=='a9')	$format=array(170.80,105.20);
		elseif($format=='a10')	$format=array(170.80,95.20);
		elseif($format=='a11')	$format=array(200,125);
		elseif($format=='letter')	$format=array(612,792);
		elseif($format=='legal') $format=array(612,1008);
		elseif($format=='a128')	$format=array(170,150);
...
function Write($h,$txt,$currentx=0,$link='') //EDITEI
{
	...
	$wmax=($w-2*$this->cMargin)*1500/$this->FontSize;
	$s=str_replace("\r",'',$txt);
	$nb=strlen($s);
	$sep=-1;
	$i=0;
	...
	while($i<$nb)
	{
		...
		if($c=="\n")
		{
			...
			if($nl==1)
			{
				...
				$w=$this->w-$this->rMargin-$this->x;
				$wmax=($w-2*$this->cMargin)*500/$this->FontSize;
			}
			$nl++;
			continue;
		}
...
//changed by REZA
function pdf_image($file,$x,$y,$w=0,$h=0,$type='',$link='',$paint=true)
{
