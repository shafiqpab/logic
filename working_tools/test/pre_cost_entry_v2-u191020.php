	...
	function show_sub_form(update_id, action, extra_str)
	{
		...
		else
		{
			...
			if(action=="partial_pre_cost_copy_action")
			{
				...
				page_link=page_link + "&txt_job_no="+txt_job_no+ "&hidd_job_id="+hidd_job_id;
...
// Yarn Cost-------------------------------------------
	...
	function fnc_fabric_yarn_cost_dtls( operation )
	{
		...
		for (var i=1; i<=row_num; i++)
		{
			...
			data_all=data_all+get_submitted_data_string('cbo_company_name*hidd_job_id*update_id*txt_quotation_id*copy_quatation_id*cbocount_'+i+'*cbocompone_'+i+'*percentone_'+i+'*color_'+i+'*cbotype_'+i+'*consqnty_'+i+'*txtrateyarn_'+i+'*avgconsqnty_'+i+'*txtamountyarn_'+i+'*supplier_'+i+'*cbostatusyarn_'+i+'*updateidyarncost_'+i+'*txtconsumptionyarn_sum*txtamountyarn_sum',"../../");
		}
...
// Trim Cost-----------------------------------------------------
	...
	function open_consumption_popup_trim(page_link,title,trorder)
	{
		...
		emailwindow.onclose=function()
		{
			...
			document.getElementById('txtconsdzngmts_'+trorder).value=number_format_common(set_avg_totcons, 5, 0);;
...
// Embellisment Cost-------------------------------------------------
	...
	function fnc_commission_cost_dtls( operation )
	{
		...
		for (var i=1; i<=row_num; i++)
		{
			...
			data_all=data_all+get_submitted_data_string('cbo_company_name*hidd_job_id*update_id*copy_quatation_id*txt_quotation_id*cboparticulars_'+i+'*cbocommissionbase_'+i+'*txtcommissionrate_'+i+'*txtcommissionamount_'+i+'*cbocommissionstatus_'+i+'*commissionupdateid_'+i+'*txtratecommission_sum*txtamountcommission_sum',"../../");
		}
...
// Dtls Form 1--------------------------------------------------------
		...
		else
		{
			...
			else
			{
				...
				if(type=='mo_sheet')
				{
					...
					//excess_val = isInteger(excess_percent);
					/*if(excess_per_val==""){
					alert("You have to enter an Excess %");
					release_freezing();
					return;
					}
					if(excess_per_val==""){
	...
	function fnc_budgete_cost_validate(type)
	{
		...
		var fab_cons_kg=0; var fab_cons_mtr=0; var fab_cons_yds=0; var fab_amount=0; var sp_oparation_amount=0; var acc_amount=0; var fright_amount=0; var lab_amount=0; var misce_amount=0; var other_amount=0; var comm_amount=0; var fob_amount=0; var cm_amount=0; var rmg_ratio=0; var is_approved=0;
		...
		if(trim(cost_control_source_data)==1)
		{
			if(costing_id!=0)
			{
        ...
        <form name="precosting_1" id="precosting_1" autocomplete="off" enctype="multipart/form-data">
            ...
            <table  width="1070" cellspacing="2" cellpadding=""  border="0">
                ...
                <tr>
                    <td>Remarks</td>
                    <td colspan="5">
                    <input class="text_boxes" type="text" style="width:615px;" name="txt_remarks" id="txt_remarks" maxlength="500" title="Maximum 500 Character" placeholder="Remarks"/></td>
                    <td>Images</td>
                ...
                <tr>
                    ...
                    <td>&nbsp;</td><!--Efficiency Wastage%-->
                    <td><input class="text_boxes_numeric" type="hidden" style="width:110px;" name="txt_efficiency_wastage" id="txt_efficiency_wastage" onChange="calculate_cm_cost_with_method()" readonly /></td>
                    <td>&nbsp;</td>
                    <td><input type="button" class="image_uploader" style="width:120px" value="Copy Without Consumption" onClick="show_sub_form(document.getElementById('update_id').value,'partial_pre_cost_copy_action','');" />
                    </td>
                </tr>
                <tr>
                    <td>File no</td>
