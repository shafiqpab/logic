<?
	...
	$duplicate_sql ="select size_name, count(id) as tot_row, listagg(cast(id as varchar(4000)),',') within group (order by id) as ids_all from LIB_SIZE
	where status_active=1 and is_deleted=0 and size_name is not null
	group by size_name
	having count(id) > 1
	...
	foreach($duplicate_sql_result as $row)
	{
		...
		$up_script="update LIB_SIZE set status_active=7, is_deleted=8 where id in(".implode(",",$ids_all_arr).") and id <> $replace_id ";
		//echo $up_script;die;
	...
	if($db_type==2)
	{
		...
		{
			oci_commit($con); 
			echo " Update Successful.=$tt <br>";die;
		}
		else
		{
			oci_rollback($con);
