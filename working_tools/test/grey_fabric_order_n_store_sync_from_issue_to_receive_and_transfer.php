...
$barcode_nos = " 20020103744, 20020111796,20020215500, 19021203262, 19021203051, 19021202944, 19021201859, 19021201305, 19021201052 ";
...
foreach ($barcode_data as $row)
{
	if($splited_data_arr[$row[csf("barcode_no")]] == "")
	{
		...
		else if($row[csf("entry_form")] == 82 || $row[csf("entry_form")] == 83 || $row[csf("entry_form")] == 110 || $row[csf("entry_form")] == 180 || $row[csf("entry_form")] == 183 || $row[csf("entry_form")] == 133)
		{
			...
			$trans_dtls_id_wise_barcode_arr[$row[csf("dtls_id")]] = $row[csf("barcode_no")];
		}
	}
}
...
if($db_type==2 && count($trans_dtls_arr)>999)
{
	...
	{
		$chunk_arr_value=implode(",",$chunk_arr);
		$transCond.="  b.id in($chunk_arr_value) or ";
	}
...
{
	$all_trans_dtls_id_cond=" and b.id in($all_trans_dtls_ids)";
}
...
$transfer_sql_arr = sql_select("select a.transfer_criteria,a.company_id,a.to_company, b.id, b.trans_id, b.to_trans_id, b.from_store, b.to_store, b.from_order_id, b.to_order_id from inv_item_transfer_dtls b,inv_item_transfer_mst a  where a.id=b.mst_id and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0 and b.item_category = 13 $all_trans_dtls_id_cond");

foreach ($transfer_sql_arr as $val)
{
	if($val[csf("company_id")]!= $val[csf("to_company")])
	{
		$company_to_company_barcode_arr[$trans_dtls_id_wise_barcode_arr[$val[csf("id")]]]=$trans_dtls_id_wise_barcode_arr[$val[csf("id")]];
	}
...
foreach ($barcode_data as $row)
{
	if($non_ord_ref_data[$row[csf("barcode_no")]] == "")
	{
		if($splited_data_arr[$row[csf("barcode_no")]] == "")
		{
			if($company_to_company_barcode_arr[$row[csf("barcode_no")]] == "")
			{
			...
			{
				$company_to_company_barcode_arr_display[$row[csf("barcode_no")]] =$row[csf("barcode_no")];
			}
		}
		else
		{
...
if(!empty($company_to_company_barcode_arr_display))
{
	echo "Company to Company transfer barcodes not updated here :".implode(",", $company_to_company_barcode_arr_display);
	oci_rollback($con);
	disconnect($con);
	die;
}
