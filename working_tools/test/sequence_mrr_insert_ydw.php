...
$entry_form_wise_mrr_tble=array(41=>"Yarn Dying With Order",42=>"Yarn Dying Without Order",114=>"Yarn Dying Without Order 2",125 =>"Yarn Dying Work Order Without Lot",94=>"Yarn Service Work Order");
...
foreach($sql_company as $row)
{
	foreach($entry_form_wise_mrr_tble as $entry_form=>$table_name)
	{
		if($entry_form==502)
		{
			$mrr_prefix_sql=sql_select("select max(barcode_suffix_no) as max_prefix_num from pro_roll_details where status_active=1  and entry_form=2 ");
			$max_mrr_prefix_num=$mrr_prefix_sql[0][csf("max_prefix_num")];
			if($max_mrr_prefix_num<1) $max_mrr_prefix_num=1;
			...
			if($roll_check[$table_name]=="")
			{
				...
				if($db_type==0)
				{
					...
					$data_arr.="('".strtoupper($table_name)."','".$max_mrr_prefix_num."',0,2,'2017',0,0,0,0,0)";
				}
				else
				{
					$data_arr[]="('".strtoupper($table_name)."','".$max_mrr_prefix_num."',0,2,'2017',0,0,0,0,0)";
		...
		else
		{
			...
			$mrr_prefix_sql=sql_select("select max(".$mrr_table_wise_prefix_num[$table_name].") as max_prefix_num from $table_name where status_active=1 and company_id=".$row[csf("id")]." $entry_form_cond ");
			$max_mrr_prefix_num=$mrr_prefix_sql[0][csf("max_prefix_num")];
			if($max_mrr_prefix_num<1) $max_mrr_prefix_num=1;
			...
			{
				if($data_arr!="") $data_arr.=",";
				$data_arr.="('".strtoupper($table_name)."','".$max_mrr_prefix_num."','".$row[csf("id")]."','".$entry_form."','2017',0,0,0,0,0)";
			}
			else
			{
				$data_arr[]="('".strtoupper($table_name)."','".$max_mrr_prefix_num."','".$row[csf("id")]."','".$entry_form."','2017',0,0,0,0,0)";
