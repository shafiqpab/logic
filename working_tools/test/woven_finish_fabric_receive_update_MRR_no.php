...
$blank_challan_barcode = sql_select("select id,issue_number_prefix,issue_number from inv_issue_master  where entry_form = 19 and is_deleted = 0 and status_active = 1");

foreach ($blank_challan_barcode as $val) 
{
	$barcode_no_arr[$val[csf("id")]] = $val[csf("id")];
}
...
$ref_sql_from_issue = sql_select("select id,issue_number_prefix,issue_number from inv_issue_master  where entry_form = 19 and is_deleted = 0 and status_active = 1 $barcode_no_cond");
...
foreach ($ref_sql_from_issue as $row) 
{
	...
	$issue_number=explode('-', $row[csf("issue_number")]);
...
{	
	...
	$data_array_roll_deleted[$val[csf("id")]]=explode("*",("'".$ref_data_arr[$val[csf("id")]]["issue_number_prefix"]."'*'".$ref_data_arr[$row[csf("id")]]["issue_number"]."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
	...
	if($flag==1)
	{
		...
		execute_query("update inv_issue_master set issue_number_prefix='$prefix',issue_number ='$issue_number',updated_by=9 where id=$id",0);
	}
}
