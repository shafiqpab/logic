...
$sql_issue="select b.prod_id as PROD_ID, b.id as TRANS_ID, b.cons_quantity as CONS_QUANTITY, b.cons_rate as CONS_RATE, b.transaction_type as TRANSACTION_TYPE, a.entry_form as ENTRY_FORM   
from inv_transaction b, inv_issue_master a 
where b.mst_id=a.id and a.entry_form in(5,28,250,298,308,392) and b.item_category in(5,6,7,23) and b.transaction_type in(2,3) and b.status_active=1 and b.is_deleted=0 
union all
select b.prod_id as PROD_ID, b.id as TRANS_ID, b.cons_quantity as CONS_QUANTITY, b.cons_rate as CONS_RATE, b.transaction_type as TRANSACTION_TYPE, a.entry_form as ENTRY_FORM    
from inv_transaction b, inv_item_transfer_mst a 
where b.mst_id=a.id and a.entry_form in(55) and b.item_category in(5,6,7,23) and b.transaction_type in(6) and b.status_active=1 and b.is_deleted=0 
order by prod_id, pi_wo_batch_no, trans_id";
//echo $sql_issue;die;
$result_issue=sql_select($sql_issue);
foreach($result_issue as $row)
{
	$issue_data_arr[$row["PROD_ID"]][$row["TRANS_ID"]]["cons_quantity"]=$row["CONS_QUANTITY"];
	$issue_data_arr[$row["PROD_ID"]][$row["TRANS_ID"]]["transaction_type"]=$row["TRANSACTION_TYPE"];
	$issue_data_arr[$row["PROD_ID"]][$row["TRANS_ID"]]["entry_form"]=$row["ENTRY_FORM"];
}
unset($result_issue);

$sql_receive="select b.id as TRANS_ID, b.prod_id as PROD_ID, b.cons_quantity as CONS_QUANTITY, b.cons_rate as CONS_RATE, b.transaction_type as TRANSACTION_TYPE from inv_transaction b where b.item_category in(5,6,7,23) and b.transaction_type in(1,4,5) and b.status_active=1 and b.is_deleted=0
order by trans_id";
...
{
	$rcv_data_arr[$row["PROD_ID"]][$row["TRANS_ID"]]["cons_quantity"]=$row["CONS_QUANTITY"];
	$rcv_data_arr[$row["PROD_ID"]][$row["TRANS_ID"]]["cons_rate"]=$row["CONS_RATE"];
	$rcv_data_arr[$row["PROD_ID"]][$row["TRANS_ID"]]["transaction_type"]=$row["TRANSACTION_TYPE"];
}
