...
$grey_receive_sql= sql_select("select c.barcode_no, z.store_id from inv_transaction z,pro_grey_prod_entry_dtls b, pro_roll_details c where z.id=b.trans_id and b.id=c.dtls_id and c.entry_form in (2,22,58) and c.status_active=1 and c.is_deleted=0 and z.status_active=1 and z.is_deleted=0 and z.transaction_type=1 and b.status_active=1 and c.status_active=1 $all_barcode_cond");// and c.re_transfer=0
...
/*$grey_trans_sql= sql_select("select c.barcode_no, b.to_store from inv_item_transfer_mst a, inv_item_transfer_dtls b, pro_roll_details c where a.id=b.mst_id and b.id=c.dtls_id and a.entry_form in(83) and c.entry_form in(83) and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and b.status_active=1 and c.status_active=1 and c.re_transfer=0 $all_barcode_cond");

foreach ($grey_trans_sql as $val)
{
	$store_arr[$val[csf("barcode_no")]] = $val[csf("to_store")];
}

$grey_trans_2_sql= sql_select("select c.barcode_no, b.to_store from inv_item_transfer_mst a, inv_item_transfer_dtls b, pro_roll_details c where a.id=b.mst_id and b.id=c.dtls_id and a.entry_form in(82) and c.entry_form in(82) and c.status_active=1 and c.is_deleted=0 and c.booking_without_order =0 and a.status_active=1 and b.status_active=1 and c.status_active=1 and c.re_transfer=0 $all_barcode_cond");

foreach ($grey_trans_2_sql as $val)
{
	$store_arr[$val[csf("barcode_no")]] = $val[csf("to_store")];
}*/
