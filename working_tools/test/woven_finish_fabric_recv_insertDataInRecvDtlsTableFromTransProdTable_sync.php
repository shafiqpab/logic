...
$issue_return_trans_sql = sql_select("select a.id,b.id trans_id,b.pi_wo_batch_no batch_ids, b.prod_id,b.cons_quantity,b.fabric_shade,b.store_id,b.no_of_roll,b.body_part_id,b.rack,b.self,b.floor_id,b.room,b.booking_no,b.batch_id,c.unit_of_measure,c.detarmination_id,c.color,c.gsm,c.dia_width
	from inv_receive_master a,inv_transaction b,product_details_master c
	where a.id=b.mst_id and b.prod_id=c.id and 
	a.entry_form=17 and a.status_active=1 and a.is_deleted=0 
	and b.item_category=3
	and b.transaction_type=1 
	and b.status_active=1 and b.is_deleted=0 
	and c.item_category_id=3 
...
$sql_ordWiseProf=sql_select("select a.trans_id,a.po_breakdown_id from order_wise_pro_details a,inv_transaction b where b.id=a.trans_id and b.transaction_type=1 and a.trans_type=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form=17");
...
foreach($issue_return_trans_sql  as $val)
{
	$mst_id   = $val[csf("id")];
	...
	if($rID){
		...
		oci_rollback($con);
		echo "insert into pro_finish_fabric_rcv_dtls (".$field_details_array.") values ".$data_array_dtls;
		die;
	}
}
