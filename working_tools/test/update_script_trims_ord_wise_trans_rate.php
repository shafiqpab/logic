...
$data_array=sql_select("select b.ID, a.CONVERSION_FACTOR from lib_item_group a, product_details_master b where a.id=b.item_group_id and a.item_category=4 and b.item_category_id=4 and b.entry_form=24 and b.status_active=1 and b.company_id=3");
...
$rcv_issue_rtn_sql="select TRANS_ID, ORDER_ID from INV_TRIMS_ENTRY_DTLS where status_active=1";
...
$issue_rcv_rtn_sql="select TRANS_ID, ORDER_ID from INV_TRIMS_ISSUE_DTLS where status_active=1 ";
...
$sql_order_trans="select b.PROD_ID, b.ID as TRANS_ID, b.CONS_QUANTITY, b.CONS_AMOUNT, b.TRANSACTION_TYPE, a.ID as PROP_ID, a.PO_BREAKDOWN_ID, a.QUANTITY, a.ORDER_RATE, a.ORDER_AMOUNT 
...
and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.company_id=3 and c.company_id=3
	...
	if($row["TRANSACTION_TYPE"]==1 || $row["TRANSACTION_TYPE"]==4 || $row["TRANSACTION_TYPE"]==5)
	{
		if($trans_id_check[$row["TRANS_ID"]]=="")
		{
			...
			if($row["TRANSACTION_TYPE"]==4)
			{
				...
				{
					$runtime_qnty+=$prod_ord_data[$row["PROD_ID"]][$ord_id]["qnty"];
					$runtime_amt+=$prod_ord_data[$row["PROD_ID"]][$ord_id]["amt"];
				}
				...
				$prod_ord_data[$row["PROD_ID"]][$row["PO_BREAKDOWN_ID"]]["qnty"]+=$row["QUANTITY"]*$conversion_factor_arr[$row["PROD_ID"]];
				$prod_ord_data[$row["PROD_ID"]][$row["PO_BREAKDOWN_ID"]]["amt"]+=(($row["QUANTITY"]*$conversion_factor_arr[$row["PROD_ID"]])*$runtime_rate);
			}
			else
			{
				$rcv_data[$row["PROD_ID"]]["qnty"]+=$row["CONS_QUANTITY"];
				...
				$prod_ord_data[$row["PROD_ID"]][$row["PO_BREAKDOWN_ID"]]["qnty"]+=$row["QUANTITY"]*$conversion_factor_arr[$row["PROD_ID"]];
				$prod_ord_data[$row["PROD_ID"]][$row["PO_BREAKDOWN_ID"]]["amt"]+=(($row["QUANTITY"]*$conversion_factor_arr[$row["PROD_ID"]])*$cons_rate);
			}
		}
		else
		{
