...
//$issue_sql = "select a.id as issue_id,a.issue_number,a.issue_basis,a.issue_purpose,b.id as trans_id,b.prod_id,b.requisition_no,sum(b.cons_quantity) as issue_qty,sum(c.quantity) as propotion_qty from inv_issue_master a , inv_transaction b, order_wise_pro_details c where a.id=b.mst_id and b.id=c.trans_id and a.entry_form = 3  and a.issue_basis=3 and b.item_category=1 and b.transaction_type=2 and c.trans_type=2  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.item_category=1 and c.entry_form=3 and a.issue_number in('MKL-YIS-19-04460','MKL-YIS-19-04414') group by a.id,a.issue_number,a.issue_basis,a.issue_purpose,b.id,b.prod_id, b.requisition_no having(sum(b.cons_quantity)>sum(c.quantity)) order by a.id desc";//'MKL-YIS-19-04460','MKL-YIS-19-04414'  // 'AOPL-YIS-19-06550' // and b.requisition_no = 38245

$issue_sql = "select a.id as issue_id,a.issue_number,a.issue_basis,a.issue_purpose,b.id as trans_id,b.prod_id,b.requisition_no,b.cons_quantity as issue_qty,sum(c.quantity) as propotion_qty from inv_issue_master a , inv_transaction b, order_wise_pro_details c where a.id=b.mst_id and b.id=c.trans_id and a.entry_form = 3  and a.issue_basis=3 and a.issue_purpose=1 and b.item_category=1 and b.transaction_type=2 and c.trans_type=2  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.item_category=1 and c.entry_form=3 group by a.id,b.id,a.issue_number,a.issue_basis,a.issue_purpose,b.prod_id, b.requisition_no,b.cons_quantity having(b.cons_quantity>round(sum(c.quantity),2) and b.cons_quantity!=sum(c.quantity) ) order by a.id desc";//and a.issue_number in('AOPL-YIS-19-06475')

//die();
//--and a.issue_number in('AOPL-YIS-19-06550')
...
from ppl_yarn_requisition_entry b where b.is_deleted=0 and b.is_deleted=0 $req_id_cond $prod_id_cond group by b.requisition_no,b.prod_id";
...
$sql_plan_po = "select c.requisition_no,booking_no,po_id,b.is_sales from ppl_planning_entry_plan_dtls a, ppl_planning_info_entry_dtls b, ppl_yarn_requisition_entry c where b.id=a.dtls_id and b.id=c.knit_id $req_id_cond group by c.requisition_no,booking_no,po_id,b.is_sales order by po_id";
	...
	foreach ($req_po_data as $po_id)
	{
		...
		if($check_order_data=="")
		{
			...
			if($order_qnty>0)
			{
				...
				//$id_proport = 0;
				$data_array_prop .= "(" . $id_proport . "," . $transactionID . ",2,3," . $order_id . "," . $prodId . ",'" . $order_qnty . "'," . $issue_purpose . ",'" . $returnable_qnty . "','" . $is_salesOrder . "',9999,'" . $pc_date_time . "')";
			}
		}
