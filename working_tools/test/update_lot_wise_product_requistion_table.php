...
$sql_trans="Select a.ID, a.COMPANY_ID, a.ITEM_GROUP_ID, a.ITEM_DESCRIPTION, b.ID as TRANS_ID, b.ITEM_LOT as BATCH_LOT
 from product_details_master a, dyes_chem_issue_requ_dtls b where a.id=b.product_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.item_category_id in(5,6,7,23) ";
//echo $sql_trans;die;
...
foreach($sql_trans_result as $row)
{
	...
	if($prod_id)
	{
		$upTrans=execute_query("update dyes_chem_issue_requ_dtls set product_id='$prod_id' where id='".$row["TRANS_ID"]."'");
		if($upTrans){ $upTrans=1; } else { echo "update dyes_chem_issue_requ_dtls set product_id='$prod_id' where id='".$row["TRANS_ID"]."'"; oci_rollback($con);disconnect($con);die;}
	}
}
...
if($db_type==2)
{
	...
	{
		oci_commit($con); 
		echo "Requisition Data Update Successfully. <br>";die;
	}
	else
	{
		oci_rollback($con);
		echo "Requisition Data Update Failed";
		die;
	}
}
?>