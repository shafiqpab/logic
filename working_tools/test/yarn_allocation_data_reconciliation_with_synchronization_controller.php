...
if ($action=="synchronize_allocation")
{
    ...
    $company_id=17;
	$product_id = "63101";//"63101,57350,57356,57358,57359"
	...
	if(!empty($result_allocation))
	{
		...
		foreach ($result_allocation as $row) 
		{
			...
			$requisitions = array_unique($issue_requisiton_no_arr);
			$requisition_nos = "";
			...
			$powise_rqsn_qty = 0;
			$booking_rqsn_qty = 0;
			$po_rqsn_wise_issue_qty = 0;
			$booking_po_rqsn_wise_issue_qty=0;
			foreach ($requisitions as $req_no) 
			{
				$powise_rqsn_qty += $plan_requisition_data_array[$booking_no][$row[csf("po_break_down_id")]][$req_no][$prod_id]['po_wise'];
				$booking_rqsn_qty += $plan_requisition_data_array[$booking_no][0][$req_no][$prod_id]['booking_wise'];

				$po_rqsn_wise_issue_qty +=$requisition_wise_issue_array[$booking_no][$row[csf("po_break_down_id")]][$req_no][$prod_id];

				$booking_po_rqsn_wise_issue_qty += $booking_requisition_wise_issue_array[$booking_no][$req_no][$prod_id];
			}
			...
			$dtls_data_update_arr[$dtls_id]["alcation_qnty"] += $row[csf("allocate_qty")];
			$dtls_data_update_arr[$dtls_id]["issue_qty"] = $issue_qty_new;
			$dtls_data_update_arr[$dtls_id]["return_qty"] = $return_qty_new;
			$dtls_data_update_arr[$dtls_id]["net_issue_qty"] = ($issue_qty_new-$return_qty_new);

			$dtls_data_update_arr[$dtls_id]["booking_wise_requisition_qty"] = $booking_rqsn_qty;
			$dtls_data_update_arr[$dtls_id]["booking_wise_requisition_issue_qty"] = $booking_po_rqsn_wise_issue_qty;
			$dtls_data_update_arr[$dtls_id]["booking_wise_requisition_issue_rtn_qty"] = $booking_requsition_wise_issure_rtn_qty[$booking_no][$prod_id];
			$dtls_data_update_arr[$dtls_id]["booking_wise_requisition_net_qty"] = ($booking_po_rqsn_wise_issue_qty-$booking_requsition_wise_issure_rtn_qty[$booking_no][$prod_id]);			
			
			$dtls_data_update_arr[$dtls_id]["powise_requisition_qty"] = $powise_rqsn_qty;
			$dtls_data_update_arr[$dtls_id]["powise_requisition_issue_qty"] = $po_rqsn_wise_issue_qty;	
			$dtls_data_update_arr[$dtls_id]["powise_requisition_issue_rtn_qty"] = $requsition_wise_issure_rtn_qty[$booking_no][$row[csf('po_break_down_id')]][$prod_id];
			$dtls_data_update_arr[$dtls_id]["powise_requisition_net_issue_qty"] = ($po_rqsn_wise_issue_qty-$requsition_wise_issure_rtn_qty[$booking_no][$row[csf('po_break_down_id')]][$prod_id]);
			
			$dtls_data_update_arr[$dtls_id]["requisition_ids"] = implode(",",array_unique(explode(",",rtrim($plan_requisition_array[$booking_no][$row[csf("po_break_down_id")]][$prod_id]['requisition_ids'],", "))));
			$dtls_data_update_arr[$dtls_id]["requisition_nos"] = implode(",",array_unique(explode(",",rtrim($plan_requisition_array[$booking_no][$row[csf("po_break_down_id")]][$prod_id]['requisition_nos'],", "))));
			$dtls_data_update_arr[$dtls_id]["program_nos"] = implode(",",array_unique(explode(",",rtrim($plan_requisition_array[$booking_no][$row[csf("po_break_down_id")]][$prod_id]['program_nos'],", "))));


			$dtls_data_update_arr[$dtls_id]["booking_wise_wo_qty"] += $wo_data_arr[$row[csf("job_no")]][$booking_no][$prod_id]['qnty'];			

			//$dtls_data_update_arr[$dtls_id]["syn_alc_qnty"] += $new_allocation;
		...
		/*
			Script summary:This script intention is only work on allocation (-) minus synchronize with full align chain 
			*Allocation * Requsition * Work order * Net issue
		*/

		$balance=0;
		$only_booking_level_requisition=$po_level_requisition=$req_issue_qnty=array();
		$synchronized_allocation_qty = 0; 
		$check_item_qnty_break_down = array();
		foreach ($dtls_data_update_arr as $dtl_id => $dtl_row) // allocation details table data prepare
		{
			$current_stock = $product_array[$dtl_row["prod_id"]]['current_stock'];	

			$mst_id  = $dtl_row["mst_id"];
			$job_no   = $dtl_row["job_no"];
			$booking_no   = $dtl_row["booking_no"];			
			$po_id = $dtl_row["po_break_down_id"]; 
			$prod_id   = $dtl_row["prod_id"];
		
			// requisition 
			$program_no = $dtl_row["program_nos"];
			$requisition_master_pk = $dtl_row["requisition_ids"];
			$requisition_nos = $dtl_row["requisition_nos"];

			$booking_wise_requisition_qty = $dtl_row["booking_wise_requisition_qty"];
			$booking_wise_requisition_net_qty = $dtl_row["booking_wise_requisition_net_qty"];

			$po_requisition_qty = $dtl_row["powise_requisition_qty"];
			$po_net_rqsn_issue_qty = $dtl_row["powise_requisition_net_issue_qty"];
			//Gross leve requisition
			
			if($booking_wise_requisition_qty<$booking_wise_requisition_net_qty)
			{
				//$only_booking_level_requisition[$program_no][$requisition_master_pk][$requisition_nos][$prod_id] = $booking_wise_requisition_net_qty;
				$only_booking_level_requisition[$requisition_master_pk] = $booking_wise_requisition_net_qty;
			}

			//echo $po_requisition_qty."<".$po_net_rqsn_issue_qty."po".$po_id."<br>";
			//  po_level requisition
			if($po_requisition_qty<$po_net_rqsn_issue_qty) 
			{
				$synchronized_po_requisition_qty = $po_net_rqsn_issue_qty;
				$po_level_requisition[$booking_no][$program_no][$requisition_master_pk][$requisition_nos][$po_id][$prod_id] = $synchronized_po_requisition_qty;
			}

		
            $net_issue_qty   = $dtl_row["net_issue_qty"];
            $allocation_qty = $dtl_row["alcation_qnty"];
            //echo $allocation_qty."<".$net_issue_qty;
			if($allocation_qty<$net_issue_qty)
			{
				$synchronized_allocation_qty = $net_issue_qty;
				$dtls_id_array[]=$dtl_id;
				$data_array_dtls_update[$dtl_id]=explode("*",("".$synchronized_allocation_qty."*'999'*'".$pc_date_time."'"));
			}
			else{
				$synchronized_allocation_qty = $allocation_qty;
			}
			
			
            // echo $current_stock .">=". $balance."--".$dtl_id."<br>";
			...
			$mst_data_update_arr[$dtl_row["mst_id"]]["qnty"] += $synchronized_allocation_qty;


			$mst_data_update_arr[$dtl_row["mst_id"]]["qnty_break_down"] .= number_format($synchronized_allocation_qty,2,".","") . "_" . $dtl_row["po_break_down_id"] . "_" . $dtl_row["job_no"] . ",";
 			
 			$balance = $balance + (number_format($synchronized_allocation_qty, 2,".","") + number_format($dtl_row["return_qty"], 2,".","")) - number_format($dtl_row["issue_qty"], 2,".","");
		...
		if(!empty($mst_data_update_arr))
		{
			foreach ($mst_data_update_arr as $mst_id => $mst_row) // allocation master table prepare
			{	
				$qnty_break_down_arr = explode(",",$mst_row["qnty_break_down"]);		
				$qnty_break_down = implode(",",array_filter($qnty_break_down_arr));
		...
		//echo "10**";
		//echo "<pre>";
		//print_r($data_array_mst_update); die();

		if(!empty($only_booking_level_requisition))
		{
			foreach($only_booking_level_requisition as $requisition_master_pk=>$req_qnty)
			{
				$requisition_id_array[]=$requisition_master_pk;
				$data_array_requisition_update[$requisition_master_pk]=explode("*",($req_qnty."*'999'*'".$pc_date_time."'"));
			}
		}

		//echo "<pre>";
		//print_r($data_array_requisition_update);
		//die();
		/*if(!empty($only_booking_level_requisition))
		{
			foreach ($only_booking_level_requisition as $knit_id => $requisition_id_arr) // requsition table prepare
			{
				foreach($requisition_id_arr as $requisition_id=>$requisition_no_arr)
				{
					foreach($requisition_no_arr as $requisition_no=>$product_id_arr)
					{
						foreach ($product_id_arr as $product_id => $req_qnty) 
						{
							...
							execute_query("UPDATE ppl_yarn_requisition_entry set yarn_qnty=".number_format($req_qnty,2,".","")." where id=$requisition_id and knit_id=$knit_id and requisition_no=$requisition_no and prod_id=$product_id");	
						}
					}
				}
			}
		...
		//die();
		$flag = 1;
		if(!empty($po_level_requisition))
		{
			$booking_req_qnty = 0;
			foreach ($po_level_requisition as $booking_no => $booking_no_arr) // requsition table prepare
			{
				foreach ($booking_no_arr as $knit_id => $requisition_id_arr) // requsition table prepare
				{
					foreach($requisition_id_arr as $requisition_id=>$requisition_no_arr)
					{
						foreach($requisition_no_arr as $requisition_no=>$po_id_arr)
						{
							foreach($po_id_arr as $po_id=>$product_id_arr)
							{
								foreach ($product_id_arr as $product_id => $po_wise_req_qnty) 
								{
									//echo "10**update ppl_yarn_requisition_breakdown set order_requisition_qty=".number_format($req_qnty,2,".","")." where booking_no='".$booking_no."' and program_id=$knit_id and requisition_id=$requisition_no and order_id=$po_id and item_id=$product_id"."<br>";

									$po_requisition_upd = execute_query("UPDATE ppl_yarn_requisition_breakdown set order_requisition_qty=".number_format($po_wise_req_qnty,2,".","")." where booking_no='".$booking_no."' and program_id=$knit_id and requisition_id=$requisition_no and order_id=$po_id and item_id=$product_id");


									$booking_req_qnty += $po_wise_req_qnty;
									$requisition_id_array[]=$requisition_id;
									$data_array_requisition_update[$requisition_id]=explode("*",($booking_req_qnty."*'999'*'".$pc_date_time."'"));	
								}
							}
						}
					}
		...
		if ($po_requisition_upd) $flag = 1; else $flag = 0;
		...
		foreach ($prod_update_arr as $pr_id => $prod_row) // product table prepare
		{
			$allocated_qty = $prod_row['cumul_balance'];
			$available_qty = ($product_array[$prod_val[csf('id')]]['current_stock']-$allocated_qty);
			$prID_array[]=$pr_id;
			$data_array_product_update[$pr_id]=explode("*",($allocated_qty."*".$available_qty."*'999'*'".$pc_date_time."'"));
		}

		//echo "<pre>";
		//print_r($data_array_product_update);
		//die();
	}
	else
	{
	...
	if(!empty($data_array_mst_update))
	{
		$rID  = execute_query(bulk_update_sql_statement("inv_material_allocation_mst","id",$field_array_mst_update,$data_array_mst_update,$mst_id_array),0);
		if ($rID) $flag = 1; else $flag = 0;
	}

	//echo "10**".$rID."test"; die();
		
	if(!empty($data_array_dtls_update))
	{
		$rID2 =execute_query(bulk_update_sql_statement("inv_material_allocation_dtls","id",$field_array_dtls_update,$data_array_dtls_update,$dtls_id_array),0);
	...
	if(!empty($data_array_requisition_update))
	{
		$rID4 =execute_query(bulk_update_sql_statement("ppl_yarn_requisition_entry","id",$field_array_requisiton_update,$data_array_requisition_update,$requisition_id_array),0);
		if ($rID3) $flag = 1; else $flag = 0;
	}
