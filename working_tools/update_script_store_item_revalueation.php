...
from inv_transaction b, product_details_master a where b.prod_id=a.id and b.item_category in(select category_id from lib_item_category_list where category_type=1) and a.entry_form<>24 and b.status_active=1 and b.is_deleted=0 
order by b.PROD_ID, b.STORE_ID, b.ID";*/
$sql_dyes_trans="select b.PROD_ID, b.STORE_ID, b.ID as TRANS_ID, b.CONS_QUANTITY, b.CONS_RATE, b.CONS_AMOUNT, b.TRANSACTION_TYPE 
from inv_transaction b where b.prod_id in(123801) and b.status_active=1 and b.is_deleted=0 
order by b.PROD_ID, b.STORE_ID, b.ID";
...
$upTransID=$upIssID=true;
foreach($result as $row)
{
	if($prod_store_check[$row["PROD_ID"]][$row["STORE_ID"]]=="")
	{
		$prod_store_check[$row["PROD_ID"]][$row["STORE_ID"]]=$row["PROD_ID"];
	...
	if($row["TRANSACTION_TYPE"]==1 || $row["TRANSACTION_TYPE"]==4 || $row["TRANSACTION_TYPE"]==5)
	{
		...
		{
			if(number_format($rcv_data[$row["PROD_ID"]][$row["STORE_ID"]]["qnty"],12,'.','') > 0 && number_format($rcv_data[$row["PROD_ID"]][$row["STORE_ID"]]["amt"],12,'.','') > 0)
			{
				$runtime_rate=number_format(($rcv_data[$row["PROD_ID"]][$row["STORE_ID"]]["amt"]/$rcv_data[$row["PROD_ID"]][$row["STORE_ID"]]["qnty"]),12,'.','');
			}
			...
			$rcv_data[$row["PROD_ID"]][$row["STORE_ID"]]["qnty"]+=$row["CONS_QUANTITY"];
			$rcv_data[$row["PROD_ID"]][$row["STORE_ID"]]["amt"]+=$issue_amount;
		}
		...
		else
		{
			...
			$rcv_data[$row["PROD_ID"]][$row["STORE_ID"]]["qnty"]+=$row["CONS_QUANTITY"];
			$rcv_data[$row["PROD_ID"]][$row["STORE_ID"]]["amt"]+=$row["CONS_AMOUNT"];
			//echo $row["TRANSACTION_TYPE"]."=".$runtime_rate."=";print_r($rcv_data);echo "<br>";
		}
		$k=0;
	}
