...
$pi_sql="select ID, TOTAL_AMOUNT, UPCHARGE, DISCOUNT, NET_TOTAL_AMOUNT from com_pi_master_details where status_active=1 and DISCOUNT >0 or upcharge >0 and id not in(select pi_id from COM_BTB_LC_PI ) 
";
...
from com_pi_item_details where status_active=1 and pi_id in(select id from com_pi_master_details where status_active=1 and DISCOUNT >0 or upcharge >0 and id not in(select pi_id from COM_BTB_LC_PI ))
";
