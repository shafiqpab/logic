...
$str_cond_b = "and b.insert_date between '11-Apr-2017' and '11-Apr-2017 11:59:59 PM' ";
$company_library=array(3=>3);
foreach($company_library_off as $compid=>$compname) /// Less EPM than CPM approved pre-costing
{
	$flag=0;	
	ob_start();
...
<?
		$to="";
		$sql = "SELECT c.email_address FROM mail_group_mst a, mail_group_child b, user_mail_address c where b.mail_group_mst_id=a.id and a.mail_item=13 and b.mail_user_setup_id=c.id and a.company_id=$compid";
...
foreach($company_library_off as $compid=>$compname) 
{
...
<?
	$to="";	
	$sql = "SELECT c.email_address FROM mail_group_mst a, mail_group_child b, user_mail_address c where b.mail_group_mst_id=a.id and a.mail_item=9 and b.mail_user_setup_id=c.id and a.company_id=$compid";
	...
	// $to="sumonrahman@logicsoftbd.com,smbsintl@gmail.com";
	 //if($to!="") echo send_mail_mailer( $to, $subject, $message, "" );
	//echo $to;
...
foreach($company_library_off as $compid=>$compname) /// Bellow 5% Profitability Orders
{
	$flag=0;	
	ob_start();
    ...
    <table width="2150" border="1" rules="all" class="rpt_table" id="table_body3" style="font-size:13px;">
        <thead style="background-color:#CCC">
            <tr align="center">
                ...
                <th rowspan="2" width="100">Total SMV</th>
                <th rowspan="2" width="70">Unit Price</th>
                <th colspan="7">Margin Summary</th>
                <th rowspan="2" width="80">Costing Date</th>
...
<?
		...
		$sql = "SELECT c.email_address FROM mail_group_mst a, mail_group_child b, user_mail_address c where b.mail_group_mst_id=a.id and a.mail_item=12 and b.mail_user_setup_id=c.id and a.company_id=$compid";
...
foreach($company_library_off as $compid=>$compname) /// Daily Order Entry
{
	$flag=0;	
