...
<script>
	...
	var tableFilters_size = 
	{
		//col_15: "none",
		col_operation: {
		id: ["value_total_rcvd","value_total_issue"],
		col: [13,14],
		operation: ["sum","sum"],
		write_method: ["innerHTML","innerHTML"]
		}
	...
	function fn_report_generated(type)
	{
		if(type==1)
		{ //alert(type);
			if (form_validation('cbo_company_id*cbo_buyer_id*txt_job_no','Comapny Name*Party Name*Job No')==false)//*txt_date_from*txt_date_to----*From Date*To Date
			{
				return;
			}
			else
				{
					...
					freeze_window(3);
					http.open("POST","requires/embellishment_rcvd_report_controller.php",true);
	...
	{ 
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/embellishment_rcvd_report_controller.php?action='+action+'&order_id='+order_id, 'Work Progress Report Details', 'width='+width+',height=400px,center=1,resize=0,scrolling=0','../');
	...
	function openImageWindow(id)
	{
		...
		var page_link = 'requires/embellishment_rcvd_report_controller.php?&action=image_view_popup&id='+id;
	...
	{ 
		...
		var page_link="requires/embellishment_rcvd_report_controller.php?action=job_no_popup&company_id="+company_name+"&cbo_buyer_name="+cbo_buyer_name+"&year="+year;
	...
	{ 
		...
		var year=document.getElementById('cbo_year').value;
		var page_link="requires/embellishment_rcvd_report_controller.php?action=style_no_popup&cbo_party_name="+cbo_party_name+"&year="+year;
		var title="Style Ref.";
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=530px,height=420px,center=1,resize=0,scrolling=0','../')
	...
	{ 
		...
		var cbo_party_name=document.getElementById('cbo_buyer_id').value;
		var year=document.getElementById('cbo_year').value;
		var page_link="requires/embellishment_rcvd_report_controller.php?action=order_no_popup&cbo_party_name="+cbo_party_name+"&year="+year;
		var title="Order No";
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=530px,height=420px,center=1,resize=0,scrolling=0','../')
	...
	function job_search_popup(page_link,title)
	{
		...
		else
		{
			...
			page_link='requires/embellishment_rcvd_report_controller.php?action=job_popup&data='+data
			...
			emailwindow.onclose=function()
			{
				...
				var list_view_orders = return_global_ajax_value( 0+'**'+theemail+'**'+1, 'load_php_dtls_form', '', 'requires/embellishment_rcvd_report_controller');
				if(list_view_orders!='')
				{
         ...
         <fieldset style="width:1200px;">
            <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr >
                        <td> 
                        <? echo create_drop_down( "cbo_company_id", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/embellishment_rcvd_report_controller', this.value+'_'+document.getElementById('cbo_within_group').value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down('requires/embellishment_rcvd_report_controller', this.value, 'load_drop_down_mainbuyer', 'mainbuyer_td' );"); ?>
                    	</td>
						
                    	<td>
							<?php echo create_drop_down( "cbo_within_group", 100, $yes_no,"", 0, "--  --", 0, "load_drop_down( 'requires/embellishment_rcvd_report_controller',document.getElementById('cbo_company_id').value+'_'+this.value, 'load_drop_down_buyer', 'buyer_td' );" ); ?>
						</td>
                        <td id="buyer_td">
                        	<? 
                        ...
                        <td align="center">
                            <input type="button" id="show_button" class="formbutton" style="width:90px" value="Size Wise" onClick="fn_report_generated(1)" />
							<input type="button" id="show_button" class="formbutton" style="width:90px" value="Color Wise" onClick="fn_report_generated(2)" />							
                        </td>
                    </tr>
                </tbody>
                <tfoot>
