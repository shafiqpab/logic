<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Yarn GRN Return Entry 
Functionality	:	
JS Functions	:
Created by		:	Jahid 
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Yarn GRN Return Entry", "../../", 1, 1,'','',''); 
...
<script>
	...
	function set_receive_basis(i)
	{
		...
		var list_view_wo =trim(return_global_ajax_value( cbo_company_id, 'mrr_details', '', 'requires/yarn_grn_return_controller'));
	...
	function openmypage_wo_pi_popup()
	{
		...
		{ 	
			...
			//var page_link = 'requires/yarn_grn_return_controller.php?cbo_company_id='+cbo_company_id+'&recieve_basis='+recieve_basis+'&update_id='+update_id+'&dtls_id='+dtls_id+'&action=wo_pi_popup';
			var page_link = 'requires/yarn_grn_return_controller.php?cbo_company_id='+cbo_company_id+'&update_id='+update_id+'&cbo_supplier_name='+cbo_supplier_name+'&action=wo_pi_popup';
			...
			emailwindow.onclose=function()
			{
				...
				var hidden_wo_pi_id=this.contentDoc.getElementById("hidden_wo_pi_id").value;	 //Knit Id for Kintting Plan
				var hidden_wo_pi_no=this.contentDoc.getElementById("hidden_wo_pi_no").value; //all data for Kintting Plan
				...
				//alert(hidden_wo_pi_id+"**"+hidden_wo_pi_no+"**"+rcv_basis+"**"+data[0]); return;
				...
				show_list_view(hidden_wo_pi_id, 'show_fabric_desc_listview', 'list_fabric_desc_container', 'requires/yarn_grn_return_controller', 'setFilterGrid("list_fabric_desc_container",-1)' ) ;
	...
	{
		var cbo_company_id=$('#cbo_company_id').val();
		show_list_view(booking_pi_id, 'show_fabric_desc_listview_update', 'list_fabric_desc_container', 'requires/yarn_grn_return_controller', 'setFilterGrid("list_fabric_desc_container",-1)' ) ;
		calculate(1);
		set_button_status(1, permission, 'fnc_yarn_return',1,1);
	}
	
	
	
	
	function fnc_yarn_return(operation)
	{
		if(operation==4)
		{
			alert("Under Construction....");
			//var report_title=$( "div.form_caption" ).html();
			//print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title, "trims_receive_entry_print", "requires/yarn_grn_return_controller" ) 
			return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			if( form_validation('cbo_company_id*txt_return_date*txt_booking_pi_no','Company*Return Date*Booking No')==false )
			{
				return;
			}
			...
			$("#tbl_fabric_desc_item").find('tbody tr').not(':first').each(function()
			{
				...
				var returnqnty=$('#returnqnty_'+i).val();
				var rate=$('#tdrate_'+i).attr('title');
				var amount=$('#tdamount_'+i).attr('title');
				...
				{
					j++;
					dataString+='&count' + j + '=' + count + '&composition' + j + '=' + composition + '&comPersent' + j + '=' + comPersent + '&yarnType' + j + '=' + yarnType + '&color' + j + '=' + color + '&uom' + j + '=' + uom + '&grnqnty' + j + '=' + grnqnty+ '&returnqnty' + j + '=' + returnqnty+ '&rate' + j + '=' + rate+ '&amount' + j + '=' + amount+ '&updatedtlsid' + j + '=' + updatedtlsid+ '&grnDtlsId' + j + '=' + grnDtlsId;
			...
			var data="action=save_update_delete&operation="+operation+'&tot_row='+j+get_submitted_data_string('txt_recieved_id*cbo_company_id*cbo_supplier*txt_return_date*txt_remarks*txt_booking_pi_no*txt_wo_pi_id*update_id',"../../")+dataString;
		
			//alert(data); return;
			
			freeze_window(operation);
			http.open("POST","requires/yarn_grn_return_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
			http.onreadystatechange =fnc_yarn_return_Reply_info;
		}
	}
	
	function fnc_yarn_return_Reply_info()
	{
		if(http.readyState == 4) 
		{
			...
			{	
				...
				if(reponse[0]==2)
				{
					...
					set_button_status(0, permission, 'fnc_yarn_return',1,1);
					release_freezing();	
				}
			}
			release_freezing();	
	...
	function yarn_receive_popup()
	{
		...
		{ 	
			var page_link='requires/yarn_grn_return_controller.php?cbo_company_id='+cbo_company_id+'&action=yarn_receive_popup_search';
			...
			emailwindow.onclose=function()
			{
				...
				if(trims_recv_id!="")
				{
					...
					get_php_form_data(trims_recv_id, "populate_data_from_trims_recv", "requires/yarn_grn_return_controller" );
	...
	function calculate(i)
	{
		...
		//alert(numRow);
		math_operation( "tot_rcv_qnty", "returnqnty_", "+", numRow,ddd );
...
<div style="width:100%;">
    ...
    <form name="trimsreceive_1" id="trimsreceive_1">
    	<div style="width:1150px;" align="center">        
            ...
            <legend>Yarn Return Entry Master Part</legend>
			<fieldset style="width:1150px;">
            <table cellpadding="0" cellspacing="2" width="100%" align="left">
                <tr>
                    <td align="right" colspan="5"><strong> Yarn Return ID </strong></td>
                    <td colspan="5">
                ...
                <tr>
                    ...
                    <td width="80" class="must_entry_caption">Return Date</td>
                    <td width="150">
                        <input class="datepicker" type="text" style="width:130px" name="txt_return_date" id="txt_return_date" value="<? echo date("d-m-Y")?>" />
                    </td>
                    <td width="80">Remarks</td>
                    <td width="150">
						<input type="text" name="txt_remarks" id="txt_remarks" class="text_boxes" style="width:130px" >
                    </td>
            ...
            <fieldset style="width:1150px; margin-top:10px;">
                 	<legend>Yarn Return Entry details part</legend>
                    <? $i=1; ?>
                    	<table class="rpt_table" width="100%" cellspacing="0" cellpadding="0" border="1" rules="all" id="tbl_fabric_desc_item">
                        	<thead>
								<tr>
                                    ...
                                    <th width="100" class="must_entry_caption"><input type="checkbox" id="check_qnty" name="check_qnty" style="display:none;" onChange="fn_fill_qnty()" />&nbsp;Return Qty</th>	
                            ...
                            <tfoot>
                                ...
                                <th>&nbsp;</th>
                                <th>Total:</th>
                                <th><input type="text" id="tot_rcv_qnty" name="tot_rcv_qnty" style="width:80px;" class="text_boxes_numeric" readonly disabled /></th>
                                <th>&nbsp;</th>
                                <th>&nbsp;</th>
                            </tfoot>
                        </table>
                 ...
                 <table width="100%">
                    <tr>
                        ...
                        <?
                        echo load_submit_buttons($permission, "fnc_yarn_return", 0,1,"reset_form('trimsreceive_1','list_container_trims*list_fabric_desc_container','','','set_receive_basis(1);','')",1);
