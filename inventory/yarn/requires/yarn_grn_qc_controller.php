...
if ($action=="wo_pi_popup")
{
	...
	<script>
		...
		var selected_id = new Array; var selected_name = new Array();var supplier_name = new Array();supplier_mix_arr = new Array();basis_mix_arr = new Array();
		...
		function js_set_value( str ) {
			...
			if(supplier_mix_arr.length==0)
			{
				supplier_mix_arr.push( $('#supplierId' + str).val() );
			}
			else if( jQuery.inArray( $('#supplierId' + str).val(), supplier_mix_arr )==-1)
			{
				alert("Supplier Mix Not Allow");
				return;
			}
			
			if(basis_mix_arr.length==0)
			{
				basis_mix_arr.push( $('#receiveBasis' + str).val() );
			}
			else if( jQuery.inArray( $('#receiveBasis' + str).val(), basis_mix_arr )==-1)
			{
				alert("Receive Basis Mix Not Allow");
				return;
			}
			
			toggle( document.getElementById( 'search' + str ), '#FFFFCC' );
			
			if( jQuery.inArray( $('#woPiId' + str).val(), selected_id ) == -1 ) {
				selected_id.push( $('#woPiId' + str).val() );
				selected_name.push( $('#woPiNo' + str).val() );
				supplier_name.push( $('#supplierId' + str).val() );
			}
			else {
				for( var i = 0; i < selected_id.length; i++ ) {
					if( selected_id[i] == $('#woPiId' + str).val() ) break;
				}
				selected_id.splice( i, 1 );
				selected_name.splice( i, 1 );
				if( jQuery.inArray( $('#supplierId' + str).val(), supplier_mix_arr )!=-1)
				{
				}
				else
				{
					supplier_name.splice( i, 1 );
				}
			...
			//alert(supplier_name[0]+"="+$('#supplierId' + str).val());
			var sup_name =supplier_name[0];

			$('#hidden_wo_pi_id').val( id );
			$('#hidden_wo_pi_no').val( name );
			$('#hidden_supplier_id').val( sup_name );
		}
            ...
            <table cellpadding="0" cellspacing="0" border="1" rules="all" width="800" class="rpt_table">
                <thead>
                    ...
                    <th>
                    	...
                    	<input type="hidden" name="hidden_wo_pi_id" id="hidden_wo_pi_id" value="">  
                        <input type="hidden" name="hidden_wo_pi_no" id="hidden_wo_pi_no" value=""> 
...
if($action=="create_wo_pi_search_list_view")
{
	...
	$sql = "SELECT a.ID, a.RECV_NUMBER, a.RECEIVE_DATE, a.SUPPLIER_ID, a.RECEIVE_BASIS, a.BOOKING_ID, a.BOOKING_NO, a.INSERTED_BY, a.RECEIVE_BASIS
	...
	group by a.ID, a.RECV_NUMBER, a.RECEIVE_DATE, a.SUPPLIER_ID, a.RECEIVE_BASIS, a.BOOKING_ID, a.BOOKING_NO, a.INSERTED_BY, a.RECEIVE_BASIS 
	order by a.ID desc"; 
	//echo $sql;
	...
	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="900" class="rpt_table">
		<thead>
			...
			<tr>
				...
				<th width="100">GRN Basis</th>
				<th width="120">WO/PI NO</th>
				<th>Insert User</th>
			</tr>
		</thead>
	</table>
	<div style="width: class="must_entry_caption"px; max-height:280px; overflow-y:scroll" id="list_container_batch" align="left">	 
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="880" class="rpt_table" id="tbl_list_search">  
		<?
			$i=1;
			foreach ($result as $row)
			{
				if($pi_receive_data[$row['ID']]=="")
				{
					if ($i%2==0)  
						$bgcolor="#E9F3FF";
					else
					...
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value(<? echo $i; ?>);" id="search<? echo $i; ?>"> 
						...
						<? echo $i; ?>
                        <input type="hidden" name="woPiNo[]" id="woPiNo<? echo $i; ?>" value="<?= $row['RECV_NUMBER'];?>"> 
                        <input type="hidden" name="woPiId[]" id="woPiId<? echo $i; ?>" value="<?= $row['ID'];?>">
                        <input type="hidden" name="supplierId[]" id="supplierId<? echo $i; ?>" value="<?= $row['SUPPLIER_ID'];?>">
                        <input type="hidden" name="receiveBasis[]" id="receiveBasis<? echo $i; ?>" value="<?= $row['RECEIVE_BASIS'];?>">
                        </td>
						...
						<td width="100"><? echo $receive_basis_arr[$row['RECEIVE_BASIS']]; ?></td>             
...
if( $action == 'mrr_details' ) 
{
    ...
    <tr id="row_1" align="center">
        ...
        <td id="tdstatus_1">
		<? 
        echo create_drop_down( "cbostatus_1",70, $yarn_qc_statusArr ,'', 1, '--Select--',0,"","","","","","","","","cbostatus[]",""); 
        ?>
        </td>
        <td id="tdgrading_1">
        <? 
...
//	where a.id=b.mst_id and a.entry_form=530 and b.entry_form=530 and b.ITEM_CATEGORY_ID=1 and b.WO_PI_ID in($bookingNo_piId) and a.status_active=1 and b.status_active=1
...
//	{
//		$prev_data[$row["WO_PI_DTLS_ID"]]=$row["PARKING_QUANTITY"];
//	}
	
	$sql="select a.ID, a.RECV_NUMBER_PREFIX_NUM, a.RECV_NUMBER, a.ENTRY_FORM, a.RECEIVE_BASIS, a.RECEIVE_DATE, b.ID as DTLS_ID, b.WO_PI_ID, b.WO_PI_DTLS_ID, b.WO_PI_NO, b.PO_BREAK_DOWN_ID, b.ITEM_ID, b.YARN_COUNT, b.YARN_COMP_TYPE1ST, b.YARN_COMP_PERCENT1ST, b.YARN_TYPE, b.COLOR_NAME, b.LOT, b.BRAND_NAME, b.PRODUCT_CODE, b.UOM, b.WO_PI_QUANTITY, b.PARKING_QUANTITY, a.id as prod_id, a.company_id
	from INV_RECEIVE_MASTER a, QUARANTINE_PARKING_DTLS b
	...
	foreach($data_array as $row)
	{
		...
		<tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $i; ?>" align="center">
			<td id="grn_<? echo $i; ?>" title="<?= $row["RECV_NUMBER"];?>"><? echo $row["RECV_NUMBER"]; ?></td>
			<td id="grndate_<? echo $i; ?>" title="<?= $row["RECEIVE_DATE"];?>"><? echo change_date_format($row["RECEIVE_DATE"]); ?></td>
			<td id="tdbrand_<? echo $i; ?>" title="<?= $row["BRAND_NAME"];?>"><? echo $row["BRAND_NAME"]; ?></td>
			<td id="tdlot_<? echo $i; ?>" title="<?= $row["LOT"];?>" onclick="fnc_print('<?echo $company_id;?>', '<?echo $prod_id;?>')"><a href="#"><? echo $row["LOT"]; ?></a></td>
			...
			<td id="tdrejectqnty_<? echo $i; ?>"><input type="text" name="rejectqnty[]" id="rejectqnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="" onBlur="calculate(<? echo $i; ?>);"/></td>
			<td id="tdqcqnty_<? echo $i; ?>"><input type="text" name="qcqnty[]" id="qcqnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="" readonly /></td>
			
			<td id="tdstatus_<? echo $i; ?>">
			<? 
			echo create_drop_down( "cbostatus_".$i,70, $yarn_qc_statusArr ,'', 1, '--Select--',0,"copy_all($i);","","","","","","","","cbostatus[]",""); 
			?>
			</td>
			<td id="tdgrading_<? echo $i; ?>">
			<? 
			echo create_drop_down( "cbograde_".$i,70, $fabric_shade,'', 1, '--Select--',0,"","","","","","","","","cbograde[]",""); 
			?>
			</td>
			<td id="tdyarntest_<? echo $i; ?>">
			<? 
			echo create_drop_down( "cboyarntest_".$i,70, $comments_acceptance_arr ,'', 1, '--Select--',0,"","","","","","","","","cboyarntest[]",""); 
			?>
			</td>
			<td id="tdcomments_<? echo $i; ?>">
			<input type="text" name="comments[]" id="comments_<? echo $i; ?>" class="text_boxes" style="width:90px;" value=""/>
			<input type="hidden" name="updatedtlsid[]" id="updatedtlsid_<? echo $i; ?>" value="" readonly>
			<input type="hidden" name="piWoId[]" id="piWoId_<? echo $i; ?>" value="<? echo $row["WO_PI_ID"]; ?>" readonly>
...
if( $action == 'show_fabric_desc_listview_update' ) 
{
	...
	//print_r($data);die;
	$bookingNo_piId=$data[0];
	$sql="select a.ID, a.QC_NUMBER_PREFIX_NUM as RECV_NUMBER_PREFIX_NUM, a.QC_NUMBER as RECV_NUMBER, a.ENTRY_FORM, a.RECEIVE_DATE, b.ID as DTLS_ID, b.WO_PI_ID, b.WO_PI_DTLS_ID, b.WO_PI_NO, b.PARKING_DATE, b.ITEM_ID, b.YARN_COUNT, b.YARN_COMP_TYPE1ST, b.YARN_COMP_PERCENT1ST, b.YARN_TYPE, b.COLOR_NAME, b.LOT, b.BRAND_NAME, b.PRODUCT_CODE, b.UOM, b.WO_PI_QUANTITY, b.PARKING_QUANTITY, b.GREY_QUANTITY, b.QC_REJECT_QNTY, b.QC_QNTY, b.QC_STATUS, b.QC_GRADE, b.QC_YARN_TEST, b.REMARKS, B.GRN_DTLS_ID, c.id as prod_id,
	c.company_id
	from QUARANTINE_PARKING_MST a, QUARANTINE_PARKING_DTLS b, INV_RECEIVE_MASTER c
	where a.id=b.mst_id AND b.WO_PI_NO = c.RECV_NUMBER and a.entry_form=530 and b.entry_form=530 and b.ITEM_CATEGORY_ID=1 and a.id in($bookingNo_piId) and a.status_active=1 and b.status_active=1";
	// echo $sql; //die;
	$data_array=sql_select($sql);
	$grn_wo_pi_dtls_arr=$wo_pi_id_arr=array();
	foreach($data_array as $val)
	{
		$grn_wo_pi_dtls_arr[$val["WO_PI_DTLS_ID"]]=$val["WO_PI_DTLS_ID"];
		$wo_pi_id_arr[$val["WO_PI_ID"]]=$val["WO_PI_ID"];
	}
	
	$grn_basis=return_field_value("max(a.receive_basis) as receive_basis","inv_receive_master a, quarantine_parking_dtls b","a.id=b.mst_id and b.wo_pi_dtls_id in(".implode(",",$grn_wo_pi_dtls_arr).") and a.entry_form=529 and b.entry_form=529","receive_basis");;
	
	if(count($grn_wo_pi_dtls_arr)>0 && $grn_basis> 0)
	{
		$stock_sql="select b.PI_WO_REQ_DTLS_ID, a.RECV_NUMBER from INV_RECEIVE_MASTER a, INV_TRANSACTION b where a.id=b.mst_id and a.entry_form=1 and b.item_category=1 and b.transaction_type=1 and a.receive_basis=$grn_basis and b.receive_basis=$grn_basis and a.status_active=1 and b.status_active=1 and b.PI_WO_REQ_DTLS_ID in(".implode(",",$grn_wo_pi_dtls_arr).")";
		$stock_sql_result=sql_select($stock_sql);
		$stock_data=array();
		foreach($stock_sql_result as $val)
		{
			$stock_data[$val["PI_WO_REQ_DTLS_ID"]]=$val["RECV_NUMBER"];
		}
		
		$return_sql="select b.WO_PI_DTLS_ID, a.ISSUE_NUMBER from INV_ISSUE_MASTER a, QUARANTINE_PARKING_DTLS b where a.id=b.mst_id and a.entry_form=531 and b.entry_form=531 and b.ITEM_CATEGORY_ID=1 and a.status_active=1 and b.status_active=1 and b.WO_PI_DTLS_ID in(".implode(",",$grn_wo_pi_dtls_arr).")";
		$return_sql_result=sql_select($return_sql);
		$return_data=array();
		foreach($return_sql_result as $val)
		{
			$return_data[$val["WO_PI_DTLS_ID"]]=$val["ISSUE_NUMBER"];
		}
	}
	...
	foreach($data_array as $row)
	{
		$disable_field="";$disable_flag=0;$disable_massage="";
		if($stock_data[$row["WO_PI_DTLS_ID"]]!="" || $return_data[$row["WO_PI_DTLS_ID"]]!="")
		{
			$disable_field=" disabled";
			$disable_flag=1;
			if($stock_data[$row["WO_PI_DTLS_ID"]]!="")
			{
				$disable_massage=" Receive Found, Receive Id ".$stock_data[$row["WO_PI_DTLS_ID"]];
			}
			else
			{
				$disable_massage=" Return Found, Return Id ".$return_data[$row["WO_PI_DTLS_ID"]];
			}
		}
		$grn = $row["WO_PI_NO"];
		$lot = $row["LOT"];
		...
		?>
		<tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $i; ?>" align="center">
			<td id="grn_<? echo $i; ?>" title="<?= $grn;?>"><? echo $grn;  ?></td>
			<td id="grndate_<? echo $i; ?>" title="<?= $row["PARKING_DATE"];?>"><? echo change_date_format($row["PARKING_DATE"]); ?></td>
			<td id="tdbrand_<? echo $i; ?>" title="<?= $row["BRAND_NAME"];?>"><? echo $row["BRAND_NAME"]; ?></td>
			<td id="tdlot_<? echo $i; ?>" title="<?= $lot;?>" onclick="fnc_print('<?echo $company_id;?>', '<?echo $prod_id;?>')"><a href="#"><? echo $lot?></a></td>
			<td id="count_<? echo $i; ?>" title="<?= $row["YARN_COUNT"];?>"><? echo $yarn_count_library[$row["YARN_COUNT"]]; ?></td>
			<td id="composition_<? echo $i; ?>" title="<?= $row["YARN_COMP_TYPE1ST"];?>"><? echo $composition[$row["YARN_COMP_TYPE1ST"]]; ?></td>
			<td id="comPersent_<? echo $i; ?>" title="<?= $row["YARN_COMP_PERCENT1ST"];?>"><? echo $row["YARN_COMP_PERCENT1ST"]; ?></td>
			<td id="color_<? echo $i; ?>" title="<?= $row["COLOR_NAME"];?>"><? echo $color_library[$row["COLOR_NAME"]]; ?></td>
			...
			<td id="uom_<? echo $i; ?>" title="<?= $row["UOM"];?>"><? echo $unit_of_measurement[$row["UOM"]]; ?></td>
			<td id="grnqnty_<? echo $i; ?>" align="right" title="<?= $row["PARKING_QUANTITY"];?>"><? echo number_format($row["PARKING_QUANTITY"],3,'.',''); ?></td>
			<td id="tdrejectqnty_<? echo $i; ?>" title="<?= $disable_massage;?>"><input type="text" name="rejectqnty[]" id="rejectqnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<?=  number_format($row["QC_REJECT_QNTY"],2,'.','');?>" onBlur="calculate(<? echo $i; ?>);" <?= $disable_field;?>/></td>
			<td id="tdqcqnty_<? echo $i; ?>" title="<?= $disable_massage;?>"><input type="text" name="qcqnty[]" id="qcqnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<?=  number_format($row["QC_QNTY"],3,'.','');?>" readonly <?= $disable_field;?> /></td>
			
			<td id="tdstatus_<? echo $i; ?>" title="<?= $disable_massage;?>">
			<? 
			echo create_drop_down( "cbostatus_".$i,70, $yarn_qc_statusArr ,'', 0, '',$row["QC_STATUS"],"copy_all($i);",$disable_flag,"","","","","","","cbostatus[]",""); 
			?>
			</td>
			<td id="tdgrading_<? echo $i; ?>">
			<? 
			echo create_drop_down( "cbograde_".$i,70, $fabric_shade,'', 1, '--Select--',$row["QC_GRADE"],"",$disable_flag,"","","","","","","cbograde[]",""); 
			?>
			</td>
			<td id="tdyarntest_<? echo $i; ?>">
			<? 
			echo create_drop_down( "cboyarntest_".$i,70, $comments_acceptance_arr ,'', 1, '--Select--',$row["QC_YARN_TEST"],"",$disable_flag,"","","","","","","cboyarntest[]",""); 
			?>
			</td>
			<td id="tdcomments_<? echo $i; ?>">
			<input type="text" name="comments[]" id="comments_<? echo $i; ?>" class="text_boxes" style="width:90px;" value="<? echo $row["REMARKS"]; ?>"/>
			<input type="hidden" name="updatedtlsid[]" id="updatedtlsid_<? echo $i; ?>" value="<? echo $row["DTLS_ID"]; ?>" readonly>
			<input type="hidden" name="piWoId[]" id="piWoId_<? echo $i; ?>" value="<? echo $row["WO_PI_ID"]; ?>" readonly>
            <input type="hidden" name="grnDtlsId[]" id="grnDtlsId_<? echo $i; ?>" value="<? echo $row["GRN_DTLS_ID"]; ?>" readonly>
			<input type="hidden" name="piWoDtlsId[]" id="piWoDtlsId_<? echo $i; ?>" value="<? echo $row["WO_PI_DTLS_ID"]; ?>" readonly>
			</td>
		</tr>
		<?
...
if ($action=="save_update_delete")
{
	...
	{ 
		...
		if(str_replace("'","",$update_id)=="")
		{
			...
			$new_trims_recv_system_id = explode("*", return_next_id_by_sequence("QUARANTINE_PARKING_MST_PK_SEQ", "quarantine_parking_mst",$con,1,$cbo_company_id,'YQC',530,date("Y",time()) ));
			$field_array="id, qc_number_prefix, qc_number_prefix_num, qc_number, entry_form, receive_date, company_id, supplier_id, qa_parson, supplier_grading_id, grn_id, grn_number, inserted_by, insert_date";
			
			$data_array="(".$id.",'".$new_trims_recv_system_id[1]."',".$new_trims_recv_system_id[2].",'".$new_trims_recv_system_id[0]."',530,".$txt_receive_date.",".$cbo_company_id.",".$cbo_supplier.",".$txt_qa_person.",".$cbo_supplier_grade.",".$txt_wo_pi_id.",".$txt_booking_pi_no.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		{
			$field_array_update="supplier_id*qa_parson*supplier_grading_id*updated_by*update_date";
			
			$data_array_update=$cbo_supplier."*".$txt_qa_person."*".$cbo_supplier_grade."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
			
			$trims_recv_num=str_replace("'","",$txt_recieved_id);
			$master_id=str_replace("'","",$update_id);
		}
		
		$field_array_dtls="id, mst_id, wo_pi_id, wo_pi_dtls_id, grn_dtls_id, wo_pi_no, parking_date, yarn_count, yarn_comp_type1st, yarn_comp_percent1st, yarn_type, color_name, lot, brand_name, uom, parking_quantity, qc_reject_qnty, qc_qnty, qc_status, qc_grade, qc_yarn_test, remarks, inserted_by, insert_date, status_active, is_deleted, item_category_id, entry_form";
		...
		for($i=1;$i<=$tot_row; $i++)
		{
			...
			$uom="uom".$i;
			$grnqnty="grnqnty".$i;
			$rejectqnty="rejectqnty".$i;
			$qcqnty="qcqnty".$i;
			
			$cbostatus="cbostatus".$i;
			$cbograde="cbograde".$i;
			$cboyarntest="cboyarntest".$i;
			$comments="comments".$i;
			...
			$data_array_dtls.="(".$id_dtls.",".$master_id.",'".$$piWoId."','".$$piWoDtlsId."','".$$grnDtlsId."','".$$grn."','".$$grndate."','".$$count."','".$$composition."','".$$comPersent."','".$$yarnType."','".$$color."','".$$TxtLot."','".$$TxtBrand."','".$$uom."','".$$grnqnty."','".$$rejectqnty."','".$$qcqnty."','".$$cbostatus."','".$$cbograde."','".$$cboyarntest."','".$$comments."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','1','0','1','530')";
			
			if(str_replace("'","",$$cbostatus)==1)
			{
		...
		{
			$rID3=execute_query("update quarantine_parking_dtls set IS_QC_PASS=1 where id in(".implode(",",$grn_dtls_id_arr).")");
		}
	...
	{ 
		...
		$field_array_dtls_update="qc_reject_qnty*qc_qnty*qc_status*qc_grade*qc_yarn_test*remarks*updated_by*update_date";
		...
		for($i=1;$i<=$tot_row; $i++)
		{
			...
			{
				$grn_dtls_back_id_arr[str_replace("'","",$$grnDtlsId)]=str_replace("'","",$$grnDtlsId);
			}
		}
		...
		if(count($grn_dtls_back_id_arr)>0)
		{
			$rID5=execute_query("update quarantine_parking_dtls set IS_QC_PASS=0 where id in(".implode(",",$grn_dtls_back_id_arr).")");
		}
			...
			<legend>Enter search words</legend>           
				<table cellpadding="0" cellspacing="0" width="820" class="rpt_table" border="1" rules="all">
					...
					<tr class="general">
						...
						<td align="center">
							<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_company_id').value+'_'+document.getElementById('cbo_supplier').value+'_'+document.getElementById('cbo_year_selection').value, 'create_trims_recv_search_list_view', 'search_div', 'yarn_grn_qc_controller', 'setFilterGrid(\'list_view\',-1);')" style="width:100px;" />
						</td>
					</tr>
					<tr>
...
if($action=="create_trims_recv_search_list_view")
{
	...
	from QUARANTINE_PARKING_MST where entry_form=530 and status_active=1 and is_deleted=0 and company_id=$company_id and supplier_id like '$supplier_name' $search_field_cond $date_cond  $year_cond order by id desc"; 
	//echo $sql;
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="820" class="rpt_table">
        <thead>
			...
			<tr>
				<th width="50">Sl</th>	
				<th width="120">QC No</th>
				<th width="50">Year</th>
				<th width="130">Supplier</th>
				<th width="130">Q A Person</th>
				<th width="100">Receive date</th>
				<th width="130">Supplier Greading</th>
				<th>Insert User</th>
			</tr>
        </thead>
	</table>
        ...
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="800" class="rpt_table" id="list_view"> 
            ...
            {  
                ...
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $row[csf('id')]; ?>');"> 
                    ...
                    <td width="130"><p><? echo $row[csf('qa_parson')]; ?>&nbsp;</p></td>
                    ...
                    <td width="130"><p><? echo $fabric_shade[$row[csf('supplier_grading_id')]]; ?>&nbsp;</p></td>
...
{
	$data_array=sql_select("select id, qc_number as recv_number, company_id, supplier_id, qa_parson, supplier_grading_id, grn_id, grn_number
	...
	{ 
		...
		//echo "load_drop_down( 'requires/yarn_grn_qc_controller', '".$row[csf("company_id")]."', 'load_drop_down_supplier', 'supplier' );";
		echo "document.getElementById('cbo_supplier').value 				= '".$row[csf("supplier_id")]."';\n";
		
		echo "document.getElementById('txt_qa_person').value 				= '".$row[csf("qa_parson")]."';\n";
		echo "document.getElementById('cbo_supplier_grade').value 			= '".$row[csf("supplier_grading_id")]."';\n";
		...
		echo "set_button_status(1, '".$_SESSION['page_permission']."', 'fnc_yarn_qc',1,1);\n";  
		exit();
	}
}
