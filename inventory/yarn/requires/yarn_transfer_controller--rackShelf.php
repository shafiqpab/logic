...
if ($action=="load_room_rack_self_bin")
{
	...
	{
		if($user_store_ids) $user_store_cond = " and a.id in ($user_store_ids)"; else $user_store_cond = "";
		echo create_drop_down( "cbo_to_store", 160, "select a.id,a.store_name from lib_store_location a,lib_store_location_category  b where a.id=b.store_location_id and a.status_active=1 and a.is_deleted=0 and a.company_id=$data $user_store_cond and b.category_type in (1) group by a.id,a.store_name order by a.store_name","id,store_name", 1, "-- Select Store --", 0, "",0 );  	 
		exit();
	}
...
{
	$brand_arr=return_library_array( "select id, brand_name from lib_brand",'id','brand_name');
	$data_array=sql_select("select id, mst_id, from_store, to_store, from_prod_id, to_prod_id, transfer_qnty, rate, transfer_value, yarn_lot, brand_id,no_of_bag,no_of_cone,weight_per_bag from inv_item_transfer_dtls where id='$data'");
	foreach ($data_array as $row)
	{
		echo "reset_form('','','cbo_store_name*cbo_to_store*txt_yarn_lot*txt_transfer_qnty*txt_rate*txt_transfer_value*hide_brand_id*txt_yarn_brand*hidden_transfer_qnty*hidden_product_id*previous_from_prod_id*previous_to_prod_id*txt_item_desc*txt_current_stock*txt_btb_selection*txt_btb_lc_id*hidden_current_stock*origin_product_id*update_trans_issue_id*update_trans_recv_id','');\n";
		...
		echo "$('#cbo_to_store').attr('disabled',true);\n";
...
//	}
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$cbo_transfer_criteria)==1)
		{
			...
			if(str_replace("'","",$txt_transfer_qnty) > $product_info[0][csf("available_qnty")]*1){
				echo "21**Transfer quantity can not be greater than available quantity";
				die;
			}
		}
		...
		if(str_replace("'","",$update_id)=="")
		{
			...
			$data_array="(".$id.",'".$new_transfer_system_id[1]."',".$new_transfer_system_id[2].",'".$new_transfer_system_id[0]."',".$cbo_company_id.",".$txt_challan_no.",".$txt_transfer_date.",".$cbo_transfer_criteria.",".$cbo_to_company_id.",0,0,".$cbo_item_category.",".$txt_remarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',10)";
		...
		else
		{
			...
			$data_array_update=$txt_challan_no."*".$txt_transfer_date."*".$cbo_to_company_id."*".$txt_remarks."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		$field_array_trans="id, mst_id, company_id, supplier_id, prod_id, origin_prod_id, item_category, transaction_type, transaction_date, store_id, brand_id, order_uom, order_qnty, order_rate, order_amount, cons_uom, cons_quantity, cons_rate, cons_amount, balance_qnty, balance_amount, inserted_by, insert_date, btb_lc_id";		

		$field_array_dtls="id, mst_id, from_prod_id, to_prod_id, yarn_lot, brand_id, item_group, from_store, to_store, item_category, transfer_qnty, rate, transfer_value, rate_in_usd, transfer_value_in_usd, uom,no_of_bag,no_of_cone,weight_per_bag, inserted_by, insert_date";
		...
		if(str_replace("'","",$cbo_transfer_criteria)==1)
		{
			...
			$row_prod=sql_select("select id, current_stock, stock_value, avg_rate_per_unit, available_qnty, dyed_type from product_details_master where company_id=$cbo_to_company_id and item_category_id=1 and supplier_id='$supplier_id' and product_name_details=$txt_item_desc and lot=$txt_yarn_lot and status_active=1 and is_deleted=0");
			...
			$max_issue_date = return_field_value("max(transaction_date) as max_date", "inv_transaction", "prod_id=$product_id and store_id = $cbo_to_store  and status_active = 1", "max_date");      
			if($max_issue_date !="")
			{
			...
			$data_array_trans="(".$id_trans.",".$transfer_update_id.",".$cbo_company_id.",'".$supplier_id."',".$hidden_product_id.",".$origin_product_id.",".$cbo_item_category.",6,".$txt_transfer_date.",".$cbo_store_name.",".$hide_brand_id.",0,0,0,0,".$cbo_uom.",".$txt_transfer_qnty.",".$txt_rate.",".$txt_transfer_value.",0,0,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$txt_btb_lc_id.")";
			

			$recv_trans_id=return_next_id_by_sequence("INV_TRANSACTION_PK_SEQ", "inv_transaction", $con);
			$data_array_trans.=",(".$recv_trans_id.",".$transfer_update_id.",".$cbo_to_company_id.",'".$supplier_id."',".$product_id.",".$origin_product_id.",".$cbo_item_category.",5,".$txt_transfer_date.",".$cbo_to_store.",".$hide_brand_id.",".$cbo_uom.",".$txt_transfer_qnty.",'".$order_rate."','".$order_amount."',".$cbo_uom.",".$txt_transfer_qnty.",".$txt_rate.",".$txt_transfer_value.",".$txt_transfer_qnty.",".$txt_transfer_value.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$txt_btb_lc_id.")";
			
			$id_dtls = return_next_id_by_sequence("INV_ITEM_TRANSFER_DTLS_PK_SEQ", "inv_item_transfer_dtls", $con);
			$data_array_dtls="(".$id_dtls.",".$transfer_update_id.",".$hidden_product_id.",".$product_id.",".$txt_yarn_lot.",".$hide_brand_id.",0,".$cbo_store_name.",".$cbo_to_store.",".$cbo_item_category.",".$txt_transfer_qnty.",".$txt_rate.",".$txt_transfer_value.",'".$order_rate."','".$order_amount."',".$cbo_uom.",".$txt_no_of_bag.",".$txt_no_of_cone.",".$txt_weight_per_bag.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		else
		{
			...
			if($max_recv_date !="")
			{
			 ...
			 //----------------Check Last Issue Date for Transfer In---------------->>>>>>>
			$max_issue_date = return_field_value("max(transaction_date) as max_date", "inv_transaction", "prod_id=$hidden_product_id and store_id = $cbo_to_store  and status_active = 1", "max_date");      
			if($max_issue_date !="")
			{
				$max_issue_date = date("Y-m-d", strtotime($max_issue_date));
				$transfer_date = date("Y-m-d", strtotime(str_replace("'", "", $txt_transfer_date)));
			...
			$id_trans = return_next_id_by_sequence("INV_TRANSACTION_PK_SEQ", "inv_transaction", $con);
			$data_array_trans="(".$id_trans.",".$transfer_update_id.",".$cbo_company_id.",0,".$hidden_product_id.",".$origin_product_id.",".$cbo_item_category.",6,".$txt_transfer_date.",".$cbo_store_name.",".$hide_brand_id.",0,0,0,0,".$cbo_uom.",".$txt_transfer_qnty.",".$txt_rate.",".$txt_transfer_value.",0,0,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$txt_btb_lc_id.")";
			
			//$id_trans=$id_trans+1;
			$recv_trans_id = return_next_id_by_sequence("INV_TRANSACTION_PK_SEQ", "inv_transaction", $con);
			$data_array_trans.=",(".$recv_trans_id.",".$transfer_update_id.",".$cbo_company_id.",0,".$hidden_product_id.",".$origin_product_id.",".$cbo_item_category.",5,".$txt_transfer_date.",".$cbo_to_store.",".$hide_brand_id.",0,0,0,0,".$cbo_uom.",".$txt_transfer_qnty.",".$txt_rate.",".$txt_transfer_value.",".$txt_transfer_qnty.",".$txt_transfer_value.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$txt_btb_lc_id.")";
			$id_dtls = return_next_id_by_sequence("INV_ITEM_TRANSFER_DTLS_PK_SEQ", "inv_item_transfer_dtls", $con);
			$data_array_dtls="(".$id_dtls.",".$transfer_update_id.",".$hidden_product_id.",".$hidden_product_id.",".$txt_yarn_lot.",".$hide_brand_id.",0,".$cbo_store_name.",".$cbo_to_store.",".$cbo_item_category.",".$txt_transfer_qnty.",".$txt_rate.",".$txt_transfer_value.",0,0,".$cbo_uom.",".$txt_no_of_bag.",".$txt_no_of_cone.",".$txt_weight_per_bag.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		$field_array_trans="supplier_id*prod_id*origin_prod_id*transaction_date*store_id*brand_id*order_qnty*order_rate*order_amount*cons_uom*cons_quantity*cons_rate*cons_amount*balance_qnty*balance_amount*updated_by*update_date*btb_lc_id";
		$field_array_dtls="from_prod_id*to_prod_id*yarn_lot*brand_id*from_store*to_store*transfer_qnty*rate*transfer_value*rate_in_usd*transfer_value_in_usd*uom*no_of_bag*no_of_cone*weight_per_bag*updated_by*update_date";
		...
		if(str_replace("'","",$cbo_transfer_criteria)==1)
		{
			...
			$row_prod=sql_select("select id, current_stock, avg_rate_per_unit, available_qnty, dyed_type from product_details_master where company_id=$cbo_to_company_id and item_category_id=1 and supplier_id='$supplier_id' and product_name_details=$txt_item_desc and lot=$txt_yarn_lot and status_active=1 and is_deleted=0");
			...
			if($max_recv_date !="")
			{
			 ...
			 //----------------Check Last Issue Date for Transfer In---------------->>>>>>>
			$max_issue_date = return_field_value("max(transaction_date) as max_date", "inv_transaction", "prod_id=$product_id and store_id = $cbo_to_store and id not in ($update_trans_recv_id , $update_trans_issue_id )  and status_active = 1", "max_date");      
			if($max_issue_date !="")
			{
				$max_issue_date = date("Y-m-d", strtotime($max_issue_date));
				$transfer_date = date("Y-m-d", strtotime(str_replace("'", "", $txt_transfer_date)));
			...
			$updateTransID_data[$update_trans_issue_id]=explode("*",("'".$supplier_id."'*'".$hidden_product_id."'*".$origin_product_id."*".$txt_transfer_date."*".$cbo_store_name."*".$hide_brand_id."*'0'*'0'*'0'*".$cbo_uom."*".$txt_transfer_qnty."*".$txt_rate."*".$txt_transfer_value."*'0'*'0'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$txt_btb_lc_id));
			
			$updateTransID_array[]=$update_trans_recv_id; 
			$updateTransID_data[$update_trans_recv_id]=explode("*",("'".$supplier_id."'*'".$product_id."'*".$origin_product_id."*".$txt_transfer_date."*".$cbo_to_store."*".$hide_brand_id."*'".$txt_transfer_qnty."'*'".$order_rate."'*'".$order_amount."'*".$cbo_uom."*".$txt_transfer_qnty."*".$txt_rate."*".$txt_transfer_value."*'".$txt_transfer_qnty."'*'".$txt_transfer_value."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$txt_btb_lc_id));
			
			$data_array_dtls=$hidden_product_id."*'".$product_id."'*".$txt_yarn_lot."*".$hide_brand_id."*".$cbo_store_name."*".$cbo_to_store."*'".$txt_transfer_qnty."'*".$txt_rate."*".$txt_transfer_value."*'".$order_rate."'*'".$order_amount."'*".$cbo_uom."*".$txt_no_of_bag."*".$txt_no_of_cone."*".$txt_weight_per_bag."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		else
		{
			...
			$max_transaction_date = return_field_value("max(transaction_date) as max_date", "inv_transaction", "prod_id=$hidden_product_id and store_id = $cbo_to_store and id not in ($update_trans_recv_id , $update_trans_issue_id)  and status_active = 1", "max_date");      
			...
			$updateTransID_array[]=$update_trans_issue_id; 
			$updateTransID_data[$update_trans_issue_id]=explode("*",("0*'".$hidden_product_id."'*".$origin_product_id."*".$txt_transfer_date."*".$cbo_store_name."*".$hide_brand_id."*0*0*0*".$cbo_uom."*".$txt_transfer_qnty."*".$txt_rate."*".$txt_transfer_value."*0*0*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$txt_btb_lc_id));
			
			$updateTransID_array[]=$update_trans_recv_id; 
			$updateTransID_data[$update_trans_recv_id]=explode("*",("0*'".$hidden_product_id."'*".$origin_product_id."*".$txt_transfer_date."*".$cbo_to_store."*".$hide_brand_id."*0*0*0*".$cbo_uom."*".$txt_transfer_qnty."*".$txt_rate."*".$txt_transfer_value."*".$txt_transfer_qnty."*".$txt_transfer_value."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$txt_btb_lc_id));
			
			$data_array_dtls=$hidden_product_id."*".$hidden_product_id."*".$txt_yarn_lot."*".$hide_brand_id."*".$cbo_store_name."*".$cbo_to_store."*".$txt_transfer_qnty."*".$txt_rate."*".$txt_transfer_value."*0*0*".$cbo_uom."*".$txt_no_of_bag."*".$txt_no_of_cone."*".$txt_weight_per_bag."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
