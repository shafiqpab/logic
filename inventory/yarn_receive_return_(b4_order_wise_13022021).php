...
//--------------------------------------------------------------------------------------------------------------------
	...
	function fnc_yarn_receive_return_entry(operation)
	{
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_id').val()+'*'+$('#txt_return_no').val()+'*'+report_title, "yarn_receive_return_print", "requires/yarn_receive_return_controller" ) 
			return;
		}
		else if(operation==5)
		{
			var report_title=$( "div.form_caption" ).html();
			print_report( $('#cbo_company_id').val()+'*'+$('#txt_return_no').val()+'*'+report_title, "yarn_receive_return_print_2", "requires/yarn_receive_return_controller" ) 
			return;
		}
		else
		{
									...
									<tr>
										<td width="49%" valign="top">
											<fieldset style="width:980px;">  
												...
												<table  width="980" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
													<thead>
														<tr>
