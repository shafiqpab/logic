...
<script>
	...
	function openmypage_booking(page_link, title) {
		...
		emailwindow.onclose = function () {
			...
			if (booking_no != "") {
				...
				show_list_view(document.getElementById('txt_job_no').value, 'yarn_description_list', 'yarn_list', 'requires/yarn_allocation_controller', '');
	...
	function open_item_popup(page_link, title) {
		...
		emailwindow = dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1125px,height=450px,center=1,resize=0,scrolling=0', '../')
		emailwindow.onclose = function () {
	...
	function open_qnty_popup(page_link, title) {
		...
		page_link = page_link + '&txt_order_id=' + txt_order_id + '&available_qnty=' + available_qnty + '&txt_qnty=' + txt_qnty + '&qnty_breck_down=' + qnty_breck_down + '&txt_booking_qnty=' + txt_booking_qnty + '&txt_job_no=' + txt_job_no + '&txt_entry_form=' + txt_entry_form + '&txt_booking_no=' + txt_booking_no + '&update_id=' + update_id + '&txt_old_qnty=' + txt_old_qnty + '&txt_selectted_fabric='+txt_selectted_fabric + '&txt_fabric_po=' + txt_fabric_po + '&txt_fab_booking_qnty=' + txt_fab_booking_qnty + '&pre_qnty_breck_down=' + pre_qnty_breck_down;
	...
	function fnc_material_allocation_entry(operation) {
		...
		*/
		...
		if (confirm("Are You Sure?")) {
			var data = "action=save_update_delete&operation=" + operation + get_submitted_data_string('txt_job_no*txt_order_id*txt_item_id*txt_item_id_old*txt_qnty*qnty_breck_down*cbo_item_category*txt_allocation_date*update_id*txt_old_qnty*txt_booking_no*cbo_company_name*pre_qnty_breck_down*hdn_dyed_type', "../../");
	...
	function fnc_material_allocation_entry_reponse() {
		if (http.readyState == 4) {
			...
			if (reponse[0] == 0 || reponse[0] == 1 || reponse[0] == 2) {
				show_list_view(document.getElementById('txt_job_no').value + '_' + document.getElementById('cbo_item_category').value + '_' + document.getElementById('txt_booking_no').value, 'show_item_active_listview', 'item_list_view', 'requires/yarn_allocation_controller', '');
					...
					<table cellpadding="0" cellspacing="2" width="100%">
						...
						<tr>
							...
							<td id="section_td" width="190">
								<input type="text" name="txt_qnty" id="txt_qnty" style="width:90px;" value="" class="text_boxes_numeric" placeholder="Click"
