            	...
            	<table  cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
        			...
        			<tr>
                    	<td> <input type="hidden" id="selected_job">
							<? 
								echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", '',"load_drop_down( 'trims_allocation_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
							?>
                    </td>
                   	<td id="buyer_td">
                     <? 
...
if($action=="populate_data_from_search_popup")
{
	...
	foreach( $sql as $row)
	{
				echo "load_drop_down( 'requires/trims_allocation_controller', '".$row[csf("company_name")]."', 'load_drop_down_location', 'location_td' ); load_drop_down( 'requires/trims_allocation_controller', '".$row[csf("company_name")]."', 'load_drop_down_buyer', 'buyer_td' ) ;\n";
...
if($action=="show_item_active_listview")
{
	...
	echo  create_list_view("list_view", "Company,Buyer,Location,Order No,Item,Qnty", "60,100,100,200,250,100","860","320",0, $sql , "get_php_form_data", "id", "'populate_material_allocation_data'", 1, "company_name,buyer_name,location_name,id,item_id,0", $arr , "company_name,buyer_name,location_name,id,item_id,qnty", 'requires/trims_allocation_controller','','0,0,0,0,0,2','',"");
}
