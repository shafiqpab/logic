...
{
	echo create_drop_down( "cbo_buyer_name", 170, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond  and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name", 1, "--Select--", "", "" );   	 
}
            	...
            	<table  cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
        			...
        			<tr>
                    	<td> <input type="hidden" id="selected_job">
							<? 
								echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", '',"load_drop_down( 'finish_allocation_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
							?>
                    </td>
                   	<td id="buyer_td">
                     <? 
                    ...
                    <td><input name="txt_date_from" id="txt_date_from" class="datepicker" style="width:70px">
            		 ...
            		 <td align="center">
                     <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value, 'create_po_search_list_view', 'search_div', 'finish_allocation_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" /></td>
        		</tr>
             </table>
          </td>
        </tr>
...
if($action=="populate_data_from_search_popup")
{
	...
	foreach( $sql as $row)
	{
				echo "load_drop_down( 'requires/finish_allocation_controller', '".$row[csf("company_name")]."', 'load_drop_down_location', 'location_td' ); load_drop_down( 'requires/finish_allocation_controller', '".$row[csf("company_name")]."', 'load_drop_down_buyer', 'buyer_td' ) ;\n";
...
if($action=="show_item_active_listview")
{
	...
	echo  create_list_view("list_view", "Company,Buyer,Location,Order No,Item,Qnty", "60,100,100,200,250,100","860","320",0, $sql , "get_php_form_data", "id", "'populate_material_allocation_data'", 1, "company_name,buyer_name,location_name,id,item_id,0", $arr , "company_name,buyer_name,location_name,id,item_id,qnty", 'requires/finish_allocation_controller','','0,0,0,0,0,2','',"");
}
