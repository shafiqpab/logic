...
function openmypage(page_link,title)
{
	...
	emailwindow.onclose=function()
	{
		...
		{
			freeze_window(5);
		    reset_form('materialallocation_1','','','cbo_item_category,13','');
		    document.getElementById('txt_job_no').value=theemail.value;
			get_php_form_data(theemail.value, "populate_data_from_search_popup", "requires/grey_allocation_controller" );
			show_list_view(theemail.value+'_'+document.getElementById('cbo_item_category').value,'show_item_active_listview','item_list_view','requires/grey_allocation_controller','');
			release_freezing();
		}
	}
}
...
function fnc_material_allocation_entry( operation )
{
	...
	else
	{
		...
		http.open("POST","requires/grey_allocation_controller.php",true);
...
function fnc_material_allocation_entry_reponse()
{
	if(http.readyState == 4) 
	{
		...
		show_msg(reponse[0]);
		show_list_view(document.getElementById('txt_job_no').value+'_'+document.getElementById('cbo_item_category').value,'show_item_active_listview','item_list_view','requires/grey_allocation_controller','');
	    reset_form('','','txt_item_id*txt_item*txt_qnty*cbo_uom*available_qnty*qnty_breck_down*update_id','cbo_item_category,13','');
		...
		<form name="materialallocation_1" id="materialallocation_1" autocomplete="off"> 
				<table cellpadding="0" cellspacing="2" width="100%">
                	<tr>
                        ...
                        <td width="190">
						<input name="txt_job_no"  id="txt_job_no" placeholder="Double Click to Search" onDblClick="openmypage('requires/grey_allocation_controller.php?action=job_popup','Job Selection Form')" readonly  style="width:158px "  class="text_boxes" />
						</td>
                        <td width="110">Company</td>
						<td width="190">
                        <?php 
						 echo create_drop_down( "cbo_company_name", 170,"select id,company_name from lib_company where is_deleted=0 and status_active=1 order by company_name","id,company_name", 1, "Display", "", "load_drop_down( 'requires/grey_allocation_controller', this.value, 'load_drop_down_location','location_td');load_drop_down( 'requires/grey_allocation_controller', this.value, 'load_drop_down_buyer', 'buyer_td' )",1 );
						 ?>   	
						</td>
					...
					<tr>
						...
						<td width="170">
                      	<input type="text" name="txt_order_no"  id="txt_order_no" style="width:160px "  placeholder="Double Click to Search" class="text_boxes" onDblClick="open_order_popup( 'requires/grey_allocation_controller.php?action=open_order_popup','Order List' );" readonly />
                        <input type="hidden" name="txt_order_id"  id="txt_order_id" style="width:160px " readonly />
						</td>
                        <td width="100">Item Category</td>
						<td id="location_td" width="200">
                        <?php 
						 echo create_drop_down( "cbo_item_category", 170,$item_category,"", "", "", 13, "",1 );
						 ?> 	
						</td>
					...
					<tr>
						...
						<td width="170">
                       <input type="text" name="txt_item"  id="txt_item" style="width:160px "  placeholder="Double Click to Search" class="text_boxes" onDblClick="open_item_popup( 'requires/grey_allocation_controller.php?action=open_item_popup','Item List' );"  />
                       <input type="hidden" name="txt_item_id"  id="txt_item_id" style="width:160px "  />		
						...
						<input type="text" name="txt_qnty"  id="txt_qnty" style="width:90px " value="" class="text_boxes_numeric" placeholder="Click" onClick="open_qnty_popup( 'requires/grey_allocation_controller.php?action=open_qnty_popup','Qnty List' )" />
                        <?php 
				  ...
				  <tr>
                     	...
                     	<? 
                     	echo load_submit_buttons($permission, "fnc_material_allocation_entry", 0,0 ,"reset_form('materialallocation_1','','','cbo_item_category,13','')",1);
						 ?>
                        </td>		
				</tr>
		</table>
