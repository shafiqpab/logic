...
if($action == "load_drop_floor")
{
	...
	$store_id=$data[0];
	$company_id = $data[1];
	echo create_drop_down( "cbo_floor", 130, "select b.floor_id,a.floor_room_rack_name from lib_floor_room_rack_mst a,lib_floor_room_rack_dtls b where  a.floor_room_rack_id=b.floor_id and b.store_id='$store_id' and a.company_id='$company_id' and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0  group by b.floor_id,a.floor_room_rack_name  order by a.floor_room_rack_name","floor_id,floor_room_rack_name", 1, "--Select Floor--", 0, "reset_on_change(this.id);load_drop_down('requires/general_item_receive_controller', this.value+'_'+$company_id+'_'+$store_id, 'load_drop_room','to_room_td');storeUpdateUptoDisable();" );
}
...
if($action == "load_drop_room")
{
	...
	echo create_drop_down( "txt_room", 130, "select b.room_id,a.floor_room_rack_name from lib_floor_room_rack_mst a,lib_floor_room_rack_dtls b where  a.floor_room_rack_id=b.room_id and b.floor_id='$floor_id' and b.store_id=$store_id and a.company_id='$company_id' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.room_id,a.floor_room_rack_name order by a.floor_room_rack_name","room_id,floor_room_rack_name", 1, "--Select Room--", 0, "reset_on_change(this.id);load_drop_down('requires/general_item_receive_controller', this.value+'_'+$company_id+'_'+$store_id, 'load_drop_rack','to_rack_td');storeUpdateUptoDisable();" );
}
...
if($action == "load_drop_rack")
{
	...
	$company_id = $data[1];
	$store_id = $data[2];
	echo create_drop_down( "txt_rack", 130, "select b.rack_id,a.floor_room_rack_name from lib_floor_room_rack_mst a,lib_floor_room_rack_dtls b where  a.floor_room_rack_id=b.rack_id and b.room_id='$room_id' and b.store_id=$store_id and a.company_id='$company_id' and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 group by b.rack_id,a.floor_room_rack_name order by a.floor_room_rack_name","rack_id,floor_room_rack_name", 1, "--Select Rack--", 0, "reset_on_change(this.id);load_drop_down('requires/general_item_receive_controller', this.value+'_'+$company_id+'_'+$store_id, 'load_drop_shelf','to_shelf_td');storeUpdateUptoDisable();" );
}
...
if($action == "load_drop_shelf")
{
	...
	$company_id = $data[1];
	$store_id = $data[2];
	echo create_drop_down( "txt_self", 130, "select b.shelf_id,a.floor_room_rack_name from lib_floor_room_rack_mst a,lib_floor_room_rack_dtls b where  a.floor_room_rack_id=b.shelf_id and b.rack_id='$rack' and b.store_id=$store_id and a.company_id='$company_id' and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 group by b.shelf_id,a.floor_room_rack_name order by a.floor_room_rack_name","shelf_id,floor_room_rack_name", 1, "--Select Shelf--", 0, "reset_on_change(this.id);load_drop_down('requires/general_item_receive_controller', this.value+'_'+$company_id+'_'+$store_id, 'load_drop_bin','to_bin_td');storeUpdateUptoDisable();" );
}
...
if ($action=="upto_variable_settings")
{
	...
	if(str_replace("'","",$item_category)==22)
	{
		$variable_lot=return_field_value("auto_transfer_rcv","variable_settings_inventory","company_name=$cbo_company_id and variable_list=32 and status_active=1 and is_deleted=0");
		echo "**".$variable_lot;
	}
	exit();
}
            ...
            <tbody>
                <tr>
                    ...
                    <td align="center">
                     ...
                     <td align="center">
                        <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $company; ?>+'_'+document.getElementById('cbo_item_category').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+'<? echo $itme_category_id; ?>', 'create_wopi_search_list_view', 'search_div', 'general_item_receive_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />				
                    </td>
            	</tr>
                <tr>                  
...
if($action=="create_wopi_search_list_view")
{
	...
	if(trim($txt_search_by)==1) // for pi
	{
		...
		if($item_cat_ref==4) $item_cat_ref="4,11";
	...
	if($txt_search_by==1 ) //pi base
	{
		...
		if($db_type==0)
		{
			if($itme_category_id==4) $itme_category_id="4,11";
	...
	else if($txt_search_by==2) // wo base
	{
 		$sql = "select a.id, a.wo_number_prefix_num as wopi_number,' ' as lc_number, a.wo_date as wopi_date, a.supplier_id as supplier_id, a.currency_id as currency_id, a.source as source, b.item_category_id
				...
				$sql_cond
				group by a.id,a.wo_number_prefix_num, a.wo_date, a.supplier_id, a.currency_id, a.source, b.item_category_id";
	...
	else if($txt_search_by==7) // requisition base
	{
	    ...
	    if($approval_need==1)
	    {
 		...
 		$sql = "select a.id, a.requ_prefix_num as wopi_number, ' ' as lc_number, a.requisition_date as wopi_date, '' as supplier_id, a.cbo_currency as currency_id, a.source as source , b.item_category as item_category
				...
				where a.id=b.mst_id and	a.status_active=1 and a.is_deleted=0 and b.item_category in($itme_category_id) and a.pay_mode=4 and a.company_id=$company $sql_cond $approval_cond
				group by a.id, a.requ_prefix_num, a.requisition_date, a.cbo_currency, a.source, b.item_category"; 
	}
	//echo $sql;
...
{ 
	...
	$receive_return_sql = sql_select("select a.received_id, b.prod_id, c.item_group_id, c.conversion_factor, sum(b.cons_quantity) as issue_qnty 
	from  inv_issue_master a, inv_transaction b, product_details_master c 
	where a.id=b.mst_id and b.prod_id=c.id and a.status_active=1 and b.transaction_type=3 and a.received_id>0 and a.entry_form=26 and b.transaction_type=3 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.received_id,b.prod_id, c.item_group_id, c.conversion_factor");
	foreach($receive_return_sql as $row)
	{
		...
		$receive_rtn_arr[$row[csf("received_id")]][$row[csf("prod_id")]]+=$row[csf("issue_qnty")]/$row[csf("conversion_factor")];
	}
	...
	{	
		...
		where a.id=$wo_pi_ID and a.id=b.pi_id and b.item_prod_id=c.id and c.item_category_id in($itme_category_id) and a.status_active=1 and b.status_active=1 and c.status_active in(1,3)  
	...
	else if($receive_basis==2) // wo basis
	{
		...
		where a.id=b.mst_id and a.id=$wo_pi_ID and a.pay_mode in (1,4) and b.item_id=c.id and c.item_category_id in($itme_category_id) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active in(1,3) 
		...
		$receive_sql = sql_select("select a.id, a.booking_id, b.prod_id, sum(b.order_qnty) as receive_qnty from  inv_receive_master a, inv_transaction b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form=20 and a.receive_basis=2 and b.transaction_type=1 group by a.id, a.booking_id, b.prod_id");
		foreach($receive_sql as $row)
		{
			$receive_arr[$row[csf("booking_id")]][$row[csf("prod_id")]]+=$row[csf("receive_qnty")]-$receive_rtn_arr[$row[csf("id")]][$row[csf("prod_id")]];
		}
    	...
    	<table class="rpt_table" border="1" cellpadding="2" cellspacing="0" width="400" rules="all">
            ...
            <tbody>
				...
				{  
					...
					?>
					<tr bgcolor="<? echo $bgcolor; ?>" onClick='get_php_form_data("<? echo $receive_basis."**".$row[csf("id")]."**".$row[csf("company_id")]."**".$row[csf("supplier_id")]."**".$row[csf("quantity")]."**".$row[csf("prod_id")]."**".$balance_quantity;?>","wo_pi_product_form_input","requires/general_item_receive_controller")' style="cursor:pointer" >
                        <td><? echo $i; ?></td>
...
if($action=="wo_pi_product_form_input")
{
	...
	{	
		...
		from com_pi_master_details a, com_pi_item_details b, product_details_master c
		where a.id=$wo_pi_ID and c.id=$product_id and a.id=b.pi_id and b.item_prod_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
...
<body>
	...
	<form name="item_popup_1"  id="item_popup_1">
      <?
 		 ...
 		 /* new dev*/
 		 $sql="select id, item_code, item_number, item_description, item_size, current_stock, brand_name, origin, model, order_uom from product_details_master where status_active=1 and is_deleted=0 and company_id=$company_id and item_category_id=$item_category and item_group_id=$item_group $entry_cond";
		  //echo $sql;
...
{	  
	...
	$txt_manufac_date=str_replace("'","",$txt_manufac_date);
	...
	if(number_format($rate,10,".","")==0)
	{
		echo "20**Rate Not Found.";die;
	}
	...
	$sql_variable_setup="select over_rcv_percent, over_rcv_payment from variable_inv_ile_standard where company_name = $cbo_company_name and category=$cbo_item_category and variable_list = 23 and status_active=1 and is_deleted=0";
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		if($rcv_basis !=4 && $rcv_basis !=6)
		{
			...
			from  inv_issue_master a, inv_transaction b, product_details_master c 
			where a.id=b.mst_id and b.prod_id=c.id and a.status_active=1 and b.transaction_type=3 and a.received_id>0 and a.entry_form=26 and b.transaction_type=3 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.prod_id=$cr_prod_id group by a.received_id, b.prod_id, c.item_group_id");
			...
			//echo "30** $prev_rcv_qnty.";die;
			if($rcv_basis==1)
			{
				$wo_pi_req_sql=sql_select(" select sum(b.quantity) as quantity, max(a.pi_date) as wo_pi_date from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and b.status_active=1 and b.pi_id=$wo_pi_req_id and b.item_prod_id=$cr_prod_id");
				$wo_pi_req_qnty=$wo_pi_req_sql[0][csf("quantity")];
				$wo_pi_date=$wo_pi_req_sql[0][csf("wo_pi_date")];
			}
			else if($rcv_basis==2)
			{
				$wo_pi_req_sql=sql_select(" select sum(b.supplier_order_quantity) as quantity, max(a.wo_date) as wo_pi_date from wo_non_order_info_mst a, wo_non_order_info_dtls b where a.id=b.mst_id and b.status_active=1 and b.mst_id=$wo_pi_req_id and b.item_id=$cr_prod_id");
				$wo_pi_req_qnty=$wo_pi_req_sql[0][csf("quantity")];
				$wo_pi_date=$wo_pi_req_sql[0][csf("wo_pi_date")];
			}
			else
			{
				$wo_pi_req_sql=sql_select(" select sum(b.quantity) as quantity, max(a.requisition_date) as wo_pi_date from inv_purchase_requisition_mst a, inv_purchase_requisition_dtls b where a.id=b.mst_id and b.status_active=1 and b.mst_id=$wo_pi_req_id and b.product_id=$cr_prod_id");
				$wo_pi_req_qnty=$wo_pi_req_sql[0][csf("quantity")];
				$wo_pi_date=$wo_pi_req_sql[0][csf("wo_pi_date")];
			}
			
			
			if(strtotime(str_replace("'","",$txt_receive_date))< strtotime($wo_pi_date) )
			{
				echo "30** MRR Date Not Allow Less Then PI/Wo/Req Date.";disconnect($con);die;
			}
			
			if($variable_over_rcv_payment>0)
			{
		...
		{*/
		$txt_boe_mushak_challan_no = str_replace("'","",$txt_boe_mushak_challan_no);
		$txt_boe_mushak_challan_date = str_replace("'","",$txt_boe_mushak_challan_date);
		...
		if(str_replace("'","",$txt_mrr_no)!="")
		{
			...
			$field_array1="receive_basis*receive_date*booking_id*challan_no*store_id*exchange_rate*currency_id*supplier_id*lc_no*pay_mode*source*supplier_referance*remarks*store_sl_no*boe_mushak_challan_no*boe_mushak_challan_date*rcvd_book_no*addi_challan_date*bill_no*bill_date*purchaser_name*carried_by*qc_check_by*receive_by*gate_entry_by*gate_entry_date*addi_rcvd_date*gate_entry_no*updated_by*update_date*receive_purpose*loan_party";
			$data_array1="".$cbo_receive_basis."*".$txt_receive_date."*".$txt_wo_pi_req_id."*".$txt_challan_no."*".$cbo_store_name."*".$txt_exchange_rate."*".$cbo_currency."*".$cbo_supplier."*".$hidden_lc_id."*".$cbo_pay_mode."*".$cbo_source."*".$txt_sup_ref."*".$txt_remarks."*".$txt_store_sl_no."*'".$txt_boe_mushak_challan_no."'*'".$txt_boe_mushak_challan_date."'*'".$txt_book_no."'*'".$txt_challan_date."'*'".$txt_bill_no."'*'".$txt_bill_date."'*'".$cbo_purchaser_name."'*'".$cbo_carried_by."'*'".$cbo_qc_check_by."'*'".$cbo_receive_by."'*'".$cbo_gate_entry_by."'*'".$txt_gate_entry_date."'*'".$txt_addi_rcvd_date."'*'".$txt_gate_entry_no."'*'".$user_id."'*'".$pc_date_time."'*'".str_replace("'","",$cbo_receive_purpose)."'*'".str_replace("'","",$cbo_loan_party)."'";
		...
		{	
			...
			$field_array1="id, recv_number_prefix, recv_number_prefix_num, recv_number, entry_form, company_id, receive_basis, receive_date, booking_id, challan_no, store_id, exchange_rate, currency_id, supplier_id, lc_no, pay_mode, source, supplier_referance, remarks,  store_sl_no, boe_mushak_challan_no, boe_mushak_challan_date, rcvd_book_no,addi_challan_date,bill_no,bill_date,purchaser_name,carried_by,qc_check_by,receive_by,gate_entry_by,gate_entry_date,addi_rcvd_date,gate_entry_no, inserted_by, insert_date,receive_purpose,loan_party";
			$data_array1="(".$id.",'".$new_recv_number[1]."','".$new_recv_number[2]."','".$new_recv_number[0]."',20,".$cbo_company_name.",".$cbo_receive_basis.",".$txt_receive_date.",".$txt_wo_pi_req_id.",".$txt_challan_no.",".$cbo_store_name.",".$txt_exchange_rate.",".$cbo_currency.",".$cbo_supplier.",".$hidden_lc_id.",".$cbo_pay_mode.",".$cbo_source.",".$txt_sup_ref.",".$txt_remarks.",".$txt_store_sl_no.",'".$txt_boe_mushak_challan_no."','".$txt_boe_mushak_challan_date."','".$txt_book_no."','".$txt_challan_date."','".$txt_bill_no."','".$txt_bill_date."','".$cbo_purchaser_name."','".$cbo_carried_by."','".$cbo_qc_check_by."','".$cbo_receive_by."','".$cbo_gate_entry_by."','".$txt_gate_entry_date."','".$txt_addi_rcvd_date."','".$txt_gate_entry_no."','".$user_id."','".$pc_date_time."','".str_replace("'","",$cbo_receive_purpose)."','".str_replace("'","",$cbo_loan_party)."')";
		...
		$field_array_trams = "id,mst_id,receive_basis,pi_wo_batch_no,company_id,supplier_id,prod_id,item_category,transaction_type,transaction_date,store_id,order_uom,order_qnty,order_rate,order_ile,order_ile_cost,order_amount,cons_uom,cons_quantity,cons_rate,cons_ile,cons_ile_cost,cons_amount,balance_qnty,balance_amount,floor_id,room,rack,self,bin_box,expire_date,manufacture_date,remarks,inserted_by,insert_date,batch_lot,store_rate,store_amount";
 		$data_array_trans = "(".$dtlsid.",".$id.",".$cbo_receive_basis.",".$txt_wo_pi_req_id.",".$cbo_company_name.",".$cbo_supplier.",".$current_prod_id.",".$cbo_item_category.",1,".$txt_receive_date.",".$cbo_store_name.",".$cbo_uom.",".$txt_receive_qty.",".number_format($txt_rate,10,".","").",".$ile.",".$ile_cost.",".number_format($txt_amount,8,".","").",".$cons_uom.",".$con_quantity.",".number_format($cons_rate,10,".","").",".$con_ile.",".$con_ile_cost.",".number_format($con_amount,8,".","").",".$con_quantity.",".number_format($con_amount,8,".","").",".$cbo_floor.",".$txt_room.",".$txt_rack.",".$txt_self.",".$txt_binbox.",".$txt_warranty_date.",'".$txt_manufac_date."',".$txt_referance.",'".$user_id."','".$pc_date_time."','".$txt_lot."',".number_format($cons_rate,10,".","").",".number_format($con_amount,8,".","").")";
		...
		{
			$sql_store = sql_select("select id, rate as avg_rate_per_unit, cons_qty as current_stock, amount as stock_value from inv_store_wise_gen_qty_dtls where status_active=1 and prod_id=$current_prod_id and category_id=$cbo_item_category and store_id=$cbo_store_name and company_id=$cbo_company_name");
			$store_up_id=0;
			if(count($sql_store)<1)
			{
				...
				$data_array_store= "(".$sdtlsid.",".$cbo_company_name.",".$cbo_store_name.",".$cbo_item_category.",".$current_prod_id.",".$con_quantity.",".number_format($cons_rate,10,".","").",".number_format($con_amount,8,".","").",".$con_quantity.",'".$_SESSION['logic_erp']['user_id']."','".$pc_date_time."',".$txt_lot.",".$txt_receive_date.",".$txt_receive_date.")";
			}
			else
			{
				if(count($sql_store)>1)
		...
		//echo "10** $rID && $dtlsrID && $prodUpdate && $serial_dtlsrID && $storeRID" ;die;
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		else
		{
			...
			if($update_id!="")
			{
				...
				$field_array_receive="receive_basis*receive_date*booking_id*challan_no*store_id*exchange_rate*currency_id*supplier_id*lc_no*pay_mode*source*supplier_referance*remarks*store_sl_no*boe_mushak_challan_no*boe_mushak_challan_date*rcvd_book_no*addi_challan_date*bill_no*bill_date*purchaser_name*carried_by*qc_check_by*receive_by*gate_entry_by*gate_entry_date*addi_rcvd_date*gate_entry_no*updated_by*update_date*receive_purpose*loan_party";
				$data_array_receive="".$cbo_receive_basis."*".$txt_receive_date."*".$txt_wo_pi_req_id."*".$txt_challan_no."*".$cbo_store_name."*".$txt_exchange_rate."*".$cbo_currency."*".$cbo_supplier."*".$hidden_lc_id."*".$cbo_pay_mode."*".$cbo_source."*".$txt_sup_ref."*".$txt_remarks."*".$txt_store_sl_no."*'".$txt_boe_mushak_challan_no."'*'".$txt_boe_mushak_challan_date."'*'".$txt_book_no."'*'".$txt_challan_date."'*'".$txt_bill_no."'*'".$txt_bill_date."'*'".$cbo_purchaser_name."'*'".$cbo_carried_by."'*'".$cbo_qc_check_by."'*'".$cbo_receive_by."'*'".$cbo_gate_entry_by."'*'".$txt_gate_entry_date."'*'".$txt_addi_rcvd_date."'*'".$txt_gate_entry_no."'*'".$user_id."'*'".$pc_date_time."'*'".str_replace("'","",$cbo_receive_purpose)."'*'".str_replace("'","",$cbo_loan_party)."'";
				...
				$field_array_trans = "receive_basis*pi_wo_batch_no*company_id*supplier_id*prod_id*item_category*transaction_type*transaction_date*store_id*order_uom*order_qnty*order_rate*order_ile*order_ile_cost*order_amount*cons_uom*cons_quantity*cons_rate*cons_ile*cons_ile_cost*cons_amount*balance_qnty*balance_amount*floor_id*room*rack*self*bin_box*expire_date*manufacture_date*remarks*updated_by*update_date*batch_lot*store_rate*store_amount";
				$data_array_trans = "".$cbo_receive_basis."*".$txt_wo_pi_req_id."*".$cbo_company_name."*".$cbo_supplier."*".$current_prod_id."*".$cbo_item_category."*1*".$txt_receive_date."*".$cbo_store_name."*".$cbo_uom."*".$txt_receive_qty."*".number_format($txt_rate,10,".","")."*".$ile."*".$ile_cost."*".number_format($txt_amount,8,".","")."*".$cons_uom."*".$con_quantity."*".number_format($cons_rate,10,".","")."*".$con_ile."*".$con_ile_cost."*".number_format($con_amount,8,".","")."*".$con_quantity."*".number_format($con_amount,8,".","")."*".$cbo_floor."*".$txt_room."*".$txt_rack."*".$txt_self."*".$txt_binbox."*".$txt_warranty_date."*'".$txt_manufac_date."'*".$txt_referance."*'".$user_id."'*'".$pc_date_time."'*'".$txt_lot."'*".number_format($cons_rate,10,".","")."*".number_format($con_amount,8,".","")."";
	...
	else if ($operation==2) // Delete Here----------------------------------------------------------
	{
		...
		else 
		{
			...
			else
			{
				...
				$rID1=sql_update("inv_transaction",$field_array_trans,$data_array_trans,"id",$update_id,1);
		...
		// echo "101**".$rID."**".$rID1."**".$rID2."**".$storeRID; die;
		if($db_type==0)
		{
			if($rID && $rID1 && $rID2 && $storeRID)
			{
				mysql_query("COMMIT");  
	...
	<table width="880" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="1">
            ...
            <tbody>
                <tr>
                    ...
                    <td align="center">
                        <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_supplier').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $company; ?>+'_'+'<? echo $item_category; ?>', 'create_mrr_search_list_view', 'search_div', 'general_item_receive_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />				
                    </td>
            </tr>
        	<tr>                  
            	<td align="center" height="40" valign="middle" colspan="5">
...
if($action=="create_mrr_search_list_view")
{
	...
	where a.id=b.mst_id and a.entry_form=20 and b.transaction_type=1 and b.status_active=1 and b.item_category in($item_category) and a.status_active=1 and a.is_deleted=0 $sql_cond $credientian_cond 
	group by a.id, b.mst_id,a.recv_number,a.supplier_id,a.challan_no,c.lc_number,a.receive_date,a.receive_basis 
	order by b.mst_id";
	//echo $sql;
	$supplier_arr = return_library_array( "select id, supplier_name from lib_supplier",'id','supplier_name');
...
if($action=="populate_data_from_data")
{
	...
	{		
		...
		{
			echo"load_drop_down( 'requires/general_item_receive_controller', ".$row[csf("company_id")].", 'load_drop_down_loan_party', 'loan_party_td' );\n";
		...
		echo "show_list_view('".$row[csf("id")]."','show_dtls_list_view','list_container','requires/general_item_receive_controller','');\n";
...
if($action=="show_dtls_list_view")
{
	$sql = "select a.recv_number, b.id, b.receive_basis,b.pi_wo_batch_no,c.product_name_details,c.lot,b.order_uom,b.order_qnty,b.order_rate,b.order_ile_cost,b.order_amount,b.cons_amount, c.item_size, c.item_category_id  
		...
		where a.id=b.mst_id and b.prod_id=c.id and b.transaction_type=1 and a.status_active=1 and b.status_active=1 and c.status_active in(1,3) and a.entry_form=20 and a.id=$data order by a.id desc";
	//echo $sql;die;		
	$result = sql_select($sql);
	$i=1;
	$totalQnty=0;
...
if($action=="child_form_input_data")
{
	...
	$sql = "SELECT a.company_id, a.currency_id, a.booking_id, a.receive_basis, a.exchange_rate, b.id, b.pi_wo_batch_no, b.prod_id, b.brand_id, c.lot, b.order_uom, b.order_qnty, b.order_rate, b.order_ile_cost, b.order_amount, b.cons_amount, b.expire_date, b.manufacture_date, b.store_id, b.floor_id, b.room, b.rack, b.self, b.bin_box, c.item_category_id, c.item_group_id, c.item_description, c.current_stock as global_stock, b.remarks, c.brand_name, c.origin, c.model, b.batch_lot
	from inv_receive_master a, inv_transaction b, product_details_master c  
	...
	{		
		...
		/*echo "load_drop_down('requires/general_item_receive_controller', '".$row[csf("company_id")].'_'.'4'."', 'load_drop_down_store','store_td');\n";
		echo "document.getElementById('cbo_store_name').value 		= '".$row[csf("store_id")]."';\n";*/
		$floor = (str_replace("'", "", $row[csf("floor_id")]) == "") ? 0 : $row[csf("floor_id")] ;
		$room = (str_replace("'", "", $row[csf("room")]) == "") ? 0 : $row[csf("room")] ;
		$rack = (str_replace("'", "", $row[csf("rack")]) == "") ? 0 : $row[csf("rack")] ;
		$self = (str_replace("'", "", $row[csf("self")]) == "") ? 0 : $row[csf("self")] ;
		$bin_box = (str_replace("'", "", $row[csf("bin_box")]) == "") ? 0 : $row[csf("bin_box")] ;

		echo "load_drop_down('requires/general_item_receive_controller','".$row[csf("store_id")].'_'.$row[csf("company_id")]."', 'load_drop_floor','to_floor_td');\n";
		echo "document.getElementById('cbo_floor').value 				= '".$floor."';\n";
		if($floor)
		{
			echo "load_drop_down('requires/general_item_receive_controller','".$floor.'_'.$row[csf("company_id")].'_'.$row[csf("store_id")]."', 'load_drop_room','to_room_td');\n";
		}
		echo "document.getElementById('txt_room').value 				= '".$room."';\n";
		if($room)
		{
			echo "load_drop_down('requires/general_item_receive_controller','".$room.'_'.$row[csf("company_id")].'_'.$row[csf("store_id")]."', 'load_drop_rack','to_rack_td');\n";
		}
		echo "document.getElementById('txt_rack').value 				= '".$rack."';\n";
		if($rack)
		{
			echo "load_drop_down('requires/general_item_receive_controller','".$rack.'_'.$row[csf("company_id")].'_'.$row[csf("store_id")]."', 'load_drop_shelf','to_shelf_td');\n";
		}
		echo "document.getElementById('txt_self').value 				= '".$self."';\n";
		if($self)
		{
			echo "load_drop_down('requires/general_item_receive_controller','".$self.'_'.$row[csf("company_id")].'_'.$row[csf("store_id")]."', 'load_drop_bin','to_bin_td');\n";
		}
		
		echo "document.getElementById('txt_binbox').value 				= '".$bin_box."';\n";
		...
		echo "disable_enable_fields( 'txt_receive_date*txt_challan_no*txt_exchange_rate*txt_store_sl_no', 0, '', '');\n";
		echo "disable_enable_fields( 'cbo_item_category*cbo_item_group*txt_rate', 1, '', '');\n";
		echo "fn_calile();\n";
	}
	exit();
}
...
if ($action=="general_item_receive_print")
{
	...
	$pi_arr=return_library_array( "select id, pi_number from  com_pi_master_details where item_category_id in(".implode(",",array_flip($general_item_category)).")", "id", "pi_number"  );
	...
	<div style="width:100%;">
		<table align="right" cellspacing="0" width="900"  border="1" rules="all" class="rpt_table" style="margin-bottom:15px;">
            <thead bgcolor="#dddddd" align="center">
                ...
                <th width="80" align="center">Lot</th> 
...
?>
    ...
    <table width="950" cellspacing="0" align="right">
        ...
        <tr>
        	<td width="130"><strong>MRR Number:</strong></td><td width="220"><? echo $dataArray[0][csf('recv_number')]; ?></td>
            <td width="130"><strong>Receive Basis :</strong></td> <td width="220"><? echo $receive_basis_arr[$dataArray[0][csf('receive_basis')]]; ?></td>
            <td width="130"><strong>Receive Date:</strong></td><td ><? echo change_date_format($dataArray[0][csf('receive_date')]); ?></td>
        </tr>
        <tr>
        ...
        <tr>
			...
			<td><strong>MRR Insert Date:</strong></td><td ><? echo change_date_format($dataArray[0][csf('insert_date')]); ?></td>
        </tr>
        <tr>       
			<td valign="top"><strong>Remarks:</strong></td><td ><? echo $dataArray[0][csf('remarks')]; ?></td>
	...
	if($db_type==2)
	{
	  $sql_dtls = "select a.id as recv_id, a.booking_id, b.item_category, b.id, b.receive_basis, b.pi_wo_batch_no, b.order_uom, b.order_qnty, b.order_rate, b.order_amount, b.cons_amount, b.balance_qnty, b.expire_date, b.batch_lot, b.prod_id, b.remarks, b.prod_id,
	...
	{
		$all_prod_ids_arr[$val[csf("prod_id")]]=$val[csf("prod_id")];
	}
	
	$item_grp_sql=sql_select("select b.id, a.conversion_factor from lib_item_group a, product_details_master b where a.id=b.item_group_id and b.id in(".implode(",",$all_prod_ids_arr).")");
	$item_conversion_factor=array();
	foreach($item_grp_sql as $row)
	{
		$item_conversion_factor[$row[csf("id")]]=$row[csf("conversion_factor")];
	...
	if($dataArray[0][csf('receive_basis')]==2 || $dataArray[0][csf('receive_basis')]==1 || $dataArray[0][csf('receive_basis')]==7)
	{
		...
		$receive_return_sql = sql_select("select a.received_id, b.prod_id, c.item_group_id, sum(b.cons_quantity) as issue_qnty 
		from  inv_issue_master a, inv_transaction b, product_details_master c 
		where a.id=b.mst_id and b.prod_id=c.id and a.status_active=1 and b.transaction_type=3 and a.received_id>0 and a.entry_form=26 and b.transaction_type=3 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.prod_id in(".implode(",",$all_prod_ids_arr).")
		group by a.received_id, b.prod_id, c.item_group_id");
		
		foreach($receive_return_sql as $row)
		{
			$receive_rtn_arr[$row[csf("received_id")]]+=$row[csf("issue_qnty")]/$item_conversion_factor[$row[csf("prod_id")]];
		}
		
		//echo "<jpre>";print_r($receive_rtn_arr);die;
		
		$order_prev_sql=sql_select("select a.id, a.booking_id, b.prod_id,sum(b.order_qnty) as wo_prev_qnty from inv_receive_master a, inv_transaction b where a.id=b.mst_id and a.booking_id='".$dataArray[0][csf('booking_id')]."' and a.id !='".$dataArray[0][csf('id')]."' and a.status_active=1 and b.status_active=1 
		group by a.id, a.booking_id, b.prod_id");
		foreach($order_prev_sql as $row)
		{
			$order_prev_qnty_arr[$row[csf("booking_id")]][$row[csf("prod_id")]]+=$row[csf("wo_prev_qnty")]-$receive_rtn_arr[$row[csf("id")]];
		}
	}
...
<div style="width:100%;">
	...
	foreach($sql_result as $row)
	{
      ...
      <table>
        ...
        <br>
		 <?
            echo signature_table(11, $data[0], "970px");
         ?>
      </div>
   </div> 
