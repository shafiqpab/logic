...
Updated by 		: 	Kausar / Jahid		/ Reza	/ Didar
Update date		: 	29-10-2013	 (Creating report	), 16-10-2016 (Store Sl No field add)
QC Performed BY	:	
QC Date			:	
Comments		:
*/
...
<?
if($_SESSION['logic_erp']['mandatory_field'][592]!="")
{
	$mandatory_field_arr= json_encode( $_SESSION['logic_erp']['mandatory_field'][592] );
...
function fnc_general_item_receive_entry(operation)
{
	...
	else if(operation==0 || operation==1 || operation==2)
	{
		...
		if('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][592]); ?>') 
		{
			if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][592]); ?>','<? echo implode('*',$_SESSION['logic_erp']['mandatory_message'][592]); ?>')==false) {release_freezing();return;}
		}
...
{	
	...
	{	  		
		...
		else if(reponse[0]==0)
		{
			...
			var receive_basis=$('#cbo_receive_basis').val();
			var rowID=$('#txt_wo_pi_req_id').val();
...
function open_mrrpopup()
{
	...
	var company = $("#cbo_company_name").val();
	var page_link='requires/general_item_receive_controller.php?action=mrr_popup&company='+company+'&item_category=11'; 
	...
	emailwindow.onclose=function()
	{
		...
		{
			show_list_view($("#cbo_receive_basis").val()+"**"+$("#txt_wo_pi_req_id").val()+"**11",'show_product_listview','list_product_container','requires/general_item_receive_controller','');
		}
		set_button_status(0, permission, 'fnc_general_item_receive_entry',1,1);
 	}
}
...
<div style="width:100%;">
    ...
    <form name="genralitem_receive_1" id="genralitem_receive_1" autocomplete="off" data-entry_form="20"> 
                 	...
                 	<fieldset style="width:920px;">                                       
                        <table  width="900" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                           ...
                           <tr>
                                ...
                                <td width="170">
                                <? 
                                echo create_drop_down( "cbo_company_name", 170, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond $company_credential_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/general_item_receive_controller', this.value, 'load_drop_down_supplier', 'supplier' );load_drop_down( 'requires/general_item_receive_controller', this.value+'_'+11, 'load_drop_down_store', 'store_td' ); independence_basis_controll_function(this.value);" );
                                ?>
                                </td>
                                ...
                                <td width="170"><input type="text" name="txt_receive_date" id="txt_receive_date" class="datepicker" style="width:160px;" placeholder="Select Date" value="<? echo date("d-m-Y",time());?>" onChange="check_exchange_rate();" /></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td width="" align="">Store SL No</td>
                                <td><input type="text" name="txt_store_sl_no" id="txt_store_sl_no" class="text_boxes" style="width:160px" value="" /></td>
                            </tr>
                             <tr>
                                ...
                                <td ><input class="text_boxes"  type="text" name="txt_wo_pi_req" id="txt_wo_pi_req" onDblClick="openmypage('xx','Order Search','11')"  placeholder="Double Click" style="width:160px;"  readonly disabled />
                            ...
                            <tr>
                                ...
                                <td><input type="text" name="txt_exchange_rate" id="txt_exchange_rate" class="text_boxes_numeric" style="width:160px" value="<? echo $exchangeRate; ?>" onBlur="fn_calile()" readonly /></td> 
                                <td >Addi. Info</td>
                                <td>
                    ...
                    <td width="49%" valign="top">
                        ...
                        <table width="250" cellspacing="2" cellpadding="0" border="0" style="float:left"> 
                                <tr>    
                                        ...
                                        <td width="130">         
                                            <?
                                                echo create_drop_down( "cbo_item_category", 130, $item_category,"", 1, "-- Select --", 0, "load_drop_down( 'requires/general_item_receive_controller', this.value, 'load_drop_down_itemgroup', 'item_group_td' );", 0,"11" );
                                            ?>
                                        </td>
                                </tr>
                                <tr>    
                                        ...
                                        <?
                                            ...
                                            echo create_drop_down( "cbo_item_group", 130, "select id,item_name from lib_item_group where item_category in (4,8,9,10,11,15,16,17,18,19,20,21,22) and status_active=1 and is_deleted=0 order by item_name", "id,item_name", 1, "-- Select --", 0, "", $disabled,"" );
                                         ?>	
                                        </td>
                                </tr>
                                <tr>    
