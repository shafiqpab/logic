...
<?
if($_SESSION['logic_erp']['mandatory_field'][590]!="")
{
	$mandatory_field_arr= json_encode( $_SESSION['logic_erp']['mandatory_field'][590] );
...
function fnc_general_item_receive_entry(operation)
{
	...
	else if(operation==0 || operation==1 || operation==2)
	{
		...
		if('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][590]); ?>') 
		{
			if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][590]); ?>','<? echo implode('*',$_SESSION['logic_erp']['mandatory_message'][590]); ?>')==false) {release_freezing();return;}
		}
...
function open_mrrpopup()
{
	...
	var company = $("#cbo_company_name").val();
	var page_link='requires/general_item_receive_controller.php?action=mrr_popup&company='+company+'&item_category=4';
	...
	emailwindow.onclose=function()
	{
		...
		{
			show_list_view($("#cbo_receive_basis").val()+"**"+$("#txt_wo_pi_req_id").val()+"**4",'show_product_listview','list_product_container','requires/general_item_receive_controller','');
		}
		set_button_status(0, permission, 'fnc_general_item_receive_entry',1,1);
 	}
}
                 	...
                 	<fieldset style="width:900px;">
                        <table  width="900" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                           ...
                           <tr>
                                ...
                                <td width="170">
                                <?
                                echo create_drop_down( "cbo_company_name", 170, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond $company_credential_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/general_item_receive_controller', this.value, 'load_drop_down_supplier', 'supplier' );load_drop_down( 'requires/general_item_receive_controller', this.value+'_'+4, 'load_drop_down_store', 'store_td' ); independence_basis_controll_function(this.value);" );
                                ?>
                                </td>
                            ...
                            <tr>
                             ...
                             <tr>
                                ...
                                <td ><input class="text_boxes"  type="text" name="txt_wo_pi_req" id="txt_wo_pi_req" onDblClick="openmypage('xx','Order Search','4')"  placeholder="Double Click" style="width:160px;"  readonly disabled />
		                   	  ...
		                   	  <fieldset style="width:950px;">
			                        ...
			                        <table width="250" cellspacing="2" cellpadding="0" border="0" style="float:left">
		                                <tr>
	                                        ...
	                                        <td width="130">
	                                            <?
	                                                echo create_drop_down( "cbo_item_category", 130, $item_category,"", 1, "-- Select --", 0, "load_drop_down( 'requires/general_item_receive_controller', this.value, 'load_drop_down_itemgroup', 'item_group_td' );", 0,"4" );
	                                            ?>
	                                        </td>
		                                </tr>
		                                <tr>
