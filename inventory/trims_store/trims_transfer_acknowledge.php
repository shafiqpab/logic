...
function fnc_transfer_acknowledgement( operation )
{
	...
	{  
		...
		http.open("POST","requires/trims_transfer_acknowledge_controller.php",true);
...
{	
	...
	{	  		
		...
		if(response[0]==0 || response[0]==1)
		{
			...
			show_list_view(response[1],'show_dtls_list_view_update','payDtlsTableTbody','requires/trims_transfer_acknowledge_controller','');
...
function openmypage_challan()
{
	...
	var page_link='requires/trims_transfer_acknowledge_controller.php?action=itemTransfer_popup&company='+company+'&location='+location+'&store='+store;
	...
	emailwindow.onclose=function()
	{
		...
		if(transfer_id!='')
		{
			...
			get_php_form_data(transfer_data_str, "populate_data_from_transfer_master", "requires/trims_transfer_acknowledge_controller" );
			show_list_view(transfer_id,'show_dtls_list_view','payDtlsTableTbody','requires/trims_transfer_acknowledge_controller','');	
		}
	}
}
...
function openmypage_systemId()
{
	...
	var page_link = 'requires/trims_transfer_acknowledge_controller.php?cbo_company_id_to='+cbo_company_id_to+'&action=itemAcknowle_popup';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(transfer_id, "populate_data_from_transfer_master_update", "requires/trims_transfer_acknowledge_controller" );
		show_list_view(transfer_id,'show_dtls_list_view_update','payDtlsTableTbody','requires/trims_transfer_acknowledge_controller','');
		set_button_status(1, permission, 'fnc_transfer_acknowledgement',1,1);
	}
}
...
function fnc_floor_load(datas){
	...
	for (var i = 1;numRow>=i; i++) {
		...
		load_drop_down( 'requires/trims_transfer_acknowledge_controller',storeId+'_'+company+'_'+location+'_'+i, 'load_drop_down_floor_to', floor_tds );
	}
}
