...
{
	load_room_rack_self_bin("requires/trims_transfer_acknowledge_controller",$data);
}

if ($action=="load_drop_down_location_to")
{ 
	$data=explode("_",$data);
	echo create_drop_down( "cbo_location_to", 160, "select id,location_name from lib_location where company_id='$data[0]' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", 0, "load_drop_down( 'requires/trims_transfer_acknowledge_controller',this.value+'_'+$data[0], 'load_drop_down_store_to', 'to_store_td' );" );	
	exit();
}
...
if ($action=="load_drop_down_location_to_up")
{
	$data=explode("_",$data);
		...
		load_room_rack_self_bin('trims_transfer_acknowledge_controller*4*cbo_store_name_to', 'store','to_store_td', $data[0],this.value);" );
	exit();
}
...
if ($action=="load_drop_down_store_to")
{
	...
	echo create_drop_down( "cbo_store_name_to", 160, "select a.id, a.store_name from lib_store_location a, lib_store_location_category b where a.id=b.store_location_id and a.company_id=$company and a.location_id=$location and a.status_active=1 and a.is_deleted=0 and b.category_type in(4) $store_location_credential_cond group by a.id, a.store_name order by a.store_name","id,store_name", 1, "-- Select Store --", 0, "fnc_floor_load(this.value+'_'+$company+'_'+$location);reset_room_rack_shelf('','cbo_store_name_to');" );
	exit();
}
...
if($action=='itemTransfer_popup')
{
	...
	where b.item_category=4 and a.to_company=$company $location_cond $store_cond and a.id=b.mst_id and a.entry_form in(112,78) and a.transfer_criteria in(1,2,4) and b.is_acknowledge=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
...
if($action=='populate_data_from_transfer_master')
{
	...
	{ 
		echo "load_drop_down('requires/trims_transfer_acknowledge_controller','".$row[csf("to_company")]."', 'load_drop_down_location_to', 'to_location_td' );\n";
		...
		if($row[csf("to_store")]>0)
		{
			...
			echo "load_drop_down('requires/trims_transfer_acknowledge_controller','".$loc_com."', 'load_drop_down_store_to', 'to_store_td' );\n";
			echo "document.getElementById('cbo_store_name_to').value 				= '".$row[csf("to_store")]."';\n";

			//echo "load_room_rack_self_bin('requires/trims_transfer_acknowledge_controller*3*cbo_store_name_to', 'store','to_store_td', '".$row[csf("to_company")]."','".$row[csf("to_location_id")]."',this.value);\n";
			//echo "document.getElementById('cbo_store_name_to').value 				= '".$row[csf("to_store")]."';\n";
		}
	...
	foreach($data_array as $row)
	{
        ...
        <tr id="dtlsTbodyTr_<?php echo $i; ?>">
            ...
            <td id="floor_td_to_<? echo $i;?>">
                <? 
                	echo create_drop_down( "cbo_floor_to_$i", 152,$floor_arr,"", 1, "--Select--",$row[csf('to_floor_id')], "load_drop_down( 'requires/trims_transfer_acknowledge_controller',this.value+'_'+$i+'_'+$company+'_'+$location+'_'+$store, 'load_drop_down_room_to', 'room_td_$i' );reset_room_rack_shelf($i,'cbo_floor_to');",0,"","","","","","","cboFloorTo[]" );
                ?>
            </td>
...
{ 
	...
	if ($operation==0)  // Insert Here
	{
		...
		$data_array="(".$id.",386,".$challan_id.",".$cbo_company_id_to.",".$cbo_store_name_to.",".$cbo_location_to.",".$cbo_transfer_criteria.",".$cbo_item_category.",".$txt_transfer_date.",'".$txt_remarks."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		for($i=1;$i<=$numRow;$i++)
		{
			...
			$conversion_factor=return_field_value("b.conversion_factor as conversion_factor","product_details_master a, lib_item_group b","a.item_group_id=b.id and a.item_category_id=4 and a.entry_form=24 and a.id='".str_replace("'","",$$productID)."' ","conversion_factor");
			// ========================================================================================
			...
			else
			{
				...
				$data_array_ord_prod.="(".$ord_prod_id.",".$cbo_company_id_to.",4,'".str_replace("'","",$$productID)."','".str_replace("'","",$$txtOrderID)."','".str_replace("'","",$order_trans_qnty)."','".str_replace("'","",$order_trans_qnty)."','".$avg_rate_to."','".$order_amount_to."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			}
			// Proportion and order wise stock end
			// =================================================================== End 
		...
		//echo "10**".$rID."**".$rID2."**".$rID3."**".$rID4."**".$rID5."**".$ordProdUpdateTo."**".$ordProdInsert."**".$prod."**".$flag;die;
	...
	?> 
		<script>		
			...
			function load_location()
			{
				...
				load_drop_down('trims_transfer_acknowledge_controller',cbo_company_id_to, 'load_drop_down_location_to_up', 'to_location_td' );
			}
    	</script>
	</head>
...
{
	$data_array=sql_select("SELECT a.id, a.entry_form, a.challan_id, a.company_id, a.store_id, a.location_id, a.transfer_criteria, a.item_category, a.acknowledg_date, a.remarks, b.transfer_system_id from inv_item_trans_acknowledgement a, inv_item_transfer_mst b where a.id='$data' and a.challan_id=b.id and a.entry_form=386 and a.transfer_criteria in(1,2,4) and a.status_active=1 and a.is_deleted=0");
	foreach ($data_array as $row)
	{ 
		echo "load_drop_down('requires/trims_transfer_acknowledge_controller','".$row[csf("company_id")]."', 'load_drop_down_location_to', 'to_location_td' );\n";
		echo "document.getElementById('cbo_location_to').value 				= '".$row[csf("location_id")]."';\n";

		if($row[csf("store_id")]>0){
			echo "load_room_rack_self_bin('requires/trims_transfer_acknowledge_controller*4*cbo_store_name_to', 'store','to_store_td', '".$row[csf("company_id")]."','".$row[csf("location_id")]."',this.value);\n";
