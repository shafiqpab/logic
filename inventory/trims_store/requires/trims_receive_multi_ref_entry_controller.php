					...
					<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="670" id="tbl_list_search">  
						...
						else
						{
							...
							{  
								...
								if(trim(str_replace("'","",$item_description)) !="" || $db_type==0)
								{
									$description_cond_rcv=" and trim(REGEXP_REPLACE((case when a.item_description like '%, [BS]' then SUBSTR(a.item_description, 0, LENGTH(a.item_description) - 6) else a.item_description end), '\s{2,}', ' '))=REGEXP_REPLACE('".trim(str_replace("'","",$item_description))."', '\s{2,}', ' ')";
								}
								else
								{
...
if($action=="put_balance_qnty")
{
	...
	if($recieve_basis==1 || $recieve_basis==2)
	{
		...
		if($recieve_basis==1 || $recieve_basis==2)
		{
			...
			if($db_type==0)
			{
				$receive_qnty=return_field_value("sum(b.quantity) as qnty","inv_receive_master m, inv_trims_entry_dtls a, order_wise_pro_details b","m.id=a.mst_id and a.id=b.dtls_id and b.entry_form=24 and m.entry_form=24 and b.po_breakdown_id in($order_id) and m.receive_basis=$recieve_basis and m.booking_id='$bookingNo_piId' and a.item_group_id='$item_group' and trim(REGEXP_REPLACE((case when a.item_description like '%, [BS]' then SUBSTR(a.item_description, 0, LENGTH(a.item_description) - 6) else a.item_description end), '\s{2,}', ' '))=trim(REGEXP_REPLACE('".str_replace("'","",$item_description)."', '\s{2,}', ' ')) and a.brand_supplier='".trim($brand_supref)."' and a.gmts_color_id='$gmts_color_id' and a.item_color='$item_color_id' and a.gmts_size_id ='$item_size_id' and a.item_size ='$item_size' $rate_condition and a.status_active=1 and a.is_deleted=0","qnty");
			}
			else
			{
				...
				if($booking_without_order==1)
				{
					$receive_qnty=return_field_value("sum(a.receive_qnty) as qnty","inv_receive_master m, inv_trims_entry_dtls a","m.id=a.mst_id and m.entry_form=24 and m.receive_basis=$recieve_basis and m.booking_id='$bookingNo_piId' and a.item_group_id='$item_group' and trim(REGEXP_REPLACE((case when a.item_description like '%, [BS]' then SUBSTR(a.item_description, 0, LENGTH(a.item_description) - 6) else a.item_description end), '\s{2,}', ' '))=trim(REGEXP_REPLACE('".str_replace("'","",$item_description)."', '\s{2,}', ' ')) and nvl(a.gmts_color_id,0)='$gmts_color_id' and nvl(a.item_color,0)='$item_color_id' and nvl(a.gmts_size_id,0)='$gmts_size_id' and $brand_supref_cond and $item_size_cond $rate_condition and a.status_active=1 and a.is_deleted=0","qnty");
				}
				else
				{
					$receive_qnty=return_field_value("sum(b.quantity) as qnty","inv_receive_master m, inv_trims_entry_dtls a, order_wise_pro_details b","m.id=a.mst_id and a.id=b.dtls_id and b.entry_form=24 and m.entry_form=24 and b.po_breakdown_id in($order_id) and m.receive_basis=$recieve_basis and m.booking_id='$bookingNo_piId' and a.item_group_id='$item_group' and trim(REGEXP_REPLACE((case when a.item_description like '%, [BS]' then SUBSTR(a.item_description, 0, LENGTH(a.item_description) - 6) else a.item_description end), '\s{2,}', ' '))=trim(REGEXP_REPLACE('".str_replace("'","",$item_description)."', '\s{2,}', ' ')) and nvl(a.gmts_color_id,0)='$gmts_color_id' and nvl(a.item_color,0)='$item_color_id' and nvl(a.gmts_size_id,0)='$gmts_size_id' and $brand_supref_cond and $item_size_cond $rate_condition and a.status_active=1 and a.is_deleted=0","qnty");
			...
			$sql_cond_prev="";
			if($item_description!="") $sql_cond_prev.="and trim(REGEXP_REPLACE((case when c.item_description like '%, [BS]' then SUBSTR(c.item_description, 0, LENGTH(a.item_description) - 6) else c.item_description end), '\s{2,}', ' '))=trim(REGEXP_REPLACE('".str_replace("'","",$item_description)."', '\s{2,}', ' '))";
...
if ($action=="save_update_delete")
{
	...
	if($rcv_basis==1 || $rcv_basis==2)
	{
		...
		$sql_cond_prev="";
		if(str_replace("'","",$hidden_item_description)!="") $sql_cond_prev.=" and trim(REGEXP_REPLACE((case when c.item_description like '%, [BS]' then SUBSTR(c.item_description, 0, LENGTH(a.item_description) - 6) else c.item_description end), '\s{2,}', ' '))=trim(REGEXP_REPLACE('".str_replace("'","",$hidden_item_description)."', '\s{2,}', ' '))";
 	...
 	foreach($delivary_data as $rows)
  	{
		...
		{
			//echo $key."<br>";
			$row_prod=sql_select("select id, current_stock, avg_rate_per_unit, stock_value from product_details_master where company_id=$cbo_company_id and item_category_id=4 and entry_form=24 and item_group_id=$cbo_item_group and trim(REGEXP_REPLACE((case when item_description like '%, [BS]' then SUBSTR(item_description, 0, LENGTH(item_description) - 6) else item_description end), '\s{2,}', ' '))=REGEXP_REPLACE('".trim(str_replace("'","",$item_desc))."', '\s{2,}', ' ') and item_color=$txt_item_color_id and item_size='$txt_item_size_id' and color=$gmts_color_id and gmts_size='$gmts_size_id' and status_active=1 and is_deleted=0");
