...
{
	$sql_variable_inventory=sql_select("select id, independent_controll, rate_optional, is_editable, rate_edit  from variable_settings_inventory where company_name=$data and variable_list=20 and status_active=1 and menu_page_id=112");
...
{
	$data=explode("_",$data);
	echo create_drop_down( "cbo_location", 160, "select id,location_name from lib_location where company_id='$data[0]' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", 0, "load_room_rack_self_bin('requires/trims_transfer_controller*4', 'store','from_store_td', $('#cbo_company_id').val(),this.value); " );
	//if( $('#cbo_transfer_criteria').val()*1==2 || $('#cbo_transfer_criteria').val()*1==4)  load_room_rack_self_bin('requires/trims_transfer_controller*4*cbo_store_name_to', 'store','to_store_td', $('#cbo_company_id').val(),this.value);
	exit();
}
...
if ($action=="load_drop_down_location_to")
{
	...
	if($data[1]==2){
		...
		echo create_drop_down( "cbo_location_to", 160, "select id,location_name from lib_location where company_id='$data[0]' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", 0, "load_room_rack_self_bin('requires/trims_transfer_controller*4*cbo_store_name_to', 'store','to_store_td', $('#cbo_company_id_to').val(),this.value);" );
	}
	exit();
}
...
?> 
	...
	<script>
		...
		function js_set_value(data)
		{
			...
			$('#floor_id').val(data_ref[2]);
			$('#room').val(data_ref[3]);
			$('#rack').val(data_ref[4]);
			$('#shelf').val(data_ref[5]);
			$('#bin').val(data_ref[6]);
		...
		function open_style_job_order(type,company,store)
		{
			...
			var page_link='trims_transfer_controller.php?action=style_job_order_popup&company='+company+'&type='+type+'&store='+store;
...
<div align="center" style="width:920px;">
	<form name="searchdescfrm"  id="searchdescfrm">
		<fieldset style="width:1120px;margin-left:10px;">
        <legend>Enter search words</legend>
            <table cellpadding="0" cellspacing="0" width="900" class="rpt_table" align="center" style=" margin-left:100px;">
                <thead>
                ...
                <tr class="general">
                    ...
                    <?
                      ...
                      echo create_drop_down( "cbo_item_group_id", 130,"select id,item_name  from lib_item_group where   status_active=1 order by item_name $item_category_cond","id,item_name", 1, "-- Select --", "", "","","","","","");
					?>
                    </td>
                    <td>
						<?
                    ...
                    <td>
                        <input type="text" style="width:100px;" class="text_boxes"  name="txt_style_no" id="txt_style_no" placeholder="Write/Browse" onDblClick="open_style_job_order(1,<? echo $cbo_company_id; ?>,<? echo $cbo_store_name; ?>)" onBlur="order_style_empty(1);" />
                    </td>
                    <td>
                        <input type="text" style="width:80px;" class="text_boxes"  name="txt_job_no" id="txt_job_no" placeholder="Write job No" onDblClick="open_style_job_order(2,<? echo $cbo_company_id; ?>,<? echo $cbo_store_name; ?>)" onBlur="order_style_empty(1);" />
                    </td>
                    <td>
                        <input type="text" style="width:100px;" class="text_boxes"  name="txt_order_no" id="txt_order_no" placeholder="Write Order No" onDblClick="open_style_job_order(3,<? echo $cbo_company_id; ?>,<? echo $cbo_store_name; ?>)" onBlur="order_style_empty(2);" />
                    </td>
                    <td>
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('cbo_item_group_id').value+'_'+<? echo $cbo_company_id; ?>+'_'+<? echo $cbo_store_name; ?>+'_'+document.getElementById('txt_style_no').value+'_'+document.getElementById('txt_order_no').value+'_'+document.getElementById('txt_job_no').value+'_'+<? echo $cbo_transfer_criteria; ?>, 'create_product_search_list_view', 'search_div', 'trims_transfer_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:80px;" />
                    </td>
                </tr>
            </table>
...
if($action=='create_product_search_list_view')
{
	...
	?>
    <table cellpadding="0" cellspacing="0" width="1590" class="rpt_table" border="1" rules="all" align="left">
    	<thead>
        	<tr>
            	<th width="40">SL</th>
    ...
    <div style="width:1608px; overflow-y:scroll; max-height:250px; float:left;" id="scroll_body">
    <table cellpadding="0" cellspacing="0" width="1590" class="rpt_table" border="1" rules="all" id="tbl_list_search" align="left">
    	<tbody>
        <?
		...
		foreach($sql_result as $row)
		{
			...
			$job_qnty=$conversion_factor[$row[csf("item_group_id")]]*$row[csf("job_balance")];
			
			$getSize=explode(",",$row[csf("product_name_details")]); 
			
			?>
        	<tr bgcolor="<? echo $bgcolor; ?>" onClick="js_set_value('<? echo $row[csf("id")].'_'.$row[csf("job_id")].'_'.$row[csf("floor_id")].'_'.$row[csf("room")].'_'.$row[csf("rack")].'_'.$row[csf("self")].'_'.$row[csf("bin_box")].'_'.$row[csf("po_id")].'_'.$row[csf("po_number")]; ?>')" id="tr_<? echo $i; ?>" style="cursor:pointer;">
            	<td width="40" align="center"><? echo $i; ?></td>
                <td width="80"><? echo $row[csf("id")]; ?></td>
                <td width="120"><p><? echo $company_arr[$row[csf("company_id")]]; ?></p></td>
                <td width="120" title="<? echo $row[csf("supplier_id")]; ?>"><p><? echo $supplier_arr[$row[csf("supplier_id")]]; ?></p></td>
                <td width="120"><p><? echo $item_group_arr[$row[csf("item_group_id")]]; ?></p></td>
                <td width="180"><p><? echo $row[csf("product_name_details")]; ?></p></td>
                <td width="100" align="center"><p><? echo $color_arr[$row[csf("color")]]; ?></p></td>
                <td width="100" align="center"><p><? echo $getSize[3];//$size_arr[$row[csf("item_size")]]; ?></p></td>
            ...
            <legend>Enter search words</legend>
                <table cellpadding="0" cellspacing="0" width="900" class="rpt_table">
                    ...
                    <tr class="general">
                        ...
                        <td align="center">
                            <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('cbo_item_group_id').value+'_'+<? echo $cbo_company_id; ?>+'_'+<? echo $cbo_store_name; ?>+'_'+<? echo $cbo_transfer_criteria; ?>, 'create_product_com_search_list_view', 'search_div', 'trims_transfer_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:80px;" />
                        </td>
                    </tr>
                </table>
                <div style="margin-top:10px" id="search_div"></div> 
...
if($action=='populate_data_from_product_master')
{
	...
	{
		$sql="SELECT a.id, a.product_name_details, a.current_stock, a.avg_rate_per_unit, a.supplier_id, a.unit_of_measure as itme_uom, a.item_group_id, 0 as job_id, null as job_no, null as style_ref_no, sum((case when b.transaction_type in(1,4,5) then b.cons_quantity else 0 end)-(case when b.transaction_type in(2,3,6) then b.cons_quantity else 0 end)) as job_balance, b.company_id, b.store_id, b.floor_id, b.room, b.rack, b.self, b.bin_box
		from product_details_master a, inv_transaction b  
	...
	else
	{
		if($order_no!="")
		{
			$sql="SELECT a.id, a.product_name_details, a.current_stock, a.avg_rate_per_unit, a.supplier_id, a.unit_of_measure as itme_uom, a.item_group_id, c.id as order_id, c.po_number, d.id as job_id, d.job_no, d.style_ref_no, sum((case when b.trans_type in(1,4,5) then quantity else 0 end)-(case when b.trans_type in(2,3,6) then quantity else 0 end)) as job_balance, e.company_id, e.store_id, e.floor_id, e.room, e.rack, e.self, e.bin_box
			from product_details_master a,  inv_transaction e, order_wise_pro_details b, wo_po_break_down c, wo_po_details_master d  
		...
		where a.id=b.prod_id and b.trans_id=c.id and a.item_category_id='4' and a.entry_form=24 and b.trans_type in(1,5) and b.entry_form in(24,78,112) and b.po_breakdown_id in ($po_id) and b.prod_id in($product_id) and c.prod_id in($product_id) and c.store_id=$store_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.current_stock>0";
		...
		foreach($sql_order_rate_result as $row)
		{
			if($trans_id_check[$row[csf("trans_id")]]=="")
			{
				...
				$order_item_data[$row[csf("po_breakdown_id")]]["cons_quantity"]+=$row[csf("cons_quantity")];
				$order_item_data[$row[csf("po_breakdown_id")]]["cons_amount"]+=$row[csf("cons_amount")];
			}
		}
	...
	//echo $sql;
	//$data_array=sql_select("select product_name_details, lot, current_stock, avg_rate_per_unit, brand,item_category_id, supplier_id, unit_of_measure as itme_uom from product_details_master where id='$data'");
	...
	foreach ($data_array as $row)
	{
		if($transfer_criteria==1)
		{
			...
			$stock_qnty=$row[csf("job_balance")];
		}
		else
		{
			...
			$order_rate=($order_item_data[$row[csf("order_id")]]["cons_amount"]/$order_item_data[$row[csf("order_id")]]["cons_quantity"]);
			$rate=number_format(($order_rate/$coversion_rate),6,".",""); 
			$stock_qnty=$row[csf("job_balance")]*$conversion_factor;
		}
		...
		echo "document.getElementById('cbo_uom').value 						= '".$row[csf("itme_uom")]."';\n";
		//
		...
		if($floor_id !=0)
		{
			echo "load_room_rack_self_bin('requires/trims_transfer_controller*4', 'floor','floor_td', '".$from_company_id."','"."','".$from_store_id."',this.value);\n";
			echo "document.getElementById('cbo_floor').value 				= '".$floor_id."';\n";
		}
		if($room !=0)
		{
			echo "load_room_rack_self_bin('requires/trims_transfer_controller*4', 'room','room_td', '".$from_company_id."','"."','".$from_store_id."','".$floor_id."',this.value);\n";
			echo "document.getElementById('cbo_room').value 				= '".$room."';\n";
		}
		if($rack !=0)
		{
			echo "load_room_rack_self_bin('requires/trims_transfer_controller*4', 'rack','rack_td', '".$from_company_id."','"."','".$from_store_id."','".$floor_id."','".$room."',this.value);\n";
			echo "document.getElementById('txt_rack').value 				= '".$rack."';\n";
		}
		if($self !=0)
		{
			echo "load_room_rack_self_bin('requires/trims_transfer_controller*4', 'shelf','shelf_td', '".$from_company_id."','"."','".$from_store_id."','".$floor_id."','".$room."','".$rack."',this.value);\n";
			echo "document.getElementById('txt_shelf').value 				= '".$self."';\n";
		}

		if($bin !=0)
		{
			echo "load_room_rack_self_bin('requires/trims_transfer_controller*4', 'bin','bin_td', '".$from_company_id."','"."','".$from_store_id."','".$floor_id."','".$room."','".$rack."','".$self."',this.value);\n";
        ...
        <legend>Enter search words</legend>
            <table cellpadding="0" cellspacing="0" width="800" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td>
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+<? echo $cbo_company_id; ?>+'_'+<? echo $cbo_transfer_criteria; ?>+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_year_selection').value, 'create_transfer_search_list_view', 'search_div', 'trims_transfer_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                    </td>
                </tr>
                <tr>
...
{
	$product_arr = return_library_array("select id, product_name_details from product_details_master where item_category_id in (4) ","id","product_name_details");
	...
	echo  create_list_view("list_view", "From Store,To Store,Item Description,Transfered Qnty", "150,150,250","680","200",0, $sql, "get_php_form_data", "id", "'populate_transfer_details_form_data'", 0, "from_store,to_store,from_prod_id,0", $arr, "from_store,to_store,from_prod_id,transfer_qnty", "requires/trims_transfer_controller",'','0,0,0,2');
...
if($action=='populate_transfer_details_form_data')
{
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_room_rack_self_bin('requires/trims_transfer_controller*4', 'store','from_store_td', $('#cbo_company_id').val(),'"."',this.value);\n";
		...
		echo "load_room_rack_self_bin('requires/trims_transfer_controller*4', 'floor','floor_td', $('#cbo_company_id').val(),'"."','".$row[csf('from_store')]."',this.value);\n";
		echo "document.getElementById('cbo_floor').value 				= '".$row[csf("floor_id")]."';\n";
		echo "load_room_rack_self_bin('requires/trims_transfer_controller*4', 'room','room_td', $('#cbo_company_id').val(),'"."','".$row[csf('from_store')]."','".$row[csf('floor_id')]."',this.value);\n";
		echo "document.getElementById('cbo_room').value 				= '".$row[csf("room")]."';\n";
		echo "load_room_rack_self_bin('requires/trims_transfer_controller*4', 'rack','rack_td', $('#cbo_company_id').val(),'"."','".$row[csf('from_store')]."','".$row[csf('floor_id')]."','".$row[csf('room')]."',this.value);\n";
		echo "document.getElementById('txt_rack').value 				= '".$row[csf("rack")]."';\n";
		echo "load_room_rack_self_bin('requires/trims_transfer_controller*4', 'shelf','shelf_td', $('#cbo_company_id').val(),'"."','".$row[csf('from_store')]."','".$row[csf('floor_id')]."','".$row[csf('room')]."','".$row[csf('rack')]."',this.value);\n";
		echo "document.getElementById('txt_shelf').value 				= '".$row[csf("shelf")]."';\n";
		echo "load_room_rack_self_bin('requires/trims_transfer_controller', 'bin','bin_td', $('#cbo_company_id').val(),'"."','".$row[csf('from_store')]."','".$row[csf('floor_id')]."','".$row[csf('room')]."','".$row[csf('rack')]."','".$row[csf('shelf')]."',this.value);\n";
		...
		echo "load_room_rack_self_bin('requires/trims_transfer_controller*4*cbo_store_name_to', 'store','to_store_td', '".$company_id."','"."',this.value);\n";
		...
		echo "load_room_rack_self_bin('requires/trims_transfer_controller*4*cbo_floor_to', 'floor','floor_td_to', '".$company_id."','"."','".$row[csf('to_store')]."',this.value);\n";
		echo "document.getElementById('cbo_floor_to').value 			= '".$row[csf("to_floor_id")]."';\n";
		echo "load_room_rack_self_bin('requires/trims_transfer_controller*4*cbo_room_to', 'room','room_td_to', '".$company_id."','"."','".$row[csf('to_store')]."','".$row[csf('to_floor_id')]."',this.value);\n";
		echo "document.getElementById('cbo_room_to').value 				= '".$row[csf("to_room")]."';\n";
		echo "load_room_rack_self_bin('requires/trims_transfer_controller*4*txt_rack_to', 'rack','rack_td_to', '".$company_id."','"."','".$row[csf('to_store')]."','".$row[csf('to_floor_id')]."','".$row[csf('to_room')]."',this.value);\n";
		echo "document.getElementById('txt_rack_to').value 				= '".$row[csf("to_rack")]."';\n";
		echo "load_room_rack_self_bin('requires/trims_transfer_controller*4*txt_shelf_to', 'shelf','shelf_td_to', '".$company_id."','"."','".$row[csf('to_store')]."','".$row[csf('to_floor_id')]."','".$row[csf('to_room')]."','".$row[csf('to_rack')]."',this.value);\n";
		echo "document.getElementById('txt_shelf_to').value 			= '".$row[csf("to_shelf")]."';\n";

		echo "load_room_rack_self_bin('requires/trims_transfer_controller*4*cbo_bin_to', 'bin','bin_td_to', '".$company_id."','"."','".$row[csf('to_store')]."','".$row[csf('to_floor_id')]."','".$row[csf('to_room')]."','".$row[csf('to_rack')]."','".$row[csf('to_shelf')]."',this.value);\n";
		...
		if($row[csf("transfer_criteria")]==1)
		{
			...
			from product_details_master a,  inv_transaction b 
			where a.id=b.prod_id and a.item_category_id in (4) and a.current_stock>0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.id=$product_id group by a.id, a.product_name_details");
		...
		echo "document.getElementById('txt_item_desc').value 				= '".$sql_balance[0][csf("product_name_details")]."';\n";
...
if($action=="save_update_delete")
{
	...
	$conversion_factor=return_field_value("a.conversion_factor as data","lib_item_group a, product_details_master b","a.id=b.item_group_id and b.entry_form=24 and b.id=$product_id","data");
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$update_id)=="")
		{
			...
			//$new_transfer_system_id=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '', 'TTE', date("Y",time()), 5, "select transfer_prefix, transfer_prefix_number from inv_item_transfer_mst where company_id=$cbo_company_id and transfer_criteria in(1,2) and entry_form=112 and $year_cond=".date('Y',time())." order by id desc ", "transfer_prefix", "transfer_prefix_number" ));
			$id = return_next_id_by_sequence("INV_ITEM_TRANSFER_MST_PK_SEQ", "inv_item_transfer_mst", $con);
			$new_transfer_system_id = explode("*", return_next_id_by_sequence("INV_ITEM_TRANSFER_MST_PK_SEQ", "inv_item_transfer_mst",$con,1,$cbo_company_id,'TTE',112,date("Y",time()) ));
		
		 
			//$id=return_next_id( "id", "inv_item_transfer_mst", 1 ) ;
			if(str_replace("'","",$cbo_transfer_criteria)==1)
			{
				$field_array="id, transfer_prefix, transfer_prefix_number, transfer_system_id, company_id, challan_no, transfer_date, transfer_criteria, to_company,entry_form,location_id,to_location_id, inserted_by, insert_date";
			
				$data_array="(".$id.",'".$new_transfer_system_id[1]."',".$new_transfer_system_id[2].",'".$new_transfer_system_id[0]."',".$cbo_company_id.",".$txt_challan_no.",".$txt_transfer_date.",".$cbo_transfer_criteria.",".$cbo_company_id_to.",112,".$cbo_location.",".$cbo_location_to.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			}
			else if(str_replace("'","",$cbo_transfer_criteria)==2)
			{
				$field_array="id, transfer_prefix, transfer_prefix_number, transfer_system_id, company_id, challan_no, transfer_date, transfer_criteria,entry_form,location_id,to_location_id, inserted_by, insert_date";
			
				$data_array="(".$id.",'".$new_transfer_system_id[1]."',".$new_transfer_system_id[2].",'".$new_transfer_system_id[0]."',".$cbo_company_id.",".$txt_challan_no.",".$txt_transfer_date.",".$cbo_transfer_criteria.",112,".$cbo_location.",".$cbo_location_to.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			}
		...
		else
		{
			...
			{
				//echo "10**fail=2";die;
				$pre_saved_store=sql_select("select a.id, b.from_store, b.to_store from inv_item_transfer_mst a, inv_item_transfer_dtls b where a.id = b.mst_id and a.entry_form = 112 and a.id = $transfer_update_id and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0 group by a.id, b.from_store, b.to_store");
		...
		$field_array_dtls="id, mst_id, from_prod_id, to_prod_id, item_group, from_store,floor_id,room,rack,shelf,bin_box, to_store,to_floor_id,to_room,to_rack,to_shelf,to_bin_box, item_category, transfer_qnty, rate, transfer_value, uom, job_id, po_number, from_order_id, to_order_id , style_ref_no, remarks, inserted_by, insert_date";
		...
		else
		{
			...
			$data_array_dtls="(".$id_dtls.",".$transfer_update_id.",".$hidden_product_id.",".$hidden_product_id.",0,".$cbo_store_name.",".$cbo_floor.",".$cbo_room.",".$txt_rack.",".$txt_shelf.",".$cbo_bin.",".$cbo_store_name_to.",".$cbo_floor_to.",".$cbo_room_to.",".$txt_rack_to.",".$txt_shelf_to.",".$cbo_bin_to.",".$cbo_item_category.",".$txt_transfer_qnty.",".$txt_rate.",".$txt_transfer_value.",".$cbo_uom.",".$job_id.",".$txt_order_no.",".$txt_order_id.",".$txt_order_id.",".$txt_style_no.",".$txt_remarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";

			if($variable_auto_rcv==2) // Heare ack dtls txt_order_id
			{
				$id_dtls_ac = return_next_id_by_sequence("INV_ITEM_TRANS_DTLS_AC_PK_SEQ", "inv_item_transfer_dtls_ac", $con);
			...
			where  b.trans_id=c.id and c.item_category=4 and c.company_id=$cbo_company_id and c.store_id =$cbo_store_name and b.po_breakdown_id =$txt_order_id and b.prod_id=$hidden_product_id and c.status_active=1 and c.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
			...
			$order_amount=$trims_qnty*$trim_ord_rate;
			$trims_qnty=str_replace("'","",$txt_transfer_qnty)/$conversion_factor;
			
			$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
			if($data_array_prop!="") $data_array_prop.=",";
			$data_array_prop.="(".$id_prop.",".$id_trans.",6,112,".$id_dtls.",".$txt_order_id.",".$hidden_product_id.",".$trims_qnty.",'".$trim_ord_rate."','".$order_amount."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			...
			if($variable_auto_rcv==1) // Heare ack dtls
			{
				$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
				if($data_array_prop!="") $data_array_prop.=",";
				$data_array_prop.="(".$id_prop.",".$id_trans_rcv.",5,112,".$id_dtls.",".$txt_order_id.",".$hidden_product_id.",".$trims_qnty.",'".$trim_ord_rate."','".$order_amount."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			}
		}
		...
		if($data_array_prop!="") {
			//echo "10**insert into order_wise_pro_details (".$field_array_propotion.") values ".$data_array_prop;die;
		...
		//echo "10** $rID=$rID2=$rID3=$rID4=$prodUpdate=$prod=$rID5";die;
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		$field_array_dtls="from_prod_id*to_prod_id*from_store*floor_id*room*rack*shelf*to_store*to_floor_id*to_room*to_rack*to_shelf*transfer_qnty*rate*transfer_value*uom*job_id*po_number*from_order_id*to_order_id*style_ref_no*remarks*updated_by*update_date";
		
		if($variable_auto_rcv != 1) // if auto receive yes(1), then no need to acknowledgement
		{
			$pre_saved_store=sql_select("select a.id, b.from_store, b.to_store from inv_item_transfer_mst a, inv_item_transfer_dtls b where a.id = b.mst_id and a.entry_form = 112 and a.id = $update_id and b.id != $update_dtls_id and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0 group by a.id, b.from_store, b.to_store");
		...
		if(str_replace("'","",$cbo_transfer_criteria)==1)
		{
			...
			$presentStock=$data_prod[0][csf('current_stock')]-str_replace("'","",$txt_transfer_qnty);
			$presentAvgRate=$data_prod[0][csf('avg_rate_per_unit')];
			$presentAvgRate=$presentStockValue=0;
			if ($presentStock != 0){				
				$presentStockValue=$presentStock*$presentAvgRate;
			}			
			...
			$sql = sql_select("select a.id,a.balance_qnty,a.balance_amount,b.issue_qnty,b.rate,b.amount from inv_transaction a, inv_mrr_wise_issue_details b where a.id=b.recv_trans_id and b.issue_trans_id=$update_trans_issue_id and b.entry_form=112 and a.item_category=$cbo_item_category "); 
		...
		else
		{
			...
			$data_array_dtls=$hidden_product_id."*".$hidden_product_id."*".$cbo_store_name."*".$cbo_floor."*".$cbo_room."*".$txt_rack."*".$txt_shelf."*".$cbo_store_name_to."*".$cbo_floor_to."*".$cbo_room_to."*".$txt_rack_to."*".$txt_shelf_to."*".$txt_transfer_qnty."*".$txt_rate."*".$txt_transfer_value."*".$cbo_uom."*".$job_id."*".$txt_order_no."*".$txt_order_id."*".$txt_order_id."*".$txt_style_no."*".$txt_remarks."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		$rID=$rID2=$rID3=$query3=$rID4=$rID5=$prodUpdate_adjust=$prodUpdate=$prod=true;
		...
		if($data_array_prop!="")
		{
			...
			$query3 = execute_query("DELETE FROM order_wise_pro_details WHERE trans_id in($update_trans_issue_id,$update_trans_recv_id) and entry_form=112 ");
			$rID4=sql_insert("order_wise_pro_details",$field_array_propotion,$data_array_prop,0);
		}
		...
		//echo "6**$rID=$rID2=$rID3=$query3=$rID4=$rID5=$prodUpdate_adjust=$prodUpdate=$prod";die;
		
		if($db_type==0)
		{
			if($rID && $rID2 && $rID3 && $query3 && $rID4 && $rID5 && $prodUpdate_adjust && $prodUpdate && $prod)
			{
				mysql_query("COMMIT");  
...
if($action=="trims_transfer_print")
{
	...
	$sql="select id, transfer_system_id, challan_no, transfer_date, transfer_criteria, item_category, to_company, from_order_id, to_order_id, item_category from inv_item_transfer_mst a where id='$data[1]' and company_id='$data[0]'";
	//echo $sql;die;
	...
	$prod_sql=sql_select("select id, product_name_details, item_group_id from product_details_master where item_category_id in (4) ");
	$product_arr=array();
	...
	<div style="width:930px;">
	    <table width="900" cellspacing="0" align="right">
	        ...
	        <tr>
	            ...
	            <td width="125"><strong>Item Category</strong></td><td width="175px"><? echo $item_category[4]; ?></td>
	        </tr>
	        <tr>
	    ...
	    <table align="right" cellspacing="0" width="900"  border="1" rules="all" class="rpt_table" style="table-layout: fixed;" >
			...
			<?
	        $sql_dtls="select id, from_store, to_store, from_prod_id, transfer_qnty, item_category, uom, job_id, po_number, remarks from inv_item_transfer_dtls where mst_id='$data[1]' and status_active=1 and is_deleted=0";
			//echo $sql_dtls;
			...
			foreach($sql_result as $row)
			{
				...
				$dtls_data[$row[csf("job_id")]][$row[csf("id")]]["id"]=$row[csf("id")];
				$dtls_data[$row[csf("job_id")]][$row[csf("id")]]["from_store"]=$row[csf("from_store")];
				$dtls_data[$row[csf("job_id")]][$row[csf("id")]]["to_store"]=$row[csf("to_store")];
				$dtls_data[$row[csf("job_id")]][$row[csf("id")]]["from_prod_id"]=$row[csf("from_prod_id")];
				$dtls_data[$row[csf("job_id")]][$row[csf("id")]]["transfer_qnty"]=$row[csf("transfer_qnty")];
				$dtls_data[$row[csf("job_id")]][$row[csf("id")]]["item_category"]=$row[csf("item_category")];
				$dtls_data[$row[csf("job_id")]][$row[csf("id")]]["uom"]=$row[csf("uom")];
				$dtls_data[$row[csf("job_id")]][$row[csf("id")]]["remarks"]=$row[csf("remarks")];
			}
			//echo "<pre>";print_r($dtls_data);
			$all_job_id=chop($all_job_id,",");
			
			/*$all_po_number=chop($all_po_number,",");
			$po_cond=$all_po="";
			if($all_po_number!="")
			{
				$all_po_num_arr=explode(",",$all_po_number);
				foreach($all_po_num_arr as $po_no)
				{
					$all_po.="'".$po_no."',";
				}
				$all_po=chop($all_po,",");
				$po_cond=" and b.po_number in($all_po)";
			}*/
			
			
			$sql_job_order=sql_select("select a.id as job_id, a.job_no, a.style_ref_no, a.buyer_name, b.po_number  from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.id in($all_job_id)");
			$job_data=array();
			foreach($sql_job_order as $row)
			{
				$job_data[$row[csf("job_id")]]["job_no"]=$row[csf("job_no")];
				$job_data[$row[csf("job_id")]]["style_ref_no"]=$row[csf("style_ref_no")];
				$job_data[$row[csf("job_id")]]["buyer_name"]=$row[csf("buyer_name")];
				$job_data[$row[csf("job_id")]]["po_number"].=$row[csf("po_number")].",";
			}
			
	        $i=1;
	        foreach($dtls_data as $job_id=>$job_val)
	        {
				?>
	            <!--<tr bgcolor="#E8E8E8"  style="font-size:13px; font-weight:bold;">
	                <td colspan="2">Buyer: <?// echo $buyer_library[$job_data[$job_id]["buyer_name"]]; ?></td>
	                <td  colspan="2">Style: <?// echo $job_data[$job_id]["style_ref_no"]; ?></td>
	                <td colspan="3" style="overflow:hidden;text-overflow: ellipsis;white-space: nowrap; font-weight:bold;">Order: <?// echo chop($job_data[$job_id]["po_number"],","); ?></td> style="word-break:break-all;"
	            </tr> style="overflow:hidden;text-overflow: ellipsis;white-space: nowrap; font-weight:bold;" 
	            <table width="900"  border="0" style="border:1px solid black; table-layout: fixed;">-->
	                <tr bgcolor="#E8E8E8"  style="font-size:13px; font-weight:bold;">
	                    <td colspan="7" style="overflow:hidden;text-overflow: ellipsis;white-space: nowrap; font-weight:bold;">Buyer: <? echo $buyer_library[$job_data[$job_id]["buyer_name"]]; ?>&nbsp;&nbsp;&nbsp;&nbsp;Style: <? echo $job_data[$job_id]["style_ref_no"]; ?>&nbsp;&nbsp;&nbsp;&nbsp;Order: <? echo chop($job_data[$job_id]["po_number"],","); ?></td>
	                </tr>
	            <!--</table>-->
	            
	            <tr bgcolor="#D1D1D1">
	                <td width="150"  align="center" style="font-size:13px; font-weight:bold;">From Store</td>
	                <td width="150"  align="center" style="font-size:13px; font-weight:bold;">To Store</td>
	                <td width="130"  align="center" style="font-size:13px; font-weight:bold;">Item Group</td> 
	                <td width="270"  align="center" style="font-size:13px; font-weight:bold;">Item Description</td>
	                <td width="50"  align="center" style="font-size:13px; font-weight:bold;"> UOM</td>
	                <td  align="center" style="font-size:13px; font-weight:bold;">Transfered Qnty</td>
	                <td  align="center" style="font-size:13px; font-weight:bold;">Remarks</td>
	            </tr>
	            <?
				foreach($job_val as $row)
				{
					 if ($i%2==0)  
						$bgcolor="#E9F3FF";
					else
					...
					<tr bgcolor="<? echo $bgcolor; ?>">
						<td><? echo $store_library[$row[("from_store")]]; ?></td>
						<td><? echo $store_library[$row[("to_store")]]; ?></td>
						<td><? echo $item_group_arr[$product_arr[$row[("from_prod_id")]]["item_group_id"]]; ?></td>
						<td><? echo $product_arr[$row[("from_prod_id")]]["product_name_details"]; ?></td>
				...
				?>
	            <tr bgcolor="#E8E8E8">
	                <td colspan="5" align="right" style="font-size:14px; font-weight:bold;">Job Total: </td>
	                <td align="right"><? echo number_format($job_total,2); $job_total=0; ?></td>
	                <td></td>
	            </tr>
	            <?
			...
			?>
	        <tr bgcolor="#CCCCCC">
	            <td colspan="5" align="right" style="font-size:14px; font-weight:bold;">Total :</td>
