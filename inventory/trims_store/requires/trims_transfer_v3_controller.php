...
if ($action=="load_room_rack_self_bin")
{
	...
	load_room_rack_self_bin("requires/trims_transfer_v3_controller",$data);
}
...
{
	$data=explode("_",$data);
	echo create_drop_down( "cbo_location", 160, "select id,location_name from lib_location where company_id='$data[0]' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", 0, "load_drop_down( 'requires/trims_transfer_v3_controller', this.value+'_'+$data[0], 'load_drop_down_store','from_store_td');");
	//if( $('#cbo_transfer_criteria').val()*1==2 || $('#cbo_transfer_criteria').val()*1==4)  load_room_rack_self_bin('requires/trims_transfer_v3_controller*4*cbo_store_name_to', 'store','to_store_td', $('#cbo_company_id').val(),this.value);load_room_rack_self_bin('requires/trims_transfer_v3_controller*4', 'store','from_store_td', $('#cbo_company_id').val(),this.value, '', '', '','', '', '','', '152');
	exit();
}
...
if ($action=="load_drop_down_location_to")
{
	...
	if($data[1]==2){
		...
		echo create_drop_down( "cbo_location_to", 160, "select id,location_name from lib_location where company_id='$data[0]' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", 0, "load_room_rack_self_bin('requires/trims_transfer_v3_controller*4*cbo_store_name_to', 'store','to_store_td', $('#cbo_company_id_to').val(),this.value, '', '', '','', '', '','', '152');" );
	}
	exit();
}
...
{
	$data=explode("_",$data);
	echo create_drop_down( "cbo_store_name", 152, "select a.id,a.store_name from lib_store_location a, lib_store_location_category b where a.id=b.store_location_id and a.company_id='$data[1]' and a.location_id = $data[0] and b.category_type in (4) and a.status_active=1 and a.is_deleted=0 group by a.id,a.store_name order by a.store_name","id,store_name", 1, "-- Select --", 0, "load_drop_down('requires/trims_transfer_v3_controller', this.value+'_'+$data[1], 'load_drop_floor','floor_td');","" );  	 
	exit();
}
...
//		if($cbo_bin && $store_update_upto >5) $str_conds.=" and d.bin_box=$cbo_bin";
		...
		if($cbo_floor>0) $str_conds.=" and d.floor_id=$cbo_floor";
		if($cbo_room>0) $str_conds.=" and d.room=$cbo_room";
		if($txt_rack>0) $str_conds.=" and d.rack=$txt_rack";
		if($txt_shelf>0) $str_conds.=" and d.self=$txt_shelf";
		if($cbo_bin>0) $str_conds.=" and d.bin_box=$cbo_bin";
		
		$sql= "SELECT a.JOB_NO_PREFIX_NUM, $year_field, a.JOB_NO, a.COMPANY_NAME, a.BUYER_NAME, a.STYLE_REF_NO, a.JOB_QUANTITY, b.GROUPING, b.ID, b.PO_NUMBER, b.PO_QUANTITY, b.pub_shipment_date as SHIPMENT_DATE, D.FLOOR_ID, D.ROOM, D.RACK, D.SELF, D.BIN_BOX, sum((case when c.trans_type in(1,4,5) then c.quantity else 0 end)-(case when c.trans_type in(2,3,6) then c.quantity else 0 end)) as BAL
		...
		group by  a.JOB_NO_PREFIX_NUM, a.insert_date, a.JOB_NO, a.COMPANY_NAME, a.BUYER_NAME, a.STYLE_REF_NO, a.JOB_QUANTITY, b.GROUPING, b.ID, b.PO_NUMBER, b.PO_QUANTITY, b.pub_shipment_date, D.FLOOR_ID, D.ROOM, D.RACK, D.SELF, D.BIN_BOX
	...
	else 
	{
		...
		$sql= "SELECT a.JOB_NO_PREFIX_NUM, $year_field, a.JOB_NO, a.COMPANY_NAME, a.BUYER_NAME, a.STYLE_REF_NO, a.JOB_QUANTITY, b.GROUPING, b.ID, b.PO_NUMBER, b.PO_QUANTITY, b.pub_shipment_date as SHIPMENT_DATE 
	...
	$lib_rack_shelf_arr=return_library_array( "select a.floor_room_rack_id, a.floor_room_rack_name from lib_floor_room_rack_mst a", "floor_room_rack_id","floor_room_rack_name" ); 
	?>
	<table border="1" class="rpt_table" rules="all" width="1250" cellpadding="0" cellspacing="0" align="left">
		<thead>
            <tr>
    ...
    <div style="width:1270px; max-height:420px; overflow-y:scroll;">
	    <table id="list_view" width="1250" border="1" rules="all" class="rpt_table" align="left">
	    	<?
	    	$i=1;
	    	foreach ($sql_res as $row) 
	    	{
	    		?>
		    	<tr onClick="js_set_value('<?= $row['ID']."_".$row['FLOOR_ID']."_".$row['ROOM']."_".$row['RACK']."_".$row['SELF']."_".$row['BIN_BOX']; ?>')" style="text-decoration:none; cursor:pointer">
	                ...
	                <td width="70"><p><?= $row['GROUPING']; ?></p></td>
	                <td width="100"><p><?= $row['PO_NUMBER']; ?></p></td>
	                <td width="80" align="right"><p><?= $row['PO_QUANTITY']; ?></p></td>
	                <td width="80" title="<?= $row['FLOOR_ID'];?>"><p><?= $lib_rack_shelf_arr[$row['FLOOR_ID']]; ?></p></td>
                    <td width="80" title="<?= $row['ROOM'];?>"><p><?= $lib_rack_shelf_arr[$row['ROOM']]; ?></p></td>
                    <td width="80" title="<?= $row['RACK'];?>"><p><?= $lib_rack_shelf_arr[$row['RACK']]; ?></p></td>
                    <td width="80" title="<?= $row['SELF'];?>"><p><?= $lib_rack_shelf_arr[$row['SELF']]; ?></p></td>
                    <td width="80" title="<?= $row['BIN_BOX'];?>"><p><?= $lib_rack_shelf_arr[$row['BIN_BOX']]; ?></p></td>
                    <td align="right"><p><?= number_format($row['BAL'],4); ?></p></td>
				</tr>
				<?
				$i++;
			}	
...
if($action=="show_dtls_list_view")
{
	...
	foreach($sql_order_rate_result as $row)
	{
		if($trans_id_check[$row["TRANS_ID"]]=="")
		{
			...
			{
				$order_item_data[$row["PROD_ID"]]["cons_quantity"]+=$row["CONS_QUANTITY"];
				$order_item_data[$row["PROD_ID"]]["cons_amount"]+=$row["CONS_AMOUNT"];
			}
			else
			{
				$order_item_data[$row["PROD_ID"]]["cons_quantity"]-=$row["CONS_QUANTITY"];
		...
		{
			$order_item_data[$row["PROD_ID"]]["QUANTITY"]+=$row["QUANTITY"];
			$order_item_data[$row["PROD_ID"]]["ORDER_AMOUNT"]+=$row["ORDER_AMOUNT"];
		}
		else
		{
			$order_item_data[$row["PROD_ID"]]["QUANTITY"]-=$row["QUANTITY"];
	...
	if($status==1){
		...
		$sql = "SELECT a.ID, a.ITEM_GROUP_ID, a.ITEM_DESCRIPTION, a.UNIT_OF_MEASURE, a.BRAND_SUPPLIER, a.ITEM_COLOR, a.ITEM_SIZE, a.COLOR, a.GMTS_SIZE, b.PO_BREAKDOWN_ID, sum((case when b.trans_type in(1,4,5) then b.quantity else 0 end)-(case when b.trans_type in(2,3,6) then b.quantity else 0 end)) as BAL, sum((case when b.trans_type in(1,4,5) then b.ORDER_AMOUNT else 0 end)-(case when b.trans_type in(2,3,6) then b.ORDER_AMOUNT else 0 end)) as BAL_AMT 
		from product_details_master a, order_wise_pro_details b, inv_transaction c 
		...
		group by a.ID, a.ITEM_GROUP_ID, a.ITEM_DESCRIPTION, a.UNIT_OF_MEASURE, a.BRAND_SUPPLIER, a.ITEM_COLOR, a.ITEM_SIZE, a.COLOR, a.GMTS_SIZE, b.PO_BREAKDOWN_ID
	...
	{  
		...
		$item_rate=$order_item_data[$row["ID"]]["cons_amount"]/$order_item_data[$row["ID"]]["cons_quantity"];
		...
		?>
		 <tr bgcolor="<? echo $bgcolor; ?>">
			...
			<input type="text" name="txtTransferQnty[]" id="txtTransferQnty_<?=$i;?>" class="text_boxes_numeric" style="width:80px;" />
...
if($action=="show_dtls_list_view_update")
{
	...
	$sql="select a.ID, a.ITEM_GROUP_ID, a.ITEM_DESCRIPTION, a.UNIT_OF_MEASURE, a.BRAND_SUPPLIER, a.ITEM_COLOR, a.ITEM_SIZE, a.COLOR, a.GMTS_SIZE, b.FROM_ORDER_ID as PO_BREAKDOWN_ID, b.ID as DTLS_ID, b.TRANSFER_QNTY, b.REMARKS, b.TRANS_ID, b.TO_TRANS_ID  
	from product_details_master a, inv_item_transfer_dtls b where a.id=b.from_prod_id and b.mst_id='$mst_id' and b.status_active = '1' and b.is_deleted = '0'";	
	//echo $sql;
	$data_array=sql_select($sql);
	$i=1;
	foreach($data_array as $row)
	{  
		if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
		$ord_rate=0;
		if($order_item_data[$row["ID"]]["ORDER_AMOUNT"]!=0 && $order_item_data[$row["ID"]]["QUANTITY"]!=0) $ord_rate=$order_item_data[$row["ID"]]["ORDER_AMOUNT"]/$order_item_data[$row["ID"]]["QUANTITY"];
        ...
        <legend>Enter search words</legend>
            <table cellpadding="0" cellspacing="0" width="800" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td>
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+<? echo $cbo_company_id; ?>+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_year_selection').value+'_'+<? echo $cbo_transfer_criteria; ?>, 'create_transfer_search_list_view', 'search_div', 'trims_transfer_v3_controller', 'setFilterGrid(\'tbl_list_search\',-1); createContentSearch(\'rpt_tabletbl_list_search\');')" style="width:100px;" />
                    </td>
                </tr>
                <tr>
...
if($action=='populate_data_from_transfer_master')
{
	...
	foreach ($data_array as $row)
	{
		...
		{
			echo "load_drop_down( 'requires/trims_transfer_v3_controller',$to_company, 'load_drop_down_location_to', 'to_location_td' );\n";
		}
		echo "document.getElementById('cbo_location_to').value 				= '".$row[csf("to_location_id")]."';\n";


		echo "load_room_rack_self_bin('requires/trims_transfer_v3_controller*4', 'store','from_store_td', '".$row[csf('company_id')]."','"."',this.value, '', '','', '', '','', '152');\n";
		echo "document.getElementById('cbo_store_name').value 				= '".$row[csf("from_store")]."';\n";
		echo "load_room_rack_self_bin('requires/trims_transfer_v3_controller*4', 'floor','floor_td', '".$row[csf('company_id')]."','"."','".$row[csf('from_store')]."',this.value);\n";
		echo "document.getElementById('cbo_floor').value 				= '".$row[csf("floor_id")]."';\n";
		
		if($row[csf("floor_id")])
		{
			echo "load_room_rack_self_bin('requires/trims_transfer_v3_controller*4', 'room','room_td', '".$row[csf('company_id')]."','"."','".$row[csf('from_store')]."','".$row[csf('floor_id')]."',this.value);\n";
		}
		echo "document.getElementById('cbo_room').value 				= '".$row[csf("room")]."';\n";
		
		if($row[csf("room")])
		{
			echo "load_room_rack_self_bin('requires/trims_transfer_v3_controller*4', 'rack','rack_td', '".$row[csf('company_id')]."','"."','".$row[csf('from_store')]."','".$row[csf('floor_id')]."','".$row[csf('room')]."',this.value);\n";
		}
		echo "document.getElementById('txt_rack').value 				= '".$row[csf("rack")]."';\n";
		
		if($row[csf("rack")])
		{
			echo "load_room_rack_self_bin('requires/trims_transfer_v3_controller*4', 'shelf','shelf_td', '".$row[csf('company_id')]."','"."','".$row[csf('from_store')]."','".$row[csf('floor_id')]."','".$row[csf('room')]."','".$row[csf('rack')]."',this.value);\n";
		}
		echo "document.getElementById('txt_shelf').value 				= '".$row[csf("shelf")]."';\n";
		
		if($row[csf("shelf")])
		{
			echo "load_room_rack_self_bin('requires/trims_transfer_v3_controller*4', 'bin','bin_td', '".$row[csf('company_id')]."','"."','".$row[csf('from_store')]."','".$row[csf('floor_id')]."','".$row[csf('room')]."','".$row[csf('rack')]."','".$row[csf('shelf')]."',this.value);\n";
		}
		echo "document.getElementById('cbo_bin').value 				= '".$row[csf("bin_box")]."';\n";


		echo "load_room_rack_self_bin('requires/trims_transfer_v3_controller*4*cbo_store_name_to', 'store','to_store_td', '".$to_company."','"."',this.value, '', '','', '', '','', '152');\n";
		echo "document.getElementById('cbo_store_name_to').value 				= '".$row[csf("to_store")]."';\n";

		echo "load_room_rack_self_bin('requires/trims_transfer_v3_controller*4*cbo_floor_to', 'floor','floor_td_to', '".$to_company."','"."','".$row[csf('to_store')]."',this.value);\n";
		echo "document.getElementById('cbo_floor_to').value 			= '".$row[csf("to_floor_id")]."';\n";
		
		if($row[csf("to_floor_id")])
		{
			echo "load_room_rack_self_bin('requires/trims_transfer_v3_controller*4*cbo_room_to', 'room','room_td_to', '".$to_company."','"."','".$row[csf('to_store')]."','".$row[csf('to_floor_id')]."',this.value);\n";
		}
		echo "document.getElementById('cbo_room_to').value 				= '".$row[csf("to_room")]."';\n";
		
		if($row[csf("to_room")])
		{
			echo "load_room_rack_self_bin('requires/trims_transfer_v3_controller*4*txt_rack_to', 'rack','rack_td_to', '".$to_company."','"."','".$row[csf('to_store')]."','".$row[csf('to_floor_id')]."','".$row[csf('to_room')]."',this.value);\n";
		}
		echo "document.getElementById('txt_rack_to').value 				= '".$row[csf("to_rack")]."';\n";
		
		if($row[csf("to_rack")])
		{
			echo "load_room_rack_self_bin('requires/trims_transfer_v3_controller*4*txt_shelf_to', 'shelf','shelf_td_to', '".$to_company."','"."','".$row[csf('to_store')]."','".$row[csf('to_floor_id')]."','".$row[csf('to_room')]."','".$row[csf('to_rack')]."',this.value);\n";
		}
		echo "document.getElementById('txt_shelf_to').value 			= '".$row[csf("to_shelf")]."';\n";
		
		if($row[csf("to_shelf")])
		{
			echo "load_room_rack_self_bin('requires/trims_transfer_v3_controller*4*cbo_bin_to', 'bin','bin_td_to', '".$to_company."','"."','".$row[csf('to_store')]."','".$row[csf('to_floor_id')]."','".$row[csf('to_room')]."','".$row[csf('to_rack')]."','".$row[csf('to_shelf')]."',this.value);\n";
		}
		...
		echo "get_php_form_data('".$row[csf("from_order_id")]."**from'".",'populate_data_from_order','requires/trims_transfer_v3_controller');\n";
		echo "get_php_form_data('".$row[csf("to_order_id")]."**to'".",'populate_data_from_order','requires/trims_transfer_v3_controller');\n";
...
if($action=="show_transfer_listview")
{
	...
	echo  create_list_view("list_view", "Item Category,Item Description,Transfered Qnty,UOM", "120,250,100","650","200",0, $sql, "get_php_form_data", "id", "'populate_transfer_details_form_data'", 0, "item_category,from_prod_id,0,uom", $arr, "item_category,from_prod_id,transfer_qnty,uom", "requires/trims_transfer_v3_controller",'','0,0,2,0');
	exit();
}
...
{	 
	...
	$updateTransIssueId_arr=$updateTransRecvId_arr=$item_group_id_arr=$pord_wise_trans_arr=array();
	for($i=1;$i<=$tot_row;$i++)
	{
		$prodId="prod_id".$i;
	...
	$sql_trim = sql_select("select b.prod_id, sum((case when b.entry_form in(24,73,78,112) and b.trans_type in(1,4,5) then b.quantity else 0 end)-(case when b.entry_form in(25,49,78,112) and b.trans_type in(2,3,6) then b.quantity else 0 end)) as balance 
	from order_wise_pro_details b, inv_transaction c
	where  b.trans_id=c.id and c.item_category=4 and c.company_id=$cbo_company_id and c.store_id =$cbo_store_name $sqlCon and b.po_breakdown_id =$txt_from_order_id and b.prod_id in(".implode(",",$all_prod_ids_arr).") and c.status_active=1 and c.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $update_trans_cond
	group by b.prod_id");
	foreach($sql_trim as $row)
	{
		$trim_stock=$row[csf("balance")]*$conversion_factor_arr[$row[csf("prod_id")]];
		$trans_qnty=$pord_wise_trans_arr[$row[csf("prod_id")]];
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		$sql_budge_check=sql_select("select a.id from wo_pre_cost_trim_cost_dtls a, wo_pre_cost_trim_co_cons_dtls b where a.id=b.wo_pre_cost_trim_cost_dtls_id and a.trim_group in(".implode(",",$item_group_id_arr).") and b.po_break_down_id=$txt_from_order_id");
		...
		{
			$row_prod=sql_select("select ID, ITEM_GROUP_ID, ITEM_DESCRIPTION, BRAND_SUPPLIER, ITEM_COLOR, ITEM_SIZE, COLOR, GMTS_SIZE, CURRENT_STOCK, AVG_RATE_PER_UNIT, STOCK_VALUE from product_details_master where entry_form=24 and company_id=$cbo_company_id_to and item_category_id=$cbo_item_category and ITEM_GROUP_ID in(".implode(",",$item_group_id_arr).") and status_active=1 and is_deleted=0");
			$toCompany_prod_data_arr=array();
			foreach($row_prod as $val)
			{
				$toCompany_prod_data_arr[$val["ITEM_GROUP_ID"]][trim($val["ITEM_DESCRIPTION"])][trim($val["BRAND_SUPPLIER"])][$val["ITEM_COLOR"]][trim($val["ITEM_SIZE"])][$val["COLOR"]][$val["GMTS_SIZE"]]["ID"]=$val["ID"];
				$toCompany_prod_data_arr[$val["ITEM_GROUP_ID"]][trim($val["ITEM_DESCRIPTION"])][trim($val["BRAND_SUPPLIER"])][$val["ITEM_COLOR"]][trim($val["ITEM_SIZE"])][$val["COLOR"]][$val["GMTS_SIZE"]]["CURRENT_STOCK"]=$val["CURRENT_STOCK"];
				$toCompany_prod_data_arr[$val["ITEM_GROUP_ID"]][trim($val["ITEM_DESCRIPTION"])][trim($val["BRAND_SUPPLIER"])][$val["ITEM_COLOR"]][trim($val["ITEM_SIZE"])][$val["COLOR"]][$val["GMTS_SIZE"]]["AVG_RATE_PER_UNIT"]=$val["AVG_RATE_PER_UNIT"];
				$toCompany_prod_data_arr[$val["ITEM_GROUP_ID"]][trim($val["ITEM_DESCRIPTION"])][trim($val["BRAND_SUPPLIER"])][$val["ITEM_COLOR"]][trim($val["ITEM_SIZE"])][$val["COLOR"]][$val["GMTS_SIZE"]]["STOCK_VALUE"]=$val["STOCK_VALUE"];
			}
			...
			for($i=1;$i<=$tot_row;$i++)
			{
				...
				$presentStock=$prod_stock_arr[$$prodId]["CURRENT_STOCK"]-$$txtTransferQnty;
				$presentAvgRate=$prod_stock_arr[$$prodId]["AVG_RATE_PER_UNIT"];
				$presentStockValue=0;
				if($presentStock != 0){				
				...
				$data_array_prodUpdate[$$prodId]=explode("*",("".$$txtTransferQnty."*".$presentStock."*".$presentStockValue."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
				
				if($toCompany_prod_data_arr[$$itemGroupId][trim($$itemDescription)][trim($$brandSupp)][$$itemColorId][trim($$itemSize)][$$gmtsColorId][$$gmtsSizeId]["ID"]>0) // $product_id // Already found Product
				{
					$product_id=$toCompany_prod_data_arr[$$itemGroupId][trim($$itemDescription)][trim($$brandSupp)][$$itemColorId][trim($$itemSize)][$$gmtsColorId][$$gmtsSizeId]["ID"];
					$stock_qnty=$toCompany_prod_data_arr[$$itemGroupId][trim($$itemDescription)][trim($$brandSupp)][$$itemColorId][trim($$itemSize)][$$gmtsColorId][$$gmtsSizeId]["CURRENT_STOCK"];	
					$stock_value=$toCompany_prod_data_arr[$$itemGroupId][trim($$itemDescription)][trim($$brandSupp)][$$itemColorId][trim($$itemSize)][$$gmtsColorId][$$gmtsSizeId]["STOCK_VALUE"];
					$avg_rate_per_unit=$toCompany_prod_data_arr[$$itemGroupId][trim($$itemDescription)][trim($$brandSupp)][$$itemColorId][trim($$itemSize)][$$gmtsColorId][$$gmtsSizeId]["AVG_RATE_PER_UNIT"];			
	
					$curr_stock_qnty=$stock_qnty+str_replace("'", '',$$txtTransferQnty);
					$curr_stock_value=0;
					if ($curr_stock_qnty != 0){
						$curr_stock_value=$stock_value+$transfer_value;
					}
					$avg_rate_per_unit=0;
					...
					{
						$updateProdRcvID_array[]=$product_id;
						$data_array_prod_update[$product_id]=explode("*",("".$avg_rate_per_unit."*".$$txtTransferQnty."*".$curr_stock_qnty."*".$curr_stock_value."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
					}
				}
				...
				$data_array_trans.="(".$id_trans.",".$transfer_update_id.",".$cbo_transfer_criteria.",".$cbo_company_id.",'".$$prodId."',".$cbo_item_category.",6,".$txt_transfer_date.",".$cbo_store_name.",".$cbo_floor.",".$cbo_room.",".$txt_rack.",".$txt_shelf.",".$cbo_bin.",".$txt_from_order_id.",'".$$uom."','".$$txtTransferQnty."','".$$txtRate."','".$transfer_value."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				...
				if($variable_auto_rcv==1)
				{
					...
					$data_array_trans.=",(".$id_trans_recv.",".$transfer_update_id.",".$cbo_transfer_criteria.",".$cbo_company_id_to.",".$product_id.",".$cbo_item_category.",5,".$txt_transfer_date.",".$cbo_store_name_to.",".$cbo_floor_to.",".$cbo_room_to.",".$txt_rack_to.",".$txt_shelf_to.",".$cbo_bin_to.",".$txt_to_order_id.",'".$$uom."','".$$txtTransferQnty."','".$$txtRate."','".$transfer_value."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				}
				if($data_array_dtls!="") $data_array_dtls.=",";
				$id_dtls = return_next_id_by_sequence("INV_ITEM_TRANSFER_DTLS_PK_SEQ", "inv_item_transfer_dtls", $con);
				$data_array_dtls.="(".$id_dtls.",".$transfer_update_id.",'".$$prodId."',".$product_id.",".$cbo_store_name.",".$cbo_floor.",".$cbo_room.",".$txt_rack.",".$txt_shelf.",".$cbo_bin.",".$cbo_store_name_to.",".$cbo_floor_to.",".$cbo_room_to.",".$txt_rack_to.",".$txt_shelf_to.",".$cbo_bin_to.",".$txt_from_order_id.",".$txt_to_order_id.",".$cbo_item_category.",'".$$itemGroupId."','".$$txtTransferQnty."','".$$txtRate."','".$transfer_value."','".$$uom."','".$$txtRemarks."',".$id_trans.",".$id_trans_recv.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				...
				if($variable_auto_rcv==2) // acknowledgement_dtls_table
				{
					...
					$data_array_dtls_ac.="(".$id_dtls_ac.",".$transfer_update_id.",".$id_dtls.",0,'".$$prodId."',".$product_id.",".$cbo_store_name.",".$cbo_floor.",".$cbo_room.",".$txt_rack.",".$txt_shelf.",".$cbo_bin.",".$cbo_store_name_to.",".$cbo_floor_to.",".$cbo_room_to.",".$txt_rack_to.",".$txt_shelf_to.",".$cbo_bin_to.",".$cbo_item_category.",'".$$itemGroupId."','".$$txtTransferQnty."','".$$txtRate."','".$transfer_value."','".$$uom."','".$$txtRemarks."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				}
				...
				$data_array_prop.="(".$id_prop.",".$id_trans.",6,".$entry_form_no.",".$id_dtls.",".$txt_from_order_id.",'".$$prodId."','".$$gmtsColorId."',".$order_trans_qnty.",'".$$txtOrdRate."','".$order_amount."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				if($variable_auto_rcv==1)
				{
					$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
					$data_array_prop.=",(".$id_prop.",".$id_trans_recv.",5,".$entry_form_no.",".$id_dtls.",".$txt_to_order_id.",".$product_id.",'".$$gmtsColorId."',".$order_trans_qnty.",'".$$txtOrdRate."','".$order_amount."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		else // order to order and store to store transfer
		{
			for($i=1;$i<=$tot_row;$i++)
			{
				...
				{
					$id_trans_recv=return_next_id_by_sequence("INV_TRANSACTION_PK_SEQ", "inv_transaction", $con);
					$data_array_trans.=",(".$id_trans_recv.",".$transfer_update_id.",".$cbo_transfer_criteria.",".$cbo_company_id_to.",'".$$prodId."',".$cbo_item_category.",5,".$txt_transfer_date.",".$cbo_store_name_to.",".$cbo_floor_to.",".$cbo_room_to.",".$txt_rack_to.",".$txt_shelf_to.",".$cbo_bin_to.",".$txt_to_order_id.",'".$$uom."','".$$txtTransferQnty."','".$$txtRate."','".$transfer_value."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				}
				
				if($data_array_dtls!="") $data_array_dtls.=",";
				$id_dtls = return_next_id_by_sequence("INV_ITEM_TRANSFER_DTLS_PK_SEQ", "inv_item_transfer_dtls", $con);
				$data_array_dtls.="(".$id_dtls.",".$transfer_update_id.",'".$$prodId."','".$$prodId."',".$cbo_store_name.",".$cbo_floor.",".$cbo_room.",".$txt_rack.",".$txt_shelf.",".$cbo_bin.",".$cbo_store_name_to.",".$cbo_floor_to.",".$cbo_room_to.",".$txt_rack_to.",".$txt_shelf_to.",".$cbo_bin_to.",".$txt_from_order_id.",".$txt_to_order_id.",".$cbo_item_category.",'".$$itemGroupId."','".$$txtTransferQnty."','".$$txtRate."','".$transfer_value."','".$$uom."','".$$txtRemarks."',".$id_trans.",".$id_trans_recv.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				...
				{
					$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
					$data_array_prop.=",(".$id_prop.",".$id_trans_recv.",5,".$entry_form_no.",".$id_dtls.",".$txt_to_order_id.",'".$$prodId."','".$$gmtsColorId."',".$order_trans_qnty.",'".$$txtOrdRate."','".$order_amount."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		{
			$prodUpdate=execute_query(bulk_update_sql_statement("product_details_master","id",$field_array_prodUpdate,$data_array_prodUpdate,$updateProdID_array));
			if(count($updateProdRcvID_array)>0)
			{
				if($variable_auto_rcv==1)
				{;
					$prod=execute_query(bulk_update_sql_statement("product_details_master","id",$field_array_prod_update,$data_array_prod_update,$updateProdRcvID_array));
				}
			}
			else
			{
				//echo "10**".$sql_prod_insert;oci_rollback($con);disconnect($con);die;
		...
		if($db_type==0)
		{
			if($rID==1 && $rID2==1 && $rID3==1 && $rID4==1 && $prodUpdate==1 && $prod==1 && $rID5==1)
			{
				mysql_query("COMMIT");  
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$cbo_transfer_criteria)==1) // add from trims transfer page
		{
			...
			for($i=1;$i<=$tot_row;$i++)
			{
				...
				$presentStock=(($prod_stock_arr[$$prodId]["CURRENT_STOCK"]+$previous_issue_prod_data[$$prodId])-$$txtTransferQnty);
				$presentAvgRate=$prod_stock_arr[$$prodId]["AVG_RATE_PER_UNIT"];
				$presentStockValue=0;
				if($presentStock != 0){				
					$presentStockValue=$presentStock*$presentAvgRate;
				...
				{
					$product_id=$toCompany_prod_data_arr[$$itemGroupId][trim($$itemDescription)][trim($$brandSupp)][$$itemColorId][trim($$itemSize)][$$gmtsColorId][$$gmtsSizeId]["ID"];
					$stock_qnty=$toCompany_prod_data_arr[$$itemGroupId][trim($$itemDescription)][trim($$brandSupp)][$$itemColorId][trim($$itemSize)][$$gmtsColorId][$$gmtsSizeId]["CURRENT_STOCK"]-$previous_rcv_prod_data[$product_id];
					$avg_rate_per_unit=$toCompany_prod_data_arr[$$itemGroupId][trim($$itemDescription)][trim($$brandSupp)][$$itemColorId][trim($$itemSize)][$$gmtsColorId][$$gmtsSizeId]["AVG_RATE_PER_UNIT"];		
				...
				$updateTransID_data[$$updateTransIssueId]=explode("*",("".$$prodId."*".$txt_transfer_date."*".$txt_from_order_id."*'".$$uom."'*'".$$txtTransferQnty."'*'".$$txtRate."'*'".$transfer_value."'*".$cbo_store_name."*".$cbo_floor."*".$cbo_room."*".$txt_rack."*".$txt_shelf."*".$cbo_bin."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
				...
				if($variable_auto_rcv==1)
				{
					...
					$updateTransID_data[$$updateTransRecvId]=explode("*",("".$product_id."*".$txt_transfer_date."*".$txt_to_order_id."*'".$$uom."'*'".$$txtTransferQnty."'*'".$$txtRate."'*'".$transfer_value."'*".$cbo_store_name_to."*".$cbo_floor_to."*".$cbo_room_to."*".$txt_rack_to."*".$txt_shelf_to."*".$cbo_bin_to."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
					unset($previous_trans_id_arr[$$updateTransRecvId]);
				}
				
				$updateDtlsID_array[]=$$updateDtlsId;
				$data_array_dtls[$$updateDtlsId]=explode("*",("".$$prodId."*".$product_id."*'".$$itemGroupId."'*".$cbo_store_name_to."*".$cbo_floor_to."*".$cbo_room_to."*".$txt_rack_to."*".$txt_shelf_to."*".$cbo_bin_to."*'".$$txtTransferQnty."'*'".$$txtRate."'*'".$transfer_value."'*'".$$uom."'*'".$$txtRemarks."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
				unset($previous_dtls_id_arr[$$updateDtlsId]);
				unset($previous_issue_prod_data[$$prodId]);
				unset($previous_rcv_prod_data[$product_id]);
				...
				$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
				if($data_array_prop!="") $data_array_prop.=",";
				$data_array_prop.="(".$id_prop.",".$$updateTransIssueId.",6,".$entry_form_no.",".$$updateDtlsId.",".$txt_from_order_id.",'".$$prodId."','".$$gmtsColorId."',".$order_trans_qnty.",'".$$txtOrdRate."','".$order_amount."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				if($variable_auto_rcv==1)
				{
					$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
					$data_array_prop.=",(".$id_prop.",".$$updateTransRecvId.",5,".$entry_form_no.",".$$updateDtlsId.",".$txt_to_order_id.",".$product_id.",'".$$gmtsColorId."',".$order_trans_qnty.",'".$$txtOrdRate."','".$order_amount."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		else // order to order and store to store transfer
		{
			for($i=1;$i<=$tot_row;$i++)
			{
				...
				{
					$updateTransID_array[]=$$updateTransRecvId; 
					$updateTransID_data[$$updateTransRecvId]=explode("*",("".$$prodId."*".$txt_transfer_date."*".$txt_to_order_id."*'".$$uom."'*'".$$txtTransferQnty."'*'".$$txtRate."'*'".$transfer_value."'*".$cbo_store_name_to."*".$cbo_floor_to."*".$cbo_room_to."*".$txt_rack_to."*".$txt_shelf_to."*".$cbo_bin_to."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
					unset($previous_trans_id_arr[$$updateTransRecvId]);
				}
				
				$updateDtlsID_array[]=$$updateDtlsId;
				$data_array_dtls[$$updateDtlsId]=explode("*",("".$$prodId."*".$$prodId."*'".$$itemGroupId."'*".$cbo_store_name_to."*".$cbo_floor_to."*".$cbo_room_to."*".$txt_rack_to."*".$txt_shelf_to."*".$cbo_bin_to."*'".$$txtTransferQnty."'*'".$$txtRate."'*'".$transfer_value."'*'".$$uom."'*'".$$txtRemarks."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
				unset($previous_dtls_id_arr[$$updateDtlsId]);
				...
				{
					$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
					$data_array_prop.=",(".$id_prop.",".$$updateTransRecvId.",5,".$entry_form_no.",".$$updateDtlsId.",".$txt_to_order_id.",".$$prodId.",'".$$gmtsColorId."',".$order_trans_qnty.",'".$$txtOrdRate."','".$order_amount."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		$rID=$rID2=$rID3=$query=$prodUpdate=$prod=$rID5=$rID4=$rID6=$rID7=$rID8=$rID9=true;
		...
		if(count($previous_trans_id_arr)>0)
		{
			...
			if(str_replace("'","",$cbo_transfer_criteria)==1)
			{
				$prev_prod_sql=sql_select("select ID, AVG_RATE_PER_UNIT, CURRENT_STOCK from PRODUCT_DETAILS_MASTER where id in(".implode(",",array_flip($previous_issue_prod_data)).",".implode(",",array_flip($previous_rcv_prod_data)).")");
				foreach($prev_prod_sql as $val)
				{
					if($previous_issue_prod_data[$val["ID"]])
					{
						$currrent_stock=$stock_value=0;
						$currrent_stock=$val["CURRENT_STOCK"]+$previous_issue_prod_data[$val["ID"]];
						$stock_value=$currrent_stock*$val["AVG_RATE_PER_UNIT"];
						$rID8=execute_query("Update product_details_master set current_stock=$currrent_stock, stock_value=$stock_value, updated_by=$user_id, update_date='$pc_date_time' where id ='".$val["ID"]."'");
						if($rID8==false)
						{
							echo "20**Update product_details_master set current_stock=$currrent_stock, stock_value=$stock_value, updated_by=$user_id, update_date='$pc_date_time' where id ='".$val["ID"]."'";oci_rollback($con);disconnect($con);die;
						}
					}
					if($previous_rcv_prod_data[$val["ID"]])
					{
						$currrent_stock=$stock_value=0;
						$currrent_stock=$val["CURRENT_STOCK"]-$previous_rcv_prod_data[$val["ID"]];
						$stock_value=$currrent_stock*$val["AVG_RATE_PER_UNIT"];
						$rID9=execute_query("Update product_details_master set current_stock=$currrent_stock, stock_value=$stock_value, updated_by=$user_id, update_date='$pc_date_time' where id ='".$val["ID"]."'");
						if($rID9==false)
						{
							echo "20**Update product_details_master set current_stock=$currrent_stock, stock_value=$stock_value, updated_by=$user_id, update_date='$pc_date_time' where id ='".$val["ID"]."'";oci_rollback($con);disconnect($con);die;
						}
		...
		if($rID==1 && $rID2==1 && $rID3==1 && $prodUpdate==1 && $prod==1 && $rID5==1 && $rID6==1 && $rID7==1 && $rID8==1 && $rID9==1)
		{
		...
		//echo "10**$rID=$rID2=$rID3=$query=$prodUpdate=$prod=$rID5=$rID4=$rID6=$rID7=$rID8=$rID9";oci_rollback($con);disconnect($con);die;
		 
		$txt_from_order_id=str_replace("'","",$txt_from_order_id);
		if($db_type==0)
		{
			if($rID==1 && $rID2==1 && $rID3==1 && $prodUpdate==1 && $prod==1 && $rID5==1 && $rID6==1 && $rID7==1 && $rID8==1 && $rID9==1 && $query==1 && $rID4==1)
			{
				mysql_query("COMMIT");  
