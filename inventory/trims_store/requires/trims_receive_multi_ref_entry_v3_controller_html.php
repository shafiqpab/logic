...
if( $action == 'mrr_details' ) 
{
    ...
    <tbody>
        <tr id="row_1" align="center">
            ...
            <td width="100" id="ref_no_1"></td>
            <td width="100" id="article_no_1"></td>
            ...
            <td width="70" id="tdrate_1">

                <input name="rate[]" id="rate_1" class="text_boxes_numeric" type="text" style="width:50px;" value=""  onBlur="calculate(1);" readonly disabled/>
            </td>
            <td width="70" id="tdile_1">
                <input name="ile" id="ile_1" class="text_boxes_numeric" type="text" style="width:50px;" value=""  placeholder="ILE COST" readonly disabled />
            </td>
            <td width="70" id="tdamount_1">
                <input type="text" name="amount[]" id="amount_1" value="" class="text_boxes_numeric" style="width:50px;" readonly disabled />	
            </td>
            <td width="70" id="tdrejectrecvqnty_1">
                <input type="text" name="rejectrecvqnty[]" id="rejectrecvqnty_1" value="" class="text_boxes_numeric" style="width:50px;" />
            </td>
            <td width="70" id="tdbookcurrency_1">
            <input type="text" name="bookcurrency[]" id="bookcurrency_1" value="" class="text_boxes_numeric" style="width:50px;" readonly disabled />
            </td>
            <td width="70" id="PrevRcvQnty_1"></td>
            <td width="70" id="tdblqty_1">
            <input class="text_boxes_numeric"  name="blqty[]" id="blqty_1" value="" type="text" style="width:50px;" readonly disabled />
            <input  name="blamt[]" id="blamt_1" value="" type="hidden"/>
            </td>
            <td width="60" id="tdcbopaymentoverrecv_1">
            <? 
...
if( $action == 'show_fabric_desc_listview' ) 
{
	...
	else if($receive_basis==2)
	{
		...
		else
		{
			...
			/*$prev_entry="select c.prod_id, c.po_breakdown_id, sum(c.quantity) as quantity, d.item_group_id, d.item_description, d.color as grms_color, d.item_color, d.gmts_size, d.item_size   
			from inv_receive_master a, inv_transaction b, order_wise_pro_details c, product_details_master d 
			where a.id=b.mst_id and b.id=c.trans_id and a.entry_form=24 and c.entry_form=24 and b.item_category=4 and b.transaction_type=1 and c.trans_type=1 and a.company_id=$company and a.receive_basis=$receive_basis and a.booking_no='$bookingNo_piId' group by c.prod_id, c.po_breakdown_id, d.item_group_id, d.item_description, d.color, d.item_color, d.gmts_size, d.item_size";*/
	...
	$poDataArr = sql_select("select A.STYLE_REF_NO, B.ID, B.PO_NUMBER, B.PUB_SHIPMENT_DATE, B.GROUPING, C.ARTICLE_NUMBER, C.SIZE_NUMBER_ID, C.COLOR_NUMBER_ID from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c where a.job_no=b.job_no_mst and b.id = c.po_break_down_id and a.company_name=$company and ($po_id_cond) group by A.STYLE_REF_NO, B.ID, B.PO_NUMBER, B.PUB_SHIPMENT_DATE, B.GROUPING, C.ARTICLE_NUMBER, C.SIZE_NUMBER_ID, C.COLOR_NUMBER_ID");
...
//    $poDataArr = sql_select("select A.STYLE_REF_NO, B.ID, B.PO_NUMBER, B.PUB_SHIPMENT_DATE, B.GROUPING from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.company_name=$company");
	...
	foreach($data_array as $row)
	{
		...
		$amount=$qnty*$row[csf('rate')];
		$ile_amt=(($amount/100)*$ile_data[$row[csf('trim_group')]]);
		...
		if($qnty>0)
		{
			...
			<tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $i; ?>" align="center">
				...
				<td width="100" id="ref_no_<? echo $i; ?>" title="<? echo $row[csf("po_break_down_id")];?>"><? echo $po_arr[$row[csf("po_break_down_id")]][4]; ?></td>
				<td width="100" id="article_no_<? echo $i; ?>" title="<? echo $row[csf("po_break_down_id")];?>">
                    <?
				...
				<td width="70" id="tdreceiveqnty_<? echo $i; ?>">
					<input type="text" name="receiveqnty[]" id="receiveqnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:50px;" value="<? echo number_format($qnty,4,'.',''); ?>" onBlur="calculate(<? echo $i; ?>);"/>
				</td>
				<td width="70" id="tdrate_<? echo $i; ?>" title="<?= $book_avg_rate;?>">
					<input name="rate[]" id="rate_<? echo $i; ?>" class="text_boxes_numeric" type="text" style="width:50px;" value="<? echo number_format($book_avg_rate,6,'.',''); ?>"  onBlur="calculate(<? echo $i; ?>);" readonly disabled/>
				</td>
				<td width="70" id="tdile_<? echo $i; ?>">
					<input name="ile[]" id="ile_<? echo $i ?>" class="text_boxes_numeric" type="text" style="width:50px;" value="<? echo $ile_amt; ?>"  placeholder="ILE COST" readonly disabled />
				</td>
				<td width="70" id="tdamount_<? echo $i; ?>">
					<input type="text" name="amount[]" id="amount_<? echo $i; ?>" value="<? echo number_format($amount,4,'.','') ; ?>" class="text_boxes_numeric" style="width:50px;" readonly disabled />	
				</td>
				<td width="70" id="tdrejectrecvqnty_<? echo $i; ?>">
					<input type="text" name="rejectrecvqnty[]" id="rejectrecvqnty_<? echo $i; ?>" value="<? //echo $row[csf('')]; ?>" class="text_boxes_numeric" style="width:50px;" />
				</td>
				<td width="70" id="tdbookcurrency_<? echo $i; ?>">
				<input type="text" name="bookcurrency[]" id="bookcurrency_<? echo $i; ?>" value="<? echo number_format(($amount*$exchange_rate),2,'.',''); //*$lib_item_group_arrr[$row[csf('trim_group')]]["conversion_factor"] ?>" title="<? echo $qnty."=".$book_avg_rate."=".$exchange_rate;?>" class="text_boxes_numeric" style="width:50px;" readonly disabled />
				</td>
				<td width="70" id="PrevRcvQnty_<? echo $i; ?>" title="<? echo $prev_rcv_qnty;?>" align="right"><? echo number_format($prev_rcv_qnty,4,'.',''); ?></td>
				<td width="70" id="tdblqty_<? echo $i; ?>">
				<input class="text_boxes_numeric"  name="blqty[]" id="blqty_<? echo $i; ?>" value="<? echo number_format($qnty,4,'.',''); ?>" type="text" style="width:50px;" readonly disabled />
				<input name="blamt[]" id="blamt_<? echo $i;?>" value="<? echo number_format($amount,4,'.','');?>" type="hidden" />
				</td>
				<td width="60" id="tdcbopaymentoverrecv_<? echo $i; ?>">
				<? 
...
if( $action == 'show_fabric_desc_listview_update' ) 
{
    ...
    $po_arr=array();
    $poDataArr = sql_select("select A.STYLE_REF_NO, B.ID, B.PO_NUMBER, B.PUB_SHIPMENT_DATE, B.GROUPING, C.ARTICLE_NUMBER, C.SIZE_NUMBER_ID, C.COLOR_NUMBER_ID from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c where a.job_no=b.job_no_mst and b.id = c.po_break_down_id and a.company_name=$company $po_id_cond group by A.STYLE_REF_NO, B.ID, B.PO_NUMBER, B.PUB_SHIPMENT_DATE, B.GROUPING, C.ARTICLE_NUMBER, C.SIZE_NUMBER_ID, C.COLOR_NUMBER_ID");
	...
	foreach($update_details_data as $prod_id=>$prod_data)
	{
		foreach($prod_data as $po_id=>$po_data)
		{
			foreach($po_data as $po_rate=>$row)
			{
				...
				$ile_amt=(($amount/100)*$ile_data[$row[('trim_group')]]);
				...
				<tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $i; ?>" align="center">
					...
					<td width="100" id="ref_no_<? echo $i; ?>" title="<? echo $row[("po_breakdown_id")];?>"><? echo $po_arr[$row[("po_breakdown_id")]][4]; ?></td>
					<td width="100" id="article_no_<? echo $i; ?>" title="<? echo $row[("po_breakdown_id")];?>">
						<?
					...
					<td width="70" id="tdrate_<? echo $i; ?>"  title="<?= $row[('rate')];?>">
						<input name="rate[]" id="rate_<? echo $i; ?>" class="text_boxes_numeric" type="text" style="width:50px;" value="<? echo number_format($row[('rate')],6,'.',''); ?>"  onBlur="calculate(<? echo $i; ?>);" readonly disabled/>
					</td>
					<td width="70" id="tdile_<? echo $i; ?>">
						<input name="ile" id="ile_<? echo $i ?>" class="text_boxes_numeric" type="text" style="width:50px;" value="<? echo $ile_amt; ?>"  placeholder="ILE COST" readonly disabled />
					</td>
					...
					<td width="70" id="tdrejectrecvqnty_<? echo $i; ?>">
						<input type="text" name="rejectrecvqnty[]" id="rejectrecvqnty_<? echo $i; ?>" value="<? echo $row[('reject_qty')]; ?>" class="text_boxes_numeric" style="width:50px;" />
					</td>
					<td width="70" id="tdbookcurrency_<? echo $i; ?>">
					<input type="text" name="bookcurrency[]" id="bookcurrency_<? echo $i; ?>" value="<? echo number_format(($amount*$exchange_rate),2,'.',''); ?>" class="text_boxes_numeric" style="width:50px;" readonly disabled />
					</td>
					<td width="70" id="PrevRcvQnty_<? echo $i; ?>" title="<? echo $prev_rcv_qnty;?>" align="right"><? echo number_format($prev_rcv_qnty,4,'.',''); ?></td>
					<td width="70" id="tdblqty_<? echo $i; ?>">
					<input class="text_boxes_numeric"  name="blqty[]" id="blqty_<? echo $i; ?>" title="<? echo $bl_qnty;?>" value="<?  echo number_format($bl_qnty,4,'.',''); ?>" type="text" style="width:50px;" placeholder="<?  ?>" readonly disabled />
					<input class="text_boxes_numeric"  name="blamt[]" id="blamt_<? echo $i; ?>" value="<? echo number_format($bl_amount,4,'.',''); ?>" type="hidden" style="width:50px;" readonly disabled />
					</td>
					<td width="60" id="tdcbopaymentoverrecv_<? echo $i; ?>">
					<? 
...
if ($action=="save_update_delete")
{
	...
	//	if( check_table_status( $_SESSION['menu_id'], 1 )==0 ) { echo "15**0"; die;}
		...
		{
			$ile_arr=return_library_array("select item_group, standard  from variable_inv_ile_standard where source=$cbo_source and company_name=$cbo_company_id and category=4 and item_group in($all_item_group_id) and status_active=1 and is_deleted=0","item_group","standard");
		...
		foreach($item_wise_data as $item_key=>$item_val)
		{
			...
			$ile_percent=$ile_data[$item_group_id];
		...
		//if($user_id==1)  echo "10**insert into inv_transaction (".$field_array_trans.") values ".$data_array_trans;oci_rollback($con);disconnect($con);die;
		if($data_array_trans!="")
		{
...
if ($action=="trims_receive_entry_print")
{
    ...
    <?
		...
		if($orderIds!="")
		{
			...
			$piArray=array();
			$sql="select a.id, a.po_number, a.grouping as internal_ref, b.BUYER_NAME as buyer_id from wo_po_break_down a, wo_po_details_master b where a.job_id=b.id and a.id in($orderIds)";
			//echo $sql;
		...
		if($dataArray[0][csf('receive_basis')]==2)
		{
			...
			/*$sql_recv = "select a.booking_no,b.order_id as po_id,b.item_group_id as item_group,b.item_description,b.item_color,b.item_size,a.recv_number,sum(c.quantity) as  receive_qnty 
			from inv_receive_master a, inv_trims_entry_dtls b, order_wise_pro_details c
			where a.id=b.mst_id and b.id=c.dtls_id and b.trans_id=c.trans_id and c.entry_form=24 and a.entry_form=24 and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0 and c.po_breakdown_id in($orderIds) 
			group by a.recv_number,a.booking_no,b.item_group_id,b.item_description,b.item_color,b.item_size,b.order_id";*/
		...
		from inv_trims_entry_dtls b left join inv_goods_placement c on c.dtls_id=b.id and c.entry_form=24 where b.mst_id='$mst_id' and b.status_active='1' and b.is_deleted='0'";*/
		 ...
		 /*$sql_dtls="select b.booking_no, b.booking_id, b.booking_without_order, b.item_group_id, b.item_description, b.order_id, b.gmts_color_id, b.item_color, b.item_size, sum(b.cons_qnty) as cons_qnty, b.cons_uom, sum(b.receive_qnty) as receive_qnty, max(b.rate) as rate, sum(b.amount) as amount, sum(b.reject_receive_qnty) as reject_receive_qnty, b.gmts_size_id 
		from inv_trims_entry_dtls b 
		where b.mst_id='$mst_id' and b.status_active='1' and b.is_deleted='0'
		group by b.booking_no, b.booking_id, b.booking_without_order, b.item_group_id, b.item_description, b.order_id, b.gmts_color_id, b.item_color, b.item_size, b.cons_uom, b.gmts_size_id";*/
                ...
                <tr bgcolor="<? echo $bgcolor; ?>">
					...
					<?
                        ...
                        else
                        {
							foreach($order_id_arr as $po_id)
							{
								//echo $row[csf('booking_no')]."=".$po_id."=".$row[csf('item_group_id')]."=".$row[csf('gmts_color_id')]."=".$row[csf('item_color')]."=".$des_dtls."=".$row[csf('gmts_size_id')]."=".$row[csf('item_size')]."<br>";
                    ...
                    <td align="right"><? echo number_format($row[csf('amount')]/$row[csf('receive_qnty')],4,'.',''); ?></td>
...
{ 
  ...
  <div style="width:1550px;margin-left:2px;">
    ...
    <br>
	<div style="width:100%; ">
        <table align="right" cellspacing="0" width="1550"  border="1" rules="all" class="rpt_table" >
            <thead bgcolor="#dddddd" align="center">
                <th width="30">SL</th>
                <th width="80" align="center">WO/PI No.</th>
    ...
    <? //book_keeping_curr
		...
		where b.mst_id='$mst_id' and b.status_active='1' and b.is_deleted='0'
		group by b.booking_no, b.booking_id, b.booking_without_order, b.item_group_id, b.item_description, b.order_id, b.gmts_color_id, b.item_color, b.item_size, b.cons_uom, b.gmts_size_id";*/
		 $sql_dtls="select b.booking_no, b.booking_id, b.booking_without_order, b.item_group_id, b.item_description, b.order_id, b.gmts_color_id, b.item_color, b.item_size, sum(b.cons_qnty) as cons_qnty, b.cons_uom, sum(b.receive_qnty) as receive_qnty,sum(b.book_keeping_curr) as book_keeping_curr, max(b.rate) as rate, sum(b.amount) as amount, sum(b.reject_receive_qnty) as reject_receive_qnty, b.gmts_size_id, b.floor, b.room_no, b.rack_no, b.self_no, b.box_bin_no 
		from inv_trims_entry_dtls b 
		where b.mst_id='$mst_id' and b.status_active='1' and b.is_deleted='0'
  ...
  <div style="width:1550px;margin-left:2px;">
    ...
    <? //book_keeping_curr
		...
		if($dataArray[0][csf('receive_basis')]==2)
		{
			...
			$recv_wo_data_arr=array();$recv_wo_data_arr_amt=array();
			$sql_recv = "select a.id as mst_id, a.booking_no, c.po_breakdown_id as po_id, b.item_group_id as item_group, b.item_description, b.gmts_color_id, b.item_color, b.item_size, a.recv_number, c.quantity as receive_qnty 
			from inv_receive_master a, inv_trims_entry_dtls b, order_wise_pro_details c
			...
			foreach($recv_data as $row)
			{ //pre_cost_fabric_cost_dtls_id
				$recv_wo_data_arr_amt[$row[csf('booking_no')]][$row[csf('po_id')]][$row[csf('item_group')]][$row[csf('item_description')]][$row[csf('gmts_color_id')]][$row[csf('item_color')]][$row[csf('item_size')]]['recv_qty']+=$row[csf('receive_qnty')];
			}
        ...
        foreach($sql_result as $row)
        {
            ...
            if ($i%2==0)  
				...
				//$recv_no_arr=implode(",",array_unique(explode(",",$recv_wo_data_arr[$row[csf('booking_no')]][$row[csf('order_id')]][$row[csf('item_group_id')]][$row[csf('item_description')]]['recv_no'])));
				...
				//{
					//if($recv_id!=$dataArray[0][csf('recv_number')])
					//{
						// echo $recv_id."_".$row[csf('booking_no')]."_".$po_id."_".$row[csf('item_group_id')]."_".$row[csf('item_description')]."--------------------------";
						//$prev_recv_qty+=$recv_wo_data_arr_amt[$recv_id][$row[csf('booking_no')]][$row[csf('order_id')]][$row[csf('item_group_id')]][$row[csf('item_description')]][$row[csf('gmts_color_id')]][$row[csf('item_color')]][$row[csf('item_size')]]['recv_qty'];
					//}
				//}
				$prev_recv_qty+=$recv_wo_data_arr_amt[$row[csf('booking_no')]][$row[csf('order_id')]][$row[csf('item_group_id')]][$row[csf('item_description')]][$row[csf('gmts_color_id')]][$row[csf('item_color')]][$row[csf('item_size')]]['recv_qty'];
				//echo $prev_recv_qty;
				// $order_number=chop($order_number,',');
				// $order_number=implode(",",array_unique(explode(",",$order_number)));
				//$grouping_number=chop($grouping_number,',');
                ...
                <tr bgcolor="<? echo $bgcolor; ?>">
					...
					<?
                        ...
                        {
                                $woorder_qty+=$booking_qty_arr[$row[csf('booking_no')]][$row[csf('po_breakdown_id')]][$row[csf('item_group_id')]][$row[csf('gmts_color_id')]][$row[csf('item_color')]][$row[csf('item_description')]][$row[csf('size_id')]][$row[csf('item_size')]];
						...
						$tot_recv_balance=$woorder_qty-($row[csf('quantity')]+$prev_recv_qty+$runtime_rcv_qnty[$row[csf('booking_no')]][$row[csf('order_id')]][$row[csf('item_group_id')]][$row[csf('item_description')]][$row[csf('gmts_color_id')]][$row[csf('item_color')]][$row[csf('item_size')]]);
                    ?>
                    </td>
                    ...
                    <td align="right" title="<?= $prev_recv_qty;?>"><? echo number_format($tot_recv_balance,2,'.',''); ?></td>
                    <td align="right"><? echo number_format($row[csf('reject_receive_qnty')],2,'.',''); ?></td>
                </tr>
            <?
			$runtime_rcv_qnty[$row[csf('booking_no')]][$row[csf('order_id')]][$row[csf('item_group_id')]][$row[csf('item_description')]][$row[csf('gmts_color_id')]][$row[csf('item_color')]][$row[csf('item_size')]]+=$row[csf('quantity')];
			$i++;
