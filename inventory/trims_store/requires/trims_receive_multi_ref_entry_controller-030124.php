						...
						<div style="width:690px; max-height:280px; overflow-y:scroll" id="list_container" align="left"> 
							<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="670" id="tbl_list_search">  
								...
								for($z=0;$z<count($explSaveData);$z++)
								{
									...
									if($receive_basis==1)
									{
										$tot_recv_qnty=return_field_value("sum(b.quantity) as qnty","inv_receive_master m, inv_trims_entry_dtls a, order_wise_pro_details b","m.id=a.mst_id and m.receive_basis=1 and m.booking_id='$booking_pi_id' and a.id=b.dtls_id and a.item_group_id='$item_group' and trim(REGEXP_REPLACE(rtrim(a.item_description,', [BS]'), '\s{2,}', ' '))=REGEXP_REPLACE('".trim(str_replace("'","",$item_description))."', '\s{2,}', ' ') and $brand_supref_cond and $gmts_color_field='$gmts_color_id' and $item_color_field='$item_color_id' and $gmts_size_field='$gmts_size_id' and $item_size_cond and b.entry_form=24 and b.trans_type=1 and b.po_breakdown_id='$order_id' $rate_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","qnty");
									}
									else
									{
										$tot_recv_qnty=return_field_value("sum(b.quantity) as qnty","inv_receive_master m, inv_trims_entry_dtls a, order_wise_pro_details b","m.id=a.mst_id and m.receive_basis=$receive_basis and m.entry_form=24 and a.id=b.dtls_id and a.item_group_id='$item_group' and trim(REGEXP_REPLACE(rtrim(a.item_description,', [BS]'), '\s{2,}', ' '))=REGEXP_REPLACE('".trim(str_replace("'","",$item_description))."', '\s{2,}', ' ') and $brand_supref_cond and $gmts_color_field='$gmts_color_id' and $item_color_field='$item_color_id' and $gmts_size_field='$gmts_size_id' and $item_size_cond and b.entry_form=24 and b.trans_type=1 and b.po_breakdown_id='$order_id' $rate_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","qnty");
									}
				...
				<div style="width:690px; max-height:280px; overflow-y:scroll" id="list_container" align="left"> 
					<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="670" id="tbl_list_search">  
						...
						else if($save_data!="" && ($receive_basis==2 || $receive_basis==1))
						{
							...
							{
								$description_cond=" and trim(REGEXP_REPLACE(c.description, '\s{2,}', ' '))=REGEXP_REPLACE('".trim(str_replace("'","",$item_description))."', '\s{2,}', ' ')";
							}
						...
						else
						{
							...
							else
							{
								...
								else if($receive_basis==2)
								{
									...
									{
										$item_description_con=" and trim(REGEXP_REPLACE(c.description, '\s{2,}', ' '))=REGEXP_REPLACE('".trim(str_replace("'","",$item_description))."', '\s{2,}', ' ')";
									}
									else 
									{
							...
							{  
								...
								if(trim(str_replace("'","",$item_description)) !="" || $db_type==0)
								{
									$description_cond_rcv=" and trim(REGEXP_REPLACE(rtrim(a.item_description,', [BS]'), '\s{2,}', ' '))=REGEXP_REPLACE('".trim(str_replace("'","",$item_description))."', '\s{2,}', ' ')";
								}
								else
								{
...
if($action=="put_balance_qnty")
{
	...
	if($recieve_basis==1 || $recieve_basis==2)
	{
		if($recieve_basis==1)
		{
			if($db_type==0)
			{
				$sql="select quantity as qnty, rate from com_pi_item_details where pi_id='$bookingNo_piId' and item_group='$item_group' and trim(REGEXP_REPLACE(item_description, '\s{2,}', ' '))=REGEXP_REPLACE('".trim(str_replace("'","",$item_description))."', '\s{2,}', ' ') and brand_supplier='".trim($brand_supref)."' and color_id='$gmts_color_id' and item_color='$item_color_id' and size_id='$gmts_size_id' and item_size='$item_size' $rate_cond and status_active=1 and is_deleted=0";
			}
			else
			{
				...
				$sql="select sum(quantity) as qnty, avg(rate) as rate from com_pi_item_details where pi_id='$bookingNo_piId' and item_group='$item_group' and trim(REGEXP_REPLACE(item_description, '\s{2,}', ' '))=REGEXP_REPLACE('".trim(str_replace("'","",$item_description))."', '\s{2,}', ' ') and nvl(color_id,0)='$gmts_color_id' and nvl(item_color,0)='$item_color_id' and nvl(size_id,0)='$gmts_size_id' and $brand_supref_cond and $item_size_cond $rate_cond and status_active=1 and is_deleted=0";
			}
		}
		else if($recieve_basis==2)
		{
			...
			else
			{
				...
				if($booking_without_order==1)
				{
					$sql = "select trim_qty as qnty, rate from wo_non_ord_samp_booking_dtls where booking_no='$bookingNo_piNo' and trim_group='$item_group' and trim(REGEXP_REPLACE(fabric_description, '\s{2,}', ' '))=REGEXP_REPLACE('".trim(str_replace("'","",$item_description))."', '\s{2,}', ' ') and nvl(gmts_color,0)='$gmts_color_id' and nvl(gmts_size,0)='$gmts_size_id' and nvl(fabric_color,0)='$item_color_id' and status_active=1 and is_deleted=0 and $item_size_cond_samp and $brand_supref_cond_samp $rate_cond";
				}
				else
				{
					$sql="select sum(c.cons) as qnty, avg(c.rate) as rate from wo_booking_dtls b, wo_trim_book_con_dtls c where b.id=c.wo_trim_booking_dtls_id and b.booking_no='$bookingNo_piNo' and b.sensitivity='$sensitivity' and b.trim_group='$item_group' and trim(REGEXP_REPLACE(c.description, '\s{2,}', ' ')) =REGEXP_REPLACE('".trim(str_replace("'","",$item_description))."', '\s{2,}', ' ') and nvl(c.color_number_id,0)='$gmts_color_id' and nvl(c.gmts_sizes,0)='$gmts_size_id' and nvl(c.item_color,0)='$item_color_id' and $brand_supref_cond and $item_size_cond  $orderCon $rate_condition and b.status_active=1 and b.is_deleted=0";
				}
			}
		}
		//echo $sql;die;
		...
		if($recieve_basis==1 || $recieve_basis==2)
		{
			...
			if($db_type==0)
			{
				$receive_qnty=return_field_value("sum(b.quantity) as qnty","inv_receive_master m, inv_trims_entry_dtls a, order_wise_pro_details b","m.id=a.mst_id and a.id=b.dtls_id and b.entry_form=24 and m.entry_form=24 and b.po_breakdown_id in($order_id) and m.receive_basis=$recieve_basis and m.booking_id='$bookingNo_piId' and a.item_group_id='$item_group' and trim(REGEXP_REPLACE(rtrim(a.item_description,', [BS]'), '\s{2,}', ' '))=trim(REGEXP_REPLACE('".str_replace("'","",$item_description)."', '\s{2,}', ' ')) and a.brand_supplier='".trim($brand_supref)."' and a.gmts_color_id='$gmts_color_id' and a.item_color='$item_color_id' and a.gmts_size_id ='$item_size_id' and a.item_size ='$item_size' $rate_condition and a.status_active=1 and a.is_deleted=0","qnty");
			}
			else
			{
				...
				if($booking_without_order==1)
				{
					$receive_qnty=return_field_value("sum(a.receive_qnty) as qnty","inv_receive_master m, inv_trims_entry_dtls a","m.id=a.mst_id and m.entry_form=24 and m.receive_basis=$recieve_basis and m.booking_id='$bookingNo_piId' and a.item_group_id='$item_group' and trim(REGEXP_REPLACE(rtrim(a.item_description,', [BS]'), '\s{2,}', ' '))=trim(REGEXP_REPLACE('".str_replace("'","",$item_description)."', '\s{2,}', ' ')) and nvl(a.gmts_color_id,0)='$gmts_color_id' and nvl(a.item_color,0)='$item_color_id' and nvl(a.gmts_size_id,0)='$gmts_size_id' and $brand_supref_cond and $item_size_cond $rate_condition and a.status_active=1 and a.is_deleted=0","qnty");
				}
				else
				{
					$receive_qnty=return_field_value("sum(b.quantity) as qnty","inv_receive_master m, inv_trims_entry_dtls a, order_wise_pro_details b","m.id=a.mst_id and a.id=b.dtls_id and b.entry_form=24 and m.entry_form=24 and b.po_breakdown_id in($order_id) and m.receive_basis=$recieve_basis and m.booking_id='$bookingNo_piId' and a.item_group_id='$item_group' and trim(REGEXP_REPLACE(rtrim(a.item_description,', [BS]'), '\s{2,}', ' '))=trim(REGEXP_REPLACE('".str_replace("'","",$item_description)."', '\s{2,}', ' ')) and nvl(a.gmts_color_id,0)='$gmts_color_id' and nvl(a.item_color,0)='$item_color_id' and nvl(a.gmts_size_id,0)='$gmts_size_id' and $brand_supref_cond and $item_size_cond $rate_condition and a.status_active=1 and a.is_deleted=0","qnty");
			...
			$sql_cond_prev="";
			if($item_description!="") $sql_cond_prev.="and trim(REGEXP_REPLACE(rtrim(c.item_description,', [BS]'), '\s{2,}', ' '))=trim(REGEXP_REPLACE('".str_replace("'","",$item_description)."', '\s{2,}', ' '))";
...
if ($action=="save_update_delete")
{
	...
	if($rcv_basis==1 || $rcv_basis==2)
	{
		...
		$sql_cond_prev="";
		if(str_replace("'","",$hidden_item_description)!="") $sql_cond_prev.=" and trim(REGEXP_REPLACE(rtrim(c.item_description,', [BS]'), '\s{2,}', ' '))=trim(REGEXP_REPLACE('".str_replace("'","",$hidden_item_description)."', '\s{2,}', ' '))";
		...
		{
			$sql_cond_pi="";
			if(str_replace("'","",$hidden_item_description)!="") $sql_cond_pi.=" and trim(REGEXP_REPLACE(item_description, '\s{2,}', ' '))=REGEXP_REPLACE('".trim(str_replace("'","",$hidden_item_description))."', '\s{2,}', ' ')";
 	...
 	foreach($delivary_data as $rows)
  	{
		...
		{
			//echo $key."<br>";
			$row_prod=sql_select("select id, current_stock, avg_rate_per_unit, stock_value from product_details_master where company_id=$cbo_company_id and item_category_id=4 and entry_form=24 and item_group_id=$cbo_item_group and trim(REGEXP_REPLACE(rtrim(item_description,', [BS]'), '\s{2,}', ' '))=REGEXP_REPLACE('".trim(str_replace("'","",$item_desc))."', '\s{2,}', ' ') and item_color=$txt_item_color_id and item_size='$txt_item_size_id' and color=$gmts_color_id and gmts_size='$gmts_size_id' and status_active=1 and is_deleted=0");
