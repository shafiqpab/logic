...
if($action=='show_fabric_desc_listview')
{
	...
	else if($receive_basis==2)
	{
		...
		/*$rate_cond_rcv=" and ROUND(rate,6)=ROUND($booking_rate,6)";
    ...
    <div style="width:<? echo $table_width+20; ?>px; overflow-y: scroll; max-height:400px;" id="scroll_body">
    	<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="<? echo $table_width; ?>" id="tbl_list_search">
            ...
            {  
				...
				//$rate=$row[csf('rate')];
				//$rate=ltrim($rate, '0');
				//echo $order_id."**".$row[csf('trim_group')]."**".$row[csf('description')]."**".$row[csf('brand_supplier')]."**".$row[csf('color_id')]."**".$row[csf('item_color')]."**".$row[csf('size_id')]."**".$row[csf('item_size')]."**".$rate."#";
			...
			<div style="width:800px; max-height:280px; overflow-y:scroll" id="list_container" align="left"> 
				<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="790" id="tbl_list_search">  
					...
					{ 
						...
						{
							$description_cond_rcv=" and trim(a.item_description)='".trim(str_replace(", [BS]","",str_replace("'","",$item_description)))."'";
						}
						else
						{
...
if($action=="put_balance_qnty")
{
	...
	if($recieve_basis==1 || $recieve_basis==2)
	{
		if($recieve_basis==1)
		{
			...
			if($db_type==0)
			{
				$sql="select quantity as qnty, rate, amount as amount from com_pi_item_details where pi_id='$bookingNo_piId' and item_group='$item_group' and rtrim(item_description,', [BS]')='".trim($item_description)."' and trim(brand_supplier)='".trim($brand_supref)."' and color_id='$gmts_color_id' and item_color='$item_color_id' and size_id='$gmts_size_id' and trim(item_size)='".trim($item_size)."' $rate_cond and status_active=1 and is_deleted=0";
			}
			else
			{
		...
		if($recieve_basis==1 || $recieve_basis==2)
		{
			...
			if($db_type==0)
			{
				$receive_qnty=return_field_value("sum(b.quantity) as qnty","inv_receive_master m, inv_trims_entry_dtls a, order_wise_pro_details b","m.id=a.mst_id and a.id=b.dtls_id and b.entry_form=24 and m.entry_form=24 and b.po_breakdown_id in($order_id) and m.receive_basis=$recieve_basis and m.booking_id='$bookingNo_piId' and a.item_group_id='$item_group' and trim(a.item_description)='".trim($item_description)."' and trim(a.brand_supplier)='".trim($brand_supref)."' and a.gmts_color_id='$gmts_color_id' and a.item_color='$item_color_id' and a.gmts_size_id ='$item_size_id' and trim(a.item_size)='".trim($item_size)."' and a.status_active=1 and a.is_deleted=0 and m.status_active=1 and m.is_deleted=0 $rate_cond_rcv","qnty");
			}
			else
			{
...
if ($action=="save_update_delete")
{
	...
	if($rcv_basis==1 || $rcv_basis==2)
	{
		...
		$sql_cond_prev="";
		if(str_replace("'","",$hidden_item_description)!="") $sql_cond_prev.=" and rtrim(c.item_description,', [BS]')='".trim(str_replace("'","",$hidden_item_description))."'"; else $sql_cond_prev.=" and c.item_description is null";
		...
		if($rcv_basis==2)
		{
			...
			{
				$sql_cond_wo="";
				if(str_replace("'","",$hidden_item_description)!="") $sql_cond_wo.=" and trim(c.description)='".trim(str_replace("'","",$hidden_item_description))."'";  else $sql_cond_wo.=" and c.description is null";
		...
		{
			$sql_cond_pi="";
			if(str_replace("'","",$hidden_item_description)!="") $sql_cond_pi.=" and trim(item_description)='".trim(str_replace("'","",$hidden_item_description))."'"; else $sql_cond_pi.=" and item_description is null";
