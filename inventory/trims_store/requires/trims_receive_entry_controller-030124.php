	...
	if($recieve_basis==1) //Pi Basis
	{
		...
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1272" class="rpt_table">
			<thead>
				...
				<th width="60">Internal File Noffffffffff</th>
...
if($action=='show_fabric_desc_listview')
{
	...
	if($receive_basis==1)
	{
		...
		$prev_rcv_sql="select b.po_breakdown_id, a.item_group_id, a.item_description, a.brand_supplier, a.gmts_color_id, a.item_color, a.gmts_size_id, a.item_size, b.quantity as qnty ,b.ORDER_RATE as rate, a.sensitivity
	...
	else if($receive_basis==2)
	{
		...
		else
		{
			...
			//ROUND(a.rate,6)
			$prev_rcv_sql="select b.po_breakdown_id, a.item_group_id, a.item_description, a.brand_supplier, a.gmts_color_id, a.item_color, a.gmts_size_id, a.item_size, b.quantity as qnty, a.rate as rate, a.sensitivity
			from inv_receive_master m, inv_trims_entry_dtls a, order_wise_pro_details b 
	...
	{
		$prev_rcv_data[$row[csf("po_breakdown_id")]][$row[csf("item_group_id")]][trim($row[csf("item_description")])][trim($row[csf("brand_supplier")])][$row[csf("gmts_color_id")]][$row[csf("item_color")]][$row[csf("gmts_size_id")]][trim($row[csf("item_size")])][trim($row[csf("sensitivity")])][number_format($row[csf("rate")],6,".","")]["qnty"]+=$row[csf("qnty")];
	}
    ...
    <div style="width:<? echo $table_width+20; ?>px; overflow-y: scroll; max-height:400px;" id="scroll_body">
    	<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="<? echo $table_width; ?>" id="tbl_list_search">
            ...
            {  
				...
				//$rate=$row[csf('rate')];
				//$rate=ltrim($rate, '0');
				//echo $order_id."**".$row[csf('trim_group')]."**".$row[csf('description')]."**".$row[csf('brand_supplier')]."**".$row[csf('color_id')]."**".$row[csf('item_color')]."**".$row[csf('size_id')]."**".$row[csf('item_size')]."**".$rate."**".$row[csf('sensitivity')]."#";
				if($row[csf('color_id')]=="") $row[csf('color_id')]=0;
				if($row[csf('item_color')]=="") $row[csf('item_color')]=0;
				if($row[csf('size_id')]=="") $row[csf('size_id')]=0;
				$prev_rec_qnty=$prev_rcv_data[$order_id][$row[csf('trim_group')]][trim($row[csf('description')])][trim($row[csf('brand_supplier')])][$row[csf('color_id')]][$row[csf('item_color')]][$row[csf('size_id')]][trim($row[csf('item_size')])][trim($row[csf('sensitivity')])][$rate]["qnty"];
			...
			<div style="width:800px; max-height:280px; overflow-y:scroll" id="list_container" align="left"> 
				<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="790" id="tbl_list_search">  
					...
					{ 
						...
						{
							$description_cond_rcv=" and trim(REGEXP_REPLACE(rtrim(a.item_description,', [BS]'), '\s{2,}', ' '))=REGEXP_REPLACE('".trim(str_replace(", [BS]","",str_replace("'","",$item_description)))."', '\s{2,}', ' ')";
						}
						else
						{
...
if($action=="put_balance_qnty")
{
	...
	if($recieve_basis==1 || $recieve_basis==2)
	{
		...
		if($recieve_basis==1 || $recieve_basis==2)
		{
			...
			if($db_type==0)
			{
				$receive_qnty=return_field_value("sum(b.quantity) as qnty","inv_receive_master m, inv_trims_entry_dtls a, order_wise_pro_details b","m.id=a.mst_id and a.id=b.dtls_id and b.entry_form=24 and m.entry_form=24 and b.po_breakdown_id in($order_id) and m.receive_basis=$recieve_basis and m.booking_id='$bookingNo_piId' and a.item_group_id='$item_group' and trim(REGEXP_REPLACE(rtrim(a.item_description,', [BS]'), '\s{2,}', ' '))=REGEXP_REPLACE('".trim($item_description)."', '\s{2,}', ' ') and trim(a.brand_supplier)='".trim($brand_supref)."' and a.gmts_color_id='$gmts_color_id' and a.item_color='$item_color_id' and a.gmts_size_id ='$item_size_id' and trim(a.item_size)='".trim($item_size)."' and a.status_active=1 and a.is_deleted=0 and m.status_active=1 and m.is_deleted=0 $rate_cond_rcv","qnty");
			}
			else
			{
				...
				$receive_qnty=return_field_value("sum(a.receive_qnty) as qnty","inv_receive_master m, inv_trims_entry_dtls a","m.id=a.mst_id and m.entry_form=24 and m.receive_basis=$recieve_basis and m.booking_id='$bookingNo_piId' and a.item_group_id='$item_group' and trim(REGEXP_REPLACE(rtrim(a.item_description,', [BS]'), '\s{2,}', ' '))=REGEXP_REPLACE('".trim($item_description)."', '\s{2,}', ' ') and nvl(a.gmts_color_id,0)='$gmts_color_id' and nvl(a.item_color,0)='$item_color_id' and nvl(a.gmts_size_id,0)='$gmts_size_id' and $brand_supref_cond and $item_size_cond and a.status_active=1 and a.is_deleted=0  and m.status_active=1 and m.is_deleted=0 $rate_cond_rcv $sensitivity_cond","qnty");
			...
			$sql_cond_prev="";
			if($item_description!="") $sql_cond_prev.=" and trim(REGEXP_REPLACE(rtrim(c.item_description,', [BS]'), '\s{2,}', ' '))=REGEXP_REPLACE('".trim($item_description)."', '\s{2,}', ' ')";
...
if ($action=="save_update_delete")
{
	...
	if($rcv_basis==1 || $rcv_basis==2)
	{
		...
		$sql_cond_prev="";
		if(str_replace("'","",$hidden_item_description)!="") $sql_cond_prev.=" and trim(REGEXP_REPLACE(rtrim(c.item_description,', [BS]'), '\s{2,}', ' '))=REGEXP_REPLACE('".trim(str_replace("'","",$hidden_item_description))."', '\s{2,}', ' ')"; else $sql_cond_prev.=" and c.item_description is null";
