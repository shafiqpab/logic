	...
	<table width="750" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
            ...
            <tbody>
                <tr class="general">
                    <td>
                        <?  
                            $search_by = array(1=>'Receive Id',2=>'Challan No');
                    ...
                    <td align="center">
                        <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $company; ?>, 'create_mrr_search_list_view', 'search_div', 'trims_receive_rtn_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />				
                    </td>
            </tr>
        	<tr>                  
...
if($action=="create_mrr_search_list_view")
{
	...
	$sql = "select a.id,a.recv_number_prefix_num,a.recv_number,$year_field, a.challan_no,a.receive_date,a.receive_basis,sum(b.cons_quantity) as receive_qnty from inv_transaction b, inv_receive_master a where a.id=b.mst_id and a.entry_form in(24) and a.status_active=1 $sql_cond group by a.id, a.recv_number_prefix_num ,a.recv_number,a.challan_no,a.receive_date,a.receive_basis,a.insert_date order by a.id";
	//echo $sql;
	?>
...
{
	$sql = "select id,recv_number,entry_form,company_id,receive_basis,receive_purpose,lc_no,supplier_id,store_id
			from inv_receive_master 
			where id='$data' and entry_form in(24)";
	//echo $sql;die;
	$res = sql_select($sql);
	foreach($res as $row)
	{
		...
		echo"show_list_view('".$row[csf("id")]."','show_product_listview','list_product_container','requires/trims_receive_rtn_controller','');\n";
   	}	
	exit();	
}
    	...
    	<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all">
            ...
            <tbody>
				...
				{ 
					...
					?>
					<tr bgcolor="<? echo $bgcolor; ?>" onClick='get_php_form_data("<? echo $row[csf("tr_id")];?>","item_details_form_input","requires/trims_receive_rtn_controller")' style="cursor:pointer" >
					<td><? echo $i; ?></td>
...
{
	$sql = "select a.id as trans_id, b.id as prod_id, b.product_name_details, b.item_group_id, b.item_color, b.item_size, b.brand_supplier, b.current_stock, a.mst_id,a.order_uom, a.order_rate, a.cons_uom, a.cons_rate, a.cons_quantity, a.cons_amount, a.balance_qnty, a.balance_amount, c.booking_without_order
			from inv_transaction a, product_details_master b, inv_receive_master c
 			where a.id=$data and a.status_active=1 and a.prod_id=b.id and c.id=a.mst_id and b.status_active=1";
 	//echo $sql;die;
	$result = sql_select($sql);
	foreach($result as $row)
	{
		...
		echo "$('#cbo_item_color').val(".$row[csf("item_color")].");\n";
		echo "$('#txt_brad_supp').val('".$row[csf("brand_supplier")]."');\n";
		echo "$('#txt_item_size').val('".$row[csf("item_size")]."');\n";
		...
		{
			echo "$('#txt_return_qnty').removeAttr('placeholder').removeAttr('readonly').removeAttr('onDblClick').attr('placeholder','write').attr('onBlur','calc_tot_amt(this.value);');\n";
		}
		else
		{
		...
		echo "$('#txt_cons_uom').val('".$row[csf("cons_uom")]."');\n";
...
if($action=="return_po_popup")
{
	...
	{
		$order_sql=sql_select("select po_breakdown_id,quantity from order_wise_pro_details where trans_id=$update_id and trans_type=3 and entry_form=49 and status_active=1");
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
 		...
 		if(str_replace("'","",$issue_mst_id)!="")
		{
			...
			$id=$issue_mst_id;
			//issue master table UPDATE here START----------------------//	 cbo_store_name	
 			$field_array_mst="company_id*store_id*issue_date*supplier_id*received_id*received_mrr_no*pi_id*updated_by*update_date";
			$data_array_mst=$cbo_company_name."*".$cbo_store_name."*".$txt_return_date."*".$cbo_return_to."*".$txt_received_id."*".$txt_mrr_no."*".$pi_id."*'".$user_id."'*'".$pc_date_time."'";
		...
		{	 
			...
			$new_return_number=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'TRR', date("Y",time()), 5, "select issue_number_prefix,issue_number_prefix_num from inv_issue_master where company_id=$cbo_company_name and entry_form=49 and $year_cond=".date('Y',time())." order by id DESC ", "issue_number_prefix", "issue_number_prefix_num" ));
			
 			$field_array_mst="id, issue_number_prefix, issue_number_prefix_num, issue_number, entry_form, item_category, company_id, store_id, issue_date,supplier_id,received_id, received_mrr_no, pi_id, inserted_by, insert_date";
			$data_array_mst="(".$id.",'".$new_return_number[1]."','".$new_return_number[2]."','".$new_return_number[0]."',49,4,".$cbo_company_name.",".$cbo_store_name.",".$txt_return_date.",".$cbo_return_to.",".$txt_received_id.",".$txt_mrr_no.",".$pi_id.",'".$user_id."','".$pc_date_time."')";
			//echo "20**".$field_array."<br>".$data_array;die;
		}
		
		//transaction table insert here START--------------------------------//cbo_uom
		
		$conversion_factor=return_field_value("conversion_factor","lib_item_group","id=$cbo_item_group and item_category=4");
		$consump_qnty=(($txt_return_qnty*1)*($conversion_factor*1));
		$consump_amount=($consump_qnty*($txt_cons_rate*1));	
		
		$transactionID = return_next_id("id", "inv_transaction", 1); 
		$field_array_trans = "id,mst_id,company_id,store_id,prod_id,item_category,transaction_type,transaction_date,order_uom,order_qnty,order_rate,order_amount,cons_uom,cons_quantity,cons_rate,cons_amount,remarks,inserted_by,insert_date";
 		$data_array_trans = "(".$transactionID.",".$id.",".$cbo_company_name.",".$cbo_store_name.",".$txt_prod_id.",4,3,".$txt_return_date.",".$cbo_uom.",".$txt_return_qnty.",".$txt_rate.",".$txt_amount.",".$txt_cons_uom.",".$consump_qnty.",".$txt_cons_rate.",".$consump_amount.",".$txt_remarks.",'".$user_id."','".$pc_date_time."')"; 
		...
		$nowStock 		= $presentStock-$consump_qnty;
		$now_stock_value = $stock_value-$consump_amount;
			
		$field_array_prod="last_issued_qnty*current_stock*stock_value*updated_by*update_date";
		$data_array_prod=$consump_qnty."*".$nowStock."*".$now_stock_value."*'".$user_id."'*'".$pc_date_time."'";
		...
		if(!empty($txt_break_qnty))
		{
			foreach($ordr_wise_rtn_qnty_arr as $val)
			{
				...
				if($order_qnty_arr[1]>0)
				{
					...
					$data_array_proportion.="(".$proportion_id.",".$transactionID.",3,49,".$order_qnty_arr[0].",".$txt_prod_id.",".$order_qnty_arr[1].",'".$user_id."','".$pc_date_time."')";
					$proportion_id++;
				}
			}
			...
			if($variable_setting_production==1)
			{
				...
				foreach($ordr_wise_rtn_roll_arr as $val)
				{
					...
					if($order_roll_arr[1]>0)
					{
						...
						$data_array_roll.="(".$roll_id.",".$id.",".$transactionID.",".$order_roll_arr[0].",49,".$order_roll_arr[1].",".$order_roll_arr[2].",'".$user_id."','".$pc_date_time."')";
						$roll_id++;
					}
				}
			}
		...
		$data_array = "(".$mrrWiseIsID.",".$hidden_receive_trans_id.",".$transactionID.",49,".$txt_prod_id.",".$consump_qnty.",".$txt_cons_rate.",".$consump_amount.",'".$user_id."','".$pc_date_time."')";
		...
		$balance_amt = $sql_receive[0][csf("balance_amount")];
		$issueQntyBalance=$balance_qnty-$consump_qnty;
		$issueamtBalance=$balance_amt-$consump_amount;
		$update_data="".$issueQntyBalance."*".$issueamtBalance."*'".$user_id."'*'".$pc_date_time."'";
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		$sql_before = sql_select( "select a.id,a.current_stock, a.stock_value, b.cons_quantity,b.cons_amount from product_details_master a, inv_transaction b where a.id=b.prod_id and a.id=$before_prod_id and b.id=$update_id and a.item_category_id=4 and b.item_category=4 and b.transaction_type=3" );
		...
		//current product ID
		$sql_current = sql_select( "select a.id,a.current_stock, a.stock_value from product_details_master a where a.id=$txt_prod_id and a.item_category_id=4" );
		...
		$update_array_prod= "last_issued_qnty*current_stock*stock_value*updated_by*update_date";
		if($before_prod_id==$txt_prod_id)
		{
			$adj_stock_qnty = ($current_stock_qnty+$before_issue_qnty)-$consump_qnty; // CurrentStock + Before Issue Qnty - Current Issue Qnty
			$adj_stock_value = ($current_stock_value+$before_issue_value)-$consump_amount;
			 
			$data_array_prod= $consump_qnty."*".$adj_stock_qnty."*".$adj_stock_value."*'".$user_id."'*'".$pc_date_time."'";
		}
		else
		{
			...
			$update_dataProd[$before_prod_id]=explode("*",("".$before_issue_qnty."*".$adj_before_stock_qnty."*".$adj_before_stock_value."*'".$user_id."'*'".$pc_date_time."'"));
			
			//current product adjust
			$adj_curr_stock_qnty = 	$current_stock_qnty-$consump_qnty;
			$adj_curr_stock_value = $current_stock_value-$consump_amount;
			
			$updateIdprod_array[]=$txt_prod_id;
			$update_dataProd[$txt_prod_id]=explode("*",("".$consump_qnty."*".$adj_curr_stock_qnty."*".$adj_curr_stock_value."*'".$user_id."'*'".$pc_date_time."'"));
		...
		$field_array_mst="company_id*store_id*issue_date*supplier_id*received_id*pi_id*received_mrr_no*updated_by*update_date";
		$data_array_mst=$cbo_company_name."*".$cbo_store_name."*".$txt_return_date."*".$cbo_return_to."*".$txt_received_id."*".$pi_id."*".$txt_mrr_no."*'".$user_id."'*'".$pc_date_time."'";
		
 		$field_array_trans="company_id*prod_id*store_id*item_category*transaction_type*transaction_date*order_uom*order_qnty*order_rate*order_amount*cons_uom*cons_quantity*cons_rate*cons_amount*remarks*updated_by*update_date";
 		$data_array_trans= "".$cbo_company_name."*".$txt_prod_id."*".$cbo_store_name."*4*3*".$txt_return_date."*".$cbo_uom."*".$txt_return_qnty."*".$txt_rate."*".$txt_amount."*".$txt_cons_uom."*".$consump_qnty."*".$txt_cons_rate."*".$consump_amount."*".$txt_remarks."*'".$user_id."'*'".$pc_date_time."'"; 
		...
		if($before_prod_id==$txt_prod_id)
		{
			...
			{
				$sql_receive = sql_select("select a.id,a.balance_qnty,balance_amount from inv_transaction a where a.id=$hidden_receive_trans_id and a.transaction_type =1"); 
				$adjBalance = (($sql_receive[0][csf("balance_qnty")]+$before_issue_qnty)-$consump_qnty);
				$adjBalanceAmt = (($sql_receive[0][csf("balance_amount")]+$before_issue_value)-$consump_amount);
				$update_data_trans="".$adjBalance."*".$adjBalanceAmt."*'".$user_id."'*'".$pc_date_time."'";
			}
		}
		else
		{
			...
			{
				$sql_receive_before = sql_select("select a.id,a.balance_qnty,balance_amount from inv_transaction a where a.id=$before_receive_trans_id and a.transaction_type =1");  
				$adjBalance = ($sql_receive_before[0][csf("balance_qnty")]+$before_issue_qnty);
				$adjBalanceAmt = ($sql_receive_before[0][csf("balance_amount")]+$before_issue_value);
				$update_data_before_trans="".$adjBalance."*".$adjBalanceAmt."*'".$user_id."'*'".$pc_date_time."'";
			}
			
			if($hidden_receive_trans_id>0)
			{
				$sql_receive = sql_select("select a.id,a.balance_qnty,balance_amount from inv_transaction a where a.id=$hidden_receive_trans_id and a.transaction_type =1"); 
				$adjBalance_af = $sql_receive[0][csf("balance_qnty")]-$consump_qnty;
				$adjBalanceAmt_af = $sql_receive[0][csf("balance_amount")]-$consump_amount;
				$update_data_trans="".$adjBalance_af."*".$adjBalanceAmt_af."*'".$user_id."'*'".$pc_date_time."'";
			}
		...
		$data_array_mrr = "(".$mrrWiseIsID.",".$hidden_receive_trans_id.",".$update_id.",49,".$txt_prod_id.",".$consump_qnty.",".$txt_cons_rate.",".$consump_amount.",'".$user_id."','".$pc_date_time."')";
		...
		if(!empty($txt_break_qnty))
		{
			foreach($ordr_wise_rtn_qnty_arr as $val)
			{
				...
				{
					if($data_array_proportion!="") $data_array_proportion.=", ";
					$data_array_proportion.="(".$proportion_id.",".$update_id.",3,49,".$order_qnty_arr[0].",".$txt_prod_id.",".$order_qnty_arr[1].",'".$user_id."','".$pc_date_time."')";
					$proportion_id++;
				}
			}
			...
			if($variable_setting_production==1)
			{
				...
				foreach($ordr_wise_rtn_roll_arr as $val)
				{
					...
					{
						if($data_array_roll!="") $data_array_roll.=", ";
						$data_array_roll.="(".$roll_id.",".$id.",".$update_id.",".$order_roll_arr[0].",49,".$order_roll_arr[1].",".$order_roll_arr[2].",'".$user_id."','".$pc_date_time."')";
						$roll_id++;
					}
				}
			}
		...
		if($before_prod_id==$txt_prod_id)
		{
			...
			if($hidden_receive_trans_id>0)
			{
				$query3 = execute_query("DELETE FROM inv_mrr_wise_issue_details WHERE issue_trans_id=$update_id and entry_form=49");
			...
			if($data_array_proportion!="")
			{
				$query4 = execute_query("DELETE FROM order_wise_pro_details WHERE trans_id=$update_id and entry_form=49");
				$propoId=sql_insert("order_wise_pro_details",$field_array_proportion,$data_array_proportion,1);
				if($variable_setting_production==1)
				{
					$rollId=sql_insert("pro_roll_details",$field_array_roll,$data_array_roll,1);
					$query5 = execute_query("DELETE FROM pro_roll_details WHERE dtls_id=$update_id and entry_form=49");
				}
			}
			//mrr wise issue data insert here----------------------------//
	...
	<table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
            ...
            <tbody>
                <tr>                    
                    ...
                    <td align="center">
                        <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $company; ?>, 'create_return_search_list_view', 'search_div', 'trims_receive_rtn_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />				
                    </td>
            </tr>
        	<tr>                  
            	<td align="center" height="40" valign="middle" colspan="5">
...
if($action=="create_return_search_list_view")
{
	...
	$sql = "select a.id, $year_field a.issue_number_prefix_num, a.issue_number, a.company_id, a.supplier_id,a.issue_date, a.item_category, a.received_id,a.received_mrr_no, sum(b.cons_quantity)as cons_quantity
			...
			where a.id=b.mst_id and b.transaction_type=3 and a.status_active=1 and a.item_category=4 and b.item_category=4 and a.entry_form=49 $sql_cond $company_credential_cond $store_location_credential_cond $supplier_credential_cond group by a.id, a.issue_number_prefix_num, a.issue_number, a.company_id, a.supplier_id, a.issue_date, a.item_category, a.received_id, a.received_mrr_no, a.insert_date order by a.id";
	//echo $sql;
	$company_arr = return_library_array("select id,company_name from lib_company","id","company_name");
	$supplier_arr=return_library_array( "select id, supplier_name from lib_supplier",'id','supplier_name');
	$arr=array(2=>$company_arr);
 	echo create_list_view("list_view", "Return No, Year, Company Name, Return Date,Return Qty,Receive MRR","70,60,150,80,100,150","700","230",0, $sql , "js_set_value", "id,issue_number,received_id", "", 1, "0,0,company_id,0,0,0", $arr, "issue_number_prefix_num,year,company_id,issue_date,cons_quantity,received_mrr_no","","",'0,0,0,3,1,0') ;	
 	exit();
}
...
{  
	...
	foreach($res as $row)
	{
		...
		//right side list view
		//echo "show_list_view('".$row[csf("received_id")]."','show_product_listview','list_product_container','requires/trims_receive_rtn_controller','');\n";
   	}	
	exit();	
}
...
if($action=="show_dtls_list_view")
{
	...
	$sql = "select a.id as issue_id, a.issue_number, a.company_id, a.supplier_id, a.issue_date, a.item_category, a.received_id, a.received_mrr_no, b.id as trans_id, b.cons_quantity, b.cons_uom, b.cons_rate, b.cons_amount, c.product_name_details, c.id as prod_id   
			...
			where a.id=b.mst_id and b.item_category=4 and b.transaction_type=3 and a.id=$data";
	//echo $sql;
	$result = sql_select($sql);
	$i=1;
	$rettotalQnty=0;
	...
	$totalAmount=0;
	?> 
     	<table class="rpt_table" border="1" cellpadding="2" cellspacing="0" style="width:800px" rules="all">
        	<thead>
            	<tr>
                	<th>SL</th>
                    <th>Return No</th>
            ...
            <tbody>
				...
				{					
					...
					?>
					<tr bgcolor="<? echo $bgcolor; ?>" onClick='get_php_form_data("<? echo $row[csf("trans_id")];?>,<? echo $rcvQnty;?>,<? echo $row[csf("issue_id")];?>","child_form_input_data","requires/trims_receive_rtn_controller")' style="cursor:pointer" >
                        ...
                        <td width="70" align="right"><p><? echo number_format($row[csf("cons_quantity")],0); ?></p></td>
                        <td width="100" align="right"><p><? echo number_format($row[csf("cons_amount")],2); ?></p></td>
					</tr>
					<? 
					$i++; 
				} 
				?>
                	<tfoot>
                        ...
                        <th><? echo number_format($rettotalQnty,0); ?></th> 
...
if($action=="child_form_input_data")
{
	...
	$issue_id=str_replace("'","",$ex_data[2]); 
 	...
 	$sql = "select b.id as prod_id, b.product_name_details, b.item_group_id, b.item_color, b.item_size, b.brand_supplier, b.current_stock, b.gsm, b.dia_width, a.id as tr_id, a.cons_uom, a.cons_quantity, a.cons_rate, a.cons_amount, a.order_uom,a.order_qnty,a.order_rate,a.order_amount,a.remarks
			from inv_transaction a, product_details_master b
 			where a.id=$data and a.status_active=1 and a.item_category=4 and transaction_type=3 and a.prod_id=b.id and b.status_active=1";
 	//echo $sql;die;
	$result = sql_select($sql);
	foreach($result as $row)
	{
		...
		echo "$('#prev_return_qnty').val('".$row[csf("order_qnty")]."');\n";	
		echo "$('#cbo_uom').val(".$row[csf("order_uom")].");\n";
		echo "$('#txt_cons_uom').val(".$row[csf("cons_uom")].");\n";
		echo "$('#txt_remarks').val('".$row[csf("remarks")]."');\n";
		
		
		//echo "select recv_trans_id from inv_mrr_wise_issue_details where issue_trans_id='".$row[csf('tr_id')]."'";
		...
		echo "$('#txt_return_qnty').val('".$row[csf("order_qnty")]."');\n";	
            ...
            <tbody>
                <tr class="general">
                    ...
                    <td align="center">
                     ...
                     <td align="center">
                        <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view (document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $company; ?>, 'create_wopi_search_list_view', 'search_div', 'trims_receive_rtn_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />				
                    </td>
            </tr>
        	<tr>                  
...
if ($action=="trims_receive_return_print")
{
	...
	$sql=" select id, issue_number, received_id, issue_date, supplier_id from  inv_issue_master where id='$data[1]' and entry_form=49 and item_category=4 and status_active=1 and is_deleted=0";
    ...
    <table width="880" cellspacing="0" align="right">
        ...
        <tr>
            ...
            <td width="100"><strong>Return To :</strong></td> <td width="175px"><? echo $supplier_library[$dataArray[0][csf('supplier_id')]]; ?></td>
        </tr>
        <tr>
