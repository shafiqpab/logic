...
function openmypage_systemId()
{
	...
	var page_link = 'requires/trims_transfer_v3_controller.php?cbo_company_id='+cbo_company_id+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&action=orderToorderTransfer_popup';
	...
	emailwindow.onclose=function()
	{
        ...
        load_drop_down( 'requires/trims_transfer_v3_controller', '', 'load_drop_down_to_company', 'cbo_company_id_to' );
        get_php_form_data(transfer_id, "populate_data_from_transfer_master", "requires/trims_transfer_v3_controller" );
		...
		show_list_view(order_id+"__"+cbo_store_name+"__"+cbo_floor+"__"+cbo_room+"__"+txt_rack+"__"+txt_shelf+"__"+cbo_bin+"__"+store_update_upto+"__"+mst_id,'show_dtls_list_view_update','body_dtls_part','requires/trims_transfer_v3_controller','setFilterGrid(\'tbl_list_search\',-1);');
...
function openmypage_orderNo(type)
{
	...
	if(type=='from')
	{
		...
		{
			if (form_validation('cbo_company_id*cbo_store_name*cbo_floor*cbo_room*txt_rack*txt_shelf*cbo_bin','Company*From Store*Floor*Room*Rack*Shelf*Bin')==false)
			{
				return;
			}
		}
		...
		{
			if (form_validation('cbo_company_id*cbo_store_name*cbo_floor*cbo_room*txt_rack*txt_shelf','Company*From Store*Floor*Room*Rack*Shelf')==false)
			{
				return;
			}
		}
		...
		{
			if (form_validation('cbo_company_id*cbo_store_name*cbo_floor*cbo_room*txt_rack','Company*From Store*Floor*Room*Rack')==false)
			{
				return;
			}
		}
		...
		{
			if (form_validation('cbo_company_id*cbo_store_name*cbo_floor*cbo_room','Company*From Store*Floor*Room')==false)
			{
				return;
			}
		}
		...
		{
			if (form_validation('cbo_company_id*cbo_store_name*cbo_floor','Company*From Store*Floor')==false)
			{
				return;
			}
		}
	...
	else
	{
		/*if(store_update_upto==6)
		{
			if (form_validation('cbo_company_id_to*cbo_store_name_to*cbo_floor_to*cbo_room_to*txt_rack_to*txt_shelf_to*cbo_bin_to','To Company To*To Store *Floor To*Room To*Rack To*Shelf To*Bin To')==false)
			{
				return;
			}
		}
		...
		{
			if (form_validation('cbo_company_id_to*cbo_store_name_to*cbo_floor_to*cbo_room_to*txt_rack_to*txt_shelf_to','To Company*To Store*To Floor*To Room*To Rack*To Shelf')==false)
			{
				return;
			}
		}
		...
		{
			if (form_validation('cbo_company_id_to*cbo_store_name_to*cbo_floor_to*cbo_room_to*txt_rack_to','To Company To*To Store*To Floor*To Room*To Rack')==false)
			{
				return;
			}
		}
		...
		{
			if (form_validation('cbo_company_id_to*cbo_store_name_to*cbo_floor_to*cbo_room_to','To Company*To Store*To Floor*To Room')==false)
			{
				return;
			}
		}
		...
		{
			if (form_validation('cbo_company_id_to*cbo_store_name_to*cbo_floor_to','To Company*To Store*To Floor')==false)
			{
				return;
			}
		}
		...
		{
			if (form_validation('cbo_company_id_to*cbo_store_name_to','To Company*To Store')==false)
			{
				return;
			}
		}*/
	...
	var page_link = 'requires/trims_transfer_v3_controller.php?cbo_company_id='+cbo_company_id+'&cbo_store_name='+cbo_store_name+'&type='+type+'&cbo_company_id_to='+cbo_company_id_to+'&cbo_floor='+cbo_floor+'&cbo_room='+cbo_room+'&txt_rack='+txt_rack+'&txt_shelf='+txt_shelf+'&cbo_bin='+cbo_bin+'&store_update_upto='+store_update_upto+'&action=order_popup';
	  
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1300px,height=380px,center=1,resize=1,scrolling=0','../');
	
	emailwindow.onclose=function()
	{
		var theform=this.contentDoc.forms[0];//("search_order_frm"); //Access the form inside the modal window
		var order_ref_arr=this.contentDoc.getElementById("order_id").value.split("_"); //Access form field with id="emailfield"
		var order_id=order_ref_arr[0];
		var floor_id=order_ref_arr[1];
		var room_id=order_ref_arr[2];
		var rack_id=order_ref_arr[3];
		var self_id=order_ref_arr[4];
		var bin_id=order_ref_arr[5];
		var transfer_criteria = $("#cbo_transfer_criteria").val();
		get_php_form_data(order_id+"**"+type+"**"+transfer_criteria, "populate_data_from_order", "requires/trims_transfer_v3_controller" );
		if(type=='from')
		{
			//show_list_view(order_id+"__"+cbo_store_name+"__"+cbo_floor+"__"+cbo_room+"__"+txt_rack+"__"+txt_shelf+"__"+cbo_bin+"__"+store_update_upto,'show_dtls_list_view','body_dtls_part','requires/trims_transfer_v3_controller','setFilterGrid(\'tbl_list_search\',-1);');
			show_list_view(order_id+"__"+cbo_store_name+"__"+floor_id+"__"+room_id+"__"+rack_id+"__"+self_id+"__"+bin_id+"__"+store_update_upto+"__"+1,'show_dtls_list_view','body_dtls_part','requires/trims_transfer_v3_controller','setFilterGrid(\'tbl_list_search\',-1);');
			$("#cbo_store_name").attr("disabled",true);
			$('#cbo_floor').val(floor_id);
			$('#cbo_room').val(room_id);
			$('#txt_rack').val(rack_id);
			$('#txt_shelf').val(self_id);
			$('#cbo_bin').val(bin_id);
...
function fnc_trims_transfer_entry(operation)
{
	if(operation==4)
	{
		...
		print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+$('#cbo_company_id_to').val(), "trims_store_order_to_order_transfer_print", "requires/trims_transfer_v3_controller" ) 
		return;
	}
	else if(operation==5)
	{
		...
		var report_title=$( "div.form_caption" ).html();
		print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+$('#cbo_company_id_to').val(), "trims_store_order_to_order_transfer_print2", "requires/trims_transfer_v3_controller");
		return;
	}
	else if(operation==0 || operation==1 || operation==2)
	{
		...
		if( form_validation('cbo_company_id*cbo_company_id_to*txt_transfer_date*cbo_store_name*cbo_store_name_to*txt_from_order_no*txt_to_order_no','Company*To Company*Transfer Date*From Store*To Store*From Order No*To Order No')==false )
		{
			return;
		}
		var transfer_criteria=$('#cbo_transfer_criteria').val();
		...
		http.open("POST","requires/trims_transfer_v3_controller.php",true);
...
{	
	...
	{	  		
		...
		if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
		{
			...
			//reset_form('','','update_dtls_id*cbo_item_desc*txt_transfer_qnty*update_trans_issue_id*update_trans_recv_id','','','');
			show_list_view(order_id+"__"+cbo_store_name+"__"+cbo_floor+"__"+cbo_room+"__"+txt_rack+"__"+txt_shelf+"__"+cbo_bin+"__"+store_update_upto+"__"+reponse[1],'show_dtls_list_view_update','body_dtls_part','requires/trims_transfer_v3_controller','setFilterGrid(\'tbl_list_search\',-1);');
...
function company_on_change(fromComp)
{
	...
	if (transfer_criteria!=1) {
		...
		load_drop_down( 'requires/trims_transfer_v3_controller',fromComp, 'load_drop_down_location_to', 'to_location_td' );
	}
	
    if(transfer_criteria == 1){
        load_drop_down( 'requires/trims_transfer_v3_controller',fromComp, 'load_drop_down_to_company_not_selected', 'cbo_company_id_to' );
    }
    ...
    xmlhttp.open("POST", "requires/trims_transfer_v3_controller.php", true);
...
function to_company_on_change(to_company)
{
    ...
    //     if($transfer_criteria == 1){
    //         load_drop_down( 'requires/trims_transfer_v3_controller',fromComp, 'load_drop_down_from_company_not_selected', 'cbo_company_id' );
    //     }
    // }
                ...
                <table width="1240" cellspacing="2" cellpadding="2" border="0" id="tbl_master">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            //load_drop_down( 'requires/trims_transfer_v3_controller', this.value, 'load_drop_down_store_to', 'store_td' )
                        	?>
                        </td>
                            ...
                            <legend>From Order</legend>
                                <table id="from_order_info"  cellpadding="0" cellspacing="1" width="100%">										
                                     <tr>
										...
										<td id="floor_td">
											<? echo create_drop_down( "cbo_floor", 152,"select a.floor_room_rack_id, a.floor_room_rack_name from lib_floor_room_rack_mst a","floor_room_rack_id,floor_room_rack_name", 1, "--Select--", 0, "",0 ); ?>
										</td>
                           			 ...
                           			 <tr>
                                     	<td>Room</td>
                                     	<td id="room_td">
											<? echo create_drop_down( "cbo_room", 152,"select a.floor_room_rack_id, a.floor_room_rack_name from lib_floor_room_rack_mst a","floor_room_rack_id,floor_room_rack_name", 1, "--Select--", 0, "",0 ); ?>
										</td>
                                        <td>Rack</td>
										<td id="rack_td">
											<? echo create_drop_down( "txt_rack", 152,"select a.floor_room_rack_id, a.floor_room_rack_name from lib_floor_room_rack_mst a","floor_room_rack_id,floor_room_rack_name", 1, "--Select--", 0, "",0 ); ?>
										</td>
                                    </tr>
                                    <tr>
                                        <td>Shelf</td>
										<td id="shelf_td">
											<? echo create_drop_down( "txt_shelf", 152,"select a.floor_room_rack_id, a.floor_room_rack_name from lib_floor_room_rack_mst a","floor_room_rack_id,floor_room_rack_name", 1, "--Select--", 0, "",0 ); ?>
										</td>
                                        <td>Bin</td>
										<td id="bin_td">
											<? echo create_drop_down( "cbo_bin", 152,"select a.floor_room_rack_id, a.floor_room_rack_name from lib_floor_room_rack_mst a","floor_room_rack_id,floor_room_rack_name", 1, "--Select--", 0, "",0 ); ?>
										</td>
                           			 </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>Style Ref.</td>
                                        <td>
                                            <input type="text" name="txt_from_style_ref" id="txt_from_style_ref" class="text_boxes" style="width:140px;" disabled="disabled" placeholder="Display" />
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>Job No</td>						
                                ...
                                <table id="tbl_item_info" class="rpt_table" border="1" rules="all" cellpadding="0" cellspacing="2" width="100%">
                                    ...
                                    <tbody id="body_dtls_part">
                                    	<tr>
                                            ...
                                            <td id="tdUom_1"></td>
                                            <td id="tdCurrentStock_1" align="center"><input type="text" name="txtCurrentStock[]" id="txtCurrentStock_1" class="text_boxes" style="width:80px" disabled="disabled"/></td>
                                            <td id="tdTransferQnty_1" align="center">
                                            <input type="text" name="txtTransferQnty[]" id="txtTransferQnty_1" class="text_boxes_numeric" style="width:80px;" />
                                            <input type="hidden" name="txtRate[]" id="txtRate_1"/>
                                            </td>
                                            <td id="tdRemarks_1" align="center">
                                            <input type="text" name="txtRemarks[]" id="txtRemarks_1" class="text_boxes" style="width:90px;" />
