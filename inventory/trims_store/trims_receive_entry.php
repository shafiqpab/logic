...
$independent_control_arr = return_library_array( "select company_name, independent_controll from variable_settings_inventory where variable_list=20 and menu_page_id=24 and status_active=1 and is_deleted=0",'company_name','independent_controll');
...
<script>
	...
	<? 
	if($_SESSION['logic_erp']['mandatory_field'][24]!="")
	{
		$mandatory_field_arr= json_encode( $_SESSION['logic_erp']['mandatory_field'][24] );
	...
	function openmypage_wo_pi_popup()
	{
		...
		{ 
			...
			emailwindow.onclose=function()
			{
				...
				if(data[6]!="")
				{
					load_drop_down( 'requires/trims_receive_entry_controller',cbo_company_id+'**'+data[6]+'**'+data[0], 'load_drop_down_supplier', 'supplier_td_id' );
				...
				show_list_view(booking_no_pi_id+"__"+recieve_basis+"__"+booking_without_order+"__"+variable_string_inventory+"__"+theemail,'show_fabric_desc_listview','list_fabric_desc_container','requires/trims_receive_entry_controller','setFilterGrid(\'tbl_list_search\',-1);');
	...
	function set_form_data(data)
	{
		...
		$('#hid_job_no').val(data[14]);
		$('#hid_ir_no').val(data[15]);
		...
		var item_size=encodeURIComponent(trim(data[10]));
		if(recieve_basis==1 || recieve_basis==2)
		{
			//alert(data[10]); return;
			//get_php_form_data(recieve_basis+'_'+booking_no_pi_id+'_'+booking_no_pi_no+'_'+data[0]+'_'+data[1]+'_'+data[2]+'_'+data[3]+'_'+data[4]+'_'+data[6]+'_'+data[8]+'_'+data[10]+'_'+booking_without_order+'_'+data[11], 'put_balance_qnty', 'requires/trims_receive_entry_controller' );
			get_php_form_data(recieve_basis+'__'+booking_no_pi_id+'__'+booking_no_pi_no+'__'+data[0]+'__'+item_description+'__'+brand_sup_ref+'__'+data[3]+'__'+data[4]+'__'+data[6]+'__'+data[8]+'__'+item_size+'__'+booking_without_order+'__'+data[11]+'____'+data[13], 'put_balance_qnty', 'requires/trims_receive_entry_controller' );
		}
	...
	function set_rate_balance(data)
	{
		...
		var item_size = encodeURIComponent(trim(data[7]));
		get_php_form_data(recieve_basis+'__'+booking_no_pi_id+'__'+booking_no_pi_no+'__'+data[0]+'__'+txt_item_description+'__'+txt_brand_supref+'__'+data[3]+'__'+data[4]+'__'+data[5]+'__'+data[6]+'__'+item_size+'__'+data[8]+'__'+data[9]+'__'+data[11]+'__'+data[10], 'put_balance_qnty', 'requires/trims_receive_entry_controller' );
	}
	...
	function openmypage_po()
	{
		...
		var item_size=encodeURIComponent(trim($('#txt_item_size').val()));
		...
		var hid_job_no=$('#hid_job_no').val();
		var hid_ir_no=$('#hid_ir_no').val();
	...
	function fnc_trims_receive(operation)
	{
		...
		if(operation==4)
		{
			 ...
			 print_report( $('#cbo_company_id').val()+'__'+$('#update_id').val()+'__'+report_title+'__'+variable_string_inventory, "trims_receive_entry_print", "requires/trims_receive_entry_controller" ) 
			 return;
		}
		else if(operation==5)
		{
			 var report_title=$( "div.form_caption" ).html();
			 print_report( $('#cbo_company_id').val()+'__'+$('#update_id').val()+'__'+report_title+'__'+variable_string_inventory, "trims_receive_entry_print_2", "requires/trims_receive_entry_controller" ) 
			 return;
		}
		else if(operation==6)
		{
			 var report_title=$( "div.form_caption" ).html();
			 print_report( $('#cbo_company_id').val()+'__'+$('#update_id').val()+'__'+report_title+'__'+variable_string_inventory, "trims_receive_entry_print_4", "requires/trims_receive_entry_controller" ) 
			 return;
		}
		else if(operation==7)
		{
			 var report_title=$( "div.form_caption" ).html();
			 print_report( $('#cbo_company_id').val()+'__'+$('#update_id').val()+'__'+report_title+'__'+variable_string_inventory, "trims_receive_entry_print_5", "requires/trims_receive_entry_controller" ) 
			 return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			<?php 
				...
				if(!empty($_SESSION['logic_erp']['mandatory_field'][24][4])) echo " isFileMandatory = ". $_SESSION['logic_erp']['mandatory_field'][24][4] . ";\n";
			?>
			// alert(isFileMandatory); return;
			if($("#multiple_file_field")[0].files.length==0 && isFileMandatory!="" && $('#txt_recieved_id').val()==''){
			...
			if('<?php echo implode('*',$_SESSION['logic_erp']['mandatory_field'][24]);?>'){
				if (form_validation('<?php echo implode('*',$_SESSION['logic_erp']['mandatory_field'][24]);?>','<?php echo implode('*',$_SESSION['logic_erp']['field_message'][24]);?>')==false)
				{
			...
			var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_recieved_id*cbo_company_id*cbo_receive_basis*txt_receive_date*txt_booking_pi_no*txt_booking_pi_id*booking_without_order*txt_receive_chal_no*txt_challan_date*lc_id*cbo_source*cbo_store_name*cbo_supplier_name*txt_boe_mushak_challan_no*txt_boe_mushak_challan_date*cbo_currency_id*txt_exchange_rate*cbo_item_group*cbo_uom*txt_item_description*txt_receive_qnty*txt_reject_recv_qnty*txt_brand_supref*txt_rate*txt_hidden_rate*txt_book_currency*txt_buyer_order*txt_ile*txt_amount*txt_hidden_amount*update_id*all_po_id*update_dtls_id*update_trans_id*save_data*previous_prod_id*hidden_sensitivity*txt_gmts_color_id*txt_gmts_color*txt_item_color_id*txt_item_color*txt_gmts_size_id*txt_gmts_size*txt_item_size*cbo_payment_over_recv*cbo_floor*cbo_room*txt_rack*txt_shelf*cbo_bin*txt_remarks*suplier_type*hidden_item_description*meterial_source*wo_order_qty*wo_order_id*store_update_upto*variable_string_inventory',"../../");
	...
	function fnc_trims_receive_Reply_info()
	{
		if(http.readyState == 4) 
		{
			...
			{	
				...
				{
					var check_system_id=$("#txt_recieved_id").val();
					document.getElementById('update_id').value = reponse[1];
					document.getElementById('txt_recieved_id').value = reponse[2];
					if (check_system_id=="") uploadFile( $("#txt_recieved_id").val());
					$('#cbo_company_id').attr('disabled','disabled');
					show_list_view(reponse[1]+"__"+variable_string_inventory,'show_trims_listview','list_container_trims','requires/trims_receive_entry_controller','');
					
					reset_form('trimsreceive_1','','','','','update_id*txt_recieved_id*cbo_company_id*cbo_receive_basis*txt_receive_date*txt_wo_pi_basis_id*txt_receive_chal_no*txt_challan_date*txt_booking_pi_no*txt_booking_pi_id*cbo_supplier_name*cbo_store_name*cbo_currency_id*txt_exchange_rate*lc_id*txt_lc_no*cbo_source*booking_without_order*meterial_source*txt_boe_mushak_challan_no*txt_boe_mushak_challan_date*suplier_type*store_update_upto*variable_string_inventory*is_rate_optional*variable_lot*is_posted_account*store_update_upto');
					$('#ile_td').html('ILE%');
					$('#cbo_payment_over_recv').val(0);
					set_button_status(0, permission, 'fnc_trims_receive',1,1);	
				}
	...
	function uploadFile(mst_id)
	{
		$(document).ready(function() { 
			...
			for( var i = 0 ; i < $("#multiple_file_field")[0].files.length ; i++)
			{
				...
				$.ajax({
					url: 'requires/trims_receive_entry_controller.php?action=file_upload&mst_id='+ mst_id, 
					type: 'post', 
					data:fd, 
					contentType: false, 
					processData: false, 
	...
	function trims_receive_popup()
	{
		...
		{ 	
			...
			emailwindow.onclose=function()
			{
				...
				var comId= $("#cbo_company_id").val();
				if(trims_recv_id!="")
				{
					freeze_window(5);
					reset_form('trimsreceive_1','list_container_trims*list_fabric_desc_container','','','cbo_company_id*store_update_upto*variable_string_inventory*is_rate_optional*variable_lot*is_posted_account*store_update_upto','');
					...
					if(recieve_basis==1 || recieve_basis==2)
					{
						show_list_view(booking_no_pi_id+"__"+recieve_basis+"__"+booking_without_order+"__"+variable_string_inventory+"__"+booking_pi_id,'show_fabric_desc_listview','list_fabric_desc_container','requires/trims_receive_entry_controller','setFilterGrid(\'tbl_list_search\',-1);');
					}
					load_drop_down( 'requires/trims_receive_entry_controller',comId+'*'+hidden_supplier_id, 'load_drop_down_supplier_new', 'supplier_td_id' );
					show_list_view(trims_recv_id+"__"+variable_string_inventory,'show_trims_listview','list_container_trims','requires/trims_receive_entry_controller','');
					set_button_status(0, permission, 'fnc_trims_receive',1,1);	
					release_freezing();
				}
			}
	...
	{
		var currency_id= $("#cbo_currency_id").val();
		get_php_form_data(currency_id, "get_library_exchange_rate", "requires/trims_receive_entry_controller" );
	...
	/*function company_on_change(company)
	{
		...
		$.ajax({
			url: 'requires/trims_receive_entry_controller.php',
			type: 'POST',
			data: page_link,
			success: function (response)
			{
	...
	function company_on_change(company)
    {
	    ...
	    xmlhttp.open("POST", "requires/trims_receive_entry_controller.php", true);
	...
	function independence_basis_controll_function(data)
	{
		...
		var varible_string=return_global_ajax_value( data, 'varible_inventory', '', 'requires/trims_receive_entry_controller');
            ...
            <fieldset style="width:650px">
	            ...
	            <table cellpadding="0" cellspacing="2" width="100%">
	                ...
	                <tr>
	                    ...
	                    <td>
							<? 
								echo create_drop_down( "cbo_company_id", 142, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond  $company_credential_cond order by comp.company_name","id,company_name", 1, "--Select Company--", 0, "load_drop_down( 'requires/trims_receive_entry_controller',this.value, 'load_drop_down_supplier', 'supplier_td_id' );set_receive_basis();independence_basis_controll_function(this.value);load_room_rack_self_bin('requires/trims_receive_entry_controller*4', 'store','store_td', this.value);company_on_change(this.value);get_receive_basis(this.value);get_php_form_data(this.value,'print_button_variable_setting','requires/trims_receive_entry_controller' );" );
									//load_drop_down( 'requires/trims_receive_entry_controller',this.value, 'load_drop_down_store', 'store_td' );
	                        ?>
	                ...
	                <tr>
	                    ...
	                    <td>
							<?
								echo create_drop_down( "cbo_source", 122, $source,"", 1, "-- Select --", 0, "get_php_form_data(document.getElementById('cbo_item_group').value+'_'+document.getElementById('cbo_company_id').value+'_'+this.value+'_'+document.getElementById('txt_rate').value, 'show_ile_load_uom', 'requires/trims_receive_entry_controller' );",1 );
							?>
	                    </td>
	                ...
	                <tr>
						...
						<td> 
							<input type="text" name="txt_boe_mushak_challan_no" id="txt_boe_mushak_challan_no" class="text_boxes" style="width:110px">
						</td>
					
					
						<td>BOE/Mushak Challan Date</td>                                              
						<td> 
							<input type="text" name="txt_boe_mushak_challan_date" id="txt_boe_mushak_challan_date" class="datepicker" style="width:110px" readonly>
						</td>
	                
	                    <td>file</td>
	                    <td>
						  	<input type="file" class="image_uploader" id="multiple_file_field" name="multiple_file_field" multiple style="width:120px">

							<input type="button" class="image_uploader" style="width:120px" value="CLICK TO ADD FILE" onClick="file_uploader ( '../../', document.getElementById('txt_recieved_id').value,'', 'trims_receive_entry', 2 ,1)"> 
						</td>
	                </tr>
	            </table>
				</fieldset>
	            ...
	            <fieldset style="width:650px">
	                ...
	                <table width="180" cellspacing="2" cellpadding="0" border="0" style="float:left">
	                	<tr>
                ...
                <table width="650" cellspacing="2" cellpadding="0" border="0">
	               <tr>
                    	<td colspan="6" style="padding-left:60px;">Remarks: <input class="text_boxes"  name="txt_remarks" id="txt_remarks" type="text" style="width:487px;"  /></td>
                    </tr>
                    ...
                    <tr>
                        <td colspan="6" align="center" class="button_container">
                            ...
                            <? //trimsreceive_1
                                echo load_submit_buttons($permission, "fnc_trims_receive", 0,0,"reset_form('trimsreceive_1','list_container_trims*list_fabric_desc_container','','','disable_enable_fields(\'cbo_company_id\');set_receive_basis();','cbo_company_id*cbo_receive_basis*txt_receive_date*txt_challan_date*cbo_store_name*store_update_upto*variable_string_inventory*is_rate_optional*variable_lot*is_posted_account*store_update_upto')",1);
                            ?>                            
                            
                            <!-- <input id="Print1" type="button" class="formbutton" style="width:80px" onClick="fnc_trims_receive(5)" name="print" value="Print 2">
                            <input id="Print2" type="button" class="formbutton" style="width:80px" onClick="fnc_trims_receive(6)" name="print" value="Print 3"> -->
                            ...
                            <input type="hidden" name="hid_job_no" id="hid_job_no">
                            <input type="hidden" name="hid_ir_no" id="hid_ir_no">
                        </td>	  
                    </tr>
                    <tr>
...
<script>$('#cbo_payment_over_recv').val(0);$('#cbo_store_name').val(0);</script>
<script>
	$(document).ready(function() { 
