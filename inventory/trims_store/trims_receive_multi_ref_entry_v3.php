...
<script>
	...
	<? 
		if($_SESSION['logic_erp']['data_arr'][631]!="")
		{
			$data_arr= json_encode( $_SESSION['logic_erp']['data_arr'][631] );
	...
	function set_receive_basis(i)
	{
		...
		$('#txt_booking_pi_no').val('').attr('disabled',false);
	...
	function openmypage_wo_pi_popup()
	{
		...
		{ 	
			...
			emailwindow.onclose=function()
			{
				...
				var exchange_rate = $('#txt_exchange_rate').val()*1;
				//alert(exchange_rate);
				show_list_view_post(booking_no_pi_id+"**"+rcv_basis+"**"+booking_without_order+"**"+cbo_company_id+"**"+data[2]+"**"+exchange_rate+"**"+hid_booking_type+"**"+theemail, 'show_fabric_desc_listview', 'list_fabric_desc_container', 'requires/trims_receive_multi_ref_entry_v3_controller', '' ) ;
				...
				{
					col_operation: {
						id: ["value_tot_rcv_qnty"],
						col: [20],
						operation: ["sum"],
						write_method: ["setvalue"]
					}
				}
				setFilterGrid("tbl_list_fabric_desc_container",-1,tableFilters);
				calculate(1);
				release_freezing();
			}
		}
	...
	function load_details_data(booking_pi_id,booking_pi_no,booking_without_order,mst_id,material_source)
	{
		...
		//show_list_view_post(booking_no_pi_id+"**"+recieve_basis+"**"+booking_without_order+"**"+cbo_company_id+"**"+cbo_source+"**"+exchange_rate+"**"+mst_id+"**"+booking_pi_id+"**"+cbo_store_name+"**"+booking_pi_id, 'show_fabric_desc_listview_update', 'list_fabric_desc_container', 'requires/trims_receive_multi_ref_entry_v3_controller', 'setFilterGrid("list_fabric_desc_container",-1)' ) ;
		//calculate(1);
		$.ajax({
			url: "requires/trims_receive_multi_ref_entry_v3_controller.php?data="+ booking_no_pi_id+"**"+recieve_basis+"**"+booking_without_order+"**"+cbo_company_id+"**"+cbo_source+"**"+exchange_rate+"**"+mst_id+"**"+booking_pi_id+"**"+cbo_store_name+"**"+booking_pi_id +"&action=show_fabric_desc_listview_update",
			type: 'POST',
			dataType: 'html',
			processData: false,
			cache: false,
			...
			success: function (result) { 
				//alert(result);
				$( "#list_fabric_desc_container" ).html( result );
			},
	...
	function fnc_trims_receive(operation)
	{
		...
		if(operation==6)
		{
			...
			if( rcv_id=="" )
			{
			 ...
			 var report_title=$( "div.form_caption" ).html();
			 print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title, "trims_receive_entry_print3", "requires/trims_receive_multi_ref_entry_v3_controller" ) 
			 return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			$("#tbl_list_fabric_desc_container").find('tbody tr').not(':first').each(function()
			{
				...
				var ile=$('#tdile_'+i).html();
				var amount=$('#tdamount_'+i).html();
				var rejectrecvqnty=$('#tdrejectrecvqnty_'+i).html();
				var bookcurrency=$('#tdbookcurrency_'+i).html();
				var blqty=$('#tdblqty_'+i).html();
				var blamt=$('#tdblqty_'+i).attr("blamt");
	...
	function fnc_trims_receive_Reply_info()
	{
		if(http.readyState == 4) 
		{
			...
			if(reponse[0]==20 || reponse[0]==40 || reponse[0]==50)
			{
				alert(reponse[1]);
				release_freezing();
				return;	
			...
			{	
				...
				if(reponse[0]==0 || reponse[0]==1 )
				{
					...
					//if(reponse[0]!=2)
					//{
						//show_list_view_post(reponse[1],'show_trims_listview','list_container_trims','requires/trims_receive_multi_ref_entry_v3_controller','');
					//}					
	...
	function trims_receive_popup()
	{
		...
		{ 	
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1120px,height=420px,center=1,resize=1,scrolling=0','../');
			emailwindow.onclose=function()
			{
				...
				if(trims_recv_id!="")
				{
					...
					//show_list_view_post(reponse[1],'show_trims_listview','list_container_trims','requires/trims_receive_multi_ref_entry_v3_controller','');
					//show_list_view_post(trims_recv_id,'show_trims_listview','list_container_trims','requires/trims_receive_multi_ref_entry_v3_controller','');
	...
	function calculate(i)
	{
		...
		var bl_qnty 		= $('#tdblqty_'+i).html()*1;
		var rate			= $('#tdrate_'+i).html()*1;	 
		var ile_cost 		= $('#tdile_'+i).html()*1;
		
		var amount 			= quantity*(rate+ile_cost);
		//alert(quantity+"="+rate+"="+ile_cost+"="+amount);
		...
		$('#tdamount_'+i).html(number_format_common(amount,"","",currency_id));
		$('#bookcurrency_'+i).html(number_format_common(bookCurrency,"","",1));
		
		var ddd={ dec_type:5, comma:0, currency:''}
		var numRow = $('table#tbl_list_fabric_desc_container tbody tr').length-1;
		//alert(numRow);
		math_operation( "value_tot_rcv_qnty", "receiveqnty_", "+", numRow,ddd );
	}
	...
	function fn_pament_over(str,pay_val,seq_no)
	{
		//alert(str+"="+pay_val);
		//$('.common_color_'+str).val(pay_val);
		var first_tr=parseInt(seq_no)+1;
		var trall=$("#tbl_list_fabric_desc_container tbody tr").length-1;
		var copy_tr=parseInt(trall);
		if($('#paymentOverRcv').is(':checked'))
		{
			for(var k=first_tr; k<=copy_tr; k++)
			{
				$("#cbopaymentoverrecv_"+k).prop('selectedIndex', pay_val);
			}
		}
	}
	...
	function fn_fill_qnty()
	{
		...
		{
			$("#tbl_list_fabric_desc_container").find('tbody tr').not(':first').each(function(index, element) {
				var issue_qnty=$(this).find('input[name="receiveqnty[]"]').val()*1;
				var bl_qnty=$("#tdblqty_"+i).html()*1;
				if(issue_qnty<=0)
				{
					if($(this).css('display') != 'none'){
						$(this).find('input[name="receiveqnty[]"]').val(bl_qnty);
					}
				}
		...
		else
		{
			$("#tbl_list_fabric_desc_container").find('tbody tr').not(':first').each(function(index, element) 
			{
				if($(this).css('display') != 'none'){
					...
					$(this).find('input[name="receiveqnty[]"]').attr("placeholder",bl_qnty);
				}
				i++;
            });
	...
	{		
		...
		var tbl_length=$('#tbl_list_fabric_desc_container tbody tr').length;
	...
	function fn_load_room(floor_id, sequenceNo)
	{
		...
		for(var i=sequenceNo; i<=tbl_length; i++)
		{
			...
			for (var key of Object.keys(JSONObject).sort())
			{
				...
				$('#cbo_room_to_'+i).attr("disabled",false);
			};
		}
	}
	...
	function fn_load_rack(room_id, sequenceNo)
	{
		...
		for(var i=sequenceNo; i<=tbl_length; i++)
		{
			...
			for (var key of Object.keys(JSONObject).sort())
			{
				...
				$('#txt_rack_to_'+i).attr("disabled",false);
			};
		}
	}
	...
	function fn_load_shelf(rack_id, sequenceNo)
	{
		...
		for(var i=sequenceNo; i<=tbl_length; i++)
		{
			...
			for (var key of Object.keys(JSONObject).sort())
			{
				...
				$('#txt_shelf_to_'+i).attr("disabled",false);
			};
		}
	}
	function fn_load_bin(shelf_id, sequenceNo)
	{
		...
		for(var i=sequenceNo; i<=tbl_length; i++)
		{
			...
			for (var key of Object.keys(JSONObject).sort())
			{
				...
				$('#txt_bin_to_'+i).attr("disabled",false);
			};
		}
	}
	...
	{
		var numRow=$('#tbl_list_fabric_desc_container tbody tr').length;
	...
	function company_on_change(data)
	{
		var varible_string=return_global_ajax_value( data, 'varible_inventory', '', 'requires/trims_receive_multi_ref_entry_v3_controller');
		...
		$('#is_rate_optional').val(varible_string_ref[1]);
...
<div style="width:100%;">
    ...
    <form name="trimsreceive_1" id="trimsreceive_1">
    	<div style="width:1350px;">        
			...
			<? //echo "<pre>"; print_r($_SESSION['logic_erp']['data_arr'][631]);?>
            <legend>Trims Receive Entry Master Part</legend>
			<fieldset style="width:1150px">
            <table cellpadding="0" cellspacing="2" width="100%">
                <tr>
                    <td align="right" colspan="5"><strong> Received ID </strong></td>
                    <td colspan="5">
                ...
                <tr>
                    ...
                    <td width="100" class="must_entry_caption"> Receive Basis </td>
                    <td width="150">
                        <? 
                        	echo create_drop_down("cbo_receive_basis",122,$receive_basis_arr,"",0,"-- Select --",1,"set_receive_basis(0)","",'1,2');
                        	// echo create_drop_down("cbo_receive_basis",122,$receive_basis_arr,"",0,"-- Select --",1,"set_receive_basis(1);","",'1,2');
                        ?>
                    </td>
                    ...
                    <td> 
                        <?
                           echo create_drop_down( "cbo_currency_id", 200,$currency,"", 1, "Select Currency ", 0, "load_exchange_rate();",1 );
                        ?>
                    </td>
                </tr> 
                <tr>
                ...
                <tr>
                    ...
                    <td width="100" >Gate Entry No</td>
                    <td width="150">
                        <input type="text" name="txt_gate_entry" id="txt_gate_entry" class="text_boxes" style="width:130px" placeholder="Write">
                    </td>
                    <td>Remarks</td>
                    <td>
                        <input type="text" name="txt_remarks" id="txt_remarks" class="text_boxes" style="width:188px" value="" placeholder="Write">
                    </td>
                </tr>
				<tr>
					<td></td>
             ...
             <fieldset style="width:2000px; margin-top:10px;">
                 	<legend>Trims Receive Entry details part</legend>
                    <? $i=1; ?>
                    	<table class="rpt_table" width="100%" cellspacing="0" cellpadding="0" border="1" rules="all" id="tbl_fabric_desc_item">
                        	<thead>
								...
								<tr>
									...
									<th width="60">WO/PI Qnty</th>
									<th width="70"><input type="checkbox" id="check_qnty" name="check_qnty" onChange="fn_fill_qnty()" /><br>Receive Qnty</th>
									<th width="70" class="rate_td">Rate</th>
									<th width="70" id="ile_td">ILE%</th>
									<th width="70" class="amount_td">Amount</th>
									<th width="70">Reject Qnty</th>
									<th width="70">Book Keeping Currency</th>
									<th width="70">Cumulative Receive Qnty</th>
									<th width="70">Balance PI/Order Qnty</th>
									<th><input type="checkbox" checked id="paymentOverRcv" name="paymentOverRcv"/><br>Payment For Over Rcv.Qty.</th>
								</tr>
                            </thead>
                        </table>
                        <table class="rpt_table" width="100%" cellspacing="0" cellpadding="0" border="1" rules="all" id="tbl_list_fabric_desc_container">
                        	<tbody id="list_fabric_desc_container">
                            	<tr id="row_1" align="center">
                                    ...
                                    <td width="70" id="tdrate_1" class="rate_td">
                                    ...
                                    <td width="70" id="tdamount_1" class="amount_td">
                        ...
                        <table class="rpt_table" width="100%" cellspacing="0" cellpadding="0" border="1" rules="all" id="list_fabric_desc_container_footer">
                            <tfoot>
                                ...
                                <th width="70"><input type="text" id="value_tot_rcv_qnty" name="value_tot_rcv_qnty" style="width:50px;" class="text_boxes_numeric" readonly disabled /></th>
                                <th width="70" class="rate_td">&nbsp;</th>
                                <th width="70">&nbsp;</th>
                                <th width="70" class="amount_td">&nbsp;</th>
                                <th width="70">&nbsp;</th>
                                <th width="70">&nbsp;</th>
                                <th width="70">&nbsp;</th>
                                <th width="70">&nbsp;</th>
                 ...
                 <table width="100%">
                    <tr>
                        <td width="80%" align="center"> 
							...
							//reset_form('trimsreceive_1','list_container_trims*list_fabric_desc_container','','','set_receive_basis(1);','')
                            echo load_submit_buttons($permission, "fnc_trims_receive", 0,0,"window.location.reload();",1);
                            ?>
                            <input type="hidden" id="is_posted_account" name="is_posted_account" value=""/>
                        </td>
                    </tr>
                     <tr>
                         <td width="80%" align="center" id="button_data_panel">
