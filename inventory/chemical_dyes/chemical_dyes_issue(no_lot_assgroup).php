...
$store_location_credential_cond = $userCredential[0][csf('store_location_id')];
...
<script>
	...
	function fnc_chemical_dyes_issue_entry(operation)
	{
		if(operation==4)
		{
			...
			generate_report_file( $('#cbo_dying_company').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+$('#txt_mrr_no').val(),'chemical_dyes_issue_print','requires/chemical_dyes_issue_controller');
			return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			if( $("#cbo_issue_basis").val()==7 && $("#cbo_issue_purpose").val()==13)
				{
					if( form_validation('cbo_dying_company*cbo_issue_basis*txt_issue_date*cbo_store_name','Company Name*Issue Basis*Issue Date*Store Name')==false )
					{
					return;
					}
				}
				...
				{
					if( form_validation('cbo_dying_company*cbo_issue_basis*txt_issue_date*cbo_sub_process*cbo_store_name','Company Name*Issue Basis*Issue Date*Sub-Process*Store Name')==false )
					{
					return;
					}	
				}
			...
			var dataString = "txt_mrr_no*update_id*cbo_company_name*cbo_location_name*cbo_issue_basis*txt_req_no*txt_req_id*txt_batch_no*txt_batch_id*cbo_issue_purpose*cbo_loan_party*cbo_dying_source*cbo_dying_company*txt_challan_no*txt_issue_date*txt_recipe_no*txt_recipe_id*txt_order_no*hidden_order_id*cbo_buyer_name*txt_style_no*cbo_sub_process*cbo_store_name*txt_machine_name*txt_machine_id*txt_return";
			
			var mst_data=get_submitted_data_string(dataString,"../../");
		
			for (var i=1; i<row_num; i++)
			{
				mst_data=mst_data+get_submitted_data_string('txt_prod_id_'+i+'*cbo_store_name_'+i+'*txt_item_cat_'+i+'*cbo_dose_base_'+i+'*txt_ratio_'+i+'*txt_recipe_qnty_'+i+'*txt_adj_per_'+i+'*cbo_adj_type_'+i+'*txt_reqn_qnty_'+i+'*txt_reqn_qnty_edit_'+i+'*updateIdDtls_'+i+'*transId_'+i+'*hidtxt_reqn_qnty_edit_'+i+'*txt_remarks_'+i,"../../",2);
			}
	...
	{	
		if(http.readyState == 4) 
		{
			...
			if($('#cbo_issue_basis').val()==7 && $('#cbo_issue_purpose').val()==13)
			{
				...
				{
					disable_enable_fields( 'cbo_company_name*cbo_issue_basis*cbo_dying_source*cbo_dying_company*cbo_sub_process', 1, '', '' );
			...
			else
			{
				if((reponse[0]==0 || reponse[0]==1))
				{
					disable_enable_fields( 'cbo_company_name*cbo_issue_basis*cbo_dying_source*cbo_dying_company', 1, '', '' ); //*cbo_sub_process
					document.getElementById('update_id').value = reponse[1];
					document.getElementById('txt_mrr_no').value = reponse[2];
					$('#cbo_company_name').attr('disabled','disabled');
...
function open_reqpopup()
{
	...
	var page_link='requires/chemical_dyes_issue_controller.php?action=req_popup&company='+company+'&issue_purpose='+issue_purpose+'&issue_basis='+issue_basis;
	...
	emailwindow.onclose=function()
	{
		...
		if(cbo_issue_basis==7 && cbo_issue_purpose==13)
		{
		...
		show_list_view(reqId+'**'+company+"**"+cbo_issue_purpose+"**"+cbo_store_name+"**"+update_id, 'machine_item_details', 'list_container_recipe_items', 'requires/chemical_dyes_issue_controller', '');	
...
function openmypage_batchNo()
	...
	function open_mrrpopup()
	{
		...
		emailwindow.onclose=function()
		{
			...
			if(mrrNumber[2]==7 && mrrNumber[3]==13)
			{
				...
				show_list_view(mrrNumber[2]+'**'+company+"**"+mrrNumber[3]+"**"+cbo_store_name+"**"+mrrNumber[1], 'machine_item_details', 'list_container_recipe_items', 'requires/chemical_dyes_issue_controller', '');
				$('#cbo_sub_process').attr('disabled','disabled');
				set_button_status(1, permission, 'fnc_chemical_dyes_issue_entry',1,1);
				$("#btn_cost_print").removeClass("formbutton_disabled");
				$("#btn_cost_print").addClass("formbutton");
			...
			disable_enable_fields( 'cbo_company_name*cbo_issue_basis*cbo_dying_source*cbo_dying_company', 1, '', '' );
		}
	}
	...
	function set_dying_company(param)
	{
		// if(param==1)
		// {
		// 	document.getElementById('cbo_dying_company').value=document.getElementById('cbo_company_name').value;
		// }
		if(param==3)
		{
                ...
                <tr>
                    <td width="80%" align="center" valign="top">   
                            ...
                            <fieldset style="width:950px;">                                       
                                <table  width="950" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                                    ...
                                    <tr>
										...
										<td width="94" align="right"> Working Company </td>
                                        <td width="160" id="dyeing_td">
                                        <?
                                        echo create_drop_down( "cbo_dying_company", 170, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "" );
									 ...
									 <tr>
                                        ...
                                        <td width="160">
                                        <? 
                                        echo create_drop_down( "cbo_issue_basis", 170, $receive_basis_arr,"", 1, "- Select Receive Basis -", $selected, "fn_independent(this.value)","","4,5,7" );//fn_independent(this.value)
                                        ?>
                                        </td>
									 </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td  width="" align="right">Issue Purpose </td>
                                        <td width="">
                                        <? 
                                        
                                        echo create_drop_down( "cbo_issue_purpose", 170, $yarn_issue_purpose,"", 1, "-- Select Purpose --", "", "check_machine_wash(this.value);", "","2,3,5,10,11,13,14,26,27,28,29,30,32,33,34,35,40");
                                        ...
                                        <input type="hidden" id="txt_req_id" name="txt_req_id" value="" />
										 ...
										 <td width="94" align="right" >  Loan Party </td>
                                    ...
                                    <td width="" align="right">Batch No </td>
                                        <td width="">
                                        <input class="text_boxes"  type="text" name="txt_batch_no" id="txt_batch_no" onDblClick="openmypage_batchNo()" placeholder="Double Click" style="width:160px;"    /> 
                                        <input class="text_boxes"  type="hidden" name="txt_batch_id" id="txt_batch_id"  style="width:160px;"    /> 
                        ...
                        <table cellpadding="0" cellspacing="1" width="100%">
                            ...
                            <input type="button" id="btn_cost_print" name="btn_cost_print"   style="width:80px;"  class="formbutton_disabled" value="Cost"  onClick="generate_report_file($('#cbo_dying_company').val()+'*'+$('#update_id').val()+'*'+$('#txt_mrr_no').val(),'chemical_dyes_issue_cost_print','requires/chemical_dyes_issue_controller');"/>
