...
if($action=="load_drop_down_dyeing")
{
	...
	echo create_drop_down( "cbo_dying_company", 170, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond $company_credential_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "reset_form('chemicaldyesissue_1','list_container_yarn*list_product_container*list_container_recipe_items','','','','txt_issue_date*cbo_dying_source*cbo_company_name*cbo_dying_company');company_onchange(this.value);","" );
	exit();
}
...
if ($action=="load_drop_down_division")
{
	...
	echo create_drop_down( "cbo_division_name", 170,$sql,"id,division_name", 1, "-- Select --", 0, "load_drop_down( 'requires/chemical_dyes_issue_controller', this.value, 'load_drop_down_department','department_td');" );
	die;
}

if ($action=="load_drop_down_department")
{
	echo create_drop_down( "cbo_department_name", 170,"select id,department_name from lib_department where division_id=$data and is_deleted=0 and status_active=1","id,department_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/chemical_dyes_issue_controller', this.value, 'load_drop_down_section','section_td');" );
	die;
}
...
if ($action=="load_drop_down_sub_process")
{
	...
	if($db_type==0)
	{
		$sub_process_id=return_field_value("group_concat(b.sub_process_id)  as sub_process_id ","pro_recipe_entry_dtls b, pro_recipe_entry_mst a","a.working_company_id=$data[0] and a.id=b.mst_id and a.entry_form in (59,60) and b.ratio>0 and a.id in($data[1]) and b.status_active=1 and b.is_deleted=0",'sub_process_id');
	}
	else if($db_type==2)
	{
		//echo "select  listagg(b.sub_process_id,',') within group (order by b.sub_process_id 	) as sub_process_id from pro_recipe_entry_dtls b, pro_recipe_entry_mst a where a.working_company_id=$data[0] and b.ratio>0 and a.id=b.mst_id and a.id in($data[1]) and b.status_active=1 and b.is_deleted=0";die;
		if($data[3]==49){  //yurn dying
			$sub_process_id=return_field_value("listagg(b.sub_process_id,',') within group (order by b.sub_process_id ) as sub_process_id ","YD_RECIPE_ENTRY_DTLS b, YD_RECIPE_ENTRY_MST a","a.company_id=$data[0] and b.ratio>0 and a.id=b.mst_id and a.entry_form in (435) and a.id in($data[1]) and b.status_active=1 and b.is_deleted=0",'sub_process_id');
		
		}
		else{
			$sub_process_id=return_field_value("listagg(b.sub_process_id,',') within group (order by b.sub_process_id ) as sub_process_id ","pro_recipe_entry_dtls b, pro_recipe_entry_mst a","a.working_company_id=$data[0] and b.ratio>0 and a.id=b.mst_id and a.entry_form in (59,60) and a.id in($data[1]) and b.status_active=1 and b.is_deleted=0",'sub_process_id');
		}
	}
	
	//echo $sub_process_id; die;
...
if($action=="req_popup")
{
	...
	function js_set_value(mrr)
	{
		...
		$("#hidden_receipe_no").val(data[8]); // recipe no
    ...
    <input type="hidden" id="hidden_receipe_no"/>
...
if($action=="create_reqisition_search_list_view")
{
	...
	else if($ex_data[4]==49) // yurn dying 
	{
		$sql ="select a.requ_no, a.requ_prefix_num, a.company_id, a.requisition_date, a.requisition_basis, a.batch_id, a.recipe_id, a.id, a.machine_id, a.store_id, sum(b.required_qnty) as requisition_qnty,c.recipe_no
		from dyes_chem_issue_requ_dtls b, dyes_chem_issue_requ_mst a left join YD_RECIPE_ENTRY_MST c on c.id = a.recipe_id and c.status_active=1 and c.entry_form = 435
		where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.company_id=$company and a.is_apply_last_update!=2 and a.entry_form in(437) and a.recipe_id!=$null_cond $sql_cond $req_cond $year_cond
		group by a.requ_no, a.requ_prefix_num, a.company_id, a.requisition_date, a.requisition_basis, a.batch_id, a.recipe_id, a.id, a.machine_id, a.store_id,c.recipe_no";
		// echo $sql;die;
	}
	else
	{
	...
	$recipe_arr=sql_select("select a.ID, a.COMPANY_ID from pro_recipe_entry_mst a where a.working_company_id=$company and a.entry_form in(60,59,445)");
    ...
    <div style="width:810px; overflow-y:scroll;  max-height:230px; cursor:pointer" id="buyer_list_view">
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="790" class="rpt_table" id="tbl_list_search">
            <tbody>
				...
				{  
					...
					{
						$company_id=$company_id=$receipe_data_arr[$recipe_ids];
					}
					...
					{
						?>
						<tr bgcolor="<? echo $bgcolor; ?>" onClick="js_set_value('<? echo $row[csf('requ_no')]."_".$row[csf('id')]."_".$batchIds."_".$row[csf('recipe_id')]."_".$row[csf('machine_id')]."_".$machine_name_arr[$row[csf('machine_id')]]."_".$company_id."_".$row[csf('store_id')]."_".$row[csf('recipe_no')]; ?>')">
							...
							<td width="80" align="center" style="word-break:break-all"><? echo $row[csf('requ_no')]; ?></td>
	...
	<div>
        ...
        <div style="width:1310px; overflow-y:scroll; max-height:230px;" id="buyer_list_view" align="center">
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1292" class="rpt_table" id="tbl_list_search">
                <tbody>
	                <?
					//$store_wise_qnty=return_library_array("select prod_id, cons_qty from inv_store_wise_qty_dtls where status_active=1 and is_deleted=0 and cons_qty>0 and store_id=$cbo_store","prod_id","cons_qty");
	                ...
	                if($is_update=="")
	                {
						...
						where a.id=b.mst_id and a.entry_form=5 and a.issue_basis=7 and a.req_no=$requsn_id and b.sub_process='0'");
						...
						//echo "<pre>";print_r($store_wise_qnty);die;
						$sql="select a.id, a.item_category_id, a.item_group_id, a.sub_group_name, a.item_description, a.item_size, a.unit_of_measure, 1 as dose_base, b.id as dtls_id, a.current_stock, b.dose_base as dose_base_curr, b.ratio, b. recipe_qnty, b.required_qnty, b.req_qny_edit, b.item_lot as batch_lot 
						from product_details_master a ,dyes_chem_issue_requ_dtls b 
						where a.id=b.product_id and b.mst_id=$requsn_id and b.status_active=1 and b.is_deleted=0 and a.company_id='$company_id' and  a.item_category_id in (5,6,7) and a.status_active in(1,3) and a.is_deleted=0 
						order by b.id";
						...
						foreach ($nameArray as $selectResult)
						{
							...
							//echo $selectResult[csf('id')].'='.$req_qnty.'='.$store_wise_qnty[$selectResult[csf("id")]].'<br>';
							if(number_format($req_qnty,4,".","")>0 && $stock_qty>0)
							{
								?>
	                            ...
	                            <td width="120"><p><? echo $selectResult[csf('batch_lot')]; ?>
								...
								<input type="text" name="reqn_qnty_edit[]" id="txt_reqn_qnty_edit_<? echo $i; ?>" class="text_boxes_numeric" style="width:80%" onKeyUp="check_data('#txt_reqn_qnty_edit_<? echo $i; ?>',<? echo $store_wise_qnty[$selectResult[csf("id")]]+$req_qnty; ?>)"  value="<? echo number_format($selectResult[csf('required_qnty')],6,".",""); ?>"></p></td>
								...
								<input type="hidden" name="hidreqn_qnty_edit[]" id="hidtxt_reqn_qnty_edit_<? echo $i; ?>" class="text_boxes_numeric" style="width:80%"  value="<? echo number_format($req_qnty,4,".",""); ?>" /></p>
	                ...
	                {
						$sql="select a.id, t.store_id, a.item_category_id, a.item_group_id, a.sub_group_name, a.item_description, a.item_size, a.unit_of_measure, a.current_stock, b.id as dtls_id, b.trans_id, b.dose_base, b.ratio, b.recipe_qnty, b.adjust_percent, b.adjust_type, b.required_qnty, b.req_qny_edit, t.batch_lot, t.remarks  
						from product_details_master a, dyes_chem_issue_dtls b, inv_transaction t 
						where a.id=b.product_id and b.trans_id=t.id and b.mst_id=$is_update and b.status_active=1 and b.is_deleted=0 and a.company_id='$company_id' and a.item_category_id in(5,6,7) and a.status_active in(1,3) and a.is_deleted=0 order by a.item_category_id";
						//echo $sql;
						$i=1;
						$nameArray=sql_select( $sql );
						foreach ($nameArray as $selectResult)
                ...
                <tbody>
	                ...
	                if($is_update=="")
	                {
						if($issue_basis==4)
						{
							...
							where a.id=b.prod_id and a.company_id='$company_id' and b.store_id=$cbo_store and a.item_category_id in(5,6,7,23) and round(b.cons_qty,6)>0 and a.status_active in(1,3) and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
							...
							foreach ($nameArray as $selectResult)
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>">
	                                ...
	                                <td width="120"><p><? echo $selectResult[csf('lot')]; ?>
						...
						if($issue_basis==5)
						{
							$sql="select a.id, a.item_category_id, a.item_group_id, a.sub_group_name, a.item_description, a.item_size, a.unit_of_measure, b.cons_qty as store_stock, b.lot 
							from product_details_master a, inv_store_wise_qty_dtls b  
							where a.id=b.prod_id and a.company_id='$company_id' and b.store_id=$cbo_store and a.item_category_id in(5,6,7,23) and round(b.cons_qty,6)>0 and a.status_active in(1,3) and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
							order by  a.item_category_id, a.id";	
							...
							foreach ($nameArray as $selectResult)
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>"> 
	                                ...
	                                <td width="120"><? echo $selectResult[csf('lot')]; ?><p>
	                                <input type="hidden" name="txt_lot[]" id="txt_lot_<? echo $i; ?>" value="<? echo $selectResult[csf('lot')]; ?>"></p></td>
	                                <td  width="100"><p><? echo $item_category[$selectResult[csf('item_category_id')]]; ?>
	                                <input type="hidden" name="txt_item_cat[]" id="txt_item_cat_<? echo $i; ?>" class="text_boxes_numeric" style="width:38px"  value="<? echo $selectResult[csf('item_category_id')]; ?>"></p>
	                                </td>
	                                ...
	                                <td width="140" id="item_description_<? echo $i; ?>"><p><? echo $selectResult[csf('item_description')]." ".$selectResult[csf('item_size')]; ?></p></td> 
	                                <td width="32" align="center" id="uom_<? echo $i; ?>"><p><? echo $unit_of_measurement[$selectResult[csf('unit_of_measure')]]; ?></p></td>
	                                <td width="70" align="center" id="dose_base_<? echo $i; ?>"><p><? echo create_drop_down("cbo_dose_base_$i", 68, $dose_base, "", 1, "- Select Dose Base -",$selectResult[csf('dose_base')],"calculate_receipe_qty($i)"); ?></p></td>
	                                <td width="50" align="center" id="ratio_<? echo $i; ?>"><p><input type="text" name="txt_ratio[]" id="txt_ratio_<? echo $i; ?>" class="text_boxes_numeric" style="width:38px"  value="<? //echo number_format($ratio,6,".",""); ?>" onChange="calculate_receipe_qty(<? echo $i; ?>)"/></p></td>
	                                <td width="70" title="<? echo number_format($stock_qty,6,".","");?>" align="center" id="td_stock_qty_<? echo $i; ?>"><p><input type="text" name="stock_qty[]" id="stock_qty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px"  value="<? echo number_format($stock_qty,6,".","");//$selectResult[csf('current_stock')]; ?>"  disabled></p></td>
	                                <td width="70" align="center" id="recipe_qnty_<? echo $i; ?>"><p><input type="text" name="txt_recipe_qnty[]" id="txt_recipe_qnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px"  value="<? //echo number_format($recipe_qnty,6,".",""); ?>" onClick="check_data('#txt_reqn_qnty_edit_<? echo $i; ?>',<? echo $stock_qty;//$selectResult[csf('current_stock')]; ?>)"  readonly /></p></td>
	                                <td width="55" align="center" id="adj_per_<? echo $i; ?>"><p><input type="text" name="txt_adj_per[]" id="txt_adj_per_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px"  value="<? //echo $ratio; ?>" onChange="calculate_receipe_qty(<? echo $i; ?>)" ></p></td>
	                                <td width="90" align="center" id="adj_type_<? echo $i; ?>"><p><? echo create_drop_down("cbo_adj_type_$i", 90, $increase_decrease, "", 1, "- Select -","","calculate_receipe_qty($i)"); ?></p></td>
	                                <td width="" align="center" id="reqn_qnty_<? echo $i; ?>"><p>
	                                <input type="hidden" name="txt_reqn_qnty[]" id="txt_reqn_qnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:80%"  value="<? //echo number_format($recipe_qnty,6,".",""); ?>" readonly>
	                                <input type="text" name="reqn_qnty_edit[]" id="txt_reqn_qnty_edit_<? echo $i; ?>" class="text_boxes_numeric" style="width:80%"  value="<? //echo number_format($recipe_qnty,6,".","");  ?>"  >
	                                <input type="hidden" name="hidreqn_qnty_edit[]" id="hidtxt_reqn_qnty_edit_<? echo $i; ?>" class="text_boxes_numeric" style="width:80%"  value="<? //echo number_format($selectResult[csf('req_qny_edit')],6,".",""); ?>" />
						...
						if($issue_basis==7)
						{
							$total_issue_sql=sql_select("select a.issue_number, b.product_id, b.req_qny_edit as req_qny_edit 
							from inv_issue_master a, dyes_chem_issue_dtls b 
							where a.id=b.mst_id and a.entry_form=5 and a.issue_basis=7 and a.status_active=1 and b.status_active=1 and a.req_no='".$req_id."' and b.sub_process=$sub_process_id");
							...
							$sql="select a.id, a.item_category_id, a.item_group_id, a.sub_group_name, a.item_description, a.item_size, a.unit_of_measure, a.current_stock, b.id as dtls_id, b.dose_base, b.ratio, b.recipe_qnty, b.adjust_percent, b.adjust_type, b.required_qnty, b.req_qny_edit, b.item_lot 
							from product_details_master a, dyes_chem_issue_requ_dtls b 
							where a.id=b.product_id and b.mst_id='".$req_id."' and b.sub_process=$sub_process_id and b.status_active=1 and b.is_deleted=0 and a.company_id='$company_id' and a.item_category_id in(5,6,7,23) and a.status_active in(1,3) and a.is_deleted=0 order by a.item_category_id, a.id";
							//echo $sql;
							$is_disabled=1;
							$i=1;
							...
							foreach ($nameArray as $selectResult)
							{
								...
								if(number_format($req_qnty,6,".","")>0)
								{
	                                ...
	                                <tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>"> 
	                                    ...
	                                    <td width="120"><p><? echo $selectResult[csf('item_lot')]; ?>
	                                    ...
	                                    <td width="32" align="center" id="uom_<? echo $i; ?>"><p><? echo $unit_of_measurement[$selectResult[csf('unit_of_measure')]]; ?></p></td>
	                                    <td width="70" align="center" id="dose_base_<? echo $i; ?>"><p><? echo create_drop_down("cbo_dose_base_$i", 68, $dose_base, "", 1, "- Select Dose Base -",$selectResult[csf('dose_base')],"calculate_receipe_qty($i)",1); ?></p></td>
	                                    <td width="50" align="center" id="ratio_<? echo $i; ?>"><p><input type="text" name="txt_ratio[]" id="txt_ratio_<? echo $i; ?>" class="text_boxes_numeric" style="width:38px"  value="<? echo number_format($selectResult[csf('ratio')],4,".",""); ?>" onChange="calculate_receipe_qty(<? echo $i; ?>)" readonly></p></td>
	                                    <td width="70" title="<? echo number_format($stock_qty,6,".","");?>" align="center" id="td_stock_qty_<? echo $i; ?>"><p><input type="text" name="stock_qty[]" id="stock_qty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px"  value="<? echo number_format($stock_qty,6,".","");//$selectResult[csf('current_stock')]; ?>"  disabled></p></td>
	                                    <td width="70" align="center" id="recipe_qnty_<? echo $i; ?>"><p><input type="text" name="txt_recipe_qnty[]" id="txt_recipe_qnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px"  value="<? echo number_format($selectResult[csf('recipe_qnty')],6,".",""); ?>"  onClick="check_data('#txt_reqn_qnty_edit_<? echo $i; ?>',<? echo $stock_qty;//$selectResult[csf('current_stock')]; ?>)" readonly></p></td>
	                                    <td width="55" align="center" id="adj_per_<? echo $i; ?>"><p><input type="text" name="txt_adj_per[]" id="txt_adj_per_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px"  value="<? echo $selectResult[csf('adjust_percent')]; ?>" onChange="calculate_receipe_qty(<? echo $i; ?>)" readonly></p></td>
	                                    <td width="90" align="center" id="adj_type_<? echo $i; ?>"><p><? echo create_drop_down("cbo_adj_type_$i", 90, $increase_decrease, "", 1, "- Select -", $selectResult[csf('adjust_type')],"calculate_receipe_qty($i)",1); ?></p></td>
	                                    <td width="" align="center" id="reqn_qnty_<? echo $i; ?>"><p>
	                                    <input type="hidden" name="txt_reqn_qnty[]" id="txt_reqn_qnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:80%"  value="<? echo number_format($req_qnty,6,".",""); ?>" readonly>
	                ...
	                {
						$sql="select a.id, t.store_id, a.item_category_id, a.item_group_id, a.sub_group_name, a.item_description, a.item_size, a.unit_of_measure, a.current_stock, b.id as dtls_id, b.trans_id, b.dose_base, b.ratio,recipe_qnty, b.adjust_percent, b.adjust_type, b.required_qnty, b.req_qny_edit, t.batch_lot, t.remarks 
						from product_details_master a, dyes_chem_issue_dtls b, inv_transaction t 
						where a.id=b.product_id and b.trans_id=t.id and b.mst_id=$is_update and b.sub_process=$sub_process_id and b.status_active=1 and b.is_deleted=0 and a.company_id='$company_id' and a.item_category_id in(5,6,7,23) and a.status_active in(1,3) and a.is_deleted=0 
						order by a.item_category_id, a.id";
						//echo $sql;
						$i=1;
						$stock_qty_arr=fnc_store_wise_stock($company_id,$cbo_store);
						...
						foreach ($nameArray as $selectResult)
						{
							...
							if($issue_basis==4)
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>"> 
	                                ...
	                                <td width="120"><? echo $selectResult[csf('batch_lot')]; ?><p>
	                                <input type="hidden" name="txt_lot[]" id="txt_lot_<? echo $i; ?>" value="<? echo $selectResult[csf('batch_lot')]; ?>"></p></td>
	                                <td  width="100"><p><? echo $item_category[$selectResult[csf('item_category_id')]]; ?>
	                                <input type="hidden" name="txt_item_cat[]" id="txt_item_cat_<? echo $i; ?>" class="text_boxes_numeric" style="width:38px"  value="<? echo $selectResult[csf('item_category_id')]; ?>"></p>
	                                </td>
							...
							if($issue_basis==5)
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>"> 
	                                ...
	                                <td width="32" align="center" id="uom_<? echo $i; ?>"><p><? echo $unit_of_measurement[$selectResult[csf('unit_of_measure')]]; ?></p></td>
	                                <td width="70" align="center" id="dose_base_<? echo $i; ?>"><p><? echo create_drop_down("cbo_dose_base_$i", 68, $dose_base, "", 1, "- Select Dose Base -",$selectResult[csf('dose_base')],"calculate_receipe_qty($i)"); ?></p></td>
	                                <td width="50" align="center" id="ratio_<? echo $i; ?>"><p><input type="text" name="txt_ratio[]" id="txt_ratio_<? echo $i; ?>" class="text_boxes_numeric" style="width:38px"  value="<? echo number_format($selectResult[csf('ratio')],6,".",""); ?>" onChange="calculate_receipe_qty(<? echo $i; ?>)" ></p></td>
	                                <td width="70" title="<? echo number_format($stock_qty,6,".","")?>" align="center" id="td_stock_qty_<? echo $i; ?>"><p><input type="text" name="stock_qty[]" id="stock_qty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px"  value="<? echo number_format($stock_qty,6,".","");//$selectResult[csf('current_stock')]; ?>"  disabled></p></td>
	                                <td width="70" align="center" id="recipe_qnty_<? echo $i; ?>"><p><input type="text" name="txt_recipe_qnty[]" id="txt_recipe_qnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px"  value="<? echo number_format($selectResult[csf('recipe_qnty')],6,".",""); ?>" onClick="check_data('#txt_reqn_qnty_edit_<? echo $i; ?>',<? echo $stock_qty;//$selectResult[csf('current_stock')]; ?>)" readonly ></p></td>
	                                <td width="55" align="center" id="adj_per_<? echo $i; ?>"><p><input type="text" name="txt_adj_per[]" id="txt_adj_per_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px"  value="<? echo $selectResult[csf('adjust_percent')]; ?>" onChange="calculate_receipe_qty(<? echo $i; ?>)" ></p></td>
	                                <td width="90" align="center" id="adj_type_<? echo $i; ?>"><p><? echo create_drop_down("cbo_adj_type_$i", 90, $increase_decrease, "", 1, "- Select -", $selectResult[csf('adjust_type')],"calculate_receipe_qty($i)"); ?></p></td>
	                                <td width="90" align="center" id="reqn_qnty_<? echo $i; ?>"><p>
	                                <input type="hidden" name="txt_reqn_qnty[]" id="txt_reqn_qnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:80%"  value="<? echo number_format($selectResult[csf('required_qnty')],6,".",""); ?>" readonly>
	                                <input type="text" name="reqn_qnty_edit[]" id="txt_reqn_qnty_edit_<? echo $i; ?>" class="text_boxes_numeric" style="width:80%"  value="<? echo number_format($selectResult[csf('req_qny_edit')],6,".",""); ?>"  onKeyUp="check_data('#txt_reqn_qnty_edit_<? echo $i; ?>',<? echo $stock_qty+$selectResult[csf('req_qny_edit')];//$selectResult[csf('current_stock')]+$selectResult[csf('req_qny_edit')]; ?>)" <? echo $desable_cond; ?> />
	                                <input type="hidden" name="hidreqn_qnty_edit[]" id="hidtxt_reqn_qnty_edit_<? echo $i; ?>" class="text_boxes_numeric" style="width:80%"  value="<? echo number_format($selectResult[csf('req_qny_edit')],6,".",""); ?>" />
							...
							if($issue_basis==7)
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>"> 
	                                ...
	                                <input type="hidden" name="txt_reqn_qnty[]" id="txt_reqn_qnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:80%"  value="<? echo number_format($selectResult[csf('required_qnty')],6,".",""); ?>" readonly>
	                                <input type="text" name="reqn_qnty_edit[]" id="txt_reqn_qnty_edit_<? echo $i; ?>" class="text_boxes_numeric" style="width:80%"  value="<? echo number_format($selectResult[csf('req_qny_edit')],6,".",""); ?>"  onKeyUp="check_data('#txt_reqn_qnty_edit_<? echo $i; ?>',<? echo $stock_qty+$selectResult[csf('req_qny_edit')];//$selectResult[csf('current_stock')]+$selectResult[csf('req_qny_edit')]; ?>)"   <? echo $desable_cond; ?> />
	                                <input type="hidden" name="hidreqn_qnty_edit[]" id="hidtxt_reqn_qnty_edit_<? echo $i; ?>" class="text_boxes_numeric" style="width:80%"  value="<? echo number_format($selectResult[csf('required_qnty')],6,".",""); ?>" readonly  />
	                                <input type="hidden" name="updateIdDtls[]" id="updateIdDtls_<? echo $i; ?>" value="<? echo $selectResult[csf('dtls_id')]; ?>">	
	                                <input type="hidden" name="transId[]" id="transId_<? echo $i; ?>" value="<? echo $selectResult[csf('trans_id')]; ?>"></p>
	                                </td>
	                                <td align="center" id="remarks_<? echo $i; ?>"><p>
...
if($action=="create_mrr_search_list_view")
{
	...
	if($batch_no!="")
	{
		$batch_id=return_field_value("listagg(cast(id as varchar(4000)),',') within group(order by id) as batch_id" ,"pro_batch_create_mst","batch_no like '$batch_no' and status_active=1 and is_deleted=0","batch_id");
		
		if($batch_id!="")
		{
			$all_batch_id_arr=explode(",",$batch_id);
			$batch_id_string="";
			foreach($all_batch_id_arr as $b_id)
			{
				$batch_id_string.="'".$b_id."',";
			}
			$batch_id_string=chop($batch_id_string,",");
			$batch_cond=" and a.batch_no in($batch_id_string)";
		}
	}
   ...
   foreach( $sql_batch as $row){
	...
	{
		$issue_sql = "select b.requ_no, a.issue_number_prefix_num, a.issue_number, a.company_id, a.issue_date, a.issue_basis, a.issue_purpose, a.batch_no, a.issue_date, a.id, $year_id,
		 a.is_posted_account 
		 from inv_issue_master a, dyes_chem_issue_requ_mst b 
		 where a.req_no=b.id and a.company_id=$company and a.entry_form=5 and a.is_multi=0 and a.issue_basis=7 and a.status_active=1 and a.is_deleted=0 and b.requ_no $req_cond $sql_cond  $batch_cond $year_cond 
		...
		$issue_sql = "select b.requ_no, a.issue_number_prefix_num, a.issue_number, a.company_id, a.issue_date, a.issue_basis, a.issue_purpose, a.batch_no, a.issue_date, a.id, $year_id,
		 ...
		 where a.company_id=$company and a.entry_form=5 and a.is_multi=0 and a.status_active=1 and a.is_deleted=0 $sql_cond  $batch_cond $year_cond order by a.id desc";
	}
	$sql = sql_select($issue_sql);
	?>
	<br/>
...
if($action=="populate_batch_receipe_data")
{
	...
	if($data[3]==49){  //yurn dying
		$total_liquor=return_field_value("total_liquor as total_liquor" ,"yd_recipe_entry_dtls","mst_id in($data[2]) and ratio>0","total_liquor");

		$sql_data=sql_select("select id, entry_form, batch_id, total_liquor, new_batch_weight,batch_qty from YD_RECIPE_ENTRY_MST where id in(".$data[2].")");
	}
	else{
	...
	echo "document.getElementById('cbo_company_name').value = '".$data[0]."';\n"; 
}
if($action=="populate_buyer_order_for_batch")
{
	$data=explode("**",$data);
	$sql = sql_select("select id, extention_no,batch_no,color_range_id,booking_without_order, batch_weight, total_liquor,booking_no,entry_form from pro_batch_create_mst where id=$data[1] and status_active=1 and is_deleted=0 and batch_against<>0"); //company_id=$data[0] and 
	
	$po_number_sub_con=return_library_array("select id,order_no from subcon_ord_dtls","id","order_no");
	
	$po_sql=sql_select("Select a.entry_form, b.po_id, c.po_number, c.grouping as ref_no, c.file_no 
	from pro_batch_create_mst a, pro_batch_create_dtls b, wo_po_break_down c  
	where a.id=b.mst_id and b.po_id=c.id and a.id=".$data[1]."");
...
if($action=="populate_buyer_order_no_data")
{
	...
	if($data[3]==49){ // for yurn dyeing
		$buyer_id=return_field_value("buyer_id","yd_recipe_entry_mst","company_id=$data[0] and id in($data[2]) and entry_form=435","buyer_id");
	}
	else{
...
if($action=="populate_data_from_data")
{
	...
	foreach($sql as $row)
	{
		...
		echo "load_drop_down( 'requires/chemical_dyes_issue_controller', '".$row[csf("lc_company")]."', 'load_drop_down_division','division_td');\n";
		echo "document.getElementById('cbo_division_name').value			= '".$row[csf("division_id")]."';\n";
		echo "load_drop_down( 'requires/chemical_dyes_issue_controller', document.getElementById('cbo_division_name').value, 'load_drop_down_department','department_td');\n";
		echo "document.getElementById('cbo_department_name').value		= '".$row[csf("department_id")]."';\n";
		echo "load_drop_down( 'requires/chemical_dyes_issue_controller', document.getElementById('cbo_department_name').value, 'load_drop_down_section','section_td');\n";
       ...
       // echo "document.getElementById('txt_tot_liquor').value = '".$liquior_arr[$row[csf("lap_dip_no")]]."';\n"; 
		...
		if(str_replace("'","",$row[csf("is_posted_account")])==1)
		{
			...
			echo "disable_enable_fields( 'cbo_company_name*cbo_issue_basis*txt_issue_date*txt_challan_no*cbo_dying_source*cbo_sub_process*cbo_dying_company*txt_req_no', 1, '', '' );\n"; // disable true
...
if($action=="chemical_dyes_issue_print_single_com")
{
 ...
 $sql_dtls = "select b.id, a.issue_number, b.store_id, b.cons_uom, b.cons_quantity, b.cons_amount, b.machine_category, b.machine_id, b.prod_id, b.location_id, b.department_id, b.section_id, b.cons_rate, b.cons_amount, c.item_description, c.item_group_id, c.sub_group_name, c.item_size, d.sub_process, d.item_category, d.dose_base, d.ratio, d.recipe_qnty, d.adjust_percent, d.adjust_type, d.required_qnty, d.req_qny_edit, b.batch_lot
	  from inv_issue_master a, inv_transaction b, product_details_master c, dyes_chem_issue_dtls d
	  where a.id=d.mst_id and b.id =d.trans_id and d.product_id=c.id  and b.transaction_type=2 and a.entry_form=5 and b.item_category in (5,6,7,23) and d.mst_id=$data[1] and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active in(1,3) and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 
	  order by d.sub_process "; 
	 // echo $sql_dtls;die;
	  $sql_result= sql_select($sql_dtls);
	  $i=1;
