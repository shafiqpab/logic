...
{
	load_room_rack_self_bin("requires/chemical_dyes_transfer_acknowledge_controller",$data);
}

if ($action=="load_drop_down_location_to")
{ 
	$data=explode("_",$data);
	echo create_drop_down( "cbo_location_to", 160, "select id,location_name from lib_location where company_id='$data[0]' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", 0, "load_drop_down( 'requires/chemical_dyes_transfer_acknowledge_controller',$data[0]+'_'+this.value, 'load_drop_down_store_to', 'to_store_td' );" );	
	exit();
}
...
{
	$data=explode("_",$data);
	echo create_drop_down( "cbo_location_to", 160, "select id,location_name from lib_location where company_id='$data[0]' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", 0, "load_drop_down( 'chemical_dyes_transfer_acknowledge_controller',$data[0]+'_'+this.value, 'load_drop_down_store_to', 'to_store_td' );" );
	exit();
}
...
if ($action=="load_drop_down_store_to")
{
	...
	echo create_drop_down( "cbo_store_name_to", 160, "select a.id, a.store_name from lib_store_location a, lib_store_location_category b where a.id=b.store_location_id and a.company_id=$company and a.location_id=$location and a.status_active=1 and a.is_deleted=0 and b.category_type in(5,6,7,22,23) $store_location_credential_cond group by a.id, a.store_name order by a.store_name","id,store_name", 1, "-- Select Store --", 0, "fnc_floor_load(this.value+'_'+$company+'_'+$location);reset_room_rack_shelf('','cbo_store_name_to');" );
	exit();
}
...
if ($action=="load_drop_down_floor_to")
{
	...
	$location=$data[2];
	$incrementId=$data[3];
	echo create_drop_down( "cbo_floor_to_$incrementId", 152, "select b.floor_id,a.floor_room_rack_name from lib_floor_room_rack_mst a,lib_floor_room_rack_dtls b where  a.floor_room_rack_id=b.floor_id and b.store_id='$store' and a.company_id='$company' and b.location_id=$location and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0  group by b.floor_id,a.floor_room_rack_name  order by a.floor_room_rack_name", "floor_id,floor_room_rack_name", 1, "-- Select Floor --", 0, "load_drop_down( 'requires/chemical_dyes_transfer_acknowledge_controller',this.value+'_'+$incrementId+'_'+$company+'_'+$location+'_'+$store, 'load_drop_down_room_to', 'room_td_$incrementId' );reset_room_rack_shelf($incrementId,'cbo_floor_to');",0,"","","","","","","cboFloorTo[]" );
	exit();
}
...
if($action=='itemTransfer_popup')
{
	...
	where b.item_category in(5,6,7,22,23) and a.to_company=$company $store_cond and a.id=b.mst_id and a.entry_form=55 and a.transfer_criteria in(1,2) and b.is_acknowledge=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
...
if($action=='populate_data_from_transfer_master')
{
	...
	{ 
		//echo "load_drop_down('requires/chemical_dyes_transfer_acknowledge_controller','".$row[csf("to_company")]."', 'load_drop_down_location_to', 'to_location_td' );\n";
		...
		/*if($row[csf("to_store")]>0)
		{
			...
			echo "load_drop_down('requires/chemical_dyes_transfer_acknowledge_controller','".$loc_com."', 'load_drop_down_store_to', 'to_store_td' );\n";
			echo "document.getElementById('cbo_store_name_to').value 				= '".$row[csf("to_store")]."';\n";

			//echo "load_room_rack_self_bin('requires/chemical_dyes_transfer_acknowledge_controller*3*cbo_store_name_to', 'store','to_store_td', '".$row[csf("to_company")]."','".$row[csf("to_location_id")]."',this.value);\n";
			//echo "document.getElementById('cbo_store_name_to').value 				= '".$row[csf("to_store")]."';\n";
		}*/
...
if($action=="show_dtls_list_view")
{
	...
	foreach($data_array as $row)
	{
        ...
        <tr id="dtlsTbodyTr_<? echo $i; ?>">
            ...
            <td>
                <? echo create_drop_down( "cbo_item_category_$i", 120, $item_category,'', 0, "", $row[csf('item_category')], "",1,"5,6,7,22,23","","","","","","cboItemCategory[]" ); ?>
            ...
            <td id="floor_td_to_<? echo $i;?>">
                <? 
                	echo create_drop_down( "cbo_floor_to_$i", 120,$floor_arr,"", 1, "--Select--",$row[csf('to_floor_id')], "load_drop_down( 'requires/chemical_dyes_transfer_acknowledge_controller',this.value+'_'+$i+'_'+$company+'_'+$location+'_'+$store, 'load_drop_down_room_to', 'room_td_$i' );reset_room_rack_shelf($i,'cbo_floor_to');",0,"","","","","","","cboFloorTo[]" );
                ?>
            </td>
...
{ 
	...
	$con = connect();
	$trans_issue_date=strtotime(return_field_value("transfer_date","inv_item_transfer_mst","status_active=1 and id=$challan_id","transfer_date"));
	$trans_date=strtotime(str_replace("'","",$txt_transfer_date));
	if($trans_issue_date>$trans_date)
	{
		echo "20**Transfer Receive Date Not Allow Less Then Transfer Issue Date";disconnect($con);die;
	}
	
	$sql_variable_lot = sql_select("select auto_transfer_rcv, id from variable_settings_inventory where company_name = $cbo_company_id_to and variable_list = 29 and is_deleted = 0 and status_active = 1");
	$variable_lot=$sql_variable_lot[0][csf("auto_transfer_rcv")];
	...
	$sql_store_rcv = sql_select("select ID, RATE, CONS_QTY AS CURRENT_STOCK, AMOUNT AS STOCK_VALUE, CATEGORY_ID, PROD_ID, LOT from inv_store_wise_qty_dtls where status_active=1 and prod_id in(".implode(",",$all_prod_id_arr).") and store_id=$cbo_store_name_to and company_id=$cbo_company_id_to");
	$store_data_all=array();
	foreach($sql_store_rcv as $val)
	{
		if($variable_lot==1) $dyes_lot=$val["LOT"]; else $dyes_lot="";
		$store_data_all[$val["CATEGORY_ID"]][$val["PROD_ID"]][$dyes_lot]["ID"]=$val["ID"];
		$store_data_all[$val["CATEGORY_ID"]][$val["PROD_ID"]][$dyes_lot]["RATE"]=$val["RATE"];
		$store_data_all[$val["CATEGORY_ID"]][$val["PROD_ID"]][$dyes_lot]["CURRENT_STOCK"]=$val["CURRENT_STOCK"];
		$store_data_all[$val["CATEGORY_ID"]][$val["PROD_ID"]][$dyes_lot]["STOCK_VALUE"]=$val["STOCK_VALUE"];
	}
	...
	if ($operation==0)  // Insert Here
	{
		...
		$data_array="(".$id.",464,".$challan_id.",".$cbo_company_id_to.",".$cbo_store_name_to.",".$cbo_location_to.",".$cbo_transfer_criteria.",".$txt_transfer_date.",'".$txt_remarks."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		for($i=1;$i<=$numRow;$i++)
		{
			...
			if($variable_lot==1) $dyes_lot=str_replace("'","",$$txtLot); else $dyes_lot="";
			
			if(str_replace("'","",$$hiddenTranRate)<=0 || str_replace("'","",$$hiddenTranRate)=="")
			{
			...
			$stock_value=$prod_data_arr[str_replace("'","",$$productID)]["STOCK_VALUE"];;
			$curr_stock_qnty=$stock_qnty+str_replace("'", '',$$hiddenTranQnty);
			$curr_stock_value=$stock_value+str_replace("'", '',$$hiddenTranValue);
			$curr_avg_rate=0;
			if($curr_stock_qnty >0 && $curr_stock_value>0) $curr_avg_rate=$curr_stock_value/$curr_stock_qnty;
			// last_purchased_qnty, current_stock, stock_value
			$prod_id_arr[]=str_replace("'","",$$productID);
			$data_array_prod_update[str_replace("'","",$$productID)]=explode("*",("'".number_format($curr_avg_rate,10,'.','')."'*".$$hiddenTranQnty."*".$curr_stock_qnty."*".number_format($curr_stock_value,8,'.','')."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
			
			
			$store_up_id_to=0;
			if($store_data_all[str_replace("'","",$$cboItemCategory)][str_replace("'","",$$productID)][$dyes_lot]["ID"]>0)
			{
				$store_up_id_to=$store_data_all[str_replace("'","",$$cboItemCategory)][str_replace("'","",$$productID)][$dyes_lot]["ID"];
				$store_presentStock	=$store_data_all[str_replace("'","",$$cboItemCategory)][str_replace("'","",$$productID)][$dyes_lot]["CURRENT_STOCK"];
				$store_presentStockValue =$store_data_all[str_replace("'","",$$cboItemCategory)][str_replace("'","",$$productID)][$dyes_lot]["STOCK_VALUE"];
			...
			else
			{
				...
				$data_array_store_rcv.= "(".$sdtlsid.",".$cbo_company_id_to.",".$cbo_store_name_to.",".$$cboItemCategory.",".$$productID.",".$$hiddenTranQnty.",'".number_format(str_replace("'","",$$hiddenStoreRate),10,'.','')."','".number_format(str_replace("'","",$$hiddenStoreValue),8,'.','')."',".$$hiddenTranQnty.",'".$_SESSION['logic_erp']['user_id']."','".$pc_date_time."','".$dyes_lot."',".$txt_transfer_date.",".$txt_transfer_date.")";
			}
			
		}
		
		$rID=$rID2=$rID3=$rID4=$rID5=$prod=$rID6=$storeInsID=true;
		...
		$rID5=sql_update("inv_item_transfer_mst",$field_array_mst_update,$data_array_mst_update,"id",$challan_id,0);
		...
		if(count($prod_id_arr)>0) // transfer_dtls table update
		{
			$prod=execute_query(bulk_update_sql_statement( "product_details_master", "id", $field_array_prod_update, $data_array_prod_update, $prod_id_arr ));
		} 
		...
		if(count($store_id_arr)>0) // store table update
		{
			//echo "10**".bulk_update_sql_statement( "inv_store_wise_qty_dtls", "id", $field_array_store, $data_array_store, $store_id_arr );oci_rollback($con);disconnect($con);die;
			$rID6=execute_query(bulk_update_sql_statement( "inv_store_wise_qty_dtls", "id", $field_array_store, $data_array_store, $store_id_arr ));
		}
		
		if($data_array_store_rcv!="")
		{
			//echo "10**insert into inv_store_wise_qty_dtls (".$field_array_store_rcv.") values ".$data_array_store_rcv;oci_rollback($con);disconnect($con);die;
			$storeInsID=sql_insert("inv_store_wise_qty_dtls",$field_array_store_rcv,$data_array_store_rcv,1); 
		}
		//echo "10**$rID=$rID2=$rID3=$rID4=$rID5=$prod=$rID6=$storeInsID";oci_rollback($con);disconnect($con);die;
		
		if($db_type==0)
		{
			if($rID && $rID2 && $rID3 && $rID4 && $rID5 && $prod && $rID6 && $storeInsID)
			{
				mysql_query("COMMIT");  
				echo "0**".$id;
			}
...
if($action=="show_dtls_list_view_update")
{
	...
	where a.id='$data' and b.id=c.dtls_id and a.challan_id=b.mst_id and d.id=b.mst_id and a.entry_form=386 and c.trans_type=5 and c.entry_form in(112,78) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0");*/
	...
	where a.id='$data' and a.challan_id=b.mst_id and d.id=b.mst_id and a.entry_form=464 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");	
	...
	?> 
		<script>		
			...
			function load_location()
			{
				...
				load_drop_down('chemical_dyes_transfer_acknowledge_controller',cbo_company_id_to, 'load_drop_down_location_to_up', 'to_location_td' );
			}
    	</script>
	</head>
...
{
	$data_array=sql_select("SELECT a.id, a.entry_form, a.challan_id, a.company_id, a.store_id, a.location_id, a.transfer_criteria, a.item_category, a.acknowledg_date, a.remarks, b.transfer_system_id from inv_item_trans_acknowledgement a, inv_item_transfer_mst b where a.id='$data' and a.challan_id=b.id and a.entry_form=464 and a.transfer_criteria in(1,2) and a.status_active=1 and a.is_deleted=0");
	foreach ($data_array as $row)
	{ 
		echo "load_drop_down('requires/chemical_dyes_transfer_acknowledge_controller','".$row[csf("company_id")]."', 'load_drop_down_location_to', 'to_location_td' );\n";
		...
		{
			echo "load_drop_down('requires/chemical_dyes_transfer_acknowledge_controller','".$row[csf("company_id")]."_".$row[csf("location_id")]."', 'load_drop_down_store_to', 'to_store_td' );\n";
