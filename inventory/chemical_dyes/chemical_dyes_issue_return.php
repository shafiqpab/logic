...
function open_returnpopup()
{
	...
	{ 
		...
		show_list_view(company+"**"+data[3],'populate_batch_no_data_update','item_description_list','requires/chemical_dyes_issue_return_controller','');
		var hidden_posted_account = $("#hidden_posted_account").val();
			
		show_list_view(data[3]+"**"+hidden_posted_account,'show_dtls_list_view','list_container_dyes','requires/chemical_dyes_issue_return_controller','');
...
function fnc_chemical_issue_return_entry(operation)
{
	if(operation==4)
	{
		...
		generate_report_file( $('#cbo_company_id').val()+'*'+$('#txt_return_no').val()+'*'+report_title+'*'+$('#update_id').val(),'issue_return_print','requires/chemical_dyes_issue_return_controller');
		return;
	}
	else if(operation==0 || operation==1 || operation==2)
	{
		...
		if(variable_lot==1)
		{
			if( form_validation('cbo_company_id*cbo_location*cbo_issue_basis*txt_return_date*cbo_store_name*txt_item_description*txt_lot*txt_return_qnty*txt_rate','Company *Location*Issue Basis*Return Date*Store*Item Description*Lot*Return Quantity*Rate')==false )
			{
				return;
			}
		}
		...
		{
			if( form_validation('cbo_company_id*cbo_location*cbo_issue_basis*txt_return_date*cbo_store_name*txt_item_description*txt_return_qnty*txt_rate','Company *Location*Issue Basis*Return Date*Store*Item Description*Return Quantity*Rate')==false )
			{
				return;
			}
		}
...
{	
	...
	{	
		...
		if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
		{
			...
			disable_enable_fields( 'cbo_company_id*cbo_issue_basis*txt_batch_name', 1, "", "" ); 
			disable_enable_fields('txt_item_description*cbo_store_name', 0, '', '' );
		}
		if(reponse[0]==50)
		{
		...
		show_list_view(reponse[2],'show_dtls_list_view','list_container_dyes','requires/chemical_dyes_issue_return_controller','');		
		set_button_status(0, permission, 'fnc_chemical_issue_return_entry',1,1);
...
{
	window.open("requires/chemical_dyes_issue_return_controller.php?data=" + data+'&action='+action, true );
}
...
function openmypage_batchNo()
	...
	function rcv_variable_check(company_id)
	{
		...
		var lots_variable=return_global_ajax_value( company_id, 'populate_data_lib_data', '', 'requires/chemical_dyes_issue_return_controller');
                ...
                <fieldset style="width:800px;">                                       
                    <table  width="100%" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                      ...
                      <tr>
                            ...
                            <td width="145">
                                <? 
                                    echo create_drop_down( "cbo_company_id", 140, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3)  $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "rcv_variable_check(this.value);load_drop_down( 'requires/chemical_dyes_issue_return_controller', this.value, 'load_drop_down_location', 'location_td' );" );
                                    //load_drop_down( 'requires/chemical_dyes_issue_return_controller', this.value, 'load_drop_down_store', 'store_td' );
                                ?>
                         ...
                         <fieldset style="width:400px;float:left; margin-left:5px">  
                                <legend>Display</legend>                                     
                                  <table  width="100%" cellspacing="2" cellpadding="0" border="0">
									...
									<tr>
										<td colspan="6" align="center" id="posted_account_td" style="max-width:100px; color:red; font-size:14px;">
										</td>												
									</tr>                              
                           </table>
                          </fieldset>
               	...
               	<table cellpadding="0" cellspacing="1" width="100%">
                	...
                	<tr>
                        <td height="18" colspan="6" align="center" valign="middle" class="button_container">
                             ...
                             <input type="hidden" id="distribution_method" readonly /><input type="hidden" id="update_id" name="update_id" value="" />
                             <!-- -->
