...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Dyes And Chemical Transfer Info","../../", 1, 1, '','',''); 
...
{
	reset_form('transferEntry_1','div_transfer_item_list','','','','variable_lot*txt_transfer_date');
...
function openmypage_systemId()
{
	...
	emailwindow.onclose=function()
	{
		...
		//alert(transfer_id)
		reset_form('transferEntry_1','div_transfer_item_list','','','','variable_lot*txt_transfer_date*txt_variable_status');
		...
		{
			show_list_view(hidden_req_id+"**"+transfer_id,'show_item_requisition_listview','div_item_requisition_listview','requires/chemical_dyes_transfer_controller','');
		}
...
function openmypage_itemDescription()
{
	...
	var page_link = 'requires/chemical_dyes_transfer_controller.php?cbo_company_id='+cbo_company_id+'&cbo_store_name='+cbo_store_name+'&cbo_item_category='+cbo_item_category+'&variable_lot='+variable_lot+'&action=itemDescription_popup';
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=950px,height=370px,center=1,resize=1,scrolling=0','../');
	emailwindow.onclose=function()
	{
		var theform=this.contentDoc.forms[0];//("search_order_frm"); //Access the form inside the modal window
		var product_id=this.contentDoc.getElementById("product_id").value.split("__"); //Access form field with id="emailfield"
...
{	
	...
	else if(operation==0 || operation==1 || operation==2)
	{
		...
		if(variable_lot==1)
		{
			if( form_validation('cbo_transfer_criteria*cbo_company_id*cbo_company_id_to*txt_transfer_date*cbo_store_name*cbo_store_name_to*txt_item_desc*txt_transfer_qnty*cbo_item_category*txt_yarn_lot*txt_rate','Transfer Criteria*Company*To Company*Transfer Date*From Store*To Store*Item Description*Transfered Qnty*Item Category*Lot*Rate')==false )
			{
				return;
			}
		}
		...
		{
			if( form_validation('cbo_transfer_criteria*cbo_company_id*cbo_company_id_to*txt_transfer_date*cbo_store_name*cbo_store_name_to*txt_item_desc*txt_transfer_qnty*cbo_item_category*txt_rate','Transfer Criteria*Company*To Company*Transfer Date*From Store*To Store*Item Description*Transfered Qnty*Item Category*Rate')==false )
			{
				return;
			}
		}
		...
		var dataString = "txt_system_id*cbo_transfer_criteria*cbo_company_id*cbo_company_id_to*txt_transfer_date*txt_challan_no*cbo_item_category*cbo_store_name*cbo_floor*cbo_room*txt_rack*txt_shelf*cbo_store_name_to*cbo_floor_to*cbo_room_to*txt_rack_to*txt_shelf_to*txt_yarn_lot*txt_transfer_qnty*txt_rate*txt_transfer_value*cbo_uom*update_id*hidden_product_id*update_dtls_id*update_trans_issue_id*update_trans_recv_id*previous_from_prod_id*previous_to_prod_id*hidden_transfer_qnty*txt_item_desc*variable_lot*txt_requisition_no*hidden_req_id*hidden_req_dtls_id*txt_variable_status";		
...
{	
	...
	{	  		
		...
		if(reponse[0]==0 || reponse[0]==1)
		{
			...
			reset_form('transferEntry_1','','','','','update_id*txt_system_id*cbo_transfer_criteria*cbo_company_id*cbo_company_id_to*txt_transfer_date*txt_challan_no*variable_lot*txt_requisition_no*txt_variable_status*hidden_req_id*cbo_store_name*cbo_store_name_to');
			show_list_view(reponse[1],'show_transfer_listview','div_transfer_item_list','requires/chemical_dyes_transfer_controller','');
			var hidden_req_id = $('#hidden_req_id').val();
			if(hidden_req_id!="" && hidden_req_id>0)
			{
				show_list_view(hidden_req_id,'show_item_requisition_listview','div_item_requisition_listview','requires/chemical_dyes_transfer_controller','');
			}
...
function rcv_variable_check(str)
{
	...
	var company_id_to=$('#cbo_company_id_to').val();
	
	if(company_id>0)
	{
		if(str==1)
		{
			reset_form('transferEntry_1','div_transfer_item_list','','',"",'cbo_company_id*cbo_transfer_criteria*variable_lot*txt_transfer_date');
		}
		else
		{
			reset_form('transferEntry_1','div_transfer_item_list','','',"",'cbo_company_id*cbo_company_id_to*cbo_transfer_criteria*variable_lot*txt_transfer_date');
		}
		...
		var lib_variable=return_global_ajax_value( company_id, 'populate_data_lib_data', '', 'requires/chemical_dyes_transfer_controller');
		var lib_variable_arr=lib_variable.split("__");
		//alert(lib_variable_arr[0]+"="+lib_variable_arr[1]);return;
		$('#variable_lot').val(lib_variable_arr[0]);
		if(lib_variable_arr[0]==1)
		{
		...
		$('#store_update_upto').val(lib_variable_arr[1]);
		if($("#cbo_transfer_criteria").val() != 1 && company_id_to)
		{
			$('#store_update_upto_to').val(lib_variable_arr[1]);
		}
		//alert(lib_variable_arr[2]);
		if(lib_variable_arr[2]==1)
		{
			$('#txt_variable_status').val(lib_variable_arr[2]);
...
function chk_issue_requisition_variable(company)
{
   ...
   var status_data = return_global_ajax_value(company, 'chk_issue_requisition_variable', '', 'requires/chemical_dyes_transfer_controller').trim();
   if(status_data== 1)
   {
...
function fnc_items_sys_popup()
{
    ...
    var page_link='requires/chemical_dyes_transfer_controller.php?cbo_company_id='+cbo_company_id+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&action=item_requisition_popup_search&cbo_company_id_to='+cbo_company_id_to;
    ...
    emailwindow.onclose=function()
    {
        ...
        if(trim(requisition_info)!="")
        {
			...
			if(data[2]!=0 && data[2]!='')
			{
				load_drop_down('requires/chemical_dyes_transfer_controller', data[2], 'load_drop_down_store','store_td_to');
			}
			show_list_view(data[0],'show_item_requisition_listview','div_item_requisition_listview','requires/chemical_dyes_transfer_controller','');
        }
        release_freezing();
    }
}
...
<div style="width:100%;" align="center">
    ...
    <div style="width:880px; float: left;position: relative;">   
        ...
        <legend>Chemical Transfer Entry</legend>
        <br>
        	<fieldset style="width:880px;">
                <table width="880" cellspacing="2" cellpadding="2" border="0" id="tbl_master">
                    ...
                    <tr>
                        ...
                        <td>
                            <?  //load_room_rack_self_bin('requires/chemical_dyes_transfer_controller*5_6_7_22_23', 'store','from_store_td', this.value);load_room_rack_self_bin('requires/chemical_dyes_transfer_controller*5_6_7_22_23*cbo_store_name_to', 'store','to_store_td', this.value);                       
								...
								company_onchange(this.value);chk_issue_requisition_variable(this.value);
								*/
								//echo create_drop_down( "cbo_company_id", 160, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond $company_credential_cond order by comp.company_name","id,company_name", 1, "--Select Company--", 0, "load_room_rack_self_bin('requires/chemical_dyes_transfer_controller*5_6_7_22_23', 'store','from_store_td', this.value);reset_on_change(this.id);load_drop_down('requires/chemical_dyes_transfer_controller', this.value, 'load_drop_down_store','store_td_to');rcv_variable_check(1);" );
								echo create_drop_down( "cbo_company_id", 160, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond $company_credential_cond order by comp.company_name","id,company_name", 1, "--Select Company--", 0, "load_room_rack_self_bin('requires/chemical_dyes_transfer_controller*5_6_7_22_23', 'store','from_store_td', this.value);reset_on_change(this.id);load_drop_down('requires/chemical_dyes_transfer_controller', this.value, 'load_drop_down_store','store_td_to');rcv_variable_check(1);" );
                        ...
                        <td>
                            ...
                            //load_room_rack_self_bin('requires/chemical_dyes_transfer_controller*5_6_7_22_23*cbo_store_name_to', 'store','to_store_td', this.value);
								...
								//echo "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond $company_credential_cond order by comp.company_name"; to_company_on_change(this.value);

								echo create_drop_down( "cbo_company_id_to", 160, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "--Select Company--", 0, "reset_on_change(this.id);load_drop_down('requires/chemical_dyes_transfer_controller', this.value, 'load_drop_down_store','store_td_to');rcv_variable_check(2);",1 );
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" name="txt_transfer_date" id="txt_transfer_date" class="datepicker" style="width:148px;" readonly placeholder="Select Date" value="<?= date("d-m-Y")?>" />
                        </td>
                        <td>Challan No.</td>
                        <td>
            ...
            <br>
            <table width="880" cellspacing="2" cellpadding="2" border="0" id="tbl_dtls">
                <tr>
                            ...
                            <table id="tbl_item_info"  cellpadding="0" cellspacing="1" width="55%" style="float: left;">										
                               <!--  <tr>
                                ...
                                <tr>
                                    ...
                                    <td>                       
                                        <input type="text" name="txt_yarn_lot_dis" id="txt_yarn_lot_dis" class="text_boxes" style="width:150px" disabled="disabled" />
                                    </td>
                                </tr>
                                <tr>
					...
					<td width="40%" valign="top">
                        ...
                        <legend>Display</legend>					
                            <table id="tbl_display_info"  cellpadding="0" cellspacing="1" width="100%" >				
                                ...
                                <tr>
                                    <td class="must_entry_caption">Avg. Rate</td>						
                ...
                <tr>
                    <td align="center" colspan="3" class="button_container" width="100%">
                        ...
                        <input type="hidden" name="txt_yarn_lot" id="txt_yarn_lot" />
                    </td>
                </tr>
            </table>
