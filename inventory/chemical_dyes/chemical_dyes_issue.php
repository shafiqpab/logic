...
<script>
	...
	function fnc_chemical_dyes_issue_entry(operation)
	{
		...
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			{
				if( form_validation('cbo_dying_company*cbo_company_name*cbo_issue_basis*txt_issue_date*cbo_sub_process*cbo_store_name*cbo_issue_purpose*cbo_loan_party','Company Name*LC Company*Issue Basis*Issue Date*Sub-Process*Store Name*Issue Purpose*Loan Party')==false )
				{
					return;
				}
			}
			...
			{
				if( form_validation('cbo_dying_company*cbo_company_name*cbo_issue_basis*txt_issue_date*cbo_sub_process*cbo_store_name*cbo_issue_purpose','Company Name*LC Company*Issue Basis*Issue Date*Sub-Process*Store Name*Issue Purpose')==false )
				{
					return;
				}	
			}  
			...
			if(cbo_issue_purpose == 1 || cbo_issue_purpose == 5 || cbo_issue_purpose == 15 || cbo_issue_purpose == 61){
	...
	function open_reqpopup()
	{
		...
		emailwindow.onclose=function()
		{
			...
			var receipeNo=this.contentDoc.getElementById("hidden_receipe_no").value; // recipe no
			...
			if(issue_basis==7 && issue_purpose==56)
			{
				...
				show_list_view(reqId+'**'+company+"**"+issue_purpose+"**"+req_store_id+"**"+update_id+"**"+variable_lot, 'machine_item_details', 'list_container_recipe_items', 'requires/chemical_dyes_issue_controller', '');
			...
			{
				$("#cbo_store_name").attr('disabled',true);
				get_php_form_data(company+"**"+batchId+"**"+receipeId+"**"+document.getElementById('cbo_issue_purpose').value, "populate_batch_receipe_data", "requires/chemical_dyes_issue_controller");
				get_php_form_data(company+"**"+batchId+"**"+receipeId+"**"+document.getElementById('cbo_issue_purpose').value, "populate_buyer_order_no_data", "requires/chemical_dyes_issue_controller");
				load_drop_down( 'requires/chemical_dyes_issue_controller', company+"**"+receipeId+"**"+receipeId+"**"+document.getElementById('cbo_issue_purpose').value, 'load_drop_down_sub_process', 'sub_process_td');
			}
	...
	function open_mrrpopup()
	{
		...
		emailwindow.onclose=function()
		{
			...
			if(mrrNumber[2]==7 && mrrNumber[3]==56)
			{
				document.getElementById("list_container_recipe_items").innerHTML = "";
				document.getElementById("recipe_items_list_view").innerHTML = "";
				var cbo_store_name="";
	...
	{ 
		...
		if(id==1 || id==5 || id==15 || id==61){
			$("#for_division").css({"color":"black"});
			$("#for_department").css({"color":"black"});
			$("#for_section").css({"color":"black"});
		} 
		else{
			$("#for_division").css({"color":"blue"});
			$("#for_department").css({"color":"blue"});
			$("#for_section").css({"color":"blue"});
		}
	}
	...
	{
		var com_all_data = return_global_ajax_value(com_id, 'com_wise_all_data', '', 'requires/chemical_dyes_issue_controller');
		...
		/*if(lots_variable==1)
		{
			load_drop_down( 'requires/chemical_dyes_issue_controller', this.value, 'load_drop_down_location', 'location_td');load_drop_down( 'requires/chemical_dyes_issue_controller', this.value, 'load_drop_down_loan_party', 'loan_party_td');load_drop_down( 'requires/chemical_dyes_issue_controller', this.value, 'load_drop_down_store', 'store_td' );get_php_form_data( this.value, 'company_wise_report_button_setting','requires/chemical_dyes_issue_controller');
                            ...
                            <fieldset style="width:950px;">                                       
                                <table  width="950" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                                    ...
                                    <tr>
                                        ...
                                        <?
                                        echo create_drop_down( "cbo_dying_company", 170, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_credential_cond $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "company_onchange(this.value);" ,1);
										?>
									 ...
									 <tr>
                                        ...
                                        echo create_drop_down( "cbo_company_name", 170, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond $company_credential_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/chemical_dyes_issue_controller', this.value, 'load_drop_down_division','division_td');" );
                                        ?>
                                        </td>
                                    ...
                                    <tr> 
                                        ...
                                        echo create_drop_down( "cbo_issue_purpose", 170, $general_issue_purpose,"", 1, "-- Select Purpose --", "", "check_machine_wash(this.value);", "","1,2,3,5,8,11,15,18,37,38,43,44,49,53,54,56,57,60,61,63,64,65,66,69,71,74,79,84,87,88,89,90,91,92,96,97,98,99");
									...
									<tr>                                    
                                        <td id="for_division" width="130" align="right">For Division</td>
                                        <td width="170" id="division_td" >
                                        <?                                         
                                       	echo create_drop_down( "cbo_division_name", 170, $blank_array,"", 1, "-- Select --", $selected, "" );
                                        ?>
                                        </td>

										<td id="for_department" width="130" align="right">For Department</td>
                                        <td width="170" id="department_td" >
                                        <?                                         
                                       	echo create_drop_down( "cbo_department_name", 170,$blank_array,"", 1, "-- Select --", $selected, "" );
                                        ?>
                                        </td>

										<td id="for_section" width="130" align="right">For Section</td>
