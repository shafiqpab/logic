...
<script>
	...
	{
		col_operation: {
		id: ["value_opening_stock","value_rcv_qnty","value_inside_iss_return","value_out_iss_return","value_trans_in","value_total_rcv","value_total_receive_amount","value_total_cutting_inside","value_total_cutting_outside","value_total_other_issue","value_total_rcv_return","value_total_transfer_out","value_total_issue","value_issue_amount","value_stock_qnty","value_stock_amount"],
		//col: [25,26,27,28,29,30,32,33,34,35,36,37,38,40,41,43],
		col: [26,27,28,29,30,31,33,34,35,36,37,38,39,41,42,44],
	...
	/*var tableFilters_2 =
	{
		...
		col: [32,35,36,37,38,39,40,50,52,53,55],
	...
	function generate_report(type)
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_store_name*cbo_buyer_id*txt_book_no*txt_book_id*cbo_year*txt_job_no*txt_job_id*txt_pi_no*hdn_pi_id*cbo_pay_mode*cbo_supplier_id*cbo_value_with*cbo_get_upto*txt_days*cbo_get_upto_qnty*txt_qnty*txt_date_from*txt_date_to*txt_batch_no',"../../../")+'&report_title='+report_title+'&cbo_report_type='+type;

		freeze_window(3);
		http.open("POST","requires/batch_and_store_wise_finish_fabric_stock_report_controller.php",true);
	...
	function openmypage_job()
	{
		...
		var page_link='requires/batch_and_store_wise_finish_fabric_stock_report_controller.php?action=job_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id;
    ...
    function openmypage_pinumber()
    {
        ...
        var page_link='requires/batch_and_store_wise_finish_fabric_stock_report_controller.php?action=pinumber_popup&companyID='+companyID;
	...
	function openmypage_booking()
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/batch_and_store_wise_finish_fabric_stock_report_controller.php?action=booking_no_popup&data='+data,'Booking No Popup', 'width=1050px,height=420px,center=1,resize=0','../../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0];
	...
	function openmypage(po_id,body_part_id,color,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/batch_and_store_wise_finish_fabric_stock_report_controller.php?companyID='+companyID+'&po_id='+po_id+'&body_part_id='+body_part_id+'&color='+color+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../../');
	}

	function open_po_number(po_id)
	{
		var popup_width='600px';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/batch_and_store_wise_finish_fabric_stock_report_controller.php?po_id='+po_id+'&action=open_po_number', 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../../');
	}
	
	function openmypage_qnty(booking_no,prod_ref,action,from_date,to_date)
	{
		var companyID = $("#cbo_company_id").val();
		var popup_width='600px';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/batch_and_store_wise_finish_fabric_stock_report_controller.php?companyID='+companyID+'&booking_no='+booking_no+'&prod_ref='+prod_ref+'&action='+action+'&from_date='+from_date+'&to_date='+to_date, 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../../');
	}

	function getCompanyId() 
	{
	    var company_id = $("#cbo_company_id").val();
	    load_drop_down( 'requires/batch_and_store_wise_finish_fabric_stock_report_controller',company_id, 'load_drop_down_store', 'store_td' );
	    set_multiselect('cbo_store_name','0','0','','0');
	    load_drop_down( 'requires/batch_and_store_wise_finish_fabric_stock_report_controller',company_id, 'load_drop_down_buyer', 'buyer_td' );       
	}
	...
	function generate_report_exel_only(excl_no)
	{
		if(excl_no==1)
		{
			...
			var report_title=$( "div.form_caption" ).html();
			var data="action=report_generate_excel_only"+get_submitted_data_string('cbo_company_id*cbo_store_name*cbo_buyer_id*txt_book_no*txt_book_id*cbo_year*txt_job_no*txt_job_id*txt_pi_no*hdn_pi_id*cbo_pay_mode*cbo_supplier_id*cbo_value_with*cbo_get_upto*txt_days*cbo_get_upto_qnty*txt_qnty*txt_date_from*txt_date_to*txt_batch_no',"../../../")+'&report_title='+report_title+'&cbo_report_type='+excl_no;
...
<div style="width:100%;" align="center">
    ...
    <h3 style="width:1625px; margin-top:20px;" align="" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel- </h3>
        <div id="content_search_panel" style="width:100%;" align="center">
            <fieldset style="width:1625px;">
                <table class="rpt_table" width="1625" cellpadding="0" cellspacing="0" border="1" rules="all">
                    <thead>
                        <tr>
                    ...
                    <tr align="center">
                        <td id="company_td">
                            <?
                               echo create_drop_down( "cbo_company_id", 130, "SELECT comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "getCompanyId();" );
                            ?>
                        </td>
                        <td id="store_td">
                            <?
	                    ...
	                    <td>
	                    <?
	                    echo create_drop_down( "cbo_pay_mode", 100, $pay_mode,"", 1, "-- Select Pay Mode --", 0, "load_drop_down( 'requires/batch_and_store_wise_finish_fabric_stock_report_controller', this.value + '_' + document.getElementById('cbo_company_id').value, 'load_drop_down_supplier', 'supplier_td' )","","1,2,3,5" );
	                    ?> 
	                    </td>
                        <td id="supplier_td">
                            <?
                	...
                	<tfoot>
						<tr>
							...
							<td align="center">
                            <input type="button" name="search" id="search" value="Excel Only" onClick="generate_report_exel_only(1)" style="width:70px" class="formbutton" />
