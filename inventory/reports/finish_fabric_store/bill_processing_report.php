<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Bill Processing Report.
Functionality	:
JS Functions	:
Created by		:	Kaiyum
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Bill Processing Report", "../../../", 1, 1,'',1,1);
?>
<script>
	...
	function fn_report_generated(type)
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_within_group*cbo_lccompany_id*cbo_buyer_id*txt_fso_no*hdn_fso_id*txt_date_from*txt_date_to',"../../../")+'&report_title='+report_title;
		freeze_window(3);
		http.open("POST","requires/bill_processing_report_controller.php",true);
	...
	function active_inactive(data)
	{
		
		if(data==1)
		{
			$('#cbo_lccompany_id').removeAttr("disabled", "false");
		}
		else{	
			$('#cbo_lccompany_id').val(0);
			$('#cbo_lccompany_id').attr("disabled", "disabled");
		}
	}
	...
	function openmypage_fsoNo()
	{
		...
		{ 	
			...
			var page_link = 'requires/bill_processing_report_controller.php?cbo_company_id='+cbo_company_id+'&action=fsoNo_popup';
    	...
    	<fieldset style="width:830px;">
            <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                <thead>
                    ...
                    <th>Buyer Name</th>
                    <th>FSOE</th>
                    <th colspan="2" class="must_entry_caption">Bill Date Range</th>
                ...
                <tbody>
                    <tr align="center" class="general">
                        <td><? echo create_drop_down( "cbo_company_id", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-Select Company-", $selected, "load_drop_down( 'requires/bill_processing_report_controller', document.getElementById('cbo_within_group').value+'_'+this.value, 'load_drop_down_buyer_working', 'buyer_td' );active_inactive(document.getElementById('cbo_within_group').value);" ); ?>
                        </td>
                        <td>
                        	<?
                        	echo create_drop_down( "cbo_within_group", 70, array("--Select--","Yes","No"),"", 0, "--Select--", 1, "load_drop_down( 'requires/bill_processing_report_controller', this.value+'_'+document.getElementById('cbo_company_id').value+'_'+'_80', 'load_drop_down_buyer_working', 'buyer_td' );active_inactive(this.value);" );
                        	?>
                        </td>
                        <td id="pocompany_td"><?
                        	echo create_drop_down( "cbo_lccompany_id", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-LC Company-", $selected, "load_drop_down( 'requires/bill_processing_report_controller', this.value+'_'+document.getElementById('cbo_within_group').value, 'load_drop_down_buyer', 'buyer_td' );" );?>
                        </td>
                        <td id="buyer_td"><? echo create_drop_down( "cbo_buyer_id", 120, $blank_array,"", 1, "-All Buyer-", $selected, "",0,"" ); ?></td>
                       
                        <td>
                            <input type="text" name="txt_fso_no" id="txt_fso_no" class="text_boxes" style="width:80px;" placeholder="Double Click" onDblClick="openmypage_fsoNo()" readonly/>
