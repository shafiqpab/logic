...
<script>
	...
	function generate_report(type)
	{
		...
		freeze_window(3);
		http.open("POST","requires/inventory_stock_ageing_urmi_report_controller.php",true);
	...
	function openmypage_job()
	{
		...
		var page_link='requires/inventory_stock_ageing_urmi_report_controller.php?action=job_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id;
    ...
    function openmypage_pinumber()
    {
        ...
        var page_link='requires/inventory_stock_ageing_urmi_report_controller.php?action=pinumber_popup&companyID='+companyID;
	...
	function openmypage_booking()
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/inventory_stock_ageing_urmi_report_controller.php?action=booking_no_popup&data='+data,'Booking No Popup', 'width=1050px,height=420px,center=1,resize=0','../../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0];
	...
	function openmypage(po_id,body_part_id,color,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/inventory_stock_ageing_urmi_report_controller.php?companyID='+companyID+'&po_id='+po_id+'&body_part_id='+body_part_id+'&color='+color+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../../');
	}

	function open_po_number(po_id)
	{
		var popup_width='600px';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/inventory_stock_ageing_urmi_report_controller.php?po_id='+po_id+'&action=open_po_number', 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../../');
	}
	...
	function openmypage_qnty(booking_no,prod_ref,action,from_date)
	{
		var companyID = $("#cbo_company_id").val();
		var popup_width='600px';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/inventory_stock_ageing_urmi_report_controller.php?companyID='+companyID+'&booking_no='+booking_no+'&prod_ref='+prod_ref+'&action='+action+'&from_date='+from_date, 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../../');
	}
	...
	function openmypage_qnty_without_gsmdia(booking_no,prod_ref,action,from_date,avg_rate)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/inventory_stock_ageing_urmi_report_controller.php?companyID='+companyID+'&booking_no='+booking_no+'&prod_ref='+prod_ref+'&action='+action+'&from_date='+from_date+'&avg_rate='+avg_rate, 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../../');
	}
	...
	function getCompanyId() 
	{
	    ...
	    load_drop_down( 'requires/inventory_stock_ageing_urmi_report_controller',company_id, 'load_drop_down_store', 'store_td' );
	    set_multiselect('cbo_store_name','0','0','','0');
	    load_drop_down( 'requires/inventory_stock_ageing_urmi_report_controller',company_id, 'load_drop_down_buyer', 'buyer_td' );       
	}
            ...
            <fieldset style="width:1550px;">
                <table class="rpt_table" width="1650" cellpadding="0" cellspacing="0" border="1" rules="all">
                    ...
                    <tr align="center">
                        <td id="company_td">
                            <?
                               echo create_drop_down( "cbo_company_id", 130, "SELECT comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/inventory_stock_ageing_urmi_report_controller',this.value, 'load_drop_down_store', 'store_td' );load_drop_down( 'requires/inventory_stock_ageing_urmi_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>
                        </td>
                        <td id="store_td">
                            <?
	                    ...
	                    <td>
	                    <?
	                    echo create_drop_down( "cbo_pay_mode", 100, $pay_mode,"", 1, "-- Select Pay Mode --", 0, "load_drop_down( 'requires/inventory_stock_ageing_urmi_report_controller', this.value + '_' + document.getElementById('cbo_company_id').value, 'load_drop_down_supplier', 'supplier_td' )","","1,2,3,5" );
	                    ?> 
	                    </td>
                        <td id="supplier_td">
                            <?
