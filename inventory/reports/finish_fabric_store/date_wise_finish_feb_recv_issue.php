...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Date Wise Finish Fabric Rcv Issue Report","../../../", 1, 1, $unicode,1,''); 
?>	
<script>
	...
	var tableFilters2 = 
	{
		...
		col: [30,31,32,33,34,35,36,37,38,40,42],
		operation: ["sum","sum","sum","sum","sum","sum","sum","sum","sum","sum","sum"],
		write_method: ["innerHTML","innerHTML","innerHTML","innerHTML","innerHTML","innerHTML","innerHTML","innerHTML","innerHTML","innerHTML","innerHTML"]
		}
	}
	...
	function generate_report(rpt_type)
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_buyer_id*cbo_store_id*cbo_year*txt_job_no*txt_job_id*txt_order_no*txt_order_id*txt_booking_no*txt_booking_id*txt_pi_no*txt_pi_id*cbo_based_on*txt_date_from*txt_date_to*txt_exchange_rate*cbo_category_type',"../../../")+'&report_title='+report_title+'&rpt_type='+rpt_type;
		//alert (data);return;
		freeze_window(3);
		http.open("POST","requires/date_wise_finish_feb_recv_issue_controller.php",true);
	...
	function openmypage_job(search_type)
	{
		...
		var page_link='requires/date_wise_finish_feb_recv_issue_controller.php?action=job_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id+'&search_type='+search_type;
	...
	{
		var cbo_company_id=val;
		var recv_date = $('#txt_date_from').val();
		cbo_company_id=cbo_company_id.split(',');
		cbo_company_id=cbo_company_id[0];
		var toDay='<? echo date('d-m-Y'); ?>'
		//var cbo_company_id = $('#cbo_company_id').val(); 
		var response=return_global_ajax_value( 2+"**"+toDay+"**"+cbo_company_id, 'check_conversion_rate', '', 'requires/date_wise_finish_feb_recv_issue_controller');
	...
	function openmypage_pi()
	{
		...
		var companyID = $("#cbo_company_id").val();
		var cbo_year_id = $("#cbo_year").val();
		var page_link='requires/date_wise_finish_feb_recv_issue_controller.php?action=pi_no_popup&companyID='+companyID+'&cbo_year_id='+cbo_year_id;
	...
	{ 
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/date_wise_finish_feb_recv_issue_controller.php?po_id='+po_id+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../../');
	}
	...
	{	 
		...
		var cboCategoryBy = document.getElementById('cbo_category_type').value;
		load_drop_down( 'requires/date_wise_finish_feb_recv_issue_controller',company_id+'_'+1, 'load_drop_down_buyer', 'buyer_td' );
		load_drop_down( 'requires/date_wise_finish_feb_recv_issue_controller',company_id+'_'+cboCategoryBy, 'load_drop_down_stores', 'store_td' );
...
<div style="width:100%;" align="center">
    ...
    <h3 style="width:1430px; margin-top:20px;" align="" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3>
        <div id="content_search_panel" style="width:100%;" align="center">
            <fieldset style="width:1430px;">
                <table class="rpt_table" width="1430" cellpadding="0" cellspacing="0" border="1" rules="all">
                    <thead>
                        <tr> 	 	
                            <th width="140" class="must_entry_caption">Company</th>
                            <th width="140">Buyer</th> 
							<th width="100">Category By</th>
                    ...
                    <tr align="center" class="general">
                        <td id="td_company">
                            <? 
								//load_drop_down( 'requires/date_wise_finish_feb_recv_issue_controller',this.value, 'load_drop_down_store', 'store_td' );
                               echo create_drop_down( "cbo_company_id", 140, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 0, "", $selected, "get_php_form_data( this.value, 'company_wise_report_button_setting','requires/date_wise_finish_feb_recv_issue_controller');" );
                            ?>                            
                        </td>
                        <td id="buyer_td"> 
                            <?
						...
						<td>
                            <?
                                ...
                                echo create_drop_down( "cbo_category_type", 100, $report_arr, "", 1, "-- Select --", 1, "getStoreId();", "", "1,2");
                            ?>
                        </td>
                        <td id="store_td">
                            <?
                        ...
                        <td>
                            <input type="text" id="txt_pi_no" name="txt_pi_no" class="text_boxes" style="width:90px" onDblClick="openmypage_pi()"  placeholder="Browse/Write" />
                        ...
                        <td><input type="text" id="txt_exchange_rate" name="txt_exchange_rate" class="text_boxes_numeric" style="width:50px" disabled/></td>
                        <td>
                            <input type="button" name="search1" id="search1" value="Receive" onClick="generate_report(1)" style="width:55px" class="formbutton" />
                            <input type="button" name="search2" id="search2" value="Issue" onClick="generate_report(2)" style="width:55px" class="formbutton" />
                            <input type="button" name="search3" id="search3" value="Summary" onClick="generate_report(3)" style="width:55px" class="formbutton" />
                        </td>
                    </tr>
                    <tr>
