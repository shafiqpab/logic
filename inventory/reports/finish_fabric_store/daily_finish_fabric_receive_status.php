...
<script>
	...
	function generate_report(type)
	{
		...
		if(search_comm=='')
		{
			if( form_validation('cbo_company_id*txt_date_from*txt_date_to','Company Name*Date range*Date range')==false )
			{
				return;
			}
		}
		...
		if(type == "1")
		{
			var data="action=report_generate"+get_submitted_data_string('cbo_company_id*txt_date_from*txt_date_to*cbo_buyer_id*cbo_year*cbo_report_type*cbo_search_by*txt_search_comm*cbo_location*cbo_product_category*cbo_store_name',"../../../")+'&report_title='+report_title;
		}
		//alert (data);
		freeze_window(3);
		http.open("POST","requires/daily_finish_fabric_receive_status_controller.php",true);
    ...
    <h3 style="width:1230px; margin-top:20px;" align="" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3>
        <div id="content_search_panel" style="width:100%;" align="center">
            <fieldset style="width:1230px;">
                <table class="rpt_table" width="1220" cellpadding="0" cellspacing="0" border="1" rules="all">
                    <thead>
                        <tr>
                            ...
                            <th width="90" style="display: none;">Year</th>
                    ...
                    <tr align="center" class="general">
                        <td>
                            <?
                               echo create_drop_down( "cbo_company_id", 140, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/daily_finish_fabric_receive_status_controller',this.value+'_'+1+'_'+4, 'load_drop_down_buyer', 'buyer_td' );load_drop_down('requires/daily_finish_fabric_receive_status_controller', this.value, 'load_drop_down_location_lc', 'location_td' );load_drop_down( 'requires/daily_finish_fabric_receive_status_controller',this.value, 'load_drop_down_store', 'store_td' );" );
                            ?>
                        </td>
                        <td id="buyer_td">
                            <?
                        ...
                        <td>
                            <?
                                ...
                                echo create_drop_down( "cbo_report_type", 110, $report_arr, "", 0, "--  --", 0, "", "", "1,2");
                            ?>
                        </td>
                        <td id="location_td">
									<?
