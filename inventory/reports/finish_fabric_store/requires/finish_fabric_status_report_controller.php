...
if($action=="generated_report")
{
    ...
    if($hidden_booking_id == ""){
        $booking_no_cond = ($txt_booking_no != "") ? " and d.booking_no_prefix_num in('$txt_booking_no')" : "";
    }else{
        $booking_no_cond = " and d.id in($hidden_booking_id)";
    }
    ...
    if ($report_criteria==1) // Receive Issue and Stock
    {
        ...
        else
        {
            ...
            where a.entry_form in(68,37) and a.id=b.mst_id and b.id=c.trans_id and c.id=d.dtls_id and d.entry_form in(68,37) and d.status_active=1 and d.is_deleted=0 and a.company_id=$company $dates_cond $poIds_cond_roll and c.batch_id=e.id and c.prod_id=f.id and a.status_active=1 and c.status_active=1 
            ...
            and nvl(d.booking_without_order,0)=0 and a.company_id=$company $poIds_cond_roll $transfer_date 
            ...
            if(count($barcodeArr ) >0 ) // production
            {
                ...
                where a.id=b.dtls_id and a.mst_id = c.id and c.entry_form in(66,37) and b.entry_form in(66,37) and a.status_active=1 and b.status_active=1 and b.barcode_no = d.barcode_no and d.userid=$user_id");
            ...
            $poArr=array(); $poIds=''; $tot_rows=0; $fileRefArr=array();
            $po_sql="SELECT b.job_no, b.buyer_name, b.style_ref_no, a.id, a.po_number, c.booking_no, a.pub_shipment_date, c.dia_width, c.gsm_weight, c.fabric_color_id, c.booking_mst_id, d.body_part_id, d.lib_yarn_count_deter_id as deter_id,
            ...
            where b.id=a.job_id and a.id=c.po_break_down_id and b.job_no=c.job_no and c.job_no=d.job_no and c.pre_cost_fabric_cost_dtls_id=d.id  and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.company_name=$company $booking_type_cond $year_cond $job_no_cond $style_no_cond $order_no_cond  $all_po_id_cond $order_cond 
            group by b.job_no, b.buyer_name, b.style_ref_no, a.id, a.po_number, c.booking_no, d.uom, a.pub_shipment_date, c.dia_width, c.gsm_weight, c.fabric_color_id, c.booking_mst_id, d.body_part_id, d.lib_yarn_count_deter_id 
            order by b.job_no"; //$booking_no_cond
            $po_result=sql_select($po_sql);
            ...
            if(!empty($po_result))
            {
                ...
                $all_booking_id = implode(",",array_filter(array_unique($booking_id_arr)));
                if($all_booking_id=="") $all_booking_id=0;
                $bidCond = $all_booking_id_cond = "";                
                $all_booking_id_arr=explode(",",$all_booking_id);
                if($db_type==2 && count($all_booking_id_arr)>999)
                {
                    $all_po_id_chunk_arr=array_chunk($all_booking_id_arr,999) ;
                    foreach($all_po_id_chunk_arr as $chunk_arr)
                    {
                        $chunk_arr_value=implode(",",$chunk_arr);
                        $bidCond.=" a.booking_no_id in($chunk_arr_value) or ";
                    }
                    $all_booking_id_cond.=" and (".chop($bidCond,'or ').")";
                }
                else
                {
                    $all_booking_id_cond=" and a.booking_no_id in($all_booking_id)";
                }

                $batch_sql="SELECT a.booking_no, a.color_id, a.batch_no
                from pro_batch_create_mst a
                where a.status_active=1 and a.is_deleted=0 $all_booking_id_cond";
                $batch_sql_result=sql_select($batch_sql);
                $batch_no_arr=array();
                foreach ($batch_sql_result as $key => $value) 
                {
                    $batch_no_arr[$value[csf("booking_no")]][$value[csf("color_id")]].=$value[csf('batch_no')].',';
                }
                // echo '<pre>';print_r( $batch_no_arr);
            }
        ...
        <div align="left">
                ...
                <table class="rpt_table" width="4225" cellpadding="0" cellspacing="0" border="1" rules="all" align="left">
                    <thead>
                        <tr>
                            ...
                            <th width="100" rowspan="2" class="word_wrap_break">Batch No.</th>
                        ...
                        <tbody>
                            ...
                            foreach ($with_order_data_arr as $job_no_key => $job_no_val)
                            {
                                ...
                                foreach ($job_no_val as $detar_id_key => $detar_id_val)
                                {
                                    ...
                                    foreach ($detar_id_val as $color_id_key => $color_id_val)
                                    {
                                        ...
                                        foreach ($color_id_val as $str_ref => $row)
                                        {
                                            ...
                                            // echo  $booking_requQntyKg.'<br>';
                                            $colors = rtrim($colors, ", ");
                                            $batchNos=chop($batch_no_arr[$booking_no][$color_id_key],',');
                                            ...
                                            <tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')">
                                                ...
                                                <td width="100"><p><? echo $batchNos; ?></p></td>
                                ...
                                if($show_job_total)
                                {
                                    ...
                                    <!-- Job Total -->
                                    <tr class="tbl_bottom">
                                        <td width="100" align="right" colspan="17">Job Total</td>
            ...
            <!-- color RECEIVE summary END -->

            <table cellpadding="0"  width="3" cellspacing="0" align="left"  rules="all" border="1">
                <tr><td>&nbsp;&nbsp;&nbsp;&nbsp;</td></tr>
            </table>

            <!-- COLOR WISE ISSUE SUMMERY START-->
            <table cellpadding="0"  width="330" cellspacing="0" align="left"  class="rpt_table" rules="all" border="1">
                <thead>
                    <tr>
            ...
            <!-- COLOR WISE ISSUE SUMMERY END-->

            <table cellpadding="0"  width="3" cellspacing="0" align="left"  rules="all" border="1">
                <tr><td>&nbsp;&nbsp;&nbsp;&nbsp;</td></tr>
            </table>

            <!-- SUPPLIER WISE RECEIVE SUMMERY START -->
            <table cellpadding="0"  width="330" cellspacing="0" align="left"  class="rpt_table" rules="all" border="1">
                <thead>
                    <tr>
            ...
            <!-- SUPPLIER WISE RECEIVE SUMMERY END -->

            <table cellpadding="0"  width="3" cellspacing="0" align="left"  rules="all" border="1">
                <tr><td>&nbsp;&nbsp;&nbsp;&nbsp;</td></tr>
            </table>

            <!-- SUPPLIER WISE ISSUE SUMMERY START -->
            <table cellpadding="0"  width="330" cellspacing="0" align="left"  class="rpt_table" rules="all" border="1">
                <thead>
                    <tr>
            ...
            <!-- SUPPLIER WISE ISSUE SUMMERY END -->
            <!-- Summary End -->
            </fieldset>
        </div>
        <?
