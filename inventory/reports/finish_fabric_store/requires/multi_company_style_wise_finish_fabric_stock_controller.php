...
if($action=="report_generate_summery")
{
	...
	if($cbo_report_type==2) // Woven Finish Start
	{
		...
		*(case when d.entry_form in (258) and c.transaction_type=5 and d.trans_type=5 then c.cons_rate else 0 end) as transfer_in_amount,
		...
		and a.company_name in(".$cbo_company_id.") $receive_date $buyer_id_cond $search_cond $year_cond $storeCond $shiping_status_cond  $uom_cond 	
		...
		ob_start();
		?>
		<fieldset style="width:<? echo 540+$uom_th_width; ?>px;">
			...
			<table width="<? echo 450+$uom_th_widthStore; ?>" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table">
				<thead>
					...
					<tr>
						...
						{
							?>
							<th width="110" title="<? echo $values; ?>">Stock Qnty (<? echo $unit_of_measurement[$values]; ?>)</th>
			...
			<div style="width:<? echo 470+$uom_th_widthStore; ?>px; max-height:350px; overflow-y:scroll;" id="scroll_body">
				<table width="<? echo 450+$uom_th_widthStore; ?>" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table" id="table_body" >
					<?
					$i=1;
					...
					foreach ($style_wise_arr_store as $storeId => $val)
					{
						...
						foreach ($uomIdsStore as $uomIdsSto)
						{
							...
							//echo $total_recQntyStore[$storeId][$uomIdsSto]."<br/>";
							//echo $total_issQntyStore[$storeId][$uomIdsSto]."<br/>";
						...
						if($cbo_value_range_by==2 &&  number_format($totalStockValueStoreValRangeChk,2,'.','')>0.00)
						{
							...
							<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
								...
								<td width="150" title="<? echo $val[("store_id")]; ?>"><p><? echo $store_lib_arr[$val[("store_id")]]; ?></p></td>
								...
								foreach ($uomArrForStore as $key => $valueUOM) 
								{
									...
									?>
									<td align="right" width="110"><? //echo $stockStoreWise[$buyerIds][$uomID] ;
									//echo $stockStoreWise[$buyerId][$valueUOM];
									echo number_format($stockStoreWise[$storeId][$valueUOM],2);
									?></td>
									<td align="right" width="110"><? echo number_format($StockValueStoreWise[$storeId][$valueUOM],2); ?></td>
									<? 
								}
								?>
                    ...
                    <tfoot>
						...
						foreach ($uomArrForStore as $key => $valuesStore) 
						{
							...
							?>
								<th width="110" align="right"><? echo number_format($totalStockUomArrStore[$valuesStore],2); ?></th>
								<th width="110" align="right"><? echo number_format($totalStockValueUomArrStore[$valuesStore],2); ?></th>
							<?
						}
						?>	
			...
			<table width="<? echo 450+$uom_th_width; ?>" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table">
				<thead>
					<tr>
						<th width="30">SL</th>
			...
			<div style="width:<? echo 470+$uom_th_width; ?>px; max-height:350px; overflow-y:scroll;" id="scroll_body">
				<table width="<? echo 450+$uom_th_width; ?>" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table" id="table_body" >
					<?
					$i=1;
					...
					foreach ($style_wise_arr as $buyerId => $val)
					{
						...
						foreach ($uomIds as $uomIdss)
						{
							...
							//echo $buyerId."=".$uomIdss."<br/>";
							//echo $total_recQnty[$buyerId][$uomIdss]."<br/>";
							//echo $total_issQnty[$buyerId][$uomIdss]."<br/>";
						...
						if($cbo_value_range_by==2 &&  number_format($totalStockValueValRangeChk,2,'.','')>0.00)
						{
							...
							<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
								<td width="30"><? echo $i; ?></td>
								<td width="150"><p><? echo $buyer_arr[$val[("buyer_name")]]; ?></p></td>
								...
								foreach ($uomArr as $key => $value) 
								{
									...
									?>
									<td align="right" width="110"><? //echo $stock[$buyerIds][$uomID] ;
									//echo $stock[$buyerId][$value];
									echo number_format($stock[$buyerId][$value],2);
									?></td>
									<td align="right" width="110"><? echo number_format($StockValue[$buyerId][$value],2); ?></td>
									<? 
								}
								?>
						...
						else if($cbo_value_range_by==1 || $cbo_value_range_by==0)
						{
							...
							<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
								<td width="30"><? echo $i; ?></td>
								<td width="150" title="<? echo $val[("buyer_name")]; ?>"><p><? echo $buyer_arr[$val[("buyer_name")]]; ?></p></td>
                    ...
                    <tfoot>
						...
						foreach ($uomArr as $key => $valuess) 
						{
							...
							?>
								<th width="110" align="right"><? echo number_format($totalStockUomArr[$valuess],2); ?></th>
								<th width="110" align="right"><? echo number_format($totalStockValueUomArr[$valuess],2); ?></th>
							<?
						}
						?>	
...
if($action=="report_generate2")
{
 	...
 	if($cbo_report_type==2) // Woven Finish Start
	{
		...
		<fieldset style="width:<? echo $table_width+10; ?>px;">
			<table cellpadding="0" cellspacing="0" width="<? echo $table_width; ?>">
				...
				<tr  class="form_caption" style="border:none;">
					<td align="center" width="100%" colspan="<? echo $colspan; ?>" style="font-size:14px"><strong> <? if($date_from!="") echo "Upto : ".change_date_format(str_replace("'","",$txt_date_from)) ;?></strong></td>
				</tr>
			</table>
			<table width="<? echo $table_width; ?>" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table">
				<thead>
					<tr>  
						...
						<th width="220" rowspan="2">Fab. Desc.</th>
						<th width="50" rowspan="2">UOM</th>			
						<th width="150" rowspan="2" title="Total Rec.- Total Issue">Stock</th>
			...
			<div style="width:<? echo $table_width+20; ?>px; max-height:350px; overflow-y:scroll;" id="scroll_body">
				<table width="<? echo $table_width; ?>" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table" id="table_body" >
					...
					$con = connect();
					execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_id and entry_form=98 and ref_from in(1)");
					oci_commit($con);
					...
					if($db_type==0) $prod_cond="group_concat(e.prod_id) as prod_id";
						...
						$sql_query="
						Select
						a.company_name, a.job_no_prefix_num, a.job_no, a.buyer_name, a.style_ref_no,a.brand_id, b.id as po_id, b.po_number as po_no,d.color_id,d.prod_id, c.order_rate, $select_fld,e.product_name_details as prod_desc,e.detarmination_id,c.store_id,c.body_part_id ,c.transaction_date as receive_date, c.transaction_date as max_date, 
						(case when d.entry_form in (17) then d.quantity else 0 end) as receive_qnty,
						(case when d.entry_form in (258) and c.transaction_type=5 and d.trans_type=5 then d.quantity else 0 end) as finish_fabric_transfer_in, 
						(case when d.entry_form in (209) and c.transaction_type=4 and d.trans_type=4 then d.quantity else 0 end) as issue_rtn,
						(case when d.entry_form in (17) and  $today_receive_date then d.quantity else 0 end) as today_receive_qnty,
					...
					foreach ($nameArray as $row)
					{
						...
						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['detarmination_id']=$row[csf('detarmination_id')];
						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['brand_id']=$row[csf('brand_id')];

						$style_wise_max_date_arr[$row[csf('company_name')]][$row[csf('store_id')]][$row[csf('prod_id')]][$row[csf('job_no')]]['max_date']=$row[csf('max_date')];
						$style_wise_min_date_arr[$row[csf('company_name')]][$row[csf('store_id')]][$row[csf('prod_id')]][$row[csf('job_no')]]['min_date']=$row[csf('receive_date')];

					

							foreach ($style_wise_max_date_arr as $compaIds => $company_datas)
							{
								foreach ($company_datas as $store_ids => $store_datas)
								{
									foreach ($store_datas as $prod_idss=> $prod_datas)
									{
										foreach ($prod_datas as $job_idss => $datasRow)
										{
										    $timestamp = strtotime($datasRow[("max_date")]);
										    if ($timestamp > strtotime($maxDate)) {
										        $maxDate = $datasRow[("max_date")];
										    }
										}
									}
								}
							...
							foreach ($style_wise_min_date_arr as $compaIdss => $company_datass)
							{
								foreach ($company_datass as $store_idss => $store_datass)
								{
									foreach ($store_datass as $prod_idsss => $prod_datass)
									{
										foreach ($prod_datass as $job_idsss => $datasRows)
										{
										    $timestamps = strtotime($datasRows[("min_date")]);
											//echo "Minimum Date: " . $datasRows[("min_date")] ."<br/>";
											//echo "Minimum Date: " . $minDate ."<br/>";
										    if ($minDateArr[$compaIdss][$store_idss][$prod_idsss][$job_idsss]['min_date']=="" || $timestamps < strtotime($minDateArr[$compaIdss][$store_idss][$prod_idsss][$job_idsss]['min_date'])) {

									    		$minDate= $datasRows[("min_date")];
									    		$minDateArr[$compaIdss][$store_idss][$prod_idsss][$job_idsss]['min_date'] = $datasRows[("min_date")];
										    }   
										}
									}
								}
						...
						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['maximum_date']=$maxDate;
						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['minimum_date']=$minDate;


						
						$ageOfDayss = datediff("d",$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['minimum_date'],$curr_date);

						foreach($caption_arr as $keys=>$value){
							$start=($keys-$range);
							if($start <= $ageOfDayss && $keys >= $ageOfDayss){
								...
								$style_wise_dayRange_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]][$keys]=($style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['receive_qnty']+$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['finish_fabric_transfer_in']+$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['issue_rtn'])-($style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['issue_qnty']+$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['finish_fabric_transfer_out']+$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['recv_rtn']);
								break;
							}
							elseif($end_key < $ageOfDayss){
								$style_wise_dayRange_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]][$end_key]=($style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['receive_qnty']+$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['finish_fabric_transfer_in']+$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['issue_rtn'])-($style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['issue_qnty']+$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['finish_fabric_transfer_out']+$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['recv_rtn']);
								break;
							}
					...
					execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_id and entry_form=2005 and ref_from in(1)");
					oci_commit($con);
					$i=1;
					foreach ($style_wise_arr as $compaId => $company_data)
					{
								...
								foreach ($cons_uom_data  as $job_key=>$job_val)
								{
									foreach ($job_val  as $color_key=>$color_val)
									{
										foreach ($color_val  as $desc_key=>$val)
										{
											...
											$daysOnHand = datediff("d",$val[("maximum_date")],date("Y-m-d")); 
											$ageOfDays = datediff("d",$val[("minimum_date")],$curr_date);
											...
											if($cbo_value_range_by==2 &&  number_format($stock,2,'.','')>0.00)
											{
												...
												<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
													...
													<td width="100"><p><? echo $store_lib_arr[$val[("store_id")]]; ?></p></td>
													<td width="100"><p><? echo $buyer_arr[$val[("buyer_name")]]; ?></p></td>
													<td width="100"><p><? echo $brand_arr[$val[("brand_id")]]; ?></p></td>
													<td width="60" align="center"><p><? echo $val[("job_no")]; ?></p></td>
													<td width="50" align="center"><p><? echo $val[("year")]; ?></p></td>
													<td width="110"><p><? echo $val[("style_ref_no")]; ?></p></td>
													<td width="100" align="center"><p><? echo $fabric_type_arr[$val[("detarmination_id")]]; ?></p></td>
													...
													<td width="100"><p><? echo $color_type[$colorType]; ?></p></td>
													<td width="110"><p><? echo $color_arr[$color_key]; ?></p></td>
													<td width="220" title="<? echo 'Prod. ID='.$desc_key;?>"><p><? echo $fab_desc_type; ?></p></td>
													<td width="50" align="center"><p><? echo $unit_of_measurement[$cons_uom]; ?></p></td>
																	
													<td width="150" align="right" title="<? echo "Receive: ".$val[("receive_qnty")]."##".$issue_ret_qnty."##".$val[("rec_trns_qnty")]."Issue: ".$val[("issue_qnty")]."##".$receive_ret_qnty."##".$val[("issue_trns_qnty")]; ?>" ><p><a href='#report_details' onClick="openmypage('<? echo $val[("companyId")]; ?>','<? echo $po_ids; ?>','<? echo $desc_key; ?>','<? echo $color_key; ?>','<? echo $date_from; ?>','<? echo 6; ?>','woven_knit_stock_popup','','','','','<? echo $val[("body_part_id")]; ?>','<? echo $val[("store_id")]; ?>');"><? echo number_format($stock,2,'.',''); ?></a></p></td>

													<? foreach($caption_arr as $key=>$fill_value){ ?>
															<td width="50" align="right"><p>
																<? 
																	//echo round($age_qty_arr[$row[csf("id")]][$key]);
																if(($style_wise_dayRange_arr[$val[("companyId")]][$val[("store_id")]][$cons_uom][$job_key][$color_key][$desc_key][$key]>=0))
																	{
																		echo	number_format($style_wise_dayRange_arr[$val[("companyId")]][$val[("store_id")]][$cons_uom][$job_key][$color_key][$desc_key][$key],2,'.','');
																	}
																	else
																	{
                	...
                	<table width="<? echo $table_width; ?>" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table" >
						<tfoot>
							...
							<th width="220">&nbsp;</th>
							<th width="50">Total</th>
							<th width="150" align="right" id="total_stock_td"><? echo number_format($total_stock,2,'.',''); ?></th>
							<? $s=1; foreach($caption_arr as $key=>$fill_value){ ?>
								<th id="age_<? echo $s++;?>" width="50" align="right"></th>
							<? } ?>
							<th width="80">&nbsp;</th>
							<th>&nbsp;</th>	
						</tfoot>
	...
	$create_new_doc = fopen($filename, 'w');
	$is_created = fwrite($create_new_doc, $html);
	echo "$html**$filename**$report_type**$s";
	exit();
}
