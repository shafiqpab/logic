...
if($action=="report_generate")
{
	...
	$select_from_to=change_date_format($txt_date_to,'','',1);
	...
	if($cbo_report_type==2) // Woven Finish Start
	{
		...
		$issue_qnty=array();
		$sql_issue=sql_select(" select b.po_breakdown_id,b.color_id, sum(b.quantity) as issue_qnty  from inv_issue_master a,inv_transaction c, order_wise_pro_details b where a.id=c.mst_id and c.prod_id=b.prod_id  and b.trans_id!=0 and a.status_active=1 and a.is_deleted=0 and a.issue_purpose in(4,9) and b.entry_form=19 and c.transaction_type=1 and c.transaction_date<'".$select_from_date."' $storeCond group by b.po_breakdown_id,b.color_id");
		...
		foreach( $sql_issue_ret as $row )
		{
			...
			$iss_return_qnty[$row[csf('job_no')]][$row[csf('po_id')]][$row[csf('color_id')]][$row[csf('prod_id')]]['issue_ret_qnty_opening']+=$row[csf('issue_ret_qnty')];
		}
		unset($sql_issue_ret);
		//$buyer_arr=return_library_array( "select id, buyer_name from lib_buyer", "id", "buyer_name"  );
		...
		<fieldset style="width:1840px;">
			...
			<table width="3350" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table">
				<thead>
					...
					<tr>
						...
						<th width="100" rowspan="2">Fabric Ref</th>
						<th width="100" rowspan="2">System Code</th>
						<th width="220" rowspan="2">Fab. Desc.</th>
						<th width="50" rowspan="2">UOM</th>

						<th width="240" colspan="3">Opening Received</th>
						<th width="240" colspan="8">Receive</th>			
						<th width="240" colspan="8">Issue</th>
						
						<th width="240" colspan="3">Closing Stock</th>
						<th width="80" rowspan="2">Age(Days)</th>
						
						
					</tr>
					<tr>
						<th width="80">Rate(BDT)</th>
						<th width="80">Stock</th>
						<th width="80">Value(BDT)</th>
						...
						<th width="80">Transfer Recv Value</th>
						...
						<th width="80"> Stock</th>
						<th width="80">Avg.Rate [BDT]</th>
						<th width="80">Stock Value(BDT)</th>
			...
			<div style="width:3370px; max-height:350px; overflow-y:scroll;" id="scroll_body">
				<table width="3350" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table" id="table_body" >
					<?
					...
					$sql_query="
					Select
					a.company_name, a.job_no_prefix_num, a.job_no, a.buyer_name, a.style_ref_no, b.id as po_id, b.po_number as po_no,d.color_id,d.prod_id, c.order_rate, $select_fld,e.product_name_details as prod_desc,e.detarmination_id,c.store_id,c.body_part_id , c.transaction_date,
					

					(case when d.entry_form in (17) and c.transaction_type=1 and c.transaction_date<'".$select_from_date."' then d.quantity else 0 end) as rcv_total_opening,
					(case when d.entry_form in (19) and c.transaction_type=2 and c.transaction_date<'".$select_from_date."' then d.quantity else 0 end) as iss_total_opening, 
					(case when d.entry_form in (202) and c.transaction_type=3  and d.trans_type=3 and c.transaction_date<'".$select_from_date."' then d.quantity else 0 end) as rcv_return_opening,
					(case when d.entry_form in (209) and c.transaction_type=4 and d.trans_type=4 and c.transaction_date<'".$select_from_date."' then d.quantity else 0 end) as issue_rtn_opening,
					(case when d.entry_form in (258) and c.transaction_type=5 and d.trans_type=5 and c.transaction_date<'".$select_from_date."' then d.quantity else 0 end) as finish_fabric_transfer_in_opening,
					(case when d.entry_form in (258) and c.transaction_type=6 and d.trans_type=6 and c.transaction_date<'".$select_from_date."' then d.quantity else 0 end) as finish_fabric_transfer_out_opening, 


					(case when d.entry_form in (17) and c.transaction_type=1 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."'  then d.quantity else 0 end) as receive_qnty,
					(case when d.entry_form in (19) and c.transaction_type=2 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."'  then d.quantity else 0 end) as issue_qnty,
					(case when d.entry_form in (202) and c.transaction_type=3 and d.trans_type=3 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."'  then d.quantity else 0 end) as recv_rtn, 
					(case when d.entry_form in (209) and c.transaction_type=4 and d.trans_type=4 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."'  then d.quantity else 0 end) as issue_rtn, 
					(case when d.entry_form in (258) and c.transaction_type=5 and d.trans_type=5 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."' then d.quantity else 0 end) as trans_in,
					(case when d.entry_form in (258) and c.transaction_type=6 and d.trans_type=6 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."'  then d.quantity else 0 end) as trans_out,
					
					(case when d.entry_form in (17) and c.transaction_type=1 and c.transaction_date<'".$select_from_date."' then c.cons_rate else 0 end) *(case when d.entry_form in (17) and c.transaction_type=1 and c.transaction_date<'".$select_from_date."' then d.quantity else 0 end) as receive_amount_opening,
					(case when d.entry_form in (19) and c.transaction_type=2 and c.transaction_date<'".$select_from_date."' then c.cons_rate else 0 end)*(case when d.entry_form in (19) and c.transaction_type=2 and c.transaction_date<'".$select_from_date."' then d.quantity else 0 end) as issue_amount_opening, 
					(case when d.entry_form in (202) and c.transaction_type=3 and d.trans_type=3 and c.transaction_date<'".$select_from_date."' then d.quantity else 0 end)*(case when d.entry_form in (202) and c.transaction_type=3 and c.transaction_date<'".$select_from_date."' and d.trans_type=3 then c.cons_rate else 0 end) as recv_rtn_amount_opening ,

					(case when d.entry_form in (209) and c.transaction_type=4 and d.trans_type=4 and c.transaction_date<'".$select_from_date."' then d.quantity else 0 end)*(case when d.entry_form in (209) and c.transaction_type=4 and c.transaction_date<'".$select_from_date."' and d.trans_type=4 then c.cons_rate else 0 end) as issue_rtn_amount_opening, 

					(case when d.entry_form in (258) and c.transaction_type=5 and d.trans_type=5 and c.transaction_date<'".$select_from_date."' then d.quantity else 0 end) *(case when d.entry_form in (258) and c.transaction_type=5 and d.trans_type=5 and c.transaction_date<'".$select_from_date."' then c.cons_rate else 0 end) as transfer_in_amount_opening,

					(case when d.entry_form in (258) and c.transaction_type=6 and d.trans_type=6 and c.transaction_date<'".$select_from_date."' then c.cons_rate else 0 end) *(case when d.entry_form in (258) and c.transaction_type=6 and d.trans_type=6 and c.transaction_date<'".$select_from_date."' then d.quantity else 0 end) as transfer_out_amount_opening, 


					(case when d.entry_form in (17)  and c.transaction_type=1 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."' then d.quantity else 0 end)*(case when d.entry_form in (17)  and c.transaction_type=1 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."' then c.cons_rate else 0 end) as receive_amount, 
					(case when d.entry_form in (19) and c.transaction_type=2 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."' then c.cons_rate else 0 end) *(case when d.entry_form in (19) and c.transaction_type=2 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."' then d.quantity else 0 end) as issue_amount, 


					(case when d.entry_form in (202)  and c.transaction_type=3 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."' and d.trans_type=3 then d.quantity else 0 end)*(case when d.entry_form in (202) and c.transaction_type=3 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."' and d.trans_type=3 then c.cons_rate else 0 end) as recv_rtn_amount ,


					(case when d.entry_form in (209) and c.transaction_type=4 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."' and d.trans_type=4 then d.quantity else 0 end)*(case when d.entry_form in (209) and c.transaction_type=4 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."' and d.trans_type=4 then c.cons_rate else 0 end) as issue_rtn_amount, 
					
					(case when d.entry_form in (258)  and c.transaction_type=5 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."' and d.trans_type=5 then d.quantity else 0 end)*(case when d.entry_form in (258)  and c.transaction_type=5 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."' and d.trans_type=5 then c.cons_rate else 0 end) as transfer_in_amount,
					
					(case when d.entry_form in (258) and c.transaction_type=6 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."' and d.trans_type=6 then c.cons_rate else 0 end)*(case when d.entry_form in (258) and c.transaction_type=6 and d.trans_type=6 and c.transaction_date between '".$select_from_date."' and '".$select_from_to."' then d.quantity else 0 end) as transfer_out_amount
					...
					and a.company_name in(".$cbo_company_id.") $buyer_id_cond $search_cond $year_cond $storeCond $shiping_status_cond $uom_cond  order by a.job_no,d.color_id";
					//and e.detarmination_id =1157 and c.body_part_id=25
					...
					foreach ($nameArray as $row)
					{
						...
						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['prod_id']=$row[csf('prod_id')];
						...
						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['rcv_total_opening']+=$row[csf('rcv_total_opening')];
						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['iss_total_opening']+=$row[csf('iss_total_opening')];
						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['issue_rtn_opening']+=$row[csf('issue_rtn_opening')];
						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['rcv_return_opening']+=$row[csf('rcv_return_opening')];
						...
						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['trans_in']+=$row[csf('trans_in')];
						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['trans_out']+=$row[csf('trans_out')];
						...
						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['finish_fabric_transfer_in_opening']+=$row[csf('finish_fabric_transfer_in_opening')];
						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['finish_fabric_transfer_out_opening']+=$row[csf('finish_fabric_transfer_out_opening')];


						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['receive_amount_opening']+=$row[csf('receive_amount_opening')];
						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['transfer_in_amount_opening']+=$row[csf('transfer_in_amount_opening')];
						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['issue_rtn_amount_opening']+=$row[csf('issue_rtn_amount_opening')];

						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['issue_amount_opening']+=$row[csf('issue_amount_opening')];
						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['transfer_out_amount_opening']+=$row[csf('transfer_out_amount_opening')];
						$style_wise_arr[$row[csf('company_name')]][$row[csf('store_id')]][$uomFromProductArr[$row[csf('prod_id')]]][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('prod_desc')]]['recv_rtn_amount_opening']+=$row[csf('recv_rtn_amount_opening')];
					...
					//echo "<pre>";
					//print_r($last_transaction_date_arr); die;

					$po_id_arr = array_filter($po_id_arr);
					if (!empty($po_id_arr) && $user_id>0) 
					{
						$con = connect();
						fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 2006, 1,$po_id_arr, $empty_arr);
					}
					
					oci_commit($con);

					$determination_info_sql = sql_select("SELECT c.id,b.body_part_id,b.lib_yarn_count_deter_id,b.uom, a.fabric_color_id as color_id,c.rd_no,c.fabric_ref,a.job_no,a.color_type from wo_booking_dtls a, wo_pre_cost_fabric_cost_dtls b ,lib_yarn_count_determina_mst c, GBL_TEMP_ENGINE d where a.pre_cost_fabric_cost_dtls_id=b.id and b.lib_yarn_count_deter_id=c.id  and a.job_no=b.job_no and a.status_active=1 and a.is_deleted=0 and a.po_break_down_id=d.ref_val and d.user_id=$user_id and d.entry_form=2006 and d.ref_from=1  group by c.id,b.body_part_id, b.lib_yarn_count_deter_id, a.fabric_color_id,b.uom,c.rd_no,c.fabric_ref,a.job_no,a.color_type");
					foreach($determination_info_sql as $row)
					{
						//$determination_info_arr[$row[csf('lib_yarn_count_deter_id')]][$row[csf('uom')]][$row[csf('color_id')]][$row[csf('body_part_id')]][$row[csf('job_no')]]['rd_no']=$row[csf('rd_no')];
						$determination_info_arr[$row[csf('lib_yarn_count_deter_id')]][$row[csf('uom')]][$row[csf('color_id')]][$row[csf('body_part_id')]][$row[csf('job_no')]]['fabric_ref']=$row[csf('fabric_ref')];
						$determination_info_arr[$row[csf('lib_yarn_count_deter_id')]][$row[csf('uom')]][$row[csf('color_id')]][$row[csf('body_part_id')]][$row[csf('job_no')]]['color_type']=$row[csf('color_type')];
						$determination_info_arr[$row[csf('lib_yarn_count_deter_id')]][$row[csf('uom')]][$row[csf('color_id')]][$row[csf('body_part_id')]][$row[csf('job_no')]]['id']=$row[csf('id')];
					}


					execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_id and entry_form=2006 and ref_from in(1)");
					oci_commit($con);
					...
					$grand_totalOpeningQnty=0;
					$grand_StockValue_opening=0;
					$grand_recv=0;
					$grand_recv_amount=0;
					$grand_issue_rtn=0;
					$grand_recv_issue_rtn_amount=0;
					$grand_trans_in=0;
					$grand_recv_trans_in_amount=0;
					$grand_totalRecvQnty=0;
					$grand_totalRecvQntyAmount=0;
					$grand_issue=0;
					$grand_issue_amount=0;
					$grand_issue_rev_rtn_amount=0;
					$grand_trans_out=0;
					$grand_issue_trans_out_amount=0;
					$grand_totalIssueQnty=0;
					$grand_totalIssueQntyAmount=0;
					$grand_stockQntyTotal=0;
					$grand_stockValueTotal=0;
							...
							foreach ($store_data as $cons_uom => $cons_uom_data)
							{
								foreach ($cons_uom_data  as $job_key=>$job_val)
								{
									foreach ($job_val  as $color_key=>$color_val)
									{
										foreach ($color_val  as $desc_key=>$val)
										{
											...
											foreach($po_ids as $po_id)
											{
												...
												$issue_ret_qnty+=$iss_return_qnty[$job_key][$po_id][$color_key][$desc_key]['issue_ret_qnty_opening'];
											...
											$recv=$val[("receive_qnty")];
											$issue=$val[("issue_qnty")];
											$recv_rtn=$val[("recv_rtn")];
											$issue_rtn=$val[("issue_rtn")];
											$trans_in=$val[("trans_in")];
											$trans_out=$val[("trans_out")];

											$totalRecvQnty=$recv+$issue_rtn+$trans_in;
											$totalIssueQnty=$issue+$recv_rtn+$trans_out;
											$stockQntyTotal=$totalRecvQnty-$totalIssueQnty;

											$rec_qty_opening=$val[("rcv_total_opening")];
											$rec_tranfIn_opening=$val[("finish_fabric_transfer_in_opening")];
											$rec_issueRtn_opening=$val[("issue_rtn_opening")];
			
											$iss_qty_opening=$val[("iss_total_opening")];
											$iss_trnsOut_opening=$val[("finish_fabric_transfer_out_opening")];
											$iss_recvRtn_opening=$val[("rcv_return_opening")];
											//echo $rec_qty_opening."+".$rec_tranfIn_opening."+".$rec_issueRtn_opening."<br/>";
											$total_recQnty_opening=$rec_qty_opening+$rec_tranfIn_opening+$rec_issueRtn_opening;
											$total_issQnty_opening=$iss_qty_opening+$iss_trnsOut_opening+$iss_recvRtn_opening;

											$totalOpeningQnty=$total_recQnty_opening-$total_issQnty_opening;

											$rec_bal=$book_qty-$total_recQnty-$rec_issueRtn; 
											
											//$receive_amount=number_format($val[("receive_amount")],2,'.','');
											$rec_qty_cal=($total_recQnty_opening);
											$iss_qty_cal=($total_issQnty_opening);
											$stock=$rec_qty_cal-$iss_qty_cal;
											
											//rate calculating
											//$rate = $receive_amount/$rec_qty;
											//$issue_amount = number_format(($val[("iss_total_opening")]*$rate),2,'.','');
											...
											$recv_amount 			=$val[("receive_amount")];
											$recv_issue_rtn_amount 	=$val[("issue_rtn_amount")];
											$recv_trans_in_amount 	=$val[("transfer_in_amount")];
											$issue_amount 			=$val[("issue_amount")];
											$issue_rev_rtn_amount 	=$val[("recv_rtn_amount")];
											$issue_trans_out_amount =$val[("transfer_out_amount")];

											$recv_amount_all_opening 				=$val[("receive_amount_opening")];
											$recv_issue_rtn_amount_all_opening 		=$val[("issue_rtn_amount_opening")];
											$recv_trans_in_amount_all_opening 		=$val[("transfer_in_amount_opening")];
											$issue_amount_all_opening				=$val[("issue_amount_opening")];
											$issue_rev_rtn_amount_all_opening 		=$val[("recv_rtn_amount_opening")];
											$issue_trans_out_amount_all_opening 	=$val[("transfer_out_amount_opening")];



											if(is_nan($recv_amount)){$recv_amount=0;}
											if(is_nan($recv_issue_rtn_amount)){$recv_issue_rtn_amount=0;}
											if(is_nan($recv_trans_in_amount)){$recv_trans_in_amount=0;}
											if(is_nan($issue_amount)){$issue_amount=0;}
											if(is_nan($issue_rev_rtn_amount)){$issue_rev_rtn_amount=0;}
											if(is_nan($issue_trans_out_amount)){$issue_trans_out_amount=0;}



											$totalRecvQntyAmount=$recv_amount+$recv_issue_rtn_amount+$recv_trans_in_amount;
											$totalIssueQntyAmount=$issue_amount+$issue_rev_rtn_amount+$issue_trans_out_amount;
											$stockValueTotal=$totalRecvQntyAmount-$totalIssueQntyAmount;
											

											if(is_nan($recv_amount_all_opening)){$recv_amount_all_opening=0;}
											if(is_nan($recv_issue_rtn_amount_all_opening)){$recv_issue_rtn_amount_all_opening=0;}
											if(is_nan($recv_trans_in_amount_all_opening)){$recv_trans_in_amount_all_opening=0;}
											if(is_nan($issue_amount_all_opening)){$issue_amount_all_opening=0;}
											if(is_nan($issue_rev_rtn_amount_all_opening)){$issue_rev_rtn_amount_all_opening=0;}
											if(is_nan($issue_trans_out_amount_all_opening)){$issue_trans_out_amount_all_opening=0;}

											$rec_amount_total_opening=($val[("receive_amount_opening")]+$val[("transfer_in_amount_opening")]+$val[("issue_rtn_amount_opening")]);
											$rec_avg_rate_total=$rec_amount_total_opening/$total_recQnty_opening;
											
											//echo $val[("issue_amount_opening")].'+'.$val[("transfer_out_amount_opening")].'+'.$val[("recv_rtn_amount_opening")].'<br/>';

											$iss_amount_total_opening=($val[("issue_amount_opening")]+$val[("transfer_out_amount_opening")]+$val[("recv_rtn_amount_opening")]);
											$issue_avg_rate_total=$iss_amount_total_opening/$total_issQnty_opening;

											$StockValue_opening=$rec_amount_total_opening-$iss_amount_total_opening;
											//echo $rec_amount_total_opening.'-'.$iss_amount_total_opening.'<br/>';

											$date1=date("d-m-Y");
											$date2 = $last_transaction_date_arr[$val[("prod_id")]]['last_transaction_date'];
											//echo $date2 ;
											...
											//stockQntyTotal
											$closingStock=$totalOpeningQnty+$totalRecvQnty-$totalIssueQnty;

											$closingStockValue=$StockValue_opening+$stockValueTotal;
											$openingQntyAvgRateTotal=$StockValue_opening/$totalOpeningQnty;
											

											$stockAvgRateTotal=$closingStockValue/$closingStock;
											if (is_nan($stockAvgRateTotal)) {
												$stockAvgRateTotal=0;
											}

											if (is_nan($openingQntyAvgRateTotal)) {
												$openingQntyAvgRateTotal=0;
											}


											$fabricRef=$determination_info_arr[$val[("detarmination_id")]][$cons_uom][$color_key][$val[("body_part_id")]][$job_key]['fabric_ref'];
											$colorType=$determination_info_arr[$val[("detarmination_id")]][$cons_uom][$color_key][$val[("body_part_id")]][$job_key]['color_type'];
											$sysCode=$determination_info_arr[$val[("detarmination_id")]][$cons_uom][$color_key][$val[("body_part_id")]][$job_key]['id'];


											$group_short_name=$lib_group_short[1];
											if($sysCode>0){$sys_code=$group_short_name.'-'.$sysCode;}
											

											if($cbo_value_range_by==2 &&  number_format($closingStock,2,'.','')>0.00)
											{
												if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
												?>
												<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
													...
													<td width="100" align="center"><p><? echo $val[("job_no")]; ?></p></td>
													...
													<td width="100" align="center"><p><? echo $fabricRef; ?></p></td>
													<td width="100" align="center"><p><? echo $sys_code; ?></p></td>
													<td width="220" title="<? echo 'Prod. ID='.$desc_key;?>"><p><? echo $fab_desc_type; ?></p></td>
													<td width="50"><p><? echo $unit_of_measurement[$cons_uom]; ?></p></td>
													
																	
													<td width="80" align="right"><p><? echo number_format($openingQntyAvgRateTotal,2,'.',''); ?></p></td>
													<td width="80" align="right"><p><? echo number_format($totalOpeningQnty,2,'.',''); ?></p></td>
													<td width="80" align="right"><p><? echo number_format($StockValue_opening,2,'.',''); ?></p></td>
													
													<td width="80" align="right"><p><a href='#report_details' onClick="openmypage('<? echo $val[("companyId")]; ?>','<? echo $po_ids; ?>','<? echo $desc_key; ?>','<? echo $color_key; ?>','<? echo $date_from; ?>','<? echo 0; ?>','woven_only_today_receive_popup','','','','','<? echo $val[("body_part_id")]; ?>','<? echo $val[("store_id")]; ?>');"><? echo number_format($recv,2,'.',''); ?>&nbsp;</a></p></td>
													<td width="80" align="right"><p><? echo number_format($recv_amount,2,'.',''); ?></p></td>
													<td width="80" align="right"><p><a href='#report_details'  onClick="openmypage('<? echo $val[("companyId")]; ?>','<? echo $po_ids; ?>','<? echo $desc_key; ?>','<? echo $color_key; ?>','<? echo $date_from; ?>','<? echo 0; ?>','woven_only_today_issue_rtn_popup','','','','','<? echo $val[("body_part_id")]; ?>','<? echo $val[("store_id")]; ?>');"><? echo number_format($issue_rtn,2,'.',''); ?>&nbsp;</a></p></td>
													<td width="80" align="right"><p><? echo number_format($recv_issue_rtn_amount,2,'.',''); ?></p></td>
													<td width="80" align="right"><p><a href='#report_details' onClick="openmypage('<? echo $val[("companyId")]; ?>','<? echo $po_ids; ?>','<? echo $desc_key; ?>','<? echo $color_key; ?>','<? echo $date_from; ?>','<? echo 0; ?>','woven_only_today_transf_in_popup','','','','','<? echo $val[("body_part_id")]; ?>','<? echo $val[("store_id")]; ?>');"><? echo number_format($trans_in,2,'.',''); ?>&nbsp;</a></p></td>
													<td width="80" align="right"><p><? echo number_format($recv_trans_in_amount,2,'.',''); ?></p></td>
													<td width="80" align="right"><p><? echo number_format($totalRecvQnty,2,'.',''); ?></p></td>
													<td width="80" align="right"><p><? echo number_format($totalRecvQntyAmount,2,'.',''); ?></p></td>
												
													<td width="80" align="right"><p><a href='#report_details' onClick="openmypage('<? echo $val[("companyId")]; ?>','<? echo $po_ids; ?>','<? echo $desc_key; ?>','<? echo $color_key; ?>','<? echo $date_from; ?>','<? echo 0; ?>','woven_only_today_issue_popup','','','','','<? echo $val[("body_part_id")]; ?>','<? echo $val[("store_id")]; ?>');"><? echo number_format($issue,2,'.',''); ?></a></p></td>
													<td width="80" align="right"><p><? echo number_format($issue_amount,2,'.',''); ?></p></td>
													<td width="80" align="right"><p><a href='#report_details' onClick="openmypage('<? echo $val[("companyId")]; ?>','<? echo $po_ids; ?>','<? echo $desc_key; ?>','<? echo $color_key; ?>','<? echo $date_from; ?>','<? echo 0; ?>','woven_only_today_recv_rtn_popup','','','','','<? echo $val[("body_part_id")]; ?>','<? echo $val[("store_id")]; ?>');"><? echo number_format($recv_rtn,2,'.',''); ?></a></p></td>
													<td width="80" align="right"><p><? echo number_format($issue_rev_rtn_amount,2,'.',''); ?></p></td>
													<td width="80" align="right"><p><a href='#report_details' onClick="openmypage('<? echo $val[("companyId")]; ?>','<? echo $po_ids; ?>','<? echo $desc_key; ?>','<? echo $color_key; ?>','<? echo $date_from; ?>','<? echo 0; ?>','woven_only_today_transf_out_popup','','','','','<? echo $val[("body_part_id")]; ?>','<? echo $val[("store_id")]; ?>');"><? echo number_format($trans_out,2,'.',''); ?></a></p></td>
													<td width="80" align="right"><p><? echo number_format($issue_trans_out_amount,2,'.',''); ?></p></td>
													
													<td width="80" align="right"><p><? echo number_format($totalIssueQnty,2,'.',''); ?></p></td>
													<td width="80" align="right"><p><? echo number_format($totalIssueQntyAmount,2,'.',''); ?></p></td>
	
													<td width="80" align="right" title="" ><p><? echo number_format($closingStock,2,'.',''); ?></p></td>
													
													<td width="80" align="right"><p><? echo number_format($stockAvgRateTotal,2,'.',''); ?></p></td>
													<td width="80" align="right"><p><? 
														// if(number_format($stock,2,'.','')>0.00) {
														// 	$StockValue=$StockValue;
															
														// }
														// else if(number_format($StockValue,2,'.','')>0.00){
														// 	echo "xxxx=".$val[("prod_id")]."="; 
														// 	//$StockValue=0; 
														// }
														// else if(number_format($StockValue,2,'.','')<0.00){
														// 	echo "xxxx=".$val[("prod_id")]."="; 
														// }
														echo number_format($closingStockValue,2,'.',''); 
													
													?></p></td>

													<td width="" align="right"><p><? echo $ageing; ?></p></td>	
									
												</tr>
													<?
													$i++;

													$sub_totalOpeningQnty+=$totalOpeningQnty;
													$sub_StockValue_opening+=$StockValue_opening;
													$sub_recv+=$recv;
													$sub_recv_amount+=$recv_amount;
													$sub_issue_rtn+=$issue_rtn;
													$sub_recv_issue_rtn_amount+=$recv_issue_rtn_amount;
													$sub_trans_in+=$trans_in;
													$sub_recv_trans_in_amount+=$recv_trans_in_amount;
													$sub_totalRecvQnty+=$totalRecvQnty;
													$sub_totalRecvQntyAmount+=$totalRecvQntyAmount;
													$sub_issue+=$issue;
													$sub_issue_amount+=$issue_amount;
													$sub_recv_rtn+=$recv_rtn;
													$sub_issue_rev_rtn_amount+=$issue_rev_rtn_amount;
													$sub_trans_out+=$trans_out;
													$sub_issue_trans_out_amount+=$issue_trans_out_amount;
													$sub_totalIssueQnty+=$totalIssueQnty;
													$sub_totalIssueQntyAmount+=$totalIssueQntyAmount;
													$sub_stockQntyTotal+=$closingStock;
													$sub_stockValueTotal+=$closingStockValue;


													$grand_totalOpeningQnty+=$totalOpeningQnty;
													$grand_StockValue_opening+=$StockValue_opening;
													$grand_recv+=$recv;
													$grand_recv_amount+=$recv_amount;
													$grand_issue_rtn+=$issue_rtn;
													$grand_recv_issue_rtn_amount+=$recv_issue_rtn_amount;
													$grand_trans_in+=$trans_in;
													$grand_recv_trans_in_amount+=$recv_trans_in_amount;
													$grand_totalRecvQnty+=$totalRecvQnty;
													$grand_totalRecvQntyAmount+=$totalRecvQntyAmount;
													$grand_issue+=$issue;
													$grand_issue_amount+=$issue_amount;
													$grand_recv_rtn+=$recv_rtn;
													$grand_issue_rev_rtn_amount+=$issue_rev_rtn_amount;
													$grand_trans_out+=$trans_out;
													$grand_issue_trans_out_amount+=$issue_trans_out_amount;
													$grand_totalIssueQnty+=$totalIssueQnty;
													$grand_totalIssueQntyAmount+=$totalIssueQntyAmount;
													$grand_stockQntyTotal+=$closingStock;
													$grand_stockValueTotal+=$closingStockValue;
											...
											else if($cbo_value_range_by==1 || $cbo_value_range_by==0)
											{
												...
												<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
													...
													<td width="100" title="<? echo $po_ids;?>"><p>
														...
														?></p></td>
													<td width="110"><p><? echo $color_arr[$color_key]; ?></p></td>
													<td width="100" title="<? echo $val[("body_part_id")]; ?>"><p><? echo $body_part_lib_arr[$val[("body_part_id")]]; ?></p></td>
													<td width="100" align="center"  title="<? echo $val[("detarmination_id")]; ?>"><p><? echo $fabric_type_arr[$val[("detarmination_id")]]; ?></p></td>
													<td width="100" align="center"><p><? echo $fabricRef; ?></p></td>
													<td width="100" align="center"><p><? echo $sys_code; ?></p></td>
													<td width="220" title="<? echo 'Prod. ID='.$desc_key;?>"><p><? echo $fab_desc_type; ?></p></td>
													<td width="50"><p><? echo $unit_of_measurement[$cons_uom]; ?></p></td>
													...
													<td width="80" align="right"><p><? if(number_format($totalOpeningQnty,2,'.','')>0.00){echo number_format($openingQntyAvgRateTotal,2,'.','');}else{echo "0.00";}  ?></p></td>
													<td width="80" align="right"><p><? echo number_format($totalOpeningQnty,2,'.',''); ?></p></td>
													<td width="80" align="right"><p><? if(number_format($totalOpeningQnty,2,'.','')>0.00){ echo number_format($StockValue_opening,2,'.','');}else{echo "0.00";} ?></p></td>
													...
													<td width="80" align="right"><p><? if(number_format($closingStock,2,'.','')>0.00){ echo number_format($stockAvgRateTotal,2,'.','');}else{echo "0.00";} ?></p></td>
													<td width="80" align="right"><p><? 
														// if(number_format($stock,2,'.','')>0.00) {
														// 	$StockValue=$StockValue;
														...
														// 	echo "xxxx=".$val[("prod_id")]."="; 
														// }
														if(number_format($closingStock,2,'.','')>0.00){
															echo number_format($closingStockValue,2,'.','');}else{echo "0.00";} 
													...
													$grand_totalOpeningQnty+=$totalOpeningQnty;
													if(number_format($totalOpeningQnty,2,'.','')>0.00){
														$grand_StockValue_opening+=$StockValue_opening;
													}else{ $grand_StockValue_opening+=0.00;}
													$grand_recv+=$recv;
													$grand_recv_amount+=$recv_amount;
													$grand_issue_rtn+=$issue_rtn;
													$grand_recv_issue_rtn_amount+=$recv_issue_rtn_amount;
													...
													else{$grand_stockValueTotal+=0.00;}
									...
									<!-- <tr style="font-weight:bold;background-color:#e0e0e0;">
										...
										<td align="right"><? echo number_format($sub_totalOpeningQnty,2,'.','') ?></td>

										<td align="right"><? echo number_format($sub_StockValue_opening,2,'.','') ?></td>
										<td align="right"><? echo number_format($sub_recv,2,'.','') ?></td>
										<td align="right"><? echo number_format($sub_recv_amount,2,'.','') ?></td>
										<td align="right"><? echo number_format($sub_issue_rtn,2,'.','') ?></td>
										<td align="right"><? echo number_format($sub_recv_issue_rtn_amount,2,'.','') ?></td>
										<td align="right"><? echo number_format($sub_trans_in,2,'.','') ?></td>

										<td align="right"><? echo number_format($sub_recv_trans_in_amount,2,'.','') ?></td>
										<td align="right"><? echo number_format($sub_totalRecvQnty,2,'.','') ?></td>
										<td align="right"><? echo number_format($sub_totalRecvQntyAmount,2,'.','') ?></td>
										<td align="right"><? echo number_format($sub_issue,2,'.','') ?></td>
										<td align="right"><? echo number_format($sub_issue_amount,2,'.','') ?></td>
										<td align="right"><? echo number_format($sub_issue_rev_rtn_amount,2,'.','') ?></td>

										<td align="right"><? echo number_format($sub_trans_out,2,'.','') ?></td>

										<td align="right"><? echo number_format($sub_issue_trans_out_amount,2,'.','') ?></td>
										<td align="right"><? echo number_format($sub_totalIssueQnty,2,'.','') ?></td>
										<td align="right"><? echo number_format($sub_totalIssueQntyAmount,2,'.','') ?></td>
										<td align="right"><? echo number_format($sub_stockQntyTotal,2,'.','') ?></td>
										<td></td>
										<td align="right"><? echo number_format($sub_stockValueTotal,2,'.','') ?></td>
										<td></td>
                    ...
                    <tfoot>
                        ...
                        <th width="80" align="right" id="value_totalOpeningQnty"><? echo number_format($grand_totalOpeningQnty,2,'.',''); ?></th>
                        
						<th width="80" align="right" id="value_StockValue_opening"><? echo number_format($grand_StockValue_opening,2,'.',''); ?></th>
						<th width="80" align="right" id="value_recv"><? echo number_format($grand_recv,2,'.',''); ?></th>
						<th width="80" align="right" id="value_recv_amount"><? echo number_format($grand_recv_amount,2,'.',''); ?></th>


						<th width="80" align="right" id="value_issue_rtn"><? echo number_format($grand_issue_rtn,2,'.',''); ?></th>
						<th width="80" align="right" id="value_recv_issue_rtn_amount"><? echo number_format($grand_recv_issue_rtn_amount,2,'.',''); ?></th>
						<th width="80" align="right" id="value_trans_in"><? echo number_format($grand_trans_in,2,'.',''); ?></th>
                     
                        <th width="80" align="right" id="value_recv_trans_in_amount"><? echo number_format($grand_recv_trans_in_amount,2,'.',''); ?></th>

						<th width="80" align="right" id="value_totalRecvQnty"><? echo number_format($grand_totalRecvQnty,2,'.',''); ?></th>
						<th width="80" align="right" id="value_totalRecvQntyAmount"><? echo number_format($grand_totalRecvQntyAmount,2,'.',''); ?></th>
						<th width="80" align="right" id="value_issue"><? echo number_format($grand_issue,2,'.',''); ?></th>

						<th width="80" align="right" id="value_recv_rtn"><? echo number_format($grand_recv_rtn,2,'.',''); ?></th>
						<th width="80" align="right" id="value_issue_amount"><? echo number_format($grand_issue_amount,2,'.',''); ?></th>
						<th width="80" align="right" id="value_issue_rev_rtn_amount"><? echo number_format($grand_issue_rev_rtn_amount,2,'.',''); ?></th>
						<th width="80" align="right" id="value_trans_out"><? echo number_format($grand_trans_out,2,'.',''); ?></th>
						<th width="80" align="right" id="value_issue_trans_out_amount"><? echo number_format($grand_issue_trans_out_amount,2,'.',''); ?></th>
						<th width="80" align="right" id="value_totalIssueQnty"><? echo number_format($grand_totalIssueQnty,2,'.',''); ?></th>
						<th width="80" align="right" id="value_totalIssueQntyAmount"><? echo number_format($grand_totalIssueQntyAmount,2,'.',''); ?></th>
						<th width="80" align="right" id="value_stockQntyTotal"><? echo number_format($grand_stockQntyTotal,2,'.',''); ?></th>
						<th width="80"></th> 
						<th width="80" align="right" id="value_stockValueTotal"><? echo number_format($grand_stockValueTotal,2,'.',''); ?></th>
						<th></th> 
