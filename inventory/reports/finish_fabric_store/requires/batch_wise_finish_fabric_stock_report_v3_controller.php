			...
			<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
				<table cellpadding="0" cellspacing="0" width="700" border="1" rules="all" class="rpt_table">
					...
					<tr class="general">
						...
						<td align="center">
							<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+<? echo $companyID; ?>+'_'+document.getElementById('cbo_within_group').value, 'create_order_no_search_list_view', 'search_div', 'batch_wise_finish_fabric_stock_report_v3_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
						</td>
					</tr>
				</table>
...
if($action=="report_generate")
{
	...
	if($buyer_id>0)
	{
		$buyer_id_cond=" and b.buyer_id=$buyer_id" ;
	}
	else
	{
		$buyer_id_cond="";
	...
	if ($company_name=="") $company_cond=""; else $company_cond=" and a.company_id in ($company_name) ";
	...
	if ($booking_no!="")
	{
		$booking_no_cond=" and b.booking_no like '%$booking_no%'";
		$booking_no_cond2=" and b.to_ord_book_no like '%$booking_no%'";
	...
	/* $sql_main = "SELECT  a.knitting_source,a.knitting_company, b.id, f.id as batch_id, f.batch_no,f.extention_no,1 as type, sum(g.batch_qnty) as batch_qty, min(a.receive_date) as mrr_date, a.company_id, c.po_breakdown_id, b.body_part_id, b.fabric_description_id, $selectRcvStore b.uom, h.color as color_id,b.dia_width_type, b.width, b.gsm, (c.quantity) as quantity , sum(e.cons_amount) as amount,0 as is_transfered,0 as to_order_id, a.receive_basis, (e.order_amount) as order_amount, e.transaction_date,b.buyer_id,b.booking_no,CAST(i.style_ref_no AS VARCHAR2(4000))  as style_ref_no, 0 as from_prod_id
    from inv_receive_master a, pro_finish_fabric_rcv_dtls b, order_wise_pro_details c,wo_po_break_down d, 
    inv_transaction e, pro_batch_create_mst f, pro_batch_create_dtls g, product_details_master h,wo_po_details_master i 
    where a.id=b.mst_id  
    and b.id=c.dtls_id and b.order_id=d.id and
    c.trans_id=e.id and e.prod_id=h.id and b.batch_id=f.id and f.id=g.mst_id and 
    CAST(g.po_id AS VARCHAR2(4000)) =b.order_id and g.body_part_id = b.body_part_id 
	and d.job_id=i.id
	...
	$company_cond
	$buyer_id_cond
    $booking_no_cond  $year_search_cond
    $to_trans_date_cond
	$po_cond
    and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0   
    group by  a.knitting_source,a.knitting_company,b.id,f.id, f.batch_no,f.extention_no,a.company_id,c.po_breakdown_id,
    b.body_part_id,b.fabric_description_id, $groupByRcvStore b.uom, e.order_amount,h.color, b.dia_width_type, b.width, a.item_category, b.gsm,c.quantity,a.receive_basis, e.transaction_date,b.buyer_id, b.booking_no,i.style_ref_no
	union all
	select  0 as knitting_source,0 as knitting_company, b.id,f.id as batch_id, f.batch_no,f.extention_no,2 as type, sum(g.batch_qnty) as batch_qty, min(a.transfer_date) as mrr_date, a.company_id,a.from_order_id as po_breakdown_id, b.body_part_id as body_part_id,b.feb_description_id as fabric_description_id, $selectTransStore b.uom, c.color as color_id,b.dia_width_type, b.dia_width as width, b.gsm, (b.transfer_qnty) as quantity , sum(e.cons_amount) as amount,1 as is_transfered,a.to_order_id, 0 as receive_basis,  (e.order_amount) as order_amount, e.transaction_date, '0' as buyer_id, b.to_ord_book_no as booking_no, 'style_ref_no' as style_ref_no, b.from_prod_id
	from inv_item_transfer_mst a, inv_item_transfer_dtls b, inv_transaction e,pro_batch_create_mst f,pro_batch_create_dtls g, product_details_master c, wo_po_break_down d 
	where a.id=b.mst_id and b.to_trans_id=e.id and e.prod_id=c.id and b.batch_id=f.id and f.id=g.mst_id and b.from_order_id=d.id  and f.status_active=1 and g.status_active=1  $company_cond $po_cond $booking_no_cond2 $buyer_id_cond $TransStoreCond $year_search_cond and a.entry_form in(14) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $to_trans_date_cond
	group by  b.id,f.id,f.batch_no,f.extention_no,a.company_id,a.to_order_id, e.order_amount,b.from_prod_id, b.body_part_id, b.feb_description_id, $groupByTransStore b.uom, c.color,b.dia_width_type, b.dia_width, b.gsm,b.transfer_qnty,a.from_order_id, e.transaction_date,b.to_ord_book_no
	order by uom,po_breakdown_id"; //AND g.body_part_id = b.body_part_id */

	$sql_main = "SELECT  a.knitting_source,a.knitting_company, b.id, f.id as batch_id, f.batch_no,f.extention_no,1 as type, 0 as batch_qty, min(a.receive_date) as mrr_date, a.company_id, c.po_breakdown_id, b.body_part_id, b.fabric_description_id, $selectRcvStore b.uom, h.color as color_id,b.dia_width_type, b.width, b.gsm, (c.quantity) as quantity , sum(e.cons_amount) as amount,0 as is_transfered,0 as to_order_id, a.receive_basis, (e.order_amount) as order_amount, e.transaction_date,b.buyer_id,b.booking_no,CAST(i.style_ref_no AS VARCHAR2(4000))  as style_ref_no, 0 as from_prod_id
    from inv_receive_master a, pro_finish_fabric_rcv_dtls b, order_wise_pro_details c,wo_po_break_down d, 
    inv_transaction e, pro_batch_create_mst f, product_details_master h,wo_po_details_master i 
    where a.id=b.mst_id  
    and b.id=c.dtls_id and c.po_breakdown_id=d.id and c.trans_id=e.id and e.prod_id=h.id and b.batch_id=f.id
	and d.job_id=i.id and f.status_active=1 and a.entry_form=37 and c.entry_form=37
	$company_cond $buyer_id_cond $booking_no_cond $year_search_cond $to_trans_date_cond $po_cond
    and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
    group by  a.knitting_source,a.knitting_company,b.id,f.id, f.batch_no,f.extention_no,a.company_id,c.po_breakdown_id,
    b.body_part_id,b.fabric_description_id, $groupByRcvStore b.uom, e.order_amount,h.color, b.dia_width_type, b.width, a.item_category, b.gsm,c.quantity,a.receive_basis, e.transaction_date,b.buyer_id, b.booking_no,i.style_ref_no
	union all
	select  0 as knitting_source,0 as knitting_company, b.id,f.id as batch_id, f.batch_no,f.extention_no,2 as type, 0 as batch_qty, min(a.transfer_date) as mrr_date, a.company_id,a.from_order_id as po_breakdown_id, b.body_part_id as body_part_id,b.feb_description_id as fabric_description_id, $selectTransStore b.uom, c.color as color_id,b.dia_width_type, b.dia_width as width, b.gsm, (b.transfer_qnty) as quantity , sum(e.cons_amount) as amount,1 as is_transfered,a.to_order_id, 0 as receive_basis,  (e.order_amount) as order_amount, e.transaction_date, '0' as buyer_id, b.to_ord_book_no as booking_no, 'style_ref_no' as style_ref_no, b.from_prod_id
	from inv_item_transfer_mst a, inv_item_transfer_dtls b, inv_transaction e,pro_batch_create_mst f, product_details_master c, wo_po_break_down d 
	where a.id=b.mst_id and b.to_trans_id=e.id and e.prod_id=c.id and b.batch_id=f.id and b.from_order_id=d.id  and f.status_active=1  $company_cond $po_cond $booking_no_cond2 $buyer_id_cond $TransStoreCond $year_search_cond and a.entry_form in(14) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $to_trans_date_cond
	group by  b.id,f.id,f.batch_no,f.extention_no,a.company_id,a.to_order_id, e.order_amount,b.from_prod_id, b.body_part_id, b.feb_description_id, $groupByTransStore b.uom, c.color,b.dia_width_type, b.dia_width, b.gsm,b.transfer_qnty,a.from_order_id, e.transaction_date,b.to_ord_book_no
	...
	foreach($nameArray as $row)
	{
		...
		if($row[csf("quantity")] > 0)
		{
			...
			if($cbo_store_wise ==1)
			{
				...
				$ref_key =$row[csf("company_id")]."**".$row[csf("batch_id")]."**".$row[csf("fabric_description_id")]."**".$row[csf("gsm")]."**".$row[csf("width")]."**".$row[csf("body_part_id")]."**".$row[csf("dia_width_type")]."**".$row[csf("color_id")]."**".$row[csf("store_id")]."**".$row[csf("batch_no")]."**".$row[csf("extention_no")]."**".$row[csf("batch_qty")]."**".$row[csf("knitting_source")]."**".$row[csf("knitting_company")]."**".$row[csf("buyer_id")]."**".$row[csf("style_ref_no")]."**".$row[csf("booking_no")]."**".$row[csf("type")]."**".$row[csf("po_breakdown_id")]."**".$row[csf("from_prod_id")];
			}else{
				$sub_total_col_span = 21;
				$ref_key = $row[csf("company_id")]."**".$row[csf("batch_id")]."**".$row[csf("fabric_description_id")]."**".$row[csf("gsm")]."**".$row[csf("width")]."**".$row[csf("body_part_id")]."**".$row[csf("dia_width_type")]."**".$row[csf("color_id")]."**".$row[csf("batch_no")]."**".$row[csf("extention_no")]."**".$row[csf("batch_qty")]."**".$row[csf("knitting_source")]."**".$row[csf("knitting_company")]."**".$row[csf("buyer_id")]."**".$row[csf("style_ref_no")]."**".$row[csf("booking_no")]."**".$row[csf("type")]."**".$row[csf("po_breakdown_id")]."**".$row[csf("from_prod_id")];
			}
			...
			if( ($txt_date_from != "" && $txt_date_to != "") )
			{
				...
				else
				{
					if(strtotime($row[csf("transaction_date")]) < strtotime($txt_date_from))
					{
						if($cbo_store_wise ==1)
						{
							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("prod_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]][$row[csf("store_id")]]["open_rcv_qnty"] += $row[csf("quantity")];

							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("prod_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]][$row[csf("store_id")]]["open_balance_amt"] += $row[csf("order_amount")];
						}else{
							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("prod_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]]["open_rcv_qnty"] += $row[csf("quantity")];

							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("prod_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]]["open_balance_amt"] += $row[csf("order_amount")];
						}
	...
	foreach(sql_select($batch_sql) as $vals)
	{
		...
		$batch_qnty_arr_new[$vals[csf("mst_id")]][$vals[csf("body_part_id")]]+=$vals[csf("batch_qnty")];
	}
	/*echo "<pre>";
	...
	if(!empty($fso_id_arr))
	{
		...
		$fsoCond4 = $all_fso_cond4 = "";
		$fsoCond5 = $all_fso_cond5 = "";
		if($db_type==2 && count($fso_id_arr)>999)
		{
			...
			foreach($fso_id_arr_chunk as $chunk_arr)
			{
				...
				$fsoCond4.=" c.po_breakdown_id in(".implode(",",$chunk_arr).") or ";
				$fsoCond5.=" a.id in(".implode(",",$chunk_arr).") or ";
			}

			$all_fso_cond.=" and (".chop($fsoCond,'or ').")";
			$all_fso_cond2.=" and (".chop($fsoCond2,'or ').")";
			$all_fso_cond3.=" and (".chop($fsoCond3,'or ').")";
			$all_fso_cond4.=" and (".chop($fsoCond4,'or ').")";
			$all_fso_cond5.=" and (".chop($fsoCond5,'or ').")";
		}
		else
		{
			...
			$all_fso_cond4=" and c.po_breakdown_id in($fso_ids)";
			$all_fso_cond5=" and a.id in($fso_ids)";
		}

		
		$delivery_qnty_sql = sql_select("SELECT b.body_part_id bodypart_id,b.uom,b.width_type,sum(c.quantity) delivery_qnty, sum(a.order_amount) as amount, c.po_breakdown_id, b.batch_id,d.detarmination_id determination_id,d.gsm,d.dia_width dia, $selectRcvStore d.color color_id, a.transaction_date
		from inv_finish_fabric_issue_dtls b,order_wise_pro_details c,product_details_master d, inv_transaction a
		where b.id=c.dtls_id and c.prod_id=d.id and c.trans_id = a.id $company_cond $all_fso_cond2 $rcvStoreCond and b.status_active=1 and c.entry_form=18 and c.status_active=1 and a.status_active=1 $to_trans_date_cond2 group by b.body_part_id,b.uom, b.width_type, c.po_breakdown_id, b.batch_id,d.detarmination_id,d.gsm, $groupByRcvStore d.dia_width,d.color,a.transaction_date");
		...
		foreach ($delivery_qnty_sql as $row)  
		{
			if( ($txt_date_from != "" && $txt_date_to != "") )
			{
				if(strtotime($row[csf("transaction_date")]) >= strtotime($txt_date_from) && strtotime($row[csf("transaction_date")]) <= strtotime($txt_date_to))
				{
					if($cbo_store_wise ==1)
					{
						$delivery_qnty_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("determination_id")]][$row[csf("gsm")]][$row[csf("dia")]][$row[csf("bodypart_id")]][$row[csf("color_id")]][$row[csf("uom")]][$row[csf("store_id")]]["delivery_qnty"] += $row[csf("delivery_qnty")];
						$delivery_qnty_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("determination_id")]][$row[csf("gsm")]][$row[csf("dia")]][$row[csf("bodypart_id")]][$row[csf("color_id")]][$row[csf("uom")]][$row[csf("store_id")]]["delivery_amount"] += $row[csf("amount")];
					}
					else
					{
						$delivery_qnty_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("determination_id")]][$row[csf("gsm")]][$row[csf("dia")]][$row[csf("bodypart_id")]][$row[csf("color_id")]][$row[csf("uom")]]["delivery_qnty"] += $row[csf("delivery_qnty")];
						$delivery_qnty_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("determination_id")]][$row[csf("gsm")]][$row[csf("dia")]][$row[csf("bodypart_id")]][$row[csf("color_id")]][$row[csf("uom")]]["delivery_amount"] += $row[csf("amount")];
					}
				}
				else
				{
					if(strtotime($row[csf("transaction_date")]) < strtotime($txt_date_from))
					{
						if($cbo_store_wise ==1)
						{
							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("determination_id")]][$row[csf("gsm")]][$row[csf("dia")]][$row[csf("bodypart_id")]][$row[csf("color_id")]][$row[csf("uom")]][$row[csf("store_id")]]["open_iss_qnty"] += $row[csf("delivery_qnty")];
							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("determination_id")]][$row[csf("gsm")]][$row[csf("dia")]][$row[csf("bodypart_id")]][$row[csf("color_id")]][$row[csf("uom")]][$row[csf("store_id")]]["open_balance_amt"] -= $row[csf("amount")];
						}
						else
						{
							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("determination_id")]][$row[csf("gsm")]][$row[csf("dia")]][$row[csf("bodypart_id")]][$row[csf("color_id")]][$row[csf("uom")]]["open_iss_qnty"] += $row[csf("delivery_qnty")];
							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("determination_id")]][$row[csf("gsm")]][$row[csf("dia")]][$row[csf("bodypart_id")]][$row[csf("color_id")]][$row[csf("uom")]]["open_balance_amt"] -= $row[csf("amount")];
						}
					}
				}
			}
		...
		foreach ($issue_return_sql as $row)
		{
			if( ($txt_date_from != "" && $txt_date_to != "") )
			{
				if(strtotime($row[csf("transaction_date")]) >= strtotime($txt_date_from) && strtotime($row[csf("transaction_date")]) <= strtotime($txt_date_to))
				{
					if($cbo_store_wise ==1)
					{
						$issue_return_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]][$row[csf("store_id")]]["quantity"] += $row[csf("quantity")];
						$issue_return_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]][$row[csf("store_id")]]["amount"] += $row[csf("amount")];
					}
					else
					{
						$issue_return_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]]["quantity"] += $row[csf("quantity")];
						$issue_return_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]]["amount"] += $row[csf("amount")];
					}
				}
				else
				{
					if(strtotime($row[csf("transaction_date")]) < strtotime($txt_date_from))
					{
						if($cbo_store_wise ==1)
						{
							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]][$row[csf("store_id")]]["open_iss_ret_qnty"] += $row[csf("quantity")];
							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]][$row[csf("store_id")]]["open_balance_amt"] += $row[csf("amount")];
						}
						else
						{
							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]]["open_iss_ret_qnty"] += $row[csf("quantity")];
							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]]["open_balance_amt"] += $row[csf("amount")];
						}
					}
				}
			}
		...
		// echo "SELECT a.from_order_id as po_breakdown_id,b.batch_id as batch_id, b.to_prod_id as prod_id, b.to_body_part as body_part_id, b.feb_description_id as fabric_description_id, $selectTransOutStore b.uom, d.color as color_id,b.fabric_shade,b.dia_width_type, b.dia_width as width, b.gsm, sum(b.transfer_qnty) as quantity, sum(c.order_amount) as amount, c.transaction_date
		// from inv_item_transfer_mst a, inv_item_transfer_dtls b, inv_transaction c, product_details_master d
		// where a.id=b.mst_id and b.trans_id = c.id and c.prod_id=d.id and c.transaction_type=6 and a.entry_form in(14) $company_cond $all_fso_cond3 $to_trans_date_cond3 and a.status_active =1 and a.is_deleted =0 and b.status_active = 1 and b.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0
		// group by a.from_order_id, b.batch_id, b.to_prod_id, b.to_body_part, b.feb_description_id, $groupByTransOutStore b.uom, d.color,b.fabric_shade,b.dia_width_type, b.dia_width, b.gsm, c.transaction_date";
		$transfered_fabric_sql = sql_select("SELECT a.from_order_id as po_breakdown_id,b.batch_id as batch_id, b.to_prod_id as prod_id, b.body_part_id as body_part_id, b.feb_description_id as fabric_description_id, $selectTransOutStore b.uom, d.color as color_id,b.fabric_shade,b.dia_width_type, b.dia_width as width, b.gsm, sum(b.transfer_qnty) as quantity, sum(c.order_amount) as amount, c.transaction_date
		from inv_item_transfer_mst a, inv_item_transfer_dtls b, inv_transaction c, product_details_master d
		where a.id=b.mst_id and b.trans_id = c.id and c.prod_id=d.id and c.transaction_type=6 and a.entry_form in(14) $company_cond $all_fso_cond3 $to_trans_date_cond3 and a.status_active =1 and a.is_deleted =0 and b.status_active = 1 and b.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0
		group by a.from_order_id, b.batch_id, b.to_prod_id, b.body_part_id, b.feb_description_id, $groupByTransOutStore b.uom, d.color,b.fabric_shade,b.dia_width_type, b.dia_width, b.gsm, c.transaction_date");
		...
		foreach ($transfered_fabric_sql as $row)
		{
			if( ($txt_date_from != "" && $txt_date_to != "") )
			{
				if(strtotime($row[csf("transaction_date")]) >= strtotime($txt_date_from) && strtotime($row[csf("transaction_date")]) <= strtotime($txt_date_to))
				{
					if($cbo_store_wise ==1)
					{
						$transfered_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]][$row[csf("store_id")]]["quantity"] += $row[csf("quantity")];

						$transfered_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]][$row[csf("store_id")]]["amount"] += $row[csf("amount")];
					}
					else
					{
						$transfered_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]]["quantity"] += $row[csf("quantity")];
						$transfered_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]]["amount"] += $row[csf("amount")];
					}
				...
				else
				{
					if(strtotime($row[csf("transaction_date")]) < strtotime($txt_date_from))
					{
						if($cbo_store_wise==1)
						{
							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]][$row[csf("store_id")]]["open_trans_out"] += $row[csf("quantity")];
							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]][$row[csf("store_id")]]["open_balance_amt"] -= $row[csf("amount")];
						}
						else
						{

							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]]["open_trans_out"] += $row[csf("quantity")];
							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("color_id")]][$row[csf("uom")]]["open_balance_amt"] -= $row[csf("amount")];
						}
					}
				}
		...
		// echo "SELECT c.po_breakdown_id, c.entry_form , c.quantity, d.store_id, d.batch_id, e.detarmination_id, e.gsm, e.dia_width, d.body_part_id, d.width_type, e.color,d.uom, f.order_amount as amount, f.transaction_date
		// from order_wise_pro_details c, inv_finish_fabric_issue_dtls d, product_details_master e, inv_transaction f
		// where c.dtls_id = d.id and d.prod_id = e.id and c.trans_id = f.id and c.entry_form = 46 $all_fso_cond2 $to_trans_date_cond4 and  e.item_category_id =2 and c.status_active =1 and c.is_deleted = 0 and d.status_active =1 and d.is_deleted = 0 and f.status_active =1 and f.is_deleted = 0";
		$rcv_return_sql = sql_select("SELECT c.po_breakdown_id, c.entry_form , c.quantity, d.store_id, d.batch_id, e.detarmination_id, e.gsm, e.dia_width, d.body_part_id, d.width_type, e.color,d.uom, f.order_amount as amount, f.transaction_date
		from order_wise_pro_details c, inv_finish_fabric_issue_dtls d, product_details_master e, inv_transaction f
		where c.dtls_id = d.id and d.prod_id = e.id and c.trans_id = f.id and c.entry_form = 46 $all_fso_cond2 $to_trans_date_cond4 and  e.item_category_id =2 and c.status_active =1 and c.is_deleted = 0 and d.status_active =1 and d.is_deleted = 0 and f.status_active =1 and f.is_deleted = 0");
		...
		foreach ($rcv_return_sql as $row)
		{
			...
			if( ($txt_date_from != "" && $txt_date_to != "") )
			{
				if(strtotime($row[csf("transaction_date")]) >= strtotime($txt_date_from) && strtotime($row[csf("transaction_date")]) <= strtotime($txt_date_to))
				{
					if($cbo_store_wise ==1)
					{
						$rcv_ret_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("detarmination_id")]][$row[csf("gsm")]][$row[csf("dia_width")]][$row[csf("body_part_id")]][$row[csf("color")]][$row[csf("uom")]][$row[csf("store_id")]]["quantity"] += $row[csf("quantity")];
						$rcv_ret_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("detarmination_id")]][$row[csf("gsm")]][$row[csf("dia_width")]][$row[csf("body_part_id")]][$row[csf("color")]][$row[csf("uom")]][$row[csf("store_id")]]["amount"] += $row[csf("amount")];
					}
					else
					{
						$rcv_ret_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("detarmination_id")]][$row[csf("gsm")]][$row[csf("dia_width")]][$row[csf("body_part_id")]][$row[csf("color")]][$row[csf("uom")]]["quantity"] += $row[csf("quantity")];
						$rcv_ret_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("detarmination_id")]][$row[csf("gsm")]][$row[csf("dia_width")]][$row[csf("body_part_id")]][$row[csf("color")]][$row[csf("uom")]]["amount"] += $row[csf("amount")];
					}
				}
				else
				{
					if(strtotime($row[csf("transaction_date")]) < strtotime($txt_date_from))
					{
						if($cbo_store_wise ==1)
						{
							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("detarmination_id")]][$row[csf("gsm")]][$row[csf("dia_width")]][$row[csf("body_part_id")]][$row[csf("color")]][$row[csf("uom")]][$row[csf("store_id")]]["open_rec_ret_qnty"] += $row[csf("quantity")];
							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("detarmination_id")]][$row[csf("gsm")]][$row[csf("dia_width")]][$row[csf("body_part_id")]][$row[csf("color")]][$row[csf("uom")]][$row[csf("store_id")]]["open_balance_amt"] -= $row[csf("amount")];
						}
						else
						{
							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("detarmination_id")]][$row[csf("gsm")]][$row[csf("dia_width")]][$row[csf("body_part_id")]][$row[csf("color")]][$row[csf("uom")]]["open_rec_ret_qnty"] += $row[csf("quantity")];
							$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("batch_id")]][$row[csf("detarmination_id")]][$row[csf("gsm")]][$row[csf("dia_width")]][$row[csf("body_part_id")]][$row[csf("color")]][$row[csf("uom")]]["open_balance_amt"] -= $row[csf("amount")];
						}
					}
				}
			}
	...
	$sql_dtls ="SELECT a.company_id, a.supplier_id,  b.buyer_id, c.po_breakdown_id as order_id, d.id as prod_id, d.product_name_details, d.color , f.booking_no as booking_no_batch,f.id as batch_id
	...
	where a.id=b.mst_id and b.id = c.dtls_id and d.id=e.prod_id and b.prod_id=d.id  and c.trans_id=e.id and b.batch_id=f.id and a.item_category=2  and a.entry_form in(7,37) and c.entry_form in(7,37) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.trans_id<>0 and c.is_sales<>1 and c.status_active=1 and c.is_deleted=0 and c.po_breakdown_id is not null $all_fso_cond4 group by a.company_id, a.supplier_id, b.buyer_id, c.po_breakdown_id, d.id, d.product_name_details, d.color, f.booking_no, f.id order by d.id"; //  and c.po_breakdown_id in($orderID) and d.id in($prodID) and f.id in($batchID)
	//echo $sql_dtls;
	...
	{
		$tarnsferArry[$row[csf('order_id')]][$row[csf('prod_id')]][$row[csf('batch_id')]]['buyer_id']=$row[csf('buyer_id')];
		$tarnsferArry[$row[csf('order_id')]][$row[csf('prod_id')]][$row[csf('batch_id')]]['booking_no_batch']=$row[csf('booking_no_batch')];
		$tarnsferArry[$row[csf('order_id')]][$row[csf('prod_id')]][$row[csf('batch_id')]]['color']=$row[csf('color')];
		$tarnsferArry[$row[csf('order_id')]][$row[csf('prod_id')]][$row[csf('batch_id')]]['product_name_details']=$row[csf('product_name_details')];
	}
	unset($sql_result);

	$sql_order= sql_select("select a.id ,a.po_number, b.style_ref_no, a.grouping from wo_po_break_down a,wo_po_details_master b  where a.job_id=b.id and a.status_active=1 and a.is_deleted =0 $all_fso_cond5 ");
	...
	if($cbo_store_wise ==1)
	{
		$sub_total_col_span = 18;
	}else{
		$sub_total_col_span = 17;
	}
	if($txt_date_from != "" && $txt_date_to != "")
	{
		$receive_col_span = 5;
...
<fieldset style="width:2430px;">
	<table width="2530" cellspacing="0" cellpadding="0" border="0" rules="all" >
	...
	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="2450" class="rpt_table" >
		<thead>
			<tr>
			...
			<div style="width:2470px; overflow-y:scroll; max-height:350px;" id="scroll_body">
				<table cellspacing="0" cellpadding="0" border="1" rules="all" width="2450" class="rpt_table" id="tbl_list_search">
					<?
					$i=1;
					$pp=1;
						...
						foreach ($uom_data as $po_breakdown_id => $po_breakdown_data)
						{
							...
							foreach ($po_breakdown_data as $prod_ref => $row)
							{
								...
								if($cbo_store_wise ==1)
								{
									...
									$knitting_source  = $sales_prod_key_arr[12];
									$knitting_company  = $sales_prod_key_arr[13];
									$buyer_name  = $buyer_arr[$sales_prod_key_arr[14]];
									$style_ref  = $sales_prod_key_arr[15];
									$booking  = $sales_prod_key_arr[16];
									$type  = $sales_prod_key_arr[17];
									$poids  = $sales_prod_key_arr[18];
									$from_prod_id  = $sales_prod_key_arr[19];
								}
								else
								{
									...
									$knitting_source  = $sales_prod_key_arr[11];
									$knitting_company  = $sales_prod_key_arr[12];
									$buyer_name  = $buyer_arr[$sales_prod_key_arr[13]];
									$style_ref  = $sales_prod_key_arr[14];
									$booking  = $sales_prod_key_arr[15];
									$type  = $sales_prod_key_arr[16];
									$poids  = $sales_prod_key_arr[17];
									$from_prod_id  = $sales_prod_key_arr[18];
								}
								...
								if($type==1)
								{
									$booking_no = $booking;
									$buyer = $buyer_name;
									$style_ref_no= $style_ref;
								
								}
								else
								{
									$booking_no = $tarnsferArry[$poids][$from_prod_id][$prod_id]['booking_no_batch'];
									$buyer = $buyer_arr[$tarnsferArry[$poids][$from_prod_id][$prod_id]['buyer_id']];
									$style_ref_no = $orderArr[$poids]['style_ref_no'];
								}

								if($knitting_source==1)
								$dye_comp=$company_arr[$knitting_company];
								else
								$dye_comp=$supllier_arr[$knitting_company];
								...
								if($cbo_store_wise ==1)
								{
									...
									$delivery_qnty = $delivery_qnty_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id][$store_id]["delivery_qnty"];
									$delivery_amount = $delivery_qnty_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id][$store_id]["delivery_amount"];

									$transferOutQnty =  $transfered_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id][$store_id]["quantity"];
									$transferOutAmount =  $transfered_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id][$store_id]["amount"];

									$rcv_ret_qnty = $rcv_ret_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id][$store_id]["quantity"];
									$rcv_ret_amount = $rcv_ret_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id][$store_id]["amount"];

									$issue_return_qnty = $issue_return_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id][$store_id]["quantity"];
									$issue_return_amount = $issue_return_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id][$store_id]["amount"];

									$opening_balance_qnty = $opening_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id][$store_id]["open_rcv_qnty"];

									$opening_issue_qnty = $opening_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id][$store_id]["open_iss_qnty"];

									$opening_trans_out_qnty = $opening_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id][$store_id]["open_trans_out"];

									$opening_recv_rtn_qnty = $opening_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id][$store_id]["open_rec_ret_qnty"];

									$open_iss_ret_qnty = $opening_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id][$store_id]["open_iss_ret_qnty"];

									$opening_balance_amount = $opening_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id][$store_id]["open_balance_amt"];
								}else{
									$is_transfered  = $sales_prod_key_arr[8];
								

									$delivery_qnty = $delivery_qnty_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id]["delivery_qnty"];
									$delivery_amount = $delivery_qnty_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id]["delivery_amount"];
									$transferOutQnty =  $transfered_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id]["quantity"];
									$transferOutAmount =  $transfered_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id]["amount"];

									$rcv_ret_qnty = $rcv_ret_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id]["quantity"];
									$rcv_ret_amount = $rcv_ret_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id]["amount"];

									$issue_return_qnty = $issue_return_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id]["quantity"];
									$issue_return_amount = $issue_return_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id]["amount"];

									$opening_balance_qnty = $opening_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id]["open_rcv_qnty"];
									$opening_issue_qnty = $opening_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id]["open_iss_qnty"];
									$opening_trans_out_qnty = $opening_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id]["open_trans_out"];
									$opening_recv_rtn_qnty = $opening_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id]["open_rec_ret_qnty"];
									$open_iss_ret_qnty = $opening_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id]["open_iss_ret_qnty"];

									$opening_balance_amount = $opening_arr[$po_breakdown_id][$prod_id][$fabric_description_id][$gsm][$width][$body_part_id][$color_id][$uom_id]["open_balance_amt"];
								}
								...
								{
									if(number_format($total_stock_qty,2,'.','')!=0 || number_format($total_rcv_qnty,2,'.','')!=0 || number_format($total_issue_qnty,2,'.','')!=0)
									{
										...
										<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $m; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $m; ?>">
											...
											<td width="60" class="word_wrap_break" ><? echo $dye_comp; ?></td>
											<td width="70" class="word_wrap_break"><? echo $company_arr[$company_id];?></td>
											<td width="80" class="word_wrap_break"><? echo $buyer; ?></td>
											<td width="100" class="word_wrap_break"><? echo $style_ref_no; ?></td>
											...
											$batch_qty=number_format($batch_qnty_arr_new[$prod_id][$body_part_id],2);
											?>
											...
											<td width="100" class="word_wrap_break" align="center"><? echo $batch_no; ?></td>
											...
											<? if($txt_date_from != "" && $txt_date_to != ""){?>
												...
												<td width="80" class="word_wrap_break" align="right"><? echo number_format($row['trans_in_qnty'],2,'.',''); ?></td>
												<td width="80" class="word_wrap_break" align="right"><? echo number_format($total_rcv_qnty,2,'.',''); ?></td>
												<td width="80" class="word_wrap_break" align="right" title='<? echo "po = $po_breakdown_id, prod = $prod_id , deter = $fabric_description_id,gsm = $gsm, width=$width, body =$body_part_id , dia type = $dia_width_type , color = $color_id, uom_id = $uom_id";?>'>
											
												<a href="##" onClick="openmypage_issue('<? echo $pop_ref;?>','issue_finish_popup')"><? echo number_format($delivery_qnty,2,'.',''); ?></a></td>
												<td width="80" class="word_wrap_break" align="right"><? echo number_format($rcv_ret_qnty,2,'.',''); ?></td>
												<td width="80" class="word_wrap_break" align="right"><? echo number_format($transferOutQnty,2,'.',''); ?></td>
					...
					<div id="report_container" style="width:100%">
						...
						<div style="width:988px; overflow-y:scroll; max-height:250px;font-size:12px; overflow-x:hidden;" id="scroll_body">
							<table border="1" class="rpt_table" rules="all" width="970" cellpadding="0" cellspacing="0" id="table_body">
								<tbody>
									<?
									$sql_data="SELECT a.id as rcv_id,a.recv_number,a.receive_date,b.batch_id, c.po_breakdown_id, (c.quantity) as quantity,b.fabric_description_id, b.gsm,b.width,b.body_part_id,b.dia_width_type,b.color_id,b.no_of_roll,b.booking_no from inv_receive_master a, pro_finish_fabric_rcv_dtls b, order_wise_pro_details c, inv_transaction e where a.id=b.mst_id and b.id=c.dtls_id and c.trans_id=e.id and a.entry_form=37 and c.entry_form=37 and a.company_id=$companyID and c.po_breakdown_id=$po_breakdown_id and b.batch_id=$prod_id and b.fabric_description_id=$fabric_description_id and b.gsm=$gsm and b.width='$width' and b.body_part_id=$body_part_id and b.dia_width_type=$dia_width_type and b.color_id=$color_id and b.uom = $uom_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 ";
									...
									foreach($source_array as $row)
									{
										...
										$rcv_data_ref[$row[csf("rcv_id")]]["booking_no"] = $row[csf("booking_no")];
									...
									foreach( $data_array as $rcv_id => $rcv_data)
									{
										foreach ($rcv_data as $batch_id => $row)
										{
											...
											$booking_no = $rcv_data_ref[$rcv_id]["booking_no"];
		...
		<div style="width:918px; overflow-y:scroll; max-height:250px;font-size:12px; overflow-x:hidden;" id="scroll_body">
			<table border="1" class="rpt_table" rules="all" width="900" cellpadding="0" cellspacing="0" id="table_body">
				<tbody>
					...
					$sql_data=("SELECT a.id issue_id, a.issue_number, a.issue_date, b.batch_id, b.body_part_id bodypart_id,b.width_type,(c.quantity) delivery_qnty, c.po_breakdown_id, c.prod_id, d.detarmination_id determination_id,d.gsm,d.dia_width dia,  d.color color_id, a.booking_no from inv_issue_master a, inv_finish_fabric_issue_dtls b,order_wise_pro_details c,product_details_master d where a.id = b.mst_id and b.id=c.dtls_id and c.prod_id=d.id  and a.entry_form = 18 and b.status_active=1 and c.entry_form=18 and c.status_active=1 and a.company_id = $companyID and c.po_breakdown_id = $po_breakdown_id and b.batch_id = $prod_id and d.detarmination_id = $fabric_description_id and d.gsm= $gsm and d.dia_width = '$width' and b.body_part_id = $body_part_id and d.color = $color_id and b.uom = $uom_id");
					// and b.width_type = $dia_width_type
					echo $sql_data;
					$source_array=sql_select($sql_data);
					foreach($source_array as $row)
					{
					...
					foreach($source_array as $row)
					{
						...
						$iss_data_ref[$row[csf("issue_id")]]["booking_no"] = $row[csf("booking_no")];
					...
					foreach( $data_array as $issue_id => $iss_data)
					{
						foreach ($iss_data as $batch_id => $row)
						{
							...
							$booking_no = $iss_data_ref[$issue_id]["booking_no"];
			...
			<div style="width:650px; overflow-y:scroll; max-height:250px;font-size:12px; overflow-x:hidden;" id="scroll_body">
				<table border="1" class="rpt_table" rules="all" width="630" cellpadding="0" cellspacing="0" id="table_body">
					<tbody>
						...
						if($width){$dia_width_cond = " and b.dia_width='$width'";}

						$sql="SELECT  a.transfer_date, a.transfer_system_id, sum(b.no_of_roll) as no_of_roll, b.to_rack, b.to_shelf,sum(b.transfer_qnty) as quantity ,a.remarks from inv_item_transfer_mst a, inv_item_transfer_dtls b where a.id=b.mst_id and a.company_id=$companyID and a.to_order_id=$po_breakdown_id and b.to_batch_id=$prod_id and b.feb_description_id=$fabric_description_id $gsm_cond $dia_width_cond and b.to_body_part=$body_part_id and b.dia_width_type=$dia_width_type and b.color_id= $color_id and b.uom=$uom_id and a.entry_form in(14) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.transfer_date, a.transfer_system_id,b.no_of_roll, b.to_rack, b.to_shelf,a.remarks ";
			...
			<div style="width:650px; overflow-y:scroll; max-height:250px;font-size:12px; overflow-x:hidden;" id="scroll_body">
				<table border="1" class="rpt_table" rules="all" width="630" cellpadding="0" cellspacing="0" id="table_body">
					<tbody>
						...
						// $transfered_fabric_sql = sql_select("SELECT a.to_order_id as po_breakdown_id,b.to_batch_id as batch_id, b.to_prod_id as prod_id, b.to_body_part as body_part_id, b.feb_description_id as fabric_description_id, $selectTransOutStore b.uom, d.color as color_id,b.fabric_shade,b.dia_width_type, b.dia_width as width, b.gsm, sum(b.transfer_qnty) as quantity, sum(c.order_amount) as amount, c.transaction_date
						// from inv_item_transfer_mst a, inv_item_transfer_dtls b, inv_transaction c, product_details_master d
						// where a.id=b.mst_id and b.trans_id = c.id and c.prod_id=d.id and c.transaction_type=6 and a.entry_form in(14) $company_cond $all_fso_cond3 $to_trans_date_cond3 and a.status_active =1 and a.is_deleted =0 and b.status_active = 1 and b.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0
						// group by a.to_order_id, b.to_batch_id, b.to_prod_id, b.to_body_part, b.feb_description_id, $groupByTransOutStore b.uom, d.color,b.fabric_shade,b.dia_width_type, b.dia_width, b.gsm, c.transaction_date");


						$trans_out_sql= "SELECT a.transfer_date, a.transfer_system_id, a.remarks,b.rack, b.shelf, sum(b.transfer_qnty) as quantity, sum(b.no_of_roll) as no_of_roll from inv_item_transfer_mst a, inv_item_transfer_dtls b, inv_transaction c, product_details_master d where a.id=b.mst_id and b.trans_id = c.id and c.prod_id=d.id and c.transaction_type=6 and a.company_id=$companyID and a.entry_form in(14) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.from_order_id=$po_breakdown_id and b.batch_id=$prod_id and b.feb_description_id=$fabric_description_id and b.gsm=$gsm and b.dia_width='$width' and b.body_part_id=$body_part_id and b.dia_width_type=$dia_width_type and d.color=$color_id and b.uom = $uom_id group by a.transfer_date, a.transfer_system_id, a.remarks,  b.rack, b.shelf";
						//echo $trans_out_sql;
