...
<script>
	...
	function openmypage_booking_no()
	{
		...
		var page_link='requires/fso_wise_finish_fabric_stock_report_controller.php?action=fabricBooking_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id;
	...
	function openmypage_order()
	{
		...
		var buyer_name = $("#cbo_buyer_id").val();
		var page_link='requires/fso_wise_finish_fabric_stock_report_controller.php?action=order_no_search_popup&companyID='+companyID+'&buyer_name='+buyer_name;
	...
	function fn_report_generated(type)
	{
		...
		{
			var action ='report_generate_roll';
		}
		...
		freeze_window(3);
		http.open("POST","requires/fso_wise_finish_fabric_stock_report_controller.php",true);
	...
	function openmypage_receive(ref_data,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/fso_wise_finish_fabric_stock_report_controller.php?companyID='+companyID+'&ref_data='+ref_data+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../../');
	}
    	...
    	<fieldset style="width:1330px;">
            <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                <thead>
                    ...
                    <th><input type="reset" name="res" id="res" value="Reset" onClick="reset_form('knittingStatusReport_1','report_container*report_container2','','','')" class="formbutton" style="width:125px" /></th>
                </thead>
                <tbody>
                    <tr align="center" class="general">
                        <td><? echo create_drop_down( "cbo_company_id", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-Select Company-", $selected, "get_php_form_data( this.value, 'company_wise_report_button_setting','requires/fso_wise_finish_fabric_stock_report_controller' );" ); ?>
                        </td>
                        <td>
                        	<?
                        	echo create_drop_down( "cbo_within_group", 70, array("--Select--","Yes","No"),"", 0, "--Select--", 0, "fnc_bookingpopup(this.value);load_drop_down( 'requires/fso_wise_finish_fabric_stock_report_controller', this.value, 'load_drop_down_po_company', 'pocompany_td' );load_drop_down( 'requires/fso_wise_finish_fabric_stock_report_controller', this.value+'_'+document.getElementById('cbo_company_id').value, 'load_drop_down_buyer_within_no', 'buyer_td' );" );
                        	?>
                        </td>
                        <td id="pocompany_td"><?
                        ...
                        <td><? echo create_drop_down( "cbo_store_wise", 70, $yes_no,"", 0, "--Select--", 2, "load_drop_down( 'requires/fso_wise_finish_fabric_stock_report_controller', document.getElementById('cbo_company_id').value+'_'+this.value, 'load_drop_down_store', 'store_td' );" ); ?></td>
                        ...
                        <td>
							<input type="button" id="show_button_1" class="formbutton" style="width:60px; display: none;" value="Show" onClick="fn_report_generated(1)" />
							<input type="button" id="show_button_2" class="formbutton" style="width:60px; display: none;" value="Show 2" onClick="fn_report_generated(2)" />
                     ...
                     <tr>
                        ...
                        <td align="center">
						<input type="button" name="search" id="show_button_3" value="Excel Only" onClick="generate_report_exel_only(1)" style="width:70px; display: none;" class="formbutton" />
						</td>
                    </tr>
                </tbody>
            </table>
