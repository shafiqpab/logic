<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Batch Wise Finish Fabric Stock Report V3.
Functionality	:
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Batch Wise Finish Fabric Stock Report V3", "../../../", 1, 1,'',1,1);
?>
<script>
	...
	{
		if( form_validation('cbo_company_id','Po Company Name')==false )
		{
			return;
		}
		...
		var page_link='requires/batch_wise_finish_fabric_stock_report_v3_controller.php?action=fabricBooking_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id;
	...
	function openmypage_order___()
	{
		...
		var companyID = $("#cbo_company_id").val();
		var buyer_name = $("#cbo_buyer_id").val();
		var page_link='requires/batch_wise_finish_fabric_stock_report_v3_controller.php?action=order_no_search_popup&companyID='+companyID+'&buyer_name='+buyer_name;
	...
	function fn_report_generated(type)
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_buyer_id*cbo_year*txt_booking_no*txt_booking_id*txt_order_no*hide_order_id*cbo_store_wise*cbo_get_upto*txt_days*cbo_get_upto_qnty*txt_qnty*txt_date_from*txt_date_to*cbo_store_name',"../../../")+'&report_title='+report_title;
		freeze_window(3);
		http.open("POST","requires/batch_wise_finish_fabric_stock_report_v3_controller.php",true);
	...
	function openmypage_receive(ref_data,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/batch_wise_finish_fabric_stock_report_v3_controller.php?companyID='+companyID+'&ref_data='+ref_data+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../../');
	}
	...
	function getCompanyId() 
	{
	    ...
	    load_drop_down( 'requires/batch_wise_finish_fabric_stock_report_v3_controller',company_id, 'load_drop_down_buyer', 'buyer_td' );       
	}
    ...
    <div id="content_search_panel" >
    	<fieldset style="width:1330px;">
            <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr align="center" class="general" id="company_td">
                        <td><? echo create_drop_down( "cbo_company_id", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-Select Company-", $selected, "load_drop_down( 'requires/batch_wise_finish_fabric_stock_report_v3_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" ); ?>
                        </td>
                        ...
                        <td>
                            <input type="text" name="txt_order_no" id="txt_order_no" class="text_boxes" style="width:80px" placeholder="Write"  onChange="$('#hide_order_id').val('');">
                            <input type="hidden" name="hide_order_id" id="hide_order_id" readonly>
                        </td>
                        <td><? echo create_drop_down( "cbo_store_wise", 70, $yes_no,"", 0, "--Select--", 2, "load_drop_down( 'requires/batch_wise_finish_fabric_stock_report_v3_controller', document.getElementById('cbo_company_id').value+'_'+this.value, 'load_drop_down_store', 'store_td' );" ); ?></td>
