...
<script>
	...
	function generate_report(rpt_type)
	{
	    ...
	    var dataString = "&cbo_company_id="+cbo_company_id+"&cbo_report_type="+cbo_report_type+"&cbo_buyer_id="+cbo_buyer_id+"&txt_job_no="+txt_job_no+"&txt_file_no="+txt_file_no+"&txt_ref_no="+txt_ref_no+"&txt_order_no="+txt_order_no+"&txt_style_ref_no="+txt_style_ref_no+"&txt_batch_no="+txt_batch_no+"&from_date="+from_date+"&to_date="+to_date+"&report_title="+report_title+'&rpt_type='+rpt_type;

	    //alert(dataString);
		var data="action=generate_report"+dataString;
		//alert (data);
		freeze_window(1);
		http.open("POST","requires/ref_to_ref_transfer_report_controller.php",true);
            ...
            <fieldset style="width:1250px;">
                <table class="rpt_table" width="1250" cellpadding="0" cellspacing="0" border="1" rules="all">
                    ...
                    <tr class="general">
                    	<td>
                            <? 
                                echo create_drop_down( "cbo_company_id", 120, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/ref_to_ref_transfer_report_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>                            
                        </td>
                        <td>
	                    	<?
							$report_type=array(1=>"Knit Grey Fabric",2=>"Knit Finish Fabric");
