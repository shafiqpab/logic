...
Purpose			: 	This form will Create Knitting Delivery To Grey Receive WIP Report
Functionality	:
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Knitting Delivery To Grey Receive WIP Report", "../../../", 1, 1,'','','');
?>
<script>
	...
	function fn_report_generated(operation)
	{
		...
		freeze_window(5);
	 	var data="action=generated_report&operation="+operation+get_submitted_data_string('cbo_company_name*cbo_knitting_source*cbo_knitting_company*cbo_floor_id*cbo_buyer_name*cbo_year*txt_job_no*txt_int_ref*txt_style_no*txt_booking_no*txt_date_from*txt_date_to*cbo_based_on',    "../../../");
	 	// alert(data);release_freezing();return;

		http.open("POST","requires/grey_fabric_transection_report_controller.php",true);
 	...
 	function jobNumber()
	{
	 	...
	 	else
	 	{
 			...
 			var page_link = 'requires/grey_fabric_transection_report_controller.php?cbo_company_id=' + cbo_company_id + '&action=job_no_popup';
 	...
 	function delivery_roll_popup(delivery_id, fso_id, dtls_id, action)
    {
        ...
        emailwindow = dhtmlmodal.open('EmailBox', 'iframe', 'requires/grey_fabric_transection_report_controller.php?delivery_id=' + delivery_id + '&action=' + action + '&fso_id=' + fso_id + '&dtls_id=' + dtls_id, 'Detail Veiw', 'width=' + popup_width + ', height=300px,center=1,resize=0,scrolling=0', '../../');
    }
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td>
                                    <?
                                        echo create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company where status_active=1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select Company --", 0, "load_drop_down('requires/grey_fabric_transection_report_controller',this.value, 'load_drop_down_buyer','cbo_buyer_name_td' );" );
                                    ?>
                                </td>
                                <td>
                                	<?
                                	echo create_drop_down("cbo_knitting_source",65,$knitting_source,"",1,"-- Select --", 0,"load_drop_down( 'requires/grey_fabric_transection_report_controller', this.value+'_'+document.getElementById('cbo_company_name').value, 'load_drop_down_knitting_com','knitting_com')",0,"1,3");
									?>
                                </td>
                                <td id="knitting_com">
                                    <?
                                ...
                                <td>
                                	<?
                                		$based_on = array('1'=>'Knitting Delivery Date','2'=>'Grey Receive Date');
