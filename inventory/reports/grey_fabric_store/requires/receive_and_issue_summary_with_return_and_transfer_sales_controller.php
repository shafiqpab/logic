				...
				<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
					<table cellpadding="0" cellspacing="0" width="700" border="1" rules="all" class="rpt_table">
						...
						<tr class="general">
							...
							<td align="center">
								<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+<? echo $companyID; ?>+'_'+document.getElementById('cbo_within_group').value, 'create_order_no_search_list_view', 'search_div', 'receive_and_issue_summary_with_return_and_transfer_sales_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
							</td>
						</tr>
					</table>
...
if($action=="item_description_search")
{
	...
	<script>
		...
		{ 
			show_list_view ( document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_search_common').value+'_'+'<? echo $company; ?>'+'_'+document.getElementById('txt_prod_id').value, 'create_item_search_list_view', 'search_div', 'receive_and_issue_summary_with_return_and_transfer_sales_controller', 'setFilterGrid("list_view",-1)');
		}
	</script>
	<body>
...
if($action=="generate_report")
{
	...
	{
		$knitting_source_cond = " and x.knitting_source= $cbo_knitting_source ";
	}
	...
	$con = connect();
	execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_id and entry_form in (2021)");
	...
	|
	*/
	$receive_sql = "SELECT a.id as fso_id, a.within_group, a.job_no, a.sales_booking_no,a.buyer_id,a.po_job_no, a.po_buyer,a.season_id,a.delivery_date, a.booking_type,a.booking_without_order, a.booking_entry_form,a.booking_id,a.style_ref_no,x.remarks,a.sales_booking_no,a.customer_buyer, x.id,x.receive_date, x.recv_number,x.booking_no as delivery_challan, x.knitting_source, x.challan_no, c.transaction_type as trans_type, b.prod_id, b.febric_description_id, b.color_id, d.po_breakdown_id, d.barcode_no, d.qnty as rcv_qnty,d.entry_form
	FROM inv_receive_master x, pro_grey_prod_entry_dtls b, inv_transaction c, pro_roll_details d, fabric_sales_order_mst a
	WHERE x.id=b.mst_id and b.trans_id = c.id and x.id = d.mst_id and b.id = d.dtls_id and d.po_breakdown_id = a.id and d.is_sales=1 and d.entry_form in(58,84)  and x.entry_form in(58,84) and x.status_active=1 and x.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.item_category =13 AND x.company_id=$cbo_company_name $str_cond $date_cond $productIdCondition $knitting_source_cond";
    // echo $receive_sql;
	...
	foreach ($receive_sql_arr as  $val) 
	{
		...
		{
			$data_array[$val[csf("fso_id")]][$val[csf("recv_number")]][$val[csf("febric_description_id")]][$val[csf("color_id")]]["job_no"] = $val[csf("job_no")];
			$data_array[$val[csf("fso_id")]][$val[csf("recv_number")]][$val[csf("febric_description_id")]][$val[csf("color_id")]]["sales_booking_no"] = $val[csf("sales_booking_no")];
			$data_array[$val[csf("fso_id")]][$val[csf("recv_number")]][$val[csf("febric_description_id")]][$val[csf("color_id")]]["cust_buyer"] = $val[csf("customer_buyer")];
			$data_array[$val[csf("fso_id")]][$val[csf("recv_number")]][$val[csf("febric_description_id")]][$val[csf("color_id")]]["transation_date"] = $val[csf("receive_date")];
			$data_array[$val[csf("fso_id")]][$val[csf("recv_number")]][$val[csf("febric_description_id")]][$val[csf("color_id")]]["delivery_challan"] =  $val[csf("delivery_challan")];
			$data_array[$val[csf("fso_id")]][$val[csf("recv_number")]][$val[csf("febric_description_id")]][$val[csf("color_id")]]["trans_type"] =  $val[csf("trans_type")];
			$data_array[$val[csf("fso_id")]][$val[csf("recv_number")]][$val[csf("febric_description_id")]][$val[csf("color_id")]]["rcv_qnty"] +=  $val[csf("rcv_qnty")];
		}
	}
	...
	|
	*/
	$issue_sql="SELECT a.id as fso_id, a.within_group, a.job_no, a.sales_booking_no, a.buyer_id, a.po_job_no, a.po_buyer, a.booking_without_order, a.booking_entry_form, a.booking_id,a.customer_buyer, c.transaction_type as trans_type, x.issue_date, x.issue_number, x.knit_dye_source, d.po_breakdown_id, d.barcode_no, d.qnty as issue_qnty, d.entry_form 
	from inv_issue_master x, inv_grey_fabric_issue_dtls b, inv_transaction c, pro_roll_details d, fabric_sales_order_mst a
	where x.id = b.mst_id and b.trans_id=c.id and x.id=c.mst_id and b.id = d.dtls_id and x.id = d.mst_id and d.po_breakdown_id = a.id and x.entry_form = 61 and d.entry_form = 61 and c.transaction_type=2 and c.item_category=13 and d.status_active =1 and b.status_active =1 and x.status_active =1 AND x.company_id=$cbo_company_name $str_cond $date_cond $productIdCondition";
    //echo $issue_sql;//and x.issue_number='LSL-KGIR-23-00026'
    $issue_sql_arr=sql_select( $issue_sql);
	
	foreach ($issue_sql_arr as  $val)
	{
		$transIssBarcodearr[$val[csf("barcode_no")]] =$val[csf("barcode_no")];
	}
	...
	|
	*/
	$sqlNoOfRoll="SELECT a.id as fso_id, a.job_no, a.sales_booking_no, a.customer_buyer, a.buyer_id, a.style_ref_no, a.company_id, b.prod_id, b.po_breakdown_id, b.trans_type, e.qnty, e.barcode_no, f.transfer_system_id, f.transfer_date, f.from_order_id, f.to_order_id 
	...
	{
		$transIssBarcodearr[$row[csf("barcode_no")]] =$row[csf("barcode_no")];
	...
	if(!empty($transIssBarcodearr))
	{
		foreach($transIssBarcodearr as $barcodeno)
		{
			execute_query("insert into tmp_barcode_no (userid, barcode_no, entry_form) values ($user_id,$barcodeno,777)");
		}
		oci_commit($con);
	}
	...
	// fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 2022, 1,$transIssBarcodearr, $empty_arr);
	oci_commit($con);
	...
	if(!empty($transIssBarcodearr))
	{
		...
		where a.mst_id = c.id and a.id=b.dtls_id and b.barcode_no=d.barcode_no and d.userid=$user_id and d.entry_form=777 and c.entry_form = 2 and b.entry_form in(2) and a.trans_id=0 and a.status_active=1 and b.status_active=1");
	...
	if(!empty($transIssBarcodearr))
	{
		$split_chk_sql = sql_select("SELECT c.barcode_no, c.qnty from pro_roll_split b, pro_roll_details c, tmp_barcode_no d where b.entry_form =75 and b.split_from_id = c.roll_split_from and b.status_active=1 and c.status_active=1 and b.barcode_no=d.barcode_no and d.entry_form=777 and d.userid=$user_id");
		if(!empty($split_chk_sql))
		{
			foreach ($split_chk_sql as $row)
			{
				//$split_barcode_arr[$val[csf("barcode_no")]]= $val[csf("barcode_no")];
				if( $split_barcode_no_check[$row[csf('barcode_no')]] =="" )
		        {
		            $split_barcode_no_check[$row[csf('barcode_no')]]=$row[csf('barcode_no')];
		            $barcodeno = $row[csf('barcode_no')];
		            execute_query("insert into tmp_barcode_no (userid, barcode_no, entry_form) values ($user_id,$barcodeno,75)");
		        }
			}
			//fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 75, 3,$split_barcode_arr, $empty_arr);
			oci_commit($con);

			$split_ref_sql = sql_select("SELECT a.barcode_no, a.qnty, a.roll_id, b.barcode_no as mother_barcode from pro_roll_details a, pro_roll_details b, tmp_barcode_no c where a.entry_form = 61 and a.roll_id = b.id and a.status_active =1 and b.status_active=1  and a.barcode_no=c.barcode_no and c.entry_form=75 and c.userid=$user_id");
	...
	foreach ($issue_sql_arr  as $val) 
	{
		if(($prodBarcodeData[$val[csf("barcode_no")]]["knitting_source"] == $cbo_knitting_source) || $cbo_knitting_source ==0)
		{
			...
			$data_array[$val[csf("fso_id")]][$val[csf("issue_number")]][$febric_description_id][$color_id]["job_no"] = $val[csf("job_no")];
			$data_array[$val[csf("fso_id")]][$val[csf("issue_number")]][$febric_description_id][$color_id]["sales_booking_no"] = $val[csf("sales_booking_no")];
			$data_array[$val[csf("fso_id")]][$val[csf("issue_number")]][$febric_description_id][$color_id]["cust_buyer"] = $val[csf("customer_buyer")];
			$data_array[$val[csf("fso_id")]][$val[csf("issue_number")]][$febric_description_id][$color_id]["transation_date"] = $val[csf("issue_date")];
			$data_array[$val[csf("fso_id")]][$val[csf("issue_number")]][$febric_description_id][$color_id]["trans_type"] = $val[csf("trans_type")];
			$data_array[$val[csf("fso_id")]][$val[csf("issue_number")]][$febric_description_id][$color_id]["issue_qnty"] +=  $val[csf("issue_qnty")];
		}
	}
	// echo '<pre>';print_r($data_array);
	// ============================== Issue Data Array End =========================

	// ============================== Issue Return Data Array Start ================
	foreach ($receive_sql_arr as  $val) 
	{
		//$rcvTransBarcodeArr[$val[csf("barcode_no")]] =$val[csf("barcode_no")];
		if ($val[csf("entry_form")]==84) 
		{
			...
			$data_array[$val[csf("fso_id")]][$val[csf("recv_number")]][$val[csf("febric_description_id")]][$val[csf("color_id")]]["transation_date"] = $val[csf("receive_date")];
			$data_array[$val[csf("fso_id")]][$val[csf("recv_number")]][$val[csf("febric_description_id")]][$val[csf("color_id")]]["trans_type"] =  $val[csf("trans_type")];
			$data_array[$val[csf("fso_id")]][$val[csf("recv_number")]][$val[csf("febric_description_id")]][$val[csf("color_id")]]["iss_rtn_qty"] +=  $val[csf("rcv_qnty")];
		}
	}
	// ============================== Issue Return Data Array End ================

	// ============================== Transfer Data Array Start ====================
	foreach ($sqlNoOfRollResult  as $val) 
	{
		if(($prodBarcodeData[$val[csf("barcode_no")]]["knitting_source"] == $cbo_knitting_source) || $cbo_knitting_source ==0)
		{
			...
			$data_array[$val[csf("fso_id")]][$val[csf("transfer_system_id")]][$febric_description_id][$color_id]["job_no"] = $val[csf("job_no")];
			$data_array[$val[csf("fso_id")]][$val[csf("transfer_system_id")]][$febric_description_id][$color_id]["sales_booking_no"] = $val[csf("sales_booking_no")];
			$data_array[$val[csf("fso_id")]][$val[csf("transfer_system_id")]][$febric_description_id][$color_id]["cust_buyer"] = $val[csf("customer_buyer")];
			$data_array[$val[csf("fso_id")]][$val[csf("transfer_system_id")]][$febric_description_id][$color_id]["transation_date"] = $val[csf("transfer_date")];
			$data_array[$val[csf("fso_id")]][$val[csf("transfer_system_id")]][$febric_description_id][$color_id]["trans_type"] = $val[csf("trans_type")];

			if ($val[csf("trans_type")]==6) 
			{
				$data_array[$val[csf("fso_id")]][$val[csf("transfer_system_id")]][$febric_description_id][$color_id]["to_order_id"] = $val[csf("to_order_id")];
				$data_array[$val[csf("fso_id")]][$val[csf("transfer_system_id")]][$febric_description_id][$color_id]["trans_out_qty"] +=  $val[csf("qnty")];
			}
			else
			{
				$data_array[$val[csf("fso_id")]][$val[csf("transfer_system_id")]][$febric_description_id][$color_id]["from_order_id"] = $val[csf("from_order_id")];
				$data_array[$val[csf("fso_id")]][$val[csf("transfer_system_id")]][$febric_description_id][$color_id]["trans_in_qty"] +=  $val[csf("qnty")];
			}
		}
	}
	// echo '<pre>';print_r($data_array);
	...
	<div style="width:1810px" id="main_body">
		<table width="1810" id="" align="left">
		...
		<br />
		<table width="1790" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all" id="table_header_1" align="left">
			<thead>
				<tr>
					<th width="30">SL</th>
					<th width="110">FSO</th>
		...
		<div style="width:1810px; overflow-y: scroll; max-height:250px; float: left;" id="scroll_body">
			<table width="1790" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body" align="left">
				<tbody>
					<?
					$i=1;
					// echo "<pre>";print_r($data_array);
					foreach($data_array as $fso_id_k => $fso_id_v)
					{
						foreach($fso_id_v as $system_no_k => $transfer_v)
						{
							foreach($transfer_v as $detar_id_k => $detar_id_v)
							{
								...
								{
									if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
									// echo $row["trans_type"].'='.$system_no_k.'<br>';
									?>
									<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
		                                ...
		                                <td width="100" title="<?echo $detar_id_k;?>"><p><? echo $constructtion_arr[$detar_id_k];?></p></td>
		                                ...
		                                <td width="60"><p><? echo change_date_format($row["transation_date"]); ?></p></td>

		                                <td width="100"><? if ($row["trans_type"]==1) echo $row["delivery_challan"]; ?></td>
										<td width="110"><? if ($row["trans_type"]==1) echo $system_no_k; ?></td>
										<td width="80" align="right"><? echo number_format($row["rcv_qnty"],2,'.',''); ?></td>
										<td width="100"><? if ($row["trans_type"]==2) echo $system_no_k; ?></td>
										<td width="80" align="right"><? echo number_format($row["issue_qnty"],2,'.',''); ?></td>
										<td width="100"><? if ($row["trans_type"]==4) echo $system_no_k; ?></td>
										<td width="80" align="right"><? echo number_format($row["iss_rtn_qty"],2,'.',''); ?></td>

		                                <td width="110"><p><? if ($row["trans_type"]==5 || $row["trans_type"]==6) echo $system_no_k; ?></p></td>
									...
									$total_iss_rtn_qty += $row["iss_rtn_qty"];
		...
		<table width="1790" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all"  align="left">
			<tfoot>
				<tr>
					<th width="30"></th>
					<th width="110"></th>
					...
					<th width="100"></th>
					<th width="110"><strong>Total</strong></th>
					<th width="80" align="right" id="value_total_recv_qnty"><strong><? echo number_format($total_rcv_qnty,2,'.',''); ?></strong></th>
					<th width="100"></th>
					<th width="80" align="right" id="value_total_issue_qnty"><strong><? echo number_format($total_issue_qnty,2,'.',''); ?></strong></th>
					<th width="100"></th>
					<th width="80" align="right" id="value_total_return_qnty"><strong><? echo number_format($total_iss_rtn_qty,2,'.',''); ?></strong></th>
