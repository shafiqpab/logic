...
if($action=="generate_report")
{
    ...
    if($report_type ==3 || $report_type ==4)
    {
		...
		else
		{
			...
			$sql_max_period=sql_select("select max(b.PERIOD_STARTING_DATE) as PERIOD_STARTING_DATE, max(b.PERIOD_ENDING_DATE) as PERIOD_ENDING_DATE 
			from LIB_AC_PERIOD_MST a, LIB_AC_PERIOD_DTLS b where a.id=b.mst_id and a.company_id=$cbo_company_name and b.MONTH_ID not in(0,13,14) and b.IS_LOCKED=1");
			$max_close_period=$sql_max_period[0]["PERIOD_ENDING_DATE"];
			
			$trans_date_form=change_date_format($max_close_period,'','',1);
			//echo $trans_date_form;die;
			$max_close_time=strtotime($max_close_period);
			$from_date_time=strtotime($from_date);
			if($max_close_time>$from_date_time)
			{
				//$sql_max_period=sql_select("select max(b.PERIOD_STARTING_DATE) as PERIOD_STARTING_DATE, max(b.PERIOD_ENDING_DATE) as PERIOD_ENDING_DATE 
				//from LIB_AC_PERIOD_MST a, LIB_AC_PERIOD_DTLS b where a.id=b.mst_id and a.company_id=$cbo_company_name and b.MONTH_ID not in(0,13,14) and b.IS_LOCKED=1 and b.PERIOD_STARTING_DATE<'$from_date'");
				$sql_qury="SELECT b.id as id,b.detarmination_id, b.gsm, b.dia_width,
				sum(case when a.transaction_type=1 and a.transaction_date<'".$from_date."' then a.cons_quantity else 0 end) as rcv_total_opening,
				sum(case when a.transaction_type=2 and a.transaction_date<'".$from_date."' then a.cons_quantity else 0 end) as iss_total_opening,
				sum(case when a.transaction_type=3 and a.transaction_date<'".$from_date."' then a.cons_quantity else 0 end) as rcv_return_opening,
			...
			{
				if($cbo_store_name>0) $stor_close_cond=" and a.REF_ID=$cbo_store_name";
				$sql_closing="select a.PROD_ID, a.CLOSING_RECEIVE, a.CLOSING_ISSUE, a.CLOSING_RECEIVE_RTN, a.CLOSING_ISSUE_RTN, a.CLOSING_TRANSFER_IN, a.CLOSING_TRANSFER_OUT, a.CLOSING, a.LAST_RATE 
				from YEAR_CLOSE_ITEM_REF a, PRODUCT_DETAILS_MASTER b 
				where a.prod_id=b.id and b.item_category_id=$cbo_item_category_id and a.COMPANY_ID=$cbo_company_name and b.COMPANY_ID=$cbo_company_name and b.status_active=1 and b.is_deleted=0 and a.REF_TYPE=1 $stor_close_cond";
				//echo $sql_closing;die;
				$sql_closing_result=sql_select($sql_closing);
				$year_closing_data=array();
				foreach($sql_closing_result as $val)
				{
					$year_closing_data[$val["PROD_ID"]]["CLOSING_RECEIVE"]+=$val["CLOSING_RECEIVE"];
					$year_closing_data[$val["PROD_ID"]]["CLOSING_ISSUE"]+=$val["CLOSING_ISSUE"];
					$year_closing_data[$val["PROD_ID"]]["CLOSING_RECEIVE_RTN"]+=$val["CLOSING_RECEIVE_RTN"];
					$year_closing_data[$val["PROD_ID"]]["CLOSING_ISSUE_RTN"]+=$val["CLOSING_ISSUE_RTN"];
					$year_closing_data[$val["PROD_ID"]]["CLOSING_TRANSFER_IN"]+=$val["CLOSING_TRANSFER_IN"];
					$year_closing_data[$val["PROD_ID"]]["CLOSING_TRANSFER_OUT"]+=$val["CLOSING_TRANSFER_OUT"];
					$year_closing_data[$val["PROD_ID"]]["CLOSING"]+=$val["CLOSING"];
					$year_closing_data[$val["PROD_ID"]]["LAST_RATE"]+=$val["LAST_RATE"];
				}
				...
				sum(case when a.transaction_type in (2,3,6) and a.transaction_date between '".$from_date."' and '".$to_date."' then a.cons_amount else 0 end) as total_issue_value
				from inv_transaction a, product_details_master b
				where a.prod_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.transaction_date>='$trans_date_form' $company_id $prod_company_id $item_category_id $store_id $item_account 
				group by b.id, b.detarmination_id, b.gsm, b.dia_width
				union all
				SELECT   c.id as id,c.detarmination_id, c.gsm, c.dia_width,
				sum(case when a.transfer_criteria=2 and a.transfer_date<'".$from_date."' then b.transfer_qnty else 0 end) as rcv_total_opening,
				...
				sum(case when a.transfer_criteria in (2) and a.transfer_date between '".$from_date."' and '".$to_date."' then b.transfer_qnty else 0 end) as total_issue_value
				from inv_item_transfer_mst a,inv_item_transfer_dtls b, product_details_master c
				where a.id=b.mst_id and b.to_prod_id=c.id $prod_company_id_trnsf_prod $item_category_id $item_account_trnsf $store_id_trnsf and a.transfer_criteria=2 and a.transfer_date  between '".$from_date."' and '".$to_date."' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.transfer_date>='$trans_date_form'
				group by c.id,c.detarmination_id, c.gsm, c.dia_width  order by id ASC";
			}
			//echo $max_close_time."=".$from_date_time;die;
		}
        //echo $sql_qury;//die;   
        ...
        {
            $prod_ids.=$row[csf("id")].',';
            $data_array[$row[csf("id")]]['rcv_total_opening']=$row[csf("rcv_total_opening")]+$year_closing_data[$row[csf("id")]]["CLOSING_RECEIVE"];
            $data_array[$row[csf("id")]]['iss_total_opening']=$row[csf("iss_total_opening")]+$year_closing_data[$row[csf("id")]]["CLOSING_ISSUE"];
            $data_array[$row[csf("id")]]['rcv_return_opening']=$row[csf("rcv_return_opening")]+$year_closing_data[$row[csf("id")]]["CLOSING_RECEIVE_RTN"];
            $data_array[$row[csf("id")]]['iss_return_opening']=$row[csf("iss_return_opening")]+$year_closing_data[$row[csf("id")]]["CLOSING_ISSUE_RTN"];
            ...
            $data_array[$row[csf("id")]]['transfer_in_opening']=$row[csf("transfer_in_opening")]+$year_closing_data[$row[csf("id")]]["CLOSING_TRANSFER_IN"];
            $data_array[$row[csf("id")]]['transfer_out_opening']=$row[csf("transfer_out_opening")]+$year_closing_data[$row[csf("id")]]["CLOSING_TRANSFER_OUT"];
                            ...
                            )  
                        {
                            ...
                            if(($cbo_value_with ==1 && (number_format($opening,2,'.','')!= 0 || number_format($total_value_opening,2,'.','')!=0 || number_format($receive,2,'.','')!= 0 ||number_format($issue_return,2,'.','')!=0 || number_format($transfer_in,2,'.','')!= 0 || number_format($totalReceive,2,'.','')!=0 || number_format($total_rcv_value,2,'.','')!= 0 || number_format($issue,2,'.','')!=0 || number_format($rec_return,2,'.','')!=0 || number_format($transfer_out,2,'.','')!= 0 || number_format($totalIssue,2,'.','')!=0) ) || ($cbo_value_with ==0 && (number_format($opening,2,'.','')== 0 || number_format($total_value_opening,2,'.','')==0 || number_format($receive,2,'.','')== 0 ||number_format($issue_return,2,'.','')==0 || number_format($transfer_in,2,'.','')== 0 || number_format($totalReceive,2,'.','')==0 || number_format($total_rcv_value,2,'.','')== 0 || number_format($issue,2,'.','')==0 || number_format($rec_return,2,'.','')==0 || number_format($transfer_out,2,'.','')== 0 || number_format($totalIssue,2,'.','')==0)))
                            {
                                ...
                                <tr bgcolor="<? echo $bgcolor;?>"<? echo $stylecolor;?> onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                                    ...
                                    <td width="80" align="right"><? echo number_format($transfer_out,2);?></td>
	...
	elseif($report_type == 5)
	{
		...
		$sql_order="Select p.id as prod_id, p.item_description, p.item_group_id, p.current_stock, p.avg_rate_per_unit, p.company_id, d.buyer_name, b.id as prop_id,
    		...
    		<div id="order_wise_stock_container" style="width: <? echo $div_width; ?>px; padding: 10px 0;" class="order_basis">
    			...
    			<table id="table_body" width="<? echo $table_width; ?>" border="1" cellspacing="0" cellpadding="0" class="rpt_table" rules="all">
    				...
    				foreach($report_arr as $buyer_id => $trans_type)
    				{
    					...
    					if($cbo_value_with==1)
    					{
    						if($closing_value>=1 || $opening_value>=1 || $rcv_total>=1 || $issue_total>=1)
    						{
                                ...
                                <tr bgcolor="<? echo $bgcolor; ?>" <? echo $stylecolor; ?> onclick="change_color('tr_<? echo $sl; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $sl; ?>">
                                    ...
                                    <td width="150" class="wrd_brk left"><? echo $buyer_arr[$buyer_id]; ?></td>
    					...
    					{
    						if($closing_value>=0 || $opening_value>=0 || $rcv_tota>=0 || $issue_total>=0)
    						{  
    							?>
    							<tr bgcolor="<? echo $bgcolor; ?>" <? echo $stylecolor; ?> onclick="change_color('tr_<? echo $sl; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $sl; ?>">
    								<td width="40" class="wrd_brk center"><? echo $sl; ?></td>
