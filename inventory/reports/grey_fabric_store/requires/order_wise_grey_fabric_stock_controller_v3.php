...
if($action=="report_generate")
{
	...
	if($db_type==0)
	{
		$year_cond_validation_a=" year(a.insert_date) >=2020";
		$year_cond_validation_c=" year(c.insert_date) >=2020";
	}
	else if($db_type==2)
	{
		$year_cond_validation_a=" TO_CHAR(a.insert_date,'YYYY') >=2020";
		$year_cond_validation_c=" TO_CHAR(c.insert_date,'YYYY') >=2020";
	}
	...
	where $year_cond_validation_a and b.id=a.job_id and a.id=g.po_breakdown_id and g.dtls_id=d.id and d.trans_id=c.id and c.transaction_type=1  and c.mst_id=e.id and d.mst_id=e.id and b.status_active=1 and b.is_deleted=0 and e.entry_form in (2,22,58) and g.entry_form in(2,22,58) and d.status_active=1 and d.is_deleted=0 and g.status_active=1 and g.is_deleted=0 AND e.company_id=$cbo_company_id and e.item_category=13 and e.status_active=1 and e.is_deleted=0 and g.booking_without_order=0
	$year_cond $buyer_id_cond $job_no_cond $search_cond $receive_date $order_cond $po_id_cond $po_date_cond $store_cond";
    // echo $sql_recv;die;// and d.prod_id=3452 and d.yarn_lot='K79NGR'

	$recvArray=sql_select( $sql_recv ); // echo $sql_recv;die;
	...
	WHERE $year_cond_validation_c and d.status_active = 1 AND d.is_deleted = 0 AND e.status_active = 1 AND e.is_deleted = 0 AND e.entry_form IN(82,83,110,183) AND a.entry_form IN(82,83,110,183) AND e.trans_type IN(5,6) AND f.status_active = 1 AND f.is_deleted = 0 AND b.status_active = 1 AND b.is_deleted = 0
	$buyer_id_cond_trans $year_cond_trans $job_no_cond_trans $order_id_cond_trans $trans_order_cond $po_date_trans_cond $trans_in_date $search_trans_cond $store_trans_cond $po_id_cond_c"; 
	//echo $trans_in_out_sql; die;//  and e.prod_id=3452 and b.yarn_lot='K79NGR'
	...
	foreach($trans_in_outResult as $row)
	{
		...
		// echo $color_range_ids.'='.$row[csf("barcode_no")].'<br>';
		...
		if($row[csf('trans_type')] == 5)
		{
			...
			$data_arr[$row[csf("job_no")]][$str_ref]['barcode_no'] .=$row[csf("barcode_no")].",";
			//$test_in_barcode_arr[$row[csf("barcode_no")]]=$row[csf("barcode_no")].'='.$row[csf('rcv_qty')];
		}
		if($row[csf('trans_type')] == 6)
		{
			$transf_out_data_arr[$row[csf("job_no")]][$str_ref]['barcode_no'] .=$row[csf("barcode_no")].",";
			...
			//$test_out_barcode_arr[$row[csf("barcode_no")]]=$row[csf("barcode_no")].'='.$row[csf('rcv_qty')];
		}
	}
	unset($trans_in_outResult);
	// echo "<pre>";print_r($test_out_barcode_arr);die;
	// =============== Transfer In End =============================
	if(empty($data_arr))
	{
		echo "Data Not Found";
	...
	// =============== Issue query Start ===========================
	$sql_issue="SELECT p.job_no_mst, a.po_breakdown_id, d.prod_id, b.yarn_count, b.yarn_lot, d.store_id as store_name, d.floor_id, d.room, d.rack, d.self, d.bin_box, a.qnty as issue_qnty, a.id as issue_roll, a.barcode_no
	from GBL_TEMP_ENGINE g, wo_po_break_down p, pro_roll_details a, inv_grey_fabric_issue_dtls b, inv_transaction d
	where g.ref_val=p.id and p.id=a.po_breakdown_id and a.dtls_id=b.id and b.trans_id=d.id and d.transaction_type in (2) and d.status_active=1 and d.item_category=13 and g.entry_form=67 and g.ref_from=1 and g.user_id=$user_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form in(61)";
	//echo $sql_issue;die;//  and b.prod_id=3452 and b.yarn_lot='K79NGR'
	...
	foreach ($result_sql_issue as $row)
	{
		...
		$issue_array[$row[csf("job_no_mst")]][$str_ref]['barcode_no'].=$row[csf("barcode_no")].',';
		$issue_array[$row[csf("job_no_mst")]][$str_ref]['issue_roll']++;

		//$test_issue_barcoe_array[$row[csf("barcode_no")]]=$row[csf("barcode_no")].'='.$row[csf("issue_qnty")];
	}	
	// echo "<pre>";print_r($test_issue_barcoe_array);die;
	// =============== Issue query End =============================
	...
	where $year_cond_validation_c and a.id=b.trans_id and a.id=f.TRANS_ID and f.id=b.dtls_id and A.PROD_ID=e.id and b.po_breakdown_id=c.id and c.id=d.ref_val and d.user_id=$user_id and d.entry_form=67 and d.ref_from=1 and a.status_active=1 and a.is_deleted=0 and c.job_id=g.id and g.status_active=1 and g.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.item_category=13 and b.entry_form in(84) and a.company_id=$cbo_company_id and a.transaction_type in(4) and b.trans_type in (4)
	...
	// echo $sql_retn;die;//  and a.prod_id=3452 and f.yarn_lot='K79NGR'
	...
	foreach($data_retn_array as $row )
	{
		...
		$retn_arr[$row[csf("job_no_mst")]][$str_ref]['rcv_rtn_qty']=0;
	...
	$sql="SELECT a.buyer_id,b.job_no, b.po_break_down_id as po_id,b.construction,b.fabric_color_id, b.grey_fab_qnty as grey_req_qnty, a.booking_no 
	...
	from GBL_TEMP_ENGINE b, order_wise_pro_details e, pro_grey_prod_entry_dtls g, inv_receive_master d
	where b.ref_val=e.prod_id and e.dtls_id=g.id and g.mst_id=d.id and d.entry_form in(2,22,58,84) and e.entry_form in(2,22,58,84) and b.entry_form=67 and b.ref_from=2 and b.user_id=$user_id ";
	// echo $sqlYarn;die;
			...
			<table width="3140" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table" align="left" id="table_body">
				...
				foreach ($data_arr as $job => $str_arr)
				{
					...
					{								
						...
						// echo $str_ref_arr.'='.$issue_retn.'<br>';
						...
						if($cbo_value_with==1 ) 
						{
							...
							<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
								...
								<td width="80" title="<?=$order_id;?>"><p><? echo $po_number; echo '<br>PO ID:'.$order_id; ?></p></td>
								...
								<td width="100" title="Prod ID: <? echo $prod_id;?>"><p><? echo $constructionArr[$product_array[$prod_id]['detarmination_id']]; echo '<br>Prod ID:'.$prod_id; ?></p></td>
								...
								<td width="60" align="right"><p><? $stock_roll_qty=$rec_roll-$iss_roll; 
								if ($stock_roll_qty>0) {echo $stock_roll_qty;} else { echo '0.00';} 
								if ($stock_roll_qty>0) {$tot_stock_roll_qty+=$stock_roll_qty;} ?>&nbsp;</p></td>
						...
						else if($cbo_value_with==2 && number_format($stock,2,".","")>0) 
						{
							...
							<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
								...
								<td width="80"><p><? echo $job_no;?></p></td>
								<td width="80"><p><? echo $buyer_name; ?></p></td>
								<td width="80" title="<? echo $order_id; ?>"><p><? echo $po_number; ?></p></td>
	                            <td width="70" align="center"><p><? echo change_date_format($row["pub_shipment_date"]); ?></p></td>
	                            <td width="70" align="center"><p><? if($row["status_active"]==3) echo change_date_format($row["calcel_date"]); ?></p></td>
								<td width="80"><p><? echo $style_ref_no; ?></p></td>
								<td width="100"><p><? echo $program_no; ?>&nbsp;</p></td>
				...
				<tr class="tbl_bottom">
					...
					<td width="60" align="right"><? if ($grand_tot_roll_qty>0) echo $grand_tot_roll_qty; else echo '0.00';?>&nbsp;</td>
					<td width="50" align="right">&nbsp;</td>
					<td width="50" align="right">&nbsp;</td>
					<td width="50" align="right">&nbsp;</td>
					<td width="50" align="right">&nbsp;</td>
	             ...
	             <div style="width:450px; max-height:410px; overflow-y:scroll" id="scroll_body">
	                <table border="1" class="rpt_table" rules="all" width="430" cellpadding="0" cellspacing="0" id="table_body">
	                    <?
							...
							{
								$issue_return_cond.=" and d.yarn_lot ='$issueRetrn_yarn_lot'";
							}
...
if($action=="generate_report_excel_only")
{
	$process = array( &$_POST );
	extract(check_magic_quote_gpc( $process ));
	$rpt_type=str_replace("'","",$rpt_type);
	...
	//ob_start();
	$html = "";
	$html .= '<table cellpadding="0" cellspacing="0" width="2770">
			<tr  class="form_caption" style="border:none;">
				<td align="center" width="100%" colspan="36" style="font-size:18px"><strong>'. $report_title . '</strong></td>
			</tr>
			<tr  class="form_caption" style="border:none;">
				<td align="center" width="100%" colspan="36" style="font-size:16px"><strong>'. $company_arr[str_replace("'","",$cbo_company_id)] .'</strong></td>
			</tr>
			<tr  class="form_caption" style="border:none;">
				<td align="center" width="100%" colspan="36" style="font-size:14px"><strong>'; 
				if($date_from!="") 
				$html .= "Upto : ".change_date_format(str_replace("'","",$txt_date_from_trans)) ;
		...
		<table width="3140" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table" align="left">
			...
			<tbody>';
						...
						if($cbo_value_with==1 ) 
						{
							...
							if($con_arr[$groupKey]=="")
							{
								if($ttt!=1)
								{
									...
									$html .= '<tr class="tbl_bottom">
										...
										<td width="80"></td>
										<td width="120"></td>
										<td width="80">'. number_format($grey_qnty,2,'.','').'</b></p></td>
										<td width="80">'. number_format($tot_rec_qty,2,'.','').'</td>
										<td width="80">'. number_format($tot_iss_retn_qty,2,'.','').'</td>
										<td width="80">'. number_format($tot_transfer_in_qty,2,'.','').'</td>
										<td width="80">'. number_format($tot_rec_bal,2,'.','').'</td>
										<td width="60">'. $tot_rec_roll.'</td>
										<td width="80">'. number_format($tot_issue_qty,2,'.','').'</td>
										<td width="80">'. number_format($tot_recv_retn_qty,2,'.','').'</td>
										<td width="80">'. number_format($tot_transfer_out_qty,2,'.','').'</td>
										<td width="80">'. number_format($tot_issue_bal,2,'.','').'</td>
										<td width="60">'. $tot_issue_roll.'</td>
										<td width="80">'. number_format($tot_stock,2,'.','').'</td>
										<td width="60">&nbsp </td>
										<td>&nbsp;</td>
										<td>&nbsp;</td>
										<td>&nbsp;</td>
										...
										<td>&nbsp;</td>
										<td>&nbsp;</td>
										<td>'. number_format($grey_qnty-$tot_rec_bal,2,'.','').'</td>
										<td>'. number_format($grey_qnty-$tot_issue_bal,2,'.','').'</td>
									</tr>';
							...
							$html .='<tr>
								...
								if($row["status_active"]==3) {
									$html .= change_date_format($row["calcel_date"]);
								}
								$html .= '</td>
								<td>'. $style_ref_no.'</td>
								<td>'. $program_no.'</td>
								<td>'. implode(",",array_unique(explode(",",chop($booking_array[$order_id]['booking_no'],",")))).'</td>					
								<td>'. $constructionArr[$product_array[$prod_id]['detarmination_id']].'</td>
								<td>'. $copmpositionArr[$product_array[$prod_id]['detarmination_id']].'</td>
								<td>'. $product_array[$prod_id]['gsm'].'</td>
								<td>'. $product_array[$prod_id]['dia_width'].'</td>
								<td>'. $machine_arr[$row['machine_no_id']].'</td>
								<td>'. implode(",",array_unique(explode(",",chop($row['stitch_length'],",")))).'</td>
								<td>'. $color_name.'</td>
								<td>'. $color_range_name.'</td>
								<td>'. $count_val.'</td> 
								<td>'. $brand_name.'</td>
								<td>'. $yarn_lot.'</td>
								<td>'. implode(",",array_unique(explode(",",chop($row['booking_no'],",")))).'</td>
								<td></td>
								<td>'.number_format($row['quantity'],2).'</td>';
									$tot_rec_qty+=$row['quantity'];
								$html .='<td>'. number_format($issue_retn,2).'</td>'; 
									$tot_iss_retn_qty+=$issue_retn;
								$html .='<td>'. number_format($row['transfer_in_qnty'],2). '</td>';
									$tot_transfer_in_qty+=$row['transfer_in_qnty'];
								$html .='<td>'. number_format($rec_bal,2).'</td>';
									$tot_rec_bal+=$rec_bal;
									$rec_roll=$row['rec_roll']+$trnsfer_in_roll;
								$html .='<td>'. $rec_roll. '</td>'; 
									$tot_rec_roll+=$rec_roll;						
								$html .='<td>'. number_format($issue_qty,2).'</td>';
									$tot_issue_qty+=$issue_qty;
								$html .='<td>'. number_format($recv_retn,2).'</td>';
									$tot_recv_retn_qty +=$recv_retn;
								$html .='<td>'. number_format($trnsfer_out_qty,2).'</td>';
									$tot_transfer_out_qty+=$trnsfer_out_qty;
								$html .='<td>'. number_format($issue_bal,2).'</td>';
								$tot_issue_bal+=$issue_bal;
								$iss_roll=$issue_roll;
								$html .='<td>'.$iss_roll.'</td>';	
								$tot_issue_roll+=$iss_roll;					
								$html .='<td>'.number_format($stock,2).'</td>';
								$tot_stock+=$stock; 
								$html .='<td>';
									$stock_roll_qty=$rec_roll-$iss_roll; 
								if ($stock_roll_qty>0) {$html .= $stock_roll_qty;} else { $html .= '0.00';} 
								if ($stock_roll_qty>0) {
									$tot_stock_roll_qty+=$stock_roll_qty;
								} 
								$html .='</td>
								<td>'. $store_array[$store] .'</td>	
								<td>'. $floor_room_rack_array[$floor].'</td>	
								<td>'. $floor_room_rack_array[$room].'</td>		
								<td>'. $floor_room_rack_array[$rack].'</td>
								<td>'. $floor_room_rack_array[$self].'</td>';
								$daysOnHand = datediff("d",change_date_format($transaction_date_array[$prod_id]['max_date'],'','',1),change_date_format(date("Y-m-d"),'','',1));			
								$html .='<td>';
								if($stock>0) $html .= $daysOnHand; 
								$html .='</td>
								<td></td>
								<td></td>
							</tr>';
							...
							if($con_arr[$groupKey]=="")
							{
								if($ttt!=1)
								{
									...
									$html .='<tr>
										...
										<td></td>
										<td></td>
										<td>'. number_format($grey_qnty,2,'.','') .'</td>
										<td>'. number_format($tot_rec_qty,2,'.','') .'</td>
										<td>'. number_format($tot_iss_retn_qty,2,'.','') .'</td>
										<td>'. number_format($tot_transfer_in_qty,2,'.','') .'</td>
										<td>'. number_format($tot_rec_bal,2,'.','') .'</td>
										<td>'. $tot_rec_roll .'</td>
										<td>'. number_format($tot_issue_qty,2,'.','') .'</td>
										<td>'. number_format($tot_recv_retn_qty,2,'.','') .'</td>
										<td>'. number_format($tot_transfer_out_qty,2,'.','') .'</td>
										<td>'. number_format($tot_issue_bal,2,'.','') .'</td>
										<td>'. $tot_issue_roll .'</td>
										<td>'. number_format($tot_stock,2,'.','') .'</td>
										<td ></td>
										<td>&nbsp;</td>
										<td>&nbsp;</td>
										<td>&nbsp;</td>
				...
				<tr class="tbl_bottom">
					...
					<td></td>
					<td>Grand total</td>
					<td>'. number_format($total_grey_qnty,2,'.','').'</td>
					<td>'. number_format($grand_tot_rec_qty,2,'.','').'</td>
					<td>'. number_format($grand_tot_issue_retn_qty,2,'.','').'</td>
					<td>'. number_format($grand_tot_transfer_in_qty,2,'.','').'</td>
					<td>'. number_format($grand_tot_rec_bal,2,'.','').'</td>
					<td>'. $grand_tot_rec_roll.'</td>
					<td>'. number_format($grand_tot_issue_qty,2,'.','').'</td>
					<td>'. number_format($grand_tot_recv_retn_qty,2,'.','').'</td>
					<td>'. number_format($grand_tot_transfer_out_qty,2,'.','').'</td>
					<td>'. number_format($grand_tot_issue_bal,2,'.','').'</td>
					<td>'. $grand_tot_issue_roll.'</td>
					<td>'. number_format($grand_tot_stock,2,'.','').'</td>
					<td>';
					 if ($grand_tot_roll_qty>0) {$html .= $grand_tot_roll_qty;} else {$html .= '0.00';}
					$html .='</td>
					<td></td>
					<td></td>
					<td></td>
					...
					<td></td>
					<td></td>
					<td>'. number_format($total_grey_qnty-$grand_tot_rec_bal,2,'.','').'</td>
					<td>'. number_format($total_grey_qnty-$grand_tot_issue_qty,2,'.','').'</td>
				</tr>	
				</tbody>
			</table>';
		...
		foreach (glob("order_wise_grey_$user_id*.xlsx") as $filename) {
			@unlink($filename);
		}
		$name=time();
		$filename='order_wise_grey_'.$user_id."_".$name.".xlsx";
