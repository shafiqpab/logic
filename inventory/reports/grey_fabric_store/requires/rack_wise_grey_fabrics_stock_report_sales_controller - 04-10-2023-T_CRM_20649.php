...
if ($action=="floor_popup")
{
	...
	/*$floor_sql = "
    	...
    	WHERE a.status_active = 1 AND a.is_deleted = 0 AND b.status_active = 1 AND b.is_deleted = 0 AND a.company_id IN(".$data[0].")
	...
	WHERE a.status_active = 1 AND a.is_deleted = 0 AND b.status_active = 1 AND b.is_deleted = 0 AND a.company_id IN(".$data[0].") and c.category_type in(13,14)
	GROUP BY a.floor_room_rack_id, a.floor_room_rack_name, a.company_id, b.location_id, b.store_id
	ORDER BY a.floor_room_rack_name";
	...
	*/
	...
	if ($reportType == 1)
    {
								...
								foreach($roomArr as $rackId=>$rackArr)
								{
									foreach($rackArr as $selfId=>$selfArr)
									{
										foreach($selfArr as $binId=>$row)
										{
											...
											$newDataArr[$compId][$orderId][$fabric_construction][$fab_color_id]['rcvQty'] +=$row['rcvQty'];
											$newDataArr[$compId][$orderId][$fabric_construction][$fab_color_id]['issueReturnQty'] +=$row['issueReturnQty'];
											$newDataArr[$compId][$orderId][$fabric_construction][$fab_color_id]['transferInQty'] +=$row['transferInQty'];
											$newDataArr[$compId][$orderId][$fabric_construction][$fab_color_id]['totalRcvQty'] +=$row['totalRcvQty'];

											$newDataArr[$compId][$orderId][$fabric_construction][$fab_color_id]['issueQty'] +=$row['issueQty'];
											$newDataArr[$compId][$orderId][$fabric_construction][$fab_color_id]['rcvReturnQty'] +=$row['rcvReturnQty'];
											$newDataArr[$compId][$orderId][$fabric_construction][$fab_color_id]['transferOutQty'] +=$row['transferOutQty'];
											$newDataArr[$compId][$orderId][$fabric_construction][$fab_color_id]['totalIssueQty'] +=$row['totalIssueQty'];
											$newDataArr[$compId][$orderId][$fabric_construction][$fab_color_id]['stockQty'] +=$row['stockQty'];

											$newDataArr[$compId][$orderId][$fabric_construction][$fab_color_id]['rollRcvQty'] +=$row['rollRcvQty'];
											$newDataArr[$compId][$orderId][$fabric_construction][$fab_color_id]['rollIssueQty'] +=$row['rollIssueQty'];
											$newDataArr[$compId][$orderId][$fabric_construction][$fab_color_id]['rollBalanceQty'] +=$row['rollBalanceQty'];
											$productIdsArr[$compId][$orderId][$fabric_construction][$fab_color_id][$productId] =$productId;
		...
		foreach ($sql_fso_book_qty as $val) 
		{
			...
			$fso_booking_req[$val[csf("mst_id")]][$fabric_construction][$color_arr[$val[csf("color_id")]]]["req_qty"] +=$val[csf("grey_qty")];
							...
							foreach($orderArr as $fabric_construction=>$fabric_constructionArr)
							{
								foreach($fabric_constructionArr as $fab_color_id=>$row)
								{
									...
									if($row['stockQty'] >= 0)
									{
										...
										$required_qnty = $fso_booking_req[$orderId][$fabric_construction][$fab_color_id]["req_qty"];

										$within_group = $sales_ord_ref[$val[csf("mst_id")]]['within_group'];

										$color_id_arr=array_unique(explode(",", $fab_color_id));
										$color_names="";
                                        ...
                                        <tr valign="middle">
                                            ...
                                            <td width="80" align="right">
                                            	<a href='#report_details' onClick="openmypage('<? echo $orderId; ?>','<? echo $productIds; ?>','<? echo $fab_color_id; ?>','grey_recv_issue_popup',1);"><? echo number_format($row['rollRcvQty'],2); ?></a>
                                            </td>
                                            ...
                                            <td width="80" align="right">
                                            	<a href='#report_details' onClick="openmypage('<? echo $orderId; ?>','<? echo $productIds; ?>','<? echo $fab_color_id; ?>','grey_recv_issue_popup',2);"><? echo number_format($row['rollIssueQty'],2); ?></a>
                                            </td>
                                            ...
                                            <td width="100" align="right"><a href="##" onclick="openmypage_rollbal('<? echo $orderId;?>','<? echo $productIds;?>','<? echo $fab_color_id;?>');"><?php echo $row['rollBalanceQty']; ?></a></td>
                                        </tr>
										<?php
										//$grandTotal
...
if($action=="report_generate_summary")
{
	...
	*/
	...
	foreach($sqlRcvRollRslt as $row)
	{
		...
		$str_ref=$productId;
		$dataArr[$compId][$orderId][$febric_description_id][$color_id]['rollRcvQty'] += count($row[csf('no_of_roll_rcv')]);
		$dataArr[$compId][$orderId][$febric_description_id][$color_id]['prod_id'] .= $productId.',';
		if($row[csf('entry_form')]  == 84)
		{
			$dataArr[$compId][$orderId][$febric_description_id][$color_id]['issueReturnQty'] += $row[csf('rcv_qty')];
		}
		else
		{
			$dataArr[$compId][$orderId][$febric_description_id][$color_id]['rcvQty'] += $row[csf('rcv_qty')];
		}
	}
	unset($sqlRcvRollRslt);
	// echo "<pre>";print_r($dataArr);die;
	...
	foreach($sqlNoOfRollResult as $row)
	{
		...
		if($row[csf('trans_type')] == 5)
		{
			$poArr[$orderId] = $orderId;
			$dataArr[$compId][$orderId][$febric_description_id][$color_id]['prod_id'] .= $productId.',';
			$dataArr[$compId][$orderId][$febric_description_id][$color_id]['rollRcvQty'] += count($row[csf('issue_roll')]);

			if ($row[csf('transfer_criteria')] !=2 ) // without store to store transfer in qty
			{
				if($row[csf('roll_rcv_qty')])
				{
					$dataArr[$compId][$orderId][$febric_description_id][$color_id]['transferInQty'] += $row[csf('roll_rcv_qty')];
				}
				else{
					$dataArr[$compId][$orderId][$febric_description_id][$color_id]['transferInQty'] += $row[csf('rcv_qty')];
				}
			}
		...
		if($row[csf('trans_type')] == 6)
		{
			$transOutArr[$compId][$orderId][$febric_description_id][$color_id]['rollIssueQty'] += count($row[csf('issue_roll')]);

			if ($row[csf('transfer_criteria')] !=2 ) // without store to store transfer out qty
			{
				if($row[csf('roll_rcv_qty')])
				{
					$transOutArr[$compId][$orderId][$febric_description_id][$color_id]['transferOutQty'] += $row[csf('roll_rcv_qty')];
				}
				else{
					$transOutArr[$compId][$orderId][$febric_description_id][$color_id]['transferOutQty'] += $row[csf('rcv_qty')];
				}
			}
		}
	...
	*/
	...
	if(!empty($poArr))
	{
		...
		foreach($sqlNoOfRollIssueResult as $row)
		{
			...
			$noOfRollIssueArr[$compId][$orderId][$deter_id][$color_id]['rollIssueQty'] += $row[csf('issue_roll')];//without split barcode count
	        ...
	        $issueQtyArr[$compId][$orderId][$deter_id][$color_id]['issueQty'] += $row[csf('issue_qty')];
		}
		unset($sqlNoOfRollIssueResult);
	}
	//echo $issueQty."=".$rollIssueQty;
	...
	*/
	...
	foreach ($sql_fso_book_qty as $val) 
	{
		...
		$fso_booking_req[$val[csf("mst_id")]][$fabric_construction][$val[csf("color_id")]]["req_qty"] +=$val[csf("grey_qty")];
					...
					foreach($compArr as $orderId=>$orderArr)
					{
						foreach($orderArr as $fabric_construction=>$fabric_constructionArr)
						{
							...
							{//foreach($color_id_val as $str_ref=>$row)
								...
								//total issue calculation
								$row['issueQty'] = $issueQtyArr[$compId][$orderId][$fabric_construction][$fab_color_id]['issueQty'];
								$row['rcvReturnQty'] = 0;
								$row['transferOutQty'] = $transOutArr[$compId][$orderId][$fabric_construction][$fab_color_id]['transferOutQty'];
								$row['totalIssueQty'] = number_format($row['issueQty'],2,'.','')+number_format($row['rcvReturnQty'],2,'.','')+number_format($row['transferOutQty'],2,'.','');
								...
								//roll balance calculation
								$row['rollIssueQty'] = $transOutArr[$compId][$orderId][$fabric_construction][$fab_color_id]['rollIssueQty']+$noOfRollIssueArr[$compId][$orderId][$fabric_construction][$fab_color_id]['rollIssueQty'];
								$row['rollBalanceQty'] = $row['rollRcvQty'] - $row['rollIssueQty'];

								$color_id_arr = array_filter(array_unique(explode(",", $fab_color_id)));
                                $colors="";
                                foreach ($color_id_arr as $color) 
                                {
                                    $colors .= $color_arr[$color] . ",";
								...
								if($row['stockQty'] >= 0)
								{
									...
									<tr valign="middle" bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $sl; ?>" onClick="change_color('tr_<? echo $sl; ?>','<? echo $bgcolor; ?>')">
                                        ...
                                        <td width="100" class="word_break" title="<?=$fab_color_id;?>"><div><?php echo $colors; ?></div></td>
...
if ($action=="stock_roll_balance_popup")
{
	...
	SELECT c.po_breakdown_id, c.barcode_no, d.store_name as store_id, d.floor_id, d.room, cast(d.rack as varchar2(100)) as rack, d.self, d.bin_box, sum(c.qnty) as qnty
	from pro_roll_details c, inv_grey_fabric_issue_dtls d 
	where c.entry_form=61 and c.dtls_id=d.id and c.status_active=1 and c.is_deleted=0 and c.po_breakdown_id in($fso_id) and d.prod_id in($product_ids) and c.booking_without_order = 0 and c.is_returned=0
	group by c.po_breakdown_id, c.barcode_no, d.store_name, d.floor_id, d.room, d.rack, d.self, d.bin_box
	) x
	group by x.po_breakdown_id, x.barcode_no, x.store_id, x.floor_id, x.room, x.rack, x.self, x.bin_box
	");
	...
	(
		...
		and c.is_deleted=0 and b.prod_id in($product_ids) and c.po_breakdown_id in($fso_id) and c.is_sales=1 and c.re_transfer=0 
		group by c.barcode_no, c.roll_no, a.store_id,b.floor_id, b.room, b.rack, b.self,b.bin_box 
		union all 
		...
		and b.from_prod_id in($product_ids) and a.to_order_id in($fso_id) and c.re_transfer=0 
		group by c.barcode_no, c.roll_no, b.to_store, b.to_floor_id, b.to_room,b.to_rack, b.to_shelf,b.to_bin_box 
		) x 
		group by x.barcode_no, x.roll_no, x.store_id,x.floor_id,x.room, x.rack, x.self, x.bin_box
	) y, pro_roll_details z, pro_grey_prod_entry_dtls d
	where y.barcode_no=z.barcode_no and z.dtls_id=d.id and z.entry_form in (2,22) and d.color_id='$color_id' and z.status_active=1";
