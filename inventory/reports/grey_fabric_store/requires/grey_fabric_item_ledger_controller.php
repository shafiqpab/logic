...
//item search------------------------------//
	...
	{		  
		...
		<script>
			...
			{ 
				show_list_view ( document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_search_common').value+'_'+<? echo $company; ?>+'_'+document.getElementById('txt_prod_id').value, 'create_lot_search_list_view', 'search_div', 'grey_fabric_item_ledger_controller', 'setFilterGrid("list_view",-1)');
			}
		...
		/*function fn_item_search(str)
		{
			...
			{
				field_type='<?//  echo create_drop_down( "txt_search_common", 130, $body_part,"", 1, "-- Select Body Part --", 0, "",1 ); ?>';
				$('#search_by_td_up').html('Enter Bordy Part');
			}
...
if($action=="create_lot_search_list_view")
{
	...
	$sql = "select id,product_name_details,gsm,dia_width from product_details_master where company_id=$company and item_category_id =13 $sql_cond"; 
	$arr=array();
...
{ 	 
	...
	if($rpt_type==1)
	{
		...
		where a.prod_id in ($txt_product_id) and a.mst_id=b.id and a.transaction_type in (1,4) and a.item_category='13'"; 
		...
		where a.prod_id in ($txt_product_id) and a.mst_id=b.id and a.transaction_type in (2,3) and a.item_category='13'";		 
		...
		where a.mst_id=b.id and a.transaction_type in (5,6) and a.item_category='13' and a.prod_id in ($txt_product_id)";		 
		...
		<fieldset>
			...
			if($cbo_method==0) //average rate #########################################################################
			{
				...
				where a.status_active=1 and a.is_deleted=0 and  a.prod_id in ($txt_product_id) and a.prod_id=b.id  and a.item_category='13' and a.company_id=$cbo_company_name $search_cond 
						...
						foreach($result as $row)
						{
							...
							if(!in_array($row[csf("prod_id")],$product_id_arr))
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" <? echo $stylecolor; ?> onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
									...
									//$total_balQunty=number_format($total_balQnty,2);
									//$total_balaValue=number_format($total_balValue,2);
									?> 
	...
	else if($rpt_type==2)
	{
		...
		$prod_rcv_booking = return_library_array("select id,booking_no from  inv_receive_master where status_active=1 and is_deleted=0 and entry_form=2 and item_category=13","id","booking_no"); 
		...
		from inv_transaction a, inv_receive_master b
		where a.mst_id=b.id and a.transaction_type in (1,4) and a.item_category='13' $rcv_prod_cond"; 
		...
		//var_dump($book_no_arr);die;

		//var_dump($prod_rcv_booking[14978]);die;
		// issue MRR array------------------------------------------------

		$sql_issue_mrr = "select a.id as trid,a.transaction_type,b.issue_number,b.issue_purpose,b.booking_no
		from inv_transaction a, inv_issue_master b
		where a.mst_id=b.id and a.transaction_type in (2,3) and a.item_category='13' $rcv_prod_cond";		 
		$result_iss = sql_select($sql_issue_mrr);
		$issueMRR=array();$issuePupose=array();
		foreach($result_iss as $row)
		{
		...
		$sql_transfer_mrr = "select a.id as trid,a.transaction_type,b.transfer_system_id, b.transfer_criteria, b.from_order_id, b.to_order_id
		from inv_transaction a, inv_item_transfer_mst b
		where a.mst_id=b.id and a.transaction_type in (5,6) and a.item_category='13' $rcv_prod_cond";		 
		$result_transfer = sql_select($sql_transfer_mrr);
		$transferMRR=array();
		foreach($result_transfer as $row)
		{
		...
		<fieldset>
			...
			//var_dump($opning_bal_arr);die;
					...
					if($txt_po_no!=''){$po_con=" and a.po_number ='$txt_po_no'";}else{$po_con="";}
					$job_sql=sql_select("select a.id as po_id, a.po_number, b.job_no from wo_po_break_down a, wo_po_details_master b, order_wise_pro_details c  where a.job_no_mst=b.job_no and a.id=c.po_breakdown_id and a.status_active=1 and b.status_active=1 $po_con");
					$select_po_id="";
					foreach($job_sql as $row)
					{
						$job_data[$row[csf("po_id")]]["po_number"]=$row[csf("po_number")];
			...
			if($cbo_method==0) //average rate #########################################################################
			{
				...
				where a.prod_id=b.id and a.id=d.trans_id  and  a.status_active=1 and a.is_deleted=0 $rcv_prod_cond and a.item_category='13' and a.company_id=$cbo_company_name $search_cond $order_cond order by  a.prod_id, a.insert_date, a.id ASC";		
			...
			else if($cbo_method==2) //LIFU #########################################################################
			{
				...
				from inv_transaction a left join inv_issue_master c on a.mst_id=c.id and a.transaction_type in (2,3,6), product_details_master b, order_wise_pro_details d 
				where a.prod_id=b.id and a.id=d.trans_id and a.status_active=1 and a.is_deleted=0 $rcv_prod_cond and  a.item_category='13' and a.company_id=$cbo_company_name $search_cond $order_cond order by  a.prod_id, a.insert_date, a.id DESC";		
			...
			if(count($trans_id_arr)>0)
			{
				if($cbo_method==0) //average rate #########################################################################
				{
					...
					from inv_transaction a left join inv_issue_master c on a.mst_id=c.id and a.transaction_type in (2,3,6), product_details_master b 
					where a.prod_id=b.id and  a.status_active=1 and a.is_deleted=0 and  a.prod_id in ($txt_product_id) and a.item_category='13' and a.company_id=$cbo_company_name and a.id not in(".implode(",",$trans_id_arr).") $search_cond order by  a.prod_id, a.insert_date, a.id ASC";		
			...
			//print_r($result_data_array);die;
			//"select a.id as po_id, a.po_number, b.job_no from wo_po_break_down a, wo_po_details_master b, order_wise_pro_details c  where a.job_no_mst=b.job_no and a.id=c.po_breakdown_id and a.status_active=1 and b.status_active=1 and b.company_name=$cbo_company_name and c.prod_id in($txt_product_id)"
						...
						foreach($result_data_array as $prod_id=>$value)
						{
							foreach($value as $tr_id=>$val)
							{
								...
								if($row[csf("transaction_type")]==2 || $row["transaction_type"]==3 || $row["transaction_type"]==6) 
									...
									//var_dump($balQnty); 
									/*if(!in_array($row[("prod_id")],$each_pro_id))
									{*/
										...
										//if($m==1) $total_balQnty= $total_balQnty+$balQnty[$row[("prod_id")]]; else $total_balQnty+=$total_balQnty;
										//if($m==1) $total_balValue= $total_balValue+$balValue[$row[("prod_id")]]; else $total_balValue+=$total_balValue;
												//$total_balQunty=number_format($total_balQnty,2);
												//$total_balaValue=number_format($total_balValue,2);
							...
							/*//total sum END-----------------------
							$each_pro_id[$row[csf("prod_id")]]=$row[csf("prod_id")];csf
							$m++;
							}
							$total_balQnty=0;
							$total_balValue=0;*/
