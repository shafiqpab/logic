...
<script>
	...
	function openmypage_order()
	{
		...
		var page_link='requires/grey_store_recv_issue_summary_report_controller.php?action=order_no_search_popup&companyID='+company+'&buyer_name='+buyer;  
	...
	function  generate_report(rptType)
	{
		...
		var dataString = "&cbo_company_name="+cbo_company_name+"&cbo_buyer_name="+cbo_buyer_name+"&txt_order="+txt_order+"&txt_order_id="+txt_order_id+"&txt_date_from="+txt_date_from+"&txt_date_to="+txt_date_to+"&cbo_based_on="+cbo_based_on+"&cbo_order_type="+cbo_order_type+"&cbo_knitting_source="+cbo_knitting_source+"&rptType="+rptType+"&cbo_store_name="+cbo_store_name;
		...
		freeze_window(5);
		http.open("POST","requires/grey_store_recv_issue_summary_report_controller.php",true);
	...
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/grey_store_recv_issue_summary_report_controller.php?barcode_nos='+barcode_nos+'&action=barcode_popup', 'Details Veiw', 'width=450, height=350px,center=1,resize=1,scrolling=0','../../');
	}
...
<body onLoad="set_hotkey()">
	<div style="width:100%;" align="center">
		...
		<form name="item_receive_issue_1" id="item_receive_issue_1" autocomplete="off" >
			<h3 align="left" id="accordion_h1" style="width:960px" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3>
			<div style="width:960px;" align="center" id="content_search_panel">
				<fieldset style="width:960px;">
					<table class="rpt_table" width="960" cellpadding="0" cellspacing="0" rules="all">
						<thead>
							<tr>
						...
						<tr class="general">
							<td>
								<?
								echo create_drop_down( "cbo_company_name", 100, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/grey_store_recv_issue_summary_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/grey_store_recv_issue_summary_report_controller',this.value, 'load_drop_down_store', 'store_td' );" );
								?>                          
							</td>
							<td id="buyer_td">
								<? 
