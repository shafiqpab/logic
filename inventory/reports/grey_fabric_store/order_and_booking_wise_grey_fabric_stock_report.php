...
<script>
	...
	var tableFilters_2 = 
	{
		//col_30: "none",
		col_operation: {
		id: ["value_tot_recv_qty","value_tot_iss_ret_qty","value_tot_trans_in_qty","value_grand_tot_recv_qty","value_tot_iss_qty","value_tot_rec_ret_qty","value_tot_trans_out_qty","value_grand_tot_iss_qty","value_grand_stock_qty"],
		col: [17,18,19,20,21,22,23,24,25],
...
//col: [18,19,20,21,22,23,24,25,26],
	...
	function generate_report(rpt_type)
	{
		...
		if(rpt_type==3){
		var data="action=report_generate_2"+get_submitted_data_string('cbo_company_id*cbo_buyer_id*cbo_year*txt_job_no*txt_job_id*txt_booking_no*txt_search_comm*txt_date_from*cbo_store*cbo_value_with',"../../../")+'&report_title='+report_title+'&rpt_type='+rpt_type;
		}
		else{
		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_buyer_id*cbo_year*txt_job_no*txt_job_id*txt_booking_no*txt_search_comm*txt_date_from*cbo_store*cbo_value_with',"../../../")+'&report_title='+report_title+'&rpt_type='+rpt_type;
		}

		//alert (data);
		freeze_window(3);
		http.open("POST","requires/order_and_booking_wise_grey_fabric_stock_report_controller.php",true);
	...
	function openmypage_job()
	{
		...
		var page_link='requires/order_and_booking_wise_grey_fabric_stock_report_controller.php?action=job_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id;
	...
	function openmypage_booking()
	{
		...
		var page_link='requires/order_and_booking_wise_grey_fabric_stock_report_controller.php?action=booking_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id+'&txt_job_no='+txt_job_no;
	...
	/*function openmypage_order()
	{
		...
		//alert (data);
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/order_and_booking_wise_grey_fabric_stock_report_controller.php?action=order_no_popup&data='+data,'Order No Popup', 'width=630px,height=420px,center=1,resize=0','../../')
	...
	{ 
		 emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/order_and_booking_wise_grey_fabric_stock_report_controller.php?action='+action+'&po_id='+po_id, 'Booking Details Info', 'width=900px,height=370px,center=1,resize=0,scrolling=0','../../');
	}
	...
	{ //alert(orderID);
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/order_and_booking_wise_grey_fabric_stock_report_controller.php?companyID='+companyID+'&orderID='+orderID+'&programNo='+programNo+'&prodID='+prodID+'&from_date='+from_date+'&to_date='+to_date+'&action='+action+'&popup_width='+popup_width+'&type='+type, 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../../');
	}
	function openmypage_delivery_all(orderID,bookingID,prodID,yarnBrand,yLot,stichLength,popup_width,action,type)
	{
		var companyID = $("#cbo_company_id").val();
		...
		/*if(type==6)
		{*/
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/order_and_booking_wise_grey_fabric_stock_report_controller.php?companyID='+companyID+'&orderID='+orderID+'&bookingID='+bookingID+'&prodID='+prodID+'&txt_date_from='+txt_date_from+'&action='+action+'&popup_width='+popup_width+'&type='+type+'&yarnBrand='+yarnBrand+'&yLot='+yLot+'&stichLength='+stichLength, 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../../');
                ...
                <table class="rpt_table" width="1000" cellpadding="0" cellspacing="0" border="1" rules="all">
                    <thead>
                        <tr> 	 	
                            ...
                            <th>Store Name</th>
                            <th>Value</th>
                            <th class="must_entry_caption">Stock Date</th>
                    ...
                    <tr class="general">
                        <td>
                            <? 
                               echo create_drop_down( "cbo_company_id", 140, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/order_and_booking_wise_grey_fabric_stock_report_controller',this.value+'_'+1+'_'+4, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/order_and_booking_wise_grey_fabric_stock_report_controller',this.value, 'load_drop_down_store', 'store_td' );" );
                            ?>                            
                        </td>
                        <td id="buyer_td"> 
                            <?
                        ...
                        <td id="store_td"> 
                            <?
                                echo create_drop_down( "cbo_store", 140, $blank_array,"", 1, "--Select Store--", 0, "",0 );
                            ?>
                        </td>
                        <td>
							<?
                        ...
                        <td>
                            <input type="button" name="search" id="search" value="Show" onClick="generate_report(2)" style="width:79px" class="formbutton" />
                        </td>
						<td>
							<input type="button" name="search" id="search" value="Show 2" onClick="generate_report(3)" style="width:79px" class="formbutton" />
                        </td>
                    </tr>
                </table> 
            </fieldset> 
