<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Receive and Issue Summary With Return and Transfer Sales
Functionality	:	
JS Functions	:
Created by		:	Tipu
...
echo load_html_head_contents("Receive and Issue Summary With Return and Transfer Sales","../../../", 1, 1, $unicode,1,1); 
?>	
<script>
	...
	var tableFilters =
	{
		col_operation:
		{
			id: ["value_total_recv_qnty","value_total_issue_qnty","value_total_return_qnty","value_total_trans_in_qnty","value_total_trans_out_qty"],
			col: [9,11,13,15,17],
	...
	function openmypage_order()
	{
		...
		var page_link='requires/receive_and_issue_summary_with_return_and_transfer_sales_controller.php?action=order_no_search_popup&companyID='+company+'&buyer_name='+buyer;  
	...
	function openmypage_booking()
	{
		...
		var txt_order_id = $("#txt_order_id").val();
		var txt_order = $("#txt_order").val();
		var page_link='requires/receive_and_issue_summary_with_return_and_transfer_sales_controller.php?action=booking_no_search_popup&companyID='+company+'&buyer_name='+buyer;  
	...
	function  generate_report(rptType)
	{
		...
		freeze_window(5);
		http.open("POST","requires/receive_and_issue_summary_with_return_and_transfer_sales_controller.php",true);
	...
	function openmypage_item()
	{
		...
		var company = $("#cbo_company_name").val();	
		var page_link='requires/receive_and_issue_summary_with_return_and_transfer_sales_controller.php?action=item_description_search&company='+company; 
				...
				<fieldset style="width:1300px;">
					<table class="rpt_table" width="1300" cellpadding="0" cellspacing="0" rules="all">
						...
						<tr class="general">
							<td>
								<?
								echo create_drop_down( "cbo_company_name", 100, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/receive_and_issue_summary_with_return_and_transfer_sales_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/receive_and_issue_summary_with_return_and_transfer_sales_controller',this.value, 'load_drop_down_cust_buyer', 'cust_buyer_td' );load_drop_down( 'requires/receive_and_issue_summary_with_return_and_transfer_sales_controller',this.value, 'load_drop_down_store', 'store_td' );" );
								?>                          
							</td>
							<td id="buyer_td">
								<? 
