...
<script>
	...
	function generate_report(rpt_type)
	{
		...
		var data="action="+action+get_submitted_data_string('cbo_company_id*cbo_buyer_id*cbo_year*txt_job_no*txt_job_id*txt_file_no*txt_ref_no*txt_order_no*txt_date_from*txt_hide_booking_id*txt_booking_no*txt_style_ref',"../../../")+'&report_title='+report_title+'&rpt_type='+rpt_type;
		//alert (data);

		http.open("POST","requires/file_wise_grey_fabric_stock_controller.php",true);
	...
	function openmypage_job()
	{
		...
		var page_link='requires/file_wise_grey_fabric_stock_controller.php?action=job_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id;
    ...
    function openmypage_booking()
    {
        ...
        var cbo_year_id = $("#cbo_year").val();
        //var cbo_month_id = $("#cbo_month").val();
        var page_link='requires/file_wise_grey_fabric_stock_controller.php?action=booking_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id;
	...
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/file_wise_grey_fabric_stock_controller.php?action='+action+'&po_id='+po_id, 'Booking Details Info', 'width=900px,height=370px,center=1,resize=0,scrolling=0','../../');
	}

	function openpage(action,data)
	{
		 emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/file_wise_grey_fabric_stock_controller.php?action='+action+'&data='+data, 'Details Info', 'width=1200px,height=390px,center=1,resize=0,scrolling=0','../../');
	}
            ...
            <fieldset style="width:1250px;">
                <table class="rpt_table" width="1250" cellpadding="0" cellspacing="0" border="1" rules="all">
                    ...
                    <tr class="general">
                        <td>
                            <?
                               echo create_drop_down( "cbo_company_id", 140, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/file_wise_grey_fabric_stock_controller',this.value+'_'+1+'_'+4, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>
                        </td>
                        <td id="buyer_td">
                            <?
						...
						<td>
                            <input type="text" id="txt_style_ref" name="txt_style_ref" class="text_boxes" style="width:80px" placeholder="Write"/>
                        </td>
						<td>
                        ...
                        <td>
							...
							<input type="button" name="search2" id="search2" value="Show 2" onClick="generate_report(2)" style="width:80px" class="formbutton" />
                        </td>
                    </tr>
                </table>
            </fieldset>
