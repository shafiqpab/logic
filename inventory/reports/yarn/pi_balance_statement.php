...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("PI Balance Statement","../../../", 1, 1, $unicode,1,1); 
...
var tableFilters2 = { 
	col_30: "none", 
	col_operation: {
		id: ["value_total_pi_value","value_total_pi_item_qtny","value_total_pi_item_value","value_total_rcv_btb_qnty","value_total_rtn_btb_qnty","value_total_balance_qnty","value_total_pipeline_qnty","value_total_rcv_value","value_total_balance_value"],
		col: [7,16,18,19,20,21,22,23,24],
...
function openmypage_pi()
{
	...
	var page_link='requires/pi_balance_statement_controller.php?action=pi_searce_popup&companyID='+companyID+'&supplierID='+supplierID+'&btbLc_id='+btbLc_id;
...
function openmypage_btbLc()
{
	...
	var page_link='requires/pi_balance_statement_controller.php?action=btbLc_popup&companyID='+companyID+'&supplierID='+supplierID+'&pi_id='+pi_id;
	var title='Yarn Requisition Info';
...
function generate_report(rpt_type)
{
	...
	if(supplierID==0 &&  pi_id=="" &&  btbLc_id=="" && receiving_status==0)
	{
		...
		{
			if( form_validation('cbo_company_name*txt_date_from_pi*txt_date_to_pi','Company Name*Date Form*Date To')==false )
			{
				return;
			}
		}
	...
	var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_suppler_name*pi_id*btbLc_id*txt_date_from_pi*txt_date_to_pi*txt_date_from*txt_date_to*cbo_receiving_status*cbo_closing_status',"../../../")+'&report_title='+report_title+'&rpt_type='+rpt_type;
	//alert(data);return;
	freeze_window(3);
	http.open("POST","requires/pi_balance_statement_controller.php",true);
...
function openmypage_popup(company_id,pi_id,page_title,action)
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/pi_balance_statement_controller.php?company_id='+company_id+'&pi_id='+pi_id+'&action='+action, page_title, 'width='+width+',height=400px,center=1,resize=0,scrolling=0','../../');
}

function openmypage_mrr(company_id,pi_id,count_id,composition_id,type_id,color_id,page_title,action)
{
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/pi_balance_statement_controller.php?company_id='+company_id+'&pi_id='+pi_id+'&count_id='+count_id+'&composition_id='+composition_id+'&type_id='+type_id+'&color_id='+color_id+'&action='+action, page_title, 'width=690px,height=400px,center=1,resize=0,scrolling=0','../../');
}
            ...
            <legend>Search Panel</legend> 
                <table class="rpt_table" width="1200" cellpadding="0" cellspacing="0" border="1" rules="all">
                    ...
                    <tr align="center">
                        <td>
                            <? 
                               echo create_drop_down( "cbo_company_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "reset_form('','','txt_btbLc_no*btbLc_id','','',''); load_drop_down( 'requires/pi_balance_statement_controller', this.value, 'load_drop_down_supplier', 'supplier_td' );" );
                            ?>                            
                        </td>
                        <td id="supplier_td"> 
                            <?
                        ...
                        <td> 
                            <?
							$close_status=array(1=>"All",2=>"Closed",3=>"Pending");
                            echo create_drop_down( "cbo_closing_status", 100, $close_status,"", 0, "-- Select Status --", 0, "" );
                            ?>
                        </td>
                        <td>
                            <input type="button" name="search" id="search" value="Show" onClick="generate_report(1)" style="width:90px; display:none" class="formbutton" />
                            <input type="button" name="search" id="search" value="Show" onClick="generate_report(2)" style="width:90px" class="formbutton" />
                        </td>
                    </tr>
                    <tr>
