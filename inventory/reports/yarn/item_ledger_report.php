...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../logout.php";
	...
	function openmypage_item()
	{
		...
		var page_link='requires/item_ledger_report_controller.php?action=item_description_search&company='+company; 
	...
	{
		if( form_validation('cbo_company_name*txt_product','Company Name*Item Description')==false )
		{
			return;
		} 
		var cbo_company_name = $("#cbo_company_name").val();
		var txt_product_id = $("#txt_product_id").val();
		var cbo_method = $("#cbo_method").val();txt_lot_no
		...
		var dataString = "&cbo_company_name="+cbo_company_name+"&txt_product_id="+txt_product_id+"&cbo_method="+cbo_method+"&from_date="+from_date+"&to_date="+to_date+"&txt_lot_no="+txt_lot_no+"&cbo_store_name="+cbo_store_name;
		var data="action=generate_report"+dataString;
		freeze_window(operation);
		http.open("POST","requires/item_ledger_report_controller.php",true);
	...
	function fnRefresh_data()
	{
		$("#report_container,#report_container2").html("");
		$("#txt_product").val("");
		$("#cbo_method").val("");
		$("#txt_lot_no").val("");
		$("#cbo_store_name").val("");
...
<div style="width:100%;" align="center">
    ...
    <h3 style="width:930px; margin-top:20px;" align="center" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3>
    <div id="content_search_panel" style="width:100%;" align="center">
        <fieldset style="width:930px;">
        <legend>Search Panel</legend> 
			<table class="rpt_table" width="920" cellpadding="0" cellspacing="0">
                <thead>
                    <tr>
                ...
                <tr class="general">
                    <td>
                            <? 
                               echo create_drop_down( "cbo_company_name", 140, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/item_ledger_report_controller', this.value, 'load_drop_down_store', 'store_td' ); fnRefresh_data()" );
                            ?>                            
                    </td>
                    <td align="center">
                    ...
                    <td id="store_td">
                        <?   
                            echo create_drop_down( "cbo_store_name", 140, $blank_array,"", 1, "-- Select Store --", 0, "", 0 );
                        ?>
                    </td>
