...
echo load_html_head_contents("Yarn Item Allocation Ledger","../../../", 1, 1, $unicode,1,1);
?>	
<script>
	...
	function openmypage_item()
	{
		...
		var page_link='requires/item_allocation_ledger_report_controller.php?action=item_description_search&company='+company; 
	...
	function generate_report(operation)
	{
		...
		if($("#hdn_product_id").val() != ""){
			if( form_validation('hdn_product_id','Item Description')==false )
			{
				return;
			}
			var txt_product_id = $("#hdn_product_id").val();
		}else{
			if( form_validation('txt_product_id','Item Description')==false )
			{
				return;
			}
		...
		var dataString = "&cbo_company_name="+cbo_company_name+"&txt_product_id="+txt_product_id+"&from_date="+from_date+"&to_date="+to_date+"&txt_lot_no="+txt_lot_no+"&txt_product_id="+txt_product_id;
		var data="action=generate_report"+dataString;
		freeze_window(operation);
		http.open("POST","requires/item_allocation_ledger_report_controller.php",true);
...
<body onLoad="set_hotkey()">
	<div align="center">
		...
		<form name="stock_ledger_1" id="stock_ledger_1" autocomplete="off" > 
			<h3 style="width:930px; margin-top:20px;" align="center" id="accordion_h1" class="accordion_h" 
			onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3>
			<div id="content_search_panel" style="width:100%;" align="center">
				<fieldset style="width:930px;">
					<legend>Search Panel</legend> 
					<table class="rpt_table" width="810" cellpadding="0" cellspacing="0">
						<thead>
							<tr>
						...
						<tr class="general">
							<td>
								<?
								echo create_drop_down( "cbo_company_name", 140, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "" );
								?>
							</td>
							<td align="center">
								...
								<input type="hidden" name="hdn_product_id" id="hdn_product_id"/>   <input type="hidden" name="txt_product_no" id="txt_product_no"/>             
							</td>
							<td>
								<input name="txt_lot_no" id="txt_lot_no" class="text_boxes" style="width:70px" placeholder="Lot No" >
							</td>
							<td id="store_td">
								<input name="txt_product_id" id="txt_product_id" class="text_boxes" style="width:70px" placeholder="Product Id" >
							</td>
							<td align="center">
