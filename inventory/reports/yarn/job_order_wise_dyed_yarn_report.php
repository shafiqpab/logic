...
function generate_report(type)
{
	...
	var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_supplier_name*cbo_year*txt_job_no*hide_job_id*txt_process_loss*cbo_date_type*txt_date_from*txt_date_to*cbo_search_by*txt_search_comm*cbo_wo_type*txt_internal_ref',"../../../")+'&report_title='+report_title+'&type='+type;
	//alert (data);
	freeze_window(3);
...
function openmypage_job()
{
	...
	var page_link='requires/job_order_wise_dyed_yarn_report_controller.php?action=job_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year='+cbo_year;
...
{ 
	...
	var data_ref='requires/job_order_wise_dyed_yarn_report_controller.php?companyID='+companyID+'&job_no='+job_no+'&booking_id='+booking_id+'&color='+color+'&lot='+lot+'&action='+action+'&trans_type='+trans_type+'&issue_ids='+issue_ids+'&issue_rtn_ids='+issue_rtn_ids;
	//alert(data_ref);
                ...
                <table class="rpt_table" width="1430" cellpadding="0" cellspacing="0" border="1" rules="all">
                    <thead>
                        <tr> 	 	
                            ...
                            <th>Date Type</th>
                            <th colspan="2" id="td_date">Booking Date</th>
                    ...
                    <tr align="center">
                        <td>
                            <? 
                                echo create_drop_down( "cbo_company_name", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/job_order_wise_dyed_yarn_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' ); load_drop_down( 'requires/job_order_wise_dyed_yarn_report_controller',this.value, 'load_drop_down_supplier', 'supplier_td' );" );
                            ?>                            
                        </td>
                        ...
                        <td align="center">
                             <input type="text" name="txt_process_loss" id="txt_process_loss" class="text_boxes" style="width:50px" value="4">
                        </td>

                        <td> 
                            <?
								$search_by=array(1=>'Work Order No',2=>'Booking No');
                        ...
                        <td>
                            <input type="button" name="search" id="search" value="Job Wise" onClick="generate_report(1)" style="width:70px" class="formbutton" />&nbsp;
                            <input type="button" name="search" id="search" value="Color Wise" onClick="generate_report(2)" style="width:70px" class="formbutton" />&nbsp;
                            <input type="button" name="search" id="search" value="Lot Wise" onClick="generate_report(3)" style="width:70px" class="formbutton" />
                        </td>
                    </tr>
                    <tfoot>
                        <tr align="center">
