...
if($action == "sales_order_no_popup")
{
	echo load_html_head_contents("Sales Order Info ","../../../../", 1, 1, '','1','');
	extract($_REQUEST);
	?>
	<script>
	/* var selected_id = new Array(); var selected_name = new Array(); var selected_po_job_prefix = new Array();
	...
	function js_set_value( str )
	{
		...
		if( jQuery.inArray( $('#txt_individual_id' + str).val(), selected_id ) == -1 ) {
			...
			selected_po_job_prefix.push( $('#txt_individual_po_job_prefix' + str).val() );
			...
			selected_po_job_prefix.splice( i, 1 );
		}

		var id = ''; var name = '';var name_po_job_prefix = '';
		for( var i = 0; i < selected_id.length; i++ ) {
			id += selected_id[i] + ',';
			name += selected_name[i] + ',';
			name_po_job_prefix += selected_po_job_prefix[i] + ',';
		}

		id = id.substr( 0, id.length - 1 );
		name = name.substr( 0, name.length - 1 );
		name_po_job_prefix = name_po_job_prefix.substr( 0, name_po_job_prefix.length - 1 );

		$('#hidden_job_no_id').val(id);
		$('#hidden_job_no').val(name);
		$('#hidden_po_job_no_prefix').val(name_po_job_prefix);
	} */

	function js_set_value(str)
	{
			var splitData = str.split("**");
			$("#hidden_job_no_id").val(splitData[0]);
			$("#hidden_job_no").val(splitData[1]);
			$("#hidden_po_job_no_prefix").val(splitData[2]);
	...
	<fieldset style="width:500px">
		...
		<input type="hidden" name="hidden_po_job_no_prefix" id="hidden_po_job_no_prefix" value="">
		<table width="470" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all">
			<thead>
				<tr>
					<th width="30">SL</th>
					<th width="100">sales Order No</th>
					<th width="80">sales Order No Prefix</th>
					<th width="130">Job No</th>
					<th width="">Booking No</th>
				</tr>
			</thead>
		</table>
		<div style="width:500px; overflow-y:scroll; max-height:300px" id="scroll_body">
		...
		$sql_sales_info="SELECT a.id, a.job_no_prefix_num,a.job_no,a.po_job_no,a.sales_booking_no,b.job_no_prefix_num as po_job_no_prefix_num, $year_field from fabric_sales_order_mst a, wo_po_details_master b where a.po_job_no=b.job_no and a.status_active=1 and a.is_deleted=0 and a.within_group =1 and a.company_id=$companyID and to_char(a.insert_date,'YYYY')=$year order by a.job_no_prefix_num desc";
		//echo $sql_sales_info;

		$rslt_sales_info = sql_select($sql_sales_info);

		$i = 1;
		foreach ($rslt_sales_info as $row)
		{
			//var_dump($val);
			$data = explode('-',$row[csf("po_job_no")]);
			$job_no_prefix = ltrim($data[2],0);
			...
			<tr bgcolor="<? echo $bgcolor; ?>"
					onClick="js_set_value('<? echo $row[csf('id')] . '**' . $row[csf('job_no')] . '**' . $row[csf("po_job_no_prefix_num")]; ?>')"
				...
				<td>
					...
					<input type="hidden" name="txt_individual" id="txt_individual<?php //echo $i ?>" value="<? //echo $row[csf("job_no")]; ?>"/>
					<input type="hidden" name="txt_individual_po_job_prefix" id="txt_individual_po_job_prefix<?php //echo $i ?>" value="<? //echo $job_no_prefix; ?>"/> -->
				</td>
				<td width="100"><p><? echo $row[csf("job_no")]; ?></p></td>
				<td width="80"><p><? echo $row[csf("job_no_prefix_num")]; ?></p></td>
				<td width="130"><p><? echo $row[csf("po_job_no")]; ?></p></td>
...
if($action=="report_generate")
{
 	...
 	if($template==1)
	{
		...
		if($sales_order_no)
		{
			if($sales_order_id !="")
			{
				$sales_order_no_cond=" and c.id in($sales_order_id)";
			}
			else
			{
				$sales_order_no_cond=" and c.job_no ='$sales_order_no'";
				//$sales_order_no_cond=" and c.job_no like '%$sales_order_no%'";
			}
		}
		...
		if($style_no !="")
		{
			$style_ref_cond=" and a.style_ref_no='$style_no'";
		}
		...
		$sql="SELECT a.company_name, a.buyer_name, $year_field, a.job_no_prefix_num, a.job_no, a.style_ref_no, a.gmts_item_id, a.order_uom, a.total_set_qnty as ratio, b.id as po_id, b.po_number, b.po_quantity as po_qnty, b.pub_shipment_date, b.shiping_status, b.insert_date, b.po_received_date, b.plan_cut, b.grouping, c.id as sales_order_id, c.job_no as sales_order_no from wo_po_details_master a, wo_po_break_down b, fabric_sales_order_mst c where a.id=b.job_id and a.job_no=c.po_job_no and a.company_name='$company_name' and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $buyer_id_cond $str_cond $job_no_cond $shipping_status_cond $ref_po_cond_for_in $ship_cond $sales_order_no_cond $style_ref_cond order by b.pub_shipment_date, a.job_no_prefix_num, b.id";

		//echo $sql;
		$nameArray=sql_select($sql);

		$main_data_array= array();
		$all_job_No_arr= array();
		$job_no_prefix = '';
		foreach($nameArray as $row)
		{
			$main_data_array[$row[csf("job_no")]][$row[csf("sales_order_id")]]['year']=$row[csf("year")];
			$main_data_array[$row[csf("job_no")]][$row[csf("sales_order_id")]]['buyer_name']=$row[csf("buyer_name")];
			$main_data_array[$row[csf("job_no")]][$row[csf("sales_order_id")]]['job_no_prefix_num']=$row[csf("job_no_prefix_num")];
			$main_data_array[$row[csf("job_no")]][$row[csf("sales_order_id")]]['grouping'] .=$row[csf("grouping")].',';
			$main_data_array[$row[csf("job_no")]][$row[csf("sales_order_id")]]['po_number'] .=$row[csf("po_number")].',';
			$main_data_array[$row[csf("job_no")]][$row[csf("sales_order_id")]]['style_ref_no'] =$row[csf("style_ref_no")];
			$main_data_array[$row[csf("job_no")]][$row[csf("sales_order_id")]]['order_uom'] =$row[csf("order_uom")];
			$main_data_array[$row[csf("job_no")]][$row[csf("sales_order_id")]]['po_id'] .=$row[csf("po_id")].',';
			$main_data_array[$row[csf("job_no")]][$row[csf("sales_order_id")]]['pub_shipment_date'] .=$row[csf("pub_shipment_date")].',';
			$main_data_array[$row[csf("job_no")]][$row[csf("sales_order_id")]]['shiping_status'] .=$row[csf("shiping_status")].',';
			$main_data_array[$row[csf("job_no")]][$row[csf("sales_order_id")]]['po_qnty'] +=$row[csf("po_qnty")];
			$main_data_array[$row[csf("job_no")]][$row[csf("sales_order_id")]]['ratio'] =$row[csf("ratio")];
			$main_data_array[$row[csf("job_no")]][$row[csf("sales_order_id")]]['gmts_item_id'] =$row[csf("gmts_item_id")];
			$main_data_array[$row[csf("job_no")]][$row[csf("sales_order_id")]]['sales_order_no'] =$row[csf("sales_order_no")];

			if($jobNoChk[$row[csf('job_no')]] == "")
            {
                $jobNoChk[$row[csf('job_no')]] = $row[csf('job_no')];
                $all_job_No_arr[$row[csf("job_no")]] = $row[csf("job_no")];
				$job_no_prefix = $row[csf("job_no_prefix_num")];
            }
        ...
        <fieldset>
            <table width="2140">
                <tr class="form_caption">
                    <td colspan="19" align="center">Style wise Yarn Cost Report [Sales]</td>
                </tr>
            </table>
            <table style="margin-top:10px" id="table_header_1" class="rpt_table" width="2140" cellpadding="0" cellspacing="0" border="1" rules="all">
                <thead>
                	<tr>
                        <th width="40" rowspan="2">SL</th>
                        <th width="125" rowspan="2">Main Booking</th>
                        <th width="125" rowspan="2">Sales Order No</th>
                        ...
                        <th width="100" rowspan="2">Shipping Status</th>
            ...
            <div style="width:2160px; max-height:400px; overflow-y:scroll" id="scroll_body">
                <table class="rpt_table" width="2140" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body">
                    <?
						...
						if($job_no_prefix !='')
						{
						$condition->job_no_prefix_num(" in($job_no_prefix)");
						}
                        ...
                        foreach($main_data_array as $k_job_no=>$v_job_no)
                        {
							foreach($v_job_no as $k_sales_order_id=>$row)
							{

								$order_qnty_in_pcs=$row['po_qnty']*$row['ratio'];
								...
								if($costing_per_id_library[$k_job_no]==1) $dzn_qnty=12;
								else if($costing_per_id_library[$k_job_no]==3) $dzn_qnty=12*2;
								else if($costing_per_id_library[$k_job_no]==4) $dzn_qnty=12*3;
								else if($costing_per_id_library[$k_job_no]==5) $dzn_qnty=12*4;
								else $dzn_qnty=1;

								$dzn_qnty=$dzn_qnty*$row[csf('ratio')];
								$dataYarn=explode(",",substr($dataArrayYarn[$k_job_no],0,-1));
								...
								foreach ($po_id_arr as $po_row)
								{

									$job_mkt_required +=$OrderWiseYarnQtyAndAmountArray[$po_row]['qty'];
									$job_mkt_required_cost +=$OrderWiseYarnQtyAndAmountArray[$po_row]['amount'];

									$ref_closing_date .= change_date_format($ref_closing_po_arr[$po_row]).',';

									//$dataArray=array_filter(explode(",",substr($dataArrayWo[$po_row],0,-1)));

									$dataArray=array_filter(explode(",",$dataArrayWo[$po_row]));
									//var_dump($dataArray);
										...
										foreach($dataArray as $woRow)
										{
											...
											if(!in_array($id,$booking_array))
											{
												...
												if($booking_type==4)
												{
													...
													/* $sample_booking.="<a href='##' style='color:#000' onclick=\"generate_worder_report('3','".$booking_no."','".$company_id."','".chop($row['po_id'],',')."','".$item_category."','".$fabric_source."','".$job_no."','".$is_approved."','".$action_name_sm."','".$entryForm."')\"><font style='font-weight:bold' color='$wo_color;'>".$booking_no."(".$system_date.")"."</font></a><br>"; */
												...
												else
												{
													...
													/* $main_booking.="<a href='##' style='color:#000' onclick=\"generate_worder_report('".$is_short."','".$booking_no."','".$company_id."','".chop($row['po_id'],',')."','".$item_category."','".$fabric_source."','".$job_no."','".$is_approved."','".$action_name."','".$entryForm."')\"><font style='font-weight:bold' color='$wo_color;'>".$booking_no."(".$system_date.")".$pre."</font></a><br>"; */
								...
								// $dataArray=array_filter(explode(",",substr($dataArrayWo[$row['po_id']],0,-1)));
								...
								$yarn_iss_qty=$dataArrayYarnIssue[$k_sales_order_id][1]-$dataArrayYarnIssueReturn[$k_sales_order_id][1];
								$yarn_iss_cost=$dataArrayYarnIssue[$k_sales_order_id][2]-$dataArrayYarnIssueReturn[$k_sales_order_id][2];
								...
								$shiping_status_arr = array_unique(explode(",",chop($row['shiping_status'] ,",")));

								$shiping_status = '';
								foreach ($shiping_status_arr as $s_status)
								{
									$shiping_status .= $shipment_status[$s_status].',';
								}

								$pub_shipment_date_arr = array_unique(explode(",",chop($row['pub_shipment_date'] ,",")));

								$pub_shipment_date = '';
								foreach ($pub_shipment_date_arr as $s_date)
								{
									$pub_shipment_date .= change_date_format($s_date).',';
								}
								...
								<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
									...
									<td width="125"><div style="word-wrap:break-word; width:125px"><? echo $row['sales_order_no']; ?></div></td>
									<td width="125"><div style="word-wrap:break-word; width:125px"><? echo $sample_booking; ?></div></td>
									<td width="70"><div style="word-wrap:break-word; width:70px"><? echo $buyer_short_name_library[$row['buyer_name']]; ?></div></td>
									<td width="60" align="center"><? echo $row['year']; ?></td>
									<td width="70" align="center"><? echo $row['job_no_prefix_num']; ?></td>
									<td width="110" style="word-break:break-all;"><? echo implode(", ",array_unique(explode(",",chop($row['style_ref_no'] ,",")))); ?></td>
									<td width="120" style="word-break:break-all;"><? echo $gmts_item; ?></td>
									<td width="50" align="center"><? echo $unit_of_measurement[$row['order_uom']]; ?></td>
									<td width="98" align="right" style="padding-right:2px"><? echo $order_qnty_in_pcs; ?></td>
									<td width="100" style="word-break:break-all;"><? echo chop($shiping_status,","); ?></td>
									<td width="70">
										<?
											$d=1;
									...
									<td width="108" align="right" style="padding-right:2px"><a href="#report_details" onClick="openmypage('<? echo chop($row['po_id'],','); ?>','yarn_req_popup','Yarn Req. Details')"><? echo number_format($job_mkt_required,2,'.',''); ?></a></td>
									...
									<td width="108" align="right" style="padding-right:2px"><a href="#report_details" onClick="openmypage('<? echo $k_sales_order_id; ?>','yarn_issue_cost','Yarn Issue Cost Details','<? echo $company_name; ?>')"><? echo number_format($yarn_iss_qty,2); ?></a></td>
                ...
                <table width="2140" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all">
                    <tfoot>
                        <tr>
                        	<th width="40">&nbsp;</th>
                            <th width="125">&nbsp;</th>
		...
		<div id="report_container">
			<table border="1" class="rpt_table" rules="all" width="950" cellpadding="0" cellspacing="0">
				...
				<thead>
                    ...
                    <th width="90">Requision  No</th>
                    <th width="70">Demand NO</th>
				...
				foreach($result as $row)
				{
				    if($checkOrdPkId[$row[csf('propotion_id')]]=="")
				    {
	                    ...
	                    <tr bgcolor="<? echo  $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
	                        ...
	                        <td width="70"><p><? echo $demand_no[$row[csf('requisition_no')]]['demand_system_no']; ?>&nbsp;</p></td>
				...
				foreach($result as $row)
				{
					if($checkRtnPkId[$row[$row[csf('propotion_id')]]]=="")
					{
	                    ...
	                    <tr bgcolor="<? echo  $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
	                        ...
	                        <td width="90"><p><? echo $row[csf('requisition_no')];?>&nbsp;</p></td>
	                        <td width="70"><p><? echo $return_demand_no[$row[csf('requisition_no')]]['demand_system_no'] ?>&nbsp;</p></td>
                ...
                <tr style="font-weight:bold">
               	...
               	<tfoot>
                    ...
                    <th>&nbsp;</th>
                    <th align="right"><? echo number_format(($total_yarn_cost)-($total_yarn_return_cost),2); ?></th>
                </tfoot>
            </table>
		</div>
	</fieldset>
			...
			<table border="1" class="rpt_table" rules="all" width="650" cellpadding="0" cellspacing="0">
				<thead>
                	<tr>
                        ...
                        <th colspan="2">PO No. : <? echo chop($po_no,','); ?></th>
                        <th>Po Qty.: <? echo $po_qty; ?></th>
                    </tr>
                	<tr>
						<th colspan="3">Gmts Item :<? echo implode(",",array_filter(array_unique(explode(",",$gmts_item)))); ?></th>
				...
				foreach($result as $row)
				{
					...
					foreach($po_id_arr as $p_row)
					{
						$req_qty +=$yarn_req_arr[$p_row][$row[csf('count_id')]][$row[csf('copm_one_id')]][$row[csf('color')]][$row[csf('type_id')]];
						$amount +=$yarn_req_amt_arr[$p_row][$row[csf('count_id')]][$row[csf('copm_one_id')]][$row[csf('color')]][$row[csf('type_id')]];
					}
		...
		<div id="report_container">
			<table border="1" class="rpt_table" rules="all" width="850" cellpadding="0" cellspacing="0">
				<thead>
                	...
                	<tr>
						<th colspan="5">Gmts Item :<? echo implode(",",array_filter(array_unique(explode(",",$gmts_item)))); ?></th>
                        <th colspan="2">Style : <? echo $style_no; ?></th>
                        <th colspan="2">Plan Cut Qty.: <? echo $plan_cut_qty; ?></th>
                    </tr>
                	<tr>
				...
				//$result=sql_select($sql);
				$i=1;
				foreach ($po_id_arr as $p_rows)
				{
					foreach($popUpDataArray[$p_rows] as $count=>$countwisevalue)
					{
