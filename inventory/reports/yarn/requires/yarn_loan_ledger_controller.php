...
if($action=="generate_report")
{
	...
	$item_sql= sql_select("SELECT id, product_name_details, item_group_id, yarn_count_id, yarn_comp_type1st, yarn_comp_percent1st, yarn_type, color, gsm, brand from product_details_master where company_id=$cbo_company_name and item_category_id in(1) and status_active=1 and is_deleted=0");
	...
	foreach($item_sql as $row)
	{
		...
		$item_descrip_arr[$row[csf("id")]]["yarn_count_id"]=$row[csf("yarn_count_id")];
		$item_descrip_arr[$row[csf("id")]]["yarn_comp_type1st"]=$row[csf("yarn_comp_type1st")];
		$item_descrip_arr[$row[csf("id")]]["yarn_comp_percent1st"]=$row[csf("yarn_comp_percent1st")];
		$item_descrip_arr[$row[csf("id")]]["yarn_type"]=$row[csf("yarn_type")];
		$item_descrip_arr[$row[csf("id")]]["color"]=$row[csf("color")];
		$item_descrip_arr[$row[csf("id")]]["gsm"]=$row[csf("gsm")];
		$item_descrip_arr[$row[csf("id")]]["brand"]=$row[csf("brand")];
	}
	/*echo '<pre>';
	...
	<div style="width:1420px;" id="scroll_body">
		<table width="1370" >
            <tr class="form_caption">
                <td colspan="14" align="center"><b>Yarn Loan Ledger</b></td>
            </tr>
            <tr class="form_caption">
                <td colspan="14" align="center"><b><? echo $companyArr[$cbo_company_name]; ?></b></td>
            </tr>
        </table>
			<?
			...
			foreach($loan_party_arr as $pary_id=>$val)
			{
                ...
                <table width="1370" > 
                    <tr class="form_caption">
                        <td colspan="16" align="center"><b><? echo  $supplierArr[$pary_id]; ?></b></td>
                    </tr>
                </table>
                <table width="1370" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" id="table_body_<? echo $m; ?>">
                	<thead>
                        <tr>
                            ...
                            <th width="70" align="center">Gate Entry No.</th>
                            <th width="70" align="center">Gate Pass No.</th> 
                            <th width="180" colspan="6" align="center">Item Description</th>
                    ...
                    foreach($all_party_data[$pary_id] as $row)
                    {
						if(!in_array($row["prod_id"],$check_product))
						{
							...
							?>
                             <tr bgcolor="#DFDFDF">
                                <td colspan="12" align="right"><b>Opening:</b>&nbsp;</td>
                        ...
                        <tr>
                            ...
                            <td width="80"><? echo $yarn_count_arr[$item_descrip_arr[$row["prod_id"]]["yarn_count_id"]]; ?></td width="100">
		                    <td width="100"><? echo $composition[$item_descrip_arr[$row["prod_id"]]["yarn_comp_type1st"]].' ';
		                    if($item_descrip_arr[$row["prod_id"]]["yarn_comp_percent1st"]!='') {echo $item_descrip_arr[$row["prod_id"]]["yarn_comp_percent1st"].'%';}?></td>
		                    <td width="70"><? echo $yarn_type[$item_descrip_arr[$row["prod_id"]]["yarn_type"]];?></td>
		                    <td width="70"><? echo $yarn_color_arr[$item_descrip_arr[$row["prod_id"]]["color"]]; ?></td>
		                    <td width="70"><? echo $item_descrip_arr[$row["prod_id"]]["gsm"];?></td>
		                    <td width="70"><? echo $yarn_brand_arr[$item_descrip_arr[$row["prod_id"]]["brand"]]; //$item_descrip_arr[$row["prod_id"]]["product_name_details"];?></td>    
                    ...
                    <tfoot>
                        <tr>
                            <th colspan="12" align="right"><b>Party Total:</b>&nbsp;</th>
