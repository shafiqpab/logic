...
if ($action == "generate_report")
{
	...
	else
	{
		...
		/*
		COMMENTED BY JAHID HASAN FOR PAY_PODE DISABLE
		$sql_receive = "SELECT a.prod_id,a.receive_basis,d.pay_mode as ydw_pay_mode,e.pay_mode as purchase_order_pay_mode,a.store_id, max(a.weight_per_bag) as weight_per_bag, max(a.weight_per_cone) as weight_per_cone,max(a.transaction_date) AS last_receive_date,
		...
		sum(case when a.transaction_type=4 and c.knitting_source!=1 and a.transaction_date between '" . $from_date . "' and '" . $to_date . "' then a.cons_amount else 0 end) as rcv_outside_return_amt,0 as rcv_adjustment_qty, 0 as rcv_adjustment_amt, a.floor_id, a.room, a.rack, a.self, a.bin_box, c.source
		from inv_transaction a left join wo_non_order_info_mst e on a.pi_wo_batch_no=e.id, inv_receive_master c left join wo_yarn_dyeing_mst d on c.booking_id=d.id and c.receive_purpose=2 where a.mst_id=c.id and a.transaction_type in (1,4) and a.item_category=1 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $company_cond $store_cond $source_cond group by a.store_id,a.prod_id,a.receive_basis,d.pay_mode,e.pay_mode,a.floor_id,a.room,a.rack, a.self, a.bin_box, c.source

		union all

		SELECT a.prod_id,a.receive_basis,0 as ydw_pay_mode,0 as purchase_order_pay_mode,a.store_id,max(a.weight_per_bag) as weight_per_bag, max(a.weight_per_cone) as weight_per_cone,max(a.transaction_date) AS last_receive_date, 0 as rcv_total_opening,0 as rcv_total_opening_amt, 0 as rcv_total_opening_rate, 0 as purchase, 0 as purchase_amt,0 as rcv_loan, 0 as rcv_loan_amt,0 as rcv_inside_return,0 as rcv_inside_return_amt,0 as rcv_outside_return,0 as rcv_outside_return_amt,sum(case when a.transaction_type in (1,4) and a.receive_basis=30 and a.transaction_date<= '". $to_date . "' then a.cons_quantity else 0 end) as rcv_adjustment_qty,sum(case when a.transaction_type in (1,4) and a.receive_basis=30 and a.transaction_date<= '" . $to_date . "' then a.cons_amount else 0 end) as rcv_adjustment_amt, a.floor_id, a.room, a.rack, a.self, a.bin_box, 0 as source from inv_transaction a where a.receive_basis=30 and a.item_category=1 and a.transaction_type in (1,4) and a.status_active=1 and a.is_deleted=0 $company_cond $store_cond group by a.store_id,a.prod_id,a.receive_basis, a.floor_id, a.room,a.rack, a.self, a.bin_box";*/

		$sql_receive = "SELECT a.prod_id,a.receive_basis,a.store_id, max(a.weight_per_bag) as weight_per_bag, max(a.weight_per_cone) as weight_per_cone,max(a.transaction_date) AS last_receive_date,
		sum(case when a.transaction_type in (1,4) and a.transaction_date<'" . $from_date . "' then a.cons_quantity else 0 end) as rcv_total_opening,
		sum(case when a.transaction_type in (1,4) and a.transaction_date<'" . $from_date . "' then a.cons_amount else 0 end) as rcv_total_opening_amt,
		sum(case when a.transaction_type in (1,4) and a.transaction_date<'" . $from_date . "' then a.cons_rate else 0 end) as rcv_total_opening_rate,
		sum(case when a.transaction_type in (1) and c.receive_purpose<>5 and a.transaction_date between '" . $from_date . "' and '" . $to_date . "' then a.cons_quantity else 0 end) as purchase,
		...
		sum(case when a.transaction_type=4 and c.knitting_source=1 and a.transaction_date between '" . $from_date . "' and '" . $to_date . "' then a.cons_amount else 0 end) as rcv_inside_return_amt,
		sum(case when a.transaction_type=4 and c.knitting_source!=1 and a.transaction_date between '" . $from_date . "' and '" . $to_date . "' then a.cons_quantity else 0 end) as rcv_outside_return,
		sum(case when a.transaction_type=4 and c.knitting_source!=1 and a.transaction_date between '" . $from_date . "' and '" . $to_date . "' then a.cons_amount else 0 end) as rcv_outside_return_amt,0 as rcv_adjustment_qty, 0 as rcv_adjustment_amt, a.floor_id, a.room, a.rack, a.self, a.bin_box,a.buyer_id, c.source, c.remarks
		from inv_transaction a, inv_receive_master c where a.mst_id=c.id and a.transaction_type in (1,4) and a.item_category=1 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $company_cond $store_cond $source_cond group by a.store_id,a.prod_id,a.receive_basis,a.floor_id,a.room,a.rack, a.self, a.bin_box,a.buyer_id, c.source, c.remarks

		union all

		SELECT a.prod_id,a.receive_basis,a.store_id,max(a.weight_per_bag) as weight_per_bag, max(a.weight_per_cone) as weight_per_cone,max(a.transaction_date) AS last_receive_date, 0 as rcv_total_opening,0 as rcv_total_opening_amt, 0 as rcv_total_opening_rate, 0 as purchase, 0 as purchase_amt,0 as rcv_loan, 0 as rcv_loan_amt,0 as rcv_inside_return,0 as rcv_inside_return_amt,0 as rcv_outside_return,0 as rcv_outside_return_amt,sum(case when a.transaction_type in (1,4) and a.receive_basis=30 and a.transaction_date<= '". $to_date . "' then a.cons_quantity else 0 end) as rcv_adjustment_qty,sum(case when a.transaction_type in (1,4) and a.receive_basis=30 and a.transaction_date<= '" . $to_date . "' then a.cons_amount else 0 end) as rcv_adjustment_amt, a.floor_id, a.room, a.rack, a.self, a.bin_box,a.buyer_id, 0 as source, null as remarks from inv_transaction a where a.receive_basis=30 and a.item_category=1 and a.transaction_type in (1,4) and a.status_active=1 and a.is_deleted=0 $company_cond $store_cond group by a.store_id,a.prod_id,a.receive_basis, a.floor_id, a.room,a.rack, a.self, a.bin_box,a.buyer_id";
		...
		foreach ($result_sql_receive as $row)
		{
			...
			$receive_array[$row[csf("prod_id")]]['last_receive_date'] = $row[csf("last_receive_date")];
			...
			if(empty($checkedRcvProdBuyerArr[$row[csf("prod_id")]][$row[csf("buyer_id")]]))
			{
				$rcv_trans_buyer_arr[$row[csf("prod_id")]][] = $buy_short_name_arr[$row[csf("buyer_id")]];
			}
			
			$checkedRcvProdBuyerArr[$row[csf("prod_id")]][$row[csf("buyer_id")]] = $row[csf("buyer_id")];
		} 
        //echo "<pre>"; print_r($receive_array);
		unset($result_sql_receive);

		$issue_array = array();
		$sql_issue = "select a.prod_id, a.store_id as store_id,max(a.transaction_date) AS last_issue_date,
		...
		where a.mst_id=c.id and a.item_category=1 and a.transaction_type IN (2, 3) and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $store_cond group by a.store_id,a.prod_id, a.floor_id, a.room,a.rack, a.self, a.bin_box

		union all

		select a.prod_id, a.store_id,max(a.transaction_date) AS last_issue_date,
		...
		where a.item_category=1 and a.transaction_type IN (2, 3) and a.status_active=1 and a.is_deleted=0 $store_cond group by a.store_id,a.prod_id, a.floor_id, a.room,a.rack, a.self, a.bin_box";
		//echo $sql_issue; die;
		...
		foreach ($result_sql_issue as $row)
		{
			...
			$issue_array[$row[csf("prod_id")]]['last_issue_date'] = $row[csf("last_issue_date")];
		...
		$trans_info_sql = "select a.transfer_criteria,b.from_prod_id,b.to_prod_id,b.buyer_id from inv_item_transfer_mst a,inv_item_transfer_dtls b where a.id = b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form in(10,11) $company_cond";
		$trans_info_sql_result = sql_select($trans_info_sql);

		$checkedProdBuyerArr = array();
		foreach ($trans_info_sql_result as $row)
		{
			$trprod_id = ($row[csf("transfer_criteria")]==1)?$row[csf("to_prod_id")]:$row[csf("from_prod_id")];

			if(empty($checkedTrnsProdBuyerArr[$trprod_id][$row[csf("buyer_id")]]))
			{
				$rcv_trans_buyer_arr[$trprod_id][] = $buy_short_name_arr[$row[csf("buyer_id")]];
			}
			
			$checkedTrnsProdBuyerArr[$trprod_id][$row[csf("buyer_id")]] = $row[csf("buyer_id")];
								...
								if($value_with == 1)
								{
									if (number_format($stockInHand, 2) > 0.00)
									{
										...
										<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
											...
											<td width="100"><p><? echo $receive_array[$row[csf("id")]]["last_receive_date"];?></p></td>
											<td width="100"><p><? echo $issue_array[$row[csf("id")]]["last_issue_date"];?></p></td>
											<td width="100"><p><? echo $totalRcv;?></p></td>
											<td width="100"><p>
												<?
													if( $yarnTestQalityCommentsArr[$row[csf("id")]] != "") { ?>
													<a href='##' onclick="show_test_report('<? echo $row[csf('company_id')]?>','<? echo $row[csf('id')] ?>')"> <? echo $row[csf("lot")]; ?></a>
                                    ...
                                    if(number_format($stockInHand, 2) <= 0.00)
                                    {
                                    	if( $receive_array[$row[csf("id")]]['purchase'] != 0 || $receive_array[$row[csf("id")]]['rcv_inside_return'] != 0 || $receive_array[$row[csf("id")]]['rcv_outside_return'] != 0 || $receive_array[$row[csf("id")]]['rcv_loan'] != 0 || $issue_array[$row[csf("id")]]['issue_inside'] != 0 || $issue_array[$row[csf("id")]]['issue_outside'] != 0 || $issue_array[$row[csf("id")]]['rcv_return']!= 0 || $issue_array[$row[csf("id")]]['issue_loan']!= 0)
                                    	{
											...
											<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
												...
												<td width="100" class="wrap_break"><? echo $yarn_type[$row[csf("yarn_type")]]; ?></td>
												<td width="120" class="wrap_break"><? echo $color_name_arr[$row[csf("color")]]; ?></td>
												<td width="100"><p><? echo $yarn_type[$row[csf("last_receive_date")]];?></p></td>
											    <td width="100"><p><? echo $yarn_type[$row[csf("last_issue_date")]];?></p></td>
											    <td width="100"><p><? echo $yarn_type[$row[csf("totalRcv")]];?></p></td>
		...
		else if ($type == 5 || $type == 10)
		{
			...
			MIN(case when transaction_type in (1,4,5) then transaction_date else null end) as all_type_rcv_mrr_min_date,
	        ...
	        AND is_deleted = 0
	        AND item_category = 1
	        AND transaction_type IN (1, 4, 5,2,3,6)  ".where_con_using_array($prod_ids_arr,0,'prod_id')."
			GROUP BY prod_id");
			$mrr_info_arr = array();
											...
											if ($i > 1) {
												...
												$sub_stock_value_usd = 0;
												  ...
												  <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
													...
													<td width="60"><? echo $row[csf("id")]; ?></td>
													<td width="60"><p style="mso-number-format:'\@';"><? echo $yarn_count_arr[$row[csf("yarn_count_id")]]; ?>&nbsp;</p></td>
													<td width="150"><p><? echo $compositionDetails; ?></p></td>
													<td width="100"><p style="mso-number-format:'\@';"><? echo $yarn_type[$row[csf("yarn_type")]]; ?></p></td>
													<td width="80"><p><? echo $color_name_arr[$row[csf("color")]]; ?></p></td>
													<td width="100"><p><? echo $mrr_info_arr[$row[csf("id")]]['mrr_recieved_last_date']; ?></p></td>
													<td width="100"><p><? echo $mrr_info_arr[$row[csf("id")]]['mrr_issue_last_date']; ?></p></td>
													<td width="100"><p><? echo number_format($mrr_info_arr[$row[csf("id")]]['mrr_total_recieved_quantity'],2,".",""); ?></p></td>
													<td width="100"><p style="mso-number-format:'\@';">
														<?
													...
													<td width="80">
														<?
														echo ltrim(implode(",",array_unique($rcv_trans_buyer_arr[$row[csf("id")]])),',');														?>
													</td>
													...
													<td align="right"><p><? echo $remarks;?></p></td>
												  </tr>
												  <?
												  $i++;
										  ...
										  else
										  {
											if (!in_array($check_string, $checkArr)) {
												...
												if ($i > 1) {
													?>
													<tr bgcolor="#CCCCCC" style="font-weight:bold">
														<td colspan="<? echo ($type == 5) ? 12 : 11; ?>" align="right">Sub Total</td>
		...
		else if($type == 8)
		{
					...
					if(($value_with ==1 && (number_format($openingBalance,2) > 0.00 || number_format($stockInHand,2) > 0.00 ) )    ||    ($value_with ==0 && (number_format($openingBalance,2) > 0.00 || number_format($stockInHand,2) > 0.00 || number_format($totalRcv,2) > 0.00 || number_format($totalIssue,2) > 0.00 )))
					{
						...
						if ((($get_upto == 1 && $ageOfDays > $txt_days) || ($get_upto == 2 && $ageOfDays < $txt_days) || ($get_upto == 3 && $ageOfDays >= $txt_days) || ($get_upto == 4 && $ageOfDays <= $txt_days) || ($get_upto == 5 && $ageOfDays == $txt_days) || $get_upto == 0) && (($get_upto_qnty == 1 && $stockInHand > $txt_qnty) || ($get_upto_qnty == 2 && $stockInHand < $txt_qnty) || ($get_upto_qnty == 3 && $stockInHand >= $txt_qnty) || ($get_upto_qnty == 4 && $stockInHand <= $txt_qnty) || ($get_upto_qnty == 5 && $stockInHand == $txt_qnty) || $get_upto_qnty == 0)) {
							...
							if (!in_array($check_string, $checkArr)) {
								...
								if ($i > 1) {
									...
									<tr bgcolor="#CCCCCC" style="font-weight:bold">
										...
										<td width="140" align="right" style="word-wrap:break-word; word-break: break-all;">&nbsp;</td>
							...
							<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
								...
								<td width="90" style="word-wrap:break-word; word-break: break-all;" align="right"><? echo number_format($issue_array[$row[csf("id")]]['issue_loan'], 2); ?>24</td>
								<td width="100" align="right" style="word-wrap:break-word; word-break: break-all;"><? echo number_format($totalIssue, 2); ?>25</td>
								<td width="100" align="right" style="word-wrap:break-word; word-break: break-all;"><? echo number_format($totalIssueValue, 2)?>26</td>
								<td width="100" align="right" style="word-wrap:break-word; word-break: break-all;"><? echo number_format($stockInHand, 2); ?>27</td>
								<?
								$stock_value = 0;
								if ($show_val_column == 1)
								{
									...
									echo '<td width="100" align="right" style="word-wrap:break-word; word-break: break-all;">' . number_format($stock_value, 2) . '</td>';
									echo '<td width="100" align="right" style="word-wrap:break-word; word-break: break-all;">' . number_format($avg_rate_usd, 2); '</td>';
                              ...
                              if ($store_wise == 1)
                              {
									...
									echo '<td width="100" style="word-wrap:break-word; word-break: break-all;">' . $store_name1 . '</td>';
								}
								else {
									if ($allocated_qty_variable_settings == 1) {
								...
								<td  width="140" align="right" style="word-wrap:break-word; word-break: break-all;"><?
								if ($stockInHand > 0)
									echo $daysOnHand;
								else
										echo "&nbsp;"; //$daysOnHand;
			...
			<table width="<? echo $table_width +$value_width; ?>" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" id="table_footer">
				<tr class="tbl_bottom">
					...
					<td width="100" align="right" style="word-wrap:break-word; word-break: break-all;"><? echo number_format($grand_total_opening_amount_value,2);?></td>
					...
					<?
					if ($show_val_column == 1) {
						echo '<td width="110" align="right" style="word-wrap:break-word; word-break: break-all;" >&nbsp;</td>';
		...
		else if ($type == 11) // count & composition Bottom
		{
									...
									if (number_format($stockInHand, 2) > 0.00)
									{
										...
										<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
											...
											if ($store_wise == 1)
											{
												...
												{
													$store_id = $receive_store_id;
												}

												foreach ($receive_store_id as $val) {
													if ($store_name == "")
														$store_name = $store_arr[$val];
													else
														$store_name .= ", " . $store_arr[$val];
		...
		else if ($type == 14)
		{
			...
			$sql = "SELECT a.id, a.yarn_count_id, a.unit_of_measure, b.source, b.remarks, d.yarn_category_type, e.yarn_count, e.sequence_no,nvl(c.buyer_id,0) as buyer_id
		    from product_details_master a, inv_receive_master b, inv_transaction c,lib_composition_array d,lib_yarn_count e
			where a.yarn_comp_type1st=d.id and a.id=c.prod_id and c.mst_id=b.id and a.yarn_count_id=e.id and a.item_category_id=1 and d.yarn_category_type >0 and b.source<>0 and c.status_active=1 and c.is_deleted=0 and b.entry_form=1 $company_cond $search_cond
			group by b.source, b.remarks, a.id, a.yarn_count_id, a.unit_of_measure, d.yarn_category_type, e.sequence_no, e.yarn_count,c.buyer_id
			...
			//echo "<pre>";print_r($import_amount_arr);
			$i = 1;
			ob_start();
			?>
			<style>
			...
			<div style="margin-top:5px">
				...
				<div style="width:1190px; overflow-y:scroll; max-height:350px" id="scroll_body" >
					<table width="1170" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all" id="table_body">
						...
						foreach ($mainArr as $k_yct => $v_yct)
						{
							$tot_local_yarn_qty=0;$tot_import_yarn_qty=0;$tot_stock_qty=0;
							$i=1;
							foreach ($v_yct as $k_count => $k_data)
							{
								foreach ($k_data as $k_buyer => $row)
											...
											if (number_format($stockInHand, 2) > 0.00)
											{
												...
												<?
													if(!in_array($k_yct,$k_chk))
													{
														$k_chk[]=$k_yct;
														?>
													<td width="150" class="wrd_brk" valign="middle" align="center" rowspan="<? echo $yarn_cat_span ;?>" ><p><? echo $yarn_type_for_entry[$k_yct]; ?>&nbsp;</p></td>
													<? } ?>
													<td width="50" class="wrd_brk" rowspan="<? echo $yarn_count_span ;?>" ><? echo $i; ?></td>
													<td width="100" class="wrd_brk" valign="middle" align="center" rowspan="<? echo $yarn_count_span ;?>"><p><? echo $yarn_count_arr[$k_count]; ?>&nbsp;</p></td>

													<td width="150" class="wrd_brk" align="center"><p><? echo $buyer_dtls[$k_buyer]; ?>&nbsp;</p></td>
													<td width="90" class="wrd_brk" align="center"><p><? echo $unit_of_measurement[$row['unit_of_measure']]; ?>&nbsp;</p></td>
									...
									foreach ($comp_val as $color => $stock_qty) {
										...
										if($value_with == 1)
										{
											if (number_format($stock_qty, 2) > 0.00)
											{
												...
												<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
													<td width="50"><? echo $i; ?></td>
													<td width="100" class="wrd_brk"><p><? echo $yarn_count_arr[$count]; ?>&nbsp;</p></td>
													<td width="200" class="wrd_brk"><p><? echo $fullCompositionValue; ?>&nbsp;</p></td>
													<td width="100" class="wrd_brk"><p><? echo $yarn_type[$type]; ?>&nbsp;</p></td>
													<td width="100" class="wrd_brk"><p><? echo $colorArr[$color]; ?>&nbsp;</p></td>
...
if ($action == "yarn_test_report")
{
	...
	if ($action == "allocation_popup")
	{
		...
		if($is_dyied_yarn==1) // old dyed and dyed yarn structure adjust
		{
			...
			{
				$sql_cond .= (!empty($po_break_down_arr))?" or a.id in(".implode(",",$po_break_down_arr).")":"";
			}
		}
		else
		{
		...
		$plan_sql="select a.po_id,a.booking_no,a.dtls_id,b.requisition_no,b.prod_id from ppl_planning_entry_plan_dtls a,ppl_yarn_requisition_entry b where a.dtls_id=b.knit_id and b.status_active=1 and b.prod_id=$prod_id group by a.po_id,a.booking_no,a.dtls_id,b.requisition_no,b.prod_id"; // and a.status_active=1  ommit it program can be delete after issue
		//echo $plan_sql; die();
		...
		foreach ($result_issue as $row)
		{
			...
			if($row[csf('dyed_type')] == 1) // Except SMN booking
			{
				...
				foreach ($booking_arr as $booking_no)
				{
					...
					if( ($booking_no != "") && ($expBookinNo[1] != 'SMN') )
					{
				...
				if($row[csf("issue_basis")] == 3 || $row[csf("issue_basis")]==1 || $row[csf("issue_basis")] == 8)
				{
					if($row[csf("issue_basis")]==1)
					{
						...
						if( ($issue_basis == 1) && ( $issue_purpose==2 || $issue_purpose== 7 || $issue_purpose== 12 || $issue_purpose== 15 || $issue_purpose== 38 || $issue_purpose== 46))
						{
							...
							$booking_no = preg_replace('/\s+/', '', $booking_no); // For all whitespace (including tabs and line ends)
							$expBookinNo = explode("-", $booking_no);
							if( ($booking_no != "") && ($expBookinNo[1] != 'SMN') )
							{
		...
		//echo "<pre>";
		//print_r($job_wose_issue_return_array);  die();
		...
		if(!empty($smnIssueIdArr))
		{
			...
			foreach($sqlRtnQtyRslt as $row)
			{
				...
				{ // Demand 
					$smn_booking_no = return_field_value("b.booking_no as BOOKING_NO","ppl_yarn_requisition_entry a,ppl_planning_entry_plan_dtls b","a.knit_id=b.dtls_id and a.requisition_no ='".$row[csf("requisition_no")]."' and a.prod_id ='".$row[csf("prod_id")]."' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","BOOKING_NO");
				}
				...
				<table width="1190" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" id="table_body">
					...
					if(!empty($result_allocation))
					{
						foreach ($result_allocation as $row)
						{
							...
							{
								$issue_basis = array_unique($issue_arr[$row[csf('job_no')]][$prod_id]);
								$buyer_id = $smn_issue_booking_qnty_arr[$row[csf("booking_no")]][$prod_id]['buyer_id'];
							...
							if($expBookinNo[1] == 'SMN')
							{
								$smn_issue_booking_qnty = $smn_issue_booking_qnty_arr[$row[csf("booking_no")]][$prod_id]['issue_qty'];
							}
							...
							if($row[csf("is_dyied_yarn")] == 1)
							{
								...
								if($expBookinNo[1] == 'SMN')
								{
									$return_qty = $job_wose_issue_return_array[$row[csf("job_no")]][$prod_id]+$smn_issue_return_array[$row[csf("booking_no")]][$prod_id];
								}
							}
							else
							{
							...
							<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" title="<? echo $title; ?>">
								...
								<td width="100"><p><? echo $group_ref;?></p></td>
								...
								<td width="70" align="right" title="<? echo $issue_id."==".$issue_qty .'=='. $issue_qty_wo .'=='.$smn_issue_booking_qnty; ?>"><? echo number_format(($issue_qty+$issue_qty_wo+$smn_issue_booking_qnty), 2); //$issue_qty."+".$issue_qty_wo."+".$smn_issue_booking_qnty;  ?></td>
				...
				<table width="700" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" id="table_body">
					...
					foreach ($result as $row)
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
							...
							<td width="100"><p><? echo $rcv_basis; ?></p></td>
							<td width="100"><p><? echo $rcv_purpose; ?></p></td>
				...
				<table width="700" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" id="table_body">
					...
					<tfoot>
						...
						<tr>
							...
							<th><? $stock_tot_qnty=$tot_recv_qnty-$tot_issue_qnty; echo number_format($stock_tot_qnty, 2); ?>&nbsp;</th>
							<th>&nbsp;</th>
							<th>&nbsp;</th>
						</tr>
					</tfoot>
	...
	if ($action == "stock_popup_store_methode_upto")
	{
		...
		{
			if($store_upto_variable==2) { $store_method_upto= ", b.store_id,b.floor_id";}
			if($store_upto_variable==3) { $store_method_upto= " , b.store_id,b.floor_id,b.room"; }
			if($store_upto_variable==4) { $store_method_upto= " , b.store_id,b.floor_id,b.room,b.rack"; }
			if($store_upto_variable==5) { $store_method_upto= " , b.store_id,b.floor_id,b.room,b.rack,b.self"; }
			if($store_upto_variable==6) { $store_method_upto= " , b.store_id,b.floor_id,b.room,b.rack,b.self,b.bin_box"; }
		}
	...
	if($action=="company_wise_report_button_setting")
	{
		...
		if($print_report_format != "")
		{
			$print_report_format_arr=explode(",",$print_report_format);
			 ...
			 echo "$('#search13').hide();\n";
			 echo "$('#search16').hide();\n";
			 echo "$('#source_wise').hide();\n";
			 echo "$('#cc_wise').hide();\n";

			foreach($print_report_format_arr as $id)
			{
				if($id==152){echo "$('#search').show();\n";}
				if($id==195){echo "$('#search1').show();\n";}
				if($id==102){echo "$('#search2').show();\n";}
				if($id==103){echo "$('#search3').show();\n";}
				if($id==104){echo "$('#search4').show();\n";}
				if($id==105){echo "$('#search5').show();\n";}
				if($id==106){echo "$('#search7').show();\n";}
				if($id==107){echo "$('#search8').show();\n";}
				if($id==812){echo "$('#search10').show();\n";}
				if($id==813){echo "$('#search11').show();\n";}
				if($id==338){echo "$('#search12').show();\n";}
				if($id==778){echo "$('#search13').show();\n";}
				if($id==811){echo "$('#search16').show();\n";}
				if($id==814){echo "$('#source_wise').show();\n";}
				if($id==815){echo "$('#cc_wise').show();\n";}
			}
		}
		else
		{
		...
		$rslt_color = sql_select("SELECT a.id, a.color_name from lib_color a, product_details_master b where a.id=b.color $com_cond and b.item_category_id=1 and a.status_active = 1 and a.is_deleted = 0 group by a.id, a.color_name order by a.color_name");
			...
			foreach ($rslt_color as $val)
			{
				...
				<tr bgcolor="<? echo $bgcolor; ?>" onclick="js_set_value(<? echo $i; ?>)" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>">
					...
					<td width="" title="<? echo $val[csf('id')];?>"><p><? echo $val[csf('color_name')]; ?></p></td>
				</tr>
				<?
				$i++;
			}
