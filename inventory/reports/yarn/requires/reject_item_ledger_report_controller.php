...
{		  
    ...
    <script>
		...
		{ 
			show_list_view ( document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_search_common').value+'_'+<? echo $company; ?>, 'create_lot_search_list_view', 'search_div', 'reject_item_ledger_report_controller', 'setFilterGrid("list_view",-1)');
		}
    </script>
    <body>
...
{ 
	...
	$sql_receive_mrr = "select a.id as trid, a.transaction_type, b.recv_number 
			...
			where a.prod_id in ($txt_product_id) and a.mst_id=b.id and a.transaction_type=4 and a.item_category=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0"; 
	$result_rcv = sql_select($sql_receive_mrr);
	$receiveMRR=array();
	$trWiseReceiveMRR=array();
	foreach($result_rcv as $row)
	{
		$receiveMRR[$row[csf("trid")]]= $row[csf("recv_number")];
	...
	$sql_issue_mrr = "select a.sys_challan_no as issue_number, b.trans_id as trid, 0 as transaction_type
			from  inv_scrap_sales_mst a, inv_scrap_sales_dtls b
			where b.prod_id in ($txt_product_id) and a.id=b.mst_id and a.item_category=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";		 
	$result_iss = sql_select($sql_issue_mrr);
	$issueMRR=array();$issuePupose=array();
	foreach($result_iss as $row)
	{
		$issueMRR[$row[csf("trid")]] = $row[csf("issue_number")];
		//$issuePupose[$row[csf("trid")]] = $yarn_issue_purpose[$row[csf("issue_purpose")]]; 
	}
	...
	{ 
		...
		$sql = "select a.id, a.prod_id, a.transaction_date, a.transaction_type, a.cons_reject_qnty, a.cons_rate, a.cons_amount, b.product_name_details, b.unit_of_measure, b.lot, c.trans_id
				from inv_transaction a left join inv_scrap_sales_dtls c on a.prod_id=c.prod_id and a.transaction_type=4, product_details_master b 
				where a.prod_id in ($txt_product_id) and a.transaction_type=4 and a.prod_id=b.id $search_string and a.item_category=1 and a.status_active=1 and a.is_deleted=0 $search_cond order by a.prod_id, a.id ASC";		
				...
				<?
					...
					foreach($result as $row)
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>" <? echo $stylecolor; ?> onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
							...
							<td width="80"><? echo $row[csf("transaction_date")]; ?></td>                                 
							<td width="120"><p><? echo $mrrArray[$row[csf("id")]]; ?></p></td>
							...
							<td width="80" align="right"><? if($row[csf("transaction_type")]==4) echo number_format($row[csf("cons_reject_qnty")],2); ?></td>
							...
							<td width="60" align="right"><? echo number_format($total_balValue/$total_balQnty,2); ?></td>
