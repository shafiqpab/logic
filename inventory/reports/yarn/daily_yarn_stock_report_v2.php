...
function generate_report(type)
{
	...
	var cbo_yarn_source = $("#cbo_yarn_source").val();
...
'<html><head><title></title><link rel="stylesheet" href="../../../css/style_print.css" type="text/css"/><style></style></head><body>'+document.getElementById('report_container2').innerHTML+'</body</html>');
	d.close(); 
...
function openmypage(prod_id,action)
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/daily_yarn_stock_report_controller_v2.php?companyID='+companyID+'&prod_id='+prod_id+'&action='+action, 'Yarn Allocation Statement', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../../');
}
...
function openmypage_stock(prod_id,action)
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/daily_yarn_stock_report_controller_v2.php?prod_id='+prod_id+'&action='+action, 'Yarn Stock Details', 'width='+popup_width+', height=400px,center=1,resize=0,scrolling=0','../../');
}
...
function openmypage_trans(prod_id,trans_type,store_name,from_date,to_date,action)
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/daily_yarn_stock_report_controller_v2.php?prod_id='+prod_id+'&trans_type='+trans_type+'&store_name='+store_name+'&from_date='+from_date+'&to_date='+to_date+'&action='+action, 'Yarn Transfer Details', 'width='+popup_width+', height=200px,center=1,resize=0,scrolling=0','../../');
}

function openmypage_remarks(prod_id)
{
	var popup_width='450px';
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/daily_yarn_stock_report_controller_v2.php?prod_id='+prod_id+'&action=mrr_remarks', 'Yarn Transfer Details', 'width=570px,height=350px,center=1,resize=0,scrolling=0','../../');
}
...
function openmypage_composition()
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/daily_yarn_stock_report_controller_v2.php?action=composition_popup&pre_composition_id='+pre_composition_id, 'Composition Details', 'width=410px,height=420px,center=1,resize=0,scrolling=0','../../');
...
{
	generate_report_file(company_id + '*' + productID, 'yarn_test_report','requires/daily_yarn_stock_report_controller_v2');
}

function show_test_report2(company_id, productID)
{
	generate_report_file(company_id + '*' + productID, 'yarn_test_report2','requires/daily_yarn_stock_report_controller_v2');
}

function generate_report_file(data, action, page) 
{
	window.open("requires/daily_yarn_stock_report_controller_v2.php?data=" + data + '&action=' + action, true);
}
</script>
</head>
...
<div style="width:100%;" align="left">
    ...
    <div style="width:100%;" align="center">
        <h3 style="width:1630px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3> 
            ...
            <fieldset style="width:1630px;">
                <table class="rpt_table" width="1630" cellpadding="0" cellspacing="0" border="1" rules="all">
                    ...
                    <tr>
                        <td>
							<? 
                               echo create_drop_down( "cbo_company_name", 110, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- All Company --", $selected, "load_drop_down( 'requires/daily_yarn_stock_report_controller_v2', this.value, 'load_drop_down_supplier', 'supplier' );load_drop_down( 'requires/daily_yarn_stock_report_controller_v2', this.value+'**'+document.getElementById('cbo_store_wise').value, 'load_drop_down_store', 'store_td' );get_php_form_data( this.value, 'eval_multi_select', 'requires/daily_yarn_stock_report_controller_v2' );get_php_form_data( this.value, 'company_wise_report_button_setting','requires/daily_yarn_stock_report_controller_v2' );" );
                            ?>                            
                        </td>
                        <td id="supplier"> 
							<?
                        ...
                        <td>
                            <? echo create_drop_down( "cbo_yarn_source", 80, $source, "", 1, "--Select--", 0, "", "", ""); ?>
                        </td>
                        <td>
                            <?   
                        ...
                        <td> 
                            <?
                                echo create_drop_down( "cbo_store_wise", 50, $yes_no,"", 0, "--Select--", 2, "load_drop_down( 'requires/daily_yarn_stock_report_controller_v2', document.getElementById('cbo_company_name').value+'**'+this.value, 'load_drop_down_store', 'store_td' );",0 );
                            ?>
                        </td>
                        <td id="store_td">
                            <? 
                    ...
                    <tr>
                        <td colspan="18">&nbsp;&nbsp;&nbsp;&nbsp;<? echo load_month_buttons(1); ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" name="search" id="search2" value="Count Wise Summ." onClick="generate_report(2)" style="width:100px;display:display;" class="formbutton" />
                            ...
                            <input type="button" name="search" id="search5" value="Stock Only" onClick="generate_report(5)" style="width:80px;display:display;" class="formbutton" />
                            <input type="button" name="search" id="search10" value="Stock Only2" onClick="generate_report(10)" style="width:80px;display:display;" class="formbutton" />
