...
<script>
	...
	var tableFilters = 
	{
		col_16: "none",
		col_operation: {
		id: ["value_tot_opening","value_tot_reject_qty","value_tot_closingStock"],
		col: [11,12,13],
	...
	function openmypage_item_account()
	{
		if( form_validation('cbo_company_id','Company Name')==false )
		{
		 ...
		 //alert(data);
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/yarn_reject_report_controller.php?action=item_account_popup&data='+data,'Item Account Popup', 'width=510px,height=400px,center=1,resize=0','../../')
	...
	function openmypage_party()
	{
		...
		var page_link='requires/yarn_reject_report_controller.php?action=party_popup&companyID='+companyID;
	...
	function generate_report(type)
	{
		...
		//var txt_product_id_des = $("#txt_product_id_des").val(); txt_supplier_id*txt_supplier*txt_lot_no
		...
		http.open("POST","requires/yarn_reject_report_controller.php",true);
	...
	{
		//alert(prod_id);
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/yarn_reject_report_controller.php?prod_id='+prod_id+'&action='+action+'&company_id='+company_id+'&from_date='+from_date+'&to_date='+to_date, title, 'width='+popup_width+', height=400px, center=1, resize=0, scrolling=0', '../../');
	}
                ...
                <table class="rpt_table" width="1200" cellpadding="0" cellspacing="0" border="1" rules="all">
                    ...
                    <tbody>
                        <tr class="general">
                            <td>
                                <? 
                                    echo create_drop_down( "cbo_company_id", 150, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "--Select Company--", $selected, "" );
                                ?>                            
                            </td>
                            <td>
                             ...
                             <td>
                                <input type="text" name="txt_lot_no" id="txt_lot_no" style="width:60px;" class="text_boxes" placeholder="Write"/>  
                            </td>
                            <td>
                           ...
                           <td>
                            ...
                            <td>
                                <input type="button" name="search" id="search" value="With Order" onClick="generate_report(1)" style="width:70px" class="formbutton" />
                                 <input type="button" name="search" id="search" value="Without Order" onClick="generate_report(2)" style="width:100px" class="formbutton" />
                                <input type="button" name="search" id="search" value="Product" onClick="generate_report(3)" style="width:100px" class="formbutton" />
                            </td>
                        </tr>
                    </tbody>
                    <tfoot>
