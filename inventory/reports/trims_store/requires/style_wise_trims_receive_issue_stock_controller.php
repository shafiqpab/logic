...
// Don't remove this function.(Because when any string end ] then chop function remove ])
...
if($action=="print_button_variable_setting")
{
    ...
    $print_report_format=return_field_value("format_id","lib_report_template","template_name ='".$data."' and module_id=6 and report_id=256 and is_deleted=0 and status_active=1");
...
// Style Wise Search. Report button
if ($action=="report_generate_style")
{
	...
	if($cbo_search_by == 2) // Ex-factory date
	{
		...
		foreach($sqlExFactoryData as $row)
		{
			...
			// }
			$trans_po_id_arr[$row['PO_BREAK_DOWN_ID']]=$row['PO_BREAK_DOWN_ID'];
		}
		//$all_po_id=chop($all_po_id,",");
		$all_po_id = implode(',',$trans_po_id_arr);
		...
		if($all_po_id!="")
		{
			...
			$all_po_id_arr=array_chunk($trans_po_id_arr,999);
	...
	if($cbo_store_name!=0) $store_cond=" and c.store_id =$cbo_store_name"; else $store_cond="";
    ...
    <fieldset style="width:2580px;">
		...
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="2900" class="rpt_table" >
			<thead>
				<th width="40">SL </th>
        ...
        <div style="width:2900px; overflow-y:scroll; max-height:350px;font-size:12px; " id="scroll_body">
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="2880" class="rpt_table" id="tbl_issue_status" align="left">
		    <?
			...
			$total_transfer_in_qty=$total_total_avl=$total_transfer_out_qty=$total_total_issue=0;
			...
			foreach($result as $row)
			{
				...
				$gblTempPoArr[$row[csf("po_id")]]=$row[csf("po_id")];
			...
			$con = connect();
			execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ENTRY_FORM=256",1);
			oci_commit($con);
			fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 256, 1, $gblTempPoArr, $empty_arr);
			//---------------------------------end;


			if($cbo_store_name!=0) $store_cond2=" and c.store_id =$cbo_store_name"; else $store_cond2="";
			$sql_recv="select c.receive_basis, c.booking_without_order as without_order, a.item_group_id, a.item_description, c.store_id, a.order_uom, a.item_color, a.item_size, a.cons_rate, b.po_breakdown_id as po_id, a.gmts_color_id, b.quantity , (b.quantity*a.rate) as amount, a.item_color_temp, a.prod_id, 1 as type
			from inv_receive_master c,inv_trims_entry_dtls a, order_wise_pro_details b, GBL_TEMP_ENGINE gbl
			where a.id=b.dtls_id and a.trans_id=b.trans_id and c.id=a.mst_id  and gbl.REF_VAL=b.po_breakdown_id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1 and b.trans_type=1 and b.entry_form=24 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $store_cond
			union all
			select 2 as receive_basis, 0 as without_order, a.item_group_id, a.item_description, c.store_id, a.order_uom, a.item_color, a.item_size, c.cons_rate, b.po_breakdown_id as po_id, a.COLOR as gmts_color_id, b.quantity , (b.quantity*b.ORDER_RATE) as amount, 0 as item_color_temp, b.prod_id, 2 as type
			from INV_TRANSACTION c, product_details_master a, order_wise_pro_details b, GBL_TEMP_ENGINE gbl
			where c.PROD_ID=a.id and a.id=b.prod_id and c.ID=b.trans_id and b.po_breakdown_id=gbl.REF_VAL and gbl.ENTRY_FORM=256 and gbl.USER_ID=165 and gbl.ref_from=1 and b.trans_type=5 and c.transaction_type=5 and b.entry_form in(78,112) and a.entry_form in(24) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $store_cond2"; //
			//echo $sql_recv;die;
			...
			{
				fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 256, 2, $prod_id_arr, $empty_arr);
				...
				$returnRes_date = "SELECT a.PROD_ID, MIN(a.TRANSACTION_DATE) AS MIN_DATE, MAX(a.TRANSACTION_DATE) AS MAX_DATE from inv_transaction a, GBL_TEMP_ENGINE gbl where a.is_deleted=0 and a.status_active=1 and a.item_category=4  and gbl.REF_VAL=a.prod_id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=2  group by a.prod_id";
			...
			$sql_bookingqty ="select c.cons as wo_qnty,c.amount,b.id as dtls_id,c.id,b.trim_group as item_group, $null_val c.description,c.brand_supplier,b.po_break_down_id as po_id,b.sensitivity from wo_booking_mst a, wo_booking_dtls b,wo_trim_book_con_dtls c,GBL_TEMP_ENGINE gbl where a.booking_no=b.booking_no and b.id=c.wo_trim_booking_dtls_id and b.booking_no=c.booking_no and gbl.REF_VAL=b.po_break_down_id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1 and a.company_id=$cbo_company and a.item_category=4 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  ";
			...
			if ($cbo_store_name > 0)
			{
				...
				from product_details_master a, order_wise_pro_details b, inv_transaction c, inv_item_transfer_mst d,GBL_TEMP_ENGINE gbl
				where a.id=b.prod_id and b.trans_id=c.id and d.id=c.mst_id and gbl.REF_VAL=b.po_breakdown_id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1 and item_category_id=4 and a.entry_form=24 and b.entry_form =112 and b.trans_type in(5,6) and c.transaction_type in(5,6) and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $store_cond and d.transfer_criteria=2";
				...
				foreach($store_to_store_sql_result as $rows)
				{
					...
					{
						$transfer_criteria_arr[$rows[csf('po_breakdown_id')]][$rows[csf('item_group_id')]][$rows[csf('item_color_id')]][$item_sizeId]["store_id"]=$rows[csf('store_id')];
					}
				}
				unset($store_to_store_sql_result);
				/* echo '<pre>';
				print_r($transfer_criteria_arr);die; */
			}
			

			$issue_qty_sql="select b.po_breakdown_id, a.item_color as item_color_id, a.item_size, a.item_group_id, b.quantity, c.cons_rate as rate, b.entry_form, b.trans_type, c.store_id
			from product_details_master a, order_wise_pro_details b, inv_transaction c,GBL_TEMP_ENGINE gbl
			where a.id=b.prod_id and b.trans_id=c.id  and gbl.REF_VAL=b.po_breakdown_id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1 and item_category_id=4 and b.entry_form in(25,49,73,78,112) and b.trans_type in(2,3,4,5,6) and c.transaction_type in(2,3,4,5,6) and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $store_cond";
			...
			foreach($issue_qty_sql_result as $row)
			{
				...
				$issue_data_arr[$row[csf('po_breakdown_id')]][$row[csf('item_group_id')]][$row[csf('item_color_id')]][$item_size_id]["store_id"]=$row[csf('store_id')];
				...
				if(($row[csf('entry_form')]==78 || $row[csf('entry_form')]==112) && $row[csf('trans_type')]==5)
				{
					if ($cbo_store_name > 0)
					{
						if ($transfer_criteria_arr[$row[csf('po_breakdown_id')]][$row[csf('item_group_id')]][$row[csf('item_color_id')]][$item_size_id]["store_id"]==$row[csf('store_id')]){
			...
			//echo $test_data."<pre>"; print_r($issue_data_arr);die;
			$req_check=array();
			foreach ($result as $row)
			{
				...
				foreach($job_po_id as $po_id)
				{
					...
					//echo $test_data."<pre>"; print_r($dataRecv);die;
					foreach($dataRecv as $recvRow)
					{
                        ...
                        $prod_id=$recvRow[13];
						$data_type=$recvRow[14];
						if($item_color_temp>0) $item_color_temp=$item_color_temp; else $item_color_temp=$item_color;

						if($quantity>0)
						{
							if($rowspan_color_check[$po_id][$item_group_id][$item_color_temp][$item_size]=="")
							{
								$rowspan_color_check[$po_id][$item_group_id][$item_color_temp][$item_size]=$item_group_id;
				...
				if($z>0)
				{
					...
					foreach($dataArray as $item_group_id=>$item_group_data)
					{
						...
						{
							$c=0;
							//echo count($item_color_data)."ab";
							foreach($item_color_data as $item_size=>$item_size_data)
							{
								$issue_qty=$issue_amount=$rcv_rtn_qty=$issue_rtn_qty=$transfer_in_qty=$transfer_out_qty=$net_recv_qnty=$net_recv_value=$net_issue_qnty=$net_issue_value=$wo_qty=$wo_amount=$ord_avg_rate=0;
								...
								if($recv_value!=0 && $recv_value!="" && $recv_qnty!=0 && $recv_qnty!="") $ord_avg_rate=$recv_value/$recv_qnty;
								...
								foreach($job_po_id as $po_id)
								{
									...
									{
										//echo $po_id."=".$item_group_id."=".$c_id."=".$item_size."<br>";									
								...
								$total_avl=$net_recv_qnty+$transfer_in_qty;
								if($net_recv_qnty!=0 && $net_recv_qnty!="" && $ord_avg_rate!=0 && $ord_avg_rate!="") $net_recv_value=$net_recv_qnty*$ord_avg_rate;
								//$net_issue_qnty=$issue_qty+$transfer_out_qty-$issue_rtn_qty;
								$net_issue_qnty=$issue_qty-$issue_rtn_qty;
								$transfer_out_qty=$transfer_out_qty;
								$total_issue=$net_issue_qnty+$transfer_out_qty;
								if($total_issue!=0 && $total_issue!="" && $ord_avg_rate!=0 && $ord_avg_rate!="") $net_issue_value=$total_issue*$ord_avg_rate;
								...
								<tr bgcolor="<? echo $bgcolor; ?>"  onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
									...
									<?
										...
										if($item_color)
										{
											$item_color_con=" and b.item_color in($item_color_id)";
											$item_color_con2=" and d.item_color in($item_color_id)";
										}
										...
										$po_id=$row[csf('po_id')]; $item_group=$item_group_id;

										$store_id_cond="";
										if ($cbo_store_name > 0) $store_id_cond=" and a.to_store_id=$cbo_store_name";	else $store_id_cond="";

										$mrr_sql="select a.id, a.transfer_system_id as recv_number, a.challan_no, a.transfer_date as receive_date, c.quantity as quantity, 0 as reject_qty, d.product_name_details, c.prod_id, c.trans_type
										from inv_item_transfer_mst a, inv_item_transfer_dtls b, order_wise_pro_details c, product_details_master d
										where a.id=b.mst_id and b.id=c.dtls_id and c.prod_id=d.id and a.entry_form in(78,112) and c.entry_form in(78,112) and c.trans_type in(5)  and  c.po_breakdown_id in($po_id)  and d.item_group_id='$item_group' and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 $item_color_con2 $item_size_con2 $store_id_cond
										";
										
										$dtlsArray=sql_select($mrr_sql);
										$transfer_in_qty=$dtlsArray[0]['QUANTITY'];
									?>

									<td width="80" align="right"><a href='#report_details' onClick="openmypage_transfer('<? echo $row[csf('po_id')]; ?>','<? echo $item_group_id; ?>','<? echo $item_color_id; ?>','<? echo $gmts_color; ?>','<? echo $item_size; ?>','<? echo $recv_basis; ?>','<? echo $without_order; ?>',1,'transfer_inpopup');"><?  echo number_format($transfer_in_qty,2); ?></a> </td>

									<td width="80" align="right"><? echo number_format($total_avl,2); ?></td>
									...
									<td width="90" align="right" title="<? echo "issue=".$issue_qty." issue rtn=".$issue_rtn_qty." transfer out=".$transfer_out_qty;?>"> <a href='#report_details' onClick="openmypage_issue('<? echo $row[csf('po_id')]; ?>','<? echo $item_group_id; ?>','<? echo $item_color_id; ?>','<? echo $gmts_color; ?>','<? echo $item_size; ?>','<? echo $item_desc; ?>','<? echo $recv_basis; ?>','<? echo $without_order; ?>',1,'issue_popup');"><?  echo number_format($net_issue_qnty,2); ?></a></td>
									<td width="80" align="right"><a href='#report_details' onClick="openmypage_transfer('<? echo $row[csf('po_id')]; ?>','<? echo $item_group_id; ?>','<? echo $item_color_id; ?>','<? echo $gmts_color; ?>','<? echo $item_size; ?>','<? echo $recv_basis; ?>','<? echo $without_order; ?>',2,'transfer_outpopup');"><?  echo number_format($transfer_out_qty,2); ?></a> </td>
									...
									<td width="100" align="right"><? $left_over=$total_avl-$total_issue;echo number_format($left_over,2); ?></td>
									<td width="60" align="right"><?  echo number_format($ord_avg_rate,2); ?></td>
									<td align="right" width="100">
									<?
										if($left_over!=0 && $left_over!="" && $ord_avg_rate!=0 && $ord_avg_rate!="") $tot_left_val=$left_over*$ord_avg_rate;										
			...
			<tr class="tbl_bottom">
				...
				<td><? echo number_format($total_transfer_in_qty,2); ?></td>
				<td><? echo number_format($total_total_avl,2); ?></td>
				...
				<td><? echo number_format($total_transfer_out_qty,2); ?></td>
				<td><? echo number_format($total_total_issue,2); ?></td>
...
// Style Wise Search. Report2 button
if ($action=="report_generate_style2")
{
    ...
    <fieldset style="width:2500px;">
        ...
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="2480" class="rpt_table" id="tbl_issue_status" >
			...
			$sql_bookingqty ="SELECT d.buyer_name, d.season_buyer_wise, d.job_no, d.style_ref_no, d.total_set_qnty, c.id as po_id, c.po_number, c.file_no, c.grouping as int_ref_no, c.po_quantity, b.trim_group, b.booking_no, b.uom, a.description, $null_val, a.cons, a.amount, p.currency_id, p.company_id, p.booking_date
			...
			foreach($sql_bookingqty_result as $row)
			{
				...
				{
					$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$row[csf("description")]][$row[csf("item_color")]][$row[csf("item_size")]]["book_amt"]+=$row[csf("amount")]*$conversion_data_arr[$row[csf("company_id")]][change_date_format($row[csf("booking_date")])];
				}
			}
			...
			execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ENTRY_FORM=256",1);
			oci_commit($con);
			fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 256, 1, $order_check, $empty_arr);
			//---------------------------------end;

			//echo "<pre>";print_r($details_data);die;e.QUANTITY, e.AMOUNT
			$all_order_id=chop($all_order_id,",");
			if($db_type==0) $null_val=" IFNULL(a.item_color, 0) as item_color, IFNULL(a.item_size, 0) as item_size";
			else if($db_type==2) $null_val=" nvl(a.item_color,0) as item_color, nvl(a.item_size,0) as item_size";
			
			$general_item_issue_sql="SELECT d.buyer_name, d.season_buyer_wise, d.job_no, d.style_ref_no, d.total_set_qnty, c.id as po_id, c.po_number, c.file_no, c.grouping as int_ref_no, c.po_quantity, a.item_group_id as trim_group, a.unit_of_measure as uom, a.item_description as description, b.store_id, a.id as prod_id, $null_val, e.QUANTITY as cons_quantity, e.AMOUNT as cons_amount
			from product_details_master a, inv_transaction b, order_wise_general_details e, wo_po_break_down c, wo_po_details_master d,GBL_TEMP_ENGINE gbl
			where a.id=b.prod_id and b.id=e.TRANS_ID and e.PO_BREAKDOWN_ID=c.id and c.job_no_mst=d.job_no and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form<>24 and b.transaction_type in(2,3,6) and gbl.REF_VAL=c.id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1  and b.item_category in(".implode(",",array_keys($general_item_category)).")";
			...
			foreach($general_item_issue_result as $row)
			{
				...
				if($po_data_check[$row[csf("job_no")]][2][$row[csf("trim_group")]][$row[csf("description")]][$row[csf("item_color")]][$row[csf("item_size")]][$row[csf("po_id")]]=="")
				{
					$po_data_check[$row[csf("job_no")]][2][$row[csf("trim_group")]][$row[csf("description")]][$row[csf("item_color")]][$row[csf("item_size")]][$row[csf("po_id")]]=$row[csf("po_id")];
			...
			from product_details_master a, order_wise_pro_details b, inv_transaction c,GBL_TEMP_ENGINE gbl
			where a.id=b.prod_id and b.trans_id=c.id and gbl.REF_VAL=b.po_breakdown_id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1 and item_category_id=4  and b.entry_form in(24,25,49,73,78,112) and b.trans_type in(1,2,3,4,5,6) and c.transaction_type in(1,2,3,4,5,6) and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $store_cond";
			// $p=1;
			// if($all_order_id!="")
			// {
			// 	$all_order_id_arr=array_chunk(array_unique(explode(",",$all_order_id)),999);
			...
			$store_to_store_sql="SELECT b.po_breakdown_id, a.item_color as item_color_id, a.item_size, a.item_group_id, a.item_description, b.quantity, c.cons_rate as rate, b.entry_form, b.trans_type, c.store_id, d.transfer_criteria
			from product_details_master a, order_wise_pro_details b, inv_transaction c, inv_item_transfer_mst d,GBL_TEMP_ENGINE gbl
			where a.id=b.prod_id and b.trans_id=c.id and d.id=c.mst_id and item_category_id=4 and a.entry_form=24 and b.entry_form =112 and gbl.REF_VAL=b.po_breakdown_id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1 and b.trans_type in(5,6) and c.transaction_type in(5,6) and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $store_cond and d.transfer_criteria=2";
			// $q=1;
			// if($all_order_id!="")
			// {
			// 	$all_order_id_arr=array_chunk(array_unique(explode(",",$all_order_id)),999);
						...
						foreach($item_desc_data as $item_color_id=>$item_color_data)
						{
							...
							foreach($item_color_data as $item_size=>$item_size_data)
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>"  onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
									...
									<td width="80" align="right"><? echo number_format($wo_qty-$net_recv_qnty,2); ?></td>
									<td width="100" align="right"><? echo number_format($net_recv_value,2); ?></td>
									<td width="90" align="right"> <a href='#report_details' onClick="openmypage_issue('<? echo $po_id_all; ?>','<? echo $item_group_id; ?>','<? echo $item_color_id; ?>','<? echo $gmts_color; ?>','<? echo $item_size; ?>','<? echo trim($item_desc); ?>','<? echo $recv_basis; ?>','<? echo $without_order; ?>',2,'issue_popup');"><?  echo number_format($net_issue_qnty,2); ?></a></td>
									<td width="80" align="right"><?  echo number_format($net_issue_value,2); ?></td>
...
if ($action=="report_generate_style4")
{
    ...
    <fieldset style="width:2845px;">
		...
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="2840" class="rpt_table" >
			<thead>
				<th width="40">SL </th>
                <th width="70">Buyer</th>
                <th width="70">Season</th>
                ...
                <th width="90">Req. Qty</th>
                <th width="100">Item Color</th>
                <th width="60">Item Ref</th>
                <th width="60">Item Size</th>
                <th width="80">WO. Qty</th>
                <th width="80">WO. Value (TK)</th>
                <th width="90">Recv. Qty</th>
                <th width="90">Reject Qty.</th>
        ...
        <div style="width:2840px; overflow-y:scroll; max-height:350px;font-size:12px; " id="scroll_body">
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="2830" class="rpt_table" id="tbl_issue_status" >
			...
			if($db_type==0) $null_val=" IFNULL(a.item_color, 0) as item_color, a.item_size as item_size, IFNULL(a.item_ref, 0) as item_ref";
			else if($db_type==2) $null_val=" nvl(a.item_color,0) as item_color, a.item_size as item_size, nvl(a.item_ref,0) as item_ref";
			...
			from  wo_booking_mst p, wo_trim_book_con_dtls a, wo_booking_dtls b, wo_po_break_down c, wo_po_details_master d
			where p.booking_no=a.booking_no and p.booking_no=b.booking_no and a.wo_trim_booking_dtls_id=b.id and b.po_break_down_id=c.id and c.job_id=d.id and d.company_name=$cbo_company and b.booking_type=2 and a.cons>0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $sql_cond order by d.job_no";

			//echo $sql_bookingqty;die;

			$sql_bookingqty_result=sql_select($sql_bookingqty);
			// echo "<pre>"; print_r($sql_bookingqty_result); die;
			if (count($sql_bookingqty_result)<1){
				echo "<p  style='text-align:left; color:red;'>This Reference Not Found In Booking</p>";die;
			}
			...
			foreach($sql_bookingqty_result as $row)
			{
				//if null convert to zero...........................
				$row[csf("item_color")] = $row[csf("item_color")]*1;
				$row[csf("item_size")] = $row[csf("item_size")];
				//...............................................end;
				...
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$row[csf("description")]][$row[csf("item_color")]][$row[csf("item_size")]]["item_color"]=$row[csf("item_color")];
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$row[csf("description")]][$row[csf("item_color")]][$row[csf("item_size")]]["item_size"]=$row[csf("item_size")];
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$row[csf("description")]][$row[csf("item_color")]][$row[csf("item_size")]]["item_ref"].=$row[csf("item_ref")].',';
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$row[csf("description")]][$row[csf("item_color")]][$row[csf("item_size")]]["book_qnty"]+=$row[csf("cons")];
				if($row[csf("currency_id")]==1)
				{
					$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$row[csf("description")]][$row[csf("item_color")]][$row[csf("item_size")]]["book_amt"]+=$row[csf("amount")];
			...
			from product_details_master a, inv_transaction b, wo_po_break_down c, wo_po_details_master d,GBL_TEMP_ENGINE gbl
			where a.id=b.prod_id and b.order_id=c.id and c.job_no_mst=d.job_no and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form<>24 and b.transaction_type in(2,3,6) and gbl.REF_VAL=c.id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1 and b.item_category in(".implode(",",array_keys($general_item_category)).")";
			// echo $general_item_issue_sql;die;    
			// $p=1;
			// if($all_order_id!="")
			// {
			...
			$item_mismatch_issue_sql="SELECT d.buyer_name, d.season_buyer_wise, d.job_no, d.style_ref_no, d.total_set_qnty, c.id as po_id, c.po_number, c.file_no, c.grouping as int_ref_no, c.po_quantity, a.item_group_id as trim_group, a.unit_of_measure as uom, a.item_description as description, b.store_id, a.id as prod_id, $null_val, b.cons_quantity, b.cons_amount, b.cons_reject_qnty as reject_qty
			from product_details_master a, inv_transaction b, wo_po_break_down c, wo_po_details_master d, order_wise_pro_details e,GBL_TEMP_ENGINE gbl
			where  a.id=b.prod_id AND b.order_id=c.id AND c.job_no_mst=d.job_no and a.id=e.prod_id and b.id=e.trans_id and c.id=e.po_breakdown_id and gbl.REF_VAL=c.id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1 AND a.status_active=1 AND a.is_deleted=0 AND b.status_active=1 AND b.is_deleted=0 AND b.transaction_type IN (5,6)";
			...
			foreach($item_mismatch_issue_result as $row)
			{
				...
				$item_description=trim($row[csf("description")]);
				$trim_str=", [BS]";
				$item_desc=chop_last_string($item_description,$trim_str);

				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$item_desc][$row[csf("item_color")]][$row[csf("item_size")]]["job_no"]=$row[csf("job_no")];
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$item_desc][$row[csf("item_color")]][$row[csf("item_size")]]["reject_qty"]=$row[csf("reject_qty")];
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$item_desc][$row[csf("item_color")]][$row[csf("item_size")]]["buyer_name"]=$row[csf("buyer_name")];
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$item_desc][$row[csf("item_color")]][$row[csf("item_size")]]["season_buyer_wise"]=$row[csf("season_buyer_wise")];
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$item_desc][$row[csf("item_color")]][$row[csf("item_size")]]["style_ref_no"]=$row[csf("style_ref_no")];
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$item_desc][$row[csf("item_color")]][$row[csf("item_size")]]["po_id"].=$row[csf("po_id")].",";
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$item_desc][$row[csf("item_color")]][$row[csf("item_size")]]["po_number"].=$row[csf("po_number")].",";
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$item_desc][$row[csf("item_color")]][$row[csf("item_size")]]["file_no"].=$row[csf("file_no")].",";
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$item_desc][$row[csf("item_color")]][$row[csf("item_size")]]["int_ref_no"].=$row[csf("int_ref_no")].",";

				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$item_desc][$row[csf("item_color")]][$row[csf("item_size")]]["trim_group"]=$row[csf("trim_group")];
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$item_desc][$row[csf("item_color")]][$row[csf("item_size")]]["uom"]=$row[csf("uom")];
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$item_desc][$row[csf("item_color")]][$row[csf("item_size")]]["store_id"].=$row[csf("store_id")].",";
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$item_desc][$row[csf("item_color")]][$row[csf("item_size")]]["prod_id"].=$row[csf("prod_id")].",";
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$item_desc][$row[csf("item_color")]][$row[csf("item_size")]]["cons_quantity"]+=$row[csf("cons_quantity")];
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$item_desc][$row[csf("item_color")]][$row[csf("item_size")]]["cons_amount"]+=$row[csf("cons_amount")];
				$details_data[$row[csf("job_no")]][1][$row[csf("trim_group")]][$item_desc][$row[csf("item_color")]][$row[csf("item_size")]]["order_ids"].=$row[csf("from_order_id")].','.$row[csf("to_order_id")].',';
			}
			...
			from product_details_master a, order_wise_pro_details b, inv_transaction c,GBL_TEMP_ENGINE gbl where a.id=b.prod_id and b.trans_id=c.id and item_category_id=4  and b.entry_form in(24,25,49,73) and b.trans_type in(1,2,3,4) and c.transaction_type in(1,2,3,4) and gbl.REF_VAL=b.po_breakdown_id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $store_cond";
			// $p=1;
			// if($all_order_id!="")
			// {
			// 	$all_order_id_arr=array_chunk(array_unique(explode(",",$all_order_id)),999);
			...
			from product_details_master a, order_wise_pro_details b, inv_transaction c ,wo_po_break_down d,GBL_TEMP_ENGINE gbl  
			where a.id=b.prod_id and b.trans_id=c.id and d.id=b.po_breakdown_id and d.id=c.order_id and item_category_id=4 and gbl.REF_VAL=b.po_breakdown_id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1  and b.entry_form in(78,112) and b.trans_type in(5,6) and c.transaction_type in(5,6) and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $store_cond";
			// $q=1;
			// if($all_order_id!="")
			// {
			// 	$all_order_id_arr=array_chunk(array_unique(explode(",",$all_order_id)),999);
			...
			foreach($issue_qty_sql_result as $row)
			{
				...
				if($row[csf('entry_form')]==24 && $row[csf('trans_type')]==1)
				{
					...
					$issue_data_arr[$row[csf('po_breakdown_id')]][$row[csf('item_group_id')]][$item_desc][$row[csf('item_color_id')]][$item_size_id]["reject_qty"]=$row[csf('reject_qty')];
				...
				if($row[csf('entry_form')]==25 && $row[csf('trans_type')]==2)
				{
					...
					$issue_data_arr[$row[csf('po_breakdown_id')]][$row[csf('item_group_id')]][$item_desc][$row[csf('item_color_id')]][$item_size_id]["issue_amt"]+=$row[csf('quantity')]*$row[csf('rate')];
				}
				if($row[csf('entry_form')]==49 && $row[csf('trans_type')]==3)
				{
					...
					$issue_data_arr[$row[csf('po_breakdown_id')]][$row[csf('item_group_id')]][$item_desc][$row[csf('item_color_id')]][$item_size_id]["rcv_rtn_amt"]+=$row[csf('quantity')]*$row[csf('rate')];
				}
				if($row[csf('entry_form')]==73 && $row[csf('trans_type')]==4)
				{
					...
					$issue_data_arr[$row[csf('po_breakdown_id')]][$row[csf('item_group_id')]][$item_desc][$row[csf('item_color_id')]][$item_size_id]["issue_rtn_amt"]+=$row[csf('quantity')]*$row[csf('rate')];
				}
				if($store_check[$row[csf('po_breakdown_id')]][$row[csf('item_group_id')]][$item_desc][$row[csf('item_color_id')]][$item_size_id][$row[csf('store_id')]]=="")
				{
					$store_check[$row[csf('po_breakdown_id')]][$row[csf('item_group_id')]][$item_desc][$row[csf('item_color_id')]][$item_size_id][$row[csf('store_id')]]=$row[csf('store_id')];
					$issue_data_arr[$row[csf('po_breakdown_id')]][$row[csf('item_group_id')]][$item_desc][$row[csf('item_color_id')]][$item_size_id]["store_id"].=$row[csf('store_id')].",";
				}
			}
			...
			foreach($trans_qty_sql_result as $row)
			{
				...
				if(($row[csf('entry_form')]==78 || $row[csf('entry_form')]==112) && $row[csf('trans_type')]==5)
				{
					//if ($transfer_criteria_arr[$row[csf('po_breakdown_id')]][$row[csf('item_group_id')]][$item_desc][$row[csf('item_color_id')]][$item_size_id]["transfer_criteria"] != 2)
						//{
							$transfer_data_arr[$row[csf('job_no')]][$row[csf('item_group_id')]][$item_desc][$row[csf('item_color_id')]][$item_size_id]["transfer_in_quantity"]+=$row[csf('quantity')];
							$transfer_data_arr[$row[csf('job_no')]][$row[csf('item_group_id')]][$item_desc][$row[csf('item_color_id')]][$item_size_id]["transfer_in_amt"]+=$row[csf('quantity')]*$row[csf('rate')];					
						//}
				...
				if(($row[csf('entry_form')]==78 || $row[csf('entry_form')]==112) && $row[csf('trans_type')]==6)
				{
					//if ($transfer_criteria_arr[$row[csf('po_breakdown_id')]][$row[csf('item_group_id')]][$item_desc][$row[csf('item_color_id')]][$item_size_id]["transfer_criteria"] != 2)
						//{
							$transfer_data_arr[$row[csf('job_no')]][$row[csf('item_group_id')]][$item_desc][$row[csf('item_color_id')]][$item_size_id]["transfer_out_quantity"]+=$row[csf('quantity')];
							$transfer_data_arr[$row[csf('job_no')]][$row[csf('item_group_id')]][$item_desc][$row[csf('item_color_id')]][$item_size_id]["transfer_out_amt"]+=$row[csf('quantity')]*$row[csf('rate')];					
						//}
					/*if (in_array($row[csf('po_breakdown_id')], $chk_all_order_id)){
				...
				if($store_check[$row[csf('po_breakdown_id')]][$row[csf('item_group_id')]][$item_desc][$item_size_id][$row[csf('store_id')]]=="")
				{
					$store_check[$row[csf('po_breakdown_id')]][$row[csf('item_group_id')]][$item_desc][$item_size_id][$row[csf('store_id')]]=$row[csf('store_id')];
					$transfer_data_arr[$row[csf('job_no')]][$row[csf('item_group_id')]][$item_desc][$item_size_id]["store_id"].=$row[csf('store_id')].",";
				}
			}
			...
			foreach ($details_data as $job_no=>$job_data)
			{
				...
				foreach($job_data[1] as $item_group_id=>$item_group_data)
				{
					foreach($item_group_data as $item_desc=>$item_desc_data)
					{
						...
						{
							if($item_color_id=='') $item_color_id=0;
							// echo "<pre>";print_r($item_color_data);die;
							foreach($item_color_data as $item_size=>$item_size_data)
							{
								$item_ref =implode(',',array_unique(explode(",",chop($item_size_data['item_ref'],","))));
								if($item_size=='') $item_size=0;
								//echo $po_quantity;
								$all_po_id_arr=array_unique(explode(",",chop($item_size_data["po_id"],",")));
								$po_id_all=implode(",",$all_po_id_arr);
								$recv_qnty=$recv_value=$issue_qty=$issue_amount=$rcv_rtn_qty=$issue_rtn_qty=$transfer_in_qty=$transfer_out_qty=$net_recv_qnty=$net_recv_value=$net_issue_qnty=$net_issue_value=$wo_qty=$wo_amount=$req_qnty=$net_recv_value=$net_issue_value=$recv_amt=$issue_amt=$rcv_rtn_amt=$issue_rtn_amt=$transfer_in_amt=$transfer_out_amt=0;$all_store="";$reject_qty="";
								foreach($all_po_id_arr as $po_id)
								{
									/*$recv_qnty+=$dataArrayRecv[$po_id][$item_group_id][$item_desc][$item_color_id][$item_size]["quantity"];
									$recv_value+=$dataArrayRecv[$po_id][$item_group_id][$item_desc][$item_color_id][$item_size]["ord_amount"];*/
									$recv_qnty+=$issue_data_arr[$po_id][$item_group_id][trim($item_desc)][$item_color_id][$item_size]["rcv_quantity"];
									$reject_qty=$issue_data_arr[$po_id][$item_group_id][trim($item_desc)][$item_color_id][$item_size]["reject_qty"];
									$recv_amt+=$issue_data_arr[$po_id][$item_group_id][trim($item_desc)][$item_color_id][$item_size]["rcv_amt"];
									...
									//echo $po_id."==".$item_group_id."==".trim($item_desc)."==".$item_color_id."==".$item_size."++";
								}
								...
								<tr bgcolor="<? echo $bgcolor; ?>"  onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
									...
									if($job_check[$job_no][1]=="")
									{
										...
										*/
										?>
										<td width="110" rowspan="<? echo $job_row_count; ?>" title="Trims Accessories"><p><? echo $item_size_data[('style_ref_no')]; ?></p></td>
										<td width="140" rowspan="<? echo $job_row_count; ?>"><p><? echo implode(",",array_unique(explode(",",chop($item_size_data["po_number"],",")))); ?></p></td>
										<td width="70" rowspan="<? echo $job_row_count; ?>"><p><? echo implode(",",array_unique(explode(",",chop($item_size_data["file_no"],",")))); ?></p></td>
										<td width="80" rowspan="<? echo $job_row_count; ?>"><p><? echo implode(",",array_unique(explode(",",chop($item_size_data["int_ref_no"],",")))); ?></p></td>
										<td width="90" align="right" rowspan="<? echo $job_row_count; ?>"><? echo  $po_quantity[$item_size_data['job_no']]; ?></td>
										<td width="80" align="right" rowspan="<? echo $job_row_count; ?>"><? echo number_format($po_quantity[$item_size_data['job_no']]/12,2); ?></td>
									...
									?>
									<td width="100" title="<? echo $item_color_id; ?>"><p><? echo $color_library[$item_color_id]; ?></p></td>
									<td width="60"><p><? if($item_ref=="0") echo "&nbsp;"; else echo $item_ref; ?></p></td>
									<td width="60"><p><? if($item_size=="0") echo "&nbsp;"; else echo $item_size; ?></p></td>
									<td width="80" align="right" title="<? echo "po id==". $po_id_all."item group id==".$item_group_id."item description==".trim($item_desc)."item color id==".$item_color_id."item size id==".$item_size; ?>"><a href='#report_details' onClick="openmypage('<? echo $po_id_all; ?>','<? echo $item_group_id; ?>','<? echo $item_color_id; ?>','<? echo $gmts_color; ?>','<? echo $item_size; ?>','<? echo trim($item_desc); ?>','<? echo $recv_basis; ?>','<? echo $without_order; ?>',1,'wo_receive_popup');"><?  echo number_format($wo_qty,2); ?></a></td>
									<td width="80" align="right" title="<? echo "po id=". $po_id." item group id=".$item_group_id." item desc=".trim($item_desc)." item color id=".$item_color_id." item size id=".$item_size; ?>"><?  echo number_format($wo_amount,2); ?></td>
									<!-- $recv_qnty.'=='.$transfer_in_qty.'=='.$rcv_rtn_qty.'=='. -->
									...
									<td width="90" align="right"><? echo number_format($reject_qty,2); ?></td>
									...
									<td width="80" align="right"><? echo number_format($wo_qty-$net_recv_qnty,2); ?></td>
									<td width="100" align="right"><? echo number_format($net_recv_value,2); ?></td>
									<td width="90" align="right"> <a href='#report_details' onClick="openmypage_issue('<? echo $po_id_all; ?>','<? echo $item_group_id; ?>','<? echo $item_color_id; ?>','<? echo $gmts_color; ?>','<? echo $item_size; ?>','<? echo trim($item_desc); ?>','<? echo $recv_basis; ?>','<? echo $without_order; ?>',3,'issue_popup');"><?  echo number_format($iss_qnty,2); ?></a></td>
									...
									<td width="100" align="right"><? $left_over=$net_recv_qnty-$net_issue_qnty;echo number_format($left_over,2); ?></td>
									<td width="60" align="right"><?  echo number_format($ord_avg_rate,2); ?></td>
									<td align="right"><? $tot_left_val=$left_over*$ord_avg_rate; echo number_format($tot_left_val,2);?>&nbsp;&nbsp;</td>
								</tr>
								<?
			...
			<tr class="tbl_bottom">
				<td colspan="8" align="right">Total</td>
				<td align="right"><? echo number_format(array_sum($po_quantity),0); ?></td>
				<td colspan="5" align="right"></td>
				<td align="right"><? //echo number_format($total_required_qnty,2); ?></td>
                <td></td>
                <td></td>
...
// Style Wise Search. Report 3 button
if ($action=="report_generate_style3")
{
	$process = array( &$_POST );
	extract(check_magic_quote_gpc( $process ));
	$cbo_company=str_replace("'","",$cbo_company_id);
    ...
    <fieldset style="width:2560px;">
        ...
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="2540" class="rpt_table" id="tbl_issue_status" >
			...
			from product_details_master a, inv_transaction b, wo_po_break_down c, wo_po_details_master d,GBL_TEMP_ENGINE gbl
			where a.id=b.prod_id and b.order_id=c.id and c.job_no_mst=d.job_no  and gbl.REF_VAL=c.id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form<>24 and b.transaction_type in(2,3,6) and b.item_category in(".implode(",",array_keys($general_item_category)).")";
			// echo $general_item_issue_sql;die;    
			// $p=1;
			// if($all_order_id!="")
			// {
			...
			$issue_qty_sql="SELECT b.po_breakdown_id, a.item_color as item_color_id, a.item_size, a.item_group_id, a.item_description, b.quantity, c.cons_rate as rate, b.entry_form, b.trans_type, c.store_id
			from product_details_master a, order_wise_pro_details b, inv_transaction c,GBL_TEMP_ENGINE gbl
			where a.id=b.prod_id and b.trans_id=c.id and gbl.REF_VAL=b.po_breakdown_id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1 and item_category_id=4 and a.entry_form=24 and b.entry_form in(24,25,49,73,78,112) and b.trans_type in(1,2,3,4,5,6) and c.transaction_type in(1,2,3,4,5,6) and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $store_cond";
			// $p=1;
			// if($all_order_id!="")
			// {
			// 	$all_order_id_arr=array_chunk(array_unique(explode(",",$all_order_id)),999);
			...
			$store_to_store_sql="SELECT b.po_breakdown_id, a.item_color as item_color_id, a.item_size, a.item_group_id, a.item_description, b.quantity, c.cons_rate as rate, b.entry_form, b.trans_type, c.store_id, d.transfer_criteria
			from product_details_master a, order_wise_pro_details b, inv_transaction c, inv_item_transfer_mst d,GBL_TEMP_ENGINE gbl
			where a.id=b.prod_id and b.trans_id=c.id and d.id=c.mst_id and item_category_id=4 and a.entry_form=24 and gbl.REF_VAL=b.po_breakdown_id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1  and b.entry_form in(112) and b.trans_type in(5,6) and c.transaction_type in(5,6) and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $store_cond and d.transfer_criteria=2";
			// $q=1;
			// if($all_order_id!="")
			// {
			// 	$all_order_id_arr=array_chunk(array_unique(explode(",",$all_order_id)),999);
			...
			$sql_booking ="SELECT a.JOB_NO, b.po_break_down_id as po_id, b.trim_group, b.booking_no, a.description, a.cons, a.amount, p.currency_id
			from  wo_booking_mst p, wo_trim_book_con_dtls a, wo_booking_dtls b,GBL_TEMP_ENGINE gbl
			where p.booking_no=a.booking_no and p.booking_no=b.booking_no and a.wo_trim_booking_dtls_id=b.id and b.booking_type=2 and gbl.REF_VAL=b.po_break_down_id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1  and a.cons>0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.JOB_NO";
				...
				foreach($job_data[1] as $item_group_id=>$item_group_data)
				{
					foreach($item_group_data as $item_desc=>$item_desc_data)
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>"  onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
							...
							<td width="80" align="right"><? echo number_format($wo_qty-$net_recv_qnty,2); ?></td>
							<td width="100" align="right"><? echo number_format($net_recv_value,2); ?></td>
							<td width="90" align="right"> <a href='#report_details' onClick="openmypage_issue('<? echo $po_id_all; ?>','<? echo $item_group_id; ?>','<? echo $item_color_id; ?>','<? echo $gmts_color; ?>','<? echo $item_size; ?>','<? echo $item_desc; ?>','<? echo $recv_basis; ?>','<? echo $without_order; ?>',4,'issue_popup');"><?  echo number_format($net_issue_qnty,2); ?></a></td>
							<td width="80" align="right"><?  echo number_format($net_issue_value,2); ?></td>
							<td width="100" align="right"><? $left_over=$net_recv_qnty-$net_issue_qnty;echo number_format($left_over,2); ?></td>
							<td width="60" align="right"><?  echo number_format($ord_avg_rate,2); ?></td>
							<td align="right"><? $tot_left_val=$net_recv_value-$net_issue_value; echo number_format($tot_left_val,2);?></td>
...
// Item Color Size button
if ($action=="report_generate_style_color_size")
{
	$process = array( &$_POST );
	extract(check_magic_quote_gpc( $process ));
	$cbo_company=str_replace("'","",$cbo_company_id);
	...
	<fieldset style="width:1790px;">
        ...
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1770" class="rpt_table" id="tbl_issue_status" >
		   <?
				...
				$gblTempPoArr[$row[csf('id')]] = $row[csf('id')];
				}  
				...
				$sql_book=sql_select("select b.trim_group as item_group_id, b.uom as order_uom,b.sensitivity, c.color_number_id as gmts_color,d.color_number_id as color_number_id, d.item_color as item_color  ,b.rate as cons_rate, d.item_size as item_size,c.size_number_id as gmts_size,  c.po_break_down_id as po_id, d.requirment as quantity from wo_booking_dtls b,wo_po_color_size_breakdown c,wo_trim_book_con_dtls d,GBL_TEMP_ENGINE gbl where   b.job_no=c.job_no_mst and b.id=d.wo_trim_booking_dtls_id and b.booking_no=d.booking_no and d.job_no=b.job_no and b.po_break_down_id=c.po_break_down_id  and  d.po_break_down_id=c.po_break_down_id and c.job_no_mst=d.job_no and gbl.REF_VAL=c.po_break_down_id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1 and b.booking_type=2  and c.status_active=1 and c.is_deleted=0   and d.color_size_table_id=c.id and d.requirment!=0   and b.status_active=1 and b.is_deleted=0 $poIds_cond order by b.trim_group ,c.color_number_id,c.size_number_id");
				...
				from wo_pre_cost_trim_cost_dtls a, wo_pre_cost_trim_co_cons_dtls b, wo_po_color_size_breakdown c,GBL_TEMP_ENGINE gbl
				where a.job_no=b.job_no and a.job_no=c.job_no_mst and a.id=b.wo_pre_cost_trim_cost_dtls_id   and b.po_break_down_id=c.po_break_down_id and gbl.REF_VAL=c.po_break_down_id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1 and c.job_no_mst=b.job_no and  c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 $poIds_cond");
			...
			/*$sql_data=sql_select("select a.trim_group as item_group_id, a.cons_uom as order_uom, b.color_number_id as gmts_color,a.rate as cons_rate, b.item_size as item_size,b.size_number_id as gmts_size,  b.po_break_down_id as po_id, b.cons as quantity
				...
				$sql_result=sql_select("select a.item_group_id, a.order_uom,a.sensitivity, a.item_color,a.gmts_color_id, a.item_size,a.gmts_size_id, a.cons_rate, b.po_breakdown_id as po_id, b.quantity,c.receive_basis from inv_trims_entry_dtls a, order_wise_pro_details b,inv_receive_master c,GBL_TEMP_ENGINE gbl where  c.id=a.mst_id and a.id=b.dtls_id and a.trans_id=b.trans_id and gbl.REF_VAL=b.po_breakdown_id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1 and b.trans_type=1 and b.entry_form=24  and c.entry_form=24  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $poIds_conds order by a.item_group_id,a.gmts_color_id,a.gmts_size_id");
				...
				else
				{
					...
					$sql="select a.style_ref_no, a.buyer_name, a.job_no,a.gmts_item_id, sum(c.order_quantity*a.total_set_qnty) as po_quantity, sum(c.order_quantity/a.total_set_qnty) as order_quantity_set from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c,GBL_TEMP_ENGINE gbl  where a.id=b.job_id and a.id=c.job_id and b.job_id=c.job_id and c.po_break_down_id=b.id and gbl.REF_VAL=c.po_break_down_id and gbl.ENTRY_FORM=256 and gbl.USER_ID=$user_id and gbl.ref_from=1 and a.company_name=$cbo_company $buyer_id_cond $style $po_id_cond $pub_date $poIds_cond group by a.id, a.job_no, a.style_ref_no, a.buyer_name,a.gmts_item_id order by a.id";
				}
				//echo $sql;
				$result=sql_select($sql);
			...
			<table border="1" class="rpt_table" rules="all" width="730" cellpadding="0" cellspacing="0" align="center">
                ...
                <?
				   ...
				   /*$mrr_sql="select a.id, a.recv_number, a.challan_no, a.receive_date, c.quantity as quantity, b.reject_receive_qnty as reject_qty, b.item_description, c.prod_id, c.trans_type
					...
					union all
					select a.id, a.issue_number, a.challan_no, a.issue_date, c.quantity as quantity, c.reject_qty as reject_qty, d.item_description, c.prod_id, c.trans_type 
		...
		<div id="scroll_body" align="center">
			<table border="1" class="rpt_table" rules="all" width="910" cellpadding="0" cellspacing="0" align="left" >
                ...
                <?
					...
					if ($type==3)
					{
						$mrr_sql="select a.id, a.issue_number, a.challan_no, c.prod_id, a.issue_date, d.item_description, c.quantity as quantity, b.sewing_line,b.floor_id as floor_sewing, c.trans_type,c.po_breakdown_id
                ...
                <?
					...
					foreach($dtlsArray as $row)
					{
						if ($i%2==0)
							...
							$bgcolor="#FFFFFF";
						if($row[csf('quantity')]>0 && ($row[csf('trans_type')]==2))
						{
							$trans_type="Issue";							
							?>
							<tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
		...
		<div id="scroll_body" align="center">
			<table border="1" class="rpt_table" rules="all" width="910" cellpadding="0" cellspacing="0" align="left">
                ...
                <?
					...
					$item_color=str_replace("'","",$item_color);
					$gmts_color=str_replace("'","",$gmts_color);
					$item_size=str_replace("'","",$item_size);
					$item_desc=str_replace("'","",$item_desc);
					$recv_basis=str_replace("'","",$recv_basis);
					$without_order=str_replace("'","",$without_order);
					if($item_color) $sql_cond.=" and a.ITEM_COLOR =$item_color";
					if($gmts_color) $sql_cond.=" and a.COLOR =$gmts_color";
					if($item_size) $sql_cond.=" and a.ITEM_SIZE ='$item_size'";
					if($item_desc) $sql_cond.=" and a.ITEM_DESCRIPTION ='$item_desc'";
					
					$mrr_sql="select a.item_description, a.id as prod_id, b.floor_id, b.line_id, b.id as trans_id, e.id, e.issue_number, e.challan_no, e.issue_date, b.transaction_type as trans_type, g.QUANTITY AS quantity, g.PO_BREAKDOWN_ID as order_id
					from product_details_master a, inv_transaction b, ORDER_WISE_GENERAL_DETAILS g, wo_po_break_down c, wo_po_details_master d, inv_issue_master e
					where a.id=b.prod_id and b.id=g.trans_id and g.PO_BREAKDOWN_ID=c.id and c.job_no_mst=d.job_no and e.id = b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form<>24 and b.transaction_type in(2,3,6) and g.PO_BREAKDOWN_ID IN ($po_id) and a.company_id=$companyID and a.item_group_id in($item_group) $sql_cond
					order by trans_id, order_id";
...
if($action=="transfer_inpopup")
{
	echo load_html_head_contents("Receive Info", "../../../../", 1, 1,'','','');
	extract($_REQUEST);

	?>
	<fieldset style="width:660px; margin-left:3px">
		<div id="scroll_body" align="center">
			<table border="1" class="rpt_table" rules="all" width="660" cellpadding="0" cellspacing="0" align="center">
				<caption><strong>Transfer IN Details</strong></caption>
                <thead>
                    <th width="30">Sl</th>
                ...
                <?
					...
					$store_id_cond="";
					if ($store_id > 0) $store_id_cond=" and a.to_store_id=$store_id";	
					...
					foreach($dtlsArray as $row)
					{
						if ($i%2==0)
							...
							$bgcolor="#FFFFFF";
						if($row[csf('quantity')]>0 && $row[csf('trans_type')]==5)
						{
							?>
							<tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
...
if($action=="transfer_outpopup")
{
	...
	<fieldset style="width:660px; margin-left:3px">
		<div id="scroll_body" align="center">
			<table border="1" class="rpt_table" rules="all" width="660" cellpadding="0" cellspacing="0" align="center">
				<caption><strong>Transfer Out Details</strong></caption>
                <thead>
                    <th width="30">Sl</th>
                    <th width="60">Prod. ID</th>
                    <th width="120">Transfer ID</th>
                ...
                <?
					...
					$store_id_cond="";
					if ($store_id > 0) $store_id_cond=" and a.from_store_id=$store_id";
					//echo $item_color_con.'=='.$item_size_con;
				 	$mrr_sql="select a.id, a.transfer_system_id as issue_number, a.challan_no, c.prod_id, a.transfer_date as issue_date, d.product_name_details, c.quantity as quantity, 0 as sewing_line, 0 as floor_sewing, c.trans_type, c.po_breakdown_id
					from inv_item_transfer_mst a, inv_item_transfer_dtls b, order_wise_pro_details c, product_details_master d
					where a.id=b.mst_id and b.id=c.dtls_id and c.prod_id=d.id and a.entry_form in(78,112) and c.trans_type in(6) and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and c.po_breakdown_id in($po_id) and a.company_id='$companyID' and d.item_group_id='$item_group' and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 $item_color_con2 $item_size_con2 $store_id_cond"; 
					...
					foreach($dtlsArray as $row)
					{
						if ($i%2==0)
							...
							$bgcolor="#FFFFFF";
						if($row[csf('quantity')]>0 && $row[csf('trans_type')]==6)
						{
							?>
							<tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
