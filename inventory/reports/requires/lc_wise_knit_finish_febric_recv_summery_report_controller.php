...
if($action=="report_generate") // Show
{
    ...
    <!--<fieldset style="width:1120px">-->
        ...
        <!-- Knit Finish Febric Receive Start -->
        <br>
        ...
        <!-- Knit Finish Febric Receive End -->

        <!-- Knit Finish Febric Receive Return Start -->
        <br>
        <table width="1100" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table">
            <thead>
                <tr>
        ...
        <!-- Knit Finish Febric Receive Return End -->
...
if($action=="report_generate2") // Show2
{
    $process = array( &$_POST );
    extract(check_magic_quote_gpc( $process ));
    ...
    <!--<fieldset style="width:1120px">-->
        ...
        <div id="report5" style="width:1620px; overflow-y:scroll; max-height:300px;"  align="left">
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1600" class="rpt_table" id="tbl_finish_feb_rcv" >
                ...
                /*if ($pi_id_all!='' || $pi_id_all!=0)
                {
                    ...
                    $sql_recv="SELECT b.id as trans_id, a.id as rec_id, a.recv_number, a.recv_number_prefix_num, a.receive_date, a.challan_no, a.booking_id as pi_id, a.booking_no as pi_number, a.knitting_company, e.fabric_description_id, b.remarks, b.prod_id, b.pi_wo_batch_no as batch_id, c.color_id, f.construction, f.copmposition, c.quantity, b.order_rate, b.order_amount, c.po_breakdown_id, c.id as propo_id
                    from inv_receive_master a, pro_finish_fabric_rcv_dtls e, inv_transaction b, order_wise_pro_details c, wo_booking_dtls f
                    where a.id = e.mst_id and e.TRANS_ID=b.id and a.id = b.mst_id and b.id=c.trans_id and c.po_breakdown_id=f.po_break_down_id and a.item_category = 2 and a.entry_form=37 and a.company_id = $cbo_company_name and a.receive_basis = 1 and b.item_category = 2 and b.transaction_type = 1 and a.booking_id in ($pi_id_all) and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0";
                }*/
                //N.B: This report only for WO basis recv
                if($pi_workd_order_id_all!="")
                {
                    if($sql_recv!="") $sql_recv.=" union all ";                      
                    $sql_recv=" SELECT b.id as trans_id, a.id as rec_id, a.recv_number, a.recv_number_prefix_num, a.receive_date, a.challan_no, a.booking_id as pi_id, a.booking_no as pi_number, a.knitting_company, e.fabric_description_id, b.remarks, b.prod_id, b.pi_wo_batch_no as batch_id, c.color_id, f.construction, f.copmposition, c.quantity, b.order_rate, b.order_amount, c.po_breakdown_id, c.id as propo_id
                    from inv_receive_master a, pro_finish_fabric_rcv_dtls e, inv_transaction b, order_wise_pro_details c, wo_booking_dtls f
                    where a.id = e.mst_id and e.TRANS_ID=b.id and a.id = b.mst_id and b.id=c.trans_id and c.po_breakdown_id=f.po_break_down_id and a.item_category = 2 and a.entry_form=37 and a.company_id = $cbo_company_name and a.receive_basis = 2 and b.item_category = 2 and b.transaction_type = 1 and a.booking_id in ($pi_workd_order_id_all) and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0";
                }// and a.booking_no='OG-FB-23-00011'  and c.color_id=2
                //and a.recv_number='OG-FFGE-23-00025' and a.booking_no='OG-FB-23-00011' and c.po_breakdown_id in(71350,71351)  --and a.booking_id=38470 and c.color_id=1 and e.fabric_description_id=1742
                //echo $sql_recv;//die;
                $pi_rec_data_arr= array();
                $dataArray=sql_select($sql_recv);
                foreach ($dataArray as $k => $value) 
                {
                    if($rcv_check[$value[csf('rec_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]=="")
                    {
                        $rcv_check[$value[csf('rec_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]=$value[csf('rec_id')];
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['rec_id'].=$value[csf('rec_id')].",";
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['recv_number'].=$value[csf('recv_number_prefix_num')].",";
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['challan_no'].=$value[csf('challan_no')].",";
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['supplier'].=$supplier_arr[$value[csf('knitting_company')]].',';
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['fabric_desc_id'].=$value[csf('fabric_description_id')].',';
                    }
                    if($batch_check[$value[csf('batch_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]=="")
                    {
                        $batch_check[$value[csf('batch_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]=$value[csf('batch_id')];
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['batch_id'].=$value[csf('batch_id')].",";
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['batch_no'].=$batch_arr[$value[csf('batch_id')]].",";
                    }
                    
                    $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['receive_date']=$value[csf('receive_date')];
                    $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['pi_id']=$value[csf('pi_id')];
                    $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['pi_number']=$value[csf('pi_number')];
                    $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['color_id']=$value[csf('color_id')];
                    
                    
                    //$pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['po_number']=$po_data[$value[csf("po_breakdown_id")]]["po_number"];
                    $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['job_no']=$po_data[$value[csf("po_breakdown_id")]]["job_no"];
                    $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['buyer_name']=$po_data[$value[csf("po_breakdown_id")]]["buyer_name"];
                    $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['style_ref_no']=$po_data[$value[csf("po_breakdown_id")]]["style_ref_no"];
                    $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['construction']=$value[csf('construction')];
                    $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['copmposition']=$value[csf('copmposition')];
                    $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['po_id'].=$value[csf('po_breakdown_id')].',';
                    
                    //echo "sumon";
                    if($propotion_check[$value[csf('propo_id')]]=="")
                    {
                        $propotion_check[$value[csf('propo_id')]]=$value[csf('propo_id')];
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['quantity']+=$value[csf('quantity')];
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['order_amount']+=$value[csf('quantity')]*$value[csf('order_rate')];
                    }
                    $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['order_rate'][$value[csf('trans_id')]][$k]=$value[csf('order_rate')];
                    $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('color_id')]][$value[csf('fabric_description_id')]]['remarks']=$value[csf('remarks')];

                    $pi_rcv_rtn_rate_arr[$value[csf('rec_id')]][$value[csf('prod_id')]]['rate']=$value[csf('order_rate')];
                }
                // echo "<pre>";print_r($pi_rec_data_arr);

                $i=1;
                foreach($pi_rec_data_arr as $pi_id=> $row_recv)
                {
                    foreach ($row_recv as  $color_id=>$color_a) 
                    {
                        foreach ($color_a as  $fabric_detr_id=>$value) 
                        {
                            if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";

                            $recv_po_id_arr=array_unique(explode(",", chop($value['po_id'],",")));
                            $po_numbers ="";
                            foreach ($recv_po_id_arr as $key => $order_id) 
                            {
                                $po_numbers .= ($po_numbers =="") ? $po_data[$order_id]["po_number"] :  ",". $po_data[$order_id]["po_number"];
                            }
                            $po_numbers =implode(",",array_filter(array_unique(explode(",", $po_numbers))));
                            $supplier =implode(",",array_filter(array_unique(explode(",", $value['supplier']))));
                            $recv_number =implode(",",array_filter(array_unique(explode(",", $value['recv_number']))));

                            $fabric_desc_id_arr=array_unique(explode(",", chop($value['fabric_desc_id'],",")));
                            $item_description ="";
                            foreach ($fabric_desc_id_arr as $key => $id) 
                            {
                                $item_description .= ($item_description =="") ? $composition_arr[$id] :  ",". $composition_arr[$id];
                            }
                            //$item_description =implode(",",array_filter(array_unique(explode(",", $item_description))));
                            ...
                            <tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('trp_<? echo $i;?>','<? echo $bgcolor;?>')" id="trp_<? echo $i;?>">
                                ...
                                <td width="140" align="center"><? echo $buyer_arr[$value['buyer_name']]; ?></td>
                                <td width="120" align="center"><? echo $value['style_ref_no']; ?></td>
                                <td width="100" align="center"><p><? echo $po_numbers;//$value['po_number']; ?></p></td>
                                <td width="90" align="center" title="<?=$color_id;?>"><? echo $color_arr[$value['color_id']]; ?></td>
                                <td width="140" align="center" title="<?= $supplier_pi_arr[$value['pi_id']]['supplier']; ?>"><? echo $supplier; ?></td>
                                <td width="90" title="<?=$pi_id;?>" align="center"><? echo $value['pi_number']; ?></td>
                                <td width="100"><p><? echo chop($recv_number,','); ?></p></td>
                                <td width="90" align="center"><? echo change_date_format($value['receive_date']); ?></td>
                                <td width="80"><p>&nbsp;<? echo chop($value['challan_no'],','); ?></p></td>
                                <td width="120" title="<?=$fabric_detr_id;?>"><p>&nbsp;<? echo $item_description; ?></p></td>
                                <td width="70" align="center" title="<?= chop($value['batch_id'],','); ?>"><p><? echo chop($value['batch_no'],','); ?></p></td>
                                <td width="80" align="right"><p><? echo $value['quantity']; ?></p></td>
                                <td width="70" align="right"><? echo $avg_rage= number_format($amount/$value['quantity'],2,'.','');//$rate; ?></td>
                                <td width="100" align="right"><? echo number_format($amount,2,'.',''); ?></td>
                                <td><p>&nbsp;<? echo $value['remarks']; ?></p></td>
                            </tr>
                            <?
        ...
        <table width="1100" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table">
            <thead>
                <tr>
                  <th colspan="9">Knit Finish Febric Receive Return</th>
                </tr>
                <tr>
                    <th width="40">SL</th>
                    <th width="100">MRR No.</th>
        ...
        <div id="report3" style="width:1120px; overflow-y:scroll; max-height:430px;"  align="left">
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1100" class="rpt_table" id="tbl_finish_feb_rcv_rtrn" >
                ...
                if(count($recv_id_array)>0)
                {
                    ...
                    $sql_retn="SELECT a.id, a.received_id,a.received_mrr_no, a.issue_number, a.issue_date, a.challan_no,b.remarks,b.prod_id,b.order_rate as rate, b.rcv_rate, b.cons_quantity, b.cons_amount, c.product_name_details,d.receive_date, d.booking_id as pi_id
                    from inv_issue_master a, inv_transaction b, product_details_master c,inv_receive_master d
                    where a.id=b.mst_id and a.received_id=d.id and b.prod_id=c.id and a.item_category=2 and a.entry_form=46 and a.company_id=$cbo_company_name and a.received_id in($recv_id_all) and  b.item_category=2 and b.transaction_type=3 and  a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 ";
                    //echo $sql_retn;
                    $dataRtArray=sql_select($sql_retn);
                    ...
                    foreach($dataRtArray as $row_retn)
                    {
                        ...
                        <tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('trrc_<? echo $i;?>','<? echo $bgcolor;?>')" id="trrc_<? echo $i;?>">
                            <td width="40" align="center"><? echo $i; ?></td>
                            <td width="100" align="center"><? echo $row_retn[csf('received_mrr_no')]; ?></td>
                            <td width="100" align="center"><? echo change_date_format($row_retn[csf('issue_date')]); ?></td>
                            <td width="130"><p><? echo $row_retn[csf('issue_number')]; ?></p></td>
                            <td width="290"><p>&nbsp;<? echo $row_retn[csf('product_name_details')]; ?></p></td>
                            <td width="110" align="right"><? echo number_format($row_retn[csf('cons_quantity')],2,'.',''); ?></td>
        ...
        <div id="report4" style="width:870px; overflow-y:scroll; max-height:430px;"  align="left">
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="850" class="rpt_table" id="tbl_finish_feb_acceptance" >
                ...
                if ($pi_id_all!='' || $pi_id_all!=0)
                {
                    ...
                    where  m.id in($pi_id_all)
                    order by m.id";*/
                    $acceptance_sql = "SELECT m.id as pi_id, b.current_acceptance_value as acceptance_value, n.work_order_id, n.work_order_no, a.bank_acc_date
                    from com_import_invoice_mst a, com_import_invoice_dtls b, com_pi_master_details m, COM_PI_ITEM_DETAILS n
                    where a.id=b.import_invoice_id and b.pi_id=m.id and m.id=n.pi_id and b.is_lc=1 and b.status_active=1 and b.is_deleted=0 and a.BANK_REF is not null and m.id in($pi_id_all) order by m.id";
                    //echo $acceptance_sql;
                    $accep_result=sql_select($acceptance_sql);
                    //echo "<pre>";print_r($pi_data_array);
                    $acceptance_data_arr=array();
                    foreach ($accep_result as $key => $row) 
                    {
                        if($temp_pi_check[$row[csf("pi_id")]]=="")
                        {
                            $temp_pi_check[$row[csf("pi_id")]]=$row[csf("pi_id")];
                            $acceptance_data_arr[$row[csf("work_order_id")]]['acceptance_value']+=$row[csf("acceptance_value")];
                            $acceptance_data_arr[$row[csf("work_order_id")]]['bank_acc_date'].=$row[csf("bank_acc_date")].',';
                            $acceptance_data_arr[$row[csf("work_order_id")]]['pi_id'].=$row[csf("pi_id")].',';
                        }
                        $acceptance_data_arr[$row[csf("work_order_id")]]['work_order_no']=$row[csf("work_order_no")];
                    }
                    //echo "<pre>";print_r($acceptance_data_arr);
                    $payble_value=0; $tot_payble_value=0; $yet_to_accept=0; $tot_accept_value=0; $tot_yet_to_accept=0; $total_receive_value=0; $total_return_value=0;

                    foreach($acceptance_data_arr as $wo_id => $row)
                    {
                        if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";

                        $payble_value=$yet_to_accept=0;
                        $payble_value=$pi_data_array[$wo_id]['rcv_value']-$pi_data_array[$wo_id]['rtn'];
                        $yet_to_accept=$payble_value-$row["acceptance_value"];

                        //$tot_accept_value+=$acceptance_arr[$row[csf("pi_id")]];
                        $bank_acc_date_arr=array_unique(explode(",", chop($row["bank_acc_date"],",")));
                        $bank_acc_date ="";
                        foreach ($bank_acc_date_arr as $key => $date) 
                        {
                            $bank_acc_date .= ($bank_acc_date =="") ? change_date_format($date) :  ",". change_date_format($date);
                        }
                        $bank_acc_date =implode(",",array_filter(array_unique(explode(",", $bank_acc_date))));

                        $pi_id_arr=array_unique(explode(",", chop($row["pi_id"],",")));
                        $pi_nos ="";
                        foreach ($pi_id_arr as $key => $piId) 
                        {
                            $pi_nos .= ($pi_nos =="") ? $pi_name_array[$piId] :  ",". $pi_name_array[$piId];
                        }
                        $pi_nos =implode(",",array_filter(array_unique(explode(",", $pi_nos))));

                        ?>
                        <tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
                            <td width="100" align="center"><p><? echo $row["work_order_no"]; ?></p></td>
                            <td width="140" align="center"><p><? echo $pi_nos; ?></p></td>
                            <td width="100"  align="right"><? echo number_format($pi_data_array[$wo_id]['rcv_value'],2,'.',''); ?></td>
                            <td width="100" align="right"><? echo number_format($pi_data_array[$wo_id]['rtn'],2,'.',''); ?></td>

                            <td width="100" align="center"><p><? echo $bank_acc_date;?></p></td>
                            <td width="100" align="right"><? echo number_format($row["acceptance_value"],2,'.',''); ?></td>  
                            <td align="right"><? echo number_format($yet_to_accept,2,'.',''); ?></td>
                        </tr>
                        <?
                        $i++;
                        $total_receive_value+=$pi_data_array[$wo_id]['rcv_value'];
                        $total_return_value+=$pi_data_array[$wo_id]['rtn'];
                        $tot_accept_value+=$row["acceptance_value"];
                        $tot_yet_to_accept+=$yet_to_accept;
                    }
                }
                ?>
