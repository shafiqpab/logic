...
if($action=="create_pi_search_list_view")
{
	...
	$sql= "select id, pi_number, supplier_id, importer_id, pi_date, last_shipment_date, total_amount from com_pi_master_details where importer_id=$company and entry_form=166 and item_category_id=3 and supplier_id like '$cbo_supplier' and pi_number like '%".$txt_search_common."%' and is_deleted=0 and status_active=1 $pi_date_cond";
    //echo $sql;
                    ...
                    <tbody>
                        <tr align="center">
                            ...
                            <td align="center" id="search_by_td">
                             ...
                             <td align="center">
                                <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_supplier_id').value+'_'+document.getElementById('txt_search_common').value+'_'+<? echo $companyID; ?>, 'create_lc_search_list_view', 'search_div', 'lc_wise_woven_febric_recv_summery_report_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />
                            </td>
                   	 	</tr>
                    </tbody>
                </table>
...
if($action=="create_lc_search_list_view")
{
	...
	/*$sql= "select id, lc_number, supplier_id, importer_id, lc_date, last_shipment_date, lc_value 
	from com_btb_lc_master_details a, where importer_id=$company and pi_entry_form=166 and supplier_id like '$cbo_supplier' and lc_number like '%".$txt_search_common."%' and is_deleted=0 and status_active=1";*/
	...
	where a.id=b.com_btb_lc_master_details_id and c.id=b.pi_id and c.item_category_id = 3 and a.pi_entry_form=166 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and c.importer_id=$company and a.supplier_id like '$cbo_supplier' and a.lc_number like '%".$txt_search_common."%'
...
if($action=="report_generate")
{
    ...
    $con = connect();
    $r_id=execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_id and entry_form in (202)");
    oci_commit($con);
    ...
    <!--<fieldset style="width:1120px">-->
    	<div style="width:100%; margin-left:10px;" align="left">
            ...
            <table width="1100" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table">
                <?
                $sql_btb_lc = "select a.id,a.lc_number, a.supplier_id, a.importer_id, a.lc_date, a.last_shipment_date, a.lc_value,c.id as pi_id, c.pi_number,c.pi_basis_id , d.work_order_id, d.net_pi_amount AS pi_value
				from com_btb_lc_master_details a, com_btb_lc_pi b, com_pi_master_details c, com_pi_item_details d
				where a.id=b.com_btb_lc_master_details_id and c.id=b.pi_id and c.id=d.pi_id and c.item_category_id = 3  and a.pi_entry_form=166 and c.importer_id=$cbo_company_name and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1 $btbLc_id_cond_mst $pi_cond_btb";
                //echo $sql_btb_lc;//die;
                $btbLcResult=sql_select($sql_btb_lc);
				if(count($btbLcResult)<1) {echo "No Fata Found;";die;}
				$btbLcData=array();$pi_id_all_array=array(); $pi_name_array=array();$pi_data_array=array();
				foreach($btbLcResult as $row)
				{
                    ...
                    if($row[csf('pi_basis_id')]==1)
                    {
                        ...
                        //$pi_data_array[$row[csf('pi_id')]]['rcv_value']+=$row[csf('pi_value')];
                    }
				}
            ...
            <div style="width:1120px; overflow-y:scroll; max-height:430px;"  align="left">
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1100" class="rpt_table" id="tbl_woven_finish_feb_btb_lc_dtls" >
                	<?
            ...
            <div style="width:1240px; overflow-y:scroll; max-height:430px;"  align="left" id="scroll_body_tbl_woven_finish_feb_pi_dtls">
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1220" class="rpt_table" id="tbl_woven_finish_feb_pi_dtls" >
                    <?
                    $i=1;
					...
					$sql_pi_details="SELECT c.id as pi_dtls_id, b.id,b.pi_number, b.pi_date, b.currency_id,b.remarks, b.item_category_id, b.supplier_id, c.color_id, c.item_group, c.work_order_dtls_id, c.fabric_construction,c.fabric_composition, c.quantity, c.rate, c.net_pi_amount, c.work_order_no as booking_no,c.work_order_id as booking_id
  					from com_pi_master_details b, com_pi_item_details c
   					where b.id = c.pi_id  and b.entry_form = 166 and b.importer_id = $cbo_company_name and b.item_category_id = 3 and b.status_active = 1 and b.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0 and b.id in(".implode(",",$pi_id_all_array).")";
                    ...
                    foreach($pi_dtls_data as $pi_ids=>$row_color_id)
                    {
                        foreach ($row_color_id as  $row)
                        {
                            ...
                            <tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('trr_<? echo $p;?>','<? echo $bgcolor;?>')" id="trr_<? echo $p;?>">
                                ...
                                <td width="100"><p><? echo $supplier_arr[$row['supplier_id']]; ?></p></td>
                                <td width="100"><p><? echo $row['booking_no']; ?></p></td>
                                <td width="90"><p><? echo $color_arr[$row['color_id']]; ?></p></td>
                                <td width="90" title="<? echo $value[csf('id')]; ?>"><p><? echo $row['pi_number']; ?></p></td>
            ...
            <div style="width:1620px; overflow-y:scroll; max-height:300px;"  align="left" id="scroll_body_tbl_woven_finish_feb_rcv">
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1600" class="rpt_table" id="tbl_woven_finish_feb_rcv" >
                 	<?
					$pi_id_all=implode(",",$pi_id_all_array);
                    $pi_workd_order_id_all=implode(",",$pi_with_work_order_array);
                    //var_dump($pi_id_all);
                    $compos=''; $tot_recv_qnty=0; $tot_recv_amnt=0; $recv_id_array=array(); $recv_pi_array=array(); 
                    //echo $pi_id_all.Jahhid;die;
					if ($pi_id_all!='' || $pi_id_all!=0)
					{
						/*$sql_recv="select a.id as rec_id, a.recv_number, a.receive_date, a.challan_no, a.booking_id as pi_id, a.booking_no as pi_number, b.remarks, b.prod_id, b.pi_wo_batch_no as batch_id, c.color_id, d.po_number, d.id as po_id, e.job_no, e.buyer_name,  e.style_ref_no, f.construction, f.copmposition, c.quantity, b.order_rate
                        from inv_receive_master a, inv_transaction b, order_wise_pro_details c, wo_po_break_down d, wo_po_details_master e, wo_booking_dtls f
                        where a.id = b.mst_id and b.id=c.trans_id and c.po_breakdown_id=d.id and d.job_no_mst=e.job_no and e.job_no=f.job_no and a.item_category = 3 and a.entry_form=17 and a.company_id = $cbo_company_name and a.receive_basis = 1 and b.item_category = 3 and b.transaction_type = 1 and a.booking_id in ($pi_id_all) and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0";*/
						...
						$sql_recv="SELECT a.id as rec_id, a.recv_number, a.recv_number_prefix_num, a.receive_date, a.challan_no, a.booking_id as pi_id, a.booking_no as pi_number, a.supplier_id, b.remarks, b.prod_id, b.batch_id as batch_id, c.color_id, f.construction, f.copmposition, c.quantity, b.order_rate, c.po_breakdown_id, c.id as propo_id, a.receive_basis
                        from inv_receive_master a, inv_transaction b, order_wise_pro_details c, wo_booking_dtls f
                        where a.id = b.mst_id and b.id=c.trans_id and c.po_breakdown_id=f.po_break_down_id and a.item_category = 3 and a.entry_form=17 and a.company_id = $cbo_company_name and a.receive_basis = 1 and b.item_category = 3 and b.transaction_type = 1 and a.booking_id in ($pi_id_all) and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0 and f.status_active = 1 and f.is_deleted = 0";
					}
                    ...
                    if($pi_workd_order_id_all!="")
                    {
                        ...
                        $sql_recv.=" SELECT a.id as rec_id, a.recv_number, a.recv_number_prefix_num, a.receive_date, a.challan_no, a.booking_id as pi_id, a.booking_no as pi_number, a.supplier_id, b.remarks, b.prod_id, b.batch_id as batch_id, c.color_id, f.construction, f.copmposition, c.quantity, b.order_rate, c.po_breakdown_id, c.id as propo_id, a.receive_basis
                        from inv_receive_master a, inv_transaction b, order_wise_pro_details c, wo_booking_dtls f
                        where a.id = b.mst_id and b.id=c.trans_id and c.po_breakdown_id=f.po_break_down_id and a.item_category = 3 and a.entry_form=17 and a.company_id = $cbo_company_name and a.receive_basis = 2 and b.item_category = 3 and b.transaction_type = 1 and a.booking_id in ($pi_workd_order_id_all) and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0 and f.status_active = 1 and f.is_deleted = 0";
                    }
                    //echo $sql_recv;//die;
                    ...
                    foreach ($dataArray as $value) 
					{
                        ...
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['receive_date']=$value[csf('receive_date')];
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['pi_id']=$value[csf('pi_id')];
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['pi_number']=$value[csf('pi_number')];
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['color_id']=$value[csf('color_id')];
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['batch_id']=$value[csf('batch_id')];
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['challan_no']=$value[csf('challan_no')];
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['supplier_id']=$value[csf('supplier_id')];
						$pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['receive_basis']=$value[csf('receive_basis')];
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['recv_number_prefix_num']=$value[csf('recv_number_prefix_num')];
                        
                        
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['po_number']=$po_data[$value[csf("po_breakdown_id")]]["po_number"];
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['job_no']=$po_data[$value[csf("po_breakdown_id")]]["job_no"];
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['buyer_name']=$po_data[$value[csf("po_breakdown_id")]]["buyer_name"];
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['style_ref_no']=$po_data[$value[csf("po_breakdown_id")]]["style_ref_no"];
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['construction']=$value[csf('construction')];
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['copmposition']=$value[csf('copmposition')];
                        //echo "sumon";
                        if($propotion_check[$value[csf('propo_id')]]=="")
                        {
                            $propotion_check[$value[csf('propo_id')]]=$value[csf('propo_id')];
                            $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['quantity']+=$value[csf('quantity')];
                        }
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['order_rate']=$value[csf('order_rate')];
                        $pi_rec_data_arr[$value[csf('pi_id')]][$value[csf('recv_number')]][$value[csf('color_id')]][$value[csf('prod_id')]][$value[csf('batch_id')]]['remarks']=$value[csf('remarks')];
						
						//$pi_data_array[$value[csf('pi_id')]]['rcv_basis'] = $value[csf('receive_basis')];
                        //$pi_data_array[$value[csf('pi_id')]]['rcv_amt'] +=($value[csf('order_rate')]*$value[csf('quantity')]);
						//$pi_data_array[$value[csf('pi_id')]]['rcvQnt'] +=$value[csf('quantity')];

                        $pi_rcv_rtn_rate_arr[$value[csf('rec_id')]][$value[csf('prod_id')]]['rate']=$value[csf('order_rate')];
                        $recv_id_array[$value[csf('rec_id')]]=$value[csf('rec_id')];
                        $recv_mrr_array[$value[csf('rec_id')]]=$value[csf('recv_number')];

                        if($value[csf('receive_basis')]==1)
                        {
                            $recv_pi_array[$value[csf('rec_id')]]=$value[csf('pi_id')];
                        }
                        else
                        {
                            $recv_pi_array[$value[csf('rec_id')]]=$booking_pi[$value[csf('pi_id')]];
                        }
                    }
                    //echo '<pre>';print_r($pi_data_array);die;
                    fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 202, 1,$recv_id_array, $empty_arr);
                    oci_commit($con);

                    $i=1;
                    foreach($pi_rec_data_arr as $pi_id=> $row_recv)
                    {
                        foreach ($row_recv as  $recv_num=>$row_recv_num) 
                        {
                            foreach ($row_recv_num as  $color_id=>$row_color) 
                            {
                                foreach ($row_color as  $prod_id=>$row_prod) 
                                {
                                    foreach ($row_prod as  $batch_id=>$value) 
                                    {
                                        if ($i%2==0) $bgcolor="#E9F3FF";
                                        else $bgcolor="#FFFFFF";

                                        $item_description=$value['construction']." ".$value['copmposition'];
                                        $rate= $value['order_rate'];
                                        $amount= $value['quantity']*$value['order_rate'];
                                        //echo $pi_id.'===';
    									if($value['receive_basis']==1)
    									{
    										$pi_data_array[$pi_id]['rcv_value']+=$value['quantity']*$rate;
    									}
    									else
    									{
                                        ...
                                        <tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('trp_<? echo $i;?>','<? echo $bgcolor;?>')" id="trp_<? echo $i;?>">
                                            ...
                                            <td width="140" align="center" title="<?= $supplier_pi_arr[$value['pi_id']]['supplier']; ?>"><? echo $supplier_arr[$value['supplier_id']]; ?></td>
                                            <td width="90" align="center"><? echo $value['pi_number']; ?></td>
                                            <td width="100"><p><? echo $value['recv_number_prefix_num']; ?></p></td>
                                            <td width="90" align="center"><? echo change_date_format($value['receive_date']); ?></td>
                                            <td width="80"><p>&nbsp;<? echo chop($value['challan_no'],','); ?></p></td>
                                            <td width="120"><p>&nbsp;<? echo $item_description; ?></p></td>
                                            <td width="70" align="center" title="<?= chop($value['batch_id'],','); ?>"><p><? echo $batch_arr[$value['batch_id']]; ?></p></td>
                                            <td width="80" align="right"><p><? echo number_format($value['quantity'],2,'.',''); ?></p></td>
            ...
            <!-- ================ Woven Finish Febric Receive End ========= -->
            <br>
            <!-- ================ Woven Finish Febric Receive Return Start ============== -->
            <table width="1200" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table">
                <thead>
                    <tr>
                      <th colspan="9">Woven Finish Febric Receive Return</th>
                    </tr>
                    <tr>
                        <th width="40">SL</th>
                        <th width="100">MRR No.</th>
            ...
            <div style="width:1220px; overflow-y:scroll; max-height:430px;"  align="left">
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1200" class="rpt_table" id="tbl_woven_finish_feb_rcv_rtrn" >
                    <?
                    ...
                    if(count($recv_id_array)>0)
                    {
                        ...
                        from inv_issue_master a, inv_transaction b, product_details_master c, inv_receive_master d, GBL_TEMP_ENGINE e
                        where a.id=b.mst_id and a.received_id=d.id and b.prod_id=c.id and a.item_category=3 and a.entry_form=202 and a.company_id=$cbo_company_name
                        and a.received_id=e.ref_val and e.user_id=$user_id and e.entry_form=202
                        and  b.item_category=3 and b.transaction_type=3 and  a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 ";//and a.received_id in($recv_id_all)
                        // echo $sql_retn;
                        ...
                        foreach($dataRtArray as $row_retn)
                        {
                            ...
                            {
                                $rate = $row_retn[csf('cons_amount')]/$row_retn[csf('cons_quantity')];
                            }
                            if($rate=='' || $rate==0)
                            {
                            ...
                            <tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('trrc_<? echo $i;?>','<? echo $bgcolor;?>')" id="trrc_<? echo $i;?>">
                                <td width="40" align="center"><? echo $i; ?></td>
                                <td width="100"><p><? echo $recv_mrr_array[$row_retn[csf('received_id')]]; ?></p></td>
            ...
            <!-- ================ Woven Finish Febric Receive Return End ================ -->
            <br>
            <!-- ================ Acceptance Details Start ============================== -->
            <table width="950" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table" id="woven_acceptance_tble">
                <thead>
                    <tr>
            ...
            <div style="width:970px; overflow-y:scroll; max-height:430px;"  align="left">
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="950" class="rpt_table" id="tbl_woven_finish_feb_acceptance" >
                    <?
                    //var_dump($pi_id_all);
    					if ($pi_id_all!='' || $pi_id_all!=0)
    					{
                            ...
                            $acceptance_sql = "SELECT m.id as pi_id, m.pi_basis_id, m.goods_rcv_status,m.supplier_id, a.company_acc_date, b.current_acceptance_value as acceptance_value, listagg(cast(n.work_order_id as varchar(4000)), ',') within group (order by n.work_order_id) work_order_id
    						...
    						left join com_import_invoice_mst a on a.id=b.import_invoice_id and a.status_active=1
    						where  m.id in($pi_id_all)
    						group by m.id, m.pi_basis_id, m.goods_rcv_status,m.supplier_id, a.company_acc_date, b.current_acceptance_value
                            order by m.id";
                            // echo $acceptance_sql;
    						...
    						foreach($accep_result as $row)
    						{
    							...
    							$payble_value=$yet_to_accept=$recv_value=$rtn_value=0;
    							//echo $row[csf("goods_rcv_status")].test;die;
    							?>
    							<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
    							<?
    							if($temp_pi[$row[csf("pi_id")]]=="")
    							{
    								$recv_value=$pi_data_array[$row[csf("pi_id")]]['rcv_value'];
    								$rtn_value=$pi_data_array[$row[csf("pi_id")]]['rtn'];
    								//echo $recv_value;die;
    								$payble_value=$recv_value-$rtn_value;
    								$yet_to_accept=$payble_value-$acceptance_arr[$row[csf("pi_id")]]["acceptance_value"];
    								//echo $yet_to_accept;die;
    								$total_receive_value += $recv_value;
    								$total_return_value += $rtn_value;
    								...
    								//echo $recv_value;die;
    								?>
    								<td width="140" title="<? echo $row[csf("pi_id")]; ?>" rowspan="<? echo $accep_breakdown_data[$row[csf("pi_id")]];?>" align="center"><p><? echo $pi_name_array[$row[csf("pi_id")]]; ?></p></td>
    								<td width="120" rowspan="<? echo $accep_breakdown_data[$row[csf("pi_id")]];?>" align="center"><p><? echo $supplier_arr[$row[csf("supplier_id")]]; ?></p></td>
    								<td width="100"  align="right" rowspan="<? echo $accep_breakdown_data[$row[csf("pi_id")]];?>"><? echo number_format($recv_value,2,'.',''); ?></td>
    ...
    <!--</fieldset>-->
	<?
    execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_id and entry_form in (202)");
    oci_commit($con);
