...
*/
			...
			else if($rptType==2)
			{
				$sql="select 
					...
					from
						inv_transaction a, order_wise_pro_details b, wo_po_break_down c,wo_po_details_master d, product_details_master p
					where
						a.id=b.trans_id and b.po_breakdown_id=c.id and c.job_no_mst=d.job_no and p.id=a.prod_id and a.item_category=4 and d.company_name=$cbo_company_name $cbo_buyer_name $txt_style_ref_id $txt_order_id $date_cond and b.trans_type in(1,4) and a.status_active=1 and a.is_deleted=0 and b.entry_form in(24)  order by a.transaction_date, a.id";
			}
			else if($rptType==3)
			{
				$sql="select 
					...
					from
						inv_transaction a, order_wise_pro_details b, wo_po_break_down c,wo_po_details_master d, product_details_master p
					where
						a.id=b.trans_id and b.po_breakdown_id=c.id and c.job_no_mst=d.job_no and p.id=a.prod_id and a.item_category=4 and b.trans_type in(2,3) and d.company_name=$cbo_company_name $cbo_buyer_name $txt_style_ref_id $txt_order_id $date_cond  and a.status_active=1 and a.is_deleted=0 and b.entry_form in(24,25,49)  order by a.transaction_date, a.id";
			}
						
			//echo $sql;die;
			$sql_result_order=sql_select($sql);
		...
		if($cbo_order_type==0 || $cbo_order_type==2)
		{
			...
			if($rptType==1)
			{
				$sql_non_order="select 
					...
					where
						b.id=a.mst_id and p.id=a.prod_id and a.item_category=4 and a.company_id=$cbo_company_name $date_cond  and a.status_active=1 and a.is_deleted=0 and b.entry_form in(20,27) and a.transaction_type in(1,4)
					...
					where
						b.id=a.mst_id and p.id=a.prod_id and a.item_category=4 and a.company_id=$cbo_company_name $date_cond  and a.status_active=1 and a.is_deleted=0 and b.entry_form in(21,26) and a.transaction_type in(2,3)
			...
			{
				$trms_non_order_arr[$row[csf("trans_id")]]["trans_id"]=$row[csf("trans_id")];
				$trms_non_order_arr[$row[csf("trans_id")]]["prod_id"]=$row[csf("prod_id")];
				$trms_non_order_arr[$row[csf("trans_id")]]["transaction_type"]=$row[csf("transaction_type")];
				$trms_non_order_arr[$row[csf("trans_id")]]["rec_issue_id"]=$row[csf("mst_id")];
				$trms_non_order_arr[$row[csf("trans_id")]]["transaction_date"]=$row[csf("transaction_date")];
				$trms_non_order_arr[$row[csf("trans_id")]]["receive_qty"]+=$row[csf("receive_qty")];
				$trms_non_order_arr[$row[csf("trans_id")]]["issue_qty"]+=$row[csf("issue_qty")];
				$trms_non_order_arr[$row[csf("trans_id")]]["cons_uom"]=$row[csf("cons_uom")];
				$trms_non_order_arr[$row[csf("trans_id")]]["cons_rate"]=$row[csf("cons_rate")];
				$trms_non_order_arr[$row[csf("trans_id")]]["inserted_by"]=$row[csf("inserted_by")];
				$trms_non_order_arr[$row[csf("trans_id")]]["insert_date"]=$row[csf("insert_date")];
				$trms_non_order_arr[$row[csf("trans_id")]]["insert_time"]=$row[csf("insert_time")];
				$trms_non_order_arr[$row[csf("trans_id")]]["item_description"]=$row[csf("item_description")];
				$trms_non_order_arr[$row[csf("trans_id")]]["product_name_details"]=$row[csf("product_name_details")];
				$trms_non_order_arr[$row[csf("trans_id")]]["color"]=$row[csf("color")];
				$trms_non_order_arr[$row[csf("trans_id")]]["item_color"]=$row[csf("item_color")];
				$trms_non_order_arr[$row[csf("trans_id")]]["gmts_size"]=$row[csf("gmts_size")];
				$trms_non_order_arr[$row[csf("trans_id")]]["item_size"]=$row[csf("item_size")];
				$trms_non_order_arr[$row[csf("trans_id")]]["item_group_id"]=$row[csf("item_group_id")];
			}*/
		}
			...
			$sql_prod="select
						...
						where
								a.id=b.prod_id and b.entry_form in(24,25) order by b.trans_id";
			...
			if($rptType==1)
			{
				$table_width_non_order=1600;
				$div_width_non_order="1620px";
				$col_span_non_order=17;
			}
			else
			{
				$table_width_non_order=1520;
				$div_width_non_order="1540px";
				$col_span_non_order=16;
			}
			if($cbo_order_type==2)
			{
				$scroll_div="scroll_body";
				$scroll_div_width="1620px";
				$table_width=1600;
			} 
			else 
			{
			...
			<div id="<? echo $scroll_div; ?>" style="width:<? echo $scroll_div_width; ?>;"> 
                    ...
                    <table width="<? echo $table_width; ?>" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all" id="table_header_1" align="left"> 
                      ...
                      <div style="width:<? echo $div_width; ?>; overflow-y: scroll; max-height:240px;" id="scroll_body">
                        <table width="<? echo $table_width; ?>" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body" align="left">
                            ...
                            $i=1;
                            foreach($sql_result_order as $val)
                            {
								if ($i%2==0)
								$bgcolor="#E9F3FF";
								else
								...
								<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
									...
									<td width="50"><p><? echo $val[csf("prod_id")]; ?>&nbsp;</p></td>
									...
									<td width="60"  align="center"><p><? echo $val[csf("job_year")]; ?>&nbsp;</p></td>
									<td width="50"  align="center"><p><? echo $val[csf("job_no_prefix_num")]; ?>&nbsp;</p></td>
									<td width="100"><p><? echo $val[csf("style_ref_no")]; ?>&nbsp;</p></td>
									<td width="90"><p><? echo $buyer_short_arr[$val[csf("buyer_name")]]; ?>&nbsp;</p></td>
									<td width="120" align="center"><p><? echo $val[csf("po_number")]; ?>&nbsp;</p></td>
									<td width="100"><p>
									<?
										if($val[csf("transaction_type")]==1 || $val[csf("transaction_type")]==4) 
										{
									...
									<?
										if($val[csf("transaction_type")]==1 || $val[csf("transaction_type")]==4) 
										{
											echo $supplier_arr[$receive_num_arr[$val[csf('rec_issue_id')]]["supplier_id"]];
										}
										else if($val[csf("transaction_type")]==2 || $val[csf("transaction_type")]==3)
										{
											echo "&nbsp;";
									...
									?>
									&nbsp;</p></td>
									<td width="80"  align="center"><p><? if($val[csf("shipment_date")]!="0000-00-00") echo change_date_format($val[csf("shipment_date")]); else echo ""; ?>&nbsp;</p></td>
									<td width="100"><p><? echo $group_arr[$val[csf("item_group_id")]]["item_name"]; ?>&nbsp;</p></td>
									<td width="220"><p><? echo $val[csf("product_name_details")]; ?>&nbsp;</p></td>
									<td width="80"><p><? echo $color_arr[$val[csf("color")]]; ?>&nbsp;</p></td>
									<td width="60"><p><? echo $size_arr[$val[csf("gmts_size")]]; ?>&nbsp;</p></td>
									<?
									if($rptType==2 || $rptType==1 )
									{
										?>
										<td width="80" align="center"><p><? if($val[csf("transaction_type")]==1) echo $currency[$receive_num_arr[$val[csf('rec_issue_id')]]["currency_id"]]; ?></p></td>
										<td width="80" align="right"><p><? if($val[csf("transaction_type")]==1) echo number_format($receive_num_arr[$val[csf('rec_issue_id')]]["exchange_rate"],2); ?></p></td>
										<td width="80" align="right"><p><? if($val[csf("transaction_type")]==1) echo number_format($val[csf("order_rate")],2); ?></p></td>
										
										<td width="80" align="right"><p>
										<?
										if($val[csf("transaction_type")]==1 || $val[csf("transaction_type")]==4) 
										{
											$rcv_issue_qnty=$group_arr[$val[csf("item_group_id")]]["conversion_factor"] *$val[csf("receive_qty")];
									...
									if($rptType==3 || $rptType==1 )
									{
										...
										<td width="80" align="right"><p>
										<?
										 if($val[csf("transaction_type")]==2 || $val[csf("transaction_type")]==3)
										 {
											 $rcv_issue_qnty=$group_arr[$val[csf("item_group_id")]]["conversion_factor"] *$val[csf("issue_qty")];
									...
									?>
									<td width="80" align="right"><p><? echo number_format($val[csf("cons_rate")],4,".",""); ?></p></td>
									<td width="100" align="right"><p>
									<?
									$amount=$rcv_issue_qnty*$val[csf("cons_rate")];
									 echo number_format($amount,2,".",""); $total_amount+=$amount; 
									?>
									 </p></td>
									<td width="48"><p><? echo $unit_of_measurement[$val[csf("cons_uom")]]; ?>&nbsp;</p></td>
									<td width="110"><p><? echo $user_name_arr[$val[csf("inserted_by")]]; ?>&nbsp;</p></td>
				...
				if($cbo_order_type==0 || $cbo_order_type==2)
				{
                    ...
                    <table width="<? echo $table_width_non_order; ?>" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all" id="table_header_3" align="left">
                        <thead>
                            <tr>
                                <th width="30" >SL</th>
                                <th width="50" >Prod. Id</th>
                            ...
                            $j=1;
                            foreach($sql_result_non_order as $val)
                            {
								if ($i%2==0)
								$bgcolor="#E9F3FF";
								else
								...
								<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
									...
									{
										?>
										<td width="80" align="right"><p><?  $rcv_issue_qnty=$val[csf("receive_qty")]; echo number_format($val[csf("receive_qty")],2,".",""); $total_receive_qty_non_order+=$val[csf("receive_qty")];?></p></td>
										<?
									}
									if($rptType==3 || $rptType==1 )
									{
										?>
										<td width="80" align="right"><p><? $rcv_issue_qnty=$val[csf("issue_qty")]; echo number_format($val[csf("issue_qty")],2,".",""); $total_issue_qty_non_order+=$val[csf("issue_qty")];?></p></td>
										<?
									}
									?>
									<td width="80" align="right"><p><? echo number_format($val[csf("cons_rate")],4,".",""); ?></p></td>
	...
	else if($cbo_item_cat==2)
	{
			...
			else if($rptType==2)
			{
				$sql="select 
					...
					from
						inv_transaction a, order_wise_pro_details b, wo_po_break_down c,wo_po_details_master d,product_details_master e
					where
						a.id=b.trans_id and b.po_breakdown_id=c.id and c.job_no_mst=d.job_no and e.id=b.prod_id and a.item_category=2 and b.trans_type in(1,4,5) and d.company_name=$cbo_company_name $cbo_buyer_name $txt_style_ref_id $txt_order_id $date_cond and a.transaction_type in(1,4,5)  and a.status_active=1 and a.is_deleted=0 and b.entry_form in (7,37,15)  order by a.transaction_date, a.id";
			}
			else if($rptType==3)
			{
				$sql="select 
		...
		if($cbo_order_type==0 || $cbo_order_type==2)
		{
			if($rptType==1)
			{
				$sql_non_order="select 
						...
						d.batch_no
					from
						inv_transaction a, inv_receive_master b, pro_finish_fabric_rcv_dtls c, pro_batch_create_mst d, product_details_master e
					where
						a.mst_id=b.id and e.id=a.prod_id  and b.id=c.mst_id and c.batch_id=d.id and d.booking_without_order=1 and a.item_category=2 and a.company_id=$cbo_company_name $date_cond and a.transaction_type in(1,4)  and a.status_active=1 and a.is_deleted=0 and b.entry_form in (7,37)  
					...
					select 
						...
						d.batch_no
					from
						inv_transaction a, inv_issue_master b, inv_finish_fabric_issue_dtls c, pro_batch_create_mst d, product_details_master e
					where
						a.mst_id=b.id and e.id=a.prod_id and b.id=c.mst_id  and c.batch_id=d.id and d.booking_without_order=1 and b.issue_purpose=8 and a.item_category=2 and a.company_id=$cbo_company_name $date_cond and a.transaction_type in(2,3)  and a.status_active=1 and a.is_deleted=0 and b.entry_form in (18,46)
			...
			else if($rptType==2)
			{
				$sql_non_order="select 
					...
					from
						inv_transaction a, inv_receive_master b, pro_finish_fabric_rcv_dtls c, pro_batch_create_mst d, product_details_master e
					where
						a.mst_id=b.id and e.id=a.prod_id  and b.id=c.mst_id and c.batch_id=d.id and d.booking_without_order=1 and a.item_category=2 and a.company_id=$cbo_company_name $date_cond and a.transaction_type in(1,4)  and a.status_active=1 and a.is_deleted=0 and b.entry_form in (7,37)  order by a.transaction_date, a.id";
			}
			else if($rptType==3)
			{
				$sql_non_order="select 
					...
					from
						inv_transaction a, inv_issue_master b, inv_finish_fabric_issue_dtls c, pro_batch_create_mst d,product_details_master e
					where
						a.mst_id=b.id and e.id=a.prod_id and b.id=c.mst_id  and c.batch_id=d.id and d.booking_without_order=1 and b.issue_purpose=8 and a.item_category=2 and a.company_id=$cbo_company_name $date_cond and a.transaction_type in(2,3)  and a.status_active=1 and a.is_deleted=0 and b.entry_form in (18,46) order by a.transaction_date, a.id";
			}
			...
			foreach($sql_result_non_order as $row)
			{
				...
				$trms_non_order_arr[$row[csf("trans_id")]]["color"]=$row[csf("color")];
				$trms_non_order_arr[$row[csf("trans_id")]]["item_color"]=$row[csf("item_color")];
				$trms_non_order_arr[$row[csf("trans_id")]]["detarmination_id"]=$row[csf("detarmination_id")];
				$trms_non_order_arr[$row[csf("trans_id")]]["gsm"]=$row[csf("gsm")];
				$trms_non_order_arr[$row[csf("trans_id")]]["dia_width"]=$row[csf("dia_width")];
				$trms_non_order_arr[$row[csf("trans_id")]]["batch_id"]=$row[csf("batch_id")];
				$trms_non_order_arr[$row[csf("trans_id")]]["batch_no"]=$row[csf("batch_no")];
			}
		...
		{
			$scroll_div="scroll_body";
			$scroll_div_width="1330px";
			$table_width=1310;
		} 
		else 
		{
			$scroll_div="";
			$scroll_div_width="1720px";
			$table_width=1700;
		}
		//var_dump($product_result_arr);die;
		...
		<div style="width:<? echo $scroll_div_width; ?>" id="<? echo $scroll_div; ?>"> 
			...
			if($cbo_order_type==0 || $cbo_order_type==2)
			{
				if(count($trms_non_order_arr)>0)
				{
				?>
			 <table width="1310"  id="table_header_3" align="left"> 
             	<tr>
                	<td colspan="15"><span style="font-size:16px; font-weight:bold;">Non Order Item</span></td>
                </tr>
             </table>
			 <table width="1310" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all" id="table_header_2" align="left"> 
					<thead>
						<tr>
							<th width="30" >SL</th>
							<th width="50" >Prod. Id</th>
				...
				<table width="1310" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body2" align="left">
					<tbody>
					<?
					...
					foreach($trms_non_order_arr as $trans_key=>$val)
					{
						...
						<tr  bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
							...
							<td width="50"><p><? echo $val["gsm"]; ?>&nbsp;</p></td>
							<td width="50"><p><? echo $val["width"]; ?>&nbsp;</p></td>
							<td width="80" align="right"><p><? echo number_format($val["receive_qty"],2); $total_receive_non_order +=$val["receive_qty"]; ?></p></td>
							<td width="80" align="right"><p><?  echo number_format($val["issue_qty"],2); $total_issue_non_order +=$val["issue_qty"]; ?></p></td>
							<td width="100"><p><?  echo $val["batch_no"];?>&nbsp;</p></td>
							<td width="110"><p><? echo $user_name_arr[$val["inserted_by"]]; ?>&nbsp;</p></td>
							<td ><p><? echo change_date_format($val["insert_date"])." ".$val["insert_time"]; ?>&nbsp;</p></td>
						</tr>
						<?
						$i++;$j++;
				...
				<table width="1310" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all" id="report_table_footer2" align="left">
					<tfoot>
						<th width="30">&nbsp;</th>
						<th width="50">&nbsp;</th>
						<th width="80">&nbsp;</th>
	...
	else if($cbo_item_cat==13)
	{
		...
		if($cbo_order_type==0 || $cbo_order_type==2)
		{
			...
			if($rptType==1)
			{
				$sql_non_order="select 
						...
						f.yarn_count
					from
						inv_transaction a, inv_receive_master b, product_details_master e, pro_grey_prod_entry_dtls f
					where
						a.mst_id=b.id and e.id=a.prod_id and f.trans_id=a.id and b.booking_without_order=1 and a.item_category=13 and a.company_id=$cbo_company_name $date_cond and a.transaction_type in(1,4) and b.entry_form in (2,22,51) and a.status_active=1 and a.is_deleted=0
					...
					select 
						...
						f.yarn_count
					from
						inv_transaction a,  inv_issue_master b, product_details_master e, inv_grey_fabric_issue_dtls f
					where
						a.mst_id=b.id and e.id=a.prod_id and f.trans_id=a.id and b.issue_purpose=8 and a.item_category=13 and a.company_id=$cbo_company_name $date_cond and a.transaction_type in(2,3) and b.entry_form in (16,45) and a.status_active=1 and a.is_deleted=0
			...
			else if($rptType==2)
			{
				$sql_non_order="select 
					...
					from
						inv_transaction a, inv_receive_master b, product_details_master e, pro_grey_prod_entry_dtls f
					where
						a.mst_id=b.id and e.id=a.prod_id and f.trans_id=a.id and b.booking_without_order=1 and a.item_category=13 and a.company_id=$cbo_company_name $date_cond and a.transaction_type in(1,4) and b.entry_form in (2,22,51) and a.status_active=1 and a.is_deleted=0  order by a.transaction_date, a.id";
			}
			else if($rptType==3)
			{
				$sql_non_order="select 
					...
					from
						inv_transaction a,  inv_issue_master b, product_details_master e, inv_grey_fabric_issue_dtls f
					where
						a.mst_id=b.id and e.id=a.prod_id and f.trans_id=a.id and b.issue_purpose=8 and a.item_category=13 and a.company_id=$cbo_company_name $date_cond and a.transaction_type in(2,3) and b.entry_form in (16,45) and a.status_active=1 and a.is_deleted=0  order by a.transaction_date, a.id";
			}
			...
			foreach($sql_result_non_order as $row)
			{
				...
				$trms_non_order_arr[$row[csf("trans_id")]]["gsm"]=$row[csf("gsm")];
				$trms_non_order_arr[$row[csf("trans_id")]]["dia_width"]=$row[csf("dia_width")];
				$trms_non_order_arr[$row[csf("trans_id")]]["dtls_color"]=$row[csf("dtls_color")];
				$trms_non_order_arr[$row[csf("trans_id")]]["stitch_length"]=$row[csf("stitch_length")];
				$trms_non_order_arr[$row[csf("trans_id")]]["yarn_lot"]=$row[csf("yarn_lot")];
				$trms_non_order_arr[$row[csf("trans_id")]]["yarn_count"]=$row[csf("yarn_count")];
			}
		...
		{
			$scroll_div="scroll_body";
			$scroll_div_width="1550px";
			$table_width=1530;
		} 
		else 
		{
			$scroll_div="";
			$scroll_div_width="1930px";
			$table_width=1910;
		}

		//var_dump($trms_non_order_arr);die;
		...
		<div style="width:<? echo $scroll_div_width; ?>" id="<? echo $scroll_div;?>"> 
			...
			if(count($trms_non_order_arr)>0)
			{
				if($cbo_order_type==0 || $cbo_order_type==2)
				{
					?>
					 <br />
					 <table width="1530" id="table_header_3" align="left">
						<tr>
							<td><span style="font-size:16px; font-weight:bold;">Non Order Item</span></td>
						</tr>
					 </table> 
					 <table width="1530" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all" id="table_header_2" align="left"> 
							<thead>
								<tr>
									<th width="30" >SL</th>
									<th width="50" >Prod. Id</th>
						...
						<table width="1530" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body2" align="left">
							<tbody>
							<?
							...
							foreach($trms_non_order_arr as $trans_key=>$val)
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
									...
									<td width="80" align="right"><p><? echo number_format($val["receive_qty"],2); $total_receive_non_order +=$val["receive_qty"]; ?></p></td>
									<td align="right" width="80"><p><? echo number_format($val["issue_qty"],2); $total_issue_non_order +=$val["issue_qty"]; ?></p></td>
									<td width="110"><p><? echo $user_name_arr[$val["inserted_by"]]; ?>&nbsp;</p></td>
									<td width="160"><p><? echo change_date_format($val["insert_date"])." ".$val["insert_time"]; ?>&nbsp;</p></td>
								</tr>
								<?
						...
						<table width="1530" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all" id="report_table_footer" align="left">
							<tfoot>
								<th width="30">&nbsp;</th>
								<th width="50">&nbsp;</th>
								<th width="80">&nbsp;</th>
