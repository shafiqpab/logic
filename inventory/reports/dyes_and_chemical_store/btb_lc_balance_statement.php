...
function openmypage_popup(company_id,pi_id,item_category_id,page_title,action)
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/btb_lc_balance_statement_controller.php?company_id='+company_id+'&pi_id='+pi_id+'&item_category_id='+item_category_id+'&action='+action, page_title, 'width='+width+',height=400px,center=1,resize=0,scrolling=0','../../');
}
                    ...
                    <tr class="general">
                        <td style="display:none"> 
                            <?
                                ...
                                echo create_drop_down( "cbo_item_category", 150, $item_category,"", 1, "-- Select Category --",0,"",0,"5,6,7,23","","","" );
                            ?>
                        </td>
                        <td>
                            <? 
                               echo create_drop_down( "cbo_company_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "reset_form('','','txt_btbLc_no*btbLc_id','','',''); load_drop_down( 'requires/btb_lc_balance_statement_controller', this.value, 'load_drop_down_supplier', 'supplier_td' );" );
                            ?>                            
                        </td>
                        <td id="supplier_td"> 
                            <?
