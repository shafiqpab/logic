...
if($action=="create_pi_search_list_view")
{
	...
	if($btbLc_id!="")
	{
		...
		where a.id=b.pi_id and b.com_btb_lc_master_details_id=$btbLc_id and a.importer_id=$company and a.entry_form=227 and a.ITEM_CATEGORY_ID in(5,6,7,23) and a.supplier_id like '$cbo_supplier' and a.pi_number like '%".$txt_search_common."%' and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1
		group by a.id, a.pi_number, a.supplier_id, a.importer_id, a.pi_date, a.last_shipment_date, a.net_total_amount";
	}
	else
	{
		$sql= "select id, pi_number, supplier_id, importer_id, pi_date, last_shipment_date, net_total_amount from com_pi_master_details where importer_id=$company and entry_form=227 and ITEM_CATEGORY_ID in(5,6,7,23) and supplier_id like '$cbo_supplier' and pi_number like '%".$txt_search_common."%' and is_deleted=0 and status_active=1";
	}
				...
				<tbody>
					<tr align="center">
						...
						<td>
						 ...
						 <td align="center">
							<input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_supplier_id').value+'_'+document.getElementById('txt_search_common').value+'_'+<? echo $companyID; ?>+'_'+'<? echo $pi_id; ?>'+'_'+document.getElementById('txt_pi_no').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value, 'create_lc_search_list_view', 'search_div', 'btb_lc_balance_statement_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />				
						</td>
					</tr>
					<tr>
...
if($action=="create_lc_search_list_view")
{
	...
	where a.id=b.com_btb_lc_master_details_id and b.pi_id=c.id and a.importer_id=$company and a.pi_entry_form=227 and a.supplier_id like '$cbo_supplier' and a.lc_number like '%".$txt_search_common."%' $sql_conds and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and a.ref_closing_status=0 
...
{ 
	...
	if($db_type==0)
	{
		...
		where a.id=b.pi_id and b.com_btb_lc_master_details_id=c.id and c.importer_id=$cbo_company_name and c.is_deleted=0 and c.status_active=1 and c.ref_closing_status=0 and c.PI_ENTRY_FORM=227 and a.ITEM_CATEGORY_ID in(5,6,7,23) $sql_cond
	...
	$pi_sql=sql_select("select a.pi_id as pi_id, a.quantity as pi_qnty, a.net_pi_amount, b.com_btb_lc_master_details_id as btb_lc_id, c.current_acceptance_value as accep_value 
	...
	where a.pi_id=b.pi_id and a.status_active=1 and a.is_deleted=0 and a.ITEM_CATEGORY_ID in(5,6,7,23) $btb_ids_cond");
	$pi_data_arr=array();
	$pi_id_check=array();
	foreach($pi_sql as $row)
	{
		...
		$pi_data_arr[$row[csf("btb_lc_id")]]["pi_amount"]+=$row[csf("net_pi_amount")];
	...
	where a.id=b.mst_id and a.receive_basis=1 and a.entry_form=4 and b.transaction_type=1 and a.company_id=$cbo_company_name and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $pi_ids_cond group by a.booking_id");
	...
	if($cbo_receiving_status==1)
	{
		foreach($btb_sql_result as $row)
		{
			if(chop($pi_data_arr[$row[csf("btb_id")]]["pi_id"],",")!="")
			{
				$rcv_btb_value=0;
				...
				{
					$btb_data_arr[$row[csf("btb_id")]]["item_category_id"]=$row[csf("item_category_id")];
		...
		<table width="1850" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table" id="table_bodyy">   
			...
			foreach($btb_data_arr as $val)
			{
				...
				<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
					...
					<td width="100" align="right"><a href="##" onClick="openmypage_popup('<? echo $val["importer_id"]; ?>','<? echo $all_pi_id; ?>','<? echo $val["item_category_id"]; ?>','PI Info','pi_popup');" ><? echo number_format($pi_data_arr[$val["btb_id"]]["pi_amount"],2); $total_pi_amount+=$pi_data_arr[$val["btb_id"]]["pi_amount"]; ?></a></td>
					...
					<td width="100" align="right"><a href="##" onClick="openmypage_popup('<? echo $val["importer_id"]; ?>','<? echo $all_pi_id; ?>','<? echo $val["item_category_id"]; ?>','Acceptance Info','accep_popup');"><? $accep_value=$pi_data_arr[$val["btb_id"]]["accep_value"]; echo number_format($accep_value,2); $total_accep_value+=$accep_value; ?></a></td> 
