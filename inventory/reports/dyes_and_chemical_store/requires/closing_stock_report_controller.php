...
if ($action=="load_drop_down_store")
{
	...
	if($store_wish==1)
	{
		...
		// echo $sql;
			...
			$sql= "select a.id,a.store_name from lib_store_location a, lib_store_location_category b where a.id=b.store_location_id and  a.status_active=1 and a.is_deleted=0 and a.company_id=$company_id and b.category_type in(5,6,7,23) group by a.id, a.store_name";
		// echo $sql;
...
if($action=="generate_report")
{
	...
	if($report_type==22)//####### this buttton is off as per beesesh vai decission ######///
	{
		$company_cond="";
		...
		{
			$mrr_rate_sql=sql_select("select prod_id, sum(cons_quantity) as cons_quantiy, sum(store_amount) as cons_amount from inv_transaction where status_active=1 and is_deleted=0 and transaction_type in(1,4,5) $category_cond $company_cond group by prod_id");
		}
		else
		{
		...
		//echo $sql;//die;
		if ($cbo_store_name != "")  $store_cond=" and store_id in($cbo_store_name) ";
		$age_sql = "select PROD_ID, min(transaction_date) as MIN_DATE from inv_transaction where is_deleted=0 and status_active=1 and item_category in(5,6,7,23) and transaction_type in(1,4,5) and BALANCE_QNTY>0 and company_id=$cbo_company_name $store_cond group by prod_id";
		...
		<div style="width:1340px; border:1px solid white;">
			...
			<table  border="1" cellpadding="2" cellspacing="0" class="rpt_table" width="1770" rules="all" id="rpt_table_header"  align="left">
				<thead>
					<tr>
						<th width="30">SL</th>
                        ...
                        <th width="100">Lot</th>
                        <th width="100">UOM</th>
						<th width="200">Closing Stock</th>
						<th width="100">Avg. Rate</th>
						<th width="200">Stock Value</th>
			...
			<div style="width:1790px; max-height:250px; overflow-y:scroll;" id="scroll_body">
			<table border="1" cellpadding="2" cellspacing="0" class="rpt_table" id="table_body_id" width="1770" rules="all" align="left">
			<?
					...
					if((($get_upto==1 && $ageOfDays>$txt_days) || ($get_upto==2 && $ageOfDays<$txt_days) || ($get_upto==3 && $ageOfDays>=$txt_days) || ($get_upto==4 && $ageOfDays<=$txt_days) || ($get_upto==5 && $ageOfDays==$txt_days) || $get_upto==0) && (($get_upto_qnty==1 && $stockInHand>$txt_qnty) || ($get_upto_qnty==2 && $stockInHand<$txt_qnty) || ($get_upto_qnty==3 && $stockInHand>=$txt_qnty) || ($get_upto_qnty==4 && $stockInHand<=$txt_qnty) || ($get_upto_qnty==5 && $stockInHand==$txt_qnty) || $get_upto_qnty==0))
					{
						...
						if($value_with==1)
						{
							if(number_format($stockInHand,2)>0.0000)
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" style=" word-wrap:break-word; <? echo $stylecolor; ?> "  onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
									...
									<td width="100" align="center"><p><? echo $unit_of_measurement[$row[csf("unit_of_measure")]]; ?></p></td>
									<td width="200" align="right" title="<? echo $totalRcv."=".$totalIssue; ?>"><p><? echo number_format($stockInHand,4); $tot_stock+=$stockInHand; ?></p></td>
									<td width="100" align="right"><p><? if(number_format($stockInHand,4)>0) echo number_format($avg_rate,4) ; else echo "0.00";?></p></td>
									<td width="200" align="right"><p><? if(number_format($stockInHand,4)>0) echo number_format($stock_value,3); else echo "0.00"; $tot_stock_value+=$stock_value; ?></p></td>
                                    ...
                                    <td width="100" align="center"><p>&nbsp;<? echo change_date_format($row[csf("expire_date")]); ?></p></td>
									<td width="100" align="center"><p><? echo $ageOfDays; ?></p></td>
									<td width="100" align="center"><p><? echo $daysOnHand;?></p></td>
								</tr>
								<?
								$i++;
							}
						...
						else
						{
							...
							<tr bgcolor="<? echo $bgcolor; ?>" style=" word-wrap:break-word; <? echo $stylecolor; ?> "  onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
								...
								<td width="100"><p><? echo $supplierArr[$row[csf("supplier_id")]]; ?></p></td>
                                <td width="100"><p><? echo $row[csf("recv_number")]; ?></p></td>
								<td width="100" align="center">&nbsp;<? echo change_date_format($row[csf("receive_date")]); ?></p></td>
                                 <td width="100" align="center"><p>&nbsp;<? echo change_date_format($row[csf("expire_date")]); ?></p></td>
								<td width="100" align="center"><p><? echo $ageOfDays; ?></p></td>
								<td width="100" align="center"><p><? echo $daysOnHand;?></p></td>
							</tr>
			...
			<table width="1770" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" id="table_body_footer" align="left">
				<tfoot>
					<tr>
						<th width="30">&nbsp;</th>
						<th width="40">&nbsp;</th>
						...
						<th width="200" id="value_tot_stock"><? echo number_format($tot_stock,4);?></th>
						<th width="100">&nbsp;</th>
						<th width="200" id="value_tot_stock_value"><? echo number_format($tot_stock_value,2);?></th>
                        <th width="100">&nbsp;</th>
                        <th width="100">&nbsp;</th>
                        <th width="100">&nbsp;</th>
                        <th width="100">&nbsp;</th>
	...
	else if($report_type==3)
	{
		...
		if($variable_store_wise_rate==1)
		{
			$trans_sql="SELECT b.id as PROD_ID, a.transaction_date as TRANSACTION_DATE, a.transaction_type as TRANSACTION_TYPE, a.cons_quantity as CONS_QUANTITY, a.store_amount as CONS_AMOUNT, a.batch_lot as LOT_NO, a.receive_basis as RECEIVE_BASIS  
			from inv_transaction a, product_details_master b
			where a.prod_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $company_id $group_id $prod_cond $store_id $item_account $search_cond $item_category_id $item_category_cond 
			order by b.id ASC";
		}
		else
		{
			$trans_sql="SELECT b.id as PROD_ID, a.transaction_date as TRANSACTION_DATE, a.transaction_type as TRANSACTION_TYPE, a.cons_quantity as CONS_QUANTITY, a.cons_amount as CONS_AMOUNT, a.batch_lot as LOT_NO, a.receive_basis as RECEIVE_BASIS  
			from inv_transaction a, product_details_master b
			where a.prod_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $company_id $group_id $prod_cond $store_id $item_account $search_cond $item_category_id $item_category_cond 
			order by b.id ASC";
		}
		...
		//		if ($cbo_store_name != "")  $store_cond=" and store_id in($cbo_store_name) ";
		//		$returnRes_date="select prod_id as PROD_ID, max(transaction_date) as max_date from inv_transaction where is_deleted=0 and status_active=1 and company_id=$cbo_company_name $item_category_cond $store_cond group by prod_id";
		...
		//		}
		//		
		//		$age_sql = "select PROD_ID, min(transaction_date) as MIN_DATE from inv_transaction where is_deleted=0 and status_active=1 and transaction_type in(1,4,5) and BALANCE_QNTY>0 and company_id=$cbo_company_name $item_category_cond $store_cond group by prod_id";
		//		//echo $age_sql;
		//		$age_sql_result = sql_select($age_sql);
		//		foreach ($age_sql_result as $row) {
		//			$date_array[$row["PROD_ID"]]['min_date'] = $row["MIN_DATE"];
		...
		if($cbo_store_name != "")  $store_cond=" and store_id in($cbo_store_name) ";
		$date_array = array();
		$returnRes_date = "select prod_id, max(case when transaction_type in (1,5) and transaction_date <='$to_date' then transaction_date else null end) as min_date, max(case when transaction_date <='$to_date' then transaction_date else null end) as max_date from inv_transaction where is_deleted=0 and status_active=1 and company_id=$cbo_company_name $item_category_cond $store_cond group by prod_id";
		//echo $returnRes_date; die;
        ...
        <p style="color:#FF0000; font-size:16px; font-weight:bold;">Value Will Be Match Only For Item Level.</p>
		<div align="center" style="height:auto; width:auto; margin:0 auto; padding:0; width:2800px">
			...
			<table  border="1" cellpadding="2" cellspacing="0" class="rpt_table" width="2950" rules="all" id="rpt_table_header">
				<thead>
					<tr>
			...
			<div style="width:2950px; max-height:250px; overflow-y:scroll; overflow-x:hidden;" id="scroll_body">
			<table border="1" cellpadding="2" cellspacing="0" class="rpt_table" id="table_body_id" width="2930" rules="all" align="left">
			<?
				$pord_cond="";
				...
				foreach($result as $row)
				{
					...
					if(number_format($openingBalance,4,'.','')==0)
					{
						...
						if($value_with==1)
						{
							if(number_format($closingStock,3)>0.000 || number_format($openingBalance,3)>0.000)
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" style=" word-wrap:break-word; <? echo $stylecolor; ?> "  onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
									...
									<td width="100" align="right"><p><? if(number_format($openingBalance,4)!=0) echo number_format($openingRate,4); else echo "0.000"; ?></p></td>
									<td width="100" align="right"><p><? echo number_format($openingBalance,4); $tot_open_bl+=$openingBalance; ?></p></td>
									<td width="110" align="right"><p><? if(number_format($openingBalance,4)!=0) {echo number_format($openingBalanceValue,2); $tot_open_bl_amt+=$openingBalanceValue;} else echo "0.000"; ?></p></td>
									...
									<td width="120"><p><? echo $data_array[$row[csf("id")]]['lot_no']; ?></p></td>
                                    <td width="70"><p><? echo change_date_format($date_array[$row[csf("id")]]['min_date']); ?></p></td>
                                    <td><p><? echo change_date_format($date_array[$row[csf("id")]]['max_date']); ?></p></td>
								</tr>
								<?
								$i++;
			...
			<table width="2950" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" id="table_body_footer">
				<tfoot>
					<tr>
						<th width="50">&nbsp;</th>
						<th width="60">&nbsp;</th>
	...
	else if($report_type==4)
	{
		...
		if($variable_store_wise_rate==1)
		{
			...
			sum(case when a.transaction_type in(1,4,5) and a.transaction_date<'".$from_date."' then a.store_amount else 0 end) as rcv_total_opening_amt,
			sum(case when a.transaction_type in(2,3,6) and a.transaction_date<'".$from_date."' then a.store_amount else 0 end) as iss_total_opening_amt,
			...
			sum(case when a.transaction_type in(1,4,5) and a.transaction_date between '".$from_date."' and '".$to_date."' then a.store_amount else 0 end) as rcv_total_amt,
			sum(case when a.transaction_type in(2,3,6) and a.transaction_date between '".$from_date."' and '".$to_date."' then a.store_amount else 0 end) as iss_total_amt,
		...
		if ($cbo_item_category_id==0) $item_category_cond=" and item_category in(5,6,7,23)"; else $item_category_cond=" and item_category='$cbo_item_category_id'";
		if ($cbo_store_name != "")  $store_cond=" and store_id in($cbo_store_name) ";
		$returnRes_date="select PROD_ID, max(transaction_date) as MAX_DATE from inv_transaction where is_deleted=0 and status_active=1 and company_id=$cbo_company_name $item_category_cond $store_cond group by prod_id";
		$result_returnRes_date = sql_select($returnRes_date);
		foreach($result_returnRes_date as $row)
		{
			$date_array[$row["PROD_ID"]]['max_date']=$row["MAX_DATE"];
	...
	else if($report_type==5)
	{
		...
		if($variable_store_wise_rate==1)
		{
			...
			sum(case when a.transaction_type in(2,3,6) and a.transaction_date<'".$from_date."' then a.store_amount else 0 end) as iss_total_opening_amt,
			sum(case when a.transaction_type=1 and a.transaction_date between '".$from_date."' and '".$to_date."' then a.cons_quantity else 0 end) as purchase,
			sum(case when a.transaction_type=1 and a.transaction_date between '".$from_date."' and '".$to_date."' then a.store_amount else 0 end) as purchase_amt,
			sum(case when a.transaction_type=2 and a.transaction_date between '".$from_date."' and '".$to_date."' then a.cons_quantity else 0 end) as issue,
			sum(case when a.transaction_type=2 and a.transaction_date between '".$from_date."' and '".$to_date."' then a.store_amount else 0 end) as issue_amt,
			sum(case when a.transaction_type in(4) and a.transaction_date between '".$from_date."'  and '".$to_date."' then a.cons_quantity else 0 end) as issue_return,
			sum(case when a.transaction_type in(4) and a.transaction_date between '".$from_date."'  and '".$to_date."' then a.store_amount else 0 end) as issue_return_amt,
			sum(case when a.transaction_type in(3) and a.transaction_date between '".$from_date."'  and '".$to_date."' then a.cons_quantity else 0 end) as receive_return,
			sum(case when a.transaction_type in(3) and a.transaction_date between '".$from_date."'  and '".$to_date."' then a.store_amount else 0 end) as receive_return_amt,

			sum(case when a.transaction_type in(6) and a.transaction_date between '".$from_date."'  and '".$to_date."' then a.cons_quantity else 0 end) as item_transfer_issue,
			sum(case when a.transaction_type in(6) and a.transaction_date between '".$from_date."'  and '".$to_date."' then a.store_amount else 0 end) as item_transfer_issue_amt,
			sum(case when a.transaction_type in(5) and a.transaction_date between '".$from_date."'  and '".$to_date."' then a.cons_quantity else 0 end) as item_transfer_receive,
			sum(case when a.transaction_type in(5) and a.transaction_date between '".$from_date."'  and '".$to_date."' then a.store_amount else 0 end) as item_transfer_receive_amt,
			sum(case when a.transaction_type in(1,4) and a.receive_basis in(1,2) then a.cons_quantity else 0 end) as rcv_total_wo,
			listagg(cast(a.batch_lot as varchar(4000)), ',') within group (order by a.batch_lot)  as lot_no
			from inv_transaction a, product_details_master b
		...
		//print_r($data_array);die;
		if ($cbo_store_name != "")  $store_cond=" and store_id in($cbo_store_name) ";
		$returnRes_date="select PROD_ID, max(transaction_date) as MAX_DATE from inv_transaction where is_deleted=0 and status_active=1 and item_category in(5,6,7,23) and company_id=$cbo_company_name $store_cond group by prod_id";
		$result_returnRes_date = sql_select($returnRes_date);
		foreach($result_returnRes_date as $row)
		{
			$date_array[$row["PROD_ID"]]['max_date']=$row["MAX_DATE"];
	...
	else if($report_type==6)
	{
		...
		if($variable_store_wise_rate==1)
		{
			$trans_sql="SELECT b.id as PROD_ID, a.transaction_date as TRANSACTION_DATE, a.transaction_type as TRANSACTION_TYPE, a.cons_quantity as CONS_QUANTITY, a.store_amount as CONS_AMOUNT, a.batch_lot as LOT_NO 
			from inv_transaction a, product_details_master b
			where a.prod_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $company_id $group_id $prod_cond $store_id $item_account $search_cond $item_category_id $item_category_cond 
			order by b.id, a.transaction_date ASC";
		}
		else
		{
			$trans_sql="SELECT b.id as PROD_ID, a.transaction_date as TRANSACTION_DATE, a.transaction_type as TRANSACTION_TYPE, a.cons_quantity as CONS_QUANTITY, a.cons_amount as CONS_AMOUNT, a.batch_lot as LOT_NO 
			from inv_transaction a, product_details_master b
			where a.prod_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $company_id $group_id $prod_cond $store_id $item_account $search_cond $item_category_id $item_category_cond 
			order by b.id, a.transaction_date ASC";
		}
		...
		if ($cbo_item_category_id==0) $item_category_cond=" and item_category in(5,6,7,23)"; else $item_category_cond=" and item_category='$cbo_item_category_id'";
		if ($cbo_store_name != "")  $store_cond=" and store_id in($cbo_store_name) ";
		$returnRes_date="select prod_id as PROD_ID, batch_lot as LOT, min(transaction_date) as MIN_DATE, max(transaction_date) as MAX_DATE from inv_transaction where is_deleted=0 and status_active=1 and company_id=$cbo_company_name $item_category_cond $store_cond group by prod_id, batch_lot";
		$result_returnRes_date = sql_select($returnRes_date);
		foreach($result_returnRes_date as $row)
		{
		...
		//## bellow query based on rcv transaction balance quantity in transaction table
		$age_sql = "select PROD_ID, batch_lot as LOT, min(transaction_date) as MIN_DATE from inv_transaction where is_deleted=0 and status_active=1 and transaction_type in(1,4,5) and BALANCE_QNTY>0 and company_id=$cbo_company_name $item_category_cond $store_cond group by prod_id, batch_lot";
		$age_sql_result = sql_select($age_sql);
		foreach ($age_sql_result as $row) {
    ...
    else if($report_type==7)
    {
		...
		if($variable_store_wise_rate==1)
		{
			$trans_sql="SELECT b.id as PROD_ID, a.transaction_date as TRANSACTION_DATE, a.transaction_type as TRANSACTION_TYPE, a.cons_quantity as CONS_QUANTITY, a.store_amount as CONS_AMOUNT, a.batch_lot as LOT_NO, a.receive_basis as RECEIVE_BASIS, a.STORE_ID 
			from inv_transaction a, product_details_master b
			where a.prod_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $company_id $group_id $prod_cond $store_id $item_account $search_cond $item_category_id $item_category_cond 
		...
		{
			$trans_sql="SELECT b.id as PROD_ID, a.transaction_date as TRANSACTION_DATE, a.transaction_type as TRANSACTION_TYPE, a.cons_quantity as CONS_QUANTITY, a.cons_amount as CONS_AMOUNT, a.batch_lot as LOT_NO, a.receive_basis as RECEIVE_BASIS, a.STORE_ID 
			from inv_transaction a, product_details_master b
			where a.prod_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $company_id $group_id $prod_cond $store_id $item_account $search_cond $item_category_id $item_category_cond 
			order by b.id,a.id ASC";
		}
        ...
        if ($cbo_item_category_id==0) $item_category_cond=" and item_category in(5,6,7,23)"; else $item_category_cond=" and item_category='$cbo_item_category_id'";
		if ($cbo_store_name != "")  $store_cond=" and store_id in($cbo_store_name) ";
        $returnRes_date="select prod_id as PROD_ID, min(transaction_date) as MIN_DATE, max(transaction_date) as MAX_DATE from inv_transaction where is_deleted=0 and status_active=1 and company_id=$cbo_company_name $item_category_cond $store_cond group by prod_id";
        $result_returnRes_date = sql_select($returnRes_date);
        foreach($result_returnRes_date as $row)
        {
            $date_array[$row["PROD_ID"]]['max_date']=$row["MAX_DATE"];
	...
	else if($report_type==8)
	{
		...
		if($from_date!="" && $to_date!="")
		{
			if($variable_store_wise_rate==1)
			{
				$sql_loan="Select a.prod_id as prod_id, sum(a.cons_quantity) as cons_quantity , sum(a.store_amount) as cons_amount, 1 as type
				from inv_transaction a, inv_receive_master b
				where a.mst_id=b.id and a.transaction_type=1 and b.receive_purpose=5 and a.transaction_date between '".$from_date."' and '".$to_date."' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $sql_loan_cond  group by a.prod_id
				union all
				Select a.prod_id as prod_id, sum(a.cons_quantity) as cons_quantity , sum(a.store_amount) as cons_amount, 2 as type
				from inv_transaction a, inv_issue_master b
				where a.mst_id=b.id and a.transaction_type=2 and b.issue_purpose=5 and a.transaction_date between '".$from_date."' and '".$to_date."' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $sql_loan_cond  group by a.prod_id
				order by prod_id ASC";
			}
		...
		//echo "<pre>";print_r($data_array);die;
		if ($cbo_item_category_id==0) $item_category_cond=" and item_category in(5,6,7,23)"; else $item_category_cond=" and item_category='$cbo_item_category_id'";
		$returnRes_date="select prod_id as PROD_ID, max(transaction_date) as MAX_DATE from inv_transaction where is_deleted=0 and status_active=1 and company_id=$cbo_company_name $item_category_cond group by prod_id";
		//echo $returnRes_date;die;
		$result_returnRes_date = sql_select($returnRes_date);
		foreach($result_returnRes_date as $row)
		{
		...
		$age_sql = "select PROD_ID, min(transaction_date) as MIN_DATE from inv_transaction where is_deleted=0 and status_active=1 and transaction_type in(1,4,5) and BALANCE_QNTY>0 and company_id=$cbo_company_name $item_category_cond group by prod_id";
		$age_sql_result = sql_select($age_sql);
		foreach ($age_sql_result as $row) {
			$date_array[$row["PROD_ID"]]['min_date'] = $row["MIN_DATE"];
		}
						...
						if($value_with==1)
						{
							if(number_format($closingStock,3)>0.000 || number_format($openingBalance,3)>0.000)
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" style=" word-wrap:break-word; <? echo $stylecolor; ?> "  onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
									...
									<td width="70"><p><? echo $row[csf("item_size")]; ?></p></td>
									<td width="60" align="center"><p><? echo $unit_of_measurement[$row[csf("unit_of_measure")]]; ?></p></td>
									<td width="100" align="right"><p><? if(number_format($openingBalance,3) !=0) echo number_format($openingRate,3); else echo "0.00"; ?></p></td>
									<td width="100" align="right"><p><? echo number_format($openingBalance,3); $tot_open_bl+=$openingBalance; ?></p></td>
									<td width="110" align="right"><p>
									<? if(number_format($openingBalance,3) !=0)
									{ 
										echo number_format($openingBalanceValue,3); 
										$tot_open_bl_amt+=$openingBalanceValue; 
									}
									else echo "0.00";
									?></p></td>
	...
	else if($report_type==9)
	{
		...
		if($variable_store_wise_rate==1)
		{
			$trans_sql="SELECT b.id as PROD_ID, a.transaction_date as TRANSACTION_DATE, a.transaction_type as TRANSACTION_TYPE, a.cons_quantity as CONS_QUANTITY, a.store_amount as CONS_AMOUNT, a.batch_lot as LOT_NO, a.receive_basis as RECEIVE_BASIS, a.STORE_ID, b.ITEM_CATEGORY_ID, b.ITEM_GROUP_ID, b.SUB_GROUP_NAME, b.ITEM_DESCRIPTION, b.ITEM_SIZE, b.UNIT_OF_MEASURE, b.is_compliance as IS_COMPLIANCE 
			from inv_transaction a, product_details_master b
	...
	else
	{
		...
		if($variable_store_wise_rate==1)
		{
			$trans_sql="SELECT b.id as PROD_ID, a.transaction_date as TRANSACTION_DATE, a.transaction_type as TRANSACTION_TYPE, a.cons_quantity as CONS_QUANTITY, a.store_amount as CONS_AMOUNT, a.batch_lot as LOT_NO, a.receive_basis as RECEIVE_BASIS, a.STORE_ID ,a.expire_date as EXPIRE_DATE, a.manufacture_date as MANUFACTURE_DATE
			from inv_transaction a, product_details_master b
			where a.prod_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $company_id $group_id $prod_cond $store_id $item_account $search_cond $item_category_id $item_category_cond $item_account_cond 
			order by b.id,a.id ASC";
		}
		...
		{
			$trans_sql="SELECT b.id as PROD_ID, a.transaction_date as TRANSACTION_DATE, a.transaction_type as TRANSACTION_TYPE, a.cons_quantity as CONS_QUANTITY, a.cons_amount as CONS_AMOUNT, a.batch_lot as LOT_NO, a.receive_basis as RECEIVE_BASIS, a.STORE_ID ,a.expire_date as EXPIRE_DATE, a.manufacture_date as MANUFACTURE_DATE
			from inv_transaction a, product_details_master b
			where a.prod_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $company_id $group_id $prod_cond $store_id $item_account $search_cond $item_category_id $item_category_cond $item_account_cond 
			order by b.id,a.id ASC";
		}
		...
		foreach($trnasactionData as $row_p)
		{
			...
			if($row_p["MANUFACTURE_DATE"]){ $data_array[$row_p["PROD_ID"]]['manufacture_date']=$row_p["MANUFACTURE_DATE"];}
			if($row_p["TRANSACTION_TYPE"]==1)
			{
				if($row_p["TRANSACTION_DATE"]){ $data_array[$row_p["PROD_ID"]]['transaction_date']=$row_p["TRANSACTION_DATE"];}
			}
			if($row_p["EXPIRE_DATE"]){ $data_array[$row_p["PROD_ID"]]['expire_date']=$row_p["EXPIRE_DATE"];}
		}
		unset($trnasactionData);
		//echo "test2";die;
		//var_dump($data_array);die;
		...
		<div align="center" style="height:auto; width:auto; margin:0 auto; padding:0; width:2248">
			...
			<table  border="1" cellpadding="2" cellspacing="0" class="rpt_table" width="<?=$tbl_width;?>" rules="all" id="rpt_table_header">
				<thead>
					<tr>
						...
						?>
						<th rowspan="2" width="60">DOH</th>
						<th width="60" rowspan="2">Age (Days)</th>
						<th rowspan="2" width="100">Pipe Line</th>
						<th width="150" rowspan="2">Lot</th>
						<th width="70" rowspan="2">Manufacture <br> Date</th>
						<th width="70" rowspan="2"> Exp. Date</th>
						<th width="60" rowspan="2">Age (Days) Based on MFG Date</th>
						<th width="60" rowspan="2">Age (Days) Based on Rcv Date</th>
                        <th width = "60" rowspan="2">Zero Discharge</th>
					</tr>
					<tr>
						<th width="60">Prod. ID</th>
						<th width="70">Item Code</th>
			...
			<?
				...
				foreach($result as $row)
				{
					...
					$manf_date = change_date_format($data_array[$row[csf("id")]]['manufacture_date']);
					$age_diff = datediff("d", date($manf_date), date("Y-m-d"));
					
					$manf_date_rcv = change_date_format($data_array[$row[csf("id")]]['transaction_date']);
					$age_diff_rcv = datediff("d", date($manf_date_rcv), date("Y-m-d"));
						...
						if($value_with==1)
						{
							if(number_format($closingStock,2)>0.00 || number_format($openingBalance,2)>0.00 )
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" style=" word-wrap:break-word; <? echo $stylecolor; ?> "  onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
									...
									<td width="80" class="wrd_brk" align="right"><p><? if(number_format($closingStock,4)>0) echo number_format($avg_rate,4); else echo "0.00"; //number_format($row[csf("avg_rate")],2); $tot_avg_rate+=$row[csf("avg_rate")]; ?></p></td>
									<td width="120" class="wrd_brk" align="right"><p><? if(number_format($closingStock,4)>0) echo number_format($stockValue,3); else echo "0.00"; $tot_stock_value+=$stockValue; ?></p></td>
									<td width="80" class="wrd_brk" align="right"><p><? if(number_format($avg_rate,4)>0 && $txt_excenge_rate>0) echo number_format($avg_rate/$txt_excenge_rate,4); else echo "0.00";?></p></td>
									<td width="120" class="wrd_brk" align="right"><p>
									<? 
									if(number_format($stockValue,4)>0 && $txt_excenge_rate>0)
									{ 
										echo number_format($stockValue/$txt_excenge_rate,3);
										$tot_stock_value_usd+=$stockValue/$txt_excenge_rate;
									}
									else 
									{
										echo "0.00"; 
									...
									<td width = "70" align="center"><p style="mso-number-format:\@"><?									
									echo ($data_array[$row[csf("id")]]['manufacture_date']);?></p></td>
									<td width = "70" align="center"><p style="mso-number-format:\@"><? echo ($data_array[$row[csf("id")]]['expire_date']);?></p></td>
									<td width = "60" align="center"><p><? echo $age_diff;?></p></td>
									<td width = "60" align="center"><p><? echo $age_diff_rcv;?></p></td>
						...
						else
						{
							if(number_format($closingStock,3)>0.000 || number_format($openingBalance,3)>0.000 || number_format($totalReceive,3)>0.000 || number_format($totalIssue,3)>0.000)
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" style=" word-wrap:break-word; <? echo $stylecolor; ?> "  onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
									...
									<td width="150" class="wrd_brk"><p><? echo chop($data_array[$row[csf("id")]]['lot_no'],","); ?></p></td>
									<td width = "70" align="center"><p style="mso-number-format:\@"><?
									echo change_date_format($data_array[$row[csf("id")]]['manufacture_date']);?></p></td>
									<td width = "70" align="center"><p style="mso-number-format:\@"><? echo change_date_format($data_array[$row[csf("id")]]['expire_date']);?></p></td>
									<td width = "60" align="center"><p><? echo $age_diff;?></p></td>
									<td width = "60" align="center"><p><? echo $age_diff_rcv;?></p></td>
                                    <td width = "60"  class="wrd_brk" align="center"><p><? echo $compliance_arr[$row[csf("is_compliance")]]; ?></p></td>
								</tr>								
								<?
								$i++;
								$totalStockValue+=$stockValue;
