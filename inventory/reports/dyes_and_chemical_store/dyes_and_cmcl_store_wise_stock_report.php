...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../logout.php";
	...
	function generate_report(operation)
	{
		...
		var dataString = "&cbo_company_name="+cbo_company_name+"&cbo_item_group="+cbo_item_group+"&cbo_item_category_id="+cbo_item_category_id+"&txt_date="+txt_date+"&txt_item_account_id="+txt_item_account_id+"&txt_item_acc="+txt_item_acc+"&report_title="+report_title;
		...
		//alert (data);
		freeze_window(3);
		http.open("POST","requires/dyes_and_cmcl_store_wise_stock_report_controller.php",true);
	...
	function getItemId()
	{
		...
		load_drop_down( 'requires/dyes_and_cmcl_store_wise_stock_report_controller', cbo_company_name+'_'+cbo_item_category_id, 'load_multiselect_item_group', 'item_td' );
		set_multiselect('cbo_item_group','0','0','','');
	}
	...
	{
		var width=750;
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/dyes_and_cmcl_store_wise_stock_report_controller.php?action='+action+'&pi_data='+pi_data+'&prod_id='+prod_id+'&porduct_data_all='+porduct_data_all, 'PI Details', 'width='+width+',height=400px,center=1,resize=0,scrolling=0','../../');
	}
...
<body onLoad="set_hotkey();">
    ...
    <? echo load_freeze_divs ("../../../",$permission);  ?><br />    		 
    	<form name="dyes_cmcl_smry_rpt" id="dyes_cmcl_smry_rpt" autocomplete="off" > 
    		<h3 style="width:870px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3> 
     		<div id="content_search_panel" style="width:870px">      
	        	<fieldset>  
	            	<table class="rpt_table" width="870" cellpadding="0" cellspacing="0" border="1" rull="all">
	                	<thead>
	                	...
	                	<tbody>
	                    	<tr class="general">
	                    		...
	                    		<td>
	                            	...
	                            	<input type="button" name="search" id="search2" value="Show 2" onClick="generate_report(2)" style="width:70px" class="formbutton" />
	                        	</td>
	                    	</tr>
	                	</tbody>
	                	<tfoot>
...
<script>
	set_multiselect('cbo_company_name*cbo_item_category_id*cbo_item_group','0*0*0','0*0*0','','0*0*0');
	setTimeout[($("#cat_td a").attr("onclick","disappear_list(cbo_item_category_id,'0');getItemId();") ,3000)];
</script> 
</html>
