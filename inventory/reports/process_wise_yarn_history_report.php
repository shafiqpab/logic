...
<script>
	...
	function generate_report(rpt_type)
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_buyer_id*cbo_shipment_status*txt_job_no*txt_job_id*txt_style_no*txt_order_no*txt_order_id*txt_date_from*txt_date_to',"../../")+'&report_title='+report_title+'&rpt_type='+rpt_type;
		//alert (data);
		freeze_window(3);
		http.open("POST","requires/process_wise_yarn_history_report_controller.php",true);
	...
	function openmypage_job()
	{
		...
		var page_link='requires/process_wise_yarn_history_report_controller.php?action=job_no_popup&companyID='+companyID+'&buyer_name='+buyer_name;
	...
	function openmypage_style()
	{
		...
		var page_link='requires/process_wise_yarn_history_report_controller.php?action=style_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&txt_job_no='+txt_job_no;
	...
	function openmypage_order()
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/process_wise_yarn_history_report_controller.php?action=order_no_popup&data='+data,'Order No Popup', 'width=650px,height=420px,center=1,resize=0','../')
	...
	function open_yarn_popup(po_id)
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/process_wise_yarn_history_report_controller.php?action=grey_yarn_issue_popup&po_id='+po_id+get_submitted_data_string('cbo_company_id*cbo_buyer_id*cbo_shipment_status*txt_job_no*txt_job_id*txt_style_no*txt_order_no*txt_order_id*txt_date_from*txt_date_to',"../../"),'Grey Yarn Issue Popup', 'width=350px,height=420px,center=1,resize=0','../')
...
<div style="width:100%;" align="center">
    ...
    <h3 style="width:980px; margin-top:10px;" align="center" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3>
            ...
            <fieldset style="width:980px;">
                <table class="rpt_table" width="980" cellpadding="0" cellspacing="0" border="1" rules="all">
                    ...
                    <tr class="general">
                        <td>
                            <? 
                               echo create_drop_down( "cbo_company_id", 140, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/process_wise_yarn_history_report_controller',this.value+'_'+1+'_'+4, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>                            
                        </td>
                        <td id="buyer_td"> 
                            <?
                        ...
                        <td> 
                            <?
								echo create_drop_down( "cbo_shipment_status", 120,$shipment_status,"", 1,"-- All --", $selected, "",0,"" );
                            ?>
                        </td>
