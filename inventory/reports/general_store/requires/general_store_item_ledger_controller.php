...
$buyerArr = return_library_array("select id,BUYER_NAME from LIB_BUYER where status_active in(1,3) and is_deleted=0","id","BUYER_NAME");
...
if($action=="create_lot_search_list_view")
{
	...
	$txt_item_number = str_replace("'","",$ex_data[6]);
	...
	if($txt_item_number != '') $sql_cond .=" and item_number='$txt_item_number'";
	//echo $sql_cond.'rakib';die;

 	$sql = "select id,item_group_id,item_description as product_name_details,item_code,item_number,item_size from product_details_master where company_id=$company and item_category_id=$cbo_item_cat $sql_cond and status_active=1 and is_deleted=0";
	$arr=array(1=>$item_group_arr);
	echo create_list_view("list_view", "Product Id, Item Group, Item Number, Item Code, Item Description,Item Size","70,160,100,100,100","700","260",0, $sql , "js_set_value", "id,product_name_details", "", 1, "0,item_group_id,0,0,0,0", $arr, "id,item_group_id,item_number,item_code,product_name_details,item_size", "","","0","",1);
	exit();
}
...
if($action=="generate_report")
{
	...
	// issue MRR array------------------------------------------------
	$sql_issue_mrr = "select a.id as trid,a.transaction_type,b.issue_number,b.issue_purpose	from inv_transaction a, inv_issue_master b
	...
	if($cbo_store_name>0) $store_cond=" and store_id=$cbo_store_name";
	if( $from_date!="" && $to_date!="" )
	{
		...
		if($variable_store_wise_rate==1)
		{
			...
			where transaction_date < '".$from_date."' and status_active=1 and is_deleted=0 $store_cond group by prod_id";
		}
		else
		{
			$sqlTR = "select  prod_id, SUM(CASE WHEN transaction_type in (1,4,5) THEN cons_quantity ELSE 0 END) as receive,
	...
	if($cbo_method==0) //average rate #########################################################################
	{
		if($variable_store_wise_rate==1)
		{
			$sql = "select a.id, a.mst_id, a.receive_basis, a.insert_date, a.prod_id,a.store_id,a.store_id, a.transaction_date, a.transaction_type, a.cons_quantity, a.store_rate as cons_rate, a.store_amount as cons_amount, a.department_id, b.item_description as product_name_details, b.unit_of_measure, b.item_group_id, b.sub_group_name, a.batch_lot, a.remarks, c.knit_dye_source, c.knit_dye_company, c.issue_purpose, c.challan_no as issuechallan, d.challan_no as receivechallan,d.challan_date
			from inv_transaction a left join inv_issue_master c on a.mst_id=c.id and a.transaction_type in (2,4,6) left join inv_receive_master d on a.mst_id=d.id and a.transaction_type in (1,3,5) left join product_details_master b on a.prod_id=b.id
			where a.status_active=1 and a.is_deleted=0 $item_desc_search_cond  and a.item_category='$cbo_item_cat' and a.company_id=$cbo_company_name $search_cond $store_cond order by a.prod_id, a.insert_date, a.id ASC";
		}
		else
		{
			$sql = "select a.id, a.mst_id, a.receive_basis, a.insert_date, a.prod_id,a.store_id,a.store_id, a.transaction_date, a.transaction_type, a.cons_quantity, a.cons_rate, a.cons_amount, a.department_id, b.item_description as product_name_details, b.unit_of_measure, b.item_group_id, b.sub_group_name, a.batch_lot, a.remarks, c.knit_dye_source, c.knit_dye_company, c.issue_purpose, c.challan_no as issuechallan, d.challan_no as receivechallan,d.challan_date
			from inv_transaction a left join inv_issue_master c on a.mst_id=c.id and a.transaction_type in (2,4,6) left join inv_receive_master d on a.mst_id=d.id and a.transaction_type in (1,3,5) left join product_details_master b on a.prod_id=b.id
			where a.status_active=1 and a.is_deleted=0 $item_desc_search_cond  and a.item_category='$cbo_item_cat' and a.company_id=$cbo_company_name $search_cond $store_cond order by a.prod_id, a.insert_date, a.id ASC";
		}
	}
	else if($cbo_method==1) //FIFU #########################################################################
	{
		if($variable_store_wise_rate==1)
		{
			$sql = "select a.id, a.mst_id, a.receive_basis, a.insert_date, a.prod_id,a.store_id, a.transaction_date, a.transaction_type, a.cons_quantity, a.store_rate as cons_rate, a.store_amount as cons_amount,  a.department_id, b.item_description as product_name_details, b.unit_of_measure, b.item_group_id, b.sub_group_name, a.batch_lot, a.remarks, c.knit_dye_source, c.knit_dye_company, c.issue_purpose, c.challan_no as issuechallan, d.challan_no as receivechallan,d.challan_date
			from inv_transaction a left join inv_issue_master c on a.mst_id=c.id and a.transaction_type in (2,4,6) left join inv_receive_master d on a.mst_id=d.id and a.transaction_type in (1,3,5) left join product_details_master b on a.prod_id=b.id
			where a.status_active=1 and a.is_deleted=0 $item_desc_search_cond  and a.item_category='$cbo_item_cat' and a.company_id=$cbo_company_name $search_cond $store_cond order by a.prod_id, a.insert_date, a.id ASC";
		}
		else
	...
	else if($cbo_method==2) //LIFU #########################################################################
	{
		if($variable_store_wise_rate==1)
		{
			$sql = "select a.id, a.mst_id, a.receive_basis, a.insert_date, a.prod_id,a.store_id, a.transaction_date, a.transaction_type, a.cons_quantity, a.store_rate as cons_rate, a.store_amount as cons_amount,  a.department_id, b.item_description as product_name_details, b.unit_of_measure, b.item_group_id, b.sub_group_name, a.batch_lot, a.remarks, c.knit_dye_source, c.knit_dye_company, c.issue_purpose, c.challan_no as issuechallan, d.challan_no as receivechallan,d.challan_date
			from inv_transaction a left join inv_issue_master c on a.mst_id=c.id and a.transaction_type in (2,4,6) left join inv_receive_master d on a.mst_id=d.id and a.transaction_type in (1,3,5) left join product_details_master b on a.prod_id=b.id
			where a.status_active=1 and a.is_deleted=0 and  $item_desc_search_cond  and a.item_category='$cbo_item_cat' and a.company_id=$cbo_company_name $search_cond $store_cond
	...
	$issue_query = "SELECT a.req_no,a.issue_number,b.prod_id from inv_issue_master a, inv_transaction b WHERE a.id=b.mst_id and b.prod_id in($txt_product_id)";
	foreach(sql_select($issue_query) as $row)
	{
		$issueArrReq[$row[csf('issue_number')]]['req_no'] = $row[csf('req_no')];
	}
	
	$receive_query="SELECT a.challan_no,a.recv_number,b.prod_id from inv_receive_master a, inv_transaction b WHERE a.id=b.mst_id and b.prod_id in($txt_product_id)";
	echo "<pre>";
		//print_r(sql_select($receive_query));
	echo "</pre>";
	foreach(sql_select($receive_query) as $row)
	{
		$receiveArrChallan[$row[csf('recv_number')]]['challan_no'] = $row[csf('challan_no')];
	}
	echo "<pre>";
		//print_r($receiveArrChallan);
	echo "</pre>";
	...
	if(str_replace("'","",$cbo_item_cat)==22)
	{
		$width="1920";
		$table_width="1900";
	}
	...
	<fieldset>
		<div>
			<table width="<?= $width;?>" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" id="table_header_1" >
				<thead>
					...
					<tr>
						...
						<td colspan="10" align="center"><b>Weighted Average Method</b></td>
					</tr>
					<tr>
						...
						<th width="100" rowspan="2">Challan/Req. No.</th>
						<th width="100" rowspan="2">Challan Date.</th>
			...
			<table width="<?=$width?>" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" id="table_body">
				...
				foreach($result as $row)
				{
					...
					if(!in_array($row[csf("prod_id")],$checkItemArr))
					{
						...
						if($i!=1) // product wise sum/total here------------
						{
							?>
							<tr class="tbl_bottom">
								<td colspan="<? if(str_replace("'","",$cbo_item_cat)==22) echo "10"; else echo "9";?>" align="right">Total</td>
								...
								<!-- product wise herder -->
							<tr>
								<td colspan="<? if(str_replace("'","",$cbo_item_cat)==17) echo "9"; else echo "16";?>"><b>Product ID : <? echo $row[csf("prod_id")]." , ".$row[csf("product_name_details")].", Item Group# ".$item_group_arr[$row[csf("item_group_id")]].", Item Sub Group# ".$row[csf("sub_group_name")].", Item Category#  ".$general_item_category[$cbo_item_cat].", UOM# ".$unit_of_measurement[$row[csf("unit_of_measure")]]; ?></b></td>
							</tr>
						...
						if($opning_bal_arr[$pro_id]['prod_id']!="")
						{
							...
							<tr style="background-color:#FFFFCC">
								<td colspan="<? if(str_replace("'","",$cbo_item_cat)==22) echo "15"; else echo "15";?>" align="right"><b>Opening Balance</b></td>
								<?
					...
					if($i==1)
					{
						?>
							<tr>
								<td colspan="<? if(str_replace("'","",$cbo_item_cat)==22) echo "21"; else echo "20";?>"><b>Product ID : <? echo $row[csf("prod_id")]." , ".$row[csf("product_name_details")].", Item Group# ".$item_group_arr[$row[csf("item_group_id")]].", Item Sub Group# ".$row[csf("sub_group_name")].", Item Category#  ".$general_item_category[$cbo_item_cat].", UOM# ".$unit_of_measurement[$row[csf("unit_of_measure")]]; ?></b></td>
							</tr>
						<?
					}
					...
					if(!in_array($row[csf("prod_id")],$product_id_arr))
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>" <? echo $stylecolor; ?> onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
							...
							?>
							<td width="80" align="center"><p><? if($row[csf("transaction_date")] !="" && $row[csf("transaction_date")] !="0000-00-00")  echo change_date_format($row[csf("transaction_date")]); ?>&nbsp;</p></td>
							<td width="120"><p>
							<?
							//, a.mst_id, a.receive_basis
							...
							<td width="100">
								<p>
									<? 
										$trans_Ref=$mrrArray[$row[csf("id")]."##".$row[csf("transaction_type")]];
										echo  $receiveArrChallan[$trans_Ref]['challan_no']; 
										//echo $row[csf("transaction_type")] == 2 || $row[csf("transaction_type")] == 4 || $row[csf("transaction_type")] == 6 ? $row[csf("issuechallan")] : $row[csf("receivechallan")] 
									?>
								</p>
							</td>
							...
							<td width="100"><p><? echo change_date_format($row[csf("challan_date")]) ?></p></td>
							...
							<td width="80" align="right" title="<? if($row[csf("transaction_type")]==1 || $row[csf("transaction_type")]==4 || $row[csf("transaction_type")]==5) echo $row[csf("cons_rate")] ; ?>"><? if($row[csf("transaction_type")]==1 || $row[csf("transaction_type")]==4 || $row[csf("transaction_type")]==5) echo number_format($row[csf("cons_rate")],4); ?></td>
							<td width="90" align="right"><? if($row[csf("transaction_type")]==1 || $row[csf("transaction_type")]==4 || $row[csf("transaction_type")]==5) echo number_format($row[csf("cons_amount")],2); ?></td>
							...
							<td width="80" align="right" title="<? if($row[csf("transaction_type")]==2 || $row[csf("transaction_type")]==3  || $row[csf("transaction_type")]==6) echo $row[csf("cons_rate")]; ?>"><? if($row[csf("transaction_type")]==2 || $row[csf("transaction_type")]==3  || $row[csf("transaction_type")]==6) echo number_format($row[csf("cons_rate")],4); ?></td>
							<td width="90" align="right"><? if($row[csf("transaction_type")]==2 || $row[csf("transaction_type")]==3 || $row[csf("transaction_type")]==6) echo number_format($row[csf("cons_amount")],2); ?></td>
							<?
							$each_pro_id=array();
							...
							if($row[csf("transaction_type")]==1 || $row[csf("transaction_type")]==4 || $row[csf("transaction_type")]==5)  $total_balValue = number_format($balValue[$row[csf("prod_id")]],8,'.','')+ number_format(trim($row[csf("cons_amount")]),6,'.','');
							if($row[csf("transaction_type")]==2 || $row[csf("transaction_type")]==3 || $row[csf("transaction_type")]==6)  $total_balValue =number_format($balValue[$row[csf("prod_id")]],8,'.','')- number_format(trim($row[csf("cons_amount")]),6,'.','');
							...
							{
								if($total_balValue!=0 && $total_balQnty !=0) $bal_rate=$total_balValue/$total_balQnty; else $bal_rate=0;
							}
							...
							<td width="80" style="word-break:break-all" align="right" title="<?= $bal_rate;?>"><? echo number_format($bal_rate,4,'.',''); ?></td>
							<td width="90" align="right" title="<?= $total_balValue;?>"><? echo number_format($total_balValue,2,'.',''); ?></td>
							...
							<td width="100">
								<p>
									<? 
										...
										$trans_Ref=$mrrArray[$row[csf("id")]."##".$row[csf("transaction_type")]]; 
										if($issueArrReq[$trans_Ref]['req_no']!=null){
											echo  $issueArrReq[$trans_Ref]['req_no']; 
										}
										else{
											echo  $receiveArrChallan[$trans_Ref]['challan_no']; 
										}
									
										//$row[csf("transaction_type")] == 2 || $row[csf("transaction_type")] == 4 || $row[csf("transaction_type")] == 6 ? $row[csf("issuechallan")] : $row[csf("receivechallan")] 
									?>
								</p>
							</td>
				...
				?>
					<tr class="tbl_bottom">
						<td colspan="<? if(str_replace("'","",$cbo_item_cat)==22) echo "11"; else echo "10";?>" align="right">Total</td>
