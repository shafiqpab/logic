...
if ($action=="load_drop_down_purpose")
{
	if($data==1) 
	{
		$arr_field_index="43,4,8";
		$select_field="43";
	}
	elseif($data==2) 
	{
		$arr_field_index="4,7,8,12,15,38,46"; 
		$select_field="";
	}
	elseif($data==6) 
	{
	...
	echo create_drop_down( "cbo_receive_purpose", 170, $yarn_issue_purpose,"", 1, "-- Select Purpose --", $select_field, "load_supplier(); rate_cond(this.value); change_placeholder(this.value)", 0,"$arr_field_index");     	 
	exit();
}
...
if ($action == "load_drop_down_company_from_eheck_wo_paymode") {
	...
	{
		// create_drop_down("cbo_supplier", 170, "select a.id, a.company_name from lib_company a,wo_yarn_dyeing_mst b where a.id=b.supplier_id and b.ydw_no='$data[2]' and b.entry_form=335 and b.service_type in(7,12,15,38,46) and b.pay_mode=$data[3] and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.company_name","id,company_name", 0, "-- Select --", 1, "", 0);
		echo create_drop_down("cbo_supplier", 170, "select a.id, a.company_name from lib_company a, wo_yarn_dyeing_mst b where a.id=b.supplier_id and b.id = '".$data[2]."' and b.entry_form=335 and b.service_type in(7,12,15,38,46) and b.pay_mode = ".$data[3]." and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.company_name","id,company_name", 0, "-- Select --", 1, "", 0);
	}
	else
	{
		if($data[0]==1)
		{
			//$sql="select a.id, a.supplier_name from lib_supplier a, com_pi_master_details b where a.id=b.supplier_id and b.pi_number='$data[2]' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.supplier_name";
			
			$sql="select a.id, a.supplier_name from lib_supplier a, com_pi_master_details b where a.id=b.supplier_id and b.id='".$data[2]."' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.supplier_name";
		}
		else if($data[0]==2)
		{
			//$sql="select a.id, a.supplier_name from lib_supplier a, wo_yarn_dyeing_mst b where a.id=b.supplier_id and b.ydw_no='$data[2]' and b.entry_form=335 and b.service_type in(7,12,15,38,46) and b.pay_mode=$data[3] and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.supplier_name";
			$sql="select a.id, a.supplier_name from lib_supplier a, wo_yarn_dyeing_mst b where a.id=b.supplier_id and b.id='".$data[2]."' and b.entry_form=335 and b.service_type in(7,12,15,38,46) and b.pay_mode=".$data[3]." and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.supplier_name";
		}
		else
		{
			$sql="select a.id, a.supplier_name from lib_supplier a where a.status_active=1 and a.is_deleted=0 order by a.supplier_name";
		}
...
if ($action == "load_drop_down_supplier_from_issue") {
	echo create_drop_down("cbo_supplier", 170, "select c.supplier_name,c.id from lib_supplier_tag_company a,lib_supplier_party_type b, lib_supplier c, inv_issue_master d where c.id=b.supplier_id and a.supplier_id = b.supplier_id and c.id=d.knit_dye_company and d.knit_dye_source=3 and d.issue_purpose=15 and d.entry_form=3 and a.tag_company='$data' and b.party_type in(2,93,94) and c.status_active=1 and c.is_deleted=0 group by c.id, c.supplier_name", "id,supplier_name", 1, "-- Select --", 0, "", 0);
	exit();
}
...
/*if ($action == "load_drop_down_store") {
	...
	echo create_drop_down("cbo_store_name", 170, "select a.id, a.store_name from lib_store_location a, lib_store_location_category b where a.id= b.store_location_id and a.company_id='$data' $user_store_cond and b.category_type=1 and a.status_active=1 and a.is_deleted=0 group by a.id, a.store_name order by a.store_name", "id,store_name", 1, "-- Select --", 0, "", 0);
...
if ($action == "load_drop_down_color") {
	...
	echo create_drop_down("cbo_color", 110, $sql, "id,color_name", 1, "--Select--", 0, "", 1);
            ...
            <table width="600" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr class="general">
                        ...
                        <td align="center">
                        	<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view ('<? echo $companyID; ?>'+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('cbo_year').value, 'create_job_no_search_list_view', 'search_div', 'yarn_receive_controller', 'setFilterGrid(\'tbl_list_search\',-1)');" style="width:80px;" />
                    </td>
                    </tr>
            	</tbody>
           	</table>
...
if ($action == "wopi_popup") {
	...
	<script>
		function js_set_value(str) {
            ...
            $("#hidden_paymode").val(splitData[2]); // wo/pi number
            parent.emailwindow.hide();
        }
		...
		<form name="searchorderfrm_1" id="searchorderfrm_1" autocomplete="off">
			...
			<tbody>
				<tr class="general">
					...
					<td>
						<input type="text" style="width:110px" class="text_boxes" name="txt_pi_no" id="txt_pi_no"
						placeholder="write" <?php echo ($receive_basis == 1) ? "" : "disabled='disabled'"; ?> />
					</td>
					<td>
						<input type="text" style="width:110px" class="text_boxes" name="txt_lc_no" id="txt_lc_no"
						<?php echo ($receive_basis == 1) ? "" : "disabled='disabled'"; ?> placeholder="write"/>
					</td>
					<td>
						<input type="text" style="width:110px" class="text_boxes" name="txt_wo_no" id="txt_wo_no"
						<?php echo ($receive_basis == 2) ? "" : "disabled='disabled'"; ?> placeholder="write"/>
					</td>
					<td>
						<input type="text" style="width:110px" class="text_boxes" name="txt_supplier_no" id="txt_supplier_no" placeholder="Browse" onDblClick="fn_supplier(<? echo $company; ?>)"/>
					...
					<td align="center">
						<input name="txt_date_from" id="txt_date_from" class="datepicker" style="width:55px"
						placeholder="From Date"/>
						<input name="txt_date_to" id="txt_date_to" class="datepicker" style="width:55px"
						placeholder="To Date"/>
					</td>
					<td align="center">
						<input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_pi_no').value+'_'+document.getElementById('txt_lc_no').value+'_'+document.getElementById('txt_wo_no').value+'_'+document.getElementById('txt_supplier_id').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_<? echo $company; ?>'+'_<? echo $receive_purpose; ?>'+'_'+document.getElementById('cbo_year').value, 'create_wopi_search_list_view', 'search_div', 'yarn_receive_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" />
					</td>
				</tr>
				<tr>
...
if ($action == "create_wopi_search_list_view") {
	...
	if ($txt_search_by == 1) 
	{
		...
		where a.item_category_id = 1 and a.importer_id=$company and a.status_active=1 and a.is_deleted=0 $sql_cond $approval_status_cond";*/
		
		if($receive_purpose==8)
		{
			$sql = "select a.id as id, a.pi_number as wopi_number, a.pi_number as wopi_prefix, a.pi_date as wopi_date, a.supplier_id as supplier_id, a.currency_id as currency_id, a.source as source, c.lc_number as lc_number, 1 as booking_without_order
			from wo_non_order_info_mst p, com_pi_item_details q, com_pi_master_details a
			left join com_btb_lc_pi b on a.id=b.pi_id
			left join com_btb_lc_master_details c on b.com_btb_lc_master_details_id=c.id
			where p.id=q.work_order_id and q.pi_id=a.id and p.wo_basis_id=3 and a.item_category_id = 1 and a.importer_id=$company and a.status_active=1 and a.is_deleted=0 $sql_cond $approval_status_cond
			group by a.id, a.pi_number, a.pi_number, a.pi_date, a.supplier_id, a.currency_id, a.source, c.lc_number";
		}
		else
		{
			$sql = "select a.id as id, a.pi_number as wopi_number, a.pi_number as wopi_prefix, a.pi_date as wopi_date, a.supplier_id as supplier_id, a.currency_id as currency_id, a.source as source, c.lc_number as lc_number, 0 as booking_without_order
			from wo_non_order_info_mst p, com_pi_item_details q, com_pi_master_details a
			left join com_btb_lc_pi b on a.id=b.pi_id
			left join com_btb_lc_master_details c on b.com_btb_lc_master_details_id=c.id
			where p.id=q.work_order_id and q.pi_id=a.id and p.wo_basis_id <> 3 and a.item_category_id = 1 and a.importer_id=$company and a.status_active=1 and a.is_deleted=0 $sql_cond $approval_status_cond
	...
	else if ($txt_search_by == 2) 
	{
		...
		if ($receive_purpose == 2 || $receive_purpose == 7 || $receive_purpose == 12 || $receive_purpose == 15 || $receive_purpose == 38 || $receive_purpose == 46)
		{
			$sql_cond = "";
			if ($txt_date_from != "" && $txt_date_to != "") {
				if ($db_type == 0) {
			...
			if($cbo_year>0) $sql_cond .= " and to_char(insert_date,'YYYY')='$cbo_year'";
			...
			$sql = "select id, yarn_dyeing_prefix_num, ydw_no, booking_date, delivery_date, supplier_id, currency, source, pay_mode, entry_form, service_type, 1 as booking_without_order
			from wo_yarn_dyeing_mst
			where  status_active=1 and is_deleted=0 and entry_form in $entry_form $purpose and company_id='$company' $sql_cond ";
		}
		else
		{
			$sql_cond = "";
			...
			if ($txt_supplier_id != "") $sql_cond .= " and supplier_id=$txt_supplier_id";
			if($receive_purpose==4 || $receive_purpose==8)
			{
				$sql = "select id, wo_number as wopi_number, wo_number_prefix_num as wopi_prefix, ' ' as lc_number, wo_date as wopi_date, supplier_id as supplier_id, currency_id as currency_id, source as source, wo_basis_id
				from wo_non_order_info_mst
				where status_active=1 and is_deleted=0 and wo_basis_id=3 and booking_type=1 and entry_form=284 and company_name='$company' and pay_mode!=2 $sql_cond $approval_status_cond order by id";
			}
			else
			{
				...
				from wo_non_order_info_mst
				where status_active=1 and is_deleted=0 and wo_basis_id <> 3 and booking_type=0 and entry_form=234 and company_name='$company' and pay_mode!=2 $sql_cond $approval_status_cond order by id";
			}
	...
	foreach ($result as $row) {
		if($row[csf('pay_mode')]==3 && $receive_basis == 2 &&  $receive_purpose == 15)
		{
	...
	/*if ($receive_basis == 2 &&  $receive_purpose == 15) {
	...
	if (($receive_purpose == 2 || $receive_purpose == 7 || $receive_purpose == 15 || $receive_purpose == 38 || $receive_purpose == 46) && $txt_search_by == 2) 
	{
...
if ($action == "show_product_listview") {
	...
	if ($receive_basis == 2 && ($receive_purpose == 2 || $receive_purpose == 7 || $receive_purpose == 12 || $receive_purpose == 15 || $receive_purpose == 38 || $receive_purpose == 46))
	{
		$all_job_id_arr=array();
		if($receive_purpose == 15)
		{
			$sql = "select a.ydw_no, a.company_id, b.id, b.mst_id, b.dtls_id, b.job_no, b.yarn_count count, b.yarn_comp yarn_comp_type1st, b.yarn_perc yarn_comp_percent1st, b.yarn_type, b.yarn_color, b.yarn_rate, a.ecchange_rate 
			from wo_yarn_dyeing_mst a,wo_yarn_dyeing_dtls_fin_prod b 
			where a.id=b.mst_id and a.id=$wo_pi_ID and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
			$result = sql_select($sql);
			
			$yarn_dyeing_sql = "select b.mst_id, b.id, b.job_no_id, b.product_id, b.yarn_wo_qty as quantity from wo_yarn_dyeing_dtls b where b.status_active =1 and b.is_deleted=0 and b.mst_id=$wo_pi_ID";
			$yarn_dyeing_result = sql_select($yarn_dyeing_sql);
			foreach ($yarn_dyeing_result as $row) {
				$dtls_data_arr[$row[csf("id")]]["quantity"]+= $row[csf("quantity")];
				$twisting_product[$row[csf("mst_id")]] = $row[csf("product_id")];
				$dtls_data_arr[$row[csf("id")]]["job_no_id"]= $row[csf("job_no_id")];
				$all_job_id_arr[$row[csf("job_no_id")]]=$row[csf("job_no_id")];
			}
		}
		else
		{
			$sql = "select a.ydw_no, a.company_id, b.id, b.job_no, b.job_no_id, b.yarn_color, b.yarn_wo_qty quantity, b.yarn_description, b.yarn_comp_percent1st, b.yarn_type, b.yarn_comp_type1st, b.count, b.product_id, b.entry_form 
			from wo_yarn_dyeing_mst a, wo_yarn_dyeing_dtls b 
			where a.id=b.mst_id and b.mst_id=$wo_pi_ID and a.status_active=1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0";
			//echo $sql;
			$result = sql_select($sql);
			foreach ($result as $row) {
			...
			$product_sql = "select a.job_no, c.id as prod_id, c.yarn_comp_percent1st,c.yarn_comp_type1st,c.yarn_count_id,c.yarn_type 
			from inv_transaction a, inv_issue_master b, product_details_master c, wo_yarn_dyeing_dtls d 
			where a.mst_id=b.id and a.prod_id=c.id and b.booking_id=d.mst_id and b.booking_id=$wo_pi_ID and b.entry_form=277 and b.issue_basis=9 and b.issue_purpose=$receive_purpose and a.item_category=1 and a.transaction_type=2 and a.status_active=1 and a.is_deleted=0 
			group by c.id, a.job_no, c.yarn_comp_percent1st, c.yarn_comp_type1st, c.yarn_count_id, c.yarn_type";
			$pr_result = sql_select($product_sql);
			$product_arr = array();
			foreach ($pr_result as $pr_row) {
				$product_arr[$pr_row[csf("job_no")]][$pr_row[csf("yarn_comp_percent1st")]][$pr_row[csf("yarn_comp_type1st")]][$pr_row[csf("yarn_count_id")]] = $pr_row[csf("prod_id")];
			}
		}
		if(count($all_job_id_arr)>0)
		{
			$job_sql=sql_select("select id, job_no, style_ref_no, buyer_name from wo_po_details_master where id in(".implode(",",$all_job_id_arr).")");
			$job_data_arr=array();
			foreach ($job_sql as $row) {
				$job_data_arr[$row[csf("id")]]["job_no"]= $row[csf("job_no")];
				$job_data_arr[$row[csf("id")]]["style_ref_no"]= $row[csf("style_ref_no")];
				$job_data_arr[$row[csf("id")]]["buyer_name"]= $row[csf("buyer_name")];
			}
		}
			...
			<tbody>
				...
				foreach ($result as $row) {
					...
					if($row[csf("product_id")] ){
						...
						$product_id = $product_arr[$row[csf("job_no")]][$row[csf("yarn_comp_percent1st")]][$row[csf("yarn_comp_type1st")]][$row[csf("count")]];
					}
					if($receive_purpose == 15)
					{
						...
						{
							$dtls_qnty += $dtls_data_arr[$dtls_row]["quantity"];
							$job_no_id=$dtls_data_arr[$dtls_row]["job_no_id"];
						}
						$style_ref_no=$job_data_arr[$job_no_id]["style_ref_no"];
						$buyer_name=$job_data_arr[$job_no_id]["buyer_name"];
						
						
						$on_click_action = "wo_product_form_yarn_dyeing_twisting";
						$on_click_action_param = $row[csf("job_no")] . "**" . $row[csf("count")] . "**" . $row[csf("yarn_comp_type1st")] . "**" . $row[csf("yarn_comp_percent1st")] . "**" . $row[csf("yarn_type")] . "**" . $row[csf("yarn_color")] . "**" . $row[csf("id")] . "**" . $wo_pi_ID . "**" . $receive_purpose . "**" . $row[csf("yarn_rate")] . "**" . $row[csf("ecchange_rate")] . "**" . $row[csf("company_id")]."**".$twisting_product[$row[csf("mst_id")]]. "**" .$job_no_id. "**" .$style_ref_no. "**" .$buyer_name;
					}
					else
					{
						
						$job_no_id=$row[csf("job_no_id")];
						$style_ref_no=$job_data_arr[$job_no_id]["style_ref_no"];
						$buyer_name=$job_data_arr[$job_no_id]["buyer_name"];
						
						$on_click_action = "wo_product_form_yarn_dyeing";
						$on_click_action_param = $product_id . "**" . $row[csf("job_no")] . "**" . $wo_pi_ID . "**" . $row[csf("yarn_color")] . "**" . $receive_purpose . "**" . $row[csf("id")]. "**" . $row[csf("entry_form")]. "**" .$job_no_id. "**" .$style_ref_no. "**" .$buyer_name;
					}
					...
					<tr bgcolor="<? echo $bgcolor; ?>"
						...
						<td title="<? echo "rcv purpose=".$receive_purpose; ?>"><? echo $i; ?></td>
						<td title="<?php echo $row[csf("ydw_no")]; ?>"><? echo $row[csf("job_no")]; ?></td>
						<td><? echo $yarn; ?></td>
						<td><? echo $color_arr[$row[csf("yarn_color")]]; ?></td>
						<td align="right"><? echo ($receive_purpose==15)?$dtls_qnty:$row[csf("quantity")];?></td>
					</tr>
					<?
					$i++;
				}
	...
	else 
	{
		if ($receive_basis == 1) // pi basis
		{
			...
			where a.id=b.pi_id and a.id=$wo_pi_ID and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.importer_id, a.supplier_id, b.id, b.count_name, b.yarn_composition_item1, b.yarn_composition_percentage1, b.yarn_composition_item2, b.yarn_composition_percentage2, b.yarn_type,b.color_id";*/
			$job_cond="";
			if ($receive_purpose == 4 || $receive_purpose == 43 ) $job_cond=" and c.job_no is not null";
			else $job_cond=" and c.job_no is null";
			if($db_type==0)
			{
				if ($receive_purpose == 4 || $receive_purpose == 43 )
				{
					$sql = "select a.id as mst_id, a.importer_id as company_id, a.supplier_id, b.count_name as yarn_count, b.yarn_composition_item1 as yarn_comp_type1st, b.yarn_composition_percentage1 as yarn_comp_percent1st, b.yarn_composition_item2 as yarn_comp_type2nd, b.yarn_composition_percentage2 as yarn_comp_percent2nd, b.yarn_type, b.color_id as color_name, sum(quantity) as quantity, c.job_id, c.job_no, c.buyer_id, c.style_no, 0 as order_type
					from com_pi_master_details a, com_pi_item_details b, wo_non_order_info_dtls c
					where a.id=b.pi_id and b.work_order_dtls_id=c.id and a.pi_basis_id=1 and a.id=$wo_pi_ID and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.job_no !=''
					group by a.id, a.importer_id, a.supplier_id, b.count_name, b.yarn_composition_item1, b.yarn_composition_percentage1, b.yarn_composition_item2, b.yarn_composition_percentage2, b.yarn_type, b.color_id, c.job_id, c.job_no, c.buyer_id, c.style_no
					union all
					select a.id as mst_id, a.importer_id as company_id, a.supplier_id, b.count_name as yarn_count, b.yarn_composition_item1 as yarn_comp_type1st, b.yarn_composition_percentage1 as yarn_comp_percent1st, b.yarn_composition_item2 as yarn_comp_type2nd, b.yarn_composition_percentage2 as yarn_comp_percent2nd, b.yarn_type, b.color_id as color_name, sum(quantity) as quantity, 0 as job_id, '' as job_no, 0 as buyer_id, '' as style_no, 0 as order_type
					from com_pi_master_details a, com_pi_item_details b
					where a.id=b.pi_id and a.pi_basis_id <> 1 and a.id=$wo_pi_ID and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
					group by a.id, a.importer_id, a.supplier_id, b.id, b.count_name, b.yarn_composition_item1, b.yarn_composition_percentage1, b.yarn_composition_item2, b.yarn_composition_percentage2, b.yarn_type, b.color_id";
				}
				else
				{
					$sql = "select a.id as mst_id, a.importer_id as company_id, a.supplier_id, b.count_name as yarn_count, b.yarn_composition_item1 as yarn_comp_type1st, b.yarn_composition_percentage1 as yarn_comp_percent1st, b.yarn_composition_item2 as yarn_comp_type2nd, b.yarn_composition_percentage2 as yarn_comp_percent2nd, b.yarn_type, b.color_id as color_name, sum(quantity) as quantity, b.work_order_id as job_id, b.work_order_no as job_no, c.buyer_id, c.style_no, 1 as order_type
					from com_pi_master_details a, com_pi_item_details b, wo_non_order_info_dtls c
					where a.id=b.pi_id and b.work_order_dtls_id=c.id and a.pi_basis_id=1 and a.id=$wo_pi_ID and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.job_no ='' 
					group by a.id, a.importer_id, a.supplier_id, b.count_name, b.yarn_composition_item1, b.yarn_composition_percentage1, b.yarn_composition_item2, b.yarn_composition_percentage2, b.yarn_type, b.color_id, b.work_order_id, b.work_order_no, c.buyer_id, c.style_no
					union all
					select a.id as mst_id, a.importer_id as company_id, a.supplier_id, b.count_name as yarn_count, b.yarn_composition_item1 as yarn_comp_type1st, b.yarn_composition_percentage1 as yarn_comp_percent1st, b.yarn_composition_item2 as yarn_comp_type2nd, b.yarn_composition_percentage2 as yarn_comp_percent2nd, b.yarn_type, b.color_id as color_name, sum(quantity) as quantity, 0 as job_id, '' as job_no, 0 as buyer_id, '' as style_no, 1 as order_type
					from com_pi_master_details a, com_pi_item_details b
					where a.id=b.pi_id and a.pi_basis_id <> 1 and a.id=$wo_pi_ID and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
					group by a.id, a.importer_id, a.supplier_id, b.id, b.count_name, b.yarn_composition_item1, b.yarn_composition_percentage1, b.yarn_composition_item2, b.yarn_composition_percentage2, b.yarn_type, b.color_id";
				}
			...
			else
			{
				if ($receive_purpose == 4 || $receive_purpose == 43 )
				{
					$sql = "select  a.id as mst_id, a.importer_id as company_id, a.supplier_id, b.count_name as yarn_count, b.yarn_composition_item1 as yarn_comp_type1st, b.yarn_composition_percentage1 as yarn_comp_percent1st, b.yarn_composition_item2 as yarn_comp_type2nd, b.yarn_composition_percentage2 as yarn_comp_percent2nd, b.yarn_type, b.color_id as color_name, sum(quantity) as quantity, c.job_id, c.job_no, c.buyer_id, c.style_no, 0 as order_type
					from com_pi_master_details a, com_pi_item_details b, wo_non_order_info_dtls c
					where a.id=b.pi_id and b.work_order_dtls_id=c.id and a.pi_basis_id=1 and a.id=$wo_pi_ID and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.job_no is not null
					group by a.id, a.importer_id, a.supplier_id, b.count_name, b.yarn_composition_item1, b.yarn_composition_percentage1, b.yarn_composition_item2, b.yarn_composition_percentage2, b.yarn_type, b.color_id, c.job_id, c.job_no, c.buyer_id, c.style_no
					union all
					select a.id as mst_id, a.importer_id as company_id, a.supplier_id, b.count_name as yarn_count,b.yarn_composition_item1 as yarn_comp_type1st, b.yarn_composition_percentage1 as yarn_comp_percent1st, b.yarn_composition_item2 as yarn_comp_type2nd, b.yarn_composition_percentage2 as yarn_comp_percent2nd, b.yarn_type, b.color_id as color_name, sum(quantity) as quantity, 0 as job_id, null as job_no, 0 as buyer_id, null as style_no, 0 as order_type
					from com_pi_master_details a, com_pi_item_details b
					where a.id=b.pi_id and a.pi_basis_id <> 1 and a.id=$wo_pi_ID and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
					group by a.id, a.importer_id, a.supplier_id, b.count_name, b.yarn_composition_item1, b.yarn_composition_percentage1, b.yarn_composition_item2, b.yarn_composition_percentage2, b.yarn_type, b.color_id";
				}
				else
				{
					$sql = "select  a.id as mst_id, a.importer_id as company_id, a.supplier_id, b.count_name as yarn_count, b.yarn_composition_item1 as yarn_comp_type1st, b.yarn_composition_percentage1 as yarn_comp_percent1st, b.yarn_composition_item2 as yarn_comp_type2nd, b.yarn_composition_percentage2 as yarn_comp_percent2nd, b.yarn_type, b.color_id as color_name, sum(quantity) as quantity, b.work_order_id as job_id, b.work_order_no as job_no, c.buyer_id, c.style_no, 1 as order_type
					from com_pi_master_details a, com_pi_item_details b, wo_non_order_info_dtls c
					where a.id=b.pi_id and b.work_order_dtls_id=c.id and a.pi_basis_id=1 and a.id=$wo_pi_ID and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.job_no is null
					group by a.id, a.importer_id, a.supplier_id, b.count_name, b.yarn_composition_item1, b.yarn_composition_percentage1, b.yarn_composition_item2, b.yarn_composition_percentage2, b.yarn_type, b.color_id, b.work_order_id, b.work_order_no, c.buyer_id, c.style_no
					union all
					select a.id as mst_id, a.importer_id as company_id, a.supplier_id, b.count_name as yarn_count,b.yarn_composition_item1 as yarn_comp_type1st, b.yarn_composition_percentage1 as yarn_comp_percent1st, b.yarn_composition_item2 as yarn_comp_type2nd, b.yarn_composition_percentage2 as yarn_comp_percent2nd, b.yarn_type, b.color_id as color_name, sum(quantity) as quantity, 0 as job_id, null as job_no, 0 as buyer_id, null as style_no, 1 as order_type
					from com_pi_master_details a, com_pi_item_details b
					where a.id=b.pi_id and a.pi_basis_id <> 1 and a.id=$wo_pi_ID and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
					group by a.id, a.importer_id, a.supplier_id, b.count_name, b.yarn_composition_item1, b.yarn_composition_percentage1, b.yarn_composition_item2, b.yarn_composition_percentage2, b.yarn_type, b.color_id";
				}
		...
		{
			$sql = "select a.id mst_id, a.company_name as company_id, a.supplier_id, b.yarn_count, b.yarn_comp_type1st, b.yarn_comp_percent1st, b.yarn_comp_type2nd, b.yarn_comp_percent2nd, b.yarn_type, b.color_name, sum(supplier_order_quantity) as quantity, b.job_id, b.job_no, b.buyer_id, b.style_no
			from wo_non_order_info_mst a, wo_non_order_info_dtls b 
			where a.id=b.mst_id and a.id=$wo_pi_ID and a.status_active=1 and b.status_active=1
			group by a.id, a.company_name, a.supplier_id, b.yarn_count, b.yarn_comp_type1st, b.yarn_comp_percent1st, b.yarn_comp_type2nd, b.yarn_comp_percent2nd, b.yarn_type, b.color_name, b.job_id, b.job_no, b.buyer_id, b.style_no";
		...
		foreach ($result as $row_count) {
			$yarn_sum_receive_arr[$row_count[csf("mst_id")]][$row_count[csf("job_no")]][$row_count[csf("yarn_count")]][$row_count[csf("yarn_comp_type1st")]][$row_count[csf("yarn_comp_percent1st")]][$row_count[csf("yarn_type")]][$row_count[csf("color_name")]] += $row_count[csf("quantity")];
		}
		$yarn_count_arr = return_library_array("select id, yarn_count from lib_yarn_count", 'id', 'yarn_count');
		...
		<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" id="tbl_product" >
			...
			<tbody>
				...
				foreach ($result as $row) {
					...
					$quantity = trim($yarn_sum_receive_arr[$row[csf("mst_id")]][$row[csf("job_no")]][$row[csf("yarn_count")]][$row[csf("yarn_comp_type1st")]][$row[csf("yarn_comp_percent1st")]][$row[csf("yarn_type")]][$row[csf("color_name")]]);
					$job_no_arr=explode("-",$row[csf("job_no")]);
					//
					?>
					<tr bgcolor="<? echo $bgcolor; ?>"
						onClick='get_php_form_data("<? echo $receive_basis . "**" . $row[csf("mst_id")] . "**" . $row[csf("company_id")] . "**" . $row[csf("supplier_id")] . "**" . number_format($quantity,6,'.','') . "**" . $row[csf("buyer_id")] . "**" . $row[csf("job_no")] . "**" . $row[csf("job_id")] . "**" . $row[csf("style_no")] . "**" . $row[csf("yarn_count")] . "**" . $row[csf("yarn_comp_type1st")] . "**" . $row[csf("yarn_type")] . "**" . $row[csf("color_name")]. "**" . $row[csf("order_type")]. "**" . $receive_purpose;?>","wo_pi_product_form_input","requires/yarn_receive_controller");change_color_tr("<? echo $i; ?>","<? echo $bgcolor; ?>")'
						style="cursor:pointer" id="tr_<? echo $i; ?>">
						<td align="center"><? echo $i; ?></td>
						<td align="center"><p><? echo $buyer_short_arr[$row[csf("buyer_id")]]; ?>&nbsp;</p></td>
                        <td align="center"><p><? echo $job_no_arr[2]*1; ?>&nbsp;</p></td>
                       	<td><p><? echo $row[csf("style_no")]; ?>&nbsp;</p></td>
...
if ($action == "wo_product_form_yarn_dyeing_twisting") 
{
	$ex_data = explode("**", $data);
	...
	$job_no_id = $ex_data[13];
	$style_ref_no = $ex_data[14];
	$buyer_name = $ex_data[15];
	...
	echo "$('#cbo_buyer_name').val(" . $buyer_name . ");\n";
	echo "$('#txt_job_no').val('" . $job_no . "');\n";
	echo "$('#hide_job_id').val('" . $job_no_id . "');\n";
	echo "$('#txt_style_no').val('" . $style_ref_no . "');\n";
...
if ($action == "wo_product_form_yarn_dyeing") 
{
	...
	$job_no_id = $ex_data[7];
	$style_ref_no = $ex_data[8];
	$buyer_name = $ex_data[9];
	if($purpose==15 || $purpose==50 || $purpose==51) $item_category_cond=" and e.item_category_id=0";
	else 			 $item_category_cond=" and e.item_category_id=24";
...
//$dyeing_charge_sql = "select e.ecchange_rate*d.dyeing_charge as dyeing_charge from inv_transaction a, inv_issue_master b, product_details_master c,wo_yarn_dyeing_dtls d,wo_yarn_dyeing_mst e where a.mst_id=b.id and a.prod_id=c.id and b.booking_id=d.mst_id and d.mst_id=e.id and b.booking_id=$wo_pi_ID and b.entry_form=3 and b.issue_basis=1 and b.issue_purpose =$purpose and a.item_category=1 and a.transaction_type=2 and a.status_active=1 and a.is_deleted=0 and d.yarn_color=$color_id and a.prod_id=$prod_id";
	if($entry_form==125)
	{
		$productCond = "";
	}else{
...
if ($action == "wo_pi_product_form_input")
{
	...
	$buyer_id = $ex_data[5];
	$job_no = trim(str_replace("'","",$ex_data[6]));
	$job_no_id = $ex_data[7];
	$style_no = trim(str_replace("'","",$ex_data[8]));
	$yarn_count = $ex_data[9];
	$yarn_comp_type1st = $ex_data[10];
	$yarn_type = $ex_data[11];
	$color_name = $ex_data[12];
	$order_type = $ex_data[13];
	$receive_purpose = $ex_data[14];
	
	//echo $job_no;die;
	//$row[csf("yarn_count")] . "**" . $row[csf("yarn_comp_type1st")] . "**" . $row[csf("yarn_type")] . "**" . $row[csf("color_name")]
	
	$material_orver_con=sql_select("select over_rcv_percent, over_rcv_payment from variable_inv_ile_standard where company_name=$company_id and variable_list=23 and CATEGORY=1 and status_active=1 and is_deleted=0");
	$over_rcv_percent=$material_orver_con[0][csf("over_rcv_percent")];
	$over_rcv_payment=$material_orver_con[0][csf("over_rcv_payment")];
	...
	if ($receive_basis == 1) // pi basis
	{
		...
		if($order_type==1)
		{
			$sql = "select a.id as mst_id,a.version, b.count_name as yarn_count,b.yarn_composition_item1 as yarn_comp_type1st, b.yarn_composition_percentage1 as yarn_comp_percent1st, b.yarn_composition_item2 as yarn_comp_type2nd, b.yarn_composition_percentage2 as yarn_comp_percent2nd, b.yarn_type, b.color_id as color_name, b.quantity, b.net_pi_rate as rate, b.uom, b.work_order_no as job_no  
			from com_pi_master_details a, com_pi_item_details b, wo_non_order_info_dtls c
			where a.id=b.pi_id and b.work_order_dtls_id=c.id and a.id=$wo_pi_ID and b.count_name=$yarn_count and b.yarn_composition_item1=$yarn_comp_type1st and b.yarn_type=$yarn_type and b.color_id=$color_name and b.work_order_no='$job_no'";
		}
		else
		{
			$sql = "select a.id as mst_id,a.version, b.count_name as yarn_count,b.yarn_composition_item1 as yarn_comp_type1st, b.yarn_composition_percentage1 as yarn_comp_percent1st, b.yarn_composition_item2 as yarn_comp_type2nd, b.yarn_composition_percentage2 as yarn_comp_percent2nd, b.yarn_type, b.color_id as color_name,b.quantity, b.net_pi_rate as rate, b.uom, c.job_no  
			from com_pi_master_details a, com_pi_item_details b, wo_non_order_info_dtls c
			where a.id=b.pi_id and b.work_order_dtls_id=c.id and a.id=$wo_pi_ID and b.count_name=$yarn_count and b.yarn_composition_item1=$yarn_comp_type1st and b.yarn_type=$yarn_type and b.color_id=$color_name and c.job_no='$job_no'";
		}
	}
	else if ($receive_basis == 2) // wo basis
	{
		if($job_no!="") $job_conds=" and b.job_no='$job_no'";
		$sql = "select a.id as mst_id, a.wo_number, b.yarn_count, b.yarn_comp_type1st, b.yarn_comp_percent1st, b.yarn_comp_type2nd, b.yarn_comp_percent2nd, b.yarn_type,b.color_name, b.supplier_order_quantity as quantity, rate, uom, b.job_no  
		from wo_non_order_info_mst a, wo_non_order_info_dtls b 
		where a.id=b.mst_id and a.id=$wo_pi_ID and b.yarn_count=$yarn_count and b.yarn_comp_type1st=$yarn_comp_type1st and b.yarn_type=$yarn_type and b.color_name=$color_name $job_conds";
	...
	//echo $sql;
	 ...
	 foreach ($result as $row)
	 {
		...
		if($receive_purpose==8 && $order_type!=1)
		{
			echo "$('#txt_job_no').val('" . $row[csf("wo_number")] . "');\n";
			echo "$('#hide_job_id').val('" . $row[csf("mst_id")] . "');\n";
		}
		else
		{
			echo "$('#txt_job_no').val('" . $job_no . "');\n";
		...
		echo "$('#txt_style_no').val('" . $style_no . "');\n";
		echo "$('#cbo_buyer_name').attr('disabled',true);\n";
		if($job_no!="")
		{
			echo "$('#txt_job_no').attr('disabled',true);\n";
		...
		{
			echo "$('#txt_job_no').attr('disabled',false);\n";
		}
	...
	$rcvReturnQnty=0;
	 ...
	 if(str_replace("'", "", $mrr_nos) != "" && $prod_arr!="")
	 {
		...
		$direct_mrr_return_res = sql_select("select sum(b.cons_quantity) as ret_qnty
	 		...
	 		where a.id=b.prod_id and b.mst_id = c.id and c.entry_form = 381 and c.received_mrr_no in ($mrr_nos)
		...
		/*########## develop later   ###########*/
	 	...
	 	/*$issue_sql = sql_select("select a.id,b.cons_quantity, b.id as trans_id
	 		...
	 		and d.recv_number in ($mrr_nos) and b.prod_id in ($prod_ids) and b.job_no='" . $row[csf("job_no")] ."'
	 		and a.entry_form = 277 and c.entry_form = 277 and d.item_category=1 and b.status_active = 1 and b.is_deleted = 0");
	 ...
	 //echo "$wo_po_qnty =  $totalRcvQnty  =  $rcvReturnQnty \n";die;
	 $orderQnty = $wo_po_qnty - $totalRcvQnty + $rcvReturnQnty;
	 if($over_rcv_payment==1 && $over_rcv_percent > 0)
	 {
		 $over_qnty=(($over_rcv_percent/100)*$wo_po_qnty);
		 $orderOverQnty=($orderQnty+$over_qnty);
	 }
	 else
	 {
		 $orderOverQnty=$orderQnty;
	 }
	 echo "$('#txt_order_qty').val(" . $orderQnty . ");\n";
	 echo "$('#txt_bal_order_qty').val(" . $orderOverQnty . ");\n";
...
if ($action == "wo_pi_product_form_input_old") {
	$ex_data = explode("**", $data);
	$receive_basis = $ex_data[0];
	$wo_pi_ID = $ex_data[1];
	$company_id = $ex_data[2];
...
if ($receive_basis == 1) // pi basis
{
	$sql = "select a.id as mst_id, b.count_name as yarn_count,b.yarn_composition_item1 as yarn_comp_type1st, b.yarn_composition_percentage1 as yarn_comp_percent1st, b.yarn_composition_item2 as yarn_comp_type2nd, b.yarn_composition_percentage2 as yarn_comp_percent2nd, b.yarn_type, b.color_id as color_name,b.quantity, b.net_pi_rate as rate, b.uom  from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and b.id=$wo_pi_ID";
}
 else if ($receive_basis == 2) // wo basis
 {
	$sql = "select a.id as mst_id, b.yarn_count, b.yarn_comp_type1st, b.yarn_comp_percent1st, b.yarn_comp_type2nd, b.yarn_comp_percent2nd, b.yarn_type,b.color_name, b.supplier_order_quantity as quantity, rate, uom  from wo_non_order_info_mst a, wo_non_order_info_dtls b where a.id=b.mst_id and b.id=$wo_pi_ID";
 }
//echo $sql;	die;
 $result = sql_select($sql);
 foreach ($result as $row) {
	...
	$whereCondition = "a.id=b.prod_id and a.yarn_count_id=" . $row[csf("yarn_count")] . " and a.yarn_comp_type1st=" . $row[csf("yarn_comp_type1st")] . " and a.yarn_comp_percent1st=" . $row[csf("yarn_comp_percent1st")] . " and $orderBy_cond(a.yarn_comp_type2nd,0)=" . $yarn_comp_type2nd . " and $orderBy_cond(a.yarn_comp_percent2nd,0)=" . $yarn_comp_percent2nd . " and a.yarn_type=" . $row[csf("yarn_type")] . " and a.color=" . $row[csf("color_name")] . " and b.company_id=$company_id and a.supplier_id=$supplier_id and a.item_category_id=1 and b.transaction_type=1 and b.item_category=1 and b.pi_wo_batch_no=" . $row[csf("mst_id")] . " and b.receive_basis=$receive_basis";
...
if ($action == "populate_data_from_wopi_popup") {
	...
	{
		if ($receive_purpose == 2 || $receive_purpose == 7 || $receive_purpose == 12 || $receive_purpose == 15 || $receive_purpose == 38 || $receive_purpose == 46) //currency as currency_id,
		{
			$sql = "select id, '' as lc_number, supplier_id, 1 as currency_id, source, pay_mode
			from wo_yarn_dyeing_mst
			where status_active=1 and  is_deleted=0 and id=$wo_pi_ID";
		}
		else
		{
			$sql = "select id,'' as lc_number,supplier_id as supplier_id,currency_id as currency_id, source as source, 0 as pay_mode
			from wo_non_order_info_mst
			where status_active=1 and is_deleted=0 and  id=$wo_pi_ID";
		}
	}
//echo $sql;die;
	...
	foreach ($result as $row) {
		...
		echo "$('#hidd_pay_mode').val('" . $row[csf("pay_mode")] . "');\n";
...
if ($action == "wo_product_form_yarn_dyeing") {
	...
	$purpose = $ex_data[4];
	$wo_dtls_ID = $ex_data[5];
	if($purpose==15) $item_category_cond=" and e.item_category_id=0";
	else 			 $item_category_cond=" and e.item_category_id=24";
	$avg_rate = return_field_value("(sum(cons_amount)/ sum(cons_quantity)) as order_rate", "inv_transaction", "status_active=1 and is_deleted=0 and prod_id=$prod_id and item_category=1 and transaction_type in(1,5)", "order_rate");

//$dyeing_charge_sql = "select e.ecchange_rate*d.dyeing_charge as dyeing_charge from inv_transaction a, inv_issue_master b, product_details_master c,wo_yarn_dyeing_dtls d,wo_yarn_dyeing_mst e where a.mst_id=b.id and a.prod_id=c.id and b.booking_id=d.mst_id and d.mst_id=e.id and b.booking_id=$wo_pi_ID and b.entry_form=3 and b.issue_basis=1 and b.issue_purpose =$purpose and a.item_category=1 and a.transaction_type=2 and a.status_active=1 and a.is_deleted=0 and d.yarn_color=$color_id and a.prod_id=$prod_id";
	$dyeing_charge_sql = "select e.ecchange_rate*d.dyeing_charge as dyeing_charge from wo_yarn_dyeing_dtls d,wo_yarn_dyeing_mst e where d.mst_id=e.id and d.id=$wo_dtls_ID  and d.status_active=1 and d.is_deleted=0 and d.yarn_color=$color_id and d.product_id=$prod_id $item_category_cond";
	//echo $dyeing_charge_sql;die;
	$dyeing_charge_result = sql_select($dyeing_charge_sql);
...
if ($action == "save_update_delete") {
	...
	if (str_replace("'", "", $txt_mrr_no) != "") 
	{
		...
		$rcv_id = $prev_dataArray[0][csf('id')];
	}
	
	$material_orver_con=sql_select("select over_rcv_percent, over_rcv_payment from variable_inv_ile_standard where company_name=$cbo_company_id and variable_list=23 and CATEGORY=1 and status_active=1 and is_deleted=0");
	$over_rcv_percent=$material_orver_con[0][csf("over_rcv_percent")];
	$over_rcv_payment=$material_orver_con[0][csf("over_rcv_payment")];
    if($over_rcv_payment==1 && $over_rcv_percent > 0 && (str_replace("'","",$cbo_receive_basis)==1 || str_replace("'","",$cbo_receive_basis)==2) )
	{
		
		if(str_replace("'","",$cbo_receive_purpose) !=2 && str_replace("'","",$cbo_receive_purpose) !=7 && str_replace("'","",$cbo_receive_purpose) !=12 && str_replace("'","",$cbo_receive_purpose) !=15 && str_replace("'","",$cbo_receive_purpose) !=38 && str_replace("'","",$cbo_receive_purpose) !=46)
		{
			$prev_cond="";
			if($rcv_id>0) $prev_cond=" and a.mst_id <> $rcv_id";
			$prev_sql="select a.id as trans_id, c.recv_number, b.id as prod_id, order_qnty as prev_qnty 
			from inv_transaction a, product_details_master b, inv_receive_master c 
			where c.id=a.mst_id and a.prod_id=b.id and a.status_active=1 and a.is_deleted=0 and a.pi_wo_batch_no=$txt_wo_pi_id and a.receive_basis=$cbo_receive_basis and b.yarn_count_id=$cbo_yarn_count and b.yarn_comp_type1st=$cbocomposition1 and b.yarn_type=$cbo_yarn_type and a.transaction_type=1 and b.color=$cbo_color and a.job_no=$txt_job_no  $prev_cond";
			$prev_rcv_result=sql_select($prev_sql);
			$prev_rcv_qnty=0;
			foreach ($prev_rcv_result as $val)
			{
				if(str_replace("'","",$update_id) != $val[csf("trans_id")])
				{
					$prev_rcv_qnty += $val[csf("recv_qnty")];
				}				
				$mrr_arr[$val[csf("recv_number")]] = $val[csf("recv_number")];
				$prod_arr[$val[csf("prod_id")]] = $val[csf("prod_id")];
			}
			unset($prev_rcv_result);
			$mrr_nos = "'".implode("','",array_filter($mrr_arr))."'";
			$prod_ids = implode(",", array_filter($prod_arr));
			//echo "10**".$mrr_nos."=".$prod_ids;die;
			if(str_replace("'", "", $mrr_nos) != "" && $prod_arr!="")
			{
				$direct_mrr_return_res = sql_select("select b.id, b.cons_quantity as ret_qnty
				from product_details_master a, inv_transaction b , inv_issue_master c
				where a.id=b.prod_id and b.mst_id = c.id and c.entry_form = 381 and c.received_mrr_no in ($mrr_nos)
				and a.id in ($prod_ids) and b.status_active = 1 and b.is_deleted=0 and c.status_active = 1 and c.is_deleted = 0");
			...
			{
				if(str_replace("'","",$cbo_receive_purpose)==8)
				{
					$pi_wo_sql="select sum(a.quantity) as wo_pi_qnty  from com_pi_item_details a
					where a.status_active=1 and a.is_deleted=0 and a.pi_id=$txt_wo_pi_id and a.count_name=$cbo_yarn_count and a.yarn_composition_item1=$cbocomposition1 and a.yarn_type=$cbo_yarn_type and a.color_id=$cbo_color and a.work_order_no=$txt_job_no";
				}
				else
				{
					$pi_wo_sql="select sum(a.quantity) as wo_pi_qnty 
					from com_pi_item_details a, wo_non_order_info_dtls b 
					where a.work_order_dtls_id=b.id and a.status_active=1 and a.is_deleted=0 and a.pi_id=$txt_wo_pi_id and a.count_name=$cbo_yarn_count and a.yarn_composition_item1=$cbocomposition1 and a.yarn_type=$cbo_yarn_type and a.color_id=$cbo_color and b.job_no=$txt_job_no";
				}
			}
			else
			{
				$pi_wo_sql="select sum(supplier_order_quantity) as wo_pi_qnty from wo_non_order_info_dtls where status_active=1 and is_deleted=0 and mst_id=$txt_wo_pi_id and yarn_count=$cbo_yarn_count and yarn_comp_type1st=$cbocomposition1 and yarn_type=$cbo_yarn_type and color_name=$cbo_color and job_no=$txt_job_no";
			}
			//echo "10**".$pi_wo_sql;die;
			$pi_wo_result=sql_select($pi_wo_sql);
			$wo_pi_qntys=($pi_wo_result[0][csf("wo_pi_qnty")]-($prev_rcv_qnty*1)+($rcvReturnQnty*1));
			$over_qnty=(($over_rcv_percent/100)*$pi_wo_result[0][csf("wo_pi_qnty")]);
			$orderOverQnty=($wo_pi_qntys+$over_qnty);
			if(str_replace("'","",$txt_receive_qty)>$orderOverQnty)
			{
				echo "30**Receive Quantity Can Not Greater Than Balance Quantity";disconnect($con);die;
			}
		}
	 }
	...
	if ($operation == 0) // Insert Here
	{
		...
		if (str_replace("'", "", $btn_color) == 'F') {
			...
			if (str_replace("'", "", trim($cbo_color)) != "") {
				if (!in_array(str_replace("'", "", trim($cbo_color)),$new_array_color)){
					$color_id = return_id( str_replace("'", "", trim($cbo_color)), $color_library, "lib_color", "id,color_name","248");
					$new_array_color[$color_id]=str_replace("'", "", trim($cbo_color));
				}
				else $color_id =  array_search(str_replace("'", "", trim($cbo_color)), $new_array_color);
			} else $color_id = 0;
		...
		if (str_replace("'", "", $txt_brand) != "") {
			...
			if (str_replace("'", "", trim($txt_brand)) != "") {
				if (!in_array(str_replace("'", "", trim($txt_brand)),$new_array_brand)){
					$brand_id = return_id( str_replace("'", "", trim($txt_brand)), $brand_arr, "lib_brand", "id,brand_name","248");
		...
		//---------------Check Product ID --------------------------//
		$insertR = true;
		$rtnString = return_product_id($cbo_yarn_count, $cbocomposition1, $cbocomposition2, $percentage1, $percentage2, $cbo_yarn_type, $cbo_color, $txt_yarn_lot, $txt_prod_code, $cbo_company_id, $cbo_supplier, $cbo_store_name, $cbo_uom, $yarn_type, $composition,$cbo_receive_purpose,$hidd_pay_mode);
		...
		$max_transaction_date = return_field_value("max(transaction_date) as max_date", "inv_transaction", "prod_id=$prodMSTID and store_id=$cbo_store_name and status_active = 1 and entry_form=248", "max_date");
		...
		$duplicate = is_duplicate_field("b.id", "inv_receive_master a, inv_transaction b", "a.id=b.mst_id and a.recv_number=$txt_mrr_no and b.prod_id=$prodMSTID and b.transaction_type=1 and a.item_category=1 and a.entry_form=248 and b.status_active=1");
		//&& str_replace("'", "", $txt_mrr_no) == ""
		if ($duplicate == 1) {
		...
		if (str_replace("'", "", $txt_mrr_no) != "") 
		{
			...
			$field_array = "item_category*receive_basis*receive_purpose*receive_date*challan_date*booking_id*challan_no*store_id*exchange_rate*currency_id*supplier_id*loan_party*yarn_issue_challan_no*issue_id*lc_no*source*remarks*updated_by*update_date";
			$data_array = "1*" . $cbo_receive_basis . "*" . $cbo_receive_purpose . "*" . $txt_receive_date . "*" . $txt_challan_date . "*" . $txt_wo_pi_id . "*" . $txt_challan_no . "*" . $cbo_store_name . "*" . $txt_exchange_rate . "*" . $cbo_currency . "*" . $cbo_supplier . "*" . $cbo_party . "*" . $txt_issue_challan_no . "*" . $txt_issue_id . "*" . $hidden_lc_id . "*" . $cbo_source . "*" . $txt_mst_remarks . "*'" . $user_id . "'*'" . $pc_date_time . "'";
		...
		else 
		{
			...
			$new_recv_number = explode("*", return_next_id_by_sequence("INV_RECEIVE_MASTER_PK_SEQ", "inv_receive_master",$con,1,$cbo_company_id,'SYRV',248,date("Y",time()),1 ));

			$field_array = "id, recv_number_prefix, recv_number_prefix_num, recv_number, entry_form, item_category, company_id, receive_basis, receive_purpose, receive_date, challan_date, booking_id, challan_no, store_id, exchange_rate, currency_id, supplier_id, loan_party, yarn_issue_challan_no, issue_id, lc_no, source, remarks, pay_mode, inserted_by, insert_date";
			$data_array = "(" . $id . ",'" . $new_recv_number[1] . "','" . $new_recv_number[2] . "','" . $new_recv_number[0] . "',248,1," . $cbo_company_id . "," . $cbo_receive_basis . "," . $cbo_receive_purpose . "," . $txt_receive_date . "," . $txt_challan_date . "," . $txt_wo_pi_id . "," . $txt_challan_no . "," . $cbo_store_name . "," . $txt_exchange_rate . "," . $cbo_currency . "," . $cbo_supplier . "," . $cbo_party . "," . $txt_issue_challan_no . "," . $txt_issue_id . "," . $hidden_lc_id . "," . $cbo_source . "," . $txt_mst_remarks . "," . $hidd_pay_mode . ",'" . $user_id . "','" . $pc_date_time . "')";
		...
		$field_array_trans = "id,mst_id,receive_basis,pi_wo_batch_no,company_id,supplier_id,prod_id,origin_prod_id,product_code,item_category,transaction_type,transaction_date,store_id,brand_id,order_uom,order_qnty,order_rate,order_ile,order_ile_cost,order_amount,cons_uom,cons_quantity,cons_rate,cons_avg_rate,dye_charge,cons_ile,cons_ile_cost,cons_amount,balance_qnty,balance_amount,no_of_bags,cone_per_bag,weight_per_bag,weight_per_cone,room,rack,self,bin_box,floor_id,remarks,job_no,buyer_id,style_ref_no,garments_qty,entry_form,inserted_by,insert_date";

		$data_array_trans = "(" . $dtlsid . "," . $id . "," . $cbo_receive_basis . "," . $txt_wo_pi_id . "," . $cbo_company_id . "," . $cbo_supplier . "," . $prodMSTID . "," . $prodMSTID . "," . $txt_prod_code . ",1,1," . $txt_receive_date . "," . $cbo_store_name . "," . $txt_brand . "," . $cbo_uom . "," . $txt_receive_qty . "," . $txt_rate . "," . $ile . "," . $ile_cost . "," . $txt_amount . "," . $cbo_uom . "," . $txt_receive_qty . "," . $cons_rate . "," . $txt_avg_rate . "," . $txt_dyeing_charge . "," . $con_ile . "," . $con_ile_cost . "," . $con_amount . "," . $txt_receive_qty . "," . $con_amount . "," . $txt_no_bag . "," . $txt_cone_per_bag . "," . $txt_weight_per_bag . "," . $txt_weight_per_cone . "," . $cbo_room . "," . $txt_rack . "," . $txt_shelf . "," . $cbo_bin . "," . $cbo_floor . "," . $txt_remarks . "," . $txt_job_no . ",'" . $cbo_buyer_name . "'," . $txt_style_no . "," . $txt_challan_qty . ",248,'" . $user_id . "','" . $pc_date_time . "')";
		...
		$data_array_prod_update = "" . $txt_brand . "*" . number_format($avgRate, $dec_place[3], ".", "") . "*" . $txt_receive_qty . "*" . $currentStock . "*" . number_format($StockValue, $dec_place[4], ".", "") . "*'0'*'" . $available_qnty . "'*'" . $user_id . "'*'" . $pc_date_time . "'";
		...
		//echo "10**".$rID ."&&". $dtlsrID ."&&". $prodUpdate ."&&". $insertR ;die;
		if ($db_type == 0) {
	...
	{
		$prev_data = sql_select("select a.id,a.currency_id,count(b.id) as dtls_row from inv_receive_master a, inv_transaction b where a.id=b.mst_id and a.recv_number=$txt_mrr_no and b.id!=$update_id and a.entry_form=248 and b.item_category=1 and b.transaction_type=1 group by a.id, a.currency_id");
		...
		if ($before_prod_id != $prodMSTID) 
		{
			...
			if (!empty($check_requisition) && $check_requisition[0][csf('requisition_no')] != "") {
				echo "13**Lot can not be changed.Requisition found(".$check_requisition[0][csf('requisition_no')].")."; disconnect($con); oci_rollback($con); die;
			}
		}
		...
		// receive_basis*receive_purpose*supplier_id*currency_id*exchange_rate*source*
		// $cbo_receive_basis . "*" . $cbo_receive_purpose . "*" . $cbo_supplier . "*" . $cbo_currency . "*" . $txt_exchange_rate . "*" . $cbo_source . "*" .
		/*#### Stop not eligible field from update operation end ####*/

		$field_array_update = "item_category*receive_date*challan_date*challan_no*store_id*loan_party*yarn_issue_challan_no*issue_id*lc_no*remarks	*updated_by*update_date";
		$data_array_update = "1*" . $txt_receive_date . "*" . $txt_challan_date . "*" . $txt_challan_no . "*" . $cbo_store_name . "*" . $cbo_party . "*" . $txt_issue_challan_no . "*" . $txt_issue_id . "*" . $hidden_lc_id . "*" . $txt_mst_remarks . "*'" . $user_id . "'*'" . $pc_date_time . "'";
		...
		$field_array_trans = "receive_basis*pi_wo_batch_no*company_id*supplier_id*prod_id*origin_prod_id*product_code*item_category*transaction_type*transaction_date*store_id*brand_id*order_uom*order_qnty*order_rate*order_ile*order_ile_cost*order_amount*cons_uom*cons_quantity*cons_rate*cons_avg_rate*dye_charge*cons_ile*cons_ile_cost*cons_amount*balance_qnty*balance_amount*no_of_bags*cone_per_bag*weight_per_bag*weight_per_cone*room*rack*self*bin_box*floor_id*remarks*job_no*buyer_id*style_ref_no*garments_qty*updated_by*update_date";
		$data_array_trans = "" . $cbo_receive_basis . "*" . $txt_wo_pi_id . "*" . $cbo_company_id . "*" . $cbo_supplier . "*" . $prodMSTID . "*" . $prodMSTID . "*" . $txt_prod_code . "*1*1*" . $txt_receive_date . "*" . $cbo_store_name . "*" . $txt_brand . "*" . $cbo_uom . "*" . $txt_receive_qty . "*" . $txt_rate . "*" . $ile . "*" . $ile_cost . "*" . $txt_amount . "*" . $cbo_uom . "*" . $txt_receive_qty . "*" . $cons_rate . "*" . $txt_avg_rate . "*" . $txt_dyeing_charge . "*" . $con_ile . "*" . $con_ile_cost . "*" . $con_amount . "*" . $adjBalanceQnty . "*" . $adjBalanceAmount . "*" . $txt_no_bag . "*" . $txt_cone_per_bag . "*" . $txt_weight_per_bag . "*" . $txt_weight_per_cone . "*" . $cbo_room . "*" . $txt_rack . "*" . $txt_shelf . "*" . $cbo_bin . "*" . $cbo_floor . "*" . $txt_remarks . "*" . $txt_job_no . "*'" . $cbo_buyer_name . "'*" . $txt_style_no  . "*" . $txt_challan_qty. "*'" . $user_id . "'*'" . $pc_date_time . "'";
		...
		if ($before_prod_id == $prodMSTID) 
		{
			...
			if($StockValue>0 && $currentStock>0)
			{
		...
		if ($db_type == 0) {
			if ($rID && $dtlsrID && $prodUpdate && $insertR && $rID_adj) {
				mysql_query("COMMIT");
	...
	else if ($operation == 2) //Delete Here
	{
		...
		{
			if ((str_replace("'", "", $cbo_receive_purpose) == 2) && (str_replace("'", "", $cbo_receive_basis) == 2)) {
				if ($is_without_order == 42 || $is_without_order == 114 || $is_without_order == 135) {
...
if ($action == "mrr_popup_info") {
	...
	<script>
		function js_set_value(mrrID) {
			...
			$("#hidden_receive_basis").val(splitArr[3]);
			$("#hidden_receive_purpose").val(splitArr[4]);
            parent.emailwindow.hide();
        }
    </script>
					...
					<tbody>
						...
						<tr>
							<td align="center" height="40" valign="middle" colspan="5">
                                ...
                                <input type="hidden" id="hidden_receive_basis" value=""/>
                                <input type="hidden" id="hidden_receive_purpose" value=""/>
								<!--END -->
							</td>
						</tr>
					</tbody>
...
if ($action == "create_mrr_search_list_view") {
	...
	//$sql = "select a.id, $year_field a.recv_number_prefix_num, a.recv_number, a.supplier_id, a.challan_no, c.lc_number, a.receive_date, a.receive_basis, sum(b.cons_quantity) as receive_qnty,a.is_posted_account from inv_transaction b, inv_receive_master a left join com_btb_lc_master_details c on a.lc_no=c.id where a.id=b.mst_id and a.entry_form=1 and b.item_category=1 and b.transaction_type=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $sql_cond group by a.id, a.recv_number_prefix_num, a.recv_number, a.supplier_id, a.challan_no, a.receive_date, a.receive_basis, a.insert_date, c.lc_number, a.is_posted_account order by a.id";
	if($user_store_ids) $user_store_cond = " and a.store_id in ($user_store_ids)"; else $user_store_cond = "";

	$sql = "select a.id, $year_field a.recv_number_prefix_num, a.recv_number,a.yarn_bag_receive, a.supplier_id, a.challan_no, c.lc_number, a.receive_date, a.receive_basis, a.receive_purpose, sum(b.cons_quantity) as receive_qnty,a.is_posted_account 
	from inv_transaction b, inv_receive_master a left join com_btb_lc_master_details c on a.lc_no=c.id 
	where a.yarn_bag_receive !=1 and a.id=b.mst_id and a.entry_form=248 and b.item_category=1 and b.transaction_type=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $sql_cond $user_store_cond group by a.id, a.recv_number_prefix_num, a.recv_number, a.supplier_id, a.challan_no, a.receive_date, a.receive_basis, a.receive_purpose, a.insert_date, c.lc_number, a.is_posted_account,a.yarn_bag_receive order by a.id";

	//echo $sql;//die;
	$supplier_arr = return_library_array("select id, supplier_name from lib_supplier", 'id', 'supplier_name');
	$arr = array(2 => $supplier_arr, 6 => $receive_basis_arr);
	echo create_list_view("list_view", "Year, MRR No, Supplier Name, Challan No, LC No, Receive Date, Receive Basis, Receive Qnty", "60,70,130,120,120,120,100,100", "900", "260", 0, $sql, "js_set_value", "id,recv_number,is_posted_account,receive_basis,receive_purpose", "", 1, "0,0,supplier_id,0,0,0,receive_basis,0", $arr, "year,recv_number_prefix_num,supplier_id,challan_no,lc_number,receive_date,receive_basis,receive_qnty", "", '', '0,0,0,0,0,3,0,1');
	exit();
...
if ($action == "populate_data_from_data") {
	...
	$sql = "select id,recv_number,company_id,receive_basis,receive_purpose,receive_date, challan_date,booking_id,challan_no,store_id,lc_no,supplier_id,loan_party,exchange_rate,currency_id,lc_no,source, yarn_issue_challan_no, issue_id, remarks, pay_mode
	from inv_receive_master
	where id=$rcvID and recv_number='$mrrNo' and entry_form=248";
	//echo $sql;die;
	$res = sql_select($sql);
	foreach ($res as $row) {
		...
		else if ($row[csf("receive_basis")] == 2 && ($row[csf("receive_purpose")] == 2 || $row[csf("receive_purpose")] == 15 || $row[csf("receive_purpose")] == 38 || $row[csf("receive_purpose")] == 46))
		...
		//echo "load_supplier();\n";
		//echo "load_drop_down( 'requires/yarn_receive_controller', '" . $row[csf("receive_basis")] . "_" . $row[csf("receive_purpose")] . "_" . $wopi . "_+" . $row[csf("pay_mode")] . "','load_drop_down_company_from_eheck_wo_paymode', 'supplier' );\n";
		echo "load_drop_down( 'requires/yarn_receive_controller', '" . $row[csf("receive_basis")] . "_" . $row[csf("receive_purpose")] . "_" . $row[csf("booking_id")] . "_" . $row[csf("pay_mode")] . "','load_drop_down_company_from_eheck_wo_paymode', 'supplier' );\n";
		echo "$('#cbo_supplier').val(" . $row[csf("supplier_id")] . ");\n";
...
if ($action == "show_dtls_list_view") {
	...
	$sql = "select a.recv_number, a.receive_purpose, b.id, a.booking_id, a.receive_basis, b.pi_wo_batch_no,c.product_name_details, c.lot, b.order_uom, b.order_qnty, b.order_rate, b.order_ile_cost, b.order_amount, b.cons_amount, b.job_no, b.buyer_id, b.style_ref_no
	from inv_receive_master a, inv_transaction b, product_details_master c
	where a.id=b.mst_id and b.prod_id=c.id and b.transaction_type=1 and b.item_category=1 and a.entry_form=248 and b.status_active=1 and b.is_deleted=0 $cond";
	//echo $sql;
	$result = sql_select($sql);
	$i = 1;
	$totalQnty = 0;
	...
	<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all">
		...
		<tbody>
			<? foreach ($result as $row) {
				...
				if ($row[csf("receive_basis")] == 1)
					$wopi = return_field_value("pi_number", "com_pi_master_details", "id=" . $row[csf("booking_id")] . "");
				else if ($row[csf("receive_basis")] == 2 && ($row[csf("receive_purpose")] == 2 || $row[csf("receive_purpose")] == 15))
					$wopi = return_field_value("ydw_no", "wo_yarn_dyeing_mst", "id=" . $row[csf("booking_id")] . "");
				else
					$wopi = return_field_value("wo_number", "wo_non_order_info_mst", "id=" . $row[csf("booking_id")] . "");
				...
				<tr bgcolor="<? echo $bgcolor; ?>"
					...
					<td width="80"><p><?php echo $buyer_short_arr[$row[csf("buyer_id")]]; ?></p></td>
                    <td width="80"><p><?php echo $row[csf("style_ref_no")]; ?></p></td>
					<td width="200"><p><?php echo $row[csf("product_name_details")]; ?></p></td>
					<td width="80"><p><?php echo $row[csf("lot")]; ?></p></td>
					<td width="60" align="center"><p><?php echo $unit_of_measurement[$row[csf("order_uom")]]; ?></p></td>
					...
					<td width="50" align="right"><p><?php echo $row[csf("order_ile_cost")]; ?></p></td>
					<td width="70" align="right"><p><?php echo number_format($row[csf("order_amount")], 2); ?></p></td>
					<td align="right"><p><?php echo number_format($row[csf("cons_amount")], 2); ?></p></td>
				</tr>
				<? $i++;
			} ?>
			<tfoot>
...
if ($action == "child_form_input_data") {
	...
	$sql = "select a.currency_id, a.receive_purpose, a.exchange_rate, b.id, b.receive_basis, b.job_no, a.company_id, a.receive_purpose, b.pi_wo_batch_no, b.prod_id, b.brand_id, c.lot, b.order_uom, b.order_qnty, b.order_rate, b.cons_avg_rate, b.dye_charge, b.order_ile_cost, b.order_amount, b.cons_amount, b.no_of_bags, b.product_code, b.store_id, b.room, b.rack, b.self, b.bin_box, b.floor_id, b.cone_per_bag, b.weight_per_bag, b.weight_per_cone, b.remarks, b.supplier_id, b.job_no, b.buyer_id, b.style_ref_no,b.garments_qty
	from inv_receive_master a, inv_transaction b, product_details_master c
	...
	foreach ($result as $row) {
		...
		$sql = sql_select("select yarn_count_id,yarn_comp_type1st,yarn_comp_percent1st,yarn_comp_type2nd,yarn_comp_percent2nd,yarn_type,color from product_details_master where id=$prod_id");
		$color_id = $sql[0][csf('color')];
		...
		echo "$('#txt_challan_qty').val(" . $row[csf("garments_qty")] . ");\n";
		...
		echo "disable_enable_fields( 'cbo_yarn_count*cbocomposition1*cbo_yarn_type*txt_yarn_lot', 1, '', '' );\n";
		...
		if ($row[csf("receive_basis")] == 1 || $row[csf("receive_basis")] == 2)
		{
			if ($sql[0][csf("yarn_comp_type2nd")] == "") {
				$yarn_comp_type2nd = 0;
			} else {
				$yarn_comp_type2nd = $sql[0][csf("yarn_comp_type2nd")];
			}
			if ($sql[0][csf("yarn_comp_percent2nd")] == "") {
				$yarn_comp_percent2nd = 0;
			} else {
				$yarn_comp_percent2nd = $sql[0][csf("yarn_comp_percent2nd")];
			}

			$whereCondition = "a.id=b.prod_id and b.mst_id = c.id and a.yarn_count_id=" . $sql[0][csf("yarn_count_id")] . " and a.yarn_comp_type1st=" . $sql[0][csf("yarn_comp_type1st")] . " and a.yarn_comp_percent1st=" . $sql[0][csf("yarn_comp_percent1st")] . " and a.yarn_comp_type2nd=" . $sql[0][csf("yarn_comp_type2nd")] . " and a.yarn_comp_percent2nd=" . $yarn_comp_percent2nd . " and a.yarn_type=" . $sql[0][csf("yarn_type")] . " and a.color=" . $sql[0][csf("color")] . " and a.supplier_id=" . $row[csf("supplier_id")] . " and a.item_category_id=1 and b.transaction_type=1 and b.item_category=1 and c.entry_form=248 and b.pi_wo_batch_no=" . $row[csf("pi_wo_batch_no")] . " and b.receive_basis=" . $row[csf("receive_basis")] . " and b.job_no='" . $row[csf("job_no")] . "'";
			if ($whereCondition == "") $totalRcvQnty = 0;

			//$totalRcvQnty = return_field_value("sum(b.cons_quantity) as recv_qnty", "product_details_master a, inv_transaction b", "$whereCondition", "recv_qnty");
			//echo "select sum(b.cons_quantity) as recv_qnty from product_details_master a, inv_transaction b where $whereCondition";
			if ($row[csf("receive_basis")] == 1) {
				if($row[csf("receive_purpose")]==8)
				{
					$wo_po_qnty = return_field_value("sum(b.quantity) as qnty", "com_pi_master_details a, com_pi_item_details b, wo_non_order_info_dtls c", "a.id=b.pi_id and b.work_order_dtls_id=c.id and a.id=" . $row[csf("pi_wo_batch_no")] . " and b.count_name=" . $sql[0][csf("yarn_count_id")] . " and b.yarn_composition_item1=" . $sql[0][csf("yarn_comp_type1st")] . " and b.yarn_composition_percentage1=" . $sql[0][csf("yarn_comp_percent1st")] . " and $orderBy_cond(b.yarn_composition_item2,0)=" . $yarn_comp_type2nd . " and $orderBy_cond(b.yarn_composition_percentage2,0)=" . $yarn_comp_percent2nd . " and b.yarn_type=" . $sql[0][csf("yarn_type")] . " and b.color_id=" . $sql[0][csf("color")] . " and b.work_order_no='". $row[csf("job_no")] ."' and b.status_active=1 and b.is_deleted=0", "qnty");
				}
				else
				{
					$wo_po_qnty = return_field_value("sum(b.quantity) as qnty", "com_pi_master_details a, com_pi_item_details b, wo_non_order_info_dtls c", "a.id=b.pi_id and b.work_order_dtls_id=c.id and a.id=" . $row[csf("pi_wo_batch_no")] . " and b.count_name=" . $sql[0][csf("yarn_count_id")] . " and b.yarn_composition_item1=" . $sql[0][csf("yarn_comp_type1st")] . " and b.yarn_composition_percentage1=" . $sql[0][csf("yarn_comp_percent1st")] . " and $orderBy_cond(b.yarn_composition_item2,0)=" . $yarn_comp_type2nd . " and $orderBy_cond(b.yarn_composition_percentage2,0)=" . $yarn_comp_percent2nd . " and b.yarn_type=" . $sql[0][csf("yarn_type")] . " and b.color_id=" . $sql[0][csf("color")] . " and c.job_no='". $row[csf("job_no")] ."' and b.status_active=1 and b.is_deleted=0", "qnty");
				}
				

				//echo "select sum(b.quantity) as qnty from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and a.id=".$row[csf("pi_wo_batch_no")]." and b.count_name=".$sql[0][csf("yarn_count_id")]." and b.yarn_composition_item1=".$sql[0][csf("yarn_comp_type1st")]." and b.yarn_composition_percentage1=".$sql[0][csf("yarn_comp_percent1st")]." and $orderBy_cond(b.yarn_composition_item2,0)=".$yarn_comp_type2nd." and $orderBy_cond(b.yarn_composition_percentage2,0)=".$yarn_comp_percent2nd." and b.yarn_type=".$sql[0][csf("yarn_type")]." and b.color_id=".$sql[0][csf("color")]." and b.status_active=1 and b.is_deleted=0";
			} else if ($row[csf("receive_basis")] == 2 && $row[csf("receive_purpose")] != 2) {

				//echo "select sum(b.supplier_order_quantity) as qnty from wo_non_order_info_mst a, wo_non_order_info_dtls b where a.id=b.mst_id and a.id=" . $row[csf("pi_wo_batch_no")] . " and b.yarn_count=" . $sql[0][csf("yarn_count_id")] . " and b.yarn_comp_type1st=" . $sql[0][csf("yarn_comp_type1st")] . " and b.yarn_comp_percent1st=" . $sql[0][csf("yarn_comp_percent1st")] . " and $orderBy_cond(b.yarn_comp_type2nd,0)=" . $yarn_comp_type2nd . " and $orderBy_cond(b.yarn_comp_percent2nd,0)=" . $yarn_comp_percent2nd . " and b.yarn_type=" . $sql[0][csf("yarn_type")] . " and b.color_name=" . $sql[0][csf("color")] . " and b.status_active=1 and b.is_deleted=0";

				$wo_po_qnty = return_field_value("sum(b.supplier_order_quantity) as qnty", "wo_non_order_info_mst a, wo_non_order_info_dtls b", "a.id=b.mst_id and a.id=" . $row[csf("pi_wo_batch_no")] . " and b.yarn_count=" . $sql[0][csf("yarn_count_id")] . " and b.yarn_comp_type1st=" . $sql[0][csf("yarn_comp_type1st")] . " and b.yarn_comp_percent1st=" . $sql[0][csf("yarn_comp_percent1st")] . " and $orderBy_cond(b.yarn_comp_type2nd,0)=" . $yarn_comp_type2nd . " and $orderBy_cond(b.yarn_comp_percent2nd,0)=" . $yarn_comp_percent2nd . " and b.yarn_type=" . $sql[0][csf("yarn_type")] . " and b.color_name=" . $sql[0][csf("color")] . " and b.job_no='". $row[csf("job_no")] ."' and b.status_active=1 and b.is_deleted=0", "qnty");
			}
		...
		$orderQnty = $wo_po_qnty - $totalRcvQnty + $rcvReturnQnty + $row[csf("order_qnty")];
		if($over_rcv_payment==1 && $over_rcv_percent > 0)
		{
			$over_qnty=(($over_rcv_percent/100)*$wo_po_qnty);
			$orderOverQnty=($orderQnty+$over_qnty);
		...
		echo "$('#txt_bal_order_qty').val(" . number_format($orderOverQnty,2,'.','') . ");\n";
...
function return_product_id($yarncount, $composition_one, $composition_two, $percentage_one, $percentage_two, $yarntype, $color, $yarnlot, $prodCode, $company, $supplier, $store, $uom, $yarn_type, $composition,$cbo_receive_purpose,$hidd_pay_mode)
{
	...
	$hidd_pay_mode = str_replace("'", "", $hidd_pay_mode);
	
	if ($percentage_one == "") $percentage_one = 0;
	if ($percentage_two == "") $percentage_two = 0;
	$cbo_receive_purpose = str_replace("'", "", $cbo_receive_purpose);
	if($cbo_receive_purpose==2 || $cbo_receive_purpose==43) $dyed_type=1; else $dyed_type=2;
	if($hidd_pay_mode==3 || $hidd_pay_mode==5) $is_supp_comp=2; else $is_supp_comp=1;
...
//return "select id from product_details_master where $whereCondition";die;
	...
	if ($prodMSTID == false || $prodMSTID == "")
	{
		...
		$field_array = "id,company_id,supplier_id,item_category_id,product_name_details,lot,item_code,unit_of_measure,yarn_count_id,yarn_comp_type1st,yarn_comp_percent1st,yarn_comp_type2nd,yarn_comp_percent2nd,yarn_type,color,dyed_type,is_supp_comp";
		$data_array = "(" . $prodMSTID . "," . $company . "," . $supplier . ",1,'" . $product_name_details . "'," . $yarnlot . "," . $prodCode . "," . $uom . "," . $yarncount . "," . $composition_one . "," . $percentage_one . "," . $composition_two . "," . $percentage_two . "," . $yarntype . "," . $color . ",'" . $dyed_type . "','" . $is_supp_comp . "')";
...
if ($action == "yarn_receive_print") {
	...
	$sql = " select id, recv_number,supplier_id,currency_id,challan_no, challan_date, receive_date, exchange_rate, store_id, receive_basis,lc_no,receive_purpose,booking_id from inv_receive_master where recv_number='$data[1]'";
		...
		<table width="<? echo $table_width; ?>" align="right">
			<tr class="form_caption">
				...
				<tr style="font-size:14px">
					...
					<td width="210px"><? echo $supplier_library[$dataArray[0][csf('supplier_id')]]; ?></td>
			...
			<table align="right" cellspacing="0" width="<? echo $table_width; ?>" border="1" rules="all"
				...
				<thead bgcolor="#dddddd" align="center" style="font-size:12px">
					<tr>
						...
						<th rowspan="2" width="60">Challan Qty</th>
						...
						<th rowspan="2" width="50">Cons Per Bag</th>
				...
				$i = 1;
				$sql="select a.recv_number, a.receive_basis, a.receive_purpose, b.id, b.receive_basis, b.pi_wo_batch_no, b.cone_per_bag, c.product_name_details, c.lot, b.order_uom, b.order_qnty, b.order_rate, b.cons_avg_rate, b.dye_charge, b.order_ile_cost, b.order_amount, b.cons_amount, b.no_of_bags, b.remarks, b.buyer_id, b.style_ref_no, b.job_no, b.garments_qty as challan_qty
				from inv_receive_master a, inv_transaction b,  product_details_master c
				where a.id=b.mst_id and b.prod_id=c.id and b.transaction_type=1 and b.item_category=1 and a.entry_form=248 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $cond order by c.product_name_details";
				$sql_result=sql_select($sql);
				$total_amt_currency = 0;
				foreach ($sql_result as $row) {
					if ($i % 2 == 0) $bgcolor = "#E9F3FF"; else $bgcolor = "#FFFFFF";
					$order_qnty_val_sum += $row[csf('order_qnty')];
					$challan_qty_val_sum += $row[csf('challan_qty')];
					...
					<tr bgcolor="<? echo $bgcolor; ?>" style="font-size:12px">
						...
						<td>
							<div style="word-wrap:break-word; width:80px"><? echo $row[csf('style_ref_no')]; ?></div>
						</td>
						<td>
							<div style="word-wrap:break-word; width:80px"><? echo $row[csf('job_no')]; ?></div>
						</td>
						<td>
						...
						<td align="right"><? echo number_format($row[csf('challan_qty')], 2, '.', ',');?></td>
				...
				<tr>
					...
					<td align="right"><? echo number_format($challan_qty_val_sum, 2, '.', ','); ?></td>
					<td align="right">&nbsp;</td>
					<?
					if ($receive_pur == 2) {
						?>
...
/*if ($action = "get_receive_basis") {
	...
	$variable_set_invent = return_field_value("independent_controll", "variable_settings_inventory", "company_name='$company_id' and variable_list=20 and menu_page_id=142 and status_active=1 and is_deleted=0", "independent_controll");
	$is_independent_controlled = ($variable_set_invent == 1) ? "1,2,4" : "1,2";
	echo create_drop_down("cbo_receive_basis", 170, $receive_basis_arr, "", 1, "- Select Receive Basis -", $selected, "fn_independent(this.value)", "", $is_independent_controlled);
	exit();
}*/
