<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Roll wise Finish Sales Order To Sales Order Transfer Entry
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Roll wise Finish Sales Order To Sales Order Transfer Entry","../../", 1, 1, '','',''); 
?>	
<script>
	...
	function openmypage_systemId()
	{
		...
		var page_link = 'requires/finish_sales_order_to_order_roll_trans_controller.php?cbo_company_id='+cbo_company_id+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&action=orderToorderTransfer_popup';
		...
		emailwindow.onclose=function()
		{
			...
			get_php_form_data(transfer_id, "populate_data_from_transfer_master", "requires/finish_sales_order_to_order_roll_trans_controller" );
			...
			var floor_result = return_global_ajax_value(all_data, 'floor_list', '', 'requires/finish_sales_order_to_order_roll_trans_controller');
			...
			if(all_floor_id!=0)
			{
				...
				var room_result = return_global_ajax_value(all_data2, 'room_list', '', 'requires/finish_sales_order_to_order_roll_trans_controller');
				...
				//alert(all_data3);
				var rack_result = return_global_ajax_value(all_data3, 'rack_list', '', 'requires/finish_sales_order_to_order_roll_trans_controller');
				...
				var self_result = return_global_ajax_value(all_data4, 'shelf_list', '', 'requires/finish_sales_order_to_order_roll_trans_controller');
				...
				var bin_result = return_global_ajax_value(all_data5, 'bin_list', '', 'requires/finish_sales_order_to_order_roll_trans_controller');
				var tbl_length=$('#scanning_tbl tbody tr').length;				
	...
	function openmypage_orderNo(type)
	{
		...
		var page_link = 'requires/finish_sales_order_to_order_roll_trans_controller.php?cbo_company_id='+cbo_company_id+'&type='+type+'&txt_from_order_id='+txt_from_order_id+'&action=order_popup';
		...
		emailwindow.onclose=function()
		{
			...
			get_php_form_data(order_id+"**"+type, "populate_data_from_order", "requires/finish_sales_order_to_order_roll_trans_controller" );
			if(type=='from')
			{
				show_list_view(order_id,'show_dtls_list_view','tbl_details','requires/finish_sales_order_to_order_roll_trans_controller','');
	...
	function fnc_grey_transfer_entry(operation)
	{
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title, "finish_fabric_order_to_order_transfer_print", "requires/finish_sales_order_to_order_roll_trans_controller" ) 
			return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			if( form_validation('cbo_transfer_criteria*cbo_company_id*cbo_to_company_id*txt_transfer_date*txt_from_order_no*txt_from_batch_no*txt_to_order_no*cbo_store_name','Transfer Criteria*Company*To Company*Transfer Date*From Order No*Batch No*To Order No*To Store Name')==false )
			{
				return;
			}
			...
			$("#scanning_tbl").find('tbody tr').each(function()
			{
				...
				if($(this).find('input[name="txtcheck[]"]').attr('checked'))
				{
					...
					var rollId=$(this).find('input[name="rollId[]"]').val();
					var rollWgt=$(this).find('input[name="rollWgt[]"]').val();					
					var qcPassQnty=$(this).find('input[name="qcPassQnty[]"]').val();	
					...
					var diaWidthType=$(this).find('input[name="diaWidthType[]"]').val();
					var reprocess=$(this).find('input[name="reprocess[]"]').val();
					var prevReprocess=$(this).find('input[name="prevReprocess[]"]').val();
					var rejectQnty=$(this).find('input[name="rejectQnty[]"]').val();

					i++;
					data_all+="&barcodeNo_" + i + "='" + barcodeNo+"'"+"&rollNo_" + i + "='" + rollNo+"'"+"&batchId_" + i + "='" + batchId+"'"+"&previousToBbatchId_" + i + "='" + previousToBbatchId+"'"+"&productId_" + i + "='" + productId+"'"+"&rollId_" + i + "='" + rollId+"'"+"&rollWgt_" + i + "='" + rollWgt+"'"+"&qcPassQnty_" + i + "='" + qcPassQnty+"'"+"&hiddenTransferqnty_" + i + "='" + hiddenTransferqnty+"'"+ "&colorName_" + i + "='" + colorName+"'"+"&floor_" + i + "='" + floor+"'"+"&room_" + i + "='" + room+"'"+"&rack_" + i + "='" + rack+"'"+"&shelf_" + i + "='" + shelf+"'"+"&binbox_" + i + "='" + binbox+"'"+"&dtlsId_" + i + "='" + dtlsId+"'"+"&transIdFrom_" + i + "='" + transIdFrom+"'"+"&transIdTo_" + i + "='" + transIdTo+"'"+"&rolltableId_" + i + "='" + rolltableId+"'"+"&transRollId_" + i + "='" + transRollId+"'"+"&storeId_" + i + "='" + storeId+"'"+"&cbo_floor_to_" + i + "='" + cbo_floor_to+"'"+"&cbo_room_to_" + i + "='" + cbo_room_to+"'"+"&txt_rack_to_" + i + "='" + txt_rack_to+"'"+"&txt_shelf_to_" + i + "='" + txt_shelf_to+"'"+"&txt_bin_to_" + i + "='" + txt_bin_to+"'"+"&frombodypartId_" + i + "='" + frombodypartId+"'"+"&cbo_To_BodyPart_" + i + "='" + cboToBodyPart+"'"+"&txtRemarks_" + i + "='" + txtRemarks+"'"+"&ItemDesc_" + i + "='" + ItemDesc+"'"+"&rollRate_" + i + "='" + rollRate+"'"+"&ItemDtls_" + i + "='" + ItemDtls+"'"+"&rollAmount_" + i + "='" + rollAmount+"'"+"&toProductUp_" + i + "='" + toProductUp+"'"+"&diaWidthType_" + i + "='" + diaWidthType+"'"+"&reprocess_" + i + "='" + reprocess+"'"+"&prevReprocess_" + i + "='" + prevReprocess+"'"+"&rejectQnty_" + i + "='" + rejectQnty+"'";
			...
			var dataString = "txt_system_id*cbo_company_id*txt_transfer_date*txt_challan_no*txt_from_batch_no*txt_from_order_id*txt_to_order_id*txt_to_order_no*update_id*cbo_from_store_name*cbo_store_name*cbo_transfer_criteria*cbo_to_company_id";
			...
			http.open("POST","requires/finish_sales_order_to_order_roll_trans_controller.php",true);
	...
	function fnc_grey_transfer_entry_reponse()
	{
		...
		{	  		
			...
			if(reponse[0]==0 || reponse[0]==1)
			{
				...
				get_php_form_data(reponse[1], "populate_data_from_transfer_master", "requires/finish_sales_order_to_order_roll_trans_controller" );
				...
				disable_enable_fields( 'cbo_transfer_criteria*cbo_company_id*cbo_to_company_id*txt_from_order_no*txt_to_order_no*cbo_from_store_name', 1, '', '' );
			}	
			release_freezing();
		}
	}
	...
	function openmypage_orderInfo(type)
	{
		...
		var title = 'Order Info';	
		var page_link = 'requires/finish_sales_order_to_order_roll_trans_controller.php?txt_order_no='+txt_order_no+'&txt_order_id='+txt_order_id+'&type='+type+'&action=orderInfo_popup';
	...
	function fnc_company_onchang_reset(company)
	{
		...
		if($("#cbo_transfer_criteria").val() != 1)
		{
			...
			load_drop_down( 'requires/finish_sales_order_to_order_roll_trans_controller',$('#cbo_transfer_criteria').val()+'_'+company, 'load_drop_store_to', 'store_td' );
		}
		else
		{
		...
		$.ajax({
			url: 'requires/finish_sales_order_to_order_roll_trans_controller.php',
			type: 'POST',
			data: page_link,
	...
	function fnc_onchang_reset(from_store)
	{
		var prev_store = $('#hid_from_store_id').val();
		if (prev_store>0 && from_store!=prev_store) 
		{
			alert("Multiple Store Not Allowed");
			$('#cbo_from_store_name').val(prev_store); return;
		}
		$('#hid_from_store_id').val(from_store);
	}

	function company_on_change(company)
	{		
		load_drop_down( 'requires/finish_sales_order_to_order_roll_trans_controller',$('#cbo_transfer_criteria').val()+'_'+company, 'load_drop_store_from', 'from_store_td' );
		fnc_reset_form();
	...
	function fn_load_room(floor_id, sequenceNo)
	{
		...
		//alert(all_data);return;
		var room_result = return_global_ajax_value(all_data, 'room_list', '', 'requires/finish_sales_order_to_order_roll_trans_controller');
		var tbl_length=$('#scanning_tbl tbody tr').length;
	...
	function fn_load_rack(room_id, sequenceNo)
	{
		...
		//alert(all_data);return;
		var rack_result = return_global_ajax_value(all_data, 'rack_list', '', 'requires/finish_sales_order_to_order_roll_trans_controller');
		var tbl_length=$('#scanning_tbl tbody tr').length;
	...
	function fn_load_shelf(rack_id, sequenceNo)
	{
		...
		//alert(all_data);return;
		var shelf_result = return_global_ajax_value(all_data, 'shelf_list', '', 'requires/finish_sales_order_to_order_roll_trans_controller');
		var tbl_length=$('#scanning_tbl tbody tr').length;
	...
	function fn_load_bin(shelf_id, sequenceNo)
	{
		...
		//alert(all_data);return;
		var shelf_result = return_global_ajax_value(all_data, 'bin_list', '', 'requires/finish_sales_order_to_order_roll_trans_controller');
		var tbl_length=$('#scanning_tbl tbody tr').length;
		//alert(shelf_result+"="+tbl_length);//return;
		var JSONObject = JSON.parse(shelf_result);
		for(var i=sequenceNo; i<=tbl_length; i++)
	...
	function load_bodypart_list()
	{
		...
		//alert(all_data);return;
		var shelf_result = return_global_ajax_value(all_data, 'bodypart_list', '', 'requires/finish_sales_order_to_order_roll_trans_controller');
		var tbl_length=$('#scanning_tbl tbody tr').length;
		//alert(shelf_result+"="+tbl_length);//return;
		var JSONObject = JSON.parse(shelf_result);
	...
	function active_inactive(str) // Onchange Transfer Criteria
	{
		$('#cbo_to_company_id').val(0);
		$('#cbo_company_id').val(0);
		load_drop_down( 'requires/finish_sales_order_to_order_roll_trans_controller',$('#cbo_transfer_criteria').val()+'_'+str, 'load_drop_from_store_balnk', 'from_store_td' );
		load_drop_down( 'requires/finish_sales_order_to_order_roll_trans_controller',$('#cbo_transfer_criteria').val()+'_'+str, 'load_drop_store_balnk', 'store_td' );
		if(str==1)
		{
	...
	function to_company_on_change(to_company)
	{
		...
		load_drop_down( 'requires/finish_sales_order_to_order_roll_trans_controller',$('#cbo_transfer_criteria').val()+'_'+to_company, 'load_drop_store_to', 'store_td' );
	}
	...
	{ 
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/finish_sales_order_to_order_roll_trans_controller.php?company_id='+company_id+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&cbo_store_name='+cbo_store_name+'&action=barcode_popup','Barcode Popup', 'width=850px,height=350px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
	...
	function create_row(is_update,barcode_no)
	{
		...
		if(is_update==0) // save
		{
			...
			var barcode_data=return_global_ajax_value(bar_code, 'show_dtls_list_view', '', 'requires/finish_sales_order_to_order_roll_trans_controller');
			...
			for(var k=0;k<barcode_data_ref.length;k++)
			{
				...
				var prod_id=barcode_data_all[3];
				var fabric_dtls=barcode_data_all[4]; // Fabric Description
				var color_names=barcode_data_all[5];
				var itemDesc=barcode_data_all[6];// description_id*gsm*width
				var amount=barcode_data_all[7];
				var roll_rate=barcode_data_all[8];
				var roll_id=barcode_data_all[9];//roll_id_prev
				var qnty=barcode_data_all[10];
				var qc_pass_qnty=barcode_data_all[11];
				var color_id=barcode_data_all[12];
				var body_part_id=barcode_data_all[13];
				var floor_id=barcode_data_all[14];
				var room_id=barcode_data_all[15];
				var rack=barcode_data_all[16];
				var shelf=barcode_data_all[17];
				var bin_box=barcode_data_all[18];
				var transRollId=barcode_data_all[19]; // recv source roll table id
				var store_id=barcode_data_all[20];
				var deterId=barcode_data_all[21];
				var batch_id=barcode_data_all[22];
				var batch_no=barcode_data_all[23];
				var dia_width_type=barcode_data_all[24];
				var reprocess=barcode_data_all[25];
				var prev_reprocess=barcode_data_all[26];
				var reject_qnty=barcode_data_all[27];

				var from_batch_id = $('#txt_from_batch_id').val();
				if (from_batch_id=="") 
				{
					$("#txt_from_batch_no").val(batch_no);
					$("#txt_from_batch_id").val(batch_id);
				}				

				var txt_from_order_no = $('#txt_from_order_no').val();
				if (txt_from_order_no=="") 
				{
					get_php_form_data(po_breakdown_id+"**"+'from', "populate_data_from_order", "requires/finish_sales_order_to_order_roll_trans_controller");
					if (cbo_transfer_criteria==2)
					{
						get_php_form_data(po_breakdown_id+"**"+'to', "populate_data_from_order", "requires/finish_sales_order_to_order_roll_trans_controller");
					}
				}

				var txt_from_batch_id = $('#txt_from_batch_id').val();
				// alert(txt_from_batch_id+'!='+batch_id);
				if (txt_from_batch_id!=batch_id) 
				{
					alert('Multiple Batch No Not Allowed');return;
				}

				var txt_from_order_id = $('#txt_from_order_id').val();
				// alert(txt_from_order_id+'!='+po_breakdown_id);
				if (txt_from_order_id!=po_breakdown_id) 
				{
					alert('Multiple Sales Order Not Allowed');return;
				}

				//var store_id=barcode_data_all[20];//from store id
				...
				$("#qcPassQnty_"+row_num).val(qc_pass_qnty);
				...
				$("#diaWidthType_"+row_num).val(dia_width_type);
				$("#reprocess_"+row_num).val(reprocess);
				$("#prevReprocess_"+row_num).val(prev_reprocess);
		...
		{
			var barcode_data=return_global_ajax_value(bar_code+"**"+$('#txt_from_order_id').val()+"**"+$('#cbo_company_id').val()+"**"+$('#cbo_store_name').val()+"**"+$('#txt_to_order_id').val()+"**"+$('#cbo_transfer_criteria').val(), 'show_transfer_listview', '', 'requires/finish_sales_order_to_order_roll_trans_controller');
			//alert(barcode_data);return;
			barcode_data=barcode_data.trim();//Remove space
			setFilterGrid('scanning_tbl',-1,tableFilters);
			var barcode_data_all=new Array(); var barcode_data_ref=new Array();
			...
			for(var k=0;k<barcode_data_ref.length;k++)
			{
				...
				var color_id=barcode_data_all[12];
				var body_part_id=barcode_data_all[13];
				var from_floor=barcode_data_all[14];
				var from_room=barcode_data_all[15];
				var from_rack=barcode_data_all[16];
				var from_self=barcode_data_all[17];
				var from_bin_box=barcode_data_all[18];
				var transRollId=barcode_data_all[19];//roll_id....
				var store_id_from=barcode_data_all[20];
				var to_prod_id=barcode_data_all[21];
				var dtls_id=barcode_data_all[22];
				var from_trans_id=barcode_data_all[23];
				var to_trans_id=barcode_data_all[24];
				var rolltableId=barcode_data_all[25];//...
				var to_body_part=barcode_data_all[26];
				var to_floor_id=barcode_data_all[27];
				var to_room=barcode_data_all[28];
				var to_rack=barcode_data_all[29];
				var to_shelf=barcode_data_all[30];
				var to_bin_box=barcode_data_all[31];
				var batch_id=barcode_data_all[32];
				var batch_no=barcode_data_all[33];
				var to_batch_no=barcode_data_all[34];
				var transfer_qnty=barcode_data_all[35];
				var checked_flag=barcode_data_all[36];
				var disabled_flag=barcode_data_all[37];
				var dia_width_type=barcode_data_all[38];
				var reprocess=barcode_data_all[39];
				var prev_reprocess=barcode_data_all[40];
				var reject_qnty=barcode_data_all[41];
				...
				$("#barcodeNo_"+row_num).val(barcode_no);
				$("#batchId_"+row_num).val(batch_id);
				$("#previousToBbatchId_"+row_num).val(to_batch_no);
				$("#rollNo_"+row_num).val(roll_no);
				$("#productId_"+row_num).val(prod_id);
				$("#toProductUp_"+row_num).val(to_prod_id);// to product id for update event
				$("#orderId_"+row_num).val(po_breakdown_id);
				$("#ItemDesc_"+row_num).val(itemDesc); // description_id*gsm*width
				$("#ItemDtls_"+row_num).val(fabric_dtls); // contruction_composition_gsm_dia
				$("#rollAmount_"+row_num).val(amount);
				$("#rollRate_"+row_num).val(roll_rate);
				$("#rollId_"+row_num).val(roll_id); //roll_id_prev
				$("#rollWgt_"+row_num).val(qnty);
	...
	function fnc_reset_form()
	{
		...
		var html='<tr id="tr__1" align="center" valign="middle"><td id="button_1" align="center" width="40"><input type="checkbox" name="txtcheck[]" id="txtcheck_1" onClick="show_selected_total(1)"/><input type="hidden" name="barcodeNo[]" id="barcodeNo_1"/><input type="hidden" name="batchId[]" id="batchId_1"/><input type="hidden" name="previousToBbatchId[]" id="previousToBbatchId_1"/><input type="hidden" name="rollNo[]" id="rollNo_1"/><input type="hidden" name="productId[]" id="productId_1"/><input type="hidden" name="toProductUp[]" id="toProductUp_1"><input type="hidden" name="orderId[]" id="orderId_1"/><input type="hidden" name="ItemDesc[]" id="ItemDesc_1"/><input type="hidden" name="ItemDtls[]" id="ItemDtls_1"/><input type="hidden" name="rollAmount[]" id="rollAmount_1"/><input type="hidden" name="rollRate[]" id="rollRate_1"/><input type="hidden" name="rollId[]" id="rollId_1"/><input type="hidden" name="rollWgt[]" id="rollWgt_1"/><input type="hidden" name="qcPassQnty[]" id="qcPassQnty_1"/><input type="hidden" name="hiddenTransferqnty[]" id="hiddenTransferqnty_1"/><input type="hidden" name="colorName[]" id="colorName_1"/><input type="hidden" name="frombodypartId[]" id="frombodypartId_1"/><input type="hidden" name="floor[]" id="floor_1"/><input type="hidden" name="room[]" id="room_1"/><input type="hidden" name="rack[]" id="rack_1"/><input type="hidden" name="shelf[]" id="shelf_1"/><input type="hidden" name="binbox[]" id="binbox_1"/><input type="hidden" name="dtlsId[]" id="dtlsId_1"/><input type="hidden" name="transIdFrom[]" id="transIdFrom_1"/><input type="hidden" name="transIdTo[]" id="transIdTo_1"/><input type="hidden" name="rolltableId[]" id="rolltableId_1"/><input type="hidden" name="transRollId[]" id="transRollId_1"/><input type="hidden" name="storeId[]" id="storeId_1"/><input type="hidden" name="txtRemarks[]" id="txtRemarks_1"/><input type="hidden" name="diaWidthType[]" id="diaWidthType_1"/><input type="hidden" name="reprocess[]" id="reprocess_1"/><input type="hidden" name="prevReprocess[]" id="prevReprocess_1"/><input type="hidden" name="rejectQnty[]" id="rejectQnty_1"/></td><td width="40" id="sl_1"></td><td width="80" id="barcode_1"></td><td width="80" id="batchNo_1"></td><td width="50" id="roll_1"></td><td width="60" id="txtProdId_1"></td><td width="180" id="txtFabricDetails_1"></td><td width="80" id="txtColorName_1"></td><td width="100" align="center" id="toBodyPartTd_1"><? echo create_drop_down( "cbo_To_BodyPart_1", 100,$blank_array,"", 1, "--Select--", 0, "copyBodyPart(\'1_0\')",0,"","","","","","","cboToBodyPart[]","onchange_void" );?></td><td width="50" align="center" id="floor_td_to" class="floor_td_to"><? echo create_drop_down( "cbo_floor_to_1", 50,$blank_array,"", 1, "--Select--", 0, "fn_load_room(this.value, 1); copy_all(\'1_0\'); reset_room_rack_shelf(1,\'cbo_floor_to\');",0,"","","","","","","cbo_floor_to[]" ,"onchange_void"); ?></td><td width="50" align="center" id="room_td_to"><? echo create_drop_down( "cbo_room_to_1", 50,$blank_array,"", 1, "--Select--", 0, "fn_load_rack(this.value, 1); copy_all(\'1_1\'); reset_room_rack_shelf(1,\'cbo_room_to\');",0,"","","","","","","cbo_room_to[]","onchange_void" ); ?></td><td width="50" align="center" id="rack_td_to"><? echo create_drop_down( "txt_rack_to_1", 50,$blank_array,"", 1, "--Select--", 0, "fn_load_shelf(this.value, 1); copy_all(\'1_2\'); reset_room_rack_shelf(1,\'txt_rack_to\');",0,"","","","","","","txt_rack_to[]","onchange_void" ); ?></td><td width="50" align="center" id="shelf_td_to"><? echo create_drop_down( "txt_shelf_to_1", 50,$blank_array,"", 1, "--Select--", 0, "fn_load_bin(this.value, 1); copy_all(\'1_3\');reset_room_rack_shelf(1,\'txt_shelf_to\');",0,"","","","","","","txt_shelf_to[]","onchange_void" ); ?></td><td width="50" align="center" id="bin_td_to"><? echo create_drop_down( "txt_bin_to_1", 50,$blank_array,"", 1, "--Select--", 0, "copy_all(\'1_4\');",0,"","","","","","","txt_bin_to[]","onchange_void" ); ?></td><td width="80" id="txtRollWgt_1" align="right"></td><td width="" id="remarks_1"></td></tr>';
...
<div align="center" style="width:100%;">
    ...
    <form name="transferEntry_1" id="transferEntry_1" autocomplete="off" >
        ...
        <legend>Roll wise Grey Fabric Sales Order To Sales Order Transfer Entry</legend>
            <fieldset style="width:750px;">
                <table width="740" cellspacing="2" cellpadding="2" border="0" id="tbl_master">
                    ...
                    <tr>
                        ...
                        <td>
                            <?
                                echo create_drop_down("cbo_transfer_criteria", 160,$item_transfer_criteria,"", 1,"-- Select --",'0',"active_inactive(this.value);",'','2,4');
                            ?>
                        </td>
                        ...
                        <td>
                            <? 
                                echo create_drop_down( "cbo_company_id", 160, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "--Select Company--", 0, "fnc_company_onchang_reset(this.value);company_on_change(this.value)" );
                            ?>
                        </td>
                        <td>To Company</td>
                        <td>
                        ...
                        <legend>From Sales Order</legend>
                            <table id="from_order_info" cellpadding="0" cellspacing="1" width="100%">										
                                ...
                                <tr>
                                    ...
                                    <td><input type="text" name="txt_from_batch_no" id="txt_from_batch_no" class="text_boxes" style="width:150px;" disabled="disabled" placeholder="Display" /></td>
                                    <input type="hidden" name="txt_from_batch_id" id="txt_from_batch_id" readonly>
                                </tr>
                                 <tr>
                                ...
                                <tr>
									...
									<td id="from_store_td">
										<?
											echo create_drop_down( "cbo_from_store_name", 160, $blank_array,"", 1, "--Select Store--", 0, "fnc_onchang_reset(this.value);" );
										?>
									</td>
									<input type="hidden" name="hid_from_store_id" id="hid_from_store_id" readonly>
								</tr>
                            </table>
                        </fieldset>
                    </td>
            ...
            <fieldset style="width:1270px;text-align:left">
				...
				<table cellpadding="0" width="1240" cellspacing="0" border="1" id="scanning_tbl_top" class="rpt_table" rules="all">
                    <thead>
                ...
                <div style="width:1270px; max-height:250px; overflow-y:scroll" align="left">
                 	<table cellpadding="0" cellspacing="0" width="1240" border="1" id="scanning_tbl" rules="all" class="rpt_table">
                    	<tbody>
                    		<tr id="tr__1" align="center" valign="middle">
                            	<td id="button_1" align="center" width="40">
                            		...
                            		<input type="hidden" name="previousToBbatchId[]" id="previousToBbatchId_1"/>
									...
									<input type="hidden" name="qcPassQnty[]" id="qcPassQnty_1"/>
									<input type="hidden" name="hiddenTransferqnty[]" id="hiddenTransferqnty_1" />
                ...
                <table cellpadding="0" cellspacing="0" width="1240" border="1" rules="all" class="rpt_table" align="left">
                    <tfoot>
                    	<tr>
	                    	<th width="40"></th>
	                    	<th width="40"></th>
