...
<script>
	<?
	$data_arr= json_encode( $_SESSION['logic_erp']['data_arr'][231] );
	...
	function openmypage_fso() 
	{
		...
		var page_link = 'requires/knit_finish_fabric_receive_by_garments_multiple_controller.php?cbo_company_id='+cbo_company_id+'&cbo_within_group='+cbo_within_group+'&cbo_party_id='+cbo_party_id+'&action=fabric_sales_order_popup';
	...
	function fnc_show_garments()
	{
		...
		if( form_validation('cbo_company_id*cbo_within_group*txt_fso_no','Company Name*Withing Group*FSO No')==false )
		{
			return;
		}	
		...
		var dataString = "&cbo_company_id="+cbo_company_id+"&cbo_within_group="+cbo_within_group+"&cbo_party_id="+cbo_party_id+"&txt_vehical_no="+txt_vehical_no+"&txt_fso_no="+txt_fso_no+"&hdn_fso_id="+hdn_fso_id+"&hdn_challan_id="+hdn_challan_id+"&txt_po_job="+txt_po_job+"&txt_dl_no="+txt_dl_no+"&txt_transport="+txt_transport+"&txt_mobile_no="+txt_mobile_no+"&txt_gate_pass_no="+txt_gate_pass_no+"&txt_remarks="+txt_remarks+"&update_id="+update_id;
		
		var data="action=list_view_garments"+dataString;
		
		freeze_window(3);
		
		http.open("POST","requires/knit_finish_fabric_receive_by_garments_multiple_controller.php",true);
	...
	function fnc_finish_receive_entry( operation )
	{
		...
		if(operation==4)
		{
			...
			generate_report_file( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_system_id').val(),'finish_fabric_receive_print','requires/knit_finish_fabric_receive_by_garments_multiple_controller');
			show_msg("3");
			return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			{*/
				...
				else 
				{
					...
					var delivery_to 		= $("#txt_delivery_to").val();
	...
	function fnc_finish_receive_entry_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			if((reponse[0]==0 || reponse[0]==1 || reponse[0]==2))
			{
				...
				set_button_status(1, permission, 'fnc_finish_receive_entry',1,1);
			}
	...
	function openmypage_systemid()
	{
		...
		else
		{
			...
			var page_link = 'requires/knit_finish_fabric_receive_by_garments_multiple_controller.php?cbo_company_id='+cbo_company_id+'&action=systemId_popup';
			...
			emailwindow.onclose=function()
			{
				...
				get_php_form_data(hidden_sys_id, "populate_data_from_finish_fabric", "requires/knit_finish_fabric_receive_by_garments_multiple_controller" );
				fnc_show_garments();
	...
	{
		window.open("requires/knit_finish_fabric_receive_by_garments_multiple_controller.php?data=" + data+'&action='+action, true );
	}	
		...
		<form name="finishFabricEntry_1" id="finishFabricEntry_1" autocomplete="off" >
					...
					<legend>Finish Fabric Receive Entry</legend>
						<table cellpadding="0" cellspacing="2" width="810" border="0">
							...
							<tr>
								...
								<td align="center">
                                    <?
									echo create_drop_down("cbo_within_group",160,$yes_no,"", 0, "-- Select --", 1,"load_drop_down( 'requires/knit_finish_fabric_receive_by_garments_multiple_controller', this.value+'_'+document.getElementById('cbo_company_id').value, 'load_drop_down_party','cbo_party');reset_withing_group();",0,'');
									?>
								</td>
								...
								<td id="cbo_party">
                                    <?
									echo create_drop_down( "cbo_party_id", 160, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "--Select Party--", 0, "partyChange()" );
									?>
								</td>
							...
							<tr>
								<td>Delivery To</td>                                              
								<td align="left"> 
									<input type="text" name="txt_delivery_to" id="txt_delivery_to" class="text_boxes" style="width:150px;">
								</td>
								<td>Delivery Address</td>                                              
								<td colspan="3" align="left"> 
									<input type="text" name="txt_delivery_address" id="txt_delivery_address" class="text_boxes" style="width:400px;margin-left: 18px;">
								</td>
                            </tr>
                            <tr> 
                            	<td class="must_entry_caption">FSO No</td>
								<td>
									<input type="text" name="txt_fso_no" id="txt_fso_no" class="text_boxes" style="width:145px;" placeholder="Browse" onDblClick="openmypage_fso();" readonly />
                               ...
                               <td>Driver Name</td>                                              
							   <td> 
									<input type="text" name="txt_driver_name" id="txt_driver_name" class="text_boxes" style="width:150px;" autocomplete="on">
								</td> 
                            ...
                            <tr>
                                ...
                                <td>DL No</td>                                              
								<td> 
									<input type="text" name="txt_dl_no" id="txt_dl_no" class="text_boxes" style="width:145px;" autocomplete="on">
								</td> 
                                <td>Transport</td>                                              
								<td align="center"> 
									<input type="text" name="txt_transport" id="txt_transport" class="text_boxes" style="width:145px;" autocomplete="on">
								</td> 
                                
                                <td>Mobile No</td>           
                                <td > 
									<input type="text" name="txt_mobile_no" id="txt_mobile_no" class="text_boxes" style="width:150px;" autocomplete="on">
								</td>
                            ...
                            <tr>
                            	<td>Gate Pass No</td>           
                                <td> 
									<input type="text" name="txt_gate_pass_no" id="txt_gate_pass_no" class="text_boxes" style="width:145px;" autocomplete="on" >
								</td>
								<td>Remarks</td>                                              
								<td colspan="3" align="left"> 
									<input type="text" name="txt_remarks" id="txt_remarks" class="text_boxes" style="width:325px; margin-left: 20px;">
                                    <input type="button" id="show_button" class="formbutton" style="width:70px" value="Show" onClick="fnc_show_garments(1)" />
								</td> 
            ...
            <tr>
                	<td colspan="6" align="center" class="button_container">
                		<? 
                		echo load_submit_buttons($permission, "fnc_finish_receive_entry", 0,1,"reset_form('finishFabricEntry_1','list_view_container','','','')",1);
                		?>
