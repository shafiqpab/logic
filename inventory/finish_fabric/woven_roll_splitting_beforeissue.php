...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Woven Roll Splitting Before Issue","../../", 1, 1, $unicode,'','');
?>	
<script>
	...
	{ 
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/woven_roll_splitting_beforeissue_controller.php?action=mrr_popup','Receive Popup', 'width=800px,height=350px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function()
		{
			...
			if(barcode_nos!="")
			{
				get_php_form_data( barcodeSels, "load_barcode_mst_form", "requires/woven_roll_splitting_beforeissue_controller" );
				...
				show_list_view(barcode_data[3], 'roll_details_update', 'scanning_tbody','requires/woven_roll_splitting_beforeissue_controller', '' );
			}
		}
	}
	...
	{ 
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/woven_roll_splitting_beforeissue_controller.php?&action=barcode_popup','Barcode Popup', 'width=780px,height=350px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
			...
			if(barcode_nos!="")
			{
				get_php_form_data( barcode_nos, "load_barcode_mst_form", "requires/woven_roll_splitting_beforeissue_controller" );
	...
	function barcode_scan_dtls(is_update,barcode_no)
	{
		...
		var response=trim(return_global_ajax_value( barcode_no, 'check_barcode_no', '', 'requires/woven_roll_splitting_beforeissue_controller')).split('**');
		...
		{
			var response_sales = return_global_ajax_value( barcode_no, 'check_sales_order', '', 'requires/woven_roll_splitting_beforeissue_controller');
	...
	{
		window.open("requires/woven_roll_splitting_beforeissue_controller.php?data=" + data+'&action='+action, true );
	}
	...
	function fnc_roll_split_entry( operation )
	{
		...
		$("#scanning_tbl_top").find('tbody tr').each(function()
		{
			...
			'&rollWgt_' + j + '=' + rollWgt + '&hideRollWgt_' + j + '=' + hideRollWgt  + '&update_dtls_id_' + j + '='+update_dtls_id ;
		});
	...
	function fnc_roll_split_entry_info()
	{
		...
		{			
			...
			if((response[0]==0 || response[0]==1))
			{
				...
				show_list_view(response[4], 'roll_details_update', 'scanning_tbody','requires/woven_roll_splitting_beforeissue_controller', '' );
				set_button_status(1, permission, 'fnc_roll_split_entry',1);
	...
	function fnc_bundle_report(type)
	{
		...
		if (type==4)
		{
			...
			var url=return_ajax_request_value(data, "direct_print_barcode", "requires/woven_roll_splitting_beforeissue_controller");
			window.open(url,"##");	return;
		}
		if (type==1)
		{
			window.open("requires/woven_roll_splitting_beforeissue_controller.php?data=" + data+'&action=report_barcode_generation', true );
		}
		if (type==2)
		{
			//window.open("requires/woven_roll_splitting_beforeissue_controller.php?data=" + data+'&action=report_barcode_generation_128', true );
			var url=return_ajax_request_value(data, "report_barcode_generation_128", "requires/woven_roll_splitting_beforeissue_controller");
		}

		if (type==3) // Barcode N
		{
			
			//window.open("requires/woven_roll_splitting_beforeissue_controller.php?data=" + data+'&action=print_barcode_one_128_v3', true );
			var url=return_ajax_request_value(data, "print_barcode_one_128_v3", "requires/woven_roll_splitting_beforeissue_controller");
		...
		if (type==5) // Barcode 128 v3
		{
			...
			var txt_bar_code_num=$("#txt_bar_code_num").val();
			data=data+"***"+productionDtlsId+"***"+txt_bar_code_num;
			var url=return_ajax_request_value(data, "print_barcode_one_128_v4", "requires/woven_roll_splitting_beforeissue_controller");
		}
		
				
		//var url=return_ajax_request_value(data, "print_report_bundle_barcode", "requires/woven_roll_splitting_beforeissue_controller");
		window.open(url,"##");		
	}
	...
	function fnc_send_printer_text()
	{
		...
		var url=return_ajax_request_value(data, "report_barcode_text_file", "requires/woven_roll_splitting_beforeissue_controller");		
    ...
    <div align="center" style="width:100%;">
        ...
        <form name="rollscanning_1" id="rollscanning_1"  autocomplete="off"  >
            <fieldset style="width:810px;">
                	...
                	<tr>
                     ...
                     <tr>
                        ...
                        <td>
                         ...
                         <input type="text" name="txt_fabric_description" id="txt_fabric_description" class="text_boxes" style="width:400px;"  placeholder="Display" disabled/>
                         </td>
                    </tr>
                    <tr>
	                    ...
	                    <td  align="left">
	                             ...
	                             <input type="hidden" name="txt_job_id" id="txt_job_id" class="text_boxes" style="width:140px;" disabled/>
	                    </td>
	                    <td  align="right" id="po_booking_td">Order</td>
	                    <td  align="left">
                     ...
                     <tr>
                        ...
                        <td  align="left">
                         ...
                         <td  align="left">
                                ...
                                <input type="hidden" name="hidden_shrinkage_shade" id="hidden_shrinkage_shade"/>
                     ...
                     <tr>
                        ...
                        <td colspan="5" align="left">
                        	<input type="text" name="txt_remarks" id="txt_remarks" class="text_boxes" style="width:645px;" placeholder="Remarks" />
                        </td>
                     </tr>
                ...
                <br>
                <table cellpadding="0" width="700" cellspacing="0" border="1" id="scanning_tbl_top" class="rpt_table" rules="all">
                    <thead>
                    	<th width="40">SL</th>
