<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Woven Finish fabric receive return
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Woven Finish Fabric Receive Return Info","../../", 1, 1, $unicode,1,1); 
$con = connect();
?>	
...
<?
	if(isset($_SESSION['logic_erp']['data_arr'][202]))
	{
		$data_arr= json_encode( $_SESSION['logic_erp']['data_arr'][202] );
...
function open_returnpopup()
{
	...
	var page_link='requires/woven_finish_fab_receive_rtn_controller.php?action=return_number_popup&company='+company; 
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(returnNumber[0], "populate_master_from_data", "requires/woven_finish_fab_receive_rtn_controller"); 
		get_php_form_data(returnNumber[2], "populate_data_from_data", "requires/woven_finish_fab_receive_rtn_controller");  
		//list view call here
		show_list_view(returnNumber[0],'show_dtls_list_view','list_container_yarn','requires/woven_finish_fab_receive_rtn_controller','');
...
function open_mrrpopup()
{
	...
	var company = $("#cbo_company_id").val();	
	var page_link='requires/woven_finish_fab_receive_rtn_controller.php?action=mrr_popup&company='+company; 
	...
	emailwindow.onclose=function()
	{
		...
		var mrrNumber=this.contentDoc.getElementById("hidden_recv_number").value.split("_"); // mrr number
  		// master part call here
		$("#pi_id").val(mrrNumber[2]);
		$("#txt_pi_no").val('');
		
		get_php_form_data(mrrNumber[0], "populate_data_from_data", "requires/woven_finish_fab_receive_rtn_controller");  
 		$("#tbl_child").find('input,select').val('');
 	}
}
...
function openmypage_pi()
{
	...
	page_link='requires/woven_finish_fab_receive_rtn_controller.php?action=pi_popup&company='+company;
...
function openmypage_rtn_qty()
{
	...
	var page_link = 'requires/woven_finish_fab_receive_rtn_controller.php?cbo_company_id='+cbo_company_id+'&txt_received_id='+txt_received_id+'&txt_prod_id='+txt_prod_id+'&hidden_receive_trans_id='+hidden_receive_trans_id+'&txt_break_qnty='+txt_break_qnty+'&txt_break_roll='+txt_break_roll+'&txt_batch_lot='+txt_batch_lot +'&txt_batch_id='+ txt_batch_id+'&hidden_color_id='+hidden_color_id+'&cbo_body_part='+cbo_body_part+'&cbo_store_name='+cbo_store_name+'&txt_width='+txt_width+'&txt_weight='+txt_weight+'&cbo_weight_type='+cbo_weight_type+'&txt_cutable_width='+txt_cutable_width+'&txt_floor='+txt_floor+'&txt_room='+txt_room+'&txt_rack='+txt_rack+'&txt_shelf='+txt_shelf+'&txt_bin='+txt_bin+'&cbouom='+cbouom+'&action='+actionName+'&fabric_desc_id='+fabric_desc_id+'&hidden_fab_ref='+hidden_fab_ref+'&hidden_rd_no='+hidden_rd_no+'&hidden_rate='+hidden_rate+'&po_popup_patern_variable='+po_popup_patern_variable+'&save_data='+save_data+'&update_id='+update_id; 
...
function fnc_yarn_receive_return_entry(operation)
{
	if(operation==4)
	{
		 ...
		 print_report( $('#cbo_company_id').val()+'*'+$('#issue_mst_id').val()+'*'+$('#cbo_return_to').val()+'*'+report_title, "yarn_receive_return_print", "requires/woven_finish_fab_receive_rtn_controller" ) 
		 return;
	}
	else if(operation==2)
	{
	...
	else
	{
		...
		if( form_validation('cbo_company_id*txt_return_date*txt_mrr_no*txt_item_description*txt_return_qnty','Company Name*Return Date*MRR Number*Item Description*Return Quantity')==false )
		{
			return;
		}
		...
		var dataString = "txt_return_no*cbo_company_id*txt_return_date*txt_received_id*txt_mrr_no*cbo_return_to*txt_pi_no*pi_id*txt_item_description*txt_prod_id*txt_return_qnty*cbo_uom*txt_break_qnty*txt_break_roll*txt_order_id_all*txt_remarks*txt_roll*hidden_receive_trans_id*before_prod_id*update_id*issue_mst_id*prev_return_qnty*before_receive_trans_id*txt_global_stock*hidden_batch_id*cbo_body_part*cbo_store_name*cbo_floor*cbo_room*cbo_bin*txt_rack*txt_shelf*txt_batch_no*txt_weight*cbo_weight_type*txt_width*txt_cutable_width*hidden_fabrication_id*hidden_rd_no*hidden_rate*hidden_update_amount*hidden_fab_ref*save_data";
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string(dataString,"../../");

		freeze_window(operation);
		http.open("POST","requires/woven_finish_fab_receive_rtn_controller.php",true);
...
{	
	...
	{	  	
		...
		else if(reponse[0]==0 || reponse[0]==1)
		{
			...
			disable_enable_fields( 'txt_return_no', 0, "", "" ); // disable false
			show_list_view(reponse[2],'show_dtls_list_view','list_container_yarn','requires/woven_finish_fab_receive_rtn_controller','');		
			//child form reset here after save data-------------//
		
			reset_form('','','update_id*before_prod_id*save_data*all_po_id*hidden_rate*hidden_update_amount*hidden_rd_no','','');
...
{
	var report_title=$( "div.form_caption" ).html(); 
	print_report( $('#cbo_company_id').val()+'*'+$('#issue_mst_id').val()+'*'+$('#cbo_return_to').val()+'*'+report_title, "yarn_receive_return_print_2", "requires/woven_finish_fab_receive_rtn_controller" ) 
	return;
}
                 	...
                 	<fieldset style="width:830px;">                                       
                        <table  width="800" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                           ...
                           <tr>
                                ...
                                <td width="170">
									<? 
                                     	echo create_drop_down( "cbo_company_id", 170, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "get_php_form_data(this.value,'load_variable_settings','requires/woven_finish_fab_receive_rtn_controller');get_php_form_data( this.value, 'company_wise_report_button_setting','requires/woven_finish_fab_receive_rtn_controller' );load_room_rack_self_bin('requires/woven_finish_fab_receive_rtn_controller*3', 'store','store_td', this.value);" );
                                    ?>
                                </td>
                                <td width="120" align="right" class="must_entry_caption" >Return Date</td>
                                <td width="170"><input type="text" name="txt_return_date" id="txt_return_date" class="datepicker" style="width:160px;" value="<? echo date("d-m-Y"); ?>" placeholder="Select Date" /></td>
                            ...
                            <tr>
                               ...
                               <!-- <td align="right">PI NO </td>-->
                                <td>
                                	<input class="text_boxes" type="hidden" name="txt_pi_no" id="txt_pi_no" onDblClick="openmypage_pi()" placeholder="Double Click To Search" style="width:160px;" readonly  />
                     ...
                     <tr>
                         ...
                         <fieldset style="width:450px; float:left">  
                                  ...
                                  <table  width="450" cellspacing="2" cellpadding="0" border="0">
                                          <tr>
                                           ...
                                           <tr>
                                               ...
                                               <td align="right">Store Name&nbsp;&nbsp;</td>
                                                <td id="store_td"><?
                                              	 echo create_drop_down( "cbo_store_name", 140, "select id, store_name from lib_store_location","id,store_name", 1,"--Display--",0,"",1); 
                                          ...
                                          <tr>
                                               ...
                                               <td><input class="text_boxes" type="text" name="txt_gsm" id="txt_gsm" style="width:110px;" placeholder="Display" readonly disabled  /></td> -->
												...
												<td>
													<input type="text" name="txt_weight" id="txt_weight" class="text_boxes_numeric" style="width:20px;" readonly disabled />
                                                ...
                                                <td width="100" align="right">Room&nbsp;&nbsp;</td>
												<td id="room_td">
                                          ...
                                          <tr>
											...
											<td>
												<input type="text" name="txt_width" id="txt_width" class="text_boxes" style="width:110px;" readonly disabled />
											</td>



                                            <td width="100" align="right">Rack&nbsp;&nbsp;</td>
											<td id="rack_td">
													<? echo create_drop_down( "txt_rack", 140,$blank_array,"", 1, "--Select--", 0, "",0 ); ?>
											</td> 
										<tr>

											<td align="right">Cutable Width</td>
											<td>
												<input type="text" name="txt_cutable_width" id="txt_cutable_width" class="text_boxes" style="width:110px;" disabled readonly />
											</td>
											<td  width="100" align="right">Shelf&nbsp;&nbsp;</td>
											<td id="shelf_td">
                                          ...
                                          <tr>
                                               ...
                                               <td>
                                               <input class="text_boxes" type="text" name="txt_color_name" id="txt_color_name" style="width:110px;" placeholder="Display" readonly disabled  />
                                               <input class="text_boxes" type="hidden" name="hidden_color_id" id="hidden_color_id" style="width:110px;" />
                                               </td>
                           ...
                           <legend>Display</legend>                                     
                            ...
                            <tr>
                                  <td title="MRR wise recv">Fabric Received</td>
                                  <td width="100"><input  type="text" name="txt_fabric_received" id="txt_fabric_received" class="text_boxes" style="width:160px" readonly disabled  placeholder="Display" /></td>
                            </tr>
                            <tr>
                                <td title="Product ref. wise total global">Global Stock</td>
                            ...
                            <tr>
                                <td title="MRR wise receive return">Cumulative Receive Return</td>
                                <td >
                                <input type="text" name="txt_cumulative_issued" id="txt_cumulative_issued" class="text_boxes" style="width:160px"  readonly disabled placeholder="Display" />
                            ...
                            <tr>
                                <td title="Fabric Received - Cummulative Receive Return" >Yet to Issue</td>
                                <td width="100" >
                                    <input  type="text" name="txt_yet_to_issue" id="txt_yet_to_issue" class="text_boxes" style="width:160px"  readonly disabled placeholder="Display"/>
                                </td>
                            </tr> 
               	...
               	<table cellpadding="0" cellspacing="1" width="820">
                	...
                	<tr>
                        <td align="center" colspan="6" valign="middle" class="button_container">
	                        ...
	                        <input type="hidden" name="hidden_fab_ref" id="hidden_fab_ref" readonly>
	                        <input type="hidden" name="hidden_rd_no" id="hidden_rd_no" readonly>
	                        <input type="hidden" name="hidden_rate" id="hidden_rate" readonly>
	                        <input type="hidden" name="hidden_update_amount" id="hidden_update_amount" readonly>
							...
							<input type="button" name="print" id="print" value="Print" onClick="fnc_yarn_receive_return_entry(4)" style="width: 80px; display:none;" class="formbutton">
