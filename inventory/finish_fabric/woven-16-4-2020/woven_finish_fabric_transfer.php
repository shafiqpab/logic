...
function openmypage_itemDescription()
{
	...
	var page_link = 'requires/woven_finish_fabric_transfer_controller.php?cbo_company_id='+cbo_company_id+'&cbo_store_name='+cbo_store_name+'&cbo_floor='+cbo_floor+'&cbo_room='+cbo_room+'&txt_rack='+txt_rack+'&txt_shelf='+txt_shelf+'&transfer_criteria='+transfer_criteria+'&txt_from_order_no='+txt_from_order_no+'&txt_from_order_id='+txt_from_order_id+'&action=itemDescription_popup';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(product_ref+"_"+transfer_criteria, "populate_data_from_product_master", "requires/woven_finish_fabric_transfer_controller" );
...
function openmypage_order(str) 
{
	...
	var title = 'PO Info';	
	var page_link = 'requires/woven_finish_fabric_transfer_controller.php?cbo_company_id_to='+cbo_company_id_to+'&cbo_store_name_to='+cbo_store_name_to+'&action=po_search_popup';
	//alert(page_link);
...
function fnc_yarn_transfer_entry(operation)
{
	if(operation==4)
	{
		if($('#update_id').val()*1 !=0){
			...
			print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title, "finish_fabric_transfer_print", "requires/woven_finish_fabric_transfer_controller" ) 
			return;
		}else{
	...
	if(operation==5)
	{
		if($('#update_id').val()*1 !=0){
			...
			print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title, "finish_fabric_transfer_print_2", "requires/woven_finish_fabric_transfer_controller" ) 
			return;
		}else{
			alert("Transfer System ID not found");return;
		}
	...
	else if(operation==0 || operation==1 || operation==2)
	{
		...
		if( form_validation('cbo_transfer_criteria*cbo_company_id*txt_transfer_date*cbo_store_name*cbo_store_name_to*txt_item_desc*txt_transfer_qnty*txt_from_order_no*txt_to_order_no','Transfer Criteria*Company*Transfer Date*From Store*To Store*Item Description*Transfered Qnty*From Order*To Order')==false )
		{
			return;
		}	
