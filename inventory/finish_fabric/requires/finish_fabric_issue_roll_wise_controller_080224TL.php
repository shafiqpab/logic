...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$field_array_roll="id, barcode_no, mst_id, dtls_id, po_breakdown_id, entry_form, qnty,qc_pass_qnty,rate,amount, roll_no, roll_id,reprocess,prev_reprocess, inserted_by, insert_date,is_sales,booking_no,booking_without_order";
		...
		for($j=1;$j<=$tot_row;$j++)
		{
			...
			$data_array_roll.="(".$id_roll.",'".$$barcodeNo."',".$id.",".$dtls_id.",'".$$orderId."',71,'".$$rollWgt."','".$$rollWgt."','".$$rollRate."','".$amount."','".$$rollNo."','".$$rollId."',".$reprocess_id.",".$$reProcess.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time. "','" . $$IsSalesId . "','".$$bwoNo."',".$$booking_without_order_status.")";
	...
	else if ($operation==1)   // Update Here
	{
		...
		for($j=1;$j<=$tot_row;$j++)
		{
			...
			else
			{
				...
				if($data_array_roll!="") $data_array_roll.= ",";
				$data_array_roll.="(".$id_roll.",'".$$barcodeNo."',".$update_id.",".$dtls_id.",'".$$orderId."',71,'".$$rollWgt."','".$$rollWgt."','".$$rollRate."','".$amount."','".$$rollNo."','".$$rollId."','".$reprocess_id."','".$$preRerocess."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time. "','" . $$IsSalesId . "','".$$bwoNo."',".$$booking_without_order_status.")";
		...
		if($txt_deleted_id!="")
		{
			/*$rollDelete=sql_select("select b.id as dtls_id, b.trans_id  from pro_roll_details a, inv_finish_fabric_issue_dtls b where a.dtls_id=b.id and a.id in($txt_deleted_id) and b.mst_id=$update_id and a.entry_form=71");
		...
		//echo "10**select a.id, a.qnty,a.rate, b.id as dtls_id, b.trans_id, b.prod_id from pro_roll_details a, inv_finish_fabric_issue_dtls b where a.dtls_id=b.id and a.barcode_no in($all_barcode_no) and b.mst_id=$update_id and a.entry_form=71";oci_rollback($con); die;
		//echo "10**delete from order_wise_pro_details where dtls_id in(".substr($update_dtls_id,0,-1).") and entry_form=71"; oci_rollback($con); die;
...
if($action=="populate_barcode_data")
{
	...
	foreach($data_array as $row)
	{
		if($scanned_barcode_data[$row[csf("barcode_no")]][$row[csf("reprocess")]]=="")
		{
			...
			$barcodeDataArr[$row[csf('barcode_no')]]=$row[csf("barcode_no")]."**".$company_id."**".$row[csf("roll_no")]."**".$roll_id."**".$body_part_id."**".$body_part[$body_part_id]."**".$row[csf("bwo")]."**".$receive_basis."**".$receive_basis_id."**".$row[csf("recv_number")]."**".$row[csf("booking_id")]."**".$color."**".$color_id."**".$knitting_source_one."**".$knitting_source[$knitting_source_one]."**".$knit_company_one."**".$knit_company."**".$row[csf("batch_id")]."**".$dia_width_type."**".$fabric_typee[$dia_width_type]."**".$batch_name_array[$row[csf("batch_id")]]."**".$row[csf("prod_id")]."**".$row[csf("fabric_description_id")]."**".$row[csf("gsm")]."**".$row[csf("width")]."**".number_format($row[csf("qc_pass_qnty")],2,'.','')."**".$rate."**".$booking_without_order."**".$row[csf("reprocess")]."**".$book_booking."**".$floor_id."**".$room_id."**".$rack_id."**".$shelf_id."**".$bin_id."**".$floor_no."**".$room_no."**".$rack_no."**".$shelf_no."**".$bin_no."**".$store_id;
...
if($action=="populate_barcode_data_update")
{
	...
	$issued_barcode_data=sql_select("select a.id, a.barcode_no, a.dtls_id, a.roll_id, a.rate, a.qnty, a.po_breakdown_id, a.booking_without_order, b.trans_id,a.reprocess,a.prev_reprocess, b.floor, b.room, b.rack_no, b.shelf_no, b.bin_box from pro_roll_details a, inv_finish_fabric_issue_dtls b where a.dtls_id=b.id and a.entry_form=71 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.mst_id=$data");
