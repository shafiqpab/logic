...
if($action=="create_mrr_search_list_view")
{
	...
	$sql = "select a.id,a.recv_number_prefix_num,a.recv_number,$year_field, a.challan_no,a.receive_date,a.receive_basis, a.knitting_source,sum(b.cons_quantity) as receive_qnty from inv_transaction b, inv_receive_master a where a.id=b.mst_id $entry_form_ref and a.status_active=1 $sql_cond group by a.id, a.recv_number_prefix_num ,a.recv_number,a.challan_no,a.receive_date,a.receive_basis,a.insert_date,a.knitting_source order by a.id";
	//echo $sql;
	?>
...
{ 
	$batch_arr=return_library_array("select id, batch_no from pro_batch_create_mst","id","batch_no");
 	...
 	$mrr_no = $data;
	$sql = "select  a.id as mrr_id, c.id as prod_id, c.product_name_details, b.batch_id, b.color_id, b.trans_id as tr_id, b.receive_qnty as receive_qnty, b.rack_no, b.shelf_no
    	...
    	<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="450">
            ...
            <tbody>
				...
				{ 
					...
					?>
					<tr bgcolor="<? echo $bgcolor; ?>" onClick='get_php_form_data("<? echo $row[csf("mrr_id")];?>,<? echo $row[csf("tr_id")];?>,<? echo $row[csf("batch_id")];?>","item_details_form_input","requires/woven_finish_fab_receive_rtn_controller")' style="cursor:pointer" >
					<td><? echo $i; ?></td>
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		{	 
			...
			$data_array_mst="(".$id.",'".$new_return_number[1]."','".$new_return_number[2]."','".$new_return_number[0]."',202,3,".$cbo_company_id.",".$txt_return_date.",".$txt_received_id.",".$txt_mrr_no.",".$pi_id.",'".$user_id."','".$pc_date_time."')";
		...
		$field_array_trans = "id,mst_id,company_id,prod_id,batch_id_from_fissuertn,pi_wo_batch_no,item_category,transaction_type,transaction_date,store_id,body_part_id,floor_id,room,rack,self,bin_box,order_uom,order_qnty,cons_uom,cons_quantity,no_of_roll,remarks,batch_lot,inserted_by,insert_date";
 		$data_array_trans = "(".$transactionID.",".$id.",".$cbo_company_id.",".$txt_prod_id.",".$hidden_batch_id.",".$hidden_batch_id.",3,3,".$txt_return_date.",".$cbo_store_name.",".$cbo_body_part.",".$cbo_floor.",".$cbo_room.",".$txt_rack.",".$txt_shelf.",".$cbo_bin.",".$cbo_uom.",".$txt_return_qnty.",".$cbo_uom.",".$txt_return_qnty.",".$txt_roll.",".$txt_remarks.",'".$txt_batch_no."','".$user_id."','".$pc_date_time."')"; 
		...
		$data_array = "(".$mrrWiseIsID.",".$hidden_receive_trans_id.",".$transactionID.",202,".$txt_prod_id.",".$txt_return_qnty.",'".$user_id."','".$pc_date_time."')";
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		//before stock update
		$sql = sql_select( "select a.id,a.current_stock, b.cons_quantity from product_details_master a, inv_transaction b where a.id=b.prod_id and a.id=$before_prod_id and b.id=$update_id and a.item_category_id=3 and b.item_category=3 and b.transaction_type=3" );
		...
		$data_array_mst=$cbo_company_id."*".$txt_return_date."*".$txt_received_id."*".$pi_id."*".$txt_mrr_no."*'".$user_id."'*'".$pc_date_time."'";
 		...
 		$field_array_trans="company_id*prod_id*batch_id_from_fissuertn*pi_wo_batch_no*item_category*transaction_type*transaction_date*store_id*body_part_id*floor_id*room*rack*self*bin_box*order_uom*order_qnty*cons_uom*cons_quantity*no_of_roll*remarks*batch_lot*updated_by*update_date";
 		$data_array_trans= "".$cbo_company_id."*".$txt_prod_id."*".$hidden_batch_id."*".$hidden_batch_id."*3*3*".$txt_return_date."*".$cbo_store_name."*".$cbo_body_part."*".$cbo_floor."*".$cbo_room."*".$txt_rack."*".$txt_shelf."*".$cbo_bin."*".$cbo_uom."*".$txt_return_qnty."*".$cbo_uom."*".$txt_return_qnty."*".$txt_roll."*".$txt_remarks."*".$txt_batch_no."*'".$user_id."'*'".$pc_date_time."'"; 
		...
		$data_array_mrr = "(".$mrrWiseIsID.",".$hidden_receive_trans_id.",".$update_id.",202,".$txt_prod_id.",".$txt_return_qnty.",'".$user_id."','".$pc_date_time."')";
		...
		/*oci_rollback($con);
		//echo "10**".$query1."&&".$query2."&&".$query3."&&".$query4 ."&&". $query5 ."&&".$rID."&&".$transID."&&".$propoId."&&".$rollId."&&".$mrrWiseIssueID."&&".$upTrID;
		die;*/
		if($db_type==0)
		{
...
if($action=="child_form_input_data")
{
	...
	$issue_id=str_replace("'","",$ex_data[2]); 
 	...
 	$sql = "select a.company_id, b.id as prod_id, b.product_name_details, b.current_stock, b.gsm, b.dia_width, b.color, a.id as tr_id, a.cons_uom, a.cons_quantity, a.cons_amount, a.batch_id_from_fissuertn, a.remarks,a.store_id,a.body_part_id,a.floor_id,a.room,a.bin_box, a.rack, a.self, a.no_of_roll
			from inv_transaction a, product_details_master b
 			where a.id=$data and a.status_active=1 and a.item_category=3 and transaction_type=3 and a.prod_id=b.id and b.status_active=1";
 	//echo $sql;die;
	$result = sql_select($sql);
	foreach($result as $row)
	{
