...
if($action=='itemTransfer_popup')
{
	...
	where a.item_category=3 and a.to_company=$company $location_cond $store_cond and a.id=b.mst_id and a.entry_form=258 and a.transfer_criteria in(1,2,4) and b.is_acknowledge=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
	group by a.id,a.insert_date, a.transfer_prefix_number, a.transfer_system_id, a.challan_no, a.to_company, a.transfer_date, a.transfer_criteria, a.item_category,a.to_location_id, b.to_store order by a.transfer_system_id";
	//echo $sql;//die;
...
{
	$data_array=sql_select("select a.transfer_criteria, a.company_id, a.to_company, a.location_id, a.to_location_id, b.from_order_id, b.to_order_id, b.id, b.mst_id, b.dtls_id,  b.from_store, b.to_store, b.floor_id, b.room, b.rack, b.shelf, b.to_floor_id, b.to_room, b.to_rack, b.to_shelf, b.from_prod_id, b.to_prod_id, b.transfer_qnty, b.rate, b.transfer_value, b.color_id, b.uom, b.batch_id 
	from inv_item_transfer_mst a, inv_item_transfer_dtls_ac b 
...
{ 
	...
	if ($operation==0)  // Insert Here
	{
		...
		$field_array_trans="id, mst_id, company_id, prod_id, pi_wo_batch_no, item_category, transaction_type, transaction_date, store_id, floor_id, room, rack, self, order_id, cons_uom, cons_quantity, cons_rate, cons_amount, balance_qnty, balance_amount, inserted_by, insert_date";
		...
		for($i=1;$i<=$numRow;$i++)
		{
			...
			$data_array_trans .="(".$recv_trans_id.",".$challan_id.",".$cbo_company_id_to.",'".str_replace("'","",$$productID)."','".str_replace("'","",$$txtBatchId)."',".$cbo_item_category.",5,".$txt_transfer_date.",".$cbo_store_name_to.",'".str_replace("'","",$$cboFloorTo)."','".str_replace("'","",$$cboRoomTo)."','".str_replace("'","",$$txtRackTo)."','".str_replace("'","",$$txtShelfTo)."','".str_replace("'","",$$txtOrderID)."','".str_replace("'","",$$cboUom)."','".str_replace("'","",$$hiddenTranQnty)."','".str_replace("'","",$$hiddenTranRate)."','".str_replace("'","",$$hiddenTranValue)."',0,0,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		//echo "10**".$rID."**".$rID2."**".$rID3."**".$rID4."**".$rID5."**".$flag;die;
		//10**1**0**0**1**1**0
		if($db_type==0)
		{
			if($flag==1)
	...
	{ 
		...
		$field_array_trans_update="transaction_date*floor_id*room*rack*self*updated_by*update_date";
		...
		for($i=1;$i<=$numRow;$i++)
		{
			...
			if($$txtTransID !='')
			{
				...
				$data_array_trans_update[str_replace("'","",$$txtTransID)] = explode("*",("$txt_transfer_date*".str_replace("'","",$$cboFloorTo)."*".str_replace("'","",$$cboRoomTo)."*".str_replace("'","",$$txtRackTo)."*".str_replace("'","",$$txtShelfTo)."*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'"));
			}
...
{
	$data_array=sql_select("select a.id, a.challan_id, a.acknowledg_date, a.remarks, b.id as dtls_id, b.mst_id, b.from_store, b.to_store, b.floor_id, b.room, b.rack, b.shelf, b.to_floor_id, b.to_room, b.to_rack, b.to_shelf, b.from_prod_id, b.to_prod_id, b.transfer_qnty, b.rate, b.transfer_value, b.color_id, b.uom, b.from_order_id, b.to_order_id, c.trans_id, b.batch_id ,a.company_id as to_company, a.location_id as to_location_id
