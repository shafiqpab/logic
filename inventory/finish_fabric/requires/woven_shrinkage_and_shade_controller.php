...
if($action=="sys_search_list_view")
{
	...
	$ids=chop($ids,",");
	$recvSqlShrinkage=sql_select("select QC_ID from INV_RECEIVE_MASTER where entry_form=564 and qc_id in($ids) and status_active=1 and is_deleted=0");
	foreach($recvSqlShrinkage as $row)
	{
		$shrinkageFoundRecvArr[$row[QC_ID]]=1;
	}
	//print_r($shrinkageFoundRecvArr);
	?>
			...
			<tbody>
				...
				foreach($sql_result as $row)
				{
					$bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF";
 					?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="cursor:pointer;" onClick="js_set_value('<? echo $row[ID]."_".$shrinkageFoundRecvArr[$row[ID]]; ?>')">
		...
		<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="700" id="list_view" align="left">
			<tbody>
				...
				foreach($sql_result as $row)
				{
					$bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF";
 					?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="cursor:pointer;" onClick="js_set_value('<? echo $row[ID]."_".$row[RECV_NUMBER]."_".$barcodeNoExisting."_".$row[LOCATION_ID]; ?>')">
						<td width="35" align="center"><?=$i; ?></td>
...
if($action=="populate_dtls_data")
{
	...
	$sql_dtls="select a.ID AS GRN_MST_ID , b.ID AS GRN_DTLS_ID,c.PO_BREAKDOWN_ID AS BOOKING_ID,c.BOOKING_NO ,c.BARCODE_NO,c.QNTY,c.ROLL_NO,c.MANUAL_ROLL_NO,b.GSM,b.CUTABLE_WIDTH,d.ID as BATCH_ID  
	from INV_RECEIVE_MASTER a,QUARANTINE_PARKING_DTLS b,PRO_ROLL_DETAILS c,PRO_BATCH_CREATE_MST d
	where a.id=b.mst_id and b.id=c.dtls_id and a.id=c.mst_id and c.batch_no=d.batch_no and c.po_breakdown_id=d.booking_no_id and d.entry_form=559 and d.status_active=1 and d.is_deleted=0 and a.entry_form=559 and c.entry_form=559 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.recv_number = '$data'
	group by  a.ID , b.ID ,c.BARCODE_NO,c.QNTY,c.ROLL_NO,c.MANUAL_ROLL_NO,b.GSM,b.CUTABLE_WIDTH,c.PO_BREAKDOWN_ID,c.BOOKING_NO,d.ID order by c.barcode_no asc";
	...
	foreach($sql_dtls_result_res as $drows){
		...
		echo "$('#barcodeNo_".$i."').val('".$drows[BARCODE_NO]."');\n";	
		echo "$('#lengthYDS_".$i."').val('".$drows[QNTY]."');\n";	
		...
		echo "$('#batchId_".$i."').val('".$drows[BATCH_ID]."');\n";
...
if($action=="save_update_delete")
{
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		foreach(explode('**',$txt_pattern_data_str) as $pattern_data_arr){
		 ...
		 //dtls data..........................................................................
		$dtls_field_array="id, mst_id, ccl_no,intellocut_roll_no,barcode_no,grn_dtls_id, length_yds, width, shade, before_wash_length_cm, beforeW_wash_width_cm, after_wash_length_cm, after_wash_width_cm, before_wash_gsm, after_wash_gsm,booking_id,booking_no,batch_id, inserted_by, insert_date";
		...
		for($i=0;$i<$total_rows;$i++){
			...
			$dtls_data_array.=$add_comma."(".$dtls_id.",".$id.",".$$ccl_no.",".$$intellocut_roll_no.",".$$barcode_no.",".$$grn_dtls_id.",".$$length_yds.",".$$width.",".$$shade.",".$$before_wash_length_cm.",".$$beforeW_wash_width_cm.",".$$after_wash_length_cm.",".$$after_wash_width_cm.",".$$before_wash_gsm.",".$$after_wash_gsm.",".$$bookingId.",".$$bookingNo.",".$$batchId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			//$dtls_id++;
		...
		//echo "insert into woven_shrink_shade_dtls  $dtls_field_array value $dtls_data_array";die;
