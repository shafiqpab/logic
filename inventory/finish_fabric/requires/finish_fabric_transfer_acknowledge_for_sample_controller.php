...
{
	load_room_rack_self_bin("requires/finish_fabric_transfer_acknowledge_for_sample_controller",$data);
}*/
...
if ($action=="load_drop_down_location_to")
{
	//echo $data;
	$data=explode("_",$data);
	echo create_drop_down( "cbo_location_to", 160, "select id,location_name from lib_location where company_id='$data[0]' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", 0, "load_drop_down( 'requires/finish_fabric_transfer_acknowledge_for_sample_controller',this.value+'_'+$data[0], 'load_drop_down_store_to', 'to_store_td' );" );
	exit();
	//load_room_rack_self_bin('requires/finish_fabric_transfer_acknowledge_for_sample_controller*2*cbo_store_name_to', 'store','to_store_td', $('#cbo_company_id_to').val(),this.value);
}
...
if ($action=="load_drop_down_location_to_up")
{
	//echo $data;
	$data=explode("_",$data);
	echo create_drop_down( "cbo_location_to", 160, "select id,location_name from lib_location where company_id='$data[0]' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", 0, "load_drop_down( 'finish_fabric_transfer_acknowledge_for_sample_controller',this.value+'_'+$data[0], 'load_drop_down_store_to', 'to_store_td' );" );
	exit();
}
...
if($action=='itemTransfer_popup')
{
	...
	where a.item_category=2 and a.to_company=$company $location_cond $store_cond and a.id=b.mst_id and a.entry_form in(306) and a.transfer_criteria in(6,7,8) and a.is_acknowledge=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
...
if($action=='populate_data_from_transfer_master')
{
	...
	{
		echo "load_drop_down('requires/finish_fabric_transfer_acknowledge_for_sample_controller','".$row[csf("to_company")]."', 'load_drop_down_location_to', 'to_location_td' );\n";
		...
		if($row[csf("to_store")]>0){
			...
			echo "load_drop_down('requires/finish_fabric_transfer_acknowledge_for_sample_controller','".$loc_com."', 'load_drop_down_store_to', 'to_store_td' );\n";
...
if($action=="show_dtls_list_view")
{
	...
	where a.status_active=1 and a.is_deleted=0 and a.transaction_type=5 and a.mst_id=$challan_id and a.prod_id in($toProdId) and a.pi_wo_batch_no in($toBatchId) and b.po_breakdown_id in($toOrderId) and a.fabric_shade in($fabricShade) and a.id = b.trans_id and a.mst_id=c.mst_id and c.dtls_id=b.dtls_id and c.from_order_id in($fromOrderId) and b.entry_form=306
	group by a.pi_wo_batch_no,a.prod_id,b.po_breakdown_id,a.fabric_shade,c.from_order_id, c.to_trans_id");*/

	$prev_rcv=sql_select("SELECT a.pi_wo_batch_no,a.prod_id,c.to_order_id as po_breakdown_id,a.fabric_shade,c.from_order_id, sum(a.cons_quantity) as cons_quantity, c.to_trans_id 
	from inv_transaction a, inv_item_transfer_dtls_ac c  
	where a.status_active=1 and a.is_deleted=0 and a.transaction_type=5 and a.mst_id=$challan_id and a.prod_id in($toProdId) and a.pi_wo_batch_no in($toBatchId) and a.fabric_shade in($fabricShade) and a.mst_id=c.mst_id and c.from_order_id in($fromOrderId) and c.status_active=1 and c.is_deleted=0
	group by a.pi_wo_batch_no,a.prod_id,a.fabric_shade,c.to_order_id,c.from_order_id, c.to_trans_id");
		...
		<table width="100%" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all">
			...
			<tbody>
				...
				foreach($data_array as $row)
				{
					...
					//echo $row[csf('to_batch_id')].'**'.$row[csf('to_order_id')].'**'.$row[csf('fabric_shade')].'**'.$row[csf('from_order_id')].'**'.$row[csf('to_prod_id')];
...
if ($action=="save_update_delete")
{
	...
	$transfer_issue_dtls = sql_select("SELECT from_prod_id,sum(transfer_qnty) transfer_qnty from inv_item_transfer_dtls where mst_id=$challan_id and to_prod_id=$productID and to_batch_id='".str_replace("'","",$batch_id)."' and active_dtls_id_in_transfer=1 group by from_prod_id");	
	...
	// echo '20**'.$cu_rcv.">".$transfer_issue_qnty;die; // 5>5
	...
	if ($operation==0)  // Insert Here
	{
		...
		else
		{
			...
			$data_array="(".$id.",364,".$challan_id.",".$cbo_company_id_to.",".$cbo_store_name_to.",".$cbo_location_to.",".$cbo_transfer_criteria.",".$cbo_item_category.",".$txt_transfer_date.",".$txt_remarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		}
		...
		$existing_transfer_dtls_row = sql_select("select b.batch_id,b.body_part_id,b.fabric_shade,b.feb_description_id, b.floor_id,b.from_booking_without_order,b.from_order_id,b.from_prod_id,b.from_store,
			b.from_trans_entry_form,b.rack,b.rate_in_usd,b.remarks,b.room,b.shelf,b.trans_id,b.transfer_qnty,b.transfer_value,b.transfer_value_in_usd,b.uom,b.to_body_part,b.roll,b.to_ord_book_id,b.to_ord_book_no,b.to_batch_id, a.transfer_criteria
			from inv_item_transfer_mst a,inv_item_transfer_dtls b
		...
		$transfer_criteria 			= $existing_transfer_dtls_row[0][csf("transfer_criteria")];
		...
		foreach ($save_string as $save_value) {
			...
			$data_array_trans .= "(" . $recv_trans_id.",".$challan_id.",".$cbo_company_id_to.",".$productID.",".$to_batch_id.",".$cbo_item_category.",5,".$txt_transfer_date.",".$cbo_store_name_to.",".$floor.",".$room.",".$rack.",".$shelf.",".$txtOrderID.",".$cbo_fabric_type.",".$to_body_part.",".$cbo_uom.",".$qnty.",".$item_rate.",'".$trans_value."',".$qnty.",".$trans_value.",".$color_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";

			if ($data_array_dtls != "") $data_array_dtls .= ",";
			$id_dtls = return_next_id_by_sequence("INV_ITEM_TRANSFER_DTLS_PK_SEQ", "inv_item_transfer_dtls", $con);
			$data_array_dtls .= "(" . $id_dtls.",".$challan_id.",".$from_prod_id.",".$productID.",".$from_batch_id.",0,0,".$color_id.",0,".$from_store.",".$from_floor_id.",".$from_room.",".$from_rack.",".$from_shelf.",".$cbo_store_name_to.",".$floor.",".$room.",".$rack.",".$shelf.",".$cbo_item_category.",".$qnty.",".$rate_in_usd.",".$trans_value.",".$uom.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$from_order_id.",".$txtOrderID.",".$trans_id.",".$recv_trans_id.",'".$remarks."',".$fabric_shade.",".$to_batch_id.",".$body_part_id.",".$to_body_part.",'".$roll."','".$to_ord_book_id."','".$to_ord_book_no."')";
			...
			$data_array_dtls_ac .= "(".$id_dtls_ac.",".$challan_id.",".$id_dtls.",0,".$from_prod_id.",".$productID.",".$from_batch_id.",0,0,".$color_id.",0,".$from_store.",".$from_floor_id.",".$from_room.",".$from_rack.",".$from_shelf.",".$cbo_store_name_to.",".$floor.",".$room.",".$rack.",".$shelf.",".$cbo_item_category.",".$qnty.",".$rate_in_usd.",".$trans_value.",".$uom.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$from_order_id.",".$txtOrderID.",".$fabric_shade.",".$to_batch_id.",".$body_part_id.",".$to_body_part.",'".$roll."','".$to_ord_book_id."','".$to_ord_book_no."',".$trans_id.",".$recv_trans_id.")";
			
			if( $transfer_criteria == 7 ) // Sample to Order
			{
				if ($data_array_prop != "") $data_array_prop .= ",";
				$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
				$data_array_prop .= "(" . $id_prop.",".$recv_trans_id.",5,306,".$id_dtls.",".$txtOrderID.",".$productID.",".$color_id.",".$qnty.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			}
		}
	...
	else if ($operation==1)   // Update Here
	{
		...
		foreach ($save_string as $save_value) {
			...
			if($trans_id > 0){
				...
				if ($data_array_dtls != "") $data_array_dtls .= ",";
				$id_dtls = return_next_id_by_sequence("INV_ITEM_TRANSFER_DTLS_PK_SEQ", "inv_item_transfer_dtls", $con);
				$data_array_dtls .= "(" . $id_dtls.",".$challan_id.",".$from_prod_id.",".$productID.",".$batch_id.",0,0,".$color_id.",0,".$from_store.",".$from_floor_id.",".$from_room.",".$from_rack.",".$from_shelf.",".$cbo_store_name_to.",".$floor.",".$room.",".$rack.",".$shelf.",".$cbo_item_category.",".$qnty.",".$rate_in_usd.",".$trans_value.",".$uom.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$from_order_id.",".$txtOrderID.",".$from_trans_id.",".$recv_trans_id.",'".$remarks."',".$fabric_shade.",".$to_batch_id.",".$body_part_id.",".$to_body_part.",'".$roll."','".$to_ord_book_id."','".$to_ord_book_no."')";

				$id_dtls_ac = return_next_id_by_sequence("INV_ITEM_TRANS_DTLS_AC_PK_SEQ", "inv_item_transfer_dtls_ac", $con);
				if ($data_array_dtls_ac != "") $data_array_dtls_ac .= ",";
				$data_array_dtls_ac .= "(".$id_dtls_ac.",".$challan_id.",".$id_dtls.",0,".$from_prod_id.",".$productID.",".$batch_id.",0,0,".$color_id.",0,".$from_store.",".$from_floor_id.",".$from_room.",".$from_rack.",".$from_shelf.",".$cbo_store_name_to.",".$floor.",".$room.",".$rack.",".$shelf.",".$cbo_item_category.",".$qnty.",".$rate_in_usd.",".$trans_value.",".$uom.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$from_order_id.",".$txtOrderID.",".$fabric_shade.",".$to_batch_id.",".$body_part_id.",".$to_body_part.",'".$roll."','".$to_ord_book_id."','".$to_ord_book_no."',".$from_trans_id.",".$recv_trans_id.")";
		...
		if( $transfer_criteria == 7 )
		{
			if($data_array_prop!="")$rID7=sql_insert("order_wise_pro_details",$field_array_proportionate,$data_array_prop,1);
		}
...
{
	$data_array=sql_select("select a.id, a.company_id, a.location_id, a.challan_id, a.acknowledg_date, a.remarks,b.mst_id, b.store_id, b.floor_id, b.room, b.rack, b.self, b.order_id, b.prod_id, b.pi_wo_batch_no, d.batch_no, b.dyeing_color_id, sum(b.cons_quantity) cons_quantity, listagg(cast(b.id as varchar(4000)),',') within group(order by b.id) as trans_id, b.cons_rate, b.cons_amount, b.cons_uom, b.fabric_shade,c.trans_id from_trans_id
		from inv_item_trans_acknowledgement a, inv_transaction b,inv_item_transfer_dtls c, pro_batch_create_mst d
		where a.id='$data' and a.challan_id=b.mst_id and b.id=c.to_trans_id and b.pi_wo_batch_no=d.id and a.entry_form=364 and b.transaction_type=5 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1
		group by a.id, a.company_id, a.location_id, a.challan_id, a.acknowledg_date, a.remarks, b.mst_id, b.store_id, b.floor_id, b.room, b.rack, b.self, b.order_id, b.prod_id, b.pi_wo_batch_no, d.batch_no, b.dyeing_color_id, b.cons_rate, b.cons_amount, b.cons_uom, b.fabric_shade,c.trans_id");
	$trans_ids = "";
	foreach($data_array as $row)
	{
...
if ($action=="itemAcknowle_popup")
{
	...
	<script>
		...
		function load_location()
		{
			...
			load_drop_down('finish_fabric_transfer_acknowledge_for_sample_controller',cbo_company_id_to, 'load_drop_down_location_to_up', 'to_location_td' );
		}
	</script>
</head>
			...
			<fieldset style="width:880px;margin-left:10px">
				...
				<table cellpadding="0" cellspacing="0" width="870" class="rpt_table">
					...
					<tr class="general">
						<td>
							<?
							echo create_drop_down("cbo_transfer_criteria", 120,$item_transfer_criteria,"", 1,"-- Select --",'0',"",'','6,7,8');
							?>
						</td>
...
{
	$data_array=sql_select("select a.id, a.entry_form, a.challan_id, a.company_id, a.store_id, a.location_id, a.transfer_criteria, a.item_category, a.acknowledg_date, a.remarks, b.transfer_system_id from inv_item_trans_acknowledgement a, inv_item_transfer_mst b where a.id='$data' and a.challan_id=b.id and a.entry_form=364 and a.transfer_criteria in(6,7,8) and a.status_active=1 and a.is_deleted=0");
	foreach ($data_array as $row)
	{
		echo "load_drop_down('requires/finish_fabric_transfer_acknowledge_for_sample_controller','".$row[csf("company_id")]."', 'load_drop_down_location_to', 'to_location_td' );\n";
		...
		if($row[csf("store_id")]>0){
			...
			//echo "load_room_rack_self_bin('requires/finish_fabric_transfer_acknowledge_for_sample_controller*2*cbo_store_name_to', 'store','to_store_td', '".$row[csf("company_id")]."','"."',this.value);\n";
			//echo "document.getElementById('cbo_store_name_to').value 				= '".$row[csf("store_id")]."';\n";
		}
		echo "document.getElementById('txt_challan_no').value 				= '".$row[csf("transfer_system_id")]."';\n";
...
if($action=='quantity_popup')
{
	...
	<script type="text/javascript">
		...
		function fn_addRow(i) {
			...
			$('#cbo_floor_to_' + row_num).attr("onchange", 'load_room_rack_self_bin("finish_fabric_transfer_acknowledge_for_sample_controller*2*cbo_room_to_' + row_num + '*' + row_num + '","room","room_td_to","<? echo $cbo_company_id_to;?>","<? echo $location;?>","<? echo $store;?>",this.value,0,"","","","","","H");');
	...
	<table cellpadding="0" width="750" cellspacing="0" border="1" id="qnty_breakdown_table" class="rpt_table" rules="all" style='margin:10px auto;'>
		...
		<tbody>
			...
			if($save_string=="") { ?>
				<tr id="tr__1">
					<td id="floor_td_to">
						<? echo create_drop_down( "cbo_floor_to_$i", 150,$lib_floor_arr,"", 1, "--Select--", 0, "load_room_rack_self_bin('finish_fabric_transfer_acknowledge_for_sample_controller*2*cbo_room_to_$i*$i', 'room',this.value, '".$cbo_company_id_to."','".$location."','".$store."',this.value,'','','','','','','H');",0,"","","","","","","cbo_floor_to[]" ); ?>
					</td>
					<td id="room_td_to">
				...
				foreach ($save_string as $row) {
					...
					<tr id="tr__<? echo $i;?>">
						<td id="floor_td_to">
							<? echo create_drop_down( "cbo_floor_to_$i", 150,$lib_floor_arr,"", 1, "--Select--", $data[0], "load_room_rack_self_bin('finish_fabric_transfer_acknowledge_for_sample_controller*2*cbo_room_to_1*1', 'room',this.value, '".$cbo_company_id_to."','".$location."','".$store."',this.value);",0,"","","","","","","cbo_floor_to[]" ); ?>
						</td>
						<td id="room_td_to">
							<? echo create_drop_down( "cbo_room_to_$i", 150,$lib_room_arr,"", 1, "--Select--", $data[1], "load_room_rack_self_bin('finish_fabric_transfer_acknowledge_for_sample_controller*2*txt_rack_to_$i*1','rack','rack_td_to','".$cbo_company_id_to."','".$location."','".$store."','".$data[0]."',this.value);",0,"","","","","","","cbo_room_to[]" ); ?>
						</td>
						<td id="rack_td_to">
							<? echo create_drop_down( "txt_rack_to_$i", 150,$lib_rack_arr,"", 1, "--Select--", $data[2], "load_room_rack_self_bin('finish_fabric_transfer_acknowledge_for_sample_controller*2*txt_shelf_to_$i*2','shelf','shelf_td_to','".$cbo_company_id_to."','".$location."','".$store."','".$data[0]."','".$data[1]."',this.value);",0,"","","","","","","txt_rack_to[]" ); ?>
						</td>
						<td id="shelf_td_to">
							<? echo create_drop_down( "txt_shelf_to_$i", 150,$lib_shelf_arr,"", 1, "--Select--", $data[3], "load_room_rack_self_bin('finish_fabric_transfer_acknowledge_for_sample_controller*2*cbo_bin_to*2','bin','bin_td_to','".$cbo_company_id_to."','".$location."','".$store."','".$data[0]."','".$data[1]."','".$data[2]."',this.value);",0,"","","","","","","txt_shelf_to[]" ); ?>
						</td>
						<td>
...
if ($action=="load_room_rack_self_bin")
{
	load_room_rack_self_bin("finish_fabric_transfer_acknowledge_for_sample_controller",$data);
}
