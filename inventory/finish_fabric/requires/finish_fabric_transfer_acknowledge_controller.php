...
{
	load_room_rack_self_bin("requires/finish_fabric_transfer_acknowledge_controller",$data);
}*/
...
if ($action=="load_drop_down_location_to")
{
	//echo $data;
	$data=explode("_",$data);
	echo create_drop_down( "cbo_location_to", 160, "select id,location_name from lib_location where company_id='$data[0]' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", 0, "load_drop_down( 'requires/finish_fabric_transfer_acknowledge_controller',this.value+'_'+$data[0], 'load_drop_down_store_to', 'to_store_td' );" );
	exit();
	//load_room_rack_self_bin('requires/finish_fabric_transfer_acknowledge_controller*2*cbo_store_name_to', 'store','to_store_td', $('#cbo_company_id_to').val(),this.value);
}
...
if ($action=="load_drop_down_location_to_up")
{
	//echo $data;
	$data=explode("_",$data);
	echo create_drop_down( "cbo_location_to", 160, "select id,location_name from lib_location where company_id='$data[0]' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", 0, "load_drop_down( 'finish_fabric_transfer_acknowledge_controller',this.value+'_'+$data[0], 'load_drop_down_store_to', 'to_store_td' );" );
	exit();
}
if ($action=="load_drop_down_store_to")
{
	...
	echo create_drop_down( "cbo_store_name_to", 160, "select a.id, a.store_name from lib_store_location a, lib_store_location_category b where a.id=b.store_location_id and a.company_id=$company and a.location_id=$location and a.status_active=1 and a.is_deleted=0 and b.category_type in(2) $store_location_credential_cond group by a.id, a.store_name order by a.store_name","id,store_name", 1, "-- Select Store --", 0, "" );
	exit();
}
...
if($action=='itemTransfer_popup')
{
	...
	where a.item_category=2 and a.to_company=$company $location_cond $store_cond and a.id=b.mst_id and a.entry_form in(14) and a.transfer_criteria in(1,2,4) and a.is_acknowledge=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
...
if($action=='populate_data_from_transfer_master')
{
	...
	foreach ($data_array as $row)
	{
		...
		if($row[csf("to_store")]>0){
			...
			echo "load_drop_down('requires/finish_fabric_transfer_acknowledge_controller','".$loc_com."', 'load_drop_down_store_to', 'to_store_td' );\n";
...
if($action=="show_dtls_list_view")
{
	...
	$data_array=sql_select("select a.transfer_criteria, a.company_id, a.to_company, a.location_id, a.to_location_id, b.from_order_id, b.to_order_id, b.id, b.mst_id, b.id,  b.from_store, b.to_store, b.floor_id, b.room, b.rack, b.shelf, b.to_floor_id, b.to_room, b.to_rack, b.to_shelf, b.from_prod_id, b.to_prod_id, b.transfer_qnty, b.rate, b.transfer_value, b.color_id, b.uom, b.batch_id , b.fabric_shade, b.to_batch_id,b.trans_id
		from inv_item_transfer_mst a, inv_item_transfer_dtls b
		where a.id=$data_ref[0] and a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.to_trans_id=0");
	...
	$prev_rcv=sql_select("select a.pi_wo_batch_no,a.prod_id,b.po_breakdown_id,a.fabric_shade,c.from_order_id,a.cons_quantity as cons_quantity from inv_transaction a, order_wise_pro_details b,inv_item_transfer_dtls_ac c where a.status_active=1 and a.is_deleted=0 and a.transaction_type=5 and a.mst_id=$challan_id and a.prod_id in($toProdId) and a.pi_wo_batch_no in($toBatchId) and b.po_breakdown_id in($toOrderId) and a.fabric_shade in($fabricShade) and a.id = b.trans_id and a.mst_id=c.mst_id and c.dtls_id=b.dtls_id and c.from_order_id in($fromOrderId) and b.entry_form =14");
	foreach($prev_rcv as $row)
	{
		$check_prev_recv_arr[$row[csf('pi_wo_batch_no')]][$row[csf('po_breakdown_id')]][$row[csf('fabric_shade')]][$row[csf('from_order_id')]][$row[csf('prod_id')]]=$row[csf('cons_quantity')];
	}
		...
		<table width="100%" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all">
			...
			<tbody>
				...
				foreach($data_array as $row)
				{
					...
					//$prev_rcv=return_field_value("sum(cons_quantity) as cons_quantity","inv_transaction","status_active=1 and is_deleted=0 and transaction_type=5 and mst_id=$challan_id and prod_id=$prod_id and pi_wo_batch_no='".$row[csf("to_batch_id")]."'","cons_quantity");

					/*$prev_rcv=return_field_value( "a.cons_quantity as cons_quantity", "inv_transaction a, order_wise_pro_details b,inv_item_transfer_dtls_ac c","a.status_active=1 and a.is_deleted=0 and a.transaction_type=5 and a.mst_id=$challan_id and a.prod_id=$prod_id and a.pi_wo_batch_no='".$row[csf("to_batch_id")]."' and b.po_breakdown_id ='".$row[csf("to_order_id")]."' and a.fabric_shade='".$row[csf("fabric_shade")]."' and a.id = b.trans_id and a.mst_id=c.mst_id and c.from_order_id='".$row[csf("from_order_id")]."' and b.entry_form =14","cons_quantity");*/

					if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
					if($row[csf("rate")]=="") $rate=0; else $rate=$row[csf("rate")];
					//$balance_qnty=$row[csf('transfer_qnty')]-$prev_rcv;
					$balance_qnty=$row[csf('transfer_qnty')]-$check_prev_recv_arr[$row[csf('to_batch_id')]][$row[csf('to_order_id')]][$row[csf('fabric_shade')]][$row[csf('from_order_id')]][$row[csf('to_prod_id')]];
					if($balance_qnty>0){
						?>
...
if ($action=="save_update_delete")
{
	...
	$prod_data 		= sql_select("select id, avg_rate_per_unit, current_stock, stock_value from product_details_master where id=$productID");

	$transfer_issue_dtls = sql_select("select from_prod_id,sum(transfer_qnty) transfer_qnty from inv_item_transfer_dtls where mst_id=$challan_id and to_prod_id=$productID and to_batch_id='".str_replace("'","",$batch_id)."' and active_dtls_id_in_transfer=1 and trans_id=$from_trans_id group by from_prod_id");
	...
	/* $up_cond="";
	if($up_trans_id>0) $up_cond=" and id <> $up_trans_id";
	$prev_rcv = sql_select("select prod_id, sum(cons_quantity) as cons_quantity from inv_transaction where status_active=1 and is_deleted=0 and mst_id=$challan_id and prod_id=$productID and transaction_type=5 and pi_wo_batch_no='".str_replace("'","",$batch_id)."' $up_cond group by prod_id");
	$prev_rcv_qnty = $prev_rcv[0][csf("cons_quantity")]*1;

	$cu_rcv 	   = $acknowledge_qnty + str_replace("'","",$txtTransQnty);

	if(str_replace("'","",$update_id)!=""){
		$cu_rcv 	   = ($prev_rcv_qnty*1) - (($prev_rcv_qnty*1) - str_replace("'","",$txtTransQnty));
	}else{
		$cu_rcv 	   = str_replace("'","",$txtTransQnty);
	}

	if($cu_rcv>$transfer_issue_qnty)
	{
		echo "20**Acknowledgement against this challan is already done.\nTransfer Quantity = ".$transfer_issue_qnty."\nAcknowledge Quantity = ".$acknowledge_qnty;die;
	} */
	...
	if($cu_rcv != $transfer_issue_qnty)
	{
		echo "20**Acknowledgement quantity does not equal to transfer quantity.";
		die;
	}

	if($operation ==0){
		$pre_acknowledge_sql = sql_select("select to_trans_id, sum(transfer_qnty) transfer_qnty from inv_item_transfer_dtls where mst_id=$challan_id and trans_id=$from_trans_id and to_trans_id!=0 and status_active=1 and is_deleted=0  group by to_trans_id");

		if(!empty($pre_acknowledge_sql))
		{
			echo "20**Acknowledgement against this challan is already done.\nTransfer Quantity = ".$transfer_issue_qnty."\nAcknowledge Quantity = ".$pre_acknowledge_sql[0][csf("transfer_qnty")];
			die;
		}
	}
	...
	if ($operation==0)  // Insert Here
	{
		...
		else
		{
			...
			$data_array="(".$id.",247,".$challan_id.",".$cbo_company_id_to.",".$cbo_store_name_to.",".$cbo_location_to.",".$cbo_transfer_criteria.",".$cbo_item_category.",".$txt_transfer_date.",".$txt_remarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		}
		...
		$existing_transfer_dtls_row = sql_select("select b.batch_id,b.body_part_id,b.fabric_shade,b.feb_description_id, b.floor_id,b.from_booking_without_order,b.from_order_id,b.from_prod_id,b.from_store,
			b.from_trans_entry_form,b.rack,b.rate_in_usd,b.remarks,b.room,b.shelf,b.bin_box,b.trans_id,b.transfer_qnty,b.transfer_value,b.transfer_value_in_usd,b.uom,b.to_body_part,b.roll,b.to_ord_book_id,b.to_ord_book_no,b.to_batch_id
			from inv_item_transfer_mst a,inv_item_transfer_dtls b
		...
		$from_bin_box 				= $existing_transfer_dtls_row[0][csf("bin_box")];
		...
		$field_array_trans="id, mst_id, company_id, prod_id, pi_wo_batch_no, item_category, transaction_type, transaction_date, store_id, floor_id, room, rack, self, bin_box, order_id, fabric_shade, body_part_id, cons_uom, cons_quantity, cons_rate, cons_amount, balance_qnty, balance_amount, dyeing_color_id, inserted_by, insert_date";
		...
		foreach ($save_string as $save_value) {
			...
			$data_array_trans .= "(" . $recv_trans_id.",".$challan_id.",".$cbo_company_id_to.",".$productID.",".$to_batch_id.",".$cbo_item_category.",5,".$txt_transfer_date.",".$cbo_store_name_to.",".$floor.",".$room.",".$rack.",".$shelf .",".$binbox.",".$txtOrderID.",".$cbo_fabric_type.",".$to_body_part.",".$cbo_uom.",".$qnty.",".$item_rate.",'".$trans_value."',".$qnty.",".$trans_value.",".$color_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";

			if ($data_array_dtls != "") $data_array_dtls .= ",";
			$id_dtls = return_next_id_by_sequence("INV_ITEM_TRANSFER_DTLS_PK_SEQ", "inv_item_transfer_dtls", $con);
			$data_array_dtls .= "(" . $id_dtls.",".$challan_id.",".$from_prod_id.",".$productID.",".$from_batch_id.",0,0,".$color_id.",0,".$from_store.",".$from_floor_id.",".$from_room.",".$from_rack.",".$from_shelf.",".$from_bin_box.",".$cbo_store_name_to.",".$floor.",".$room.",".$rack.",".$shelf.",".$binbox.",".$cbo_item_category.",".$qnty.",".$item_rate.",".$trans_value.",".$uom.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$from_order_id.",".$txtOrderID.",".$trans_id.",".$recv_trans_id.",'".$remarks."',".$fabric_shade.",".$to_batch_id.",".$body_part_id.",".$to_body_part.",'".$roll."','".$to_ord_book_id."','".$to_ord_book_no."')";
			...
			$data_array_dtls_ac .= "(".$id_dtls_ac.",".$challan_id.",".$id_dtls.",0,".$from_prod_id.",".$productID.",".$from_batch_id.",0,0,".$color_id.",0,".$from_store.",".$from_floor_id.",".$from_room.",".$from_rack.",".$from_shelf.",".$from_bin_box.",".$cbo_store_name_to.",".$floor.",".$room.",".$rack.",".$shelf.",".$binbox.",".$cbo_item_category.",".$qnty.",".$item_rate.",".$trans_value.",".$uom.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$from_order_id.",".$txtOrderID.",".$fabric_shade.",".$to_batch_id.",".$body_part_id.",".$to_body_part.",'".$roll."','".$to_ord_book_id."','".$to_ord_book_no."',".$trans_id.",".$recv_trans_id.")";

			if ($data_array_prop != "") $data_array_prop .= ",";
			$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
			$data_array_prop .= "(" . $id_prop.",".$recv_trans_id.",5,14,".$id_dtls.",".$txtOrderID.",".$productID.",".$color_id.",".$qnty.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		}
	...
	else if ($operation==1)   // Update Here
	{
		...
		if(str_replace("'","",$productID)=="")
		{
			echo "10**Update Id Null";disconnect($con);die;
		}
		...
		$field_array_trans_update="transaction_date*pi_wo_batch_no*floor_id*room*rack*self*bin_box*cons_uom*cons_quantity*cons_rate*cons_amount*updated_by*update_date";
		$field_array_dtls_update="to_floor_id*to_room*to_rack*to_shelf*to_bin_box*transfer_qnty*updated_by*update_date";
		...
		foreach ($save_string as $save_value) 
		{
			...
			if($trans_id > 0){
				...
				$update_trans_arr[$trans_id]=explode("*",("".$txt_transfer_date."*".$to_batch_id."*".$floor."*".$room."*".$rack."*".$shelf."*".$binbox."*".$uom."*".$qnty."*".$item_rate."*".$trans_value."*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'"));

				$update_dtlsid[]=$trans_id;
				$update_dtls_arr[$trans_id]=explode("*",("".$floor."*".$room."*".$rack."*".$shelf."*".$binbox."*".$qnty."*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'"));
			...
			else
			{
				...
				if ($data_array_dtls != "") $data_array_dtls .= ",";
				$id_dtls = return_next_id_by_sequence("INV_ITEM_TRANSFER_DTLS_PK_SEQ", "inv_item_transfer_dtls", $con);
				$data_array_dtls .= "(" . $id_dtls.",".$challan_id.",".$from_prod_id.",".$productID.",".$batch_id.",0,0,".$color_id.",0,".$from_store.",".$from_floor_id.",".$from_room.",".$from_rack.",".$from_shelf.",".$from_bin_box.",".$cbo_store_name_to.",".$floor.",".$room.",".$rack.",".$shelf.",".$binbox.",".$cbo_item_category.",".$qnty.",".$item_rate.",".$trans_value.",".$uom.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$from_order_id.",".$txtOrderID.",".$from_trans_id.",".$recv_trans_id.",'".$remarks."',".$fabric_shade.",".$to_batch_id.",".$body_part_id.",".$to_body_part.",'".$roll."','".$to_ord_book_id."','".$to_ord_book_no."')";

				$id_dtls_ac = return_next_id_by_sequence("INV_ITEM_TRANS_DTLS_AC_PK_SEQ", "inv_item_transfer_dtls_ac", $con);
				if ($data_array_dtls_ac != "") $data_array_dtls_ac .= ",";
				$data_array_dtls_ac .= "(".$id_dtls_ac.",".$challan_id.",".$id_dtls.",0,".$from_prod_id.",".$productID.",".$batch_id.",0,0,".$color_id.",0,".$from_store.",".$from_floor_id.",".$from_room.",".$from_rack.",".$from_shelf.",".$from_bin_box.",".$cbo_store_name_to.",".$floor.",".$room.",".$rack.",".$shelf.",".$binbox.",".$cbo_item_category.",".$qnty.",".$item_rate.",".$trans_value.",".$uom.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$from_order_id.",".$txtOrderID.",".$fabric_shade.",".$to_batch_id.",".$body_part_id.",".$to_body_part.",'".$roll."','".$to_ord_book_id."','".$to_ord_book_no."',".$from_trans_id.",".$recv_trans_id.")";
		...
		else
		{
			...
			if($adj_stcok_value>0 && $adj_stcok_qnty>0) $adj_avg_rate=$adj_stcok_value/$adj_stcok_qnty; else $adj_avg_rate=0;
		...
		//echo "10**".bulk_update_sql_statement("inv_item_transfer_dtls","to_trans_id",$field_array_dtls_update,$update_dtls_arr,$update_dtlsid);oci_rollback($con); die;


		$rID2=execute_query(bulk_update_sql_statement("inv_transaction","id",$field_array_trans_update,$update_trans_arr,$update_transid));
		$rID3=execute_query(bulk_update_sql_statement("inv_item_transfer_dtls","to_trans_id",$field_array_dtls_update,$update_dtls_arr,$update_dtlsid));
		$rID4=execute_query(bulk_update_sql_statement("inv_item_transfer_dtls_ac","to_trans_id",$field_array_dtls_update,$update_dtls_arr,$update_dtlsid));
...
{
	$data_array=sql_select("select a.id, a.company_id, a.location_id, a.challan_id, a.acknowledg_date, a.remarks,b.mst_id, b.store_id, b.floor_id, b.room, b.rack, b.self, b.bin_box, b.order_id, b.prod_id, b.pi_wo_batch_no, d.batch_no, b.dyeing_color_id, sum(b.cons_quantity) cons_quantity, listagg(cast(b.id as varchar(4000)),',') within group(order by b.id) as trans_id, b.cons_rate, b.cons_amount, b.cons_uom, b.fabric_shade,c.trans_id from_trans_id
		from inv_item_trans_acknowledgement a, inv_transaction b,inv_item_transfer_dtls c, pro_batch_create_mst d
		where a.id='$data' and a.challan_id=b.mst_id and b.id=c.to_trans_id and b.pi_wo_batch_no=d.id and a.entry_form=247 and b.transaction_type=5 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1
		group by a.id, a.company_id, a.location_id, a.challan_id, a.acknowledg_date, a.remarks, b.mst_id, b.store_id, b.floor_id, b.room, b.rack, b.self, b.bin_box, b.order_id, b.prod_id, b.pi_wo_batch_no, d.batch_no, b.dyeing_color_id, b.cons_rate, b.cons_amount, b.cons_uom, b.fabric_shade,c.trans_id");
	$trans_ids = "";
	foreach($data_array as $row)
	{
	...
	$trans_ids = rtrim($trans_ids, ", ");
	$trans_data_array=sql_select("select b.id,b.floor_id, b.room, b.rack, b.self, b.bin_box, b.cons_quantity from inv_transaction b where b.id in(".$trans_ids.") and b.status_active=1 and b.is_deleted=0");
	foreach($trans_data_array as $row)
	{
		$save_string_arr[$row[csf("id")]]= $row[csf("floor_id")] . "_" . $row[csf("room")] . "_" . $row[csf("rack")] . "_" . $row[csf("self")] . "_" . $row[csf("cons_quantity")] . "_" . $row[csf("id")]. "_" . $row[csf("bin_box")];
	}
...
if ($action=="itemAcknowle_popup")
{
	...
	<script>
		...
		function load_location()
		{
			...
			load_drop_down('finish_fabric_transfer_acknowledge_controller',cbo_company_id_to, 'load_drop_down_location_to_up', 'to_location_td' );
		}
	</script>
</head>
...
{
	$data_array=sql_select("select a.id, a.entry_form, a.challan_id, a.company_id, a.store_id, a.location_id, a.transfer_criteria, a.item_category, a.acknowledg_date, a.remarks, b.transfer_system_id from inv_item_trans_acknowledgement a, inv_item_transfer_mst b where a.id='$data' and a.challan_id=b.id and a.entry_form=247 and a.transfer_criteria in(1,2,4) and a.status_active=1 and a.is_deleted=0");
	foreach ($data_array as $row)
	{
		echo "load_drop_down('requires/finish_fabric_transfer_acknowledge_controller','".$row[csf("company_id")]."', 'load_drop_down_location_to', 'to_location_td' );\n";
		...
		if($row[csf("store_id")]>0){
			...
			//echo "load_room_rack_self_bin('requires/finish_fabric_transfer_acknowledge_controller*2*cbo_store_name_to', 'store','to_store_td', '".$row[csf("company_id")]."','"."',this.value);\n";
			//echo "document.getElementById('cbo_store_name_to').value 				= '".$row[csf("store_id")]."';\n";
		}
		echo "document.getElementById('txt_challan_no').value 				= '".$row[csf("transfer_system_id")]."';\n";
...
if($action=='quantity_popup')
{
	...
	foreach ($lib_rrsb_arr as $room_rack_shelf_row) {
		...
		if($store !="" && $floor_id!="" && $room_id!="" && $rack_id!="" && $shelf_id=="" && $bin_id==""){
			...
			$lib_rack_arr[$room_id][$rack_id] = $room_rack_shelf_row[csf("rack_name")];
		}

		if($store !="" && $floor_id!="" && $room_id!="" && $rack_id!="" && $shelf_id!="" && $bin_id==""){
			$lib_shelf_arr[$rack_id][$shelf_id] = $room_rack_shelf_row[csf("shelf_name")];
		}

		if($store !="" && $floor_id!="" && $room_id!="" && $rack_id!="" && $shelf_id!="" && $bin_id!=""){
			$lib_binbox_arr[$rack_id][$shelf_id][$bin_id] = $room_rack_shelf_row[csf("bin_name")];
		}
	}
	...
	<script type="text/javascript">
		...
		function fn_addRow(i) {
			...
			$('#cbo_floor_to_' + row_num).attr("onchange", 'load_room_rack_self_bin("finish_fabric_transfer_acknowledge_controller*2*cbo_room_to_' + row_num + '*' + row_num + '","room","room_td_to","<? echo $cbo_company_id_to;?>","<? echo $location;?>","<? echo $store;?>",this.value,0,"","","","","","H");');
		...
		function fnc_close()
		{
			...
			$("#qnty_breakdown_table").find('tbody tr').each(function()
			{
				...
				var cbo_bin_to 		= $("#cbo_bin_to_"+i).val();
				var txt_trans_qnty  = $("#txt_trans_qnty_"+i).val()*1;
				var hidden_trans_id = $("#hidden_trans_id_"+i).val()*1;
				save_string 		+= cbo_floor_to + "_" + cbo_room_to + "_" + txt_rack_to + "_" + txt_shelf_to + "_" + txt_trans_qnty + "_" + hidden_trans_id + "_" + cbo_bin_to + "**";
				total_ac_qnty 		+= txt_trans_qnty;
				i++;
			});
	...
	<table cellpadding="0" width="750" cellspacing="0" border="1" id="qnty_breakdown_table" class="rpt_table" rules="all" style='margin:10px auto;'>
		...
		<tbody>
			...
			if($save_string=="") { ?>
				<tr id="tr__1">
					<td id="floor_td_to">
						<? echo create_drop_down( "cbo_floor_to_$i", 150,$lib_floor_arr,"", 1, "--Select--", 0, "load_room_rack_self_bin('finish_fabric_transfer_acknowledge_controller*2*cbo_room_to_$i*$i', 'room',this.value, '".$cbo_company_id_to."','".$location."','".$store."',this.value,'','','','','','','H');",0,"","","","","","","cbo_floor_to[]" ); ?>
					</td>
					<td id="room_td_to">
					...
					<td id="bin_td_to">
						<? echo create_drop_down( "cbo_bin_to_$i", 150,$blank_array,"", 1, "--Select--", 0, "",0,"","","","","","","cbo_bin_to[]" ); ?>
					</td>
			...
			else
			{
				...
				foreach ($save_string as $row) 
				{
					...
					//print_r($lib_binbox_arr[$data[2]][$data[3]])."<br>";

					$reset_element_floor = "reset_room_rack_self_bin('floor*to_$i');";
					$reset_element_room = "reset_room_rack_self_bin('room*to_$i');";
					$reset_element_rack = "reset_room_rack_self_bin('rack*to_$i');";
					$reset_element_shelf = "reset_room_rack_self_bin('shelf*to_$i');";

					?>
					 <tr id="tr__<? echo $i;?>">
						<td id="floor_td_to">
							<? echo create_drop_down( "cbo_floor_to_$i", 150,$lib_floor_arr,"", 1, "--Select--", $data[0], "$reset_element_floor load_room_rack_self_bin('finish_fabric_transfer_acknowledge_controller*2*cbo_room_to_$i*$i', 'room',this.value, '".$cbo_company_id_to."','".$location."','".$store."',this.value,'','','','','','','H');",0,"","","","","","","cbo_floor_to[]","onchange_void" ); ?>
						</td>
						<td id="room_td_to">
						<? echo create_drop_down( "cbo_room_to_$i", 150,$lib_room_arr[$data[0]],"", 1, "--Select--", $data[1], "$reset_element_room load_room_rack_self_bin('finish_fabric_transfer_acknowledge_controller*2*txt_rack_to_$i*$i','rack','rack_td_to','".$cbo_company_id_to."','".$location."','".$store."',document.getElementById('cbo_floor_to_".$i."').value,this.value,'','','','','','H');",0,"","","","","","","cbo_room_to[]","onchange_void" ); ?>
						</td> 
						<td id="rack_td_to">
							<? 
							echo create_drop_down( "txt_rack_to_$i", 150,$lib_rack_arr[$data[1]],"", 1, "--Select--", $data[2], "$reset_element_rack load_room_rack_self_bin('finish_fabric_transfer_acknowledge_controller*2*txt_shelf_to_$i*$i','shelf','shelf_td_to','".$cbo_company_id_to."','".$location."','".$store."','".$data[0]."','".$data[1]."',this.value,'','','','','H');",0,"","","","","","","txt_rack_to[]","onchange_void" );
							
							?>
						</td>
						<td id="shelf_td_to">
							<? echo create_drop_down( "txt_shelf_to_$i", 150,$lib_shelf_arr[$data[2]],"", 1, "--Select--", $data[3], "$reset_element_shelf load_room_rack_self_bin('finish_fabric_transfer_acknowledge_controller*2*cbo_bin_to_$i*$i','bin','bin_td_to','".$cbo_company_id_to."','".$location."','".$store."',document.getElementById('cbo_floor_to_".$i."').value, document.getElementById('cbo_room_to_".$i."').value, document.getElementById('txt_rack_to_".$i."').value,this.value,'','','','H');",0,"","","","","","","txt_shelf_to[]","onchange_void" ); ?>
						</td>

						<td id="bin_td_to">
							<? echo create_drop_down( "cbo_bin_to_$i", 150,$lib_binbox_arr[$data[2]][$data[3]],"", 1, "--Select--", $data[6], "",0,"","","","","","","cbo_bin_to[]" ); ?>
						</td>
						<td>
...
if ($action=="load_room_rack_self_bin")
{
	load_room_rack_self_bin("finish_fabric_transfer_acknowledge_controller",$data);
}
