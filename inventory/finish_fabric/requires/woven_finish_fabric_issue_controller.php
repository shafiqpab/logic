...
if($action=='show_fabric_desc_listview_requ')
{
	...
	$issData = sql_select("SELECT a.prod_id, a.batch_id,a.body_part_id,a.store_id,(case when a.floor_id is null or a.floor_id=0 then 0 else a.floor_id end) floor_id,$rackCond,(case when a.room is null or a.room=0 then 0 else a.room end) room,(case when a.self is null or a.self=0 then 0 else a.self end) self,(case when a.bin_box is null or a.bin_box=0 then 0 else a.bin_box end) bin_box, sum(case when e.trans_type=2 then e.quantity end) as issue_qnty,sum(case when e.trans_type=2 then a.cons_rate*e.quantity  end) as cons_amount
	from inv_issue_master b,inv_transaction a,inv_wvn_finish_fab_iss_dtls c, pro_batch_create_mst d ,order_wise_pro_details e
	where b.entry_form=19 and b.id=a.mst_id and a.id=c.trans_id and a.status_active=1 and a.is_deleted=0 and a.item_category=3 and a.transaction_type=2 and c.status_active=1 and c.is_deleted=0 and a.batch_id = d.id and a.id=e.trans_id and e.trans_type=2 and e.status_active=1 and e.is_deleted=0 and e.po_breakdown_id in($po_id)  and d.booking_no in($bookingNos) and b.status_active=1 and b.is_deleted=0 and b.company_id=$company_id and a.batch_id in(".$batchIds.") group by a.prod_id, a.batch_id, a.body_part_id,a.store_id,a.floor_id, a.room, a.rack, a.self,a.bin_box");
	//and b.req_id=$requ_mst_id  


	foreach($issData as $row)
	{
		$issue_qty_array[$row[csf('prod_id')]][$row[csf('batch_id')]][$row[csf('body_part_id')]][$row[csf('store_id')]][$row[csf('floor_id')]][$row[csf('room')]][$row[csf('rack')]][$row[csf('self')]][$row[csf('bin_box')]]+=$row[csf('issue_qnty')];
		$issue_amount_array[$row[csf('prod_id')]][$row[csf('batch_id')]][$row[csf('body_part_id')]][$row[csf('store_id')]][$row[csf('floor_id')]][$row[csf('room')]][$row[csf('rack')]][$row[csf('self')]][$row[csf('bin_box')]]+=$row[csf('cons_amount')];
	}


	$recvRt_qty_array=array(); $issRt_qty_array=array();

	$receiveReturnData=sql_select("select a.prod_id, a.batch_id_from_fissuertn as batch_id, a.body_part_id, a.floor_id,a.room, a.rack, a.self,a.bin_box, sum(case when a.transaction_type=3 then a.cons_quantity end) as recvrqnty,sum(case when d.trans_type=3 then a.cons_rate*d.quantity  end) as cons_amount from inv_transaction a,pro_batch_create_mst c, order_wise_pro_details d  where a.status_active=1 and a.is_deleted=0 and a.item_category=3 and a.transaction_type in(3) and a.batch_id_from_fissuertn = c.id and a.id=d.trans_id and d.status_active=1 and d.is_deleted=0 and d.trans_type=3 and d.po_breakdown_id in($po_id)  and c.booking_no in($bookingNos) and a.company_id=$company_id and d.entry_form=202 and a.batch_id_from_fissuertn in($batchIds) group by a.prod_id, a.body_part_id, a.batch_id_from_fissuertn,a.floor_id,a.room, a.rack, a.self,a.bin_box");
	...
	$issueReturnData=sql_select("select a.prod_id, a.batch_id_from_fissuertn as batch_id, a.body_part_id,a.floor_id,a.room, a.rack, a.self,a.bin_box, sum(case when a.transaction_type=4 then a.cons_quantity end) as issrqnty,sum(case when d.trans_type=4 then a.cons_rate*d.quantity end) as cons_amount from inv_transaction a,pro_batch_create_mst c,order_wise_pro_details d where a.batch_id_from_fissuertn = c.id and a.id=d.trans_id and d.trans_type=4 and a.status_active=1 and a.is_deleted=0 and a.item_category=3 and a.transaction_type in(4)  and d.status_active=1 and d.is_deleted=0 and d.po_breakdown_id in($po_id) and c.booking_no in($bookingNos) and a.company_id=$company_id and a.batch_id_from_fissuertn in($batchIds) and d.entry_form=209 group by a.prod_id, a.body_part_id, a.batch_id_from_fissuertn,a.floor_id,a.room, a.rack, a.self,a.bin_box"); 
	...
	$sql_rackWiseBalanceShow=sql_select("select id, rack_balance from variable_settings_inventory where company_name=$company_id and item_category_id=3 and variable_list=21 and status_active=1 and is_deleted=0");
	$varriable_setting_rack_self_maintain=$sql_rackWiseBalanceShow[0][csf('rack_balance')];
	if ($varriable_setting_rack_self_maintain==1) $table_width=1115;
	else $table_width=815;
    ...
    <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="<? echo $table_width; ?>">
        <thead>
	        ...
	        ?>
            <th width="60" title="Style Wise Global Recv Qnty">Recv. Qty</th>
            <th width="60" title="Style Wise Global Issue Qnty">Issue Qty</th>
            <th width="60" title="Style Wise Global Balance Qnty">Balance</th>
            <th width="60" title="Style Wise Only Requisition Issue Qnty">Req. Issue</th>
            <th title="Style Wise Only Requisition Balance Qnty. This balance not consider in style popup balance Quantity">Req. Balance</th>
    ...
    <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="<? echo $table_width; ?>" id="fabric_listview">
        ...
        foreach($data_array as $row)
        {
			...
			$iss_qnty=$issue_qty_array[$row[csf('id')]][$row[csf('batch_id')]][$row[csf('body_part_id')]][$row[csf('store_id')]][$row[csf('floor')]][$row[csf('room')]][$row[csf('rack_no')]][$row[csf('shelf_no')]][$row[csf('bin')]];
			$recvRt_qnty=$recvRt_qty_array[$row[csf('id')]][$row[csf('batch_id')]][$row[csf('body_part_id')]][$row[csf('floor')]][$row[csf('room')]][$row[csf('rack_no')]][$row[csf('shelf_no')]][$row[csf('bin')]];
			$issRt_qnty=$issRt_qty_array[$row[csf('id')]][$row[csf('batch_id')]][$row[csf('body_part_id')]][$row[csf('floor')]][$row[csf('room')]][$row[csf('rack_no')]][$row[csf('shelf_no')]][$row[csf('bin')]];
			...
			//Amount
			$iss_amount=$issue_amount_array[$row[csf('id')]][$row[csf('batch_id')]][$row[csf('body_part_id')]][$row[csf('store_id')]][$row[csf('floor')]][$row[csf('room')]][$row[csf('rack_no')]][$row[csf('shelf_no')]][$row[csf('bin')]];
			...
			if($balance>0)
			{
	            ...
	            <tr bgcolor="<? echo $bgcolor; ?>" onClick='set_form_data("<? echo $row[csf('id')]."**".$row[csf('product_name_details')]."**".$row[csf('current_stock')]."**".$row[csf('body_part_id')]."**".$unit_of_measurement[$row[csf('unit_of_measure')]]."**".$row[csf('booking_without_order')]."**".$row[csf('batch_id')]."**".$row[csf('color')]."**".$row[csf('dia_width')]."**".$row[csf('weight')]."**".$row[csf('rack_no')]."**".$row[csf('shelf_no')]."**".$color_arr[$row[csf('color')]]."**".$floor_name."**".$room_name."**".$rack_name."**".$shelf_name."**".$row[csf('floor')]."**".$row[csf('room')]."**".$batch_arr[$row[csf('batch_id')]]."**".$row[csf('store_id')]."**".$cons_rate."**".$row[csf('detarmination_id')]."**".$row[csf('bin')]."**".$bin."**".$row[csf('unit_of_measure')]."**".$row[csf('fabric_ref')]."**".$row[csf('rd_no')]."**".$row[csf('weight_type')]."**".$row[csf('cutable_width')]."**".$row[csf('original_gsm')]."**".$row[csf('original_width')]."**".$row[csf('po_breakdown_id')]."**".$requitionBalance; ?>")' style="cursor:pointer" >
					...
					<td width="60" align="right" title="<? echo $cum_iss_qty_title; ?>"><? echo number_format($cum_iss_qty,2,'.',''); ?></td>
					<td width="60" align="right" title="<? echo $ref_title?>"><? echo number_format($balance,2,'.',''); ?></td>
					<td width="60" align="right" title="<? echo $ref_title?>"><? echo number_format($requisition_iss_qnty,2,'.',''); ?></td>
			...
			<fieldset style="width:820px;margin-left:10px">
	            ...
	            <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="850">
	                <thead>
	                    ...
	                    <th width="120">Booking No</th>
	                    <th width="100" title="<? if($issue_basis == 1){echo "Booking Required Qnty";}else{echo "Requisition Qnty";} ?>">Req. Qnty</th>
	            ...
	            <div style="width:868px; max-height:280px; overflow-y:scroll" id="list_container" align="left">
	                <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="850" id="tbl_list_search">
	                <?
						...
						$shipmentDeliveryStatusSql = sql_select("select a.po_break_down_id,c.job_no,c.style_ref_no,d.booking_no,a.shiping_status 
						from pro_ex_factory_mst a,wo_po_break_down b, wo_po_details_master c,wo_booking_dtls d 
						where a.po_break_down_id=b.id and b.job_id=c.id and c.job_no=d.job_no and b.id=d.po_break_down_id and a.po_break_down_id in($poIDS) and d.booking_no in($bookingNos)
						...
						group by a.po_break_down_id,c.job_no,c.style_ref_no,d.booking_no,a.shiping_status");
						foreach($shipmentDeliveryStatusSql as $rowData)
						{
							//$shippingStatusArr[$rowData[csf('job_no')]][$rowData[csf('booking_no')]][$rowData[csf('style_ref_no')]]['shiping_status']=$rowData[csf('shiping_status')];
							$shippingStatusPoArr[$rowData[csf('po_break_down_id')]][$rowData[csf('job_no')]][$rowData[csf('booking_no')]][$rowData[csf('style_ref_no')]]['shiping_status_po_wise']=$rowData[csf('shiping_status')];
						}
						...
						if($issue_basis == 2) // Requisition basis
						{
							...
							$prev_requsition_issue_sql=sql_select("select a.po_breakdown_id as order_id, b.transaction_type,a.quantity as quantity,c.job_no,c.booking_no ,d.style_ref_no from inv_issue_master x,inv_wvn_finish_fab_iss_dtls y,inv_transaction b, order_wise_pro_details a,wo_booking_dtls c,wo_po_details_master d,wo_pre_cost_fabric_cost_dtls e,lib_yarn_count_determina_mst f where x.id=y.mst_id and y.trans_id=b.id and b.id=a.trans_id and a.po_breakdown_id=c.po_break_down_id and c.job_no=d.job_no and d.job_no=e.job_no and c.pre_cost_fabric_cost_dtls_id=e.id  and e.lib_yarn_count_deter_id=f.id and a.entry_form in (19) and b.transaction_type in (2) and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and b.item_category=3 and b.company_id=$cbo_company_id and b.prod_id=$hidden_prod_id and a.prod_id=$hidden_prod_id and b.batch_id=$txt_batch_id $floor_cond $room_cond $rack_cond $shelf_cond $bin_cond  $store_cond $bodyPartCond_2  $txt_width_cond $txt_weight_cond and c.fabric_color_id=$hidden_color_id $txt_fabric_ref_cond $txt_rd_no_cond $cbo_weight_type_cond $txt_cutable_width_cond and e.lib_yarn_count_deter_id=$fabric_desc_id and a.po_breakdown_id in($hidden_selected_po_ids) and x.req_id=$requisition_id group by a.po_breakdown_id, b.transaction_type,a.quantity,c.job_no,c.booking_no ,d.style_ref_no");
							foreach($prev_requsition_issue_sql as $row)
							{
 								$cumu_current_requisition_issue_qty_arr[$row[csf('job_no')]][$row[csf('booking_no')]][$row[csf('style_ref_no')]]['qnty']+=$row[csf('quantity')];
							}

							$prev_issue_sql=sql_select("select a.po_breakdown_id as order_id, b.transaction_type,a.quantity as quantity,c.job_no,c.booking_no ,d.style_ref_no from inv_issue_master x,inv_wvn_finish_fab_iss_dtls y,inv_transaction b, order_wise_pro_details a,wo_booking_dtls c,wo_po_details_master d,wo_pre_cost_fabric_cost_dtls e,lib_yarn_count_determina_mst f where x.id=y.mst_id and y.trans_id=b.id and b.id=a.trans_id and a.po_breakdown_id=c.po_break_down_id and c.job_no=d.job_no and d.job_no=e.job_no and c.pre_cost_fabric_cost_dtls_id=e.id  and e.lib_yarn_count_deter_id=f.id and a.entry_form in (19) and b.transaction_type in (2) and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and b.item_category=3 and b.company_id=$cbo_company_id and b.prod_id=$hidden_prod_id and a.prod_id=$hidden_prod_id and b.batch_id=$txt_batch_id $floor_cond $room_cond $rack_cond $shelf_cond $bin_cond  $store_cond $bodyPartCond_2  $txt_width_cond $txt_weight_cond and c.fabric_color_id=$hidden_color_id $txt_fabric_ref_cond $txt_rd_no_cond $cbo_weight_type_cond $txt_cutable_width_cond and e.lib_yarn_count_deter_id=$fabric_desc_id and a.po_breakdown_id in($hidden_selected_po_ids) group by a.po_breakdown_id, b.transaction_type,a.quantity,c.job_no,c.booking_no ,d.style_ref_no");
							foreach($prev_issue_sql as $row)
							{
								$cumu_issue_qty[$row[csf('order_id')]]+=$row[csf('quantity')];
									...
									foreach($booking_data as $styleRef => $row)
									{
										...
										//{
											...
											foreach ($po_arr_uniq as $poID) {
												...
												if($shippingStatusPoArr[$poID][$jobNo][$bookingNo][$styleRef]['shiping_status_po_wise']==2 || $shippingStatusPoArr[$poID][$jobNo][$bookingNo][$styleRef]['shiping_status_po_wise']=="")
												{
													$shipstaPoArr[]=$poID;
													$shipGetStatus=1;
												}	
											...
											if($shipGetStatus==1)
											{
												if(count($po_arr_uniq)>=count($shipstaPoArr))
												{
													$shippingStatus=2;
												}
												$shippingStatus=2;
											...
											if ($issue_basis == 2) {
												...
												$cumuIissQty=($cumu_requsition_issue_qty_arr[$jobNo][$bookingNo][$styleRef]["qnty"]+$prevTransOutQnty)-$prevIssueRtn_recv_Qnty;
												$cumuCurrentRequisitionIssQty=$cumu_current_requisition_issue_qty_arr[$jobNo][$bookingNo][$styleRef]["qnty"];
												$cumul_balance=$requ_qty-$cumuIissQty;
												$cumul_currentRequisiton_balance=$requ_qty-$cumuCurrentRequisitionIssQty;
											}
											//$shippingStatus=$shippingStatusArr[$jobNo][$bookingNo][$styleRef]['shiping_status'];
											if(($cbo_issue_purpose==9 || $cbo_issue_purpose==4) && $shippingStatus==3){$disabled= "disabled"; $msg="Disabled for Full Delivery Found";}
					                     	...
					                     	<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
					                        	<td align="center" width="100" title="<? echo  $requ_qty; ?>">
					                        		<? echo $jobNo; ?>
					                        	</td>
					                            ...
					                            <td width="100" align="center">
					                                ...
					                                if($update_dtls_id!="")
					                                {
					                                	...
					                                	{*/
					                                		...
					                                		$cumul_currentRequisiton_balance=($requ_qty-($cumuCurrentRequisitionIssQty-$iss_qty));
					                                	//}
					                                	if($iss_qty>0)
														{
					                            ...
					                            <td width="40">
				                					<input type="text" name="txtRollNo[]" id="txtRollNo_<? echo $i; ?>" class="text_boxes_numeric" style="width:28px" value="<? echo $roll_no; ?>" <? echo $disabled; ?>>
				                				</td>
					                            <td align="center">
					                                <input title="<? echo $disabled; ?>" type="text" name="txtIssueQnty[]" id="txtIssueQnty_<? echo $i; ?>" class="text_boxes_numeric" style="width: 60px" value="<? if($iss_qty>0){echo $iss_qty;} ?>" placeholder="<? 
					                                	//echo $cumul_balance; 
					                                	echo $cumul_currentRequisiton_balance;

					                            		?>" onChange="qty_check(<? echo $i ?>,<? echo $issue_basis ?>)" onKeyUp="check_balance(<? echo $i; ?>);" <? echo $disabled; ?>>
										...
										//{
											...
											if(($cbo_issue_purpose==9 || $cbo_issue_purpose==4) && $shippingStatus==3){$disabled= "disabled"; $msg="Disabled for Full Delivery Found";}
					                     	...
					                     	<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
					                            ...
					                            <td align="center">
					                                <input title="<? echo $msg; ?>" type="text" name="txtIssueQnty[]" id="txtIssueQnty_<? echo $i; ?>" class="text_boxes_numeric" style="width: 60px" value="<? if($iss_qty>0){echo $iss_qty;} ?>" placeholder="<? echo $cumul_balance; ?>" onChange="qty_check(<? echo $i ?>,<? echo $issue_basis ?>)" onKeyUp="check_balance(<? echo $i; ?>);" 
					                                <? echo $disabled; ?>>
