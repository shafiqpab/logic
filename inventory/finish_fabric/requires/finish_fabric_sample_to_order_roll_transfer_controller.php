        ...
        <legend>Enter search words</legend>
            <table cellpadding="0" cellspacing="0" width="850" class="rpt_table" border="1" rules="all">
                ...
                <tr class="general">
                    ...
                    <td>
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_order_no').value+'_'+<? echo $cbo_company_id; ?>+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_file_no').value+'_'+document.getElementById('txt_ref_no').value, 'create_po_search_list_view', 'search_div', 'finish_fabric_sample_to_order_roll_transfer_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:80px;" />
                    </td>
                </tr>
                <tr>
...
if($action=='create_po_search_list_view')
{
	...
	$sql= "select a.job_no, $year_field, a.job_no_prefix_num,a.company_name, a.buyer_name, a.style_ref_no, a.job_quantity, b.id, b.po_number, b.po_quantity, b.pub_shipment_date as shipment_date, b.file_no, b.grouping as ref_no from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.company_name=$company_id and a.buyer_name like '$buyer' and b.po_number like '$search_string' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  $shipment_date $str_cond $year_cond order by b.id, b.pub_shipment_date";  
...
if($action=="show_transfer_listview")
{
	...
	$transfer_arr=array();
	$transfer_dataArray=sql_select("select a.id, a.trans_id, a.to_trans_id, b.id as roll_id, b.barcode_no from inv_item_transfer_dtls a, pro_roll_details b where a.id=b.dtls_id and a.mst_id=$mst_id and b.mst_id=$mst_id and b.entry_form=219 and b.transfer_criteria=7 and b.status_active=1 and b.is_deleted=0");
	...
	$re_trans_arr=return_library_array("select barcode_no from pro_roll_details where entry_form=219 and re_transfer=1 and status_active=1 and is_deleted=0 and mst_id=$mst_id ", "barcode_no", "barcode_no");
	...
	$sql="select a.id, a.entry_form, 0 as receive_basis, 0 as booking_id, b.from_prod_id as prod_id, b.to_rack as rack, b.to_shelf as self,b.feb_description_id,b.color_id,b.gsm,b.dia_width, c.id as roll_id, c.barcode_no, c.po_breakdown_id, c.roll_no, c.qnty, c.roll_id as roll_id_prev, 3 as type 
	from inv_item_transfer_mst a, inv_item_transfer_dtls b, pro_roll_details c 
	WHERE a.id=b.mst_id and b.id=c.dtls_id and a.entry_form in(219) and c.entry_form in(219) and c.status_active=1 and c.is_deleted=0 and c.mst_id=$mst_id
	order by barcode_no";
        ...
        <legend>Enter search words</legend>
            <table cellpadding="0" cellspacing="0" width="550" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td>
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+<? echo $cbo_company_id; ?>, 'create_transfer_search_list_view', 'search_div', 'finish_fabric_sample_to_order_roll_transfer_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                    </td>
                </tr>
            </table>
        	<div style="margin-top:10px" id="search_div"></div> 
...
if($action=='populate_data_from_transfer_master')
{
	...
	{ 
		...
		echo "get_php_form_data('".$row[csf("from_order_id")]."','populate_data_from_sample','requires/finish_fabric_sample_to_order_roll_transfer_controller');\n";
		echo "get_php_form_data('".$row[csf("to_order_id")]."','populate_data_from_order','requires/finish_fabric_sample_to_order_roll_transfer_controller');\n";
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$update_id)=="")
		{
			...
			$new_transfer_system_id = explode("*", return_next_id_by_sequence("INV_ITEM_TRANSFER_MST_PK_SEQ", "inv_item_transfer_mst",$con,1,$cbo_company_id,"FFSTOTE",219,date("Y",time()),13 ));
			...
			$data_array="(".$id.",'".$new_transfer_system_id[1]."',".$new_transfer_system_id[2].",'".$new_transfer_system_id[0]."',".$cbo_company_id.",".$txt_challan_no.",".$txt_transfer_date.",219,7,0,".$txt_sam_book_id.",".$txt_order_id.",2,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		{ 	
			...
			$data_array_trans.="(".$id_trans.",".$transfer_update_id.",".$cbo_company_id.",".$$productId.",2,6,".$txt_transfer_date.",".$txt_sam_book_id.",12,".$$rollWgt.",".$rate.",".$amount.",".$$rack.",".$$shelf.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			
			$from_trans_id=$id_trans;
			//$id_trans=$id_trans+1;
			$id_trans = return_next_id_by_sequence("INV_TRANSACTION_PK_SEQ", "inv_transaction", $con);
			$data_array_trans.=",(".$id_trans.",".$transfer_update_id.",".$cbo_company_id.",".$$productId.",2,5,".$txt_transfer_date.",".$txt_order_id.",12,".$$rollWgt.",".$rate.",".$amount.",".$$rack.",".$$shelf.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			...
			$data_array_roll.="(".$id_roll.",".$$barcodeNo.",".$transfer_update_id.",".$id_dtls.",".$txt_order_id.",219,".$$rollWgt.",".$$rollNo.",".$$rollId.",".$$transRollId.",5,7,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			
			if($data_array_prop!="") $data_array_prop.= ",";
			$data_array_prop.="(".$id_prop.",".$from_trans_id.",5,219,".$id_dtls.",".$txt_order_id.",".$$productId.",".$$rollWgt.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		{ 	
			...
			if(str_replace("'","",$$rolltableId)>0)
			{
				...
				$data_array_update_trans[str_replace("'","",$$transIdFrom)]=explode("*",($$productId."*".$txt_transfer_date."*".$txt_sam_book_id."*".$$rollWgt."*".$rate."*".$amount."*".$$rack."*".$$shelf."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
				
				$transId_arr[]=str_replace("'","",$$transIdTo);
				$data_array_update_trans[str_replace("'","",$$transIdTo)]=explode("*",($$productId."*".$txt_transfer_date."*".$txt_order_id."*".$$rollWgt."*".$rate."*".$amount."*".$$rack."*".$$shelf."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
			...
			else
			{
				...
				if($data_array_trans!="") $data_array_trans.=",";
				$data_array_trans.="(".$id_trans.",".$update_id.",".$cbo_company_id.",".$$productId.",2,6,".$txt_transfer_date.",".$txt_sam_book_id
.",12,".$$rollWgt.",".$rate.",".$amount.",".$$rack.",".$$shelf.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				
				$transIdfromProp=$id_trans;
				//$id_trans=$id_trans+1;
				$id_trans = return_next_id_by_sequence("INV_TRANSACTION_PK_SEQ", "inv_transaction", $con);
				$transIdtoProp=$id_trans;
				$data_array_trans.=",(".$id_trans.",".$update_id.",".$cbo_company_id.",".$$productId.",2,5,".$txt_transfer_date.",".$txt_order_id.",12,".$$rollWgt.",".$rate.",".$amount.",".$$rack.",".$$shelf.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				...
				if($data_array_roll!="") $data_array_roll.= ",";
				$data_array_roll.="(".$id_roll.",".$$barcodeNo.",".$update_id.",".$id_dtls.",".$txt_order_id.",219,".$$rollWgt.",".$$rollNo.",".$$rollId.",".$$transRollId.",5,7,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			...
			$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
			if($data_array_prop!="") $data_array_prop.= ",";
			$data_array_prop.="(".$id_prop.",".$transIdfromProp.",5,219,".$dtlsIdProp.",".$txt_order_id.",".$$productId.",".$$rollWgt.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			//$id_prop=$id_prop+1;
		...
		if($update_dtls_id!="")
		{
			$query = execute_query("DELETE FROM order_wise_pro_details WHERE dtls_id in(".$update_dtls_id.") and entry_form=219");
			if($flag==1) 
			{
...
<div style="width:930px;">
    ...
    <div style="width:100%;">
        <table align="right" cellspacing="0" width="900"  border="1" rules="all" class="rpt_table" >
            ...
            <tbody> 
			<?
            $sql_dtls="select a.from_prod_id, a.transfer_qnty, a.uom, a.color_id, a.gsm, a.dia_width, a.to_rack, a.to_shelf, a.feb_description_id, b.barcode_no, b.roll_no from inv_item_transfer_dtls a, pro_roll_details b where a.id=b.dtls_id and b.entry_form=219 and a.mst_id='$data[1]' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by b.barcode_no";
