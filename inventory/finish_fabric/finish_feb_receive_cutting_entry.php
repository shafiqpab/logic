 ...
 //alert(issue_details_arr )
	...
	function fnc_grey_receive_roll_wise( operation )
	{
	 ...
	 /*	if(form_validation('txt_delivery_date*cbo_company_id*cbo_knitting_source*txt_knit_company','Delivery Date*Company*Knitting Source*Knitting Company')==false)
		...
		{
			//var barcodeNo=$(this).find('input[name="barcodNumber[]"]').val();alert(333+"**"+barcodeNo);return;
			//alert($(this).find('input[name="checkRow[]"]').val());
			...
			'=' + rolldia + '&bookingNo_' + j + '=' + bookingNo+ '&updateDetailsId_' + j + '=' + updateDetailsId+ '&activeId_' + j + '=' + activeId+ '&barcodeNo_' + j + '=' + barcodeNo+ '&rollNo_' + j + '=' + rollNo;
		...
		var data="action=save_update_delete&operation="+operation+'&tot_row='+j+get_submitted_data_string('txt_delivery_date*txt_challan_no*cbo_company_id*cbo_knitting_source*cbo_knitting_company*update_id*txt_system_no*cbo_basis*txt_batch_no',"../../")+dataString;
	
		http.open("POST","requires/finish_feb_receive_cutting_entry_controller.php",true);
	...
	function grey_receive_popup()
	{
		...
		var garments_nature =2;
			...
			var page_link='requires/finish_feb_receive_cutting_entry_controller.php?cbo_company_id='+cbo_company_id+'&garments_nature='+garments_nature+'&action=challan_popup';
			...
			emailwindow.onclose=function()
			{
				...
				{
					show_list_view(grey_recv_no, 'grey_item_details', 'scanning_tbl','requires/finish_feb_receive_cutting_entry_controller', '' );
					get_php_form_data(grey_recv_no, "load_php_form", "requires/finish_feb_receive_cutting_entry_controller" );
...
{
	var company=1;
	var page_link='requires/finish_feb_receive_cutting_entry_controller.php?action=mrr_popup&company='+company; 
  ...
  function scan_challan_no(str)
		 {
			...
			show_list_view(str, 'grey_item_details', 'scanning_tbl','requires/finish_feb_receive_cutting_entry_controller', '' );
			get_php_form_data(str, "load_php_form", "requires/finish_feb_receive_cutting_entry_controller" );
	...
	function open_mrrpopup()
	{
		...
		var garments_nature =2;
			...
			var page_link='requires/finish_feb_receive_cutting_entry_controller.php?cbo_company_id='+cbo_company_id+'&garments_nature='+garments_nature+'&action=update_system_popup';
			var title='Grey Receive Form';
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=800px,height=390px,center=1,resize=1,scrolling=0','../');
			emailwindow.onclose=function()
			{
				...
				if(trim(grey_recv_id)!="")
				{
					show_list_view(grey_recv_id, 'grey_item_details_update', 'scanning_tbl','requires/finish_feb_receive_cutting_entry_controller', '' );
					get_php_form_data(grey_recv_id, "load_php_form_update", "requires/finish_feb_receive_cutting_entry_controller" );
					set_button_status(1, permission, 'fnc_grey_receive_roll_wise',1);
					//release_freezing();
				}
    ...
    <form name="rollscanning_1" id="rollscanning_1"  autocomplete="off"  >
                ...
                <table cellpadding="0" cellspacing="2" width="800">
                    ...
                    <td align="right">Dyeing Company</td>
                      ...
                      <td align="right" class="must_entry_caption" width="100">Receive Date</td>
                        ...
                        <td width="120" align="right" class="must_entry_caption">Purpose</td>
            ...
            <br/>
              <fieldset style="width:1260px;text-align:left">
				...
				<table cellpadding="0" width="1215" cellspacing="0" border="1" id="scanning_tbl_top" class="rpt_table" rules="all">
                    <thead>
                    	<th width="60">SL</th>
                 ...
                 <div style="width:1240px; max-height:250px; overflow-y:scroll" align="left">
                    ...
                    <tr>
                        <td align="center" class="button_container">
                            ...
                            <? 
                               echo load_submit_buttons($permission,"fnc_grey_receive_roll_wise",0,0,"",1);
                            ?>
                        </td>
                    </tr>  
                </table>
