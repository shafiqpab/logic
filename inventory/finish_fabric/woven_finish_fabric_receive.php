...
<script>
	...
	<?
		...
		if($_SESSION['logic_erp']['mandatory_field'][17]!="")
		{
			$mandatory_field_arr= json_encode( $_SESSION['logic_erp']['mandatory_field'][17] );
...
function openmypage(page_link,title)
{
	...
	emailwindow.onclose=function()
	{
			...
			if (rowID!="")
			{
				...
				get_php_form_data(receive_basis+"**"+rowID+"**"+hidden_is_non_ord_sample+"**"+wopiNumber+"**"+hidden_basis+"**"+style_and_po_wise_variable, "populate_data_from_wopi_popup", "requires/woven_finish_fabric_receive_controller" );
				
				show_list_view(receive_basis+"**"+rowID+"**"+hidden_is_non_ord_sample+"**"+wopiNumber,'show_product_listview','list_product_container','requires/woven_finish_fabric_receive_controller','setFilterGrid(\'table_body\',-1);');
...
function fn_independent(val)
{
	...
	$('#cbo_supplier').val(0);
	reset_form('yarn_receive_1','list_product_container','','','','cbo_company_id*cbo_receive_basis*txt_receive_date*roll_maintained*barcode_generation*cbouom*hdn_batch_control_variable*style_and_po_wise_variable');
	...
	if(val==4)
	{
		...
		$("#cbouom").removeAttr('disabled','disabled');
		$("#txt_cutable_width").removeAttr('disabled','disabled');
		$("#cbo_weight_type").removeAttr('disabled','disabled');
		$("#txt_fabric_ref").removeAttr('disabled','disabled');
		$("#txt_rd_no").removeAttr('disabled','disabled');
...
function fnc_woben_finish_fab_receive_entry(operation)
{
	...
	else
	{
		...
		if(fabric_source == 3)
		{
			if(hdn_batch_control_variable==1)
			{
				if( form_validation('cbo_company_id*cbo_receive_basis*txt_receive_date*txt_challan_no*cbo_store_name*cbo_supplier*cbo_currency*txt_exchange_rate*cbo_source*txt_fabric_description*txt_color*txt_receive_qty*cbo_body_part*txt_roll','Company Name*Receive Basis*Receive Date*Challan No*Store Name*Supplier*Currency*Exchange Rate*Source*Fabric Desc*Color*Receive Quantity*Body Part*Roll')==false )
				{
					return;
				}
			}
		...
		{ 
			if(hdn_batch_control_variable==1)
			{
				if( form_validation('cbo_company_id*cbo_receive_basis*txt_receive_date*txt_challan_no*cbo_store_name*cbo_supplier*cbo_currency*txt_exchange_rate*cbo_source*txt_fabric_description*txt_color*txt_receive_qty*txt_rate*cbo_body_part*txt_roll','Company Name*Receive Basis*Receive Date*Challan No*Store Name*Supplier*Currency*Exchange Rate*Source*Fabric Desc*Color*Receive Quantity*Rate*Body Part*Roll')==false )
				{
					return;
				}
			}
		...
		<?php 
			...
			if(!empty($_SESSION['logic_erp']['mandatory_field'][17][4])) echo " isFileMandatory = ". $_SESSION['logic_erp']['mandatory_field'][17][4] . ";\n";
		?>
		...
		if('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][17]); ?>') 
		{
			if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][17]); ?>','<? echo implode('*',$_SESSION['logic_erp']['mandatory_message'][17]); ?>')==false) {return;}
		}
...
{	
	if(http.readyState == 4) 
	{
		...
		if((reponse[0]==0 || reponse[0]==1))
		{
			...
			var compID = $("#cbo_company_id").val();

			show_list_view(reponse[2]+'**'+reponse[1]+'**'+compID,'show_dtls_list_view','list_container_yarn','requires/woven_finish_fabric_receive_controller','');
		...
		else if(reponse[0]==2)
		{
			...
			if(reponse[3]==2)
			{
				...
				reset_form('','','txt_fabric_description*original_fabric_description*fabric_desc_id*txt_color*txt_width*txt_weight*txt_batch_lot*txt_receive_qty*txt_rate*txt_ile*txt_amount*txt_book_currency*txt_bla_order_qty*txt_prod_code*cbo_floor*cbo_room*txt_rack*txt_shelf*cbo_bin*save_data*update_dtls_id*update_trans_id*previous_prod_id*hidden_receive_qnty*all_po_id*cbo_body_part*txt_roll*txt_remarks*hdn_booking_no*hdn_booking_id*txt_fabric_ref*txt_rd_no*cbo_weight_type*txt_cutable_width*update_finish_fabric_id*txt_weight_edit*txt_width_edit*cbo_pay_over_rcv_qty','','','');
			}
...
function uploadFile(txt_mrr_no)
{
	$(document).ready(function() { 
		...
		for( var i = 0 ; i < $("#multiple_file_field")[0].files.length ; i++)
		{
			...
			// alert(txt_mrr_no);
			$.ajax({
				url: 'requires/woven_finish_fabric_receive_controller.php?action=file_upload&txt_mrr_no='+ txt_mrr_no, 
				type: 'post', 
				data:fd, 
				contentType: false, 
				processData: false, 
...
function open_mrrpopup()
{
	...
	emailwindow.onclose=function()
	{
		reset_form('yarn_receive_1','list_product_container','','','','cbo_company_id*roll_maintained*barcode_generation*hdn_batch_control_variable*style_and_po_wise_variable');
		var theform=this.contentDoc.forms[0]; 
...
function openmypage_po()
{
	...
	var txt_weight_original = $('#txt_weight').val();
	var txt_width_original = $('#txt_width').val();
	...
	var po_popup_patern_variable = $('#style_and_po_wise_variable').val(); 
	var cbo_pay_over_rcv_qty=$('#cbo_pay_over_rcv_qty').val(); 
	...
	if(receive_basis==6 && cbo_currency==0)
	{
	...
	var po_popup_patern_variable=1;
	...
	var page_link = 'requires/woven_finish_fabric_receive_controller.php?receive_basis='+receive_basis+'&cbo_company_id='+cbo_company_id+'&booking_no='+booking_no+'&dtls_id='+dtls_id+'&all_po_id='+all_po_id+'&roll_maintained='+roll_maintained+'&save_data='+save_data+'&txt_receive_qnty='+txt_receive_qnty+'&prev_distribution_method='+distribution_method+'&txt_deleted_id='+txt_deleted_id+'&hidden_pi_id='+hidden_pi_id+'&hdn_buyer_id='+hdn_buyer_id+'&txt_color='+txt_color+'&hidden_color_id='+hidden_color_id+'&txt_bla_order_qty='+txt_bla_order_qty+'&cbo_body_part_id='+cbo_body_part_id+'&hidden_dia_width='+hidden_dia_width+'&txt_width_original='+txt_width_original+'&fabric_desc_id='+fabric_desc_id+'&update_hdn_transaction_id='+update_hdn_transaction_id+'&hidden_gsm_weight='+hidden_gsm_weight+'&txt_weight_original='+txt_weight_original+'&txt_rate='+txt_rate+'&txt_fabric_ref='+txt_fabric_ref+'&txt_rd_no='+txt_rd_no+'&cbo_weight_type='+cbo_weight_type+'&txt_cutable_width='+txt_cutable_width+'&txt_weight_edit='+txt_weight_edit+'&txt_width_edit='+txt_width_edit+'&cbouom='+cbouom+'&hdn_booking_id='+hdn_booking_id+'&hdn_booking_no='+hdn_booking_no+'&all_booking_id='+all_booking_id+'&all_booking_no='+all_booking_no+'&booking_without_order='+booking_without_order+'&cbo_pay_over_rcv_qty='+cbo_pay_over_rcv_qty+'&action='+actionName;   
  	...
  	emailwindow.onclose=function()
  	{
		...
		{
			$("#txt_bla_order_qty").val('');
			$('#all_booking_id').val(all_booking_id);
			$('#all_booking_no').val(all_booking_no);
		}
		...
		$('#cbo_pay_over_rcv_qty').attr("disabled",true);
		fn_calile();
	}
}
...
function fn_report_generated(type)
{
	...
	{
		var report_title=$( "div.form_caption" ).html();
			print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+ $('#txt_wo_pi_id').val()+'*'+report_title, "gwoven_finish_fabric_receive_print_4", "requires/woven_finish_fabric_receive_controller" ) 
			return;
	}
	else if(type==5)
	{
		var report_title=$( "div.form_caption" ).html();
			print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+ $('#txt_wo_pi_id').val()+'*'+report_title, "gwoven_finish_fabric_receive_print_5", "requires/woven_finish_fabric_receive_controller" ) 
			return;
	}
...
function put_data_dtls_part(datas)
{
	...
	var booking_without_order = $('#booking_without_order').val();	
	var styleAndPOwiseVar = $('#style_and_po_wise_variable').val();	
	var datas=encodeURIComponent(datas);

	get_php_form_data(datas+"**"+styleAndPOwiseVar, "child_form_input_data", "requires/woven_finish_fabric_receive_controller");
...
{
	var varible_data=return_global_ajax_value( data, 'varible_inv_auto_batch_maintain', '', 'requires/woven_finish_fabric_receive_controller');
	if(varible_data==1)
	{
...
{
	var varible_string=return_global_ajax_value( data, 'varible_inventory_upto_rack', '', 'requires/woven_finish_fabric_receive_controller');
...
function independence_basis_controll_function(data)
{
	...
	var varible_string=return_global_ajax_value( data, 'varible_inventory', '', 'requires/woven_finish_fabric_receive_controller');
...
function openmypage_addiInfo()
{
	...
	page_link='requires/woven_finish_fabric_receive_controller.php?action=addi_info_popup&pre_addi_info='+pre_addi_info;
	...
	{
		rcv_basis_reset();
		get_php_form_data( company_id,'company_wise_load' ,'requires/woven_finish_fabric_receive_controller');

		//load_drop_down( 'requires/woven_finish_fabric_receive_controller', company_id, 'load_drop_down_supplier', 'supplier' );
		//load_drop_down( 'requires/woven_finish_fabric_receive_controller', company_id, 'load_drop_down_location', 'location_td');
		//load_drop_down( 'requires/woven_finish_fabric_receive_controller', company_id, 'load_drop_down_buyer', 'buyer_td');
		load_room_rack_self_bin('requires/woven_finish_fabric_receive_controller*3', 'store','store_td', company_id);
		//get_php_form_data( company_id, 'company_wise_report_button_setting','requires/woven_finish_fabric_receive_controller' );
		//get_php_form_data(company_id,'roll_maintained','requires/woven_finish_fabric_receive_controller' );
	...
	{
		var cbo_company_id = $("#cbo_company_id").val();
		load_room_rack_self_bin('requires/woven_finish_fabric_receive_controller*3', 'store','store_td', cbo_company_id,location);
		//load_drop_down( 'requires/woven_finish_fabric_receive_controller', cbo_company_id+'_'+location, 'load_drop_down_store', 'store_td');
	}
	function regExpValidation()
	{
		var batchNO = $("#txt_batch_lot").val();
		if (batchNO.match(/,/g)) { 
			return 1;
		}
		if (batchNO.match(/[+]/g)) { 
				...
				<fieldset style="width:880px; float:left;">
					...
					<fieldset style="width:880px;">                                       
						<table width="870" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
							...
							<tr>
								<td width="130" align="left" class="must_entry_caption">Company Name </td>
								<td width="170">
									<? 
									//echo create_drop_down( "cbo_company_id", 160, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "rcv_basis_reset();load_drop_down( 'requires/woven_finish_fabric_receive_controller', this.value, 'load_drop_down_supplier', 'supplier' );load_drop_down( 'requires/woven_finish_fabric_receive_controller', this.value, 'load_drop_down_location', 'location_td');load_drop_down( 'requires/woven_finish_fabric_receive_controller', this.value, 'load_drop_down_buyer', 'buyer_td');load_room_rack_self_bin('requires/woven_finish_fabric_receive_controller*3', 'store','store_td', this.value);get_php_form_data( this.value, 'company_wise_report_button_setting','requires/woven_finish_fabric_receive_controller' );get_php_form_data(this.value,'roll_maintained','requires/woven_finish_fabric_receive_controller' );independence_basis_controll_function(this.value);wvn_finish_fabric_auto_batch_maintain_fnc(this.value);" ); 
									echo create_drop_down( "cbo_company_id", 160, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "company_wise_load(this.value)" ); 
                                 //load_drop_down( 'requires/woven_finish_fabric_receive_controller', this.value, 'load_drop_down_store', 'store_td' );
                                	...
                                	<input type="hidden" id="style_and_po_wise_variable" name="style_and_po_wise_variable">
								</td>
								<td width="94" align="left" class="must_entry_caption"> Receive Basis </td>
								<td width="160" id="rcv_basis_td">
									<? 
								...
								<td width="80" align="left">WO / PI </td>
								<td width="140">
							...
							<tr>
								<td width="130" align="left" class="must_entry_caption">Receive Date </td>
								<td width="170">
								...
								<td width="94" align="left" class="must_entry_caption"> Challan No/Inv. </td>
								<td width="160">
									<input type="text" name="txt_challan_no" id="txt_challan_no" class="text_boxes" style="width:148px" >
								</td>
								<td width="100" align="left">Challan Date</td>
	                           	<td width="170">
									<input type="text" name="txt_challan_date" id="txt_challan_date" class="datepicker" style="width:148px;" value="" placeholder="Select Challan Date" readonly />
								</td>
							</tr>
							<tr>
								...
								<td width="130" align="left" class="must_entry_caption">Store Name</td>
								...
								<td width="94" align="left" class="must_entry_caption"> Supplier </td>
							...
							<tr>
								<td width="130" align="left"> L/C No </td>
								...
								<td width="130" align="left" class="must_entry_caption">Currency</td>
								<td width="170" id="currency"> 
									<?
									echo create_drop_down( "cbo_currency", 160, $currency,"", 1, "-- Select Currency --", '', "exchange_rate(this.value)",1 );
									?>
								</td>
								<td  width="130" align="left" class="must_entry_caption">Exchange Rate</td>
								<td width="170">
							...
							<tr>
								<td width="94" align="left" class="must_entry_caption">Source</td>
								<td width="160" id="sources">  
									<?
									echo create_drop_down( "cbo_source", 160, $source,"", 1, "-- Select --", $selected, "",1 );
									?>
								</td>
								<td width="94" align="left">Dyeing Source</td>
								<td width="160">  
									<?
							...
							<tr>								
								<td width="100" align="left">Addi. Info</td>
                                <td width="160">
	                                <input type="text" id="txt_addi_popup" name="txt_addi_popup" class="text_boxes" onDblClick="openmypage_addiInfo()"  placeholder="Double Click" style="width:150px;" readonly >

	                                <input type='hidden' id="txt_addi_info" name="txt_addi_info" value="">
	                            </td>
	                            <td align="left">BOE/Mushak Challan No</td>                                              
								<td> 
									<input type="text" name="txt_boe_mushak_challan_no" id="txt_boe_mushak_challan_no" class="text_boxes" style="width:150px">
								</td>
								<td align="left">BOE/Mushak Challan Date</td>                                              
								<td> 
									<input type="text" name="txt_boe_mushak_challan_date" id="txt_boe_mushak_challan_date" class="datepicker" style="width:150px">
								</td>
							...
							<tr>
	                            ...
	                            <td align="left">File</td> 
								   ...
								   <input type="file" class="image_uploader" id="multiple_file_field" name="multiple_file_field" multiple style="width:160px">
									...
									<input type="button" class="image_uploader" style="width:160px" value="CLICK TO ADD FILE" id="btn_fileadd"onClick="file_uploader ( '../../', document.getElementById('txt_mrr_no').value,'', 'woven_finish_fabric_receive', 2 ,1)">
								</td>
					...
					<br />
					<div id="ac_posted_msg_id" style="text-align: center;font-size:20px; color:red;"><P><b></b></p></div>
									...
									<table width="220" cellspacing="2" cellpadding="0" border="0" style="float:left;">
										...
										<tr>
											...
											<td>
												...
												<span style="width: 50px;"><input type="text" name="txt_rd_no" id="txt_rd_no" class="text_boxes" style="width:50px;" disabled /></span>
											</td>
										</tr>
										<tr>   
										...
										<tr>                 
											...
											<td>
												<?
													...
													echo create_drop_down( "cbouom", 167, $unit_of_measurement,"", 1, "-Uom-", 27, "","1","1,12,23,27","","" );
												?>
											</td>
										</tr>
										<tr>    
											...
											<td>
												<input name="txt_receive_qty" id="txt_receive_qty"  class="text_boxes_numeric" type="text" style="width:155px;" onClick="openmypage_po()" placeholder="Single Click" onBlur="fn_calile()" readonly />
									...
									<table width="250" cellspacing="2" cellpadding="0" border="0" style="float:left">
										...
										<tr>    
											...
											<td>
												...
												echo create_drop_down( "cbo_pay_over_rcv_qty", 152, $payment_yes_no,"", 0, "-- Select --", '0', "",0 );
												?>
											</td>
										</tr>
										<tr>    
					...
					<table cellpadding="0" cellspacing="1" width="100%">
						...
						<tr>
							<td align="center" colspan="6" valign="middle" class="button_container">
								...
								<input type="button" id="show_button3" class="formbutton" style="width: 80px;display:none;" value="Print 3" onClick="fn_report_generated(3)" />
								<input type="button" id="show_button4" class="formbutton" style="width: 80px;display:none;" value="Print 4" onClick="fn_report_generated(4)" />
								<input type="button" id="show_button5" class="formbutton" style="width: 80px;display:none;" value="Print 5" onClick="fn_report_generated(5)" />
							</td>
						</tr> 
					</table>
