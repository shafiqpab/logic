...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Finish Fabric Order To Sample Roll Transfer Info","../../", 1, 1, '','',''); 
...
function openmypage_systemId()
{
	...
	var page_link = 'requires/finish_fabric_order_to_sample_roll_transfer_controller.php?cbo_company_id='+cbo_company_id+'&action=orderToorderTransfer_popup';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(transfer_id, "populate_data_from_transfer_master", "requires/finish_fabric_order_to_sample_roll_transfer_controller" );
		show_list_view(transfer_id+"**"+$('#txt_from_order_id').val(),'show_transfer_listview','tbl_details','requires/finish_fabric_order_to_sample_roll_transfer_controller','');
	}
}
...
function openmypage_orderNo(type)
{
	...
	var page_link = 'requires/finish_fabric_order_to_sample_roll_transfer_controller.php?cbo_company_id='+cbo_company_id+'&type='+type+'&action=order_popup';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(order_id+"**"+type, "populate_data_from_order", "requires/finish_fabric_order_to_sample_roll_transfer_controller" );
		if(type=='from')
		{
			show_list_view(order_id,'show_dtls_list_view','tbl_details','requires/finish_fabric_order_to_sample_roll_transfer_controller','');
		}
	}
}
...
function fn_open_semple()
{
	...
	var page_link = 'requires/finish_fabric_order_to_sample_roll_transfer_controller.php?cbo_company_id='+cbo_company_id+'&action=sample_popup';
	...
	emailwindow.onclose=function()
	{
		...
		var sample_id=this.contentDoc.getElementById("sample_id").value; //Access form field with id="emailfield"
 		get_php_form_data(sample_id, "populate_data_from_sample", "requires/finish_fabric_order_to_sample_roll_transfer_controller" );
	}
}
...
function fnc_finish_transfer_entry(operation)
{
	if(operation==4)
	{
		...
		print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title, "finish_fabric_order_to_order_transfer_print", "requires/finish_fabric_order_to_sample_roll_transfer_controller" ) 
		return;
	}
	else if(operation==0 || operation==1 || operation==2)
	{
		...
		var dataString = "txt_system_id*cbo_company_id*txt_transfer_date*txt_challan_no*txt_from_order_id*txt_sam_book_id*update_id";
		...
		http.open("POST","requires/finish_fabric_order_to_sample_roll_transfer_controller.php",true);
...
{	
	...
	{	  		
		...
		if(reponse[0]==0 || reponse[0]==1)
		{
			...
			//show_list_view(reponse[1],'show_transfer_listview','div_transfer_item_list','requires/finish_fabric_order_to_sample_roll_transfer_controller','');
			show_list_view(reponse[1]+"**"+$('#txt_from_order_id').val(),'show_transfer_listview','tbl_details','requires/finish_fabric_order_to_sample_roll_transfer_controller','');
...
function openmypage_orderInfo(type)
{
	...
	var title = 'Order Info';	
	var page_link = 'requires/finish_fabric_order_to_sample_roll_transfer_controller.php?txt_order_no='+txt_order_no+'&txt_order_id='+txt_order_id+'&type='+type+'&action=orderInfo_popup';
...
<div align="center" style="width:100%;">
    ...
    <form name="transferEntry_1" id="transferEntry_1" autocomplete="off" >
        ...
        <legend>Roll Wise Finish Fabric Order To Sample Transfer Entry</legend>
        <br>
