...
<script>
	...
	function fnc_transfer_acknowledgement( operation )
	{
		...
		if(operation==0 || operation==1)
		{
			...
			http.open("POST","requires/finish_fabric_transfer_acknowledge_controller.php",true);
	...
	function fnc_transfer_acknowledgement_response()
	{
		if(http.readyState == 4)
		{
			...
			if(response[0]==0 || response[0]==1)
			{
				...
				show_list_view(response[1],'show_dtls_list_view_update','list_container','requires/finish_fabric_transfer_acknowledge_controller','');
				show_list_view(response[2]+"**"+1,'show_dtls_list_view','transfer_item','requires/finish_fabric_transfer_acknowledge_controller','');
	...
	function openmypage_challan()
	{
		...
		var page_link= 'requires/finish_fabric_transfer_acknowledge_controller.php?action=itemTransfer_popup&company='+company+'&location='+location+'&store='+store;
		...
		emailwindow.onclose=function()
		{
			...
			if(transfer_id!='')
			{
				...
				get_php_form_data(transfer_id, "populate_data_from_transfer_master", "requires/finish_fabric_transfer_acknowledge_controller" );
				show_list_view(transfer_id+"**"+0,'show_dtls_list_view','transfer_item','requires/finish_fabric_transfer_acknowledge_controller','');
			}
		}
	}
	...
	function openmypage_systemId()
	{
		...
		var page_link = 'requires/finish_fabric_transfer_acknowledge_controller.php?cbo_company_id_to='+cbo_company_id_to+'&action=itemAcknowle_popup';
		...
		get_php_form_data(transfer_id[0], "populate_data_from_transfer_master_update", "requires/finish_fabric_transfer_acknowledge_controller" );
		//show_list_view(transfer_id,'show_dtls_list_view_update','payDtlsTableTbody','requires/finish_fabric_transfer_acknowledge_controller','');
		show_list_view(transfer_id[0],'show_dtls_list_view_update','list_container','requires/finish_fabric_transfer_acknowledge_controller','');
		show_list_view(transfer_id[1]+"**"+1,'show_dtls_list_view','transfer_item','requires/finish_fabric_transfer_acknowledge_controller','');
		set_button_status(0, permission, 'fnc_transfer_acknowledgement',1,1);
	}
}
...
function openmypage_qnty()
{
	...
	var page_link = 'requires/finish_fabric_transfer_acknowledge_controller.php?cbo_company_id_to='+cbo_company_id_to+'&store='+store+'&location='+location+'&save_string='+save_string+'&hidden_rcv_qnty='+hidden_rcv_qnty+'&action=quantity_popup';
...
function fnc_floor_load(datas){
	...
	load_drop_down( 'requires/finish_fabric_transfer_acknowledge_controller',storeId+'_'+company+'_'+location, 'load_drop_down_floor_to', floor_tds );
}
				...
				<form name="transAcknowledgement" id="transAcknowledgement" autocomplete="off" method="POST"  >
					...
					<table width="880" cellspacing="2" cellpadding="2" border="0" id="tbl_master">
						...
						<tr>
							...
							<td>
								<?
								echo create_drop_down( "cbo_company_id_to", 160, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name","id,company_name", 1, "--Select Company--", 0, "resetFnc();load_drop_down( 'requires/finish_fabric_transfer_acknowledge_controller',this.value, 'load_drop_down_location_to', 'to_location_td' );" );
								?>
