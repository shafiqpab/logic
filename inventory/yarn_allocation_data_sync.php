...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Yarn allocation data synchronization","../", 1, 1, $unicode);
?>
<style type="text/css">
...
<script>
	...
	{
		if( form_validation('cbo_company_id*txt_product_id','Company Name*Product Id') == false )
		{
			return;
		}
		
		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*txt_product_id',"../");	
		freeze_window(3);
		http.open("POST","requires/yarn_allocation_data_sync_controller.php",true);
	...
	function synchronize_allocation()
	{
		if(confirm("Are you sure?")){
			...
			var data="action=synchronize_allocation&product_id="+product_id+"&company_id="+company_id;
			freeze_window(3);
			http.open("POST","requires/yarn_allocation_data_sync_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
	...
	<div style="width:100%;" align="left">
		...
		<form name="storeItemInquiry_1" id="storeItemInquiry_1" autocomplete="off" > 
			<div style="width:100%;" align="center">
				<h3 style="width:430px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3> 
				<div style="width:100%;" id="content_search_panel">
					<fieldset style="width:430px;">
						<table class="rpt_table" width="430" cellpadding="0" cellspacing="0" border="1" rules="all">
							<thead>
								<tr> 	 	
