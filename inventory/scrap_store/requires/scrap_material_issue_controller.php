...
if($action === "load_drop_down_location")
{
    ...
    echo create_drop_down( "cbo_location", 160, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id='$ex_data[0]' $location_credential_cond order by location_name","id,location_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/scrap_material_issue_controller', $ex_data[0]+'_'+this.value+'_'+$ex_data[1], 'load_drop_down_store', 'store_td');",0 );
	exit();
}
...
if ($action=="itemDescription_popup")
{
	...
	$sql_issue = "select b.PROD_ID, sum(b.sales_qty) as ISSUE_QTY from inv_scrap_sales_mst a, inv_scrap_sales_dtls b where a.id=b.mst_id and a.company_id=$cbo_company_id and a.item_category=$cbo_category_id $store_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.prod_id";
	...
	$sql = "SELECT b.PRODUCT_ID, b.ITEM_GROUP_ID, b.UOM, b.COLOR, max(b.lot) as LOT, b.GSM, c.PRODUCT_NAME_DETAILS, sum(b.receive_qnty) as RECEIVE_QNTY
	FROM inv_scrap_receive_mst a, inv_scrap_receive_dtls b, product_details_master c
	WHERE a.id=b.mst_id and b.product_id=c.id and a.company_id=$cbo_company_id and a.item_category_id=$cbo_category_id $store_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 
...
{
	//echo 'system';die;
	list($company_id, $item_category_id, $prod_ids, $purpose,$cbo_store_id) = explode("**", $data);
	//echo '<pre>';print_r($data);die;
	//$color_arr = return_library_array("select id, color_name from lib_color","id","color_name");
	$item_group_arr=return_library_array("select id, item_name from lib_item_group",'id','item_name');
	$sql_issue = "select b.PROD_ID, sum(b.sales_qty) as ISSUE_QTY from inv_scrap_sales_mst a, inv_scrap_sales_dtls b where a.id=b.mst_id and a.company_id=$company_id and a.item_category=$item_category_id and b.prod_id in($prod_ids) and a.store_id=$cbo_store_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.prod_id";
	$sql_issue_res = sql_select($sql_issue);
	$scrap_issue_qty_arr=array();
	foreach ($sql_issue_res as $row)
	{
	...
	FROM inv_scrap_receive_mst a, inv_scrap_receive_dtls b, product_details_master c
	WHERE a.id=b.mst_id and b.product_id=c.id and a.company_id=$company_id and a.item_category_id=$item_category_id and b.PRODUCT_ID in($prod_ids) and a.store_id=$cbo_store_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
	GROUP BY b.product_id, b.item_group_id, b.uom, b.color, b.gsm, c.product_name_details";
	$sql_res=sql_select($sql);
	$i=1;
	...
	foreach ($sql_res as $row) 
	{
		...
		<tr bgcolor="<?= $bgcolor; ?>" id="row_<?= $i; ?>">
            ...
            <td>            	
            	<input type="text" name="txt_itemgroup[]" id="txt_itemgroup_<?= $i; ?>" class="text_boxes" style="width:100px" value="<?= $item_group_arr[$row['ITEM_GROUP_ID']]; ?>" disabled="disabled" />
            	<input type="hidden" name="itemgroup[]" id="itemgroup_<?= $i; ?>" class="text_boxes"  value="<?= $row['ITEM_GROUP_ID']; ?>" disabled="disabled" />
            </td>
            ...
            <td>
            	<input type="text" name="txt_stock[]" id="txt_stock_<?= $i; ?>" class="text_boxes_numeric" style="width:80px" value="<?= number_format($stock_qty,2,'.',''); ?>" disabled="disabled" />
            </td>

            <td>
            	<input type="text" name="txt_salesqty[]" id="txt_salesqty_<?= $i; ?>" class="text_boxes_numeric" style="width:80px" value="<?= number_format($stock_qty,2,'.',''); ?>" onkeyup="amount_calculation(<?= $i; ?>,<?= $purpose; ?>);"/>
...
if ($action=="save_update_delete")
{
	...
	$sql_customer_list=sql_select($sql_customer);
	$txt_customer_id=str_replace("'","",$txt_customer_id);
	 
	foreach($sql_customer_list as $val)
	{
		
		echo $val['BUYER_NAME']."<br>";
			if($val['BUYER_NAME']==$txt_customer_id){
				$txt_customer_id=$val['ID'];echo "yes";
			}
	...
	if ($operation==0)  // Insert Here
	{
		...
		if(str_replace("'","",$update_id)=='')
		{
			...
			$new_system_id=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '', 'SSE', date("Y",time()), 5, "select challan_no_prefix, challan_no_prefix_num from inv_scrap_sales_mst where company_id=$cbo_company_id and status_active=1 and is_deleted=0 and $year_cond=".date('Y',time())." order by id desc ", "challan_no_prefix", "challan_no_prefix_num"));
...
if ($action=="populate_data_from_dtls")
{
	...
	foreach ($nameArray as $row)
	{
		...
		$stock_qty = number_format($scrap_receive_qty_arr[$row['PROD_ID']]-$scrap_iss_qty_arr[$row['PROD_ID']],2,'.','');
		$txt_salesqty=number_format($row["ISSUE_QTY"],2,'.','');
	    ...
	    echo "document.getElementById('txt_itemgroup_1').value            = '".$item_group_arr[$row["ITEM_GROUP_ID"]]."';\n";
	    echo "document.getElementById('itemgroup_1').value            = '".$row["ITEM_GROUP_ID"]."';\n";
	    ...
	    echo "document.getElementById('txt_salesqty_1').value            = '".$txt_salesqty."';\n";
...
if($action=="scrap_material_challan_print") // Print
{	
	list($company_id,$update_id,$location_id,$show_item,$report_title)=explode('**',$data);
	...
	?>
    <table cellspacing="0" width="<?echo $width ?>"  border="1" rules="all" class="rpt_table" align="left" style="margin-top: 10px;">
		...
		foreach($sql_dtls_res as $row)
		{
            ...
            <tr bgcolor="<?= $bgcolor; ?>">
				...
				<td width="80" align="right"><p>	<?
					echo number_format($row['ISSUE_RATE'],4,'.',''); ?> &nbsp;</p></td>
					<?
				  }
				 ?>
                <? 
	...
	<br>
	   <?
		  echo signature_table(204, $company_id, "1160px");
...
if($action=="scrap_material_challan_print2") // Print 2
{
    ...
    <table width="930" cellspacing="0" align="right">
        ...
        <tr>
            <!-- <td colspan="6" align="center" style="font-size:20px"><u><strong><?= $report_title; ?></strong></u></td> -->
			<td colspan="6" align="center" style="font-size:20px"><u><strong>Scrap Material Delivery/Bill</strong></u></td>
        </tr>
        <tr>
			<?
                $buyer_add = $dataArray[0]['CUSTOMER_ID'];
            ...
            <td width="125"><strong>Item Category :</strong></td>
            <td width="175px"><?= $category_arr[$dataArray[0]['ITEM_CATEGORY']]; ?></td>
            <td width="125"><strong>Gate Pass No:</strong></td><!-- Challan No : -->
            <td width="175px"></td>
        </tr>
        <tr>
            <td width="125"><strong>Issue No:</strong></td>
            <td width="175px"><?= $dataArray[0]['SYS_CHALLAN_NO']; ?></td>
        	<td colspan="2"></td>
        	<td width="125"><strong>Issue Purpose :</strong></td>
        	<td width="175px"><?= $purpose_array[$dataArray[0]['PURPOSE']]; ?></td>
        ...
        <tr>
            ...
            <td width="175px"><?= $buyer_library[$buyer_add]; ?></td>
            <td colspan="2"></td>
        	<td width="125"><strong>Manual Challan No:</strong></td>
            <td width="175px"></td>
        </tr>
        <tr>
            ...
            <td colspan="2"></td>
        	<td width="125"><strong>Bar Code :</strong></td>
        	 <!-- <td colspan="5"><?= $dataArray[0]['Bar Code']; ?></td>  -->
        </tr>     
        <tr>
         	<td width="125"><strong>Pay Term :</strong></td>
        	<td width="175px"><?= $pay_mode[$dataArray[0]['PAY_TERM']]; ?></td>
    ...
    <table cellspacing="0" width="930"  border="1" rules="all" class="rpt_table" align="left" style="margin-top: 10px;">
		...
		?>
            ...
            <tr bgcolor="#dddddd">
				<td align="left" colspan="10">&nbsp;<strong>In Word: </strong><?= number_to_words(number_format($tot_amount,0,'',',')); ?></td>
			</tr>
        
    </table>
    </div>
    <div>
    	<table width="730" cellspacing="0" align="left" style="padding-top: 40px;">
	        <tr>
	            <td width="125"><strong>Transport No:</strong></td>
	            <td width="120px"></td>
	        	<td width="125"><strong>Driver Name:</strong></td>
	            <td width="120px"></td>
	        	<td width="125"><strong>D/L No:</strong></td>
	        	<td width="120px"></td>
	        	<td width="125"><strong>Mobile No:</strong></td>
	        	<td width="120px"></td>
	        </tr>
	    </table>
    </div>
    <?
    echo signature_table(204, $company_id, "930px");
	exit();
}
...
{
	extract($_REQUEST);
	$print_report_format=return_field_value("format_id","lib_report_template","template_name ='".$data."'  and module_id=6 and report_id=119 and is_deleted=0 and status_active=1");
	...
	if($print_report_format != "")
	{
		...
		{
			if($id==115){echo "$('#search1').show();\n";}
			if($id==116){echo "$('#search2').show();\n";}
		}
	}
	else
	{
