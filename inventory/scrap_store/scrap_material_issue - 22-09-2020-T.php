...
<script>
	...
	function openmypage_ItemDescription()
	{
		...
		emailwindow.onclose=function()
		{
			...
			if(prod_ids != '')
			{
				var all_data = cbo_company_id+'**'+cbo_category_id+'**'+prod_ids+'**'+cbo_purpose;				
	...
	function fnc_material_issue(operation)
	{
		if(operation==4)
		{
			...
			print_report(company_id+'**'+update_id+'**'+location_id+'**'+report_title, "scrap_material_challan_print", "requires/scrap_material_issue_controller");
			return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			{				
				if (purpose==1){
					if (form_validation('txt_prodid_'+i+'*txt_salesqty_'+i+'*txt_salesrate_'+i,'Browse Product Id*Issue Qty*Issue Rate')==false)
					{
						return;
					}
				} else {
				...
				dtls_data += "*txt_prodid_"+i+"*dtls_update_id_"+i+"*txt_itemgroup_"+i+"*txt_itemdes_"+i+"*cbo_rejuomid_"+i+"*txt_salesqty_"+i+"*txt_salesrate_"+i+"*txt_salesamount_"+i+"*txt_bag_"+i+"*txt_remarks_dtls_"+i;
			}
        	...
        	<fieldset style="width:850px;">
                <table width="850" cellspacing="2" cellpadding="0" border="0" id="tbl_master" align="center">
                    ...
                    <tr>
                        ...
                        <td>
                            <? 
                                echo create_drop_down( "cbo_company_id", 160, "select id,company_name from lib_company  where status_active=1 and is_deleted=0 $company_cond $company_credential_cond order by company_name","id,company_name", 1, "-- Select Company --", 0, "fnc_load_location(this.value);" );
                            ?>
                        </td>
                    ...
                    <tr>
                    	<td>Customer</td>
                        <td>
                            <input name="txt_customer_no" id="txt_customer_no" class="text_boxes" style="width:150px"  placeholder="Browse Customer" onDblClick="openpage_customer();"/>
            ...
            <table width="870" cellspacing="1" cellpadding="1" border="1" rules="all" class="rpt_table">
                ...
                <tr>
                    <td align="center" colspan="10" class="button_container" width="100%">
                        <?
                            echo load_submit_buttons($permission, "fnc_material_issue", 0,1,"reset_form('materialIssue_1','div_details_list_view','','','')",1);
                        ?>
                    </td>
                </tr>
			</table>
