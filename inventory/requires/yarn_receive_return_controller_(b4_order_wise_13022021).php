...
{
	// print_r($data);
	load_room_rack_self_bin("requires/yarn_receive_return_controller",$data);
}*/
...
if($action=="populate_data_from_data")
{
	...
	if($mrr_type==1)
	{
		$sql = "select id,recv_number,entry_form,receive_basis,receive_date,challan_no,booking_id,booking_no,issue_id from inv_receive_master where id=$mst_id and entry_form in(1,9)";
	}
	else
	{
...
{
	$sql = "SELECT b.id as prod_id, b.product_name_details, a.no_of_bags, a.cone_per_bag, a.store_id, a.floor_id, a.room, a.rack, a.self, a.bin_box, a.order_rate, a.order_ile_cost, a.cons_uom, a.cons_rate, a.cons_quantity, a.cons_amount, a.balance_qnty, a.balance_amount, a.mst_id as receive_id, a.transaction_type, a.issue_id, a.company_id
...
if($action=="save_update_delete")
{
	...
	if($pi_id!="" && $pi_id>0)
	{
		//and a.prod_id=$txt_prod_id and b.prod_id=$txt_prod_id and c.prod_id=$txt_prod_id and d.prod_id=$txt_prod_id 
		/* no need issue rtn pi value
		$yarn_issue_return_sql ="select a.id, a.cons_quantity  from inv_transaction a, inv_transaction b, inv_mrr_wise_issue_details c, inv_transaction d
		where a.item_category=1 and b.item_category=1 and d.item_category=1 and a.transaction_type=4 and b.transaction_type=2 and d.transaction_type=1 and a.issue_id=b.mst_id  and b.id=c.issue_trans_id and c.recv_trans_id=d.id and d.pi_wo_batch_no=$pi_id ";
		$yarn_issue_return_result = sql_select( $yarn_issue_return_sql);
		$issue_rtn_pi_value=0;
		foreach($yarn_issue_return_result as $row)
		{
			if($id_check[$row[csf("id")]]=="")
			{
				$id_check[$row[csf("id")]]=$row[csf("id")];
				$issue_rtn_pi_value+=$row[csf("cons_quantity")];
			}
		}*/
		...
		/*
		if($accept_value>0){
			$cumu_accept_value = ($pi_value-$accept_value);	
		}else{
			$cumu_accept_value=0;
		}*/
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		if($rec_entry_form == 9){
			...
			/*$issue_receive_ids=sql_select("select c.receive_date, c.exchange_rate from inv_transaction b,inv_mrr_wise_issue_details a left join inv_transaction d on a.recv_trans_id=d.id and d.status_active=1 left join inv_receive_master c on d.mst_id=c.id and c.entry_form=1 and c.status_active=1 where b.id=a.issue_trans_id and b.mst_id in($issue_ids) and a.status_active=1 and b.status_active=1 and b.transaction_type=2");*/
			...
			//if($presentAvgRate > 0 && $exchange_rate > 0) $order_rate = number_format($presentAvgRate/$exchange_rate,2,".",""); else $order_rate =0;
			//$order_rate = number_format($presentAvgRate/$exchange_rate,2,".","");
		}
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		if($rec_entry_form == 9){
			...
			$order_rate = number_format($curr_avg_rate/$exchange_rate,2,".","");*/
			$issue_receive_ids=sql_select("select d.order_rate 
			from inv_transaction b, inv_mrr_wise_issue_details a left join inv_transaction d on a.recv_trans_id=d.id and d.status_active=1 and d.transaction_type=1 
			where b.id=a.issue_trans_id and b.mst_id in($issue_ids) and a.status_active=1 and b.status_active=1 and b.transaction_type=2");
			$order_rate =0;
		...
		<tbody>
			...
			foreach($result as $row){
				...
				<tr bgcolor="<? echo $bgcolor; ?>" onClick='get_php_form_data("<? echo $row[csf("id")]."_";?>,<? echo $rcvQnty;?>","child_form_input_data","requires/yarn_receive_return_controller");hidden_ref_closing(<?php echo $row[csf("cons_quantity")]; ?>);' style="cursor:pointer" >
					...
					<td width="70" align="right"><p><?php echo $row[csf("no_of_bags")]; ?></p></td>
					<td width="70" align="right"><p><?php echo $row[csf("cone_per_bag")]; ?></p></td>
...
if($action=="child_form_input_data")
{
	...
	$sql = "select b.id as prod_id, b.product_name_details, a.id as tr_id, a.no_of_bags, a.cone_per_bag, a.company_id, a.store_id, a.floor_id, a.room, a.rack, a.self, a.bin_box, a.order_rate, a.order_ile_cost, a.cons_uom, a.rcv_rate as cons_rate, a.cons_quantity, a.rcv_amount as cons_amount
	from inv_transaction a, product_details_master b
...
if ($action=="yarn_receive_return_print")
{
	...
	//if($up_id!="") $cond .= " and a.id='$up_id'";
				$i=1;
				$sql_dtls = "select b.id as prod_id, b.product_name_details, a.id as tr_id, a.no_of_bags, a.cone_per_bag, a.store_id, a.cons_uom, a.cons_quantity, round(to_number(a.rcv_rate), 4) as cons_rate,a.rcv_amount as cons_amount
...
if ($action=="yarn_receive_return_print_2")
{
	...
	<div style="width:1030px;">
		<table width="1010" cellspacing="0" align="right">
			<tr>
				<td colspan="6" align="center" style="font-size:xx-large"><strong><? echo $company_library[$data[0]]; ?></strong></td>
			</tr>
			<tr class="form_caption">
    ...
    //if($up_id!="") $cond .= " and a.id='$up_id'";
				$i = 1;
				$sql_dtls = "select b.id as prod_id, b.product_name_details, a.id as tr_id, a.no_of_bags, a.cone_per_bag, a.store_id, a.cons_uom, a.cons_quantity, round(to_number(a.rcv_rate), 4) as cons_rate,a.rcv_amount as cons_amount,b.lot, b.yarn_count_id
				from inv_transaction a, product_details_master b, inv_issue_master c
				where c.id=a.mst_id and a.status_active=1 and a.company_id='$data[0]' and c.issue_number='$data[1]' and a.item_category=1 and transaction_type=3 and a.prod_id=b.id and b.status_active=1 ";
				$sql_result = sql_select($sql_dtls);
				...
				<tfoot>
					<tr>
						<th colspan="10" style="text-align: left;">In Word : <?php  echo number_to_words(number_format($qnty, 2)); ?></th>
					</tr>
				</tfoot>
			</table>
			<br>
	...
	<div style="width:1030px;">
		<table cellspacing="0" border="0" width="100%">
			<tr>
				...
				<td colspan="10" style="font-size:x-large" align="center"><strong><u>Purchase Return/Delivery Challan</u></strong></td>
			</tr>
			<tr>
