...
if($action=="load_drop_down_year")
{
	...
	echo create_drop_down( "cbo_year", 150, $sql,"id,year_start",1, "-- Select --", 0, "load_drop_down( 'requires/store_item_closing_controller',this.value, 'load_drop_down_month', 'month_td' );" );
	exit(); 
}
...
if($action=="load_drop_down_month")
{
	...
	//echo $sql;die;
	echo create_drop_down( "cbo_month", 150, $sql,"month_id,financial_period",1, "-- Select --", 0, "" );
	exit(); 
}
...
if ($action=="save_update_delete_process")
{
	...
	$sql_month="select month_id, financial_period as financial_period, period_ending_date from lib_ac_period_dtls where status_active=1 and is_deleted=0 and period_locked=0 and mst_id=$cbo_year and financial_period not in('Opening','Closing','Post Closing') $month_cond order by month_id";
	//echo $sql_month;die;
	...
	foreach($sql_month_result as $row)
	{
		...
		{
			$year_month=date("Y-m",strtotime($row[csf("period_ending_date")]));
			$priod_year=date("Y",strtotime($row[csf("period_ending_date")]));
			$priod_month=date("m",strtotime($row[csf("period_ending_date")]));
		}
		...
		$lib_year_month_arr[date("Y-m",strtotime($row[csf("period_ending_date")]))]=date("Y-m",strtotime($row[csf("period_ending_date")]));
		$lib_month_colum_arr["period_".$row[csf("month_id")]]=date("Y-m",strtotime($row[csf("period_ending_date")]));
		if($first_month=="") $first_month=date("Y-m",strtotime($row[csf("period_ending_date")]));
		$i++;
	}
	...
	$prev_close_item="select id as ID, prod_id as PROD_ID, closing as CLOSING, last_rate as LAST_RATE, closing_receive as CLOSING_RECEIVE, closing_issue as CLOSING_ISSUE, closing_receive_rtn as CLOSING_RECEIVE_RTN, closing_issue_rtn as CLOSING_ISSUE_RTN, closing_transfer_in as CLOSING_TRANSFER_IN, closing_transfer_out as CLOSING_TRANSFER_OUT from year_close_item where company_id=$cbo_company_id and year_id=$cbo_year";
	...
	$prev_close_item_ref="select id as ID, prod_id as PROD_ID, ref_type as REF_TYPE, ref_id as REF_ID, closing as CLOSING, last_rate as LAST_RATE, closing_receive as CLOSING_RECEIVE, closing_issue as CLOSING_ISSUE, closing_receive_rtn as CLOSING_RECEIVE_RTN, closing_issue_rtn as CLOSING_ISSUE_RTN, closing_transfer_in as CLOSING_TRANSFER_IN, closing_transfer_out as CLOSING_TRANSFER_OUT from year_close_item_ref where company_id=$cbo_company_id and year_id=$cbo_year";
	...
	$all_item_sql="select id as ID from product_details_master where status_active=1 and is_deleted=0 and company_id=$cbo_company_id";
	...
	$sql_transac="select id as ID, company_id as COMPANY_ID, prod_id as PROD_ID, item_category as ITEM_CATEGORY, store_id as STORE_ID, floor_id as FLOOR_ID, room as ROOM, rack3 as RACK, self as SELF, bin_box as BIN_BOX, transaction_type as TRANSACTION_TYPE, cons_quantity as CONS_QUANTITY, cons_amount as CONS_AMOUNT, insert_date as INSERT_DATE from inv_transaction where status_active=1 and is_deleted=0 and company_id=$cbo_company_id $year_cond order by PROD_ID";
	...
	{
		$transaction_date=date("Y-m",strtotime($row["INSERT_DATE"]));
		...
		if($row["TRANSACTION_TYPE"] == 1 || $row["TRANSACTION_TYPE"] == 4 || $row["TRANSACTION_TYPE"] == 5)
		{
			...
			{
				$data_pord[$row["PROD_ID"]][$lib_year_month_arr[$transaction_date]]["QNT"]+=$row["CONS_QUANTITY"];
				$data_pord[$row["PROD_ID"]][$lib_year_month_arr[$transaction_date]]["AMT"]+=$row["CONS_AMOUNT"];
				if($row["TRANSACTION_TYPE"] == 1) $data_pord[$row["PROD_ID"]][$lib_year_month_arr[$transaction_date]]["RECEIVE"]+=$row["CONS_QUANTITY"];
				if($row["TRANSACTION_TYPE"] == 4) $data_pord[$row["PROD_ID"]][$lib_year_month_arr[$transaction_date]]["ISSUE_RTN"]+=$row["CONS_QUANTITY"];
				if($row["TRANSACTION_TYPE"] == 5) $data_pord[$row["PROD_ID"]][$lib_year_month_arr[$transaction_date]]["TRANSFER_IN"]+=$row["CONS_QUANTITY"];
				
				$data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$lib_year_month_arr[$transaction_date]]["QNT"]+=$row["CONS_QUANTITY"];
				$data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$lib_year_month_arr[$transaction_date]]["AMT"]+=$row["CONS_AMOUNT"];
				if($row["TRANSACTION_TYPE"] == 1) $data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$lib_year_month_arr[$transaction_date]]["RECEIVE"]+=$row["CONS_QUANTITY"];
				if($row["TRANSACTION_TYPE"] == 4) $data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$lib_year_month_arr[$transaction_date]]["ISSUE_RTN"]+=$row["CONS_QUANTITY"];
				if($row["TRANSACTION_TYPE"] == 5) $data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$lib_year_month_arr[$transaction_date]]["TRANSFER_IN"]+=$row["CONS_QUANTITY"];
				
				if($row["FLOOR_ID"])
				{
					$data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$lib_year_month_arr[$transaction_date]]["QNT"]+=$row["CONS_QUANTITY"];
					$data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$lib_year_month_arr[$transaction_date]]["AMT"]+=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 1) $data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$lib_year_month_arr[$transaction_date]]["RECEIVE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 4) $data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$lib_year_month_arr[$transaction_date]]["ISSUE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 5) $data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$lib_year_month_arr[$transaction_date]]["TRANSFER_IN"]+=$row["CONS_QUANTITY"];
				}
				
				if($row["ROOM"])
				{
					$data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$lib_year_month_arr[$transaction_date]]["QNT"]+=$row["CONS_QUANTITY"];
					$data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$lib_year_month_arr[$transaction_date]]["AMT"]+=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 1) $data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$lib_year_month_arr[$transaction_date]]["RECEIVE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 4) $data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$lib_year_month_arr[$transaction_date]]["ISSUE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 5) $data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$lib_year_month_arr[$transaction_date]]["TRANSFER_IN"]+=$row["CONS_QUANTITY"];
				}
				
				if($row["RACK"])
				{
					$data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$lib_year_month_arr[$transaction_date]]["QNT"]+=$row["CONS_QUANTITY"];
					$data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$lib_year_month_arr[$transaction_date]]["AMT"]+=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 1) $data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$lib_year_month_arr[$transaction_date]]["RECEIVE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 4) $data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$lib_year_month_arr[$transaction_date]]["ISSUE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 5) $data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$lib_year_month_arr[$transaction_date]]["TRANSFER_IN"]+=$row["CONS_QUANTITY"];
				}
				
				if($row["SELF"])
				{
					$data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$lib_year_month_arr[$transaction_date]]["QNT"]+=$row["CONS_QUANTITY"];
					$data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$lib_year_month_arr[$transaction_date]]["AMT"]+=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 1) $data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$lib_year_month_arr[$transaction_date]]["RECEIVE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 4) $data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$lib_year_month_arr[$transaction_date]]["ISSUE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 5) $data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$lib_year_month_arr[$transaction_date]]["TRANSFER_IN"]+=$row["CONS_QUANTITY"];
				}
				
				if($row["BIN_BOX"])
				{
					$data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$lib_year_month_arr[$transaction_date]]["QNT"]+=$row["CONS_QUANTITY"];
					$data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$lib_year_month_arr[$transaction_date]]["AMT"]+=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 1) $data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$lib_year_month_arr[$transaction_date]]["RECEIVE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 4) $data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$lib_year_month_arr[$transaction_date]]["ISSUE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 5) $data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$lib_year_month_arr[$transaction_date]]["TRANSFER_IN"]+=$row["CONS_QUANTITY"];
				}
			}
			else
			{
				$data_pord[$row["PROD_ID"]][$first_month]["QNT"]+=$row["CONS_QUANTITY"];
				$data_pord[$row["PROD_ID"]][$first_month]["AMT"]+=$row["CONS_AMOUNT"];
				if($row["TRANSACTION_TYPE"] == 1) $data_pord[$row["PROD_ID"]][$first_month]["RECEIVE"]+=$row["CONS_QUANTITY"];
				if($row["TRANSACTION_TYPE"] == 4) $data_pord[$row["PROD_ID"]][$first_month]["ISSUE_RTN"]+=$row["CONS_QUANTITY"];
				if($row["TRANSACTION_TYPE"] == 5) $data_pord[$row["PROD_ID"]][$first_month]["TRANSFER_IN"]+=$row["CONS_QUANTITY"];
				
				$data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$first_month]["QNT"]+=$row["CONS_QUANTITY"];
				$data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$first_month]["AMT"]+=$row["CONS_AMOUNT"];
				if($row["TRANSACTION_TYPE"] == 1) $data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$first_month]["RECEIVE"]+=$row["CONS_QUANTITY"];
				if($row["TRANSACTION_TYPE"] == 4) $data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$first_month]["ISSUE_RTN"]+=$row["CONS_QUANTITY"];
				if($row["TRANSACTION_TYPE"] == 5) $data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$first_month]["TRANSFER_IN"]+=$row["CONS_QUANTITY"];
				
				if($row["FLOOR_ID"])
				{
					$data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$first_month]["QNT"]+=$row["CONS_QUANTITY"];
					$data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$first_month]["AMT"]+=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 1) $data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$first_month]["RECEIVE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 4) $data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$first_month]["ISSUE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 5) $data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$first_month]["TRANSFER_IN"]+=$row["CONS_QUANTITY"];
				}
				
				if($row["ROOM"])
				{
					$data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$first_month]["QNT"]+=$row["CONS_QUANTITY"];
					$data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$first_month]["AMT"]+=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 1) $data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$first_month]["RECEIVE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 4) $data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$first_month]["ISSUE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 5) $data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$first_month]["TRANSFER_IN"]+=$row["CONS_QUANTITY"];
				}
				
				if($row["RACK"])
				{
					$data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$first_month]["QNT"]+=$row["CONS_QUANTITY"];
					$data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$first_month]["AMT"]+=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 1) $data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$first_month]["RECEIVE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 4) $data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$first_month]["ISSUE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 5) $data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$first_month]["TRANSFER_IN"]+=$row["CONS_QUANTITY"];
				}
				
				if($row["SELF"])
				{
					$data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$first_month]["QNT"]+=$row["CONS_QUANTITY"];
					$data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$first_month]["AMT"]+=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 1) $data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$first_month]["RECEIVE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 4) $data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$first_month]["ISSUE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 5) $data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$first_month]["TRANSFER_IN"]+=$row["CONS_QUANTITY"];
				}
				
				if($row["BIN_BOX"])
				{
					$data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$first_month]["QNT"]+=$row["CONS_QUANTITY"];
					$data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$first_month]["AMT"]+=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 1) $data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$first_month]["RECEIVE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 4) $data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$first_month]["ISSUE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 5) $data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$first_month]["TRANSFER_IN"]+=$row["CONS_QUANTITY"];
				}
			}
		}
		else
		{
			if($lib_year_month_arr[$transaction_date]!="")
			{
				$data_pord[$row["PROD_ID"]][$lib_year_month_arr[$transaction_date]]["QNT"]-=$row["CONS_QUANTITY"];
				$data_pord[$row["PROD_ID"]][$lib_year_month_arr[$transaction_date]]["AMT"]-=$row["CONS_AMOUNT"];				
				if($row["TRANSACTION_TYPE"] == 2) $data_pord[$row["PROD_ID"]][$lib_year_month_arr[$transaction_date]]["ISSUE"]+=$row["CONS_QUANTITY"];
				if($row["TRANSACTION_TYPE"] == 3) $data_pord[$row["PROD_ID"]][$lib_year_month_arr[$transaction_date]]["RECEIVE_RTN"]+=$row["CONS_QUANTITY"];
				if($row["TRANSACTION_TYPE"] == 6) $data_pord[$row["PROD_ID"]][$lib_year_month_arr[$transaction_date]]["TRANSFER_OUT"]+=$row["CONS_QUANTITY"];
				
				$data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$lib_year_month_arr[$transaction_date]]["QNT"]-=$row["CONS_QUANTITY"];
				$data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$lib_year_month_arr[$transaction_date]]["AMT"]-=$row["CONS_AMOUNT"];				
				if($row["TRANSACTION_TYPE"] == 2) $data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$lib_year_month_arr[$transaction_date]]["ISSUE"]+=$row["CONS_QUANTITY"];
				if($row["TRANSACTION_TYPE"] == 3) $data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$lib_year_month_arr[$transaction_date]]["RECEIVE_RTN"]+=$row["CONS_QUANTITY"];
				if($row["TRANSACTION_TYPE"] == 6) $data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$lib_year_month_arr[$transaction_date]]["TRANSFER_OUT"]+=$row["CONS_QUANTITY"];
				
				if($row["FLOOR_ID"])
				{
					$data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$lib_year_month_arr[$transaction_date]]["QNT"]-=$row["CONS_QUANTITY"];
					$data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$lib_year_month_arr[$transaction_date]]["AMT"]-=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 2) $data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$lib_year_month_arr[$transaction_date]]["ISSUE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 3) $data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$lib_year_month_arr[$transaction_date]]["RECEIVE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 6) $data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$lib_year_month_arr[$transaction_date]]["TRANSFER_OUT"]+=$row["CONS_QUANTITY"];
				}
				...
				if($row["ROOM"])
				{
					$data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$lib_year_month_arr[$transaction_date]]["QNT"]-=$row["CONS_QUANTITY"];
					$data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$lib_year_month_arr[$transaction_date]]["AMT"]-=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 2) $data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$lib_year_month_arr[$transaction_date]]["ISSUE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 3) $data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$lib_year_month_arr[$transaction_date]]["RECEIVE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 6) $data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$lib_year_month_arr[$transaction_date]]["TRANSFER_OUT"]+=$row["CONS_QUANTITY"];
				}
				...
				if($row["RACK"])
				{
					$data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$lib_year_month_arr[$transaction_date]]["QNT"]-=$row["CONS_QUANTITY"];
					$data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$lib_year_month_arr[$transaction_date]]["AMT"]-=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 2) $data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$lib_year_month_arr[$transaction_date]]["ISSUE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 3) $data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$lib_year_month_arr[$transaction_date]]["RECEIVE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 6) $data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$lib_year_month_arr[$transaction_date]]["TRANSFER_OUT"]+=$row["CONS_QUANTITY"];
				}
				...
				if($row["SELF"])
				{
					$data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$lib_year_month_arr[$transaction_date]]["QNT"]-=$row["CONS_QUANTITY"];
					$data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$lib_year_month_arr[$transaction_date]]["AMT"]-=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 2) $data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$lib_year_month_arr[$transaction_date]]["ISSUE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 3) $data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$lib_year_month_arr[$transaction_date]]["RECEIVE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 6) $data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$lib_year_month_arr[$transaction_date]]["TRANSFER_OUT"]+=$row["CONS_QUANTITY"];
				}
				...
				if($row["BIN_BOX"])
				{
					$data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$lib_year_month_arr[$transaction_date]]["QNT"]-=$row["CONS_QUANTITY"];
					$data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$lib_year_month_arr[$transaction_date]]["AMT"]-=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 2) $data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$lib_year_month_arr[$transaction_date]]["ISSUE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 3) $data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$lib_year_month_arr[$transaction_date]]["RECEIVE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 6) $data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$lib_year_month_arr[$transaction_date]]["TRANSFER_OUT"]+=$row["CONS_QUANTITY"];
				}
			...
			else
			{
				$data_pord[$row["PROD_ID"]][$first_month]["QNT"]-=$row["CONS_QUANTITY"];
				$data_pord[$row["PROD_ID"]][$first_month]["AMT"]-=$row["CONS_AMOUNT"];
				if($row["TRANSACTION_TYPE"] == 2) $data_pord[$row["PROD_ID"]][$first_month]["ISSUE"]+=$row["CONS_QUANTITY"];
				if($row["TRANSACTION_TYPE"] == 3) $data_pord[$row["PROD_ID"]][$first_month]["RECEIVE_RTN"]+=$row["CONS_QUANTITY"];
				if($row["TRANSACTION_TYPE"] == 6) $data_pord[$row["PROD_ID"]][$first_month]["TRANSFER_OUT"]+=$row["CONS_QUANTITY"];
				
				$data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$first_month]["QNT"]-=$row["CONS_QUANTITY"];
				$data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$first_month]["AMT"]-=$row["CONS_AMOUNT"];
				if($row["TRANSACTION_TYPE"] == 2) $data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$first_month]["ISSUE"]+=$row["CONS_QUANTITY"];
				if($row["TRANSACTION_TYPE"] == 3) $data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$first_month]["RECEIVE_RTN"]+=$row["CONS_QUANTITY"];
				if($row["TRANSACTION_TYPE"] == 6) $data_pord_store[$row["PROD_ID"]][$row["STORE_ID"]][$first_month]["TRANSFER_OUT"]+=$row["CONS_QUANTITY"];
				
				if($row["FLOOR_ID"])
				{
					$data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$first_month]["QNT"]-=$row["CONS_QUANTITY"];
					$data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$first_month]["AMT"]-=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 2) $data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$first_month]["ISSUE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 3) $data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$first_month]["RECEIVE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 6) $data_pord_store_floor[$row["PROD_ID"]][$row["FLOOR_ID"]][$first_month]["TRANSFER_OUT"]+=$row["CONS_QUANTITY"];
				}
				...
				if($row["ROOM"])
				{
					$data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$first_month]["QNT"]-=$row["CONS_QUANTITY"];
					$data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$first_month]["AMT"]-=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 2) $data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$first_month]["ISSUE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 3) $data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$first_month]["RECEIVE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 6) $data_pord_store_floor_room[$row["PROD_ID"]][$row["ROOM"]][$first_month]["TRANSFER_OUT"]+=$row["CONS_QUANTITY"];
				}
				...
				if($row["RACK"])
				{
					$data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$first_month]["QNT"]-=$row["CONS_QUANTITY"];
					$data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$first_month]["AMT"]-=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 2) $data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$first_month]["ISSUE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 3) $data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$first_month]["RECEIVE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 6) $data_pord_store_floor_room_rac[$row["PROD_ID"]][$row["RACK"]][$first_month]["TRANSFER_OUT"]+=$row["CONS_QUANTITY"];
				}
				...
				if($row["SELF"])
				{
					$data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$first_month]["QNT"]-=$row["CONS_QUANTITY"];
					$data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$first_month]["AMT"]-=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 2) $data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$first_month]["ISSUE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 3) $data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$first_month]["RECEIVE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 6) $data_pord_store_floor_room_rac_self[$row["PROD_ID"]][$row["SELF"]][$first_month]["TRANSFER_OUT"]+=$row["CONS_QUANTITY"];
				}
				...
				if($row["BIN_BOX"])
				{
					$data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$first_month]["QNT"]-=$row["CONS_QUANTITY"];
					$data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$first_month]["AMT"]-=$row["CONS_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 2) $data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$first_month]["ISSUE"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 3) $data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$first_month]["RECEIVE_RTN"]+=$row["CONS_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 6) $data_pord_store_floor_room_rac_self_bin[$row["PROD_ID"]][$row["BIN_BOX"]][$first_month]["TRANSFER_OUT"]+=$row["CONS_QUANTITY"];
				}
			}
		}
	}
	unset($sql_trans_result);
	//echo count($testy_arr);die;
	...
	$sql_inv_order="select a.id as ID, b.company_id as COMPANY_ID, a.prod_id as PROD_ID, b.item_category_id as ITEM_CATEGORY, a.po_breakdown_id as PO_BREAKDOWN_ID, a.trans_type as TRANSACTION_TYPE, a.quantity as ORD_QUANTITY, a.order_amount as ORD_AMOUNT, a.insert_date as INSERT_DATE, a.is_sales as IS_SALES 
	from order_wise_pro_details a, product_details_master b 
	where a.prod_id=b.id and a.TRANS_ID>0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.company_id=$cbo_company_id $year_cond_order order by PROD_ID";
	...
	foreach($sql_inv_order_result as $row)
	{
		...
		$order_date=date("Y-m",strtotime($row["INSERT_DATE"]));
		if($row["PO_BREAKDOWN_ID"])
		{
			...
			if($row["TRANSACTION_TYPE"] == 1 || $row["TRANSACTION_TYPE"] == 4 || $row["TRANSACTION_TYPE"] == 5)
			{
				...
				{
					$data_pord_order[$row["PROD_ID"]][$order_id_ref][$lib_year_month_arr[$order_date]]["QNT"]+=$row["ORD_QUANTITY"];
					$data_pord_order[$row["PROD_ID"]][$order_id_ref][$lib_year_month_arr[$order_date]]["AMT"]+=$row["ORD_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 1) $data_pord_order[$row["PROD_ID"]][$order_id_ref][$lib_year_month_arr[$order_date]]["RECEIVE"]+=$row["ORD_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 4) $data_pord_order[$row["PROD_ID"]][$order_id_ref][$lib_year_month_arr[$order_date]]["ISSUE_RTN"]+=$row["ORD_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 5) $data_pord_order[$row["PROD_ID"]][$order_id_ref][$lib_year_month_arr[$order_date]]["TRANSFER_IN"]+=$row["ORD_QUANTITY"];
				}
				else
				{
					$data_pord_order[$row["PROD_ID"]][$order_id_ref][$first_month]["QNT"]+=$row["ORD_QUANTITY"];
					$data_pord_order[$row["PROD_ID"]][$order_id_ref][$first_month]["AMT"]+=$row["ORD_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 1) $data_pord_order[$row["PROD_ID"]][$order_id_ref][$first_month]["RECEIVE"]+=$row["ORD_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 4) $data_pord_order[$row["PROD_ID"]][$order_id_ref][$first_month]["ISSUE_RTN"]+=$row["ORD_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 5) $data_pord_order[$row["PROD_ID"]][$order_id_ref][$first_month]["TRANSFER_IN"]+=$row["ORD_QUANTITY"];
				}
			...
			else
			{
				if($lib_year_month_arr[$order_date]!="")
				{
					$data_pord_order[$row["PROD_ID"]][$order_id_ref][$lib_year_month_arr[$order_date]]["QNT"]-=$row["ORD_QUANTITY"];
					$data_pord_order[$row["PROD_ID"]][$order_id_ref][$lib_year_month_arr[$order_date]]["AMT"]-=$row["ORD_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 2) $data_pord_order[$row["PROD_ID"]][$order_id_ref][$lib_year_month_arr[$order_date]]["ISSUE"]+=$row["ORD_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 3) $data_pord_order[$row["PROD_ID"]][$order_id_ref][$lib_year_month_arr[$order_date]]["RECEIVE_RTN"]+=$row["ORD_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 6) $data_pord_order[$row["PROD_ID"]][$order_id_ref][$lib_year_month_arr[$order_date]]["TRANSFER_OUT"]+=$row["ORD_QUANTITY"];
				}
				else
				{
					$data_pord_order[$row["PROD_ID"]][$order_id_ref][$first_month]["QNT"]-=$row["ORD_QUANTITY"];
					$data_pord_order[$row["PROD_ID"]][$order_id_ref][$first_month]["AMT"]-=$row["ORD_AMOUNT"];
					if($row["TRANSACTION_TYPE"] == 2) $data_pord_order[$row["PROD_ID"]][$order_id_ref][$first_month]["ISSUE"]+=$row["ORD_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 3) $data_pord_order[$row["PROD_ID"]][$order_id_ref][$first_month]["RECEIVE_RTN"]+=$row["ORD_QUANTITY"];
					if($row["TRANSACTION_TYPE"] == 6) $data_pord_order[$row["PROD_ID"]][$order_id_ref][$first_month]["TRANSFER_OUT"]+=$row["ORD_QUANTITY"];
				}
			}
		}
	}
		...
		if($transaction_test_arr[$prod_id])
		{
			...
			if($prev_close_item_data[$prod_id]["ID"]=="")
			{
				...
				foreach($lib_month_colum_arr as $key=>$value)
				{
					...
					{
						$prod_qnty+=$data_pord[$prod_id][$value]["QNT"];
					}
					if($data_pord[$prod_id][$value]["AMT"]) 
					{
						$prod_amt+=$data_pord[$prod_id][$value]["AMT"];
					}
					if($data_pord[$prod_id][$value]["RECEIVE"]) 
					{
						$receive_qnty+=$data_pord[$prod_id][$value]["RECEIVE"];
					}
					if($data_pord[$prod_id][$value]["ISSUE"]) 
					{
						$issue_qnty+=$data_pord[$prod_id][$value]["ISSUE"];
					}
					if($data_pord[$prod_id][$value]["RECEIVE_RTN"]) 
					{
						$receive_rtn_qnty+=$data_pord[$prod_id][$value]["RECEIVE_RTN"];
					}
					if($data_pord[$prod_id][$value]["ISSUE_RTN"]) 
					{
						$issue_rtn_qnty+=$data_pord[$prod_id][$value]["ISSUE_RTN"];
					}
					if($data_pord[$prod_id][$value]["TRANSFER_IN"]) 
					{
						$transfer_in_qnty+=$data_pord[$prod_id][$value]["TRANSFER_IN"];
					}
					if($data_pord[$prod_id][$value]["TRANSFER_OUT"]) 
					{
						$transfer_out_qnty+=$data_pord[$prod_id][$value]["TRANSFER_OUT"];
					}
					
					if($prod_amt!=0 && $prod_qnty!=0) $prod_rate=number_format(($prod_amt/$prod_qnty),4,".",""); else $prod_rate=0;
			...
			foreach($data_pord_store[$prod_id] as $store_id=>$store_data)
			{
				...
				if($prev_close_item_ref_data[$prod_id][1][$store_id]["ID"]=="")
				{
					...
					foreach($lib_month_colum_arr as $key=>$value)
					{
						...
						{
							$store_qnty +=$store_data[$value]["QNT"];
						}
						if($store_data[$value]["AMT"]) 
						{
							$store_amt +=$store_data[$value]["AMT"];
						}
						if($store_data[$value]["RECEIVE"]) 
						{
							$receive_qnty+=$store_data[$value]["RECEIVE"];
						}
						if($store_data[$value]["ISSUE"]) 
						{
							$issue_qnty+=$store_data[$value]["ISSUE"];
						}
						if($store_data[$value]["RECEIVE_RTN"]) 
						{
							$receive_rtn_qnty+=$store_data[$value]["RECEIVE_RTN"];
						}
						if($store_data[$value]["ISSUE_RTN"]) 
						{
							$issue_rtn_qnty+=$store_data[$value]["ISSUE_RTN"];
						}
						if($store_data[$value]["TRANSFER_IN"]) 
						{
							$transfer_in_qnty+=$store_data[$value]["TRANSFER_IN"];
						}
						if($store_data[$value]["TRANSFER_OUT"]) 
						{
							$transfer_out_qnty+=$store_data[$value]["TRANSFER_OUT"];
						}
						
						if($store_amt!=0 && $store_qnty!=0) $store_rate=number_format(($store_amt/$store_qnty),4,".",""); else $store_rate=0;
					...
					if($floor_id && trim(str_replace("'","",$floor_id)) !="")
					{
						...
						if($prev_close_item_ref_data[$prod_id][2][$floor_id]["ID"]=="")
						{
							...
							foreach($lib_month_colum_arr as $key=>$value)
							{
								...
								{
									$floor_qnty +=$floor_data[$value]["QNT"];
								}
								if($floor_data[$value]["AMT"]) 
								{
									$floor_amt +=$floor_data[$value]["AMT"];
								}
								if($floor_data[$value]["RECEIVE"]) 
								{
									$receive_qnty+=$floor_data[$value]["RECEIVE"];
								}
								if($floor_data[$value]["ISSUE"]) 
								{
									$issue_qnty+=$floor_data[$value]["ISSUE"];
								}
								if($floor_data[$value]["RECEIVE_RTN"]) 
								{
									$receive_rtn_qnty+=$floor_data[$value]["RECEIVE_RTN"];
								}
								if($floor_data[$value]["ISSUE_RTN"]) 
								{
									$issue_rtn_qnty+=$floor_data[$value]["ISSUE_RTN"];
								}
								if($floor_data[$value]["TRANSFER_IN"]) 
								{
									$transfer_in_qnty+=$floor_data[$value]["TRANSFER_IN"];
								}
								if($floor_data[$value]["TRANSFER_OUT"]) 
								{
									$transfer_out_qnty+=$floor_data[$value]["TRANSFER_OUT"];
								}
								
								if($floor_amt!=0 && $floor_qnty!=0) $floor_rate=number_format(($floor_amt/$floor_qnty),4,".",""); else $floor_rate=0;
					...
					if($room_id && trim(str_replace("'","",$room_id)) !="")
					{
						...
						if($prev_close_item_ref_data[$prod_id][3][$room_id]["ID"]=="")
						{
							...
							foreach($lib_month_colum_arr as $key=>$value)
							{
								...
								{
									$room_qnty +=$room_data[$value]["QNT"];
								}
								if($room_data[$value]["AMT"]) 
								{
									$room_amt +=$room_data[$value]["AMT"];
								}
								if($room_data[$value]["RECEIVE"]) 
								{
									$receive_qnty+=$room_data[$value]["RECEIVE"];
								}
								if($room_data[$value]["ISSUE"]) 
								{
									$issue_qnty+=$room_data[$value]["ISSUE"];
								}
								if($room_data[$value]["RECEIVE_RTN"]) 
								{
									$receive_rtn_qnty+=$room_data[$value]["RECEIVE_RTN"];
								}
								if($room_data[$value]["ISSUE_RTN"]) 
								{
									$issue_rtn_qnty+=$room_data[$value]["ISSUE_RTN"];
								}
								if($room_data[$value]["TRANSFER_IN"]) 
								{
									$transfer_in_qnty+=$room_data[$value]["TRANSFER_IN"];
								}
								if($room_data[$value]["TRANSFER_OUT"]) 
								{
									$transfer_out_qnty+=$room_data[$value]["TRANSFER_OUT"];
								}
								
								if($room_amt!=0 && $room_qnty!=0) $room_rate=number_format(($room_amt/$room_qnty),4,".",""); else $room_rate=0;
					...
					if($rack_id>0 && trim(str_replace("'","",$rack_id)) !="")
					{
						...
						if($prev_close_item_ref_data[$prod_id][4][$rack_id]["ID"]=="")
						{
							...
							foreach($lib_month_colum_arr as $key=>$value)
							{
								...
								{
									$rack_qnty +=$rack_data[$value]["QNT"];
								}
								if($rack_data[$value]["AMT"]) 
								{
									$rack_amt +=$rack_data[$value]["AMT"];
								}
								if($rack_data[$value]["RECEIVE"]) 
								{
									$receive_qnty+=$rack_data[$value]["RECEIVE"];
								}
								if($rack_data[$value]["ISSUE"]) 
								{
									$issue_qnty+=$rack_data[$value]["ISSUE"];
								}
								if($rack_data[$value]["RECEIVE_RTN"]) 
								{
									$receive_rtn_qnty+=$rack_data[$value]["RECEIVE_RTN"];
								}
								if($rack_data[$value]["ISSUE_RTN"]) 
								{
									$issue_rtn_qnty+=$rack_data[$value]["ISSUE_RTN"];
								}
								if($rack_data[$value]["TRANSFER_IN"]) 
								{
									$transfer_in_qnty+=$rack_data[$value]["TRANSFER_IN"];
								}
								if($rack_data[$value]["TRANSFER_OUT"]) 
								{
									$transfer_out_qnty+=$rack_data[$value]["TRANSFER_OUT"];
								}
								
								if($rack_amt!=0 && $rack_qnty!=0) $rack_rate=number_format(($rack_amt/$rack_qnty),4,".",""); else $rack_rate=0;
					...
					if($shelf_id && trim(str_replace("'","",$shelf_id)) !="")
					{
						...
						if($prev_close_item_ref_data[$prod_id][5][$shelf_id]["ID"]=="")
						{
							...
							foreach($lib_month_colum_arr as $key=>$value)
							{
								...
								{
									$shelf_qnty +=$shelf_data[$value]["QNT"];
								}
								if($shelf_data[$value]["AMT"]) 
								{
									$shelf_amt +=$shelf_data[$value]["AMT"];
								}
								if($shelf_data[$value]["RECEIVE"]) 
								{
									$receive_qnty+=$shelf_data[$value]["RECEIVE"];
								}
								if($shelf_data[$value]["ISSUE"]) 
								{
									$issue_qnty+=$shelf_data[$value]["ISSUE"];
								}
								if($shelf_data[$value]["RECEIVE_RTN"]) 
								{
									$receive_rtn_qnty+=$shelf_data[$value]["RECEIVE_RTN"];
								}
								if($shelf_data[$value]["ISSUE_RTN"]) 
								{
									$issue_rtn_qnty+=$shelf_data[$value]["ISSUE_RTN"];
								}
								if($shelf_data[$value]["TRANSFER_IN"]) 
								{
									$transfer_in_qnty+=$shelf_data[$value]["TRANSFER_IN"];
								}
								if($shelf_data[$value]["TRANSFER_OUT"]) 
								{
									$transfer_out_qnty+=$shelf_data[$value]["TRANSFER_OUT"];
								}
								
								if($shelf_amt!=0 && $shelf_qnty!=0) $shelf_rate=number_format(($shelf_amt/$shelf_qnty),4,".",""); else $shelf_rate=0;
					...
					if($bin_id && trim(str_replace("'","",$bin_id)) !="")
					{
						...
						if($prev_close_item_ref_data[$prod_id][6][$bin_id]["ID"]=="")
						{
							...
							foreach($lib_month_colum_arr as $key=>$value)
							{
								...
								{
									$bin_qnty +=$bin_data[$value]["QNT"];
								}
								if($bin_data[$value]["AMT"]) 
								{
									$bin_amt +=$bin_data[$value]["AMT"];
								}
								if($bin_data[$value]["RECEIVE"]) 
								{
									$receive_qnty+=$bin_data[$value]["RECEIVE"];
								}
								if($bin_data[$value]["ISSUE"]) 
								{
									$issue_qnty+=$bin_data[$value]["ISSUE"];
								}
								if($bin_data[$value]["RECEIVE_RTN"]) 
								{
									$receive_rtn_qnty+=$bin_data[$value]["RECEIVE_RTN"];
								}
								if($bin_data[$value]["ISSUE_RTN"]) 
								{
									$issue_rtn_qnty+=$bin_data[$value]["ISSUE_RTN"];
								}
								if($bin_data[$value]["TRANSFER_IN"]) 
								{
									$transfer_in_qnty+=$bin_data[$value]["TRANSFER_IN"];
								}
								if($bin_data[$value]["TRANSFER_OUT"]) 
								{
									$transfer_out_qnty+=$bin_data[$value]["TRANSFER_OUT"];
								}
								
								if($bin_amt!=0 && $bin_qnty!=0) $bin_rate=number_format(($bin_amt / $bin_qnty),4,".",""); else $bin_rate=0;
					...
					if($order_id && trim(str_replace("'","",$order_id)) !="")
					{
						...
						if($prev_close_item_ref_data[$prod_id][$ref_type_seq][$order_id]["ID"]=="")
						{
							...
							foreach($lib_month_colum_arr as $key=>$value)
							{
								...
								{
									$order_qnty +=$order_data[$value]["QNT"];
								}
								if($order_data[$value]["AMT"]) 
								{
									$order_amt +=$order_data[$value]["AMT"];
								}
								if($order_data[$value]["RECEIVE"]) 
								{
									$receive_qnty+=$order_data[$value]["RECEIVE"];
								}
								if($order_data[$value]["ISSUE"]) 
								{
									$issue_qnty+=$order_data[$value]["ISSUE"];
								}
								if($order_data[$value]["RECEIVE_RTN"]) 
								{
									$receive_rtn_qnty+=$order_data[$value]["RECEIVE_RTN"];
								}
								if($order_data[$value]["ISSUE_RTN"]) 
								{
									$issue_rtn_qnty+=$order_data[$value]["ISSUE_RTN"];
								}
								if($order_data[$value]["TRANSFER_IN"]) 
								{
									$transfer_in_qnty+=$order_data[$value]["TRANSFER_IN"];
								}
								if($order_data[$value]["TRANSFER_OUT"]) 
								{
									$transfer_out_qnty+=$order_data[$value]["TRANSFER_OUT"];
								}
								
								if($order_amt!=0 && $order_qnty!=0) $order_rate=number_format(($order_amt/$order_qnty),4,".",""); else $order_rate=0;
			...
			if(count($prev_ref_data[$prod_id][1])>0)
			{
				foreach($prev_ref_data[$prod_id][1] as $store_id)
				{
					...
					foreach($lib_month_colum_arr as $key=>$value)
					{
						...
						{
							$store_qnty+=$data_pord_store[$prod_id][$store_id][$value]["QNT"];
						}
						if($data_pord_store[$prod_id][$store_id][$value]["AMT"]) 
						{
							$store_amt+=$data_pord_store[$prod_id][$store_id][$value]["AMT"];
						}
						if($data_pord_store[$prod_id][$store_id][$value]["RECEIVE"]) 
						{
							$receive_qnty+=$data_pord_store[$prod_id][$store_id][$value]["RECEIVE"];
						}
						if($data_pord_store[$prod_id][$store_id][$value]["ISSUE"]) 
						{
							$issue_qnty+=$data_pord_store[$prod_id][$store_id][$value]["ISSUE"];
						}
						if($data_pord_store[$prod_id][$store_id][$value]["RECEIVE_RTN"]) 
						{
							$receive_rtn_qnty+=$data_pord_store[$prod_id][$store_id][$value]["RECEIVE_RTN"];
						}
						if($data_pord_store[$prod_id][$store_id][$value]["ISSUE_RTN"]) 
						{
							$issue_rtn_qnty+=$data_pord_store[$prod_id][$store_id][$value]["ISSUE_RTN"];
						}
						if($data_pord_store[$prod_id][$store_id][$value]["TRANSFER_IN"]) 
						{
							$transfer_in_qnty+=$data_pord_store[$prod_id][$store_id][$value]["TRANSFER_IN"];
						}
						if($data_pord_store[$prod_id][$store_id][$value]["TRANSFER_OUT"]) 
						{
							$transfer_out_qnty+=$data_pord_store[$prod_id][$store_id][$value]["TRANSFER_OUT"];
						}
						if($store_amt!=0 && $store_qnty!=0) $store_rate=number_format(($store_amt/$store_qnty),4,".",""); else $store_rate=0;
						$insert_year_close_item_ref .="$key='".$store_qnty."', $key"."_rate='".$store_rate."', $key"."_receive='".$receive_qnty."', $key"."_issue='".$issue_qnty."', $key"."_receive_rtn='".$receive_rtn_qnty."', $key"."_issue_rtn='".$issue_rtn_qnty."', $key"."_transfer_in='".$transfer_in_qnty."', $key"."_transfer_out='".$transfer_out_qnty."',";
					}
				...
				foreach($prev_ref_data[$prod_id][2] as $floor_id)
				{
					if($floor_id && trim(str_replace("'","",$floor_id)) !="")
					{
						...
						foreach($lib_month_colum_arr as $key=>$value)
						{
							...
							{
								$floor_qnty+=$data_pord_store_floor[$prod_id][$floor_id][$value]["QNT"];
							}
							if($data_pord_store_floor[$prod_id][$floor_id][$value]["AMT"]) 
							{
								$floor_amt+=$data_pord_store_floor[$prod_id][$floor_id][$value]["AMT"];
							}
							if($data_pord_store_floor[$prod_id][$floor_id][$value]["RECEIVE"]) 
							{
								$receive_qnty+=$data_pord_store_floor[$prod_id][$floor_id][$value]["RECEIVE"];
							}
							if($data_pord_store_floor[$prod_id][$floor_id][$value]["ISSUE"]) 
							{
								$issue_qnty+=$data_pord_store_floor[$prod_id][$floor_id][$value]["ISSUE"];
							}
							if($data_pord_store_floor[$prod_id][$floor_id][$value]["RECEIVE_RTN"]) 
							{
								$receive_rtn_qnty+=$data_pord_store_floor[$prod_id][$floor_id][$value]["RECEIVE_RTN"];
							}
							if($data_pord_store_floor[$prod_id][$floor_id][$value]["ISSUE_RTN"]) 
							{
								$issue_rtn_qnty+=$data_pord_store_floor[$prod_id][$floor_id][$value]["ISSUE_RTN"];
							}
							if($data_pord_store_floor[$prod_id][$floor_id][$value]["TRANSFER_IN"]) 
							{
								$transfer_in_qnty+=$data_pord_store_floor[$prod_id][$floor_id][$value]["TRANSFER_IN"];
							}
							if($data_pord_store_floor[$prod_id][$floor_id][$value]["TRANSFER_OUT"]) 
							{
								$transfer_out_qnty+=$data_pord_store_floor[$prod_id][$floor_id][$value]["TRANSFER_OUT"];
							}
				...
				foreach($prev_ref_data[$prod_id][3] as $room_id)
				{
					if($room_id && trim(str_replace("'","",$room_id)) !="")
					{
						...
						foreach($lib_month_colum_arr as $key=>$value)
						{
							...
							{
								$room_qnty +=$data_pord_store_floor_room[$prod_id][$room_id][$value]["QNT"];
							}
							if($data_pord_store_floor_room[$prod_id][$room_id][$value]["AMT"]) 
							{
								$room_amt +=$data_pord_store_floor_room[$prod_id][$room_id][$value]["AMT"];
							}
							if($data_pord_store_floor_room[$prod_id][$room_id][$value]["RECEIVE"]) 
							{
								$receive_qnty+=$data_pord_store_floor_room[$prod_id][$room_id][$value]["RECEIVE"];
							}
							if($data_pord_store_floor_room[$prod_id][$room_id][$value]["ISSUE"]) 
							{
								$issue_qnty+=$data_pord_store_floor_room[$prod_id][$room_id][$value]["ISSUE"];
							}
							if($data_pord_store_floor_room[$prod_id][$room_id][$value]["RECEIVE_RTN"]) 
							{
								$receive_rtn_qnty+=$data_pord_store_floor_room[$prod_id][$room_id][$value]["RECEIVE_RTN"];
							}
							if($data_pord_store_floor_room[$prod_id][$room_id][$value]["ISSUE_RTN"]) 
							{
								$issue_rtn_qnty+=$data_pord_store_floor_room[$prod_id][$room_id][$value]["ISSUE_RTN"];
							}
							if($data_pord_store_floor_room[$prod_id][$room_id][$value]["TRANSFER_IN"]) 
							{
								$transfer_in_qnty+=$data_pord_store_floor_room[$prod_id][$room_id][$value]["TRANSFER_IN"];
							}
							if($data_pord_store_floor_room[$prod_id][$room_id][$value]["TRANSFER_OUT"]) 
							{
								$transfer_out_qnty+=$data_pord_store_floor_room[$prod_id][$room_id][$value]["TRANSFER_OUT"];
							}
				...
				foreach($prev_ref_data[$prod_id][4] as $rack_id)
				{
					if($rack_id && trim(str_replace("'","",$rack_id)) !="")
					{
						...
						foreach($lib_month_colum_arr as $key=>$value)
						{
							...
							{
								$rack_qnty +=$data_pord_store_floor_room_rac[$prod_id][$rack_id][$value]["QNT"];
							}
							if($data_pord_store_floor_room_rac[$prod_id][$rack_id][$value]["AMT"]) 
							{
								$rack_amt +=$data_pord_store_floor_room_rac[$prod_id][$rack_id][$value]["AMT"];
							}
							if($data_pord_store_floor_room_rac[$prod_id][$rack_id][$value]["RECEIVE"]) 
							{
								$receive_qnty+=$data_pord_store_floor_room_rac[$prod_id][$rack_id][$value]["RECEIVE"];
							}
							if($data_pord_store_floor_room_rac[$prod_id][$rack_id][$value]["ISSUE"]) 
							{
								$issue_qnty+=$data_pord_store_floor_room_rac[$prod_id][$rack_id][$value]["ISSUE"];
							}
							if($data_pord_store_floor_room_rac[$prod_id][$rack_id][$value]["RECEIVE_RTN"]) 
							{
								$receive_rtn_qnty+=$data_pord_store_floor_room_rac[$prod_id][$rack_id][$value]["RECEIVE_RTN"];
							}
							if($data_pord_store_floor_room_rac[$prod_id][$rack_id][$value]["ISSUE_RTN"]) 
							{
								$issue_rtn_qnty+=$data_pord_store_floor_room_rac[$prod_id][$rack_id][$value]["ISSUE_RTN"];
							}
							if($data_pord_store_floor_room_rac[$prod_id][$rack_id][$value]["TRANSFER_IN"]) 
							{
								$transfer_in_qnty+=$data_pord_store_floor_room_rac[$prod_id][$rack_id][$value]["TRANSFER_IN"];
							}
							if($data_pord_store_floor_room_rac[$prod_id][$rack_id][$value]["TRANSFER_OUT"]) 
							{
								$transfer_out_qnty+=$data_pord_store_floor_room_rac[$prod_id][$rack_id][$value]["TRANSFER_OUT"];
							}
				...
				foreach($prev_ref_data[$prod_id][5] as $shelf_id)
				{
					if($shelf_id && trim(str_replace("'","",$shelf_id)) !="")
					{
						...
						foreach($lib_month_colum_arr as $key=>$value)
						{
							...
							{
								$shelf_qnty +=$data_pord_store_floor_room_rac_self[$prod_id][$shelf_id][$value]["QNT"];
							}
							if($data_pord_store_floor_room_rac_self[$prod_id][$shelf_id][$value]["AMT"]) 
							{
								$shelf_amt +=$data_pord_store_floor_room_rac_self[$prod_id][$shelf_id][$value]["AMT"];
							}
							if($data_pord_store_floor_room_rac_self[$prod_id][$shelf_id][$value]["RECEIVE"]) 
							{
								$receive_qnty+=$data_pord_store_floor_room_rac_self[$prod_id][$shelf_id][$value]["RECEIVE"];
							}
							if($data_pord_store_floor_room_rac_self[$prod_id][$shelf_id][$value]["ISSUE"]) 
							{
								$issue_qnty+=$data_pord_store_floor_room_rac_self[$prod_id][$shelf_id][$value]["ISSUE"];
							}
							if($data_pord_store_floor_room_rac_self[$prod_id][$shelf_id][$value]["RECEIVE_RTN"]) 
							{
								$receive_rtn_qnty+=$data_pord_store_floor_room_rac_self[$prod_id][$shelf_id][$value]["RECEIVE_RTN"];
							}
							if($data_pord_store_floor_room_rac_self[$prod_id][$shelf_id][$value]["ISSUE_RTN"]) 
							{
								$issue_rtn_qnty+=$data_pord_store_floor_room_rac_self[$prod_id][$shelf_id][$value]["ISSUE_RTN"];
							}
							if($data_pord_store_floor_room_rac_self[$prod_id][$shelf_id][$value]["TRANSFER_IN"]) 
							{
								$transfer_in_qnty+=$data_pord_store_floor_room_rac_self[$prod_id][$shelf_id][$value]["TRANSFER_IN"];
							}
							if($data_pord_store_floor_room_rac_self[$prod_id][$shelf_id][$value]["TRANSFER_OUT"]) 
							{
								$transfer_out_qnty+=$data_pord_store_floor_room_rac_self[$prod_id][$shelf_id][$value]["TRANSFER_OUT"];
							}
				...
				foreach($prev_ref_data[$prod_id][6] as $bin_id)
				{
					if($bin_id && trim(str_replace("'","",$bin_id)) !="")
					{
						...
						foreach($lib_month_colum_arr as $key=>$value)
						{
							...
							{
								$bin_qnty +=$data_pord_store_floor_room_rac_self_bin[$prod_id][$bin_id][$value]["QNT"];
							}
							if($data_pord_store_floor_room_rac_self_bin[$prod_id][$bin_id][$value]["AMT"]) 
							{
								$bin_amt +=$data_pord_store_floor_room_rac_self_bin[$prod_id][$bin_id][$value]["AMT"];
							}
							if($data_pord_store_floor_room_rac_self_bin[$prod_id][$bin_id][$value]["RECEIVE"]) 
							{
								$receive_qnty+=$data_pord_store_floor_room_rac_self_bin[$prod_id][$bin_id][$value]["RECEIVE"];
							}
							if($data_pord_store_floor_room_rac_self_bin[$prod_id][$bin_id][$value]["ISSUE"]) 
							{
								$issue_qnty+=$data_pord_store_floor_room_rac_self_bin[$prod_id][$bin_id][$value]["ISSUE"];
							}
							if($data_pord_store_floor_room_rac_self_bin[$prod_id][$bin_id][$value]["RECEIVE_RTN"]) 
							{
								$receive_rtn_qnty+=$data_pord_store_floor_room_rac_self_bin[$prod_id][$bin_id][$value]["RECEIVE_RTN"];
							}
							if($data_pord_store_floor_room_rac_self_bin[$prod_id][$bin_id][$value]["ISSUE_RTN"]) 
							{
								$issue_rtn_qnty+=$data_pord_store_floor_room_rac_self_bin[$prod_id][$bin_id][$value]["ISSUE_RTN"];
							}
							if($data_pord_store_floor_room_rac_self_bin[$prod_id][$bin_id][$value]["TRANSFER_IN"]) 
							{
								$transfer_in_qnty+=$data_pord_store_floor_room_rac_self_bin[$prod_id][$bin_id][$value]["TRANSFER_IN"];
							}
							if($data_pord_store_floor_room_rac_self_bin[$prod_id][$bin_id][$value]["TRANSFER_OUT"]) 
							{
								$transfer_out_qnty+=$data_pord_store_floor_room_rac_self_bin[$prod_id][$bin_id][$value]["TRANSFER_OUT"];
							}
				...
				foreach($prev_ref_data[$prod_id][7] as $order_id)
				{
					if($order_id && trim(str_replace("'","",$order_id)) !="")
					{
						...
						foreach($lib_month_colum_arr as $key=>$value)
						{
							...
							{
								$order_qnty +=$data_pord_order[$prod_id][$order_id_ref][$value]["QNT"];
							}
							if($data_pord_order[$prod_id][$order_id_ref][$value]["AMT"]) 
							{
								$order_amt +=$data_pord_order[$prod_id][$order_id_ref][$value]["AMT"];
							}
							if($data_pord_order[$prod_id][$order_id_ref][$value]["RECEIVE"]) 
							{
								$receive_qnty+=$data_pord_order[$prod_id][$order_id_ref][$value]["RECEIVE"];
							}
							if($data_pord_order[$prod_id][$order_id_ref][$value]["ISSUE"]) 
							{
								$issue_qnty+=$data_pord_order[$prod_id][$order_id_ref][$value]["ISSUE"];
							}
							if($data_pord_order[$prod_id][$order_id_ref][$value]["RECEIVE_RTN"]) 
							{
								$receive_rtn_qnty+=$data_pord_order[$prod_id][$order_id_ref][$value]["RECEIVE_RTN"];
							}
							if($data_pord_order[$prod_id][$order_id_ref][$value]["ISSUE_RTN"]) 
							{
								$issue_rtn_qnty+=$data_pord_order[$prod_id][$order_id_ref][$value]["ISSUE_RTN"];
							}
							if($data_pord_order[$prod_id][$order_id_ref][$value]["TRANSFER_IN"]) 
							{
								$transfer_in_qnty+=$data_pord_order[$prod_id][$order_id_ref][$value]["TRANSFER_IN"];
							}
							if($data_pord_order[$prod_id][$order_id_ref][$value]["TRANSFER_OUT"]) 
							{
								$transfer_out_qnty+=$data_pord_order[$prod_id][$order_id_ref][$value]["TRANSFER_OUT"];
							}
	...
	if($db_type==0)
	{
		if($update_period_mst_rid && $update_period_dtls_rid && $update_year_item_rid && $update_year_item_ref_rid)
		{
			mysql_query("COMMIT");  
