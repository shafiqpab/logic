...
if ($action == "wo_product_form_yarn_dyeing")
{
	...
	if($prod_id!="")
	{
		...
		foreach ($result as $row) 
		{
			...
			if( $purpose==7 || $purpose==12 || $purpose==38 || $purpose==46 )
			{
...
if ($action == "save_update_delete") 
{
	...
	$variable_set_allocation = return_field_value("allocation", "variable_settings_inventory", "company_name=$cbo_company_id and variable_list=18 and item_category_id = 1","allocation");
	...
	if ($operation == 0) //Insert Here
	{
		...
		if((str_replace("'", "", $cbo_receive_basis) == 2))
		{
			...
			*/
			...
			if( $cbo_receive_purpose== 2 || $cbo_receive_purpose==12 || $cbo_receive_purpose==15 || $cbo_receive_purpose==38 )
			{
				...
				//echo "40**".$allow_total_val."<".$grey_quantity_balance; die();
				//1.20<2.00
		...
		*/
		...
		if (str_replace("'", "", $txt_mrr_no) != "")
		{
			...
			$field_array = "item_category*receive_basis*receive_purpose*receive_date*booking_id*booking_no*challan_no*store_id*exchange_rate*currency_id*supplier_id*loan_party*yarn_issue_challan_no*issue_id*lc_no*source*remarks*updated_by*update_date";
			$data_array = "1*" . $cbo_receive_basis . "*" . $cbo_receive_purpose . "*" . $txt_receive_date . "*" . $txt_wo_pi_id . "*" . $txt_wo_pi . "*" . $txt_challan_no . "*" . $cbo_store_name . "*" . $txt_exchange_rate . "*" . $cbo_currency . "*" . $cbo_supplier . "*" . $cbo_party . "*" . $txt_issue_challan_no . "*" . $txt_issue_id . "*" . $hidden_lc_id . "*" . $cbo_source . "*" . $txt_mst_remarks . "*'" . $user_id . "'*'" . $pc_date_time . "'";
		...
		if ($variable_set_allocation == 1)
		{
			...
			{
				if ( ($variable_set_smn_allocation == 1) && (str_replace("'", "", $cbo_receive_purpose) == 2) && ( $is_without_order == 42 || $is_without_order == 114) )
				{
					$allocated_qnty = $allocated_qnty + $txt_receive_qty;
					$available_qnty = $available_qnty;
				}
				else if ( ($variable_set_smn_allocation != 1) && (str_replace("'", "", $cbo_receive_purpose) == 2) && ( $is_without_order == 42 || $is_without_order == 114) )
				{
		...
		if ($variable_set_allocation == 1)
		{
			...
			if ( (str_replace("'", "", $cbo_receive_basis) == 2) && (str_replace("'", "", $cbo_receive_purpose) == 2 || str_replace("'", "", $cbo_receive_purpose) == 12 || str_replace("'", "", $cbo_receive_purpose) == 15 || str_replace("'", "", $cbo_receive_purpose) == 38 || str_replace("'", "", $cbo_receive_purpose) == 46 || str_replace("'", "", $cbo_receive_purpose) == 50 || str_replace("'", "", $cbo_receive_purpose) == 51 ) ) 
			{
				if ( ($variable_set_smn_allocation == 1) && (str_replace("'", "", $cbo_receive_purpose) == 2) && ( $is_without_order == 42 || $is_without_order == 114) )
				{
					...
					*/
					...
					else
					{
						...
						*/
						...
						$field_allocation = "id,mst_id,entry_form,item_category,allocation_date,item_id,qnty,is_dyied_yarn,po_break_down_id,booking_no,booking_without_order,qnty_break_down,inserted_by,insert_date";
						$qnty_breakdown = $txt_receive_qty.'_'.$bookingId.'_';
						$data_allocation = "(".$allocation_id.",".$id.",1,1".",".$txt_receive_date.",".$prodMSTID.",".$txt_receive_qty.",1,'".$bookingId."','".$bookingNo."',1,'".$qnty_breakdown."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
						...
						*/
						...
						$field_allocation_dtls = "id,mst_id,item_category,allocation_date,item_id,qnty,is_dyied_yarn,po_break_down_id,booking_no,inserted_by,insert_date";
						$data_allocation_dtls = "(".$allocation_dtls_id.",".$allocation_id.",1,".$txt_receive_date.",".$prodMSTID.",".$txt_receive_qty.",1,'".$bookingId."','".$bookingNo."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				...
				else
				{
					...
					else
					{
						/*
						|--------------------------------------------------------------------------
						| if receive basis yarn dyeing workorder then
						| booking no will be fabric bookin_no
						|--------------------------------------------------------------------------
						|
						*/
						if (str_replace("'", "", $cbo_receive_basis) == 2 && str_replace("'", "", $cbo_receive_purpose) == 2)
						{
							$expBookinNo = explode("-", $txt_wo_pi);
							
							if($expBookinNo[1] == 'YDW')
							{
								if( $is_without_order==41 || $is_without_order==125 )
								{
								...
								$sqlFabricBookin = "SELECT a.booking_no AS BOOKING_NO FROM wo_booking_mst a INNER JOIN wo_yarn_dyeing_dtls b ON a.job_no = b.job_no WHERE b.mst_id = ".$txt_wo_pi_id."  $booking_type_cond AND a.status_active = 1 AND a.is_deleted = 0 AND b.status_active = 1 AND b.is_deleted = 0 GROUP BY a.booking_no";
						...
						*/
						...
						foreach($expRcvString as $expRcv)
						{
							...
							$data_allocation_dtls .= "(".$allocation_dtls_id.",".$allocation_id.",".$job_no.",".$rcvData[0].",".$txt_wo_pi.",1,".$txt_receive_date.",".$prodMSTID.",".$rcvData[1].",1,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
						}
						$allocation_dtls_insert = sql_insert("inv_material_allocation_dtls", $field_allocation_dtls, $data_allocation_dtls, 0);
						...
						|
						*/
						$field_allocation = "id,entry_form,job_no,po_break_down_id,booking_no,item_category,allocation_date,item_id,qnty,is_dyied_yarn,qnty_break_down,inserted_by,insert_date";
						$data_allocation = "(".$allocation_id.",1,".$job_no.",'".$po_breakdown_id."',".$txt_wo_pi.",1".",".$txt_receive_date.",".$prodMSTID.",".$txt_receive_qty.",1,'".$qnty_breakdown."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
						$allocation_mst_insert = sql_insert("inv_material_allocation_mst", $field_allocation, $data_allocation, 0);
					}					
				}
			} 
		...
		*/
		...
		if($hdnReceiveString != '')
		{
			$field_proportionate = "id, trans_id, trans_type, entry_form, po_breakdown_id, prod_id, color_id, quantity, inserted_by, insert_date";
			$expRcvString = explode(',', $hdnReceiveString);
			foreach($expRcvString as $expRcv)
			{
				...
				//hdnReceiveString += hdnOrderNo+'_'+orderReceiveQty+'_'+receiveQty+'_'+distribiutionMethod;
				$data_proportionate .= "(".$id_prop.",".$dtlsid.",1,1,".$rcvData[0].",".$prodMSTID.",".$cbo_color.",".$rcvData[1].",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			}
		}
		...
		*/
		...
		//oci_rollback($con);
		//echo "10**".$rID ."&&". $dtlsrID ."&&". $prodUpdate ."&&". $insertR  ."&&". $rslt_proportionate ."&&". $allocation_mst_insert ."&&". $allocation_dtls_insert; die();
		
		if ($db_type == 0)
		{
			if ($rID && $dtlsrID && $prodUpdate && $insertR && $rslt_proportionate && $allocation_mst_insert && $allocation_dtls_insert)
			{
				mysql_query("COMMIT");
	...
	else if ($operation == 1) //Update Here
	{
		...
		if ($variable_set_allocation == 1)
		{
			...
			{
				if ( ($variable_set_smn_allocation == 1) && ($before_receive_purpose == 2) && ( $is_without_order == 42 || $is_without_order == 114) )
				{
					$adj_allocated_qnty = $before_allocated_qnty - $before_receive_qnty;
					$adj_beforeAvailableQnty = $before_available_qnty;
				}
				else if ( ($variable_set_smn_allocation != 1) && ($before_receive_purpose == 2) && ( $is_without_order == 42 || $is_without_order == 114) )
				{
				...
				if ((str_replace("'", "", $cbo_receive_purpose) == 2 || str_replace("'", "", $cbo_receive_purpose) == 12 || str_replace("'", "", $cbo_receive_purpose) == 15 || str_replace("'", "", $cbo_receive_purpose) == 38 ||str_replace("'", "", $cbo_receive_purpose) == 46 || str_replace("'", "", $cbo_receive_purpose) == 50 || str_replace("'", "", $cbo_receive_purpose) == 51 ) && (str_replace("'", "", $cbo_receive_basis) == 2))
				{
						...
						*/
						...
						else
						{
							...
							*/
							...
							$field_allocation = "id,mst_id,entry_form,item_category,allocation_date,item_id,qnty,is_dyied_yarn,po_break_down_id,booking_no,booking_without_order,qnty_break_down,inserted_by,insert_date";
							$qnty_breakdown = $txt_receive_qty.'_'.$bookingId.'_';
							$data_allocation = "(".$allocation_id.",".$recieve_id.",1,1".",".$txt_receive_date.",".$prodMSTID.",".$txt_receive_qty.",1,'".$bookingId."','".$bookingNo."',1,'".$qnty_breakdown."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
							$allocation_mst_insert = sql_insert("inv_material_allocation_mst", $field_allocation, $data_allocation, 0);
					...
					else
					{
						...
						else
						{
							...
							*/
							$allocation_id = return_next_id_by_sequence("INV_ALLOCATION_MST_PK_SEQ", "inv_material_allocation_mst", $con);
							$field_allocation_dtls = "id,job_no,po_break_down_id,booking_no,item_category,allocation_date,item_id,qnty,is_dyied_yarn,inserted_by,insert_date";
							$data_allocation_dtls = '';
							$qnty_breakdown = '';
							$po_breakdown_id = '';
							$expRcvString = explode(',', $hdnReceiveString);
							foreach($expRcvString as $expRcv)
							{
								...
								$po_breakdown_id .= $rcvData[0];
								$qnty_breakdown .= $rcvData[1].'_'.$rcvData[0].'_'.str_replace("'", "", $job_no);
								$data_allocation_dtls .= "(" . $allocation_dtls_id . "," . $job_no . "," . $rcvData[0] . "," . $txt_wo_pi . ",1," . $txt_receive_date . "," . $prodMSTID . "," . $rcvData[1] . ",1," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
							}
							...
							|
							*/
							$field_allocation = "id,mst_id,entry_form,job_no,po_break_down_id,booking_no,item_category,allocation_date,item_id,qnty,is_dyied_yarn,qnty_break_down,inserted_by,insert_date";
							$data_allocation = "(" . $allocation_id . ",".$id.",1," . $job_no . ",'" . $po_breakdown_id . "'," . $txt_wo_pi . ",1" . "," . $txt_receive_date . "," . $prodMSTID . "," . $txt_receive_qty . ",1,'" . $qnty_breakdown . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
							$allocation_mst_insert = sql_insert("inv_material_allocation_mst", $field_allocation, $data_allocation, 0);
		...
		*/
		...
		if($hdnReceiveString != '')
		{
			...
			foreach($expRcvString as $expRcv)
			{
				...
				//hdnReceiveString += hdnOrderNo+'_'+orderReceiveQty+'_'+receiveQty+'_'+distribiutionMethod;
				$data_proportionate .= "(".$id_prop.",".$update_id.",1,1,".$rcvData[0].",".$prodMSTID.",".$cbo_color.",".$rcvData[1].",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			}
		}
		...
		*/
		...
		if ($db_type == 0)
		{
			if ($rID && $dtlsrID && $prodUpdate && $insertR && $rID_adj && $rID_adjal && $rslt_proportionate && $allocation_mst_insert && $allocation_dtls_insert)
			{
				mysql_query("COMMIT");
	...
	else if ($operation == 2) //Delete Here
	{
		...
		*/
		...
		//echo "10**$rcvID && $prodID && $dtlsrID && $rslt_proportionate && $allocation_mst_update && $allocation_dtls_update";oci_rollback($con);disconnect($con); die;

		if ($db_type == 0)
		{
			if ($rcvID && $prodID && $dtlsrID && $rslt_proportionate && $allocation_mst_update && $allocation_dtls_update)
			{
				mysql_query("COMMIT");
...
if ($action == "populate_data_from_data")
{
	...
	$sql = "select id,recv_number,company_id,receive_basis,receive_purpose,receive_date,booking_id,challan_no,store_id,lc_no,supplier_id,loan_party,exchange_rate,currency_id,lc_no,source, yarn_issue_challan_no, issue_id, remarks, is_audited from inv_receive_master
	...
	foreach ($res as $row) 
	{
		...
		else if ($row[csf("receive_basis")] == 2 && ($row[csf("receive_purpose")] == 2 || $row[csf("receive_purpose")] == 12 || $row[csf("receive_purpose")] == 15 || $row[csf("receive_purpose")] == 38 || $row[csf("receive_purpose")] == 46 || $row[csf("receive_purpose")] == 50 || $row[csf("receive_purpose")] == 51 ))
		{
			...
			{
				echo "$('#txt_receive_qty').removeAttr('onclick','func_onclick_qty()').attr('onblur','fn_calile()');\n";
			}
...
if ($action == "child_form_input_data")
{
	...
	//for order wise qty
	$sql = "SELECT po_breakdown_id, quantity FROM order_wise_pro_details WHERE trans_type = 1 AND status_active = 1 AND is_deleted = 0 AND trans_id = ".$rcv_dtls_id."";
	$rslt = sql_select($sql);
	$orderQtyString = '';
	foreach($rslt as $row)
	{
		...
		$orderQtyString.= $row[csf('po_breakdown_id')]."_".$row[csf('quantity')]."_".$row[csf('quantity')];
	}
...
if ($action == "actn_onclick_qty")
{
	...
	<script>
		function func_onkeyup_total_requisition_qty()
		{
			...
			if(hdnIssueBalanceQty < receiveQty)
			{
				...
				alert("Receive quantity can't exceed issue balance quantity.");
				$('#txt_receive_qnty').val(updateQty);
				var s=1;
				$("#tblReceiveQty tbody tr").each(function(){
					$('#textReceiveQty_'+s).val('');
		...
		function func_close()
		{
			...
			$("#tblReceiveQty tbody tr").each(function(){
				...
				/*
				if(orderReceiveQty*1 > orderPrevReceiveQty*1)
				{
					alert(orderReceiveQty+'='+orderPrevReceiveQty);
					isError = 1;
				}
				*/
				...
				if(orderReceiveQty*1 > 0)
				{
					...
					//hdnReceiveString += hdnOrderNo+'_'+orderReceiveQty+'_'+receiveQty+'_'+distribiutionMethod;
					hdnReceiveString += hdnOrderNo+'_'+orderReceiveQty+'_'+originalOrderRcvQty;
				}
				i++;
			});
    ...
    <body>
		...
		if( str_replace("'","",$cbo_receive_purpose) == 2 )
		{
			$dyeing_color_id_cond = " AND b.dyeing_color_id =$color_id";
		}

		$sql = "
			SELECT
				a.booking_id, c.po_breakdown_id, c.quantity 
			FROM
				inv_issue_master a
        ...
        foreach($sqlRslt as $row)
        {
			...
			$dataIssueArr[$row[csf('booking_id')]][$row[csf('po_breakdown_id')]]['issue_qnty'] += $row[csf('quantity')];
		}
		//echo "<pre>";
		...
		$sqlRcv = "
			SELECT
				a.booking_id, c.trans_id, c.po_breakdown_id, c.quantity 
			FROM
		...
		foreach($dataIssueArr as $bookingId=>$bookingArr)
		{
			...
			{
				$dataArr[$orderId]['issue_qnty'] = $row['issue_qnty'];
				$dataArr[$orderId]['receive_qnty'] = $dataRcvArr[$bookingId][$orderId]['receive_qnty'];
				...
				$hdnIssueBalanceQty += $dataArr[$orderId]['balance_qnty'];
			}
		}
		//echo "<pre>";
		//print_r($dataArr);
                    ...
                    <table id="tblReceiveQty" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
						...
						foreach($dataArr as $orderId=>$row)
						{
							...
							<tr valign="middle">
								...
								<td><?php echo $order_details[$orderId]; ?></td>
								...
								<td>
									...
									<input type="hidden" name="hdnOriginalRcvQty[]" id="hdnOriginalRcvQty_<?php echo $sl; ?>" class="text_boxes_numeric" style="text-align:right" readonly value="<?php echo number_format($row['issue_qnty'], 2, '.', ''); ?>" />
									<input type="hidden" name="hdnReceiveQty[]" id="hdnReceiveQty_<?php echo $sl; ?>" class="text_boxes_numeric" value="<?php echo number_format($row['receive_qnty'], 2, '.', ''); ?>" style="text-align:right" readonly />
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <th><?php echo number_format($hdnIssueBalanceQty, 2, '.', ''); ?></th>
                                <th>
                                    ...
                                    <input type="hidden" name="hdnIssueBalanceQty" id="hdnIssueBalanceQty" class="text_boxes" value="<? echo number_format($hdnIssueBalanceQty, 2, '.', ''); ?>" style="text-align:right" />
                                </th>
                            </tr>
                        </tfoot>                   
                    </table>
