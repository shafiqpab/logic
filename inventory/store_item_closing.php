<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Store Item Year Clossing
...
{
	if( form_validation('cbo_company_id*cbo_year','Company Name*Month')==false )
	{
		return;
	}
	...
	if(confirm_msg)
	{
		var data="action=save_update_delete_process"+get_submitted_data_string('cbo_company_id*cbo_year*cbo_month',"../");
		//alert(data);return;
		freeze_window(3);
		http.open("POST","requires/store_item_closing_controller.php",true);
            ...
            <fieldset style="width:730px;">
                <table class="rpt_table" width="730" cellpadding="0" cellspacing="0" border="1" rules="all">
                    ...
                    <tr class="general">
                        <td>
							<? 
                            	echo create_drop_down( "cbo_company_id", 200, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/store_item_closing_controller',this.value, 'load_drop_down_year', 'year_td' );" );
                            ?>                            
                        </td>
                        <td id="year_td"> 
                            <? 
                        ...
                        <td>
                            <input type="button" name="search" id="search" value="Closing Process Start" onClick="fnc_start_processing()" style="width:180px" class="formbutton" />
                        </td>
                    </tr>
                </table> 
            </fieldset> 
