...
function openmypage_systemId()
{
	...
	var page_link = 'requires/grey_fabric_transfer_v2_controller.php?cbo_company_id='+cbo_company_id+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&action=orderToorderTransfer_popup';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(transfer_id, "populate_data_from_transfer_master", "requires/grey_fabric_transfer_v2_controller" );
		load_drop_down( 'requires/grey_fabric_transfer_v2_controller', $('#txt_from_order_id').val(), 'load_drop_down_item_desc', 'itemDescTd' );

		if ($("#txt_requisition_id").val()=="") 
		{
			//alert('empty req id');
			show_list_view($('#txt_from_order_id').val(),'show_dtls_list_view','list_fabric_desc_container','requires/grey_fabric_transfer_v2_controller','');
		}
		else{
			//alert('req id');
			show_list_view($("#txt_requisition_id").val(),'show_dtls_list_requisition_view','list_fabric_desc_container','requires/grey_fabric_transfer_v2_controller','');
		...
		show_list_view(transfer_id,'show_transfer_listview','div_transfer_item_list','requires/grey_fabric_transfer_v2_controller','');
...
function openmypage_orderNo(type)
{
	...
	var page_link = 'requires/grey_fabric_transfer_v2_controller.php?cbo_company_id='+cbo_company_id+'&type='+type+'&cbo_company_id_to='+cbo_company_id_to+'&action=order_popup';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(order_id+"**"+type+"**"+transfer_criteria, "populate_data_from_order", "requires/grey_fabric_transfer_v2_controller" );
		if(type=='from')
		{
			load_drop_down( 'requires/grey_fabric_transfer_v2_controller', order_id, 'load_drop_down_item_desc', 'itemDescTd' );
			show_list_view(order_id,'show_dtls_list_view','list_fabric_desc_container','requires/grey_fabric_transfer_v2_controller','');
		}
	}
}
...
function fnc_yarn_transfer_entry(operation)
{
	if(operation==4)
	{
		...
		print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title, "grey_fabric_order_to_order_transfer_print", "requires/grey_fabric_transfer_v2_controller" ) 
		return;
	}	
	...
	{		
		...
		if($('#cbo_transfer_criteria').val() ==1 && $('#cbo_company_id').val() == $('#cbo_company_id_to').val())
		{
			alert("Same Companies are not allowed in this Criteria");
			$('#cbo_transfer_criteria').focus();
			return;
		}
		
		if( form_validation('cbo_transfer_criteria*cbo_company_id*txt_transfer_date*txt_from_order_no*cbo_company_id_to*txt_to_order_no*cbo_store_name*cbo_store_name_to*cbo_item_desc*txt_transfer_qnty','Transfer Criteria*Company*Transfer Date*From Order No*To Company*To Order No*From Store*To Store*Item Description*Transfered Qnty')==false )
		{
			return;
		}
		...
		http.open("POST","requires/grey_fabric_transfer_v2_controller.php",true);
...
{	
	...
	{	  		
		...
		if(reponse[0]==0 || reponse[0]==1)
		{
			...
			{
				//alert('empty req id');
				show_list_view(reponse[1],'show_transfer_listview','div_transfer_item_list','requires/grey_fabric_transfer_v2_controller','');
				show_list_view($('#txt_from_order_id').val(),'show_dtls_list_view','list_fabric_desc_container','requires/grey_fabric_transfer_v2_controller','');
			}
			else{
				//alert('req id');
...
function reset_form_all()
{
	...
	$("#div_transfer_item_list").html("");

	reset_form('','','update_dtls_id*cbo_item_desc*txt_transfer_qnty*txt_roll*txt_ycount*hid_ycount*txt_ybrand*hid_ybrand*txt_ylot*stitch_length*cbo_store_name*cbo_floor*cbo_room*txt_rack*txt_shelf*cbo_store_name_to*cbo_floor_to*cbo_room_to*txt_rack_to*txt_shelf_to*update_trans_issue_id*update_trans_recv_id*previous_from_prod_id*previous_to_prod_id*txt_form_prog*txt_to_prog*txt_stock*hide_trans_qty*hidden_requ_dtls_id*txt_from_order_no*txt_from_po_qnty*cbo_from_buyer_name*txt_from_style_ref*txt_from_job_no*txt_from_gmts_item*txt_from_shipment_date','','','');
}
...
function openmypage_orderInfo(type)
{
	...
	var title = 'Order Info';	
	var page_link = 'requires/grey_fabric_transfer_v2_controller.php?txt_order_no='+txt_order_no+'&txt_order_id='+txt_order_id+'&type='+type+'&action=orderInfo_popup';
...
function populate_stock()
{
	...
	get_php_form_data(txt_order_id+"**"+prod_id+"**"+program_no+"**"+company_id+"**"+store_id, "populate_data_about_order", "requires/grey_fabric_transfer_v2_controller" );
}
...
function fnc_company_onchang(company_id)
{
	...
	$.ajax({
		url: 'requires/grey_fabric_transfer_v2_controller.php',
		type: 'POST',
		data: page_link,
...
function openmypage_requisition_no()
{
	...
	var page_link = 'requires/grey_fabric_transfer_v2_controller.php?cbo_company_id='+cbo_company_id+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&action=itemTransfer_requisition_popup';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(transfer_id, "populate_data_from_transfer_requisition_master", "requires/grey_fabric_transfer_v2_controller" );
		load_drop_down( 'requires/grey_fabric_transfer_v2_controller', $('#txt_from_order_id').val(), 'load_drop_down_item_desc', 'itemDescTd' );
		//show_list_view($('#txt_from_order_id').val(),'show_dtls_list_view','list_fabric_desc_container','requires/grey_fabric_transfer_v2_controller','');
		show_list_view(transfer_id,'show_dtls_list_requisition_view','list_fabric_desc_container','requires/grey_fabric_transfer_v2_controller','');
		disable_enable_fields( 'cbo_transfer_criteria*cbo_company_id*txt_from_order_no*txt_to_order_no*cbo_company_id_to', 1, '', '' );
		//set_button_status(0, permission, 'fnc_yarn_transfer_entry',1,1);
	}
}
