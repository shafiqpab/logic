...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		*/
		...
		$data_array="(".$id.",'".$new_mrr_number[1]."',".$new_mrr_number[2].",'".$new_mrr_number[0]."',2,".$cbo_location.",".$cbo_store_name.",61,13,".$cbo_company_id.",".$txt_fso_no.",".$hdn_fso_id.",".$txt_issue_date.",".$cbo_party.",".$txt_booking_no.",".$hdn_booking_id.",".$txt_remarks.",".$txt_delivery_addr.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		for($j=1;$j<=$tot_row;$j++)
		{
			...
			*/
			...
			$data_array_roll.="(".$id_roll.",'".$$barcodeNo."',".$id.",".$dtls_id.",'".$$orderId."',61,'".$$rollWgt."','".$cons_rate."','".$cons_amount."','".$$rollNo."','".$$rollId."','".$$bookWithoutOrder."','".$$smnBooking."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$$isSalesOrder.",".$$hiddenQtyInPcs.",".$is_service.",".$fso_delivery_type.")";
		...
		*/
		...
		|
		*/
		$field_array="id,issue_number_prefix,issue_number_prefix_num,issue_number,issue_basis,location_id,store_id,entry_form,item_category,company_id,fso_no,fso_id,issue_date,supplier_id,booking_no,booking_id,remarks,delivery_addr,inserted_by,insert_date";
		...
		*/
		...
		|
		*/
		$field_array_roll="id,barcode_no,mst_id,dtls_id, po_breakdown_id,entry_form,qnty,rate,amount,roll_no,roll_id,booking_without_order,booking_no,inserted_by,insert_date,is_sales,qc_pass_qnty_pcs,is_service,fso_delivery_type";
	...
	else if ($operation==1)   // Update Here
	{
        ...
        for($j=1;$j<=$tot_row;$j++)
        {
        	...
        	else
        	{
				...
				*/
				...
				$fso_delivery_type=1;
				$id_roll = return_next_id_by_sequence("PRO_ROLL_DTLS_PK_SEQ", "pro_roll_details", $con);
				$data_array_roll[$id_roll] ="(".$id_roll.",'".$$barcodeNo."',".$update_id.",".$dtls_id.",'".$$orderId."',61,'".$$rollWgt."','".$cons_rate."','".$cons_amount."','".$$rollNo."','".$$rollId."','".$$bookWithoutOrder."','".$$smnBooking."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$$isSalesOrder.",".$$hiddenQtyInPcs.",".$is_service.",".$fso_delivery_type.")";
		...
		*/
		...
		|
		*/
		$field_array="location_id*issue_date*remarks*delivery_addr*store_id*updated_by*update_date";
		$data_array=$cbo_location."*".$txt_issue_date."*".$txt_remarks."*".$txt_delivery_addr."*".$cbo_store_name."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
			...
			<fieldset style="width:760px; margin-left:2px">
				<table cellpadding="0" cellspacing="0" width="750" border="1" rules="all" class="rpt_table">
					...
					<tr class="general">
						...
						<td align="center">
							<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $cbo_company_id; ?>, 'create_challan_search_list_view', 'search_div', 'roll_wise_grey_fabric_delivery_party_sales_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
						</td>
					</tr>
					<tr>
...
{
	$sql = "select id, company_id, issue_number, location_id, store_id, fso_no, fso_id, supplier_id, booking_no, booking_id, issue_date, remarks, delivery_addr from inv_issue_master where id=$data and entry_form=61";
	//echo $sql;
	$res = sql_select($sql);
	foreach($res as $row)
	{
		...
		echo "load_drop_down('requires/roll_wise_grey_fabric_delivery_party_sales_controller', '".$row[csf('location_id')]."_".$row[csf('company_id')]."', 'load_drop_down_store','store_td');\n";
		echo "document.getElementById('cbo_store_name').value 				= '".$row[csf("store_id")]."';\n";
		echo "$('#cbo_store_name').attr('disabled','true')".";\n";		
		
		//echo "load_drop_down( 'requires/roll_wise_grey_fabric_delivery_party_sales_controller', ".$row[csf("knit_dye_source")]."+'**'+".$row[csf("company_id")].", 'load_drop_down_company', 'dyeing_company_td' );\n";
		...
		echo "$('#txt_delivery_addr').val('".$row[csf("delivery_addr")]."');\n";
...
if($action=="create_barcode_search_list_view")
{
	...
	if ($sales_order_no != '' || $is_sales == 'true')
	{
		...
		where a.id=b.mst_id and b.id=c.dtls_id and c.po_breakdown_id=d.id and a.company_id=$company_id and b.trans_id<>0 and a.entry_form in(2,22,58) and c.entry_form in(2,22,58) and c.status_active=1 and c.is_deleted=0 and c.re_transfer=0 and c.roll_no>0  $booking_no_cond $job_no_cond $sales_order_cond $location_cond $barcode_cond $trans_id_cond2 $store_cond_rcv  and c.fso_delivery_type=1
	...
	else
	{
		...
		if($booking_no!="" || $barcode_cond != "" || $trans_id!="")
		{
			...
			and c.booking_without_order=1 $barcode_cond $non_order_booking $location_cond $trans_id_cond2 $store_cond_rcv  and c.fso_delivery_type=1
...
if($action=="populate_barcode_data")
{
	...
	$scanned_barcode_data=sql_select("SELECT a.barcode_no, a.entry_form, a.mst_id, a.dtls_id, a.is_returned, a.po_breakdown_id, a.re_transfer, a.booking_without_order
	from pro_roll_details a
	where a.barcode_no in($barcode) and a.entry_form =61 and a.is_returned=0 and a.status_active=1 and a.is_deleted=0");
	...
	$data_array_receive_basis = sql_select("select a.id, a.company_id, a.recv_number, a.booking_no, a.receive_basis, a.receive_date, a.booking_no, a.knitting_source, a.knitting_company, c.roll_no, c.barcode_no, c.id as roll_id, c.roll_no, c.po_breakdown_id, c.qnty, c.qc_pass_qnty, c.coller_cuff_size
		...
		where c.barcode_no in($barcode) and c.entry_form=2 and c.status_active=1 and c.is_deleted=0 and a.entry_form=2 and a.receive_basis in(1,2) and a.status_active=1 and a.is_deleted=0 and a.id=c.mst_id and c.fso_delivery_type=1");
...
if($action=="populate_barcode_data_update")
{
	...
	FROM inv_receive_master a, pro_grey_prod_entry_dtls b, pro_roll_details c, fabric_sales_order_mst d
	WHERE a.id=b.mst_id and b.id=c.dtls_id and c.po_breakdown_id=d.id and b.trans_id<>0 and a.entry_form in(2,22,58) and c.entry_form in(2,22,58) and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and c.fso_delivery_type=1  $all_barcode_nos_cond"); //and c.barcode_no in($barcode_nos)
    ...
    <div style="width:1010px;">
		...
		<table width="1250" cellspacing="0" align="center" border="0">
			<tr>
                ...
                <td style="font-size:16px; font-weight:bold;" width="330">Delivery Challan No.</td>
			...
			<tr>
                ...
                <td style="font-size:16px; font-weight:bold;" width="200">Delivery Date</td>
                <td width="200" align="left">:&nbsp;<? echo change_date_format($sql_data[0][csf('issue_date')]); ?></td>
                <td width="190"></td>
                <td width="200">&nbsp;</td>

                <td width="390" id="barcode_img_id"></td>
			</tr>
			<tr>
				<td width="">&nbsp;</td>
		    </tr>
        ...
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1250" class="rpt_table" >
			...
			b.body_part_id, b.trans_id, b.prod_id, d.gsm, d.dia_width as dia, d.detarmination_id as deter_id, b.color_id, a.qnty ,a.reject_qnty, c.job_no, c.id as fso_id, c.sales_booking_no, c.booking_id, c.buyer_id, c.po_buyer, c.po_job_no, c.po_company_id, c.within_group, c.style_ref_no, d.unit_of_measure as uom, null as recv_number, c.customer_buyer 
			...
			foreach($sql_update as $row)
			{
				...
				$data_grouping[$row[csf("fso_id")]][$row[csf("deter_id")]][$row[csf("dia")]][$row[csf("gsm")]]['customer_buyer']=$row[csf("customer_buyer")];
			...
			?>
            <tr> 
                <td align="left" colspan="3"><strong>Barcode No - </strong><? echo chop($barcode_NOs,", "); ?></td>
