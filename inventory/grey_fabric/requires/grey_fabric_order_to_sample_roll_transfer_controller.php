...
if($action=='populate_data_from_order')
{
	...
	{ 
		...
		/*echo "document.getElementById('txt_from_order_id').value 			= ".$data.";\n";
		echo "document.getElementById('txt_from_order_no').value 			= '".$row[csf("po_number")]."';\n";
		echo "document.getElementById('txt_from_po_qnty').value 			= '".$row[csf("po_quantity")]."';\n";
		...
		echo "document.getElementById('txt_form_shipment_date').value 		= '".change_date_format($row[csf("shipment_date")])."';\n";*/
        ...
        <legend>Enter search words</legend>
            <table cellpadding="0" cellspacing="0" width="800" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td>
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_sample_no').value+'_'+<? echo $cbo_company_id; ?>+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value, 'create_sample_search_list_view', 'search_div', 'grey_fabric_order_to_sample_roll_transfer_controller', 'setFilterGrid(\'tbl_list_sample\',-1);')" style="width:100px;" />
                    </td>
                </tr>
                <tr>
...
if($action=="populate_data_from_sample")
{
	...
	from wo_non_ord_samp_booking_mst a, wo_non_ord_samp_booking_dtls b  
	where a.booking_no=b.booking_no and b.id=$data and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
	...
	{ 
		...
		echo "document.getElementById('txt_sam_book_dtls_id').value 		= '".$row[csf("booking_dtls_id")]."';\n";
...
if($action=="show_dtls_list_view")
{
	...
	WHERE a.id=b.mst_id and b.id=c.dtls_id and b.trans_id<>0 and a.entry_form in(2,22,58) and c.entry_form in(2,22,58) and c.re_transfer=0 and c.status_active=1 and c.is_deleted=0 and c.po_breakdown_id=$data
	union all
	 select a.id, a.entry_form, 0 as receive_basis, 0 as booking_id, b.from_prod_id as prod_id, b.yarn_lot, b.y_count as yarn_count, b.stitch_length, b.brand_id, b.rack, b.shelf as self, c.id as roll_id, c.barcode_no, c.po_breakdown_id, c.roll_no, c.qnty, c.roll_id as roll_id_prev, 2 as type, b.to_store as store_id
	from inv_item_transfer_mst a, inv_item_transfer_dtls b, pro_roll_details c 
	WHERE a.id=b.mst_id and b.id=c.dtls_id  and c.entry_form in(82,83,183) and c.re_transfer=0 and c.status_active=1 and c.is_deleted=0 and c.po_breakdown_id=$data";
	
	//echo $sql;
	
	//echo $sql;
	/*$sql="select a.id, a.entry_form, a.receive_basis, a.booking_id, b.prod_id, b.yarn_lot, b.yarn_count, b.stitch_length, b.brand_id, b.rack, b.self, c.id as roll_id, c.barcode_no, c.po_breakdown_id, c.roll_no, c.qnty, c.roll_id as roll_id_prev, 1 as type
	from inv_receive_master a, pro_grey_prod_entry_dtls b, pro_roll_details c 
	WHERE a.id=b.mst_id and b.id=c.dtls_id and b.trans_id<>0 and a.entry_form in(2,22,58) and c.entry_form in(2,22,58) and c.is_transfer!=6 and c.status_active=1 and c.is_deleted=0 and c.po_breakdown_id=$data order by c.barcode_no";*/
	//echo $sql;
	...
	//$barcodeData = sql_select("select barcode_no,entry_form from pro_roll_details where entry_form=61 and status_active=1 and is_deleted=0 and po_breakdown_id=$data and is_returned = 0");
...
if($action=="show_transfer_listview")
{
	...
	$re_trans_arr=return_library_array("select barcode_no from pro_roll_details where entry_form=110 and re_transfer=1 and status_active=1 and is_deleted=0 and mst_id=$mst_id ", "barcode_no", "barcode_no");
	
	$transfer_arr=array();
	$transfer_dataArray=sql_select("select a.id, a.trans_id, a.to_trans_id, b.id as roll_id, b.barcode_no from inv_item_transfer_dtls a, pro_roll_details b where a.id=b.dtls_id and a.mst_id=$mst_id and b.mst_id=$mst_id and b.entry_form=110  and b.status_active=1 and b.is_deleted=0");
	//and b.transfer_criteria=6
	...
	if(!empty($barcode_arr))
	{
		...
		{
			$all_barcode_no_cond=" and barcode_no in($all_barcode_no)";	 
		}

		$issued_barcode_arr=return_library_array("select barcode_no from pro_roll_details where entry_form=61 and status_active=1 and is_deleted=0 $all_barcode_no_cond and is_returned = 0","barcode_no", "barcode_no");
	}
	...
	from inv_item_transfer_mst a, inv_item_transfer_dtls b, pro_roll_details c 
	WHERE a.id=b.mst_id and b.id=c.dtls_id and a.entry_form in(110) and c.entry_form in(110) and c.status_active=1 and c.is_deleted=0 and c.mst_id=$mst_id
	order by barcode_no";
	...
	{  
		...
		//{
			...
			if($re_trans_arr[$row[csf('barcode_no')]]!="" || $issued_barcode_arr[$row[csf('barcode_no')]]!="")
			{
				$disabled="disabled";
			}
			else {$disabled=""; 
        ...
        <legend>Enter search words</legend>
            <table cellpadding="0" cellspacing="0" width="550" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td>
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+<? echo $cbo_company_id; ?>, 'create_transfer_search_list_view', 'search_div', 'grey_fabric_order_to_sample_roll_transfer_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                    </td>
                </tr>
            </table>
        	<div style="margin-top:10px" id="search_div"></div> 
...
if($action=='populate_data_from_transfer_master')
{
	...
	{ 
		...
		//echo "get_php_form_data('".$row[csf("from_order_id")]."','populate_data_from_order','requires/grey_fabric_order_to_sample_roll_transfer_controller');\n";
		echo "get_php_form_data('".$row[csf("from_order_id")]."**from'".",'populate_data_from_order','requires/grey_fabric_order_to_sample_roll_transfer_controller');\n";
		echo "get_php_form_data('".$row[csf("to_samp_dtls_id")]."','populate_data_from_sample','requires/grey_fabric_order_to_sample_roll_transfer_controller');\n";
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$update_id)=="")
		{
			...
			/*$new_transfer_system_id=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '', 'GFOTSTE', date("Y",time()), 5, "select transfer_prefix, transfer_prefix_number from inv_item_transfer_mst where company_id=$cbo_company_id and entry_form=110 and transfer_criteria=6 and item_category=13 and $year_cond=".date('Y',time())." order by id desc ", "transfer_prefix", "transfer_prefix_number" ));
			...
			$new_transfer_system_id = explode("*", return_next_id_by_sequence("INV_ITEM_TRANSFER_MST_PK_SEQ", "inv_item_transfer_mst",$con,1,$cbo_company_id,"GFOTSTE",110,date("Y",time()),13 ));
					 
			$field_array="id, transfer_prefix, transfer_prefix_number, transfer_system_id, company_id, challan_no, transfer_date, entry_form, transfer_criteria, to_company, from_order_id, to_order_id, to_samp_dtls_id, item_category, inserted_by, insert_date"; 
			
			$data_array="(".$id.",'".$new_transfer_system_id[1]."',".$new_transfer_system_id[2].",'".$new_transfer_system_id[0]."',".$cbo_company_id.",".$txt_challan_no.",".$txt_transfer_date.",110,6,0,".$txt_from_order_id.",".$txt_sam_book_id.",".$txt_sam_book_dtls_id.",13,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		{
			$field_array_update="challan_no*transfer_date*from_order_id*to_order_id*to_samp_dtls_id*updated_by*update_date";
			$data_array_update=$txt_challan_no."*".$txt_transfer_date."*".$txt_from_order_id."*".$txt_sam_book_id."*".$txt_sam_book_dtls_id."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		{ 	
			...
			//$id_trans=$id_trans+1;
			$id_trans = return_next_id_by_sequence("INV_TRANSACTION_PK_SEQ", "inv_transaction", $con);
			$data_array_trans.=",(".$id_trans.",".$transfer_update_id.",".$cbo_company_id.",".$$productId.",13,5,".$txt_transfer_date.",".$txt_sam_book_id.",12,".$$rollWgt.",".$rate.",".$amount.",".$$rack.",".$$shelf.",".$$progId.",".$$stichLn.",".$$storeId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			...
			$data_array_roll.="(".$id_roll.",".$$barcodeNo.",".$transfer_update_id.",".$id_dtls.",".$txt_sam_book_id.",110,".$$rollWgt.",".$$rollNo.",".$$rollId.",".$$transRollId.",5,6,1,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			
			if($data_array_prop!="") $data_array_prop.= ",";
			$data_array_prop.="(".$id_prop.",".$from_trans_id.",6,110,".$id_dtls.",".$txt_from_order_id.",".$$productId.",".$$rollWgt.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			//$id_prop=$id_prop+1;
			
			//$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);	
			//$data_array_prop.=",(".$id_prop.",".$id_trans.",5,110,".$id_dtls.",".$txt_to_order_id.",".$$productId.",".$$rollWgt.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		{ 	
			...
			if(str_replace("'","",$$rolltableId)>0)
			{
				...
				$data_array_update_trans[str_replace("'","",$$transIdTo)]=explode("*",($$productId."*".$txt_transfer_date."*".$txt_sam_book_id."*".$$rollWgt."*".$rate."*".$amount."*".$$rack."*".$$shelf."*".$$progId."*".$$stichLn."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
			...
			else
			{
				...
				$data_array_trans.=",(".$id_trans.",".$update_id.",".$cbo_company_id.",".$$productId.",13,5,".$txt_transfer_date.",".$txt_sam_book_id.",12,".$$rollWgt.",".$rate.",".$amount.",".$$rack.",".$$shelf.",".$$progId.",".$$stichLn.",".$$storeId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				...
				if($data_array_roll!="") $data_array_roll.= ",";
				$data_array_roll.="(".$id_roll.",".$$barcodeNo.",".$update_id.",".$id_dtls.",".$txt_sam_book_id.",110,".$$rollWgt.",".$$rollNo.",".$$rollId.",".$$transRollId.",5,6,1,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			...
			$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
			if($data_array_prop!="") $data_array_prop.= ",";
			$data_array_prop.="(".$id_prop.",".$transIdfromProp.",6,110,".$dtlsIdProp.",".$txt_from_order_id.",".$$productId.",".$$rollWgt.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			//$id_prop=$id_prop+1;
			//$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
			//$data_array_prop.=",(".$id_prop.",".$transIdtoProp.",5,110,".$dtlsIdProp.",".$txt_to_order_id.",".$$productId.",".$$rollWgt.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			//$id_prop=$id_prop+1;
		}
		...
		if($txt_deleted_id!="")
		{
			...
			if($delBarcodeNo != "")
			{
				...
				union all 
				select a.barcode_no , b.transfer_system_id as system_no, a.entry_form, 'Transfer' as msg_source from pro_roll_details a, inv_item_transfer_mst b where a.mst_id = b.id and a.entry_form = 110 and b.entry_form = 110 and a.re_transfer = 0 and  a.status_active=1 and  b.status_active=1 and a.barcode_no in ($delBarcodeNo) and a.id not in ($rollIDDel) ");
				...
				$child_split_sql=sql_select("select barcode_no, id from pro_roll_details where roll_split_from >0 and barcode_no in ($delBarcodeNo) and entry_form = 110 order by barcode_no");
		...
		*/
		...
		if($update_dtls_id!="")
		{
			$query = execute_query("DELETE FROM order_wise_pro_details WHERE dtls_id in(".$update_dtls_id.") and entry_form=110");
			if($flag==1) 
			{
...
<div style="width:930px;">
    ...
    <div style="width:100%;">
        <table align="right" cellspacing="0" width="900"  border="1" rules="all" class="rpt_table" >
            ...
            <tbody> 
			<?
            $sql_dtls="select a.from_prod_id, a.transfer_qnty, a.uom, a.y_count, a.brand_id, a.yarn_lot, a.to_rack, a.to_shelf, a.stitch_length, b.barcode_no, b.roll_no from inv_item_transfer_dtls a, pro_roll_details b where a.id=b.dtls_id and b.entry_form=110 and a.mst_id='$data[1]' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by b.barcode_no";
