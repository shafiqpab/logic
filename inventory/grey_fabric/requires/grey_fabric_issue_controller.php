...
if ($action=="load_drop_down_knit_com")
{
	...
	{
		echo create_drop_down( "cbo_dyeing_com", 170, $blank_array,"", 1, "-- Select --", 0, "",0 );
	}
	else if($knit_source==1)
	{
		echo create_drop_down( "cbo_dyeing_com", 170, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select --", $company, "" );
	}
	else if($knit_source==3 && ($issuePurpose==4 || $issuePurpose==8 || $issuePurpose==11))
	{
		echo create_drop_down( "cbo_dyeing_com", 170, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b, lib_supplier_tag_company c where a.id=b.supplier_id and b.supplier_id=c.supplier_id and b.party_type in(21,24,25,26) and a.status_active=1 $company_cond2 group by a.id,a.supplier_name order by a.supplier_name","id,supplier_name", 1, "-- Select --", 0, "",0 );
	}
	else
	{
...
if($action=="load_drop_down_knit_com_new")
{
	...
	{
		$sql="select id,company_name as supplier_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name";
	}
	else if($knit_source==3 && ($issuePurpose==4 || $issuePurpose==8 || $issuePurpose==11))
	{  
		$sql="SELECT DISTINCT c.supplier_name,c.id from lib_supplier_tag_company a,lib_supplier_party_type b, lib_supplier c where c.id=b.supplier_id and a.supplier_id = b.supplier_id and a.tag_company='$company' and b.party_type in(21,24,25,26) and c.status_active IN(1) and c.is_deleted=0 group by c.id, c.supplier_name   UNION ALL  SELECT DISTINCT c.supplier_name,c.id from lib_supplier_tag_company a,lib_supplier_party_type b, lib_supplier c, inv_issue_master d where c.id=b.supplier_id and a.supplier_id = b.supplier_id and c.id=d.supplier_id and a.tag_company='$company'  and b.party_type  in(21,24,25,26) and c.status_active IN(1,3) and c.is_deleted=0 group by c.id, c.supplier_name    order by supplier_name";
	}
	else
	{
		$sql="SELECT DISTINCT a.id, a.supplier_name FROM lib_supplier a, lib_supplier_party_type b, lib_supplier_tag_company c  WHERE   a.id = b.supplier_id AND b.supplier_id = c.supplier_id AND a.status_active = 1 AND c.tag_company = $company GROUP BY a.id, a.supplier_name UNION ALL  SELECT DISTINCT c.id, c.supplier_name FROM lib_supplier_tag_company a, lib_supplier_party_type b, lib_supplier  c, inv_issue_master  d  WHERE   c.id = b.supplier_id AND a.supplier_id = b.supplier_id AND c.id = d.supplier_id	 AND a.tag_company = $company AND c.status_active IN (1, 3) AND c.is_deleted = 0 GROUP BY c.id, c.supplier_name ORDER BY supplier_name";
	}

	echo create_drop_down( "cbo_dyeing_com", 170, $sql,"id,supplier_name", 1, "-- Select --", $company, "" );
	exit();
}
	/*if ($action=="load_room_rack_self_bin")
	{
		load_room_rack_self_bin("requires/grey_fabric_issue_controller",$data);
	}*/

//load drop down store
	if ($action=="load_drop_down_store")
	{
		echo create_drop_down( "cbo_store_name", 170, "select a.id, a.store_name from lib_store_location a, lib_store_location_category b where a.id= b.store_location_id and a.company_id='$data' and a.status_active=1 and a.is_deleted=0 and b.category_type in(13,14) group by a.id, a.store_name order by a.store_name","id,store_name", 1, "-- Select Store --", 0, "reset_product_info()",0 );
		exit();
	}
	/*if ($action=="load_drop_down_store")
	{
	...
	if ($action=="load_drop_down_color")
	{
		...
		if($issue_purpose==8)
		{
			/*echo " select entry_form_id from wo_non_ord_samp_booking_mst where booking_no='$all_po_id_or_booking_no' and status_active=1 and is_deleted=0";die;*/
			$entry_form_id=return_field_value("entry_form_id", "wo_non_ord_samp_booking_mst","booking_no='$all_po_id_or_booking_no' and status_active=1 and is_deleted=0");
			if($entry_form_id==140)
			{
				$sql=" SELECT c.id, c.color_name from wo_non_ord_samp_booking_dtls b, lib_color c,sample_development_rf_color d where d.fabric_color=c.id and b.dtls_id=d.dtls_id and b.booking_no='$all_po_id_or_booking_no' and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by c.id, c.color_name";
			}
			else if($is_sales==2 && $basis==3)
			{
				$sql="SELECT d.id, d.color_name from ppl_planning_info_entry_mst a, ppl_planning_info_entry_dtls b, wo_non_ord_samp_booking_dtls c, lib_color d
				where a.id=b.mst_id and a.booking_no=c.booking_no and c.fabric_color=d.id and b.id='$all_po_id_or_booking_no' group by d.id, d.color_name";
			}
			else
			{
				$sql="SELECT c.id, c.color_name from wo_non_ord_samp_booking_dtls b, lib_color c where b.fabric_color=c.id and b.booking_no='$all_po_id_or_booking_no' and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by c.id, c.color_name";
			}
		}
		else if($is_sales==1 && $basis==3)
		{

			$sql="SELECT c.id, c.color_name from fabric_sales_order_dtls a, lib_color c where a.color_id=c.id and a.mst_id in($hidden_order_id) and  a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by c.id, c.color_name";
		}
		else if($is_sales==0 && $basis==3)
		{

			$sql="SELECT c.id, c.color_name from wo_booking_mst a, wo_booking_dtls b, lib_color c where a.booking_no=b.booking_no and b.fabric_color_id=c.id and b.po_break_down_id in($hidden_order_id) and a.item_category=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by c.id, c.color_name";
		}
		else
		{
			$sql="SELECT c.id, c.color_name from wo_booking_mst a, wo_booking_dtls b, lib_color c 
			where a.booking_no=b.booking_no and b.fabric_color_id=c.id and b.po_break_down_id in($hidden_order_id) and a.item_category=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by c.id, c.color_name";
		}
		// echo $sql;
	...
	if ($action=="is_roll_maintain")
	{
		...
		echo "$('#store_update_upto').val(".$store_maintain.");\n";
		exit();
	}
	...
	{
		$buttonIdArr = ['109#print_1', '66#print_2', '85#print_3', '137#print_4', '95#print_vat', '909#print_sb', '863#multiple_issue_no_print'];
		$print_report_format_arr = get_report_button_array($data, 6, 33, $user_id,$buttonIdArr);
	}
...
if($action=="create_batch_search_list")
{
 ...
 if($action=="job_no_for_duplication_check")
 {
	...
	$job_sql=sql_select("SELECT b.job_no_mst as job_no from inv_grey_fabric_issue_dtls i, order_wise_pro_details a, wo_po_break_down b 
	where i.id=a.dtls_id and a.po_breakdown_id=b.id and i.mst_id=$update_id and a.entry_form=16 and a.trans_type=2 and a.status_active=1 and a.is_deleted=0 $dtls_id_cond and a.is_sales!=1 union all select b.job_no as job_no from inv_grey_fabric_issue_dtls i, order_wise_pro_details a, fabric_sales_order_mst b where i.id=a.dtls_id and a.po_breakdown_id=b.id and i.mst_id=$update_id and a.entry_form=16 and a.trans_type=2 and a.status_active=1 and a.is_deleted=0 $dtls_id_cond and a.is_sales=1");
	//echo "SELECT b.job_no_mst as job_no from inv_grey_fabric_issue_dtls i, order_wise_pro_details a, wo_po_break_down b where i.id=a.dtls_id and a.po_breakdown_id=b.id and i.mst_id=$update_id and a.entry_form=16 and a.trans_type=2 and a.status_active=1 and a.is_deleted=0 $dtls_id_cond and a.is_sales!=1 union all select b.job_no as job_no from inv_grey_fabric_issue_dtls i, order_wise_pro_details a, fabric_sales_order_mst b where i.id=a.dtls_id and a.po_breakdown_id=b.id and i.mst_id=$update_id and a.entry_form=16 and a.trans_type=2 and a.status_active=1 and a.is_deleted=0 $dtls_id_cond and a.is_sales=1";
 			...
 			if(check_or_not==2)
 			{
 				if(update_id!="")
 				{
 					...
 					if(job_no!="")
 					{
 						...
 						{
 							alert("Job/Sales Order Mix Not Allowed");
 							return;
 						}
 					}
 				}
			...
			<table width="850" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
				...
				<tbody>
					<tr align="center">
						<td>
							<?
							echo create_drop_down( "cbo_buyer_name", 150, "select buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$company' $buyer_cond and buy.id in (select buyer_id from lib_buyer_party_type where party_type in (1,3,21,30,90)) order by buy.buyer_name","id,buyer_name", 1, "-- Select --", $cbo_buyer_name, "" );
							?>
						</td>
						<td>
							<?
							...
							echo create_drop_down( "cbo_search_by", 130, $search_by, "", 0, "-- --", $selected, $dd, 0);
							?>
						</td>
						<td width="180" align="center" id="search_by_td">
						...
						<td align="center">
							...
							echo create_drop_down("cbo_is_sales", 70, $sales_yes_no, "", 0, "--Select--", $hidden_is_sales, "");
							?>
						</td>
						<td align="center">
...
if($action=="create_fabbook_search_view")
{
	...
	if($cbo_basis==3)
	{
		$sql_job_mixing = sql_select("select allocation from variable_settings_inventory where variable_list=40 and company_name=$company and status_active=1 and is_deleted=0");
		$var_job_mixing = $sql_job_mixing[0][csf("allocation")] * 1;
		...
		{
			$sql = "SELECT a.id, a.within_group, a.booking_no, a.buyer_id, a.body_part_id, a.determination_id, a.fabric_desc, a.gsm_weight, a.dia, b.color_range, b.id as knit_id, b.knitting_source, b.knitting_party, b.stitch_length, b.program_date, a.is_sales, 
			c.po_id
			...
			a.is_sales,c.po_id, a.within_group, a.booking_no, a.buyer_id
			union all
			select a.id, a.within_group, a.booking_no, a.buyer_id, a.body_part_id, a.determination_id, a.fabric_desc, a.gsm_weight, a.dia, b.color_range, b.id as knit_id, b.knitting_source, b.knitting_party, b.stitch_length, 
			...
			a.is_sales, c.to_order_id, a.within_group, a.booking_no, a.buyer_id";
			//echo $sql;die;
		}
		else if ($is_sales==1)
		{
			...
			where a.id=b.mst_id and b.id=c.dtls_id and a.company_id=$company and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.is_sales=$is_sales and c.po_id=e.id and ((e.within_group=1 and e.po_buyer like '$buyer') or (e.within_group=2 and e.buyer_id like '$buyer')) $program_cond $po_no_cond $sales_order_and_job_cond
			group by b.id, a.id, a.body_part_id, a.determination_id, a.fabric_desc, a.gsm_weight, a.dia, b.color_range,b.color_id, b.knitting_source, b.knitting_party, b.stitch_length, b.program_date,
			a.is_sales,c.po_id, a.within_group,a.booking_no, a.buyer_id
			union all
			select a.id, a.within_group, a.booking_no,a.buyer_id, a.body_part_id, a.determination_id, a.fabric_desc, a.gsm_weight, a.dia, b.color_range, b.id as knit_id, b.knitting_source, b.knitting_party, b.stitch_length, 
			b.program_date, a.is_sales, c.to_order_id as po_id
			from ppl_planning_info_entry_mst a, ppl_planning_info_entry_dtls b, inv_item_transfer_mst c, inv_item_transfer_dtls d, fabric_sales_order_mst e
			...
			and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 
			and c.to_order_id=e.id and ((e.within_group=1 and e.po_buyer like '$buyer') or (e.within_group=2 and e.buyer_id like '$buyer'))
			
			and a.buyer_id like '$buyer' $to_program_cond $trans_in_po_no_cond $trans_in_order_and_job_cond
			group by a.id, a.body_part_id, a.determination_id, a.fabric_desc, a.gsm_weight, a.dia, b.color_range, b.id, b.knitting_source, b.knitting_party, b.stitch_length, b.program_date, 
			a.is_sales, c.to_order_id, a.within_group, a.booking_no,a.buyer_id"; //  and d.to_program in($txt_search_common)
			//echo $sql;die;
		}
		else
		{
		...
		<div style="width:1190px; max-height:280px; overflow-y:scroll" id="container_batch" align="left">
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1172" class="rpt_table" id="view">
				<?
				$i=1;
				foreach ($result as $row)
				{
					...
					$yarn_lot_no=$rqsn_array[$row[csf('knit_id')]]['lot'];
					...
					?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $row[csf('knit_id')]; ?>__<? echo $row[csf('knit_id')]; ?>__<? echo $buyerID; ?>__<? echo $style_ref_no; ?>__<? echo $po_NO; ?>__<? echo $po_no_id; ?>__<? echo $row[csf('is_sales')]; ?>__<? echo $row[csf('color_id')]; ?>','<? echo $jobNo; ?>',<? echo $var_job_mixing;?>);">
						...
						<td width="100"><p><? echo $yarn_lot_no; ?></p></td>
...
if ($action=="roll_popup")
{
	...
	<script>
		...
		{
			show_list_view ( document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_search_common').value+'_'+'<? echo $concate; ?>', 'create_roll_search_list', 'search_div', 'grey_fabric_issue_controller', 'setFilterGrid(\'view\',-1)');
			set_previous_data();
		}
...
if($action=="populate_data_about_order")
{
	...
	if ($receive_basis == 3)
	{
		...
		if($fabric_store_auto_update==1)
		{
			...
			from inv_receive_master b, pro_grey_prod_entry_dtls c, order_wise_pro_details d, inv_transaction e
			where b.id=c.mst_id and c.id=d.dtls_id and c.trans_id>0 and d.trans_id>0 and b.entry_form=2 and d.entry_form=2 and b.receive_basis=2 and b.status_active=1 and c.status_active=1 and d.status_active=1 and c.prod_id=$prod_id and d.prod_id=$prod_id and b.booking_id=$program_no and d.po_breakdown_id in($order_id) and d.trans_id = e.id and e.status_active=1 and e.store_id='$store_id' and e.floor_id='$floor_id' and e.room='$room_id' and e.rack='$rack_id' and e.self='$self_id'";
		}
		else
		{
			$receive_sql="select b.id, d.po_breakdown_id, d.quantity
			from inv_receive_master a, inv_receive_master b, pro_grey_prod_entry_dtls c, order_wise_pro_details d, inv_transaction e
			where a.id=b.booking_id and b.id=c.mst_id and c.id=d.dtls_id and c.trans_id>0 and d.trans_id>0 and a.entry_form=2 and b.entry_form=22 and d.entry_form=22 and b.receive_basis=9 and b.status_active=1 and c.status_active=1 and d.status_active=1 and c.prod_id=$prod_id and d.prod_id=$prod_id and a.booking_id=$program_no and d.po_breakdown_id in($order_id) and d.trans_id = e.id and e.status_active=1 and e.store_id='$store_id' and e.floor_id='$floor_id' and e.room='$room_id' and e.rack='$rack_id' and e.self='$self_id'";
		}
		//echo $receive_sql;

		$receive_result=sql_select($receive_sql);
		$all_rcv_id="";$yet_issue=0;
		foreach($receive_result as $row)
		{
			...
			$grey_fabric_recv += $row[csf('quantity')];
		...
		if($all_rcv_id!="")
		{
			...
			where b.id=c.mst_id and c.id=d.trans_id and c.transaction_type=3 and b.entry_form=45 and d.entry_form=45 and b.status_active=1 and c.status_active=1 and d.status_active=1 and c.prod_id=$prod_id and d.prod_id=$prod_id and b.received_id in($all_rcv_id) and d.po_breakdown_id in($order_id) and c.store_id='$store_id' and c.floor_id='$floor_id' and c.room='$room_id' and c.rack='$rack_id' and c.self='$self_id'";
			$rcv_rtn_result=sql_select($rcv_rtn_sql);
			foreach($rcv_rtn_result as $row)
			{
				$grey_fabric_issued += $row[csf('quantity')];
				$yet_issue-=$row[csf('quantity')];
			}
		}


		$issue_sql="select d.po_breakdown_id, d.quantity  from inv_grey_fabric_issue_dtls c, order_wise_pro_details d, inv_transaction e where c.id=d.dtls_id and c.trans_id>0 and d.trans_id>0 and d.entry_form=16 and c.prod_id=$prod_id and d.prod_id=$prod_id and c.program_no=$program_no and d.po_breakdown_id in($order_id) and c.status_active=1 and d.status_active=1 and d.trans_id = e.id and e.status_active=1 and e.store_id='$store_id' and e.floor_id='$floor_id' and e.room='$room_id' and e.rack='$rack_id' and e.self='$self_id'";
		...
		$issue_rtn_sql=" select d.po_breakdown_id, d.quantity from inv_receive_master b, inv_transaction c, order_wise_pro_details d where b.id=c.mst_id and c.id=d.trans_id and c.transaction_type=4 and b.entry_form=51 and d.entry_form=51 and c.prod_id=$prod_id and d.prod_id=$prod_id and b.booking_id=$program_no and d.po_breakdown_id in($order_id) and b.status_active=1 and c.status_active=1 and d.status_active=1 and c.store_id='$store_id' and c.floor_id='$floor_id' and c.room='$room_id' and c.rack='$rack_id' and c.self='$self_id' ";
		...
		$transfer_sql="select d.trans_type, d.po_breakdown_id, d.quantity from inv_item_transfer_mst b, inv_item_transfer_dtls c, order_wise_pro_details d, inv_transaction e where b.id=c.mst_id and c.id=d.dtls_id  and b.entry_form in(13,80,81,362) and d.entry_form in(13,80,81,362) and b.status_active=1 and c.status_active=1 and d.status_active=1 and d.prod_id=$prod_id and c.from_program=$program_no and d.po_breakdown_id in($order_id) and d.trans_id = e.id and e.status_active=1 and e.store_id='$store_id' and e.floor_id='$floor_id' and e.room='$room_id' and e.rack='$rack_id' and e.self='$self_id'";
	...
	else
	{
		...
		$sql=sql_select("select
			sum(case when a.entry_form in(2,22) then a.quantity end) as grey_fabric_recv,
			sum(case when a.entry_form in(16) then a.quantity end) as grey_fabric_issued,
			sum(case when a.entry_form=45 then a.quantity end) as grey_fabric_recv_return,
			sum(case when a.entry_form=51 then a.quantity end) as grey_fabric_issue_return,
	...
	if($order_id!="")
	{
		if ($receive_basis == 3 && $hidden_is_sales == 1)
		{
			/*echo "SELECT max(buyer_id) as buyer_name, LISTAGG(cast(a.style_ref_no as varchar2(4000)), ',') WITHIN GROUP (ORDER BY a.id) as style_ref_no,LISTAGG(a.id, ',') WITHIN GROUP (ORDER BY a.id) as b_id, LISTAGG(cast(a.job_no as varchar2(4000)), ',') WITHIN GROUP (ORDER BY a.id) as b_ponum from fabric_sales_order_mst a where a.id in($order_id) and a.status_active=1 and a.is_deleted=0";die;*/
			$orderSQL=sql_select("SELECT max(buyer_id) as buyer_name, LISTAGG(cast(a.style_ref_no as varchar2(4000)), ',') WITHIN GROUP (ORDER BY a.id) as style_ref_no,LISTAGG(a.id, ',') WITHIN GROUP (ORDER BY a.id) as b_id, LISTAGG(cast(a.job_no as varchar2(4000)), ',') WITHIN GROUP (ORDER BY a.id) as b_ponum from fabric_sales_order_mst a where a.id in($order_id) and a.status_active=1 and a.is_deleted=0");
		}
		else
		{
			if($db_type==0)
		...
		if($receive_basis == 3)
		{
			$sales_no_sql=sql_select("select id,job_no, style_ref_no, buyer_id, customer_buyer, within_group from fabric_sales_order_mst where id in($order_id) and status_active=1 and is_deleted=0");
			foreach ($sales_no_sql as $row) 
			{
				$sales_no_arr[$row[csf('id')]]["job_no"]=$row[csf('job_no')];
			...
			if (empty($sales_no_sql))
			{
				$order_no=$orderSQL[0][csf("b_ponum")];
				$style_ref_no=$orderSQL[0][csf("style_ref_no")];
				$buyer_id=$orderSQL[0][csf("buyer_name")];
			}
		}
		else
		{
...
if($action=="populate_data_about_sample")
{
	...
	union all
	SELECT sum(c.transfer_qnty) as qnty, 0 as qnty2 from inv_item_transfer_mst b, inv_item_transfer_dtls c where b.id=c.mst_id and c.item_category=13 and b.transfer_criteria in(6,8) and b.to_order_id=$booking_id and b.status_active=1 and b.is_deleted=0 and c.from_prod_id=$prod_id
...
if($action=="itemDescription_popup")
{
	...
	if($hidden_order_id!="")
	{
		$sql_rate=sql_select("select b.prod_id,sum(c.quantity*b.cons_rate)/sum(c.quantity) as avg_rate from inv_receive_master a, inv_transaction b,order_wise_pro_details c where a.id=b.mst_id and b.id=c.trans_id  and b.prod_id=c.prod_id and b.cons_amount>0 and a.item_category=13 and b.transaction_type=1 and a.entry_form in (2,22)  and c.entry_form in (2,22) and c.trans_type=1 and b.item_category=13 and c.po_breakdown_id in ($hidden_order_id) and b.status_active=1 and b.is_deleted=0 group by b.prod_id");
	...
	{
		$stock_arr = return_library_array( "select id, avg_rate_per_unit from product_details_master where item_category_id=13",'id','avg_rate_per_unit');
	}

	$sql_dye_wo=sql_select("SELECT b.fabric_desc, b.machine_gg, b.machine_dia, b.color_id from dyeing_work_order_mst a, dyeing_work_order_dtls b where a.id=b.mst_id and a.company_id=$cbo_company_id and a.status_active=1 and a.is_deleted=0 and a.do_no='$txt_service_booking_no'");
	if(!empty($sql_dye_wo))
	{
		foreach ($sql_dye_wo as $val) 
		{
			$dye_wo_fabric_arr[$val[csf("fabric_desc")]][$val[csf("machine_gg")]][$val[csf("machine_dia")]][$val[csf("color_id")]]=$val[csf("fabric_desc")];
		}
	}
		...
		if($cbo_basis==3)
		{
			...
			if($db_type==0)
			{
				...
				from ( 
					...
					sum(case when d.receive_basis=9 and d.booking_id in($recv_id) and b.entry_form=22 then b.quantity else 0 end) as qnty2,b.is_sales,d.within_group
			...
			else
			{
				...
				$sql = "SELECT id, product_name_details, detarmination_id, gsm, dia_width, current_stock, po_id, floor_id, room, rack, self, yarn_lot, yarn_count, stitch_length, sum(qnty) as qnty, sum(qnty2) as qnty2,is_sales,within_group from (
				select a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock,b.po_breakdown_id as po_id, e.floor_id, e.room,  e.rack, e.self, c.yarn_lot as yarn_lot, c.yarn_count as yarn_count, c.stitch_length, sum(case when d.receive_basis=2 and d.booking_id=$txt_booking_no and b.entry_form=2 then b.quantity else 0 end ) as qnty,
				sum(case when d.receive_basis=9 and d.booking_id in($recv_id) and b.entry_form=22 then b.quantity else 0 end) as qnty2 ,b.is_sales,d.within_group
				from product_details_master a, inv_receive_master d, order_wise_pro_details b, pro_grey_prod_entry_dtls c, inv_transaction e where a.id=b.prod_id and b.dtls_id=c.id and d.id=c.mst_id and a.current_stock>0 and a.item_category_id=13 and b.entry_form in(2,22) and b.trans_type=1 and b.po_breakdown_id in ($po_id) and b.trans_id!=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.booking_id in ($all_booking_id) and c.trans_id = e.id and e.store_id = '$cbo_store_name' group by a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, b.po_breakdown_id, c.yarn_lot, c.yarn_count, c.stitch_length, e.floor_id, e.room,  e.rack, e.self, b.is_sales,d.within_group
				union all
				select a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, b.po_breakdown_id as po_id, d.floor_id, d.room, d.rack, d.self, c.yarn_lot as yarn_lot, c.y_count as yarn_count, c.stitch_length, 0 as qnty, sum(b.quantity) as qnty2,b.is_sales,0 as within_group 
				from product_details_master a, order_wise_pro_details b, inv_item_transfer_dtls c, inv_transaction d 
				where a.id=b.prod_id and b.dtls_id=c.id and a.current_stock>0 and a.item_category_id=13 and b.entry_form in(13,81,362) and c.TO_PROGRAM=$txt_booking_no and b.trans_type=5 and c.item_category=13 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.to_trans_id = d.id and d.store_id = '$cbo_store_name' group by a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, b.po_breakdown_id, c.yarn_lot, c.y_count, c.stitch_length, d.floor_id, d.room, d.rack, d.self, b.is_sales
				union all
				select a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, b.po_breakdown_id as po_id, c.floor_id, c.room, c.rack, c.self, c.batch_lot as yarn_lot, c.yarn_count as yarn_count, c.stitch_length, sum(b.quantity) as qnty, 0 as qnty2,b.is_sales,0 as within_group 
				from product_details_master a, order_wise_pro_details b, inv_transaction c, INV_RECEIVE_MASTER d  
				where a.id=b.prod_id and b.trans_id=c.id and c.mst_id=d.id and d.entry_form=51 and d.RECEIVE_BASIS=3 and d.booking_id=$txt_booking_no and a.current_stock>0 and a.item_category_id=13 and c.transaction_type=4 and b.entry_form=51 and b.trans_type=4 and c.item_category=13 and b.po_breakdown_id in ($po_id) and b.status_active=1 and b.is_deleted=0 and c.store_id = '$cbo_store_name' group by a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, b.po_breakdown_id, c.batch_lot, c.yarn_count, c.stitch_length, c.floor_id, c.room, c.rack, c.self, b.is_sales ) product_details_master group by id, product_name_details, detarmination_id, gsm, dia_width, current_stock, floor_id, room, rack, self, is_sales,within_group, yarn_lot, yarn_count, stitch_length,po_id";
				
			}// and b.po_breakdown_id in ($po_id) > trasfer part as per tofael vai
		}
		else
		{
			if($db_type==0)
			...
			$sql="SELECT id, product_name_details, detarmination_id, gsm, dia_width, current_stock, po_id,floor_id,room, rack, self, yarn_lot, yarn_count, stitch_length, sum(qnty) as qnty,is_sales,within_group 
			from 
			(
				select a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock,b.is_sales,0 as within_group, d.floor_id, d.room, d.rack, d.self,
				...
				where a.id=b.prod_id and b.dtls_id=c.id and a.current_stock>0 and a.item_category_id=13 and b.entry_form in(2,22) and b.trans_type=1 and b.po_breakdown_id in ($po_id) and b.trans_id!=0 and c.trans_id= d.id and d.store_id = '$cbo_store_name' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
				group by a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock,b.is_sales, d.floor_id, d.room, d.rack, d.self,b.po_breakdown_id, c.yarn_lot, c.yarn_count, c.stitch_length
				union all
				select a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock,b.is_sales,0 as within_group, d.floor_id, d.room, d.rack, d.self,
				$field2, sum(b.quantity) as qnty
				from product_details_master a, order_wise_pro_details b, inv_item_transfer_dtls c, inv_transaction d
				where a.id=b.prod_id and b.dtls_id=c.id and a.current_stock>0 and a.item_category_id=13 and b.entry_form in(13,81) and b.trans_type=5 and c.item_category=13 and b.po_breakdown_id in ($po_id) and c.to_trans_id = d.id and d.store_id = '$cbo_store_name' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
				group by a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, b.is_sales, d.floor_id, d.room, d.rack, d.self,b.po_breakdown_id, c.yarn_lot, c.y_count, c.stitch_length
				union all 
				select a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock,b.is_sales,0 as within_group, c.floor_id, c.room, c.rack, c.self,
				$field3, sum(b.quantity) as qnty 
				from product_details_master a, order_wise_pro_details b, inv_transaction c
				where a.id=b.prod_id and b.trans_id=c.id and a.current_stock>0 and a.item_category_id=13 and c.transaction_type=4 and b.entry_form=51 and b.trans_type=4 and c.item_category=13 and c.store_id = '$cbo_store_name' and b.po_breakdown_id in ($po_id) and b.status_active=1 and b.is_deleted=0 group by a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock,b.is_sales, c.floor_id, c.room, c.rack, c.self,b.po_breakdown_id, c.batch_lot,	c.yarn_count, c.stitch_length
			) product_details_master
			group by id, product_name_details, detarmination_id, gsm, dia_width, current_stock, po_id, floor_id, room, rack, self, yarn_lot, yarn_count, stitch_length, is_sales, within_group";
	...
	else if(($po_id=="" || $po_id==0) && ($cbo_issue_purpose==8 || $cbo_issue_purpose==3 || $cbo_issue_purpose==26 || $cbo_issue_purpose==29 || $cbo_issue_purpose==30 || $cbo_issue_purpose==31))
	{
		...
		if($recv_id=="")
		{
			$sql = "SELECT id, product_name_details, detarmination_id, gsm, dia_width, current_stock, floor_id, room ,rack, self, yarn_lot, yarn_count, stitch_length, sum(qnty) as qnty, sum(qnty2) as qnty2,is_sales,within_group from 
			(
				SELECT a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, d.floor_id, d.room, d.rack, d.self, c.yarn_lot as yarn_lot, c.yarn_count as yarn_count, c.stitch_length, sum(c.grey_receive_qnty) as qnty, 0 as qnty2,NULL as is_sales,b.within_group
				...
				group by a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, b.booking_id, c.yarn_lot, c.yarn_count, c.stitch_length, d.floor_id, d.room, d.rack, d.self, b.within_group
				union all
				SELECT a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, c.floor_id, c.room, c.rack, c.self, c.batch_lot as yarn_lot, c.yarn_count as yarn_count, c.stitch_length, sum(c.cons_quantity) as qnty, 0 as qnty2 ,NULL as is_sales,b.within_group
				...
				group by a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, b.booking_id, c.batch_lot, c.yarn_count, c.stitch_length, c.floor_id, c.room, c.rack, c.self, b.within_group
				union all
				SELECT a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, d.floor_id, d.room, d.rack, d.self, c.yarn_lot as yarn_lot, c.y_count as yarn_count, c.stitch_length, sum(c.transfer_qnty) as qnty, sum(c.transfer_qnty) as qnty2,NULL as is_sales,0 as within_group
				from inv_item_transfer_mst b, inv_item_transfer_dtls c, product_details_master a, inv_transaction d
				where b.id=c.mst_id and c.from_prod_id=a.id and a.current_stock>0 and a.item_category_id=13 and b.entry_form in(80,432) and c.item_category=13 and b.to_order_id=$txt_booking_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.to_trans_id = d.id and d.store_id ='$cbo_store_name'
				group by a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, c.yarn_lot, c.y_count, c.stitch_length, d.floor_id, d.room, d.rack, d.self 
			)
			product_details_master 
			group by id, product_name_details, detarmination_id, gsm, dia_width, current_stock, floor_id, room, rack, self, yarn_lot, yarn_count, stitch_length,is_sales,within_group ";
		}
		else
		{
			$all_booking_id=$txt_booking_id.",".$recv_id;
			$sql = "SELECT id, product_name_details, detarmination_id, gsm, dia_width, current_stock, floor_id, room, rack, self, yarn_lot, yarn_count, stitch_length, sum(qnty) as qnty, sum(qnty2) as qnty2,is_sales,within_group  from (
			SELECT a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, d.floor_id, d.room, d.rack, d.self, c.yarn_lot as yarn_lot, c.yarn_count as yarn_count, c.stitch_length, sum(case when b.receive_basis!=9 and b.booking_id=$txt_booking_id then c.grey_receive_qnty else 0 end ) as qnty, sum(case when b.receive_basis=9 and b.booking_id in($recv_id) then c.grey_receive_qnty else 0 end) as qnty2,NULL as is_sales,b.within_group
			from product_details_master a, inv_receive_master b, pro_grey_prod_entry_dtls c, inv_transaction d
			where a.id=c.prod_id and b.id=c.mst_id and a.current_stock>0 and a.item_category_id=13 and b.entry_form in(2,22) and b.booking_without_order=1 and c.trans_id!=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.booking_id in ($all_booking_id) and c.trans_id=d.id and d.store_id = '$cbo_store_name' group by a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, c.yarn_lot, c.yarn_count, c.stitch_length, d.floor_id, d.room, d.rack, d.self, b.within_group
			union all
			SELECT a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, c.floor_id, c.room, c.rack, c.self, c.batch_lot as yarn_lot, c.yarn_count as yarn_count, c.stitch_length, sum(c.cons_quantity) as qnty, 0 as qnty2,NULL as is_sales,b.within_group
			from product_details_master a, inv_receive_master b, inv_transaction c
			where a.id=c.prod_id and b.id=c.mst_id and a.current_stock>0 and a.item_category_id=13 and b.entry_form=51 and b.receive_purpose=8 and c.item_category=13 and c.transaction_type=4 and b.booking_id=$txt_booking_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.store_id = '$cbo_store_name' group by a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, c.batch_lot, c.yarn_count, c.stitch_length,c.floor_id, c.room, c.rack, c.self, b.within_group
			union all
			SELECT a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, d.floor_id, d.room, d.rack, d.self, c.yarn_lot as yarn_lot, c.y_count as yarn_count, c.stitch_length, sum(c.transfer_qnty) as qnty, sum(c.transfer_qnty) as qnty2,NULL as is_sales,0 as within_group
			from inv_item_transfer_mst b, inv_item_transfer_dtls c, product_details_master a, inv_transaction d
			where b.id=c.mst_id and c.from_prod_id=a.id and a.current_stock>0 and a.item_category_id=13 and c.item_category=13 and b.transfer_criteria in(6,8) and b.to_order_id=$txt_booking_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.to_trans_id=d.id and d.store_id ='$cbo_store_name'
			group by a.id, a.product_name_details, a.detarmination_id, a.gsm, a.dia_width, a.current_stock, c.yarn_lot, c.y_count, c.stitch_length, d.floor_id, d.room, d.rack, d.self )
			product_details_master
			group by id, product_name_details, detarmination_id, gsm, dia_width, current_stock,floor_id,room, rack, self, yarn_lot, yarn_count, stitch_length,is_sales,within_group ";
		}
	...
	if (($cbo_issue_purpose==8 || $cbo_issue_purpose==3 || $cbo_issue_purpose==26 || $cbo_issue_purpose==29 || $cbo_issue_purpose==30 || $cbo_issue_purpose==31) && $cbo_basis==1)
	{
		...
		if($recv_id=="")
		{
			$iss_sql=sql_select("SELECT c.prod_id as id, b.booking_id, d.floor_id, d.room, d.rack, d.self, c.yarn_lot, c.yarn_count, c.stitch_length, sum(c.issue_qnty) as qnty
				...
				where b.id=c.mst_id and b.item_category=13 and b.entry_form=16 and b.issue_purpose in(3,8,26,29,30,31) and b.issue_basis=1 and b.booking_id=$txt_booking_id and b.status_active=1 and b.is_deleted=0 and c.trans_id = d.id and d.status_active=1 and d.store_id='$cbo_store_name'
				...
				where b.id=c.mst_id and b.entry_form=81 and c.item_category=13 and b.from_order_id=$txt_booking_id and b.status_active=1 and b.is_deleted=0 and c.trans_id = d.id and d.store_id ='$cbo_store_name'
		...
		else
		{
			$iss_sql=sql_select("SELECT c.prod_id as id, b.booking_id, d.floor_id, d.room, d.rack, d.self, c.yarn_lot, c.yarn_count, c.stitch_length, sum(c.issue_qnty) as qnty
				...
				select c.from_prod_id as id, b.from_order_id as booking_id, d.floor_id, d.room, d.rack, d.self, c.yarn_lot as yarn_lot, c.y_count as yarn_count, c.stitch_length, sum(c.transfer_qnty) as qnty
				from inv_item_transfer_mst b, inv_item_transfer_dtls c, inv_transaction d
				where b.id=c.mst_id and b.entry_form=81 and c.item_category=13 and b.from_order_id=$txt_booking_id and b.status_active=1 and b.is_deleted=0 and c.trans_id = d.id and d.store_id ='$cbo_store_name' group by c.from_prod_id, b.from_order_id, c.yarn_lot, c.y_count, c.stitch_length, d.floor_id, d.room, d.rack, d.self");
		}
	...
	else if($cbo_issue_purpose==8 && $cbo_basis==3) // Issue Purpose: Sample Without Order and Issue Basis 3: Kniting Plan
	{
		$iss_sql=sql_select("SELECT c.prod_id as id, b.booking_id, d.floor_id, d.room, d.rack, d.self, c.yarn_lot, c.yarn_count, c.stitch_length, sum(c.issue_qnty) as qnty
		from inv_issue_master b, inv_grey_fabric_issue_dtls c, inv_transaction d
		where b.id=c.mst_id and b.item_category=13 and b.entry_form=16 and b.issue_purpose in(3,8,26,29,30,31) and b.issue_basis=3 and b.booking_id=$txt_booking_id and b.status_active=1 and b.is_deleted=0 and c.trans_id = d.id and d.status_active=1 and d.store_id='$cbo_store_name' 
		group by c.prod_id, b.booking_id, c.yarn_lot, c.yarn_count, c.stitch_length, d.floor_id, d.room, d.rack, d.self
		union all
		select c.prod_id as id, d.booking_id, c.floor_id, c.room, c.rack, c.self, c.batch_lot as yarn_lot, c.yarn_count as yarn_count, c.stitch_length, sum(c.cons_quantity) as qnty from inv_issue_master b, inv_transaction c, inv_receive_master d where b.id=c.mst_id and b.received_id=d.id and ((d.booking_id=$txt_booking_id and d.receive_basis!=9) or (d.booking_id in ($recv_id) and d.receive_basis=9)) and d.booking_without_order=1 and b.item_category=13 and b.entry_form=45 and c.transaction_type=3 and c.item_category=13 and c.store_id ='$cbo_store_name' group by c.prod_id, d.booking_id, c.batch_lot, c.yarn_count, c.stitch_length, c.floor_id, c.room, c.rack, c.self
		union all
	...
	else
	{
		...
		// condition as per tofael vai
		if ($cbo_basis==3) 
		{
			$prog_issue_cond=" and c.program_no=$txt_booking_no";
			$from_prog_cond=" and c.from_program=$txt_booking_no";
		}
		else {
			$transfer_po_cond=" and b.po_breakdown_id in ($po_id)";
		}
		if ($cbo_basis==1) 
		{
			$prog_issue_cond=" and c.program_no=$txt_booking_id";
		}
		...
		$iss_sql=sql_select("SELECT c.prod_id as id, d.floor_id, d.room, d.rack, d.self,
			...
			where b.dtls_id=c.id and b.entry_form=16 and b.trans_type=2 and b.po_breakdown_id in ($po_id) and b.status_active=1 and b.is_deleted=0 and c.trans_id = d.id and d.status_active=1 and d.store_id = '$cbo_store_name' $prog_issue_cond
			...
			where b.dtls_id=c.id and b.entry_form in(13,80) and b.trans_type=6 and c.item_category=13 and b.status_active=1 and b.is_deleted=0 and c.trans_id = d.id and d.store_id= '$cbo_store_name' $from_prog_cond $transfer_po_cond
			...
			where b.trans_id=c.id and c.transaction_type=3 and b.entry_form=45 and b.trans_type=3 and c.item_category=13 and b.po_breakdown_id in ($po_id) and b.status_active=1 and b.is_deleted=0 and c.store_id= '$cbo_store_name'
	...
	<div style="width:<? echo $width; ?>px; max-height:310px; overflow-y:scroll" id="container_batch" >
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $width-20; ?>" class="rpt_table" id="tbl_search" >
			<?
			//print_r($dye_wo_fabric_arr);
			foreach ($result as $row)
			{
				if( (!empty($dye_wo_fabric_arr) && $dye_wo_fabric_arr[$row[csf("detarmination_id")]][$row[csf("gsm")]][$row[csf("dia_width")]] )  || empty($dye_wo_fabric_arr))
				{
			...
			foreach ($rcvDataArr as $rcv_product_id=>$rcv_product_data)
			{
				foreach ($rcv_product_data as $recvProdStr => $row)
				{
					...
					if(($cbo_issue_purpose==8 || $cbo_issue_purpose==3 || $cbo_issue_purpose==26 || $cbo_issue_purpose==29 || $cbo_issue_purpose==30 || $cbo_issue_purpose==31) && $cbo_basis==1)
					{
						...
						$iss_qnty=$grey_iss_array[$row['id']][$row[csf('booking_id')]][$floorID][$roomID][$rackID][$shelfID];
					}
...
if($action=="itemDescription_popup__BK")
{
		...
		if($cbo_basis==3)
		{
			...
			else
			{
				...
				if($cbo_basis==3)
				{
					...
					{
						$sql = "select id, product_name_details, current_stock, po_id, floor_id, room, rack, self, yarn_lot, yarn_count, stitch_length, sum(qnty) as qnty, sum(qnty2) as qnty2,is_sales,within_group from ( select a.id, a.product_name_details, a.current_stock, group_concat(distinct(b.po_breakdown_id)) as po_id, e.floor_id, e.room,  e.rack, e.self, c.yarn_lot as yarn_lot, c.yarn_count as yarn_count, c.stitch_length,
						sum(case when d.receive_basis=2 and d.booking_id=$txt_booking_no and b.entry_form=2 then b.quantity else 0 end ) as qnty, sum(case when d.receive_basis=9 and d.booking_id in($recv_id) and b.entry_form=22 then b.quantity else 0 end) as qnty2,b.is_sales,d.within_group
						from product_details_master a, inv_receive_master d, order_wise_pro_details b, pro_grey_prod_entry_dtls c, inv_transaction e where a.id=b.prod_id and b.dtls_id=c.id and d.id=c.mst_id and a.current_stock>0 and a.item_category_id=13 and b.entry_form in(2,22) and b.trans_type=1 and b.po_breakdown_id in ($po_id) and b.trans_id!=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.booking_id in ($all_booking_id) and c.trans_id = e.id and e.store_id = '$cbo_store_name' group by a.id, a.product_name_details, a.current_stock, c.yarn_lot, c.yarn_count, c.stitch_length, e.floor_id, e.room,  e.rack, e.self, b.is_sales,d.within_group
						union all
						select a.id, a.product_name_details, a.current_stock, group_concat(distinct(b.po_breakdown_id)) as po_id, d.floor_id, d.room, d.rack, d.self, c.yarn_lot as yarn_lot, c.y_count as yarn_count, c.stitch_length, 0 as qnty, sum(b.quantity) as qnty2,b.is_sales,0 as within_group from product_details_master a, order_wise_pro_details b, inv_item_transfer_dtls c, inv_transaction d where a.id=b.prod_id and b.dtls_id=c.id and a.current_stock>0 and a.item_category_id=13 and b.entry_form in(13,81) and b.trans_type=5 and c.item_category=13 and b.po_breakdown_id in ($po_id) and c.to_program in ($txt_booking_no) and c.to_trans_id=d.id and d.store_id = '$cbo_store_name' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id, a.product_name_details, a.current_stock, c.yarn_lot, c.y_count, c.stitch_length, d.floor_id, d.room, d.rack, d.self, b.is_sales
						union all
						select a.id, a.product_name_details, a.current_stock, group_concat(distinct(b.po_breakdown_id)) as po_id, c.floor_id, c.room, c.rack, c.self, c.batch_lot as yarn_lot, c.yarn_count as yarn_count, c.stitch_length, sum(b.quantity) as qnty, 0 as qnty2,b.is_sales,0 as within_group from product_details_master a, order_wise_pro_details b, inv_transaction c where a.id=b.prod_id and b.trans_id=c.id and a.current_stock>0 and a.item_category_id=13 and c.transaction_type=4 and b.entry_form=51 and b.trans_type=4 and c.item_category=13 and b.po_breakdown_id in ($po_id) and b.status_active=1 and b.is_deleted=0 and c.store_id = '$cbo_store_name' group by b.prod_id, c.batch_lot, c.yarn_count, c.stitch_length, c.floor_id, c.room, c.rack, c.self, b.is_sales) product_details_master group by id, po_id, floor_id,room, rack, self, yarn_lot, yarn_count, stitch_length,is_sales,within_group";
					}
					else
					{
						...
						from product_details_master a, inv_receive_master d, order_wise_pro_details b, pro_grey_prod_entry_dtls c, inv_transaction e where a.id=b.prod_id and b.dtls_id=c.id and d.id=c.mst_id and a.current_stock>0 and a.item_category_id=13 and b.entry_form in(2,22) and b.trans_type=1 and b.po_breakdown_id in ($po_id) and b.trans_id!=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.booking_id in ($all_booking_id) and c.trans_id = e.id and e.store_id = '$cbo_store_name' group by a.id, a.product_name_details, a.current_stock, c.yarn_lot, c.yarn_count, c.stitch_length, e.floor_id, e.room,  e.rack, e.self, b.is_sales,d.within_group
						union all
						select a.id, a.product_name_details, a.current_stock, LISTAGG(b.po_breakdown_id, ',') WITHIN GROUP (ORDER BY b.po_breakdown_id) as po_id, d.floor_id, d.room, d.rack, d.self, c.yarn_lot as yarn_lot, c.y_count as yarn_count, c.stitch_length, 0 as qnty, sum(b.quantity) as qnty2,b.is_sales,0 as within_group from product_details_master a, order_wise_pro_details b, inv_item_transfer_dtls c, inv_transaction d where a.id=b.prod_id and b.dtls_id=c.id and a.current_stock>0 and a.item_category_id=13 and b.entry_form in(13,81,362) and b.trans_type=5 and c.item_category=13 and b.po_breakdown_id in ($po_id) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.to_trans_id = d.id and d.store_id = '$cbo_store_name' group by a.id, a.product_name_details, a.current_stock, c.yarn_lot, c.y_count, c.stitch_length, d.floor_id, d.room, d.rack, d.self, b.is_sales
						union all
				...
				else
				{
					...
					{//======================================
						...
						( select a.id, a.product_name_details, a.current_stock, group_concat(distinct(b.po_breakdown_id)) as po_id, d.floor_id, d.room, d.rack, d.self, c.yarn_lot as yarn_lot, c.yarn_count as yarn_count, c.stitch_length, sum(case when b.entry_form=2 then b.quantity else 0 end ) as qnty,
      						sum(case when b.entry_form=22 then b.quantity else 0 end ) as qnty2, b.is_sales, NULL as within_group
						...
						$sql="select id, product_name_details, current_stock, po_id, floor_id, room, rack, self, yarn_lot, yarn_count, stitch_length, sum(qnty) as qnty, sum(qnty2) as qnty2, is_sales, within_group
							...
							(
								...
								$field, sum(case when b.entry_form=2 then b.quantity else 0 end ) as qnty,
	      						sum(case when b.entry_form=22 then b.quantity else 0 end ) as qnty2
								from product_details_master a, order_wise_pro_details b, pro_grey_prod_entry_dtls c, inv_transaction d
								where a.id=b.prod_id and b.dtls_id=c.id and a.current_stock>0 and a.item_category_id=13 and b.entry_form in(2,22) and b.trans_type=1 and b.po_breakdown_id in ($po_id) and b.trans_id!=0
								...
								from product_details_master a, order_wise_pro_details b, inv_item_transfer_dtls c, inv_transaction d
								where a.id=b.prod_id and b.dtls_id=c.id and a.current_stock>0 and a.item_category_id=13 and b.entry_form in(13,81) and b.trans_type=5 and c.item_category=13 and b.po_breakdown_id in ($po_id)
								and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.trans_id = d.id and d.status_active=1 and d.store_id = '$cbo_store_name'
								group by a.id, a.product_name_details, a.current_stock,b.is_sales, d.floor_id, d.room, d.rack, d.self
								...
								from product_details_master a, order_wise_pro_details b, inv_transaction c
								where a.id=b.prod_id and b.trans_id=c.id and a.current_stock>0 and a.item_category_id=13 and c.transaction_type=4 and b.entry_form=51 and b.trans_type=4 and c.item_category=13 			and b.po_breakdown_id in ($po_id) and c.store_id = '$cbo_store_name' and b.status_active=1 and b.is_deleted=0 group by a.id, a.product_name_details, a.current_stock,b.is_sales, c.floor_id, c.room, c.rack, c.self
							) product_details_master
	...
	else
	{
		...
		$iss_sql=sql_select("SELECT c.prod_id as id, d.floor_id, d.room, d.rack, d.self,
			...
			sum(b.quantity) as qnty
			from order_wise_pro_details b, inv_grey_fabric_issue_dtls c, inv_transaction d
			where b.dtls_id=c.id and b.entry_form=16 and b.trans_type=2 and b.po_breakdown_id in ($po_id) and b.status_active=1 and b.is_deleted=0 and c.trans_id = d.id and d.status_active=1 and d.store_id = '$cbo_store_name'
			group by c.prod_id, d.floor_id, d.room, d.rack, d.self
			union all
			select b.prod_id as id, d.floor_id, d.room, d.rack, d.self,
			$field2,
			sum(b.quantity) as qnty
			from order_wise_pro_details b, inv_item_transfer_dtls c, inv_transaction d
			where b.dtls_id=c.id and b.entry_form in(13,80) and b.trans_type=6 and c.item_category=13 and b.po_breakdown_id in ($po_id) and b.status_active=1 and b.is_deleted=0 and c.trans_id = d.id and d.store_id= '$cbo_store_name'
			group by b.prod_id, d.floor_id, d.room, d.rack, d.self
			union all
			select b.prod_id as id, c.floor_id, c.room, c.rack, c.self,
			$field3,
...
if ($action=="po_popup")
{
	...
	if($receive_basis==3 && $dtls_tbl_id!="")
	{
		/*$hide_qty_array=return_library_array( " select d.po_breakdown_id, d.quantity  from inv_grey_fabric_issue_dtls c, order_wise_pro_details d
			where c.id=d.dtls_id and c.trans_id>0 and d.trans_id>0 and d.entry_form=16 and c.program_no=$program_no and d.dtls_id in($dtls_tbl_id) and c.status_active=1 and d.status_active=1 ",'po_breakdown_id','quantity');*/
		$hide_qty_array_sql=sql_select("select d.po_breakdown_id, d.quantity,d.quantity_pcs  from inv_grey_fabric_issue_dtls c, order_wise_pro_details d
			where c.id=d.dtls_id and c.trans_id>0 and d.trans_id>0 and d.entry_form=16 and c.program_no=$program_no and d.dtls_id in($dtls_tbl_id) and c.status_active=1 and d.status_active=1");
		foreach($hide_qty_array_sql as $req_vals)
		{
			$hide_qty_array[$req_vals[csf('po_breakdown_id')]]=$req_vals[csf('quantity')];
			$hide_qtyinpcs_array[$req_vals[csf('po_breakdown_id')]]=$req_vals[csf('quantity_pcs')];
		}

	}
	else if($dtls_tbl_id!="")
	{
		$hide_qty_array=return_library_array( "select po_breakdown_id, quantity from order_wise_pro_details where dtls_id=$dtls_tbl_id and entry_form=16 and status_active=1 and is_deleted=0",'po_breakdown_id','quantity');
	}
	...
	<script>
		...
		function check_balance2(row_no)
		{
			var placeholder_value =$('#txtQntyInPcs_'+row_no).attr('placeholder')*1;
			var issued_qnty =$('#hideQntyInPcs_'+ row_no).val()*1;
			var qnty =$('#txtQntyInPcs_'+row_no).val()*1;

			if(qnty>(placeholder_value+issued_qnty))
			{
				alert("Issue Qty In Pcs Exceeds Balance Qty.");
				if(issued_qnty==0) issued_qnty='';
				$('#txtQntyInPcs_'+row_no).val(issued_qnty);
			}
		}
		...
		function fnc_close()
		{
			...
			$("#tbl_search").find('tr').each(function()
			{
				...
				var txtQntyInPcs=$(this).find('input[name="txtQntyInPcs[]"]').val();
				var hideSize=$(this).find('input[name="hideSize[]"]').val();
				...
				if(txtGreyQnty*1>0)
				{
					if(save_string=="")
					{
						save_string=txtPoId+"**"+txtGreyQnty+"**"+txtQntyInPcs+"**"+hideSize;
					}
					else
					{
						save_string+=","+txtPoId+"**"+txtGreyQnty+"**"+txtQntyInPcs+"**"+hideSize;
		...
		function sum_totalQntyInPcs()
		{
			var tblRows = $("#tbl_search tr").length;
			var dddd={dec_type:1}
			math_operation( "txt_total_sum_qtyinpcs", "txtQntyInPcs_", "+", tblRows, dddd );
		}
...
<body>
	...
	<form name="searchdescfrm"  id="searchdescfrm">
		...
		?>
		<fieldset style="width:<? echo $tableWidth; ?>;margin-left:5px">
			<!-- previous data here -->
			...
			else if($receive_basis==3)
			{
				if($db_type==0)
				{
					$trans_po_id=return_field_value("group_concat(distinct(a.to_order_id)) as po_id","inv_item_transfer_mst a, inv_item_transfer_dtls b","a.id=b.mst_id and a.transfer_criteria=4 and b.to_program='$program_no' and b.item_category=13 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","po_id");
				}
				else
				{
					$trans_po_id=return_field_value("LISTAGG(a.to_order_id, ',') WITHIN GROUP (ORDER BY a.to_order_id) as po_id","inv_item_transfer_mst a, inv_item_transfer_dtls b","a.id=b.mst_id and a.transfer_criteria=4 and b.to_program='$program_no' and b.item_category=13 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","po_id");
				}

				$req_booking='';
				$req_booking_sql=sql_select("select c.po_id, c.booking_no,c.is_sales,c.determination_id,c.color_id,c.gsm_weight,c.dia,c.color_type_id,c.body_part_id from ppl_planning_info_entry_dtls b, ppl_planning_entry_plan_dtls c where b.id=c.dtls_id and b.id='$program_no' and b.is_deleted=0 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0");
				//$req_booking=$req_booking_sql[0][csf('booking_no')];
				foreach($req_booking_sql as $row)
				{
					$program_poIds.=$row[csf('po_id')].",";
					$req_booking=$row[csf('booking_no')];
				...
				$program_poIds=chop($program_poIds,',');

				if($is_sales==1)
				{
					$reqQnty = "select mst_id po_break_down_id, sum(grey_qty) as fabric_qty from fabric_sales_order_dtls where status_active=1 and is_deleted=0 and mst_id='$hidden_order_id' and determination_id=$determination_id and gsm_weight=$gsm_weight and dia='$dia' and body_part_id=$body_part_id group by mst_id";
				}
				else
				{
					if($trans_po_id!="")
					{
						$reqQnty = "select po_break_down_id, sum(grey_fab_qnty) as fabric_qty from wo_booking_dtls where status_active=1 and is_deleted=0 and (booking_no='$req_booking' or po_break_down_id in($trans_po_id)) group by po_break_down_id";
					}
					else
					{
				...
				$sql_production=sql_select("select a.id, a.detarmination_id, a.gsm, a.dia_width from product_details_master a where a.id=$prod_id");
				$detarmination_id = $sql_production[0][csf("detarmination_id")];
				$gsm = $sql_production[0][csf("gsm")];
				$dia_width = $sql_production[0][csf("dia_width")];
				
				$sql_dye_wo=sql_select("SELECT a.PO_BREAKDOWN_ID, sum(b.wo_qty) as wo_qty, sum(b.amount) as wo_amount
				from dyeing_work_order_mst a, dyeing_work_order_dtls b
				where a.id=b.mst_id and a.company_id=$cbo_company_id and a.status_active=1 and a.is_deleted=0 and a.do_no='$txt_service_booking_no'
				and b.fabric_desc=$detarmination_id and b.machine_gg=$gsm and  b.machine_dia='$dia_width'
				...
				if(!empty($sql_dye_wo))
				{
					foreach ($sql_dye_wo as $val) 
					{
						$dye_wo_fabric_arr[$val[csf("PO_BREAKDOWN_ID")]]+=$val[csf("wo_qty")];
					}
				}
			}
			else
			{
				if($hidden_order_id!="")
				{
					$reqQnty = "select po_break_down_id, sum(grey_fab_qnty) as fabric_qty from wo_booking_dtls where status_active=1 and is_deleted=0 and po_break_down_id in($hidden_order_id) group by po_break_down_id";
					$reqQnty_res = sql_select($reqQnty);
					foreach($reqQnty_res as $req_val)
					{
						$req_qty_array[$req_val[csf('po_break_down_id')]]=$req_val[csf('fabric_qty')];
			...
			if($receive_basis==2 || $receive_basis==3)
			{
				...
				<div id="search_div" style="margin-top:10px">
					<div style="<? echo $tableWidth3; ?>; margin-top:10px" align="center">
						<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="300" align="center">
							...
							<tr class="general">
								<td><input type="text" name="txt_prop_grey_qnty" id="txt_prop_grey_qnty" class="text_boxes_numeric" value="<? if($prev_method==1) echo $issueQnty; ?>" style="width:120px"  onBlur="distribute_qnty(document.getElementById('cbo_distribiution_method').value)"  <? echo $readonlyCond; ?> /></td>
								<td>
									<?
					...
					<div style="margin-left:1px; margin-top:10px">
						<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="<? echo $tableWidth2; ?>">
							<thead>
								<th width="110">PO No/FSO No</th>
								...
								if($receive_basis==3)
								{
									?>
									<th width="50">Size</th>
									<th width="50">Prod. Qty In Pcs</th>
									<th width="50">Cumu. Qty In Pcs</th>
						...
						<div style="<? echo $tableWidth4; ?>; max-height:220px; overflow-y:scroll" id="container" align="left">
							<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="<? echo $tableWidth2; ?>" id="tbl_search">
								<?
								$i=1; $tot_po_qnty=0;
								//$original_po = return_field_value("po_id", "PPL_PLANNING_ENTRY_PLAN_DTLS", "dtls_id=$program_no", "po_id");
								if($hidden_order_id!="")
								{
									...
									if($receive_basis==3)
									{
										if($trans_po_id!="") $hidden_order_id.=",".$trans_po_id;
										if($program_poIds!="") $hidden_order_id.=",".$program_poIds;

										$hidden_order_id=implode(",",array_unique(explode(",",$hidden_order_id)));

										//echo $fabric_store_auto_update."===jahid";
										if($fabric_store_auto_update==1)
										{
											$receive_sql="select b.id, d.po_breakdown_id, d.quantity,d.coller_cuff_size,d.quantity_pcs 
											from inv_receive_master b, pro_grey_prod_entry_dtls c, order_wise_pro_details d, inv_transaction a
											where b.id=c.mst_id and c.id=d.dtls_id and c.trans_id>0 and d.trans_id>0 and b.entry_form=2 and d.entry_form=2 and b.receive_basis=2 and b.status_active=1 and c.status_active=1 and d.status_active=1 and c.prod_id=$prod_id and d.prod_id=$prod_id and b.booking_id=$program_no and d.po_breakdown_id in($hidden_order_id) and d.trans_id = a.id and a.store_id = '$store_id' and a.floor_id='$floor_id' and a.room = '$room_id' and a.rack='$rack_id' and a.self = '$self_id'";
										}
										else
										{
											$receive_sql="select b.id, d.po_breakdown_id, d.quantity,d.coller_cuff_size,d.quantity_pcs 
											from inv_receive_master a, inv_receive_master b, pro_grey_prod_entry_dtls c, order_wise_pro_details d, inv_transaction e
											where a.id=b.booking_id and b.id=c.mst_id and c.id=d.dtls_id and c.trans_id>0 and d.trans_id>0 and a.entry_form=2 and b.entry_form=22 and d.entry_form=22 and b.receive_basis=9 and b.status_active=1 and c.status_active=1 and d.status_active=1 and c.prod_id=$prod_id and d.prod_id=$prod_id and a.booking_id=$program_no and d.po_breakdown_id in($hidden_order_id) and d.trans_id = e.id and e.store_id = '$store_id' and e.floor_id='$floor_id' and e.room = '$room_id' and e.rack='$rack_id' and e.self = '$self_id'";
										}
										...
										foreach($receive_result as $row)
										{
											...
											$propDataArray[$row[csf('po_breakdown_id')]]['recv'] +=$row[csf('quantity')];
											$yet_issue+=$row[csf('quantity')];
											$propDataArray[$row[csf('po_breakdown_id')]]['bl']+=$row[csf('quantity')];

											$po_wise_qntyInPcs_info[$row[csf('po_breakdown_id')]]['size']=$row[csf('coller_cuff_size')];
											$po_wise_qntyInPcs_info[$row[csf('po_breakdown_id')]]['qtyInPcs']+=$row[csf('quantity_pcs')];
										}
										$all_rcv_id=chop($all_rcv_id,",");
										if($all_rcv_id!="")
										{
											$rcv_rtn_sql=" select d.po_breakdown_id, d.quantity
											from inv_issue_master b, inv_transaction c, order_wise_pro_details d, inv_receive_master e
											where b.id=c.mst_id and c.id=d.trans_id and b.received_id=e.id and c.transaction_type=3 and b.entry_form=45 and d.entry_form=45 and b.status_active=1 and c.status_active=1 and d.status_active=1 and c.prod_id=$prod_id and d.prod_id=$prod_id and b.received_id in($all_rcv_id) and d.po_breakdown_id in($hidden_order_id) and e.store_id = '$store_id' and c.floor_id='$floor_id' and c.room = '$room_id' and c.rack='$rack_id' and c.self = '$self_id'";
											//echo $rcv_rtn_sql;//die;
											$rcv_rtn_result=sql_select($rcv_rtn_sql);
											foreach($rcv_rtn_result as $row)
											{
												$propDataArray[$row[csf('po_breakdown_id')]]['iss']+=$row[csf('quantity')];
												$yet_issue-=$row[csf('quantity')];
												$propDataArray[$row[csf('po_breakdown_id')]]['bl'] -=$row[csf('quantity')];
											}
										}

										$issue_sql=" select d.po_breakdown_id, d.quantity,d.quantity_pcs from inv_grey_fabric_issue_dtls c, order_wise_pro_details d, inv_transaction a
										where c.id=d.dtls_id and c.trans_id>0 and d.trans_id>0 and d.entry_form=16 and c.prod_id=$prod_id and d.prod_id=$prod_id and c.program_no=$program_no and d.po_breakdown_id in($hidden_order_id) and c.status_active=1 and d.status_active=1 and d.trans_id=a.id and a.store_id ='$store_id' and a.floor_id='$floor_id' and a.room = '$room_id' and a.rack='$rack_id' and a.self = '$self_id'";
										$issue_result=sql_select($issue_sql);
										foreach($issue_result as $row)
										{
											$propDataArray[$row[csf('po_breakdown_id')]]['iss']+=$row[csf('quantity')];
											...
											$po_wise_qntyInPcs_info[$row[csf('po_breakdown_id')]]['qtyInPcs_issue']+=$row[csf('quantity_pcs')];
										}

										$issue_rtn_sql=" select d.po_breakdown_id, d.quantity
										from inv_receive_master b, inv_transaction c, order_wise_pro_details d
										where b.id=c.mst_id and c.id=d.trans_id and c.transaction_type=4 and b.entry_form=51 and d.entry_form=51 and c.prod_id=$prod_id and d.prod_id=$prod_id and b.booking_id=$program_no and d.po_breakdown_id in($hidden_order_id) and b.status_active=1 and c.status_active=1 and d.status_active=1 and c.store_id='$store_id' and c.floor_id='$floor_id' and c.room = '$room_id' and c.rack='$rack_id' and c.self = '$self_id'";
										$issue_rtn_result=sql_select($issue_rtn_sql);
										foreach($issue_rtn_result as $row)
										{
											$propDataArray[$row[csf('po_breakdown_id')]]['recv'] +=$row[csf('quantity')];
										...
										$transfer_sql="select d.trans_type, d.po_breakdown_id, d.quantity, c.from_store, c.floor_id, c.room, c.rack, c.shelf, c.to_store, c.to_floor_id, c.to_room, c.to_rack,c.to_shelf 
										from inv_item_transfer_mst b, inv_item_transfer_dtls c, order_wise_pro_details d
										where b.id=c.mst_id and c.id=d.dtls_id  and b.entry_form in(13,80,81,362) and d.entry_form in(13,80,81,362) and b.status_active=1 and c.status_active=1 and d.status_active=1 and d.prod_id=$prod_id and c.from_program=$program_no and d.po_breakdown_id in($hidden_order_id) ";
										...
										foreach($transfer_result as $row)
										{
											if($row[csf('trans_type')]==5)
											{
												if(($row[csf('to_store')] == $store_id) && ($row[csf('to_floor_id')] == $floor_id) && ($row[csf('to_room')] == $room_id) && ($row[csf('to_rack')] == $rack_id) && ($row[csf('to_shelf')] == $self_id))
												{
													$propDataArray[$row[csf('po_breakdown_id')]]['recv'] +=$row[csf('quantity')];
													$yet_issue+=$row[csf('quantity')];
													$propDataArray[$row[csf('po_breakdown_id')]]['bl'] +=$row[csf('quantity')];
											...
											{
												if(($row[csf('from_store')] ==$store_id) && ($row[csf('floor_id')] == $floor_id) && ($row[csf('room')] == $room_id) && ($row[csf('rack')] == $rack_id) && ($row[csf('shelf')] == $self_id))
												{
													$propDataArray[$row[csf('po_breakdown_id')]]['iss']+=$row[csf('quantity')];
													$yet_issue-=$row[csf('quantity')];
													$propDataArray[$row[csf('po_breakdown_id')]]['bl'] -=$row[csf('quantity')];
									...
									{
										$propData=sql_select("select a.po_breakdown_id, sum(case when a.entry_form in(2) then a.quantity end) as grey_fabric_auto_recv,
											sum(case when a.entry_form in(22) then a.quantity end) as grey_fabric_recv,
											sum(case when a.entry_form in(16) then a.quantity end) as grey_fabric_issued,
											sum(case when a.entry_form=45 then a.quantity end) as grey_fabric_recv_return,
											sum(case when a.entry_form=51 then a.quantity end) as grey_fabric_issue_return,
											sum(case when a.entry_form in(13,81,362) and a.trans_type=5 then a.quantity end) as grey_fabric_trans_recv,
											sum(case when a.entry_form in(13,80,362) and a.trans_type=6 then a.quantity end) as grey_fabric_trans_issued
											from order_wise_pro_details a, inv_transaction b where a.trans_id = b.id and a.trans_id<>0 and a.prod_id=$prod_id and a.po_breakdown_id in($hidden_order_id) and a.is_deleted=0 and a.status_active=1 and b.store_id = '$store_id' and b.floor_id='$floor_id' and b.room = '$room_id' and b.rack='$rack_id' and b.self = '$self_id' group by a.po_breakdown_id");
										foreach($propData as $prop_row)
										{
											if($fabric_store_auto_update == 1)
											{
												$recv_qnty=$prop_row[csf('grey_fabric_auto_recv')];
											}
											else
											{
												$recv_qnty=$prop_row[csf('grey_fabric_recv')];
											}

											$propDataArray[$prop_row[csf('po_breakdown_id')]]['recv']=$recv_qnty+$prop_row[csf('grey_fabric_issue_return')]+$prop_row[csf('grey_fabric_trans_recv')];
											...
											$yet_issue=($recv_qnty+$prop_row[csf('grey_fabric_issue_return')]+$prop_row[csf('grey_fabric_trans_recv')])-($prop_row[csf('grey_fabric_issued')]+$prop_row[csf('grey_fabric_recv_return')]+$prop_row[csf('grey_fabric_trans_issued')]);
									...
									{
										$po_sql="select id,job_no as po_number,buyer_id, null as po_qnty_in_pcs, null as pub_shipment_date from fabric_sales_order_mst where id in($hidden_order_id) and status_active=1 and is_deleted=0";
										$sales_no_sql=sql_select($po_sql);
										foreach ($sales_no_sql as $row) {
											$sales_no_arr[$row[csf('id')]]["job_no"]=$row[csf('po_number')];
											$sales_no_arr[$row[csf('id')]]["buyer_id"]=$row[csf('buyer_id')];
										}
									}
									else
									{
										$po_sql="select b.id, b.po_number, (a.total_set_qnty*b.po_quantity) as po_qnty_in_pcs, b.pub_shipment_date from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and b.id in ($hidden_order_id) order by b.pub_shipment_date, b.id";
									}
								}

								//echo "<pre>";print_r($propDataArray);die;
                    			//echo $po_sql;

								$po_data_array=array();$po_data_array2=array();
								$explSaveData = explode(",",$save_data);
								foreach($explSaveData as $val)
								{
									$woQnty = explode("**",$val);
									$po_data_array[$woQnty[0]]=$woQnty[1];
									$po_data_array2[$woQnty[0]]=$woQnty[2];
								}
								...
								foreach($nameArray as $row)
								{
									...
									$qntyInPcs = $po_data_array2[$row[csf('id')]];
									$hideQnty=$hide_qty_array[$row[csf('id')]];
									$hideQntyInPcs=$hide_qtyinpcs_array[$row[csf('id')]];

									if($dye_wo_fabric_arr[$row[csf('id')]] && $receive_basis==3)
									{
										//Dyeing wo quantity will be required here
										$requied_quantity = $dye_wo_fabric_arr[$row[csf('id')]];
									}
									else
									{
										$requied_quantity = $req_qty_array[$row[csf('id')]];
									}

									?>
									<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
										<td width="110">
											<p><? if($receive_basis==3 && !empty($sales_no_sql)){echo $sales_no_arr[$row[csf('id')]]["job_no"];}else{ echo $row[csf('po_number')];} ?></p>
										...
										<td width="80" align="right"><? echo $requied_quantity; ?></td>

										<?
										if($receive_basis==3)
										{

											$qntyInPcsBal=$po_wise_qntyInPcs_info[$row[csf('id')]]['qtyInPcs']-$po_wise_qntyInPcs_info[$row[csf('id')]]['qtyInPcs_issue'];
											?>
											<td width="50" align="center"><? echo $po_wise_qntyInPcs_info[$row[csf('id')]]['size']; ?>
												<input type="hidden" name="hideSize[]" id="hideSize_<? echo $i; ?>" value="<? echo $po_wise_qntyInPcs_info[$row[csf('id')]]['size']; ?>">
											</td>
											<td width="50" align="right"><? echo number_format($po_wise_qntyInPcs_info[$row[csf('id')]]['qtyInPcs'],2,'.',''); ?></td>
											<td width="50" align="right"><? echo number_format($po_wise_qntyInPcs_info[$row[csf('id')]]['qtyInPcs_issue'],2,'.',''); ?></td>
											<td  width="50" align="center">
												<input type="text" name="txtQntyInPcs[]" id="txtQntyInPcs_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px" value="<? echo $qntyInPcs; ?>" placeholder="<? echo number_format($qntyInPcsBal,2,'.',''); ?>" onKeyUp="check_balance2(<? echo $i; ?>); sum_totalQntyInPcs();" >
												<input type="hidden" name="hideQntyInPcs[]" id="hideQntyInPcs_<? echo $i; ?>" value="<? echo $hideQntyInPcs; ?>">
											</td>
											<?
										}
										?>
							...
							<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="<? echo $tableWidth2; ?>">
								<tr class="tbl_bottom">
									<td width="110">&nbsp;</td><td width="70" align="right">&nbsp;</td><td width="80" align="right">&nbsp;</td><td width="80" align="right">&nbsp;</td>
										...
										if($receive_basis==3)
										{
											?>
												<td width="50">&nbsp;</td>
												<td width="50" ali>&nbsp;</td>
												<td width="50" align="right">Sum</td>
												<td style="text-align:center;width: 50px;"><input type="text" id="txt_total_sum_qtyinpcs" class="text_boxes_numeric" style="width:40px" readonly /></td><td width="80" align="right">&nbsp;</td><td width="80" align="right">&nbsp;</td><td style="text-align:center"><input type="text" id="txt_total_sum" class="text_boxes_numeric" style="width:70px" readonly /></td>
											<?
										}
										else
										{
											?>
												<td width="80" align="right">&nbsp;</td><td width="80" align="right">Sum</td><td style="text-align:center"><input type="text" id="txt_total_sum" class="text_boxes_numeric" style="width:70px" readonly /></td>
						...
						<table width="<? echo $tableWidth; ?>" id="table_id">
							<tr>
								<td align="center" >
					...
					<div style="width:630px; max-height:220px; overflow-y:scroll" id="container" align="left">
						<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="610" id="tbl_search">
							...
							$propData=sql_select("SELECT a.po_breakdown_id, 
								...
								where a.trans_id = b.id and a.trans_id<>0 and a.prod_id=$prod_id and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and b.store_id = '$store_id' and b.floor_id='$floor_id' and b.room = '$room_id' and b.rack='$rack_id' and b.self = '$self_id' group by a.po_breakdown_id");
							foreach($propData as $prop_row)
							{
							...
							foreach($nameArray as $row)
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
									...
									<input type="hidden" name="txtQntyInPcs[]" id="txtQntyInPcs_<? echo $i; ?>" value="0">
									<input type="hidden" name="hideSize[]" id="hideSize_<? echo $i; ?>" value="">
								</tr>
								<?
								$i++;
							}
...
if ($action=="po_search_popup")
{
	...
	<script>
		...
		function fn_show_check()
		{
			...
			show_list_view ( $('#txt_search_common').val()+'_'+$('#cbo_search_by').val()+'_'+<? echo $cbo_company_id; ?>+'_'+$('#cbo_buyer_name').val()+'_'+'<? echo $hidden_order_id; ?>', 'create_po_search_view', 'search_div', 'grey_fabric_issue_controller', 'setFilterGrid(\'tbl_search\',-1);hidden_field_reset();');
			set_all();
		}
...
if($action=="save_update_delete")
{
	...
	if( str_replace("'", "", $hidden_is_sales) ==2 )
	{
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		if( str_replace("'","",$hidden_is_batch_maintain)==2 && str_replace("'","",$cbo_issue_purpose)!=3)
		{
			...
			$data_array_batch_mst= "(".$batchID.",".$txt_batch_no.",16,".$txt_issue_date.",".$cbo_company_id.",".$txt_booking_id.",".$txt_booking_no.",".$booking_without_order.",".$txtIssueQnty.",'".$user_id."','".$pc_date_time."')";
		...
		if( str_replace("'","",$txt_system_no) == "" ) //new insert
		{
			...
			//$new_mrr_number=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '', 'KGI', date("Y",time()), 5, "select issue_number_prefix,issue_number_prefix_num from inv_issue_master where company_id=$cbo_company_id and entry_form=16 and $year_cond=".date('Y',time())." order by id DESC ", "issue_number_prefix", "issue_number_prefix_num" ));

			$new_mrr_number = explode("*", return_next_id_by_sequence("INV_ISSUE_MASTER_PK_SEQ", "inv_issue_master",$con,1,$cbo_company_id,"KGI",16,date("Y",time()),13 ));

			$field_array="id,issue_number_prefix, issue_number_prefix_num, issue_number, issue_basis, issue_purpose, entry_form, item_category, company_id, buyer_id, style_ref, booking_id, booking_no, batch_no, issue_date, knit_dye_source, knit_dye_company, challan_no, service_booking_no, order_id, inserted_by, insert_date";
			$data_array="(".$id.",'".$new_mrr_number[1]."','".$new_mrr_number[2]."','".$new_mrr_number[0]."',".$cbo_basis.",".$cbo_issue_purpose.",16,13,".$cbo_company_id.",".$cbo_buyer_name.",".$txt_style_ref.",".$txt_booking_id.",".$txt_booking_no.",".$batchID.",".$txt_issue_date.",".$cbo_dyeing_source.",".$cbo_dyeing_com.",".$txt_challan_no.",".$txt_service_booking_no.",".$hidden_order_id.",'".$user_id."','".$pc_date_time."')";
		...
		else
		{
			...
			$field_array="issue_basis*issue_purpose*entry_form*item_category*company_id*buyer_id*style_ref*booking_id*booking_no*batch_no*issue_date*knit_dye_source*knit_dye_company*challan_no*service_booking_no*order_id*updated_by*update_date";
			$data_array="".$cbo_basis."*".$cbo_issue_purpose."*16*13*".$cbo_company_id."*".$cbo_buyer_name."*".$txt_style_ref."*".$txt_booking_id."*".$txt_booking_no."*".$txt_batch_no."*".$txt_issue_date."*".$cbo_dyeing_source."*".$cbo_dyeing_com."*".$txt_challan_no."*".$txt_service_booking_no."*".$hidden_order_id."*'".$user_id."'*'".$pc_date_time."'";
		...
		$tr_data_array = "(".$transactionID.",".$id.",".$txt_program_no.",".$cbo_company_id.",".$hiddenProdId.",13,2,".$txt_issue_date.",".$cbo_store_name.",".$txtIssueQnty.",".$hiddenAvgRate.",'".$cons_amnt."',".$cbo_floor_hidden.",".$cbo_room_hidden.",".$txt_rack_hidden.",".$txt_shelf_hidden.",'".$user_id."','".$pc_date_time."')";
		...
		$isLIFOfifo=return_field_value("store_method","variable_settings_inventory","company_name=$cbo_company_id and variable_list=17 and item_category_id=13 and status_active=1 and is_deleted=0");
		if($isLIFOfifo==2) $cond_lifofifo=" DESC"; else $cond_lifofifo=" ASC";

		$sql = sql_select("select id, cons_rate, balance_qnty, balance_amount from inv_transaction where prod_id=$hiddenProdId and balance_qnty>0 and transaction_type in (1,4,5) and item_category=13 order by transaction_date $cond_lifofifo");
		foreach($sql as $result)
		{
			...
			if($issueQntyBalance>=0)
			{
				...
				$data_array_mrr .= "(".$mrrWiseIsID.",".$recv_trans_id.",".$transactionID.",16,".$hiddenProdId.",".$txtIssueQnty.",'".$cons_rate."',".$amount.",'".$_SESSION['logic_erp']['user_id']."','".$pc_date_time."')";
				//for update
			...
			else if($issueQntyBalance<0)
			{
				...
				//for insert
				if($data_array_mrr!="") $data_array_mrr .= ",";
				$data_array_mrr .= "(".$mrrWiseIsID.",".$recv_trans_id.",".$transactionID.",16,".$hiddenProdId.",".$balance_qnty.",'".$cons_rate."',".$amount.",'".$_SESSION['logic_erp']['user_id']."','".$pc_date_time."')";
				//for update
				$updateID_array[]=$recv_trans_id;
		...
		if(count($save_string)>0 && str_replace("'","",$save_data)!="")
		{
			...
			$field_array_proportionate="id,trans_id,trans_type,entry_form,dtls_id,po_breakdown_id,prod_id,quantity,issue_purpose,is_sales,quantity_pcs,coller_cuff_size,inserted_by,insert_date";
			//order_wise_pro_details table data insert START-----//
			$po_array=array();$po_array_qntyinpcs=array();$po_array_size=array();
			for($i=0;$i<count($save_string);$i++)
			{
				$order_dtls=explode("**",$save_string[$i]);
				$order_id=$order_dtls[0];
				$order_qnty=$order_dtls[1];
				$order_qntyinpcs=$order_dtls[2];
				$hideSize=$order_dtls[3];
				...
				$po_array[$order_id]+=$order_qnty;
				$po_array_qntyinpcs[$order_id]+=$order_qntyinpcs;
				$po_array_size[$order_id]=$hideSize;
			}

			foreach($po_array as $key=>$val)
			{
				$id_proport = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
				$order_id=$key; $order_qnty=$val; $order_qntyinpcs=$po_array_qntyinpcs[$key];$hideSize=$po_array_size[$key];

				if($data_array_prop!="") $data_array_prop.= ",";

				$data_array_prop.="(".$id_proport.",".$transactionID.",2,16,".$dtls_id.",".$order_id.",".$hiddenProdId.",".$order_qnty.",".$cbo_issue_purpose.",".$hidden_is_sales.",".$order_qntyinpcs.",'".$hideSize."',".$user_id.",'".$pc_date_time."')";
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		$issue_sql = sql_select("SELECT a.issue_number, a.booking_no, b.program_no, b.prod_id, a.issue_basis, c.po_breakdown_id, c.is_sales, b.issue_qnty, c.quantity
			from inv_issue_master a, inv_grey_fabric_issue_dtls b left join order_wise_pro_details c on b.id=c.dtls_id and c.entry_form=16 and c.status_active=1
			where a.id=b.mst_id and a.company_id=$cbo_company_id and a.entry_form=16 and b.status_active=1 and b.id=$dtls_tbl_id");
		$booking_nos="";
		...
		if($ref_order_nos != "")
		{
			...
			where a.company_id=$cbo_company_id and b.prod_id=$ref_prod_id and c.entry_form=16 and a.id=b.mst_id and b.id=c.dtls_id and  c.status_active=1 $ref_order_cond and a.entry_form=16 and b.status_active=1 and b.id !=$dtls_tbl_id and c.is_sales=$ref_is_sales");
			//"and a.issue_basis=3"
		...
		elseif (!empty($issueBasisBooking) || !empty($issueBasisProgram)) 
		{
			...
			where a.id=b.mst_id and a.company_id=$cbo_company_id and b.prod_id=$ref_prod_id $ref_booking_cond and a.entry_form=16 and b.status_active=1 and a.issue_basis=1 and b.id !=$dtls_tbl_id");
			foreach ($ref_issue_sql as $val) 
			{
			...
			from inv_issue_master a, inv_grey_fabric_issue_dtls b
			where a.id=b.mst_id and a.company_id=$cbo_company_id and b.prod_id=$ref_prod_id and b.program_no in ($ref_program_nos) and a.entry_form=16 and a.issue_basis=3 and b.status_active=1 and b.id !=$dtls_tbl_id");
		...
		if( str_replace("'","",$hidden_is_batch_maintain)==2 && str_replace("'","",$cbo_issue_purpose)!=3 && str_replace("'","",$txt_batch_id)!="" )
		{
			...
			$data_array_batch_mst= "".$txt_batch_no."*16*".$txt_issue_date."*".$cbo_company_id."*".$txt_booking_id."*".$txt_booking_no."*".$booking_without_order."*".$txtIssueQnty."*'".$user_id."'*'".$pc_date_time."'";
		...
		// $cbo_basis."*".$cbo_company_id."*". $cbo_dyeing_source."*".$cbo_dyeing_com."*".
		/*#### Stop not eligible field from update operation end ####*/

		$field_array="issue_purpose*entry_form*item_category*buyer_id*style_ref*booking_id*booking_no*batch_no*issue_date*challan_no*service_booking_no*order_id*updated_by*update_date";
		$data_array="".$cbo_issue_purpose."*16*13*".$cbo_buyer_name."*".$txt_style_ref."*".$txt_booking_id."*".$txt_booking_no."*".$txt_batch_id."*".$txt_issue_date."*".$txt_challan_no."*".$txt_service_booking_no."*".$hidden_order_id."*'".$user_id."'*'".$pc_date_time."'";
		...
		$field_array_dtls="distribution_method*program_no*no_of_roll*roll_no*roll_po_id*prod_id*roll_wise_issue_qnty*issue_qnty*rate*amount*color_id*yarn_lot*yarn_count*store_name*floor_id*room*rack*self*stitch_length*remarks*style_ref*updated_by*update_date";  
		...
		$tr_data_array = "".$cbo_company_id."*".$txt_program_no."*".$hiddenProdId."*13*2*".$txt_issue_date."*".$cbo_store_name."*".$txtIssueQnty."*".$hiddenAvgRate."*'".$cons_amnt."'*".$cbo_floor_hidden."*".$cbo_room_hidden."*".$txt_rack_hidden."*".$txt_shelf_hidden."*'".$user_id."'*'".$pc_date_time."'";
		...
		$update_array = "balance_qnty*balance_amount*updated_by*update_date";
		$sql = sql_select("select a.id,a.balance_qnty,a.balance_amount,b.issue_qnty,b.rate,b.amount from inv_transaction a, inv_mrr_wise_issue_details b where a.id=b.recv_trans_id and b.issue_trans_id=$trans_tbl_id and b.entry_form=16 and a.item_category=13");
		$updateID_array = array();
		$update_data = array();
		foreach($sql as $result)
		{
		...
		$sql = sql_select("select id, cons_rate, balance_qnty, balance_amount from inv_transaction where prod_id=$hiddenProdId and $balance_cond and transaction_type in (1,4,5) and item_category=13 order by transaction_date $cond_lifofifo");
		foreach($sql as $result)
		{
			$recv_trans_id = $result[csf("id")]; // this row will be updated
			...
			if($issueQntyBalance>=0)
			{
				...
				//for insert
				if($data_array_mrr!="") $data_array_mrr .= ",";
				$data_array_mrr .= "(".$mrrWiseIsID.",".$recv_trans_id.",".$trans_tbl_id.",16,".$hiddenProdId.",".$txtIssueQnty.",'".$cons_rate."',".$amount.",'".$_SESSION['logic_erp']['user_id']."','".$pc_date_time."')";
				//for update
			...
			else if($issueQntyBalance<0)
			{
				...
				//for insert
				if($data_array_mrr!="") $data_array_mrr .= ",";
				$data_array_mrr .= "(".$mrrWiseIsID.",".$recv_trans_id.",".$trans_tbl_id.",16,".$hiddenProdId.",".$balance_qnty.",'".$cons_rate."',".$amount.",'".$_SESSION['logic_erp']['user_id']."','".$pc_date_time."')";
				//for update
				if(!in_array($recv_trans_id,$updateID_array))
				{
					$updateID_array[]=$recv_trans_id;
		...
		if(count($save_string)>0 && str_replace("'","",$save_data)!="")
		{
			...
			for($i=0;$i<count($save_string);$i++)
			{
				...
				{
					$po_array[$order_id]=$order_qnty;
					$po_array_qntyinpcs[$order_id]=$order_qntyinpcs*1;
					$po_array_size[$order_id]=$hideSize;
				}
			}
			$i=0;
			foreach($po_array as $key=>$val)
			{
				...
				$order_id=$key;
				$order_qnty=$val;
				$qntyPcs=$po_array_qntyinpcs[$key];
				$hideSizes=$po_array_size[$key];

				$data_array_prop.="(".$id_proport.",".$transactionID.",2,16,".$dtls_id.",".$order_id.",".$hiddenProdId.",".$order_qnty.",".$cbo_issue_purpose.",".$hidden_is_sales.",".$qntyPcs.",'".$hideSizes."',".$user_id.",'".$pc_date_time."')";
				$i++;
			}
			/*if($data_array_prop!="")
			{
		...
		$deleteMrr = execute_query("DELETE FROM inv_mrr_wise_issue_details WHERE issue_trans_id=$trans_tbl_id and entry_form=16");
		//****************************************** BEFORE ENTRY ADJUST START *****************************************//
			...
			<table width="880" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
				...
				<tbody>
					<tr>
						...
						<td align="center">
							<input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_supplier').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $company; ?>, 'create_mrr_search_view', 'search_div', 'grey_fabric_issue_controller', 'setFilterGrid(\'view\',-1)')" style="width:100px;" />
						</td>
					</tr>
					<tr>
...
if($action=="create_mrr_search_view")
{
	...
	if($txt_search_common!="")
	{
		if($txt_search_by==5)
		{
			$issuQntyData = sql_select("select a.mst_id, sum(b.quantity) as qty from inv_grey_fabric_issue_dtls a, order_wise_pro_details b, wo_po_break_down c where a.id=b.dtls_id and b.po_breakdown_id=c.id and b.entry_form=16 and c.job_no_mst like '%$txt_search_common' and a.status_active=1 and a.is_deleted=0 group by a.mst_id");
			$mst_ids='';
	...
	where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and a.entry_form=16 $sql_cond and b.status_active=1 and b.is_deleted=0
	...
	foreach ($result as $key => $row) 
	{
		if ($row[csf("issue_basis")]==3) // Knitting Plan Basis
		{
	...
	{
		$issue_id=implode(",", $issue_id_arr);
		$program_sql="SELECT a.id, a.issue_number_prefix_num, c.booking_no, c.dtls_id
		from inv_issue_master a, inv_grey_fabric_issue_dtls b, ppl_planning_entry_plan_dtls c
		where a.id=b.mst_id and b.PROGRAM_NO=c.DTLS_ID and a.status_active=1 and a.is_deleted=0 and a.entry_form=16 and a.company_id=$company and a.id in($issue_id)";
		$program_sql_result = sql_select( $program_sql );
		$issue_id_arr=array();
		foreach ($program_sql_result as $key => $row) 
		{
			$issue_id_arr[$row[csf("id")]].=$row[csf("booking_no")].',';
		}
	}
	?>
	<div>
			...
			<table cellspacing="0" cellpadding="0" width="927" class="rpt_table" id="view" border="1" rules="all">
				...
				foreach( $result as $row )
				{
					...
					/*if ($row[csf("is_sales")] == 1)  // for is sales
					{
                   	...
                   	//echo $issue_id_arr[$row[csf("id")]];
                   	if ($row[csf("issue_basis")]==3) 
                   	{
                   		$booking_no=implode(",", array_unique(explode(",", $issue_id_arr[$row[csf("id")]])));
                   	}
                   	else
                   	{
					...
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"  onclick="js_set_value('<? echo $row[csf("issue_number")];?>','<? echo $row[csf("is_posted_account")];?>','<? echo $row[csf("id")];?>');">
						...
						<td width="120"><p><?php echo chop($booking_no,','); ?></p></td>
						<td width="120"><p><?php
...
if($action=="populate_data_from_data")
{
	$sql = "SELECT a.id, a.issue_number, a.issue_basis, a.issue_purpose, a.entry_form, a.item_category, a.company_id, a.location_id, a.supplier_id, a.store_id, a.buyer_id, a.buyer_job_no, a.style_ref, a.booking_id, a.booking_no, a.batch_no, a.issue_date, a.sample_type, a.knit_dye_source, a.knit_dye_company, a.challan_no, a.loan_party, a.lap_dip_no, a.gate_pass_no, a.item_color, a.color_range, a.remarks, a.other_party, a.order_id, a.service_booking_no, c.is_sales, c.po_breakdown_id
		...
		where a.id=b.mst_id and a.id='$data' and a.entry_form=16 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
	...
	foreach($res as $row)
	{
		...
		echo "load_drop_down( 'requires/grey_fabric_issue_controller', ".$row[csf("knit_dye_source")]."+'**'+".$row[csf("company_id")].", 'load_drop_down_knit_com_new', 'dyeing_company_td' );\n";
		echo "$('#cbo_dyeing_com').val(".$row[csf("knit_dye_company")].");\n";
		echo "$('#cbo_dyeing_com').attr('disabled','true')".";\n";
...
if($action=="show_dtls_list_view")
{
	...
	$sql = "SELECT b.id, a.company_id,a.issue_number, a.challan_no, b.program_no, b.issue_qnty, b.color_id, b.yarn_lot, b.yarn_count, b.store_name,e.floor_id,e.room, e.rack, e.self, d.product_name_details
	    ...
	    where a.id=b.mst_id and b.prod_id=d.id and a.entry_form=16 and a.id=$data and b.trans_id = e.id and b.status_active=1 and b.is_deleted=0";
	//echo $sql;
	$result = sql_select($sql);
	...
	<div style="width:1000px;overflow-y:scroll;max-height:210px;" id="search_div" >
		<table cellspacing="0" cellpadding="0" width="982" class="rpt_table" id="view" border="1" rules="all">
			...
			foreach( $result as $row )
			{
				...
				$shelf_name=return_field_value("floor_room_rack_name", "lib_floor_room_rack_mst a,lib_floor_room_rack_dtls b","  a.floor_room_rack_id=b.shelf_id and b.store_id=$store_id and a.company_id=$company_id and b.floor_id=$floor_id and b.room_id=$room_id $rackIDcond and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 group by b.shelf_id,a.floor_room_rack_name order by a.floor_room_rack_name","floor_room_rack_name");*/

				?>
				<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"  onclick="get_php_form_data('<? echo $row[csf("id")]; ?>','populate_child_from_data_for_update','requires/grey_fabric_issue_controller');set_form_data('<? echo $row[csf("floor_id")]."**".$row[csf("room")]."**".$row[csf("rack")]."**".$row[csf("self")]."**".$floor_name."**".$room_name."**".$rack_name."**".$shelf_name; ?>');">
					...
					<td width="50"><p><?php echo $rack_name;//$floorRoomRackShelf_array[$row[csf("rack")]]; ?>&nbsp;</p></td>
					<td width="50"><p><?php echo $shelf_name;//$floorRoomRackShelf_array[$row[csf("self")]]; ?>&nbsp;</p></td>
...
if($action=="populate_child_from_data_for_update")
{
	...
	foreach($result as $row)
	{
		...
		if ($row[csf('issue_basis')]==3)  {
			$is_salesOrder = return_field_value("is_sales", "ppl_planning_info_entry_dtls", "id=$program_no");
			if ($is_salesOrder == "") $is_salesOrder = 0;
			echo "$('#hidden_is_sales').val('".$is_salesOrder."');\n";
		}
		else
		{
			$is_salesOrder = 0;
		...
		$sqlIN = sql_select("select trans_id,po_breakdown_id,quantity,quantity_pcs,coller_cuff_size from order_wise_pro_details where dtls_id=".$row[csf("id")]." and entry_form=16 and trans_type=2 and status_active=1 and is_deleted=0");
		$poWithValue="";
		$poID="";
		$transaction_id="";
		foreach($sqlIN as $res)
		{
			...
			if($poID!="") $poID .=",";
			$poWithValue .= $res[csf("po_breakdown_id")]."**".$res[csf("quantity")]."**".$res[csf("quantity_pcs")]."**".$res[csf("coller_cuff_size")];
		...
		if($poID!="")
		{
			//+'_'+'".$row[csf('issue_basis')]."+'_'+'".$row[csf('order_id')]."
			if ($row[csf('issue_purpose')]==8)
			{
				echo "load_drop_down( 'requires/grey_fabric_issue_controller', '$poID'+'_'+".$row[csf('issue_purpose')]."+'_'+'".$row[csf('color_id')]."'+'_'+'".$row[csf('issue_basis')]."'+'_'+'".$row[csf('order_id')]."'+'_'+'".$is_salesOrder."', 'load_drop_down_color','color_td');\n";
				echo "set_multiselect('cbo_color_id','0','0','','0');\n";
			}

			else if ($row[csf('issue_basis')]==3 && $is_salesOrder==1)
			{
		...
		if ($row[csf('issue_purpose')]==8 && $row[csf('issue_basis')]==3 && $is_salesOrder==2)
		{
			echo "load_drop_down( 'requires/grey_fabric_issue_controller', '$program_no'+'_'+".$row[csf('issue_purpose')]."+'_'+'".$row[csf('color_id')]."'+'_'+'".$row[csf('issue_basis')]."'+'_'+'".$row[csf('order_id')]."'+'_'+'".$is_salesOrder."', 'load_drop_down_color','color_td');\n";
			echo "set_multiselect('cbo_color_id','0','0','','0');\n";
		}
		...
		{
			//echo "get_php_form_data('".$row[csf('order_id')]."'+\"**\"+'".$row[csf('prod_id')]."'+\"**\"+'".$row[csf('company_id')]."'+\"**\"+'".$program_no."'+\"**\"+".$row[csf('issue_basis')].", \"populate_data_about_order\", \"requires/grey_fabric_issue_controller\" );";
			echo "get_php_form_data('".$row[csf('order_id')]."'+\"**\"+'".$row[csf('prod_id')]."'+\"**\"+'".$row[csf('company_id')]."'+\"**\"+'".$program_no."'+\"**\"+".$row[csf('issue_basis')]."+\"**\"+".$row[csf('store_id')]."+\"**\"+".$row[csf('floor_id')]."+\"**\"+".$row[csf('room')]."+\"**\"+".$row[csf('rack')]."+\"**\"+".$row[csf('self')]."+\"**\"+".$is_salesOrder.", \"populate_data_about_order\", \"requires/grey_fabric_issue_controller\" );";
		}
...
if ($action=="grey_fabric_issue_print")
{
	...
	$sql="SELECT a.id, a.issue_number, a.issue_number_prefix_num, a.issue_date, a.issue_basis, a.issue_purpose, a.knit_dye_source, a.knit_dye_company, a.booking_id, a.batch_no, a.buyer_id, a.challan_no, a.style_ref, a.order_id, b.program_no, a.inserted_by from  inv_issue_master a, INV_GREY_FABRIC_ISSUE_DTLS b where a.id=b.mst_id and a.id='$data[1]' and a.company_id='$data[0]' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	//echo $sql;die;
	...
	$program_nos=implode(",", $program_no_arr);
	...
	<div style="width:930px;">
		<table width="900" cellspacing="0" align="right" style="margin-bottom:10px">
			...
			<tr>
				...
				<?
				$knit_dye_company=$dataArray[0][csf('knit_dye_company')];
				$nameArray=sql_select( "select address_1,web_site,email,country_id from lib_supplier where id=$knit_dye_company");
				//$nameArrayData=sql_select( "select city,email,country_id from lib_company where id=$knit_dye_company");
				$nameArrayData=sql_select( "select plot_no,level_no,road_no,block_no,country_id,province,city,zip_code,email,website from lib_company where id=$knit_dye_company");
				//print_r($nameArray);
				foreach ($nameArrayData as $result)
				{
					$address1="";
					if($result!="") $address1=$result[csf('plot_no')].' '.$result[csf('level_no')].' '.$result[csf('road_no')].' '.$result[csf('block_no')].' '.$result[csf('city')].' '.$result[csf('zip_code')].' '.$result[csf('province')].' '.$country_arr[$result[csf('country_id')]];
				}
				foreach ($nameArray as $result)
				{
					$address="";
				...
				if ($dataArray[0][csf('knit_dye_source')]==1) echo $company_library[$dataArray[0][csf('knit_dye_company')]].'<br>'.'Address :- '.$address1; else if ($dataArray[0][csf('knit_dye_source')]==3) echo $supplier_library[$dataArray[0][csf('knit_dye_company')]].'<br>'.'Address :- '.$address; ?></td>
			...
			<tr>
				...
				<td><strong>F. Booking No:</strong></td><td width="175px">
					...
					elseif ($dataArray[0][csf('issue_basis')]==3) 
					{
						// $program_no=$dataArray[0][csf('booking_id')];
						$program_sql_data=sql_select("SELECT booking_no from ppl_planning_entry_plan_dtls where dtls_id in($program_nos) and status_active=1 and is_deleted=0");
						$booking_no='';
						foreach ($program_sql_data as $key => $row) 
						{
							$booking_no.=$row[csf('booking_no')].',';
						}
						echo chop(implode(",", array_unique(explode(",", $booking_no))),","); 
						//$booking_no=return_field_value("booking_no", "ppl_planning_entry_plan_dtls","dtls_id in($program_nos) and status_active=1 and is_deleted=0");
						//echo $booking_no;
					}
					else
					{						
			...
			<tr>
				<td>
					...
					if($dataArray[0][csf('issue_purpose')]==8)
					{
						...
						if($db_type==0)
						{
							$prodID=return_field_value("group_concat(b.prod_id) as prod_id","inv_issue_master c, inv_grey_fabric_issue_dtls a, pro_grey_prod_entry_dtls b","c.id=a.mst_id and a.prod_id=b.prod_id and a.mst_id='$data[1]' and c.entry_form=16 and c.issue_basis=1 and c.issue_purpose=8 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0","prod_id");
						}
						else
						{
							$prodID=return_field_value("LISTAGG(b.prod_id, ',') WITHIN GROUP (ORDER BY b.prod_id)  as prod_id","inv_issue_master c, inv_grey_fabric_issue_dtls a, pro_grey_prod_entry_dtls b","c.id=a.mst_id and a.prod_id=b.prod_id and a.mst_id='$data[1]' and c.entry_form=16 and c.issue_basis=1 and c.issue_purpose=8 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0","prod_id");
						}
					}
					else
					{
		...
		<div style="width:100%;">
				<table align="left" cellspacing="0" width="930"  border="1" rules="all" class="rpt_table" >
					...
					<tbody>
						...
						$color_arr = return_library_array( "select id, color_name from lib_color where status_active =1 and is_deleted=0",'id','color_name');
						$product_array=array();
						$product_sql = sql_select("select id, item_description, supplier_id,item_category_id,product_name_details, lot,gsm,dia_width,color,brand,unit_of_measure,yarn_count_id from product_details_master where item_category_id in(1,13)");
						...
						else
						{
							...
							//$mc_concat as machine_no_id,
							$sql_prod_mc_id=sql_select("SELECT a.po_breakdown_id, b.prod_id, b.machine_dia from order_wise_pro_details a, pro_grey_prod_entry_dtls b where b.id=a.dtls_id and a.entry_form in (2,22) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.po_breakdown_id, b.prod_id,b.machine_dia");
						}
						...
						$all_program_no=implode(",", $all_program_arr);
						//echo $all_program_no;
						if (!empty($all_program_arr)) 
						{
							$prog_sql = "SELECT id, machine_dia from ppl_planning_info_entry_dtls where status_active = 1 and is_deleted = 0 and id in ($all_program_no)";
							//echo $prog_sql;					
							$result_prog = sql_select($prog_sql);
							$prog_array=array();
							foreach($result_prog as $row)
							{
								$prog_array[$row[csf("id")]]['machine_dia'].=$row[csf("machine_dia")].',';
							}
						}						
						...
						foreach($sql_result as $row)
						{
							...
							if($row[csf("issue_basis")]==3)
							{
								$mc_dia=$prog_array[$row[csf("program_no")]]['machine_dia'];
							}
							else
							{
								if($row[csf("issue_purpose")]==8)
							...
							$mc_dia=implode(",", array_filter(array_unique(explode(",", chop($mc_dia,",")))));
					...
					<tfoot>
						<tr>
							...
							<td align="right"><?php echo $issue_qnty_sum=number_format($issue_qnty_sum,2); ?></td>
	...
	if($dataArray[0][csf('issue_basis')]==3)
	{
		...
		echo signature_table(17,$data[0],"900px",$template_id,10,$user_lib_name[$inserted_by]);
		?>
	</div>
	</div>
...
if ($action=="grey_fabric_issue_print_7")
{
	...
	$sql="SELECT id, issue_number, issue_number_prefix_num, issue_date, issue_basis, issue_purpose, knit_dye_source, knit_dye_company, booking_id, batch_no, buyer_id, challan_no, style_ref, order_id, inserted_by from  inv_issue_master where id='$data[1]' and company_id='$data[0]'";
	//echo $sql;die;
	$supplier_library=array(); $supplier_short_library=array();
	$supplier_data=sql_select( "select id,supplier_name,short_name from lib_supplier");
	foreach ($supplier_data as $row)
	...
	if($dataArray[0][csf("order_id")]!="")
		{
			$orSql=sql_select("select id as order_id,job_no as po_number,buyer_id from fabric_sales_order_mst where id in(".$dataArray[0][csf("order_id")].") and status_active=1 and is_deleted=0");

			if (empty($orSql)) {
				$orSql = sql_select("select po_number from wo_po_break_down where id in (".$dataArray[0][csf("order_id")].")");
			}
			$orderNumbers="";
			foreach($orSql as $key=>$val)
			{
	...
	<div style="width:930px;">
		<table width="900" cellspacing="0" align="right" style="margin-bottom:10px">
					...
					<div style="width:100%;">
						<table align="left" cellspacing="0" width="1130"  border="1" rules="all" class="rpt_table" >
							...
							<tbody>
								...
								$prog_sql = "select id, machine_dia from ppl_planning_info_entry_dtls where status_active = 1 and is_deleted = 0";
								//echo $sql;
								...
								$product_array=array();
								$product_sql = sql_select("select id, item_description, supplier_id,item_category_id,product_name_details, lot,gsm,dia_width,color,brand,unit_of_measure from product_details_master where item_category_id in(1,13)");
								foreach($product_sql as $row)
								{
									$product_array[$row[csf("id")]]['product_name_details']=$row[csf("product_name_details")];
									$product_array[$row[csf("id")]]['item_description']=$row[csf("item_description")];
								...
								// echo "<pre>";
								// print_r($machine_id_arr);die; fabric_sales_order_mst
			...
			<tr bgcolor="<? echo $bgcolor; ?>">
				...
				<td align="center"><p><? echo $product_array[$row[csf("prod_id")]]['gsm']; ?></p></td>
				<td align="center"><p><? echo $product_array[$row[csf("prod_id")]]['dia_width']; ?></p></td>
				<td align="center" style="word-break:break-all;"><p><? echo $prog_array[$row[csf("program_no")]]['machine_dia'] //$mc_dia; ?></p></td>
				<td style="word-break:break-all;"><p><? echo $color_names;//$color_arr[$row[csf("color_id")]]; ?></p></td>
				<td align="right"><? echo $row[csf("no_of_roll")]; ?></td>
				<td align="right"><? echo number_format($row[csf("issue_qnty")],2); ?></td>
				<td align="center"><p><? echo $unit_of_measurement[$product_array[$row[csf("prod_id")]]['uom']]; ?></p></td>
...
if ($action=="service_booking_popup")
{
	...
	<script>
		...
		function fnc_show()
		{
			if($("#txt_booking_no").val()=="" && $("#txt_job_no").val()=="" && $("#cbo_buyer_name").val()==0)
			{
				...
				<tbody>
					<tr class="general">
						<td> <input type="hidden" id="selected_booking">
							<?
							echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --",  $company, "load_drop_down( 'grey_fabric_issue_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
							?>
						</td>
					...
					<td>
						...
						<input name="hdn_fso_id" id="hdn_fso_id" type="hidden" style="width:70px" value="<? echo $fso_id;?>">
...
<script>
	load_drop_down( 'grey_fabric_issue_controller', <? echo $company;?>, 'load_drop_down_buyer', 'buyer_td' );
</script>
<script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
</html>
<?
...
if ($action=="create_booking_search_list_view")
{
	...
	$sql= "SELECT a.id, a.company_id, a.buyer_id, a.delivery_date, a.booking_no, a.booking_no_prefix_num, a.booking_date, a.item_category, a.fabric_source, a.supplier_id from wo_booking_mst a,wo_booking_dtls b where a.booking_no=b.booking_no and a.booking_type=3 and b.booking_type=3 and a.entry_form=229 and a.status_active=1 and a.is_deleted=0 and a.process=31 and b.status_active=1 and b.is_deleted=0 $sql_cond group by a.id, a.process, a.booking_no_prefix_num, a.booking_no ,a.booking_date,a.delivery_date, a.company_id, a.buyer_id, a.job_no, a.po_break_down_id, a.item_category, a.fabric_source, a.supplier_id";

	$sql_cond2="";
	if($booking_no!="" || ($date_form!="" &&  $date_to!=""))
	{
		if($booking_no!="") $sql_cond2 .=" and a.do_number_prefix_num=$booking_no";
		if ($date_form!="" &&  $date_to!="") $sql_cond2 .= "and a.wo_date  between '".change_date_format($date_form, "yyyy-mm-dd", "-",1)."' and '".change_date_format($date_to, "yyyy-mm-dd", "-",1)."'";
		if ($buyer_id!=0) $sql_cond2 .=" and a.buyer_id='$buyer_id'";
		if ($fso_id!="") $sql_cond2 .=" and a.po_breakdown_id=$fso_id";

		$sql .=" union all
		select a.id, a.company_id, a.buyer_id, a.delivery_date,a.do_no as booking_no,a.do_number_prefix_num as booking_no_prefix_num, a.wo_date as booking_date, 
		72 as item_category,null as fabric_source, a.dyeing_compnay_id
		from dyeing_work_order_mst a
		where a.company_id=$company_id and a.status_active=1 and is_deleted=0 $sql_cond2 ";
	}
	$sql .="order by item_category asc, id DESC";
...
if($action=="multiple_issue_no_listview")
{
	...
	where a.id=b.mst_id and b.transaction_type=2 and b.item_category=13 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form=16".$company_cond.$issue_to_cond.$issue_no_cond.$date_cond.$challan_no_cond."
...
if ($action=="multiple_issue_no_print")
{
	...
	$sql_issue = "SELECT a.id,a.company_id, a.issue_number, a.issue_basis, a.issue_purpose, a.issue_date, a.booking_id, a.booking_no, a.knit_dye_company, a.knit_dye_source, a.buyer_id, b.id as trans_id, b.requisition_no, b.pi_wo_batch_no, b.prod_id, b.transaction_date, b.store_id, b.floor_id, b.room, sum(b.cons_quantity) as issue_qnty, d.remarks, c.detarmination_id, d.yarn_lot as lot, c.yarn_count_id, c.yarn_comp_type1st, c.yarn_comp_type2nd, c.yarn_comp_percent1st, c.yarn_type, c.color, c.supplier_id, b.booking_without_order, a.service_booking_no, a.style_ref, d.color_id, d.stitch_length, d.yarn_count, d.no_of_roll, d.program_no, a.challan_no,e.is_sales
	FROM inv_issue_master a, inv_transaction b, product_details_master c, inv_grey_fabric_issue_dtls d left join order_wise_pro_details e on d.trans_id=e.trans_id and d.id=e.dtls_id and e.status_active=1 and e.is_deleted=0
	WHERE a.id=d.mst_id and a.id=b.mst_id and b.prod_id=c.id and b.id=d.trans_id and a.entry_form=16 and a.item_category=13 and a.id in(".$issue_id.") and a.status_active=1 and a.is_deleted=0 
	and b.item_category=13 and b.transaction_type=2 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 
	GROUP BY a.id, a.company_id, a.issue_number, a.issue_basis, a.issue_purpose, a.issue_date, a.booking_id, a.booking_no, a.knit_dye_company, a.knit_dye_source, a.buyer_id, b.id, b.requisition_no, b.pi_wo_batch_no, b.prod_id, b.transaction_date, b.store_id, b.floor_id, b.room, d.remarks, c.detarmination_id, d.yarn_lot, c.yarn_count_id, c.yarn_comp_type1st, c.yarn_comp_type2nd, c.yarn_comp_percent1st, c.yarn_type, c.color, c.supplier_id, b.booking_without_order, a.service_booking_no, a.style_ref, d.color_id, d.stitch_length, d.yarn_count, d.no_of_roll, d.program_no, a.challan_no,e.is_sales order by a.issue_number ASC";
	//echo $sql_issue; //
	...
	// echo "<pre>";print_r($program_no_arr);
	$program_nos = implode(",",array_unique($program_no_arr)); 
	// echo $program_nos;die;
	...
	$fso_trans_ids = implode(",",array_unique(explode(",",chop($fso_trans_id,',')))); 
	...
	if($booking_ids!="")
	{
		$sample_sql= "SELECT a.id, a.booking_no, a.company_id, c.style_ref_no, c.internal_ref from wo_non_ord_samp_booking_mst  a left join wo_non_ord_samp_booking_dtls b on a.booking_no=b.booking_no  left join sample_development_mst c on b.style_id=c.id and b.status_active=1 and b.is_deleted=0  where a.id in(".$booking_ids.") and a.entry_form_id=140 and  b.entry_form_id=140 and   a.company_id='$company'". set_user_lavel_filtering(' and a.buyer_id','buyer_id')."  and a.booking_type=4 and  a.status_active=1 and a.is_deleted=0  order by b.id desc";
		//echo $sample_sql;
		...
		if(count($sample_sql_Array)>0) 
		{
			...
			foreach ($sample_sql_Array as $row) 
			{
				...
				$booking_non_order_arr[$row[csf("id")]]['booking_no'] = $row[csf("booking_no")];
			}
	...
	if($program_nos!="")
	{
		$program_sql_data=sql_select("SELECT dtls_id, booking_no from ppl_planning_entry_plan_dtls where dtls_id in($program_nos) and status_active=1 and is_deleted=0");
		$program_booking_arr=array();
		foreach ($program_sql_data as $key => $row) 
		{
			$program_booking_arr[$row[csf("dtls_id")]] = $row[csf("booking_no")];
		}
	}
	...
	if($fso_trans_ids!="")
	{
		$fso_job_sql="SELECT a.trans_id, a.po_breakdown_id,b.po_buyer, b.po_job_no,b.job_no, c.style_ref_no, d.grouping from order_wise_pro_details a, fabric_sales_order_mst b,wo_po_details_master c, wo_po_break_down d where a.trans_id in(".$fso_trans_ids.") and a.po_breakdown_id=b.id and b.po_job_no=c.job_no and c.id=d.job_id and b.status_active=1 and b.is_deleted=0 and c.is_deleted=0 and c.status_active=1 group by a.trans_id, a.po_breakdown_id,b.po_buyer, b.po_job_no,b.job_no, c.style_ref_no, d.grouping order by b.job_no";
		//echo $fso_job_sql;

		//echo $job_sql;
		$fso_job_result_Array=sql_select($fso_job_sql);
		if(count($fso_job_result_Array)>0) 
		{
			$fso_job_alldata_arr = array();
			foreach($fso_job_result_Array as $row)
			{	
				$fso_job_alldata_arr[$row[csf("trans_id")]]['job_no']   	= $row[csf("po_job_no")];				
				$fso_job_alldata_arr[$row[csf("trans_id")]]['style_ref_no'] = $row[csf("style_ref_no")];
				$fso_job_alldata_arr[$row[csf("trans_id")]]['buyer_name']   = $row[csf("po_buyer")];
				$fso_job_alldata_arr[$row[csf("trans_id")]]['order_no']     = $row[csf("job_no")];

				if(!empty($fso_job_alldata_arr[$row[csf("trans_id")]]['grouping']))
				{
					$fso_job_alldata_arr[$row[csf("trans_id")]]['grouping'] .= ",".$row[csf("grouping")];
				}
				else
				{
					$fso_job_alldata_arr[$row[csf("trans_id")]]['grouping'] = $row[csf("grouping")];
	...
	if($dataArray[0][csf('issue_purpose')]==8)
	{
		...
		if($db_type==0)
		{
			$prodID=return_field_value("group_concat(b.prod_id) as prod_id","inv_issue_master c, inv_grey_fabric_issue_dtls a, pro_grey_prod_entry_dtls b","c.id=a.mst_id and a.prod_id=b.prod_id and a.mst_id ($issue_id) and c.entry_form=16 and c.issue_basis=1 and c.issue_purpose=8 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0","prod_id");
		}
		else
		{
			$prodID=return_field_value("LISTAGG(b.prod_id, ',') WITHIN GROUP (ORDER BY b.prod_id)  as prod_id","inv_issue_master c, inv_grey_fabric_issue_dtls a, pro_grey_prod_entry_dtls b","c.id=a.mst_id and a.prod_id=b.prod_id and a.mst_id in($issue_id) and c.entry_form=16 and c.issue_basis=1 and c.issue_purpose=8 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0","prod_id");
		}
	}
	else
	{
		if($db_type==0)
		{
			$po_id=return_field_value("group_concat(b.po_breakdown_id) as po_id","inv_grey_fabric_issue_dtls a, order_wise_pro_details b","a.id=b.dtls_id and a.mst_id in ($issue_id) and b.entry_form=16 and b.trans_type=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","po_id");
		}
		else
		{
			/* echo "select LISTAGG(b.po_breakdown_id, ',') WITHIN GROUP (ORDER BY b.po_breakdown_id) as po_id from inv_grey_fabric_issue_dtls a, order_wise_pro_details b where a.id=b.dtls_id and a.mst_id='$issue_id' and b.entry_form=16 and b.trans_type=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0"; */

			$po_id=return_field_value("LISTAGG(b.po_breakdown_id, ',') WITHIN GROUP (ORDER BY b.po_breakdown_id) as po_id","inv_grey_fabric_issue_dtls a, order_wise_pro_details b","a.id=b.dtls_id and a.mst_id in($issue_id) and b.entry_form=16 and b.trans_type=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","po_id");
		}
	}
	
	$product_sql = sql_select("select id, item_description, supplier_id,item_category_id,product_name_details, lot,gsm,dia_width,color,brand,unit_of_measure from product_details_master where item_category_id in(1,13) ".where_con_using_array($prod_id_arr, '0', 'id')."");
	$product_array=array();
	foreach($product_sql as $row)
	{
		$product_array[$row[csf("id")]]['item_description']=$row[csf("item_description")];
		...
		<div style="width:100%;">
			<table align="center" cellspacing="0" border="1" rules="all" class="rpt_table" >
				...
				foreach ($sql_issue_rslt as $row)
				{
					...
					if($row[csf('issue_basis')]==1)
					{
						if(($row[csf("issue_purpose")]==8 || $row[csf("issue_purpose")]==3 || $row[csf("issue_purpose")]==26 || $row[csf("issue_purpose")]==29 || $row[csf("issue_purpose")]==30 || $row[csf("issue_purpose")]==31) && $row[csf('issue_basis')]==1)
						{
							$fabric_booking=$booking_non_order_arr[$row[csf("booking_id")]]['booking_no'];
						}
						else
						{
							$fabric_booking=$row[csf("booking_no")];
						}
					}
					elseif ($row[csf('issue_basis')]==3) 
					{
						$fabric_booking=$program_booking_arr[$row[csf("program_no")]];
					}
					else
					{
						$fabric_booking=$row[csf("booking_no")];
					...
					if(!in_array($row[csf('issue_number')],$issue_array))
					{
						if($i!=1)
						{
							...
							?>
							<tr>
								<td colspan="19" bgcolor="#EEEEEE" style="word-break:break-all"><b>
					...
					<tr bgcolor="<? echo $bgcolor; ?>">
						...
						<td width="100" align="center">
							<? if($row[csf("is_sales")]==1)
							{
								echo $buyer_arrs[$row[csf("buyer_id")]];
							}
							else
							{
								echo $buyer_name;
						...
						<td width="130" align="center">
							...
							if($row[csf("is_sales")]==1)
							{
								echo $fso_job_alldata_arr[$row[csf("trans_id")]]['job_no'];
							}
							else
							{
						...
						<td width="130" align="center">
							...
							if($row[csf("is_sales")]==1)
							{
								echo $fso_job_alldata_arr[$row[csf("trans_id")]]['grouping'];
							}
							else
							{
								echo $job_alldata_arr[$row[csf("trans_id")]]['grouping'];
							}
							?>
						</td>
						<td width="100" align="center"><? echo $row[csf("service_booking_no")];?></td> 
						<td width="100" align="center"><? echo $fabric_booking;?></td> 
						<td width="100" align="center"><? echo $yarn_issue_purpose[$row[csf('issue_purpose')]];?></td> 
						<td width="50" align="center"><? if($row[csf("program_no")]==0) echo "&nbsp;"; else echo $row[csf("program_no")]; ?></td> 
						...
						<td width="100" align="center">
							...
							if($row[csf("is_sales")]==1)
							{
								echo $fso_job_alldata_arr[$row[csf("trans_id")]]['order_no'];
							}
							else
							{
								echo $job_alldata_arr[$row[csf("trans_id")]]['order_no'];
							}
							?>
				...
				?>
				<tr>
					<td align="right" colspan="18" ><b>Grand Total :</b></td>
			...
			<?
				...
				echo signature_table(17, $data[1], "1200px",'','',$prepared_by);
			?>
		</div>
	</div>
	<?
