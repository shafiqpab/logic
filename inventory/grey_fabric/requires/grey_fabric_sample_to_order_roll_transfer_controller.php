        ...
        <legend>Enter search words</legend>
            <table cellpadding="0" cellspacing="0" width="850" class="rpt_table" border="1" rules="all">
                ...
                <tr class="general">
                    ...
                    <td>
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_order_no').value+'_'+<? echo $cbo_company_id; ?>+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+'<? echo $type; ?>'+'_'+document.getElementById('txt_file_no').value+'_'+document.getElementById('txt_ref_no').value, 'create_po_search_list_view', 'search_div', 'grey_fabric_sample_to_order_roll_transfer_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:80px;" />
                    </td>
                </tr>
                <tr>
...
if($action=='populate_data_from_order')
{
	...
	{ 
		...
		echo "document.getElementById('txt_gmts_item').value 			= '".$gmts_item."';\n";
...
if($action=="show_dtls_list_view")
{
	...
	WHERE a.id=b.mst_id and b.id=c.dtls_id and a.entry_form in(2,22,58) and b.trans_id<>0 and c.entry_form in(2,22,58) and c.is_transfer!=6 and c.booking_without_order=1 and c.status_active=1 and c.is_deleted=0 and c.po_breakdown_id=$data";
		...
		?>
            <tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
				...
				<td align="right" style="padding-right:2px"><? echo $row[csf('qnty')]; ?>
                    ...
                    <input type="hidden" name="yarnCount[]" id="yarnCount_<? echo $i; ?>" value="<? echo $row[csf('yarn_count')]; ?>"/>					                    <input type="hidden" name="stichLn[]" id="stichLn_<? echo $i; ?>" value="<? echo $row[csf('stitch_length')]; ?>"/>					
...
if($action=="show_transfer_listview")
{
	...
	$re_trans_arr=return_library_array("select barcode_no from pro_roll_details where entry_form=183 and re_transfer=1 and status_active=1 and is_deleted=0 and mst_id=$mst_id ", "barcode_no", "barcode_no");
	...
	$transfer_arr=array();
	$transfer_dataArray=sql_select("select a.id, a.trans_id, a.to_trans_id, b.id as roll_id, b.barcode_no from inv_item_transfer_dtls a, pro_roll_details b where a.id=b.dtls_id and a.mst_id=$mst_id and b.mst_id=$mst_id and b.entry_form=183 and b.transfer_criteria=7 and b.status_active=1 and b.is_deleted=0");
	...
	WHERE a.id=b.mst_id and b.id=c.dtls_id and a.entry_form in(183) and c.entry_form in(183) and c.status_active=1 and c.is_deleted=0 and c.mst_id=$mst_id
	order by barcode_no";
        ...
        <legend>Enter search words</legend>
            <table cellpadding="0" cellspacing="0" width="550" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td>
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+<? echo $cbo_company_id; ?>, 'create_transfer_search_list_view', 'search_div', 'grey_fabric_sample_to_order_roll_transfer_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                    </td>
                </tr>
            </table>
        	<div style="margin-top:10px" id="search_div"></div> 
...
if($action=='populate_data_from_transfer_master')
{
	...
	{ 
		...
		echo "get_php_form_data('".$row[csf("from_order_id")]."','populate_data_from_sample','requires/grey_fabric_sample_to_order_roll_transfer_controller');\n";
		echo "get_php_form_data('".$row[csf("to_order_id")]."','populate_data_from_order','requires/grey_fabric_sample_to_order_roll_transfer_controller');\n";
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$update_id)=="")
		{
			...
			$new_transfer_system_id = explode("*", return_next_id_by_sequence("INV_ITEM_TRANSFER_MST_PK_SEQ", "inv_item_transfer_mst",$con,1,$cbo_company_id,"GFSTOTE",183,date("Y",time()),13 ));
			...
			//echo $txt_sam_book_id."===".$txt_order_id."===".$cbo_company_id."==".$txt_challan_no; die;

			$data_array="(".$id.",'".$new_transfer_system_id[1]."',".$new_transfer_system_id[2].",'".$new_transfer_system_id[0]."',".$cbo_company_id.",".$txt_challan_no.",".$txt_transfer_date.",183,7,0,".$txt_sam_book_id.",".$txt_order_id.",13,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		else
		{
			...
			$data_array_update=$txt_challan_no."*".$txt_transfer_date."*".$txt_sam_book_id."*".$txt_order_id."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		$field_array_roll="id, barcode_no, mst_id, dtls_id, po_breakdown_id, entry_form, qnty, roll_no, roll_id, from_roll_id, is_transfer, transfer_criteria,inserted_by, insert_date";
		...
		{ 	
			...
			$data_array_trans.="(".$id_trans.",".$transfer_update_id.",".$cbo_company_id.",".$$productId.",13,6,".$txt_transfer_date.",".$txt_sam_book_id.",12,".$$rollWgt.",".$rate.",".$amount.",".$$rack.",".$$shelf.",".$$progId.",".$$stichLn.",".$$storeId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			
			$from_trans_id=$id_trans;
			//$id_trans=$id_trans+1;
			$id_trans = return_next_id_by_sequence("INV_TRANSACTION_PK_SEQ", "inv_transaction", $con);
			$data_array_trans.=",(".$id_trans.",".$transfer_update_id.",".$cbo_company_id.",".$$productId.",13,5,".$txt_transfer_date.",".$txt_order_id.",12,".$$rollWgt.",".$rate.",".$amount.",".$$rack.",".$$shelf.",".$$progId.",".$$stichLn.",".$$storeId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			...
			$data_array_roll.="(".$id_roll.",".$$barcodeNo.",".$transfer_update_id.",".$id_dtls.",".$txt_order_id.",183,".$$rollWgt.",".$$rollNo.",".$$rollId.",".$$transRollId.",5,7,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			
			if($data_array_prop!="") $data_array_prop.= ",";
			$data_array_prop.="(".$id_prop.",".$from_trans_id.",5,183,".$id_dtls.",".$txt_order_id.",".$$productId.",".$$rollWgt.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		$rID4=sql_multirow_update("pro_roll_details","is_transfer*transfer_criteria","6*7","id",$rollIds,0);
		if($flag==1) 
		{
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		{ 	
			...
			if(str_replace("'","",$$rolltableId)>0)
			{
				...
				$data_array_update_trans[str_replace("'","",$$transIdFrom)]=explode("*",($$productId."*".$txt_transfer_date."*".$txt_sam_book_id."*".$$rollWgt."*".$rate."*".$amount."*".$$rack."*".$$shelf."*".$$progId."*".$$stichLn."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
				
				$transId_arr[]=str_replace("'","",$$transIdTo);
				$data_array_update_trans[str_replace("'","",$$transIdTo)]=explode("*",($$productId."*".$txt_transfer_date."*".$txt_order_id."*".$$rollWgt."*".$rate."*".$amount."*".$$rack."*".$$shelf."*".$$progId."*".$$stichLn."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
			...
			else
			{
				...
				if($data_array_trans!="") $data_array_trans.=",";
				$data_array_trans.="(".$id_trans.",".$update_id.",".$cbo_company_id.",".$$productId.",13,6,".$txt_transfer_date.",".$txt_sam_book_id
.",12,".$$rollWgt.",".$rate.",".$amount.",".$$rack.",".$$shelf.",".$$progId.",".$$stichLn.",".$$storeId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				
				$transIdfromProp=$id_trans;
				//$id_trans=$id_trans+1;
				$id_trans = return_next_id_by_sequence("INV_TRANSACTION_PK_SEQ", "inv_transaction", $con);
				$transIdtoProp=$id_trans;
				$data_array_trans.=",(".$id_trans.",".$update_id.",".$cbo_company_id.",".$$productId.",13,5,".$txt_transfer_date.",".$txt_order_id.",12,".$$rollWgt.",".$rate.",".$amount.",".$$rack.",".$$shelf.",".$$progId.",".$$stichLn.",".$$storeId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				...
				if($data_array_roll!="") $data_array_roll.= ",";
				$data_array_roll.="(".$id_roll.",".$$barcodeNo.",".$update_id.",".$id_dtls.",".$txt_order_id.",183,".$$rollWgt.",".$$rollNo.",".$$rollId.",".$$transRollId.",5,7,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			...
			$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
			if($data_array_prop!="") $data_array_prop.= ",";
			$data_array_prop.="(".$id_prop.",".$transIdfromProp.",5,183,".$dtlsIdProp.",".$txt_order_id.",".$$productId.",".$$rollWgt.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			//$id_prop=$id_prop+1;
		...
		if($txt_deleted_id!="")
		{
			...
			if($delBarcodeNo != "")
			{
				...
				union all 
				select a.barcode_no , b.transfer_system_id as system_no, a.entry_form, 'Transfer' as msg_source from pro_roll_details a, inv_item_transfer_mst b where a.mst_id = b.id and a.entry_form = 183 and b.entry_form = 183 and a.re_transfer = 0 and  a.status_active=1 and  b.status_active=1 and a.barcode_no in ($delBarcodeNo) and a.id not in ($rollIDDel) ");
				...
				$child_split_sql=sql_select("select barcode_no, id from pro_roll_details where roll_split_from >0 and barcode_no in ($delBarcodeNo) and entry_form = 183 order by barcode_no");
		...
		if($rollIds!="")
		{
			$rID5=sql_multirow_update("pro_roll_details","is_transfer*transfer_criteria","6*7","id",$rollIds,0);
			if($flag==1) 
			{
				if($rID5) $flag=1; else $flag=0; 
			} 
		...
		if($update_dtls_id!="")
		{
			$query = execute_query("DELETE FROM order_wise_pro_details WHERE dtls_id in(".$update_dtls_id.") and entry_form=183");
			if($flag==1) 
			{
...
<div style="width:930px;">
    ...
    <div style="width:100%;">
        <table align="right" cellspacing="0" width="900"  border="1" rules="all" class="rpt_table" >
            ...
            <tbody> 
			<?
            $sql_dtls="select a.from_prod_id, a.transfer_qnty, a.uom, a.y_count, a.brand_id, a.yarn_lot, a.to_rack, a.to_shelf, a.stitch_length, b.barcode_no, b.roll_no from inv_item_transfer_dtls a, pro_roll_details b where a.id=b.dtls_id and b.entry_form=183 and a.mst_id='$data[1]' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by b.barcode_no";
