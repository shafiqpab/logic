...
if($action=='itemTransfer_popup')
{
	...
	where a.item_category=13 and a.to_company=$company $location_cond $store_cond and a.id=b.mst_id and a.entry_form in(362) and a.transfer_criteria in(2,4) and a.is_acknowledge=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
...
if($action=='populate_data_from_transfer_master')
{
	...
	foreach ($data_array as $row)
	{
		if($row[csf("to_store")]>0){
			...
			echo "load_drop_down('requires/grey_fso_to_fso_trans_acknowledgement_controller','".$loc_com."', 'load_drop_down_store_to', 'to_store_td' );\n";
...
if($action=="show_dtls_list_view")
{
	...
	$order_name_arr=return_library_array( "select id, job_no_prefix_num from fabric_sales_order_mst where status_active=1 and is_deleted=0 and id in(".$order_id.")",'id','job_no_prefix_num');
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		else
		{
			...
			$data_array="(".$id.",371,".$challan_id.",".$cbo_company_id_to.",".$cbo_store_name_to.",".$cbo_transfer_criteria.",".$cbo_item_category.",".$txt_transfer_date.",".$txt_remarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		}
		...
		foreach ($save_string as $save_value) 
		{
			...
			$data_array_prop .= "(" . $id_prop.",".$recv_trans_id.",5,362,".$id_dtls.",".$txtOrderID.",".$productID.",".$color_id.",".$qnty.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		}
...
if($action=="show_dtls_list_view_update")
{
	$data_array=sql_select("SELECT a.id, a.company_id, a.location_id, a.challan_id, a.acknowledg_date, a.remarks,b.mst_id, b.order_id, b.prod_id, b.pi_wo_batch_no, sum(b.cons_quantity) cons_quantity, listagg(cast(b.id as varchar(4000)),',') within group(order by b.id) as trans_id, b.cons_rate, b.cons_uom, b.fabric_shade,c.trans_id from_trans_id
		...
		where a.id='$data' and a.challan_id=b.mst_id and b.id=c.to_trans_id and a.entry_form=371 and b.transaction_type=5 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1
...
if ($action=="itemAcknowle_popup")
{
	...
	<script>
		...
		function load_store()
		{
			...
			load_drop_down('grey_fso_to_fso_trans_acknowledgement_controller',cbo_company_id_to, 'load_drop_down_store_to', 'cbo_store_name_to' );
		}
	</script>
</head>
			...
			<fieldset style="width:880px;margin-left:10px">
				...
				<table cellpadding="0" cellspacing="0" width="870" class="rpt_table">
					...
					<tr class="general">
						<td>
							<?
							echo create_drop_down("cbo_transfer_criteria", 120,$item_transfer_criteria,"", 1,"-- Select --",'0',"",'','2,4');
							?>
						</td>
						<td id="to_store_td">
							<?
...
if($action=='create_transfer_search_list_view')
{
	...
	where a.challan_id=b.id and a.entry_form=371 and a.transfer_criteria in(2,4) $company_cond $transfer_criteria_cond $store_cond $system_id_cond $acknowledg_date_cond $tran_challan_cond and a.status_active=1 and a.is_deleted=0";
...
if($action=='populate_data_from_transfer_master_update')
{
	$data_array=sql_select("SELECT a.id, a.entry_form, a.challan_id, a.company_id, a.store_id, a.location_id, a.transfer_criteria, a.item_category, a.acknowledg_date, a.remarks, b.transfer_system_id 
		from inv_item_trans_acknowledgement a, inv_item_transfer_mst b 
		where a.id='$data' and a.challan_id=b.id and a.entry_form=371 and a.transfer_criteria in(2,4) and a.status_active=1 and a.is_deleted=0");
	foreach ($data_array as $row)
	{		
...
if($action=='quantity_popup')
{
	...
	<script type="text/javascript">
		...
		function fn_addRow(i) {
			...
			$('#cbo_floor_to_' + row_num).attr("onchange", 'load_room_rack_self_bin("grey_fso_to_fso_trans_acknowledgement_controller*2*cbo_room_to_' + row_num + '*' + row_num + '","room","room_td_to","<? echo $cbo_company_id_to;?>","<? echo $location;?>","<? echo $store;?>",this.value,0,"","","","","","H");');
	...
	<table cellpadding="0" width="750" cellspacing="0" border="1" id="qnty_breakdown_table" class="rpt_table" rules="all" style='margin:10px auto;'>
		...
		<tbody>
			...
			if($save_string=="") { ?>
				<tr id="tr__1">
					<td id="floor_td_to">
						<? echo create_drop_down( "cbo_floor_to_$i", 150,$lib_floor_arr,"", 1, "--Select--", 0, "load_room_rack_self_bin('grey_fso_to_fso_trans_acknowledgement_controller*2*cbo_room_to_$i*$i', 'room',this.value, '".$cbo_company_id_to."','".$location."','".$store."',this.value,'','','','','','','H');",0,"","","","","","","cbo_floor_to[]" ); ?>
					</td>
					<td id="room_td_to">
				...
				foreach ($save_string as $row) {
					...
					<tr id="tr__<? echo $i;?>">
						<td id="floor_td_to">
							<? echo create_drop_down( "cbo_floor_to_$i", 150,$lib_floor_arr,"", 1, "--Select--", $data[0], "load_room_rack_self_bin('grey_fso_to_fso_trans_acknowledgement_controller*2*cbo_room_to_1*1', 'room',this.value, '".$cbo_company_id_to."','".$location."','".$store."',this.value);",0,"","","","","","","cbo_floor_to[]" ); ?>
						</td>
						<td id="room_td_to">
							<? echo create_drop_down( "cbo_room_to_$i", 150,$lib_room_arr,"", 1, "--Select--", $data[1], "load_room_rack_self_bin('grey_fso_to_fso_trans_acknowledgement_controller*2*txt_rack_to_$i*1','rack','rack_td_to','".$cbo_company_id_to."','".$location."','".$store."','".$data[0]."',this.value);",0,"","","","","","","cbo_room_to[]" ); ?>
						</td>
						<td id="rack_td_to">
							<? echo create_drop_down( "txt_rack_to_$i", 150,$lib_rack_arr,"", 1, "--Select--", $data[2], "load_room_rack_self_bin('grey_fso_to_fso_trans_acknowledgement_controller*2*txt_shelf_to_$i*2','shelf','shelf_td_to','".$cbo_company_id_to."','".$location."','".$store."','".$data[0]."','".$data[1]."',this.value);",0,"","","","","","","txt_rack_to[]" ); ?>
						</td>
						<td id="shelf_td_to">
							<? echo create_drop_down( "txt_shelf_to_$i", 150,$lib_shelf_arr,"", 1, "--Select--", $data[3], "load_room_rack_self_bin('grey_fso_to_fso_trans_acknowledgement_controller*2*cbo_bin_to*2','bin','bin_td_to','".$cbo_company_id_to."','".$location."','".$store."','".$data[0]."','".$data[1]."','".$data[2]."',this.value);",0,"","","","","","","txt_shelf_to[]" ); ?>
						</td>
						<td>
...
{
	load_room_rack_self_bin("grey_fso_to_fso_trans_acknowledgement_controller",$data);
}
