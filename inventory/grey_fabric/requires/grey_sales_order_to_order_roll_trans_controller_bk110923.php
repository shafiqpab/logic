...
if($action=="show_transfer_requi_listview")
{
	$data=explode("**",$data);
	$mst_id=$data[0];
	$order_id=$data[1];
	...
	foreach($data_arr as $rows)
	{
		...
		<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
			...
			<td align="right" style="padding-right:2px"><? echo number_format($rows[csf('qnty')],2); ?>
				...
				<input type="hidden" name="toProductUp[]" id="toProductUp_<? echo $i;?>" value="<? echo $row[csf('prod_id')]; ?>">
				<input type="hidden" name="ItemDesc[]" id="ItemDesc_<? echo $i; ?>" value="<? echo $itemDesc; ?>"/>
				...
				<input type="hidden" name="requiDtlsId[]" id="requiDtlsId_<? echo $i; ?>" value="<? echo $rows[csf('dtls_id')]; ?>"/>
...
if($action=="create_barcode_search_list_view") //  Roll Scan/Browse Pupup List view
{
	...
	<div style="width:1620px; max-height:210px; overflow-y:scroll" id="list_container_batch" align="left">
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1600" class="rpt_table" id="tbl_list_search">
        <?
        $i=1;$total_grey_qnty=0;
		foreach ($result as $row)
        {
        	if($scanned_barcode_arr[$row[csf('barcode_no')]]=="")
        	{
				...
				<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value(<? echo $i; ?>)">
					...
					<td width="150"><p><? echo $construction_composition; ?></p></td>
					<td width="90"><p><? echo $row[csf('yarn_lot')]; ?></p></td>
					<td width="40"><p><? echo $row[csf('gsm')]; ?></p></td>
					<td width="40"><p><? echo $row[csf('dia_width')]; ?></p></td>
					<td width="100"><p><? echo $all_color; ?></p></td>
					...
					<td width="50"><p><? echo $floorRoomRackShelf_array[$row[csf('floor_id')]]; ?>&nbsp;</p></td>
					<td width="50"><p><? echo $floorRoomRackShelf_array[$row[csf('room')]]; ?>&nbsp;</p></td>
					<td width="50"><p><? echo $floorRoomRackShelf_array[$row[csf('rack')]]; ?>&nbsp;</p></td>
					<td width="50"><p><? echo $floorRoomRackShelf_array[$row[csf('self')]]; ?>&nbsp;</p></td>
    ...
    <table width="1600" cellspacing="0" cellpadding="0" border="1" rules="all"  class="rpt_table" >
    	<tr class="tbl_bottom">
	    	<td width="30"></td>
			<td width="50"></td>
			<td width="50"></td>
			...
			<td width="50" id="value_total_selected_value_td" align="right"><?php echo number_format($total_grey_qnty,2); ?></td>
			<td width="50"></td>
			<td width="50"></td>
			<td width="50"></td>
			<td width="50"></td>
		...
		<tr class="tbl_bottom">
			...
			<td width="100"></td>
			<td width="150"></td>
			<td width="130" colspan="2">Count of Selected Row</td>
			<td width="40">
...
if ($action=="to_color_popup")
{
	...
	<div style="width:290;max-height:180px; overflow-y:scroll" id="sewing_production_list_view" align="left">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="270" class="rpt_table" id="tbl_list_search">
			...
			from fabric_sales_order_mst a, fabric_sales_order_dtls b
			where a.id=b.mst_id and a.company_id=1 and a.status_active=1 and a.is_deleted=0 and b.is_deleted=0 and a.id=$txt_to_order_id";
...
if($action=="populate_barcode_for_requisition")
{
	$count_arr=return_library_array( "select id, yarn_count from lib_yarn_count",'id','yarn_count');
	$brand_arr=return_library_array( "select id, brand_name from lib_brand",'id','brand_name');
	$product_arr=return_library_array( "select id, product_name_details from product_details_master where item_category_id=13",'id','product_name_details');
	...
	WHERE a.id=b.mst_id and b.id=c.dtls_id and b.prod_id=d.id and b.trans_id<>0 and a.entry_form in(2,22,58,84) and c.entry_form in(2,22,58,84) and c.is_transfer!=6 and c.re_transfer=0 and c.status_active=1 and c.is_deleted=0 and c.po_breakdown_id=$data and c.is_service=0
	union all
	...
	WHERE a.id=b.mst_id and b.id=c.dtls_id and b.from_prod_id=d.id and a.entry_form in(133) and c.entry_form in(133) and c.re_transfer=0 and c.status_active=1 and c.is_deleted=0 and c.po_breakdown_id=$data and c.is_service=0
	order by barcode_no";
	// echo $sql;die;
    ...
    if(count($ref_barcode_arr)>0)
    {
		...
		$transfer_requ_arr=return_library_array("SELECT barcode_no, remarks from inv_item_transfer_requ_dtls where status_active=1 and is_deleted=0 $all_ref_barcode_no","barcode_no", "remarks");
		// echo $sql_qry="SELECT b.barcode_no, b.remarks from inv_item_transfer_requ_dtls b WHERE b.status_active=1 and b.is_deleted=0 $all_ref_barcode_no";
	}
...
if($action=="show_dtls_list_view")
{
	...
	$programArr=return_library_array("SELECT b.barcode_no, a.booking_id from inv_receive_master a, pro_roll_details b where a.id=b.mst_id and a.entry_form=2 and a.receive_basis=2 and b.entry_form=2 and b.barcode_no in($data) and b.status_active=1 and b.is_deleted=0", "barcode_no", "booking_id");

	$sql="SELECT a.id, a.entry_form, a.receive_basis, a.booking_id, b.prod_id, b.yarn_lot, b.color_id as color_names, b.yarn_count, b.stitch_length, b.brand_id, b.body_part_id, b.floor_id, b.room, b.rack, b.self, b.bin_box, c.id as roll_id, c.barcode_no, c.po_breakdown_id, c.roll_no, c.qnty, c.roll_id as roll_id_prev, c.rate,c.amount, 1 as type,c.booking_no,a.store_id, d.detarmination_id, d.gsm, d.dia_width
	from inv_receive_master a, pro_grey_prod_entry_dtls b, pro_roll_details c, product_details_master d
	WHERE a.id=b.mst_id and b.id=c.dtls_id and b.prod_id=d.id and b.trans_id<>0 and a.entry_form in(2,22,58,84) and c.entry_form in(2,22,58,84) and c.is_transfer!=6 and c.re_transfer=0 and c.status_active=1 and c.is_deleted=0 and c.barcode_no in($data) and c.is_service=0 and c.is_sales=1
	union all
	select a.id, a.entry_form, 0 as receive_basis, 0 as booking_id, b.from_prod_id as prod_id, b.yarn_lot, b.color_names, b.y_count as yarn_count, b.stitch_length, b.brand_id, b.to_body_part as body_part_id, b.to_floor_id as floor_id, b.to_room as room, b.to_rack as rack,  b.to_shelf as self, b.to_bin_box as bin_box, c.id as roll_id, c.barcode_no, c.po_breakdown_id, c.roll_no, c.qnty, c.roll_id as roll_id_prev, c.rate,c.amount, 2 as type, c.booking_no, b.to_store as store_id, d.detarmination_id, d.gsm, d.dia_width
	from inv_item_transfer_mst a, inv_item_transfer_dtls b, pro_roll_details c, product_details_master d
	WHERE a.id=b.mst_id and b.id=c.dtls_id and b.from_prod_id=d.id and a.entry_form in(133) and c.entry_form in(133) and c.re_transfer=0 and c.status_active=1 and c.is_deleted=0 and c.barcode_no in($data) and c.is_service=0 and c.is_sales=1
	order by barcode_no";
	// echo $sql;
	$data_array=sql_select($sql);
...
if($action=="show_transfer_listview")
{
	...
	$barcodeNos="";$ref_barcode_arr=array();
	foreach($data_arr as $vals)
	{
		$barcodeNos.=$vals[csf('barcode_no')].",";
...
if($action=='create_transfer_search_list_view')
{
	...
	$sql="select id, transfer_prefix_number, $year_field transfer_system_id, challan_no, company_id, transfer_date, transfer_criteria, item_category from inv_item_transfer_mst where item_category=13 and company_id=$company_id and $search_field like '$search_string' and transfer_criteria=$transfer_criteria and entry_form=133 and status_active=1 and is_deleted=0 order by id";
...
if($action=="save_update_delete")
{
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$update_id)=="")
		{
			...
			$field_array="id, transfer_prefix, transfer_prefix_number, transfer_system_id, company_id, challan_no, transfer_date, transfer_criteria, to_company, entry_form, from_order_id, to_order_id, transfer_requ_no, transfer_requ_id, item_category,purpose,to_color_id, inserted_by, insert_date";

			$data_array="(".$id.",'".$new_transfer_system_id[1]."',".$new_transfer_system_id[2].",'".$new_transfer_system_id[0]."',".$cbo_company_id.",".$txt_challan_no.",".$txt_transfer_date.",".$cbo_transfer_criteria.",".$cbo_to_company_id.",133,".$txt_from_order_id.",".$txt_to_order_id.",".$txt_requisition_no.",".$txt_requisition_id.",13,".$cbo_purpose_id.",".$hid_to_color_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
         ...
         */
		$field_array_update="challan_no*transfer_date*to_color_id*updated_by*update_date";
		$data_array_update=$txt_challan_no."*".$txt_transfer_date."*".$hid_to_color_id."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
