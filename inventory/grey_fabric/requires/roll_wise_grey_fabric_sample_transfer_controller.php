...
{
	load_room_rack_self_bin("requires/roll_wise_grey_fabric_sample_transfer_controller",$data);
}
...
if($action=='create_sample_requisition_search_list_view') // sample requisition Transfer System ID list view 
{
	...
	where a.id=b.mst_id and a.item_category=13 and a.company_id=$company_id and a.transfer_criteria=$transfer_criteria  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form=$entry_form_no and a.transfer_requ_id IS NOT NULL
...
{
	$data_array=sql_select("SELECT a.transfer_criteria, a.transfer_system_id, a.challan_no, a.company_id, a.to_company, transfer_date, a.item_category, a.from_order_id, a.to_order_id, a.from_samp_dtls_id, a.to_samp_dtls_id, b.body_part_id, b.to_body_part from inv_item_transfer_requ_mst a, inv_item_transfer_requ_dtls b where a.id=b.mst_id and a.id='$data'");
	...
	{ 
		...
		echo "get_php_form_data('".$from_order_book_id."**"."from"."**".$row[csf("transfer_criteria")]."**".$row[csf("body_part_id")]."','populate_data_from_sample','requires/roll_wise_grey_fabric_sample_transfer_controller');\n";
		echo "get_php_form_data('".$to_order_book_id."**".$row[csf("transfer_criteria")]."**".$row[csf("to_body_part")]."','populate_data_to_order','requires/roll_wise_grey_fabric_sample_transfer_controller');\n";
		...
		echo "load_room_rack_self_bin('requires/roll_wise_grey_fabric_sample_transfer_controller*13*cbo_store_name_to', 'store','to_store_td', $('#cbo_company_id_to').val());\n";
...
if($action=="show_sample_requisition_transfer_listview") // sample_requisition Dtls list view
{
	...
	{
		$sql2="SELECT b.body_part_id, b.yarn_rate, b.kniting_charge, c.rate, c.barcode_no
		from pro_grey_prod_entry_dtls b, pro_roll_details c
		WHERE b.id=c.dtls_id and b.trans_id<>0 and c.entry_form in(2,22,58,84) and c.re_transfer=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and c.is_sales=0 and c.booking_without_order=0 $all_barcode_no_cond
		union all
		 select b.to_body_part as body_part_id, b.yarn_rate, b.kniting_charge, c.rate, c.barcode_no
		from inv_item_transfer_dtls b, pro_roll_details c
		WHERE b.id=c.dtls_id and c.entry_form in(82,83,183) and c.re_transfer=0 and c.status_active=1 and c.is_deleted=0 and c.booking_without_order=0 $all_barcode_no_cond";
	}
	else // sample
	{
		// Sample to Sample
		...
		WHERE b.id=c.dtls_id and b.trans_id<>0 and c.entry_form in(2,22,58,84) and c.booking_without_order=1 and c.status_active=1 and c.is_deleted=0 and c.re_transfer =0 $all_barcode_no_cond
		UNION ALL
		SELECT b.to_body_part as body_part_id, b.yarn_rate, b.kniting_charge, c.rate, c.barcode_no
		from inv_item_transfer_dtls b, pro_roll_details c
		where b.id=c.dtls_id and c.entry_form in(110,180) and c.status_active=1 and c.is_deleted=0 and c.booking_without_order=1 and c.re_transfer =0 $all_barcode_no_cond
		UNION ALL
		SELECT b.to_body_part as body_part_id, b.yarn_rate, b.kniting_charge, c.rate, c.barcode_no
		from inv_item_transfer_dtls b, pro_roll_details c
		where b.id=c.dtls_id and c.entry_form in(82) and c.status_active=1 and c.is_deleted=0 and c.booking_without_order=1 and c.re_transfer=0 $all_barcode_no_cond";
	}
	// echo $sql2;
	...
	{
		$barcod_data_arr[$row[csf('barcode_no')]]['yarn_rate'] =$row[csf('yarn_rate')];
		$barcod_data_arr[$row[csf('barcode_no')]]['rate'] =$row[csf('rate')];
		$barcod_data_arr[$row[csf('barcode_no')]]['kniting_charge'] =$row[csf('kniting_charge')];
	}
	...
	{  
		...
		$yarn_rate=$barcod_data_arr[$row[csf('barcode_no')]]['yarn_rate'];
		$rate=$barcod_data_arr[$row[csf('barcode_no')]]['rate'];
		$kniting_charge=$barcod_data_arr[$row[csf('barcode_no')]]['kniting_charge'];
		        ...
		        <legend>Enter search words</legend>
		            <table cellpadding="0" cellspacing="0" width="800" class="rpt_table">
		                ...
		                <tr class="general">
		                    ...
		                    <td>
		                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_sample_no').value+'_'+<? echo $cbo_company_id; ?>+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $cbo_transfer_criteria; ?>, 'create_sample_search_list_view', 'search_div', 'roll_wise_grey_fabric_sample_transfer_controller', 'setFilterGrid(\'tbl_list_sample\',-1);')" style="width:100px;" />
		                    </td>
		                </tr>
		                <tr>
		        ...
		        <legend>Enter search words</legend>
		            <table cellpadding="0" cellspacing="0" width="800" class="rpt_table">
		                ...
		                <tr class="general">
		                    ...
		                    <td>
		                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_sample_no').value+'_'+<? echo $cbo_company_id; ?>+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $txt_from_order_book_no;?>, 'create_sample_search_to_list_view', 'search_div', 'roll_wise_grey_fabric_sample_transfer_controller', 'setFilterGrid(\'tbl_list_sample\',-1);')" style="width:100px;" />
		                    </td>
		                </tr>
		                <tr>
		                    <td colspan="4" align="center" height="40" valign="middle"><? echo load_month_buttons(1); ?></td>
...
if($action=='create_sample_search_to_list_view')
{
	...
	from wo_non_ord_samp_booking_mst a, wo_non_ord_samp_booking_dtls b 
	where a.booking_no=b.booking_no  and a.booking_type=4 and a.company_id=$company_id and a.buyer_id like '$buyer' and b.booking_no like '$search_string' and a.status_active=1 and a.is_deleted=0 and b.is_deleted=0 $booking_date $from_booking_chk order by a.id, b.id";
	
	//echo  $sql;//die;
	 
	echo create_list_view("tbl_list_sample", "Booking No,Year,Company,Buyer Name,Style Ref. No,Sample,Body Part,Style Description,Fab. Color,Booking Date, Grey Qnty", "60,60,80,80,100,100,130,160,80,80,60","1100","200",0, $sql , "js_set_value", "id,body_part", "", 1, "0,0,company_id,buyer_id,style_id,sample_type,body_part,0,fabric_color,0,0", $arr , "booking_no_prefix_num,year,company_id,buyer_id,style_id,sample_type,body_part,fabric_description,fabric_color,booking_date,grey_fabric", "",'','0,0,0,0,0,0,0,0,0,3');
...
if($action=="show_dtls_list_view")
{
	...
	if ($transfer_criteria==6) // order
	{
		...
		WHERE a.id=b.mst_id and b.id=c.dtls_id and b.to_prod_id = d.id  and c.entry_form in(82,83,183) and c.re_transfer=0 and c.status_active=1 and c.is_deleted=0 and c.po_breakdown_id=$booking_order_id and b.to_body_part=$body_part_id and c.booking_without_order=0";
	}
	else // sample
	{		
		// Sample to Order sql
		...
		$sql="SELECT a.id, a.entry_form, a.receive_basis, a.booking_id, b.prod_id, b.yarn_lot, b.yarn_count, b.stitch_length, b.brand_id, b.floor_id, b.room, b.rack, b.self, c.id as roll_id, c.barcode_no, c.po_breakdown_id, c.roll_no, c.qnty, c.roll_id as roll_id_prev, 1 as type, a.store_id, d.gsm, d.dia_width, d.detarmination_id, d.item_description, d.product_name_details, b.body_part_id, b.yarn_rate, b.kniting_charge, c.rate
		from inv_receive_master a, pro_grey_prod_entry_dtls b, pro_roll_details c, product_details_master d
		WHERE a.id=b.mst_id and b.id=c.dtls_id and b.prod_id = d.id and a.entry_form in(2,22,58,84) and b.trans_id<>0 and c.entry_form in(2,22,58,84) and c.booking_without_order=1 and c.status_active=1 and c.is_deleted=0 and c.po_breakdown_id=$booking_order_id and b.body_part_id=$body_part_id and c.re_transfer =0
		...
		and a.to_order_id =$booking_order_id and b.to_body_part=$body_part_id  and c.booking_without_order=1 and c.re_transfer =0
	...
	if(count($barcod_NOs_arr)>0)
	{
		...
		$barcodeData = sql_select("select barcode_no,entry_form from pro_roll_details where entry_form=61 and status_active=1 and is_deleted=0 and is_returned = 0 $all_barcode_no_cond");
		...
		if($issued_barcode_arr[$row[csf('barcode_no')]]=="")
		{
			...
			<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
				...
				<td align="right" style="padding-right:2px"><? echo $row[csf('qnty')]; ?>
                	...
                	<input type="hidden" name="constructCompo[]" id="constructCompo_<? echo $i; ?>" value="<? echo $row[csf('item_description')]; ?>"/>
                	<input type="hidden" name="bodyPart[]" id="bodyPart_<? echo $i; ?>" value="<? echo $row[csf('body_part_id')]; ?>"/>
                	<input type="hidden" name="rollRate[]" id="rollRate_<? echo $i; ?>" value="<? echo $row[csf('rate')]; ?>"/>
                    <input type="hidden" name="yarnRate[]" id="yarnRate_<? echo $i; ?>" value="<? echo $row[csf('yarn_rate')]; ?>"/>
                    <input type="hidden" name="knittingCharge[]" id="knittingCharge_<? echo $i; ?>" value="<? echo $row[csf('kniting_charge')]; ?>"/>
                </td>
			</tr> 
			<? 
			$i++; 
...
if($action=="show_transfer_listview")
{
	...
	$sql="SELECT a.id, a.entry_form, 0 as receive_basis, 0 as booking_id, b.from_prod_id, to_prod_id as prod_id,  b.yarn_lot, b.y_count as yarn_count, b.stitch_length, b.brand_id, b.floor_id, b.room, b.rack as rack, b.shelf as self, c.id as roll_id, c.barcode_no, c.po_breakdown_id, c.roll_no, c.qnty, c.roll_id as roll_id_prev, 3 as type , b.to_store as store_id, transfer_requ_dtls_id, d.gsm, d.dia_width, d.item_description, d.product_name_details, d.detarmination_id, b.yarn_rate, b.kniting_charge, c.rate, b.from_program
	from inv_item_transfer_mst a, inv_item_transfer_dtls b, pro_roll_details c, product_details_master d
	...
	{  
		if($issued_barcode_arr[$row[csf('barcode_no')]]=="")
		{
			...
			/*else if($row[csf('entry_form')]==110)
			{
        ...
        <legend>Enter search words</legend>
            <table cellpadding="0" cellspacing="0" width="550" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td>
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+<? echo $cbo_company_id; ?>+'_'+<? echo $cbo_transfer_criteria; ?>, 'create_transfer_search_list_view', 'search_div', 'roll_wise_grey_fabric_sample_transfer_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                    </td>
                </tr>
            </table>
        	<div style="margin-top:10px" id="search_div"></div> 
...
if($action=='populate_data_from_transfer_master')
{
	...
	$data_array=sql_select("SELECT a.transfer_criteria, a.transfer_system_id,a.challan_no, a.company_id, a.to_company, a.transfer_date, a.item_category, a.from_order_id, a.to_order_id, a.from_samp_dtls_id, a.to_samp_dtls_id, a.transfer_requ_no, a.transfer_requ_id , b.to_store, b.to_floor_id, b.to_room, b.to_rack, b.to_shelf, b.body_part_id, b.to_body_part from inv_item_transfer_mst a, inv_item_transfer_dtls b where a.id = b.mst_id and a.id='$data' group by a.transfer_criteria, a.transfer_system_id,a.challan_no, a.company_id, a.to_company, a.transfer_date, a.item_category, a.from_order_id, a.to_order_id, a.from_samp_dtls_id, a.to_samp_dtls_id, a.transfer_requ_no, a.transfer_requ_id , b.to_store, b.to_floor_id,b.to_room, b.to_rack, b.to_shelf, b.body_part_id, b.to_body_part");
	...
	{ 
		...
		echo "get_php_form_data('".$from_order_book_id."**"."from"."**".$row[csf("transfer_criteria")]."**".$row[csf("body_part_id")]."','populate_data_from_sample','requires/roll_wise_grey_fabric_sample_transfer_controller');\n";
		echo "get_php_form_data('".$to_order_book_id."**".$row[csf("transfer_criteria")]."**".$row[csf("to_body_part")]."','populate_data_to_order','requires/roll_wise_grey_fabric_sample_transfer_controller');\n";
		/*echo "get_php_form_data('".$row[csf("from_order_id")]."**"."from"."**".$row[csf("transfer_criteria")]."','populate_data_from_sample','requires/roll_wise_grey_fabric_sample_transfer_controller');\n";
		echo "get_php_form_data('".$row[csf("to_order_id")]."**".$row[csf("transfer_criteria")]."','populate_data_to_order','requires/roll_wise_grey_fabric_sample_transfer_controller');\n";*/
		echo "$('#cbo_transfer_criteria').attr('disabled','disabled');\n";
		echo "$('#cbo_company_id').attr('disabled','disabled');\n";
		echo "$('#cbo_company_id_to').attr('disabled','disabled');\n";
		echo "$('#txt_from_order_book_no').attr('disabled','disabled');\n";
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		$field_array_dtls="id, mst_id, trans_id, to_trans_id, from_prod_id, to_prod_id, item_category, transfer_qnty, roll, rate, transfer_value, uom, y_count, brand_id, yarn_lot, floor_id, room, rack, shelf, to_floor_id, to_room, to_rack, to_shelf, from_program, to_program, stitch_length, from_store, to_store, transfer_requ_dtls_id, active_dtls_id_in_transfer, inserted_by, insert_date, body_part_id, to_body_part, yarn_rate, kniting_charge";

		$field_array_roll="id, barcode_no, mst_id, dtls_id, po_breakdown_id, entry_form, qnty, roll_no, roll_id, from_roll_id, is_transfer, transfer_criteria, booking_without_order, booking_no, re_transfer, inserted_by, insert_date, rate, amount";
		...
		for($j=1;$j<=$total_row;$j++)
		{
			...
			{
				$row_prod = sql_select("select id, current_stock, avg_rate_per_unit, stock_value from product_details_master where company_id=$cbo_company_id_to and item_category_id=13 and detarmination_id=".$$febDescripId." and gsm=".$$gsm." and dia_width='".str_replace("'", "", strtoupper($$diaWidth))."' and status_active=1 and is_deleted=0");
				if (count($row_prod) > 0 || $new_prod_ref_arr[$cbo_company_id_to."**".$$febDescripId."**".$$gsm."**".strtoupper($$diaWidth)."**13"] != "")
				{
					if(count($row_prod) > 0)
					{
					...
					{
						$new_prod_id = $new_prod_ref_arr[$cbo_company_id_to."**".$$febDescripId."**".$$gsm."**".strtoupper($$diaWidth)."**13"];
						$product_id_insert_parameter[$new_prod_id."**".$$febDescripId."**".$$gsm."**".strtoupper($$diaWidth)."**".$$constructCompo."**13"]+=str_replace("'", "", $$rollWgt);
	           	...
	           	else
	           	{
	           		...
	           		$new_prod_ref_arr[$cbo_company_id_to."**".$$febDescripId."**".$$gsm."**".strtoupper($$diaWidth)."**13"] = $new_prod_id;
	           		$product_id_insert_parameter[$new_prod_id."**".$$febDescripId."**".$$gsm."**".strtoupper($$diaWidth)."**".$$constructCompo."**13"]+=str_replace("'", "", $$rollWgt);
	           		$product_id_insert_amount[$new_prod_id."**".$$febDescripId."**".$$gsm."**".strtoupper($$diaWidth)."**".$$constructCompo."**13"]+=$cons_amount;
	           	}
	        }
			...
			$data_array_trans.="(".$id_trans.",".$transfer_update_id.",".$cbo_company_id.",".$$productId.",13,6,".$txt_transfer_date.",".$txt_from_order_book_id.",12,".$$rollWgt.",'".$cons_rate."','".$cons_amount."',".$$floor.",".$$room.",".$$rack.",".$$shelf.",".$$progId.",".$$stichLn.",".$$storeId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$cbo_from_body_part."')";
			...
			if($variable_auto_rcv!=2) // if auto receive yes, then no need to acknowledgement
			{
				...
				$data_array_trans.=",(".$recv_trans_id.",".$transfer_update_id.",".$cbo_company_id.",".$new_prod_id.",13,5,".$txt_transfer_date.",".$txt_to_order_book_id.",12,".$$rollWgt.",'".$cons_rate."','".$cons_amount."',".$cbo_floor_to.",".$cbo_room_to.",".$txt_rack_to.",".$txt_shelf_to.",".$$progId.",".$$stichLn.",".$cbo_store_name_to.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$cbo_to_body_part."')";
			}

			if($data_array_dtls!="") $data_array_dtls.=",";
			$data_array_dtls.="(".$id_dtls.",".$transfer_update_id.",".$from_trans_id.",".$recv_trans_id.",".$$productId.",".$new_prod_id.",13,".$$rollWgt.",".$$rollNo.",".$cons_rate.",".$cons_amount.",12,".$$yarnCount.",".$$brandId.",".$$yarnLot.",".$$floor.",".$$room.",".$$rack.",".$$shelf.",".$cbo_floor_to.",".$cbo_room_to.",".$txt_rack_to.",".$txt_shelf_to.",".$$progId.",".$$progId.",".$$stichLn.",".$$storeId.",".$cbo_store_name_to.",".$$requiDtlsId.",1,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$cbo_from_body_part."','".$cbo_to_body_part."','".$$yarnRate."','".$$knittingCharge."')";	
			...
			$data_array_roll.="(".$id_roll.",".$$barcodeNo.",".$transfer_update_id.",".$id_dtls.",".$txt_to_order_book_id.",".$entry_form_no.",".$$rollWgt.",".$$rollNo.",".$$rollId.",".$$transRollId.",5,".$cbo_transfer_criteria.",".$booking_without_order.",".$txt_to_order_book_no.",".$re_transfer.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$cons_rate."','".$cons_amount."')";

			if ($cbo_transfer_criteria==6) // Order to Sample
			{
				if($data_array_prop!="") $data_array_prop.= ",";
				$data_array_prop.="(".$id_prop.",".$from_trans_id.",6,110,".$id_dtls.",".$txt_from_order_book_id.",".$$productId.",".$$rollWgt.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			}
			...
			if ($cbo_transfer_criteria==7) // Sample to Order
			{
				...
				{
					if($data_array_prop!="") $data_array_prop.= ",";
					$data_array_prop.="(".$id_prop.",".$recv_trans_id.",5,183,".$id_dtls.",".$txt_to_order_book_id.",".$new_prod_id.",".$$rollWgt.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				}
			}

			$prodData_array[str_replace("'", "",$$productId)]['qnty']+=str_replace("'", "", $$rollWgt);
			$prodData_array[str_replace("'", "",$$productId)]['amount']+=str_replace("'", "", $cons_amount);
		...
		if(!empty($product_id_insert_parameter))
		{
			foreach ($product_id_insert_parameter as $key => $val)
			{
				...
				if($data_array_prod_insert!="") $data_array_prod_insert.=",";
               	$data_array_prod_insert .= "(" . $prod_id . "," . $cbo_company_id_to . "," . $cbo_store_name_to . ",13," . $fabric_desc_id . ",'" . $cons_compo . "','" . $prod_name_dtls . "'," . "12" . "," . $avg_rate_per_unit . "," . $product_quantity . "," . $product_quantity . "," . $roll_amount . "," . $txt_gsm . ",'" . $txt_width . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
			}
		}
		...
		{
			$issue_qty=$prodData_array[$row[csf('id')]]['qnty'];
			$issue_amount=$prodData_array[$row[csf('id')]]['amount'];
		...
		die;*/
		//echo "10**".bulk_update_sql_statement( "product_details_master", "id", $field_array_prod_update, $data_array_prod_update, $prod_id_array );die;
		// echo "10**$rID##$rID2##$rID3##$rID4##$rID5##$rID6##$rID7##$rID8##$prodUpdate##$flag";oci_rollback($con);die;
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		if($all_barcodeNo!="")
		{
			...
			$issue_data_refer = sql_select("select a.id, a.barcode_no, b.issue_number from pro_roll_details a, inv_issue_master b where a.mst_id = b.id and a.entry_form = 61 $all_barcode_no_cond  and a.status_active = 1 and a.is_deleted = 0 and a.is_returned=0"); //and a.barcode_no in ($all_barcodeNo)
			if($issue_data_refer[0][csf("barcode_no")] != "")
			{
				echo "20**Sorry Barcode No : ". $issue_data_refer[0][csf("barcode_no")] ."\nFound in Issue No ".$issue_data_refer[0][csf("issue_number")];
				disconnect($con);
			...
			$current_transfer_sql = sql_select("select a.barcode_no, b.transfer_system_id as system_id, a.roll_split_from from pro_roll_details a, inv_item_transfer_mst b where a.mst_id=b.id and a.entry_form in (83,82,110,180,183) $all_barcode_no_cond and a.status_active=1 and a.is_deleted=0 and a.re_transfer=0
			union all 
			select a.barcode_no, b.recv_number as system_id, a.roll_split_from from pro_roll_details a, inv_receive_master b where a.mst_id=b.id and a.entry_form in (84) $all_barcode_no_cond and a.status_active=1 and a.is_deleted=0 and a.re_transfer=0");
			...
			if (!empty($saved_roll_arr)) // Saved barcode to next transaction found
			{
				foreach ($saved_roll_arr as $barcode => $saved_roll_id) 
				{
					if ($saved_roll_id != $next_transfer_arr[$barcode]) 
					{
						...
						{
							echo "20**Sorry Barcode No : ". $barcode ." \nFound in Transfer/Return No : ".$next_transfer_ref[$barcode]["transfer_no"].'='.$saved_roll_id .'!='. $next_transfer_arr[$barcode];
							disconnect($con);
							die;
						}
					}
			...
			$child_split_sql=sql_select("select a.barcode_no, a.id from pro_roll_details a where roll_split_from >0 $all_barcode_no_cond and a.entry_form = 82 order by a.barcode_no");
			...
			if($all_dtlsId !="")
			{
				...
				$deleted_dtls=sql_select("select b.barcode_no, a.id from inv_item_transfer_dtls a, pro_roll_details b where a.id=b.dtls_id and b.entry_form=82 $all_dtls_id_cond and a.status_active=0 and a.is_deleted=1");
		...
		//$id_trans=return_next_id( "id", "inv_transaction", 1 ) ;
		$field_array_trans="id, mst_id, company_id, prod_id, item_category, transaction_type, transaction_date, order_id, cons_uom, cons_quantity, cons_rate, cons_amount, floor_id, room, rack, self, program_no, stitch_length,store_id, inserted_by, insert_date, body_part_id";
		$field_array_trans_update="prod_id*transaction_date*order_id*cons_quantity*cons_rate*cons_amount*floor_id*room*rack*self*program_no*stitch_length*updated_by*update_date*body_part_id";
		
		//$id_dtls=return_next_id( "id", "inv_item_transfer_dtls", 1 ) ; 
		$field_array_dtls="id, mst_id, trans_id, to_trans_id, from_prod_id, to_prod_id, item_category, transfer_qnty, roll, rate, transfer_value, uom, y_count, brand_id, yarn_lot, floor_id, room, rack, shelf, to_floor_id, to_room, to_rack, to_shelf, from_program, to_program, stitch_length, from_store, to_store, transfer_requ_dtls_id, active_dtls_id_in_transfer, inserted_by, insert_date, body_part_id, to_body_part, yarn_rate, kniting_charge";		
		$field_array_dtls_update="from_prod_id*to_prod_id*transfer_qnty*roll*rate*transfer_value*y_count*brand_id*yarn_lot*floor_id*room*rack*shelf*to_floor_id*to_room*to_rack*to_shelf*from_program*to_program*stitch_length*updated_by*update_date*body_part_id*to_body_part";
		
		//$id_roll = return_next_id( "id", "pro_roll_details", 1 );
		$field_array_roll="id, barcode_no, mst_id, dtls_id, po_breakdown_id, entry_form, qnty, roll_no, roll_id, from_roll_id, is_transfer, transfer_criteria, booking_without_order, booking_no, inserted_by, insert_date, rate, amount";
		...
		for($j=1;$j<=$total_row;$j++)
		{
			...
			if(str_replace("'","",$$rolltableId)>0)
			{
				...
				$data_array_update_trans[str_replace("'","",$$transIdFrom)]=explode("*",($$fromProductUp."*".$txt_transfer_date."*".$txt_from_order_book_id."*".$$rollWgt."*".$rate."*".$amount."*".$$floor."*".$$room."*".$$rack."*".$$shelf."*".$$progId."*".$$stichLn."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*'".$cbo_from_body_part."'"));
				
				if($variable_auto_rcv != 2 )
				{
					$transId_arr[]=str_replace("'","",$$transIdTo);
					$data_array_update_trans[str_replace("'","",$$transIdTo)]=explode("*",($$productId."*".$txt_transfer_date."*".$txt_to_order_book_id."*".$$rollWgt."*".$rate."*".$amount."*".$cbo_floor_to."*".$cbo_room_to."*".$txt_rack_to."*".$txt_shelf_to."*".$$progId."*".$$stichLn."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*'".$cbo_to_body_part."'"));
				}
				
				$dtlsId_arr[]=str_replace("'","",$$dtlsId);
				$data_array_update_dtls[str_replace("'","",$$dtlsId)]=explode("*",($$fromProductUp."*".$$productId."*".$$rollWgt."*".$$rollNo."*".$rate."*".$amount."*".$$yarnCount."*".$$brandId."*".$$yarnLot."*".$$floor."*".$$room."*".$$rack."*".$$shelf."*".$cbo_floor_to."*".$cbo_room_to."*".$txt_rack_to."*".$txt_shelf_to."*".$$progId."*".$$progId."*".$$stichLn."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*'".$cbo_from_body_part."'*'".$cbo_to_body_part."'"));
			...
			else
			{
				...
				if($cbo_company_id != $cbo_company_id_to)
				{
					...
					$prodData_array_amount[$$productId]+=$cons_amount;
					$all_prod_id.=$$productId.",";
	            }
	            else
	            {
				...
				if($data_array_trans!="") $data_array_trans.=",";
				$data_array_trans.="(".$id_trans.",".$update_id.",".$cbo_company_id.",".$$fromProductUp.",13,6,".$txt_transfer_date.",".$txt_from_order_book_id
				.",12,".$$rollWgt.",'".$cons_rate."','".$cons_amount."',".$$floor.",".$$room.",".$$rack.",".$$shelf.",".$$progId.",".$$stichLn.",".$$storeId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$cbo_from_body_part."')";
				...
				if($variable_auto_rcv!=2)
				{
					...
					$data_array_trans.=",(".$transIdtoProp.",".$update_id.",".$cbo_company_id.",".$new_prod_id.",13,5,".$txt_transfer_date.",".$txt_to_order_book_id.",12,".$$rollWgt.",'".$cons_rate."','".$cons_amount."',".$cbo_floor_to.",".$cbo_room_to.",".$txt_rack_to.",".$txt_shelf_to.",".$$progId.",".$$stichLn.",".$cbo_store_name_to.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$cbo_to_body_part."')";
				}
				if($data_array_dtls!="") $data_array_dtls.=",";
				$data_array_dtls.="(".$id_dtls.",".$update_id.",".$transIdfromProp.",".$transIdtoProp.",".$$fromProductUp.",".$new_prod_id.",13,".$$rollWgt.",".$$rollNo.",".$cons_rate.",".$cons_amount.",12,".$$yarnCount.",".$$brandId.",".$$yarnLot.",".$$floor.",".$$room.",".$$rack.",".$$shelf.",".$cbo_floor_to.",".$cbo_room_to.",".$txt_rack_to.",".$txt_shelf_to.",".$$progId.",".$$progId.",".$$stichLn.",".$$storeId.",".$cbo_store_name_to.",".$$requiDtlsId.",1,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$cbo_from_body_part."','".$cbo_to_body_part."','".$$yarnRate."','".$$knittingCharge."')";
				...
				if($data_array_roll!="") $data_array_roll.= ",";
				$data_array_roll.="(".$id_roll.",".$$barcodeNo.",".$update_id.",".$id_dtls.",".$txt_to_order_book_id.",".$entry_form_no.",".$$rollWgt.",".$$rollNo.",".$$rollId.",".$$transRollId.",5,".$cbo_transfer_criteria.",".$booking_without_order.",".$txt_to_order_book_no.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$cons_rate."','".$cons_amount."')";
				$dtlsIdProp=$id_dtls;
				$all_trans_roll_id.=$$transRollId.",";
			...
			{
				if($data_array_prop!="") $data_array_prop.= ",";
				$data_array_prop.="(".$id_prop.",".$transIdfromProp.",6,110,".$dtlsIdProp.",".$txt_from_order_book_id.",".$$fromProductUp.",".$$rollWgt.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			}
			...
			if ($cbo_transfer_criteria==7) // Sample to Order
			{
				...
				{
					if($data_array_prop!="") $data_array_prop.= ",";
					$data_array_prop.="(".$id_prop.",".$transIdtoProp.",5,183,".$dtlsIdProp.",".$txt_to_order_book_id.",".$new_prod_id.",".$$rollWgt.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				}
			}
		...
		if(!empty($product_id_insert_parameter))
		{
			foreach ($product_id_insert_parameter as $key => $val)
			{
				...
				if($data_array_prod_insert!="") $data_array_prod_insert.=",";
               	$data_array_prod_insert .= "(" . $prod_id . "," . $cbo_to_company_id . "," . $cbo_store_name . ",13," . $fabric_desc_id . ",'" . $cons_compo . "','" . $prod_name_dtls . "'," . "12" . "," . $avg_rate_per_unit . "," . $product_quantity . "," . $product_quantity . "," . $roll_amount . "," . $txt_gsm . ",'" . $txt_width . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
		...
		// echo "10**5**$flag";die;
		echo "10**".$rID.'**'.$rID2.'**'.$rID3.'**'.$rID4.'**'.$rIDinv.'**'.$rIDDtls.'**'.$rIDRoll.'**'.$rID7.'**'.$query.'**'.$rIDProp.'**'.$rID9.'**'.$prodUpdate;
		oci_rollback($con);
		die; 
