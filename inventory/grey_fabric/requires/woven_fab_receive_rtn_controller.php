	...
	<table width="750" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
            ...
            <tbody>
                <tr class="general">
                    ...
                    <td align="center">
                        <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $company; ?>, 'create_mrr_search_list_view', 'search_div', 'woven_fab_receive_rtn_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />
                    </td>
            </tr>
        	<tr>
...
if($action=="populate_data_from_data")
{
	$sql = "select id,recv_number,entry_form,company_id,receive_basis,receive_purpose,lc_no,knitting_source,knitting_company
			...
			where id='$data' and entry_form in(23)";
	//echo $sql;
	$res = sql_select($sql);
	foreach($res as $row)
	{
		...
		echo "load_drop_down( 'requires/woven_fab_receive_rtn_controller', $kniting_company+'_'+$kniting_source+'_'+$company_id, 'load_drop_down_knitting_com','knitting_com');\n";
		...
		echo"show_list_view('".$row[csf("id")]."','show_product_listview','list_product_container','requires/woven_fab_receive_rtn_controller','');\n";
   	}
	exit();
}
...
if($action=="load_drop_down_knitting_com")
{
	...
	{
		echo create_drop_down( "cbo_return_to", 152, "select a.id,a.supplier_name from lib_supplier a where a.id=$kniting_company","id,supplier_name", 1, "--Select--", 1, "",1 );
	}
	else
	{
    	...
    	<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all">
            ...
            <tbody>
            	<? foreach($result as $row)
				{
					...
					?>
					<tr bgcolor="<? echo $bgcolor; ?>" onClick='get_php_form_data("<? echo $row[csf("tr_id")];?>","item_details_form_input","requires/woven_fab_receive_rtn_controller")' style="cursor:pointer" >
					<td><? echo $i; ?></td>
...
if($action=="return_po_popup")
{
	...
	{
		$order_sql=sql_select("select po_breakdown_id,quantity from order_wise_pro_details where trans_id=$update_id and trans_type=3 and entry_form=50 and status_active=1");
...
if($action=="save_update_delete")
{
	...
	extract(check_magic_quote_gpc( $process ));
        ...
        $max_recv_date = return_field_value("max(transaction_date) as max_date", "inv_transaction", "prod_id=$txt_prod_id and transaction_type in (1,4,5)", "max_date");
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		else
		{
			...
			//$new_return_number=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'GRR', date("Y",time()), 5, "select issue_number_prefix,issue_number_prefix_num from inv_issue_master where company_id=$cbo_company_name and entry_form=50 and $year_cond=".date('Y',time())." order by id DESC ", "issue_number_prefix", "issue_number_prefix_num" ));
			$new_return_number = explode("*", return_next_id_by_sequence("INV_ISSUE_MASTER_PK_SEQ", "inv_issue_master",$con,1,$cbo_company_name,'GRR',50,date("Y",time()),13 ));
			...
			$data_array_mst="(".$id.",'".$new_return_number[1]."','".$new_return_number[2]."','".$new_return_number[0]."',50,14,".$cbo_company_name.",".$txt_return_date.",".$txt_received_id.",".$txt_mrr_no.",".$pi_id.",'".$user_id."','".$pc_date_time."')";
		...
		$field_array_trans = "id,mst_id,company_id,prod_id,item_category,transaction_type,transaction_date,order_uom,order_qnty,cons_uom,cons_quantity,remarks,inserted_by,insert_date";
 		$data_array_trans = "(".$transactionID.",".$id.",".$cbo_company_name.",".$txt_prod_id.",14,3,".$txt_return_date.",".$cbo_uom.",".$txt_return_qnty.",".$cbo_uom.",".$txt_return_qnty.",".$txt_remarks.",'".$user_id."','".$pc_date_time."')";
		...
		if(!empty($txt_break_qnty))
		{
			foreach($ordr_wise_rtn_qnty_arr as $val)
			{
				...
				if($order_qnty_arr[1]>0)
				{
					...
					$data_array_proportion.="(".$proportion_id.",".$transactionID.",3,50,".$order_qnty_arr[0].",".$txt_prod_id.",".$order_qnty_arr[1].",'".$user_id."','".$pc_date_time."')";
					//$proportion_id++;
				}
			}
			...
			if($variable_setting_production==1)
			{
				...
				foreach($ordr_wise_rtn_roll_arr as $val)
				{
					...
					if($order_roll_arr[1]>0)
					{
						...
						$data_array_roll.="(".$roll_id.",".$id.",".$transactionID.",".$order_roll_arr[0].",50,".$order_roll_arr[1].",".$order_roll_arr[2].",'".$user_id."','".$pc_date_time."')";
						//$roll_id++;
					}
				}
			}
		...
		$data_array = "(".$mrrWiseIsID.",".$hidden_receive_trans_id.",".$transactionID.",50,".$txt_prod_id.",".$txt_return_qnty.",'".$user_id."','".$pc_date_time."')";
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		//before stock update
		$sql = sql_select( "select a.id,a.current_stock, b.cons_quantity from product_details_master a, inv_transaction b where a.id=b.prod_id and a.id=$before_prod_id and b.id=$update_id and a.item_category_id=14 and b.item_category=14 and b.transaction_type=3" );
		...
		$curr_stock_qnty=return_field_value("current_stock","product_details_master","id=$txt_prod_id and item_category_id=14");
        ...
        /**
         * list of fields that will not update on update event
         * company_id*received_id*cbo_return_to*pi_id*
         * company_id."*".$cbo_company_name."*".$txt_received_id."*".$pi_id."*".
         */
		$field_array_mst="issue_date*received_mrr_no*updated_by*update_date";
		$data_array_mst=$txt_return_date."*".$txt_mrr_no."*'".$user_id."'*'".$pc_date_time."'";

 		$field_array_trans="prod_id*item_category*transaction_type*transaction_date*order_uom*order_qnty*cons_uom*cons_quantity*remarks*updated_by*update_date";
 		$data_array_trans= "".$txt_prod_id."*14*3*".$txt_return_date."*".$cbo_uom."*".$txt_return_qnty."*".$cbo_uom."*".$txt_return_qnty."*".$txt_remarks."*'".$user_id."'*'".$pc_date_time."'";
		//echo $field_array."<br>".$data_array;die;
		$update_id = str_replace("'","",$update_id);
		$hidden_receive_trans_id = str_replace("'","",$hidden_receive_trans_id);
		...
		$data_array_mrr = "(".$mrrWiseIsID.",".$hidden_receive_trans_id.",".$update_id.",50,".$txt_prod_id.",".$txt_return_qnty.",'".$user_id."','".$pc_date_time."')";
		...
		if(!empty($txt_break_qnty))
		{
			foreach($ordr_wise_rtn_qnty_arr as $val)
			{
				...
				if($order_qnty_arr[1]>0)
				{
					$proportion_id = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
					if($data_array_proportion!="") $data_array_proportion.=", ";
					$data_array_proportion.="(".$proportion_id.",".$update_id.",3,50,".$order_qnty_arr[0].",".$txt_prod_id.",".$order_qnty_arr[1].",'".$user_id."','".$pc_date_time."')";
					//$proportion_id++;
				}
			}
			...
			if($variable_setting_production==1)
			{
				...
				foreach($ordr_wise_rtn_roll_arr as $val)
				{
					...
					if($order_roll_arr[2]>0)
					{
						$roll_id = return_next_id_by_sequence("PRO_ROLL_DTLS_PK_SEQ", "pro_roll_details", $con);
						if($data_array_roll!="") $data_array_roll.=", ";
						$data_array_roll.="(".$roll_id.",".$id.",".$update_id.",".$order_roll_arr[0].",50,".$order_roll_arr[1].",".$order_roll_arr[2].",'".$user_id."','".$pc_date_time."')";
						//$roll_id++;
					}
				}
			}
		...
		if($before_prod_id==$txt_prod_id)
		{
			...
			if($hidden_receive_trans_id>0)
			{
				$query3 = execute_query("DELETE FROM inv_mrr_wise_issue_details WHERE issue_trans_id=$update_id and entry_form=50");
			...
			if($data_array_proportion!="")
			{
				$query4 = execute_query("DELETE FROM order_wise_pro_details WHERE trans_id=$update_id and entry_form=50");
				$propoId=sql_insert("order_wise_pro_details",$field_array_proportion,$data_array_proportion,1);
				if($variable_setting_production==1)
				{
					$rollId=sql_insert("pro_roll_details",$field_array_roll,$data_array_roll,1);
					$query5 = execute_query("DELETE FROM pro_roll_details WHERE dtls_id=$update_id and entry_form=50");
				}
			}
	...
	<table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
            ...
            <tbody>
                <tr>
                    ...
                    <td align="center">
                        <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $company; ?>, 'create_return_search_list_view', 'search_div', 'woven_fab_receive_rtn_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />
                    </td>
            </tr>
        	<tr>
            	<td align="center" height="40" valign="middle" colspan="5">
...
if($action=="populate_master_from_data")
{
	$sql = "select id,issue_number,company_id,supplier_id,issue_date,item_category,received_id,received_mrr_no,pi_id
			...
			where id='$data' and item_category=14 and entry_form=50";
	//echo $sql;
	$res = sql_select($sql);
	foreach($res as $row)
	{
		...
		//right side list view
		//echo "show_list_view('".$row[csf("received_id")]."','show_product_listview','list_product_container','requires/woven_fab_receive_rtn_controller','');\n";
   	}
	exit();
}
...
if($action=="show_dtls_list_view")
{
	...
	$sql = "select a.id as issue_id, a.issue_number, a.company_id, a.supplier_id, a.issue_date, a.item_category, a.received_id, a.received_mrr_no, b.id as trans_id, b.cons_quantity, b.cons_uom, b.cons_rate, b.cons_amount, c.product_name_details, c.id as prod_id
			...
			where a.id=b.mst_id and b.item_category=14 and b.transaction_type=3 and a.id=$data";
	//echo $sql;
	$result = sql_select($sql);
	$i=1;
	$rettotalQnty=0;
     	...
     	<table class="rpt_table" border="1" cellpadding="2" cellspacing="0" style="width:800px" >
            ...
            <tbody>
				...
				foreach($result as $row)
				{
					...
					if($row[csf("prod_id")]!="")
					{
					$sqlTr = sql_select("select b.balance_qnty from inv_receive_master a, inv_transaction b where a.id=b.mst_id and b.prod_id=".$row[csf("prod_id")]." and b.item_category=14 and b.transaction_type in (1,4) and a.id='".$row[csf("received_id")]."'");
					}
					...
					?>
					<tr bgcolor="<? echo $bgcolor; ?>" onClick='get_php_form_data("<? echo $row[csf("trans_id")];?>,<? echo $rcvQnty;?>,<? echo $row[csf("issue_id")];?>","child_form_input_data","requires/woven_fab_receive_rtn_controller")' style="cursor:pointer" >
                        ...
                        <td width="70" align="right"><p><? echo $row[csf("cons_quantity")]; ?></p></td>
					</tr>
					<?
					$i++;
				}
	...
	$issue_id=str_replace("'","",$ex_data[2]);
 	...
 	$sql = "select b.id as prod_id, b.product_name_details, b.current_stock, b.gsm, b.dia_width, a.id as tr_id, a.cons_uom, a.cons_quantity, a.cons_amount, a.remarks
			from inv_transaction a, product_details_master b
 			where a.id=$data and a.status_active=1 and a.item_category=14 and transaction_type=3 and a.prod_id=b.id and b.status_active=1";
 	//echo $sql;die;
	$result = sql_select($sql);
	foreach($result as $row)
	{
