	...
	else if ($operation==1) // Update Here
	{
		...
		if($prev_split_barcode!="")
		{
			...
			$nxtProcessedBarcodeRes = sql_select("select $issue_number_ref as issue_number, b.entry_form, b.barcode_no
				...
				union all
				select a.recv_number as issue_number , b.entry_form, b.barcode_no
				from  inv_receive_mas_batchroll a, pro_roll_details b
				where a.id = b.mst_id and a.entry_form = 62 and b.entry_form = 62
				and b.status_active = 1 and b.is_deleted = 0 and b.barcode_no in ($prev_split_barcode)
				union all
				select $batch_no_ref as issue_number , b.entry_form, b.barcode_no
				from  pro_batch_create_mst a, pro_roll_details b
				where a.id = b.mst_id and b.entry_form = 64 and b.barcode_no in ($prev_split_barcode)
				and b.status_active = 1 and b.is_deleted = 0
				union all
				select $trans_no_ref as issue_number , b.entry_form, b.barcode_no
				from  inv_item_transfer_mst a, pro_roll_details b
				where a.id = b.mst_id and a.entry_form in (82,83,110,180,183,133) and b.entry_form in (82,83,110,180,183,133) 
			...
			foreach ($nxtProcessedBarcodeRes as $val) 
			{
				if($up_roll_qnty[$val[csf("barcode_no")]] != $up_hide_roll_qnty[$val[csf("barcode_no")]])
				{
					if($val[csf("entry_form")] == 61){
						...
						echo "30**Receive by Batch found against these barcode. Receive No : ".$val[csf("issue_number")];disconnect($con);
						die;
					}else if($val[csf("entry_form")] == 64){
						...
						echo "30**Transfer found against these barcode. Transfer No : ".$val[csf("issue_number")];disconnect($con);
						die;
					}
				}
			}

			$sql_split_barcode="SELECT barcode_no from pro_roll_split where barcode_no in($prev_split_barcode) and status_active=1 and is_deleted=0 and entry_form=113";
...
if($action=="roll_details_update")
{
	...
	if($splited_barcode)
	{
		...
		$nxtProcessSql2 = sql_select("SELECT a.id, a.barcode_no from PRO_ROLL_SPLIT a where a.barcode_no in (".$splited_barcode.") and a.entry_form =113 and a.status_active=1 and a.is_deleted=0");
...
{  
	...
	/*$batch_details=sql_select("SELECT a.id,a.entry_form, b.company_id, b.to_company, b.transfer_criteria, a.barcode_no,a.mst_id,a.dtls_id,a.po_breakdown_id,a.roll_no,a.roll_id, a.qnty, a.booking_without_order, a.qc_pass_qnty_pcs, b.to_company, b.transfer_criteria 
		...
		$batch_details=sql_select("SELECT a.id,a.entry_form, b.company_id, b.to_company, b.transfer_criteria, a.barcode_no,a.mst_id,a.dtls_id,a.po_breakdown_id,a.roll_no,a.roll_id, a.qnty, a.booking_without_order, a.qc_pass_qnty_pcs 
		from pro_roll_details a,  inv_item_transfer_mst b 
		where b.id=a.mst_id and a.entry_form in(82,83,133,183,110,180) and re_transfer =0 and a.status_active=1 and a.is_deleted=0 and a.barcode_no='".$barcode."'
		union all
		SELECT a.id,a.entry_form, b.company_id, 0 as to_company, 0 as transfer_criteria, a.barcode_no, a.mst_id, a.dtls_id, a.po_breakdown_id, a.roll_no, a.roll_id, a.qnty, a.booking_without_order, a.qc_pass_qnty_pcs
	...
	if( empty($batch_details) )
	{
		...
		$batch_details=sql_select("select a.id,a.entry_form, a.barcode_no, a.mst_id,a.dtls_id,a .po_breakdown_id, a.roll_no,a.roll_id, a.qnty, a.qc_pass_qnty, b.grey_receive_qnty, a.booking_without_order, a.qc_pass_qnty_pcs from pro_roll_details a, pro_grey_prod_entry_dtls b where a.dtls_id=b.id and a.entry_form in(22,58,2) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and  b.trans_id<>0 and a.barcode_no='".$barcode."'");
...
if($action=="check_barcode_no")
{
	...
	$existing_sql=sql_select("select a.barcode_no, a.system_number from pro_roll_split a, pro_roll_details b where a.split_from_id=b.id and a.entry_form=113 and a.status_active=1 and a.is_deleted=0 and a.barcode_no=".$data." and b.re_transfer=0");
