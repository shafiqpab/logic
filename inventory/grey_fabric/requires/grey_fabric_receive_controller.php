...
{
	$data=explode("_",$data);
	echo create_drop_down( "cbo_buyer_name", 152, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond  $company_credential_cond order by comp.company_name","id,company_name", 1, "-- Select Unit --", $selected, "",$data[0] );
...
if ($action=="load_drop_down_location")
{
	...
	if($knit_source==1)
	{
		echo create_drop_down( "cbo_location", 151, "select id,location_name from lib_location where company_id='$data[0]' and status_active =1 and is_deleted=0  $location_credential_cond order by location_name","id,location_name", 1, "-- Select Location --", 0, "load_floor();load_room_rack_self_bin('requires/grey_fabric_receive_controller*13', 'store','store_td', $('#cbo_company_id').val(), this.value);" );//$('#cbo_knitting_company').val()
	}
	else
	{
		echo create_drop_down( "cbo_location", 151, "select id,location_name from lib_location where company_id='$data[0]' and status_active =1 and is_deleted=0  $location_credential_cond order by location_name","id,location_name", 1, "-- Select Location --", 0, "load_floor();load_room_rack_self_bin('requires/grey_fabric_receive_controller*13', 'store','store_td', $('#cbo_company_id').val(), this.value);" );
	}
...
{
	//echo $data;die;
	load_room_rack_self_bin("requires/grey_fabric_receive_controller",$data);
}
...
if($action=="load_drop_down_knitting_com_new")
{
	...
	{
		echo create_drop_down( "cbo_knitting_company", 152, "SELECT DISTINCT c.supplier_name,c.id from lib_supplier_tag_company a,lib_supplier_party_type b, lib_supplier c where c.id=b.supplier_id and a.supplier_id = b.supplier_id and a.tag_company='$company_id' and b.party_type in(9,20) and c.status_active IN(1) and c.is_deleted=0 group by c.id, c.supplier_name   UNION ALL  SELECT DISTINCT c.supplier_name,c.id from lib_supplier_tag_company a,lib_supplier_party_type b, lib_supplier c, inv_receive_master d where c.id=b.supplier_id and a.supplier_id = b.supplier_id and c.id=d.supplier_id and a.tag_company='$company_id'  and b.party_type  in(9,20) and c.status_active IN(1,3) and c.is_deleted=0 group by c.id, c.supplier_name    order by supplier_name","id,supplier_name", 1, "--Select Knit Company--", 0, "","");//load_location();
	}
	else
	{
...
if($action=="create_wo_pi_production_search_list_view")
{
	...
	if($recieve_basis==1)
	{
		...
		$sql = "select id, pi_number, supplier_id, pi_date, last_shipment_date, pi_basis_id, internal_file_no, currency_id, source from com_pi_master_details where item_category_id='$category_id' and pi_basis_id=2 and status_active=1 and is_deleted=0 and importer_id=$company_id $search_field_cond $dateCond $year_cond order by id desc";
		//echo $sql;
	...
	else if($recieve_basis==2)
	{
		...
		$sql = "select a.id, a.booking_no_prefix_num, a.supplier_id, a.booking_no, a.booking_date, a.buyer_id, c.po_break_down_id, a.item_category, a.delivery_date, c.job_no as job_no_mst, 0 as type from wo_booking_mst a, wo_booking_dtls c,wo_po_break_down b where a.booking_no=c.booking_no and c.po_break_down_id=b.id and a.company_id=$company_id and a.item_category=$category_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.fabric_source in (1,2,3) $search_field_cond $dateCond $year_cond group by a.id,a.supplier_id, a.booking_no_prefix_num, a.booking_no, a.booking_date, a.buyer_id, c.po_break_down_id, a.item_category, a.delivery_date, c.job_no
		union all
		SELECT s.id, s.booking_no_prefix_num,s.supplier_id, s.booking_no, s.booking_date, s.buyer_id, null as po_break_down_id, s.item_category, s.delivery_date, null as job_no_mst, 1 as type FROM wo_non_ord_samp_booking_mst s WHERE s.company_id=$company_id and s.status_active=1 and s.is_deleted=0 and s.fabric_source in (2,3) and s.item_category=$category_id $search_field_cond_sample $sdateCond $syear_cond order by type, id DESC";
	...
	else if($recieve_basis==11)
	{
		...
		/*$sql = "select a.id, a.booking_no_prefix_num,a.supplier_id, a.booking_no, a.booking_date, a.buyer_id, a.po_break_down_id, a.item_category, a.delivery_date, a.job_no as job_no_mst, 0 as type from wo_booking_mst a, wo_po_break_down b where a.job_no=b.job_no_mst and a.company_id=$company_id and a.item_category=$category_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.fabric_source in (2,3) $search_field_cond group by a.id,a.supplier_id, a.booking_no_prefix_num, a.booking_no, a.booking_date, a.buyer_id, a.po_break_down_id, a.item_category, a.delivery_date, a.job_no
				union all
				SELECT s.id, s.booking_no_prefix_num,s.supplier_id, s.booking_no, s.booking_date, s.buyer_id, null as po_break_down_id, s.item_category, s.delivery_date, null as job_no_mst, 1 as type FROM wo_non_ord_samp_booking_mst s WHERE s.company_id=$company_id and s.status_active=1 and s.is_deleted=0 and s.fabric_source in (2,3) and s.item_category=$category_id $search_field_cond_sample order by type, id"; */

			 	/*$sql = "select a.id, a.booking_no_prefix_num, a.booking_no, a.booking_date, a.buyer_id, a.supplier_id, c.po_break_down_id, a.item_category, a.delivery_date, c.job_no as job_no_mst, 0 as type from wo_booking_dtls c,wo_booking_mst a, wo_po_break_down b where c.booking_no=a.booking_no and c.job_no=b.job_no_mst and a.company_id=$company_id and a.item_category=12 and a.booking_type=3 and c.process=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $search_field_cond group by a.id, a.booking_no_prefix_num, a.booking_no, a.booking_date, a.buyer_id, c.po_break_down_id, a.item_category, a.delivery_date, c.job_no, a.supplier_id
				union all
				...
				order by type, id"; */
				$sql = "select a.id, a.booking_no_prefix_num, a.booking_no, a.booking_date, a.buyer_id, a.supplier_id, c.po_break_down_id, a.item_category, a.delivery_date, c.job_no as job_no_mst, 0 as type from wo_booking_dtls c,wo_booking_mst a, wo_po_break_down b where c.booking_no=a.booking_no and c.job_no=b.job_no_mst and a.company_id=$company_id and a.item_category=12 and a.booking_type=3 and c.process=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $search_field_cond $dateCond $year_cond group by a.id, a.booking_no_prefix_num, a.booking_no, a.booking_date, a.buyer_id, c.po_break_down_id, a.item_category, a.delivery_date, c.job_no, a.supplier_id
				union all
				select s.id, s.prefix_num as booking_no_prefix_num, s.booking_no, s.booking_date, s.buyer_id, s.supplier_id, null as po_break_down_id, x.process_id as item_category, null as delivery_date, null as job_no_mst, 1 as type from wo_non_ord_knitdye_booking_mst s,wo_non_ord_knitdye_booking_dtl x where  s.id=x.mst_id and s.company_id=$company_id and s.status_active=1 and s.is_deleted=0  $search_field_cond_sample $sdateCond $syear_cond group by s.id, s.prefix_num, s.booking_no, s.booking_date, s.buyer_id, s.supplier_id, x.process_id order by type, id DESC";
				//and s.item_category=1
		/* $sql = "select a.id, a.booking_no_prefix_num,a.supplier_id, a.booking_no, a.booking_date, a.buyer_id, a.po_break_down_id, a.item_category, a.delivery_date, a.job_no as job_no_mst, 0 as type from wo_booking_mst a, wo_po_break_down b where a.job_no=b.job_no_mst and a.company_id=$company_id and a.item_category=$category_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.fabric_source in (2,3) $search_field_cond group by a.id,a.supplier_id, a.booking_no_prefix_num, a.booking_no, a.booking_date, a.buyer_id, a.po_break_down_id, a.item_category, a.delivery_date, a.job_no
				union all
				SELECT s.id, s.booking_no_prefix_num,s.supplier_id, s.booking_no, s.booking_date, s.buyer_id, null as po_break_down_id, s.item_category, s.delivery_date, null as job_no_mst, 1 as type FROM wo_non_ord_samp_booking_mst s WHERE s.company_id=$company_id and s.status_active=1 and s.is_deleted=0 and s.fabric_source in (2,3) and s.item_category=$category_id $search_field_cond_sample order by type, id"; */
		//echo $sql;
	...
	else if($recieve_basis==14)
	{
		...
		if($search_string!="")
		{
			if($string_search_type==1)
			{
				$search_field_cond="and a.job_no='$search_string'";
			}
			if($string_search_type==4 || $string_search_type==0)
			{
		...
		$sql = "SELECT a.id, a.job_no, a.booking_date, a.delivery_date, a.buyer_id, a.within_group from fabric_sales_order_mst a, fabric_sales_order_dtls b where a.id=b.mst_id and a.company_id=$company_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.sales_order_type in (2) $search_field_cond $dateCond group by a.id, a.job_no, a.booking_date, a.delivery_date, a.buyer_id, a.within_group order by a.id desc";
		...
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="910" class="rpt_table">
			<thead>
				...
				<th width="100">Sales Order Date</th>
				<th width="100">Buyer</th>
				<th width="150">Delivary date</th>
			</thead>
		</table>
		<div style="width:928px; max-height:250px; overflow-y:scroll" id="list_container_batch" align="left">
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="910" class="rpt_table" id="tbl_list_search">
				...
				foreach ($result as $row)
				{
					if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
					?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value(<? echo $row[csf('id')]; ?>,'<? echo $row[csf('job_no')]; ?>','0','0','','<? echo $row[csf('supplier_id')]; ?>');">
						<td width="30"><? echo $i; ?></td>
						...
						<p>
							...
							if($row[csf('within_group')]==2){
								echo $buyer_short_arr[$row[csf('buyer_id')]];
							}else{
								echo $company_name_arr[$row[csf('buyer_id')]];
							}
							?>
							&nbsp;
						</p>
						</td>
						<td width="150" align="center"><? echo change_date_format($row[csf('delivery_date')]); ?>&nbsp;</td>
					</tr>
					<?
					$i++;
				}
	...
	else
	{
		...
		$po_array=array();
		$po_sql=sql_select("select a.style_ref_no, a.job_no, b.id, b.po_number, b.grouping, b.file_no from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and company_name='$company_id'");
		...
		if($search_string!="")
		{
			if($string_search_type==1) $search_field_cond="and recv_number_prefix_num='$search_string'";
			if($string_search_type==4 || $string_search_type==0) $search_field_cond="and recv_number_prefix_num like '%$search_string%'";
			if($string_search_type==2) $search_field_cond="and recv_number_prefix_num like '$search_string%'";
			if($string_search_type==3) $search_field_cond="and recv_number_prefix_num like '%$search_string'";
		}

		if($db_type==0)
		{
			if ($date_from!="" &&  $date_to!="") $dateCond = "and receive_date between '".change_date_format($date_from,'yyyy-mm-dd')."' and '".change_date_format($date_to,'yyyy-mm-dd')."'"; else $dateCond ="";
			$year_cond=" and YEAR(insert_date)=$year_selection";
		}
		else
		{
			if ($date_from!="" &&  $date_to!="") $dateCond = "and receive_date between '".change_date_format($date_from, "", "",1)."' and '".change_date_format($date_to, "", "",1)."'"; else $dateCond ="";
			$year_cond=" and to_char(insert_date,'YYYY')=$year_selection";
		}
		...
		$sql = "select id, recv_number_prefix_num, booking_without_order, $year_field as year, recv_number, receive_basis, buyer_id, knitting_source, knitting_company, receive_date, challan_no, yarn_issue_challan_no from inv_receive_master where entry_form=2 and status_active=1 and is_deleted=0 and roll_maintained!=1 and company_id=$company_id $search_field_cond $dateCond $year_cond order by id DESC";
		// echo $sql;
		$result = sql_select($sql);
		?>
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1010" class="rpt_table">
			...
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1010" class="rpt_table" id="tbl_list_search">
				...
				foreach ($result as $row)
				{
					...
					if ($row[csf('booking_without_order')]==0)
					{
						...
						$order_no=''; $style_ref=''; $job_no=''; $internal_ref=''; $file_no='';
						foreach($order_id as $value)
						{
							...
							if($internal_ref=='') $internal_ref=$po_array[$value]['ref']; else $internal_ref.=",".$po_array[$value]['ref'];
							if($file_no=='') $file_no=$po_array[$value]['file_no']; else $file_no.=",".$po_array[$value]['file_no'];
							$job_no=$po_array[$value]['job_no'];
						}

						$style_ref=implode(",",array_unique(explode(",",$style_ref)));
						$internal_ref=implode(",",array_unique(explode(",",$internal_ref)));
						$file_no=implode(",",array_unique(explode(",",$file_no)));
						if($row[csf('receive_basis')]==0) { $job_no='';}
					}
					$data=$row[csf('knitting_source')]."**".$row[csf('knitting_company')]."**".$row[csf('challan_no')]."**".$row[csf('yarn_issue_challan_no')]."**".$job_no;
					?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value(<? echo $row[csf('id')]; ?>,'<? echo $row[csf('recv_number')]; ?>','<? echo $row[csf('booking_without_order')]; ?>','<? echo $row[csf('buyer_id')]; ?>','<? echo $data; ?>');">
						<td width="30"><? echo $i; ?></td>
						<td width="55">&nbsp;&nbsp;<? echo $row[csf('recv_number_prefix_num')]; ?></td>
						<td width="45" align="center"><? echo $row[csf('year')]; ?></td>
						<td width="75" align="center"><p><? echo change_date_format($row[csf('receive_date')]); ?></p></td>
						<td width="70"><p><? echo $row[csf('challan_no')]; ?>&nbsp;</p></td>
						<td width="80"><p><? echo $knitting_source[$row[csf('knitting_source')]]; ?></p></td>
						<td width="115"><p><? echo $knit_comp; ?></p></td>
						<td width="70"><p><? echo $buyer_short_arr[$row[csf('buyer_id')]]; ?>&nbsp;</p></td>
...
if($action=='populate_data_from_sales_order')
{
	...
	foreach ($data_array as $row)
	{
		...
		{
			echo "load_drop_down('requires/grey_fabric_receive_controller','". $row[csf("buyer_id")] ."', 'load_drop_down_company_in_buyer_td','buyer_td_id');\n";
		}
...
if($action=='populate_data_from_production')
{
	...
	foreach ($data_array as $row)
	{
		...
		{
			$data_po_array=sql_select("select po_breakdown_id, quantity,quantity_pcs,coller_cuff_size from order_wise_pro_details where dtls_id='$id' and entry_form=2 and status_active=1 and is_deleted=0");
			foreach($data_po_array as $row_po)
			{
				if($save_string=="")
				{
					$save_string=$row_po[csf("po_breakdown_id")]."**".$row_po[csf("quantity")]."**"."**"."**"."**"."**".$row_po[csf("quantity_pcs")]."**".$row_po[csf("coller_cuff_size")]."**1";
				}
				else
				{
					$save_string.=",".$row_po[csf("po_breakdown_id")]."**".$row_po[csf("quantity")]."**"."**"."**"."**"."**".$row_po[csf("quantity_pcs")]."**".$row_po[csf("coller_cuff_size")]."**1";
...
if($action=='quantity_check_for_service_booking')
{
	...
	if(empty($recive_no))
	{
		...
		and a.status_active=1 and a.is_deleted=0 and a.item_category=13 and a.entry_form=22
...
if($action=='show_fabric_desc_listview')
{
	...
	if($receive_basis==1)
	{
		$sql="select a.currency_id, b.determination_id, '' as body_part_id, b.gsm as gsm_weight, b.dia_width, sum(b.quantity) as qnty, avg(b.rate) as rate from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and b.pi_id='$booking_pi_production_no' and b.status_active=1 and b.is_deleted=0 group by a.currency_id, b.determination_id, b.gsm, b.dia_width";
	}
	else if($receive_basis==2)
	{
		if($is_sample==0)
		{
			...
			$sql="select b.lib_yarn_count_deter_id as determination_id, b.body_part_id, b.gsm_weight, a.dia_width, sum($field) as qnty, avg(a.rate) as rate from wo_booking_dtls a, wo_pre_cost_fabric_cost_dtls b where a.pre_cost_fabric_cost_dtls_id=b.id and a.booking_no='$booking_pi_production_no' and a.job_no=b.job_no and a.status_active=1 and a.is_deleted=0 group by b.lib_yarn_count_deter_id, b.body_part_id, b.gsm_weight, a.dia_width";

			$exchange_rate=return_field_value("exchange_rate","wo_booking_mst","booking_no='$booking_pi_production_no'");
		}
		else
		{
			$sql="select lib_yarn_count_deter_id as determination_id, body_part as body_part_id, gsm_weight, dia_width, construction, composition, sum(grey_fabric) as qnty, avg(rate) as rate from wo_non_ord_samp_booking_dtls where booking_no='$booking_pi_production_no' and status_active=1 and is_deleted=0 group by lib_yarn_count_deter_id, body_part, gsm_weight, dia_width, construction, composition";
	...
	else if($receive_basis==11)
	{
		...
		if($is_sample==0)
		{
			...
			if($row_prog_2 != "")
			{
				$sql="SELECT a.program_no,b.lib_yarn_count_deter_id as determination_id, b.body_part_id, b.gsm_weight, a.dia_width, sum($field) as qnty, avg(a.rate) as rate,a.lot_no,a.yarn_count,a.brand, 0 as fabric_color_id,a.slength,d.stitch_length,d.color_id,d.color_range 
				from wo_booking_dtls a, wo_pre_cost_fab_conv_cost_dtls c, wo_pre_cost_fabric_cost_dtls b,ppl_planning_info_entry_dtls d 
				where a.pre_cost_fabric_cost_dtls_id=c.id and c.fabric_description=b.id and a.booking_no='$booking_pi_production_no' and a.job_no=b.job_no and a.status_active=1 and a.is_deleted=0 and a.program_no=d.id 
				group by a.program_no,b.lib_yarn_count_deter_id, b.body_part_id, b.gsm_weight, a.dia_width,a.lot_no,a.yarn_count,a.brand,a.slength,d.stitch_length,d.color_id,d.color_range"; //new
				$exchange_rate=return_field_value("exchange_rate","wo_booking_mst","booking_no='$booking_pi_production_no'");
			}
			else
			{
				$sql="SELECT b.lib_yarn_count_deter_id as determination_id, b.body_part_id, b.gsm_weight, a.dia_width, sum($field) as qnty, avg(a.rate) as rate 
				from wo_booking_dtls a, wo_pre_cost_fab_conv_cost_dtls c, wo_pre_cost_fabric_cost_dtls b 
				where a.pre_cost_fabric_cost_dtls_id=c.id and c.fabric_description=b.id and a.booking_no='$booking_pi_production_no' and a.job_no=b.job_no and a.status_active=1 and a.is_deleted=0 
				group by b.lib_yarn_count_deter_id, b.body_part_id, b.gsm_weight, a.dia_width"; //old
				$exchange_rate=return_field_value("exchange_rate","wo_booking_mst","booking_no='$booking_pi_production_no'");
		...
		{
			$sql="select c.lib_yarn_count_deter_id as determination_id, c.body_part as body_part_id, c.gsm_weight, c.dia_width, sum(b.wo_qty) as qnty, b.rate
			from wo_non_ord_knitdye_booking_mst a, wo_non_ord_knitdye_booking_dtl b, wo_non_ord_samp_booking_dtls c
			where a.id=b.mst_id and b.fab_des_id=c.id and b.fabric_source=1 and b.process_id=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.booking_no='$booking_pi_production_no' group by c.lib_yarn_count_deter_id, c.body_part, c.gsm_weight, c.dia_width, b.rate";
	...
	else
	{
		...
		/*echo $sql="select b.id, b.prod_id, b.febric_description_id as determination_id, b.body_part_id, b.gsm as gsm_weight, b.width as dia_width, b.grey_receive_qnty as qnty, rate, a.booking_id as program_no
		from inv_receive_master a, pro_grey_prod_entry_dtls b where a.id=b.mst_id and b.mst_id='$booking_pi_production_no' and b.status_active=1 and b.is_deleted=0";*/
	}
	//echo $sql;
	$data_array=sql_select($sql);
	$over_receive = 0;
	...
	$over_receive = !empty($variable_set_invent)?$variable_set_invent[0][csf('over_rcv_percent')]:0;
	$receive_arr = array();
	$received_qty = sql_select("SELECT a.booking_no, sum(b.grey_receive_qnty) grey_receive_qnty, b.body_part_id, b.febric_description_id, b.original_gsm FROM inv_receive_master a
		INNER JOIN pro_grey_prod_entry_dtls b ON a.id = b.mst_id
		WHERE a.booking_no = '$booking_pi_production_no' and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0 and item_category=13 and entry_form=22 GROUP BY a.booking_no, b.body_part_id, b.febric_description_id, b.original_gsm");
	foreach ($received_qty as $row)
	{
		$receive_arr[$booking_pi_production_no][$row[csf('body_part_id')]][$row[csf('febric_description_id')]][$row[csf('original_gsm')]]['grey_receive_qnty'] = $row[csf('grey_receive_qnty')];
	}
	?>
			...
			foreach($data_array as $row)
			{
				...
				else
				{
					...
					if($receive_basis==2 && $is_sample==1 && ($row[csf('determination_id')]==0 || $row[csf('determination_id')]==""))
					{
						...
						$data=$row[csf('body_part_id')]."**".$cons_comp."**".$row[csf('gsm_weight')]."**".$row[csf('dia_width')]."**".$row[csf('determination_id')]."**".$rate."**".$row[csf('currency_id')];
					}
					else
					{
						$grey_receive_qnty = $receive_arr[$booking_pi_production_no][$row[csf('body_part_id')]][$row[csf('determination_id')]][$row[csf('gsm_weight')]]['grey_receive_qnty'];
						$rec_qty = (!empty($receive_arr)?$grey_receive_qnty:0);

						if($row_prog==$row_prog_2)
						{
							$data=$row[csf('body_part_id')]."**".$composition_arr[$row[csf('determination_id')]]."**".$row[csf('gsm_weight')]."**".$row[csf('dia_width')]."**".$row[csf('determination_id')]."**".$rate."**".$row[csf('currency_id')]."**".$row_prog_2."**".$row[csf('lot_no')]."**".$row[csf('yarn_count')]."**".$row[csf('slength')]."**".$row[csf('brand')]."**".$row[csf('fabric_color_id')]."**".$row[csf('stitch_length')]."**".$color_library[$row[csf('color_id')]]."**".$row[csf('color_id')]."**".$row[csf('color_range')]."**".$row[csf('program_no')]."**".$row[csf('qnty')]."**".$rec_qty."**".$over_receive; // new
						}
						else
						{
							$data=$row[csf('body_part_id')]."**".$composition_arr[$row[csf('determination_id')]]."**".$row[csf('gsm_weight')]."**".$row[csf('dia_width')]."**".$row[csf('determination_id')]."**".$rate."**".$row[csf('currency_id')]."**".$row[csf('qnty')]."**".$rec_qty."**".$over_receive; // old $row_prog_2
						}
					}
				}
				?>
...
if ($action=="po_popup")
{
	...
	if($receive_basis==9)
	{
		$body_part_type = return_library_array("select id,body_part_type from  lib_body_part where status_active=1 and is_deleted=0 and id='$cbo_body_part' order by body_part_type", "id", "body_part_type");
	}
	...
	if($receive_basis==2 || $receive_basis==11)
	{
		...
		//$recvData=sql_select("select a.po_breakdown_id, sum(quantity) as grey_fabric_recv from order_wise_pro_details a, product_details_master b where a.prod_id=b.id and b.detarmination_id='$fabric_desc_id' and b.gsm='$txt_gsm' $dia_widthCond and b.item_category_id=13 and a.entry_form=22 and a.is_deleted=0 and a.status_active=1 $dtls_id_cond group by a.po_breakdown_id");
		$recvData=sql_select("SELECT a.po_breakdown_id, sum(quantity) as grey_fabric_recv from order_wise_pro_details a, product_details_master b, pro_grey_prod_entry_dtls c, inv_receive_master d where a.prod_id=b.id and a.dtls_id = c.id and c.mst_id= d.id and d.booking_no= '$booking_no' and b.detarmination_id='$fabric_desc_id' and b.gsm='$txt_gsm' $dia_widthCond and b.item_category_id=13 and a.entry_form=22 and a.is_deleted=0 and a.status_active=1 $dtls_id_cond group by a.po_breakdown_id");
	...
	<script>
		...
		function calculate_tot_qntyInPcs()
		{
			var tot_grey_qntyInPcs='';
			$("#tbl_list_search").find('tr').each(function()
			{
				var txtQntyInPcs=$(this).find('input[name="txtQntyInPcs[]"]').val()*1;
				tot_grey_qntyInPcs=tot_grey_qntyInPcs*1+txtQntyInPcs*1;
			});

			$('#txt_tot_grey_qnty_in_pcs').val( tot_grey_qntyInPcs.toFixed(2));
		}
		...
		function fnc_close()
		{
			...
			$("#tbl_list_search").find('tr').each(function()
			{
				...
				if(txtGreyQnty*1>0)
				{
					...
					if(save_string=="")
					{
						save_string=txtPoId+"**"+txtGreyQnty+"**"+txtRoll+"**"+txtRollId+"**"+txtRollTableId+"**"+txtBarcodeNo+"**"+txtQntyInPcs+"**"+txtSize;
					}
					else
					{
						save_string+=","+txtPoId+"**"+txtGreyQnty+"**"+txtRoll+"**"+txtRollId+"**"+txtRollTableId+"**"+txtBarcodeNo+"**"+txtQntyInPcs+"**"+txtSize;
		...
		<div style="margin-left:5px; margin-top:10px">
			<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="<? echo $width-20; ?>">
				<thead>
					...
					if(($receive_basis==2 || $receive_basis==11) && $booking_without_order!=1)
					{
					...
					<th width="90">Reject Qnty</th>
					<?
					if($receive_basis==9 && ($body_part_type[$cbo_body_part] ==40 || $body_part_type[$cbo_body_part] ==50) && $roll_maintained==0)
					{
						echo '<th width="50">Size</th>';
						echo '<th width="50">Qnty In Pcs</th>';
					}
					?>
			...
			<div style="width:<? echo $width; ?>px; max-height:200px; overflow-y:scroll" id="list_container" align="left">
				<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="<? echo $width-20; ?>" id="tbl_list_search">
					<?
					$i=1; $tot_po_qnty=0; $tot_grey_qnty=0; $tot_reject_qnty=0; $tot_grey_qnty_in_pcs=0; $po_array=array();
					...
					else if($save_data!="" && ($receive_basis==2 || $receive_basis==11))
					{
						//sql_select("select  a.po_break_down_id,  sum (a.wo_qnty) as qnty from wo_booking_dtls a, wo_pre_cost_fab_conv_cost_dtls b,  wo_pre_cost_fabric_cost_dtls c where a.pre_cost_fabric_cost_dtls_id = b.id and a.booking_type = 3 and b.fabric_description = c.id  and a.status_active = 1 and b.status_active = 1 and a.job_no = b.job_no and a.booking_no = '$booking_no' and c.lib_yarn_count_deter_id ='$fabric_desc_id' and c.gsm_weight='$txt_gsm' and c.body_part_id = '$cbo_body_part' and a.dia_width='$txt_width' group by  a.po_break_down_id");

						$booking_qnty_array=return_library_array("select a.po_break_down_id, sum(a.grey_fab_qnty) as qnty from wo_booking_dtls a, wo_pre_cost_fabric_cost_dtls b where a.pre_cost_fabric_cost_dtls_id=b.id and a.booking_no='$booking_no' and b.lib_yarn_count_deter_id='$fabric_desc_id' and b.body_part_id='$cbo_body_part' and b.gsm_weight='$txt_gsm' and a.dia_width='$txt_width' and a.job_no=b.job_no and a.booking_type <> 3 and a.status_active=1 and a.is_deleted=0 group by a.po_break_down_id
							union all
							select  a.po_break_down_id,  sum(a.wo_qnty) as qnty from wo_booking_dtls a, wo_pre_cost_fab_conv_cost_dtls b,  wo_pre_cost_fabric_cost_dtls c where a.pre_cost_fabric_cost_dtls_id = b.id and a.booking_type = 3 and b.fabric_description = c.id  and a.status_active = 1 and b.status_active = 1 and a.job_no = b.job_no and a.booking_no = '$booking_no' and c.lib_yarn_count_deter_id ='$fabric_desc_id' and c.gsm_weight='$txt_gsm' and c.body_part_id = '$cbo_body_part' and a.dia_width='$txt_width' group by  a.po_break_down_id","po_break_down_id","qnty");
						if($roll_maintained==1)
						{
							...
							for($z=0;$z<count($explSaveData);$z++)
							{
								...
								if($receive_basis==11 && $booking_without_order==1)
								{
									...
									$po_data=sql_select("select a.id as book_id, a.fab_booking_id as id, a.booking_no as po_number, a.booking_date as pub_shipment_date, sum(b.wo_qty) as po_qnty_in_pcs
										from wo_non_ord_knitdye_booking_mst a, wo_non_ord_knitdye_booking_dtl b
										where a.id=b.mst_id and a.fab_booking_id=$order_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
					...
					else if($save_data!="" && $receive_basis==9)
					{
						...
						$is_sales=sql_select("select b.is_sales from inv_receive_master a, ppl_planning_info_entry_dtls b where a.booking_id=b.id and a.recv_number='$booking_no'");
						...
						for($z=0;$z<count($explSaveData);$z++)
						{
							...
							$roll_id=$po_wise_data[4];
							$barcode_no=$po_wise_data[5];
							$grey_qnty_in_pcs=$po_wise_data[6];
							$collarCuf_size=$po_wise_data[7];
							$new_and_old_status=$po_wise_data[8];

							if ($is_sales[0][csf('is_sales')]==1)
							{
								$fso_po_data=sql_select("select a.job_no, a.delivery_date from fabric_sales_order_mst a, fabric_sales_order_dtls b where a.id=b.mst_id and a.id=$order_id group by a.job_no, a.delivery_date");
							}else{
								$po_data=sql_select("select b.po_number, (a.total_set_qnty*b.po_quantity) as po_qnty_in_pcs, b.pub_shipment_date, b.grouping from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and b.id=$order_id");
							}
							...
							if ($is_sales[0][csf('is_sales')]==1)
							{
								$po_number = $fso_po_data[0][csf('job_no')];
								$delivery_date=change_date_format($fso_po_data[0][csf('delivery_date')]);
							}
							else{
								$po_number = $po_data[0][csf('po_number')];
								$delivery_date=change_date_format($po_data[0][csf('pub_shipment_date')]);
							}

							$tot_grey_qnty+=$grey_qnty;
							$tot_reject_qnty+=$reject_qnty;
							$tot_grey_qnty_in_pcs+=$grey_qnty_in_pcs;
							?>
							<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
								<td width="100">
									<p><? echo $po_number; ?></p>
								...
								<td width="80" align="center"><? echo $delivery_date; ?></td>
								<td align="center" width="90">
									<input type="text" name="txtGreyQnty[]" id="txtGreyQnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:77px" value="<? echo $grey_qnty; ?>" onKeyUp="calculate_tot_qnty();">
								</td>
								<td align="center" width="90">
									<input type="text" name="txtRejectQnty[]" id="txtRejectQnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:77px" value="<? echo $reject_qnty; ?>" onKeyUp="calculate_tot_qnty2();">
								</td>
								<?
								if($receive_basis==9 && ($body_part_type[$cbo_body_part] ==40 || $body_part_type[$cbo_body_part] ==50) && $roll_maintained==0)
								{
									...
									<td align="center" width="50">
										<?
											echo $collarCuf_size;
										?>
										<input type="hidden" name="txtSize[]" id="txtSize_<? echo $i; ?>" class="text_boxes" style="width:40" value="<? echo $collarCuf_size; ?>">
									</td>
									<td align="center" width="50">
										<input type="text" name="txtQntyInPcs[]" id="txtQntyInPcs_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px" placeholder="<? if($new_and_old_status==1){echo $grey_qnty_in_pcs;} ?>"  value="<? if($new_and_old_status!=1){echo $grey_qnty_in_pcs;} ?>" onKeyUp="calculate_tot_qntyInPcs();">
									</td>
									<?
								}
								?>
					...
					else
					{
						if($receive_basis==11 && $booking_without_order==1)
						{
							...
							$po_sql="select a.id as book_id, a.fab_booking_id as id, a.booking_no as po_number, a.booking_date as pub_shipment_date, 1 as total_set_qnty, sum(b.wo_qty) as po_quantity
							from wo_non_ord_knitdye_booking_mst a, wo_non_ord_knitdye_booking_dtl b
							where a.id=b.mst_id and a.booking_no='$booking_no' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
							group by a.id, a.fab_booking_id, a.booking_no, a.booking_date order by a.id";

							$nonOrdKnitDyeArray=sql_select($po_sql);
							$sample_non_order_id="";
							foreach($nonOrdKnitDyeArray as $row)
							{
								$sample_non_order_ids.=$row[csf('id')].",";
							}
							$sample_non_order_ids=chop($sample_non_order_ids,",");

							$sample_non_order_sql=sql_select("select id,booking_no from wo_non_ord_samp_booking_mst where id in($sample_non_order_ids) and status_active=1 and is_deleted=0");
							foreach($sample_non_order_sql as $row)
							{
								$sample_non_order_arr[$row[csf('id')]]=$row[csf('booking_no')];
							}
						...
						else
						{
							...
							else
							{
								...
								$booking_qnty_array=return_library_array("select a.po_break_down_id, sum(a.grey_fab_qnty) as qnty from wo_booking_dtls a, wo_pre_cost_fabric_cost_dtls b where a.pre_cost_fabric_cost_dtls_id=b.id and a.booking_no='$booking_no' and b.lib_yarn_count_deter_id='$fabric_desc_id' and b.body_part_id='$cbo_body_part' and b.gsm_weight='$txt_gsm' $txt_width_cond  and a.job_no=b.job_no and a.booking_type <> 3 and a.status_active=1 and a.is_deleted=0 group by a.po_break_down_id
									union all
									select  a.po_break_down_id, sum(a.wo_qnty) as qnty from wo_booking_dtls a, wo_pre_cost_fab_conv_cost_dtls b, wo_pre_cost_fabric_cost_dtls c where a.pre_cost_fabric_cost_dtls_id = b.id and a.booking_type = 3 and b.fabric_description = c.id and a.status_active = 1 and b.status_active = 1 and a.job_no = b.job_no and a.booking_no = '$booking_no' and c.lib_yarn_count_deter_id ='$fabric_desc_id' and c.gsm_weight='$txt_gsm' and  c.body_part_id = '$cbo_body_part'  $txt_width_cond group by  a.po_break_down_id","po_break_down_id","qnty");
						...
						foreach($nameArray as $row)
						{
							...
							<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
								...
								if($receive_basis==11 && $booking_without_order==1)
								{
									?>
									<td width="130">
										<p><? echo $sample_non_order_arr[$row[csf('id')]]; ?></p>
										<input type="hidden" name="txtPoId[]" id="txtPoId_<? echo $i; ?>" value="<? echo $row[csf('id')]; ?>">
										<input type="hidden" name="txtOrginal[]" id="txtOrginal_<? echo $i; ?>" value="<? echo $orginal_val; ?>">
										<input type="hidden" name="txtRollId[]" id="txtRollId_<? echo $i; ?>" value="<? echo $roll_not_delete_id; ?>">
										<!--This is used for not delete row which is used in batch -->
			...
			<table width="<? echo $width-20; ?>" border="1" cellpadding="0" cellspacing="0" rules="all" class="tbl_bottom">
				<tr>
					...
					if($receive_basis==9 && ($body_part_type[$cbo_body_part] ==40 || $body_part_type[$cbo_body_part] ==50) && $roll_maintained==0)
					{
						...
						<td style="text-align:center;width:50px"><input type="text" name="txt_tot_grey_qnty_in_pcs" id="txt_tot_grey_qnty_in_pcs" class="text_boxes_numeric" style="width:40px" value="<? echo number_format($tot_grey_qnty_in_pcs,2); ?>" disabled></td>
						<?
					}
					?>
			...
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="810" class="rpt_table" id="tbl_list_search" >
				...
				foreach ($nameArray as $selectResult)
				{
					...
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value(<? echo $i; ?>,1)">
						...
						<td width="100"><p><? echo $selectResult[csf('file_no')]; ?>&nbsp;</p></td>
...
if ($action=="save_update_delete")
{
	...
	if(str_replace("'","",$garments_nature)*1 !=2)
	{
		echo "30**Garments Nature not found";
		die;
	}
	...
	if(str_replace("'","",$update_id)!="")
	{
		...
		if($recBill=="") // Subcon Bill Issue
		{
			if(str_replace("'","",$cbo_knitting_source)==1)
			{
				$nameArray_vari= sql_select("select id, dyeing_fin_bill, allow_per,variable_list from variable_settings_subcon where company_id=$cbo_company_id and variable_list=7 order by id");
				...
				if($in_house_knit_bill_from=="") $in_house_knit_bill_from=2;

				if ($in_house_knit_bill_from==0) // source Recv page
				{					
					$inboundBill=return_field_value("bill_no","subcon_inbound_bill_mst a, subcon_inbound_bill_dtls b","a.id=b.mst_id and b.delivery_id=$update_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","bill_no");
					if($inboundBill!="")
					{
						echo "30**In-Bound->Knitting Bill Issue.Bill No:".$inboundBill;
						die;
				...
				else // source delivery page
				{					
					$rec_challan_no=return_field_value("booking_no","inv_receive_master","id=$update_id and status_active=1 and is_deleted=0","booking_no");//booking_no is GPE
					if($rec_challan_no!="")
					{
						$inboundBill=return_field_value("bill_no","subcon_inbound_bill_mst a, subcon_inbound_bill_dtls b","a.id=b.mst_id and b.rec_challan_no='$rec_challan_no' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","bill_no");
						if($inboundBill!="")
						{
							echo "30**In-Bound->Knitting Bill Issue.Bill No:".$inboundBill;
							die;
			...
			{
				$outboundBill2=return_field_value("bill_no","subcon_outbound_bill_mst a, subcon_outbound_bill_dtls b","a.id=b.mst_id and a.entry_form=438 and b.receive_id=$update_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","bill_no");
				if($outboundBill2!="")
				{
					echo "30**Out-Bound->Knitting Bill Entry[Gross].Bill No:".$outboundBill2;
					die;
				}
			}
		}
	...
	if ($operation==0)  // Insert Here
	{
		...
		if(str_replace("'","",$garments_nature)==2 || str_replace("'","",$garments_nature)==0)
		{
		...
		if(str_replace("'","",$update_id)=="")
		{
			...
			//echo "10**"; print_r($new_grey_recv_system_id); die;
		...
		{
			$field_array_update="receive_basis*receive_date*challan_no*booking_id*booking_no*booking_without_order*store_id*location_id*knitting_source*knitting_company*buyer_id*yarn_issue_challan_no*boe_mushak_challan_no*boe_mushak_challan_date*remarks*updated_by*update_date";

			$data_array_update=$cbo_receive_basis."*".$txt_receive_date."*".$txt_receive_chal_no."*".$txt_booking_no_id."*".$txt_booking_no."*".$booking_without_order."*".$cbo_store_name."*".$cbo_location."*".$cbo_knitting_source."*".$cbo_knitting_company."*".$cbo_buyer_name."*".$txt_yarn_issue_challan_no."*".$txt_boe_mushak_challan_no."*".$txt_boe_mushak_challan_date."*".$txt_remarks."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		else
		{
			...
			{
				$row_prod=sql_select("select id, current_stock, avg_rate_per_unit, stock_value from product_details_master where company_id=$cbo_company_id and item_category_id=$category_id and detarmination_id=$fabric_desc_id and gsm=$txt_gsm and upper(dia_width)=$txt_width and status_active=1 and is_deleted=0");
			}
		...
		// $rate=0; $amount=0; replace by  $cons_rate=0; $cons_amount=0;
		// ******** Add For Accountce **************************************************
		...
		//$barcode_suffix_no 	= return_next_id_by_sequence("ROLL_BARCODE_SUFFIX_NO_SEQ", "barcode_suffix_no", "pro_roll_details", $con);
		...
		{
			$roll_arr=return_library_array("select po_breakdown_id,max(roll_no) as roll_no from pro_roll_details where entry_form in(2,22,62) group by po_breakdown_id",'po_breakdown_id','roll_no');
		}
		...
		$po_array=array();$po_array2=array();$po_array_qntyInPcs=array();$po_array_qntyInPcs_size=array(); $barcode_year = date("y");
		//echo "10**";
		for($i=0;$i<count($save_string);$i++)
		{
			...
			$order_qntyInPcs_wise=$order_dtls[6];
			$order_qntyInPcs_wise_size=$order_dtls[7];
			...
			*/
			...
			if(array_key_exists($order_id,$po_array))
			{
				...
				$po_array_qntyInPcs[$order_id]+=$order_qntyInPcs_wise;
				$po_array_qntyInPcs_size[$order_id]=$order_qntyInPcs_wise_size;
			}
			else
			{
				$po_array[$order_id]=$order_qnty_roll_wise;
		...
		if(str_replace("'", '',$cbo_receive_basis)==9 && ($body_part_type[str_replace("'", '',$cbo_body_part)] ==40 || $body_part_type[str_replace("'", '',$cbo_body_part)] ==50) && str_replace("'","",$roll_maintained)==0)
		{
			$field_array_proportionate="id, trans_id, trans_type, entry_form, dtls_id, po_breakdown_id, prod_id, quantity,reject_qty, is_sales,quantity_pcs,coller_cuff_size, inserted_by, insert_date";//, color_id
		}
		else
		{
		...
		foreach($po_array as $key=>$val)
		{
			...
			$qtyInPcs=$po_array_qntyInPcs[$order_id];
			$qtyInPcs_size=$po_array_qntyInPcs_size[$order_id];
			if(str_replace("'", '',$cbo_receive_basis)==9 && ($body_part_type[str_replace("'", '',$cbo_body_part)] ==40 || $body_part_type[str_replace("'", '',$cbo_body_part)] ==50) && str_replace("'","",$roll_maintained)==0)
			{
				if($data_array_prop!="") $data_array_prop.= ",";
				$data_array_prop.="(".$id_prop.",".$id_trans.",1,$entry_form,".$id_dtls.",'".$order_id."',".$prod_id.",'".$order_qnty."','".$tot_reject_qty."',".$is_sales.",'".$qtyInPcs."','".$qtyInPcs_size."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";//,'".$color_id."'
			}
			else
			{
				if($data_array_prop!="") $data_array_prop.= ",";
				$data_array_prop.="(".$id_prop.",".$id_trans.",1,$entry_form,".$id_dtls.",'".$order_id."',".$prod_id.",'".$order_qnty."','".$tot_reject_qty."',".$is_sales.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";//,'".$color_id."'
			}
		...
		//echo str_replace("'", '',$cbo_receive_basis).'='.$body_part_type[str_replace("'", '',$cbo_body_part)] .'='.$body_part_type[str_replace("'", '',$cbo_body_part)].'='.str_replace("'","",$roll_maintained).'*'.str_replace("'", '',$cbo_body_part);die;

		//echo "5**insert into pro_roll_details (".$field_array_roll.") values ".$data_array_roll;
		...
		//echo "10**".$flag;die;
		//echo "5**$rID==$rID2==$rID3==$rID4==$rID5==$rID6==$rID7 && $flag";oci_rollback($con);die;
	...
	else if ($operation==1)   // Update Here
	{
		...
		if(str_replace("'","",$roll_maintained)==1 && str_replace("'","",$txt_deleted_id)!="")
		{
			$deleted_roll_id=str_replace("'","",$txt_deleted_id);
			$roll_recv_sql=sql_select("SELECT barcode_no from pro_roll_details where id in($deleted_roll_id) and entry_form=22 and status_active=1 and is_deleted=0");
			foreach ($roll_recv_sql as $key => $row) 
			{
				$deleted_barcode_arr[$row[csf('barcode_no')]]=$row[csf('barcode_no')];
			}
			$deleted_barcode=implode(",", $deleted_barcode_arr);
			$iss_sql=sql_select("SELECT barcode_no from pro_roll_details where barcode_no in($deleted_barcode) and entry_form=61 and status_active=1 and is_deleted=0");
			$txt_deleted_id=str_replace("'","",$txt_deleted_id);
			if($txt_deleted_id!="" && !empty($iss_sql))
			{
				echo '30**Issue Found Delete Not Allowed';die;
			}
		}
		// echo "10**string";die;
		...
		$field_array_update="receive_date*challan_no*booking_id*booking_no*booking_without_order*store_id*buyer_id*yarn_issue_challan_no*boe_mushak_challan_no*boe_mushak_challan_date*remarks*updated_by*update_date";

		$data_array_update=$txt_receive_date."*".$txt_receive_chal_no."*".$txt_booking_no_id."*".$txt_booking_no."*".$booking_without_order."*".$cbo_store_name."*".$cbo_buyer_name."*".$txt_yarn_issue_challan_no."*".$txt_boe_mushak_challan_no."*".$txt_boe_mushak_challan_date."*".$txt_remarks."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		$po_array=array(); $po_array2=array(); $po_array_qntyInPcs=array(); $po_array_qntyInPcs_size=array();
		for($i=0;$i<count($save_string);$i++)
		{
			...
			//for reject Production QtyInPcs
			$order_qntyInPcs_wise=$order_dtls[6];
			$order_qntyInPcs_wise_size=$order_dtls[7];
		...
		foreach($po_array as $key=>$val)
		{
			...
			{
				if($data_array_prop!="") $data_array_prop.= ",";
				$data_array_prop.="(".$id_prop.",".$update_trans_id.",1,$entry_form,".$update_dtls_id.",'".$order_id."',".$prod_id.",'".$order_qnty."','".$tot_reject_qty."',".$is_sales.",'".$qtyInPcs."','".$qtyInPcs_size."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";//,'".$color_id."'
			}
			else
			{
				if($data_array_prop!="") $data_array_prop.= ",";
				$data_array_prop.="(".$id_prop.",".$update_trans_id.",1,$entry_form,".$update_dtls_id.",'".$order_id."',".$prod_id.",'".$order_qnty."','".$tot_reject_qty."',".$is_sales.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";//,'".$color_id."'
			}
		}
...
if($action=="create_grey_recv_search_list_view")
{
	...
	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="940" class="rpt_table">
		<thead>
			...
			<th width="115">Knitting Source</th>
...
if($action=='populate_data_from_grey_recv')
{
	...
	foreach ($data_array as $row)
	{
		...
		{
			$within_group=return_field_value("within_group","fabric_sales_order_mst","id='".$row[csf("booking_id")]."'");
			if($within_group ==1)
			{
				echo "load_drop_down('requires/grey_fabric_receive_controller','". $row[csf("buyer_id")] ."', 'load_drop_down_company_in_buyer_td','buyer_td_id');\n";
			}
		...
		echo "load_room_rack_self_bin('requires/grey_fabric_receive_controller*13', 'store','store_td', '".$row[csf('company_id')]."','".$row[csf('location_id')]."',this.value);\n";
		...
		echo "load_drop_down( 'requires/grey_fabric_receive_controller', ".$row[csf("knitting_source")]."+'_'+".$row[csf("company_id")].", 'load_drop_down_knitting_com_new','knitting_com');\n";
...
if($action=='populate_grey_details_form_data')
{
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_room_rack_self_bin('requires/grey_fabric_receive_controller', 'floor','floor_td', $('#cbo_company_id').val(),$('#cbo_location').val(),$('#cbo_store_name').val(),this.value);\n";
		...
		echo "load_room_rack_self_bin('requires/grey_fabric_receive_controller', 'room','room_td', $('#cbo_company_id').val(),$('#cbo_location').val(),$('#cbo_store_name').val(),'".$row[csf('floor_id')]."',this.value);\n";
		echo "document.getElementById('cbo_room').value 					= '".$row[csf("room")]."';\n";

		echo "load_room_rack_self_bin('requires/grey_fabric_receive_controller', 'rack','rack_td', $('#cbo_company_id').val(),$('#cbo_location').val(),$('#cbo_store_name').val(),'".$row[csf('floor_id')]."','".$row[csf('room')]."',this.value);\n";
		echo "document.getElementById('txt_rack').value 					= '".$row[csf("rack")]."';\n";
		echo "load_room_rack_self_bin('requires/grey_fabric_receive_controller', 'shelf','shelf_td', $('#cbo_company_id').val(),$('#cbo_location').val(),$('#cbo_store_name').val(),'".$row[csf('floor_id')]."','".$row[csf('room')]."','".$row[csf('rack')]."',this.value);\n";
		echo "document.getElementById('txt_shelf').value 					= '".$row[csf("self")]."';\n";
		echo "load_room_rack_self_bin('requires/grey_fabric_receive_controller', 'bin','bin_td', $('#cbo_company_id').val(),$('#cbo_location').val(),$('#cbo_store_name').val(),'".$row[csf('floor_id')]."','".$row[csf('room')]."','".$row[csf('rack')]."','".$row[csf('self')]."',this.value);\n";
		...
		if($receive_basis==11)
		{
			$material_data_sql = sql_select("select id,prod_id,item_category,used_qty,rate,amount, yarn_percentage, porcess_loss from pro_material_used_dtls where status_active=1 and is_deleted=0 and mst_id=".$row[csf('mst_id')]." and dtls_id=".$row[csf('id')]." and item_category=1 and entry_form=22 ");
		...
		{
			$data_po_array=sql_select("select po_breakdown_id, quantity,reject_qty,quantity_pcs,coller_cuff_size from order_wise_pro_details where dtls_id='$id' and entry_form=$entry_form and status_active=1 and is_deleted=0");
			foreach($data_po_array as $row_po)
			{
				if($save_string=="")
				{
					$save_string=$row_po[csf("po_breakdown_id")]."**".$row_po[csf("quantity")]."**"."**"."**"."**"."**".$row_po[csf("quantity_pcs")]."**".$row_po[csf("coller_cuff_size")];
...
if($action=="roll_maintained")
{
	...
	echo "reset_form('greyreceive_1','list_fabric_desc_container','','','set_receive_basis();','update_id*txt_recieved_id*cbo_company_id*cbo_receive_basis*txt_receive_date*txt_receive_chal_no*cbo_store_name*cbo_knitting_source*cbo_knitting_company*txt_boe_mushak_challan_no*txt_boe_mushak_challan_date*txt_remarks*roll_maintained*barcode_generation*txt_yarn_issue_challan_no*txt_shift_name*txt_width*txt_gsm*cbo_floor*cbo_machine_name*cbo_room*txt_rack*txt_reject_fabric_recv_qnty*txt_shelf*cbo_uom*cbo_bin*txt_yarn_lot*cbo_yarn_count*txt_brand*cbo_color_range*process_costing_maintain*grey_prod_dtls_id*store_update_upto');\n";
...
if ($action=="grey_fabric_receive_print")
{
	...
	$sql="SELECT a.id, a.recv_number, a.item_category, a.receive_basis, a.receive_date, a.challan_no, a.booking_id, a.booking_no, a.store_id, a.knitting_source, a.knitting_company, a.location_id, a.yarn_issue_challan_no, a.boe_mushak_challan_no, a.boe_mushak_challan_date, a.buyer_id, a.fabric_nature, a.remarks, b.order_id, a.booking_without_order
	...
	$job_sql = sql_select("select a.id, a.job_no,a.style_ref_no, b.id as po_id, b.po_number from  wo_po_details_master a, wo_po_break_down b where  a.id=b.job_id and a.status_active=1 and a.is_deleted=0");
	foreach($job_sql as $row){
		$po_arr[$row[csf("id")]] = $row[csf("job_no")];
		$po_number_arr[$row[csf("po_id")]] = $row[csf("po_number")];
		$style_ref_no_arr[$row[csf("po_id")]] = $row[csf("style_ref_no")];
	}
	unset($job_sql);
	...
	<div style="width:1050px;">
		<table width="1020" cellspacing="0" align="right">
			...
			<tr>
				...
				<td><strong>Knit. Com:</strong></td><td width="200px"><? if ($dataArray[0][csf('knitting_source')]==1) echo $company_library[$dataArray[0][csf('knitting_company')]]; else if($dataArray[0][csf('knitting_source')]==3) echo $supplier_library[$dataArray[0][csf('knitting_company')]];  ?></td>
			...
			<tr>
				...
				<?
					...
					foreach($po_number as $po_num)
					{
						...
						if($style_no=="")
						{
							$style_no=$style_ref_no_arr[$po_num];
						}
						else
						{
							$style_no.=",".$style_ref_no_arr[$po_num];
			...
			<tr>
                ...
                <td><strong>Style No:</strong></td><td width="175px"><? echo implode(",",array_unique(array_filter(explode(",",$style_no)))); ?></td>
			</tr>
		</table>
		<br>
		<div style="width:100%;">
			<table align="right" cellspacing="0" width="1020"  border="1" rules="all" class="rpt_table" >
				<thead bgcolor="#dddddd" align="center">
					...
					<th width="110" >Fab. Description</th>
...
if($action=="show_roll_listview")
{
	...
	if($booking_without_order==1)
	{
		$query="select id,roll_no,barcode_no,po_breakdown_id,qnty,booking_no as po_number, is_sales from pro_roll_details  where dtls_id=$dtls_id and entry_form=22 and status_active=1 and is_deleted=0";
		//$caption="Booking No.";
	}
	else
	{
		...
		$query="SELECT a.id, a.roll_no, a.barcode_no, a.po_breakdown_id, a.qnty, b.po_number, a.booking_without_order, a.is_sales from pro_roll_details a, wo_po_break_down b where a.po_breakdown_id=b.id and a.dtls_id=$dtls_id and a.entry_form=22 and a.is_sales=0 and a.is_sales=0 and a.status_active=1 and a.is_deleted=0 union all select a.id, a.roll_no, a.barcode_no, a.po_breakdown_id, a.qnty, b.job_no as po_number, a.booking_without_order, a.is_sales from pro_roll_details a, fabric_sales_order_mst b where a.po_breakdown_id=b.id and a.dtls_id=$dtls_id and a.entry_form=22 and a.is_sales=1 and a.status_active=1 and a.is_deleted=0 order by id";
...
if($action=="bill_generate")
{
	...
	$sqlRec="select knitting_source, receive_basis, company_id, knitting_company, location_id, receive_date, booking_without_order from inv_receive_master where entry_form=22 and id='$recId' and item_category=13";
	//echo $sqlRec; die;
		...
		if($booking_without_order==0)//with Order
		{
			if($receive_basis==9)//Receive Basis Production
			{
				$sql="select a.id, d.entry_form, a.recv_number_prefix_num, a.challan_no, a.receive_date, b.prod_id, b.body_part_id, b.febric_description_id, sum(b.no_of_roll) as roll_qty, c.po_breakdown_id, sum(c.quantity) as quantity, sum(b.rate) as rate, sum(b.amount) as amount, d.receive_basis, d.booking_no, d.booking_id from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c, inv_receive_master d
				 where a.id=b.mst_id and $bookingWithoutOrderCond=0 and b.id=c.dtls_id and d.id=a.booking_id and a.knitting_source=1 and a.company_id='$company_id' and a.knitting_company='$knitting_company' and a.location_id='$location_id' and c.trans_type=1 and a.entry_form=22 and c.entry_form=22 and a.receive_basis=9 and d.receive_basis!=4 and a.item_category=13 and d.entry_form=2 and c.trans_id!=0
			...
			where a.id=b.mst_id and $bookingWithoutOrderCond=0 and b.id=c.dtls_id and a.knitting_source=1 and a.company_id='$company_id' and a.knitting_company='$knitting_company' and a.location_id='$location_id' and c.trans_type=1 and a.entry_form=22 and c.entry_form=22 and a.item_category=13 and a.receive_basis in (2,4,11) and c.trans_id!=0
			and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.id='$recId'
		...
		else if($booking_without_order==1)//with out Order
		{
			if($receive_basis==9)//Receive Basis Production
			{
				$sql="select a.id, a.recv_number_prefix_num, a.buyer_id, a.challan_no, a.receive_date, b.prod_id, b.body_part_id, b.febric_description_id, sum(b.no_of_roll) as roll_qty, sum(b.grey_receive_qnty) as quantity, c.entry_form, c.receive_basis, c.booking_no, c.booking_id, 0 as order_id
					...
					where a.id=b.mst_id and c.id=a.booking_id and a.booking_without_order=1 and a.knitting_source=1 and a.company_id='$company_id' and a.knitting_company='$knitting_company' and a.location_id='$location_id' and a.entry_form=22 and a.receive_basis=9 and a.item_category=13 and c.entry_form=2 and c.receive_basis in (0,1,2) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.trans_id!=0 and a.id='$recId'
			...
			else if($receive_basis==2 || $receive_basis==4 || $receive_basis==11) //Receive Basis WO/Booking Based, Independent, Service Booking Based
			{
				$sql="select a.id, a.recv_number_prefix_num, a.buyer_id, a.challan_no, a.receive_date, b.prod_id, b.body_part_id, b.febric_description_id, sum(b.no_of_roll) as roll_qty, sum(b.grey_receive_qnty) as quantity, a.entry_form, a.receive_basis, a.booking_no, a.booking_id, 0 as order_id
					from inv_receive_master a, pro_grey_prod_entry_dtls b
					where a.id=b.mst_id and a.booking_without_order=1 and a.knitting_source=1 and a.company_id='$company_id' and a.knitting_company='$knitting_company' and b.trans_id!=0 and a.location_id='$location_id' and a.entry_form=22 and a.item_category=13 and a.receive_basis in (2,4,11)
	...
	else if($partySource==3)
	{
		...
		else if($booking_without_order==1)//with out Order
		{
			...
			else if($receive_basis==2 || $receive_basis==4 || $receive_basis==11) //Receive Basis WO/Booking Based, Independent, Service Booking Based
			{
				$sql="select a.id, a.recv_number_prefix_num, a.buyer_id, a.challan_no, a.receive_date, b.prod_id, b.body_part_id, b.febric_description_id, sum(b.no_of_roll) as roll_qty, sum(b.grey_receive_qnty) as quantity, a.entry_form, a.receive_basis, a.booking_no, a.booking_id, 0 as order_id
					from inv_receive_master a, pro_grey_prod_entry_dtls b
					where a.id=b.mst_id and a.booking_without_order=1 and a.knitting_source=3 and a.company_id='$company_id' and a.knitting_company='$knitting_company' and b.trans_id!=0 and a.location_id='$location_id' and a.entry_form=22 and a.item_category=13 and a.receive_basis in (2,4,11)
					and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.id='$recId'
					group by a.id, a.recv_number_prefix_num, a.buyer_id, a.challan_no, a.receive_date, b.prod_id, b.body_part_id, b.febric_description_id, a.entry_form, a.receive_basis, a.booking_no, a.booking_id";
			}
		}
...
if($action=="company_wise_report_button_setting")
{
	...
	$print_report_format=return_field_value("format_id","lib_report_template","template_name ='".$data."'  and module_id=6 and report_id=257 and is_deleted=0 and status_active=1");
