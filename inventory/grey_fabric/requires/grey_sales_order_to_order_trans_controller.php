...
if ($action=="order_popup") // From Roll wise Grey Sales Order To Sales Order Transfer
{
				...
				<fieldset style="width:750px;">
					<table cellpadding="0" cellspacing="0" width="750" class="rpt_table" border="1" rules="all">
						...
						<tr class="general">
							<td>
								<?
								echo create_drop_down("cbo_within_group", 70, $yes_no, "", 1, "--Select--", 0, "load_drop_down( 'grey_sales_order_to_order_trans_controller', this.value+'_'+".$cbo_company_id.", 'load_drop_down_buyer', 'buyer_td' );");
								?>
							</td>
...
if($action=='create_po_search_list_view') // From Roll wise Grey Sales Order To Sales Order Transfer
{
	$data=explode('_',$data);
	$company_id=$data[2];
	//$fromOrderId=$data[7];
	$cbo_company_id_to=$data[7];
	...
	if($type=="from") $company_cond=" and a.company_id=$company_id "; else $company_cond=" and a.company_id=$cbo_company_id_to";
...
if($action=='populate_data_from_order')
{
	...
	from fabric_sales_order_mst a, fabric_sales_order_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.is_deleted=0 and a.id='$po_id'
	group by a.job_no, a.job_no_prefix_num, a.company_id, a.style_ref_no, a.sales_booking_no,a.po_company_id,a.po_buyer, a.buyer_id, a.booking_id");
...
if($action=="load_drop_down_item_desc")
{
	...
	$sql="select a.id, a.product_name_details from product_details_master a, order_wise_pro_details b where a.id=b.prod_id and b.po_breakdown_id=$data and b.entry_form in(2,22,362) and b.trans_type in(1,5)  and b.status_active=1 and b.is_deleted=0";
...
if($action=="show_dtls_list_view")
{
	...
	and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.receive_basis<>9 and c.trans_id>0 and b.is_sales=1
	UNION ALL
	...
	and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.receive_basis=9 and c.trans_id>0 and b.is_sales=1
	UNION ALL
	...
	where a.id=b.prod_id and b.dtls_id=c.id and c.mst_id=d.id and a.item_category_id=13 and b.entry_form in(362) and d.entry_form in(362) and b.po_breakdown_id=$data and b.trans_type=5 and a.status_active=1
	...
	foreach($data_array as $row )
	{
		...
		$all_data_arr[$array_group]['brand_id'] = $row[csf('brand')];
	...
	where b.id=c.mst_id and c.id=d.dtls_id and a.id=d.trans_id and a.id=c.trans_id and d.trans_id=c.trans_id and b.entry_form in(362,81) and d.entry_form in(362,81) and b.status_active=1 and c.status_active=1 and d.status_active=1  and d.po_breakdown_id in($data) and d.trans_type=6
...
if($action=='create_transfer_search_list_view')
{
	...
	if($db_type==0)
	{
		...
		from inv_item_transfer_mst where item_category=13 and company_id=$company_id and $search_field like '$search_string' and transfer_criteria=$transfer_criteria_id $transfer_date and entry_form in(362) and status_active=1 and is_deleted=0 order by id";
	}
	else
	{
		$sql="SELECT id, transfer_prefix_number, $year_field transfer_system_id, challan_no, company_id, transfer_date, transfer_criteria, item_category, to_company
...
if($action=='populate_data_from_transfer_master')
{
	...
	foreach ($data_array as $row)
	{
		...
		echo "get_php_form_data('".$row[csf("from_order_id")]."**from**$transfer_criteria'".",'populate_data_from_order','requires/grey_sales_order_to_order_trans_controller');\n";
		echo "get_php_form_data('".$row[csf("to_order_id")]."**to**$transfer_criteria'".",'populate_data_from_order','requires/grey_sales_order_to_order_trans_controller');\n";
...
if($action=="show_transfer_listview")
{
	...
	echo create_list_view("list_view", "Item Category,Item Description,Transfered Qnty,UOM, Rack, Shelf", "120,250,100,70,80","730","200",0, $sql, "get_php_form_data", "id", "'populate_transfer_details_form_data'", 0, "item_category,from_prod_id,0,uom,0,0", $arr, "item_category,from_prod_id,transfer_qnty,uom,rack,shelf", "requires/grey_sales_order_to_order_trans_controller",'','0,0,2,0,0,0');
...
if($action=='populate_transfer_details_form_data')
{
	...
	(case when  c.from_program !=0  then c.from_program else 0 end) as booking_id
	from product_details_master a, order_wise_pro_details b, inv_item_transfer_dtls c, inv_item_transfer_mst d
	where a.id=b.prod_id and b.dtls_id=c.id and c.mst_id=d.id and a.item_category_id=13 and b.entry_form in(362) and d.entry_form in(362) and b.po_breakdown_id=".$data_array[0][csf('from_order_id')]." and b.trans_type=5 and a.status_active=1
	and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	...
	$trans_out_sql="SELECT a.prod_id, a.store_id, a.floor_id,a.room, a.rack, a.self, d.trans_type, d.po_breakdown_id, sum(d.quantity) as trans_out_qnty, c.from_program
	from inv_transaction a, inv_item_transfer_mst b, inv_item_transfer_dtls c, order_wise_pro_details d
	where b.id=c.mst_id and c.id=d.dtls_id and a.id=d.trans_id and a.id=c.trans_id and d.trans_id=c.trans_id and b.entry_form in(362,81) and d.entry_form in(362,81) and b.status_active=1 and c.status_active=1 and d.status_active=1 and d.po_breakdown_id =".$data_array[0][csf('from_order_id')]." and d.trans_type=6
	group by a.prod_id, a.store_id, a.floor_id,a.room, a.rack, a.self, d.trans_type, d.po_breakdown_id, c.from_program";
	...
	foreach ($data_array as $row)
	{
		...
		if($row[csf("to_store")]>0){
			echo "load_drop_down('requires/grey_sales_order_to_order_trans_controller','".$to_company_id."', 'load_drop_down_store_to', 'to_store_td' );\n";
		...
		if($row[csf("to_floor_id")]>0){
			...
			echo "load_drop_down('requires/grey_sales_order_to_order_trans_controller','".$store_com."', 'load_drop_down_floor_to', 'floor_td_to' );\n";
		...
		if($row[csf("to_room")]>0){
			...
			echo "load_drop_down('requires/grey_sales_order_to_order_trans_controller','".$floor_com_store."', 'load_drop_down_room_to', 'room_td_to' );\n";
		...
		if($row[csf("to_rack")]>0){
			...
			echo "load_drop_down('requires/grey_sales_order_to_order_trans_controller','".$room_floor_com_store."', 'load_drop_down_rack_to', 'rack_td_to' );\n";
		...
		if($row[csf("to_shelf")]>0){
			...
			echo "load_drop_down('requires/grey_sales_order_to_order_trans_controller','".$rack_room_floor_com_store."', 'load_drop_down_shelf_to', 'shelf_td_to' );\n";
...
{
	echo "<div style='color: red; font-size:20px;'>"."&diams; Development Later &diams;"."</div>";die;
	...
	where a.id=b.mst_id and a.item_category=13 and a.company_id=$company_id and a.transfer_criteria=$transfer_criteria_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $transfer_date and a.entry_form=362 and a.transfer_requ_id IS NOT NULL
            ...
            <table border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all" width="750" align="center">
                ...
                <?
					...
					$sql="SELECT
								...
								sum(CASE WHEN entry_form ='362' and trans_type=5 THEN quantity ELSE 0 END) AS transfer_out_qnty,
								sum(CASE WHEN entry_form ='362' and trans_type=6 THEN quantity ELSE 0 END) AS transfer_in_qnty,
...
if($action=="save_update_delete")
{
	...
	from(
	SELECT a.id as prod_id, a.product_name_details, a.lot, d.store_id as to_store, c.store_floor as to_floor, c.room as to_room, c.rack as to_rack, c.self as to_shelf, sum(b.quantity) as qnty,
	(case when d.entry_form =2 and d.receive_basis=2  then d.booking_id else 0 end) as booking_id
	from product_details_master a, order_wise_pro_details b, pro_grey_prod_entry_dtls c, inv_receive_master d
	where a.id=b.prod_id and b.dtls_id=c.id and c.mst_id=d.id and a.item_category_id=13 and b.entry_form in(2,22) and d.entry_form in(2,22) and b.po_breakdown_id=$from_order_id and d.store_id=$store_cond and a.id=$item_desc_cond $form_prog_conda and c.store_floor=$floor_cond and c.room=$room_cond and c.rack=$rack_cond and c.self=$shelf_cond and a.status_active=1
	and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.receive_basis<>9 and c.trans_id>0
	group by a.id, a.product_name_details, a.lot, d.receive_basis, d.booking_id, d.entry_form, d.store_id, c.store_floor, c.room, c.rack, c.self
	UNION ALL
	...
	(case when  c.from_program !=0  then c.from_program else 0 end) as booking_id
	from product_details_master a, order_wise_pro_details b, inv_item_transfer_dtls c, inv_item_transfer_mst d
	where a.id=b.prod_id and b.dtls_id=c.id and c.mst_id=d.id and a.item_category_id=13 and b.entry_form in(362) and d.entry_form in(362) and b.po_breakdown_id=$from_order_id and c.to_store=$store_cond and a.id=$item_desc_cond $form_prog_condc and c.to_floor_id=$floor_cond and c.to_room=$room_cond and c.to_rack=$rack_cond and c.to_shelf=$shelf_cond and b.trans_type=5 and a.status_active=1
	and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
	...
	$trans_out_sql="SELECT a.prod_id, a.store_id, a.floor_id,a.room, a.rack, a.self, d.trans_type, d.po_breakdown_id, sum(d.quantity) as trans_out_qnty, c.from_program
	from inv_transaction a, inv_item_transfer_mst b, inv_item_transfer_dtls c, order_wise_pro_details d
	where b.id=c.mst_id and c.id=d.dtls_id and a.id=d.trans_id and a.id=c.trans_id and d.trans_id=c.trans_id and b.entry_form in(362,81) and d.entry_form in(362,81) and b.status_active=1 and c.status_active=1 and d.status_active=1  and d.po_breakdown_id in($from_order_id) and d.trans_type=6
	group by a.prod_id, a.store_id, a.floor_id,a.room, a.rack, a.self, d.trans_type, d.po_breakdown_id, c.from_program";
	//echo $trans_out_sql; and c.from_program in($program_no)
	$transfer_result=sql_select($trans_out_sql);
	$floor_id=$room=$rack=$self="";$trans_out_qnty_array=array();
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$update_id)=="")
		{
			...
			$field_array="id, transfer_prefix, transfer_prefix_number, transfer_system_id, company_id, challan_no, transfer_date, entry_form, transfer_criteria, to_company, from_order_id, to_order_id, item_category, transfer_requ_no, transfer_requ_id, is_acknowledge, is_sales, inserted_by, insert_date";
			$is_acknowledge=0;
			if($variable_auto_rcv!=2) // if auto receive yes(1), then no need to acknowledgement
			{
				$is_acknowledge=1;
			}
			$data_array="(".$id.",'".$new_transfer_system_id[1]."',".$new_transfer_system_id[2].",'".$new_transfer_system_id[0]."',".$cbo_company_id.",".$txt_challan_no.",".$txt_transfer_date.",".$entry_form_no.",".$cbo_transfer_criteria.",".$cbo_company_id_to.",".$txt_from_order_id.",".$txt_to_order_id.",".$cbo_item_category.",".$txt_requisition_no.",".$txt_requisition_id.",".$is_acknowledge.",1,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		else // Store to Store or order to order
		{
			...
			$id_dtls = return_next_id_by_sequence("INV_ITEM_TRANSFER_DTLS_PK_SEQ", "inv_item_transfer_dtls", $con);
			// check existing product or Create new product for company to company $field_array_dtls="to_prod_id"; $product_id;
			$data_array_dtls="(".$id_dtls.",".$transfer_update_id.",".$id_trans.",".$id_trans_recv.",".$cbo_item_desc.",".$cbo_item_desc.",".$txt_from_order_id.",".$txt_to_order_id.",".$cbo_item_category.",".$txt_transfer_qnty.",".$txt_roll.",'".$rate."','".$amount."',".$cbo_uom.",".$hid_ycount.",".$txt_ylot.",".$cbo_store_name.",".$cbo_floor.",".$cbo_room.",".$txt_rack.",".$txt_shelf.",".$cbo_store_name_to.",".$cbo_floor_to.",".$cbo_room_to.",".$txt_rack_to.",".$txt_shelf_to.",".$txt_form_prog.",".$txt_to_prog.",".$stitch_length.",".$hidden_requ_dtls_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1)";
		}

		$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
		$field_array_proportionate="id, trans_id, trans_type, entry_form, dtls_id, po_breakdown_id, prod_id, quantity, inserted_by, insert_date, is_sales";

		$data_array_prop="(".$id_prop.",".$id_trans.",6,362,".$id_dtls.",".$txt_from_order_id.",".$cbo_item_desc.",".$txt_transfer_qnty.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1)";

		//$id_prop=$id_prop+1;
		if($variable_auto_rcv!=2)
	 	{
			$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
			$data_array_prop.=",(".$id_prop.",".$id_trans_recv.",5,362,".$id_dtls.",".$txt_to_order_id.",".$cbo_item_desc.",".$txt_transfer_qnty.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1)";
		}
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		else
		{
			...
			{
				$updateTransID_array[]=$update_trans_recv_id;
				$updateTransID_data[$update_trans_recv_id]=explode("*",("".$cbo_item_desc."*".$txt_transfer_date."*".$txt_to_order_id."*".$cbo_uom."*".$txt_transfer_qnty."*'".$rate."'*'".$amount."'*".$cbo_store_name_to."*".$cbo_floor_to."*".$cbo_room_to."*".$txt_rack_to."*".$txt_shelf_to."*".$txt_to_prog."*".$stitch_length."*0*0*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
			}
			//echo "10**".$update_trans_recv_id;die;

			$data_array_dtls=$update_trans_issue_id."*".$update_trans_recv_id."*".$cbo_item_desc."*".$cbo_item_desc."*".$txt_from_order_id."*".$txt_to_order_id."*".$txt_transfer_qnty."*".$txt_roll."*'".$rate."'*'".$amount."'*".$cbo_uom."*".$hid_ycount."*".$txt_ylot."*".$cbo_store_name."*".$cbo_floor."*".$cbo_room."*".$txt_rack."*".$txt_shelf."*".$cbo_store_name_to."*".$cbo_floor_to."*".$cbo_room_to."*".$txt_rack_to."*".$txt_shelf_to."*".$txt_form_prog."*".$txt_to_prog."*".$stitch_length."*".$hidden_requ_dtls_id."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		}
		...
		$data_array_prop="(".$id_prop.",".$update_trans_issue_id.",6,362,".$update_dtls_id.",".$txt_from_order_id.",".$cbo_item_desc.",".$txt_transfer_qnty.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1)";

		//$id_prop=$id_prop+1;
		if($variable_auto_rcv != 2 ) // if auto receive Yes(1), then no need to acknowledgement
		{
			$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details", $con);
			$data_array_prop.=",(".$id_prop.",".$update_trans_recv_id.",5,362,".$update_dtls_id.",".$txt_to_order_id.",".$cbo_item_desc.",".$txt_transfer_qnty.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1)";
		}
		...
		$query = execute_query("DELETE FROM order_wise_pro_details WHERE dtls_id=$update_dtls_id and entry_form=362");
		{
		...
		// if auto receive Yes(1), then no need to acknowledgement
		// echo "10**".$rID.'**'.$rID2.'**'.$rID3.'**'.$query.'**'.$rID4.'##'.$variable_auto_rcv;die;
		//echo "10**".$flag;die;
