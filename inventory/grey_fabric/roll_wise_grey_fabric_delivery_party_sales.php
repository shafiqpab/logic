...
//--------------------------------------------------------------------------------------------------------------------
	...
	function load_scanned_barcode(issue_id)
	{
		...
		var scanned_barcode_nos=trim(return_global_ajax_value( issue_id, 'load_scanned_barcode_nos', '', 'requires/roll_wise_grey_fabric_delivery_party_sales_controller'));
	...
	function openmypage_issue()
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/roll_wise_grey_fabric_delivery_party_sales_controller.php?cbo_company_id='+cbo_company_id+'&action=issue_popup','Issue Popup', 'width=880px,height=350px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function()
		{
			...
			if(issue_id!="")
			{
				...
				get_php_form_data(issue_id, "populate_data_from_data", "requires/roll_wise_grey_fabric_delivery_party_sales_controller");

				load_drop_down('requires/roll_wise_grey_fabric_delivery_party_sales_controller', po_company_id+"_"+within_group+'_'+cbo_company_id, 'load_drop_down_company', 'party_td' );
				$('#cbo_party').val(po_company_id).attr('disabled','disabled');

				var html=return_global_ajax_value(issue_id, 'populate_barcode_data_update', '', 'requires/roll_wise_grey_fabric_delivery_party_sales_controller');
				if(trim(html)!="")
				{
	...
	{
		window.open("requires/roll_wise_grey_fabric_delivery_party_sales_controller.php?data=" + data+'&action='+action, true );
	}
	...
	function fnc_grey_fabric_issue_roll_wise( operation )
	{
		...
		if(operation==2)
		{
			var response=trim(return_global_ajax_value($("#update_id").val()+"_"+all_barcodes, 'check_barcode_for_delete', '', 'requires/roll_wise_grey_fabric_delivery_party_sales_controller'));
		...
		//cbo_dyeing_comp
		// txt_batch_id
		var data="action=save_update_delete&operation="+operation+'&tot_row='+j+get_submitted_data_string('txt_issue_no*cbo_company_id*cbo_location*cbo_store_name*txt_issue_date*txt_fso_no*hdn_fso_id*cbo_party*update_id*txt_deleted_id*txt_remarks*txt_booking_no*hdn_booking_id*txt_delivery_addr',"../../")+dataString+'&new_barcode_nos='+new_barcode_no;
		// alert(data);return;
		freeze_window(operation);

		http.open("POST","requires/roll_wise_grey_fabric_delivery_party_sales_controller.php",true);
	...
	function create_row(barcode_no)
	{
		...
		var barcode_data=trim(return_global_ajax_value(barcode_nos+"_"+cbo_location+"_"+cbo_store_name, 'populate_barcode_data', '', 'requires/roll_wise_grey_fabric_delivery_party_sales_controller'));
		...
		for(var k=0; k<barcode_datas.length; k++)
		{
			...
			if(withinGroup==1)
			{
				load_drop_down('requires/roll_wise_grey_fabric_delivery_party_sales_controller', po_company_id+"_"+withinGroup, 'load_drop_down_company', 'party_td' );				
			}
			else
			{
				load_drop_down('requires/roll_wise_grey_fabric_delivery_party_sales_controller', buyer_id+"_"+withinGroup, 'load_drop_down_buyer', 'party_td' );
	...
	function fn_deleteRow( rid )
	{
		...
		if(bar_code!="" && dtlsId!="")
		{
			var response=trim(return_global_ajax_value( bar_code+"_"+dtlsId, 'roll_used_check', '', 'requires/roll_wise_grey_fabric_delivery_party_sales_controller'));
			if(response==1)
			{
	...
	{
		var report_ids=return_global_ajax_value( com_id, 'check_report_button', '', 'requires/roll_wise_grey_fabric_delivery_party_sales_controller');
		//alert(report_ids);
		if(trim(report_ids)!="")
		{
			$("#print1").hide();
			...
			<fieldset style="width:810px;">
				...
				<table cellpadding="0" cellspacing="2" width="800">
					...
					<tr>
						...
						<td>
							<?
							echo create_drop_down( "cbo_company_id", 152, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select --", 0, "print_report_button_setting(this.value);load_drop_down('requires/roll_wise_grey_fabric_delivery_party_sales_controller', this.value, 'load_drop_down_location', 'location_td' );",0 );
							?>
						</td>
					...
					<tr>
						...
						<td align="right">Sales/Booking</td>
						<td>
					...
					<tr>
						...
						<td align="right">Delivery Addr.</td>
						<td colspan="3">
							<input type="text" name="txt_delivery_addr" id="txt_delivery_addr" class="text_boxes" style="width:390px">
						</td>
