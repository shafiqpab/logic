<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Woven Grey Fabric Receive
Functionality	:	
JS Functions	:
...
<script>
	...
	function openmypage_wo_pi_production_popup()
	{
		...
		{ 	
			...
			var page_link = 'requires/woven_grey_fabric_receive_controller.php?cbo_company_id='+cbo_company_id+'&recieve_basis='+recieve_basis+'&garments_nature='+garments_nature+'&action=wo_pi_production_popup';
			...
			emailwindow.onclose=function()
			{
				...
				if(theemail!="")
				{
					...
					{
						get_php_form_data(theemail+"**"+booking_without_order+"**"+recieve_basis+"**"+roll_maintained, "populate_data_from_booking", "requires/woven_grey_fabric_receive_controller" );
						show_list_view(theename+"**"+booking_without_order+"**"+recieve_basis+"**"+cbo_company_id,'show_fabric_desc_listview','list_fabric_desc_container','requires/woven_grey_fabric_receive_controller','');
						//if(recieve_basis==11)
						//{
							$('#cbo_knitting_source').val(3);
							load_drop_down('requires/woven_grey_fabric_receive_controller','3_'+cbo_company_id, 'load_drop_down_knitting_com','knitting_com');
					...
					{
						get_php_form_data(theemail+"**"+booking_without_order+"**"+recieve_basis+"**"+roll_maintained, "populate_data_from_sales_order", "requires/woven_grey_fabric_receive_controller" );
						
						show_list_view(theename+"**"+booking_without_order+"**"+recieve_basis+"**"+cbo_company_id,'show_fabric_desc_listview_sales_order','list_fabric_desc_container','requires/woven_grey_fabric_receive_controller','');
					...
					else
					{
						if(recieve_basis==9)
						{
							...
							load_drop_down( 'requires/woven_grey_fabric_receive_controller',data[0]+'_'+document.getElementById('cbo_company_id').value, 'load_drop_down_knitting_com','knitting_com');
						...
						{
							$('#cbo_knitting_source').val(3);
							load_drop_down( 'requires/woven_grey_fabric_receive_controller',3+'_'+document.getElementById('cbo_company_id').value, 'load_drop_down_knitting_com','knitting_com');
							$('#cbo_knitting_company').val(knitting_company);
						}
						...
						show_list_view(theemail+"**"+booking_without_order+"**"+recieve_basis+"**"+cbo_company_id,'show_fabric_desc_listview','list_fabric_desc_container','requires/woven_grey_fabric_receive_controller','');
					}
					//set_auto_complete();
					release_freezing();
				} 
	...
	function set_form_data(data)
	{
		...
		if(recieve_basis==9)
		{
			get_php_form_data(data+"**"+roll_maintained+"**"+knitting_source+"**"+process_costing_maintain, "populate_data_from_production", "requires/woven_grey_fabric_receive_controller" );
		}
		else if(recieve_basis==14)
		{
			var data=data.split("**");
		...
		else
		{
			...
			if(recieve_basis==1)
			{
				...
				{
					var currency_id=2;
					var response=return_global_ajax_value( currency_id+"**"+recv_date, 'check_conversion_rate', '', 'requires/woven_grey_fabric_receive_controller');
	...
	function openmypage_fabricDescription()
	{
		...
		var page_link = 'requires/woven_grey_fabric_receive_controller.php?action=fabricDescription_popup&garments_nature='+garments_nature;
	...
	function fnc_grey_fabric_receive(operation)
	{
		...
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+$('#txt_booking_no').val()+'*'+$('#cbo_receive_basis').val()+'*'+$('#cbo_location').val(), "grey_fabric_receive_print", "requires/woven_grey_fabric_receive_controller" ) 
			return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			if(receive_vasis==1 || receive_vasis==2)
			{
				if( form_validation('cbo_company_id*cbo_receive_basis*txt_receive_date*txt_receive_chal_no*cbo_store_name*cbo_knitting_source*cbo_knitting_company','Company*Receive Basis*Production Date*Challan*Store Name*Knitting Source*Knitting Com')==false )
				{
					return;
				}
			}
			...
			{
				if( form_validation('cbo_company_id*cbo_receive_basis*txt_receive_date*cbo_store_name*cbo_knitting_source*cbo_knitting_company','Company*Receive Basis*Production Date*Challan*Store Name*Knitting Source*Knitting Com')==false )
				{
					return;
				}
			}
			...
			if( form_validation('txt_fabric_description*txt_gsm*txt_width*txt_receive_qnty*cbo_color_range','Fabric Description*Weight*Dia / Width*Grey Receive Qnty*Color Range')==false )
			{
				return;
			}
	...
	function fnc_grey_fabric_receive_Reply_info()
	{
		if(http.readyState == 4) 
		{
			...
			if((reponse[0]==0 || reponse[0]==1))
			{
				...
				$('#txt_booking_no').attr('disabled','disabled');
				show_list_view(reponse[1],'show_grey_prod_listview','list_container_knitting','requires/woven_grey_fabric_receive_controller','');
				
				reset_form('greyreceive_1','roll_details_list_view','','','','update_id*txt_recieved_id*cbo_company_id*cbo_receive_basis*txt_receive_date*txt_receive_chal_no*txt_booking_no*txt_booking_no_id*cbo_buyer_name*cbo_store_name*cbo_knitting_source*cbo_knitting_company*cbo_location*txt_yarn_issue_challan_no*txt_job_no*txt_remarks*roll_maintained*barcode_generation*booking_without_order*process_costing_maintain');
	...
	function grey_receive_popup()
	{
		...
		{ 	
			var page_link='requires/woven_grey_fabric_receive_controller.php?cbo_company_id='+cbo_company_id+'&garments_nature='+garments_nature+'&action=grey_receive_popup_search';
			...
			emailwindow.onclose=function()
			{
				...
				{
					freeze_window(5);
					reset_form('greyreceive_1','list_container_knitting*list_fabric_desc_container','','','','roll_maintained*barcode_generation*process_costing_maintain');
					var process_costing_maintain = $('#process_costing_maintain').val();
					get_php_form_data(grey_recv_id+"**"+process_costing_maintain, "populate_data_from_grey_recv", "requires/woven_grey_fabric_receive_controller" );
					...
					{
						show_list_view(booking_pi_production_no+"**"+booking_without_order+"**"+cbo_receive_basis+"**"+cbo_company_id,'show_fabric_desc_listview','list_fabric_desc_container','requires/woven_grey_fabric_receive_controller','');
					}
					else if(cbo_receive_basis==14)
					{
						show_list_view(booking_pi_production_no+"**"+booking_without_order+"**"+cbo_receive_basis+"**"+cbo_company_id,'show_fabric_desc_listview_sales_order','list_fabric_desc_container','requires/woven_grey_fabric_receive_controller','');
					}
					else if(cbo_receive_basis==1 || cbo_receive_basis==9)
					{
						show_list_view(booking_pi_production_id+"**"+booking_without_order+"**"+cbo_receive_basis+"**"+cbo_company_id,'show_fabric_desc_listview','list_fabric_desc_container','requires/woven_grey_fabric_receive_controller','');
					}
					
					show_list_view(grey_recv_id,'show_grey_prod_listview','list_container_knitting','requires/woven_grey_fabric_receive_controller','');
	...
	function put_data_dtls_part(id,type,page_path)
	{
		...
		if(roll_maintained==1)
		{
			show_list_view("'"+id+"**"+barcode_generation+"**"+booking_without_order+"'",'show_roll_listview','roll_details_list_view','requires/woven_grey_fabric_receive_controller','');
		}
		else
		{
			$('#roll_details_list_view').html('');
	...
	function set_auto_complete()
	{
		...
		if(receive_basis==2 || receive_basis==11)
		{
			...
			var booking_without_order = $('#booking_without_order').val();
			get_php_form_data(booking_id+"**"+booking_without_order+"**"+receive_basis, 'load_color', 'requires/woven_grey_fabric_receive_controller');
		}
		else
		{
			$("#txt_color").autocomplete({
	...
	function openmypage_color()
	{
		...
		var page_link='requires/woven_grey_fabric_receive_controller.php?recieve_basis='+recieve_basis+'&color_id='+color_id+'&booking_id='+booking_id+'&booking_without_order='+booking_without_order+'&program_no='+program_no+'&cbo_body_part='+cbo_body_part+'&fabric_desc_id='+fabric_desc_id+'&txt_gsm='+txt_gsm+'&txt_width='+txt_width+'&action=color_popup';
	...
	function issue_challan_no()
	{
		...
		{ 	
			var page_link='requires/woven_grey_fabric_receive_controller.php?cbo_company_id='+cbo_company_id+'&action=issue_challan_no_popup';
	...
	function fnc_check_issue(issue_num)
	{
		if(issue_num!="")
		{
			var issue_result = trim(return_global_ajax_value(issue_num, 'issue_num_check', '', 'requires/woven_grey_fabric_receive_controller'));
			if(issue_result=="")
			{
	...
	function fnc_send_printer_text()
	{
		...
		data=data+"***"+dtls_id;
		var url=return_ajax_request_value(data, "report_barcode_text_file", "requires/woven_grey_fabric_receive_controller");
	...
	function fnc_barcode_generation()
	{
		...
		data=data+"***"+dtls_id;
		window.open("requires/woven_grey_fabric_receive_controller.php?data=" + data+'&action=report_barcode_generation', true );
	}
	...
	function load_location() 
	{
		...
		{
			load_drop_down( 'requires/woven_grey_fabric_receive_controller',cbo_knitting_company+'_'+cbo_knitting_source, 'load_drop_down_location', 'location_td' );
		}
		else
		{
			load_drop_down( 'requires/woven_grey_fabric_receive_controller',cbo_company_id+'_'+cbo_knitting_source, 'load_drop_down_location', 'location_td' );
		}*/
		load_drop_down( 'requires/woven_grey_fabric_receive_controller',cbo_company_id+'_'+cbo_knitting_source, 'load_drop_down_location', 'location_td' ); // only company wise location, store, floor, room, rack, shelf load confirm by Rasel bhai
	}
	...
	function load_floor()
	{
		...
		if(cbo_knitting_source==1)
		{
			load_drop_down( 'requires/woven_grey_fabric_receive_controller',cbo_knitting_company+'_'+cbo_location, 'load_drop_down_floor', 'floor_td' );
		}
		else
		{
			load_drop_down( 'requires/woven_grey_fabric_receive_controller',cbo_company_id+'_'+cbo_location, 'load_drop_down_floor', 'floor_td' );
		}
	}
	...
	function load_machine()
	{
		...
		if(cbo_knitting_source==1)
		{
			load_drop_down( 'requires/woven_grey_fabric_receive_controller',cbo_knitting_company+'_'+floor_td, 'load_drop_machine', 'machine_td' );
		}
		else
		{
			load_drop_down( 'requires/woven_grey_fabric_receive_controller',cbo_company_id+'_'+floor_td, 'load_drop_machine', 'machine_td' );
		}
	}
	...
	function fnc_generate_bill()
	{
		...
		{
			var bill_response=return_global_ajax_value($('#update_id').val(), 'bill_generate', '', 'requires/woven_grey_fabric_receive_controller');
		...
		<form name="greyreceive_1" id="greyreceive_1">
			<div style="width:950px; float:left;" align="center">        
				<fieldset style="width:950px">
					<legend>Woven Grey Fabric Receive Entry</legend>
					<fieldset style="width:930px">
						<table cellpadding="0" cellspacing="2" width="100%">
							...
							<tr>
								...
								<td width="150">
									<? 
									echo create_drop_down( "cbo_company_id", 151, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3)  $company_cond order by comp.company_name","id,company_name", 1, "--Select Company--", 0, "load_drop_down( 'requires/woven_grey_fabric_receive_controller', document.getElementById('cbo_receive_basis').value+'_'+this.value, 'load_drop_down_buyer', 'buyer_td_id' );load_drop_down( 'requires/woven_grey_fabric_receive_controller', this.value, 'load_drop_down_location', 'location_td' );load_drop_down( 'requires/woven_grey_fabric_receive_controller', this.value, 'load_drop_machine', 'machine_td' );get_php_form_data(this.value,'roll_maintained','requires/woven_grey_fabric_receive_controller' ); load_receive_basis();" );
									//load_drop_down( 'requires/woven_grey_fabric_receive_controller', this.value, 'load_drop_down_floor', 'floor_td' );
									//load_drop_down( 'requires/woven_grey_fabric_receive_controller', this.value+'_'+document.getElementById('garments_nature').value, 'load_drop_down_store', 'store_td' );
									?>
								</td>
							...
							<tr>
								...
								<td>
									<?
									echo create_drop_down("cbo_knitting_source",150,$knitting_source,"", 1, "-- Select --", 0,"load_drop_down( 'requires/woven_grey_fabric_receive_controller', this.value+'_'+document.getElementById('cbo_company_id').value, 'load_drop_down_knitting_com','knitting_com');",0,'1,3');
									?>
								</td>
                ...
                <tr>
                	<td colspan="6" align="center" class="button_container">
                		...
                		<input type="hidden" name="txt_stitch_length" id="txt_stitch_length" />
