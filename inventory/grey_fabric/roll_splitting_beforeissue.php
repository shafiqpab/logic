...
<script>
	...
	function barcode_scan_dtls(is_update,barcode_no)
	{
		...
		var system_no 		= response[1];
		var system_id 		= response[2]; // update_id
		var barcode 		= response[3];
		var split_from_id 	= response[4];
		var roll_wgt 		= response[5];
		var is_sales 		= response[6];
		var qty_in_pcs 		= response[7];
		var productionDtlsId= response[8];
		var company_id 		= response[9];
		...
		if(system_id!="" && barcode!="" && response[0]==1)
		{
			$("#txt_system_no").val(system_no);
			$("#update_id").val(system_id);
			$("#hdn_productionDtlsId").val(productionDtlsId); //this filed for getting dtlsID for entry from 2 for Direct Print sticker

			$("#deleted_all_id").val('');

			var barcodeSels= barcode+"_"+is_sales+"_"+system_id;
			// alert(barcodeSels);
			get_php_form_data( barcodeSels, "load_barcode_mst_form", "requires/roll_splitting_beforeissue_controller" );
			$("#txt_original_wgt").val(roll_wgt);
			$("#hidden_roll_wgt").val(roll_wgt);

			$("#txt_original_pcs").val(qty_in_pcs);
			$("#hidden_original_pcs").val(qty_in_pcs);
			$("#hidden_table_id").val(split_from_id);
			...
			set_button_status(1, permission, 'fnc_roll_split_entry',1);
			show_list_view(split_from_id, 'roll_details_update', 'scanning_tbody','requires/roll_splitting_beforeissue_controller', '' );
		}
		else if(barcode_no!="" || response[0]==2)
		{
	...
	function check_qty_in_pcs(id)
	{
		...
		if($('#pcsQtyCheck').is(':checked'))
		{
			// roll_wgt=Original Wgt/Original Pcs*Input Qty. In Pcs
			var roll_wgt=original_wgt/original_qtyInPcs*splited_qtyInPcs;
			$("#rollWgt_"+id).val(number_format(roll_wgt,2,'.' , ""));;
			$("#rollWgt_"+id).attr('readonly','readonly');
		}
	...
	function fnc_bundle_report(type)
	{
		...
		if (type==8) // Barcode 128 v3 NZ
		{
			var productionDtlsId=$("#hdn_productionDtlsId").val();
			var txt_bar_code_num=$("#txt_bar_code_num").val();
			data=data+"***"+productionDtlsId+"***"+txt_bar_code_num;
			var url=return_ajax_request_value(data, "print_barcode_128_v3_nz", "requires/roll_splitting_beforeissue_controller");
			window.open(url,"##");	return;
		}
	...
	function pdf_print_demo()
	{
		...
		var pdf_path=return_ajax_request_value(data, "auto_create_and_print_pdf_file", "requires/roll_splitting_beforeissue_controller");
		printPDF(pdf_path);
            ...
            <fieldset style="width:810px;">
                     ...
                     <tr>
                        ...
                        <td  align="left">
                         ...
                         <td  align="left">
                                ...
                                <input type="hidden" name="hidden_re_transfer" id="hidden_re_transfer"/>
                ...
                <table cellpadding="0" width="760" cellspacing="0" border="1" id="scanning_tbl_top" class="rpt_table" rules="all">
                    <thead>
                        ...
                        <th width="60"><input type="checkbox" id="pcsQtyCheck" name="pcsQtyCheck"/><br>Qty. In Pcs</th>
