...
*/
/*============== Variable Settings =================
-Variable Settings -> Production -> Variable List -> Auto Fabric Store Update (if "No" need to Grey Fabric Delivery to Store and Knit Grey Fabric Receive, if "Yes" auto receive from Knitting Production Page)
-Variable Settings -> Production -> Variable List -> Fabric in Roll Level (if "No" show Production Basis in Knit Grey Fabric Receive Page)
-Variable Settings -> Inventory-> Variable List -> Auto Transfer Receive (if "No" need to Grey Fabric Transfer Acknowledgement Entry)
-Variable Settings -> Inventory-> Variable List -> Requisition Basis Transfer (if "No" no need to Grey Fabric Requisition For Transfer Entry)
-Variable Settings -> Inventory-> Variable List -> Rack Wise Balance Show (Store, Floor, Room, Rack, Shelf validation)
==============================================*/
session_start();
...
function openmypage_systemId()
{
	...
	var page_link = 'requires/grey_sales_order_to_order_trans_controller.php?cbo_company_id='+cbo_company_id+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&action=orderToorderTransfer_popup';
	...
	emailwindow.onclose=function()
	{
		...
		reset_form('','','update_dtls_id*cbo_item_desc*txt_transfer_qnty*txt_roll*txt_ycount*hid_ycount*txt_ybrand*hid_ybrand*txt_ylot*stitch_length*cbo_store_name*cbo_floor*cbo_room*txt_rack*txt_shelf*cbo_store_name_to*cbo_floor_to*cbo_room_to*txt_rack_to*txt_shelf_to*update_trans_issue_id*update_trans_recv_id*previous_from_prod_id*previous_to_prod_id*txt_stock*hide_trans_qty*hidden_requ_dtls_id','','','');
		
		get_php_form_data(transfer_id, "populate_data_from_transfer_master", "requires/grey_sales_order_to_order_trans_controller" );
		load_drop_down( 'requires/grey_sales_order_to_order_trans_controller', $('#txt_from_order_id').val(), 'load_drop_down_item_desc', 'itemDescTd' );

		if ($("#txt_requisition_id").val()=="") 
		{
			//alert('empty req id');
			show_list_view($('#txt_from_order_id').val(),'show_dtls_list_view','list_fabric_desc_container','requires/grey_sales_order_to_order_trans_controller','');
		}
		else{
			//alert('req id');
			show_list_view($("#txt_requisition_id").val(),'show_dtls_list_requisition_view','list_fabric_desc_container','requires/grey_sales_order_to_order_trans_controller','');
			disable_enable_fields( 'txt_challan_no', 1, '', '' );
		}

		//show_list_view($('#txt_from_order_id').val(),'show_dtls_list_view','list_fabric_desc_container','requires/grey_sales_order_to_order_trans_controller','');


		show_list_view(transfer_id,'show_transfer_listview','div_transfer_item_list','requires/grey_sales_order_to_order_trans_controller','');
...
function openmypage_orderNo(type)
{
	...
	var page_link = 'requires/grey_sales_order_to_order_trans_controller.php?cbo_company_id='+cbo_company_id+'&type='+type+'&cbo_company_id_to='+cbo_company_id_to+'&action=order_popup';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(order_id+"**"+type+"**"+transfer_criteria, "populate_data_from_order", "requires/grey_sales_order_to_order_trans_controller" );
		if(type=='from')
		{
			load_drop_down( 'requires/grey_sales_order_to_order_trans_controller', order_id, 'load_drop_down_item_desc', 'itemDescTd' );
			show_list_view(order_id,'show_dtls_list_view','list_fabric_desc_container','requires/grey_sales_order_to_order_trans_controller','');
		}
	}
}
...
function fnc_yarn_transfer_entry(operation)
{
	if(operation==4)
	{
		...
		print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title, "grey_fabric_order_to_order_transfer_print", "requires/grey_sales_order_to_order_trans_controller" ) 
		return;
	}	
	...
	{		
		...
		var dataString = "txt_system_id*cbo_transfer_criteria*cbo_company_id*cbo_company_id_to*txt_transfer_date*txt_challan_no*txt_requisition_no*txt_requisition_id*txt_from_order_id*txt_to_order_id*cbo_item_category" +
                        "*cbo_item_desc*txt_transfer_qnty*txt_rate*txt_transfer_value*txt_roll*cbo_uom*txt_ycount*hid_ycount*txt_ybrand*hid_ybrand*txt_ylot*stitch_length*cbo_store_name*cbo_floor*cbo_room*txt_rack*txt_shelf*cbo_store_name_to*cbo_floor_to*cbo_room_to*txt_rack_to*txt_shelf_to*txt_form_prog*txt_to_prog*update_id*update_dtls_id*update_trans_issue_id*update_trans_recv_id*previous_from_prod_id*previous_to_prod_id*hidden_requ_dtls_id*hide_trans_qty";
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string(dataString,"../../");
		
		// alert(data);return;

		freeze_window(operation);
		http.open("POST","requires/grey_sales_order_to_order_trans_controller.php",true);
...
{	
	...
	{	  		
		...
		if(reponse[0]==0 || reponse[0]==1)
		{
			...
			reset_form('','','update_dtls_id*cbo_item_desc*txt_transfer_qnty*txt_roll*txt_ycount*hid_ycount*txt_ybrand*hid_ybrand*txt_ylot*stitch_length*cbo_store_name*cbo_floor*cbo_room*txt_rack*txt_shelf*cbo_store_name_to*cbo_floor_to*cbo_room_to*txt_rack_to*txt_shelf_to*update_trans_issue_id*update_trans_recv_id*previous_from_prod_id*previous_to_prod_id*txt_form_prog*txt_to_prog*txt_stock*hide_trans_qty*hidden_requ_dtls_id','','','');
			
			if ($("#txt_requisition_id").val()=="") 
			{
				//alert('empty req id');
				show_list_view(reponse[1],'show_transfer_listview','div_transfer_item_list','requires/grey_sales_order_to_order_trans_controller','');
				show_list_view($('#txt_from_order_id').val(),'show_dtls_list_view','list_fabric_desc_container','requires/grey_sales_order_to_order_trans_controller','');
			}
			else{
				//alert('req id');
			...
			disable_enable_fields( 'cbo_transfer_criteria*cbo_company_id*txt_requisition_no*txt_from_order_no*txt_to_order_no*', 1, '', '' );
			set_button_status(0, permission, 'fnc_yarn_transfer_entry',1,1);
		}	
		release_freezing();
	}
...
function openmypage_orderInfo(type)
{
	...
	var title = 'Order Info';	
	var page_link = 'requires/grey_sales_order_to_order_trans_controller.php?txt_order_no='+txt_order_no+'&txt_order_id='+txt_order_id+'&type='+type+'&action=orderInfo_popup';
...
function populate_stock()
{
	...
	get_php_form_data(txt_order_id+"**"+prod_id+"**"+program_no+"**"+company_id+"**"+store_id, "populate_data_about_order", "requires/grey_sales_order_to_order_trans_controller" );
}
...
function fnc_company_onchang(company_id)
{
	...
	$.ajax({
		url: 'requires/grey_sales_order_to_order_trans_controller.php',
		type: 'POST',
		data: page_link,
		success: function (response)
		{
			...
			if (variable_settings[0]==1) 
			{
				...
				$('#enableDisabled').val(variable_settings[0]);
			}				
			else if (variable_settings[0]==2 || variable_settings[0]==0 || variable_settings[0]=="") 
			{
				...
				var transfer_criteria = $('#cbo_transfer_criteria').val();
				//alert(transfer_criteria+'ok');
				if(transfer_criteria==2) // Store to Store
				{
					$('#cbo_company_id_to').attr('disabled','disabled');	
					$('#txt_to_order_no').attr('disabled','disabled');	
				}
				else if(transfer_criteria==4) // Order to Order
				{
					$('#cbo_company_id_to').attr('disabled','disabled');
...
{
	var enableDisabled = $('#enableDisabled').val();
	//alert(enableDisabled);
	if (enableDisabled!=1) 
	{
...
function openmypage_requisition_no()
{
	...
	var page_link = 'requires/grey_sales_order_to_order_trans_controller.php?cbo_company_id='+cbo_company_id+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&action=itemTransfer_requisition_popup';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(transfer_id, "populate_data_from_transfer_requisition_master", "requires/grey_sales_order_to_order_trans_controller" );
		load_drop_down( 'requires/grey_sales_order_to_order_trans_controller', $('#txt_from_order_id').val(), 'load_drop_down_item_desc', 'itemDescTd' );
		//show_list_view($('#txt_from_order_id').val(),'show_dtls_list_view','list_fabric_desc_container','requires/grey_sales_order_to_order_trans_controller','');
		show_list_view(transfer_id,'show_dtls_list_requisition_view','list_fabric_desc_container','requires/grey_sales_order_to_order_trans_controller','');
		disable_enable_fields( 'cbo_transfer_criteria*cbo_company_id*txt_from_order_no*txt_to_order_no*cbo_company_id_to', 1, '', '' );
		//set_button_status(0, permission, 'fnc_yarn_transfer_entry',1,1);
	}
}
                ...
                <fieldset style="width:750px;">
                    <table width="740" cellspacing="2" cellpadding="2" border="0" id="tbl_master">
                        ...
                        <tr>                        	 
	                        ...
	                        <td>
	                            <input type="hidden" name="txt_requisition_no" id="txt_requisition_no" class="text_boxes" style="width:148px;" readonly placeholder="Double Click To Search" onDblClick="openmypage_requisition_no();"  disabled="disabled"/>
	                        	...
	                        	<input type="hidden" name="enableDisabled" id="enableDisabled">
	                        </td>
                        </tr>
                    </table>
                </fieldset>
                ...
                <table width="750" cellspacing="2" cellpadding="2" border="0" id="tbl_dtls">
                            ...
                            <legend>From Sales Order</legend>
                                <table id="from_order_info" cellpadding="0" cellspacing="1" width="100%">										
                                    ...
                                    <tr>
                                        <td>Style Ref.</td>
                                        <td>
                                            <input type="text" name="txt_from_style_ref" id="txt_from_style_ref" class="text_boxes" style="width:150px;" disabled="disabled" placeholder="Display" />
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>Job No</td>						
                                    ...
                                    <tr>
                                        <td>Gmts Item</td>
                                        <td>
                                            <input type="text" name="txt_from_gmts_item" id="txt_from_gmts_item" class="text_boxes" style="width:150px;" disabled="disabled" placeholder="Display" />
                                        </td>
                                    </tr>
                                </table>
                            </fieldset>
                            ...
                            <legend>To Sales Order</legend>					
                                <table id="to_order_info"  cellpadding="0" cellspacing="1" width="100%" >				
                                    ...
                                    <tr>                                    	
                                        <td>Style Ref.</td>
                                        <td>
                                        	<input type="text" name="txt_to_style_ref" id="txt_to_style_ref" class="text_boxes" style="width:150px;" disabled="disabled" placeholder="Display" />
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>Job No</td>						
                    ...
                    <tr>
                        <td align="center" colspan="3" class="button_container" width="100%">
                            ...
                            <input type="hidden" name="hidden_requ_dtls_id" id="hidden_requ_dtls_id" readonly>
