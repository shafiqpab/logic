<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Roll Wise Grey Fabric Sample Requisition For Transfer Entry
...
<script>
	...
	function openmypage_systemId()
	{
		...
		if (form_validation('cbo_transfer_criteria*cbo_company_id','Requisition  Criteria*Company')==false)
		{
			return;
		}
		
		var title = 'Item Transfer Requisition Info';	
		var page_link = 'requires/roll_wise_grey_fabric_sample_requisition_for_transfer_controller.php?cbo_company_id='+cbo_company_id+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&action=sampleToOrderTransfer_popup';
		...
		emailwindow.onclose=function()
		{
			...
			get_php_form_data(transfer_id, "populate_data_from_transfer_master", "requires/roll_wise_grey_fabric_sample_requisition_for_transfer_controller" );
			show_list_view(transfer_id+"**"+$('#txt_from_order_book_id').val()+"**"+cbo_transfer_criteria,'show_transfer_listview','tbl_details','requires/roll_wise_grey_fabric_sample_requisition_for_transfer_controller','');
		}
	} 
	...
	function from_openmypage(type)
	{
		...
		var page_link = 'requires/roll_wise_grey_fabric_sample_requisition_for_transfer_controller.php?cbo_company_id='+cbo_company_id+'&type='+type+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&action=from_order_popup';
		...
		emailwindow.onclose=function() // return_id is booking or order no
		{
			...
			get_php_form_data(return_booking_order_dtls_id+"**"+type+"**"+cbo_transfer_criteria+"**"+body_part_id, "populate_data_from_sample", "requires/roll_wise_grey_fabric_sample_requisition_for_transfer_controller" ); // return_id
			if(type=='from')
			{
				show_list_view(return_booking_order_id+"**"+cbo_transfer_criteria+"**"+body_part_id,'show_dtls_list_view','tbl_details','requires/roll_wise_grey_fabric_sample_requisition_for_transfer_controller',''); // return_id
			}
		}
	}
	...
	function to_openmypage()
	{
		...
		if (form_validation('cbo_transfer_criteria*cbo_company_id_to*txt_from_order_book_no','Transfer Criteria*To Company*From Reference')==false)
		{
			return;
		}
		...
		var page_link = 'requires/roll_wise_grey_fabric_sample_requisition_for_transfer_controller.php?cbo_company_id='+cbo_company_id_to+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&action=to_order_popup';
		...
		emailwindow.onclose=function()
		{
			...
			get_php_form_data(order_id+"**"+cbo_transfer_criteria+"**"+body_part_id, "populate_data_to_order", "requires/roll_wise_grey_fabric_sample_requisition_for_transfer_controller" );
	...
	function fnc_grey_transfer_entry(operation)
	{
		if(operation==4) // print Operation
		{
			...
			print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+$('#cbo_transfer_criteria').val(), "grey_fabric_order_to_order_transfer_print", "requires/roll_wise_grey_fabric_sample_requisition_for_transfer_controller" ) 
			return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			if( form_validation('cbo_company_id*txt_transfer_date*cbo_company_id_to*txt_from_order_book_id*txt_to_order_book_id','Company*Requisition Date*To Company*Sample Booking No*To Order No')==false )
			{
				return;
			}
			...
			var dataString = "cbo_transfer_criteria*txt_system_id*cbo_company_id*txt_transfer_date*cbo_company_id_to*txt_challan_no*txt_from_order_book_no*txt_from_order_book_id*txt_from_order_book_dtls_id*txt_to_order_book_no*txt_to_order_book_id*txt_to_order_book_dtls_id*cbo_ready_to_approved*update_id*cbo_from_body_part*cbo_to_body_part";
			...
			http.open("POST","requires/roll_wise_grey_fabric_sample_requisition_for_transfer_controller.php",true);
	...
	{	
		...
		{	  		
			...
			if(reponse[0]==0 || reponse[0]==1)
			{
				...
				show_list_view(reponse[1]+"**"+$('#txt_from_order_book_id').val()+"**"+$('#cbo_transfer_criteria').val(),'show_transfer_listview','tbl_details','requires/roll_wise_grey_fabric_sample_requisition_for_transfer_controller','');
	...
	function openmypage_orderInfo(type)
	{
		...
		var page_link = 'requires/roll_wise_grey_fabric_sample_requisition_for_transfer_controller.php?txt_to_order_book_no='+txt_to_order_book_no+'&txt_to_order_book_id='+txt_to_order_book_id+'&type='+type+'&action=orderInfo_popup';
	...
	function check_all(tot_check_box_id)
	{
		...
		else
		{
			$('#scanning_tbl tbody tr').each(function() {
				...
				/*if($('#scanning_tbl tbody tr input:checkbox').is(':disabled'))
				{
					alert('check false and disabled')
					$('#scanning_tbl tbody tr input:checkbox').attr('checked', true);
				}
				else
				{
					alert('check false and not disabled');
					$('#scanning_tbl tbody tr input:checkbox').attr('checked', false);
				}*/
			});
		} 
...
<div align="center" style="width:100%;">
    ...
    <form name="transferEntry_1" id="transferEntry_1" autocomplete="off" >
        ...
        <legend>Roll Wise Grey Fabric Sample Requisition For Transfer</legend>
        <br>
            <fieldset style="width:780px;">
                <table width="780" cellspacing="2" cellpadding="2" border="0" id="tbl_master">
                    <tr>
                        <td colspan="2"></td>
                        <td align="right"><strong>Requisition System ID</strong></td>
                        <td>
                           <input type="text" name="txt_system_id" id="txt_system_id" class="text_boxes" style="width:150px;" placeholder="Double click to search" onDblClick="openmypage_systemId();" readonly />
                        <input type="hidden" name="update_id" id="update_id" /></td>
                        </td>
                        <td colspan="2"></td>
                    </tr>
                    <!--<tr>
                    ...
                    <tr>
                        ...
                        <td align="right" class="must_entry_caption">To Company</td>
                        <td>
                            <? 
                        ...
                        <td>
							...
							<? echo create_drop_down( "cbo_ready_to_approved", 110, $yes_no,"", 1, "-- Select--", 2, "","","" );?>
                        </td>
                    </tr>
                </table>
            </fieldset>
            ...
            <fieldset style="width:1210px;text-align:left">
				<table cellpadding="0" width="1190" cellspacing="0" border="1" id="scanning_tbl_top" class="rpt_table" rules="all">
                    <thead>
                 ...
                 <div style="width:1210px; max-height:250px; overflow-y:scroll" align="left">
                 	<table cellpadding="0" cellspacing="0" width="1190" border="1" id="scanning_tbl" rules="all" class="rpt_table">
                    	<tbody id="tbl_details">
                        </tbody>
                	</table>
                </div>
                <br>
                <table width="1190" cellpadding="0" cellspacing="0" border="1" id="" rules="all">
                    <tr>
