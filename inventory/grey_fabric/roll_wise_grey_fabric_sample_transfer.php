...
<script>
	...
	function openmypage_systemId()
	{
		...
		var page_link = 'requires/roll_wise_grey_fabric_sample_transfer_controller.php?cbo_company_id='+cbo_company_id+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&action=sampleToOrderTransfer_popup';
		...
		emailwindow.onclose=function()
		{
			...
			get_php_form_data(transfer_id, "populate_data_from_transfer_master", "requires/roll_wise_grey_fabric_sample_transfer_controller" );
			show_list_view(transfer_id+"**"+$('#txt_from_order_book_id').val()+"**"+cbo_transfer_criteria,'show_transfer_listview','tbl_details','requires/roll_wise_grey_fabric_sample_transfer_controller','setFilterGrid(\'tbl_details\',-1)');
		}
	} 
	...
	function from_openmypage(type)
	{
		...
		var page_link = 'requires/roll_wise_grey_fabric_sample_transfer_controller.php?cbo_company_id='+cbo_company_id+'&type='+type+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&action=from_order_popup';
		...
		emailwindow.onclose=function() // return_id is booking or order no
		{
			...
			get_php_form_data(return_booking_order_dtls_id+"**"+type+"**"+cbo_transfer_criteria+"**"+body_part_id, "populate_data_from_sample", "requires/roll_wise_grey_fabric_sample_transfer_controller" ); // return_id
			if(type=='from')
			{
				show_list_view(return_booking_order_id+"**"+cbo_transfer_criteria+"**"+body_part_id,'show_dtls_list_view','tbl_details','requires/roll_wise_grey_fabric_sample_transfer_controller','setFilterGrid(\'tbl_details\',-1)'); // return_id
			}
		}
	}
	...
	function to_openmypage()
	{
		...
		var page_link = 'requires/roll_wise_grey_fabric_sample_transfer_controller.php?cbo_company_id='+cbo_company_id_to+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&action=to_order_popup'+"&txt_from_order_book_no='"+txt_from_order_book_no+"'";
		...
		emailwindow.onclose=function()
		{
			...
			get_php_form_data(order_id+"**"+cbo_transfer_criteria+"**"+body_part_id, "populate_data_to_order", "requires/roll_wise_grey_fabric_sample_transfer_controller" );
	...
	function fnc_grey_transfer_entry(operation)
	{
		if(operation==4) // print Operation
		{
			...
			print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+$('#cbo_transfer_criteria').val(), "grey_fabric_order_to_order_transfer_print", "requires/roll_wise_grey_fabric_sample_transfer_controller" ) 
			return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			if( form_validation('cbo_company_id*cbo_company_id_to*txt_transfer_date*txt_from_order_book_id*txt_to_order_book_id*cbo_store_name_to*cbo_from_body_part*cbo_to_body_part','Company*To Company*Transfer Date*Sample Booking No*To Order No*To Store*From Body Part*To Body Part')==false )
			{
				return;
			}
			...
			for (var j=1; j<=row_num; j++)
			{
				...
				if(updateIdDtls!="" && $('#tbl_'+j).is(':not(:checked)'))
				{
					...
					if(txt_deleted_prod_qty=='') txt_deleted_prod_qty=productId+'='+rollWgt+'='+fromProductUp+'='+delete_amount; 
					else txt_deleted_prod_qty =txt_deleted_prod_qty+','+productId+'='+rollWgt+'='+fromProductUp+'='+delete_amount;
			...
			var dataString = "cbo_transfer_criteria*txt_system_id*cbo_company_id*cbo_company_id_to*txt_transfer_date*txt_challan_no*txt_from_order_book_no*txt_from_order_book_id*txt_from_order_book_dtls_id*txt_to_order_book_no*txt_to_order_book_id*txt_to_order_book_dtls_id*update_id*txt_requisition_no*txt_requisition_id*cbo_store_name_to*cbo_floor_to*cbo_room_to*txt_rack_to*txt_shelf_to*cbo_from_body_part*cbo_to_body_part"; 
			...
			http.open("POST","requires/roll_wise_grey_fabric_sample_transfer_controller.php",true);
	...
	{	
		...
		{	  		
			...
			if(reponse[0]==0 || reponse[0]==1)
			{
				...
				show_list_view(reponse[1]+"**"+$('#txt_from_order_book_id').val()+"**"+$('#cbo_transfer_criteria').val(),'show_transfer_listview','tbl_details','requires/roll_wise_grey_fabric_sample_transfer_controller','setFilterGrid(\'tbl_details\',-1)');
	...
	function openmypage_orderInfo(type)
	{
		...
		var page_link = 'requires/roll_wise_grey_fabric_sample_transfer_controller.php?txt_to_order_book_no='+txt_to_order_book_no+'&txt_to_order_book_id='+txt_to_order_book_id+'&type='+type+'&action=orderInfo_popup';
	...
	function company_on_change(company)
	{
		...
		$.ajax({
			url: 'requires/roll_wise_grey_fabric_sample_transfer_controller.php',
			type: 'POST',
			data: page_link,
	...
	function to_company_on_change()
	{
		...
		load_room_rack_self_bin('requires/roll_wise_grey_fabric_sample_transfer_controller*13*cbo_store_name_to', 'store','to_store_td', $("#cbo_company_id_to").val());
	...
	{ 
		...
		var page_link = 'requires/roll_wise_grey_fabric_sample_transfer_controller.php?cbo_company_id='+cbo_company_id+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&action=sampleRequisitionTransfer_popup';
		...
		emailwindow.onclose=function()
		{
			...
			get_php_form_data(transfer_id, "populate_data_from_sample_requisition_master", "requires/roll_wise_grey_fabric_sample_transfer_controller" );
			show_list_view(transfer_id+"**"+$('#txt_from_order_book_id').val()+"**"+cbo_transfer_criteria,'show_sample_requisition_transfer_listview','tbl_details','requires/roll_wise_grey_fabric_sample_transfer_controller','');
		}
	}
...
<div align="center" style="width:100%;">
    ...
    <form name="transferEntry_1" id="transferEntry_1" autocomplete="off" >
        ...
        <legend>Roll Wise Grey Fabric Sample Transfer Entry</legend>
        <br>
