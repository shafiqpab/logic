<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Woven fabric receive return
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Woven Receive Return Info","../../", 1, 1, $unicode,1,1); 
$con = connect();
?>	
...
function open_returnpopup()
{
	...
	var page_link='requires/woven_fab_receive_rtn_controller.php?action=return_number_popup&company='+company; 
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(returnNumber[0], "populate_master_from_data", "requires/woven_fab_receive_rtn_controller"); 
		get_php_form_data(returnNumber[2], "populate_data_from_data", "requires/woven_fab_receive_rtn_controller");  
		//list view call here
		show_list_view(returnNumber[0],'show_dtls_list_view','list_container_yarn','requires/woven_fab_receive_rtn_controller','');
...
function open_mrrpopup()
{
	...
	var company = $("#cbo_company_name").val();	
	var page_link='requires/woven_fab_receive_rtn_controller.php?action=mrr_popup&company='+company; 
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(mrrNumber[0], "populate_data_from_data", "requires/woven_fab_receive_rtn_controller");  
 		$("#tbl_child").find('input,select').val('');
 	}
}
...
function openmypage_pi()
{
	...
	page_link='requires/woven_fab_receive_rtn_controller.php?action=pi_popup&company='+company;
...
function openmypage_rtn_qty()
{
	...
	var page_link = 'requires/woven_fab_receive_rtn_controller.php?cbo_company_name='+cbo_company_name+'&txt_received_id='+txt_received_id+'&txt_prod_id='+txt_prod_id+'&hidden_receive_trans_id='+hidden_receive_trans_id+'&txt_return_qnty='+txt_return_qnty+'&update_id='+update_id+'&action=return_po_popup';
...
function fnc_yarn_receive_return_entry(operation)
{
	if(operation==4)
	{
		 ...
		 print_report( $('#cbo_company_name').val()+'*'+$('#txt_return_no').val()+'*'+report_title, "yarn_receive_return_print", "requires/woven_fab_receive_rtn_controller" ) 
		 return;
	}
	else if(operation==2)
	{
	...
	else
	{
		...
		var dataString = "txt_return_no*cbo_company_name*txt_return_date*txt_received_id*txt_mrr_no*cbo_return_to*txt_pi_no*pi_id*txt_item_description*txt_prod_id*txt_return_qnty*cbo_uom*txt_break_qnty*txt_break_roll*txt_order_id_all*txt_remarks*hidden_receive_trans_id*before_prod_id*update_id*issue_mst_id*prev_return_qnty*before_receive_trans_id*txt_global_stock";
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string(dataString,"../../");
		freeze_window(operation);
		http.open("POST","requires/woven_fab_receive_rtn_controller.php",true);
...
{	
	...
	{	  	
		...
		else if(reponse[0]==0 || reponse[0]==1)
		{
			...
			disable_enable_fields( 'txt_return_no', 0, "", "" ); // disable false
			show_list_view(reponse[2],'show_dtls_list_view','list_container_yarn','requires/woven_fab_receive_rtn_controller','');		
...
<div style="width:100%;" align="left">
    ...
    <form name="woven_fab_receive_rtn_1" id="woven_fab_receive_rtn_1" autocomplete="off" > 
    <div style="width:80%;">       
                                  ...
                                  <table  width="450" cellspacing="2" cellpadding="0" border="0">
                                          <tr>
                                           ...
                                           <tr>
                                               ...
                                               <td width="131"><input class="text_boxes" type="text" name="txt_dia" id="txt_dia" style="width:110px;" placeholder="Display" readonly disabled  /></td>
                                          </tr>
                                          <tr>
                                               ...
                                               <td ><? echo create_drop_down( "cbo_uom", 110, $unit_of_measurement,"", 1, "Display", 0, "",1 ); ?></td>
                                          </tr>
