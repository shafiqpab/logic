...
<script>
	...
	{ 
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/grey_fabric_transfer_roll_wise_controller.php?cbo_company_id='+cbo_company_id+'&action=issue_popup','Transfer Popup', 'width=780px,height=350px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function()
		{
			...
			{
				fnc_reset_form(1);
				get_php_form_data(issue_id, "populate_data_from_data", "requires/grey_fabric_transfer_roll_wise_controller");

				var com_id=$('#cbo_to_company_id').val();
				var store_id=$('#cbo_store_name').val();
				var all_data=com_id + "__" + store_id;
				//alert(all_data);return;
				var floor_result = return_global_ajax_value(all_data, 'floor_list', '', 'requires/grey_fabric_transfer_roll_wise_controller');
				...
				var barcode_nos=return_global_ajax_value( issue_id, 'barcode_nos', '', 'requires/grey_fabric_transfer_roll_wise_controller');
				if(trim(barcode_nos)!="")
				{					
	...
	{ 
		...
		var requisition_basis = $("#txt_requisition_basis").val()*1;
		if (requisition_basis==1) 
		{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/grey_fabric_transfer_roll_wise_controller.php?company_id='+company_id+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&cbo_store_name='+cbo_store_name+'&requisition_id='+requisition_id+'&action=barcode_popup','Barcode Popup', 'width=1480px,height=450px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
	...
	{
		window.open("requires/grey_fabric_transfer_roll_wise_controller.php?data=" + data+'&action='+action, true );
	}
	...
	function fnc_grey_fabric_issue_roll_wise( operation )
	{
		...
		{
			var report_title=$( "div.form_caption" ).html();
			generate_report_file( $('#cbo_company_id').val()+'*'+$('#txt_transfer_no').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+operation+'*'+$('#txt_delv_company_id').val(),'grey_issue_print_2');
			return;
		}
		...
		{
			var report_title=$( "div.form_caption" ).html();
			generate_report_file( $('#cbo_company_id').val()+'*'+$('#txt_transfer_no').val()+'*'+$('#update_id').val()+'*'+report_title,'grey_issue_print_4');
			return;
		}
		...
		if (requisition_basis==1) 
		{
			...
			var total_roll_wgt = $("#total_rollwgt").text()*1;
			var requi_qty = $("#hidd_requi_qty").val()*1;
			var prev_transfer_qnty = $("#txt_prev_transfer_qnty").val()*1;
			// alert(requi_qty+'='+total_roll_wgt);return;
			// alert(requi_qty +'-'+ (prev_transfer_qnty+'+'+total_roll_wgt));return;
			if (requi_qty < (prev_transfer_qnty+total_roll_wgt))
			{
				alert("Over Requisition Quantity is not Allowed.");return;
			}
			/*if (requi_qty==(total_roll_wgt)) // Full
			{
		...
		var j=0; var dataString=''; var blank_store=0;
		var store_update_upto=$('#store_update_upto').val()*1;
		var to_store=$('#cbo_store_name').val()*1;
		$("#scanning_tbl").find('tbody tr').each(function()
		{
			var fromStoreId=$(this).find('input[name="fromStoreId[]"]').val()*1;
			var fromFloor=$(this).find('input[name="floor[]"]').val()*1;
			var fromRoom=$(this).find('input[name="room[]"]').val()*1;
			var fromRack=$(this).find('input[name="rack[]"]').val()*1;
			var fromShelf=$(this).find('input[name="shelf[]"]').val()*1;
			var fromBin=$(this).find('input[name="bin[]"]').val()*1;

			var toFloor=$(this).find('select[name="cboFloorTo[]"]').val()*1;
			var toRoom=$(this).find('select[name="cboRoomTo[]"]').val()*1;
			var toRack=$(this).find('select[name="txtRackTo[]"]').val()*1;
			var toShelf=$(this).find('select[name="txtShelfTo[]"]').val()*1;
			var toBin=$(this).find('select[name="txtBinTo[]"]').val()*1;
			var fromLocation=fromFloor+'*'+fromRoom+'*'+fromRack+'*'+fromShelf+'*'+fromBin;
			var toLocation=toFloor+'*'+toRoom+'*'+toRack+'*'+toShelf+'*'+toBin;
			if (store_update_upto <= 1 && transfer_criteria==2) 
			{
				if(fromStoreId==to_store)
				{
					alert('Same Store Not Allowed');return;
				}
			}
			
			if(store_update_upto > 1 && transfer_criteria==2)
			{
				//alert(fromLocation+'=='+toLocation);
				if (fromLocation==toLocation) 
				{
					alert('Please change at least one location');return;
				}
			}
			...
			var rollNo=$(this).find("td:eq(5)").text();
			var constructCompo=$(this).find("td:eq(7)").text();
			...
			var cboToColor=$(this).find('select[name="cboToColor[]"]').val();
		...
		var data="action=save_update_delete&operation="+operation+'&tot_row='+j+get_submitted_data_string('txt_transfer_no*cbo_transfer_criteria*cbo_company_id*cbo_to_company_id*txt_transfer_date*txt_challan_no*cbo_item_category*cbo_store_name*update_id*txt_deleted_id*txt_deleted_trnsf_dtls_id*txt_deleted_trans_id*txt_deleted_prod_id*txt_deleted_prod_qty*txt_deleted_source_roll_id*txt_deleted_barcode*txt_remarks*txt_requisition_no*txt_requisition_id*txt_delv_company_id',"../../")+dataString;
		
		// alert(data);return;
		freeze_window(operation);		
		http.open("POST","requires/grey_fabric_transfer_roll_wise_controller.php",true);
	...
	function fnc_grey_fabric_issue_roll_wise_Reply_info()
	{
		if(http.readyState == 4) 
		{
			...
			if((response[0]==0 || response[0]==1))
			{
				...
				var barcode_nos=return_global_ajax_value( response[1], 'barcode_nos', '', 'requires/grey_fabric_transfer_roll_wise_controller');
				if(trim(barcode_nos)!="")
				{
					create_row(1,barcode_nos);
				}
	...
	function create_row(is_update,barcode_no)
	{
		...
		var cbo_store_name=$('#cbo_store_name').val();
		var txt_from_detar_id=$('#txt_from_detar_id').val();
		var txt_from_gsm=$('#txt_from_gsm').val();
		...
		if(is_update==0) // Save Event
		{
			...
			var barcode_data=return_global_ajax_value( bar_code+"**"+system_id+"**"+cbo_transfer_criteria+"**"+cbo_store_name, 'populate_barcode_data', '', 'requires/grey_fabric_transfer_roll_wise_controller');
			...
			for(var k=0;k<barcode_data_ref.length;k++)
			{
				...
				var roll_rate=barcode_data_all[54];
				var yarn_rate=barcode_data_all[55];
				var knitting_charge=barcode_data_all[56];
				var program_no=barcode_data_all[57];
				var machine_name=barcode_data_all[58];
				...
				/*if (requi_system_id !="") // no need this validation bcoz already data filter (requi from order, from detar, from gsm) in roll popup
				{
					alert(txt_from_order_id +'!='+ po_breakdown_id +'||'+ txt_from_detar_id +'!='+ febric_description_id +'||'+ txt_from_gsm +'!='+ gsm);
					if( (txt_from_order_id != po_breakdown_id) || (txt_from_detar_id != febric_description_id) || (txt_from_gsm != gsm))
					{
				...
				$("#machineNo_"+row_num).text(machine_name);
				...
				$("#knittingCharge_"+row_num).val(knitting_charge);
				...
				$("#cboToColor_"+row_num).removeAttr('onchange').attr("onchange","copyToColor(this.id,this.value);"); 
				...
				$('#cboToColor_'+row_num).html('<option value="'+0+'">Select</option>');
		...
		else if(is_update==2) // For requisition basis
		{
			...
			var barcode_data=return_global_ajax_value( bar_code+'**'+requi_system_id+"**"+cbo_store_name, 'populate_barcode_data_from_requisition', '', 'requires/grey_fabric_transfer_roll_wise_controller');
			//alert(barcode_data);return;
			var transfer_data_ref=barcode_data.split("####");
			var transfer_found=transfer_data_ref[0];
			var transfer_number=transfer_data_ref[1];
			if (transfer_found==82) 
			{
				alert('Transfer Found, Transfer Number: '+transfer_number); return;
			}

			if (barcode_data=="") 
			{
				alert('This Requisition Already Transfer'); return;
			}
			...
			for(var k=0;k<barcode_data_ref.length;k++)
			{
				...
				var requi_dtls_id=barcode_data_all[42];
				...
				var floor_id=barcode_data_all[54];
				var room_id=barcode_data_all[55];
				...
				var internal_ref=barcode_data_all[56];
				var bin_box_id=barcode_data_all[57];
				var to_job=barcode_data_all[58];

				var multi_floor=barcode_data_all[59];
				var multi_room=barcode_data_all[60];
				var multi_rack=barcode_data_all[61];
				var multi_self=barcode_data_all[62];
				var multi_bin=barcode_data_all[63];

				var roll_rate=barcode_data_all[64];
				var yarn_rate=barcode_data_all[65];
				var knitting_charge=barcode_data_all[66];
				var program_no=barcode_data_all[67];
				var machine_name=barcode_data_all[68];
				

				//$multi_floor."**".$multi_room."**".$multi_rack."**".$multi_self."**".$multi_bin."**".$roll_rate."**".$yarn_rate."**".$kniting_charge
				...
				if (requi_system_id !="")
				{
					$("#txt_from_detar_id").val(febric_description_id);
					$("#txt_from_gsm").val(gsm);
				...
				$("#fromBookingWithoutOrder_"+row_num).val(from_book_without_order);
				$("#requiDtlsId_"+row_num).val(requi_dtls_id);
				...
				$('#decrease_'+row_num).removeAttr("onclick").attr("onclick","fn_deleteRow("+row_num+");");
				$('#txtToOrder_'+row_num).attr("disabled","").removeAttr("onDblClick").attr("onDblClick","opneToOrder("+row_num+");");
		...
		{
			var barcode_data=return_global_ajax_value( system_id, 'populate_barcode_data_update', '', 'requires/grey_fabric_transfer_roll_wise_controller');
			//alert(barcode_data); return;
			...
			for(var k=0; k<barcode_data_ref.length; k++)
			{
				...
				var to_body_part_id=barcode_data_all[65];

				var multi_floor=barcode_data_all[66];
				var multi_room=barcode_data_all[67];
				var multi_rack=barcode_data_all[68];
				var multi_self=barcode_data_all[69];
				var multi_bin=barcode_data_all[70];
				var program_no=barcode_data_all[71];
				var machine_name=barcode_data_all[72];
				var to_color_id=barcode_data_all[73];
				...
				$("#hideToColorForUpdate_"+row_num).val(to_color_id);	
				...
				*/
				...
				/*if (requi_system_id!="") 
				{
					if (barcode_no==barcode_for_issue ) 
					{
						...
						$('#txtToOrder_'+row_num).removeAttr("onDblClick").attr("disabled","");
	...
	{
		var bodypart_result = return_global_ajax_value(order_ids, 'bodypart_list_order_wise', '', 'requires/grey_fabric_transfer_roll_wise_controller');
		var JSONObject = JSON.parse(bodypart_result);

		var toColor_result = return_global_ajax_value(order_ids, 'toColor_list_order_wise', '', 'requires/grey_fabric_transfer_roll_wise_controller');
		var JSONObjectColor = JSON.parse(toColor_result);
		...
		$("#scanning_tbl").find('tbody tr').each(function()
		{
			...
			if( cbo_transfer_criteria == 4)
			{	
				//alert('he');
				$(this).find('select[name="cboToColor[]"]').html('<option value="'+0+'">Select</option>');
		 		for (var key of Object.keys(JSONObjectColor[toOrder_id]).sort())
				{
	 				$(this).find('select[name="cboToColor[]"]').append('<option value="'+key+'">'+JSONObjectColor[toOrder_id][key]+'</option>');
				};
			}
			var to_color_id = $("#hideToColorForUpdate_"+toOrderIdArr[1]*1).val();
			$("#cboToColor_"+toOrderIdArr[1]*1).val(to_color_id);
	...
	{
		var bodypart_result = return_global_ajax_value(order_ids, 'bodypart_list_sample_wise', '', 'requires/grey_fabric_transfer_roll_wise_controller');
		var JSONObject = JSON.parse(bodypart_result);
	...
	$('#txt_requisition_no').live('keydown', function(e) {
		if (e.keyCode === 13) 
		{
			e.preventDefault();
			var requisition_no=$('#txt_requisition_no').val();
			scan_requisiti_on(this.value); 
		}
	});

	function scan_requisiti_on(str)
	{
		...
		var type=1;
		get_php_form_data(str+"**"+type, "populate_requisition_data_from_data", "requires/grey_fabric_transfer_roll_wise_controller");
		...
		var requisition_id=$('#txt_requisition_id').val();
		var barcode_nos=return_global_ajax_value( requisition_id, 'requisition_barcode_nos', '', 'requires/grey_fabric_transfer_roll_wise_controller');
		if(trim(barcode_nos)!="")
		{					
	...
	function opneToOrder(str)
	{
		...
		{
			var item_desc_ids='';var item_gsm=""; var item_dia=""; var requisition_id="";
			...
			var page_link='requires/grey_fabric_transfer_roll_wise_controller.php?cbo_to_company_id='+cbo_to_company_id+'&action=to_order_popup'+'&item_desc_ids='+item_desc_ids+'&item_gsm='+item_gsm+'&item_dia='+item_dia+'&txt_requisition_basis='+txt_requisition_basis+'&requisition_id='+requisition_id;
			...
			emailwindow.onclose=function()
			{
				...
				if(order_id!="")
				{
					var bodypart_result = return_global_ajax_value(order_id, 'bodypart_list', '', 'requires/grey_fabric_transfer_roll_wise_controller');
					var JSONObject = JSON.parse(bodypart_result);
					...
					var toColor_result = return_global_ajax_value(order_id, 'toColor_list', '', 'requires/grey_fabric_transfer_roll_wise_controller');
					var JSONObject = JSON.parse(toColor_result);

					$("#scanning_tbl").find('tbody tr').each(function()
					{
						var toColorId = $(this).find('select[name="cboToColor[]"]').attr("id");
						var toColorIdSlArr = toColorId.split("_");
						// copy only that and below selected data
						if( str >= toColorIdSlArr[1]*1 )
						{
							$(this).find('select[name="cboToColor[]"]').html('<option value="'+0+'">Select</option>');
							for (var key of Object.keys(JSONObject).sort())
							{
								$(this).find('select[name="cboToColor[]"]').append('<option value="'+key+'">'+JSONObject[key]+'</option>');
							};
						}
					});
				}
	...
	function copyToColor(id,value)
	{
		var idArr = id.split("_");
		var sl = idArr[1]*1;
		if($('#toColorIds').is(':checked'))
		{
			$("#scanning_tbl").find('tbody tr').each(function()
			{
				var colorId = $(this).find('select[name="cboToColor[]"]').attr("id");
				var toColorIdslArr = colorId.split("_");
				// copy only that and below selected data
				if( sl >= toColorIdslArr[1]*1 )
				{
					$("#cboToColor_"+toColorIdslArr[1]*1).val(value);
				}
			});
		}		
	}
	...
	{
		return load_room_rack_self_bin('requires/grey_fabric_transfer_roll_wise_controller*13*cboRoomTo_1*1', 'room',roomTdTo_1, document.getElementById('cbo_to_company_id').value,'',document.getElementById('cbo_store_name').value,document.getElementById('cboFloorTo_1').value,'','','','','','50','H');
	}
	...
	function fnc_reset_form(source) // Inserted Data Show and Reset Data
	{
		...
		/*
		|	1 means all reset which is used before system ID generate
		|	2 means details part reset only which is used after save data load
		|	3 means details part reset only which is used when data not saved yet  (as per client)
		*/
		...
		//$('#cbo_store_name').val();
		fn_load_floor($('#cbo_store_name').val());
	}
	...
	function active_inactive(str) // Onchange Transfer Criteria
	{
		$('#cbo_to_company_id').val(0);
		$('#cbo_company_id').val(0);
		load_drop_down( 'requires/grey_fabric_transfer_roll_wise_controller',$('#cbo_transfer_criteria').val()+'_'+str, 'load_drop_store_balnk', 'store_td' );
		if(str==1)
		{
	...
	{
		var html='<tr id="tr__1" align="center" valign="middle"><td width="20" id="sl_1"></td><td width="100" id="fromStore_1"></td><td width="100" id="programNo_1"></td><td width="100" id="machineNo_1"></td><td width="70" id="barcode_1"></td><td width="45" id="roll_1"></td><td style="word-break:break-all;" width="80" id="bodyPart_1"></td><td style="word-break:break-all;" width="150" id="cons_1" title="" ></td><td style="word-break:break-all;" width="150" id="yLot_1" align="left" title=""></td><td style="word-break:break-all;" width="100" id="stitchLength_1" title=""></td><td style="word-break:break-all;" width="40" id="gsm_1"></td><td style="word-break:break-all;" width="40" id="dia_1"></td><td style="word-break:break-all;" width="70" id="color_1"></td><td width="60" id="rollWeight_1" align="right"></td><td style="word-break:break-all;" width="60" id="buyer_1"></td><td style="word-break:break-all;" width="120" id="booking_1"></td><td style="word-break:break-all;" width="80" id="job_1"></td><td style="word-break:break-all;" width="80" id="intRef_1"></td><td style="word-break:break-all;" width="80" id="order_1" align="left"></td><td style="word-break:break-all;" width="100" id="knitCompany_1"></td><td style="word-break:break-all;" width="70" id="basis_1"></td><td width="90" id="toOrder_1" placeholder="Browse For Order"><input type="text"  class="text_boxes" id="txtToOrder_1" name="txtToOrder[]" style="width:60px;" onDblClick="opneToOrder(1)" placeholder="Browse For Order" readonly/></td><td width="90" align="center" id="toBodyPartTd_1"><? echo create_drop_down( "cboToBodyPart_1", 90,$blank_array,"", 1, "--Select--", 0, "copyBodyPart(this.id,this.value)",0,"","","","","","","cboToBodyPart[]","onchange_void" ); ?></td><td width="90" align="center" id="cboToColorTd_1"><? echo create_drop_down( "cboToColor_1", 90,$blank_array,"", 1, "--Select--", 0, "copyToColor(this.id,this.value)",0,"","","","","","","cboToColor[]","onchange_void" ); ?></td><td style="word-break:break-all;" width="80" id="toJob_1"></td><td width="50" align="center" id="floor_td_to" class="floor_td_to"><? echo create_drop_down( "cboFloorTo_1", 50,$blank_array,"", 1, "--Select--", 0, "change_floor(this.value,this.id);",0,"","","","","","","cboFloorTo[]","onchange_void"); ?></td><td width="50" align="center" id="roomTdTo_1"><? echo create_drop_down( "cboRoomTo_1", 50,$blank_array,"", 1, "--Select--", 0, "",0,"","","","","","","cboRoomTo[]","onchange_void" ); ?></td><td width="50" align="center" id="rackTdTo_1"><? echo create_drop_down( "txtRackTo_1", 50,$blank_array,"", 1, "--Select--", 0, "",0,"","","","","","","txtRackTo[]","onchange_void" ); ?></td><td width="50" align="center" id="shelfTdTo_1"><? echo create_drop_down( "txtShelfTo_1", 50,$blank_array,"", 1, "--Select--", 0, "",0,"","","","","","","txtShelfTo[]","onchange_void" ); ?></td><td width="50" align="center" id="binTdTo_1"><? echo create_drop_down( "txtBinTo_1", 50,$blank_array,"", 1, "--Select--", 0, "",0,"","","","","","","txtBinTo[]","onchange_void" ); ?></td><td style="word-break:break-all;" width="115" id="progBookPiNo_1"></td><td id="button_1" align="center"><input type="button" id="decrease_1" name="decrease[]" style="width:30px" class="formbuttonplasminus" value="-" onClick="fn_deleteRow(1);" /><input type="hidden" name="barcodeNo[]" id="barcodeNo_1"/><input type="hidden" name="recvBasis[]" id="recvBasis_1"/><input type="hidden" name="progBookPiId[]" id="progBookPiId_1"/><input type="hidden" name="productId[]" id="productId_1"/><input type="hidden" name="orderId[]" id="orderId_1"/><input type="hidden" name="rollId[]" id="rollId_1"/><input type="hidden" name="rollWgt[]" id="rollWgt_1"/><input type="hidden" name="yarnLot[]" id="yarnLot_1"/><input type="hidden" name="yarnCount[]" id="yarnCount_1"/><input type="hidden" name="colorId[]" id="colorId_1"/><input type="hidden" name="stichLn[]" id="stichLn_1"/><input type="hidden" name="brandId[]" id="brandId_1"/><input type="hidden" name="floor[]" id="floor_1"/><input type="hidden" name="room[]" id="room_1"/><input type="hidden" name="rack[]" id="rack_1"/><input type="hidden" name="shelf[]" id="shelf_1"/><input type="hidden" name="bin[]" id="bin_1"/><input type="hidden" name="fromStoreId[]" id="fromStoreId_1"/><input type="hidden" name="toOrderId[]" id="toOrderId_1"/><input type="hidden" name="dtlsId[]" id="dtlsId_1"/><input type="hidden" name="requiDtlsId[]" id="requiDtlsId_1"/><input type="hidden" name="transId[]" id="transId_1"/><input type="hidden" name="transIdTo[]" id="transIdTo_1"/><input type="hidden" name="rolltableId[]" id="rolltableId_1"/><input type="hidden" name="febDescripId[]" id="febDescripId_1"/><input type="hidden" name="machineNoId[]" id="machineNoId_1"/><input type="hidden" name="prodGsm[]" id="prodGsm_1"/><input type="hidden" name="diaWidth[]" id="diaWidth_1"/><input type="hidden" name="knitDetailsId[]" id="knitDetailsId_1"/><input type="hidden" name="transferEntryForm[]" id="transferEntryForm_1"/><input type="hidden" name="bookWithoutOrder[]" id="bookWithoutOrder_1"/><input type="hidden" name="barcodeIssue[]" id="barcodeIssue_1"/><input type="hidden" name="rollMstId[]" id="rollMstId_1"/><input type="hidden" name="rollAmount[]" id="rollAmount_1"/><input type="hidden" name="fromProductUp[]" id="fromProductUp_1"/><input type="hidden" name="fromBookingWithoutOrder[]" id="fromBookingWithoutOrder_1"/><input type="hidden" name="fromBodyPart[]" id="fromBodyPart_1"/><input type="hidden" name="hideBodyPartForUpdate[]" id="hideBodyPartForUpdate_1"/><input type="hidden" name="hideToColorForUpdate[]" id="hideToColorForUpdate_1"/><input type="hidden" name="rollRate[]" id="rollRate_1"/><input type="hidden" name="yarnRate[]" id="yarnRate_1"/><input type="hidden" name="knittingCharge[]" id="knittingCharge_1"/></td></tr>'; 
		$("#scanning_tbl tbody").html(html);
	}
	...
	function company_on_change(company)
	{
		...
		if($("#cbo_transfer_criteria").val() != 1)
		{
			...
			load_drop_down( 'requires/grey_fabric_transfer_roll_wise_controller',$('#cbo_transfer_criteria').val()+'_'+company, 'load_drop_store_to', 'store_td' );
		}
		else
		{
		...
		$.ajax({
			url: 'requires/grey_fabric_transfer_roll_wise_controller.php',
			type: 'POST',
			data: page_link,
	...
	function to_company_on_change(to_company)
	{
		...
		load_drop_down( 'requires/grey_fabric_transfer_roll_wise_controller',$('#cbo_transfer_criteria').val()+'_'+to_company, 'load_drop_store_to', 'store_td' );
	}
	...
	{ 
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/grey_fabric_transfer_roll_wise_controller.php?cbo_company_id='+cbo_company_id+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&cbo_store_name='+cbo_store_name+'&action=itemTransfer_requisition_popup','Requisition Popup', 'width=780px,height=350px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
			var issue_id=this.contentDoc.getElementById("hidden_system_id").value;	 //challan Id and Number
			...
			if(issue_id!="")
			{
				fnc_reset_form(1);
				var type=0;
				get_php_form_data(issue_id+"**"+type, "populate_requisition_data_from_data", "requires/grey_fabric_transfer_roll_wise_controller");
				...
				var barcode_nos=return_global_ajax_value( issue_id, 'requisition_barcode_nos', '', 'requires/grey_fabric_transfer_roll_wise_controller');
				if(trim(barcode_nos)!="")
				{					
					create_row(2,barcode_nos);
				}
	...
	function fn_load_room(floor_id, sequenceNo)
	{
		...
		//alert(all_data);return;
		var room_result = return_global_ajax_value(all_data, 'room_list', '', 'requires/grey_fabric_transfer_roll_wise_controller');
		var tbl_length=$('#scanning_tbl tbody tr').length;
	...
	function fn_load_rack(room_id, sequenceNo)
	{
		...
		//alert(all_data);return;
		var rack_result = return_global_ajax_value(all_data, 'rack_list', '', 'requires/grey_fabric_transfer_roll_wise_controller');
		var tbl_length=$('#scanning_tbl tbody tr').length;
	...
	function fn_load_shelf(rack_id, sequenceNo)
	{
		...
		//alert(all_data);return;
		var shelf_result = return_global_ajax_value(all_data, 'shelf_list', '', 'requires/grey_fabric_transfer_roll_wise_controller');
		var tbl_length=$('#scanning_tbl tbody tr').length;
	...
	function fn_load_bin(shelf_id, sequenceNo)
	{
		...
		//alert(all_data);return;
		var shelf_result = return_global_ajax_value(all_data, 'bin_list', '', 'requires/grey_fabric_transfer_roll_wise_controller');
		var tbl_length=$('#scanning_tbl tbody tr').length;
		//alert(shelf_result+"="+tbl_length);//return;
		var JSONObject = JSON.parse(shelf_result);
	...
	function reset_room_rack_shelf(id,fieldName)
	{
		...
		{			
			$("#scanning_tbl").find('tbody tr').each(function()
			{
 				$(this).find('select[name="cboFloorTo[]"]').val("0");
				$(this).find('select[name="cboRoomTo[]"]').val("0");
				$(this).find('select[name="txtRackTo[]"]').val("0");
				$(this).find('select[name="txtShelfTo[]"]').val("0");
				$(this).find('select[name="txtBinTo[]"]').val("0");
			});
		}
	...
	function change_floor(value,id)
    {
		...
		load_drop_down( 'requires/grey_fabric_transfer_roll_wise_controller', value+"_"+roomTd, 'load_drop_down_room', roomTd);
    }
    ...
    {     	
		...
		load_drop_down( 'requires/grey_fabric_transfer_roll_wise_controller', value+"_"+rackTd, 'load_drop_down_rack', rackTd);
    }
    ...
    function change_rack(value,id)
    {
		...
		load_drop_down( 'requires/grey_fabric_transfer_roll_wise_controller', value+"_"+shelfTd, 'load_drop_down_shelf', shelfTd);
    }
    ...
    function change_shelf(value,id)
    {
		...
		load_drop_down( 'requires/grey_fabric_transfer_roll_wise_controller', value+"_"+binTd, 'load_drop_down_bin', binTd);
    }

    function othersClick(id)
    {
    	if(id=="floorIds")
    	{
	    	if($('#floorIds').is(':checked'))
	    	{
	    		...
	    		$( "#roomIds" ).prop( "checked", false );
	    		$( "#rackIds" ).prop( "checked", false );
	    		$( "#shelfIds" ).prop( "checked", false );
	    		$( "#binIds" ).prop( "checked", false );
	    	}
	    }
...
<body onLoad="set_hotkey();$('#txt_bar_code_num').focus();">
    <div align="center" style="width:100%;">
        ...
        <form name="rollscanning_1" id="rollscanning_1"  autocomplete="off"  >
            <fieldset style="width:810px;">
                ...
                <table cellpadding="0" cellspacing="2" width="800">
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" name="txt_requisition_no" id="txt_requisition_no" class="text_boxes" style="width:148px;" placeholder="Double Click To Search" onDblClick="openmypage_requisition_no();"  disabled="disabled"/>
                        	...
                        	<input type="hidden" name="txt_from_detar_id" id="txt_from_detar_id">
                        	<input type="hidden" name="txt_from_gsm" id="txt_from_gsm">
			...
			<br>
			<fieldset style="width:2130px;text-align:left;">
				<style>
                    #scanning_tbl tr td
                    {
                ...
                <div id="test_dv"></div>
				<table cellpadding="0" width="2490" cellspacing="0" border="1" id="scanning_tbl_top" class="rpt_table" rules="all">
                    <thead>
                    	<th width="20">SL</th>
                        ...
                        <th width="90">To Color<br><input type="checkbox" checked id="toColorIds" name="toColorIds"/></th>
                        <th width="80">To Job</th>
                    	<th width="50">Floor<br><input type="checkbox" checked id="floorIds" name="floorIds" onclick="othersClick(this.id);" /></th>
						<th width="50">Room<br><input type="checkbox" checked id="roomIds" name="roomIds" onclick="othersClick(this.id);"/></th>
						<th width="50">Rack<br><input type="checkbox" checked id="rackIds" name="rackIds" onclick="othersClick(this.id);"/></th>
						<th width="50">Shelf<br><input type="checkbox" checked id="shelfIds" name="shelfIds" onclick="othersClick(this.id);"/></th>
						<th width="50">Bin/Box<br><input type="checkbox" checked id="binIds" name="binIds" onclick="othersClick(this.id);"/></th>
                        <th width="115">Knit. Delivery Challan</th>
                        <th></th>
                    </thead>
                 </table>
                 <div style="width:2510px; max-height:250px; overflow-y:scroll" align="left">
                 	<table cellpadding="0" cellspacing="0" width="2490" border="1" id="scanning_tbl" rules="all" class="rpt_table">
                    	<tbody>
                        	<tr id="tr__1" align="center" valign="middle">
                                <td width="20" id="sl_1"></td>
                                <td width="100" id="fromStore_1"></td>
                                <td width="100" id="programNo_1"></td>
                                <td width="100" id="machineNo_1"></td>
								...
								<td width="90" align="center" id="toColorTd_1">
								<? echo create_drop_down( "cboToColor_1", 90,$blank_array,"", 1, "--Select--", 0, "copyToColor(this.id,this.value)",0,"","","","","","","cboToColor[]","onchange_void" );
								?>
								</td>
                                ...
                                <td id="button_1" align="center">
                                    ...
                                    <input type="hidden" name="hideToColorForUpdate[]" id="hideToColorForUpdate_1"/>
                ...
                <table width="1950" cellpadding="0" cellspacing="0" border="1" id="" rules="all">
                    <tr>
                        <td align="center" class="button_container">
                            ...
                            <? 
                            	echo load_submit_buttons($permission,"fnc_grey_fabric_issue_roll_wise",0,1,"fnc_reset_form(3);",1); //page_reload()
                            ?>
                             ...
                             <input type="button" name="print_4" id="print_4" class="formbutton" value="Print 4" style="width:100px;" onClick="fnc_grey_fabric_issue_roll_wise(9);" >
                        </td>
                    </tr>  
                </table>
			</fieldset>
