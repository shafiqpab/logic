...
function openmypage_systemId()
{
	...
	var page_link = 'requires/grey_fabric_sample_to_order_transfer_controller.php?cbo_company_id='+cbo_company_id+'&action=orderToorderTransfer_popup';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(transfer_id, "populate_data_from_transfer_master", "requires/grey_fabric_sample_to_order_transfer_controller" );
		load_drop_down( 'requires/grey_fabric_sample_to_order_transfer_controller', $('#txt_sam_book_id').val(), 'load_drop_down_item_desc', 'itemDescTd' );
		show_list_view($('#txt_sam_book_id').val(),'show_dtls_list_view','list_fabric_desc_container','requires/grey_fabric_sample_to_order_transfer_controller','');
		show_list_view(transfer_id,'show_transfer_listview','div_transfer_item_list','requires/grey_fabric_sample_to_order_transfer_controller','');
		set_button_status(0, permission, 'fnc_yarn_transfer_entry',1,1);
		disable_enable_fields("txt_sam_book_no*txt_to_order_no",1);
	}
}
...
function openmypage_orderNo(type)
{
	...
	var page_link = 'requires/grey_fabric_sample_to_order_transfer_controller.php?cbo_company_id='+cbo_company_id+'&type='+type+'&action=order_popup';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(order_id+"**"+type, "populate_data_from_order", "requires/grey_fabric_sample_to_order_transfer_controller" );
	}
}
...
function fn_open_semple()
{
	...
	var page_link = 'requires/grey_fabric_sample_to_order_transfer_controller.php?cbo_company_id='+cbo_company_id+'&action=sample_popup';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(sample_id, "populate_data_from_sample", "requires/grey_fabric_sample_to_order_transfer_controller" );
		load_drop_down( 'requires/grey_fabric_sample_to_order_transfer_controller', sample_id, 'load_drop_down_item_desc', 'itemDescTd' );
		show_list_view(sample_id,'show_dtls_list_view','list_fabric_desc_container','requires/grey_fabric_sample_to_order_transfer_controller','');
	}
}
...
function fnc_yarn_transfer_entry(operation)
{
	if(operation==4)
	{
		...
		print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title, "grey_fabric_order_to_order_transfer_print", "requires/grey_fabric_sample_to_order_transfer_controller" ) 
		return;
	}
	else if(operation==0 || operation==1 || operation==2)
	{
		...
		if( form_validation('cbo_company_id*txt_transfer_date*txt_sam_book_no*txt_to_order_no*cbo_item_desc*txt_transfer_qnty','Company*Transfer Date*From Order No*To Order No*Item Description*Transfered Qnty')==false )
		{
			return;
		}
...
{	
	...
	{	  		
		...
		if(reponse[0]==0 || reponse[0]==1)
		{
			...
			show_list_view(reponse[1],'show_transfer_listview','div_transfer_item_list','requires/grey_fabric_sample_to_order_transfer_controller','');
			set_button_status(0, permission, 'fnc_yarn_transfer_entry',1,1);
			disable_enable_fields("cbo_company_id*txt_sam_book_no*txt_to_order_no",1,"","");
			release_freezing();
		}
...
function openmypage_orderInfo(type)
{
	...
	var title = 'Order Info';	
	var page_link = 'requires/grey_fabric_sample_to_order_transfer_controller.php?txt_order_no='+txt_order_no+'&txt_order_id='+txt_order_id+'&type='+type+'&action=orderInfo_popup';
...
function set_form_data(data)
{
	...
	get_php_form_data(data[0]+'**'+data[10], "populate_stock_data", "requires/grey_fabric_sample_to_order_transfer_controller" );
}
