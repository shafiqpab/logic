<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Grey Fabric Transfer Acknowledgement Entry
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Grey Fabric Transfer Acknowledgement Entry","../../", 1, 1, '','','');
...
<script>
	...
	function fnc_transfer_acknowledgement( operation )
	{
		...
		{
			var data_string="txt_system_id*update_id*cbo_company_id_to*cbo_store_name_to*txt_challan_no*challan_id*cbo_transfer_criteria*txt_transfer_date*cbo_item_category*txt_remarks*txtOrderNo*txtOrderID*txtItemDesc*productID*txtTransQnty*hidden_rcv_qnty*cbo_uom*update_trans_id*update_dtls_id*update_dtls_ac_id*update_prop_id*batch_id*color_id*item_rate*save_string*txt_deleted_trans_ids*from_trans_id";
			var data="action=save_update_delete&operation="+operation+get_submitted_data_string(data_string,"../../");

			freeze_window(operation);
			http.open("POST","requires/grey_fabric_transfer_v2_acknowledgement_controller.php",true);
	...
	function fnc_transfer_acknowledgement_response()
	{
		if(http.readyState == 4)
		{
			...
			if(response[0]==0 || response[0]==1)
			{
				...
				show_list_view(response[1],'show_dtls_list_view_update','list_container','requires/grey_fabric_transfer_v2_acknowledgement_controller','');
				show_list_view(response[2]+"**"+1,'show_dtls_list_view','transfer_item','requires/grey_fabric_transfer_v2_acknowledgement_controller','');
	...
	function openmypage_challan()
	{
		...
		var page_link= 'requires/grey_fabric_transfer_v2_acknowledgement_controller.php?action=itemTransfer_popup&company='+company+'&store='+store;
		...
		emailwindow.onclose=function()
		{
			...
			if(transfer_id!='')
			{
				...
				get_php_form_data(transfer_id, "populate_data_from_transfer_master", "requires/grey_fabric_transfer_v2_acknowledgement_controller" );
				show_list_view(transfer_id+"**"+0,'show_dtls_list_view','transfer_item','requires/grey_fabric_transfer_v2_acknowledgement_controller','');
			}
		}
	}
	...
	function openmypage_systemId()
	{
		...
		var page_link = 'requires/grey_fabric_transfer_v2_acknowledgement_controller.php?cbo_company_id_to='+cbo_company_id_to+'&action=itemAcknowle_popup';
		...
		get_php_form_data(transfer_id[0], "populate_data_from_transfer_master_update", "requires/grey_fabric_transfer_v2_acknowledgement_controller" );
		//show_list_view(transfer_id,'show_dtls_list_view_update','payDtlsTableTbody','requires/grey_fabric_transfer_v2_acknowledgement_controller','');
		show_list_view(transfer_id[0],'show_dtls_list_view_update','list_container','requires/grey_fabric_transfer_v2_acknowledgement_controller','');
		show_list_view(transfer_id[1]+"**"+1,'show_dtls_list_view','transfer_item','requires/grey_fabric_transfer_v2_acknowledgement_controller','');
		set_button_status(0, permission, 'fnc_transfer_acknowledgement',1,1);
	}
}
...
function openmypage_qnty()
{
	...
	var page_link = 'requires/grey_fabric_transfer_v2_acknowledgement_controller.php?cbo_company_id_to='+cbo_company_id_to+'&store='+store+'&save_string='+save_string+'&hidden_rcv_qnty='+hidden_rcv_qnty+'&action=quantity_popup';
...
function fnc_floor_load(datas){
	...
	load_drop_down( 'requires/grey_fabric_transfer_v2_acknowledgement_controller',storeId+'_'+company+'_'+location, 'load_drop_down_floor_to', floor_tds );
}
		...
		<div style="width:900px; float:left; position:relative" align="center">
			<fieldset style="width:900px;">
				<form name="transAcknowledgement" id="transAcknowledgement" autocomplete="off" method="POST"  >
					<legend>Grey Fabric Transfer Acknowledgement Entry</legend>
					<br>
					<table width="880" cellspacing="2" cellpadding="2" border="0" id="tbl_master">
						...
						<tr>
							...
							<td>
								<? echo create_drop_down("cbo_transfer_criteria", 160,$item_transfer_criteria,"", 1,"-- Select --",'0',"",'','1,2,4'); ?>
							</td>
							<td>Item Category</td>
							<td>
					...
					<br>
					<legend>Grey Fabric Transfer Details</legend>
					<br>
