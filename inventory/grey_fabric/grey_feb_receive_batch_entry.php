...
?>
	...
	{
		window.open("requires/grey_feb_receive_batch_entry_controller.php?data=" + data+'&action='+action, true );
	}
	...
	function fnc_grey_receive_roll_wise( operation )
	{
		...
		if(form_validation('txt_delivery_date*txt_issue_challan_no','Delivery Date*Issue Challan No')==false)
		{
			return;
		}
		...
		$("#scanning_tbl").find('tbody tr').each(function()
		{
			...
			j + '=' + knittingSource + '&knittingComp_' + j + '=' + knittingComp+ '&deterId_' + j + '=' + deterId + '&fabricId_' + j + '=' +
			fabricId + '&receiveBasis_' + j + '=' + receiveBasis+ '&job_no_' + j + '=' + job_no+ '&rollwgt_' + j + '=' + rollwgt + '&rolldia_' + j +
			'=' + rolldia + '&bookingNo_' + j + '=' + bookingNo+ '&updateDetailsId_' + j + '=' + updateDetailsId+ '&activeId_' + j + '=' + activeId+ '&barcodeNo_' + j + '=' + barcodeNo+ '&rollNo_' + j + '=' + rollNo+ '&updateRollId_' + j + '=' + updateRollId+ '&bookWithoutOrder_' + j + '=' + bookWithoutOrder+ '&disabledStatus_' + j + '=' + disabled+ '&isSales_' +j + '=' + isSales+ '&hiddenQtyInPcs_' +j + '=' + hiddenQtyInPcs;
		});
		if(j<1)
		{
			alert('No data found');
		...
		var data="action=save_update_delete&operation="+operation+'&tot_row='+j+get_submitted_data_string('txt_delivery_date*txt_challan_no*txt_issue_challan_no*cbo_company_id*cbo_knitting_source*cbo_knitting_company*update_id*txt_system_no*cbo_basis*txt_batch_no',"../../")+dataString;
		freeze_window(5);
		http.open("POST","requires/grey_feb_receive_batch_entry_controller.php",true);
	...
	function fnc_grey_delivery_roll_wise_Reply_info()
	{
		if(http.readyState == 4)
		{
			...
			if((response[0]==0 || response[0]==1))
			{
				...
				$("#btn_print_tg").removeClass('formbutton_disabled');
			    $("#btn_print_tg").addClass('formbutton');

			    var grey_challan_no = $("#txt_issue_challan_no").val();

			    show_list_view(response[1]+"_"+grey_challan_no, 'grey_item_details_update', 'scanning_tbl','requires/grey_feb_receive_batch_entry_controller', '' );
			}
	...
	function grey_receive_popup()
	{
		...
		var page_link='requires/grey_feb_receive_batch_entry_controller.php?cbo_company_id='+cbo_company_id+'&garments_nature='+garments_nature+'&action=challan_popup';
		...
		emailwindow.onclose=function()
		{
			...
			{
				show_list_view(grey_recv_no, 'grey_item_details', 'scanning_tbl','requires/grey_feb_receive_batch_entry_controller', '' );
				get_php_form_data(grey_recv_no, "load_php_form", "requires/grey_feb_receive_batch_entry_controller" );
			}
	...
	{
		var company=1;
		var page_link='requires/grey_feb_receive_batch_entry_controller.php?action=mrr_popup&company='+company;
	...
	function scan_challan_no(str)
	{
		...
		var response=return_global_ajax_value(str, 'check_challan_no', '', 'requires/grey_feb_receive_batch_entry_controller');
		if(response==2)
		{
		...
		{
			show_list_view(str, 'grey_item_details', 'scanning_tbl','requires/grey_feb_receive_batch_entry_controller', '' );
			get_php_form_data(str, "load_php_form", "requires/grey_feb_receive_batch_entry_controller" );
		}
	...
	function open_mrrpopup()
	{
		...
		var page_link='requires/grey_feb_receive_batch_entry_controller.php?cbo_company_id='+cbo_company_id+'&garments_nature='+garments_nature+'&action=update_system_popup';
		var title='Grey Receive Form';
		...
		emailwindow.onclose=function()
		{
			...
			if(trim(grey_recv_id)!="")
			{
				show_list_view(grey_recv_id+"_"+grey_challan_no, 'grey_item_details_update', 'scanning_tbl','requires/grey_feb_receive_batch_entry_controller', '' );
				get_php_form_data(grey_recv_id, "load_php_form_update", "requires/grey_feb_receive_batch_entry_controller" );
				set_button_status(1, permission, 'fnc_grey_receive_roll_wise',1);
				$("#btn_fabric_details").removeClass('formbutton_disabled');
				$("#btn_fabric_details").addClass('formbutton');
				$("#btn_print_tg").removeClass('formbutton_disabled');
	...
	function print_tg_details()
	{
		generate_report_file( $('#cbo_company_id').val()+'*'+$('#txt_system_no').val()+'*'+$('#update_id').val(),'fabric_details_tg_print');
    ...
    <form name="rollscanning_1" id="rollscanning_1"  autocomplete="off"  >
            <fieldset style="width:810px;">
                ...
                <table cellpadding="0" cellspacing="2" width="800">
                    ...
                    <tr>
                        ...
                        <td  align="left">
                        	<input type="text" name="txt_challan_no" id="txt_challan_no" class="text_boxes" style="width:140px;" onDblClick="grey_receive_popup()" placeholder="Scan/Browse/Write" /></td>
                      <td align="left" class="must_entry_caption">Issue Challan </td>
                       <td align="left"><input type="text" name="txt_issue_challan_no" id="txt_issue_challan_no" class="text_boxes" style="width:140px" placeholder="" onDblClick=""    disabled/></td>
                    ...
                    <tr>
                        ...
                        <td width="160"><input type="text" name="txt_delivery_date" id="txt_delivery_date" class="datepicker" style="width:140px;" value="<?php echo date('d-m-Y');?>" /></td>
            ...
            <br/>
              <fieldset style="width:1525px;text-align:left">
				...
				<table cellpadding="0" width="1610" cellspacing="0" border="1" id="scanning_tbl_top" class="rpt_table" rules="all">
                    <thead>
                    	<th width="50">SL</th>
                        ...
                        <th width="90">Kniting Comp.</th>
                        <th width="60">M/C No</th>
                        <th width="90">Program/ Booking /Pi No</th>
                        <th>Opening / Independent</th>
                    </thead>
                 </table>
                 <div style="width:1630px; max-height:250px; overflow-y:scroll" align="left">
                 	<table cellpadding="0" cellspacing="0" width="1610" border="1" id="scanning_tbl" rules="all" class="rpt_table">
                    	<tbody>
                        	<tr id="tr_1" align="center" valign="middle">
                                ...
                                <td width="80" id="intRef_1" style="word-break:break-all;" align="left"></td>
                                <td width="70" id="file_1" style="word-break:break-all;" align="left"></td>
                                <td width="90" id="cons_1" style="word-break:break-all;" align="left"></td>
                                <td width="60" id="mc_1" style="word-break:break-all;" align="left"></td>
                    ...
                    <table width="1550" cellpadding="0" cellspacing="0" border="1" id="" rules="all">
                        <tr>
                            <td align="center" class="button_container">
								...
								<input type="button" name="btn_print_tg" id="btn_print_tg" class="formbutton_disabled" value="Print TG" style=" width:100px" onClick="print_tg_details();" >
                            </td>
                        </tr>
                    </table>
                </div>
