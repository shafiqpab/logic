...
function openmypage_systemId()
{
	...
	var page_link = 'requires/grey_fabric_sample_to_order_roll_transfer_controller.php?cbo_company_id='+cbo_company_id+'&action=sampleToOrderTransfer_popup';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(transfer_id, "populate_data_from_transfer_master", "requires/grey_fabric_sample_to_order_roll_transfer_controller" );
		show_list_view(transfer_id+"**"+$('#txt_sam_book_id').val(),'show_transfer_listview','tbl_details','requires/grey_fabric_sample_to_order_roll_transfer_controller','');
	}
} 
...
function fn_open_semple(type)
{
	...
	var page_link = 'requires/grey_fabric_sample_to_order_roll_transfer_controller.php?cbo_company_id='+cbo_company_id+'&type='+type+'&action=sample_popup';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(sample_id+"**"+type, "populate_data_from_sample", "requires/grey_fabric_sample_to_order_roll_transfer_controller" );
		if(type=='from')
		{
			show_list_view(sample_id,'show_dtls_list_view','tbl_details','requires/grey_fabric_sample_to_order_roll_transfer_controller','');
		}
	}
}
...
function openmypage_orderNo()
{
	...
	var page_link = 'requires/grey_fabric_sample_to_order_roll_transfer_controller.php?cbo_company_id='+cbo_company_id+'&action=order_popup';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(order_id, "populate_data_from_order", "requires/grey_fabric_sample_to_order_roll_transfer_controller" );
...
function fnc_grey_transfer_entry(operation)
{
	if(operation==4) // print Operation
	{
		...
		print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title, "grey_fabric_order_to_order_transfer_print", "requires/grey_fabric_sample_to_order_roll_transfer_controller" ) 
		return;
	}
	else if(operation==0 || operation==1 || operation==2)
	{
		...
		http.open("POST","requires/grey_fabric_sample_to_order_roll_transfer_controller.php",true);
...
{	
	...
	{	  		
		...
		if(reponse[0]==0 || reponse[0]==1)
		{
			...
			show_list_view(reponse[1]+"**"+$('#txt_sam_book_id').val(),'show_transfer_listview','tbl_details','requires/grey_fabric_sample_to_order_roll_transfer_controller','');
...
function openmypage_orderInfo(type)
{
	...
	var title = 'Order Info';	
	var page_link = 'requires/grey_fabric_sample_to_order_roll_transfer_controller.php?txt_order_no='+txt_order_no+'&txt_order_id='+txt_order_id+'&type='+type+'&action=orderInfo_popup';
...
<div align="center" style="width:100%;">
    ...
    <form name="transferEntry_1" id="transferEntry_1" autocomplete="off" >
        ...
        <legend>Roll Wise Grey Fabric Sample To Order Transfer Entry</legend>
        <br>
                        ...
                        <legend>To Order</legend>
                            <table id="from_order_info" cellpadding="0" cellspacing="1" width="100%">										
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" name="txt_order_no" id="txt_order_no" class="text_boxes" style="width:150px;" placeholder="Double click to search" onDblClick="openmypage_orderNo('to');" readonly />
