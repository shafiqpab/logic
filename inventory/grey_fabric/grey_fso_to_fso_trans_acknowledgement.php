...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Grey FSO To FSO Transfer Acknowledgement","../../", 1, 1, '','','');
...
<script>
	...
	function fnc_transfer_acknowledgement( operation )
	{
		...
		if(operation==0 || operation==1)
		{
			...
			http.open("POST","requires/grey_fso_to_fso_trans_acknowledgement_controller.php",true);
	...
	function fnc_transfer_acknowledgement_response()
	{
		if(http.readyState == 4)
		{
			...
			if(response[0]==0 || response[0]==1)
			{
				...
				show_list_view(response[1],'show_dtls_list_view_update','list_container','requires/grey_fso_to_fso_trans_acknowledgement_controller','');
				show_list_view(response[2]+"**"+1,'show_dtls_list_view','transfer_item','requires/grey_fso_to_fso_trans_acknowledgement_controller','');
	...
	function openmypage_challan()
	{
		...
		var page_link= 'requires/grey_fso_to_fso_trans_acknowledgement_controller.php?action=itemTransfer_popup&company='+company+'&store='+store;
		...
		emailwindow.onclose=function()
		{
			...
			if(transfer_id!='')
			{
				...
				get_php_form_data(transfer_id, "populate_data_from_transfer_master", "requires/grey_fso_to_fso_trans_acknowledgement_controller" );
				show_list_view(transfer_id+"**"+0,'show_dtls_list_view','transfer_item','requires/grey_fso_to_fso_trans_acknowledgement_controller','');
			}
		}
	}
	...
	function openmypage_systemId()
	{
		...
		var page_link = 'requires/grey_fso_to_fso_trans_acknowledgement_controller.php?cbo_company_id_to='+cbo_company_id_to+'&action=itemAcknowle_popup';
		...
		get_php_form_data(transfer_id[0], "populate_data_from_transfer_master_update", "requires/grey_fso_to_fso_trans_acknowledgement_controller" );
		//show_list_view(transfer_id,'show_dtls_list_view_update','payDtlsTableTbody','requires/grey_fso_to_fso_trans_acknowledgement_controller','');
		show_list_view(transfer_id[0],'show_dtls_list_view_update','list_container','requires/grey_fso_to_fso_trans_acknowledgement_controller','');
		show_list_view(transfer_id[1]+"**"+1,'show_dtls_list_view','transfer_item','requires/grey_fso_to_fso_trans_acknowledgement_controller','');
		set_button_status(0, permission, 'fnc_transfer_acknowledgement',1,1);
	}
}
...
function openmypage_qnty()
{
	...
	var page_link = 'requires/grey_fso_to_fso_trans_acknowledgement_controller.php?cbo_company_id_to='+cbo_company_id_to+'&store='+store+'&save_string='+save_string+'&hidden_rcv_qnty='+hidden_rcv_qnty+'&action=quantity_popup';
...
function fnc_floor_load(datas){
	...
	load_drop_down( 'requires/grey_fso_to_fso_trans_acknowledgement_controller',storeId+'_'+company+'_'+location, 'load_drop_down_floor_to', floor_tds );
}
		...
		<div style="width:900px; float:left; position:relative" align="center">
			<fieldset style="width:900px;">
				<form name="transAcknowledgement" id="transAcknowledgement" autocomplete="off" method="POST"  >
					<legend>Grey FSO To FSO Transfer Acknowledgement</legend>
					<br>
					...
					<br>
					<legend>Grey FSO To FSO Transfer Acknowledgement Details</legend>
					<br>
