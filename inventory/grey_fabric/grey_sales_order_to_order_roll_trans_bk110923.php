...
<script>
	...
	function fnc_grey_transfer_entry(operation)
	{
		...
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			var dataString = "txt_system_id*cbo_company_id*txt_transfer_date*txt_challan_no*txt_from_order_id*txt_to_order_id*update_id*txt_requisition_no*txt_requisition_id*cbo_complete_status*requisition_and_order_basis*cbo_from_store_name*cbo_store_name*cbo_transfer_criteria*cbo_to_company_id*cbo_purpose_id*hid_to_color_id";
	...
	function openmypage_requisition_no()
	{
		...
		emailwindow.onclose=function()
		{
			...
			else // Data will come from Requisition page
			{
				show_list_view(transfer_id+"**"+$('#txt_from_order_id').val(),'show_transfer_requi_listview','tbl_details','requires/grey_sales_order_to_order_roll_trans_controller','');
			}
	...
	function openmypage_barcode() // Barcode browse
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/grey_sales_order_to_order_roll_trans_controller.php?company_id='+company_id+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&cbo_store_name='+cbo_store_name+'&action=barcode_popup','Barcode Popup', 'width=1240px,height=350px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
	...
	function create_row(is_update,barcode_no)
	{
		...
		if(is_update==0) // save
		{
			...
			var barcode_data=return_global_ajax_value(bar_code, 'show_dtls_list_view', '', 'requires/grey_sales_order_to_order_roll_trans_controller');
			...
			for(var k=0;k<barcode_data_ref.length;k++)
			{
				...
				$("#requiDtlsId_"+row_num).val();// only for update event
		...
		else if(is_update==2) // Requisition Basis
		{
			// var barcode_data=return_global_ajax_value( bar_code, 'populate_barcode_for_requisition', '', 'requires/grey_sales_order_to_order_roll_trans_controller');
