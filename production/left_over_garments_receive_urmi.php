...
<script>
	...
	function fnc_left_over_gmts_input(operation)
	{
		...
		/*else if(operation==4)
		{
			 ...
			 print_report( $('#cbo_company_name').val()+'*'+$('#txt_mst_id').val()+'*'+report_title+'*'+$('#txt_system_no').val(), "left_over_gmts_receive_print", "requires/left_over_garments_receive_urmi_controller" ); 
			 return;
		}*/
		if(operation==0 || operation==1 || operation==2)
		{
	 		if ( form_validation('cbo_company_name*cbo_order_type*cbo_store_name*cbo_goods_type*txt_order_no*txt_total_left_over_receive*cbo_rcv_from','Company Name*Order Type*Goods Type*Order No* Receive Qty*Gmts Receive From')==false )
			{
				return;
			}		
			else
			{			
				...
				var data="action=save_update_delete&operation="+operation+"&colorIDvalue="+colorIDvalue+get_submitted_data_string('cbo_company_name*cbo_location_name*txt_leftover_date*cbo_order_type*cbo_buyer_name*cbo_store_name*exchange_rate*cbo_working_company_name*cbo_working_location_name*txt_remark*cbo_goods_type*txt_order_no*txt_po_id*txt_style_name*cbo_item_name*cbo_country_name*txt_total_left_over_receive*txt_remark2*cbo_currency*txt_fob_rate*txt_leftover_amount*txt_bdt_amount*cbo_room_no*cbo_rack_no*cbo_shelf_no*cbo_bin_no*sewing_production_variable*country_maintain_variable*leftover_source*hidden_po_break_down_id*hidden_colorSizeID*styleOrOrderWisw*iron_production_variable_rej*variable_is_controll*txt_user_lebel*hidden_currency_id*hidden_exchange_rate*hidden_piece_rate*hidden_currency_rate*txt_mst_id*txt_system_no*hidden_dtls_id*hidden_job_no*cbo_working_floor_name*cbo_category_id*cbo_rcv_from',"../");
	 			//alert(data);return;
	 			http.open("POST","requires/left_over_garments_receive_urmi_controller.php",true);
	...
	function fnc_left_over_gmts_input_Reply_info()
	{
	 	if(http.readyState == 4) 
		{
			...
			{ 
				...
				show_list_view(response[1]+'**'+country_maintain+'**'+leftover_source+'**'+goods_type,'show_dtls_listview_system','list_view_container','requires/left_over_garments_receive_urmi_controller','setFilterGrid(\'tbl_list_search\',-1)');	
				show_list_view(response[4]+'**'+company_name+'**'+location_name+'**'+order_type+'**'+buyer_name+'**'+goods_type+'**'+working_company_name+'**'+working_location_name+'**'+country_maintain+'**'+leftover_source,'show_country_listview','list_view_country','requires/left_over_garments_receive_urmi_controller');
				...
				// show_list_view(response[4],'show_dtls_listview','list_view_container','requires/left_over_garments_receive_urmi_controller','setFilterGrid(\'tbl_list_search\',-1)');	
			...
			else if(response[0]==2)
			{
				...
				show_list_view(response[1]+'**'+country_maintain+'**'+leftover_source+'**'+goods_type,'show_dtls_listview_system','list_view_container','requires/left_over_garments_receive_urmi_controller','setFilterGrid(\'tbl_list_search\',-1)');
				show_list_view(response[4]+'**'+company_name+'**'+location_name+'**'+order_type+'**'+buyer_name+'**'+goods_type+'**'+working_company_name+'**'+working_location_name,'show_country_listview','list_view_country','requires/left_over_garments_receive_urmi_controller');
	...
	function openmypage( )
	{
		...
		else
		{
			...
			var page_link = 'requires/left_over_garments_receive_urmi_controller.php?action=order_popup&company='+$('#cbo_company_name').val()+'&location_name='+$('#cbo_location_name').val()+'&order_type='+$('#cbo_order_type').val()+'&goods_type='+$('#cbo_goods_type').val()+'&garments_nature='+$('#garments_nature').val()+'&buyer_name='+$('#cbo_buyer_name').val()+'&store_name='+$('#cbo_store_name').val()+'&working_company_name='+$('#cbo_working_company_name').val()+'&working_location_name='+$('#cbo_working_location_name').val()+'&country_maintain_variable='+$('#country_maintain_variable').val()+'&leftover_source='+$('#leftover_source').val()+'&sewing_production_variable='+$('#sewing_production_variable').val();
			...
			emailwindow.onclose=function()
			{
				...
				console.log(company_id+'='+po_id+'='+location_name+'='+po_number+'='+country_id+'='+gmts_item+'='+currency_id);
				get_php_form_data(company_id,'load_variable_settings','requires/left_over_garments_receive_urmi_controller');			
				// alert(po_id);
				if (po_id!="")
				{				
					...
					var country_maintain 		= $('#country_maintain_variable').val();
					var leftover_source			= $('#leftover_source').val();	
					//load_drop_down( 'requires/left_over_garments_receive_urmi_controller', company_id, 'load_drop_down_store_name', 'store_name_td' ); 
					//load_drop_down( 'requires/left_over_garments_receive_urmi_controller', location_name, 'load_drop_down_floor', 'cbo_floor' );
					...
					if(goods_type!=3)
					{					
						get_php_form_data(po_id+'**'+country_id+'**'+company_id+'**'+gmts_item+'**'+$('#cbo_goods_type').val()+'**'+variableSettings+'**'+$('#cbo_order_type').val()+'**'+location_name+'**'+$('#garments_nature').val()+'**'+country_maintain+'**'+leftover_source, "color_and_size_level_left_over", "requires/left_over_garments_receive_urmi_controller" ); 
						
						show_list_view(po_id+'**'+gmts_item+'**'+country_id+'**'+company_id+'**'+country_maintain+'**'+leftover_source+'**'+goods_type,'show_dtls_listview','list_view_container','requires/left_over_garments_receive_urmi_controller','setFilterGrid(\'tbl_list_search\',-1)');
						show_list_view(po_id+'**'+company_name+'**'+location_name+'**'+order_type+'**'+buyer_name+'**'+goods_type+'**'+working_company_name+'**'+working_location_name+'**'+country_maintain+'**'+leftover_source,'show_country_listview','list_view_country','requires/left_over_garments_receive_urmi_controller');
					...
					{					
						get_php_form_data(po_id+'**'+country_id+'**'+company_id+'**'+gmts_item+'**'+$('#cbo_goods_type').val()+'**'+variableSettings+'**'+$('#cbo_order_type').val()+'**'+location_name+'**'+$('#garments_nature').val()+'**'+country_maintain+'**'+leftover_source, "color_and_size_level_left_over_sample", "requires/left_over_garments_receive_urmi_controller" ); 
	...
	function put_country_data(po_id, item_id, country_id, lft_qnty)
	{
		...
		get_php_form_data(po_id+'**'+item_id+'**'+country_id+'**'+variableSettingsCountry+'**'+leftover_source, "populate_data_from_search_popup", "requires/left_over_garments_receive_urmi_controller" );
		...
		{
			get_php_form_data(po_id+'**'+countryId+'**'+company_id+'**'+item_id+'**'+$('#cbo_goods_type').val()+'**'+variableSettings+'**'+$('#cbo_order_type').val()+'**'+location_name+'**'+$('#garments_nature').val()+'**'+variableSettingsCountry+'**'+leftover_source, "color_and_size_level_left_over", "requires/left_over_garments_receive_urmi_controller" ); 

			// get_php_form_data(po_id+'**'+countryId+'**'+company_id+'**'+item_id+'**'+$('#cbo_goods_type').val()+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+$('#hidden_preceding_process').val(), "color_and_size_level_left_over", "requires/left_over_garments_receive_urmi_controller" );
		}
		else
		{
		...
		show_list_view(po_id+'**'+item_id+'**'+countryId+'**'+company_id+'**'+variableSettingsCountry+'**'+leftover_source+'**'+goods_type,'show_dtls_listview','list_view_container','requires/left_over_garments_receive_urmi_controller','setFilterGrid(\'tbl_list_search\',-1)');
		reset_form('','','txt_total_left_over_receive*txt_remark','','');
		set_button_status(0, permission, 'fnc_left_over_gmts_input',1,0);
		release_freezing();
	}
	...
	function get_left_over_balance(po_id,country_id,gmts_item,company,location,wo_company,wo_location,country_maintain,leftover_source)
	{
		var order_type = $('#cbo_order_type').val();
		var goods_type = $('#cbo_goods_type').val();
		var data = 'po_id='+po_id+'&country_id='+country_id+'&gmts_item='+gmts_item+'&company='+company+'&location='+location+'&wo_company='+wo_company+'&wo_location='+wo_location+'&country_maintain='+country_maintain+'&leftover_source='+leftover_source+'&order_type='+order_type+'&goods_type='+goods_type+'&action=get_left_over_balance';	
		// var data = {"po_id":po_id,"gmts_item":gmts_item,"country_id":country_id,"action":get_left_over_balance};	
		// alert(data);
		$.ajax({
			type:'POST',
			url: 'requires/left_over_garments_receive_urmi_controller.php',
			data: data,
			cache: false,
	...
	function system_number_popup()
	{
		...
		{*/
			...
			var page_link = 'requires/left_over_garments_receive_urmi_controller.php?action=system_number_popup&company='+$('#cbo_company_name').val()+'&location_name='+$('#cbo_location_name').val()+'&leftover_date='+$('#txt_leftover_date').val()+'&order_type='+$('#cbo_order_type').val()+'&goods_type='+$('#cbo_goods_type').val();
			
			var title = 'Left Over Garments Search';
			
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1100px,height=370px,center=1,resize=0,scrolling=1','');
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0];
				...
				get_php_form_data(responseDataArr[0], "populate_mst_form_data", "requires/left_over_garments_receive_urmi_controller" );

				var country_maintain = $("#country_maintain_variable").val();
				var leftover_source = $("#leftover_source").val();
				var goods_type = $("#cbo_goods_type").val();
				
				show_list_view(responseDataArr[0]+'**'+country_maintain+'**'+leftover_source+'**'+goods_type,'show_dtls_listview_system','list_view_container','requires/left_over_garments_receive_urmi_controller','setFilterGrid(\'tbl_list_search\',-1)');
	...
	function check_exchange_rate(curr_id)
	{
		...
		var response=return_global_ajax_value( cbo_currercy+"**"+booking_date, 'check_conversion_rate', '', 'requires/left_over_garments_receive_urmi_controller');
	...
	function populate_selected_data(dtls_id,mst_id,po_id,country_id,item_id,variableSettings)
	{
		...
		//if(variableSettings!=1)
		//{ 
			get_php_form_data(po_id+'**'+country_id+'**'+item_id+'**'+dtls_id+'**'+mst_id+'**'+variableSettings+'**'+$('#cbo_order_type').val()+'**'+$('#country_maintain_variable').val()+'**'+$('#leftover_source').val()+'**'+$('#cbo_goods_type').val(), "color_and_size_level_left_over_update", "requires/left_over_garments_receive_urmi_controller" ); 
			...
			show_list_view(po_id+'**'+company_name+'**'+location_name+'**'+order_type+'**'+buyer_name+'**'+goods_type+'**'+working_company_name+'**'+working_location_name+'**'+country_maintain_variable+'**'+leftover_source,'show_country_listview','list_view_country','requires/left_over_garments_receive_urmi_controller');
	...
	function left_over_gmts_receive_print()
	{
		...
		{
			var report_title=$( "div.form_caption" ).html();
			print_report( $('#cbo_company_name').val()+'*'+$('#txt_mst_id').val()+'*'+report_title+'*'+$('#txt_system_no').val()+'*'+$('#cbo_order_type').val(), "left_over_gmts_receive_print", "requires/left_over_garments_receive_urmi_controller");
		}
  				...
  				<fieldset>
                    <table width="100%" border="0">
                        ...
                        <tr>
                            ...
                            <td width="180">                                
                            <? 
                            echo create_drop_down( "cbo_company_name", 170, "select company_name,id from lib_company comp where is_deleted=0  and status_active=1 $company_cond  order by company_name",'id,company_name', 1, '--- Select Company ---', 0, "load_drop_down( 'requires/left_over_garments_receive_urmi_controller', this.value, 'load_drop_down_location', 'location' );load_drop_down( 'requires/left_over_garments_receive_urmi_controller',this.value, 'load_drop_down_store_name', 'store_name_td');get_php_form_data(this.value,'load_variable_settings','requires/left_over_garments_receive_urmi_controller');" );
                            ?>	
                        ...
                        <tr>
                            ...
                            <td>Remarks</td>
                            <td>
                            	<input name="txt_remark" id="txt_remark" class="text_boxes"  style="width:158px;" >
                            </td>
                        </tr>
                        <tr>
                            
                            <td width="110" id="locations"  class="must_entry_caption">Gmts. Rcv. From</td>
                            <td width="180" id="working_location">
                            <?							
                            echo create_drop_down( "cbo_rcv_from", 170, $gmts_rcv_from_arr,'', 1, '--- Select ---', $selected, "",0,0 );
                            ?>
                            </td>
                        </tr>
                    </table>
                            ...
                            <legend>New Entry</legend>
                                <table  cellpadding="0" cellspacing="2" width="100%">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                        <input type="text" name="txt_total_left_over_receive" id="txt_total_left_over_receive" class="text_boxes_numeric" style="width:120px" disabled/>
