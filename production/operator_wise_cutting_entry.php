<?
/*--- ----------------------------------------- Comments
Purpose			: 	This form will create Operator Wise Cutting Entry
Functionality	:	
JS Functions	:
Created by		:	Kamrul Hasan 
Creation date 	: 	11-01-2023
Updated by 		: 	Shafiq	
Update date		: 	23-02-2023
QC Performed BY	:		
QC Date			:	
Comments		:
*/
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Operator Wise Cutting Entry", "../", 1,1, $unicode,1,'');
?>
<script>
	...
	function fnc_operator_wise_cutting_entry( operation )
	{
		...
		for (var i=1; i<=row_num; i++)
		{
			...
			{
				j++;
				dataString+='&empWiseCuttingData_' + j + '=' + cuttingdata + '&jobId_' + j + '=' + jobid + '&orderId_' + j + '=' + orderid + '&gtmsId_' + j + '=' + gtmsid + '&colorId_' + j + '=' + colorId +'&styleRefNo_' + j + '=' + styleId+'&mstId_' + j + '=' + mstId;
			}
		}
		...
		if(j<1)
		{
			...
			http.open("POST","requires/operator_wise_cutting_entry_controller.php",true);
	...
	function fnc_operator_wise_cutting_entry_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			if(response[0]==0 || response[0]==1)
			{
				...
				show_list_view($('#cbo_company_id').val()+'***'+$('#cbo_location_name').val()+'***'+$('#txt_cuitting_date').val(),'show_list_view','operator_wise_cutting_entry_list','requires/operator_wise_cutting_entry_controller','setFilterGrid("tbl_list_search",-1);','','');
	...
	function cutting_popup(row)
	{
		...
		var page_link = 'requires/operator_wise_cutting_entry_controller.php?action=today_cutting_popup&company=' + company + '&location=' + location_id + '&empWiseCuttingData=' + empWiseCuttingData;
	...
	function fn_get_table(id,floor_id)
	{
		...
		//alert(container);
		load_drop_down( 'requires/operator_wise_cutting_entry_controller', data,'load_dropdown_table', container );
						...
						<fieldset>
							<table cellpadding="0" cellspacing="2" width="100%">
								...
								<tr>
									...
									<td>
										<? 
											echo create_drop_down( "cbo_company_id",150,"select id, company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "--Select Company--", $selected, "load_drop_down( 'requires/operator_wise_cutting_entry_controller', this.value, 'load_drop_down_location', 'location_td'); get_php_form_data(this.value,'load_variable_settings','requires/operator_wise_cutting_entry_controller');","","","","","",2);
										?>
									</td>
