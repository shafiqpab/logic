<?
/*-------------------------------------------- Commentssss
Purpose			: 	This form will create batch creation for grey woven
Functionality	:
JS Functions	:
...
<script>
	...
	function openmypage_fabricBooking()
	{
		...
		else
		{
			...
			var page_link 	= 'requires/batch_creation_woven_controller.php?cbo_company_id='+cbo_company_id+'&batch_against='+cbo_batch_against+'&action=fabricBooking_popup';
			...
			emailwindow.onclose=function()
			{
				...
				console.log(booking_without_order);
				...
				reset_form('','','cboProgramNo_1*cboPoNo_1*poId_1*cboItemDesc_1*cboBodyPart_1*barcodeNo_1*txtBatchQnty_1*txt_total_batch_qnty*hide_job_no*txtQtyPcs_1*txt_total_qtyPcs','',"$('#tbl_item_details tbody tr:not(:first)').remove();",'');
				...
				show_list_view(booking+'**'+booking_without_order+'**'+batch_against+'**'+search_type+'**'+hidden_entry_form+'**'+cbo_company_id,'show_color_listview','list_color','requires/batch_creation_woven_controller','');
			}
		}
	}
	...
	function add_break_down_tr( i )
	{
		...
		else
		{
			...
			$('#cboPoNo_'+i).removeAttr("disabled");
			$('#cboItemDesc_'+i).removeAttr("disabled");
	...
	function calculate_batch_qnty()
	{
		...
		$("#tbl_item_details tbody").find('tr').each(function()
		{
			...
			var txtQtyPcs=$(this).find('input[name="txtQtyPcs[]"]').val();
			if(txtQtyPcs*1 < batchQnty*1 )
			{
				alert('Batch Qnty can not over balance');
				$(this).find('input[name="txtBatchQnty[]"]').val(0);
				return;
			}
	...
	function roll_maintain()
	{
		...
		get_php_form_data($('#cbo_company_id').val(),'roll_maintained','requires/batch_creation_woven_controller' );
	...
	function openmypage_rollnum(row_no)
	{
		...
		var page_link = 'requires/batch_creation_woven_controller.php?cbo_company_id='+cbo_company_id+'&action=roll_popup';
	...
	function openmypage_po(row_no)
	{
		...
		var page_link = 'requires/batch_creation_woven_controller.php?cbo_company_id='+cbo_company_id+'&batch_against='+batch_against+'&hide_job_no='+hide_job_no+'&no_of_row='+no_of_row+'&action=po_popup';
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/batch_creation_woven_controller', po_id+"**"+row_no, 'load_drop_down_program_against_po', 'programNoTd_'+row_no );
			...
			var batch_maintained=$('#batch_maintained').val();
			load_drop_down( 'requires/batch_creation_woven_controller', po_id+"**"+row_no+"**0**"+program_no+"**"+batch_maintained, 'load_drop_down_item_desc', 'itemDescTd_'+row_no );
			...
			if(item_length==2)
			{
				...
				load_drop_down( 'requires/batch_creation_woven_controller', po_id+'**'+row_no+'**0**'+prod_id, 'load_drop_down_body_part', 'bodyPartTd_'+row_no );
	...
	{
		window.open("requires/batch_creation_woven_controller.php?data=" + data+'&action='+action, true );
	}
	...
	function fnc_batch_creation(operation)
	{
		...
		if(operation==4)
		{
			...
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#cbo_working_company_id').val(),'batch_card_print','requires/batch_creation_woven_controller');
			return;
		}
		else if(operation==5)//Print Button 2;
		{
			...
			var report_title=$( "div.form_caption" ).html();
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#txt_booking_no_id').val()+'*'+$('#cbo_working_company_id').val()+'*'+$('#roll_maintained').val(),'batch_card_print_2','requires/batch_creation_woven_controller');
			return;
		}
		else if(operation==6)//Print Button 3;
		{
			...
			var report_title=$( "div.form_caption" ).html();
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#txt_booking_no_id').val()+'*'+$('#cbo_working_company_id').val(),'batch_card_print_3','requires/batch_creation_woven_controller');
			return;
		}
		else if(operation==7)//Print Button 4;
		{
			...
			var report_title=$( "div.form_caption" ).html();
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#txt_booking_no_id').val()+'*'+$('#cbo_working_company_id').val()+'*'+$('#roll_maintained').val(),'batch_card_print_4','requires/batch_creation_woven_controller');
			return;
		}
		else if(operation==8)//Print Button 5;
		{
			...
			var report_title=$( "div.form_caption" ).html();
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#txt_booking_no_id').val()+'*'+$('#cbo_working_company_id').val()+'*'+$('#roll_maintained').val(),'batch_card_print_5','requires/batch_creation_woven_controller');
			return;
		}
		else if(operation==9)//Print Button 6;
		{
			...
			var report_title=$( "div.form_caption" ).html();
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#txt_booking_no_id').val()+'*'+$('#cbo_working_company_id').val()+'*'+$('#roll_maintained').val(),'batch_card_print_6','requires/batch_creation_woven_controller');
			return;
		}
		else if(operation==10)//Print Button 7;
		{
			...
			var report_title=$( "div.form_caption" ).html();
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#txt_booking_no_id').val()+'*'+$('#cbo_working_company_id').val()+'*'+$('#roll_maintained').val(),'batch_card_print_7','requires/batch_creation_woven_controller');
			return;
		}
		else if(operation==11)//Print Button 8;
		{
			...
			var report_title=$( "div.form_caption" ).html();
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#txt_booking_no_id').val()+'*'+$('#cbo_working_company_id').val()+'*'+$('#roll_maintained').val(),'batch_card_print_8','requires/batch_creation_woven_controller');
			return;
		}
		else if(operation==12)//Print Button 9;
		{
			...
			var report_title=$( "div.form_caption" ).html();
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#txt_booking_no_id').val()+'*'+$('#cbo_working_company_id').val()+'*'+$('#roll_maintained').val(),'batch_card_print_9','requires/batch_creation_woven_controller');
			return;
		}
		else if(operation==13)//Print Button 10;
		{
			...
			var report_title=$( "div.form_caption" ).html();
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#txt_booking_no_id').val()+'*'+$('#cbo_working_company_id').val()+'*'+$('#roll_maintained').val(),'batch_card_print_10','requires/batch_creation_woven_controller');
			return;
		}
		else if(operation==14)//Print Button 11;
		{
			...
			var report_title=$( "div.form_caption" ).html();
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#txt_booking_no_id').val()+'*'+$('#cbo_working_company_id').val()+'*'+$('#roll_maintained').val(),'batch_card_print_11','requires/batch_creation_woven_controller');
			return;
		}
		else if(operation==15)//Print Button 12;
		{
			...
			var report_title=$( "div.form_caption" ).html();
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#txt_booking_no_id').val()+'*'+$('#cbo_working_company_id').val()+'*'+$('#roll_maintained').val(),'batch_card_print_12','requires/batch_creation_woven_controller');
			return;
		}
		else if(operation==16)//Prog.Wise;
		{
			...
			var report_title=$( "div.form_caption" ).html();
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#txt_booking_no_id').val()+'*'+$('#cbo_working_company_id').val()+'*'+$('#roll_maintained').val(),'batch_card_prog_wise','requires/batch_creation_woven_controller');
			return;
		}
		else if(operation==17)//Print Button 14;
		{
			...
			var report_title=$( "div.form_caption" ).html();
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#txt_booking_no_id').val()+'*'+$('#cbo_working_company_id').val()+'*'+$('#roll_maintained').val(),'batch_card_print_14','requires/batch_creation_woven_controller');
			return;
		}
		else if(operation==18)// Batch Card 15;
		{
			...
			var report_title=$( "div.form_caption" ).html();
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#txt_booking_no_id').val()+'*'+$('#cbo_working_company_id').val()+'*'+$('#roll_maintained').val(),'batch_card_print_15','requires/batch_creation_woven_controller');
			return;
		}
		else if(operation==19)//Print Button 2;
		{
			...
			var report_title=$( "div.form_caption" ).html();
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#txt_booking_no_id').val()+'*'+$('#cbo_working_company_id').val()+'*'+$('#roll_maintained').val(),'batch_card_print_19','requires/batch_creation_woven_controller');
			return;
		}
		else if(operation==20) // Batch Card 17;
		{
			...
			var report_title=$( "div.form_caption" ).html();
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#txt_booking_no_id').val()+'*'+$('#cbo_working_company_id').val()+'*'+$('#roll_maintained').val(),'batch_card_print_17','requires/batch_creation_woven_controller');
			return;
		}
		else if(operation==21)
		{
			var report_title=$( "div.form_caption" ).html();
			generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#cbo_working_company_id').val(),'batch_card_print_18','requires/batch_creation_woven_controller');
			return;
		}
		...
		$("#tbl_item_details tbody").find('tr').each(function()
		{
			...
			if (form_validation('cboPoNo_'+i+'*cboItemDesc_'+i+'*txtWeight_'+i+'*txtFullWidth_'+i+'*txtBatchQnty_'+i,'Po*Item Description*Weight*FullWidth*Batch Qnty')==false)
			{
				breakOut = false;
				return false;
			}
			...
			j++;
			for(var sl = 1; sl <= row_num ; sl++)
			{
				if(sl != j)
				{
					if($('#cboPoNo_'+i).val() == $('#cboPoNo_'+sl).val() )
					{
						is_duplicate = 1;
						break;
						return;
			...
			data_all+="&"+po_field + j + "='" + $('#'+po_field+i).val()+"'"+"&cboItemDesc_" + j + "='" + $('#cboItemDesc_'+i).val()+"'"+"&cboBodyPart_" + j + "='" + $('#cboBodyPart_'+i).val()+"'"+"&txtFullWidth_" + j + "='" + $('#txtFullWidth_'+i).val()+"'"+"&hideRollNo_" + j + "='" + $('#hideRollNo_'+i).val()+"'"+"&txtBatchQnty_" + j + "='" + $('#txtBatchQnty_'+i).val()+"'"+"&updateIdDtls_" + j + "='" + $('#updateIdDtls_'+i).val()+"'"+"&cboProgramNo_" + j + "='" + $('#cboProgramNo_'+i).val()+"'"+"&txtPoBatchNo_" + j + "='" + $('#txtPoBatchNo_'+i).val()+"'"+"&txtCutableWidth_" + j + "='" + $('#txtCutableWidth_'+i).val()+"'"+"&barcodeNo_" + j + "='" + $('#barcodeNo_'+i).val()+"'"+"&txtQtyPcs_" + j + "='" + $('#txtQtyPcs_'+i).val()+"'"+"&txtWeight_" + j + "='" + $('#txtWeight_'+i).val()+"'"+"&cboColorType_" + j + "='" + $('#cboColorType_'+i).val()+"'";

			all_po_ids += $("#"+po_field + i).val() + ",";
		});

		if(is_duplicate == 1 ) {
			alert('Duplication entry found');
			return;
		}
		...
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_batch_sl_no*cbo_batch_against*cbo_batch_for*cbo_company_id*batch_no_creation*batch_maintained*txt_batch_number*txt_batch_date*txt_batch_weight*txt_tot_trims_weight*txt_booking_no_id*txt_booking_no*txt_ext_no*txt_batch_color*cbo_color_range*txt_organic*txt_process_id*txt_du_req_hr*txt_du_req_min*update_id*booking_without_order*hide_update_id*hide_batch_against*roll_maintained*txt_remarks*cbo_ready_to_approved*cbo_machine_name*save_data*txt_sales_booking_no*txt_search_type*txt_sales_id*cbo_working_company_id*txt_process_seq*cbo_floor*unloaded_batch*ext_from*hidden_booking_entry_form*txt_service_booking*service_booking_id*txt_batch_color_id*cbo_double_dyeing*cbo_batch_type*style_breakdown_str*cbo_template_name',"../")+data_all+'&total_row='+row_num+'&txt_deleted_id='+txt_deleted_id+'&all_po_ids='+all_po_ids;
		//console.log(data);return;
		//alert(data);return;
		freeze_window(operation);

		http.open("POST","requires/batch_creation_woven_controller.php",true);
	...
	function fnc_batch_creation_Reply_info()
	{
		if(http.readyState == 4)
		{
			...
			else if((reponse[0]==0 || reponse[0]==1))
			{
				...
				show_list_view(batch_against+'**'+batch_for+'**'+reponse[1]+'**'+roll_maintained+'**'+batch_maintained,'batch_details','batch_details_container','requires/batch_creation_woven_controller','');
	...
	function openmypage_batchNo()
	{
		...
		else
		{
			...
			var page_link = 'requires/batch_creation_woven_controller.php?cbo_company_id='+cbo_company_id+'&batch_against='+batch_against+'&action=batch_popup';
			...
			emailwindow.onclose=function()
			{
				...
				{
					freeze_window(5);
					get_php_form_data(batch_against+'**'+batch_for+'**'+batch_id+'**'+batch_no+'**'+cbo_company_id+'**'+unloaded_batch+'**'+ext_from, "populate_data_from_search_popup", "requires/batch_creation_woven_controller" );
					if(load_unload == 1)
					{
						$('#txt_booking_no').attr('disabled','disabled');
					}
					show_list_view(batch_against+'**'+batch_for+'**'+batch_id+'**'+roll_maintained+'**'+batch_maintained,'batch_details','batch_details_container','requires/batch_creation_woven_controller','');
					release_freezing();
					if(roll_maintained==1)
					{
	...
	function openmypage_process()
	{
		...
		var page_link = 'requires/batch_creation_woven_controller.php?txt_process_id='+txt_process_id+'&process_seq='+txt_process_seq+'&action=process_name_popup';
	...
	function load_item_desc(value,id)
	{
		...
		try
		{
			...
			var batch_maintained=$('#batch_maintained').val();
			var fabric_source=$('#fabric_source').val();
			load_drop_down( 'requires/batch_creation_woven_controller', $('#cboPoNo_'+row_no).val()+'**'+4+'**'+booking_no, 'load_drop_down_item_desc', "itemDescTd_"+row_num );
		}
		catch(err)
		{
	...
	function load_po(value,id,po_ids)
	{
		...
		try
		{
			...
			load_drop_down( 'requires/batch_creation_woven_controller',po_ids+'**'+value+"**"+booking_no+"**"+row_no, 'load_drop_down_po', 'poNoTd_'+row_no);
			console.log(booking_no);
			//$('#cboPoNo_'+row_no).val($('#poNoTd_'+row_no+' option:last').val());
		}
		catch(err)
		{
			console.log(err.message);
	...
	function load_body_part(value,id)
	{
		...
		{
			var booking_no=$('#txt_booking_no').val();
			get_php_form_data(booking_no+'**'+value+'**'+row_no, 'get_data_from_po', 'requires/batch_creation_woven_controller');
		}
		catch(err)
		{
			console.log(err.message);
	...
	function put_country_data(color_id,color,color_range_id,data)
	{
		var data = data.split('**');
		var data_source = data[0];
		var all_po_ids = data[1];
		var grey_qnty = data[2];
		var balance = data[3];
		...
		{
			var batch_num=return_global_ajax_value( company_id+'**'+update_id, 'dyeing_check_batch', '', 'requires/batch_creation_woven_controller');
		...
		load_drop_down( 'requires/batch_creation_woven_controller', all_po_ids+'**'+data_source+'**'+booking_no+'**1', 'load_drop_down_item_desc', "itemDescTd_1" );
	}
	...
	$('#txt_bar_code_num').live('keydown', function(e) {
		...
		/*	var is_barcode_scanned=return_global_ajax_value(bar_code, 'check_if_barcode_scanned', '', 'requires/batch_creation_woven_controller');
	...
	{
		var barcode_data=return_global_ajax_value(barcode_nos+"**"+batch_against+"**"+booking_without_order+"**"+booking_no+"**"+search_type+"**"+sales_id+"**"+booking_id, 'populate_barcode_data', '', 'requires/batch_creation_woven_controller');
	...
	function openmypage_trims()
	{
		...
		var page_link='requires/batch_creation_woven_controller.php?save_data='+save_data+'&action=trims_weight_popup';
	...
	{
		var report_ids='';
		var report_ids = return_global_ajax_value( data, 'load_report_format', '', 'requires/batch_creation_woven_controller');
	...
	function openmypage_serviceBooking(type) //Service Booking Knitting
	{
		...
		var page_link = 'requires/batch_creation_woven_controller.php?cbo_company_id='+cbo_company_id+'&cbo_knitting_source='+cbo_knitting_source+'&cbo_knitting_company='+cbo_knitting_company+'&action=serviceBooking_popup';
	...
	function fnc_style_wise_popup()
	{
		...
		if(sales_booking_no[1] != 'SMN' && txt_search_type == 7 && roll_maintained==1 && style_wise_popup==1)
		{
			var action = "style_wise_popup";
			var popup_width='1250x';

			var page_link = 'requires/batch_creation_woven_controller.php?cbo_company_id='+cbo_company_id+'&hdn_fso_id='+hdn_fso_id+'&txt_color_id='+txt_color_id+'&update_id='+update_id+'&style_breakdown_str='+style_breakdown_str+'&action='+action;
			...
			<fieldset style="width:1070px;">
				...
				<form name="batchcreation_1" id="batchcreation_1">
					<fieldset style="width:1000px;">
						<table width="1069" align="center" border="0">
							...
							<tr>
								...
								<td>
									<?
									echo create_drop_down( "cbo_company_id", 144, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name",'id,company_name', 1, '--Select Company--', 0,"get_php_form_data(this.value,'batch_no_creation','requires/batch_creation_woven_controller' );roll_maintain();get_php_form_data(this.value,'load_fabric_source_from_variable_settings','requires/batch_creation_woven_controller');get_php_form_data(this.value,'load_variable_qnty_popup','requires/batch_creation_woven_controller' );fnc_load_report_format(this.value);",'','','','','',3);
									?>
								</td>
								...
								<td>
									<?
									echo create_drop_down( "cbo_working_company_id", 144, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name",'id,company_name', 1, '--Select Working--', 0,"load_drop_down('requires/batch_creation_woven_controller',this.value, 'load_drop_down_floor', 'td_floor' );",'','','','','',3);
									?>
								</td>
							...
							<tr>
								...
								<td>
									<?
									echo create_drop_down( "cbo_template_name", 144, "select id,template_name from lib_template_process_mst  where status_active=1 and is_deleted=0  order by template_name","id,template_name",1, "-- Select --", 0, "get_php_form_data(this.value,'load_process','requires/batch_creation_woven_controller');" );
									?>
								</td>
							</tr>
							<tr>
                    ...
                    <fieldset style="width:1141px; margin-top:10px">
                        ...
                        <table cellpadding="0" cellspacing="0" width="1240" class="rpt_table" border="1" rules="all" id="tbl_item_details">
                            <thead>
                                <th>SL</th>
                                <th>Program No</th>
                                <th class="must_entry_caption">PO No./FSO No</th>
                                <th class="must_entry_caption">Item Description</th>
                                <th class="must_entry_caption">Weight</th>
                            ...
                            <tbody id="batch_details_container">
                                <tr class="general" id="tr_1">
                                    ...
                                    <td>
                                       <input type="text" name="txtWeight[]" id="txtWeight_1" class="text_boxes_numeric" placeholder="" style="width:60px" />
                                    </td>
                                    <td>
                                      <input type="text" name="txtFullWidth[]" id="txtFullWidth_1" class="text_boxes_numeric" placeholder="" style="width:60px" />
                                    </td>

                                    <td>
                                    	<input type="text" name="txtCutableWidth[]" id="txtCutableWidth_1" class="text_boxes_numeric" placeholder="" style="width:60px" />
                                        <input type="hidden" name="hideRollNo[]" id="hideRollNo_1" class="text_boxes"  />
                                        <input type="hidden" name="poId[]" id="poId_1" class="text_boxes"  />
                                        <input type="hidden" name="hiddenRollqty[]" id="hiddenRollqty_1" class="text_boxes"  />
                                        <input type="hidden" name="updateIdDtls[]" id="updateIdDtls_1" class="text_boxes"  />
                                        <input type="hidden" name="hiddenQtyPcs[]" id="hiddenQtyPcs_1" class="text_boxes"  />
                                        <input type="hidden" name="barcodeNo[]" id="barcodeNo_1" class="text_boxes_numeric"  />
                                        <input type="hidden" name="cboBodyPart[]" id="cboBodyPart_1" class="text_boxes_numeric"  />
                                        <!--<input type="hidden" name="barcodeNo_1" id="barcodeNo_1"/>-->
                                    </td>
                                    
                                    <td align="left">
                                        <input type="text" name="txtBatchQnty[]" id="txtBatchQnty_1" class="text_boxes_numeric" onKeyUp="calculate_batch_qnty();" style="width:60px;justify-content: left;text-align: left;" />
                                    </td>
                                    <td align="left">
                                        <input type="text" name="txtQtyPcs[]" id="txtQtyPcs_1" class="text_boxes_numeric" onKeyUp="calculate_qtyPcs();" style="width:60px;justify-content: left;text-align: left;" disabled />
                                    </td>
                            ...
                            <tfoot class="tbl_bottom">
                                ...
                                <td>&nbsp;</td>
                                <td>Sum</td>
                                <td align="left"><input type="text" name="txt_total_batch_qnty" id="txt_total_batch_qnty" class="text_boxes_numeric" style="width:60px;justify-content: left;text-align: left;" readonly /></td>
                                <td align="left"><input type="text" name="txt_total_qtyPcs" id="txt_total_qtyPcs" class="text_boxes_numeric" style="width:60px;justify-content: left;text-align: left;" readonly /></td>
								<td>&nbsp;</td>
                                <td>&nbsp;</td>
                            </tfoot>
                        </table>
                    ...
                    <table width="1140">
                        <tr>
                            <td colspan="5" align="center" class="button_container">
                                ...
                                <input type="hidden" name="txt_deleted_id" id="txt_deleted_id">
                            </td>
                        </tr>
                    </table>
                </form>
...
load_drop_down('requires/batch_creation_woven_controller',document.getElementById('cbo_floor').value, 'load_drop_machine', 'td_dyeing_machine');
</script>
</html>