...
if($action=="load_drop_working_company")
{
	...
	if($data==3)
	{
 	...
 	else if($data==1)
 	{
 		...
 		echo create_drop_down( "cbo_working_company", 210, $blank_array,"", 1, "--- Select Working Company ---", $selected, "load_drop_down( 'requires/buyer_inspection_controller_v2', 0, 'load_drop_down_working_location', 'working_location_td' );",0,0 );
 	exit();
}

if ($action=="load_drop_down_working_location")
{
	echo create_drop_down( "cbo_working_location", 210, "select id,location_name from lib_location where company_id='$data' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Working Location --", $selected, "load_drop_down( 'requires/buyer_inspection_controller_v2', $data+'**'+this.value, 'load_drop_down_working_floor', 'working_floor_td' );" );
	exit();
}
...
{ 
	...
	echo create_drop_down( "cbo_order_id",100, "select id,po_number from wo_po_break_down where job_no_mst='$data' and status_active in(1,2,3) and is_deleted=0","id,po_number", 1, "--Select--", "", "load_drop_down( 'requires/buyer_inspection_controller_v2', '__'+this.value, 'load_drop_down_week_no', 'week_drop_down_td' );load_drop_down( 'requires/buyer_inspection_controller_v2', '__'+this.value, 'load_drop_down_country_id', 'country_drop_down_td' );get_php_form_data( this.value+','+document.getElementById('cbo_week_no').value+','+document.getElementById('cbo_country_id').value+','+document.getElementById('cbo_inspection_level').value, 'set_po_qnty_ship_date', 'requires/buyer_inspection_controller_v2');","","","","","","" );
}
...
{ 
	list($job,$po_id,$rowId)=explode('__',$data);
	if($po_id)$con="b.po_break_down_id=$po_id"; else $con="b.job_no_mst='$job'";
	
	echo create_drop_down( "cboweekno_".$rowId,40, "SELECT a.week from week_of_year a,wo_po_color_size_breakdown b where $con and a.week_date =b.country_ship_date  and b.status_active=1 and b.is_deleted=0 group by a.week","week,week", 1, "--Select--", "", "load_drop_down( 'requires/buyer_inspection_controller_v2', '__'+document.getElementById('cboorderid_".$rowId."').value+'__'+this.value+'__'+".$rowId.", 'load_drop_down_country_id', 'country_drop_down_td_".$rowId."' );get_php_form_data( document.getElementById('cboorderid_".$rowId."').value+','+this.value+','+document.getElementById('cbocountryid_".$rowId."').value+','+document.getElementById('cbo_inspection_level').value+','+document.getElementById('cbo_company_name').value+','+".$rowId.",'set_po_qnty_ship_date', 'requires/buyer_inspection_controller_v2');","","","","","","" );
}

if ($action=="load_drop_down_country_id")
{ 
	list($job,$po_id,$week,$rowId)=explode('__',$data);
	if($po_id)$con="b.po_break_down_id=$po_id"; else $con="b.job_no_mst='$job'";
	
	if($week)$con_week=" and c.week=$week"; else $con_week="";
	
	//echo create_drop_down( "cbo_country_id",100, "select a.id,a.country_name from lib_country a,wo_po_color_size_breakdown b where $con and a.id=b.country_id group by a.id,a.country_name order by a.country_name","id,country_name", 1, "--Select--", "", "get_php_form_data( document.getElementById('cbo_order_id').value+','+document.getElementById('cbo_week_no').value+','+this.value, 'set_po_qnty_ship_date', 'requires/buyer_inspection_controller_v2')","","","","","","" );
	echo create_drop_down( "cbocountryid_$rowId",100, "SELECT a.id,a.country_name from lib_country a inner join wo_po_color_size_breakdown b on a.id=b.country_id left join week_of_year c on c.week_date=b.country_ship_date $con_week  where $con  and b.status_active=1 and b.is_deleted=0 group by a.id,a.country_name order by a.country_name","id,country_name", 1, "--Select--", "", "get_php_form_data( document.getElementById('cboorderid_".$rowId."').value+','+document.getElementById('cboweekno_".$rowId."').value+','+this.value+','+document.getElementById('cbo_inspection_level').value+','+document.getElementById('cbo_company_name').value+','+".$rowId.", 'set_po_qnty_ship_date', 'requires/buyer_inspection_controller_v2')","","","","","","" );
}
...
if ($action == "load_all_drop_down") 
{
	...
	list($po_id,$wo_company,$wo_location,$source, $inspection_type,$buyer_id,$company_id,$rid) = explode("_",$data);
	
	if($po_id)$con="b.po_break_down_id=$po_id";
	
	echo create_drop_down( "cboweekno_".$rid,100, "SELECT a.week from week_of_year a,wo_po_color_size_breakdown b where $con and a.week_date =b.country_ship_date  and b.status_active=1 and b.is_deleted=0 group by a.week","week,week", 1, "--Select--", "", "load_drop_down( 'requires/buyer_inspection_controller_v2', '__'+document.getElementById('cboorderid_".$rid."').value+'__'+this.value+'__'+".$rid.", 'load_drop_down_country_id', 'country_drop_down_td_".$rid."' );get_php_form_data( document.getElementById('cboorderid_".$rid."').value+','+this.value+','+document.getElementById('cbocountryid_".$rid."').value+','+document.getElementById('cbo_inspection_level').value+','+document.getElementById('cbo_company_name').value+','+".$rid.",'set_po_qnty_ship_date', 'requires/buyer_inspection_controller_v2');","","","","","","" );

	echo "****";
	
	if($po_id)$con="b.po_break_down_id=$po_id";
	echo create_drop_down( "cbocountryid_".$rid,100, "SELECT a.id,a.country_name from lib_country a inner join wo_po_color_size_breakdown b on a.id=b.country_id left join week_of_year c on c.week_date=b.country_ship_date  where $con  and b.status_active=1 and b.is_deleted=0 group by a.id,a.country_name order by a.country_name","id,country_name", 1, "--Select--", "", "get_php_form_data( document.getElementById('cboorderid_".$rid."').value+','+document.getElementById('cboweekno_".$rid."').value+','+this.value+','+document.getElementById('cbo_inspection_level').value+','+document.getElementById('cbo_company_name').value+','+".$rid.", 'set_po_qnty_ship_date', 'requires/buyer_inspection_controller_v2')","","","","","","" );
	            	...
	            	<table  cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
	        			...
	        			<tr>
	                    	<td> <input type="hidden" id="selected_job">
								<? 
									echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", '',"load_drop_down( 'buyer_inspection_controller_v2', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
								?>
	                    </td>
	                   	<td id="buyer_td">
	                     <? 
...
if($action=="open_order_popup")
{
	...
	<? 
		...
		$sql= "SELECT a.id,a.po_number,sum(distinct (a.po_quantity*c.total_set_qnty)) as po_quantity,SUM (case when b.inspection_level=3 and b.inspection_status=1 then b.inspection_qnty else 0 end) as insp_qty,sum(distinct (a.po_quantity*c.total_set_qnty))-SUM (case when b.inspection_level=3 and b.inspection_status=1 then b.inspection_qnty else 0 end) as balance from wo_po_details_master c,wo_po_break_down a left join pro_buyer_inspection b on  a.id=b.po_break_down_id and b.status_active=1 and b.entry_form=470 where  c.id=a.job_id and a.status_active=1  and a.is_deleted=0 and a.job_no_mst='$txt_job_no' group by  a.id,a.po_number order by a.po_number";
		// echo $sql;
		echo  create_list_view("list_view", "Order No.,Order Qty,Insp Qty,Balance", "150,140,140,150","590","350",0, $sql , "js_set_value", "id,po_number", "", 1, "0,0,0,0", $arr , "po_number,po_quantity,insp_qty,balance", "requires/buyer_inspection_controller_v2",'setFilterGrid("list_view",-1);','','');
...
if($action=="open_actual_order_popup")
{
	...
	<? 
		...
		// echo $sql;
		echo  create_list_view("list_view", "Actual Order No.", "100","590","350",0, $sql , "js_set_value", "actual_id,acc_po_no", "", 1, "0", $arr , "acc_po_no", "requires/buyer_inspection_controller_v2",'setFilterGrid("list_view",-1);','','');
...
if ($action=="open_insp_qty_popup")
{
	...
	<script>
		...
		function fnc_close( )
		{
			...
			for(var i=1; i<=rowCount; i++)
			{
				...
				if(breck_down_data=="")
				{
					breck_down_data+=$('#itemId_'+i).val()+'_'+($('#colorId_'+i).val()*1)+'_'+($('#colorQty_'+i).val()*1)+'_'+($('#itemColorInsQty_'+i).val()*1)+'_'+($('#sizeId_'+i).val()*1);
				}
				else
				{
					breck_down_data+="----"+$('#itemId_'+i).val()+'_'+($('#colorId_'+i).val()*1)+'_'+($('#colorQty_'+i).val()*1)+'_'+($('#itemColorInsQty_'+i).val()*1)+'_'+($('#sizeId_'+i).val()*1);
	...
	<?
		...
		if ($color_size_variable_setting == 3)  //Color Size Level
		{
			...
			?>         
			<table border="1" cellpadding="0" cellspacing="0" id="ins_tbl" rules="all" class="rpt_table" width="500">
				<thead>
					<tr>
						<th width="40">Sl.</th>
				...
				<tbody>
					...
					if($hidden_ins_data)
					{
						...
						{	
							...
							$avail_qty= $val[2] - $buyer_inp_qty_arry[$val[0]][$val[1]][$val[4]];	
							?>
							<tr>
								<td><?= $i; ?></td>
								<td>
									<?= $garments_item[$val[0]];?>
									<input type="hidden" id="itemId_<?= $i;?>" name="itemColorInsQty<?= $i;?>" value="<?= $val[0];?>">
								</td>
								<td>
									<?= $color_arr[$val[1]];?>
									<input type="hidden" id="colorId_<?= $i;?>" name="colorId_<?= $i;?>" value="<?= $val[1];?>">
								</td>
								<td>
									<?= $size_library[$val[4]];?> 
									<input type="hidden" id="sizeId_<?= $i;?>" name="sizeId_<?= $i;?>" value="<?=$val[4];?>">
								</td>
								<td align="right">
									<?= $val[csf("qnty")];?>
									<input type="text" id="colorQty_<?= $i;?>" name="colorQty_<?= $i;?>" disabled="" readonly value="<?= $val[2];?>">
								</td>
		...
		else	 								//Color Level
		{
			...
			$buyer_inp_sql="SELECT a.po_break_down_id,a.country_id,b.item_id,b.color_id,b.ins_qty from pro_buyer_inspection a, pro_buyer_inspection_breakdown b where a.id=b.mst_id and a.entry_form=470 and  a.po_break_down_id=$order $ins_cond and a.status_active=1 and a.inspection_status<>3 and a.entry_form=470 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
			// echo $buyer_inp_sql; die;
			...
			<table border="1" cellpadding="0" cellspacing="0" id="ins_tbl" rules="all" class="rpt_table" width="500">
				...
				<tbody>
					...
					if($hidden_ins_data)
					{
						...
						{	
							$val=explode("_", $all_val);	
							$avail_qty = $val[2] - $buyer_inp_qty_arry[$val[0]][$val[1]];
							?>
							<tr>
								<td><?= $i ;?></td>
								<td><?= $garments_item[$val[0]];?>
								<input type="hidden" id="itemId_<?= $i;?>" name="itemColorInsQty<?= $i;?>" value="<?=$val[0];?>">
								...
								<td><input class="text_boxes_numeric" type="text" id="itemColorInsQty_<?= $i;?>" name="itemColorInsQty_<?= $i;?>" value="<?= $val[3]?>" onkeyup="validateInsQty(<?=$i?>,<?= $avail_qty ?>)" placeholder="<?=$avail_qty?>"></td> 
...
if ($action=="populate_order_data_from_search_popup")
{
	...
	$data_array=sql_select("SELECT a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.style_description,a.order_uom,a.set_break_down,a.gmts_item_id,a.total_set_qnty, sum(b.po_quantity) as po_quantity   from wo_po_details_master a, wo_po_break_down b where  a.job_no ='".$data."' and a.id=b.job_id $gro_field");
...
if($action=="set_po_qnty_ship_date")
{
	// list($order_id,$week_id,$country_id,$inspection_level,$company_id,$update_id,$rid)=explode(",",$data);
	list($order_id,$week_id,$country_id,$inspection_level,$company_id,$rid)=explode(",",$data);
	...
	echo "document.getElementById('txtinspectionqnty_".$rid."').value = '';\n";  
	echo "document.getElementById('txtpoquantity_".$rid."').value = '';\n";
	echo "document.getElementById('txtfinishingqnty_".$rid."').value = '';\n";  
	echo "document.getElementById('txtpubshipmentdate_".$rid."').value = '';\n";
	echo "document.getElementById('txtcuminspectionqnty_".$rid."').value = '';\n"; 
	echo "document.getElementById('cboinspectionstatus_".$rid."').value = 0;\n";
	echo "document.getElementById('cbocause_".$rid."').value = 0;\n";
	...
	if($order_id>0)
	{
		...
		$prev_ins_qnty=return_field_value("sum(inspection_qnty) as inspection_qnty","pro_buyer_inspection","po_break_down_id=$order_id  and inspection_status <> 3 and entry_form=470 and inspection_level=$inspection_level $country_cond $update_cond and status_active=1 and is_deleted=0","inspection_qnty");
		...
		if ($inspection_level == 1) //In-line Inspection
		{
			...
			echo "document.getElementById('txtfinishingqnty_".$rid."').value = '$balance';\n";
		}
		else
		{ 
	...
	if($order_id && $week_id && $country_id)
	{
		...
		foreach ($data_array as $row)
		{
			...
			echo "document.getElementById('txtpoquantity_".$rid."').value = '".$row[csf("order_quantity")]."';\n";  
			echo "document.getElementById('txtpubshipmentdate_".$rid."').value = '".change_date_format($row[csf("country_ship_date")], "dd-mm-yyyy", "-")."';\n";
			echo "document.getElementById('txtcuminspectionqnty_".$rid."').value = '".$cum_previour_qty."';\n"; 
			echo "$('#txtcuminspectionqnty_".$rid."').attr('disabled',true);\n"; 
		 }
	 
	}
	else if($order_id && $week_id && $country_id==0)
	{
		$data_cum_ins=sql_select("select po_break_down_id,week_id, sum(inspection_qnty) as inspection_qnty from  pro_buyer_inspection  where po_break_down_id=$order_id and inspection_level=$inspection_level and inspection_status <> 3 and status_active=1 and is_deleted=0 and entry_form=470 group by  po_break_down_id,week_id" );
		$cum_ins_arr=array();
		foreach($data_cum_ins as $row)
		{
		...
		foreach ($data_array as $row)
		{
			...
			$cum_previour_qty=$cum_ins_arr[$key]['cum_prev'];
			echo "document.getElementById('txtpoquantity_".$rid."').value = '".$row[csf("order_quantity")]."';\n";  
			echo "document.getElementById('txtpubshipmentdate_".$rid."').value = '".change_date_format($row[csf("pub_shipment_date")], "dd-mm-yyyy", "-")."';\n";
			echo "document.getElementById('txtcuminspectionqnty_".$rid."').value = '".$cum_previour_qty."';\n"; 
			echo "$('#txtcuminspectionqnty_".$rid."').attr('disabled',true);\n"; 
		 }
	...
	{
		$data_cum_ins=sql_select("SELECT po_break_down_id,country_id, sum(inspection_qnty) as inspection_qnty from  pro_buyer_inspection  where po_break_down_id=$order_id and inspection_level=$inspection_level and inspection_status <> 3 and status_active=1 and is_deleted=0 and entry_form=470 group by  po_break_down_id,country_id" );
		$cum_ins_arr=array();
		foreach($data_cum_ins as $row)
		{
	...
	else if($order_id && $week_id==0 && $country_id==0)
	{
		...
		$data_cum_ins=sql_select("select po_break_down_id, sum(inspection_qnty) as inspection_qnty from  pro_buyer_inspection  where po_break_down_id=$order_id and inspection_level=$inspection_level and inspection_status <> 3 and status_active=1 and is_deleted=0 and entry_form=470 group by  po_break_down_id" );
		$cum_ins_arr=array();
		foreach($data_cum_ins as $row)
		{
			$cum_ins_arr[$row[csf('po_break_down_id')]]['cum_prev']=$row[csf('inspection_qnty')];
		...
		{
			$cum_previour_qty=$cum_ins_arr[$row[csf('id')]]['cum_prev'];
			echo "document.getElementById('txtpoquantity_".$rid."').value = '".$row[csf("po_quantity")]."';\n";  
			echo "document.getElementById('txtpubshipmentdate_".$rid."').value = '".change_date_format($row[csf("pub_shipment_date")], "dd-mm-yyyy", "-")."';\n";
			echo "document.getElementById('txtcuminspectionqnty_".$rid."').value = '".$cum_previour_qty."';\n"; 
			echo "$('#txtcuminspectionqnty_".$rid."').attr('disabled',true);\n";  
		 }
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		for ($i = 1; $i <= $row_num; $i++) 
		{
			...
			$cboinspectionstatus = "cboinspectionstatus_".$i;
			$txtinspectionqnty = "txtinspectionqnty_".$i;
			$cboinspectionstatus = "cboinspectionstatus_".$i;

			$txt_inspection_qnty=str_replace("'","",$$txtinspectionqnty);
			$cbo_inspection_status=str_replace("'","",$$cboinspectionstatus);
			...
			if($is_control==1 && $user_level!=2)
			{
				if($order_id>0 && $cbo_inspection_status==1)
				{
					if($country_id>0) $country_cond=" and country_id=$country_id"; else $country_cond="";
					$finishing_quantity=return_field_value("sum(production_quantity) as production_quantity","pro_garments_production_mst","po_break_down_id=$order_id and production_type=$qty_source  $country_cond and status_active=1 and is_deleted=0","production_quantity");
					$prev_ins_qnty=return_field_value("sum(inspection_qnty) as inspection_qnty","pro_buyer_inspection","po_break_down_id=$order_id  $country_cond and inspection_level=$cbo_inspection_level and inspection_status=1 and status_active=1 and is_deleted=0 and entry_form=470","inspection_qnty");
			...
			if($is_control==1 && $user_level!=2)
			{
				...
				if($cbo_country_id>0)
				{
					$country_insfection_qty=return_field_value("sum(inspection_qnty)","pro_buyer_inspection","po_break_down_id=$cbo_order_id and country_id=$cbo_country_id and inspection_level=$cbo_inspection_level and inspection_status=1 and status_active=1 and is_deleted=0 and entry_form=470");
				...
				{
					$order_insfection_qty=return_field_value("sum(inspection_qnty)","pro_buyer_inspection","po_break_down_id=$cbo_order_id and inspection_level=$cbo_inspection_level and inspection_status=1 and status_active=1 and is_deleted=0 and entry_form=470");
			...
			$txtinsreason = "txtinsreason_".$i;
			$hidden_ins_data = "hiddeninsdata_".$i;
			$cbo_order_id = "cboorderid_".$i;
			$cbo_actual_order_id = "cboactualorderid_".$i;
			$cbo_week_no = "cboweekno_".$i;
			$cbo_country_id = "cbocountryid_".$i;
			$txt_inspection_qnty = "txtinspectionqnty_".$i;
			$cbo_inspection_status = "cboinspectionstatus_".$i;
			$cbo_cause = "cbocause_".$i;
			$txt_comments = "txtcomments_".$i;
			...
			$data_array .="INTO pro_buyer_inspection (".$field_array.") VALUES(".$id.",".$txt_job_no.",'".$$txtinsreason."','".$$hidden_ins_data."','".$$cbo_order_id."','".$$cbo_actual_order_id."',".$cbo_inspection_company.",".$cbo_source.",".$cbo_working_company.",".$cbo_working_location.",".$cbo_working_floor.",".$cbo_inspection_by.",'".$$cbo_week_no."','".$$cbo_country_id."',".$txt_inp_date.",'".$$txt_inspection_qnty."','".$$cbo_inspection_status."',".$cbo_inspection_level.",'".$$cbo_cause."','".$$txt_comments."',470,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";	
			
			$id_dtls=return_next_id( "id", "pro_buyer_inspection_breakdown", 1 ) ;
			$hidden_ins_data2=str_replace("'","",$$hidden_ins_data);
			$field_array_dtls=" id,mst_id, item_id, color_id,size_id, color_qty, ins_qty, inserted_by, insert_date,  status_active, is_deleted";
			...
			foreach($dtls_data as $data_val)
			{
				...
				if ($data_array_dtls) $data_array_dtls .=",";
					$data_array_dtls .="(".$id_dtls.",".$id.",'".$val[0]."','".$val[1]."','".$val[4]."','".$val[2]."','".$val[3]."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','1',0)";
					$id_dtls++;
	...
	else if ($operation==1)   // Update Here
	{
		...
		if($is_control==1 && $user_level!=2)
		{
			if($order_id>0 && $cbo_inspection_status==1)
			{
				if($country_id>0) $country_cond=" and country_id=$country_id"; else $country_cond="";
				$finishing_quantity=return_field_value("sum(production_quantity) as production_quantity","pro_garments_production_mst","po_break_down_id=$order_id and production_type=$qty_source  $country_cond and status_active=1 and is_deleted=0","production_quantity");
				$prev_ins_qnty=return_field_value("sum(inspection_qnty) as inspection_qnty","pro_buyer_inspection","po_break_down_id=$order_id and id<>$txt_mst_id and inspection_level=$cbo_inspection_level $country_cond and status_active=1 and is_deleted=0 and entry_form=470","inspection_qnty");
				$cu_finish_qnty=$finishing_quantity-$prev_ins_qnty;
				$insfec_qnty=str_replace("'","",$txt_inspection_qnty);
				if($insfec_qnty>$cu_finish_qnty)
				{
		...
		if($is_control==1 && $user_level!=2)
		{
			...
			if($cbo_country_id)
			{
				$country_insfection_qty=return_field_value("sum(inspection_qnty)","pro_buyer_inspection","po_break_down_id=$cbo_order_id and country_id=$cbo_country_id and inspection_level=$cbo_inspection_level and inspection_status=1 and status_active=1 and is_deleted=0 and id<>$txt_mst_id and entry_form=470");
				$country_finishing_qty=return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=$cbo_order_id and production_type=$qty_source and country_id=$cbo_country_id and status_active=1 and is_deleted=0");
			...
			{
				$order_insfection_qty=return_field_value("sum(inspection_qnty)","pro_buyer_inspection","po_break_down_id=$cbo_order_id and inspection_level=$cbo_inspection_level and inspection_status=1 and status_active=1 and is_deleted=0 and id<>$txt_mst_id and entry_form=470");
				$order_finishing_qty=return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=$cbo_order_id and production_type=$qty_source and status_active=1 and is_deleted=0");
		...
		foreach($dtls_data as $data_val)
		{
			$val=explode("_", $data_val);
			if ($data_array_dtls) $data_array_dtls .=",";
				$data_array_dtls .="(".$id_dtls.",".$txt_mst_id.",'".$val[0]."','".$val[1]."','".$val[4]."','".$val[2]."','".$val[3]."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','1',0)";
				$id_dtls++;
...
if($action=="show_active_listview")
{
	...
	$inspection_type=return_field_value("inspected_by","pro_buyer_inspection","job_no='$data' and entry_form=470 and status_active=1 and is_deleted=0");
	
	
	//$arr=array (0=>$po_number,1=>$insp_company_library,4=>$inspection_status,5=>$inspection_cause);
 	$sql= "SELECT actual_po_id, id,po_break_down_id,inspected_by,inspection_company,inspection_date,inspection_qnty,inspection_status,inspection_level,inspection_cause,comments from  pro_buyer_inspection  where   status_active=1 and is_deleted=0 and job_no='$data' and entry_form=470"; 
	...
	?>
			...
			<tr bgcolor="<? echo $bgcolor;?>" id="tr_<? echo $i;?>" onClick="get_php_form_data('<? echo $row[csf('id')]; ?>','populate_inspection_details_form_data','requires/buyer_inspection_controller_v2')" style="cursor:pointer;">
...
{	
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_drop_down_multiple( 'requires/buyer_inspection_controller_v2',".$po_id."+'_'+" . $working_company . "+'_'+" . $working_location. "+'_'+" . $source. "+'_'+" . $inspected_by . "+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('cbo_company_name').value+'_'+'" . $i . "', 'load_all_drop_down', 'week_drop_down_td_".$i."*country_drop_down_td_".$i."*working_company_td*working_location_td*working_floor_td*cutt_company_td' );\n";
		
		// echo "load_drop_down( 'requires/buyer_inspection_controller_v2', '__".$row[csf("po_break_down_id")]."', 'load_drop_down_week_no', 'week_drop_down_td' );";
		// echo "load_drop_down( 'requires/buyer_inspection_controller_v2', '__".$row[csf("po_break_down_id")]."', 'load_drop_down_country_id', 'country_drop_down_td' );";
		 
		//  echo "load_drop_down( 'requires/buyer_inspection_controller_v2', $source, 'load_drop_working_company', 'working_company_td' );";
		//  echo "load_drop_down( 'requires/buyer_inspection_controller_v2', $working_company, 'load_drop_down_working_location', 'working_location_td' );";
		//  echo "load_drop_down( 'requires/buyer_inspection_controller_v2', $working_location, 'load_drop_down_working_floor', 'working_floor_td' );";
		...
		//  echo "load_drop_down( 'requires/buyer_inspection_controller_v2',".$row[csf("inspected_by")]."+','+document.getElementById('cbo_buyer_name').value+','+document.getElementById('cbo_company_name').value, 'load_drop_down_buyer_party_company', 'cutt_company_td' );";
		
	 	echo "get_php_form_data('".$row[csf("po_break_down_id")]."','".$row[csf("week_id")]."','".$row[csf("country_id")]."','".$row[csf("inspection_level")]."','".$working_company."','".$i."','set_po_qnty_ship_date', 'requires/buyer_inspection_controller_v2');\n";
		...
		echo "document.getElementById('cboorderid_".$i."').value = '".$row[csf("po_break_down_id")]."';\n";  
		echo "document.getElementById('cboorderval_".$i."').value = '".$row[csf("po_number")]."';\n";  
		echo "document.getElementById('cboactualorderid_".$i."').value = '".$row[csf("actual_po_id")]."';\n";  
		//echo "document.getElementById('txt_po_quantity').value = '".$row[csf("po_quantity")]."';\n";  
		echo "document.getElementById('txtpubshipmentdate_".$i."').value = '".change_date_format($row[csf("pub_shipment_date")], "dd-mm-yyyy", "-")."';\n"; 
	    echo "document.getElementById('txtinsreason_".$i."').value = '".$row[csf("ins_reason")]."';\n"; 
	    echo "document.getElementById('hiddeninsdata_".$i."').value = '".$row[csf("all_data")]."';\n"; 
	    echo "document.getElementById('txtpoquantity_".$i."').value = '".$row[csf("po_quantity")]."';\n"; 
		echo "document.getElementById('cboinspectionstatus_".$i."').value = '".$row[csf("inspection_status")]."';\n";  
		echo "document.getElementById('cbo_inspection_level').value = '".$row[csf("inspection_level")]."';\n";  
		echo "document.getElementById('cbocause_".$i."').value = '".$row[csf("inspection_cause")]."';\n"; 
		echo "document.getElementById('txtcomments_".$i."').value = '".$row[csf("comments")]."';\n"; 
		echo "document.getElementById('txtinspectionqnty_".$i."').value = '".$row[csf("inspection_qnty")]."';\n"; 
		echo "document.getElementById('txtfinishingqnty_".$i."').value = '".$finish_qty."';\n"; 
		...
		echo "document.getElementById('cboweekno_".$i."').value = '".$row[csf("week_id")]."';\n";  
		echo "document.getElementById('cbocountryid_".$i."').value = '".$row[csf("country_id")]."';\n";  
