...
if($action=="bundle_popup_rescan")
{
	...
	$replace_field_disable_arr = return_library_array("select COMPANY_NAME,SEWING_PRODUCTION from VARIABLE_SETTINGS_PRODUCTION where VARIABLE_LIST = 68 and COMPANY_NAME=$company", "COMPANY_NAME", "SEWING_PRODUCTION");
...
if($action=="create_bundle_search_list_view")
{
	...
	if($job_no!='')
    {
        ...
        else  
		{
			...
			$po_arr=return_library_array( "SELECT a.id, a.id from wo_po_break_down a,wo_po_details_master b where a.job_id=b.id and b.job_no_prefix_num=$job_no",'id','id');			
			// $poIds = implode(",",$po_arr);
			// $jobCon_rescan = " and a.po_break_down_id in($poIds)";
			$jobCon_rescan = where_con_using_array($po_arr,0,"a.po_break_down_id");
		}
...
if($action=="populate_bundle_data")
{
	...
	$replace_field_disable_arr = return_library_array("select COMPANY_NAME,SEWING_PRODUCTION from VARIABLE_SETTINGS_PRODUCTION where VARIABLE_LIST = 68", "COMPANY_NAME", "SEWING_PRODUCTION");	
...
if($action=="populate_bundle_data_rescan")
{
	...
	// 17990000010408,17990000010405
	 $sql="SELECT max(c.id)  as prdid,d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, MAX($year_field) as year, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no,c.barcode_no, sum(c.production_qnty) as qty, e.po_number,a.sewing_line,a.COMPANY_ID from pro_gmts_delivery_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name=$ex_data[3] and a.id=c.delivery_mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.production_type=4 $str_cond and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active in(1,2,3) and d.is_deleted=0 and e.status_active in(1,2,3) and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 $bundle_nos_cond group by d.id, e.id, f.job_no_prefix_num, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no,c.barcode_no, e.po_number,a.sewing_line,a.COMPANY_ID  order by  c.cut_no, length(c.bundle_no) asc, c.bundle_no asc";  //and is_rescan=1
...
if($action=="populate_bundle_data_update")
{
	...
	$replace_field_disable = return_library_array("select COMPANY_NAME,SEWING_PRODUCTION from VARIABLE_SETTINGS_PRODUCTION where VARIABLE_LIST = 68 and COMPANY_NAME=$company_id", "COMPANY_NAME", "SEWING_PRODUCTION");
	...
	?>
		<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i;?>">
			...
			<td width="80" id="prodQty_<? echo $i; ?>" align="right"><? 
			//echo ($row[csf('qty')]+$row[csf('reject_qty')]+$row[csf('alter_qty')]+$row[csf('spot_qty')])-$row[csf('replace_qty')];
			echo $sewing_in_qty; 
			...
			<td width="50" id="rejQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="rejectQty[]" id="rejectQty_<? echo $i; ?>" style="width:35px; " value="<? echo $row[csf('reject_qty')];?>" onBlur="calculate_qcpasss(<? echo $i; ?>,<?=$replace_field_disable[$company_id];?>)" onKeyPress="return numOnly(this,event,this.id);" onDblClick="pop_entry_reject(<? echo $i; ?>)"/></td>

			<td width="50" id="altQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="alterQty[]" id="alterQty_<? echo $i; ?>" style="width:35px; <? echo $colo; ?>" value="<? echo $row[csf('alter_qty')];?>" onBlur="calculate_qcpasss(<? echo $i; ?>,<?=$replace_field_disable[$company_id];?>)" onKeyPress="return numOnly(this,event,this.id);" onDblClick="pop_entry_alter(<? echo $i; ?>)"/></td>
            <?  $colo=''; if($bundle_spot_data[$row[csf('barcode_no')]]!='') $colo="  border-color:#FF0000";?>
			<td width="50" id="sptQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="spotQty[]" id="spotQty_<? echo $i; ?>" style="width:35px; <? echo $colo; ?>" value="<? echo $row[csf('spot_qty')];?>" onBlur="calculate_qcpasss(<? echo $i; ?>,<?=$replace_field_disable[$company_id];?>)" onKeyPress="return numOnly(this,event,this.id);" onDblClick="pop_entry_spot(<? echo $i; ?>)"/></td>
			<td width="50" id="repQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="replaceQty[]" id="replaceQty_<? echo $i; ?>" style="width:35px" value="<? echo $row[csf('replace_qty')];?>" onBlur="calculate_qcpasss(<? echo $i; ?>,<?=$replace_field_disable[$company_id];?>)" <?=$readonly;?> onKeyPress="return numOnly(this,event,this.id);"/></td>
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$delivery_basis)==3)
		{
			...
			foreach($dtlsArr as $bundle_no=>$qty)
			{
				...
				{
					echo "20**Sewing output qty is over than input qty.Bundle no=$bundle_no";die();
				}
			}
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$challan_no=(int) $txt_chal_no[3];
			...
			// ================== getting input qty =====================
			$sql="SELECT c.production_qnty,c.bundle_no from pro_garments_production_dtls c where  c.bundle_no  in ($bundle)  and c.production_type=4 and c.status_active=1 and c.is_deleted=0";
			$result = sql_select($sql);
			$prev_qty_arr = array();
			foreach ($result as $row)
			{
			...
			// ================== getting output qty =====================
			$sql="SELECT c.production_qnty,c.bundle_no from pro_garments_production_dtls c where  c.bundle_no  in ($bundle)  and c.production_type=5 and c.status_active=1 and c.is_deleted=0  and c.delivery_mst_id!=$mst_id";
			$result = sql_select($sql);
			foreach ($result as $row)
			{
		...
		if(str_replace("'","",$delivery_basis)==3)
		{
			...
			$field_array_mst="id, delivery_mst_id,cut_no, company_id, garments_nature, challan_no, po_break_down_id, item_number_id, country_id, production_source, serving_company, location, production_date,production_hour, production_quantity,reject_qnty,alter_qnty,spot_qnty,replace_qty, production_type, entry_break_down_type, shift_name, remarks, floor_id,sewing_line,prod_reso_allo,wo_order_no, inserted_by, insert_date";
			...
			if($data_array_mst!="" && $data_array_dtls!="")
			{
				...
				$rejectDelete=execute_query("DELETE FROM pro_gmts_prod_dft WHERE mst_id in (".$production_mst_id.") and defect_type_id in(2,3,4) and production_type=5",0);
				//echo "DELETE FROM pro_gmts_prod_dft WHERE mst_id in (".$production_mst_id.") and defect_type_id in(3,4) and production_type=5";

				$delete = execute_query("UPDATE pro_garments_production_mst SET is_tab=2, is_deleted=1, status_active=0 WHERE delivery_mst_id=$mst_id and production_type=5",0);
				$delete_dtls = execute_query("UPDATE pro_garments_production_dtls SET is_deleted=1, status_active=0 WHERE delivery_mst_id=$mst_id and production_type=5",0);
				$delete_bundle = execute_query("UPDATE pro_cut_delivery_color_dtls SET is_deleted=1, status_active=0 WHERE delivery_mst_id=$mst_id and production_type=5",0);
			}
...
if($action=="emblishment_issue_print_4")
{
    ...
    <table cellspacing="0" >
        ...
        <tr>
            <td colspan="6" align="center" style="font-size:20px"><u><strong>Delivery to Sewing Floor Challan</strong></u></td>
        </tr>
        <tr>
            <td width="95"><strong>Challan No:</strong></td>
			<td width="20">:</td>
...
if($action=="emblishment_issue_print_5")
{
	...
	$buyer_arr=return_library_array( "select id, buyer_name from lib_buyer",'id','buyer_name');
	$order_array=array();
	$order_sql="SELECT a.job_no, a.buyer_name,a.style_ref_no, a.style_description,b.id, b.po_number, b.po_quantity,b.grouping from  wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0";
	$order_sql_result=sql_select($order_sql);
	foreach ($order_sql_result as $row)
	{
		$order_array[$row[csf('id')]]['buyer_name']=$row[csf('buyer_name')];
