...
if($action=="create_po_search_list_view__")
{
	...
	$total_entry_qty_data_arr=array();
	$total_entry_qty=sql_select( "select po_break_down_id, item_number_id, country_id, sum(production_quantity) as production_quantity, pack_type from pro_garments_production_mst where status_active=1 and is_deleted=0 and production_type=8 group by po_break_down_id, item_number_id, country_id, pack_type");
...
if($action=="color_and_size_level")
{
	...
	unset($sql_woven_finish_receive);
	...
	if($qty_source==0)
	{
		foreach($colorResult as $color)
		{
			...
			{ 
				...
				$order_rate=$con_per_dzn[$color[csf("color_number_id")]]/$costing_per_qty;
				$colorHTML .='<tr><td>'.$color_library[$color[csf("color_number_id")]].'</td><td><input type="text" name="txt_color" id="colSize_'.($i+1).'" style="width:60px"  class="text_boxes_numeric" placeholder="'.($production_quantity-$color[csf("production_qnty")]).'" onblur="fn_colorlevel_total('.($i+1).')"></td><td><input type="text" name="txtColSizeRej" id="colSizeRej_'.($i+1).'" style="width:60px"  class="text_boxes_numeric" placeholder="Rej." onblur="fn_colorRej_total('.($i+1).') '.$disable.'"><input type="hidden" name="colorSizefabricRate" id="colorSizefabricRate_'.($i+1).'" style="width:60px"  class="text_boxes_numeric" value="'.$order_rate.'"></td></tr>';				
	...
	<div style="width:100%;max-height:180px; overflow:y-scroll" id="iron_list_view" align="left">
		...
		<?php  
			...
			foreach($sqlResult as $selectResult){
				...
				$total_production_qnty+=$selectResult[csf('production_quantity')];
				$material_cost_id= 0//return_field_value("id","pro_garments_material_cost","mst_id='".$selectResult[csf('id')]."'");
 		?>
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data('<? echo $selectResult[csf('id')]."_".$job_no."_".$material_cost_id; ?>','populate_input_form_data','requires/finishing_entry_controller');" > 
				<td width="30" align="center"><? echo $i; ?></td>
                <td width="150" align="center"><p><? echo $garments_item[$selectResult[csf('item_number_id')]]; ?></p></td>
                ...
                $source= $selectResult[csf('production_source')];
                if($source==3)
                $serving_company= $supplier_name[$selectResult[csf('serving_company')]];
                else
                $serving_company= $company_arr[$selectResult[csf('serving_company')]];
                ?>	
                <td width="120" align="center"><p><?php echo $serving_company; ?></p></td>
...
if($action=="populate_input_form_data")
{
	...
	if($db_type==0)
	{
		$sqlResult =sql_select("SELECT id, company_id, garments_nature, po_break_down_id, item_number_id, challan_no, country_id,  pack_type, production_source, serving_company, sewing_line, location, produced_by, embel_name, embel_type, production_date, production_quantity, production_source, production_type, entry_break_down_type, break_down_type_rej, TIME_FORMAT( production_hour, '%H:%i' ) as production_hour, sewing_line, supervisor, carton_qty, remarks, floor_id, alter_qnty, spot_qnty, reject_qnty, total_produced, yet_to_produced  from pro_garments_production_mst where id='$data[0]' and production_type='8' and status_active=1 and is_deleted=0 order by id");
	}
	else
	{
		$sqlResult =sql_select("SELECT id, company_id, garments_nature, po_break_down_id, item_number_id, challan_no, country_id,  pack_type, production_source, serving_company, sewing_line, location, produced_by, embel_name, embel_type, production_date, production_quantity, production_source, production_type, entry_break_down_type, break_down_type_rej, TO_CHAR(production_hour,'HH24:MI') as production_hour, sewing_line, supervisor, carton_qty, remarks, floor_id, alter_qnty, spot_qnty, reject_qnty, total_produced, yet_to_produced  from pro_garments_production_mst where id='$data[0]' and production_type='8' and status_active=1 and is_deleted=0 order by id");	
	}
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		//production_type array	
  		$field_array1="id, garments_nature, company_id, challan_no, po_break_down_id, item_number_id, country_id, pack_type, production_source, serving_company, location, produced_by, production_date, production_quantity, production_type, entry_break_down_type, break_down_type_rej, production_hour, carton_qty, remarks, floor_id, alter_qnty, spot_qnty, reject_qnty, total_produced, yet_to_produced, inserted_by, insert_date"; 
		if($db_type==0)
		{
			$data_array1="(".$id.",".$garments_nature.",".$cbo_company_name.",".$txt_challan.",".$hidden_po_break_down_id.",".$cbo_item_name.",".$cbo_country_name.",".$txt_pack_type.",".$cbo_source.",".$cbo_finish_company.",".$cbo_location.",".$cbo_produced_by.",".$txt_finishing_date.",".$txt_finishing_qty.",8,".$sewing_production_variable.",".$finish_production_variable_rej.",".$txt_reporting_hour.",".$txt_carton_qty.",".$txt_remark.",".$cbo_floor.",".$txt_alter_qnty.",".$txt_spot_qnty.",".$txt_reject_qnty.",".$txt_cumul_finish_qty.",".$txt_yet_to_finish.",".$user_id.",'".$pc_date_time."')";
		}
	  	else if($db_type==2)
		{
		...
		$fabric_cost = ($fabric_cost=="") ? 0 : $fabric_cost;
		...
		{  
			...
			{
				if($rID && $dtlsrID && $rID_material && $defectSpot)
				{
					mysql_query("COMMIT");  
			...
			{
				if($rID && $rID_material && $defectSpot)
				{
					mysql_query("COMMIT");  
					echo "0**".str_replace("'","",$hidden_po_break_down_id);
				}
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		if($db_type==0)
		{
			if(str_replace("'","",$sewing_production_variable)!=1)
			{
				if($rID && $dtlsrDelete && $dtlsrID && $rIDmaterial && $defectSpot)
				{
					mysql_query("COMMIT");  
