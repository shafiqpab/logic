	...
	?>
		...
		function js_set_value(str)
		{
			...
			var splitArr = str.split("_");
	 		$("#hidden_return_id").val(splitArr[0]);
	 		$("#hidden_posted_in_account").val(splitArr[1]);
...
if($action=="create_return_search_list")
{
	...
	if($db_type==0)
	{
		$sql = "select a.id, a.sys_number_prefix_num, year(a.insert_date) as delivery_year, a.sys_number, a.company_id, a.location_id, a.challan_no, a.buyer_id, b.is_posted_account, a.transport_supplier, a.delivery_date, a.lock_no, a.driver_name, a.truck_no, a.dl_no,group_concat(b.po_break_down_id) as po_break_down_id
	...
	group by  a.id, a.sys_number_prefix_num, year(a.insert_date), a.sys_number, a.company_id, a.location_id, a.challan_no, a.buyer_id, b.is_posted_account, a.transport_supplier, a.delivery_date, a.lock_no, a.driver_name, a.truck_no, a.dl_no  order by a.id desc";
	}
	else
	{
		$sql = "select a.id, a.sys_number_prefix_num, to_char(a.insert_date,'YYYY') as delivery_year, a.sys_number, a.company_id, a.location_id, a.challan_no, a.buyer_id, b.is_posted_account, a.transport_supplier, a.delivery_date, a.lock_no, a.driver_name, a.truck_no, a.dl_no, listagg(CAST(b.po_break_down_id as VARCHAR(4000)),',') within group (order by b.po_break_down_id) as po_break_down_id
	from  pro_ex_factory_delivery_mst a,  pro_ex_factory_mst b
	where a.id=b.delivery_mst_id and a.entry_form=85 and a.status_active=1 and a.is_deleted=0 $sql_cond $po_cond
	group by  a.id, a.sys_number_prefix_num, to_char(a.insert_date,'YYYY'), a.sys_number, a.company_id, a.location_id, a.challan_no, a.buyer_id, b.is_posted_account, a.transport_supplier, a.delivery_date, a.lock_no, a.driver_name, a.truck_no, a.dl_no  order by a.id desc";
	}
	//echo $sql;die;
...
{
	// print_r(1234);die;
	$sql_mst=sql_select("SELECT id, sys_number, company_id, location_id, challan_no, transport_supplier, delivery_date, lock_no, driver_name, truck_no, dl_no,destination_place,forwarder,mobile_no, do_no, gp_no,remarks,delivery_company_id,delivery_location_id
	...
	foreach($sql_mst as $row)
	{
		...
		echo "$('#cbo_delivery_company_id').val(".$row[csf('delivery_company_id')].");\n";
		echo "$('#cbo_delivery_location_id').val(".$row[csf('delivery_location_id')].");\n";
		...
		echo "$('#remarks').val('".$row[csf('remarks')]."');\n";
...
{
	$sql_mst=sql_select("select id, sys_number, company_id, location_id, challan_no, buyer_id, transport_supplier, delivery_date, lock_no, driver_name, truck_no, dl_no,destination_place,forwarder,mobile_no, do_no, gp_no,source,delivery_company_id, delivery_location_id
	...
	foreach($sql_mst as $row)
	{
		...
		echo "$('#txt_do_no').val('".$row[csf('do_no')]."');\n";
		echo "$('#txt_gp_no').val('".$row[csf('gp_no')]."');\n";
		echo "$('#txt_source').val('".$row[csf('source')]."');\n";
		echo "$('#txt_destination').val('".$row[csf('destination_place')]."');\n";
		echo "$('#cbo_forwarder').val(".$row[csf('forwarder')].");\n";
		echo  "$('#cbo_transport_company').attr('disabled',true);\n";
		echo  "$('#cbo_forwarder').attr('disabled',true);\n";
...
if($action=="show_dtls_listview_mst")
{
	...
	if($variable_settings_production !=1) // color size level
	{
		...
		where a.id=b.delivery_mst_id  and b.id=c.mst_id and d.id=c.color_size_break_down_id and b.po_break_down_id=d.po_break_down_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and( a.challan_no='$data[1]' or   a.sys_number='$data[1]')
		...
		from  pro_ex_factory_delivery_mst a,  pro_ex_factory_mst b ,pro_ex_factory_dtls c,wo_po_color_size_breakdown d
		where a.id=b.delivery_mst_id  and b.id=c.mst_id and d.id=c.color_size_break_down_id and b.po_break_down_id=d.po_break_down_id and a.status_active=1 and a.id=$data[0] and a.is_deleted=0 and b.status_active=1 and c.status_active=1 and d.status_active=1
...
if($action=="show_dtls_listview_mst2")
{
	...
	if($variable_settings_production !=1) // for color and size level
	{
		...
		from  pro_ex_factory_delivery_mst a,  pro_ex_factory_mst b ,pro_ex_factory_dtls c,wo_po_color_size_breakdown d
		where a.id=b.delivery_mst_id  and b.id=c.mst_id and d.id=c.color_size_break_down_id and b.po_break_down_id=d.po_break_down_id and a.status_active=1 and a.sys_number='$data[1]' and a.entry_form!=85  and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
		group by  b.po_break_down_id,b.item_number_id,c.color_size_break_down_id,b.country_id,b.id,c.id,d.size_number_id,d.color_number_id order by b.po_break_down_id";
		$result=sql_select($sql_data);
		$po_color_arr=array();
		foreach($result as $row)
		...
		from  pro_ex_factory_delivery_mst a,  pro_ex_factory_mst b ,pro_ex_factory_dtls c,wo_po_color_size_breakdown d
		where a.id=b.delivery_mst_id  and b.id=c.mst_id and d.id=c.color_size_break_down_id and b.po_break_down_id=d.po_break_down_id and a.status_active=1 and a.challan_no='$data[1]'  and a.entry_form=85  and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
		group by  a.id,b.po_break_down_id,b.item_number_id,c.color_size_break_down_id,b.country_id,b.id,c.id,d.size_number_id,d.color_number_id order by b.po_break_down_id";
		...
		from  pro_ex_factory_delivery_mst a,  pro_ex_factory_mst b ,pro_ex_factory_dtls c,wo_po_color_size_breakdown d
		where a.id=b.delivery_mst_id  and b.id=c.mst_id and d.id=c.color_size_break_down_id and b.po_break_down_id=d.po_break_down_id and a.status_active=1 and a.id=$data[0] and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
		group by a.id,  b.po_break_down_id,b.item_number_id,c.color_size_break_down_id,b.country_id,b.id,c.id,d.size_number_id,d.color_number_id order by b.po_break_down_id";
		$sql_result=sql_select($sql_gmt);
		$k=1;
		foreach($sql_result as $row)
		{
			...
			$color_size=$size_library[$row[csf('size_number_id')]];
			 ...
			 $return_qty2=$previous_prod_qty; //Ex-Factory Qty
			// echo $previous_prod_qty.'='.$previous_prod_qty2.'++'.$row[csf('prod_qty')];
		?>
			<tr class="" id="tr_<? echo $k; ?>">
	            <td><? echo $k; ?></td>
	            <td>
					<? echo create_drop_down( "cbobuyer_".$k, 110,"select id,buyer_name from lib_buyer where status_active=1 and is_deleted=0","id,buyer_name",1, "-- Select --",$buyer_id , "1",1,1 ); ?>
				</td>
	            <td>
	                <input type="text" name="txtjob[]" id="txtjob_<? echo $k; ?>" class="text_boxes" style="width:110px;" value="<? echo $job_library[$row[csf('po_id')]];?>" disabled >
	            </td>
...
if ($action=="save_update_delete")
{
	...
	{
		//$delivery_mst_id=str_replace("'","",$delivery_mst_id);

		$accounting_check = return_field_value("is_posted_account","pro_ex_factory_mst","id=$delivery_mst_id and status_active=1 and entry_form=85","is_posted_account");
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$txt_return_id)=="")
		{
			...
			$field_array_delivery="id, sys_number_prefix, sys_number_prefix_num, sys_number, company_id,buyer_id, location_id, challan_no,transport_supplier, delivery_date, lock_no, driver_name, truck_no, dl_no, mobile_no, do_no, gp_no,source, destination_place, forwarder,entry_form,inserted_by,insert_date,remarks,delivery_company_id,delivery_location_id";
			$data_array_delivery="(".$return_mst_id.",'".$new_sys_number[1]."','".$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",".$buyer_id.",".$cbo_location_name.",".$txt_challan_no.",".$cbo_transport_company.",".$txt_return_date.",".$txt_lock_no.",".$txt_driver_name.",".$txt_truck_no.",".$txt_dl_no.",".$txt_mobile_no.",".$txt_do_no.",".$txt_gp_no.",".$txt_source.",".$txt_destination.",".$cbo_forwarder.",85,".$user_id.",'".$pc_date_time."',".$remarks.",".$cbo_delivery_company_id.",".$cbo_delivery_location_id.")";
		...
		else
		{
			...
			$field_array_delivery="company_id*buyer_id*location_id*buyer_id*transport_supplier*delivery_date*lock_no*driver_name*truck_no*dl_no*mobile_no*do_no*gp_no*destination_place*forwarder*updated_by*update_date*remarks";
			$data_array_delivery="".$cbo_company_name."*".$buyer_id."*".$cbo_location_name."*".$cbo_transport_company."*".$txt_return_date."*".$txt_lock_no."*".$txt_driver_name."*".$txt_truck_no."*".$txt_dl_no."*".$txt_mobile_no."*".$txt_do_no."*".$txt_gp_no."*".$txt_destination."*".$cbo_forwarder."*".$user_id."*'".$pc_date_time."'*".$remarks."";
		}
  	...
  	else if ($operation==1000) // Update Here End------------------------------------------------------
	{
		$con = connect();
		if($db_type==0)	{ mysql_query("BEGIN"); }
		...
		$field_array_delivery="company_id*location_id*transport_supplier*delivery_date*lock_no*driver_name*truck_no*dl_no*mobile_no*do_no*gp_no*destination_place*forwarder*updated_by*update_date*remarks";
			$data_array_delivery="".$cbo_company_name."*".$cbo_location_name."*".$cbo_transport_company."*".$txt_return_date."*".$txt_lock_no."*".$txt_driver_name."*".$txt_truck_no."*".$txt_dl_no."*".$txt_mobile_no."*".$txt_do_no."*".$txt_gp_no."*".$txt_destination."*".$cbo_forwarder."*".$user_id."*'".$pc_date_time."'*".$remarks."";
		...
		$field_array_dtls="id,delivery_mst_id,garments_nature,po_break_down_id,item_number_id,country_id,location,ex_factory_date,ex_factory_qnty,total_carton_qnty,entry_form,challan_no,invoice_no,lc_sc_no,carton_qnty,transport_com,remarks,shiping_status,entry_break_down_type,inspection_qty_validation,status_active,is_deleted,inserted_by,insert_date";
		$mst_dtls_arr=("SELECT id,delivery_mst_id,garments_nature,po_break_down_id,item_number_id,country_id,location,ex_factory_date,ex_factory_qnty,total_carton_qnty,entry_form,challan_no,invoice_no,lc_sc_no,carton_qnty,transport_com,remarks,shiping_status,entry_break_down_type,inspection_qty_validation,status_active,is_deleted from pro_ex_factory_mst where  delivery_mst_id=$txt_prev_mst_id and status_active=1 and is_deleted=0 order by id" );
		// echo "10**". $mst_dtls_arr;die;
		$resutl=sql_select($mst_dtls_arr);
		$mst_dtls_id=return_next_id("id", "pro_ex_factory_mst", 1);

		$data_array_dtls="";
		$j=0;
		$return_actual_ret_qty='';
		$delivery_qty=0;
		foreach($resutl  as $row)
		{
			$mst_id_arr[$row[csf('id')]]=$mst_dtls_id;

			if($j==0) $data_array_dtls= "(".$mst_dtls_id.",".$txt_return_id.",'".$row[csf('garments_nature')]."','".$row[csf('po_break_down_id')]."','".$row[csf('item_number_id')]."','".$row[csf('country_id')]."','".$row[csf('location')]."',".$txt_return_date.",'".$mst_return_qty_arr[$row[csf('id')]]."','".$row[csf('total_carton_qnty')]."',85,".$txt_challan_no.",'".$row[csf('invoice_no')]."','".$row[csf('lc_sc_no')]."','".$row[csf('carton_qnty')]."','".$row[csf('transport_com')]."','".$row[csf('remarks')]."','".$row[csf('shiping_status')]."','".$row[csf('entry_break_down_type')]."','".$row[csf('inspection_qty_validation')]."','".$row[csf('status_active')]."','".$row[csf('is_deleted')]."',".$user_id.",'".$pc_date_time."')";
			else
			 $data_array_dtls.=",(".$mst_dtls_id.",".$txt_return_id.",'".$row[csf('garments_nature')]."','".$row[csf('po_break_down_id')]."','".$row[csf('item_number_id')]."','".$row[csf('country_id')]."','".$row[csf('location')]."',".$txt_return_date.",'".$mst_return_qty_arr[$row[csf('id')]]."','".$row[csf('total_carton_qnty')]."',85,".$txt_challan_no.",'".$row[csf('invoice_no')]."','".$row[csf('lc_sc_no')]."','".$row[csf('carton_qnty')]."','".$row[csf('transport_com')]."','".$row[csf('remarks')]."','".$row[csf('shiping_status')]."','".$row[csf('entry_break_down_type')]."','".$row[csf('inspection_qty_validation')]."','".$row[csf('status_active')]."','".$row[csf('is_deleted')]."',".$user_id.",'".$pc_date_time."')";

			$mst_dtls_id=$mst_dtls_id+1;
 			$j++;

		}
		// echo "10**INSERT INTO pro_ex_factory_mst (".$field_array_dtls.") VALUES ".$data_array_dtls;die;
		// pro_ex_factory_dtls table entry here ----------------------------------///
		...
		for($i=1; $i<=$row_num; $i++)
		{
			...
			$update_mst_id="dtlsmstid_".$i;
			$mst_dtld_update_id=str_replace("'","",$$update_mst_id);
			$updateID_array[] = $mst_dtld_update_id;
			$mst_return_qty_arr[$mst_id_arr[$mst_dtld_update_id]]+=str_replace("'","",$$txtreturnqty);
		}


		$field_array_dtls_up="ex_factory_qnty,ex_factory_date";
		$field_array_dtls2="id,mst_id,color_size_break_down_id,production_qnty";
		$dtls_id=return_next_id("id", "pro_ex_factory_dtls", 1);
		//$dltreturnid=explode(",",$dtls_return_id);
		$add_comma=0;$return_ids=="";
		$update_details_id_arr = array();
		for($i=1; $i<=$row_num; $i++)
		{
			$wo_po_color_mst_id="colormstid_".$i;
			$txtreturnqty="txtreturnqty_".$i;
			...
			$update_details_id_arr[$$update_details_id] = $$update_details_id;
			//echo $mst_id_arr[$mst_dtld_update_id];
			//echo $delivery_qty=$presentStock_qty_arr[$mst_id_arr[$mst_dtld_update_id]];
			 $return_actual_ret_qty=$delivery_qty-$$txtreturnqty;
			//echo "0**".$$update_mst_id; die;
		...
		// ================= delete mst and dtls table data ======================
		$mstIds = implode(",",array_filter($updateID_array));
		$dtlsIds = implode(",",array_filter($update_details_id_arr));
		$rMstId = execute_query("UPDATE pro_ex_factory_mst SET is_deleted=1, status_active=0 WHERE id in($mstIds)");
		$rDtlsId = execute_query("UPDATE pro_ex_factory_dtls SET is_deleted=1, status_active=0 WHERE id in($dtlsIds)");
		// echo "10**UPDATE pro_ex_factory_dtls SET is_deleted=1, status_active=0 WHERE id in($dtlsIds)";die;
		// ==================== end delete ======================
		...
		// echo "10**".$rID."=". $deliveryrID."=". $dtlsrID."=". $rID_dtls;oci_rollback($con);die;
		
		if($db_type==2 || $db_type==1 )
		{

				if($rID && $deliveryrID && $dtlsrID && $rID_dtls)
				{
					oci_commit($con);
	...
	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$field_array_delivery="company_id*location_id*transport_supplier*delivery_date*lock_no*driver_name*truck_no*dl_no*mobile_no*do_no*gp_no*destination_place*forwarder*updated_by*update_date*remarks*delivery_company_id*delivery_location_id";
			$data_array_delivery="".$cbo_company_name."*".$cbo_location_name."*".$cbo_transport_company."*".$txt_return_date."*".$txt_lock_no."*".$txt_driver_name."*".$txt_truck_no."*".$txt_dl_no."*".$txt_mobile_no."*".$txt_do_no."*".$txt_gp_no."*".$txt_destination."*".$cbo_forwarder."*".$user_id."*'".$pc_date_time."'*".$remarks."'*".$cbo_delivery_company_id."'*".$cbo_delivery_location_id."";
		...
		$field_array_dtls_up2="ex_factory_qnty*ex_factory_date";
		$mst_return_qty_arr=array();
		for($i=1; $i<=$row_num; $i++)
		{
			$txtreturnqty="txtreturnqty_".$i;
		...
		foreach($mst_return_qty_arr as $update_key=>$val_qty)
		{
			$ret_val_qty=$val_qty."*".$txt_return_date;//+$presentStock_qty_arr[str_replace("'","",$update_key)];
			$updateID_array_up[]=str_replace("'",'',$update_key);
			$data_array_dtls_upate[str_replace("'",'',$update_key)]=explode("*",("".$ret_val_qty."*".$txt_return_date));
		}
		...
		$field_array_dtls_up="mst_id*color_size_break_down_id*production_qnty";
			...
			// echo bulk_update_sql_statement("pro_ex_factory_mst","id",$field_array_dtls_up2,$data_array_dtls_upate,$updateID_array_up);die;
...
if($action=="garments_exfactory_print")
{
	...
	$delivery_mst_sql=sql_select("select id, sys_number, transport_supplier, driver_name, truck_no, dl_no, lock_no, destination_place,challan_no,sys_number_prefix_num,remarks from pro_ex_factory_delivery_mst where id=$data[1]");
	...
	<div style="width:810px; margin-top:5px;">
	    <table width="800" cellspacing="0" align="right" style="margin-bottom:20px;">
	        ...
	        <tr class="form_caption">
	            ...
	            ?>
	        <tr>
	            <td colspan="5" style="font-size:18px;; padding-left:252px;"><strong><? echo "Ex-Factory Return Challan" //$data[3]; ?></strong></td>
	        ...
	        <tr >
	            ...
	            <td width="180" valign="top" style="font-size:16px;"><? echo "Dyes and Chemical"; //echo $supplier_library[$supplier_name]; ?></td>
	            <td width="100" valign="top" style="font-size:16px;"><strong>Return No</strong></td>
				<td width="10"><strong>:</strong></td>
	            <td width="180" valign="top" style="font-size:16px;"><? echo $system_num; ?> </td>
	           <!-- <td width="60" valign="top" style="font-size:16px;">&nbsp;&nbsp;</td> -->
	            <td width="100" valign="top" style="font-size:16px;"><strong>DL/NO</strong></td>
				<td width="10"><strong>:</strong></td>
	            <td width="180" valign="top" style="font-size:16px;"><? echo $dl_no; ?> </td>
	        </tr>
	        ...
	        <tr>
				...
				<td width="10"><strong>:</strong></td>
	            <td valign="top" style="font-size:16px;"><? echo $challan_no; ?> </td>
	            <!-- <td width="60" valign="top" style="font-size:16px;">&nbsp;&nbsp;</td> -->
			...
			<tr>
				...
				<td style="font-size:16px;"></td>
	            <td style="font-size:16px;"></td>
			   <td rowspan="2" align="right" colspan="4" id="barcode_img_id"></td>
			</tr>
	    </table><br>
	        <?
...
if($action=="garments_exfactory_print2")
{
	//$start = microtime(true);
	extract($_REQUEST);
	$data=explode('*',$data);
	...
	$buyer_library=return_library_array( "select id, short_name from   lib_buyer", "id", "short_name"  );
	$invoice_library=return_library_array( "select id, invoice_no from  com_export_invoice_ship_mst", "id", "invoice_no"  );
	 $forward_library=return_library_array("select a.id,a.supplier_name from lib_supplier a, lib_supplier_tag_company b where a.id=b.supplier_id and a.status_active=1 and a.is_deleted=0  and a.id in (select  supplier_id from  lib_supplier_party_type where party_type in(30,31,32)) group by a.id, a.supplier_name order by a.supplier_name","id","supplier_name");
	$order_sql=sql_select("select a.id,a.job_no_mst,a.po_number, b.buyer_name, b.gmts_item_id, b.style_ref_no,c.country_id,c.color_number_id from  wo_po_break_down a, wo_po_details_master b,wo_po_color_size_breakdown c where a.job_no_mst=b.job_no and a.id=c.po_break_down_id and a.status_active=1 and b.status_active=1 and c.status_active=1");
	...
	{
		$order_job_arr[$row[csf("id")]]['po_number']=$row[csf("po_number")];
		$order_job_arr[$row[csf("id")]]['job_no_mst']=$row[csf("job_no_mst")];
		$order_job_arr[$row[csf("id")]]['buyer_name']=$row[csf("buyer_name")];
		$order_job_arr[$row[csf("id")]]['gmts_item_id']=$row[csf("gmts_item_id")];
		$order_job_arr[$row[csf("id")]]['style_ref_no']=$row[csf("style_ref_no")];
		$order_job_arr[$row[csf("id")]]['country_id']=$row[csf("country_id")];
		$order_job_arr[$row[csf("id")]]['color_number_id']=$row[csf("color_number_id")];


	}

	//echo "select id, sys_number, transport_supplier, driver_name, truck_no, dl_no, lock_no, destination_place,challan_no,sys_number_prefix_num from pro_ex_factory_delivery_mst where id=$data[1]";die;
	$delivery_mst_sql=sql_select("select id, sys_number, transport_supplier, driver_name, truck_no, dl_no, lock_no,mobile_no, destination_place,delivery_date,challan_no,gp_no,forwarder,sys_number_prefix_num from pro_ex_factory_delivery_mst where id=$data[1]");
	foreach($delivery_mst_sql as $row)
	{
		$supplier_name=$row[csf("transport_supplier")];
		$driver_name=$row[csf("driver_name")];
		...
		$return_date=$row[csf('delivery_date')];
	...
	<div style="width:810px; margin-top:5px;">
	    <table width="800" cellspacing="0" align="right" style="margin-bottom:20px;">
		  ...
		  <table>
	        <tr>
	            <td colspan="5" style="font-size:18px; padding-left:252px;text-decoration:underline;"><strong><? echo "Garments Ex-Factory Return" //$data[3]; ?></strong></td>

	        </tr>
			</table>
			<div style="width:1520px; margin-top:5px;">
				<table width="1500px" align="right" border="1" rules="all" class="rpt_table" >
					<tr >
						<td width="100" valign="top" style="font-size:16px;"><strong>Name:</strong></td>
						<td width="180" valign="top" style="font-size:16px;"><? echo $supplier_library[$supplier_name]; ?></td>
						<td  valign="top" style="font-size:16px;"><strong>Challan No :</strong></td>
						<td  valign="top" style="font-size:16px;"><? echo $challan_no; ?> </td>
						<td  width="60" valign="top" style="font-size:16px;"><strong>Ex-Factory Date: </strong></td>
						<td  width="60" valign="top" style="font-size:16px;"><? echo change_date_format($data[2]); ?></td>
						<td  width="60" valign="top" style="font-size:16px;"><strong>Return Date: </strong></td>
						<td  width="60" valign="top" style="font-size:16px;"><? echo $return_date; ?></td>

						<!--  <td width="60" valign="top" style="font-size:16px;">&nbsp;&nbsp;</td> -->
						<td width="120" valign="top" style="font-size:16px;"><strong>&nbsp;&nbsp;&nbsp;DL/NO:</strong></td>
						<td width="140" valign="top" style="font-size:16px;"><? echo "&nbsp;&nbsp;&nbsp;&nbsp;".$dl_no; ?> </td>
					</tr>

					<tr>
						<td valign="top" style="font-size:16px;"><strong>Address:</strong></td>
						<td valign="top" colspan="7" style="font-size:16px;"><? echo $address_1."<br>"; if($contact_no!="") echo "Phone : ".$contact_no; ?> </td>

						<!-- <td width="60" valign="top" style="font-size:16px;">&nbsp;&nbsp;</td> -->
						<td width="60" vlaign="top" style="font-size:16px;"><strong>Truck No:</strong></td>
						<td  width="60" valign="top" style="font-size:16px;"><? echo "&nbsp;&nbsp;&nbsp;&nbsp;".$truck_no; ?> </td>
					</tr>
					<tr>
					    <td  width="60" valign="top" style="font-size:16px;"><strong>Phone: </strong></td>
						<td  width="60" valign="top" style="font-size:16px;"><? echo $mobile_no; ?></td>
						<td  width="60" valign="top" style="font-size:16px;"><strong>Forwarder: </strong></td>
						<td  width="60" valign="top" style="font-size:16px;"><? echo $forward_library[$forwarder]; ?></td>
					...
					<tr >
						...
						<td  valign="top" style="font-size:16px;"><? echo $driver_name; ?> </td>
						<!-- <td width="60" valign="top" style="font-size:16px;">&nbsp;&nbsp;</td> -->
						<td  width="60" valign="top" style="font-size:16px;"><strong>GP NO: </strong></td>
						<td  width="60" valign="top" style="font-size:16px;"><? echo $GP_NO; ?></td>
						<td style="font-size:16px;"><strong >Lock No :</strong></td>
						<td style="font-size:16px;"><? echo "&nbsp;&nbsp;&nbsp;&nbsp;".$lock_no; ?> </td>
					</tr>
				</table>
			...
			<table align="right" cellspacing="0" width="<? echo $table_width;?>"  border="1" rules="all" class="rpt_table">
				...
				foreach($result as $row)
				{
					...
					if ($row[csf("total_qnty")]>0)
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>">
							<td style="font-size:12px;"><? echo $i;  ?></td>
							<td style="font-size:12px;"><? echo $order_job_arr[$row[csf("po_break_down_id")]]['job_no_mst']; ?></td>
							<td style="font-size:12px;"><p><? echo $buyer_library[$order_job_arr[$row[csf("po_break_down_id")]]['buyer_name']]; ?>&nbsp;</p></td>
							<td style="font-size:12px;"><p><? echo $order_job_arr[$row[csf("po_break_down_id")]]['po_number']; ?>&nbsp;</p></td>
							<td style="font-size:12px;"><p><? echo $order_job_arr[$row[csf("po_break_down_id")]]['style_ref_no']; ?>&nbsp;</p></td>
							<td style="font-size:12px;"><? echo $garments_item[$order_job_arr[$row[csf("po_break_down_id")]]['gmts_item_id']]; ?></td>
							<td style="font-size:12px;"><? echo $country_library[$order_job_arr[$row[csf("po_break_down_id")]]['country_id']]; ?></td>
							<td style="font-size:12px;"><? echo $color_library[$order_job_arr[$row[csf("po_break_down_id")]]['color_number_id']]; ?></td>
							<td style="font-size:12px;"><p>
							<?
							$invoice_id="";
							$invoice_id_arr=array_unique(explode(",",$row[csf("invoice_no")]));
