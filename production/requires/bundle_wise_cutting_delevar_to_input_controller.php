...
if($action=="company_wise_report_button_setting")
{
	...
	$print_report_format=return_field_value("format_id"," lib_report_template","template_name ='".$data."'  and module_id=7 and report_id=284 and is_deleted=0 and status_active=1");
...
if ($action == "load_drop_down_embro_issue_source") {
    ...
    {
        if ($db_type == 0) {
             echo create_drop_down("cbo_emb_company", 180, "select id,supplier_name from lib_supplier where status_active=1 and is_deleted=0 and find_in_set(22,23,party_type) order by supplier_name", "id,supplier_name", 1, "--- Select ---", $selected, "get_php_form_data(this.value,'load_variable_settings_for_working_company','requires/bundle_wise_cutting_delevar_to_input_controller');");
        }
        else
        {
            echo create_drop_down("cbo_emb_company", 180, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type in(22,23) and a.status_active=1 group by a.id,a.supplier_name order by a.supplier_name", "id,supplier_name", 1, "--Select--", 0, "get_php_form_data(this.value,'load_variable_settings_for_working_company','requires/bundle_wise_cutting_delevar_to_input_controller');load_html();");
        }
    }
    else if ($data == 1)
    {
...
if ($action=="create_booking_search_list_view")
{
    ...
    if (!$data[4] && !$data[5] && !$data[8] && !$data[10] && !$data[11] && !$data[12] && !($data[2] && $data[3])) 
    {
        echo "<h3 align='center' style='color:#dc3545 !important';'>Please Enter Job No or Order No or Internal Ref. or File No or Style No. or WO No or Date Range <h3>"; die;
    }
...
if($action=="create_bundle_rescan_search_list_view")
{
    ...
    if (trim($cut_no) == '' ) 
    {
        echo "<h2 style='color:#D00; text-align:center;'> Please Enter Cut No </h2>";
        exit();
    }
    ...
    $last_operation=gmt_production_validation_script( 9, 1,'',$cutting_no, $production_squence);
...
if ($action == "bundle_popup") {
    ...
    <body>
    <div align="center" style="width:100%;height:320px;">
        <form name="searchwofrm" id="searchwofrm">
            <fieldset style="width:810px;">
...
if ($action == "create_bundle_search_list_view") {
    ...
    $syear = substr($ex_data[6],2); 
    $is_exact=$ex_data[7];
    $job_year=substr($ex_data[10],2); 
    
    if (!trim($style_no) == '' && !trim($cut_no) == '' ) 
    {
        echo "<h2 style='color:#D00; text-align:center;'>Please Enter Style or Cut No</h2>";
        exit();
    }
   ...
   // $scanned_bundle_arr = return_library_array("select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=4 and  b.status_active=1 and b.is_deleted=0", 'bundle_no', 'bundle_no');
    ...
    $tmp_job=trim($company_short_arr[$company].'-'.$job_year.'-'.str_pad($job_no,5,"0",STR_PAD_LEFT));
    if ($cut_no != '')
     {
        if($is_exact=='true')
        {
           ...
           // $cutCon_a = " and a.cut_no = '".trim($company_short_arr[$company].'-'.$syear.'-'.str_pad($cut_no,6,"0",STR_PAD_LEFT))."'";
            ...
            $cut_job_no=return_field_value("job_no","ppl_cut_lay_mst","cutting_no='$cut_no'","job_no");
        }
        else
        {
            $cutCon = " and c.cut_no = '$tmp_cut'";
            $cutCon_a = " and b.cut_no = '$tmp_cut'";
            $cut_job_no=return_field_value("job_no","ppl_cut_lay_mst","cutting_no='$tmp_cut'","job_no");
        }
    ...
    {
        if($is_exact=='true') $jobCon=" and f.job_no = '$job_no'";
        else  $jobCon=" and f.job_no ='$tmp_job'";
    ...
    if($style_no!='')
    {
        ...
        $styleCon=" and f.style_ref_no = '$style_no'";
    ...
    if($bndl_no)
    {
        // if($is_exact=='true') $bndlCon=" and c.bundle_no = '$bndl_no'";
        // else  $bndlCon=" and c.bundle_no like '%$bndl_no%'";
        $bndlCon=" and c.bundle_no = '$bndl_no'";
    ...
    if($job_year)
    {
        $year_cond .= " and d.job_no_mst like '%-$job_year-%' ";
    }

    

    if(trim($job_no)!='' || trim($ex_data[0])!="" || $style_no !="" || $cut_job_no !="")
    {
        if(trim($job_no)!='')
        {
            if($is_exact=='true')
        ...
        if(trim($cut_job_no)!='')
        {
            $jobnoCond = " and a.job_no='$cut_job_no'";
        }
        if(trim($ex_data[0])!='')
        {
        ...
        if(trim($style_no)!='')
        {
            $styleCond = " and a.style_ref_no='$style_no'";
        }
        $sql = "SELECT b.id from wo_po_details_master a,wo_po_break_down b where a.id=b.job_id and a.status_active=1 and b.status_active=1 $jobnoCond $poCond $styleCond";
        // echo $sql;die;
        $res = sql_select($sql);
        $po_id_arr = array();
        foreach ($res as $val)
  ...
  /*  $scanned_bundle_arr = return_library_array("SELECT bundle_no, bundle_no from pro_garments_production_dtls where production_type=9 and cut_no='".$tmp_cut."' and status_active=1 and is_deleted=0", 'bundle_no', 'bundle_no');
    ...
    $scanne=sql_select( "SELECT b.bundle_no, sum(b.production_qnty) as production_qnty,a.sewing_line from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=9  and b.status_active=1 and b.is_deleted=0  $po_id_conds $cutCon_a group by b.bundle_no,a.sewing_line");
    ...
    <div style="width:1020px; max-height:350px; overflow-y:auto" id="list_container_batch" align="left">    
        ...
        <?
            ...
            foreach($last_operation as  $item_id=>$operation_cond)
            {
                ...
                {
                    // echo "SELECT c.bundle_no, c.reject_qty, c.alter_qty, c.spot_qty, c.replace_qty  from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e,wo_po_details_master f where d.job_id=f.id and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and a.company_id=$company  and c.production_type in (3)  $orderCon $bndlCon  $jobCon $styleCon $cutCon $year_cond and a.status_active=1 and a.is_deleted=0";
                    $sqld = sql_select( "SELECT c.bundle_no, c.reject_qty, c.alter_qty, c.spot_qty, c.replace_qty  from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e,wo_po_details_master f where d.job_id=f.id and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and a.company_id=$company  and c.production_type in (3)  $po_id_conds $orderCon $bndlCon  $jobCon $styleCon $cutCon $year_cond and a.status_active=1 and a.is_deleted=0" );
                    foreach($sqld as $arows)
                    {
                        $reject_qty[$arows[csf('bundle_no')]]+=$arows[csf('reject_qty')];
                        $alter_qty[$arows[csf('bundle_no')]]+=$arows[csf('alter_qty')];
                ...
                foreach (sql_select($sql) as  $v) 
                {
                    ...
                    $item_id_array[$v['ITEM_NUMBER_ID']]=$v['ITEM_NUMBER_ID'];
                    $color_id_array[$v['color_id_array']]=$v['COLOR_NUMBER_ID'];

                }
                //echo "<pre>"; print_r($item_id_array);die;
                $po_id_con=implode(",",$po_id_array);
                $item_id_con=implode(",",$item_id_array);
                $color_id_con=implode(",",$color_id_array);

                $order_cut_sql=sql_select("SELECT a.order_id,b.color_id,b.order_cut_no,b.gmt_item_id
                from ppl_cut_lay_bundle a, ppl_cut_lay_dtls b
                where a.dtls_id=b.id and a.status_active=1 and b.status_active=1  and a.order_id in ($po_id_con)and b.gmt_item_id in ($item_id_con) and b.color_id in ($color_id_con)"); 
                // echo $order_cut_sql;die;

               $order_cut_no_arr=array();
               foreach($order_cut_sql as $r)
               {
                $order_cut_no_arr[$r['ORDER_ID']][$r['GMT_ITEM_ID']][$r['COLOR_ID']]['order_cut_no']=$r['ORDER_CUT_NO'];
               }
                //echo "<pre>"; print_r($order_cut_no_arr);die;
                ...
                {  
                    ...
                    // echo "((".$row[csf('qty')].") ) - (".$reject_qty[$row[csf('bundle_no')]] ."+". $alter_qty[$row[csf('bundle_no')]]."+". $spt_qty[$row[csf('bundle_no')]].")<br>";
                    ...
                    ?>
                        <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value(<? echo $i; ?>)"> 
                            ...
                            <td width="50"><p><? echo $size_arr[$row[csf('size_number_id')]]; ?></p></td>
                            <td width="70"><? echo $row[csf('cut_no')]; ?></td>
                            <td width="70"><? echo $order_cut_no_arr[$row[csf('po_break_down_id')]][$row[csf('item_number_id')]][$row[csf('color_number_id')]]['order_cut_no']; ?></td>
                            <td width="80"><? echo $row[csf('bundle_no')]; ?></td>
                            <td align="right"><? echo $row[csf('qty')]; ?>&nbsp;&nbsp;</td>
                        </tr>
                        <?
...
if ($action == "create_bundle_search_list_view_bk") {
    $ex_data = explode("_", $data);
    $txt_order_no = "%" . trim($ex_data[0]) . "%";
    $company = $ex_data[1];
    //$bundle_no = "%".trim($ex_data[2])."%";
   ...
   // $scanned_bundle_arr = return_library_array("select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=4 and  b.status_active=1 and b.is_deleted=0", 'bundle_no', 'bundle_no');
    ...
    $tmp_cut=trim($company_short_arr[$company].'-'.$syear.'-'.str_pad($cut_no,6,"0",STR_PAD_LEFT));
    $tmp_job=trim($company_short_arr[$company].'-'.$syear.'-'.str_pad($job_no,5,"0",STR_PAD_LEFT));
    if ($cut_no != '')
     {
        if($is_exact=='true')
        {
  ...
  /*  $scanned_bundle_arr = return_library_array("SELECT bundle_no, bundle_no from pro_garments_production_dtls where production_type=9 and cut_no='".$tmp_cut."' and status_active=1 and is_deleted=0", 'bundle_no', 'bundle_no');
    ...
    <div style="width:950px; max-height:350px; overflow-y:auto" id="list_container_batch" align="left">
        ...
        <?
            ...
            foreach($last_operation as  $item_id=>$operation_cond)
            {
                ...
                // echo $last_operation_string;die;
                $sql="SELECT c.cut_no,d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, f.style_ref_no, c.bundle_no, SUM (c.production_qnty) as qty, sum(c.replace_qty) as replace_qty, e.po_number,c.barcode_no from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e,wo_po_details_master f where d.job_id=f.id and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and a.company_id=$company $orderCon $bndlCon  $item_id $jobCon $styleCon $cutCon $year_cond and a.status_active=1 and a.is_deleted=0 $operation_conds group by c.cut_no, f.style_ref_no, c.bundle_no, d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.bundle_no, e.po_number,c.barcode_no order by  f.style_ref_no, c.cut_no,length(c.bundle_no) asc, c.bundle_no asc";
...
if ($action == "populate_bundle_data")
{
    ...
    if ($db_type == 0) {
        ...
        $sql="SELECT max(c.id) as prdid, d.id as colorsizeid, d.po_break_down_id as po_id, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.cut_no,c.bundle_no,sum(c.production_qnty) as production_qnty,c.barcode_no from pro_garments_production_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d where a.id=c.mst_id and c.color_size_break_down_id=d.id and  a.company_id='$ex_data[3]'  and a.PRODUCTION_TYPE=1 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and d.status_active=1 and d.is_deleted=0  $bundle_nos_cond $item_id $operation_conds  group by d.id, d.po_break_down_id, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no,c.barcode_no order by c.cut_no, length(c.bundle_no) asc, c.bundle_no asc";
     ...
     //c.delivery_mst_id =a.delivery_mst_id
        ...
        $order_sql = "SELECT a.id as job_id ,a.job_no_prefix_num, $year_field as year, a.buyer_name,b.po_number,b.id from wo_po_details_master a,wo_po_break_down b where a.id=b.job_id and b.status_active=1 and b.id in($po_ids)";
        // echo $order_sql;die;
    ...
    // $is_print_emb = return_field_value("EMB_NAME", "WO_PRE_COST_EMBE_COST_DTLS", "1=1 and job_id=$job_id and EMB_NAME in(1,2)");
    $qty_source = 3;
    /* if($is_print_emb!="")
    {
        $qty_source = 3;
    ...
    foreach($last_operation as  $item_id=>$operation_cond)
    {
        ...
        {
            $sqld = sql_select( "SELECT  c.bundle_no, c.reject_qty, c.alter_qty, c.spot_qty, c.replace_qty  from pro_garments_production_mst a, pro_garments_production_dtls c where a.id=c.mst_id  and a.company_id=$ex_data[3] and c.production_type in($qty_source) and c.status_active=1 and c.is_deleted=0 $bundle_nos_cond" );
            foreach($sqld as $arows)
            {
                $reject_qty[$arows[csf('bundle_no')]]+=$arows[csf('reject_qty')];
                $alter_qty[$arows[csf('bundle_no')]]+=$arows[csf('alter_qty')];
        ...
        foreach ($result as $row)
        {
            ...
            if(trim($scanned_bundle_arr[$row[csf('bundle_no')]])=="")
            {
                ...
                // echo "(".$row[csf('production_qnty')]."+".$replace_qty[$row[csf('bundle_no')]] .") - (".$reject_qty[$row[csf('bundle_no')]] ."+ ".$alter_qty[$row[csf('bundle_no')]]."+". $spt_qty[$row[csf('bundle_no')]].")<br>";
...
if ($action == "create_challan_search_list_view") {
    ...
    if (!$challan && !$issue_date && !$order_no && !$cutting_no) 
    {
        echo "<h3 align='center' style='color:#dc3545 !important';'>Please Enter Order No or Challan No or Cutting No or Input Date <h3>"; die;
    } 
    //echo $company_id;die;
...
if($action=="sewing_input_challan_print")
{
    ...
    $line_library = return_library_array("select id,line_name from lib_sewing_line where status_active=1 and is_deleted=0", "id", "line_name");
    $buyer_arr=return_library_array( "select id, buyer_name from lib_buyer where status_active=1 and is_deleted=0",'id','buyer_name');
    $buyer_code_arr=return_library_array( "select id, remark from lib_buyer where status_active=1 and is_deleted=0",'id','remark');
    $body_part_arr=return_library_array( "select id,bundle_use_for from ppl_bundle_title where company_id=$data[0]",'id','bundle_use_for');
	...
	<div style="width:900px;">
	    <table cellspacing="0" style="font: 11px tahoma; width: 100%;">
	     ...
	     <tr>
		   ...
		   /* $sqls="SELECT b.cut_no, b.production_qnty, a.country_id, d.color_number_id, d.size_number_id, e.style_ref_no, e.style_description, e.job_no, e.buyer_name,f.po_number,f.id as po_id, f.po_quantity,h.order_cut_no,b.bundle_no
			...
			$sqls="SELECT a.item_number_id, b.cut_no, b.production_qnty, a.country_id, d.color_number_id, d.size_number_id, e.style_ref_no, e.style_description, e.job_no, e.buyer_name,f.po_number,f.id as po_id, f.po_quantity,b.bundle_no,f.grouping from  pro_garments_production_mst a, pro_garments_production_dtls b,wo_po_color_size_breakdown d, wo_po_details_master e,wo_po_break_down f  where
		...
		$order_cut_no_sql=sql_select("select  b.order_cut_no as ord_cut, a.batch_id, a.cutting_no from ppl_cut_lay_mst a, ppl_cut_lay_dtls b where a.id=b.mst_id and a.cutting_no in (".$cut_number_string.")");
		$batchArr = array();
		$order_cut_no_arr = array();
		foreach($order_cut_no_sql as $rows)
		{
			 ...
			 {
				 $order_cut_no_arr[$rows[csf("cutting_no")]]=$rows[csf("ord_cut")];
			 }
			 else
			 {
				$po_ids=$rows[csf("po")];
	    ...
	    <table align="left" cellspacing="0" width="<? echo $table_width; ?>"  border="1" rules="all" class="rpt_table" style=" margin-top:20px;" >
	        <thead bgcolor="#dddddd" align="center">
	            <tr>
                    ...
                    <th width="80" align="center" rowspan="2">IR/IB</th>
	            ...
	            foreach($dataArr as $key=>$row)
	            {
	                ...
	                $bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF";
	                    ...
	                    <tr bgcolor="<? echo $bgcolor; ?>"  style="font-size:12px">
	                        ...
	                        <td align="center"><? echo $order_cut_no_arr[$row['cut_no']]; ?></td>
	                        <?
...
if($action=="sewing_input_challan_print_5")
{
    ...
    $sql="select id, sys_number_prefix, sys_number_prefix_num, sys_number, company_id, production_type, location_id, delivery_basis, embel_name, embel_type,
    production_source, serving_company, floor_id, sewing_line, organic, delivery_date,working_company_id,working_location_id,inserted_by from pro_gmts_delivery_mst where production_type=9 and id='$data[1]' and
    status_active=1 and is_deleted=0 ";
	...
	<div style="width:900px;">
	    ...
	    <table align="left" cellspacing="0" width="<? echo $table_width; ?>"  border="1" rules="all" class="rpt_table" style=" margin-top:20px;" >
	     ...
	     <?
	        ...
	        echo signature_table(301, $data[0], "900px",$cbo_template_id,40,$user_lib_name[$inserted_by]);
	        ...
	        <tbody>
	            ...
	            foreach($dataArr as $key=>$row)
	            {
	                ...
	                $bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF";
	                    ...
	                    <tr bgcolor="<? echo $bgcolor; ?>"  style="font-size:12px">
	                        <td><? echo $i;  ?></td>
	                        <td align="center" title="<? echo $buyer_arr[$row['buyer_name']];?>"><? echo $buyer_arr[$row['buyer_name']]; ?></td>
	                        <td align="center"><? echo $row['job_no']; ?></td>
	                        <td align="center"><? echo $row['style_ref_no']; ?></td>
	                        <td align="center"><? echo $row['po_number'];?></td>
	                        <td align="center"><? echo $garments_item[$row['item_number_id']];?></td>
