...
if ($action=="load_drop_down_location")
{
	...
	if(count($locationArr)==1)
	{
		...
		echo create_drop_down( "cbo_location", 152, "select id,location_name from lib_location where company_id='$data' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", $key, "load_drop_down('requires/woven_fabric_requisition_for_cutting_entry_tg_controller','".$data."_'+this.value, 'load_drop_down_production_floor', 'floor_td');" );
		exit();
	}
	else
	{
		echo create_drop_down( "cbo_location", 152, "select id,location_name from lib_location where company_id='$data' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", 0, "load_drop_down('requires/woven_fabric_requisition_for_cutting_entry_tg_controller','".$data."_'+this.value, 'load_drop_down_production_floor', 'floor_td');" );
			...
			<legend>Enter search words</legend>           
	            <table cellpadding="0" cellspacing="0" border="1" rules="all" width="1100" class="rpt_table">
	                ...
	                <tr class="general">
	                	<td>
	                    	<?
								echo create_drop_down( "cbo_buyer_name", 150, "select buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$company_id' $buyer_cond and buy.id in (select buyer_id from lib_buyer_party_type where party_type in (1,3,21,90)) order by buy.buyer_name","id,buyer_name", 1, "-- All Buyer --", $selected, "load_drop_down( 'woven_fabric_requisition_for_cutting_entry_tg_controller', this.value+'*'+1, 'load_drop_down_season', 'season_td'); load_drop_down( 'woven_fabric_requisition_for_cutting_entry_tg_controller', this.value+'*'+1, 'load_drop_down_brand', 'brand_td');",$data[0] ); 
							?>       
	                    </td>
...
if($action=="create_fabric_search_list_view")
{
	...
	if($hdn_variable_setting_status==1)// style wise
	{
		...
		else // color and size level
		{
			...
			from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, wo_pre_cos_fab_co_avg_con_dtls d, wo_pre_cost_fabric_cost_dtls e
			where a.id=b.job_id and b.id=c.po_break_down_id and c.po_break_down_id=d.po_break_down_id and c.color_number_id=d.color_number_id and c.size_number_id=d.gmts_sizes and d.pre_cost_fabric_cost_dtls_id=e.id and c.job_no_mst=e.job_no and c.item_number_id=e.item_number_id and a.is_deleted=0 and a.status_active=1 and b.is_confirmed=1 and a.company_name=$company_id $buyer_id_cond $job_no_cond $year_cond $po_cond $int_ref_cond $batch_cond $style_ref_cond $brand_no_cond $season_no_cond $season_year_cond
			group by  a.buyer_name,a.job_no,a.style_ref_no, c.item_number_id, e.body_part_id, e.lib_yarn_count_deter_id, c.color_number_id, nvl(c.size_number_id,0),e.uom, e.avg_finish_cons ,a.insert_date,b.id,b.grouping, e.gsm_weight, d.dia_width,e.color_size_sensitive");
			foreach($sql_po_wise as $row)
			{
			...
			if($cutting_chk=='true') // when check box checked
			{
				...
				from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, wo_pre_cos_fab_co_avg_con_dtls d, wo_pre_cost_fabric_cost_dtls e
				where a.id=b.job_id and b.id=c.po_break_down_id and c.po_break_down_id=d.po_break_down_id and c.color_number_id=d.color_number_id and c.size_number_id=d.gmts_sizes and d.pre_cost_fabric_cost_dtls_id=e.id and c.job_no_mst=e.job_no and c.item_number_id=e.item_number_id and a.is_deleted=0 and a.status_active=1 and b.is_confirmed=1 and a.company_name=$company_id $buyer_id_cond $job_no_cond $year_cond $po_cond $int_ref_cond $batch_cond $style_ref_cond $brand_no_cond $season_no_cond $season_year_cond and c.color_number_id in($color_id) and c.item_number_id in($gmt_item_id) and c.po_break_down_id in($order_ids)
				group by  a.buyer_name,a.job_no, a.job_no_prefix_num,a.style_ref_no, c.item_number_id, e.body_part_id, e.lib_yarn_count_deter_id, c.color_number_id, nvl(c.size_number_id,0),e.uom, e.avg_finish_cons ,a.insert_date,b.grouping, e.gsm_weight, d.dia_width,e.color_size_sensitive";
			}
			else
			{
				$sql = "SELECT a.buyer_name,a.job_no, a.job_no_prefix_num,a.style_ref_no, c.item_number_id as item_id, e.body_part_id, e.lib_yarn_count_deter_id, c.color_number_id as color_id, NVL(c.size_number_id,0) as size_id,e.uom, e.avg_finish_cons as avg_cons ,sum((d.cons/d.pcs)*c.plan_cut_qnty) as budget_qty, TO_CHAR(a.insert_date,'YYYY') as year ,LISTAGG(b.id,',') WITHIN GROUP (ORDER BY b.id) as po_id ,LISTAGG(b.po_number,',') WITHIN GROUP (ORDER BY b.po_number) as po_number ,b.grouping, e.gsm_weight, d.dia_width,e.color_size_sensitive 
	...
	else // po wise
	{
		if($hdn_variable_colorSize_status==2)
		{
			if($cutting_chk=='true') // when check box checked
			{
				...
				$sql = "SELECT a.job_no, a.job_no_prefix_num, a.buyer_name, $year, b.id as po_id, b.po_number,b.grouping, c.item_number_id as item_id, c.color_number_id as color_id, e.body_part_id, e.lib_yarn_count_deter_id, e.gsm_weight, d.dia_width, sum((d.cons/d.pcs)*c.plan_cut_qnty) as budget_qty,e.uom, e.avg_finish_cons as avg_cons,a.style_ref_no,e.color_size_sensitive
				from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, wo_pre_cos_fab_co_avg_con_dtls d, wo_pre_cost_fabric_cost_dtls e
				where a.id=b.job_id and b.id=c.po_break_down_id and c.po_break_down_id=d.po_break_down_id and c.color_number_id=d.color_number_id and c.size_number_id=d.gmts_sizes and d.pre_cost_fabric_cost_dtls_id=e.id and c.job_no_mst=e.job_no and c.item_number_id=e.item_number_id and a.is_deleted=0 and a.status_active=1 and b.is_confirmed=1 and a.company_name=$company_id $buyer_id_cond $job_no_cond $year_cond $po_cond $int_ref_cond $batch_cond $style_ref_cond $brand_no_cond $season_no_cond $season_year_cond and c.color_number_id in($color_id) and c.item_number_id in($gmt_item_id) and c.po_break_down_id in($order_ids)
				group by a.job_no, a.job_no_prefix_num, a.buyer_name,a.insert_date, b.id, b.po_number,b.grouping, c.item_number_id, c.color_number_id, e.body_part_id, e.lib_yarn_count_deter_id, e.gsm_weight, d.dia_width,e.uom, e.avg_finish_cons,a.style_ref_no,e.color_size_sensitive";
			}
			else
			{
		...
		else
		{
			if($cutting_chk=='true') // when check box checked
			{
				...
				from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, wo_pre_cos_fab_co_avg_con_dtls d, wo_pre_cost_fabric_cost_dtls e
				where a.id=b.job_id and b.id=c.po_break_down_id and c.po_break_down_id=d.po_break_down_id and c.color_number_id=d.color_number_id and c.size_number_id=d.gmts_sizes and d.pre_cost_fabric_cost_dtls_id=e.id and c.job_no_mst=e.job_no and c.item_number_id=e.item_number_id and a.is_deleted=0 and a.status_active=1 and b.is_confirmed=1 and a.company_name=$company_id $buyer_id_cond $job_no_cond $year_cond $po_cond $int_ref_cond $batch_cond $style_ref_cond $brand_no_cond $season_no_cond $season_year_cond and c.color_number_id in($color_id) and c.item_number_id in($gmt_item_id) and c.po_break_down_id in($order_ids)
				group by a.job_no, a.job_no_prefix_num, a.buyer_name,a.insert_date, b.id, b.po_number,b.grouping, c.item_number_id, c.color_number_id, nvl(c.size_number_id,0), e.body_part_id, e.lib_yarn_count_deter_id, e.gsm_weight, d.dia_width,e.uom, e.avg_finish_cons,a.style_ref_no,e.color_size_sensitive";
			}
			else
			{
				$sql = "SELECT a.job_no, a.job_no_prefix_num, a.buyer_name, $year, b.id as po_id, b.po_number,b.grouping, c.item_number_id as item_id, c.color_number_id as color_id, $null_cond(c.size_number_id,0) as size_id, e.body_part_id, e.lib_yarn_count_deter_id, e.gsm_weight, d.dia_width, sum((d.cons/d.pcs)*c.plan_cut_qnty) as budget_qty,e.uom, e.avg_finish_cons as avg_cons,a.style_ref_no,e.color_size_sensitive
	...
	<div style="width:1350px; max-height:250px; overflow-y:scroll" id="list_container_batch" align="left">	 
		...
		<? 
			...
			{  
				...
				$balance_qty=$reqnQty-$prevRcvQnty;
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		if($hdn_variable_setting_status==1)
		{
			$field_array_dtls="id, mst_id, buyer_id, po_id, job_no, item_id, body_part, determination_id, gsm, dia, color_id,fab_color_id, size_id,fabric_ref,weight_type,cutable_width,rd_no,style_ref_no,cons,reqn_qty, inserted_by, insert_date";
			for($j=1;$j<=$tot_row;$j++)
			{ 
				...
				foreach ($hdnPoRatioQtyArr as $poData) 
				{
					...
					$data_array_dtls.="(".$dtls_id.",".$id.",".$$buyerId.",'".$hdnPoAndQtyArr[0]."','".$$jobNo."','".$$itemId."','".$$bodyPartId."','".$$deterId."','".$$gsm."','".$$dia."','".$$colorId."','".$$fabColorId."','".$$sizeId."','".$$fabricRef."','".$$weightType."','".$$cutableWidth."','".$$rdNo."','".$$styleRef."','".$$cons."','".$hdnPoAndQtyArr[1]."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
					$dtls_id = $dtls_id+1;
...
if($action=='populate_data_from_requisition')
{
	...
	{ 
		...
		echo "load_drop_down( 'requires/woven_fabric_requisition_for_cutting_entry_tg_controller', '".$row[csf("working_company")]."', 'load_drop_down_location','location_td');\n";
		echo "load_drop_down( 'requires/woven_fabric_requisition_for_cutting_entry_tg_controller', '".$row[csf("working_company")]."_".$row[csf("location_id")]."', 'load_drop_down_production_floor','floor_td');\n";
...
{	
	...
	else
	{
		if($data[2]==2)
		{
			$sql="SELECT id, buyer_id, po_id, job_no, item_id, body_part, determination_id, gsm, dia, color_id,fab_color_id, reqn_qty,style_ref_no,cons from pro_fab_reqn_for_cutting_dtls where status_active=1 and is_deleted=0 and mst_id=$data[0]";
		}
		else
		{
			$sql="SELECT id, buyer_id, po_id, job_no, item_id, body_part, determination_id, gsm, dia, color_id,fab_color_id, size_id, reqn_qty,style_ref_no,cons from pro_fab_reqn_for_cutting_dtls where status_active=1 and is_deleted=0 and mst_id=$data[0]";
		}
	}
    ...
    $fabric= new fabric($condition);
    $fabric_req_arr=$fabric->getQtyArray_by_OrderBodypartDeterminIdAndGmtscolor_knitAndwoven_greyAndfinish();
    // $fabric_req_arr=$fabric->getQtyArray_by_JobColorBodyTypeDeterminIdGsmAndDiaWidth_knitAndwoven_greyAndfinish();
    // echo "<pre>";print_r($fabric_req_arr);die();
	...
	// echo "<pre>";print_r($budget_qty_array);die;
	...
	foreach($result as $row)
	{
		...
		if($data[1]==1)
		{
			...
			if($data[2]==2)
			{
				...
				$prevRcvQnty = $prevRcvQntyArr[$row[csf('buyer_id')]][$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part')]][$row[csf('determination_id')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('color_id')]][$val[csf('fabric_ref')]][$val[csf('rd_no')]][$val[csf('cutable_width')]][$val[csf('weight_type')]][$row[csf('cons')]];
				//echo $prevRcvQnty."=";
			}
			else
			{
				...
				$prevRcvQnty = $prevRcvQntyArr[$row[csf('buyer_id')]][$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part')]][$row[csf('determination_id')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('color_id')]][$row[csf('size_id')]][$val[csf('fabric_ref')]][$val[csf('rd_no')]][$val[csf('cutable_width')]][$val[csf('weight_type')]][$row[csf('cons')]];

			}
			//echo $budget_qty ."-". $prevRcvQnty."=";
			...
			foreach ($po_arr_uniq as $poID) 
			{
				...
				//echo $poID."##"."(".$orderRequiredQnty."/".$budget_qty.")*".$row[csf('reqn_qty')].",";
		...
		else
		{
			...
			if($data[2]==2)
			{
				...
				$fabric_req_knit=array_sum($fabric_req_arr['knit']['finish'][$row['PO_ID']][$row['BODY_PART']][$row['DETERMINATION_ID']][$row['FAB_COLOR_ID']]);
				$fabric_req_wov=array_sum($fabric_req_arr['woven']['finish'][$row['PO_ID']][$row['BODY_PART']][$row['DETERMINATION_ID']][$row['FAB_COLOR_ID']]);
				$budget_qty = $fabric_req_knit+$fabric_req_wov; 
				// echo $row['PO_ID']."=".$row['BODY_PART']."=".$row['DETERMINATION_ID']."=".$row['COLOR_ID']."<br>";
