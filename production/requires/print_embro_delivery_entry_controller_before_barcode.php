...
if($action=="create_po_search_list_view")
{
	...
	if(trim($company)!="") $sql_cond .= " and a.company_name='$company'";
 	...
 	$sql = "select b.id, a.order_uom, a.buyer_name, a.company_name, a.total_set_qnty, a.set_break_down, a.job_no, a.style_ref_no, a.gmts_item_id, a.location_name, b.shipment_date, b.po_number, b.po_quantity, b.plan_cut
			...
			where
			a.job_no = b.job_no_mst and a.job_no = c.job_no and a.status_active=1 and  a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.garments_nature=$garments_nature and c.emb_name=1
...
if($action=="create_bundle_rescan_search_list_view")
{
	...
	$sql_scan_bundle=sql_select(" select b.bundle_no,sum(b.production_qnty) as production_qnty from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=2 and a.embel_name=1 and b.status_active=1 and b.is_deleted=0 $not_null_bundle group by b.bundle_no");
	foreach($sql_scan_bundle as $val)
	{
		$scanned_bundle_qty_arr[$val[csf('bundle_no')]]=$val[csf('production_qnty')];
		$scanned_bundle_arr[]=$val[csf('bundle_no')];
	}
	
	if($ex_data[0]=='' && $ex_data[2]=='' && $ex_data[5]=='' && $ex_data[4]=='' ){echo "<h2 style='color:#D00; text-align:center;'>Please Select- Job No or Order No or Cut No or Bundle No</h2>";exit();}
	...
	{
		// Pre-cost
		$full_job_no=return_field_value("b.job_no"," wo_po_details_master b"," b.job_no_prefix_num=$job_no
and b.company_name=$company and extract(year from b.insert_date)=$ex_data[6] and b.status_active=1 and b.is_deleted=0","job_no");

	  	$precost_job=return_field_value("a.job_no","wo_pre_cost_mst a","a.job_no='".$full_job_no."' and a.status_active=1 and a.is_deleted=0 ","job_no");
		if(!empty($precost_job))
		{
			$sql_extrawork=sql_select("select b.id,b.emb_name,a.item_number_id from wo_pre_cos_emb_co_avg_con_dtls a,wo_pre_cost_embe_cost_dtls b  where a.job_no=b.job_no and b.emb_name in (1,2,4) and b.job_no='".$precost_job."' and a.requirment>0  and b.status_active=1 and b.is_deleted=0 group by b.emb_name,b.id,a.item_number_id order by b.id desc");
			...
			if(count($sql_extrawork)>0)
			{
				...
				{
					$i++;
					if( $lst[$row[csf("item_number_id")]]==1 )
						$first_op_arr[$row[csf("item_number_id")]]=$row[csf("emb_name")]; 
					$lst[$row[csf("item_number_id")]]=$row[csf("emb_name")];
				}
				
				$lemb=''; $last_operation=array();
				foreach($first_op_arr as $item=>$emb)
				{
					
					if( $emb==0 ) 
						$last_operation[$item]=" and c.production_type=1";
					else if( $emb==1 ) 
						$last_operation[$item]=" and c.production_type=1";
					else if( $emb==2 ) 
						$last_operation[$item]=" and c.production_type=3 and a.embel_name=2";
					else if( $emb==4 ) 
						$last_operation[$item]=" and c.production_type=3  and a.embel_name=4";
				}
			}
		}
		else // job entry
		{
			$sql_extrawork=sql_select(" select gmts_item_id ,job_no,embelishment,printseq, embro,embroseq, spworks, spworksseq from wo_po_details_mas_set_details where  job_no='".$full_job_no."'");
			
			$last_operation=array();
			foreach($sql_extrawork as $val)
			{
				if($val[csf("embelishment")]==1)
				{
					...
					if($print_sequence==1)
						$last_operation[$val[csf("gmts_item_id")]]=" and c.production_type=1";
						
					else if($print_sequence>=1 && $val[csf("embro")]==1 && $val[csf("spworks")]==0)
					{
						if($emblishment_sequence<$print_sequence && $emblishment_sequence>0)
							$last_operation[$val[csf("gmts_item_id")]]=" and c.production_type=3 and a.embel_name=2";
						else
							$last_operation[$val[csf("gmts_item_id")]]=" and c.production_type=1";
					}
						
					else if($print_sequence>=1 && $val[csf("embro")]==0 && $val[csf("spworks")]==1) 
					{ 
						if($spwork_sequence<$print_sequence && $spwork_sequence>0)
							$last_operation[$val[csf("gmts_item_id")]]=" and c.production_type=3 and a.embel_name=4";
						else
							$last_operation[$val[csf("gmts_item_id")]]=" and c.production_type=1";
					}
					else if($print_sequence>=1 && $val[csf("spworks")]==1 && $val[csf("embro")]==1)
					{
						if($emblishment_sequence<$spwork_sequence)
							$last_operation[$val[csf("gmts_item_id")]]=" and c.production_type=3 and a.embel_name=4";
						
						else if($spwork_sequence<$emblishment_sequence)
							$last_operation[$val[csf("gmts_item_id")]]=" and c.production_type=3 and a.embel_name=2";
						else
							$last_operation[$val[csf("gmts_item_id")]]=" and c.production_type=1";
					}
	...
	{
		//select job_no from ppl_cut_lay_mst where cut_num_prefix_no=200 and company_id=1 and  extract(year from insert_date)=2017
		$full_job_no=return_field_value("job_no","ppl_cut_lay_mst","cut_num_prefix_no=$cut_no
and company_id=$company and extract(year from insert_date)=$ex_data[6] and status_active=1 and is_deleted=0 ","job_no");
		$precost_job=return_field_value("a.job_no","wo_pre_cost_mst a","a.job_no='".$full_job_no."' and a.status_active=1 and a.is_deleted=0 ","job_no");
		if(!empty($precost_job))
		{
			$sql_extrawork=sql_select("select b.id,b.emb_name,a.item_number_id from wo_pre_cos_emb_co_avg_con_dtls a,wo_pre_cost_embe_cost_dtls b  where a.job_no=b.job_no and b.emb_name in (1,2,4) and b.job_no='".$precost_job."' and a.requirment>0  and b.status_active=1 and b.is_deleted=0 group by b.emb_name,b.id,a.item_number_id order by b.id desc");
	...
	else if($ex_data[0]!='')// Order no
	{
		$full_job_no=return_field_value("a.job_no_mst"," wo_po_break_down a,wo_po_details_master b "," a.job_no_mst=b.job_no and a.po_number='".$ex_data[0]."' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.company_name=$company ","job_no_mst");
		//echo $full_job_no;die;
		$precost_job=return_field_value("a.job_no","wo_pre_cost_mst a","a.job_no='".$full_job_no."' and a.status_active=1 and a.is_deleted=0 ","job_no");
		if(!empty($precost_job))
		{
			$sql_extrawork=sql_select("select b.id,b.emb_name,a.item_number_id from wo_pre_cos_emb_co_avg_con_dtls a,wo_pre_cost_embe_cost_dtls b  where a.job_no=b.job_no and b.emb_name in (1,2,4) and b.job_no='".$precost_job."' and a.requirment>0  and b.status_active=1 and b.is_deleted=0 group by b.emb_name,b.id,a.item_number_id order by b.id desc");
	...
	else if($ex_data[2]!='')// Bundle no
	{
		$bundle_job=sql_select("select c.job_no, b.gmt_item_id from ppl_cut_lay_bundle a,ppl_cut_lay_dtls b,ppl_cut_lay_mst c where a.bundle_no='".$ex_data[2]."' and a.dtls_id=b.id and b.mst_id=c.id and a.mst_id=c.id");
		foreach($bundle_job as $b_val)
		{
			$full_job_no=$b_val[csf('job_no')];
			$gmt_item_id=$b_val[csf('gmt_item_id')];
		}
		
		$precost_item_cond=" and a.item_number_id='".$gmt_item_id."'" ;
		$job_item_cond=" and gmts_item_id='".$gmt_item_id."'" ;
		
		$precost_job=return_field_value("a.job_no","wo_pre_cost_mst a","a.job_no='".$full_job_no."' and a.status_active=1 and a.is_deleted=0 ","job_no");
		if(!empty($precost_job))
		{
			$sql_extrawork=sql_select("select b.id,b.emb_name,a.item_number_id from wo_pre_cos_emb_co_avg_con_dtls a,wo_pre_cost_embe_cost_dtls b  where a.job_no=b.job_no and b.emb_name in (1,2,4) and b.job_no='".$precost_job."' and a.requirment>0  and b.status_active=1 and b.is_deleted=0  $precost_item_cond group by b.emb_name,b.id,a.item_number_id order by b.id desc");
		...
		{
			$sql_extrawork=sql_select(" select gmts_item_id ,job_no,embelishment,printseq, embro,embroseq, spworks, spworksseq from wo_po_details_mas_set_details where  job_no='".$full_job_no."' $job_item_cond");
		...
		<legend>Enter search words <input type="checkbox" value="1" name="is_exact" id="is_exact" checked> is exact</legend>           
            <table cellpadding="0" cellspacing="0" width="550" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td><input type="text" name="bundle_no" id="bundle_no" style="width:120px" class="text_boxes" /></td>  		
            		<td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view (document.getElementById('txt_order_no').value+'_'+<? echo $company; ?>+'_'+document.getElementById('bundle_no').value+'_'+'<? echo trim($bundleNo,','); ?>'+'_'+document.getElementById('txt_job_no').value+'_'+document.getElementById('txt_cut_no').value+'_'+document.getElementById('cbo_cut_year').value+'_'+$('#is_exact').is(':checked'), 'create_bundle_search_list_view', 'search_div', 'print_embro_delivery_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1);reset_hide_field();')" style="width:100px;" />
                     </td>
                </tr>
           </table>
           <div style="width:100%; margin-top:5px; margin-left:10px" id="search_div" align="left"></div>
...
if($action=="challan_duplicate_check")
{
	...
	$msg=1;
	$result=sql_select("select a.sys_number,b.bundle_no from pro_gmts_delivery_mst a,pro_garments_production_dtls b where a.id=b.delivery_mst_id and b.bundle_no in ($bundle_no) and b.production_type=2 and a.status_active=1 and a.embel_name=1 and
	a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  group by a.sys_number,b.bundle_no");
...
if($action=="populate_bundle_data_update")
{
	...
	$sql="select d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, $year_field as year, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.cut_no,c.bundle_no,c.production_qnty,c.is_rescan, e.po_number from pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name='$ex_data[3]' and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.bundle_no in ($bundle_nos) and c.production_type=2 and c.delivery_mst_id=".$mst_id." and c.status_active=1 and c.is_deleted=0 ";
	//echo $sql;die;
	$result = sql_select($sql);
	$count=count($result);
	$i=$ex_data[1]+$count;
...
if($action=="populate_bundle_data")
{
	...
	if( $vscan=='Browse')
	{
		$source_cond_arr=explode(",",$source_cond);
		foreach($source_cond_arr as $single_cond)
		{
			$single_cond_arr=explode(":",$single_cond);
			$last_operation[$single_cond_arr[0]]=$single_cond_arr[1];
		}
	}
	else
	{
		
		$bundle_job=sql_select("select c.job_no, b.gmt_item_id from ppl_cut_lay_bundle a,ppl_cut_lay_dtls b,ppl_cut_lay_mst c where a.bundle_no=$bundle_nos and a.dtls_id=b.id and b.mst_id=c.id and a.mst_id=c.id");
		foreach($bundle_job as $b_val)
		{
			$full_job_no=$b_val[csf('job_no')];
			//$gmt_item_id=$b_val[csf('gmt_item_id')];
			if($gmt_item_id!="")$gmt_item_id.=",".$b_val[csf('gmt_item_id')];
			$gmt_item_id=$b_val[csf('gmt_item_id')];
		}
	...
	{
		//echo $item_id;die;
	  $sql="select d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, MAX($year_field) as year, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.cut_no,c.bundle_no, sum(c.production_qnty) as production_qnty, e.po_number from pro_garments_production_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name='$ex_data[3]' and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.bundle_no in ($bundle_nos) $operation_cond and c.status_active=1 and c.is_deleted=0 and d.item_number_id=$item_id group by d.id, e.id, f.job_no_prefix_num, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no, e.po_number";
...
if($action=="populate_bundle_data_rescan")
{
	...
	$scanned_bundle_arr=return_library_array( "select b.bundle_no, sum(b.production_qnty) as production_qnty from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=2 and a.embel_name=1 and b.status_active=1 and b.is_deleted=0 and b.bundle_no in($bundle_nos) group by b.bundle_no",'bundle_no','production_qnty');
...
{
	$bundle_nos=return_library_array( "select b.bundle_no, b.bundle_no from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id and a.delivery_mst_id=$data and b.production_type=2 and a.embel_name=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","bundle_no","bundle_no");
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$txt_system_id)=="")
		{
			...
			$new_sys_number=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'PER', date("Y",time()), 5, "select sys_number_prefix, sys_number_prefix_num from pro_gmts_delivery_mst where company_id=$cbo_company_name and production_type='2' and embel_name=$cbo_embel_name and $year_cond=".date('Y',time())." order by id desc ", "sys_number_prefix", "sys_number_prefix_num" ));
			...
			$data_array_delivery="(".$mst_id.",'".$new_sys_number[1]."','".$new_sys_number[2]."','".$new_sys_number[0]."', ".$cbo_company_name.",2,".$cbo_location.",".$delivery_basis.",".$cbo_embel_name.",".$cbo_embel_type.",".$cbo_source.",".$cbo_emb_company.",".$cbo_floor.",".$txt_organic.",".$txt_issue_date.",".$cbo_body_part.",".$cbo_working_company_name.",".$cbo_working_location.",".$user_id.",'".$pc_date_time."')";
		...
		if(str_replace("'","",$delivery_basis)==3)
		{
			...
			$field_array_dtls="id,delivery_mst_id,mst_id,production_type,color_size_break_down_id,production_qnty,cut_no,bundle_no,is_rescan";
			...
			foreach($dtlsArr as $bundle_no=>$qty)
			{
				...
				$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",2,'".$dtlsArrColorSize[$bundle_no]."','".$qty."','".$cut_no."','".$bundle_no."','".$bundleRescanArr[$bundle_no]."')";
				$dtls_id = $dtls_id+1;
			}
			...
			//echo $challanrID."**".$rID."**".$dtlsrID;die;
...
if($action=="create_challan_search_list_view")
{
 ...
 group by a.id, a.delivery_date, a.insert_date, a.sys_number_prefix_num, a.sys_number, a.embel_type, a.production_source, a.serving_company, a.location_id, a.floor_id, a.organic,b.cut_no	 order by a.id desc"; 
	//echo $sql;//die;
	$result = sql_select($sql);
...
{
	$data_array=sql_select("select id, company_id, sys_number, embel_type, embel_name, production_source, serving_company, location_id, floor_id, organic, delivery_date,body_part,working_company_id,working_location_id from pro_gmts_delivery_mst where id='$data'  and production_type=2");
...
if($db_type==2) $group_concat="  listagg(cast(b.cut_no AS VARCHAR2(4000)),',') within group (order by b.cut_no) as cut_no" ;
			...
			if($data[2]==3)
			{
				...
				if($db_type==0)
				{
					$sql="SELECT $group_concat,sum(b.production_qnty) as production_qnty,a.po_break_down_id,a.item_number_id,a.country_id, d.color_number_id,
			...
			else
			{
				if($db_type==0)
				{
					$sql="SELECT $group_concat,sum(a.production_qnty) as production_qnty,c.po_break_down_id,c.item_number_id,c.country_id, b.color_number_id 
            ...
            $tot_qnty=array();
                foreach($result as $val)
                {
                    ...
                    <tr bgcolor="<? echo $bgcolor; ?>"  style="font-size:12px">
                        ...
                        <td align="center"><? echo implode(',',array_unique(explode(',',$val[csf('cut_no')]))); ?></td>
