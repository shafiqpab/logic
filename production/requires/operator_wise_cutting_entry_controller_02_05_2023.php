...
{ 
	...
	$floor_cond=$data[2];
	$row=$data[3];
	echo create_drop_down( "cbotableName_$row", 100, "select id,table_name from lib_table_entry where company_name='$data[0]' and location_name='$data[1]' and floor_name='$data[2]'  and status_active =1 and is_deleted=0 and table_name is not null order by table_name","id,table_name", 1, "--Select Table--", $selected, "","","","","","");
	exit();	 
}
...
if($action=="today_cutting_popup")
{
	...
	<script>
		...
		function fn_close()
		{
			let tot_row = $("#tbl_list tbody tr").length;
			let data_string = "";
			let qty = 0;
			let cbofloorName=0;
			let cbotableName=0;
			...
			for (let i = 1; i <= tot_row; i++) 
			{
				cbofloorName = $('#cbofloorName_'+i).val();
				cbotableName = $('#cbotableName_'+i).val();
				cboprocess = $('#cboprocess_'+i).val();
				cboEmpName = $('#cboEmpName_'+i).val();
				qty = $('#qty_'+i).val()*1;
				//data_string=qty + emp_id;
				data_string +=  (data_string=="") ? cbofloorName+'**'+cbotableName+'**'+cboprocess+'**'+cboEmpName+'**'+qty : "__"+cbofloorName+'**'+cbotableName+'**'+cboprocess+'**'+cboEmpName+'**'+qty;
			}
			// alert(data_string);
			...
			if(rowNo==1 && numRow >1)
			{
				...
				for(i = rowNo;i <= numRow;i++)
				{
					$("#tbl_list tr:eq("+i+")").find("input,select").each(function() 
					{
						...
						$('#increaseset_'+i).removeAttr("onClick").attr("onClick","add_break_down_tr("+i+",this);");
						$('#decreaseset_'+i).removeAttr("onClick").attr("onClick","fn_deletebreak_down_tr("+i+",'tbl_list',this);");
					})
				}
				fn_sum_qty();
			}
    ...
    <div align="center" style="width:650px;">
		...
		<fieldset style="width:620px;">
            <table width="620" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
            	<thead>
					<tr>
						<th colspan="3">Qty Distribution Basis:</th>
						<th colspan="3">
							<? 
					     ...
					     <th  style="text-align: left">Total:<span id="tot_dis_qty"></span></th>
					</tr>
					<tr>
						<th width="30">SL</th>
                ...
                <tbody>
					...
					if(count($emp_data)>0)
					{
						foreach ($emp_data as $key => $v) 
						{
							$break_down_arr=explode("**",$v);
							?>
							<tr bgcolor='<? echo $bgcolor_; ?>' onclick="change_color('tr_2nd<? echo $i;?>','<? echo $bgcolor_; ?>')" id="tr_2nd<? echo $i;?>">
								<td align="center"><?=$i;?></td>
								<td align="center" id="td_floor_<?=$i;?>">
									<? 
										echo create_drop_down( "cbofloorName_$i", 100, $floor_array,"", 1, "-- Select Floor--", $break_down_arr[0] , "fn_get_table(this.id,this.value);fn_get_employee(this.id,this.value);",0);
									?>
								</td> 
								<td align="center" id="td_table_<?=$i;?>">
									<? 
										echo create_drop_down( "cbotableName_$i", 100, $table_array,"", 1, "-- Select Table --", $break_down_arr[1] , "","");
									?>
								</td> 
								<td align="center" id="td_process_<?=$i;?>">
									<? 
										echo create_drop_down( "cboprocess_$i", 100, $process_array,"", 1, "-- Select Process --", $break_down_arr[2] , "","");
									?>
								</td>
								<td align="center" id="td_emp_<?=$i;?>">
									<? 
										echo create_drop_down( "cboEmpName_$i", 100, $emp_array,"", 1, "-- Select --", $break_down_arr[3] , "",0);
									?>
								</td>         
								<td align="center">
								<input type="text" id="qty_<?=$i;?>"  name="qty_<?=$i;?>"  style="width:50px" class="text_boxes_numeric" maxlength="20" onblur="fn_sum_qty();" value="<?=$break_down_arr[4];?>"/>
								</td>	
								<td>
									<input type="button" id="increaseset_<?=$i;?>" style="width:20px" class="formbutton" value="+" onclick="add_break_down_tr(<?=$i;?>,this)">
					...
					else
					{
						...
						<tr bgcolor='<? echo $bgcolor_; ?>' onclick="change_color('tr_2nd<? echo $i;?>','<? echo $bgcolor_; ?>')" id="tr_2nd<? echo $i;?>">
							...
							<td align="center" id="td_floor_1">
								<? 
									echo create_drop_down( "cbofloorName_1", 100, $floor_array,"", 1, "-- Select Floor--", $floor_id , "fn_get_table(this.id,this.value);fn_get_employee(this.id,this.value);",0);
								?>
							</td> 
							<td align="center" id="td_table_1">
								<? 
									echo create_drop_down( "cbotableName_1", 100, $table_array,"", 1, "-- Select Table --", $table_id , "","");
								?>
							</td> 
							...
							<td align="center" id="td_emp_1">
								<? 
									echo create_drop_down( "cboEmpName_1", 100, $emp_array,"", 1, "-- Select --", $emp_id , "",0);
								?>
							</td>         
							<td align="center">
							...
							<td>
								...
								<input type="button" id="decreaseset_1" style="width:20px" class="formbutton" value="-" onclick="javascript:fn_deletebreak_down_tr(1 ,this );">
							</td> 
...
if ($action=="show_list_view") 
{
   ...
   //echo $color_sql;die;
	...
	$sql_result=("SELECT a.id, a.style_ref,a.job_id,a.po_id,a.item_id,a.color_id,b.floor_id,b.table_id,b.process_id,b.operator_id,b.qty from operator_wise_cutting_entry_mst a ,operator_wise_cutting_entry_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and  b.status_active=1 and b.is_deleted=0 $company_id_cond1
	$location1 $date_con1 ");
	// echo $sql_result;die;
	...
	foreach($result as $row)
	{
		...
		{
			$operator_data_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']]['emp_data'] = $row['FLOOR_ID']."**".$row['TABLE_ID']."**".$row['PROCESS_ID']."**".$row['OPERATOR_ID']."**".$row['QTY'];
			$operator_data_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']]['qty'] = $row['QTY'];
			// echo $row['PO_ID']."*".$row['ITEM_ID']."*".$row['COLOR_ID']."*".$row['QTY']."<br>";
		}
		else
		{
			$operator_data_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']]['emp_data'] .= "__".$row['FLOOR_ID']."**".$row['TABLE_ID']."**".$row['PROCESS_ID']."**".$row['OPERATOR_ID']."**".$row['QTY'];
			$operator_data_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']]['qty'] += $row['QTY'];
			// echo $row['PO_ID']."*".$row['ITEM_ID']."*".$row['COLOR_ID']."*".$row['QTY']."<br>";
		}		
...
if ($action=="save_update_delete")
{
	...
	{		
		...
		$field_array="id, company_id, location_id,entry_date,style_ref, job_id, po_id,item_id, color_id, inserted_by, insert_date,status_active,is_deleted";
		
		$field_array_up="company_id*location_id*entry_date*style_ref*job_id*po_id*item_id*color_id*updated_by*update_date";
		...
		for($j=1;$j<=$row_num;$j++)
		{
			...
			if($$mstId>0)
			{
				...
				if(str_replace("'","",$$cuttingdata) !="")
				{
					...
					$mst_id_array[]=$$mstId;
					$data_array_up[$$mstId]=explode("*",("".$cbo_company_id."*".$cbo_location_name."*".$txt_cuitting_date."*'".$$styleRefNo."'*".$$jobId."*".$$orderId."*".$$gtmsId."*".$$colorId."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
					...
					foreach ($cuttingdataArr as $rowStr) 
					{
						...
						$data_array_dtls.="(".$mst_id.",".$$mstId.",".$floor_id . ",".$table_id . ",".$process_id . ",".$op_id . "," . $qty . ",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','1','0')";
					}
				}
			}
			else
			{
				if(str_replace("'","",$$cuttingdata) !="")
				{
					...
					$data_array.="(".$id.",".$cbo_company_id.",".$cbo_location_name.",".$txt_cuitting_date.",'".$$styleRefNo."',".$$jobId.",".$$orderId.",".$$gtmsId.",".$$colorId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','1','0')";
					...
					foreach ($cuttingdataArr as $rowStr) 
					{
						...
						if($data_array_dtls!="") $data_array_dtls.=",";
						$data_array_dtls.="(".$mst_id.",".$id.",".$floor_id . ",".$table_id . ",".$process_id . ",".$op_id . "," . $qty . ",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','1','0')";
					}
