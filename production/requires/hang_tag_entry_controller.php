...
if ($action=="load_variable_settings")
{
	...
	$sql_result = sql_select("select hang_tag_update,production_entry from variable_settings_production where company_name=$data and variable_list=1 and status_active=1");
 	foreach($sql_result as $result)
	{
		echo "$('#sewing_production_variable').val(".$result[csf("hang_tag_update")].");\n";
		echo "$('#styleOrOrderWisw').val(".$result[csf("production_entry")].");\n";
		
	}
	
	echo "$('#iron_production_variable_rej').val(0);\n";
	$sql_result_rej = sql_select("select hang_tag_update from variable_settings_production where company_name=$data and variable_list=28 and status_active=1");
 	foreach($sql_result_rej as $result)
	{
		
		echo "$('#iron_production_variable_rej').val(".$result[csf("hang_tag_update")].");\n";
		if($result[csf("hang_tag_update")]==3)
		{
	...
	$variable_is_control=return_field_value("is_control","variable_settings_production","company_name=$data and variable_list=33 and page_category_id=256","is_control");
...
{
	echo create_drop_down( "cbo_location", 172, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id='$data' order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down( 'requires/hang_tag_entry_controller', this.value, 'load_drop_down_floor', 'floor_td' );",0 );     	 
}

if ($action=="load_drop_down_floor")
{
	echo create_drop_down( "cbo_floor", 170, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process in (20) order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down('requires/hang_tag_entry_controller', document.getElementById('cbo_hang_tag_company').value+'_'+document.getElementById('cbo_location').value+'_'+this.value, 'load_drop_down_table', 'table_td' );",0 );     	 
}
...
if($action=="load_drop_down_source")
{
	...
	if($data==3)
	{
		if($db_type==0)
		{
			echo create_drop_down( "cbo_hang_tag_company", 170, "select id,supplier_name from lib_supplier where status_active=1 and is_deleted=0 and find_in_set(22,party_type) order by supplier_name","id,supplier_name", 1, "--- Select ---", $selected, "fnc_workorder_search(this.value);",0,0 );
		}
		else
		{
			echo create_drop_down( "cbo_hang_tag_company", 170, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=22 and a.status_active=1 group by a.id,a.supplier_name order by a.supplier_name","id,supplier_name", 1, "--Select--", $selected, "fnc_workorder_search(this.value);" );
		}
	}
	else if($data==1)
 		echo create_drop_down( "cbo_hang_tag_company", 170, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1 $company_cond order by company_name","id,company_name", 1, "--- Select ---", "", "load_drop_down( 'requires/hang_tag_entry_controller', this.value, 'load_drop_down_location', 'location_td' );fnc_company_check(document.getElementById('cbo_source').value);",0,0 ); 
 	else
		echo create_drop_down( "cbo_hang_tag_company", 170, $blank_array,"", 1, "--- Select ---", $selected, "",0,0 );	
...
if($action=="create_po_search_list_view")
{
	...
	if($cbo_year !=0)
	{
		if($db_type==0) { $sql_shipment_year_cond=" and YEAR(b.shipment_date)=$cbo_year";   }
		if($db_type==2) {$sql_shipment_year_cond=" and to_char(b.shipment_date,'YYYY')=$cbo_year";}
	}
	...
	if(trim($txt_search_by)==4 && trim($txt_search_common)!="")
	{
		$sql = "SELECT b.id,a.order_uom,a.buyer_name,a.company_name,a.total_set_qnty,a.set_break_down, a.job_no,a.style_ref_no,a.gmts_item_id,a.location_name,b.shipment_date,b.po_number,b.file_no,b.grouping,b.po_quantity,b.plan_cut
			...
			$sql_cond $qty_source_cond $sql_shipment_year_cond  group by b.id,a.order_uom,a.buyer_name,a.company_name,a.total_set_qnty,a.set_break_down, a.job_no,a.style_ref_no,a.gmts_item_id,a.location_name,b.shipment_date,b.po_number,b.file_no,b.grouping,b.po_quantity,b.plan_cut order by b.shipment_date desc";
	}
	else
	{	
 		$sql = "SELECT b.id,a.order_uom,a.buyer_name,a.company_name,a.total_set_qnty,a.set_break_down, a.job_no,a.style_ref_no,a.gmts_item_id,a.location_name,b.shipment_date,b.po_number,b.file_no,b.grouping,b.po_quantity ,b.plan_cut
			...
			b.is_deleted=0 and
			a.garments_nature=$garments_nature
			$sql_cond $qty_source_cond $sql_shipment_year_cond order by b.shipment_date desc"; 
	}
	//   echo $sql;//die;
	...
	$poCountryData=sql_select( "SELECT po_break_down_id, item_number_id, country_id, sum(order_quantity) as qnty, sum(plan_cut_qnty) as plan_cut_qnty from wo_po_color_size_breakdown where status_active=1 and is_deleted=0 $po_cond group by po_break_down_id, item_number_id, country_id");
	...
	$total_out_qty=sql_select( "SELECT po_break_down_id, item_number_id, country_id,production_type, sum(production_quantity) as production_quantity from pro_garments_production_mst where status_active=1 and is_deleted=0 and production_type in(5,7) $po_cond group by po_break_down_id, item_number_id, country_id,production_type");
	...
	/*$total_sewing_out_qty_data_arr=array();
	$total_sewing_out_qty=sql_select( "SELECT po_break_down_id, item_number_id, country_id, sum(production_quantity) as production_quantity from pro_garments_production_mst where status_active=1 and is_deleted=0 and production_type=5 group by po_break_down_id, item_number_id, country_id");
	
	foreach($total_sewing_out_qty as $row)
	{
		$total_sewing_out_qty_data_arr[$row[csf('po_break_down_id')]][$row[csf('item_number_id')]][$row[csf('country_id')]]=$row[csf('production_quantity')];
	}*/
	// echo "<pre>";
	// print_r($total_sewing_out_qty_data_arr);
	...
	?>
     ...
     <div style="width:1285px; max-height:240px;overflow-y:scroll;" >	 
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1264" class="rpt_table" id="tbl_po_list">
			<?
			$i=1;
						...
						?>
							<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value(<? echo $row[csf("id")];?>,'<? echo $grmts_item;?>','<? echo $po_qnty;?>','<? echo $plan_cut_qnty;?>','<? echo $country_id;?>');" > 
                                ...
                                <td width="80" align="right">
									...
									echo $total_sewing_out_qty=$total_out_qty_data_arr[$row[csf('id')]][$grmts_item][$country_id][5];
									?> &nbsp;
                                </td>
                                <td width="80" align="right">
								<?php
                                 
                                 echo $total_cut_qty=$total_out_qty_data_arr[$row[csf('id')]][$grmts_item][$country_id][7];
								//echo  $row[csf('id')];
                                 ?> &nbsp;
                               </td>
                               <td width="80" align="right">
...
if($action=="populate_data_from_search_popup")
{
  	...
  	foreach($res as $result)
	{
		...
		echo "$('#txt_style_no').val('".$result[csf('style_ref_no')]."');\n";
  		if($qty_source!=0)
   		{		 
   		   $dataArray=sql_select("SELECT SUM(CASE WHEN production_type='$qty_source' THEN production_quantity END) as totalinput,SUM(CASE WHEN production_type=15 THEN production_quantity ELSE 0 END) as totalsewing, table_no from pro_garments_production_mst WHERE po_break_down_id=".$result[csf('id')]."  and item_number_id='$item_id' and country_id='$country_id' and status_active=1 and is_deleted=0 and production_type in($qty_source,15) group by table_no");
			...
			{  
				...
				echo "$('#txt_cumul_hang_tag_qty').val('".$row[csf('totalsewing')]."');\n";
				$yet_to_produced = $row[csf('totalinput')]-$row[csf('totalsewing')];
				echo "$('#txt_yet_to_hang_tag').val('".$yet_to_produced."');\n";
		...
		if($qty_source==0)
		{
			...
			$total_produced = return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=".$result[csf('id')]." and item_number_id='$item_id' and country_id='$country_id' and production_type=15 and is_deleted=0");
			echo "$('#txt_sewing_quantity').val('".$plan_cut_qnty."');\n";		
			echo "$('#txt_cumul_hang_tag_qty').attr('placeholder','".$total_produced."');\n";
			echo "$('#txt_cumul_hang_tag_qty').val('".$total_produced."');\n";
			$yet_to_produced = $plan_cut_qnty - $total_produced;
			echo "$('#txt_yet_to_hang_tag').attr('placeholder','".$yet_to_produced."');\n";
			echo "$('#txt_yet_to_hang_tag').val('".$yet_to_produced."');\n";
		}
...
if($action=="color_and_size_level")
{
		...
		if($qty_source!=0)
		{
			if( $variableSettings==2 ) // color level
			{
				if($db_type==0)
				{
					$sql = "SELECT id, item_number_id, size_number_id, color_number_id, order_quantity, sum(plan_cut_qnty) as plan_cut_qnty, (select sum(CASE WHEN pdtls.color_size_break_down_id=wo_po_color_size_breakdown.id then pdtls.production_qnty ELSE 0 END) from pro_garments_production_dtls pdtls where pdtls.production_type='$qty_source' and pdtls.is_deleted=0 ) as production_qnty, (select sum(CASE WHEN cur.color_size_break_down_id=wo_po_color_size_breakdown.id then cur.production_qnty ELSE 0 END) from pro_garments_production_dtls cur where cur.production_type='15' and cur.is_deleted=0 ) as cur_production_qnty 
				...
				else
				{
					$sql = "SELECT a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty,
							...
							sum(CASE WHEN c.production_type=15 then b.production_qnty ELSE 0 END) as cur_production_qnty,
							sum(CASE WHEN c.production_type=15 then b.reject_qty ELSE 0 END) as reject_qty
			...
			else if( $variableSettings==3 ) //color and size level
			{
					...
					$sql ="SELECT a.color_size_break_down_id,
											...
											sum(CASE WHEN a.production_type=15 then a.production_qnty ELSE 0 END) as cur_production_qnty,
											sum(CASE WHEN a.production_type=15 then a.reject_qty ELSE 0 END) as reject_qty
		...
		else // if preceding process =0 in variable setting then plan cut quantity will show
		{
			if( $variableSettings==2 ) // color level
			{
				...
				else
				{
					...
					from wo_po_color_size_breakdown a left join pro_garments_production_dtls b on a.id=b.color_size_break_down_id and b.production_type=15
					where a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' and a.is_deleted=0 and a.status_active=1 group by a.item_number_id, a.color_number_id";	
			...
			{				
				...
				$dtlsData = sql_select("SELECT a.color_size_break_down_id,
											sum(CASE WHEN a.production_type=15 then a.production_qnty ELSE 0 END) as production_qnty
											from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in(15) group by a.color_size_break_down_id");
		...
		if($qty_source==0)
		{
			foreach($colorResult as $color)
			{
				if( $variableSettings==2 ) // color level
				{ 
					$colorHTML .='<tr><td>'.$color_library[$color[csf("color_number_id")]].'</td><td><input type="text" name="txt_color" id="colSize_'.($i+1).'" style="width:60px"  class="text_boxes_numeric" placeholder="'.($color[csf("plan_cut_qnty")]-$color[csf("production_qnty")]).'" onkeyup="fn_colorlevel_total('.($i+1).')"></td><td><input type="text" name="txtColSizeRej" id="colSizeRej_'.($i+1).'" style="width:60px"  class="text_boxes_numeric" placeholder="Rej." onkeyup="fn_colorRej_total('.($i+1).') '.$disable.'"></td><td><input type="text" name="txtColSizeReiron" id="colSizeReiron_'.($i+1).'" style="width:60px"  class="text_boxes_numeric" placeholder="Re-Hang" onkeyup="fn_colorReiron_total('.($i+1).') '.$disable.'"></td></tr>';				
    ...
    <div style="width:100%;">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="100%" class="rpt_table">
            <thead>
                ...
                <th width="80" align="center">Re-Hang Tag Qty</th> 
	...
	<div style="width:100%;max-height:180px; overflow:y-scroll" id="iron_list_view" align="left">
		...
		<?php  
 		...
 		?>
			...
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_input_form_data','requires/hang_tag_entry_controller');" > 
...
if($action=="show_country_listview")
{
	...
	{
		$qty_source=15;//Hang Tag Entry
		 $heading = "Hang Tag Qty";
	}
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="435" class="rpt_table">
        <thead>
            ...
            <th width="65">Hang Tag Entry</th>                  
            <th><? echo $heading;?></th>                  
            <!-- <th>Hang Tag Entry</th>                   -->
        </thead>
		<?
		$issue_qnty_arr=sql_select("SELECT a.po_break_down_id, a.item_number_id, a.country_id,a.production_type, b.production_qnty from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id and a.po_break_down_id='$po_id' and a.production_type in(15,$qty_source) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
		$qty_data_arr=array();
		foreach($issue_qnty_arr as $row)
		{
			if($row[csf("production_type")]==15)
			{
				$qty_data_arr[$row[csf("po_break_down_id")]][$row[csf("country_id")]][$row[csf("item_number_id")]]['hang_tag_qty']+=$row[csf("production_qnty")];
			}
			else
			{
		...
		{
			if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
			$issue_qnty=$qty_data_arr[$row[csf("po_break_down_id")]][$row[csf("country_id")]][$row[csf("item_number_id")]]['hang_tag_qty'];
			$output_qty=$qty_data_arr[$row[csf("po_break_down_id")]][$row[csf("country_id")]][$row[csf("item_number_id")]]['iron_qty'];
		?>
...
if($action=="populate_input_form_data")
{
	...
	if($db_type==0)
	{
	$sqlResult =sql_select("select id,company_id, garments_nature, po_break_down_id, item_number_id, challan_no, country_id, production_source, serving_company, sewing_line, location, produced_by, embel_name, embel_type, production_date, production_quantity, re_production_qty, production_source, production_type, entry_break_down_type, break_down_type_rej, TIME_FORMAT( production_hour, '%H:%i' ) as production_hour, sewing_line, supervisor, carton_qty, remarks, floor_id, alter_qnty, reject_qnty, total_produced, yet_to_produced ,wo_order_id,currency_id,exchange_rate,rate, table_no from pro_garments_production_mst where id='$data' and production_type='15' and status_active=1 and is_deleted=0 order by id");
	}
	if($db_type==2)
	{
	$sqlResult =sql_select("select id,company_id, garments_nature, po_break_down_id, item_number_id, challan_no, country_id, production_source, serving_company, sewing_line, location, produced_by, embel_name, embel_type, production_date, production_quantity, re_production_qty, production_source, production_type, entry_break_down_type, break_down_type_rej, TO_CHAR(production_hour,'HH24:MI') as production_hour, sewing_line, supervisor, carton_qty, remarks, floor_id, table_no, alter_qnty, reject_qnty, total_produced, yet_to_produced,wo_order_id,currency_id,exchange_rate,rate from pro_garments_production_mst where id='$data' and production_type='15' and status_active=1 and is_deleted=0 order by id");
	}
	...
	$control_and_preceding=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=33 and page_category_id=256 and company_name='$company'");  
	...
	{ 
		...
		echo "$('#txt_hang_tag_date').val('".change_date_format($result[csf('production_date')])."');\n";
		echo "$('#cbo_source').val('".$result[csf('production_source')]."');\n";
		echo "load_drop_down( 'requires/hang_tag_entry_controller', ".$result[csf('production_source')].", 'load_drop_down_source', 'iron_company_td' );\n";
		echo "$('#cbo_hang_tag_company').val('".$result[csf('serving_company')]."');\n";
		echo "load_drop_down( 'requires/hang_tag_entry_controller',".$result[csf('serving_company')].", 'load_drop_down_location', 'location_td' );";
		echo "$('#cbo_location').val('".$result[csf('location')]."');\n";
		echo "load_drop_down( 'requires/hang_tag_entry_controller', ".$result[csf('location')].", 'load_drop_down_floor', 'floor_td' );\n";
		echo "$('#cbo_floor').val('".$result[csf('floor_id')]."');\n";

		echo "load_drop_down( 'requires/hang_tag_entry_controller', '".$tableArgs."', 'load_drop_down_table', 'table_td' );\n";
		...
		if($result[csf('production_source')]==3)
		{
			...
			echo "load_drop_down( 'requires/hang_tag_entry_controller', '".$result[csf('company_id')]."_".$result[csf('serving_company')]."_".$result[csf('po_break_down_id')]."', 'load_drop_down_workorder', 'workorder_td' );\n";
		...
		echo "$('#txt_reporting_hour').val('".$result[csf('production_hour')]."');\n";
 		echo "$('#txt_hang_tag_qty').val('".$result[csf('production_quantity')]."');\n";
		echo "$('#txt_re_hang_tag_qty').val('".$result[csf('re_production_qty')]."');\n";
		...
		echo "$('#txt_challan').val('".$result[csf('challan_no')]."');\n";
 		...
 		if($qty_source!=0)
		{
		   $dataArray=sql_select("select SUM(CASE WHEN production_type='$qty_source' THEN production_quantity END) as totalinput,SUM(CASE WHEN production_type=15 THEN production_quantity ELSE 0 END) as totalsewing from pro_garments_production_mst WHERE po_break_down_id=".$result[csf('po_break_down_id')]."  and item_number_id=".$result[csf('item_number_id')]." and country_id=".$result[csf('country_id')]." and status_active=1 and is_deleted=0");
	 		foreach($dataArray as $row)
			{  
				echo "$('#txt_sewing_quantity').val('".$row[csf('totalinput')]."');\n";
				echo "$('#txt_cumul_hang_tag_qty').val('".$row[csf('totalsewing')]."');\n";
			...
			{  
				...
				$total_produced = return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=".$result[csf('po_break_down_id')]." and item_number_id=".$result[csf('item_number_id')]." and country_id=".$result[csf('country_id')]." and production_type=15 and is_deleted=0");
				echo "$('#txt_sewing_quantity').val('".$plan_cut_qnty."');\n";	
				echo "$('#txt_cumul_hang_tag_qty').val('".$total_produced."');\n";
				$yet_to_produced = $plan_cut_qnty - $total_produced;
				echo "$('#txt_yet_to_hang_tag').val('".$yet_to_produced."');\n";
		...
		if($qty_source!=0)
		{
			...
			{ 
				...
				if( $variableSettings==2 ) // color level
				{
					if($db_type==0)
					{
						$sql = "SELECT id, item_number_id, size_number_id, color_number_id, order_quantity, sum(plan_cut_qnty) as plan_cut_qnty, (select sum(CASE WHEN pdtls.color_size_break_down_id=wo_po_color_size_breakdown.id then pdtls.production_qnty ELSE 0 END) from pro_garments_production_dtls pdtls where pdtls.production_type='$qty_source' and pdtls.is_deleted=0 ) as production_qnty, (select sum(CASE WHEN cur.color_size_break_down_id=wo_po_color_size_breakdown.id then cur.production_qnty ELSE 0 END) from pro_garments_production_dtls cur where cur.production_type=15 and cur.is_deleted=0 ) as cur_production_qnty, (select sum(CASE WHEN cur.color_size_break_down_id=wo_po_color_size_breakdown.id then cur.reject_qty ELSE 0 END) from pro_garments_production_dtls cur where cur.production_type=15 and cur.is_deleted=0 ) as reject_qty 
						from wo_po_color_size_breakdown
						where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' and is_deleted=0 and status_active=1 group by color_number_id";
					}
					else
					{
						$sql = "SELECT a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty,
								...
								sum(CASE WHEN c.production_type=15 then b.production_qnty ELSE 0 END) as cur_production_qnty,
								sum(CASE WHEN c.production_type=15 then b.reject_qty ELSE 0 END) as reject_qty,
								sum(CASE WHEN c.production_type=15 then b.re_production_qty ELSE 0 END) as re_production_qty
								from wo_po_color_size_breakdown a 
								left join pro_garments_production_dtls b on a.id=b.color_size_break_down_id
								left join pro_garments_production_mst c on c.id=b.mst_id
								where a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' and a.is_deleted=0 and a.status_active=1 group by a.item_number_id, a.color_number_id";	
				...
				else if( $variableSettings==3 ) //color and size level
				{
					$dtlsData = sql_select("SELECT a.color_size_break_down_id,
											...
											sum(CASE WHEN a.production_type=15 then a.production_qnty ELSE 0 END) as cur_production_qnty,
											sum(CASE WHEN a.production_type=15 then a.reject_qty ELSE 0 END) as reject_qty,
											sum(CASE WHEN a.production_type=15 then a.re_production_qty ELSE 0 END) as re_production_qty
											from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in($qty_source,15) group by a.color_size_break_down_id");
				...
				{
					/*$sql = "select id, item_number_id, size_number_id, color_number_id, order_quantity, plan_cut_qnty, (select sum(CASE WHEN pdtls.color_size_break_down_id=wo_po_color_size_breakdown.id then pdtls.production_qnty ELSE 0 END) from pro_garments_production_dtls pdtls where pdtls.production_type=5 and pdtls.is_deleted=0 ) as production_qnty, (select sum(CASE WHEN cur.color_size_break_down_id=wo_po_color_size_breakdown.id then cur.production_qnty ELSE 0 END) from pro_garments_production_dtls cur where cur.production_type=15 and cur.is_deleted=0 ) as cur_production_qnty 
						from wo_po_color_size_breakdown
						where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' and is_deleted=0 and status_active=1";*/
				...
				foreach($colorResult as $color)
				{
					...
					{  
						...
						$colorHTML .='<tr><td>'.$color_library[$color[csf("color_number_id")]].'</td><td><input type="text" name="txt_color" id="colSize_'.($i+1).'" style="width:60px"  class="text_boxes_numeric" placeholder="'.($color[csf("production_qnty")]-$color[csf("cur_production_qnty")]+$amount).'" value="'.$amount.'" onkeyup="fn_colorlevel_total('.($i+1).')"></td><td><input type="text" name="txtColSizeRej" id="colSizeRej_'.($i+1).'" style="width:60px" class="text_boxes_numeric" placeholder="Rej." value="'.$rejectAmt.'" onkeyup="fn_colorRej_total('.($i+1).') '.$disable.'"></td><td><input type="text" name="txtColSizeReiron" id="colSizeReiron_'.($i+1).'" style="width:60px" class="text_boxes_numeric" placeholder="Re-Hang" value="'.$reironAmt.'" onkeyup="fn_colorReiron_total('.($i+1).') '.$disable.'"></td></tr>';				
						$totalQnty += $amount;
				...
				{ 
					...
					if( $variableSettings==2 ) // color level
					{
						if($db_type==0)
						{
							...
							$sql = "SELECT id, item_number_id, size_number_id, color_number_id, order_quantity, sum(plan_cut_qnty) as plan_cut_qnty, (select sum(CASE WHEN pro_garments_production_dtls.color_size_break_down_id=wo_po_color_size_breakdown.id then production_qnty ELSE 0 END) from pro_garments_production_dtls where is_deleted=0 and  	production_type=15 ) as production_qnty, (select sum(CASE WHEN pro_garments_production_dtls.color_size_break_down_id=wo_po_color_size_breakdown.id then reject_qty ELSE 0 END) from pro_garments_production_dtls where is_deleted=0 and production_type=15 ) as reject_qty
							from wo_po_color_size_breakdown
...
if ($action=="save_update_delete")
{
	...
	if($is_projected_po_allow ==2)
	{
 	...
 	$is_control_sql=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=33 and page_category_id=256 and company_name=$cbo_company_name");  
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		{
			$txt_hang_tag_qty=str_replace("'","",$txt_hang_tag_qty);
			...
			$country_iron_qty=return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=$hidden_po_break_down_id and production_type=15 and country_id=$cbo_country_name and status_active=1 and is_deleted=0");
			//echo $country_sewing_output_qty .'<'. $country_iron_qty.'+'.$txt_hang_tag_qty;die;
			if($country_sewing_output_qty < $country_iron_qty+$txt_hang_tag_qty)
			{
				echo "25**0";
				disconnect($con);
				die;
		...
		if($rate!="") {  $amount=$rate*str_replace("'","",$txt_hang_tag_qty);}
		if($db_type==0)
		{
			$data_array1="(".$id.",".$garments_nature.",".$cbo_company_name.",".$txt_challan.",".$hidden_po_break_down_id.",".$cbo_item_name.",".$cbo_country_name.",".$cbo_source.",".$cbo_hang_tag_company.",".$cbo_location.",".$cbo_produced_by.",".$txt_hang_tag_date.",".$txt_hang_tag_qty.",".$txt_re_hang_tag_qty.",15,".$sewing_production_variable.",".$iron_production_variable_rej.",".$txt_reporting_hour.",".$txt_remark.",".$cbo_floor.",".$txt_cumul_hang_tag_qty.",".$txt_yet_to_hang_tag.",".$txt_reject_qnty.",".$cbo_work_order.",".$hidden_currency_id.",".$hidden_exchange_rate.",'".$rate."','".$amount."',".$cbo_table_name.",".$user_id.",'".$pc_date_time."')";

		}
		if($db_type==2)
		{
			$txt_reporting_hour=str_replace("'","",$txt_hang_tag_date)." ".str_replace("'","",$txt_reporting_hour);
		...
		$dtlsData = sql_select("select a.color_size_break_down_id,
										...
										where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name and b.country_id=$cbo_country_name and a.color_size_break_down_id!=0 and a.production_type in(5,15) 
		...
		{		
			...
			foreach($rowEx as $rowE=>$val)
			{
				...
				//15 for Hang Tag Entry
				$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq", "pro_garments_production_dtls", $con );
				if($j==0)$data_array = "(".$dtls_id.",".$id.",15,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."','".$reironQtyArr[$colorSizeNumberIDArr[0]]."')";
				else $data_array .= ",(".$dtls_id.",".$id.",15,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."','".$reironQtyArr[$colorSizeNumberIDArr[0]]."')";
			//	$dtls_id=$dtls_id+1;							
 				$j++;								
			}
 		}//color level wise
		...
		{		
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				//15 for Iron output Entry
				$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",  "pro_garments_production_dtls", $con );
				if($j==0)$data_array = "(".$dtls_id.",".$id.",15,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."','".$reironQtyArr[$index]."')";
				else $data_array .= ",(".$dtls_id.",".$id.",15,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."','".$reironQtyArr[$index]."')";
				//$dtls_id=$dtls_id+1;
 				$j++;
			}
		}//color and size wise
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		if($is_control==1 && $user_level!=2)
		{
			...
			$country_sewing_output_qty=return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=$hidden_po_break_down_id and production_type=5 and country_id=$cbo_country_name and status_active=1 and is_deleted=0");
			$country_iron_qty=return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=$hidden_po_break_down_id and production_type=15 and country_id=$cbo_country_name and status_active=1 and is_deleted=0 and id <> $txt_mst_id");
			if($country_sewing_output_qty < $country_iron_qty+$txt_hang_tag_qty)
			{
		...
		$data_array1="".$cbo_source."*".$cbo_hang_tag_company."*".$cbo_location."*".$cbo_produced_by."*".$txt_hang_tag_date."*".$txt_hang_tag_qty."*".$txt_re_hang_tag_qty."*15*".$sewing_production_variable."*".$iron_production_variable_rej."*".$txt_reporting_hour."*".$txt_challan."*".$txt_remark."*".$cbo_floor."*".$txt_cumul_hang_tag_qty."*".$txt_yet_to_hang_tag."*".$txt_reject_qnty."*".$cbo_work_order."*".$hidden_currency_id."*".$hidden_exchange_rate."*'".$rate."'*'".$amount."'*".$cbo_table_name."*".$user_id."*'".$pc_date_time."'";
		...
		if(str_replace("'","",$sewing_production_variable)!=1 && str_replace("'","",$txt_mst_id)!='' ) // check is not gross level
		{
			...
			$dtlsData = sql_select("select a.color_size_break_down_id,
										...
										sum(CASE WHEN a.production_type=15 then a.production_qnty ELSE 0 END) as cur_production_qnty 
										from pro_garments_production_dtls a,pro_garments_production_mst b 
										where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name and b.country_id=$cbo_country_name and a.color_size_break_down_id!=0 and a.production_type in(5,15) and b.id !=$txt_mst_id
										group by a.color_size_break_down_id");
			$color_pord_data=array();							
			foreach($dtlsData as $row)
			{				  
			...
			//	$dtls_id=return_next_id("id", "pro_garments_production_dtls", 1);
				...
				foreach($rowEx as $rowE=>$val)
				{
					...
					//15 for Iron output Entry
					$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",   "pro_garments_production_dtls", $con );
					if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",15,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."','".$reironQtyArr[$colorSizeNumberIDArr[0]]."')";
					else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",15,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."','".$reironQtyArr[$colorSizeNumberIDArr[0]]."')";
					//$dtls_id=$dtls_id+1;							
					$j++;								
				}
			}
			...
			{		
				...
				foreach($rowEx as $rowE=>$valE)
				{
					...
					//15 for Iron output Entry
					$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",  "pro_garments_production_dtls", $con );
					if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",15,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."','".$reironQtyArr[$index]."')";
					else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",15,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."','".$reironQtyArr[$index]."')";
					//$dtls_id=$dtls_id+1;
					$j++;
				}
			}
  ...
  ?>
    ...
    <table align="right" cellspacing="0" width="900"  border="1" rules="all" class="rpt_table" >
        ...
        <? 
            ...
            echo signature_table(256, $data[0], "900px");
         ?>
	</div>
	</div>
<?
...
if($action=="production_process_control")
{
    ...
    $control_and_preceding=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=33 and page_category_id=256 and company_name='$data'");  
