	...
	*/
	...
	foreach ($yarn_wise_sql_data as $row)
	{
		...
		if($stripeColorTypeArr[$row[csf('color_type_id')]])
		{
			...
			{
				$final_stripe_color_arr[$deler_color_type_range_string][$composition[$row[csf('composition_id')]].'=='.$color_library[$row[csf('color_id')]].'=='][1000]=$row[csf('cons_qty')];
	...
	//echo "<pre>";
	//print_r($stripe_td_span_arr);
	//------------------------------------------

	$fso_wise_sql="SELECT b.determination_id,  b.gsm_weight, b.color_range_id, b.color_type_id, sum(b.grey_qty), sum(b.amount), sum(b.amount)/sum(b.grey_qty)  as avg_rate
	from fabric_sales_order_mst a, fabric_sales_order_dtls b 
	...
	group by b.determination_id,  b.gsm_weight, b.color_range_id, b.color_type_id ";
			...
			<table border="1" rules="all" cellpadding="3" style="font: 12px tahoma;">
				...
				foreach ($yarn_wise_arr as $fabric_desc=>$febric_data)
				{
					...
					?>
						...
						if($mm==1)
						{
							$unit_rate  = $fso_dtls_wise_arr[$fabric_desc]['unit_rate'];
							$grey_qty  = $fso_dtls_wise_arr[$fabric_desc]['grey_qty'];								
							$amount = $grey_qty*number_format($unit_rate,8,'.','');

							$grand_total += $grey_qty;
							$grand_amount += number_format($amount,2,'.','');;
						?>
						<td align="right" rowspan="<? echo $feb_colspan;?>"><p><? echo number_format($grey_qty,2); ?></p></td>
						<td rowspan="<? echo $feb_colspan;?>"><p><? //echo $row['grey_qty']; ?></p></td>
						<?
						}
						?>
						</tr>
				...
				foreach ($final_stripe_color_arr as $febDes => $febData) 
				{
					...
					foreach ($febData as $yarnDesColor => $yarnDesColorData) 
					{
						foreach ($yarnDesColorData as $stripeid => $row) 
						{
							$unit_rate = $fso_dtls_wise_arr[$febDes]['unit_rate'];
							...
							?>
								...
								if($mm==1)
								{
									...
									<td rowspan="<? echo $feb_colspan;?>" align="center" width="100"></td> 
									<?
									$grand_total += $grey_qty;
									$grand_amount += $amount;
								}
