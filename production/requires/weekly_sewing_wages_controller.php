...
<script type="text/javascript">
	load_drop_down('weekly_sewing_wages_controller',<? echo $cbo_company_id;?>, 'load_drop_down_location', 'location_td' );
</script>
</html>
<?
exit();
...
if($action=="system_id_list_view")
{
	...
	if($db_type==0){
		$sql = "select a.id,a.sys_number,a.company_id,a.location,a.bill_for,a.final_bill,a.week_from_date,a.week_to_date,GROUP_CONCAT(b.emp_name) as emp_name from pro_weekly_wages_bill_mst a, pro_weekly_wages_bill_dtls b where a.id=b.mst_id and a.company_id=$company_id and $location and $sys_number and a.bill_for=30 $date_con_from $date_con_to and a.status_active=1 and a.is_deleted=0 group by a.id,a.sys_number,a.company_id,a.location,a.bill_for,a.final_bill,a.week_from_date,a.week_to_date order by a.id"; 
	
	}
	else
	{
		$sql = "select a.id,a.sys_number,a.company_id,a.location,a.bill_for,a.final_bill,a.week_from_date,a.week_to_date,LISTAGG( CAST(b.emp_name as VARCHAR(4000)),',') WITHIN GROUP(ORDER BY b.emp_name) AS emp_name from pro_weekly_wages_bill_mst a, pro_weekly_wages_bill_dtls b where a.id=b.mst_id and a.company_id=$company_id and $location and $sys_number and a.bill_for=30 $date_con_from $date_con_to and a.status_active=1 and a.is_deleted=0 group by a.id,a.sys_number,a.company_id,a.location,a.bill_for,a.final_bill,a.week_from_date,a.week_to_date order by a.id"; 
	}
        ...
        <legend>Enter search words</legend>
            <table cellpadding="0" cellspacing="0" width="100%" border="1" rules="all" class="rpt_table">
                ...
                <tr>
                    ...
                    <td align="center">
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( <? echo $cbo_company_id; ?>+'_'+document.getElementById('cbo_category').value, 'enployee_info_list_view', 'search_div', 'weekly_sewing_wages_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                    </td>
                </tr>
            </table>
...
list($company,$rate_for,$from_date,$to_date,$location,$line,$shift,$supplier_id,$buyer,$job_no,$style,$order_no,$order_id,$gmt_item_name)=explode("_",$data);
...
if($buyer==0)$buyer_2=""; else $buyer_2=$buyer;
	...
	$sql = "select c.order_id,b.line_id,a.location,c.bill_qty as bill_qty,c.gmt_item_id  from pro_weekly_wages_bill_mst a, pro_weekly_wages_bill_dtls b, pro_weekly_wages_order_brk c 
where a.id=b.mst_id and b.id=c.weekly_wages_dtls_id and a.location='$location'
 and b.line_id=$line and a.company_id='$company' and a.bill_for=30 and a.week_from_date >= '$from_date' and  a.week_to_date <='$to_date' $order_id_con $gmt_item_name_con and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0";

	$result = sql_select($sql);
		foreach ($result as $row)
		{ 
			$key=$row[csf('order_id')].$row[csf('location')].$row[csf('line_id')].$row[csf('gmt_item_id')];
			$bill_qty_arr[$key]+=$row[csf('bill_qty')];
		}

	
	$sql = "select b.color_type,b.item_id,b.avg_rate,b.style_ref,b.order_id,b.wo_qty,b.uom,b.amount from piece_rate_wo_mst a,piece_rate_wo_dtls b,wo_po_details_master c,pro_garments_production_mst d where a.id=b.mst_id and b.job_id=c.id and d.po_break_down_id=b.order_id  and d.sewing_line='$line' and d.production_type=5 and a.company_id='$company' and a.rate_for='$rate_for' and b.style_ref like '%$style%' and b.buyer_id like '%$buyer_2%' and c.job_no like '%$job_no%' and d.production_date BETWEEN '$from_date' and '$to_date' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0";
	//echo $sql;
	...
	$sql = "select a.id as job_id,b.id,a.company_name,a.buyer_name,a.style_ref_no,c.item_number_id,b.job_no_mst,b.po_number,max(b.po_quantity) as po_quantity  
from wo_po_details_master a,wo_po_break_down b,pro_garments_production_mst c where a.job_no=b.job_no_mst
and c.location='$location' and c.sewing_line=$line and a.company_name='$company' $buyer_id
  $style_ref $job and c.po_break_down_id=b.id  and c.production_date BETWEEN '$from_date' and '$to_date' 
and c.production_type=5 $order_no and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by a.id,a.company_name,a.style_ref_no,c.item_number_id,a.buyer_name,b.job_no_mst,b.id,b.po_number order by a.id";
	...
	<div style="width:915px; max-height:220px; overflow-y:scroll" id="list_container_batch" align="left">	 
        ...
        <?
		   ...
		   //in_array(,);
            ...
            foreach ($result as $row)
            { 
			  $item_arr= explode(',',str_replace("'",'',$gmt_item_name));
              $key=$row[csf('id')].$location.$line.$row[csf('item_number_id')]; 
			  if($bill_qty_arr[$key]==''){
			...
			?>
                <tr id="tr_<? echo $key2; ?>" bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"  onClick="js_set_value(<? echo $row[csf('id')]; ?>,'<? echo $wo_rate_arr[$key2]; ?>','<? echo $rate_variable_arr[$key2]; ?>','<? echo $row[csf('style_ref_no')]; ?>','<? echo $row[csf('item_number_id')]; ?>','<? echo $row[csf('job_id')]; ?>','<? echo $bgcolor; ?>')" > 
                    ...
                    <td width="80" align="right"><p><? echo $wo_amount_arr[$key2]; ?></p></td>
...
if($action=="show_weekly_wages_bill_listview")
{
	...
	$sql = "select b.id,b.mst_id,b.emp_id,b.emp_name,b.designation,b.order_id,b.buyer_id,b.style_ref,b.gmt_item,b.apv_order_qty,b.cutting_bill_qty,b.previous_bill_qty,b.yet_to_bill_qty,b.line_id,b.prod_reso_allo from pro_weekly_wages_bill_dtls b where b.mst_id=$data and b.status_active=1 and b.is_deleted=0 order by b.id";
	$result = sql_select($sql);
	...
	$line_arr = return_library_array("select id,line_name from lib_sewing_line","id","line_name");
		...
		foreach($line_data as $row)
		{
			...
			{
				if($line=='') $line=$line_arr[$val]; else $line.=",".$line_arr[$val];
			}
	...
	<div style="width:850px; max-height:220px; overflow-y:scroll" id="list_container_batch" align="left">	 
        ...
        <?
            ...
            {  
				...
				if($row[csf('prod_reso_allo')]==1) $lineArr=$line_array; else  $lineArr=$line_arr;
			?>
                <tr id="tr_<? echo $row[csf('id')]; ?>" bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"  onClick="get_php_form_data('<? echo $row[csf('mst_id')].'_'.$row[csf('id')]; ?>', 'populate_sewing_wages_dtls_form_data', 'requires/weekly_sewing_wages_controller')" > 
                    <td width="35" align="center"><? echo $i; ?></td>
                    <td width="60" align="center"><p><? echo $row[csf('emp_id')]; ?></p></td>
                    <td width="50"><p><? echo $lineArr[$row[csf('line_id')]]; ?></p></td>
                    ...
                    <td width="60" align="right"><p><? echo $row[csf('apv_order_qty')]; ?></p></td>
                    <td width="60" align="right"><p><? echo $row[csf('cutting_bill_qty')]; ?></p></td>
                    <td width="60" align="right"><p><? echo $row[csf('previous_bill_qty')]; ?></p></td>
                    <td align="right"><? echo $row[csf('yet_to_bill_qty')]; ?></td>
                </tr>
        	<?
            $i++;
            }
...
if($action=='populate_sewing_wages_mst_form_data')
{
	...
	foreach($result as $row){
		...
		echo "load_drop_down('requires/weekly_sewing_wages_controller','".$row[csf("company_id")]."_".$row[csf("location")]."_".$row[csf("floor_id")]."','load_drop_down_line','line_td' );\n"; 	
...
if($action=='populate_sewing_wages_dtls_form_data')
{
	list($mst_id,$dtls_id)=explode("_",$data);
	$sql = "select a.id,b.id as dtls_id,b.line_id,b.emp_id,b.emp_name,b.designation,b.order_id,b.buyer_id,b.apv_order_qty,b.cutting_bill_qty,b.previous_bill_qty,b.yet_to_bill_qty,b.style_ref,b.gmt_item,b.wo_rate,b.amount,b.deducted_emp,b.net_amount,b.rate_variables,b.cutting_bill_uom, b.rate_uom, b.previous_bill_uom, b.yet_to_bill_uom,b.deducted_qty , 
...
";
	...
	foreach($result as $row){
		...
		echo "document.getElementById('cbo_line_num').value = '".$row[csf("line_id")]."';\n";
	...
	echo "set_button_status(1, permission, 'fnc_weekly_sewing_wages_entry',1);";
...
{ 
	list($po_id,$job_id,$rate,$rate_var,$style,$items,$company,$location,$floor,$line,$bill_for,$final_bill,$from_date,$to_date,$supplier_id,$dtls_update_id)=explode("**",$data);
	
	

if($dtls_update_id)$dtls_update_id_con="and b.id not in($dtls_update_id)"; else $dtls_update_id_con=""; 		

	$seftyParcentArray= sql_select("select sewing_sefty_parcent from variable_settings_production where company_name='$company' and variable_list=30");

	$total_order=count(explode(',',$po_id));
	$deducted_parcent=$seftyParcentArray[0][csf("sewing_sefty_parcent")]; //$deducted_parcent assicen 5 for deduct cut qty;
	...
	if(str_replace("'","",$floor)==0){
		$floor_con_b="";
		$floor_con_c="";
	}
	else{
		$floor_con_b=" and b.floor_id=$floor";
		$floor_con_c=" and c.floor_id=$floor";
		
	}
//--------------------------------------------------------------------------	
	
	if($db_type==0){
		$sql = "select a.job_no,a.buyer_name,a.style_ref_no,GROUP_CONCAT(c.item_number_id) as item_number_id,GROUP_CONCAT(b.po_number) AS po_number,c.location,c.floor_id,c.sewing_line  from wo_po_details_master a,wo_po_break_down b,pro_garments_production_mst c where a.job_no=b.job_no_mst and b.id=c.po_break_down_id and b.id in($po_id) and a.id in($job_id) and c.item_number_id in($items) and c.location='$location' and c.production_type=5 and c.production_date BETWEEN '$from_date' and '$to_date' and c.sewing_line=$line and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  group by a.job_no,c.location,c.floor_id,c.sewing_line,a.buyer_name,a.style_ref_no"; //$floor_con_c
	}
	else
	{
		$sql = "select a.job_no,a.buyer_name,a.style_ref_no,LISTAGG( CAST(c.item_number_id as VARCHAR(4000)),',') WITHIN GROUP(ORDER BY c.item_number_id) AS item_number_id,LISTAGG( CAST(b.po_number as VARCHAR(4000)),',') WITHIN GROUP(ORDER BY b.po_number) AS po_number,c.location,c.floor_id,c.sewing_line  from wo_po_details_master a,wo_po_break_down b,pro_garments_production_mst c where a.job_no=b.job_no_mst and b.id=c.po_break_down_id and b.id in($po_id) and a.id in($job_id) and c.item_number_id in($items) and c.location='$location' and c.production_type=5 and c.production_date BETWEEN '$from_date' and '$to_date' and c.sewing_line=$line and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by a.job_no,c.location,c.floor_id,c.sewing_line,a.buyer_name,a.style_ref_no";//$floor_con_c
	}
...
//echo $sql; die;	
	...
	{ 
		...
		echo "document.getElementById('txt_order_no').value	= '".$po_num."';\n";
		...
		$item_id_arr=array_unique(explode(",",$row[csf("item_number_id")]));
		$item='';
		foreach($item_id_arr as $item_id)
		{
			if($item=='')$item=$garments_item[$item_id]; else $item.=','.$garments_item[$item_id]; 	
		}
		
		echo "document.getElementById('txt_gmt_item_name').value	= '".$item."';\n";
...
// and a.floor_id='$floor'	and b.line_id='$line' and a.location='$location' and b.emp_id='$supplier_id'
	...
	where 
		...
		and b.sewing_line=$line
		and b.production_type=5
		and b.status_active=1
		and b.is_deleted=0 
	group by 
		b.po_break_down_id,b.item_number_id";//and b.floor_id='$floor' 
...
$order_by_previous_bill_qty=return_field_value("sum(c.bill_qty)","pro_weekly_wages_bill_mst a, pro_weekly_wages_bill_dtls b, pro_weekly_wages_order_brk c","a.id=b.mst_id and b.id=c.weekly_wages_dtls_id and a.id=c.weekly_wages_mst_id and c.order_id ='".$row[csf("order_id")]."' and a.company_id='$company' and a.bill_for='$bill_for' and c.gmt_item_id='".$row[csf("item_id")]."' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $dtls_update_id_con");
...
//---------------------------------
	...
	//-------------------------------------------------------
	if(($tot_cut_qty+$tot_prev_deducted_qty) <= $tot_wo_qty && $tot_cut_qty!=0){	
		echo "document.getElementById('txt_cutting_bill_qty').value	= '".$tot_cut_qty."';\n";
		...
		echo "document.getElementById('txt_amount').value	= '".round($tot_cut_qty*($tot_avg_rate/$total_order))."';\n";
		echo "document.getElementById('txt_net_amount').value='".round($tot_cut_qty*($tot_avg_rate/$total_order))."';\n";
		...
		echo "document.getElementById('txt_previous_bill_qty').value	= '".$tot_prev_bill_qty."';\n";
		
		$yet_to_bill_qty=($yet_to_bill_qty-$tot_cut_qty);
		echo "document.getElementById('txt_yet_to_bill_qty').value	= '".round($yet_to_bill_qty)."';\n";
		echo "document.getElementById('txt_appv_wo_order_qty').value	= '".round($tot_wo_qty)."';\n";
...
{
	echo create_drop_down( "cbo_location", 160, "select location_name,id from lib_location where company_id='$data' and is_deleted=0  and status_active=1  order by location_name",'id,location_name', 1, '--- Select Location ---', 0, "load_drop_down( 'requires/weekly_sewing_wages_controller', this.value, 'load_drop_down_floor', 'floor_td' )"  );
exit();	
}
...
{
	echo create_drop_down( "cbo_floor_name", 160, "select floor_name,id from  lib_prod_floor where location_id='$data' and is_deleted=0  and status_active=1  order by floor_name",'id,floor_name', 1, '--- Select Floor ---', 0, "load_drop_down( 'requires/weekly_sewing_wages_controller', document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_location').value+'_'+this.value, 'load_drop_down_line', 'line_td' )" );
exit();	
}
...
if($action=="load_drop_down_line")
{
	...
	if($prod_reso_allocation==1)
	{
		...
		echo create_drop_down( "cbo_line_num", 160,$line_array,"", 1, "--- Select ---", $selected, "",0,0 );
	}
	else
	{
...
// master part--------------------------------------------------------------;
			$weekly_wages_bill_system_id=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '', 'WSW', date("Y",time()), 5, "select sys_number_prefix, sys_number_prefix_num from pro_weekly_wages_bill_mst where company_id=$cbo_company_id AND bill_for=30 and $year_cond=".date('Y',time())." order by id desc", "sys_number_prefix", "sys_number_prefix_num" ));
			...
			$field_array_mst="id,sys_number_prefix,sys_number_prefix_num,sys_number,company_id,bill_for,final_bill,location, division_id,department_id,shift,floor_id,week_from_date,week_to_date,inserted_by,insert_date,status_active,is_deleted";
			
			$data_array_mst="(".$mst_id.",'".$weekly_wages_bill_system_id[1]."',".$weekly_wages_bill_system_id[2].",'".$weekly_wages_bill_system_id[0]."',".$cbo_company_id.",".$cbo_bill_for.",".$cbo_final_bill.",".$cbo_location.",".$cbo_division.",".$cbo_department.",".$cbo_shift.",".$cbo_floor_name.",".$txt_week_date_from.",".$txt_week_date_to.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','1','0')";
...
//echo $hidden_order_break_down;die;
...
// details part--------------------------------------------------------------;
			...
			$deducted_amount=$deducted_qty*$deducted_rate;
			$txt_order_id=str_replace("'","",$txt_order_id);
			$data_array_dtls="(".$dtls_id.",".$update_id.",".$txt_emp_code.",".$txt_emp_name.",".$cbo_line_num.",'".$prod_reso_allocation."','".$txt_order_id."',".$cbo_buyer_name.",".$txt_style_ref_no.",".$txt_gmt_item_name.",".$txt_appv_wo_order_qty.",".$cbo_wages_rate_variables.",".$txt_cutting_bill_qty.",".$txt_wo_rate.",".$txt_amount.",".$tex_emp_deduct_amount.",".$txt_net_amount.",".$txt_previous_bill_qty.",".$txt_yet_to_bill_qty.",".$cbo_cutting_bill_uom.",".$cbo_wo_rate_uom.",".$cbo_previous_bill_qty_uom.",".$cbo_yet_to_bill_qty_uom.",'".$deducted_qty."','".$deducted_rate."','".$deducted_amount."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','1','0')";
...
//echo $rID2; die;	
			...
			if($db_type==0)
			{
				...
				{
					mysql_query("COMMIT");  
					echo "0**".$update_id."**".$txt_system_id;
				}
				else
				{
					mysql_query("ROLLBACK"); 
	...
	else if ($operation==1)   // Update Here
	{
		...
		if($db_type==0)
		{
			if($rID2==1 && $rID3==1){
				...
				mysql_query("ROLLBACK"); 
				//echo "10**".$update_id.'**'.$txt_system_id;
			}
		}
...
// details part--------------------------------------------------------------;
			...
			$txt_gmt_item_name=str_replace("'","",$txt_gmt_item_name);
			$txt_appv_wo_order_qty=str_replace("'","",$txt_appv_wo_order_qty);
			$deducted_qty=str_replace("'","",$txt_deducted_qty_hidden);
			$deducted_rate=str_replace("'","",$txt_wo_rate);
			$deducted_amount=$deducted_qty*$deducted_rate;
			$txt_order_id=str_replace("'","",$txt_order_id);
...
//echo $rID3.'='.$rID3; die;
			...
			if($db_type==0)
			{
				...
				{
					mysql_query("COMMIT");  
					echo "1**".$update_id.'**'.$txt_system_id;
				}
				else
				{
					mysql_query("ROLLBACK"); 
