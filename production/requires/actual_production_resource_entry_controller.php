                ...
                <tbody>
                	<tr class="general">
                    	...
                    	<?
							...
							echo create_drop_down( "cbo_order_type", 120, $order_type_arr,"",1, "--Select--", "","",0 );
						?>
                        </td>
                        <td>
                        ...
                        <td align="center">
                        	<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view (document.getElementById('cbo_company_name').value+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('cbo_year').value+'**'+document.getElementById('hide_prod_update_var').value+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('txt_date_from').value+'**'+document.getElementById('txt_date_to').value+'**'+'<? echo $sid; ?>'+'**'+'<? echo $from_date; ?>'+'**'+'<? echo $to_date; ?>'+'**'+'<? echo $mst_dtls_id; ?>'+'**'+document.getElementById('cbo_order_type').value, 'create_job_no_search_list_view', 'search_div', 'actual_production_resource_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1)'); set_all();" style="width:60px;" />
                    </td>
                    </tr>
                    <tr>
		...
		function fnc_Ac_Production_Resource_color(operation)
		{
			...
			$("#tbl_list_search").find('tbody tr').each(function()
			{
				if($(this).find('input[name="tr_selection[]"]').is(":checked"))
				{
					...
					j++;
					dataString+='&po_data_' + j + '=' + po_data + '&target_' + j + '=' + target+ '&operator_' + j + '=' + operator+ '&helper_' + j + '=' + helper+ '&working_hour_' + j + '=' + working_hour+ '&actual_smv_' + j + '=' + actual_smv+ '&effi_per_' + j + '=' + effi_per+ '&iron_man_' + j + '=' + iron_man+ '&qi_' + j + '=' + qi;
				}
			});
			...
			var variable_settings=<? echo $variable_settings; ?>;

			var data="action=save_update_delete_color_size&operation="+operation+dataString+"&mst_dtls_id="+mst_dtls_id+"&system_id="+system_id+"&variable_settings="+variable_settings+"&tot_row="+j;
			//alert(data)
...
if($action=='create_job_no_search_list_view')
{
	...
	if($variable_settings == 1) // order level
	{
		...
		<form>
	        ...
	        <br/>
	        <br/>
	        <table width="1000" cellspacing="0" cellpadding="0" style="border:none" align="left">
	                <tr>
	                    <td align="center" height="30" valign="bottom" >
	                        <div style="width:100%">
	                            <div style="width:50%;" align="center">
...
{  	$flag=0;
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'",'',$h_dtl_mst_id)=="")
		{
			...
			$field_array3="id, mst_id, from_date, to_date, man_power, operator, helper,iron_man,qi, line_chief, active_machine, target_per_hour, working_hour, capacity,target_efficiency, total_smv, inserted_by, insert_date, is_deleted";

			$total_smv=((str_replace("'","",$txt_man_power)*str_replace("'","",$txt_working_hour))*60);
			$data_array3="(".$id3.",".$id.",".$txt_date_from.",".$txt_date_to.",".$txt_man_power.",".$txt_operator.",".$txt_helper.",".$txt_iron_man.",".$txt_qi.",".$txt_Line_chief.",".$txt_active_machine.",".$txt_target_hour.",".$txt_working_hour.",".$txt_capacity.",".$txt_efficiency.",".$total_smv.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',0)";
		...
		$rID=1;	$rID2=1; $rID3=1; $rID_time=false;
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		if(str_replace("'",'',$system_id)!="")
		{
			...
			$field_array_up3="from_date*to_date*man_power*operator*helper*iron_man*qi*line_chief*active_machine*target_per_hour*working_hour*capacity*target_efficiency*total_smv*updated_by*update_date*is_deleted";

			$total_smv=((str_replace("'","",$txt_man_power)*str_replace("'","",$txt_working_hour))*60);
			$data_array_up3="".$txt_date_from."*".$txt_date_to."*".$txt_man_power."*".$txt_operator."*".$txt_helper."*".$txt_iron_man."*".$txt_qi."*".$txt_Line_chief."*".$txt_active_machine."*".$txt_target_hour."*".$txt_working_hour."*".$txt_capacity."*".$txt_efficiency."*".$total_smv."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*0";
...
if ($action=="adjustment_minint_popup")
{
	...
	<form name="searchdescfrm"  id="searchdescfrm">
		<fieldset style="width:920px;margin-left:10px">
			...
			if($poId!="")
			{
				$sql = "SELECT a.STYLE_REF_NO,a.job_no from wo_po_details_master a,wo_po_break_down b where a.id=b.job_id and b.id in($poId) and a.style_ref_no is not null";

				$res = sql_select($sql);
				foreach ($res as $val)
				{
					$style_arr[$val['JOB_NO']] = $val['JOB_NO']." / ".$val['STYLE_REF_NO'];
				}
			}
	...
	<script>
	    ...
	    function mankindpopup(row)
		{
			...
			let page_link='actual_production_resource_entry_controller.php?action=employee_popup&h_dtl_mst_id='+h_dtl_mst_id+'&actual_emplloyee_data='+actual_emplloyee_data;
