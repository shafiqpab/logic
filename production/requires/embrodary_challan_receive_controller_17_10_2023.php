...
{
	$print_report_format=return_field_value("format_id","lib_report_template","template_name ='".$data."' and module_id=7 and report_id in(53) and is_deleted=0 and status_active=1");		 
	...
	?>
 		<table cellpadding="0" cellspacing="0" width="700" border="1" rules="all" class="rpt_table">
			<thead>
				...
				<th width="150">Cut/Print Rate</th>
					...
					foreach ($po_data as $itm_id=>$itm_data) 
					{
						foreach ($itm_data as $color_id=>$v) 
						{
							...
							<tr bgcolor="<? echo $bgcolor;?>">
								...
								<td align="right"><?=$prev_rate_array[ $po_id][$itm_id][$color_id]['cost_per_pcs'];?></td>
...
if($action=="create_bundle_search_list_view")
{
	...
	$sql="SELECT c.cut_no, c.bundle_no, d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.barcode_no, c.production_qnty as qty, e.po_number,c.delivery_mst_id from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e where  a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and a.company_id=$company and e.po_number like '$txt_order_no' and c.bundle_no like '$bundle_no' $jobCon $cutCon and c.production_type=2 and a.embel_name=2 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active =1 and d.is_deleted=0 and e.status_active =1 and e.is_deleted=0 order by c.cut_no,  length(c.bundle_no) asc, c.bundle_no asc";
	// echo $sql;die();
	...
	$scanned_bundle_arr=return_library_array( "SELECT b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=3 and a.embel_name=2 $cutCon_a and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 $po_id_cond",'bundle_no','bundle_no');
...
if($action=="create_rescan_bundle_search_list_view")
{
	...
	$receive_sql="select c.barcode_no,a.issue_challan_id , sum(c.production_qnty) as qty,sum(c.reject_qty) as reject_qty,sum(c.replace_qty) as replace_qty from pro_gmts_delivery_mst a,pro_garments_production_dtls c where a.id=c.delivery_mst_id and c.production_type=3 and a.embel_name=2 and a.status_active=1 and a.is_deleted=0  and c.status_active=1 and c.is_deleted=0 $bundle_nos_cond group by c.barcode_no,a.issue_challan_id";
	//echo $receive_sql;die;
	$receive_result = sql_select($receive_sql);
	foreach ($receive_result as $row){ 
		$receive_qty_arr[$row[csf('issue_challan_id')]][$row[csf('barcode_no')]]=$row[csf('qty')];  //+$row[csf('replace_qty')] 
...
if($action=="populate_bundle_data_rescan")
{
	...
	else
	{
	 ...
	 $bundle_nos_cond and c.production_type=3 and a.embel_name=2 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0  and a.issue_challan_id=".$mst_id."";
...
if($action=="load_mst_data_bundle_scan")
{
	...
	$sql_mst_data=sql_select("select a.sys_number, a.company_id,a.location_id, a.embel_name, a.embel_type, a.serving_company, a.floor_id, a.organic, a.production_source, a.working_company_id, a.working_location_id, a.remarks
	from pro_gmts_delivery_mst a,pro_garments_production_dtls c where a.id=c.delivery_mst_id and c.production_type=2
	and a.embel_name=2 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.id=".$challan_id." $bundle_nos_cond
	group by a.sys_number, a.company_id,a.location_id, a.embel_name, a.embel_type, a.serving_company, a.floor_id, a.organic, a.production_source, a.working_company_id, a.working_location_id, a.remarks");
		...
		if(count($result)>0)
		{
			...
			{ 
				...
				if($production_qty>0)
				{
					$msg=2;
					if($datastr!="") $datastr.=",";
					$datastr.=$row[csf('id')]."*".$row[csf('sys_number')]."*".$row[csf('bundle_no')];
				}
			}
		}
	}
...
if($action=="populate_bundle_data")
{
	...
	$scanned_bundle_arr=return_library_array( "select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=3 and a.embel_name=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $bundle_nos_cond",'bundle_no','bundle_no');
	...
	//$sql="select d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, MAX($year_field) as year, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no, c.production_qnty as qty, e.po_number from pro_gmts_delivery_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name=$ex_data[3] and a.id=c.delivery_mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.bundle_no in ($bundle_nos) and c.production_type=2 and embel_name=2 and c.status_active=1 and c.is_deleted=0 group by d.id, e.id, f.job_no_prefix_num, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no, c.production_qnty, e.po_number";
	
		$sql="SELECT max(c.id) as prdid, d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, MAX($year_field) as year, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no,c.barcode_no, c.production_qnty as qty, e.po_number from pro_gmts_delivery_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name=$ex_data[3] and a.id=c.delivery_mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.production_type=2 and a.embel_name=2 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active =1 and d.is_deleted=0 and e.status_active =1  and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 $recbundle_nos_cond $challan_cond group by d.id, e.id, f.job_no_prefix_num, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no,c.barcode_no, c.production_qnty, e.po_number order by c.cut_no, length(c.bundle_no) asc, c.bundle_no asc";
	...
	{ 
		...
		?>
			<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i;?>"> 
                ...
                <td width="50" id="RepQty_<? echo $i; ?>" align="right">
				<input type="text" class="text_boxes_numeric" name="replaceQty[]" id="replaceQty_<? echo $i; ?>" style="width:40px" onBlur="calculate_qcpasss_rescan(<? echo $i; ?>)" /></td>
...
if ($action == "populate_bundle_data_update")
{
	...
	$issue_sql="select c.bundle_no,c.barcode_no, c.production_qnty as qty from pro_garments_production_mst a,pro_garments_production_dtls c where a.id=c.mst_id and c.delivery_mst_id=$issue_challan_id and c.production_type=2 and a.embel_name=2 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 $bundle_nos_cond";
	...
	$receive_sql="select c.barcode_no, sum(c.production_qnty) as qty from pro_gmts_delivery_mst a,pro_garments_production_dtls c where a.id=c.delivery_mst_id and c.production_type=3 and a.embel_name=2 and a.issue_challan_id=$issue_challan_id and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $bundle_nos_cond group by c.barcode_no";
	...
	{ 
		...
		?>
			<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i;?>"> 
                ...
                <td width="50" id="RepQty_<? echo $i; ?>" align="right">
				<input type="text" class="text_boxes_numeric" name="replaceQty[]" id="replaceQty_<? echo $i; ?>" style="width:40px" value="<? echo $row[csf('replace_qty')];?>" onBlur="calculate_qcpasss_rescan(<? echo $i; ?>)" <?php echo $disable_replace; ?>/>
				</td>
                <td width="50" id="QcQty_<? echo $i; ?>" align="right"><? echo $row[csf('qty')]; ?>&nbsp;</td>
...
{
	$bundle_nos=return_library_array( "select b.barcode_no, b.barcode_no from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id and a.delivery_mst_id=$data and b.production_type=2 and a.embel_name=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","barcode_no","barcode_no");
...
if($action=="bundle_nos_update")
{
	...
	$bundle_sql="SELECT b.barcode_no from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id and a.delivery_mst_id=$data and b.production_type=3 and a.embel_name=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 ";
	...
	<div style="width:100%;max-height:180px; overflow-y:scroll" id="sewing_production_list_view" align="left">
		...
		<?php  
			...
			foreach($sqlResult as $selectResult)
			{
 			...
 			?>
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_receive_form_data','requires/embrodary_challan_receive_controller');" > 
...
?>	
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="370" class="rpt_table">
		...
		foreach($sqlResult as $row)
		{
			if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
		?>
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data(<? echo $row[csf('id')]; ?>,'populate_issue_form_data','requires/embrodary_challan_receive_controller');"> 
				<td width="30"><? echo $i; ?></td>
...
if($action=="populate_issue_form_data")
{
	...
	{ 
		...
		$dataArray=sql_select("select SUM(CASE WHEN production_type=2 and embel_name=2 THEN production_quantity END) as totalcutting,SUM(CASE WHEN production_type=3 and embel_name=2 THEN production_quantity ELSE 0 END) as totalprinting from pro_garments_production_mst WHERE po_break_down_id=".$result[csf('po_break_down_id')]." and item_number_id=".$result[csf('item_number_id')]."  and country_id=".$result[csf('country_id')]." and is_deleted=0");
 		foreach($dataArray as $row)
		{ 
 			echo "$('#txt_issue_qty').val('".$row[csf('totalcutting')]."');\n";
			echo "$('#txt_cumul_receive_qty').attr('placeholder','".$row[csf('totalprinting')]."');\n";
	...
	$sql_order=sql_select("SELECT a.buyer_name,a.style_ref_no,b.po_quantity from wo_po_break_down b,wo_po_details_master a where a.job_no=b.job_no_mst and b.id=".$result[csf('po_break_down_id')]." and a.status_active=1 and a.is_deleted=0 and b.status_active =1  and b.is_deleted=0");
	foreach($sql_order as $inf)
	{
		...
		else if( $variableSettings==3 ) //color and size level
		{
			...
			$dtlsData = sql_select("select a.color_size_break_down_id,
										...
										from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.embel_name=2 and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in(2,3) group by a.color_size_break_down_id");	
...
if($action=="populate_receive_form_data")
{
	//production type=2 come from array
	$poNumber_arr=return_library_array( "SELECT id, po_number from   wo_po_break_down",'id','po_number');
	$sqlResult =sql_select("select id, garments_nature, challan_no, po_break_down_id, item_number_id, country_id, production_source, serving_company, location, embel_name, embel_type, production_date, production_quantity, production_source, production_type, entry_break_down_type, break_down_type_rej, production_hour, sewing_line, supervisor, carton_qty, remarks, floor_id, alter_qnty, reject_qnty, total_produced, yet_to_produced,get_entry_no,get_entry_date from pro_garments_production_mst where id='$data' and production_type='3' and status_active=1 and is_deleted=0 order by id");
  	//echo "sdfds".$sqlResult;die;
	foreach($sqlResult as $result)
	{ 
 		echo "$('#txt_receive_qty').val('".$result[csf('production_quantity')]."');\n";
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$txt_system_id)=="")
		{
			...
			if($db_type==0) $year_cond="YEAR(insert_date)"; 
 			...
 			$new_sys_number = explode("*", return_next_id_by_sequence("", "pro_gmts_delivery_mst",$con,1,$cbo_company_name,'BRE',0,date("Y",time()),0,0,3,$cbo_embel_name,0 ));


			$field_array_delivery="id, sys_number_prefix, sys_number_prefix_num, sys_number, company_id, production_type, location_id, delivery_basis, embel_name, embel_type, production_source, serving_company, floor_id, boe_mushak_challan_no, boe_mushak_challan_date, organic, delivery_date,issue_challan_id,body_part,working_company_id,working_location_id,remarks, inserted_by, insert_date";
			$mst_id = return_next_id_by_sequence(  "pro_gmts_delivery_mst_seq", "pro_gmts_delivery_mst", $con );
			$data_array_delivery="(".$mst_id.",'".$new_sys_number[1]."','".(int)$new_sys_number[2]."','".$new_sys_number[0]."', ".$cbo_company_name.",3,".$txt_location_id.",".$delivery_basis.",".$cbo_embel_name.",".$cbo_embel_type.",".$cbo_source.",".$txt_embl_company_id.",".$txt_location_id.",".$txt_boe_mushak_challan_no.",".$txt_boe_mushak_challan_date.",".$txt_organic.",".$txt_issue_date.",".$txt_issue_challan_id.",".$cbo_body_part.",".$cbo_working_company_name.",".$cbo_working_location.",".$txt_remark_bundle.",".$user_id.",'".$pc_date_time."')";
		...
		else
		{
			...
			$field_array_delivery="location_id*delivery_basis*embel_name*embel_type*production_source*serving_company*floor_id*boe_mushak_challan_no*boe_mushak_challan_date*organic*delivery_date*body_part*remarks*updated_by*update_date";
			$data_array_delivery="".$txt_location_id."*".$delivery_basis."*".$cbo_embel_name."*".$cbo_embel_type."*".$cbo_source."*".$txt_embl_company_id."*".$txt_floor_id."*".$txt_boe_mushak_challan_no."*".$txt_boe_mushak_challan_date."*".$txt_organic."*".$txt_issue_date."*".$cbo_body_part."*".$cbo_working_company_name."*".$cbo_working_location."*".$txt_remark_bundle."*".$user_id."*'".$pc_date_time."'";
		...
		$receive_sql="SELECT a.production_type,c.barcode_no,c.bundle_no,c.production_qnty from pro_garments_production_mst a,pro_garments_production_dtls c where a.id=c.mst_id and a.production_type in(2,3) and  a.embel_name=2 and c.barcode_no  in ($bundle)  and c.production_type in(2,3) and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and (c.is_rescan=0 or c.is_rescan is null)"; 
		$receive_result = sql_select($receive_sql);
		foreach ($receive_result as $row)
		{		
			if($row['PRODUCTION_TYPE']==3) // rcv
			{
				$duplicate_bundle[$row[csf('bundle_no')]]=$row[csf('bundle_no')];
			}
			else // issue
			{
				$issue_qty_arr[$row[csf('bundle_no')]] += $row[csf('production_qnty')];
			}
		}
		...
		if(str_replace("'","",$delivery_basis)==3)
		{
			...
			$field_array_mst="id, delivery_mst_id,cut_no, company_id, garments_nature, challan_no, po_break_down_id, item_number_id, country_id, production_source, serving_company, location, embel_name, embel_type, production_date, production_quantity,reject_qnty,replace_qty, production_type, entry_break_down_type, floor_id,cost_of_fab_per_pcs,cost_per_pcs, inserted_by, insert_date,get_entry_no, get_entry_date";
			...
			// echo "10**<pre>";print_r($dtlsArr);die;
			/* ======================================================================== /
			...
			if($wip_valuation_for_accounts==1)
			{
				...
				/* ================================= get fabric cost =================================== */			
				$bundle_chk_arr=return_library_array( "SELECT b.bundle_no, b.bundle_no as bundle_nos from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id  and b.bundle_no in($emb_bundle) and b.production_type=3 and a.embel_name=1", "bundle_no", "bundle_nos" );
			...
			foreach($mstArr as $orderId=>$orderData)
			{
				foreach($orderData as $gmtsItemId=>$gmtsItemIdData)
				{
					foreach($gmtsItemIdData as $countryId=>$qty)
					{
						...
						// ========== emb rate =====================
						$emb_rate = ($emb_budget_qty_arr[$orderId][2]) ? $emb_budget_amount_arr[$orderId][2] / $emb_budget_qty_arr[$orderId][2] : 0;
			...
			{
				if($qty <= $issue_qty_arr[$bundle_no])
				{
					...
					if($wip_valuation_for_accounts==1)
					{
						...
						// ========== emb rate =====================
						$emb_rate = ($emb_budget_qty_arr[$colorSizedData[0]][2]) ? $emb_budget_amount_arr[$colorSizedData[0]][2] / $emb_budget_qty_arr[$colorSizedData[0]][2] : 0;
						...
						// echo "10**(".$amount."/".$prod_qty.") + ".$emb_rate."<br>SS";die;
						$cost_per_pcs = number_format($cost_per_pcs,$dec_place[3],'.','');
			...
			{ 
				...
				if($$actualReject!="")
				{
					...
					for($rls=0;$rls<count($actual_reject_info); $rls++)
					{
						...
						$data_array_defect.="(".$dft_id.",".$gmtsMstId.",64,".$$orderId.",3,".$defectPointId.",'".$defect_qty."','".$$barcodeNo."',".$$colorSizeId.",1,".$user_id.",'".$pc_date_time."')";
						//$dft_id++;
						$dft_id = return_next_id_by_sequence(  "pro_gmts_prod_dft_seq",  "pro_gmts_prod_dft", $con );
					}
				}
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$field_array_delivery="location_id*embel_name*embel_type*production_source*serving_company*floor_id*boe_mushak_challan_no*boe_mushak_challan_date*organic*delivery_date*body_part*working_company_id*working_location_id*remarks*updated_by*update_date";
		$data_array_delivery="".$txt_location_id."*".$cbo_embel_name."*".$cbo_embel_type."*".$cbo_source."*".$txt_embl_company_id."*".$txt_floor_id."*".$txt_boe_mushak_challan_no."*".$txt_boe_mushak_challan_date."*".$txt_organic."*".$txt_issue_date."*".$cbo_body_part."*".$cbo_working_company_name."*".$cbo_working_location."*".$txt_remark_bundle."*".$user_id."*'".$pc_date_time."'";
		...
		if(str_replace("'","",$delivery_basis)==3)
		{
			...
			//$dtls_id=return_next_id("id", "pro_garments_production_dtls", 1);
            //  echo "20***"."abc";
			$field_array_mst="id, delivery_mst_id,cut_no, company_id, garments_nature, challan_no, po_break_down_id, item_number_id, country_id, production_source, serving_company, location, embel_name, embel_type, production_date, production_quantity,reject_qnty,replace_qty, production_type, entry_break_down_type, remarks, floor_id,cost_of_fab_per_pcs,cost_per_pcs, inserted_by, insert_date, get_entry_no, get_entry_date";
			// echo "10**";
			 //  echo "5***"."abc";
			$non_delete_arr=production_validation($mst_id,'3_2');
			$issue_data_arr=production_data($mst_id,'3_2');
			...
			{ 	
				...
				$last_operation=gmt_production_validation_script( 92, 2, $$colorSizeId, $$cutNo, $production_squence);
				echo "10**".print_r($last_operation);die;
			...
			{
				$last_operation=array();
				$last_operation=gmt_production_validation_script( 92, 2, $issue_data_arr[trim($bi)][csf('color_size_break_down_id')], $issue_data_arr[trim($bi)][csf('cut_no')],$production_squence);
			...
			$delete = execute_query("DELETE FROM pro_garments_production_mst WHERE delivery_mst_id=$txt_system_id and production_type=3 and embel_name=2");
			...
			$delete_bundle = execute_query("DELETE FROM pro_cut_delivery_color_dtls WHERE delivery_mst_id=$txt_system_id and production_type=3 and embel_name=2");
			$challanrID=sql_update("pro_gmts_delivery_mst",$field_array_delivery,$data_array_delivery,"id",$txt_system_id,1);
			if($data_array_mst!='') $rID=sql_insert("pro_garments_production_mst",$field_array_mst,$data_array_mst,1); else $rID=1;
			if($data_array_dtls!='') $dtlsrID=sql_insert("pro_garments_production_dtls",$field_array_dtls,$data_array_dtls,1); else $dtlsrID=1;
			//$bundlerID=sql_insert("pro_cut_delivery_color_dtls",$field_array_bundle,$data_array_bundle,1);
		...
		<legend>Enter search words</legend>           
            <table cellpadding="0" cellspacing="0" width="700" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
                    <td align="center">
                    	<?
							echo create_drop_down( "cbo_embel_type", 160, $emblishment_embroy_type,"", 1, "--- Select Printing ---", $selected, "" );  
						?>       
                    </td>
            		...
            		<td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_embel_type').value+'_'+document.getElementById('txt_company_id').value+'_'+document.getElementById('txt_cutt_no').value+'_'+document.getElementById('cbo_string_search_type').value, 'create_challan_search_list_view', 'search_div', 'embrodary_challan_receive_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                     </td>
...
if($action=="create_challan_search_list_view")
{
	...
	if($data[4]==1)
	{
		if($data[0]!=""){ $search_field_cond=" and a.sys_number_prefix_num like '$search_string' ";} else {$search_field_cond="";}
		if($data[3]!=""){ $cut_no=" and b.cut_no like '$search_string_cut' ";} else {$cut_no="";}
        ...
        <?
        	...
        	?>
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $row[csf('id')]."**".$row[csf('issue_challan_id')]; ?>');"> 
                    ...
                    <td width="80"><p><? echo $emblishment_embroy_type[$row[csf('embel_type')]]; ?></p></td>               
...
{
	$data_array=sql_select("SELECT a.company_id,a.id,
	a.sys_number,
	a.embel_type,
	a.embel_name,
	a.production_source,
	...
	a.delivery_basis,
	a.floor_id,
	a.boe_mushak_challan_no,
	a.boe_mushak_challan_date,
	a.organic,
	a.delivery_date,
	a.issue_challan_id,
	a.body_part,
...
//echo $data_array;
	...
	{ 
		...
		//echo "load_drop_down( 'requires/embrodary_challan_receive_controller', ".$row[csf('production_source')].", 'load_drop_down_embro_issue_source', 'emb_company_td' );\n";
		echo "$('#txt_embl_company').val('".$serv_comp."');\n";
		
		echo "$('#txt_embl_company_id').val('".$row[csf('serving_company')]."');\n";
		echo "load_drop_down( 'requires/embrodary_challan_receive_controller', '".$row[csf('working_company_id')]."', 'load_drop_down_working_location', 'working_location_td' );\n";
		echo "$('#txt_location_name').val('".$location."');\n";
		echo "$('#txt_location_id').val('".$row[csf('location_id')]."');\n";
		//echo "load_drop_down( 'requires/embrodary_challan_receive_controller', ".$row[csf('location_id')].", 'load_drop_down_floor', 'floor_td' );\n";
		...
		echo "disable_enable_fields('cbo_company_name*cbo_emb_company*txt_location_name*txt_floor_name',1);\n";
...
if($action=="emblishment_issue_print")
{
	...
	/* $order_array=array();
	$order_sql="select a.job_no, a.buyer_name,a.style_ref_no,a.style_description, b.id, b.po_number, b.po_quantity from  wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.status_active=1 and a.is_deleted=0 and b.status_active =1  and b.is_deleted=0";
...
<div style="width:930px;">
    <table width="900" cellspacing="0" align="right">
        ...
        <?
			...
			if($data[2]==3)
			{
				...
				and a.id=b.mst_id and b.color_size_break_down_id=d.id and b.production_type=3 and b.status_active=1 and b.is_deleted=0 and d.status_active =1
				and d.is_deleted=0 
			...
			else
			{
				...
				and c.id=a.mst_id  and a.color_size_break_down_id=b.id and b.production_type=3 and a.status_active=1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 
    ...
    <table align="right" cellspacing="0" width="900"  border="1" rules="all" class="rpt_table" style=" margin-top:20px;" >
        ...
        <br>
		 <?
           echo signature_table(273, $data[0], "1000px"); 
         ?>
	</div>
	</div>
...
if($action=="load_mst_data")
{
	...
	a.production_source
	from  pro_gmts_delivery_mst a,pro_garments_production_dtls c where   a.id=c.delivery_mst_id and c.bundle_no in ('$ex_data')  and c.production_type=2
	and a.embel_name=2 and a.status_active=1 and a.is_deleted=0  and c.status_active=1 and c.is_deleted=0
	...
	foreach($sql_mst_data as $val)
	{
		...
		echo "$('#txt_floor_id').val(".$val[csf('floor_id')].");\n";
		echo "$('#txt_location_id').val(".$val[csf('location_id')].");\n";
		echo "disable_enable_fields('cbo_company_name*txt_location_name*txt_floor_name*txt_floor_nameo',1);\n";
	}
	exit();
}
			...
			<legend>Enter search words</legend>           
				<table cellpadding="0" cellspacing="0" width="700" border="1" rules="all" class="rpt_table">
					...
					<tr class="general">
						...
						<td align="center">
							<?
								echo create_drop_down( "cbo_embel_type", 160, $emblishment_embroy_type,"", 1, "--- Select Embroidery ---", $selected, "" );  
							?>       
						</td>
						<td align="center" id="search_by_td">				
							<input type="text" style="width:130px" class="text_boxes"  name="txt_search_common" id="txt_search_common" />	
						...
						<td align="center">
							<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_embel_type').value+'_'+document.getElementById('cbo_company_id').value+'_'+<? echo $delivery_basis; ?>, 'create_issue_challan_search_list_view', 'search_div', 'embrodary_challan_receive_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
						 </td>
...
if($action=="create_bundle_challan_search_list_view")
{
	...
	$color_arr=return_library_array( "select id, color_name from lib_color", "id", "color_name");
	$country_arr=return_library_array( "select id, country_name from lib_country",'id','country_name');
	$scanned_bundle_arr=return_library_array( "select bundle_no, bundle_no from pro_cut_delivery_color_dtls where production_type=3 and embel_name=2 and status_active=1 and is_deleted=0",'bundle_no','bundle_no');
	
	$sql="select d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.bundle_no, c.production_qnty as qty, e.po_number from  pro_gmts_delivery_mst a,pro_cut_delivery_color_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e where   a.id=c.delivery_mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and a.company_id=$company and e.po_number like '$txt_order_no' and a.sys_number_prefix_num like '$challan_no' and c.production_type=2 and c.embel_name=2 and a.status_active=1 and a.is_deleted=0  and c.status_active=1 and c.is_deleted=0";
	//echo $sql;die;
	$result = sql_select($sql);
...
if($action=="create_bundle_search_list_view")
{
	...
	$sql="select d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.bundle_no, c.production_qnty as qty, e.po_number from  pro_gmts_delivery_mst a,pro_cut_delivery_color_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e where  a.id=c.delivery_mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and a.company_id=$company and e.po_number like '$txt_order_no' and c.bundle_no like '$bundle_no' and c.production_type=2 and c.embel_name=2 and a.status_active=1 and a.is_deleted=0  and c.status_active=1 and c.is_deleted=0";
	//echo $sql;die;
	$result = sql_select($sql);
...
if($action=="create_issue_challan_search_list_view")
{
	...
	if($data[1]==0) $print_type_cond=""; else $print_type_cond=" and a.embel_type=$data[1]";
	$company_id =$data[2];
	$search_field_cond=" and a.sys_number_prefix_num like '$search_string'";
	...
	$delivery_basis=return_field_value("cut_panel_delevery","variable_settings_production","company_name=$company_id and variable_list=32 and status_active=1 and is_deleted=0");
		...
		<table width="900" cellspacing="0" align="left">
			...
			if($db_type==2) $group_concat="  listagg(cast(a.cut_no AS VARCHAR2(4000)),',') within group (order by a.cut_no) as cut_no" ;
				...
				if($data[2]==3)
				{
					...
					where a.delivery_mst_id ='$data[1]' 
					and e.id=f.job_id and f.id=a.po_break_down_id and a.id=b.mst_id and b.color_size_break_down_id=d.id and b.status_active=1 and b.is_deleted=0 and d.status_active =1 
					and d.is_deleted=0 
...
if($action=="embrodary_color_wise_print_2")
{
	extract($_REQUEST);
	$data=explode('*',$data);
	// print_r ($data);die;
	...
	$sql="SELECT id, sys_number_prefix, sys_number_prefix_num, sys_number, company_id, production_type, location_id, delivery_basis, embel_name, embel_type,
	production_source, serving_company, floor_id, remarks, organic, delivery_date,body_part,working_company_id,working_location_id from pro_gmts_delivery_mst where production_type=3 and id='$data[1]' and 
	status_active=1 and is_deleted=0 ";
	$dataArray=sql_select($sql);
	?>
	<div style="width:1100px;">
	    <table width="900" cellspacing="0" align="left">
	        ...
	        <?
				...
				// base on Embel. Name
						...
						/*$sql="SELECT b.cut_no, sum(b.production_qnty) as production_qnty,a.po_break_down_id,a.country_id,b.bundle_no, d.color_number_id,d.size_number_id,
						...
						group by b.cut_no,a.po_break_down_id,a.country_id,b.bundle_no,d.color_number_id,d.size_number_id
						order by a.po_break_down_id,d.color_number_id ";*/




					$sql="SELECT b.cut_no, b.production_qnty, a.country_id, d.color_number_id, d.size_number_id, e.style_ref_no, e.style_description, e.job_no, e.buyer_name,f.po_number,f.id as po_id, f.po_quantity,b.bundle_no, f.grouping
					from 
				...
				$cut_lay_arr=array();
				$lay_sql="SELECT a.cutting_no, b.order_id, b.order_cut_no, b.color_id, c.country_id from ppl_cut_lay_mst a, ppl_cut_lay_dtls b,ppl_cut_lay_bundle c where a.id=b.mst_id and a.id=c.mst_id and b.id=c.dtls_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $cut_no_cond";
				// echo $lay_sql;die;
	    ...
	    <tr bgcolor="#dddddd" align="center">
	        ...
	        <tr bgcolor="#dddddd" align="center">
				...
				<th width="70" align="center">IR No</th>
	            <th width="80" align="center">Country</th>
	            <th width="80" align="center">Color</th>
...
if($action=="embrodary_color_wise_print_5")
{
	extract($_REQUEST);
	$data=explode('*',$data);
	//print_r ($data);
	...
	$sql="select id, sys_number_prefix, sys_number_prefix_num, sys_number, company_id, production_type, location_id, delivery_basis, embel_name, embel_type,
	production_source, serving_company, floor_id,remarks, boe_mushak_challan_no, boe_mushak_challan_date, organic, delivery_date,body_part,working_company_id,working_location_id from pro_gmts_delivery_mst where production_type=3 and id='$data[1]' and 
	status_active=1 and is_deleted=0 ";
	// echo $sql;die;
	$dataArray=sql_select($sql);
...
if($action=="embrodary_color_wise_print_6")
{
	extract($_REQUEST);
	$data=explode('*',$data);
	$company_library=return_library_array( "select id, company_name from lib_company", "id", "company_name");
	...
	$sql="select id, sys_number_prefix, sys_number_prefix_num, sys_number, company_id, production_type, location_id, delivery_basis, embel_name, embel_type,sys_number,issue_challan_id,
	production_source, serving_company, floor_id,remarks, boe_mushak_challan_no, boe_mushak_challan_date, organic, delivery_date,body_part,working_company_id,working_location_id from pro_gmts_delivery_mst where production_type=3 and id='$data[1]' and 
	status_active=1 and is_deleted=0 ";
	...
	$sql="SELECT b.cut_no, b.production_qnty, a.country_id, d.color_number_id, d.size_number_id, e.style_ref_no, e.style_description, e.job_no, e.buyer_name,f.po_number,f.id as po_id, f.po_quantity,b.bundle_no,f.grouping,d.item_number_id,
	a.po_break_down_id,b.reject_qty,a.get_entry_date,a.get_entry_no
				from 
					pro_garments_production_mst a, 
					pro_garments_production_dtls b,
					wo_po_color_size_breakdown d, 
				...
				foreach($result as $rows)
				{
					...
					$key=$rows[csf('country_id')].$rows[csf('color_number_id')];
					...
					$orderCutArr[$key][$rows[csf('order_cut_no')]]=$rows[csf('order_cut_no')];
					$productionQtyArr[$key]+=$rows[csf('production_qnty')];
					$sizeQtyArr[$key][$rows[csf('size_number_id')]]['ok_qty']+=$rows[csf('production_qnty')];
					$sizeRejArr[$key][$rows[csf('size_number_id')]]['rej_qty']+=$rows[csf('reject_qty')];
					$bundleArr[$key][$rows[csf('bundle_no')]]=$rows[csf('bundle_no')];
					$color_array[$rows[csf('color_number_id')]]=$rows[csf('color_number_id')];
...
<div style="width:1100px;">
    <table width="900" cellspacing="0" align="left">
        ...
        <tr>
            <td colspan="6" align="center" style="font-size:20px"><u><strong>Embroidery Receive Entry Challan</strong></u></td>
        </tr>
        ...
        <tr>
			...
			$order_qty = "";
			   foreach($dataArr as $key=>$row){
				$order_no .= $row['po_number'].',';
                
			   }
			 

			?>
			<td width="125"><strong>Order No :</strong></td><td width="175px"><? echo  trim($order_no,','); ?></td>
            <?
				foreach($dataArr as $key=> $row)
				{
					
					$order_qnty=return_field_value("f.po_quantity"," wo_po_break_down","id=".$row[csf("po_break_down_id")],"f.po_quantity");
				}
                //echo "<pre>11";
				//print_r($order_qnty);die;
			?>
			<?php
			$job=array();
			$style_ref_no="";
			...
			$order ="";
			  foreach($dataArr as $key=>$val){
				...
				$item = $val['item_number_id'];
				$val['po_quantity'] = $val['po_quantity'];
               
				$total_po += $val['po_quantity'];
			...
			?>
        	<td width="125"><strong>Style Ref. :</strong></td><td width="175px"><? echo $style_ref_no; ?></td>
        </tr>
        <tr>
        	<td><strong>Order Qty :</strong></td><td><? echo $total_po; ?></td>
	...
	foreach($dataArr as $key=>$val){
	  ...
	  $get_entry_no = $val['get_entry_no'];
	  $get_entry_date = $val['get_entry_date'];
	}
			?>
			...
			<td>
				<? 
					...
					echo change_date_format($get_entry_date);
				?>
			</td>
        </tr>
        <tr>
        	<td colspan="4" ><strong>Remarks :  <? echo $dataArray[0][csf('remarks')]; ?></strong> 
			<h3>
			Goods Quantity Description:
			</h3></td>
        </tr>
	...
	<thead  style="font-weight:bold" align="center">
            ...
            <tr>
                ...
                foreach ($bundle_size_arr as $inf)
                {
                    ...
                    ?>
                        <td colspan="2" width="150"><strong><? echo  $size_library[$inf];  ?></strong></td>
                    <?
                }
                ?>
	...
	foreach($dataArr as $key=>$row)
	{
		...
		<tr bgcolor="<? echo $bgcolor; ?>">
			...
			foreach ($bundle_size_arr as $size_id)
               ...
               ?>
	            <td><? echo $sizeQtyArr[$key][$size_id]['ok_qty']  ?></td>
                <td><?  echo  $sizeRejArr[$key][$size_id]['rej_qty'] ?></td>
				
				
              <?
			      $reject_tot_qnty[$size_id]+=$sizeRejArr[$key][$size_id]['rej_qty'];
				  $rcv_tot_qty[$size_id] += $sizeQtyArr[$key][$size_id]['ok_qty'];
                  
				  $rec_t_qnty[$key] +=  $sizeQtyArr[$key][$size_id]['ok_qty'];
				  $rej_t_qtny[$key] += $sizeRejArr[$key][$size_id]['rej_qty'];
                }
                ?>
			
                <td><? echo $rec_t_qnty[$key]; ?></td>
                <td><? echo $rej_t_qtny[$key]; ?></td>
                <td></td>
        ...
        ?>
            ...
            <tr>
                ...
                {
               ?>
			      <td><?  echo $rcv_tot_qty[$size_id] ; ?></td>
                  <td><? echo $reject_tot_qnty[$size_id]  ?></td>
			   <?
                }
                ?>
...
if ($action=="load_drop_down_embl_type")
{
	...
	if($expData[0]==1)
		echo create_drop_down( "cbo_embel_type", 180, $emblishment_print_type,"", 1, "--- Select Printing ---", $embl_type, "" ,"", $embl_type );
	elseif($expData[0]==2)
		echo create_drop_down( "cbo_embel_type", 180, $emblishment_embroy_type,"", 1, "--- Select Embroidery---", $embl_type ,"","", $embl_type );
	elseif($expData[0]==3)
		echo create_drop_down( "cbo_embel_type", 180, $emblishment_wash_type,"", 1, "--- Select wash---", $embl_type,"","", $embl_type );
	elseif($expData[0]==4)
		echo create_drop_down( "cbo_embel_type", 180, $emblishment_spwork_type,"", 1, "--- Select Special Works---", $embl_type,"","", $embl_type );
	elseif($expData[0]==5)
		echo create_drop_down( "cbo_embel_type", 180, $emblishment_gmts_type,"", 1, "--- Select---", $embl_type,"","", $embl_type );
	else
		echo create_drop_down( "cbo_embel_type", 180, $blank_array,"", 1, "--- Select---", $embl_type, "" );
	exit();
}
