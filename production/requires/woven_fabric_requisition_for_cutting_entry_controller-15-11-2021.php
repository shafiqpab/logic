...
if($action=="create_fabric_search_list_view")
{
	...
	$hdn_variable_setting_status=trim($data[8]);
	...
	if($entry_break_down_type==2) // For color level
	{
		...
		{
			$sql = "SELECT a.job_no, a.job_no_prefix_num, a.buyer_name, $year,LISTAGG (b.id, ',') WITHIN GROUP (ORDER BY b.id) AS po_id,LISTAGG (b.po_number, ',') WITHIN GROUP (ORDER BY b.po_number) AS po_number,b.grouping, c.item_number_id as item_id, c.color_number_id as color_id, e.body_part_id, e.lib_yarn_count_deter_id, e.gsm_weight, d.dia_width, sum((d.cons/d.pcs)*c.plan_cut_qnty) as budget_qty,e.uom, e.avg_cons,a.style_ref_no
			from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, wo_pre_cos_fab_co_avg_con_dtls d, wo_pre_cost_fabric_cost_dtls e
			where a.id=b.job_id and b.id=c.po_break_down_id and c.po_break_down_id=d.po_break_down_id and c.color_number_id=d.color_number_id and d.pre_cost_fabric_cost_dtls_id=e.id and c.job_id=e.job_id and c.item_number_id=e.item_number_id and a.is_deleted=0 and a.status_active=1 and b.is_confirmed=1 and e.fab_nature_id=3 and a.company_name=$company_id $buyer_id_cond $job_no_cond $year_cond $po_cond $int_ref_cond $batch_cond $style_ref_cond
			group by a.job_no, a.job_no_prefix_num, a.buyer_name,a.insert_date,b.grouping, c.item_number_id, c.color_number_id, e.body_part_id, e.lib_yarn_count_deter_id, e.gsm_weight, d.dia_width,e.uom, e.avg_cons,a.style_ref_no";

		}
		else
		{
			$sql = "SELECT a.job_no, a.job_no_prefix_num, a.buyer_name, $year, b.id as po_id, b.po_number,b.grouping, c.item_number_id as item_id, c.color_number_id as color_id, e.body_part_id, e.lib_yarn_count_deter_id, e.gsm_weight, d.dia_width, sum((d.cons/d.pcs)*c.plan_cut_qnty) as budget_qty,e.uom, e.avg_cons,a.style_ref_no
			from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, wo_pre_cos_fab_co_avg_con_dtls d, wo_pre_cost_fabric_cost_dtls e
			where a.id=b.job_id and b.id=c.po_break_down_id and c.po_break_down_id=d.po_break_down_id and c.color_number_id=d.color_number_id and d.pre_cost_fabric_cost_dtls_id=e.id and c.job_id=e.job_id and c.item_number_id=e.item_number_id and a.is_deleted=0 and a.status_active=1 and b.is_confirmed=1 and e.fab_nature_id=3 and a.company_name=$company_id $buyer_id_cond $job_no_cond $year_cond $po_cond $int_ref_cond $batch_cond $style_ref_cond
			group by a.job_no, a.job_no_prefix_num, a.buyer_name,a.insert_date, b.id, b.po_number,b.grouping, c.item_number_id, c.color_number_id, e.body_part_id, e.lib_yarn_count_deter_id, e.gsm_weight, d.dia_width,e.uom, e.avg_cons,a.style_ref_no";
		}
		...
		$po_id_array = array(); $color_ids=""; $bodypart_ids="";
		...
		foreach ($result as $val) 
		{
			...
			$yarn_count_dtr_id_arr[$val[csf('lib_yarn_count_deter_id')]] = $val[csf('lib_yarn_count_deter_id')];
			 ...
			 $bodypart_ids.=$val[csf('body_part_id')].",";
		}
		$color_ids=chop($color_ids,",");
		$bodypart_ids=chop($bodypart_ids,",");
		...
		$reqnSql="SELECT buyer_id, po_id, job_no, item_id, body_part, determination_id, gsm, dia, color_id, sum(reqn_qty) as qnty from pro_fab_reqn_for_cutting_dtls where status_active=1 and is_deleted=0 $po_id_cond group by buyer_id, po_id, job_no, item_id, body_part, determination_id, gsm, dia, color_id";
		// echo $reqnSql;die();
		...
		// print_r($reqn_qnty_array);die();
			...
			$buyer_id_cond $job_no_cond $year_cond $po_cond $int_ref_cond $batch_cond $style_ref_cond and c.color_number_id in($color_ids) 
		...
		WHERE    
		d.pre_cost_fabric_cost_dtls_id = e.id $po_id_cond and e.body_part_id in($bodypart_ids) and d.color_number_id in($color_ids) 
		GROUP BY 
		e.item_number_id ,                                  
		e.body_part_id,
		e.lib_yarn_count_deter_id,
		...
		foreach($sql_pre_cost_results  as $row){
			$pre_cost_array[$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('uom')]][$row[csf('color_id')]]['cons_pcs'] += $row[csf('cons')] / $row[csf('pcs')];
		}
		//echo "<pre>";
		...
		<div style="width:1280px; max-height:250px; overflow-y:scroll" id="list_container_batch" align="left">	 
			...
			<?
				...
				{  
					...
					$budget_qnty = $budget_qty_array[$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('color_id')]]['plan_cut_qnty']*$pre_cost_array[$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('uom')]][$row[csf('color_id')]]['cons_pcs'];
                    
					//$budget_qty_array[$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('color_id')]]['plan_cut_qnty']."*".$pre_cost_array[$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('color_id')]]['cons_pcs'].$row[csf('style_ref_no')]."<br>";
					//echo $budget_qty_array[$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('color_id')]]['plan_cut_qnty']."*".$pre_cost_array[$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('uom')]][$row[csf('color_id')]]['cons_pcs']."<br>";
	...
	else  // For color and size level
	{
		$sql = "SELECT a.job_no, a.job_no_prefix_num, a.buyer_name, $year, b.id as po_id, b.po_number,b.grouping, c.item_number_id as item_id, c.color_number_id as color_id, $null_cond(c.size_number_id,0) as size_id, e.body_part_id, e.lib_yarn_count_deter_id, e.gsm_weight, d.dia_width, sum((d.cons/d.pcs)*c.plan_cut_qnty) as budget_qty,e.uom, e.avg_cons,a.style_ref_no
		from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, wo_pre_cos_fab_co_avg_con_dtls d, wo_pre_cost_fabric_cost_dtls e
		where a.job_no=b.job_no_mst and b.id=c.po_break_down_id and c.po_break_down_id=d.po_break_down_id and c.color_number_id=d.color_number_id and c.size_number_id=d.gmts_sizes and d.pre_cost_fabric_cost_dtls_id=e.id and c.job_no_mst=e.job_no and c.item_number_id=e.item_number_id and a.is_deleted=0 and a.status_active=1 and b.is_confirmed=1 and e.fab_nature_id=3 and a.company_name=$company_id $buyer_id_cond $job_no_cond $year_cond $po_cond $int_ref_cond $batch_cond $style_ref_cond
		group by a.job_no, a.job_no_prefix_num, a.buyer_name,a.insert_date, b.id, b.po_number,b.grouping, c.item_number_id, c.color_number_id, nvl(c.size_number_id,0), e.body_part_id, e.lib_yarn_count_deter_id, e.gsm_weight, d.dia_width,e.uom, e.avg_cons,a.style_ref_no";
...
if ($action=="save_update_delete")
{
	...
	{ 	
		...
		$new_mrr_number=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '', 'WFFRC', date("Y",time()), 5, "select reqn_number_prefix, reqn_number_prefix_num from pro_fab_reqn_for_cutting_mst where company_id=$cbo_company_id and $year_cond=".date('Y',time())." and entry_form=507 order by id desc ", "reqn_number_prefix","reqn_number_prefix_num"));
		$id=return_next_id( "id", " pro_fab_reqn_for_cutting_mst", 1 ) ;

		// print_r($new_mrr_number);die();
		...
		$data_array="(".$id.",'".$new_mrr_number[1]."',".$new_mrr_number[2].",'".$new_mrr_number[0]."',".$cbo_company_id.",".$layPlan_id.",".$txt_requisition_date.",".$entry_break_down_type.",507,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		
		$field_array_dtls="id, mst_id, buyer_id, po_id, job_no, item_id, body_part, determination_id, gsm, dia, color_id, size_id, reqn_qty,budget_qty,fab_nature_id, inserted_by, insert_date";
		...
		{ 	
			...
			$data_array_dtls.="(".$dtls_id.",".$id.",".$$buyerId.",'".$$poId."','".$$jobNo."','".$$itemId."','".$$bodyPartId."','".$$deterId."','".$$gsm."','".$$dia."','".$$colorId."','".$$sizeId."','".$$reqsnQty."','".$$budgetQty."',3,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			$dtls_id = $dtls_id+1;
		}
	...
	else if ($operation==1)   // Update Here
	{
		...
		$field_array_dtls="id, mst_id, buyer_id, po_id, job_no, item_id, body_part, determination_id, gsm, dia, color_id, size_id, reqn_qty,budget_qty, inserted_by, insert_date";
		$dtls_id = return_next_id( "id", "pro_fab_reqn_for_cutting_dtls", 1 );
		...
		{ 	
			...
			{
				if($data_array_dtls!="") $data_array_dtls.=",";
				$data_array_dtls.="(".$dtls_id.",".$update_id.",".$$buyerId.",'".$$poId."','".$$jobNo."','".$$itemId."','".$$bodyPartId."','".$$deterId."','".$$gsm."','".$$dia."','".$$colorId."','".$$sizeId."','".$$reqsnQty."','".$$budgetQty."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				$dtls_id = $dtls_id+1;	
			}
		}
