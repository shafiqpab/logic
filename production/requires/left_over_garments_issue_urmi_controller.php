...
{ 
	echo create_drop_down( "cbo_location_name", 170, "select location_name,id from lib_location where is_deleted=0  and status_active=1 and company_id='$data' order by location_name",'id,location_name', 1, '--- Select Location ---', 0, "" );
	 //load_drop_down( 'requires/left_over_garments_issue_urmi_controller', this.value, 'load_drop_down_floor', 'cbo_floor' )     	 
}
...
{ 
	echo create_drop_down( "cbo_location_name", 120, "select location_name,id from lib_location where is_deleted=0  and status_active=1 and company_id='$data' order by location_name",'id,location_name', 1, '--- Select Location ---', 0, "load_drop_down( 'left_over_garments_issue_urmi_controller', this.value, 'load_drop_down_store_name2', 'cbo_store_name' );" );     	 
}
...
{ 
	echo create_drop_down( "cbo_working_location_name", 170, "select location_name,id from lib_location where is_deleted=0  and status_active=1 and company_id='$data' order by location_name",'id,location_name', 1, '--- Select Location ---', 0, "load_drop_down( 'requires/left_over_garments_issue_urmi_controller', this.value+'_'+document.getElementById('cbo_working_company_name').value, 'load_drop_down_working_floor', 'working_floor' )" ); 
}
...
if($action=="create_po_search_list_view")
{
	...
	$sql_cond .= ($wo_com != 0) ? " and a.working_company_id=$wo_com" : "";
	$sql_cond .= ($wo_loc != 0) ? " and a.working_location_id=$wo_loc" : "";
	$sql_cond .= ($wo_floor != 0) ? " and a.working_floor_id=$wo_floor" : "";
	...
	foreach ($sql_issue_res as $val) 
	{
		...
		{
			$issue_qnty_array[$val[csf('po_id')]][$val[csf('item_id')]][$val[csf('store_name')]] += $val[csf('issue_qty')];
		}
	}
	     ...
	     <div style="width:900px; max-height:240px;overflow-y:auto;" >	 
			...
			<?
	            ...
	            foreach( $result as $row )
	            {
	            	...
	            	{
	            		$issue_qty = $issue_qnty_array[$row[csf('po_break_down_id')]][$row[csf('item_number_id')]][$row[csf('store_name')]];
	            	}
	            ...
	            <tr onClick="js_set_value('<? echo $row[csf('po_break_down_id')] ?>','<? echo $row[csf('company_id')] ?>','<? echo $row[csf('location')] ?>','<? echo $row[csf('store_name')] ?>','<? echo ($country_maintain==1) ? $row[csf('country_id')] : 0;?>','<? echo $row[csf('item_number_id')] ?>')" style="cursor:pointer" bgcolor="<?php echo $bgcolor; ?>">
...
{	
	...
	$dataEx = explode("**", $data);
	// ============================= GETTING RECEIVE QTY ============================
	$sqlrRcv ="SELECT b.po_break_down_id as po_id, b.item_number_id as item_id, b.country_id, sum(b.total_left_over_receive) as rcv_qnty from pro_leftover_gmts_rcv_mst a, pro_leftover_gmts_rcv_dtls b where a.id=b.mst_id and b.po_break_down_id=$dataEx[0] and a.goods_type=$dataEx[3] and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.po_break_down_id, b.item_number_id, b.country_id";
	...
	foreach ($sqlrRcv_res as $val) 
	{
		...
		{
			$rcv_qty_array[$val[csf('po_id')]][$val[csf('item_id')]] += $val[csf('rcv_qnty')];
		}
	}
	
	$sqlLeft ="SELECT b.po_break_down_id as po_id, b.item_number_id as item_id, b.country_id, sum(b.total_issue) as issue_qnty from pro_leftover_gmts_issue_mst a, pro_leftover_gmts_issue_dtls b where a.id=b.mst_id and b.po_break_down_id=$dataEx[0] and a.goods_type=$dataEx[3] and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.po_break_down_id, b.item_number_id, b.country_id";
	// echo $sql;
	...
	foreach ($sqlLeftResult as $val) 
	{
		...
		{
			$left_qty_array[$val[csf('po_id')]][$val[csf('item_id')]] += $val[csf('issue_qnty')];
		}
	}

	// =================================== main query ============================
	$country_select = ($dataEx[1]==1) ? ",country_id" : "";
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="380" class="rpt_table">
		...
		foreach($sqlResult as $row)
		{
			...
			{
				$rcv_qty 	= $rcv_qty_array[$row[csf("po_break_down_id")]][$row[csf("item_number_id")]];
				$issueQty 	= $left_qty_array[$row[csf("po_break_down_id")]][$row[csf("item_number_id")]];
			}
...
if($action=="system_number_popup")
{
	...
	<script>
		$(document).ready(function(e) {
            //$("#txt_search_common").focus();
			load_drop_down( 'left_over_garments_issue_urmi_controller','<? echo $company; ?>', 'load_drop_down_location2', 'location_td' );
        });
	        		...
	        		<td align="center" width="100%">
	                    <table ellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
	        				...
	        				<tr>
	                        	<td>                                
									<? 
									echo create_drop_down( "cbo_company_name", 170, "select company_name,id from lib_company comp where is_deleted=0  and status_active=1 order by company_name",'id,company_name', 1, '--- Select Company ---',$company, "load_drop_down( 'left_over_garments_issue_urmi_controller', this.value, 'load_drop_down_location2', 'location_td' );");?>
								</td>
	                             <td id="location_td">
									 <?
	...
	<div style="width:100%;max-height:180px; overflow:y-scroll" id="iron_list_view" align="left">
		...
		<?php  
		    ...
		    if($db_type==2)
			{
				...
				$sql ="SELECT a.issue_date, a.goods_type, b.id, b.mst_id, b.receive_dtls_id, b.buyer_id, b.receive_dtls_id, b.po_break_down_id, b.order_no, b.production_type, b.style_ref_no, b.item_number_id, b.country_id, b.total_issue, b.currency_id, b.fob_rate, b.issue_amount, b.bdt_amount, b.room_no, b.rack_no, b.shelf_no, b.bin_no, b.sewing_production_variable,b.variable_country_maintain,b.leftover_source, b.color_size_id, b.style_order_wisw, b.variable_is_controll, b.user_lebel, b.remarks 
...
{
	$sql ="SELECT a.id, a.sys_number, a.company_id, a.location, a.issue_date, a.goods_type, a.order_type, a.party_name, a.party_id, a.issue_purpose, a.store_name, a.pay_term, a.currency_id, a.challan_no, a.exchange_rate,a.working_company_id,a.working_location_id,a.working_floor_id, a.remarks from pro_leftover_gmts_issue_mst a where a.id='$data' and a.status_active=1 and a.is_deleted =0 ";
	
	//echo $sql.";\n";
	$result =sql_select($sql);
	echo"load_drop_down( 'requires/left_over_garments_issue_urmi_controller', '".$result[0][csf('company_id')]."_".$result[0][csf('location')]."', 'load_drop_down_store_name', 'store_name_td' );\n"; 
	
	//echo"load_drop_down( 'requires/left_over_garments_issue_urmi_controller', ".$result[0][csf('location')].", 'load_drop_down_floor', 'cbo_floor' );\n";
	...
	echo"load_drop_down( 'requires/left_over_garments_issue_urmi_controller', ".$result[0][csf('working_company_id')].", 'load_drop_down_working_location', 'working_location' );\n";
	echo "$('#cbo_working_location_name').val('".$result[0][csf('working_location_id')]."');\n";

	echo"load_drop_down( 'requires/left_over_garments_issue_urmi_controller', ".$result[0][csf('working_location_id')]."+'_'+document.getElementById('cbo_working_company_name').value, 'load_drop_down_working_floor', 'working_floor' );\n";
...
if($action=="get_left_over_balance")
{
	...
	$wo_cond = "";
	$wo_cond .= ($wo_com != 0) ? " and a.working_company_id=$wo_com" : "";
	$wo_cond .= ($wo_loc != 0) ? " and a.working_location_id=$wo_loc" : "";
	$wo_cond .= ($wo_floor != 0) ? " and a.working_floor_id=$wo_floor" : "";		
	// ============================= GETTING LEFTOVER RCV QTY ============================
	$country_cond = ($country_maintain==1) ? " and b.country_id=$country_id" : "";
	$sql_leftover = "SELECT sum(b.total_left_over_receive) as leftover_qty from pro_leftover_gmts_rcv_mst a, pro_leftover_gmts_rcv_dtls b where a.id=b.mst_id and a.goods_type=$goods_type and b.po_break_down_id=$po_id $country_cond $order_type_cond $wo_cond and b.item_number_id=$gmts_item and a.status_active=1 and b.status_active=1";
	...
	$country_cond = ($country_maintain==1) ? " and b.country_id=$country_id" : "";
	$sql_leftover = "SELECT sum(b.total_issue) as leftover_qty from pro_leftover_gmts_issue_mst a, pro_leftover_gmts_issue_dtls b where a.id=b.mst_id and a.goods_type=$goods_type and b.po_break_down_id=$po_id $country_cond $wo_cond and b.item_number_id=$gmts_item and a.status_active=1 and b.status_active=1 ";
	// echo $sql_leftover;
	$sql_leftover_res = sql_select($sql_leftover);
...
if($action=="color_and_size_level_left_over")
{
		...
		b.id, b.mst_id, b.po_break_down_id, b.order_no, b.production_type, b.style_ref_no, b.item_number_id, b.country_id, b.total_left_over_receive, b.currency_id, b.fob_rate, b.leftover_amount, b.bdt_amount, b.room_no, b.rack_no, b.shelf_no, b.bin_no, b.sewing_production_variable, b.color_size_id, b.style_order_wisw, b.variable_is_controll,b.variable_country_maintain, b.user_lebel, b.remarks 
		from pro_leftover_gmts_rcv_mst a, pro_leftover_gmts_rcv_dtls b 
		where a.id=b.mst_id and a.goods_type=$goods_type and b.po_break_down_id='$poId'  and b.item_number_id=$item_id $country_cond $wo_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
		...
		if( $variableSettings==2 ) // color level
		{
			if($order_type==1) // Self Order
			{
				...
				$dtlsData = sql_select("SELECT  d.color_number_id, sum(case when c.production_type=1 then c.production_qnty else 0 end) as production_qnty from pro_leftover_gmts_issue_mst a, pro_leftover_gmts_issue_dtls b, pro_leftover_gmts_issue_clr_sz c, wo_po_color_size_breakdown d where a.id=b.mst_id and a.id=c.mst_id and b.id=c.dtls_id  and d.id=c.color_size_break_down_id and a.goods_type=$goods_type and b.po_break_down_id=$po_id $country_cond $wo_cond and b.item_number_id=$item_id and c.color_size_break_down_id!=0 and c.production_type=1 group by d.color_number_id"); 
		...
		else if( $variableSettings==3 ) //color and size level
		{
			if($order_type==1) // Self Order
			{
				...
				$sql = "SELECT id,size_order, item_number_id, size_number_id, color_number_id, order_quantity, plan_cut_qnty from wo_po_color_size_breakdown where po_break_down_id='$po_id' and item_number_id='$item_id' $country_cond2 and is_deleted=0 and status_active=1 order by color_number_id,size_order,id";
			}
			else	// Subcontruct Order
			{
				$dtlsData = sql_select("SELECT  c.color_size_break_down_id, d.color_id, sum(case when c.production_type=1 then c.production_qnty else 0 end) as production_qnty from pro_leftover_gmts_rcv_dtls b, pro_leftover_gmts_rcv_clr_sz c, subcon_ord_breakdown d where b.id=c.dtls_id  and d.id=c.color_size_break_down_id and b.id=$dtls_id and b.country_id = '$country_name' and c.color_size_break_down_id!=0 and c.production_type=1 group by c.color_size_break_down_id, d.color_id");
...
if($action=="color_and_size_level_left_over_update")
{
	//dtls_id+"**"+mst_id+"**"+po_id+"**"+country_id+"**"+item_id+"**"+variableSettings
		...
		$sql = "SELECT a.sys_number, a.company_id, a.location, a.issue_date, a.goods_type, a.order_type, a.party_name, a.party_id, a.issue_purpose, a.store_name, a.pay_term, a.challan_no, a.exchange_rate, a.remarks,
			...
			b.total_issue, b.sale_rate, b.currency_id, b.fob_rate, b.issue_amount, b.bdt_amount, b.room_no, b.rack_no, b.shelf_no, b.bin_no, b.sewing_production_variable,b.variable_country_maintain, b.color_size_id, b.style_order_wisw, b.variable_is_controll, b.user_lebel,b.remarks
			from pro_leftover_gmts_issue_mst a, pro_leftover_gmts_issue_dtls b
			where a.id=b.mst_id and b.id='$dtls_id' and b.mst_id='$mst_id' $country_cond
		...
		else if( $variableSettings==3 ) //color and size level
		{
			...
			where b.id=c.dtls_id  and d.id=c.color_size_break_down_id and b.id=$dtls_id $country_cond and c.color_size_break_down_id!=0 and c.production_type=1
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$new_sys_number=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'LGI', date("Y",time()), 5, "select sys_number_prefix,sys_number_prefix_num from pro_leftover_gmts_issue_mst where company_id=$cbo_company_name $mrr_cond order by id DESC ", "sys_number_prefix", "sys_number_prefix_num" ));
		 ...
		 $field_array_mst="id,sys_number_prefix,sys_number_prefix_num,sys_number,company_id,location,issue_date,goods_type,order_type,party_name,party_id,issue_purpose,store_name,pay_term,currency_id,challan_no,exchange_rate, working_company_id, working_location_id, working_floor_id,remarks,inserted_by, insert_date"; 
		...
		{
			if(str_replace("'","",$txt_mst_id)==''){			
				$data_array_mst="(".$id.",'".$new_sys_number[1]."','".$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",".$cbo_location_name.",".$txt_issue_date.",".$cbo_goods_type.",".$cbo_order_type.",".$txt_party_name.",".$hidden_party_id.",".$cbo_issue_purpose.",".$cbo_store_name.",".$cbo_pay_term.",".$cbo_currency_mst.",".$txt_challan_no.",".$exchange_rate.",".$cbo_working_company_name.",".$cbo_working_location_name.",".$cbo_working_floor_name.",".$txt_remark.",".$user_id.",'".$pc_date_time."')";
			
				$data_array_dtls="(".$dtls_id.",'".$id."',".$hidden_receive_dtls_id.",".$txt_po_id.",".$txt_order_no.",".$cbo_buyer_name.",1,".$txt_style_name.",".$cbo_item_name.",".$cbo_country_name.",".$txt_total_issue.",".$txt_sale_rate.",".$cbo_currency.",".$txt_fob_rate.",".$txt_issue_amount.",".$txt_bdt_amount.",".$cbo_room_no.",".$cbo_rack_no.",".$cbo_shelf_no.",".$cbo_bin_no.",".$sewing_production_variable.",".$styleOrOrderWisw.",".$variable_is_controll.",".$country_maintain_variable.",".$txt_user_lebel.",".$txt_remark2.",".$user_id.",'".$pc_date_time."')";
			}else{
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		if($db_type==0)	{ mysql_query("BEGIN"); }
		 ...
		 $field_array_mst_update="issue_date*issue_purpose*currency_id*party_id*party_name*pay_term*challan_no*working_company_id*working_location_id*working_floor_id*remarks*updated_by*update_date"; 

		 $data_array_mst_update="".$txt_issue_date."*".$cbo_issue_purpose."*".$cbo_currency_mst."*".$hidden_party_id."*".$txt_party_name."*".$cbo_pay_term."*".$txt_challan_no."*".$cbo_working_company_name."*".$cbo_working_location_name."*".$cbo_working_floor_name."*".$txt_remark."*".$user_id."*'".$pc_date_time."'";
...
if($action=='left_over_gmts_issue_print_2') {
	...
	$sql_dtls="SELECT a.issue_date, a.issue_purpose, a.challan_no, a.party_name, a.party_id, a.sys_number, a.goods_type, a.company_id, a.location, a.remarks, b.id, b.mst_id, b.buyer_id, b.po_break_down_id, b.order_no, b.sale_rate, b.style_ref_no, sum(b.total_issue) as total_issue,sum( b.issue_amount) as issue_amount, a.currency_id, b.item_number_id, b.remarks as dtls_remarks
	from pro_leftover_gmts_issue_mst a, pro_leftover_gmts_issue_dtls b
	where a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and b.mst_id=$txt_mst_id
	group by a.issue_date, a.issue_purpose, a.challan_no,a.party_name, a.party_id, a.sys_number, a.goods_type, a.company_id, a.location, a.remarks, b.id, b.mst_id, b.buyer_id, b.po_break_down_id, b.order_no, b.style_ref_no, b.sale_rate, a.currency_id, b.item_number_id, b.remarks";
	// echo $sql_dtls;
	$result_dtls=sql_select($sql_dtls);
	//print_r($result_dtls);
	$po_id_arr = array();
	...
	$job_array = sql_select("select a.id, a.job_no_mst, b.style_description, b.style_ref_no, b.season, b.item_number_id
								from wo_po_break_down a, wo_po_details_master b
								where a.id in($poIds) and a.job_no_mst=b.job_no");
	...
	<div style="width:1010px;">
	    ...
	    <table width="1000" cellspacing="0" align="right" style="margin-top: 20px;">
			...
			<tr>
				...
				<td width="250" colspan="5"><?php echo $result_dtls[0][csf("remarks")]; ?></td>
			</tr>			
