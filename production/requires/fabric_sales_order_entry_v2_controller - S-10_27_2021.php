...
if ($action == "save_update_delete") {
	...
	} else if ($operation == 1)   // Update Here
	{
		...
		if ($cbo_within_group_combo == 1) {
			$field_array_dtls_update = "body_part_id*color_type_id*fabric_desc*determination_id*gsm_weight*dia*width_dia_type*color_id*color_range_id*finish_qty*rmg_qty*avg_rate*amount*process_loss*grey_qty*work_scope*order_uom*pre_cost_fabric_cost_dtls_id*item_number_id*pre_cost_remarks*updated_by*update_date*status_active*is_deleted*grey_qnty_by_uom*cons_uom*process_id*process_seq*process_id_main*process_seq_main*after_wash_gsm*cuttable_dia*pp_qnty*mtl_qnty*fpt_qnty*gpt_qnty*ccmp_data*ply*coller_cuff_cons*lab_dip_no*sl_no";
		} else {
			$field_array_dtls_update = "body_part_id*color_type_id*fabric_desc*determination_id*gsm_weight*dia*width_dia_type*color_id*color_range_id*finish_qty*rmg_qty*avg_rate*amount*process_loss*grey_qty*work_scope*order_uom*pre_cost_remarks*updated_by*update_date*status_active*is_deleted*grey_qnty_by_uom*cons_uom*process_id*process_seq*process_id_main*process_seq_main*after_wash_gsm*cuttable_dia*pp_qnty*mtl_qnty*fpt_qnty*gpt_qnty*ccmp_data*ply*coller_cuff_cons*lab_dip_no*sl_no";
		}
		$is_new = 0;
		$dtls_ids_to_delete = "";
...
if ($action == "create_job_search_list_view") {
	...
	if ($cbo_buyer_name == 0) $cust_buyer_cond = ""; else $cust_buyer_cond = " and customer_buyer=$cbo_buyer_name";
	...
	$sql = "select id, $year_field, job_no_prefix_num, job_no, within_group, sales_order_type, sales_booking_no, booking_date, buyer_id, customer_buyer, style_ref_no, location_id from fabric_sales_order_mst where entry_form=472 and status_active=1 and is_deleted=0 and company_id=$company_id $within_group_cond $sales_type_cond $search_field_cond $cust_buyer_cond order by id desc";
	//echo $sql;//die;
	$result = sql_select($sql);
	?>
...
if ($action == 'yarn_details') {
	...
	$data_arr = explode('__', $data);
	 ...
	 if($data_arr[1] !='')
	 {
	 	$mst_id=return_field_value("id","fabric_sales_order_mst","job_no ='".$data_arr[1]."' ");
	 }
	 else
	 {
		$mst_id=$data_arr[0];
	...
	$saveDataArr = array();
	$sql_yarn = "select deter_id, gsm, yarn_data from fabric_sales_order_yarn where mst_id=$mst_id and status_active=1 and is_deleted=0 ";
	...
	$sql = "select determination_id, gsm_weight, sum(grey_qty) as grey_qty, $dtls_ids from fabric_sales_order_dtls where mst_id=$data_arr[0] and status_active=1 and is_deleted=0 group by determination_id, gsm_weight";
	//echo $sql;die;
	$data_array = sql_select($sql);
	$i = 0;
	foreach ($data_array as $row) {
...
if ($action == "fabric_sales_order_print6") 
{
	...
	<tr>
		...
		<td><? echo change_date_format($nameArray_approved[0][csf('last_approved_date')]);?></td>
		<td><strong>Update Serial</strong></td>
		<td>
				...
				foreach ($dtls_sql_data as $rows) 
				{
					...
					?>
						<tr bgcolor="<? echo $bgcolor; ?>">
							...
							<td>
								<? echo $processSeqName;// $processName;?>
							</td>
