...
if($action=="create_batch_search_list_view")
{
	...
	a.booking_without_order, a.re_dyeing_from, a.sales_order_id, a.sales_order_no, a.is_sales, b.sales_order_type
	...
	<div style="width:920px; overflow-y:scroll; max-height:250px;" id="buyer_list_view" align="center">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="900" class="rpt_table" id="tbl_list_search" >
			...
			foreach ($nameArray as $selectResult)
			{
				...
				?>
				<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value(<? echo $selectResult[csf('id')]; ?>,'<? echo $selectResult[csf('batch_no')]; ?>','<? echo $selectResult[csf('booking_no')]; ?>','<? echo $job_no; ?>','<? echo $is_sales; ?>','<? echo $unload_dyeing_production_arr[$selectResult[csf('id')]];?>','<? echo $sales_order_type;?>')">
...
if($action=='show_fabric_desc_listview')
{
	...
	$production_sql="SELECT b.batch_id, b.fabric_description_id, b.original_gsm, b.original_width, b.body_part_id, b.color_id, sum(b.receive_qnty) as production_qty 
	...
	group by B.BATCH_ID, B.ORIGINAL_GSM, B.ORIGINAL_WIDTH, b.FABRIC_DESCRIPTION_ID, b.BODY_PART_ID, b.COLOR_ID";
	// echo $production_sql;
	$production_sql_result=sql_select($production_sql);
	foreach ($production_sql_result as $key => $value) 
	{
		$production_qty_arr[$value[csf('fabric_description_id')]][$value[csf('original_gsm')]][$value[csf('original_width')]][$value[csf('body_part_id')]][$value[csf('color_id')]]['production_qty']+=$value[csf('production_qty')];
	}
	...
	<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="510">
		...
		<tbody>
			...
			foreach($data_array as $row)
			{
				...
				$production_qty=$production_qty_arr[$determination_id][$gsm][$dia][$row[csf('body_part_id')]][$row[csf('color_id')]]['production_qty'];
		...
		function wgtlostcalculation()
		{
			...
			$("#tbl_list_search").find('tr').each(function()
			{
				...
				//{
					...
					//alert('grey='+txtgreyQnty+', txtBatchQnty='+txtBatchQnty+', preGreyUsed='+preGreyUsed);
					if((txtgreyQnty > txtBatchQnty -preGreyUsed) && production_basis==5)
					{
					...
					{
						txtgreyQnty=txtgreyQnty;
						txtProcessQnty=txtProcessQnty;
					}
				...
				<div style="width:500px; margin-top:10px" align="center">
					<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="520" align="center">
						...
						<tr class="general">
							...
							<td>
								<input type="text" name="txt_prop_grey_qnty" id="txt_prop_grey_qnty" class="text_boxes_numeric" value="<? echo $txt_grey_used; ?>" style="width:120px" onBlur="distribute_qnty(document.getElementById('cbo_distribiution_method').value)" disabled>
								<input type="hidden" id="txt_get_value_delv_entry" name="txt_get_value_delv_entry">
							</td>
							<td>
								<?
							...
							for($z=0;$z<count($explSaveData);$z++)
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
									...
									<td width="70" align="center">
										<input type="text" name="txtgreyQnty[]" id="txtgreyQnty_<? echo $i; ?>" onBlur="wgtlostcalculation();fn_check_balance(<? echo $i; ?>);"  class="text_boxes_numeric" style="width:50px" value="<? echo $grey_qnty; ?>" disabled>
		...
		<div style="width:840px; margin-top:10px" align="center">
			<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="400" align="center">
				...
				<tr class="general">
					...
					<td>
						<input type="text" name="txt_prop_grey_qnty" id="txt_prop_grey_qnty" class="text_boxes_numeric" value="<? echo $txt_grey_used; ?>" style="width:120px" onBlur="distribute_qnty(document.getElementById('cbo_distribiution_method').value)"  <? echo $IsGreyUsedDisabled;?> disabled>
						<input type="hidden" id="txt_get_value_delv_entry" name="txt_get_value_delv_entry">
					</td>
					<td>
						<?
					...
					else if($save_data!="" && $production_basis==5)
					{
						...
						foreach($nameArray as $row)
						{
							...
							<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
								...
								<td width="70" align="center">
									<input type="text" name="txtgreyQnty[]" id="txtgreyQnty_<? echo $i; ?>" onBlur="wgtlostcalculation();fn_check_balance(<? echo $i; ?>);"  class="text_boxes_numeric" style="width:50px" value="<? echo $grey_qnty; ?>" <? echo $IsGreyUsedDisabled;?> disabled>
					...
					else
					{
						...
						foreach($nameArray as $row)
						{
							...
							<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
								...
								<td width="70" align="center">
									<input type="text" name="txtgreyQnty[]" onBlur="wgtlostcalculation();fn_check_balance(<? echo $i; ?>);" id="txtgreyQnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:50px" value="" <? echo $IsGreyUsedDisabled;?> disabled readonly>
...
if ($action=="save_update_delete")
{
	...
	if(str_replace("'","",$cbo_production_basis)==5)
	{
		...
		if(str_replace("'","",$txt_original_dia_width)=="")
		{
			if($db_type==0)
			{
				$dia_width_cond = " and c.dia_width = '".str_replace("'","",$txt_original_dia_width)."'";
				$dia_cond_a= " and a.original_width = '".str_replace("'","",$txt_original_dia_width)."'";
			}
			else
			{
	...
	if ($operation==0)  // Insert Here
	{
		...
		if(str_replace("'","",$txt_dia_width)=="")
		{
			if($db_type==0)
			{
				$dia_width_cond = " and dia_width = $txt_dia_width";
			}
			else
			{
