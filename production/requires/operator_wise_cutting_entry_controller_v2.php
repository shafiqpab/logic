...
if ($action=="load_php_data_to_form_issue")
{
	...
	{	
		echo "load_drop_down( 'requires/operator_wise_cutting_entry_controller_v2', '".$row[csf("company_id")]."', 'load_drop_down_location', 'location_td');\n";
...
if($action=="today_cutting_popup")
{
	...
	<script>
		function fn_get_table(id,floor_id)
		{
			...
			//alert(container);
			load_drop_down( 'operator_wise_cutting_entry_controller_v2', data,'load_dropdown_table', container );
		...
		function fnGenerateOperatorListView()
		{
			...
			show_list_view ( data, 'create_operator_search_list_view', 'search_div', 'operator_wise_cutting_entry_controller_v2', '');
		}
		...
		function fn_get_employee(id,emp_id)
		{
			...
			//alert(container);
			load_drop_down( 'operator_wise_cutting_entry_controller_v2', data,'load_dropdown_emp', container );
		}
...
if($action=="create_operator_search_list_view")
{
	...
	{
		?>
		<div style="width:70%" class="alert alert-danger">Please select cutting table then try again.</div>
		<?
		die;
	}
	$sql_cond = "";
...
if ($action=="show_list_view") 
{
	...
	$table_array=return_library_array( "SELECT id,table_no from lib_cutting_table where working_company_id=$data[0]  and location_id=$data[1]  and status_active=1 and table_no is not null order by table_no ", "id", "table_no"  );

	// echo "SELECT id,table_no from lib_cutting_table where company_id=$data[0]  and location_id=$data[1]  and status_active=1 and table_no is not null order by table_no"; die;
	$sql=("SELECT a.id as job_id, b.id as po_id,d.serving_company,d.location, a.job_no,a.style_ref_no,b.po_number,c.id as col_size_id,c.item_number_id,c.color_number_id,c.order_quantity,c.plan_cut_qnty,c.excess_cut_perc,e.cut_no,
	  (case when d.production_type=1 then e.production_qnty else 0 end) as cutting_qty
	  from wo_po_details_master a,wo_po_break_down b,wo_po_color_size_breakdown c,pro_garments_production_mst d,pro_garments_production_dtls e where a.id=b.job_id and a.id=c.job_id and b.id=c.po_break_down_id and d.id=e.mst_id and d.po_break_down_id=b.id and e.color_size_break_down_id=c.id and a.status_active=1 and a.is_deleted=0 and  b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and e.production_qnty>0 and d.production_type=1 $company_id_cond $location $date_con order by b.id");
	//echo $sql;die;
	$res = sql_select($sql);
	if(count($res)==0)
	{
	...
	foreach($res as $val)
	{
		$cut_no_array[$val['CUT_NO']] = $val['CUT_NO'];
	}
	$cut_no_cond = where_con_using_array($cut_no_array,1,"a.cutting_no");
	// ========================= getting cut no wise table ==============================
	$cutting_table_array=return_library_array( "SELECT a.cutting_no,a.TABLE_NO from ppl_cut_lay_mst a where a.status_active=1 $cut_no_cond", "cutting_no", "TABLE_NO");
	// echo "SELECT a.cutting_no,a.TABLE_NO from ppl_cut_lay_mst a where a.status_active=1 $cut_no_cond";
	$order_cut_array=return_library_array( "SELECT a.cutting_no,b.ORDER_CUT_NO from ppl_cut_lay_mst a,ppl_cut_lay_dtls b where a.id=b.mst_id and b.status_active=1 $cut_no_cond", "cutting_no", "ORDER_CUT_NO");
	// print_r($cutting_table_array);die;
	
	foreach($res as $val)
	{
		$data_array[$cutting_table_array[$val['CUT_NO']]][$val['PO_ID']][$val['ITEM_NUMBER_ID']][$val['COLOR_NUMBER_ID']][$val['CUT_NO']]['job_no'] = $val['JOB_NO'];
		$data_array[$cutting_table_array[$val['CUT_NO']]][$val['PO_ID']][$val['ITEM_NUMBER_ID']][$val['COLOR_NUMBER_ID']][$val['CUT_NO']]['job_id'] = $val['JOB_ID'];
		$data_array[$cutting_table_array[$val['CUT_NO']]][$val['PO_ID']][$val['ITEM_NUMBER_ID']][$val['COLOR_NUMBER_ID']][$val['CUT_NO']]['style_ref_no'] = $val['STYLE_REF_NO'];
		$data_array[$cutting_table_array[$val['CUT_NO']]][$val['PO_ID']][$val['ITEM_NUMBER_ID']][$val['COLOR_NUMBER_ID']][$val['CUT_NO']]['po_number'] = $val['PO_NUMBER'];
		
		$data_array[$cutting_table_array[$val['CUT_NO']]][$val['PO_ID']][$val['ITEM_NUMBER_ID']][$val['COLOR_NUMBER_ID']][$val['CUT_NO']]['cutting_qty'] += $val['CUTTING_QTY'];
   ...
   //echo $color_sql;die;
	...
	foreach($result as $row)
	{
		$operator_wise_tbl_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']][$row['OPERATOR_ID']] .= ($operator_wise_tbl_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']][$row['OPERATOR_ID']]=="") ? $row['TABLE_ID'] : ",".$row['TABLE_ID'];
	}
	...
	foreach($result as $row)
	{
		$operator_data_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']]['floor_id'] = $row['FLOOR_ID'];

		$operator_data_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']]['table_id'] = $row['TABLE_ID'];

		$operator_data_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']]['process_id'] = $row['PROCESS_ID'];
		$operator_data_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']]['operator_id'] = $row['OPERATOR_ID'];
		$operator_data_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']]['job_id'] = $row['JOB_ID'];
		$operator_data_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']]['style_ref'] = $row['STYLE_REF'];
		// $operator_data_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']]['qty'] = $row['QTY'];
		$operator_data_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']]['id'] = $row['ID'];
		if($operator_wise_tbl_chk_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']][$row['OPERATOR_ID']]=="")
		{
			$tbl_id = $operator_wise_tbl_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']][$row['OPERATOR_ID']];
			if($operator_data_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']]['emp_data']=="")
			{
				$operator_data_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']]['emp_data'] = $row['FLOOR_ID']."**".$tbl_id."**".$row['PROCESS_ID']."**".$row['OPERATOR_ID']."**".$row['QTY']."**".$row['QTY_DIST_TYPE'];
				$operator_data_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']]['qty'] = $row['QTY'];
				// echo $row['PO_ID']."*".$row['ITEM_ID']."*".$row['COLOR_ID']."*".$row['QTY']."<br>";
			}
			else
			{
				$operator_data_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']]['emp_data'] .= "__".$row['FLOOR_ID']."**".$tbl_id."**".$row['PROCESS_ID']."**".$row['OPERATOR_ID']."**".$row['QTY']."**".$row['QTY_DIST_TYPE'];
				$operator_data_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']]['qty'] += $row['QTY'];
				// echo $row['PO_ID']."*".$row['ITEM_ID']."*".$row['COLOR_ID']."*".$row['QTY']."<br>";
			}

			$operator_wise_tbl_chk_array[$row['PO_ID']][$row['ITEM_ID']][$row['COLOR_ID']][$row['CUTTING_NO']][$row['OPERATOR_ID']] = $row['TABLE_ID'];
		}		
	...
	// echo "<pre>";print_r($operator_data_array);die;
		...
		?>
            <div >
                <table cellspacing="0" cellpadding="0" border="1" rules="all"  align="center" width="1310px" class="rpt_table" >
                    <thead>
						<tr>
							<th width="50">SL</th>
			 ...
			 <div>
				<table  cellspacing="0" cellpadding="0" border="1" rules="all"  align="center" width="1310px" class="rpt_table" id="tbl_list_search">  
					<tbody>
						<?
						$i=1;
						$gr_cut_qty = 0;
						...
						foreach ($data_array as $tbl_id =>$tbl_data)
						{
									...
									foreach ($item_data as $color_id => $color_data)
									{
										...
										{ 										
											if($row['cutting_qty']>0)
											{
												...
												if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";				
												?>
												<tr class="activeRow" bgcolor='<? echo $bgcolor; ?>
												' onclick="change_color('tr_<? echo $i;?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i;?>">
													...
													<td width="80" align="center"><p><?echo $order_cut_array[$cut_no];?></p></td>
													<td width="150"><p><?echo  $garments_item[$item_id];?></p></td>
													<td width="150"><p><?echo  $color_library[$color_id];?></p></td>
													<td width="80" align="center"><p><?=$cut_no;?></p></td>
													<td width="60" align="center"><p><?=$table_array[$tbl_id];?></p></td>
													<td width="60" align="right"><p><?echo  number_format($row['cutting_qty'],0);?></p></td>
													<td width="120" >
														<input type="text" id="txtcutting_<?=$i;?>"  name="txtcutting_<?=$i;?>"  style="width:110px" class="text_boxes_numeric" onDblClick="cutting_popup(<?=$i;?>)" placeholder="Browse" readonly value="<?=$operator_data_array[$po_id][$item_id][$color_id][$cut_no]['qty'];?>" />
														<input type="hidden" name="empWiseCuttingData_<?=$i;?>" id="empWiseCuttingData_<?=$i;?>" value="<?= $operator_data_array[$po_id][$item_id][$color_id][$cut_no]['emp_data']; ?>">
														...
														<input type="hidden" name="cutNo_<?=$i;?>" id="cutNo_<?=$i;?>" value="<?=$cut_no;?>">
														<input type="hidden" name="tblNo_<?=$i;?>" id="tblNo_<?=$i;?>" value="<?=$tbl_id;?>">
														<input type="hidden" name="mstId_<?=$i;?>" id="mstId_<?=$i;?>" value="<?= $operator_data_array[$po_id][$item_id][$color_id][$cut_no]['id']; ?>">
													</td>
												</tr>
												...
												$gr_bal_qty += $blance;												
												$tbl_tot_cut_qty += $row['cutting_qty'];
												$tbl_tot_bal_qty += $blance;
											}
										}	
									}
								}
							...
							?>
							<tr style="background: #dccdcd;text-align:right;font-weight:bold;">
								<td></td>
								<td></td>
								<td></td>
								<td></td>
								...
								<td></td>
								<td>Sub Total</td>
								<td><?=number_format($tbl_tot_cut_qty,0);?></td>
								<td></td>
							</tr>
							<?
						}
					...
					<tfoot>						
						<tr>
							...
							<th></th>
							<th>G Total</th>
							<th><?=number_format($gr_cut_qty,0);?></th>
							<th></th>
						</tr>
					</tfoot>
               </table>	
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   // Insert Here========================================================================================
	{
		...
		$field_array="id, company_id, location_id,entry_date,style_ref, job_id, po_id,item_id, color_id,qty_dist_type, entry_form,cutting_no,cutting_tbl_id, inserted_by, insert_date,status_active,is_deleted";
		
		$field_array_up="company_id*location_id*entry_date*style_ref*job_id*po_id*item_id*color_id*qty_dist_type*cutting_no*cutting_tbl_id*updated_by*update_date";
		...
		for($j=1;$j<=$row_num;$j++)
		{
			...
			if($$mstId>0)
			{
				...
				if(str_replace("'","",$$cuttingdata) !="")
				{
					...
					$mst_id_array[]=$$mstId;
					$data_array_up[$$mstId]=explode("*",("".$cbo_company_id."*".$cbo_location_name."*".$txt_cuitting_date."*'".$$styleRefNo."'*".$$jobId."*".$$orderId."*".$$gtmsId."*".$$colorId."*".$dist_type."*'".$$cutNo."'*'".$$tblNo."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
			...
			else
			{
				if(str_replace("'","",$$cuttingdata) !="")
				{
					...
					$data_array.="(".$id.",".$cbo_company_id.",".$cbo_location_name.",".$txt_cuitting_date.",'".$$styleRefNo."',".$$jobId.",".$$orderId.",".$$gtmsId.",".$$colorId.",".$dist_type.",657,'".$$cutNo."','".$$tblNo."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','1','0')";
	...
	else if ($operation==1)   // Update Here
	{
		...
		// echo "6**insert into operator_wise_cutting_entry_mst (".$field_array.") values ".$data_array;die;
	    if($data_array!="")
		{
			$rID2=sql_insert("operator_wise_cutting_entry_mst",$field_array,$data_array);
		}
		...
		// echo "6**insert into operator_wise_cutting_entry_dtls (".$field_array_dtls.") values ".$data_array_dtls;die;
	    if($data_array_dtls!="")
		{
			$rID4=sql_insert("operator_wise_cutting_entry_dtls",$field_array_dtls,$data_array_dtls);
		}	
