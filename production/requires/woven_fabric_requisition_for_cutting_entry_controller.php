...
if ($action=="load_drop_down_location")
{
	...
	if(count($locationArr)==1)
	{
		...
		echo create_drop_down( "cbo_location", 152, "select id,location_name from lib_location where company_id='$data' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", $key, "load_drop_down('requires/woven_fabric_requisition_for_cutting_entry_controller','".$data."_'+this.value, 'load_drop_down_production_floor', 'floor_td');" );
		exit();
	}
	else
	{
		echo create_drop_down( "cbo_location", 152, "select id,location_name from lib_location where company_id='$data' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", 0, "load_drop_down('requires/woven_fabric_requisition_for_cutting_entry_controller','".$data."_'+this.value, 'load_drop_down_production_floor', 'floor_td');" );
			...
			<legend>Enter search words</legend>           
	            <table cellpadding="0" cellspacing="0" border="1" rules="all" width="1100" class="rpt_table">
	                ...
	                <tr class="general">
	                	<td>
	                    	<?
								echo create_drop_down( "cbo_buyer_name", 150, "select buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$company_id' $buyer_cond and buy.id in (select buyer_id from lib_buyer_party_type where party_type in (1,3,21,90)) order by buy.buyer_name","id,buyer_name", 1, "-- All Buyer --", $selected, "load_drop_down( 'woven_fabric_requisition_for_cutting_entry_controller', this.value+'*'+1, 'load_drop_down_season', 'season_td'); load_drop_down( 'woven_fabric_requisition_for_cutting_entry_controller', this.value+'*'+1, 'load_drop_down_brand', 'brand_td');",$data[0] ); 
							?>       
	                    </td>
...
if($action=="create_fabric_search_list_view")
{
	...
	if($hdn_variable_setting_status==1)// style wise
	{
		if($hdn_variable_colorSize_status==2)// color level
		{
			...
			where a.id=b.job_id and b.id=c.po_break_down_id and c.po_break_down_id=d.po_break_down_id and c.color_number_id=d.color_number_id and c.size_number_id=d.gmts_sizes and d.pre_cost_fabric_cost_dtls_id=e.id and c.job_no_mst=e.job_no and c.item_number_id=e.item_number_id and a.is_deleted=0 and a.status_active=1  and a.company_name=$company_id $buyer_id_cond $job_no_cond $year_cond $po_cond $int_ref_cond $batch_cond $style_ref_cond $brand_no_cond $season_no_cond $season_year_cond 
			group by  a.buyer_name,a.job_no,a.style_ref_no, c.item_number_id, e.body_part_id, e.lib_yarn_count_deter_id, c.color_number_id,e.uom, e.avg_finish_cons ,a.insert_date,b.id,b.grouping, e.gsm_weight, d.dia_width,e.color_size_sensitive");
			foreach($sql_po_wise as $row)
			{
				...
				$job_wise_budgt_qnt_arr[$row[csf('buyer_name')]][$row[csf('job_no')]][$row[csf('style_ref_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('color_id')]][$row[csf('grouping')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]]['qnty']+=$row[csf('budget_qty')];
			}
			...
			if($cutting_chk=='true') // when check box checked
			{
				...
				where a.id=b.job_id and b.id=c.po_break_down_id and c.po_break_down_id=d.po_break_down_id and c.color_number_id=d.color_number_id and c.size_number_id=d.gmts_sizes and d.pre_cost_fabric_cost_dtls_id=e.id and c.job_no_mst=e.job_no and c.item_number_id=e.item_number_id and a.is_deleted=0 and a.status_active=1  and a.company_name=$company_id $buyer_id_cond $job_no_cond $year_cond $po_cond $int_ref_cond $batch_cond $style_ref_cond $brand_no_cond $season_no_cond $season_year_cond and c.color_number_id in($color_id) and c.item_number_id in($gmt_item_id) and c.po_break_down_id in($order_ids) and e.job_no=f.job_no and d.pre_cost_fabric_cost_dtls_id=f.pre_cost_fabric_cost_dtls_id  and  e.gsm_weight=f.gsm_weight and d.dia_width=f.dia_width and f.po_break_down_id=b.id and f.is_deleted=0 and f.status_active=1
			...
			else
			{
				...
				from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, wo_pre_cos_fab_co_avg_con_dtls d, wo_pre_cost_fabric_cost_dtls e,wo_booking_dtls f
				where a.id=b.job_id and b.id=c.po_break_down_id and c.po_break_down_id=d.po_break_down_id and c.color_number_id=d.color_number_id and c.size_number_id=d.gmts_sizes and d.pre_cost_fabric_cost_dtls_id=e.id and c.job_no_mst=e.job_no and c.item_number_id=e.item_number_id and a.is_deleted=0 and a.status_active=1  and a.company_name=$company_id $buyer_id_cond $job_no_cond $year_cond $po_cond $int_ref_cond $batch_cond $style_ref_cond $brand_no_cond $season_no_cond $season_year_cond and e.job_no=f.job_no and d.pre_cost_fabric_cost_dtls_id=f.pre_cost_fabric_cost_dtls_id  and  e.gsm_weight=f.gsm_weight and d.dia_width=f.dia_width and f.po_break_down_id=b.id and f.pre_cost_fabric_cost_dtls_id=e.id and f.is_deleted=0 and f.status_active=1
				group by  a.buyer_name,a.job_no, a.job_no_prefix_num,a.style_ref_no, c.item_number_id, e.body_part_id, e.lib_yarn_count_deter_id, c.color_number_id,e.uom, e.avg_finish_cons ,a.insert_date,b.grouping, e.gsm_weight, d.dia_width,e.color_size_sensitive, a.id, e.seq,f.fin_fab_qnty order by a.id, e.seq asc";*/

				/*$sql = "SELECT a.buyer_name,a.job_no, a.job_no_prefix_num,a.style_ref_no, c.item_number_id as item_id, e.body_part_id, e.lib_yarn_count_deter_id, c.color_number_id as color_id,e.uom, e.avg_finish_cons as avg_cons ,sum((d.cons/d.pcs)*c.plan_cut_qnty) as budget_qty, TO_CHAR(a.insert_date,'YYYY') as year ,LISTAGG(b.id,',') WITHIN GROUP (ORDER BY b.id) as po_id ,LISTAGG(b.po_number,',') WITHIN GROUP (ORDER BY b.po_number) as po_number ,b.grouping, e.gsm_weight, d.dia_width,e.color_size_sensitive,e.fabric_source,f.stripe_color,g.contrast_color_id 
				from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, wo_pre_cos_fab_co_avg_con_dtls d, wo_pre_cost_fabric_cost_dtls e  left join wo_pre_cos_fab_co_color_dtls  g on  e.id=g.pre_cost_fabric_cost_dtls_id and g.is_deleted=0 and g.status_active=1 left join wo_pre_stripe_color f on  e.id=f.pre_cost_fabric_cost_dtls_id 
				where a.id=b.job_id and b.id=c.po_break_down_id and c.po_break_down_id=d.po_break_down_id and c.color_number_id=d.color_number_id and c.size_number_id=d.gmts_sizes and d.pre_cost_fabric_cost_dtls_id=e.id and c.job_no_mst=e.job_no and c.item_number_id=e.item_number_id and a.is_deleted=0 and a.status_active=1  and a.company_name=$company_id $buyer_id_cond $job_no_cond $year_cond $po_cond $int_ref_cond $batch_cond $style_ref_cond $brand_no_cond $season_no_cond $season_year_cond 
				group by  a.buyer_name,a.job_no, a.job_no_prefix_num,a.style_ref_no, c.item_number_id, e.body_part_id, e.lib_yarn_count_deter_id, c.color_number_id,e.uom, e.avg_finish_cons ,a.insert_date,b.grouping, e.gsm_weight, d.dia_width,e.color_size_sensitive,e.fabric_source,f.stripe_color,g.contrast_color_id , a.id, e.seq order by a.id, e.seq asc";*/

				$sql = "SELECT a.buyer_name,a.job_no, a.job_no_prefix_num,a.style_ref_no, c.item_number_id as item_id, d.body_part_id, d.lib_yarn_count_deter_id, c.color_number_id as color_id,d.uom,d.avg_finish_cons as avg_cons ,sum((e.cons/e.pcs)*c.plan_cut_qnty) as budget_qty, TO_CHAR(a.insert_date,'YYYY') as year ,
				LISTAGG(b.id,',') WITHIN GROUP (ORDER BY b.id) as po_id  ,b.grouping, d.gsm_weight, e.dia_width,d.color_size_sensitive,d.fabric_source,h.stripe_color,g.contrast_color_id 
				from wo_po_details_master a join wo_po_break_down b on a.id=b.job_id join wo_po_color_size_breakdown c on a.id=c.job_id and b.id=c.po_break_down_id join wo_pre_cost_fabric_cost_dtls d on a.id=d.job_id and c.item_number_id= d.item_number_id and d.is_deleted=0 and d.status_active=1 join wo_pre_cos_fab_co_avg_con_dtls e on a.id=e.job_id and d.id=e.pre_cost_fabric_cost_dtls_id and c.po_break_down_id=e.po_break_down_id and c.color_number_id=e.color_number_id and c.size_number_id=e.gmts_sizes and e.is_deleted=0 and e.status_active=1 and e.cons !=0 left join wo_pre_cos_fab_co_color_dtls g on a.id=g.job_id and d.id=g.pre_cost_fabric_cost_dtls_id and g.is_deleted=0 and g.status_active=1 and c.color_number_id=g.gmts_color_id and e.color_number_id =g.gmts_color_id left join wo_pre_stripe_color h on a.id=h.job_id and c.item_number_id= h.item_number_id and d.id=h.pre_cost_fabric_cost_dtls_id and e.color_number_id =h.color_number_id and e.po_break_down_id=h.po_break_down_id and e.gmts_sizes=h.size_number_id 
				where a.id=b.job_id and b.id=c.po_break_down_id and c.po_break_down_id=e.po_break_down_id and c.color_number_id=e.color_number_id and c.size_number_id=e.gmts_sizes and e.pre_cost_fabric_cost_dtls_id=d.id and c.job_no_mst=d.job_no and c.item_number_id=d.item_number_id and a.is_deleted=0 and a.status_active=1  and a.company_name=$company_id $buyer_id_cond $job_no_cond $year_cond $po_cond $int_ref_cond $batch_cond $style_ref_cond $brand_no_cond $season_no_cond $season_year_cond 
				group by a.buyer_name,a.job_no, a.job_no_prefix_num,a.style_ref_no, c.item_number_id, d.body_part_id, d.lib_yarn_count_deter_id, c.color_number_id,d.uom,d.avg_finish_cons ,a.insert_date,b.grouping,d.gsm_weight, e.dia_width,d.color_size_sensitive,d.fabric_source,h.stripe_color,g.contrast_color_id , a.id, d.seq order by a.id, d.seq asc";
		...
		$poIDs="";$poIdsChkArr=array();$poIds_arr=array();
		foreach ($results as $val) 
		{

			$poExp=explode(",", $val[csf('po_id')]);
			foreach ($poExp as $poIds) 
			{

				if($poIdsChkArr[$poIds] != $poIds)
				{
					$poIDs.=$poIds.",";
					$poIds_arr[$poIds] = $poIds;
					$poIdsChkArr[$poIds] = $poIds;
				}
		...
		$poIDs=chop($poIDs,",");
		//print_r($poIds_arr); die;

		if(!empty($poIds_arr))
		{
			$all_poIds = implode(",",$poIds_arr);
		    $all_poIds=implode(",",array_filter(array_unique(explode(",",$all_poIds))));
		    if($all_poIds!="")
		    {
		        $all_poIds=explode(",",$all_poIds);  
		        $poIds_chnk=array_chunk($all_poIds,999);
		        $poIds_cond=" and";
		        foreach($poIds_chnk as $ordr_ids)
		        {
		        	if($poIds_cond==" and")
						$poIds_cond.="(a.po_break_down_id in(".implode(',',$ordr_ids).")";
					else
						$poIds_cond.=" or a.po_break_down_id in(".implode(',',$ordr_ids).")";
		        }
		        $poIds_cond.=")";
		        //echo $poIds_cond;die;
		    }	
			//$poIds_cond = "and a.po_break_down_id in(".implode(",",$poIds_arr).")";
		}

		//echo $poIds_cond ;
	
		$determination_info_sql =sql_select("SELECT a.job_no,b.body_part_id,b.lib_yarn_count_deter_id, b.gsm_weight, a.booking_no as wopi_number,b.uom, a.dia_width, a.fabric_color_id,a.gmts_color_id,b.color_size_sensitive,c.fabric_ref,c.rd_no,b.gsm_weight_type as weight_type,g.item_size as cutable_width from wo_booking_dtls a, wo_pre_cost_fabric_cost_dtls b join  wo_pre_cos_fab_co_avg_con_dtls g on b.id=g.pre_cost_fabric_cost_dtls_id ,lib_yarn_count_determina_mst c  where a.pre_cost_fabric_cost_dtls_id=b.id and b.lib_yarn_count_deter_id=c.id and a.status_active=1 and a.is_deleted=0 $poIds_cond group by a.job_no,b.body_part_id,b.lib_yarn_count_deter_id, b.gsm_weight, a.booking_no,b.uom, a.dia_width, a.fabric_color_id,a.gmts_color_id,b.color_size_sensitive,c.fabric_ref,c.rd_no,b.gsm_weight_type,g.item_size");

		//and a.po_break_down_id in($poIDs)
		foreach($determination_info_sql as $row)
		{
			$determination_info_arr[$row[csf('lib_yarn_count_deter_id')]][$row[csf('uom')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('gmts_color_id')]][$row[csf('body_part_id')]][$row[csf('color_size_sensitive')]]['rd_no']=$row[csf('rd_no')];
			$determination_info_arr[$row[csf('lib_yarn_count_deter_id')]][$row[csf('uom')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('gmts_color_id')]][$row[csf('body_part_id')]][$row[csf('color_size_sensitive')]]['fabric_ref']=$row[csf('fabric_ref')];
			$determination_info_arr[$row[csf('lib_yarn_count_deter_id')]][$row[csf('uom')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('gmts_color_id')]][$row[csf('body_part_id')]][$row[csf('color_size_sensitive')]]['weight_type']=$row[csf('weight_type')];
			$determination_info_arr[$row[csf('lib_yarn_count_deter_id')]][$row[csf('uom')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('gmts_color_id')]][$row[csf('body_part_id')]][$row[csf('color_size_sensitive')]]['cutable_width']=$row[csf('cutable_width')];
	...
	else // po wise
	{
		if($hdn_variable_colorSize_status==2)
		{
			...
			else
			{
				...
				from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, wo_pre_cos_fab_co_avg_con_dtls d, wo_pre_cost_fabric_cost_dtls e,wo_booking_dtls f
				where a.id=b.job_id and b.id=c.po_break_down_id and c.po_break_down_id=d.po_break_down_id and c.color_number_id=d.color_number_id and c.size_number_id=d.gmts_sizes and d.pre_cost_fabric_cost_dtls_id=e.id and c.job_no_mst=e.job_no and c.item_number_id=e.item_number_id and a.is_deleted=0 and a.status_active=1  and a.company_name=$company_id $buyer_id_cond $job_no_cond $year_cond $po_cond $int_ref_cond $batch_cond $style_ref_cond $brand_no_cond $season_no_cond $season_year_cond 
				and e.job_no=f.job_no and d.pre_cost_fabric_cost_dtls_id=f.pre_cost_fabric_cost_dtls_id and  e.gsm_weight=f.gsm_weight and d.dia_width=f.dia_width and f.po_break_down_id=b.id and f.is_deleted=0 and f.status_active=1  
				group by a.job_no, a.job_no_prefix_num, a.buyer_name,a.insert_date, b.id, b.po_number,b.grouping, c.item_number_id, c.color_number_id, e.body_part_id, e.lib_yarn_count_deter_id, e.gsm_weight, d.dia_width,e.uom, e.avg_finish_cons,a.style_ref_no,e.color_size_sensitive, a.id, e.seq,f.fin_fab_qnty order by a.id, e.seq asc";
	 ...
	 $booking_qnty_sql = "SELECT a.buyer_name,a.job_no, a.job_no_prefix_num,a.style_ref_no, c.item_number_id as item_id, e.body_part_id, e.lib_yarn_count_deter_id, c.color_number_id as color_id,f.fabric_color_id,e.uom, e.avg_finish_cons as avg_cons ,sum((d.cons/d.pcs)*c.plan_cut_qnty) as budget_qty, TO_CHAR(a.insert_date,'YYYY') as year ,LISTAGG(b.id,',') WITHIN GROUP (ORDER BY b.id) as po_id ,LISTAGG(b.po_number,',') WITHIN GROUP (ORDER BY b.po_number) as po_number ,b.grouping, e.gsm_weight, d.dia_width,e.color_size_sensitive,f.fin_fab_qnty as booking_qnty
	from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, wo_pre_cos_fab_co_avg_con_dtls d, wo_pre_cost_fabric_cost_dtls e,wo_booking_dtls f
	where a.id=b.job_id and b.id=c.po_break_down_id and c.po_break_down_id=d.po_break_down_id and c.color_number_id=d.color_number_id and c.size_number_id=d.gmts_sizes and d.pre_cost_fabric_cost_dtls_id=e.id and c.job_no_mst=e.job_no and c.item_number_id=e.item_number_id and a.is_deleted=0 and a.status_active=1  and a.company_name=$company_id $buyer_id_cond $job_no_cond $year_cond $po_cond $int_ref_cond $batch_cond $style_ref_cond $brand_no_cond $season_no_cond $season_year_cond and e.job_no=f.job_no and d.pre_cost_fabric_cost_dtls_id=f.pre_cost_fabric_cost_dtls_id  and  e.gsm_weight=f.gsm_weight and d.dia_width=f.dia_width and f.po_break_down_id=b.id and f.pre_cost_fabric_cost_dtls_id=e.id and c.color_number_id=f.gmts_color_id and f.is_deleted=0 and f.status_active=1
	group by  a.buyer_name,a.job_no, a.job_no_prefix_num,a.style_ref_no, c.item_number_id, e.body_part_id, e.lib_yarn_count_deter_id, c.color_number_id,f.fabric_color_id,e.uom, e.avg_finish_cons ,a.insert_date,b.grouping, e.gsm_weight, d.dia_width,e.color_size_sensitive, a.id, e.seq,f.fin_fab_qnty order by a.id, e.seq asc";

	$booking_qnty_sqlArr = sql_select($booking_qnty_sql);
	foreach ($booking_qnty_sqlArr as $rowData) 
	{
		$bookintQntyArr[$rowData[csf('job_no')]][$rowData[csf('gsm_weight')]][$rowData[csf('dia_width')]][$rowData[csf('color_id')]][$rowData[csf('body_part_id')]][$rowData[csf('lib_yarn_count_deter_id')]][$rowData[csf('color_size_sensitive')]]['booking_qnty']+=$rowData[csf('booking_qnty')];
		//$bookintFabColorArr[$rowData[csf('job_no')]][$rowData[csf('gsm_weight')]][$rowData[csf('dia_width')]][$rowData[csf('color_id')]][$rowData[csf('body_part_id')]][$rowData[csf('lib_yarn_count_deter_id')]][$rowData[csf('color_size_sensitive')]]['fabric_color_id']=$rowData[csf('fabric_color_id')];
	}
	unset($booking_qnty_sqlArr);

	//echo "<pre>";
	//print_r($bookintQntyArr);echo "</pre>";
	//echo $sql;
	$result = sql_select($sql);

	$po_id_array = array();
	$job_id_array = array();$po_id_chk_array = array();
	foreach ($result as $val) 
	{
		...
		foreach ($poExp as $poIds) 
		{
			if($po_id_array[$poIds] != $poIds)
			{
				$po_id_array[$poIds] = $poIds;
			}
    ...
    //echo "<pre>";print_r($fabric_req_arr['woven']['grey']);//die();
	...
	if($hdn_variable_setting_status==1) // po to style
	{
		...
		{
			$sqlPrev="SELECT id, buyer_id, job_no, item_id, body_part, determination_id, gsm, dia, color_id, reqn_qty,cons from pro_fab_reqn_for_cutting_dtls where status_active=1 and is_deleted=0 $po_id_cond";
			// echo $sqlPrev;die();
			$resultPrev=sql_select($sqlPrev);
			$prevRcvQntyArr = array();
			foreach ($resultPrev as $val) 
			{
				$prevRcvQntyArr[$val[csf('buyer_id')]][$val[csf('job_no')]][$val[csf('item_id')]][$val[csf('body_part')]][$val[csf('determination_id')]][$val[csf('gsm')]][$val[csf('dia')]][$val[csf('color_id')]][$val[csf('cons')]]+=$val[csf('reqn_qty')];
			}
		}
		else
		{
	...
	$wvn_recv_sql="select x.buyer_id,a.mst_id,a.fabric_description_id,a.original_gsm,a.original_width,b.body_part_id, b.fabric_ref,b.rd_no,b.cutable_width,b.weight_type,c.po_breakdown_id,c.color_id,c.quantity,d.job_no_mst from inv_receive_master x, pro_finish_fabric_rcv_dtls a,inv_transaction b,order_wise_pro_details c,wo_po_break_down d 
	where x.id=b.mst_id and a.trans_id=b.id and c.trans_id=b.id and c.po_breakdown_id=d.id and c.entry_form=17 and b.item_category=3 and b.transaction_type=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 $po_id_cond_wvn_recv";
	...
	foreach($wvn_recv_sql_rslt as $rows)
	{
		if($hdn_variable_setting_status==1) // style wise
		{
			$wvn_recv_qnty_array[$rows[csf('job_no_mst')]][$rows[csf('buyer_id')]][$rows[csf('fabric_description_id')]][$rows[csf('body_part_id')]][$rows[csf('original_gsm')]][$rows[csf('original_width')]][$rows[csf('fabric_ref')]][$rows[csf('rd_no')]][$rows[csf('cutable_width')]][$rows[csf('weight_type')]][$rows[csf('color_id')]]['wvn_recv_qty']+=$rows[csf('quantity')];
		}
		else
		{
	...
	$wvn_issue_sql="select x.buyer_id,a.mst_id,a.fabric_description_id,a.original_gsm,a.original_width,b.body_part_id, b.fabric_ref,b.rd_no,b.cutable_width,b.weight_type,c.po_breakdown_id,c.color_id,c.quantity,d.job_no_mst from inv_issue_master x, INV_WVN_FINISH_FAB_ISS_DTLS a,inv_transaction b,order_wise_pro_details c,wo_po_break_down d 
	where x.id=b.mst_id and a.trans_id=b.id and c.trans_id=b.id and c.po_breakdown_id=d.id and c.entry_form=19 and b.item_category=3 and b.transaction_type=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 $po_id_cond_wvn_recv";

	$wvn_issue_sql_rslt=sql_select($wvn_issue_sql);
	$wvn_issue_qnty_array=array();
	foreach($wvn_issue_sql_rslt as $rows)
	{
		if($hdn_variable_setting_status==1) // style wise
		{
			$wvn_issue_qnty_array[$rows[csf('job_no_mst')]][$rows[csf('fabric_description_id')]][$rows[csf('body_part_id')]][$rows[csf('original_gsm')]][$rows[csf('original_width')]][$rows[csf('fabric_ref')]][$rows[csf('rd_no')]][$rows[csf('cutable_width')]][$rows[csf('weight_type')]][$rows[csf('color_id')]]['wvn_issue_qty']+=$rows[csf('quantity')];
			
		}
		else
		{
			$wvn_issue_qnty_array[$rows[csf('fabric_description_id')]][$rows[csf('body_part_id')]][$rows[csf('original_gsm')]][$rows[csf('original_width')]][$rows[csf('po_breakdown_id')]][$rows[csf('color_id')]]['wvn_issue_qty']+=$rows[csf('quantity')];
		}
	}
	unset($wvn_issue_sql_rslt);
	//echo "<pre>";
	//print_r($wvn_issue_qnty_array);

	$wvn_recv_rtn_sql="select x.buyer_id,a.DETARMINATION_ID as fabric_description_id,b.weight_editable,b.width_editable
	,b.body_part_id, 
	b.fabric_ref,b.rd_no,b.cutable_width,b.weight_type,c.po_breakdown_id,c.color_id,c.quantity 
	from inv_issue_master x, inv_transaction b,order_wise_pro_details c,product_details_master a 
	where x.id=b.mst_id  and c.trans_id=b.id and c.prod_id=a.id and b.prod_id=a.id and c.entry_form=202 and b.item_category=3 
	and b.transaction_type=3  and b.status_active=1 and b.is_deleted=0 
	and c.status_active=1 and c.is_deleted=0 $po_id_cond_wvn_recv";
	
	$wvn_recv_rtn_sql_rslt=sql_select($wvn_recv_rtn_sql);
	$wvn_recv_rtn_qnty_array=array();
	foreach($wvn_recv_rtn_sql_rslt as $rows)
	{
		if($hdn_variable_setting_status==1) // style wise
		{

			$wvn_recv_rtn_qnty_array[$rows[csf('fabric_description_id')]][$rows[csf('body_part_id')]][$rows[csf('weight_editable')]][$rows[csf('width_editable')]][$rows[csf('fabric_ref')]][$rows[csf('rd_no')]][$rows[csf('cutable_width')]][$rows[csf('weight_type')]][$rows[csf('color_id')]]['wvn_rcv_rtn_qty']=$rows[csf('quantity')];
			
		}
		else
		{
			$wvn_recv_rtn_qnty_array[$rows[csf('fabric_description_id')]][$rows[csf('body_part_id')]][$rows[csf('original_gsm')]][$rows[csf('original_width')]][$rows[csf('po_breakdown_id')]][$rows[csf('color_id')]]['wvn_rcv_rtn_qty']=$rows[csf('quantity')];
		}
	}
	unset($wvn_recv_rtn_sql_rslt);

	$wvn_issue_rtn_sql="select a.DETARMINATION_ID as fabric_description_id,b.weight_editable,b.width_editable,b.body_part_id, b.fabric_ref,b.rd_no,b.cutable_width,b.weight_type,c.po_breakdown_id,c.color_id,c.quantity 
	from inv_receive_master x, inv_transaction b,order_wise_pro_details c,product_details_master a 
	where x.id=b.mst_id and c.trans_id=b.id and c.prod_id=a.id and b.prod_id=a.id  and c.entry_form=209 and b.item_category=3 and b.transaction_type=4 
	and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $po_id_cond_wvn_recv";
	$wvn_issue_rtn_sql_rslt=sql_select($wvn_issue_rtn_sql);
	$wvn_issue_rtn_qnty_array=array();
	foreach($wvn_issue_rtn_sql_rslt as $rows)
	{
		if($hdn_variable_setting_status==1) // style wise
		{
			$wvn_issue_rtn_qnty_array[$rows[csf('fabric_description_id')]][$rows[csf('body_part_id')]][$rows[csf('weight_editable')]][$rows[csf('width_editable')]][$rows[csf('fabric_ref')]][$rows[csf('rd_no')]][$rows[csf('cutable_width')]][$rows[csf('weight_type')]][$rows[csf('color_id')]]['wvn_issue_rtn_qty']=$rows[csf('quantity')];
		}
		else
		{
			$wvn_issue_rtn_qnty_array[$rows[csf('fabric_description_id')]][$rows[csf('body_part_id')]][$rows[csf('weight_editable')]][$rows[csf('width_editable')]][$rows[csf('po_breakdown_id')]][$rows[csf('color_id')]]['wvn_issue_rtn_qty']=$rows[csf('quantity')];
		}
	}
	//print_r($wvn_recv_qnty_array);
	unset($wvn_issue_rtn_sql_rslt);


	$prev_transIn_sql=sql_select("select a.DETARMINATION_ID as fabric_description_id,b.weight_editable,b.width_editable,b.body_part_id, b.fabric_ref,b.rd_no,b.cutable_width,b.weight_type,c.po_breakdown_id,c.color_id,c.quantity from inv_item_transfer_mst x, inv_transaction b, inv_item_transfer_dtls d,order_wise_pro_details c,product_details_master a where x.id=b.mst_id and b.mst_id=d.mst_id and x.id=d.mst_id and b.id=d.trans_id and c.trans_id=d.to_trans_id and c.prod_id=a.id and b.prod_id=a.id  and c.entry_form=258 and b.item_category=3 and b.transaction_type=5 $po_id_cond_wvn_recv and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0");
	$wvn_transIn_qnty_array=array();
	foreach($prev_transIn_sql as $rows)
	{
		if($hdn_variable_setting_status==1) // style wise
		{
			$wvn_transIn_qnty_array[$rows[csf('fabric_description_id')]][$rows[csf('body_part_id')]][$rows[csf('weight_editable')]][$rows[csf('width_editable')]][$rows[csf('fabric_ref')]][$rows[csf('rd_no')]][$rows[csf('cutable_width')]][$rows[csf('weight_type')]][$rows[csf('color_id')]]['wvn_transfer_in_qty']=$rows[csf('quantity')];
		}
		else
		{
			$wvn_transIn_qnty_array[$rows[csf('fabric_description_id')]][$rows[csf('body_part_id')]][$rows[csf('weight_editable')]][$rows[csf('width_editable')]][$rows[csf('po_breakdown_id')]][$rows[csf('color_id')]]['wvn_transfer_in_qty']=$rows[csf('quantity')];
		}
	}
	unset($prev_transOut_sql);
	

	$prev_transOut_sql=sql_select("select a.DETARMINATION_ID as fabric_description_id,b.weight_editable,b.width_editable,b.body_part_id, b.fabric_ref,b.rd_no,b.cutable_width,b.weight_type,c.po_breakdown_id,c.color_id,c.quantity from inv_item_transfer_mst x, inv_transaction b, inv_item_transfer_dtls d,order_wise_pro_details c,product_details_master a where x.id=b.mst_id and b.mst_id=d.mst_id and x.id=d.mst_id and b.id=d.trans_id and c.trans_id=d.trans_id and c.prod_id=a.id and b.prod_id=a.id  and c.entry_form=258 and b.item_category=3 and b.transaction_type=6 $po_id_cond_wvn_recv and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0");
	$wvn_transOut_qnty_array=array();
	foreach($prev_transOut_sql as $rows)
	{
		if($hdn_variable_setting_status==1) // style wise
		{
			$wvn_transOut_qnty_array[$rows[csf('fabric_description_id')]][$rows[csf('body_part_id')]][$rows[csf('weight_editable')]][$rows[csf('width_editable')]][$rows[csf('fabric_ref')]][$rows[csf('rd_no')]][$rows[csf('cutable_width')]][$rows[csf('weight_type')]][$rows[csf('color_id')]]['wvn_transfer_out_qty']=$rows[csf('quantity')];
		}
		else
		{
			$wvn_transOut_qnty_array[$rows[csf('fabric_description_id')]][$rows[csf('body_part_id')]][$rows[csf('weight_editable')]][$rows[csf('width_editable')]][$rows[csf('po_breakdown_id')]][$rows[csf('color_id')]]['wvn_transfer_out_qty']=$rows[csf('quantity')];
		}
	}
	unset($prev_transOut_sql);
		...
		<? 
			...
			{  
				...
				if($row[csf('color_size_sensitive')]==3)
				{
					...
					$fab_color_id = $fab_color_id_arr[$row[csf('job_no')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('gsm_weight')]][$row[csf('color_id')]];
				}
				else
				{
					$fab_color_id = $row[csf('color_id')];
				}
				...
				if($hdn_variable_setting_status==1) // po to style wise
				{
					...
					if($hdn_variable_colorSize_status==2) // color level
					{						
						$prevRcvQnty=$prevRcvQntyArr[$row[csf('buyer_name')]][$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('color_id')]][$row[csf('avg_cons')]];
						...
						//echo $row[csf('job_no')].'='.$row[csf('color_id')].'='.$fabricColorId.'='.$row[csf('body_part_id')].'='.$row[csf('lib_yarn_count_deter_id')].'='.$row[csf('gsm_weight')].'='.$row[csf('dia_width')].'='.$row[csf('fabric_source')].'='.$row[csf('uom')].'<br/>';


						$row[csf('budget_qty')]=$fabric_req_arr['woven']['grey'][$row[csf('job_no')]][$row[csf('color_id')]][$fabricColorId][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('fabric_source')]][$row[csf('uom')]];
						...
						//[$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('color_id')]][$avg_cons];


						//getQtyArray_by_JobColorFabricColorBodyPartIdDeterminIdGsmAndDiaWidth_knitAndwoven_greyAndfinish();

						//echo $fabric_req_knit.",";
                ...
                if($row[csf('budget_qty')] < $reqnQty)
				{
					$reqnQty = $row[csf('budget_qty')];
                    $balance_qty = 0;
				}
				...
				if($hdn_variable_setting_status==1) // style wise
				{
					...
					foreach ($po_arr_uniq as $poID) 
					{
						if($hdn_variable_colorSize_status==2)
						{
							$orderRequiredQnty=  $po_wise_budgt_qnt_arr[$poID][$row[csf('buyer_name')]][$row[csf('job_no')]][$row[csf('style_ref_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('color_id')]][$row[csf('grouping')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]]['qnty'];

							$jobRequiredQnty=$job_wise_budgt_qnt_arr[$row[csf('buyer_name')]][$row[csf('job_no')]][$row[csf('style_ref_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('color_id')]][$row[csf('grouping')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]]['qnty'];
							
							//echo $orderRequiredQnty.'<br/>';
						}
						else
						{
							...
							$jobRequiredQnty=  $job_wise_budgt_qnt_arr[$row[csf('buyer_name')]][$row[csf('job_no')]][$row[csf('style_ref_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('color_id')]][$row[csf('size_id')]][$row[csf('grouping')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]]['qnty'];  
						}
						//echo $poID."##".$orderRequiredQnty.'/'.$row[csf('budget_qty')].')*'.$balance_qty.'<br/>';
						//echo $poID."##".$orderRequiredQnty.'/'.$jobRequiredQnty.')*'.$balance_qty.'<br/>';

						//$hdn_po_ratio_qnty.=$poID."##".($orderRequiredQnty/$row[csf('budget_qty')])*$balance_qty.",";
						$hdn_po_ratio_qnty.=$poID."##".($orderRequiredQnty/$jobRequiredQnty)*$balance_qty.",";
					...
					//$fabricColorID=	$bookintFabColorArr[$row[csf('job_no')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('color_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('color_size_sensitive')]]['fabric_color_id'];

					//echo $fabricColorID."<br/>";

					$rd_no=$determination_info_arr[$row[csf('lib_yarn_count_deter_id')]][$row[csf('uom')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('color_id')]][$row[csf('body_part_id')]][$row[csf('color_size_sensitive')]]['rd_no'];
					$fabric_ref=$determination_info_arr[$row[csf('lib_yarn_count_deter_id')]][$row[csf('uom')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('color_id')]][$row[csf('body_part_id')]][$row[csf('color_size_sensitive')]]['fabric_ref'];
					$weight_type=$determination_info_arr[$row[csf('lib_yarn_count_deter_id')]][$row[csf('uom')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('color_id')]][$row[csf('body_part_id')]][$row[csf('color_size_sensitive')]]['weight_type'];
					
					$cutable_width=$determination_info_arr[$row[csf('lib_yarn_count_deter_id')]][$row[csf('uom')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('color_id')]][$row[csf('body_part_id')]][$row[csf('color_size_sensitive')]]['cutable_width']; 



					//wvn recv

					if($cutable_width==""){$cutable_widthx=0;}else{$cutable_widthx=$cutable_width;}
					...
					$wvn_rcv_qnty=$wvn_recv_qnty_array[$row[csf('job_no')]][$row[csf('buyer_name')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('body_part_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$fabric_ref][$rd_no][$cutable_widthx][$weight_type][$fab_color_id]['wvn_recv_qty'];
					$wvn_issueRtn_qnty=$wvn_issue_rtn_qnty_array[$row[csf('lib_yarn_count_deter_id')]][$row[csf('body_part_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$fabric_ref][$rd_no][$cutable_widthx][$weight_type][$fab_color_id]['wvn_issue_rtn_qty'];
					$wvn_transfIn_qnty=$wvn_transIn_qnty_array[$row[csf('lib_yarn_count_deter_id')]][$row[csf('body_part_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$fabric_ref][$rd_no][$cutable_widthx][$weight_type][$fab_color_id]['wvn_transfer_in_qty'];

					$wvn_issue_qnty=$wvn_issue_qnty_array[$row[csf('job_no')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('body_part_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$fabric_ref][$rd_no][$cutable_widthx][$weight_type][$fab_color_id]['wvn_issue_qty'];
					$wvn_recvRtn_qnty=$wvn_recv_rtn_qnty_array[$row[csf('lib_yarn_count_deter_id')]][$row[csf('body_part_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$fabric_ref][$rd_no][$cutable_widthx][$weight_type][$fab_color_id]['wvn_rcv_rtn_qty'];
					$wvn_transfOut_qnty=$wvn_transOut_qnty_array[$row[csf('lib_yarn_count_deter_id')]][$row[csf('body_part_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$fabric_ref][$rd_no][$cutable_widthx][$weight_type][$fab_color_id]['wvn_transfer_out_qty'];
					$totalWovenRecv=($wvn_rcv_qnty+$wvn_issueRtn_qnty+$wvn_transfIn_qnty);
					$totalWovenStock=($wvn_rcv_qnty+$wvn_issueRtn_qnty+$wvn_transfIn_qnty)-($wvn_issue_qnty+$wvn_recvRtn_qnty+$wvn_transfOut_qnty);
					//echo $totalWovenStock;
					//$booking_qnty_blnc=$wvn_rcv_qnty-$wvn_issue_qnty;
					
					//echo $wvn_rcv_qnty.'-'.$wvn_issue_qnty.'='. $booking_qnty_blnc."<br/>";

					$bookingQuantity=$bookintQntyArr[$row[csf('job_no')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('color_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('color_size_sensitive')]]['booking_qnty'];

					$data=$buyer_short_name_arr[$row[csf('buyer_name')]]."**".$row[csf('buyer_name')]."**".$row[csf('year')]."**".$row[csf('job_no_prefix_num')]."**".$row[csf('job_no')]."**".$row[csf('po_number')]."**".$row[csf('po_id')]."**".$row[csf('item_id')]."**".$garments_item[$row[csf('item_id')]]."**".$row[csf('body_part_id')]."**".$body_part[$row[csf('body_part_id')]]."**".$cons_comp."**".$row[csf('lib_yarn_count_deter_id')]."**".$row[csf('gsm_weight')]."**".$row[csf('dia_width')]."**".$color_arr[$row[csf('color_id')]]."**".$row[csf('color_id')]."**".$sizeArr[$row[csf('size_id')]]."**".$row[csf('size_id')]."**".$unit_of_measurement[$row[csf('uom')]]."**".$avg_cons."**".number_format($row[csf('budget_qty')],2,'.','')."**".number_format($balance_qty,2,'.','')."**".number_format($prevRcvQnty,2,'.','')."**".$row[csf('style_ref_no')]."**".$fabric_ref."**".$rd_no."**".$weight_type."**".$fabric_weight_type[$weight_type]."**".$cutable_width."**".$hdn_po_ratio_ref."**".$hdn_po_ratio_qnty."**".$color_arr[$fab_color_id]."**".$fab_color_id."**".$is_disable."**".number_format($reqnQty,2)."**".number_format($bookingQuantity,2)."**".number_format($totalWovenRecv,2)."**".number_format($totalWovenStock,2)."**".$row[csf('fabric_source')]."**".$uomData;
				}
				else // po wise
				{
					...
					$data=$buyer_short_name_arr[$row[csf('buyer_name')]]."**".$row[csf('buyer_name')]."**".$row[csf('year')]."**".$row[csf('job_no_prefix_num')]."**".$row[csf('job_no')]."**".$row[csf('po_number')]."**".$row[csf('po_id')]."**".$row[csf('item_id')]."**".$garments_item[$row[csf('item_id')]]."**".$row[csf('body_part_id')]."**".$body_part[$row[csf('body_part_id')]]."**".$cons_comp."**".$row[csf('lib_yarn_count_deter_id')]."**".$row[csf('gsm_weight')]."**".$row[csf('dia_width')]."**".$color_arr[$row[csf('color_id')]]."**".$row[csf('color_id')]."**".$sizeArr[$row[csf('size_id')]]."**".$row[csf('size_id')]."**".$unit_of_measurement[$row[csf('uom')]]."**".$avg_cons."**".number_format($row[csf('budget_qty')],2,'.','')."**".number_format($balance_qty,2,'.','')."**".number_format($prevRcvQnty,2,'.','')."**".$row[csf('style_ref_no')]."**".$color_arr[$fab_color_id]."**".$fab_color_id."**".$is_disable."**".number_format($reqnQty,2)."**".$row[csf('booking_qnty')]."**".$wvn_rcv_qnty."**".$booking_qnty_blnc;

					//echo $data."<br>";

				}
				 //echo $totalWovenRecv."<br>";

				if($totalWovenRecv>0) // conditon add for data showing if found woven recv. crm issue id: 16539
				{
					?>
					<tr title="<?=$row[csf('color_size_sensitive')];?>" bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i; ?>" onClick="js_set_value(<? echo $i; ?>,<? echo $row[csf('buyer_name')];?>,<? echo $row[csf('budget_qty')];?>)"> 
						...
						<td style="word-wrap: break-word;word-break: break-all;" width="60" title="PO Number:<? echo $row[csf('po_number')]; ?>"><p>&nbsp;&nbsp;<? echo $row[csf('job_no_prefix_num')]; ?></p></td>
						<? 
						if($hdn_variable_setting_status!=1)
						{
							?>
	                    ...
	                    <td style="word-wrap: break-word;word-break: break-all;" width="80" title="<? echo $row[csf('body_part_id')]; ?>"><p><? echo $body_part[$row[csf('body_part_id')]]; ?></p></td>
						<td style="word-wrap: break-word;word-break: break-all;" width="140" title="<? echo $row[csf('lib_yarn_count_deter_id')]; ?>"><p><? echo $cons_comp; ?></p></td>
						<td style="word-wrap: break-word;word-break: break-all;" width="40"><p><? echo $row[csf('gsm_weight')]; ?></p></td>
						<td style="word-wrap: break-word;word-break: break-all;" width="40" title="<? echo $row[csf('dia_width')]; ?>"><p><? echo $row[csf('dia_width')]; ?></p></td>
						<td style="word-wrap: break-word;word-break: break-all;" width="70" title="<? echo $row[csf('color_id')]; ?>"><p><? echo $color_arr[$row[csf('color_id')]]; ?></p></td>
						<td style="word-wrap: break-word;word-break: break-all;" width="70" title="<? echo $fab_color_id; ?>"><p><? echo $color_arr[$fab_color_id]; ?></p></td>
	...
	<table width="1330" cellspacing="0" cellpadding="0" border="1" align="center">
		<tr>
...
if ($action=="save_update_delete")
{
	...
	{ 	
		...
		$data_array="(".$id.",'".$new_mrr_number[1]."',".$new_mrr_number[2].",'".$new_mrr_number[0]."',".$cbo_company_id.",".$cbo_working_company_id.",".$cbo_location.",".$cbo_production_floor.",".$layPlan_id.",".$txt_requisition_date.",507,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		if($hdn_variable_setting_status==1)
		{
			$field_array_dtls="id, mst_id, buyer_id, po_id, job_no, item_id, body_part, determination_id, gsm, dia, color_id,fab_color_id, size_id,fabric_ref,weight_type,cutable_width,rd_no,style_ref_no,cons,fabric_source,uom,reqn_qty, inserted_by, insert_date";
			for($j=1;$j<=$tot_row;$j++)
			{ 
				...
				foreach ($hdnPoRatioQtyArr as $poData) 
				{
					...
					$styleRef="styleRef".$j;
					$cons="cons".$j;
					$fabricSource="fabricSource".$j;
					$uom="uom".$j;

					$reqsnQty="reqsnQty".$j;

					if($data_array_dtls!="") $data_array_dtls.=",";
					$data_array_dtls.="(".$dtls_id.",".$id.",".$$buyerId.",'".$hdnPoAndQtyArr[0]."','".$$jobNo."','".$$itemId."','".$$bodyPartId."','".$$deterId."','".$$gsm."','".$$dia."','".$$colorId."','".$$fabColorId."','".$$sizeId."','".$$fabricRef."','".$$weightType."','".$$cutableWidth."','".$$rdNo."','".$$styleRef."','".$$cons."','".$$fabricSource."','".$$uom."','".$hdnPoAndQtyArr[1]."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
					$dtls_id = $dtls_id+1;
		...
		{
			$field_array_dtls="id, mst_id, buyer_id, po_id, job_no, item_id, body_part, determination_id, gsm, dia, color_id,fab_color_id, size_id,style_ref_no,cons,reqn_qty, inserted_by, insert_date";
			for($j=1;$j<=$tot_row;$j++)
			{ 
				$buyerId="buyerId".$j;
				$jobNo="jobNo".$j;
				...
				if($data_array_dtls!="") $data_array_dtls.=",";
				$data_array_dtls.="(".$dtls_id.",".$id.",".$$buyerId.",'".$$poId."','".$$jobNo."','".$$itemId."','".$$bodyPartId."','".$$deterId."','".$$gsm."','".$$dia."','".$$colorId."','".$$fabColorId."','".$$sizeId."','".$$styleRef."','".$$cons."','".$$reqsnQty."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				$dtls_id = $dtls_id+1;
			}
		}
	...
	else if ($operation==1)   // Update Here
	{
		...
		if($hdn_variable_setting_status==1)
		{
			$field_array_dtls="id, mst_id, buyer_id, po_id, job_no, item_id, body_part, determination_id,style_ref_no,cons,fabric_source,uom, gsm, dia, color_id,fab_color_id, size_id,fabric_ref,weight_type,cutable_width,rd_no, reqn_qty, inserted_by, insert_date";
			...
			{ 
				...
				foreach ($hdnPoRatioQtyArr as $poData) 
				{
					...
					{
						if($data_array_dtls!="") $data_array_dtls.=",";
						$data_array_dtls.="(".$dtls_id.",".$update_id.",".$$buyerId.",'".$hdnPoAndQtyArr[0]."','".$$jobNo."','".$$itemId."','".$$bodyPartId."','".$$deterId."','".$$styleRef."','".$$cons."','".$$fabricSource."','".$$uom."','".$$gsm."','".$$dia."','".$$colorId."','".$$fabColorId."','".$$sizeId."','".$$fabricRef."','".$$weightType."','".$$cutableWidth."','".$$rdNo."','".$hdnPoAndQtyArr[1]."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
						$dtls_id = $dtls_id+1;	
					}
				}
			}
		...
		else
		{
			...
			{ 	
				...
				{
					if($data_array_dtls!="") $data_array_dtls.=",";
					$data_array_dtls.="(".$dtls_id.",".$update_id.",".$$buyerId.",'".$$poId."','".$$jobNo."','".$$itemId."','".$$bodyPartId."','".$$deterId."','".$$styleRef."','".$$cons."','".$$gsm."','".$$dia."','".$$colorId."','".$$fabColorId."','".$$sizeId."','".$$reqsnQty."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
					$dtls_id = $dtls_id+1;	
				}
			}
		}
...
if($action=='populate_data_from_requisition')
{
	...
	{ 
		...
		echo "load_drop_down( 'requires/woven_fabric_requisition_for_cutting_entry_controller', '".$row[csf("working_company")]."', 'load_drop_down_location','location_td');\n";
		echo "load_drop_down( 'requires/woven_fabric_requisition_for_cutting_entry_controller', '".$row[csf("working_company")]."_".$row[csf("location_id")]."', 'load_drop_down_production_floor','floor_td');\n";
...
{	
	...
	if(count($all_po_arr))
	{
		$po_id_cond_wvn_recv=where_con_using_array($all_po_arr,0,"c.po_breakdown_id");
	}
	...
	if($data[1]==1)
	{
		if($data[2]==2)
		{
			...
			foreach ($resultPrev as $val) 
			{
				$prevRcvQntyArr[$val[csf('buyer_id')]][$val[csf('job_no')]][$val[csf('item_id')]][$val[csf('body_part')]][$val[csf('determination_id')]][$val[csf('gsm')]][$val[csf('dia')]][$val[csf('color_id')]][$val[csf('fabric_ref')]][$val[csf('rd_no')]][$val[csf('cutable_width')]][$val[csf('weight_type')]][$val[csf('cons')]]+=$val[csf('reqn_qty')];
			}
	...
	if($data[1]==1)
	{
		if($data[2]==2)
		{
            ...
            foreach ($sql_budget_po_wise as $row)
			{
				...
				$job_wise_budgt_qnt_arr[$row[csf('buyer_name')]][$row[csf('job_no')]][$row[csf('style_ref_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('color_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]]['qnty']+=$row[csf('budget_qty')];
				//$po_wise_booking_qnt_arr[$row[csf('po_id')]][$row[csf('buyer_name')]][$row[csf('job_no')]][$row[csf('style_ref_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('color_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]]['booking_qnty']+=$row[csf('booking_qnty')];

			}

			//$sql_budget= sql_select("SELECT a.buyer_name,a.job_no, a.job_no_prefix_num,a.style_ref_no, c.item_number_id as item_id, e.body_part_id, e.lib_yarn_count_deter_id, c.color_number_id as color_id,e.uom, e.avg_finish_cons as avg_cons ,sum((d.cons/d.pcs)*c.plan_cut_qnty) as budget_qty, TO_CHAR(a.insert_date,'YYYY') as year ,LISTAGG(b.id,',') WITHIN GROUP (ORDER BY b.id) as po_id ,LISTAGG(b.po_number,',') WITHIN GROUP (ORDER BY b.po_number) as po_number ,b.grouping, e.gsm_weight, d.dia_width,e.color_size_sensitive,e.fabric_source,f.stripe_color,g.contrast_color_id  from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, wo_pre_cos_fab_co_avg_con_dtls d, wo_pre_cost_fabric_cost_dtls e  left join wo_pre_cos_fab_co_color_dtls  g on  e.id=g.pre_cost_fabric_cost_dtls_id and g.is_deleted=0 and g.status_active=1 left join wo_pre_stripe_color f on  e.id=f.pre_cost_fabric_cost_dtls_id where a.id=b.job_id and b.id=c.po_break_down_id and c.po_break_down_id=d.po_break_down_id and c.color_number_id=d.color_number_id and c.size_number_id=d.gmts_sizes and d.pre_cost_fabric_cost_dtls_id=e.id and c.job_no_mst=e.job_no and c.item_number_id=e.item_number_id and a.is_deleted=0 and a.status_active=1   and b.id in($all_po_id) group by  a.buyer_name,a.job_no, a.job_no_prefix_num,a.style_ref_no, c.item_number_id, e.body_part_id, e.lib_yarn_count_deter_id, c.color_number_id,e.uom, e.avg_finish_cons ,a.insert_date,b.grouping, e.gsm_weight, d.dia_width,e.color_size_sensitive, a.id, e.seq ,e.fabric_source,f.stripe_color,g.contrast_color_id order by a.id, e.seq asc");	

			$sql_budget = sql_select("SELECT a.buyer_name,a.job_no, a.job_no_prefix_num,a.style_ref_no, c.item_number_id as item_id, d.body_part_id, d.lib_yarn_count_deter_id, c.color_number_id as color_id,d.uom,d.avg_finish_cons as avg_cons ,sum((e.cons/e.pcs)*c.plan_cut_qnty) as budget_qty, TO_CHAR(a.insert_date,'YYYY') as year ,LISTAGG(b.id,',') WITHIN GROUP (ORDER BY b.id) as po_id ,LISTAGG(b.po_number,',') WITHIN GROUP (ORDER BY b.po_number) as po_number ,b.grouping, d.gsm_weight, e.dia_width,d.color_size_sensitive,d.fabric_source,h.stripe_color,g.contrast_color_id 
				from wo_po_details_master a join wo_po_break_down b on a.id=b.job_id join wo_po_color_size_breakdown c on a.id=c.job_id and b.id=c.po_break_down_id join wo_pre_cost_fabric_cost_dtls d on a.id=d.job_id and c.item_number_id= d.item_number_id and d.is_deleted=0 and d.status_active=1 join wo_pre_cos_fab_co_avg_con_dtls e on a.id=e.job_id and d.id=e.pre_cost_fabric_cost_dtls_id and c.po_break_down_id=e.po_break_down_id and c.color_number_id=e.color_number_id and c.size_number_id=e.gmts_sizes and e.is_deleted=0 and e.status_active=1 and e.cons !=0 left join wo_pre_cos_fab_co_color_dtls g on a.id=g.job_id and d.id=g.pre_cost_fabric_cost_dtls_id and g.is_deleted=0 and g.status_active=1 and c.color_number_id=g.gmts_color_id and e.color_number_id =g.gmts_color_id left join wo_pre_stripe_color h on a.id=h.job_id and c.item_number_id= h.item_number_id and d.id=h.pre_cost_fabric_cost_dtls_id and e.color_number_id =h.color_number_id and e.po_break_down_id=h.po_break_down_id and e.gmts_sizes=h.size_number_id 
				where a.id=b.job_id and b.id=c.po_break_down_id and c.po_break_down_id=e.po_break_down_id and c.color_number_id=e.color_number_id and c.size_number_id=e.gmts_sizes and e.pre_cost_fabric_cost_dtls_id=d.id and c.job_no_mst=d.job_no and c.item_number_id=d.item_number_id and a.is_deleted=0 and a.status_active=1  and b.id in($all_po_id)
				group by a.buyer_name,a.job_no, a.job_no_prefix_num,a.style_ref_no, c.item_number_id, d.body_part_id, d.lib_yarn_count_deter_id, c.color_number_id,d.uom,d.avg_finish_cons ,a.insert_date,b.grouping,d.gsm_weight, e.dia_width,d.color_size_sensitive,d.fabric_source,h.stripe_color,g.contrast_color_id , a.id, d.seq order by a.id, d.seq asc");	
            ...
            foreach ($sql_budget as $row)
			{
				...
				$budget_qty_array_info[$row[csf('buyer_name')]][$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('color_id')]][$row[csf('avg_cons')]]['contrast_color_id']=$row[csf('contrast_color_id')];
				$budget_qty_array_info[$row[csf('buyer_name')]][$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('color_id')]][$row[csf('avg_cons')]]['stripe_color']=$row[csf('stripe_color')];
				$budget_qty_array_info[$row[csf('buyer_name')]][$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('color_id')]][$row[csf('avg_cons')]]['gmts_color']=$row[csf('color_id')];
				//$booking_qty_array[$row[csf('buyer_name')]][$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('color_id')]][$row[csf('avg_cons')]]+=$row[csf('booking_qnty')];
			...
			//echo "<pre>";
			//print_r($budget_qty_array_info);

			$sql_booking_qnty= sql_select("SELECT a.job_no, a.job_no_prefix_num, a.buyer_name,a.style_ref_no, $year, LISTAGG(b.id,',') WITHIN GROUP (ORDER BY b.id) as  po_id,LISTAGG(b.po_number,',') WITHIN GROUP (ORDER BY b.po_number) as po_number, c.item_number_id as item_id, c.color_number_id as color_id, e.body_part_id, e.lib_yarn_count_deter_id, e.gsm_weight, d.dia_width, sum((d.cons/d.pcs)*c.plan_cut_qnty) as budget_qty, e.uom, e.avg_finish_cons AS avg_cons,f.fin_fab_qnty as booking_qnty,e.color_size_sensitive from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, wo_pre_cos_fab_co_avg_con_dtls d, wo_pre_cost_fabric_cost_dtls e,wo_booking_dtls f where a.id=b.job_id and b.id=c.po_break_down_id and c.po_break_down_id=d.po_break_down_id and c.color_number_id=d.color_number_id and c.size_number_id=d.gmts_sizes and d.pre_cost_fabric_cost_dtls_id=e.id and c.job_no_mst=e.job_no and c.item_number_id=e.item_number_id and a.is_deleted=0 and a.status_active=1  and e.job_no=f.job_no and d.pre_cost_fabric_cost_dtls_id=f.pre_cost_fabric_cost_dtls_id  and e.gsm_weight=f.gsm_weight and d.dia_width=f.dia_width and f.po_break_down_id=b.id  and f.pre_cost_fabric_cost_dtls_id=e.id and c.color_number_id=f.gmts_color_id  and f.is_deleted=0 and f.status_active=1   and b.id in($all_po_id) group by a.job_no, a.job_no_prefix_num, a.buyer_name,a.style_ref_no,a.insert_date, b.po_number, c.item_number_id, c.color_number_id, e.body_part_id, e.lib_yarn_count_deter_id, e.gsm_weight, d.dia_width, e.uom, e.avg_finish_cons,f.fin_fab_qnty,e.color_size_sensitive");

            foreach ($sql_booking_qnty as $row)
			{
				$booking_qty_array[$row[csf('buyer_name')]][$row[csf('job_no')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('color_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]]['booking_qnty']+=$row[csf('booking_qnty')];
			}
		}
		else
		{
	...
	else
	{
		...
		foreach ($sql_budget as $row)
		{
			if($data[2]==2)
			{
				...
				$booking_qty_array[$row[csf('buyer_name')]][$row[csf('po_id')]][$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('color_id')]][$row[csf('avg_cons')]]=$row[csf('booking_qnty')];
			}
			else
			{
				...
				$booking_qty_array[$row[csf('buyer_name')]][$row[csf('po_id')]][$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('color_id')]][$row[csf('size_id')]][$row[csf('avg_cons')]]=$row[csf('booking_qnty')];
			}
			$avg_cons_data[$row[csf('job_no')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]]=$row[csf('avg_cons')];
			$uom_data[$row[csf('job_no')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]]=$row[csf('uom')];
	...
	foreach($result as $row)
	{
		...
		if($data[1]==1)
		{
			...
			if($data[2]==2)
			{
				...
				$fabricColorId= $budget_qty_array_info[$row[csf('buyer_id')]][$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part')]][$row[csf('determination_id')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('color_id')]][$row['CONS']]['stripe_color'];

				$contrast_color_id=$budget_qty_array_info[$row[csf('buyer_id')]][$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part')]][$row[csf('determination_id')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('color_id')]][$row['CONS']]['contrast_color_id'];
				$color_number_id=$budget_qty_array_info[$row[csf('buyer_id')]][$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part')]][$row[csf('determination_id')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('color_id')]][$row['CONS']]['gmts_color'];
				
				$fabricColorId=$fabricColorId;
			
				if(!$fabricColorId){
					$fabricColorId=$contrast_color_id;
				}
				if(!$fabricColorId){
					$fabricColorId=$color_number_id;
				}

				//echo "==".$fabricColorId."==";

				//echo 'woven'.'='.'grey'.'='.$row[csf('job_no')].'='.$row[csf('color_id')].'='.$fabricColorId.'='.$row[csf('body_part')].'='.$row[csf('determination_id')].'='.$row[csf('gsm')].'='.$row[csf('dia')].'='.$row[csf('fabric_source')].'='.$row[csf('uom')].'<br/>';


				$budget_qty=$fabric_req_arr['woven']['grey'][$row[csf('job_no')]][$row[csf('color_id')]][$fabricColorId][$row[csf('body_part')]][$row[csf('determination_id')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('fabric_source')]][$row[csf('uom')]];
                
				
				$prevRcvQnty = $prevRcvQntyArr[$row[csf('buyer_id')]][$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part')]][$row[csf('determination_id')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('color_id')]][$row[csf('fabric_ref')]][$row[csf('rd_no')]][$row[csf('cutable_width')]][$row[csf('weight_type')]][$row[csf('cons')]];
			...
			foreach ($po_arr_uniq as $poID) 
			{
				...
				if($data[2]==2)
				{
					...
					$jobRequiredQnty=  $job_wise_budgt_qnt_arr[$row[csf('buyer_id')]][$row[csf('job_no')]][$style_ref_no][$row[csf('item_id')]][$row[csf('body_part')]][$row[csf('determination_id')]][$row[csf('color_id')]][$row[csf('gsm')]][$row[csf('dia')]]['qnty'];
					//echo $orderRequiredQnty."=";
				}
				else
				{
					$orderRequiredQnty=  $po_wise_budgt_qnt_arr[$row[csf('buyer_id')]][$row[csf('job_no')]][$style_ref_no][$row[csf('item_id')]][$row[csf('body_part')]][$row[csf('determination_id')]][$row[csf('color_id')]][$row[csf('size_id')]][$row[csf('gsm')]][$row[csf('dia')]]['qnty'];

					$jobRequiredQnty=  $job_wise_budgt_qnt_arr[$poID][$row[csf('buyer_id')]][$row[csf('job_no')]][$style_ref_no][$row[csf('item_id')]][$row[csf('body_part')]][$row[csf('determination_id')]][$row[csf('color_id')]][$row[csf('size_id')]][$row[csf('gsm')]][$row[csf('dia')]]['qnty'];
				}

				//$hdn_po_ratio_qnty.=$poID."##".($orderRequiredQnty/$budget_qty)*$row[csf('reqn_qty')].",";
				$hdn_po_ratio_qnty.=$poID."##".($orderRequiredQnty/$jobRequiredQnty)*$row[csf('reqn_qty')].",";
			...
			//=======================================================================
			//if($cutable_width==""){$cutable_widthx=0;}else{$cutable_widthx=$cutable_width;}
			$cutable_widthx=$row[csf('cutable_width')];
			
			

			//echo $row[csf('buyer_id')].'='.$row[csf('determination_id')].'='.$row[csf('body_part')].'='.$row[csf('gsm')].'='.$row[csf('dia')].'='.$row[csf('fabric_ref')].'='.$row[csf('rd_no')].'='.$cutable_widthx.'='.$row[csf('weight_type')].'='.$row[csf('fab_color_id')]."<br/>";

			$wvn_rcv_qnty=$wvn_recv_qnty_array[$row[csf('job_no')]][$row[csf('buyer_id')]][$row[csf('determination_id')]][$row[csf('body_part')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('fabric_ref')]][$row[csf('rd_no')]][$cutable_widthx][$row[csf('weight_type')]][$row[csf('fab_color_id')]]['wvn_recv_qty'];
			$wvn_issueRtn_qnty=$wvn_issue_rtn_qnty_array[$row[csf('determination_id')]][$row[csf('body_part')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('fabric_ref')]][$row[csf('rd_no')]][$cutable_widthx][$row[csf('weight_type')]][$row[csf('fab_color_id')]]['wvn_issue_rtn_qty'];
			$wvn_transfIn_qnty=$wvn_transIn_qnty_array[$row[csf('determination_id')]][$row[csf('body_part')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('fabric_ref')]][$row[csf('rd_no')]][$cutable_widthx][$row[csf('weight_type')]][$row[csf('fab_color_id')]]['wvn_transfer_in_qty'];

			$wvn_issue_qnty=$wvn_issue_qnty_array[$row[csf('job_no')]][$row[csf('determination_id')]][$row[csf('body_part')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('fabric_ref')]][$row[csf('rd_no')]][$cutable_widthx][$row[csf('weight_type')]][$row[csf('fab_color_id')]]['wvn_issue_qty'];
			$wvn_recvRtn_qnty=$wvn_recv_rtn_qnty_array[$row[csf('determination_id')]][$row[csf('body_part')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('fabric_ref')]][$row[csf('rd_no')]][$cutable_widthx][$row[csf('weight_type')]][$row[csf('fab_color_id')]]['wvn_rcv_rtn_qty'];
			$wvn_transfOut_qnty=$wvn_transOut_qnty_array[$row[csf('determination_id')]][$row[csf('body_part')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('fabric_ref')]][$row[csf('rd_no')]][$cutable_widthx][$row[csf('weight_type')]][$row[csf('fab_color_id')]]['wvn_transfer_out_qty'];
			$totalWovenRecv=($wvn_rcv_qnty+$wvn_issueRtn_qnty+$wvn_transfIn_qnty);
			$totalWovenStock=($wvn_rcv_qnty+$wvn_issueRtn_qnty+$wvn_transfIn_qnty)-($wvn_issue_qnty+$wvn_recvRtn_qnty+$wvn_transfOut_qnty);

			//$bookingQnty=$booking_qty_array[$row[csf('buyer_id')]][$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part')]][$row[csf('determination_id')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('fab_color_id')]][$row['CONS']];

			$bookingQnty=$booking_qty_array[$row[csf('buyer_id')]][$row[csf('job_no')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('color_id')]][$row[csf('body_part')]][$row[csf('determination_id')]]['booking_qnty'];
		...
		<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>"> 
            ...
            <td width="80" align="right"><? echo number_format($bookingQnty,2,'.',''); ?></td>
            <td width="80" align="right"><? echo number_format($totalWovenRecv,2,'.',''); ?></td>
            <td width="80" align="right"><? echo number_format($totalWovenStock,2,'.',''); ?></td>
	        	...
	        	<td align="left" valign="top" colspan="2">
	            	<table align="left" cellspacing="0" border="1" width="100%" class="rpt_table" rules="all">
	                    <tbody>
	                      ...
	                      $sql_qry="SELECT a.id,a.reqn_number,a.company_id,a.reqn_date,b.job_no,b.buyer_id,b.po_id,b.gsm,b.dia,b.body_part,b.color_id,b.size_id,b.determination_id,sum(b.reqn_qty) as reqn_qty,b.item_id, b.UOM, b.FABRIC_SOURCE, b.FAB_COLOR_ID
	                      from pro_fab_reqn_for_cutting_mst a,pro_fab_reqn_for_cutting_dtls b
	                      where  a.reqn_number='$data[1]' and a.id=b.mst_id and a.is_deleted=0 and a.status_active=1 group by a.id,a.reqn_number, a.company_id, a.reqn_date, b.job_no, b.buyer_id, b.po_id, b.gsm, b.dia, b.body_part, b.color_id, b.size_id, b.determination_id, b.item_id, b.UOM, b.FABRIC_SOURCE, b.FAB_COLOR_ID ";
						// echo $sql_qry;
							...
							foreach($result as $row)
							{
								if($hdn_variable_colorSize_status==2)
								{
									...
									$budget_qty=$fabric_req_arr['woven']['grey'][$row[csf('job_no')]][$row[csf('color_id')]][$row[csf('fab_color_id')]][$row[csf('body_part')]][$row[csf('determination_id')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('fabric_source')]][$row[csf('uom')]];
								}
								else
								{
	            	...
	            	<table align="left" cellspacing="0" border="1" width="100%" class="rpt_table" rules="all">
	                    <tbody>
	                      ...
	                      where  a.reqn_number='$data[1]' and a.id=b.mst_id and a.is_deleted=0 and a.status_active=1 group by a.id,a.reqn_number, a.company_id, a.reqn_date,
						       b.job_no, b.buyer_id, b.po_id, b.gsm, b.dia, b.body_part, b.color_id, b.size_id, b.determination_id, b.item_id , b.UOM, b.FABRIC_SOURCE, b.FAB_COLOR_ID";
						       //echo $sql_qry;
							$result=sql_select($sql_qry);
							...
							foreach($result as $row)
							{
								$data_array[$row[csf('buyer_id')]][$row[csf('job_no')]][$row[csf('po_id')]][$row[csf('item_id')]][$row[csf('body_part')]][$row[csf('determination_id')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('color_id')]][$row[csf('size_id')]] [$row[csf('fabric_source')]][$row[csf('uom')]] [$row[csf('fab_color_id')]] ['reqn_qty'] += $row[csf('reqn_qty')];
														...
														foreach ($gsm_data as $dia => $dia_data) 
														{
															foreach ($dia_data as $color => $color_data) 
															{
																...
																foreach ($color_data as $size => $row) 
																{
																	...
																	foreach($row as $fabSo => $fabric_source )
																	{
																		foreach( $fabric_source as $uomSo => $uom )
																		{
																			foreach ($uom as $fabcolor => $fab_color_id){

																				$fabric_source_count=$fabSo;
																				$uom_count=$uomSo;
																				$fab_color_id_count=$fabcolor;
																			}
																	...
																	{															
																		...
																		$budget_qty=$fabric_req_arr['woven']['grey'][$job][$color][$fab_color_id_count][$body][$deterId][$gsm][$dia][$fabric_source_count][$uom_count];
																	}
																	else
																	{
	        	...
	        	<td align="left" valign="top" colspan="2">
	            	<table align="left" cellspacing="0" border="1" width="100%" class="rpt_table" rules="all">
	                    <tbody>
	                      ...
	                      $sql_qry="SELECT a.id,a.reqn_number,a.company_id,a.reqn_date,b.job_no,b.buyer_id,b.gsm,b.dia,b.body_part,b.color_id,b.size_id,b.determination_id,sum(b.reqn_qty) as reqn_qty,b.item_id,b.fabric_ref,b.rd_no,b.weight_type,b.cutable_width , b.UOM, b.FABRIC_SOURCE, b.FAB_COLOR_ID
	                      from pro_fab_reqn_for_cutting_mst a,pro_fab_reqn_for_cutting_dtls b 
	                      where  a.reqn_number='$data[1]' and a.id=b.mst_id and a.is_deleted=0 and a.status_active=1 group by a.id,a.reqn_number, a.company_id, a.reqn_date,
						       b.job_no, b.buyer_id, b.gsm, b.dia, b.body_part, b.color_id, b.size_id, b.determination_id, b.item_id,b.fabric_ref,b.rd_no,b.weight_type,b.cutable_width , b.UOM, b.FABRIC_SOURCE, b.FAB_COLOR_ID";
						       //echo $sql_qry;
							$result=sql_select($sql_qry);
	            	...
	            	<table align="left" cellspacing="0" border="1" width="100%" class="rpt_table" rules="all">
	                    <tbody>
							...
							// print_r($avg_cons_data);die;
	                      	...
	                      	$sql_qry="SELECT a.id,a.reqn_number,a.company_id,a.reqn_date,a.lay_plan_id,b.job_no,b.buyer_id,b.gsm,b.dia,b.body_part,b.color_id,b.fab_color_id,b.size_id,b.determination_id,sum(b.reqn_qty) as reqn_qty,b.item_id,b.fabric_ref,b.rd_no,b.weight_type,b.cutable_width , b.UOM, b.FABRIC_SOURCE, b.FAB_COLOR_ID
	                      	from pro_fab_reqn_for_cutting_mst a,pro_fab_reqn_for_cutting_dtls b
	                      	where  a.reqn_number='$data[1]' and a.id=b.mst_id and a.is_deleted=0 and a.status_active=1 group by a.id,a.reqn_number, a.company_id, a.reqn_date,a.lay_plan_id,
						       b.job_no, b.buyer_id, b.gsm, b.dia, b.body_part, b.color_id,b.fab_color_id, b.size_id, b.determination_id, b.item_id,b.fabric_ref,b.rd_no,b.weight_type,b.cutable_width , b.UOM, b.FABRIC_SOURCE, b.FAB_COLOR_ID";
						    //echo $sql_qry;
							...
							$sql="SELECT b.job_no,b.buyer_id,b.gsm,b.dia,b.body_part,b.color_id,b.size_id,b.determination_id,sum(b.reqn_qty) as reqn_qty,b.item_id,b.fabric_ref,b.rd_no,b.weight_type,b.cutable_width , b.UOM, b.FABRIC_SOURCE, b.FAB_COLOR_ID
	                      from pro_fab_reqn_for_cutting_mst a,pro_fab_reqn_for_cutting_dtls b
	                      where a.reqn_number!='$data[1]' and a.id=b.mst_id and a.is_deleted=0 and a.status_active=1 $job_no_cond  
	                      group by b.job_no, b.buyer_id, b.gsm, b.dia, b.body_part, b.color_id, b.size_id, b.determination_id, b.item_id,b.fabric_ref,b.rd_no,b.weight_type,b.cutable_width , b.UOM, b.FABRIC_SOURCE, b.FAB_COLOR_ID";
						    // echo $sql;die();
