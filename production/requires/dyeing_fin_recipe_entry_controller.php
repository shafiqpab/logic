...
if ($action == "create_recipe_search_list_view") {
	...
	$sql = "select id, labdip_no,batch_id, recipe_description, recipe_date, order_source, style_or_order, buyer_id, color_id, color_range, pickup, surplus_solution from pro_recipe_entry_mst where entry_form=445 and status_active=1 and is_deleted=0 $lc_working_company_cond $labdip_cond $sysid_cond $rec_des_cond $date_cond $batch_cond order by id DESC";
...
if ($action == 'populate_data_from_search_popup') {
	...
	foreach ($data_array as $row) {
		...
		//echo "load_drop_down('requires/dyeing_fin_recipe_entry_controller', ".$row[csf('company_id')].", 'load_drop_down_location', 'location_td' );\n";
		//echo "load_drop_down('requires/dyeing_fin_recipe_entry_controller', ".$row[csf('company_id')].", 'load_drop_down_buyer', 'buyer_td_id' );\n";
		echo "load_drop_down('requires/dyeing_fin_recipe_entry_controller', '" . $row[csf("working_company_id")] . "', 'load_drop_down_location', 'location_td' );\n";
		...
		echo "get_php_form_data(" . $row[csf("working_company_id")] . "+'**'+" . $row[csf("batch_id")] . "+'**'+" . $order_source . ", 'load_data_from_batch', 'requires/dyeing_fin_recipe_entry_controller');\n";
		...
		echo "$('#btn_recipe_calc3').removeAttr('disabled','disabled');\n";
		echo "$('#btn_recipe_calc3').removeClass('formbutton_disabled').addClass('formbutton');\n";
			...
			<fieldset style="width:100%;">
				...
				<table cellpadding="0" cellspacing="0" width="750" class="rpt_table" border="1" rules="all">
					...
					<tr>
						...
						<td align="center">
							<input type="button" name="button2" class="formbutton" value="Show"
							onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_company_id').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+'<? echo $batch_against; ?>', 'create_booking_search_list_view', 'search_div', 'dyeing_fin_recipe_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1);')"
							style="width:100px;"/>
						</td>
					</tr>
				</table>
			...
			<form name="searchorderfrm_1" id="searchorderfrm_1" autocomplete="off">
				<table cellpadding="0" cellspacing="0" border="1" rules="all" width="500" class="rpt_table">
					...
					<tr class="general">
						...
						<td>
                        <input type="text" name="txt_date_from" id="txt_date_from" class="datepicker" style="width:50px;" tabindex="3" value="" />
                        </td>
                        <td>
                        <input type="text" name="txt_date_to" id="txt_date_to" class="datepicker" style="width:50px;" tabindex="4" value="" />
                        </td>
						<td align="center">
							<input type="button" name="button2" class="formbutton" value="Show"
							onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+'<? echo $cbo_company_id; ?>'+'_'+document.getElementById('cbo_string_search_type').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value, 'create_batch_search_list_view', 'search_div', 'dyeing_fin_recipe_entry_controller', 'setFilterGrid(\'list_view\',-1);')"
							style="width:100px;"/>
						</td>
					</tr>
				</table>
...
if ($action == "create_batch_search_list_view") {
	...
	if($search_common=="" && $date_from=="" && $date_to=="")
	{
		echo "<p style='color:firebrick; text-align: center; font-weight: bold;'>Batch No /Date  is required</p>";die;
	}
	...
	if ($action == "ratio_data_from_dtls") {
		...
		$update_id = $ex_data[2];
		if ($sub_process == 93 || $sub_process == 94 || $sub_process == 95 || $sub_process == 96 || $sub_process == 97 || $sub_process == 98) {
			$sql_rec_dtls = "select id,sub_process_id,process_remark,liquor_ratio,total_liquor,trough_water from pro_recipe_entry_dtls where mst_id=" . $update_id . " and sub_process_id=$sub_process ";
		} else {
			$sql_rec_dtls = "select id,sub_process_id,process_remark,liquor_ratio,total_liquor,trough_water from pro_recipe_entry_dtls where mst_id=" . $update_id . " and sub_process_id=$sub_process and ratio>0";
		}
		//echo $sql_rec_dtls;
		...
		//txt_total_liquor_ratio*txt_liquor_ratio_dtls
			...
			echo "document.getElementById('txt_trough_water').value 			= '" . $row[csf("trough_water")] . "';\n";
...
$sql_recipe="select recipe_date from pro_recipe_entry_mst where  entry_form=445 and  recipe_date>='$from_date' and status_active=1";
	//echo $sql_recipe;die;
	...
	//echo $sql;
		...
		//echo "load_drop_down( 'requires/dyeing_fin_recipe_entry_controller', '" . $row[csf("company_id")] . "', 'load_drop_down_location', 'location_td' );\n";
			...
			if($batch_cat!=4)
			{
				echo "get_php_form_data('" . $po_id . "'+'**'+'" . $prod_id . "'+'**'+'" . $booking_no . "'+'**'+'" . $booking_without_order . "'+'**'+'" . $ex_data[1] . "'+'**'+'" . $row[csf("company_id")] . "', 'lode_data_from_grey_production', 'requires/dyeing_fin_recipe_entry_controller');\n";
			}
			echo "caculate_tot_liquor();\n";
		}
		exit();
...
/*if ($action == "item_details")
{
	...
	if ($update_id != "")
	{
		...
		if ($sub_process_id == 93 || $sub_process_id == 94 || $sub_process_id == 95 || $sub_process_id == 96 || $sub_process_id == 97 || $sub_process_id == 98) {
			...
			$ration_cond = " and a.ratio>0 ";
		}

		$recipeData_info = sql_select("select a.id rec_id, a.prod_id, a.item_lot,a.comments,a.dose_base, a.ratio, a.seq_no,b.id, b.item_category_id, b.item_group_id, b.sub_group_name, b.item_description, b.item_size, b.unit_of_measure, b.current_stock from pro_recipe_entry_dtls a,product_details_master b where a.prod_id=b.id and a.mst_id=$update_id and a.sub_process_id=$sub_process_id and a.status_active=1 and a.is_deleted=0 $ration_cond order by a.seq_no");

		foreach ($recipeData_info as $row) {
			...
			$recipe_data_arr[$row[csf('prod_id')]][5] = $row[csf('rec_id')];
			$recipe_data_arr[$row[csf('prod_id')]][6] = $row[csf('comments')];

			$recipe_data_arr[$row[csf('prod_id')]][7] = $row[csf('item_category_id')];
			$recipe_data_arr[$row[csf('prod_id')]][8] = $row[csf('item_group_id')];
			$recipe_data_arr[$row[csf('prod_id')]][9] = $row[csf('sub_group_name')];
			$recipe_data_arr[$row[csf('prod_id')]][10] = $row[csf('item_description')];
			$recipe_data_arr[$row[csf('prod_id')]][11] = $row[csf('item_size')];
			$recipe_data_arr[$row[csf('prod_id')]][12] = $row[csf('unit_of_measure')];
			$recipe_data_arr[$row[csf('prod_id')]][13] = $row[csf('current_stock')];
	...
	$product_con = ($product_ids != "") ? " and a.id not in($product_ids)" : "";
	$sql = "select a.id, a.item_category_id, a.item_group_id, a.sub_group_name, a.item_description, a.item_size, a.unit_of_measure, a.current_stock from product_details_master a where a.company_id='$company_id' and a.item_category_id in(5,6,7) and a.status_active=1 and a.is_deleted=0 $product_con";
	$nameArray = sql_select($sql);
	foreach ($nameArray as $row)
	{
            ...
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1032" class="rpt_table"
				...
				if ($sub_process_id == 93 || $sub_process_id == 94 || $sub_process_id == 95 || $sub_process_id == 96 || $sub_process_id == 97 || $sub_process_id == 98) //Wash start...
				{
					...
					foreach ($recipe_prod_id_arr as $prodId) {
						...
						$item_category_id = $recipe_data_arr[$prodId][7];
						$item_group_id = $recipe_data_arr[$prodId][8];
						$sub_group_name = $recipe_data_arr[$prodId][9];
						$item_description = $recipe_data_arr[$prodId][10];
						$item_size = $recipe_data_arr[$prodId][11];
						$unit_of_measure = $recipe_data_arr[$prodId][12];
						$current_stock = $recipe_data_arr[$prodId][13];
				...
				} else                //Wash End....
				{
					...
					foreach ($product_data_arr as $prodId => $data) {
                        ...
                        <tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>">
                            <td width="30" align="center" id="sl_<? echo $i; ?>"><? echo $i; ?></td>
                            <td width="80" id="item_category_<? echo $i; ?>">
                               <? echo $item_category[$item_category_id]; ?></td>
                            <td width="100" id="item_group_id_<? echo $i; ?>">
                                <? echo $item_group_arr[$item_group_id]; ?></td>
                            <td width="70" id="sub_group_name_<? echo $i; ?>"><? echo $sub_group_name; ?></td>
                            <td width="130" id="item_description_<? echo $i; ?>"><? echo $item_description . " " . $item_size; ?></td>
                            <td width="80" id="item_lot_<? echo $i; ?>">
                                <input type="text" name="txt_item_lot[]"
                                       id="txt_item_lot_<? echo $i; ?>"
                                       class="text_boxes" style="width:68px"
                                       onDblClick="openmypage_itemLot(<? echo $i; ?>)"
                                       placeholder="Browse"
                            ...
                            <td width="40" align="center"
                                id="uom_<? echo $i; ?>"><? echo $unit_of_measurement[$unit_of_measure]; ?></td>
                            <td width="70" align="center" id="dose_base_<? echo $i; ?>">
								<? echo create_drop_down("cbo_dose_base_$i", 68, $dose_base, "", 1, "-Select Dose Base-", $selected_dose); ?>
                            </td>
                            <td width="50" align="center" id="ratio_<? echo $i; ?>">
                                <input type="text"
                                       name="txt_ratio[]"
                                       id="txt_ratio_<? echo $i; ?>"
                                       class="text_boxes_numeric"
                            ...
                            <td width="40" align="center" id="seqno_<? echo $i; ?>">
                                <input type="text"
                                       name="txt_seqno[]"
                                       id="txt_seqno_<? echo $i; ?>"
                                       class="text_boxes_numeric"
                            ...
                            <td width="100" id="sub_process_<? echo $i; ?>">
                                <? echo $dyeing_sub_process[$sub_process_id]; ?>
                                <input type="hidden"
                                       name="updateIdDtls[]"
                                       id="updateIdDtls_<? echo $i; ?>" value="<? echo $dtls_id; ?>">
                            </td>
                            <td width="50" align="center" id="product_id_<? echo $i; ?>"><? echo $prodId; ?><input
                                        type="hidden" name="txt_prod_id[]" id="txt_prod_id_<? echo $i; ?>"
                                        value="<? echo $prodId; ?>"></td>
                            <td align="right" width="70"
                                id="stock_qty_<? echo $i; ?>"><? echo number_format($current_stock, 2, '.', ''); ?></td>
                            <td width="" align="center" id="comments_<? echo $i; ?>">
                                <input type="text"
                                       name="txt_comments[]"
                                       id="txt_comments_<? echo $i; ?>"
                                       class="text_boxes"
...
//echo $stoch_check.'G';
	...
	where a.id=b.prod_id and c.prod_id=b.prod_id and c.use_for=3 and a.company_id='$company_id' and b.store_id=$store_id and a.item_category_id in(5,6,7) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $stock_check";
						...
						if(!in_array($prodId,$recipe_prod_id_arr))
						{
							...
							if($current_stock_check>0)
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>">
									...
									<td  width="100" id="sub_process_<? echo $i; ?>"><p><? echo $dyeing_sub_process[$sub_process_id]; ?></p><input type="hidden" name="updateIdDtls[]" id="updateIdDtls_<? echo $i; ?>" value="<? //echo $dtls_id; ?>"></td>
									<td width="50" align="center" id="product_id_<? echo $i; ?>"><? echo $prodId_ref[0]; ?><input type="hidden" name="txt_prod_id[]" id="txt_prod_id_<? echo $i; ?>" value="<? echo $prodId_ref[0]; ?>"></td>
									<td align="right" width="70" title="<? echo "After decimal 2 digit allowed,current stock=".$prodData[6]."store stock=".$store_stock;  ?>" id="stock_qty_<? echo $i; ?>"><? echo number_format($store_stock,3,'.',''); ?></td>
									<td width="" align="center" id="comments_<? echo $i; ?>"><input type="text" name="txt_comments[]" id="txt_comments_<? echo $i; ?>" class="text_boxes" style="width:80px" value="<? echo $comments; ?>"></td>
								</tr>
								<?
							//$max_seq_no[]=$selectResult[csf('seq_no')];
...
if ($action == "itemLot_popup")
{
	...
	echo create_list_view("list_view", "Item Lot", "200", "330", "250", 0, $sql, "js_set_value", "batch_lot", "", 1, "", 0, "batch_lot", "dyeing_fin_recipe_entry_controller", 'setFilterGrid("list_view",-1);', '0', '', 1);
	die;
}
...
if ($action == "save_update_delete") {
	...
	if ($operation == 0)  // Insert Here
	{
		...
		if(str_replace("'","",$txt_color)!="")
		{
			...
			{
				$color_id = return_id( str_replace("'","",$txt_color), $color_arr, "lib_color", "id,color_name","445");
	...
	if($stock_check==0 || $stock_check==2) $stock_check=0;else $stock_check=$stock_check;
		...
		{
			$recipe_serial_no =return_field_value("max(a.recipe_serial_no) as recipe_serial_no","pro_recipe_entry_mst a"," a.batch_id=" . $batch_id . " and a.entry_form=445 and a.status_active=1 and a.is_deleted=0", "recipe_serial_no");
		...
		if (str_replace("'", "", $copy_id) == 1 || str_replace("'", "", $copy_id) == 2) {
			...
			if($batch_process_ids!= "")
			{
				...
				if(!in_array("137", $batch_process_id_arr))
				{
					...
					$batch_id_count =return_field_value("count(a.batch_id) as batch_id_count","pro_recipe_entry_mst a"," a.batch_id=" . $batch_id . " and a.entry_form=445 and a.status_active=1 and a.is_deleted=0", "batch_id_count");
					...
					$recipe_nos=sql_select("select a.id as id from pro_recipe_entry_mst a,pro_recipe_entry_dtls b where a.id=b.mst_id and b.sub_process_id=$subprocess and a.batch_id=" . $batch_id . " and a.entry_form=445 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by  a.id");
					$recipe_no="";
		...
		if (str_replace("'", "", $update_id) == "") {
			...
			$field_array = "id, entry_form, labdip_no, company_id,working_company_id, location_id, recipe_description, batch_id, method, recipe_date, order_source, style_or_order, booking_id, color_id, buyer_id, color_range, booking_type, total_liquor, batch_ratio, liquor_ratio,batch_qty, remarks, inserted_by, insert_date,surplus_solution,copy_from,recipe_serial_no";
			//echo $txt_liquor;
			$data_array = "(" . $id . ",445," . $txt_labdip_no . "," . $cbo_company_id . "," . $cbo_working_company_id . "," . $cbo_location . "," . $txt_recipe_des . "," . $txt_batch_id . "," . $cbo_method . "," . $txt_recipe_date . "," . $cbo_order_source . "," . $txt_booking_order . "," . $txt_booking_id . ",'" . $color_id . "'," . $cbo_buyer_name . "," . $cbo_color_range . "," . $txt_booking_type . "," . $txt_liquor . "," . $txt_batch_ratio . "," . $txt_liquor_ratio . "," . $txt_batch_weight . "," . $txt_remarks . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "'," .$surpls_solution . "," .$txt_copy_from . ",'" .$recipe_serial_no . "')";
			...
			$field_array_update = "labdip_no*company_id*working_company_id*location_id*recipe_description*batch_id*method*recipe_date*order_source*style_or_order*color_id*buyer_id*color_range*booking_id*booking_type*total_liquor*batch_ratio*liquor_ratio*batch_qty*remarks*updated_by*update_date*surplus_solution";

			$data_array_update = $txt_labdip_no . "*" . $cbo_company_id . "*" . $cbo_working_company_id . "*" . $cbo_location . "*" . $txt_recipe_des . "*" . $txt_batch_id . "*" . $cbo_method . "*" . $txt_recipe_date . "*" . $cbo_order_source . "*" . $txt_booking_order . "*" . $color_id . "*" . $cbo_buyer_name . "*" . $cbo_color_range . "*" . $txt_booking_id . "*" . $txt_booking_type . "*" . $txt_liquor . "*" . $txt_batch_ratio . "*" . $txt_liquor_ratio . "*" . $txt_batch_weight . "*" . $txt_remarks . "*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'" . "*" . $surpls_solution;
		...
		if (str_replace("'", "", $copy_id) == 2) {
			if ($subprocess == 93 || $subprocess == 94 || $subprocess == 95 || $subprocess == 96 || $subprocess == 97 || $subprocess == 98) {
				...
				$field_array_dtls = "id,mst_id,sub_process_id,store_id,process_remark,comments,liquor_ratio,trough_water,total_liquor,ratio,seq_no,sub_seq,dose_base,inserted_by,insert_date";
				$dtls_id = return_next_id("id", "pro_recipe_entry_dtls", 1);
				$data_array_dtls = "(" . $dtls_id . "," . $recipe_update_id . "," . $cbo_sub_process . "," . $cbo_store_name . "," . $txt_subprocess_remarks . ",'" . $txt_comments_1 . "'," . $txt_liquor_ratio_dtls . "," . $txt_trough_water . "," . $txt_total_liquor_ratio . ",'" . $txt_ratio_1 . "','" . $txt_seqno_1 . "'," . $txt_subprocess_seq . ",'" . $cbo_dose_base_1 . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
				//echo "10**insert into pro_recipe_entry_dtls (".$field_array_dtls.") Values ".$data_array_dtls."";die;
			} else {

				$field_array_dtls = "id,mst_id,sub_process_id,store_id,process_remark,prod_id,item_lot,comments,liquor_ratio,trough_water,total_liquor,dose_base,ratio,seq_no,sub_seq,inserted_by,insert_date";
				$dtls_id = return_next_id("id", "pro_recipe_entry_dtls", 1);
				...
				for ($i = 1; $i <= $total_row; $i++) {
					...
					$data_array_dtls .= "(" . $dtls_id . "," . $recipe_update_id . "," . $cbo_sub_process . "," . $cbo_store_name . "," . $txt_subprocess_remarks . ",'" . str_replace("'", "", $$product_id) . "','" . str_replace("'", "", $$txt_item_lot) . "','" . str_replace("'", "", $$txt_comments) . "'," . $txt_liquor_ratio_dtls . "," . $txt_trough_water . "," . $txt_total_liquor_ratio . ",'" . str_replace("'", "", $$cbo_dose_base) . "','" . str_replace("'", "", $$txt_ratio) . "','" . str_replace("'", "", $$txt_seqno) . "','" . str_replace("'", "", $txt_subprocess_seq) . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
	...
	and c.item_category_id in(5,6,7) and c.status_active=1 and c.is_deleted=0 and b.status_active=1 and b.mst_id=$update_id_check  ";
		$result_stock = sql_select($sql_stock);
			...
			$field_array_dtls = "id,mst_id,sub_process_id,store_id,process_remark,prod_id,item_lot,comments,liquor_ratio,trough_water,total_liquor,dose_base,ratio,seq_no,sub_seq,inserted_by,insert_date";
			$dtls_id = return_next_id("id", "pro_recipe_entry_dtls", 1);
			$sql = "select id, sub_process_id, prod_id, item_lot,comments,process_remark, dose_base,liquor_ratio,trough_water,total_liquor,ratio,seq_no,sub_seq, store_id from pro_recipe_entry_dtls where mst_id=$update_id_check  and status_active=1  order by id";
			$nameArray = sql_select($sql);
			...
			foreach ($nameArray as $row) {
				...
				if ($i != 1) $data_array_dtls .= ",";
				$data_array_dtls .= "(" . $dtls_id . "," . $recipe_update_id . ",'" . $row[csf('sub_process_id')] . "','" . $row[csf('store_id')] . "','" . $process_remark . "','" . $row[csf('prod_id')] . "','" . $row[csf('item_lot')] . "','" . $row[csf('comments')] . "'," . $txt_liquor_ratio_dtls . "," . $txt_trough_water . "," . $txt_total_liquor_ratio . ",'" . $row[csf('dose_base')] . "','" . $row[csf('ratio')] . "','" . $row[csf('seq_no')] . "','" . $row[csf('sub_seq')] . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
				$dtls_id = $dtls_id + 1;
				$i++;
			}
			//}
	...
	//	txt_subprocess_seq
		...
		if (str_replace("'", "", $copy_id) == 1) {
			...
			if($batch_process_ids!= "")
			{
				...
				if(!in_array("137", $batch_process_id_arr))
				{
					$recipe_no = return_field_value("a.id", "pro_recipe_entry_mst a,pro_recipe_entry_dtls b", "a.id=b.mst_id and b.sub_process_id=$subprocess and a.batch_id=" . $batch_id . " and a.entry_form=445 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0", "id");
					if ($recipe_no != '') {
						echo "14**0**$recipe_no";
						disconnect($con);
						die;
		...
		if ($subprocess == 93 || $subprocess == 94 || $subprocess == 95 || $subprocess == 96 || $subprocess == 97 || $subprocess == 98) {
			...
			$field_array_dtls_update2 = "sub_process_id*process_remark*comments*liquor_ratio*trough_water*total_liquor*ratio*seq_no*sub_seq*dose_base*updated_by*update_date";
			//$dtls_id=return_next_id( "id","pro_recipe_entry_dtls", 1 ) ;
			//$data_array_dtls_update="$txt_item_lot)."'*";
			$data_array_dtls_update2 = $subprocess . "*" . $txt_subprocess_remarks . "*" . $txt_comments_1 . "*" . $txt_liquor_ratio_dtls . "*" . $txt_trough_water . "*" . $txt_total_liquor_ratio . "*" . $txt_ratio_1 . "*" . $txt_seqno_1 . "*" . $txt_subprocess_seq . "*" . $cbo_dose_base_1 . "*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'";
			$update_dtls_id = str_replace("'", "", $updateIdDtls_1);


		} else {
			$field_array_dtls = "id,mst_id,sub_process_id,store_id,process_remark,prod_id,item_lot,comments,liquor_ratio,trough_water,total_liquor,dose_base,ratio,seq_no,sub_seq,inserted_by,insert_date";
			$field_array_dtls_update = "prod_id*item_lot*comments*liquor_ratio*trough_water*total_liquor*dose_base*ratio*seq_no*sub_seq*sub_process_id*process_remark*updated_by*update_date";
			$dtls_id = return_next_id("id", "pro_recipe_entry_dtls", 1);
			...
			for ($i = 1; $i <= $total_row; $i++) {
				...
				if (str_replace("'", "", $$updateIdDtls) != "") {
					...
					$data_array_dtls_update[str_replace("'", '', $$updateIdDtls)] = explode("*", (str_replace("'", "", $$product_id) . "*'" . str_replace("'", "", $$txt_item_lot) . "'*'" . str_replace("'", "", $$txt_comments) . "'*" . $txt_liquor_ratio_dtls . "*" . $txt_trough_water . "*" . $txt_total_liquor_ratio . "*'" . str_replace("'", "", $$cbo_dose_base) . "'*'" . str_replace("'", "", $$txt_ratio) . "'*'" . str_replace("'", "", $$txt_seqno) . "'*" . $txt_subprocess_seq . "*" . $cbo_sub_process . "*" . $txt_subprocess_remarks . "*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'"));
				} else {
					if ($data_array_dtls != "") $data_array_dtls .= ",";

					$data_array_dtls .= "(" . $dtls_id . "," . $update_id . "," . $cbo_sub_process . "," . $cbo_store_name . "," . $txt_subprocess_remarks . ",'" . str_replace("'", "", $$product_id) . "','" . str_replace("'", "", $$txt_item_lot) . "','" . str_replace("'", "", $$txt_comments) . "'," . $txt_liquor_ratio_dtls . "," . $txt_trough_water . "," . $txt_total_liquor_ratio . ",'" . str_replace("'", "", $$cbo_dose_base) . "','" . str_replace("'", "", $$txt_ratio) . "','" . str_replace("'", "", $$txt_seqno) . "'," . $txt_subprocess_seq . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
	...
	else if ($operation==2)  // Delete Here----------------------------------------------------------
	{
		...
		$recipe_no = return_field_value("a.id as id", "pro_recipe_entry_mst a,pro_recipe_entry_dtls b", "a.id=b.mst_id and a.id=" . $update_id . " and a.entry_form!=445 and  b.status_active=1 and b.is_deleted=0", "id");
		if ($recipe_no != '')
		{
	...
	if ($action == "recipe_entry_print") {
		...
		$sql_recipe_mst = "select distinct a.id, a.labdip_no, a.company_id, a.working_company_id     as w_com_id, a.location_id, a.recipe_description, a.batch_id, a.method, a.recipe_date, a.order_source, a.style_or_order, a.booking_id, a.total_liquor, a.batch_ratio, b.liquor_ratio, a.remarks, a.buyer_id, a.color_id, a.color_range, a.remarks from pro_recipe_entry_mst a,  pro_recipe_entry_dtls b where a.id ='$data[1]' and a.id = b.mst_id";
		// echo $sql_recipe_mst; die;
			...
			<table width="930" cellspacing="0" align="right" border="0">
                ...
                <tr>
                	...
                	<td>:&nbsp;
                		...
                		<td>:&nbsp;<? echo $dataArray[0][csf('LIQUOR_RATIO')]; ?></td>
                	</tr>
                	<tr>
                ...
                <br>
                <div style="width:100%; margin-left: 200">
                	<table align="right" cellspacing="0" width="1130" border="1" rules="all" class="rpt_table">
                		<thead bgcolor="#dddddd" align="center">
                			...
                			<th rowspan="2" width="100">Ratio</th>
                ...
                foreach ($process_array as $process_id) {
                		...
                		if($process_id==93 || $process_id==94 || $process_id==95 || $process_id==96 || $process_id==97 || $process_id==98)
						{
							$id = $data[0];
								...
								if($dose_base_id==1 && $item_category_id==5)
								{
									...
									$amount = number_format($ratio*$batch_weight*$l_ratio_percent/1000,6, '.', '');
					...
					if($tot_mgm>=1000){
						...
						//echo $tot_mgm.'=AAAAAAAAAAAAAAAAADDDDDDDDDDDDDDDD';
                	}
...
if($action == "recipe_entry_print_2")
{
	...
	if ($action == "recipe_entry_print3")
	{
		...
		$party_library = return_library_array("select id, supplier_name from lib_supplier", "id", "supplier_name");
		$buyer_library = return_library_array("select id, buyer_name from lib_buyer", "id", "buyer_name");
		$item_sub_group_arr = return_library_array("select item_group_id,sub_group_name from lib_item_sub_group", 'item_group_id', 'sub_group_name');
		$color_arr = return_library_array("select id, color_name from lib_color", 'id', 'color_name');
		$group_library = return_library_array("select id, group_name from lib_group", "id", "group_name");
		...
		$batch_array = array();
		if ($db_type == 0) {
			$sql = "SELECT a.id, a.batch_no,a.batch_against, a.entry_form, a.total_trims_weight, group_concat(b.po_id) as po_id, group_concat(b.prod_id) as prod_id, sum(b.batch_qnty) as batch_qnty,b.is_sales from pro_batch_create_mst a,  pro_batch_create_dtls b where a.id=b.mst_id and a.company_id=$data[0] and a.id in($batch_id) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id, a.batch_no,a.batch_against, a.entry_form, a.total_trims_weight, b.is_sales order by a.id DESC";
		} else if ($db_type == 2) {

			$sql = "SELECT a.id, a.batch_no,a.batch_against,a.entry_form, a.total_trims_weight, listagg(b.po_id,',') within group (order by b.po_id) as po_id, listagg(b.prod_id,',') within group (order by b.prod_id) as prod_id,  sum(b.batch_qnty) as batch_qnty,b.is_sales  from pro_batch_create_mst a, pro_batch_create_dtls b where a.id=b.mst_id and a.company_id=$data[0] and a.id in($batch_id) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id,a.batch_against, a.batch_no,a.entry_form,a.total_trims_weight,b.is_sales order by a.id DESC";
		}
		//echo $sql;
		$result_sql = sql_select($sql);
		...
		foreach ($result_sql as $row)
		{
			...
			foreach ($order_id as $val) {
				...
				{
					$chkPo[$val]=$val;
					array_push($all_po_id_arr,$val);
				}
			}

			//echo "<pre>";print_r($all_po_id_arr);
			if(!empty($all_po_id_arr))
			{
				if($is_sales==1)
				{
					$sales_order_result = sql_select("select id,job_no,within_group,sales_booking_no from fabric_sales_order_mst where status_active=1 and is_deleted=0 ".where_con_using_array($all_po_id_arr,0,'id')."");
					$sales_arr = array();
					foreach ($sales_order_result as $sales_row) {
						$sales_arr[$sales_row[csf("id")]]["sales_order_no"] 	= $sales_row[csf("job_no")];
					}
				...
				{
					$po_arr = return_library_array("select id,po_number from wo_po_break_down where status_active=1 ".where_con_using_array($all_po_id_arr,0,'id')."", 'id', 'po_number');
					$sub_po_arr = return_library_array("select id,order_no from  subcon_ord_dtls where status_active=1 ".where_con_using_array($all_po_id_arr,0,'id')."", 'id', 'order_no');
				}
			}
		}
		...
		$group_id=$nameArray[0][csf('group_id')];
		?>
		<div style="width:950px; font-size:20px">
			<table width="950" cellspacing="0" align="right" border="0" style="font-size:20px">
				<tr>
					<td colspan="6" align="center" style="font-size:x-large">
				...
				<tr>
					<td width="130"><strong>Recipe No</strong></td>
					<td width="175">:&nbsp;<? echo $dataArray[0][csf('id')]; ?></td>
					<td width="160"><strong>Recipe Date</strong></td>
					<td width="175px">:&nbsp; <? echo change_date_format($dataArray[0][csf('recipe_date')]); ?></td>
					<td width="130"><strong>Order Source</strong></td>
					<td width="175">:&nbsp;<? echo $order_source[$dataArray[0][csf('order_source')]]; ?></td>
				</tr>
				<tr>
					<td><strong>Batch No</strong></td>
					<td>:&nbsp;<? echo $batch_array[$dataArray[0][csf('batch_id')]]['batch_no']; ?></td>
				...
				<tr>
					...
					<td colspan="2">:&nbsp;<? echo $batch_array[$dataArray[0][csf('batch_id')]]['order'];?></td>
                </tr>
				<tr>
					<td><strong>Remarks</strong></td>
					<td colspan="5">:&nbsp;<? echo $dataArray[0][csf('remarks')]; ?></td>
			...
			<br>
			<div style="width:100%; margin-left: 200">
                	<table align="right" cellspacing="0" width="1150" border="1" rules="all" class="rpt_table" style="font-size:20px">
                		<thead bgcolor="#dddddd" align="center">
							<tr>
                			<th width="30">SL</th>
                			...
                			<th width="50">UOM</th>
                			<th width="100">Ratio/ GPL</th>
							<th width="70">Machine Item Qty.</th>
							<th width="70">Item Qty</th>
							<th width="70">Total Item Qty</th>
                			<th width="">Comments</th>
                		...
                		$sub_process_data_array = array();
                		$sub_process_remark_array = array();
                		$sql_ratio = "select id, sub_process_id as sub_process_id,process_remark,total_liquor,liquor_ratio,trough_water from pro_recipe_entry_dtls where mst_id=$mst_id and status_active=1 and is_deleted=0 order by id";
                		$nameArray = sql_select($sql_ratio);
                		...
                		foreach ($nameArray as $row)
						{
                			...
                			$sub_process_remark_array[$row[csf("sub_process_id")]]['total_liquor'] = $row[csf("total_liquor")];
                			$sub_process_remark_array[$row[csf("sub_process_id")]]['liquor_ratio'] = $row[csf("liquor_ratio")];
                			$sub_process_remark_array[$row[csf("sub_process_id")]]['trough_water'] = $row[csf("trough_water")];
                		}
						...
						$grand_machine_item_qnty=$grand_tot_ratio=$grand_item_qnty=$grand_total_item_qnty=0;

						foreach ($process_array as $process_id)
						{
							$liquor_ratio = $sub_process_remark_array[$process_id]['total_liquor'];
							$trough_water = $sub_process_remark_array[$process_id]['trough_water'];
							$l_ratio = $sub_process_remark_array[$process_id]['liquor_ratio'];
							$liquor_ratio_process = $sub_process_remark_array[$process_id]['liquor_ratio'];
							$remark = $sub_process_remark_array[$process_id]['remark'];
							if ($remark == '') $remark = ''; else $remark .= $remark . ', ';
							...
							<tr bgcolor="#EEEFF0">
								<td colspan="13" align="left"><b>Sub Process
									Name:- <? echo $dyeing_sub_process[$process_id] . ', ' . $remark .' Total Liquor(ltr): '.$liquor_ratio.', '. 'Machine Trough  Water(Ltr): '.$trough_water; ?></b>
								</td>
							</tr>
							<?
							$tot_ratio =$tot_machine_item_qnty=$tot_item_qnty=$total_tot_item_qnty= 0;
							...
							foreach ($sub_process_data as $data) {
								...
								if($process_id==93 || $process_id==94 || $process_id==95 || $process_id==96 || $process_id==97 || $process_id==98)
								{
									$id = $data[0];
										...
										$machine_item_qnty = ($ratio*$trough_water)/1000;
										$item_qnty = ($ratio*$liquor_ratio)/1000;
										$total_item_qnty = $machine_item_qnty+$item_qnty;
										...
										<tr bgcolor="<? echo $bgcolor; ?>">
											<td><? echo $i; ?></td>
											<td align="center"><p><? echo $item_description; ?></p></td>
											<td align="center"><p><? echo $item_sub_group_arr[$item_group_id]; ?></p></td>
											<td align="center"><? echo $unit_of_measurement[$unit_of_measure]; ?>&nbsp;</td>
											<td align="right"><? echo number_format($ratio, 3, '.', ''); ?>&nbsp;</td>
											<td align="right"><b><? echo number_format($machine_item_qnty, 3, '.', ''); ?>&nbsp;</b></td>
											<td align="right"><b><? echo number_format($item_qnty, 3, '.', ''); ?>&nbsp;</b></td>
											<td align="right"><b><? echo number_format($total_item_qnty, 3, '.', ''); ?>&nbsp;</b></td>
											<td align="center"><? echo $comments; ?>&nbsp;</td>
										</tr>
										<?
										$tot_ratio += number_format($ratio, 3, '.', '');
										$grand_tot_ratio += number_format($ratio, 3, '.', '');
										$tot_machine_item_qnty += number_format($machine_item_qnty, 3, '.', '');
										$grand_machine_item_qnty += number_format($machine_item_qnty, 3, '.', '');
										$tot_item_qnty += number_format($item_qnty, 3, '.', '');
										$grand_item_qnty += number_format($item_qnty, 3, '.', '');
										$total_tot_item_qnty += number_format($total_item_qnty, 3, '.', '');
										$grand_total_item_qnty += number_format($total_item_qnty, 3, '.', '');
										$i++;
								}
								else
								{
									...
									if($current_stock_check>0)
									{
										...
										<tr bgcolor="<? echo $bgcolor; ?>">
											<td><? echo $i; ?></td>
											<td align="center"><p><? echo $item_description; ?></p></td>
											<td align="center" title="<? echo $item_group_id;?>"><p><? echo $item_sub_group_arr[$item_group_id]; ?></p></td>
											<td align="center"><? echo $unit_of_measurement[$unit_of_measure]; ?>&nbsp;</td>
											<td align="right"><? echo number_format($ratio, 3, '.', ''); ?>&nbsp;</td>
											<td align="right"><b><? echo number_format($machine_item_qnty, 3, '.', ''); ?>&nbsp;</b></td>
											<td align="right"><b><? echo number_format($item_qnty, 3, '.', ''); ?>&nbsp;</b></td>
							...
							?>
							<tr class="tbl_bottom">
								<td align="right" colspan="4"><strong>Sub Process Total</strong></td>
								<td align="right"><? echo number_format($tot_ratio, 3, '.', ''); ?>&nbsp;</td>
								<td align="right"><? echo number_format($tot_machine_item_qnty, 3, '.', ''); ?>&nbsp;</td>
								<td align="right"><? echo number_format($tot_item_qnty, 3, '.', ''); ?>&nbsp;</td>
								<td align="right"><? echo number_format($total_tot_item_qnty, 3, '.', ''); ?>&nbsp;</td>
								<td align="right">&nbsp;</td>
						...
						<tr class="tbl_bottom">
							...
							<td align="right"><? echo number_format($grand_tot_ratio, 3, '.', ''); ?>&nbsp;</td>
							<td align="right"><? echo number_format($grand_machine_item_qnty, 3, '.', ''); ?>&nbsp;</td>
							<td align="right"><? echo number_format($grand_item_qnty, 3, '.', ''); ?>&nbsp;</td>
							<td align="right"><? echo number_format($grand_total_item_qnty, 3, '.', ''); ?>&nbsp;</td>
							<td align="right">&nbsp;</td>
				...
				<br>
				<?
				echo signature_table(62, $com_id, "930px");
				?>
			</div>
		</div>
			<?
