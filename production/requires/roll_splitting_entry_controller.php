	...
	*/
	if ($operation==0)
	{
		...
		*/
		...
		for($j=1;$j<=$tot_row;$j++)
		{
			...
			*/
			...
			$data_array_roll.="(".$id_roll.",".$barcode_year.",".$barcode_suffix_no[2].",".$barcode_no.",".$hidden_mst_id.",".$id_dtls.",".$hidden_po_breakdown_id.",62,'".$$rollWgt."','".$$rollWgt."','".$roll_reject_qty."',".$hidden_rollId.",'".$rollNo."',".$hidden_table_id.",".$booking_without_order.",'".$booking_no."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$$qtyInPcs."',".$hidden_coller_cuff_size.",".$hidden_is_sales.")";
		...
		*/
		if($flag==1)
		{
			$field_array_roll="id,barcode_year,barcode_suffix_no,barcode_no,mst_id,dtls_id,po_breakdown_id,entry_form,qnty,qc_pass_qnty, reject_qnty,roll_id,roll_no,roll_split_from,booking_without_order,booking_no,inserted_by,insert_date,qc_pass_qnty_pcs,coller_cuff_size,is_sales";
...
{
	//$data=explode("_",$data);
	$sql=sql_select("SELECT a.id,a.barcode_no,a.roll_no,a.qc_pass_qnty,a.qc_pass_qnty_pcs, a.coller_cuff_size,b.id as dtls_id from  pro_roll_details a,  pro_grey_batch_dtls b where b.id=a.dtls_id  and a.roll_split_from=$data and  a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  order by a.id" );
	...
	foreach($sql as $row)
	{
		...
		<tr id="tr_<? echo $i;  ?>" align="center" valign="middle">
			...
			<td width="60" >
				<input type="text" name="collerCuffSize[]" id="collerCuffSize_<? echo $i;  ?>" style="width:50px" class="text_boxes" value="<? echo $row[csf('coller_cuff_size')] ;  ?>"  placeholder="Display" readonly/>
			</td>
			<td width="180" >
...
if($action=="load_barcode_mst_form")
{
	...
	foreach($data_array as $val)
	{
		...
		echo "document.getElementById('hidden_coller_cuff_size').value = '".($val[csf('coller_cuff_size')])."';\n";
...
if ($action == "auto_create_and_print_pdf_file")
{
	...
	$sql_yarn_info=sql_select("SELECT a.prod_id,b.brand,b.yarn_type,b.yarn_comp_type1st,b.yarn_comp_type2nd,b.yarn_comp_percent1st,b.yarn_comp_percent2nd,b.lot,b.yarn_count_id from pro_material_used_dtls a,product_details_master b, pro_roll_details c where a.prod_id=b.id and a.mst_id=c.mst_id and  c.barcode_no=".$data[2]." and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
	...
	$query = "SELECT a.id, a.roll_no, a.po_breakdown_id, a.barcode_no, a.qnty,a.reject_qnty, b.fabric_grade, a.qc_pass_qnty_pcs, a.coller_cuff_size from pro_roll_details a left join pro_qc_result_mst b on a.barcode_no=b.barcode_no where a.id in($data[0]) and a.entry_form=62";
	//a.id in($data[0])
	//echo $query;die;
	$res = sql_select($query);
	$split_data_arr = array();
