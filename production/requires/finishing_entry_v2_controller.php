...
if($action=="production_process_control")
{
    ...
    $control_and_preceding=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=50 and page_category_id=31 and company_name='$data'");
...
if ($action=="load_drop_down_location")
{
	...
	else if($data[1]==2) { $dropdown_name="cbo_iron_location"; $load_function="load_drop_down( 'requires/finishing_entry_v2_controller', this.value, 'load_drop_down_floor', 'floor_td' );"; }
...
{
 	echo create_drop_down( "cbo_floor", 130, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process in (11) order by floor_name","id,floor_name", 1, "--Select Floor--", $selected, "",0 );
	//7,8,9,
	exit();
}
...
if($action=="create_po_search_list_view")
{
	...
	if($data[6]==1)
	{
		...
		if (trim($data[9])!="") $int_ref_cond=" and b.grouping='$data[9]'  "; //else  $style_cond="";
	}
	else if($data[6]==4 || $data[6]==0)
	{
		...
		if (trim($data[9])!="") $int_ref_cond=" and b.grouping like '%$data[9]%'  "; //else  $style_cond="";
	}
	else if($data[6]==2)
	{
		if (str_replace("'","",$data[4])!="") $job_cond=" and a.job_no_prefix_num like '$data[4]%'  $year_cond"; //else  $job_cond="";
...
if ($action=="order_details")
{
	...
	$control_and_preceding=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=33 and page_category_id=337 and company_name='$company_id'");    
	$preceding_process = $control_and_preceding[0][csf("preceding_page_id")];
	// echo $preceding_process; die;
	$qty_source=0;
	if($preceding_process==338) $qty_source=7; //IRON ENTRY V2
	...
	// echo "<pre>"; print_r($preceding_qty_array); die;
	$dtlsDataArr=array(); 
	$mstDataArr=array();
	$sql_prod="SELECT a.id, a.delivery_mst_id, a.po_break_down_id, a.item_number_id, a.country_id, b.color_size_break_down_id, b.production_qnty, b.alter_qty, b.spot_qty, b.reject_qty, b.re_production_qty from  pro_garments_production_mst a, pro_garments_production_dtls b, pro_gmts_delivery_mst c where c.job_no='$jobno' and a.production_type=8 and a.id=b.mst_id and c.id=b.delivery_mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0";
	// echo $sql_prod;die;
...
if($action=="system_number_popup")
{
    ...
    <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            ...
            <tbody>
                <tr class="general">                    
                    <td><? echo create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select Company --",$company, ""); ?></td>
...
if($action=="create_system_search_list_view")
{
	...
	$order_no= $ex_data[6];
	$style_ref= $ex_data[7];
	$int_ref=  str_replace("'","",$ex_data[8]);
	
	if(str_replace("'","",$company)==0) { echo "Please select company First"; die;}
	$job 		= str_replace("'","",$job_no);
	$form_date2 = str_replace("'","",$from_date);
	$to_date2 	= str_replace("'","",$to_date);
	$order 		= str_replace("'","",$order_no);
	$style 		= str_replace("'","",$style_ref);
	$intRef 	= str_replace("'","",$int_ref);
	$iron		= str_replace("'","",$iron_no);
	 
	if ( (!$job && (!$form_date2 && !$to_date2) && !$order && !$style && !$intRef && !$iron) )
	{
        echo "<h1 style='color:red; font-size: 17px;text-align:center;margin-top:20px;'> ** Please select at least one ** </h1>" ;
        die();
    }
    if($db_type==2)
	{ 
	...
	if($int_ref == "") $int_ref_cond=""; else $int_ref_cond="and d.grouping='$int_ref'";
	...
	$sql_pop="SELECT a.id, a.sys_number_prefix_num, a.sys_number, a.company_id, a.location_id, a.production_source, a.working_company_id, a.working_location_id, a.floor_id, a.job_id, a.job_no, a.delivery_date, a.sending_company, a.sending_location, a.challan_no, a.remarks, $production_hour as production_hour, b.wo_order_id, b.table_no ,c.style_ref_no, c.buyer_name, $year,d.grouping
    FROM pro_gmts_delivery_mst a, pro_garments_production_mst b, wo_po_details_master c, wo_po_break_down d
    where a.entry_form=714 and b.entry_form=714 and a.id=b.delivery_mst_id and b.po_break_down_id=d.id and c.job_no=d.job_no_mst $conpany_cond $job_cond $sql_cond $order_cond $system_cond $int_ref_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
	group by a.id, a.sys_number_prefix_num, a.sys_number, a.company_id, a.location_id, a.production_source, a.working_company_id, a.working_location_id, a.floor_id, a.job_id, a.job_no, a.delivery_date, a.sending_company, a.sending_location, a.challan_no, a.remarks, b.production_hour, b.wo_order_id, b.table_no, c.style_ref_no, c.buyer_name, a.insert_date,d.grouping order by a.id DESC";
	// echo $sql_pop; die;
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if (str_replace("'","",$txt_update_id)=="")
		{
			...
			$new_sys_number = explode("*", return_next_id_by_sequence("", "pro_gmts_delivery_mst",$con,1,$cbo_company_name,'GFNH',714,date("Y",time()),0,0,7,0,0 ));
			...
			$data_array_delivery="(".$mst_id.",'".$new_sys_number[1]."','".(int)$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",8,".$cbo_location.",".$cbo_source.",".$cbo_iron_company.",".$cbo_iron_location.",".$cbo_floor.",".$txt_job_id.",".$txt_job_no.",".$txt_iron_date.",'".$sending_company."','".$sending_location."',".$txt_challan.",".$txt_remark.",714,".$user_id.",'".$pc_date_time."',1,0)";
		}
		else
		{
		...
		$field_array_mst="id, delivery_mst_id, garments_nature, company_id, location, production_source, sending_location, sending_company, floor_id, production_date, production_hour, challan_no, remarks, po_break_down_id, item_number_id, country_id, serving_company, production_quantity, alter_qnty, spot_qnty, reject_qnty, production_type, entry_break_down_type, entry_form, wo_order_id, wo_order_no,inserted_by, insert_date, status_active, is_deleted";
		$data_array_mst="";
		...
		foreach($mstArr as $orderId=>$orderData)
		{
			foreach($orderData as $gmtsItemId=>$gmtsItemIdData)
			{
				foreach($gmtsItemIdData as $countryId=>$qtyData)
				{
					...
					if($db_type==2)
					{
						$data_array_mst.=" INTO pro_garments_production_mst (".$field_array_mst.") VALUES(".$id.",".$mst_id.",".$garments_nature.",".$cbo_company_name.",".$cbo_location.",".$cbo_source.",'".$sending_company."','".$sending_location."',".$cbo_floor.",".$txt_iron_date.",".$txt_reporting_hour.",".$txt_challan.",".$txt_remark.",".$orderId.", ".$gmtsItemId.",".$countryId.",".$cbo_iron_company.",".$qtyData['qc'].",".$qtyData['alt'].",".$qtyData['spt'].",".$qtyData['rej'].",8,3,714,".$txt_wo_id.",".$txt_wo_no.",".$user_id.",'".$pc_date_time."',1,0)";
					}
					else
					{
		...
		$field_array_dtls ="id, delivery_mst_id, mst_id, production_type, color_size_break_down_id, production_qnty, alter_qty, spot_qty, reject_qty, entry_form, status_active, is_deleted";
		...
		foreach($dtlsArr as $colorSizeid=>$qtyStr)
		{
			...
			$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",8,'".$colorSizeid."','".$qtyStr['qc']."','".$qtyStr['alt']."','".$qtyStr['spt']."','".$qtyStr['rej']."',714,1,0)"; 
		}
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$field_array_up="location*production_source*sending_location*sending_company*floor_id*production_date*production_hour*challan_no*remarks*po_break_down_id*item_number_id*country_id*serving_company*production_quantity*alter_qnty*spot_qnty*reject_qnty*wo_order_id*wo_order_no*updated_by*update_date";
			...
			if($orderId)
			{
				foreach ($orderData as $gmtsItemId => $gmtsItemIdData)
				{
					foreach ($gmtsItemIdData as $countryId =>$qtyData) 
					{
						...
						{
							$data_array_up[$gmtProdId] =explode("*",("".$cbo_location."*".$cbo_source."*'".$sending_company."'*'".$sending_location."'*".$cbo_floor."*".$txt_iron_date."*".$txt_reporting_hour."*".$txt_challan."*".$txt_remark."*".$orderId."*".$gmtsItemId."*".$countryId."*".$cbo_iron_company."*'".$qtyData['qc']."'*'".$qtyData['alt']."'*'".$qtyData['spt']."'*'".$qtyData['rej']."'*".$txt_wo_id."*".$txt_wo_no."*'".$user_id."'*'".$pc_date_time."'"));
							$id=$gmtProdId;
							$is_up=1;
							$id_arr[]=$gmtProdId;
						}
		...
		$dtlsrDelete = execute_query("update pro_garments_production_dtls set status_active=0, is_deleted=1 WHERE delivery_mst_id=$mst_id and production_type=8 and status_active=1 and is_deleted=0");
