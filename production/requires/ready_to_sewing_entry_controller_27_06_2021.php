...
if($action=="order_details_list")
{
	...
	$prod_data_array=sql_select("select a.po_break_down_id, a.production_type, a.embel_name, a.country_id, c.item_number_id, c.color_number_id, b.production_qnty
	...
	$ready_to_sewing=sql_select("select id as dtls_id, mst_id, po_break_down_id, country_id, garments_itme_id, color_id, sewing_qnty from ready_to_sewing_dtls where status_active=1 and is_deleted=0 and mst_id=$data[1]");
	$update_data=array();
...
if ($action=="save_update_delete")
{
	...
	{ 
		...
		$field_array_dtls="id, mst_id, po_break_down_id, country_id, garments_itme_id, color_id, sewing_qnty, status_active, is_deleted, inserted_by, insert_date";
		...
		for($j=1;$j<=$tot_row;$j++)
		{
			...
			$data_array_dtls.="(".$dtls_id.",".$id.",".$txt_order_id.",'".$$hiddcountryid."','".$$gramentitem."','".$$colorid."','".$$sewingissu."',1,0,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			$dtls_id = $dtls_id+1;
		}
		//echo "10**";
	...
	{ 
		...
		$field_array_dtls="id, mst_id, po_break_down_id, country_id, garments_itme_id, color_id, sewing_qnty, inserted_by, insert_date";
		$dtls_id = return_next_id( "id", "ready_to_sewing_dtls", 1 );
		...
		for($j=1;$j<=$tot_row;$j++)
		{
			...
			{
				if($data_array_dtls!="") $data_array_dtls.=",";
				$data_array_dtls.="(".$dtls_id.",".$id.",".$txt_order_id.",'".$$hiddcountryid."','".$$gramentitem."','".$$colorid."','".$$sewingissu."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				$dtls_id = $dtls_id+1;	
			}
		}
