...
if($action=="create_po_search_list_view")
{
	...
	SUM (CASE WHEN production_type = $qty_source THEN production_quantity ELSE 0 END) AS iron_qty, 
	pack_type from pro_garments_production_mst where status_active=1 and is_deleted=0 and production_type IN ($qty_source, 8) $po_cond group by po_break_down_id, item_number_id, country_id, pack_type");
							...
							?>
								<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value(<? echo $row[csf("id")];?>,'<? echo $grmts_item;?>','<? echo $po_qnty;?>','<? echo $plan_cut_qnty;?>','<? echo $country_id;?>','<? echo $pack_type;?>');" > 
								   ...
								   <td width="80" align="right">
								   	...
								   	else{$balance=$total_entry_qty_data_arr[$row[csf('id')]][$grmts_item][$country_id][$pack_type]['iron_qty']-$total_in_qty;}
...
if($action=="color_and_size_level")
{
	...
	$pack_type=$dataArr[8];
	$qty_source=0;
	if($dataArr[7]==28) $qty_source=4; //Sewing Input
	else if($dataArr[7]==29) $qty_source=5;//Sewing Output
	else if($dataArr[7]==30) $qty_source=7;//Iron Output
	else if($dataArr[7]==31) $qty_source=8;//Packing And Finishing
	else if($dataArr[7]==32) $qty_source=7;//Iron Output
	else if($dataArr[7]==91) $qty_source=7;//Iron Output
	else if($dataArr[7]==103) $qty_source=11;//Poly Entry
	else if($dataArr[7]==250) $qty_source=80;//woven fin. entry
	else if($dataArr[7]==256) $qty_source=15;//Hang Tag Entry
	...
	if($qty_source !=0)
	{
		...
		else if( $variableSettings==3 ) //color and size level
		{
			...
			$sql = "SELECT item_number_id, size_number_id, color_number_id, sum(order_quantity) as order_quantity, sum(plan_cut_qnty) as plan_cut_qnty from wo_po_color_size_breakdown
			where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' $pack_type_cond and is_deleted=0 and status_active=1 group by item_number_id, size_number_id, color_number_id,size_order order by color_number_id, size_order";
		}
	...
	{  
		...
		{				
			...
			where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' and status_active=1 and is_deleted=0  order by color_number_id,size_order"; //color_number_id, id
	...
	<div style="width:100%;">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="100%" class="rpt_table">
            <thead>
                ...
                <th width="" align="center">Floor</th>
            </thead>
		</table>
	</div>
		...
		<?php  
 		...
 		?>
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data('<? echo $selectResult[csf('id')]."_".$job_no."_".$material_cost_id; ?>','populate_input_form_data','requires/finishing_entry_controller');" > 
                ...
                <td width="110" align="center"><?php echo $selectResult[csf('production_hour')]; ?></td>
				<?php
                ...
                <td width="" align="center"><? echo $floor_name; ?></td>
			</tr>
			<?php
			$i++;
			}
	...
	<div style="width:100%;">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="100%" class="rpt_table">
            <thead>
                ...
                <th width="80" align="center">Sys. Challan</th>
            </thead>
		</table>
	</div>
	<div style="width:100%;max-height:180px; overflow:y-scroll" id="iron_list_view" align="left">
		...
		<?php  
 		...
 		?>
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data('<? echo $selectResult[csf('id')]."_".$job_no."_".$material_cost_id; ?>','populate_input_form_data','requires/finishing_entry_controller');" > 
                ...
                <td width="80" align="center"><? echo $selectResult[csf('id')]; ?></td>
			</tr>
			<?php
			$i++;
			}
...
if($action=="populate_input_form_data")
{
	...
	{ 
		...
		else
		{
		    ...
		    $dataSql_plan_cut="SELECT  sum(plan_cut_qnty) as totalinput from wo_po_color_size_breakdown  WHERE po_break_down_id=".$result[csf('po_break_down_id')]."  and item_number_id=".$result[csf('item_number_id')]." and country_id=".$result[csf('country_id')]."  and status_active=1 and is_deleted=0";
		...
		{ 
			...
			else if( $variableSettings==3 ) //color and size level
			{
				...
				if($qty_source)
				{
					$dtlsData = "SELECT c.color_number_id,c.size_number_id,sum(CASE WHEN a.production_type='$qty_source' then a.production_qnty ELSE 0 END) as production_qnty,sum(CASE WHEN a.production_type=8 then a.production_qnty ELSE 0 END) as cur_production_qnty,sum(CASE WHEN a.production_type=8 and b.id=$data[0] then a.reject_qty ELSE 0 END) as reject_qty  from pro_garments_production_dtls a, pro_garments_production_mst b,wo_po_color_size_breakdown c where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' $pack_cond and a.color_size_break_down_id!=0 and a.production_type in($qty_source,8) and c.id=a.color_size_break_down_id group by c.color_number_id,c.size_number_id";
					 

				}
				else
				{
					 $dtlsData = "SELECT c.color_number_id,c.size_number_id,sum(CASE WHEN a.production_type=8 then b.production_qnty ELSE 0 END) as cur_production_qnty,sum(CASE WHEN a.production_type=8 and a.id='$data[0]' then b.reject_qty ELSE 0 END) as reject_qty from pro_garments_production_mst a,pro_garments_production_dtls b,wo_po_color_size_breakdown c where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.production_type =8 and a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' $pack_cond and c.id=b.color_size_break_down_id group by c.color_number_id,c.size_number_id ";

					$dtlsData_colsize =sql_select("SELECT id as color_size_break_down_id,sum( plan_cut_qnty) as production_qnty  from wo_po_color_size_breakdown  where status_active=1 and is_deleted=0  and po_break_down_id='$po_id' and item_number_id='$item_id' and  country_id='$country_id'   group by id");
				...
				{				  
					...
					else
					{
						...
						{
							$color_size_qnty_array[$rows[csf('color_number_id')]][$rows[csf('size_number_id')]]['iss']= $rows[csf('production_qnty')];
						}
					}
				...
				$sql = "SELECT item_number_id, size_number_id, color_number_id, sum(order_quantity) as order_quantity, sum(plan_cut_qnty) as plan_cut_qnty from wo_po_color_size_breakdown where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' $pack_typeCond and is_deleted=0 and status_active=1 group by item_number_id, size_number_id, color_number_id order by color_number_id,size_order";
			}
			else // by default color and size level
			{
				$dtlsData = sql_select("select a.color_size_break_down_id,sum(CASE WHEN a.production_type='$qty_source' then a.production_qnty ELSE 0 END) as production_qnty,sum(CASE WHEN a.production_type=8 then a.production_qnty ELSE 0 END) as cur_production_qnty,sum(CASE WHEN a.production_type=8 then a.reject_qty ELSE 0 END) as reject_qty from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in($qty_source,8) group by a.color_size_break_down_id");
...
if ($action=="save_update_delete")
{
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		if(str_replace("'","",$sewing_production_variable)!=1 && str_replace("'","",$txt_mst_id)!='' ) //  not gross level
		{
			...
			if(str_replace("'","",$sewing_production_variable)==3)//color and size wise
			{		
				$color_sizeID_arr=sql_select( "select id,size_number_id,color_number_id from wo_po_color_size_breakdown where po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name and country_id=$cbo_country_name $packType_cond and status_active=1 and is_deleted=0 order by size_number_id,color_number_id" );
				$colSizeID_arr=array(); 
				foreach($color_sizeID_arr as $val){
					$index = $val[csf("size_number_id")].$color_arr[$val[csf("color_number_id")]].$val[csf("color_number_id")];
					$colSizeID_arr[$index]=$val[csf("id")];
