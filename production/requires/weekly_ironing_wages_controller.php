        ...
        <legend>Search Panel</legend>
            <table cellpadding="0" cellspacing="0" width="700" border="1" rules="all" class="rpt_table">
                ...
                <tr>
                    ...
                    <td>
						<input type="text" style="width:100px;" class="text_boxes"  name="tex_order" id="tex_order" />
                    </td>
                    <td id="location_td">
						<?
...
<script type="text/javascript">
	load_drop_down('weekly_ironing_wages_controller',<? echo $cbo_company_id;?>, 'load_drop_down_location', 'location_td' );
</script>
</html>
<?
exit();
...
{
	list($company_id,$sys_number,$location,$from_date,$to_date,$order_no)=explode("_",$data);	
	...
	if($order_no!="")$order_con=" and d.po_number like('%$order_no%')"; else $order_con="";	
	...
	$sql = "select a.id,a.sys_number,a.company_id,a.location,a.bill_for,a.final_bill,a.week_from_date,a.week_to_date,b.emp_name,d.po_number from pro_weekly_wages_bill_mst a, pro_weekly_wages_bill_dtls b,pro_weekly_wages_order_brk c,wo_po_break_down d where a.id=b.mst_id and a.id=c.weekly_wages_mst_id and d.id=c.order_id and a.company_id=$company_id and $location and $sys_number and a.bill_for=35 $date_con_from $order_con $date_con_to and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0  group by a.id,a.sys_number,a.company_id,a.location,a.bill_for,a.final_bill,b.emp_name,a.week_from_date,a.week_to_date,d.po_number order by a.id"; 
			...
			?>
                <tr id="tr_<? echo $row[csf('id')]; ?>" bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"  onClick="js_set_value(<? echo $row[csf('id')]; ?>,'<? echo $row[csf('sys_number')]; ?>')" > 
                   ...
                   <td width="150"><p><? echo implode(',',array_unique(explode(',',$row[csf('emp_name')]))); ?></p></td>
                    <td width="100"><p><? echo $location_details[$row[csf('location')]]; ?></p></td>
        ...
        <legend>Search Panel</legend>
            <table cellpadding="0" cellspacing="0" width="100%" border="1" rules="all" class="rpt_table">
                ...
                <tr>
                    ...
                    <td align="center">
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( <? echo $cbo_company_id; ?>+'_'+document.getElementById('cbo_category').value, 'enployee_info_list_view', 'search_div', 'weekly_ironing_wages_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                    </td>
                </tr>
            </table>
...
where a.id=b.mst_id and b.id=c.weekly_wages_dtls_id and a.location='$location' and
  a.company_id='$company' and a.bill_for=35 and a.week_from_date >= '$from_date' and  a.week_to_date <='$to_date' $order_id_con $gmt_item_name_con and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0";
	...
	$result = sql_select($sql);
		...
		$sql = "select b.color_type,b.item_id,b.avg_rate,b.style_ref,b.order_id,b.wo_qty,b.uom,b.amount from piece_rate_wo_mst a,piece_rate_wo_dtls b,wo_po_details_master c,pro_garments_production_mst d where a.id=b.mst_id and b.job_id=c.id and d.po_break_down_id=b.order_id  and d.production_type=7 and a.company_id='$company' and a.rate_for='$rate_for' and b.style_ref like '%$style%' and b.buyer_id like '%$buyer_2%' and c.job_no like '%$job_no%' and d.production_date BETWEEN '$from_date' and '$to_date' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0";//and d.location='$location' and a.service_provider_id='$supplier_id'
	$result = sql_select($sql);
		foreach ($result as $row)
		{ 
		 $key=$row[csf('order_id')].str_replace("  "," ",$row[csf('style_ref')]).$row[csf('item_id')];
	...
	<div style="width:915px; max-height:220px; overflow-y:scroll" id="list_container_batch" align="left">	 
        ...
        <?
            ...
            {  
				...
				if($bill_qty_arr[$key]==''){
				$key2=$row[csf('id')].str_replace("  "," ",$row[csf('style_ref_no')]).$row[csf('item_number_id')];
				$bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF";	
				if(in_array($row[csf('id')],explode(',',$order_id)) && in_array($row[csf('item_number_id')],explode(',',$gmt_item_name))){$bgcolor="#F9DA42";} 
			?>
                <tr id="tr_<? echo $key2; ?>" bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"  onClick="js_set_value(<? echo $row[csf('id')]; ?>,'<? echo $wo_rate_arr[$key2]; ?>','<? echo $rate_variable_arr[$key2]; ?>','<? echo str_replace("  "," ",$row[csf('style_ref_no')]); ?>','<? echo $row[csf('item_number_id')]; ?>','<? echo $row[csf('job_id')]; ?>','<? echo $bgcolor; ?>')" > 
                    ...
                    <td width="80" align="right"><p><? echo round($wo_amount_arr[$key2]); ?></p></td>
...
if($action=="show_weekly_wages_bill_listview")
{
	...
	//$sql = "select a.id,b.emp_id,b.emp_name,b.designation,b.order_id,b.buyer_id,b.apv_order_qty,b.cutting_bill_qty,b.previous_bill_qty,b.yet_to_bill_qty from pro_weekly_wages_bill_mst a,pro_weekly_wages_bill_dtls b where a.id='$data' and a.id=b.mst_id";
	
	$sql = "select b.id,b.mst_id,b.emp_id,b.emp_name,b.designation,b.style_ref,b.gmt_item,b.order_id,b.buyer_id,b.apv_order_qty,b.cutting_bill_qty,b.previous_bill_qty,b.yet_to_bill_qty from pro_weekly_wages_bill_dtls b where b.mst_id=$data and b.status_active=1 and b.is_deleted=0 order by b.id";
	$result = sql_select($sql);
	?>
	...
	<div style="width:850px; max-height:220px; overflow-y:scroll" id="list_container_batch" align="left">	 
        ...
        <?
			...
			?>
                <tr id="tr_<? echo $row[csf('id')]; ?>" bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"  onClick="get_php_form_data('<? echo $row[csf('mst_id')].'_'.$row[csf('id')]; ?>', 'populate_ironing_wages_dtls_form_data', 'requires/weekly_ironing_wages_controller')" > 
                    <td width="50" align="center"><? echo $i; ?></td>
                    <td width="60" align="center"><p><? echo $row[csf('emp_id')]; ?></p></td>
                    <td width="120"><p><? echo $row[csf('emp_name')]; ?></p></td>
                    <td width="110"><p><? echo $order_id; ?></p></td>
...
if($action=='populate_ironing_wages_mst_form_data')
{
...
if($action=='populate_ironing_wages_dtls_form_data')
{
...
group by 
	...
	foreach($result as $row){
		...
		echo "document.getElementById('txt_gmt_item_name').value	= '".$row[csf("gmt_item")]."';\n";
		echo "document.getElementById('txt_gmt_item_id').value	= '".$row[csf("gmt_item_id")]."';\n";
...
{ 
	list($po_id,$job_id,$rate,$rate_var,$style,$items,$company,$location,$bill_for,$final_bill,$from_date,$to_date,$supplier_id,$dtls_update_id)=explode("**",$data);
if($dtls_update_id)$dtls_update_id_con="and b.id not in($dtls_update_id)"; else $dtls_update_id_con=""; 		
	...
	$seftyParcentArray= sql_select("select iron_sefty_parcent from variable_settings_production where company_name='$company' and variable_list=30");
		
	$total_order=count(explode(',',$po_id));
	$deducted_parcent=$seftyParcentArray[0][csf("iron_sefty_parcent")]; //$deducted_parcent assicen 5 for deduct cut qty;
	...
	{  
		...
		else
		{
			...
			if(($production_quantity+$total_previous_bill_qty[$key]+$total_previous_deducted_qty[$key]) > $row[csf("wo_qty")]){
				...
				$excise_qty .=($production_quantity+$total_previous_bill_qty[$key]+$total_previous_deducted_qty[$key])- $row[csf("wo_qty")].'\n';
				//-----------------------------------------------
				$capacity_cutting=$row[csf("wo_qty")]-$total_previous_bill_qty[$key];
				$cut_qty=$capacity_cutting;
				$amount=$cut_qty*$row[csf("avg_rate")];
...
// order level data info------------------------------
	...
	$order_by_previous_bill_qty=return_field_value("sum(c.bill_qty)","pro_weekly_wages_bill_mst a, pro_weekly_wages_bill_dtls b, pro_weekly_wages_order_brk c","a.id=b.mst_id and b.id=c.weekly_wages_dtls_id and a.id=c.weekly_wages_mst_id and c.order_id ='".$row[csf("order_id")]."' and a.company_id='$company' and a.bill_for='$bill_for' and a.location='$location' and c.gmt_item_id='".$row[csf("item_id")]."' $dtls_update_id_con and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and c.status_active=1 and c.is_deleted=0 group by c.bill_qty");
...
// master part--------------------------------------------------------------;
			$weekly_wages_bill_system_id=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '', 'WIW', date("Y",time()), 5, "select sys_number_prefix, sys_number_prefix_num from pro_weekly_wages_bill_mst where company_id=$cbo_company_id AND bill_for=35 and $year_cond=".date('Y',time())." order by id desc", "sys_number_prefix", "sys_number_prefix_num" ));
