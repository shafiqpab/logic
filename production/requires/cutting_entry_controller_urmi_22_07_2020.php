...
if($action=="load_drop_down_cutt_company")
{
	...
	if($data==3)
	{
		...
		echo create_drop_down( "cbo_cutting_company", 150, "select id,company_name from lib_company where is_deleted=0 and status_active=1 order by company_name","id,company_name", 1, "--- Select ---", $selected_company, "load_drop_down( 'requires/cutting_entry_controller_urmi', this.value, 'load_drop_down_location', 'location_td' );",0,0 );
	else
...
if($action=="save_update_delete")
{
	...
	$color_size_arr=sql_select("select id,size_number_id,color_number_id,po_break_down_id,country_id,item_number_id from wo_po_color_size_breakdown where status_active in(1,2,3) and is_deleted=0");
	...
	$bundle_wise_type_sql="SELECT b.bundle_no ,b.color_type_id from ppl_cut_lay_mst a,ppl_cut_lay_bundle b where a.id=b.mst_id and  a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.cutting_no='$txt_cutting_no' ";
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
	    ...
	    $field_array="id,delivery_mst_id, garments_nature, company_id, production_source, serving_company,country_id, po_break_down_id,item_number_id, location, production_date,production_quantity,production_type, entry_break_down_type, production_hour, floor_id, reject_qnty,total_produced, yet_to_produced, cut_no, batch_no,wo_order_id,currency_id,exchange_rate,rate,amount, inserted_by, insert_date";
	    ...
	    $field_array_qc_dtls="id,mst_id,order_id,country_id,color_id,size_id,color_size_id,bundle_no,barcode_no,number_start,number_end,bundle_qty, reject_qty, replace_qty, qc_pass_qty, inserted_by, insert_date, status_active,is_deleted";
		    ...
		    for($m=0;   $m<count($txt_bundle_number); $m++)
		    {
				...
				//echo $grp_col_id."==";
				 ...
				 if(str_replace("'","",$txt_qcpass_qty[$m])!="")
				 {
					...
					// die;
					 ...
					 $data_array_qc_detls.="(".$qc_dtls_id.",".$qc_id.",".$txt_order_id[$i].",'".$txt_country_id[$m]."','".$hidden_color[$i]."','".$txt_size_id[$m]."','".$color_size_bkdown_id."','".$txt_bundle_number[$m]."','".$hidden_barcode_no_all[$m]."','".$txt_rmg_start[$m]."','".$txt_rmg_end[$m]."','".trim($txt_size_qty[$m])."','".$txt_reject_qty[$m]."','".$txt_replace_qty[$m]."','".$txt_qcpass_qty[$m]."',".$user_id.",'".$pc_date_time."',1,0)";
					  $qc_dtls_id = return_next_id_by_sequence(  "pro_gmts_cutting_qc_dtls_seq","pro_gmts_cutting_qc_dtls", $con );
				...
				// print_r($country_qcpass_qty);die;
				 foreach($country_qcpass_qty as $c_id=>$c_qty)
				 {
					...
					and item_number_id=".$txt_gmt_id[$i]." and  country_id=".$c_id." and status_active in(1,2,3) and is_deleted=0","plan_cut_qty");
					...
					if($db_type==0)
					{
						...
						$data_array.="(".$id.",".$qc_id.",".$garments_nature.",".$cbo_company.",'".$cbo_source."','".$cbo_cutting_company."',".$c_id.",".$txt_order_id[$i].", ".$txt_gmt_id[$i].",'".$cbo_location_name."','".$txt_cutting_date."',".trim($c_qty).",1,3,'".$txt_cutting_hour."','".$cbo_floor_name."',".$country_reject_qty[$c_id].",'".$total_produced."','".$yet_to_produced."','".$txt_cutting_no."','".$txt_batch_no."','".$cbo_work_order."','".$currency_id."','".$exchange_rate."','".$rate."','".$amount."',".$user_id.",'".$pc_date_time."')";
						//$id=$id+1;
						$id= return_next_id_by_sequence(  "pro_gar_production_mst_seq", "pro_garments_production_mst", $con );
					}
					else
					{
						$data_array.=" INTO pro_garments_production_mst (".$field_array.") VALUES(".$id.",".$qc_id.",".$garments_nature.",".$cbo_company.",'".$cbo_source."','".$cbo_cutting_company."','".$c_id."',".$txt_order_id[$i].", ".$txt_gmt_id[$i].",'".$cbo_location_name."', '".$txt_cutting_date."',".trim($c_qty).",1,3,".$txt_cutting_hour.",'".$cbo_floor_name."','".$country_reject_qty[$c_id]."','".$total_produced."', '".$yet_to_produced."','".$txt_cutting_no."','".$txt_batch_no."','".$cbo_work_order."','".$currency_id."','".$exchange_rate."','".$rate."', '".$amount."', ".$user_id.",'".$pc_date_time."')";
						//$id=$id+1;
						$id= return_next_id_by_sequence(  "pro_gar_production_mst_seq",  "pro_garments_production_mst", $con );
					}
				 }
		 ...
		 if ($color_size_is_zero!=1)
		  {
 		  ...
 		  $receive_sql="SELECT c.barcode_no,c.bundle_no from pro_garments_production_dtls c where  c.bundle_no  in ($bundle)  and c.production_type=1 and c.status_active=1 and c.is_deleted=0"; ;
            ...
            if(count($duplicate_bundle)>0) { echo "200**"; die;}
 ...
 // echo $data_array_qc_detls;
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
	   ...
	   // if( check_table_status( 160, 1 )==0 ) { echo "15**1"; die;}
		...
		$field_array1="production_source*serving_company*location*floor_id*production_date*production_quantity*production_hour*reject_qnty*wo_order_id* currency_id* exchange_rate* rate*amount*updated_by*update_date";
		...
		$field_array_qc_mst="production_source*serving_company*location_id*floor_id*work_order_id*workorder_rate_breakdown*cutting_qc_date*cutting_qc_time*remarks*updated_by*update_date";
	    $field_array_qc_dtls="reject_qty*replace_qty*qc_pass_qty* updated_by*update_date*is_deleted*status_active";
		$field_array_defect="id, mst_id, production_type, po_break_down_id, defect_type_id, defect_point_id, defect_qty,bundle_no, inserted_by, insert_date";
		$field_array_qc_save="id,mst_id,order_id,country_id,color_id,size_id,color_size_id,bundle_no,barcode_no,number_start,number_end,bundle_qty,reject_qty, replace_qty, qc_pass_qty,inserted_by,insert_date,status_active,is_deleted";
		...
		if($db_type==2)
		{
	  	...
	  	$data_arra_cutt_mst="'".$cbo_source."'*".$cbo_cutting_company."*'".$cbo_location_name."'*'".$cbo_floor_name."'*'".$cbo_work_order."'*'".$piece_rate_data_string."'*'".$txt_cutting_date."'*".$txt_cutting_hour."*".$txt_remarks."*".$user_id."*'".$pc_date_time."'";
		    ...
		    for($m=0;$m<count($txt_bundle_number); $m++)
		    {
			   ...
			   if( $txt_actual_reject[$m]!="" )
			   {
					...
					for($rls=0;$rls<count($actual_reject_info); $rls++)
					{
						...
						$data_array_defect.="(".$dft_id.",".$product_mst_arr[$txt_order_id[$i]][$txt_country_id[$m]].",1,".$txt_order_id[$i].",3,".$defectPointId.",'".$defect_qty."','".$hidden_barcode_no_all[$m]."',".$user_id.",'".$pc_date_time."')";
						//$dft_id++;
						$dft_id = return_next_id_by_sequence(  "pro_gmts_prod_dft_seq", "pro_gmts_prod_dft", $con );
					}
			   }
				...
				//if(0==0)
				{
					if(str_replace("'","",$txt_qcpass_qty[$m])!="")
					{
					   ...
					   $data_array_dtls .= "(".$dtls_id.",'".$product_mst_arr[$txt_order_id[$i]][$txt_country_id[$m]]."',1,'".$color_size_bkdown_id."','".$txt_qcpass_qty[$m]."','".$txt_cutting_no."','".$txt_bundle_number[$m]."','".$hidden_barcode_no_all[$m]."','".$txt_reject_qty[$m]."','".$txt_replace_qty[$m]."','".$update_id."','".$bundle_wise_type_array[$txt_bundle_number[$m]]."')";
					   //$dtls_id=$dtls_id+1;
					   $dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",   "pro_garments_production_dtls", $con );
			 ...
			 foreach($country_qcpass_qty as $c_id=>$c_qty)
			 {
				...
				if($country_reject_qty[$c_id]==""){$country_reject_qty[$c_id]=0;}
				$update_id_mst_arr[]=str_replace("'",'',$product_mst_arr[$txt_order_id[$i]][$c_id]);
				$data_array_prod[str_replace("'",'',$product_mst_arr[$txt_order_id[$i]][$c_id])] =explode("*",("'".$cbo_source."'*'".$cbo_cutting_company."'*'".$cbo_location_name."'*'".$cbo_floor_name."'*'".$txt_cutting_date."'*'".trim($country_qcpass_qty[$c_id])."'*".$txt_cutting_hour."*'".$country_reject_qty[$c_id]."'*'".$cbo_work_order."'*'".$currency_id."'*'".$exchange_rate."'*'".$rate."'*'".$amount."'*'".$user_id."'*'".$pc_date_time."'"));
...
<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            ...
            <tbody>
                  <tr class="general">
                        ...
                        <td align="center">
                               <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('txt_cut_no').value+'_'+document.getElementById('txt_job_search').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_cut_qc').value+'_'+document.getElementById('txt_order_search').value, 'create_system_search_list_view', 'search_div', 'cutting_entry_controller_urmi', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />
                        </td>
                 </tr>
        		 <tr>
...
if($action=="create_system_search_list_view")
{
    ...
    where a.cutting_no=b.cutting_no and  b.job_no=c.job_no and c.job_no=d.job_no_mst $conpany_cond  $cut_cond $job_cond $sql_cond $order_cond $system_cond and a.status_active=1 and a.is_deleted=0 and b.id=e.mst_id   order by a.id desc";
...
if($action=="load_system_mst_form")
{
	...
	foreach($sql_qc_mst as $val)
	{
		...
		$sql=sql_select("select distinct c.id,c.buyer_name,$insert_year from  wo_po_break_down a,wo_po_details_master b,lib_buyer c where  a.job_no_mst=b.job_no and b.job_no='".$val[csf("job_no")]."' and b.buyer_name=c.id and a.status_active in(1,2,3) ");
...
//	die;
	...
	<?
				   $bundle_data=sql_select("select DISTINCT a.id,a.country_id,a.bundle_no,a.barcode_no,a.size_id,a.number_start,a.number_end,a.size_qty from ppl_cut_lay_bundle a, ppl_cut_lay_size b where  a.mst_id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1  and c.is_deleted=0  and a.dtls_id=b.dtls_id and a.size_id=b.size_id and a.mst_id=".$val[csf('id')]." and a.dtls_id=".$val[csf('details_id')]." and a.order_id=".$val[csf('order_id')]." and a.size_qty>0 order by a.id ASC");
					$i=1;
					$color_qty=0;
				...
				?>
				  ...
				  <tr  style=" background-color:#B0B0B0;"  b height="10">
						  ...
						  <td align="right" ><input type="hidden" id="hidden_total_qc_qty_<? echo $j; ?>"
						   name="hidden_total_qc_qty_<? echo $j; ?>"  value="<? echo $color_qty;  ?> "/></td>
						  <td align="right" ><input type="hidden" id="hidden_reject_qty_<? echo $j; ?>"
						  name="hidden_reject_qty_<? echo $j; ?>"  value="<? echo $order_color_qty[$val[csf("order_id")]][$val[csf('country_id')]][$val[csf("color_id")]]['reject_qty'];  ?>" />
                          <input type="hidden" id="hidden_replace_qty_<? echo $j; ?>"
						  name="hidden_replace_qty_<? echo $j; ?>"  value="<? echo $order_color_qty[$val[csf("order_id")]][$val[csf('country_id')]][$val[csf("color_id")]]['replace_qty'];  ?>" />
               ...
               <tr>
                   <td colspan="7" align="center" class="">
                        <?
                           echo load_submit_buttons( $permission, "fnc_cut_qc_info", 1,0,"reset_form('','','','','clear_tr()')",1);
                        ?>
                         <input class="formbutton" value="Print" onClick="fnc_cut_qc_info(5)" style="width:80px;" type="button"><input class="formbutton" value="Reject Qnty Challan" onClick="fnc_cut_qc_info(6)" style="width:150px;" type="button">
                        </td>
              </tr>
            </table>
        <input type="hidden" id="total_order_id" name="total_order_id" value="<? echo $j-1; ?>"  />
...
if ($action == "print_reject_report") {
	...
	<br>
	<?
	echo signature_table(117, $company, "1060px");
	?>
	</div>
...
if ($action == "print_only_reject_report") {
	...
	$sql_dtls_lay=sql_select("SELECT b.id,b.cutting_no,c.order_id,a.ship_date,a.color_id,a.gmt_item_id,a.plies,a.marker_qty,a.order_qty,a.total_lay_qty, a.lay_balance_qty,a.batch_id,b.job_no,b.job_year,b.company_id ,a.id as details_id from ppl_cut_lay_dtls a, ppl_cut_lay_mst b,ppl_cut_lay_bundle c where b.id=a.mst_id and b.cutting_no='".$cutting_no."' and b.id=c.mst_id and a.id=c.dtls_id group by b.id,b.cutting_no,c.order_id,a.ship_date,a.color_id,a.gmt_item_id,a.plies,a.marker_qty,a.order_qty,a.total_lay_qty, a.lay_balance_qty,a.batch_id,b.job_no,b.job_year,b.company_id ,a.id order by a.id");
	...
	// ================= getting reject qty ====================
	$sql = "SELECT c.color_number_id as color_id, c.size_number_id as size_id, b.reject_qty from pro_garments_production_mst a, pro_garments_production_dtls b, wo_po_color_size_breakdown c where a.id=b.mst_id and b.color_size_break_down_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active in(1,2,3) and c.is_deleted=0 and a.cut_no='".$cutting_no."' and a.po_break_down_id=c.po_break_down_id";
	...
	<div style="width: 1000px;">
		...
		<?
		echo signature_table(117, $company, "1000px");
		?>
