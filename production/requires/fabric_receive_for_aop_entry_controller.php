...
{
	//select c.supplier_name,c.id from lib_supplier_tag_company a, lib_supplier_party_type b, lib_supplier c where c.id=b.supplier_id and a.supplier_id = b.supplier_id and a.tag_company=$data and b.party_type=25 and c.status_active=1 and c.is_deleted=0 group by c.id, c.supplier_name order by c.supplier_name
	echo create_drop_down( "cbo_supplier_name", 172, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=25 and   a.status_active =1 and a.is_deleted=0 order by supplier_name","id,supplier_name", 1, "--Select Supplier--",$selected,"",0 );
	/*$exdata=explode('_',$data);
	if($exdata[0]==5 || $exdata[0]==3)
	{
	   echo create_drop_down( "cbo_supplier_name", 172, "select id,company_name from lib_company where status_active=1 and is_deleted=0 order by company_name", "id,company_name",1, "--Select Company--", $selected, "get_php_form_data( this.value, 'load_drop_down_attention', 'requires/fabric_receive_for_aop_entry_controller');",0,"" );
	}
	else
	{

	   echo create_drop_down( "cbo_supplier_name", 172, "select c.supplier_name,c.id from lib_supplier_tag_company a, lib_supplier_party_type b, lib_supplier c where c.id=b.supplier_id and a.supplier_id = b.supplier_id and a.tag_company=$exdata[1] and b.party_type=25 and c.status_active=1 and c.is_deleted=0 group by c.id, c.supplier_name order by c.supplier_name","id,supplier_name", 1, "--Select Supplier--",$selected,"get_php_form_data( this.value, 'load_drop_down_attention', 'requires/fabric_receive_for_aop_entry_controller');",0 );
	}*/
	exit();
}
...
if ($action=="fabric_search_list_view")
{
	...
	foreach($sql_data as $sql_row)
	{
        ...
        <tr style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value(<? echo $i;?>)">
            ...
            <td width="70"><? echo $sql_row[csf('gsm_weight')]; ?></td>
            <td width="70"><? echo $sql_row[csf('dia')]; ?></td>
...
if ($action=="populate_order_data_from_search_popup")
{
	...
	foreach ($data_array as $row){
		...
		echo "load_drop_down( 'requires/fabric_receive_for_aop_entry_controller', '".$row[csf("job_no")]."', 'load_drop_down_fabric_description', 'fabric_description_td' )\n";
	}
}
...
if ($action=="generate_aop_booking")
{
	...
	if($is_update==1) // save
	{
		if($entry_form==462)
		{
			...
			WHERE   c.id = d.po_id AND d.mst_id = e.id AND g.batch_id = e.id AND d.prod_id = h.id and d.mst_id=e.id and g.batch_id=e.id and d.prod_id=h.id and d.prod_id = g.prod_id and j.id=g.mst_id and j.entry_form=462 and  c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1 and g.status_active=1 and g.is_deleted=0 and g.mst_id in ($issue_id) and g.id in ($dtls_id) $company  group by  c.id, c.company_id, c.po_buyer, c.style_ref_no, c.job_no, c.within_group, c.sales_booking_no, e.batch_no, e.extention_no, e.color_id, h.item_description, h.gsm, h.dia_width, e.id, g.id, g.batch_dtls_id, g.process_type_id, g.quantity, g.rate, g.amount, g.number_of_roll, g.remark,h.id, j.sys_no, j.sys_date ,j.order_no,j.id,g.mst_id, j.entry_form order by g.id desc";
			//echo $sql;
			$sql_mst="select id, entry_form, sys_no_prefix, sys_no_prefix_num, sys_no, sys_date, delivery_date, company_id, buyer_id,supplier_id,supplier_address, issue_purpose, source, order_id, order_no, attention, vehical_no, driver_name, dl_no, transport, mobile_no, gate_pass_no, is_short, remark from wo_fabric_aop_mst where id in ($issue_id)  and entry_form=462 and status_active=1 and company_id='$data[4]'";
		...
		{
			$sql= "SELECT c.id, c.company_id, c.po_buyer as buyer_id, c.style_ref_no, c.job_no AS fso_number, c.within_group, c.sales_booking_no, e.batch_no, e.extention_no, e.color_id, h.item_description AS fabric_desc, h.gsm AS gsm_weight, h.dia_width AS dia, e.id AS batch_id, g.id as issue_dtls_id, null as batch_dtls_id, g.process_type_id, g.issue_qnty as quantity, g.rate, g.amount, g.roll_no as number_of_roll, g.remarks as remark,h.id as prod_id, j.sys_number as sys_no, j.delevery_date as sys_date ,c.job_no as order_no,j.id as issue_id,g.mst_id, j.entry_form, g.bodypart_id
			FROM  fabric_sales_order_mst c, pro_batch_create_dtls d, pro_batch_create_mst e, aop_multi_issue_dtls g, product_details_master h, aop_multi_issue_mst j
			WHERE   c.id = d.po_id AND d.mst_id = e.id AND g.batch_id = e.id AND d.prod_id = h.id and d.mst_id=e.id and g.batch_id=e.id and d.prod_id=h.id and d.prod_id = g.product_id and j.id=g.mst_id and j.entry_form=522 and  c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1 and g.status_active=1 and g.is_deleted=0 and g.mst_id in ($issue_id) and g.id in ($dtls_id) $company  group by  c.id, c.company_id, c.po_buyer, c.style_ref_no, c.job_no, c.within_group, c.sales_booking_no, e.batch_no, e.extention_no, e.color_id, h.item_description, h.gsm, h.dia_width, e.id, g.id, g.process_type_id, g.issue_qnty, g.rate, g.amount, g.roll_no, g.remarks,h.id, j.sys_number, j.delevery_date,j.id,g.mst_id, j.entry_form, g.bodypart_id order by g.id desc";
			//echo $sql;die;
	...
	{
		$sql= "SELECT c.id, c.company_id, c.po_buyer AS buyer_id, c.style_ref_no, c.job_no AS fso_number, c.within_group, c.sales_booking_no, e.batch_no, e.extention_no, e.color_id, h.item_description AS fabric_desc, h.gsm AS gsm_weight, h.dia_width AS dia, e.id AS batch_id, g.id AS dtls_id, g.batch_dtls_id, g.process_type_id, g.quantity, g.rate, g.amount, g.number_of_roll, g.remark,h.id as prod_id, j.sys_no, j.sys_date ,j.order_no,j.id as issue_id, g.issue_dtls_id,g.mst_id,g.issue_entry_form as entry_form
		FROM  fabric_sales_order_mst c, pro_batch_create_dtls d, pro_batch_create_mst e, wo_fabric_aop_dtls g, product_details_master h, wo_fabric_aop_mst j
		WHERE   c.id = d.po_id AND d.mst_id = e.id AND g.batch_id = e.id AND d.prod_id = h.id and d.mst_id=e.id and g.batch_id=e.id and d.prod_id=h.id and d.prod_id = g.prod_id and j.id=g.mst_id and j.entry_form=467 and  c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1 and g.status_active=1 and g.is_deleted=0 and g.mst_id=$mst_id  group by  c.id, c.company_id, c.po_buyer, c.style_ref_no, c.job_no, c.within_group, c.sales_booking_no, e.batch_no, e.extention_no, e.color_id, h.item_description, h.gsm, h.dia_width, e.id, g.id, g.batch_dtls_id, g.process_type_id, g.quantity, g.rate, g.amount, g.number_of_roll, g.remark,h.id, j.sys_no, j.sys_date ,j.order_no,j.id ,g.issue_dtls_id,g.mst_id,g.issue_entry_form order by g.id desc";


		/*$sql= "select c.id,c.company_id, c.buyer_id,c.style_ref_no,c.job_no as fso_number, c.within_group, c.sales_booking_no, e.batch_no, e.extention_no, b.color_id, b.body_part_id , b.fabric_desc, b.gsm_weight, b.dia ,b.id as so_dtls_id,e.id as batch_id,g.id as dtls_id, g.process_type_id, g.quantity, g.rate, g.amount, g.number_of_roll, g.remark ,g.issue_dtls_id, sum(d.batch_qnty)  as batch_weight,g.mst_id from 
		fabric_sales_order_dtls b, fabric_sales_order_mst c, pro_batch_create_dtls d , pro_batch_create_mst e, wo_fabric_aop_dtls g ,product_details_master  h where b.mst_id=c.id and c.id=d.po_id and d.mst_id=e.id and g.sales_order_dtls_id=b.id and g.batch_id=e.id and e.color_id=b.color_id and d.prod_id=h.id and g.mst_id=$mst_id and b.body_part_id= d.body_part_id and h.item_description= trim(b.fabric_desc) and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1 and g.status_active=1 and g.is_deleted=0 group by  c.id,c.company_id, c.buyer_id,c.style_ref_no,c.job_no, c.within_group, c.sales_booking_no, e.batch_no, e.extention_no, b.color_id, b.body_part_id , b.fabric_desc, b.gsm_weight, b.dia ,b.id,e.id,g.id, g.process_type_id, g.quantity, g.rate, g.amount, g.number_of_roll, g.remark, g.issue_dtls_id,g.mst_id order by g.id desc";*/

		$sql_mst="select id,issue_id entry_form, sys_no_prefix, sys_no_prefix_num, sys_no, sys_date, delivery_date, company_id, buyer_id,supplier_id,supplier_address, issue_purpose, source, order_id, order_no, attention, vehical_no, driver_name, dl_no, transport, mobile_no, gate_pass_no, is_short, remark from wo_fabric_aop_mst where id in ($mst_id) and status_active=1";
	...
	//echo $sql;
	$sql_data=sql_select($sql); $batch_ids='';
	...
	$issue_qty_sql= "select g.id,g.quantity from wo_fabric_aop_dtls g , wo_fabric_aop_mst h where h.id=g.mst_id and g.is_deleted=0 and g.status_active=1 and h.is_deleted=0 and h.status_active=1 and h.entry_form=462";
	$prev_qty_sqlArray=sql_select($issue_qty_sql); $issue_qty_arr=array();
	foreach ($prev_qty_sqlArray as  $row) 
	{
		$issue_qty_arr[$row[csf("id")]]["quantity"] =$row[csf("quantity")];
	}

	$multi_issue_qty_sql= "select g.id,g.issue_qnty from aop_multi_issue_mst h, aop_multi_issue_dtls g where h.id=g.mst_id and g.is_deleted=0 and g.status_active=1 and h.is_deleted=0 and h.status_active=1 and h.entry_form=522";

	$prev_multi_issue_qty_res=sql_select($multi_issue_qty_sql); $multi_issue_qty_arr=array();
	foreach ($prev_multi_issue_qty_res as  $row) 
	{
		$multi_issue_qty_arr[$row[csf("id")]]["quantity"] =$row[csf("issue_qnty")];
	}


	$prev_rcv_qty_sql= "select g.issue_dtls_id,g.quantity from wo_fabric_aop_dtls g , wo_fabric_aop_mst h where h.id=g.mst_id and g.is_deleted=0 and g.status_active=1 and h.is_deleted=0 and h.status_active=1 and h.entry_form=467 and g.issue_dtls_id is not null";
	$prev_rcv_qty_sqlArray=sql_select($prev_rcv_qty_sql); $prev_rcv_qty_arr=array();
	foreach ($prev_rcv_qty_sqlArray as  $row) 
	{
		$prev_rcv_qty_arr[$row[csf("issue_dtls_id")]]["quantity"] +=$row[csf("quantity")];
	}
	...
	foreach($sql_data as $sql_row)
	{
		...
		if($is_update==1) // save
		{
			$issue_qty=$sql_row[csf('quantity')];
			$rcv_qty='';
			$rcv_rate=$sql_row[csf('rate')];
			$rcv_amount='';
			$remark='';
			$balance=$issue_qty-$prev_rcv_qty_arr[$sql_row[csf("issue_dtls_id")]]["quantity"];
		}
		else{
			if($sql_row[csf("entry_form")] == 522)
			{
				$issue_qty=$multi_issue_qty_arr[$sql_row[csf("issue_dtls_id")]]["quantity"];
			}else{	
				$issue_qty=$issue_qty_arr[$sql_row[csf("issue_dtls_id")]]["quantity"];
			}
		

			$rcv_qty=$sql_row[csf('quantity')];
			$rcv_rate=$sql_row[csf('rate')];
			$rcv_amount=$sql_row[csf('amount')];
			$remark=$sql_row[csf('remark')];
			$balance=$issue_qty-$prev_rcv_qty_arr[$sql_row[csf("issue_dtls_id")]]["quantity"]+$rcv_qty;
		}

		$batch_dtls_id=array_unique(explode(",",$sql_row[csf("batch_dtls_id")]));
		$batchQty=$bodyPartID='';
		foreach ($batch_dtls_id as $value) {
			$bodyPartID=$batch_dtls_arr[$value]["body_part_id"];
			$batchQty +=$batch_dtls_arr[$value]["batch_qnty"];
		}
		//echo $issue_qty.'=='.$prev_rcv_qty_arr[$sql_row[csf("issue_dtls_id")]]["quantity"].'=='.$sql_row[csf("issue_dtls_id")];
		?>
		<tr align="center" id="row_<? echo $i;?>">
			<td style="overflow:hidden;text-overflow: ellipsis;white-space: nowrap;"><? echo $i; ?></td>
			<td style="overflow:hidden;text-overflow: ellipsis;white-space: nowrap;">
				<?
				if($mst_arr[$sql_row[csf("mst_id")]]['buyer_id'] !='')
				{
					echo $buyer_arr[$mst_arr[$sql_row[csf("mst_id")]]['buyer_id']];
				}else{echo $buyer_arr[$sql_row[csf("buyer_id")]];}
				?>
			</td>
			...
			<td style="overflow:hidden;text-overflow: ellipsis;white-space: nowrap;">
				<?
				if(!empty($sql_row[csf('bodypart_id')])){
					echo $body_part[$sql_row[csf('bodypart_id')]];
				}else{echo $body_part[$bodyPartID]; }
				 ?>
			</td>
			...
			<td><input type="text" name="txtWoqnty[]" id="txtWoqnty_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric"  value="<? echo number_format($issue_qty,2,'.',''); ?>" readonly /></td>
			<td><input type="text" name="txtRcvqnty[]" id="txtRcvqnty_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric" onChange="calculate_amount(<? echo $i; ?>)" value="<? echo number_format($rcv_qty,2,'.',''); ?>"  placeholder="<? echo number_format($balance,2); ?>" /></td>
			<td><input type="text" name="txtRate[]" id="txtRate_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric" onChange="calculate_amount(<? echo $i; ?>)" value="<? echo number_format($rcv_rate,2); ?>";></td>
			<td><input type="text" name="txtAmount[]" id="txtAmount_<? echo $i; ?>" style="width:60px;" class="text_boxes_numeric" value="<? echo number_format($rcv_amount,4,'.',''); ?>";  disabled="disabled"/></td>
			...
			<td><input type="text" name="txtRemarks[]"  id="txtRemarks_<? echo $i; ?>" style="width:60px;" class="text_boxes" value="<? echo $remark; ?>";/></td>
			...
			<input type="hidden" id="hndIssueDtlsId_<? echo $k;?>" name="hndIssueDtlsId[]"  value="<? echo $sql_row[csf('issue_dtls_id')]; ?>"  /></td>
			...
			<input type="hidden" id="IssueEntryForm_<? echo $k;?>" name="IssueEntryForm[]"  value="<? echo $sql_row[csf('entry_form')]; ?>"  /></td>
		</tr>
		<?
		$i++;
	}
...
if ($action=="save_update_delete_dtls")
{
	...
	{
		$sql=sql_select("select embellishment_job, issue_id from subcon_ord_mst where order_no=$txt_booking_no and status_active=1 and is_deleted=0");
		foreach($sql as $row){

			if($row[csf('embellishment_job')]=="") $row[csf('embellishment_job')]=$row[csf('issue_id')];
	...
	if ($operation==0)  // Insert Here
	{
		...
		$response_booking_no="";
		if($db_type==0){
			$new_booking_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'FRFA', date("Y",time()), 5, "select id, sys_no_prefix, sys_no_prefix_num from wo_fabric_aop_mst where company_id=$cbo_company_name and entry_form=467 and YEAR(insert_date)=".date('Y',time())." order by id desc ", "sys_no_prefix", "sys_no_prefix_num" ));
		}
		else if($db_type==2){
			$new_booking_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'FRFA', date("Y",time()), 5,"select id, sys_no_prefix, sys_no_prefix_num from wo_fabric_aop_mst where company_id=$cbo_company_name and entry_form=467 and to_char(insert_date,'YYYY')=".date('Y',time())." order by id desc ", "sys_no_prefix", "sys_no_prefix_num" ));
		}

		$id=return_next_id( "id", "wo_fabric_aop_mst", 1 ) ;
		$field_array="id, entry_form, sys_no_prefix, sys_no_prefix_num, sys_no, sys_date, company_id, supplier_id, supplier_address, issue_purpose, source, issue_id, order_id, order_no, rcv_challan_no, is_short, remark,buyer_id, within_group, currency_id, inserted_by, insert_date";
		if($db_type==0){
		...
		//$hid_issue_id=array_unique(explode(",",$hid_issue_id));
		$hid_issue_id=implode(",",array_unique(explode(",",$hid_issue_id)));
		$within_group=implode(",",array_unique(explode(",",$within_group)));
		//$hid_issue_id=7;
		$data_array ="(".$id.", 467 ,'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."','".$txt_receive_date."',".$cbo_company_name.",".$cbo_supplier_name.",'".$txt_party_location."',".$cbo_receive_purpose.",".$cbo_source.",".$hid_issue_id.",".$hid_order_id.",'".$txt_order_no."','".$txt_rcv_challan_no."',".$cbo_is_short.",'".$txt_remarks."','".$buyer_id."',".$within_group.",".$cbo_currency_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		$field_array1=" id, mst_id, order_id, batch_id, sales_order_dtls_id, batch_dtls_id,issue_dtls_id, prod_id,issue_entry_form, process_type_id, quantity, rate, amount, number_of_roll, remark, inserted_by, insert_date";
		...
		{			
			...
			$hndIssueDtlsId		= "hndIssueDtlsId_".$i; 
			$prodId				= "prodId_".$i;
			$IssueEntryForm		= "IssueEntryForm_".$i;
			
			//echo "10**".$$IssueEntryForm; die;
			if ($add_commaa!=0) $data_array1 .=","; $add_comma=0;
			$data_array1 .="(".$id_dtls.",".$id.",".$hid_order_id.",".$$batchId.",".$$soDtlsId.",".$$batchDtlsId.",".$$hndIssueDtlsId.",".$$prodId.",".$$IssueEntryForm.",".$$cboProcessType.",".str_replace(",",'',$$txtRcvqty).",".str_replace(",",'',$$txtRate).",".str_replace(",",'',$$txtAmount).",".$$txtNumberRoll.",".$$txtRemarks.",'".$user_id."','".$pc_date_time."')";
		...
		$rID2=sql_insert("wo_fabric_aop_dtls",$field_array1,$data_array1,0);
		//echo "10**".$rID2.'=='.$rID; die;
		if($db_type==0)
		{
			if($rID && $rID2)
			{
	...
	else if ($operation==1)   // Update Here
	{
		...
		$field_array_up="sys_date*supplier_id*supplier_address*issue_purpose*source*issue_id*order_id*order_no*rcv_challan_no*is_short*remark*buyer_id*currency_id*updated_by*update_date";
		//$data_array_up ="".$txt_receive_date."*".$cbo_supplier_name."*".$txt_party_location."*".$cbo_receive_purpose."*".$cbo_source."*".$hid_issue_id."*".$hid_order_id."*".$txt_order_no."*".$txt_rcv_challan_no."*".$cbo_is_short."*".$txt_remarks."*".$buyer_id."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		$data_array_up ="'".$txt_receive_date."'*'".$cbo_supplier_name."'*'".$txt_party_location."'*'".$cbo_receive_purpose."'*'".$cbo_source."'*'".$hid_issue_id."'*'".$hid_order_id."'*'".$txt_order_no."'*'".$txt_rcv_challan_no."'*'".$cbo_is_short."'*'".$txt_remarks."'*'".$buyer_id."'*'".$cbo_currency_id."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		
		
	   $rID=sql_update("wo_fabric_aop_mst",$field_array_up,$data_array_up,"id","".$update_id."",0); 
		//echo "10**nnn".$field_array_up.'**'.$data_array_up; die;
		$field_array_up1="order_id*batch_id*sales_order_dtls_id*batch_dtls_id*prod_id*issue_entry_form*process_type_id*quantity*rate*amount*number_of_roll*remark*status_active*is_deleted*updated_by*update_date";
		//$update_id=1; 
		$add_commaa=0;
		for($i=1; $i<=$total_row; $i++)
		{			
			...
			if(str_replace("'",'',$$updateDtlsId)!=""){
				...
				$data_array_up1[str_replace("'",'',$$updateDtlsId)] =explode("*",("".$hid_order_id."*".$$batchId."*".$$soDtlsId."*".$$batchDtlsId."*".$$prodId."*".$$IssueEntryForm."*".$$cboProcessType."*".str_replace(",",'',$$txtRcvqty)."*".str_replace(",",'',$$txtRate)."*".str_replace(",",'',$$txtAmount)."*".$$txtNumberRoll."*".$$txtRemarks."*1*0*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
			}
		}

		$field_array_status = "updated_by*update_date*status_active*is_deleted";
		$data_array_status = $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'*0*1";
		$rID1 = sql_multirow_update("wo_fabric_aop_dtls", $field_array_status, $data_array_status, "mst_id", $update_id, 1);

		$rID2=execute_query(bulk_update_sql_statement( "wo_fabric_aop_dtls", "id", $field_array_up1, $data_array_up1, $id_arr ),1);
        //check_table_status( $_SESSION['menu_id'],0);
...
if ($action=="issue_popup")
{
            ...
            <table width="100%" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                ...
                <tr class="general">
                    <td>
                        ...
                        <? echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", '', "load_drop_down( 'fabric_receive_for_aop_entry_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );",0); ?>
                    </td>
	...
	<script type="text/javascript">
		...
		//load_drop_down( 'fabric_receive_for_aop_entry_controller', $("#cbo_company_mst").val(), 'load_drop_down_buyer', 'buyer_td' );
	</script>
	</html>
	<?
	exit();
...
if ($action=="create_issue_search_list_view")
{
	...
	if ($data[1]!='') $order_no=" and a.order_no like '%$data[1]%'"; else $order_no="";
	if ($data[2]!='') $sys_no=" and a.sys_no like '%$data[2]%'"; else $sys_no="";
	...
	<div style="width:700px;">
     	<table cellspacing="0" cellpadding="0" border="1" align="left" rules="all" width="680" class="rpt_table">
            <thead> 
                ...
                <th width="140">Work Order</th>
                <th width="140">Issue Id</th>
                <th>Issue Date</th>
            </thead>
     	</table>
    </div>
	    ...
	    foreach($result as $row)
	    {
			if ($i%2==0)$bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
	     	...
	     	<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value('<? echo $row[csf("order_no")];?>' , '<? echo $row[csf("id")];?>')" >
				...
				<td width="180" style="word-break:break-all"><?php echo $comp[$row[csf("company_id")]]; ?></td>
				<td width="140" style="word-break:break-all"><?php echo $row[csf("order_no")]; ?></td>
				<td width="140" style="word-break:break-all"><?php echo $row[csf("sys_no")]; ?></td>
				<td align="center"><?php echo change_date_format($row[csf("sys_date")]); ?></td>
			</tr>
     	<?
     	$i++;
        }
...
if ($action=="issue_wo_popup")
{
	...
	function js_set_value(issue_id,order_no,within_group,dtls_id)
	{
		//alert(within_group);
		document.getElementById('selected_booking').value=issue_id;
		document.getElementById('selected_job').value=order_no;
		document.getElementById('within_group').value=within_group;
		document.getElementById('dtls_id').value=dtls_id;
		parent.emailwindow.hide();
	}*/
	...
	function toggle( x, origColor ) {
		...
		var selected_id = new Array; var selected_id_dtls = new Array; var subconArr = new Array; var selected_within_group = new Array; var selected_entry_form = new Array;
		...
		function js_set_value(str) 
		{  	//alert(str);
			var issue_id = $('#hidden_issue_id'+str).val();
			var dtls_id = $('#hidden_dtls_id'+str).val();
			var within_group = $('#hidden_within_group'+str).val();
			var entry_form = $('#hidden_entry_form'+str).val();
			
			if(subconArr.length==0){
				subconArr.push( issue_id );
			}else if( jQuery.inArray( issue_id, subconArr )==-1 &&  subconArr.length>0){
				alert("Issue ID Mixed is Not Allow");return true;
			}
			
			toggle( document.getElementById( 'search' + str ), '#FFFFCC' );
			if( jQuery.inArray( dtls_id, selected_id_dtls  ) == -1 ) {
				//alert(dtls_id)
				selected_id.push( issue_id );
				selected_id_dtls.push( dtls_id );
				selected_within_group.push( within_group );
				selected_entry_form.push( entry_form );
			}else{
				//alert(1111)
				for( var i = 0; i < selected_id.length; i++ ){
				...
				selected_within_group.splice( i, 1 );
				selected_entry_form.splice( i, 1 );
			}
			var id ='';  var id_dtls = ''; var id_within_group = ''; var id_entry_form = '';
			for( var i = 0; i < selected_id_dtls.length; i++ ) {
				id += selected_id[i] + ',';
				id_dtls += selected_id_dtls[i] + ',';
				id_within_group += selected_within_group[i] + ',';
				id_entry_form += selected_entry_form[i] + ',';
			}
			id 		= id.substr( 0, id.length - 1 );
			if(id_dtls!=""&& id_dtls !=null) id_dtls= id_dtls.substr( 0, id_dtls.length - 1 );
			if(id_within_group!=""&& id_within_group !=null) id_within_group= id_within_group.substr( 0, id_within_group.length - 1 );
			if(id_entry_form!=""&& id_entry_form !=null) id_entry_form= id_entry_form.substr( 0, id_entry_form.length - 1 );
			
			$('#all_issue_id').val( id );	
			$('#all_sub_dtls_id').val( id_dtls );
			$('#all_within_group').val( id_within_group );
			$('#all_entry_form').val( id_entry_form );
		}
	</script>
	</head>
	<body>
            ...
            <table width="1000" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                ...
                <tr class="general">
                    <td>
                        ...
                        <input type="hidden" id="entry_form">
                        <? echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", '', "load_drop_down( 'fabric_receive_for_aop_entry_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'fabric_receive_for_aop_entry_controller', this.value, 'load_drop_down_supplier', 'supplier_td' );",0); ?>
                    </td>
                    ...
                    <td>
                    <?
                        $search_by_arr=array(1=>"Batch no",2=>"Issue ID",3=>"FSO No",4=>"Fabric booking no",5=>"Style reference no");
                        echo create_drop_down( "cbo_type",100, $search_by_arr,"",0, "",2,'',0 );
                    ?>
                    </td>
                    <td><input name="txt_booking_prifix" id="txt_booking_prifix" class="text_boxes" style="width:87px" placeholder="Enter Full Prefix"></td>
                    <td><input name="txt_date_from" id="txt_date_from" class="datepicker" style="width:60px"></td>
                    <td><input name="txt_date_to" id="txt_date_to" class="datepicker" style="width:60px"></td>
                    <td><input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_supplier_name').value+'_'+document.getElementById('txt_booking_prifix').value+'_'+document.getElementById('cbo_search_category').value+'_'+document.getElementById('cbo_type').value+'_'+document.getElementById('cbo_within_group').value, 'create_booking_search_list_view', 'search_div', 'fabric_receive_for_aop_entry_controller','setFilterGrid(\'tbl_po_list\',-1)')" style="width:80px;" /></td>
                </tr>
                <tr>
                    <td align="center" valign="middle" colspan="11"><? echo load_month_buttons(1); ?></td>
                </tr>
	...
	<script type="text/javascript">
		$("#cbo_company_mst").val(<? echo $cbo_company_id?>);
		load_drop_down( 'fabric_receive_for_aop_entry_controller', $("#cbo_company_mst").val(), 'load_drop_down_buyer', 'buyer_td' );
		load_drop_down( 'fabric_receive_for_aop_entry_controller', $("#cbo_company_mst").val(), 'load_drop_down_supplier', 'supplier_td' );
...
if ($action=="create_booking_search_list_view")
{
	...
	if ($data[0]!=0) {
		$company=" and j.company_id='$data[0]'";
		$batch_company=" and e.company_id='$data[0]'";
	}
	else 
	{ 
	...
	if($db_type==0)
	{
		if ($data[2]!="" &&  $data[3]!="") $sys_date  = "and j.sys_date  between '".change_date_format($data[2], "yyyy-mm-dd", "-")."' and '".change_date_format($data[3], "yyyy-mm-dd", "-")."'"; else $sys_date ="";

		if ($data[2]!="" &&  $data[3]!="") $delivery_date  = "and j.delevery_date  between '".change_date_format($data[2], "yyyy-mm-dd", "-")."' and '".change_date_format($data[3], "yyyy-mm-dd", "-")."'"; else $delivery_date ="";
		//$year_cond=" and SUBSTRING_INDEX(b.insert_date, '-', 1)=$data[7]";
	}
	else if($db_type==2)
	{
		if ($data[2]!="" &&  $data[3]!="") $sys_date  = "and j.sys_date  between '".change_date_format($data[2], "yyyy-mm-dd", "-",1)."' and '".change_date_format($data[3], "yyyy-mm-dd", "-",1)."'"; else $sys_date ="";
		if ($data[2]!="" &&  $data[3]!="") $delivery_date  = "and j.delevery_date  between '".change_date_format($data[2], "yyyy-mm-dd", "-",1)."' and '".change_date_format($data[3], "yyyy-mm-dd", "-",1)."'"; else $delivery_date ="";
		//$year_cond=" and to_char(b.insert_date,'YYYY')=$data[7]";
	}

	if ($data[4]!=0) $supplier=" and j.supplier_id='$data[4]'"; else $supplier="";
	if ($data[4]!=0) $delivery_to=" and j.delivery_to='$data[4]'"; else $delivery_to="";
	if ($data[5]!=''){
		if($data[7]==1) $search_cond=" and e.batch_no like '%$data[5]%'";
		if($data[7]==2) $search_cond=" and j.sys_no like '%$data[5]%'";
		if($data[7]==2) $search_cond1=" and j.sys_number like '%$data[5]%'";
		if($data[7]==3) $search_cond=" and c.job_no like '%$data[5]%'";
		if($data[7]==4) $search_cond=" and c.sales_booking_no like '%$data[5]%'";
		if($data[7]==5) $search_cond=" and c.style_ref_no like '%$data[5]%'";
	}

	if ($data[5]!=''){
		if($data[7]==1) $search_cond1=" and e.batch_no like '%$data[5]%'";
		if($data[7]==2) $search_cond1=" and j.sys_number like '%$data[5]%'";
		if($data[7]==3) $search_cond1=" and c.job_no like '%$data[5]%'";
		if($data[7]==4) $search_cond1=" and c.sales_booking_no like '%$data[5]%'";
		if($data[7]==5) $search_cond1=" and c.style_ref_no like '%$data[5]%'";
	}
	...
	/*$sql= "select h.id,c.company_id, c.po_buyer as buyer_id,c.style_ref_no,c.job_no as fso_number, c.within_group, c.sales_booking_no, e.batch_no, e.extention_no, b.color_id, b.body_part_id , b.fabric_desc, b.gsm_weight, b.dia, e.batch_weight,b.id as so_dtls_id,e.id as batch_id,g.id as dtls_id, g.process_type_id, g.quantity, h.sys_no, h.sys_date ,h.order_no,h.id as issue_id from 
	fabric_sales_order_dtls b, fabric_sales_order_mst c, pro_batch_create_dtls d , pro_batch_create_mst e, wo_fabric_aop_dtls g, wo_fabric_aop_mst h where  h.id=g.mst_id and b.mst_id=c.id and c.id=d.po_id and d.mst_id=e.id and g.sales_order_dtls_id=b.id and g.batch_id=e.id and e.color_id=b.color_id and h.entry_form=462 and b.body_part_id= d.body_part_id and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1 and g.status_active=1 and g.is_deleted=0 $company $buyer $sys_date $supplier $search_cond $within_group_cond group by  h.id,c.company_id,  c.po_buyer,c.style_ref_no,c.job_no, c.within_group, c.sales_booking_no, e.batch_no, e.extention_no, b.color_id, b.body_part_id , b.fabric_desc, b.gsm_weight, b.dia, e.batch_weight,b.id,e.id,g.id, g.process_type_id, g.quantity, h.sys_no, h.sys_date,h.order_no,h.id order by g.id desc";*/

	$singleIssuesql= "SELECT c.id, c.company_id, c.po_buyer AS buyer_id, c.style_ref_no, c.job_no AS fso_number, c.within_group, c.sales_booking_no, e.batch_no, e.extention_no, e.color_id, h.item_description AS fabric_desc, h.gsm AS gsm_weight, h.dia_width AS dia, e.id AS batch_id, g.id AS dtls_id, g.batch_dtls_id, g.process_type_id, g.quantity, g.rate, g.amount, g.number_of_roll, g.remark,h.id as prod_id, j.sys_no, j.sys_date ,j.order_no,j.id as issue_id, null as bodypart_id, j.entry_form
	FROM  fabric_sales_order_mst c, pro_batch_create_dtls d, pro_batch_create_mst e, wo_fabric_aop_dtls g, product_details_master h, wo_fabric_aop_mst j
	WHERE   c.id = d.po_id AND d.mst_id = e.id AND g.batch_id = e.id AND d.prod_id = h.id and d.mst_id=e.id and g.batch_id=e.id and d.prod_id=h.id and d.prod_id = g.prod_id and j.id=g.mst_id and j.entry_form=462 and  c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1 and g.status_active=1 and g.is_deleted=0  $company $buyer $sys_date $supplier $search_cond $within_group_cond group by  c.id, c.company_id, c.po_buyer, c.style_ref_no, c.job_no, c.within_group, c.sales_booking_no, e.batch_no, e.extention_no, e.color_id, h.item_description, h.gsm, h.dia_width, e.id, g.id, g.batch_dtls_id, g.process_type_id, g.quantity, g.rate, g.amount, g.number_of_roll, g.remark,h.id, j.sys_no, j.sys_date ,j.order_no,j.id, j.entry_form order by g.id desc";

	$multiIssuesql= "SELECT c.id, c.company_id, c.po_buyer AS buyer_id, c.style_ref_no, c.job_no AS fso_number, c.within_group, c.sales_booking_no, e.batch_no,e.extention_no, e.color_id, h.item_description AS fabric_desc, h.gsm AS gsm_weight, h.dia_width AS dia, e.id AS batch_id, g.id AS dtls_id, null as batch_dtls_id,g.process_type_id,g.issue_qnty as quantity,g.rate,g.amount,g.roll_no as number_of_roll, g.remarks as remark,h.id as prod_id,j.sys_number as sys_no,j.delevery_date as sys_date, c.job_no as order_no, j.id as issue_id,g.bodypart_id, j.entry_form 
	FROM fabric_sales_order_mst c, pro_batch_create_dtls d, pro_batch_create_mst e, aop_multi_issue_dtls g,product_details_master h, aop_multi_issue_mst j WHERE c.id = d.po_id AND d.mst_id = e.id AND g.batch_id = e.id AND d.prod_id = h.id and 
	d.mst_id=e.id and g.batch_id=e.id and d.prod_id=h.id and d.prod_id = g.product_id and j.id=g.mst_id and j.entry_form=522 and c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1 and g.status_active=1 and g.is_deleted=0 $company $buyer $delivery_date $delivery_to $search_cond1 $within_group_cond
	group by c.id, c.company_id, c.po_buyer, c.style_ref_no, c.job_no, c.within_group, c.sales_booking_no, e.batch_no, 
	e.extention_no, e.color_id, h.item_description, h.gsm , h.dia_width, e.id, g.id, g.process_type_id,g.issue_qnty,g.rate,g.amount,g.roll_no, g.remarks,h.id,j.sys_number,
	j.delevery_date, j.id, g.bodypart_id, j.entry_form  
	order by g.id desc ";
		
	//echo $multiIssuesql; 	
	$singleIssuesqlRes=sql_select($singleIssuesql); 
	$multiIssuesqlRes=sql_select($multiIssuesql); 
	$result = array_merge($singleIssuesqlRes,$multiIssuesqlRes);
	//var_dump($result);
	$batch_ids='';
	foreach($result as $row)
	{
	...
	<div width="100%">
		<div style="width:1500px;">
	     	<table cellspacing="0" cellpadding="0" border="1" align="left" rules="all" width="1480" class="rpt_table">
	            <thead>
	                <th width="30">SL</th>
	                ...
	                <th width="90">Ext. No</th>
	                <th width="110">Fab Color</th>
	                <th width="110">Bodypart</th>
	    ...
	    <div style="width:1500px; max-height:240px;overflow-y:scroll;">
	        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1480" class="rpt_table" id="tbl_po_list">
		    <?
		    $i=1;
		    foreach($result as $row)
		    {
				...
				foreach ($batch_dtls_id as $value) {
		     	...
		     	<tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value(<? echo $i;?>)" >
					...
					<td width="70" style="word-break:break-all"><?php echo $row[csf("sys_date")]; ?></td>
					<td width="110" style="word-break:break-all"><?php echo $row[csf("sys_no")]; ?></td>
					<td width="110" style="word-break:break-all"><?php echo $buyer_arr[$row[csf("buyer_id")]]; ?></td>
					<td width="90" style="word-break:break-all"><?php echo $row[csf("style_ref_no")]; ?></td>
					<td width="100" style="word-break:break-all"><?php echo $row[csf("fso_number")]; ?></td>
					<td width="100" style="word-break:break-all"><?php echo $row[csf("sales_booking_no")]; ?></td>
					<td width="90" style="word-break:break-all"><?php echo $row[csf("batch_no")]; ?></td>
					<td width="90" style="word-break:break-all"><?php echo $row[csf("extention_no")]; ?></td>
					<td width="110" style="word-break:break-all"><?php echo $color_library[$row[csf("color_id")]]; ?></td>
					<td width="110" style="word-break:break-all">
					<?php 
					if(!empty($row[csf("bodypart_id")]))
					{
						echo $body_part[$row[csf("bodypart_id")]];
					}else{echo $body_part[$bodyPartID]; }
					
					?></td>
					<td width="110" style="word-break:break-all"><?php echo $row[csf("fabric_desc")]; ?></td>
					<td width="80" style="word-break:break-all"><?php echo $row[csf("gsm_weight")]; ?></td>
					<td width="80" style="word-break:break-all"><?php echo $row[csf("dia")]; ?></td>
					<td style="word-break:break-all" align="right"><?php echo number_format($row[csf("quantity")],2); ?>
						<input  class="text_boxes" type="hidden" name="hidden_issue_id<? echo $i; ?>" id="hidden_issue_id<? echo $i; ?>" value="<? echo $row[csf('issue_id')]; ?>" style="width:70px">
						<input class="text_boxes" type="hidden"  name="hidden_dtls_id<? echo $i; ?>" id="hidden_dtls_id<? echo $i; ?>" value="<? echo $row[csf('dtls_id')]; ?>" style="width:70px">
						<input class="text_boxes" type="hidden" name="hidden_within_group<? echo $i; ?>" id="hidden_within_group<? echo $i; ?>" value="<? echo $row[csf('within_group')]; ?>" style="width:70px">	
						<input class="text_boxes" type="hidden" name="hidden_entry_form<? echo $i; ?>" id="hidden_entry_form<? echo $i; ?>" value="<? echo $row[csf('entry_form')]; ?>" style="width:70px">	
					</td>
				</tr>
	     	<?
	     	$i++;
	            ...
	            <td align="center" height="30" valign="bottom">
	                <div style="width:100%">
	                    ...
	                    <div style="width:53%; float:left" align="left">
	                        <input type="button" name="close" onClick="parent.emailwindow.hide();" class="formbutton" value="Close" style="width:100px" />
	                        <input type="hidden"  id="all_issue_id" />
					    	<input type="hidden"  id="all_sub_dtls_id" />
					    	<input type="hidden"  id="all_within_group" />
					    	<input type="hidden"  id="all_entry_form" />
	                    </div>
	                </div>
	            </td>
	        </tr>
...
{
	echo load_html_head_contents("receive Search","../../", 1, 1, $unicode);
	extract($_REQUEST);
 	?>
	<script>
	function set_checkvalue(){
	...
	function js_set_value(recv_id,order_no,within_group)
	{
		//alert(within_group);
		document.getElementById('selected_booking').value=recv_id;
		document.getElementById('selected_job').value=order_no;
		document.getElementById('within_group').value=within_group;
		parent.emailwindow.hide();
	}
        ...
        <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            <table width="100%" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                ...
                <tr class="general">
                    ...
                    <td><input name="txt_date_from" id="txt_date_from" class="datepicker" style="width:60px"></td>
                    <td><input name="txt_date_to" id="txt_date_to" class="datepicker" style="width:60px"></td>
                    <td><input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_booking_prifix').value+'_'+document.getElementById('cbo_search_category').value+'_'+document.getElementById('cbo_type').value, 'create_recv_search_list_view', 'search_div', 'fabric_receive_for_aop_entry_controller','setFilterGrid(\'tbl_po_list\',-1)')" style="width:80px;" /></td>
                </tr>
                <tr>
                    <td align="center" valign="middle" colspan="11"><? echo load_month_buttons(1); ?></td>
                </tr>
...
if ($action=="create_recv_search_list_view")
{
	//echo load_html_head_contents("Booking PopUp","../../", 1, 1, $unicode,'','');
	$data=explode('_',$data);
	...
	/*$sql= "select f.id,f.company_id, f.buyer_id,a.job_no,c.style_ref_no,c.job_no as fso_number, c.within_group, c.sales_booking_no, e.batch_no, e.extention_no, b.color_id, b.body_part_id , b.fabric_desc, b.gsm_weight, b.dia, e.batch_weight,b.id as so_dtls_id,e.id as batch_id,g.id as dtls_id, g.process_type_id, g.quantity, g.rate, g.amount, g.number_of_roll, g.remark, h.sys_no,h.order_no,h.id as recv_id,h.supplier_id   from 
		wo_pre_cost_fabric_cost_dtls a, fabric_sales_order_dtls b, fabric_sales_order_mst c, pro_batch_create_dtls d , pro_batch_create_mst e, wo_booking_mst f, wo_fabric_aop_dtls g, wo_fabric_aop_mst h where h.id=g.mst_id and f.buyer_id=h.buyer_id and a.id=b.pre_cost_fabric_cost_dtls_id and b.mst_id=c.id and c.id=d.po_id and d.mst_id=e.id and f.tagged_booking_no = c.sales_booking_no and g.sales_order_dtls_id=b.id and f.id=g.order_id and g.batch_id=e.id and e.color_id=b.color_id and f.booking_type=3 and f.process=35 and h.entry_form=467 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1 and g.status_active=1 and g.is_deleted=0 $company $buyer $sys_date $search_cond group by  f.id,f.company_id, f.buyer_id,a.job_no,c.style_ref_no,c.job_no, c.within_group, c.sales_booking_no, e.batch_no, e.extention_no, b.color_id, b.body_part_id , b.fabric_desc, b.gsm_weight, b.dia, e.batch_weight,b.id,e.id,g.id, g.process_type_id, g.quantity, g.rate, g.amount, g.number_of_roll, g.remark, h.sys_no,h.order_no,h.id,h.supplier_id order by g.id desc";*/

		/*$sql= "select c.company_id, g.quantity , h.sys_no,h.order_no,h.id as recv_id,h.supplier_id,h.within_group from 
		 fabric_sales_order_dtls b, fabric_sales_order_mst c, pro_batch_create_dtls d , pro_batch_create_mst e, wo_fabric_aop_dtls g, wo_fabric_aop_mst h where h.id=g.mst_id and b.mst_id=c.id and c.id=d.po_id and d.mst_id=e.id and g.sales_order_dtls_id=b.id and g.batch_id=e.id and e.color_id=b.color_id and h.entry_form=467 and b.body_part_id= d.body_part_id and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1 and g.status_active=1 and g.is_deleted=0 $company $buyer $sys_date $search_cond group by  c.company_id, h.sys_no,h.order_no,h.id,h.supplier_id,h.within_group, g.quantity   order by h.id desc";*/

	$sql= "select c.company_id as company_id,g.quantity,h.sys_no,h.order_no,h.id as issue_id,h.supplier_id,h.within_group,h.id as recv_id from 
	fabric_sales_order_mst c, pro_batch_create_dtls d , pro_batch_create_mst e, wo_fabric_aop_dtls g, wo_fabric_aop_mst h where h.id=g.mst_id and  c.id=d.po_id and d.mst_id=e.id and g.batch_id=e.id and h.entry_form=467 and c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1 and g.status_active=1 and g.is_deleted=0 $company $buyer $sys_date $search_cond group by  c.company_id,h.sys_no,h.order_no,h.id,h.supplier_id,h.within_group,g.quantity,h.id order by h.id desc";
	//echo $sql;
	$result=sql_select($sql);

	foreach( $result as $row){
		$receive_arr[$row[csf("company_id")]][$row[csf("recv_id")]][$row[csf("sys_no")]][$row[csf("supplier_id")]]['quantity']+=$row[csf("quantity")];
		$receive_arr[$row[csf("company_id")]][$row[csf("recv_id")]][$row[csf("sys_no")]][$row[csf("supplier_id")]]['order_no']=$row[csf("order_no")];
		$receive_arr[$row[csf("company_id")]][$row[csf("recv_id")]][$row[csf("sys_no")]][$row[csf("supplier_id")]]['within_group']=$row[csf("within_group")];
	}
	?>
	<div style="width:500px;">
     	<table cellspacing="0" cellpadding="0" border="1" align="left" rules="all" width="480" class="rpt_table">
            <thead>
                <th width="30">SL</th>
                <th width="130">Company Name</th>
                <th width="130">Receive  No</th>
                <th width="120">Supplier</th>
                <th>Receive qty.</th>
            </thead>
     	</table>
    </div>
    <div style="width:500px; max-height:240px;overflow-y:scroll;">
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="480" class="rpt_table" id="tbl_po_list">
	    <?
	    $i=1;
	    foreach($receive_arr as $company_id=>$company_id_data)
	    {
	    	foreach($company_id_data as $recv_id=>$issue_id_data)
		    {
		    	foreach($issue_id_data as $sys_no=>$sys_no_data)
			    {
				    foreach($sys_no_data as $supplier_id=>$row)
				    {
						if ($i%2==0)$bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
				     	//$booking_no="'".$row[csf("booking_no")]."'";
				     	?>
				     	<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value('<? echo $recv_id;?>' , '<? echo $row['order_no'];?>', '<? echo $row['within_group'];?>')" >
							<td width="30" align="center"><?php echo $i; ?></td>
							<td width="130" style="word-break:break-all"><?php echo $comp[$company_id]; ?></td>
							<td width="130" style="word-break:break-all"><?php echo $sys_no; ?></td>
							<td width="120" style="word-break:break-all"><?php echo $supplier_name_arr[$supplier_id]; ?></td>
							<td style="word-break:break-all"><?php echo number_format($row['quantity'],2) ?></td>
						</tr>
			     	<?
			     	$i++;
			     	}
...
if($action=="issue_print")
{
	...
	$sql= "select f.id,f.company_id, f.buyer_id,a.job_no,c.style_ref_no,c.job_no as fso_number, c.within_group, c.sales_booking_no, e.batch_no, e.extention_no, b.color_id, b.body_part_id , b.fabric_desc, b.gsm_weight, b.dia, e.batch_weight,b.id as so_dtls_id,e.id as batch_id,g.id as dtls_id, g.process_type_id, g.quantity, g.rate, g.amount, g.number_of_roll, g.remark  from wo_pre_cost_fabric_cost_dtls a, fabric_sales_order_dtls b, fabric_sales_order_mst c, pro_batch_create_dtls d , pro_batch_create_mst e, wo_booking_mst f, wo_fabric_aop_dtls g where a.id=b.pre_cost_fabric_cost_dtls_id and b.mst_id=c.id and c.id=d.po_id and d.mst_id=e.id and f.tagged_booking_no = c.sales_booking_no and g.sales_order_dtls_id=b.id and f.id=g.order_id and g.batch_id=e.id and e.color_id=b.color_id and b.body_part_id= d.body_part_id and f.booking_type=3 and f.process=35 and g.mst_id=$data[1] and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1 and g.status_active=1 and g.is_deleted=0 group by  f.id,f.company_id, f.buyer_id,a.job_no,c.style_ref_no,c.job_no, c.within_group, c.sales_booking_no, e.batch_no, e.extention_no, b.color_id, b.body_part_id , b.fabric_desc, b.gsm_weight, b.dia, e.batch_weight,b.id,e.id,g.id, g.process_type_id, g.quantity, g.rate, g.amount, g.number_of_roll, g.remark order by g.id desc";
	...
	<div style="width:1400px;">
		...
		<div style="width:100%;">
			<table align="left" cellspacing="0" width="1400"  border="1" rules="all" class="rpt_table"  >
				...
				<tbody>
				<?
				$tblRow=1; $i=1; $tot_quantity=$tot_number_of_roll=0;
				...
				<tfoot>
					<tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $tblRow; ?>" align="center">
		                ...
		                <td width="80" align="right"><?  echo number_format($tot_quantity); ?></td>
...
if ($action=="populate_receive_data_from_search_popup")
{
	//$data=explode('_',$data);
	
	$sql= "select a.id, a.entry_form, a.sys_no_prefix, a.sys_no_prefix_num, a.sys_no, a.sys_date, a.delivery_date, a.company_id, a.buyer_id, a.issue_purpose, a.source, a.order_id, a.order_no, a.attention, a.vehical_no, a.driver_name, a.dl_no, a.transport, a.mobile_no, a.gate_pass_no, a.is_short, a.remark,a.supplier_id,a.supplier_address,within_group,rcv_challan_no,issue_id, currency_id from wo_fabric_aop_mst a where a.id=$data and a.is_deleted=0 ";

	$data_array=sql_select($sql);
	foreach ($data_array as $row){
		echo "document.getElementById('txt_system_id').value 	 	= '".$row[csf("sys_no")]."';\n";
		echo "document.getElementById('txt_receive_date').value 	= '".change_date_format($row[csf("sys_date")])."';\n";
		echo "document.getElementById('cbo_company_name').value 	= '".$row[csf("company_id")]."';\n";
		echo "document.getElementById('buyer_id').value 		 	= '".$row[csf("buyer_id")]."';\n";
		echo "document.getElementById('cbo_supplier_name').value 	= '".$row[csf("supplier_id")]."';\n";
		...
		echo "document.getElementById('txt_rcv_challan_no').value 	= '".$row[csf("rcv_challan_no")]."';\n";
		echo "document.getElementById('cbo_source').value 		 	= '".$row[csf("source")]."';\n";
		echo "document.getElementById('hid_issue_id').value 		= '".$row[csf("issue_id")]."';\n";
		echo "document.getElementById('cbo_is_short').value 		= '".$row[csf("is_short")]."';\n";
		...
		echo " $('#cbo_supplier_name').attr('disabled',true);\n";
		echo " $('#txt_party_location').attr('disabled',true);\n";
		echo " $('#cbo_is_short').attr('disabled',true);\n";
...
if ($action=="populate_issue_data_from_search_popup")
{
	...
	$issue_ids=implode(",",array_unique(explode(",",chop($data[0],","))));
	$entry_form=implode(",",array_unique(explode(",",chop($data[1],","))));
	//var_dump($entry_form);
	if($entry_form==462)
	{
		$IssueSql= "SELECT a.id, a.entry_form, a.sys_no_prefix, a.sys_no_prefix_num, a.sys_no, a.company_id, a.buyer_id, a.issue_purpose, a.source, a.order_id, a.vehical_no, a.driver_name, a.dl_no, a.transport, a.mobile_no, a.supplier_id,a.supplier_address from wo_fabric_aop_mst a where a.id in ($issue_ids) and a.is_deleted=0 ";
	}else if($entry_form==522){
		$IssueSql= "SELECT a.id, a.entry_form, a.sys_number_prefix as sys_no_prefix, a.sys_number_prefix_num as sys_no_prefix_num, a.sys_number as sys_no, a.company_id, null as buyer_id,  a.issue_purpose, '0' as source, b.fso_id as order_id, a.vehicle_no as vehical_no, a.driver_name, a.dl_no, a.transport, a.mobile_no,  a.delivery_to as supplier_id,a.delivery_address as supplier_address from aop_multi_issue_mst a, aop_multi_issue_dtls b  where a.id=b.mst_id and a.id in ($issue_ids) and a.is_deleted=0 ";
	}
	
	
	$data_array=sql_select($IssueSql);
	 ...
	 foreach ($data_array as $row){
		...
		//echo "document.getElementById('txt_system_id').value 	 	= '".$row[csf("sys_no")]."';\n";
		echo "document.getElementById('cbo_company_name').value 	= '".$row[csf("company_id")]."';\n";
		echo "document.getElementById('txt_order_no').value 		= '".$row[csf("sys_no")]."';\n";
		echo "document.getElementById('hid_issue_id').value 		= '".$row[csf("id")]."';\n";
		echo "document.getElementById('hid_order_id').value 		= '".$row[csf("order_id")]."';\n";
		echo "document.getElementById('buyer_id').value 		 	= '".$row[csf("buyer_id")]."';\n";
		echo "document.getElementById('cbo_source').value 		 	= '".$row[csf("source")]."';\n";
		echo "document.getElementById('cbo_supplier_name').value 	= '".$row[csf("supplier_id")]."';\n";
