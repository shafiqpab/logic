...
if($action=="create_rescan_bundle_search_list_view")
{
	...
	$sql=" select c.cut_no, c.bundle_no, d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, 
 sum(c.reject_qty-c.replace_qty) as qty, e.po_number,de.issue_challan_id from pro_gmts_delivery_mst de, pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e ,wo_po_details_master f where d.job_no_mst=f.job_no and de.id=a.delivery_mst_id and de.id=c.delivery_mst_id and  a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and a.company_id=$company and e.po_number like '$txt_order_no' and c.bundle_no like '$bundle_no' $jobCon $cutCon and c.production_type=3 and a.embel_name=1 and a.status_active=1 and a.is_deleted=0  group by c.cut_no, c.bundle_no, d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,e.po_number,de.issue_challan_id having sum(c.reject_qty-c.replace_qty)>0 order by c.cut_no, c.bundle_no desc";
					...
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value(<? echo $i; ?>)"> 
						<td width="40">
							<? echo $i; ?>
							 ...
							 <input type="hidden" name="txt_challan_id" id="txt_challan_id<?php echo $i; ?>" value="<?php echo $row[csf('issue_challan_id')]; ?>"/>
						</td>
...
if($action=="create_bundle_search_list_view")
{
	...
	$color_arr=return_library_array( "select id, color_name from lib_color", "id", "color_name");
	$country_arr=return_library_array( "select id, country_name from lib_country",'id','country_name');
	$scanned_bundle_sql=sql_select( "select a.issue_challan_id, b.bundle_no from pro_gmts_delivery_mst a,pro_garments_production_dtls b where a.id=b.delivery_mst_id and b.production_type=3 and a.embel_name=1 and b.status_active=1 and b.is_deleted=0");
	
	foreach($scanned_bundle_sql as $val)
	{
		$scanned_bundle_arr[$val['issue_challan_id']][$val['bundle_no']]=$val['bundle_no'];
	}

	
	foreach(explode(",",$selectedBuldle) as $bn)
	{
		$scanned_bundle_arr[$previous_issue_id][$bn]=$bn;	
	}
	...
	$sql="select c.cut_no, c.bundle_no, d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.bundle_no, c.production_qnty as qty, e.po_number,c.delivery_mst_id from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e ,wo_po_details_master f where d.job_no_mst=f.job_no and  a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and a.company_id=$company and e.po_number like '$txt_order_no' and c.bundle_no like '$bundle_no' $jobCon $cutCon and c.production_type=2 and a.embel_name=1 and a.status_active=1 and a.is_deleted=0  order by c.cut_no, c.bundle_no DESC";
	...
	<div style="width:850px; max-height:230px; overflow-y:scroll" id="list_container_batch" align="left">	 
        ...
        <?
            ...
            foreach ($result as $row)
            {  
				if($scanned_bundle_arr[$row[csf('delivery_mst_id')]][$row[csf('bundle_no')]]=="")
				{
					if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
					list($shortName,$year,$job)=explode('-',$row[csf('job_no_mst')]);	
				?>
...
if($action=="challan_duplicate_check")
{
	...
	$msg=1;
	$result=sql_select("select a.sys_number,b.bundle_no,a.issue_challan_id from pro_gmts_delivery_mst a,pro_garments_production_dtls b where a.id=b.delivery_mst_id and b.bundle_no in ($bundle_no) and b.production_type=3 and a.embel_name=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
	 group by a.sys_number, b.bundle_no,a.issue_challan_id");
	...
	$issue_result=sql_select("select a.id,a.sys_number,b.bundle_no from pro_gmts_delivery_mst a,pro_garments_production_dtls b where a.id=b.delivery_mst_id and b.bundle_no in ($bundle_no) and b.production_type=2 and a.embel_name=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $issue_challan_cond group by a.sys_number, b.bundle_no,a.id");
...
if($action=="challan_duplicate_check_resacn")
{
	...
	if(count($result)>0)
	{
		...
		{ 
			$msg=2;
			if($datastr!="") $datastr.=",";
			$datastr.=$row[csf('issue_challan_id')]."*".$row[csf('sys_number')]."*".$row[csf('bundle_no')];
		}
	}
...
if($action=="populate_bundle_data_rescan")
{
	...
	else if($db_type==2) 
	{
	 ...
	 $issue_sql="select c.bundle_no, c.production_qnty as qty from pro_garments_production_mst a,pro_garments_production_dtls c where a.id=c.mst_id  and c.bundle_no in ($bundle_nos) and c.production_type=2 and a.embel_name=1 and c.status_active=1 and c.is_deleted=0  and c.delivery_mst_id=".$mst_id."";
	 //echo $issue_sql;die;
	...
	$sql="select d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, MAX($year_field) as year, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no,  sum(c.production_qnty) as pre_qty, e.po_number from pro_gmts_delivery_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name=$ex_data[3] and a.id=c.delivery_mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.bundle_no in ($bundle_nos) and c.production_type=3 and a.embel_name=1 and c.status_active=1 and c.is_deleted=0 and a.issue_challan_id=".$mst_id." group by d.id, e.id, f.job_no_prefix_num, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no, e.po_number";
	...
	foreach ($result as $row)
	{ 
		$production_qty=$issue_qty_arr[$row[csf('bundle_no')]]-$row[csf('pre_qty')];
		if($production_qty>0)
		{
			if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
		?>
...
if($action=="populate_bundle_data")
{
	...
	$mst_id=$ex_data[2];
	$bundle_nos="'".trim(implode("','",$bundle))."'";
	$scanned_bundle_arr=return_library_array( "select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=3 and a.embel_name=1 and b.status_active=1 and b.is_deleted=0",'bundle_no','bundle_no');
	$size_arr=return_library_array( "select id, size_name from lib_size",'id','size_name');
	$color_arr=return_library_array( "select id, color_name from lib_color", "id", "color_name");
	$country_arr=return_library_array( "select id, country_name from lib_country",'id','country_name');
	$buyer_arr=return_library_array( "select id, short_name from lib_buyer",'id','short_name');
	...
	{ 
		...
		?>
			<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i;?>"> 
                ...
                <td width="50" id="RepQty_<? echo $i; ?>" align="right"><input type="text" class="text_boxes_numeric" name="replaceQty[]" id="replaceQty_<? echo $i; ?>" style="width:40px" onBlur="calculate_qcpasss_rescan(<? echo $i; ?>)" disabled/></td>
...
if($action=="populate_bundle_data_update")
{
	...
	$issue_sql="select c.bundle_no, c.production_qnty as qty from pro_garments_production_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.bundle_no in ($bundle_nos) and c.production_type=2 and a.embel_name=1 and c.status_active=1 and c.is_deleted=0";
	$issue_result = sql_select($issue_sql);
	foreach ($issue_result as $row){
		$issue_qty_arr[$row[csf('bundle_no')]]=$row[csf('qty')];	
	}
	
	$issue_sql="select c.bundle_no, sum(c.production_qnty) as qty from pro_garments_production_mst a,pro_garments_production_dtls c where a.id=c.mst_id  and c.bundle_no in ($bundle_nos) and c.production_type=3 and a.embel_name=1 and c.status_active=1 and c.is_deleted=0 group by c.bundle_no";
	$issue_result = sql_select($issue_sql);
	foreach ($issue_result as $row){
		$pre_receive_qty_arr[$row[csf('bundle_no')]]=$row[csf('qty')];	
	}
	...
	$sql="select c.bundle_no,d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, $year_field as year, f.buyer_name, d.item_number_id, d.country_id,c.reject_qty, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no, c.production_qnty as qty,c.replace_qty, e.po_number from pro_garments_production_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.bundle_no in ($bundle_nos) and c.production_type=3 and a.embel_name=1 and c.status_active=1 and c.is_deleted=0 and c.delivery_mst_id=".$mst_id." order by c.bundle_no DESC";
	  // echo $sql; die;
	$result = sql_select($sql);
	$count=count($result);
	$i=$ex_data[1]+$count;
	...
	{ 
			...
			{
				 $production_qty=($issue_qty_arr[$row[csf('bundle_no')]]-$pre_receive_qty_arr[$row[csf('bundle_no')]]+$row[csf("replace_qty")]);
				 $disble_rej="disabled";
				 $disable_replace="";
			}
			else
			{
				$production_qty=$issue_qty_arr[$row[csf('bundle_no')]];
				$disble_rej="";
		...
		?>
			<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i;?>"> 
				...
				?>
                &nbsp;</td>
                <td width="50" id="RejQty_<? echo $i; ?>" align="right"><input type="text" class="text_boxes_numeric" name="rejectQty[]" id="rejectQty_<? echo $i; ?>" style="width:40px" value="<? echo $row[csf('reject_qty')];?>" onBlur="calculate_qcpasss(<? echo $i; ?>)" <?php echo $disble_rej; ?>/></td>
...
if($action=="bundle_nos_update")
{
	...
	if($db_type==0) 
	{
		$bundle_nos=return_field_value("group_concat(b.bundle_no order by b.id desc) as bundle_no", "pro_garments_production_mst a, pro_garments_production_dtls b","a.id=b.mst_id and a.delivery_mst_id=$data and b.production_type=3 and a.embel_name=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0",'bundle_no');
	}
	else if($db_type==2) 
	{
		$bundle_nos=return_field_value("LISTAGG(b.bundle_no, ',') WITHIN GROUP (ORDER BY b.id desc) as bundle_no", "pro_garments_production_mst a, pro_garments_production_dtls b","a.id=b.mst_id and a.delivery_mst_id=$data and b.production_type=3 and a.embel_name=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0",'bundle_no');
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$txt_system_id)=="")
		{
			...
			$new_sys_number=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'BRP', date("Y",time()), 5, "select sys_number_prefix, sys_number_prefix_num from pro_gmts_delivery_mst where company_id=$cbo_company_name and production_type='3' and embel_name=$cbo_embel_name and $year_cond=".date('Y',time())." order by id desc ", "sys_number_prefix", "sys_number_prefix_num" ));
		...
		if(str_replace("'","",$delivery_basis)==3)
		{
			...
			$field_array_dtls="id,delivery_mst_id,mst_id,production_type,color_size_break_down_id,production_qnty,reject_qty,replace_qty,cut_no,bundle_no";
			...
			foreach($dtlsArr as $bundle_no=>$qty)
			{
				...
				$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",3,'".$dtlsArrColorSize[$bundle_no]."','".$qty."','".$dtlsArrRej[$bundle_no]."','".$dtlsArrReplace[$bundle_no]."','".$cut_no."','".$bundle_no."')";
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		if(str_replace("'","",$delivery_basis)==3)
		{
			...
			//$delete_bundle = execute_query("DELETE FROM pro_cut_delivery_color_dtls WHERE delivery_mst_id=$txt_system_id and production_type=3 and embel_name=1");
			$challanrID=sql_update("pro_gmts_delivery_mst",$field_array_delivery,$data_array_delivery,"id",$txt_system_id,1);
			$rID=sql_insert("pro_garments_production_mst",$field_array_mst,$data_array_mst,1);
			$dtlsrID=sql_insert("pro_garments_production_dtls",$field_array_dtls,$data_array_dtls,1);
			//$bundlerID=sql_insert("pro_cut_delivery_color_dtls",$field_array_bundle,$data_array_bundle,1);
		...
		<legend>Enter search words</legend>           
            <table cellpadding="0" cellspacing="0" width="700" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
            		...
            		<td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_embel_type').value+'_'+document.getElementById('txt_company_id').value, 'create_challan_search_list_view', 'search_div', 'print_embro_bundle_receive_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                     </td>
...
<div style="width:900px;">
    <table width="900" cellspacing="0" align="left">
        ...
        <tr>
        	<td><strong>Floor </strong></td><td>: <? echo $floor_library[$dataArray[0][csf('floor_id')]]; ?></td>
        	<td><strong>Organic </strong></td><td>: <? echo $dataArray[0][csf('organic')]; ?></td>
            <td><strong>Receive Date </strong></td><td>: <? echo change_date_format($dataArray[0][csf('delivery_date')]); ?></td>
        </tr>
       <tr>
...
if($action=="load_mst_data_bundle_scan")
{
	//echo $data;die;
 	$ex_data = explode("_",$data);
	$bundle_no = implode("','",explode(",",$ex_data[0]));
	//$bundle_no=$ex_data[0];
	...
	a.production_source
	from  pro_gmts_delivery_mst a,pro_garments_production_dtls c where   a.id=c.delivery_mst_id and c.bundle_no in ('$bundle_no')  and c.production_type=2
	and a.embel_name=1 and a.status_active=1 and a.is_deleted=0  and c.status_active=1 and c.is_deleted=0 and a.id=".$challan_id." 
...
if($action=="load_mst_data")
{
	...
	and a.embel_name=1 and a.status_active=1 and a.is_deleted=0  and c.status_active=1 and c.is_deleted=0
	group by a.sys_number, a.company_id,a.location_id, a.embel_name, a.embel_type, a.serving_company,a.floor_id,a.organic,a.production_source");
	//print_r($sql_mst_data);die;
	if(count($sql_mst_data)>0)
	{
