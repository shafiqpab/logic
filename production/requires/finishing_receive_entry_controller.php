...
if ($action=="load_variable_settings")
{
	...
	$sql_result = sql_select("select bulletin_type, production_entry from variable_settings_production where company_name=$lcCompany and variable_list=1 and status_active=1");
 	foreach($sql_result as $result)
	{
		echo "$('#wash_production_variable').val(".$result[csf("bulletin_type")].");\n";
	...
	$sql_result_rej = sql_select("select bulletin_type from variable_settings_production where company_name=$working_company and variable_list=28 and status_active=1");
 	foreach($sql_result_rej as $result)
	{
		echo "$('#wash_production_variable_rej').val(".$result[csf("bulletin_type")].");\n";
	}
	...
	$variable_is_control=return_field_value("is_control","variable_settings_production","company_name=$working_company and variable_list=33 and page_category_id=312","is_control");
...
{
	echo create_drop_down( "cbo_location", 150, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id='$data' order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down( 'requires/finishing_receive_entry_controller', this.value, 'load_drop_down_floor', 'floor_td'); load_drop_down( 'requires/finishing_receive_entry_controller', document.getElementById('cbo_location').value+'_'+document.getElementById('prod_reso_allo').value+'_'+document.getElementById('txt_wash_date').value, 'load_drop_down_wash_output_line', 'wash_line_td'); get_php_form_data(document.getElementById('cbo_source').value, 'line_disable_enable','requires/finishing_receive_entry_controller');");
	exit();
}

if ($action=="load_drop_down_floor")
{
	echo create_drop_down( "cbo_floor", 150, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process in (4) order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/finishing_receive_entry_controller', this.value+'_'+document.getElementById('cbo_location').value+'_'+document.getElementById('prod_reso_allo').value+'_'+document.getElementById('txt_wash_date').value, 'load_drop_down_wash_line_floor', 'wash_line_td' );",0 );  
	exit();   	 
}
...
if($action=="load_drop_down_wash_output")
{
	...
	{
 		echo create_drop_down( "cbo_wash_company", 150, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1 $company_cond order by company_name","id,company_name", 1, "--- Select ---", "",  "fnc_loadvariable(this.value); load_drop_down( 'requires/finishing_receive_entry_controller', this.value, 'load_drop_down_location', 'location_td' ); fnc_company_check(document.getElementById('cbo_source').value);",0,0 ); 
     ...
     <div style="width:1270px;">
     	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="100%" class="rpt_table">
            <thead>
                ...
                <th width="80">Total Fin. Rcv. Qty.</th>
...
if($action=="populate_data_from_search_popup")
{
	...
	else if($dataArr[3]==103) $qty_source=11;//wash Entry
  	...
  	foreach($res as $result)
	{
		...
		echo "$('#txt_style_no').val('".$result[csf('style_ref_no')]."');\n";
   		...
   		if($qty_source!=0)
   		{
   			$dataArray=sql_select("select SUM(CASE WHEN a.production_type='$qty_source'  THEN a.production_quantity END) as totalinput,SUM(CASE WHEN a.production_type=85  THEN a.production_quantity ELSE 0 END) as total_fin from pro_garments_production_mst a WHERE  a.po_break_down_id=".$result[csf('id')]."  and a.item_number_id='$item_id' and a.country_id='$country_id' $pack_type_cond and a.status_active=1 and a.is_deleted=0");
			// echo $dataArray;die;
	 		foreach($dataArray as $row)
			{  
				echo "$('#txt_input_quantity').val('".$row[csf('totalinput')]."');\n";
				echo "$('#txt_cumul_wash_qty').val('".$row[csf('total_fin')]."');\n";
				$yet_to_produced = $row[csf('totalinput')]-$row[csf('total_fin')];
...
if($action=="color_and_size_level")
{
	...
	if(!$variableSettings)$variableSettings=2;
	...
	else if($dataArr[6]==103) $qty_source=11;//wash Entry	
	else if($dataArr[6]==312) $qty_source=85;//Finishing Receive Entry 	
	...
	if($qty_source !=0)
	{
	 ...
	 if( $variableSettings==2 ) // color level
	 {
				...
				$sql = "SELECT a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty,
						...
						sum(CASE WHEN c.production_type=85  then b.production_qnty ELSE 0 END) as cur_production_qnty,
						sum(CASE WHEN c.production_type=85  then b.reject_qty ELSE 0 END) as reject_qty
	...
	else // if preceding process =0 in variable setting then plan cut quantity will show
	{
		if( $variableSettings==2 ) // color level
		{
			...
			else
			{
				...
				from wo_po_color_size_breakdown a left join pro_garments_production_dtls b on a.id=b.color_size_break_down_id and b.production_type=5
		...
		else // by default color and size level
		{
			...
			$dtlsData = sql_select("select a.color_size_break_down_id,
										sum(CASE WHEN a.production_type=5 then a.production_qnty ELSE 0 END) as production_qnty
										from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in(5) group by a.color_size_break_down_id");
...
?>	 
	...
	<div style="width:100%;max-height:180px; overflow:y-scroll" id="wash_production_list_view" align="left">
		...
		<?php  
  		...
  		?>
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data('<? echo $selectResult[csf('id')].'**'.$variable_settings; ?>','populate_input_form_data','requires/finishing_receive_entry_controller');" > 
...
?>	
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="400" class="rpt_table">
		...
		<? 
		$issue_qnty_arr=sql_select("select a.po_break_down_id, a.item_number_id, a.country_id, a.pack_type, a.production_quantity as cutting_qnty from pro_garments_production_mst a where  a.po_break_down_id='$data' and a.production_type=85  and a.status_active=1 and a.is_deleted=0 ");
...
if($action=="populate_input_form_data")
{
	...
	else
	{
		...
		{
			$sql_dtls ="SELECT a.id,a.company_id, a.garments_nature, a.po_break_down_id, a.challan_no, a.item_number_id, a.country_id, a.pack_type, a.production_source, a.produced_by, a.serving_company, a.sewing_line, a.location, a.production_date, sum(a.production_quantity) as production_quantity,  a.production_type, a.entry_break_down_type, a.break_down_type_rej, TIME_FORMAT( a.production_hour, '%H:%i' ) as production_hour, a.supervisor, a.carton_qty, a.remarks, a.floor_id, a.reject_qnty, a.alter_qnty, a.total_produced, a.yet_to_produced, a.spot_qnty, a.wo_order_id,a.currency_id, a.exchange_rate, a.rate from pro_garments_production_mst a where  a.id='$id' and a.production_type='85'  and a.status_active=1 and a.is_deleted=0 group by a.id,a.company_id, a.garments_nature, a.po_break_down_id, a.challan_no, a.item_number_id, a.country_id, a.pack_type, a.production_source, a.produced_by, a.serving_company, a.sewing_line, a.location, a.embel_type, a.production_date,  a.production_type, a.entry_break_down_type, a.break_down_type_rej, a.production_hour, a.supervisor, a.carton_qty, a.remarks, a.floor_id, a.reject_qnty, a.alter_qnty, a.total_produced, a.yet_to_produced, a.spot_qnty, a.wo_order_id,a.currency_id, a.exchange_rate, a.rate   order by a.id";
		}
	...
	$control_and_preceding=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=33 and page_category_id=312 and company_name='$company_id'");  
	...
	/*if($preceding_process==28) $qty_source=4; //Sewing Input
		...
		{ 
			...
			echo "load_drop_down( 'requires/finishing_receive_entry_controller', ".$result[csf('production_source')].", 'load_drop_down_wash_output', 'sew_company_td' );\n";
			echo "$('#cbo_wash_company').val('".$result[csf('serving_company')]."');\n";
			echo "load_drop_down( 'requires/finishing_receive_entry_controller',".$result[csf('serving_company')].", 'load_drop_down_location', 'location_td' );";

			echo "$('#cbo_location').val('".$result[csf('location')]."');\n";
			echo "load_drop_down( 'requires/finishing_receive_entry_controller', ".$result[csf('location')].", 'load_drop_down_floor', 'floor_td' );\n";
			echo "$('#cbo_floor').val('".$result[csf('floor_id')]."');\n";
			
			echo "load_drop_down( 'requires/finishing_receive_entry_controller', document.getElementById('cbo_floor').value+'_'+document.getElementById('cbo_location').value+'_'+document.getElementById('prod_reso_allo').value+'_'+document.getElementById('txt_wash_date').value, 'load_drop_down_wash_line_floor', 'wash_line_td' );\n";
			
			echo "$('#cbo_wash_line').val('".$result[csf('sewing_line')]."');\n";
			echo "get_php_form_data(".$result[csf('production_source')].",'line_disable_enable','requires/finishing_receive_entry_controller');\n";
			...
			if($result[csf('production_source')]==3)
			{
				...
				$preceding_process= $control_and_preceding[0][csf("preceding_page_id")];
				echo "$('#hidden_variable_cntl').val('".$control_and_preceding[0][csf('is_control')]."');\n";
 	 			echo "load_drop_down( 'requires/finishing_receive_entry_controller', '".$result[csf('company_id')]."_".$result[csf('serving_company')]."_".$result[csf('po_break_down_id')]."', 'load_drop_down_workorder', 'workorder_td' );\n";
			...
			if($qty_source==0)
			{
				...
				{ 
					...
					else if( $variableSettings==3 ) //color and size level
					{
							...
							$dtlsData = sql_select("select a.color_size_break_down_id,
												sum(CASE WHEN a.production_type=85 then a.production_qnty ELSE 0 END) as production_qnty,
												sum(CASE WHEN a.production_type=85 and then a.reject_qty ELSE 0 END) as reject_qty
...
if ($action=="save_update_delete")
{
    ...
    $control_and_preceding=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=33 and page_category_id=312 and company_name='$cbo_company_name'");  
    $preceding_process=$control_and_preceding[0][csf("preceding_page_id")];
	/*$qty_source=0;
	if($preceding_process==28)  $qty_source=4; //Sewing Input
	else if($preceding_process==29) $qty_source=5;//Sewing Output
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$field_array1="id, garments_nature, company_id, challan_no, po_break_down_id, item_number_id, country_id, pack_type, production_source, serving_company, location, produced_by, production_date, production_quantity, production_type,entry_form, entry_break_down_type, break_down_type_rej, sewing_line, supervisor, production_hour, remarks, floor_id, alter_qnty, reject_qnty, total_produced, yet_to_produced, prod_reso_allo, spot_qnty,wo_order_id,currency_id, exchange_rate, rate, amount, inserted_by, insert_date";
		...
		if($rate!="") {  $amount=$rate*str_replace("'","",$txt_wash_qty);}
			...
			$data_array1="INSERT INTO pro_garments_production_mst (".$field_array1.") VALUES (".$id.",".$garments_nature.",".$cbo_company_name.",".$txt_challan.",".$hidden_po_break_down_id.",".$cbo_item_name.",".$cbo_country_name.",".$txt_pack_type.",".$cbo_source.",".$cbo_wash_company.",".$cbo_location.",".$cbo_produced_by.",".$txt_wash_date.",".$txt_wash_qty.",85,627,".$wash_production_variable.",".$wash_production_variable_rej.",".$cbo_wash_line.",".$txt_super_visor.",".$txt_reporting_hour.",".$txt_remark.",".$cbo_floor.",".$txt_alter_qnty.",".$txt_reject_qnty.",".$txt_cumul_wash_qty.",".$txt_yet_to_wash.",".$prod_reso_allo.",".$txt_spot_qnty.",".$cbo_work_order.",".$hidden_currency_id.",".$hidden_exchange_rate.",'".$rate."','".$amount."',".$user_id.",'".$pc_date_time."')";
		
		//echo "10**INSERT INTO pro_garments_production_mst ($data_array1)"; die;
		...
		$dtlsData = sql_select("select a.color_size_break_down_id,
										...
										where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name and b.country_id=$cbo_country_name and a.color_size_break_down_id!=0 and a.production_type in($qty_source) $pack_type_cond
		...
		{		
			...
			foreach($rowEx as $rowE=>$val)
			{
				...
				if($j==0)$data_array = "(".$dtls_id.",".$id.",85,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."')";
				else $data_array .= ",(".$dtls_id.",".$id.",85,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."')";
				//$dtls_id=$dtls_id+1;							
 				$j++;								
			}
 		}//color level wise
		...
		//	$dtls_id=return_next_id("id", "pro_garments_production_dtls", 1);
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq", "pro_garments_production_dtls", $con );
				if($j==0)$data_array = "(".$dtls_id.",".$id.",85,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."')";
				else $data_array .= ",(".$dtls_id.",".$id.",85,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."')";
				//$dtls_id=$dtls_id+1;
 				$j++;
			}
		}//color and size wise
		...
		{		 
			...
			foreach($defect_array as $key=>$val)
			{
				...
				$defectPointId=$key;
				$defect_qty=$val;
				$data_array_defect.="(".$dft_id.",".$id.",85,".$hidden_po_break_down_id.",".$defect_type_id.",".$defectPointId.",'".$defect_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			} 
		}
		...
		{		 
			...
			foreach($defectSpt_array as $keysp=>$valsp)
			{
				...
				$defectsp_qty=$valsp;
				$data_array_defectsp.="(".$dftSp_id.",".$id.",85,".$hidden_po_break_down_id.",".$defectSpot_type_id.",".$defectspPointId.",'".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			} 
		}
	...
	$sql_result=sql_select("select b.bundle_no from  pro_garments_production_mst a,pro_garments_production_dtls b where  a.id=b.mst_id and a.id =$txt_mst_id and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1");
		...
		$data_array1="".$txt_pack_type."*".$cbo_source."*".$cbo_wash_company."*".$cbo_location."*".$cbo_produced_by."*".$txt_wash_date."*".$txt_wash_qty."*85*".$wash_production_variable."*".$wash_production_variable_rej."*".$cbo_wash_line."*".$txt_super_visor."*".$txt_reporting_hour."*".$txt_challan."*".$txt_remark."*".$cbo_floor."*".$txt_reject_qnty."*".$txt_alter_qnty."*".$txt_cumul_wash_qty."*".$txt_yet_to_wash."*".$prod_reso_allo."*".$txt_spot_qnty."*".$cbo_work_order."*".$hidden_currency_id."*".$hidden_exchange_rate."*'".$rate."'*'".$amount."'*".$user_id."*'".$pc_date_time."'";
		
		//echo "10**".$field_array1.'=='.$data_array1;die;
 		// $rID=sql_update("pro_garments_production_mst",$field_array1,"id","".$txt_mst_id."",1);die;
		//
		if(str_replace("'","",$txt_pack_type)=="") $pack_type_cond=""; else $pack_type_cond=" and b.pack_type=$txt_pack_type";
		...
		if(str_replace("'","",$wash_production_variable)!=1 && str_replace("'","",$txt_mst_id)!='' ) // check is not gross level
		{
			...
			{		
				...
				foreach($rowEx as $rowE=>$val)
				{
					...
					//4 for wash Input Entry
					$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",   "pro_garments_production_dtls", $con );
					if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",85,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."')";
					else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",85,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."')";
					//$dtls_id=$dtls_id+1;							
					$j++;								
				}
			}
			...
			{		
				...
				foreach($rowEx as $rowE=>$valE)
				{
					...
					//4 for wash Input Entry
					$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq", "pro_garments_production_dtls", $con );
					if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",85,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."')";
					else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",85,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."')";
					//$dtls_id=$dtls_id+1;
					$j++;
				}
			}
		...
		{		 
			...
			foreach($defect_array as $key=>$val)
			{
				...
				$defectPointId=$key;
				$defect_qty=$val;
				$data_array_defect.="(".$dft_id.",".$txt_mst_id.",85,".$hidden_po_break_down_id.",".$defect_type_id.",".$defectPointId.",'".$defect_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			} 
		}
		if($data_array_defect!="")
		{
			...
			$query3=execute_query("DELETE FROM pro_gmts_prod_dft WHERE mst_id=$txt_mst_id and defect_type_id=1 and production_type=85");
			$defectQ=sql_insert("pro_gmts_prod_dft",$field_array_defect,$data_array_defect,1);
		}
		$defectSpot=true;
		$data_array_defectsp="";
		...
		{		 
			...
			foreach($defectSpt_array as $keysp=>$valsp)
			{
				...
				$defectspPointId=$keysp;
				$defectsp_qty=$valsp;
				$data_array_defectsp.="(".$dftSp_id.",".$txt_mst_id.",85,".$hidden_po_break_down_id.",".$defectSpot_type_id.",".$defectspPointId.",'".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			} 
		}
		...
		if($data_array_defectsp!="")
		{
			...
			$query4=execute_query("DELETE FROM pro_gmts_prod_dft WHERE mst_id=$txt_mst_id and defect_type_id=2 and production_type=85");
			$defectSpot=sql_insert("pro_gmts_prod_dft",$field_array_defectsp,$data_array_defectsp,0);
		}
		//echo $rID=sql_update("pro_garments_production_mst",$field_array1,$data_array1,"id","".$txt_mst_id."",1);
...
if($action=="wash_output_print")
{
	...
	if($db_type==2)
	{
		$sql="select id, company_id, challan_no, sewing_line, po_break_down_id, item_number_id,entry_break_down_type,break_down_type_rej, country_id, production_source, produced_by, serving_company, location,production_date, TO_CHAR(production_hour,'HH24:MI') as production_hour, production_quantity, production_type, remarks, floor_id, sewing_line, alter_qnty, reject_qnty, spot_qnty from pro_garments_production_mst where production_type=3 and id='$data[1]' and status_active=1 and is_deleted=0 ";
	}
	else
	{
		$sql="select id, company_id, challan_no, sewing_line, po_break_down_id, item_number_id, country_id, entry_break_down_type,break_down_type_rej,production_source, produced_by, serving_company, location, production_date, TIME_FORMAT( production_hour, '%H:%i' ) as production_hour, production_quantity, production_type, remarks, floor_id, sewing_line, alter_qnty, reject_qnty, spot_qnty from pro_garments_production_mst where production_type=3 and id='$data[1]' and status_active=1 and is_deleted=0 ";
	}
	//echo $sql;
...
if($action=="production_process_control")
{
    ...
    $control_and_preceding=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=33 and page_category_id=312 and company_name='$data'");  
