...
{
	$data=explode("_",$data);
	echo create_drop_down( "cbo_location", 150, "select id,location_name from lib_location where company_id='$data[0]' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "--Select--", 0, "load_room_rack_self_bin('requires/finish_fabric_receive_controller*2', 'store','store_td', $('#cbo_company_id').val(), this.value);" );
	exit();
}
...
if($action=="create_finish_search_list_view")
{
	...
	if($db_type==0)
	{
		$sql = "select a.id, a.recv_number, a.recv_number_prefix_num, a.knitting_source, a.knitting_company, a.receive_date, a.challan_no, YEAR(a.insert_date) as year, sum(b.receive_qnty) as qnty, group_concat(b.batch_id) as batch_id from inv_receive_master a, pro_finish_fabric_rcv_dtls b where a.id=b.mst_id and a.entry_form=7 and a.status_active=1 and a.is_deleted=0 and a.company_id=$company_id $search_field_cond $date_cond $batch_cond group by a.id, a.recv_number, a.recv_number_prefix_num, a.knitting_source, a.knitting_company, a.receive_date, a.challan_no, a.insert_date order by a.id DESC";
	}
	else
	{
		$sql = "select a.id, a.recv_number, a.recv_number_prefix_num, a.knitting_source, a.knitting_company, a.receive_date, a.challan_no, to_char(a.insert_date,'YYYY') as year, sum(b.receive_qnty) as qnty, LISTAGG(cast(b.batch_id as varchar2(4000)), ',') WITHIN GROUP (ORDER BY b.batch_id) as batch_id from inv_receive_master a, pro_finish_fabric_rcv_dtls b where a.id=b.mst_id and a.entry_form=7 and a.status_active=1 and a.is_deleted=0 and a.company_id=$company_id $search_field_cond $date_cond $batch_cond group by a.id, a.recv_number, a.recv_number_prefix_num, a.knitting_source, a.knitting_company, a.receive_date, a.challan_no, a.insert_date order by a.id DESC";
	}
	//echo $sql;//die;
...
if ($action=="batch_number_popup")
{
	...
	?>
	<script>
		function js_set_value(id,batch_no,booking_no,job_no,is_sales)
		{
	...
	<div style="width:920px; overflow-y:scroll; max-height:250px;" id="buyer_list_view" align="center">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="900" class="rpt_table" id="tbl_list_search" >
			...
			foreach ($nameArray as $selectResult)
			{
				...
				?>
				<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value(<? echo $selectResult[csf('id')]; ?>,'<? echo $selectResult[csf('batch_no')]; ?>','<? echo $selectResult[csf('booking_no')]; ?>','<? echo $job_no; ?>','<? echo $is_sales; ?>')">
...
if($action=='populate_data_from_batch')
{
	...
	if ($db_type==0) {
		...
		$dia_cond = (trim($dia_width)!="")?" and width = '$dia_width'":" and width =''";
	}
	elseif ($db_type==2) 
	{
		$dia_cond_c = (trim($dia_width)!="")?" and c.dia_width = '$dia_width'":" and c.dia_width is null";
		$dia_cond = (trim($dia_width)!="")?" and width = '$dia_width'":" and width is null";
	}

	$data_array=sql_select("SELECT a.company_id, a.process_id, a.batch_no, a.batch_weight, a.extention_no, a.color_id, a.booking_without_order,b.is_sales,b.po_id,b.prod_id,b.width_dia_type , sum(b.batch_qnty) as batch_qnty,a.booking_no_id,a.service_booking_id,a.service_booking_no from pro_batch_create_mst a,pro_batch_create_dtls b, product_details_master c where a.id=b.mst_id and b.prod_id = c.id and b.mst_id='$batch_id' and c.detarmination_id=$determination_id and b.width_dia_type=$width_type and b.body_part_id=$body_part_id and c.gsm=$gsm_weight $dia_cond_c and a.status_active=1 and b.status_active=1 and b.is_deleted=0 group by a.company_id, a.process_id, a.batch_no, a.extention_no, a.batch_weight, a.color_id, a.booking_without_order, b.is_sales, b.po_id, b.prod_id, b.width_dia_type, a.booking_no_id, a.service_booking_id, a.service_booking_no");	
	...
	$variable_set_production=sql_select("select distribute_qnty from variable_settings_production where variable_list =51 and company_name=$company_id and item_category_id=2 and auto_update=1");
...
if($action=='show_fabric_desc_listview')
{
	...
	if ($db_type==0)
	{
		$data_array=sql_select("SELECT a.color_id,concat(b.po_id,',') as po_id,b.prod_id, c.detarmination_id, b.body_part_id,concat(b.roll_id,',') as roll_id, sum(b.batch_qnty) as qnty,b.width_dia_type from pro_batch_create_mst a,pro_batch_create_dtls b,product_details_master c where a.id=b.mst_id and b.prod_id=c.id and a.id='$batch_info[0]' and b.status_active=1 and b.is_deleted=0 group by a.color_id,b.prod_id, c.detarmination_id,b.body_part_id, b.width_dia_type");
	}
	else
	{
		$data_array=sql_select("SELECT a.color_id,listagg(b.po_id, ',') within group (order by b.po_id) as po_id,b.prod_id, c.detarmination_id, b.body_part_id,listagg(b.roll_id, ',') within group (order by b.roll_id) as roll_id, sum(b.batch_qnty) as qnty,b.width_dia_type from pro_batch_create_mst a,pro_batch_create_dtls b,product_details_master c where a.id=b.mst_id and b.prod_id=c.id and a.id='$batch_info[0]' and b.status_active=1 and b.is_deleted=0 group by a.color_id,b.prod_id, c.detarmination_id,b.body_part_id, b.width_dia_type  order by   b.body_part_id");
	}
	...
	if($batch_info[1] == 1 && !empty($sales_arr)){
		...
		$get_uom_from_sales_order = sql_select("select a.id,b.id dtls_id,b.determination_id,b.cons_uom,b.width_dia_type,b.color_id, b.body_part_id, b.avg_rate, a.currency_id from fabric_sales_order_mst a,fabric_sales_order_dtls b where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and a.id in(".implode(",",array_unique($sales_arr)).") group by a.id,b.id,b.determination_id,b.cons_uom,b.width_dia_type,b.color_id,b.body_part_id, b.avg_rate, a.currency_id");
	...
	<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="350">
		...
		<tbody>
			...
			foreach($data_array as $row)
			{
				...
				?>
				<tr bgcolor="<? echo $bgcolor; ?>" onClick='set_form_data("<? echo $prod_array[$row[csf('prod_id')]]['desc']."**".$determination_id."**".$gsm."**".$dia."**".$prod_id."**".$width_dia_type."**".$row[csf('qnty')]."**".$uom_id."**".$row[csf('body_part_id')]."**".$batch_info[1]."**".$rate."**".$currency_id; ?>")' style="cursor:pointer">
					<td><? echo $i; ?></td>
					<td title="ProdID=<? echo $row[csf('prod_id')];?>"><? echo $body_part[$row[csf('body_part_id')]].", ".$prod_array[$row[csf('prod_id')]]['desc']; ?></td>
...
if ($action=="po_popup")
{
	...
	$production_vali_source 	= ($production_vali_source_arr[0][csf('auto_update')] == 2) ? 0 : 1;
	...
	$process_loss_method = ($process_loss_method_variable[csf("process_loss_method")] ==2) ? 2: 1;


	$over_receive_limit = ($variable_set_production[0][csf('distribute_qnty')] != "") ? $variable_set_production[0][csf('distribute_qnty')] : 0;
	...
	*/
	...
	<script>
				...
				$("#tbl_list_search").find('tr').each(function()
				{
					...
					*/
					...
					if(production_basis==5)
					{
						...
						/*if(finish_qnty>fabric_qnty)
						{
							$(this).find('input[name="txtfinishQnty[]"]').val(fabric_qnty.toFixed(2));
						}else {
		...
		function fnc_close()
		{
			...
			var overValueIndependBasis = 0; var overValueBatchBasis = 0;
			var vali_total_finish=0; var vali_total_pre_rcv = 0;
			$("#tbl_list_search").find('tr').each(function()
			{
				var txtfinishQnty=$(this).find('input[name="txtfinishQnty[]"]').val()*1;
				...
				if(txtfinishQnty > 0)
				{
					...
					//alert(txtfinishQnty +' + ' + previous_recieve_qty +' > '+ txtfinish_required_qnty*1);
			...
			$("#tbl_list_search").find('tr').each(function()
			{
				...
				if(txtfinishQnty*1>0 || txtrejectQnty*1>0  || txtgreyQnty*1>0 || txtwgtlostQnty*1>0)
				{
					if(save_string=="")
					{
						save_string=txtPoId+"**"+txtfinishQnty+"**"+txtRoll+"**"+txtrejectQnty+"**"+txtgreyQnty+"**"+txtwgtlostQnty+"**"+sales_booking_no;
					}
					else
					{
						save_string+=","+txtPoId+"**"+txtfinishQnty+"**"+txtRoll+"**"+txtrejectQnty+"**"+txtgreyQnty+"**"+txtwgtlostQnty+"**"+sales_booking_no;
...
<body>
						...
						<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="940" id="tbl_list_search">
							...
							for($z=0;$z<count($explSaveData);$z++)
							{
								...
								else
								{
									...
									$grey_qnty=$po_wise_data[3];
									$wgtlost_qnty=$po_wise_data[4];
								}
								...
								<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
									...
									<td width="70" align="center">
										...
										<input type="text" name="txtgreyQnty[]" id="txtgreyQnty_<? echo $i; ?>" onBlur="wgtlostcalculation();fn_check_balance(<? echo $i; ?>);"  class="text_boxes_numeric" style="width:50px" value="<? echo $grey_qnty; ?>">
	...
	else // echo "Batch Based";
	{
		...
		<div style="margin-left:10px; margin-top:10px">
			...
			<div style="width:860px; max-height:280px; overflow-y:scroll" id="list_container" align="left">
				<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="840" id="tbl_list_search">
					...
					else if($save_data!="" && $production_basis==5)
					{
						...
						else
						{
							...
							if ($db_type==0) {
								$dia_cond_a = (trim($txt_original_dia_width)!="")?" and a.width = '$txt_original_dia_width'":" and a.width =''";
							}
							elseif ($db_type==2) 
							{
								$dia_cond_a = (trim($txt_original_dia_width)!="")?" and a.width = '$txt_original_dia_width'":" and a.width is null";
							}

							$dtls_id_cond = ($dtls_id !="") ? " and a.id!=$dtls_id" : "";
							$po_cumulative_sql=sql_select("select a.batch_id,a.body_part_id, a.fabric_description_id,a.order_id as poid, sum(a.receive_qnty) as qntity from pro_finish_fabric_rcv_dtls a, inv_receive_master b where a.mst_id=b.id and b.entry_form=7 and b.status_active=1 and b.is_deleted=0 $dtls_id_cond and  a.batch_id='$txt_batch_id' and a.body_part_id = '$cbo_body_part' and a.fabric_description_id = '$fabric_desc_id' and a.gsm='$txt_original_gsm' $dia_cond_a and a.status_active=1 and a.is_deleted=0 group by a.batch_id,a.body_part_id, a.fabric_description_id,a.order_id");
						...
						foreach($nameArray as $row)
						{
							...
							<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
								...
								<td width="70" align="center">
									<input type="text" name="txtgreyQnty[]" id="txtgreyQnty_<? echo $i; ?>" onBlur="wgtlostcalculation();fn_check_balance(<? echo $i; ?>);"  class="text_boxes_numeric" style="width:50px" value="<? echo $grey_qnty; ?>" <? echo $IsGreyUsedDisabled;?> >
					...
					else
					{
						...
						else
						{
							...
							else
							{
								...
								$po_cumulative_sql=sql_select("select a.batch_id,a.body_part_id, a.fabric_description_id,a.order_id as poid, sum(a.receive_qnty) as qntity from pro_finish_fabric_rcv_dtls a, inv_receive_master b where a.mst_id=b.id and b.entry_form=7 and b.status_active=1 and b.is_deleted=0 and  a.batch_id='$txt_batch_id' and a.body_part_id = '$cbo_body_part' and a.fabric_description_id = '$fabric_desc_id' and a.gsm='$txt_original_gsm' $dia_cond_a and a.status_active=1 and a.is_deleted=0 group by a.batch_id,a.body_part_id, a.fabric_description_id,a.order_id");
						...
						foreach($nameArray as $row)
						{
							...
							<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
								...
								<td width="70" align="center">
									<input type="text" name="txtgreyQnty[]" onBlur="wgtlostcalculation();fn_check_balance(<? echo $i; ?>);" id="txtgreyQnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:50px" value="" <? echo $IsGreyUsedDisabled;?> >
				...
				<table width="840"  border="1" cellpadding="0" cellspacing="0" rules="all" class="rpt_table">
					<tr class="tbl_bottom">
...
if($action=="check_fin_fab_dlv_action")
{
	...
	$data_array_delivery_qty=sql_select("select a.sys_number as recv_issue_sys,sum(b.current_delivery) as current_delivery, 1 as delivery from pro_grey_prod_delivery_mst a,pro_grey_prod_delivery_dtls b where a.id=b.mst_id and b.grey_sys_id='$update_id' and b.sys_dtls_id=$update_dtls_id  group by a.sys_number");
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		{
			$row_prod=sql_select("select id, current_stock from product_details_master where company_id=$cbo_company_id and item_category_id=2 and detarmination_id=$fabric_desc_id and gsm=$txt_gsm and dia_width=$txt_dia_width and color='$color_id' and unit_of_measure=$cbouom and status_active=1 and is_deleted=0");
		}
		...
		$field_array_proportionate="id, trans_id, trans_type, entry_form, dtls_id, po_breakdown_id, prod_id, color_id, quantity, returnable_qnty, grey_used_qty, wgt_lost_qty, is_sales, inserted_by, insert_date";
		foreach($po_array as $key=>$val)
		{
			...
			$data_array_prop.="(".$id_prop.",".$id_trans.",1,7,".$id_dtls.",'".$order_id."','".$prod_id."','".$color_id."','".$order_qnty_qc."','".$order_qnty_rej."','".$order_qnty_grey."','".$order_qnty_wgt."',".$is_sales.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		}
		...
		//echo "insert into order_wise_pro_details (".$field_array_proportionate.") values ".$data_array_prop;oci_rollback($con);die;
	...
	else if ($operation==1)   // Update Here
	{
		...
		foreach($po_array as $key=>$val)
		{
			...
			if($data_array_prop!="" ) $data_array_prop.=",";
			$data_array_prop.="(".$id_prop.",".$update_trans_id.",1,7,".$update_dtls_id.",'".$order_id."','".$prod_id."','".$color_id."','".$order_qnty_qc."','".$order_qnty_rej."','".$order_qnty_grey."','".$order_qnty_wgt."',".$is_sales.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		}
...
if($action=='populate_finish_details_form_data')
{
	...
	foreach ($data_array as $row)
	{
		...
		{	
			...
			echo "get_php_form_data(".$row[csf('batch_id')] ."+'_'+".  $row[csf('fabric_description_id')] ."+'_'+". $row[csf('body_part_id')] ."+'_'+". $row[csf('dia_width_type')]."+'_'+". $row[csf('gsm')]."+'_'+". $row[csf('width')].", 'populate_data_from_batch', 'requires/finish_fabric_receive_controller' );\n";
		...
		{
			$data_po_array=sql_select("select po_breakdown_id, quantity, returnable_qnty,grey_used_qty,wgt_lost_qty from order_wise_pro_details where dtls_id='$id' and entry_form=7 and status_active=1 and is_deleted=0");
			foreach($data_po_array as $row_po)
			{
				if($save_string=="")
				{
					$save_string=$row_po[csf("po_breakdown_id")]."**".$row_po[csf("quantity")]."**0**".$row_po[csf("returnable_qnty")]."**".$row_po[csf("grey_used_qty")]."**".$row_po[csf("wgt_lost_qty")];
				}
				else
				{
					$save_string.=",".$row_po[csf("po_breakdown_id")]."**".$row_po[csf("quantity")]."**0**".$row_po[csf("returnable_qnty")]."**".$row_po[csf("grey_used_qty")]."**".$row_po[csf("wgt_lost_qty")];
...
if($action=="roll_maintained")
{
	...
	echo "reset_form('finishFabricEntry_1','list_fabric_desc_container','','','set_production_basis();','cbo_production_basis*cbo_company_id*txt_production_date*txt_challan_no*roll_maintained*fabric_store_auto_update*process_costing_maintain');\n";
...
if($action=="check_batch_no")
{
	...
	$sql="select id, batch_no from pro_batch_create_mst where batch_no='".trim($data[0])."' and company_id='".$data[1]."' and is_deleted=0 and status_active=1 and entry_form=0
...
if($action=="finish_fab_production_print")
{
	...
	$sql_dtls="select id, prod_id, batch_id, color_id, body_part_id, fabric_description_id, gsm, width, receive_qnty, reject_qty, no_of_roll, machine_no_id, rack_no, shelf_no, order_id, buyer_id,grey_used_qty,remarks,is_sales from pro_finish_fabric_rcv_dtls where mst_id='$data[1]' and status_active=1 and is_deleted=0";
