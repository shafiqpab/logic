			...
			if($qty_source==0)
			{
				...
				{ 
					...
					else if( $variableSettings==3 ) //color and size level
					{
							...
							$dtlsData = sql_select("select a.color_size_break_down_id,
												sum(CASE WHEN a.production_type=3 and b.embel_name=3then a.production_qnty ELSE 0 END) as production_qnty,
												sum(CASE WHEN a.production_type=3 and b.embel_name=3 then a.reject_qty ELSE 0 END) as reject_qty
												from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id  and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in(3) group by a.color_size_break_down_id");
							//and b.id='$data'
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$field_array1="id, garments_nature, company_id, challan_no, po_break_down_id, item_number_id, country_id, pack_type, production_source, serving_company, location, produced_by, production_date, production_quantity, production_type,embel_name, entry_break_down_type, break_down_type_rej, sewing_line, supervisor, production_hour, remarks, floor_id, alter_qnty, reject_qnty, total_produced, yet_to_produced, prod_reso_allo, spot_qnty,wo_order_id,currency_id, exchange_rate, rate, amount, inserted_by, insert_date";
