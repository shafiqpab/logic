...
if ($action == "load_all_drop_down") 
{
	...
	echo create_drop_down("cbo_location_name", 172, "select id,location_name from lib_location where company_id='$dataEx[0]' and status_active =1 and is_deleted=0 $location_credential_cond order by location_name", "id,location_name", 1, "-- Select Location --", $selected, "");
	...
	echo create_drop_down("cbo_delivery_location", 172, "select id,location_name from lib_location where company_id='$dataEx[1]' and status_active =1 and is_deleted=0 order by location_name", "id,location_name", 1, "-- Select Delivery Location --", $selected, "load_drop_down( 'requires/garments_delivery_entry_controller', $dataEx[1]+'**'+this.value, 'load_drop_down_del_floor', 'del_floor_td' );");

	echo "****";

	echo create_drop_down("cbo_delivery_floor", 172, "select id,floor_name from lib_prod_floor where company_id='$dataEx[2]' and location_id='$data[1]' and status_active =1 and is_deleted=0 and production_process=11 order by floor_name", "id,floor_name", 1, "-- Select Floor --", $selected, "");
...
if ($action == "create_delivery_search_list") {
	...
	where a.id=b.delivery_mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form!=85 $sql_cond $po_cond $buyerCond
	...
	<div style="width:1350px">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="100%" class="rpt_table">
			<thead>
				...
				<th width="80" style="word-break: break-all;word-wrap: break-word;" align="center">Delivery Status</th>
			</thead>
		</table>
	</div>
	<div style="width:1350px;max-height:180px; overflow:y-scroll" id="sewing_production_list_view" align="left">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" id="details_table">
			...
			?>
				<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_exfactory_form_data','requires/garments_delivery_entry_controller');get_php_form_data('<? echo $selectResult[csf('po_break_down_id')]; ?>+**+<? echo $selectResult[csf('item_number_id')]; ?>+**+<? echo $selectResult[csf('country_id')]; ?>'+'**'+$('#hidden_preceding_process').val()+'**'+$('#txt_mst_id').val()+'**1'+'**'+$('#sewing_production_variable').val()+'**'+$('#variable_is_controll').val(),'populate_data_from_search_popup','requires/garments_delivery_entry_controller');">
					...
					<td style="word-break: break-all;word-wrap: break-word;" width="130" align="center">
						<p><? echo $country_short_array[$selectResult[csf('country_id')]]; ?>&nbsp;</p>
					</td>
					<td style="word-break: break-all;word-wrap: break-word;" width="130" align="left">
...
if ($action == "create_po_search_list_view") {
	...
	$is_approval_need = return_field_value("approval_need", "APPROVAL_SETUP_MST a,APPROVAL_SETUP_DTLS b", "a.id=b.mst_id and a.COMPANY_ID=$company and b.page_id=25 and b.VALIDATE_PAGE=1 and a.status_active=1 and b.status_active=1 ORDER BY a.SETUP_DATE desc FETCH FIRST 1 ROWS ONLY");
	if($is_approval_need==1)
	{
		$sql = "SELECT b.id, a.order_uom, a.buyer_name, a.company_name, a.total_set_qnty, a.set_break_down, a.job_no, a.style_ref_no, a.gmts_item_id, a.location_name, b.shipment_date, b.po_number, b.po_quantity, b.plan_cut, b.grouping
				...
				where
				a.job_no = b.job_no_mst and a.id=c.job_id and c.approved=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.garments_nature=$garments_nature $sql_cond  $buyerCond $qty_source_cond $sql_shipment_year_cond $projected_po_cond order by b.shipment_date DESC";
	}
	else
	{
		$sql = "SELECT b.id, a.order_uom, a.buyer_name, a.company_name, a.total_set_qnty, a.set_break_down, a.job_no, a.style_ref_no, a.gmts_item_id, a.location_name, b.shipment_date, b.po_number, b.po_quantity, b.plan_cut, b.grouping
				...
				where
				a.job_no = b.job_no_mst and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.garments_nature=$garments_nature $sql_cond  $buyerCond $qty_source_cond $sql_shipment_year_cond $projected_po_cond order by b.shipment_date DESC";
	}
	// echo $sql;
...
?>
	...
	<div style="width:1260px; max-height:240px;overflow-y:scroll;">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1242" class="rpt_table" id="tbl_po_list">
			<?
			$i = 1;
			foreach ($result as $row) {
			...
			?>
						<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value(<? echo $row[csf("id")]; ?>,'<? echo $grmts_item; ?>','<? echo $po_qnty; ?>','<? echo $plan_cut_qnty; ?>','<? echo $country_id; ?>');">
							...
							<td width="80" align="right"><? echo $po_qnty; //$po_qnty*$set_qty;
															?>&nbsp;</td>
							<td width="70" align="right"><?= $total_cut_qty = $total_ex_fac_data_arr[$row[csf('id')]][$grmts_item][$country_id]; ?>&nbsp;</td>
							<td width="70" align="right"><? $balance = $po_qnty - $total_cut_qty;
															echo $balance; ?>&nbsp;</td>
...
if ($action == "populate_data_from_search_popup") 
{
	...
	foreach ($res as $result) {
		...
		if ($qty_source != 0) {
			...
			if ($qty_source == 4) {
				echo "$('#source_msg').text('Sewing Input Qnty');\n";
			} else if ($qty_source == 5) {
				echo "$('#source_msg').text('Sewing Output Qnty');\n";
			} else if ($qty_source == 7) {
				echo "$('#source_msg').text('Iron Output Qnty');\n";
			} else if ($qty_source == 77) {
				echo "$('#source_msg').text('Buyer Inspection Qnty');\n";
			} else if ($qty_source == 8) {
				echo "$('#source_msg').text('Packing And Finishing');\n";
			} else if ($qty_source == 11) {
				echo "$('#source_msg').text('Poly Entry Qnty');\n";
			} else {
				echo "$('#source_msg').text('Sewing Finish Qnty');\n";
			}
	...
	if ($qty_source != 0) {
		if ($variableSettings == 2) // color level
		{
			if ($db_type == 0) 
			{
				...
				$trans_sql = sql_select("SELECT c.item_number_id,c.color_number_id, (sum( CASE WHEN b.trans_type=5 and b.production_type=10 THEN b.production_qnty ELSE 0 END)-sum( CASE WHEN b.trans_type=6 and b.production_type=10 THEN b.production_qnty ELSE 0 END)) as trans_qnty from pro_garments_production_mst a ,pro_garments_production_dtls b, wo_po_color_size_breakdown c where c.id=b.color_size_break_down_id and c.po_break_down_id=a.po_break_down_id and c.item_number_id=a.item_number_id and c.country_id=a.country_id and a.id=b.mst_id and a.po_break_down_id=$po_id and a.item_number_id='$item_id' and b.production_type=10 and a.country_id='$country_id' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by c.item_number_id,c.color_number_id");
			...
			{
				$sql = "SELECT a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty,sum(CASE WHEN b.production_type='$qty_source' then b.production_qnty ELSE 0 END) as production_qnty,(sum( CASE WHEN b.trans_type=5 and b.production_type=10 THEN b.production_qnty ELSE 0 END)-sum( CASE WHEN b.trans_type=6 and b.production_type=10 THEN b.production_qnty ELSE 0 END)) as trans_qnty
		...
		else if ($variableSettings == 3) //color and size level
		{
			if($qty_source==82) // finish gmts issue
			{
				...
				from pro_garments_production_dtls a,pro_garments_production_mst b, PRO_GMTS_DELIVERY_MST c where a.status_active=1 and a.mst_id=b.id and c.id=b.delivery_mst_id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id<>0 and a.production_type='$qty_source' and c.purpose_id=1 group by a.color_size_break_down_id");
...
?>
	...
	<div style="width:930px;max-height:180px; overflow:y-scroll" id="sewing_production_list_view" align="left">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="100%" class="rpt_table">
			...
			$i = 1;
			$total_production_qnty = 0;
			echo "select id,po_break_down_id,item_number_id,country_id,ex_factory_date,ex_factory_qnty,location,lc_sc_no,invoice_no,challan_no from  pro_ex_factory_mst where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' and entry_form<>85 and status_active=1 and is_deleted=0 order by id";
			die;
...
?>
	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="450" class="rpt_table">
		<thead>
			...
			<th width="40">Country Short Name</th>
			<th width="80">Cut Off Number</th>
	...
	<div id="scroll_body" style="width:460px; max-height:450px; overflow-x:hidden;  overflow-y:auto;">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="450" class="rpt_table" id="tbl_body_1">
			<?
			$i = 1;
			$sqlResult = sql_select("SELECT po_break_down_id, item_number_id, country_id, max(country_ship_date) as country_ship_date, sum(order_quantity) as order_qnty, sum(plan_cut_qnty) as plan_cut_qnty, max(cutup) as cutup from wo_po_color_size_breakdown where po_break_down_id='$data' and status_active in(1,2,3) and is_deleted=0 group by po_break_down_id, item_number_id, country_id order by country_ship_date");
			foreach ($sqlResult as $row) {
				if ($i % 2 == 0) $bgcolor = "#E9F3FF";
				else $bgcolor = "#FFFFFF";
			?>
				<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="put_country_data(<? echo $row[csf('po_break_down_id')] . "," . $row[csf('item_number_id')] . "," . $row[csf('country_id')] . "," . $row[csf('order_qnty')] . "," . $row[csf('plan_cut_qnty')]; ?>);">
					...
					<td width="80">
						<p><? echo $cut_up_array[$row[csf('cutup')]]; ?>&nbsp;</p>
					</td>
...
if ($action == "populate_exfactory_form_data") 
{
	...
	$actual_po = sql_select("Select actual_po_id,actual_po_dtls_id from pro_ex_factory_actual_po_details where mst_id = $mst_id ");
	$actual_po_no = "";
	$actual_po_id = "";
	foreach ($actual_po as $val) {
		...
		else $actual_po_id .= ',' . $val[csf('actual_po_id')] . '_' . $val[csf('actual_po_dtls_id')];
	}
				...
				if ($variableSettings == 2) // color level
				{
					...
					else 
					{
						...
						$sql_exfac = sql_select("SELECT a.item_number_id,a.color_number_id,sum(ex.production_qnty) as ex_production_qnty from wo_po_color_size_breakdown a
								left join pro_ex_factory_dtls ex on ex.color_size_break_down_id=a.id and ex.status_active=1
								where a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' and a.is_deleted=0 and a.status_active in(1,2,3) group by a.item_number_id, a.color_number_id  order by a.item_number_id, a.color_number_id");
						foreach ($sql_exfac as $row_exfac) {
							$ex_fac_value[$row_exfac[csf("item_number_id")]][$row_exfac[csf("color_number_id")]] = $row_exfac[csf("ex_production_qnty")];
						}
...
if ($action == "save_update_delete") {
	...
	if ($operation == 0) // Insert Here----------------------------------------------------------
	{
		...
		foreach($actual_po_arr as $act_po)
		{
			...
			$act_data .= "(" . $act_poid . "," . $id . ",'" . $ex[0]. "','" . $ex[1] . "','" . $_SESSION['logic_erp']['user_id'] . "','" . $pc_date_time . "',1,0)";
			$act_poid = $act_poid + 1;
		}
		...
		$sql_exfac = sql_select("SELECT c.size_number_id, c.color_number_id, sum(case when b.entry_form<>85 then a.production_qnty else 0 end )- sum(case when b.entry_form=85 then a.production_qnty else 0 end ) as ex_production_qnty
							...
							where b.id=a.mst_id and c.po_break_down_id=b.po_break_down_id and c.id=a.color_size_break_down_id and a.status_active=1 and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name and b.country_id=$cbo_country_name
		...
		{
			$color_sizeID_arr = sql_select("SELECT id,color_number_id from wo_po_color_size_breakdown where po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name and country_id=$cbo_country_name   and status_active in(1,2,3)  and is_deleted=0  order by id"); //and color_mst_id<>0
			$colSizeID_arr = array();
		...
		if ($invoice_id != "") {
			...
			$data_array_invoice = "" . $txt_ex_factory_date . "*" . $shipping_mode . "*'" . $tot_carton_qnty . "'";
		...
		if(!empty($act_data))
		{
			$field_actual_po = "id,mst_id,actual_po_id,actual_po_dtls_id,inserted_by,insert_date,status_active,is_deleted";
		...
		//release lock table
		//check_table_status( $_SESSION['menu_id'],0);
		// echo "10**$rID && $DeliveryrID && $dtlsrID && $sts_ex_mst && $sts_country && $sts_ex && $invoiceID && $actualPoId";
	...
	else if ($operation == 1) // Update Here End------------------------------------------------------
	{
		...
		if(!empty($act_data))
		{
			$field_actual_po = "id,mst_id,actual_po_id,actual_po_dtls_id,updated_by,update_date,status_active,is_deleted";
			$actualPoId=sql_insert("pro_ex_factory_actual_po_details",$field_actual_po,$act_data,0);
		}
	...
	<div style="width:900px; margin-top:10px; margin-left:55px;">
		...
		<div style="width:<? echo $table_width; ?>px;">
			...
			<table style="margin-top:10px;" align="right" cellspacing="0" width="<? echo $table_width; ?>" border="1" rules="all" class="rpt_table">
				...
				from wo_po_details_master a, wo_po_break_down b, pro_ex_factory_mst c
				where a.job_no=b.job_no_mst and b.id=c.po_break_down_id and c.delivery_mst_id=$data[1] and c.status_active=1 and c.is_deleted=0
				group by c.foc_or_claim, c.id, a.buyer_name, a.gmts_item_id, a.style_ref_no,a.job_no,a.sustainability_standard,a.fab_material, b.id, b.po_number, c.country_id,c.shiping_mode,c.lc_sc_no
				order by a.style_ref_no";
					} else if ($db_type == 0) {
						$sql = "SELECT c.foc_or_claim,c.id, a.buyer_name, a.gmts_item_id, a.style_ref_no,a.job_no,a.sustainability_standard,a.fab_material, b.id as po_break_down_id, b.po_number, c.country_id, group_concat(c.invoice_no) as invoice_no, sum(c.total_carton_qnty) as total_carton_qnty , sum(c.ex_factory_qnty) as total_qnty,group_concat(c.remarks) as remarks ,group_concat(actual_po) as actual_po,c.shiping_mode,c.lc_sc_no
				from wo_po_details_master a, wo_po_break_down b, pro_ex_factory_mst c
...
if ($action == "actual_po_action") {
	...
	<body>
		...
		<div style="width:690px; max-height:400px; overflow-y:scroll" id="list_container_batch" align="left">
		   ...
		   FROM wo_po_acc_po_info a, WO_PO_ACC_PO_INFO_DTLS b WHERE a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and b.po_break_down_id in($po_id) 
...
if ($action == "ExFactoryPrintSonia") {
	...
	$delivery_mst_sql = sql_select("SELECT id, sys_number, transport_supplier, driver_name, truck_no, dl_no, lock_no, destination_place,challan_no,sys_number_prefix_num,sys_number,mobile_no,do_no,gp_no,forwarder,forwarder_2,delivery_company_id,delivery_location_id,delivery_floor_id,attention,remarks,delivery_date,TO_CHAR(insert_date,'HH24:MI') as delv_time from pro_ex_factory_delivery_mst where id=$data[1] and entry_form!=85");
	$sys_arr = array();
	foreach ($delivery_mst_sql as $row) {
		$supplier_name = $row[csf("transport_supplier")];
		$driver_name = $row[csf("driver_name")];
			...
			<div style="width:<? echo $table_width; ?>px;">
				...
				<table style="margin-top:10px;" align="right" cellspacing="0" width="<? echo $table_width; ?>" border="1" rules="all" class="rpt_table">
					...
					<tbody>
						...
						$sc_num_arr = return_library_array("select id, contract_no from com_sales_contract where status_active=1 and is_deleted=0", "id", "contract_no");
						if ($db_type == 2) {
							$sql = "SELECT c.foc_or_claim, c.id,a.buyer_name, a.gmts_item_id, a.style_ref_no, b.id as po_break_down_id, b.po_number, c.country_id,d.color_number_id, listagg(CAST(c.invoice_no as VARCHAR(4000)),',') within group (order by c.invoice_no) as invoice_no, sum(c.total_carton_qnty) as total_carton_qnty, sum(e.production_qnty) as total_qnty, listagg(CAST(c.remarks as VARCHAR(4000)),',') within group (order by c.remarks) as remarks , listagg(CAST(actual_po as VARCHAR(4000)),',') within group (order by actual_po) as actual_po,c.shiping_mode,c.lc_sc_no
							...
							And d.id=e.color_size_break_down_id 
		 					and b.id=c.po_break_down_id and c.delivery_mst_id=$data[1] and a.job_no=d.job_no_mst and c.status_active=1 and c.is_deleted=0
...
if($action=="file_upload")
{
	...
	$data_array .="(".$id.",".$mst_id.",'gmts_delivery_entry','file_upload/".$filename."','2','".$filename."')";
