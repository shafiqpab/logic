...
if ($action=="load_variable_settings")
{
	...
	$variable_is_control=return_field_value("is_control","variable_settings_production","company_name=$working_company and variable_list=33 and page_category_id=250","is_control");
...
{
	echo create_drop_down( "cbo_location", 150, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id='$data' order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down( 'requires/woven_finishing_entry_controller', this.value, 'load_drop_down_floor', 'floor_td'); load_drop_down( 'requires/woven_finishing_entry_controller', document.getElementById('cbo_location').value+'_'+document.getElementById('prod_reso_allo').value+'_'+document.getElementById('txt_poly_date').value, 'load_drop_down_poly_output_line', 'poly_line_td'); get_php_form_data(document.getElementById('cbo_source').value, 'line_disable_enable','requires/woven_finishing_entry_controller');");
	exit();
}

if ($action=="load_drop_down_floor")
{
	echo create_drop_down( "cbo_floor", 150, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process in (13) order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/woven_finishing_entry_controller', this.value+'_'+document.getElementById('cbo_location').value+'_'+document.getElementById('prod_reso_allo').value+'_'+document.getElementById('txt_poly_date').value, 'load_drop_down_poly_line_floor', 'poly_line_td' );",0 );
	exit();
}
...
if($action=="load_drop_down_poly_output")
{
	...
	{
 		echo create_drop_down( "cbo_poly_company", 150, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1 $company_cond order by company_name","id,company_name", 1, "--- Select ---", "",  "fnc_loadvariable(this.value); load_drop_down( 'requires/woven_finishing_entry_controller', this.value, 'load_drop_down_location', 'location_td' ); fnc_company_check(document.getElementById('cbo_source').value);",0,0 );
...
{
	$explode_data = explode("_",$data);
	$prod_reso_allocation = 0;//$explode_data[2];
...
if($action=="order_popup")
{
	...
	function js_set_value(id,item_id,po_qnty,plan_qnty,country_id,pack_type,job_no,country_ship_date)
	{
        				...
        				<tr>
                    		...
                    		<td align="center">
							 ...
							 <?
							   ...
							   $production_company = str_replace("'","",$production_company);
							   $hidden_variable_cntl = str_replace("'","",$hidden_variable_cntl);
							   $hidden_preceding_process = str_replace("'","",$hidden_preceding_process);
							 ?>

							<input type="hidden" id="hidden_company" value="<? echo $company; ?>">
							<input type="hidden" id="hidden_garments_nature" value="<? echo $garments_nature; ?>">
							<input type="hidden" id="hidden_production_company" value="<? echo $production_company; ?>">
							<input type="hidden" id="hidden_variable_cntl" value="<? echo $hidden_variable_cntl; ?>">
							<input type="hidden" id="hidden_preceding_process" value="<? echo $hidden_preceding_process; ?>">


            		 		<td align="center">
                     			<input type="button" name="btn_show" id="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_by').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('hidden_company').value+'_'+document.getElementById('hidden_garments_nature').value+'_'+document.getElementById('hidden_production_company').value+'_'+document.getElementById('hidden_variable_cntl').value+'_'+document.getElementById('hidden_preceding_process').value+'_'+document.getElementById('cbo_year_selection').value, 'create_po_search_list_view', 'search_div', 'woven_finishing_entry_controller', 'setFilterGrid(\'tbl_po_list\',-1)')" style="width:100px;" />
                            </td>
        				</tr>
             		</table>
          		</td>
...
if($action=="create_po_search_list_view")
{
	...
	if(($txt_date_from=="" || $txt_date_to=="") && $txt_search_common =="" )
	{
		echo "<h1 style='color:red; font-size: 17px;text-align:center;margin-top:20px;'> ** Please give at least one data ** </h1>" ;
        die();
	}
	...
	{
		$sql = "SELECT b.id, a.order_uom, a.buyer_name, a.company_name, a.total_set_qnty, a.set_break_down, a.job_no, a.style_ref_no, a.gmts_item_id, a.location_name, b.shipment_date, b.po_number,b.file_no,b.grouping, b.po_quantity, b.plan_cut,a.id as job_id
			from wo_po_details_master a, wo_po_break_down b, wo_po_acc_po_info c
			where a.id = b.job_id and b.id=c.po_break_down_id and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.garments_nature=$garments_nature
			$sql_cond $projected_po_cond group by b.id, a.order_uom, a.buyer_name, a.company_name, a.total_set_qnty, a.set_break_down, a.job_no, a.style_ref_no, a.gmts_item_id, a.location_name, b.shipment_date, b.po_number,b.file_no,b.grouping, b.po_quantity, b.plan_cut,a.id order by b.shipment_date DESC";
	}
	else
	{
 		$sql = "SELECT b.id, a.order_uom, a.buyer_name, a.company_name, a.total_set_qnty, a.set_break_down, a.job_no, a.style_ref_no, a.gmts_item_id, a.location_name, b.shipment_date, b.po_number,b.file_no,b.grouping, b.po_quantity, b.plan_cut,a.id as job_id
			from wo_po_details_master a, wo_po_break_down b
			where a.id = b.job_id and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0  $sql_cond $projected_po_cond $sql_shipment_year_cond order by b.shipment_date DESC";//and a.garments_nature=$garments_nature
	}
	// echo $sql;die;
	...
	$order_id_cond = where_con_using_array($order_id_arr,0,"a.po_break_down_id");
	$country_sql=sql_select( "SELECT a.country_ship_date ,a.po_break_down_id, a.item_number_id, a.country_id, a.pack_type, sum(a.order_quantity) as po_qty, sum(a.plan_cut_qnty) as plan_cut_qty,a.color_number_id,a.job_no_mst as job_no from wo_po_color_size_breakdown a where a.status_active in(1,2,3) and a.is_deleted=0 $order_id_cond group by a.country_ship_date, a.po_break_down_id,a.item_number_id, a.country_id, a.pack_type,a.color_number_id,a.job_no_mst");
	 //echo $country_sql;die;

	foreach($country_sql as $row)
	{
		$po_country_data_arr[$row[csf('po_break_down_id')]][$row[csf('item_number_id')]][$row[csf('country_id')]][$row[csf('country_ship_date')]][$row[csf('pack_type')]]['po_qty'] +=$row[csf('po_qty')];
		$po_country_data_arr[$row[csf('po_break_down_id')]][$row[csf('item_number_id')]][$row[csf('country_id')]][$row[csf('country_ship_date')]][$row[csf('pack_type')]]['plan_cut_qty'] +=$row[csf('plan_cut_qty')];
	...
	$order_id_cond = where_con_using_array($order_id_arr,0,"a.po_break_down_id");
	$total_out_qty=sql_select( "SELECT a.po_break_down_id, a.item_number_id, a.country_id,a.country_ship_date, a.pack_type, sum(b.production_qnty) as production_quantity from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0  and a.production_type=80 and b.production_type=80 $order_id_cond group by a.po_break_down_id, a.item_number_id, a.country_id,a.country_ship_date, a.pack_type");

	foreach($total_out_qty as $row)
	{
		$total_out_qty_data_arr[$row[csf('po_break_down_id')]][$row[csf('item_number_id')]][$row[csf('country_id')]][$row[csf('country_ship_date')]][$row[csf('pack_type')]] +=$row[csf('production_quantity')];
	}
	unset($total_out_qty);
	$qty_source=0;
	/*Sewing Input=28 Sewing Output=29 Iron entry=30
	...
	else if($preceding_process==252) $qty_source=3;//wash  Entry
	// default source for poly from sewing output
	$emb_cond = ($qty_source==3) ? " and a.embel_name=2" : "";  
	$embel_name = ($qty_source==3) ? 2 : 0;  
	if($preceding_process != "")
	{
		$all_job = implode(',',$job_id_arr);
		$wash_job_array= return_library_array("SELECT job_id  from wo_pre_cost_embe_cost_dtls where job_id in ($all_job) and emb_name=3 and is_deleted=0 and status_active=1","job_id","job_id");
 
		if(count($wash_job_array)>0)
		{
			$var_qty_source = $qty_source;
			$qty_source="$qty_source,3";
			$emb_cond = " and a.embel_name in($embel_name,3)"; 
		}
		/* else
		{
			$qty_source=5; // issue id
		} */
	} 
	// echo "<pre>"; print_r($wash_job_array); die;

	$total_in_qty_data_arr=array();
    $sql= "SELECT a.po_break_down_id, a.item_number_id, a.country_id,a.country_ship_date, sum(b.production_qnty) as production_quantity, a.pack_type, b.production_type,a.embel_name from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and  a.status_active=1 and a.is_deleted=0 and  b.status_active=1 and b.is_deleted=0  and a.production_type in($qty_source) $emb_cond and b.production_type in($qty_source) and a.company_id=$production_company group by a.po_break_down_id, a.item_number_id, a.country_id, a.pack_type,a.country_ship_date, b.production_type,a.embel_name ";
	// echo $sql;die;
	$total_in_qty=sql_select($sql);
	$total_wash_qty_array= array();
	foreach($total_in_qty as $row)
	{
		$total_in_qty_data_arr[$row[csf('po_break_down_id')]][$row[csf('item_number_id')]][$row[csf('country_id')]][$row[csf('country_ship_date')]][$row[csf('pack_type')]][$row[csf('production_type')]] +=$row[csf('production_quantity')];
	}
	unset($total_in_qty);
	// echo "<pre>"; print_r($total_in_qty_data_arr); die;
	 //echo $sql;
	?>
     ...
     <div style="width:1270px;">
     	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="100%" class="rpt_table">
            <thead>
                ...
                <th width="80">Total Fin. Input Qty</th>
                <th width="80">Total Fin. Output Qty</th>
     ...
     <div style="width:1270px; max-height:240px;overflow-y:scroll;" >
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1252" class="rpt_table" id="tbl_po_list">
			...
			foreach( $result as $key=>$row )
            {
				...
				//print_r($po_country_arr[$row[csf("id")]]);die;
				$set_qty=""; $grmts_item="";	
					...
					{ 
						foreach ($coun_ship_date_data as $pack_type=>$pack_data) 
						{
							       $country_ship_date = $coun_ship_date;
									...
									$po_qnty=$po_country_data_arr[$row[csf('id')]][$grmts_item][$country_id][$coun_ship_date][$pack_type]['po_qty'];
									
									$plan_cut_qnty=$po_country_data_arr[$row[csf('id')]][$grmts_item][$country_id][$coun_ship_date][$pack_type]['plan_cut_qty'];

									$production_type =  ($job_id_arr[$row['JOB_ID']]) ? 3 : $var_qty_source; 

									$total_in_qty= $total_in_qty_data_arr[$row[csf('id')]][$grmts_item][$country_id][$coun_ship_date][$pack_type][$production_type];
									
									?>
										<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value(<? echo $row[csf("id")];?>,'<? echo $grmts_item;?>','<? echo $po_qnty;?>','<? echo $plan_cut_qnty;?>','<? echo $country_id;?>','<? echo $pack_type;?>','<? echo $row[csf("job_no")];?>','<?  echo $country_ship_date ?>');" >
											<td width="30" align="center"><?php echo $i; ?></td>
											<td width="60"><p><?php echo change_date_format($coun_ship_date);?></p></td>
											<td width="100" title="<?=$color_name?$color_name:"f"?>" ><p><?php echo $row[csf("po_number")]; ?></p></td>
											...
											<td width="80" align="right"><?php echo $total_in_qty;  ?>&nbsp;</td>
											<td width="80" align="right"><?php $total_cut_qty=$total_out_qty_data_arr[$row[csf('id')]][$grmts_item][$country_id][$coun_ship_date][$pack_type]; echo $total_cut_qty; ?>&nbsp;</td>
...
if($action=="populate_data_from_search_popup")
{
	...
	else if($dataArr[3]==252) $qty_source=3;//wash Entry

	if($dataArr[3] != "")
	{
		$wash_check=return_field_value("emb_name","WO_PRE_COST_EMBE_COST_DTLS","job_no='$txt_job_no' and emb_name=3 and is_deleted=0 and status_active=1");
  	...
  	foreach($res as $result)
	{
		...
		echo "$('#txt_style_no').val('".$result[csf('style_ref_no')]."');\n";
   		...
   		if($qty_source!=0)
   		{
   			$dataArray=sql_select("SELECT SUM(CASE WHEN b.production_type='$qty_source' $embl_name THEN b.production_qnty END) as totalinput,SUM(CASE WHEN b.production_type=80  THEN b.production_qnty ELSE 0 END) as totalpoly from pro_garments_production_mst a,pro_garments_production_dtls b WHERE a.id=b.mst_id and a.po_break_down_id=".$result[csf('id')]."  and a.item_number_id='$item_id' and a.country_id='$country_id' $pack_type_cond $country_ship_date_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
			// echo $dataArray;die;
		...
		if($qty_source==0)
		{
			$plan_cut_qnty=return_field_value("sum(plan_cut_qnty)","wo_po_color_size_breakdown","po_break_down_id=".$result[csf('id')]." and item_number_id='$item_id' and country_id='$country_id' $country_ship_date_cond and status_active in(1,2,3) and is_deleted=0");

			$total_produced = return_field_value("sum(b.production_qnty)","wo_po_color_size_breakdown a,pro_garments_production_dtls b","a.id=b.COLOR_SIZE_BREAK_DOWN_ID and  a.po_break_down_id=".$result[csf('id')]." and a.item_number_id='$item_id' and a.country_id='$country_id' $country_ship_date_cond and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.production_type=80 and a.status_active=1 ");
...
if($action=="color_and_size_level")
{
	...
	else if($dataArr[6]==252) $qty_source=3;//wash Entry
	...
	if($qty_source !=0)
	{
	 ...
	 if( $variableSettings==2 ) // color level
	 {
			if($db_type==0)
			{
				$sql = "select id, item_number_id, size_number_id, color_number_id, order_quantity, sum(plan_cut_qnty) as plan_cut_qnty, (select sum(CASE WHEN pdtls.color_size_break_down_id=wo_po_color_size_breakdown.id then pdtls.production_qnty ELSE 0 END) from pro_garments_production_dtls pdtls where pdtls.production_type='$qty_source' and pdtls.is_deleted=0 ) as production_qnty, (select sum(CASE WHEN cur.color_size_break_down_id=wo_po_color_size_breakdown.id then cur.production_qnty ELSE 0 END) from pro_garments_production_dtls cur where cur.production_type=80 and cur.is_deleted=0 ) as cur_production_qnty
			...
			else
			{
				$sql = "select a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty,
						...
						sum(CASE WHEN c.production_type=80 then b.production_qnty ELSE 0 END) as cur_production_qnty,
						sum(CASE WHEN c.production_type=80 then b.reject_qty ELSE 0 END) as reject_qty
	...
	else //if( $variableSettings==3) color and size level (if not lib set, defaulst this;)
	{
		...
		if ($qty_source=="3") 
		{
			$dtlsData = sql_select("SELECT a.color_size_break_down_id,
									sum(CASE WHEN a.production_type='$qty_source'$embl_name then a.production_qnty ELSE 0 END) as production_qnty,
									sum(CASE WHEN a.production_type=80 then a.production_qnty ELSE 0 END) as cur_production_qnty ,
									sum(CASE WHEN a.production_type=80 then a.reject_qty ELSE 0 END) as reject_qty
									from pro_garments_production_dtls a, wo_po_color_size_breakdown b,pro_garments_production_mst c where a.mst_id=c.id and c.PO_BREAK_DOWN_ID=b.PO_BREAK_DOWN_ID and a.color_size_break_down_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and  b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in($qty_source,80) $pack_typeCond  group by a.color_size_break_down_id");
		...
		else
		{
			$dtlsData = sql_select("SELECT a.color_size_break_down_id,
									...
									sum(CASE WHEN a.production_type=80 then a.production_qnty ELSE 0 END) as cur_production_qnty ,
									sum(CASE WHEN a.production_type=80 then a.reject_qty ELSE 0 END) as reject_qty
									from pro_garments_production_dtls a, wo_po_color_size_breakdown b where a.color_size_break_down_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and  b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in($qty_source,80) $pack_typeCond $country_ship_date_cond3 group by a.color_size_break_down_id");
		...
		$sql = "select a.color_order, a.id, a.item_number_id, a.size_number_id, a.color_number_id, a.order_quantity, a.plan_cut_qnty
			from wo_po_color_size_breakdown a
			where a.po_break_down_id='$po_id'  $country_ship_date_cond2 and a.item_number_id='$item_id' and a.country_id='$country_id' and a.is_deleted=0 and a.status_active in(1,2,3) $pack_type_cond order by a.color_order, a.size_order";
	...
	else // if preceding process =0 in variable setting then plan cut quantity will show
	{
		if( $variableSettings==2 ) // color level
		{
			...
			else
			{
				...
				from wo_po_color_size_breakdown a left join pro_garments_production_dtls b on a.id=b.color_size_break_down_id and b.production_type=80
		...
		else if( $variableSettings==3 ) //color and size level
		{
			...
			$dtlsData = sql_select("select a.color_size_break_down_id,
										sum(CASE WHEN a.production_type=80 then a.production_qnty ELSE 0 END) as production_qnty
			...
			$sql = "select a.color_order, a.id, a.item_number_id, a.size_number_id, a.color_number_id, a.order_quantity, a.plan_cut_qnty
			from wo_po_color_size_breakdown a
			where a.po_break_down_id='$po_id' $country_ship_date_cond2 and a.item_number_id='$item_id' and a.country_id='$country_id' and a.is_deleted=0 and a.status_active in(1,2,3) order by a.color_order,a.size_order"; //color_number_id, id
		...
		else // by default color and size level
		{
			...
			$dtlsData = sql_select("select a.color_size_break_down_id,
										sum(CASE WHEN a.production_type=80 then a.production_qnty ELSE 0 END) as production_qnty
										from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in(80) group by a.color_size_break_down_id");
		...
		foreach($colorResult as $color)
		{
			...
			else //color and size level
			{
				if( !in_array( $color[csf("color_number_id")], $chkColor ) )
				{
					...
					$colorHTML .= '<div id="content_search_panel_'.$color[csf("color_number_id")].'" style="display:none" class="accord_close"><div style="padding-left: 29px;text-align:left"><input type="checkbox" onClick="active_placeholder_qty(' . $color[csf("color_number_id")] . ')" id="set_all_' . $color[csf("color_number_id")] . '">&nbsp;Available Qty Auto Fill</div><table id="table_'.$color[csf("color_number_id")].'">';
	...
	<div style="width:100%;max-height:180px; overflow:y-scroll" id="poly_production_list_view" align="left">
		...
		<?php
			...
			foreach($sqlResult as $selectResult){
				...
				$poly_line=$poly_line_arr[$selectResult[csf('sewing_line')]];

  		?>
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data('<? echo $selectResult[csf('id')].'**'.$variableSettings; ?>','populate_input_form_data','requires/woven_finishing_entry_controller');" >
...
?>
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="420" class="rpt_table">
		...
		if($preceding_process != "")
		{
			$wash_check=return_field_value("emb_name","WO_PRE_COST_EMBE_COST_DTLS","job_no='$job_no' and emb_name=3 and is_deleted=0 and status_active=1");
			// if(!$wash_check)
			// {
			// 	$qty_source=5; // issue id
			// }
		...
		if ($qty_source=="3") 
		{
			$issue_qnty_arr=sql_select("SELECT a.po_break_down_id, a.item_number_id, a.country_id,a.country_ship_date, a.pack_type, a.production_type, b.production_qnty as cutting_qnty from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id and a.po_break_down_id='$data_ex[0]' and a.production_type in($qty_source,80) $embl_name and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
		}else 
		{
			$issue_qnty_arr=sql_select("SELECT a.po_break_down_id, a.item_number_id, a.country_id,a.country_ship_date, a.pack_type, a.production_type, b.production_qnty as cutting_qnty from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id and a.po_break_down_id='$data_ex[0]' and a.production_type in($qty_source,80) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
		}
		
		//echo $issue_qnty_arr;die;
		$issue_data_arr=array();
		foreach($issue_qnty_arr as $row)
		{
			$issue_data_arr[$row[csf("po_break_down_id")]][$row[csf("country_id")]][$row[csf("item_number_id")]][$row[csf("country_ship_date")]][$row[csf("pack_type")]][$row[csf("production_type")]]+=$row[csf("cutting_qnty")];
		}
		$i=1;
		$sqlResult =sql_select("SELECT a.po_break_down_id, a.item_number_id, a.country_id, a.pack_type,b.job_no, a.country_ship_date, sum(a.order_quantity) as order_qnty, sum(a.plan_cut_qnty) as plan_cut_qnty from wo_po_color_size_breakdown a,wo_po_details_master b where  a.job_id = b.id and  a.po_break_down_id='$data_ex[0]' and a.status_active in(1,2,3) and a.is_deleted=0 group by a.po_break_down_id, a.item_number_id, a.country_id, a.pack_type,b.job_no,a.country_ship_date order by a.country_ship_date Asc");
		// echo $sqlResult;die;
		foreach($sqlResult as $row)
		{
			if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
			$issue_qnty=$issue_data_arr[$row[csf("po_break_down_id")]][$row[csf("country_id")]][$row[csf("item_number_id")]][$row[csf('country_ship_date')]][$row[csf("pack_type")]][80];
			$out_qnty=$issue_data_arr[$row[csf("po_break_down_id")]][$row[csf("country_id")]][$row[csf("item_number_id")]][$row[csf('country_ship_date')]][$row[csf("pack_type")]][$qty_source];
		?>
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="put_country_data(<? echo $row[csf('po_break_down_id')].",".$row[csf('item_number_id')].",".$row[csf('country_id')].",".$row[csf('order_qnty')].",".$row[csf('plan_cut_qnty')].",'".$row[csf('pack_type')]."','".$row[csf('job_no')]."'"; ?>,'<?  echo $row[csf('country_ship_date')] ?>');">
                ...
                <td width="30"><?php  echo $row[csf('pack_type')]; ?>&nbsp;</td>
                <td align="right" width="60"><?php  echo $out_qnty; ?></td>
                <td align="right" width="60"><?php  echo $issue_qnty; ?></td>
			</tr>
		<?
			$i++;
		}
...
if($action=="populate_input_form_data")
{
	...
	$job_no = $colsize_res[0][csf('job_no_mst')];
	$update_shidate = execute_query("UPDATE pro_garments_production_mst set country_ship_date='$country_ship_date' WHERE id=$id");
	...
	*/	
	...
	if($country_ship_date=='') $country_ship_date_cond2=""; else $country_ship_date_cond2="and country_ship_date='$country_ship_date'";
	...
	$control_and_preceding=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=33 and page_category_id=250 and company_name='$company_id'");
	...
	// echo $qty_source;die;
 		...
 		foreach($sqlResult as $result)
		{
			...
			echo "load_drop_down( 'requires/woven_finishing_entry_controller', ".$result[csf('production_source')].", 'load_drop_down_poly_output', 'sew_company_td' );\n";
			echo "$('#cbo_poly_company').val('".$result[csf('serving_company')]."');\n";
			echo "load_drop_down( 'requires/woven_finishing_entry_controller',".$result[csf('serving_company')].", 'load_drop_down_location', 'location_td' );";

			echo "$('#cbo_location').val('".$result[csf('location')]."');\n";
			echo "load_drop_down( 'requires/woven_finishing_entry_controller', ".$result[csf('location')].", 'load_drop_down_floor', 'floor_td' );\n";
			echo "$('#cbo_floor').val('".$result[csf('floor_id')]."');\n";

			echo "load_drop_down( 'requires/woven_finishing_entry_controller', ".$result[csf('po_break_down_id')].", 'load_drop_down_color_type', 'color_type_td' );\n";
			echo "$('#cbo_color_type').val('".$result[csf('color_type_id')]."');\n";

			echo "load_drop_down( 'requires/woven_finishing_entry_controller', document.getElementById('cbo_floor').value+'_'+document.getElementById('cbo_location').value+'_'+document.getElementById('prod_reso_allo').value+'_'+document.getElementById('txt_poly_date').value, 'load_drop_down_poly_line_floor', 'poly_line_td' );\n";

			echo "$('#cbo_poly_line').val('".$result[csf('sewing_line')]."');\n";
			echo "get_php_form_data(".$result[csf('production_source')].",'line_disable_enable','requires/woven_finishing_entry_controller');\n";
			...
			if($result[csf('production_source')]==3)
			{
				...
				$preceding_process= $control_and_preceding[0][csf("preceding_page_id")];
				echo "$('#hidden_variable_cntl').val('".$control_and_preceding[0][csf('is_control')]."');\n";
 	 			echo "load_drop_down( 'requires/woven_finishing_entry_controller', '".$result[csf('company_id')]."_".$result[csf('serving_company')]."_".$result[csf('po_break_down_id')]."', 'load_drop_down_workorder', 'workorder_td' );\n";
			...
			if($qty_source!=0)
			{
				$dataArray=sql_select("SELECT SUM(CASE WHEN a.production_type='$qty_source' and b.production_type='$qty_source' THEN  b.production_qnty END) as totalinput,SUM(CASE WHEN a.production_type=80 and b.production_type=80 THEN b.production_qnty ELSE 0 END) as totalpoly from pro_garments_production_mst a,pro_garments_production_dtls b WHERE a.id=b.mst_id and a.po_break_down_id=".$result[csf('po_break_down_id')]." and a.item_number_id=".$result[csf('item_number_id')]." and a.country_id=".$result[csf('country_id')]." $pack_typeCond $country_ship_date_cond and a.production_source=".$result[csf('production_source')]." and a.serving_company=".$result[csf('serving_company')]." and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
				// echo $dataArray;die;
		  		foreach($dataArray as $row)
				{
					echo "$('#txt_input_quantity').val('".$row[csf('totalinput')]."');\n";
			...
			{
				$plan_cut_qnty=return_field_value("sum(plan_cut_qnty)","wo_po_color_size_breakdown","po_break_down_id=".$result[csf('po_break_down_id')]." and item_number_id=".$result[csf('item_number_id')]." and country_id=".$result[csf('country_id')]." $country_ship_date_cond2 and status_active in(1,2,3) and is_deleted=0");

				$total_produced = return_field_value("sum(b.production_qnty)","pro_garments_production_mst a,pro_garments_production_dtls b","a.id=b.mst_id and  a.po_break_down_id=".$result[csf('po_break_down_id')]." and a.item_number_id=".$result[csf('item_number_id')]." and a.country_id=".$result[csf('country_id')]." $country_ship_date_cond and a.production_type=80 and b.production_type=80  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
				echo "$('#txt_input_quantity').val('".$plan_cut_qnty."');\n";
				echo "$('#txt_cumul_poly_qty').val('".$total_produced."');\n";
				$yet_to_produced = $plan_cut_qnty - $total_produced;
				echo "$('#txt_yet_to_poly').val('".$yet_to_produced."');\n";
			...
			$defect_sql=sql_select("SELECT id, po_break_down_id, defect_type_id, defect_point_id, defect_qty from pro_gmts_prod_dft where mst_id='".$result[csf('id')]."' and status_active=1 and is_deleted=0 and production_type='80'");
			foreach($defect_sql as $dft_row)
			{
				...
				{
					if($getup_save_data=="") $getup_save_data=$dft_row[csf('id')].'**'.$dft_row[csf('defect_point_id')].'**'.$dft_row[csf('defect_qty')]; else $getup_save_data.=','.$dft_row[csf('id')].'**'.$dft_row[csf('defect_point_id')].'**'.$dft_row[csf('defect_qty')];
					if($getuppoint_id=="") $getuppoint_id=$dft_row[csf('defect_point_id')]; else $getuppoint_id.=','.$dft_row[csf('defect_point_id')];
					$getupType_id=$dft_row[csf('defect_type_id')];
				}

				if($dft_row[csf('defect_type_id')]==4)
				{
					if($wsh_save_data=="") $wsh_save_data=$dft_row[csf('id')].'**'.$dft_row[csf('defect_point_id')].'**'.$dft_row[csf('defect_qty')]; else $wsh_save_data.=','.$dft_row[csf('id')].'**'.$dft_row[csf('defect_point_id')].'**'.$dft_row[csf('defect_qty')];
					if($wshpoint_id=="") $wshpoint_id=$dft_row[csf('defect_point_id')]; else $wshpoint_id.=','.$dft_row[csf('defect_point_id')];
					$wshType_id=$dft_row[csf('defect_type_id')];
				}

				if($dft_row[csf('defect_type_id')]==5)
				{
					if($mesr_save_data=="") $mesr_save_data=$dft_row[csf('id')].'**'.$dft_row[csf('defect_point_id')].'**'.$dft_row[csf('defect_qty')]; else $mesr_save_data.=','.$dft_row[csf('id')].'**'.$dft_row[csf('defect_point_id')].'**'.$dft_row[csf('defect_qty')];
					if($sptpoint_id=="") $sptpoint_id=$dft_row[csf('defect_point_id')]; else $sptpoint_id.=','.$dft_row[csf('defect_point_id')];
					$mesrType_id=$dft_row[csf('defect_type_id')];
				}

				if($dft_row[csf('defect_type_id')]==6)
				{
					if($fbdy_save_data=="") $fbdy_save_data=$dft_row[csf('id')].'**'.$dft_row[csf('defect_point_id')].'**'.$dft_row[csf('defect_qty')]; else $fbdy_save_data.=','.$dft_row[csf('id')].'**'.$dft_row[csf('defect_point_id')].'**'.$dft_row[csf('defect_qty')];
					if($mesrpoint_id=="") $mesrpoint_id=$dft_row[csf('defect_point_id')]; else $mesrpoint_id.=','.$dft_row[csf('defect_point_id')];
					$fbdyType_id=$dft_row[csf('defect_type_id')];
				}
			}
			...
			echo "$('#save_dataGateup').val('".$getup_save_data."');\n";
			echo "$('#allGateup_defect_id').val('".$getuppoint_id."');\n";
			echo "$('#defectGateup_type_id').val('".$getupType_id."');\n";

			echo "$('#save_dataWash').val('".$wsh_save_data."');\n";
			echo "$('#allWash_defect_id').val('".$wshpoint_id."');\n";
			echo "$('#defectWash_type_id').val('".$wshType_id."');\n";

			echo "$('#save_dataMeasurement').val('".$mesr_save_data."');\n";
			echo "$('#allMeasurement_defect_id').val('".$mesrpoint_id."');\n";
			echo "$('#defectMeasurement_type_id').val('".$mesrType_id."');\n";

			echo "$('#save_dataFinalBody').val('".$fbdy_save_data."');\n";
			echo "$('#allFinalBody_defect_id').val('".$fbdypoint_id."');\n";
			echo "$('#defectFinalBody_type_id').val('".$fbdyType_id."');\n";
				...
				if( $variableSettings!=1 ) // gross level
				{
					...
					if( $variableSettings==2 ) // color level
					{
						if($db_type==0)
						{
							...
							$sql = "SELECT id, item_number_id, size_number_id, color_number_id, order_quantity, sum(plan_cut_qnty) as plan_cut_qnty, (select sum(CASE WHEN pdtls.color_size_break_down_id=wo_po_color_size_breakdown.id then pdtls.production_qnty ELSE 0 END) from pro_garments_production_dtls pdtls where pdtls.production_type='$qty_source' and pdtls.is_deleted=0 ) as production_qnty, (select sum(CASE WHEN cur.color_size_break_down_id=wo_po_color_size_breakdown.id then cur.production_qnty ELSE 0 END) from pro_garments_production_dtls cur where cur.production_type=80 and cur.is_deleted=0) as cur_production_qnty, (select sum(CASE WHEN cur.color_size_break_down_id=wo_po_color_size_breakdown.id then cur.reject_qty ELSE 0 END) from pro_garments_production_dtls cur where cur.production_type=80 and cur.is_deleted=0 ) as reject_qty
							from wo_po_color_size_breakdown
							where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' and is_deleted=0 and status_active in(1,2,3) $pack_typeCond $country_ship_date_cond2 group by color_number_id";
						}
						else
						{
							$sql = "SELECT a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty,
									...
									where a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' and a.is_deleted=0 and a.status_active in(1,2,3) $packTypecond $country_ship_date_cond group by a.item_number_id, a.color_number_id";
						}
					}
					else if( $variableSettings==3 ) //color and size level
					{
							$dtlsData = sql_select("SELECT a.color_size_break_down_id,
												...
												sum(CASE WHEN a.production_type=80 then a.production_qnty ELSE 0 END) as cur_production_qnty,
												sum(CASE WHEN a.production_type=80 then a.reject_qty ELSE 0 END) as reject_qty
												from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in($qty_source,80) $pack_cond $country_ship_date_cond2 group by a.color_size_break_down_id ");
							...
							$sql = "SELECT a.color_order, a.id, a.item_number_id, a.size_number_id, a.color_number_id, a.order_quantity, a.plan_cut_qnty
								from wo_po_color_size_breakdown a
								where a.po_break_down_id='$po_id' $country_ship_date_cond and a.item_number_id='$item_id' and a.country_id='$country_id' and a.is_deleted=0 and a.status_active in(1,2,3) $pack_typeCond order by a.color_order, a.size_order";
					}
					else // by default color and size level
					{
				...
				if( $variableSettings!=1 ) // gross level
				{
					...
					if( $variableSettings==2 ) // color level
					{
						if($db_type==0)
						{
							...
							$sql = "select id, item_number_id, size_number_id, color_number_id, order_quantity, sum(plan_cut_qnty) as plan_cut_qnty, (select sum(CASE WHEN pro_garments_production_dtls.color_size_break_down_id=wo_po_color_size_breakdown.id then production_qnty ELSE 0 END) from pro_garments_production_dtls where is_deleted=0 and  	production_type=80 ) as production_qnty, (select sum(CASE WHEN pro_garments_production_dtls.color_size_break_down_id=wo_po_color_size_breakdown.id then reject_qty ELSE 0 END) from pro_garments_production_dtls where is_deleted=0 and production_type=80 ) as reject_qty
							from wo_po_color_size_breakdown
						...
						from wo_po_color_size_breakdown a left join pro_garments_production_dtls b on a.id=b.color_size_break_down_id and b.production_type=80
						where a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' $country_ship_date_cond and a.is_deleted=0 and a.status_active in(1,2,3) group by a.item_number_id, a.color_number_id";
					...
					else if( $variableSettings==3 ) //color and size level
					{
							...
							$sql = "SELECT id, item_number_id, size_number_id, color_number_id, order_quantity, plan_cut_qnty
							from wo_po_color_size_breakdown
							where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' $country_ship_date_cond2  and is_deleted=0 and status_active in(1,2,3) order by color_number_id,size_order";
...
if ($action=="save_update_delete")
{
	...
	//$is_control=return_field_value("is_control","variable_settings_production","company_name=$cbo_company_name and variable_list=33 and page_category_id=250");
    ...
    $control_and_preceding=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=33 and page_category_id=250 and company_name='$cbo_company_name'");
    $preceding_process=$control_and_preceding[0][csf("preceding_page_id")];
	$qty_source=0;
	if($preceding_process==28)  $qty_source=4; //Sewing Input
	else if($preceding_process==29) $qty_source=5;//Sewing Output
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		/								For Mst Table								 /
		/ ==========================================================================*/
		$field_array1="id, garments_nature, company_id, challan_no, po_break_down_id, item_number_id, country_id, pack_type, production_source, serving_company, location, produced_by, production_date, production_quantity, production_type, entry_break_down_type, break_down_type_rej, sewing_line, supervisor, production_hour, remarks, floor_id, alter_qnty, reject_qnty, total_produced, yet_to_produced, prod_reso_allo, spot_qnty,wo_order_id,currency_id, exchange_rate, rate, amount, inserted_by, insert_date,country_ship_date";
		...
		$dtlsData = sql_select("SELECT a.color_size_break_down_id,
										...
										where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name and b.country_id=$cbo_country_name and a.color_size_break_down_id!=0 and a.production_type in($qty_source,80) $pack_type_cond $country_ship_date_cond
		...
		if(str_replace("'","",$poly_production_variable)==2)//color level wise
		{
			...
			foreach($rowEx as $rowE=>$val)
			{
				...
				if($j==0)$data_array = "(".$dtls_id.",".$id.",80,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."',".$cbo_color_type.")";
		...
		if(str_replace("'","",$poly_production_variable)==3)//color and size wise
		{
			...
			$color_sizeID_arr=sql_select( "select id,size_number_id,color_number_id from wo_po_color_size_breakdown where po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name and country_id=$cbo_country_name $country_ship_date_cond and status_active in(1,2,3) and is_deleted=0 $packType_cond order by size_number_id,color_number_id" );
			$colSizeID_arr=array();
			foreach($color_sizeID_arr as $val){
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq", "pro_garments_production_dtls", $con );
				if($j==0)$data_array = "(".$dtls_id.",".$id.",80,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$cbo_color_type.")";
				else $data_array .= ",(".$dtls_id.",".$id.",80,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$cbo_color_type.")";
				//$dtls_id=$dtls_id+1;
 				$j++;
			}
		}//color and size wise
		...
		if(count($save_string)>0 && str_replace("'","",$save_data)!="")
		{
			...
			foreach($defect_array as $key=>$val)
			{
				...
				$defectPointId=$key;
				$defect_qty=$val;
				$data_array_defect.="(".$dft_id.",".$id.",80,".$hidden_po_break_down_id.",".$defect_type_id.",".$defectPointId.",'".$defect_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			}
		...
		if(count($save_dataSpot)>0 && str_replace("'","",$save_dataSpot)!="")
		{
			...
			foreach($defectSpt_array as $keysp=>$valsp)
			{
				...
				$defectsp_qty=$valsp;
				$data_array_defectsp.="(".$dftSp_id.",".$id.",80,".$hidden_po_break_down_id.",".$defectSpot_type_id.",".$defectspPointId.",'".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			}
			if($data_array_defectsp!="")
			{
				//echo "10**INSERT INTO pro_gmts_prod_dft (".$field_array_defectsp.") VALUES ".$data_array_defectsp."";// die;
				$defectQ=sql_insert("pro_gmts_prod_dft",$field_array_defectsp,$data_array_defectsp,1);
			}
		}
		/* ==========================================================================/
		/									Getup defect							 /
		...
		$defectSpot=true;
		$data_array_gateup="";
		$save_dataGateup=explode(",",str_replace("'","",$save_dataGateup));
		if(count($save_dataGateup)>0 && str_replace("'","",$save_dataGateup)!="")
		{
 			//order_wise_pro_details table data insert START-----//
			$field_array_gateup="id, mst_id, production_type, po_break_down_id, defect_type_id, defect_point_id, defect_qty, inserted_by, insert_date";
 			$defectSpt_array=array();
			for($i=0;$i<count($save_dataGateup);$i++)
			{
				$order_dtls=explode("**",$save_dataGateup[$i]);
				$defect_update_id=$order_dtls[0];
				$defectsp_point_id=$order_dtls[1];
				$defect_qnty=$order_dtls[2];
			...
			foreach($defectSpt_array as $keysp=>$valsp)
			{
				if( $i>0 ) $data_array_gateup.=",";
				//if( $dftSp_id=="" ) $dftSp_id=return_next_id("id", "pro_gmts_prod_dft", 1); else $dftSp_id = $dftSp_id+1;
				$dftSp_id=return_next_id_by_sequence("PRO_GMTS_PROD_DFT_SEQ", "pro_gmts_prod_dft", $con);
				$defectspPointId=$keysp;
				$defectsp_qty=$valsp;
				$data_array_gateup.="(".$dftSp_id.",".$id.",80,".$hidden_po_break_down_id.",".$defectGateup_type_id.",".$defectspPointId.",'".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			}
			if($data_array_gateup!="")
			{
				//echo "10**INSERT INTO pro_gmts_prod_dft (".$field_array_gateup.") VALUES ".$data_array_gateup."";// die;
				$defectQ=sql_insert("pro_gmts_prod_dft",$field_array_gateup,$data_array_gateup,1);
			}
		}
		/* ==========================================================================/
		/									Wash defect 							 /
		...
		$defectSpot=true;
		$data_array_wash="";
		$save_dataWash=explode(",",str_replace("'","",$save_dataWash));
		if(count($save_dataWash)>0 && str_replace("'","",$save_dataWash)!="")
		{
 			//order_wise_pro_details table data insert START-----//
			$field_array_wash="id, mst_id, production_type, po_break_down_id, defect_type_id, defect_point_id, defect_qty, inserted_by, insert_date";
 			$defectSpt_array=array();
			for($i=0;$i<count($save_dataWash);$i++)
			{
				$order_dtls=explode("**",$save_dataWash[$i]);
				$defect_update_id=$order_dtls[0];
				$defectsp_point_id=$order_dtls[1];
				$defect_qnty=$order_dtls[2];
			...
			foreach($defectSpt_array as $keysp=>$valsp)
			{
				if( $i>0 ) $data_array_wash.=",";
				//if( $dftSp_id=="" ) $dftSp_id=return_next_id("id", "pro_gmts_prod_dft", 1); else $dftSp_id = $dftSp_id+1;
				$dftSp_id=return_next_id_by_sequence("PRO_GMTS_PROD_DFT_SEQ", "pro_gmts_prod_dft", $con);
				$defectspPointId=$keysp;
				$defectsp_qty=$valsp;
				$data_array_wash.="(".$dftSp_id.",".$id.",80,".$hidden_po_break_down_id.",".$defectWash_type_id.",".$defectspPointId.",'".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			}
			if($data_array_wash!="")
			{
				//echo "10**INSERT INTO pro_gmts_prod_dft (".$field_array_wash.") VALUES ".$data_array_wash."";// die;
				$defectQ=sql_insert("pro_gmts_prod_dft",$field_array_wash,$data_array_wash,1);
			}
		}
		/* ==========================================================================/
		/								Measurement defect							 /
		/ ==========================================================================*/
		$defectSpot=true;
		$data_array_measurement="";
		$save_dataMeasurement=explode(",",str_replace("'","",$save_dataMeasurement));
		if(count($save_dataMeasurement)>0 && str_replace("'","",$save_dataMeasurement)!="")
		{
 			//order_wise_pro_details table data insert START-----//
			$field_array_measurement="id, mst_id, production_type, po_break_down_id, defect_type_id, defect_point_id, defect_qty, inserted_by, insert_date";
 			$defectSpt_array=array();
			for($i=0;$i<count($save_dataMeasurement);$i++)
			{
				$order_dtls=explode("**",$save_dataMeasurement[$i]);
				$defect_update_id=$order_dtls[0];
				$defectsp_point_id=$order_dtls[1];
				$defect_qnty=$order_dtls[2];
			...
			foreach($defectSpt_array as $keysp=>$valsp)
			{
				if( $i>0 ) $data_array_measurement.=",";
				//if( $dftSp_id=="" ) $dftSp_id=return_next_id("id", "pro_gmts_prod_dft", 1); else $dftSp_id = $dftSp_id+1;
				$dftSp_id=return_next_id_by_sequence("PRO_GMTS_PROD_DFT_SEQ", "pro_gmts_prod_dft", $con);
				$defectspPointId=$keysp;
				$defectsp_qty=$valsp;
				$data_array_measurement.="(".$dftSp_id.",".$id.",80,".$hidden_po_break_down_id.",".$defectMeasurement_type_id.",".$defectspPointId.",'".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			}
			if($data_array_measurement!="")
			{
				//echo "10**INSERT INTO pro_gmts_prod_dft (".$field_array_measurement.") VALUES ".$data_array_measurement."";// die;
				$defectQ=sql_insert("pro_gmts_prod_dft",$field_array_measurement,$data_array_measurement,1);
			}
		}
		/* ==========================================================================/
		/								Final Body defect							 /
		/ ==========================================================================*/
		$defectSpot=true;
		$data_array_finalbody="";
		$save_dataFinalBody=explode(",",str_replace("'","",$save_dataFinalBody));
		if(count($save_dataFinalBody)>0 && str_replace("'","",$save_dataFinalBody)!="")
		{
 			//order_wise_pro_details table data insert START-----//
			$field_array_finalbody="id, mst_id, production_type, po_break_down_id, defect_type_id, defect_point_id, defect_qty, inserted_by, insert_date";
 			$defectSpt_array=array();
			for($i=0;$i<count($save_dataFinalBody);$i++)
			{
				$order_dtls=explode("**",$save_dataFinalBody[$i]);
				$defect_update_id=$order_dtls[0];
				$defectsp_point_id=$order_dtls[1];
				$defect_qnty=$order_dtls[2];
			...
			foreach($defectSpt_array as $keysp=>$valsp)
			{
				if( $i>0 ) $data_array_finalbody.=",";
				//if( $dftSp_id=="" ) $dftSp_id=return_next_id("id", "pro_gmts_prod_dft", 1); else $dftSp_id = $dftSp_id+1;
				$dftSp_id=return_next_id_by_sequence("PRO_GMTS_PROD_DFT_SEQ", "pro_gmts_prod_dft", $con);
				$defectspPointId=$keysp;
				$defectsp_qty=$valsp;
				$data_array_finalbody.="(".$dftSp_id.",".$id.",80,".$hidden_po_break_down_id.",".$defectFinalBody_type_id.",".$defectspPointId.",'".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			}
			if($data_array_finalbody!="")
			{
				//echo "10**INSERT INTO pro_gmts_prod_dft (".$field_array_finalbody.") VALUES ".$data_array_finalbody."";// die;
				$defectQ=sql_insert("pro_gmts_prod_dft",$field_array_finalbody,$data_array_finalbody,1);
			}
		}
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$data_array1="".$txt_pack_type."*".$cbo_source."*".$cbo_poly_company."*".$cbo_location."*".$cbo_produced_by."*".$txt_poly_date."*".$txt_poly_qty."*80*".$poly_production_variable."*".$poly_production_variable_rej."*".$cbo_poly_line."*".$txt_super_visor."*".$txt_reporting_hour."*".$txt_challan."*".$txt_remark."*".$cbo_floor."*".$txt_reject_qnty."*".$txt_alter_qnty."*".$txt_cumul_poly_qty."*".$txt_yet_to_poly."*".$prod_reso_allo."*".$txt_spot_qnty."*".$cbo_work_order."*".$hidden_currency_id."*".$hidden_exchange_rate."*'".$rate."'*'".$amount."'*".$user_id."*'".$pc_date_time."'";
		...
		if(str_replace("'","",$poly_production_variable)!=1 && str_replace("'","",$txt_mst_id)!='' ) // check is not gross level
		{
			...
			$dtlsData = sql_select("select a.color_size_break_down_id,
										...
										sum(CASE WHEN a.production_type=80 then a.production_qnty ELSE 0 END) as cur_production_qnty
										from pro_garments_production_dtls a,pro_garments_production_mst b
										where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name and b.country_id=$cbo_country_name and a.color_size_break_down_id!=0 and a.production_type in($qty_source,80) and b.id!=$txt_mst_id $pack_type_cond $country_ship_date_cond
										group by a.color_size_break_down_id");
			$color_pord_data=array();
			foreach($dtlsData as $row)
			{
			...
			if(str_replace("'","",$poly_production_variable)==2)//color level wise
			{
				...
				foreach($rowEx as $rowE=>$val)
				{
					...
					//4 for poly Input Entry
					$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",   "pro_garments_production_dtls", $con );
					if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",80,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."',".$cbo_color_type.")";
					else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",80,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."',".$cbo_color_type.")";
					//$dtls_id=$dtls_id+1;
					$j++;
				}
			}
			...
			if(str_replace("'","",$poly_production_variable)==3)//color and size wise
			{
				...
				foreach($rowEx as $rowE=>$valE)
				{
					...
					//4 for poly Input Entry
					$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq", "pro_garments_production_dtls", $con );
					if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",80,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$cbo_color_type.")";
					else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",80,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$cbo_color_type.")";
					//$dtls_id=$dtls_id+1;
					$j++;
				}
			}
		...
		if(count($save_string)>0 && str_replace("'","",$save_data)!="")
		{
			...
			foreach($defect_array as $key=>$val)
			{
				...
				$defectPointId=$key;
				$defect_qty=$val;
				$data_array_defect.="(".$dft_id.",".$txt_mst_id.",80,".$hidden_po_break_down_id.",".$defect_type_id.",".$defectPointId.",'".$defect_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			}
			if($data_array_defect!="")
			{
				...
				$query3=execute_query("DELETE FROM pro_gmts_prod_dft WHERE mst_id=$txt_mst_id and defect_type_id=1 and production_type=80",1);
				$defectQ=sql_insert("pro_gmts_prod_dft",$field_array_defect,$data_array_defect,1);
			}
		}
		/* ==========================================================================/
		...
		if(count($save_dataSpot)>0 && str_replace("'","",$save_dataSpot)!="")
		{
			...
			foreach($defectSpt_array as $keysp=>$valsp)
			{
				...
				$defectspPointId=$keysp;
				$defectsp_qty=$valsp;
				$data_array_defectsp.="(".$dftSp_id.",".$txt_mst_id.",80,".$hidden_po_break_down_id.",".$defectSpot_type_id.",".$defectspPointId.",'".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			}
			if($data_array_defectsp!="")
			{
				...
				$query4=execute_query("DELETE FROM pro_gmts_prod_dft WHERE mst_id=$txt_mst_id and defect_type_id=2 and production_type=80",1);
		...
		/ ==========================================================================*/
		$defectGateup=true;
		$data_array_defect_getup="";
		$save_dataGateup=explode(",",str_replace("'","",$save_dataGateup));
		if(count($save_dataGateup)>0 && str_replace("'","",$save_dataGateup)!="")
		{
 			//order_wise_pro_details table data insert START-----//
			$field_array_defectsp="id, mst_id, production_type, po_break_down_id, defect_type_id, defect_point_id, defect_qty, inserted_by, insert_date";
 			$defectGateup_array=array();
			for($i=0;$i<count($save_dataGateup);$i++)
			{
				$order_dtls=explode("**",$save_dataGateup[$i]);
				$defect_update_id=$order_dtls[0];
				...
				if( array_key_exists($defectsp_point_id,$defectGteup_array) )
				{
					$defectGateup_array[$defectsp_point_id]=$defect_qnty;
				}
				else
				{
					$defectGateup_array[$defectsp_point_id]=$defect_qnty;
			...
			$i=0;
			foreach($defectGateup_array as $keysp=>$valsp)
			{
				if( $i>0 ) $data_array_defect_getup.=",";
				//if( $dftSp_id=="" ) $dftSp_id=return_next_id("id", "pro_gmts_prod_dft", 1); else $dftSp_id = $dftSp_id+1;
				$dftSp_id=return_next_id_by_sequence("PRO_GMTS_PROD_DFT_SEQ", "pro_gmts_prod_dft", $con);
				$defectspPointId=$keysp;
				$defectsp_qty=$valsp;
				$data_array_defect_getup.="(".$dftSp_id.",".$txt_mst_id.",80,".$hidden_po_break_down_id.",".$defectGateup_type_id.",".$defectspPointId.",'".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			}

			if($data_array_defect_getup!="")
			{
				// echo "10**INSERT INTO pro_gmts_prod_dft (".$field_array_defectsp.") VALUES ".$data_array_defect_getup.""; die;
				//echo "DELETE FROM pro_gmts_prod_dft WHERE mst_id='$txt_mst_id' and defect_type_id=2";die;
				$query4=execute_query("DELETE FROM pro_gmts_prod_dft WHERE mst_id=$txt_mst_id and defect_type_id=3 and production_type=80",1);
				$defectGateup=sql_insert("pro_gmts_prod_dft",$field_array_defectsp,$data_array_defect_getup,1);
			}
		}
		/* ==========================================================================/
		/								Wash Chk defect 							 /
		...
		if(count($save_dataWash)>0 && str_replace("'","",$save_dataWash)!="")
		{
			...
			foreach($defectSpt_array as $keysp=>$valsp)
			{
				if( $i>0 ) $data_array_defect_wash.=",";
				//if( $dftSp_id=="" ) $dftSp_id=return_next_id("id", "pro_gmts_prod_dft", 1); else $dftSp_id = $dftSp_id+1;
				$dftSp_id=return_next_id_by_sequence("PRO_GMTS_PROD_DFT_SEQ", "pro_gmts_prod_dft", $con);
				$defectspPointId=$keysp;
				$defectsp_qty=$valsp;
				$data_array_defect_wash.="(".$dftSp_id.",".$txt_mst_id.",80,".$hidden_po_break_down_id.",".$defectWash_type_id.",".$defectspPointId.",'".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			}
			if($data_array_defect_wash!="")
			{
				//echo "10**INSERT INTO pro_gmts_prod_dft (".$field_array_defectsp.") VALUES ".$data_array_defect_wash.""; die;
				//echo "DELETE FROM pro_gmts_prod_dft WHERE mst_id='$txt_mst_id' and defect_type_id=2";die;
				$query4=execute_query("DELETE FROM pro_gmts_prod_dft WHERE mst_id=$txt_mst_id and defect_type_id=4 and production_type=80",1);
				$defectWash=sql_insert("pro_gmts_prod_dft",$field_array_defectsp,$data_array_defect_wash,1);
			}
		}
		/* ==========================================================================/
		/								Measurment defect							 /
		/ ==========================================================================*/
		$defectMeasurment=true;
		$data_array_defectsp_measurement="";
		$save_dataMeasurement=explode(",",str_replace("'","",$save_dataMeasurement));
		if(count($save_dataMeasurement)>0 && str_replace("'","",$save_dataMeasurement)!="")
		{
 			//order_wise_pro_details table data insert START-----//
			...
			foreach($defectSpt_array as $keysp=>$valsp)
			{
				if( $i>0 ) $data_array_defectsp_measurement.=",";
				//if( $dftSp_id=="" ) $dftSp_id=return_next_id("id", "pro_gmts_prod_dft", 1); else $dftSp_id = $dftSp_id+1;
				$dftSp_id=return_next_id_by_sequence("PRO_GMTS_PROD_DFT_SEQ", "pro_gmts_prod_dft", $con);
				$defectspPointId=$keysp;
				$defectsp_qty=$valsp;
				$data_array_defectsp_measurement.="(".$dftSp_id.",".$txt_mst_id.",80,".$hidden_po_break_down_id.",".$defectMeasurement_type_id.",".$defectspPointId.",'".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			}

			if($data_array_defectsp_measurement!="")
			{
				//echo "10**INSERT INTO pro_gmts_prod_dft (".$field_array_defectsp.") VALUES ".$data_array_defectsp_measurement.""; die;
				//echo "DELETE FROM pro_gmts_prod_dft WHERE mst_id='$txt_mst_id' and defect_type_id=2";die;
				$query4=execute_query("DELETE FROM pro_gmts_prod_dft WHERE mst_id=$txt_mst_id and defect_type_id=5 and production_type=80",1);
				$defectMeasurment=sql_insert("pro_gmts_prod_dft",$field_array_defectsp,$data_array_defectsp_measurement,1);
			}
		}
		/* ==========================================================================/
		/								Final Body defect							 /
		...
		if(count($save_dataFinalBody)>0 && str_replace("'","",$save_dataFinalBody)!="")
		{
			...
			foreach($defectSpt_array as $keysp=>$valsp)
			{
				if( $i>0 ) $data_array_defect_body.=",";
				//if( $dftSp_id=="" ) $dftSp_id=return_next_id("id", "pro_gmts_prod_dft", 1); else $dftSp_id = $dftSp_id+1;
				$dftSp_id=return_next_id_by_sequence("PRO_GMTS_PROD_DFT_SEQ", "pro_gmts_prod_dft", $con);
				$defectspPointId=$keysp;
				$defectsp_qty=$valsp;
				$data_array_defect_body.="(".$dftSp_id.",".$txt_mst_id.",80,".$hidden_po_break_down_id.",".$defectFinalBody_type_id.",".$defectspPointId.",'".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			}

			if($data_array_defect_body!="")
			{
				//echo "10**INSERT INTO pro_gmts_prod_dft (".$field_array_defectsp.") VALUES ".$data_array_defect_body.""; die;
				//echo "DELETE FROM pro_gmts_prod_dft WHERE mst_id='$txt_mst_id' and defect_type_id=2";die;
				$query4=execute_query("DELETE FROM pro_gmts_prod_dft WHERE mst_id=$txt_mst_id and defect_type_id=6 and production_type=80",1);
				$defectFinal=sql_insert("pro_gmts_prod_dft",$field_array_defectsp,$data_array_defect_body,1);
			}
		}

		// echo "10**$defectQ**$defectSpot**$defectGateup**$defectWash**$defectMeasurment**$defectFinal";die();
	...
	else if ($operation==2)  // Delete Here----------------------------------------------------------
	{
		...
		$dftrID = sql_delete("pro_gmts_prod_dft","status_active*is_deleted","0*1",'mst_id',$txt_mst_id,1);

 		if($db_type==0)
		{
			if($rID && $dtlsrID && $dftrID)
			{
				mysql_query("COMMIT");
...
<div style="width:930px;">
    <table width="900" cellspacing="0" align="right">
        ...
        <tr>
            <td><strong>Fin. Line: </strong></td><td><? echo $poly_library[$line_data_variable[$dataArray[0][csf('sewing_line')]]]; ?></td>
...
if($action=="defect_data")
{
	...
	echo load_html_head_contents("Popup Info","../../", 1, 1, $unicode);
	$caption_name="";
	if($type==1) $caption_name="Inside Check Qty";
	else if($type==2) $caption_name="Top Side Check Qty";
	else if($type==3) $caption_name="Reject Qty";
	else if($type==4) $caption_name="Wash Defect Qty";
	else if($type==5) $caption_name="Measurement Check Qty";
	else if($type==6) $caption_name="Final Body Check Qty";
	?>
    <script>
		function fnc_close()
		{
		...
		function calculate_rejecttype4()
		{
			var reject_qty=0;
			$("#tbl_list_search").find('tbody tr').each(function()
				{
				    ...
				    <tbody>
						<?
							...
							else if($type==4)
							{
								...
								$i=1;
								$total_reject=0;
								foreach($sew_fin_wash_check_array as $id=>$val)
								{
									if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
									$total_reject+=$defect_dataArray[$id]['defectQnty'];
									?>
							...
							else if($type==5)
							{
								...
								foreach($sew_fin_measurment_check_array as $id=>$val)
								{
									...
									<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
										...
										<td width="150"><? echo $val; ?></td>
										<td align="center">
											<input type="text" name="txtDefectQnty[]" id="txtDefectQnty_<? echo $i;?>" class="text_boxes_numeric" style="width:80px" value="<? echo $defect_dataArray[$id]['defectQnty']; ?>" onKeyUp="calculate_rejecttype4()">
											<input type="hidden" name="txtDefectId[]" id="txtDefectId_<? echo $i; ?>" style="width:40px" value="<? echo $id; ?>">
											<input type="hidden" name="txtDefectUpdateId[]" id="txtDefectUpdateId_<? echo $i; ?>" style="width:40px" value="<? echo $defect_dataArray[$id]['up_id']; ?>">
										</td>
									</tr>
...
if($action=="production_process_control")
{
    ...
    $control_and_preceding=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=33 and page_category_id=250 and company_name='$data'");
