...
if ($action=="load_drop_down_line")
{
	...
	if($prod_reso_allocation==1)
	{
		...
		if($db_type==0)
		{
			$line_data=sql_select("select a.id, a.line_number from prod_resource_mst a, prod_resource_dtls b where a.id=b.mst_id and a.is_deleted=0 and b.is_deleted=0 $cond group by a.id, a.line_number,a.prod_resource_num  order by a.prod_resource_num");
		}
...
if($action=="create_bundle_search_list_view")
{
	...
	//if($cut_no!=''){$cutCon=" and c.bundle_no like '%$cut_no%'";}
	...
	$scanned_bundle_arr=return_library_array( "select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=5  and b.status_active=1 and b.is_deleted=0",'bundle_no','bundle_no');
...
{		
	...
	from pro_gmts_delivery_mst a, pro_garments_production_dtls c where a.id=c.delivery_mst_id and c.bundle_no in ('$ex_data') $str_cond  and c.production_type=4
	 and a.status_active=1 and a.is_deleted=0  and c.status_active=1 and c.is_deleted=0
	group by a.sys_number, a.company_id,a.location_id, a.embel_name, a.embel_type, a.serving_company,a.floor_id,a.sewing_line,a.organic,a.production_source,a.delivery_date");
	...
	if(count($sql_mst_data)>0)
	{
		foreach($sql_mst_data as $val)
		{
			...
			echo "load_drop_down( 'requires/bundle_wise_sewing_output_controller','".$val[csf('serving_company')]."_".$val[csf('location_id')]."_".$val[csf('floor_id')]."_".change_date_format($val[csf('delivery_date')])."', 'load_drop_down_line', 'line_td');\n";
...
if ($action == "populate_bundle_data_check") {
	...
	if( $ex_data[4]!=0 ) $str_cond=" and a.sewing_line= $ex_data[4]";
	else $str_cond="";
	

	//$sql = "select d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.cut_no, c.bundle_no, f.company_name from pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.bundle_no in ($bundle_nos) and c.production_type=4 and c.status_active=1 and c.is_deleted=0 group by d.id, e.id, f.job_no_prefix_num,f.company_name, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.cut_no, c.bundle_no, e.po_number order by c.bundle_no desc";
...
if($action=="populate_bundle_data_update")
{
	...
	$sql_bundle_reject=sql_select("select a.defect_type_id,a.production_type,a.defect_point_id,a.defect_qty,a.bundle_no from  pro_gmts_prod_dft a where a.production_type=5 and defect_type_id =3 and bundle_no in ($bundle_nos)");
	 ...
	 foreach($sql_bundle_reject as $inf)
	 {
		...
		$check_arr[]=$inf[csf('bundle_no')]; 
	 }
	
	 $sql_bundle_reject=sql_select("select a.defect_type_id,a.production_type,a.defect_point_id,a.defect_qty,a.bundle_no from  pro_gmts_prod_dft a where a.production_type=5 and defect_type_id =4 and bundle_no in ($bundle_nos)");
	...
	?>
		<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i;?>"> 
			...
			<td width="50" id="altQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="alterQty[]" id="alterQty_<? echo $i; ?>" style="width:35px" value="<? echo $row[csf('alter_qty')];?>" onBlur="calculate_qcpasss(<? echo $i; ?>)" onKeyPress="return numOnly(this,event,this.id);" onDblClick="pop_entry_reject(<? echo $i; ?>)"/></td>
			<td width="50" id="sptQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="spotQty[]" id="spotQty_<? echo $i; ?>" style="width:35px" value="<? echo $row[csf('spot_qty')];?>" onBlur="calculate_qcpasss(<? echo $i; ?>)" onKeyPress="return numOnly(this,event,this.id);" onDblClick="pop_entry_spot(<? echo $i; ?>)"/></td>
			...
			<td id="button_1" align="center">
				...
				<input type="hidden" name="prod_qty[]" id="prod_qty_<? echo $i; ?>" value="<? echo $row[csf('qty')]; ?>"/>
				<input type="hidden" name="dtlsId[]" id="dtlsId_<? echo $i; ?>"/>
				<input type="hidden" name="actual_reject[]" id="actual_reject_<? echo $i; ?>"  value="<? echo $bundle_alter_data[$row[csf('bundle_no')]]; ?>"  />
				<input type="hidden" name="actual_spot[]" id="actual_spot_<? echo $i; ?>"  value="<? echo $bundle_spot_data[$row[csf('bundle_no')]]; ?>"  />
			</td>
		</tr>
	<?
		$i--;
...
{
	$bundle_nos=return_library_array( "select b.bundle_no, b.bundle_no from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id and a.delivery_mst_id=$data and b.production_type=5 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","bundle_no","bundle_no");
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$txt_system_id)=="")
		{
			...
			$new_sys_number=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'SWO', date("Y",time()), 5, "select sys_number_prefix, sys_number_prefix_num from pro_gmts_delivery_mst where company_id=$cbo_company_name and production_type='5' and $year_cond=".date('Y',time())." order by id desc ", "sys_number_prefix", "sys_number_prefix_num"));
			...
			$data_array_delivery="(".$mst_id.",'".$new_sys_number[1]."','".$new_sys_number[2]."','".$new_sys_number[0]."', ".$cbo_company_name.",5,".$cbo_location.",".$delivery_basis.",".$cbo_source.",".$cbo_emb_company.",".$cbo_floor.",".$cbo_line_no.",".$txt_organic.",".$txt_issue_date.",".$cbo_working_company_name.",".$cbo_working_location.",".$user_id.",'".$pc_date_time."')";
		...
		if(str_replace("'","",$delivery_basis)==3)
		{
			...
			$field_array_dtls="id,delivery_mst_id,mst_id,production_type,color_size_break_down_id,production_qnty,reject_qty,alter_qty,spot_qty,replace_qty,cut_no,bundle_no";
			...
			foreach($dtlsArr as $bundle_no=>$qty)
			{
				...
				$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",5,'".$dtlsArrColorSize[$bundle_no]."','".$qty."','".$dtlsArrRej[$bundle_no]."','".$dtlsArrAlt[$bundle_no]."','".$dtlsArrSpot[$bundle_no]."','".$dtlsArrRep[$bundle_no]."','".$cut_no."','".$bundle_no."')";
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		if(str_replace("'","",$delivery_basis)==3)
		{
			...
			$rejectDelete=execute_query("DELETE FROM pro_gmts_prod_dft WHERE mst_id in (".$production_mst_id.") and defect_type_id in(3,4) and production_type=5");
		...
		<legend>Enter search words</legend>           
            <table cellpadding="0" cellspacing="0" width="700" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td align="center">
                    	<input type="text" name="txt_issue_date" id="txt_issue_date" value="" class="datepicker" style="width:100px;"  />       
                    </td>
                    <td align="center" id="emb_company_td">				
                    	<?
							echo create_drop_down( "cbo_emb_company", 120, $line_library,"", 1, "--- Select ---", $selected, "" );  
						?>       
                    </td> 	
            		<td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_line_no').value+'_'+document.getElementById('txt_issue_date').value+'_'+document.getElementById('txt_company_id').value+'_'+document.getElementById('cbo_emb_company').value+'_'+document.getElementById('txt_order_no').value, 'create_challan_search_list_view', 'search_div', 'bundle_wise_sewing_output_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                     </td>
...
<script src="../../includes/functions_bottom.js" type="text/javascript"></script>
<script>
load_drop_down('bundle_wise_sewing_output_controller', 3, 'load_drop_down_embro_issue_source', 'emb_company_td');
    ...
    <table width="900" cellspacing="0" align="right">
        ...
        <?
			...
			if($data[2]==3)
			{
				...
				and a.id=b.mst_id and b.color_size_break_down_id=d.id and b.production_type=5 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 
				and d.is_deleted=0 
			...
			else
			{
				...
				and c.id=a.mst_id  and a.color_size_break_down_id=b.id and b.production_type=5 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
