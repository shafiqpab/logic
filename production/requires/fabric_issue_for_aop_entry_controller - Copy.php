...
if ($action=="generate_aop_booking")
{
	...
	{ 
		$sql= "select f.id,f.company_id, f.buyer_id,a.job_no,c.style_ref_no,c.job_no as fso_number, c.within_group, c.sales_booking_no, e.batch_no, e.extention_no, b.color_id, b.body_part_id , b.fabric_desc, b.gsm_weight, b.dia ,b.id as so_dtls_id,e.id as batch_id,g.id as dtls_id, g.process_type_id, g.quantity, g.rate, g.amount, g.number_of_roll, sum(d.batch_qnty) as batch_weight, g.remark  from 
		wo_pre_cost_fabric_cost_dtls a, fabric_sales_order_dtls b, fabric_sales_order_mst c, pro_batch_create_dtls d , pro_batch_create_mst e, wo_booking_mst f, wo_fabric_aop_dtls g ,product_details_master  h where a.id=b.pre_cost_fabric_cost_dtls_id and b.mst_id=c.id and c.id=d.po_id and d.mst_id=e.id and f.tagged_booking_no = c.sales_booking_no and g.sales_order_dtls_id=b.id and f.id=g.order_id and g.batch_id=e.id and e.color_id=b.color_id and d.prod_id=h.id and h.item_description=b.fabric_desc and f.booking_type=3 and f.process=35 and g.mst_id=$mst_id and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1 and g.status_active=1 and g.is_deleted=0 group by  f.id,f.company_id, f.buyer_id,a.job_no,c.style_ref_no,c.job_no, c.within_group, c.sales_booking_no, e.batch_no, e.extention_no, b.color_id, b.body_part_id , b.fabric_desc, b.gsm_weight, b.dia ,b.id,e.id,g.id, g.process_type_id, g.quantity, g.rate, g.amount, g.number_of_roll, g.remark order by b.id desc";
				    ...
				    foreach($sys_no_data as $supplier_id=>$row)
				    {
						if ($i%2==0)$bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
				     	...
				     	<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value('<? echo $row['order_no'];?>' , '<? echo $issue_id;?>')" >
							...
							<td width="170" style="word-break:break-all"><?php echo $comp[$company_id]; ?></td>
							<td width="140" style="word-break:break-all"><?php echo $sys_no; ?></td>
							<td width="170" style="word-break:break-all"><?php echo $supplier_name_arr[$supplier_id]; ?></td>
...
if ($action=="create_issue_search_list_view_old")
{
	//echo load_html_head_contents("Booking PopUp","../../", 1, 1, $unicode,'','');
	$data=explode('_',$data);
	...
	$sql= "select a.id, a.entry_form, a.sys_no_prefix, a.sys_no_prefix_num, a.sys_no, a.sys_date, a.delivery_date, a.company_id, a.buyer_id, a.issue_purpose, a.source, a.order_id, a.order_no, a.attention, a.vehical_no, a.driver_name, a.dl_no, a.transport, a.mobile_no, a.gate_pass_no, a.is_short, a.remark ,a.supplier_id from wo_fabric_aop_mst a where $company and entry_form=462 and a.is_deleted=0 and a.status_active=1 $order_no $sys_no $sys_date order by a.id desc";
	    ...
	    foreach($result as $row)
	    {
			if ($i%2==0)$bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
	     	...
	     	<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value('<? echo $row[csf("order_no")];?>' , '<? echo $row[csf("id")];?>')" >
				...
				<td width="130" style="word-break:break-all"><?php echo $row[csf("sys_no")]; ?></td>
				<td width="140" style="word-break:break-all"><?php echo $supplier_arr[$row[csf("supplier_id")]]; ?></td>
...
if ($action=="service_booking_popup")
{
	...
	function js_set_value(booking_no,job_no,within_group)
	{
		//alert(within_group);
        ...
        <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            <table width="100%" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all">
                ...
                <tr class="general">
                    ...
                    <td><input name="txt_date_from" id="txt_date_from" class="datepicker" style="width:60px"></td>
                    <td><input name="txt_date_to" id="txt_date_to" class="datepicker" style="width:60px"></td>
                    <td><input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_supplier_name').value+'_'+document.getElementById('txt_booking_prifix').value+'_'+document.getElementById('cbo_search_category').value+'_'+document.getElementById('txt_style').value+'_'+document.getElementById('cbo_within_group').value, 'create_booking_search_list_view', 'search_div', 'fabric_issue_for_aop_entry_controller','setFilterGrid(\'tbl_po_list\',-1)')" style="width:80px;" /></td>
                </tr>
                <tr>
                    <td align="center" valign="middle" colspan="11"><? echo load_month_buttons(1); ?></td>
                </tr>
    ...
    <div style="width:1000px; max-height:240px;overflow-y:scroll;">
	    ...
	    foreach($result as $row)
	    {
			if ($i%2==0)$bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
	     	//$booking_no="'".$row[csf("booking_no")]."'";
	     	?>
	     	<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value('<? echo $row[csf("booking_no")];?>' , '<? echo $row[csf("job_no")];?>', '<? echo $row[csf("within_group")];?>')" >
				<td width="30" align="center"><?php echo $i; ?></td>
...
if ($action=="populate_issue_data_from_search_popup")
{
	...
	$sql= "select a.id, a.entry_form, a.sys_no_prefix, a.sys_no_prefix_num, a.sys_no, a.sys_date, a.delivery_date, a.company_id, a.buyer_id, a.issue_purpose, a.source, a.order_id, a.order_no, a.attention, a.vehical_no, a.driver_name, a.dl_no, a.transport, a.mobile_no, a.gate_pass_no, a.is_short, a.remark,a.supplier_id,a.supplier_address,within_group,lc_company_name,fab_booking_no from wo_fabric_aop_mst a where a.id=$data and a.is_deleted=0 ";
