...
{
/* 	echo create_drop_down( "cbo_floor", 180, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process in (9) order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/bundle_wise_sewing_output_test_controller', $('#cbo_company_name').val()+'_'+$('#cbo_location').val()+'_'+this.value, 'load_drop_down_line', 'line_td' );",0 );
*/
 	echo create_drop_down( "cbo_floor", 180, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process=5 order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/bundle_wise_sewing_output_test_controller', $('#cbo_emb_company').val()+'_'+$('#cbo_location').val()+'_'+this.value+'_'+$('#txt_issue_date').val(), 'load_drop_down_line', 'line_td' );",0 );
	exit();
}
...
if($action=="load_drop_down_embro_issue_source")
{
	...
	if($data==3)
	{
		if($db_type==0)
		{
			echo create_drop_down( "cbo_emb_company", 180, "select id,supplier_name from lib_supplier where status_active=1 and is_deleted=0 and find_in_set(23,party_type) order by supplier_name","id,supplier_name", 1, "--- Select ---", $selected, "load_drop_down( 'requires/bundle_wise_sewing_output_test_controller', this.value, 'load_drop_down_location', 'location_td' );get_php_form_data(this.value,'load_variable_settings','requires/bundle_wise_sewing_output_test_controller');get_php_form_data(this.value,'load_variable_settings_for_working_company','requires/bundle_wise_sewing_output_test_controller');load_html();" );
		}
		else
		{
			echo create_drop_down( "cbo_emb_company", 180, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type in(22,23) and a.status_active=1 group by a.id,a.supplier_name order by a.supplier_name","id,supplier_name", 1, "--Select--", 0, "load_drop_down( 'requires/bundle_wise_sewing_output_test_controller', this.value, 'load_drop_down_location', 'location_td' );get_php_form_data(this.value,'load_variable_settings','requires/bundle_wise_sewing_output_test_controller');get_php_form_data(this.value,'load_variable_settings_for_working_company','requires/bundle_wise_sewing_output_test_controller');load_html();" );
		}
	}
	else if($data==1)
		echo create_drop_down( "cbo_emb_company", 180, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1 $company_cond order by company_name","id,company_name", 1, "--- Select ---", $selected_company, "load_drop_down( 'requires/bundle_wise_sewing_output_test_controller', this.value, 'load_drop_down_location', 'location_td' );get_php_form_data(this.value,'load_variable_settings','requires/bundle_wise_sewing_output_test_controller');get_php_form_data(this.value,'load_variable_settings_for_working_company','requires/bundle_wise_sewing_output_test_controller');load_html();",0,0 );
	else
		...
		<legend>Enter search words <input type="checkbox" value="1" name="is_exact" id="is_exact"  checked> is exact</legend>
            <table cellpadding="0" cellspacing="0" width="550" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td><input type="text" name="bundle_no" id="bundle_no" style="width:120px" class="text_boxes" /></td>
            		<td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view (document.getElementById('txt_order_no').value+'_'+<? echo $company; ?>+'_'+document.getElementById('bundle_no').value+'_'+'<? echo trim($bundleNo,','); ?>'+'_'+document.getElementById('txt_job_no').value+'_'+document.getElementById('txt_cut_no').value+'_'+document.getElementById('cbo_cut_year').value+'_'+$('#is_exact').is(':checked')+'_'+'<? echo trim($line_id,','); ?>', 'create_bundle_search_list_view', 'search_div', 'bundle_wise_sewing_output_test_controller', 'setFilterGrid(\'tbl_list_search\',-1);reset_hide_field();')" style="width:100px;" />
                     </td>
                </tr>
           </table>
           <div style="width:100%; margin-top:5px; margin-left:10px" id="search_div" align="left"></div>
...
if($action=="load_mst_data")
{
	...
	if(count($sql_mst_data)>0)
	{
		...
		//	$cutting_no=return_field_value("cut_no", "pro_garments_production_dtls", "barcode_no in ($bundle_nos)");
			...
			echo "load_drop_down_multiple( 'requires/bundle_wise_sewing_output_test_controller', '".$val[csf('serving_company')]."_".$val[csf('location_id')]."_".$val[csf('floor_id')]."_".$data[2]."', 'load_drop_down_location_and_floor', 'location_td*line_td' );\n";

			// echo "load_drop_down( 'requires/bundle_wise_sewing_output_test_controller', '".$val[csf('serving_company')]."', 'load_drop_down_location', 'location_td' );\n";

			// echo "load_drop_down( 'requires/bundle_wise_sewing_output_test_controller','".$val[csf('serving_company')]."_".$val[csf('location_id')]."_".$val[csf('floor_id')]."_".$data[2]."', 'load_drop_down_line', 'line_td');\n";
	...
	foreach ($result as $row)
	{
		...
		?>
			<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i;?>">
                ...
                <td width="50" id="rejQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="rejectQty[]" <? echo $disabled;?> id="rejectQty_<? echo $i; ?>" style="width:35px" value="" onBlur="calculate_qcpasss(<? echo $i; ?>)" onKeyPress="return numOnly(this,event,this.id);"/></td>
                <td width="50" id="altQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="alterQty[]" <? echo $disabled;?> id="alterQty_<? echo $i; ?>" style="width:35px" value="" onBlur="calculate_qcpasss(<? echo $i; ?>)" onKeyPress="return numOnly(this,event,this.id);" onDblClick="pop_entry_reject(<? echo $i; ?>)"/></td>
...
if($action=="populate_bundle_data_update")
{
	...
	?>
		<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i;?>">
			...
			<td width="50" id="altQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="alterQty[]" id="alterQty_<? echo $i; ?>" style="width:35px; <? echo $colo; ?>" value="<? echo $row[csf('alter_qty')];?>" onBlur="calculate_qcpasss(<? echo $i; ?>)" onKeyPress="return numOnly(this,event,this.id);" onDblClick="pop_entry_reject(<? echo $i; ?>)"/></td>
			...
			<td id="button_1" align="center">
				...
				<input type="hidden" name="prod_qty[]" id="prod_qty_<? echo $i; ?>" value="<? echo $row[csf('qty')]; ?>"/>
				<input type="hidden" name="dtlsId[]" id="dtlsId_<? echo $i; ?>" value="<? echo $row[csf('prdid')]; ?>"/>
				<input type="hidden" name="actual_reject[]" id="actual_reject_<? echo $i; ?>"  value="<? echo $bundle_alter_data[$row[csf('barcode_no')]]; //$row[csf('reject_qty')]; // ?>"  />
	...
	<div style="width:100%;max-height:180px; overflow-y:scroll" id="sewing_production_list_view" align="left">
		...
		<?php
			...
			foreach($sqlResult as $selectResult)
			{
 			...
 			?>
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_issue_form_data','requires/bundle_wise_sewing_output_test_controller');" >
...
if($action=="populate_issue_form_data")
{
	...
	foreach($sqlResult as $result)
	{
		...
		echo "get_php_form_data(".$result[csf('po_break_down_id')]."+'**'+".$result[csf("item_number_id")]."+'**'+".$result[csf("embel_name")]."+'**'+".$result[csf("country_id")].", 'populate_data_from_search_popup', 'requires/bundle_wise_sewing_output_test_controller' );\n";

		echo "$('#cbo_item_name').val('".$result[csf('item_number_id')]."');\n";
		echo "$('#cbo_country_name').val('".$result[csf('country_id')]."');\n";

		echo "show_list_view('".$result[csf('po_break_down_id')]."','show_country_listview','list_view_country','requires/bundle_wise_sewing_output_test_controller','');\n";
...
if ($action=="save_update_delete")
{
	...
	if(!$prod_reso_allocation){$prod_reso_allocation=2;}
			...
			for($j=1;$j<=$tot_row;$j++)
			{
				...
				if($$actual_reject!="")
				{
					...
					for( $rls=0; $rls<count($actual_reject_info); $rls++ )
					{
						...
						$data_array_defect.="(".$dft_id.",".$dtlsId.",5,".$$orderId.",3,".$defectPointId.",'".$defect_qty."','".$bundleBarcodeArr[$$bundleNo]."',".$user_id.",'".$pc_date_time."')";
						//$dft_id = $dft_id+1;
					}
				}
				...
				if($$actual_spot!="")
				{
					...
					for( $rls=0; $rls<count($actual_spot_info); $rls++ )
					{
						...
						if( trim($data_array_defect)!="") $data_array_defect.=",";
						$dft_id = return_next_id_by_sequence(  "pro_gmts_prod_dft_seq", "pro_gmts_prod_dft", $con );
						$data_array_defect.="(".$dft_id.",".$dtlsId.",5,".$$orderId.",4,".$defectPointId.",'".$defect_qty."','".$bundleBarcodeArr[$$bundleNo]."',".$user_id.",'".$pc_date_time."')";
						//$dft_id = $dft_id+1;
					}
				}
			}
...
<script src="../../includes/functions_bottom.js" type="text/javascript"></script>
<script>
	load_drop_down('bundle_wise_sewing_output_test_controller','<?php echo $cbo_source; ?>_<?php echo $cbo_serving_company; ?>'  , 'load_drop_down_sewing_company', 'emb_company_td');
...
if($action=='populate_data_from_challan_popup')
{
	...
	{
		//echo "load_drop_down( 'requires/bundle_wise_sewing_output_test_controller', $('#cbo_company_name').val()+'_'+".$row[csf('location_id')]."+'_'+".$row[csf('floor_id')].", 'load_drop_down_line', 'line_td' );\n";

	    echo "load_drop_down( 'requires/bundle_wise_sewing_output_test_controller',".$row[csf("serving_company")]."+'_'+" . $row[csf('location_id')] . "+'_'+" . $row[csf('floor_id')] . "+'_'+'" . change_date_format($row[csf('delivery_date')]) . "', 'load_drop_down_line', 'line_td' );\n";

		echo "load_drop_down( 'requires/bundle_wise_sewing_output_test_controller','".$row[csf('working_company_id')]."', 'load_drop_down_working_location', 'working_location_td' );\n";
		echo "load_drop_down( 'requires/bundle_wise_sewing_output_test_controller', '".$row[csf('location_id')]."', 'load_drop_down_floor', 'floor_td' );\n";
		...
		echo "load_drop_down( 'requires/bundle_wise_sewing_output_test_controller', '".$row[csf('production_source')]."', 'load_drop_down_embro_issue_source', 'emb_company_td' );\n";

		echo "load_drop_down( 'requires/bundle_wise_sewing_output_test_controller', '".$row[csf('serving_company')]."', 'load_drop_down_location', 'location_td' );\n";
