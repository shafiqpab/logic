...
if($action=="load_drop_down_wash_company")
{
	...
	if($data==3)
	{
		...
		echo create_drop_down( "cbo_emb_company", 170, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1 $company_cond order by company_name","id,company_name", 1, "--- Select ---", "", "load_drop_down( 'requires/gmts_rcve_from_wash_v2_controller', this.value, 'load_drop_down_wash_location', 'wash_location_td' );",0,0 );
	else
...
{
	$explode_data = explode("*",$data);
	$receive_location = $explode_data[0];
	$receive_company = $explode_data[1]; 
 	echo create_drop_down( "cbo_floor", 170, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id=$receive_location and production_process=5 order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "",0 );
	exit();
}
...
if($action=="create_po_search_list_view")
{
	...
	if(trim($source)!="") $source_cond .= " and production_source=$source";
	$total_rcve_qty_data_arr=array();
	$total_rcve_qty_arr=sql_select( "SELECT po_break_down_id, item_number_id, country_id,production_type, sum(production_quantity) as production_quantity from pro_garments_production_mst where status_active=1 and is_deleted=0 and production_type in(2,3,5) and embel_name in(0,3) and po_break_down_id in($allPoIds) $source_cond group by po_break_down_id, item_number_id, country_id,production_type");
	

	foreach($total_rcve_qty_arr as $row)
	{
		$total_rcve_qty_data_arr[$row['PO_BREAK_DOWN_ID']][$row['ITEM_NUMBER_ID']][$row['COUNTRY_ID']][$row['PRODUCTION_TYPE']]+=$row['PRODUCTION_QUANTITY'];
	}
	?>
            ...
            foreach( $result as $row )
            {
				...
				for($k=0; $k<$numOfItem; $k++)
				{
					...
					foreach($country as $country_id)
					{
						...
						$total_sew_out = $total_rcve_qty_data_arr[$row['ID']][$grmts_item][$country_id][5];
						$total_issue = $total_rcve_qty_data_arr[$row['ID']][$grmts_item][$country_id][2];
						$total_rcve = $total_rcve_qty_data_arr[$row['ID']][$grmts_item][$country_id][3];
						$balance= $total_issue - $total_rcve;
						...
						<tr bgcolor="<?=$bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value(<?=$row["ID"];?>,'<?=$grmts_item;?>','<?=$po_qnty;?>','<?=$plan_cut_qnty;?>','<?=$country_id;?>');" >
							...
							<td width="110" style="word-break: break-all;"><?=$garments_item[$grmts_item];?></td>
							...
							<td width="60" align="right"><?= $total_issue?></td>
                            <td width="80" align="right"><?= $total_rcve; ?></td>
                            <td width="60" align="right" title="<?= "balance= Total Issue Qty($total_issue) - Total Receive Qty($total_rcve)";?>"><?= $balance; ?></td>
...
if($action=="populate_data_from_search_popup")
{
	...
	$source_cond = $source ? " and b.production_source=$source " : '';  ;
	$data_sql = "SELECT 
		...
		SUM ( CASE WHEN a.production_type = 3 AND b.embel_name = 3 and a.entry_form=648 THEN a.production_qnty ELSE 0 END) AS total_wash_rcve,
		SUM ( CASE WHEN a.production_type = 2 AND b.embel_name = 3 and a.entry_form=645 THEN a.production_qnty ELSE 0 END) AS total_wash_issue
		FROM pro_garments_production_dtls a, pro_garments_production_mst b
		WHERE a.status_active = 1
		and b.status_active=1  and a.is_deleted=0 and  b.is_deleted=0 and a.mst_id=b.id and b.po_break_down_id=$po_id and b.item_number_id=$item_id and b.country_id=$country_id  $source_cond and a.production_type in(1,2,3,5) " ;
		// echo "<br>". $data_sql ."<br>";
	...
	{	
		...
		$ttl_wash_rcve	= $row['TOTAL_WASH_RCVE'];
		$ttl_wash_issue	= $row['TOTAL_WASH_ISSUE'];
		echo "$('#txt_cut_qty').val('".$ttl_cut_qty."');\n";
		echo "$('#txt_sewing_qty').val('".$ttl_sew_out."');\n";
		echo "$('#ttl_issue_qty').val('".$ttl_wash_issue."');\n";
		echo "$('#txt_cum_rcve_qty').attr('placeholder','".$ttl_wash_rcve."');\n";
		echo "$('#txt_cum_rcve_qty').val('".$ttl_wash_rcve."');\n";
		
		$yet_to_wash = $ttl_wash_issue - $ttl_wash_rcve;
...
if($action=="color_and_size_level")
{
	...
	{
		$source_cond = $source ? " and c.production_source= $source ": "";
		$sql = "SELECT a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty,
				sum(CASE WHEN c.production_type=2 and c.entry_form=645 then b.production_qnty ELSE 0 END) as production_qnty,
				sum(CASE WHEN c.production_type=3 and c.embel_name=3 and c.entry_form=648 then b.production_qnty ELSE 0 END) as cur_production_qnty,
				sum(CASE WHEN c.production_type=3 and c.entry_form=648 then b.reject_qty ELSE 0 END) as reject_qty
				...
				where a.po_break_down_id=$po_id and a.item_number_id=$item_id and a.country_id=$country_id $source_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by a.item_number_id, a.color_number_id";
	...
	else if( $variableSettings==3 ) //color and size level
	{
		...
		$dtlsData = sql_select("SELECT a.color_size_break_down_id,
			sum(CASE WHEN a.production_type=2 and a.entry_form=645 then a.production_qnty ELSE 0 END) as production_qnty,
			sum(CASE WHEN a.production_type=3 and a.entry_form=648 then a.production_qnty ELSE 0 END) as cur_production_qnty,
			sum(CASE WHEN a.production_type=3 and a.entry_form=648 then a.reject_qty ELSE 0 END) as reject_qty
			from pro_garments_production_dtls a,pro_garments_production_mst b where a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.embel_name=3 and b.country_id='$country_id' $source_cond and a.color_size_break_down_id!=0 and a.production_type in(2,3) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  group by a.color_size_break_down_id");
	...
	else // by default color and size level
	{
		...
		$dtlsData = sql_select("SELECT a.color_size_break_down_id,
			...
			sum(CASE WHEN a.production_type=3 and a.entry_form=648 then a.production_qnty ELSE 0 END) as cur_production_qnty,
			sum(CASE WHEN a.production_type=3 and a.entry_form=648 then a.reject_qty ELSE 0 END) as reject_qty
			from pro_garments_production_dtls a,pro_garments_production_mst b where a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' $source_cond  and b.embel_name=3 and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in(2,3)  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.color_size_break_down_id");
...
if ($action=="save_update_delete")
{
	...
	{  
		...
		$con = connect(); 
		$cbo_received_location = explode("*",str_replace("'","",$cbo_received_location));
		$sending_location = $cbo_received_location[0];
		$sending_company = $cbo_received_location[1];
		// ========================================================================================
										//Create System ID 
		// ========================================================================================
		if (str_replace("'", "", $txt_system_no) == "") 
		{
            $year_cond="to_char(insert_date,'YYYY')";
          	...
          	$new_sys_number = explode("*", return_next_id_by_sequence("", "pro_gmts_delivery_mst",$con,1,$cbo_company_name,'GRW',648,date("Y",time()),0,0,2,3,0 ));
            ...
            $data_array_delivery = "(" . $mst_id . ",'" . $new_sys_number[1] . "','" .(int) $new_sys_number[2] . "','" . $new_sys_number[0] . "', " . $cbo_company_name . ", " . $cbo_buyer_name . ",3,3," . $cbo_source . "," . $txt_rcve_date . ",648,".$cbo_emb_company.",".$cbo_wash_location.",".$cbo_location_name."," . $cbo_floor.",'". $sending_company."',". $sending_location."," . $txt_remark . "," . $user_id . ",'" . $pc_date_time . "')";
        ...
        else 
        {
            ...
            // $data_array_delivery = "" . $cbo_company_name . "*" . $cbo_source . "*" . $cbo_floor . "*" . $txt_rcve_date . "*" . $cbo_emb_company . "*" . $cbo_wash_location . "*" . $user_id . "*'" . $pc_date_time . "'";
            $data_array_delivery = "".$cbo_company_name."*". $cbo_source."*". $txt_rcve_date."*".$cbo_emb_company."*".$cbo_wash_location."*".$cbo_location_name."*" . $cbo_floor."*'". $sending_company."'*'". $sending_location."'*'".$remark."'*".$user_id."*'".$pc_date_time."'";
		...
		// ========================================================================================
		$field_array1="id,delivery_mst_id, company_id, garments_nature, challan_no, po_break_down_id, item_number_id, country_id, production_source, serving_company, location, embel_name, embel_type, production_date, production_quantity,reject_qnty, production_type, entry_break_down_type, remarks, floor_id, total_produced, yet_to_produced, inserted_by, insert_date, sending_location, sending_company, entry_form, status_active, is_deleted";
		$data_array1="(".$id.",".$mst_id.",".$cbo_company_name.",".$garments_nature.",".$challan_no.",".$hidden_po_break_down_id.", ".$cbo_item_name.",".$cbo_country_name.", ".$cbo_source.",".$cbo_emb_company.",".$cbo_wash_location.",3,".$cbo_embel_type.",".$txt_rcve_date.",".$txt_rcve_qty.",".$txt_reject_qty.",3,".$sewing_production_variable.",".$txt_remark.",".$cbo_floor.",".$txt_cum_rcve_qty.",".$txt_yet_to_rcve.",".$user_id.",'".$pc_date_time."','".$sending_location."','".$sending_company."',648,1,0)";
		...
		$dtlsData = sql_select("select a.color_size_break_down_id,
			...
			sum(CASE WHEN a.production_type=3 and b.embel_name=$embelName then a.production_qnty ELSE 0 END) as cur_production_qnty
			from pro_garments_production_dtls a,pro_garments_production_mst b
			where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name and b.country_id=$cbo_country_name and a.color_size_break_down_id!=0 and a.production_type in(5,3)
		...
		if(str_replace("'","",$sewing_production_variable)==2)//color level wise
		{
			...
			foreach($rowEx as $rowE=>$val)
			{
				...
				if($colSizeID_arr[$colorSizeNumberIDArr[0]]!="")
				{
					...
					if($j==0) $data_array = "(".$dtls_id.",".$id.",3,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."',648,1,0)";
					else $data_array .= ",(".$dtls_id.",".$id.",3,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."',648,1,0)";
					//$dtls_id=$dtls_id+1;
					$j++;
				}
			}
		...
		if(str_replace("'","",$sewing_production_variable)==3)//color and size wise
		{
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				if($colSizeID_arr[$index]!="")
				{
					...
					//3 for Receive From Print / Emb Entry
					$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq", "pro_garments_production_dtls", $con );
					if($j==0)$data_array = "(".$dtls_id.",".$id.",3,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."',648,1,0)";
					else $data_array .= ",(".$dtls_id.",".$id.",3,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."',648,1,0)";
					//$dtls_id=$dtls_id+1;
					$j++;
				}
			}
		...
		// echo  "10**".$rID ."**". $dtlsrID ."**". $challanrID;
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		// =============================================================================================
		$field_array_delivery = "company_id*production_source*delivery_date*working_company_id*working_location_id*location_id*floor_id*sending_company*sending_location*remarks*updated_by*update_date"; 
		$data_array_delivery = "".$cbo_company_name."*". $cbo_source."*". $txt_rcve_date."*".$cbo_emb_company."*".$cbo_wash_location."*".$cbo_location_name."*" . $cbo_floor."*'". $sending_company."'*'". $sending_location."'*".$txt_remark."*".$user_id."*'".$pc_date_time."'";

		// pro_garments_production_mst table data entry here
		
 		$field_array1="production_source*serving_company*location*embel_type*production_date*production_quantity*reject_qnty*production_type*entry_break_down_type*remarks*floor_id*total_produced*yet_to_produced*updated_by*update_date*sending_location*sending_company";
		

		$data_array1="".$cbo_source."*".$cbo_emb_company."*".$cbo_wash_location."*".$cbo_embel_type."*".$txt_rcve_date."*".$txt_rcve_qty."*".$txt_reject_qty."*3*".$sewing_production_variable."*".$txt_remark."*".$cbo_floor."*".$txt_cum_rcve_qty."*".$txt_yet_to_rcve."*".$user_id."*'".$pc_date_time."'*'".$sending_location."'*'".$sending_company."'";
		...
		{
			$dtlsData = sql_select("select a.color_size_break_down_id,
				sum(CASE WHEN a.production_type=2 and b.embel_name=3 and b.entry_form =645 then a.production_qnty ELSE 0 END) as production_qnty,
				sum(CASE WHEN a.production_type=3 and b.embel_name=3 and b.entry_form =648 then a.production_qnty ELSE 0 END) as cur_production_qnty
				from pro_garments_production_dtls a,pro_garments_production_mst b
				where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name and b.country_id=$cbo_country_name and a.color_size_break_down_id!=0 and a.production_type in(5,3) and b.id<>$txt_mst_id
				group by a.color_size_break_down_id");
			...
			if(str_replace("'","",$sewing_production_variable)==2)//color level wise
			{
				...
				foreach($rowEx as $rowE=>$val)
				{
					...
					if($colSizeID_arr[$colorSizeNumberIDArr[0]]!="")
					{
						...
						$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq", "pro_garments_production_dtls", $con );
						if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",3,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."',648,1,0)";
						else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",3,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."',648,1,0)";
						//$dtls_id=$dtls_id+1;
						$j++;
					}
				}
			...
			if(str_replace("'","",$sewing_production_variable)==3)//color and size wise
			{
				...
				foreach($rowEx as $rowE=>$valE)
				{
					...
					if($colSizeID_arr[$index]!="")
					{
						//2 for Issue to Print / Emb Entry
						$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq", "pro_garments_production_dtls", $con );
						if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",3,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."',648,1,0)";
						else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",3,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."',648,1,0)";
						//$dtls_id=$dtls_id+1;
						$j++;
					}
				}
		...
		// echo "10**".$hidden_po_break_down_id ."". $challanrID;die;
		...
		else
		{
			...
			{
				oci_rollback($con);
				echo "10**".str_replace("'","",$hidden_po_break_down_id)."**mst*".$rID ."**dtls*". $dtlsrID ."**cln*". $challanrID."**else";
			}
		}
		disconnect($con);
		die;
...
{
	$data=explode("**",$data); 
	$sqlResult =sql_select("SELECT id, company_id, challan_no, po_break_down_id, item_number_id, country_id, production_source, serving_company, location, embel_name, embel_type, production_date, production_quantity, reject_qnty,production_source, production_type, entry_break_down_type, remarks, floor_id, total_produced, yet_to_produced, sending_location, sending_company from pro_garments_production_mst where id='".$data[0]."' and production_type=3 and status_active=1 and is_deleted=0 order by id");

	foreach($sqlResult as $result)
	{
		$sending_location 	= $result['SENDING_LOCATION'];
		$sending_company 	= $result['SENDING_COMPANY'];
		$cbo_received_location = "$sending_location"."*"."$sending_company" ; 

		$po_company =  $result[csf('company_id')]; 
		echo "$('#txt_rcve_date').val('".change_date_format($result[csf('production_date')])."');\n";
		echo "$('#cbo_source').val('".$result[csf('production_source')]."');\n";
		echo "load_drop_down( 'requires/gmts_rcve_from_wash_v2_controller', ".$result[csf('production_source')].", 'load_drop_down_wash_company', 'emb_company_td' );\n";
		echo "$('#cbo_emb_company').val('".$result[csf('serving_company')]."');\n";
		echo "$('#cbo_company_name').val('".$po_company."');\n"; 
	    ...
	    echo "load_drop_down( 'requires/gmts_rcve_from_wash_v2_controller', ".$po_company.", 'load_drop_down_po_location', 'location_td' );\n";
		echo "load_drop_down( 'requires/gmts_rcve_from_wash_v2_controller',`".$cbo_received_location."`, 'load_drop_down_floor', 'floor_td' );\n";
		echo"load_drop_down( 'requires/gmts_rcve_from_wash_v2_controller', ".$result['SERVING_COMPANY'].", 'load_drop_down_wash_location', 'wash_location_td' );\n"; 
		echo "$('#cbo_floor').val('".$result[csf('floor_id')]."');\n";
		echo "$('#cbo_embel_name').val('".$result[csf('embel_name')]."');\n";
		echo "$('#cbo_received_location').val(`".$cbo_received_location."`);\n";
		echo "load_drop_down( 'requires/gmts_rcve_from_wash_v2_controller', '".$result[csf('embel_name')].'**'.$result[csf('po_break_down_id')]."', 'load_drop_down_embro_issue_type', 'embro_type_td' );\n";
		//$result[csf('po_break_down_id')]
		echo "$('#cbo_embel_type').val('".$result[csf('embel_type')]."');\n";

  		echo "$('#txt_rcve_qty').val('".$result[csf('production_quantity')]."');\n";
	...
	$system_arr = sql_select("SELECT id,sys_number,working_location_id,location_id from pro_gmts_delivery_mst where id=$data[2] and production_type=3 and embel_name=3  and entry_form=648 and status_active=1 and is_deleted=0");
	...
	$data_sql = "SELECT 
		SUM ( CASE WHEN a.production_type = 1 THEN a.production_qnty ELSE 0 END) AS total_cut_qty,
		SUM ( CASE WHEN a.production_type = 5 THEN a.production_qnty ELSE 0 END) AS total_sew_out,
		SUM ( CASE WHEN a.production_type = 2 AND b.embel_name = 3 and a.entry_form=645 THEN a.production_qnty ELSE 0 END) AS total_issue,
		SUM ( CASE WHEN a.production_type = 3 AND b.embel_name = 3 and a.entry_form=648 THEN a.production_qnty ELSE 0 END) AS total_receive,
		SUM ( CASE WHEN a.production_type = 3 AND b.embel_name = 3 and a.entry_form=648 THEN a.reject_qty ELSE 0 END) AS total_reject
		FROM pro_garments_production_dtls a, pro_garments_production_mst b
		WHERE a.status_active = 1
		and b.status_active=1  and a.is_deleted=0 and  b.is_deleted=0 and a.mst_id=b.id and b.po_break_down_id=$po_id and b.item_number_id=$item_id and b.country_id=$country_id and a.production_type in(1,2,5,3) " ;
		// echo $data_sql; die;
	$dataArray=sql_select($data_sql); 
	foreach($dataArray as $row)
	{	
 	...
 	foreach($res as $result)
	{
		echo "$('#txt_order_no').val('".$result['PO_NUMBER']."');\n";
		echo "$('#hidden_po_break_down_id').val('".$result['ID']."');\n";
		// echo "$('#cbo_buyer_name').val('".$result['BUYER_NAME']."');\n";
		echo "$('#txt_style_no').val('".$result['STYLE_REF_NO']."');\n";
		echo "$('#txt_job_no').val('".$result['JOB_NO']."');\n";
		echo "$('#txt_ir').val('".$result['GROUPING']."');\n";
		echo "$('#txt_country_ship_date').val('".$result['COUNTRY_SHIP_DATE']."');\n";
	...
	echo "$('#txt_rcve_date').attr('disabled','disabled');\n";
	echo "set_button_status(1, permission, 'fnc_rcve_print_embroidery_entry',1,1);\n";  
	...
	if( $variableSettings!=1 ) // gross level
	{
		...
		if( $variableSettings==2 ) // color level
		{
			...
			$sql = "SELECT a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty,
				sum(CASE WHEN c.production_type=2 and c.entry_form=645 then b.production_qnty ELSE 0 END) as production_qnty,
				sum(CASE WHEN c.production_type=3 and c.entry_form=648 then b.production_qnty ELSE 0 END) as cur_production_qnty,
				sum(CASE WHEN c.production_type=3 and c.entry_form=648 then b.reject_qty ELSE 0 END) as reject_qty
				from wo_po_color_size_breakdown a
				left join pro_garments_production_dtls b on a.id=b.color_size_break_down_id
				left join pro_garments_production_mst c on c.id=b.mst_id
		...
		{
			$dtlsData = sql_select("SELECT a.color_size_break_down_id,
				sum(CASE WHEN a.production_type=2 and b.entry_form=645 then a.production_qnty ELSE 0 END) as production_qnty,
				sum(CASE WHEN a.production_type=3 and b.entry_form=648 then a.production_qnty ELSE 0 END) as cur_production_qnty,
				sum(CASE WHEN a.production_type=3 and b.entry_form=648 then a.reject_qty ELSE 0 END) as reject_qty
...
if($action=="system_number_popup")
{
	...
	<script>		
		function js_set_value(str)
		{
			...
			console.log($("#hidden_search_data").val());
			parent.emailwindow.hide();
		}
    </script>
	<?
	        ...
	        <table ellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
	            ...
	            <tbody>
	                <tr>
						...
						<td>
							<input type="text" style="width:100px" class="text_boxes"  name="txt_receive_no" id="txt_receive_no" />
...
if($action=="create_system_number_list_view")
{
    ...
    $rcve_id 	= trim($ex_data[4]);
	$order_no 	= trim($ex_data[5]);
    $txt_date_from = $ex_data[6];
	$txt_date_to = $ex_data[7];  
	$source = $ex_data[8];   //Data need to pass from main page
	if ($source==3) 
	{
	...
	if($rcve_id!="")
	{
		$sql_cond .= " and a.sys_number like '%".$rcve_id."'";
	}
	if($order_no!="")
	{
	...
	$sql ="SELECT a.id,a.company_id,c.buyer_name as buyer,c.job_no,c.style_ref_no as style,b.po_number,a.sys_number,a.sys_number_prefix_num as rcve_id,d.production_quantity as rcve_qty,d.serving_company as wash_comp,d.production_date as rcve_date  from pro_gmts_delivery_mst a, wo_po_break_down b, wo_po_details_master c,pro_garments_production_mst d where a.id=d.delivery_mst_id and b.job_id=c.id and b.id=d.po_break_down_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and a.production_type=3 and a.embel_name='3' and a.entry_form=648 $sql_cond  order by a.id DESC";
...
if($action=="populate_mst_form_data")
{
	...
	echo"load_drop_down( 'requires/gmts_rcve_from_wash_v2_controller', ".$result[0]['PRODUCTION_SOURCE'].", 'load_drop_down_wash_company', 'cbo_emb_company' );\n";
	echo"load_drop_down( 'requires/gmts_rcve_from_wash_v2_controller', ".$result[0]['WORKING_COMPANY_ID'].", 'load_drop_down_wash_location', 'cbo_wash_location' );\n";
	echo"load_drop_down( 'requires/gmts_rcve_from_wash_v2_controller',`".$cbo_received_location."`, 'load_drop_down_floor', 'cbo_floor' );\n";
	echo "load_drop_down( 'requires/gmts_rcve_from_wash_v2_controller', `".$result[0]['COMPANY_ID']."`, 'load_drop_down_buyer', 'buyer_td' );\n";
	...
	echo "$('#cbo_location_name').attr('disabled','disabled');\n"; 
	echo "$('#txt_rcve_date').attr('disabled','disabled');\n";
	echo "set_button_status(0, permission, 'fnc_rcve_print_embroidery_entry',1,0);\n";
 	exit();
}
if($action=="wash_receive_print")
{
	extract($_REQUEST);
	$data			= explode('*',$data);
	$lc_company 	= $data[0];
	...
	foreach ($sql_res as  $v) 
	{
		...
		$data_arr[$v['PO_ID']][$v['COLOR']]['RECEIVE_QTY'] 		+= $v['PROD_QTY'];
		...
		elseif ($prod_variable ==3) //color and Size Level
		{
			...
			$prod_arr[$v['PO_ID']][$v['COLOR']][$v['SIZE_NUMBER_ID']]['RECEIVE_QTY'] += $v['PROD_QTY'];
	...
	//echo "<pre>"; print_r($supplier_address);die;
	...
	<div style="width:<?= $width+20?>px;"> 
		<div style="display: flex;">
			...
			<div style="text-align:center;width:<?= $width-100?>px">
				...
				<p style="font-size:18px;margin:0;"><strong>Receive Delivery Challan-<?= $sys_no ?></strong></p>
			</div> 
		</div>
								...
								<tr>
									...
									<?
										if ($prod_variable==3) //Color Size Level
										{
											...
											{
												$size_qty = $prod_arr[$po_id][$color_id][$size]['RECEIVE_QTY'];
												?>
									...
									?>
									<td align="center"><?= $v['RECEIVE_QTY']?> </td>
								</tr>
							<?
						}
		...
		<br>
		 <?
            echo signature_table(322, $data[0], "900px","",10,$inserted_by);
         ?>
	</div>
	<?
	exit();
