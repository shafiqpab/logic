        ...
        function js_set_value(str)
        {
            ...
            if (jQuery.inArray($('#txt_individual' + str).val(), selected_id) == -1) {
                ...
                for (var i = 0; i < selected_id.length; i++) {
                    if (selected_id[i] == $('#txt_individual' + str).val())
                        break;
                }
...
if ($action == "create_bundle_search_list_view") {
    ...
    $scanned_bundle_arr = return_library_array("select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=3 and a.embel_name=4 and b.status_active=1 and b.is_deleted=0", 'bundle_no', 'bundle_no');

    foreach (explode(",", $selectedBuldle) as $bn) {
        $scanned_bundle_arr[$bn] = $bn;
    }
	
    if ($cut_no != '') $cutCon = " and c.cut_no = '" . trim($company_short_arr[$company] . '-' . $cut_no) . "'";
    if ($job_no != '') $jobCon = " and f.job_no_prefix_num = $job_no"; $jobCon = "";

    $sql = "select c.cut_no, c.bundle_no, d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.production_qnty as qty, e.po_number from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e ,wo_po_details_master f where d.job_no_mst=f.job_no and  a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and a.company_id=$company and e.po_number like '$txt_order_no' and c.bundle_no like '$bundle_no' $jobCon $cutCon and c.production_type=2 and a.embel_name=4 and a.status_active=1 and a.is_deleted=0 order by c.cut_no, c.bundle_no DESC";
...
if($action=="challan_duplicate_check")
{
	...
	$msg=1;
	$result=sql_select("select a.sys_number,b.bundle_no from pro_gmts_delivery_mst a,pro_garments_production_dtls b where a.id=b.delivery_mst_id and b.bundle_no in ($bundle_no) and b.production_type=3 and a.embel_name=4 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  group by a.sys_number,b.bundle_no");
	$datastr="";
	if(count($result)>0)
	{
		foreach ($result as $row)
...
if ($action == "populate_bundle_data_update") {
    ...
    $issue_sql = "select d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, $year_field as year, f.buyer_name, d.item_number_id, d.country_id,c.reject_qty, d.size_number_id, d.color_number_id, c.bundle_no, c.production_qnty as qty, e.po_number from pro_garments_production_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name=$company_id and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.bundle_no in ($bundle_nos) and c.production_type=2 and a.embel_name=4 and c.status_active=1 and c.is_deleted=0";
    ...
    $sql = "select c.bundle_no,d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, $year_field as year, f.buyer_name, d.item_number_id, d.country_id,c.reject_qty, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no, c.production_qnty as qty, e.po_number from pro_garments_production_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name=$company_id and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.bundle_no in ($bundle_nos) and c.production_type=3 and a.embel_name=4 and c.status_active=1 and c.is_deleted=0 order by c.bundle_no DESC";
...
{
    $bundle_nos = return_library_array("select b.bundle_no, b.bundle_no from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id and a.delivery_mst_id=$data and b.production_type=2 and a.embel_name=4 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0", "bundle_no", "bundle_no");
...
{
    if ($db_type == 0) {
        $bundle_nos = return_field_value("group_concat(b.bundle_no order by b.id desc) as bundle_no", "pro_garments_production_mst a, pro_garments_production_dtls b", "a.id=b.mst_id and a.delivery_mst_id=$data and b.production_type=3 and a.embel_name=4 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0", 'bundle_no');
    } else if ($db_type == 2) {
...
if ($action == "save_update_delete") {
    ...
    if ($operation == 0) { // Insert Here----------------------------------------------------------
        ...
        if (str_replace("'", "", $txt_system_id) == "") {
            ...
            $new_sys_number = explode("*", return_mrr_number(str_replace("'", "", $cbo_company_name), '', 'BRS', date("Y", time()), 5, "select sys_number_prefix, sys_number_prefix_num from pro_gmts_delivery_mst where company_id=$cbo_company_name and production_type='3' and embel_name=4 and $year_cond=" . date('Y', time()) . " order by id desc", "sys_number_prefix", "sys_number_prefix_num"));
        ...
        if (str_replace("'", "", $delivery_basis) == 3) {
            ...
            $field_array_dtls = "id,delivery_mst_id,mst_id,production_type,color_size_break_down_id,production_qnty,reject_qty,cut_no,bundle_no";
            ...
            foreach ($dtlsArr as $bundle_no => $qty) {
                ...
                if ($data_array_dtls != "")
                    $data_array_dtls .= ",";
                $data_array_dtls.="(".$dtls_id.",".$mst_id.",".$gmtsMstId.",3,'".$dtlsArrColorSize[$bundle_no]."','".$qty."','".$dtlsArrRej[$bundle_no]."','".$cut_no."','".$bundle_no."')";
...
if ($action == "challan_no_popup") {
    ...
    ?>       
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view(document.getElementById('txt_search_common').value + '_' + document.getElementById('cbo_embel_type').value + '_' + document.getElementById('txt_company_id').value, 'create_challan_search_list_view', 'search_div', 'special_work_challan_receive_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                            </td>
...
if ($action == "load_mst_data") {
	...
	and a.embel_name=4 and a.status_active=1 and a.is_deleted=0  and c.status_active=1 and c.is_deleted=0
...
if ($action == "create_bundle_search_list_view") {
    ...
    $sql = "select d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.bundle_no, c.production_qnty as qty, e.po_number from  pro_gmts_delivery_mst a,pro_cut_delivery_color_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e where   a.id=c.delivery_mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and a.company_id=$company and e.po_number like '$txt_order_no' and c.bundle_no like '$bundle_no' and c.production_type=2 and c.embel_name=4 and a.status_active=1 and a.is_deleted=0  and c.status_active=1 and c.is_deleted=0";
    //echo $sql;die;
    $result = sql_select($sql);
    ?>
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="800" class="rpt_table">
...
if ($action == "bundle_receive_special_work") {
    ...
    if ($data[2] == 3) {
        ...
        $sql = "SELECT a.cut_no, a.country_id, b.id, b.reject_qty, b.production_qnty, e.buyer_name,  d.color_number_id, d.size_number_id, e.job_no, e.style_ref_no, e.style_description, f.id as po_id
				...
				where a.id=b.mst_id and b.color_size_break_down_id=d.id  and d.po_break_down_id=f.id and f.job_no_mst=e.job_no and f.id=a.po_break_down_id and a.delivery_mst_id ='$data[1]' and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0";
        //echo $sql; 
        $result = sql_select($sql);
