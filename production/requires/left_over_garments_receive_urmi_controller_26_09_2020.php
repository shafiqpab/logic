...
if($action=="create_po_search_list_view")
{
	...
	if($order_type==1) // Self Order Type
	{
		...
		if($goods_type==2) 
		{
			$sql_po_actv = "SELECT d.po_break_down_id as po_id,sum(e.$prod_qty_source) as sewing_out_qty from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, pro_garments_production_mst d, pro_garments_production_dtls e where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id and d.id=e.mst_id and b.id=d.po_break_down_id  and e.production_type in(5,11) and d.production_type=e.production_type and a.status_active=1 and b.status_active=1 and d.status_active=1 and e.status_active=1 and c.status_active=1 and b.shiping_status=3 and c.id=e.color_size_break_down_id $sql_cond group by d.po_break_down_id";
		} 
		else 
		{
			$sql_po_actv = "SELECT d.po_break_down_id as po_id,sum(e.$prod_qty_source) as sewing_out_qty from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, pro_garments_production_mst d, pro_garments_production_dtls e where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id and d.id=e.mst_id and b.id=d.po_break_down_id  and e.production_type=$prod_type_id and d.production_type=e.production_type and a.status_active=1 and b.status_active=1 and d.status_active=1 and e.status_active=1 and c.status_active=1 and b.shiping_status=3 and c.id=e.color_size_break_down_id $sql_cond group by d.po_break_down_id";
		}
		// echo $sql_po_actv;
		$sql_po_actv_res = sql_select($sql_po_actv);

		$po_arr = array();
		$poly_and_reject_qty_arr = array();
		foreach ($sql_po_actv_res as $val) 
		{
			$po_arr[$val[csf('po_id')]] = $val[csf('po_id')];
			$poly_and_reject_qty_arr[$val[csf('po_id')]]['poly_and_reject_pototal'] += $val[csf('sewing_out_qty')];			
		}
		...
		$sql_ex_fact = "SELECT a.po_break_down_id as po_id, sum(b.production_qnty) as ex_fact_qty from pro_ex_factory_mst a, pro_ex_factory_dtls b where a.id=b.mst_id $poCond and a.status_active=1 and b.status_active=1 group by a.po_break_down_id";
		// echo $sql_ex_fact;
		...
		$sql_sewing_out = "SELECT d.po_break_down_id as po_id,sum(e.$prod_qty_source) as sewing_out_qty from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, pro_garments_production_mst d, pro_garments_production_dtls e where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id and d.id=e.mst_id and b.id=d.po_break_down_id  and e.production_type=$prod_type_id and d.production_type=e.production_type and a.status_active=1 and b.status_active=1 and d.status_active=1 and e.status_active=1 and c.status_active in(1,3) and b.shiping_status in(1,2,3) $sql_cond group by d.po_break_down_id";//b.shiping_status=1
		// echo $sql_sewing_out;
		...
		$sql_sewing_out = "SELECT a.po_break_down_id as po_id,a.item_number_id as item_id,a.country_id, sum(b.$prod_qty_source) as sewing_out_qty from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id and a.po_break_down_id in($left_over_po_id) and a.item_number_id in($left_over_item_id) and a.company_id=$company and a.location=$working_location and a.serving_company=$working_company and b.production_type=$prod_type_id and a.production_type=b.production_type and a.status_active=1 and b.status_active=1 group by a.po_break_down_id,a.item_number_id,a.country_id";
		echo $sql_sewing_out;
		$sql_sewing_out_res = sql_select($sql_sewing_out);
         ...
         <div style="width:<? echo $tbl_width+20;?>px;max-height:240px;overflow-y:scroll;" >	 
			...
			foreach( $result as $row )
			{
				...
				if($goods_type == 1) {
					...
					$tmpBalance = $poly_and_reject_qty_arr[$row[csf('id')]][$row[csf('item_number_id')]];
					$balance = $tmpBalance ? $poly_and_reject_qty_arr[$row[csf('id')]]['poly_and_reject_pototal'] : 0;
				} else {
...
if($action=="get_left_over_balance")
{
	...
	if($goods_type==1) {
		$po_quantity_arr = sql_select("select id, po_quantity from wo_po_break_down where id = $po_id");
...
{	
	...
	if($order_type==1)
	{
		...
		if($goods_type == 2) {
			$sql_sewing_out = "SELECT a.po_break_down_id as po_id,a.item_number_id as item_id, a.country_id, sum(b.$prod_qty_source) as sewing_out_qty, a.reject_qnty
			from pro_garments_production_mst a, pro_garments_production_dtls b
			where a.id=b.mst_id and a.po_break_down_id=$po_id and b.production_type in(5,11) and a.company_id=$company_name and a.location=$working_location_name and a.serving_company=$working_company_name and a.production_type=b.production_type and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.po_break_down_id,a.item_number_id, a.country_id, a.reject_qnty";
		} else {
			$sql_sewing_out = "SELECT a.po_break_down_id as po_id,a.item_number_id as item_id, a.country_id, sum(b.$prod_qty_source) as sewing_out_qty, a.reject_qnty
			from pro_garments_production_mst a, pro_garments_production_dtls b
			where a.id=b.mst_id and a.po_break_down_id=$po_id and b.production_type=$prod_type_id and a.company_id=$company_name and a.location=$working_location_name and a.serving_company=$working_company_name and a.production_type=b.production_type and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.po_break_down_id,a.item_number_id, a.country_id, a.reject_qnty";
		}
		// echo $sql_sewing_out;
		$sql_sewing_out_res = sql_select($sql_sewing_out);
		...
		foreach ($sql_sewing_out_res as $val) 
		{
			...
			$poly_and_reject_qty_arr[$val[csf('po_id')]]['poly_and_reject_pototal'] += $val[csf('sewing_out_qty')];
			$poly_and_reject_qty_arr[$val[csf('po_id')]][$val[csf('item_id')]]['total'] += $val[csf('sewing_out_qty')];
		}
	    ...
	    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $tbl_width;?>" class="rpt_table">
			...
			foreach($sqlResult as $row)
			{
				...
				if($goods_type == 1) {
					$balance = $po_quantity_arr[$row[csf("po_break_down_id")]];
				} else if($goods_type == 2) {
					// $balance = "$sewing_out_qty, $exfact_out_qty, $delv_out_qty, $cumLft_qty";
					// $balance = $poly_and_reject_qty_arr[$row[csf("po_break_down_id")]]['poly_and_reject_pototal'];

					$tmpBalance = $poly_and_reject_qty_arr[$row[csf('po_break_down_id')]][$row[csf('item_number_id')]];
					$balance = $tmpBalance ? $poly_and_reject_qty_arr[$row[csf('po_break_down_id')]]['poly_and_reject_pototal'] : 0;
				} else {
...
{
	$sql ="SELECT a.id, a.sys_number, a.company_id, a.location, a.leftover_date, a.order_type, a.buyer_name, a.store_name, a.floor_id, a.garments_nature, a.goods_type, a.exchange_rate,a.working_company_id,a.working_location_id,a.floor_id, a.remarks from pro_leftover_gmts_rcv_mst a where a.id='$data' ";
	...
	echo"load_drop_down( 'requires/left_over_garments_receive_urmi_controller', ".$result[0][csf('working_location_id')].", 'load_drop_down_working_floor', 'working_floor' );\n";
	...
	echo "$('#cbo_working_floor_name').val('".$result[0][csf('floor_id')]."');\n";
		...
		foreach($colorResult as $color)
		{
			...
			else //color and size level
			{
				if($order_type==1) // Self Order
				{
					...
					$colorHTML .='<tr><td>'.$size_library[$color[csf("size_number_id")]].'</td><td><input type="text" name="colorSize" id="colSize_'.$color[csf("color_number_id")].($i+1).'"  class="text_boxes_numeric" placeholder="'.$color[csf("order_quantity")].'" style="width:100px" value="'.$amount.'" onblur="fn_total('.$color[csf("color_number_id")].','.($i+1).')"></td></tr>';
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$new_sys_number=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'LGR', date("Y",time()), 5, "select sys_number_prefix,sys_number_prefix_num from pro_leftover_gmts_rcv_mst where company_id=$cbo_company_name $mrr_cond order by id DESC ", "sys_number_prefix", "sys_number_prefix_num" ));
		 ...
		 $field_array_mst="id, sys_number_prefix, sys_number_prefix_num, sys_number, company_id, location, leftover_date, order_type, buyer_name, store_name, exchange_rate, goods_type,working_company_id,working_location_id,floor_id,remarks, inserted_by, insert_date"; 
