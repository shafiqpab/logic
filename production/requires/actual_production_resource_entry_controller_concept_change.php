...
if($action=="job_no_popup")
{
	...
	$prod_update_var=$sql_result[0][csf('sewing_production')];
	 ...
	 $data_style_ref_arr=array_unique(explode(",",$data_style_ref));
	 $data_style_ref=implode(",",array_unique(explode(",",$data_style_ref)));
	 $data_po_id=implode(",",array_unique(explode(",",$data_po_id)));
	 $data_po_id_arr=array_unique(explode(",",$data_po_id));
	 $style_temp_arr=explode(",",$data_string);
	?>
	<script>
		var selected_id; var selected_name; var selected_data;
	
		<? 
			$jsstyle_temp_arr=json_encode($style_temp_arr);
			echo " selected_data = ". $jsstyle_temp_arr . ";\n";
			
			$jsdata_po_id_arr=json_encode($data_po_id_arr);
			echo " selected_id = ". $jsdata_po_id_arr . ";\n";
			
			$jsdata_style_ref_arr=json_encode($data_style_ref_arr);
			echo " selected_name = ". $jsdata_style_ref_arr . ";\n";
			
			?>
	
			//console.log(selected_data.toString());	
			$('#hide_po_id').val( '<? echo $data_po_id; ?>' );
			$('#hide_style_no').val( '<? echo $data_style_ref; ?>'  );
			$('#hide_all_data').val( '<? echo $data_string; ?>' );
		...
		function js_set_value( str, val) {
			...
			if (string!="") string=string.split("***");
			string[0]=string[0]+"_"+$('#txt_target' + str).val();
			
			if (string[1]!="") jstr=string[1].split("_");
			if(val==1)
			{
			...
			if( jQuery.inArray( string[0], selected_data ) == -1 ) {
				selected_id.push( jstr[0] );
				selected_name.push( jstr[1] );
				selected_data.push( string[0] );
			}
			else {
				for( var i = 0; i < selected_data.length; i++ ) {
					if( selected_data[i] == string[0] ) break;
				}
				selected_id.splice( i, 1 );
				selected_name.splice( i, 1 );
				selected_data.splice( i, 1 );
			}
			var id = ''; var name = ''; var data = ''; var itm = ''; var color = ''; var size = '';
			for( var i = 0; i < selected_data.length; i++ ) {
				id += selected_id[i] + ',';
			...
			var old_po_id=$('#txt_old_po_id').val();
			var old_data=$('#txt_old_row_id').val();

			$('#hide_po_id').val( id );
			$('#hide_style_no').val( name );
			$('#hide_all_data').val( data );
		}		
            ...
            <table width="790" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr class="general">
                        ...
                        <td align="center">
                        	<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view ('<? echo $company_name; ?>'+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('cbo_year').value+'**'+document.getElementById('hide_prod_update_var').value+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('txt_date_from').value+'**'+document.getElementById('txt_date_to').value+'**'+'<? echo $data_string; ?>'+'**'+'<? echo $sid; ?>'+'**'+'<? echo $from_date; ?>'+'**'+'<? echo $to_date; ?>'+'**'+'<? echo $data_po_id; ?>'+'**'+'<? echo $data_style_ref; ?>', 'create_job_no_search_list_view', 'search_div', 'actual_production_resource_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1)'); set_all();" style="width:100px;" />
                    </td>
                    </tr>
                    <tr>
...
if($action=='create_job_no_search_list_view')
{
	...
	$form_date=$exdata[6];
	$to_date=$exdata[7];
	$data_string=$exdata[8];
	$line=$exdata[9];
	
	$reform_date=$exdata[10];
	$reto_date=$exdata[11];
	$data_po_id=$exdata[12];
	$data_style_ref=$exdata[13];
	
	$data_style_ref_arr=array_unique(explode(",",$data_style_ref));
	$data_po_id_arr=array_unique(explode(",",$data_po_id));
	$style_temp_arr=explode(",",$data_string);
	foreach($style_temp_arr  as $r_val)
	{
		$re_string_arr=explode("_",$r_val);
		$real_string=$re_string_arr[0]."_".$re_string_arr[1]."_".$re_string_arr[2]."_".$re_string_arr[3]."_".$re_string_arr[4];
		$style_real_arr[$real_string]=$re_string_arr[5];
	}
	if($db_type==0)
	{ 
            ...
            <table cellspacing="0" width="1060" border="1" rules="all" class="rpt_table">
                <thead>
                    <tr>
                        ...
                        <th width="">Target/Line/Hr</th>
                    </tr>
                </thead>
            </table>  
        </div>         
        <div style="width:1060px; max-height:260px; overflow-y:auto;">
            <table cellspacing="0" width="1042"  border="1" rules="all" class="rpt_table" id="tbl_list_search" >
                <tbody>  
				<?
                //echo $sql;
                ...
                foreach($sql_data as $row)
				{
					...
					$row_select='';
					$row_select=$row[csf('id')].'_'.$row[csf('item_number_id')].'_'.$row[csf('country_id')].'_'.$row[csf('color_number_id')].'_'.$row[csf('size_number_id')].'***'.$row[csf('id')].'_'.$job_no;
					...
					$new_style_arr[]=$job_no;
					?>    
					<tr  id="tr_<? echo $sl; ?>" data-sl="<? echo $sl; ?>" data-prod-id="<? echo $is_prod; ?>" style="cursor:pointer;background-color:<? echo $bgcolor ; ?>">
                        <td width="30"><? echo $sl; ?><input type="hidden" name="txt_individual_str" id="txt_individual_str<?php echo $sl ?>" value="<? echo $row_select; ?>"/></td>
                        ...
                        <td align="right" class="not_clickable"><input type="text" class="text_boxes"  style=" width:60px;"id="txt_target<?php echo $sl ?>" onKeyUp="calculate_target_hour(<?php echo $sl ?>)" name="txt_target[]" value="<?php  echo $style_real_arr[$str]; ?>" /></td>
					</tr>
					<? 
					$sl++; 
				} 
				...
				$str_style_list='';
				foreach($style_temp_arr as $val)
				{
					if(in_array($val,$list_style_arr))
					{
						if($str_style_list=="") $str_style_list="";
					}
					else
					{
						if($str_style_list=="") $str_style_list=$val; else $str_style_list.=','.$val;
					}
				}
				$str_po_list="";
				
				foreach($data_po_id_arr as $pval)
				{
					if(in_array($pval,$new_po_id_arr))
					{
						if($str_po_list=="") $str_po_list="";
					}
					else
					{
						if($str_po_list=="") $str_po_list=$pval; else $str_po_list.=','.$pval;
					}
				}
				?>
                <input type="hidden" name="txt_select_row_id" id="txt_select_row_id" value="<?php echo $style_temp_id; ?>"/>
                <input type="hidden" name="txt_old_row_id" id="txt_old_row_id" value="<?php echo $str_style_list; ?>"/>
                <input type="hidden" name="txt_old_po_id" id="txt_old_po_id" value="<?php echo $str_po_list; ?>"/>
                <tbody>
            </table>
        </div> 
...
{  $flag=0;
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		{	
			...
			$field_array3="id, mst_id, from_date, to_date, man_power, operator, helper, line_chief, active_machine, target_per_hour, working_hour, po_id, capacity,target_efficiency, total_smv, inserted_by, insert_date, is_deleted";
		
		
			
			$po_id=implode(",",array_filter(array_unique(explode(',',str_replace("'","",$txt_style_ref)))));
			
			$total_smv=((str_replace("'","",$txt_man_power)*str_replace("'","",$txt_working_hour))*60);
			$data_array3="(".$id3.",".$id.",".$txt_date_from.",".$txt_date_to.",".$txt_man_power.",".$txt_operator.",".$txt_helper.",".$txt_Line_chief.",".$txt_active_machine.",".$txt_target_hour.",".$txt_working_hour.",'".$po_id."',".$txt_capacity.",".$txt_efficiency.",".$total_smv.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',0)";
			...
			for($i=0; $i < $date_diff ; $i++)
			{
				...
				$field_array="id, mast_dtl_id,mst_id, pr_date, man_power, operator, helper, line_chief, active_machine, target_per_hour, working_hour, po_id, total_smv, inserted_by, insert_date, is_deleted";     
				if($i!=0)$data_array.=",";
				$data_array.="(".$id2.",".$id3.",".$id.",'".$dd."',".$txt_man_power.",".$txt_operator.",".$txt_helper.",".$txt_Line_chief.",".$txt_active_machine.",".$txt_target_hour.",".$txt_working_hour.",'".$po_id."',".$total_smv.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',0)";
				$id2++;
			}
		...
		$field_colSiz="id, mst_id, dtls_id, po_id, gmts_item_id, country_id, color_id, size_id,target_per_line, inserted_by, insert_date";
		if(str_replace("'","",$txt_data_string)!='')
		{
			$colSiz_data_string=explode(",",str_replace("'","",$txt_data_string));
			
			$data_colSiz="";
			foreach($colSiz_data_string as $data_string)
			{
				$po_id=$item_id=$color_id=$size_id=0;
				$ex_data=explode("_",$data_string);
				$po_id=$ex_data[0];
				$item_id=$ex_data[1];
				$country_id=$ex_data[2];
				$color_id=$ex_data[3];
				$size_id=$ex_data[4];
				$target_line=$ex_data[5];
				if($data_colSiz!="") $data_colSiz.=",";
				$data_colSiz.="(".$id_colSiz.",".$id.",".$id3.",'".$po_id."','".$item_id."','".$country_id."','".$color_id."','".$size_id."','".$target_line."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				$id_colSiz = $id_colSiz+1;
			}
		}
		
		//echo $data_colSiz;die;
		$rID=1;	$rID2=1; $rID3=1; $rID_time=true; $rID_colSiz=true;
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		/*if($txt_style_ref != $pre_po_id)
		{
			$sqls="select sewing_line from pro_garments_production_mst where status_active=1 and is_deleted=0 and production_type in(4,5) and location=$cbo_location and floor_id=$cbo_floor and production_date between $txt_date_from and $txt_date_to and sewing_line=$system_id and a.po_break_down_id in ($txt_style_ref)";
			//echo "505**".$txt_style_ref."==".$sqls;die;
			if(count(sql_select($sqls))>0) {echo "505**"; die;}
		}*/
		
		
		if(str_replace("'",'',$system_id)!="")
		{
			/* $resource=is_duplicate_field("a.resource_num","prod_resource_mst a,prod_resource_dtls b","a.id=b.mst_id and  b.mast_dtl_id!=".str_replace("'","",$h_dtl_mst_id)." and a.company_id=$cbo_company_name and a.location_id=$cbo_location and a.floor_id=$cbo_floor and a.line_number=$cbo_line_no and b.is_deleted=0 and b.pr_date between ".$txt_date_from." and ".$txt_date_to." and a.is_deleted=0 and b.is_deleted=0");*/
			$cbo_line=explode(',',str_replace("'","",$cbo_line_no));
			//echo "select a.id, a.line_number from prod_resource_mst a, prod_resource_dtls b where a.id=b.mst_id and b.mast_dtl_id!=".str_replace("'","",$h_dtl_mst_id)." and a.company_id=$cbo_company_name and a.location_id=$cbo_location and a.floor_id=$cbo_floor and b.pr_date between ".$txt_date_from." and ".$txt_date_to." and a.is_deleted=0 and b.is_deleted=0";
			...
			//echo "0**"."select a.resource_num from prod_resource_mst a, prod_resource_dtls b where a.id=b.mst_id and b.mast_dtl_id!=".str_replace("'","",$h_dtl_mst_id)." and a.company_id=$cbo_company_name and a.location_id=$cbo_location and a.floor_id=$cbo_floor and ( a.line_number=$cbo_line_no $line_cond) and b.pr_date between ".$txt_date_from." and ".$txt_date_to." and a.is_deleted=0 and b.is_deleted=0";
			$resource=return_field_value("a.resource_num","prod_resource_mst a,prod_resource_dtls b","a.id=b.mst_id and  b.mast_dtl_id!=".str_replace("'","",$h_dtl_mst_id)." and a.company_id=$cbo_company_name and a.location_id=$cbo_location and a.floor_id=$cbo_floor and a.line_marge=2 and ( a.line_number=$cbo_line_no $line_cond) and b.pr_date between ".$txt_date_from." and ".$txt_date_to." and a.is_deleted=0 and b.is_deleted=0 ","resource_num");
			if($resource!='' && $cbo_line_merge==2)
			{
				echo "6**".$resource;
			...
			$field_array_up3="from_date*to_date*man_power*operator*helper*line_chief*active_machine*target_per_hour*working_hour*po_id*capacity*target_efficiency*total_smv*updated_by*update_date*is_deleted";
			$po_id=implode(",",array_filter(array_unique(explode(',',str_replace("'","",$txt_style_ref)))));
		
			$total_smv=((str_replace("'","",$txt_man_power)*str_replace("'","",$txt_working_hour))*60);
			$data_array_up3="".$txt_date_from."*".$txt_date_to."*".$txt_man_power."*".$txt_operator."*".$txt_helper."*".$txt_Line_chief."*".$txt_active_machine."*".$txt_target_hour."*".$txt_working_hour."*'".$po_id."'*".$txt_capacity."*".$txt_efficiency."*".$total_smv."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*0";
			$field_array="id,mast_dtl_id,mst_id,pr_date,man_power,operator,helper,line_chief,active_machine,target_per_hour,working_hour,po_id,smv_adjust,smv_adjust_type ,total_smv,inserted_by,insert_date, is_deleted";
			...
			for($i=0; $i < $date_diff; $i++)
			{
			      ...
			      //echo strtotime($date);die;
				   ...
				   $resource_data_arr[$previous_resource[strtotime($date)]['id']]=explode("*",($id));
					if($i!=0)$data_array.=",";
					$data_array.="(".$id.",".$h_dtl_mst_id.",".$mst_id.",'".$dd."',".$txt_man_power.",".$txt_operator.",".$txt_helper.",".$txt_Line_chief.",".$txt_active_machine.",".$txt_target_hour.",".$txt_working_hour.",'".$po_id."','".$adjoustment_min."','".$adjustment_type."',".$total_smv.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',0)";
					$id++;
			}
		}
		
		//print_r($resource_data_arr);die;
		$field_array_time="id,mst_id,mast_dtl_id,shift_id,prod_start_time,lunch_start_time,remarks";
		$id_time= return_next_id( "id", "prod_resource_dtls_time", 1 );
		$save_string=explode(",",str_replace("'","",$save_string));
		foreach($save_string as $prevData)
		...
		execute_query("delete from prod_resource_color_size where mst_id=$mst_id and dtls_id=$h_dtl_mst_id");
		$id_colSiz= return_next_id( "id", "prod_resource_color_size", 1 );
		$field_colSiz="id, mst_id, dtls_id, po_id, gmts_item_id, country_id, color_id, size_id,target_per_line, inserted_by, insert_date";
		
		//$row[csf('id')].'_'.$row[csf('item_number_id')].'_'.$row[csf('country_id')].'_'.$row[csf('color_number_id')].'_'.$row[csf('size_number_id')]
		if(str_replace("'","",$txt_data_string)!='')
		{
			$colSiz_data_string=explode(",",str_replace("'","",$txt_data_string));
			$data_colSiz="";
			foreach($colSiz_data_string as $data_string)
			{
				...
				$target_line=$ex_data[5];
				if($data_colSiz!="") $data_colSiz.=",";
				$data_colSiz.="(".$id_colSiz.",".$mst_id.",".$h_dtl_mst_id.",'".$po_id."','".$item_id."','".$country_id."','".$color_id."','".$size_id."','".$target_line."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				$id_colSiz = $id_colSiz+1;
			}
		}
		//echo $data_colSiz;
		...
		//echo "10**".$rID_colSiz;die;	
		if($db_type==0)
		{
		 if($rID && $rID1 && $delete && $rID2 && $delete2 && $rID_time  && $rID_colSiz)
			{
				mysql_query("COMMIT");  
...
if($action=='create_acl_pdc_rec_entry_list_view')
{
	...
	foreach($sql_colSiz_res as $row)
	{
		$tot_rows++;
		$poIds.=$row[csf("po_id")].",";
		$actual_col_size_arr[$row[csf('dtls_id')]][$row[csf('po_id')]]['po_id'].=$row[csf('po_id')].',';
		$actual_col_size_arr[$row[csf('dtls_id')]][$row[csf('po_id')]]['item_id'].=$row[csf('gmts_item_id')].',';
		$actual_col_size_arr[$row[csf('dtls_id')]][$row[csf('po_id')]]['color_id'].=$row[csf('color_id')].',';
		$actual_col_size_arr[$row[csf('dtls_id')]][$row[csf('po_id')]]['size_id'].=$row[csf('size_id')].',';
	}
	//echo $poIds;die;
	...
	<form>
        ...
        <div style="width:1130px; max-height:180px; overflow-y:scroll;">
            <table cellspacing="0" width="1110"  border="1" rules="all" align="left" class="rpt_table" id="list_view" >
                <tbody>  
				<?
				$sql_dtls ="SELECT id, from_date, to_date, man_power, operator, helper, active_machine, target_per_hour,style_ref_id, working_hour, po_id FROM prod_resource_dtls_mast where mst_id=$data and is_deleted=0 order by from_date desc";
                //echo $sql_dtls;
                ...
                foreach($sql_data as $row)
				{
					...
					$ex_po_id=array_filter(array_unique(explode(',',$row[csf('po_id')])));
					...
					foreach($ex_po_id as $po_id)
					{
						...
						$ex_item='';
						$ex_item=array_filter(array_unique(explode(',',$actual_col_size_arr[$row[csf('id')]][$po_id]['item_id'])));
						foreach($ex_item as $item)
						{
							if($gmt_item=='') $gmt_item=$garments_item[$item]; else $gmt_item.=','.$garments_item[$item];
						}
						$ex_color='';
						$ex_color=array_filter(array_unique(explode(',',$actual_col_size_arr[$row[csf('id')]][$po_id]['color_id'])));
						...
						$ex_size='';
						$ex_size=array_filter(array_unique(explode(',',$actual_col_size_arr[$row[csf('id')]][$po_id]['size_id'])));
...
//echo $frm_d=change_date_format($form_date);die;
	...
	foreach($res as $result)
	{
		...
		//echo "$('#txt_style_ref_show').attr('disabled',true);\n";
		echo "$('#txt_style_ref').val('".$result[csf('po_id')]."');\n";
		...
		$sql_colSiz=sql_select("select id, mst_id, dtls_id, po_id, gmts_item_id, country_id, color_id, size_id,target_per_line from prod_resource_color_size where mst_id=".$result[csf('mst_id')]." and dtls_id=$id");
		if(count($sql_colSiz)>0)
		{
			$data_string='';
			foreach($sql_colSiz as $res)
			{//_ _0_ _
				if($res[csf('po_id')]!='')
				{
					if($data_string=='') $data_string=$res[csf('po_id')].'_'.$res[csf('gmts_item_id')].'_'.$res[csf('country_id')].'_'.$res[csf('color_id')].'_'.$res[csf('size_id')].'_'.$res[csf('target_per_line')]; else  $data_string.=','.$res[csf('po_id')].'_'.$res[csf('gmts_item_id')].'_'.$res[csf('country_id')].'_'.$res[csf('color_id')].'_'.$res[csf('size_id')].'_'.$res[csf('target_per_line')];
				}
			}
			echo "$('#txt_data_string').val('".$data_string."');\n";
		}
