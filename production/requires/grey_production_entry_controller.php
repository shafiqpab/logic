...
{
		//=====================================start==========Get Api Link================================================
		$api_data=sql_select("select company_id,user_id,api_link,id from lib_weight_machine_setup where user_id=$user_id and company_id=$data and  status=1 and status_active=1 and is_deleted=0");
...
if ($action=="load_weight_machine_data")
{
	...
	//  $url = "http://192.168.11.199";

	extract($_REQUEST);

	$api_data=sql_select("select company_id,user_id,api_link,id from lib_weight_machine_setup where user_id=$user_id and company_id=$company and  status=1 and status_active=1 and is_deleted=0");
	$api=$api_data[0]['API_LINK'];
...
//load_room_rack_self_bin('requires/grey_production_entry_controller*13', 'store','store_td', '$data',this.value);
...
if ($action == "load_drop_down_floor")
{
	...
	if ($action == "load_drop_machine")
	{
		...
		echo create_drop_down("cbo_machine_name", 132, "select id, machine_no as machine_name from lib_machine_name where category_id=1 and company_id=$company_id and status_active=1 and is_deleted=0 and is_locked=0 and is_subcon =2 $floor_cond $machine_cond order by seq_no", "id,machine_name", 1, "-- Select Machine --", 0, "", "");
		exit();
	}

	if ($action == "load_drop_machine_out_bound")
	{
		echo create_drop_down("cbo_machine_name", 132, "select id, machine_no as machine_name from lib_machine_name where category_id=1 and company_id=$data and status_active=1 and is_deleted=0 and is_locked=0 and is_subcon =1 order by seq_no", "id,machine_name", 1, "-- Select Machine --", 0, "", "");
		exit();
	}
				...
				<fieldset style="width:1030px; margin-left:3px">
					...
					<table cellpadding="0" cellspacing="0" border="1" rules="all" width="720" class="rpt_table">
						...
						<tr>
							...
							<td align="center">
								<input type="button" name="button2" class="formbutton" value="Show"	onClick="show_list_view ( document.getElementById('txt_search_common').value +'_'+document.getElementById('cbo_receive_basis').value +'_'+ document.getElementById('txt_company_id').value+'_'+'<? echo $cbo_knitting_source; ?>'+'_'+'<? echo $cbo_knitting_company; ?>'+'_'+'<? echo $txt_booking_no_id; ?>'+'_'+'<? echo $recieve_basis; ?>'+'_'+'<? echo $is_service_booking_mandatory; ?>', 'create_wo_no_production_search_list_view', 'search_div', 'grey_production_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1);')"
								style="width:100px;"/>
							</td>
						</tr>
					</table>
...
if ($action == "create_wo_no_production_search_list_view")
{
	...
	if (trim($data[0]) != "") {
		...
		$search_field_cond_wo = "and a.wo_no like '$search_string'";
	} else {
		$search_field_cond = "";
		$search_field_cond_wo="";
	}
	...
	group by s.id, s.prefix_num, s.booking_no, s.booking_date, s.buyer_id, s.supplier_id, s.item_category 
	union all
	SELECT a.id, a.wo_number_prefix_num as booking_no_prefix_num, a.wo_no as booking_no, a.booking_date, null as buyer_id, a.supplier_id, null as po_break_down_id, 13 as item_category, a.delivery_date, c.fabric_sales_order_no as job_no_mst, cast(c.program_no as varchar2(2000)) as program_no, 0 as type_id 
from knitting_work_order_dtls c,knitting_work_order_mst a, fabric_sales_order_mst b 
where c.mst_id=a.id and c.fabric_sales_order_no=b.job_no and a.company_id=$company_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $search_field_cond_wo $supplier_con and a.pay_mode not in(3,5) 
group by a.id, a.wo_number_prefix_num, a.wo_no, a.booking_date, a.supplier_id, c.id, a.delivery_date, c.fabric_sales_order_no, c.program_no
					...
					<tr>
						...
						<td align="center">
							...
							if ($recieve_basis == 1) {
								...
								$search_by_arr = array(1 => "Booking No", 2 => "Buyer Order", 3 => "Job No", 4 => "Fabric Desc.", 5 => "Program No", 6 => "Internal Ref", 7 => "File No", 8 => "Requisition No");
...
if ($action == "create_booking_search_list_view")
{
	...
	if ($recieve_basis == 1)
	{
		...
		$booking_type_cond = "";($variable_set_allocation==1)?" and a.booking_type in(1,4)":""; */

		//N.B. upper variable settings user (company) not found so, it is removed with decision of Mr. Raju
		$booking_type_cond = " and a.booking_type in(1,4)";
		if (trim($data[0]) != "" && ($search_by == 2 || $search_by == 3 || $search_by == 5 || $search_by == 6)) {
			$sql = "select a.id, a.booking_no,a.booking_no_prefix_num, a.booking_date, a.buyer_id,a.entry_form,c.po_break_down_id, a.item_category, a.delivery_date, c.job_no as job_no_mst, 0 as type,a.booking_type, a.is_short, $year_cond from wo_booking_mst a,wo_booking_dtls c, wo_po_break_down b where a.id=c.booking_mst_id and c.po_break_down_id=b.id and a.company_id=$company_id and a.buyer_id like '$buyer_id' and a.item_category=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and (a.fabric_source =1 or c.fabric_source=1) and a.ref_closing_status=0 $booking_type_cond $search_field_cond $booking_year_condition group by a.id,a.booking_no, a.booking_date, a.buyer_id,a.entry_form,c.po_break_down_id,a.item_category, a.delivery_date, c.job_no,a.insert_date,a.booking_no_prefix_num, a.booking_type,a.is_short order by a.id";
		}
		else
		{
			...
			WHERE s.booking_no=t.booking_no and s.company_id=$company_id and s.buyer_id like '$buyer_id' and s.status_active =1 and s.is_deleted=0 and t.status_active=1 and t.is_deleted=0 and s.item_category=2 and (s.fabric_source=1 OR t.fabric_source=1) and s.ref_closing_status=0 $search_field_cond_sample $booking_year_non_order_condition
		...
		<div style="width:1040px; max-height:280px; overflow-y:scroll" id="list_container_batch" align="left">
			...
			id="tbl_list_search">
				...
				foreach ($result as $row) {
					...
					?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"
						onClick="js_set_value(<? echo $row[csf('id')]; ?>,'<? echo $row[csf('job_no')]; ?>','0','<? echo $salesData; ?>','');">
	...
	else
	{
		...
		if ($sales_order == 1)
		{
			...
			if (trim($data[0]) != "") {
				if ($search_by == 1) {
					...
					$search_field_cond = "and e.requisition_no =" . trim($data[0]) ;
				} else {
			...
			if ($data[3])
			{
				$buyer_condition = " and a.buyer_id = " . $data[3];
			}
			...
			$sql = "SELECT a.id as plan_id, a.booking_no, d.job_no, a.within_group, d.style_ref_no, d.buyer_id,d.po_buyer as buy_name, c.body_part_id, a.determination_id, a.fabric_desc, a.gsm_weight, a.dia, b.fabric_dia, b.color_range, b.color_id, b.id as knit_id, b.knitting_source, b.knitting_party, b.stitch_length, b.machine_dia, b.machine_gg, b.is_sales, e.requisition_no, f.brand, f.yarn_count_id, f.lot, f.id as yarn_prod_id, c.po_id
			from ppl_planning_info_entry_mst a, ppl_planning_info_entry_dtls b left join ppl_yarn_requisition_entry e on b.id=e.knit_id and e.status_active=1 and e.is_deleted=0 left join product_details_master f on e.prod_id=f.id, ppl_planning_entry_plan_dtls c, fabric_sales_order_mst d
			where a.id=b.mst_id and b.id=c.dtls_id and c.po_id=d.id and a.company_id=$company_id $buyer_condition and a.status_active=1 and a.is_deleted=0 and (b.status_active=1 and b.is_deleted=0 OR b.active_for_production=1 ) and (c.status_active=1 and c.is_deleted=0 OR c.active_for_production=1 ) and d.status_active=1 and d.is_deleted=0 and b.is_sales=1 $search_field_cond $is_plan_revised_cond $fso_status_cond order by b.id desc";
			...
			foreach ($result as $row)
			{
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["plan_id"]=$row[csf("plan_id")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["knit_id"]=$row[csf("knit_id")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["booking_no"]=$row[csf("booking_no")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["job_no"]=$row[csf("job_no")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["within_group"]=$row[csf("within_group")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["style_ref_no"]=$row[csf("style_ref_no")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["buyer_id"]=$row[csf("buyer_id")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["buy_name"]=$row[csf("buy_name")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["body_part_id"]=$row[csf("body_part_id")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["determination_id"]=$row[csf("determination_id")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["fabric_desc"]=$row[csf("fabric_desc")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["gsm_weight"]=$row[csf("gsm_weight")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["dia"]=$row[csf("dia")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["fabric_dia"]=$row[csf("fabric_dia")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["color_range"]=$row[csf("color_range")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["color_id"]=$row[csf("color_id")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["knitting_source"]=$row[csf("knitting_source")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["knitting_party"]=$row[csf("knitting_party")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["stitch_length"]=$row[csf("stitch_length")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["machine_dia"]=$row[csf("machine_dia")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["machine_gg"]=$row[csf("machine_gg")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["is_sales"]=$row[csf("is_sales")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["requisition_no"] =$row[csf("requisition_no")];
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["brand"] .=$row[csf("brand")].",";
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["count"] .=$row[csf("yarn_count_id")].",";
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["lot"] .=$row[csf("lot")].",";
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["yarn_prod_id"] .=$row[csf("yarn_prod_id")].",";
				$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["po_id"] .=$row[csf("po_id")].",";
			}
			...
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1030" class="rpt_table">
				...
				<div style="width:1050px; max-height:280px; overflow-y:scroll" id="list_container_batch" align="left">
					...
					<?
					$i = 1;
					foreach ($data_array as $knit_id=> $knit_data)
					{
						foreach ($knit_data as $row)
						{
							if ($i % 2 == 0) $bgcolor = "#E9F3FF"; else $bgcolor = "#FFFFFF";

							$reqn_no = $row['requisition_no'];
							$job_no = $row['job_no'];
							...
							$color = chop($color, ',');
							$count = implode(",", array_unique(explode(",", chop($row['count'],','))));
							$yarn_lot_no = implode(",", array_unique(explode(",", chop($row['lot'],','))));
							$yarn_prod_id = implode(",", array_unique(explode(",", chop($row['yarn_prod_id'],','))));

							$body_part_type_id=$body_part_type_arr[$row['body_part_id']];
							$data = $row[csf('body_part_id')] . "**" . $row['determination_id'] . "**" . $row['fabric_desc'] . "**" . $row['gsm_weight'] . "**" . $row['fabric_dia'] . "**" . $job_no . "**" . $buyer . "**" . chop($row['po_id'],',') . "**" . $row['knitting_source'] . "**" . $row['knitting_party'] . "**" . $row['stitch_length'] . "**" . $row['color_range'] . "**" . $count . "**" . $yarn_lot_no . "**" . $brand_arr[chop($row['brand'],',')] . "**" . $color . "**" . $row['color_id'] . "**" . $row['machine_dia'] . "**" . $row['machine_gg'] . "**" . $row['within_group'] . "**" . $buyer_arr[$buyer] . "**" . $reqn_no. "**" . $yarn_prod_id. "**" . $sales_order. "**" . $body_part_type_id;
							?>
							<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"
								onClick="js_set_value(<? echo $row['knit_id']; ?>,'<? echo $data; ?>','0','','');">
								<td width="40"><? echo $i; ?></td>
								<td width="60" align="center"><? echo $row['plan_id']; ?></td>
								<td width="60" align="center"><? echo $row['knit_id']; ?></td>
								...
								<td width="150"><p><? echo $row['fabric_desc']; ?></p></td>
								<td width="60"><p><? echo $row['gsm_weight']; ?></p></td>
								<td width="60"><p><? echo $row['fabric_dia']; ?></p></td>
								<td><p><? echo $color_range[$row['color_range']]; ?></p></td>
							</tr>
							<?
							$i++;
						}
		...
		else
		{
			...
			if($bookingType == 0)
			{
				...
				$sql = "SELECT a.id as plan_id, a.booking_no, a.buyer_id, c.body_part_id, a.determination_id, a.fabric_desc, a.gsm_weight, a.dia, b.fabric_dia, b.color_range, b.color_id, b.id as knit_id, b.knitting_source, b.knitting_party, b.stitch_length, b.machine_dia, b.machine_gg, c.po_id, b.is_sales, e.requisition_no, f.brand, f.yarn_count_id, f.lot, f.id as yarn_prod_id, d.job_no_mst,d.po_number, d.grouping, d.file_no
				from ppl_planning_info_entry_mst a, ppl_planning_info_entry_dtls b
				left join ppl_yarn_requisition_entry e on b.id=e.knit_id and e.status_active=1 and e.is_deleted=0
				left join product_details_master f on e.prod_id=f.id,
				ppl_planning_entry_plan_dtls c left join wo_po_break_down d on c.po_id=d.id and c.is_sales=0
				where a.id=b.mst_id and b.id=c.dtls_id and a.company_id=$company_id $buyer_condition and a.status_active=1 and a.is_deleted=0  and (b.status_active=1 and b.is_deleted=0 OR b.active_for_production=1 )   and (c.status_active=1 and c.is_deleted=0 OR c.active_for_production=1 )  and b.is_sales != 1 $search_field_cond order by b.id desc, e.id asc";
				...
				foreach ($result as $row)
				{
					...
					$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["yarn_prod_id"] .=$row[csf("yarn_prod_id")].",";
					$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["po_id"] .=$row[csf("po_id")].",";
					$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["file_no"] .=$row[csf("file_no")].",";
					$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["grouping"] .=$row[csf("grouping")].",";
					$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["job_no_mst"] .=$row[csf("job_no_mst")].",";
					$data_array[$row[csf("knit_id")]][$row[csf("body_part_id")]]["po_number"] .=$row[csf("po_number")].",";
				}
				...
				<div style="width:1050px; max-height:280px; overflow-y:scroll" id="list_container_batch" align="left">
					...
					foreach ($data_array as $knit_id=>$knit_data)
					{
						foreach ($knit_data as $row)
						{
							...
							$po_id =implode(",", array_unique(explode(",",chop($row["po_id"],','))));
							$po_no =implode(",", array_unique(explode(",",chop($row["po_number"],','))));
							$job_no =implode(",", array_unique(explode(",",chop($row["job_no_mst"],','))));
							$internal_ref = implode(",", array_unique(explode(",", chop($row["grouping"],','))));
							$file_nos = implode(",", array_unique(explode(",", chop($row["file_no"],','))));

							$brand = '';
							$brand_id = explode(",", chop($row['brand'],','));
							...
							$body_part_type_id=$body_part_type_arr[$row['body_part_id']];
							$data = $row['body_part_id'] . "**" . $row['determination_id'] . "**" . $row['fabric_desc'] . "**" . $row['gsm_weight'] . "**" . $row['fabric_dia'] . "**" . $job_no . "**" . $row['buyer_id'] . "**" . $po_id . "**" . $row['knitting_source'] . "**" . $row['knitting_party'] . "**" . $row['stitch_length'] . "**" . $row['color_range'] . "**" . $count . "**" . $yarn_lot_no . "**" . $brand_arr[chop($row['brand'],',')] . "**" . $color . "**" . $row['color_id'] . "**" . $row['machine_dia'] . "**" . $row['machine_gg'] . "**0**" . $buyer_arr[$row['buyer_id']] . "**" . $reqn_no. "**" . $yarn_prod_id . "**" . $sales_order . "**" . $body_part_type_id;
							...
							?>
							<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"
								onClick="js_set_value(<? echo $row['knit_id']; ?>,'<? echo $data; ?>','<?php echo $bookinWithoutOrder; ?>','','');">
								<td width="30"><? echo $i; ?></td>
								<td width="55" align="center"><? echo $row['plan_id']; ?></td>
								<td width="55" align="center"><? echo $row['knit_id']; ?></td>
								<td width="50" align="center"><? echo $reqn_no; ?>&nbsp;</td>
								<td width="110"><p><? echo $row['booking_no']; ?></p></td>
								<td width="60"><p><? echo $buyer_arr[$row['buyer_id']]; ?></p></td>
								<td width="100"><p><? echo $po_no; ?></p></td>
								<td width="80"><p><? echo $job_no; ?></p></td>
								<td width="70"><p><? echo $internal_ref; ?>&nbsp;</p></td>
								<td width="70"><p><? echo $file_nos; ?>&nbsp;</p></td>
								<td width="130"><p><? echo $row['fabric_desc']; ?></p></td>
								<td width="55"><p><? echo $row['gsm_weight']; ?></p></td>
								<td width="55"><p><? echo $row['fabric_dia']; ?></p></td>
								<td><p><? echo $color_range[$row['color_range']]; ?></p></td>
							</tr>
							<?
							$i++;
			...
			else if($bookingType == 1)
			{
				...
				*/
				...
				$sql ="SELECT a.id as plan_id, a.booking_no, a.buyer_id, c.body_part_id, a.determination_id, a.fabric_desc, a.gsm_weight, a.dia, b.fabric_dia, b.color_range, b.color_id, b.id as knit_id, b.knitting_source, b.knitting_party, b.stitch_length,
				b.machine_dia, b.machine_gg,c.po_id , e.requisition_no, f.brand, f.yarn_count_id, f.lot, f.id as yarn_prod_id, d.job_no_mst,d.po_number, d.grouping, d.file_no
				from ppl_planning_info_entry_mst a, ppl_planning_info_entry_dtls b
				left join ppl_yarn_requisition_entry e on b.id=e.knit_id and e.status_active=1 and e.is_deleted=0
				left join product_details_master f on e.prod_id=f.id, ppl_planning_entry_plan_dtls c, wo_po_break_down d
				where a.id=b.mst_id and b.id=c.dtls_id and a.company_id=$company_id and c.po_id=d.id $buyer_condition and a.status_active=1 and a.is_deleted=0  and (b.status_active=1 and b.is_deleted=0 OR b.active_for_production=1 )   and (c.status_active=1 and c.is_deleted=0 OR c.active_for_production=1 )  and b.is_sales = 0 $search_field_cond
				order by b.id desc, e.id asc";
				$result = sql_select($sql);
				...
				<div style="width:1050px; max-height:280px; overflow-y:scroll" id="list_container_batch" align="left">
					...
					foreach ($data_array as $knit_id=>$knit_data)
					{
						foreach ($knit_data as $row)
						{
							...
							$brand = '';
							$brand_id = array_unique(explode(",", chop($row[csf('brand')],',')));
							foreach ($brand_id as $val) {
								if ($val > 0) $brand .= $brand_arr[$val] . ",";
							}
			...
			else
			{
				...
				/* $sql = "
					...
					$sql = "SELECT
						a.id as plan_id, a.booking_no, a.buyer_id, c.body_part_id, a.determination_id, a.fabric_desc, a.gsm_weight, a.dia, b.fabric_dia, b.color_range,
						b.color_id, b.id as knit_id, b.knitting_source, b.knitting_party, b.stitch_length, b.machine_dia, b.machine_gg,
						c.po_id, e.requisition_no, f.brand, f.yarn_count_id, f.lot, f.id as yarn_prod_id
					from
						ppl_planning_info_entry_mst a,
						ppl_planning_info_entry_dtls b left join ppl_yarn_requisition_entry e on b.id=e.knit_id and e.status_active=1 and e.is_deleted=0
						left join product_details_master f on e.prod_id=f.id ,
						ppl_planning_entry_plan_dtls c
					where
						a.id=b.mst_id and b.id=c.dtls_id and a.company_id = ".$company_id." and a.buyer_id like '".$buyer_id."' and a.status_active=1 and a.is_deleted=0  and (b.status_active=1 and b.is_deleted=0 OR b.active_for_production=1 )   and (c.status_active=1 and c.is_deleted=0 OR c.active_for_production=1 )  and b.is_sales = 2 $search_field_cond
					order by b.id desc, e.id asc";
				//echo $sql;die; //b.machine_dia,
				$result = sql_select($sql);
				...
				<div style="width:900px; max-height:280px; overflow-y:scroll" id="list_container_batch" align="left">
					...
					foreach ($data_array as $knit_id=>$knit_data)
					{
						foreach ($knit_data as $row)
						{
							...
							?>
							<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"
								onClick="js_set_value(<? echo $row['knit_id']; ?>,'<? echo $data; ?>','1','','');">
								<td width="30"><? echo $i; ?></td>
								<td width="55" align="center"><? echo $row['plan_id']; ?></td>
								<td width="55" align="center"><? echo $row['knit_id']; ?></td>
								<td width="50" align="center"><? echo $reqn_no; ?>&nbsp;</td>
								...
								<td width="200"><p><? echo $row['fabric_desc']; ?></p></td>
								<td width="55" align="center"><p><? echo $row['gsm_weight']; ?></p></td>
								<td width="55" align="center"><p><? echo $row['fabric_dia']; ?></p></td>
								<td><p><? echo $color_range[$row['color_range']]; ?></p></td>
							</tr>
							<?
							$i++;
...
if($action=="populate_barcode_data")
{
	...
	if ($db_type == 0) {
		$plan_details_array = return_library_array("select dtls_id, group_concat(distinct(po_id)) as po_id from ppl_planning_entry_plan_dtls where company_id=$company_id and dtls_id=$program_id group by dtls_id", "dtls_id", "po_id");
	} else {
		$plan_details_array = return_library_array("select dtls_id, LISTAGG(po_id, ',') WITHIN GROUP (ORDER BY po_id) as po_id from ppl_planning_entry_plan_dtls where company_id=$company_id and dtls_id=$program_id group by dtls_id", "dtls_id", "po_id");
	}



	//b.is_sales!=1
	$sql = "select a.id, a.booking_no, a.buyer_id, a.body_part_id, a.determination_id, a.fabric_desc, a.gsm_weight, a.dia, b.fabric_dia, b.color_range,
	b.color_id, b.id as knit_id, b.knitting_source, b.knitting_party, b.stitch_length, b.machine_dia, b.machine_gg,c.po_id from ppl_planning_info_entry_mst a, ppl_planning_info_entry_dtls b, ppl_planning_entry_plan_dtls c where a.id=b.mst_id and b.id=c.dtls_id and a.company_id=$company_id  and a.status_active=1 and a.is_deleted=0  and (b.status_active=1 and b.is_deleted=0 OR b.active_for_production=1 )   and (c.status_active=1 and c.is_deleted=0 OR c.active_for_production=1 )  and b.is_sales = 0 and b.id=$program_id  group by b.id, b.knitting_source, b.knitting_party, b.stitch_length, b.color_range, b.color_id, b.fabric_dia, b.machine_dia, b.machine_gg, a.id, a.booking_no, a.buyer_id, a.body_part_id, a.determination_id, a.fabric_desc, a.gsm_weight, a.dia,c.po_id order by b.id desc";
	//echo $sql; //b.machine_dia,
	$result = sql_select($sql);
	foreach ($result as $row) {
		$po_nos[] = $row[csf("po_id")];
	...
	foreach ($po_sql as $row) {
		...
		foreach ($result as $row) {
			...
			$count = implode(",", array_unique(explode(",", $rqsn_array[$row[csf('knit_id')]]['count'])));
			$body_part_type_id=$body_part_type_arr[$row[csf('body_part_id')]];
			$data = $row[csf('knit_id')]."##".$row[csf('body_part_id')] . "**" . $row[csf('determination_id')] . "**" . $row[csf('fabric_desc')] . "**" . $row[csf('gsm_weight')] . "**" . $row[csf('fabric_dia')] . "**" . $job_no . "**" . $row[csf('buyer_id')] . "**" . $plan_details_array[$row[csf('knit_id')]] . "**" . $row[csf('knitting_source')] . "**" . $row[csf('knitting_party')] . "**" . $row[csf('stitch_length')] . "**" . $row[csf('color_range')] . "**" . $count . "**" . $rqsn_array[$row[csf('knit_id')]]['lot'] . "**" . $brand_arr[$rqsn_array[$row[csf('knit_id')]]['brand']] . "**" . $color . "**" . $row[csf('color_id')] . "**" . $row[csf('machine_dia')] . "**" . $row[csf('machine_gg')] . "**0**" . $buyer_arr[$row[csf('buyer_id')]] . "**" . $reqn_no. "**" . $rqsn_array[$row[csf('knit_id')]]['yarn_prod_id'] . "**" . $sales_order . "**" . $body_part_type_id;
			echo $data;
			break;
		}
...
if ($action == 'populate_data_from_plan') //No Need
{
	...
	$program_qnty_array = sql_select("select sum(a.program_qnty) as qnty,location_id from ppl_planning_info_entry_dtls a where a.id=$data  and (a.status_active=1 and a.is_deleted=0 OR a.active_for_production=1 )  group by location_id");
...
if ($action == 'populate_data_from_booking2')
{
	...
	*/
	...
	$sql_progqty_data = sql_select("SELECT a.id,a.program_qnty,a.location_id, a.is_sales , b.po_id FROM ppl_planning_info_entry_dtls a, ppl_planning_entry_plan_dtls b WHERE a.id=".$dataArr[0]."  and (a.status_active=1 and a.is_deleted=0 OR a.active_for_production=1 )  and a.id = b.dtls_id and b.status_active=1 and b.is_deleted=0 group by a.id,a.program_qnty,a.location_id, a.is_sales,b.po_id");
...
if ($action == 'populate_data_from_service_booking') {
	...
	$recv_sql = ("SELECT a.service_booking_no,a.knitting_company as supplier_id, sum(b.grey_receive_qnty) grey_receive_qnty
		FROM inv_receive_master a INNER JOIN pro_grey_prod_entry_dtls b ON a.id = b.mst_id and b.status_active=1 and b.is_deleted=0 WHERE a.service_booking_no ='$booking_no' and a.knitting_source=$knitting_source  $supplier_cond and a.entry_form=2
	...
	$sql_progqty_data = sql_select("SELECT a.id,a.program_qnty,a.location_id, a.is_sales , b.po_id FROM ppl_planning_info_entry_dtls a, ppl_planning_entry_plan_dtls b WHERE a.id=".$pogramBooking."  and (a.status_active=1 and a.is_deleted=0 OR a.active_for_production=1 )  and a.id = b.dtls_id  and (b.status_active=1 and b.is_deleted=0 OR b.active_for_production=1 )  group by a.id,a.program_qnty,a.location_id, a.is_sales,b.po_id");
	foreach ($sql_progqty_data as  $val) 
	{
		$orderIDs.=$val[csf('po_id')].",";
	}
...
if ($action == 'show_fabric_desc_listview') {
	...
	$sql_deter = "select a.id, a.construction, b.copmposition_id, b.percent from lib_yarn_count_determina_mst a, lib_yarn_count_determina_dtls b where a.id=b.mst_id and b.status_active=1 group by a.id, a.construction, b.copmposition_id, b.percent";
	$data_array = sql_select($sql_deter);
	...
	$pre_production_sql = sql_select("SELECT b.febric_description_id, b.body_part_id, b.width, b.gsm, a.booking_no, sum(b.grey_receive_qnty) as production_qty from inv_receive_master a, pro_grey_prod_entry_dtls b where a.id =b.mst_id and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.booking_no='$booking_no' group by b.febric_description_id, b.body_part_id, b.width, b.gsm, a.booking_no");

	foreach ($pre_production_sql as $val)
	{
		$pre_production_qnty_arr[$val[csf("febric_description_id")]][$val[csf("body_part_id")]][$val[csf("width")]][$val[csf("gsm")]]+=$val[csf("production_qty")];
	}
	...
	<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="400">
		...
		<tbody>
			...
			foreach ($data_array as $row) {
				...
				//$pre_production_qnty = $pre_production_qnty_arr[$booking_no];
				$pre_production_qnty =$pre_production_qnty_arr[$row[csf('lib_yarn_count_deter_id')]][$row[csf('body_part_id')]][$row[csf('dia_width')]][$row[csf('gsm_weight')]];


				$balace_qnty = $row[csf('qnty')] - $pre_production_qnty;
				?>
				<tr bgcolor="<? echo $bgcolor; ?>"
					...
					<td align="right"><? echo number_format($pre_production_qnty,2,'.',''); ?></td>
					<td align="right"><? echo number_format($balace_qnty,2,'.',''); ?></td>
				</tr>
				<?
				$i++;
			}
...
if ($action == 'show_service_booking_programlist') {
	...
	$sql = "select a.booking_no, b.description,b.program_no,d.fabric_desc,d.gsm_weight,b.dia_width,c.machine_dia,c.machine_gg,c.machine_id,c.spandex_stitch_length,c.color_range,$color_fld,c.stitch_length,c.knitting_source,c.knitting_party,d.buyer_id,d.body_part_id,d.determination_id from wo_booking_mst a,wo_booking_dtls b,ppl_planning_info_entry_dtls c,ppl_planning_info_entry_mst d where a.booking_no=b.booking_no and b.program_no=c.id and c.mst_id=d.id and a.id=$data and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and (c.status_active=1 and c.is_deleted=0 OR c.active_for_production=1 )  group by $group_by"; */

	$sql ="SELECT a.booking_no, b.description,b.program_no,d.fabric_desc,d.gsm_weight, cast(b.dia_width as varchar2(400)) as  dia_width,c.machine_dia,c.machine_gg,c.machine_id,c.spandex_stitch_length,
	c.color_range,LISTAGG(c.color_id, ',') WITHIN GROUP (ORDER BY c.color_id) as color_id,c.stitch_length,c.knitting_source,c.knitting_party,d.buyer_id,
	d.body_part_id,d.determination_id 
	from wo_booking_mst a,wo_booking_dtls b,ppl_planning_info_entry_dtls c,ppl_planning_info_entry_mst d 
	where a.booking_no=b.booking_no and b.program_no=c.id and c.mst_id=d.id and a.booking_no='$data' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 
	and b.is_deleted=0 and (c.status_active=1 and c.is_deleted=0 OR c.active_for_production=1 ) 
	group by a.booking_no, b.description,b.program_no,d.fabric_desc,d.gsm_weight,b.dia_width,c.machine_dia,c.machine_gg,c.machine_id,c.spandex_stitch_length,c.color_range,c.color_id,c.stitch_length,c.knitting_source,c.knitting_party,d.buyer_id,d.body_part_id,d.determination_id 
	union all
	select a.wo_no, b.FABRIC_DESC as description,cast(b.program_no as varchar2(400)) as program_no, d.fabric_desc, d.gsm_weight, cast(b.machine_dia as varchar2(400)) as dia_width, c.machine_dia,c.machine_gg,c.machine_id, c.spandex_stitch_length, c.color_range, LISTAGG(c.color_id, ',') WITHIN GROUP (ORDER BY c.color_id) as color_id, c.stitch_length, c.knitting_source, c.knitting_party, d.buyer_id, d.body_part_id, d.determination_id 
	from knitting_work_order_mst a,knitting_work_order_dtls b,ppl_planning_info_entry_dtls c,ppl_planning_info_entry_mst d 
	where a.id=b.mst_id and b.program_no=c.id and c.mst_id=d.id and a.wo_no='$data' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and (c.status_active=1 and c.is_deleted=0 OR c.active_for_production=1 ) 
	group by a.wo_no, b.FABRIC_DESC,b.program_no, d.fabric_desc,d.gsm_weight,b.machine_dia,c.machine_dia, c.machine_gg,c.machine_id,c.spandex_stitch_length,c.color_range,c.color_id, c.stitch_length, c.knitting_source,c.knitting_party,d.buyer_id, d.body_part_id,d.determination_id";
	$data_array = sql_select($sql);

	foreach ($data_array as $row) 
	{
		$all_program_arr[$row[csf('program_no')]] =$row[csf('program_no')];
	}

	$all_program_arr =array_filter($all_program_arr);
	$all_programs = implode(",",$all_program_arr);

	$rqsn_arr = array();

	if(!empty($all_program_arr))
	{
		$reqsn_dataArray = sql_select("SELECT a.knit_id, b.brand, b.yarn_count_id, b.lot from ppl_yarn_requisition_entry a, product_details_master b where a.prod_id=b.id and a.status_active=1 and a.is_deleted=0 and a.knit_id in ($all_programs)");
	
		foreach($reqsn_dataArray as $rqsn_row){
			$rqsn_arr[$rqsn_row[csf("knit_id")]]["brand"] .= $rqsn_row[csf("brand")].',';
			$rqsn_arr[$rqsn_row[csf("knit_id")]]["count"] = $rqsn_row[csf("yarn_count_id")].',';
			$rqsn_arr[$rqsn_row[csf("knit_id")]]["lot"] .= $rqsn_row[csf("lot")].',';
		}
	}
	?>
	<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="300">
		...
		<tbody>
			...
			foreach ($data_array as $row) {
				...
				$brand =implode(",", array_unique(explode(",", chop($rqsn_arr[$row[csf('program_no')]]["brand"],","))));
				$lot =implode(",", array_unique(explode(",", chop($rqsn_arr[$row[csf('program_no')]]["lot"],","))));
				$count =implode(",", array_unique(explode(",", chop($rqsn_arr[$row[csf('program_no')]]["count"],","))));
...
if ($action == "po_popup")
{
	...
	$vari_sql = sql_select("select max_roll_weight, auto_update, production_entry, distribute_qnty, variable_list, item_category_id from variable_settings_production where status_active=1 and company_name=$cbo_company_id and variable_list in (49,67,51)");
	$tube_ref_sett=0;
	foreach ($vari_sql as $val)
	{
		if($val[csf("variable_list")] == 49)
		{
			$max_roll_weight = $val[csf("max_roll_weight")];
		}

		if($val[csf("variable_list")] == 67)
		{
			$tube_ref_sett = $val[csf("production_entry")];
		}


		if($val[csf("variable_list")] == 51  && $val[csf("auto_update")]*1 == 1)
		{
			if($booking_without_order==0 && $val[csf("item_category_id")] == 1)
			{
				//Bulk over production
				$over_production_qnty_kg = $val[csf("production_entry")]*1;
				$over_production_percentage = $val[csf("distribute_qnty")]*1;
			}
			else if($booking_without_order==1 && $val[csf("item_category_id")] == 4)
			{
				//Sample without order over production
				$over_production_qnty_kg = $val[csf("production_entry")]*1;
				$over_production_percentage = $val[csf("distribute_qnty")]*1;
			}
	...
	if ($production_control == 1)
	{
		...
		if($receive_basis==2)
		{
			...
			if(count($yarn_issue_id_arr)>0)
			{
				...
				$yarn_issue_return_sql = sql_select("select d.id, d.quantity, d.po_breakdown_id from inv_receive_master a, inv_transaction b, ppl_yarn_requisition_entry c, order_wise_pro_details d where a.id = b.mst_id and a.entry_form =  9 and a.receive_basis = 3 and b.transaction_type =4 and b.item_category =1 and c.knit_id =$booking_id and a.booking_id = c.requisition_no and a.company_id= $cbo_company_id $all_yarn_issue_cond and b.id = d.trans_id and d.entry_form =9 and b.status_active=1 and c.status_active=1 and d.status_active=1 group by d.id, d.quantity, d.po_breakdown_id
					...
					where a.id=b.mst_id and a.entry_form =9 and a.receive_basis =8 and b.transaction_type =4 and b.item_category =1 and c.knit_id =$booking_id and b.requisition_no=c.requisition_no and a.company_id= $cbo_company_id $all_yarn_issue_cond and b.id=d.trans_id and d.entry_form =9 and b.status_active=1 and c.status_active=1 and d.status_active=1 group by d.id, d.quantity, d.po_breakdown_id ");
		...
		else if($receive_basis==1 && $booking_without_order==1)
		{
			$yarn_issue_sql = sql_select("SELECT a.cons_quantity, b.booking_no from inv_transaction a, inv_issue_master b where a.mst_id=b.id and a.item_category =1 and a.transaction_type =2 and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0 and b.company_id = $cbo_company_id and  b.booking_no='$booking_no' and b.entry_form =3 and b.issue_basis in (1)");

			foreach ($yarn_issue_sql as $val)
			{
				$yarn_issue_qnty_arr[$val[csf('booking_no')]] += $val[csf("cons_quantity")];
				$yarn_balance_qty_arr[$val[csf('booking_no')]] += $val[csf("cons_quantity")];
			}

			$yarn_issue_return_sql = sql_select("SELECT a.cons_quantity, b.booking_no from inv_transaction a, inv_receive_master b where a.mst_id=b.id and a.item_category =1 and a.transaction_type =4 and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0 and b.company_id = $cbo_company_id and b.booking_no='$booking_no' and b.entry_form =9 and b.receive_basis in (1)");

			foreach ($yarn_issue_return_sql as $val)
				{
					$yarn_balance_qty_arr[$val[csf('booking_no')]] -= $val[csf("cons_quantity")];
					$yarn_issue_return_qnty_arr[$val[csf('booking_no')]] += $val[csf("cons_quantity")];
				}

			$prod_qnty_arr = sql_select("SELECT b.grey_receive_qnty, a.booking_no, b.id from inv_receive_master a, pro_grey_prod_entry_dtls b where a.id =b.mst_id and a.receive_basis =1 and a.entry_form =2 and a.booking_no='$booking_no' and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted =0 and a.company_id= $cbo_company_id");

			foreach ($prod_qnty_arr as $val)
			{
				$yarn_balance_qty_arr[$val[csf('booking_no')]] -= $val[csf("grey_receive_qnty")];
				$this_challan_rcv[$val[csf('id')]][$val[csf('booking_no')]] += $val[csf("grey_receive_qnty")];
				$grey_production_qnty_arr[$val[csf('booking_no')]] += $val[csf("grey_receive_qnty")];
			}
		}
		else
		{
		...
		/*echo "<pre>";
		echo "yarn issue=";print_r($yarn_issue_qnty_arr); echo "<br>";
		echo "yarn issue return=";print_r($yarn_issue_return_qnty_arr); echo "<br>";
		echo "grey production=";print_r($grey_production_qnty_arr);echo "<br>";
		echo "yarn balance=";print_r($yarn_balance_qty_arr);echo "<br>";
		echo "</pre>";*/
	...
	if ($roll_maintained == 1)
	{
		...
		where b.status_active=1 and b.is_deleted=0 and b.company_id=$cbo_company_id and b.store_id=$cbo_store_name and b.location_id=$cbo_com_location_name";
	...
	if($is_salesOrder == 1 && $receive_basis ==2 && $tube_ref_sett ==1)
	{
		$tube_ref_sql = sql_select("select id, batch_no, reference_no, reference_qty from ppl_reference_creation where program_no='$booking_no' and status_active=1 order by id");

		foreach ($tube_ref_sql as $trv)
		{
			$batch_no_arr[$trv[csf("batch_no")]] = $trv[csf("batch_no")];
			$batch_ref_arr[$trv[csf("batch_no")]]['ref'] = $trv[csf("reference_no")];
			$batch_ref_qnty_arr[$trv[csf("batch_no")]][$trv[csf("reference_no")]] += $trv[csf("reference_qty")];
			$load_batch_ref_arr[$trv[csf("batch_no")]][$trv[csf("reference_no")]] = $trv[csf("reference_no")];
		}
	}
	...
	<script>
		...
		var tube_ref_sett =<? echo $tube_ref_sett; ?>;
		...
		<?
		if(isset($batch_ref_qnty_arr))
		{
			$batch_ref_qnty_arr= json_encode($batch_ref_qnty_arr);
			echo "batch_ref_qnty_arr= ". $batch_ref_qnty_arr . ";\n";
		}
		?>
		...
		function fn_show_check() {
			...
			show_list_view(document.getElementById('txt_search_common').value + '_' + document.getElementById('cbo_search_by').value + '_<? echo $cbo_company_id; ?>_' + document.getElementById('cbo_buyer_name').value + '_' + '<? echo $all_po_id; ?>' + '_' + '<? echo $cbo_body_part; ?>', 'create_po_search_list_view', 'search_div', 'grey_production_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1);hidden_field_reset();');
			set_all();
		}
		...
		function add_break_down_tr(i)
		{
			...
			$('#txtGreyQnty_' + row_num).removeAttr("onBlur").attr("onBlur", "fn_check_balance(" + row_num + ");");
			$('#txtGreyQnty_' + row_num).removeAttr("onKeyUp").attr("onKeyUp", "fn_check_balance(" + row_num + ");fn_tube_qnty_chk(" + row_num + ");calculate_tot_qnty();fn_check_max_roll_weight('txtGreyQnty_"+ row_num +"');");
			...
			$('#txtQcBarcode_' + row_num).val('');
			$('#preQCPassQty_' + row_num).val('');
			...
			if (document.getElementById('cboTubeRef_'+row_num))
			{
				var program_no_ref = '<? echo $booking_no;?>'
				$('#cboTubeRef_' + row_num).closest('td').removeAttr('id').attr('id','tubeTd_'+row_num);
				$("#cboBatchNo_" + row_num).removeAttr('onchange').attr("onchange","fnc_tube_reference_load("+row_num+","+program_no_ref+",this.value);");
				$("#cboTubeRef_" + row_num).removeAttr('onchange').attr("onchange","load_tube_value("+row_num+","+program_no_ref+",this.value);");
				$('#txtTubeQnty_' + row_num).val('');

				$('#cboTubeRef_'+row_num).html('<option value="'+0+'">Select</option>');
				$("#cboBatchNo_" + row_num).val(0);
			}
		...
		function fn_check_balance(rowNo)
		{
			...
			if (roll_maintained == 1)
			{
				...
				var txtGreyQnty = $('#txtGreyQnty_' + rowNo).val()*1;
				// alert(txtQcBarcode+'='+txtBarcodeNo);
				if (txtQcBarcode == txtBarcodeNo && txtQcBarcode!=0 && txyPreQCPassQty != txtGreyQnty)
				{
		...
		function fn_tube_qnty_chk(rowNo)
		{
			if (roll_maintained == 1 && tube_ref_sett==1 )
			{
				var greyQty = '';
				var cboTubeRef = $('#cboTubeRef_' + rowNo).val();
				if(cboTubeRef ==0)
				{
					$('#txtGreyQnty_' + rowNo).val("");
					$('#cboTubeRef_' + rowNo).focus();
					return;
				}
				$("#tbl_list_search").find('tbody tr').each(function()
				{
					var tube_check=$(this).find('select[name="cboTubeRef[]"]').val();
					if(cboTubeRef == tube_check)
					{
						greyQty= greyQty *1 + $(this).find('input[name="txtGreyQnty[]"]').val() * 1;
					}
				});

				var balanceQty = $('#txtTubeQnty_' + rowNo).val() * 1;
				if (greyQty > balanceQty) {
					alert("QC Pass Qty. Exceeds Tube Ref Qty.");
					$('#txtGreyQnty_' + rowNo).val('');
					return;
				}
			}
		...
		function fnc_close()
		{
			...
			var validat_row=0;
			var batch_tube_qty_arr = new Array();
			var batch_tube_chk_arr = new Array();

			if(tube_ref_sett ==1)
			{
				$("#tbl_list_search").find('tr').each(function ()
				{
					var cboBatchNo = $(this).find('select[name="cboBatchNo[]"]').val();
					var cboTubeRef = $(this).find('select[name="cboTubeRef[]"]').val();
					var txtGreyQnty = $(this).find('input[name="txtGreyQnty[]"]').val();
					var txtRejectQnty = $(this).find('input[name="txtRejectQnty[]"]').val();
					var txtTubeQnty = $(this).find('input[name="txtTubeQnty[]"]').val();
					if (txtGreyQnty*1 >0 || txtRejectQnty*1 >0)
					{
						if( cboBatchNo==0 || cboTubeRef==0)
						{
							validat_row +=1;
						}
						else
			...
			if(validat_row>0){
				alert("Batch and Tube no required");
				return;
			}
			...
			$("#tbl_list_search").find('tr').each(function ()
			{
				...
				if (txtGreyQnty*1 >0 || txtRejectQnty*1 >0) {
					...
					if(receive_basis==2 && tube_ref_sett ==1)
					{
						var cboBatchNo = $(this).find('select[name="cboBatchNo[]"]').val();
						var cboTubeRef = $(this).find('select[name="cboTubeRef[]"]').val();
						var txtTubeQnty = $(this).find('input[name="txtTubeQnty[]"]').val();
						var tube_ref = '**' + cboBatchNo + '**' + cboTubeRef + '**' + txtTubeQnty;
					}else{
						var tube_ref = "";
					}
					...
					if (save_string == "") {
						save_string = txtPoId + "**" + txtGreyQnty + "**" + txtRejectQnty + "**" + txtRoll + "**" + txtRollId + "**" + txtRollTableId + "**" + txtBarcodeNo + "**" + txtGreyQnty_pcs+ "**" + txtSizeQty + "**" + txtRFId + tube_ref;
					}
					else {
						save_string += "," + txtPoId + "**" + txtGreyQnty + "**" + txtRejectQnty + "**" + txtRoll + "**" + txtRollId + "**" + txtRollTableId + "**" + txtBarcodeNo + "**" + txtGreyQnty_pcs+ "**" + txtSizeQty + "**" + txtRFId + tube_ref;
					}
		...
		function fnc_send_printer_text() {
			...
			data = data + "***" + dtls_id;
			var url = return_ajax_request_value(data, "report_barcode_text_file", "grey_production_entry_controller");
			window.open(url + ".zip", "##");
		}
		...
		function fnc_barcode_generation() {
			...
			data = data + "***" + dtls_id;
			window.open("grey_production_entry_controller.php?data=" + data + '&action=report_barcode_generation', true);
		}

		function get_weight_from_weight_machine(i){

			$.post( "grey_production_entry_controller.php", { company: '<? echo $cbo_company_id;?>', action: "load_weight_machine_data"})
			.done(function( data ) {
		...
		{ 
        	//var page_link = 'grey_production_entry_controller.php?action=action_finishSize&program_no=' + '<? //echo $booking_no; ?>';
        	var page_link = 'grey_production_entry_controller.php?action=action_finishSize&program_no=' + '<? echo $booking_no; ?>&rowNo=' + rowNo+'&is_salesOrder='+'<? echo $is_salesOrder; ?>'+'&cbo_company_id='+'<? echo $cbo_company_id; ?>'; 
		...
		function fnc_tube_reference_load(rowNo,program,value)
		{
			//alert();
			load_drop_down( 'grey_production_entry_controller', rowNo + '_' + program + '_' + value, 'load_tube_reference_load','tubeTd_'+rowNo);
			$( "#txtTubeQnty_" +rowNo).val("");
		}

		function load_tube_value(rowNo,program,value)
		{
			get_php_form_data(rowNo + '_' + program + '_' + value, "load_tube_value", "grey_production_entry_controller" );
		}
	</script>
	</head>
				...
				<div style="width:<? echo $width+220; ?>px; max-height:200px; overflow-y:scroll" id="list_container" align="left">
					<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="<? echo $width + 200; ?>" id="tbl_list_search">
						...
						for ($z = 0; $z < count($explSaveData); $z++)
						{
							...
							if($over_production_qnty_kg > 0)
							{	//Over Qnty KG added
								$yarn_issue_with_over_perc = $over_production_qnty_kg + $yarn_issue_quantity;
							}
							else
							{	//Over percentage % added
							...
							<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
								...
								<td align="center" >
									<input type="text" name="txtGreyQnty[]" id="txtGreyQnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px" value="<? echo $grey_qnty; ?>" <? echo $disable; ?> onKeyUp="calculate_tot_qnty();fn_check_max_roll_weight('txtGreyQnty_<? echo $i; ?>');" onBlur="fn_check_balance(<? echo $i; ?>);">
								</td>
								<td align="center">
								...
								if ($roll_maintained == 1)
								{
									...
									<td id="floor_td" width="80">
										<? echo create_drop_down( "cbo_floor_$i", 80,$lib_floor_arr,"", 1, "--Select--", 0, "load_room_rack_self_bin('grey_production_entry_controller*13*cbo_room_to_$i*$i', 'room',this.value, '".$cbo_company_id."','".$cbo_com_location_name."','".$cbo_store_name."',this.value);",0,"","","","","","","cbo_floor_to[]" ); ?>
									</td>
	...
	else
	{
		...
		if (($booking_without_order == 1 && $receive_basis != 2) || $receive_basis == 4) // sample and without program
		{
			...
			if($over_production_qnty_kg > 0)
			{
				$over_production_caption = "Over Production Quantity: ".$over_production_qnty_kg." KG";
			}
			else
			{
				$over_production_caption = "Over Production Percentage: ".$over_production_percentage." %";
			}
			...
			<div style="margin-left:5px; margin-top:10px">
				<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="<? echo $width + 180; ?>">
					<caption style="font-weight: bold;color: red;"><? echo $over_production_caption; ?></caption>
					<thead>
						<?
						if ($receive_basis == 4) {
                ...
                <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="<? echo $width + 180; ?>" id="tbl_list_search">
                    ...
                    if ($receive_basis == 4) {
						...
						if($over_production_qnty_kg > 0)
						{	//Over Qnty KG added
							$req_qnty_with_over_perc = $over_production_qnty_kg + $req_qnty;
						}
						else
						{	//Over percentage % added
					...
					else
					{
						...
						$yarn_issue_quantity = number_format($yarn_issue_qnty_arr[$booking_no],2,".","");
						...
						//$yarn_issue_with_over_perc = ($yarn_issue_quantity*$over_production_percentage)/100 + $yarn_issue_quantity;
						$yarn_over_balance = $yarn_issue_with_over_perc - $yarn_issue_return_qnty_arr[$booking_no] - $grey_production_qnty_arr[$booking_no] + $this_challan_rcv[$dtls_id][$booking_no];

						//echo $yarn_issue_with_over_perc ."-". $yarn_issue_return_qnty_arr[$booking_no] ."-". $grey_production_qnty_arr[$booking_no] ."+". $this_challan_rcv[$dtls_id][$booking_no];
					...
					if ($save_data == "")
					{
						...
						<tr bgcolor="#FFFFFF" id="tr_1">
							...
							<td align="center" width="80">
								<input type="text" name="txtGreyQnty[]" id="txtGreyQnty_1" class="text_boxes_numeric" style="width:60px" value="" placeholder="<? if ($roll_maintained == 0) echo number_format($production_balance, 2, '.', ''); ?>" onKeyUp="calculate_tot_qnty();fn_check_max_roll_weight('txtGreyQnty_<? echo $i; ?>');" onBlur="fn_check_balance(1);">
							</td>
							<td align="center" width="80">
					...
					else
					{
						...
						for ($z = 0; $z < count($explSaveData); $z++)
						{
							...
							<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
                                ...
                                <td align="center" width="80">
                                    <input type="text" name="txtGreyQnty[]" id="txtGreyQnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px" value="<? echo $grey_qnty; ?>" <? echo $disable; ?> placeholder="<? if ($roll_maintained == 0) echo number_format($bl_qnty, 2, '.', ''); ?>" onKeyUp="calculate_tot_qnty();fn_check_max_roll_weight('txtGreyQnty_<? echo $i; ?>');" onBlur="fn_check_balance(<? echo $i; ?>);">
                                </td>
                                <td align="center" width="80">
				...
				<div style="margin-left:5px; margin-top:10px">
					<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="<? echo $width + 220; ?>">
						...
						<thead>
							...
							<? if($receive_basis==2 && $is_salesOrder == 1 && $tube_ref_sett==1){?>
								<th width="60" class="must_entry_caption">Batch No</th>
								<th width="80" class="must_entry_caption">Tube/Ref. No</th>
								<th width="70" class="must_entry_caption">Tube/Ref. Qty</th>
							<?
							}
							else
							{?>
					...
					<div style="width:<? echo $width+240; ?>px; max-height:200px; overflow-y:scroll" id="list_container" align="left">
						<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="<? echo $width + 220; ?>" id="tbl_list_search">
								...
								if ($result != "")
								{
									...
									foreach ($nameArray as $row)
									{
										...
										<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
											...
											<td align="center" width="80">
												<input type="text" name="txtGreyQnty[]" id="txtGreyQnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px" value="" onKeyUp="calculate_tot_qnty();fn_check_max_roll_weight('txtGreyQnty_<? echo $i; ?>');" onBlur="fn_check_balance(<? echo $i; ?>);">
											</td>
											<td width="80" align="right"><? echo "" ?> </td>
											<td width="80" align="right"><? echo ""; ?></td>
											<td align="center" width="80">
											...
											if ($roll_maintained == 1)
											{
												...
												<td id="floor_td_to" width="80" align="center">
													<? echo create_drop_down( "cbo_floor_to_$i", 80,$lib_floor_arr,"", 1, "--Select--", 0, "load_room_rack_self_bin('grey_production_entry_controller*13*cbo_room_to_$i*$i', 'room',this.value, document.getElementById('cbo_company_id').value,document.getElementById('cbo_location').value,document.getElementById('cbo_store_name').value,this.value,'','','','','','80','H');",0,"","","","","","","cbo_floor_to[]" ,"onchange_void"); ?>
												</td>
								   	...
								   	if ($result != "")
								   	{
										...
										foreach ($nameArray as $row)
										{
											...
											<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
												...
												<td width="80" align="right"><? echo ""; ?></td>
												<td align="center" width="80">
													<input type="text" name="txtGreyQnty[]" id="txtGreyQnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px" value="" placeholder="<? if ($roll_maintained == 0) echo number_format($bl_qnty, 2, '.', ''); ?>" onKeyUp="calculate_tot_qnty(); fn_check_max_roll_weight('txtGreyQnty_<? echo $i; ?>');" onBlur="fn_check_balance(<? echo $i; ?>);">
												</td>
												<td align="center" width="80">
								...
								else
								{
									...
									foreach ($nameArray as $row)
									{
										...
										<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
											...
											<td width="80" align="right"><? echo ""; ?></td>
											<td align="center" width="80">
												<input type="text" name="txtGreyQnty[]" id="txtGreyQnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px" value="<? echo $grey_qnty; ?>" placeholder="<? if ($roll_maintained == 0) echo number_format($production_balance, 2, '.', ''); ?>" onKeyUp="calculate_tot_qnty();fn_check_max_roll_weight('txtGreyQnty_<? echo $i; ?>');" onBlur="fn_check_balance(<? echo $i; ?>);">
											</td>
											<td align="center" width="80">
								...
								if ($roll_maintained == 1)
								{
									...
									if($barcode_nums!="")
									{
								   	...
								   	for ($z = 0; $z < count($explSaveData); $z++)
								   	{
										...
										$batch_no = $po_wise_data[10];
										$tube_ref_no = $po_wise_data[11];
										$tube_qnty = $po_wise_data[12];

										//2114**10.00****3**0**144735**21020005887********FAL21-913-01**FAL21-913-01-R1**230
										...
										//echo $po_data_arr[$order_id]['po_number']." = $yarn_issue_quantity ($yarn_issue_with_over_perc) - $yarn_issue_return_qnty_arr[$order_id] - $grey_production_qnty_arr[$order_id] + $prev_own_rcv<br>";
										?>
										<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
											<td width="110">
											...
											if ($receive_basis == 2 && $is_salesOrder==1 && $tube_ref_sett==1)
											{
												$load_batch_ref_arr[$batch_no];
											?>
												<td width="60">
												<?
													echo create_drop_down( "cboBatchNo_".$i, 60, $batch_no_arr,"", 1, "-- Select Batch --", $batch_no, "fnc_tube_reference_load($i,$booking_no,this.value)",'' ,'','','','','','','cboBatchNo[]' );
												?>
												</td>

												<td width="80" id="tubeTd_<? echo $i;?>"><?
													echo create_drop_down( "cboTubeRef_".$i, 80, $load_batch_ref_arr[$batch_no],"", 1, "-- Select tube --", $tube_ref_no, "",'','','','','','','','cboTubeRef[]' );
													?>
												</td>
												<td width="70"><p><input type="text" name="txtTubeQnty[]" id="txtTubeQnty_<? echo $i;?>" value="<? echo $tube_qnty;?>" class="text_boxes" style="width:60px;" disabled/></p></td>
												<?
											}
											else
											{
										...
										foreach ($nameArray as $row)
										{
											...
											<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
												...
												if ($receive_basis == 2 && $is_salesOrder==1 && $tube_ref_sett==1)
												{
													...
													<td width="60">
													<?
													echo create_drop_down( "cboBatchNo_".$i, 60, $batch_no_arr,"", 1, "-- Select Batch --", 0, "fnc_tube_reference_load($i,$booking_no,this.value)",'' ,'','','','','','','cboBatchNo[]' );
													?>
													</td>

													<td width="80" id="tubeTd_<? echo $i;?>">
													<?
														echo create_drop_down( "cboTubeRef_".$i, 80, $blank_array,"", 1, "-- Select tube --", 0, "",'','','','','','','','cboTubeRef[]' );
														?>
													</td>
													<td width="70"><p><input type="text" name="txtTubeQnty[]" id="txtTubeQnty_<? echo $i;?>" class="text_boxes" style="width:60px;" disabled/></p></td>
													<?
												}
												else
												{
							...
							{
								if ($receive_basis == 2 && $is_salesOrder==1 && $tube_ref_sett==1) //Kniting plan un-save data with tube reference
								{
									$i = 1;
									$orginal_val = 1;
									$roll_id = 0;
									...
									foreach ($po_data as $row)
									{
										...
										<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
											...
											<td width="80" align="center">
												<input type="text" name="txtGreyQnty[]" id="txtGreyQnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px" value="" <? echo $disable; ?> placeholder="<? if ($roll_maintained == 0) echo number_format($production_balance, 2, '.', ''); ?>"  onKeyUp="calculate_tot_qnty(); fn_check_max_roll_weight('txtGreyQnty_<? echo $i; ?>'); fn_tube_qnty_chk(<? echo $i; ?>);" onClick="get_weight_from_weight_machine(<? echo $i; ?>)" onBlur="fn_check_balance(<? echo $i; ?>);">
											</td>
											<td width="80" align="center">
								...
								else if ($receive_basis == 2) //Kniting plan un-save data
								{
									$i = 1;
									$orginal_val = 1;
									$roll_id = 0;
									...
									foreach ($po_data as $row)
									{
										...
										//echo $row[csf('po_number')]."= $yarn_issue_with_over_perc - ".$yarn_issue_return_qnty_arr[$row[csf('id')]] ."- ".$grey_production_qnty_arr[$row[csf('id')]]."<br>";
										...
										<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
											...
											<td width="80" align="center">
												<input type="text" name="txtGreyQnty[]" id="txtGreyQnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px" value="" <? echo $disable; ?> placeholder="<? if ($roll_maintained == 0) echo number_format($production_balance, 2, '.', ''); ?>" onClick="get_weight_from_weight_machine(<? echo $i; ?>)" onKeyUp="calculate_tot_qnty();fn_check_max_roll_weight('txtGreyQnty_<? echo $i; ?>');" onBlur="fn_check_balance(<? echo $i; ?>);">
											</td>
											<td width="80" align="center">
												<input type="text" name="txtGreyQntyPcs[]" id="txtGreyQntyPcs_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px" value="" <? echo $disable_td; ?> placeholder="<? if ($roll_maintained == 0) echo number_format($bl_qnty_pcs, 2, '.', ''); ?>" onKeyUp="fn_check_balance(<? echo $i; ?>);calculate_tot_qnty();">
											</td>
                    	...
                    	<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="<? echo $width; ?>" id="tbl_list_search">
							...
							if ($production_control == 1)
							{
								...
								*/
								...
								foreach ($yarn_issue_sql as $val)
								{
									$yarn_issue_qnty += $val[csf("issue_qnty")];
							...
							*/
							$programSql = "
                                ...
                                WHERE
                                    ...
                                    and (b.status_active=1 and b.is_deleted=0 OR b.active_for_production=1 ) 
                                GROUP BY
                                    b.dtls_id, c.id
                            ";
							...
							*/
							...
							if($over_production_qnty_kg > 0)
							{	//Over Qnty KG added
								$req_qnty_with_over_perc = $over_production_qnty_kg + $programQty;
							}
							else
							{	//Over percentage % added
							...
							$yarn_issue_quantity = number_format($yarn_issue_qnty,2,".","");
                            ...
                            if($save_data == "")
                            {
                                ...
                                <tr bgcolor="#FFFFFF" id="tr_1">
                                    ...
                                    <td align="center" width="80">
                                        <input type="text" name="txtGreyQnty[]" id="txtGreyQnty_1" class="text_boxes_numeric" style="width:60px" value="" placeholder="<? if ($roll_maintained == 0) echo number_format($productionBalance, 2, '.', ''); ?>" onKeyUp="calculate_tot_qnty();fn_check_max_roll_weight('txtGreyQnty_<? echo $i; ?>');" onBlur="fn_check_balance(1);">
                                    </td>
                                    <td align="center" width="80">
                                        <input type="text" name="txtGreyQntyPcs[]" id="txtGreyQntyPcs_1" class="text_boxes_numeric" style="width:60px" value="" placeholder="<? if ($roll_maintained == 0) echo number_format($bl_qnty_pcs, 2, '.', ''); ?>" onKeyUp="calculate_tot_qnty();" <? echo $disable_td; ?>>
                                    </td>
...
if($action == 'action_finishSize')
{
	...
	<script>
        show_list_view ('<? echo $program_no; ?>'+'**'+'<? echo $rowNo; ?>'+'**'+'<? echo $is_salesOrder; ?>'+'**'+'<? echo $cbo_company_id; ?>', 'action_finishSize_listview', 'search_div', 'grey_production_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1)');
    </script>
    <script src="../../includes/functions_bottom.js" type="text/javascript"></script>
    </html>
	<?
...
if ($action == "action_finishSize_listview")
{
	...
	$data = explode('**', $data);
	//print_r($data);
	$is_salesOrder=$data[2];
	$cbo_company_id=$data[3];
	if($is_salesOrder==1)
	{
		$cullarAndCuffVar = return_field_value("coller_cuf_size_planning", "variable_settings_production", "variable_list=53 and company_name=$cbo_company_id and status_active=1 and is_deleted=0", "coller_cuf_size_planning");
		if($cullarAndCuffVar==1)
		{
			$sql="SELECT a.id, a.finish_size_id as item_size from ppl_size_wise_break_down a WHERE a.program_no in(".$data[0].") and a.status_active=1 and a.is_deleted=0 group by a.id, a.finish_size_id";
			$item_size_data=sql_select($sql);
			foreach ($item_size_data as $row)
			{
				$item_size_arr[]=$row[csf('item_size')];
			}

		}
		else
		{
			$sql="SELECT a.id, a.finish_size as item_size from ppl_planning_collar_cuff_dtls a WHERE a.dtls_id in(".$data[0].") and a.status_active=1 and a.is_deleted=0 group by a.id, a.finish_size";
			$item_size_data=sql_select($sql);
			foreach ($item_size_data as $row)
			{
				$item_size_arr[]=$row[csf('item_size')];
	...
	{
		$sql="SELECT a.id, a.collar_cuff_data as item_size FROM ppl_planning_info_entry_dtls a WHERE a.id in(".$data[0].")  and (a.status_active=1 and a.is_deleted=0 OR a.active_for_production=1 )  group by a.id, a.collar_cuff_data";
	...
	<div align="center">
		...
		<div style="width:200px; overflow-y:scroll; max-height:240px;" id="buyer_list_view" align="center">
			...
			<?
			$i = 1;
			foreach ($item_size_arr as $keys => $row)
			{
				if ($i % 2 == 0)
					$bgcolor = "#E9F3FF";
				else
...
if($action == "check_production_qnty")
{
	...
	else if ($data[2] == 2)
	{
		...
		$program_qnty_array = sql_select("select sum(a.program_qnty) as qnty from ppl_planning_info_entry_dtls a where a.id=" . $data[0] . "  and (a.status_active=1 and a.is_deleted=0 OR a.active_for_production=1 ) ");
	...
	{
		$sql_prev_production = "select sum(b.grey_receive_qnty) as grey_receive_qnty from inv_receive_master a,  pro_grey_prod_entry_dtls b where a.id=b.mst_id and a.booking_without_order=0 and booking_id=" . $data[0] . " and a.receive_basis=4 and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
		$bookingData = sql_select("SELECT sum(b.grey_qty) as qnty from fabric_sales_order_mst a, fabric_sales_order_dtls b where a.id=b.mst_id and a.id=" . $data[0] . " and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0");

		$current_production_data = sql_select("select b.id, sum(b.grey_receive_qnty) as grey_receive_qnty from inv_receive_master a,  pro_grey_prod_entry_dtls b where a.id=b.mst_id and booking_id=" . $data[0] . "  and b.id =" . $data[5] . " and a.receive_basis=4 and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.id");
...
if ($action == "save_update_delete")
{
	...
	$txt_width = strtoupper($txt_width); //For avoiding case sensitivity of dia. 05/08/2023
	$txt_original_dia_width = strtoupper($txt_original_dia_width); //For avoiding case sensitivity of dia. 05/08/2023
	...
	if($receive_basis==2)
	{
		...
		$sql_ref="select ref_closing_status,id as prog_no from ppl_planning_info_entry_dtls where id=$prog_no_id   and (status_active=1 and is_deleted=0 OR active_for_production=1 ) ";
	...
	if($vari_knit_charge_source_arr[0][csf("editable")] == 0 || $vari_knit_charge_source_arr[0][csf("editable")] == "")
	{
	...
	*/
	...
	if(str_replace("'", "", $cbo_receive_basis) == 2)
	{
		...
		$program_qnty_array = sql_select("select sum(a.program_qnty) as qnty,location_id from ppl_planning_info_entry_dtls a where a.id=$txt_booking_no_id   and (a.status_active=1 and a.is_deleted=0 OR a.active_for_production=1 )  group by location_id");
	...
	//$over_production_variable_sql = sql_select("select auto_update, distribute_qnty from variable_settings_production where company_name =$cbo_company_id and variable_list in(51) and item_category_id=1 and is_deleted=0 and status_active=1");

	$booking_without_order = str_replace("'", "", $booking_without_order);
	if (str_replace("'", "", $cbo_receive_basis) == 0) {
		$booking_without_order = 0;
	}

	$over_production_variable_sql = sql_select("select auto_update, distribute_qnty, production_entry, item_category_id from variable_settings_production where company_name =$cbo_company_id and variable_list in(35) and item_category_id in (13,101) and is_deleted=0 and status_active=1 order by id desc");
	foreach($over_production_variable_sql as $row)
	{
		if($booking_without_order==0 && $row[csf("item_category_id")]==13)
		{
			$over_production_variable =  $row[csf("auto_update")];
			$over_production_percentage =  $row[csf("distribute_qnty")]*1;
			$over_prog_production_qnty_kg =  $row[csf("production_entry")]*1;
		}
		else if($booking_without_order==1 && $row[csf("item_category_id")]==101)
		{
			$over_production_variable =  $row[csf("auto_update")];
			$over_production_percentage =  $row[csf("distribute_qnty")]*1;
			$over_prog_production_qnty_kg =  $row[csf("production_entry")]*1;
	...
	/*
	|	N.B. 16-Nov-2021  (Fabric Production over control [51] ) --> is changed to (Fabric Production control [35] )
	|	Over production of booking or program will be calculated with Fabric production Control variable (yes/no) and percentage will added if available
	|
	*/
	...
	//{
		if(str_replace("'", "", $cbo_receive_basis) == 2)
		{
			if($over_prog_production_qnty_kg*1 > 0)
			{
				$program_qnty_with_over_perc =$program_qnty_array[0][csf("qnty")] + $over_prog_production_qnty_kg*1;
				$addi_msg = "\nwith over limit Qnty =".$program_qnty_with_over_perc.' KG';
			}
			else
			{
		...
		else if (str_replace("'", "", $cbo_receive_basis) == 1)
		{
			...
			{
				$booking_required_sql = sql_select("select sum(b.grey_fabric) as qnty,b.lib_yarn_count_deter_id,b.body_part from wo_non_ord_samp_booking_mst a, wo_non_ord_samp_booking_dtls b where a.booking_no=b.booking_no and a.booking_type=4  and (b.fabric_source=1 OR a.fabric_source=1) and a.status_active=1  and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.booking_no = $txt_booking_no group by  b.lib_yarn_count_deter_id,b.body_part");
	...
	// ========================== Over production variable check end here =======================================

	$tube_ref_sett_var = sql_select("select production_entry from variable_settings_production where company_name =$cbo_company_id and variable_list in(67)  and is_deleted=0 and status_active=1");

	if( $tube_ref_sett_var[0][csf("production_entry")] ==1 && str_replace("'", "", $cbo_receive_basis) == 2)
	{
		$up_dtls_cond = " and dtls_id !=$update_dtls_id";
		$tube_ref_rcv_sql = sql_select("select batch_no, tube_ref_no, sum(qnty) as qnty from PRO_ROLL_DETAILS where entry_form=2 and is_sales=1 and booking_no=$txt_booking_no_id and batch_no is not null and receive_basis =2 and is_deleted=0 and status_active=1 $up_dtls_cond group by batch_no, tube_ref_no");

		foreach($tube_ref_rcv_sql as $trRcv)
		{
			$tube_ref_rcv_arr[$trRcv[csf("batch_no")]."*".$trRcv[csf("tube_ref_no")]] =$trRcv[csf("qnty")];
		}

		$save_str = explode(",", str_replace("'", "", $save_data));
		for ($i = 0; $i < count($save_str); $i++)
		{
			$order_dtls = explode("**", $save_str[$i]);

			$order_id = trim($order_dtls[0]);
			$order_qnty_roll_wise = $order_dtls[1];
			$batch_tube = $order_dtls[10];
			$tube_ref = $order_dtls[11];
			$tube_qnty = $order_dtls[12];

			$tube_ref_qnty[$batch_tube."*".$tube_ref]['roll_qnty'] +=$order_qnty_roll_wise;
			$tube_ref_qnty[$batch_tube."*".$tube_ref]['tube_qnty'] =$tube_qnty;
		}
		//echo "10**";
		foreach ($tube_ref_qnty as $rowK=>$rowV)
		{
			if($rowV['tube_qnty'] - $tube_ref_rcv_arr[$rowK] - $rowV['roll_qnty'] <0)
			{
				$rowKArr = explode("*", $rowK);
				echo "30**"."batch_no=".$rowKArr[0].", tube/ref no=".$rowKArr[1]." does not have balance";
				die;
			}
		}
		//die;
	...
	if ($operation == 0)  // Insert Here
	{
		...
		if(str_replace("'", "", $cbo_receive_basis) == 2)
		{
			...
			{
				$currency_ref = sql_select("select c.currency_id from  ppl_planning_info_entry_mst a, ppl_planning_info_entry_dtls b, wo_booking_mst c where b.id = $txt_booking_no_id and a.id =b.mst_id and a.booking_no = c.booking_no  and (b.status_active=1 and b.is_deleted=0 OR b.active_for_production=1 ) ");
			}
			else
			{
				$currency_ref = sql_select("select c.currency_id from  ppl_planning_info_entry_mst a, ppl_planning_info_entry_dtls b, wo_non_ord_samp_booking_mst c where b.id = $txt_booking_no_id and a.id =b.mst_id and a.booking_no = c.booking_no  and (b.status_active=1 and b.is_deleted=0 OR b.active_for_production=1 ) ");
			}
		...
		if (str_replace("'", "", $cbo_receive_basis) == 2 && str_replace("'", "", $booking_without_order) == 1 && str_replace("'", "", $fabric_desc_id) == 0)
		{
			...
			$row_prod = sql_select("select id, current_stock, stock_value from product_details_master where company_id=$cbo_company_id and item_category_id=13 and detarmination_id=$fabric_desc_id and item_description='$fabric_description' and gsm=$txt_gsm and upper(dia_width)=$txt_width and status_active=1 and is_deleted=0");
		} else {
			$row_prod = sql_select("select id, current_stock, stock_value from product_details_master where company_id=$cbo_company_id and item_category_id=13 and detarmination_id=$fabric_desc_id and gsm=$txt_gsm and upper(dia_width)=$txt_width and status_active=1 and is_deleted=0");
		}
		...
		//echo "10**".$knitting_charge_taka.'='.str_replace("'", "", $process_costing_maintain)*1 ."=".str_replace("'", "", $is_sales_knitting_program)*1 .'='. str_replace("'", "", $cbo_receive_basis) ;die;

		//Knitting charge validation with variable settings
		//if($knitting_charge_taka <= 0 && ( (str_replace("'", "", $process_costing_maintain) == 1 &&  $vari_knit_charge_source ==2) || (  str_replace("'", "", $is_sales_knitting_program)*1 !=0  && str_replace("'", "", $cbo_receive_basis)==2 && $var_rate_mandatory==1) ))

		if($knitting_charge_taka <= 0 && ( str_replace("'", "", $process_costing_maintain) == 1 || (  str_replace("'", "", $is_sales_knitting_program)*1 !=0  && str_replace("'", "", $cbo_receive_basis)==2 && $var_rate_mandatory==1) ))
		{
		...
		// *** Below code commented with CRM 23-19520
		...
		$yarn_prod_id = array_unique(explode(",", str_replace("'", "", $yarn_prod_id)));
		asort($yarn_prod_id);
		...
		$data_array_dtls = "(" . $id_dtls . "," . $grey_update_id . "," . $id_trans . "," . $prod_id . "," . $cbo_body_part . "," . $fabric_desc_id . "," . $txt_gsm . "," . $txt_width . "," . $txt_original_gsm . "," . $txt_original_dia_width . "," . $txt_roll_no . "," . $all_po_id . "," . $txt_receive_qnty . "," . $txt_receive_qnty_pcs . ",".$txt_coller_cuff_size."," . $txt_reject_fabric_recv_qnty . "," . $rate . "," . $amount . "," . $cbo_uom . ",'" . $txt_yarn_lot . "','" . $cbo_yarn_count . "'," . $brand_id . "," . $txt_shift_name . "," . $cbo_floor_id . "," . $cbo_machine_name . "," . $cbo_room . "," . $txt_rack . "," . $txt_shelf . "," . $cbo_floor . "," . $color_id . "," . $cbo_color_range . "," . $txt_stitch_length . "," . $txt_machine_dia . "," . $txt_machine_gg . ",'" . $yarn_rate_usd . "','" . $knitting_charge_usd . "','" . $yarn_rate_taka . "','" . $knitting_charge_taka . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "','" . $operator_name . "','" . $yarn_prod_id . "','" . $hdn_rate . "','" . $hdn_rate_string . "')";
		...
		$barcode_year = date("y");
		$field_array_roll = "id, barcode_year,barcode_suffix_no,barcode_no, mst_id, dtls_id, po_breakdown_id, entry_form, qnty,qc_pass_qnty,qc_pass_qnty_pcs,coller_cuff_size, reject_qnty, roll_no, rate, amount, booking_no, booking_without_order, receive_basis, is_sales, rf_id, batch_no, tube_ref_no, tube_qnty, fso_delivery_type, inserted_by, insert_date";
		...
		{
			$sales_flag_sql=sql_select("select a.is_sales, b.po_id from ppl_planning_info_entry_dtls a left join ppl_planning_entry_plan_dtls b on a.id=b.dtls_id  and (b.status_active=1 and b.is_deleted=0 OR b.active_for_production=1 )  where a.id=$txt_booking_no_id group by a.is_sales, b.po_id");
			$is_salesOrder =$sales_flag_sql[0][csf("is_sales")];
			$fso_no =$sales_flag_sql[0][csf("po_id")];
		...
		if (str_replace("'", "", $roll_maintained) == 1)
		{
			...
			if(str_replace("'","",$cbo_receive_basis)==2 && $is_salesOrder ==1)
			{
				$sales_type_sql=sql_select("select sales_order_type, b.work_scope from fabric_sales_order_mst a, fabric_sales_order_dtls b where a.id=b.mst_id and b.status_active=1 and a.id=$fso_no group by sales_order_type, b.work_scope");

				foreach ($sales_type_sql as $val)
				{
					if($val[csf("sales_order_type")] == 1 && $val[csf("work_scope")] ==13)
					{
						$fso_delivery_type = 1;
					}
				}
		...
		for ($i = 0; $i < count($save_string); $i++) {
			...
			$data_array_roll .= "$add_comma(" . $id_roll . "," . $barcode_year . "," . $barcode_suffix_no[2] . "," . $barcode_no . "," . $grey_update_id . "," . $id_dtls . "," . $order_id . ",2,'" . $order_qnty_roll_wise . "','" . $order_qnty_roll_wise . "','" . $order_qnty_roll_wise_pcs . "','" . $coller_cuff_size_roll_wise . "','" . $roll_reject_qty . "','" . $roll_no . "','" . $roll_rate . "','" . $roll_amount . "','" . str_replace("'", "", $txt_booking_no) . "','" . str_replace("'", "", $booking_without_order) . "'," . $cbo_receive_basis . ",'" . $is_salesOrder . "','" . $rf_id . "','" . $batch_tube. "','" . $tube_ref. "','" . $tube_qnty . "','" . $fso_delivery_type . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
		...
		//echo "10**".count($row_prod)."*".str_replace("'", "", $fabric_store_auto_update); oci_rollback($con); die;
		if (count($row_prod) > 0) {
			if (str_replace("'", "", $fabric_store_auto_update) == 1) {
		...
		//echo "10**insert into pro_roll_details (".$field_array_roll.") values ".$data_array_roll."**".$flag;oci_rollback($con);die;
		$rID6=1;
		...
		$rID7=1;
		if (  ($data_array_material_used != "" && str_replace("'", "", $process_costing_maintain) == 1)  ||  ($is_salesOrder==1 && str_replace("'", "", $cbo_receive_basis)==2 && str_replace("'", "", $var_rate_mandatory) == 1) ) {
	...
	else if ($operation == 1)   // Update Here
	{
		...
		$production_roll_sql = sql_select("SELECT b.gsm, b.width, b.stitch_length, b.febric_description_id, c.barcode_no, c.qnty from inv_receive_master a, pro_grey_prod_entry_dtls b
		left join pro_roll_details c on b.id=c.dtls_id and c.entry_form = 2 and c.mst_id=$update_id and c.dtls_id=$update_dtls_id and c.status_active=1 and c.is_deleted=0
		where a.id=b.mst_id and a.id=$update_id and b.id=$update_dtls_id and a.status_active=1 and a.is_deleted =0 and b.status_active=1 and b.is_deleted=0");
		foreach ($production_roll_sql as  $row)
		{
			$all_barcodeNo_arr[$row[csf("barcode_no")]] = $row[csf("barcode_no")];
			$production_qnty_arr[$row[csf("barcode_no")]] = $row[csf("qnty")];
		}

		if ($fabric_store_auto_update!=1) // without auto receive
		{
			$delivery_roll_sql = sql_select("SELECT a.barcode_no, a.qnty from pro_roll_details a, pro_grey_prod_delivery_dtls b where a.dtls_id = b.id and b.grey_sys_id=$update_id and b.sys_dtls_id= $update_dtls_id and a.entry_form = 56 and b.entry_form =56 and a.status_active =1 and a.is_deleted =0");
			foreach ($delivery_roll_sql as $row) 
			{
				$delivery_barcode_arr[$row[csf("barcode_no")]] = $row[csf("barcode_no")];
				$delivery_qnty_arr[$row[csf("barcode_no")]] = $row[csf("qnty")];
			}

			$grose_recv_sql = sql_select("SELECT a.id from INV_RECEIVE_MASTER a, PRO_GREY_PROD_ENTRY_DTLS b where a.id = b.mst_id and a.booking_id=$update_id and b.grey_prod_dtls_id=$update_dtls_id and a.receive_basis=9 and a.entry_form = 22 and a.item_category=13 and a.status_active =1 and a.is_deleted =0");

			if(!empty($delivery_roll_sql) || !empty($grose_recv_sql))
			{
				if (($production_roll_sql[0][csf("gsm")]!=str_replace("'", "", $txt_gsm)) || (strtoupper($production_roll_sql[0][csf("width")])!=str_replace("'", "", $txt_width)) || ($production_roll_sql[0][csf("febric_description_id")]!=str_replace("'", "", $fabric_desc_id)) || ($production_roll_sql[0][csf("stitch_length")]!=str_replace("'", "", $txt_stitch_length))) 
				{
					echo "30**Next Transaction Found, Update Not Allowed.";
					disconnect($con);
					die;
				}
			}			
		...
		else // auto recv
		{
			...
			if(count($all_barcodeNo_arr)>0)
			{
				...
				$next_trans_check = sql_select("SELECT a.barcode_no from pro_roll_details a where a.entry_form in(61,83,133,82,180,110,183) $all_barcode_no_cond and a.status_active = 1 and a.is_deleted=0");
				if(!empty($next_trans_check))
				{
					if (($production_roll_sql[0][csf("gsm")]!=str_replace("'", "", $txt_gsm)) || (strtoupper($production_roll_sql[0][csf("width")])!=str_replace("'", "", $txt_width)) || ($production_roll_sql[0][csf("febric_description_id")]!=str_replace("'", "", $fabric_desc_id)) || ($production_roll_sql[0][csf("stitch_length")]!=str_replace("'", "", $txt_stitch_length))) {
						echo "30**Next Transaction Found, Update Not Allowed.";
						disconnect($con);
		...
		// echo "30**checking=".$fabric_store_auto_update.'='.$roll_maintained;die;

		$mst_sql = sql_select("SELECT booking_id booking_id, count(b.barcode_no) barcode_count from inv_receive_master a left join pro_roll_details b on a.id= b.mst_id and b.entry_form=2 where a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and a.id=$update_id group by booking_id");

		$barcode_count = $mst_sql[0][csf("barcode_count")]*1;
		$roll_maintain_field = str_replace("'", '', $roll_maintained)*1;
		$program_no = $mst_sql[0][csf("booking_id")];
		...
		if($barcode_count > 0 && $roll_maintain_field==0 )
		{
			//N. B inv_receive_master table should have roll_maintained field 1;
			echo "30**Roll maintained variable can not change for this production";
			disconnect($con);
			die;
		}
		...
		$field_array_update = "receive_basis*receive_date*challan_no*booking_id*booking_no*booking_without_order*store_id*knitting_location_id*knitting_source*knitting_company*buyer_id*yarn_issue_challan_no*remarks*within_group*service_booking_without_order*service_booking_no*updated_by*update_date";
		//*roll_maintained

		$data_array_update = $cbo_receive_basis . "*" . $txt_receive_date . "*" . $txt_receive_chal_no . "*" . $txt_booking_no_id . "*" . $txt_booking_no . "*" . $booking_without_order . "*" . $cbo_store_name . "*" . $cbo_location_name . "*" . $cbo_knitting_source . "*" . $cbo_knitting_company . "*" . $cbo_buyer_name . "*" . $txt_yarn_issue_challan_no . "*" . $txt_remarks . "*" . $within_group . "*" . $service_booking_without_order . "*" . $txt_service_booking . "*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'";
		//. "*" . $roll_maintained
		//N. B. roll maintained can not updated
		...
		if (str_replace("'", "", $cbo_receive_basis) == 2 && str_replace("'", "", $booking_without_order) == 1 && str_replace("'", "", $fabric_desc_id) == 0) {
			$fabric_description = trim(str_replace("'", "", $txt_fabric_description));
			$row_prod = sql_select("select id, current_stock, stock_value, avg_rate_per_unit from product_details_master where company_id=$cbo_company_id and item_category_id=13 and detarmination_id=$fabric_desc_id and item_description='$fabric_description' and gsm=$txt_gsm and upper(dia_width)=$txt_width and status_active=1 and is_deleted=0");
		} else {
			$row_prod = sql_select("select id, current_stock, stock_value, avg_rate_per_unit from product_details_master where company_id=$cbo_company_id and item_category_id=13 and detarmination_id=$fabric_desc_id and gsm=$txt_gsm and upper(dia_width)=$txt_width and status_active=1 and is_deleted=0");
		}
		...
		$amount = $cons_amount;
		$hdn_rate = str_replace("'", "", $hdn_rate);
		if($hdn_rate==""){$hdn_rate="''";}else{$hdn_rate = str_replace("'", "", $hdn_rate);}

		//$hdn_rate_string = str_replace("'", "", $hdn_rate_string);

		$field_array_dtls_update = "prod_id*body_part_id*febric_description_id*gsm*width*original_gsm*original_width*no_of_roll*order_id*grey_receive_qnty*grey_receive_qnty_pcs*coller_cuff_size*reject_fabric_receive*rate*amount*uom*yarn_lot*yarn_count*brand_id*shift_name*floor_id*machine_no_id*room*rack*self*store_floor*color_id*color_range_id*stitch_length*machine_dia*machine_gg*order_yarn_rate*order_knitting_charge*yarn_rate*kniting_charge*updated_by*update_date*operator_name*yarn_prod_id*production_rate*production_rate_string";
		...
		$field_array_roll = "id, barcode_year,barcode_suffix_no,barcode_no, mst_id, dtls_id, po_breakdown_id, entry_form, qnty,qc_pass_qnty,qc_pass_qnty_pcs,coller_cuff_size, reject_qnty, roll_no,rate, amount,booking_no, booking_without_order, receive_basis, is_sales, rf_id, batch_no, tube_ref_no, tube_qnty, fso_delivery_type, inserted_by, insert_date";
		$field_array_roll_update = "po_breakdown_id*booking_no*booking_without_order*qnty*qc_pass_qnty*qc_pass_qnty_pcs*coller_cuff_size*reject_qnty*roll_no*rate*amount*rf_id*batch_no*tube_ref_no*tube_qnty*updated_by*update_date";
		...
		for ($i = 0; $i < count($save_string); $i++)
		{
			...
			$inserted_barcode_no = $order_dtls[6];
			$roll_amount = $order_qnty_roll_wise * $roll_rate;
			...
			if ($roll_id == "" || $roll_id == 0) {
				...
				$data_array_roll .= "(" . $id_roll . "," . $barcode_year . "," . $barcode_suffix_no[2] . "," . $barcode_no . "," . $update_id . "," . $update_dtls_id . "," . $order_id . ",2,'" . $order_qnty_roll_wise . "','" . $order_qnty_roll_wise . "','" . $order_qnty_roll_wise_pcs . "','" . $coller_cuff_size_roll_wise . "','" . $roll_reject_qty . "','" . $roll_no . "','" . $roll_rate . "','" . $roll_amount . "','" . str_replace("'", "", $txt_booking_no) . "','" . str_replace("'", "", $booking_without_order) . "'," . $cbo_receive_basis . ",'" . $is_salesOrder . "','" .$rf_id . "','" .$batch_tube . "','" .$tube_ref . "','" .$tube_qnty. "','" . $fso_delivery_type . "',". $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
				$all_barcode_no .= $barcode_no . ",";
			} 
			else 
			{
				// for after delivery (without production page reload) update production qnty
				if ($fabric_store_auto_update!=1 && str_replace("'", '', $roll_maintained)*1==1 && !empty($delivery_roll_sql) && $delivery_barcode_arr[$inserted_barcode_no]==$inserted_barcode_no)
				{
					if (number_format($delivery_qnty_arr[$inserted_barcode_no], 2, ".", "")  != number_format($order_qnty_roll_wise, 2, ".", "")) 
					{
						echo "30**Sorry! This barcode (" . $inserted_barcode_no . ") has been already delivered " . number_format($delivery_qnty_arr[$inserted_barcode_no], 2, ".", "") . " doesn't match with current " . number_format($order_qnty_roll_wise, 2, ".", "") . "";
						disconnect($con);
						die;
					}
				}
				...
				$roll_id_arr[] = $roll_id;
				$roll_data_array_update[$roll_id] = explode("*", ($order_id . "*'" . str_replace("'", "", $txt_booking_no) . "'*'" . str_replace("'", "", $booking_without_order) . "'*'" . $order_qnty_roll_wise . "'*'" . $order_qnty_roll_wise . "'*'" . $order_qnty_roll_wise_pcs . "'*'" . $coller_cuff_size_roll_wise . "'*'" . $roll_reject_qty . "'*'" . $roll_no . "'*'" . $roll_rate . "'*'" . $roll_amount ."'*'" .$rf_id ."'*'" .$batch_tube ."'*'" .$tube_ref ."'*'" .$tube_qnty. "'*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'"));
			}
		...
		if (str_replace("'", "", $process_costing_maintain) == 1 ||  ( $is_salesOrder==1 && str_replace("'", "", $cbo_receive_basis)==2 && str_replace("'", "", $var_rate_mandatory) == 1) ) {
		...
		//echo "10**".$field_array_dtls_update."<br>".$data_array_dtls_update;die;
		// echo "10**".$flag."=".$deletedMaterial.'='.$rID8.'='.$materialUpdate.'='.$rID7.'='.$delete_prop.'='.$rID6.'='.$rID5.'='.$rID4.'='.$rID2.'='.$rID."*";die;
...
if ($action == "grey_receive_popup_search") {
	...
	<script>
		...
		function fnc_show()
		{
			if($("#txt_search_common").val()=="" && $("#txt_date_from").val()=="" && $("#txt_date_to").val()=="")
			{
				alert("Please select any search criteria.");
				return;
			}

			show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_company_id').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('cbo_year_selection').value, 'create_grey_recv_search_list_view', 'search_div', 'grey_production_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1);');
		}
...
if ($action == "create_grey_recv_search_list_view") {
	...
	group by a.id, a.insert_date, a.recv_number_prefix_num, a.recv_number, a.booking_no, a.buyer_id, a.knitting_source, a.knitting_company, a.receive_date, a.challan_no, a.within_group order by a.id desc";
...
if ($action == 'populate_data_from_grey_recv') {
	...
	foreach ($data_array as $row) {
		...
		//echo "load_drop_down( 'requires/grey_production_entry_controller', " . $row[csf("knitting_source")] . "+'_'+" . $row[csf("company_id")] . ", 'load_drop_down_knitting_com','knitting_com');\n";
		...
		if ($row[csf("knitting_source")] == 1) {
			...
			$sql="SELECT DISTINCT a.id, a.supplier_name FROM lib_supplier a, lib_supplier_party_type b, lib_supplier_tag_company c  WHERE   a.id = b.supplier_id AND b.supplier_id = c.supplier_id AND a.status_active = 1  and b.party_type=20 GROUP BY a.id, a.supplier_name UNION ALL  SELECT DISTINCT c.id, c.supplier_name FROM lib_supplier_tag_company a, lib_supplier_party_type b, lib_supplier  c, inv_receive_master  d  WHERE   c.id = b.supplier_id AND a.supplier_id = b.supplier_id AND c.id = d.supplier_id	and b.party_type=20  AND c.status_active IN (1, 3) AND c.is_deleted = 0 GROUP BY c.id, c.supplier_name ORDER BY supplier_name";

			// $sql = "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=20 and a.status_active=1 group by a.id,a.supplier_name order by a.supplier_name";
			$knitting_com_td_show = create_drop_down("cbo_knitting_company", 152, " $sql ", "id,supplier_name", 1, "--Select Knit Company--", 0, "load_location();");
	...
	{
		if ($db_type == 0) {
			$reqsn_dataArray = sql_select("select a.knit_id, a.requisition_no, group_concat(distinct(b.id)) as yarn_prod_id from ppl_yarn_requisition_entry a, product_details_master b where a.prod_id=b.id and a.knit_id = $book_plan_id and a.status_active=1 and a.is_deleted=0 group by a.knit_id, a.requisition_no");
		} else {
			$reqsn_dataArray = sql_select("select a.knit_id, a.requisition_no, LISTAGG(b.id, ',') WITHIN GROUP (ORDER BY b.id) as yarn_prod_id from ppl_yarn_requisition_entry a, product_details_master b where a.prod_id=b.id and a.knit_id = $book_plan_id and a.status_active=1 and a.is_deleted=0 group by a.knit_id, a.requisition_no");
		}
		foreach ($reqsn_dataArray as $row) {
	...
	echo "document.getElementById('req_no_id').value	= '" . $REQUISITION_NO . "';\n";
	...
	<div style="width:950px; max-height:200px; overflow-y:scroll" id="list_container_batch" align="left">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="930" class="rpt_table" id="list_view">
			...
			foreach ($result as $row)
			{
				...
				?>
				<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"
					onClick="put_data_dtls_part(<? echo $row[csf('id')]; ?>,'populate_grey_details_form_data', 'requires/grey_production_entry_controller');">
...
if ($action == 'populate_grey_details_form_data')
{
	...
	$rcv_sql = sql_select("SELECT a.roll_maintained as roll_maintained, a.company_id FROM INV_RECEIVE_MASTER a, pro_grey_prod_entry_dtls b WHERE a.id=b.mst_id and b.id='$id'");
	...
	$tube_ref_sett = return_field_value("production_entry", "variable_settings_production", "status_active=1 and company_name=$company_id and variable_list=67");
	...
	foreach ($data_array as $row) {
		...
		if ($row[csf('febric_description_id')] == 0 || $row[csf('febric_description_id')] == "") {
			...
			$determination_sql = sql_select("select a.construction, b.copmposition_id, b.percent from lib_yarn_count_determina_mst a, lib_yarn_count_determina_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.id=" . $row[csf('febric_description_id')]);
		...
		echo "document.getElementById('hdn_rate_string').value 				= '" . $row[csf("production_rate_string")] . "';\n";
		echo "document.getElementById('hdn_rate').value 					= '" . $row[csf("production_rate")] . "';\n";
		echo "document.getElementById('txt_rate').value 					= '" . number_format($row[csf("production_rate")],4) . "';\n";
		...
		if ($roll_maintained == 1)
		{
			$data_roll_array = sql_select("SELECT id, roll_used, po_breakdown_id, qnty,qc_pass_qnty_pcs, roll_no, barcode_no, reject_qnty,coller_cuff_size,rf_id, batch_no, tube_ref_no, tube_qnty, is_sales from pro_roll_details where dtls_id='$id' and entry_form=2 and status_active=1 and is_deleted=0 order by id");
			foreach ($data_roll_array as $row_roll)
			{
				...
				$tube_refer_data ="";
				if($tube_ref_sett ==1 || $row_roll[csf('is_sales')] ==1){
					$tube_refer_data = "**" .$row_roll[csf("batch_no")] . "**". $row_roll[csf("tube_ref_no")] . "**" .$row_roll[csf("tube_qnty")];
				}

				if ($save_string == "")
				{
					$save_string = $row_roll[csf("po_breakdown_id")] . "**" . number_format($row_roll[csf("qnty")],2,'.','') . "**" . $row_roll[csf("reject_qnty")] . "**" . $row_roll[csf("roll_no")] . "**" . $roll_id . "**" . $row_roll[csf("id")] . "**" . $row_roll[csf("barcode_no")] . "**" . $row_roll[csf("qc_pass_qnty_pcs")]. "**" . $row_roll[csf("coller_cuff_size")]. "**" . $row_roll[csf("rf_id")] . $tube_refer_data;
				}
				else
				{
					$save_string .= "," . $row_roll[csf("po_breakdown_id")] . "**" . number_format($row_roll[csf("qnty")],2,'.','') . "**" . $row_roll[csf("reject_qnty")] . "**" . $row_roll[csf("roll_no")] . "**" . $roll_id . "**" . $row_roll[csf("id")] . "**" . $row_roll[csf("barcode_no")] . "**" . $row_roll[csf("qc_pass_qnty_pcs")]. "**" . $row_roll[csf("coller_cuff_size")]. "**" . $row_roll[csf("rf_id")] . $tube_refer_data;
...
if ($action == "roll_maintained") {
	...
	echo "reset_form('greyproductionentry_1','list_fabric_desc_container','','','set_receive_basis();','update_id*txt_recieved_id*cbo_company_id*cbo_receive_basis*txt_receive_date*txt_receive_chal_no*cbo_store_name*cbo_knitting_source*cbo_knitting_company*txt_remarks*roll_maintained*barcode_generation*fabric_store_auto_update*store_update_upto*production_control*txt_yarn_issue_challan_no*txt_shift_name*txt_width*txt_gsm*txt_original_gsm*txt_original_dia_width*txt_old_gsm*txt_old_dia*cbo_floor_id*cbo_machine_name*txt_room*txt_rack*txt_reject_fabric_recv_qnty*txt_shelf*cbo_uom*txt_binbox*txt_yarn_lot*cbo_yarn_count*txt_brand*txt_stitch_length*txt_machine_dia*txt_machine_gg*txt_color*cbo_color_range*hidden_qc_result');\n";
	exit();
}
...
if ($action == "action_rate") {
	...
	if ($receive_basis == 1)
	{
		if ($is_sample == 0)
		{
			if($var_grey_rate_come_hdn==1)
			{
				$yarnLots=explode(",",$yarnLot);
				$yarnLotNo="";
				foreach ($yarnLots as $lotrows)
				{
					$yarnLotNo.="'".$lotrows."',";
				}
				$yarnLotNo=chop($yarnLotNo,",");
				$yarn_issue_sql=sql_select("select a.id,b.cons_rate,c.po_breakdown_id,a.issue_date from inv_issue_master a,inv_transaction b,order_wise_pro_details c,product_details_master d where a.id=b.mst_id and b.id=c.trans_id and c.prod_id=d.id and d.lot in($yarnLotNo) and d.id in( $yarnProd) and a.issue_basis=1 and a.entry_form=3 and c.entry_form=3 and a.booking_id=$booking_id and c.po_breakdown_id in($all_po_id) and a.item_category=1 and b.item_category=1 and b.transaction_type=2");
				$yarnIssueQnty=0;
				foreach ($yarn_issue_sql as $issueRow)
				{
					$yarnIssueQnty+=$issueRow[csf('cons_rate')];
				}
				...
				$exchange_rate=return_field_value("conversion_rate","currency_conversion_rate"," currency=2 and company_id=$company_id and con_date<='$issueDate' and status_active=1 and is_deleted=0 order by con_date desc","conversion_rate");

				//echo $yarnIssueQnty."/".$exchange_rate ."/".$issueDate ;
				$yarnIssueRate=$yarnIssueQnty/$exchange_rate;
				$preCostConverRate=$pre_costing_conversion_cost_sql[0][csf("charge_unit")];
				$yarnIssueString="1_$yarnIssueRate".','."2_$preCostConverRate";
			}
		}
		else
		{
		...
		$totalRate=$yarnIssueRate+$preCostConverRate;
	}
	//echo $sql;die;
	echo "document.getElementById('txt_rate').value 			= '" . number_format($totalRate,4) . "';\n";
	echo "document.getElementById('hdn_rate').value 			= '" . $totalRate . "';\n";
...
if ($action == "rate_popup") {
	...
	<div>
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="340" class="rpt_table">
			...
			$pageName=array(1=>"Yarn Issue",2=>"Knitting Rate");
...
if ($action == "yarn_lot_popup") {
	...
	if($txt_service_booking!=""){
		...
		$precost_sql = sql_select("SELECT exchange_rate from wo_booking_mst where booking_no='$txt_service_booking' and status_active=1 and is_deleted=0 union all select exchange_rate from knitting_work_order_mst where booking_no='$txt_service_booking' and status_active=1 and is_deleted=0");
		$precost_exchange_rate = $precost_sql[0][csf("exchange_rate")];
	...
	if($txt_service_booking!="")
	{
		...
		union all select sum(a.amount)/ sum(a.wo_qty) as charge_unit 
			from knitting_work_order_mst d, knitting_work_order_dtls a ,ppl_planning_info_entry_dtls b,ppl_planning_info_entry_mst c
			where d.id=a.mst_id and a.program_no=b.id  and b.mst_id=c.id 
			and c.determination_id=".$fabric_description_id." and d.wo_no='".$txt_service_booking."'
	...
	else
	{
		...
		if ($is_salesOrder == 1) {
			...
			if($sales_booking_without_order==0)
			{
				/* if($sales_po_job_no)
				{
					$sales_po_job_arr = explode(",",chop($sales_po_job_no,","));
					if(count($sales_po_job_arr)>1)
					{
						$sql_sales_booking = sql_select("SELECT exchange_rate, currency_id from wo_booking_mst a where a.booking_no='$sales_booking_no' group by a.po_id, b.sales_booking_no");

						$precost_exchange_rate = $sql_sales_booking[0][csf("exchange_rate")];

						$sales_currency_id = $sql_sales_booking[0][csf("currency_id")];
						if($precost_exchange_rate=="")
						{
							if($sales_currency_id=="" || $sales_currency_id==0)
							{
								$sales_currency_id=2;
							}
							$conversion_date=change_date_format($txt_receive_date, "d-M-y", "-",1);
							$precost_exchange_rate=set_conversion_rate( $sales_currency_id, $conversion_date, $cbo_company_id );
							//echo $precost_exchange_rate;die;
						}
					}
					else
					{
						...
						$conversion_cost = sql_select("select a.id,a.charge_unit from  wo_pre_cost_fab_conv_cost_dtls a,wo_pre_cost_fabric_cost_dtls b where a.job_no='$sales_po_job_no' and a.fabric_description=b.id and a.job_no=b.job_no and b.lib_yarn_count_deter_id=" . $fabric_description_id . " and a.cons_process in (1,3,4) order by id");
						echo "select a.id,a.charge_unit from  wo_pre_cost_fab_conv_cost_dtls a,wo_pre_cost_fabric_cost_dtls b where a.job_no='$sales_po_job_no' and a.fabric_description=b.id and a.job_no=b.job_no and b.lib_yarn_count_deter_id=" . $fabric_description_id . " and a.cons_process in (1,3,4) order by id";

					}
				} */
				
				$sql_sales_booking = sql_select("SELECT exchange_rate, currency_id from wo_booking_mst a where a.booking_no='$sales_booking_no' group by exchange_rate, currency_id");
	...
	<script>
		...
		function fnc_process_cost() {
			...
			var is_sales_knitting_program =<? echo $is_sales_knitting_program*1; ?>;
			var var_rate_mandatory =<? echo $var_rate_mandatory*1; ?>;

			if($("#txt_knitting_charge_taka").val() * 1 <= 0 && (vari_knit_charge_source ==2 || (process_costing_maintain !=0 && is_sales_knitting_program ==1 && var_rate_mandatory==1)) )
			{
				alert("Knitting Charge Needed "+ process_costing_maintain +'='+is_sales_knitting_program +'='+var_rate_mandatory+'='+$("#txt_knitting_charge_taka").val() * 1);
				return;
			}
    ...
    <div>
		...
		*/
    	...
    	?>
    	<table cellspacing="0" cellpadding="0" border="0" rules="all" width="1040" class="" align="center">
    		<tr>
    			...
    			<td colspan="5" align="center" style="font-size:16px">
    				<strong>Knitting Charge:
    					<?php
						//if($process_costing_maintain !=0 && $is_sales_knitting_program ==1 && $var_rate_mandatory==1)
						if($vari_knit_charge_source==3)
						{
							$sql_sales = sql_select("SELECT sum(b.knit_charge) as knit_charge from fabric_sales_order_dtls a, fabric_sales_order_prcs_rate b where a.id=b.dtls_id and a.status_active=1 and b.status_active=1 and a.mst_id = $sales_order_id and a.determination_id=$fabric_description_id and a.gsm_weight=$txt_original_gsm and a.dia=$txt_original_dia_width and a.body_part_id=$cbo_body_part ");
							$knit_charge_sales = $sql_sales[0][csf("knit_charge")];
							echo number_format($knit_charge_sales, 2);
							?>
    						<input type="hidden" name="txt_knitting_charge_taka" id="txt_knitting_charge_taka" class="text_boxes_numeric" value="<? echo number_format($knit_charge_sales, 2,'.',''); ?>">
    						<?
						}
    					else if($vari_knit_charge_source == 1)
    					{
							//echo $conversion_cost[0][csf('charge_unit')] .'*'. $precost_exchange_rate;die;
    	...
    	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1060" class="rpt_table">
    		<thead>
    			...
    			<th width="80">Cum. Used Qty</th>
    			<th width="50">Yarn %</th>
    			<th width="100">Avg Yarn Rate /Kg (Tk.)</th>
    			<th width="100">Net Qty</th>
    			<th width="80">Process Loss %</th>
    			<th>Used Qty</th>
    		</thead>
    	</table>
    	<div style="width:1060px; max-height:280px; overflow-y:scroll">
    		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1040" class="rpt_table"
    		id="tbl_lot_list">
    		<?php
    		$i = 1;
    		...
    		if ($recieve_basis == 1) {
    			if ($booking_without_order == 0) {
    				...
    				$issue_rtn_sql = "select c.id,c.lot,c.brand, c.yarn_count_id, c.yarn_comp_type1st, c.yarn_comp_percent1st, c.yarn_comp_type2nd, c.yarn_comp_percent2nd, c.yarn_type,sum(b.cons_quantity) issue_rtn_qty,sum(b.cons_amount) cons_amount from inv_receive_master a, inv_transaction b, product_details_master c where a.id=b.mst_id and  a.booking_id=$booking_id and a.entry_form=9 and b.prod_id=c.id and a.item_category=1 and a.status_active=1 and a.is_deleted=0 and b.transaction_type=4 and b.item_category=1  and b.status_active=1 and b.is_deleted=0 and a.knit_dye_source in(1,3) group by c.id,c.lot,c.brand,c.supplier_id, c.yarn_count_id, c.yarn_comp_type1st, c.yarn_comp_percent1st, c.yarn_comp_type2nd, c.yarn_comp_percent2nd, c.yarn_type";
    				...
    				$issue_rtn_sql = "select c.id,c.lot,c.brand, c.yarn_count_id, c.yarn_comp_type1st, c.yarn_comp_percent1st, c.yarn_comp_type2nd, c.yarn_comp_percent2nd, c.yarn_type,sum(b.cons_quantity) issue_rtn_qty,sum(b.cons_amount) cons_amount from inv_receive_master a, inv_transaction b, product_details_master c where a.id=b.mst_id and  a.booking_no='$txt_booking_no' and a.entry_form=9 and b.prod_id=c.id and a.item_category=1 and a.status_active=1 and a.is_deleted=0 and b.transaction_type=4 and b.item_category=1  and b.status_active=1 and b.is_deleted=0 and a.knit_dye_source in(1,3) group by c.id,c.lot,c.brand,c.supplier_id, c.yarn_count_id, c.yarn_comp_type1st, c.yarn_comp_percent1st, c.yarn_comp_type2nd, c.yarn_comp_percent2nd, c.yarn_type";
    			}
    		} else if ($recieve_basis == 2) {
    			...
    			$issue_rtn_sql = "select c.id,c.lot,c.brand, c.yarn_count_id, c.yarn_comp_type1st, c.yarn_comp_percent1st, c.yarn_comp_type2nd, c.yarn_comp_percent2nd, c.yarn_type,sum(b.cons_quantity) issue_rtn_qty,sum(b.cons_amount) cons_amount from inv_receive_master a, inv_transaction b, product_details_master c where a.id=b.mst_id and b.requisition_no in($reqsition_number) and a.entry_form=9 and b.prod_id=c.id and a.item_category=1 and a.status_active=1 and a.is_deleted=0 and b.transaction_type=4 and b.item_category=1 and b.status_active=1 and b.is_deleted=0 group by c.id, c.lot, c.brand, c.supplier_id, c.yarn_count_id, c.yarn_comp_type1st, c.yarn_comp_percent1st, c.yarn_comp_type2nd, c.yarn_comp_percent2nd, c.yarn_type";
    		}
			else if ($recieve_basis == 4)
			{
				$sql = "select c.id,c.lot,c.brand, c.yarn_count_id, c.yarn_comp_type1st, c.yarn_comp_percent1st, c.yarn_comp_type2nd, c.yarn_comp_percent2nd, c.yarn_type,sum(b.cons_quantity) issue_qty,sum(b.cons_amount) cons_amount from inv_issue_master a, inv_transaction b, product_details_master c where a.id=b.mst_id and  a.booking_no='$txt_booking_no' and a.entry_form=3 and b.prod_id=c.id and a.item_category=1 and a.status_active=1 and a.is_deleted=0 and b.transaction_type=2 and b.item_category=1  and b.status_active=1 and b.is_deleted=0 and a.knit_dye_source in(1,3) and a.issue_basis=4 group by c.id,c.lot,c.brand,c.supplier_id, c.yarn_count_id, c.yarn_comp_type1st, c.yarn_comp_percent1st, c.yarn_comp_type2nd, c.yarn_comp_percent2nd, c.yarn_type";
			...
			$issue_rtn_data = sql_select($issue_rtn_sql);
			foreach ($issue_rtn_data as $row) {
				$issue_qnty_arr[$row[csf('id')]][$row[csf('lot')]][$row[csf('brand')]][$row[csf('yarn_count_id')]][$row[csf('yarn_comp_type1st')]][$row[csf('yarn_comp_percent1st')]][$row[csf('yarn_comp_type2nd')]][$row[csf('yarn_comp_percent2nd')]][$row[csf('yarn_type')]]['issueRtnQnty']+= $row[csf('issue_rtn_qty')];			
			}
			...
			$cum_usedQnty_sql = sql_select("select id,prod_id,used_qty,rate,amount,yarn_percentage,porcess_loss from pro_material_used_dtls where prod_id in($prodIds)");
			foreach ($cum_usedQnty_sql as $rows) {
				$cum_usedQnty_arr[$rows[csf('prod_id')]]["used_qty"]+=$rows[csf('used_qty')];
			}
    		...
    		foreach ($nameArray as $row) {
    			if ($i % 2 == 0)
    				...
    				$bgcolor = "#FFFFFF";
    			$cummUsedQnty=$cum_usedQnty_arr[$row[csf('id')]]["used_qty"];
    			...
    			if (in_array($row[csf("id")], $check_arr)) {
    				...
    				$issueRtnQnty=$issue_qnty_arr[$row[csf('id')]][$row[csf('lot')]][$row[csf('brand')]][$row[csf('yarn_count_id')]][$row[csf('yarn_comp_type1st')]][$row[csf('yarn_comp_percent1st')]][$row[csf('yarn_comp_type2nd')]][$row[csf('yarn_comp_percent2nd')]][$row[csf('yarn_type')]]['issueRtnQnty'];
    				?>
    				...
    				<td width="60" align="right" title="Yarn issue - yarn Issue Return"><p><?php echo $row[csf('issue_qty')]-$issueRtnQnty; ?></p></td>
    				<td width="80" align="right"><p><?php echo $cummUsedQnty; ?></p></td>
    				...
    				<td width="100" align="right" id="net_used_td_<? echo $i; ?>">
    					<p><?php echo number_format($net_used, 4, ".", ""); ?></p>
    				</td>
    				<td width="80" align="right">
    					<input type="text" id="txt_process_loss_<? echo $i; ?>" name="txt_process_loss[]" style="width:70px" class="text_boxes_numeric"
    			...
    			<td width="100" align="right">
    				...
    				<td width="100" align="right" id="net_used_td_<? echo $i; ?>"><p><?php echo number_format($net_used,4,".",""); ?></p>
    				</td>
    				<td width="80" align="right"><input type="text" id="txt_process_loss_<? echo $i; ?>"
    					name="txt_process_loss[]" style="width:70px"
    					class="text_boxes_numeric"
...
<table width="1040" cellspacing="0" cellpadding="0" border="1" align="center">
	<tr>
		<td align="center" height="30" valign="bottom">
			<div style="width:100%">
...
if ($action == "show_roll_listview") {
	...
	{
		$query = "select a.id, a.roll_no, a.barcode_no, a.po_breakdown_id, a.qnty, b.job_no as po_number, a.rf_id from pro_roll_details a, fabric_sales_order_mst b where a.po_breakdown_id=b.id and a.dtls_id=$dtls_id and a.entry_form=2 and a.receive_basis=4 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.barcode_no";
	}
	else
	{
	...
	<div align="center" id="button_data_panel">
		...
		foreach($report_format_id as $result){
			...
			<?}elseif($result==390){?>
				<input type="button" id="btn_barcode_k" name="btn_barcode_k" value="Barcode K" class="formbutton" onClick="fnc_barcode_code128(11)"/>
			<?}elseif($result==810){?>
				<input type="button" id="btn_barcode_ccl" name="btn_barcode_ccl" value="Barcode CCL" class="formbutton" onClick="fnc_barcode_code128(12)"/>
			<?}elseif($result==847){?>
				<input type="button" id="btn_barcode_k2" name="btn_barcode_k2" value="Barcode K2" class="formbutton" onClick="fnc_barcode_code128(13)"/>
			<?}elseif($result==872){?>
				<input type="button" id="btn_barcode_ccl_v2" name="btn_barcode_ccl_v2" value="Barcode CCL V2" class="formbutton" onClick="fnc_barcode_code128(14)"/>
			<?}elseif($result==583){?>
				<input type="button" id="btn_barcode_k3" name="btn_barcode_k3" value="Barcode K3" class="formbutton" onClick="fnc_barcode_code128(15)"/>
			<?}
		}
...
if ($action == "print_barcode_one_128") {
	...
	/*	if ($booking_without_order == 1) {
		...
		$pdf->SetXY($i, $j+19.8);
		$pdf->Write(0, "Count:".substr($yarn_count, 0, 10) . "; Lot: " . substr($yarn_lot, 0, 10));
		$pdf->SetXY($i, $j+23.1);
		$pdf->Write(0, "SL: " . trim(substr($stitch_length, 0, 15)).";Clr: " .substr($color, 0, 10));
...
if ($action == "direct_print_barcode") {
	...
	{
		$sales_info = sql_select("select a.job_no_prefix_num,a.job_no,a.sales_booking_no, a.within_group, a.buyer_id, a.po_buyer, a.style_ref_no from fabric_sales_order_mst a where a.id='" . $row[csf("booking_id")] . "'");
		if($sales_info[0][csf('within_group')]==1)
		{
			$buyer_name = return_field_value("short_name", "lib_buyer", "id=" . $sales_info[0][csf('po_buyer')]);
		}
		else
		{
			$buyer_name = return_field_value("short_name", "lib_buyer", "id=" . $sales_info[0][csf('buyer_id')]);
		...
		$bookingNo = $sales_info[0][csf('job_no')];
	...
	else
	{
		...
		if ($is_salesOrder == 1) {
			$po_sql = sql_select("select a.job_no_prefix_num,a.job_no as po_number,a.sales_booking_no,a.style_ref_no,b.buyer_id from fabric_sales_order_mst a inner join wo_booking_mst b on a.sales_booking_no = b.booking_no where a.id in($order_id)");
			foreach ($po_sql as $row) {
				$po_array[$row[csf('id')]]['no'] = $row[csf('po_number')];
				$po_array[$row[csf('id')]]['job_no'] = $row[csf('job_no')];
			...
			$po_sql = sql_select("select a.job_no, a.job_no_prefix_num, a.style_ref_no, b.id,b.grouping, b.po_number from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and b.id in($order_id) group by a.job_no,a.job_no_prefix_num, a.style_ref_no,b.id,b.grouping,b.po_number");
			foreach ($po_sql as $row) {
				$po_array[$row[csf('id')]]['no'] = $row[csf('po_number')];
				$po_array[$row[csf('id')]]['job_no'] = $row[csf('job_no')];
				$po_array[$row[csf('id')]]['prefix'] = $row[csf('job_no_prefix_num')];
	...
	$i = 1;
	$barcode_array = array();
	$query = "SELECT a.id, a.roll_no, a.po_breakdown_id, a.barcode_no, a.qnty, a.qc_pass_qnty_pcs as qnty_pcs, a.coller_cuff_size, b.fabric_grade, a.is_sales from pro_roll_details a left join pro_qc_result_mst b on a.barcode_no=b.barcode_no where a.id in($data[0])";
	// echo $query;
	$res = sql_select($query);
	...
	foreach ($res as $row)
	{
		...
		$pdf->SetXY($i, $j+9.2);
		$pdf->Write(0, "Sty.No :" . $style_ref);

		$pdf->SetXY($i, $j+12);
		$pdf->Write(0, "Po:" . substr($order_no,0,25) );

		$pdf->SetXY($i, $j+14.5);
		$pdf->Write(0, substr($comp,0,42) );

		$pdf->SetXY($i, $j+18);
		$pdf->Write(0, "F/GSM: " . substr($gsm, 0, 6).";Clr: " .substr($color, 0, 22));

		$pdf->SetXY($i, $j+21);
		//$pdf->Write(0, "C:".$yarn_count . "; L: " . $yarn_lot);
		$pdf->Write(0, "C:".substr($yarn_count, 0, 10) . "; L: " . substr($yarn_lot, 0, 30));
		...
		$pdf->Code128($i+1,$j+42.5,$row[csf("barcode_no")],50,8);
...
if ($action == "print_barcode_one_128_v2") {
	...
	foreach ($result as $row) {
		...
		foreach ($lot_string as $val)
		{
			if($val!="")
			{
				if($brandChk[$brand_id_arr[$val]]=="")
				{
					$brandChk[$brand_id_arr[$val]]=$brand_id_arr[$val];
					$brand .= $brand_arr[$brand_id_arr[$val]] . ",";
				}
			}
		}
		$brand = chop($brand, ',');
	...
	foreach ($res as $row) {
		...
		$pdf->Code128($i+10,$j,$row[csf("barcode_no")],50,8);
		$pdf->SetXY($i, $j+10);
		...
		$pdf->SetXY($i, $j+26);
		$pdf->Write(0, "Ct:".substr($yarn_count.",Lt:".$yarn_lot,0,35));

		$pdf->SetXY($i, $j+30);
		$pdf->Write(0, "Br:". substr($brand.",".$constuction,0,35));
	//
		$pdf->SetXY($i, $j+34);
...
if ($action == "print_barcode_one_128_v4")  // Barcode 128 v3 button Tipu
{
	require('../../ext_resource/pdf/code128.php');
	define('FPDF_FONTPATH', '../../ext_resource/pdf/fpdf/font/');
	...
	$sql = "SELECT a.company_id,a.receive_basis,a.booking_id, a.booking_no, a.booking_without_order, a.within_group, a.receive_date,a.buyer_id, a.knitting_source, a.knitting_company, b.order_id, b.prod_id, b.gsm,b.width, b.yarn_lot, b.yarn_count, b.brand_id, b.machine_no_id, b.stitch_length, b.machine_dia, b.machine_gg, b.color_id, b.febric_description_id, b.insert_date, b.color_range_id,b.operator_name, b.floor_id from inv_receive_master a, pro_grey_prod_entry_dtls b where a.id=b.mst_id and b.id=$data[1] order by yarn_count";
	//echo $sql;die;
	$result = sql_select($sql);
	$party_name = '';
	$prod_date = '';
	...
	foreach ($result as $row)
	{
		...
		$yarn_lotArr = explode(",", $row[csf('yarn_lot')]);
		foreach ($yarn_lotArr as $val)
		{
			if ($val !="") {
				if ($yarn_lotString == "") $yarn_lotString = "'".$val."'"; else $yarn_lotString .= "," . "'".$val."'";
			}
		}
		...
		$brand = chop($brand, ',');*/
		$brand = $brand_arr[$row[csf('brand_id')]];
		$yarnCountString=$row[csf('yarn_count')];
		$yarn_count = '';
		$count_id = explode(",", $row[csf('yarn_count')]);
		foreach ($count_id as $val)
		{
		...
		if ($row[csf("receive_basis")] == 2)
		{
			...
			$grey_dia = $planning_data[0][csf('machine_dia')];
			// $tube_type = $fabric_typee[$planning_data[0][csf('width_dia_type')]];
			$tube_type = $planning_data[0][csf('width_dia_type')];
		...
		$comp=implode(",", array_unique(explode(",", $comp)));


		$company_short_name = return_field_value("company_short_name", "lib_company", "id=" . $row[csf('company_id')]);
	}
	$reqsn_dataArraySql = sql_select("select knit_id, requisition_no,requisition_date,prod_id,sum(no_of_cone) as no_of_cone , sum(yarn_qnty) as yarn_qnty from ppl_yarn_requisition_entry where knit_id in(".$program_no.") and status_active=1 and is_deleted=0 group by knit_id, prod_id, requisition_no,requisition_date");


	$product_idss="";
	foreach ($reqsn_dataArraySql as $rowx)
	{
		$product_idss.= $rowx[csf('prod_id')].",";
	}
	$product_idss=chop($product_idss,",");

	$sqlYarnIssueCountLot= sql_select("select c.yarn_count_id,c.lot from product_details_master c where c.lot in($yarn_lotString) and c.item_category_id=1 and c.yarn_count_id in($yarnCountString) and c.id in($product_idss)  and c.status_active=1 and c.is_deleted=0  order by  c.yarn_count_id,c.lot desc");
	//group by c.yarn_count_id,c.lot
	$yarnLotString2="";
	foreach ($sqlYarnIssueCountLot as $row22)
	{
		if ($row22 > 0) {
			if ($yarn_count == "") $yarn_count = $count_arr[$row22[csf('yarn_count_id')]]; else $yarn_count .= "," . $count_arr[$row22[csf('yarn_count_id')]];
		}


		$yarnLotString2.=$row22[csf('lot')].",";
	}
	$yarnLotString2=chop($yarnLotString2,",");
	...
	$coller_cuff_size_sql="SELECT c.coller_cuff_size, c.qc_pass_qnty_pcs, c.barcode_no,a.booking_id
    ...
    // echo $coller_cuff_size_sql;
    $coller_cuff_size_res = sql_select($coller_cuff_size_sql);
    $coller_cuff_size_arr = array();$programNo="";
    foreach ($coller_cuff_size_res as $row)
	{
		...
		$programNo.=$row[csf('booking_id')].",";
	}

	$programNo=chop($programNo,",");
	$internalRefQry="select a.dtls_id as program,d.grouping as internal_ref from PPL_PLANNING_ENTRY_PLAN_DTLS a,wo_booking_dtls b, wo_po_details_master c, WO_PO_BREAK_DOWN d where  a.dtls_id in($programNo) and a.booking_no=b.booking_no and b.job_no=c.job_no and c.id=d.job_id group by  a.dtls_id,d.grouping";
	$internalRefSql = sql_select($internalRefQry);
	foreach ($internalRefSql as $rows)
	{
		$interRef_arr[$rows[csf('program')]]['internal_ref'] = $rows[csf('internal_ref')];
	}

	$i = 1;
	$barcode_array = array();
	$query = "SELECT a.id,a.inserted_by, a.insert_date, a.roll_no, a.po_breakdown_id, a.barcode_no, a.qnty, b.fabric_grade,c.shift_name,d.recv_number_prefix_num, d.recv_number,d.booking_id from pro_roll_details a left join pro_qc_result_mst b on a.barcode_no=b.barcode_no left join pro_grey_prod_entry_dtls c on a.dtls_id=c.id left join inv_receive_master d on c.mst_id=d.id where a.id in($data[0])";
	$res = sql_select($query);
	...
	foreach ($res as $row)
	{
		...
		$coller_cuff_size = $coller_cuff_size_arr[$row[csf('barcode_no')]]['coller_cuff_size'];
		$qc_pass_qnty_pcs = $coller_cuff_size_arr[$row[csf('barcode_no')]]['qc_pass_qnty_pcs'];
		...
		$inteRef=$interRef_arr[$row[csf('booking_id')]]['internal_ref'];
		...
		$pdf->SetXY($i, $j+13);
		//." B: " . $po_array[$row[csf('po_breakdown_id')]]['booking_no']
		$pdf->Write(0, $machine_name . "," . $machine_dia_width . "X" . $machine_gauge. ",RW:" . number_format($row[csf('qnty')], 2, '.', '') . ",S-" . $coller_cuff_size. ",P-" . number_format($qc_pass_qnty_pcs). ",R.N:" . $row[csf("roll_no")]);

		$pdf->SetXY($i, $j+17);
		$pdf->Write(0, $buyer_name . ",B/N:" . $booking_no . ",Pg:" .$program_no.",FSO:". $order_no.",IR:". $inteRef);//24

		$pdf->SetXY($i, $j+20);
		$pdf->Write(0, "Clr:" .substr($color.",".$program_dye_type, 0, 35));


		$pdf->SetXY($i, $j+23);
		//$pdf->Write(0, "Ct:".$yarn_count.",Lt:".$yarn_lot);
		// $pdf->Write(0, $constuction.", Ct:".$yarn_count);
		$pdf->Write(0, substr($constuction, 0, 35).", Ct:".substr($yarn_count, 0, 15));

		$pdf->SetXY($i, $j+26);
		$pdf->Write(0, "Lt:".substr($yarnLotString2, 0, $avilLotLen));

		$pdf->SetXY($i, $j+30);
		$pdf->Write(0, substr($comp, 0, 40));
...
if ($action == "print_barcode_one_128_v3")
{
	...
	foreach ($result as $row)
	{
		...
		$receive_date=$row[csf('receive_date')];
		$booking_no = $row[csf('booking_no')];
		$machine_no_id = $machine_no_arr[$row[csf('machine_no_id')]];
	...
	foreach ($res as $row)
	{
		...
		$pdf->SetXY($i, $j+14);
		$pdf->Write(0, $job_no."::" . $buyer_name ."::" . $machine_no_id ."::" . $machine_dia_width . "X" . $machine_gauge. "::" . $coller_cuff_size);
...
if ($action == "print_barcode_one_88Y") // For Youth Group Create by Tipu
{
	...
	foreach ($res as $row)
	{
		...
		$pdf->SetXY($i+35, $j+8.8);
		$pdf->Write(0, $probram_no);
		$pdf->SetXY($i+35, $j+11.4);
		$pdf->Write(0, "R ".$row[csf("roll_no")]);
		...
		$pdf->SetXY($i, $j+23);
		$pdf->Write(0, substr($yarn_count, 0, 35).",".substr($brand_arr[$brand_id], 0, 70).",Lot:".substr($yarn_lot, 0, 20));
...
if ($action == "report_barcode_text_file") {
	...
	else
	{
		...
		else
		{
			...
			$po_sql = sql_select("select a.job_no, a.style_ref_no, a.buyer_name, a.job_no_prefix_num, b.id, b.po_number, b.grouping, b.file_no, a.quality_level from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and b.id in($order_id)");
			foreach ($po_sql as $row) {
				$po_array[$row[csf('id')]]['no'] = $row[csf('po_number')];
				$po_array[$row[csf('id')]]['job_no'] = $row[csf('job_no')];
				$po_array[$row[csf('id')]]['prefix'] = $row[csf('job_no_prefix_num')];
				...
				$po_array[$row[csf('id')]]['order_nature'] = $row[csf('quality_level')];
			}
		}
	}
	$within_group = $row[csf("within_group")];
	...
	foreach ($res as $row) {
		...
		$txt .= "" . $fbooking_order_nature[$po_array[$row[csf('po_breakdown_id')]]['order_nature']] . "\r\n";
...
if ($action == "print_barcode_k") // Create by Tipu for AKH (duplicate Send to Printer)
{
	require('../../ext_resource/pdf/code128.php');
	define('FPDF_FONTPATH', '../../ext_resource/pdf/fpdf/font/');
	...
	$pdf=new PDF_Code128('P','mm',array(60,40)); // Convert Pixel to Millimeter array(mm,mm)
	$pdf->SetAutoPageBreak(false);
	$pdf->AddPage();
	$pdf->SetFont('Times','',8);
	//echo $within_group;die;
	...
	foreach ($res as $row)
	{
		...
		$iref_no=$po_array[$row[csf('po_breakdown_id')]]['grouping'];//$internal_reference_no

		if ($booking_without_order == 1)
		{
			//$txt .= $party_name;
			$job_no = $booking_no_prefix;
			$machine = $machine_name . "-" . $machine_dia_width . "X" . $machine_gauge;
			//$full_job_no = $full_booking_no;
		}
		else
		{
			//$txt .= $party_name;
			$job_no = $po_array[$row[csf('po_breakdown_id')]]['prefix'];
			$machine = $machine_name . "-" . $machine_dia_width . "X" . $machine_gauge;
			//$full_job_no = $po_array[$row[csf('po_breakdown_id')]]['job_no'];
		}
		$company_job_mc=$company_short_name . " Job No:" . $job_no. ", M/C:" . $machine;
		//$text="ABCDEFGHIJKLMNOPABCDEFGHIJKLMNOPX";//31 CAR
		...
		$pdf->SetXY($i, $j+2);
		$pdf->Write(0, substr($company_job_mc, 0, 33));

		$pdf->Code128($i,$j+4,$row[csf("barcode_no")],50,5);
		$pdf->SetXY($i, $j+11);
		$pdf->Write(0, substr($row[csf("barcode_no")]. ", Wt:" . number_format($row[csf('qnty')], 2, '.', '')."Kg", 0, 33));

		$pdf->SetXY($i, $j+14);
		$pdf->Write(0, substr("D:".change_date_format($receive_date). ", ".$buyer_name, 0, 33));

		$pdf->SetXY($i, $j+17);
		$pdf->Write(0, substr("F-No:" .$file_no.", RF-No:".$iref_no.", ".$shiftName, 0, 33));

		$pdf->SetXY($i, $j+20);
		$pdf->Write(0, substr($construction.', '.$composi, 0, 33));

		$pdf->SetXY($i, $j+23.4);
		$pdf->Write(0, substr("G/F-Dia:".trim($grey_dia).'/'.trim($finish_dia) ." SL:". trim($stitch_length) ." ". trim($tube_type), 0, 33));

		$pdf->SetXY($i, $j+26.4);
		$pdf->Write(0, substr($yarn_count.' '.$brand.", Lot:".$yarn_lot, 0, 33));

		$pdf->SetXY($i, $j+29.5);
		$pdf->Write(0, substr("Prog:" . $program_no.", GSM:". $gsm, 0, 33));

		$pdf->SetXY($i, $j+32.5);
		$pdf->Write(0, substr("RL No:". $row[csf("roll_no")].", " .$color, 0, 25));

		$pdf->SetXY($i, $j+35.5);
		$pdf->Write(0, substr("Color range: ". $colorRange, 0, 33));
...
if ($action == "print_barcode_k2") // Create by Tipu for AKH (duplicate Send to Printer)
{
	require('../../ext_resource/pdf/code128.php');
	define('FPDF_FONTPATH', '../../ext_resource/pdf/fpdf/font/');
	...
	$sql = "SELECT a.company_id, a.recv_number, a.knitting_location_id, a.location_id, a.receive_basis, a.booking_id, a.booking_no, a.booking_without_order, a.within_group, a.receive_date, a.buyer_id, a.knitting_source, a.knitting_company,a.yarn_issue_challan_no, b.order_id, b.prod_id, b.gsm, b.width, b.yarn_lot, b.yarn_count, b.brand_id, b.machine_no_id, b.stitch_length, b.machine_dia, b.machine_gg, b.color_id, b.febric_description_id, b.shift_name, b.insert_date,b.operator_name, b.color_range_id, b.body_part_id, b.floor_id,a.recv_number_prefix_num  from inv_receive_master a, pro_grey_prod_entry_dtls b where a.id=b.mst_id and b.id=$data[1]";
	$result = sql_select($sql);
	$party_name = '';
	$prod_date = '';
	$order_id = '';
	...
	//$pdf=new PDF_Code128('P','mm',array(60,40)); // Convert Pixel to Millimeter array(mm,mm)
	$pdf=new PDF_Code128('P','mm',array(60,60)); // Convert Pixel to Millimeter array(mm,mm)
	$pdf->SetAutoPageBreak(false);
	$pdf->AddPage();
	$pdf->SetFont('Times','',8);
	//echo $within_group;die;
	...
	foreach ($res as $row)
	{
		...
		$company_job_mc=$company_short_name .", M/C:" . $machine. ", KC:" . $party_name;
		//$text="ABCDEFGHIJKLMNOPABCDEFGHIJKLMNOPX";//31 CAR
		...
		$pdf->Code128($i,$j+4,$row[csf("barcode_no")],55,5);
		$pdf->SetXY($i, $j+11);
		$pdf->Write(0, substr($row[csf("barcode_no")]. ", Wt:" . number_format($row[csf('qnty')], 2, '.', '')."Kg", 0, 33));
		...
		$pdf->SetXY($i, $j+20);
		$pdf->Write(0, substr($construction.', '.$composi, 0, 50));

		$pdf->SetXY($i, $j+23.4);
		$pdf->Write(0, substr("G/F-Dia:".trim($grey_dia).'/'.trim($finish_dia) ." SL:". trim($stitch_length) ." ". trim($tube_type), 0, 33));

		$pdf->SetXY($i, $j+26.4);
		$pdf->Write(0, substr($yarn_count.' '.$brand, 0, 33));

		$pdf->SetXY($i, $j+29.4);
		$pdf->Write(0, substr("Lot:".$yarn_lot, 0, 45));
		...
		$pdf->SetXY($i, $j+38.5);
		$pdf->Write(0, substr("Color range: ". $colorRange.", PDR:".$recv_number_prefix_num, 0, 50));
...
if ($action == "print_barcode_k3") // Create by Abu Sayed for AKH 
{
	$data = explode("***", $data);
	...
	$sql_yarn_info=sql_select("SELECT a.prod_id,b.brand,b.yarn_type,b.yarn_comp_type1st,b.yarn_comp_type2nd,b.yarn_comp_percent1st,b.yarn_comp_percent2nd,b.lot,b.yarn_count_id from pro_material_used_dtls a,product_details_master b  where a.prod_id=b.id and  a.mst_id=".$data[4]." and a.dtls_id=".$data[1]." and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
	...
	{
		$costing_yarn_composition='';
		$costing_yarn_brand=$brand_arr[$p_val[csf('brand')]];
		$costing_yarn_lot=trim($p_val[csf('lot')]);
		$costing_yarn_count=$count_arr[$p_val[csf('yarn_count_id')]];
		$costing_yarn_composition=$composition[$p_val[csf('yarn_comp_type1st')]] . " " . $p_val[csf('yarn_comp_percent1st')] . "%";
		if ($p_val[csf('yarn_comp_type2nd')] != 0) $costing_yarn_composition .= " " . $composition[$p_val[csf('yarn_comp_type2nd')]] . " " . $p_val[csf('yarn_comp_percent2nd')] . "%";
		$yarn_information_string.=$costing_yarn_brand." ".$costing_yarn_lot." ".$costing_yarn_count." ".$costing_yarn_composition. "\r\n";
		$yarn_information_string_without_compo.=$costing_yarn_brand." ".$costing_yarn_lot." ".$costing_yarn_count. "\r\n";
		$all_yarn_type.=",".$yarn_type[$p_val[csf('yarn_type')]];
		$yarn_counter--;
	}
	...
	$mpdf = new mPDF('',    // mode - default ''
					array(39,75),		// array(65,210),    // format - A4, for example, default ''
	...
	{
		$file_no=$po_array[$row[csf('po_breakdown_id')]]['file_no'];
		$iref_no=$po_array[$row[csf('po_breakdown_id')]]['grouping'];
		...
		$filename = $PNG_TEMP_DIR.md5($row[csf("barcode_no")]).'.png';
		...
		<table  cellpadding="0" cellspacing="0" style="padding-top:6px;font-size:11px;width:100%;">
			<tr>
				<td colspan="2">'.$company_job_mc.'</td>
			...
			<tr >
				...
				<td style="width:75%;">
					<table cellpadding="0" cellspacing="0" >
						...
						<tr>
							<td >'.$row[csf("barcode_no")].", ".$buyer_name.'</td>
						</tr>
						<tr>
						<td>'."Date:".change_date_format($receive_date). ", Wt:".number_format($row[csf('qnty')], 2, '.', '')."Kg, Shift: ".trim($shiftName).'</td>
						</tr>
						
					</table>
				</td>
				<td  align="right" style="padding-top: -20px;padding-right: -5px; width:25%;">
					<div id="div_'.$i.'"><img src="'.$PNG_WEB_DIR.basename($filename).'" height="55" width=""></div>
				</td>
			</tr>
			<tr>
				<td colspan="2">
					<table cellpadding="0" cellspacing="0" >
							<tr >
								<td >'.substr("F.No: ".$file_no . "Ref-No: ".$iref_no, 0, 70). '</td>
							</tr>
							<tr >
								<td colspan="2" >'.substr($construction.', '.$composi, 0, 70).'</td>
							</tr>
							<tr >
								<td colspan="2" >'.substr("G/F-Dia: ".trim($grey_dia).'/'.trim($finish_dia) .", ". trim($tube_type) .", Prog: ". $program_no, 0, 70).'</td>
							</tr>
							<tr >
								<td colspan="2" >'.substr("GSM: ".$gsm .", SL: ". trim($stitch_length). ", RL No:". $row[csf("roll_no")], 0, 70).'</td>
							</tr>
							<tr >
								<td colspan="2" >'.substr($yarn_count.' '.$brand.", Lot: ".$yarn_lot  , 0, 70).'</td>
							</tr>
							<tr >
								<td colspan="2" >'.substr($color, 0, 40).", " .substr("Color range: ".$colorRange, 0, 70). '</td>
							</tr>
					</table>
				</td>
			</tr>
...
if ($action == "print_barcode_ccl")
{
	...
	// $sql_brand = "select lot, brand from product_details_master where item_category_id=1 order by lot desc";
	// echo $sql_brand;
	// $brand_id_arr = array();
	// $result_brand = sql_select($sql_brand);
	// foreach ($result_brand as $b_row)
	// {
	// 	$brand_id_arr[$row[csf('lot')]]['brand'] = $row[csf('brand')];
	// }
	// echo "<pre>";
	...
	if ($data[1] != "") {
		...
		$update_sql = sql_select("select id,prod_id,used_qty,rate,amount,yarn_percentage,porcess_loss from pro_material_used_dtls where mst_id=$rcv_id and dtls_id =$data[1]");
		$update_data_arr = array();
		foreach ($update_sql as $val) {
	...
	if ($booking_without_order == 1) {
		if ($row[csf("receive_basis")] == 4) {
			$sales_info = sql_select("select a.job_no_prefix_num,a.job_no,a.sales_booking_no, a.po_buyer, a.buyer_id, a.within_group from fabric_sales_order_mst a inner join wo_booking_mst b on a.sales_booking_no = b.booking_no where a.id='" . $row[csf("booking_id")] . "'");
			if($sales_info[0][csf('within_group')]==1)
			{
				$buyer_name_id= $sales_info[0][csf('po_buyer')];
			}else{
				$buyer_name_id= $sales_info[0][csf('buyer_id')];
			}
			$buyer_name = return_field_value("short_name", "lib_buyer", "id=" . $buyer_name_id);
		...
		if ($is_salesOrder == 1) {
			...
			$po_sql = sql_select("select a.id, a.job_no_prefix_num,a.job_no as po_number,a.sales_booking_no,a.style_ref_no, customer_buyer,a.booking_id, a.po_buyer, a.buyer_id, a.within_group from fabric_sales_order_mst a where a.id in ($order_id) ");
			...
			foreach ($po_sql as $row) {
				...
				if($row[csf('within_group')]==1)
				{
					$buyer_name_id= $row[csf('po_buyer')];
				}else{
					$buyer_name_id= $row[csf('buyer_id')];
				}
	...
	foreach ($res as $row) {
		...
		$pdf->Write(0, "Buyer: ");
		$pdf->SetFont('Arial','B',7);
		$pdf->Write(0, substr($buyer_name, 0, 20) );
		$pdf->SetFont('Arial','B',7);
		$pdf->Write(0, ", Int.B: ");
		$pdf->SetFont('Arial','B',7);
		$pdf->Write(0, substr($internal_ref, 0, 20));
		$pdf->SetFont('Arial','B',7);
		$pdf->Write(0, ", P: ");
		$pdf->SetFont('Arial','B',7);
...
if ($action == "print_barcode_ccl_v2")
{
	require('../../ext_resource/pdf/code128.php');
	define('FPDF_FONTPATH', '../../ext_resource/pdf/fpdf/font/');
	...
	if ($booking_without_order == 1) {
		if ($recieve_basis == 4) {
			...
			$booking_nos = return_field_value("b.booking_no as booking_no", "ppl_planning_info_entry_dtls a,ppl_planning_info_entry_mst b", " b.id=a.mst_id and a.id='" . $booking_id . "'", "booking_no");
			$booking_no_prefix = return_field_value("booking_no_prefix_num", "wo_non_ord_samp_booking_mst", "booking_no='" . $booking_no . "'");
			//$non_internal_ref= return_field_value("grouping", "wo_non_ord_samp_booking_mst", "booking_no='" . $booking_no . "'");
			$non_internal_ref = return_field_value("internal_ref", "wo_non_ord_samp_booking_dtls a,sample_development_mst b", " a.style_id=b.id and a.booking_no='" . $booking_nos . "'", "internal_ref");
			$bookingNo = $booking_no;
		}
	} else {
		$is_salesOrder = 0;
	...
	foreach ($res as $row) {
		...
		if (($recieve_basis == 1 && $booking_without_order=1) || ($recieve_basis == 2 && $booking_without_order=1))
		{
			$internal_ref=$non_internal_ref;
		}
		else
	...
	<script>
        ...
        function fn_barcode() {
        	...
        	if (dtls_id == "" && roll_maintain != 1) {
        		...
        		var page_link = 'grey_production_entry_controller.php?mstId='+mstId+'&update_dtls_id=' + dtls_id + '&roll_maintained=' + roll_maintain + '&action=barcode_defect_popup';
        		...
        		emailwindow.onclose = function () {
        			...
        			if (bar_code_ref[1] != "") {
        				//get_php_form_data(bar_code_ref[0], 'barcode_roll_find', 'grey_production_entry_controller');
        				if(mstId!="" && dtls_id!="" )
        				{
        					$.ajax({
        						url: 'grey_production_entry_controller.php',
        						type: 'POST',
        ...
        if (roll_maintain == 1) {
        	$('#txt_barcode').live('keydown', function (e) {
        		if (e.keyCode === 13) {
        			...
        			//get_php_form_data(bar_code, 'barcode_roll_find', 'grey_production_entry_controller');
        ...
        function fn_panelty_point(i) {
         ...
         {
         	window.open("grey_production_entry_controller.php?data=" + data+'&action='+action, true );
         }
         ...
         function fnc_grey_defect_entry(operation) {
         	...
         	if(operation == 4)
         	{
         		generate_report_file($('#update_id').val() ,
         			'KnittingProductionPrint', 'requires/grey_production_entry_controller');
         		return;
         	}
         	if(operation == 5)
         	{
         		generate_report_file($('#update_id').val() ,
         			'KnittingProductionPrint_2', 'requires/grey_production_entry_controller');
         		return;
         	}
            ...
            http.open("POST", "grey_production_entry_controller.php", true);
        ...
        function fnc_grey_defect_entry_response() {
        	if (http.readyState == 4) {
                ...
                if ((reponse[0] == 0 || reponse[0] == 1)) {
                	...
                	show_list_view(prod_dtls_id, 'show_qc_listview', 'dtls_list_container', 'grey_production_entry_controller', '');
											...
											<table cellspacing="0" cellpadding="0" border="1" rules="all" width="850" class="rpt_table">
												...
												<tbody>
													...
													foreach ($sql_dtls as $row) {
														...
														?>
														<tr bgcolor="<? echo $bgcolor; ?>"
															onClick='get_php_form_data(<? echo $row[csf("id")]; ?>, "populate_qc_from_grey_recv", "grey_production_entry_controller" );'
															style="cursor:pointer;">
															<td align="center"><? echo $i; ?></td>
        	...
        	<div>
        		...
        		<div>
        			...
        			if($results[0][csf("receive_basis")]==2){
        				$sql_feeder= sql_select("select feeder,width_dia_type from ppl_planning_info_entry_dtls where id=$data   and (status_active=1 and is_deleted=0 OR active_for_production=1 ) ");
        			}
...
if($action=="company_wise_load")
{
	...
	//grey_recvd_basis,grey_rate_come_from,finish_recvd_basis,finish_rate_come_from
	...
	foreach ($varialbe_production_sql as $row)
	{
		...
		{
			$process_costing_maintain = $row[csf('process_costing_maintain')];
			$var_rate_mandatory = $row[csf('auto_update')];
		}
		if ($row[csf('variable_list')] == 46)
		{
			//$grey_recv_basis_hdn = $row[csf('grey_recvd_basis')];
			$grey_rate_come_hdn = $row[csf('process_wise_rate_source')];
			//$finish_recv_basis_hdn = $row[csf('finish_recvd_basis')];
			//$finish_rate_come_hdn = $row[csf('finish_rate_come_from')];
		}
	}
	...
	if ($roll_maintained == 1) $roll_maintained = $roll_maintained; else $roll_maintained = 0;
	if ($fabric_store_auto_update == 1) $fabric_store_auto_update = 1; else $fabric_store_auto_update = 0;
	if ($production_control == 1) {$production_control = $production_control;} else {$production_control = 0;}
	if ($barcode_generation =="") {$barcode_generation=0;}
	if ($hidden_qc_result =="") {$hidden_qc_result=0;}
	if ($hidden_qc_result == 1)
	...
	if ($process_costing_maintain == 1)
	{
		...
		//echo "document.getElementById('var_grey_recv_basis_hdn').value='" . $grey_recv_basis_hdn . "';\n";
		echo "document.getElementById('var_grey_rate_come_hdn').value='" . $grey_rate_come_hdn . "';\n";
		//echo "document.getElementById('var_finish_recv_basis_hdn').value='" . $finish_recv_basis_hdn . "';\n";
	...
	echo "document.getElementById('var_rate_mandatory').value='" . $var_rate_mandatory . "';\n";
...
if($action=="load_tube_reference_load")
{
	$data_ref = explode("_", $data);
	$row_num = $data_ref[0];
	$program_no = $data_ref[1];
	$batch_no = $data_ref[2];

	echo create_drop_down("cboTubeRef_$row_num", 80, "select id, batch_no, reference_no, reference_qty from ppl_reference_creation where program_no=$program_no and batch_no='$batch_no' and status_active=1", "reference_no,reference_no", 1, "-- Select tube --", 0, "load_tube_value($row_num,$program_no,this.value);",'','','','','','','','cboTubeRef[]');
	exit();
}

if($action=="load_tube_value")
{
	$data_ref = explode("_", $data);
	$row_num = $data_ref[0];
	$program_no = $data_ref[1];
	$reference_no = $data_ref[2];


	$reference_qty = return_field_value("reference_qty", "ppl_reference_creation", "program_no=$program_no and reference_no='$reference_no' and status_active=1");
	echo "document.getElementById('txtTubeQnty_".$row_num."').value = '" . $reference_qty . "';\n";
	exit();
}
?>
