...
if ($action == "save_update_delete") {
	...
	} else if ($operation == 1) // Update Here End------------------------------------------------------
	{
		...
		$bundle = "'" . implode("','", $bundleCheckArr) . "'";
		$receive_sql = "select c.barcode_no,c.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls c where a.id=c.mst_id and a.production_type=3 and  a.embel_name=1 and c.bundle_no  in ($bundle)  and c.production_type=3 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.delivery_mst_id!=$mst_id and c.delivery_mst_id!=$mst_id and a.is_deleted=0 and (c.is_rescan=0 or c.is_rescan is null)";
		$receive_result = sql_select($receive_sql);
		foreach ($receive_result as $row) {
	...
	} else if ($operation == 2)  // Delete Here---------------------------------------------------------- 
	{
		...
		$rID = sql_delete("pro_garments_production_mst", "updated_by*update_date*status_active*is_deleted", "" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'*0*1", 'id ', $txt_system_id, 1);
		$dtlsrID = sql_delete("pro_garments_production_dtls", "status_active*is_deleted", "0*1", 'mst_id', $txt_system_id, 1);
		$mst_id = str_replace("'", "", $txt_system_id);
...
if ($action == "load_mst_data_bundle_scan") {
	...
	a.production_source, a.working_company_id, a.working_location_id, a.remarks, a.boe_mushak_challan_no, a.boe_mushak_challan_date
	...
	group by a.sys_number, a.company_id,a.location_id, a.embel_name, a.embel_type, a.serving_company,a.floor_id,a.organic,a.production_source, a.working_company_id,a.working_location_id, a.remarks, a.boe_mushak_challan_no, a.boe_mushak_challan_date");
