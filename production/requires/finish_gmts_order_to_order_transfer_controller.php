...
if($action=="color_and_size_level")
{
	...
	$exfactData = sql_select("SELECT sum(d.production_qnty) as ex_fact_qty, d.color_size_break_down_id from pro_ex_factory_mst c, pro_ex_factory_dtls d, pro_ex_factory_delivery_mst e where c.id=d.mst_id and e.id=c.delivery_mst_id and c.po_break_down_id=$po_id and c.item_number_id=$item_id and c.country_id=$country_id and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 group by d.color_size_break_down_id");
	...
	else if( $variableSettings==3 ) //color and size level
	{
		...
		sum(CASE WHEN b.production_type=8 then a.production_qnty ELSE 0 END) as production_qnty,
		sum(CASE WHEN b.production_type=10 and b.trans_type=5 then a.production_qnty ELSE 0 END) as trans_in_qty,
		sum(CASE WHEN b.production_type=10 and b.trans_type=6 then a.production_qnty ELSE 0 END) as trans_out_qty,
		sum(CASE WHEN b.production_type=14 then a.production_qnty ELSE 0 END) as del_qty,
		sum(CASE WHEN b.production_type=84 then a.production_qnty ELSE 0 END) as rcv_rtn_qty
		from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in(8,10,14,84) group by a.color_size_break_down_id");
...
if($action=="populate_transfer_form_data")
{
	...
	sum(CASE WHEN b.production_type=14 then a.production_qnty ELSE 0 END) as del_qty,
	sum(CASE WHEN b.production_type=84 then a.production_qnty ELSE 0 END) as rcv_rtn_qty
	from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.mst_id=b.id and b.production_type in (8,10,14,84) and b.po_break_down_id='$from_po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and b.production_type in(8,10,14,84) ");
	...
	foreach($sqlResult as $result)
	{
		...
		if($variableSettings!=1 )
		{
			...
			$exfactData = sql_select("SELECT sum(d.production_qnty) as ex_fact_qty, b.color_size_break_down_id from pro_ex_factory_mst c, pro_ex_factory_dtls d, pro_ex_factory_delivery_mst e where c.id=d.mst_id and e.id=c.delivery_mst_id and c.po_break_down_id=$po_id and c.item_number_id=$item_id and c.country_id=$country_id and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 group by b.color_size_break_down_id");
			$color_size_exfactData_array=array();
			foreach($exfactData as $row)
			{
				$color_size_exfactData_array[$row[csf('color_size_break_down_id')]]['exfact']= $row[csf('ex_fact_qty')];
			...
			else if( $variableSettings==3 ) //color and size level
			{
				$dtlsData = sql_select("SELECT a.color_size_break_down_id,
							...
							sum(CASE WHEN a.production_type=14 then a.production_qnty ELSE 0 END) as del_qty,
...
if($action=="show_dtls_listview")
{
	...
	$po_arr=return_library_array( "SELECT b.id, b.po_number from pro_garments_production_mst a, wo_po_break_down b where a.po_break_down_id=b.id and a.delivery_mst_id=$delivery_mst_id and a.production_type=10 group by b.id, b.po_number",'id','po_number');
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$field_array1="id, delivery_mst_id, dtls_id, garments_nature, company_id, challan_no, po_break_down_id, item_number_id, country_id, production_date, production_quantity, production_type, trans_type, entry_break_down_type, remarks, inserted_by, insert_date";

		$data_array1="(".$id.",".$mst_id.",".$delivery_dtlsId.",".str_replace("'","",$garments_nature).",".str_replace("'","",$cbo_company_id).",".$txt_challan_no.",".str_replace("'","",$txt_from_order_id).",".str_replace("'","",$cbo_from_gmts_item).",".str_replace("'","",$cbo_from_country_name).",".$txt_transfer_date.",".str_replace("'","",$txt_transfer_qty).",10,6,".str_replace("'","",$sewing_production_variable).",".$txt_remark.",".$user_id.",'".$pc_date_time."')";

		if($id_recv != "" && str_replace("'","",$sewing_production_variable)!=1)
		{
			$data_array1.=",(".$id_recv.",".$mst_id.",".$delivery_dtlsId.",".str_replace("'","",$garments_nature).",".str_replace("'","",$cbo_company_id).",".$txt_challan_no.",".str_replace("'","",$txt_to_order_id).",".str_replace("'","",$cbo_to_gmts_item).",".str_replace("'","",$cbo_to_country_name).",".$txt_transfer_date.",".str_replace("'","",$txt_transfer_qty).",10,5,".str_replace("'","",$sewing_production_variable).",".$txt_remark.",".$user_id.",'".$pc_date_time."')";
		}
		else
		{
			$id= return_next_id_by_sequence(  "pro_gar_production_mst_seq",  "pro_garments_production_mst", $con );
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		if($update_dtls_issue_id != ''){
			...
			$updateID_data[$update_dtls_issue_id]=explode("*",("".$txt_challan_no."*".$txt_from_order_id."*".$cbo_from_gmts_item."*".$cbo_from_country_name."*".$txt_transfer_date."*".$txt_transfer_qty."*".$txt_remark."*".$user_id."*'".$pc_date_time."'"));
		}
		if($update_dtls_recv_id != ''){
			$updateId_array[]=$update_dtls_recv_id;
			$updateID_data[$update_dtls_recv_id]=explode("*",("".$txt_challan_no."*".$txt_to_order_id."*".$cbo_to_gmts_item."*".$cbo_to_country_name."*".$txt_transfer_date."*".$txt_transfer_qty."*".$txt_remark."*".$user_id."*'".$pc_date_time."'"));
		}
