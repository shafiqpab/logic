...
{
	echo create_drop_down( "cbo_floor", 140, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process in (5) order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "get_php_form_data(document.getElementById('cbo_source').value,'line_disable_enable','requires/sewing_input_controller'); load_drop_down( 'requires/sewing_input_controller', this.value+'_'+document.getElementById('cbo_location').value+'_'+document.getElementById('prod_reso_allo').value+'_'+document.getElementById('txt_sewing_date').value+'_'+document.getElementById('cbo_sewing_company').value, 'load_drop_down_sewing_line_floor', 'sewing_line_td' );",0 );
}
...
if($action=="load_drop_down_sewing_input")
{
	...
	if($data==3)
	{
		...
		else
		{
  		...
  		echo create_drop_down( "cbo_sewing_company", 140, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1 $company_cond $company_credential_cond order by company_name","id,company_name", 1, "--- Select ---", "", "load_drop_down( 'requires/sewing_input_controller', this.value, 'load_drop_down_location', 'location_td' );fnc_company_check(document.getElementById('cbo_source').value);",0,0 );
 	else
...
if($action=="create_po_search_list_view")
{
	...
	$sql = "select b.id, a.order_uom, a.buyer_name, a.company_name, a.total_set_qnty, a.set_break_down, a.job_no, a.style_ref_no, a.gmts_item_id, a.location_name, b.shipment_date, b.po_number, b.po_number_acc, b.po_quantity, b.plan_cut, b.grouping, b.file_no
			...
			where
			a.job_no = b.job_no_mst and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $sql_cond $sql_shipment_year_cond  order by b.shipment_date desc"; 
     ...
     <div style="width:1170px;">
     	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="100%" class="rpt_table">
            <thead>
                ...
                <th width="80">Total Linking. Input Qty</th>
                <th>Balance</th>
            </thead>
     	</table>
     </div>
     <div style="width:1170px; max-height:240px;overflow-y:scroll;" >
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1152" class="rpt_table" id="tbl_po_list">
			<?
			$i=1;
					...
					foreach($country as $country_id)
					{
						...
						?>
							<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value(<? echo $row[csf("id")];?>,'<? echo $grmts_item;?>','<? echo $po_qnty;?>','<? echo $plan_cut_qnty;?>','<? echo $country_id;?>');" >
                                ...
                                <td width="80" align="right"><?php echo $total_cut_qty=$total_input_data_arr[$row[csf('id')]][$grmts_item][$country_id]; ?>&nbsp;</td>
