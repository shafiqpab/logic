...
if ($action=="load_variable_settings")
{
	...
	$sql_result = sql_select("select qty_source_poly, production_entry from variable_settings_production where company_name=$lcCompany and variable_list=1 and status_active=1");
 	foreach($sql_result as $result)
	{
		echo "$('#poly_production_variable').val(".$result[csf("qty_source_poly")].");\n";
	...
	$variable_qty_source_poly=return_field_value("preceding_page_id","variable_settings_production","company_name=$working_company and variable_list=33 and page_category_id=103","preceding_page_id");
...
if($action=="load_drop_down_poly_output_line")
{
	...
	if($prod_reso_allocation==1)
	{
		...
		echo create_drop_down( "cbo_poly_line", 110,$line_array,"", 1, "--- Select ---", $selected, "fn_generate_color_size_break_down(this.value)",0,0 );		
	}
	else
	{
		echo create_drop_down( "cbo_poly_line", 110, "select id, line_name from lib_sewing_line where is_deleted=0 and status_active=1 and location_name='$location' and location_name!=0 order by sewing_line_serial","id,line_name", 1, "Select Line", $selected, "fn_generate_color_size_break_down(this.value)" );
	}
	exit();
}
...
if($action=="load_drop_down_poly_line_floor")
{
	...
	if( $explode_data[0]==0 && $explode_data[1]!=0 ) $cond = " and location_name= $explode_data[1]";
		...
		echo create_drop_down( "cbo_poly_line", 110, "select id,line_name from lib_sewing_line where  is_deleted=0 and status_active=1 and floor_name!=0 and FLOOR_NAME=$floor_id and LOCATION_NAME=$location_id order by sewing_line_serial","id,line_name", 1, "--- Select ---", $selected, "fn_generate_color_size_break_down(this.value)",0,0 );
	exit();
}
...
if($action=="create_po_search_list_view")
{
	...
	if(trim($txt_search_by)==4 && trim($txt_search_common)!="")
	{
		$sql = "SELECT b.id, a.order_uom, a.buyer_name, a.company_name, a.total_set_qnty, a.set_break_down, a.job_no, a.style_ref_no, a.gmts_item_id, a.location_name, b.shipment_date, b.po_number,b.file_no,b.grouping, b.po_quantity, b.plan_cut
			...
			where a.id = b.job_id and b.id=c.po_break_down_id and b.shiping_status!=3 and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.garments_nature=$garments_nature
			$sql_cond $projected_po_cond group by b.id, a.order_uom, a.buyer_name, a.company_name, a.total_set_qnty, a.set_break_down, a.job_no, a.style_ref_no, a.gmts_item_id, a.location_name, b.shipment_date, b.po_number,b.file_no,b.grouping, b.po_quantity, b.plan_cut order by b.shipment_date DESC";
	}
	else
	{	
 		$sql = "SELECT b.id, a.order_uom, a.buyer_name, a.company_name, a.total_set_qnty, a.set_break_down, a.job_no, a.style_ref_no, a.gmts_item_id, a.location_name, b.shipment_date, b.po_number,b.file_no,b.grouping, b.po_quantity, b.plan_cut
			from wo_po_details_master a, wo_po_break_down b 
			where a.id = b.job_id and b.shiping_status!=3 and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0 and a.garments_nature=$garments_nature $sql_cond $projected_po_cond order by b.shipment_date DESC";
	}
	// echo $sql;die;
	...
	$country_sql=sql_select( "SELECT po_break_down_id, item_number_id, country_id, pack_type, sum(order_quantity) as po_qty, sum(plan_cut_qnty) as plan_cut_qty,color_number_id,country_ship_date from wo_po_color_size_breakdown where status_active in(1,2,3) and is_deleted=0 $po_id_cond  group by po_break_down_id, item_number_id, country_id, pack_type,color_number_id,country_ship_date"); 
	...
	$total_out_qty=sql_select( "SELECT a.po_break_down_id, a.item_number_id, a.country_id, a.pack_type, sum(b.production_qnty) as production_quantity,a.country_ship_date from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0  and a.production_type=11 and b.production_type=11 $po_id_cond group by a.po_break_down_id, a.item_number_id, a.country_id, a.pack_type,a.country_ship_date");
     ...
     <div style="width:1270px;">
     	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="100%" class="rpt_table">
            <thead>
                ...
                <th width="60">Total Input/W.Fin Qty</th>
                <th width="60">Total Poly Qty</th>
        ...
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1252" class="rpt_table" id="tbl_po_list">
			...
			foreach( $result as $key=>$row )
            {
				...
				foreach($po_country_data_arr[$row[csf('id')]] as $grmts_item=>$item_data)
				{
					...
					{
						foreach($country_data as $coun_ship_date=>$coun_ship_date_data)
						{
							foreach($coun_ship_date_data as $pack_type=>$val)
							{
								if ($i%2==0)  $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
								$total_in_qty=0; $total_cut_qty=0; $balance=0;
								$po_qnty=$val['po_qty'];
								$plan_cut_qnty=$val['plan_cut_qty'];
								// this condition off cause when last process prod qty controller variable is not set
								// if($total_in_qty_data_arr[$row[csf('id')]][$grmts_item][$country_id][$pack_type]!=0)
								// {
									unset($result[$key]);
									?>
									<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value(<? echo $row[csf("id")];?>,'<? echo $grmts_item;?>','<? echo $po_qnty;?>','<? echo $plan_cut_qnty;?>','<? echo $country_id;?>','<? echo $pack_type;?>','<?  echo $coun_ship_date  ?>');" > 
										<td width="30" align="center"><?php echo $i; ?></td>
										<td width="60"><p><?php echo change_date_format($coun_ship_date);?></p></td>
										<td width="100" title="<?php echo $color_name?$color_name:''?>" ><p><?php echo $row[csf("po_number")]; ?></p></td>
										...
										<td width="60" align="right"><?php $total_in_qty=$total_in_qty_data_arr[$row[csf('id')]][$grmts_item][$country_id][$coun_ship_date][$pack_type];echo $total_in_qty;  ?>&nbsp;</td>
										<td width="60" align="right"><?php $total_cut_qty=$total_out_qty_data_arr[$row[csf('id')]][$grmts_item][$country_id][$coun_ship_date][$pack_type]; echo $total_cut_qty; ?>&nbsp;</td>
									<td width="60" align="right"><?php $balance=$total_in_qty-$total_cut_qty; if($pack_type!="") echo ""; else if ($balance==0) echo ''; else echo $balance;?>&nbsp;</td>
...
if($action=="populate_data_from_search_popup")
{
	...
	$poly_line_cond = "";
	if($poly_line!="" && $poly_line!=0)
	{
		$poly_line_cond = " and a.sewing_line=$poly_line";
	}
  	...
  	foreach($res as $result)
	{
		...
		echo "$('#txt_internal_ref').val('".$result[csf('grouping')]."');\n";
   		...
   		if($qty_source!=0)
   		{
   			$dataArray=sql_select("SELECT SUM(CASE WHEN a.production_type='$qty_source' $poly_line_cond and b.production_type='$qty_source' THEN b.production_qnty END) as totalinput,SUM(CASE WHEN a.production_type=11 and b.production_type=11 $poly_line_cond THEN b.production_qnty ELSE 0 END) as totalpoly from pro_garments_production_mst a,pro_garments_production_dtls b WHERE a.id=b.mst_id and a.po_break_down_id=".$result[csf('id')]."  and a.item_number_id='$item_id' and a.country_id='$country_id' $pack_type_cond $country_ship_date_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
...
if($action=="color_and_size_level")
{
	...
	if($dataArr[6]==250)
	{
		if($poly_line!="")
		{
			$poly_line_cond = " and c.sewing_line=$poly_line";
			$poly_line_cond2 = " and b.sewing_line=$poly_line";
		}
	}
	...
	if( $pack_type=='') $pack_typeCond2=''; else $pack_typeCond2=" and c.pack_type='$pack_type'";
	
	if( $country_ship_date=='') $country_ship_date_cond=''; else $country_ship_date_cond=" and country_ship_date='$country_ship_date'";
	if( $country_ship_date=='') $country_ship_date_cond1=''; else $country_ship_dateCond=" and b.country_ship_date='$country_ship_date'";
	if( $country_ship_date=='') $country_ship_dateCond2=''; else $country_ship_dateCond2=" and c.country_ship_date='$country_ship_date'";
	if( $country_ship_date=='') $country_ship_dateCond3=''; else $country_ship_dateCond3=" and a.country_ship_date='$country_ship_date'";
	...
	else if($dataArr[6]==256) $qty_source=15;//Hang Tag Entry
	else if($dataArr[6]==250) $qty_source=80;//woven fin. entry
	$color_library=return_library_array( "select id, color_name from lib_color",'id','color_name');
	$size_library=return_library_array( "select id, size_name from lib_size",'id','size_name');
	// echo "string".$qty_source;die;
	if($qty_source !=0)
	{
		...
		if( $variableSettings==2 ) // color level
		{
				if($db_type==0)
				{
					...
					where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' and is_deleted=0 and status_active in(1,2,3) $pack_type_cond $country_ship_date_cond group by color_number_id";
				}
				else
				{
					$sql = "SELECT a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty,
							sum(CASE WHEN c.production_type='$qty_source' $poly_line_cond then b.production_qnty ELSE 0 END) as production_qnty,
							sum(CASE WHEN c.production_type=11 $poly_line_cond then b.production_qnty ELSE 0 END) as cur_production_qnty,
							sum(CASE WHEN c.production_type=11 $poly_line_cond then b.reject_qty ELSE 0 END) as reject_qty
							...
							where a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' and a.is_deleted=0 and a.status_active in(1,2,3)  $pack_type_cond $country_ship_date_cond3 group by a.item_number_id, a.color_number_id";	
		...
		else //if( $variableSettings==3) color and size level (if not lib set, defaulst this;)
		{
			...
			$dtlsData = sql_select("SELECT a.color_size_break_down_id,
									sum(CASE WHEN a.production_type='$qty_source' $poly_line_cond2 then a.production_qnty ELSE 0 END) as production_qnty,
									sum(CASE WHEN a.production_type=11 $poly_line_cond2 then a.production_qnty ELSE 0 END) as cur_production_qnty ,
									sum(CASE WHEN a.production_type=11 $poly_line_cond2 then a.reject_qty ELSE 0 END) as reject_qty
									from pro_garments_production_dtls a, pro_garments_production_mst b,wo_po_color_size_breakdown c where a.mst_id=b.id and a.color_size_break_down_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and  b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in($qty_source,11) $pack_typeCond2 $country_ship_date_cond2 group by a.color_size_break_down_id");
		...
		$sql = "SELECT id, item_number_id, size_number_id, color_number_id, order_quantity, plan_cut_qnty 
			from wo_po_color_size_breakdown
			where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' and is_deleted=0 and status_active in(1,2,3) $pack_type_cond $country_ship_date_cond order by color_number_id, size_order";
	...
	else // if preceding process =0 in variable setting then plan cut quantity will show
	{
		if( $variableSettings==2 ) // color level
		{
			...
			$sql = "SELECT id, item_number_id, size_number_id, color_number_id, order_quantity, sum(plan_cut_qnty) as plan_cut_qnty, (select sum(CASE WHEN pro_garments_production_dtls.color_size_break_down_id=wo_po_color_size_breakdown.id then production_qnty ELSE 0 END) from pro_garments_production_dtls where is_deleted=0 and production_type=11 ) as production_qnty 
				from wo_po_color_size_breakdown
				where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' $country_ship_date_cond and is_deleted=0 and status_active in(1,2,3) group by color_number_id";
			}
			else
			{
		...
		{				
			...
			$sql = "SELECT id, item_number_id, size_number_id, color_number_id, order_quantity, plan_cut_qnty 
			from wo_po_color_size_breakdown
			where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' $country_ship_date_cond and is_deleted=0 and status_active in(1,2,3) order by color_number_id,size_order"; //color_number_id, id
		...
		else // by default color and size level
		{
			...
			$dtlsData = sql_select("SELECT a.color_size_break_down_id,
										sum(CASE WHEN a.production_type=11 then a.production_qnty ELSE 0 END) as production_qnty
										from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' $country_ship_date_cond1 and a.color_size_break_down_id!=0 and a.production_type in(11) group by a.color_size_break_down_id");
		...
		<?php  
  		...
  		?>
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data('<? echo $selectResult[csf('id')].'**'.$variableSettings; ?>','populate_input_form_data','requires/poly_entry_controller');" > 
                ...
                <td width="90" align="center"><p><? echo $garments_item[$selectResult[csf('item_number_id')]]; ?></p></td>
                <td width="50" align="center"><p><? echo $country_library[$selectResult[csf('country_id')]]; ?></p></td>
                ...
                <td width="50" align="center"><p><? echo $floor_name_arr[$selectResult[csf('floor_id')]]; ?></p></td>
			</tr>
			<?php
			$i++;
			}
...
{
	list($data,$poly_company) = explode("**",$data);
	$control_and_preceding=sql_select("SELECT is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=33 and page_category_id=103 and company_name='$poly_company'"); 
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="400" class="rpt_table">
        <thead>
            ...
            <th width="60">Order Qty.</th>
            <th width="30">Sewing/W.Fin Qty</th>
            <th>Poly Qty.</th>                    
        </thead>
		<? 
		$issue_qnty_arr=sql_select("SELECT a.po_break_down_id, a.item_number_id, a.country_id, a.pack_type,b.production_type, b.production_qnty as cutting_qnty,a.country_ship_date from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id and a.po_break_down_id='$data' and b.production_type in($qty_source,11) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
		...
		$i=1;
		$sqlResult =sql_select("SELECT po_break_down_id, item_number_id, country_id, pack_type, country_ship_date, sum(order_quantity) as order_qnty, sum(plan_cut_qnty) as plan_cut_qnty from wo_po_color_size_breakdown where po_break_down_id='$data' and status_active in(1,2,3) and is_deleted=0 group by po_break_down_id, item_number_id, country_id, pack_type,country_ship_date order by country_ship_date Asc");
		foreach($sqlResult as $row)
		{
			if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
			$issue_qnty=$issue_data_arr[$row[csf("po_break_down_id")]][$row[csf("country_id")]][$row[csf("item_number_id")]][$row[csf("country_ship_date")]][$row[csf("pack_type")]][11];
			$sewing_out_qnty=$issue_data_arr[$row[csf("po_break_down_id")]][$row[csf("country_id")]][$row[csf("item_number_id")]][$row[csf("country_ship_date")]][$row[csf("pack_type")]][$qty_source];
		?>
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="put_country_data(<? echo $row[csf('po_break_down_id')].",".$row[csf('item_number_id')].",".$row[csf('country_id')].",".$row[csf('order_qnty')].",".$row[csf('plan_cut_qnty')].",'".$row[csf('pack_type')]."','".$row[csf('country_ship_date')]."'"; ?>);"> 
                ...
                <td align="right" width="60"><?php  echo $sewing_out_qnty; ?>&nbsp;</td>
...
if($action=="populate_input_form_data")
{
	...
	$company_id_sql=sql_select("SELECT  serving_company,sewing_line,po_break_down_id,item_number_id,country_id from  pro_garments_production_mst where  id='$id' and status_active=1 and is_deleted=0 ");
	$company_id=$company_id_sql[0][csf("serving_company")];
	$poly_line=$company_id_sql[0][csf("sewing_line")];
	...
	*/	
	...
	if($preceding_process==28) $qty_source=4; //Sewing Input
		...
		{ 
			...
			if($qty_source!=0)
			{
				$dataArray=sql_select("SELECT SUM(CASE WHEN a.production_type='$qty_source' and b.production_type='$qty_source' $poly_line_cond THEN  b.production_qnty END) as totalinput,SUM(CASE WHEN a.production_type=11 and b.production_type=11 $poly_line_cond THEN b.production_qnty ELSE 0 END) as totalpoly from pro_garments_production_mst a,pro_garments_production_dtls b WHERE a.id=b.mst_id and a.po_break_down_id=".$result[csf('po_break_down_id')]." and a.item_number_id=".$result[csf('item_number_id')]." and a.country_id=".$result[csf('country_id')]." $pack_typeCond $country_ship_date_cond and a.production_source=".$result[csf('production_source')]." and a.serving_company=".$result[csf('serving_company')]." and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
		  		foreach($dataArray as $row)
				{  
					echo "$('#txt_input_quantity').val('".$row[csf('totalinput')]."');\n";
					echo "$('#txt_cumul_poly_qty').val('".$row[csf('totalpoly')]."');\n";
			...
			{  
				$plan_cut_qnty=return_field_value("sum(plan_cut_qnty)","wo_po_color_size_breakdown","po_break_down_id=".$result[csf('po_break_down_id')]." and item_number_id=".$result[csf('item_number_id')]." and country_id=".$result[csf('country_id')]." $country_ship_date_cond and status_active in(1,2,3) and is_deleted=0");
				...
				{ 
					...
					if( $variableSettings==2 ) // color level
					{
						if($db_type==0)
						{
							...
							from wo_po_color_size_breakdown
							where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' and is_deleted=0 and status_active in(1,2,3) $pack_typeCond $country_ship_date_cond group by color_number_id";
						}
						else
						{
							$sql = "SELECT a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty,
									sum(CASE WHEN c.production_type='$qty_source' $poly_line_cond3 then b.production_qnty ELSE 0 END) as production_qnty,
									sum(CASE WHEN c.production_type=11 $poly_line_cond3 then b.production_qnty ELSE 0 END) as cur_production_qnty,
									sum(CASE WHEN c.production_type=11 $poly_line_cond3 then b.reject_qty ELSE 0 END) as reject_qty
									from wo_po_color_size_breakdown a 
									left join pro_garments_production_dtls b on a.id=b.color_size_break_down_id AND b.is_deleted = 0
									AND b.status_active=1
									left join pro_garments_production_mst c on c.id=b.mst_id AND c.is_deleted = 0
									AND c.status_active=1
									where a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' and a.is_deleted=0 and a.status_active in(1,2,3) $packTypecond $country_ship_date_cond2 group by a.item_number_id, a.color_number_id";	
						}
					}
					else if( $variableSettings==3 ) //color and size level
					{
						$dtlsData = sql_select("SELECT a.color_size_break_down_id,
												...
												sum(CASE WHEN a.production_type=11 $poly_line_cond2 then a.production_qnty ELSE 0 END) as cur_production_qnty,
												sum(CASE WHEN a.production_type=11 $poly_line_cond2 then a.reject_qty ELSE 0 END) as reject_qty
												from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in($qty_source,11) $pack_cond $country_ship_date_cond group by a.color_size_break_down_id ");
						...
						$sql = "SELECT id, item_number_id, size_number_id, color_number_id, order_quantity, plan_cut_qnty 
								from wo_po_color_size_breakdown
								where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' and is_deleted=0 and status_active in(1,2,3) $pack_typeCond $country_ship_date_cond order by color_number_id, size_order";

						// ============================== For Reject with defect type =================================
					    $sql_reject=sql_select("SELECT a.color_size_break_down_id,a.production_type,a.defect_point_id,a.defect_qty,a.bundle_no from  pro_gmts_prod_dft a where a.production_type=11 and a.defect_type_id=3 and a.po_break_down_id='$po_id' and a.mst_id=$id ");
						$reject_data_array=array();
						foreach($sql_reject as $inf)
						{
							if(in_array($inf[csf('color_size_break_down_id')],$check_arr_rej))
							{
							 	$reject_data_array[$inf[csf('color_size_break_down_id')]].="__".$inf[csf('defect_point_id')]."*".$inf[csf('defect_qty')];
							}
							else
							{
							 	$reject_data_array[$inf[csf('color_size_break_down_id')]]=$inf[csf('defect_point_id')]."*".$inf[csf('defect_qty')];
							}
							$check_arr_rej[]=$inf[csf('color_size_break_down_id')];
						}		
					}
					else // by default color and size level
					{
			...
			if($qty_source==0)
			{
				...
				{ 
					...
					$sql_dtls = sql_select("SELECT color_size_break_down_id, production_qnty, reject_qty, size_number_id, color_number_id from pro_garments_production_dtls a,wo_po_color_size_breakdown b where a.mst_id=$id and a.status_active=1 and a.color_size_break_down_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' $country_ship_date_cond");	
					...
					if( $variableSettings==2 ) // color level
					{
						if($db_type==0)
						{
							...
							from wo_po_color_size_breakdown
							where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' $country_ship_date_cond and is_deleted=0 group by color_number_id";
						}
						else
						{
					...
					else if( $variableSettings==3 ) //color and size level
					{
						...
						$dtlsData = sql_select("SELECT a.color_size_break_down_id,
												sum(CASE WHEN a.production_type=11 then a.production_qnty ELSE 0 END) as production_qnty,
												sum(CASE WHEN a.production_type=11 then a.reject_qty ELSE 0 END) as reject_qty
												from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id  and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' $country_ship_date_cond and a.color_size_break_down_id!=0 and a.production_type in(11) group by a.color_size_break_down_id");
						//and b.id='$data'
						...
						$sql = "SELECT id, item_number_id, size_number_id, color_number_id, order_quantity, plan_cut_qnty 
							from wo_po_color_size_breakdown
							where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' $country_ship_date_cond and is_deleted=0 and status_active in(1,2,3) order by color_number_id,size_order"; 
					...
					foreach($colorResult as $color)
					{
						...
						else //color and size level
						{
							...
							$colorHTML .='<tr><td>'.$size_library[$color[csf("size_number_id")]].'</td><td><input type="hidden" name="bundlemst" id="bundle_mst_'.$color[csf("color_number_id")].($i+1).'" value="'.$bundle_mst_data.'"  class="text_boxes_numeric" style="width:100px"  ><input type="hidden" name="bundledtls" id="bundle_dtls_'.$color[csf("color_number_id")].($i+1).'"  class="text_boxes_numeric" style="width:100px" value="'.$bundle_dtls_data.'" ><input type="text" name="colorSize" data-colorSizeBreakdown="'.$clr_sz_brk_id.'" id="colSize_'.$color[csf("color_number_id")].($i+1).'"  class="text_boxes_numeric" style="width:50px" placeholder="'.($color[csf("plan_cut_qnty")]-$cut_qnty+$amount).'" onkeyup="fn_total('.$color[csf("color_number_id")].','.($i+1).')" value="'.$amount.'" ><input type="text" name="colorSizeRej" data-colorSizeBreakdown="'.$clr_sz_brk_id.'" id="colSizeRej_'.$color[csf("color_number_id")].($i+1).'"  class="text_boxes_numeric" style="width:50px" placeholder="Rej. Qty" onkeyup="fn_total_rej('.$color[csf("color_number_id")].','.($i+1).')" onDblClick="pop_entry_reject('.$color[csf("color_number_id")].($i+1).')" value="'.$rej_qnty.'" '.$disable.'><input type="hidden" name="actual_reject" id="actual_reject_'.$color[csf("color_number_id")].($i+1).'"  value="'.$reject_data_array[$color[csf('id')]].'"  /></td><td><input type="button" name="button" value="Click For Bundle" class="formbutton" style="size:30px;" onclick="openmypage_bandle('.$color[csf("id")].','.$color[csf("color_number_id")].($i+1).','.$tmp_col_size.');" /></td></tr>';				
							//$colorWiseTotal += $amount;
							 $bundle_dtls_data="";
							 $bundle_dtls_data="";
						}
...
if ($action=="save_update_delete")
{
	...
	else if($preceding_process==250) $qty_source=80;//Hang Tag Entry
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if($is_control==1 && $user_level!=2)//Dont Hide ISD-23-00519
		{
			$txt_poly_qty=str_replace("'","",$txt_poly_qty);
			$country_poly_input_qty=return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=$hidden_po_break_down_id and production_type=5 and country_id=$cbo_country_name and status_active=1 and is_deleted=0");
			
			$country_poly_output_qty=return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=$hidden_po_break_down_id and production_type=11 and country_id=$cbo_country_name and status_active=1 and is_deleted=0");
		...
		$dtlsData = sql_select("SELECT a.color_size_break_down_id,
										...
										where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name and b.country_id=$cbo_country_name and a.color_size_break_down_id!=0 and a.production_type in($qty_source,11) $pack_type_cond $country_ship_date_cond
		...
		{		
			$color_sizeID_arr=sql_select( "select id,color_number_id from wo_po_color_size_breakdown where po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name and country_id=$cbo_country_name and status_active in(1,2,3) and is_deleted=0 $packType_cond $country_ship_date_cond order by id" );
			$colSizeID_arr=array(); 
		...
		{				
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				{ */
					...
					$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq", "pro_garments_production_dtls", $con );
					if($j==0)$data_array = "(".$dtls_id.",".$id.",11,'".$clr_sz_brk_id."','".$colorSizeValue."','".$rejQtyArr[$clr_sz_brk_id]."',".$cbo_color_type.")";
					else $data_array .= ",(".$dtls_id.",".$id.",11,'".$clr_sz_brk_id."','".$colorSizeValue."','".$rejQtyArr[$clr_sz_brk_id]."',".$cbo_color_type.")";
					//$dtls_id=$dtls_id+1;
					$j++;
				// }
			}

			// ================== reject with defect type =================
			$rejQtyArr = array();
			$rowExRej = array_filter(explode("***",$colorIDvalueRejWithDefect));
			foreach($rowExRej as $rowR=>$valR)
			{
				$colorAndSizeRej_arr = explode("**",$valR);
				// print_r($colorAndSizeRej_arr);
				$colorAndSizeRej_arr2 = explode("*",$colorAndSizeRej_arr[0]);
				$sizeID = $colorAndSizeRej_arr2[0];
				$colorID = $colorAndSizeRej_arr2[1];
				$index = $sizeID.$colorID;
				// $colorAndSizeRej_arr3 = explode("__",$colorAndSizeRej_arr[1]);

				$actual_reject_info = explode("__",$colorAndSizeRej_arr[1]);
				for( $rls=0; $rls<count($actual_reject_info); $rls++ )
				{
					...
					// $field_array_defectsp="id, mst_id, production_type, po_break_down_id,color_size_break_down_id, defect_type_id, defect_point_id, defect_qty, inserted_by, insert_date";
					$data_array_reject.="(".$dft_id.",".$id.",11,".$hidden_po_break_down_id.",".$colSizeID_arr[$index].",3,".$defectPointId.",'".$defect_qty."',".$user_id.",'".$pc_date_time."')";
					//$dft_id = $dft_id+1;
				}
			}	
			...
			{
				$field_array_defect="id, mst_id, production_type, po_break_down_id,color_size_break_down_id, defect_type_id, defect_point_id, defect_qty, inserted_by, insert_date";

				// echo "10**INSERT INTO pro_gmts_prod_dft (".$field_array_defect.") VALUES ".$data_array_reject.""; die;
				$defectSpot=sql_insert("pro_gmts_prod_dft",$field_array_defect,$data_array_reject,0);
			}
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		if($is_control==1 && $user_level!=2)//Dont Hide ISD-23-00519
		{
			...
			$country_poly_output_qty=return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=$hidden_po_break_down_id and production_type=11 and country_id=$cbo_country_name and status_active=1 and is_deleted=0 and id <> $txt_mst_id");
		...
		if(str_replace("'","",$poly_production_variable)!=1 && str_replace("'","",$txt_mst_id)!='' ) // check is not gross level
		{
			...
			$dtlsData = sql_select("SELECT a.color_size_break_down_id,
										...
										sum(CASE WHEN a.production_type=11 then a.production_qnty ELSE 0 END) as cur_production_qnty 
										from pro_garments_production_dtls a,pro_garments_production_mst b 
										where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name and b.country_id=$cbo_country_name and a.color_size_break_down_id!=0 and a.production_type in($qty_source,11) and b.id!=$txt_mst_id $pack_type_cond $country_ship_date_cond
										group by a.color_size_break_down_id");
			$color_pord_data=array();							
			foreach($dtlsData as $row)
			{				  
			...
			{		
				...
				foreach($rowEx as $rowE=>$valE)
				{
					...
					{ */
						//4 for poly Input Entry
						$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq", "pro_garments_production_dtls", $con );
						if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",11,'".$clr_sz_brk_id."','".$colorSizeValue."','".$rejQtyArr[$clr_sz_brk_id]."',".$cbo_color_type.")";
						else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",11,'".$clr_sz_brk_id."','".$colorSizeValue."','".$rejQtyArr[$clr_sz_brk_id]."',".$cbo_color_type.")";
						//$dtls_id=$dtls_id+1;
						$j++;
					// }
				}
				...
				foreach($rowExRej as $rowR=>$valR)
				{
					...
					for( $rls=0; $rls<count($actual_reject_info); $rls++ )
					{
						...
						$dft_id = return_next_id_by_sequence(  "pro_gmts_prod_dft_seq", "pro_gmts_prod_dft", $con );
						// $field_array_defectsp="id, mst_id, production_type, po_break_down_id,color_size_break_down_id, defect_type_id, defect_point_id, defect_qty, inserted_by, insert_date";
						$data_array_reject.="(".$dft_id.",".$txt_mst_id.",11,".$hidden_po_break_down_id.",".$colSizeID_arr[$index].",3,".$defectPointId.",'".$defect_qty."',".$user_id.",'".$pc_date_time."')";
						//$dft_id = $dft_id+1;
					}
				}

				$dftDelete = execute_query("delete from pro_gmts_prod_dft where mst_id=$txt_mst_id and defect_type_id=3",0);
				if($data_array_reject!="")
				{
					$field_array_defect="id, mst_id, production_type, po_break_down_id,color_size_break_down_id, defect_type_id, defect_point_id, defect_qty, inserted_by, insert_date";
	...
	else if ($operation==2)  // Delete Here---------------------------------------------------------- 
	{
		...
		$dftsrID = sql_delete("pro_gmts_prod_dft","status_active*is_deleted","0*1",'mst_id',$txt_mst_id,1);
...
if($action=="poly_output_print")
{
	...
	$job_array=array();
	$job_sql="SELECT a.job_no, a.buyer_name, a.style_ref_no, b.id, b.po_number from wo_po_details_master a, wo_po_break_down b where a.id=b.job_id and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0";
