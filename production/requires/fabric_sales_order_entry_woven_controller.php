...
if ($action == "load_drop_down_location") {
	echo create_drop_down("cbo_location_name", 162, "select id,location_name from lib_location where company_id='$data' and status_active =1 and is_deleted=0 order by location_name", "id,location_name", 0, "-- Select Location --", 1, "");
	exit();
}
...
if ($action == "load_drop_down_buyer") {
	...
	if ($company_id == 0) {
		...
		if ($data[0] == 1) {
			echo create_drop_down("cbo_buyer_name", 162, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond group by comp.id, comp.company_name order by comp.company_name", "id,company_name", 1, "-- Select Buyer --", "0", "", 1);
		} else if ($data[0] == 2) {
			echo create_drop_down("cbo_buyer_name", 162, "select buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$company_id' $buyer_cond and buy.id in (select buyer_id from lib_buyer_party_type where party_type in (1,3,21,90,80)) group by buy.id, buy.buyer_name order by buy.buyer_name", "id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down( 'requires/fabric_sales_order_entry_woven_controller', this.value, 'load_drop_down_season', 'season_td' )", 0);
		}
	}
	exit();
}
...
if ($action == "fabricDescription_popup")
{
	...
	<script>
		...
		function js_set_value(id, comp, gsm, color_range,fabric_dia,cuttable_width,weight_type) {
			$('#hidden_desc_id').val(id);
			$('#hidden_desc_no').val(comp);
			$('#hidden_gsm').val(gsm);
			$('#hidden_color_range').val(color_range);
			$('#hiddenFabricDia').val(fabric_dia);
			$('#hiddenGsmType').val(weight_type);
			$('#hiddenCuttableWidth').val(cuttable_width);
	...
	<body>
		<form name="searchdescfrm" id="searchdescfrm">
			<fieldset style="width:720px;margin-left:10px">
				...
				<input type="hidden" name="hiddenFabricDia" id="hiddenFabricDia" class="text_boxes" value="">
				<input type="hidden" name="hiddenGsmType" id="hiddenGsmType" class="text_boxes" value="">
				<input type="hidden" name="hiddenCuttableWidth" id="hiddenCuttableWidth" class="text_boxes" value="">
				...
				<div style="margin-left:10px; margin-top:10px">
					...
					<div style="width:700px; max-height:300px; overflow-y:scroll" id="list_container" align="left">
						<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="680" id="tbl_list_search">
							<?
							$i = 1;
							$data_array = sql_select("select id, construction, fab_nature_id, gsm_weight, color_range_id,full_width,cutable_width,weight_type from lib_yarn_count_determina_mst where fab_nature_id=3 and status_active=1 and is_deleted=0");
							foreach ($data_array as $row) {
								...
								<tr bgcolor="<? echo $bgcolor; ?>"
									onClick="js_set_value(<? echo $row[csf('id')]; ?>,'<? echo $comp; ?>','<? echo $row[csf('gsm_weight')]; ?>','<? echo $row[csf('color_range_id')]; ?>','<? echo $row[csf('full_width')]; ?>','<? echo $row[csf('cutable_width')]; ?>','<? echo $row[csf('weight_type')]; ?>')"
...
if ($action == "check_approvl_necessity_setup") {
	...
	if($booking_without_order==0)
	{
		...
		{
			$sql=sql_select("select approval_need,allow_partial from approval_setup_mst a,approval_setup_dtls b where a.id=b.mst_id and a.company_id=$company_id and b.page_id=5 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.setup_date desc");
		}
		else if($booking_type==1 && $is_short==1)
		{
			$sql=sql_select("select approval_need,allow_partial from approval_setup_mst a,approval_setup_dtls b where a.id=b.mst_id and a.company_id=$company_id and b.page_id=6 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.setup_date desc");
		}

		else if($booking_type==4)
		{
			$sql=sql_select("select approval_need,allow_partial from approval_setup_mst a,approval_setup_dtls b where a.id=b.mst_id and a.company_id=$company_id and b.page_id=7 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.setup_date desc");
		}
	...
	else
	{
		...
		$sql=sql_select("select approval_need,allow_partial from approval_setup_mst a,approval_setup_dtls b where a.id=b.mst_id and a.company_id=$company_id and b.page_id=8 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.setup_date desc");
...
if ($action == "fabricBooking_popup") {
	...
	<script>
		...
		function js_set_value(data, is_approved) 
		{
			...
			{
				var response=return_global_ajax_value( data, 'check_approvl_necessity_setup', '', 'fabric_sales_order_entry_woven_controller');
				if(response==1)
				{
					$('#hidden_booking_data').val(data);
					parent.emailwindow.hide();
				...
				<div id="content_search_panel">
					<table cellpadding="0" cellspacing="0" width="100%" class="rpt_table" border="1" rules="all">
						...
						<tr>
							...
							<td align="center">
								<?
								$search_by_arr = array(1 => "Booking No", 2 => "Job No", 3 => "Sample Booking With Order", 4 => "Sample Booking Without Order");
...
if ($action == "create_booking_search_list_view")
{
	...
	FROM wo_booking_mst a, wo_po_details_master b, wo_booking_dtls c
	WHERE a.booking_no=c.booking_no and c.job_no = b.job_no and a.pay_mode=5 and a.fabric_source in (1,2,4) and a.supplier_id=$company_id and a.entry_form in (86) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.item_category=3 $buyer_id_cond $search_field_cond $date_cond $year_cond $fabric_source_cond $restiction_date_cond
	...
	from wo_non_ord_samp_booking_mst a, wo_non_ord_samp_booking_dtls b 
	where a.booking_no=b.booking_no and a.pay_mode=5 and (a.fabric_source in (1,2,4)  or b.fabric_source in(1,2,4) ) and a.supplier_id=$company_id and a.status_active=1 and a.is_deleted =0 and a.item_category=3 $buyer_id_cond  $search_field_cond $date_cond $year_cond $fabric_source_cond2 $restiction_date_cond 
...
if ($action == 'show_fabric_details')
{
	...
	group by a.company_id, c.id,a.booking_type,a.is_short, c.lib_yarn_count_deter_id, c.body_part_id, c.color_type_id, c.gsm_weight_type, c.uom, c.gsm_weight, b.dia_width, b.fabric_color_id,b.pre_cost_remarks,b.process_loss_percent,c.item_number_id,a.entry_form
	union all
	select a.company_id, 0 as pre_cost_fabric_cost_dtls_id,a.booking_type,a.is_short, b.dia_width,null as adjust_qty, b.body_part as body_part_id, b.color_type_id,null as gsm_weight_type, b.gsm_weight,b.remarks as pre_cost_remarks, b.lib_yarn_count_deter_id, b.fabric_color as fabric_color_id, b.uom, (sum(b.bh_qty)+sum(b.rf_qty)) as rmg_qty, sum(b.grey_fabric) as qnty,sum(b.finish_fabric) as fqnty, sum(b.grey_fabric*b.rate) as amnt,sum(b.finish_fabric*b.rate) as partial_amnt,b.process_loss as process_loss_percent,b.gmts_item_id as item_number_id,b.entry_form_id as entry_form,b.color_all_data as color_all_data from wo_non_ord_samp_booking_mst a, wo_non_ord_samp_booking_dtls b where a.booking_no=b.booking_no $dia_width_cond and a.booking_no='$data' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.grey_fabric>0 group by a.company_id, b.id,a.booking_type,a.is_short, b.lib_yarn_count_deter_id, b.body_part, b.color_type_id,b.width_dia_type, b.uom, b.gsm_weight, b.dia, b.dia_width,b.fabric_color,b.remarks,b.process_loss, b.gmts_item_id,b.entry_form_id,b.color_all_data
	union all
	select a.company_id, 0 as pre_cost_fabric_cost_dtls_id,a.booking_type,a.is_short, b.dia as dia_width,null as adjust_qty, b.body_part as body_part_id, b.color_type_id, null AS gsm_weight_type, b.gsm_weight,b.remarks as pre_cost_remarks, b.lib_yarn_count_deter_id, b.fabric_color as fabric_color_id, b.uom, (sum(b.bh_qty)+sum(b.rf_qty)) as rmg_qty, sum(b.grey_fabric) as qnty,sum(b.finish_fabric) as fqnty, sum(b.grey_fabric*b.rate) as amnt,sum(b.finish_fabric*b.rate) as partial_amnt,b.process_loss as process_loss_percent,b.gmts_item_id as item_number_id,b.entry_form_id as entry_form,b.color_all_data as color_all_data from wo_non_ord_samp_booking_mst a, wo_non_ord_samp_booking_dtls b where a.booking_no=b.booking_no and a.entry_form_id=140 and b.entry_form_id=140 and a.booking_no='$data' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.grey_fabric>0 group by a.company_id, b.id,a.booking_type,a.is_short, b.lib_yarn_count_deter_id, b.body_part, b.color_type_id, b.uom, b.gsm_weight, b.dia, b.dia_width,b.fabric_color,b.remarks,b.process_loss, b.gmts_item_id,b.entry_form_id,b.color_all_data ";
	...
	if ($data_array[0][csf('entry_form')]==139) // Sample Requisition Fabric Booking -With order // For Dia
	{
		...
		group by a.company_id, c.id,a.booking_type,a.is_short, c.lib_yarn_count_deter_id, c.body_part_id, c.color_type_id, c.gsm_weight_type, c.uom, c.gsm_weight, b.dia, b.fabric_color_id,b.pre_cost_remarks,b.process_loss_percent,c.item_number_id,a.entry_form";
		$data_array = sql_select($sql);
	}
	...
	if (!empty($data_array)) 
	{
		...
		foreach ($data_array as $row) 
		{
			...
			<tr class="general" id="tr_<? echo $i; ?>">
				<td width="35" id="slTd_<? echo $i; ?>">
					<span><? echo $i; ?></span>
					<input type="hidden" name="txtSerial[]" id="txtSerial_<? echo $i; ?>" class="text_boxes" value="<? echo $i; ?>" readonly/>
				</td>
				<td width="90">
					<?
					echo create_drop_down("cboGarmItemId_" . $i, 70, $garments_item, "", 1, "- Select -", $row[csf('item_number_id')], "", "1", "", "", "", "", "", "", "cboGarmItemId[]");
					?>
				</td>
				<td width="90">
					<?
					echo create_drop_down("cboBodyPart_" . $i, 70, $body_part, "", 1, "- Select -", $row[csf('body_part_id')], "", "1", "", "", "", "", "", "", "cboBodyPart[]");
					?>
				</td>
				<td width="80">
					<?
					echo create_drop_down("cboColorType_" . $i, 60, $color_type, "", 1, "- Select -", $row[csf('color_type_id')], "", "1", "", "", "", "", "", "", "cboColorType[]");
					?>
				</td>
				<td width="152">
				...
				<td width="57">
					...
					style="width:35px" value="<? echo $row[csf('gsm_weight')]; ?>" title="<? echo $row[csf('gsm_weight')]; ?>" disabled="disabled"/>
				</td>
				<td width="90">
					<?
					echo create_drop_down("cboGsmType_".$i, 75, $fabric_weight_type, "", 1, "-- Select --", $row[csf('gsm_weight_type')], "", "1", "", "", "", "", "", "", "cboGsmType[]");
					?>
				</td>
				<td width="52">
					<input type="text" name="txtFabricDia[]" id="txtFabricDia_<? echo $i; ?>" class="text_boxes"
					style="width:35px" value="<? echo $row[csf('dia_width')]; ?>" title="<? echo $row[csf('dia_width')]; ?>" disabled="disabled"/>
				</td>
				<td width="52">
					<input type="text" name="txtCuttableWidth[]" id="txtCuttableWidth_<? echo $i; ?>" class="text_boxes"
					style="width:35px"
					onKeyUp="calculate_fin_qty(<? echo $i; ?>);copy_process_loss(<? echo $i; ?>);"
					disabled="disabled"/>
				</td>
				<td width="87">
					<input type="text" name="txtColor[]" id="txtColor_<? echo $i; ?>" class="text_boxes"
					style="width:70px" value="<? echo $color_library[$row[csf('fabric_color_id')]]; ?>" readonly
				...
				<td width="80">
					...
					style="width:60px" value="<? echo number_format($grey_qty, 4, '.', ''); ?>" title="<? echo number_format($grey_qty, 4, '.', ''); ?>" readonly/>
				</td>
				<td width="57">
...
if ($action == 'show_fabric_details_update') {
	...
	foreach ($data_array as $row) {
		...
		<tr class="general" id="tr_<? echo $i; ?>">
			...
			<td width="90">
				<?
				echo create_drop_down("cboGarmItemId_" . $i, 80, $garments_item, "", 1, "- Select -", $row[csf('item_number_id')], "", "", "", "", "", "", "", "", "cboGarmItemId[]");
				?>
			</td>
			<td width="90">
				<?
			...
			<td width="90">
				<?
				echo create_drop_down("cboGsmType_1", 75, $fabric_weight_type, "", 1, "-- Select --",  $row[csf('weight_type')], "", "1", "", "", "", "", "", "", "cboGsmType[]");
				?>
			</td>
			<td width="52">
				<input type="text" name="txtFabricDia[]" id="txtFabricDia_<? echo $i; ?>" class="text_boxes"
				style="width:35px" value="<? echo $row[csf('dia')]; ?>" title="<? echo $row[csf('dia')]; ?>" <? echo $disable; ?> />
			</td>
			<td width="52">
				<input type="text" name="txtCuttableWidth[]" id="txtCuttableWidth_1" class="text_boxes"
				style="width:35px"
				value="<? echo $row[csf('cutable_width')]; ?>"
			...
			<td width="87">
				...
				if ($cbo_within_group == 1) {
					?>
					<input type="text" name="txtColor[]" id="txtColor_<? echo $i; ?>" class="text_boxes"
					style="width:70px" value="<? echo $color_library[$row[csf('color_id')]]; ?>" title="<? echo $color_library[$row[csf('color_id')]]; ?>"
					...
					if ($color_from_library == 2) {
						...
						?>
						<input type="text" name="txtColor[]" id="txtColor_<? echo $i; ?>" class="text_boxes"
						style="width:70px" onDblClick="openmypage_color(<? echo $i; ?>)"
			...
			<td width="70">
				...
				style="width:50px" value="<? echo number_format($row[csf('grey_qnty_by_uom')], 4, '.', ''); ?>" title="<? echo number_format($row[csf('grey_qnty_by_uom')], 4, '.', ''); ?>" onKeyUp="calculate_amount(<?php echo $i;?>);calculate_fin_qty(<?php echo $i;?>);" <? echo $disable; ?> />
			</td>
			<td width="57">
				<input type="text" name="txtAvgRate[]" id="txtAvgRate_<? echo $i; ?>" class="text_boxes_numeric"
				style="width:40px" value="<? echo $row[csf('avg_rate')]; ?>" title="<? echo $row[csf('avg_rate')]; ?>"
			...
			<td width="52">
				<?
					if($textile_sales_maintain ==1 && $cbo_within_group ==1){
			...
			<td width="90">
				<?
				echo create_drop_down("cboWorkScope_" . $i, 80, $item_category, "", 1, "-- Select --", $row[csf('work_scope')], "", "0", "3,14", "", "", "", "", "", "cboWorkScope[]");
				?>
			</td>
			<td width="100">
...
if ($action == "save_update_delete") {
	...
	if ($operation == 0)  // Insert Here
	{
		...
		$new_sales_order_system_id = explode("*", return_next_id_by_sequence("FABRIC_SALES_ORDER_MST_PK_SEQ", "fabric_sales_order_mst",$con,1,$cbo_company_id,'FSOE',547,date("Y",time()),0 ));
		...
		for ($i = 1; $i <= $total_row; $i++)
		{
			...
			$cboGsmType = "cboGsmType" . $i;
			$txtCuttableWidth = "txtCuttableWidth" . $i;
			...
			if (str_replace("'", '', $color_from_library) == 2 && str_replace("'", '', $cbo_within_group) == 2) 
			{
				if(str_replace("'","",$$txt_color)!="")
				{
					...
					{
						$color_id = return_id( str_replace("'","",$$txt_color), $color_library, "lib_color", "id,color_name","547");
			...
			$barcode_suffix_no = explode("*", return_next_id_by_sequence("ROLL_BARCODE_SUFFIX_NO_SEQ", "fabric_sales_order_dtls",$con,1,0,'',547,date("Y",time()),13 ));
			$barcode_no = $barcode_year . "547" . str_pad($barcode_suffix_no[2], 6, "0", STR_PAD_LEFT);

			if ($cbo_within_group_combo == 1) 
			{
				$id_dtls = return_next_id_by_sequence("FABRIC_SALES_ORDER_DTLS_PK_SEQ", "fabric_sales_order_dtls", $con);

				$data_array_dtls .= "(" . $id_dtls . "," . $job_update_id . ",'" . $job_no . "','" . $$cboBodyPart . "','" . $$cboColorType . "','" . $$txtFabricDesc . "','" . $$fabricDescId . "','" . $$txtFabricGsm . "','" . $$txtFabricDia . "','" . $$cboGsmType. "','" . $$txtCuttableWidth . "','" . $color_id . "','" . $$cboColorRange . "','" . $$txtFinishQty . "','" . $$rmgQty . "','" . $$txtAvgRate . "','" . $$txtAmount . "','" . $$txtProcessLoss . "','" . $$txtGreyQty . "','" . $$cboWorkScope . "','" . $$cboUom . "'," . $$pre_cost_fabric_cost_dtls_id . "," . $$cboGarmItemId . ",'" . $$txtRemarks . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "'," . $$booking_qnty_by_uom . "," . $$cboConsUom . ",'" . $$txtProcessId . "','" . $$txtProcessSeq . "','". $$txtProcessIdMain . "','" . $$txtProcessSeqMain . "'," . $barcode_year . "," . $barcode_suffix_no[2] . "," . $barcode_no . ")";
			} 
			else 
			{
				$id_dtls = return_next_id_by_sequence("FABRIC_SALES_ORDER_DTLS_PK_SEQ", "fabric_sales_order_dtls", $con);
				$data_array_dtls .= "(" . $id_dtls . "," . $job_update_id . ",'" . $job_no . "','" . $$cboBodyPart . "','" . $$cboColorType . "','" . $$txtFabricDesc . "','" . $$fabricDescId . "','" . $$txtFabricGsm . "','" . $$txtFabricDia . "','" . $$cboGsmType . "','" . $$txtCuttableWidth. "','" . $color_id . "','" . $$cboColorRange . "','" . $$txtFinishQty . "','" . $$rmgQty . "','" . $$txtAvgRate . "','" . $$txtAmount . "','" . $$txtProcessLoss . "','" . $$txtGreyQty . "','" . $$cboWorkScope . "','" . $$cboUom . "','" . $$txtRemarks . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "'," . $$booking_qnty_by_uom . "," . $$cboConsUom . ",'" . $$txtProcessId . "','" . $$txtProcessSeq . "','". $$txtProcessIdMain . "','" . $$txtProcessSeqMain . "'," . $barcode_year . "," . $barcode_suffix_no[2] . "," . $barcode_no . ")";
			}
			//$id_dtls = $id_dtls + 1;
		...
		if ($db_type == 0) {
			...
			if ($rID && $rID2 && $rID4) {
				...
				oci_rollback($con);
				echo "5**0**0**$rID && $rID2 && $rID4**insert into fabric_sales_order_prcs_rate (".$field_array_process_wise_rate.") values ".$data_array_process_wise_rate;
			}
		}
	...
	else if ($operation == 1)   // Update Here
	{
		...
		foreach ($check_sales_dtls as $orows) {
			if($orows[csf("within_group")] == 1){
				$old_data[$orows[csf("pre_cost_fabric_cost_dtls_id")]][$orows[csf('item_number_id')]][$orows[csf('body_part_id')]][$orows[csf('gsm_weight')]][$orows[csf('dia')]][$orows[csf('color_type_id')]][$orows[csf('fabric_desc')]][$orows[csf('weight_type')]][$orows[csf('color_id')]][$orows[csf('pre_cost_remarks')]]['id'] = $orows[csf('id')];
				$oldids[$orows[csf('id')]] = $orows[csf('id')];
			}else{
				$old_data[$orows[csf('body_part_id')]][$orows[csf('gsm_weight')]][$orows[csf('dia')]][$orows[csf('color_type_id')]][$orows[csf('fabric_desc')]][$orows[csf('weight_type')]][$orows[csf('color_id')]]['id'] = $orows[csf('id')];

				$old_id_desc[$orows[csf('id')]] = $orows[csf('body_part_id')]."*".$orows[csf('color_type_id')]."*".$orows[csf('determination_id')]."*".$orows[csf('gsm_weight')]."*".$orows[csf('dia')]."*".$orows[csf('fabric_desc')]."*".$orows[csf('weight_type')]."*".$orows[csf('color_id')]."*".$orows[csf('color_range_id')]."*".$orows[csf('finish_qty')]."*".$orows[csf('rmg_qty')]."*".$orows[csf('avg_rate')]."*".$orows[csf('amount')]."*".$orows[csf('process_loss')]."*".$orows[csf('grey_qty')]."*".$orows[csf('work_scope')]."*".$orows[csf('order_uom')]."*".$orows[csf('pre_cost_remarks')];
			}
		}

		if ($cbo_within_group_combo == 1) {
			$field_array_dtls_update = "body_part_id*color_type_id*fabric_desc*determination_id*gsm_weight*dia*weight_type*cutable_width*color_id*color_range_id*finish_qty*rmg_qty*avg_rate*amount*process_loss*grey_qty*work_scope*order_uom*pre_cost_fabric_cost_dtls_id*item_number_id*pre_cost_remarks*updated_by*update_date*status_active*is_deleted*grey_qnty_by_uom*cons_uom*process_id*process_seq*process_id_main*process_seq_main";
		} else {
		...
		for ($i = 1; $i <= $total_row; $i++) 
		{
			...
			if ($cbo_within_group_combo == 1) 
			{
				if (str_replace("'", "", $$updateIdDtls) == "") 
				{
					...
					$data_array_dtls_update[str_replace("'", '', $$updateIdDtls)] = explode("*", ("'" . $$cboBodyPart . "'*'" . $$cboColorType . "'*'" . $$txtFabricDesc . "'*'" . $$fabricDescId . "'*'" . $$txtFabricGsm . "'*'" . $$txtFabricDia . "'*'" . $$cboGsmType. "'*'" . $$txtCuttableWidth . "'*'" . $color_id . "'*'" . $$cboColorRange . "'*'" . $$txtFinishQty . "'*'" . $$rmgQty . "'*'" . $$txtAvgRate . "'*'" . $$txtAmount . "'*'" . $$txtProcessLoss . "'*'" . $$txtGreyQty . "'*'" . $$cboWorkScope . "'*'" . $$cboUom . "'*" . $$pre_cost_fabric_cost_dtls_id . "*" . $$cboGarmItemId . "*'" . $$txtRemarks . "'*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'*1*0*" . $$booking_qnty_by_uom . "*" . $$cboConsUom. "*'" . $$txtProcessId. "'*'" . $$txtProcessSeq. "'*'" .$$txtProcessIdMain ."'*'" . $$txtProcessSeqMain . "'"));
					$rID2 = 1;
					//=================
				}
			} else {
				...
				if (str_replace("'", "", $$updateIdDtls) == "") {
					$id_dtls = return_next_id_by_sequence("FABRIC_SALES_ORDER_DTLS_PK_SEQ", "fabric_sales_order_dtls", $con);
					if ($data_array_dtls != "") $data_array_dtls .= ",";
					$data_array_dtls .= "(" . $id_dtls . "," . $job_update_id . ",'" . $job_no . "','" . $$cboBodyPart . "','" . $$cboColorType . "','" . $$txtFabricDesc . "','" . $$fabricDescId . "','" . $$txtFabricGsm . "','" . $$txtFabricDia . "','" . $$cboGsmType . "','" . $$txtCuttableWidth  . "','" . $color_id . "','" . $$cboColorRange . "','" . $$txtFinishQty . "','" . $$rmgQty . "','" . $$txtAvgRate . "','" . $$txtAmount . "','" . $$txtProcessLoss . "','" . $$txtGreyQty . "','" . $$cboWorkScope . "','" . $$cboUom . "','" . $$txtRemarks . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "',1,0," . $$booking_qnty_by_uom . "," . $$cboConsUom . ",'" . $$txtProcessId . "','" . $$txtProcessSeq . "','" . $$txtProcessIdMain . "','" . $$txtProcessSeqMain . "'," . $barcode_year . "," . $barcode_suffix_no[2] . "," . $barcode_no . ")";
					//$id_dtls = $id_dtls + 1;
					$dtlsId = $id_dtls;
					$rID2 = 1;
				} else {
					...
					$dtlsId = str_replace("'", '', $$updateIdDtls);
					$id_arr[] = str_replace("'", '', $$updateIdDtls);
					$data_array_dtls_update[str_replace("'", '', $$updateIdDtls)] = explode("*", ("'" . $$cboBodyPart . "'*'" . $$cboColorType . "'*'" . $$txtFabricDesc . "'*'" . $$fabricDescId . "'*'" . $$txtFabricGsm . "'*'" . $$txtFabricDia . "'*'" . $$cboGsmType . "'*'" . $$cboGsmType . "'*'" . $color_id . "'*'" . $$cboColorRange . "'*'" . $$txtFinishQty . "'*'" . $$rmgQty . "'*'" . $$txtAvgRate . "'*'" . $$txtAmount . "'*'" . $$txtProcessLoss . "'*'" . $$txtGreyQty . "'*'" . $$cboWorkScope . "'*'" . $$cboUom . "'*'" . $$txtRemarks . "'*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'*1*0*" . $$booking_qnty_by_uom . "*" . $$cboConsUom. "*'" . $$txtProcessId. "'*'" . $$txtProcessSeq . "'*'" . $$txtProcessIdMain . "'*'" . $$txtProcessSeqMain ."'"));
				}
			}
...
if ($action == 'populate_data_from_sales_order') {
	...
	foreach ($data_array as $row)
	{
		...
		//echo "load_drop_down( 'requires/fabric_sales_order_entry_woven_controller', ".$row[csf("within_group")]."+'_'+".$row[csf("company_id")].", 'load_drop_down_buyer','buyer_td');\n";
		...
		//echo "load_drop_down('requires/fabric_sales_order_entry_woven_controller', '".$row[csf("team_leader")]."', 'load_drop_down_dealing_merchant','team_td');\n";
		echo "load_drop_down('requires/fabric_sales_order_entry_woven_controller', " . $row[csf("within_group")] . "+'_'+" . $row[csf("team_leader")] . ", 'load_drop_down_dealing_merchant','team_td');\n";
		...
		echo "document.getElementById('cbo_order_status').value 			= '" . $row[csf("sales_order_status")] . "';\n";
...
if ($action == "yarnDetails_popup") {
	...
	<script>
		...
		{
			var page_link="fabric_sales_order_entry_woven_controller.php?action=composition_popup&inc="+inc;
...
if ($action == 'yarn_details') 
{
	...
	$saveDataArr = array();
	$sql_yarn = "SELECT deter_id, gsm, color_range_id, yarn_data,fabric_source,construction from fabric_sales_order_yarn where mst_id='$data' and status_active=1 and is_deleted=0 ";
	$saveFabricSource=array();
	$saveConstruction=array();
	...
	foreach ($data_array_yarn as $row) {
		...
		$saveFabricSource[$row[csf('deter_id')]][$row[csf('gsm')]][$row[csf('color_range_id')]] = $row[csf('fabric_source')];
		$saveConstruction[$row[csf('deter_id')]][$row[csf('gsm')]][$row[csf('color_range_id')]] = $row[csf('construction')];
	}
	...
	$data_array = sql_select($sql);
	$yarn_sql="SELECT deter_id, gsm, color_range_id, yarn_data,fabric_source,construction from fabric_sales_order_yarn where mst_id=$data and yarn_data is not null and status_active=1 and is_deleted=0";
	// echo $yarn_sql;
	...
	foreach ($data_array as $row) 
	{
		...
		$fabric_source_id=$saveFabricSource[$row[csf('determination_id')]][$row[csf('gsm_weight')]][$row[csf('color_range_id')]];
		$construction=$saveConstruction[$row[csf('determination_id')]][$row[csf('gsm_weight')]][$row[csf('color_range_id')]];
		if(empty($construction))
		{
			$construction = $construction_arr[$row[csf('determination_id')]];
		}
		...
		<tr bgcolor="<? echo $bgcolor;?>" id="tr_<? echo $i;?>">
			<td>
				<input type="text" name="txtConstructionY[]" id="txtConstructionY_<? echo $i; ?>" class="text_boxes"
				style="width:380px" value="<? echo $construction; ?>" 
				/>
            ...
            <td>	
	            <? 	
	            	echo create_drop_down( "cboFabricSource_". $i, 80, $fabric_source,"", 1, "-- Select --", $fabric_source_id,"", "", "", "","", "", "", "", "cboFabricSource[]");		
	            ?>
            </td>
			<td>
	...
	echo "##" . max(count($data_array_yarn),count($yarn_sql_data));
	exit();
}
...
if ($action == "save_update_delete_yarn") 
{
	...
	if ($operation == 0)  // Insert Here
	{
		...
		$field_array_dtls = "id, mst_id, deter_id, gsm, color_range_id,fabric_source, grey_qty,construction, yarn_data, inserted_by, insert_date";
		...
		for ($i = 1; $i <= $total_row; $i++) {
			...
			$yarnData = "yarnData" . $i;
			$cboFabricSource = "cboFabricSource" . $i;
			$txtConstructionY = "txtConstructionY" . $i;
			$id_dtls = return_next_id_by_sequence("FABRIC_SALES_ORDER_YARN_PK_SEQ", "fabric_sales_order_yarn", $con);
			if ($data_array_dtls != "") $data_array_dtls .= ",";
			$data_array_dtls .= "(" . $id_dtls . "," . $update_id . ",'" . $$fabricDescIdY . "','" . $$txtFabricGsmY . "','" . $$cboColorRangeY . "','" . $$cboFabricSource . "','" . str_replace(",", '', $$txtGreyQtyY). "','" . str_replace(",", '', $$txtConstructionY) . "','" . $$yarnData . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
			...
			if (str_replace("'", '', $$yarnData) != "") {
				...
				foreach ($yarnDatas as $value) {
					...
					if(str_replace("'","",$txtColor)!="")
					{
						...
						{
							$color_id = return_id( str_replace("'","",$txtColor), $color_library, "lib_color", "id,color_name","547");
							//echo $$txtColorName.'='.$color_id.'<br>';
		...
		if ($db_type == 0) {
			...
			if ($rID && $rID2 && $rID3) {
				...
				oci_rollback($con);
				echo "5**".str_replace("'", "", $update_id)."**".$rID ."**". $rID2 ."**". $rID3."**insert into fabric_sales_order_yarn (".$field_array_dtls.") values ".$data_array_dtls;
			}
		}
	...
	else if ($operation == 1)   // Update Here
	{
		...
		$field_array_dtls = "id, mst_id, deter_id, gsm, color_range_id,fabric_source, grey_qty,construction, yarn_data, inserted_by, insert_date,update_sl";
		...
		for ($i = 1; $i <= $total_row; $i++) {
			...
			$id_dtls = return_next_id_by_sequence("FABRIC_SALES_ORDER_YARN_PK_SEQ", "fabric_sales_order_yarn", $con);
			if ($data_array_dtls != "") $data_array_dtls .= ",";
			$data_array_dtls .= "(" . $id_dtls . "," . $update_id . ",'" . $$fabricDescIdY . "','" . $$txtFabricGsmY . "','" . $$cboColorRangeY. "','" . $$cboFabricSource . "','" . $$txtGreyQtyY . "','" . str_replace(",", '', $$txtConstructionY) . "','" . $$yarnData . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "',$update_serial)";
...
if ($action == "process_loss_method") {
	...
	$variable_textile_sales_maintain = sql_select("select production_entry from variable_settings_production where company_name=$data and variable_list=66 and status_active=1");
...
if ($action == 'fabric_sales_order_print') 
{
	...
	$entry_status_sql=sql_select("SELECT  id, inserted_by, insert_date, updated_by, update_date  
		from fabric_sales_order_mst where entry_form=547 and id=$sales_id and sales_booking_no ='$booking_no' order by  inserted_by");
	$user_lib_name=return_library_array("SELECT id,user_full_name from user_passwd", "id", "user_full_name");
	$user_lib_desg=return_library_array("SELECT id,designation from user_passwd", "id", "designation");
	$designation_lib=return_library_array("SELECT id,custom_designation from lib_designation", "id", "custom_designation");
...
if ($action == "fabric_sales_order_print2") 
{
	...
	where d.status_active =1 and d.is_deleted =0 and d.entry_form=547 and d.job_no='$salesOrderNo'";
	if ($sales_id == '') {
		$mst_data = sql_select($sql);
		$mst_data = array_change_key_case($mst_data[0], CASE_LOWER);
		extract($mst_data);
	...
	<table width="100%" border="1" cellpadding="3" rules="all" style="font: 12px tahoma;">
		...
		<br/>
		<?
		$dtls_sql = "SELECT b.cutable_width,b.weight_type,b.pre_cost_remarks,b.body_part_id, b.color_type_id,b.fabric_desc,b.gsm_weight, b.dia, b.color_id, sum(b.finish_qty) finish_qty, b.process_loss, b.work_scope, sum(b.grey_qty) grey_qty,sum(b.rmg_qty) as rmg_qty, b.pre_cost_fabric_cost_dtls_id,a.within_group,b.process_id
		from fabric_sales_order_mst a,fabric_sales_order_dtls b
		where a.id=$sales_id and a.job_no ='$salesOrderNo' and a.id=b.mst_id and a.status_active=1 and b.is_deleted=0
		group by b.cutable_width,b.body_part_id,b.color_type_id,b.fabric_desc,b.work_scope, b.gsm_weight,b.weight_type, b.dia, b.process_loss, b.color_id,b.pre_cost_remarks, b.pre_cost_fabric_cost_dtls_id,a.within_group,b.process_id order by b.body_part_id";
		//echo $dtls_sql;
		$dtls_sql_data = sql_select($dtls_sql);
		$process_count=0;
		foreach ($dtls_sql_data as $rows) {
			...
			$gsm_weight_arr[$key] = $rows[csf('gsm_weight')];
			$cuttable_arr[$key] = $rows[csf('cutable_width')];
			$dia_arr[$key] = $rows[csf('dia')] . ',' . $fabric_weight_type[$rows[csf('weight_type')]];
		...
		<table width="100%" border="1" cellpadding="3" rules="all" style="font: 12px tahoma;">
			...
			<tr>
				<td colspan="3"><strong>Full Width/Width (Inch)</strong></td>
				<? foreach ($dia_arr as $val) {
					echo '<td colspan="3" align="center">' . $val . '</td>';
				} ?>
			</tr>
			<tr>
				<td colspan="3"><strong>Cutable Width</strong></td>
				<? foreach ($cuttable_arr as $val) {
					echo '<td colspan="3" align="center">' . $val . '</td>';
				} ?>
			</tr>
			<tr>
...
if ($action == 'fabric_sales_order_print_kds2')  // Print 4 // KDS 2
{
	...
	if ($withinGroup==1) 
	{
		...
		WHERE a.id=d.booking_id and a.job_no=b.job_no and b.job_no=c.job_no_mst and a.pay_mode=5 and d.entry_form=547 and a.fabric_source in(1,2) and a.supplier_id=$companyId and a.status_active =1 and a.is_deleted =0 and a.item_category=2 and a.booking_no='$bookingNo' 
		...
		where a.booking_no='$bookingNo' and c.entry_form=547 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.grey_fabric>0 
	...
	<?
	$dtls_sql = "SELECT b.weight_type,b.cutable_width,b.pre_cost_remarks,b.body_part_id, b.color_type_id,b.fabric_desc,b.gsm_weight, b.dia, b.color_id, sum(b.finish_qty) finish_qty, b.process_loss, b.grey_qty,sum(b.rmg_qty) as rmg_qty, c.item_number_id,b.pre_cost_fabric_cost_dtls_id,b.process_id, b.process_id_main
	from fabric_sales_order_mst a,fabric_sales_order_dtls b
	left join wo_pre_cost_fabric_cost_dtls c on b.pre_cost_fabric_cost_dtls_id=c.id
	where a.id=$sales_id and a.sales_booking_no='$bookingNo' and a.id=b.mst_id and a.status_active=1 and b.is_deleted=0 
	group by b.cutable_width,b.body_part_id,b.color_type_id,b.fabric_desc, b.gsm_weight,b.weight_type, b.dia, b.process_loss, b.grey_qty,b.color_id,b.pre_cost_remarks, c.item_number_id, b.pre_cost_fabric_cost_dtls_id,b.process_id, b.process_id_main 
	...
	foreach ($dtls_sql_data as $rows)
	{
		...
		$gsm_weight_arr[$key] = $rows[csf('gsm_weight')];
		$cutable_width_arr[$key] = $rows[csf('cutable_width')];
		$dia_arr[$key] = $rows[csf('dia')] . ',' . $fabric_weight_type[$rows[csf('weight_type')]];
		$remarks_arr[$key] = $rows[csf('pre_cost_remarks')];
		$rmg_qty_arr[$key] += $rows[csf('rmg_qty')];
		$fin_qty_arr[$key] += $rows[csf('finish_qty')];
	...
	<table width="100%" border="1" cellpadding="3" rules="all" style="font: 12px tahoma;">
		...
		<tr>
			<td colspan="3"><strong>Cutable Width</strong></td>
			<? foreach ($cutable_width_arr as $val) {
				echo '<td colspan="3" align="center">' . $val . '</td>';
			} ?>
		</tr>
...
if ($action == "fabric_sales_order_print5") 
{
	...
	$sql = "select d.id as sales_id,d.currency_id,d.delivery_date,d.within_group,d.job_no, d.buyer_id,d.booking_date,
	d.sales_booking_no,d.company_id,d.delivery_date,d.dealing_marchant,d.remarks,d.style_ref_no,d.season,d.attention,d.fabric_composition from fabric_sales_order_mst d
	where d.status_active =1 and d.entry_form=547 and d.is_deleted =0 and d.job_no='$salesOrderNo'";
	if ($sales_id == '') {
		$mst_data = sql_select($sql);
		$mst_data = array_change_key_case($mst_data[0], CASE_LOWER);
		extract($mst_data);
...
if ($action == "terms_condition_popup")
{
	...
	<script>
		...
		function fnc_sales_order_terms_condition(operation) {
			...
			http.open("POST", "fabric_sales_order_entry_woven_controller.php", true);
...
if ($action == "process_name_popup_main___________________bk_______________") 
{
	...
	<script>
		...
		function openmypage_subProcess(incr_id) // Sub process popup
		{
			...
			var page_link = 'fabric_sales_order_entry_woven_controller.php?txt_process_id='+txt_process_id+'&process_seq='+txt_process_seq+'&action=sub_process_name_popup';
...
if ($action == "process_name_popup_main") 
{
	...
	<script>
		...
		function fnc_get_charge(main_process)  
		{
			...
			if(main_process == 1)
			{
				get_php_form_data(main_process+'_'+cbo_within_group+'_'+cbo_buyer_name+'_'+cbo_fabric_type+'_'+cbo_count_range_from+'_'+cbo_count_range_to, "populate_finish_change", "fabric_sales_order_entry_woven_controller");
			}
			else if(main_process == 30)
			{
				get_php_form_data(main_process+'_'+cbo_within_group+'_'+cbo_buyer_name+'_'+cbo_yarn_dyeing_part+'_'+cbo_yarn_color_range, "populate_finish_change", "fabric_sales_order_entry_woven_controller");
			}
			else if(main_process == 31)
			{
				get_php_form_data(main_process+'_'+cbo_within_group+'_'+cbo_buyer_name+'_'+cbo_color_range+'_'+cbo_dyeing_part+'_'+cbo_diawidthtype+'_'+cbo_dyeing_upto, "populate_finish_change", "fabric_sales_order_entry_woven_controller");
			}
			else if(main_process == 35)
			{
				get_php_form_data(main_process+'_'+cbo_within_group+'_'+cbo_buyer_name+'_'+cbo_aop_type+'_'+cbo_no_color+'_'+txt_coverage_from+'_'+txt_coverage_to+'_'+cbo_aop_upto, "populate_finish_change", "fabric_sales_order_entry_woven_controller");
			}
			else if(main_process == 1000)
			{
				get_php_form_data(main_process+'_'+cbo_within_group+'_'+cbo_buyer_name+'_'+cbo_addi_process, "populate_finish_change", "fabric_sales_order_entry_woven_controller");
			}
			...
			<table cellspacing="0" cellpadding="0" border="0" rules="all" width="500" >
				<legend>Process wise Rate Entry</legend>
					<tr style="font-weight:bold; background-color:lightsteelblue; line-height: 1.8; ">
						<td width="100">Woven Parameter</td>
						<td width="150">&nbsp;</td>
						<td width="100">Woven Charge</td>
					...
					<tr>
						...
						<td>
							...
							echo create_drop_down("cbo_dyeing_part", 130, $blank_array, "", 1, "-- Select --", $DYEING_PART, "fnc_get_charge(31)");
							?>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>
							<?
							echo create_drop_down( "cbo_aop_type", 130,  $print_type,"", "", "-- Select --", $AOP_TYPE, "fnc_get_charge(35)","","" ); 
							?>
						</td>
						<td>No of Color</td>
						<td>
					...
					<tr>
						...
						<td>
							<?
								echo create_drop_down("cbo_yarn_dyeing_part", 130, $blank_array, "", 1, "-- Select --", $YARN_DYEING_PART, "fnc_get_charge(30)");
							?>
						</td>
						<td>Color Range</td>
						<td>
					...
					<tr>
						...
						<td>
							<? 
								echo create_drop_down("cbo_addi_process", 130, $blank_array, "", 1, "-- Select --", $ADDITIONAL_PROCESS, "fnc_get_charge(1000)");
							?>
						</td>
						<td></td>
						<td></td>
