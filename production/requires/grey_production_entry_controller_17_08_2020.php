...
{
	echo create_drop_down("cbo_location_name", 152, "select id,location_name from lib_location where company_id='$data' and status_active =1 and is_deleted=0 order by location_name", "id,location_name", 1, "-- Select Location --", 0, "load_floor();store_load(this.value);");
	exit();
}
...
{
	echo create_drop_down("cbo_com_location_name", 152, "select id,location_name from lib_location where company_id='$data' and status_active =1 and is_deleted=0 order by location_name", "id,location_name", 1, "-- Select Location --", 0, "store_load(this.value);");
	exit();
}
...
if ($action == "load_drop_down_store")
{
	...
	echo create_drop_down("cbo_store_name", 152, "select a.id, a.store_name from lib_store_location a, lib_store_location_category b where a.id= b.store_location_id and a.company_id='$data_ref[0]' and a.location_id='$data_ref[1]' and b.category_type=13 and a.status_active=1 and a.is_deleted=0 group by a.id, a.store_name order by a.store_name", "id,store_name", 1, "--Select Store--", 0, "", '');
	exit();
}
...
if ($action == "load_drop_down_floor")
{
	...
	if ($action == "load_drop_machine")
	{
		...
		echo create_drop_down("cbo_machine_name", 132, "select id, machine_no as machine_name from lib_machine_name where category_id=1 and company_id=$company_id and status_active=1 and is_deleted=0 and is_locked=0 and is_subcon =2 $floor_cond order by seq_no", "id,machine_name", 1, "-- Select Machine --", 0, "", "");
		exit();
	}
					...
					<table cellpadding="0" cellspacing="0" border="1" rules="all" width="720" class="rpt_table">
						...
						<tr>
							...
							<td align="center">
								...
								onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_receive_basis').value+'_'+document.getElementById('txt_company_id').value+'_<? echo $cbo_knitting_source; ?>_'+<? echo $cbo_knitting_company; ?>, 'create_wo_no_production_search_list_view', 'search_div', 'grey_production_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1);')"
								style="width:100px;"/>
							</td>
						</tr>
					</table>
...
if ($action == "create_booking_search_list_view")
{
	...
	else if ($recieve_basis == 4)
	{
		...
		$sql = "select id, $year_field, job_no_prefix_num, job_no, within_group, sales_booking_no, booking_date, delivery_date, buyer_id, style_ref_no, location_id from fabric_sales_order_mst where status_active=1 and is_deleted=0 and company_id=$company_id $search_field_cond $year_cond order by id";
			//echo $sql;
		$result = sql_select($sql);
		?>
...
if ($action == 'populate_data_from_booking2')
{
	...
	//$orderID=implode(",",array_unique(explode(",", $orderIDs)));
...
if ($action == "po_popup")
{
	...
	if ($production_control == 1)
	{
		...
		if($receive_basis==2)
		{
			...
			{
				//$yarn_issue_qnty[$val[csf('po_breakdown_id')]] += $val[csf("issue_qnty")];
			...
			if (roll_maintained == 1) 
			{
				...
				for (var j = 1; j <= row_num; j++) {
					...
					if (po_id == po_id_check) {
						...
						greyQtyPcs = greyQty * 1 + $('#txtGreyQntyPcs_' + j).val() * 1;
					}
				}
			}
			else {
...
<body>
	...
	else
	{
		...
		if ($booking_without_order == 1 && $receive_basis != 2)
		{
					...
					if ($save_data == "")
					{
						...
						<tr bgcolor="#FFFFFF" id="tr_1">
							<td width="130">
								...
								<input type="hidden" name="balanceQty[]" id="balanceQty_1" value="<? echo number_format($production_balance, 2, '.', '');//echo $balanceQty; ?>">
							</td>
						...
						for ($z = 0; $z < count($explSaveData); $z++) 
						{
							...
							<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
								<td width="130">
									...
									<!--This is used for Duplication Check -->
									<input type="hidden" name="balanceQty[]" id="balanceQty_<? echo $i; ?>" value="<? echo number_format($req_qnty-$grey_receive_qnty_arr[$bookingData[0][csf('po_ids')]]["prod_quantity"], 2, '.', ''); ?>">
								</td>
								<td width="80" align="right"><? echo number_format($req_qnty, 2, '.', ''); ?></td>
							...
							else if ($save_data != "" && $receive_basis == 2)
							{
								...
								else
								{
									...
									foreach ($po_data as $row) {
										...
										$production_over_balance = ($production_balance*$over_production_percentage/100) + $production_balance;
								...
								if ($receive_basis == 2)
								{
									...
									foreach ($po_data as $row)
									{
										...
										<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
											...
											<td width="80" align="center">
												<input type="text" name="txtGreyQnty[]" id="txtGreyQnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px" value="" <? echo $disable; ?> placeholder="<? if ($roll_maintained == 0) echo number_format($production_balance, 2, '.', ''); ?>" onKeyUp="fn_check_balance(<? echo $i; ?>);calculate_tot_qnty();fn_check_max_roll_weight('txtGreyQnty_<? echo $i; ?>');">
											</td>
											<td width="80" align="center">
		...
		if ($action == "save_update_delete")
		{
			...
			if ($operation == 0)  // Insert Here
			{
				...
				if (str_replace("'", "", $roll_maintained) == 1) {
					/*if(str_replace("'","",$cbo_receive_basis)==4)
					{
						$roll_arr=return_library_array("select po_breakdown_id,max(roll_no) as roll_no from pro_roll_details where entry_form in(2,22,62) and booking_without_order=$booking_without_order and booking_no=$txt_booking_no and receive_basis=4 group by po_breakdown_id",'po_breakdown_id','roll_no');
					}
					else
					{
						if(str_replace("'","",$cbo_receive_basis)==2)
						...
						if($is_salesOrder!=1) $is_salesOrder=0;

						$roll_arr=return_library_array("select po_breakdown_id,max(roll_no) as roll_no from pro_roll_details where entry_form in(2,22,62) and booking_without_order=$booking_without_order and receive_basis!=4 and is_sales=$is_salesOrder group by po_breakdown_id",'po_breakdown_id','roll_no');
					}*/
...
if(str_replace("'","",$cbo_receive_basis)==2)
{
	$rID8 = execute_query("update ppl_planning_info_entry_dtls set is_posted_sql=2 where id = $txt_booking_no_id");
	if ($flag == 1) {
		if ($rID8) $flag = 1; else $flag = 0;
	}
}

// echo "10**".$rID7."**".$rID6."**".$rID5."**".$rID4."**".$rID3."**".$rID2."**".$rID;die;
if ($db_type == 0) {
	if ($flag == 1) {
		mysql_query("COMMIT");
	...
	/*
		if ($receive_basis == 1) {
			if ($booking_without_order == 1) {
				$sql_prev_production = "select sum(b.grey_receive_qnty) as grey_receive_qnty from inv_receive_master a,  pro_grey_prod_entry_dtls b where a.id=b.mst_id and a.booking_without_order=1 and booking_id=" . $booking_no_id . " and a.receive_basis=1 and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
				$bookingData = sql_select("select sum(b.grey_fabric) as qnty from wo_non_ord_samp_booking_mst a, wo_non_ord_samp_booking_dtls b
					where a.booking_no=b.booking_no and a.booking_type in(1,4) and a.id=" . $booking_no_id . " and b.fabric_source=1 and a.status_active=1  and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
			} else {
				$sql_prev_production = "select sum(b.grey_receive_qnty) as grey_receive_qnty from inv_receive_master a,  pro_grey_prod_entry_dtls b where a.id=b.mst_id and a.booking_without_order=0 and booking_id=" . $booking_no_id . " and a.receive_basis=1 and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 ";
				$bookingData = sql_select("select sum(b.grey_fab_qnty) as qnty from  wo_booking_mst a, wo_booking_dtls b
					where a.booking_no=b.booking_no and a.booking_type=1 and a.id=" . $booking_no_id . " and a.status_active=1  and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
			}
			$prev_production_data = sql_select($sql_prev_production);
			$previous_grey_receive_qnty = $prev_production_data[0][csf("grey_receive_qnty")];
			$required_qnty = $bookingData[0][csf("qnty")];
		} else if ($receive_basis == 2) //Knitting Plan
		{
			$sql_prev_production = "select sum(b.grey_receive_qnty) as grey_receive_qnty from inv_receive_master a,  pro_grey_prod_entry_dtls b where a.id=b.mst_id and booking_id=" . $booking_no_id . " and a.receive_basis=2 and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
			$prev_production_data = sql_select($sql_prev_production);
			//echo "document.getElementById('previous_book_plan_production').value = '".$prev_production_data[0][csf("grey_receive_qnty")]."';\n";

			$program_qnty_array = sql_select("select sum(a.program_qnty) as qnty from ppl_planning_info_entry_dtls a where a.id=" . $booking_no_id . " and a.status_active=1 and a.is_deleted=0");
			$previous_grey_receive_qnty = $prev_production_data[0][csf("grey_receive_qnty")];
			$required_qnty = $program_qnty_array[0][csf("qnty")];
		}

		$tot_prod = $previous_grey_receive_qnty - ($previous_receive_qnty + $receive_qnty);
		$rr = "";
		if ($tot_prod > $required_qnty) {
			//echo "31**Production Quantity Exceeded Required Quantity";
			$rr = 31;
		}
	*/
	...
	if (str_replace("'", "", $roll_maintained) == 1)
	{
		...
		for ($i = 0; $i < count($save_string); $i++) {
			...
			else
			{
				...
				{
					echo "30** $barcode_no_exists is already deleted by another user. Please reload the System ID again.";die;
				}
		...
		if($txt_deleted_id != "")
		{
			...
			{
				echo "30**Barcode already delivered to store.\nDelivery challan no ".$delivery_roll_chk_sql[0][csf("sys_number")];die;
			}
		}

		if(count(array_filter($database_roll_info_arr)) >0)
		{
			echo "30**Another user already changes some information of this System ID. Please reload the System ID and try again.";die;
		}
	}
//echo "10**Fail";die;
...
if (str_replace("'", "", $roll_maintained) == 1) {
	...
	{
		$roll_arr=return_library_array("select po_breakdown_id,max(roll_no) as roll_no from pro_roll_details where entry_form in(2,22,62) and booking_without_order=$booking_without_order and receive_basis!=4 group by po_breakdown_id",'po_breakdown_id','roll_no');
	}*/
...
if(str_replace("'","",$cbo_receive_basis)==2)
{
	$rID9 = execute_query("update ppl_planning_info_entry_dtls set is_posted_sql=2 where id = $txt_booking_no_id");
	if ($flag == 1) {
	...
	<div style="width:930px; max-height:200px; overflow-y:scroll" id="list_container_batch" align="left">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="910" class="rpt_table" id="list_view">
			...
			foreach ($result as $row) {
				...
				<td width="70"><p><? echo $shift_name[$row[csf('shift_name')]]; ?></p></td>
...
if ($action == "roll_maintained") {
	...
	$varialbe_production_sql = sql_select("select variable_list, distribute_qnty, smv_source, auto_update, fabric_roll_level, item_category_id from variable_settings_production where company_name ='$data' and variable_list in(3,15,27,51) and is_deleted=0 and status_active=1");
...
if ($action == "yarn_lot_popup") {
	...
	<script>
		...
		function fnc_process_cost() {
			...
			var tempCountId = '';
			var yarn_count_arr = new Array(); var i=0;
		...
		*/
    	...
    	if($txt_service_booking!=""){
			...
			if ($is_salesOrder == 1) {
				...
				$conversion_cost = sql_select("select a.id,b.rate as charge_unit from wo_non_ord_samp_booking_mst a, wo_non_ord_samp_booking_dtls b , ppl_planning_info_entry_mst c, ppl_planning_info_entry_dtls d
where a.booking_no = b.booking_no and b.booking_no = c.booking_no and c.id = d.mst_id and d.id=$booking_id and b.lib_yarn_count_deter_id=" . $fabric_description_id . " and a.status_active=1 and b.status_active=1");
			} else{
    	...
    	<table cellspacing="0" cellpadding="0" border="0" rules="all" width="840" class="" align="center">
    		<tr>
    			...
    			<td colspan="5" align="center" style="font-size:16px"><strong>Knitting Charge:<?php echo number_format($conversion_cost[0][csf('charge_unit')] * $precost_exchange_rate, 2) . "Tk./Kg"; ?></strong>
    			</td>
    		</tr>
    	</table>
		...
		<td align="center" height="30" valign="bottom">
			<div style="width:100%">
				<div style="width:100%; float:left" align="center">
					<input type="hidden" name="txt_knitting_charge" id="txt_knitting_charge"
					value="<?php echo $conversion_cost[0][csf('charge_unit')]; ?>"/>
...
//quality_level
	...
	if ($booking_without_order == 1) {
		...
		if ($is_salesOrder == 1) {
			...
			if ($row[csf("receive_basis")] == 2)
			{
				$planning_booking_sql = sql_select("select a.quality_level,a.booking_no_prefix_num from wo_booking_mst a,ppl_planning_entry_plan_dtls b where a.booking_no=b.booking_no and   b.dtls_id='" . $row[csf('booking_id')] . "'");
			}
