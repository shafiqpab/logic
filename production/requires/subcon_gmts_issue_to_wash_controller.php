		...
		<table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
            ...
            <tbody>
                <tr>
                    <td>  
						<? 
							echo create_drop_down( "cbo_party_name", 140, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$company' $buyer_cond  order by buyer_name","id,buyer_name", 1, "-- Select Party --",$buyer_id, "",1 );   	 //and buy.id in (select buyer_id from lib_buyer_party_type where party_type in (9,3))
                        ?>
                    </td>
...
if($action=="populate_data_from_search_popup")
{
	...
	$data_sql = "SELECT 
		...
		SUM ( CASE WHEN a.production_type = 9 AND a.embel_name=3 and a.entry_form=651 THEN a.production_qnty ELSE 0 END) AS total_wash
		FROM subcon_gmts_prod_dtls a
		WHERE a.status_active=1 and a.is_deleted=0 and a.order_id=$po_id and a.gmts_item_id=$item_id and a.production_type in(1,2,9)" ;
...
if($action=="color_and_size_level")
{
	...
	if( $variableSettings==2 ) // color level
	{
		...
		$sql = "SELECT a.item_id as gmts_item_id, a.color_id as color_number_id, sum(a.qnty) as order_quantity, sum(a.plan_cut) as plan_cut_qnty,
				sum(CASE WHEN c.production_type=2 then b.prod_qnty ELSE 0 END) as production_qnty,
				sum(CASE WHEN c.production_type=9 and c.embel_name=3 then b.prod_qnty ELSE 0 END) as cur_production_qnty
	...
	else if( $variableSettings==3 ) //color and size level
	{
		...
		SUM (
			CASE WHEN a.production_type = 2 THEN a.prod_qnty ELSE 0 END)
			AS production_qnty,
		SUM (
			CASE
				WHEN a.production_type = 9 AND b.embel_name =3
				THEN
					a.prod_qnty
				ELSE
					0
			...
			AS cur_production_qnty
			  ...
			  WHERE  a.dtls_id=b.id and b.status_active=1 and a.dtls_id=b.id and b.status_active=1 and b.is_deleted=0 and b.order_id='$po_id' and b.gmts_item_id='$item_id' and a.ord_color_size_id!=0 and a.production_type in(2,9) group by a.ord_color_size_id" );
		...
		<?php
			...
			foreach($sqlResult as $row)
			{
                ...
                <tr bgcolor="<?=$bgcolor; ?>" style="text-decoration:none; cursor:pointer" >
					...
					<td width="100" align="center" onClick="fnc_load_from_dtls('<?=$row['ID'].'**'.$row['EMBEL_NAME'].'**'.$row['SYSTEM_MST_ID']; ?>');"><p><?= $row['CUST_STYLE_REF']?></p></td>
					<td width="80" align="center" onClick="fnc_load_from_dtls('<?=$row['ID'].'**'.$row['EMBEL_NAME'].'**'.$row['SYSTEM_MST_ID']; ?>');"><p><?= $row['ORDER_NO']?></p></td> 
                    <td width="150" align="center" onClick="fnc_load_from_dtls('<?=$row['ID'].'**'.$row['EMBEL_NAME'].'**'.$row['SYSTEM_MST_ID']; ?>');"><p><?=$garments_item[$row['GMTS_ITEM_ID']]; ?></p></td>
                    <td width="80" align="center" onClick="fnc_load_from_dtls('<?=$row['ID'].'**'.$row['EMBEL_NAME'].'**'.$row['SYSTEM_MST_ID']; ?>');"><p><?=change_date_format($row['PRODUCTION_DATE']); ?></p></td>
                    <td width="80" align="center" onClick="fnc_load_from_dtls('<?=$row['ID'].'**'.$row['EMBEL_NAME'].'**'.$row['SYSTEM_MST_ID']; ?>');"><p><?=$row['PRODUCTION_QNTY']; ?></p></td>
                    <td width="150" align="center" onClick="fnc_load_from_dtls('<?=$row['ID'].'**'.$row['EMBEL_NAME'].'**'.$row['DELIVERY_MST_ID']; ?>');"><p><?=$company_arr[$row['SERVING_COMPANY']]; ?></p></td>
                    <td width="120" align="center" onClick="fnc_load_from_dtls('<?=$row['ID'].'**'.$row['EMBEL_NAME'].'**'.$row['DELIVERY_MST_ID']; ?>');"><p><?=$location_arr[$row['LOCATION_ID']]; ?></p></td>
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if (str_replace("'", "", $txt_system_no) == "") 
		{
            $year_cond="to_char(insert_date,'YYYY')";
          	...
          	$new_sys_number = explode("*", return_next_id_by_sequence("", "subcon_pro_gmts_system_mst",$con,1,$cbo_company_name,'SGIW',651,date("Y",time()),0,0,9,3,0 ));
            ...
            $data_array_delivery = "(" . $mst_id . ",'" . $new_sys_number[1] . "','" .(int) $new_sys_number[2] . "','" . $new_sys_number[0] . "', " . $cbo_company_name . "," . $cbo_location_name . ",9,3," . $txt_issue_date . ",651,".$cbo_emb_company.",".$cbo_wash_location."," . $txt_remark . "," . $cbo_party_name . "," . $user_id . ",'" . $pc_date_time . "')";
        ...
        else 
        {
            ...
            $data_array_delivery = "".$cbo_company_name."*". $cbo_location_name."*". $txt_issue_date."*".$cbo_emb_company."*".$cbo_wash_location."*".$txt_remark."*".$user_id."*'".$pc_date_time."'";
		...
		// ========================================================================================
		$field_array1="id,system_mst_id, company_id,order_id, gmts_item_id,serving_company, location_id, embel_name, production_date, production_qnty, production_type, entry_break_down_type, remarks, total_produced, yet_to_produced, inserted_by, insert_date,entry_form,status_active, is_deleted";
		$data_array1="(".$id.",".$mst_id.",".$cbo_company_name.",".$hidden_po_break_down_id.", ".$cbo_item_name.",".$cbo_emb_company.",".$cbo_wash_location.",3,".$txt_issue_date.",".$txt_issue_qty.",9,".$sewing_production_variable.",".$txt_remark.",".$txt_cum_issue_qty.",".$yet_to_issue.",".$user_id.",'".$pc_date_time."',651,1,0)";
		...
		// ========================================================================================
		$field_array="id, dtls_id,gmts_item_id,production_type, ord_color_size_id,prod_qnty,status_active, is_deleted";
  		$dtlsrID=true;
		if(str_replace("'","",$sewing_production_variable)==2)//color level wise
		{
			...
			foreach($rowEx as $rowE=>$val)
			{
				...
				if($colSizeID_arr[$colorSizeNumberIDArr[0]]!="")
				{
					...
					if($j==0) $data_array = "(".$dtls_id.",".$id.",".$cbo_item_name.",9,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."',1,0)";
					else $data_array .= ",(".$dtls_id.",".$id.",".$cbo_item_name.",9,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."',1,0)";
					//$dtls_id=$dtls_id+1;
					$j++;
				}
			}
		...
		if(str_replace("'","",$sewing_production_variable)==3)//color and size wise
		{
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				if($colSizeID_arr[$index]!="")
				{
					...
					if($j==0)$data_array = "(".$dtls_id.",".$id.",".$cbo_item_name.",9,'".$colSizeID_arr[$index]."','".$colorSizeValue."',1,0)";
					else $data_array .= ",(".$dtls_id.",".$id.",".$cbo_item_name.",9,'".$colSizeID_arr[$index]."','".$colorSizeValue."',1,0)";
					//$dtls_id=$dtls_id+1;
					$j++;
				}
			}
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		// =============================================================================================
 		...
 		$field_array1="serving_company*location_id*production_date*production_qnty*entry_break_down_type*remarks*total_produced*yet_to_produced*updated_by*update_date";
		

		$data_array1="".$cbo_emb_company."*".$cbo_wash_location."*".$txt_issue_date."*".$txt_issue_qty."*".$sewing_production_variable."*".$txt_remark."*".$txt_cum_issue_qty."*".$yet_to_issue."*".$user_id."*'".$pc_date_time."'";
			...
			$field_array="id,dtls_id,gmts_item_id,production_type, ord_color_size_id,prod_qnty,status_active, is_deleted";
			 ...
			 if(str_replace("'","",$sewing_production_variable)==2)//color level wise
			 {
				 ...
				 foreach($rowEx as $rowE=>$val)
				 {
					 ...
					 {
						 $dtls_id=return_next_id("id", "subcon_gmts_prod_col_sz", 1);
						 if($j==0) $data_array = "(".$dtls_id.",".$txt_mst_id.",".$cbo_item_name.",9,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."',1,0)";
						 else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",".$cbo_item_name.",9,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."',1,0)";
						 //$dtls_id=$dtls_id+1;
						 $j++;
					 }
				 }
			 ...
			 if(str_replace("'","",$sewing_production_variable)==3)//color and size wise
			 {
				 ...
				 foreach($rowEx as $rowE=>$valE)
				 {
					 ...
					 if($colSizeID_arr[$index]!="")
					 {
						 ...
						 //2 for Issue to Print / Emb Entry
						 $dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq", "subcon_gmts_prod_col_sz", $con );
						 if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",".$cbo_item_name.",9,'".$colSizeID_arr[$index]."','".$colorSizeValue."',1,0)";
						 else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",".$cbo_item_name.",9,'".$colSizeID_arr[$index]."','".$colorSizeValue."',1,0)";
						 //$dtls_id=$dtls_id+1;
						 $j++;
					 }
				 }
		...
		$challanrID=sql_update("subcon_pro_gmts_system_mst",$field_array_delivery,$data_array_delivery,"id","".$txt_system_id."",1);
		$dtls_delete = execute_query("delete subcon_gmts_prod_col_sz where dtls_id=$txt_mst_id");
		if(str_replace("'","",$sewing_production_variable)!=1 && str_replace("'","",$txt_mst_id)!='')// check is not gross level
		{
			$dtlsrID=sql_insert("subcon_gmts_prod_col_sz",$field_array,$data_array,1); 
		}
		// echo "10**".$rID ."=". $dtlsrID ."=". $challanrID;die;
		// echo "insert into subcon_gmts_prod_col_sz (".$field_array.") values ".$data_array; 

		  

		if(str_replace("'","",$sewing_production_variable)!=1)
		{
			if($rID && $dtlsrID && $challanrID && $dtls_delete)
			{
				oci_commit($con);
			...
			{
				oci_rollback($con);
				echo "10**".str_replace("'","",$hidden_po_break_down_id)."**mst*".$txt_mst_id ."**dtls*". $dtlsrID ."**cln*". $challanrID ."**Dtls Delete*". $dtls_delete; 
			}
		}
		else
		{
	...
	else if ($operation==2)  // Delete Here---------------------------------------------------------- 
	{
		...
		$wash_receive_data=sql_select("SELECT id,order_id,production_qnty as prod_qty,reject_qnty from subcon_gmts_prod_dtls where order_id=$po_id and embel_name=3 and production_type=3 and entry_form=652 and status_active=1 and is_deleted=0");
		...
		{ 
			...
			$dtlsrID = execute_query("delete from subcon_gmts_prod_col_sz where dtls_id=$txt_mst_id",0);
...
{
	$data=explode("**",$data); 
	$sql = "SELECT id, company_id,order_id, gmts_item_id, serving_company, location_id, embel_name,production_date,production_qnty, production_type, entry_break_down_type, remarks, total_produced, yet_to_produced from subcon_gmts_prod_dtls where id='".$data[0]."' and production_type=9 and status_active=1 and is_deleted=0 order by id";
	...
	foreach($sqlResult as $result)
	{
	    ...
	    echo "load_drop_down( 'requires/subcon_gmts_issue_to_wash_controller', ".$result['COMPANY_ID'].", 'load_drop_down_po_location', 'location_td' );\n";
		echo"load_drop_down( 'requires/subcon_gmts_issue_to_wash_controller', ".$result['SERVING_COMPANY'].", 'load_drop_down_wash_location', 'wash_location_td' );\n";
		
		echo "$('#cbo_embel_name').val('".$result[csf('embel_name')]."');\n";
		
		echo "$('#txt_issue_qty').val('".$result[csf('production_qnty')]."');\n";
	...
	$sys_sql = "SELECT id,sys_number,location_id,working_location_id from subcon_pro_gmts_system_mst where id=$data[2] and production_type=9 and embel_name=3  and entry_form=651 and status_active=1 and is_deleted=0";
	...
	{  
		$sql_dtls = sql_select("select ord_color_size_id, a.prod_qnty, size_id, color_id from subcon_gmts_prod_col_sz a,subcon_ord_breakdown b where a.dtls_id=". $data[0] ."  and a.ord_color_size_id=b.id and b.order_id=$po_id and b.item_id=$item_id");
		foreach($sql_dtls as $row)
		{
			if( $variableSettings==2 ) $index = $row[csf('color_id')]; else $index = $row[csf('size_id')].$row[csf('color_id')];
			$amountArr[$index] = $row[csf('prod_qnty')];
		}
		...
		{
			$dtlsData =sql_select("SELECT a.ord_color_size_id,
				SUM (CASE WHEN a.production_type = 2 THEN a.prod_qnty ELSE 0 END)AS production_qnty,
				SUM (CASE WHEN a.production_type = 9 AND b.embel_name =3 THEN a.prod_qnty ELSE 0 END) AS cur_production_qnty
					FROM subcon_gmts_prod_col_sz a, subcon_gmts_prod_dtls b
					WHERE  a.dtls_id=b.id and b.status_active=1  and b.is_deleted=0 and b.order_id=$po_id and b.gmts_item_id=$item_id and a.ord_color_size_id!=0 and a.production_type in(2,9) group by a.ord_color_size_id" );
				//echo $dtlsData;die;
			
			foreach($dtlsData as $row) 
			{
				$color_size_qnty_array[$row[csf('ord_color_size_id')]]['iss']= $row[csf('production_qnty')];
				$color_size_qnty_array[$row[csf('ord_color_size_id')]]['rcv']= $row[csf('cur_production_qnty')];
			}
			unset($dtlsData);
	        ...
	        <table ellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
	            ...
	            <tbody>
	                <tr>
	                    ...
	                    <td align="center"> 
	                    	<input type="button" name="btn_show" id="btn_show" class="formbutton" value="Show" onClick="show_list_view(document.getElementById('cbo_party_name').value+'_'+document.getElementById('wash_company').value+'_'+document.getElementById('txt_job_no').value+'_'+document.getElementById('txt_issue_no').value+'_'+document.getElementById('txt_order_no').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value, 'create_system_number_list_view', 'search_div', 'subcon_gmts_issue_to_wash_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:80px;" />
	                    </td>
	                </tr>
	            </tbody>
	            <tfoot>
...
if($action=="create_system_number_list_view")
{
	...
	$sql ="SELECT a.id,a.company_id,c.party_id as buyer,c.subcon_job,b.cust_style_ref as style,b.order_no,a.sys_number,a.sys_number_prefix_num as issue_id,d.production_qnty as issue_qty,d.serving_company as wash_comp,d.production_date as issue_date  from subcon_pro_gmts_system_mst a, subcon_ord_dtls b, subcon_ord_mst c,subcon_gmts_prod_dtls d where  b.mst_id=c.id and b.id=d.order_id and a.id=d.system_mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and a.production_type=9 and a.embel_name='3' and a.entry_form=651 $sql_cond group by a.id,a.company_id,c.party_id,c.subcon_job,b.cust_style_ref,b.order_no,a.sys_number_prefix_num,a.sys_number,d.production_qnty,d.serving_company,d.production_date order by a.id DESC";
	
	$arr=array(0=>$buyer_arr,6=>$company_arr); 
	echo create_list_view("list_view", "Buyer,Job No,Style,Order No,Issue ID, Issue Qty,Wash Company,Issue Date","80,100,100,100,60,60,100,100","770","340",0, $sql , "js_set_value","id,sys_number,company_id", "",1, "buyer,0,0,0,0,0,wash_comp,0", $arr,"buyer,subcon_job,style,order_no,issue_id,issue_qty,wash_comp,issue_date", "","setFilterGrid('list_view',-1)","0,0,0,0,0,0,0,0") ;
	exit();
}

if($action=="populate_mst_form_data")
{
	$sql ="SELECT id, sys_number_prefix, sys_number_prefix_num, sys_number, company_id, delivery_date,working_company_id,location_id,working_location_id,buyer_id,remarks from subcon_pro_gmts_system_mst a where a.id=$data and production_type=9 and embel_name=3 and entry_form=651";
	$result =sql_select($sql);
	 
	echo"load_drop_down( 'requires/subcon_gmts_issue_to_wash_controller', ".$result[0]['WORKING_COMPANY_ID'].", 'load_drop_down_wash_location', 'cbo_wash_location' );\n";
