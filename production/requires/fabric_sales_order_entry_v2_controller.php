...
{
	$company_id = $data; 
	$sql_auto_allocation_variable = sql_select("SELECT allocation from variable_settings_inventory where company_name ='$data' and variable_list in (18) and item_category_id=1 and is_deleted=0 and status_active=1");
	foreach($sql_auto_allocation_variable as $row)
	{
		$auto_allocation= $row[csf("allocation")];
	}

	$sql_auto_allocate_yarn_variable = sql_select("SELECT auto_allocate_yarn_from_requis from variable_settings_production where company_name ='$data' and variable_list in (6) and is_deleted=0 and status_active=1");
	foreach($sql_auto_allocate_yarn_variable as $row)
	{
		$auto_allocate_yarn= $row[csf("auto_allocate_yarn_from_requis")];
	}
	echo "document.getElementById('hdn_auto_allocation').value 	= '".$auto_allocation."';\n";
	echo "document.getElementById('hdn_auto_allocate_yarn').value 	= '".$auto_allocate_yarn."';\n";
	exit();
}
// if ($action == "booking_previ_copy_check")
// {
...
// 	$sql_booking="select booking_date from fabric_sales_order_mst where  entry_form=472 and  booking_date>='$from_date' and status_active=1";

// 	//echo $sql_booking;die;
// 	$data_array=sql_select($sql_booking);
		...
		<fieldset style="width:720px;margin-left:10px">
			...
			<div style="margin-left:10px; margin-top:10px">
				...
				<div style="width:700px; max-height:300px; overflow-y:scroll" id="list_container" align="left">
					...
					<?
					$i = 1;
					$data_array = sql_select("select id, construction, fab_nature_id, gsm_weight, color_range_id, fabric_composition_id from lib_yarn_count_determina_mst where fab_nature_id=2 and status_active=1 and is_deleted=0");
					foreach ($data_array as $row) {
						...
						<tr bgcolor="<? echo $bgcolor; ?>"
							...
							<td width="90"><? echo $item_category[$row[csf('fab_nature_id')]]; ?></td>
							<td width="150"><p><? echo $row[csf('construction')]; ?></p></td>
							<td width="120"><p><? echo $lib_fabric_composition[$row[csf('fabric_composition_id')]]; ?></p></td>
							<td><p><? echo $composition_arr[$row[csf('id')]]; ?></p></td>
		...
		foreach ($data_array as $row) 
		{
			...
			<tr class="general" id="tr_<? echo $i; ?>">
				...
				<td width="67">
					...
					$grey_quantity = round($grey_quantity);
					?>
...
if ($action == 'show_fabric_details_update') {
	...
	foreach ($data_array as $row) {
		...
		<tr class="general" id="tr_<? echo $i; ?>">
			...
			<td width="82">
				<?
				echo create_drop_down("cboBodyPart_" . $i, 80, $body_part, "", 1, "- Select -", $row[csf('body_part_id')], "show_ccmp(".$i.");", $disableConsUOM, "", "", "", "", "", "", "cboBodyPart[]");
				?>
			</td>
			<td width="72">
				<?
			...
			<td width="50">
				<input type="button" class="image_uploader" style="width:50px" id="btnimg_<? echo $i; ?>" value="IMG" onClick="file_uploader ( '../','<? echo $row[csf("id")]; ?>','', 'fabric_sales_order_entry_v2', 0 ,1)" />
			</td>
			<td width="100">
			...
			<td width="52">
				<?
				echo create_drop_down("cboConsUom_" . $i, 50, $unit_of_measurement, "", 0, "", $row[csf('cons_uom')], "", $disableConsUOM, "1,12,23,27", "", "", "", "", "", "cboConsUom[]");
				?>
			</td>
			<td width="67">
				<input type="text" name="txtBookingQnty[]" id="txtBookingQnty_<? echo $i; ?>" class="text_boxes_numeric"
				style="width:55px" value="<? echo number_format($row[csf('grey_qnty_by_uom')], 4, '.', ''); ?>" title="<? echo number_format($row[csf('grey_qnty_by_uom')], 4, '.', ''); ?>" onKeyUp="calculate_amount(<?php echo $i;?>);calculate_fin_qty(<?php echo $i;?>);calculate_grey_qty(<?php echo $i;?>);" onClick="openmypage_ccmp(<? echo $i; ?>);" <? echo $disable; ?> />
			</td>
			<td width="57">
				<input type="text" name="txtAvgRate[]" id="txtAvgRate_<? echo $i; ?>" class="text_boxes_numeric"
			...
			<td width="52">
				<input type="text" name="txtAdjustFinQty[]" id="txtAdjustFinQty_<? echo $i; ?>" class="text_boxes_numeric"
				style="width:45px" value="<? echo $row[csf('adjust_finish_qnty')]; ?>"  readonly disabled/>
			</td>
			<td width="67">
				...
				<input type="hidden" name="rmgQty[]" id="rmgQty_<? echo $i; ?>" value="<? echo $row[csf('rmg_qty')]; ?>" readonly/>
				<input type="hidden" name="hiddenFinishQty[]" id="hiddenFinishQty_<? echo $i; ?>" value="<? echo $row[csf('finish_qty')]; ?>" readonly/>
			</td>
			...
			<td width="67">
				<input type="text" name="txtGreyQty[]" id="txtGreyQty_<? echo $i; ?>" class="text_boxes_numeric"
				style="width:55px" value="<? echo round($row[csf('grey_qty')]);//echo number_format($row[csf('grey_qty')], 4, '.', ''); ?>" title="<? echo number_format($row[csf('grey_qty')], 4, '.', ''); ?>"
				readonly/>
			</td>
			<td width="82">
				<?
	...
	foreach ($data_array as $row) 
	{
		...
		if ($data_arr[$index]) 
		{
			...
			<tr class="general" id="tr_<? echo $i; ?>">
				...
				<td width="52">
					<input type="text" name="txtAdjustQnty[]" id="txtAdjustQnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px" ondblclick="openmypage_adjust_qnty(<? echo $i; ?>)" value="<? echo $row[csf('adjust_qnty')]; ?>" readonly/>
					<input type="hidden" name="txtAdjustQntyString[]" id="txtAdjustQntyString_<? echo $i; ?>" />
				</td>
				<td width="67">
	...
	if (count($data_arr) > 0) 
	{
		foreach ($data_arr as $key => $val) 
		{
			...
			<tr class="general" id="tr_<? echo $i; ?>">
				...
				<td width="52">
					<input type="text" name="txtAdjustQnty[]" id="txtAdjustQnty_<? echo $i; ?>" class="text_boxes" style="width:40px" value="" ondblclick="openmypage_adjust_qnty(<? echo $i; ?>)" readonly/>
				</td>
				<td width="67">
					<input type="text" name="txtGreyQty[]" id="txtGreyQty_<? echo $i; ?>" class="text_boxes_numeric" style="width:55px" value="<? echo round($gray_qty_with_process_loss); ?>" title="<? echo number_format($gray_qty_with_process_loss, 4, '.', ''); ?>"
					readonly/>
				</td>
				<td width="82">
					<?
...
if ($action == "save_update_delete") {
	...
	$delivery_month_year = date("n,Y",strtotime($delivery_date));
	$month_target = sql_select("select sum(sales_target_qty) as sales_target_qty, b.year_month_name from wo_sales_target_mst a, wo_sales_target_dtls b where a.id=b.sales_target_mst_id and a.entry_form =513 and a.company_id=$cbo_company_id and a.team_leader=$cbo_team_leader and b.year_month_name = '$delivery_month_year' and a.status_active=1 group by b.year_month_name");

	$month_target_qnty = $month_target[0][csf("sales_target_qty")]*1;
	...
	for ($i = 1; $i <= $total_row; $i++) 
	{
        ...
        $txtAdjustFinQty = "txtAdjustFinQty" . $i;

		if($txt_fso_ref_id =="")
		{
			$total_current_finish += $$txtFinishQty + $$txtPPQty +$$txtMTLQty +$$txtFPTQty + $$txtGPTQty;
		}
		else
		{
			$total_current_finish += $$txtAdjustFinQty;
		}

		
	}
	$delivery_month = date("m",strtotime($delivery_date));
	$delivery_year = date("Y",strtotime($delivery_date));

	if($operation==1){
		$fso_up_cond = " and a.id <>$update_id";
	}

	$pre_fso_sql = sql_select("select sum(b.finish_qty) as fin_qnty, sum(b.pp_qnty) as pp_qnty, sum(b.mtl_qnty) as mtl_qnty, sum(b.fpt_qnty) as fpt_qnty, sum(b.gpt_qnty) as gpt_qnty, sum(adjust_finish_qnty) as adjust_finish_qnty from fabric_sales_order_mst a, fabric_sales_order_dtls b where a.id=b.mst_id and a.entry_form= 472 and b.status_active=1 and a.team_leader=$cbo_team_leader and to_char(a.delivery_date, 'YYYY') = '$delivery_year' and to_char(a.delivery_date, 'MM') = '$delivery_month' $fso_up_cond group by a.team_leader");

	if($txt_fso_ref_id =="")
	{
		$pre_fso_qnty_by_TL = $pre_fso_sql[0][csf("fin_qnty")]*1 + $pre_fso_sql[0][csf("pp_qnty")]*1 + $pre_fso_sql[0][csf("mtl_qnty")]*1 + $pre_fso_sql[0][csf("fpt_qnty")]*1 + $pre_fso_sql[0][csf("gpt_qnty")]*1;
	}
	else
	{
		$pre_fso_qnty_by_TL = $pre_fso_sql[0][csf("adjust_finish_qnty")]*1;
	}

	if($pre_fso_qnty_by_TL + $total_current_finish > $month_target_qnty)
	{
		echo "5**Total finish qty exceeded team wise target qty\nMonthly Target :".$month_target_qnty."\nPrevious Qnty :".$pre_fso_qnty_by_TL."\nCurrent Qnty :".$total_current_finish;
		die;
	}
	//echo "5**failed";die;
	...
	if ($operation == 0)  // Insert Here
	{
		...
		if($txt_fso_ref_id != ""){
			$rID4 = sql_insert("fabric_sales_order_adjust_dtls", $field_array_adjust_fso_dtls, $data_array_adjust_fso_dtls, 1);
		}
		...
		/* echo "10**insert into fabric_sales_order_adjust_dtls (".$field_array_adjust_fso_dtls.") values ".$data_array_adjust_fso_dtls; 
		oci_rollback($con); die; */


		//echo "5**"."$rID && $rID2 && $rID3 && $rID4"; oci_rollback($con);die;
		if ($db_type == 0) {
	...
	} else if ($operation == 1)   // Update Here
	{
		...
		// for within group no: check if program found while changing sales order info or cancelled this sales order
		if ($cbo_within_group_combo == 2) 
		{
			if(str_replace("'", "", $txt_booking_no) != str_replace("'", "", $txt_hdn_booking_no) || str_replace("'","",$cbo_fso_status) ==3)
			{
				...
				{
					if(str_replace("'","",$cbo_fso_status) ==3)
					{
						//echo "5**Program found. Sales Order No can not be cancelled.";
						//die;
					}
					else
					{
			...
			$sales_order_dtls_ids_arr=array();
			$plan_sql="SELECT a.po_id,a.sales_order_dtls_ids,a.body_part_id,a.determination_id,a.gsm_weight,a.dia,a.dia,a.color_type_id,b.color_id,b.color_prog_qty  
			from ppl_planning_entry_plan_dtls a,ppl_color_wise_break_down b 
			where a.mst_id=b.plan_id and a.dtls_id=b.program_no and a.po_id=$job_update_id and a.is_sales=1 and a.status_active=1 and b.status_active=1";
			$plan_data_sql=sql_select($plan_sql);
			foreach ($plan_data_sql as $key => $row) 
			{
				$sales_order_dtls_ids .= $row[csf('sales_order_dtls_ids')].",";
				// $sales_order_dtls_ids_arr=explode(",", $row[csf('sales_order_dtls_ids')]);
				// echo $row[csf('body_part_id')].']['.$row[csf('determination_id')].']['.$row[csf('gsm_weight')].']['.$row[csf('dia')].']['.$row[csf('color_type_id')].']['.$row[csf('color_id')].'=<br>';

				$progQntyArr[$row[csf('body_part_id')]][$row[csf('determination_id')]][$row[csf('gsm_weight')]][$row[csf('dia')]][$row[csf('color_type_id')]][$row[csf('color_id')]]['program_qty']+=$row[csf('color_prog_qty')];
			}
		}		
		$sales_order_dtls_ids_arr=array_unique(explode(",",chop($sales_order_dtls_ids,",")));
		// echo '<pre>';print_r($sales_order_dtls_ids_arr);die;
		...
		/* $attached_adjust_fso = sql_select("select id, job_no from fabric_sales_order_mst where from_fso_id='". $job_update_id . "' and status_active=1 and is_deleted=0" );

		if(!empty($attached_adjust_fso))
		{
			echo "5**Sales order is adjusted in another one. Sales Order can not be changed\nSales Order no : ".$attached_adjust_fso[0][csf("job_no")];
			die;
		} */
		...
		if(!empty($attached_adjust_fso))
		{
			...
			{
				$adDtlsStr = $adrow[csf('determination_id')]."*".$adrow[csf('gsm_weight')]."*".$adrow[csf('after_wash_gsm')];
		...
		if(str_replace("'","",$cbo_fso_status) ==3)
		{
			$check_allocation_info = return_field_value("job_no", "inv_material_allocation_mst", "job_no=$txt_job_no and is_sales=1 and status_active=1 and is_deleted=0 group by job_no","job_no");
			if($check_allocation_info !="")
			{
				echo "5**Allocation found. Sales Order No can not be cancelled.";
				die;
			}

			$check_requisition_info = return_field_value("requisition_no", "ppl_yarn_requisition_entry a, ppl_planning_entry_plan_dtls b, fabric_sales_order_mst c", "a.knit_id=b.dtls_id and b.po_id=c.id and c.job_no=$txt_job_no and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.requisition_no","requisition_no");
			if($check_requisition_info !="")
			{
				echo "5**Requisition found. Sales Order No can not be cancelled.";
				die;
			}

			$check_purchase_requisition_info = return_field_value("requ_no", "inv_purchase_requisition_mst a, inv_purchase_requisition_dtls b", "a.id=b.mst_id and a.entry_form=70 and item_category=1 and b.booking_no=$txt_job_no 
			and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","requ_no");
			if($check_purchase_requisition_info !="")
			{
				echo "5**Yarn Purchase Requisition found. Sales Order No can not be cancelled.";
				die;
			}

			$check_export_pi_info = return_field_value("pi_number", "com_export_pi_mst a, com_export_pi_dtls b", "a.id =b.pi_id and a.item_category_id=10 and b.work_order_no=$txt_job_no and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","pi_number");
			if($check_export_pi_info !="")
			{
				echo "5**Export PI found. Sales Order No can not be cancelled.";
				die;
			}
		...
		foreach ($check_sales_dtls as $orows) {
			if($orows[csf("within_group")] == 1){
				...
				$old_id_desc[$orows[csf('id')]] = $orows[csf('body_part_id')]."*".$orows[csf('color_type_id')]."*".$orows[csf('determination_id')]."*".$orows[csf('gsm_weight')]."*".$orows[csf('dia')]."*".$orows[csf('fabric_desc')]."*".$orows[csf('width_dia_type')]."*".$orows[csf('color_id')]."*".$orows[csf('color_range_id')]."*".$orows[csf('finish_qty')]."*".$orows[csf('rmg_qty')]."*".$orows[csf('avg_rate')]."*".$orows[csf('amount')]."*".$orows[csf('process_loss')]."*".$orows[csf('grey_qty')]."*".$orows[csf('work_scope')]."*".$orows[csf('order_uom')]."*".$orows[csf('pre_cost_remarks')]."*".$orows[csf('after_wash_gsm')];
			}
		}

		if ($cbo_within_group_combo == 1) {
			$field_array_dtls_update = "body_part_id*color_type_id*fabric_desc*determination_id*gsm_weight*dia*width_dia_type*color_id*color_range_id*finish_qty*rmg_qty*avg_rate*amount*process_loss*grey_qty*work_scope*order_uom*pre_cost_fabric_cost_dtls_id*item_number_id*pre_cost_remarks*updated_by*update_date*status_active*is_deleted*grey_qnty_by_uom*cons_uom*process_id*process_seq*process_id_main*process_seq_main*after_wash_gsm*cuttable_dia*pp_qnty*mtl_qnty*fpt_qnty*gpt_qnty*ccmp_data*ply*coller_cuff_cons*lab_dip_no*sl_no*adjust_grey_qnty*adjust_finish_qnty";
		} else {
			$field_array_dtls_update = "body_part_id*color_type_id*fabric_desc*determination_id*gsm_weight*dia*width_dia_type*color_id*color_range_id*finish_qty*rmg_qty*avg_rate*amount*process_loss*grey_qty*work_scope*order_uom*pre_cost_remarks*updated_by*update_date*status_active*is_deleted*grey_qnty_by_uom*cons_uom*process_id*process_seq*process_id_main*process_seq_main*after_wash_gsm*cuttable_dia*pp_qnty*mtl_qnty*fpt_qnty*gpt_qnty*ccmp_data*ply*coller_cuff_cons*lab_dip_no*sl_no*adjust_grey_qnty*adjust_finish_qnty";
		}
		$is_new = 0;
		$dtls_ids_to_delete = "";
		...
		for ($i = 1; $i <= $total_row; $i++) {
			...
			{
				$block_fso_dtls_qnty[$$fabricDescId."*".$$txtFabricGsm."*".$$txtAfterFabricGsm] += $$txtGreyQty;
			}

            if(in_array($$updateIdDtls, $sales_order_dtls_ids_arr))
            {
            	// echo $$cboBodyPart.']['.$$fabricDescId.']['.$$txtFabricGsm.']['.$$txtFabricDia.']['.$$cboColorType.']['.$$colorId.'<br>';

            	$fso_qty=$sales_data_arr[$$cboBodyPart][$$fabricDescId][$$txtFabricGsm][$$txtFabricDia][$$cboColorType][$$colorId]['grey_qty']+=$$txtGreyQty;

            	$program_qty=$progQntyArr[$$cboBodyPart][$$fabricDescId][$$txtFabricGsm][$$txtFabricDia][$$cboColorType][$$colorId]['program_qty'];
            	// echo $program_qty.'>'.$fso_qty;die;
            	/*if ($program_qty>$fso_qty)
            	{
            		echo "5**Grey qty can not less than program qty.";die;
            	} */
            }
            // echo '<pre>';print_r($sales_order_dtls_ids_arr);die;
            // echo '5**check='.$$updateIdDtls;die;
						...
						if(str_replace("'","",$$txtAdjustQntyString) !="")
						{
							...
							foreach ($txtAdjustQntyString as $adVal) 
							{
								...
								if($adjust_qnty !="")
								{
									...
									if ($data_array_adjust_fso_dtls != "") $data_array_adjust_fso_dtls .= ",";
									$data_array_adjust_fso_dtls .= "(" . $id_adjust_dtls . "," . $job_update_id . ",'" . $job_no . "'," . $tid . ",'" . $txt_fso_ref_id . "','" . $from_fso_dtls_id . "','" . $adjust_qnty ."',". $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
								}
							}
						}
					}
					...
					if(str_replace("'","",$$txtAdjustQntyString) !="")
					{
						...
						foreach ($txtAdjustQntyString as $adVal) 
						{
							...
							if($adjust_qnty !="")
							{
								...
								if ($data_array_adjust_fso_dtls != "") $data_array_adjust_fso_dtls .= ",";
								$data_array_adjust_fso_dtls .= "(" . $id_adjust_dtls . "," . $job_update_id . ",'" . $job_no . "'," . str_replace("'", '', $$updateIdDtls) . ",'" . $txt_fso_ref_id . "','" . $from_fso_dtls_id . "','" . $adjust_qnty ."',". $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
							}
						}
					}
			...
			else 
			{
				...
				if (str_replace("'", "", $$updateIdDtls) == "") 
				{
					//N. B. If found adjustment FSO then new row can not be added.
					if(!empty($attached_adjust_fso))
					{
						//Can not add row
						echo "5**Sales order is adjusted in another one. New row can not be added\nSales Order no : ".$attached_adjust_fso[0][csf("job_no")];
						die;
					}
				...
				{
					//echo "10**".$deletedDtlsIds; oci_rollback($con);die;
					$rID2 = 1;
					$get_existing_dtls_row = explode("*",$old_id_desc[str_replace("'", "", $$updateIdDtls)]);
					
					//N. B. If found adjustment FSO then row info ( description id, gsm, after gsm) can not be changed.
					if(!empty($attached_adjust_fso))
					{
						if( ($get_existing_dtls_row[2] != $$fabricDescId) || ($get_existing_dtls_row[3] != $$txtFabricGsm) ||($get_existing_dtls_row[18] != $$txtAfterFabricGsm) )
						{
							//Can not Change row
							echo "5**Sales order is adjusted in another one. Existing row's fabrication, gsm, after gsm can not be Updated.\nSales Order no : ".$attached_adjust_fso[0][csf("job_no")];
							die;
						}
					}


					// This part created for sales dtls row deleted by clicking (-) button from interface.
					...
					if($deleted_ids)
					{					
						 $check_program_in_req_issues = sql_select("select a.id dtls_id,e.body_part_id,a.color_id,e.color_type_id,e.determination_id,e.dia, e.fabric_desc,e.gsm_weight,e.width_dia_type,listagg(b.id,',') within group (order by b.id) as requisition_id,listagg(b.prod_id,',') within group (order by b.id) as  requisition_prod_id,listagg(b.yarn_qnty,',') within group (order by b.id) as  requisition_qnty,listagg(d.mst_id,',') within group (order by d.mst_id) as  issue_id, listagg(c.id,',') within group (order by c.id) as  production_id from ppl_planning_info_entry_dtls a left join ppl_planning_info_entry_mst e on a.mst_id=e.id left join ppl_yarn_requisition_entry b on a.id=b.knit_id left join inv_transaction d on b.requisition_no=d.requisition_no left join inv_receive_master c on (a.id=c.booking_id and e.company_id=c.company_id and c.entry_form=2 and c.item_category=13 and c.receive_basis=2), ppl_planning_entry_plan_dtls f where a.id=f.dtls_id and e.booking_no=$txt_booking_no and f.po_id=$update_id and f.sales_order_dtls_ids in('".$deleted_ids."') group by a.id,e.body_part_id,a.color_id,e.color_type_id,e.determination_id,e.dia,e.fabric_desc, e.gsm_weight,e.width_dia_type");
						 	$user_id = $_SESSION['logic_erp']['user_id'];

							if (!empty($check_program_in_req_issues)) {
							foreach ($check_program_in_req_issues as $plan_row) {
					...
					if(($get_existing_dtls_row[0] != $$cboBodyPart) || ($get_existing_dtls_row[1] != $$cboColorType) || ($get_existing_dtls_row[2] != $$fabricDescId) || ($get_existing_dtls_row[3] != $$txtFabricGsm) || ($get_existing_dtls_row[4] != $$txtFabricDia) ||($get_existing_dtls_row[6] != $$cboDiaWidthType) ||($get_existing_dtls_row[18] != $$txtAfterFabricGsm) )
					{
						$id_dtls = return_next_id_by_sequence("FABRIC_SALES_ORDER_DTLS_PK_SEQ", "fabric_sales_order_dtls", $con);
						...
						$check_program_in_req_issue = sql_select("select a.id dtls_id,e.body_part_id,a.color_id,e.color_type_id,e.determination_id,e.dia, e.fabric_desc,e.gsm_weight,e.width_dia_type,listagg(b.id,',') within group (order by b.id) as requisition_id,listagg(b.prod_id,',') within group (order by b.id) as  requisition_prod_id,listagg(b.yarn_qnty,',') within group (order by b.id) as  requisition_qnty,listagg(d.mst_id,',') within group (order by d.mst_id) as  issue_id, listagg(c.id,',') within group (order by c.id) as  production_id from ppl_planning_info_entry_dtls a left join ppl_planning_info_entry_mst e on a.mst_id=e.id left join ppl_yarn_requisition_entry b on a.id=b.knit_id left join inv_transaction d on b.requisition_no=d.requisition_no left join inv_receive_master c on (a.id=c.booking_id and e.company_id=c.company_id and c.entry_form=2 and c.item_category=13 and c.receive_basis=2), ppl_planning_entry_plan_dtls f where a.id=f.dtls_id and e.booking_no=$txt_booking_no and f.po_id=$update_id group by a.id,e.body_part_id,a.color_id,e.color_type_id,e.determination_id,e.dia,e.fabric_desc, e.gsm_weight,e.width_dia_type");
						//echo "10**".$check_program_in_req_issue; oci_rollback($con); die;
						$user_id = $_SESSION['logic_erp']['user_id'];
					...
					{	//echo '5**titffffk<br>';
						//echo "(".$get_existing_dtls_row[0] ."!=". $$cboBodyPart .")||(". $get_existing_dtls_row[1] ."!=". $$cboColorType.")||(".$get_existing_dtls_row[2] ."!=". $$fabricDescId.")||(".$get_existing_dtls_row[3] ."!=". $$txtFabricGsm.")||(".$get_existing_dtls_row[4] ."!=". $$txtFabricDia.")||(".$get_existing_dtls_row[6] ."!=". $$cboDiaWidthType;
						//oci_rollback($con);die;
						$dtlsId = str_replace("'", '', $$updateIdDtls);
						$id_arr[] = str_replace("'", '', $$updateIdDtls);
						$data_array_dtls_update[str_replace("'", '', $$updateIdDtls)] = explode("*", ("'" . $$cboBodyPart . "'*'" . $$cboColorType . "'*'" . $$txtFabricDesc . "'*'" . $$fabricDescId . "'*'" . $$txtFabricGsm . "'*'" . $$txtFabricDia . "'*'" . $$cboDiaWidthType . "'*'" . $color_id . "'*'" . $$cboColorRange . "'*'" . $$txtFinishQty . "'*'" . $$rmgQty . "'*'" . $$txtAvgRate . "'*'" . $$txtAmount . "'*'" . $$txtProcessLoss . "'*'" . $$txtGreyQty . "'*'" . $$cboWorkScope . "'*'" . $$cboUom . "'*'" . $$txtRemarks . "'*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'*1*0*" . $$booking_qnty_by_uom . "*" . $$cboConsUom. "*'" . $$txtProcessId. "'*'" . $$txtProcessSeq . "'*'" . $$txtProcessIdMain . "'*'" . $$txtProcessSeqMain . "'*'" .$$txtAfterFabricGsm ."'*'". $$txtFabricCuttableDia . "'*'" . $$txtPPQty . "'*'" . $$txtMTLQty . "'*'" . $$txtFPTQty . "'*'". $$txtGPTQty . "'*'". $$ccmpSaveData."'*'".$$cboPly."'*'".$$hdnCollerCuffCon."'*'".$$txtLabDipNo."'*'".$i ."'*'" . $$txtAdjustQnty . "'*'" .$$txtAdjustFinQty. "'"));
		...
		if(!empty($allocation_product_qnty_arr)  && str_replace("'","",$hdn_auto_allocation) ==1 && str_replace("'","",$hdn_auto_allocate_yarn) ==1){
		...
		//echo "10**$rID3 rr";oci_rollback($con);die;
		...
		$rID4 = true;
		if ($data_array_dtls != "") {
			//echo "10**insert into fabric_sales_order_dtls (".$field_array_dtls.") values ".$data_array_dtls;oci_rollback($con);die;
		...
		$adjust_dtls_insert=1;
		if($txt_fso_ref_id != "")
		{
			$adjust_dtls_insert =sql_insert("fabric_sales_order_adjust_dtls", $field_array_adjust_fso_dtls, $data_array_adjust_fso_dtls, 1);
		}
			...
			<form name="searchorderfrm_1" id="searchorderfrm_1" autocomplete="off">
				<table cellpadding="0" cellspacing="0" width="700" border="1" rules="all" class="rpt_table">
					...
					<tr class="general">
						...
						<td align="center">
							<input type="button" name="button2" class="formbutton" value="Show"
							onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+'<? echo $cbo_company_id; ?>'+'_'+document.getElementById('cbo_within_group').value + '_' + document.getElementById('cbo_sales_order_type').value + '_' + document.getElementById('cbo_buyer_name').value, 'create_job_search_list_view', 'search_div', 'fabric_sales_order_entry_v2_controller', 'setFilterGrid(\'tbl_list_search\',-1);')"
							style="width:100px;"/>
						</td>
					</tr>
				</table>
...
if ($action == "create_job_search_list_view") {
	...
	if ($cbo_buyer_name == 0) $cust_buyer_cond = ""; else $cust_buyer_cond = " and a.customer_buyer=$cbo_buyer_name";
	...
	$sql = "select a.id, $year_field, a.job_no_prefix_num, a.job_no, a.within_group, a.sales_order_type, a.sales_booking_no, a.booking_date, a.buyer_id, a.customer_buyer, a.style_ref_no, a.location_id,a.inserted_by,b.user_name,b.user_level from fabric_sales_order_mst a,user_passwd b where  a.inserted_by=b.id and a.entry_form=472 and a.status_active=1 and a.is_deleted=0 and a.company_id=$company_id $within_group_cond $sales_type_cond $search_field_cond $cust_buyer_cond order by a.id desc";
	//echo $sql;//die;
	$result = sql_select($sql);
	?>
	...
	<div style="width:900px; max-height:260px; overflow-y:scroll" id="list_container_batch" align="left">
		...
		if(!empty($result)){
			foreach ($result as $row) {
				...
				if($user_level==1 && $row[csf('user_level')]==1)
				{
					if ($i % 2 == 0) $bgcolor = "#E9F3FF"; else $bgcolor = "#FFFFFF";

					if ($row[csf('within_group')] == 1)
						$customer = $company_arr[$row[csf('buyer_id')]];
					else
						$customer = $buyer_arr[$row[csf('buyer_id')]];
					...
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"
						...
						<td width="70"><p><? echo $customer; ?>&nbsp;</p></td>
						<td width="70"><p><? echo $buyer_arr[$row[csf('customer_buyer')]]; ?>&nbsp;</p></td>
						<td width="100"><p><? echo $row[csf('sales_booking_no')]; ?></p></td>
						<td width="80" align="center"><? echo change_date_format($row[csf('booking_date')]); ?></td>
						<td width="110"><p><? echo $row[csf('style_ref_no')]; ?></p></td>
						<td width="100"><p><? echo $location_arr[$row[csf('location_id')]]; ?></p></td>
						<td><p><? echo $row[csf('user_name')]; ?></p></td>
					</tr>
					<?
					$i++;
				}
			...
			<form name="searchorderfrm_1" id="searchorderfrm_1" autocomplete="off">
				<table cellpadding="0" cellspacing="0" width="700" border="1" rules="all" class="rpt_table">
					...
					<tr class="general">
						...
						<td align="center">
							<input type="button" name="button2" class="formbutton" value="Show"
							onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+'<? echo $cbo_company_id; ?>'+'_'+document.getElementById('cbo_within_group').value + '_' + document.getElementById('cbo_sales_order_type').value + '_' + '<? echo $job_id; ?>'+'_'+document.getElementById('cbo_fso_status').value, 'create_from_fso_search_list_view', 'search_div', 'fabric_sales_order_entry_v2_controller', 'setFilterGrid(\'tbl_list_search\',-1);')"
							style="width:100px;"/>
						</td>
					</tr>
				</table>
...
if ($action == "create_from_fso_search_list_view") {
	...
	$sql = "select id, $year_field, job_no_prefix_num, job_no, within_group, sales_order_type, sales_booking_no, booking_date, buyer_id, style_ref_no, location_id, fso_status from fabric_sales_order_mst where entry_form=472 and status_active=1 and is_deleted=0 $within_group_cond $sales_type_cond $search_field_cond $sales_order_id_cond and fso_status=$sales_status order by id desc";
...
if ($action == 'populate_data_from_sales_order') {
	...
	foreach ($data_array as $row)
	{
		...
		echo "document.getElementById('cbo_fso_status').value 				= '" . $row[csf("fso_status")] . "';\n";
		...
		echo "document.getElementById('txt_garments_merchant').value 		= '" . $row[csf("garments_marchant")] . "';\n";
		echo "document.getElementById('txt_copy_from').value 				= '" . $row[csf("copy_from")] . "';\n";
		echo "document.getElementById('deletedDtlsIds').value 				= '';\n";
			...
			<div style="margin-top:5px; margin-left:5px">
				<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="1120">
					<thead>
						...
						<th width="90">Type</th>
						<th width="80" class="must_entry_caption">Cons. Ratio</th>
				...
				<div style="width:1120px; max-height:300px; overflow-y:scroll" id="list_container" align="left">
							...
							foreach ($yarnDatas as $value) {
								...
								<tr align="center" id="tr_<? echo $i; ?>">
									...
									<td width="80">
										...
										value="<? 
										//if($copy_val==2 && $update_type==1)
										echo $txtConsRatio; ?>"
										onKeyUp="calculate(<? echo $i; ?>,1);"/>
									</td>
									<td width="80">
										<input type="text" name="txtConsQty[]" id="txtConsQty_<? echo $i; ?>" class="text_boxes_numeric" style="width:65px" value="<? 
										//if($copy_val==2 && $update_type==1) 
										echo $txtConsQty; 
										
										?>" onKeyUp="calculate(<? echo $i; ?>,2);calculateConsAmount(<? echo $i; ?>);"/>
									</td>
					...
					<tfoot>
						...
						<th><input type="text" name="txtTotGreyQty" id="txtTotGreyQty"
							value="<? if($copy_val==2 && $update_type==1) echo number_format($tot_grey_qty, 2, '.', ''); ?>"
...
if ($action == 'yarn_details') {
	...
	if($data_arr[1] != "")
	{
		$copy_from_fso_mst_id=return_field_value("id","fabric_sales_order_mst","job_no ='".$data_arr[1]."' ");
	}
	...
	//N.B if fso comes from copy option and yarn data is not saved then yarn data reference comes from copy job
	$saveDataArr = array();
	//own yarn data
	$data_array_yarn = sql_select("select deter_id, gsm, yarn_data from fabric_sales_order_yarn where mst_id=$data_arr[0] and status_active=1 and is_deleted=0 ");

	if(!empty($data_array_yarn))
	{
		$is_yarn_part_saved = 1;
	}
	else
	{
		$is_yarn_part_saved = 0;
		//copy from fso yarn data
		$data_array_yarn = sql_select("select deter_id, gsm, yarn_data from fabric_sales_order_yarn where mst_id=$copy_from_fso_mst_id and status_active=1 and is_deleted=0 ");
	}
	//echo $sql_yarn;
	...
	$sql = "select determination_id, gsm_weight,color_range_id, sum(grey_qty) as grey_qty, $dtls_ids from fabric_sales_order_dtls where mst_id=$data_arr[0] and status_active=1 and is_deleted=0 group by determination_id, gsm_weight ,color_range_id";
	//echo $sql;die;
	$data_array = sql_select($sql);
	$i = 0;
	foreach ($data_array as $row) 
	{
		...
		$modified_yarnData="";
		if($is_yarn_part_saved ==0)
		{
			if($saveDataArr[$row[csf('determination_id')]][$row[csf('gsm_weight')]] !="")
			{
				...
				$yarn_data = $saveDataArr[$row[csf('determination_id')]][$row[csf('gsm_weight')]];
				$yarn_data_arr = explode("|",$yarn_data);
				foreach ($yarn_data_arr as  $yval) 
				{
					$yarnData_details = explode("_",$yval);
					if ($modified_yarnData == "") {
						$modified_yarnData = $yarnData_details[0]*1 . "_" . $yarnData_details[1]*1 . "_" . $yarnData_details[2]*1 . "_" . $yarnData_details[3]*1 . "_" . $yarnData_details[4]*1 . "_" . "" . "_" . "" . "_" . $yarnData_details[7]*1 . "_" . $yarnData_details[8]*1 . "_" . "" . "_" . "";
					}
					else{
						$modified_yarnData .= "|" . $yarnData_details[0]*1 . "_" . $yarnData_details[1]*1 . "_" . $yarnData_details[2]*1 . "_" . $yarnData_details[3]*1 . "_" . $yarnData_details[4]*1 . "_" . "" . "_" . "" . "_" . $yarnData_details[7]*1 . "_" . $yarnData_details[8]*1 . "_" . "" . "_" . "";
					}
		...
		{
			$modified_yarnData = $saveDataArr[$row[csf('determination_id')]][$row[csf('gsm_weight')]];
		}

		//echo $modified_yarnData."<br>";
		...
		<tr class="general" id="tr_<? echo $i; ?>">
			...
			<td>
				...
				<input type="hidden" name="yarnData[]" id="yarnData_<? echo $i; ?>" value="<? echo $modified_yarnData;//$saveDataArr[$row[csf('determination_id')]][$row[csf('gsm_weight')]]; ?>" class="text_boxes">
				<input type="hidden" name="fsoDtlsIds[]" id="fsoDtlsIds_<? echo $i; ?>" value="<? echo $row[csf('dtls_ids')]; ?>" class="text_boxes">
			</td>
		</tr>
		<?
		$total_qnty +=number_format($row[csf('grey_qty')], 4, '.', '');
	}
	echo "##" . count($data_array_yarn)."##".$total_qnty."##".$is_yarn_part_saved;
	exit();
}
...
if ($action == "save_update_delete_yarn") {
	...
	if ($operation == 0)  // Insert Here
	{
		...
		$field_array_dtls = "id, mst_id, deter_id, gsm, color_range_id, grey_qty, yarn_data, fso_dtls_ids, inserted_by, insert_date";
		...
		for ($i = 1; $i <= $total_row; $i++) {
			...
			if ($data_array_dtls != "") $data_array_dtls .= ",";
			$data_array_dtls .= "(" . $id_dtls . "," . $update_id . ",'" . $$fabricDescIdY . "','" . $$txtFabricGsmY . "','" . $$cboColorRangeY . "','" . str_replace(",", '', $$txtGreyQtyY) . "','" . $$yarnData ."','".$$fsoDtlsIds. "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
		...
		//oci_rollback($con);
		//echo "10**insert into fabric_sales_order_yarn (".$field_array_dtls.") values ".$data_array_dtls;oci_rollback($con);die;
	...
	} else if ($operation == 1)   // Update Here
	{
		...
		$field_array_dtls = "id, mst_id, deter_id, gsm, color_range_id, grey_qty, yarn_data, fso_dtls_ids, inserted_by, insert_date,update_sl";
		...
		for ($i = 1; $i <= $total_row; $i++) {
			...
			$id_dtls = return_next_id_by_sequence("FABRIC_SALES_ORDER_YARN_PK_SEQ", "fabric_sales_order_yarn", $con);
			if ($data_array_dtls != "") $data_array_dtls .= ",";
			$data_array_dtls .= "(" . $id_dtls . "," . $update_id . ",'" . $$fabricDescIdY . "','" . $$txtFabricGsmY . "','" . $$cboColorRangeY . "','" . $$txtGreyQtyY . "','" . $$yarnData . "','" .$$fsoDtlsIds."',". $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "',$update_serial)";
...
<table width="100%" border="1" cellpadding="3" rules="all" style="font: 12px tahoma;">
	...
	<table border="1" rules="all" cellpadding="3" style="font-size: 12px;">
		...
		<?
		$data_array = sql_select("select id, terms from  wo_booking_terms_condition where booking_no='$salesOrderNo' and entry_form=472");
		if (count($data_array) > 0) {
			$i = 0;
			foreach ($data_array as $row) {
				$i++;
			...
			<fieldset>
				<form id="termscondi_1" autocomplete="off">
					...
					<table width="650" cellspacing="0" class="rpt_table" border="0" id="tbl_termcondi_details" rules="all">
						...
						<tbody>
							<?
						$data_array = sql_select("select id, terms from  wo_booking_terms_condition where booking_no=$txt_job_no and entry_form=472");// quotation_id='$data'
						if (count($data_array) > 0) {
							$i = 0;
							foreach ($data_array as $row) {
								$i++;
							...
							$data_array = sql_select("select id, terms from  lib_terms_condition where is_default=1 and entry_form=472");// quotation_id='$data'
							if (count($data_array) > 0) {
								foreach ($data_array as $row) {
									$i++;
									?>
...
if ($action == "collar_cuff_masurement_popup") 
{
	echo load_html_head_contents("Process Name Info", "../../", 1, 1, '', '1', '');
	extract($_REQUEST);
		//echo $bodypart_type;die;
	?>
	<script>
	var bodypart_type = '<? echo $bodypart_type;?>'
		function window_close()
		{
			var ccmp_save_data ="";

			
        	var	txtCollarGmtSize ="";
        	var	txtCollarExP="";
        	var	txtCollarSize="";
        	var	txtCollarQnty="";
			...
			$("#tbl_list_search").find('tbody tr').each(function () 
			{
				if(bodypart_type ==40)
				{
					txtCollarSize = $(this).find('input[name="txtCollarSize[]"]').val();
		            txtCollarQnty = $(this).find('input[name="txtCollarQnty[]"]').val();
		            txtCollarGmtSize = $(this).find('input[name="txtCollarGmtSize[]"]').val();
		            txtCollarExP = $(this).find('input[name="txtCollarExP[]"]').val();
	        	}
	        	else
	        	{
	        		txtCuffSize = $(this).find('input[name="txtCuffSize[]"]').val();
	            	txtCuffQnty = $(this).find('input[name="txtCuffQnty[]"]').val();
	            	txtCuffGmtSize = $(this).find('input[name="txtCuffGmtSize[]"]').val();
	            	txtCuffExP = $(this).find('input[name="txtCuffExP[]"]').val();
	        	}
	    ...
	    function calculate_total()
	    {
			...
			else
			{
				$("#total_cuff_qnty").text(total_cuff);
				$("#total_cuff_qnty_with_perc").text(total_cuff_with_perc);
				$("#grand_collar_qnty").val(total_cuff_with_perc);
				$("#hdn_total_collar_qnty_without_ex_percent").val(total_cuff);
			}
			...
			var txt_collercuff_con = $("#txt_collercuff_con").val()*1;
			//var grand_cons =(txt_collercuff_con/12)*total_collar_with_perc;
			var grand_cons =(txt_collercuff_con/12)*$("#grand_collar_qnty").val()*1;
		 	grand_cons = Math.round(grand_cons);
		 	$("#grand_cons").val(grand_cons);
	    }
			...
			<tfoot>
				<tr>
					...
					if ($bodypart_type ==40) 
					{
						$title="Collar";
						?>
					<th width="260">Total Qty.</th>
					<th width="130" id="total_collar_qnty">&nbsp;</th>
					<th width="130">&nbsp;</th>
					<th width="130" id="total_collar_qnty_with_perc">&nbsp;</th>
						<?
					}
					else
					{
						...
						?>
					<th width="260">Total Qty.</th>
					<th width="130" id="total_cuff_qnty">&nbsp;</th>
					<th width="130">&nbsp;</th>
					<th width="130" id="total_cuff_qnty_with_perc">&nbsp;</th>
						<?
					}
					?>
				...
				<tr>
					...
					<th colspan="2">Total <? echo $title;?> Qty.</th>
					<th><input type="text" id="grand_collar_qnty" name="grand_collar_qnty" class="text_boxes_numeric" onKeyUp="" width="40" disabled readonly></th>
					<th>&nbsp;</th>
				</tr>
				<tr>
					<th colspan="2">&nbsp;</th>
					<th colspan="2"><? echo $title;?> Cons/ Dzn</th>
					<th>
						<input type="text" id="txt_collercuff_con" name="txt_collercuff_con" class="text_boxes_numeric" onKeyUp="calculate_total();" width="40" value="<? echo $hdnCollerCuffCon;?>">
						<input type="hidden" id="hdn_total_collar_qnty_without_ex_percent" name="hdn_total_collar_qnty_without_ex_percent" class="text_boxes_numeric" onKeyUp="calculate_total();" width="40" value="<? echo $hdnCollerCuffCon;?>"> 
					</th>
...
if ($action == "fabric_sales_order_print6") 
{
	...
	if($is_approved==1)  $approval_msg_td= 'This Sales Order is Approved';
	else if($is_approved==3)  $approval_msg_td='This Sales Order is Partially Approved';
	else  $approval_msg_td="";
	...
	style="font: 12px tahoma; border-bottom: 1px solid #999; margin-bottom: 2px;">
		<tr>
			<td rowspan="3" width="250" style="font-size:18px; color:#F00";><? echo $approval_msg_td; ?></td>
			<td colspan="4" align="center"><strong style="font-size:20px;"><? echo $companyArr[$companyId]; ?></strong></td>
			<td width="250">&nbsp;</td>
		</tr>
		<tr>
			<td colspan="4" align="center"><strong><? echo $addressArr[$companyId]; ?></strong></td>
			<td width="250">&nbsp;</td>
		</tr>
		<tr>
			<td colspan="4" align="center"><strong><? echo $formCaption; ?></strong></td>
			<td width="250">&nbsp;</td>
		</tr>
	</table>

	<div align="right" style="color: red;font: 20px tahoma;"><b><? echo ($order_nature) ? $fbooking_order_nature[$order_nature]." Order" : "Color-Up";?></b></div>
	<table width="100%" border="1" cellpadding="3" rules="all" style="font: 12px tahoma;">
		<tr>
		</tr>
		<tr>
		...
		<tr>
			...
			<td><strong>Currency</strong></td>
			<td><? echo $currency[$currency_id]; ?></td>
			<td><strong>Garments Marchant</strong></td>
			<td><? echo $garments_marchant; ?></td>
		</tr>
		<tr>
		...
		<tr>
			...
			//$nameArray_approved = sql_select("select max(b.approved_no) as approved_no, max(b.approved_date) as last_approved_date from approval_history b where mst_id='" . $sales_id . "' and b.entry_form=24 and un_approved_date is not null");

			$nameArray_approved = sql_select("select max(approved_no) - 1 as approved_no, max(update_date) as last_approved_date from fabric_sales_order_mst_his where sales_ordwe_id = '" . $sales_id . "' and status_active=1");

			echo $nameArray_approved[0][csf('approved_no')]; ?></td>
			<td><strong>Last Revise Date</strong></td>
			<td><? if ($update_date) echo $last_update_date=date('d-m-Y h:i:s A', strtotime($update_date));//change_date_format($nameArray_approved[0][csf('last_approved_date')]);?></td>
			<td><strong>Update Serial</strong></td>
			<td>
	...
	$collor_cuff_sql= sql_select("SELECT a.coller_size, a.coller_qnty, a.cuff_size, a.cuff_qnty, a.coller_gmt_size, a.coller_ex_percent, a.cuff_gmt_size, a.cuff_ex_percent, b.color_id,b.body_part_id 
	from fabric_sales_order_coller_cuff a, fabric_sales_order_dtls b  where a.dtls_id=b.id and a.mst_id=b.mst_id and  a.mst_id= $sales_id and a.status_active=1 and a.is_deleted=0");


	$dtls_sql = "SELECT b.id, b.width_dia_type,b.pre_cost_remarks,b.body_part_id, b.color_type_id,b.fabric_desc,b.determination_id,b.gsm_weight,b.after_wash_gsm, b.dia,b.cuttable_dia, b.color_id, sum(b.finish_qty) finish_qty, sum(b.pp_qnty) as pp_qnty, sum(b.mtl_qnty) as mtl_qnty, sum(b.fpt_qnty) as fpt_qnty, sum(b.gpt_qnty) as gpt_qnty, b.process_loss, b.work_scope, sum(b.grey_qty) grey_qty,sum(b.rmg_qty) as rmg_qty, b.pre_cost_fabric_cost_dtls_id,a.within_group,b.process_id,b.process_seq, b.ply, b.lab_dip_no, SUM(b.adjust_grey_qnty) as adjust_grey_qnty, sum(b.grey_qnty_by_uom) as booking_qty,b.color_range_id 
	from fabric_sales_order_mst a,fabric_sales_order_dtls b
	where a.id=$sales_id and a.job_no ='$salesOrderNo' and a.id=b.mst_id and a.status_active=1 and b.is_deleted=0
	group by b.id, b.body_part_id,b.color_type_id,b.fabric_desc,b.determination_id,b.work_scope, b.gsm_weight,b.after_wash_gsm,b.width_dia_type, b.dia,b.cuttable_dia, b.process_loss, b.color_id,b.pre_cost_remarks, b.pre_cost_fabric_cost_dtls_id,a.within_group,b.process_id,b.process_seq,b.ply, b.lab_dip_no,b.id,b.sl_no,b.color_range_id order by b.sl_no";
	...
	foreach ($dtls_sql_data as $rows) 
	{
		...
		$sample_qty_arr[$key][$rows[csf('color_id')]][$rows[csf('lab_dip_no')]][$rows[csf('process_loss')]]["adjust_grey_qnty"] += $rows[csf('adjust_grey_qnty')];
	...
	$fab_desc_arr = return_library_array("select a.id, (a.construction || ', ' || b.fabric_composition_name) as fab_desc  from lib_yarn_count_determina_mst a left join lib_fabric_composition b on a.fabric_composition_id=b.id where  a.fab_nature_id=2 and a.status_active=1 and a.is_deleted=0 and a.id in (". $all_determination_id.")","id","fab_desc");
	...
	$imge_arr = return_library_array("SELECT master_tble_id, image_location from common_photo_library where form_name='fabric_sales_order_entry_v2' and file_type=1", 'master_tble_id', 'image_location');
	?>
	<table width="100%" border="1" cellpadding="3" rules="all" style="font: 12px tahoma;">
		<thead>								 									
			<th>Sl</th>
			<th>Body Part</th>
			<th>Color Type</td>
			<th>Fabrics Description</th>
			<th>GSM</th>
			<th>Dia/Width (Inch)</th>
			<th>Ply</th>
			<th>Process</th>
			<th>Color</th>
			<th>Color Range</th>
			...
			<th>FPT<br>/GPT</th>
			<th>Buying<br>/SMS</th>
			<th>Total Finish Fab. Qty</th>
			<th>Booking Qty [Grey]</th>
			<th>Process Loss %</th>
			<th>Adjust Grey Quantity</th>
			<th>Grey Fab. Qty</th>
			<th>Remarks</th>
		</thead>
		<tbody>
			...
			foreach ($dtls_sql_data as $rows) 
			{
				...
				$processName= '';
				$processId_array = explode(",", $rows[csf("process_id")]);
				foreach ($processId_array as $val) {
					if ($processName == "") $processName = $conversion_cost_head_array[$val]; else $processName .= "," . $conversion_cost_head_array[$val];
				}
				
				
				$process_seqArr=explode(",", $rows[csf('process_seq')]);
				
				$arrSeq=array();
				foreach ($process_seqArr as  $value) {
					$proSeArr=explode("_", $value);
					
					$arrSeq[$proSeArr[1]] .=$conversion_cost_head_array[$proSeArr[0]].",";
				}
				//var_dump($arrSeq);die;
				
				$proSeqAr=array_values($arrSeq);
				
				$proSeqAr=array_filter($proSeqAr);
				$processSeqName=implode(",", $proSeqAr);
				
				$bgcolor = ($sll % 2 == 0) ? "#E9F3FF" : "#FFFFFF";
				?>
					<tr bgcolor="<? echo $bgcolor; ?>">
						<td><? echo $sll; ?></td>
						<td><? echo $body_part[$rows[csf('body_part_id')]]; ?></td>
						<td><? echo $color_type[$rows[csf('color_type_id')]]; ?></td>
						<td><? echo chop($fab_desc_arr[$rows[csf('determination_id')]],", ");//echo $rows[csf('fabric_desc')]; ?></td>
						<td>
							<?  
							if($rows[csf('gsm_weight')]!=" "){echo "B/W-".$rows[csf('gsm_weight')];}if($rows[csf('after_wash_gsm')]=="" || $rows[csf('after_wash_gsm')]==0){echo "";}else{echo ", A/W-".$rows[csf('after_wash_gsm')];}
							?>
						</td>
						<td>
							<? 
							if($rows[csf('dia')]!=" "){echo "F/D-".$rows[csf('dia')];}if($rows[csf('cuttable_dia')]==""){echo "";}else{echo ", C/D-".$rows[csf('cuttable_dia')];}if($fabric_typee[$rows[csf('width_dia_type')]]!="" || $fabric_typee[$rows[csf('width_dia_type')]]>0){echo ", ".$fabric_typee[$rows[csf('width_dia_type')]];}
					 		?>
					 	</td>
						<td><? echo $rows[csf('ply')]; ?></td>
						<td>
							<? echo chop($processSeqName,',');// $processName;?>
						</td>
						<td><? echo $color_library[$rows[csf('color_id')]]; ?></td>
						<td><? echo  $color_range[$rows[csf('color_range_id')]]; ?></td> 

						<td width=""><?if($imge_arr[$rows[csf('id')]]){?><img src="../../<?  echo $imge_arr[$rows[csf('id')]]; ?>" width="40" height="40" border="2" /><?}?></td>

						<td><? echo $rows[csf('lab_dip_no')]; ?></td>
						<td align="right"><? echo number_format($rows[csf('finish_qty')],2); ?></td>
						<td align="right"><? echo number_format($rows[csf('pp_qnty')],2); ?></td>
						<td align="right"><? echo number_format($rows[csf('mtl_qnty')],2); ?></td>
						<td align="right"><? echo number_format($rows[csf('fpt_qnty')],2); ?></td>
						<td align="right"><? echo number_format($rows[csf('gpt_qnty')],2); ?></td>
						<td align="right"><? $totFinQnty=$rows[csf('finish_qty')]+$rows[csf('pp_qnty')]+$rows[csf('mtl_qnty')]+$rows[csf('fpt_qnty')]+$rows[csf('gpt_qnty')]; echo  $totFinQnty; ?></td>
						<td align="right" title="Total Finish Fab. Qty+(Total Finish Fab. Qty*Process Loss %)/100"><? echo number_format($totFinQnty+($totFinQnty*$rows[csf('process_loss')])/100,2); ?></td>
						<td align="right"><? echo number_format($rows[csf('process_loss')],2); ?></td>
						<td align="right"><? echo number_format($rows[csf('adjust_grey_qnty')],2); ?></td>
						<td align="right"><? echo round($rows[csf('grey_qty')]); ?></td>
						<td><? echo $rows[csf('pre_cost_remarks')]; ?></td>
					</tr>
				<?
				$sll++;

				$grdTotalFinis+=$rows[csf('finish_qty')];
				$grdTotal_pp+=$rows[csf('pp_qnty')];
				$grdTotal_mtl+=$rows[csf('mtl_qnty')];
				$grdTotal_fpt+=$rows[csf('fpt_qnty')];
				$grdTotal_gpt+=$rows[csf('gpt_qnty')];
				$grdTotal_finish+=$totFinQnty;
				$grdTotal_grey+=round($rows[csf('grey_qty')]);
				$grdTotal_booking_qty+=$totFinQnty+($totFinQnty*$rows[csf('process_loss')])/100;
				$adjust_grey_qnty+=round($rows[csf('adjust_grey_qnty')]);
			}
			?>
			<tfoot>
				<tr bgcolor="#EEEEEE">
					...
					<td align="right"><? echo number_format($grdTotalFinis,2); ?></td>
					<td align="right"><? echo number_format($grdTotal_pp,2); ?></td>
					<td align="right"><? echo number_format($grdTotal_mtl,2); ?></td>
					<td align="right"><? echo number_format($grdTotal_fpt,2); ?></td>
					<td align="right"><? echo number_format($grdTotal_gpt,2); ?></td>
					<td align="right"><? echo number_format($grdTotal_finish,2); ?></td>
					<td align="right"><? echo number_format($grdTotal_booking_qty,2); ?></td>
					<td></td>
					<td align="right"><? echo number_format($adjust_grey_qnty,2); ?></td>
					<td align="right"><? echo number_format($grdTotal_grey,2); ?></td>
					<td></td>
				</tr>
			</tfoot>
		</tbody>
	...
	$tnaSql="select a.JOB_NO,a.TASK_START_DATE,a.TASK_FINISH_DATE,a.TASK_NUMBER,b.TASK_SHORT_NAME from TNA_PROCESS_MST a,LIB_TNA_TASK b where a.TASK_NUMBER=b.TASK_NAME and b.TASK_TYPE=2 and a.TASK_TYPE=2 and a.JOB_NO='$job_no' AND plan_start_flag=1 AND plan_finish_flag=1 AND a.TASK_NUMBER IN(48,52,60,61,8,40,63,239) order by TASK_SEQUENCE_NO";
	//echo $tnaSql; 
	$tnaSqlResult = sql_select($tnaSql);
	foreach($tnaSqlResult as $row){
		$tnaDataArr[$row[TASK_NUMBER]][START]=change_date_format($row[TASK_START_DATE]);
		$tnaDataArr[$row[TASK_NUMBER]][FINISH]=change_date_format($row[TASK_FINISH_DATE]);
		$tnaDataArr[$row[TASK_NUMBER]][TASK_SHORT_NAME]=$row[TASK_SHORT_NAME];
	...
	<?
	$coller_qnty=0;$cuff_qnty=0;
	foreach ($collor_cuff_sql as $row) 
	{
		if($row[csf("coller_qnty")] >0)
		{
			$coller_qnty += 1;
		}
		if($row[csf("cuff_qnty")] >0)
		{
			$cuff_qnty += 1;
		}
	}
	...
	<div style="float: left; padding-right: 50px;">
		...
		<table border="1" rules="all" cellpadding="3" style="font: 12px tahoma;">
			<tr bgcolor="#CCCCCC">
				...
				<td align="center" colspan="6"><strong>Collar</strong></td>
			</tr>
			<tr>
				<td align="center"><strong>Color</strong></td>
			...
			foreach ($collor_cuff_sql as $row) 
			{
				if($row[csf("coller_gmt_size")] !="" && $row[csf("coller_size")] !="")
				{
				?>
				<tr>
					<td align="center"><strong><? echo $i;?></strong></td>
					<td align="center"><strong><? echo $color_library[$row[csf("color_id")]];?></strong></td>
	...
	?>
	<div style="float: left; padding-right: 50px;">
		<strong style="font: bold 12px tahoma;">Cuff  Measurement Info</strong>
		<table border="1" rules="all" cellpadding="3" style="font: 12px tahoma;">
			<tr bgcolor="#CCCCCC">
				<td align="center" width="40" rowspan="2"><strong>SL</strong></td>
				<td align="center" colspan="6"><strong>Cuff</strong></td>
			</tr>
			<tr>
				<td align="center"><strong>Color</strong></td>
				<td align="center"><strong>Gmt Size</strong></td>
			...
			foreach ($collor_cuff_sql as $row) 
			{
				if($row[csf("cuff_gmt_size")] !="" && $row[csf("cuff_size")] !="")
				{
				?>
				<tr>
					<td align="center"><strong><? echo $i;?></strong></td>
	...
	?>
	<div style="float: left; padding-right: 50px;">
		<strong style="font: bold 12px tahoma;">Hem  Measurement Info</strong>
		<table border="1" rules="all" cellpadding="3" style="font: 12px tahoma;">
			<tr bgcolor="#CCCCCC">
				<td align="center" width="40" rowspan="2"><strong>SL</strong></td>
				<td align="center" colspan="6"><strong>Hem</strong></td>
			</tr>
			<tr>
				<td align="center"><strong>Color</strong></td>
				<td align="center"><strong>Gmt Size</strong></td>
			...
			$i=1;
			$total_qnty = 0;
			$total_qnty_with_perc = 0;
			foreach ($collor_cuff_sql as $row) 
			{
				if($row[csf("body_part_id")] ==670 )
				{
				?>
				<tr>
					<!-- <td align="center"><strong><? echo $row[csf("coller_gmt_size")];?></strong></td>
					...
					<td align="center">
						<strong>
							<? 
							if($row[csf("coller_gmt_size")])
								echo $row[csf("coller_gmt_size")];
							else
							echo $row[csf("cuff_gmt_size")];
							?>
						</strong>
					</td>
					<td align="center">
						<strong>
							<? 
							if($row[csf("coller_size")])
								echo $row[csf("coller_size")];
							else
							echo $row[csf("cuff_size")];
							?>
						</strong>
					</td>
					<td align="center">
						<strong>
							<? 
							if($row[csf("coller_qnty")])
							echo round($row[csf("coller_qnty")]);
							else
							echo round($row[csf("cuff_qnty")]);
							?>
						</strong>
					</td>
					<td align="center">
						<strong>
							<? 
							if($row[csf("coller_ex_percent")])
							echo $row[csf("coller_ex_percent")];
							else
							echo $row[csf("cuff_ex_percent")];
							?>
						</strong>
					</td>
					<td align="center">
				...
				if($row[csf("coller_qnty")])
				{
					$total_qnty +=$row[csf("coller_qnty")];
					$total_qnty_with_perc +=round($row[csf("coller_qnty")] + ($row[csf("coller_qnty")]*$row[csf("coller_ex_percent")])/100);
				}
				else
				{
					$total_qnty +=$row[csf("cuff_qnty")];
					$total_qnty_with_perc +=round($row[csf("cuff_qnty")] + ($row[csf("cuff_qnty")]*$row[csf("cuff_ex_percent")])/100);
				}
			...
			<tr>
				<td align="center" colspan="4"><strong>Total</strong></td>
				<td align="center"><strong><? echo $total_qnty;?></strong></td>
				<td align="center"></td>
				<td align="center"><strong><? echo round($total_qnty_with_perc);?></strong></td>
			</tr>
		</table>
	</div>
...
<?
	...
	$approve_data_array=sql_select("select b.approved_by, min(b.approved_date) as approved_date from  approval_history b where  b.entry_form=24 and b.mst_id=$sales_id  group by  b.approved_by ");
	$unapprove_data_array=sql_select("select b.approved_by, b.un_approved_by, b.approved_date, b.un_approved_reason, b.un_approved_date, b.approved_no from approval_history b where b.mst_id=$sales_id and b.entry_form=24 order by b.approved_date, b.approved_by");
	if(count($approve_data_array)>0)
	{
		...
		<div style="margin-top:15px">
			<table border="1" rules="all" cellpadding="3" style="font: 12px tahoma; width:630px;" rules="all">
				<thead>
					<tr bgcolor="#CCCCCC">
						<th colspan="4"><strong style="font: bold 12px tahoma;">Approval Status</strong></th>
					</tr>
					<tr bgcolor="#CCCCCC">
						<th width="20">SL</th>
						<th width="150">Name</th>
						<th width="80">Designation</th>
						<th width="30">Approval Date</th>
					</tr>
				</thead>
				<? foreach ($approve_data_array  as $key => $value)
				{
					$approved_by=$user_lib_name_arr[$value[csf("approved_by")]];
					$designation=$lib_designation_arr[$user_lib_designation_arr[$value[csf("approved_by")]]];
					//$approved_date=explode(" ",$value[csf("approved_date")]);
					//$am_pm=$approved_date[2];

					$approved_date= date ("d-m-Y h:i:A",strtotime($value[csf('approved_date')]));
					?>
					<tr>
						<td width="20"><? echo $sl; ?></td>
						<td width="150"><? echo $approved_by; ?></td>
						<td width="80"><? echo $designation;  ?></td>
						<td width="30"><? echo $approved_date; ?></td>
					</tr>
					<?
					$sl++;
				}
	...
	if(count($unapprove_data_array)>0)
	{
		...
		<div style="margin-top:15px">
			<table border="1" rules="all" cellpadding="3" style="font: 12px tahoma; width:630px;" rules="all">
				<thead>
					<tr bgcolor="#CCCCCC">
						<th colspan="5"><strong style="font: bold 12px tahoma;">Approval / Un Approval History</strong></th>
					</tr>
					<tr bgcolor="#CCCCCC">
						<th width="20">SL</th>
						<th width="150">Name</th>
						<th width="80">Designation</th>
						<th width="30">Approval Status</th>
						<th width="30">Date</th>
					</tr>
				</thead>
				<? foreach ($unapprove_data_array  as $key => $row)
				{
					$approved_by=$user_lib_name_arr[$row[csf("approved_by")]];
					$designation=$lib_designation_arr[$user_lib_designation_arr[$row[csf("approved_by")]]];
					?>
					<tr>
						<td width="20"><? echo $i; ?></td>
						<td width="150"><? echo $approved_by; ?></td>
						<td width="80"><? echo $designation;  ?></td>
						<td width="30"><? echo 'Yes'; ?></td>
						<td width="30"><? if($row[csf('approved_date')]!="") echo date ("d-m-Y h:i:A",strtotime($row[csf('approved_date')])); else echo ""; ?></td>
...
if ($action == "fabric_sales_order_print7") // Merchant Print
{
	list($companyId, $bookingId, $bookingNo, $salesOrderNo, $formCaption, $input_number) = explode('*', $data);
	// echo $input_number.'====';die;
	$companyArr = return_library_array("select id,company_name from lib_company", 'id', 'company_name');
	$addressArr = return_library_array("select id,city from lib_company where id=$companyId", 'id', 'city');
	$buyerArr = return_library_array("select id,buyer_name from lib_buyer", 'id', 'buyer_name');
	$departmentArr = return_library_array("select id,department_name from lib_department", 'id', 'department_name');
	...
	$sql = "select d.id as sales_id, d.currency_id, d.delivery_start_date, d.delivery_date, d.within_group, d.job_no, d.buyer_id, d.booking_date, d.booking_approval_date, d.sales_booking_no, d.company_id, d.delivery_date, d.dealing_marchant, d.remarks, d.style_ref_no, d.season, d.attention, d.customer_buyer, d.customer_buyer_brand, d.season_year, d.ship_to, d.order_nature, d.from_fso_no, d.update_sl, d.is_approved from fabric_sales_order_mst d
	where d.status_active =1 and d.is_deleted =0 and d.job_no='$salesOrderNo' and entry_form=472";
	...
	<table width="100%" border="1" cellpadding="3" rules="all" style="font: 12px tahoma;">
		...
		<tr>
			...
			$add_days=$input_number;
			$delivery_start_add_days = strtotime("+$add_days day", strtotime($delivery_start_date));
			echo date('d-m-Y', $delivery_start_add_days);
			//echo change_date_format($delivery_start_date); 
			?></td>
			<td><strong>Delivery End Date</strong></td>
			<td><? 
			$delivery_end_add_days = strtotime("+$add_days day", strtotime($delivery_date));
			echo $delivery_end_date=date('d-m-Y', $delivery_end_add_days);
			//echo change_date_format($delivery_date); 
			?></td>
			<td><strong>Lead Time</strong></td>
			<td>
				<?
					echo datediff("d", $booking_date, $delivery_end_date)
				?>
			</td>
		</tr>
		<tr>
	...
	$dtls_sql = "select b.width_dia_type,b.pre_cost_remarks,b.body_part_id, b.color_type_id,b.fabric_desc,b.determination_id,b.gsm_weight,b.after_wash_gsm, b.dia,b.cuttable_dia, b.color_id, sum(b.finish_qty) finish_qty, sum(b.pp_qnty) as pp_qnty, sum(b.mtl_qnty) as mtl_qnty, sum(b.fpt_qnty) as fpt_qnty, sum(b.gpt_qnty) as gpt_qnty, b.process_loss, b.work_scope, sum(b.grey_qty) grey_qty,sum(b.rmg_qty) as rmg_qty, b.pre_cost_fabric_cost_dtls_id,a.within_group,b.process_id,b.process_seq, b.ply, b.lab_dip_no
	from fabric_sales_order_mst a,fabric_sales_order_dtls b
	where a.id=$sales_id and a.job_no ='$salesOrderNo' and a.id=b.mst_id and a.status_active=1 and b.is_deleted=0
	group by b.body_part_id,b.color_type_id,b.fabric_desc,b.determination_id,b.work_scope, b.gsm_weight,b.after_wash_gsm,b.width_dia_type, b.dia,b.cuttable_dia, b.process_loss, b.color_id,b.pre_cost_remarks, b.pre_cost_fabric_cost_dtls_id,a.within_group,b.process_id,b.process_seq,b.ply, b.lab_dip_no,b.id,b.sl_no order by b.sl_no";
	...
	<div style="float: left; padding-right: 50px;">					
		...
		<table border="1" rules="all" cellpadding="3" style="font: 12px tahoma;">
			<tr bgcolor="#CCCCCC">
				<td align="center" width="40" rowspan="2"><strong>SL</strong></td>
				<td align="center" colspan="5"><strong>Collar</strong></td>
			</tr>
			<tr>
				<td align="center"><strong>Gmt Size</strong></td>
				<td align="center"><strong>Item Size</strong></td>
...
if ($action == "fabric_sales_order_print8") 
{
	list($companyId, $bookingId, $bookingNo, $salesOrderNo, $formCaption) = explode('*', $data);
	$companyArr = return_library_array("select id,company_name from lib_company", 'id', 'company_name');
	$addressArr = return_library_array("select id,city from lib_company where id=$companyId", 'id', 'city');
	...
	$sql = "SELECT d.id as sales_id, d.currency_id, d.delivery_start_date, d.delivery_date, d.within_group, d.job_no, d.buyer_id, d.booking_date, d.booking_approval_date, d.sales_booking_no, d.company_id, d.delivery_date, d.dealing_marchant, d.remarks, d.style_ref_no, d.season, d.attention, d.customer_buyer, d.customer_buyer_brand, d.season_year, d.ship_to, d.order_nature, d.from_fso_no, d.update_sl, d.is_approved, d.garments_marchant from fabric_sales_order_mst d
	where d.status_active =1 and d.is_deleted =0 and d.job_no='$salesOrderNo' and entry_form=472";
	...
	$pi_sql=sql_select("SELECT a.pi_number, a.pi_date, a.attention, b.work_order_id as fso_id, b.work_order_dtls_id as fso_dtls_id, b.amount from com_export_pi_mst a, com_export_pi_dtls b where  a.id=b.pi_id and b.work_order_id=$sales_id and b.status_active=1 and a.status_active=1 and a.is_deleted=0 group by a.pi_number, a.pi_date, a.attention, b.work_order_id, b.work_order_dtls_id, b.amount");
	$pi_number_arr=array();
	foreach($pi_sql as $row)
	{
		$pi_number_arr[$row[csf('fso_id')]]['pi_number']=$row[csf('pi_number')];
		$pi_number_arr[$row[csf('fso_id')]]['pi_date']=$row[csf('pi_date')];
		$pi_number_arr[$row[csf('fso_id')]]['attention']=$row[csf('attention')];
		$pi_amount_arr[$row[csf('fso_dtls_id')]]['pi_amount']=$row[csf('amount')];
	}

	//$update_serial_yarn = return_field_value("update_sl", "fabric_sales_order_yarn", "mst_id='" . $sales_id . "'");
	?>
	<table width="100%" border="0" cellpadding="3" cellspacing="0" style="font: 12px tahoma; margin-bottom: 2px;">
		<tr>
			<td rowspan="3" width="250" style="font-size:18px; color:#F00";><? echo $approval_msg_td; ?></td>
			<td colspan="4" align="center"><strong style="font-size:20px;"><? echo $companyArr[$companyId]; ?></strong></td>
			<td width="250">&nbsp;</td>
		...
		<tr>
			<td colspan="4" align="center"><strong>Fabric Cost Breakdown</strong></td>
			<td width="250">&nbsp;</td>
		</tr>
	</table>
	...
	<table width="100%"  style="font: 12px tahoma;">
		...
		<tr>
			<td width="150"><strong>Cust. Buyer Name</strong></td>
			<td width="235"><? echo $buyerArr[$customer_buyer]; ?></td>
			<td width="170"><strong>PI No</strong></td>
			<td width="235"><? echo $pi_number_arr[$sales_id]['pi_number']; ?></td>
		</tr>
		<tr>
			<td><strong>Customer Name</strong></td>
			<td><? echo $buyerArr[$buyer_id]; ?></td>
			<td><strong>PI Date</strong></td>
			<td><? echo change_date_format($pi_number_arr[$sales_id]['pi_date']); ?></td>
		</tr>
		<tr>
			<td><strong>Sales Order No.</strong></td>
			<td><? echo $job_no; ?></td>
			<td><strong>Merchandiser/Manager</strong></td>
			<td><? echo $pi_number_arr[$sales_id]['attention']; ?></td>
		</tr>
		<tr>
			<td><strong>Sales Job / Booking No</strong></td>
			<td><? echo $sales_booking_no; ?></td>
			<td width="135"><strong>Currency</strong></td>
			<td><? echo $currency[$currency_id]; ?></td>
			<td width="135"></td>
			<td></td>
		</tr>		
	...
	<br/>
	<?
	$dtls_sql = "SELECT a.id as fso_id, a.within_group, b.id as fso_dtls_id, b.width_dia_type, b.pre_cost_remarks, b.body_part_id, b.color_type_id, b.fabric_desc, b.determination_id, b.gsm_weight, b.after_wash_gsm, b.dia, b.cuttable_dia, b.color_id, sum(b.finish_qty) finish_qty, sum(b.pp_qnty) as pp_qnty, sum(b.mtl_qnty) as mtl_qnty, sum(b.fpt_qnty) as fpt_qnty, sum(b.gpt_qnty) as gpt_qnty, b.process_loss, b.work_scope, sum(b.grey_qty) grey_qty, sum(b.rmg_qty) as rmg_qty, b.pre_cost_fabric_cost_dtls_id, b.process_id, b.process_seq, b.ply, b.lab_dip_no
	from fabric_sales_order_mst a, fabric_sales_order_dtls b
	where a.id=$sales_id and a.job_no ='$salesOrderNo' and a.id=b.mst_id and a.status_active=1 and b.is_deleted=0
	group by a.id, b.id, b.body_part_id,b.color_type_id,b.fabric_desc,b.determination_id,b.work_scope, b.gsm_weight,b.after_wash_gsm,b.width_dia_type, b.dia,b.cuttable_dia, b.process_loss, b.color_id,b.pre_cost_remarks, b.pre_cost_fabric_cost_dtls_id,a.within_group,b.process_id,b.process_seq,b.ply, b.lab_dip_no,b.id,b.sl_no order by b.sl_no";
	...
	$composition_arr = array();
	$sql_deter = "select a.id, a.construction, b.copmposition_id, b.percent from lib_yarn_count_determina_mst a, lib_yarn_count_determina_dtls b where a.id=b.mst_id and a.id in (". $all_determination_id.")";
	$data_array = sql_select($sql_deter);
	if (count($data_array) > 0) {
		foreach ($data_array as $row) {
			if (array_key_exists($row[csf('id')], $composition_arr)) {
	...
	//$yarn_sql = "SELECT b.mst_id, b.yarn_count_id, b.deter_id, b.composition_id, b.composition_perc, b.color_id, b.yarn_type, b.cons_ratio, b.unit_rate, sum(b.cons_qty) as cons_qty from fabric_sales_order_yarn_dtls b where b.status_active=1 and b.is_deleted=0 and b.mst_id in ($sales_id) group by b.mst_id, b.yarn_count_id, b.deter_id, b.composition_id, b.composition_perc, b.color_id, b.yarn_type, b.cons_ratio, b.unit_rate";
	$yarn_sql = " SELECT b.mst_id, b.yarn_count_id, b.deter_id, b.composition_id, b.composition_perc, b.color_id, b.yarn_type, b.cons_ratio, b.unit_rate, sum(b.cons_qty) as cons_qty,b.gsm,c.fso_dtls_ids
	from fabric_sales_order_yarn_dtls b,fabric_sales_order_yarn c
	where b.yarn_dtls_id=c.id and b.mst_id=c.mst_id and b.deter_id=c.deter_id and b.gsm=c.gsm and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  and b.mst_id in ($sales_id) 
	group by b.mst_id, b.yarn_count_id, b.deter_id, b.composition_id, b.composition_perc, b.color_id, b.yarn_type, b.cons_ratio, b.unit_rate,b.gsm,c.fso_dtls_ids";
	 //echo $yarn_sql;
	$yarn_info = sql_select($yarn_sql);
	foreach ($yarn_info as $row) {

		$yarn_desc_key = $yarnCount_arr[$row[csf('yarn_count_id')]].", ".$composition[$row[csf('composition_id')]]." ".$row[csf('composition_perc')]." % ".$yarn_type[$row[csf('yarn_type')]].','.$color_library[$row[csf('color_id')]];

		$salesDtlsWiseArr=explode(",",$row[csf('fso_dtls_ids')]);
		foreach ($salesDtlsWiseArr as $fsoDtlsID) 
		{
			$yarn_sales_description_array[$fsoDtlsID][$row[csf('deter_id')]][$yarn_desc_key][$row[csf('gsm')]]["cons_ratio"] += $row[csf('cons_ratio')];
			$yarn_sales_description_array[$fsoDtlsID][$row[csf('deter_id')]][$yarn_desc_key][$row[csf('gsm')]]["unit_rate"] += $row[csf('unit_rate')];
		}
	...
	/*echo "<pre>";
	print_r($yarn_sales_description_array);
	echo "</pre>";*/

	?>
	<div>
		<table class="rpt_table" border="1" rules="all" width="1100" cellpadding="1" cellspacing="0" id="tbl_list_search" align="left" style="font: 12px tahoma;">
			<thead bgcolor="#CCCCCC">
				<tr>
					...
					<th width="240">Yarn Count/Process</th>
					<th width="50">Feeding %</th>
					<th width="80">Yarn Qty.</th>
					<th width="50">Unit Price</th>
					<th width="80">Total cost With Profit</th>

					<th width="">Fabric Price in PI</th>
				</tr>
			</thead>
		</table>
		<div style="width:1120px;"  id="scroll_body">
			<table class="rpt_table" border="1" rules="all" width="1100" cellpadding="1" cellspacing="0" id="tbl_list_search" align="left" style="margin-bottom: 20px; font: 12px tahoma;">
				<tbody>
					<?
					$sll=1;
					foreach ($dtls_sql_data as $rows) 
					{
						...
						$process_seqArr=array_filter(explode(",", $rows[csf('process_seq')]));				
						$arrSeq=array();
						foreach ($process_seqArr as  $value) 
						{
							$proSeArr=explode("_", $value);
							...
							$arrSeq[$proSeArr[0]] .=$conversion_cost_head_array[$proSeArr[0]].",";
							$arrSeq_rate[$proSeArr[0]]=$proSeArr[2];
						}				
						$proSeqAr=array_values($arrSeq);				
						$proSeqAr=array_filter($proSeqAr);
						$processSeqName=implode(",", $proSeqAr);
						// echo "<pre>";print_r($arrSeq);

						$totFinQnty=$rows[csf('finish_qty')]+$rows[csf('pp_qnty')]+$rows[csf('mtl_qnty')]+$rows[csf('fpt_qnty')]+$rows[csf('gpt_qnty')];
						$pi_amount=$pi_amount_arr[$rows[csf('fso_dtls_id')]]['pi_amount'];
						
						if ($rows[csf('gsm_weight')]!="") $bw_gsm=', B/W GSM-'.$rows[csf('gsm_weight')];
						if ($rows[csf('after_wash_gsm')]!="") $aw_gsm=', A/W GSM-'.$rows[csf('after_wash_gsm')];
						if ($rows[csf('dia')]!="") $fdia=', F/Dia-'.$rows[csf('dia')];
						if ($rows[csf('cuttable_dia')]!="") $cdia=', C/Dia-'.$rows[csf('cuttable_dia')];

						$fabrication=$composition_arr[$rows[csf('determination_id')]].$bw_gsm.$aw_gsm.$fdia.$cdia;

						$bgcolor = ($sll % 2 == 0) ? "#E9F3FF" : "#FFFFFF";
						?>
						<tr bgcolor="<? echo $bgcolor; ?>">
							<td title="<? echo $rows[csf('fso_dtls_id')];  ?>" width="40"><? echo $sll; ?></td>
							<td title="<? echo $rows[csf('determination_id')]; ?>" width="210"><? echo $fabrication; ?></td>
							<td width="80" align="right"><? echo number_format($totFinQnty,2,".",""); ?></td>
							<td width="80" align="right"><? echo number_format($rows[csf('process_loss')],2,".",""); ?></td>
							<td width="80" align="right"><? echo number_format($rows[csf('grey_qty')],2,".",""); ?></td>

							<td width="500" title="<? echo $rows[csf('fso_dtls_id')]; ?>" colspan="5" >
								<table width="100%" class="rpt_table" border="1" rules="all" width="1100" cellpadding="1" cellspacing="0" style="font: 12px tahoma;">
									<?
									$yarn_sales_description = $yarn_sales_description_array[$rows[csf('fso_dtls_id')]][$rows[csf('determination_id')]];
									// echo "<pre>";print_r($yarn_sales_description);
									$total_yarn_qty=0;$total_yarn_profit_qty=0;
									foreach ($yarn_sales_description as $yarnDesc => $gsmValues)
									{
										foreach ($gsmValues as $gsmKey => $values)
										{
			    							if($yarn_sales_description[$yarnDesc] =="") $description = "";else $description =$yarnDesc;
											?>
											<tr>
												<td  width="240" align="left"><p><? echo $description;?></p></td>
												<td width="50" align="right"><? echo $values['cons_ratio'].'%'; ?></td>
												<td title="<? echo "(Total Grey Qty*Feeding % )/100" ?>" width="80" align="right"><? echo number_format(($rows[csf('grey_qty')]*$values['cons_ratio'])/100,2,".",""); ?></td>
												<td width="50" align="right"><? echo $values['unit_rate']; ?></td>
												<td title="<? echo "(Total Grey Qty*Feeding % )/100*Unit Price"; ?>" width="80" align="right"><? echo number_format(($rows[csf('grey_qty')]*$values['cons_ratio'])/100*$values['unit_rate'],2,".","");?></td>
											</tr>
											<?
											$total_yarn_qty += ($rows[csf('grey_qty')]*$values['cons_ratio'])/100;
											$total_yarn_profit_qty += ($rows[csf('grey_qty')]*$values['cons_ratio'])/100*$values['unit_rate'];
										}
									}
									?>
									<td width="240">Financial Cost(2% of Total Yarn Value)</td>
									<td width="50"></td>
									<td width="80" align="right"><? echo number_format($total_yarn_qty,2,".",""); ?></td>
									<td width="50" align="right"><? echo number_format($total_yarn_profit_qty*(2/100)/$total_yarn_qty,2,".",""); ?></td>
									<td width="80" align="right"><? echo $total_yarn_value=number_format($total_yarn_profit_qty*(2/100),2,".","");?></td>
									<?

									$total_profit=0;
									foreach ($arrSeq as $sub_process_id => $val)
									{
										$sub_process_rate=$arrSeq_rate[$sub_process_id];
										?>
										<tr>
											<td width="240" align="left"><p><? echo chop($val,',');?></p></td>
											<td width="50" align="right"></td>
											<td width="80" align="right"><? echo number_format($rows[csf('grey_qty')],2,".",""); ?></td>
											<td width="50" align="right"><? echo number_format($sub_process_rate,2,".",""); ?></td>
											<td width="80" align="right"><? echo number_format($rows[csf('grey_qty')]*$sub_process_rate,2,".","");?></td>
										</tr>
										<?
										$total_profit += $rows[csf('grey_qty')]*$sub_process_rate;
									}
									?>
								</table>
							</td>					
							<td width="" align="right"><? echo number_format($pi_amount,2,".",""); ?></td>
						</tr>
						<?
						$sll++;
						...
						<tr style="background: #E5E7E9;">
							...
							<td width="80" align="right"><strong><? echo number_format($totFinQnty,2,".",""); ?></strong></td>
							<td width="80" align="right"></td>
							<td width="80" align="right"><strong><? echo number_format($rows[csf('grey_qty')],2,".",""); ?></strong></td>
							<td width="240" align="right"></td>
							<td width="50" align="right"></td>
							<td width="80" align="right"></td>
							<td width="50" align="right"></td>
							<td width="80" align="right"><strong><? echo $tot_profit=number_format($total_profit+$total_yarn_profit_qty+$total_yarn_value,2,".",""); $tot_cost_value+=number_format($total_profit+$total_yarn_profit_qty+$total_yarn_value,2,".","") ?></strong></td>
							<td align="right"><strong><? echo number_format($pi_amount,2,".",""); ?></strong></td>
						</tr>
						<tr style="background: #E5E7E9;">
							...
							<td width="50" align="right"></td>
							<td width="80" align="right"></td>
							<td width="50" align="right"><strong>Per Kg</strong></td>
							<td width="80" align="right"><strong><? echo number_format($tot_profit/$totFinQnty,2); ?></strong></td>
							<td align="right"><strong><? echo number_format($pi_amount/$totFinQnty,2); ?></strong></td>
						</tr>
						<?
					}
					?>
					<!-- <tfoot>
						<tr bgcolor="#EEEEEE">
							...
							<td align="right"><? //echo number_format($grdTotal_FinQnty,2); ?></td>
							<td align="right"></td>
							<td align="right"><? //echo number_format($grdTotal_grey_qty,2); ?></td>
							<td align="right"></td>
							<td align="right"></td>
							<td align="right"></td>
							<td align="right"></td>
							<td align="right"><? //echo number_format($grdtotal_profit,2); ?></td>
							<td align="right"><? //echo number_format($grdTotal_pi_amount,2); ?></td>
						</tr>
					</tfoot> -->
				</tbody>
			</table>
	...
	<div style="width:1100px;" align="right" id="scroll_body">
		<table border="1" rules="all" cellpadding="3" style="font: 12px tahoma;">
			<tr bgcolor="#CCCCCC">
				<td align="center"><strong>Particulers</strong></td>
				<td align="center"><strong>Value</strong></td>
				<td align="center"><strong>Margin %</strong></td>
			</tr>
			
			<tr>
				<td align="center"><strong>PI Value</strong></td>
				<td align="right"><strong><? echo number_format($total_pi_amount,2,".",""); ?></strong></td>
				<td align="center"><strong></strong></td>
			</tr>
			<tr>
				<td align="center"><strong>Cost Value</strong></td>
				<td align="right"><strong><? echo number_format($tot_cost_value,2,".",""); ?></strong></td>
				<td align="center"><strong></strong></td>
			</tr>
			<tr>
				<td align="center"><strong>Margin Value</strong></td>
				<td align="right"><strong><? echo $total_marging=number_format($total_pi_amount-$tot_cost_value,2,".",""); ?></strong></td>
				<td align="right"><strong><? echo number_format(($total_marging/$tot_cost_value)*100,2,".",""); ?></strong></td>
			</tr>
		</table>
	</div>
			...
			for (var k = 1; k <= no_of_row; k++) 
     		{
     			if(adjustStr=='')
				{
					//adjustStr=$('#cboBodyPart_'+k).val()+'_'+$('#cboColorType_'+k).val()+'_'+$('#fabricDescId_'+k).val()+'_'+$('#txtFabricGsm_'+k).val()+'_'+$('#txtAfterFabricGsm_'+k).val()+'_'+$('#cboPly_'+k).val()+'_'+$('#txtFabricDia_'+k).val()+'_'+$('#txtFabricCuttableDia_'+k).val()+'_'+$('#cboDiaWidthType_'+k).val()+'_'+$('#cboColorRange_'+k).val()+'_'+$('#cboConsUom_'+k).val()+'_'+$('#txtTotalFinishQty_'+k).val()+'_'+$('#txtCurrentAdjustQty_'+k).val();
	...
	<div align="center">
		...
		<form name="searchprocessfrm_1" id="searchprocessfrm_1" autocomplete="off">
			...
			foreach ($fso_data as $row) 
			{
				++$i;
				$disable ="";
				if ($row[csf('determination_id')] ==$txtFabricDesc && $row[csf('gsm_weight')] ==$txtFabricGsm && $row[csf('after_wash_gsm')] ==$txtAfterFabricGsm )
				{
					$bgcolor = "#7af17a";
				}else{
					$bgcolor ="";
				...
				<tr id="tr_<? echo $i; ?>" bgcolor="<? echo $bgcolor;?>">
					...
					<td width="50">
						<input type="text" name="txtAfterFabricGsm[]" id="txtAfterFabricGsm_<? echo $i; ?>" style="width:45px" value="<? echo $row[csf('after_wash_gsm')]; ?>" disabled="disabled"/>
					</td>
					<td width="50">
						<?
						$plyArr = array(3=>3,4=>4,5=>5);
						echo create_drop_down("cboPly_".$i, 45, $plyArr, "", 1, "- Select -", $row[csf('ply')], "", "1", "", "", "", "", "", "", "cboPly[]");
						?>
					</td>
					<td width="50">
						<input type="text" name="txtFabricDia[]" id="txtFabricDia_<? echo $i; ?>" class="text_boxes"
					...
					<td width="50">
						<input type="text" name="txtFabricCuttableDia[]" id="txtFabricCuttableDia_<? echo $i; ?>" class="text_boxes" style="width:40px" disabled readonly/>
					</td>
					<td width="50">
						<?
...
if ($action == 'ld_approval_popup') 
{
	echo load_html_head_contents("Process Name Info", "../../", 1, 1, '', '1', '');
	extract($_REQUEST);
	?>
	<script>
		function fnc_fabric_ld_approval_entry(operation) 
		{
			if (operation == 2) {
				show_msg('13');
				return;
			...
			if (form_validation('fso_mst_id', 'Sales Order No') == false) {
				return;
			}

			var fso_mst_id = $("#fso_mst_id").val();
			...
			$("#tbl_list_search").find('tr').each(function () 
			{
				...
				var cboLdApproval = $(this).find('select[name="cboLdApproval[]"]').val();
				var cboFblApproval = $(this).find('select[name="cboFblApproval[]"]').val();
				var cboStrikeApproval = $(this).find('select[name="cboStrikeApproval[]"]').val();
				
				j++;
				dataString += '&fabricDescId' + j + '=' + fabricDescId + '&txtGsm' + j + '=' + txtGsm + '&txtDia' + j + '=' + txtDia + '&txtColorId' + j + '=' + txtColorId + '&cboLdApproval' + j + '=' + cboLdApproval + '&cboFblApproval' + j + '=' + cboFblApproval + '&cboStrikeApproval' + j + '=' + cboStrikeApproval;

			});

			var data = "action=save_update_delete_approval&operation=" + operation + get_submitted_data_string('fso_mst_id', "../../") + dataString + '&total_row=' + j;
			freeze_window(operation);
			http.open("POST", "fabric_sales_order_entry_v2_controller.php", true);
			http.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
			http.send(data);
			http.onreadystatechange = fnc_fabric_ld_approval_entry_Reply_info;
		}

		function fnc_fabric_ld_approval_entry_Reply_info() {
			if (http.readyState == 4) {
				...
				if ((response[0] == 0 || response[0] == 1)) {
					//set_button_status(1, permission, 'fnc_fabric_ld_approval_entry', 2);
					parent.emailwindow.hide();
				}
	...
	$saveDataArr = array();
	$saved_approval = sql_select("select deter_id, gsm, dia, color_id, ld_approval, fbl_approval, strike_approval from fabric_sales_order_approval where mst_id=$fso_mst_id and status_active=1 and is_deleted=0 ");

	if(!empty($saved_approval))
	{
		$is_yarn_part_saved = 1;
	}
	else
	...
	foreach ($saved_approval as $row) {
		$saveDataArr[$row[csf('deter_id')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('color_id')]]['ld_approval'] = $row[csf('ld_approval')];
		$saveDataArr[$row[csf('deter_id')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('color_id')]]['fbl_approval'] = $row[csf('fbl_approval')];
		$saveDataArr[$row[csf('deter_id')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('color_id')]]['strike_approval'] = $row[csf('strike_approval')];
	}


	$color_library = return_library_array("select id,color_name from lib_color where status_active=1 and is_deleted=0", "id", "color_name");

	$sql = "select determination_id, gsm_weight, dia,  color_id from fabric_sales_order_dtls where mst_id=$fso_mst_id and status_active=1 and is_deleted=0 group by determination_id, gsm_weight, dia,  color_id";
	//echo $sql;die;
	$data_array = sql_select($sql);
	$i = 1;
	?>
		...
		<form name="searchprocessfrm_1" id="searchprocessfrm_1" autocomplete="off">
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="755" class="rpt_table" align="left">
				<thead>
					...
					<th width="100">LD Approval</th>
					<th width="100">FBL/Dyelot/Bulk Hanger Approval</th>
					<th width="100">Strike Off Approval</th>
				</thead>
			</table>
			<div style="width:770px; overflow-y:scroll; max-height:280px; float:left;" id="buyer_list_view" align="left">
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="750" class="rpt_table" id="tbl_list_search" align="left">
		<?
		foreach ($data_array as $row) 
		{
			$ld_approval = $saveDataArr[$row[csf('determination_id')]][$row[csf('gsm_weight')]][$row[csf('dia')]][$row[csf('color_id')]]['ld_approval'];
			$fbl_approval = $saveDataArr[$row[csf('determination_id')]][$row[csf('gsm_weight')]][$row[csf('dia')]][$row[csf('color_id')]]['fbl_approval'];
			$strike_approval = $saveDataArr[$row[csf('determination_id')]][$row[csf('gsm_weight')]][$row[csf('dia')]][$row[csf('color_id')]]['strike_approval'];
			?>
			<tr id="tr_<? echo $i; ?>">
				<td>
					<p style="width:30px"><? echo $i;?></p>
				...
				<td >
					<input type="text" name="txtFabricDesc[]" id="txtFabricDesc_<? echo $i; ?>" class="text_boxes" style="width:180px" value="<? echo $composition_arr[$row[csf('determination_id')]]; ?>" disabled readonly/>
					<input type="hidden" name="fabricDescId[]" id="fabricDescId_<? echo $i; ?>" class="text_boxes" value="<? echo $row[csf('determination_id')]; ?>">
				</td>
				<td style="width:50px">
					<input type="text" name="txtGsm[]" id="txtGsm_<? echo $i; ?>" class="text_boxes" style="width:50px" value="<? echo $row[csf('gsm_weight')]; ?>" disabled="disabled"/>
				</td>
				<td style="width:50px">
					<input type="text" name="txtDia[]" id="txtDia_<? echo $i; ?>" class="text_boxes" 	style="width:50px" value="<? echo $row[csf('dia')]; ?>" disabled="disabled"/>
				</td>
				<td style="width:100px">
					<input type="text" name="txtColorName[]" id="txtColorName_<? echo $i; ?>" class="text_boxes" style="width:90px" value="<? echo $color_library[$row[csf('color_id')]]; ?>" disabled="disabled"/>
					<input type="hidden" name="txtColorId[]" id="txtColorId_<? echo $i; ?>" class="text_boxes" style="width:90px" value="<? echo $row[csf('color_id')]; ?>" />
				</td>
				<td>
					<?
						echo create_drop_down("cboLdApproval_" . $i, 100, $yes_no, "", 1, "-- Select --", "$ld_approval", "", "0", "", "", "", "", "", "", "cboLdApproval[]");

					?>
				</td>
				<td>
					<?
						echo create_drop_down("cboFblApproval_" . $i, 100, $yes_no, "", 1, "-- Select --", "$fbl_approval", "", "0", "", "", "", "", "", "", "cboFblApproval[]");

					?>
				</td>
				<td>
					<?
						echo create_drop_down("cboStrikeApproval_" . $i, 100, $yes_no, "", 1, "-- Select --", "$strike_approval", "", "0", "", "", "", "", "", "", "cboStrikeApproval[]");
			...
			<table width="700px">
				<tr>
					<td width="100%" align="center" colspan="7">
						<? echo load_submit_buttons($_SESSION['page_permission'], "fnc_fabric_ld_approval_entry", $is_yarn_part_saved, 0, "reset_form('','','','','')", 2); ?>
						<input type="hidden" id="fso_mst_id" name="fso_mst_id" value="<? echo $fso_mst_id;?>" />
					</td>
				</tr>
			</table>
		</form>
...
if ($action == "save_update_delete_approval") {
	$process = array(&$_POST);
	extract(check_magic_quote_gpc($process));
	...
	if ($operation == 0)  // Insert Here
	{
		...
		$field_array_dtls = "id, mst_id, deter_id, gsm, dia, color_id, ld_approval, fbl_approval, strike_approval, inserted_by, insert_date";
		for ($i = 1; $i <= $total_row; $i++) {
			$fabricDescId = "fabricDescId" . $i;
			$txtGsm = "txtGsm" . $i;
			$txtDia = "txtDia" . $i;
			$txtColorId = "txtColorId" . $i;
			$cboLdApproval = "cboLdApproval" . $i;
			$cboFblApproval = "cboFblApproval" . $i;
			$cboStrikeApproval = "cboStrikeApproval" . $i;

			$id_dtls = return_next_id_by_sequence("fabric_sales_approval_pk_seq", "fabric_sales_order_approval", $con);
			if ($data_array_dtls != "") $data_array_dtls .= ",";
			$data_array_dtls .= "(" . $id_dtls . "," . $fso_mst_id . ",'" . $$fabricDescId . "','" . $$txtGsm . "','" . $$txtDia . "','" . $$txtColorId . "','" . $$cboLdApproval ."','".$$cboFblApproval ."','".$$cboStrikeApproval. "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
		}
		
		$rID = sql_insert("fabric_sales_order_approval", $field_array_dtls, $data_array_dtls, 0);
		//echo "10**insert into fabric_sales_order_approval (".$field_array_dtls.") values ".$data_array_dtls;oci_rollback($con);die;
	...
	} else if ($operation == 1)   // Update Here
	{
		...
		$rID = execute_query("delete from fabric_sales_order_approval where mst_id=$fso_mst_id", 0);
		$rID2 = sql_insert("fabric_sales_order_approval", $field_array_dtls, $data_array_dtls, 1);
