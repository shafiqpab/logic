...
if($action=="load_drop_down_location_and_floor")
{
...
if ($action=="load_drop_down_line__")
{
	list($company_id,$location,$floor,$issue_date)=explode("_",$data);
	$nameArray= sql_select("select id, auto_update from variable_settings_production where company_name='$company_id' and variable_list=23 and status_active=1 and is_deleted=0");
	$prod_reso_allocation = $nameArray[0][csf('auto_update')];
	...
	if($prod_reso_allocation==1)
	{
		...
		if($db_type==0)
		{
			$sql="SELECT a.id, a.line_number,c.sewing_line_serial from prod_resource_mst a, prod_resource_dtls b,lib_sewing_line c where a.id=b.mst_id and a.line_number=c.id and a.is_deleted=0 and b.is_deleted=0 $cond group by a.id, a.line_number,a.prod_resource_num,c.sewing_line_serial  order by c.sewing_line_serial";
			$line_data=sql_select($sql);
		}
		else if($db_type==2 || $db_type==1)
		{
			$sql="SELECT a.id, a.line_number,c.sewing_line_serial from prod_resource_mst a, prod_resource_dtls b,lib_sewing_line c where a.id=b.mst_id and a.line_number=c.id and a.is_deleted=0 and b.is_deleted=0 $cond group by a.id, a.line_number,a.prod_resource_num,c.sewing_line_serial  order by c.sewing_line_serial";
...
if ($action == "load_variable_settings") {
    ...
    $replace_field_disable = return_field_value("sewing_production", "variable_settings_production","variable_list = 68 and company_name=$data and status_active=1");
    if($replace_field_disable!=1)
    {
    	// echo "$('#defect_is_disable').css('display','none');\n";
    }
...
if($action=="load_drop_down_wo_company")
{
    $user_id = $_SESSION['logic_erp']["user_id"];
	...
	if($data==3)
	{
		...
		echo create_drop_down( "cbo_emb_company", 180, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1 $working_credential_cond order by company_name","id,company_name", 1, "--- Select ---", $selected_company, "load_drop_down( 'requires/bundle_wise_sewing_output_controller', this.value, 'load_drop_down_location', 'location_td' );load_html();",0,0 );
	else
		echo create_drop_down( "cbo_emb_company", 180, $blank_array,"", 1, "--- Select ---", $selected, "load_drop_down( 'requires/bundle_wise_sewing_output_controller', this.value, 'load_drop_down_location', 'location_td' );load_html();",0 );

	exit();
}
if($action=="load_drop_down_wo_company_new")
{
    $user_id = $_SESSION['logic_erp']["user_id"];
    //========== user credential start ========
    $userCredential = sql_select("SELECT WORKING_UNIT_ID, unit_id as company_id, store_location_id, item_cate_id, company_location_id as location_id FROM user_passwd where id=$user_id");
	...
	if($data==3)
	{
		...
		{			
			$sql =" SELECT a.id, a.supplier_name		FROM lib_supplier a, lib_supplier_party_type b	   WHERE a.id = b.supplier_id AND b.party_type IN(22, 23) AND a.status_active = 1	  	GROUP BY a.id, a.supplier_name	UNION ALL SELECT c.id, c.supplier_name		FROM lib_supplier c, lib_supplier_party_type b , pro_gmts_delivery_mst a  	   WHERE c.id = b.supplier_id AND b.party_type in(22, 23) AND   c.id = a.serving_company 		 AND c.status_active IN (1, 3)	GROUP BY c.id, c.supplier_name 	ORDER BY supplier_name  ";		echo create_drop_down( "cbo_emb_company", 180, "$sql","id,supplier_name", 1, "--Select--", 0, "load_drop_down( 'requires/bundle_wise_sewing_output_controller', this.value, 'load_drop_down_location', 'location_td' );load_html();" );
		}
	}
	else if($data==1)
		echo create_drop_down( "cbo_emb_company", 180, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1 $working_credential_cond order by company_name","id,company_name", 1, "--- Select ---", $selected_company, "load_drop_down( 'requires/bundle_wise_sewing_output_controller', this.value, 'load_drop_down_location', 'location_td' );load_html();",0,0 );
...
if($action=="show_cost_details")
{
	...
	// $sqlResult =sql_select("SELECT a.po_break_down_id, b.po_number,a.country_id,a.item_number_id, a.cost_of_fab_per_pcs,a.cut_oh_per_pcs,a.trims_cost_per_pcs,a.cost_per_pcs from pro_garments_production_mst a,wo_po_break_down b,lib_country c where b.id=a.po_break_down_id and a.country_id=c.id and a.delivery_mst_id='$sys_id' and a.status_active=1 and a.is_deleted=0 and a.production_type=5");

	$sqlResult =sql_select("SELECT b.id as po_id,b.po_number,c.item_number_id,c.color_number_id, a.cost_of_fab_per_pcs,a.cut_oh_per_pcs,a.cost_per_pcs,a.fab_rate_per_pcs,a.trims_cost_per_pcs,a.bundle_no from pro_garments_production_dtls a,WO_PO_COLOR_SIZE_BREAKDOWN c,wo_po_break_down b,lib_country d where a.COLOR_SIZE_BREAK_DOWN_ID=c.id and b.id=c.po_break_down_id  and c.country_id=d.id and a.delivery_mst_id='$sys_id' and a.status_active=1 and a.is_deleted=0 and a.production_type=5");// and a.embel_name=2
	...
	foreach ($sqlResult as $v) 
	{
		...
		$bndle_no_arr[$v['BUNDLE_NO']] = $v['BUNDLE_NO'];
	...
	$bndle_nos = "'".implode("','",$bndle_no_arr)."'";

	// $sqlRes =sql_select("SELECT a.po_break_down_id,a.country_id,a.item_number_id, a.cost_of_fab_per_pcs,a.cut_oh_per_pcs,a.trims_cost_per_pcs,a.cost_per_pcs from pro_garments_production_mst a,wo_po_break_down b,lib_country c where b.id=a.po_break_down_id and a.country_id=c.id  and a.status_active=1 and a.is_deleted=0 and a.production_type=4 $poIds $itmIds $colorIds");

	$sqlResult =sql_select("SELECT b.id as po_id,b.po_number,c.item_number_id,c.color_number_id, a.cost_of_fab_per_pcs,a.cut_oh_per_pcs,a.cost_per_pcs,a.fab_rate_per_pcs,a.trims_cost_per_pcs from pro_garments_production_dtls a,WO_PO_COLOR_SIZE_BREAKDOWN c,wo_po_break_down b,lib_country d where a.COLOR_SIZE_BREAK_DOWN_ID=c.id and b.id=c.po_break_down_id  and c.country_id=d.id and a.status_active=1 and a.is_deleted=0 and a.production_type=4 and a.bundle_no in($bndle_nos) $poIds $itmIds $colorIds");// and a.embel_name=2
	$input_rate = array();
	foreach ($sqlResult as $v) 
	{
		$input_rate[$v['PO_ID']][$v['ITEM_NUMBER_ID']][$v['COLOR_NUMBER_ID']] = $v['COST_PER_PCS'];
	}
	...
	?>
 		<table cellpadding="0" cellspacing="0" width="700" border="1" rules="all" class="rpt_table">
			<thead>
				...
				<th width="90">Sewing Input Rate</th>
					...
					foreach ($po_data as $itm_id=>$itm_data) 
					{
						foreach ($itm_data as $color_id=>$v) 
						{
							...
							<tr bgcolor="<? echo $bgcolor;?>">
								...
								<td><?=$input_rate[$po_id][$itm_id][$color_id];?></td>
								<td><?=$v['trims_cost_per_pcs'];?></td>
								<td><?=$v['cut_oh_per_pcs'];?></td>
...
if($action=="create_po_search_list_view")
{
	...
	if(trim($company)!="") $sql_cond .= " and a.company_name='$company'";
 	...
 	$sql = "SELECT b.id, a.order_uom, a.buyer_name, a.company_name, a.total_set_qnty, a.set_break_down, a.job_no, a.style_ref_no, a.gmts_item_id, a.location_name, b.shipment_date, b.po_number, b.po_quantity, b.plan_cut
			...
			where
			a.id = b.job_id and a.id = c.job_id and a.status_active=1 and  a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.garments_nature=$garments_nature and c.emb_name=2
...
if ($action=="create_booking_search_list_view")
{
	...
	where a.id = b.mst_id  and b.po_id=d.id and c.id=d.job_id  $company $buyer $wo_date $wo_cond $style_cond $file_cond $po_number_cond $internal_ref_cond  and  a.status_active=1 and a.is_deleted=0 and b.rate_for=30 $job_cond
...
if($action=="bundle_popup_rescan")
{
	...
	$replace_field_disable_arr = return_library_array("select COMPANY_NAME,SEWING_PRODUCTION from VARIABLE_SETTINGS_PRODUCTION where VARIABLE_LIST = 68 and COMPANY_NAME=$company and status_active=1", "COMPANY_NAME", "SEWING_PRODUCTION");
	...
	<div align="center" style="width:100%;" >
		<form name="searchwofrm"  id="searchwofrm">
			...
			<legend>Enter search words 
                <input type="checkbox" value="1" name="is_exact" id="is_exact"  checked> is exact
                <? if($replace_field_disable_arr[$company]!=1){?>
                &nbsp;<input type="checkbox" value="1" name="is_defect" id="is_defect"> is defect 
                <? }else{ ?>
                    <input type="checkbox" value="1" name="is_defect" id="is_defect" style="display:none">                
                <? } ?>
            </legend>
...
if($action=="create_bundle_rescan_search_list_view")
{
	...
	if (trim($cut_no) == '' && trim($job_no) == '' && trim($bndl_no) == '' && trim($ex_data[0]) == '')
    {
        echo "<h2 style='color:#D00; text-align:center;'>Please enter search field value</h2>";
        exit();
    }
	if($cut_no!='')
	{
	...
	$sql="SELECT c.cut_no, c.bundle_no,c.barcode_no, d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, sum(c.production_qnty) as qty, e.po_number,a.sewing_line from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where d.job_id=f.id and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and a.company_id=$company and e.po_number like '$txt_order_no' and c.bundle_no like '$bundle_no' and c.production_type=4 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $jobCon $cutCon $line_id_cond $year_cond group by c.cut_no, c.bundle_no,c.barcode_no, d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, e.po_number,a.sewing_line order by  c.cut_no,length(c.bundle_no) asc, c.bundle_no asc";
	// echo $sql;die();
	$result = sql_select($sql);
	$barcode_arr = array();
    ...
    if(count($barcode_arr))
    {
    	$barcode_cond = where_con_using_array($barcode_arr,1,"c.barcode_no");
    }

   	if($is_defect=='false')
   	{ 
   		 $receive_sql="SELECT c.barcode_no, sum(c.production_qnty+c.alter_qty+c.spot_qty) as qty,sum(c.production_qnty) as production_qnty,a.sewing_line from pro_gmts_delivery_mst  a, pro_garments_production_dtls c where  a.id=c.delivery_mst_id  and c.production_type=5 and a.status_active=1 and a.is_deleted=0 $cutCon $bndlCon $barcode_cond and c.status_active=1 and c.is_deleted=0 group by c.barcode_no,a.sewing_line";
   	}
   	else
   	{
	...
	{
		if($replace_field_disable_arr[$company]==1 && $is_defect=='false'){
			$row[csf('qty')]=$row[csf('production_qnty')];
	...
	<div align="center" style="width:100%;" >
		<form name="searchwofrm"  id="searchwofrm">
			...
			<legend>Enter search words <input type="checkbox" value="1" name="is_exact" id="is_exact"  checked> is exact</legend>
	            <table cellpadding="0" cellspacing="0" width="670" border="1" rules="all" class="rpt_table">
	                <thead>
	                    <th>Cut Year</th>
	                    <th>Job No</th>
	                    <th>Style Ref</th>
	                    <th>Order No</th>
	                    <th class="must_entry_caption">Cut No</th>
	                    <th title="Sewing Input Challan">Challan No</th>
	                    <th>Bundle No</th>
	                    <th>
	                    	<input type="reset" name="reset" id="reset" value="Reset" style="width:100px" class="formbutton" />
	                        <input type="hidden" name="hidden_bundle_nos" id="hidden_bundle_nos">
	                ...
	                <tr class="general">
	                    ...
	                    <td><input type="text" name="txt_cut_no" id="txt_cut_no" style="width:120px" class="text_boxes" /></td>
	                    <td><input type="text" name="txt_challan_no" id="txt_challan_no" style="width:120px" class="text_boxes" /></td>
	                    <td><input type="text" name="bundle_no" id="bundle_no" style="width:120px" class="text_boxes" /></td>
	            		<td align="center">
	                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view (document.getElementById('txt_order_no').value+'_'+<? echo $company; ?>+'_'+document.getElementById('bundle_no').value+'_'+'<? echo trim($bundleNo,','); ?>'+'_'+document.getElementById('txt_job_no').value+'_'+document.getElementById('txt_cut_no').value+'_'+document.getElementById('cbo_cut_year').value+'_'+$('#is_exact').is(':checked')+'_'+'<? echo trim($line_id,','); ?>'+'_'+document.getElementById('txt_style_no').value+'_'+document.getElementById('txt_challan_no').value, 'create_bundle_search_list_view', 'search_div', 'bundle_wise_sewing_output_controller', 'setFilterGrid(\'tbl_list_search\',-1);reset_hide_field();')" style="width:100px;" />
	                     </td>
	                </tr>
	           </table>
	           <div style="width:100%; margin-top:5px; margin-left:10px" id="search_div" align="left"></div>
...
if($action=="create_bundle_search_list_view")
{
	...
	if (trim($cut_no) == '' && $job_no=="" && $order_no=="" && $bndl_no=="" && $style_ref=="" && $input_challan_no=="")
    {
        echo "<h2 style='color:#D00; text-align:center;'>Please enter search field value.</h2>";
        exit();
    }
    $style_cond = ''; $style_cond1 = '';
	if($cut_no!='')
	{
		if($is_exact=='true')
		{
	...
	if($style_ref!='')
    {
        if($is_exact=='true') 
		{
			$style_cond=" and f.style_ref_no = '$style_ref'";
			$style_cond1=" and style_ref_no = '$style_ref'";
		}
        else  
		{
			$style_cond=" and f.style_ref_no like '%$style_ref%'";
			$style_cond1=" and b.style_ref_no like '%$style_ref%'";
		}
	...
	if($job_no!='' || $style_ref!='')
    {
        if($is_exact=='true') 
		{
			...
			$po_arr=return_library_array( "select id, id from wo_po_break_down where job_no_mst='$job_no' $style_cond1",'id','id');
			//print_r($po_arr);
			$poIds = implode(",",$po_arr);
			// $jobCon_rescan = " and a.po_break_down_id in($poIds)";
			$jobCon_rescan = where_con_using_array($po_arr,0,"a.po_break_down_id");
		}
        else  
		{
			$jobCon=" and f.job_no like '%$job_no'";
			$po_arr=return_library_array( "SELECT a.id, a.id from wo_po_break_down a,wo_po_details_master b where a.job_id=b.id and b.job_no_prefix_num=$job_no $style_cond1",'id','id');			
			// $poIds = implode(",",$po_arr);
			// $jobCon_rescan = " and a.po_break_down_id in($poIds)";
			$jobCon_rescan = where_con_using_array($po_arr,0,"a.po_break_down_id");
		}
    ...
    if($input_challan_no)
    {
        $input_challan_cond .= " and a.challan_no='$input_challan_no' and a.production_type=4 ";
    } 
	...
	$bndlCon_scan = str_replace("c.bundle_no", "b.bundle_no", $bndlCon);
	// $scanne=sql_select( "SELECT b.bundle_no, sum(b.production_qnty) as production_qnty,a.sewing_line from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=5 and b.status_active=1 and b.is_deleted=0 $cutCon_a $jobCon_rescan $poCon_rescan $bndlCon_scan group by b.bundle_no,a.sewing_line");
	$scanne=sql_select( "SELECT b.bundle_no, b.production_qnty as production_qnty,a.sewing_line from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=5 and b.status_active=1 and b.is_deleted=0 $cutCon_a $jobCon_rescan $poCon_rescan $bndlCon_scan ");
	...
	$sql="SELECT c.cut_no, c.bundle_no,c.barcode_no, d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.bundle_no, sum(c.production_qnty) as qty, e.po_number,a.sewing_line, f.style_ref_no from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_id=f.id and a.company_id=$company $orderCon $bndlCon and c.production_type=4 and a.status_active=1 and a.is_deleted=0 $jobCon $style_cond $year_cond $cutCon $line_id_cond $jobCon_rescan $poCon_rescan $input_challan_cond group by c.cut_no, c.bundle_no,c.barcode_no, d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.bundle_no, e.po_number,a.sewing_line, f.style_ref_no order by  c.cut_no,length(c.bundle_no) asc, c.bundle_no asc";
            ...
            foreach ($result as $row)
            {
				...
				?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value(<? echo $i; ?>)">
						...
						<td width="90"><p><? echo $garments_item[$row[csf('item_number_id')]]; ?></p></td>
                        <td width="80"><p><? echo $country_arr[$row[csf('country_id')]]; ?></p></td>
                        <td width="60"><p><? echo $color_arr[$row[csf('color_number_id')]]; ?></p></td>
                        <td width="50"><p><? echo $size_arr[$row[csf('size_number_id')]]; ?></p></td>
                        <td width="80" id="td_line<? echo $i;?>" title="<? echo $row[csf('sewing_line')];?>">
                        <p>
                            <?
...
if($action=="load_mst_data")
{
	...
	if(count($sql_mst_data)>0)
	{
		...
		//	$cutting_no=return_field_value("cut_no", "pro_garments_production_dtls", "barcode_no in ($bundle_nos)");
			...
			echo "load_drop_down_multiple( 'requires/bundle_wise_sewing_output_controller', '".$val[csf('serving_company')]."_".$val[csf('location_id')]."_".$val[csf('floor_id')]."_".$data[2]."', 'load_drop_down_location_and_floor', 'location_td*line_td' );\n";
			...
			echo "$('#txt_sewing_input_date').val('".change_date_format($val[csf('delivery_date')])."');\n";
...
if($action=="challan_duplicate_check") // 18/05/2022
{
	$data=explode("__",$data);
	$bundle_no="'".implode("','",explode(",",$data[0]))."'";
...
if($action=="challan_duplicate_check__bk")
{
	$data=explode("__",$data);
	$bundle_no="'".implode("','",explode(",",$data[0]))."'";
	...
	foreach ($sql_mst_data as $row)
	{
 		if($data[2])
		{
 			...
 			{
 				$msg="Line Mixing Not Allow";
 				echo ($msg)."####".($datastr)."####".$datastrmst."####".$kk."####".$line;
 				disconnect($con);
 				die;
 			}
		}
	...
	if($msg !="") // Duplicate scan found
	{
		echo ($msg)."####".($datastr)."####".$datastrmst."####".$kk."####".$line;
		disconnect($con);
		exit();
	}
	// ================================== populate bundle data check =========================================
	$result=sql_select( "SELECT a.sewing_line from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=4  and b.status_active=1 and b.is_deleted=0 and b.barcode_no in ($bundle_no)");
	// $msg_type=1;
	if(count($result)>0)
	{
		foreach($result as $row)
		{
			if( $row[csf('sewing_line')] != trim($data[2]) && ($data[3]*1)>0  )
			{
				echo "####2####";
				disconnect($con);
				die();
	...
	if( $data[2]!=0 )
		$str_cond=" and a.sewing_line= $data[2]";
	else
		$str_cond="";

	$sql="SELECT max(c.id)  as prdid,d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, MAX($year_field) as year, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no,c.barcode_no, sum(c.production_qnty) as qty, e.po_number,a.sewing_line from pro_gmts_delivery_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name=$data[1] and a.id=c.delivery_mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_id=f.id and c.production_type=4 $str_cond and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active in(1,2,3) and d.is_deleted=0 and e.status_active in(1,2,3) and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and c.barcode_no in ($bundle_no) group by d.id, e.id, f.job_no_prefix_num, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no,c.barcode_no, e.po_number,a.sewing_line order by c.cut_no, length(c.bundle_no) asc, c.bundle_no asc";
	...
	{
		$balance_qnty= $row[csf('qty')]-$duplicate_bundle[$row[csf('bundle_no')]];
		if( $scanned_bundle_arr[$row[csf('sewing_line')]][$row[csf('bundle_no')]]=="" && $balance_qnty>0)
		{
			if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
...
if($action=="populate_bundle_data")
{
	...
	$replace_field_disable_arr = return_library_array("select COMPANY_NAME,SEWING_PRODUCTION from VARIABLE_SETTINGS_PRODUCTION where VARIABLE_LIST = 68 and status_active=1", "COMPANY_NAME", "SEWING_PRODUCTION");	
	
	 //echo "select COMPANY_NAME,SEWING_PRODUCTION from VARIABLE_SETTINGS_PRODUCTION where VARIABLE_LIST = 68 and COMPANY_NAME=$ex_data[3]";die;
	...
	$sql="SELECT max(c.id)  as prdid,d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, MAX($year_field) as year, f.buyer_name,f.style_ref_no, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no,c.barcode_no, sum(c.production_qnty) as qty, e.po_number,e.grouping,a.sewing_line,a.COMPANY_ID from pro_gmts_delivery_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name=$ex_data[3] and a.id=c.delivery_mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_id=f.id and c.production_type=4 $str_cond and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active in(1,2,3) and d.is_deleted=0 and e.status_active in(1,2,3) and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 $bundle_nos_cond group by d.id, e.id, f.job_no_prefix_num,f.style_ref_no, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no,c.barcode_no, e.po_number,e.grouping,a.sewing_line,a.COMPANY_ID order by c.cut_no, length(c.bundle_no) asc, c.bundle_no asc";
	...
	foreach ($result as $row)
	{
		...
		if( $scanned_bundle_arr[$row[csf('sewing_line')]][$row[csf('bundle_no')]]=="" && $curentscanned_bundle_arr[$row[csf('bundle_no')]]=='' && $balance_qnty>0)
		{
			...
			$readonly="";
			if($replace_field_disable_arr[$row['COMPANY_ID']]==1){
				$readonly="readonly";
			}
		
			$replace_field_disable=$replace_field_disable_arr[$row['COMPANY_ID']];
		...
		?>
			<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i;?>">
                ...
                <td width="70" align="center"><p><? echo $row[csf('style_ref_no')]; ?></p></td>
                <td width="65" style="word-break:break-all"><? echo $buyer_arr[$row[csf('buyer_name')]]; ?></td>
                ...
                <td width="50" id="rejQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="rejectQty[]" id="rejectQty_<? echo $i; ?>" style="width:35px" value="" onBlur="calculate_qcpasss(<? echo $i; ?>,<?=$replace_field_disable;?>)" onKeyPress="return numOnly(this,event,this.id);" onDblClick="pop_entry_reject(<? echo $i; ?>,<?=$replace_field_disable;?>)"/></td>
                <td width="50" id="altQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="alterQty[]" id="alterQty_<? echo $i; ?>" style="width:35px" value="" onBlur="calculate_qcpasss(<? echo $i; ?>,<?=$replace_field_disable;?>)" onKeyPress="return numOnly(this,event,this.id);" onDblClick="pop_entry_alter(<? echo $i; ?>)"/></td>
                <td width="50" id="sptQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="spotQty[]" id="spotQty_<? echo $i; ?>" style="width:35px" value="" onBlur="calculate_qcpasss(<? echo $i; ?>,<?=$replace_field_disable;?>)" onKeyPress="return numOnly(this,event,this.id);" onDblClick="pop_entry_spot(<? echo $i; ?>)"/></td>
                <td width="50" id="repQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="replaceQty[]" id="replaceQty_<? echo $i; ?>" style="width:35px" value="" onBlur="calculate_qcpasss(<? echo $i; ?>,<?=$replace_field_disable;?>)" onKeyPress="return numOnly(this,event,this.id);" <?=$readonly;?> /></td>

                <td width="50" id="qcQty_<? echo $i; ?>" align="right"><? echo $row[csf('qty')]; ?>&nbsp;</td>
				<td width="50" id="holdBndl_<? echo $i; ?>" align="right">
					<?
						echo create_drop_down( "cboHoldBundle_$i", 50, $bundle_hold_reason_array,"", 1, "-- Select --", $selected, "",0,0,"","","","","","cboHoldBundle[]" );
					?> 
				</td>
...
if($action=="populate_bundle_data_rescan")
{
	...
	$receive_sql="SELECT a.COMPANY_ID,c.barcode_no,c.bundle_no,sum(c.production_qnty+c.alter_qty+c.spot_qty)  as qty,sum(c.production_qnty)  as production_qnty,a.sewing_line from pro_gmts_delivery_mst a,pro_garments_production_dtls c where a.id=c.delivery_mst_id and c.production_type=5 and c.status_active=1 and c.is_deleted=0 $bundle_nos_cond $str_cond group by a.COMPANY_ID,c.bundle_no,c.barcode_no,a.sewing_line order by  length(c.bundle_no) asc, c.bundle_no asc";
	//echo $receive_sql;die;
	if($ex_data[5]=='true')
	{
	...
	$receive_result = sql_select($receive_sql);
	foreach ($receive_result as $row){
		if($replace_field_disable_arr[$row[COMPANY_ID]]==1 && $ex_data[5]!='true'){
			$row[csf('qty')]=$row[csf('production_qnty')];
		}
	...
	// 17990000010408,17990000010405
	 $sql="SELECT max(c.id)  as prdid,d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, MAX($year_field) as year, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no,c.barcode_no, sum(c.production_qnty) as qty, e.po_number,a.sewing_line,a.COMPANY_ID,f.style_ref_no, e.grouping from pro_gmts_delivery_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name=$ex_data[3] and a.id=c.delivery_mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_id=f.id and c.production_type=4 $str_cond and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active in(1,2,3) and d.is_deleted=0 and e.status_active in(1,2,3) and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 $bundle_nos_cond group by d.id, e.id, f.job_no_prefix_num, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no,c.barcode_no, e.po_number,a.sewing_line,a.COMPANY_ID ,f.style_ref_no, e.grouping order by  c.cut_no, length(c.bundle_no) asc, c.bundle_no asc";  //and is_rescan=1
	...
	foreach ($result as $row)
	{
		...
		?>
			<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i;?>">
                ...
                <td width="50" align="center"><? echo $row[csf('year')]; ?></td>
                <td width="60" align="center"><? echo $row[csf('job_no_prefix_num')]; ?></td>
                <td width="70" style="word-break:break-all;"><?= $row['GROUPING']; ?></td>
                <td width="70" style="word-break:break-all;"><?= $row['STYLE_REF_NO']; ?></td>
                <td width="65" style="word-break:break-all;"><? echo $buyer_arr[$row[csf('buyer_name')]]; ?></td>
                <td width="90" style="word-break:break-all;" align="left"><? echo $row[csf('po_number')]; ?></td>
                <td width="120" style="word-break:break-all;" align="left"><? echo $garments_item[$row[csf('item_number_id')]]; ?></td>
                <td width="100" style="word-break:break-all;" align="left"><? echo $country_arr[$row[csf('country_id')]]; ?></td>
                ...
                <td width="50" id="rejQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="rejectQty[]" <? echo $disabled;?> id="rejectQty_<? echo $i; ?>" style="width:35px" value="" onBlur="calculate_qcpasss(<? echo $i; ?>,<?=$replace_field_disable;?>)" onKeyPress="return numOnly(this,event,this.id);" onDblClick="pop_entry_reject(<? echo $i; ?>,<?=$replace_field_disable;?>)"/></td>
                <td width="50" id="altQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="alterQty[]" <? echo $disabled;?> id="alterQty_<? echo $i; ?>" style="width:35px" value="" onBlur="calculate_qcpasss(<? echo $i; ?>,<?=$replace_field_disable;?>)" onKeyPress="return numOnly(this,event,this.id);" onDblClick="pop_entry_alter(<? echo $i; ?>)"/></td>
                <td width="50"  id="sptQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="spotQty[]" <? echo $disabled;?> id="spotQty_<? echo $i; ?>" style="width:35px" value="" onBlur="calculate_qcpasss(<? echo $i; ?>,<?=$replace_field_disable;?>)" onKeyPress="return numOnly(this,event,this.id);" onDblClick="pop_entry_spot(<? echo $i; ?>)"/></td>
                <td width="50"  id="repQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="replaceQty[]" <? echo $disabled;?> id="replaceQty_<? echo $i; ?>" style="width:35px" value="<? //echo $rep_val;?>" onBlur="calculate_qcpasss(<? echo $i; ?>,<?=$replace_field_disable;?>)" onKeyPress="return numOnly(this,event,this.id);"  <?= $readonly;?>/></td>
...
if($action=="populate_bundle_data_update")
{
    ...
    // ============================== For Reject =================================
    $sql_bundle_reject=sql_select("SELECT a.defect_type_id,a.production_type,a.defect_point_id,a.defect_qty,a.bundle_no from  pro_gmts_prod_dft a where a.production_type=5 and defect_type_id =2 $rejbundle_nos_cond and status_active=1 and is_deleted=0");
	...
	$sql_bundle_reject=sql_select("SELECT a.defect_type_id,a.production_type,a.defect_point_id,a.defect_qty,a.bundle_no from  pro_gmts_prod_dft a where a.production_type=5 and defect_type_id =3 $rejbundle_nos_cond and status_active=1 and is_deleted=0");
    ...
    // ============================== For Spot =================================
	 ...
	 $sql_bundle_reject=sql_select("select a.defect_type_id,a.production_type,a.defect_point_id,a.defect_qty,a.bundle_no from  pro_gmts_prod_dft a where a.production_type=5 and defect_type_id =4 $rejbundle_nos_cond and status_active=1 and is_deleted=0");
	...
	$replace_field_disable = return_library_array("select COMPANY_NAME,SEWING_PRODUCTION from VARIABLE_SETTINGS_PRODUCTION where VARIABLE_LIST = 68 and COMPANY_NAME=$company_id and status_active=1", "COMPANY_NAME", "SEWING_PRODUCTION");
	...
	$sql="SELECT max(c.id)  as prdid, d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, $year_field as year, f.buyer_name,f.style_ref_no, d.item_number_id,c.barcode_no, d.country_id,c.reject_qty,c.alter_qty,c.spot_qty,c.replace_qty, d.size_number_id, d.color_number_id, c.cut_no,c.bundle_no,c.is_rescan, c.production_qnty as qty,c.bndl_hold_reason, e.po_number,e.grouping from pro_garments_production_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_id=f.id and a.delivery_mst_id=$mst_id and a.company_id=$company_id and c.production_type=5 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active in(1,2,3) and d.is_deleted=0 and e.status_active in(1,2,3) and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 $bundle_nos_cond group by d.id, e.id, f.job_no_prefix_num,f.insert_date, f.buyer_name,f.style_ref_no, d.item_number_id, d.country_id,c.reject_qty,c.alter_qty,c.spot_qty,c.replace_qty, d.size_number_id, d.color_number_id, c.cut_no,c.bundle_no,c.barcode_no,c.is_rescan, c.production_qnty,c.bndl_hold_reason, e.po_number,e.grouping order by c.cut_no, length(c.bundle_no) asc, c.bundle_no asc";  ///and f.company_name='$company_id'
	//echo $sql; die; entry_form
	$result = sql_select($sql);
	$count=count($result);
	$i=$ex_data[1]+$count;
	foreach ($result as $row)
	{
		...
		$readonly="";
		if($replace_field_disable[$company_id]==1){
			$sewing_in_qty = ($row[csf('qty')]+$row[csf('reject_qty')])-$row[csf('replace_qty')];
			$readonly="readonly";
		}
		else{
			$sewing_in_qty = ($row[csf('qty')]+$row[csf('reject_qty')]+$row[csf('alter_qty')]+$row[csf('spot_qty')])-$row[csf('replace_qty')];
		}
	...
	?>
		<tr bgcolor="<?= $bgcolor; ?>" id="tr_<?= $i;?>">
			...
			<td width="70" align="center"><p><?= $row[csf('grouping')]; ?></p></td>
			...
			<td width="80" id="prodQty_<?= $i; ?>" align="right"><?= $sewing_in_qty;?>&nbsp;</td>

			<?  $colo=''; if($bundle_alter_data[$row[csf('barcode_no')]]!='') $colo="  border-color:#FF0000";?>

			<td width="50" id="rejQty_<?= $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="rejectQty[]" id="rejectQty_<?= $i; ?>" style="width:35px; " value="<?= $row[csf('reject_qty')];?>" onBlur="calculate_qcpasss(<?= $i; ?>,<?=$replace_field_disable[$company_id];?>)" onKeyPress="return numOnly(this,event,this.id);" onDblClick="pop_entry_reject(<?= $i; ?>)"/></td>

			<td width="50" id="altQty_<?= $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="alterQty[]" id="alterQty_<?= $i; ?>" style="width:35px; <?= $colo; ?>" value="<?= $row[csf('alter_qty')];?>" onBlur="calculate_qcpasss(<?= $i; ?>,<?=$replace_field_disable[$company_id];?>)" onKeyPress="return numOnly(this,event,this.id);" onDblClick="pop_entry_alter(<?= $i; ?>)"/></td>
            <?  $colo=''; if($bundle_spot_data[$row[csf('barcode_no')]]!='') $colo="  border-color:#FF0000";?>
			<td width="50" id="sptQty_<?= $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="spotQty[]" id="spotQty_<?= $i; ?>" style="width:35px; <?= $colo; ?>" value="<?= $row[csf('spot_qty')];?>" onBlur="calculate_qcpasss(<?= $i; ?>,<?=$replace_field_disable[$company_id];?>)" onKeyPress="return numOnly(this,event,this.id);" onDblClick="pop_entry_spot(<?= $i; ?>)"/></td>
			<td width="50" id="repQty_<?= $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="replaceQty[]" id="replaceQty_<?= $i; ?>" style="width:35px" value="<?= $row[csf('replace_qty')];?>" onBlur="calculate_qcpasss(<?= $i; ?>,<?=$replace_field_disable[$company_id];?>)"  onKeyPress="return numOnly(this,event,this.id);"/></td>

			<td width="50" id="qcQty_<?= $i; ?>" align="right"><?= $row[csf('qty')]; ?>&nbsp;</td>
			<td width="50" id="holdBndl_<?= $i; ?>" align="right">
			 	<?= create_drop_down( "cboHoldBundle_$i", 50, $bundle_hold_reason_array,"", 1, "-- Select --", $row[csf('bndl_hold_reason')], "",0,0,"","","","","","cboHoldBundle[]" );?> 
			</td>
			<td id="button_1" align="center">

				<input type="hidden" name="cutNo[]" id="cutNo<?= $i; ?>" value="<?= $row[csf('cut_no')]; ?>"/>
				...
				<input type="hidden" name="qty[]" id="qty_<?= $i; ?>" value="<?= $row[csf('qty')]; ?>"/>
				<input type="hidden" name="prod_qty[]" id="prod_qty_<?= $i; ?>" value="<?= $row[csf('qty')]; ?>"/>
				<input type="hidden" name="dtlsId[]" id="dtlsId_<?= $i; ?>" value="<?= $row[csf('prdid')]; ?>"/>
				<input type="hidden" name="actual_reject[]" id="actual_reject_<?= $i; ?>" value="<?= $bundle_reject_data[$row[csf('barcode_no')]];?>"/>
				<input type="hidden" name="actual_alter[]" id="actual_alter_<?= $i; ?>" value="<?= $bundle_alter_data[$row[csf('barcode_no')]];?>"/>
				<input type="hidden" name="actual_spot[]" id="actual_spot_<?= $i; ?>" value="<?= $bundle_spot_data[$row[csf('barcode_no')]];?>"/>
	...
	?>
	   ...
	   function calculate_alter()
	   {
		 ...
		 $("#tbl_list_search").find('tbody tr').each(function()
			{
				//alert(4);
				input_qty = $(this).find('input[name="txtDefectQnty[]"]').val(); 
				qty 	  = input_qty ? (input_qty *1) : 0;
				alter_qty += qty;
			});
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$delivery_basis)==3)
		{
			...
			$field_array_mst="id, delivery_mst_id,cut_no, company_id, garments_nature, challan_no, po_break_down_id, item_number_id, country_id, production_source, serving_company, location, production_date, production_hour, production_quantity, reject_qnty, alter_qnty, spot_qnty, replace_qty, production_type, entry_break_down_type,wo_order_id,wo_order_no, shift_name, remarks, floor_id,sewing_line, prod_reso_allo,cost_of_fab_per_pcs,cut_oh_per_pcs,trims_cost_per_pcs,cost_per_pcs, inserted_by, insert_date";
			...
			// ================== getting input qty =====================
			$sql="SELECT c.production_type,c.production_qnty,c.bundle_no from pro_garments_production_dtls c where  c.bundle_no  in ($bundle)  and c.production_type in(4,5) and c.status_active=1 and c.is_deleted=0";
			$result = sql_select($sql);
			$prev_qty_arr = array();
			foreach ($result as $row)
			{
			...
			for($j=1;$j<=$tot_row;$j++)
			{
				...
				$cboHoldBundle="cboHoldBundle_".$j;
                ...
                {                	
					...
					$bundleHoldArr[$$bundleNo]=$$cboHoldBundle;

					$dtlsArrAlt[$$bundleNo]+=$$alterQty*1;
					$dtlsArrSpot[$$bundleNo]+=$$spotQty*1;
					$dtlsArrRep[$$bundleNo]+=$$replaceQty*1;
					$colorQtyArr[$$orderId][$$gmtsitemId][$$colorId]['ok']+=$$qty;
					$colorQtyArr[$$orderId][$$gmtsitemId][$$colorId]['rej']+=$$rejectQty;
					$colorQtyArr[$$orderId][$$gmtsitemId][$$colorId]['alter']+=$$alterQty;
					$colorQtyArr[$$orderId][$$gmtsitemId][$$colorId]['spot']+=$$spotQty;
					$colorQtyArr[$$orderId][$$gmtsitemId][$$colorId]['replace']+=$$replaceQty;
                }
                else
                {
			...
			if($wip_valuation_for_accounts==1)
			{
				...
				$sql = "SELECT a.smv_pcs, b.id,a.gmts_item_id from WO_PO_DETAILS_MAS_SET_DETAILS a,WO_PO_BREAK_DOWN b where a.job_id=b.job_id and b.id in($poIds)";
				$res = sql_select($sql);
				$item_smv_array = array();
				foreach ($res as $v) 
				{
					$item_smv_array[$v['ID']][$v['GMTS_ITEM_ID']] = $v['SMV_PCS'];
				}
				...
				/								COST PER MIN								/
				========================================================================= */
				$month = date('m',strtotime(str_replace("'","",$txt_issue_date)));
				$sql = "select sum(b.COST_PER_MIN) as COST_PER_MIN from lib_process_ac_head_standard_mst a, lib_process_ac_head_standard_dtls b where a.ID=b.MST_ID  and a.PROCESS_MONTH=$month and a.proces_type=2 and company_id=$cbo_company_name and a.STATUS_ACTIVE=1 and a.IS_DELETED=0 and b.STATUS_ACTIVE=1 and b.IS_DELETED=0";
				// echo "10**".$sql;die;
				$res= sql_select($sql);
				$cpm = 0;
				foreach ($res as $v) 
				...
				/* ================================= trims cost =================================== */
				$sql = "SELECT  b.PROD_ID,sum(round(b.cons_amount,2))/sum(round(b.garments_qty,2)) as avg_rage from INV_ISSUE_MASTER a, INV_TRANSACTION b, ORDER_WISE_PRO_DETAILS c where a.id=b.mst_id and b.id=c.trans_id and a.status_active=1 and b.status_active=1 and c.status_active=1 and a.ISSUE_PURPOSE=36 and c.trans_type=2 and b.ITEM_CATEGORY=4 and c.po_breakdown_id in($poIds) and a.entry_form in(25) and a.company_id=$cbo_company_name group by b.PROD_ID";
				// echo "10**$sql";die;
				$res = sql_select($sql);
			...
			$field_array_dtls="id,delivery_mst_id,mst_id,production_type,color_size_break_down_id,production_qnty,reject_qty,alter_qty,spot_qty,replace_qty,cut_no,bundle_no,barcode_no,is_rescan,color_type_id,cost_of_fab_per_pcs,cut_oh_per_pcs,trims_cost_per_pcs,cost_per_pcs,total_cut_fab_cost,total_oh_cost,total_trims_cost,total_process_cost,bndl_hold_reason";
			...
			{
				if($prev_qty_arr[4][$bundle_no] >= $qty+$prev_qty_arr[5][$bundle_no]) // here check output qty not over input qty
				{
					...
					if($wip_valuation_for_accounts==1)
					{
						// ==================== calculate sewing cost per pcs ==================================
						// $cost_per_pcs[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['cost_per_pcs']
						...
						$fab_cost_per_pcs = (($colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['ok']+$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['rej']+$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['spot']+$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['alter']- $colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['replace'])*$cost_per_pcs)/$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['ok'];
						// $fab_cost_per_pcs = $fab_cost_array[$orderId][$gmtsItemId][$countryId]['cost_per_pcs'];
						$fab_cost_per_pcs = number_format($fab_cost_per_pcs,$dec_place[3],'.','');
						// $trims_cost_per_pcs = $trims_issue_avg_rate;
						$trims_cost_per_pcs = (($colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['ok']+$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['rej']+$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['spot']+$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['alter']- $colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['replace'])*$trims_issue_avg_rate)/$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['ok'];
						$trims_cost_per_pcs = number_format($trims_cost_per_pcs,$dec_place[3],'.','');
						$sewing_oh = $cpm*$item_smv_array[$colorSizedData[0]][$colorSizedData[1]];
						$sewing_oh = number_format($sewing_oh,$dec_place[3],'.','');
						$sew_cost_per_pcs = $fab_cost_per_pcs+$trims_cost_per_pcs+$sewing_oh;

						$tot_fab_cost = $colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['ok']*$fab_cost_per_pcs;
						$tot_fab_cost = number_format($tot_fab_cost,$dec_place[3],'.','');
						$tot_trims_cost = $colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['ok']*$trims_cost_per_pcs;
						$tot_trims_cost = number_format($tot_trims_cost,$dec_place[3],'.','');
						$tot_oh_cost = $colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['ok']*$sewing_oh;
						$tot_oh_cost = number_format($tot_oh_cost,$dec_place[3],'.','');
						$tot_process_cost = $colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['ok']*$sew_cost_per_pcs;
						$tot_process_cost = number_format($tot_process_cost,$dec_place[3],'.','');

						// ==================== end sewing cost per pcs ==================================
					}
					// echo "10**$fab_cost_per_pcs+$trims_cost_per_pcs+$sewing_oh";die;
					...
					$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",5,'".$dtlsArrColorSize[$bundle_no]."','".$qty."','".$dtlsArrRej[$bundle_no]."','".$dtlsArrAlt[$bundle_no]."','".$dtlsArrSpot[$bundle_no]."','".$dtlsArrRep[$bundle_no]."','".$cut_no."','".$bundle_no."','".$bundleBarcodeArr[$bundle_no]."','".$bundleRescanArr[$bundle_no]."','".$bundle_wise_type_array[$bundle_no]."','".$fab_cost_per_pcs."','".$sewing_oh."','".$trims_cost_per_pcs."','".$sew_cost_per_pcs."','".$tot_fab_cost."','".$tot_oh_cost."','".$tot_trims_cost."','".$tot_process_cost."',".$bundleHoldArr[$bundle_no].")";
					$colorSizeIdArr[$bundle_no]=$gmtsMstId;
					//$dtls_id = $dtls_id+1;
				}
				else
				{
					echo "20**Sewing output qty is over than input qty.Bundle no=$bundle_no"; disconnect($con);die();
				}
			}
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$challan_no=(int) $txt_chal_no[3];
			...
			$receive_sql="SELECT c.barcode_no,c.bundle_no from pro_garments_production_dtls c where  c.barcode_no  in ($bundle)  and c.production_type=5 and c.status_active=1 and c.is_deleted=0 and (c.is_rescan=0 or c.is_rescan is null) and c.delivery_mst_id!=$mst_id";
			$receive_result = sql_select($receive_sql);
			foreach ($receive_result as $row)
			{
			...
			// ================== getting input qty =====================
			$sql="SELECT c.production_qnty,c.bundle_no from pro_garments_production_dtls c where  c.barcode_no  in ($bundle)  and c.production_type=4 and c.status_active=1 and c.is_deleted=0";
			$result = sql_select($sql);
			$prev_qty_arr = array();
			foreach ($result as $row)
			{
				$prev_qty_arr[4][trim($row[csf('bundle_no')])]+=trim($row[csf('production_qnty')]);
			}

			// ================== getting output qty =====================
			$sql="SELECT c.production_qnty,c.bundle_no from pro_garments_production_dtls c where  c.barcode_no  in ($bundle)  and c.production_type=5 and c.status_active=1 and c.is_deleted=0  and c.delivery_mst_id!=$mst_id";
			$result = sql_select($sql);
			foreach ($result as $row)
			{
				$prev_qty_arr[5][trim($row[csf('bundle_no')])]+=trim($row[csf('production_qnty')]);
			}
		...
		if(str_replace("'","",$delivery_basis)==3)
		{
			...
			//echo "10**";
			//-------
			$production_mst_id_arr=return_library_array( "select id, id as mst_id  from  pro_garments_production_mst where delivery_mst_id=$mst_id and production_type=5",'id','mst_id');
			...
			$field_array_mst="id, delivery_mst_id,cut_no, company_id, garments_nature, challan_no, po_break_down_id, item_number_id, country_id, production_source, serving_company, location, production_date,production_hour, production_quantity,reject_qnty,alter_qnty,spot_qnty,replace_qty, production_type, entry_break_down_type, shift_name, remarks, floor_id,sewing_line,prod_reso_allo,wo_order_no,cost_of_fab_per_pcs,cut_oh_per_pcs,trims_cost_per_pcs,cost_per_pcs, inserted_by, insert_date";
            ...
            $colorIds = implode(",",array_filter($colorIdArr));
            $sew_bundle = "'".implode("','",array_filter($bundleNoArr))."'";
			// echo "10**";print_r($colorQtyArr);die;
			...
			if($wip_valuation_for_accounts==1)
			{
				...
				// echo "10**".number_format($trims_issue_avg_rate,$dec_place[3],'.','');die;
				/* ================================= get fabric cost =================================== */
			...
			foreach($mstArr as $orderId=>$orderData)
			{
				foreach($orderData as $gmtsItemId=>$gmtsItemIdData)
				{
					foreach($gmtsItemIdData as $countryId=>$qty)
					{
						...
						if($db_type==2)
						{
							$data_array_mst.=" INTO pro_garments_production_mst (".$field_array_mst.") VALUES(".$id.",".$mst_id.",'".$cutArr[$orderId][$gmtsItemId][$countryId]."',".$cbo_company_name.",".$garments_nature.",'".$challan_no."',".$orderId.", ".$gmtsItemId.",".$countryId.", ".$cbo_source.",".$cbo_emb_company.",".$cbo_location.",".$txt_issue_date.",".$txt_reporting_hour.",".$qty.",'".$mstArrRej[$orderId][$gmtsItemId][$countryId]."','".$mstArrAlt[$orderId][$gmtsItemId][$countryId]."','".$mstArrSpot[$orderId][$gmtsItemId][$countryId]."','".$mstArrRep[$orderId][$gmtsItemId][$countryId]."',5,".$sewing_production_variable.",".$cbo_shift_name.",".$txt_remark.",".$cbo_floor.",".$cbo_line_no.",".$prod_reso_allocation.",".$txt_wo_no.",'".$fab_cost_per_pcs."','".$sewing_oh."','".$trims_cost_per_pcs."','".$sew_cost_per_pcs."',".$user_id.",'".$pc_date_time."')";
						}
						else
						{
							if($data_array_mst!="") $data_array_mst.=",";
			...
			foreach($dtlsArr as $bundle_no=>$qty)
			{
				// echo $prev_qty_arr[4][$bundle_no] .">=". $qty."+".$prev_qty_arr[5][$bundle_no]."<br>";
				if($prev_qty_arr[4][$bundle_no] >= $qty+$prev_qty_arr[5][$bundle_no]) // here check output qty not over input qty
				{
					$colorSizedData=explode("**",$colorSizeArr[$bundle_no]);
					$gmtsMstId=$mstIdArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[2]];
					...
					if($wip_valuation_for_accounts==1)
					{
						...
						$fab_cost_per_pcs = (($colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['ok']+$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['rej']+$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['spot']+$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['alter'] - $colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['replace'])*$cost_per_pcs)/$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['ok'];
						// echo $fab_cost_per_pcs."<br>";
						// $fab_cost_per_pcs = $fab_cost_array[$orderId][$gmtsItemId][$countryId]['cost_per_pcs'];
						$fab_cost_per_pcs = number_format($fab_cost_per_pcs,$dec_place[3],'.','');
						// $trims_cost_per_pcs = $trims_issue_avg_rate;
						$trims_cost_per_pcs = (($colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['ok']+$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['rej']+$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['spot']+$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['alter'] - $colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['replace'])*$trims_issue_avg_rate)/$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['ok'];

						// echo "((".$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['ok']."+".$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['rej']."+".$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['spot']."+".$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['alter']."-". $colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['replace'].")*".$trims_issue_avg_rate.")/".$colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['ok']."<br>";


						$trims_cost_per_pcs = number_format($trims_cost_per_pcs,$dec_place[3],'.','');
						$sewing_oh = $cpm*$item_smv_array[$colorSizedData[0]][$colorSizedData[1]];
						// echo $sewing_oh."=".$cpm."*".$item_smv_array[$colorSizedData[0]][$colorSizedData[1]]."=".$fab_cost_per_pcs."<br>";
						$sewing_oh = number_format($sewing_oh,$dec_place[3],'.','');
						$sew_cost_per_pcs = $fab_cost_per_pcs+$trims_cost_per_pcs+$sewing_oh;
						...
						$tot_process_cost = number_format($tot_process_cost,$dec_place[3],'.','');
						// ==================== end sewing cost per pcs ==================================
						// $tot_trims_costss  += $tot_trims_cost;
						// echo $colorQtyArr[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['ok']."*".$trims_cost_per_pcs."<br>";
					}
					// echo "10**$fab_cost_per_pcs+$trims_cost_per_pcs+$sewing_oh";die;
					$cut_no=$bundleCutArr[$bundle_no];
					if($data_array_dtls!="") $data_array_dtls.=",";
			...
			// echo "10**insert into pro_gmts_prod_dft (".$field_array_defect.") values ".$data_array_defect;die;
			// echo "10**".$challanrID ."&&". $rID ."&&". $dtlsrID ."&&". $bundlerID ."&&". $delete ."&&". $delete_dtls ."&&". $delete_bundle;die;
			//release lock table
	...
	else if ($operation==2)  // Delete Here----------------------------------------------------------
	{
		$con = connect();
		if($db_type==0)	{ mysql_query("BEGIN"); }
		$delv_id=str_replace("'","",$txt_system_id);

		// ========= chk next process ===========

		// $delvrID = sql_delete("pro_gmts_delivery_mst","updated_by*update_date*status_active*is_deleted","".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*0*1",'id',$delv_id,1);
		
		$rID = sql_delete("pro_garments_production_mst","updated_by*update_date*status_active*is_deleted","".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*0*1",'delivery_mst_id*production_type',$delv_id."*5",1);
		$dtlsrID = sql_delete("pro_garments_production_dtls","status_active*is_deleted","0*1",'delivery_mst_id*production_type',$delv_id."*5",1);

		$production_mst_id_arr=return_library_array( "select id, id as mst_id  from  pro_garments_production_mst where delivery_mst_id=$delv_id and production_type=5",'id','mst_id');
		//print_r($production_mst_id_arr);
		$production_mst_id=implode(',',$production_mst_id_arr); 
		$dft_delete = execute_query("UPDATE pro_gmts_prod_dft set status_active=0,is_deleted=1 WHERE mst_id in (".$production_mst_id.") and defect_type_id in(2,3,4) and production_type=5");   
 		...
 		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");
				echo "2**".$delv_id."**".str_replace("'","",$txt_challan_no);
			}
			else
			{
				mysql_query("ROLLBACK");
				echo "10**".$delv_id."**".str_replace("'","",$txt_challan_no);
			}
		}
		else if($db_type==2 || $db_type==1 )
		{
			if($rID && $dtlsrID && $dft_delete)
			{
				oci_commit($con);
				echo "2**".$delv_id."**".str_replace("'","",$txt_challan_no);
			}
...
if ($action=="challan_no_popup")
{
	...
	<script src="../../includes/functions_bottom.js" type="text/javascript"></script>
	<script>
		load_drop_down( 'bundle_wise_sewing_output_controller', '<?php echo $cbo_serving_company; ?>_<?php echo $cbo_location; ?>_<?php echo $cbo_floor; ?>_<?php echo $txt_issue_date; ?>', 'load_drop_down_line_no', 'line_no_id' );
...
if($action=='populate_data_from_challan_popup')
{
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_drop_down( 'requires/bundle_wise_sewing_output_controller', '".$row[csf('production_source')]."', 'load_drop_down_wo_company_new', 'emb_company_td' );\n";
...
if($action=="emblishment_issue_print")
{
    ...
    <table width="900" cellspacing="0" align="right">
        ...
        <?
			...
			$order_array=array();
			$order_sql="SELECT a.job_no, a.buyer_name,a.style_ref_no, a.style_description,b.id, b.po_number, b.po_quantity from  wo_po_details_master a, wo_po_break_down b where a.id=b.job_id and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0 $po_id_cond";
...
if($action=="emblishment_issue_print_2")
{
	...
	$buyer_arr=return_library_array( "select id, buyer_name from lib_buyer",'id','buyer_name');
	$order_array=array();
	$order_sql="SELECT a.job_no, a.buyer_name,a.style_ref_no, a.style_description,b.id, b.po_number, b.po_quantity from  wo_po_details_master a, wo_po_break_down b where a.id=b.job_id and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0";
	$order_sql_result=sql_select($order_sql);
	foreach ($order_sql_result as $row)
	{
		$order_array[$row[csf('id')]]['buyer_name']=$row[csf('buyer_name')];
...
if($action=="emblishment_issue_print_3")
{
	...
	<div style="width:1250px;">
     <table cellspacing="0" >
        ...
        <tr>
            <td colspan="6" align="center" style="font-size:20px"><u><strong>Reject to Leftover Challan</strong></u></td>
        </tr>
        <tr>
            <td width="95"><strong>Challan No:</strong></td>
			<td width="20">:</td>
		...
		<tr>
			...
			<td width="20">:</td>
			 <td>
				<?
					 echo $result[csf('plot_no')].", ".$result[csf('city')];
        ...
        <?
			...
			if($data[2]==3)
			{
				...
				$sql="SELECT a.po_break_down_id,a.item_number_id,a.country_id,a.production_date,b.bundle_no,b.barcode_no,d.size_number_id,d.color_number_id,
				b.reject_qty,e.floor_id,f.job_no_prefix_num
				from pro_garments_production_mst a, 
				pro_garments_production_dtls b, 
				wo_po_color_size_breakdown d,
				pro_gmts_delivery_mst e,
				wo_po_details_master f
				where a.delivery_mst_id ='$data[1]'
				and a.id=b.mst_id 
				and e.id=a.delivery_mst_id
				...
				and d.is_deleted=0
				group by a.po_break_down_id,a.item_number_id,a.country_id,a.production_date,b.reject_qty,b.bundle_no,b.barcode_no,d.size_number_id,d.color_number_id,e.floor_id,f.job_no_prefix_num
				order by  length(b.bundle_no) asc, b.bundle_no  asc";
			}
			else
			{
				$sql="SELECT c.po_break_down_id,c.item_number_id,c.country_id,b.bundle_no,d.size_number_id, b.color_number_id,d.floor_id
				from pro_garments_production_dtls a, 
				wo_po_color_size_breakdown b,pro_garments_production_mst c, 
				pro_gmts_delivery_mst d 
				where c.delivery_mst_id ='$data[1]'
				and c.id=a.mst_id  
				and e.id=b.delivery_mst_id
				and a.color_size_break_down_id=b.id 
				and b.production_type=5 
				and a.status_active=1 
				and a.is_deleted=0 
				and b.status_active in(1,2,3) 
				and b.is_deleted=0
				group by c.po_break_down_id,c.item_number_id,c.country_id,b.bundle_no,d.size_number_id,b.color_number_id ";
			}
			//echo $sql;die;
	...
	<div style="width:100%;">
    	<table cellspacing="0" width="1250"  border="1" rules="all" class="rpt_table">
        	<thead bgcolor="#dddddd" align="center">
				<th width="30">SL</th>
				<th width="80" align="center">Sewing Unit</th>
				<th width="40" align="center">Line</th>
				<th width="80" align="center">Buyer</th>
				<th width="40" align="center">Job</th>
				<th width="40" align="center">Order</th>				
				<th width="80" align="center">Item</th>
				<th width="60" align="center">Style Ref</th>
				<th width="90" align="center">Production Date</th>
				<th width="60" align="center">Country</th>
				<th width="80" align="center">Color Name</th>
				<th width="40" align="center">Size</th>
				<th width="100" align="center">Actual Reject Qty</th>
				<th width="80" align="center">Bundle ID</th>
        ...
        ?>
                    <tr bgcolor="<? echo $bgcolor; ?>"  style="font-size:12px">
                        <td><? echo $i;  ?></td>
                        <td align="center"><? echo $floor_library[$val[csf('floor_id')]];?></td>
                        <td align="center"><? echo $line;?></td>
                        <td align="center"><?echo $buyer_arr[$order_array[$val[csf('po_break_down_id')]]['buyer_name']] ?></td>
                        <td align="center"><? echo $val[csf('job_no_prefix_num')]; ?></td>
                        <td align="center"><? echo $order_array[$val[csf('po_break_down_id')]]['po_number']; ?></td>
                        <td align="center"><?  echo $garments_item[$val[csf('item_number_id')]];?></td>
                        <td align="center"><? echo $order_array[$val[csf('po_break_down_id')]]['style_ref'];  ?></td>
                        <td align="center"><? echo $val[csf('production_date')];?></td>
						...
						?>
							<td align="right"><?  echo $val[csf('reject_qty')]; ?></td>
						<?
						}else {
						?>
						<td align="right">&nbsp;</td>
                        ...
                        <td align="right"><?  echo $val[csf('bundle_no')];?></td>

                    </tr>
                    <?
					$production_quantity+=$val[csf('reject_qty')];
					$size_qty_arr[$val[csf('size_number_id')]]+=$val[csf('reject_qty')];
					$i++;
                }
            ?>
        </tbody>
...
if($action=="emblishment_issue_print_4")
{
    ...
    <table cellspacing="0" >
        ...
        <tr>
            <td colspan="6" align="center" style="font-size:20px"><u><strong>Sewing Rejection Rectify Challan</strong></u></td>
        </tr>
        <tr>
            <td width="95"><strong>Challan No:</strong></td>
			<td width="20">:</td>
        ...
        <?
			...
			if($data[2]==3)
			{
				$sql="SELECT sum(b.production_qnty) as production_qnty,a.po_break_down_id,a.item_number_id,a.country_id,a.production_date,b.bundle_no,b.barcode_no,d.size_number_id,d.color_number_id,
				a.reject_qnty,e.floor_id,f.job_no_prefix_num
				from pro_garments_production_mst a, 
				pro_garments_production_dtls b, 
				wo_po_color_size_breakdown d,
				pro_gmts_delivery_mst e,
				...
				and d.status_active in(1,2,3)
				and d.is_deleted=0
				group by a.po_break_down_id,a.item_number_id,a.country_id,a.production_date,a.reject_qnty,b.bundle_no,b.barcode_no,d.size_number_id,d.color_number_id,e.floor_id,f.job_no_prefix_num
				order by  length(b.bundle_no) asc, b.bundle_no  asc";//order by a.po_break_down_id,d.color_number_id
			}
			else
			{
				$sql="SELECT sum(a.production_qnty) as production_qnty,c.po_break_down_id,c.item_number_id,c.country_id,b.bundle_no,d.size_number_id, b.color_number_id,d.floor_id
				from pro_garments_production_dtls a, 
				wo_po_color_size_breakdown b,pro_garments_production_mst c, 
				pro_gmts_delivery_mst d 
				where c.delivery_mst_id ='$data[1]'
	...
	<div style="width:100%;">
    	<table cellspacing="0" width="1250"  border="1" rules="all" class="rpt_table">
        	<thead bgcolor="#dddddd" align="center">
				...
				<th width="80" align="center">Color Name</th>
				<th width="40" align="center">Size</th>
				<th width="100" align="center">Recovery Qty</th>
				<th width="80" align="center">Bundle ID</th>
				<th width="80" align="center">Bundle No</th>
       		 </thead>
        <tbody>
...
if($action=="emblishment_issue_print_5")
{
	...
	$buyer_arr=return_library_array( "select id, buyer_name from lib_buyer",'id','buyer_name');
	$order_array=array();
	$order_sql="SELECT a.job_no, a.buyer_name,a.style_ref_no, a.style_description,b.id, b.po_number, b.po_quantity,b.grouping from  wo_po_details_master a, wo_po_break_down b where a.id=b.job_id and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0";
	$order_sql_result=sql_select($order_sql);
	foreach ($order_sql_result as $row)
	{
		$order_array[$row[csf('id')]]['buyer_name']=$row[csf('buyer_name')];
    ...
    <table cellspacing="0" >
        ...
        <tr>
            <td colspan="6" align="center" style="font-size:20px"><u><strong>Sewing Output and Packing Receive Challan</strong></u></td>
        </tr>
        <tr>
            <td width="110"><strong>Challan No:</strong></td>
            <td width="180"><? echo $dataArray[0][csf('sys_number')]; ?></td>
            <td width="80"><strong>Source:</strong></td>
            <td width="190"><? echo $knitting_source[$dataArray[0][csf('production_source')]]; ?></td>
            <td width="120"><strong>Sew. Company:</strong></td>
            <td>
        ...
        <?
			...
			if($data[2]==3)
			{
				...
				$sql="SELECT sum(b.production_qnty) as production_qnty,a.po_break_down_id,a.item_number_id,a.country_id,b.reject_qty, b.bundle_no,d.size_number_id,d.color_number_id,
				count(b.id) as 	num_of_bundle
				from pro_garments_production_mst a, pro_garments_production_dtls b, wo_po_color_size_breakdown d
				where a.delivery_mst_id ='$data[1]'
				and a.id=b.mst_id and b.color_size_break_down_id=d.id and b.production_type=5 and b.status_active=1 and b.is_deleted=0 and d.status_active in(1,2,3)
				and d.is_deleted=0
				group by a.po_break_down_id,b.reject_qty,a.item_number_id,a.country_id,b.bundle_no,d.size_number_id,d.color_number_id
				order by  length(b.bundle_no) asc, b.bundle_no  asc";//order by a.po_break_down_id,d.color_number_id wo_po_break_down
			}
			else
			{
				$sql="SELECT sum(a.production_qnty) as production_qnty,c.po_break_down_id,c.item_number_id,c.country_id,b.bundle_no,d.size_number_id, b.color_number_id,a.reject_qty
				from pro_garments_production_dtls a, wo_po_color_size_breakdown b,pro_garments_production_mst c where c.delivery_mst_id ='$data[1]'
				and c.id=a.mst_id  and a.color_size_break_down_id=b.id and b.production_type=5 and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0
				group by c.po_break_down_id,c.item_number_id,c.country_id,b.bundle_no,d.size_number_id,b.color_number_id,a.reject_qty ";
			}
			//echo $sql;
    ...
    <table cellspacing="0" width="900"  border="1" rules="all" class="rpt_table">
        ...
        <tbody>
            ...
            <td colspan="4"><strong>No. Of Bundle :<?  echo $total_bundle; ?></strong></td>
            <td colspan="7" align="right"><strong>Grand Total :</strong></td>
            <td align="right"><?  echo $production_quantity; ?></td>
            <td align="right">&nbsp;</td>
    </table>
