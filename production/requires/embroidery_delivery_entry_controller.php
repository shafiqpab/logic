...
{
	$print_report_format=return_field_value("format_id","lib_report_template","template_name ='".$data."' and module_id=7 and report_id in(52) and is_deleted=0 and status_active=1");
...
{
 	echo create_drop_down( "cbo_floor", 180, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process in (9) order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "",0 );
	exit();
}
...
if ($action=="load_variable_settings")
{
	...
	if($workorder_material_autoreceive ==1 ){
...
if($action=="load_drop_down_embro_issue_source")
{
	...
	if($data==3)
	{
		if($db_type==0)
		{
			echo create_drop_down( "cbo_emb_company", 180, "select id,supplier_name from lib_supplier where status_active=1 and is_deleted=0 and find_in_set(23,party_type) order by supplier_name","id,supplier_name", 1, "--- Select ---", $selected, "load_drop_down( 'requires/embroidery_delivery_entry_controller', this.value, 'load_drop_down_location', 'location_td' );" );
		}
		else
		{
			echo create_drop_down( "cbo_emb_company", 180, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=23 and a.status_active=1 group by a.id,a.supplier_name order by a.supplier_name","id,supplier_name", 1, "--Select--", 0, "load_drop_down( 'requires/embroidery_delivery_entry_controller', this.value, 'load_drop_down_location', 'location_td' );" );
		}
	}
	else if($data==1)
		echo create_drop_down( "cbo_emb_company", 180, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1 $company_cond order by company_name","id,company_name", 1, "--- Select ---", $selected_company, "load_drop_down( 'requires/embroidery_delivery_entry_controller', this.value, 'load_drop_down_location', 'location_td' );",0,0 );
		//echo create_drop_down( "cbo_location", 180, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id='$data' order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down('requires/embroidery_delivery_entry_controller', this.value, 'load_drop_down_floor', 'floor_td' );",0 );
	else
		echo create_drop_down( "cbo_emb_company", 180, $blank_array,"", 1, "--- Select ---", $selected, "load_drop_down( 'requires/embroidery_delivery_entry_controller', this.value, 'load_drop_down_location', 'location_td' );",0 );
...
if($action=="load_drop_down_embro_issue_source_new")
{
	...
	if($data==3)
	{
		...
		{		$sql="SELECT a.id, a.supplier_name	FROM lib_supplier a, lib_supplier_party_type b  WHERE a.id = b.supplier_id AND b.party_type = 23 AND a.status_active = 1	  GROUP BY a.id, a.supplier_name	UNION ALL SELECT c.id, c.supplier_name	FROM lib_supplier c, lib_supplier_party_type b , pro_gmts_delivery_mst a  WHERE c.id = b.supplier_id AND b.party_type = 23 AND  c.id = a.serving_company 	 AND c.status_active IN (1, 3)	GROUP BY c.id, c.supplier_name 	ORDER BY supplier_name  ";
			...
			echo create_drop_down( "cbo_emb_company", 180, "$sql","id,supplier_name", 1, "--Select--", 0, "load_drop_down( 'requires/embroidery_delivery_entry_controller', this.value, 'load_drop_down_location', 'location_td' );" );
		}
	}
	else if($data==1)
		echo create_drop_down( "cbo_emb_company", 180, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1 $company_cond order by company_name","id,company_name", 1, "--- Select ---", $selected_company, "load_drop_down( 'requires/embroidery_delivery_entry_controller', this.value, 'load_drop_down_location', 'location_td' );",0,0 );
...
if($action=="create_bundle_rescan_search_list_view")
{
	...
	$last_operation=gmt_production_validation_script( 92, 1,'',$cutting_no, $production_squence);
	...
	$sql_scan_bundle=sql_select(" select b.barcode_no,sum(b.production_qnty) as production_qnty from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type in (2,4) and a.embel_name in (2,0) and b.status_active=1 and b.is_deleted=0 $not_null_bundle  $cutCon_a  group by b.barcode_no");
...
if($action=="populate_bundle_data_update")
{
	...
	//$scanned_bundle_arr=return_library_array( "select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=2 and a.embel_name=1 and b.status_active=1 and b.is_deleted=0",'bundle_no','bundle_no');
	$bundle_nos=return_library_array( "select b.barcode_no, b.bundle_no from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id and a.delivery_mst_id=$mst_id and b.production_type=2 and a.embel_name=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","bundle_no","barcode_no");
		...
		<legend>Enter search words <input type="checkbox" value="1" name="is_exact" id="is_exact" checked> is exact</legend>
            <table cellpadding="0" cellspacing="0" width="550" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td><input type="text" name="bundle_no" id="bundle_no" style="width:120px" class="text_boxes" /></td>
            		<td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view (document.getElementById('txt_order_no').value+'_'+<? echo $company; ?>+'_'+document.getElementById('bundle_no').value+'_'+'<? echo trim($bundleNo,','); ?>'+'_'+document.getElementById('txt_job_no').value+'_'+document.getElementById('txt_cut_no').value+'_'+document.getElementById('cbo_cut_year').value+'_'+$('#is_exact').is(':checked'), 'create_bundle_search_list_view', 'search_div', 'embroidery_delivery_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1);reset_hide_field();')" style="width:100px;" />
                     </td>
                </tr>
           </table>
           <div style="width:100%; margin-top:5px; margin-left:10px" id="search_div" align="left"></div>
...
if ($action=="embroidery_order_popup")
{
	...
	<script>
		...
		{
			load_drop_down( 'embroidery_delivery_entry_controller', company+'_'+1+'_'+party_name, 'load_drop_down_company', 'buyer_td' );
			$('#cbo_company_name').attr('disabled',true);
		}

		// function fnc_load_party_order_popup(company,party_name)
		// {
		// 	load_drop_down( 'embroidery_delivery_entry_controller', company+'_'+1+'_'+party_name, 'load_drop_down_buyer', 'buyer_td' );
...
{
	echo create_drop_down( "cbo_company_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $data[0], "$load_function");
	exit();
}
...
// 	exit();
// }
if($action == "create_embroidery_booking_search_list_view")
{
	$data=explode('_',$data);
	$search_type=$data[7];
	//echo $data[1]; die;
	...
	{
		$sql = sql_select("SELECT b.id from  wo_po_details_master a, wo_po_break_down b where a.id=b.job_id $job_cond $style_cond $po_cond $company_cond and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0");
		foreach ($sql as $val)
		{
	...
	else
	{
		...
		$sql = sql_select("SELECT b.po_break_down_id from  wo_booking_mst a, wo_booking_dtls b where a.id=b.booking_mst_id $woorder_cond and a.booking_type=6 and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0");
		foreach ($sql as $val)
		{
	...
	$buyer_po_arr=array();
	$po_sql ="SELECT a.style_ref_no, a.job_no, a.buyer_name, b.id, b.po_number from wo_po_details_master a, wo_po_break_down b where a.id=b.job_id and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0 $po_idsCond2";
	...
	$pre_sql ="SELECT a.id, a.emb_name, a.emb_type, a.body_part_id  from wo_pre_cost_embe_cost_dtls a,wo_po_break_down b where a.job_id=b.job_id and  a.emb_name=2 and a.status_active=1 and a.is_deleted=0 $po_idsCond2";
	...
	$pre_cost_trims_cond="b.pre_cost_fabric_cost_dtls_id";
	$gmts_item_id_cond="b.gmt_item_id";
	$gmts_item_cond="b.gmt_item";
	$po_id_cond="b.po_break_down_id";
	...
	$sql= "SELECT $wo_year as year, a.id,a.entry_form, a.booking_type, a.booking_no, a.booking_no_prefix_num, a.company_id, a.buyer_id, a.booking_date, a.currency_id, b.pre_cost_fabric_cost_dtls_id as pre_cost_trims_id, b.gmt_item as gmts_item, b.po_break_down_id as po_id, 1 as wo_type from  wo_booking_mst a, wo_booking_dtls b, wo_pre_cost_embe_cost_dtls c where a.booking_no=b.booking_no and b.pre_cost_fabric_cost_dtls_id=c.id and a.booking_type=6 and a.status_active=1 and c.emb_name=2 and a.lock_another_process=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $booking_date $company $party_cond $woorder_cond $year_cond $po_idsCond
	UNION ALL
	SELECT $wo_year as year, a.id,a.entry_form_id, a.booking_type, a.booking_no, a.booking_no_prefix_num, a.company_id, a.buyer_id, a.booking_date, a.currency_id, 0 as pre_cost_trims_id, b.gmt_item_id as gmts_item, 0 as po_id, 2 as wo_type  from wo_non_ord_embl_booking_mst a, wo_non_ord_embl_booking_dtls b where a.booking_no=b.booking_no and a.booking_type=6 and a.entry_form_id=399 and a.status_active=1 and b.status_active=1 $booking_date $company $party_cond $woorder_cond $year_cond";

	$sql_res=sql_select($sql);
	$data_array = array();
	foreach ($sql_res as  $v) {
		$data_array[$v['BOOKING_NO']]['ID'] = $v['ID'];
		$data_array[$v['BOOKING_NO']]['CURRENCY_ID'] = $v['CURRENCY_ID'];
		$data_array[$v['BOOKING_NO']]['BOOKING_TYPE'] = $v['BOOKING_TYPE'];
		$data_array[$v['BOOKING_NO']]['WO_TYPE'] = $v['WO_TYPE'];
		$data_array[$v['BOOKING_NO']]['YEAR'] = $v['YEAR'];
		$data_array[$v['BOOKING_NO']]['BOOKING_DATE'] = $v['BOOKING_DATE'];
		$data_array[$v['BOOKING_NO']]['BUYER_ID'] =$v['BUYER_ID'];
		$data_array[$v['BOOKING_NO']]['PO_ID'] .=','.$v['PO_ID'];
		$data_array[$v['BOOKING_NO']]['PRE_COST_TRIMS_ID'] .=','.$v['PRE_COST_TRIMS_ID'];
		$data_array[$v['BOOKING_NO']]['GMTS_ITEM'] .=','.$v['GMTS_ITEM'];
	}
	?>
    ...
    <div style="width:1140px; max-height:270px;overflow-y:scroll;" >
        ...
        <tbody>
            <?
            $i=1;
            foreach($data_array as $booking_no => $v)
			{
                if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
				$expo_id=array_unique(explode(",",$v['PO_ID']));
				...
				$expre_cost_trims_id=array_unique(explode(",",$v['PRE_COST_TRIMS_ID']));
				// print_r($expre_cost_trims_id);
				$body_part_name=""; $embl_name=""; $embl_type="";
				foreach ($expre_cost_trims_id as $pre_cost_id)
				{
					if($body_part_name=="")
					{
						$body_part_name=$body_part[$pre_cost_trims_arr[$pre_cost_id]['body_part_id']];
					}
					else
					{
						$body_part_name.=','.$body_part[$pre_cost_trims_arr[$pre_cost_id]['body_part_id']];
					...
					if($embl_name=="")
					{
						$embl_name=$emblishment_name_array[$pre_cost_trims_arr[$pre_cost_id]['emb_name']];
					}
					else
					{
						$embl_name.=','.$emblishment_name_array[$pre_cost_trims_arr[$pre_cost_id]['emb_name']];
					}
					// print_r($pre_cost_trims_arr); die;
				...
				$gmts_item_name="";
				$exgmts_item_id=explode(",",$v['GMTS_ITEM']);
				...
				if($v['WO_TYPE']==1) $booking_type='With Order'; else $booking_type='Without Order';
                ?>
                <tr bgcolor="<?= $bgcolor; ?>" onClick='js_set_value("<?= $v['ID'].'_'.$booking_no.'_'.$v['CURRENCY_ID'].'_'.$v['BOOKING_TYPE'].'_'.$v['WO_TYPE']; ?>")' style="cursor:pointer" >
                    <td width="30"><?= $i; ?></td>
                    <td width="40" align="center"><?= $v['YEAR']; ?></td>
                    <td width="100" align="center"><?= $booking_no ?></td>
                    <td width="60"><?= change_date_format($v['BOOKING_DATE']); ?></td>
                    <td width="100" style="word-break:break-all"><?= $buyer_name; ?></td>
                    <td width="120" style="word-break:break-all"><?= $po_no; ?></td>
                    <td width="120" style="word-break:break-all"><?= $buyer_style; ?></td>
                    <td width="120" style="word-break:break-all"><?= $buyer_job; ?></td>

                    <td width="130" style="word-break:break-all"><?= $gmts_item_name; ?></td>
                    <td width="120" style="word-break:break-all"><?= $body_part_name ; ?></td>
                    <td width="100" style="word-break:break-all"><?= $embl_type; ?></td>
                    <td style="word-break:break-all"><?= $booking_type; ?></td>
                </tr>
				<?
                $i++;
            }
...
if($action=="create_bundle_search_list_view")
{
	 ...
	 if($txt_order_no!="")
	 {
		 $order_no_cond = " and e.po_number = '$txt_order_no'";
	 }
	$company = $ex_data[1];
	...
	if($bundle_no!="")
	{
		$bundle_no_cond = " and c.bundle_no = '$bundle_no'";
	}
	...
	// echo "<pre>";print_r($last_operation);die();
	...
	$scanned_bundle_arr=return_library_array( "SELECT b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type in (2,4) and a.embel_name in (2,0) $cutCon_a and b.status_active=1 and b.is_deleted=0",'bundle_no','bundle_no');
	// echo "SELECT b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type in (2,4) and a.embel_name in (2,0) $cutCon_a and b.status_active=1 and b.is_deleted=0";die;
	foreach(explode(",",$selectedBuldle) as $bn)
	{
	...
	<div style="width:850px; max-height:230px; overflow-y:scroll" id="list_container_batch" align="left">
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="830" class="rpt_table" id="tbl_list_search">
			...
			foreach($last_operation as  $item_id=>$operation_cond)
			{
				...
				//else $last_operation_string.=",".$item_id.":".$operation_cond;
				$sql="SELECT c.cut_no, c.bundle_no,c.barcode_no, d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.bundle_no, sum(c.production_qnty) as qty, e.po_number from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e,wo_pre_cost_embe_cost_dtls f where a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_id=f.job_id and a.company_id=$company $order_no_cond $bundle_no_cond  $operation_conds  $item_id  $jobCon $cutCon  and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.emb_name=2
...
if($action=="challan_duplicate_check")
{
	...
	//echo "select a.sys_number, b.bundle_no from pro_gmts_delivery_mst a,pro_garments_production_dtls b where a.id=b.delivery_mst_id and b.bundle_no in ($bundle_no) and b.production_type=2 and a.embel_name=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.sys_number, b.bundle_no";
	$datastr="";
	if(count($result)>0)
	{
		foreach ($result as $row)
...
if($action=="populate_bundle_data")
{
	...
	$scanned_bundle_arr=return_library_array( "select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type in (2,4) and a.embel_name in( 2,0) and b.status_active=1 and b.is_deleted=0 $scnbundle_nos_cond",'bundle_no','bundle_no');
	...
	foreach($last_operation as  $item_id=>$operation_cond)
	{
		...
		{
			$sqld = sql_select( "SELECT c.bundle_no, SUM(c.reject_qty) as reject_qty, SUM(c.alter_qty) as alter_qty, SUM(c.spot_qty) as spot_qty, SUM(c.replace_qty) as replace_qty  from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e,wo_po_details_master f where d.job_no_mst=f.job_no and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and a.company_id='$ex_data[3]' and c.production_type in (3) and a.status_active=1 and a.is_deleted=0 $bundle_nos_cond group by c.bundle_no order by length(c.bundle_no) asc, c.bundle_no asc" );
			foreach($sqld as $arows)
			{
				$reject_qty[$arows[csf('bundle_no')]]+=$arows[csf('reject_qty')];
				$alter_qty[$arows[csf('bundle_no')]]+=$arows[csf('alter_qty')];
	...
	//echo $sql;die;
		...
		$is_emb_in_budget = return_field_value("EMB_NAME","WO_PRE_COST_EMBE_COST_DTLS","JOB_ID=$job_id and emb_name=2 and is_deleted=0 and status_active=1");
		...
		foreach ($result as $row)
		{
			  $row[csf('production_qnty')]= ($row[csf('production_qnty')] ) - ($reject_qty[$row[csf('bundle_no')]] + $alter_qty[$row[csf('bundle_no')]]+ $spt_qty[$row[csf('bundle_no')]])-$scanned_bundle_arr[$row[csf('bundle_no')]];
			  //+ $replace_qty[$row[csf('bundle_no')]]
...
if($action=="populate_bundle_data_rescan")
{
	...
	$scanned_bundle_arr=return_library_array( "select b.bundle_no, sum(b.production_qnty) as production_qnty from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type in (2,4) and a.embel_name in( 2,0) and b.status_active=1 and b.is_deleted=0 $scnbundle_nos_cond group by b.bundle_no",'bundle_no','production_qnty');
...
{
	$bundle_nos=return_library_array( "select b.barcode_no, b.bundle_no from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id and a.delivery_mst_id=$data and b.production_type=2 and a.embel_name=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","bundle_no","barcode_no");
	$bundle_nos=implode(",",$bundle_nos);
	echo $bundle_nos;
	exit();
}
	...
	<div style="width:100%;max-height:180px; overflow-y:scroll" id="sewing_production_list_view" align="left">
		...
		<?php
			...
			foreach($sqlResult as $selectResult)
			{
 			...
 			?>
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_issue_form_data','requires/embroidery_delivery_entry_controller');" >
...
if($action=="populate_issue_form_data")
{
	...
	foreach($sqlResult as $result)
	{
		...
		echo "get_php_form_data(".$result[csf('po_break_down_id')]."+'**'+".$result[csf("item_number_id")]."+'**'+".$result[csf("embel_name")]."+'**'+".$result[csf("country_id")].", 'populate_data_from_search_popup', 'requires/embroidery_delivery_entry_controller' );\n";

		echo "$('#cbo_item_name').val('".$result[csf('item_number_id')]."');\n";
		echo "$('#cbo_country_name').val('".$result[csf('country_id')]."');\n";

		echo "show_list_view('".$result[csf('po_break_down_id')]."','show_country_listview','list_view_country','requires/embroidery_delivery_entry_controller','');\n";
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$txt_system_id)=="")
		{
			...
			if($db_type==0) $year_cond="YEAR(insert_date)";
 			...
 			$new_sys_number = explode("*", return_next_id_by_sequence("", "pro_gmts_delivery_mst",$con,1,$cbo_company_name,'EDE',0,date("Y",time()),0,0,2,$cbo_embel_name,0 ));
		...
		if(str_replace("'","",$delivery_basis)==3)
		{
			...
			$field_array_dtls="id,delivery_mst_id,mst_id,production_type,color_size_break_down_id,production_qnty,cut_no,bundle_no,barcode_no,is_rescan,color_type_id,cost_of_fab_per_pcs,cut_oh_per_pcs,cost_per_pcs,total_cost_of_fabric";
			...
			foreach($dtlsArr as $bundle_no=>$qty)
			{
				...
				$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",2,'".$dtlsArrColorSize[$bundle_no]."','".$qty."','".$cut_no."','".$bundle_no."','".$bundleBarcodeArr[$bundle_no]."','".$bundleRescanArr[$bundle_no]."','".$bundle_wise_type_array[$bundle_no]."','".$cost_of_fab."','".$cutting_oh."','".$cost_per_pcs."','".$total_cost_of_fabric."')";
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$data_array_delivery="".$cbo_location."*".$cbo_embel_name."*".$cbo_embel_type."*".$cbo_source."*".$cbo_emb_company."*".$cbo_floor."*".$txt_organic."*".$txt_issue_date."*".$cbo_body_part."*".$cbo_working_company_name."*".$cbo_working_location."*".$user_id."*'".$pc_date_time."'*'".str_replace("'","",$txt_remarks)."'";
            ...
            $bundle="'".implode("','",$bundleCheckArr)."'";
            $receive_sql="select c.barcode_no,c.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls c where a.id=c.mst_id and a.production_type=2 and  a.embel_name=2 and c.barcode_no  in ($bundle)  and c.production_type=2 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and a.delivery_mst_id!=$mst_id and c.delivery_mst_id!=$mst_id and (c.is_rescan=0 or c.is_rescan is null)";
            $receive_result = sql_select($receive_sql);
            foreach ($receive_result as $row)
            {
		...
		if(str_replace("'","",$delivery_basis)==3)
		{
			...
			$issue_data_arr=production_data($mst_id,'2_2');
			// echo "10**<pre>";print_r($issue_data_arr);die;

			// $delete = execute_query("DELETE FROM pro_garments_production_mst WHERE delivery_mst_id=$mst_id and production_type=2 and embel_name=2");
			// $delete_dtls = execute_query("DELETE FROM pro_garments_production_dtls WHERE delivery_mst_id=$mst_id and production_type=2");
			//$delete_bundle = execute_query("DELETE FROM pro_cut_delivery_color_dtls WHERE delivery_mst_id=$mst_id and production_type=2 and embel_name=2");
			...
			foreach($dtlsArr as $bundle_no=>$qty)
			{
				...
				$cost_of_fab = $fab_cost_array[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['cost_of_fab_per_pcs'];
				$cost_of_fab = number_format($cost_of_fab,$dec_place[3],'.','');
				// echo "10**$cost_of_fab=".$colorSizedData[0]."=".$colorSizedData[1]."=".$colorSizedData[3]."<br>";
				$cutting_oh = $fab_cost_array[$colorSizedData[0]][$colorSizedData[1]][$colorSizedData[3]]['cut_oh_per_pcs'];
				$cutting_oh = number_format($cutting_oh,$dec_place[3],'.','');
		...
		else
		{
			...
			$data_array1="".$cbo_source."*".$cbo_emb_company."*".$cbo_location."*".$cbo_embel_name."*".$cbo_embel_type."*".$txt_issue_date."*".$txt_issue_qty."*2*".$sewing_production_variable."*'".$challan_no."'*".$txt_remark."*".$cbo_floor."*".$txt_cumul_issue_qty."*".$txt_yet_to_issue."*".$hid_order_id."*".$txt_embroidery_order_no."*".$user_id."*'".$pc_date_time."'";
	...
	?>
 		<table cellpadding="0" cellspacing="0" width="700" border="1" rules="all" class="rpt_table">
			<thead>
				...
				<!-- <th width="100">Cutting Rate</th> -->
				<th width="100">Cutting Cost Per Pcs</th>
			</thead>
			<tbody>
				<?
				$i=1;
		...
		<legend>Enter search words</legend>
            <table cellpadding="0" cellspacing="0" width="700" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
            		...
            		<td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_embel_type').value+'_'+document.getElementById('txt_company_id').value+'_'+document.getElementById('txt_cutt_no').value+'_'+document.getElementById('cbo_string_search_type').value, 'create_challan_search_list_view', 'search_div', 'embroidery_delivery_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                     </td>
                </tr>
           </table>
...
if($action=="create_challan_search_list_view")
{
	...
	if($search_type==1){
		$search_field_cond=" and a.sys_number '$search_string'";
	    if($data[3]!=""){ $cut_no=" and b.cut_no '$data[3]' ";} else {$cut_no="";}
...
{
	$data_array=sql_select("select a.id, a.company_id, a.sys_number, a.embel_type, a.embel_name, a.production_source, a.serving_company, a.location_id, a.floor_id, a.organic, a.delivery_date,a.body_part,a.working_company_id,a.working_location_id,a.remarks,b.wo_order_id,b.wo_order_no from pro_gmts_delivery_mst a,pro_garments_production_mst b where a.id=b.delivery_mst_id and a.id='$data' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");

	// echo "select a.id, a.company_id, a.sys_number, a.embel_type, a.embel_name, a.production_source, a.serving_company, a.location_id, a.floor_id, a.organic, a.delivery_date,a.body_part,a.working_company_id,a.working_location_id,a.remarks,b.wo_order_id,b.wo_order_no from pro_gmts_delivery_mst a,pro_garments_production_mst b where a.id=b.delivery_mst_id and id='$data' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0"; die;
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_drop_down( 'requires/embroidery_delivery_entry_controller', '".$row[csf('production_source')]."', 'load_drop_down_embro_issue_source_new', 'emb_company_td' );\n";
		echo "load_drop_down( 'requires/embroidery_delivery_entry_controller', '".$row[csf('serving_company')]."', 'load_drop_down_location', 'location_td' );\n";
		echo "$('#cbo_emb_company').val('".$row[csf('serving_company')]."');\n";
		echo "$('#cbo_location').val('".$row[csf('location_id')]."');\n";
		echo "load_drop_down( 'requires/embroidery_delivery_entry_controller', '".$row[csf('location_id')]."', 'load_drop_down_floor', 'floor_td' );\n";
		echo "load_drop_down( 'requires/embroidery_delivery_entry_controller', '".$row[csf('working_company_id')]."', 'load_drop_down_working_location', 'working_location_td' );\n";
		...
		echo "$('#txt_embroidery_order_no').val('".$row[csf('wo_order_no')]."');\n";
		echo "$('#hid_order_id').val('".$row[csf('wo_order_id')]."');\n";

		echo "disable_enable_fields('cbo_company_name*cbo_source*cbo_floor*txt_embroidery_order_no*cbo_location',1);\n";
...
<div style="width:930px;">
    <table width="900" cellspacing="0"  style="font: 12px tahoma;">
        ...
        <?
			...
			if($data[2]==3)
			{
				...
				and a.id=b.mst_id and b.color_size_break_down_id=d.id and a.production_type=2 and a.embel_name=2 and b.status_active=1 and b.is_deleted=0 and d.status_active =1
				and d.is_deleted=0
    ...
    <table cellspacing="0" width="900"  border="1" rules="all" class="rpt_table" style=" margin-top:20px; font: 12px tahoma;">
        ...
        <br>
	 <?
        echo signature_table(242, $data[0], "900px");
     ?>
	</div>
	</div>
...
if($action=="emblishment_issue_print_2")
{
	...
	?>
	<div style="width:930px;">
	    <table cellspacing="0" style="font: 12px tahoma;">
	        <tr>
	            <td colspan="6" align="center" style="font-size:24px"><strong><? echo $company_library[$data[0]]; ?></strong></td>
	        </tr>
	        <tr class="form_caption">
	       ...
	       <tr>
	        ...
	        <?
				...
				if($data[2]==3)
				{
					...
					/*$sql="SELECT sum(b.production_qnty) as production_qnty,a.po_break_down_id,a.item_number_id,a.country_id,b.bundle_no,d.size_number_id, d.color_number_id, count(b.id) as num_of_bundle,
						(select sum(c.number_start) from ppl_cut_lay_bundle c where  b.bundle_no = c.bundle_no) number_start,
						(select sum(c.number_end) from ppl_cut_lay_bundle c where  b.bundle_no = c.bundle_no) number_end
						from pro_garments_production_mst a, pro_garments_production_dtls b, wo_po_color_size_breakdown d
						where a.delivery_mst_id ='$data[1]'
						...
						order by a.po_break_down_id,d.color_number_id ";*/
						$sql="SELECT sum(b.production_qnty) as production_qnty,a.po_break_down_id,a.item_number_id,a.country_id,b.bundle_no,d.size_number_id, d.color_number_id, count(b.id) as num_of_bundle, sum(c.number_start) as number_start,sum(c.number_end) as number_end
						from pro_garments_production_mst a, pro_garments_production_dtls b,ppl_cut_lay_bundle c, wo_po_color_size_breakdown d
						where a.delivery_mst_id ='$data[1]'
						and a.id=b.mst_id and b.color_size_break_down_id=d.id and a.production_type=2 and a.embel_name=2 and b.bundle_no=c.bundle_no and d.po_break_down_id=c.order_id and b.status_active=1 and b.is_deleted=0 and d.status_active =1
						and d.is_deleted=0 and c.status_active=1
						group by a.po_break_down_id,a.item_number_id,a.country_id,b.bundle_no,d.color_number_id,d.size_number_id
						order by a.po_break_down_id,d.color_number_id ";
				}
				else
				{
					$sql="SELECT sum(a.production_qnty) as production_qnty,c.po_break_down_id,c.item_number_id,c.country_id, b.color_number_id, sum(c.number_start) as number_start,sum(c.number_end) as number_end
					from pro_garments_production_dtls a, wo_po_color_size_breakdown b,ppl_cut_lay_bundle c,pro_garments_production_mst c
					where c.delivery_mst_id ='$data[1]'
					and c.id=a.mst_id  and a.color_size_break_down_id=b.id and b.bundle_no=c.bundle_no and d.po_break_down_id=c.order_id and a.status_active=1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 and c.status_active=1
					group by c.po_break_down_id,c.item_number_id,c.country_id,b.color_number_id ";
				}
				$result=sql_select($sql);
			?>
	            ...
	            $tot_qnty=array();
	                foreach($result as $val)
	                {
	                    ...
	                    <tr bgcolor="<? echo $bgcolor; ?>"  style="font-size:12px">
	                        ...
	                        <td align="right"><?  echo $val[csf('production_qnty')] - $val[csf('REJECT_QNTY')]; ?></td>
	                    </tr>
	                    <?
						$production_quantity+=$val[csf('production_qnty')];
						$total_bundle+=$val[csf('num_of_bundle')];
...
if($db_type==2) $group_concat="  listagg(cast(b.cut_no AS VARCHAR2(4000)),',') within group (order by b.cut_no) as cut_no" ;
			...
			if($data[2]==3)
			{
				if($db_type==0)
				{
					...
					from pro_garments_production_mst a, pro_garments_production_dtls b, wo_po_color_size_breakdown d
					where a.delivery_mst_id ='$data[1]' and a.id=b.mst_id and a.production_type=2 and a.embel_name=2 and b.color_size_break_down_id=d.id and b.status_active=1 and b.is_deleted=0 and d.status_active =1
				...
				else
				{
					...
					where a.delivery_mst_id ='$data[1]' and a.id=b.mst_id and a.production_type=2 and a.embel_name=2 and b.bundle_no = c.bundle_no and b.color_size_break_down_id=d.id and b.status_active=1 and b.is_deleted=0 and d.status_active =1
...
if($action=="emblishment_issue_print_4")
{
	...
	<div style="width:900px;">
	    <table cellspacing="0" style="font: 11px tahoma; width: 100%;">
	        ...
	        <tr>
	            <td colspan="6" align="center" style="font-size:20px"><u><strong>Embroidery Issue Challan/Gate Pass</strong></u></td>
	        </tr>
	        <tr>
	            <td width="90"><strong>Challan No</strong></td> <td width="140px">: <? echo $dataArray[0][csf('sys_number')]; ?></td>
	            <td width="110"><strong>Embel. Name</strong></td><td width="175px"> : <? echo $emblishment_name_array[$dataArray[0][csf('embel_name')]]; ?></td>
	        ...
	        <?
				$delivery_mst_id =$dataArray[0][csf('id')];
					...
					where
						...
						and a.production_type=2 and a.embel_name=2
	    ...
	    <div style="width:100%;">
	    <table cellspacing="0" width="950" border="1" rules="all" class="rpt_table" style="font: 12px tahoma;">
	    ...
	    <tr bgcolor="#dddddd" align="center">
	        ...
	        <tr bgcolor="#dddddd" align="center">
	            ...
	            <th width="100" align="center">Buyer / Brand</th>
	        ...
	        <tbody>
				...
				foreach($dataArr as $key=>$row)
				{
					...
					<tr bgcolor="<? echo $bgcolor; ?>"  style="font-size:12px">
						<td align="center"><? echo $i;  ?></td>
						<td align="center" title="<? echo $buyer_arr[$row[buyer_name]];?>"><p><? echo $buyer_code_arr[$row[buyer_name]]." / ".$brand_arr[$row[brand_id]]; ?></p></td>
	        ...
	        <tr>
	         ...
	         <br>
	            ...
	            <br>
				<?
	                  echo signature_table(242, $data[0], "900px",$cbo_template_id,20,$user_library[$insert_by]);
	            ?>
		</div>
		</div>
	   	<script type="text/javascript" src="../../js/jquery.js"></script>
 ...
 if($action=="emblishment_issue_print_4_bk")
{
	extract($_REQUEST);
	$data=explode('*',$data);
	//print_r ($data);
...
if($action=="emblishment_issue_print_5")
{
	...
	$cut_lay_arr=array();
	$lay_sql="select a.cutting_no, a.floor_id, b.order_id, b.order_cut_no, b.color_id, b.gmt_item_id from ppl_cut_lay_mst a, ppl_cut_lay_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	$lay_sql_data=sql_select($lay_sql);
	foreach($lay_sql_data as $row)
	{
		$cut_lay_arr[$row[csf('cutting_no')]][$row[csf('color_id')]].=$row[csf('order_cut_no')].',';
	...
	$sql="select id,remarks, sys_number_prefix, sys_number_prefix_num, sys_number, company_id, production_type, location_id, delivery_basis, embel_name, embel_type,inserted_by,
	production_source, serving_company, floor_id, organic, delivery_date,body_part,working_company_id,working_location_id from pro_gmts_delivery_mst where production_type=2 and id='$data[1]' and
	status_active=1 and is_deleted=0 ";
	$dataArray=sql_select($sql);
 ...
 <div style="width:900px;">
    <table cellspacing="0" style="font: 11px tahoma; width: 100%;">
       ...
       <tr>
        ...
        <tr>
        	<td  colspan="4" id="barcode_img_id">
			<br>
			  <span style="font-size:0.7rem; font-weight:bold">Remarks :  </span><? echo $dataArray[0]['REMARKS']; ?>
			</td>
        ...
        <?
			$delivery_mst_id =$dataArray[0][csf('id')];
				...
				$sql="SELECT b.cut_no, b.production_qnty, a.country_id, d.color_number_id, d.size_number_id, e.style_ref_no, e.style_description, e.job_no, e.buyer_name,f.po_number,f.grouping,f.id as po_id, f.po_quantity,h.order_cut_no,b.bundle_no
				from
					pro_garments_production_mst a,
					pro_garments_production_dtls b,
					wo_po_color_size_breakdown d,
        ...
        <tr>
           ...
           <br><br>
            ...
            <table cellspacing="0" width="300" border="1" rules="all" class="rpt_table" style="font: 12px tahoma;">
                ...
                <tbody>
				<?
				/* $sql="SELECT c.id as prodid, d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, MAX($year_field) as year, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.cut_no,c.bundle_no, sum(c.production_qnty) as production_qnty, e.po_number,c.barcode_no from pro_garments_production_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name='$ex_data[3]' and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no  and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and d.status_active =1 and d.is_deleted=0 and e.status_active =1 and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 $bundle_nos_cond $operation_conds $item_id group by d.id, e.id, f.job_no_prefix_num, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no,c.barcode_no, e.po_number,c.id order by c.cut_no, length(c.bundle_no) asc, c.bundle_no asc";
				*/
				$sql2="select d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.cut_no,c.bundle_no,c.barcode_no,c.production_qnty,c.is_rescan, e.po_number from pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where  c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no  and c.production_type=2 and c.delivery_mst_id=".$data[1]." and c.status_active=1 and c.is_deleted=0 order by  c.cut_no,length(c.bundle_no) asc, c.bundle_no asc ";
 ...
 <div style="width:900px;">
       ...
       <!--  <td ><strong> Barcode  : </strong></td> <td  id="barcode_img_id"></td> -->
        ...
        <?
			$delivery_mst_id =$dataArray[0][csf('id')];
				...
				$sql="SELECT b.cut_no, b.production_qnty, a.country_id, d.color_number_id, d.size_number_id,d.item_number_id, e.style_ref_no, e.style_description, e.job_no, e.buyer_name,f.po_number,f.id as po_id, f.po_quantity,h.order_cut_no,b.bundle_no,f.grouping
				from
					pro_garments_production_mst a,
					pro_garments_production_dtls b,
					wo_po_color_size_breakdown d,
				...
				foreach($result as $rows)
				{
					...
					$dataArr[$key]=array(
						...
						cut_no=>$rows[csf('cut_no')],
						order_cut_no=>$rows[csf('order_cut_no')],
						internal_ref=>$rows[csf('grouping')]
					);
					$orderCutArr[$key][$rows[csf('order_cut_no')]]=$rows[csf('order_cut_no')];
					$productionQtyArr[$key]+=$rows[csf('production_qnty')];
					$sizeQtyArr[$key][$rows[csf('size_number_id')]]+=$rows[csf('production_qnty')];
    ...
    <tr bgcolor="#dddddd" align="center">
        ...
        <tr bgcolor="#dddddd" align="center">
            ...
            <th width="80" align="center">Style Ref</th>
            <th width="100" align="center">Style Des</th>
			<th width="130" align="center">IR No</th>
        ...
        <tbody>
			...
			foreach($dataArr as $key=>$row)
			{
				...
				<tr bgcolor="<? echo $bgcolor; ?>"  style="font-size:12px">
					...
					<td align="center" width=""><p><? echo $row['internal_ref']; ?></p></td>
