...
function load_drop_down_suplier($data){
	...
	if($data==5 || $data==3){
		...
		$cbo_supplier_name= create_drop_down( "cbo_supplier_name", 152, "select id,company_name from lib_company where status_active=1 and is_deleted=0 order by company_name", "id,company_name",1, "-- Select Supplier --", "", "validate_suplier()",0,"" );
	}
	else{
		$cbo_supplier_name= create_drop_down( "cbo_supplier_name", 152, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=9 and   a.status_active =1 and a.is_deleted=0 group by a.id,a.supplier_name order by supplier_name","id,supplier_name", 1, "-- Select Supplier --", $selected, "",0 );
	}
...
if ($action=="load_drop_down_buyer_popup"){
	...
	if($within_group == 1)
	{
		echo create_drop_down( "cbo_buyer_name", 152, "select id,company_name from lib_company where status_active=1 and is_deleted=0 order by company_name","id,company_name", 1, "-- Select Buyer --", $selected, "" );
	}
	else
	{
		if($company != 0)
		{
			echo create_drop_down( "cbo_buyer_name", 152, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and b.buyer_id=buy.id and b.tag_company='$company' and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) group by buy.id,buy.buyer_nameorder by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "" );
			exit();
		}
		else
	    {
...
{
	echo create_drop_down( "cbo_season_id", 152, "select id, season_name from lib_buyer_season where buyer_id='$data' and status_active =1 and is_deleted=0 order by season_name ASC","id,season_name", 1, "-- Select Season--", "", "" );
	exit();
}
if ($action=="load_drop_down_brand")
{
	...
	echo create_drop_down( "cbo_brand_id", 152, "select id, brand_name from lib_buyer_brand brand where buyer_id='$data' and status_active =1 and is_deleted=0 $brand_cond order by brand_name ASC","id,brand_name", 1, "--Brand--", $selected, "" );
	exit();
}
if ($action=="load_drop_down_suplier"){
	if($data==5 || $data==3){
	echo create_drop_down( "cbo_supplier_name", 152, "select id,company_name from lib_company where status_active=1 and is_deleted=0 order by company_name", "id,company_name",1, "-- Select Supplier --", "", "validate_suplier()",0,"" );
	}
	else{
...
if ($action=="fabric_booking_popup"){
	...
	<body>
        ...
        <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off" style="justify-content: center;text-align: center;">
        	<center>
	            <table width="680" cellspacing="0" cellpadding="0" border="1" class="rpt_table" rules="all" style="justify-content: center;text-align: center;">
	                <thead>
	                ...
	                <tr class="general">
	                    ...
	                    if(empty($company))
	                    {
	                    	$call_buyer="load_drop_down( 'woven_grey_fabric_purchase_booking_controller', this.value+'_'+".$cbo_within_group.", 'load_drop_down_buyer_popup', 'buyer_td' );";
	                    }
	                    ...
	                    <?
	                      ...
	                      {
	                      	echo create_drop_down( "cbo_buyer_name", 130, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Buyer --",$cbo_buyer_name );
	                      }
	                      else
	                      {
	                      	echo create_drop_down( "cbo_buyer_name", 130, "select  buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) group by buy.id, buy.buyer_name order by buyer_name","id,buyer_name", 1, "-- Select Buyer --",$cbo_buyer_name );
	                      }
	                     
	                     ?>
	                    </td>
	                    <td><input name="txt_fso_no" id="txt_fso_no" class="text_boxes" style="width:80px"></td>
	                    <td><input name="txt_booking_no" id="txt_booking_no" class="text_boxes" style="width:80px"></td>
	                    ...
	                    <td align="center">
	                    <input type="button" style="width:70px;" name="button2" class="formbutton" value="Show" 
	                    onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_fso_no').value+'_'+document.getElementById('txt_booking_no').value+'_'+document.getElementById('chk_job_wo_po').value, 'create_booking_search_list_view', 'search_div', 'woven_grey_fabric_purchase_booking_controller','setFilterGrid(\'list_view\',-1)')"  /></td>
	                </tr>
	                <tr>
...
if ($action=="load_drop_down_buyer")
{
	...
	if ($data[0] == 1)
	{
		echo create_drop_down("cbo_buyer_name", 152, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0  order by comp.company_name", "id,company_name", 1, "-- Select Buyer --", 0, "", 0);
	}
	else if ($data[0] == 2)
	{
		if($data[1] != 0)
		{
			echo create_drop_down( "cbo_buyer_name", 152, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 $buyer_cond and b.buyer_id=buy.id and b.tag_company='$data[1]' and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) group by buy.id,buy.buyer_name order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "" );
			exit();
		}
		else
		{
...
if ($action=="create_booking_search_list_view"){
	...
	if (str_replace("'","",$data[4])!="") $job_cond=" and b.job_no like '%$data[4]'    "; else  $job_cond="";
	...
	if($data[6]==0)
	{
		$sql="SELECT a.id, a.booking_no_prefix_num, a.pay_mode, a.booking_no, a.company_id, a.buyer_id, a.booking_date, a.delivery_date, a.item_category, a.fabric_source, a.supplier_id, a.is_approved, a.ready_to_approved from wo_booking_mst a,wo_booking_dtls b where a.booking_no=b.booking_no and a.entry_form=549 and  b.status_active =1 and b.is_deleted=0 $company $buyer $job_cond $booking_cond $booking_date  order by a.id DESC";
	}
	else
	{
		$sql="SELECT a.id, a.job_no as job_no_prefix_num, a.booking_no_prefix_num, a.pay_mode, a.booking_no, company_id, a.buyer_id, a.supplier_id, a.booking_date, a.delivery_date, a.item_category, a.fabric_source, a.is_approved from wo_booking_mst a
		   where a.booking_no not in ( select a.booking_no from  wo_booking_mst a , wo_booking_dtls b, wo_po_break_down c where a.booking_no=b.booking_no  and b.po_break_down_id=c.id and  a.booking_type=7 and a.entry_form=549 and  a.status_active =1 and a.is_deleted=0  and  b.status_active =1 and b.is_deleted=0 $company $buyer $job_cond $booking_cond $booking_date group by a.booking_no_prefix_num, a.booking_no,company_id,a.supplier_id,a.booking_date,a.delivery_date  ) and a.booking_type=7 and a.entry_form=549 and  a.status_active =1 and a.is_deleted=0
		   $company $buyer  $booking_cond $booking_date group by a.id, a.booking_no_prefix_num, a.booking_no,a.job_no,company_id,a.buyer_id,a.supplier_id,a.pay_mode,a.booking_date,a.delivery_date,a.item_category,a.fabric_source,a.is_approved order by a.id DESC";
	}
	//echo $sql; die;
	  ?>
...
if ($action=="populate_data_from_search_popup")
{
	 ...
	 foreach ($data_array as $row)
	 {
		...
		echo "load_drop_down( 'requires/woven_grey_fabric_purchase_booking_controller','".$row[csf("within_group")]."_".$row[csf("company_id")]."', 'load_drop_down_buyer', 'buyer_td' );\n";
		...
		echo "document.getElementById('cbo_agent_name').value = '".$row[csf("agent_id")]."';\n";
	...
	<script>
		...
		function js_set_value( str ) {
			if($("#search"+str).css("display") !='none'){
				...
				if( !selected_id.includes($('#txt_fso_id' + str).val()) || !selected_item.includes($('#txt_fso_dtls_id' + str).val()) || !selected_po.includes($('#txt_fso_yarn_id' + str).val())) {
					selected_id.push( $('#txt_fso_id' + str).val() );
					selected_item.push($('#txt_fso_dtls_id' + str).val());
					selected_po.push($('#txt_fso_yarn_id' + str).val());
					console.log(1)
				}
				else
				{
					for( var i = 0; i < selected_id.length; i++ ) {
						if( selected_id[i] == $('#txt_fso_id' + str).val() ) break;
					}
			...
			for( var i = 0; i < selected_id.length; i++ ) {
				id += selected_id[i] + ',';
				txt_fso_dtls_id+=selected_item[i]+ ',';
				txt_fso_yarn_id+=selected_po[i]+ ',';
			}
			id = id.substr( 0, id.length - 1 );
			txt_fso_dtls_id = txt_fso_dtls_id.substr( 0, txt_fso_dtls_id.length - 1 );
			txt_fso_yarn_id = txt_fso_yarn_id.substr( 0, txt_fso_yarn_id.length - 1 );
			$('#txt_fso_id').val( id );
			$('#txt_fso_dtls_id').val( txt_fso_dtls_id );
			$('#txt_fso_yarn_id').val( txt_fso_yarn_id );
		}
                ...
                <table  width="600" class="rpt_table" align="center" rules="all">
                    ...
                    <tr>
                        ...
                        <? 
                        	...
                        	if($cbo_within_group == 1)
                        	{
                        		echo create_drop_down( "cbo_buyer_name", 150, "select id,company_name from lib_company where status_active=1 and is_deleted=0","id,company_name", 1, "-- Select Buyer --", str_replace("'","",$cbo_buyer_name), "","1" );
                        	}
                        	else
                        	{
                        ...
                        <td><input name="txt_fso_no" id="txt_fso_no" class="text_boxes" style="width:100px"></td>
                        <td><input name="txt_booking_no" id="txt_booking_no" class="text_boxes" style="width:110px"></td>
                        ...
                        <input type="hidden" name="cbo_within_group" id="cbo_within_group" class="text_boxes" style="width:60px" value="<? echo $cbo_within_group; ?>"  />
                        ...
                        <input type="button" name="button2" class="formbutton" value="Show" 
						 onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('cbo_job_year').value+'_'+document.getElementById('txt_fso_no').value+'_'+document.getElementById('txt_booking_no').value+'_'+document.getElementById('cbo_currency').value+'_'+document.getElementById('cbo_fabric_natu').value+'_'+document.getElementById('cbo_fabric_source').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+document.getElementById('cbo_supplier_name').value+'_'+document.getElementById('cbo_within_group').value, 'fabric_search_list_view', 'search_div', 'woven_grey_fabric_purchase_booking_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" />
                        </td>
                    </tr>
                    <tr>
                        <td colspan="11" align="center">
                        <input type="hidden" class="text_boxes" readonly style="width:550px" id="txt_fso_id">
                        <input type="hidden" id="txt_fso_dtls_id">
                        <input type="hidden" id="txt_fso_yarn_id">
                        </td>
                    </tr>
                </table>
                </td>
...
if ($action=="fabric_search_list_view"){
	...
	$cbo_fabric_source=$data[7];
	$search_category=$data[8];
	$cbo_supplier=$data[9];
	$cbo_within_group=$data[10];

	
	
	
	
	if ($company!=0) $company_cond=" and a.company_id='$company'"; else { echo "Please Select Company First."; die; }
	if ($buyer!=0)   $buyer_cond=" and a.buyer_id='$buyer'"; else{ echo "Please Select Buyer First."; die; }
	if ($cbo_currency!="")   $currency_cond=" and a.currency_id='$cbo_currency'"; else{ echo "Please Select Currency First."; die; }

	if ($cbo_within_group!="")   $within_group_cond=" and a.within_group='$cbo_within_group'"; else{ echo "Please Select  Within Group First."; die; }
	

	if($db_type==0)
	{
		if ($date_from!="" &&  $date_to!="") $booking_date = "and a.booking_date between '".change_date_format($date_from, "yyyy-mm-dd", "-")."' and '".change_date_format($date_to, "yyyy-mm-dd", "-")."'"; else $booking_date ="";
	}
	else if($db_type==2)
	{
		if ($date_from!="" &&  $date_to!="") $booking_date = "and a.booking_date between '".change_date_format($date_from, "yyyy-mm-dd", "-",1)."' and '".change_date_format($date_to, "yyyy-mm-dd", "-",1)."'"; else $booking_date ="";
	}
	...
	if (str_replace("'","",$fso_no)=="" && str_replace("'","",$booking_no)=="" )
	{
		//echo "<span style='color:red; font-weight:bold; font-size:20px; text-align:center'>Please Insert FSO or Booking or Date First.";
		//die;
	}

	$cu_sql=sql_select("SELECT b.PRE_COST_FABRIC_COST_DTLS_ID,b.PO_BREAK_DOWN_ID,b.COLOR_SIZE_TABLE_ID,b.GREY_FAB_QNTY FROM WO_BOOKING_MST a,WO_BOOKING_DTLS b WHERE a.booking_no=b.booking_no and a.entry_form=549 and a.is_deleted=0 and b.is_deleted=0");
	$cu_booking_data_arr=array();
	foreach ($cu_sql as $row)
	{
		$cu_booking_data_arr[$row[csf('PRE_COST_FABRIC_COST_DTLS_ID')]][$row[csf('PO_BREAK_DOWN_ID')]][$row[csf('COLOR_SIZE_TABLE_ID')]]+=$row[csf('GREY_FAB_QNTY')];
	}
	...
	{
		if (str_replace("'","",$fso_no)!="") $job_cond=" and a.job_no_prefix_num='$fso_no'"; //else  $job_cond="";
		if (str_replace("'","",$booking_no)!="") $order_cond=" and a.sales_booking_no = '$booking_no'"; //else  $order_cond="";
	}
	else if($search_category==2)
	{
		if (str_replace("'","",$fso_no)!="") $job_cond=" and a.job_no_prefix_num like '$fso_no%'"; //else  $job_cond="";
		if (str_replace("'","",$booking_no)!="") $order_cond=" and a.sales_booking_no like '$booking_no%'  "; //else  $order_cond="";
	}
	...
	$sql= "SELECT a.id as id,
		       a.job_no as job_no,
		       b.item_number_id as item_number_id,
		       b.body_part_id as body_part_id,
		       a.buyer_id as buyer_id,
		       b.determination_id as deter_id,
		       b.weight_type as weight_type,
		       b.width_dia_type as width_dia_type,
		       b.id as dtls_id,
		       c.id as yarn_dtls_id,
		       c.fabric_source as fabric_source,
		       b.dia,
       		   b.cutable_width,
       		   c.grey_qty,
       		   b.cons_uom,
		  ...
		  FROM fabric_sales_order_mst a,
		       ...
		       fabric_sales_order_yarn c
		 WHERE     a.id = b.mst_id
		       AND a.id = c.mst_id
		       AND a.entry_form = 547
		       AND c.fabric_source = 2
		       and c.deter_id=b.determination_id
		       $company_cond $within_group_con $job_cond $order_cond $buyer_cond $currency_cond $year_cond
		       ";
	//echo $sql;
	...
	$sql_deter = "SELECT a.id, a.construction, b.copmposition_id, b.percent,a.design,a.type from lib_yarn_count_determina_mst a, lib_yarn_count_determina_dtls b where a.id=b.mst_id and b.status_active=1 order by b.id";
	...
	if (count($data_array) > 0)
	{
		foreach ($data_array as $row)
		{
			...
			$construction_arr[$row[csf('id')]]['construction'] = $row[csf('construction')];
			$construction_arr[$row[csf('id')]]['design'] = $row[csf('design')];
			$construction_arr[$row[csf('id')]]['type'] = $row[csf('type')];
		}
	}
	?>
     	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1083" class="rpt_table">
            <thead>
                <th width="30">SL</th>
                <th width="80">FSO No</th>
                <th width="80">Item</th>
     ...
     <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1083" class="rpt_table" id="list_view">
    <?
	$i=1;
	$booked = 0;
	foreach($sql_data as $sql_row){
		$reqQty=$sql_row[csf('grey_qty')];
		


		$cuBooking=$cu_booking_data_arr[$sql_row[csf('id')]][$sql_row[csf('dtls_id')]][$sql_row[csf('yarn_dtls_id')]];
		$balQty=number_format($reqQty-$cuBooking,4,".","");
		//echo "<pre>".$balQty."=".$reqQty."-".$cuBooking."</pre>";

		if($balQty > 0 ){ ?>
	        <tr style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value(<? echo $i;?>)">
	        <td width="30">
			<? echo $i; ?>
	        <input type="hidden" name="txt_fso_id" id="txt_fso_id<?php echo $i ?>" value="<? echo $sql_row[csf('id')]; ?>"/>
	        <input type="hidden" name="txt_fso_dtls_id" id="txt_fso_dtls_id<?php echo $i ?>" value="<? echo $sql_row[csf('dtls_id')]; ?>"/>
	        <input type="hidden" name="txt_fso_yarn_id" id="txt_fso_yarn_id<?php echo $i ?>" value="<? echo $sql_row[csf('yarn_dtls_id')]; ?>"/>
	        </td>
	        <td width="80"><? echo $sql_row[csf('job_no')]; ?></td>
	        <td width="80"><? echo $garments_item[$sql_row[csf('item_number_id')]]; ?></td>
	        <td width="100"><? echo $body_part[$sql_row[csf('body_part_id')]]; ?></td>
	        <td width="100"><? echo $construction_arr[$sql_row[csf('deter_id')]]['type']; ?></td>
	        <td width="100"><? echo $sql_row[csf('construction')]; ?></td>
	        <td width="100"><? echo $construction_arr[$sql_row[csf('deter_id')]]['design']; ?></td>
	        <td width="70"><? echo $composition_arr[$sql_row[csf('deter_id')]]; ?></td>
	        <td width="80"><?=$sql_row[csf('dia')];?></td>
	        <td width="70"><?=$fabric_weight_type[$sql_row[csf('weight_type')]];?></td>
	        <td width="100"><? echo $item_category[3]; ?></td>
	        <td width="100"><? echo $fabric_source[$sql_row[csf('fabric_source')]]; ?></td>
	        <td width=""><? echo $unit_of_measurement[$sql_row[csf('cons_uom')]]; ?></td>
	        </tr>
	        <?
			$i++;
		} else{
        ...
        <div align="center" style="width:100%;" >
            ...
            <table width="750"  align="center" rules="all">
                ...
                <table  width="750" class="rpt_table" align="center" rules="all">
                    ...
                    <tr>
                        <td><? echo create_drop_down( "cbo_company_mst", 140, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "- Select Company -", str_replace("'","",$cbo_company_mst), "","1");//load_drop_down( 'woven_grey_fabric_purchase_booking_controller', this.value, 'load_drop_down_buyer_popup', 'buyer_td' ); ?>
                        </td>
                        <td id="buyer_td">
                        ...
                        <input type="hidden" name="cbo_fabric_source" id="cbo_fabric_source" class="text_boxes" style="width:60px" value="<? echo str_replace("'","",$cbo_fabric_source); ?>"  />
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_job_prifix').value+'_'+document.getElementById('txt_order_search').value+'_'+document.getElementById('txt_style').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+document.getElementById('txt_internal_ref').value+'_'+document.getElementById('txt_file_no').value+'_'+document.getElementById('cbo_job_year').value+'_'+document.getElementById('cbo_currency').value+'_'+document.getElementById('cbo_fabric_natu').value+'_'+document.getElementById('cbouom').value+'_'+document.getElementById('cbo_fabric_source').value, 'create_job_search_list_view', 'search_div', 'woven_grey_fabric_purchase_booking_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" />
                        </td>
                    </tr>
                    <tr>
                        <td colspan="11" align="center">
        ...
        <div align="center" style="width:100%;" >
            ...
            <table width="940"  align="center" rules="all">
                ...
                <table  width="940" class="rpt_table" align="center" rules="all">
                    ...
                    <tr>
                        <td><? echo create_drop_down( "cbo_company_mst", 140, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "- Select Company -", str_replace("'","",$cbo_company_name), "load_drop_down( 'woven_grey_fabric_purchase_booking_controller', this.value, 'load_drop_down_buyer_popup', 'buyer_td' );","1"); ?>
                        </td>
                        <td id="buyer_td">
                        <? echo create_drop_down( "cbo_buyer_name", 150, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active=1 and buy.is_deleted=0 $buyer_cond and b.buyer_id=buy.id and b.tag_company=$cbo_company_name and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) group by buy.id,buy.buyer_name order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", str_replace("'","",$cbo_buyer_name), "","1" ); ?>
                        </td>
                        ...
                        <input type="hidden" name="cbouom" id="cbouom" class="text_boxes" style="width:60px" value="<? echo str_replace("'","",$cbouom); ?>"  />
                        <input type="hidden" name="cbo_fabric_source" id="cbo_fabric_source" class="text_boxes" style="width:60px" value="<? echo str_replace("'","",$cbo_fabric_source); ?>"  />
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_job_prifix').value+'_'+document.getElementById('txt_order_search').value+'_'+document.getElementById('txt_style').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+document.getElementById('txt_internal_ref').value+'_'+document.getElementById('txt_file_no').value+'_'+document.getElementById('cbo_job_year').value+'_'+document.getElementById('cbo_currency').value+'_'+document.getElementById('cbo_fabric_natu').value+'_'+document.getElementById('cbouom').value+'_'+document.getElementById('cbo_fabric_source').value, 'create_po_search_list_view', 'search_div', 'woven_grey_fabric_purchase_booking_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" />
                        </td>
                    </tr>
                    <tr>
                        <td colspan="11" align="center">
...
if ($action=="generate_fabric_booking")
{
	...
	$txt_fso_dtls_id=str_replace("'","",$txt_fso_dtls_id);
	$txt_fso_yarn_id=str_replace("'","",$txt_fso_yarn_id);
	...
	$cu_sql=sql_select("SELECT b.PRE_COST_FABRIC_COST_DTLS_ID,b.PO_BREAK_DOWN_ID,b.COLOR_SIZE_TABLE_ID,b.GREY_FAB_QNTY FROM WO_BOOKING_MST a,WO_BOOKING_DTLS b WHERE a.booking_no=b.booking_no and a.entry_form=549 and a.is_deleted=0 and b.is_deleted=0 and b.PRE_COST_FABRIC_COST_DTLS_ID=$txt_fso_id ");
	$cu_booking_data_arr=array();
	foreach ($cu_sql as $row)
	{
		$cu_booking_data_arr[$row[csf('PRE_COST_FABRIC_COST_DTLS_ID')]][$row[csf('PO_BREAK_DOWN_ID')]][$row[csf('COLOR_SIZE_TABLE_ID')]]+=$row[csf('GREY_FAB_QNTY')];
	...
	$sql= "SELECT a.id as id,
		 ...
		 WHERE     a.id = b.mst_id
		       ...
		       AND c.fabric_source = 2
		       and c.deter_id=b.determination_id
		       AND a.id in ($txt_fso_id)
		       AND b.id in ($txt_fso_dtls_id)
		       AND c.id in ($txt_fso_yarn_id)
		       ";
	//echo $sql; die;
	$sql_data=sql_select($sql);
	$deter_id_arr=array();
	foreach($sql_data as $sql_row)
	{
		$deter_id_arr[$sql_row[csf('deter_id')]]=$sql_row[csf('deter_id')];
	}

	$composition_arr = array();
	$construction_arr = array();

	$deter_id_cond=where_con_using_array($deter_id_arr,0,"a.id");
	$sql_deter = "SELECT a.id, a.construction, b.copmposition_id, b.percent,a.design,a.type,a.mill_ref,a.rd_no from lib_yarn_count_determina_mst a, lib_yarn_count_determina_dtls b where a.id=b.mst_id and b.status_active=1 $deter_id_cond order by b.id";
	$data_array = sql_select($sql_deter);
	if (count($data_array) > 0)
	{
		foreach ($data_array as $row)
		{
			...
			$construction_arr[$row[csf('id')]]['design'] = $row[csf('design')];
			$construction_arr[$row[csf('id')]]['type'] = $row[csf('type')];
			$construction_arr[$row[csf('id')]]['mill_ref'] = $row[csf('mill_ref')];
			$construction_arr[$row[csf('id')]]['rd_no'] = $row[csf('rd_no')];
		}
	}
	...
	<table width="2350" class="rpt_table" id="tbl_fabric_booking" border="0" rules="all">
	<thead>
			<tr>
				...
				<th width="100">Job CU WO Qty.</th>
				<th width="100">Balance Qty.</th>
				<th width="100">WO. Qty.</th>
				<th width="50">UOM</th>
				<th width="60">Rate</th>
				<th width="100">Amount</th>
				<th width="100">Total End</th>
				<th>Remark</th>
			</tr>
		</thead>
		<tbody>
			...
			foreach($sql_data as $sql_row)
			{
				$fso_id=$sql_row[csf('id')];
				$job_no=$sql_row[csf('job_no')];
				$fso_dtls_id=$sql_row[csf('dtls_id')];
				$yarn_dtls_id=$sql_row[csf('yarn_dtls_id')];
				$body_part_id=$sql_row[csf('body_part_id')];
				//$construction=$construction_arr[$sql_row[csf('deter_id')]]['construction'];
				$construction=$sql_row[csf('construction')];
				$design=$construction_arr[$sql_row[csf('deter_id')]]['design'];
				$type=$construction_arr[$sql_row[csf('deter_id')]]['type'];
				$mill_ref=$construction_arr[$sql_row[csf('deter_id')]]['mill_ref'];
				$rd_no=$construction_arr[$sql_row[csf('deter_id')]]['rd_no'];
				$compositi=$composition_arr[$sql_row[csf('deter_id')]];
				$gsm_weight=$sql_row[csf('dia')];
				$color_type_id=$sql_row[csf('color_type_id')];
				$width_dia_type=$sql_row[csf('width_dia_type')];

				$color_number_id=$sql_row[csf('color_id')];
				$item_color=$sql_row['contrast_color_id'];
				if($item_color== "" || $item_color==0)
				{
					$item_color=$sql_row[csf('color_id')];
				}
				$dia_width=$sql_row[csf('dia_width')];
				$pre_cost_remarks=$sql_row['remarks'];
				$req_qty=$sql_row[csf('grey_qty')];
				$cu_booking_qty= $cu_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id];
				$bal_qty=$req_qty - $cu_booking_qty ;//$cu_booking_qty;
				$rate = 0;
				$bal_amt=$bal_qty*$rate;
				$total_bal_amt += $bal_amt;


				$total_qo_qnty+=$req_qty;
				$total_ac_qnty+=$bal_qty;

				$uom_i=$sql_row[csf('cons_uom')];
				if($uom_unique=="")
				{
					$uom_unique =$uom_i;
				}
					...
					?>
						<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>">
							...
							<td  style="word-break: break-all;word-wrap: break-word;">
								...
								<input type="hidden" id="txtpoid_<? echo $i;?>" value="<? echo $fso_dtls_id;?>"  readonly  />
								<input type="hidden" id="txtcolorsizetableid_<? echo $i;?>" value="<? echo $yarn_dtls_id;?>"  readonly  />
								<? echo $body_part[$body_part_id]; ?>
								<input type="hidden" id="txtpre_cost_fabric_cost_dtls_id_<? echo $i;?>" value="<? echo $fso_id;?>"  readonly  />
							...
							<td >
								<?php echo $fabric_weight_type[$sql_row[csf('weight_type')]]; ?>
							</td>
							<td >
								<? echo $dia_width; ?>
							...
							<td>
								<?php echo $sql_row[csf('cutable_width')]; ?>
							</td>
							...
							<td>
								...
								<input type="text"  style="width:90%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; text-align:right; background-color:<? echo $bgcolor; ?>" id="txtreqqnty_<? echo $i;?>" value="<? echo number_format($req_qty,4,'.','');?>"  readonly  />

								
							</td>
							<td>
								<input type="text"  style="width:90%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; text-align:right; background-color:<? echo $bgcolor; ?>" id="cuqnty_<? echo $i;?>" value="<? echo number_format($cu_booking_qty,4,'.','');?>"  readonly  />
								<input type="hidden"  style="width:90%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtwoqprev_<? echo $i;?>" value="<? echo number_format($cu_booking_qty,4,'.','');?>" class="text_boxes_numeric" readonly />
							</td>
							<td >
								<input type="text"  style="width:80%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; text-align:right; background-color:<? echo $bgcolor; ?>" id="txtbalqnty_<? echo $i;?>" value="<? echo number_format($bal_qty,4,'.','');?>"  readonly  />
								<input type="hidden"  style="width:90%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtadj_<? echo $i;?>" value="<? //echo number_format($adj_qty,4,'.','');?>" class="text_boxes_numeric"  onChange="claculate_amount(<? echo $i; ?>)"  />
							</td>
							<td >
								<input type="hidden"  style="width:90%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtacwoq_<? echo $i;?>" onChange="claculate_amount(<? echo $i; ?>)" value="<? echo number_format($bal_qty,4,'.','');?>" class="text_boxes_numeric"   />
								<input type="text"  style="width:90%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtwoq_<? echo $i;?>" value="<? echo number_format($bal_qty,4,'.','');?>" onChange="claculate_amount(<? echo $i; ?>)" />
							</td>
							<td ><? echo $unit_of_measurement[$sql_row[csf('cons_uom')]]; ?></td>
							<td>
								<input type="text"  style="width:70%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtrate_<? echo $i;?>" value="0"  class="text_boxes_numeric" onChange="claculate_amount(<? echo $i; ?>)" data-pre-cost-rate="0" data-current-rate="0"  />
							</td>
							<td >
								<input type="text"  style="width:90%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtamount_<? echo $i;?>" value="" class="text_boxes_numeric" readonly  />

							</td>
							<td ><input type="text" style="width: 70px;" class="text_boxes" id="totalends_<? echo $i;?>" value=""   /></td>
							<td>
								<input type="text"  style="width:100%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  background-color:<? echo $bgcolor; ?>" id="txtremark_<? echo $i;?>" value="<? //echo number_format($bal_amt,4,'.','');?>" />
		...
		<tfoot>
			<tr>
				<?
					...
					{
						?>
							<th  colspan="16" align="right">Total: &nbsp;&nbsp;&nbsp;
								<input type="hidden" name="uniqe_uom" id="uniqe_uom" value="1">
							</th>
							<th  style="justify-content: right;text-align: right;" >
								<input style="width:90%;" type="text" id="total_qo_qnty" value="<? echo number_format($total_qo_qnty,4,'.',''); ?>" readonly   />
							</th>
							<th >&nbsp;</th>
							<th >&nbsp;</th>
							<th  style="justify-content: right;text-align: right;"> 
								<input  style="width:90%;" type="text" id="total_ac_qnty" value="<? echo number_format($total_ac_qnty,4,'.',''); ?>" readonly   />
							</th>
							<th >&nbsp;</th>
							<th >&nbsp;</th>
							<th  style="justify-content: right;text-align: right;">
								<input  style="width:90%;" type="text" id="total_bal_amt" value="<? echo number_format($total_bal_amt,4,'.',''); ?>" readonly   />
							</th>
							<th >&nbsp;</th>
							<th >&nbsp;</th>
						<?
					...
					{
						?>
							<th  colspan="21" align="right">Total Amount: &nbsp;&nbsp;&nbsp;
...
if ($action=="show_fabric_booking")
{
	...
	$txt_fso_dtls_id=str_replace("'","",$txt_fso_dtls_id);
	$txt_fso_yarn_id=str_replace("'","",$txt_fso_yarn_id);
	$booking_dtls_id=str_replace("'","",$booking_dtls_id);

	$cbouom=str_replace("'","",$cbouom);

	 $booking_data_arr=array();
	 $sql='SELECT a.pre_cost_fabric_cost_dtls_id AS "pre_cost_fabric_cost_dtls_id",a.po_break_down_id AS "po_break_down_id", a.color_size_table_id AS "color_size_table_id" ,a.id AS "booking_id",a.fin_fab_qnty AS "fin_fab_qnty",a.grey_fab_qnty AS "grey_fab_qnty", a.rate AS "rate", a.amount AS "amount", a.adjust_qty AS "adjust_qty",a.remark AS "remark",a.dia_width AS "dia_width",a.pre_cost_remarks AS "pre_cost_remarks",a.total_end as "total_end"  from wo_booking_dtls a  where a.booking_no='.$txt_booking_no.' and a.id='.$booking_dtls_id.' and a.pre_cost_fabric_cost_dtls_id='.$txt_fso_id.' and a.po_break_down_id='.$txt_fso_dtls_id.' and a.color_size_table_id='.$txt_fso_yarn_id.'  and a.booking_type=7 and a.status_active=1 and a.is_deleted=0';
	//echo $sql;
	$dataArray=sql_select($sql);
	$fso_id_arr=array();
	$fso_dtls_id_arr=array();
	$fso_yarn_id_arr=array();
	foreach($dataArray as $dataArray_row){
		

		$fso_id_arr[$dataArray_row['pre_cost_fabric_cost_dtls_id']]=$dataArray_row['pre_cost_fabric_cost_dtls_id'];
		$fso_dtls_id_arr[$dataArray_row['po_break_down_id']]=$dataArray_row['po_break_down_id'];
		$fso_yarn_id_arr[$dataArray_row['color_size_table_id']]=$dataArray_row['color_size_table_id'];


	}
	$pre_cost_fabric_cost_dtls_id_cond=where_con_using_array($fso_id_arr,0,"a.pre_cost_fabric_cost_dtls_id");
	$sql='SELECT a.pre_cost_fabric_cost_dtls_id AS "pre_cost_fabric_cost_dtls_id",a.po_break_down_id AS "po_break_down_id", a.color_size_table_id AS "color_size_table_id",a.fin_fab_qnty AS "fin_fab_qnty",a.grey_fab_qnty AS "grey_fab_qnty"  from wo_booking_dtls a,wo_booking_mst b  where a.booking_no=b.booking_no and b.entry_form=549   and b.booking_type=7 and a.status_active=1 and a.is_deleted=0  '.$pre_cost_fabric_cost_dtls_id_cond ;
	//echo $sql;
	$dataArray2=sql_select($sql);
	$cu_booking_data_arr=array();
	foreach ($dataArray2  as  $dataArray_row) {
		$cu_booking_data_arr[$dataArray_row['pre_cost_fabric_cost_dtls_id']][$dataArray_row['po_break_down_id']][$dataArray_row['color_size_table_id']]+=$dataArray_row['grey_fab_qnty'];
	}

	$fso_id_cond=where_con_using_array($fso_id_arr,0,"a.id");
	$fso_dtls_id_cond=where_con_using_array($fso_dtls_id_arr,0,"b.id");
	$fso_yarn_id_cond=where_con_using_array($fso_yarn_id_arr,0,"a.id");

	$sqldata= "SELECT a.id as id,
		       a.job_no as job_no,
		       b.item_number_id as item_number_id,
		       b.body_part_id as body_part_id,
		       a.buyer_id as buyer_id,
		 ...
		 WHERE     a.id = b.mst_id
		       ...
		       and c.deter_id=b.determination_id
		       AND a.id =$txt_fso_id
		       AND b.id =$txt_fso_dtls_id
		       AND c.id =$txt_fso_yarn_id
		       ";
	//echo $sqldata; //die;
	$sql_data=sql_select($sqldata);
	$deter_id_arr=array();
	$sales_booking_data_arr=array();
	foreach($sql_data as $row)
	{
		$deter_id_arr[$row[csf('deter_id')]]=$row[csf('deter_id')];
		$sales_booking_data_arr[$row[csf('id')]][$row[csf('dtls_id')]][$row[csf('yarn_dtls_id')]]['job_no']=$row[csf('job_no')];
		$sales_booking_data_arr[$row[csf('id')]][$row[csf('dtls_id')]][$row[csf('yarn_dtls_id')]]['item_number_id']=$row[csf('item_number_id')];
		$sales_booking_data_arr[$row[csf('id')]][$row[csf('dtls_id')]][$row[csf('yarn_dtls_id')]]['body_part_id']=$row[csf('body_part_id')];
		$sales_booking_data_arr[$row[csf('id')]][$row[csf('dtls_id')]][$row[csf('yarn_dtls_id')]]['buyer_id']=$row[csf('buyer_id')];
		$sales_booking_data_arr[$row[csf('id')]][$row[csf('dtls_id')]][$row[csf('yarn_dtls_id')]]['deter_id']=$row[csf('deter_id')];
		$sales_booking_data_arr[$row[csf('id')]][$row[csf('dtls_id')]][$row[csf('yarn_dtls_id')]]['weight_type']=$row[csf('weight_type')];
		$sales_booking_data_arr[$row[csf('id')]][$row[csf('dtls_id')]][$row[csf('yarn_dtls_id')]]['width_dia_type']=$row[csf('width_dia_type')];
		$sales_booking_data_arr[$row[csf('id')]][$row[csf('dtls_id')]][$row[csf('yarn_dtls_id')]]['fabric_source']=$row[csf('fabric_source')];
		$sales_booking_data_arr[$row[csf('id')]][$row[csf('dtls_id')]][$row[csf('yarn_dtls_id')]]['dia']=$row[csf('dia')];
		$sales_booking_data_arr[$row[csf('id')]][$row[csf('dtls_id')]][$row[csf('yarn_dtls_id')]]['cutable_width']=$row[csf('cutable_width')];
		$sales_booking_data_arr[$row[csf('id')]][$row[csf('dtls_id')]][$row[csf('yarn_dtls_id')]]['grey_qty']+=$row[csf('grey_qty')];
		$sales_booking_data_arr[$row[csf('id')]][$row[csf('dtls_id')]][$row[csf('yarn_dtls_id')]]['cons_uom']=$row[csf('cons_uom')];
		$sales_booking_data_arr[$row[csf('id')]][$row[csf('dtls_id')]][$row[csf('yarn_dtls_id')]]['color_type_id']=$row[csf('color_type_id')];
		$sales_booking_data_arr[$row[csf('id')]][$row[csf('dtls_id')]][$row[csf('yarn_dtls_id')]]['color_id']=$row[csf('color_id')];
		$sales_booking_data_arr[$row[csf('id')]][$row[csf('dtls_id')]][$row[csf('yarn_dtls_id')]]['avg_rate']=$row[csf('avg_rate')];
		$sales_booking_data_arr[$row[csf('id')]][$row[csf('dtls_id')]][$row[csf('yarn_dtls_id')]]['construction']=$row[csf('construction')];
		...
		<table width="2350" class="rpt_table" id="tbl_fabric_booking" border="0" rules="all">
			...
			<tbody>
				...
				foreach($dataArray as $sql_row)
				{
					...
					$fso_id=$sql_row['pre_cost_fabric_cost_dtls_id'];
					$fso_dtls_id=$sql_row['po_break_down_id'];
					$yarn_dtls_id=$sql_row['color_size_table_id'];

					$job_no=$sales_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id]['job_no'];
					$body_part_id=$sales_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id]['body_part_id'];
					$deter_id=$sales_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id]['deter_id'];
					$gsm_weight=$sales_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id]['dia'];
					$color_type_id=$sales_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id]['color_type_id'];
					$width_dia_type=$sales_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id]['width_dia_type'];
					$color_number_id=$sales_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id]['color_id'];
					$item_color=$sales_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id]['color_id'];
					$grey_qty=$sales_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id]['grey_qty'];
					$avg_rate=$sales_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id]['avg_rate'];
					$cons_uom=$sales_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id]['cons_uom'];
					$cutable_width=$sales_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id]['cutable_width'];
					$item_number_id=$sales_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id]['item_number_id'];
					$buyer_id=$sales_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id]['buyer_id'];
					$weight_type=$sales_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id]['weight_type'];
					$fabric_source=$sales_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id]['fabric_source'];
					
					//$construction=$construction_arr[$deter_id]['construction'];
					$construction=$sales_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id]['construction'];
					$design=$construction_arr[$deter_id]['design'];
					$type=$construction_arr[$deter_id]['type'];
					$mill_ref=$construction_arr[$deter_id]['mill_ref'];
					$rd_no=$construction_arr[$deter_id]['rd_no'];
					$compositi=$composition_arr[$deter_id];

					
					$dia_width=$gsm_weight;
					$pre_cost_remarks=$sql_row['remarks'];
					
					$cu_booking_qty=$cu_booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id];
					$booking_qty=$sql_row['grey_fab_qnty'];
					$ac_booking_qty=$booking_qty;//$booking_data_arr[$fso_id][$fso_dtls_id][$yarn_dtls_id];
					$adjust_qty=$sql_row['adjust_qty'];
					$remark=$sql_row['remark'];
					$rate=$sql_row['rate'];
					$amount=$sql_row['amount'];
					$booking_id=$sql_row['booking_id'];

					
					//$rate=$amount/$ac_booking_qty;
					$req_qty=$grey_qty;
					$req_amt=$avg_rate*$grey_qty;
					...
					?>
						<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>">
							<td  style="word-break:break-all" title="<? echo $rate.'-'.$amount; ?>">
								<? echo $job_no; ?>
								<input type="hidden" id="txtjob_<? echo $i;?>" value="<? echo $job_no;?>"  readonly  />
							</td>
							<td  style="word-break: break-all;word-wrap: break-word;">
							...
							<td >	
								<input type="text"  style="width:90%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; text-align:right; background-color:<? echo $bgcolor; ?>" id="txtreqqnty_<? echo $i;?>" value="<? echo number_format($req_qty,4,'.','');?>" class="text_boxes_numeric"  readonly  />
								
								
								
							</td>
							<td>
								<input type="text"  style="width:90%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; text-align:right; background-color:<? echo $bgcolor; ?>" id="cuqnty_<? echo $i;?>" value="<? echo number_format($cu_booking_qty,4,'.','');?>" class="text_boxes_numeric"   readonly  />
							...
							<td >
								<input type="text"  style="width:80%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px; text-align:right; background-color:<? echo $bgcolor; ?>" id="txtbalqnty_<? echo $i;?>" value="<? echo number_format($bal_qty,4,'.','');?>" class="text_boxes_numeric"   readonly  />
								<input type="hidden"  style="width:90%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtadj_<? echo $i;?>" value="<? echo number_format($adjust_qty,4,'.','');?>" class="text_boxes_numeric"   onChange="claculate_amount(<? echo $i; ?>)"  />
								
							</td>
							<td >
								<input type="text"  style="width:90%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtwoq_<? echo $i;?>" value="<? echo number_format($booking_qty,4,'.','');?>" class="text_boxes_numeric" onChange="claculate_amount(<? echo $i; ?>)"  />
								<input type="hidden"  style="width:90%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:#FFC" id="txtacwoq_<? echo $i;?>" value="<? echo number_format($ac_booking_qty,4,'.','');?>" class="text_boxes_numeric"   />
							</td>
							...
							<td >
								<input type="text"  style="width:90%;height:100%; font-family:Verdana, Geneva, sans-serif; font-size:11px;  text-align:right; background-color:<? echo $bgcolor; ?>" id="txtamount_<? echo $i;?>" value="<? echo number_format($amount,4,'.','');?>" class="text_boxes_numeric"   readonly  />

							</td>
							<td>
								<input type="text"  style="width:70px;height:100%; " id="totalends_<? echo $i;?>" value="<? echo $sql_row['total_end'];?>" class="text_boxes_numeric"    />
							</td>
							<td>
			...
			<tfoot>
				...
				if($flag==true)
				{
					?>
					<!-- 1570 -->
					<th  colspan="16" style="justify-content: left;text-align: left;">Total <input type="hidden" name="uniqe_uom" id="uniqe_uom" value="1"></th>
					<th  style="justify-content: right;text-align: right;" >
						<input type="text" style="width:80%;" class="text_boxes_numeric" id="total_qo_qnty" value="<? echo number_format($total_qo_qnty,4,'.',''); ?>" readonly   /></th>
					<th >&nbsp;</th>
					<th >&nbsp;</th>
					<th  style="justify-content: right;text-align: right;" > <input class="text_boxes_numeric" style="width:80%;" type="text" id="total_ac_qnty" value="<? echo number_format($total_ac_qnty,4,'.',''); ?>" readonly   /></th>
					<th >&nbsp;</th>
					<th >&nbsp;</th>
					<th ><input style="width:80%;" class="text_boxes_numeric" type="text" id="total_bal_amt" value="<? echo number_format($total_amount,4,'.',''); ?>" readonly   /></th>
					<th>&nbsp;</th>
					<th>&nbsp;</th>
					<?
				}
				else
				{
					...
					<th colspan="21" >Total Amount <input style="width:80%;" type="hidden" name="uniqe_uom" id="uniqe_uom" value="0"></th>
					<th ><input style="width:80%;" class="text_boxes_numeric" type="text" id="total_bal_amt" value="<? echo number_format($total_amount,4,'.',''); ?>" readonly   /></th>
					<th>&nbsp;</th>
					<th>&nbsp;</th>
					<?
...
{
	extract($_REQUEST);
	$txt_order_no_id=str_replace("'","",$txt_fso_id);
	$cbo_fabric_natu=str_replace("'","",$cbo_fabric_natu);
	$cbo_fabric_source=str_replace("'","",$cbo_fabric_source);
	...
	$flag=true;
	?>
	<table width="1450" class="rpt_table" border="0" rules="all">
		<thead>
			<th width="100"></th>
			<th width="100" style="word-break: break-all;word-wrap: break-word;" >Job No</th>
			<th width="150" style="word-break: break-all;word-wrap: break-word;" >Po Number</th>
			<th width="250" style="word-break: break-all;word-wrap: break-word;" >Fab. Description</th>
			<th width="100" style="word-break: break-all;word-wrap: break-word;" >Gmts Color</th>
			<th width="100" style="word-break: break-all;word-wrap: break-word;" >Dia</th>
			<th width="100" style="word-break: break-all;word-wrap: break-word;" >WO. Qnty</th>
			<th width="100" style="word-break: break-all;word-wrap: break-word;" >Adj. Qnty</th>
			<th width="100" style="word-break: break-all;word-wrap: break-word;" >Ac. Wo. Qnty</th>
			<th width="60" style="word-break: break-all;word-wrap: break-word;" >Rate</th>
			<th width="80" style="word-break: break-all;word-wrap: break-word;" >Amount</th>
			<th width="" style="word-break: break-all;word-wrap: break-word;" ></th>
		</thead>
	</table>
	<div style=" max-height:200px; overflow-y:scroll; width:1450px"  align="left">
		<table width="1433" class="rpt_table" id="tbl_fabric_booking_list" border="0" rules="all">
			<tbody>
				<?
					foreach($dataArray as $row) 
					{
						...
						$color_size_table_id=$row['color_size_table_id'];
						...
						<tr>
							<td width="100">
								<a href="#" onClick="set_data('<? echo $po_break_down_id;  ?>','<? echo $color_size_table_id; ?>','<? echo $row['pre_cost_fabric_cost_dtls_id']; ?>','<? echo $booking_id?>')">Edit</a>
							</td>
							<td width="100">
								<? echo $job_no; ?>
							</td>
			...
			<tfoot>
				<tr>
					<?php 
						...
						{
							?>
								<th colspan="6" align="right">Total Amout</th>
								<th width="100" align="right"><? echo number_format($total_qo_qnty,4,'.','') ?></th>
								<th width="100"></th>
								<th width="100" align="right"><? echo number_format($total_ac_qnty,4,'.','') ?></th>
								<th width="60"></th>
								<th width="80" align="right"><? echo number_format($total_amount,4,'.','') ?></th>
								<th></th>
							<?
						}
						else
						{
							?>
								<th colspan="10" align="right">Total Amout</th>
...
if ($action=="refusing_cause_popup")
{
	...
	$refusing_reason=return_field_value("refusing_reason", "refusing_cause_history", "mst_id='".$wo_id."'  and entry_form=549");
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0){
		...
		if($db_type==0){
			$new_booking_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'WGFB', date("Y",time()), 5, "select booking_no_prefix, booking_no_prefix_num from wo_booking_mst where company_id=$cbo_company_name and booking_type=1 and entry_form=549 and YEAR(insert_date)=".date('Y',time())." order by booking_no_prefix_num desc ", "booking_no_prefix", "booking_no_prefix_num" ));
		}
		if($db_type==2){
			$new_booking_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'WGFB', date("Y",time()), 5, "select booking_no_prefix, booking_no_prefix_num from wo_booking_mst where company_id=$cbo_company_name and booking_type=7 and entry_form=549 and  to_char(insert_date,'YYYY')=".date('Y',time())." order by booking_no_prefix_num desc ", "booking_no_prefix", "booking_no_prefix_num" ));
		}
		$id=return_next_id( "id", "wo_booking_mst", 1 ) ;
		$field_array="id,booking_type,is_short,booking_no_prefix,booking_no_prefix_num,booking_no,company_id,buyer_id,item_category,fabric_source,currency_id,exchange_rate,pay_mode,source,booking_date,supplier_id,attention,ready_to_approved,inserted_by,insert_date,entry_form,delivery_address,within_group,delivery_date,agent_id";
		 $data_array ="(".$id.",7,2,'".$new_booking_no[1]."',".$new_booking_no[2].",'".$new_booking_no[0]."',".$cbo_company_name.",".$cbo_buyer_name.",14,2,".$cbo_currency.",".$txt_exchange_rate.",".$cbo_pay_mode.",".$cbo_source.",".$txt_booking_date.",".$cbo_supplier_name.",".$txt_attention.",".$cbo_ready_to_approved.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',549,'".$delivery_address."',".$cbo_within_group.",".$txt_delivery_date.",".$cbo_agent_name.")";
		...
		if($db_type==0){
			...
			if($rID){
				...
				oci_rollback($con);
				echo "10**".$rID."**insert into wo_booking_mst (".$field_array.") values ".$data_array;
			}
		}
		disconnect($con);
		die;
		...
		$field_array="item_category*fabric_source*currency_id*exchange_rate*pay_mode*source*booking_date*supplier_id*attention*ready_to_approved*updated_by*update_date*delivery_address*within_group*buyer_id*delivery_date*agent_id";
		 $data_array ="14*2*".$cbo_currency."*".$txt_exchange_rate."*".$cbo_pay_mode."*".$cbo_source."*".$txt_booking_date."*".$cbo_supplier_name."*".$txt_attention."*".$cbo_ready_to_approved."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*'".$delivery_address."'*".$cbo_within_group."*".$cbo_buyer_name."*".$txt_delivery_date."*".$cbo_agent_name;
	...
	else if ($operation==2)   // Delete Here
	{
		...
		$rID=execute_query( "update wo_booking_mst set status_active=0,is_deleted=1,delete_cause='$delete_cause',updated_by=".$_SESSION['logic_erp']['user_id'].", update_date='".$pc_date_time."'   where  id=$update_id",0);
		$rID1=execute_query( "update wo_booking_dtls set status_active=0,is_deleted=1,delete_cause='$delete_cause',updated_by=".$_SESSION['logic_erp']['user_id'].", update_date='".$pc_date_time."'   where  booking_mst_id=$update_id",0);
...
if($action=="save_update_delete_dtls")
{
	...
	if ($operation==0)
	{
		...
		echo "15**0";disconnect($con);die; die;
		 ...
		 $field_array1="id,is_short,booking_mst_id,pre_cost_fabric_cost_dtls_id,po_break_down_id,color_size_table_id,job_no,booking_no,booking_type,color_type,construction,copmposition,gsm_weight,dia_width,fabric_color_id,gmts_color_id,fin_fab_qnty,grey_fab_qnty,adjust_qty,rate,amount,total_end,remark,pre_cost_remarks,inserted_by,insert_date";
		 $j=1;
		 ...
		 for ($i=1;$i<=$total_row;$i++)
		 {
			...
			$txtcolorsizetableid="txtcolorsizetableid_".$i;
			...
			//============
			 ...
			 $process="process_".$i;
			 $totalends="totalends_".$i;

			 $totalends=str_replace("'","",$$totalends);

			 $precostid=str_replace("'","",$$txtpre_cost_fabric_cost_dtls_id);
			 $txtcolorsizetableid=str_replace("'","",$$txtcolorsizetableid);
			 ...
			 $txtcompositi=str_replace("'","",$$txtcompositi_);
			 $txtgsm_weight=str_replace("'","",$$txtgsm_weight_);
			 $txtdia=str_replace("'","",$$txtdia_);
			 $txtitemcolor=str_replace("'","",$$txtitemcolor);
			 ...
			 //foreach($json_data->$precostid->$colorid->po_id as $poId){
				if($woq>0)
				{
					 ...
					 if ($j!=1) $data_array1 .=",";
					 $data_array1 .="(".$id_dtls.",2,".$update_id.",".$precostid.",".$$txtpoid_.",".$txtcolorsizetableid.",".$$txtjob.",".$txt_booking_no.",7,".$$txtcolortype_.",'".$txtconstruction."','".$txtcompositi."','".$txtgsm_weight."','".$txtdia."','".$txtitemcolor."','".$colorid."',".$acwoq.",".$woq.",".$$txtadj.",".$rate.",".$amount.",'".$totalends."','".$txtremark."','".$process."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
					 $id_dtls=$id_dtls+1;
					 $j++;
				}
			 //}
		 ...
		 if($db_type==2 || $db_type==1 ){
			if($rID){
				...
				oci_rollback($con);
				echo "10**".$rID."Insert into wo_booking_dtls ($field_array1) values $data_array1";
			}
		 }
		 disconnect($con);
		 die;
	...
	else if ($operation==1)
	{
		...
		if($recv_number){
		 ...
		 $field_array_up1="color_type*construction*copmposition*gsm_weight*dia_width*fabric_color_id*gmts_color_id*fin_fab_qnty*grey_fab_qnty*adjust_qty*rate*amount*total_end*remark*pre_cost_remarks*updated_by*update_date";
		 $new_array_color=array();
		 for ($i=1;$i<=$total_row;$i++){
			...
			//============
			 ...
			 $txtremark=str_replace("'","",$$txtremark);
			 $process=str_replace("'","",$$process);
			 $txtcolortype=str_replace("'","",$$txtcolortype_);
			 $txtadj=str_replace("'","",$$txtadj);

			 if(str_replace("'",'',$$bookingid)!=""){
				$id_arr[]=str_replace("'",'',$$bookingid);
				$data_array_up1[str_replace("'",'',$$bookingid)] =explode("*",("'".$txtcolortype."'*'".$txtconstruction."'*'".$txtcompositi."'*'".$txtgsm_weight."'*'".$txtdia."'*'".$txtitemcolor."'*'".$colorid."'*".$acwoq."*".$woq."*'".$txtadj."'*".$rate."*".$amount."*'".$totalends."'*'".$txtremark."'*'".$process."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
		...
		if($db_type==2 || $db_type==1 )
		{
			...
			if($rID==1)
			{
				...
				oci_rollback($con);
				echo "10**".$rID."**".bulk_update_sql_statement( "wo_booking_dtls", "id", $field_array_up1, $data_array_up1, $id_arr );
			}
		}
		disconnect($con);
		die;
	...
	else if ($operation==2)   // Delete Here
	{
		...
		$string="";
		 ...
		 for ($i=1;$i<=$total_row;$i++){
			 ...
			 $string="update wo_booking_dtls set status_active=0,is_deleted=1,delete_cause='$delete_cause',updated_by=".$_SESSION['logic_erp']['user_id'].",update_date='".$pc_date_time."' where  id in (".str_replace("'","",$$bookingid).") and booking_no=$txt_booking_no";
			 $rID=execute_query( $string,0);
		 }
		if($db_type==0)
		{
			if($rID)
		...
		if($db_type==2 || $db_type==1 )
		{
			if($rID){
				...
				oci_rollback($con);
				echo "10**".str_replace("'","",$txt_job_no)."**".str_replace("'","",$txt_booking_no)."**".str_replace("'","",$cbo_colorsizesensitive)."**".$string;
			}
		}
		disconnect($con);
		die;
...
function fnc_fabric_booking_terms_condition( operation ){
	...
	http.open("POST","woven_grey_fabric_purchase_booking_controller.php",true);
            ...
            <table width="650" cellspacing="0" class="rpt_table" border="0" id="tbl_termcondi_details" rules="all">
                ...
                <table width="650" cellspacing="0" class="" border="0">
                	<tr>
                        ...
                        <input type="button" id="set_button4" class="image_uploader" style="width:160px;" value="Add More.." onClick="open_extra_terms_popup('woven_grey_fabric_purchase_booking_controller.php?action=extra_terms_popup','Terms Condition')" />
                        </td>
                    </tr>
                	<tr>
...
{
show_list_view(document.getElementById('cbo_fabric_part').value+'**'+document.getElementById('booking_no').value+'**'+document.getElementById('cbo_level').value+'**'+<? echo "'$permission'";?>,'load_color_size_form','form_data_con','woven_grey_fabric_purchase_booking_controller','');
}
...
function show_list()
{
	...
	show_list_view(booking_no,'show_list_view','list_view_con','woven_grey_fabric_purchase_booking_controller','');
}
function show_sub_form_with_data(booking_no,fabric_cost_id){
	...
	show_list_view(document.getElementById('cbo_fabric_part').value+'**'+document.getElementById('booking_no').value+'**'+'00'+'**'+<? echo "'$permissions'";?>,'load_color_size_form','form_data_con','woven_grey_fabric_purchase_booking_controller','');
...
function fnc_colar_culff_dtls( operation ){
		...
		var booking=return_global_ajax_value(booking_no, 'check_booking_approved', '', 'woven_grey_fabric_purchase_booking_controller');
...
if($action=="print1") // print button 1
{
	extract($_REQUEST);
	...
	$imge_arr=return_library_array( "select master_tble_id,image_location from common_photo_library where form_name='company_details' and file_type=1",'master_tble_id','image_location');
	$marchentrArr = return_library_array("select id,team_member_name from lib_mkt_team_member_info  where status_active=1 and is_deleted=0","id","team_member_name");
	$agent_library = return_library_array("select id,supplier_name from lib_supplier","id","supplier_name");

	$sql = "SELECT id,booking_type,is_short,booking_no_prefix,booking_no_prefix_num,booking_no,company_id,buyer_id,item_category,fabric_source,currency_id,exchange_rate,pay_mode,source,booking_date,supplier_id,attention,ready_to_approved,inserted_by,insert_date,entry_form,delivery_address,within_group,delivery_date,agent_id FROM WO_BOOKING_MST WHERE id = $update_id";
	//echo $sql;
	$res = sql_select($sql);
	$booking_date =""; $attention =""; $delivery_date =""; $agent_name ="";
	if($res[0][csf('booking_date')]){
		$booking_date = $res[0][csf('booking_date')];
	}
	if($res[0][csf('attention')]){
		$attention = $res[0][csf('attention')];
	}
	if($res[0][csf('delivery_date')]){
		$delivery_date = $res[0][csf('delivery_date')];
	}
	if($res[0][csf('agent_id')]){
		$agent_name = $res[0][csf('agent_id')];
	}
	 

	$sql_dtls = "SELECT id,is_short,booking_mst_id,pre_cost_fabric_cost_dtls_id,po_break_down_id,color_size_table_id,job_no,booking_no,booking_type,color_type,construction,copmposition,gsm_weight,dia_width,fabric_color_id,gmts_color_id,fin_fab_qnty,grey_fab_qnty,adjust_qty,rate,amount,remark,pre_cost_remarks,inserted_by,insert_date,total_end FROM wo_booking_dtls WHERE booking_no = '".$txt_booking_no."' and is_deleted = 0 order by id ";
	...
	$amount = 0;
	$quantity = 0;
	$technical_instructions = array();
	foreach($res_dtls as $row)
	{
		$index = $row[csf('pre_cost_fabric_cost_dtls_id')].",".$row[csf('po_break_down_id')].",".$row[csf('color_size_table_id')];
		$dtls_data[$index]['job_no'] 			= $row[csf('job_no')];
		$dtls_data[$index]['mst_id'] 			= $row[csf('booking_mst_id')];
		$dtls_data[$index]['pcfcd_id'] 			= $row[csf('pre_cost_fabric_cost_dtls_id')];
		$dtls_data[$index]['po_id'] 			= $row[csf('po_break_down_id')];
		$dtls_data[$index]['color_size_id'] 	= $row[csf('color_size_table_id')];
		$dtls_data[$index]['booking_no'] 		= $row[csf('booking_no')];
		$dtls_data[$index]['booking_type'] 		= $row[csf('booking_type')];
		$dtls_data[$index]['construction'] 		= $row[csf('construction')];
		$dtls_data[$index]['gsm_weight'] 		= $row[csf('gsm_weight')];
		$dtls_data[$index]['copmposition'] 		= $row[csf('copmposition')];
		$dtls_data[$index]['dia_width'] 		= $row[csf('dia_width')];
		$dtls_data[$index]['fabric_color_id'] 	= $row[csf('fabric_color_id')];
		$dtls_data[$index]['gmts_color_id'] 	= $row[csf('gmts_color_id')];
		$dtls_data[$index]['fin_fab_qnty'] 		+= $row[csf('fin_fab_qnty')];
		$dtls_data[$index]['grey_fab_qnty'] 	+= $row[csf('grey_fab_qnty')];
		$dtls_data[$index]['adjust_qty'] 		+= $row[csf('adjust_qty')];
		$dtls_data[$index]['amount'] 			+= $row[csf('amount')];
		$dtls_data[$index]['total_end'] 		+= $row[csf('total_end')];
		$dtls_data[$index]['remark'] 			= $row[csf('remark')];
		$dtls_data[$index]['pre_cost_remarks'] 	= $row[csf('pre_cost_remarks')];

		$job_no_arr[$row[csf('job_no')]]            = $row[csf('job_no')];

		$amount+= $row[csf('amount')];
		$quantity+= $row[csf('grey_fab_qnty')];
		$technical_instructions[$row[csf('remark')]] = $row[csf('remark')];
	}
	...
	<div style="width:850px; font-size:20px; font-weight:bold" align="center">
        ...
        <table>
			<tr>
				<td width ="20%" align="left"> <img  src='<?=base_url( $imge_arr[$company_name]); ?>' height='100' width='100%' /></td>
				<td width="60%">
					<table width="100%" cellpadding="0" cellspacing="0" >
						<tr>
						...
						<tr>
							<td align="center" style="font-size:20px">
								<strong><? //echo $report_title;?>(WOVEN GREY FABRIC PRODUCTION  ORDER)</strong>
							</td>
						</tr>
					</table>
				</td>
        ...
        <table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
            <tr>
                <td align="left" > <strong>UCDML SC Work Order</strong></td>
				<td align="left"> <strong># <?=$txt_booking_no;?></strong></td>
				<td align="left" > <strong>Date</strong></td>
				<td align="left"> <strong>:<?=change_date_format($booking_date);?></strong></td>
            </tr>
			<tr>
                <td align="left" > <strong>UCDML MKT PO   </strong></td>
				<td align="left"> <strong>#<?=implode(",",$job_no_arr)?></strong></td>
				<td align="left" > <strong>Mill Name</strong></td>
				<td align="left"> <strong>:</strong></td>
            </tr>
			<tr>
                <td align="left" > <strong>Attn</strong></td>
				<td align="left"> <strong>:<?=$attention?></strong></td>
				<td align="left" > <strong>Price</strong></td>
				<td align="left"> <strong>:<?=fn_number_format($amount/$quantity,2);?></strong></td>
            </tr>
			<tr>
                <td align="left" > <strong>Delivery Date</strong></td>
				<td align="left"> <strong>:<?=change_date_format($delivery_date);?></strong></td>
				<td align="left" > <strong>Agent</strong></td>
				<td align="left"> <strong>:<?=$agent_name;?></strong></td>
            </tr>
            
        </table>
		<div style="margin-top: 10px;"><h3>WOVEN GREY FABRIC PRODUCTION  ORDER</h3></div>
		
		<table  border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
			<thead>
				<tr>
					<th width="5%">SL</th>
					<th width="20%">FABRIC COMPOSITION</th>
					<th width="20%">CONSTRUCTION</th>
					<th width="10%">GREY WIDTH -INCHES</th>
					<th width="10%">Price US$/YDS</th>
					<th width="10%">QUANTITY (YDS)</th>
			...
			<tbody>
				<?
				$i = 1;
				foreach($dtls_data as $index => $row)
				{
					if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
					?>
					<tr  bgcolor="<?=$bgcolor?>">
						<td><?=$i;?></td>
						<td><?=$row['copmposition'];?></td>
						<td><?=$row['construction'];?></td>
						<td><?=$row['dia_width'];?></td>
						<td><?=fn_number_format($row['amount']/$row['grey_fab_qnty'],2);?></td>
						<td><?=fn_number_format($row['grey_fab_qnty'],2);?></td>
						<td><?=fn_number_format($row['amount'],2);?></td>
						<td><?=fn_number_format($row['total_end'],2)?></td>
		...
		<?=get_spacial_instruction($txt_booking_no,  "100%", 549)?>
		<div style="margin-top: 10px;">
			<table border="1" align="left" class="rpt_table"  cellpadding="0" width="100%" cellspacing="0" rules="all" >
				<caption>Technical Instructions</caption>
				<thead>
					<tr>
						<th width="35">Sl</th>
						<th>Instruction</th>
				...
				<tbody>
					<?
					$i = 1;
					foreach($technical_instructions as $key => $ins)
					{
						if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
						?>
						<tr  bgcolor="<?=$bgcolor?>">
		...
		<div style="margin-top: 10px;">&nbsp;</div>
		<?=signature_table(305, $company_name, '100%',1);?>
   	</div>
