...
if ($action=="load_variable_settings")
{
	...
	$variable_is_control=return_field_value("is_control","variable_settings_production","company_name=$data and variable_list=33 and page_category_id=31","is_control");
...
if($action=="load_drop_down_source_location_and_floor")
{
	list($company_id,$location,$source)=explode("_",$data);
	...
	echo create_drop_down( "cbo_location", 167, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id='$company_id' $location_credential_cond order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down( 'requires/finishing_entry_controller', this.value, 'load_drop_down_floor', 'floor_td' );",0 ); 

	echo "****";	

	echo create_drop_down( "cbo_floor", 170, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$location' and production_process in (11) order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "",0 );
...
if($action=="order_popup")
{
	...
	function js_set_value(id,item_id,po_qnty,plan_qnty,country_id,pack_type,ship_date)
	{
...
if($action=="create_po_search_list_view")
{
	...
	$qty_source=0;$heading = '';
	if($preceding_process==28) {$qty_source=4;$heading="Input Qty";	} //Sewing Input
	else if($preceding_process==29) {$qty_source=5;$heading="Output Qty";}//Sewing Output
	else if($preceding_process==30) {$qty_source=7;$heading="Iron Qty";}//Iron Output
	else if($preceding_process==31) {$qty_source=8;$heading="Fin Qty";}//Packing And Finishing
	else if($preceding_process==32) {$qty_source=7;$heading="Iron Qty";}//Iron Output
	else if($preceding_process==91) {$qty_source=7;$heading="Iron Qty";}//Iron Output
	else if($preceding_process==103) {$qty_source=11;$heading="Poly Qty";}//Poly Entry
	else if($preceding_process==250) {$qty_source=80;$heading="woven fin. Qty";}//woven fin. entry
	else if($preceding_process==256) {$qty_source=15;$heading="Hang Tag Qty";}//Hang Tag Entry

	// if($qty_source==0){$qty_source=7;}
	...
	if(trim($txt_search_by)==4 && trim($txt_search_common)!="")
	{
		$sql = "SELECT b.id, a.order_uom, a.buyer_name, a.company_name, a.total_set_qnty, a.set_break_down, a.job_no, a.style_ref_no, a.gmts_item_id, a.location_name, b.shipment_date, b.po_number,b.file_no,b.grouping, b.po_quantity, b.plan_cut
			...
			where a.id = b.job_id and b.id=c.po_break_down_id and b.shiping_status!=3 and a.status_active=1 and  a.is_deleted=0 and b.status_active in(1)  and  b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.garments_nature=$garments_nature $sql_cond $projected_po_cond $year_cond  group by b.id, a.order_uom, a.buyer_name, a.company_name, a.total_set_qnty, a.set_break_down, a.job_no, a.style_ref_no, a.gmts_item_id, a.location_name, b.shipment_date, b.po_number, b.file_no, b.grouping, b.po_quantity, b.plan_cut order by b.shipment_date DESC";
	}
	else
	{
 		$sql = "SELECT b.id, a.order_uom, a.buyer_name, a.company_name, a.total_set_qnty, a.set_break_down, a.job_no, a.style_ref_no, a.gmts_item_id, a.location_name, b.shipment_date, b.po_number,b.file_no,b.grouping, b.po_quantity, b.plan_cut
		 from wo_po_details_master a, wo_po_break_down b 
		 where a.id = b.job_id and a.status_active=1 and b.shiping_status!=3 and b.status_active in(1) and a.is_deleted=0  and  b.is_deleted=0 and a.garments_nature=$garments_nature 
		 $sql_cond $projected_po_cond $year_cond order by b.shipment_date DESC"; 
	}
	...
	$con = connect();
	execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ref_from =1 and ENTRY_FORM=70");
	oci_commit($con);
	
	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 70, 1, $poIdArr, $empty_arr);//Po ID
	disconnect($con);
	...
	$country_sql=sql_select( "SELECT a.po_break_down_id, a.item_number_id, a.country_id, a.pack_type, (a.order_quantity) as po_qty, (a.plan_cut_qnty) as plan_cut_qty,a.country_ship_date,a.color_number_id from wo_po_color_size_breakdown a,GBL_TEMP_ENGINE tmp where a.po_break_down_id=tmp.ref_val and tmp.entry_form=70  and tmp.user_id=$user_id and tmp.ref_from=1 and a.status_active=1 and  a.is_deleted=0 "); 
	
	foreach($country_sql as $row)
	{
		//$po_country_arr[$row[csf('po_break_down_id')]].=$row[csf('country_id')].',';
		$po_country_data_arr[$row[csf('po_break_down_id')]][$row[csf('item_number_id')]][$row[csf('country_id')]][$row[csf('pack_type')]][$row[csf('country_ship_date')]]['po_qty'] += $row[csf('po_qty')];
		$po_country_data_arr[$row[csf('po_break_down_id')]][$row[csf('item_number_id')]][$row[csf('country_id')]][$row[csf('pack_type')]][$row[csf('country_ship_date')]]['plan_cut_qty'] += $row[csf('plan_cut_qty')];
	...
	/* $total_entry_qty=sql_select( "SELECT a.po_break_down_id, a.item_number_id, a.country_id, 
	 (CASE WHEN a.production_type = 8 THEN a.production_quantity ELSE 0 END) AS finishing_quantity,
	 (CASE WHEN a.production_type = $qty_source THEN a.production_quantity ELSE 0 END) AS iron_qty, 
	a.pack_type,a.country_ship_date from pro_garments_production_mst a,GBL_TEMP_ENGINE tmp where a.po_break_down_id=tmp.ref_val and tmp.entry_form=70  and tmp.user_id=$user_id and tmp.ref_from=1 and a.status_active=1 and a.is_deleted=0 and a.production_type IN ($qty_source, 8)"); */
	
	$total_entry_qty=sql_select( "SELECT b.po_break_down_id, b.item_number_id, b.country_id, 
	 (CASE WHEN a.production_type = 8 THEN a.production_qnty ELSE 0 END) AS finishing_quantity,
	 (CASE WHEN a.production_type = $qty_source THEN a.production_qnty ELSE 0 END) AS iron_qty, 
	b.pack_type,b.country_ship_date from pro_garments_production_dtls a,wo_po_color_size_breakdown b,GBL_TEMP_ENGINE tmp where a.COLOR_SIZE_BREAK_DOWN_ID=b.id and b.po_break_down_id=tmp.ref_val and tmp.entry_form=70  and tmp.user_id=$user_id and tmp.ref_from=1 and a.status_active=1 and a.is_deleted=0 and a.production_type IN ($qty_source, 8)");
	...
	foreach($total_entry_qty as $row)
	{
		$total_entry_qty_data_arr[$row[csf('po_break_down_id')]][$row[csf('item_number_id')]][$row[csf('country_id')]][$row[csf('pack_type')]][$row[csf('country_ship_date')]]['finishing_quantity'] += $row[csf('finishing_quantity')];
		$total_entry_qty_data_arr[$row[csf('po_break_down_id')]][$row[csf('item_number_id')]][$row[csf('country_id')]][$row[csf('pack_type')]][$row[csf('country_ship_date')]]['iron_qty'] += $row[csf('iron_qty')];
	}
        ...
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1252" class="rpt_table" id="tbl_po_list">
            ...
            foreach( $result as $row )
            {
				...
				foreach($po_country_data_arr[$row[csf('id')]] as $grmts_item=>$item_data)
				{
					...
					{
						foreach($country_data as $pack_type=>$pack_data)
						{
							foreach($pack_data as $ship_date=>$val)
							{
								...
								?>
									<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value(<? echo $row[csf("id")];?>,'<? echo $grmts_item;?>','<? echo $po_qnty;?>','<? echo $plan_cut_qnty;?>','<? echo $country_id;?>','<? echo $pack_type;?>','<? echo $ship_date;?>');" > 
										<td width="30" align="center"><?php echo $i; ?></td>
										<td width="60"><p><?php echo change_date_format($ship_date);?></p></td>
										...
										<td width="80" align="right"><?php echo $total_entry_qty_data_arr[$row[csf('id')]][$grmts_item][$country_id][$pack_type][$ship_date]['iron_qty'];?>&nbsp;</td>
										<td width="80" align="right"><?php 
										//echo $total_in_qty=$total_in_qty_data_arr[$row[csf('id')]][$grmts_item][$country_id][$pack_type]; 
										echo $total_in_qty=$total_entry_qty_data_arr[$row[csf('id')]][$grmts_item][$country_id][$pack_type][$ship_date]['finishing_quantity'];
										?> &nbsp;</td>
									<td width="80" align="right">
										<?php 
										if($qty_source==0){$balance=$po_qnty-$total_in_qty;}
										else{$balance=$total_entry_qty_data_arr[$row[csf('id')]][$grmts_item][$country_id][$pack_type][$ship_date]['iron_qty']-$total_in_qty;}
										// $balance=$po_qnty-$total_in_qty; 
										echo $balance; ?>
									&nbsp;</td>
...
if($action=="create_po_search_list_view__")
{
	...
	else if($preceding_process==250) $qty_source=80;//woven fin. entry
	else if($preceding_process==256) $qty_source=15;//Hang Tag Entry
	...
	$po_country_data_arr=array();
	$country_sql=sql_select( "SELECT po_break_down_id, item_number_id, country_id, pack_type, sum(order_quantity) as po_qty, sum(plan_cut_qnty) as plan_cut_qty from wo_po_color_size_breakdown where status_active=1 and  is_deleted=0 $po_id_cond group by po_break_down_id, item_number_id, country_id, pack_type"); 
	...
	$total_entry_qty_data_arr=array();
	$total_entry_qty=sql_select( "SELECT po_break_down_id, item_number_id, country_id, sum(production_quantity) as production_quantity, pack_type from pro_garments_production_mst where status_active=1 and is_deleted=0 and production_type=8 $po_id_cond group by po_break_down_id, item_number_id, country_id, pack_type");
...
if($action=="populate_data_from_search_popup")
{
	...
	else if($dataArr[3]==250) $qty_source=80;//woven fin. entry
	else if($dataArr[3]==256) $qty_source=15;//Hang Tag Entry
...
if ($action == "actual_po_action__") // 6-2-2023
{
	extract($_REQUEST);
	echo load_html_head_contents("Popup Info", "../../", 1, 1, $unicode);
	...
	{
		$dt_arr =  explode("**",$val);
		$act_po_data_ar[$dt_arr[3]]['ok']=$dt_arr[8];
		$act_po_data_ar[$dt_arr[3]]['rej']=$dt_arr[9];
		$tot_cur_qty += $dt_arr[8];
		$tot_cur_rej_qty += $dt_arr[9];
	...
	<script>
		...
		function fnc_close() 
		{
			...
			{
				if ($(this).val() != '' || $("#acc_po_rej_qty_"+i).val()!='') 
                {
                    var acc_po_rej_qty = $("#acc_po_rej_qty_"+i).val()*1;
					if(acc_po_rej_qty=="") { acc_po_rej_qty=0;}
                    var hidden_data = $("#hidden_row_data_"+i).val()+'**'+$(this).val()*1+'**'+acc_po_rej_qty;
			...
			// alert(data_ex_arr.join('/'));
			
			// alert(checkDuplicate(data_ex_arr));
			/* if (checkDuplicate(data_ex_arr)) 
			{
				alert('Error: you can not mix PO and country !');
				// data_ex_arr = new Array();
				$("input[name=acc_po_qty]").val('');
				return;
			} */
			// data_ex_arr = new Array();
			// return;
			// alert(data_string);
			// $("#data_string").val(data_string);
		...
		function fn_set_rej_grnd_total()
		{
			var total=0;
			$("input[name=acc_po_rej_qty]").each(function (index, element) 
			{
				if ($(this).val() != '') 
                {
                    total += parseInt($(this).val());
	...
	<body>
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="910" class="rpt_table">
			<thead>
				...
				<th width="60">Cum. Fin. Qty.</th>
				<th width="60">Cur. Fin. Qty.</th>
				<th width="60">Rej. Qty.</th>
				<th width="60">Bal. Qty.</th>

			</thead>
		</table>
		<div style="width:930px; max-height:360px; overflow-y:auto" id="list_container_batch" align="left">
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="910" class="rpt_table" id="tbl_list_search">
				...
				foreach ($result as $row) 
				{
					...
					$balance = $row[csf('acc_po_qty')]-$prev_fin_qty[$row[csf('dtls_id')]];
					...
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i; ?>">
						...
						<td width="60" align="right"><?=$prev_fin_qty[$row[csf('dtls_id')]]; ?></td>
						<td width="60" align="right"><input type="text" class="text_boxes_numeric" id="acc_po_qty" name="acc_po_qty" value="<?=$act_po_data_ar[$row[csf('dtls_id')]]['ok'];?>" style="width:50px" onKeyUp="fn_set_grnd_total();"></td>
						
						<td width="60" align="right"><input type="text" class="text_boxes_numeric" id="acc_po_rej_qty_<?=$i;?>" name="acc_po_rej_qty" value="<?=$act_po_data_ar[$row[csf('dtls_id')]]['rej'];?>" style="width:50px" onKeyUp="fn_set_rej_grnd_total();"></td>
		...
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="910" class="rpt_table">
			<tfoot>
				<tr>
					...
					<th width="60" align="right" id="total_rej_qty"><?=$tot_cur_rej_qty;?></th>
	...
	<body>
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="910" class="rpt_table">
			<thead>
				...
				<th width="50" title="Sewing Output Balance">S.Out Balance</th>
				<th width="60">Cum. Fin. Qty.</th>
				<th width="60">Cur. Fin. Qty.</th>
				<th width="60">Rej. Qty.</th>
				<th width="60">Bal. Qty.</th>
		...
		<div style="width:930px; max-height:360px; overflow-y:auto" id="list_container_batch" align="left">
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="910" class="rpt_table" id="tbl_list_search">
				...
				//echo "SELECT a.actual_po_dtls_id as id, sum(a.prod_qty) as qty from pro_garments_prod_actual_po_details a, pro_garments_production_mst b where a.mst_id=b.id and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0 and b.production_type=8 and b.po_break_down_id=$po_id and b.item_number_id=$item_id $acc_po_id_cond group by a.actual_po_dtls_id";
				
				$prev_rej_fin_qty = return_library_array("SELECT a.actual_po_dtls_id as id, sum(a.reject_qty) as reject_qty from pro_garments_prod_actual_po_details a, pro_garments_production_mst b where a.mst_id=b.id and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0 and b.production_type=8 and b.po_break_down_id=$po_id and b.item_number_id=$item_id $acc_po_id_cond group by a.actual_po_dtls_id", "id", "reject_qty");
				
				// echo "SELECT a.actual_po_dtls_id as id, sum(a.ex_fact_qty) as qty from pro_ex_factory_actual_po_details a, pro_ex_factory_mst b where a.mst_id=b.id and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0 and b.po_break_down_id=$po_id $acc_po_id_cond group by a.actual_po_dtls_id";

				$sewing_output_sql = "SELECT B.ITEM_NUMBER_ID,B.COLOR_NUMBER_ID, B.SIZE_NUMBER_ID, A.PRODUCTION_QNTY FROM pro_garments_production_dtls a, wo_po_color_size_breakdown b WHERE a.color_size_break_down_id = b.id AND a.status_active = 1 AND a.is_deleted = 0 AND b.status_active = 1 AND b.is_deleted = 0 AND a.production_type = 5 AND b.po_break_down_id = $po_id AND b.item_number_id = $item_id";
				// echo $sql;
				$sewing_output_result = sql_select($sewing_output_sql);
				
				$swo_arr=array();
				if(!empty($sewing_output_result))
				{
					foreach($sewing_output_result as $so_row)
					{
						$swo_arr[$so_row["ITEM_NUMBER_ID"]][$so_row["COLOR_NUMBER_ID"]][$so_row["SIZE_NUMBER_ID"]] += $so_row["PRODUCTION_QNTY"];
					}
				}

				unset($sewing_output_result);
				...
				$js_set_string = "";
				$row_data = "";
				$packing_item_color_size_arr = array();
				foreach ($result as $row) 
				{
					$packing_item_color_size_arr[$row[csf('gmts_item')]][$row[csf('gmts_color_id')]][$row[csf('gmts_size_id')]] += ($prev_fin_qty[$row[csf('dtls_id')]]+$prev_rej_fin_qty[$row[csf('dtls_id')]]);
				}
				...
				foreach ($result as $row) 
				{
					...
					$sewing_output_qnty = $swo_arr[$row[csf('gmts_item')]][$row[csf('gmts_color_id')]][$row[csf('gmts_size_id')]];
					$packing_item_color_size_qnty = $packing_item_color_size_arr[$row[csf('gmts_item')]][$row[csf('gmts_color_id')]][$row[csf('gmts_size_id')]];
					$swo_title = "Sewing Output = ".$sewing_output_qnty . "\n Finishing Entry = " . $packing_item_color_size_qnty;
					...
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i; ?>">
						...
						<td width="100" title="<?= $row[csf('gmts_color_id')]; ?>">
							<p><? echo substr($color_library[$row[csf('gmts_color_id')]], 0, 15); ?></p>
						</td>
						<td width="60" title="<? echo $row[csf('gmts_size_id')];?>">
							<p><? echo $size_library[$row[csf('gmts_size_id')]]; ?></p>
						</td>
						<td width="60" align="right"><? echo $row[csf('acc_po_qty')]; ?></td>
						<td width="50" align="right" title="<? echo $swo_title; ?>"><? echo ($sewing_output_qnty-$packing_item_color_size_qnty); ?></td>
						<td width="60" align="right"><?=$prev_fin_qty[$row[csf('dtls_id')]]; ?></td>
						<td width="60" align="right"><input type="text" class="text_boxes_numeric" id="acc_po_qty" name="acc_po_qty" value="<?=$act_po_data_ar[$row[csf('dtls_id')]]['ok'];?>" title="" style="width:50px" onKeyUp="fn_set_grnd_total();"></td>
		...
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="910" class="rpt_table">
			<tfoot>
				<tr>
					...
					<th width="60" id="total_po_qty"></th>
...
if($action=="color_and_size_level")
{
	...
	// $pack_type=''; // pack type blank due to multiple pack type problem.
	$qty_source=0;
	$qty_source_txt="PLan Cut Qty";
	if($dataArr[7]==28) {$qty_source=4; $qty_source_txt="Sewing Input Qty";}//Sewing Input
	else if($dataArr[7]==29) {$qty_source=5;$qty_source_txt="Sewing Output Qty";}//Sewing Output
	else if($dataArr[7]==30) {$qty_source=7;$qty_source_txt="Iron Input Qty";}//Iron Output
	else if($dataArr[7]==31) {$qty_source=8;$qty_source_txt="Packing And Finishing Qty";}//Packing And Finishing
	else if($dataArr[7]==32) {$qty_source=7;$qty_source_txt="Iron Input Qty";}//Iron Output
	else if($dataArr[7]==91) {$qty_source=7;$qty_source_txt="Iron Input Qty";}//Iron Output
	else if($dataArr[7]==103) {$qty_source=11;$qty_source_txt="Poly Entry Qty";}//Poly Entry
	else if($dataArr[7]==250) {$qty_source=80;$qty_source_txt="woven fin. Qty";}//woven fin. entry
	else if($dataArr[7]==256) {$qty_source=15;$qty_source_txt="Hang Tag Qty";}//Hang Tag Entry
	
	if( $pack_type=='') $pack_type_cond=''; else $pack_type_cond=" and pack_type='$pack_type'";
	if( $pack_type=='') $pack_typeCond=''; else $pack_typeCond=" and b.pack_type='$pack_type'";
	if( $pack_type=='') $pack_type_cond2=''; else $pack_type_cond2=" and a.pack_type='$pack_type'";
	...
	// echo $pack_type_cond;
	// ==================== set mst form data ===============================
	// echo $qty_source."sdsd";die;
	$cumulQty_arr=array();
	if($qty_source!=0)
   	{
		
		$dataArray=sql_select("SELECT a.item_number_id, a.country_id,c.pack_type,c.country_ship_date, SUM(CASE WHEN a.production_type='$qty_source' THEN b.production_qnty END) as totalinput,SUM(CASE WHEN a.production_type=8 THEN b.production_qnty ELSE 0 END) as totalsewing from pro_garments_production_mst a, pro_garments_production_dtls b,wo_po_color_size_breakdown c WHERE a.id=b.mst_id and a.po_break_down_id=$po_id and c.id=b.color_size_break_down_id and b.status_active=1 and b.is_deleted=0  and a.status_active=1 and a.is_deleted=0  and c.status_active=1 and c.is_deleted=0 group by a.item_number_id, a.country_id,c.pack_type,c.country_ship_date");
		foreach($dataArray as $row)
		{ 
			$cumulQty_arr[$row[csf('item_number_id')]][$row[csf('country_id')]][$row[csf('pack_type')]][$row[csf('country_ship_date')]]['input']+=$row[csf('totalinput')];
			$cumulQty_arr[$row[csf('item_number_id')]][$row[csf('country_id')]][$row[csf('pack_type')]][$row[csf('country_ship_date')]]['totalsewing']+=$row[csf('totalsewing')];
		}
		unset($dataArray);
	}
	
	$plan_cut_qnty=return_field_value("sum(plan_cut_qnty)","wo_po_color_size_breakdown","po_break_down_id=$po_id and item_number_id='$item_id' and country_id='$country_id' $country_ship_date_cond $pack_type_cond and is_deleted=0");
	
	$total_produced = return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=$po_id and item_number_id='$item_id' and country_id='$country_id' $country_ship_date_cond $pack_type_cond and production_type=8 and is_deleted=0");
  	...
  	foreach($res as $result)
	{
  		...
  		if($qty_source!=0)
   		{
	 		...
	 		foreach($dataArray as $row)
			{*/ 
			$totalinput=$cumulQty_arr[$item_id][$country_id][$pack_type][$country_ship_date]['input'] ;
			$totalsewing=$cumulQty_arr[$item_id][$country_id][$pack_type][$country_ship_date]['totalsewing'];
		...
		echo "$('#prod_qty').text('".$qty_source_txt."');\n";
  	}
	...
	if($qty_source !=0)
	{
		...
		else if( $variableSettings==3 ) //color and size level
		{
			...
			where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' $pack_type_cond $country_ship_date_cond and is_deleted=0 and status_active=1 order by color_number_id, size_order";
			// die('sdfsf');
		}
	...
	{  
		if( $variableSettings==2 ) // color level
		{
			...
			$sql = "SELECT a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty,sum(b.production_qnty) as production_qnty
				...
				where a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' $pack_type_cond $country_ship_date_cond and a.is_deleted=0 and a.status_active=1  group by a.item_number_id, a.color_number_id";	

			$sql_plan_cut="SELECT color_number_id, sum(plan_cut_qnty) as quantity from wo_po_color_size_breakdown where po_break_down_id='$po_id' and item_number_id='$item_id' and  country_id='$country_id' $country_ship_date_cond $pack_type_cond and status_active=1 and is_deleted=0 group by color_number_id";
		...
		{				
			...
			where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' $pack_type_cond $country_ship_date_cond and status_active=1 and is_deleted=0  order by color_number_id,size_order"; //color_number_id, id
...
if($action=="show_all_listview")
{
	...
	<div style="width:100%;">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="100%" class="rpt_table">
            <thead>
                ...
                <th width="70" align="center">Prod Date</th>
                <th width="70" align="center">Prod Qty</th> 
                <th width="50" align="center">Reject Qty</th>                   
                <th width="50" align="center">Reporting Hour</th>
                <th width="120" align="center">WO Company</th>
                <th width="100" align="center">Location</th>
                <th width="100">Floor</th>
                <th  width="60" align="center">Sys. Challan</th>
            </thead>
		</table>
	</div>
	<div style="width:100%;max-height:180px; overflow:y-scroll" id="iron_list_view" align="left">
		...
		<?php  
			...
			if($db_type==0)
			{
				$sqlResult =sql_select("SELECT id, po_break_down_id, item_number_id, country_id, production_date, production_quantity, reject_qnty, production_source, TIME_FORMAT( production_hour, '%H:%i' ) as production_hour, serving_company, location, floor_id from pro_garments_production_mst where po_break_down_id='$po_id' and item_number_id='$item_id' and production_type='8' and status_active=1 and is_deleted=0 order by production_date");
			}
			else
			{
				$sqlResult =sql_select("SELECT id, po_break_down_id, item_number_id, country_id, production_date, production_quantity, reject_qnty, production_source, TO_CHAR(production_hour,'HH24:MI') as production_hour, serving_company, location,floor_id from pro_garments_production_mst where po_break_down_id='$po_id' and item_number_id='$item_id' and production_type='8' and status_active=1 and is_deleted=0 order by production_date");
			}
			$mst_id_arr = array();
			...
			$material_cost_id_arr = return_library_array( "SELECT id, id as ids from pro_garments_material_cost where status_active=1 $mst_id_cond",'id','ids');
			
			$actual_po_library = return_library_array("SELECT id, acc_po_no from wo_po_acc_po_info where po_break_down_id = '$po_id'", 'id', 'acc_po_no');
			
			$actual_po = sql_select("SELECT mst_id, actual_po_id from PRO_GARMENTS_PROD_ACTUAL_PO_DETAILS where status_active=1 $mst_id_cond group by mst_id, actual_po_id");
			$acc_po_arr = array();
			...
			foreach($sqlResult as $selectResult){
				...
				$material_cost_id= $material_cost_id_arr[$selectResult[csf('id')]];
 		?>
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data('<? echo $selectResult[csf('id')]."_".$job_no."_".$material_cost_id; ?>','populate_input_form_data','requires/finishing_entry_controller');" > 
                ...
                <td width="100" align="center" style="word-break:break-all"><p><? echo $acc_po_arr[$selectResult[csf('id')]]; ?></p></td>
                <td width="120" align="center" style="word-break:break-all"><p><? echo $garments_item[$selectResult[csf('item_number_id')]]; ?></p></td>
                ...
                <td width="70" align="center" style="word-break:break-all"><?php echo change_date_format($selectResult[csf('production_date')]); ?></td>
                <td width="70" align="center" style="word-break:break-all"><?php  echo $selectResult[csf('production_quantity')]; ?></td>
                <td width="50" align="center" style="word-break:break-all"><?php  echo $selectResult[csf('reject_qnty')]; ?></td>
 			
                <td width="50" align="center" style="word-break:break-all"><?php echo $selectResult[csf('production_hour')]; ?></td>
				<?php
                ...
                ?>	
                <td width="120" align="center" style="word-break:break-all"><p><?php echo $serving_company; ?></p></td>
                <?php 
                $location_name= $location_arr[$selectResult[csf('location')]];
                $floor_name= $floor_arr[$selectResult[csf('floor_id')]];
                ?>
                <td width="100" align="center" style="word-break:break-all"><? echo $location_name; ?></td>
                <td width="100" style="word-break:break-all"><? echo $floor_name; ?></td>
				<td  width="60" align="center"><? echo $selectResult[csf('id')]; ?></td>
			</tr>
			<?php
			$i++;
			}
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="380" class="rpt_table">
        <thead>
            ...
            <th width="45">Pack Type.</th>
            <th width="45">Sew.Out/Poly Qty</th>
            <th>Finishing Qty.</th>                    
        </thead>
		<?
		$issue_qnty_arr=sql_select("SELECT a.serving_company,a.production_type,a.po_break_down_id, a.item_number_id, a.country_id,c.pack_type,c.country_ship_date, (case when a.production_type=8 then b.production_qnty else 0 end) as fin_qty, (case when a.production_type=5 then b.production_qnty else 0 end) as out_qty, (case when a.production_type=11 then b.production_qnty else 0 end) as poly_qty from pro_garments_production_mst a, pro_garments_production_dtls b,wo_po_color_size_breakdown c where a.id=b.mst_id and b.COLOR_SIZE_BREAK_DOWN_ID=c.id and a.po_break_down_id=c.po_break_down_id and a.po_break_down_id='$po_id' and a.production_type in(5,8,11) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
		$output_data_arr=array();
		$issue_data_arr=array();
		foreach($issue_qnty_arr as $row)
		{
			$issue_data_arr[$row[csf("po_break_down_id")]][$row[csf("country_id")]][$row[csf("item_number_id")]][$row[csf("pack_type")]][$row[csf("country_ship_date")]]+=$row[csf("fin_qty")];
			$output_data_arr[$row[csf("production_type")]][$row[csf("po_break_down_id")]][$row[csf("country_id")]][$row[csf("item_number_id")]][$row[csf("pack_type")]][$row[csf("country_ship_date")]]+=$row[csf("out_qty")] + $row[csf("poly_qty")];
			$wo_com = $row[csf("serving_company")];
		}
		$control_and_preceding=sql_select("SELECT is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=33 and page_category_id=31 and company_name='$wo_com'"); 		
		$preceding_process = $control_and_preceding[0]['PRECEDING_PAGE_ID'];
		$qty_source=0;
		if($preceding_process==28) $qty_source=4; //Sewing Input
		else if($preceding_process==29) $qty_source=5;//Sewing Output
		else if($preceding_process==30) $qty_source=7;//Iron Output
		...
		if($country_ship_date!=""){ $country_ship_date_cond=" and country_ship_date='$country_ship_date'"; }

		$sqlResult =sql_select("SELECT po_break_down_id, item_number_id, country_id, sum(order_quantity) as order_qnty, sum(plan_cut_qnty) as plan_cut_qnty,pack_type,country_ship_date from wo_po_color_size_breakdown where po_break_down_id='$po_id' and status_active=1  and is_deleted=0 group by po_break_down_id, item_number_id, country_id,pack_type,country_ship_date  order by country_ship_date");
		foreach($sqlResult as $row)
		{
			if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
			$out_qnty=$output_data_arr[$qty_source][$row[csf("po_break_down_id")]][$row[csf("country_id")]][$row[csf("item_number_id")]][$row[csf("pack_type")]][$row[csf("country_ship_date")]];
			$issue_qnty=$issue_data_arr[$row[csf("po_break_down_id")]][$row[csf("country_id")]][$row[csf("item_number_id")]][$row[csf("pack_type")]][$row[csf("country_ship_date")]];
			?>
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="put_country_data(<? echo $row[csf('po_break_down_id')].",".$row[csf('item_number_id')].",".$row[csf('country_id')].",".$row[csf('order_qnty')].",".$row[csf('plan_cut_qnty')];?>,'<?=$row[csf('pack_type')]; ?>','<?=$row[csf('country_ship_date')]; ?>');"> 
				<td width="20"><? echo $i; ?></td>
				...
				<td align="left" width="65"><?  echo $row[csf('pack_type')]; ?></td>
...
if($action=="show_dtls_listview")
{
	...
	<div style="width:100%;max-height:180px; overflow:y-scroll" id="iron_list_view" align="left">
		...
		<?php  
			...
			$material_cost_id_cond = where_con_using_array($mst_id_arr,0,"mst_id");
			$material_cost_id_arr=return_library_array( "SELECT id, id AS ids from  pro_garments_material_cost where status_active=1 $material_cost_id_cond",'id','ids');
			...
			foreach($sqlResult as $selectResult)
			{
				...
				<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data('<? echo $selectResult[csf('id')]."_".$job_no."_".$material_cost_id; ?>','populate_input_form_data','requires/finishing_entry_controller');" > 
					...
					<td width="50" align="center"><?php  echo $selectResult[csf('reject_qnty')]; ?></td>
				
					<td width="50" align="center"><?php echo $selectResult[csf('production_hour')]; ?></td>
					<?php
					$source= $selectResult[csf('production_source')];
					if($source==3)
					$serving_company= $supplier_arr[$selectResult[csf('serving_company')]];
					...
					<td width="100" align="center"><? echo $location_arr[$selectResult[csf('location')]]; ?></td>
					<td width="100" align="center"><? echo $floor_arr[$selectResult[csf('floor_id')]]; ?></td>
					<td  width="60" align="center"><? echo $selectResult[csf('id')]; ?></td>
				</tr>
				<?php
				$i++;
...
if($action=="show_country_listview")
{
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="380" class="rpt_table">
		...
		$issue_qnty_arr=sql_select("SELECT a.serving_company,a.production_type,a.po_break_down_id, a.item_number_id, a.country_id,a.pack_type,a.country_ship_date, b.production_qnty from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id  and a.po_break_down_id='$data' and a.production_type in(5,8,11) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
		$issue_data_arr=array();
		$wo_com = 0;
		foreach($issue_qnty_arr as $row)
		{
			$issue_data_arr[$row[csf("production_type")]][$row[csf("po_break_down_id")]][$row[csf("country_id")]][$row[csf("item_number_id")]][$row[csf("pack_type")]][$row[csf("country_ship_date")]]+=$row[csf("production_qnty")];
			$wo_com = $row[csf("serving_company")];
		}
		$control_and_preceding=sql_select("SELECT is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=33 and page_category_id=31 and company_name='$wo_com'"); 		
		$preceding_process = $control_and_preceding[0]['PRECEDING_PAGE_ID'];
		...
		$i=1;
		$sqlResult =sql_select("SELECT po_break_down_id, item_number_id, country_id, country_ship_date, sum(order_quantity) as order_qnty, sum(plan_cut_qnty) as plan_cut_qnty,pack_type from wo_po_color_size_breakdown where po_break_down_id='$data' and status_active=1  and is_deleted=0 group by po_break_down_id, item_number_id, country_id,pack_type,country_ship_date order by country_ship_date");
		foreach($sqlResult as $row)
		{
			if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
			$out_qnty=$issue_data_arr[$qty_source][$row[csf("po_break_down_id")]][$row[csf("country_id")]][$row[csf("item_number_id")]][$row[csf("pack_type")]][$row[csf("country_ship_date")]];
			$issue_qnty=$issue_data_arr[8][$row[csf("po_break_down_id")]][$row[csf("country_id")]][$row[csf("item_number_id")]][$row[csf("pack_type")]][$row[csf("country_ship_date")]];
			?>
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="put_country_data(<? echo $row[csf('po_break_down_id')].",".$row[csf('item_number_id')].",".$row[csf('country_id')].",".$row[csf('order_qnty')].",".$row[csf('plan_cut_qnty')];?>,'<?=$row[csf('pack_type')]; ?>','<?=$row[csf('country_ship_date')]; ?>');"> 
				<td width="20"><? echo $i; ?></td>
				<td width="100"><p><? echo $garments_item[$row[csf('item_number_id')]]; ?></p></td>
...
if($action=="populate_input_form_data")
{
	$data = explode("_",$data);
	$id = $data[0];
	$sql=sql_select("SELECT  serving_company,po_break_down_id,item_number_id,country_id from  pro_garments_production_mst where  id='$id' and status_active=1 and is_deleted=0 ");
	$company_id=$sql[0][csf("serving_company")];
	$po_break_down_id = $sql[0][csf('po_break_down_id')];
	$item_number_id = $sql[0][csf('item_number_id')];
	$country_id = $sql[0][csf('country_id')];

	// ======================= get country ship date and update prod mst table ====================
	$con = connect();
	$sql_colsize ="SELECT a.mst_id,b.pack_type,b.country_ship_date from  pro_garments_production_dtls a,wo_po_color_size_breakdown b where a.color_size_break_down_id=b.id and a.mst_id=$id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and  b.po_break_down_id='$po_break_down_id' and b.item_number_id='$item_number_id' and b.country_id='$country_id'";
	...
	$update_shidate = execute_query("UPDATE pro_garments_production_mst set country_ship_date='$country_ship_date',pack_type='$pack_type' WHERE id=$id");
	...
	*/	
	...
	$actual_po = sql_select("SELECT actual_po_id,actual_po_dtls_id,sum(prod_qty) as prod_qty,sum(reject_qty) as reject_qty from PRO_GARMENTS_PROD_ACTUAL_PO_DETAILS where mst_id = $mst_id group by actual_po_id,actual_po_dtls_id");
	$actual_po_no = "";
	$actual_po_id = "";
	...
	foreach ($actual_po as $val) 
	{
		...
		if ($actual_po_id == "") $actual_po_id ='**'.$actual_po_library[$val[csf('actual_po_id')]].'**'.$val[csf('actual_po_id')]. '**' . $val[csf('actual_po_dtls_id')].'**********'.$val[csf('prod_qty')].'**'.$val[csf('reject_qty')];
		else $actual_po_id .= '==**'.$actual_po_library[$val[csf('actual_po_id')]].'**'.$val[csf('actual_po_id')] . '**' . $val[csf('actual_po_dtls_id')].'**********'.$val[csf('prod_qty')].'**'.$val[csf('reject_qty')];
	}
	...
	{ 
		...
		echo "load_drop_down_multiple( 'requires/finishing_entry_controller', '".$result[csf('serving_company')]."_".$result[csf('location')]."_".$result[csf('production_source')]."', 'load_drop_down_source_location_and_floor', 'finishing_td*location_td*floor_td' );\n";
		...
		echo "$('#txt_challan').val('".$result[csf('challan_no')]."');\n";
  		...
  		echo "$('#hidden_ship_date').val('".$result[csf('country_ship_date')]."');\n";
		...
		$country_ship_date=$result[csf('country_ship_date')];		
		// $pack_type=''; // pack type blank due to multiple pack type problem.
		...
		if($qty_source)
		{
			$dataSql="SELECT SUM(CASE WHEN a.production_type='$qty_source' THEN b.production_qnty END) as totalinput,SUM(CASE WHEN a.production_type=8 THEN b.production_qnty ELSE 0 END) as totalsewing from pro_garments_production_mst a,pro_garments_production_dtls b,wo_po_color_size_breakdown c WHERE a.id=b.mst_id and c.id=b.color_size_break_down_id and a.po_break_down_id=".$result[csf('po_break_down_id')]."  and a.item_number_id=".$result[csf('item_number_id')]." and a.country_id=".$result[csf('country_id')]." and a.production_type in($qty_source,8) $pack_typeCond2 $country_ship_date_cond2 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0";
			// echo $dataSql;die;
		}
		else
		{
			 $dataSql="SELECT SUM(CASE WHEN production_type=8 THEN production_quantity ELSE 0 END) as totalsewing   from pro_garments_production_mst WHERE po_break_down_id=".$result[csf('po_break_down_id')]."  and item_number_id=".$result[csf('item_number_id')]." and country_id=".$result[csf('country_id')]." $pack_typeCond $country_ship_date_cond and status_active=1 and is_deleted=0";

		    $dataSql_plan_cut="SELECT  sum(plan_cut_qnty) as totalinput from wo_po_color_size_breakdown  WHERE po_break_down_id=".$result[csf('po_break_down_id')]."  and item_number_id=".$result[csf('item_number_id')]." and country_id=".$result[csf('country_id')]." $pack_typeCond $country_ship_date_cond and status_active=1 and is_deleted=0";
		...
		if( $variableSettings!=4)
		{
			...
			{ 
				...
				$country_ship_date_cond2 = str_replace("country_ship_date","b.country_ship_date",$country_ship_date_cond);
				$sql_dtls = sql_select("SELECT color_size_break_down_id, production_qnty, reject_qty, size_number_id, color_number_id from  pro_garments_production_dtls a,wo_po_color_size_breakdown b where a.color_size_break_down_id=b.id and a.mst_id=$data[0] and a.status_active=1 and  b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.status_active=1 and b.is_deleted=0 and country_id='$country_id' $pack_cond $country_ship_date_cond2");	
				...
				$color_size_qty_arr=array();
				$color_size_sql=sql_select ("SELECT  po_break_down_id,item_number_id,size_number_id,color_number_id,sum(plan_cut_qnty) as plan_cut_qnty  from wo_po_color_size_breakdown where status_active=1 and  is_deleted=0    and  po_break_down_id in (".str_replace("'","",$po_id).") and item_number_id=$item_id $pack_typeCond $country_ship_date_cond group by po_break_down_id,item_number_id,size_number_id,color_number_id");
				foreach($color_size_sql as $s_id)
				{
					$color_size_qty_arr[$s_id[csf('color_number_id')]][$s_id[csf('size_number_id')]]+=$s_id[csf('plan_cut_qnty')]; 
				}
				...
				if( $variableSettings==2 ) // color level
				{
					...
					$country_ship_date_cond2 = str_replace("country_ship_date","a.country_ship_date",$country_ship_date_cond);
					$sql = "SELECT a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty,sum(CASE WHEN c.production_type='$qty_source' then b.production_qnty ELSE 0 END) as production_qnty,sum(CASE WHEN c.production_type=8 then b.production_qnty ELSE 0 END) as cur_production_qnty,sum(CASE WHEN c.production_type=8 then b.reject_qty ELSE 0 END) as reject_qty from wo_po_color_size_breakdown a left join pro_garments_production_dtls b on a.id=b.color_size_break_down_id and b.is_deleted=0 and b.status_active=1 left join pro_garments_production_mst c on c.id=b.mst_id and c.is_deleted=0 and c.status_active=1 where a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' $packTypecond $country_ship_date_cond2 and a.is_deleted=0 and a.status_active=1   group by a.item_number_id, a.color_number_id";	
					$sql_plan_cut="SELECT color_number_id, sum(plan_cut_qnty) as quantity from wo_po_color_size_breakdown where po_break_down_id='$po_id' and item_number_id='$item_id' and  country_id='$country_id' $pack_typeCond $country_ship_date_cond and status_active=1 and is_deleted=0 group by color_number_id";
					foreach(sql_select($sql_plan_cut) as $key=>$value)
					{
						$plan_cut_arr[$value[csf("color_number_id")]] +=$value[csf("quantity")];
					}
				...
				else if( $variableSettings==3 ) //color and size level
				{
					...
					else
					{
						...
						$dtlsData_colsize =sql_select("SELECT id as color_size_break_down_id,sum( plan_cut_qnty) as production_qnty  from wo_po_color_size_breakdown  where status_active=1 and is_deleted=0  and po_break_down_id='$po_id' and item_number_id='$item_id' and  country_id='$country_id'  $pack_typeCond $country_ship_date_cond group by id");
					...
					$sql = "SELECT id, item_number_id, size_number_id, color_number_id, order_quantity, plan_cut_qnty from wo_po_color_size_breakdown where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' $pack_typeCond $country_ship_date_cond and is_deleted=0 and status_active=1 order by color_number_id,size_order";
				}
				else // by default color and size level
				{
					$dtlsData = sql_select("SELECT a.color_size_break_down_id,sum(CASE WHEN a.production_type='$qty_source' then a.production_qnty ELSE 0 END) as production_qnty,sum(CASE WHEN a.production_type=8 then a.production_qnty ELSE 0 END) as cur_production_qnty,sum(CASE WHEN a.production_type=8 then a.reject_qty ELSE 0 END) as reject_qty from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in($qty_source,8) group by a.color_size_break_down_id");
...
if ($action=="save_update_delete")
{
	...
	if(!str_replace("'","",$sewing_production_variable)) $sewing_production_variable=3;
 	$control_and_preceding=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=33 and page_category_id=31 and company_name=$cbo_company_name");  
    $is_control=$control_and_preceding[0][csf("is_control")];
    $preceding_process=$control_and_preceding[0][csf("preceding_page_id")];
	$qty_source=5;
	$source_txt="Sewing Input";
	...
	else if($preceding_process==31) 
	{
		...
		$source_txt="Packing And Finishing";
	}
	...
	else if($preceding_process==103) 
	{
		...
		$source_txt="Poly Output";
	}
	else if($preceding_process==256) 
	{
		$qty_source=15;//Poly Entry
		$source_txt="Hang Tag Entry";
	}
	 
	if($variable_qty_source_packing==1) $qty_source=7; //Iron Output
	else if($variable_qty_source_packing==2) $qty_source=11;//Poly Output

	// echo "10**".$hidden_ship_date = "'".date('d-M-Y',strtotime(str_replace("'","",$hidden_ship_date)))."'";die;
	...
	if($wip_valuation_for_accounts==1)
	{
		...
		/* $cost_per_pcs_prev = $cost_per_pcs_sew + $cost_per_pcs_wash;
	...
	if ($operation!=0)
	{
		...
		$backValisql = "SELECT  b.color_size_break_down_id,
					(sum(CASE WHEN b.production_type='8' then b.production_qnty ELSE 0 END)-sum(CASE WHEN b.production_type='8' and b.mst_id=$txt_mst_id then b.production_qnty ELSE 0 END)) as packfinqty,
					(sum(CASE WHEN b.production_type='14' then b.production_qnty ELSE 0 END) + (sum( CASE WHEN b.trans_type=6 and b.production_type=10 THEN b.production_qnty ELSE 0 END)-sum( CASE WHEN b.trans_type=5 and b.production_type=10 THEN b.production_qnty ELSE 0 END))) as production_qnty
					...
					where a.po_break_down_id=$hidden_po_break_down_id and a.item_number_id=$cbo_item_name and a.country_id=$cbo_country_name $ship_date_cond $pack_type_cond and a.is_deleted=0 and a.status_active in(1,2,3) and b.production_type in(8,14,10) group by b.color_size_break_down_id";
		//echo "10**".$sql; die;
		$backValiResult = sql_select($backValisql);
		$nxtQtyArr=array();
		foreach ($backValiResult as $row) {
			$nxtQtyArr[$row[csf("color_size_break_down_id")]]['finDelTrnsQty']+=$row[csf("production_qnty")];
			$nxtQtyArr[$row[csf("color_size_break_down_id")]]['packfinqty']+=$row[csf("packfinqty")];
		}
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$txt_reject_qnty=str_replace("'","",$txt_reject_qnty);
		...
		if($is_control==1 && $user_level!=2)//Dont Hide ISD-23-00520
		{
			if(str_replace("'","",$txt_pack_type)=="") $packType_cond=""; else $packType_cond=" and pack_type=$txt_pack_type";
			if(str_replace("'","",$hidden_ship_date)=="") $country_ship_date_cond=""; else $country_ship_date_cond=" and b.country_ship_date=$hidden_ship_date";
			...
			$sql = "SELECT sum(case when a.production_type=$qty_source then a.production_qnty else 0 end) as output_qty,sum(case when a.production_type=8 then a.reject_qty else 0 end) as reject_qty, sum(case when a.production_type=8 then a.production_qnty else 0 end) as finish_qty from pro_garments_production_dtls a,wo_po_color_size_breakdown b where a.color_size_break_down_id=b.id and a.production_type in('$qty_source',8) and b.po_break_down_id=$hidden_po_break_down_id  and b.country_id=$cbo_country_name and b.item_number_id=$cbo_item_name $packType_cond $country_ship_date_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
			$res = sql_select($sql);
			$prev_process_qty = 0;
			$finishing_qty = 0;
			foreach ($res as $v) 
			{
				$prev_process_qty += $v['OUTPUT_QTY'];
				$finishing_qty += ($v['FINISH_QTY']+$v['REJECT_QTY']);
			}
		
			if($prev_process_qty < $finishing_qty+$txt_finishing_qty+$txt_reject_qnty)
			{
				echo "25**0";
		...
		$field_array="id, mst_id, production_type, color_size_break_down_id, production_qnty, reject_qty,cost_of_fab_per_pcs,cut_oh_per_pcs,trims_cost_per_pcs,cost_per_pcs";
		if(str_replace("'","",$txt_pack_type)=="") $pack_type_cond=""; else $pack_type_cond=" and c.pack_type=$txt_pack_type";
		if(str_replace("'","",$hidden_ship_date)=="") $country_ship_date_cond=""; else $country_ship_date_cond=" and c.country_ship_date=$hidden_ship_date";
		$dtlsData = sql_select("SELECT a.color_size_break_down_id,
										...
										from pro_garments_production_dtls a,wo_po_color_size_breakdown c
										where a.color_size_break_down_id=c.id and c.po_break_down_id=$hidden_po_break_down_id and c.item_number_id=$cbo_item_name and c.country_id=$cbo_country_name and a.status_active=1 and a.color_size_break_down_id<>0 and a.production_type in($qty_source,8) $pack_type_cond  $country_ship_date_cond
		...
		{				  
  		...
  		if(str_replace("'","",$txt_pack_type)=="") $packType_cond=""; else $packType_cond=" and pack_type=$txt_pack_type"; 
		if(str_replace("'","",$hidden_ship_date)=="") $country_ship_date_cond=""; else $country_ship_date_cond=" and country_ship_date=$hidden_ship_date";
		if(str_replace("'","",$sewing_production_variable)==2)//color level wise
		{		
			$color_sizeID_arr=sql_select( "SELECT id,color_number_id from wo_po_color_size_breakdown where po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name  and status_active=1 and is_deleted=0  and country_id=$cbo_country_name  $packType_cond  $country_ship_date_cond order by id" );
			$colSizeID_arr=array(); 
			...
			foreach($rowEx as $rowE=>$val)
			{
				...
				if($wip_valuation_for_accounts==1)
				{
					if($wash_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorSizeNumberIDArr[0]]['cost_per_pcs'])
					{
						// $cost_per_pcs_prev = $wash_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorSizeNumberIDArr[0]]['cost_per_pcs'];				

						$prod_qty = $wash_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorSizeNumberIDArr[0]]['prod_qty'];
						$amount = $wash_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorSizeNumberIDArr[0]]['amount'];

						$cost_per_pcs_prev = $amount/$prod_qty;
					}
					else
					{
						// $cost_per_pcs_prev = $sew_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorSizeNumberIDArr[0]]['cost_per_pcs'];				

						$prod_qty = $sew_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorSizeNumberIDArr[0]]['prod_qty'];
						$amount = $sew_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorSizeNumberIDArr[0]]['amount'];

						$cost_per_pcs_prev = $amount/$prod_qty;
					}

					$finishing_qty = $color_data_array[$colorSizeNumberIDArr[0]]['ok']; 
					$reject_qnty = $color_data_array[$colorSizeNumberIDArr[0]]['rej']; 
					$trims_cost = (($finishing_qty+$reject_qnty)*$trims_issue_avg_rate)/$finishing_qty;
					$trims_cost = number_format($trims_cost,$dec_place[3],'.','');
					$finish_oh = $cpm*$item_smv;
					// echo $cpm."*".$item_smv;die;
				...
				if($colSizeID_arr[$colorSizeNumberIDArr[0]]!="")
				{
					...
					if($j==0)$data_array = "(".$dtls_id.",".$id.",8,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."','".$cost_per_pcs_prev."','".$finish_oh."','".$trims_cost."','".$cost_per_pcs."')";
					else $data_array .= ",(".$dtls_id.",".$id.",8,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."','".$cost_per_pcs_prev."','".$finish_oh."','".$trims_cost."','".$cost_per_pcs."')";
					//$dtls_id=$dtls_id+1;							
	 				$j++;
	 			}								
	 			else
		...
		{		
			...
			foreach($rowExRej as $rowR=>$valR)
			{
				...
				$clr_sz_brk_id = $colorAndSizeRej_arr[0];
				$colorSizeRej  = $colorAndSizeRej_arr[1];
				$rejQtyArr[$clr_sz_brk_id]=$colorSizeRej; 
			}
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				if($is_control==1 && $user_level!=2)//Dont Hide ISD-23-00520
				{
					if($colorSizeValue>0)
					{
						if(($colorSizeValue + $rejQtyArr[$clr_sz_brk_id])>($color_pord_data[$clr_sz_brk_id]*1))
						{
							echo "35**Production Quantity Not Over $source_txt Qnty";
							//check_table_status( 160,0);
							disconnect($con);
				...
				if($wip_valuation_for_accounts==1)
				{
					if($wash_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorID]['cost_per_pcs'])
					{
						// $cost_per_pcs_prev = $wash_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorID]['cost_per_pcs'];				

						$prod_qty = $wash_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorID]['prod_qty'];
						$amount = $wash_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorID]['amount'];

						$cost_per_pcs_prev = $amount/$prod_qty;
					}
					else
					{
						// $cost_per_pcs_prev = $sew_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorID]['cost_per_pcs'];				

						$prod_qty = $sew_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorID]['prod_qty'];
						$amount = $sew_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorID]['amount'];

						$cost_per_pcs_prev = $amount/$prod_qty;

					}
					$finishing_qty = $color_data_array[$colorID]['ok']; 
					$reject_qnty = $color_data_array[$colorID]['rej']; 
					$trims_cost = (($finishing_qty+$reject_qnty)*$trims_issue_avg_rate)/$finishing_qty;
 				...
 				{ */
					//8 for Garments Finishing Entry
					$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",   "pro_garments_production_dtls", $con );
					if($j==0)$data_array = "(".$dtls_id.",".$id.",8,'".$clr_sz_brk_id."','".$colorSizeValue."','".$rejQtyArr[$clr_sz_brk_id]."','".$cost_per_pcs_prev."','".$finish_oh."','".$trims_cost."','".$cost_per_pcs."')";
					else $data_array .= ",(".$dtls_id.",".$id.",8,'".$clr_sz_brk_id."','".$colorSizeValue."','".$rejQtyArr[$clr_sz_brk_id]."','".$cost_per_pcs_prev."','".$finish_oh."','".$trims_cost."','".$cost_per_pcs."')";
					//$dtls_id=$dtls_id+1;
	 				$j++;
	 			/* }
	 			else
		...
		// ============================== actual po wise entry ==================================
		$actual_po_arr= array_filter(explode("==",str_replace("'","",$hidden_actual_po)));
		$act_data="";
		$act_poid = return_next_id("id", "PRO_GARMENTS_PROD_ACTUAL_PO_DETAILS", 1);
		$color_id_arr = array();
		$size_id_arr = array();
		$acc_col_size_qty_arr = array();
		$acc_col_size_rej_qty_arr = array();
		$accColorIDvalue = "";
		foreach($actual_po_arr as $act_po)
		{
			...
			$act_data .= "(" . $act_poid . "," . $id . ",'" . $ex[2]. "','" . $ex[3] . "',8,'" . $ex[8] . "','" . $ex[9] . "','" . $_SESSION['logic_erp']['user_id'] . "','" . $pc_date_time . "',1,0)";
			$act_poid = $act_poid + 1;
			$color_id_arr[$ex[6]] = $ex[6];
			$size_id_arr[$ex[7]] = $ex[7];
			$accColorIDvalue.= ($accColorIDvalue=="") ? $ex[7]."*".$ex[6]."*".$ex[8] : "***".$ex[7]."*".$ex[6]."*".$ex[8]."*".$ex[9];
			$acc_col_size_qty_arr[$ex[6]][$ex[7]] += $ex[8];
			$acc_col_size_rej_qty_arr[$ex[6]][$ex[7]] += $ex[9];
		}
		...
		if(str_replace("'","",$sewing_production_variable)==4)//acc po wise color and size level
		{
			...
			foreach($rowExRej as $rowR=>$valR)
			{
 			...
 			$rowEx = array_filter(explode("***",$accColorIDvalue)); 
			// print_r($colSizeID_arr);die;
			//$dtls_id=return_next_id("id", "pro_garments_production_dtls", 1);
			$data_array="";$j=0;
			foreach($rowEx as $rowE=>$valE)
			{
				...
				if($is_control==1 && $user_level!=2)
				{
 				...
 				if($colSizeID_arr[$index]!="")
 				{
					//8 for Garments Finishing Entry
					$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",   "pro_garments_production_dtls", $con );
					if($j==0)$data_array = "(".$dtls_id.",".$id.",8,'".$colSizeID_arr[$index]."','".$acc_col_size_qty_arr[$colorID][$sizeID]."','".$acc_col_size_rej_qty_arr[$colorID][$sizeID]."')";
					else $data_array .= ",(".$dtls_id.",".$id.",8,'".$colSizeID_arr[$index]."','".$acc_col_size_qty_arr[$colorID][$sizeID]."','".$acc_col_size_rej_qty_arr[$colorID][$sizeID]."')";
					//$dtls_id=$dtls_id+1;
	 				$j++;
					if($acc_col_size_qty_arr[$colorID][$sizeID] > 0 || $acc_col_size_rej_qty_arr[$colorID][$sizeID] > 0)
					{
						$chk = 1;
					}
	 			}
		...
		}//acc po wise color and size level
		...
		if(!empty($act_data))
		{
			$field_actual_po = "id,mst_id,actual_po_id,actual_po_dtls_id,production_type,prod_qty,reject_qty,inserted_by,insert_date,status_active,is_deleted";
			$actualPoId=sql_insert("pro_garments_prod_actual_po_details",$field_actual_po,$act_data,0);
		}
		
		// echo "10**$rID && $dtlsrID && $rID_material && $defectSpot && $actualPoId==".$data_array1;die;
		...
		if($db_type==2 || $db_type==1 )
		{
			if(str_replace("'","",$sewing_production_variable)!=1)
			{
				if($rID && $dtlsrID && $rID_material && $defectSpot && $defectRej && $actualPoId)
				{
					oci_commit($con); 
					echo "0**".str_replace("'","",$hidden_po_break_down_id);
				}
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		{
			if(str_replace("'","",$txt_pack_type)=="") $packType_cond=""; else $packType_cond=" and pack_type=$txt_pack_type";
			$country_iron_qty=return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=$hidden_po_break_down_id and production_type='$qty_source' and country_id=$cbo_country_name and status_active=1 and is_deleted=0");
		...
		if(str_replace("'","",$sewing_production_variable)!=1 && str_replace("'","",$txt_mst_id)!='' ) //  not gross level
		{
				...
				$dtlsData = sql_select("SELECT a.color_size_break_down_id,
										...
										where a.status_active=1 and a.mst_id=b.id and c.id=a.color_size_break_down_id and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name and b.country_id=$cbo_country_name and a.color_size_break_down_id<>0 and a.production_type in($qty_source,8) and b.id !=$txt_mst_id $pack_type_cond $country_ship_date_cond
										group by a.color_size_break_down_id");
			$color_pord_data=array();							
			foreach($dtlsData as $row)
			{				  
			...
			if(str_replace("'","",$sewing_production_variable)==2)//color level wise
			{		
				$color_sizeID_arr=sql_select( "SELECT id,color_number_id from wo_po_color_size_breakdown where po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name and country_id=$cbo_country_name  $packType_cond and status_active=1 and is_deleted=0  order by id" );
				$colSizeID_arr=array(); 
				foreach($color_sizeID_arr as $val){
					$index = $val[csf("color_number_id")];
					$colSizeID_arr[$index]=$val[csf("id")];
				...
				foreach($rowEx as $rowE=>$val)
				{
					...
					if($wip_valuation_for_accounts==1)
					{
						...
						// $cost_per_pcs_prev = $sew_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorSizeNumberIDArr[0]]['cost_per_pcs'] + $wash_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorSizeNumberIDArr[0]]['cost_per_pcs'];
					...
					$nxt_process_qty=$nxtQtyArr[$colSizeID_arr[$colorSizeNumberIDArr[0]]]['finDelTrnsQty']*1;
					$current_priv_qty=($nxtQtyArr[$colSizeID_arr[$colorSizeNumberIDArr[0]]]['packfinqty']*1)+($colorSizeNumberIDArr[1]*1);
					
					if( $nxt_process_qty > $current_priv_qty)
					{
						echo "36**Packing N Finishing Qty is not less then Gmts Delivery and Trnsfer.";
						//check_table_status( 160,0);
						disconnect($con);
						die;
					}
					...
					{
						$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",  "pro_garments_production_dtls", $con );
						if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",8,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."','".$cost_per_pcs_prev."','".$finish_oh."','".$trims_cost."','".$cost_per_pcs."')";
						else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",8,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."','".$cost_per_pcs_prev."','".$finish_oh."','".$trims_cost."','".$cost_per_pcs."')";
						$j++;	
					}
					else
					{
			...
			if(str_replace("'","",$sewing_production_variable)==3)//color and size wise
			{		
				/* $country_ship_date_cond2 = str_replace("b.country_ship_date","country_ship_date",$country_ship_date_cond);
				$color_sizeID_arr=sql_select( "select id,size_number_id,color_number_id from wo_po_color_size_breakdown where po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name and country_id=$cbo_country_name $packType_cond $country_ship_date_cond2 and status_active=1 and is_deleted=0 order by size_number_id,color_number_id" );
				$colSizeID_arr=array(); 
				foreach($color_sizeID_arr as $val){
					$index = $val[csf("size_number_id")].$color_arr[$val[csf("color_number_id")]].$val[csf("color_number_id")];
					$colSizeID_arr[$index]=$val[csf("id")];
				...
				foreach($rowEx as $rowE=>$valE)
				{
					...
					if($is_control==1 && $user_level!=2)//Dont Hide ISD-23-00520
					{
						if($colorSizeValue>0)
						{
							if(($colorSizeValue*1)>($color_pord_data[$clr_sz_brk_id]*1))
							{
					...
					$nxt_process_qty=$nxtQtyArr[$clr_sz_brk_id]['finDelTrnsQty']*1;
					$current_priv_qty=($nxtQtyArr[$clr_sz_brk_id]['packfinqty']*1)+($colorSizeValue*1);
					
					//echo "10**".$nxt_process_qty.'-'.$nxtQtyArr[$colSizeID_arr[$index]]['packfinqty'].'-'.$colorSizeValue; die;
					
					if( $nxt_process_qty > $current_priv_qty)
					{
						echo "36**Packing N Finishing Qty is not less then Gmts Fin. Delivery and Transfer.";
						//check_table_status( 160,0);
						disconnect($con);
						die;
					}									
					if($wip_valuation_for_accounts==1)
					{
						...
						// $cost_per_pcs_prev = $sew_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorID]['cost_per_pcs'] + $wash_cost_array[str_replace("'","",$hidden_po_break_down_id)][str_replace("'","",$cbo_item_name)][$colorID]['cost_per_pcs'];
					...
					{ */
						//8 for Garments Finishing Entry
						$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",   "pro_garments_production_dtls", $con );
						if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",8,'".$clr_sz_brk_id."','".$colorSizeValue."','".$rejQtyArr[$clr_sz_brk_id]."','".$cost_per_pcs_prev."','".$finish_oh."','".$trims_cost."','".$cost_per_pcs."')";
						else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",8,'".$clr_sz_brk_id."','".$colorSizeValue."','".$rejQtyArr[$clr_sz_brk_id]."','".$cost_per_pcs_prev."','".$finish_oh."','".$trims_cost."','".$cost_per_pcs."')";
						//$dtls_id=$dtls_id+1;
						$j++;
					/* }
					else
			...
			foreach($actual_po_arr as $act_po)
			{
				...
				$ex[9] = ($ex[9]=="") ? 0 : $ex[9];
				if ($act_data != "") $act_data .= ",";
				$act_data .= "(" . $act_poid . "," . $txt_mst_id . ",'" . $ex[2]. "','" . $ex[3] . "',8,'" . $ex[8] . "','" . $ex[9] . "','" . $_SESSION['logic_erp']['user_id'] . "','" . $pc_date_time . "',1,0)";
				$act_poid = $act_poid + 1;
				$color_id_arr[$ex[6]] = $ex[6];
				$size_id_arr[$ex[7]] = $ex[7];
				$accColorIDvalue.= ($accColorIDvalue=="") ? $acc_size_lib[$ex[3]]."*".$acc_color_lib[$ex[3]]."*".$ex[8]."*".$ex[9] : "***".$acc_size_lib[$ex[3]]."*".$acc_color_lib[$ex[3]]."*".$ex[8]."*".$ex[9];
				$acc_col_size_qty_arr[$acc_color_lib[$ex[3]]][$acc_size_lib[$ex[3]]] += $ex[8];
				$acc_col_size_rej_qty_arr[$acc_color_lib[$ex[3]]][$acc_size_lib[$ex[3]]] += $ex[9];
			}
			
			if(str_replace("'","",$sewing_production_variable)==4)//color and size wise
			{
				$chk=0;
				$field_array="id, mst_id, production_type, color_size_break_down_id, production_qnty, reject_qty";
				$color_sizeID_arr=sql_select( "SELECT id,size_number_id,color_number_id from wo_po_color_size_breakdown where po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name  and status_active=1 and is_deleted=0  and country_id=$cbo_country_name $packType_cond order by size_number_id,color_number_id" );
				...
				foreach($rowEx as $rowE=>$valE)
				{
					...
					$nxt_process_qty=$nxtQtyArr[$colSizeID_arr[$index]]['finDelTrnsQty']*1;
					$current_priv_qty=($nxtQtyArr[$colSizeID_arr[$index]]['packfinqty']*1)+($acc_col_size_qty_arr[$colorID][$sizeID]*1);
					...
					if($colSizeID_arr[$index]!="")
					{
						//8 for Garments Finishing Entry
						$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",   "pro_garments_production_dtls", $con );
						if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",8,'".$colSizeID_arr[$index]."','".$acc_col_size_qty_arr[$colorID][$sizeID]."','".$acc_col_size_rej_qty_arr[$colorID][$sizeID]."')";
						else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",8,'".$colSizeID_arr[$index]."','".$acc_col_size_qty_arr[$colorID][$sizeID]."','".$acc_col_size_rej_qty_arr[$colorID][$sizeID]."')";
						//$dtls_id=$dtls_id+1;
						$j++;
						if($acc_col_size_qty_arr[$colorID][$sizeID] > 0 || $acc_col_size_rej_qty_arr[$colorID][$sizeID] > 0)
						{
		...
		$rID = $dtlsrDelete = $dtlsrID = $rIDmaterial=$defectSpot=$actualPoId=true;
		if($data_array_defectsp!="")
		{
		...
		// echo "10**$rID**$txt_mst_id";die;
		if(str_replace("'","",$sewing_production_variable)!=1 && str_replace("'","",$txt_mst_id)!='' && $data_array!="")// check is not gross level
		{
			...
			if(!empty($act_data))
			{
				$actualPoDelete = execute_query("delete from pro_garments_prod_actual_po_details where mst_id=$txt_mst_id", 1);
				$field_actual_po = "id,mst_id,actual_po_id,actual_po_dtls_id,production_type,prod_qty,reject_qty,updated_by,update_date,status_active,is_deleted";
				$actualPoId=sql_insert("pro_garments_prod_actual_po_details",$field_actual_po,$act_data,0);
			}
		}
		// echo "10**INSERT INTO pro_garments_prod_actual_po_details (".$field_actual_po.") VALUES ".$act_data;die;
		$rIDmaterial=1;
		...
		// echo "10**".$rID."**".$rID ."**". $dtlsrDelete ."**". $dtlsrID ."**". $rIDmaterial ."**". $defectSpot ."**". $defectRej ."**". $actualPoId;die;
		...
		if($db_type==2 || $db_type==1 )
		{
			if(str_replace("'","",$sewing_production_variable)!=1)
			{
				if($rID && $dtlsrDelete && $dtlsrID && $rIDmaterial && $defectSpot && $defectRej && $actualPoId)
				{
					oci_commit($con); 
					echo "1**".str_replace("'","",$hidden_po_break_down_id);
				}
	...
	else if ($operation==2)  // Delete Here---------------------------------------------------------- 
	{
		...
		$ex_fact_sql   = sql_select("SELECT id from pro_ex_factory_mst where po_break_down_id=$hidden_po_break_down_id and status_active=1 and is_deleted=0");
		$buyer_ins_sql = sql_select("SELECT id from pro_buyer_inspection where po_break_down_id=$hidden_po_break_down_id and status_active=1 and is_deleted=0");
		$transfer_sql  = sql_select("SELECT id from pro_garments_production_mst where po_break_down_id=$hidden_po_break_down_id and production_type=10 and trans_type=6 and status_active=1 and is_deleted=0");
		
		$ex_fact_records = count($ex_fact_sql);
		$buyer_ins_records = count($buyer_ins_sql);
		$transfer_sql_records = count($transfer_sql); 
		if ($ex_fact_records || $buyer_ins_records || $transfer_sql_records) 
		{
			echo "99**$ex_fact_records**$buyer_ins_records**$transfer_sql_records**$hidden_po_break_down_id"; die;
		} 
...
<div style="width:930px;">
    <table width="900" cellspacing="0" align="right">
        ...
        <tr>
            ...
            ?> 
        	<td width="270" rowspan="3" valign="top" colspan="2"><strong>Issue To : <? if($dataArray[0][csf('production_source')]==1) echo $company_library[$dataArray[0][csf('serving_company')]]; else if($dataArray[0][csf('production_source')]==3) echo $supplier_library[$dataArray[0][csf('serving_company')]].'<br>'.$address;  ?></strong></td>
        ...
        <tr>
            ...
            <td><strong>Order Qnty:</strong></td><td><? echo $dataArray[0][csf('production_quantity')]; ?></td>
        </tr>
        <tr>
            <td><strong>Source:</strong></td><td><? echo $knitting_source[$dataArray[0][csf('production_source')]]; ?></td>
            <td><strong>Date:</strong></td><td><? echo change_date_format($dataArray[0][csf('production_date')]); ?></td>
        </tr>
        <tr>
        ...
        <tr>
        	<td><strong>Carton Qty:</strong></td> <td><? echo $dataArray[0][csf('carton_qty')]; ?></td>
        	<td><strong>WO No:</strong></td> <td><? echo $wo_no; ?></td>
        ...
        <tr>
            ...
            <td><strong>Sys. Challan No:</strong></td> <td><? echo $dataArray[0][csf('id')]; ?></td>
...
if($action=="production_process_control")
{
    ...
    $control_and_preceding=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=33 and page_category_id=31 and company_name='$data'");  
