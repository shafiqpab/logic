...
{
/* 	echo create_drop_down( "cbo_floor", 180, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process in (9) order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/bundle_wise_iron_entry_controller', $('#cbo_company_name').val()+'_'+$('#cbo_location').val()+'_'+this.value, 'load_drop_down_line', 'line_td' );",0 );
*/	
 	echo create_drop_down( "cbo_floor", 180, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/bundle_wise_iron_entry_controller', $('#cbo_company_name').val()+'_'+$('#cbo_location').val()+'_'+this.value, 'load_drop_down_line', 'line_td' );",0 );
...
{
	list($company_id,$location,$floor)=explode("_",$data);	
	...
	if($prod_reso_allocation==1)
	{
		...
		echo create_drop_down( "cbo_line_no", 180,$line_array,"", 1, "--- Select ---", $selected, "",0,0 );
	}
	else
	{
...
if($action=="load_drop_down_embro_issue_source")
{
	...
	if($data==3)
	{
		if($db_type==0)
		{
			echo create_drop_down( "cbo_emb_company", 180, "select id,supplier_name from lib_supplier where status_active=1 and is_deleted=0 and find_in_set(23,party_type) order by supplier_name","id,supplier_name", 1, "--- Select ---", $selected, "load_drop_down( 'requires/bundle_wise_iron_entry_controller', this.value, 'load_drop_down_location', 'location_td' );" );
		}
		else
		{
			echo create_drop_down( "cbo_emb_company", 180, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=23 and a.status_active=1 group by a.id,a.supplier_name order by a.supplier_name","id,supplier_name", 1, "--Select--", 0, "load_drop_down( 'requires/bundle_wise_iron_entry_controller', this.value, 'load_drop_down_location', 'location_td' );" );
		}
	}
	else if($data==1)
		echo create_drop_down( "cbo_emb_company", 180, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1 $company_cond order by company_name","id,company_name", 1, "--- Select ---", $selected_company, "load_drop_down( 'requires/bundle_wise_iron_entry_controller', this.value, 'load_drop_down_location', 'location_td' );",0,0 );	
	else
		echo create_drop_down( "cbo_emb_company", 180, $blank_array,"", 1, "--- Select ---", $selected, "load_drop_down( 'requires/bundle_wise_iron_entry_controller', this.value, 'load_drop_down_location', 'location_td' );",0 );	
...
if($action=="create_bundle_search_list_view")
{
	...
	$scanned_bundle_arr=return_library_array( "select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=7  and b.status_active=1 and b.is_deleted=0",'bundle_no','bundle_no');
...
if($action=="load_mst_data")
{
	...
	$sql_mst_data=sql_select("SELECT a.sys_number, a.company_id,a.location_id, a.embel_name, a.embel_type, a.serving_company,a.floor_id,a.sewing_line,a.organic,
	a.production_source
	from  pro_gmts_delivery_mst a,pro_garments_production_dtls c where  a.id=c.delivery_mst_id and c.production_type=5
	 and a.status_active=1 and a.is_deleted=0  and c.status_active=1 and c.is_deleted=0 $bundle_nos_cond
	group by a.sys_number, a.company_id,a.location_id, a.embel_name, a.embel_type, a.serving_company,a.floor_id,a.sewing_line,a.organic,a.production_source");
	echo "SELECT a.sys_number, a.company_id,a.location_id, a.embel_name, a.embel_type, a.serving_company,a.floor_id,a.sewing_line,a.organic,
	a.production_source
	from  pro_gmts_delivery_mst a,pro_garments_production_dtls c where  a.id=c.delivery_mst_id and c.production_type=5
	 and a.status_active=1 and a.is_deleted=0  and c.status_active=1 and c.is_deleted=0 $bundle_nos_cond
	group by a.sys_number, a.company_id,a.location_id, a.embel_name, a.embel_type, a.serving_company,a.floor_id,a.sewing_line,a.organic,a.production_source";
	...
	if(count($sql_mst_data)>0)
	{
		...
		{
			echo "load_drop_down( 'requires/bundle_wise_iron_entry_controller', $('#cbo_company_name').val()+'_'+".$val[csf('location_id')]."+'_'+".$val[csf('floor_id')].", 'load_drop_down_line', 'line_td' );\n";
			...
			echo "disable_enable_fields('cbo_company_name*cbo_source*cbo_emb_company*cbo_location*cbo_floor*cbo_line_no',1);\n";
		}
	}
	else
	{
...
if($action=="challan_duplicate_check")
{
	...
	$result=sql_select("SELECT a.sys_number,b.bundle_no from pro_gmts_delivery_mst a,pro_garments_production_dtls b where a.id=b.delivery_mst_id and b.production_type=7 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $bundle_nos_cond group by a.sys_number,b.bundle_no");
	$datastr="";
	if(count($result)>0)
	{
		foreach ($result as $row)
...
if($action=="populate_bundle_data")
{
	...
	$scanned_bundle_arr=return_library_array( "SELECT c.bundle_no, c.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls c where a.id=c.mst_id and c.production_type=7 and c.status_active=1 and c.is_deleted=0 $bundle_nos_cond",'bundle_no','bundle_no');
...
if($action=="populate_bundle_data_update")
{
	...
	$sql="SELECT d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, $year_field as year, f.buyer_name, d.item_number_id, d.country_id,c.reject_qty,c.alter_qty,c.spot_qty,c.replace_qty, d.size_number_id, d.color_number_id, c.cut_no, c.bundle_no,c.barcode_no, c.production_qnty as qty, e.po_number from pro_garments_production_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name=$company_id and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_id=f.id and c.production_type=7 and c.status_active=1 and c.is_deleted=0 $bundle_nos_cond order by c.cut_no, c.bundle_no DESC";
	// echo $sql; die;
	$result = sql_select($sql);
	$count=count($result);
	$i=$ex_data[1]+$count;
	...
	{ 
		...
		<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i;?>"> 
			...
			<td width="80" style="word-break:break-all;" align="left"><? echo $color_arr[$row[csf('color_number_id')]]; ?></td>
			<td width="70" style="word-break:break-all;" align="left">&nbsp;<? echo $size_arr[$row[csf('size_number_id')]]; ?></td>
			<td width="80" id="prodQty_<? echo $i; ?>" align="right"><? echo ($row[csf('qty')]+$row[csf('reject_qty')])-$row[csf('replace_qty')]; ?>&nbsp;</td>
			
			<td width="50" id="rejQty_<? echo $i; ?>" align="center"><input type="text" class="text_boxes_numeric" name="rejectQty[]" id="rejectQty_<? echo $i; ?>" style="width:35px" value="<? echo $row[csf('reject_qty')];?>" onBlur="calculate_qcpasss(<? echo $i; ?>)" onKeyPress="return numOnly(this,event,this.id);"/></td>
...
{
	$bundle_nos=return_library_array( "select b.bundle_no, b.bundle_no from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id and a.delivery_mst_id=$data and b.production_type=7 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","bundle_no","bundle_no");
	...
	<div style="width:100%;max-height:180px; overflow-y:scroll" id="sewing_production_list_view" align="left">
		...
		<?php  
			...
			foreach($sqlResult as $selectResult)
			{
 			...
 			?>
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_issue_form_data','requires/bundle_wise_iron_entry_controller');" > 
...
if($action=="populate_issue_form_data")
{
	...
	{ 
		...
		echo "get_php_form_data(".$result[csf('po_break_down_id')]."+'**'+".$result[csf("item_number_id")]."+'**'+".$result[csf("embel_name")]."+'**'+".$result[csf("country_id")].", 'populate_data_from_search_popup', 'requires/bundle_wise_iron_entry_controller' );\n";

		echo "$('#cbo_item_name').val('".$result[csf('item_number_id')]."');\n";
		echo "$('#cbo_country_name').val('".$result[csf('country_id')]."');\n";
		
		echo "show_list_view('".$result[csf('po_break_down_id')]."','show_country_listview','list_view_country','requires/bundle_wise_iron_entry_controller','');\n";
		
		echo "$('#txt_mst_id').val('".$result[csf('id')]."');\n";
 		echo "set_button_status(1, permission, 'fnc_iron_bundle_entry',1,1);\n";
...
if ($action=="save_update_delete")
{
	...
	//	if ( check_table_status( $_SESSION['menu_id'], 1 )==0 ) { echo "15**0"; die;} 
		if(str_replace("'","",$txt_system_id)=="")
		{
			...
			if($db_type==0) $year_cond="YEAR(insert_date)"; 
  		    ...
  		    $new_sys_number = explode("*", return_next_id_by_sequence("", "pro_gmts_delivery_mst",$con,1,$cbo_company_name,'BIE',0,date("Y",time()),0,0,7,0,0 ));
			...
			$data_array_delivery="(".$mst_id.",'".$new_sys_number[1]."','".(int)$new_sys_number[2]."','".$new_sys_number[0]."', ".$cbo_company_name.",7,".$cbo_location.",".$delivery_basis.",".$cbo_source.",".$cbo_emb_company.",".$cbo_floor.",".$cbo_line_no.",".$txt_organic.",".$txt_issue_date.",".$cbo_working_company_name.",".$cbo_working_location.",".$user_id.",'".$pc_date_time."')";
		...
		else
		{
			...
			$field_array_delivery="location_id*delivery_basis*production_source*serving_company*floor_id*sewing_line*organic*delivery_date*working_company_id*working_location_id*updated_by*update_date";
			$data_array_delivery="".$cbo_location."*".$delivery_basis."*".$cbo_source."*".$cbo_emb_company."*".$cbo_floor."*".$cbo_line_no."*".$txt_organic."*".$txt_issue_date."*".$cbo_working_company_name."*".$cbo_working_location."*".$user_id."*'".$pc_date_time."'";
		}
		...
		{			
			...
			$field_array_mst="id, delivery_mst_id, company_id, garments_nature, challan_no, po_break_down_id, item_number_id, country_id, production_source, serving_company, location, production_date, production_hour,production_quantity,reject_qnty,replace_qty, production_type, entry_break_down_type, remarks, floor_id,sewing_line, inserted_by, insert_date";
			...
			foreach($mstArr as $orderId=>$orderData)
			{
				foreach($orderData as $gmtsItemId=>$gmtsItemIdData)
				{
					foreach($gmtsItemIdData as $countryId=>$qty)
					{
						...
						if($db_type==2)
						{
						$data_array_mst.=" INTO pro_garments_production_mst (".$field_array_mst.") VALUES(".$id.",".$mst_id.",".$cbo_company_name.",".$garments_nature.",'".$challan_no."',".$orderId.", ".$gmtsItemId.",".$countryId.", ".$cbo_source.",".$cbo_emb_company.",".$cbo_location.",".$txt_issue_date.",".$txt_reporting_hour.",".$qty.",'".$mstArrRej[$orderId][$gmtsItemId][$countryId]."','".$mstArrRep[$orderId][$gmtsItemId][$countryId]."',7,".$sewing_production_variable.",".$txt_remark.",".$cbo_floor.",".$cbo_line_no.",".$user_id.",'".$pc_date_time."')";
						}
						else
						{
						if($data_array_mst!="") $data_array_mst.=",";
						$data_array_mst.="(".$id.",".$mst_id.",".$cbo_company_name.",".$garments_nature.",'".$challan_no."',".$orderId.", ".$gmtsItemId.",".$countryId.", ".$cbo_source.",".$cbo_emb_company.",".$cbo_location.",".$txt_issue_date.",".$txt_reporting_hour.",".$qty.",'".$mstArrRej[$orderId][$gmtsItemId][$countryId]."','".$mstArrRep[$orderId][$gmtsItemId][$countryId]."',7,".$sewing_production_variable.",".$txt_remark.",".$cbo_floor.",".$cbo_line_no.",".$user_id.",'".$pc_date_time."')";
						}
			...
			$field_array_dtls="id,delivery_mst_id,mst_id,production_type,color_size_break_down_id,production_qnty,reject_qty,replace_qty,cut_no,bundle_no,color_type_id";
			...
			foreach($dtlsArr as $bundle_no=>$qty)
			{
				...
				$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",7,'".$dtlsArrColorSize[$bundle_no]."','".$qty."','".$dtlsArrRej[$bundle_no]."','".$dtlsArrRep[$bundle_no]."','".$cut_no."','".$bundle_no."','".$bundle_wise_type_array[$bundle_no]."')";
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$field_array_delivery="location_id*production_source*serving_company*floor_id*organic*delivery_date*working_company_id*working_location_id*updated_by*update_date";
		$data_array_delivery="".$cbo_location."*".$cbo_source."*".$cbo_emb_company."*".$cbo_floor."*".$txt_organic."*".$txt_issue_date."*".$cbo_working_company_name."*".$cbo_working_location."*".$user_id."*'".$pc_date_time."'";
		...
		if(str_replace("'","",$delivery_basis)==3)
		{
			...
			$delete = execute_query("DELETE FROM pro_garments_production_mst WHERE delivery_mst_id=$mst_id and production_type=7");
			$delete_dtls = execute_query("DELETE FROM pro_garments_production_dtls WHERE delivery_mst_id=$mst_id and production_type=7");
			$delete_bundle = execute_query("DELETE FROM pro_cut_delivery_color_dtls WHERE delivery_mst_id=$mst_id and production_type=7");
		...
		else
		{
			...
			$field_array1="production_source*serving_company*location*production_date*production_quantity*production_type*entry_break_down_type*challan_no*remarks*floor_id*sewing_line*total_produced*yet_to_produced*updated_by*update_date";
			
			$data_array1="".$cbo_source."*".$cbo_emb_company."*".$cbo_location."*".$txt_issue_date."*".$txt_issue_qty."*5*".$sewing_production_variable."*'".$challan_no."'*".$txt_remark."*".$cbo_floor."*".$cbo_line_no."*".$txt_cumul_issue_qty."*".$txt_yet_to_issue."*".$user_id."*'".$pc_date_time."'";
		...
		<legend>Enter search words</legend>           
            <table cellpadding="0" cellspacing="0" width="700" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td align="center">
                    	<input type="text" name="txt_issue_date" id="txt_issue_date" value="" class="datepicker" style="width:107px;"  />       
                    </td>
                    <td align="center" id="emb_company_td">				
                    	<?
							echo create_drop_down( "cbo_emb_company", 100, $line_library,"", 1, "--- Select ---", $selected, "" );  
						?>       
                    </td> 	
            		<td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_line_no').value+'_'+document.getElementById('txt_issue_date').value+'_'+document.getElementById('txt_company_id').value+'_'+document.getElementById('cbo_emb_company').value+'_'+document.getElementById('txt_order_no').value, 'create_challan_search_list_view', 'search_div', 'bundle_wise_iron_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                     </td>
...
<script src="../../includes/functions_bottom.js" type="text/javascript"></script>
<script>
load_drop_down('bundle_wise_iron_entry_controller', 3, 'load_drop_down_embro_issue_source', 'emb_company_td');
...
{
	list($challan,$line_no,$issue_date,$company_id,$sew_company,$order_no) = explode("_",$data);
	$line_library=return_library_array( "select id,line_name from lib_sewing_line", "id", "line_name"  );
	$search_string="%".trim($data[0])."";
	if($challan!=''){$challan_con=" and b.challan_no ='$challan'";}
	if($line_no!=0){$line_con=" and b.sewing_line ='$line_no'";}
	...
	$sql = "select a.id, $year_field, a.sys_number_prefix_num, a.sys_number, a.delivery_date, a.production_source, a.serving_company, a.location_id, a.floor_id,b.sewing_line,c.po_number from pro_gmts_delivery_mst a,pro_garments_production_mst b ,wo_po_break_down c  where b.po_break_down_id=c.id and a.id=b.delivery_mst_id and a.production_type=7 and b.production_type=7 and a.status_active=1 and a.is_deleted=0 and a.company_id=$company_id $order_con $challan_con $line_con $issue_date_con $sew_company_con group by a.id , a.sys_number_prefix_num, a.sys_number, a.delivery_date, a.production_source, a.serving_company,a.location_id, a.floor_id,b.sewing_line,c.po_number order by a.id DESC"; 
...
if($action=='populate_data_from_challan_popup')
{
	if($db_type==2)
	{
		$data_array=sql_select("select a.id, a.company_id, a.sys_number, a.embel_type, a.embel_name, a.production_source, a.serving_company, a.location_id, a.floor_id, a.organic, a.delivery_date,b.sewing_line,TO_CHAR(b.production_hour,'HH24:MI') as production_hour,a.working_company_id,a.working_location_id from pro_gmts_delivery_mst a,pro_garments_production_mst b where a.id=b.delivery_mst_id and a.id='$data'");
	}
	else
	{
		$data_array=sql_select("select a.id, a.company_id, a.sys_number, a.embel_type, a.embel_name, a.production_source, a.serving_company, a.location_id, a.floor_id, a.organic, a.delivery_date,b.sewing_line,b.production_hour,a.working_company_id,a.working_location_id from pro_gmts_delivery_mst a,pro_garments_production_mst b where a.id=b.delivery_mst_id and a.id='$data'");
	}
	
	foreach ($data_array as $row)
	{ 
		echo "load_drop_down( 'requires/bundle_wise_iron_entry_controller', $('#cbo_company_name').val()+'_'+".$row[csf('location_id')]."+'_'+".$row[csf('floor_id')].", 'load_drop_down_line', 'line_td' );\n";
		...
		echo "load_drop_down( 'requires/bundle_wise_iron_entry_controller', ".$row[csf('production_source')].", 'load_drop_down_embro_issue_source', 'emb_company_td' );\n";
		echo "load_drop_down( 'requires/bundle_wise_iron_entry_controller','".$row[csf('company_id')]."', 'load_drop_down_location', 'location_td' );\n";
		echo "$('#cbo_emb_company').val('".$row[csf('serving_company')]."');\n";
		echo "$('#cbo_location').val('".$row[csf('location_id')]."');\n";
		echo "load_drop_down( 'requires/bundle_wise_iron_entry_controller', '".$row[csf('location_id')]."', 'load_drop_down_floor', 'floor_td' );\n";
		echo "load_drop_down( 'requires/bundle_wise_iron_entry_controller','".$row[csf('working_company_id')]."', 'load_drop_down_working_location', 'working_location_td' );\n";
		...
		echo "set_button_status(0, '".$_SESSION['page_permission']."', 'fnc_iron_bundle_entry',1,1);\n";  
		exit();
	}
}
...
if($action=="emblishment_issue_print")
{
	...
	production_source, serving_company, floor_id, organic, delivery_date,working_company_id,working_location_id from pro_gmts_delivery_mst where production_type=7 and id='$data[1]' and 
    ...
    <table width="900" cellspacing="0" align="right">
        ...
        <?
			...
			if($data[2]==3)
			{
				...
				and a.id=b.mst_id and b.color_size_break_down_id=d.id and b.production_type=7 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 
				and d.is_deleted=0 
			...
			else
			{
				...
				and c.id=a.mst_id  and a.color_size_break_down_id=b.id and b.production_type=7 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
			...
			foreach ($result as $v) 
			{
				$po_id_array[$v[csf('po_break_down_id')]] = $v[csf('po_break_down_id')];
			}
