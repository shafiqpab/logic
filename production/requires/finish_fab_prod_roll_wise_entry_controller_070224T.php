...
if ($action == "save_update_delete") {
	...
	{   
		...
		$field_array_roll_update = "qnty*qc_pass_qnty*reject_qnty*updated_by*update_date";
		...
		for ($j = 1; $j <= $tot_row; $j++) 
		{
			...
			if ($$rolltableId > 0) {
				...
				$data_array_update_roll[$$rolltableId] = explode("*", ( $$rollWgt . "*'" . $$qcPassQty . "'*'" . $$reJectQty . "'*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'"));
	...
	*/
	...
	foreach ($res as $row) 
	{
		...
		// $pdf -> Line(62, 0, 62, 14);
		 ...
		 $logo = "../../" . $_SESSION['logic_erp']["group_logo"];
