            ...
            <fieldset style="width:1060px;">
                ...
                <tr class="general">
                    ...
                    ?>
                        ...
                        <td align="center">
                            <input type="text" style="width:130px" class="text_boxes" name="txt_internal_ref"
                                   id="txt_internal_ref"/>
                        </td>
                        <td align="center" id="search_by_td">
...
if ($action == "create_bundle_search_list_view")
{
    ...
    if (trim($job_no) == '' && trim($order_no) == '' && trim($cut_no) == '' && trim($bndl_no) == '' && trim($internal_ref) == '' && trim($order_cut_no) == '')
    {
   ...
   // $scanned_bundle_arr = return_library_array("select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=4 and  b.status_active=1 and b.is_deleted=0", 'bundle_no', 'bundle_no');
    ...
    if(trim($job_no)!='' || trim($ex_data[0])!="")
    {
        ...
        $sql = "SELECT b.id from wo_po_details_master a,wo_po_break_down b where a.id=b.job_id and a.status_active=1 and b.status_active=1 $jobnoCond $poCond";
        $res = sql_select($sql);
        $po_id_arr = array();
        foreach ($res as $val) 
        {
   ...
   /*$scanned_bundle_arr = return_library_array("select bundle_no, bundle_no from pro_garments_production_dtls where production_type=4 and cut_no='".$tmp_cut."' and status_active=1 and is_deleted=0", 'bundle_no', 'bundle_no');
    ...
    from pro_garments_production_dtls c, pro_gmts_delivery_mst d
    where c.delivery_mst_id=d.id $bndlCon $year_cond $cutCon and d.status_active=1 and d.is_deleted=0 and c.production_type=$qty_source group by d.id, d.sys_number_prefix_num, c.delivery_mst_id");
    $challan_arr=array();
        ...
        <?
            ...
            foreach($last_operation as  $item_id=>$operation_cond)
            {
                ...
                else
                {
                    ...
                    $sqld = sql_select( "SELECT c.bundle_no, SUM(c.reject_qty) as reject_qty, SUM(c.alter_qty) as alter_qty, SUM(c.spot_qty) as spot_qty, SUM(c.replace_qty) as replace_qty  from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e,wo_po_details_master f where d.job_id=f.id and e.job_id=f.id and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and a.company_id=$company  and c.production_type=$qty_source  $orderCon $bndlCon $year_cond   $jobCon $cutCon  and a.status_active=1 and a.is_deleted=0 group by  c.bundle_no order by  length(c.bundle_no) asc, c.bundle_no asc" );
                    foreach($sqld as $arows)
                    {
                        $reject_qty[$arows[csf('bundle_no')]]+=$arows[csf('reject_qty')];
                        $alter_qty[$arows[csf('bundle_no')]]+=$arows[csf('alter_qty')];
                ...
                // {
                    ...
                    if($order_cut_no=="")
                    {
                        ...
                        $sql="SELECT c.cut_no,d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.bundle_no, sum(c.production_qnty) as qty, sum(c.replace_qty) as replace_qty, e.po_number,c.barcode_no, c.delivery_mst_id  
                        from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e,wo_po_details_master f, ppl_cut_lay_mst g, ppl_cut_lay_dtls h
                        where d.job_id=f.id and e.job_id=f.id and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and g.job_no = f.job_no and g.id = h.mst_id and a.company_id=$company $orderCon $bndlCon $year_cond  $jobCon $cutCon $internal_ref_con $order_cut_no_con  and a.status_active=1 and a.is_deleted=0 AND g.status_active = 1 AND g.is_deleted = 0 AND h.status_active = 1 AND h.is_deleted = 0 AND c.production_type = $qty_source  and c.barcode_no is not null
                        group by c.cut_no, c.bundle_no, d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.bundle_no, e.po_number,c.barcode_no, c.delivery_mst_id 
                        order by  c.cut_no,length(c.bundle_no) asc, c.bundle_no asc";
                    }
               /* }
            ...
            {
                die;
                $sql="SELECT c.cut_no,d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.bundle_no, sum(c.production_qnty) as qty, e.po_number,c.barcode_no from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e,wo_po_details_master f where d.job_id=f.id and f.id=e.job_id and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and a.company_id=$company and e.po_number like '$txt_order_no' and c.bundle_no like '$bundle_no' and  c.production_type=1 $jobCon $cutCon  and a.status_active=1 and a.is_deleted=0 group by c.cut_no, c.bundle_no, d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.bundle_no, e.po_number,c.barcode_no order by c.cut_no, length(c.bundle_no) asc, c.bundle_no asc";
...
if ($action == "save_update_delete") {
         ...
         if (str_replace("'", "", $delivery_basis) == 3) {
            ...
            foreach ($mstArr as $orderId => $orderData) {
                foreach ($orderData as $gmtsItemId => $gmtsItemIdData) {
                    foreach ($gmtsItemIdData as $countryId => $qty) {
                        ...
                        $data_array_mst .= "(" . $id . "," . $mst_id . ",'" . $cutArr[$orderId][$gmtsItemId][$countryId] . "'," . $cbo_company_name . "," . $garments_nature . ",'" . $challan_no . "'," . $orderId . ", " . $gmtsItemId . "," . $countryId . ", " . $cbo_source . "," . $cbo_emb_company . "," . $cbo_location . "," . $txt_issue_date . "," . $qty . ",4," . $sewing_production_variable . ",'" . $txt_remark . "'," . $cbo_floor . "," . $cbo_line_no . "," . $prod_reso_allocation . ",96," . $user_id . ",'" . $pc_date_time . "')";
          ...
          //  $dtls_id = return_next_id("id", "pro_garments_production_dtls", 1);
            ...
            foreach ($dtlsArr as $bundle_no => $qty) {
                ...
                $data_array_dtls .= "(" . $dtls_id . "," . $mst_id . "," . $gmtsMstId . ",4,'" . $dtlsArrColorSize[$bundle_no] . "','" . $qty . "','" . $cut_no . "','" . $bundle_no . "',96,'".$bundleBarcodeArr[$bundle_no]."','".$bundleRescanArr[$bundle_no]."','".$bundle_wise_type_array[$bundle_no]."')";
        ...
        if (str_replace("'", "", $delivery_basis) == 3)
         {
           ...
           // $dtls_id = return_next_id("id", "pro_garments_production_dtls", 1);
            ...
            $bundle="'".implode("','",$bundleCheckArr)."'";
            $receive_sql="select c.barcode_no,c.bundle_no from pro_garments_production_dtls c where  c.bundle_no  in ($bundle)  and c.production_type=4 and c.status_active=1 and c.is_deleted=0 and (c.is_rescan=0 or c.is_rescan is null) and delivery_mst_id<>$mst_id";
            $receive_result = sql_select($receive_sql);
            foreach ($receive_result as $row)
            {
                $duplicate_bundle[trim($row[csf('bundle_no')])]=trim($row[csf('bundle_no')]);
            ...
            $field_array_mst = "id, delivery_mst_id,cut_no, company_id, garments_nature, challan_no, po_break_down_id, item_number_id, country_id, production_source, serving_company, location, production_date, production_quantity, production_type, entry_break_down_type, remarks, floor_id,sewing_line,prod_reso_allo,entry_form, inserted_by, insert_date,updated_by,update_date";
            //echo "0**";
            $mstArr = array();
            $dtlsArr = array();
            $colorSizeArr = array();
    ...
    else if ($operation == 2) // Delete Here  ------------------------------------------------------
    {
        ...
        $txt_chal_no = explode("-", str_replace("'", "", $txt_challan_no));
        $challan_no = (int)$txt_chal_no[3];
        $is_output=sql_select("select bundle_no from pro_garments_production_dtls where status_active=1 and is_deleted=0 and production_type=5 and bundle_no in($bundle) order by bundle_no");
...
if($action=="emblishment_issue_print_3")
{
	...
	<div style="width:900px;">
			...
			$group_concat=" group_concat(b.cut_no) as cut_no" ;
	            ...
	            if($data[2]==3)
	            {
	                ...
	                else
	                {
	                    ...
	                    group by a.po_break_down_id,a.item_number_id,a.country_id,b.bundle_no,d.color_number_id,d.size_number_id, d.order_quantity
						...
						$size_condition = " and d.size_number_id in(";
	                    $sql_prev_group=" group by a.po_break_down_id, a.item_number_id, a.country_id, b.bundle_no, d.color_number_id, d.size_number_id, d.order_quantity order by d.size_number_id ";
			...
			foreach($po_size_qty_arr as $poId=>$poArr)
			{
                ...
                {				
    				foreach($colorArr as $sizeId=>$row)
    				{
    					...
    					$balanceQty = $row['order_quantity'] - ($row['qty'] + $prev_qty_arr[$poId][$colorId][$sizeId]['qty']);
                        // echo $row['order_quantity'] ."- (".$row['qty'] ."+". $prev_qty_arr[$poId][$sizeId]['qty'].")<br>";
    					...
    					if($z == 1)
    					{
                            ...
                            <tr bgcolor="<? echo $bgcolor; ?>">
                                ...
                                <td align="center"><? echo $row['order_quantity']; ?></td>
    					...
    					$totalQty += $row['qty'];
    					$totalOrderQty += $row['order_quantity'];
    					$totalBalanceQty += $balanceQty;
    					
    					$grandTotalOrderQty += $row['order_quantity'];
        ...
        <br>
		 <?
			echo signature_table(226, $data[0], "950px");
		 ?>
	</div>
</div>
<script type="text/javascript" src="../../js/jquery.js"></script>
	       ...
	       <tr>
	        ...
	        foreach($cut_nos_all as $cut_val)
	        {
	            ...
	            else
	            {
	                ...
	                $sqls="SELECT b.cut_no, b.production_qnty, a.country_id, d.color_number_id, d.size_number_id, e.style_ref_no, e.style_description, e.job_no, e.buyer_name,f.po_number, f.grouping,f.id as po_id, f.po_quantity,b.bundle_no from  pro_garments_production_mst a, pro_garments_production_dtls b,wo_po_color_size_breakdown d, wo_po_details_master e,wo_po_break_down f  where
...
if($action=="sewing_input_challan_print_10")// for FFL (Shafiq)
{
    ...
    {
        $sql="SELECT sum(a.production_qnty) as production_qnty,sum(a.reject_qty) as reject_qnty,a.bundle_no,a.id,b.size_number_id,c.country_id
        from pro_garments_production_dtls a, wo_po_color_size_breakdown b,pro_garments_production_mst c where c.delivery_mst_id ='$data[1]' and c.id=a.mst_id
        and a.color_size_break_down_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.bundle_no is not null
        group by a.bundle_no,b.size_number_id,a.id,c.country_id order by a.id, b.size_number_id asc";
    }
    $sql_res = sql_select($sql);
    $bundleNoArr = "";
    $bundleNo = "";
        ...
        foreach ($size_qty_arr as $bundle_no => $bundle_data)
        {
            foreach ($bundle_data as $size_id => $size_qty)
            {
                ...
                $column_data .='<tr bgcolor='.$bgcolor.'>
                                        ...
                                        <td width="12%" class="inner" align="right">'.($rej_qty_arr[$bundle_no][$size_id]['cut_reject_qnty']+$rej_qty_arr[$bundle_no][$size_id]['emb_reject_qnty']+$cutting_rej_array[$bundle_no][$size_id]).'</td>
                                        <td width="12%" class="outer" align="right">'.($size_qty-($rej_qty_arr[$bundle_no][$size_id]['cut_reject_qnty']+$rej_qty_arr[$bundle_no][$size_id]['emb_reject_qnty']+$cutting_rej_array[$bundle_no][$size_id])).'</td>
                                </tr>';
                                $current_col_count_2++;
                                if ($current_col == $num_item -1)  // Close the row if $current_col equals to 2 in the ($num_cols -1)
                                {
