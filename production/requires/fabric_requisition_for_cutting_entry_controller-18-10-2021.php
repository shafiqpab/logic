...
{
	$sql_fabric_requisition_cutting_variable=sql_select("select id,finish_fabric_requisition_cutting from variable_settings_production where company_name=$data and variable_list=1 and status_active=1");

 	echo $sql_fabric_requisition_cutting_variable[0][csf("finish_fabric_requisition_cutting")];
	//if(count($sql_variable_inv_wvn_style_wise)>0){echo 1;}else{echo 0;}
	die;
}
...
{
	echo "$('#finish_fabric_requisition_cutting_variable').val(0);\n";
	$sql_result = sql_select("select id,finish_fabric_requisition_cutting from variable_settings_production where company_name=$data and variable_list=1 and status_active=1");
 	foreach($sql_result as $result)
	{
		echo "$('#finish_fabric_requisition_cutting_variable').val('".$result[csf("finish_fabric_requisition_cutting")]."');\n";
	}
 	exit();
}
	...
	?> 
		<script>
			...
			function checkValidation(){
				...
				{
					show_list_view ( <? echo $company_id; ?>+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('cbo_year').value+'_'+document.getElementById('txt_job_no').value+'_'+document.getElementById('txt_order_no').value+'_'+document.getElementById('txt_internal_ref_no').value+'_'+document.getElementById('txt_style_ref').value+'_'+<? echo $hdn_variable_setting_status; ?>+'_'+<? echo $hdn_finish_fabric_requisition_cutting_variable; ?>, 'create_fabric_search_list_view', 'search_div', 'fabric_requisition_for_cutting_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1);reset_hide_field();');
				}
				else
				{
...
if($action=="create_fabric_search_list_view")
{
    ...
    $hdn_finish_fabric_requisition_cutting_variable=trim($data[8]);
    // echo $hdn_finish_fabric_requisition_cutting_variable;die;
	...
	if($hdn_finish_fabric_requisition_cutting_variable==2) // For color level
	{
		// echo "color level";
		// echo $hdn_variable_setting_status;
		...
		if($hdn_variable_setting_status==1)
		{
			...
			$sql = "SELECT a.buyer_name,a.job_no, a.job_no_prefix_num,a.style_ref_no, c.item_number_id as item_id, e.body_part_id, e.lib_yarn_count_deter_id, c.color_number_id as color_id,e.uom, e.avg_cons ,sum((d.cons/d.pcs)*c.plan_cut_qnty) as budget_qty, TO_CHAR(a.insert_date,'YYYY') as year ,LISTAGG(b.id,',') WITHIN GROUP (ORDER BY b.id) as po_id ,b.grouping, e.gsm_weight, d.dia_width 
			from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, wo_pre_cos_fab_co_avg_con_dtls d, wo_pre_cost_fabric_cost_dtls e
			where a.id=b.job_id and b.id=c.po_break_down_id and c.po_break_down_id=d.po_break_down_id and c.color_number_id=d.color_number_id and d.pre_cost_fabric_cost_dtls_id=e.id and c.job_no_mst=e.job_no and c.item_number_id=e.item_number_id and a.is_deleted=0 and a.status_active=1 and b.is_confirmed=1 and a.company_name=$company_id $buyer_id_cond $job_no_cond $year_cond $po_cond $int_ref_cond $batch_cond $style_ref_cond
			group by  a.buyer_name,a.job_no, a.job_no_prefix_num,a.style_ref_no, c.item_number_id, e.body_part_id, e.lib_yarn_count_deter_id, c.color_number_id,e.uom, e.avg_cons ,a.insert_date,b.grouping, e.gsm_weight, d.dia_width";
            // echo $sql;
		...
		$sqlPrev="SELECT id, buyer_id, po_id, job_no, item_id, body_part, determination_id, gsm, dia, color_id,reqn_qty from pro_fab_reqn_for_cutting_dtls where status_active=1 and is_deleted=0 $po_id_cond";
		// echo $sqlPrev;die();
		$resultPrev=sql_select($sqlPrev);
		$prevRcvQntyArr = array();
		foreach ($resultPrev as $val) 
		{
			$prevRcvQntyArr[$val[csf('buyer_id')]][$val[csf('po_id')]][$val[csf('job_no')]][$val[csf('item_id')]][$val[csf('body_part')]][$val[csf('determination_id')]][$val[csf('gsm')]][$val[csf('dia')]][$val[csf('color_id')]]+=$val[csf('reqn_qty')];
		}
		...
		$reqnData=sql_select("SELECT buyer_id, po_id, job_no, item_id, body_part, determination_id, gsm, dia, color_id, size_id, sum(reqn_qty) as qnty from pro_fab_reqn_for_cutting_dtls where status_active=1 and is_deleted=0 $po_id_cond group by buyer_id, po_id, job_no, item_id, body_part, determination_id, gsm, dia, color_id");
		foreach($reqnData as $row)
		{
			$reqn_qnty_array[$row[csf('buyer_id')]][$row[csf('po_id')]][$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part')]][$row[csf('determination_id')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('color_id')]]=$row[csf('qnty')];
		}
		...
		?>
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1210" class="rpt_table">
			<thead>
				...
				<?
				if($hdn_finish_fabric_requisition_cutting_variable!=2) 
				{
					?>
		...
		<div style="width:1280px; max-height:250px; overflow-y:scroll" id="list_container_batch" align="left">	 
			...
			<?
				...
				{  
					if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";				
					$reqnQty=$reqn_qnty_array[$row[csf('buyer_name')]][$row[csf('po_id')]][$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('color_id')]];
					$prevRcvQnty=$prevRcvQntyArr[$row[csf('buyer_name')]][$row[csf('po_id')]][$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('body_part_id')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('color_id')]];
					...
					// echo $budget_qty_array[$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('color_id')]]['plan_cut_qnty']."__".$pre_cost_array[$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('color_id')]]['cons_pcs'].$row[csf('style_ref_no')]."<br>";
				...
				?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i; ?>" onClick="js_set_value(<? echo $i; ?>,<? echo $row[csf('buyer_name')];?>,<? echo $budget_qnty;?>)"> 
						...
						<td style="word-wrap: break-word;word-break: break-all;" width="75" align="right"><p>
							<? 
							// $budget_qnty = $budget_qty_array[$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('color_id')]]['plan_cut_qnty']*$pre_cost_array[$row[csf('job_no')]][$row[csf('item_id')]][$row[csf('color_id')]]['cons_pcs'];
...
if ($action=="save_update_delete")
{
	...
	{ 	
		...
		$field_array="id,reqn_number_prefix,reqn_number_prefix_num,reqn_number,company_id,lay_plan_id,reqn_date,inserted_by,entry_break_down_type,insert_date";
		$data_array="(".$id.",'".$new_mrr_number[1]."',".$new_mrr_number[2].",'".$new_mrr_number[0]."',".$cbo_company_id.",".$layPlan_id.",".$txt_requisition_date.",".$_SESSION['logic_erp']['user_id'].",".$finish_fabric_requisition_cutting_variable.",'".$pc_date_time."')";
		...
		// $hdn_finish_fabric_requisition_cutting_variable=str_replace("'","",$hdn_finish_fabric_requisition_cutting_variable);
		$hdn_finish_fabric_requisition_cutting_variable=2;
		// echo $hdn_finish_fabric_requisition_cutting_variable; die;
		if($hdn_finish_fabric_requisition_cutting_variable==2) //color level
		{
			echo "ddd";
			if($hdn_variable_setting_status==1)
			{
				...
				{ 
					...
					foreach ($hdnPoRatioQtyArr as $poData) 
					{
						...
						$data_array_dtls.="(".$dtls_id.",".$id.",".$$buyerId.",'".$hdnPoAndQtyArr[0]."','".$$jobNo."','".$$itemId."','".$$bodyPartId."','".$$deterId."','".$$gsm."','".$$dia."','".$$colorId."','".$$sizeId."','".$hdnPoAndQtyArr[1]."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
						$dtls_id = $dtls_id+1;
	...
	else if ($operation==1)   // Update Here
	{
		...
		$field_array="company_id*lay_plan_id*reqn_date*updated_by*entry_break_down_type*update_date";
		$data_array=$cbo_company_id."*".$layPlan_id."*".$txt_requisition_date."*".$_SESSION['logic_erp']['user_id']."*".$finish_fabric_requisition_cutting_variable."*'".$pc_date_time."'";
			...
			<fieldset style="width:760px; margin-left:2px">
	            <table cellpadding="0" cellspacing="0" width="750" border="1" rules="all" class="rpt_table">
	                ...
	                <tr class="general">
	            		...
	            		<td align="center">
	                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_reqn_no').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $company_id; ?>+'_'+document.getElementById('cbo_year_selection').value+'_'+<? echo ($buyerId !="") ? $buyerId : 0; ?>, 'create_reqn_search_list_view', 'search_div', 'fabric_requisition_for_cutting_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
	                     </td>
	                </tr>
	                <tr>
...
{
	$data_array=sql_select("select id, reqn_number, company_id, lay_plan_id, reqn_date,entry_break_down_type from pro_fab_reqn_for_cutting_mst where id='$data'");
	foreach ($data_array as $row)
	{ 
...
{	
	...
	if($data[1]==1)
	{
		$sql="SELECT  LISTAGG(id,',') WITHIN GROUP (ORDER BY id) as id, buyer_id, LISTAGG(po_id,',') WITHIN GROUP (ORDER BY po_id) as po_id , job_no, item_id, body_part, determination_id, gsm, dia, color_id, size_id, sum(reqn_qty) as  reqn_qty from pro_fab_reqn_for_cutting_dtls where status_active=1 and is_deleted=0 and mst_id=$data[0] group by buyer_id, job_no, item_id, body_part, determination_id, gsm, dia, color_id, size_id ";
	}
	else
	{
	...
	if($data[1]==1)
	{
		$sqlPrev="SELECT  buyer_id, LISTAGG(po_id,',') WITHIN GROUP (ORDER BY po_id) as po_id, job_no, item_id, body_part, determination_id, gsm, dia, color_id, size_id, sum(reqn_qty) as reqn_qty from pro_fab_reqn_for_cutting_dtls where status_active=1 and is_deleted=0 and po_id in($all_po_id) and mst_id not in($data[0]) group by buyer_id,job_no, item_id, body_part, determination_id, gsm, dia, color_id, size_id";
	}
	else
	{
	...
	foreach($result as $row)
	{
		...
		<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>"> 
            ...
            <td width="100"><p><? echo $po_data_array[$row[csf('po_id')]]['prefix']; ?></p></td>
            ...
            <td width="60" style="word-break:break-all;" id="fabricRef<? echo $i; ?>"><? //echo $row[csf('gsm')]; ?></td>
            <td width="60" style="word-break:break-all;" id="rdNo<? echo $i; ?>"><? //echo $row[csf('gsm')]; ?></td>
            <td width="60" style="word-break:break-all;" id="gsm<? echo $i; ?>"><? echo $row[csf('gsm')]; ?></td>
            <td width="60" style="word-break:break-all;" id="weightType<? echo $i; ?>"><? //echo $row[csf('gsm')]; ?></td>
            <td width="60" style="word-break:break-all;" id="dia<? echo $i; ?>"><? echo $row[csf('dia')]; ?></td>
            <td width="60" style="word-break:break-all;" id="cutableWidth<? echo $i; ?>"><? //echo $row[csf('gsm')]; ?></td>
