...
if($action=="create_po_search_list_view")
{
	...
	$sql_ex_fact = "SELECT a.po_break_down_id as po_id, sum(b.production_qnty) as ex_fact_qty from pro_ex_factory_mst a, pro_ex_factory_dtls b where a.id=b.mst_id and a.status_active=1 and b.status_active=1 group by a.po_break_down_id";
	...
	$sql_delivery = "SELECT b.from_po_id as po_id,sum(b.production_quantity) as delivery_qty from pro_gmts_delivery_mst a, pro_gmts_delivery_dtls b where a.id=b.mst_id and b.production_type=10 and a.status_active=1 and b.status_active=1 group by b.from_po_id";
	...
	$sql_po_actv = "SELECT d.po_break_down_id as po_id,sum(e.production_qnty) as sewing_out_qty from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, pro_garments_production_mst d, pro_garments_production_dtls e where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id and d.id=e.mst_id and b.id=d.po_break_down_id  and e.production_type=5 and d.production_type=e.production_type and a.status_active=1 and b.status_active=1 and d.status_active=1 and e.status_active=1 and c.status_active=1 and b.shiping_status=3 and c.id=e.color_size_break_down_id $sql_cond group by d.po_break_down_id";
	// echo $sql_po_actv;
	...
	foreach ($sql_po_actv_res as $val) 
	{
		...
		$sew_balance = $val[csf('sewing_out_qty')] - $exFactory_qty;
		if($sew_balance > 0)
		{
	...
	$sql_sewing_out = "SELECT d.po_break_down_id as po_id,sum(e.production_qnty) as sewing_out_qty from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, pro_garments_production_mst d, pro_garments_production_dtls e where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id and d.id=e.mst_id and b.id=d.po_break_down_id  and e.production_type=5 and d.production_type=e.production_type and a.status_active=1 and b.status_active=3 and d.status_active=1 and e.status_active=1 and c.status_active in(1,3) and b.shiping_status=1 $sql_cond group by d.po_break_down_id";
	// echo $sql_sewing_out;
	...
	if($order_type==1) // Self Order Type
	{
		...
		{
			$sql= "SELECT a.job_no, a.style_ref_no, a.buyer_name, a.gmts_item_id, a.location_name, a.currency_id, a.company_name, b.po_number, b.id, b.pub_shipment_date, b.shipment_date, b.unit_price as order_rate,c.item_number_id,c.country_id
            FROM wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c
            WHERE a.job_no=b.job_no_mst AND b.id=c.po_break_down_id AND a.status_active=1 AND a.is_deleted=0 AND b.status_active in(1,3) AND b.is_deleted=0 AND c.status_active in(1,3) AND c.is_deleted=0  AND b.shiping_status in(1,3) $sql_cond $po_cond
            GROUP BY a.job_no, a.style_ref_no, a.buyer_name, a.gmts_item_id, a.location_name, a.currency_id, a.company_name, b.po_number, b.id, b.pub_shipment_date, b.shipment_date,b.unit_price, c.item_number_id,c.country_id
	...
	$sql_sewing_out = "SELECT a.po_break_down_id as po_id,a.item_number_id as item_id,a.country_id, sum(b.production_qnty) as sewing_out_qty from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id and a.po_break_down_id in($left_over_po_id) and a.item_number_id in($left_over_item_id) and a.company_id=$company and a.location=$working_location and a.serving_company=$working_company and b.production_type=5 and a.production_type=b.production_type and a.status_active=1 and b.status_active=1 group by a.po_break_down_id,a.item_number_id,a.country_id";
	// echo $sql_sewing_out;
	$sql_sewing_out_res = sql_select($sql_sewing_out);
	...
	// ============================= GETTING EX-FACTORY QTY ============================
	$sql_ex_fact = "SELECT a.po_break_down_id as po_id,a.item_number_id as item_id,a.country_id, sum(b.production_qnty) as ex_fact_qty from pro_ex_factory_mst a, pro_ex_factory_dtls b, pro_ex_factory_delivery_mst c where a.id=b.mst_id and c.id=a.delivery_mst_id and a.po_break_down_id in($left_over_po_id) and a.item_number_id in($left_over_item_id) and c.company_id=$company and c.location_id=$location and c.delivery_company_id=$working_company and c.delivery_location_id=$working_location and a.status_active=1 and b.status_active=1 and c.status_active=1 group by a.po_break_down_id,a.item_number_id,a.country_id";
	...
	// echo $sewing_out_qty - ($leftover_qty + $delivery_qty + $ex_fact_qty);
            ...
            <style type="text/css">
             ...
             <div style="width:1020px;max-height:240px;overflow-y:scroll;" >	 
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1000" class="rpt_table" id="tbl_po_list">
                	<tbody>
                <?
				$i=1;
				foreach( $result as $row )
				{
					...
					$balance 		= $sewing_out_qty - ($leftover_qty + $delivery_qty + $ex_fact_qty);
					// echo $sewing_out_qty ."- (".$leftover_qty ."+". $delivery_qty ."+". $ex_fact_qty.")";
                ?>
                    ...
                    <tr bgcolor="<?php echo $bgcolor; ?>" style="cursor:pointer" onClick="js_set_value('<? echo $row[csf('id')]; ?>',
                        ...
                        <td width="100"><? echo $country_library[$row[csf('country_id')]]; ?></td>
	...
	else // Sub contruct order
	{
		...
		if( $goods_type ==1 || $goods_type == 2 )	//Goods Type [Good GMT In Hand , Damage GMT]
		{
			...
			//and a.id=d.order_id and d.production_type=2 , d.order_id
			//and a.id='$po_id' and c.item_id='$item_id'
		}
...
if($action=="get_left_over_balance")
{
	...
	// ============================= GETTING SEWING OUTPUT QTY ============================
	$sql_sewing_out = "SELECT sum(b.production_qnty) as sewing_out_qty from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id and a.po_break_down_id=$po_id and a.country_id=$country_id and a.item_number_id=$gmts_item and b.production_type=5 and a.production_type=b.production_type and a.status_active=1 and b.status_active=1 and a.company_id=$company and a.location=$wo_location and a.serving_company=$wo_company";
	$sql_sewing_out_res = sql_select($sql_sewing_out);
	$sewing_out_qty = 0;
	foreach ($sql_sewing_out_res as $val) 
	{
	...
	$sql_leftover = "SELECT sum(b.total_left_over_receive) as leftover_qty from pro_leftover_gmts_rcv_mst a, pro_leftover_gmts_rcv_dtls b where a.id=b.mst_id and b.po_break_down_id=$po_id and b.country_id=$country_id and b.item_number_id=$gmts_item and a.status_active=1 and b.status_active=1 and a.company_id=$company and a.location=$location";
	$sql_leftover_res = sql_select($sql_leftover);
	$leftover_qty = 0;
	foreach ($sql_leftover_res as $val) 
	{
	...
	// ============================= GETTING DELIVERY QTY ============================
	$sql_delivery = "SELECT sum(b.production_quantity) as delivery_qty from pro_gmts_delivery_mst a, pro_gmts_delivery_dtls b where a.id=b.mst_id and b.from_po_id=$po_id and b.country_id=$country_id and b.item_number_id=$gmts_item and b.production_type=10 and a.status_active=1 and b.status_active=1";
	$sql_delivery_res = sql_select($sql_delivery);
	$delivery_qty = 0;
	foreach ($sql_delivery_res as $val) 
	{
	...
	// ============================= GETTING EX-FACTORY QTY ============================
	$sql_ex_fact = "SELECT sum(b.production_qnty) as ex_fact_qty from pro_ex_factory_mst a, pro_ex_factory_dtls b, pro_ex_factory_delivery_mst c where a.id=b.mst_id and c.id=a.delivery_mst_id and a.po_break_down_id=$po_id and a.country_id=$country_id and a.item_number_id=$gmts_item and a.status_active=1 and b.status_active=1 and c.status_active=1 and c.company_id=$company and c.location_id=$location and c.delivery_company_id=$wo_company and c.delivery_location_id=$wo_location";
	$sql_ex_fact_res = sql_select($sql_ex_fact);
	$ex_fact_qty = 0;
	foreach ($sql_ex_fact_res as $val) 
	{
...
if($action=="show_dtls_listview_system") 
{
	...
	<div style="width:100%;max-height:180px; overflow:y-scroll" id="iron_list_view" align="left">
		...
		<?php  
			...
			if($db_type==0)
			{
				$sql ="SELECT b.id, b.mst_id, b.po_break_down_id, b.order_no, b.production_type, b.style_ref_no, b.item_number_id, b.country_id, b.total_left_over_receive,c.PRODUCTION_QNTY, b.currency_id, b.fob_rate, b.leftover_amount, b.bdt_amount, b.room_no, b.rack_no, b.shelf_no, b.bin_no, b.sewing_production_variable, b.color_size_id, b.style_order_wisw, b.variable_is_controll, b.user_lebel, b.remarks 
				from pro_leftover_gmts_rcv_dtls b,   pro_leftover_gmts_rcv_clr_sz c,wo_po_color_size_breakdown d  
				where b.status_active=1 and b.is_deleted=0 $po_cond and b.id=c.dtls_id  and  c.color_size_break_down_id = d.id
				order by b.id";
			}
		    if($db_type==2)
			{
				...
				where a.id=b.mst_id and b.id=c.dtls_id  and  c.color_size_break_down_id = d.id and b.status_active=1 and b.is_deleted=0 $po_cond
				order by b.id";
			}
			//echo $sql;
			$sqlResult =sql_select($sql);
 		...
 		?>
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="populate_selected_data(<? echo $selectResult[csf('id')]; ?>,<? echo $selectResult[csf('mst_id')]; ?>,<? echo $selectResult[csf('po_break_down_id')]; ?>,<? echo $selectResult[csf('country_id')]; ?>,<? echo $selectResult[csf('item_number_id')]; ?>,<? echo $selectResult[csf('sewing_production_variable')]; ?>);" > 
                ...
                <td width="80" align="center"><p><? echo $country_library[$selectResult[csf('country_id')]]; ?></p></td>
                <td width="80" align="center"><p><? echo $goods_type_arr[$selectResult[csf('goods_type')]]; ?></p></td>
                <td width="80" align="center"><p><? echo $selectResult[csf('PRODUCTION_QNTY')];//$selectResult[csf('total_left_over_receive')]; ?></p></td>
			</tr>
			<?php
			$i++;
			}
...
{	
	...
	// ============================= GETTING DELIVERY QTY ============================
	$sql_delivery = "SELECT b.from_po_id as po_id,b.country_id,b.item_number_id as item_id,sum(b.production_quantity) as delivery_qty from pro_gmts_delivery_mst a, pro_gmts_delivery_dtls b where a.id=b.mst_id and b.from_po_id=$po_id and a.company_id=$company_name and a.location_id=$location_name and a.working_company_id=$working_company_name and a.working_location_id=$working_location_name and b.production_type=10 and a.status_active=1 and b.status_active=1 group by b.from_po_id,b.country_id,b.item_number_id";
	$sql_delivery_res = sql_select($sql_delivery);
	...
	// ============================= GETTING EX-FACTORY QTY ============================
	$sql_ex_fact = "SELECT a.po_break_down_id as po_id,a.item_number_id as item_id, a.country_id,sum(b.production_qnty) as ex_fact_qty from pro_ex_factory_mst a, pro_ex_factory_dtls b, pro_ex_factory_delivery_mst c where a.id=b.mst_id and c.id=a.delivery_mst_id and a.po_break_down_id=$po_id and c.company_id=$company_name and c.delivery_company_id=$working_company_name and c.delivery_location_id=$working_location_name and a.status_active=1 and b.status_active=1 and c.status_active=1 group by a.po_break_down_id,a.item_number_id, a.country_id";
	$sql_ex_fact_res = sql_select($sql_ex_fact);
	$ex_fact_qty_array = array();
	foreach ($sql_ex_fact_res as $val) 
	{
	...
	// ============================= GETTING SEWING OUTPUT QTY ============================
	$sql_sewing_out = "SELECT a.po_break_down_id as po_id,a.item_number_id as item_id, a.country_id, sum(b.production_qnty) as sewing_out_qty from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id and a.po_break_down_id=$po_id and b.production_type=5 and a.company_id=$company_name and a.location=$working_location_name and a.serving_company=$working_company_name and a.production_type=b.production_type and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.po_break_down_id,a.item_number_id, a.country_id";
	// echo $sql_sewing_out;
	$sql_sewing_out_res = sql_select($sql_sewing_out);
	...
	// =================================== main query ============================
	$sql = "SELECT po_break_down_id,item_number_id,max(country_ship_date) as sdate,country_id from wo_po_color_size_breakdown where status_active in(1,2,3) and is_deleted=0 and po_break_down_id=$po_id group by po_break_down_id,item_number_id,country_id";
	$sqlResult = sql_select($sql);
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="380" class="rpt_table">
		...
		foreach($sqlResult as $row)
		{
			...
			$lftOvr_qty 	= $sewing_out_qty - ($exfact_out_qty+$delv_out_qty);//$cumLft_qty+
...
if($action=="populate_mst_form_data")
{
	...
	echo"load_drop_down( 'requires/left_over_garments_receive_controller', ".$result[0][csf('working_location_id')].", 'load_drop_down_working_floor', 'working_floor' );\n";
...
if($action=="color_and_size_level_left_over")
{
	...
	if($order_type==1) // Self Order
	{
		...
		from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c
		where a.job_no=b.job_no_mst and b.id=c.po_break_down_id and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,3) and b.is_deleted=0 and c.status_active in(1,3) and c.is_deleted=0   and b.shiping_status in(1,3) and b.id=$po_id and a.company_name=$company_id and a.location_name=$location and c.item_number_id='$item_id' $country_cond";//and a.garments_nature=$garments_nature
	...
	if($goodsType==1 || $goodsType==2)	//Good GMT In Hand - Damage GMT
	{
		...
		else if( $variableSettings==3 ) //color and size level
		{
			if($order_type==1) // Self Order
			{
				...
				$countryCond = str_replace("c.country_id", "country_id", $country_cond);
				$sql = "SELECT id,size_order, item_number_id, size_number_id, color_number_id, order_quantity, plan_cut_qnty from wo_po_color_size_breakdown where po_break_down_id='$po_id' and item_number_id='$item_id' $countryCond and is_deleted=0 and status_active in(1,3) order by color_number_id,size_order, id"; 
			}
			else // Subcontruct Order
			{
		...
		foreach($colorResult as $color)
		{
			...
			else //color and size level
			{
				if($order_type==1) // Self Order
				{
					...
					$exfac_qnty=$ex_fac_value[$color[csf('id')]];
				...
				else // subcontruct Order
				{
					...
					$colorHTML .='<tr><td>'.$size_library[$break_size_library[$color[csf("ord_color_size_id")]]].'</td><td><input type="text" name="colorSize" id="colSize_'.$color[csf("color_id")].($j+1).'" class="text_boxes_numeric"  value=""  onblur="fn_total('.$color[csf("color_id")].','.($j+1).' )"style="text-align:right;width:100px;" /></td></tr>';//placeholder="'.$left_over_qty.'"
				}
			}
			$j++;
			$i++; 
...
if($action=="color_and_size_level_left_over_update")
{
	...
	$size_library=return_library_array( "select id, size_name from lib_size",'id','size_name');
		...
		{ 
			if($order_type==1) // Self Order
			{
				$preProdData = sql_select("SELECT  c.color_size_break_down_id, d.color_number_id, sum(case when c.production_type=1 then c.production_qnty else 0 end) as production_qnty from pro_leftover_gmts_rcv_dtls b, pro_leftover_gmts_rcv_clr_sz c, wo_po_color_size_breakdown d where b.id=c.dtls_id  and d.id=c.color_size_break_down_id and c.color_size_break_down_id!=0 and c.production_type=1 and d.item_number_id='$item_id' and d.country_id=$country_id group by c.color_size_break_down_id, d.color_number_id");
				foreach($preProdData as $row)
				{				  
					$color_size_qnty_array[$row[csf('color_size_break_down_id')]]['pre_rcv'] += $row[csf('production_qnty')];
		...
		if( $variableSettings==2 ) // color level
		{
			if($order_type==1) // Self Order
			{
				...
				$prodData = "SELECT c.color_number_id, sum(CASE WHEN a.production_type=4 then a.production_qnty ELSE 0 END) as production_qnty, sum(CASE WHEN a.production_type=5 then a.production_qnty ELSE 0 END) as cur_production_qnty , sum(CASE WHEN a.production_type=5 then a.reject_qty ELSE 0 END) as reject_qty from pro_garments_production_dtls a, pro_garments_production_mst b,wo_po_color_size_breakdown c where a.status_active=1 and a.mst_id=b.id and a.color_size_break_down_id=c.id and c.po_break_down_id=b.po_break_down_id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in(4,5) group by c.color_number_id";
				// echo $prodData;
				...
				$sql = "SELECT color_order, item_number_id, color_number_id, sum(order_quantity) as order_quantity, sum(plan_cut_qnty) as plan_cut_qnty from wo_po_color_size_breakdown where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' and is_deleted=0 and status_active in(1,3) group by color_order, item_number_id, color_number_id order by color_number_id,color_order"; 
			}
			else // Subcontruct Order
			{
		...
		else if( $variableSettings==3 ) //color and size level
		{
			if($order_type==1) // Self Order
			{
				...
				$sql = "SELECT id,size_order, item_number_id, size_number_id, color_number_id, order_quantity, plan_cut_qnty from wo_po_color_size_breakdown where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' and is_deleted=0 and status_active in(1,3) order by color_number_id,size_order,id"; }
			else	// Subcontruct Order
			{
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		{
			if(str_replace("'","",$txt_mst_id)==''){			
				$data_array_mst="(".$id.",'".$new_sys_number[1]."','".$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",".$cbo_location_name.",".$txt_leftover_date.",".$cbo_order_type.",".$cbo_buyer_name.",".$cbo_store_name.",".$exchange_rate.",".$cbo_goods_type.",".$cbo_working_company_name.",".$cbo_working_location_name.",".$txt_remark.",".$user_id.",'".$pc_date_time."')";
			
				$data_array_dtls="(".$dtls_id.",'".$id."',".$txt_po_id.",".$txt_order_no.",1,".$txt_style_name.",".$cbo_item_name.",".$cbo_country_name.",".$txt_total_left_over_receive.",".$cbo_currency.",".$txt_fob_rate.",".$txt_leftover_amount.",".$txt_bdt_amount.",".$cbo_room_no.",".$cbo_rack_no.",".$cbo_shelf_no.",".$cbo_bin_no.",".$sewing_production_variable.",".$styleOrOrderWisw.",".$variable_is_controll.",".$txt_user_lebel.",".$txt_remark2.",".$user_id.",'".$pc_date_time."',".$hidden_job_no.")";
			}else{
				$data_array_dtls="(".$dtls_id.",".$txt_mst_id.",".$txt_po_id.",".$txt_order_no.",1,".$txt_style_name.",".$cbo_item_name.",".$cbo_country_name.",".$txt_total_left_over_receive.",".$cbo_currency.",".$txt_fob_rate.",".$txt_leftover_amount.",".$txt_bdt_amount.",".$cbo_room_no.",".$cbo_rack_no.",".$cbo_shelf_no.",".$cbo_bin_no.",".$sewing_production_variable.",".$styleOrOrderWisw.",".$variable_is_controll.",".$txt_user_lebel.",".$txt_remark2.",".$user_id.",'".$pc_date_time."',".$hidden_job_no.")";
			}
		...
		if(str_replace("'","",$sewing_production_variable)==2)//color level wise
		{
			if(str_replace("'","",$cbo_order_type)==1)	//self order
			{
				...
				$color_sizeID_arr=sql_select( "select id,color_number_id from wo_po_color_size_breakdown where po_break_down_id=$txt_po_id  and country_id=$cbo_country_name $item_cond and status_active=1 and is_deleted=0 order by id" );
				$colSizeID_arr=array();
		...
		{	
			...
			if(str_replace("'","",$cbo_order_type)==1)	//self order
			{
				...
				$color_sizeID_arr=sql_select( "select id,size_number_id,color_number_id from wo_po_color_size_breakdown where po_break_down_id=$txt_po_id  and country_id=$cbo_country_name $item_cond and status_active=1 and is_deleted=0  order by size_number_id,color_number_id" );
				$colSizeID_arr=array(); 
				foreach($color_sizeID_arr as $val){
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		if($db_type==0)	{ mysql_query("BEGIN"); }
		 ...
		 $field_array_mst_update="working_company_id*working_location_id*remarks*updated_by*update_date"; 
		 $data_array_mst_update="".$cbo_working_company_name."*".$cbo_working_location_name."*".$txt_remark."*".$user_id."*'".$pc_date_time."'";
		
		
		
		
		 $field_array_dtls_update="total_left_over_receive*leftover_amount*bdt_amount*room_no*rack_no*shelf_no*bin_no*remarks*updated_by*update_date";
		 $data_array_dtls_update="".$txt_total_left_over_receive."*".$txt_leftover_amount."*".$txt_bdt_amount."*".$cbo_room_no."*".$cbo_rack_no."*".$cbo_shelf_no."*".$cbo_bin_no."*".$txt_remark2."*".$user_id."*'".$pc_date_time."'";
		...
		{		
			if(str_replace("'","",$cbo_order_type)==1)
			{
				...
				$color_sizeID_arr=sql_select( "select id,color_number_id from wo_po_color_size_breakdown where po_break_down_id=$txt_po_id  and country_id=$cbo_country_name $item_cond and status_active=1 and is_deleted=0  order by size_number_id,color_number_id" );
				$colSizeID_arr=array(); 
				foreach($color_sizeID_arr as $val){
					$colSizeID_arr[$val[csf("color_number_id")]]=$val[csf("id")];
				}
...
if($action=="left_over_gmts_receive_print")
{
	...
	<div style="width:1010px;">
	    <table width="1000" cellspacing="0" align="right" style="margin-bottom:20px;">
			...
			$sql_mst = "select a.id, a.sys_number, a.company_id, a.location, a.leftover_date, a.order_type, a.buyer_name, a.store_name, a.floor_id, a.exchange_rate, a.garments_nature, a.goods_type, a.remarks as mst_remarks
	        ...
	        <tr style="font-size:12px;">
	            ...
	            <td width="" valign=""><strong>: </strong><? echo $floor_name_library[$result_mst[0][csf("floor_id")]]; ?></td>
	    ...
	    <table align="right" cellspacing="0" width="1000"  border="1" rules="all" class="rpt_table" >
			...
			<?
			/*
			$sql_master ="select a.id, a.sys_number, a.company_id, a.location, a.leftover_date, a.order_type, a.buyer_name, a.store_name, a.floor_id, a.exchange_rate, a.garments_nature, a.goods_type, a.remarks as mst_remarks,
			b.id as dtls_id, b.po_break_down_id, b.order_no, b.production_type, b.style_ref_no, b.item_number_id, b.country_id, b.total_left_over_receive, b.currency_id, b.fob_rate, b.leftover_amount, b.bdt_amount, b.room_no, b.rack_no, b.shelf_no, b.bin_no, b.sewing_production_variable, b.color_size_id, b.style_order_wisw, b.variable_is_controll, b.remarks, c.color_size_break_down_id, c.production_qnty  
			from pro_leftover_gmts_rcv_mst a, pro_leftover_gmts_rcv_dtls b, pro_leftover_gmts_rcv_clr_sz c
			where a.id=b.mst_id and b.id = c.dtls_id and  a.company_id='$data[0]' and a.id=$data[1] and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 ";
			$result_master=sql_select($sql_master);
			$orderNoArr= array();
			foreach($result_master as $ids ){
				$orderNoArr[$ids[csf("po_break_down_id")]]=$ids[csf("po_break_down_id")];
			}
			
			$sql = "select id, mst_id, order_id, item_id, color_id, size_id, qnty, rate, amount, excess_cut, plan_cut, process_loss, gsm embellishment_type, description, dia_width_type, grey_dia, finish_dia, body_part from subcon_ord_breakdown  where order_id in(".implode(',',$orderNoArr).")";
			*/
