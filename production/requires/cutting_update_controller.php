...
{
	echo create_drop_down( "cbo_location", 210, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id='$data'  $location_credential_cond order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down( 'requires/cutting_update_controller', this.value, 'load_drop_down_floor', 'floor_td' );load_floor_select();",0 );
	exit();
}

if ($action=="load_drop_down_floor")
{
	echo create_drop_down( "cbo_floor", 210, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process=1 order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/cutting_update_controller', document.getElementById('cbo_cutting_company').value+'_'+document.getElementById('cbo_location').value+'_'+this.value, 'load_drop_down_table', 'table_td' );",0 );
	exit();
}
...
if($action=="load_drop_down_all")
{
	list($company_id, $location, $floor,$po_id,$source) = explode("_", $data);
	if($source==3)
	{
		if($db_type==0)
		{
	...
	else if($source==1)//$selected_company
	...
	echo create_drop_down( "cbo_location", 210, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id='$company_id'  $location_credential_cond order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down( 'requires/cutting_update_controller', this.value, 'load_drop_down_floor', 'floor_td' );",0 );

	echo "****";

	echo create_drop_down( "cbo_floor", 210, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$location' and production_process=1 order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/cutting_update_controller', document.getElementById('cbo_cutting_company').value+'_'+document.getElementById('cbo_location').value+'_'+this.value, 'load_drop_down_table', 'table_td' );",0 );

	echo "****";

	echo create_drop_down( 'cbo_table', 210, "select id, table_name from lib_table_entry where status_active=1 and is_deleted=0 and company_name=$company_id and location_name=$location and floor_name=$floor and table_type=1 order by table_name", "id,table_name", 1, "-- Select Table --", $selected, '', 0 );

	echo "****";

	$sql="SELECT b.color_type_id from  wo_po_break_down a,wo_pre_cost_fabric_cost_dtls b,wo_pre_cos_fab_co_avg_con_dtls c where a.job_id=b.job_id  and b.id=c.pre_cost_fabric_cost_dtls_id and a.id=c.po_break_down_id and b.job_id=c.job_id  and   a.status_active=1 and b.status_active=1 and a.is_deleted=0 and b.is_deleted=0 and a.id='$po_id' and c.cons>0  group by b.color_type_id";
	...
	{
		echo create_drop_down( "cbo_color_type", 122, $color_type_arr,"", 1, "Select Type", $selected,"");
	}
	else
	{
		echo create_drop_down( "cbo_color_type", 122, $color_type_arr,"", 1, "Select Type", $selected,"");
...
if($action=="order_popup")
{
	...
	function js_set_value(id,item_id,po_qnty,plan_qnty,country_id,job_num,company,ship_date)
	{
...
if($action=="create_po_search_list_view")
{
	...
	{
 		$sql = "SELECT b.id,a.order_uom,a.buyer_name,b.grouping,b.file_no,a.company_name,a.total_set_qnty,a.set_break_down, a.job_no,a.style_ref_no,a.gmts_item_id,a.location_name,b.shipment_date,b.po_number,sum(d.order_quantity) as po_quantity ,sum(d.plan_cut_qnty) as plan_cut,d.country_ship_date
			from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown d
			where
			a.id = b.job_id and
			...
			b.is_deleted=0 and			 
			a.garments_nature=$garments_nature
			$sql_cond $sql_shipment_year_cond $projected_po_cond group by b.id,a.order_uom,a.buyer_name,b.grouping,b.file_no,a.company_name,a.total_set_qnty,a.set_break_down, a.job_no,a.style_ref_no,a.gmts_item_id,a.location_name,b.shipment_date,b.po_number,d.country_ship_date order by d.country_ship_date"; // to_char(b.insert_date,'YYYY')='$year' and
	}
	// echo $sql;die;
	...
	$po_country_data_arr=array(); $pocountry_arr=array(); $po_color_arr=array();
	$poCountryData=sql_select( "SELECT po_break_down_id, item_number_id, country_id, sum(order_quantity) as qnty, sum(plan_cut_qnty) as plan_cut_qnty,color_number_id,country_ship_date from wo_po_color_size_breakdown where status_active=1 and is_deleted=0 $po_id_cond group by po_break_down_id, item_number_id, country_id,color_number_id,country_ship_date");

	foreach($poCountryData as $row)
	{
		$po_country_data_arr[$row[csf('po_break_down_id')]][$row[csf('item_number_id')]][$row[csf('country_id')]][$row[csf('country_ship_date')]]['po_qnty'] +=$row[csf('qnty')];
		$po_country_data_arr[$row[csf('po_break_down_id')]][$row[csf('item_number_id')]][$row[csf('country_id')]][$row[csf('country_ship_date')]]['plan_cut_qnty'] +=$row[csf('plan_cut_qnty')];
		$pocountry_arr[$row[csf('po_break_down_id')]].=$row[csf('country_id')].',';
		// $po_color_arr[$row[csf("po_break_down_id")]].=$row[csf("color_number_id")];
		$po_color_arr[$row[csf("po_break_down_id")]].=','.$row[csf("color_number_id")];
	}
	// print_r($po_color_arr);

	$total_cut_data_arr=array();
	$total_cut_qty_arr=sql_select("SELECT b.po_break_down_id, b.item_number_id, b.country_id,b.country_ship_date, sum(a.production_qnty) as production_quantity from pro_garments_production_dtls a,wo_po_color_size_breakdown b where a.color_size_break_down_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.production_type=1 $po_id_cond group by b.po_break_down_id, b.item_number_id, b.country_id,b.country_ship_date");

	foreach($total_cut_qty_arr as $row)
	{
		$total_cut_data_arr[$row[csf('po_break_down_id')]][$row[csf('item_number_id')]][$row[csf('country_id')]][$row[csf('country_ship_date')]] += $row[csf('production_quantity')];
	}
            ...
            foreach( $result as $row )
            {
				...
				for($k=0;$k<$numOfItem;$k++)
				{
					...
					foreach($country as $country_id)
					{
						...
						$po_qnty=$po_country_data_arr[$row[csf('id')]][$grmts_item][$country_id][$country_ship_date]['po_qnty'];
						$plan_cut_qnty=$po_country_data_arr[$row[csf('id')]][$grmts_item][$country_id][$country_ship_date]['plan_cut_qnty'];
						if($po_qnty>0)
						{
							?>
							<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value(<? echo $row[csf("id")];?>,'<? echo $grmts_item;?>','<? echo $po_qnty;?>','<? echo $plan_cut_qnty;?>','<? echo $country_id;?>','<? echo$job_num;?>','<? echo $company_id;?>','<? echo $country_ship_date;?>');" >
								<td width="30" align="center"><?php echo $i; ?></td>
								<td width="100" title="<?echo $color_name? $color_name:''?>" ><p><?php echo $row[csf("po_number")]; ?></p></td>
								...
								<td width="70" align="center"><?php echo change_date_format($row[csf("country_ship_date")]);?></td>
								<td width="80" align="right"><?php echo $plan_cut_qnty; //$po_qnty*$set_qty;?>&nbsp;</td>
								<td width="80" align="right"><?php echo $total_cut_qty=$total_cut_data_arr[$row[csf('id')]][$grmts_item][$country_id][$country_ship_date]; ?>&nbsp;</td>
...
if($action=="populate_data_from_search_popup")
{
 	...
 	foreach($res as $result)
	{
		...
		// $total_produced = return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=".$result[csf('id')]." and item_number_id='$item_id' and country_id='$country_id' $country_ship_date_cond and production_type=1 and is_deleted=0");
		$total_produced = return_field_value("sum(production_qnty)","pro_garments_production_dtls a,wo_po_color_size_breakdown b","a.color_size_break_down_id=b.id and po_break_down_id=".$result[csf('id')]." and item_number_id='$item_id' and country_id='$country_id' $country_ship_date_cond and production_type=1 and a.is_deleted=0 and b.is_deleted=0 and b.status_active=1");
		echo "$total_produced;\n";
...
if($action=="color_and_size_level")
{
		...
		$sqls_col_size="SELECT id,color_number_id,size_number_id, sum(plan_cut_qnty) as qnty , sum(order_quantity) as ord_qty from wo_po_color_size_breakdown c where po_break_down_id=$po_id $country_ship_date_cond and status_active=1 and is_deleted=0 group by id,color_number_id,size_number_id";
		...
		if( $variableSettings==2 ) // color level
		{
			...
			$sql = "SELECT a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty
				from wo_po_color_size_breakdown a where a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' $country_ship_date_cond2 and a.is_deleted=0 and a.status_active=1 group by a.item_number_id, a.color_number_id";
		...
		else if( $variableSettings==3 ) //color and size level
		{
			...
			$dtlsData = sql_select("SELECT a.color_size_break_down_id,
										sum(CASE WHEN a.production_type=1 then a.production_qnty ELSE 0 END) as production_qnty
										from pro_garments_production_dtls a,pro_garments_production_mst b,wo_po_color_size_breakdown c where  a.mst_id=b.id and a.color_size_break_down_id=c.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' $country_ship_date_cond and a.status_active=1 and a.color_size_break_down_id!=0 and a.production_type in(1) and c.status_active=1 group by a.color_size_break_down_id");
			...
			$sql = "SELECT id, item_number_id,size_order,color_order, size_number_id, color_number_id, order_quantity, plan_cut_qnty
			from wo_po_color_size_breakdown c
			where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' $country_ship_date_cond and is_deleted=0 and status_active=1 order by color_order,size_order"; //color_number_id, id
		...
		else // by default color and size level
		{
			...
			$sql = "SELECT id, item_number_id, size_number_id,color_order,size_order, color_number_id, order_quantity, plan_cut_qnty
			from wo_po_color_size_breakdown c
			where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' $country_ship_date_cond and is_deleted=0 and status_active=1 order by color_order,size_order";//color_number_id, id
		}
 		...
 		foreach($colorResult as $color)
		{
			...
			else //color and size level
			{
				if( !in_array( $color[csf("color_number_id")], $chkColor ) )
				{
					...
					$colorHTML .= '<div id="content_search_panel_'.$color[csf("color_number_id")].'" style="display:none" class="accord_close"><div style="padding-left: 30px;text-align:left"><input type="checkbox" onClick="active_placeholder_qty(' . $color[csf("color_number_id")] . ')" id="set_all_' . $color[csf("color_number_id")] . '">&nbsp;Available Qty Auto Fill</div><table id="table_'.$color[csf("color_number_id")].'">';
				...
				$color_size_breakdown_id = $color["ID"];
 				...
 				$colorHTML .='<tr><td>'.$size_library[$color[csf("size_number_id")]].'</td><td><input type="hidden" name="bundlemst" id="bundle_mst_'.$color[csf("color_number_id")].($i+1).'" value="'.$bundle_mst_data.'"  class="text_boxes_numeric" style="width:100px"  ><input type="hidden" name="bundledtls" id="bundle_dtls_'.$color[csf("color_number_id")].($i+1).'"  class="text_boxes_numeric" style="width:100px" value="'.$bundle_dtls_data.'" ><input type="text" name="colorSize" id="colSize_'.$color[csf("color_number_id")].($i+1).'" data-colorSizeBreakdown="'.$color_size_breakdown_id.'" class="text_boxes_numeric" style="width:50px" placeholder="'.($color[csf("plan_cut_qnty")]-$cut_qnty).'" onkeyup="fn_total('.$color[csf("color_number_id")].','.($i+1).')"><input type="text" name="colorSizeRej" id="colSizeRej_'.$color[csf("color_number_id")].($i+1).'"  class="text_boxes_numeric" data-colorSizeBreakdown="'.$color_size_breakdown_id.'"  style="width:50px" placeholder="Rej. Qty" onkeyup="fn_total_rej('.$color[csf("color_number_id")].','.($i+1).')" '.$disable.'></td><td><input title ="Order Qty" type="text" name="bundlemst" id="bundle_mst_'.$color[csf("color_number_id")].($i+1).'" value="'.$po_color_size_order_qnty_arr[$color[csf('id')]].'"  class="text_boxes_numeric"  disabled="" readonly="" style="width:60px"></td><td><input title ="Plan Cut Qty" type="text" name="button" id="button_'.$color[csf("color_number_id")].($i+1).'" value="'.$po_color_size_qnty_arr[$color[csf('id')]].'" class="text_boxes_numeric" disabled="" readonly=""  style="width: 60px;;"  /></td></tr>';
			}
		...
		<div style="width:100%;">
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1120" class="rpt_table" align="left">
	            <thead>
	                ...
	                <th width="" align="center">Cutting No</th>
		...
		<div style="width:1140px;max-height:250px; overflow-y:auto; overflow-x: auto;" id="sewing_production_list_view" align="left">
			...
			?>
				...
				<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"  >
	                ...
	                <td width="" onClick="<? echo $fnc_var;?>" align="right"><?php  echo $selectResult[csf('cut_no')]; ?></td>
...
if($action=="show_country_listview")
{
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="390" class="rpt_table">
		...
		<?
		$i=1;
		$cutting_qnty_arr=sql_select("SELECT a.po_break_down_id, a.item_number_id, a.country_id, b.production_qnty as cutting_qnty,c.country_ship_date from pro_garments_production_mst a, pro_garments_production_dtls b,wo_po_color_size_breakdown c where a.id=b.mst_id and b.COLOR_SIZE_BREAK_DOWN_ID=c.id and a.po_break_down_id='$data' and a.production_type=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1");
		$cutting_data_arr=array();
		foreach($cutting_qnty_arr as $row)
		{
			$cutting_data_arr[$row[csf("po_break_down_id")]][$row[csf("country_id")]][$row[csf("item_number_id")]][$row[csf("country_ship_date")]] += $row[csf("cutting_qnty")];
		}
		$sqlResult =sql_select("SELECT po_break_down_id, item_number_id, country_id, country_ship_date as country_ship_date, sum(order_quantity) as order_qnty, sum(plan_cut_qnty) as plan_cut_qnty from wo_po_color_size_breakdown where po_break_down_id='$data' and status_active=1 and is_deleted=0 group by po_break_down_id, item_number_id,country_ship_date, country_id order by country_ship_date");
		foreach($sqlResult as $row)
		{
			if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
			$cutting_qnty=0;
			$cutting_qnty=$cutting_data_arr[$row[csf("po_break_down_id")]][$row[csf("country_id")]][$row[csf("item_number_id")]][$row[csf("country_ship_date")]];
		?>
...
if($action=="populate_cutting_form_data")
{
	...
	/* 
	@update cuntry ship date
	@cause : user change cuntry ship date from order entry after cutting
	@issue ID : 14142
	*/
	
	$sqlRes = sql_select("SELECT po_break_down_id,item_number_id,country_id from pro_garments_production_mst where id='$data[0]' and status_active=1 and is_deleted=0");
	...
	$update_shidate = execute_query("UPDATE pro_garments_production_mst set country_ship_date='$country_ship_date' WHERE id=$data[0]");
	...
	*/
    ...
    $sqls_col_size="SELECT id,color_number_id,size_number_id, sum(plan_cut_qnty) as qnty , sum(order_quantity) as ord_qty from wo_po_color_size_breakdown where po_break_down_id=$po_id $country_ship_date_cond and status_active=1 and is_deleted=0 group by id,color_number_id,size_number_id";
	foreach(sql_select($sqls_col_size) as $key=>$value)
	{
		// $po_color_size_qnty_arr[$value[csf("color_number_id")]][$value[csf("size_number_id")]] +=$value[csf("qnty")];
		// $po_color_size_order_qnty_arr[$value[csf("color_number_id")]][$value[csf("size_number_id")]] +=$value[csf("ord_qty")];
  	...
  	foreach($sqlResult as $result)
	{
		...
		echo "load_drop_down_multiple( 'requires/cutting_update_controller',".$result[csf("serving_company")]."+'_'+" . $result[csf('location')] . "+'_'+" . $result[csf('floor_id')] . "+'_'+'" . change_date_format($result[csf('po_break_down_id')]) . "'+'_'+'" . change_date_format($result[csf('production_source')]) . "', 'load_drop_down_all', 'cutt_company_td*location_td*floor_td*table_td*color_type_td' );\n";
		...
		$select_val = $color_type_val[0][csf("color_type_id")];
		$select_text = $color_type[$select_val];
		echo "$('#cbo_color_type').append($('<option>', {value: '".$select_val."', text: '".$select_text."' })).val('".$select_val."');\n";
		...
		echo "$('#save_data').val('".$result[csf('reject_qty_kg_break_down')]."');\n";
  		...
  		echo "$('#txt_country_ship_date').val('".$country_ship_date."');\n";

		$plan_cut_qnty=return_field_value("sum(plan_cut_qnty)","wo_po_color_size_breakdown","po_break_down_id=".$result[csf('po_break_down_id')]." and item_number_id=".$result[csf('item_number_id')]." and country_id=".$result[csf('country_id')]." $country_ship_date_cond and status_active=1 and is_deleted=0");

		$total_produced = return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=".$result[csf('po_break_down_id')]." and item_number_id=".$result[csf('item_number_id')]." and country_id=".$result[csf('country_id')]." $country_ship_date_cond and production_type=1 and is_deleted=0");
		...
		if( $variableSettings!=1 ) // gross level
		{
			...
			if( $variableSettings==2 ) // color level
			{
				if($db_type==0)
				{
					...
					from wo_po_color_size_breakdown
					where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' $country_ship_date_cond and is_deleted=0 group by color_number_id order by color_number_id,id";
				}
				else
				{
				...
				where a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' $country_ship_date_cond and a.is_deleted=0 and a.status_active=1 group by a.item_number_id, a.color_number_id order by  a.color_number_id";
			...
			else if( $variableSettings==3 ) //color and size level
			{
				/*$sql = "select id, item_number_id, size_number_id, color_number_id, order_quantity, plan_cut_qnty, (select sum(CASE WHEN pro_garments_production_dtls.color_size_break_down_id=wo_po_color_size_breakdown.id then production_qnty ELSE 0 END) from pro_garments_production_dtls where is_deleted=0 and  	production_type=1 ) as production_qnty
					...
					$dtlsData = sql_select("SELECT a.color_size_break_down_id,
										...
										from pro_garments_production_dtls a,pro_garments_production_mst b,wo_po_color_size_breakdown c where  a.mst_id=b.id and a.color_size_break_down_id=c.id  and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' $country_ship_date_cond2 and a.color_size_break_down_id <> 0 and a.status_active=1 and a.production_type in(1) group by a.color_size_break_down_id");
					...
					$sql = "SELECT id, item_number_id, size_number_id,color_order,size_order, color_number_id, order_quantity, plan_cut_qnty
					from wo_po_color_size_breakdown
					where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' $country_ship_date_cond and is_deleted=0 and status_active=1 order by color_order,size_order";
			...
			foreach($colorResult as $color)
			{
				...
				else //color and size level
				{
					...
					$color_size_breakdow_id = $color['ID'];
					$amount 	= $amountColorSizeArr[$color_size_breakdow_id];
					$rej_qnty 	= $rejectColorSizeArr[$color_size_breakdow_id];
					...
					$colorHTML .='<tr><td>'.$size_library[$color[csf("size_number_id")]].'</td><td><input type="hidden" name="bundlemst" id="bundle_mst_'.$color[csf("color_number_id")].($i+1).'" value="'.$bundle_mst_data.'"  class="text_boxes_numeric" style="width:100px"  ><input type="hidden" name="bundledtls" id="bundle_dtls_'.$color[csf("color_number_id")].($i+1).'"  class="text_boxes_numeric" style="width:100px" value="'.$bundle_dtls_data.'" ><input type="text" name="colorSize" data-colorSizeBreakdown="'.$color_size_breakdow_id.'" id="colSize_'.$color[csf("color_number_id")].($i+1).'"  class="text_boxes_numeric" style="width:50px" placeholder="'.($color[csf("plan_cut_qnty")]-$cut_qnty+$amount).'" onkeyup="fn_total('.$color[csf("color_number_id")].','.($i+1).');" onblur="fn_chk_next_process_qty('.$color[csf("color_number_id")].','.($i+1).','.$color[csf("size_number_id")].')" value="'.$amount.'" ><input type="hidden" name="colorSizeUpQty" id="colSizeUpQty_'.$color[csf("color_number_id")].($i+1).'" value="'.$amount.'" ><input type="text" name="colorSizeRej" data-colorSizeBreakdown="'.$color_size_breakdow_id.'"   id="colSizeRej_'.$color[csf("color_number_id")].($i+1).'"  class="text_boxes_numeric" style="width:50px" placeholder="Rej. Qty" onkeyup="fn_total_rej('.$color[csf("color_number_id")].','.($i+1).')" value="'.$rej_qnty.'" '.$disable.'></td><td><input title ="Order Qty" type="text" name="bundlemst" id="bundle_mst_'.$color[csf("color_number_id")].($i+1).'" value="'.$po_color_size_order_qnty_arr[$color[csf('id')]].'"  class="text_boxes_numeric"  disabled="" readonly="" style="width:60px"></td><td><input title ="Plan Cut Qty" type="text" name="button" value="'.$po_color_size_qnty_arr[$color[csf('id')]].'" readonly="" disabled="" class="text_boxes_numeric" style="width:60px;"  /></td></tr>';
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		{
			$ffSql = "SELECT a.id as issue_id, c.QUANTITY from inv_issue_master a, inv_transaction b,order_wise_pro_details c,WO_PO_BREAK_DOWN d where a.id=b.mst_id and b.id=c.trans_id and c.po_breakdown_id=d.id and c.entry_form in(18,19) and c.trans_type=2 and c.status_active=1 and c.po_breakdown_id=$hidden_po_break_down_id and a.issue_purpose in(9,4)";
			// echo "20**".$ffSql;die;
		...
		if(str_replace("'","",$txt_country_ship_date)=="") $ship_date_cond=""; else $ship_date_cond=" and a.country_ship_date=$txt_country_ship_date";
		...
		else
		{
			...
			$data_array2="INSERT INTO pro_garments_production_mst (".$field_array2.") VALUES(".$id.",".$garments_nature.",121,".$cbo_company_name.",".$txt_challan_no.",".$hidden_po_break_down_id.", ".$cbo_item_name.", ".$cbo_country_name.",".$txt_country_ship_date.", ".$cbo_source.",".$cbo_cutting_company.",".$cbo_location.",".$cbo_produced_type.",".$txt_cutting_date.",".$txt_cutting_qty.",1,".$sewing_production_variable.",".$cutting_production_variable_reject.",".$txt_reporting_hour.",".$txt_remark.",".$cbo_floor.",".$cbo_table.",".$txt_reject_qty.",".$txt_cutting_no.",".$txt_used_qty_kg.",".$txt_reject_qty_kg.",".$save_data.",".$txt_cumul_cutting.",".$txt_yet_cut.",".$cbo_work_order.",".$hidden_currency_id.",".$hidden_exchange_rate.",'".$rate."','".$amount."',".$user_id.",'".$pc_date_time."')";
		...
		{
			$color_sizeID_arr=sql_select( "select id,color_number_id from wo_po_color_size_breakdown a where po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name and country_id=$cbo_country_name $ship_date_cond and status_active=1 and is_deleted=0 order by id" );
			$colSizeID_arr=array();
		...
		if(str_replace("'","",$sewing_production_variable)==3)//color and size wise
		{
		   ...
		   $color_all=implode(",",$color_all_id);
			...
			/* $color_sizeID_arr=sql_select( "SELECT id,size_number_id,color_number_id from wo_po_color_size_breakdown a where po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name and country_id=$cbo_country_name and status_active=1 and is_deleted=0 and color_number_id in($color_all)  $ship_date_cond order by color_number_id,size_number_id" );
			$colSizeID_arr=array();
			foreach($color_sizeID_arr as $val){
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				$color_size_id = $colorAndSizeAndValue_arr[0]; //color_size_break_down_id
				$colorSizeValue = $colorAndSizeAndValue_arr[1];

 				//1 means cutting update
 				$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq", "pro_garments_production_dtls", $con );

				if($j==0)$data_array = "(".$dtls_id.",".$id.",1,'".$color_size_id."','".$colorSizeValue."','".$rejQtyArr[$color_size_id]."',".$cbo_color_type.")";
				else $data_array .= ",(".$dtls_id.",".$id.",1,'".$color_size_id."','".$colorSizeValue."','".$rejQtyArr[$color_size_id]."',".$cbo_color_type.")";
				//$dtls_id=$dtls_id+1;
 				$j++;
			}
  	...
  	else if ($operation==1) // Update Here------------------------------------------------------
	{
		...
		if(str_replace("'","",$txt_country_ship_date)=="") $ship_date_cond=""; else $ship_date_cond=" and country_ship_date=$txt_country_ship_date";
		if(str_replace("'","",$txt_country_ship_date)=="") $ship_date_cond2=""; else $ship_date_cond2=" and c.country_ship_date=$txt_country_ship_date";
		// pro_garments_production_mst table data entry here
		$field_array1="company_id*challan_no*production_source*serving_company*location*produced_by*production_date*production_quantity*production_type* entry_break_down_type*break_down_type_rej*production_hour*remarks*floor_id*table_no*reject_qnty*cut_no*used_qty_kg*reject_qty_kg*reject_qty_kg_break_down*total_produced*yet_to_produced*wo_order_id*currency_id*exchange_rate*rate*amount*country_ship_date*updated_by*update_date";
		...
		if(str_replace("'","",$sewing_production_variable)!=1 && str_replace("'","",$txt_mst_id)!='')// check is not gross level
		{
			...
			if(str_replace("'","",$sewing_production_variable)==2)//color level wise
			{
				$color_sizeID_arr=sql_select( "select id,color_number_id from wo_po_color_size_breakdown where po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name and country_id=$cbo_country_name $ship_date_cond  and status_active=1 and is_deleted=0  order by id" );
				$colSizeID_arr=array();
				foreach($color_sizeID_arr as $val){
					$index = $val[csf("color_number_id")];
					$colSizeID_arr[$index]=$val[csf("id")];
			...
			if(str_replace("'","",$sewing_production_variable)==3)//color and size wise
			{
				...
				$sql_prod = "SELECT a.production_type,b.color_size_break_down_id as colSizeId, sum(b.production_qnty) as production_qnty from pro_garments_production_mst a,pro_garments_production_dtls b,wo_po_color_size_breakdown c where a.id=b.mst_id and b.color_size_break_down_id=c.id and a.status_active=1 and a.is_deleted=0 and a.production_type in(1,4) and a.po_break_down_id=$hidden_po_break_down_id and a.item_number_id=$cbo_item_name and a.country_id=$cbo_country_name $ship_date_cond2 group by a.production_type,b.color_size_break_down_id";
				...
				/* $color_sizeID_arr=sql_select( "select id,size_number_id,color_number_id from wo_po_color_size_breakdown where po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name and country_id=$cbo_country_name $ship_date_cond and status_active=1 and is_deleted=0  order by color_number_id,size_number_id" );
				$colSizeID_arr=array();
				foreach($color_sizeID_arr as $val){
					$index = $val[csf("size_number_id")].$color_arr[$val[csf("color_number_id")]].$val[csf("color_number_id")];
					$colSizeID_arr[$index]=$val[csf("id")];
				...
				foreach($rowEx as $rowE=>$valE)
				{
					...
					$cur_value = $prod_qty_array[$color_size_id][1]+$colorSizeValue;
					// CUTTING QTY IS NOT LESS THAN SEWING IN QTY				
					if($cur_value < $prod_qty_array[$color_size_id][4])
					{
					...
					if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",1,'".$color_size_id."','".$colorSizeValue."','".$rejQtyArr[$color_size_id]."',".$cbo_color_type.")";
					else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",1,'".$color_size_id."','".$colorSizeValue."','".$rejQtyArr[$color_size_id]."',".$cbo_color_type.")";
					//$dtls_id=$dtls_id+1;
					$j++;
				}
			}
...
if($action == "cutting_reject_challan")
{
	...
	$mst_sql=sql_select("SELECT cut_no,challan_no  from pro_garments_production_mst where id in($mstIds) and status_active=1 and production_type=1");
	$challanNo=$cutNo="";
	foreach($mst_sql as $row)
	{
		if($row["CHALLAN_NO"]){ $challanNo.=$row["CHALLAN_NO"].", "; }
		if($row["CUT_NO"]){ $cutNo.=$row["CUT_NO"].", "; }
	}
	$challanNo=rtrim($challanNo,", ");
	$cutNo=rtrim($cutNo,", ");
    ...
    <table width="100%" cellspacing="0" align="center" style="margin-bottom: 5px;">
        ...
        <tr>
        	<td><strong>Cutting No.</strong></td> <td>: <? echo $cutNo; ?></td>
            <td></td>            
        </tr>
    </table>
