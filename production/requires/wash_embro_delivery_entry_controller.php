...
if($action=="load_drop_down_embro_issue_source")
{
	...
	if($data==3)
	{
		if($db_type==0)
		{
			echo create_drop_down( "cbo_emb_company", 180, "select id,supplier_name from lib_supplier where status_active=1 and is_deleted=0 and find_in_set(23,party_type) order by supplier_name","id,supplier_name", 1, "--- Select ---", $selected, "load_drop_down( 'requires/wash_embro_delivery_entry_controller', this.value, 'load_drop_down_location', 'location_td' );" );
		}
		else
		{
			echo create_drop_down( "cbo_emb_company", 180, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=23 and a.status_active=1 group by a.id,a.supplier_name order by a.supplier_name","id,supplier_name", 1, "--Select--", 0, "load_drop_down( 'requires/wash_embro_delivery_entry_controller', this.value, 'load_drop_down_location', 'location_td' );" );
		}
	}
	else if($data==1)
		echo create_drop_down( "cbo_emb_company", 180, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1 $company_cond order by company_name","id,company_name", 1, "--- Select ---", $selected_company, "load_drop_down( 'requires/wash_embro_delivery_entry_controller', this.value, 'load_drop_down_location', 'location_td' );",0,0 );	
	else
		echo create_drop_down( "cbo_emb_company", 180, $blank_array,"", 1, "--- Select ---", $selected, "load_drop_down( 'requires/wash_embro_delivery_entry_controller', this.value, 'load_drop_down_location', 'location_td' );",0 );	
            ...
            <table cellpadding="0" cellspacing="0" width="550" border="1" rules="all" class="rpt_table">
                <thead>
                    ...
                    <th>
						...
						<input type="hidden" name="hidden_job_no" id="hidden_job_no">  
...
if($action=="create_bundle_search_list_view")
{
	...
	$scanned_bundle_arr=return_library_array( "SELECT b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=2 and a.embel_name=3 $cutCon_a and b.status_active=1 and b.is_deleted=0",'bundle_no','bundle_no');
	foreach(explode(",",$selectedBuldle) as $bn)
	{
		$scanned_bundle_arr[$bn]=$bn;	
	}
	   
	$sql="SELECT c.cut_no, c.bundle_no, d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.production_qnty as qty, e.po_number from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e ,wo_po_details_master f where d.job_id=f.id and  a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and a.company_id=$company and e.po_number like '$txt_order_no' and c.bundle_no like '$bundle_no' $jobCon $cutCon and c.production_type=5 and a.status_active=1 and a.is_deleted=0 order by  length(c.bundle_no) asc, c.bundle_no asc";
					...
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value(<? echo $i; ?>)"> 
						<td width="40">
							<? echo $i; ?>
							 ...
							 <input type="hidden" name="txt_job_no" id="txt_job_no<?php echo $i; ?>" value="<?php echo $row[csf('job_no_mst')]; ?>"/>
						</td>
...
if($action=="challan_duplicate_check")
{
	...
	if($db_type==2 && $bundle_count>400)
	{
		...
		foreach($bundleArr as $bundleNos)
		{
			...
			$bundle_nos_cond.=" b.bundle_no in($bundleNos) or ";
		}
	...
	{
		$bundle_nos_cond=" and b.bundle_no in ($bundle_no)";
	}
	
	$result=sql_select("select a.sys_number,b.bundle_no from pro_gmts_delivery_mst a,pro_garments_production_dtls b where a.id=b.delivery_mst_id and b.production_type=2 and a.embel_name=3 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $bundle_nos_cond group by a.sys_number,b.bundle_no ");
	$datastr="";
	if(count($result)>0)
	{
		foreach ($result as $row)
...
if($action=='populate_embel_type')
{
	$data=explode("**",$data);
	$job_no_arr = explode(",",$data[0]);
	$job_ond = where_con_using_array($job_no_arr,1,"job_no");
	$sql =sql_select("SELECT emb_type FROM WO_PRE_COST_EMBE_COST_DTLS WHERE  emb_name=3 $job_ond  and  status_active=1 and is_deleted=0 ");
	$wash_type_arr = array();
	foreach ($sql as $v) 
	{
		$wash_type_arr[$v['EMB_TYPE']] = $emblishment_wash_type[$v['EMB_TYPE']];
	}
	echo create_drop_down( "cbo_embel_type", 180, $wash_type_arr,"", 1, "-- Select --", $selected, "" );
	...
	foreach($result as $row){ ?>
		<td> <?  echo create_drop_down( "cbo_embel_type".$i, 180, $type_array[$row[csf("emb_name")]],"", 1, "--Select--", $row[csf("emb_type")], "check_duplicate(".$i.")",$dis,"" ); ?></td>  

		<?
		$i++;
	} */

	//$emblishment_wash_type = array(1=>"fiz",2=>"khan",3=>"hello");
	//echo create_drop_down( "cbo_embel_type", 180, $emblishment_wash_type,"", 1, "--- Select Type ---", $selected, "" ); 
}
if($action=="populate_bundle_data")
{
	...
	$bundle_count=count(explode(",",$bundle_nos)); $qtybundle_nos_cond=""; $bundle_nos_cond="";
	if($db_type==2 && $bundle_count>400)
	{
		$qtybundle_nos_cond=" and (";
		$bundle_nos_cond=" and (";
		$bundleArr=array_chunk(explode(",",$bundle_nos),399);
		foreach($bundleArr as $bundleNos)
		{
			$bundleNos=implode(",",$bundleNos);
			$qtybundle_nos_cond.=" bundle_no in($bundleNos) or ";
			$bundle_nos_cond.=" c.bundle_no in($bundleNos) or ";
		}
		$qtybundle_nos_cond=chop($qtybundle_nos_cond,'or ');
		$qtybundle_nos_cond.=")";
	...
	{
		$qtybundle_nos_cond=" and bundle_no in ($bundle_nos)";
		
		$bundle_nos_cond=" and c.bundle_no in ($bundle_nos)";
	}
	
	$scanned_bundle_arr=return_library_array( "select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=2 and a.embel_name=3 and b.status_active=1 and b.is_deleted=0",'bundle_no','bundle_no');
	
	$size_arr=return_library_array( "select id, size_name from lib_size",'id','size_name');
	$color_arr=return_library_array( "select id, color_name from lib_color", "id", "color_name");
	$country_arr=return_library_array( "select id, country_name from lib_country",'id','country_name');
	$buyer_arr=return_library_array( "select id, short_name from lib_buyer",'id','short_name');
	$bundle_qty_arr=return_library_array( "select bundle_no,sum((production_qnty+reject_qty)-replace_qty) as qty from pro_garments_production_dtls where production_type=1 $qtybundle_nos_cond group by bundle_no",'bundle_no','qty');
	...
	$sql="SELECT d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, MAX($year_field) as year, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id,c.cut_no, c.bundle_no, 
	SUM(c.reject_qty) as raj_qty, SUM(c.alter_qty) as alt_qty, SUM(c.spot_qty) as spt_qty, SUM(c.replace_qty) as replace_qty,e.po_number from pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name=$ex_data[3] and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.production_type in(1,2,3) and c.status_active=1 and c.is_deleted=0 $bundle_nos_cond group by d.id, e.id, f.job_no_prefix_num, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.cut_no,c.bundle_no, e.po_number order by  length(c.bundle_no) asc, c.bundle_no asc";

	 //echo $sql; die;
	
	//var_dump($bundle_qty_arr);
	...
	<div style="width:100%;max-height:180px; overflow-y:scroll" id="sewing_production_list_view" align="left">
		...
		<?php  
			...
			foreach($sqlResult as $selectResult)
			{
 			...
 			?>
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_issue_form_data','requires/wash_embro_delivery_entry_controller');" > 
...
if($action=="populate_issue_form_data")
{
	...
	{ 
		...
		echo "get_php_form_data(".$result[csf('po_break_down_id')]."+'**'+".$result[csf("item_number_id")]."+'**'+".$result[csf("embel_name")]."+'**'+".$result[csf("country_id")].", 'populate_data_from_search_popup', 'requires/wash_embro_delivery_entry_controller' );\n";

		echo "$('#cbo_item_name').val('".$result[csf('item_number_id')]."');\n";
		echo "$('#cbo_country_name').val('".$result[csf('country_id')]."');\n";
		
		echo "show_list_view('".$result[csf('po_break_down_id')]."','show_country_listview','list_view_country','requires/wash_embro_delivery_entry_controller','');\n";
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$txt_system_id)=="")
		{
			...
			if($db_type==0) $year_cond="YEAR(insert_date)"; 
 		    ...
 		    $new_sys_number = explode("*", return_next_id_by_sequence("", "pro_gmts_delivery_mst",$con,1,$cbo_company_name,'WDE',0,date("Y",time()),0,0,2,$cbo_embel_name,0 ));


			$field_array_delivery="id, sys_number_prefix, sys_number_prefix_num, sys_number, company_id, production_type, location_id, delivery_basis, embel_name, embel_type, production_source, serving_company, floor_id, organic, delivery_date,working_company_id,working_location_id, inserted_by, insert_date";
			$data_array_delivery="(".$mst_id.",'".$new_sys_number[1]."','".(int)$new_sys_number[2]."','".$new_sys_number[0]."', ".$cbo_company_name.",2,".$cbo_location.",".$delivery_basis.",".$cbo_embel_name.",".$cbo_embel_type.",".$cbo_source.",".$cbo_emb_company.",".$cbo_floor.",".$txt_organic.",".$txt_issue_date.",".$cbo_working_company_name.",".$cbo_working_location.",".$user_id.",'".$pc_date_time."')";
		...
		else
		{
			...
			$field_array_delivery="location_id*delivery_basis*embel_name*embel_type*production_source*serving_company*floor_id*organic*delivery_date*working_company_id*working_location_id*updated_by*update_date";
			$data_array_delivery="".$cbo_location."*".$delivery_basis."*".$cbo_embel_name."*".$cbo_embel_type."*".$cbo_source."*".$cbo_emb_company."*".$cbo_floor."*".$txt_organic."*".$txt_issue_date."*".$cbo_working_company_name."*".$cbo_working_location."*".$user_id."*'".$pc_date_time."'";
		}
		...
		if(str_replace("'","",$delivery_basis)==3)
		{
			...
			$field_array_dtls="id,delivery_mst_id,mst_id,production_type,color_size_break_down_id,production_qnty,cut_no,bundle_no,color_type_id";
			...
			foreach($dtlsArr as $bundle_no=>$qty)
			{
				...
				$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",2,'".$dtlsArrColorSize[$bundle_no]."','".$qty."','".$cut_no."','".$bundle_no."','".$bundle_wise_type_array[$bundle_no]."')";
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$field_array_delivery="location_id*embel_name*embel_type*production_source*serving_company*floor_id*organic*delivery_date*working_company_id*working_location_id*updated_by*update_date";
		$data_array_delivery="".$cbo_location."*".$cbo_embel_name."*".$cbo_embel_type."*".$cbo_source."*".$cbo_emb_company."*".$cbo_floor."*".$txt_organic."*".$txt_issue_date."*".$cbo_working_company_name."*".$cbo_working_location."*".$user_id."*'".$pc_date_time."'";
		
		if(str_replace("'","",$delivery_basis)==3)
		{
			//$id=return_next_id("id", "pro_garments_production_mst", 1);
			//	$dtls_id=return_next_id("id", "pro_garments_production_dtls", 1);
			$non_delete_arr=production_validation($mst_id,3);
			$issue_data_arr=production_data($mst_id,2);

			$delete = execute_query("DELETE FROM pro_garments_production_mst WHERE delivery_mst_id=$mst_id and production_type=2 and embel_name=3");
			$delete_dtls = execute_query("DELETE FROM pro_garments_production_dtls WHERE delivery_mst_id=$mst_id and production_type=2");
			$delete_bundle = execute_query("DELETE FROM pro_cut_delivery_color_dtls WHERE delivery_mst_id=$mst_id and production_type=2 and embel_name=3");
		...
		<legend>Enter search words</legend>           
            <table cellpadding="0" cellspacing="0" width="700" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
            		...
            		<td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_embel_type').value+'_'+document.getElementById('txt_company_id').value+'_'+document.getElementById('txt_cutt_no').value, 'create_challan_search_list_view', 'search_div', 'wash_embro_delivery_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                     </td>
...
if($action=="create_challan_search_list_view")
{
	...
	$sql = "select a.id, $year_field, a.sys_number_prefix_num, a.sys_number ,a.delivery_date, a.embel_type, a.production_source, a.serving_company, a.location_id, a.floor_id, a.organic,b.cut_no,b.production_quantity from pro_gmts_delivery_mst a,pro_garments_production_mst b where a.id=b.delivery_mst_id and  a.production_type=2 and a.embel_name=3 and a.status_active=1 and a.is_deleted=0 and a.company_id=$company_id $search_field_cond $print_type_cond $cut_no order by a.id DESC"; 
	//echo $sql;//die;
	$result = sql_select($sql);
...
{
	$data_array=sql_select("select id, company_id, sys_number, embel_type, embel_name, production_source, serving_company, location_id, floor_id, organic, delivery_date,working_company_id,working_location_id from pro_gmts_delivery_mst where id='$data'");
	...
	{ 
		...
		echo "load_drop_down( 'requires/wash_embro_delivery_entry_controller', ".$row[csf('production_source')].", 'load_drop_down_embro_issue_source', 'emb_company_td' );\n";
		echo "load_drop_down( 'requires/wash_embro_delivery_entry_controller','".$row[csf('company_id')]."', 'load_drop_down_location', 'location_td' );\n";
		echo "load_drop_down( 'requires/wash_embro_delivery_entry_controller','".$row[csf('working_company_id')]."', 'load_drop_down_working_location', 'working_location_td' );\n";
		echo "$('#cbo_emb_company').val('".$row[csf('serving_company')]."');\n";
		echo "$('#cbo_location').val('".$row[csf('location_id')]."');\n";
		echo "load_drop_down( 'requires/wash_embro_delivery_entry_controller', ".$row[csf('location_id')].", 'load_drop_down_floor', 'floor_td' );\n";
...
if($action=="emblishment_issue_print")
{
	...
	production_source, serving_company, floor_id, organic, delivery_date,working_company_id,working_location_id from pro_gmts_delivery_mst where production_type=2 and id='$data[1]' and 
...
<div style="width:930px;">
    <table cellspacing="0"  style="font: 12px tahoma; width: 100%;">
        ...
        <?
			...
			{
				$sql="SELECT sum(a.production_qnty) as production_qnty,c.po_break_down_id,c.item_number_id,c.country_id, b.color_number_id,sum(d.NUMBER_START) number_start, sum(d.NUMBER_END) number_end,
				(select sum(c.number_start) from ppl_cut_lay_bundle c where  b.bundle_no = c.bundle_no) number_start,
				(select sum(e.number_end) from ppl_cut_lay_bundle e where  b.bundle_no = e.bundle_no) number_end
					...
					and c.id=a.mst_id and b.bundle_no = d.bundle_no and a.color_size_break_down_id=b.id and b.production_type=2 and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0 
					group by c.po_break_down_id,c.item_number_id,c.country_id,b.color_number_id ";
			}
			$result=sql_select($sql);
		?> 
...
if($db_type==2) $group_concat="  listagg(cast(b.cut_no AS VARCHAR2(4000)),',') within group (order by b.cut_no) as cut_no" ;
			...
			if($data[2]==3)
			{
				...
				else
				{
					...
					where a.delivery_mst_id ='$data[1]' and a.id=b.mst_id and b.bundle_no = c.bundle_no and b.color_size_break_down_id=d.id and b.status_active=1 and b.is_deleted=0 and d.status_active in(1,2,3) 
