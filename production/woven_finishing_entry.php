	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			if (po_id!="")
			{
				...
				// alert($('#hidden_preceding_process').val());

				childFormReset();//child from reset
				get_php_form_data(po_id+'**'+item_id+'**'+country_id+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'**'+job_no, "populate_data_from_search_popup", "requires/woven_finishing_entry_controller" );
				...
				// let txt_job_no=$('#txt_job_no').val();
				if(variableSettings!=1){
					get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'**'+job_no+'**'+country_ship_date, "color_and_size_level", "requires/woven_finishing_entry_controller" );
				...
				show_list_view(po_id+'**'+$('#hidden_preceding_process').val()+'**'+job_no,'show_country_listview','list_view_country','requires/woven_finishing_entry_controller','');
...
function fnc_poly_output_entry(operation)
{
	...
	else if(operation==0 || operation==1 || operation==2)
	{
		...
		else
		{
			...
			var data="action=save_update_delete&operation="+operation+"&colorIDvalue="+colorIDvalue+"&colorIDvalueRej="+colorIDvalueRej+get_submitted_data_string('garments_nature*cbo_company_name*cbo_country_name*poly_production_variable*poly_production_variable_rej*hidden_po_break_down_id*hidden_colorSizeID*cbo_buyer_name*txt_style_no*cbo_item_name*txt_order_qty*cbo_source*cbo_poly_company*cbo_location*cbo_floor*txt_poly_date*cbo_produced_by*cbo_poly_line*txt_reporting_hour*cbo_color_type*txt_super_visor*txt_poly_qty*txt_reject_qnty*txt_alter_qnty*txt_challan*txt_remark*txt_input_quantity*txt_cumul_poly_qty*txt_yet_to_poly*hidden_break_down_html*txt_mst_id*prod_reso_allo*txt_spot_qnty*save_data*defect_type_id*save_dataSpot*defectSpot_type_id*save_dataGateup*defectGateup_type_id*save_dataWash*defectWash_type_id*save_dataMeasurement*defectMeasurement_type_id*save_dataFinalBody*defectFinalBody_type_id*hidden_currency_id*hidden_exchange_rate*hidden_piece_rate*cbo_work_order*txt_pack_type*txt_job_no*country_ship_date',"../");
 			// alert (data); return;
...
function fnc_poly_output_entry_Reply_info()
{
 	if(http.readyState == 4)
	{
		...
		else if(reponse[0]==0)
		{
			...
			//  cbo_produced_by*
			reset_form('','','txt_reporting_hour*cbo_color_type*txt_super_visor*txt_poly_qty*txt_reject_qnty*txt_alter_qnty*txt_remark*txt_input_quantity*txt_cumul_poly_qty*txt_yet_to_poly*hidden_break_down_html*txt_mst_id*txt_spot_qnty*save_data*defect_type_id*all_defect_id*save_dataSpot*allSpot_defect_id*defectSpot_type_id*country_ship_date','','','cbo_produced_by');
		...
		else if(reponse[0]==1)
		{
			...
			show_list_view(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id+'**'+prod_reso_allo+'**'+pack_type+'**'+variableSettings,'show_dtls_listview','list_view_container','requires/woven_finishing_entry_controller','setFilterGrid(\'tbl_list_search\',-1)');
			// cbo_produced_by*
			reset_form('','','txt_reporting_hour*cbo_color_type*txt_super_visor*txt_poly_qty*txt_reject_qnty*txt_alter_qnty*txt_remark*txt_input_quantity*txt_cumul_poly_qty*txt_yet_to_poly*hidden_break_down_html*txt_mst_id*txt_spot_qnty*save_data*defect_type_id*all_defect_id*save_dataSpot*allSpot_defect_id*defectSpot_type_id','txt_challan,0','');
			get_php_form_data(po_id+'**'+$('#cbo_item_name').val()+'**'+country_id+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'**'+job_no, "populate_data_from_search_popup", "requires/woven_finishing_entry_controller" );
			if(variableSettings!=1) {
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'**'+job_no+'**'+country_ship_date, "color_and_size_level", "requires/woven_finishing_entry_controller" );
			}
			else
		...
		else if(reponse[0]==2)
		{
			...
			show_msg(trim(reponse[0]));
			show_list_view(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id+'**'+prod_reso_allo+'**'+pack_type+'**'+variableSettings,'show_dtls_listview','list_view_container','requires/woven_finishing_entry_controller','setFilterGrid(\'tbl_list_search\',-1)');
			reset_form('','','cbo_produced_by*txt_reporting_hour*cbo_color_type*txt_super_visor*txt_poly_qty*txt_reject_qnty*txt_alter_qnty*txt_remark*txt_input_quantity*txt_cumul_poly_qty*txt_yet_to_poly*hidden_break_down_html*txt_mst_id*txt_spot_qnty*save_data*defect_type_id*all_defect_id*save_dataSpot*allSpot_defect_id*defectSpot_type_id','txt_challan,0','');
			get_php_form_data(po_id+'**'+$('#cbo_item_name').val()+'**'+country_id+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'**'+job_no, "populate_data_from_search_popup", "requires/woven_finishing_entry_controller" );
			if(variableSettings!=1) {
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'**'+job_no, "color_and_size_level", "requires/woven_finishing_entry_controller" );
			}
			else
			{
				$("#txt_poly_qty").removeAttr("readonly");
...
function put_country_data(po_id, item_id, country_id, po_qnty, plan_qnty, pack_type,txt_job_no,country_ship_date)
{
	...
	childFormReset();//child from reset
	get_php_form_data(po_id+'**'+item_id+'**'+country_id+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'**'+txt_job_no+'**'+country_ship_date, "populate_data_from_search_popup", "requires/woven_finishing_entry_controller" );
	...
	if(variableSettings!=1)
	{
		get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'**'+txt_job_no+'**'+country_ship_date, "color_and_size_level", "requires/woven_finishing_entry_controller" );
		$("#txt_poly_qty").attr("readonly","readonly");
	}
	else
	{
	...
	show_list_view(po_id+'**'+item_id+'**'+country_id+'**'+prod_reso_allo+'****'+variableSettings,'show_dtls_listview','list_view_container','requires/woven_finishing_entry_controller','');
	set_button_status(0, permission, 'fnc_poly_output_entry',1,0);
	release_freezing();
}
...
<div style="width:100%;">
	...
	<div style="width:800px; float:left;">
                ...
                <fieldset>
                    <table width="100%" border="0">
                        <tr>
                            ...
                            <td width="150">
								<?
                                 echo create_drop_down( "cbo_company_name", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "fnc_loadvariable(this.value);fnc_company_check($('#cbo_source').val()); load_drop_down( 'requires/woven_finishing_entry_controller', $('#cbo_source').val()+'**'+this.value, 'load_drop_down_poly_output', 'sew_company_td' ); get_php_form_data($('#cbo_source').val(),'line_disable_enable','requires/woven_finishing_entry_controller');dynamic_must_entry_caption($('#cbo_source').val());");
                                ?>
                    ...
                    <tr>
                                ...
                                <table  cellpadding="0" cellspacing="1" width="100%">
                                   ...
                                   <tr>
                                     ...
                                     <td><input type="text" name="txt_reject_qnty" id="txt_reject_qnty" class="text_boxes_numeric" style="width:100px;" readonly /></td>
                                      <td><input type="button" name="btn" id="btn" class="formbuttonplasminus" value="Reject chk" style="width:60px" onClick="openmypage_defectQty(3);"/></td>
                     ...
                     <tr>
		   				<td align="center" colspan="9" valign="middle" class="button_container">
							<?
								$date=date('d-m-Y');
								echo load_submit_buttons( $permission, "fnc_poly_output_entry", 0,1,"reset_form('polyoutput_1','list_view_country','','txt_challan,0','childFormReset(),childFormReset1()','cbo_produced_by')",1);
                            ?>
		...
		<div id="list_view_country" style="width:420px; overflow:auto; float:left; padding-top:5px; margin-top:5px; position:relative; margin-left:10px"></div>
    </div>
</body>
