...
<script>
	...
	{ 
		...
		emailwindow.onclose=function()
		{
			...
			if(reqn_id!="")
			{
				...
				var list_view = trim(return_global_ajax_value(reqn_id+"**"+hdn_variable_setting_status, 'populate_list_view', '', 'requires/fabric_requisition_for_cutting_entry_controller'));
	...
	function company_wise_finish_fabric_requisition_cutting_variable(data)
	{
		...
		// alert(varible_data);
			$("#hdn_finish_fabric_requisition_cutting_variable").val(varible_data);
	}
	...
	function com_variable_chk()
	{
		...
		get_php_form_data(cbo_company_name,'load_variable_settings','requires/fabric_requisition_for_cutting_entry_controller');
	}
	...
	function openmypage_po()
	{
		...
		{ 	
			var buyerId = $("#hidden_buyer_id").val();
			var hdn_variable_setting_status = $("#hdn_variable_setting_status").val();
			var hdn_finish_fabric_requisition_cutting_variable = $("#hdn_finish_fabric_requisition_cutting_variable").val();
			var title = 'Fabric Selection Form';	
			var page_link ='requires/fabric_requisition_for_cutting_entry_controller.php?company_id='+cbo_company_id+'&buyerId='+buyerId+'&hdn_variable_setting_status='+hdn_variable_setting_status+'&hdn_finish_fabric_requisition_cutting_variable='+hdn_finish_fabric_requisition_cutting_variable+'&action=po_popup';
			...
			emailwindow.onclose=function()
			{
				...
				for(var k=0; k<data.length; k++)
				{
					...
					// alert(row_data[18]+"__"+row_data[19]+"__"+row_data[20]+"Reqn_qnty:"+row_data[21]);
	...
	function fnc_fabric_requisition_for_cutting( operation )
	{
		...
		for (var i=1; i<=row_num; i++)
		{
			...
			{
				j++;
				dataString+='&buyerId' + j + '=' + buyerId + '&jobNo' + j + '=' + jobNo + '&poId' + j + '=' + poId + '&itemId' + j + '=' + itemId + '&bodyPartId' + j + '=' + bodyPartId + '&deterId' + j + '=' + deterId + '&gsm' + j + '=' + gsm + '&dia' + j + '=' + dia + '&colorId' + j + '=' + colorId + '&sizeId' + j + '=' + sizeId + '&reqsnQty' + j + '=' + reqsnQty + '&dtlsId' + j + '=' + dtlsId + '&hdnPoRatioQty' + j + '=' + hdnPoRatioQty;
			}
		}
		...
		var data="action=save_update_delete&operation="+operation+'&tot_row='+j+get_submitted_data_string('cbo_company_id*layPlan_id*txt_requisition_date*txt_requisition_no*finish_fabric_requisition_cutting_variable*update_id*hdn_variable_setting_status',"../")+dataString;
		//alert(data);//return;
	...
	function fnc_fabric_requisition_for_cutting_Reply_info()
	{
		if(http.readyState == 4) 
		{
			...
			if((response[0]==0 || response[0]==1))
			{
				...
				var hdn_variable_setting_status= $('#hdn_variable_setting_status').val();
				var list_view = trim(return_global_ajax_value(response[1]+"**"+hdn_variable_setting_status, 'populate_list_view', '', 'requires/fabric_requisition_for_cutting_entry_controller'));
				$("#scanning_tbl tbody").html(list_view);
	...
	function generate_print_report_2()
	{
		...
		print_report( $('#cbo_company_id').val()+'*'+$('#txt_requisition_no').val()+'*'+$('#update_id').val()+'*'+$('#txt_layPlan_No').val()+'*'+$('#cbo_template_id').val(), "grey_delivery_print2", "requires/fabric_requisition_for_cutting_entry_controller" );
			return;
	}
            ...
            <fieldset style="width:810px;">
                ...
                <table cellpadding="0" cellspacing="2" width="800">
                    ...
                    <tr>
                        ...
                        <td>
                            <? 
                                echo create_drop_down( "cbo_company_id", 152, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", 0, "wvn_finish_fabric_po_to_style_wise_fnc(this.value);com_variable_data(this.value);com_variable_chk();company_wise_finish_fabric_requisition_cutting_variable(this.value);",0 );
                            ?>
                            <input type="hidden" name="hdn_variable_setting_status" id="hdn_variable_setting_status" class="text_boxes"/>
                            <input type="hidden" name="hdn_finish_fabric_requisition_cutting_variable" id="hdn_finish_fabric_requisition_cutting_variable" class="text_boxes"/>
							<input type="hidden" name="finish_fabric_requisition_cutting_variable" id="finish_fabric_requisition_cutting_variable" value="" />
                        </td>
