...
<script>
	...
	function fnc_left_over_gmts_input(operation)
	{
		...
		{
	 		if ( form_validation('cbo_company_name*cbo_order_type*cbo_store_name*cbo_goods_type*txt_order_no','Company Name*Order Type*Goods Type*Order No')==false )
			{
				return;
			}		
			else
	...
	function fnc_left_over_gmts_input_Reply_info()
	{
	 	if(http.readyState == 4) 
		{
			...
			{ 
				...
				show_list_view(response[1],'show_dtls_listview_system','list_view_container','requires/left_over_garments_receive_controller','setFilterGrid(\'tbl_list_search\',-1)');	
				show_list_view(response[4]+'**'+company_name+'**'+location_name+'**'+order_type+'**'+buyer_name+'**'+goods_type+'**'+working_company_name+'**'+working_location_name,'show_country_listview','list_view_country','requires/left_over_garments_receive_controller');
				get_left_over_balance(response[4],country_id,gmts_item,company_name,location_name,working_company_name,working_location_name);
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				{				
					...
					get_php_form_data(po_id+'**'+country_id+'**'+company_id+'**'+gmts_item+'**'+$('#cbo_goods_type').val()+'**'+variableSettings+'**'+$('#cbo_order_type').val()+'**'+location_name+'**'+$('#garments_nature').val()+'**'+variableSettingsCountry, "color_and_size_level_left_over", "requires/left_over_garments_receive_controller" ); 
					
					show_list_view(po_id+'**'+gmts_item+'**'+country_id+'**'+company_id,'show_dtls_listview','list_view_container','requires/left_over_garments_receive_controller','setFilterGrid(\'tbl_list_search\',-1)');
					show_list_view(po_id+'**'+company_name+'**'+location_name+'**'+order_type+'**'+buyer_name+'**'+goods_type+'**'+working_company_name+'**'+working_location_name,'show_country_listview','list_view_country','requires/left_over_garments_receive_controller');
					...
					get_left_over_balance(po_id,country_id,gmts_item,company_name,location_name,working_company_name,working_location_name);
	...
	function put_country_data(po_id, item_id, country_id, lft_qnty)
	{
		...
		get_php_form_data(po_id+'**'+item_id+'**'+country_id, "populate_data_from_search_popup", "requires/left_over_garments_receive_controller" );
		...
		{
			get_php_form_data(po_id+'**'+country_id+'**'+company_id+'**'+item_id+'**'+$('#cbo_goods_type').val()+'**'+variableSettings+'**'+$('#cbo_order_type').val()+'**'+location_name+'**'+$('#garments_nature').val()+'**'+variableSettingsCountry, "color_and_size_level_left_over", "requires/left_over_garments_receive_controller" ); 

			// get_php_form_data(po_id+'**'+country_id+'**'+company_id+'**'+item_id+'**'+$('#cbo_goods_type').val()+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+$('#hidden_preceding_process').val(), "color_and_size_level_left_over", "requires/left_over_garments_receive_controller" );
		}
		else
		{
		...
		show_list_view(po_id+'**'+item_id+'**'+country_id,'show_dtls_listview','list_view_container','requires/left_over_garments_receive_controller','setFilterGrid(\'tbl_list_search\',-1)');
		reset_form('','','txt_total_left_over_receive*txt_remark','','');
		set_button_status(0, permission, 'fnc_left_over_gmts_input',1,0);
		release_freezing();
	}

	function get_left_over_balance(po_id,country_id,gmts_item,company,location,wo_company,wo_location)
	{
		var data = 'po_id='+po_id+'&country_id='+country_id+'&gmts_item='+gmts_item+'&company='+company+'&location='+location+'&wo_company='+wo_company+'&wo_location='+wo_location+'&action=get_left_over_balance';	
	...
	function system_number_popup()
	{
		...
		{*/
			...
			emailwindow.onclose=function()
			{
				...
				show_list_view(responseDataArr[0],'show_dtls_listview_system','list_view_container','requires/left_over_garments_receive_controller','setFilterGrid(\'tbl_list_search\',-1)');
	...
	function populate_selected_data(dtls_id,mst_id,po_id,country_id,item_id,variableSettings)
	{
		...
		if(variableSettings!=1)
		{ 
			get_php_form_data(po_id+'**'+country_id+'**'+item_id+'**'+dtls_id+'**'+mst_id+'**'+variableSettings+'**'+$('#cbo_order_type').val()+'**'+$('#country_maintain_variable').val(), "color_and_size_level_left_over_update", "requires/left_over_garments_receive_controller" ); 
			get_left_over_balance(po_id,country_id,item_id);
			show_list_view(po_id+'**'+company_name+'**'+location_name+'**'+order_type+'**'+buyer_name+'**'+goods_type+'**'+working_company_name+'**'+working_location_name,'show_country_listview','list_view_country','requires/left_over_garments_receive_controller');
	...
	function left_over_gmts_receive_print()
	{
		...
		{
			var report_title=$( "div.form_caption" ).html();
			print_report( $('#cbo_company_name').val()+'*'+$('#txt_mst_id').val()+'*'+report_title+'*'+$('#txt_system_no').val()+'*'+$('#cbo_order_type').val(), "left_over_gmts_receive_print", "requires/left_over_garments_receive_controller");
		}
                            ...
                            <legend>New Entry</legend>
                                <table  cellpadding="0" cellspacing="2" width="100%">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                        <input type="text" name="txt_total_left_over_receive" id="txt_total_left_over_receive" class="text_boxes_numeric" style="width:120px" keypress="left_over_amount(this.value)" disabled/>
