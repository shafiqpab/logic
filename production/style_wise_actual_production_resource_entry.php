<?
/*-------------------------------------------- Comments
Purpose			: 	This form for Style Wise Actual Production Resource Entry
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Style Wise Actual Production Resource Entry","../", 1, 1, $unicode,1,'');
?>	
...
{ 
	...
	let hide_style_data = $("#style_data").val();
	
	var page_link="requires/style_wise_actual_production_resource_entry_controller.php?action=job_no_popup&company_name="+company_name+'&sid='+sid+'&from_date='+from_date+'&to_date='+to_date+'&target_hour='+target_hour+'&mst_dtls_id='+mst_dtls_id+'&hide_style_data='+hide_style_data;
	var title="Job PopUp";
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1240px,height=450px,center=1,resize=0,scrolling=0','')

	emailwindow.onclose=function()
	{
		let hide_style_data=this.contentDoc.getElementById("hide_style_data").value; 
		// alert(hide_style_data);
		$("#style_data").val(hide_style_data);
		let hide_style_data_arr = hide_style_data.split("!!");
		// alert(hide_style_data_arr.length);
		let style_name = "";
		let target = 0;
		let operator = 0;
		let helper = 0;
		...
		var target_hour_arr = [];
		var wo_hour_arr = [];
		for (let i = 0; i < hide_style_data_arr.length; i++) 
		{
			var exData = hide_style_data_arr[i].split("_");
			// alert(exData[14]);
			style_data_arr.push(exData[14]);
			man_power_arr.push((exData[7]*1)+(exData[8]*1)+(exData[12]*1)+(exData[13]*1));
			operator_arr.push((exData[7]*1));
			helper_arr.push((exData[8]*1));
			ironman_arr.push((exData[12]*1));
			qi_arr.push((exData[13]*1));
			target_hour_arr.push((exData[6]*1));
			wo_hour_arr.push((exData[9]*1));
		}
		style_data_arr = [...new Set(style_data_arr)];
		// alert(style_data_arr.join('/'));
		
		for (let index = 0; index < hide_style_data_arr.length; index++) 
		{
			var ex_data = hide_style_data_arr[index].split("_");
			// alert(ex_data[14]);
			if (style_data_arr.length>1) // when multi style
			{				
				style_name += (style_name=="") ? ex_data[14] : ","+ex_data[14];
				
				target += ex_data[6]*1;
				operator = ex_data[7]*1;
				helper = ex_data[8]*1;
				iron_man = ex_data[12]*1;
				qi = ex_data[13]*1;
				wo_hour += ex_data[9]*1;
				acct_smv = ex_data[10]*1;
				effi = ex_data[11]*1;
				capacity = wo_hour*target;
				tot_wo_hour += ex_data[9]*1;
				tot_target += ex_data[9]*ex_data[6]*1;
				avaiable_min += (ex_data[9]*60)*((ex_data[7]*1)+(ex_data[8]*1));
				target_min += (ex_data[6]*ex_data[10]*ex_data[9]);
			}
			else
			{
				style_name = ex_data[14];
		...
		// alert(target);
		// target = Math.max(...target_hour_arr);
		// wo_hour = Math.max(...wo_hour_arr);
		var efficiency = (target_min/avaiable_min)*100;
		capacity = tot_wo_hour*target;
		let target_per_hour = tot_target/tot_wo_hour;
		let op = Math.max(...operator_arr);
		let hp = Math.max(...helper_arr);
		let iron = Math.max(...ironman_arr);
		let qis = Math.max(...qi_arr);
		let tot_manpower = op+hp+iron+qis;
		$("#txt_style_ref_show").val(style_name);
		$("#txt_target_hour").val(target_per_hour.toFixed());
		$("#txt_operator").val(Math.max(...operator_arr));
		$("#txt_helper").val(Math.max(...helper_arr));
		$("#txt_iron_man").val(Math.max(...ironman_arr));
		$("#txt_qi").val(Math.max(...qi_arr));
		$("#txt_working_hour").val(wo_hour);
		$("#txt_capacity").val(tot_target);
		$("#txt_efficiency").val(efficiency.toFixed(2));
		// $("#txt_man_power").val(operator+helper+iron_man+qi);
		// $("#txt_man_power").val(Math.max(...man_power_arr));
		$("#txt_man_power").val(tot_manpower);

		show_list_view($('#system_id').val(),'create_acl_pdc_rec_entry_list_view','list_container','requires/style_wise_actual_production_resource_entry_controller','setFilterGrid("list_view",-1)');
		release_freezing();
		
	}
}

function fn_populate_system_data(data)
{
	var data_ex = data.split('_');
	show_list_view(data_ex[1],'show_system_data','system_data_container','requires/style_wise_actual_production_resource_entry_controller','setFilterGrid("tbl_style_ref",-1)');
}

function set_system_data(data)
{
	freeze_window();
	theemail=data.split("_");
	...
	load_drop_down( 'requires/style_wise_actual_production_resource_entry_controller', theemail[2], 'load_drop_down_location', 'location_td' );
	document.getElementById('cbo_location').value=theemail[3];
	load_drop_down( 'requires/style_wise_actual_production_resource_entry_controller',theemail[3]+'_'+document.getElementById('cbo_company_name').value, 'load_drop_down_floor', 'floor_td' );			
	...
	show_list_view(theemail[0],'create_acl_pdc_rec_entry_list_view','list_container','requires/style_wise_actual_production_resource_entry_controller','setFilterGrid("list_view",-1)');

	var data = theemail[2]+'*'+theemail[3]+'*'+theemail[4]+'*'+theemail[5]+'*'+theemail[6];

	get_php_form_data(data,'sweing_production_start','requires/style_wise_actual_production_resource_entry_controller' );
...
{ 
	var page_link="requires/style_wise_actual_production_resource_entry_controller.php?action=Line_popup"+"**"+document.getElementById('cbo_line_no').value+"**"+document.getElementById('cbo_line_merge').value+"**"+document.getElementById('cbo_company_name').value+"**"+document.getElementById('cbo_location').value+"**"+document.getElementById('cbo_floor').value; 
	var title="Style Ref";
...
function openmypage_time()
{
	...
	var page_link = 'requires/style_wise_actual_production_resource_entry_controller.php?cbo_company_name='+cbo_company_name+'&cbo_location='+cbo_location+'&cbo_floor='+cbo_floor+'&cbo_line_no='+cbo_line_no+'&cbo_line_merge='+cbo_line_merge+'&save_string='+save_string+'&action=time_popup';
...
function openmypage_employee(order_id,action,width,height)
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/style_wise_actual_production_resource_entry_controller.php?order_id='+order_id+'&action='+action, 'Employee Popup', 'width='+popup_width+','+'height='+popup_height+',center=1,resize=0,scrolling=0','../');
...
function openmypage_adjustment_min()
{
	...
	var title="Production Info";
	var page_link = 'requires/style_wise_actual_production_resource_entry_controller.php?cbo_company_name='+cbo_company_name+'&cbo_company_name='+cbo_company_name+'&system_id='+system_id+'&h_dtl_mst_id='+h_dtl_mst_id+'&txt_date_to='+txt_date_to+'&txt_date_from='+txt_date_from+'&action=adjustment_minint_popup';
...
function system_id_popup()
{
	...
	var company_name=$('#cbo_company_name').val(); 
	var page_link="requires/style_wise_actual_production_resource_entry_controller.php?action=SystemIdPopup&company_name="+company_name; 
	var title="Style Ref";
...
function fnc_emp_info_reponse()
{
	...
	{ 
		...
		{	
			...
			show_list_view(response[1],'create_acl_pdc_rec_entry_list_view','list_container','requires/style_wise_actual_production_resource_entry_controller','setFilterGrid("list_view",-1)');
			set_button_status(0, permission, 'fnc_Ac_Production_Resource_Entry',1);
...
function fnResetForm()
{
	...
	// reset_form('style_wise_actual_production_resource_entry_1','list_container','','','','');
...
<legend>Production Module</legend>  
    <form name="style_wise_actual_production_resource_entry_1" id="style_wise_actual_production_resource_entry_1" autocomplete="off" >
            <table width="100%" border="0">
               <tr>
                ...
                <tr>
                    ...
                    <td width="170">                                
                        <? 
                         echo create_drop_down( "cbo_company_name",170, "SELECT id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $working_credential_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/style_wise_actual_production_resource_entry_controller', this.value, 'load_drop_down_location', 'location_td' );get_php_form_data(this.value,'sweing_production_start','requires/style_wise_actual_production_resource_entry_controller' );" ,"","","","","",2 );
                        ?>
                    </td>
                ...
                <tr>    
                    ...
                    <td id="line_no_td">
                     ...
                     <td>
                         <input type="text" name="txt_style_ref_show" id="txt_style_ref_show" class="text_boxes" style="width:160px;" tabindex="12" placeholder="Browse" onDblClick="open_popup();" readonly>
						 <input type="hidden" name="style_data" id="style_data">
		   ...
		   <br clear="all">
            <table width="80%" border="0" id="acprsreenTable">
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="txt_target_hour" id="txt_target_hour" class="text_boxes_numeric" tabindex="15" onkeyup="calculate(this.value);" style="width:208px " disabled>
                    </td>
                </tr>
                <tr>
                     <td align="right">Operator/line</td>
                     <td>
                         <input type="text" name="txt_operator" id="txt_operator" class="text_boxes_numeric" tabindex="10" style="width:208px" onblur="fn_set_manpower();" disabled>
                     </td> 
                    <td class="must_entry_caption" align="right">Working Hour</td>
                    <td>
                        <input type="text" name="txt_working_hour" id="txt_working_hour" class="text_boxes_numeric"  tabindex="16" onkeyup="calculate(this.value);" style="width:208px " disabled>
                    </td>
                </tr>
                <tr>
                     <td align="right">Helper / Iron Man / QI</td>
                     <td>
                         <input type="text" name="txt_helper" id="txt_helper" class="text_boxes_numeric" placeholder="Helper" style="width:57px " onblur="fn_set_manpower();" disabled>&nbsp;&nbsp;<input type="text" name="txt_iron_man" id="txt_iron_man" class="text_boxes_numeric" placeholder="Iron Man" style="width:57px" onblur="fn_set_manpower();" disabled>&nbsp;&nbsp;<input type="text" name="txt_qi" id="txt_qi" class="text_boxes_numeric" placeholder="QI" style="width:57px" onblur="fn_set_manpower();" disabled>
                     </td> 
                        <td align="right" class="must_entry_caption">Capacity & Target Eff.</td>
                     <td>
                         <input type="text" name="txt_capacity" id="txt_capacity" class="text_boxes_numeric" tabindex="17" style="width:95px " readonly>
                         <input type="text" name="txt_efficiency" id="txt_efficiency" class="text_boxes_numeric" tabindex="17" style="width:97px " readonly>
                     </td>
                </tr>
                <tr>
...
<fieldset style="width:300px;position: absolute;right: -180px;">
	<div id="system_data_container"></div>	
</fieldset>
<br>
<fieldset style="width:1050px; float:left;">
