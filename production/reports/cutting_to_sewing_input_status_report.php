...
{
	//alert("su..re"); return;
	if (form_validation('cbo_company_name*txt_date','Company Name*Production Date')==false)
	{
		return;
	}
	else if($('#txt_ref_no').val()=="" && $('#txt_order_no').val()=="" &&  $('#txt_date').val()=="" )
	{
		form_validation('txt_date')==false
		return;
	}
	else
	{		
		var data="action=report_generate&type="+type+get_submitted_data_string('cbo_company_name*cbo_source_name*cbo_buyer_name*cbo_working_company_name*txt_ref_no*txt_job_no*txt_order_no*txt_date',"../../");
		freeze_window('3');
		http.open("POST","requires/cutting_to_sewing_input_status_report_controller.php",true);
...
function openmypage_job()
{
	...
	var page_link='requires/cutting_to_sewing_input_status_report_controller.php?action=job_no_popup&companyID='+companyID+'&job_no='+job_no;
...
function openmypage_order()
{
	...
	var companyID = $("#cbo_company_name").val();
	var job_no = $("#txt_job_no").val();
	var page_link='requires/cutting_to_sewing_input_status_report_controller.php?action=order_no_search_popup&companyID='+companyID+'&job_no='+job_no;
...
function openmypage_style()
{
	...
	var page_link='requires/cutting_to_sewing_input_status_report_controller.php?action=style_no_popup&companyID='+companyID+'&style_ref_no='+style_ref_no;
	...
	emailwindow.onclose=function()
	{
		...
		var theform=this.contentDoc.forms[0];
		 var style_ref_no=this.contentDoc.getElementById("hide_style_ref_no").value;
		 var style_ref_no_id=this.contentDoc.getElementById("hide_style_ref_id").value;
		
		$('#txt_ref_no').val(style_ref_no);
		$('#txt_style_ref_id').val(style_ref_no_id);	 
	}
}
...
function generate_report_bundle_list(cut_no)
{
    ...
    var data = return_global_ajax_value( cut_no, 'str_data_from_cut_no', '', 'requires/cutting_to_sewing_input_status_report_controller');
    var data = data.trim();
    //alert (data);
    window.open("../../prod_planning/cutting_plan/requires/cut_and_lay_ratio_wise_entry_controller_urmi.php?data=" + data+'&action='+action, true );
}
...
function openmypage_fab_issue(po_id,color,type,action)
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/cutting_to_sewing_input_status_report_controller.php?action='+action+'&data='+data, 'Finish Fabric Issue', 'width=680px,height=250px,center=1,resize=0,scrolling=0','../');
}

function openmypage_production_popup(po,item,color,wo_company,type,day,action,title,popup_width,popup_height) 	 		 	 
{	
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/cutting_to_sewing_input_status_report_controller.php?po='+po+'&action='+action+'&item='+item+'&color='+color+'&wo_company='+wo_company+'&type='+type+'&day='+day+get_submitted_data_string('txt_date',"../../"), title, 'width='+popup_width+','+'height='+popup_height+',center=1,resize=0,scrolling=0','../');
}
...
function getCompanyId() 
{
    ...
    if(company_id !='') {
	  ...
	  load_drop_down( 'requires/cutting_to_sewing_input_status_report_controller', company_id, 'load_drop_down_location', 'location_td' );
         ...
         <fieldset style="width:1400px;">
             <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr class="general">
                        <td id="company_td">
							<? echo create_drop_down( "cbo_company_name", 120, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 0, "-- Select Company --", $selected, "" ); 
							?> 
                        	<input type="hidden" name="size_wise_repeat_cut_no" id="size_wise_repeat_cut_no" readonly>
                        </td>
						</td>
                        <td><? echo create_drop_down( "cbo_source_name", 120, $knitting_source,"", 1, "-- Select Source --", $selected, "","","" ); ?>
					   </td>
                        ...
                        <input type="hidden" name="txt_ref_no" id="txt_ref_no"/>                       
					    </td>
                        <td>                        
                        ...
                        <td><input type="button" id="show_button" class="formbutton" style="width:70px" value="Cutting" onClick="fn_show_report(1)" />&nbsp;
                        <input type="button" id="show_button" class="formbutton" style="width:70px" value="Emb. Issue" onClick="fn_show_report(2)" />
						<input type="button" id="show_button" class="formbutton" style="width:70px" value="Emb. Rcv" onClick="fn_show_report(3)" />
						<input type="button" id="show_button" class="formbutton" style="width:70px" value="Sew. Input" onClick="fn_show_report(4)" />
                        </td>
                    </tr>
                    <!-- <tr>
