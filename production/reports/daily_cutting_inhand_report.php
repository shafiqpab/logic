...
Update date		: 	16-04-2019 (item wise button add shiping status)
QC Performed BY	:
QC Date			:
Comments		:
*/
...
<script>
	...
	//  console.log('hello');
		...
		get_php_form_data($('#cbo_company_name').val(),'print_button_variable_setting','requires/daily_cutting_inhand_report_controller' );
	}
	...
	function generate_report(type)
	{
		...
		var data="action=generate_report"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_location_name*cbo_floor_name*txt_job_no*txt_style_no*txt_order_no*hidden_order_id*hidden_job_id*hidden_style_id*txt_date_from*txt_date_to*cbo_year*txt_production_date*txt_inhand_type*txt_file_no*txt_int_ref_no',"../../")+'&report_title='+report_title+'&type='+type;
		freeze_window(3);
	...
	function openmypage_job_total(job_no,insert_date,action,width,height)
	{
		var popup_width=width;
		var popup_height=height;
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/daily_cutting_inhand_report_controller.php?job_no='+job_no+'&insert_date='+insert_date+'&action='+action, 'Total Popup', 'width='+popup_width+','+'height='+popup_height+',center=1,resize=0,scrolling=0','../');

	}
	function openmypage_job_wise_total(job_no,action,width,height)
	{
		var popup_width=width;
		var popup_height=height;
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/daily_cutting_inhand_report_controller.php?job_no='+job_no+'&action='+action, 'Job Total Popup', 'width='+popup_width+','+'height='+popup_height+',center=1,resize=0,scrolling=0','../');
            ...
            <table class="rpt_table" width="1650" cellpadding="0" cellspacing="0" align="center" border="1" rules="all">
               	<thead>
                    <tr>
                        ...
                        <!-- <th width="90" title="Only for Item Wise Button">Shiping Status</th> -->
                ...
                <tr class="general">
                    <td>
                        <?
                            echo create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "print_button_setting();load_drop_down( 'requires/daily_cutting_inhand_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/daily_cutting_inhand_report_controller',this.value, 'load_drop_down_location', 'location_td' )" );
                        ?>
                    </td>
                    <td id="location_td">
                        <?
                    ...
                    <td id="floor_td">
                        <?
                        echo create_drop_down( "cbo_floor_name", 120, $blank_array,"", 1, "-- Select Buyer --", $selected, "",1,"" );
                        ?>
                    </td>
                    <td id="buyer_td">
                        <?
                    ...
                    <!-- <td>
                     	<?
						// echo create_drop_down( "cbo_shiping_status", 80, $shipment_status,"", 1, "-Select Type-", 0, "",0 );
						?>
                    </td> -->
                    ...
                    <td>
                        ...
                        <!-- <input type="button" id="show_button_po" class="formbutton" style="width:60px" value="PO Wise" onClick="generate_report(1)" />
                        <input type="button" id="show_button_Item_Wise" class="formbutton" style="width:60px" value="Item Wise" onClick="generate_report(2)" />
                        <input type="button" id="show_button_Item_Wise2" class="formbutton" style="width:70px" value="Item Wise2" onClick="generate_report(3)" />
						<input type="button" id="show_buttonItem_Wise3" class="formbutton" style="width:70px" value="Item Wise3" onClick="generate_report(4)" /> -->
                    </td>
