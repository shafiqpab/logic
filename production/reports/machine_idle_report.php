...
var permission = '<? echo $permission; ?>';	
	...
	function openmypage_machine()
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/machine_idle_report_controller.php?action=machine_no_popup&data='+data,'Machine Name Popup', 'width=470px,height=420px,center=1,resize=0','../')
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_location_id*cbo_machine_type*cbo_floor_id*txt_date_from*txt_date_to*txt_machine_name*txt_machine_id',"../../")+'&report_title='+report_title+'&datediff='+datediff;
		freeze_window(3);
		http.open("POST","requires/machine_idle_report_controller.php",true);
	...
	function openmypage_idle(machine_id,date_from,date_to,reason_id,action)
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/machine_idle_report_controller.php?machine_id='+machine_id+'&date_from='+date_from+'&date_to='+date_to+'&reason_id='+reason_id+'&action='+action, 'Cause of Machine Idle', 'width=600px,height=450px,center=1,resize=0,scrolling=0','../');
	}
         ...
         <div id="content_search_panel" style="width:850px" >      
            ...
            <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all">
                <thead>                    
                    ...
                    <th colspan="2" class="must_entry_caption">Idle Date</th>
                    <th width="100" >Machine Name</th>
                    <th width="70"><input type="reset" id="reset_btn" class="formbutton" style="width:70px" value="Reset" onClick="reset_form('machineidlebreakdown_1','report_container*report_container2','','','')" /></th>
                </thead>
                <tbody>
                    <tr class="general">
                        <td> 
							<?
								echo create_drop_down( "cbo_company_id", 140, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down('requires/machine_idle_report_controller', this.value, 'load_drop_down_location', 'location_td' );load_drop_down( 'requires/machine_idle_report_controller', this.value, 'load_drop_down_floor', 'floor_td' );" );
                            ?>
                        </td>
                        <td id="location_td">
							<? 
