...
<script>
	...
	var tableFilters = 
	{
		col_0: "none",
		col_operation: {
			id: ["total_trims_weight_gt","total_batch_qty","total_load_qty","total_unload_qty","total_sliting_qty","total_stenter_qty","total_compect_qty","total_drying_qty","total_special_qty","total_finish_qty","total_process_loss_qty","total_delivery_qty","total_balance_qty"],
	   //col: [14,26,27],
	   col: [20,21,25,26,28,29,30,31,32,33,34,36,38],
	...
	{
		window.open("requires/batch_progress_report_controller.php?data=" + data+'&action='+action, true );
	}
	function print_batch_card(batch_id, batch_no)
		{
			generate_report_file($('#cbo_company_name').val()+'*'+batch_id+'*'+batch_no,'batch_card_print_11','requires/batch_progress_report_controller');
			return;	
		}
                ...
                <tbody>
                    <tr align="center">
					 <td>
                        ...
                        <td> 
                            <?
                                echo create_drop_down( "cbo_company_name", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name","id,company_name", 1, "- Select Company -", $selected, "load_drop_down( 'requires/batch_progress_report_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>
                        </td>
						  <td> 
                                    <?
