<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Sewing Input and Output Report for Urmi
Functionality	:	
JS Functions	:
Created by		:	Shafiq
...
<script>
	...
	{	
		...
		var page_link='requires/style_and_color_wise_production_report_controller.php?action=job_popup&buyer_name='+buyer_name+'&cbo_year='+cbo_year;
	...
	function fn_generate_report(type)
	{
		...
		var data="action=generate_report"+get_submitted_data_string('cbo_company_name*cbo_wo_company_name*cbo_buyer_name*hidden_job_id*txt_date_from*txt_date_to',"../../")+'&type='+type+'&report_title='+report_title;
		 
		freeze_window(3);
		http.open("POST","requires/style_and_color_wise_production_report_controller.php",true);
	...
	function new_window()
	{
		document.getElementById('smry_scroll_body').style.overflow="auto";
		document.getElementById('smry_scroll_body').style.maxHeight="none"; 
	...
	'<html><head><link rel="stylesheet" href="../../css/style_common.css" type="text/css" media="all" /><title></title></head><body>'+document.getElementById('report_container2').innerHTML+'</body</html>');
		...
		document.getElementById('smry_scroll_body').style.overflowY="scroll"; 
		document.getElementById('smry_scroll_body').style.maxHeight="200px";
	...
	function openmypage(po_break_down_id,item_id,action,country_id)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox','iframe','requires/style_and_color_wise_production_report_controller.php?po_break_down_id='+po_break_down_id+'&item_id='+item_id+'&action='+action+'&country_id='+country_id, popup_caption, popupWidth+'center=1,resize=0,scrolling=0','../');
	}	
	function openmypage_remarks_popup(search_string) 	 		 	 
	{
		var popup_width = 600;
		var popup_height = 400;
		var action = "remarks_popup";
		var title = "Remarks Popup";
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/style_and_color_wise_production_report_controller.php?search_string='+search_string+'&action='+action, title, 'width='+popup_width+','+'height='+popup_height+',center=1,resize=0,scrolling=0','../');
            ...
            <table class="rpt_table" width="750" cellpadding="0" cellspacing="0" align="center" border="1" rules="all">
               <thead>                    
                ...
                <tr class="general">
                    <td align="center"> 
                        <?
                            echo create_drop_down( "cbo_company_name", 130, "SELECT id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/style_and_color_wise_production_report_controller', this.value, 'load_drop_down_buyer', 'td_buyer' );" );
                        ?>
                    </td>
                    <td align="center"> 
                        <?
                    ...
                    <td align="center" id="td_buyer"> 
                        <?
							...
							echo create_drop_down( "cbo_buyer_name", 130, "SELECT DISTINCT a.id,a.buyer_name from lib_buyer a, lib_buyer_tag_company b where a.id=b.buyer_id and a.status_active=1 and a.is_deleted=0 order by a.buyer_name","id,buyer_name", 1, "-- Select buyer --", 0, "","" );
                        ?>
                    </td>
                    <td>
