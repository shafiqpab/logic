<?
/*-------------------------------------------- Comments
Purpose         :   This form will create Dyeing WIP Report
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Dyeig WIP Report","../../", 1, 1, $unicode,1,0); 
...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../logout.php";
    ...
    {           
        ...
        {       
            ...
            //alert(data);
            freeze_window(3);
            http.open("POST","requires/dyeing_wip_report_controller.php",true);
            ...
            <legend>Search Panel</legend> 
                <table class="rpt_table" width="1090" cellpadding="0" cellspacing="0" border="1" rules="all">
                    ...
                    <tr class="general">
                        <td>
                            <? 
                               echo create_drop_down( "cbo_company_name", 140, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down('requires/dyeing_wip_report_controller',this.value, 'load_drop_down_location', 'cbo_lc_location_td' );" );
                            ?>                            
                        </td>
                        ...
                        <td>
                            <?
                                ...
                                echo create_drop_down("cbo_source",130,$knitting_source,"", 1, "-- Select --", 0,"load_drop_down( 'requires/dyeing_wip_report_controller', this.value+'_'+document.getElementById('cbo_company_name').value, 'load_drop_down_party','party_td');",0,'1,3');
                            ?>
                        </td> 
                        <td id="party_td">
                            <?
