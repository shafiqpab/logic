...
<script>
	...
	function generate_report(rpt_type)
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_location_id*cbo_floor_id*txt_mc_no*cbo_source*cbo_buyer_name*cbo_search_by*txt_dynamic_search*hide_dynamic_id*cbo_year_selection*txt_date_from*txt_date_to',"../../")+'&report_title='+report_title+'&rpt_type='+rpt_type;
		//alert (data);
		freeze_window(3);
...
<div style="width:100%;" align="center">
    ...
    <h3 style="width:1270px; margin-top:10px;" align="center" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3>
            ...
            <fieldset style="width:1260px;">
                <table class="rpt_table" width="1060" cellpadding="0" cellspacing="0" border="1" rules="all">
                    ...
                    <tr class="general">
                        <td>
                            <? 
                               echo create_drop_down( "cbo_company_id", 160, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/finishing_production_report_controller',this.value, 'load_drop_down_location', 'location_td' );load_drop_down( 'requires/finishing_production_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>                            
                        </td>
                        ...
                        <td id="machine_td"> 
                            <?
                                echo create_drop_down( "txt_mc_no", 100, $blank_array,"", 1, "--Select Machine--", 0, "",0 );
                            ?>
                        </td>
