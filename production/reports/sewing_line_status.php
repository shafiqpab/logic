...
<script type="text/javascript"> 
    ...
    $floorArr = return_library_array("select id,floor_name from lib_prod_floor where status_active=1","id","floor_name"); 
    ...
    foreach ($sql as $val) 
    {
        ...
        {
            $line_name .= ( $line_name=="") ?  $lineArr[$val['LINE_NUMBER']] : ",".$lineArr[$val['LINE_NUMBER']];
        }
...
<script>
    ...
    {   
        ...
        {*/    
            ...
            if(cbo_line_id!=undefined)
            {
                ...
                {
                    $("#line_name").text(" Line : "+line_name);
                }
                else
                {
                    $("#line_name").text(" Line : All Line");
                }
            }
            else
            {
            ...
            var data="action=report_generate&cbo_company_name="+cbo_company_name+"&txt_date="+txt_sewing_date+"&hidden_line_id="+cbo_line_id+"&cbo_floor_id="+floorIds+"&page_width="+page_width+"&page_height="+page_height;
            // freeze_window(3);
            http.open("POST","requires/sewing_line_status_controller.php",true);
    ...
    function get_sewing_line()
    {
        ...
        // alert(floor_ids.join(", "));
        var floorIds = floor_ids.join(",");
        load_drop_down( 'requires/sewing_line_status_controller',floorIds+'_'+document.getElementById('cbo_company_id').value+'_'+document.getElementById('txt_sewing_date').value, 'load_drop_down_sewing_line_floor', 'sewing_line' );
        get_php_form_data( floorIds, 'eval_multi_select', 'requires/sewing_line_status_controller' );
    }
    ...
    function openmypage_line() // For Line
    {
        ...
        var page_link='requires/sewing_line_status_controller.php?action=line_search_popup&wo_company_name='+wo_company_name+'&floor_name='+floorIds+'&date_from='+date_from;  
                ...
                <div style="padding: 0; margin:0;">
                    <div style="padding: 0; margin:0;"> 
                        <div style="font-size:24px;color:#FFFFFF;background:#000000;width:100%;z-index:-1;">
                            <marquee behavior="alternate" width="100%" direction="right" scrollamount="2" onmouseover="this.stop();" onmouseout="this.start();">
                                <!-- <div style="font-size:24px;color:#FFFFFF;text-align: center;">Hourly Sewing Production</div> -->
                                <div style="text-align: center;" >
                                    <span id="company_name" style="color: #ffffff;font-size:20px;">Company : ,</span>
                                    <span id="floor_name" style="color: #ffffff;font-size:20px;">Unit : All Floor,</span>
                                    <span id="line_name" style="color: #ffffff;font-size:20px;">, Line : All Line,</span>
                                    <span id="prod_date" style="color: #ffffff;font-size:20px;"></span> 
                                </div>
                            </marquee>
                        </div>
