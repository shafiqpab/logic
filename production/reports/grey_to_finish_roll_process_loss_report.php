...
var tableFilters = 
{
	//col_0: "none",
	col_operation: {
		id: ["value_total_grey","value_total_process","value_total_finish"],
   		col: [18,19,20],
...
function fn_report_generated(type)
{
	...
	var location_id=document.getElementById('cbo_lc_location_name').value;
	...
	var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_lc_location_name*cbo_buyer_name*txt_job_no*txt_job_id*txt_order_no*txt_batch_no*txt_barcode_no*txt_date_from*txt_date_to',"../../")+'&report_title='+report_title;
	freeze_window(3);
	http.open("POST","requires/grey_to_finish_roll_process_loss_report_controller.php",true);
...
function openmypage_batch()
{
	...
	var page_link='requires/grey_to_finish_roll_process_loss_report_controller.php?action=batch_no_search_popup&companyID='+companyID+'&buyer_name='+buyer_name;
...
function openmypage_job()
{
	...
	//alert(cbo_year_id);
	var page_link='requires/grey_to_finish_roll_process_loss_report_controller.php?action=job_no_popup&companyID='+companyID+'&buyer_name='+buyer_name;
...
{ //alert(des_prod)
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/grey_to_finish_roll_process_loss_report_controller.php?companyID='+companyID+'&po_id='+po_id+'&prog_no='+prog_no+'&booking_no='+booking_no+'&prog_no='+prog_no+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../');
}
...
function openmypage_booking()
{
	...
	var cbo_year_id = $("#cbo_year").val();
	//var cbo_month_id = $("#cbo_month").val();
	var page_link='requires/grey_to_finish_roll_process_loss_report_controller.php?action=booking_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id;
             ...
             <table class="rpt_table" width="1090px" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr align="center">
						<td> 
                            <?
                                echo create_drop_down( "cbo_company_name", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "- Select Company -", $selected, "load_drop_down( 'requires/grey_to_finish_roll_process_loss_report_controller', this.value, 'load_drop_down_location', 'cbo_lc_location_td' );load_drop_down( 'requires/grey_to_finish_roll_process_loss_report_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>
                        </td>
						...
						<td>
                            <input type="text" name="txt_barcode_no" id="txt_barcode_no" class="text_boxes" style="width:70px" placeholder="Write" >
                        </td>                      
                        <td align="center">
