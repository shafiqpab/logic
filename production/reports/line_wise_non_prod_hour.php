...
{
    if (form_validation('cbo_company_name*txt_date_from','Company*Date')==false)
    {
        return;
    }
    else
    {
        ...
        var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_location*cbo_floor*cbo_line*hidden_color_id*hidden_po_id*txt_date_from',"../../");
        freeze_window(3);
        http.open("POST","requires/line_wise_non_prod_hour_controller.php",true);
...
<form id="lineWiseProductionReport_1">
    <div style="width:740px; margin:auto" align="center">    
            ...
            <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" align="center">
               <thead>                    
                ...
                <tr >
                    <td width="100"> 
                        <?
                            echo create_drop_down( "cbo_company_name", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, " load_drop_down( 'requires/line_wise_non_prod_hour_controller', this.value, 'load_drop_down_location', 'location_td' );" );
                        ?>
                    ...
                    <td width="100" id="location_td">
                        <? 
                            echo create_drop_down( "cbo_location", 100, $blank_array,"", 1, "-- Select --", $selected, " load_drop_down( 'requires/line_wise_non_prod_hour_controller', this.value, 'load_drop_down_floor', 'floor_td' );", 1, "" );
                        ?>
                    </td>
                    ...
                    <input type="text" name="txt_date_from" id="txt_date_from" value="" class="datepicker" style="width:85px">                                             
             
                    <input type="hidden" name="txt_date_to" id="txt_date_to" value="" class="datepicker" style="width:55px"/>
                    </td>
