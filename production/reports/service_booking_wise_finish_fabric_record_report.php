...
function fn_report_generated(operation)
{
	...
	if (booking_no=="" && order_no=="" && job_no=="")
	{
		...
		if( form_validation('cbo_company_name*cbo_process','Company Name*Process')==false )
		{
			return;
		}
	}
	freeze_window(5);
    var data="action=report_generated&operation="+operation+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_supplier_name*cbo_service_source*cbo_process*txt_hide_booking_id*txt_booking_no*txt_job_no*order_no*hidden_order_id*cbo_year_selection*txt_date_from*txt_date_to',"../../");
	//alert(data);
...
function openmypage_job(id)
{
	...
	var page_link="requires/service_booking_wise_finish_fabric_record_report_controller.php?action=job_no_popup&company_name="+company_name+"&buyer_name="+buyer_name+"&year="+year;
	var title="Job/Style No Popup";
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=400px,height=420px,center=1,resize=0,scrolling=0','../')
	emailwindow.onclose=function()
	{
		var theemail=this.contentDoc.getElementById("selected_id").value;
		var theemail=theemail.split("__");
		// document.getElementById('hidden_style_no').value=theemail[0];
		document.getElementById('txt_job_no').value=theemail[1];
		release_freezing();
	}
}
...
function openmypage_qnty(booking_no,poID,action,fabricDescId,bodyPartId,diaWidth,gmts_color,dtlsId,from_date,to_date,processId,preCsotFabricCsot)
{
	var companyID = $("#cbo_company_name").val();
                ...
                <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <?
                                        echo create_drop_down( "cbo_process", 150, $conversion_cost_head_array,'', 1, '--Select Process--', 0,"",'','1,35,31,193','','','',3);
                                    ?>
                                </td>
                               	<td>
									<?
                                ...
                                <td>
                                     <input type="text"  name="txt_job_no" id="txt_job_no" class="text_boxes" style="width:100px;" tabindex="1" placeholder="Write/Browse" onDblClick="openmypage_job()">
