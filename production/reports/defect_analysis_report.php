...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Production QC Report", "../../", 1, 1,$unicode,1,1);
...
<script>
	...
	{		
		...
		else
		{
			...
			var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_working_company_id*cbo_production_type*cbo_buyer_name*cbo_location*cbo_floor*txt_job_no*txt_int_ref*txt_date_from*txt_date_to*cbo_line_id',"../../")+"&report_title="+report_title+"&type="+type+"&datediff="+datediff;
			freeze_window(3);
			http.open("POST","requires/defect_analysis_report_controller.php",true);
	...
	function fn_floor_multiselect()
	{
		set_multiselect('cbo_floor','0','0','','0'); 
		setTimeout[($("#floor_td a").attr("onclick","getLineDropDown();") ,3000)];
	}
	...
	{			 
	    ...
	    var cbo_floor = document.getElementById('cbo_floor').value;

	    if(cbo_floor !='') 
		{
	      	var data="action=load_drop_down_line&data="+cbo_working_company_id+'_'+cbo_location+'_'+cbo_floor;
	      	http.open("POST","requires/defect_analysis_report_controller.php",true);
	      	http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	      	http.send(data); 
...
<form id="sewingQcReport_1">
               ...
               <thead>                    
                ...
                <tr class="general">
                    <td width="120" align="center"> 
                        <?
                            echo create_drop_down( "cbo_company_name", 120, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/defect_analysis_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );fn_disable_com(1,this.value)" );
                        ?>
                    </td>
                    <td width="120" align="center"> 
                        <?
                            echo create_drop_down( "cbo_working_company_id", 120, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/defect_analysis_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/defect_analysis_report_controller', this.value+'_'+$('#cbo_working_company_id').val(), 'load_drop_down_location', 'location_td' );fn_disable_com(2,this.value)" );
                        ?>
                    </td>
                    ...
                    <td width="70" id="floor_td" align="center">
                    	<? 
                            echo create_drop_down( "cbo_floor", 100, $blank_array,"", 1, "-- Select --", $selected, "",0,"" );
                        ?>
                    </td>
                    <td width="90" align="center">
                        <? 
                            echo create_drop_down( "cbo_production_type", 90, $production_process,"", 1, "--Select Type--", $selected, "load_drop_down( 'requires/defect_analysis_report_controller', this.value+'_'+$('#cbo_working_company_id').val(), 'load_drop_down_location', 'location_td' );","","5" );
                        ?>
                    </td>
					<td id="line_td">
                        <? 
                            echo create_drop_down( "cbo_line_id", 70, $blank_array,"", 1, "-- Select Line --",  $selected, "",0,"" );
                        ?>                            
                    </td>
    ...
    <div id="report_container2" align="center"></div>
     ...
     <div id="paretoChartContainer" style="height: 370px; width: 50%;float:left;"></div>
