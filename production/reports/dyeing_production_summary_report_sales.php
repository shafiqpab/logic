...
/* -------------------------------------------- Comments
  ...
  Purpose			: 	This form will Create Dyeing Production Summary Report Sales
  Functionality	:
  JS Functions	:
  Created by		:	
  Creation date 	: 	
...
<script>
    ...
    function fn_dyeing_report_generated(operation)
    {
		...
		if (operation == 2)
        {
            var data = "action=generate_report2&operation=" + operation + get_submitted_data_string('cbo_company_name*cbo_buyer_name*job_number_show*job_number*batch_number*batch_number_show*fso_no*hidden_fso_no*booking_no*hidden_booking_no*cbo_type*cbo_year*txt_date_from*txt_date_to*cbo_result_name*cbo_batch_type*txt_machine_name*txt_machine_id*cbo_floor_id*cbo_shift_name*cbo_group_by*search_type*cbo_prod_type*cbo_party_id*cbo_working_company_name', "../../");
        } 
				
        http.open("POST", "requires/dyeing_production_summary_report_sales_controller.php", true);
    ...
    function batchnumber()
    {
        ...
        var batch_type = document.getElementById('cbo_batch_type').value;
        var page_link='requires/dyeing_production_summary_report_sales_controller.php?action=batch_no_search_popup&companyID='+companyID+'&buyer_name='+buyer_name + "&batch_type=" + batch_type;
    ...
    {
        if (form_validation('cbo_batch_type*cbo_company_name', 'Batch Type*Company') == false)
        {
            return;
        }
        ...
        var page_link = "requires/dyeing_production_summary_report_sales_controller.php?action=jobnumbershow&company_id=" + company_name + "&cbo_buyer_name=" + cbo_buyer_name + "&year=" + year + "&batch_type=" + batch_type;
    ...
    function openmypage_fso(id)
    {
        ...
        var batch_type = document.getElementById('cbo_batch_type').value;
        var year = document.getElementById('cbo_year').value;
        var page_link = "requires/dyeing_production_summary_report_sales_controller.php?action=FSO_No_popup&company_name=" + company_name + "&buyer_name=" + buyer_name + "&year=" + year + "&batch_type=" + batch_type;
    ...
    function openmypage_booking()
    {
        ...
        var page_link='requires/dyeing_production_summary_report_sales_controller.php?action=booking_no_popup&companyID='+companyID+ '&cbo_year='+cbo_year;
    ...
    function openmypage_color(id)
    {
        ...
        var year = document.getElementById('cbo_year').value;
        var page_link = "requires/dyeing_production_summary_report_sales_controller.php?action=check_color_id&txtcolor=" + txtcolor;
    ...
    function openmypage_machine()
    {
        ...
        emailwindow = dhtmlmodal.open('EmailBox', 'iframe', 'requires/dyeing_production_summary_report_sales_controller.php?action=machine_no_popup&data=' + data, 'Machine Name Popup', 'width=470px,height=420px,center=1,resize=0', '../')
    ...
    function fabric_dtls_poup(batch_id, dia_type, order_id, action)
    {
        ...
        //alert(batch_id);
        emailwindow = dhtmlmodal.open('EmailBox', 'iframe', 'requires/dyeing_production_summary_report_sales_controller.php?company_name=' + company_name + '&batch_id=' + batch_id + '&dia_type=' + dia_type + '&order_id=' + order_id + '&action=' + action, 'Details Veiw', 'width=' + popup_width + ', height=350px,center=1,resize=0,scrolling=0', '../');
    }
                    ...
                    <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr>
                                <td>
                                    <?
                                    $search_by_arr = array(2 => "Dyeing Production Report");
                                    echo create_drop_down("cbo_type", 80, $search_by_arr, "", 0, "", 2, '', 0);
                                    ?>
                                </td>
                                <td>
                                    <?
                                    $batch_type_arr = array(1 => "Self Batch", 2 => "Inbound subcontract");
                                    echo create_drop_down("cbo_batch_type", 70, $batch_type_arr, "", 1, "--All--", 0, "load_drop_down('requires/dyeing_production_summary_report_sales_controller',document.getElementById('cbo_working_company_name').value+'_'+this.value, 'load_drop_down_buyer', 'cbo_buyer_name_td' );", 0);
                                    ?>
                                </td>
                                 
                                 <td> 
                                    <?
                                echo create_drop_down("cbo_working_company_name", 100, "select id,company_name from lib_company where status_active=1 and is_deleted=0  order by company_name", "id,company_name", 1, "-- Select Working Company --", 0, "load_drop_down('requires/dyeing_production_summary_report_sales_controller', this.value+'_'+ document.getElementById('cbo_batch_type').value , 'load_drop_down_buyer' ,'cbo_buyer_name_td' );load_drop_down( 'requires/dyeing_production_summary_report_sales_controller', this.value, 'load_drop_down_floor', 'floor_td' );");
                                    ?>
                                </td>
                                ...
                                <td id="">
                                    <?
									 echo create_drop_down("cbo_prod_type",70,$knitting_source,"", 1, "-- Select Source --", 1,"load_drop_down( 'requires/dyeing_production_summary_report_sales_controller',this.value,'load_drop_down_knitting_com','cbo_party_td' );",0,'1,3');
                                    ?>
                                </td>
                                <td id="cbo_party_td">
                                    <?
                                ...
                                <td>
                                    <input type="button" id="show_button" class="formbutton" style="width:50px" value="Summary" onClick="fn_dyeing_report_generated(2)" />
