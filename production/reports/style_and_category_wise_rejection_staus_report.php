<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Style and Category wise Rejection Staus Report
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Style and Category wise Rejection Staus Report","../../", 1, 1, $unicode,1); 
?>	
<script>
...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../logout.php";
	...
	function generate_report(type)
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_working_company_id*cbo_location_id*cbo_floor_id*cbo_line_id*cbo_lc_company_id*cbo_buyer_id*txt_search_by*cbo_job_year*cbo_production_type*txt_date_from*txt_date_to*txt_order_id',"../../")+'&report_title='+report_title+'&type='+type;
		
		//alert(data); return;
		
		freeze_window(3);
		http.open("POST","requires/style_and_category_wise_rejection_staus_report_controller.php",true);
	...
	{		
		var page_link='requires/style_and_category_wise_rejection_staus_report_controller.php?action=open_prod_popup&data='+param;  
	...
	{		
		var page_link='requires/style_and_category_wise_rejection_staus_report_controller.php?action=open_prod_popup2&data='+param;  
		var title="Production Popup";
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=620px,height=370px,center=1,resize=0,scrolling=0','../')
	}
	...
	{		
		var page_link='requires/style_and_category_wise_rejection_staus_report_controller.php?action=open_prod_popup_wip&data='+param;  
	...
	{		
		var page_link='requires/style_and_category_wise_rejection_staus_report_controller.php?action=open_prod_popup_wip2&data='+param;  
		var title="Production WIP Popup";
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=620px,height=370px,center=1,resize=0,scrolling=0','../')
	}
	...
	{
		if( form_validation('cbo_lc_company_id','LC Company Name')==false )
		{
			return;
		}
		...
		var page_link='requires/style_and_category_wise_rejection_staus_report_controller.php?action=search_by_action&w_company='+w_company+'&lc_company='+lc_company+'&buyer='+buyer+'&job_year='+job_year;  
	...
	function openmypage_color() // For Line number
	{
		...
		var page_link='requires/style_and_category_wise_rejection_staus_report_controller.php?action=color_popup&txt_search_by='+txt_search_by+'&txt_order_id='+txt_order_id;  
	    ...
	    if(working_company_id !='') {
		  ...
		  http.onreadystatechange = function(){
	          if(http.readyState == 4) 
	          {
	              ...
	              //========================
	              // load_drop_down( 'requires/style_and_category_wise_rejection_staus_report_controller', working_company_id, 'load_drop_down_buyer', 'buyer_td' );
	          }			 
	      };
	    }         
	}
	...
	function getFloorId() 
	{
	    ...
	    if(working_company_id !='') 
	    {
		  	var data="action=load_drop_down_line&data="+working_company_id+'_'+location_id+'_'+floor_id+'_'+date_from+'_'+date_to;
		  	//alert(data);die;
		  	http.open("POST","requires/style_and_category_wise_rejection_staus_report_controller.php",true);
		  	http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		  	http.send(data); 
                ...
                <tbody>
                    <tr class="general">
                       <td id="working_company_td">
                        ...
                        <td id="line_td">
                            <? 
                                echo create_drop_down( "cbo_line_id", 120, $blank_array,"", "", "-- Select Line --", "", "" );
                            ?> 
                        </td>
                        ...
                        <td>
                         <input type="text" id="txt_search_by"  name="txt_search_by"  style="width:100px" class="text_boxes" placeholder="Browse" ondblclick="openmypage_searchby()" readonly="" />
                         <input type="hidden" name="txt_order_id" id="txt_order_id" value="">
                        </td>
                        <td>
                        <?
							echo create_drop_down( "cbo_production_type", 115, $production_type,"", 1, "-- Select --", 1, "",0,'5,11','','','0,1' );
						?>                        	
                        </td>                         
                        <td>
                        ...
                        <td>
                            ...
                            <input type="button" name="search2" id="search2" value="Summary" onClick="generate_report(2)" style="width:70px" class="formbutton" />
                        </td>
