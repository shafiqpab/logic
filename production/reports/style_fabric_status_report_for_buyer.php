...
<script>
	...
	function fn_report_generated(type)
	{
		...
		else
		{
			...
			// else if(type==4)
			// {
			// 	var data="action=report_generate_fabric_wise"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_date_type*txt_date_from*txt_date_to*cbo_year*txt_job_no*txt_booking_no*cbo_type*txt_search_string*cbo_order_status*cbo_discrepancy*cbo_active_status',"../../")+'&type='+type + '&chk_no_boking=' + chk_no_boking;
			// }
			//if(type==2)
			//{
				var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_date_type*txt_date_from*txt_date_to*cbo_year*txt_job_no*txt_booking_no*cbo_type*txt_search_string*cbo_order_status*cbo_discrepancy*cbo_active_status',"../../")+'&type='+type + '&chk_no_boking=' + chk_no_boking;
			...
			freeze_window(3);
			http.open("POST","requires/style_fabric_status_report_for_buyer_controller.php",true);
	...
	function open_febric_receive_status_order_wise_popup(order_id,type,color)
	{
		...
		else popup_width='960px';
		
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/style_fabric_status_report_for_buyer_controller.php?order_id='+order_id+'&action='+type+'&color='+color, 'Detail Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../');
	}

	function openmypage(order_id,type,yarn_count,yarn_comp_type1st,yarn_comp_percent1st,yarn_comp_type2nd,yarn_comp_percent2nd,yarn_type,alocate_type)
	{
		var popup_width='';
		if(type=="yarn_issue_not") popup_width='1000px'; else popup_width='890px';
		
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/style_fabric_status_report_for_buyer_controller.php?order_id='+order_id+'&action='+type+'&yarn_count='+yarn_count+'&yarn_comp_type1st='+yarn_comp_type1st+'&yarn_comp_percent1st='+yarn_comp_percent1st+'&yarn_comp_type2nd='+yarn_comp_type2nd+'&yarn_comp_percent2nd='+yarn_comp_percent2nd+'&yarn_type_id='+yarn_type+'&alocate_type='+alocate_type, 'Detail Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../');
	}

	function generate_worder_report(type,booking_no,company_id,order_id,fabric_nature,fabric_source,job_no,approved,action,page,entryForm)
	{
		var show_yarn_rate='';
		...
		else if(action=="show_fabric_booking_report_jk")
		{
	...
	{  
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/style_fabric_status_report_for_buyer_controller.php?po_id='+po_id+'&start_date='+start_date+'&end_date='+end_date+'&buyer_id='+buyer_id+'&job_no='+job_no+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../');
	}
	...
	function openmypage_booking()
    {
        ...
        var page_link='requires/style_fabric_status_report_for_buyer_controller.php?action='+action+'&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year='+cbo_year;
	...
	function print_report_button_setting(report_ids) 
    {
        ...
        $('#summary_button').hide();
        $('#order_wise_button').hide();
        $('#style_wise_button').hide();
        $('#fabric_wise_button').hide();
        ...
        report_id.forEach(function(items)
        {
            ...
            else if(items==724){$('#fabric_wise_button').show();}
        });
    }
</script>
             ...
             <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr class="general">
                        <td> 
                            <?
                                echo create_drop_down( "cbo_company_name", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name","id,company_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/style_fabric_status_report_for_buyer_controller',this.value, 'load_drop_down_buyer', 'buyer_td' ); get_php_form_data(this.value,'load_variable_settings','requires/style_fabric_status_report_for_buyer_controller');get_php_form_data(this.value,'print_button_variable_setting','requires/style_fabric_status_report_for_buyer_controller' );" );
                            ?>
                        </td>
                        <td id="buyer_td">
                            <? 
                    ...
                    <tr class="general">
                        ...
                        <td>
                            <!-- <input type="button" id="summary_button" class="formbutton" style="width:85px;display: none;" value="Summary" onClick="fn_report_generated(3)" /> -->
                        </td>
                    	<td>
                            <!-- <input type="button" id="style_wise_button" class="formbutton" style="width:85px;display: none;" value="Style Wise" onClick="fn_report_generated(2)" /> -->
                        </td>
                        <td>
                            <!-- <input type="button" id="fabric_wise_button" class="formbutton" style="width:70px;display: none;" value="Fabric Wise" onClick="fn_report_generated(4)" /> -->
                        </td>
                    </tr>
                </tbody>
            </table>
