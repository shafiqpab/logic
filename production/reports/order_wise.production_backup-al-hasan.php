	...
	<div align="center" style="width:100%;" >
		<form name="searchpofrm"  id="searchpofrm">
			<fieldset style="width:1120px">
				<table width="1100" cellspacing="0" cellpadding="0" class="rpt_table" border="1" rules="all">
					<thead>
						<tr>
							<th colspan="9">
...
if($action=="create_po_search_list_view")
{
	...
	if($action_types=='order_select_popup')
	{
		...
		//echo $sql."<br>";
		if($cbo_export_item_category!=10 && $cbo_lc_for!=2){ $tbl_width=1220;}
		else{$tbl_width=1170;}
	 	?>
        <div>
...
if ($action == "sales_contact_lien_letter12") {
	...
	local_comn,claim_adjustment, $date_diff_cond as date_diff from com_sales_contract where id=$system_id");
	...
	<div style="width:900px;height:50px;">
		<h1 style="text-decoration:underline;text-align:center">SALES CONTRACT</h1>
	</div>
	<div style="width:900px;">
		<div style="width:900px;display:flex;">
			<div style="width:400px;border:1px solid black; font-size: 13px; ">
				<div style="width:360px;height:130px;border-bottom:1px solid black;border-right:1px solid black; font-size: 14px;">
		...
		<div style=" width: 900px;height:30px ;border:1px solid black;display:flex;font-size:14px;border-bottom:none">
			...
			if($is_sales==0)
			{
				$sql = "SELECT wb.id, ci.id as idd, wm.gmts_item_id, wb.po_number, wb.po_total_price, wb.po_quantity, wb.shipment_date as shipment_date, wb.job_no_mst,wm.product_dept, wm.style_ref_no, wm.order_uom, wm.total_set_qnty as ratio, ci.attached_qnty, ci.attached_rate, ci.attached_value, ci.status_active,wm.STYLE_DESCRIPTION, wb.po_number, sb.COLOR_NUMBER_ID,sb.ITEM_NUMBER_ID,sb.ARTICLE_NUMBER, sum(sb.ORDER_QUANTITY) as ORDER_QUANTITY
				from wo_po_break_down wb, wo_po_details_master wm, com_sales_contract_order_info ci, wo_po_color_size_breakdown sb
				where wb.job_id = wm.id and   wb.id = sb.PO_BREAK_DOWN_ID and sb.po_break_down_id=ci.wo_po_break_down_id and ci.com_sales_contract_id='$sales_contract_id' and ci.status_active = '1' and ci.is_deleted = '0' group by wb.id, ci.id, wm.gmts_item_id, wb.po_number, wb.po_total_price, wb.po_quantity, wb.shipment_date, wb.job_no_mst,wm.product_dept, wm.style_ref_no, wm.order_uom, wm.total_set_qnty, ci.attached_qnty, ci.attached_rate, ci.attached_value, ci.status_active,wm.STYLE_DESCRIPTION, wb.po_number, sb.COLOR_NUMBER_ID,sb.ITEM_NUMBER_ID,sb.ARTICLE_NUMBER order by ci.id";
			}
			else
			{
				$sql = "SELECT wm.id, ci.id as idd, 0 as gmts_item_id, wm.job_no as po_number, sum(wb.amount) as po_total_price, sum(wb.finish_qty) as po_quantity, wm.delivery_date as shipment_date, wb.job_no_mst, wm.style_ref_no, wb.order_uom, 1 as ratio, ci.attached_qnty, ci.attached_rate, ci.attached_value, ci.status_active 
		...
		<table id="mstDiv" border="1" style="width: 900px;border-collapse:collapse">
			<thead>
				<tr>
			...
			<tbody>
				...
				foreach ($nameArray as $row) {
					...
					?>
					<tr>
						<td width="90"style="word-wrap: break-word;word-break: break-all; "><?= change_date_format($row["SHIPMENT_DATE"]) ?></td>
						...
						<td width="90"><?
							/*  if($row["ORDER_UOM"]== "58")
							{
								echo ($row["ATTACHED_QNTY"]*2);
							}
							elseif($row["ORDER_UOM"]== "57")
							{
								//echo ($row["ATTACHED_QNTY"]*2);
							}
							else{
								echo  $row["ATTACHED_QNTY"];
							} */
							echo  $row["ATTACHED_QNTY"];
							?>
						</td>
						<td width="45" ><?= $unit_of_measurement[$row["ORDER_UOM"]] ?></td>
						<td width="45" ><?= $currency_sign . " " . $row["ATTACHED_RATE"] ?></td>
						<td width="90" ><?= $currency_sign . " " . $row["ATTACHED_VALUE"] ?></td>
					</tr>
					<?
					if ($counter % 13 == 0) {
						?>
					...
					<div>
						<p align="center">Cont Pg -<? echo $i++;?></p> 
						<h1 style="text-decoration:underline;text-align:center">SALES CONTRACT</h1>
						<p align="center">page:<? echo $page++;?></p>
					</div>

					<table border="1" style="width: 900px;border-collapse:collapse">
						<thead>
							<tr>
								<th colspan="2" align="left" width="180"> <strong>SALES CONTACT NO.:</strong>
								<br>
								<? echo $contract_no;?> &nbsp;&nbsp; &nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;REV
								</th>
								<th colspan="6"></th>
								<th colspan="2"><strong> DATE :</strong> <? echo $contract_date;?></th>
							</tr>
							<tr></tr>
							<tr></tr>
						</thead>
						<?
							...
							if($row["ORDER_UOM"]== "58")
							{
								$total_Attached_Quantity += $row["ATTACHED_QNTY"]*2;
							}
							else{
								$total_Attached_Quantity +=$row["ATTACHED_QNTY"];
							}
							$total_Attached_Value += $row["ATTACHED_VALUE"];
						}
					?>
			</tbody>
			<tfoot>
				...
				<tr>
					...
					<td colspan="4" style="text-align:left;">Less,&nbsp;<?=$foreign_comn?>% &nbsp;BONUS</td>
					<td align="right">
					<? 
				...
				<tr>
					<td  colspan="4"  style="text-align: left;">Less,&nbsp;<?=$local_comn?>% &nbsp;PFR</td>
					<td align="right">
					<? 
					...
					<tr>
					<td colspan="4" style="text-align: left;">Less,&nbsp;<?=$claim_adjustment?>% &nbsp;DFR</td>
					<td align="right">
				...
				<tr>
					<td colspan="10" align="left"><strong> TOTAL VALUE IN WORDS:</strong><?echo  number_to_words($total_Attached_Value-($bonus+$dfr_value+$pfr_value));	?></td>
				</tr>
			</tfoot>
		</table>
	</div>
