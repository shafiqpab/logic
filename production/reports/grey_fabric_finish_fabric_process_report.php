...
function fn_report_generated(operation)
{
	...
	{			
		if( form_validation('cbo_company_name*cbo_process*txt_date_from*txt_date_to','Company Name*Process*Form Date*To Date')==false )
		{
			return;
		}
	}
	...
	if(operation==1)
	{
		var action = "report_generated";
	}
	else if(operation==2)
	{
		var action = "report_generated_2";
	}
	
    var data="action="+action+"&operation="+operation+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_supplier_name*cbo_service_source*cbo_process*txt_hide_booking_id*txt_booking_no*order_no*hidden_order_id*cbo_year_selection*txt_date_from*txt_date_to',"../../");
	//alert(data);
		http.open("POST","requires/grey_fabric_finish_fabric_process_report_controller.php",true);
...
function openmypage_booking()
{
    ...
    var page_link='requires/grey_fabric_finish_fabric_process_report_controller.php?action=booking_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_selection_id='+cbo_year_selection_id;
...
{ 
	...
	var page_link="requires/grey_fabric_finish_fabric_process_report_controller.php?action=order_number_popup&company_name="+company_name+"&buyer_name="+buyer_name+"&year="+year;
...
<body onLoad="set_hotkey();">
	...
	<? echo load_freeze_divs ("../../",''); ?>
         ...
         <h3 style="width:1190px; margin-top:10px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu(this.id,'content_search_panel','')">-Search Panel</h3> 
                ...
                <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr>
                                <td> 
                                    <?
                                    echo create_drop_down( "cbo_company_name", 150, "select id,company_name from lib_company where status_active=1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select Company --", 0, "load_drop_down('requires/grey_fabric_finish_fabric_process_report_controller',this.value, 'load_drop_down_buyer','cbo_buyer_name_td' );" );
                                    ?>
                                </td>
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_process", 150, $conversion_cost_head_array,'', 1, '--Select Process--', 0,"",'','','','','',3);
                                    ?>
                                </td>
                               	<td>
									<?
                                	echo create_drop_down( "cbo_service_source", 120, $knitting_source, "", 1, "-- Select --", $selected, "load_drop_down( 'requires/grey_fabric_finish_fabric_process_report_controller',this.value+'**'+$('#cbo_company_name').val(),'load_drop_down_knitting_com','supplier_td' );check_supplier_td(this.value);","","1,3" );
                            	?>
                      			</td>
                                ...
                                <td>
									...
									<input type="button" id="show_button" class="formbutton" style="width:50px" value="Show 2" onClick="fn_report_generated(2)" align="right"/>
								</td>
                            </tr>
                        </tbody>
                    </table>
