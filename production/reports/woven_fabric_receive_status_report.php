<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Woven Fabric Receive Status Report.
Functionality	:	
JS Functions	:
Created by		:	Kausar
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Woven Fabric Receive Status Report", "../../", 1, 1,'',1,1);
...
function fn_report_generated(type)
{
	...
	else
	{
		if(type==1)
		{
			var data="action=report_generate"+get_submitted_data_string('cbo_type*cbo_company_name*cbo_buyer_name*txt_search_string*txt_date_from*txt_date_to*txt_date_from_po*txt_date_to_po',"../../");
		}
		else
		{
			var data="action=report_generate"+get_submitted_data_string('cbo_type*cbo_company_name*cbo_buyer_name*txt_search_string*txt_date_from*txt_date_to*txt_date_from_po*txt_date_to_po*txt_style_number*cbo_shipping_status*txt_fab_color',"../../");
		}
		
		freeze_window(3);
		http.open("POST","requires/woven_fabric_receive_status_report_controller.php",true);
		...
		if(tot_rows>1)
		{
			...
			if(type==1)
			{
			 ...
			 // display_all_text: " ---Show All---",
										col_operation: {
										id: ["tot_order_qnty","value_tot_budget","value_tot_booking","value_tot_fin_rec","value_tot_fini_receive","value_tot_fini_balance","value_tot_fini_cut_issue"],
									   col: [6,12,15,17,18,19,20],
...
function search_by(val)
{
	...
	if(val==1)
	{
		$('#search_by_td_up').html('Write Order No');
	}
	else
	{
		$('#search_by_td_up').html('Write Style');
	}
}
...
{
	var popup_width='';
	if(type=="fabric_receive" || type=="grey_issue" || type=="dye_qnty") 
	{
		popup_width='900px';
	}
	else popup_width='700px';
	
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/woven_fabric_receive_status_report_controller.php?order_id='+order_id+'&action='+type+'&color='+color, 'Detail Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../');
}
         ...
         <fieldset style="width:1050px;">
             <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
             	<thead>
                    ...
                    <th>Type</th>
                    <th id="search_by_td_up">Write Order No</th>
                    <th><input type="reset" name="res" id="res" value="Reset" onClick="reset_form('fabricReceiveStatusReport_1','report_container*report_container2','','','')" class="formbutton" style="width:100px" /></th>
                </thead>
                <tbody>
                    <tr class="general">
                        <td> 
                            <?
                                echo create_drop_down( "cbo_company_name", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/woven_fabric_receive_status_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>
                        </td>
                        <td id="buyer_td">
                            <? 
                        ...
                        <td>
                            <input name="txt_date_from_po" id="txt_date_from_po" class="datepicker" style="width:80px" placeholder="From Date" readonly>To
                        </td> 
                        <td>
                            <input name="txt_date_to_po" id="txt_date_to_po" class="datepicker" style="width:80px"  placeholder="To Date" readonly>
                        </td>
                        <td>
                        	<?
								$search_by_arr=array(1=>"Show Order Wise",2=>"Show Style Wise");
								echo create_drop_down( "cbo_type", 130, $search_by_arr,"",0, "", "",'search_by(this.value)',0 );
							?>
                        </td> 
                        <td id="search_by_td"><input type="text" name="txt_search_string" id="txt_search_string" class="text_boxes" style="width:90px" /></td>
                        <td>
