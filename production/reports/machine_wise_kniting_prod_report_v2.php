...
var permission = '<? echo $permission; ?>';	
	...
	function openmypage_machine()
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/machine_wise_kniting_prod_report_v2_controller.php?action=machine_no_popup&data='+data,'Machine Name Popup', 'width=470px,height=420px,center=1,resize=0','../')
	...
	function fn_report_generated(operation)
	{
		...
		var report_title=$( "div.form_caption" ).html();
			...
			var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_location_id*cbo_floor_id*txt_date_from*txt_date_to*txt_machine_name*txt_machine_id*cbo_lbuyer_id',"../../")+'&report_title='+report_title+'&datediff='+datediff;
		
		
		
		freeze_window(3);
		http.open("POST","requires/machine_wise_kniting_prod_report_v2_controller.php",true);
	...
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/machine_wise_kniting_prod_report_v2_controller.php?machine_id='+machine_id+'&date='+date+'&action='+action, 'Cause of Machine Idle', 'width=600px,height=450px,center=1,resize=0,scrolling=0','../');
	}
	...
	{	 
		...
		if(splidData[1]==null){
			load_drop_down('requires/machine_wise_kniting_prod_report_v2_controller', company_id, 'load_drop_down_buyer', 'buyer_td' );
			load_drop_down('requires/machine_wise_kniting_prod_report_v2_controller', company_id, 'load_drop_down_location', 'location_td' );
			load_drop_down( 'requires/machine_wise_kniting_prod_report_v2_controller', company_id, 'load_drop_down_floor', 'floor_td' );
		}
		else
		{
			document.getElementById("cbo_lbuyer_id").disabled=true;	
			document.getElementById("cbo_location_id").disabled=true;	
			document.getElementById("cbo_floor_id").disabled=true;	
		}
...
<div style="width:100%;" align="center">
    <? echo load_freeze_divs ("../../",$permission);  ?>    		 
        <form name="machinewiseproduction_1" id="machinewiseproduction_1" autocomplete="off" > 
         <h3 style="width:980px; margin-top:20px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> - Search Panel</h3> 
            ...
            <table class="rpt_table" width="100%" cellpadding="1" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr class="general">
                        ...
                        <td id="buyer_td">
							<? 
								echo create_drop_down( "cbo_lbuyer_id", 100, $blank_array,"", 1, "-- Select --", $selected, "",1,"" );
                            ?>
                        </td>
                        <td id="location_td">
							<? 
                        ...
                        <td align="center">
                            <input type="button" id="show_button_2" class="formbutton" style="width:70px" value="Show" onClick="fn_report_generated(2)" />
                        </td>
                    </tr>
                </tbody>
                <tr>
