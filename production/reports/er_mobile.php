...
<script>
	...
	{		
		...
		var page_link='er_popup.php?action=details_popup&search_string='+search_string;
		var title='Line Wise Details';

		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=280px,height=300px,center=1,resize=1,scrolling=0','../');
...
ob_start();
?> 
<fieldset style="width:99%;margin:0 auto;">
	<div style="width:100%;"> 
		<table width="100%" cellspacing="0" style="margin: 20px 0"> 
		...
		<div>
			<table width="100%" cellspacing="0" border="1" class="rpt_table" rules="all">
				<thead> 	 	 	 	 	 	
					<tr>
						<th width="5%">SL.</th>  
						<th width="45%">Sewing Floor</th>  
						<th width="25%">Day Efficiency</th>
						<th width="25%"><p>Day Produced Pcs</p></th>
					</tr>
				</thead>
			</table>
			<div style="max-height:400px; overflow-y:auto; width:100%" id="scroll_body">
				<table border="1" cellpadding="0" cellspacing="0" class="rpt_table"  width="100%" rules="all" id="table_body" >
				<? $i=1;
				foreach($curResourceDataArr as $flid=>$floordata)
				{
					...
					<tr bgcolor="<?=$bgcolor; ?>" onClick="change_color('tr_1nd<?=$i; ?>','<?=$bgcolor; ?>');" id="tr_1nd<?=$i; ?>" >
						...
						<td style="word-wrap: break-word;word-break: break-all;" width="45%" title="<?=$flid; ?>">
							<a href="javascript:void(0)" onclick="show_details('<?=$search_string;?>');">
								<?=$floor_library[$flid]; ?>
							</a>
						</td>  
						<td width="25%" bgcolor="<?=$bg_color;?>" align="right" title="<?=$dayproducehour.'/'.$daymanpower; ?>"><?=number_format($dayeff,2); ?>%</td>
						<td width="25%" align="right"><?=number_format($todayproduction,0); ?></td>
					</tr>
					<?
					$i++;
			...
			<table width="100%" cellspacing="0" border="1" class="rpt_table" rules="all" >
				<tfoot> 	 	 	 	 	 	
					<tr>
						...
						<th width="25%" align="right"><?=number_format($gdayeff,2); ?></th>
						<th width="25%"><?=number_format($gcuproduction,0);?></th>
					</tr>
				</tfoot>
			</table>
		</div>
	...
	<table width="<?=$tblwidth; ?>%" cellspacing="0" border="1" class="rpt_table" rules="all">
		<thead> 	 	 	 	 	 	
			<tr>
				<th style="word-wrap: break-word;word-break: break-all;" width="5%">SL.</th>  
				<th style="word-wrap: break-word;word-break: break-all;" width="45%">Sewing Floor</th>  
				<th style="word-wrap: break-word;word-break: break-all;" width="25%">MTD Avg. Eff%</th>
				<th style="word-wrap: break-word;word-break: break-all;" width="25%">MTD P PCS</th>
			</tr>
		</thead>
		<? $n=1;
			foreach($floordatewisearr as $flrid=>$fldata)
			{
				...
				<tr bgcolor="<?=$nbgcolor; ?>" onClick="change_color('tr_<?=$n; ?>','<?=$nbgcolor; ?>');" id="tr_1nd<?=$n; ?>" >
					...
					<td style="word-wrap: break-word;word-break: break-all;" ><?=$floor_library[$flrid]; ?></td>
					<td bgcolor="<?=$bg_color;?>"  align="right"><?=number_format($mtdavgeff,2); ?></td>
					<td  align="right"><?=number_format($productionqty,1); ?></td>
				</tr>
				<?	
				$n++;
			}
