...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Monthly Shipment Status Report", "../../", 1, 1, $unicode, 1, '');
?>	
...
<script>
    ...
    function fn_report_generated(operation)
    {
        ...
        else
        {
            ...
            var report_title = $("div.form_caption").html();
            var data = "action=report_generate" + get_submitted_data_string('cbo_company_id*cbo_location*cbo_buyer_name*txt_date_from*txt_date_to*cbo_type*cbo_dealing_merchant', "../../") + '&report_title=' + report_title + '&datediff=' + datediff + '&report_type=' + operation;
            freeze_window(3);
            http.open("POST", "requires/monthly_shipment_status_controller.php", true);
    ...
    function openmypage_ex_popup(company,location,buyer,color_size,date_from,date_to,return_qnty)
    {
	   var cbo_type = $('#cbo_type').val();
        ...
        emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/monthly_shipment_status_controller.php?company='+company+'&action=ex_qnty_popup&location='+location+'&buyer='+buyer+'&color_size='+color_size+'&date_from='+date_from+'&date_to='+date_to+'&return_qnty='+return_qnty+'&cbo_type='+cbo_type, 'Detail Veiw', 'width=700px, height=450px,center=1,resize=1,scrolling=1','../');
    }
    function openmypage_ex_popup_one(company,buyer,color_size,date_from,date_to,export_qnty)
    {
	   var cbo_type = $('#cbo_type').val();
		
        emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/monthly_shipment_status_controller.php?company='+company+'&action=ex_qnty_popup_one&buyer='+buyer+'&color_size='+color_size+'&date_from='+date_from+'&date_to='+date_to+'&export_qnty='+export_qnty+'&cbo_type='+cbo_type, 'Detail Veiw', 'width=700px, height=450px,center=1,resize=1,scrolling=1','../');
    }
    
    function openmypage_actual_popup(company,client,buyer,type,dtls_id,action = '')
    {
        if(action == '') action = 'actual_po_poup';
        emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/monthly_shipment_status_controller.php?company='+company+'&action='+action+'&client='+client+'&buyer='+buyer+'&dtls_id='+dtls_id+'&types='+type, (type.replace("_"," ")).toUpperCase(), 'width=770px, height=450px,center=1,resize=1,scrolling=1','../');
    }
	
    function myPopup(po_id,date,type,action)
    {
		
		var page_link='requires/monthly_shipment_status_controller.php?po_id='+po_id+'&date='+date+'&cbo_type='+type+'&action='+action;
...
<body onLoad="set_hotkey();">
    <div style="width:100%;" align="center">
        ...
        <form name="unitwiseproduction_1" id="unitwiseproduction_1" autocomplete="off" > 
            <h3 style="width:1285px; margin-top:5px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu(this.id, 'content_search_panel', '')"> -Search Panel</h3> 
            <div id="content_search_panel" style="width:1285px" align="center" >      
                <fieldset>  
                    <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" align="center" border="1" rules="all">
                        ...
                        <tbody>
                            <tr class="general">
                                <td><? echo create_drop_down("cbo_company_id", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "-- Select Company --", $selected, "load_drop_down('requires/monthly_shipment_status_controller', this.value, 'load_drop_down_location', 'location_td' );load_drop_down( 'requires/monthly_shipment_status_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );"); ?> </td>
                                ...
                                <td align="center">
                                    <input type="button" id="show_button" class="formbutton" style="width:75px" value="Show" onClick="fn_report_generated(3)" />
                                    <input type="button" id="show_button_2" class="formbutton" style="width:75px" value="Show 2" onClick="fn_report_generated(4)" />
                                    <input title="Based on Ex-factory of Actual PO Tagging" type="button" id="show_button_3" class="formbutton" style="width:75px" value="Show 3" onClick="fn_report_generated(5)" />
                                    <input title=" Based on Ex-factory Qty of Actual PO" type="button" id="show_button_4" class="formbutton" style="width:75px" value="Show 4" onClick="fn_report_generated(6)" />
                                    <input type="button" id="show_button_5" class="formbutton" style="width:75px" value="Show 5" onClick="fn_report_generated(7)" />
