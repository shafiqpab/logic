<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Lot Wise Cutting Status Report.. Create for Urmi
Functionality	:	
JS Functions	:
Created by		:	Shafiq
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Lot Wise Cutting Status Report.", "../../", 1, 1,$unicode,1,1,1);
...
<script>
	...
	{	
		...
		var page_link='requires/lot_wise_cutting_status_report_controller.php?action=job_popup&buyer_name='+buyer_name+'&cbo_year='+cbo_year;
	...
	function fn_generate_report(type)
	{
		...
		freeze_window(3);
		http.open("POST","requires/lot_wise_cutting_status_report_controller.php",true);
	...
	function openmypage(po_break_down_id,item_id,action,country_id)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox','iframe','requires/lot_wise_cutting_status_report_controller.php?po_break_down_id='+po_break_down_id+'&item_id='+item_id+'&action='+action+'&country_id='+country_id, popup_caption, popupWidth+'center=1,resize=0,scrolling=0','../');
	}	
	function fn_show_popup(search_string,type) 	 		 	 
	{
		var popup_width = 300;
		var popup_height = 400;
		var action = "cutting_popup";
		var title = (type=='1') ? "Reject Popup" : "Replace Popup";
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/lot_wise_cutting_status_report_controller.php?search_string='+search_string+'&action='+action+'&type='+type, title, 'width='+popup_width+','+'height='+popup_height+',center=1,resize=0,scrolling=0','../');
...
<body onLoad="set_hotkey();">
  <div style="width:100%;" align="center"> 
   <? echo load_freeze_divs ("../../",'');  ?>
    <h3 style="width:650px; margin-top:20px;" align="center" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3>
       <div style="width:100%;" align="center" id="content_search_panel">
    <form id="dateWiseProductionReport_1">    
      <fieldset style="width:650px;">
            <table class="rpt_table" width="650" cellpadding="0" cellspacing="0" align="center" border="1" rules="all">
               <thead>                    
                       <tr>
                ...
                <tr class="general">
                    <td align="center"> 
                        <?
                            echo create_drop_down( "cbo_company_name", 130, "SELECT id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/lot_wise_cutting_status_report_controller', this.value, 'load_drop_down_buyer', 'td_buyer' );" );
                        ?>
                    </td>
                    <td align="center"> 
                        <?
