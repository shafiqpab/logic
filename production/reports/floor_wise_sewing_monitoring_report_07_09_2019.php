...
<script>
    ...
    {
        if (form_validation('cbo_company_name*cbo_floor*txt_date_from*txt_date_to','Company Name*Floor Name*From Date*To Date')==false)
        {
            return;
        }
        else
        {
            ...
            freeze_window(3);
            http.open("POST","requires/floor_wise_sewing_monitoring_report_controller.php",true);
    ...
    function show_line_remarks(company_id,order_id,floor_id,line_no,prod_date,action)
    {
        ...
        popup_width='550px'; 
        emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/floor_wise_sewing_monitoring_report_controller.php?order_id='+order_id+'&company_id='+company_id+'&floor_id='+floor_id+'&sewing_line='+line_no+'&prod_date='+prod_date+'&action='+action, 'Detail Veiw','width='+popup_width+', height=270px,center=1,resize=0,scrolling=0','../');
    }
    ...
    function getFloorId() 
    {
        ...
        if(company_id !='') 
        {
          var data="action=load_drop_down_floors&data="+company_id;
          http.open("POST","requires/floor_wise_sewing_monitoring_report_controller.php",true);
          http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
          http.send(data); 
          http.onreadystatechange = function(){
    ...
    function openmypage_line()
    {
        ...
        var page_link='requires/floor_wise_sewing_monitoring_report_controller.php?action=line_search_popup&company='+company+'&location='+location+'&floor_id='+floor_id+'&date_from='+date_from+'&date_to='+date_to+'&line_id='+line_id; 
            ...
            <table class="rpt_table" width="850" cellpadding="0" cellspacing="0" border="1" align="center">
               <thead>                    
                    <tr>
                        ...
                        <th width="130" class="must_entry_caption">Floor</th>
                    ...
                    <tr >
                        ...
                        <td id="floor_td">
                            <? 
                                ...
                                echo create_drop_down( "cbo_floor", 130, $floor_sql,"", 1, "-- Select --", $selected, "", 1, "" );
                            ?>
                        </td>
                         <td id="line_td">
                            <? 
