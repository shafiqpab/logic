...
{ 
	...
	$sql_cond="";
	if($txt_sales_order_no!="") $sql_cond.=" and a.job_no like '%$txt_sales_order_no%'";
	...
	if($cbo_buyer_name>0) $sql_cond.=" and f.buyer_id=$cbo_buyer_name";
	...
	$position_sql=sql_select("select roll_id, po_breakdown_id, 
					...
					from pro_roll_details where status_active=1 and is_deleted=0 and roll_id>0 $barcode_cond_prod  group by roll_id, po_breakdown_id");
	...
	$sql=" select a.company_id, a.style_ref_no, a.sales_booking_no, a.job_no, b.color_id, b.color_range_id, b.body_part_id, b.fabric_desc, b.dia,b.gsm_weight, d.roll_no, d.barcode_no,f.buyer_id,e.machine_dia,e.spandex_stitch_length,e.machine_gg from fabric_sales_order_mst a, fabric_sales_order_dtls b, wo_booking_mst f,ppl_planning_entry_plan_dtls c,ppl_planning_info_entry_dtls e, pro_roll_details d 	where a.id=b.mst_id and a.sales_booking_no=f.booking_no and a.id=c.po_id and c.dtls_id=e.id and a.id=d.po_breakdown_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and b.status_active=1 and b.is_deleted=0 and a.id=105 and a.company_id=$company_name $sql_cond $year_cond $bar_code_cond $style_ref_cond order by c.id, b.id, a.id";

	$nameArray=sql_select( $sql);
	$summary_data=$garph_data=array();
	foreach($nameArray as $row)
	{
		$summary_data[$row[csf("color_range_id")]][$row[csf("body_part_id")]][$row[csf("fabric_desc")]][$row[csf("gsm_weight")]][$row[csf("dia")]]["grey_qnty"]+=$row[csf("grey_qnty")];
		$summary_data[$row[csf("color_range_id")]][$row[csf("body_part_id")]][$row[csf("fabric_desc")]][$row[csf("gsm_weight")]][$row[csf("dia")]]["batch_issue_qtny"]+=$batch_issue_qtny_arr[$row[csf("roll_id")]][$row[csf("po_id")]];
		$summary_data[$row[csf("color_range_id")]][$row[csf("body_part_id")]][$row[csf("fabric_desc")]][$row[csf("gsm_weight")]][$row[csf("dia")]]["batch_creat_qnty"]+=$batch_creat_qnty_arr[$row[csf("roll_id")]][$row[csf("po_id")]];
		...
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $table_width; ?>" class="rpt_table" align="left">
			<thead>
				<tr>
					...
					?>
<!--					<th width="70">Finishing Start</th>-->
				...
				<?
					...
					// 	if($row[csf("entry_form")]==2) $roll_entry_form="Production"; else $roll_entry_form="Receive";
						...
						<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $m; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $m; ?>"> 
                            ...
                            <td width="170"><p><? echo $row[csf("fabric_desc")]; ?>&nbsp;</p></td>
							...
							//$finish_receive_dt = $row[csf("finishing_start")];
							?>
...
/*	foreach ($nameArray as $row)
    ...
    <div style="float:left; margin-left:100px;">
      ...
      <table cellspacing="0" cellpadding="0" border="1" rules="all" width="2500" class="rpt_table" style="float:left;">
			<thead>
				<tr>
					<th width="30">SL</th>
	   	...
	   	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="2500" class="rpt_table" id="table_body" style="float:left;">
					...
					?>
						...
						<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $m; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $m; ?>"> 
							 ...
							 if(!in_array($groupData_1,$date_array_1))
								{ 
								?>
		                            <td width="30" rowspan="<? echo $gData_1[$groupData_1];?>" align="center"><? echo $m; ?></td>
		                            <td width="120" rowspan="<? echo $gData_1[$groupData_1];?>"><? echo $buyer_arr[$row[csf("buyer_name")]]; ?></td>
		                            <td width="120" rowspan="<? echo $gData_1[$groupData_1];?>"><? echo $row[csf("job_no")]; ?></td>
		                            <td width="120" rowspan="<? echo $gData_1[$groupData_1];?>"><? echo $row[csf("po_number")]; ?></td>
                                    <td width="120" rowspan="<? echo $gData_1[$groupData_1];?>"><? echo  $body_part[$row[csf("body_part_id")]]; ?></td>
                                    <td width="200" rowspan="<? echo $gData_1[$groupData_1];?>"><? echo  $row[csf("item_description")]; ?></td>
							 ...
							 if(!in_array($groupData_2,$date_array_2))
								...
								?>
                                    ...
                                    <td width="60" rowspan="<? echo $gData_2[$groupData_2];?>"><? echo  $color_library[$row[csf("color_id")]]; ?></td>
                                    <td width="60" rowspan="<? echo $gData_2[$groupData_2];?>"><? echo  $row[csf("batch_no")]; ?></td>
        ...
        <!-- No Batch -->   
		  ...
		  <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1500" class="rpt_table" style="float:left;">
		  <thead>
		      <tr>
		          <th width="30">SL</th>
...
$order_cond.=" or b.id  not in(".implode(",",$value).")";
   ...
   $ji++;
	...
	where p.id=a.dtls_id and a.po_breakdown_id=b.id and b.job_no_mst=c.job_no and a.entry_form in(2,22) and a.roll_id=0 and a.status_active=1 and a.is_deleted=0 and c.company_name=$company_name  $sql_cond $bar_code_cond $style_ref_cond $batch_no_cond order by c.id, b.id, a.id,p.body_part_id,p.febric_description_id,a.roll_no";  */
		  ...
		  ?>
          <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1500" class="rpt_table" id="table_body" style="float:left;">
          <?
		  		$i=1;
