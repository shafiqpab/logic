...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Daily Dyeing Production Report", "../../", 1, 1,$unicode,1,1);
?>	
<script>
    ...
    function fn_dyeing_report_generated(operation)
    {
        ...
        if (operation == 3)
        {
            var data = "action=generate_report3&operation=" + operation + get_submitted_data_string('cbo_company_name*cbo_buyer_name*job_number_show*job_number*batch_number*batch_number_show*fso_no*hidden_fso_no*booking_no*hidden_booking_no*cbo_type*cbo_year*txt_date_from*txt_date_to*cbo_result_name*cbo_batch_type*txt_machine_name*txt_machine_id*cbo_floor_id*cbo_shift_name*cbo_group_by*search_type*cbo_prod_type*cbo_party_id*cbo_working_company_name', "../../");
        } 
    ...
    function openmypage_popup(batch_id,fso_id,action)
    {
        emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/dyeing_production_sales_report_controller.php?fso_id='+fso_id+'&batch_id='+batch_id+'&action='+action, 'Details', 'width=600px,height=350px,center=1,resize=0,scrolling=0','../');
    }
    ...
    {    
        ...
        if(company_id !='') {
            var data="action=load_drop_down_floor&choosenCompany="+company_id;
            http.open("POST","requires/dyeing_production_sales_report_controller.php",true);
            http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
            http.send(data); 
...
<body onLoad="set_hotkey();">
    <div style="width:100%;" align="center">
        ...
        <form name="dailyYarnStatusReport_1" id="dailyYarnStatusReport_1"> 
            <h3 style="width:1750px; margin-top:10px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu(this.id, 'content_search_panel', '')">-Search Panel</h3>         <div id="content_search_panel" >      
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                 ...
                                 <td> 
                                    <?
                                echo create_drop_down("cbo_working_company_name", 100, "select id,company_name from lib_company where status_active=1 and is_deleted=0  order by company_name", "id,company_name", 1, "-- Select Working Company --", 0, "load_drop_down('requires/dyeing_production_sales_report_controller', this.value+'_'+ document.getElementById('cbo_batch_type').value , 'load_drop_down_buyer' ,'cbo_buyer_name_td' );getCompanyId();");
                                //load_drop_down( 'requires/dyeing_production_sales_report_controller', this.value, 'load_drop_down_floor', 'floor_td' );
                                    ?>
                                </td>

                                <td> 
                                    <?
                                    echo create_drop_down("cbo_company_name",100, "select id,company_name from lib_company where status_active=1 and is_deleted=0  order by company_name", "id,company_name", 0, "-- Select Company --", 0, "");
                                    ?>
                                </td>
                                ...
                                <td>
                                    ...
                                    <input type="button" id="show_button" class="formbutton" style="width:50px" value="Show2" onClick="fn_dyeing_report_generated(2)" />
                                    <input type="button" id="show_button" class="formbutton" style="width:50px" value="Show3" onClick="fn_dyeing_report_generated(3)" />
...
<script> 
 set_multiselect('cbo_company_name*cbo_floor_id','0*0','0*0','','0*0');
</script>
