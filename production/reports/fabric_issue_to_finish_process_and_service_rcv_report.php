...
{
    col_operation: {
    id: ["issue_qty","rcv_grey_qty","rcv_finish_qty"],
    col: [21,22,23],
...
function fn_report_generated(type)
{
	...
	var data="action="+action+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_job_no*cbo_order_type*cbo_presentation*txt_date_from*txt_date_to*cbo_service_source*cbo_supplier_name*txt_ref_no*txt_file_no*txt_wo_no',"../../")+'&report_title='+report_title;
	freeze_window(3);
         ...
         <fieldset style="width:1050px;">
             <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
             	<thead>
                    ...
                    <th colspan="2"><input type="reset" name="res" id="res" value="Reset" onClick="reset_form('fabricReceiveStatusReport_1','report_container*report_container2','','','')" class="formbutton" style="width:150px" /></th>
                </thead>
                <tbody>
                    <tr class="general">
                        <td>
                            <?
                                echo create_drop_down( "cbo_company_name", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name","id,company_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/fabric_issue_to_finish_process_and_service_rcv_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/fabric_issue_to_finish_process_and_service_rcv_report_controller', this.value, 'load_drop_down_supplier', 'supplier_td' );;" );
                            ?>
                        </td>
                        <td id="buyer_td">
                            <?
