...
<script>
	...
	{	
		...
		var page_link='requires/style_closing_statement_report_controller.php?action=job_popup&buyer_name='+buyer_name+'&company_name='+company_name;
	...
	function fn_generate_report(type)
	{
		...
		var data="action=generate_report"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_year*txt_job_no*txt_style_no*hidden_job_id*txt_date_from*txt_date_to',"../../")+'&type='+type+'&report_title='+report_title;
		 
		freeze_window(3);
		http.open("POST","requires/style_closing_statement_report_controller.php",true);
	...
	{	
		...
		{			 
			...
			document.getElementById('report_container').innerHTML='<a onclick="exportToExcel()" id="dlink" style="text-decoration:none"><input type="button" value="Excel Preview"  name="excel" id="exportBtn" class="formbutton" style="width:100px"/></a>&nbsp;&nbsp;<input type="button" onclick="new_window()" value="Print Preview" name="Print" class="formbutton" style="width:100px"/>';
	...
	function openmypage(po_break_down_id,item_id,action,country_id)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox','iframe','requires/style_closing_statement_report_controller.php?po_break_down_id='+po_break_down_id+'&item_id='+item_id+'&action='+action+'&country_id='+country_id, popup_caption, popupWidth+'center=1,resize=0,scrolling=0','../');
	}	
	function openmypage_production_popup(search_string,type) 	 		 	 
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/style_closing_statement_report_controller.php?search_string='+search_string+'&action='+action+'&type='+type, title, 'width='+popup_width+','+'height='+popup_height+',center=1,resize=0,scrolling=0','../');
            ...
            <table class="rpt_table" width="850" cellpadding="0" cellspacing="0" align="center" border="1" rules="all">
               <thead>                    
                   <tr>
                        ...
                        <th class="must_entry_caption"  width="200">Closing Month</th>
                        <th><input type="button" class="formbutton" style="width:70px" value="Summary" onClick="fn_generate_report(1)"/></th>
                    </tr>   
              </thead>
                <tbody>
                <tr class="general">
                    <td align="center"> 
                        <?
                            echo create_drop_down( "cbo_company_name", 130, "SELECT id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/style_closing_statement_report_controller', this.value, 'load_drop_down_buyer', 'td_buyer' );get_php_form_data( this.value, 'eval_multi_select', 'requires/style_closing_statement_report_controller' );" );
                        ?>
                    </td>
                    ...
                    <td>
                       <input type="text" id="txt_job_no"  name="txt_job_no"  style="width:100px" class="text_boxes" onDblClick="open_job_no()" placeholder="Write/Browse" title="write coma seperate like 10,12,13" />
                       <input type="hidden" id="hidden_job_id"  name="hidden_job_id" />
                    </td>
                    <td>
                       <input type="text" id="txt_style_no"  name="txt_style_no"  style="width:100px" class="text_boxes" onDblClick="open_job_no()" placeholder="Write/Browse"/>
                    </td>
                    ...
                    <td>
                         <input type="button" id="show_button" class="formbutton" style="width:70px" value="Details" onClick="fn_generate_report(2)" />                                            
