...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Fabric Finishing Wip Report ", "../../", 1, 1,'','','');
...
<script>
	...
	//	var cbo_buyer_name=$("#cbo_buyer_name").val();
		...
		if(type==2)
		{
		var data="action=report_generate2&type="+type+get_submitted_data_string('cbo_company_name*cbo_buyer_name*batch_number*batch_number_show*booking_number*booking_number_show*cbo_type*cbo_year*txt_date_from*txt_date_to*cbo_floor_id*cbo_location_id*batch_extension',"../../")+'&report_title='+report_title;
		}
		else
		{
			var data="action=report_generate&type="+type+get_submitted_data_string('cbo_company_name*batch_number*batch_number_show*booking_number*booking_number_show*cbo_type*cbo_year*txt_date_from*txt_date_to*batch_extension',"../../")+'&report_title='+report_title;
		}

		http.open("POST","requires/fabric_finishing_report_controller_wip.php",true);
	...
	function bookingnumber()
	{
		...
		var page_link="requires/fabric_finishing_report_controller_wip.php?action=bookingnumbershow&company_name="+company_name;
	...
	function batchnumber()
	{
		...
		var company_name=document.getElementById('cbo_company_name').value;
		var batch_number=document.getElementById('batch_number_show').value;
		var page_link="requires/fabric_finishing_report_controller_wip.php?action=batchnumbershow&company_name="+company_name;
...
<body onLoad="set_hotkey();">
	<div style="width:100%;" align="center">
		...
		<form name="dailyYarnStatusReport_1" id="dailyYarnStatusReport_1">
			<h3 style="width:900px; margin-top:10px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu(this.id,'content_search_panel','')">-Search Panel</h3>  <div id="content_search_panel" >
				<fieldset style="width:900px;">
					<table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
						...
						<tbody>
                            ...
                            <td>
                            	<?
								//load_drop_down('requires/fabric_finishing_report_controller_wip', this.value, 'load_drop_down_location','location_td2');load_drop_down('requires/fabric_finishing_report_controller_wip', this.value, 'load_drop_down_buyer', 'cbo_buyer_name_td2');
                            	echo create_drop_down( "cbo_company_name", 200, "select id,company_name from lib_company where status_active=1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select Company --", 0, "" );
                            	?>
                            ...
                            <td>
                            	<input type="text"  name="booking_number_show" id="booking_number_show" class="text_boxes" style="width:70px;" tabindex="1" placeholder="Write" >
                            ...
                            <td><input type="button" id="show_button2" class="formbutton" style="width:40px" value="Show" onClick="fn_dyeing_report_generated(0)" />&nbsp;
                            	<input type="button" id="show_button" class="formbutton" style="width:55px; display:none" value="Summary" onClick="fn_dyeing_report_generated(1)" />
