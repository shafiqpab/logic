<?php
/**-------------------------------------------- Comments -----------------------
 * Purpose            :    This Form Will Create Shipdate Wise Fabric Production Status Report
 * Functionality    :
 * JS Functions    :
...
//--------------------------------------------------------------------------------------------------------------------
	...
	<script>
		...
		function fn_report_generated(type) {
			...
			freeze_window(3);
			http.open("POST", "requires/shipment_date_wise_fabric_status_report_sales_order_controller.php", true);
	...
	function new_window()
	{
		...
		document.getElementById('scroll_body').style.maxHeight="540px";
	}
		...
		function open_febric_receive_status_order_wise_popup(order_id, type, color, deter_id) {
			...
			if (type == "fabric_receive" || type == "fabric_purchase" || type == "grey_issue_popup" || type == "dye_qnty" || type == "finish_trans_popup" || type == "finish_trans_in_popup" || type == "finish_trans_out_popup") {
				popup_width = '1090px';
			} else if ( type == "grey_purchase_popup" || type == "grey_receive_by_batch_popup" || type == "batch_popup" || type == "dyeing_popup") {
				popup_width = '1050px';
			} 
			...
			emailwindow = dhtmlmodal.open('EmailBox', 'iframe', 'requires/shipment_date_wise_fabric_status_report_sales_order_controller.php?order_id=' + order_id + '&action=' + type + '&color=' + color + '&deter_id=' + deter_id, 'Detail Veiw', 'width=' + popup_width + ', height=450px,center=1,resize=0,scrolling=0', '../');
		}
		...
		function openmypage_job() {
			...
			var page_link = 'requires/shipment_date_wise_fabric_status_report_sales_order_controller.php?action=style_ref_search_popup&companyID=' + companyID + '&buyerID=' + buyerID + '&within_group=' + within_group;
			;
		...
		function openmypage_booking() {
			...
			var page_link = 'requires/shipment_date_wise_fabric_status_report_sales_order_controller.php?action=booking_no_search_popup&companyID=' + companyID + '&cbo_within_group=' + cbo_within_group;
		...
		$(document).on("click", ".view_order", function (e) {
			...
			dhtmlmodal.open('EmailBox', 'iframe', 'requires/shipment_date_wise_fabric_status_report_sales_order_controller.php?company_id=' + company_id + '&job_no=' + job_no + '&action=order_popup', 'Order Popup', 'width=640px,height=350px,center=1,resize=1,scrolling=0', '')
		});
	</script>
	<script type="text/javascript">
		...
		function fnc_fabric_sales_order_print(company_name, booking_id, booking_no, sales_job_no, action) {
			...
			window.open("../requires/fabric_sales_order_entry_controller.php?data=" + data + '&action=' + action, true);
		}
					...
					<thead>
						...
						<tbody>
							<tr class="general">
								<td>
									<?php
									echo create_drop_down("cbo_company_name", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/shipment_date_wise_fabric_status_report_sales_order_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );");
									?>
								</td>
								<td id="buyer_td">
									<?php
								...
								<td>
									<input type="hidden" id="show_button" class="formbutton" style="width:70px" value="Report"
