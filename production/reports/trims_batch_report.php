...
Purpose			: 	This form will Create Trims Batch Reprot
Functionality	:	
JS Functions	:
...
function fn_report_generated(operation)
{
	...
	else
	{
		...
		freeze_window(5);
	    var data="action=batch_report&operation="+operation+get_submitted_data_string('cbo_company_name*cbo_working_company_id*cbo_buyer_name*job_number_show*job_number*batch_number*batch_number_show*txt_hide_booking_id*txt_booking_no*txt_ext_no*hidden_ext_no*order_no*hidden_order_no*cbo_type*cbo_year*cbo_batch_type*txt_date_from*txt_date_to*file_no*ref_no*cbo_floor',"../../");
		//alert(data);
			http.open("POST","requires/trims_batch_report_controller.php",true);
...
{ 
	...
	var page_link="requires/trims_batch_report_controller.php?action=batchnumbershow&company_name="+company_name; 
...
function openmypage_booking()
{
    ...
    var page_link='requires/trims_batch_report_controller.php?action=booking_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id;
...
{ 
	...
	//alert(batch_type);
	var page_link="requires/trims_batch_report_controller.php?action=jobnumbershow&company_id="+company_name+"&cbo_buyer_name="+cbo_buyer_name+"&year="+year+"&batch_type="+batch_type;
...
{ 
	...
	var year=document.getElementById('cbo_year').value;
	var batch_type=document.getElementById('cbo_batch_type').value;
	var page_link="requires/trims_batch_report_controller.php?action=order_number_popup&company_name="+company_name+"&buyer_name="+buyer_name+"&year="+year+"&batch_type="+batch_type;
...
{ 
	...
	var company_name=document.getElementById('cbo_company_name').value;
	var page_link="requires/trims_batch_report_controller.php?action=batchextensionpopup&company_name="+company_name+"&buyer_name="+buyer_name+"&year="+year+"&job_number_show="+job_number_show+"&batch_number_show="+batch_number_show+"&batch_number_hidden="+batch_number_hidden;
	//var page_link="requires/trims_batch_report_controller.php?action=batchextensionpopup&company_name="+company_name; 
                            ...
                            <tr>
                                 <td>
                                    <? 
                                        ...
                                        echo create_drop_down( "cbo_batch_type",70, $batch_type_arr,"",1, "--All--", 0,"load_drop_down('requires/trims_batch_report_controller',document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_buyer', 'cbo_buyer_name_td' );",0 );
                                    ?>
                                </td>
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 100, "select id,company_name from lib_company where status_active=1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select Company --", 0, "load_drop_down('requires/trims_batch_report_controller',this.value+'_'+document.getElementById('cbo_batch_type').value, 'load_drop_down_buyer','cbo_buyer_name_td' );" );
                                    ?>
                                </td>
                                ...
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_working_company_id", 100, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, '--Select Working--', 0,"load_drop_down('requires/trims_batch_report_controller',this.value, 'load_drop_down_floor', 'td_floor' );",'','','','','',3);
                                    ?>
                                </td>
                                ...
                                <td>
                                <?
								$search_by_arr=array(1=>"Date Wise Report");
