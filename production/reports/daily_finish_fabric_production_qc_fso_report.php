...
//-----------------------------------------------------------------------------------------------------
echo load_html_head_contents("Daily Finish Fabric Production QC Report", "../../", 1, 1,'',1,1);
?>	
<script>
	...
	function openmypage_job() 
	{
		...
		var page_link = 'requires/daily_finish_fabric_production_qc_fso_report_controller.php?action=style_ref_search_popup&companyID=' + companyID + '&buyerID=' + buyerID + '&within_group=' + within_group;
		;
	...
	function openmypage_booking() 
	{
		...
		var page_link = 'requires/daily_finish_fabric_production_qc_fso_report_controller.php?action=booking_no_search_popup&companyID=' + companyID + '&cbo_within_group=' + cbo_within_group;
	...
	function openmypage_batchNo()
	{
		...
		{ 	
			...
			var page_link = 'requires/daily_finish_fabric_production_qc_fso_report_controller.php?cbo_company_id='+cbo_company_id+'&action=batch_popup';		  
    ...
    function openmypage_productionNo()
	{
		var cbo_company_id = $('#cbo_company_name').val();	
		if(form_validation('cbo_company_name','Company Name')==false)
		{
		...
		{ 
			var title = 'Prodution Pop Up';	
			var page_link = 'requires/daily_finish_fabric_production_qc_fso_report_controller.php?cbo_company_id='+cbo_company_id+'&action=production_popup';		  
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=825px,height=350px,center=1,resize=1,scrolling=0','../');
			emailwindow.onclose = function () 
			{
				var theform = this.contentDoc.forms[0];
				var production_id = this.contentDoc.getElementById("hidden_production_id").value;
				var production_no = this.contentDoc.getElementById("hidden_production_no").value;
                
				$('#hdn_Production_id').val(production_id);
				$('#txt_Production_no').val(production_no);
			}
		}
	}
	...
	*/
	...
	{
		var sales_job_no = $('#txt_sales_job_no').val();
		var booking_no = $('#txt_booking_no').val();
		var batch_no = $('#txt_batch_no').val();
        var Production_no = $('#txt_Production_no').val();

		if(sales_job_no=="" && booking_no=="" && batch_no=="" && Production_no=="")
		{
		...
		// {	
			...
			var data = "action=report_generate" + get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_booking_no*txt_date_from*txt_date_to*txt_sales_job_no*cbo_year_selection*cbo_within_group*hide_job_id*hide_booking_id*txt_batch_no*hdn_batch_no*txt_Production_no*hdn_Production_id', "../../")+'&report_title='+report_title;

			freeze_window(3);
			http.open("POST","requires/daily_finish_fabric_production_qc_fso_report_controller.php",true);
	...
	*/
    ...
    function openmypage_roll_qnty(ref_data,action)
	{
		...
		var popup_width='940px';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/daily_finish_fabric_production_qc_fso_report_controller.php?companyID='+companyID+'&ref_data='+ref_data+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../');
	}

    function generate_report_production(roll_id) 
	{
       // alert(roll_id);
        var url=return_ajax_request_value(roll_id, "print_barcode_b", "../requires/finish_fab_prod_roll_wise_entry_controller");
            ...
            <fieldset style="width:1090px;">
                <table class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                    ...
                    <tr class="general">
                      	<td>
							<?php
							echo create_drop_down("cbo_company_name", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/daily_finish_fabric_production_qc_fso_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );");
							?>
						</td>
						<td id="buyer_td">
							<?php
						...
						<td align="center">
                            <input type="text" name="txt_Production_no" id="txt_Production_no" class="text_boxes" placeholder="Browse Or Write" onDblClick="openmypage_productionNo();"  style="width:110px" />
                            <input type="hidden" name="hdn_Production_id" id="hdn_Production_id" class="text_boxes"   style="width:110px" />
                        </td>
						<td>
