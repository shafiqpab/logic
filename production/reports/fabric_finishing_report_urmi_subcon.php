...
function fn_dyeing_report_generated(operation)
{
  ...
  //  var booking_number_show=$("#booking_number_show").val();
    ...
    if(cbo_buyer_name!=0 || batch_number_show ||  cbo_machine_id!=0 || cbo_floor_id!=0)
    {
        //alert(23);
    }
    else
    ...
    freeze_window(5);
    var data="action=report_generate&operation="+operation+get_submitted_data_string('cbo_company_name*cbo_buyer_name*batch_number*batch_number_show*cbo_type*cbo_year*txt_date_from*txt_date_to*txt_days*cbo_gate_upto*cbo_source*cbo_floor_id*cbo_location_id*cbo_machine_id',"../../")+'&report_title='+report_title;
    // alert(data);
    http.open("POST","requires/fabric_finishing_report_urmi_subcon_controller.php",true);
...
{ 
	...
	var page_link="requires/fabric_finishing_report_urmi_subcon_controller.php?action=bookingnumbershow&company_name="+company_name; 
...
{ 
	...
	var company_name=document.getElementById('cbo_company_name').value;
	var batch_number=document.getElementById('batch_number_show').value;
	var page_link="requires/fabric_finishing_report_urmi_subcon_controller.php?action=batchnumbershow&company_name="+company_name; 
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 120, "select id,company_name from lib_company where status_active=1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select Company --", 0, "load_drop_down('requires/fabric_finishing_report_urmi_subcon_controller', this.value, 'load_drop_down_location','location_td2');load_drop_down('requires/fabric_finishing_report_urmi_subcon_controller', this.value, 'load_drop_down_buyer', 'cbo_buyer_name_td2');" );
                                    ?>
