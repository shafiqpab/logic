...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Floor Wise Production Report", "../../", 1, 1,$unicode,1,1);
?>	
...
<script>
 	...
 	function open_order_no()
	{
	    ...
	    var page_link='requires/floor_wise_daily_rmg_production_without_value_controller.php?action=order_wise_search&buyer='+buyer+'&job_id='+job_id+'&job_no='+job_no+'&cbo_year='+cbo_year;
 	...
 	function open_int_ref_popup()
	{
		...
		var buyer=$("#cbo_buyer_name").val();
		var cbo_year=$("#cbo_year").val();
	    var page_link='requires/floor_wise_daily_rmg_production_without_value_controller.php?action=int_ref_wise_search&buyer='+buyer+'&job_id='+job_id+'&job_no='+job_no+'&cbo_year='+cbo_year;
		//alert(page_link);return; 
		var title="Search Order Popup";
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=580px,height=390px,center=1,resize=0,scrolling=0','../')
		emailwindow.onclose=function()
	...
	function open_job_no()
	{
		var buyer=$("#cbo_buyer_name").val();
		var cbo_year=$("#cbo_year").val();
	    var page_link='requires/floor_wise_daily_rmg_production_without_value_controller.php?action=job_popup&buyer='+buyer+'&cbo_year='+cbo_year;
		var title="Search Order Popup";
	...
	function fn_generate_report(type)
	{
		...
		//alert(data);return;
		freeze_window(3);
		http.open("POST","requires/floor_wise_daily_rmg_production_without_value_controller.php",true);
	...
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/floor_wise_daily_rmg_production_without_value_controller.php?data='+arg+'&action='+action, 'Ex-factory Popup', 'width=450px,height=250px,center=1,resize=0,scrolling=0','../');
	}
	...
	function openmypage_fab_popup(po_id,color,type,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/floor_wise_daily_rmg_production_without_value_controller.php?action='+action+'&data='+data, title, 'width=680px,height=250px,center=1,resize=0,scrolling=0','../');
	}
	...
	function openmypage_production_popup(po,item,color,type,day,action,title,popup_width,popup_height) 	 		 	 
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/floor_wise_daily_rmg_production_without_value_controller.php?po='+po+'&action='+action+'&item='+item+'&color='+color+'&type='+type+'&day='+day, title, 'width='+popup_width+','+'height='+popup_height+',center=1,resize=0,scrolling=0','../');
...
<body onLoad="set_hotkey();">
  <div style="width:100%;" align="center"> 
   <? echo load_freeze_divs ("../../",'');  ?>
    <h3 style="width:1220px; margin-top:20px;" align="center" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3>
       <div style="width:100%;" align="center" id="content_search_panel">
    <form id="dateWiseProductionReport_1">    
      <fieldset style="width:1220px;">
            <table class="rpt_table" width="1120" cellpadding="0" cellspacing="0" align="center" border="1" rules="all">
               <thead>                    
                       <tr>
