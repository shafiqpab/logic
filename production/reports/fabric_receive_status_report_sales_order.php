...
//--------------------------------------------------------------------------------------------------------------------
	...
	<script>
		...
		function fn_report_generated(type) {
			...
			if(type == 1)
			{
				var data = "action=report_generate" + get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_booking_no*txt_date_from*txt_date_to*txt_sales_job_no*cbo_year_selection*cbo_within_group*hide_job_id*hide_booking_id*txt_ref_no*cbo_cust_buyer_name*cbo_date_search_type', "../../");
			}
			else if(type == 2)
			{
				var data = "action=report_generate_2" + get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_booking_no*txt_date_from*txt_date_to*txt_sales_job_no*cbo_year_selection*cbo_within_group*hide_job_id*hide_booking_id*cbo_cust_buyer_name*cbo_date_search_type', "../../");
			}
			else if(type == 3)
			{
				var data = "action=report_generate_3" + get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_booking_no*txt_date_from*txt_date_to*txt_sales_job_no*cbo_year_selection*cbo_within_group*hide_job_id*hide_booking_id*cbo_cust_buyer_name*cbo_date_search_type', "../../");
			}
			else
			{
				var data = "action=report_generate_4" + get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_booking_no*txt_date_from*txt_date_to*txt_sales_job_no*cbo_year_selection*cbo_within_group*hide_job_id*hide_booking_id*cbo_cust_buyer_name*cbo_date_search_type', "../../");
			}
		...
		function open_febric_receive_status_order_wise_popup(order_id, type, color, deter_id)
		{
			...
			else if (type == "knitting_balance_popup")
			{
				popup_width = '1000px';
			}
			else if (type == "fabric_color_popup")
			{
				popup_width = '900px';
			}
			else if (type == "dyeing_popup_for_report2")
			{
				popup_width = '870px';
			}
	...
	<script type="text/javascript">
		function generate_worder_report(type, booking_no, company_id, order_id, fabric_nature, fabric_source, job_no, approved, entry_form, is_short, action,requisitionMst_id) {
			...
			else if( (entry_form==118 && type==1) || (entry_form==87 && type==2))
			{
				...
				else if(booking_format_val=="SMN")
				{
					...
					//http.open("POST", "../../order/woven_order/requires/sample_booking_non_order_controller.php", true);
					print_report( company_id+'*'+requisitionMst_id+'*'+booking_no+'*'+1, action, "../../order/woven_order/requires/sample_requisition_with_booking_controller" );
			...
			if(entry_form==87 && type==2 || (entry_form==140))
			{
				if(booking_format_val=="SMN")
				{
					print_report( company_id+'*'+requisitionMst_id+'*'+booking_no+'*'+1, action, "../../order/woven_order/requires/sample_requisition_with_booking_controller" );
				}

			}
			else if(entry_form==201)
			{
				if(booking_format_val=="SMN")
				{
					print_report( company_id+'*'+requisitionMst_id+'*'+booking_no+'*'+1, action, "../../order/woven_order/requires/sample_requisition_with_booking_controller" );
		...
		function search_populate(str)
		{
			...
			{
				document.getElementById('search_by_th_up').innerHTML = "Dalivery Date";
				//$('#search_by_th_up').css('color', 'blue');
			}
...
<body>
	...
	<form id="fabricReceiveStatusReport_1">
		<div align="center">
			<h3 style="width:1180px;" align="left" id="accordion_h1" class="accordion_h"
			...
			<div id="content_search_panel">
				<fieldset style="width:1190px;">
					...
					<thead>
						...
						<th colspan="2" id="search_by_th_up">Booking Date </th>
						<th>Booking No</th>
						...
						<tbody>
							<tr class="general">
								<td>
									<?php
									echo create_drop_down("cbo_company_name", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/fabric_receive_status_report_sales_order_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/fabric_receive_status_report_sales_order_controller',this.value, 'load_drop_down_cust_buyer', 'cust_buyer_td' );get_php_form_data(this.value,'print_button_variable_setting','requires/fabric_receive_status_report_sales_order_controller' );");
									?>
								</td>
								<td id="buyer_td">
									<?php
								...
								<td id="cust_buyer_td">
									<?php
									echo create_drop_down("cbo_cust_buyer_name", 120, $blank_array, "", 1, "-- All Cust Buyer --", $selected, "", 0, "");
									?>
								</td>
								<td>
								...
								<td>
									<?
									$search_type_arr = array(1 => "Booking Date", 2 => "Dalivery Date");
									$fnc_name = "search_populate(this.value)";
									echo create_drop_down("cbo_date_search_type", 100, $search_type_arr, "", 0, "-Select-", 0, $fnc_name, 0, "", "", "", "", "");
									?>
								</td>
								<td>
...
onClick="fn_report_generated(2)"/>
									<input type="button" id="show_button3" class="formbutton" style="width:70px; display: none;" value="Report 2" onClick="fn_report_generated(3)"/>
									<input type="button" id="show_button4" class="formbutton" style="width:70px; display: none;" value="Show 2" onClick="fn_report_generated(4)"/>
								</td>
							</tr>
						</tbody>
					</table>
