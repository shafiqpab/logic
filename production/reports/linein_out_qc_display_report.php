...
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Line In-Out QC Display Report
Functionality	:	
JS Functions	:
Created by		:	Rakib 
...
//------------------------------------------------------------------------------------
echo load_html_head_contents('Line In-Out QC Display Report', '../../', 1, 1, $unicode, 1, 1);
...
<script>
    ...
    {
        if (form_validation('cbo_company_name*txt_date','Comapny Name*Production Date')==false) {
            return;
        }

        // console.log(document.getElementById('cbo_floor'));
        ...
        freeze_window(3);
        //alert(data);return;
        http.open("POST","requires/linein_out_qc_display_report_controller.php",true);
                ...
                <table class="rpt_table" width="1050px" cellpadding="0" cellspacing="0" border="1" align="center">
                    ...
                    <tbody>
                        <tr>
                            <td width="140">
                                <?php
                                    echo create_drop_down( 'cbo_company_name', 140, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name", 'id,company_name', 1, '-- Select Company --', $selected, "load_drop_down( 'requires/linein_out_qc_display_report_controller', this.value, 'load_drop_down_location', 'location_td' );load_drop_down( 'requires/linein_out_qc_display_report_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                                ?>
                            </td>
                            <td>
                                <input name="txt_date" id="txt_date" class="datepicker" style="width:114px" onChange="load_drop_down( 'requires/linein_out_qc_display_report_controller',document.getElementById('cbo_floor').value+'_'+document.getElementById('cbo_location').value+'_'+document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_line', 'line_td' );" readonly >
                            </td>
                            <td width="110" id="location_td">
                                <?php
                                    echo create_drop_down( 'cbo_location', 110, $blank_array, '', "", '-- Select --', $selected, '', 1, '' );
                                ?>
                            </td>
                            <td width="110" id="floor_td">
                                <?php
                                    echo create_drop_down( 'cbo_floor', 140, $blank_array, '', 1, '-- Select --', $selected, '', 1, '' );
                                ?>
                            </td>  
...
<script>set_multiselect('cbo_floor', '0', '0', '', '0');</script> 
