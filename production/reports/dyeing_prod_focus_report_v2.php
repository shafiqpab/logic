...
function fn_report_generated(type)
{
	...
	var data="action=report_generate"+get_submitted_data_string('cbo_working_company*cbo_floor_id*txt_machine_id*txt_batch_no*txt_date_from*txt_date_to*exchange_rate',"../../")+'&report_title='+report_title;
	freeze_window(3);
	http.open("POST","requires/dyeing_prod_focus_report_v2_controller.php",true);
...
function openmypage_batch()
{
	...
	var page_link='requires/dyeing_prod_focus_report_v2_controller.php?action=batch_no_search_popup&companyID='+companyID;
...
function openmypage_job()
{
	...
	//alert(cbo_year_id);
	var page_link='requires/dyeing_prod_focus_report_v2_controller.php?action=job_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id;
...
{ //alert(des_prod)
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/dyeing_prod_focus_report_v2_controller.php?companyID='+companyID+'&date_key='+date_key+'&batch_id='+batch_id+'&type='+type+'&exchange_rate='+exchange_rate+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../');
}
function openmypage_issue(po_id,prog_no,booking_no,action)
{ //alert(des_prod)
	var companyID = $("#cbo_company_name").val();
	var popup_width='580px';
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/dyeing_prod_focus_report_v2_controller.php?companyID='+companyID+'&po_id='+po_id+'&prog_no='+prog_no+'&booking_no='+booking_no+'&prog_no='+prog_no+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../');
}
 ...
 function openmypage_machine()
    {
        ...
        emailwindow = dhtmlmodal.open('EmailBox', 'iframe', 'requires/dyeing_prod_focus_report_v2_controller.php?action=machine_no_popup&data=' + data, 'Machine Name Popup', 'width=470px,height=420px,center=1,resize=0', '../')
             ...
             <table class="rpt_table" width="750px" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr align="center">
						...
						<td>
                             <?
                                echo create_drop_down("cbo_working_company", 100, "select id,company_name from lib_company where status_active=1 and is_deleted=0  order by company_name", "id,company_name", 1, "-- Select Working Company --", 0, "load_drop_down( 'requires/dyeing_prod_focus_report_v2_controller', this.value, 'load_drop_down_floor', 'floor_td' );");
                              ?>
                        </td>
                        <td>
                         ...
                         <td id="floor_td">
                          ...
                          <td>
                            <input type="text"  name="exchange_rate" id="exchange_rate" class="text_boxes_numeric" style="width:60px;" value="80" />
                          </td>
