...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../logout.php";
	...
	function generate_report(type)
	{
		...
		if(type=='0'){
			...
			alert("NO action found!"); return false;
		}
		
		
		var report_title=$( "div.form_caption" ).html();
		
			var data="action="+action+"&type="+type+get_submitted_data_string('cbo_company_id*cbo_location_id*cbo_floor_id*cbo_line*hidden_line_id*cbo_buyer_name*txt_date*txt_parcentage*txt_file_no*txt_ref_no*cbo_no_prod_type*cbo_shift_name',"../../")+'&report_title='+report_title;
			//alert(data);
		freeze_window(3);
		http.open("POST","requires/company_wise_hourly_production_monitoring_chaity_controller.php",true);
	...
	function generate_in_out_popup(po_id,action,floor_id,line,i,company_id,date)
	{
		 ...
		 popup_width='1020px'; 
		//var company_id = $("#cbo_company_id").val();	
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/company_wise_hourly_production_monitoring_chaity_controller.php?po_id='+po_id+'&company_id='+company_id+'&action='+action+'&type='+i+'&production_date='+date+'&floor='+floor_id+'&sewing_line='+line, 'Detail Veiw', 'width='+popup_width+', height=390px,center=1,resize=0,scrolling=0','../');
	...
	function openmypage_company()
	{
		...
		var page_link='requires/company_wise_hourly_production_monitoring_chaity_controller.php?action=company_search_popup&company='+company; 
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/company_wise_hourly_production_monitoring_chaity_controller',prodID, 'load_drop_down_buyer', 'buyer_td_id' );

			var string="action=print_button_variable_setting"+'&data='+prodID;
			//alert(data);return;
			//freeze_window(3);
			http.open("POST","requires/company_wise_hourly_production_monitoring_chaity_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(string);
			http.onreadystatechange = show_hide_buttons;
    ...
    {
		var button_arr = [222,259];
        if(http.readyState == 4) 
        {
        	var response=trim(http.responseText).split(",");
        	for(var i=1; i<= response.length; i++){
				for(var j = 0; j < button_arr.length; j++){
					if(button_arr[j] == response[i-1]){
						$("#search"+i).css("visibility", "visible"); break;
					}else{
						$("#search"+i).css("visibility", "hidden"); 
					}
	...
	function openmypage_location()
	{
		...
		var page_link='requires/company_wise_hourly_production_monitoring_chaity_controller.php?action=location_search_popup&company='+company+'&location='+location; 
	...
	function openmypage_floor()
	{
		...
		var page_link='requires/company_wise_hourly_production_monitoring_chaity_controller.php?action=floor_search_popup&location='+location+'&floor='+floor_id; 
	...
	function openmypage_line()
	{
		...
		var page_link='requires/company_wise_hourly_production_monitoring_chaity_controller.php?action=line_search_popup&company='+company+'&location='+location+'&floor_id='+floor_id+'&txt_date='+txt_date+'&line_id='+line_id; 
	...
	function show_line_remarks(company_id,order_id,floor_id,line_no,action,prod_date)
	{
		//alert(action)
		popup_width='550px'; 
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/company_wise_hourly_production_monitoring_chaity_controller.php?po_id='+order_id+'&company_id='+company_id+'&floor_id='+floor_id+'&sewing_line='+line_no+'&prod_date='+prod_date+'&action='+action, 'Detail Veiw','width='+popup_width+', height=270px,center=1,resize=0,scrolling=0','../');
...
<form id="LineWiseProductivityAnalysis_1">
    <div style="width:1020px" align="center">    
        ...
        <h3 style="width:1168px;" align="center" id="accordion_h1" class="accordion_h" onClick="accordion_menu(this.id,'content_search_panel','')">-Search Panel</h3>
        ...
        <fieldset style="width:1080px;">
            <table class="rpt_table" width="1080" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
             	<thead>
                    ...
                    <th width="160"><input type="reset" name="res" id="res" value="Reset" style="width:80px" class="formbutton" onClick="reset_form('LineWiseProductivityAnalysis_1','report_container','','','')" /></th>
                </thead>
                <tbody>
                    <tr class="general">
                       <td>
						...
						<td>
							<?
							echo create_drop_down( "cbo_shift_name", 100, $shift_name,"", 1, "-Select Shift-", 0, "",0 );
							?>
						</td>
                        <td id="floor_td">
                        ...
                        <td width="160" style="display:block;" id="button_box">
       
                            <input type="button" name="search1" id="search1" value="Show" onClick="generate_report(0)" style="width:75px;   visibility: hidden;" class="formbutton" />
                            <input type="button" name="search2" id="search2" value="Show2" onClick="generate_report(1)" style="width:75px;  visibility: hidden;" class="formbutton" />
