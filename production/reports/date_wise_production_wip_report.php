<?
/*-------------------------------------------- Comments
Purpose			: 	This form will Generate Date Wise Production WIP Report
...
echo load_html_head_contents("Date Wise Production WIP Report","../../", 1, 1, $unicode,1,1); 
?>	
<script>
...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../logout.php";
	...
	function generate_report()
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_buyer_id*cbo_location_id*cbo_floor_id*cbo_line_id*txt_date_from*cbo_working_company_id',"../../")+'&report_title='+report_title;
	
		freeze_window(3);
		http.open("POST","requires/date_wise_production_wip_report_controller.php",true);
	...
	{
  		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/date_wise_production_wip_report_controller.php?datas='+datas+'&action='+action, 'Prod. Qty View', 'width=720px,height=450px,center=1,resize=0,scrolling=0','../');
	}
	function openmypage_cut_no_qty(datas,action)
	{
  		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/date_wise_production_wip_report_controller.php?datas='+datas+'&action='+action, 'System Cut No', 'width=610px,height=330px,center=1,resize=0,scrolling=0','../');
	}
                       ...
                       <td>
                        ...
                        <td>
							<? 
                                ...
                                echo create_drop_down( "cbo_working_company_id", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 order by comp.company_name","id,company_name", 1, "-- Select Company --", 0, "" );
                            ?>                            
                        </td>
                        ...
                        <td id="line_td">
                            <? 
                                echo create_drop_down( "cbo_line_id", 150, $blank_array,"", 1, "-- Select Line --", "", "" );
                            ?>                            
                        </td>
