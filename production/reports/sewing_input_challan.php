<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Sewing Input Challan Report.
Functionality	:
JS Functions	:
Created by		:	Kaiyum
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Sewing Input Challan Report", "../../", 1, 1,$unicode,'','');
?>
<script>
...
var permission = '<? echo $permission; ?>';
	...
	{
		if( $('#txt_job_no').val()!="" || $('#txt_order_no').val()!="" || $('#txt_file_no').val()!="" || $('#txt_challan_no').val()!="" || $('#txt_internal_ref').val()!="")
		{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_job_no*txt_order_no*txt_file_no*txt_challan_no*txt_internal_ref*txt_date_from*txt_date_to',"../../");
		freeze_window(3);
		http.open("POST","requires/sewing_input_challan_controller.php",true);
	...
	function fn_with_source_report(operation)
	{
	 	if(operation==4) // Print
		{
			 ...
			 freeze_window(3);
			 var report_title="Sewing Input Challan";
			 print_report( $('#cbo_company_name').val()+'*'+master_ids+'*'+report_title+'*'+$('#txt_delivery_date').val(), "sewing_input_challan_print", "requires/sewing_input_challan_controller" );
			 release_freezing();
			 return;
		}
		...
		if(operation==5) // Print 2
		{
			...
			for(i=1; i<total_tr; i++)
			{
				try
				{
					if ($('#tbl_'+i).is(":checked"))
					{
						if(total_selected<1)
						{
							company_id = $('#companyId_'+i).val();
							delivery_basis_id = $('#deliveryBasisId_'+i).val();
							master_id = $('#deliveryMstId_'+i).val()*1;
							...
							{
								if(company_ids=="") company_ids= company_id; else company_ids +='_'+company_id;
								if(delivery_basis_ids=="") delivery_basis_ids= delivery_basis_id; else delivery_basis_ids +='_'+delivery_basis_id;
								if(master_ids=="") master_ids= master_id; else master_ids +='_'+master_id;
							}
						}
						else
						{
							alert("Multiple Challan Not Allow");
							return;
						}
						total_selected++;
					}
			...
			var report_title="Bundle Wise Sewing Input";
			var data = company_ids+'*'+master_ids+'*'+delivery_basis_ids;

			freeze_window(3);
			print_report(data+'*'+report_title+'*1', "emblishment_issue_print_4", "../requires/bundle_wise_sewing_input_controller" );
			release_freezing();
			return;
		}
	}
    ...
    <div style="width:100%;" align="center">
        ...
        <? echo load_freeze_divs ("../../",'');  ?>
         ...
         <fieldset style="width:1090px;">
            ...
            <table class="rpt_table" width="1090px" cellpadding="0" cellspacing="0" align="center" border="1" rules="all">
               <thead>
                    <tr>
                        ...
                        <th>Challan No</th>
                        <th>IR/IB</th>
                        <th id="search_text_td" class="must_entry_caption">Sewing Input Date</th>
                ...
                <tr class="general">
                    <td width="150">
                        <?
                            echo create_drop_down( "cbo_company_name", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/sewing_input_challan_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                        ?>
                    </td>
            ...
            <table>
                ...
                <tr align="center">
                	<td>
                		...
                		<input id="print2" class="formbutton" style="width:90px;" value="Print 2" name="print2" onClick="fn_with_source_report(5)" type="button">
                	</td>
                </tr>
            </table>
            <br />
