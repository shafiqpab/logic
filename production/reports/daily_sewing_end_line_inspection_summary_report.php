<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Daily Sewing End Line Inspection Summary Report.
Functionality	:	
JS Functions	:
Created by		:	Shafiq 
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Daily Sewing End Line Inspection Summary Report", "../../", 1, 1,$unicode,'','');
...
<script>
	...
	function fn_report_generated(type)
	{
		...
		else
		{
			...
			freeze_window(3);
			http.open("POST","requires/daily_sewing_end_line_inspection_summary_report_controller.php",true);
               ...
               <thead>                    
                ...
                <tr class="general">
                    <td width="150" align="center"> 
                        <?
                            echo create_drop_down( "cbo_company_name", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/daily_sewing_end_line_inspection_summary_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/daily_sewing_end_line_inspection_summary_report_controller', this.value, 'load_drop_down_location', 'location_td' );fn_disable_com(1)" );
                        ?>
                    </td>
                    <td width="150" align="center"> 
                        <?
                            echo create_drop_down( "cbo_working_company_id", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/daily_sewing_end_line_inspection_summary_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/daily_sewing_end_line_inspection_summary_report_controller', this.value, 'load_drop_down_location', 'location_td' );fn_disable_com(2)" );
                        ?>
                    </td>
                    <td width="100" align="center">
                        <? 
                            echo create_drop_down( "cbo_production_type", 100, $production_type,"", 1, "--Select Type--", $selected, "","","1,5,11" );
                        ?>
                    </td>
