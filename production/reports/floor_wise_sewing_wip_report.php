...
function fn_report_generated(type)
{
    if(type==1)
    {
        ...
        if(po_id=="" && txt_date=="")
        {
            alert('Style or production date required!');
            return;
        }
    ...
    }else if(type==2)
    {
        ...
        if(po_id=="" && txt_date=="")
        {
            alert('IR/IB or production date required!');
            return;
        }
    ...
    freeze_window(3);
    http.open("POST","requires/floor_wise_sewing_wip_report_controller.php",true);
 ...
 function show_line_remarks(company_id,order_id,floor_id,line_no,prod_date,action)
    {
        ...
        popup_width='550px'; 
        emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/floor_wise_sewing_wip_report_controller.php?order_id='+order_id+'&company_id='+company_id+'&floor_id='+floor_id+'&sewing_line='+line_no+'&prod_date='+prod_date+'&action='+action, 'Detail Veiw','width='+popup_width+', height=270px,center=1,resize=0,scrolling=0','../');
    }
...
function open_job_no()
{
    ...
    var page_link='requires/floor_wise_sewing_wip_report_controller.php?action=job_popup&company_name='+company_name+'&cbo_year='+cbo_year;
...
function open_internal_ref()
{
    ...
    var company_name=$("#cbo_company_name").val();
    var cbo_year=$("#cbo_year").val();
    var page_link='requires/floor_wise_sewing_wip_report_controller.php?action=internal_ref_popup&company_name='+company_name+'&cbo_year='+cbo_year;
            ...
            <table class="rpt_table" width="1000px" cellpadding="0" cellspacing="0" border="1" align="center">
               <thead>                    
                    <tr>
                        ...
                        <th><input type="reset" id="reset_btn" class="formbutton" style="width:150px" value="Reset" /></th>
                    </tr>    
                 </thead>
                <tbody>
                <tr >
                    <td width="140"> 
                        <?
                            echo create_drop_down( "cbo_company_name", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/floor_wise_sewing_wip_report_controller', this.value, 'load_drop_down_location', 'location_td' );" );
                        ?>
                    </td>                   
                    <td width="110" id="location_td">
                        <? 
                            echo create_drop_down( "cbo_location", 110, $blank_array,"", 1, "-- Select --", $selected, " load_drop_down( 'requires/floor_wise_sewing_wip_report_controller', this.value, 'load_drop_down_floor', 'floor_td' );", 1, "" );
                        ?>
                    </td>
                    <td>
                    ...
                    <td width="">
                        <input name="txt_date" id="txt_date" class="datepicker" style="width:75px" onChange="load_drop_down( 'requires/floor_wise_sewing_wip_report_controller',document.getElementById('cbo_floor').value+'_'+document.getElementById('cbo_location').value+'_'+document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_line', 'line_td' );" readonly >
                    </td>   
                    ...
                    <td width="200">
                    <input type="button" id="show_button" class="formbutton" style="width:50px" value="Show" onClick="fn_report_generated(0)" />

                    <input type="button" id="show_button" class="formbutton" style="width:50px" value="Show2" onClick="fn_report_generated(1)" />
                    <input type="button" id="show_button" class="formbutton" style="width:50px" value="Show3" onClick="fn_report_generated(2)" />
                    </td>
