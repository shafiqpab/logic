...
function fn_report_generated()
{
	...
	else
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_location*cbo_floor*cbo_line*cbo_shift_name*txt_style_no*txt_date*cbo_subcon',"../../");
		freeze_window(3);
		http.open("POST","requires/line_item_wise_hourly_prod_report_controller.php",true);
...
function openmypage_order(company_id,order_id,item_id,action)
	{
		//var garments_nature = $("#cbo_garments_nature").val();
		popup_width='750px'; 
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/line_item_wise_hourly_prod_report_controller.php?order_id='+order_id+'&company_id='+company_id+'&item_id='+item_id+'&action='+action, 'Order Quantity','width='+popup_width+', height=270px,center=1,resize=0,scrolling=0','../');
	}
 ...
 function show_line_remarks(company_id,order_id,floor_id,line_no,prod_date,action)
	{
		...
		popup_width='550px'; 
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/line_item_wise_hourly_prod_report_controller.php?order_id='+order_id+'&company_id='+company_id+'&floor_id='+floor_id+'&sewing_line='+line_no+'&prod_date='+prod_date+'&action='+action, 'Detail Veiw','width='+popup_width+', height=270px,center=1,resize=0,scrolling=0','../');
	}
            ...
            <table class="rpt_table" width="950px" cellpadding="0" cellspacing="0" border="1" align="center">
               <thead>                    
                ...
                <tr >
                    <td width="140"> 
                        <?
                            echo create_drop_down( "cbo_company_name", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/line_item_wise_hourly_prod_report_controller', this.value, 'load_drop_down_location', 'location_td' );" );
                        ?>
                    </td>  
                    <td width="">
                    	<input name="txt_date" id="txt_date" class="datepicker" style="width:75px" onChange="load_drop_down( 'requires/line_item_wise_hourly_prod_report_controller',document.getElementById('cbo_floor').value+'_'+document.getElementById('cbo_location').value+'_'+document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_line', 'line_td' );" readonly >
                    </td>                  
                    ...
                    <td>
                        <?= create_drop_down( "cbo_shift_name", 100, $shift_name,"", 1, "-- Select --", $selected, "" );?>                           
                    </td>
                    <td width="130">
