<?
/*-------------------------------------------- Comments
Purpose			: 	This form will Create Daily Knitting Production Report-Sales v3
Functionality	:	
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Daily Knitting Production Report-Sales v3", "../../", 1, 1, '', 1, 1,1);
...
<script>
    ...
    {
        col_operation: {
        id: ["value_shift_qnty_1","value_shift_qnty_2","value_shift_qnty_3","value_shift_qnty_total","value_shift_reject_qnty"],
        col: [27,29,31,33,37],
    ...
    {
        col_operation: {
        id: ["value_shift_qnty_1","value_shift_qnty_2","value_shift_qnty_3","value_shift_qnty_total"],
        col: [23,25,27,29],
    ...
    function fn_report_generated(report_type) {
        ...
        if (txt_sales_order != "" || txt_booking_no != "") {
            if (form_validation('cbo_working_company_name', 'Company') == false) {
                return;
            }
        }
        else {
            if (form_validation('cbo_working_company_name*txt_date_from', 'Company*From Date') == false) {
                return;
            }
        }
        ...
        var data = "action=report_generate" + get_submitted_data_string('cbo_company_name*cbo_working_company_name*cbo_type*cbo_year*txt_sales_order*cbo_knitting_source*cbo_shift*cbo_floor_id*txt_date_from*txt_date_to*txt_booking_no*txt_ir_hdn_booking_no*cbo_within_group*cbo_cust_buyer_id', "../../") + '&report_title=' + report_title + '&report_type=' + report_type;

        freeze_window(5);
        http.open("POST", "requires/daily_knitting_production_report_sales_v3_controller.php", true);
    ...
    function fn_report_generated_reponse() {
        if (http.readyState == 4) {
            ...
            if (response[2]==1) 
            {
                setFilterGrid("table_body_main",-1,tableFilters);
            }
            else
            {
                setFilterGrid("table_body_main",-1,tableFilters2);
            }
            show_msg('3');
            release_freezing();
    ...
    function generate_delivery_challan_report() {
        ...
        //alert (program_ids)
        print_report(program_ids + '_' + source_ids + '_' + company + '_' + from_date + '_' + to_date, "delivery_challan_print", "requires/daily_knitting_production_report_sales_v3_controller");
    }
    ...
    function openmypage_booking_no() {
        ...
        var page_link = 'requires/daily_knitting_production_report_sales_v3_controller.php?action=booking_no_search_popup&companyID=' + companyID + '&yearID=' + yearID + '&ordType=' + ordType;
    ...
    function openmypage_sales_order() {
        ...
        var page_link = 'requires/daily_knitting_production_report_sales_v3_controller.php?action=sales_order_no_search_popup&companyID=' + companyID + '&yearID=' + yearID + '&ordType=' + ordType + '&cbo_within_group=' + cbo_within_group;
	...
	function openmypage_internal_ref_no() {
        ...
        var ordType = $("#cbo_type").val();
        var cbo_within_group = $("#cbo_within_group").val();
        var page_link = 'requires/daily_knitting_production_report_sales_v3_controller.php?action=internal_ref_no_search_popup&companyID=' + companyID + '&yearID=' + yearID + '&ordType=' + ordType + '&cbo_within_group=' + cbo_within_group;
    ...
    function openmypageDeliveryRecv(dtls_id,type,action)
    {
        var companyID           = $("#cbo_company_name").val();
        var cbo_knitting_source = $("#cbo_knitting_source").val();
        var cbo_within_group = $("#cbo_within_group").val();
        var popup_width='410px';
    ...
    {    
        ...
        load_drop_down( 'requires/daily_knitting_production_report_sales_v3_controller', company_id, 'load_drop_down_cust_buyer', 'cust_buyer_td');
    }
...
<div style="width:100%;" align="center">
    ...
    <form name="dailyYarnStatusReport_1" id="dailyYarnStatusReport_1">
        <h3 style="width:1530px;" align="left" id="accordion_h1" class="accordion_h"
                ...
                <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all"
                    ...
                    <tr class="general">
                        <td id="td_company">
                         ...
                         <td>
                            <?php
                            echo create_drop_down("cbo_working_company_name", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "-- Select Working Company --", $selected, "load_drop_down( 'requires/daily_knitting_production_report_sales_v3_controller', $('#cbo_working_company_name').val(), 'load_drop_down_floor', 'floor_td');");
                            ?>
                        </td>
                        <td align="center">
							<?php
							$gen_type = array(1 => "Self Order", 2 => "Subcontract Order");
							echo create_drop_down("cbo_type", 70, $gen_type, "", 1, "-- All --", 0, "load_drop_down( 'requires/daily_knitting_production_report_sales_v3_controller', $('#cbo_company_name').val()+'**'+this.value,); fnc_active_inactive(this.value);", 0, '');
							?>
                        </td>
                        <td>
                        ...
                        <td>
                            <?  
                              $shift = array(1=>"A",2=>"B",3=>"C");
                              echo create_drop_down("cbo_shift", 100, $shift, "", 1, "-- All --", 0, "", 0, ''); 
                            ?>
                        </td>
                        <td>
                        ...
                        <td>
                            ...
                            <input type="button" id="show_button" class="formbutton" style="width:80px"
                                   value="Show 2" onClick="fn_report_generated(2)"/>       
                        </td>
                    </tr>
                    <tr>
