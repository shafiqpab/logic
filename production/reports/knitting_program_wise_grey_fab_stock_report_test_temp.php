...
var tableFilters = 
	{
	   ...
	   //col: [13,15,17,18,19,20,21,22,23],
	   //col: [14,16,18,19,20,21,22,23,24],
	   //col: [15,17,19,20,21,22,23,24,25,26],
	   col: [15,17,20,21,22,23,24,25,26,27,28],
...
function fn_report_generated(type)
{
	...
	var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_job_no*txt_job_id*txt_order_no*hide_order_id*txt_booking_id*txt_booking_no*txt_internal_ref*txt_program_no*txt_date_from*txt_date_to*cbo_year_selection*cbo_value_with*cbo_get_upto_qnty*txt_qnty',"../../")+'&report_title='+report_title;
	freeze_window(3);
	http.open("POST","requires/knitting_program_wise_grey_fab_stock_report_controller_test_temp.php",true);
...
function openmypage_order()
{
	...
	var page_link='requires/knitting_program_wise_grey_fab_stock_report_controller_test_temp.php?action=order_no_search_popup&companyID='+companyID+'&buyer_name='+buyer_name;
...
function openmypage_job()
{
	...
	var page_link='requires/knitting_program_wise_grey_fab_stock_report_controller_test_temp.php?action=job_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id;
...
{ //alert(des_prod)
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/knitting_program_wise_grey_fab_stock_report_controller_test_temp.php?companyID='+companyID+'&po_id='+po_id+'&prog_no='+prog_no+'&booking_no='+booking_no+'&knit_source='+knit_source+'&mc_dia='+mc_dia+'&action='+action, 'Details View', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../');
}
...
{  
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/knitting_program_wise_grey_fab_stock_report_controller_test_temp.php?companyID='+companyID+'&po_id='+po_id+'&prog_no='+prog_no+'&booking_no='+booking_no+'&knit_source='+knit_source+'&action='+action, 'Details View', 'width=600px, height=350px,center=1,resize=0,scrolling=0','../');
}

function openmypage_issue_return(po_id,prog_no,booking_no,action)
{  
	var companyID = $("#cbo_company_name").val();
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/knitting_program_wise_grey_fab_stock_report_controller_test_temp.php?companyID='+companyID+'&po_id='+po_id+'&prog_no='+prog_no+'&booking_no='+booking_no+'&action='+action, 'Details View', 'width=700px, height=350px,center=1,resize=0,scrolling=0','../');
}
...
{ 
	var companyID = $("#cbo_company_name").val();
	var popup_width='600px';
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/knitting_program_wise_grey_fab_stock_report_controller_test_temp.php?companyID='+companyID+'&po_id='+po_id+'&prog_no='+prog_no+'&booking_no='+booking_no+'&knit_source='+knit_source+'&action='+action, 'Details View', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../');
}
...
{ //alert(des_prod)
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/knitting_program_wise_grey_fab_stock_report_controller_test_temp.php?companyID='+companyID+'&po_id='+po_id+'&prog_no='+prog_no+'&booking_no='+booking_no+'&action='+action, 'Details View', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../');
}
...
function openmypage_booking()
{
	...
	var cbo_year_id = $("#cbo_year_selection").val();
	//var cbo_month_id = $("#cbo_month").val();
	var page_link='requires/knitting_program_wise_grey_fab_stock_report_controller_test_temp.php?action=booking_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id;
...
<form id="knittingStatusReport_1">
    <div style="width:100%;" align="center">    
        <? echo load_freeze_divs ("../../",'');  ?>
         <h3 style="width:1290px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu(this.id,'content_search_panel','')">-Search Panel</h3> 
             ...
             <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr align="center">
                        <td> 
                            <?
                                echo create_drop_down( "cbo_company_name", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "- Select Company -", $selected, "load_drop_down( 'requires/knitting_program_wise_grey_fab_stock_report_controller_test_temp', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>
                        </td>
                        <td id="buyer_td">
                            <? 
                    ...
                    <tr>
                        <td colspan="11" align="center" width="95%"><? echo load_month_buttons(1); ?></td>
                    </tr>
                </tbody>
            </table>
        </fieldset>
