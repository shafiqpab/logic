...
<script>
	...
	{	
		...
		var page_link='requires/style_wise_cutting_and_input_report_controller.php?action=job_popup&buyer_name='+buyer_name+'&cbo_year='+cbo_year;
	...
	function fn_generate_report(type)
	{
		...
		var data="action=generate_report"+get_submitted_data_string('cbo_wo_company_name*cbo_location_name*cbo_floor_name*cbo_year*cbo_buyer_name*hidden_job_id*txt_date_from*txt_date_to',"../../")+'&type='+type+'&report_title='+report_title;
		 
		freeze_window(3);
		http.open("POST","requires/style_wise_cutting_and_input_report_controller.php",true);
	...
	function openmypage(po_break_down_id,item_id,action,country_id)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox','iframe','requires/style_wise_cutting_and_input_report_controller.php?po_break_down_id='+po_break_down_id+'&item_id='+item_id+'&action='+action+'&country_id='+country_id, popup_caption, popupWidth+'center=1,resize=0,scrolling=0','../');
	}	
	function openmypage_production_popup(search_string,type) 	 		 	 
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/style_wise_cutting_and_input_report_controller.php?search_string='+search_string+'&action='+action+'&type='+type, title, 'width='+popup_width+','+'height='+popup_height+',center=1,resize=0,scrolling=0','../');
            ...
            <table class="rpt_table" width="950" cellpadding="0" cellspacing="0" align="center" border="1" rules="all">
               <thead>                    
                       <tr>
                        ...
                        <th class="" width="130" >Cutting Floor</th>
                        <th width="60">Job Year</th>
                        <th width="130">Buyer Name</th>
                        <th class="must_entry_caption"  width="100">Job No</th>
                        <th class="must_entry_caption"  width="200">Cutting Date Range </th>
                ...
                <tr class="general">
                    <td align="center"> 
                        <?
                            echo create_drop_down( "cbo_wo_company_name", 130, "SELECT id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/style_wise_cutting_and_input_report_controller', this.value, 'load_drop_down_location', 'location_td' );load_drop_down( 'requires/style_wise_cutting_and_input_report_controller', this.value, 'load_drop_down_buyer', 'td_buyer' );" );
                        ?>
                    </td>
                    ...
                    <td align="center" id="floor_td"> 
                     ...
                     <td>
						...
						echo create_drop_down( "cbo_year", 60, $year,"", 1, "--All--", 0, "",0 );
						?>
                    </td>
