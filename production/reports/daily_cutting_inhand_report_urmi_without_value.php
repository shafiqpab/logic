...
<script>
	...
	var tableFilters = 
	{
 	...
 	function open_order_no()
	 {
	    ...
	    var page_link='requires/daily_cutting_inhand_report_urmi_without_value_controller.php?action=order_wise_search&buyer='+buyer+'&job_id='+job_id+'&job_no='+job_no+'&cbo_year='+cbo_year;
	...
	function open_job_no()
	{
		var buyer=$("#cbo_buyer_name").val();
		var cbo_year=$("#cbo_year").val();
	    var page_link='requires/daily_cutting_inhand_report_urmi_without_value_controller.php?action=job_popup&buyer='+buyer+'&cbo_year='+cbo_year;
		var title="Search Order Popup";
	...
	function fn_generate_report(type)
	{
		...
		//alert(data);return;
		freeze_window(3);
		http.open("POST","requires/daily_cutting_inhand_report_urmi_without_value_controller.php",true);
	...
	function openmypage_cutting_sewing_total(po,item,cutting,color,type,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/daily_cutting_inhand_report_urmi_without_value_controller.php?data='+data+'&action='+action, 'Remarks View', 'width=650px,height=450px,center=1,resize=0,scrolling=0','../');
	}
	function openmypage_ex_fac_total(po,item,color,action)
	{
		var data=po+'**'+item+'**'+color;
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/daily_cutting_inhand_report_urmi_without_value_controller.php?data='+data+'&action='+action, 'Remarks View', 'width=450px,height=250px,center=1,resize=0,scrolling=0','../');
	}
	
	 function openmypage_fab_issue(po_id,color,type,action)
	{
		var data=po_id+'_'+color+'_'+type;
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/daily_cutting_inhand_report_urmi_without_value_controller.php?action='+action+'&data='+data, 'Issue Popup', 'width=780px,height=450px,center=1,resize=0,scrolling=0','../');
	}
  ...
  function openmypage_swing(company_id,order_id,order_number,insert_date,type,action,width,height,resource,color_id,today_total)
	...
	function today_fab_recv_po(company_id,order_id,color_id,prod_date,type)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/daily_cutting_inhand_report_urmi_without_value_controller.php?company_id='+company_id+'&action='+action+'&order_id='+order_id+'&color_id='+color_id+'&prod_date='+prod_date, title, 'width='+popup_width+','+'height='+popup_height+',center=1,resize=0,scrolling=0','../');
	...
	function openmypage(po_break_down_id,item_id,action,country_id)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox','iframe','requires/daily_cutting_inhand_report_urmi_without_value_controller.php?po_break_down_id='+po_break_down_id+'&item_id='+item_id+'&action='+action+'&country_id='+country_id, popup_caption, popupWidth+'center=1,resize=0,scrolling=0','../');
	}	
	function openmypage_production_popup(po,item,color,type,day,action,title,popup_width,popup_height) 	 		 	 
	{

		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/daily_cutting_inhand_report_urmi_without_value_controller.php?po='+po+'&action='+action+'&item='+item+'&color='+color+'&type='+type+'&day='+day, title, 'width='+popup_width+','+'height='+popup_height+',center=1,resize=0,scrolling=0','../');
	...
	function openmypage_production_sewing_popup(po,item,color,txt_production_date,type,day,action,title,popup_width,popup_height) 	
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/daily_cutting_inhand_report_urmi_without_value_controller.php?po='+po+'&action='+action+'&item='+item+'&color='+color+'&type='+type+'&txt_production_date='+txt_production_date+'&day='+day, title, 'width='+popup_width+','+'height='+popup_height+',center=1,resize=0,scrolling=0','../');
               ...
               <thead>                    
                ...
                <tr class="general">
                    ...
                    <td>
                         ...
                         <input type="button" id="show_button" class="formbutton" style="width:90px" value="Cutting Status2" onClick="fn_generate_report(44)" />

                         <input type="button" id="show_button" class="formbutton" style="width:95px;" value="Cutt to Ship" onClick="fn_generate_report(5)" />   -->

                         <input type="button" id="show_button6" class="formbutton" style="width:105px" value="Production Status" onClick="fn_generate_report(6)" />                    
