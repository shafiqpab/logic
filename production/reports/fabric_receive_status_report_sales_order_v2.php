...
if ($_SESSION['logic_erp']['user_id'] == "") header("location:login.php");
	...
	<script>
		...
		function fn_report_generated(type) 
		{
			...
			freeze_window(3);
			http.open("POST", "requires/fabric_receive_status_report_sales_order_v2_controller.php", true);
		...
		function open_febric_receive_status_order_wise_popup(order_id, type, color, deter_id) 
		{
			...
			emailwindow = dhtmlmodal.open('EmailBox', 'iframe', 'requires/fabric_receive_status_report_sales_order_v2_controller.php?order_id=' + order_id + '&action=' + type + '&color=' + color + '&deter_id=' + deter_id, 'Detail Veiw', 'width=' + popup_width + ', height=450px,center=1,resize=0,scrolling=0', '../');
		}
		...
		function openmypage_job() 
		{
			...
			var page_link = 'requires/fabric_receive_status_report_sales_order_v2_controller.php?action=style_ref_search_popup&companyID=' + companyID + '&buyerID=' + buyerID + '&within_group=' + within_group;
			;
		...
		function openmypage_booking() {
			...
			var page_link = 'requires/fabric_receive_status_report_sales_order_v2_controller.php?action=booking_no_search_popup&companyID=' + companyID + '&cbo_within_group=' + cbo_within_group;
		...
		$(document).on("click", ".view_order", function (e) {
			...
			dhtmlmodal.open('EmailBox', 'iframe', 'requires/fabric_receive_status_report_sales_order_v2_controller.php?company_id=' + company_id + '&job_no=' + job_no + '&action=order_popup', 'Order Popup', 'width=640px,height=350px,center=1,resize=1,scrolling=0', '')
		});
	</script>
	<script type="text/javascript">
		...
		function fnc_fabric_sales_order_print(company_name, booking_id, booking_no, sales_job_no, entry_form, btn) 
		{
				...
				<fieldset style="width:970px;">
					...
					align="center">
						<thead>
							...
							<th colspan="2"><input type="reset" name="res" id="res" value="Reset" onClick="reset_form('fabricReceiveStatusReport_1','report_container*report_container2','','','')" class="formbutton" style="width:70px"/></th>
						</thead>
						<tbody>
							<tr class="general">
								<td>
									<?php
									echo create_drop_down("cbo_company_name", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/fabric_receive_status_report_sales_order_v2_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );");
									// get_php_form_data(this.value,'print_button_variable_setting','requires/fabric_receive_status_report_sales_order_v2_controller' );
									?>
								</td>
								<td id="buyer_td">
									<?php
								...
								<td>
									...
									<input type="button" id="show_button2" class="formbutton" style="width:70px;" value="Summary" onClick="fn_report_generated(2)"/>
									<!-- display: none; -->
								</td>
							</tr>
						</tbody>
