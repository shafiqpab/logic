...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../logout.php";
	...
	/*    var cbo_get_upto = $("#cbo_get_upto").val();
		...
		freeze_window(3);
		http.open("POST","requires/hourly_production_monitoring_report_v3_controller.php",true);
	...
	function openmypage_line()
	{
		...
		var page_link='requires/hourly_production_monitoring_report_v3_controller.php?action=line_search_popup&company='+company+'&location='+location+'&floor_id='+floor_id+'&txt_date='+txt_date; 
                    ...
                    <tr class="general">
                       <td>
							<? 
                                ...
                                echo create_drop_down( "cbo_company_id", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", 0, "load_drop_down( 'requires/hourly_production_monitoring_report_v3_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );load_drop_down( 'requires/hourly_production_monitoring_report_v3_controller', this.value, 'load_drop_down_location', 'location_td' );" );
                            ?>                            
                        </td>
                         <td>
                        ...
                        <? 
                            echo create_drop_down( "cbo_sewing_group", 110, "SELECT sewing_group as id,sewing_group from lib_sewing_line where status_active =1 and is_deleted=0 and sewing_group is not null   GROUP BY sewing_group order by sewing_group asc","sewing_group,sewing_group", 1, "-- Select Group --", $selected, "" );// $company_cond
                        ?>
                    </td>
                        <td id="location_td">
                            <? 
                        ...
                        <td align="left">
                            <input type="text" id="txt_parcentage" name="txt_parcentage" class="text_boxes_numeric" style="width:60px; text-align:left" value="60" />
                        </td>
                        <td>
                            <!--Not Use --hidden button> --> 
                            <input type="hidden" name="search" id="search" value="Show" onClick="generate_report(1)" style="width:60px" class="formbutton" />
                            <input type="button" name="search" id="search" value="V2" onClick="generate_report(2)" style="width:45px" class="formbutton" />
                            <input type="button" name="search" id="search" value="V3" onClick="generate_report(3)" style="width:45px" class="formbutton" />
                        </td>
                    </tr>
                </tbody>
            </table>
