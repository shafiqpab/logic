...
<script>
 	...
 	function open_order_no()
	{
		...
		let job_id=$("#hidden_job_id").val();
		let buyer=$("#cbo_buyer_name").val();
	    let page_link='requires/color_wise_rmg_production_status_v2_controller.php?action=order_popup&buyer='+buyer+'&job_id='+job_id+'&job_no='+job_no+'&company_id='+company_name;
		//alert(page_link);return; 
		let title="Search Order Popup";
		...
		emailwindow.onclose=function()
		{
			...
			let prodID=this.contentDoc.getElementById("selected_id").value;
			let po_data=prodID.split("_");
			$("#txt_order_no").val(po_data[1]);
			$("#hidden_order_id").val(po_data[0]); 
		}
	}
	...
	function open_job_no()
	{
		...
		let buyer=$("#cbo_buyer_name").val();
		let company_name=$("#cbo_company_name").val();
	    let page_link='requires/color_wise_rmg_production_status_v2_controller.php?action=job_popup&buyer='+buyer+'&company_id='+company_name;
		let title="Search Style Popup";
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=850px,height=390px,center=1,resize=0,scrolling=0','../')
		emailwindow.onclose=function()
		{
	...
	function fn_generate_report(type)
	{
		...
		freeze_window(3);
		http.open("POST","requires/color_wise_rmg_production_status_v2_controller.php",true);
	...
	function open_report_popup(arg,type,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/color_wise_rmg_production_status_v2_controller.php?data='+arg+'&type='+type+'&action='+action, title, 'width=450px,height=350px,center=1,resize=0,scrolling=0','../');
	}
	...
	function fnDisableCompany(com_id,value)
	{
		if(com_id=="cbo_work_company_name")
		{
			if(value!=0)
			{
		...
		else if(com_id=="cbo_company_name")
		{
			if(value!=0)
			{
				document.getElementById("cbo_work_company_name").disabled=true;
			}
			else
			{
				document.getElementById("cbo_work_company_name").disabled=false;
			}
		}
		// console.log(com_id);
	}
            ...
            <table class="rpt_table" width="1210" cellpadding="0" cellspacing="0" align="center" border="1" rules="all">
               <thead>                    
                ...
                <tr class="general">
                    <td align="center"> 
                        <?
                            echo create_drop_down( "cbo_company_name", 120, "SELECT id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 0, "-- Select --", $selected, "load_drop_down( 'requires/color_wise_rmg_production_status_v2_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );fnDisableCompany(this.id,this.value);" );
                        ?>
                    </td>
                    <td align="center"> 
                        <?
                            echo create_drop_down( "cbo_work_company_name", 120, "SELECT id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 0, "-- Select --", $selected, "fnDisableCompany(this.id,this.value);" );
                        ?>
                    </td>
                    ...
                    <td>
                        <?
							$shipment_status_arr=array(1=>"All",2=>"Full/Partial Pending",3=>"Full Delivery/Closed");
							// $shipment_status = array(1 => "Full Pending", 2 => "Partial Delivery", 3 => "Full Delivery/Closed");
							echo create_drop_down( "cbo_shipment_status", 120, $shipment_status_arr,"", 0, "-- Select --", 2, "",0,'','','','' );
						?>
                    </td>
