...
Purpose			: 	This Report will Create Fabric Finishing Info
Functionality	:	
JS Functions	:
...
function fn_dyeing_report_generated(operation)
{
    ...
    if(booking_number_show || cbo_buyer_name!=0 || batch_number_show ||  cbo_machine_id!=0 || cbo_floor_id!=0)
    {
        //alert(23);
    }
    else
    ...
    freeze_window(5);
    var data="action=report_generate&operation="+operation+get_submitted_data_string('cbo_company_name*cbo_buyer_name*batch_number*batch_number_show*booking_number*booking_number_show*cbo_type*cbo_year*txt_date_from*txt_date_to*txt_days*cbo_gate_upto*cbo_source*cbo_floor_id*cbo_location_id*cbo_machine_id',"../../")+'&report_title='+report_title;
    // alert(data);
    http.open("POST","requires/fabric_finishing_report_urmi_controller.php",true);
...
{ 
	...
	var page_link="requires/fabric_finishing_report_urmi_controller.php?action=bookingnumbershow&company_name="+company_name; 
...
{ 
	...
	var company_name=document.getElementById('cbo_company_name').value;
	var batch_number=document.getElementById('batch_number_show').value;
	var page_link="requires/fabric_finishing_report_urmi_controller.php?action=batchnumbershow&company_name="+company_name; 
...
<body onLoad="set_hotkey();">
	...
	<? echo load_freeze_divs ("../../",''); ?>
         ...
         <h3 style="width:1500px; margin-top:10px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu(this.id,'content_search_panel','')">-Search Panel</h3>  <div id="content_search_panel" >      
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr>
                            	<td>
                                <? // 9=>"Stentering";
								$process_format=array(0=>"ALL", 32 => 'Heat Setting', 47 => "Singeing", 30 => 'Slitting/Squeezing', 31 => 'Drying', 48 => "Stentering", 33 => 'Compacting',  68 => 'Brush', 67 => 'Peach', 35 => 'AOP', 194 => 'Chemical Finish');
								echo create_drop_down( "cbo_type",120, $process_format,"",0, "", "",'',0 );
								?>
                                </td>
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 120, "select id,company_name from lib_company where status_active=1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select Company --", 0, "load_drop_down('requires/fabric_finishing_report_urmi_controller', this.value, 'load_drop_down_location','location_td2');load_drop_down('requires/fabric_finishing_report_urmi_controller', this.value, 'load_drop_down_buyer', 'cbo_buyer_name_td2');" );
                                    ?>
                                ...
                                <td id="location_td2">
                                 ...
                                 <td id="machine_td">
                                	<?
                                        echo create_drop_down( "cbo_machine_id", 110, $blank_array,"", 1, "-- Select Machine --", 0, "" );
									?>
                                </td>
                            	...
                            	<td>
								<? 
									 $gate_upto=array(1=>"Greater Than",2=>"Less Than",3=>"Greater Than/Equal",4=>"Less Than/Equal",5=>"Equal");
									echo create_drop_down("cbo_gate_upto", 70, $gate_upto,"", 1, "-Select-", 0, "",0 ,"1,3","","","",""); ?>
                            	</td>

                            	 <td align="center">
                                     <input type="text" name="txt_days" id="txt_days" value="" class="text_boxes" style="width:50px" placeholder=""/> 
                                </td>
