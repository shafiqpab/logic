<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create QI Wise Report
Functionality	:	
JS Functions	:
...
<script>
	...
	{		
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_location_id*txt_style_ref_no*txt_operator_id*txt_date_from*txt_date_to',"../../../")+'&report_title='+report_title+'&type='+type;		
		
		// alert(data); return;
		freeze_window(3);
		http.open("POST","requires/qc_wise_date_and_style_inspection_report_controller.php",true);
    ...
    {
        var page_link='requires/qc_wise_date_and_style_inspection_report_controller.php?action=operator_popup';
        ...
        {				
            ...
            $("#txt_operator_id").val(employee_data[2]);	
        }
    }
	function open_style_no()
	{
	    ...
	    var page_link='requires/qc_wise_date_and_style_inspection_report_controller.php?action=style_ref_search_popup&company='+company+'&style=1'; 
		...
		emailwindow.onclose=function()
		{
			...
			var prodID=this.contentDoc.getElementById("hide_style_ref_no").value;
			//alert(prodID); // product ID
			var prodDescription=this.contentDoc.getElementById("hide_style_ref_no").value; // product Description
			$("#txt_style_ref_no").val(prodDescription);
			$("#hide_txt_style_ref_no_id").val(prodID); 
			

			//alert($("#txt_job_no").val())
		}
	 }
	 function openmypage_inspec_supp()
{
	...
	function openmypage_order_no() 
	{
		...
		var page_link='requires/qc_wise_date_and_style_inspection_report_controller.php?action=order_no_popup&lc_company='+lc_company+'&buyer='+buyer+'&job_no='+job_no;  
	...
	{ 
		...
		var page_link='requires/qc_wise_date_and_style_inspection_report_controller.php?action=lotratio_popup&company_id='+company_id; 
	...
	function fnc_bundelDtls(companyid,bundleNo,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/qc_wise_date_and_style_inspection_report_controller.php?companyid='+companyid+'&bundleNo='+bundleNo+'&action='+action, 'Details View', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../');
	}
         ...
         <fieldset style="width:930px;">
             <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
             	<thead>
                    ...
                    <th width="135">QC Name</th>
                   
					<th  width="" class="must_entry_caption" id="process_name">QC Date </th>
                    <th width="245">
                    	<input type="reset" name="res" id="res" value="Reset" style="width:140px" class="formbutton" onClick="reset_form( 'bundleTrackReport_1', 'report_container','','','')" />
                    </th>
                </thead>
                <tbody>
                    <tr class="general">
                        <td>
                        	<? echo create_drop_down( "cbo_company_id", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", 0, "load_drop_down( 'requires/qc_wise_date_and_style_inspection_report_controller', this.value, 'load_drop_down_location', 'location_td' );" ); ?>
						...
						<td>
                                <input type="text" id="txt_style_ref_no" name="txt_style_ref_no" style="width:90px" class="text_boxes" placeholder="Write/Browse" onDblClick="open_style_no();"/>
                            </td>   
							<td>
                            <input 
                        ...
                        <td>
                        	<input type="button" name="search1" id="search1" value="Show" onClick="generate_report(1);" style="width:50px" class="formbutton" />
