 ...
 function open_style_ref()
	 {
		...
		var page_link='requires/daily_production_report_controller.php?action=job_no_search_popup&company='+company+'&style=1'; 
	  ...
	  function open_job_no()
	 {
	    ...
	    var page_link='requires/daily_production_report_controller.php?action=job_no_search_popup&company='+company+'&style=0'; 
...
function generate_report(type)
	{
		...
		freeze_window(3);
		http.open("POST","requires/daily_production_report_controller.php",true);
...
function openmypage_embl(company_id,order_id,order_number,insert_date,type,action,width,height,embl_type,color_id)
	...
	function openmypage_buyer_ins(style,po_id,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/daily_production_report_controller.php?data='+data+'&action='+action, 'Inspection View', 'width=550px,height=300px,center=1,resize=0,scrolling=0','../../../');
	}
	...
	function print_report_button_setting(company_id)
	{
		...
		var report_ids=return_global_ajax_value(company_id, 'print_report_button_setting', '', 'requires/daily_production_report_controller');
            ...
            <table class="rpt_table" width="1300px" cellpadding="0" cellspacing="0" align="center">
               <thead>                    
                ...
                <tr class="general">
                    <td width="140"> 
                        <?
                            echo create_drop_down( "cbo_company_name", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/daily_production_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );print_report_button_setting(this.value)" );
                        ?>
                    </td>
                     <td width="140"> 
                        <?
