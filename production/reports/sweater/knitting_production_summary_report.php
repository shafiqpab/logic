 ...
 function open_style_ref()
	 {
		...
		var page_link='requires/knitting_production_summary_report_controller.php?action=job_no_search_popup&company='+company+'&style=1'; 
	  ...
	  function open_job_no()
	 {
	    ...
	    var page_link='requires/knitting_production_summary_report_controller.php?action=job_no_search_popup&company='+company+'&style=0'; 
	...
	function generate_report2(job_no_mst, gmt_item_id, color_id,size_id) 
	{
        print_report(job_no_mst + '**'+gmt_item_id+ '**' + color_id+ '**' + size_id, "requisition_print_two", "requires/knitting_production_summary_report_controller");
    }
	...
	function generate_report(type)
	{
		...
		if(txt_job_no.length==0 && txt_style_no.length==0)
		{
		...
		var data="action=generate_report"+get_submitted_data_string('cbo_company_name*cbo_working_company*cbo_buyer_name*txt_job_no*cbo_year_selection*txt_style_no*hidden_job_id*hidden_style_id*txt_date_from*cbo_status',"../../../")+'&report_title='+report_title+'&type='+type;
		freeze_window(3);
		http.open("POST","requires/knitting_production_summary_report_controller.php",true);
	...
	{	
		if(http.readyState == 4) 
		{
			...
			var tableFilters = {
				col_operation: {
				id: ["order_qty_total","lot_ratio_qty_total","bundle_qnty_total","lot_ratio_weight_total","knitting_issue","knitting_issue_bal","knitting_receive","knitting_receive_weight","balance"],
			...
			var tableFilters2 = {
				col_operation: {
				id: ["order_qty_total","lot_ratio_qty_total","bundle_qnty_total","lot_ratio_weight_total","knitting_issue","knitting_issue_bal","knitting_receive","balance"],
			...
			if(reponse[2]==2)
			{
				setFilterGrid("scroll_body",-1,tableFilters2);
			}
...
function openmypage_embl(company_id,order_id,order_number,insert_date,type,action,width,height,embl_type,color_id)
	...
	function openmypage_buyer_ins(style,po_id,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/knitting_production_summary_report_controller.php?data='+data+'&action='+action, 'Inspection View', 'width=550px,height=300px,center=1,resize=0,scrolling=0','../../../');
	}
	...
	function print_report_button_setting(company_id)
	{
		...
		var report_ids=return_global_ajax_value(company_id, 'print_report_button_setting', '', 'requires/knitting_production_summary_report_controller');
            ...
            <table class="rpt_table" width="1300px" cellpadding="0" cellspacing="0" align="center">
               <thead>                    
                ...
                <tr class="general">
                    <td width="140"> 
                        <?
                            echo create_drop_down( "cbo_company_name", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/knitting_production_summary_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                        ?>
                    </td>
                     <td width="140"> 
                        <?
                    ...
                    <td width="">
                       <input type="text" id="txt_job_no" name="txt_job_no"  style="width:120px" class="text_boxes" onDblClick="open_job_no();" placeholder="Browse" readonly />
                       <input type="hidden" id="hidden_job_id"  name="hidden_job_id" />
                    </td>
                    <td width="" id="location_td">
                     <input type="text" id="txt_style_no" name="txt_style_no"  style="width:120px" class="text_boxes" onDblClick="open_style_ref();" placeholder="Browse" readonly />
                    ...
                    <input type="hidden" name="txt_date_to" id="txt_date_to" class="datepicker" style="width:55px"  placeholder="To Date"  ></td>
                   
                  
                    <td width="180">
                        <input type="button" id="show_button" class="formbutton" style="width:80px;" value="Show" onClick="generate_report(1)" />
						<input type="button" id="show_button" class="formbutton" style="width:80px;" value="Access" onClick="generate_report(2)" />
