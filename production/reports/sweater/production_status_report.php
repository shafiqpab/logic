<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Production Status Sweater Report .
Functionality	:	
JS Functions	:
Created by		:	Shafiq
Creation date 	: 	1-03-2023
Updated by 		: 		
Update date		: 		   
QC Performed BY	:		
QC Date			:	
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Production Status Sweater Report ", "../../../", 1, 1,$unicode,1,1);
?>	
...
function open_style_ref()
{
	...
	var page_link='requires/production_status_report_controller.php?action=job_no_search_popup&company='+company+'&style=1'; 
	...
	emailwindow.onclose=function()
		{
			...
			var styleDescription=this.contentDoc.getElementById("hide_job_no").value;
			var jobYear=this.contentDoc.getElementById("hide_job_year").value;
			console.log(poID);
			console.log(styleDescription);
			$("#txt_style_no").val(styleDescription);
			$("#hidden_style_id").val(poID); 
			$("#hidden_year").val(jobYear); 
		}
	}	
	...
	function open_job_no()
	{
	    ...
	    var page_link='requires/production_status_report_controller.php?action=job_no_search_popup&company='+company+'&style=0'; 
	...
	{
		let gg = $("#cbo_gg").val();
		let job_id = $("#txt_job_no").val();
		let style_id = $("#hidden_style_id").val();
		if(gg==0 && job_id=="" && style_id=="")
		{
		...
		var data="action=generate_report"+get_submitted_data_string('cbo_company_name*cbo_gg*cbo_buyer_name*txt_job_no*cbo_year_selection*txt_style_no*hidden_job_id*hidden_style_id*txt_date_from*cbo_status*txt_date_to*cbo_date_type*hidden_year',"../../../")+'&report_title='+report_title+'&type='+type;
		freeze_window(3);
		http.open("POST","requires/production_status_report_controller.php",true);
...
function openmypage_embl(company_id,order_id,order_number,insert_date,type,action,width,height,embl_type,color_id)
	...
	function openmypage_buyer_ins(style,po_id,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/production_status_report_controller.php?data='+data+'&action='+action, 'Inspection View', 'width=550px,height=300px,center=1,resize=0,scrolling=0','../../../');
	}
	...
	function print_report_button_setting(company_id)
	{
		...
		var report_ids=return_global_ajax_value(company_id, 'print_report_button_setting', '', 'requires/production_status_report_controller');
            ...
            <table class="rpt_table" width="1300px" cellpadding="0" cellspacing="0" align="center">
               <thead>                    
                ...
                <tr class="general">
                    <td width="140"> 
                        <?
                            echo create_drop_down( "cbo_company_name", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/production_status_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );print_report_button_setting(this.value)" );
                        ?>
                    </td>
                    ...
                    <td width="" id="location_td">
                     ...
                     <td>
						<?
                        echo create_drop_down( "cbo_gg", 110, $gauge_arr,"", 1, "-- All --", 0, "",0,"" );
                     	?>
                     </td>
				  ...
				  <td>
					<?
					  ...
					  echo create_drop_down( "cbo_status", 110, $shipment_status,"", 1, "-- All --", 0, "",0,"" );
				  ?>
				  </td>
                  
				  <td> 
					<?
					  $date_type=array(1=>"Pub Shipment Date",2=>"Ex-factory Date");
					  echo create_drop_down( "cbo_date_type", 110, $date_type,"", 1, "-- All --", 0, "",0,"" );
				  ?>
				  </td>
