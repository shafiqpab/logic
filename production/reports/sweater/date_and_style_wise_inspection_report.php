...
<script>
	...
	{		
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_location_id*cbo_buyer_id*txt_job_no*txt_style_ref_no*hide_job_id*txt_date_from*txt_date_to',"../../../")+'&report_title='+report_title+'&type='+type;		
		
		// alert(data); return;
		freeze_window(3);
		http.open("POST","requires/date_and_style_wise_inspection_report_controller.php",true);
	...
	function open_job_no()
	{
	    ...
	    var page_link='requires/date_and_style_wise_inspection_report_controller.php?action=job_no_search_popup&company='+company+'&style=0'; 
	...
	function openmypage_order_no() 
	{
		...
		var page_link='requires/date_and_style_wise_inspection_report_controller.php?action=order_no_popup&lc_company='+lc_company+'&buyer='+buyer+'&job_no='+job_no;  
	...
	{ 
		...
		var page_link='requires/date_and_style_wise_inspection_report_controller.php?action=lotratio_popup&company_id='+company_id; 
	...
	function fnc_bundelDtls(companyid,bundleNo,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/date_and_style_wise_inspection_report_controller.php?companyid='+companyid+'&bundleNo='+bundleNo+'&action='+action, 'Details View', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../');
	}
    ...
    function showChart(styleName, defectQty, rejectQty) 
    {
        ...
        Highcharts.chart('report_container2', {
            ...
            title: 
            {
                text: 'Date and Style wise Inspection Report Chart',
                style:
                    {
                        color: 'black',
                        fontSize: '22px',
    ...
    function showChart2(styleName, effiQty, defectQty, rejectQty) 
    {
        // $("#chart_container").show('fast');
        var style_name_arr = styleName.split('__');
        // var style_name_arr = style_name_arr.toString();
        var effi_total_arr = effiQty.split('__');
        var effi_total_arr = effi_total_arr.map(Number);
        ...
        Highcharts.chart('report_container2', {
            ...
            subtitle: 
            {
                ...
                align: 'center',
                y: 40,
                text: '<b>Date wise Efficiency%, Alter% And Defect%</b>' 
            },
            plotOptions: 
            {
                column: 
            ...
            yAxis: 
            {
                title: 
                {
                    text: 'Date wise Efficiency%, Alter% And Refect%',
                    style:
                    {
                        color: 'black',
                        fontSize: '14px',
    ...
    function fn_on_change()
	{
		...
		load_drop_down( 'requires/date_and_style_wise_inspection_report_controller', cbo_company_name, 'load_drop_down_buyer', 'buyer_td' );
	...
	{
		//$('#data_panel').html('');
		get_php_form_data($('#cbo_company_id').val(),'print_report_button_setting','requires/date_and_style_wise_inspection_report_controller' ); 
	}
    function print_report_format(report_ids) 
	{
		$('#search1').hide();
		$('#search2').hide();
		$('#search3').hide();
            ...
            <fieldset style="width:1050px;">
                <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                    <thead>
                        ...
                        <th  width="200" class="must_entry_caption" id="process_name"> Date </th>
                        <th width="auto">
                    ...
                    <tbody>
                        <tr class="general">
                            ...
                            <td>
                                <input type="button" name="search1" id="search1" value="Date and Style" onClick="generate_report(1);" style="width:100px;display: none;" class="formbutton" />

                                <input type="button" name="search2" id="search2" value="Style Graph" onClick="generate_report(2);" style="width:80px;display: none;" class="formbutton" />

                                <input type="button" name="search5" id="search5" value="Style wise" onClick="generate_report(5);" style="width:70px;display: none;" class="formbutton" />
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="button" name="search3" id="search3" value="Date Wise" onClick="generate_report(3);" style="width:70px;display: none;" class="formbutton" />
                                <input type="button" name="search4" id="search4" value="Date Graph" onClick="generate_report(4);" style="width:80px;display: none;" class="formbutton" />
