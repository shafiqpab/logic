...
<script>
	...
	function generate_report(type)
	{
		...
		if(type==2){
			...
			var data="action=report_summery_generate"+get_submitted_data_string('cbo_company_id*cbo_buyer_id*txt_job_no*txt_style_ref_no*hiden_order_id*txt_date_from*txt_date_to*cbo_wo_company_name*cbo_ship_status',"../../../")+'&report_title='+report_title+'&type='+type;
		}else{

		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_buyer_id*txt_job_no*txt_style_ref_no*hiden_order_id*txt_date_from*txt_date_to*cbo_wo_company_name*cbo_ship_status',"../../../")+'&report_title='+report_title+'&type='+type;
		}
		
		// alert(data); return;
		freeze_window(3);
		http.open("POST","requires/date_wise_linking_receive_report_controller.php",true);
	...
	function openmypage_job_no() // For Line number
	{
		...
		var page_link='requires/date_wise_linking_receive_report_controller.php?action=openJobNoPopup&companyID='+company+'&buyer_name='+buyer;  
	...
	function openmypage_order_no() 
	{
		...
		var buyer = $("#cbo_buyer_id").val();
		var job_no = $("#txt_job_no").val();
		var page_link='requires/date_wise_linking_receive_report_controller.php?action=order_no_popup&lc_company='+lc_company+'&buyer='+buyer+'&job_no='+job_no;  
	...
	{ 
		...
		var page_link='requires/date_wise_linking_receive_report_controller.php?action=lotratio_popup&company_id='+company_id; 
	...
	function fnc_bundelDtls(companyid,bundleNo,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/date_wise_linking_receive_report_controller.php?companyid='+companyid+'&bundleNo='+bundleNo+'&action='+action, 'Details View', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../');
	}
         ...
         <fieldset style="width:1000px;">
             <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
             	<thead>
					...
					<th  width="160" class="must_entry_caption">Linking Receive Date </th>
