...
function fn_report_generated(type)
{
	...
	var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_store_name*cbo_buyer_name*txt_job_no*txt_job_id*txt_order_no*hide_order_id*txt_booking_id*txt_booking_no*txt_internal_ref*txt_program_no*txt_date_from*txt_date_to*cbo_year_selection*cbo_value_with*cbo_get_upto_qnty*txt_qnty',"../../")+'&report_title='+report_title;
	freeze_window(3);
...
{  
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/knitting_program_wise_grey_fab_stock_report_controller.php?companyID='+companyID+'&po_id='+po_id+'&prog_no='+prog_no+'&booking_no='+booking_no+'&knit_source='+knit_source+'&action='+action, 'Details View', 'width=600px, height=350px,center=1,resize=0,scrolling=0','../');
}

function openmypage_issue_return(po_id,prog_no,booking_no,action)
{  
	var companyID = $("#cbo_company_name").val();
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/knitting_program_wise_grey_fab_stock_report_controller.php?companyID='+companyID+'&po_id='+po_id+'&prog_no='+prog_no+'&booking_no='+booking_no+'&action='+action, 'Details View', 'width=700px, height=350px,center=1,resize=0,scrolling=0','../');
}
...
{ //alert(des_prod)
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/knitting_program_wise_grey_fab_stock_report_controller.php?companyID='+companyID+'&po_id='+po_id+'&prog_no='+prog_no+'&booking_no='+booking_no+'&action='+action, 'Details View', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../');
}
             ...
             <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr align="center">
                        <td> 
                            <?
                                echo create_drop_down( "cbo_company_name", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name","id,company_name", 1, "- Select Company -", $selected, "load_drop_down( 'requires/knitting_program_wise_grey_fab_stock_report_controller', this.value, 'load_drop_down_store', 'store_td' );load_drop_down( 'requires/knitting_program_wise_grey_fab_stock_report_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>
                        </td>
                        <td id="store_td">
                            <? 
