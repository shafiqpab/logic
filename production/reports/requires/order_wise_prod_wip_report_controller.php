...
if($action=="search_list_view")
{
	...
	/*$sql_input=sql_select( "SELECT a.id as job_id,b.job_no_mst, b.id from wo_po_details_master a, wo_po_break_down b where a.id=b.job_id and  b.id=c.po_break_down_id and a.status_active=1   and a.company_name=$company_id and c.production_type=4 $search_cond $buyer_id_cond $job_no_cond $job_year_cond  $date_cond" );
...
{ 
	   ...
	   /*	$sql_input=" SELECT b.id as po_id,a.id as job_id,d.production_type,d.embel_name,e.production_qnty,e.reject_qty,e.alter_qty,e.spot_qty
		...
		/==========================================================================================*/
		 ...
		 $sql_po=" SELECT $year_field, a.id as job_id,a.job_no,a.order_uom,a.set_smv,a.gmts_item_id, a.job_no_prefix_num, a.company_name, a.buyer_name, a.style_ref_no,b.id,b.po_number,b.pub_shipment_date,b.unit_price,d.order_total as po_total_price,d.plan_cut_qnty,d.order_quantity as po_pcs_qty,d.item_number_id,d.color_number_id,d.size_number_id,c.sew_effi_percent,c.costing_per,c.exchange_rate
			from wo_po_details_master a,wo_po_break_down b,wo_pre_cost_mst c,wo_po_color_size_breakdown d where a.id=b.job_id  and  a.id=c.job_id and  b.job_id=c.job_id  and  d.job_id=c.job_id and b.id=d.po_break_down_id  and b.is_confirmed in(1) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0   and d.status_active=1 and d.is_deleted=0  $sql_cond $buyer_id_cond $job_no_cond $job_year_cond";		
		 // echo $sql_po; 
		...
		//========================Main Query End============***********************
		if(count($sql_res_po)==0) //Check here
		{
		...
		/==========================================================================================*/ 	
		 ...
		 /*  $sqlLay="SELECT e.order_id as poid,d.job_no,
		 (case when   d.entry_date<'$start_date'    then d.fabric_width end) as opening_cutlay,
		(case when    d.other_fabric_weight is not null   then d.other_fabric_weight end) as cutlay_qty,
		...
		(case when   $cutlay_recv_date_cond   then d.other_fabric_weight end) as other_cutlay_qty
		  ...
		  $sqlLay="SELECT d.job_no as JOB_NO,
		
		(case when    d.other_fabric_weight is not null   then d.other_fabric_weight end) as cutlay_qty,
		(case when   d.entry_date<'$start_date'    then d.other_fabric_weight end) as other_opening_cutlay,
		(case when   $cutlay_recv_date_cond   then d.other_fabric_weight end) as other_cutlay_qty
		  from wo_po_details_master a,ppl_cut_lay_mst d,ppl_cut_lay_bundle e where a.job_no=d.job_no  and d.id=e.mst_id and a.status_active=1 and a.is_deleted=0  and d.status_active=1 and d.is_deleted=0   and e.status_active=1 and e.is_deleted=0  and d.other_fabric_weight is not null $poId_no_cond group by d.other_fabric_weight,d.job_no,d.entry_date order by d.entry_date desc";		
		...
		foreach ($layData as  $val) 
		{
			...
			//$po_cut_lay_data_arr[$val['POID']]['fab_wgt']+=$val['OTHER_CUTLAY_QTY'];
			//$po_cut_lay_data_arr[$val['POID']]['opening_fab_wgt']+=$val['OTHER_OPENING_CUTLAY'];
		...
		$sql_po_job=" SELECT b.id as poid,a.id as job_id,a.job_no,d.order_total as po_total_price,d.plan_cut_qnty,d.order_quantity as po_pcs_qty,d.item_number_id,d.color_number_id,c.costing_per
		...
		foreach( $sql_po_job_result as $row )
		{
			...
			$po_qty_data_for_lay_arr[$row['POID']]['po_qty']+=$row['PO_PCS_QTY'];
		}
		unset($sql_po_job_result);
   ...
   $sql_cm_new = "select b.id as po_id,(b.po_quantity*a.total_set_qnty) as pcs_qty,c.job_no,c.cm_cost, c.cm_cost_percent from wo_pre_cost_dtls c,wo_po_details_master a,wo_po_break_down b where a.id=c.job_id and a.id=b.job_id  and b.job_id=c.job_id  and c.status_active=1 and c.is_deleted=0  and b.status_active=1 and b.is_deleted=0  and a.status_active=1 and a.is_deleted=0 $job_no_cond";
	...
	//print_r($po_wise_cm_cost_arr);
		...
		//============$job_no_cond_pre = where_con_using_array($job_no_arr,0,"a.job_id");
		  ...
		  $po_id_cond_prod = where_con_using_array($po_id_arr,0,"d.po_break_down_id");
		   $po_id_emb_cond = where_con_using_array($po_id_arr,0,"c.po_break_down_id");
		...
		unset($gmtsitemRatioSqlRes);
			...
			////PO_BREAK_DOWN_ID,REQUIRMENT,po_break_down_id,requirment
		$sqlfab_emb="select b.job_id AS JOB_ID, c.po_break_down_id AS POID,  c.requirment AS REQUIRMENT
		from wo_pre_cost_embe_cost_dtls b, wo_pre_cos_emb_co_avg_con_dtls c
		where 1=1 and b.id=c.pre_cost_emb_cost_dtls_id and c.requirment!=0 and b.emb_name=3 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  $po_id_emb_cond";
	 //	echo $sqlfab_emb;  
		$sqlEmbRes = sql_select($sqlfab_emb);
		foreach($sqlEmbRes as $row)
		{
			$PreQtyWashArr[$row['POID']]['pre_wash']+=$row['REQUIRMENT'];
		}
	...
	//	$cuting_qc_data_array[$val['PO_ID']]['cut_qty'] +=$val['PROD_QTY'];
		...
		(case when   c.entry_form in(17)  then c.quantity end) as rec_qty,
		(case when    c.entry_form in(17)  then (c.quantity*b.cons_rate) end) as recv_amount,
		(case when   b.transaction_date<'$start_date' and  c.entry_form in(19)   then c.quantity end) as opening_issue_quantity,
		(case when   b.transaction_date<'$start_date' and  c.entry_form in(19)   then (c.quantity*b.cons_rate) end) as opening_issue_val,
		...
		if($val['OPENING_ISSUE_VAL'])
		{
		$opening_fin_rcv_data_array[$val['PO_ID']]['fin_val'] +=$val['OPENING_ISSUE_VAL'];
		}
		if($val['ISSUE_QUANTITY_PCS'])
		{
		...
		if($val['REC_QTY'])
		{
		$fin_issue_data_array[$val['PO_ID']]['fin_qty'] +=$val['REC_QTY'];
		$fin_issue_data_array[$val['PO_ID']]['fin_val'] +=$val['RECV_AMOUNT'];
		}
	...
	$sqlSew="SELECT  a.company_id, a.location, a.floor_id, a.prod_reso_allo, a.production_date, a.sewing_line,b.buyer_name  as buyer_name,b.style_ref_no,b.job_no, a.po_break_down_id, a.item_number_id,c.po_number as po_number,c.file_no,c.unit_price,c.grouping as ref,d.color_type_id,a.remarks,e.floor_serial_no,sum(d.production_qnty) as good_qnty,"; 
		...
		WHERE a.production_type=5 and a.po_break_down_id=c.id and c.job_id=b.id and a.id=d.mst_id and e.id=a.floor_id and a.status_active=1 and a.is_deleted=0 and b.is_deleted=0 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and a.company_id=$company_name   $date_cond_prod_sewout $po_id_cond_po
	...
	//=====================End=====================================TOT_SEW_IN_PROD_QTY,TOT_SEW_OUT_PROD_QTY
		
		   $sql_prod=" SELECT  d.production_date,d.po_break_down_id as po_id,
		 (case when  d.production_type in(1) then e.production_qnty end) as tot_cut_prod_qty,
		 (case when  d.production_type in(4) then e.production_qnty end) as tot_sew_in_prod_qty,
		(case when   d.production_type in(5) then e.production_qnty end) as tot_sew_out_prod_qty,
		...
		from pro_garments_production_mst d,pro_garments_production_dtls e  where  d.id=e.mst_id and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and d.production_type in(1,2,3,4,5,8) and e.production_type in(1,2,3,4,5,8) and e.production_qnty>0   $po_id_cond_prod";		
		  // echo $sql_prod;
		...
		//$production_date=date($val['production_date']);
		$cuting_qc_data_array[$val['PO_ID']]['tot_cut_qty'] +=$val['TOT_CUT_PROD_QTY'];
		$sew_data_array[$val['PO_ID']]['tot_sew_in'] +=$val['TOT_SEW_IN_PROD_QTY'];
		$sew_data_array[$val['PO_ID']]['tot_sew_out'] +=$val['TOT_SEW_OUT_PROD_QTY'];
		...
		$sewing_inputQty_arr[$val['PO_ID']]['sew_in']+=$val['SEW_IN_PROD_QTY'];
		...
		$opening_sew_data_array[$val['PO_ID']]['opening_sew_out_qty'] +=$val['OPENING_SEW_OUT_PROD_QTY'];
		$opening_wash_data_array[$val['PO_ID']]['opening_issue_wash_qty']+= $val['OPENING_WASH_ISSUE_PROD_QTY'];
		$opening_wash_data_array[$val['PO_ID']]['opening_recv_wash_qty']+= $val['OPENING_WASH_RECV_PROD_QTY'];
		...
		$opeing_wash_rcv_sew_out=$wash_sew_out=$opeing_sew_out=0;
		if($val['OPENING_WASH_RECV_PROD_QTY']>0) 
			$opeing_wash_rcv_sew_out=$val['OPENING_WASH_RECV_PROD_QTY'];
		
		if($val['OPENING_SEW_OUT_PROD_QTY']>0) 
			$opeing_sew_out=$val['OPENING_SEW_OUT_PROD_QTY'];
		
		if($val['WASH_RECV_PROD_QTY']) $wash_sew_out=$val['WASH_RECV_PROD_QTY'];
		else $wash_sew_out=$val['SEW_OUT_PROD_QTY'];
		//echo $val['OPENING_WASH_RECV_PROD_QTY'].'='.$val['SEW_OUT_PROD_QTY'].'<br>';
		 
		
		//$opening_fin_data_array[$val['PO_ID']]['opening_fin_qty']+=$opeing_wash_sew_out-$val['OPENING_FIN_PROD_QTY'];
		if($opeing_wash_rcv_sew_out>0)
		{
			//echo $opeing_wash_rcv_sew_out.'<br>';
		$opening_fin_data_array[$val['PO_ID']]['opening_recv_fin_qty']+=$opeing_wash_rcv_sew_out;
		}
		$opening_fin_data_array[$val['PO_ID']]['opening_sew_out_qty']+=$opeing_sew_out;
		$opening_fin_data_array[$val['PO_ID']]['opening_finish_qty']+=$val['OPENING_FIN_PROD_QTY'];
		...
		foreach($job_qty_data_arr as $job_key=>$row) 
		{
			 ...
			 // $opening_layQty=$Cut_lay_data_arr[$job_key]['opening_fab_wgt'];
			  ...
			  $opening_used_fab_wgt=0;
			   ...
			   //Cut_lay_data_arr
			   $po_qty=$po_value=$po_wise_cm_cost=$opening_fin_rcv=$fin_rcv=$fin_rcv_val=$tot_fab_fin_req_qty=$tot_fab_fin_req_qty=$opening_fin_val=$cutting_used_fab_val=0;
			   $shipDate="";$fin_issue=$fin_issue_val=0;
			  ...
			  foreach($po_idArr as $poId)
			  {
				...
				$po_qty=$po_qty_data_for_lay_arr[$poId]['po_qty'];
				//$fab_wgt=$po_cut_lay_data_arr[$poId]['fab_wgt'];
				//$opening_layQty=$po_cut_lay_data_arr[$poId]['opening_fab_wgt'];
				 if($opening_layQty>0)
				 {
				 $opening_used_fab_wgt+=($opening_layQty/$job_qty)*$po_qty;
				 }
				 if($fab_wgt>0)
				  {
				  $cutting_used_fab_wgt+=($fab_wgt/$job_qty)*$po_qty;
				  }
				 
				 //echo $po_cut_lay.'d'.$job_qty.'<br>';
				...
				$fin_issue+=$fin_issue_data_array[$poId]['fin_qty'];
				$fin_issue_val+=$fin_issue_data_array[$poId]['fin_val'];
			  ...
			  // echo $fin_rcv_val.'D'.$fin_rcv.'<br>';;
			   	$fin_rcv_avg_rate=0;
				  if($fin_issue_val>0)
				  {
					 $fin_rcv_avg_rate=$fin_issue_val/$fin_issue; 
				  }
				 ...
				 if($fin_rcv_avg_rate>0)
				  ...
				  // echo $opening_used_fab_wgt.'='.$fin_rcv_avg_rate.'<br>';
				  
				   $opening_fin_val=$opening_fin_rcv*$dfin_rcv_avg_rate;
				  
				  if($fin_rcv_avg_rate>0)//$opening_used_fab_val= $opening_used_fab_wgt*$fin_rcv_avg_rate;
				  {
				  $opening_used_fab_val= $opening_used_fab_wgt*$fin_rcv_avg_rate;
				  }
				  else $opening_used_fab_val=0;
				  
				  if($job_key=='OG-23-00045')
				  {
					//echo $opening_used_fab_val.'='.$fin_rcv_avg_rate.'<br>';  
				  }
				  $fin_rcv_val=0;
				   $fin_rcv_val=$fin_rcv*$fin_rcv_avg_rate;
				   
			$fab_clsoing_qty=0;	  
			 $fab_clsoing_qty=(($opening_fin_rcv-$opening_used_fab_wgt)+$fin_rcv)-$cutting_used_fab_wgt;
			// if( $fab_clsoing_qty>0)  $fab_clsoing_qty=$fab_clsoing_qty;else  $fab_clsoing_qty=0;
		//	echo $fin_rcv_avg_rate.'D';
			 if($fin_rcv_avg_rate>0)
			 {
			 $fab_clsoing_val=$fab_clsoing_qty*$fin_rcv_avg_rate;//(($opening_fin_val-$opening_used_fab_val)+$fin_rcv_val)-$cutting_used_fab_val;
			 }
			 else $fab_clsoing_val=0;
			
			 
			
			 // $fab_clsoing_qty=($opening_fin_rcv+$fin_rcv)-$cutting_used_fab_wgt;
			//  $fab_clsoing_val=($opening_fin_val+$fin_rcv_val)-$cutting_used_fab_val;
			//if($opening_fin_rcv-$opening_used_fab_wgt>0)
			//{
			$summary_prod_wip_wipArr[1]['cutting_stock_opening_qty']+=$opening_fin_rcv-$opening_used_fab_wgt;
			//}
			//if($opening_fin_rcv-$opening_used_fab_wgt>0)
			//{
			$summary_prod_wip_wipArr[1]['cutting_stock_opening_val']+=($opening_fin_rcv-$opening_used_fab_wgt)*$fin_rcv_avg_rate;
			//}
...
// Summary For Cutting Stock (Finished Fabric) WIP
		...
		foreach($prod_data_arr as $poId=>$row)
		{
			...
			{
				$cfab_rate=$grey_fab_avg_rate*$job_ex_rate;
			}
			else
			{
				$cfab_rate=$grey_fab_avg_rate;
			}
			//echo $fab_rate.'='.$avg_fab_cons.'<br>';
			
			  $cut_panel_rate=$cfab_rate*$avg_fab_cons;
			  $cutting_in_hand=($opening_cut_qty+$cuting_qcQty)-$sewing_inputQty_arr[$poId]['sew_in'];
			  if($cutting_in_hand>0)  $cutting_in_hand=$cutting_in_hand;else  $cutting_in_hand=0;
			//  echo $opening_cut_qty.'='.$cuting_qcQty.'='.$sewing_inputQty_arr[$poId]['sew_in'].'<br>';
			  if($cut_panel_rate>0)
			  {
			...
			if($emb_stock_qty>0)  $emb_stock_qty=$emb_stock_qty;else  $emb_stock_qty=0;
			...
			//=========Sewing Stock=================
			$gbl_rate3=0;$sew_amt=0;$trim_rate_sew=0;$ex_rate=0;$sew_qty=0;$style_wiseSewingOut=0;$avg_avilable_min=0;$avg_avilable_min=0;$sum_sew_rate=0;	$sew_stock_qty=0;$opening_sew_in_qty=0;$currency_id=0;$job_qty=0;$opening_sew_out_qty=0;
			$opening_sew_out_qty=$opening_sew_data_array[$poId]['opening_sew_out_qty'];
			$opening_sew_in_qty=$opening_sew_data_array[$poId]['opening_sew_in_qty']-$opening_sew_out_qty;
			...
			if($sew_stock_qty>0)  $sew_stock_qty=$sew_stock_qty;else  $sew_stock_qty=0;
			...
			$opening_issue_wash_qty=0;
			$opening_issue_wash_qty=$opening_wash_data_array[$poId]['opening_issue_wash_qty']-$opening_wash_data_array[$poId]['opening_recv_wash_qty'];
			//if($opening_issue_wash_qty>0)  $opening_issue_wash_qty=$opening_issue_wash_qty;else  $opening_issue_wash_qty=0;
		...
		//	if($wash_stock_qty>0)  $wash_stock_qty=$wash_stock_qty;else  $wash_stock_qty=0;
			...
			//$opening_fin_qty=$opening_fin_data_array[$poId]['opening_fin_qty'];
			 $opening_recv_fin_qty=0;$opening_finish_qty=0;
		$opening_recv_fin_qty=	$opening_fin_data_array[$poId]['opening_recv_fin_qty'];
		
		$opening_sew_out_qty=$opening_fin_data_array[$poId]['opening_sew_out_qty'];
		$opening_finish_qty=$opening_fin_data_array[$poId]['opening_finish_qty'];
		$pre_wash_cons_summ=0;$sew_out_qty_chk=0;
		$pre_wash_cons_summ=$PreQtyWashArr[$poId]['pre_wash'];
		//echo $pre_wash_cons.'<br>';
		$sew_out_qty_chk=$sew_data_array[$poId]['sew_out_qty'];
		if($pre_wash_cons_summ>0) //For Wash in Budget Found
		{
			 $opening_recv_fin_qty=$opening_recv_fin_qty;
		}
		else
		{
			 $opening_recv_fin_qty=$opening_sew_out_qty;
		}
		
		/*if($opening_recv_fin_qty>0)
		{
			$opening_recv_fin_qty=$opening_recv_fin_qty;
		...
		else 
		{
			if($pre_wash_cons_summ>0) //For Wash in Budget Found
			{ 
		  	 $opening_recv_fin_qty=0;
			}
			else $opening_recv_fin_qty=$opening_sew_out_qty;
		}*/
		// echo $opening_recv_fin_qty.'='.$opening_sew_out_qty.'<br>';
		
		
			
		$opening_fin_qty=0;
		$opening_fin_qty=$opening_recv_fin_qty-$opening_finish_qty; 
		//if($opening_fin_qty>0) $opening_fin_qty=$opening_fin_qty;else $opening_fin_qty=0;
		//echo $opening_recv_fin_qty.'='.$opening_finish_qty.'<br>';
			...
			$sew_out_qty=$wash_data_array[$poId]['sew_out_qty'];
			 if($pre_wash_cons_summ>0) //For Wash in Budget Found
			{
				$wash_sew_qty=$recv_wash_qty;
			}
			else
			{
				$wash_sew_qty=$sew_out_qty_chk;
			}
			...
			//  echo $trim_rate_fin.'='.$gbl_rate5.'<br>';
			...
			//if($fin_stock_qty>0)  $fin_stock_qty=$fin_stock_qty;else  $fin_stock_qty=0;
			//if($opening_fin_qty>0)
			//{
			...
			//========== Gmts Finished Goods====================
			$ex_rate=0;$g_fin_rate=0;$gbl_rate7=0;$wo_gfin_amt=0;$wo_gfin_qty=0;$sew_in_qty=$sew_out_qty=0;$job_qty=0;$currency_id=0;
			...
			if($currency_id==2)
			{
				if($wo_gfin_amt>0 && $wo_gfin_qty>0)
				{
			...
			$g_fin_rate= $trim_rate_fin+$gbl_grate7;
			$gbl_rate8_arr[$poId]=$gbl_grate7;
			  // echo $poId.'='.$trim_rate_fin.'<br>';
			   if($poId==71146)
				 {
					 //echo $g_fin_rate.'='.$trim_rate_fin.'='.$gbl_grate7.'<br>';
				 }
			// $opening_sew_in_qty=$opening_sew_data_array[$val['PO_ID']]['opening_sew_in_qty'];
			...
			// if($clsoing_stock_qty>0) $clsoing_stock_qty=$clsoing_stock_qty;else $clsoing_stock_qty=0;
			$summary_prod_wip_wipArr[8]['cutting_stock_opening_qty']+=$opening_fin_bal_qty;
			if($g_fin_rate>0)
			{
			$summary_prod_wip_wipArr[8]['cutting_stock_opening_val']+=$opening_fin_bal_qty*$g_fin_rate;
			}
			$summary_prod_wip_wipArr[8]['cutting_stock_recv_qty']+=$fin_qty;
			if($g_fin_rate>0)
			{
			$summary_prod_wip_wipArr[8]['cutting_stock_recv_val']+=$fin_qty*$g_fin_rate;
			}
			$summary_prod_wip_wipArr[8]['cutting_stock_issue_qty']+=$exfact_qty;
			if($g_fin_rate>0)
			{
			$summary_prod_wip_wipArr[8]['cutting_stock_issue_val']+=$exfact_qty*$g_fin_rate;
			}
			...
			$summary_prod_wip_wipArr[8]['cutting_stock_clsoing_val']+=$clsoing_stock_qty*$g_fin_rate;
			}
			//=====================End=============
		}
		...
		?>
			<div style="width:<?=$tbl_width;?>px; margin-left:5px;">
                ...
                <h3 style="width:<? echo $summary_tbl_width;?>px; float:left"  id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'summary_table_id', '')"> +<b>Summary</b></h3>
                
                <div id="summary_table_id" style="width:<?=$summary_tbl_width;?>px; float:left;">
                    ...
                    <caption> <b style="float:left;font-size:18px;"></b></caption>
						<thead>
							<tr>
                           		<th colspan="4" >&nbsp;</th>								
							...
							?>
                                  ...
                                  <td width="100" align="right" title="PreviousIssueQty-PreviousCuttingUSEDQty"><? echo number_format($row['cutting_stock_opening_qty'],2);?></td>
                            ...
                            <tr style="font-weight: bold; font-size: 12px; text-align:right; background-color:#ccc">
				 ...
				 <h3 style="width:<? echo $tbl_width-20;?>px; float:left"  id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'dtls_fin_table_id', '')"> +<b>Cutting Stock (Finished Fabric) WIP </b></h3>
                <div  id="dtls_fin_table_id" style="width:<?=$tbl_width-20;?>px; float:left;">
               
					<table cellspacing="0" cellpadding="0"  border="1" style="border-collapse: collapse;" rules="all"  width="<?=$tbl_width-20;?>" class="rpt_table" align="left">
                    <caption> <b style="float:left;"></b></caption>
						<thead>
							<tr>
								<th width="30">SL#</th>								
								...
								<th width="100" title="">Order Value</th>
								<th width="60">SMV</th>
								<th width="60">Effi%</th>
								...
								<th width="100" title="">Fabric <br>Closing Qty</th>
								<th width="" title="Cutting+Print+Emb+Sewing+Finishing">Fab.Closing<br>Value Tk.</th>
							</tr>								
							</thead>
                        </table> 
                     
                   <div style=" max-height:380px; width:<?=($tbl_width-20)+18;?>px; overflow-y:scroll;" id="scroll_body">
                    <table class="rpt_table" id="table_body" width="<?=($tbl_width-20);?>" cellpadding="0" cellspacing="0" border="1" rules="all" align="">
                     <tbody>
                        <?
						//$job_qty_data_arr[$val['JOB_NO']]['job_qty']+=$val['PO_PCS_QTY'];
			//$job_qty_data_arr[$val['JOB_NO']]['po_ids'].=$val['ID'].",";
						...
						foreach($job_qty_data_arr as $job_key=>$row) 
						{
							 ...
							 $po_ids=rtrim($row['po_ids'],',');
							  ...
							  if($shipDate=="") $shipDate=$prod_data_arr[$poId]['ship_date'];else $shipDate.=",".$prod_data_arr[$poId]['ship_date'];
							   $po_value+= $prod_data_arr[$poId]['po_value'];			
								...
								$c_po_qty=$po_qty_data_for_lay_arr[$poId]['po_qty'];
								//$fab_wgt=$po_cut_lay_data_arr[$poId]['fab_wgt'];
								//$opening_layQty=$po_cut_lay_data_arr[$poId]['opening_fab_wgt'];
								 
								if($opening_layQty>0)
								 {
									 $opening_used_fab_wgt+=($opening_layQty/$job_qty)*$c_po_qty;
								 }
								 if($fab_wgt>0)
								 {
									  $cutting_used_fab_wgt+=($fab_wgt/$job_qty)*$c_po_qty;
								 }
							  ...
							  //echo $shipDate.'D';
								...
								$dfin_rcv_avg_rate=0;
							   	  if($fin_issue_val>0)
								  {
									 $dfin_rcv_avg_rate=$fin_issue_val/$fin_issue; 
								  }
							 	 if($dfin_rcv_avg_rate>0)
								  {
								  $cutting_used_fab_val= $cutting_used_fab_wgt*$dfin_rcv_avg_rate;
								  }
								  else $cutting_used_fab_val=0;
								  
								  if($dfin_rcv_avg_rate>0)
								  {
								  $opening_used_fab_val= $opening_used_fab_wgt*$dfin_rcv_avg_rate;
								  }
								  else $opening_used_fab_val=0;
							...
							// if($fab_clsoing_qty>0) $fab_clsoing_qty=$fab_clsoing_qty;else $fab_clsoing_qty=0;
							 ...
							 if($dfin_rcv_avg_rate>0)
							 {
							 $fab_clsoing_val=$fab_clsoing_qty*$dfin_rcv_avg_rate;//(($opening_fin_val-$opening_used_fab_val)+$fin_rcv_val)-$cutting_used_fab_val;
							 }
							 else  $fab_clsoing_val=0;
							// $fab_clsoing_qty=($opening_fin_rcv+$fin_rcv)-$cutting_used_fab_wgt;
							// $fab_clsoing_val=($opening_fin_val+$fin_rcv_val)-$cutting_used_fab_val;
							  ...
							  $fin_rcv_val=0;
							  $fin_rcv_val=$fin_rcv*$dfin_rcv_avg_rate;
							 ...
							 //echo $opening_fin_rcv.'-'.$opening_used_fab_wgt.'<br>';
							   $opeinging_bal_qty=0;
							 $opeinging_bal_qty=$opening_fin_rcv-$opening_used_fab_wgt;
							//if($opeinging_bal_qty>0)
							//{
								//$opeinging_bal_qty=$opeinging_bal_qty;	
							//}
							//else $opeinging_bal_qty=0;
							$opeinging_bal_val=0;
							 $opeinging_bal_val=$opening_fin_val-$opening_used_fab_val;
							 
							// if($opeinging_bal_val>0)
							//{
								$opeinging_bal_val=$opeinging_bal_val;	
							//}
							//else $opeinging_bal_val=0;
                            ...
                            <td  width="100"><p style="word-wrap: break-word;word-break: break-all;"><? echo $row['style']; ?></p></td>
                            <td style="word-wrap: break-word;word-break: break-all;" width="100"><p><? echo $po_nos; ?></p></td>
                            <td style="word-wrap: break-word;word-break: break-all;" width="100"><p><a href="##" onClick="openmypage_job_popup(<? echo "'".$company_name."','".$job_key."','".$po_idss."'"; ?>,'JobopowisePopup')"><? echo $job_key; ?></a></p></td> 
                            <td style="word-wrap: break-word;word-break: break-all;" width="60"><? echo $row['year']; ?></td>
                            <td style="word-wrap: break-word;word-break: break-all;" width="100"><p><? echo $item_name; ?></p></td>
                            <td style="word-wrap: break-word;word-break: break-all;" width="80"><? echo change_date_format($ship_Dates); ?></td>
                            <td style="word-wrap: break-word;word-break: break-all;" align="right" width="80"><? echo number_format($po_qty,0); ?></td>
                            <td style="word-wrap: break-word;word-break: break-all;" align="right" width="60"><? echo number_format($po_value/$po_qty,2); ?></td>
                            <td style="word-wrap: break-word;word-break: break-all;" align="right" width="100"><? echo number_format($po_value,2); ?></td> 
                            <td style="word-wrap: break-word;word-break: break-all;" align="right"  width="60"><? echo $row['set_smv']; ?></td>
                            <td style="word-wrap: break-word;word-break: break-all;" align="right" width="60"><? echo number_format($row['sew_eff'],2); ?></td> 
                            <td style="word-wrap: break-word;word-break: break-all;" align="right" width="80"><?  echo number_format($po_wise_cm_cost,2); ?></td> 
                            <td style="word-wrap: break-word;word-break: break-all;" title="PreviousIssueQty(<?=$opening_fin_rcv;?>)-PreviousCuttingUSEDQty(<?=$opening_used_fab_wgt;?>)" align="right" width="80"><? echo number_format($opeinging_bal_qty,2); ?></td> 
                             
                            <td style="word-wrap: break-word;word-break: break-all;"title="PreviousIssueAmt(<?=$opening_fin_val;?>)-CuttingUSEDAmt(<?=$opening_used_fab_val;?>)"  align="right" width="80"><? 
							$opeinging_bal_val=0;
							$opeinging_bal_val=$opeinging_bal_qty*$dfin_rcv_avg_rate;
							
							echo number_format($opeinging_bal_val,2); ?></td> 
                            <td style="word-wrap: break-word;word-break: break-all;" align="right" width="80"><?php echo number_format($fin_rcv,2);?></td> 
                            <td style="word-wrap: break-word;word-break: break-all;" align="right"  title="AvgRate=<?=$dfin_rcv_avg_rate;?>" width="80"><?php echo number_format($fin_rcv_val,2);?></td> 
                            ...
                            <td style="word-wrap: break-word;word-break: break-all;" title="AvgRate=<?=$dfin_rcv_avg_rate;?>" align="right"  width="80"><? echo number_format($cutting_used_fab_val,2); ?></td> 
                            <td  style="word-wrap: break-word;word-break: break-all;" title="OpeingFinQty+FabRecvQty-CuttingUsedWgt" align="right"  width="100"><? echo number_format($fab_clsoing_qty,2); ?></td> 
                            
                            <td style="word-wrap: break-word;word-break: break-all;"  title="ClosingQty(<?=$fab_clsoing_qty;?>)*AvgRate(<?=$dfin_rcv_avg_rate;?>)"  align="right"  width=""><? echo number_format($fab_clsoing_val,2); ?></td> 
                            </tr>
                            <?
							$ff++;
							...
							//if($opeinging_bal_val>0)
							//{
							$total_opening_fin_rcv_val+=$opeinging_bal_val;
							//}
							//if($opeinging_bal_qty>0)
							//{
							$total_opening_fin_rcv_qty+=$opeinging_bal_qty;
							//}
							...
							?>
							 ...
							 <tr style="font-weight: bold; font-size: 12px; text-align:right; background-color:#ccc">
								...
								<td><?=number_format($total_po_qty_pcs,0);?></td>
                                <td>&nbsp;</th>
								<td><?=number_format($total_po_value,2);?></td>
								<td>&nbsp;</td>
								<td>&nbsp;</td>
								<td><?=number_format($total_cm_cost,2);?></td>
								<td><?=number_format($total_opening_fin_rcv_qty,2);?></td>
                              
                                <td><?=number_format($total_opening_fin_rcv_val,2);?></td>
                             	<td><?=number_format($total_fin_rcv_qty,2);?></td>
								<td><?=number_format($total_fin_rcv_val,2);?></td>
								<td><?=number_format($total_cutting_used_fab_wgt,2);?></td>
								<td><?=number_format($total_cutting_used_fab_val,2);?></td>
								<td><?=number_format($total_fab_clsoing_qty,2);?></td>
								<td><?=number_format($total_fab_clsoing_val,2);?></td>
            ...
            <br>
             <h3 style="width:<? echo $tbl_width;?>px; float:left"  id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'dtls_cutting_table_id', '')"> +<b>Cutting Stock (Cut Pannel) WIP</b></h3>
            <div id="dtls_cutting_table_id" style="width:<?=$tbl_width;?>px; float:left;">
					<table cellspacing="0" cellpadding="0"  border="1" style="border-collapse: collapse;" rules="all"  width="<?=$tbl_width;?>" class="rpt_table" align="left">
                      ...
                      <div style=" max-height:390px; width:<?=$tbl_width+18;?>px; overflow-y:scroll;" id="scroll_body2">
                    <table class="rpt_table" id="table_body2" width="<?=$tbl_width;?>" cellpadding="0" cellspacing="0" border="1" rules="all" align="">
                     <tbody>
						...
						foreach($prod_data_arr as $poId=>$row)
						{
                            ...
                            <td style="word-wrap: break-word;word-break: break-all;" align="right" width="80"><? echo number_format($row['po_qty'],0); ?></td>
                            <td style="word-wrap: break-word;word-break: break-all;" align="right" width="60"><? echo number_format($row['rate'],2); ?></td>
                            <td style="word-wrap: break-word;word-break: break-all;" align="right" width="80"><? echo number_format($row['po_value'],2); ?></td> 
                            <td style="word-wrap: break-word;word-break: break-all;" align="right"  width="60"><? echo $row['set_smv']; ?></td>
                            <td style="word-wrap: break-word;word-break: break-all;" align="right" width="60"><? echo number_format($row['sew_eff'],2); ?></td> 
                            ...
                            <td style="word-wrap: break-word;word-break: break-all;" align="right"  title="Rate(Cut Panel)*PreviousCutQty" width="80"><?php  if($opening_cut_qty>0 && $cut_panel_rate>0) echo number_format($opening_cut_qty*$cut_panel_rate,2);else echo "";?></td> 
                            ...
                            <td  style="word-wrap: break-word;word-break: break-all;" title="Opening Cut+CutingQty-Sew in" align="right"  width="80"><? echo number_format($cutting_in_hand,2); ?></td> 
                            <td style="word-wrap: break-word;word-break: break-all;" align="right"  width=""><? if($cutting_in_hand>0 && $cut_panel_rate>0) echo number_format($cutting_in_hand*$cut_panel_rate,2);else echo ""; ?></td> 
                            </tr>
                            <?
							$cp++;
							...
							?>
							 ...
							 <tr style="font-weight: bold; font-size: 12px; text-align:right; background-color:#ccc">
								...
								<td>&nbsp;</td>
								<td><?=number_format($total_cm_cost,2);?></td>
								<td><?=number_format($total_avg_fab_cons,4);?></td>
                                <td>&nbsp; </td>
                                <td><?=number_format($total_opening_cut_qty,2);?></td>
                             	<td>&nbsp;</td>
								<td><?=number_format($total_opening_cut_val,2);?></td>
								<td><?=number_format($total_cuting_qcQty,2);?></td>
								<td><?=number_format($total_inputQty,2);?></td>
								<td><?=number_format($total_cutting_in_hand,2);?></td>
								<td><?=number_format($total_cutting_in_hand_val,2);?></td>
							</tr>
                              </tbody>
            ...
            <br>
            <h3 style="width:<? echo $tbl_width;?>px; float:left"  id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'dtls_print_table_id', '')"> +<b>Sub Contract Print  Factory-WIP Stock Report </b></h3>
            <div id="dtls_print_table_id" style="width:<?=$tbl_width;?>px; float:left;">
					<table cellspacing="0" cellpadding="0"  border="1" style="border-collapse: collapse;" rules="all"  width="<?=$tbl_width;?>" class="rpt_table" align="left">
                    <caption> <b style="float:left;"></b></caption>
						<thead>
							<tr>
								...
								<th width="80" title="">Print Stock<br> Qty (Pcs)</th>
								<th width="" title="">Print Stock<br> Value (Pcs)</th>
							</tr>								
							</thead>
                        </table>
                     
                      <div style=" max-height:390px; width:<?=$tbl_width+18;?>px; overflow-y:scroll;" id="scroll_body3">
                     ...
                     <tbody>
						...
						foreach($prod_data_arr as $poId=>$row)
						{
							 ...
							 if ($sub%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
							  ...
							  $job_qty=$job_qty_data_arr[$row['job_no']]['job_qty'];
							 // $cuting_qcQty= $cuting_qc_data_array[$poId]['cut_qty'];
							  $cuting_qcQty= $cuting_qc_data_array[$poId]['tot_cut_qty'];
							  $gbl_rate=$gbl_rate_arr[$poId];
							 $print_rate= $embl_print_data_array[$poId]['print_rate']+$gbl_rate;
							 $gbl_rate2_arr[$poId]=$print_rate;
                            ...
                            <td style="word-wrap: break-word;word-break: break-all;" align="right"  title="Rate(Print WIP)*OpeningIssueQty" width="80"><?php if($opening_issue_print_qty>0 && $print_rate>0) echo number_format($opening_issue_print_qty*$print_rate,0);?></td> 
                            ...
                            <td style="word-wrap: break-word;word-break: break-all;" align="right"  width=""><? if($emb_stock_qty>0 && $print_rate>0) echo number_format($emb_stock_qty*$print_rate,2); ?></td> 
							...
							?>
							 ...
							 <tr style="font-weight: bold; font-size: 12px; text-align:right; background-color:#ccc">
								...
								<td>&nbsp;</td>
								<td><?=number_format($total_cm_cost,2);?></td>
								<td><? //number_format($total_avg_fab_cons,0);?></td>
                                <td><?=number_format($total_cuting_qcQty,2);?></td>
                                <td> </td>
                             	<td><?=number_format($total_opening_issue_print_qty,2);?></td>
								<td><?=number_format($total_opening_issue_print_val,2);?></td>
								<td><?=number_format($total_print_issue_qty,2);?></td>
								<td><?=number_format($total_print_recv_qty,2);?></td>
								<td><?=number_format($total_emb_stock_qty,2);?></td>
								<td><?=number_format($total_emb_stock_val,2);?></td>
			 ...
			 ?>
              <h3 style="width:<? echo $tbl_width_emb;?>px; float:left"  id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'dtls_embro_table_id', '')"> +<b>Sub Contract Embroidary  Factory-WIP Stock Report </b></h3>
            <div id="dtls_embro_table_id" style="width:<?=$tbl_width_emb;?>px; float:left;">
                    ...
                    <caption> <b style="float:left;"> </b></caption>
                      ...
                      <div style=" max-height:390px; width:<?=$tbl_width_emb+18;?>px; overflow-y:scroll;" id="scroll_body4">
                    <table class="rpt_table" id="table_body4" width="<?=$tbl_width_emb;?>" cellpadding="0" cellspacing="0" border="1" rules="all" align="">
                     <tbody>
						...
						foreach($prod_data_arr as $poId=>$row)
						{
                            ...
                            <td style="word-wrap: break-word;word-break: break-all;" width="100"><? echo $item_name; ?></td>
                            <td style="word-wrap: break-word;word-break: break-all;" width="80"><? echo change_date_format($row['ship_date']); ?></td>
                            <td style="word-wrap: break-word;word-break: break-all;" align="right" width="80"><? echo number_format($row['po_qty'],2); ?></td>
                            <td style="word-wrap: break-word;word-break: break-all;" align="right" width="60"><? echo number_format($row['rate'],2); ?></td>
                            ...
                            <td style="word-wrap: break-word;word-break: break-all;" title=""  align="right" width="80"><? if($opening_embro_issue_qty>0 && $embro_rate>0) echo number_format($opening_embro_issue_qty*$embro_rate,2); ?></td> 
                            ...
                            <td style="word-wrap: break-word;word-break: break-all;" align="right"   width=""><? if($emb_stock_qty>0 && $embro_rate>0)  echo number_format($emb_stock_qty*$embro_rate,2); ?></td> 
							...
							<tr style="font-weight: bold; font-size: 12px; text-align:right; background-color:#ccc">
								...
								<td><?=number_format($total_cuting_qcQty,2);?></td>
                                <td><? //number_format($total_avg_fab_cons,0);?></td>
                                <td><?=number_format($total_opening_embro_issue_qty,2);?></td>
                                <td><?=number_format($total_opening_embro_val,2);?></td>
                             	<td><?=number_format($total_embro_issue_qty,2);?></td>
								<td><?=number_format($total_embro_recv_qty,2);?></td>
								<td><?=number_format($total_emb_stock_qty,2);?></td>
								<td><?=number_format($total_emb_stock_val,2);?></td>
							</tr>
                             </tbody>
            ...
            // $tbl_width_emb=$tbl_width-80;
			 ?>
              <h3 style="width:<? echo $tbl_width_emb;?>px; float:left"  id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'dtls_sewing_table_id', '')"> +<b>Sewing-WIP Stock Report </b></h3>
            <div id="dtls_sewing_table_id" style="width:<?=$tbl_width_emb;?>px; float:left;">
					<table cellspacing="0" cellpadding="0"  border="1" style="border-collapse: collapse;" rules="all"  width="<?=$tbl_width_emb;?>" class="rpt_table" align="left">
                    <caption> <b style="float:left;"> </b></caption>
						<thead>
							<tr>
                   ...
                   <div style=" max-height:390px; width:<?=$tbl_width_emb+18;?>px; overflow-y:scroll;" id="scroll_body5">
                     ...
                     <tbody>
						...
						foreach($prod_data_arr as $poId=>$row)
						{
							 ...
							 if ($sew_w%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
							  ...
							  $gbl_rate3=0;$sew_in_qty=0;$ex_rate=0;$sew_out_qty=0;$trim_rate_sew=0;$avg_avilable_min=0;$avg_avilable_min=0;$sew_rate=0;$sew_amt=0;$sew_qty=0;$style_wiseSewingOut=0;$opening_sew_in_qty=0;$currency_id=0;$job_qty=0;$cuting_qcQty=0;$opening_sew_out_qty=0;
							  $opening_sew_out_qty=$opening_sew_data_array[$poId]['opening_sew_out_qty'];
							  $opening_sew_in_qty=$opening_sew_data_array[$poId]['opening_sew_in_qty']-$opening_sew_out_qty;
							  $job_qty=$job_qty_data_arr[$row['job_no']]['job_qty'];
							 // $cuting_qcQty= $cuting_qc_data_array[$poId]['cut_qty'];
						...
						//	echo $po_wise_used_minArr[$poId].'D=<br>';
                            ...
                            <td style="word-wrap: break-word;word-break: break-all;" title="Rate=<?=$sewRate_cal;?>" align="right" width="80"><? echo number_format($sew_rate,2); ?></td> 
                             <td style="word-wrap: break-word;word-break: break-all;" title="Previous Sew In(<?=$opening_sew_data_array[$poId]['opening_sew_in_qty'];?>)-Opeing Sew Out(<?=$opening_sew_out_qty;?>)" align="right" width="80"><? echo number_format($opening_sew_in_qty,2);//$unit_of_measurement[$row['uom']]; ?></td>
                            ...
                            <td style="word-wrap: break-word;word-break: break-all;" align="right"   width=""><? if($sew_stock_qty>0 && $sew_rate>0) echo number_format($sew_stock_qty*$sew_rate,2); ?></td> 
                            </tr>
                            <?
							$sew_w++;
							$total_po_qty_pcs+=$row['po_qty'];
							...
							<tr style="font-weight: bold; font-size: 12px; text-align:right; background-color:#ccc">
								...
								<td><?=number_format($total_cuting_qcQty,2);?></td>
                                <td><? //number_format($total_avg_fab_cons,0);?></td>
                                <td><?=number_format($total_opening_sew_in_qty,2);?></td>
                                 <td><?=number_format($total_opening_sew_in_val,2);?></td>
                             	<td><?=number_format($total_sew_in_qty,2);?></td>
								<td><?=number_format($total_sew_out_qty,2);?></td>
								<td><?=number_format($total_sew_stock_qty,2);?></td>
								<td><?=number_format($total_sew_stock_val,2);?></td>
			 ...
			 ?>
              <h3 style="width:<? echo $tbl_width_wash;?>px; float:left"  id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'dtls_wash_table_id', '')"> +<b>Sub Contract Washing Factory-WIP Stock Report </b></h3>
            <div id="dtls_wash_table_id" style="width:<?=$tbl_width_wash;?>px; float:left;">
                    ...
                    <caption> <b style="float:left;"> </b></caption>
                      ...
                      <div style=" max-height:390px; width:<?=$tbl_width_wash+18;?>px; overflow-y:scroll;" id="scroll_body6">
                     ...
                     <tbody>
						...
						foreach($prod_data_arr as $poId=>$row)
						{
							 ...
							 if ($ww%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
							  ...
							  $cuting_qcQty=0;$opening_issue_wash_qty=0;
							$opening_issue_wash_qty=$opening_wash_data_array[$poId]['opening_issue_wash_qty']-$opening_wash_data_array[$poId]['opening_recv_wash_qty'];
							//if($opening_issue_wash_qty>0)  $opening_issue_wash_qty=$opening_issue_wash_qty;else  $opening_issue_wash_qty=0;
							$issue_wash_qty=$wash_data_array[$poId]['issue_wash_qty']; 
							$recv_wash_qty=$wash_data_array[$poId]['recv_wash_qty'];
							...
							$gbl_rate5_arr[$poId]=$wash_rate;
							$sew_in_qty=$sew_out_qty=0;
							$sew_in_qty=$sew_data_array[$poId]['tot_sew_in'];//$sew_data_array[$poId]['sew_in_qty'];
							$sew_out_qty=$sew_data_array[$poId]['tot_sew_out'];
							$wash_stock_qty=0;
							$wash_stock_qty= ($opening_issue_wash_qty+$issue_wash_qty)-$recv_wash_qty;
							//if($wash_stock_qty>0)  $wash_stock_qty=$wash_stock_qty;else  $wash_stock_qty=0;
							?>
                            ...
                            <td style="word-wrap: break-word;word-break: break-all;" title="Prev Wash issue<?=$opening_wash_data_array[$poId]['opening_recv_wash_qty'];?>-Previous Recv<?=$opening_wash_data_array[$poId]['opening_issue_wash_qty'];?>" align="right" width="80"><?php echo number_format($opening_issue_wash_qty,2);?></td> 
                            <td style="word-wrap: break-word;word-break: break-all;" align="right"  title="" width="80"><?php  if($wash_rate>0) echo number_format($opening_issue_wash_qty*$wash_rate,2);?></td> 
                            ...
                            <td style="word-wrap: break-word;word-break: break-all;" align="right" title="Opening Wash Issue(<?=$opening_issue_wash_qty;?>)+Wash Issue Qty(<?=$issue_wash_qty;?>)-Wash Recv" width="80"><? echo number_format($wash_stock_qty,2); ?></td> 
                            
                            <td style="word-wrap: break-word;word-break: break-all;" align="right"   width=""><? if($wash_rate>0) echo number_format($wash_stock_qty*$wash_rate,2); ?></td> 
                            </tr>
                            <?
							$ww++;
							$total_po_qty_pcs+=$row['po_qty'];
						...
						<tr style="font-weight: bold; font-size: 12px; text-align:right; background-color:#ccc">
                                ...
                                <td><?=number_format($total_sew_out_qty,2);?></td>
                                <td><? //number_format($total_avg_fab_cons,0);?></td>
                             	<td><?=number_format($total_opening_wash_qty,2);?></td>
								<td><?=number_format($total_opening_wash_val,2);?></td>
								<td><?=number_format($total_issue_wash_qty,2);?></td>
								<td><?=number_format($total_recv_wash_qty,2);?></td>
                                <td><?=number_format($total_wash_stock_qty,2);?></td>
                                <td><?=number_format($total_wash_stock_val,2);?></td>
							</tr>
                             </tbody>
             ...
             $tbl_width_wash=$tbl_width+120;
			 ?>
             <h3 style="width:<? echo $tbl_width_wash;?>px; float:left"  id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'dtls_gmt_fin_table_id', '')"> +<b>Gmts Finishing Stock-WIP </b></h3>
            <div id="dtls_gmt_fin_table_id" style="width:<?=$tbl_width_wash;?>px; float:left;">
					<table cellspacing="0" cellpadding="0"  border="1" style="border-collapse: collapse;" rules="all"  width="<?=$tbl_width_wash;?>" class="rpt_table" align="left">
                    <caption> <b style="float:left;"></b></caption>
						<thead>
							<tr>
                      ...
                      <div style=" max-height:390px; width:<?=$tbl_width_wash+18;?>px; overflow-y:scroll;" id="scroll_body7">
                    <table class="rpt_table" id="table_body7" width="<?=$tbl_width_wash;?>" cellpadding="0" cellspacing="0" border="1" rules="all" align="">
                     <tbody>
						...
						foreach($prod_data_arr as $poId=>$row)
						{
							...
							$opening_sew_out_qty=$opening_fin_data_array[$poId]['opening_sew_out_qty'];
							$opening_finish_qty=$opening_fin_data_array[$poId]['opening_finish_qty']; 
							$pre_wash_cons=0;	$sew_out_qty_chk=0;
							$pre_wash_cons=$PreQtyWashArr[$poId]['pre_wash'];
							
							// echo $sew_out_qty_chk.'TT';
		
							/*if($opening_recv_fin_qty>0)
							{
								$opening_recv_fin_qty_chk=$opening_recv_fin_qty;
							
							}
							else 
							{
									if($pre_wash_cons>0) //For Wash in Budget Found
									{
										 $opening_recv_fin_qty_chk=0;
									}
									else
									{
										 $opening_recv_fin_qty_chk=$opening_sew_out_qty;
									}
									...
									else
									{
										...
										//echo $sew_out_qty_chk.'Dyy';
									
								$opening_fin_qty=0;$fin_qty=0;
							$opening_fin_qty=$opening_recv_fin_qty_chk-$opening_finish_qty;
							//echo $opening_recv_fin_qty.'='.$opening_sew_out_qty.'='.$opening_finish_qty.'<br>';
							
							  $fin_qty=$fin_data_array[$poId]['fin_qty']; 
							//  $recv_wash_qty=$wash_data_array[$poId]['recv_wash_qty'];
							$gbl_rate5=0;$ex_rate=0;$trim_rate_fin=0;$currency_id=0;$cuting_qcQty=0;
							$recv_wash_qty=$wash_data_array[$poId]['recv_wash_qty'];
							//$sew_out_qty_chk=$wash_data_array[$poId]['sew_out_qty'];
							...
							/*if($recv_wash_qty>0)
							{
							 ...
							 else 
							 {
									...
									{
								  		$wash_sew_qty=$sew_out_qty;
									}
							 }*/
							// if($pre_wash_cons=="") $pre_wash_cons=0;
									...
									// echo $recv_wash_qty.'='.$sew_out_qty.'='.$sew_out_qty_chk.'<br>';
							...
							if($currency_id==2)
							{
								...
								{
									 $trim_rate_fin=($wo_fin_amt/$wo_fin_qty)*$ex_rate;
									 //echo $wo_fin_amt.'='.$wo_fin_qty.'*'.$ex_rate.'<br>';
								}
							}
							else {
									if($wo_fin_amt>0 && $wo_fin_qty>0)
							 ...
							 $sew_in_qty=$sew_out_qty=0;
							$sew_in_qty=$sew_data_array[$poId]['tot_sew_in'];
							$sew_out_qty=$sew_data_array[$poId]['tot_sew_out'];
							$sew_stock_qty=0;
                            ...
                            <td style="word-wrap: break-word;word-break: break-all;" width="100"><? echo $buyer_arr[$row['buyer']]; ?></td>
                            <td style="word-wrap: break-word;word-break: break-all;" width="100"><? echo $row['style']; ?></td>
                            <td style="word-wrap: break-word;word-break: break-all;" title="PoId-<? echo $poId;?>" width="100"><? echo $row['po_no']; ?></td>
                            <td style="word-wrap: break-word;word-break: break-all;" width="100"><? echo $row['job_no']; ?></td> 
                            <td style="word-wrap: break-word;word-break: break-all;" width="60"><? echo $row['year']; ?></td>
                            <td style="word-wrap: break-word;word-break: break-all;" width="100"><? echo $item_name; ?></td>
                            <td style="word-wrap: break-word;word-break: break-all;" width="80"><? echo change_date_format($row['ship_date']); ?></td>
                            ...
                            <td style="word-wrap: break-word;word-break: break-all;" title="" align="right" width="80"><? echo number_format($sew_in_qty,2); ?></td> 
                             <td style="word-wrap: break-word;word-break: break-all;" align="right" width="80"><? echo number_format($sew_out_qty,2);//$unit_of_measurement[$row['uom']]; ?></td>
                            <td style="word-wrap: break-word;word-break: break-all;" title="Previous Rate(<?=$gbl_rate5;?>)+Gmts Fin Avg Rate(<?=$trim_rate_fin;?>)"  align="right" width="80"><? echo number_format($fin_rate,4); ?></td> 
                            <td style="word-wrap: break-word;word-break: break-all;" title="Previous Wash Recv Qty<?=$opening_recv_fin_qty.' (SewOut'.$opening_sew_out_qty.')';?>-Previoue Fin Openinig<?=$opening_finish_qty;?>" align="right" width="80"><?php echo number_format($opening_fin_qty,2);?></td> 
                            <td style="word-wrap: break-word;word-break: break-all;" align="right"  title="" width="80"><?php if($fin_rate>0) echo number_format($opening_fin_qty*$fin_rate,2);?></td> 
                            ...
                            <td style="word-wrap: break-word;word-break: break-all;" align="right" title="Opening Fin(<? echo number_format($opening_fin_qty,2); ?>) +Wash/SewOut(<? echo number_format($fin_sew_out_qty,2); ?>)-Fin Qty(<?=$fin_qty;?>)" width="80"><? echo number_format($sew_stock_qty,0); ?></td> 
                            
                            <td style="word-wrap: break-word;word-break: break-all;" align="right"   width=""><? if($fin_rate>0)  echo number_format($sew_stock_qty*$fin_rate,2);//else $sew_stock_qty=0;  ?></td> 
                            </tr>
                            <?
							$gf++;
							$total_po_qty_pcs+=$row['po_qty'];
							...
							<tr style="font-weight: bold; font-size: 12px; text-align:right; background-color:#ccc">
                                ...
                                <td><?=number_format($total_sew_out_qty,2);?></td>
                                <td><? //number_format($total_avg_fab_cons,0);?></td>
                             	<td><?=number_format($total_opening_fin_qty,2);?></td>
								<td><?=number_format($total_opening_fin_val,2);?></td>
								<td><?=number_format($total_recv_wash_qty,2);?></td>
								<td><?=number_format($total_fin_qty,2);?></td>
                                <td><?=number_format($total_sew_stock_qty,2);?></td>
                                <td><?=number_format($total_sew_stock_val,2);?></td>
            ...
            <br>
             ...
             $tbl_width_wash=$tbl_width+120;
			 ?>
               <h3 style="width:<? echo $tbl_width_wash;?>px; float:left"  id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'dtls_gmt_finGood_table_id', '')"> +<b>Gmts Finished Goods</b></h3>
            <div id="dtls_gmt_finGood_table_id" style="width:<?=$tbl_width_wash;?>px; float:left;">
					<table cellspacing="0" cellpadding="0"  border="1" style="border-collapse: collapse;" rules="all"  width="<?=$tbl_width_wash;?>" class="rpt_table" align="left">
                    <caption> <b style="float:left;"></b></caption>
						<thead>
							<tr>
                      ...
                      <div style=" max-height:390px; width:<?=$tbl_width_wash+18;?>px; overflow-y:scroll;" id="scroll_body8">
                     ...
                     <tbody>
						...
						foreach($prod_data_arr as $poId=>$row)
						{
							 ...
							 if ($gfg%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
							  ...
							  $sew_in_qty=$sew_out_qty=$cuting_qcQty=0;
							// $opening_fin_data_array[$val['PO_ID']]['opening_goods_fin_qty']
							$exfact_qty=$opening_fin_qty=$exfact_fin_qty=$opening_fin_bal_qty=$fin_qty=$exfact_qty=$gbl_rate7=$clsoing_stock_qty=0;
							 $opening_fin_qty=$opening_fin_data_array[$poId]['opening_goods_fin_qty'];
							 $exfact_fin_qty=$exfact_data_array[$poId]['opening_ex_qty'];
							 $opening_fin_bal_qty=$opening_fin_qty-$exfact_fin_qty;
							 $fin_qty=$fin_data_array[$poId]['fin_qty']; 
							...
							if($currency_id==2)
							{
								if($wo_g_fin_amt>0 && $wo_g_fin_qty>0)
								{
								$trim_rate_gfin=($wo_g_fin_amt/$wo_g_fin_qty)*$ex_rate;
								}
							}
							else {
									if($wo_g_fin_amt>0 && $wo_g_fin_qty>0)
									{
									$trim_rate_gfin=$wo_g_fin_amt/$wo_g_fin_qty;
									}
								}
								
							 $gfin_rate= $trim_rate_gfin+$gbl_rate7;
							 if($poId==71146)
							 {
								// echo $trim_rate_gfin.'='.$gbl_rate7.'='.$gfin_rate.'<br>';
							 }
                            ...
                            <td style="word-wrap: break-word;word-break: break-all;" align="right"  title="" width="80"><?php if($opening_fin_bal_qty>0 && $gfin_rate>0) echo number_format($opening_fin_bal_qty*$gfin_rate,2);?></td> 
                            <td style="word-wrap: break-word;word-break: break-all;" align="right" title="" width="80"><? echo number_format($fin_qty,2); ?></td> 
                            <td style="word-wrap: break-word;word-break: break-all;" align="right" title="" width="80"><? echo number_format($exfact_qty,2); ?></td> 
                            <td style="word-wrap: break-word;word-break: break-all;" align="right" title="Opening Qty+Fin Qty-ExFact Qty" width="80"><? if($clsoing_stock_qty>0)  echo number_format($clsoing_stock_qty,2); ?></td> 
                            
                            <td style="word-wrap: break-word;word-break: break-all;" title="ClosingQty*Rate" align="right"   width=""><? if($clsoing_stock_qty>0 && $gfin_rate>0)  echo number_format($clsoing_stock_qty*$gfin_rate,2); ?></td> 
                            </tr>
                            <?
							$gfg++;
							$total_po_qty_pcs+=$row['po_qty'];
						 ...
						 <tr style="font-weight: bold; font-size: 12px; text-align:right; background-color:#ccc">
								...
								<td>&nbsp;</td>
								<td><?=number_format($total_cm_cost,2);?></td>
								<td><?=number_format($total_cuting_qcQty,0);?></td>
                                <td><?=number_format($total_sew_in_qty,2);?></td>
                                <td><?=number_format($total_sew_out_qty,2);?></td>
                                <td><? //number_format($total_avg_fab_cons,0);?></td>
                             	<td><?=number_format($total_opening_fin_qty,2);?></td>
								<td><?=number_format($total_opening_fin_val,2);?></td>
								<td><?=number_format($total_fin_qty,2);?></td>
								<td><?=number_format($total_exfact_qty,2);?></td>
                                <td><?=number_format($total_closing_stock_qty,2);?></td>
                                <td><?=number_format($total_closing_stock_val,2);?></td>
