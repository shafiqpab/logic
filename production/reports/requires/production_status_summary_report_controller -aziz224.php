...
{ 
	...
	if($report_type==1)
	{
        ...
        <table id="table_header_1" class="rpt_table" width="3650" cellpadding="0" cellspacing="0" border="1" rules="all">
            ...
            <div style="width:3670px; max-height:400px; overflow-y:scroll" id="scroll_body">
			<table class="rpt_table" width="3650" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body">
            <?
             	
				$prod_data=sql_select("select a.febric_description_id as deter_id, b.po_breakdown_id as po_id,b.color_id
         ...
         d.po_break_down_id=b.id and e. color_size_break_down_id=d.id and c.id=e.mst_id and c.production_type in(1,3,4,5,11,8) and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1  $date_cond $buyer_id_cond $year_cond $job_no_cond $order_id_cond_trans $order_no_cond  order by b.id,c.country_id ");
				...
				//	$deter_id=$precost_fab_arr[$fab_id][$row[csf('po_id')]]['desc'];
					//echo $count_deter_id.'=';
				/*	$prod_detail_arr[$row[csf('po_id')]]['pub_date']=$row[csf('pub_shipment_date')];
					$prod_detail_arr[$row[csf('po_id')]]['job_no']=$row[csf('job_no')];
					$prod_detail_arr[$row[csf('po_id')]]['buyer_name']=$row[csf('buyer_name')];
					$prod_detail_arr[$row[csf('po_id')]]['style_ref_no']=$row[csf('style_ref_no')];
					$prod_detail_arr[$row[csf('po_id')]]['season_matrix']=$row[csf('season_matrix')];
					
					$prod_detail_arr[$row[csf('po_id')]]['gmts_item_id']=$row[csf('gmts_item_id')];
					$prod_detail_arr[$row[csf('po_id')]]['total_set_qnty']=$row[csf('total_set_qnty')];
					$prod_detail_arr[$row[csf('po_id')]]['po_no']=$row[csf('po_number')];
					$prod_detail_arr[$row[csf('po_id')]]['po_qty_pcs']+=($row[csf('order_quantity')]);
				...
				//print_r($fab_fab_rowspan_arr);
				 ...
				 if(str_replace("'","",$txt_style_ref)!='')
				 {
					...
					$condition->style_ref_no("in($all_style_job)");
				 }
				 if($job_no!='')
				 {
					//$condition->style_ref_no("in($txt_style_ref)"); 
				...
				$total_tot_print_qty=$total_tot_print_balance_qnty=$total_tot_embrod_qty=$total_tot_sew_in_qty=$total_tot_sew_in_balance_qnty=$total_tot_sew_out_qty=0;
				foreach($prod_detail_tmp as $po_key=>$val)
				{
                       ...
                       <tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>">
							...
							<td width="130"  ><div style="width:130px; word-wrap:break-word;"><? 
							...
							echo $gmts_item; 
							 ?></div></td>
							<td width="100" ><div style="word-break:break-all"><? echo $val['style_ref_no']; ?></div></td>
                            <td width="80" ><div style="word-break:break-all"><? echo  $val['job_no']; ?></div></td>
							<td width="80"  ><div style="word-wrap:break-word:80px;"><? echo $val['po_no']; ?></div></td>
							<td width="80" ><div style="word-wrap:break-word:80px;"><? echo change_date_format($val['pub_date']); ?></div></td>
                            ...
                            <td    valign="top" width="516">
                            	<table  class="rpt_table" width="100%" cellspacing="0" cellpadding="0" rules="all" border="0">
									...
									$tot_grey_fab_qnty=$tot_knitting_com_qty=$tot_knit_balance_qnty=0;
										...
										{
											$fab_descs=$precost_yarnCount_arr[$deter]['desc'];
											...
											$grey_fab_qnty=$prod_detail_booking_arr[$po_key][$deter]['grey_fab_qnty'];
									...
									?>
                                	<tr>
                                    	<td width="170"  title="<? echo $pre_fab_id.' Fab'.$po_key; ?>"> 
                                        <div style="word-break:break-all"> <? echo $fab_descs; ?>   </div> </td>
                                        
                                        <td width="50"  align="center" > <? echo $yarn_counts; ?></td>
                            ...
                            <td   valign="top" width="340">
                            	<table  class="rpt_table" width="100%" cellspacing="0" cellpadding="0" rules="all" border="1">
                                	<? 
										...
										{
											$fin_fab_qnty=$prod_detail_booking_arr2[$po_key][$fab_colo]['fin_fab_qnty'];
                            ...
                            <td   valign="top" width="">
                            	<table  class="rpt_table" width="100%" cellspacing="0" cellpadding="0" rules="all"  border="1">
                                	<? 
									$tot_cut_qnty=$tot_cut_balance_qnty=$tot_print_qty=$tot_print_balance_qnty=$tot_embrod_balance_qnty=$tot_sew_in_balance_qnty=$tot_sew_in_qty=$tot_sew_out_qty=0;
										foreach( $prod_detail_tmp[$po_key]['gmt_color_id'] as $gmt_colo )//Garments part
										{
											...
											$print_qty=$gmt_prod_qty_arr[$po_key][$gmt_colo]['print_qty'];
											$embrod_qty=$gmt_prod_qty_arr[$po_key][$gmt_colo]['embrod_qty'];
											$wash_qty=$gmt_prod_qty_arr[$po_key][$gmt_colo]['wash_qty'];
											
											 $cut_balance_qnty=$val['po_qty_pcs']-$cut_qnty;
											 $tot_cut_balance_qnty+=$cut_balance_qnty;
											 $print_balance_qnty=$val['po_qty_pcs']-$print_qty;
											 $tot_print_balance_qnty+=$print_balance_qnty;
											 $embrod_balance_qnty=$val['po_qty_pcs']-$embrod_qty;
											 $tot_embrod_balance_qnty+=$embrod_balance_qnty;
											 //$sew_in_balance_qnty=$val['po_qty_pcs']-$sew_in_qty;
											...
											$tot_embrod_qty+=$embrod_qty;
											$tot_wash_qty+=$wash_qty;
									?>
                                	<tr>
                                    	<td width="80" title="" > <div style="word-break:break-all"> <? echo $color_details[$gmt_colo]; ?>   </div>	</td>
                                        <td width="100"   title="<? echo $gmt_colo;?>" align="right"> <div style="word-break:break-all"> <? echo $val['po_qty_pcs']; ?>  </div></td>
                                        
                                        <td width="80"  title="Cutting 3"  align="right"> <div style="word-break:break-all"> <? echo $val['po_qty_pcs']; ?>  </div>	</td>
                                        <td width="80"  title=""  align="right"> <div style="word-break:break-all"> 
                                        <? echo number_format($cut_qnty,2); ?> </div></td>
                                        <td width="80"  title="" align="right"> <div style="word-break:break-all">
                                         <?  echo number_format($cut_balance_qnty,2); ?> </div>	</td>
                                        
                                        <td width="80" title="Print 3" align="right"> <div style="word-break:break-all"> <? echo $val['po_qty_pcs']; ?>  </div></td>
                                        <td width="80"  title="" align="right"> <div style="word-break:break-all">
                                         <? echo number_format($print_qty,2); ?>  </div>	</td>
                                        <td width="80"  title="" align="right"> <div style="word-break:break-all">
                                         <? echo number_format($print_balance_qnty,2); ?>  </div></td>
                                        
                                        <td width="80"   title="Emrodery 3"  align="right"> <div style="word-break:break-all"> <? echo $val['po_qty_pcs']; ?> </div>	</td>
                                        <td width="80"   title=""  align="right"> <div style="word-break:break-all">
                                         <? echo number_format($embrod_qty,2); ?>   </div>	</td>
                                        ...
                                        <td width="80"   title="" align="right"> <div style="word-break:break-all"> <? echo number_format($sew_in_balance_qnty,2); ?>  </div>	</td>
                                       
                                        <td width="80"  title="Wash 3"  align="right"> <div style="word-break:break-all"> <? echo $val['po_qty_pcs']; ?>   </div>	</td>
                                        <td width="80"   title=""  align="right"> <div style="word-break:break-all"> <? $sew_out_balance_qnty=$val['po_qty_pcs']-$sew_out_qty;
										echo number_format($sew_out_qty,2);  ?>   </div>	</td>
                                        <td width="80"   title=""  align="right"> 
                                        <div style="word-break:break-all"> <? echo number_format($sew_out_balance_qnty,2); ?>   </div>	</td>
                                         
                                         <td width="80"  title="Finish 2" align="right"> <div style="word-break:break-all"> <? echo $val['po_qty_pcs']; ?>   </div>	</td>
                                        <td width="80"  title=""  align="right"> <div style="word-break:break-all">
                                         <? echo $wash_qty_balance_qnty=$val['po_qty_pcs']-$wash_qty;echo number_format($wash_qty_balance_qnty,2); ?>   </div>	</td>
                                        
                                        <td width="80"  title=""  align="right"> <div style="word-break:break-all">SDSDS <? //echo $row[csf('po_number')]; ?>   </div>	</td>
                                        <td width=""  title=""  align="right"> <div style="word-break:break-all">End <? //echo $row[csf('po_number')]; ?>   </div>	</td>
                                    </tr>
                                    <? }?>
                                </table>
                            </td>
							...
							$total_tot_print_qty+=$tot_print_qty;	
							$total_tot_print_balance_qnty+=$tot_print_balance_qnty;
							$total_tot_embrod_qty+=$tot_embrod_qty;
        ...
        <table class="rpt_table" width="3650" id="report_table_footer" cellpadding="0" cellspacing="0" border="1" rules="all">
				<tfoot>
                    ...
                    <th  width="80"> <? //echo number_format($total_fab_req_qty,2); ?> </th>
                    <th  width="80"> <? echo number_format($total_tot_print_qty,2); ?> </th>
                    <th  width="80"> <? echo number_format($total_tot_print_balance_qnty,2); ?> </th>
                
                    <th  width="80"> <? //echo number_format($total_fab_req_qty,2); ?> </th>
                    <th  width="80"> <? echo number_format($total_tot_embrod_qty,2); ?> </th>
                    ...
                    <th  width="80"> <? echo number_format($total_tot_sew_in_balance_qnty,2); ?> </th>
                    <th  width="80"> <? echo number_format($total_fab_req_qty,2); ?> </th>
                    <th  width="80"> <? echo number_format($total_fab_req_qty,2); ?> </th>
                    <th  width="80"> <? echo number_format($total_fab_req_qty,2); ?> </th>
                    <th  width="80"> <? echo number_format($total_fab_req_qty,2); ?> </th>
