...
*/ // $all_batch[$row[csf('batch_id')]]
 /* echo $sql_prod2="select aa.batch_no,aa.result,max(aa.batch_ext_no) as batch_ext_no,b.batch_id,sum(b.receive_qnty) as receive_qnty,b.machine_no_id from inv_receive_master a,pro_finish_fabric_rcv_dtls b,pro_fab_subprocess aa where a.id=b.mst_id and aa.batch_id=b.batch_id and b.receive_qnty>0 and aa.load_unload_id=2 and a.status_active=1 and a.is_deleted=0 and a.entry_form=7 $batch_no_cond_for_in group by aa.batch_no,aa.result,b.batch_id,b.machine_no_id";//batch_no
	...
	$sql_prod2="select b.batch_id,b.receive_qnty as receive_qnty,b.machine_no_id from inv_receive_master a,pro_finish_fabric_rcv_dtls b where a.id=b.mst_id  and b.receive_qnty>0 and a.status_active=1 and a.is_deleted=0 and a.entry_form=7 and b.batch_id in (".implode(",",$all_batch).")  ";//batch_no
		$fin_prod2=sql_select($sql_prod2);
		foreach($fin_prod2 as $row)
		{
		...
		$self_rowspan_arr=array();
		$self_func_batch_arr=array();
		$mc_ut_qty_arr=array();$self_prod_qty=0;
		foreach ($self_date_data_arr as $p_date=>$prod_date_data) 
		{
			foreach ($prod_date_data as $machine_id => $machine_data) 
			{
				...
				{  
					...
					//echo $self_fin_prod_summ.'='.$row[('result')].'='.$row[('batch_ext_no')].'<br>';
					//echo $row['batch_qty'].'DD'. $row['color_range_id'];
					if (($row['result']==1 || $row['result']==2) && $self_fin_prod_summ3>0) 
					{
						
							if ($row['color_range_id']==4)
							{
								$white_qty += $row['batch_qty']; 
							} 
							else if ($row['color_range_id']==7)
							{
								$wash_qty += $row['batch_qty']; 
							}
							else $color_batch_qty += $row['batch_qty']; 
						
					}
					else if ($row['result']==5  && $self_fin_prod_summ3=='') // //Under Trial/Fin Prod not Available
					{
					...
					if (($row['result']==1 || $row['result']==2)  && $self_fin_prod_summ3>0 && $row['batch_ext_no']>0)  //
						{
							//$prod_qty += $row['batch_qty']; 
							//$self_re_process_qty_arr[$row['batch_no']][$row['result']]['re_process_qty']+=$row[('batch_qty')];							
						}	
				
					$trims_weight_qty+=$row['total_trims_weight'];						
					$self_prod_qty+=$row['batch_qty']+$row['total_trims_weight'];// $color_batch_qty+$white_qty+$wash_qty+$prod_qty+$trims_weight_qty;


					// Total batch_qty qty
					/*if ($row['color_range_id']!=4 && $row['color_range_id']!=7 && $row['result']==1)
					{ 
	...
	/* $sql_subprod="select aa.batch_no,aa.result,aa.batch_ext_no,b.batch_id,b.receive_qnty,b.machine_no_id from inv_receive_master a,pro_finish_fabric_rcv_dtls b,pro_fab_subprocess aa where a.id=b.mst_id and aa.batch_id=b.batch_id and b.receive_qnty>0 and aa.load_unload_id=2 and a.status_active=1 and a.is_deleted=0 and a.entry_form=7 $sub_batch_cond_for_in";
		$sub_fin_prod=sql_select($sql_subprod);
		foreach($sub_fin_prod as $row)
		{
			if($row[csf("result")]==4 && $row[csf("batch_ext_no")]=="")
			{
			$subcon_fin_prod_arr[$row[csf("batch_no")]][$row[csf("result")]]["fin_qnty"]=$row[csf("receive_qnty")];
			}
			if($row[csf("batch_ext_no")]=="")
			{
			$subcon_fin_prod_arr2[$row[csf("batch_no")]]["fin_qnty"]=$row[csf("receive_qnty")];
			}
...
*/
		...
		$subcon_func_batch_arr=array();
		$subc_mc_ut_qty_arr=array();$subcon_total_prod_qty=0;
		foreach ($subcon_date_data_arr as $p_date=>$prod_date_data) 
		{
			foreach ($prod_date_data as $machine_id => $machine_data) 
			{
				foreach ($machine_data as $batch_id => $row) 
				{
					...
					if (($row['result']==1 || $row['result']==2) && $subcon_fin_prod_sum3=='') 
					{
							if ($row['color_range_id']==4)
							{
								$subcon_white_qty += $row['batch_qnty']+$row['total_trims_weight']; 
							}
							else if ($row['color_range_id']==7)
							{
								$subcon_pro_qty += $row['batch_qnty']+$row['total_trims_weight'];
							}
							else $subcon_color_qty += $row['batch_qnty']+$row['total_trims_weight'];
					...
					/*if ($row['result']==4 && $row['batch_ext_no']>0 && $subcon_fin_prod_sum>0) 
					{
						$subcon_prod_qty += $row['batch_qnty']; 
					}*/
					if (($row['result']==1 || $row['result']==2)  && $subcon_fin_prod_sum3>0)  //
						{
							$subcon_actual_pro_qty = $row['batch_qnty']+$row['total_trims_weight'];
							$subcon_total_prod_qty +=$row['batch_qnty']+$row['total_trims_weight'];						
						}	
						else if (($row['result']==1 || $row['result']==2) && ($row['redyeing_needed']==2 || $row['shade_matched']==1) && $re_process_qty3=='')//Shade match/Redyeing Needed/Fin Prod not Available 
						{
								if($row['batch_ext_no']>0)
								{
										$subcon_actual_pro_qty = 0;
										//echo "B";
								}
								else {
									 $subcon_actual_pro_qty=$row['batch_qnty']+$row['total_trims_weight'];//$subcon_total_prod_qty +=$row['batch_qnty']+$row['total_trims_weight'];
								 }
						}
	...
	if ($order_type==0 || $order_type==3) // Sample Without Order  sql and array
	{
		...
		/* $sql_sam_prod="select aa.batch_no,aa.result,aa.batch_ext_no,b.batch_id,b.receive_qnty,b.machine_no_id from inv_receive_master a,pro_finish_fabric_rcv_dtls b,pro_fab_subprocess aa where a.id=b.mst_id and aa.batch_id=b.batch_id and b.receive_qnty>0 and aa.load_unload_id=2 and a.status_active=1 and a.is_deleted=0 and a.entry_form=7 $sub_batch_cond_for_in";
		$samp_fin_prod=sql_select($sql_sam_prod);
		foreach($samp_fin_prod as $row)
		{
			
			if($row[csf("result")]==1 && $row[csf("batch_ext_no")]=="")
			{
			$samp_fin_prod_arr[$row[csf("batch_no")]][1]["fin_qnty"]=$row[csf("receive_qnty")];
			}
			if($row[csf("batch_ext_no")]=="")
			{
			$samp_fin_prod_arr2[$row[csf("batch_no")]]["fin_qnty"]=$row[csf("receive_qnty")];
			}
		...
		$rowspan_arr=array();
		$smp_mc_ut_qty_arr=array();$sample_total_prod_qty=0;
		foreach ($date_data_arr as $p_date=>$prod_date_data) 
		{
			foreach ($prod_date_data as $machine_id => $machine_data) 
			{
		...
		<div style="float: right;">
			<table width="200" id="tdtag" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all">
				<thead>
				  ...
				  <tr>
				    <td id="tdtag" style="background: #E9F3FF;"><strong>Total Production</strong></td>
				    <td id="tdtag" style="background: #E9F3FF;"><strong><?php echo $self_prod_qty+$subcon_total_prod_qty+$sample_total_prod_qty; ?></strong></td>
				  </tr>
				  <tr>
				    <td id="tdtag" style="background: #FFFFFF;"><strong>Total Machine Capacity</strong></td>
				    <td id="tdtag" style="background: #FFFFFF;"><strong><?php echo $self_total_machine_capacity+$subcon_total_machine_capacity+$sample_total_machine_capacity; ?></strong></td>
				  </tr>
				  <tr>
				  ...
				  <tr>
				    ...
				    <td id="tdtag" style="background: #FFFFFF;"><strong><?php  
							echo number_format($tot_machine_no/$tot_functional_batch,2,'.','');//number_format($self_avg_batch_per_mc+$subcon_avg_batch_per_mc+$sample_avg_batch_per_mc,2,'.','');
						?></strong></td>
				  </tr>
				</thead>
			</table> 
	...
	if ($order_type==0 || $order_type==1) // Self Order
	{
		?>
		<div>
			<table width="2250" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
				<thead>
					<tr>
						<th colspan="5" style="text-align: left;">Self Order</th>
					</tr>
				</thead>
			</table>
		<table width="2510" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
			<thead>
				<tr>
	                <th rowspan="2" width="30">SL No</th>
	                ...
	                <th rowspan="2" width="80">MC Total Production</th>
                    <th rowspan="2" width="80">Actual Production</th>
	                <th rowspan="2" width="80">Incomplete Production</th>
                    <th rowspan="2" width="80">Result</th>
	                <th rowspan="2" width="80">Water/kg in Ltr</th>
	                <th rowspan="2" width="80">M/C UT%y</th>
	                <th rowspan="2" width="80">Loading Time</th>
	                <th rowspan="2" width="80">Unloading Time</th>
	                <th rowspan="2" width="80">Total Time (Hour)</th>
	                <th rowspan="2" width="100">Chemical Cost</th>
	                <th rowspan="2" width="100">Dyes Cost</th>
	    ...
	    <div style="width:2510px; overflow-y:scroll; max-height:300px;" id="scroll_body">
		<table align="center" cellspacing="0" width="2490"  border="1" rules="all" class="rpt_table" >
			...
			foreach ($self_date_data_arr as $p_date=>$prod_date_data) 
			{
				...
				foreach ($prod_date_data as $machine_id => $machine_data) 
				{
					...
					//echo $self_fin_prod_check."=";
						...
						<tr id="<? echo 'v_'.$i; ?>" onClick="change_color('v_<? echo $i; ?>','#FFFFFF')" bgcolor="<? echo $bgcolor; ?>">
							<?							
								if ($r==0) 
								{
									...
									?>" width="100" style="word-wrap:break-word; word-break: break-all;" valign="middle"><?echo $row['machine_no'].' - '.$row['prod_capacity'].'kg';?></td>
									<?
								} $r++;
							...
							?>						
							<td width="80" style="word-wrap:break-word; word-break: break-all;" valign="middle"><? echo $buyer_library[$row['buyer_id']];?></td>
							<td width="80" valign="middle"><? echo $row['batch_no']; $mcw_wise_total_batch+=count($row['batch_no']); ?></td>
							<td width="80" valign="middle"><p><? echo $color_library[$row['color_id']]?></p></td>
							<td width="80" valign="middle"><? echo $row['booking_no'];?></td>
                            <td width="80" valign="middle" align="right"><? echo $row['batch_qty'];?></td>
                            <td width="80" valign="middle" align="center"><? echo $row['batch_ext_no'];?></td>
							<td width="80" valign="middle"><? echo $color_range[$row['color_range_id']];?></td>
							<? 
							...
							if ($row['result']==1) //Shade match
							{
										...
										$trims_weight=$row['total_trims_weight'];
							}
							else if ($row['result']==5  && $re_process_qty3=='')  //Under Trial/Fin Prod not Available
							{
											if ($row['color_range_id']==4)
											{
												$white_batch_qty =0; 
							...
							//if ($row['result']==1)  //Shade match/Fin Prod Available
							if ($row['result']==1 && $self_fin_prod_check=="" && $row['batch_ext_no']=="")
							{
						...
						if (($row['result']==1) && $row['batch_ext_no']>0 && $re_process_qty3>0 ) //Shade match/Redyeing Needed/Re Process /Fin Prod Available
							{
								$re_pro_qty =$row['batch_qty']+$row['total_trims_weight'];// $row['batch_qty'];		
								$is_fnished[$row['batch_no']]=1;					
							}
							
							if($is_fnished[$row['batch_no']]==1) $re_pro_qty =$row['batch_qty']+$row['total_trims_weight'];// $row['batch_qty'];	
							...
							?>
							<td width="80" valign="middle" align="right"><? echo $avarage_batch_qty; $other_mcw_wise_total+=$avarage_batch_qty;?></td>
							<td width="80" valign="middle" align="right"><? echo $white_batch_qty; $white_mcw_wise_total+=$white_batch_qty;?></td> 
							<td width="80" valign="middle" align="right"><? echo $wash_batch_qty; $wash_mcw_wise_total+=$wash_batch_qty;?></td> 
							<td width="80" valign="middle" align="right"><? echo $re_pro_qty; $total_pro_qty+=$re_pro_qty;?></td>
							<td width="80" valign="middle" align="right"><? echo $trims_weight; $total_trims_weight+=$trims_weight;?></td>
							<td width="80" valign="middle" align="right"><? echo $total_self_production=$row['batch_qty']+$row['total_trims_weight'];//($avarage_batch_qty+$white_batch_qty+$wash_batch_qty+$re_pro_qty+$trims_weight); 
							$total_self_production_qty+=$total_self_production; ?></td>
                            <td width="80" valign="middle" align="right"><? echo $actual_prod_qty; $total_actual_prod_qty+=$actual_prod_qty;?></td>
							<td width="80" valign="middle" align="right"><? echo $self_incomplete_qty;//if($row['incomplete_result']==4) echo $total_self_incomplete_qty=$row['batch_qty']; $total_self_incomplete_qty+=$self_incomplete_qty; ?></td>
                            <? if($row['result']==1) { $resultBgcolor = "green";} else if($row['result']==2) {$resultBgcolor = "yellow";} else{$resultBgcolor = "#FFFFFF";} ?>
							<td width="80" valign="middle" bgcolor="<? echo $resultBgcolor; ?>"><? echo $dyeing_result[$row['result']];?></td>
							...
							?>
							<td width="80" valign="middle"><? echo number_format($mc_ut_result,2,'.','');?></td>
							<?
							...
							<td width="100" valign="middle" align="right"><? echo number_format($chemical_cost+$dyes_cost,2,'.',''); ?></td>
							...
							<?
								...
								if ($unload_remarks=="") 
								{
									?><td valign="middle"><?echo $load_remarks;?></td><?	
								}
								else
								{
									?><td valign="middle"><? echo $row['remarks']; ?></td><?							
								}							
							?>					
						</tr>											
						<?						
	    ...
	    <tfoot>
	        <tr class="tbl_bottom">
	            ...
	            <td width="80"><strong><? echo $wash_grnd_total;?></strong></td>
	            <td width="80"><strong><? echo $pro_qty_grnd_total;?></strong></td>
	            <td width="80"><strong><? echo $trims_weight_grnd_total;?></strong></td>
	            <td width="80"><strong><? echo $self_production_grnd_total;?></strong></td>
                 <td width="80"><strong><? echo $self_actual_production_grnd_total;?></strong></td>
	            <td width="80"><strong><? echo $incomplete_qty_grnd_total;?></strong></td>
	            <td colspan="11"></td>
	        </tr> 
	...
	if ($order_type==0 || $order_type==2) // Subcontract Order
	{
		...
		<div>
			<table width="2520" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
				<thead>
					<tr>
						<th colspan="5" style="text-align: left;">Subcontract Order</th>
					</tr>
				</thead>
			</table>
		<table width="2520" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
	    ...
	    <div style="width:2520px; overflow-y:scroll; max-height:300px;" id="scroll_body">
		<table align="center" cellspacing="0" width="2490"  border="1" rules="all" class="rpt_table" >
						...
						<tr id="<? echo 'vs_'.$i; ?>" onClick="change_color('vs_<? echo $i; ?>','#FFFFFF')" bgcolor="<? echo $bgcolor; ?>">
							<?
								if ($r==0) 
								{
									...
									?>" width="100" valign="middle"><?echo $row['machine_no'].' - '.$row['prod_capacity'].'kg';?></td>
									<?
								} $r++;
							...
							<td width="80" valign="middle"><? echo $buyer_library[$row['party_id']];?></td>
							<td width="80" valign="middle"><? echo $row['batch_no']; $subcon_mc_wise_total_batch+=count($row['batch_no']);?></td>
							<td width="80" valign="middle"><p><? echo $color_library[$row['color_id']]?></p></td>
							<td width="80" valign="middle"><? echo $row['order_no'];?></td>
                            
                            <td width="80" valign="middle" align="right"><? echo $row['batch_qnty'];?></td>
                            <td width="80" valign="middle"><? echo $row['batch_ext_no'];?></td>
							<td width="80" valign="middle"><? echo $color_range[$row['color_range_id']];?></td>
							<?
							...
							if (($row['result']==1 || $row['result']==2) && $sub_re_process_qty3=='')   // Shade match/Redyeing Needed/Fin Prod Available
                            {
										if ($row['color_range_id']==4)
										{
											$subcon_white_pro_qty = $row['batch_qnty']+$row['total_trims_weight']; 
										}
										else if ($row['color_range_id']==7)
										{
											$subcon_wash_pro_qty = $row['batch_qnty']+$row['total_trims_weight']; 
										}
										else $subcon_other_pro_qty = $row['batch_qnty']; 
										//$sub_actual_prod_qty = $row['batch_qnty']+$row['total_trims_weight'];   
                            }
							else if($row['result']==5 && $sub_re_process_qty3=='')   // Shade match/Redyeing Needed/Fin Prod Available
                            {
										if ($row['color_range_id']==4)
										{
							...
							if (($row['result']==1 || $row['result']==2) && $sub_re_process_qty3>0)  //Shade match/Redyeing Needed/Fin Prod Available
							{
									$sub_actual_prod_qty =$row['batch_qnty']+$row['total_trims_weight'];
							}
							else if (($row['result']==1 || $row['result']==2) && ($row['redyeing_needed']==2 || $row['shade_matched']==1) && $sub_re_process_qty3=='')//Shade match/Redyeing Needed/Fin Prod not Available 
							{
									if($row['batch_ext_no']>0)
									{
									...
									//else $sub_actual_prod_qty=$row['batch_qnty']+$row['total_trims_weight'];
							}
							...
							{
								//echo "XXXXXXXXXXXX";
								$pro_qty =$row['batch_qnty']+$row['total_trims_weight'];//$sub_re_process_qty3;// $row['batch_qty'];	
								$is_fnished[$row['batch_no']]=1;						
							}
							if($is_fnished[$row['batch_no']]==1) $pro_qty =$row['batch_qnty']+$row['total_trims_weight'];
							if($row['incomplete_result']==4) 
							{
							...
							?>
							<td width="80" valign="middle" align="right"><? echo $subcon_other_pro_qty; $subcon_other_mcw_wise_total+=$subcon_other_pro_qty;?></td>
							<td width="80" valign="middle" align="right"><? echo $subcon_white_pro_qty; $subcon_white_mcw_wise_total+=$subcon_white_pro_qty;?></td>
							<td width="80" valign="middle" align="right"><? echo $subcon_wash_pro_qty; $subcon_wash_mcw_wise_total+=$subcon_wash_pro_qty;?></td>
							<td width="80" valign="middle" align="right"><? echo $pro_qty; $subcon_pro_qty_total+=$pro_qty;?></td>
							<td width="80" valign="middle" align="right"><? echo $trims_weight = $row['total_trims_weight']; $subcon_trims_weight_total+=$trims_weight;?></td>
							<td width="80" valign="middle" align="right"><? echo $total_production=$row['batch_qnty']+$row['total_trims_weight'];//$subcon_other_pro_qty+$subcon_white_pro_qty+$subcon_wash_pro_qty+$pro_qty+$trims_weight+$incomplete_qty+$sub_other_qty; 
								$total_subc_production_qty+=$total_production;?></td>
                             <td width="80" valign="middle" align="right" title="BatchID=<? echo $batch_id;?>"><? echo $sub_actual_prod_qty; $subcon_actual_prod_qty_total+=$sub_actual_prod_qty;?></td>
							<td width="80" valign="middle"  align="right"><? echo $incomplete_qty;//if($row['incomplete_result']==4) echo $incomplete_qty=$row['batch_qnty']; $total_subc_incomplete_qty+=$incomplete_qty; ?></td>
							<td width="80" valign="middle"  align="right"><? echo $row['water_flow_meter'];?></td>
							<?
							...
							?>
							<td width="80" valign="middle"><? echo number_format($subc_mc_ut_result,2,'.','');?></td>
							<?
	    ...
	    <tfoot>
	        <tr class="tbl_bottom">
	        	...
	        	<td width="80"></td>
	        	<td width="80"><strong><? echo $subcon_grnd_total_batch;?></strong>&nbsp;</td>
	        	<td width="80"></td>
	        	<td width="80"></td>
                <td width="80"></td>
	        	<td width="80"></td>
	            ...
	            <td width="80"><strong><? echo $subcon_wash_grnd_total;?></strong></td>
	            <td width="80"><strong><? echo $subcon_pro_qty_grnd_total;?></strong></td>
	            <td width="80"><strong><? echo $subcon_trims_weight_grnd_total;?></strong></td>
	            <td width="80"><strong><? echo $subc_production_grnd_total;?></strong></td>
                <td width="80"><strong><? echo $subcon_grnd_total_actual_batch;?></strong></td>
	            <td width="80"><strong><? echo $subc_incomplete_qty_grnd_total;?></strong></td>
	...
	if ($order_type==0 || $order_type==3) // Sample Without Order
	{
		?>
		<div>
			<table width="2430" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
				<thead>
					<tr>
						<th colspan="5" style="text-align: left;">Sample Without Order</th>
					</tr>
				</thead>
			</table>
		<table width="2430" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
	    ...
	    <div style="width:2430px; overflow-y:scroll; max-height:300px;" id="scroll_body">
		<table align="center" cellspacing="0" width="2410"  border="1" rules="all" class="rpt_table" >
					...
					foreach ($machine_data as $batch_id => $row) 
					{
						...
						<tr id="<? echo 'vSamp_'.$i; ?>" onClick="change_color('vSamp_<? echo $i; ?>','#FFFFFF')" bgcolor="<? echo $bgcolor; ?>">
							<?							
								...
								if($samp_arrs[$p_date][$machine_id][$row['system_no']]=="")
								{
									?>
									<td rowspan="<? echo $samp_fun_batch_rowspan_arr[$p_date][$machine_id][$row['system_no']] ?>" width="80" valign="middle" style="word-wrap:break-word; word-break: break-all;"><? echo $sample_functional_batch=$row['system_no']; 
										$sample_functional_batch_arr[$row['system_no']]=$row['system_no']; ?></td>	
									<? 
									$samp_arrs[$p_date][$machine_id][$row['system_no']]=$row['system_no'];
								}
							?>
							<td width="80" valign="middle"><? echo $buyer_library[$row['buyer_id']];?></td>
							<td width="80" valign="middle"><? echo $row['batch_no'];$sm_mcw_wise_total_batch+=count($row['batch_no']);?></td>
							<td width="80" valign="middle"><p><? echo $color_library[$row['color_id']]?></p></td>
							<td width="80" valign="middle"><? echo $row['booking_no'];?></td>
                           <td width="80" valign="middle"><? echo $row['batch_qty'];?></td>
                            <td width="80" valign="middle"><? echo $row['batch_ext_no'];?></td>
							<td width="80" valign="middle"><? echo $color_range[$row['color_range_id']];?></td>
							<?

							$other_pro_qty=$white_pro_qty=$wo_wash_pro_qty=$samp_actual_prod_qty=$pro_qty=$samp_other_qty=0; 
							if (($row['result']==1 || $row['result']==2) && $samp_re_process3=='') 
                            {
							...
							else if ($row['result']==5  && $samp_re_process3=='')  
							{
										if ($row['color_range_id']==4)
										{
											$white_pro_qty = 0; 
							...
							if (($row['result']==1 || $row['result']==2) && $samp_re_process3>0)  //Shade match/Redyeing Needed/Fin Prod Available
							{
								 $samp_actual_prod_qty = $row['batch_qty']+$row['total_trims_weight'];	
							}
							else if (($row['result']==1 || $row['result']==2) && ($row['redyeing_needed']==2 || $row['shade_matched']==1) && $samp_re_process3=='')//Shade match/Redyeing Needed/Fin Prod not Available 
							{
								if($row['batch_ext_no']>0)
									{
									...
									//else $samp_actual_prod_qty=$row['batch_qty']+$row['total_trims_weight'];
							}
                            
							if (($row['result']==1 || $row['result']==2) && $row['batch_ext_no']>0 && $samp_re_process3>0 )  // Shade Match/Redyeing Needed/Redying batch nd fin Production avaliable
							{
								//echo "XXXXXXXXXXXX";
								$samp_pro_qty =$row['batch_qty']+$row['total_trims_weight'];// $row['batch_qty'];	
								$is_fnished[$row['batch_no']]=1;						
							}
							if($is_fnished[$row['batch_no']]==1) $samp_pro_qty =$row['batch_qty']+$row['total_trims_weight'];// $row['batch_qty'];	
							...
							?>
							<td width="80" valign="middle" align="right"><? echo $other_pro_qty; $other_mcw_wise_total+=$other_pro_qty;?></td>
							<td width="80" valign="middle" align="right"><? echo $white_pro_qty; $white_mcw_wise_total+=$white_pro_qty;?></td>
							<td width="80" valign="middle" align="right"><? echo $wo_wash_pro_qty; $wo_wash_mcw_wise_total+=$wo_wash_pro_qty;?></td>
							<td width="80" valign="middle" align="right"><? echo $samp_pro_qty; $wo_pro_qty_total+=$samp_pro_qty;?></td>
							<td width="80" valign="middle"><? echo $trims_weight = $row['total_trims_weight']; $wo_trims_weight_total+=$trims_weight;?></td>
							<td width="80" valign="middle" align="right"><? echo $total_production=$row['batch_qty']+$row['total_trims_weight'];//($other_pro_qty+$white_pro_qty+$wo_wash_pro_qty+$samp_pro_qty+$trims_weight+$samp_incomplete_qty); 
							$total_samp_production+=$total_production; ?></td>
                            <td width="80" valign="middle" align="right"><? echo $samp_actual_prod_qty; $samp_actual_pro_qty_total+=$samp_actual_prod_qty;?></td>
							<td width="80" valign="middle"><? echo $incomplete_qty;//if($row['result']==4) echo $incomplete_qty=$row['batch_qty'];
								$total_samp_incomplete_qty+=$incomplete_qty; ?></td>
							<td width="80" valign="middle"><?echo $row['water_flow_meter'];?></td>
							<?
							...
							?>
							<td width="80" valign="middle"><? echo number_format($smp_mc_ut_result,2,'.','');?></td>
							<?
								$load_hour = $load_time_array[$row['machine_id']][$row['batch_id']][$row['batch_ext_no']]["load_end_hours"];
								$load_minut = $load_time_array[$row['machine_id']][$row['batch_id']][$row['batch_ext_no']]["load_end_minutes"];
								$process_start_date = $load_time_array[$row['machine_id']][$row['batch_id']][$row['batch_ext_no']]["process_start_date"];
	    ...
	    <tfoot>
	        <tr class="tbl_bottom">
	        	...
	        	<td width="80"></td>
	        	<td width="80"><strong><? echo $sm_grnd_total_batch;?></strong>&nbsp;</td> 
	        	<td width="80"></td> 
	        	<td width="80"></td>
                <td width="80"></td> 
	        	<td width="80"></td>
	            ...
	            <td width="80"><strong><? echo $wash_grnd_total;?></strong></td>
	            <td width="80"><strong><? echo $wo_pro_qty_grnd_total;?></strong></td>
	            <td width="80"><strong><? echo $wo_trims_weight_grnd_total;?></strong></td>
	            <td width="80"><strong><? echo $grnd_total_samp_production;?></strong></td>
                <td width="80"><strong><? echo $actual_wo_pro_qty_grnd_total;?></strong></td>
	            <td width="80"><strong><? echo $grnd_total_samp_incomplete_qty;?></strong></td>
	            <td colspan="11"></td>
	        </tr> 
	        <tr class="tbl_bottom">
	        	<td colspan="2" align="right"><b>Avg Batch Per M/C:</b></td>  
				<td width="80"><strong><? echo number_format($total_mc=$sm_grnd_total_batch/($i-1),2,'.','') ;?></strong>&nbsp;</td>
                <td colspan="27" align="right"> </td>  
	        </tr>
	     </tfoot>
	    </table>
	    </div>
