...
if($action=="report_generate") 
{
	...
	{		
		...
		where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id  $sql_cond $sql_cond2 $po_ids_cond and a.status_active=1 and b.status_active=1 and c.status_active=1 and a.is_deleted=0 and b.is_deleted=0 and c.is_deleted=0  and c.cutup <> 0 order by c.cutup";
		// echo $sql;die();
		...
		// die();
	    // ================================================ MAIN QUERY ==================================================
		$sql_prod="SELECT a.style_ref_no as style,b.id as po_id,b.po_number,c.country_id,c.country_ship_date,c.cutup,d.floor_id,sum(e.production_qnty) as qty,max(d.production_date) as last_input_date
		...
		where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id and b.id=d.po_break_down_id and d.id=e.mst_id and d.production_type=4 $sql_cond $sql_cond2 $po_ids_cond $prod_po_ids_cond and a.status_active=1 and b.status_active=1 and c.status_active=1 and d.status_active=1 and e.status_active=1 and c.id=e.color_size_break_down_id and c.cutup <> 0 group by a.style_ref_no,b.id,b.po_number,c.country_id,c.country_ship_date,c.cutup,d.floor_id order by c.cutup";
		// echo $sql_prod;die();
		...
		foreach ($sql_prod_res as $row) 
		{
			...
			$floor_wise_qty_arr[$row[csf('style')]][$row[csf('po_id')]][$row[csf('cutup')]][$row[csf('country_ship_date')]][$row[csf('country_id')]]['last_input_date'] = $row[csf('last_input_date')];
		}
		unset($sql_res);
		// echo "<pre>";print_r($floor_wise_qty_arr);
	    // =============================================== Exfactory =========================================
		...
		$table_width = 1020+count($floorArr)*80;
		ob_start();
		?>
		<style type="text/css">
		    	...
		    	<table class="rpt_table" cellspacing="0" cellpadding="0" border="1" rules="all" id="" width="<? echo $table_width;?>">
		    		<thead>
			    		<tr>
			    			...
			    			<th rowspan="2" width="80">Last Input Date</th>
	    								...
	    								{ 
	    									...
	    									?>
	    										<tr bgcolor="<? echo $bgcolor;?>" onclick="change_color('tr_1nd<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_1nd<? echo $i; ?>" >
									    			...
									    			<? }?>
									    			<td align="center" width="80"><? echo change_date_format($floor_wise_qty_arr[$style][$po_id][$cutup][$shipdate][$country_id]['last_input_date']); ?></td>
