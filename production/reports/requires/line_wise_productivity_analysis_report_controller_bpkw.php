...
{ 
...

	{
...

		{
			...
			$sql="select a.job_no, a.company_name, a.buyer_name, a.style_ref_no, a.order_uom, a.gmts_item_id, a.set_break_down, a.total_set_qnty as ratio, a.job_quantity ,LISTAGG(b.id, ',') WITHIN GROUP (ORDER BY b.id) as po_id, LISTAGG(cast(b.id || '**' || b.po_number || '**' || b.po_quantity as varchar2(4000)), ',') WITHIN GROUP (ORDER BY b.id) as po_data, c.item_number_id, c.location, c.floor_id, c.prod_reso_allo, c.sewing_line, sum(c.production_quantity) as qnty from wo_po_details_master a, wo_po_break_down b, pro_garments_production_mst c where a.job_no=b.job_no_mst and a.company_name='$comapny_id' and b.id=c.po_break_down_id and c.location like '$location' and c.floor_id like '$floor' and c.sewing_line like '$line' and production_date=$txt_date and production_type=5 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $buyer_id_cond group by a.job_no, a.job_quantity, a.company_name, a.buyer_name, a.style_ref_no, a.order_uom, a.gmts_item_id, a.set_break_down, a.total_set_qnty, c.location, c.floor_id, c.prod_reso_allo, c.sewing_line, c.item_number_id order by c.location, c.floor_id";	
...

		{
...

			{
...

				{
					...
					$html.='<tr bgcolor="#EFEFEF"><td colspan="32"><b>Floor name: '.$floorArr[$row[csf('floor_id')]].'; Location name: '.$locationArr[$row[csf('location')]].'</b></td></tr>';
			...
			$mst_sql= "SELECT  
							...
							min(CASE WHEN production_type ='4' and po_break_down_id in(".$row[csf("po_id")].") THEN production_date END) AS frstinput_date,

							sum(CASE WHEN production_type ='4' and production_date=$txt_date and po_break_down_id in(".$row[csf("po_id")].") THEN production_quantity ELSE 0 END) AS today_input_qnty,
		   ...
		   // $no_of_days=return_field_value("count(id)","lib_capacity_calc_dtls","date_calc between '$fstinput_date' and $txt_date and day_status=1");
			...
			$html.='<td width="40">'.$i.'</td>
						...
						<td width="75" align="right">'.$row[csf('job_quantity')].'&nbsp;</td>
						...
						<td width="100" align="right"><a href="##" onclick="openmypage('.$comapny_id.",'".$job_po_id."',".$row[csf('item_number_id')].",".$row[csf('location')].",".$row[csf('floor_id')].",".$row[csf('sewing_line')].",".$txt_date.",'tot_smv_used',"."'".$fstinput_date."'".",".$row[csf('prod_reso_allo')].')">'.number_format($total_smv_used,2,'.','').'</a>&nbsp;</td>
				  ...
				  $html.='<td><input type="button"  value="View"  class="formbutton" onclick="show_line_remarks('.$comapny_id.",'".$total_po_id."','".$row[csf("floor_id")]."','".$line_number_id."','remarks_popup',".$txt_date.')"/></td>
		...
		<fieldset style="width:2580px">
			...
			<table width="2570" cellpadding="0" cellspacing="0"> 

				<tr class="form_caption">

					<td colspan="31" align="center"><strong><? echo $report_title; ?></strong></td> 
			...
			<table id="table_header" class="rpt_table" width="1300" cellpadding="0" cellspacing="0" border="1" rules="all">
				...
				<thead>
					...
					<th width="110">Today Input</th>

					<th width="110">Today Target</th>

					<th width="110">Today Prod.</th>

					<th width="100">Achv. %</th>

					<th width="110">Today SMV</th>

					<th width="110">Achieved SMV</th>

					<th width="100">Eff. %</th>

					<th width="100">Worker Used</th>

					<th width="100">Operator Engaged</th>

					<th width="100">Helper Engaged</th>
				...
				foreach($buyer_data_array as $key=>$val)

				{
					...
					$achv_perc=$buyer_data_array[$key]['topd']/$buyer_data_array[$key]['tpd']*100;

					$eff_perc=$buyer_data_array[$key]['achv_smv']/$buyer_data_array[$key]['tosmv']*100;*/

				?>

					<!--<tr bgcolor="<?echo $bgcolor;?>" onclick="change_color('trs_<?echo $z; ?>','<?echo $bgcolor;?>')" id="trs_<?echo $z; ?>">

						<td width="40"><?echo $z; ?></td>

						<td width="80"><?echo $buyerArr[$key]; ?></td>

						<td width="110" align="right"><?echo $buyer_data_array[$key]['toin']; ?>&nbsp;</td>

						<td width="110" align="right"><?echo $buyer_data_array[$key]['tpd']; ?>&nbsp;</td>

						<td width="110" align="right"><?echo $buyer_data_array[$key]['topd']; ?>&nbsp;</td>

						<td width="100" align="right"><?echo number_format($achv_perc,2,'.',''); ?>&nbsp;</td>

						<td width="110" align="right"><?echo $buyer_data_array[$key]['tosmv']; ?>&nbsp;</td>

						<td width="110" align="right"><?echo $buyer_data_array[$key]['achv_smv']; ?>&nbsp;</td>

						<td width="100" align="right"><?echo number_format($eff_perc,2,'.',''); ?>&nbsp;</td>

						<td width="100" align="right"><?echo $buyer_data_array[$key]['man_power']; ?>&nbsp;</td>

						<td width="100" align="right"><?echo $buyer_data_array[$key]['operator']; ?>&nbsp;</td>

						<td width="100" align="right"><?echo $buyer_data_array[$key]['helper']; ?>&nbsp;</td>

						<td align="right"><?echo number_format($buyer_data_array[$key]['cm'],2,'.',''); ?>&nbsp;</th>
				...
				<?
					...
					/*$summ_toin+=$buyer_data_array[$key]['toin'];

					$summ_tpd+=$buyer_data_array[$key]['tpd'];

					$summ_topd+=$buyer_data_array[$key]['topd'];

					$summ_tosmv+=$buyer_data_array[$key]['tosmv'];

					$summ_achv_smv+=$buyer_data_array[$key]['achv_smv'];

					$summ_man_power+=$buyer_data_array[$key]['man_power'];

					$summ_operator+=$buyer_data_array[$key]['operator'];

					$summ_helper+=$buyer_data_array[$key]['helper'];

					$summ_cm+=$buyer_data_array[$key]['cm'];
				...
				$summ_achv_perc=$summ_topd/$summ_tpd*100;

				$summ_eff_perc=$summ_achv_smv/$summ_tosmv*100;*/
			   ...
			   <!-- <tfoot>
					...
					<th align="right"><?//echo $summ_toin; ?>&nbsp;</th>

					<th align="right"><?//echo $summ_tpd; ?>&nbsp;</th>

					<th align="right"><?//echo $summ_topd; ?>&nbsp;</th>

					<th align="right"><?//echo number_format($summ_achv_perc,2,'.',''); ?>&nbsp;</th>

					<th align="right"><?//echo $summ_tosmv; ?>&nbsp;</th>

					<th align="right"><?//echo $summ_achv_smv; ?>&nbsp;</th>

					<th align="right"><?//echo number_format($summ_eff_perc,2,'.',''); ?>&nbsp;</th>

					<th align="right"><?//echo $summ_man_power; ?>&nbsp;</th>

					<th align="right"><?//echo $summ_operator; ?>&nbsp;</th>

					<th align="right"><?//echo $summ_helper; ?>&nbsp;</th>

					<th align="right"><?//echo number_format($summ_cm,2,'.',''); ?>&nbsp;</th>
				...
				<? // $total_man=return_field_value( "count(emp_code) as emp_code", "hrm_employee", "status_active=1 and is_deleted=0 and designation_id=15","emp_code", integration_params( 2 ));?>
			   ...
			   <!-- <tfoot>
					...
					<th colspan="9" align="right">Total Manpower (HRM System)</th>

					<th align="right"><?echo $total_man; ?>&nbsp;</th>

					<th align="right"><?echo $total_man; ?>&nbsp;</th>

					<th align="right"><?echo $total_man; ?>&nbsp;</th>

					<th align="right"><?echo number_format($total_man,2,'.',''); ?>&nbsp;</th>
			...
			<div style="width:2670px; max-height:400px; overflow-y:scroll" id="scroll_body">
				...
				<table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body">
					...
					<tfoot>
						...
						<!--<tr>
							...
							<th align="right"><?echo $total_machine_qnty; ?>&nbsp;</th>
							...
							<th align="right"><?echo $tot_today_target; ?>&nbsp;</th>
							...
							<th align="right"><?echo $tot_today_smv; ?>&nbsp;</th>
							...
							<th align="right"><?echo number_format($grand_tot_smv_used,2,'.',''); ?>&nbsp;</th>
						...
						<tr>
							...
							<th><? echo number_format($grand_today_ach_perc,2,'.',''); ?>&nbsp;</th>
							...
							<th><? echo number_format($grand_today_aff_perc,2,'.',''); ?></th>
							...
							<th><? echo number_format($grand_avg_aff_perc,2,'.',''); ?>&nbsp;</th>
...
<script>
		...
		<div id="report_container" >
			...
			<table border="1" class="rpt_table" rules="all" width="970" cellpadding="0" cellspacing="0">
				...
				$sql="SELECT  
					...
					sum(case when production_hour>'9:00'  and  production_hour<='10:00' then production_quantity else 0 end ) AS am10,
	...
	sum(case when TO_CHAR(production_hour,'HH24:MI')>'9:00'  and  TO_CHAR(production_hour,'HH24:MI')<='10:00' then production_quantity else 0 end ) AS am10,
	...
	where po_break_down_id in(".$po_id.") and item_number_id='".$item_id."' and location='".$location."' and floor_id='".$floor_id."' and sewing_line='".$sewing_line."' and prod_reso_allo='".$prod_reso_allo."' and production_type='$prod_type' and production_date='$prod_date' and is_deleted=0 and status_active=1";
                ...
                <tr><td colspan="11"><strong>Total: &nbsp;&nbsp;<? echo  $total_qnty;?> </strong></td></tr>
                 ...
                 <table border="1" class="rpt_table" rules="all" width="660" cellpadding="0" cellspacing="0">
                    ...
                    ?>
                        <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
                            ...
                            <td align="right"><? echo $day_smv; ?>&nbsp;</td>
                        </tr>
                    <?
                    $i++;
                    }
                    ...
                    <tfoot>
                        ...
                        <th align="right"><? echo $total_smv_used; ?>&nbsp;</th>
                    </tfoot>
                </table>
            </div>	
        </div>
