...
{ 
	...
	foreach ($sql_res as $val) 
	{
		...
		$smry_data_array[$buyer_lib[$val['BUYER_NAME']]][$buyer_lib[$val['CLIENT_ID']]][$val['JOB_NO']][$garments_item[$val['ITEM_ID']]]['total_lay'] += $val['TOTAL_LAY'];	
	...
	sum(e.production_qnty) as total_prod,sum(case when d.production_type=5 then e.reject_qty else 0 end) as sew_rej_qty,sum(case when d.production_type=11 then e.reject_qty else 0 end) as poly_rej_qty
		...
		from wo_po_details_master a,wo_po_color_size_breakdown c,pro_garments_production_mst d,pro_garments_production_dtls e where a.id=c.job_id and c.po_break_down_id=d.po_break_down_id and d.id=e.mst_id and c.id=e.color_size_break_down_id and a.status_active=1 and a.is_deleted=0 and c.status_active in(1,2,3) and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and d.production_type in(1,4,5,8,11) and e.production_qnty>0 $sql_gmts_cond group by  a.id,a.job_no,a.buyer_name,a.style_ref_no,a.client_id,c.item_number_id,c.color_number_id,c.size_number_id,d.remarks,d.production_type";		
	// echo $sql;die;
	$sql_res = sql_select($sql);
	$job_id_array = array();
	$item_id_array = array();
	...
	foreach ($sql_res as $val) 
	{
		...
		$smry_data_array[$buyer_lib[$val['BUYER_NAME']]][$buyer_lib[$val['CLIENT_ID']]][$val['JOB_NO']][$garments_item[$val['ITEM_ID']]]['style'] = $val['STYLE'];	
		$smry_data_array[$buyer_lib[$val['BUYER_NAME']]][$buyer_lib[$val['CLIENT_ID']]][$val['JOB_NO']][$garments_item[$val['ITEM_ID']]]['total_ex_qty'] += $val['TOTAL_PROD'];
					...
					<table cellspacing="0" cellpadding="0"  border="1" style="border-collapse: collapse;" rules="all"  width="<?=$smry_tbl_width;?>" class="rpt_table" align="left">
						...
						<thead>
							<tr>
								...
								<th width="80">Total Lay Cut Qty</th>
								<th width="80">Total QC Qty</th>
							...
							<tbody>
								...
								foreach ($smry_data_array as $byr_name => $byr_data) 
								{
									...
									foreach ($byr_data as $client_name => $client_data) 
									{
										...
										{											
											...
											foreach ($job_data as $itm_name => $row) 
											{
												...
												$total_ex_qty = $row['total_ex_qty'];
												$ex_bal = $order_qty - $total_ex_qty;
												?>
												<tr bgcolor="<? echo $bgcolor;?>" id="tr_<?= $i;?>" onClick="change_color('tr_<?= $i; ?>','<?= $bgcolor; ?>')">
													...
													<td width="80" align="right"><?=number_format($total_lay,0);?></td>
													<td width="80" align="right"><?=number_format($total_qc,0);?></td>
													...
													<td width="80" align="right"><?=number_format($total_ex_qty,0);?></td>
													<td width="80" align="right"><?=number_format($ex_bal,0);?></td>
											...
											<tr style="background: #E4CDA7;font-weight: bold;text-align: right;">
												...
												<td><?=number_format($job_total_lay,0);?></td>
												<td><?=number_format($job_total_qc,0);?></td>
												...
												<td><?=number_format($job_ex_qty,0);?></td>
												<td><?=number_format($job_ex_bal,0);?></td>
												<td></td>
											</tr>
											<?
										}
									...
									<tr style="background: #95D1CC;font-weight: bold;text-align: right;">
										...
										<td><?=number_format($byr_total_lay,0);?></td>
										<td><?=number_format($byr_total_cut_qc,0);?></td>
										...
										<td><?=number_format($byr_ex_qty,0);?></td>
										<td><?=number_format($byr_ex_bal,0);?></td>
										<td></td>
									</tr>
									<?
								}
					...
					<table style="border-collapse: collapse;"  border="1" class="rpt_table"  width="<?=$smry_tbl_width;?>" rules="all" align="left" >
						<tfoot>
							<tr>
								...
								<th width="80"><?=number_format($gr_total_lay,0);?></th>
								<th width="80"><?=number_format($gr_total_qc,0);?></th>
								...
								<th width="80"><?=number_format($gr_ex_qty,0);?></th>
								<th width="80"><?=number_format($gr_ex_bal,0);?></th>
								<th width="80"></th>
							</tr>
						</tfoot>
					</table>
					...
					<table cellspacing="0" cellpadding="0"  border="1" style="border-collapse: collapse;" rules="all"  width="<?=$tbl_width;?>" class="rpt_table" align="left">
						...
						<thead>
							<tr>
								...
								<th colspan="3" width="180">Cutting QC Qty </th>
							...
							<tbody>
								...
								{										
									...
									foreach ($byr_data as $client_name => $client_data) 
									{
										...
										foreach ($client_data as $job => $job_data) 
										{
											...
											{												
												...
												foreach ($itm_data as $clr_name => $clr_data) 
												{
													...
													foreach ($clr_data as $size_id => $row) 
													{
														...
														$today_cut_qc = $row[1]['today_prod'];
														$total_cut_qc = $row[1]['total_prod'];
														$cut_qc_balance = $total_lay - $total_cut_qc;

														$today_in = $row[4]['today_prod'];
														$total_in = $row[4]['total_prod'];
														$in_balance = $cut_qc_balance - $total_in;
														...
														$balance_qty = $order_qty - $total_ex;
														?>
														<tr bgcolor="<? echo $bgcolor;?>" id="tr_<?= $i;?>" onClick="change_color('tr_<?= $i; ?>','<?= $bgcolor; ?>')">
															<td width="20"><p><?=$i;?></p></td>
															<td width="100"><p><?=$byr_name;?></p></td>
															...
															<td width="60" align="right"><?=number_format($today_cut_qc,0);?></td>
															<td width="60" align="right"><?=number_format($total_cut_qc,0);?></td>
															<td width="60" align="right"><?=number_format($cut_qc_balance,0);?></td>
															...
															<td width="80" align="right"><?=number_format($balance_qty,0);?></td>
														</tr>
														<?
														$i++;
														...
														$clr_today_cut_qc += $today_cut_qc;
														$clr_total_cut_qc += $total_cut_qc;
														$clr_cut_qc_balance += $cut_qc_balance;
														...
														$itm_today_cut_qc += $today_cut_qc;
														$itm_total_cut_qc += $total_cut_qc;
														$itm_cut_qc_balance += $cut_qc_balance;
														...
														$job_today_cut_qc += $today_cut_qc;
														$job_total_cut_qc += $total_cut_qc;
														$job_cut_qc_balance += $cut_qc_balance;
														...
														$byr_today_cut_qc += $today_cut_qc;
														$byr_total_cut_qc += $total_cut_qc;
														$byr_cut_qc_balance += $cut_qc_balance;
														...
														$gr_today_cut_qc += $today_cut_qc;
														$gr_total_cut_qc += $total_cut_qc;
														$gr_cut_qc_balance += $cut_qc_balance;
													...
													<tr style="background: #cddcdc;font-weight: bold;text-align: right;">
														...
														<td width="60"><?=number_format($clr_today_cut_qc,0);?></td>
														<td width="60"><?=number_format($clr_total_cut_qc,0);?></td>
														<td width="60"><?=number_format($clr_cut_qc_balance,0);?></td>
														...
														<td width="80"><?=number_format($clr_balance,0);?></td>
													</tr>
													<?
												}
												?>
												<tr style="background: #dccdcd;font-weight: bold;text-align: right;">
													...
													<td width="60"><?=number_format($itm_today_cut_qc,0);?></td>
													<td width="60"><?=number_format($itm_total_cut_qc,0);?></td>
													<td width="60"><?=number_format($itm_cut_qc_balance,0);?></td>
													...
													<td width="80"><?=number_format($itm_balance,0);?></td>
												</tr>
												<?
											}
											?>
											<tr style="background: #E4CDA7;font-weight: bold;text-align: right;">
												...
												<td width="60"><?=number_format($job_today_cut_qc,0);?></td>
												<td width="60"><?=number_format($job_total_cut_qc,0);?></td>
												<td width="60"><?=number_format($job_cut_qc_balance,0);?></td>
												...
												<td width="80"><?=number_format($job_balance,0);?></td>
											</tr>
											<?
										}
									}
									...
									<tr style="background: #95D1CC;font-weight: bold;text-align: right;">
										...
										<td width="60"><?=number_format($byr_today_cut_qc,0);?></td>
										<td width="60"><?=number_format($byr_total_cut_qc,0);?></td>
										<td width="60"><?=number_format($byr_cut_qc_balance,0);?></td>
										...
										<td width="80"><?=number_format($byr_balance,0);?></td>
									</tr>
									<?
								}
								?>
					...
					<table style="border-collapse: collapse;"  border="1" class="rpt_table"  width="<?=$tbl_width;?>" rules="all" align="left" >
						<tfoot>
							<tr>
								...
								<th width="60"><?=number_format($gr_today_cut_qc,0);?></th>
								<th width="60"><?=number_format($gr_total_cut_qc,0);?></th>
								<th width="60"><?=number_format($gr_cut_qc_balance,0);?></th>
