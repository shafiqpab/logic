...
if($action=="report_generated")
{
	...
	$fabric_isssueToProcess_sql =sql_select("select b.booking_no, b.order_id,b.body_part_id,b.color_id,b.width,
	...
	$fabric_Desc_sql= sql_select("select a.id,b.lib_yarn_count_deter_id,c.booking_no,b.body_part_id  
		...
		where b.job_no=c.job_no and b.status_active=1 and c.is_deleted=0 and c.po_break_down_id in($po_break_down_ids) and a.fabric_description = b.id and c.booking_type = 3 group by a.id,b.lib_yarn_count_deter_id,c.booking_no,b.body_part_id order by c.booking_no");
					...
					<?  
						...
						foreach($sql_serviceBooking as $row)
						{
							...
							<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
								...
								<td width="60" align="right"><? $fab_issue=$fabric_issue_recv_arr[$row[csf("booking_no")]][$row[csf("po_break_down_id")]][$fabric_des_id_arr[$row[csf("pre_cost_fabric_cost_dtls_id")]][$row[csf("booking_no")]]][$fabric_des_body_part_id_arr[$row[csf("pre_cost_fabric_cost_dtls_id")]][$row[csf("booking_no")]]][$row[csf("dia_width")]][$row[csf("fabric_color_id")]]['batch_issue_qty']; echo number_format($fab_issue,2,'.',''); 
								...
								?></td>
								<td width="60" align="right"><? $fab_recv=$fabric_issue_recv_arr[$row[csf("booking_no")]][$row[csf("po_break_down_id")]][$fabric_des_id_arr[$row[csf("pre_cost_fabric_cost_dtls_id")]][$row[csf("booking_no")]]][$fabric_des_body_part_id_arr[$row[csf("pre_cost_fabric_cost_dtls_id")]][$row[csf("booking_no")]]][$row[csf("dia_width")]][$row[csf("fabric_color_id")]]['batch_recv_qty']; echo number_format($fab_recv,2,'.',''); ?></td>
