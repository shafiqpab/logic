...
if($action=="report_generate")
{
	...
	if ($cbo_store_name>0) 
	{
		...
		$store_cond2 = " and a.store_name = $cbo_store_name";
		$store_cond3 = " and b.store_name = $cbo_store_name";
		$store_cond4 = " and c.store_id = $cbo_store_name";
		$store_cond5 = " and b.store_id = $cbo_store_name";
		$from_store_cond = " and b.from_store = $cbo_store_name";
	...
	if(!empty($program_cond_trans) || !empty($booking_no_cond_program))
	{
		$programSqlForTrans = sql_select("select b.id , a.booking_no
		from ppl_planning_info_entry_mst a, ppl_planning_info_entry_dtls b
		where a.id = b.mst_id
		and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and (b.is_sales is null or b.is_sales = 0 or b.is_sales = 2) and a.company_id = $company_name $program_cond_trans $booking_no_cond_program $libas_date_cond_2");
		...
		//echo "<pre>";
		//print_r($all_prog_for_issue); die;

		$program_cond_trans = "";
		if ($programNos != "") 
		{
			$programNos = substr($programNos, 0, -1);
			if ($db_type == 0)
				$program_cond_trans = " and b.to_program in ($programNos) ";
			else
			{
				$programNoses = explode(",", $programNos);
				if (count($programNoses) > 1000)
				{
					$program_cond_trans = " and (";
					$programNoses = array_chunk($programNoses, 1000);
					$z = 0;
					foreach ($programNoses as $id)
					{
						$id = implode(",", $id);
						if ($z == 0)
							$program_cond_trans .= " b.to_program in(" . $id . ")";
						else
							$program_cond_trans .= " or b.to_program in(" . $id . ")";
						$z++;
					}
					$program_cond_trans .= ")";
				} else
	...
	else
	{
		if(str_replace("'","",$hide_order_id)!="")
		{
			...
			$search_po_cond=" and e.id in(".$po_id.")";
		...
		else
		{
			...
			$search_po_cond=" and e.po_number like '$po_number'";
			$trans_po_cond_roll=" and e.po_number like '$po_number'";
		}
	}

	$sql="SELECT a.job_no_prefix_num, d.id as prog_no, d.program_date, a.job_no, a.company_name, a.buyer_name, b.id as po_id,b.grouping, d.machine_dia,d.fabric_dia, b.po_number, c.gsm_weight, sum(c.program_qnty) as program_qnty, d.stitch_length, d.color_id, e.booking_no, e.fabric_desc,d.knitting_source, d.batch_no
	from ppl_planning_entry_plan_dtls c, ppl_planning_info_entry_dtls d, ppl_planning_info_entry_mst e,wo_po_details_master a, wo_po_break_down b
	where a.job_no=b.job_no_mst and e.id=d.mst_id and c.dtls_id=d.id and c.po_id=b.id and a.company_name=$company_name $buyer_id_cond $po_cond $job_no_cond $program_no_cond $date_cond $booking_no_cond $internal_ref_cond and c.is_sales !=1 and d.is_sales !=1
	group by d.id, b.id, b.grouping, d.machine_dia, d.fabric_dia, d.program_date, e.booking_no, e.fabric_desc, d.color_id, c.gsm_weight, d.stitch_length, a.job_no, a.job_no_prefix_num, a.company_name, a.buyer_name, b.po_number, a.insert_date,d.knitting_source, d.batch_no
	order by a.buyer_name, a.job_no, b.id";
	// echo $sql;
	...
	foreach ($nameArray as $val)
	{
		...
		//$all_knitting_source[$val[csf("knitting_source")]] = $val[csf("knitting_source")];
	}
	...
	if(count($program_arr)>0)
	{
		...
		$prod_sql="SELECT a.id, b.id as bid, c.id as cid, d.id as did, a.booking_id,c.po_breakdown_id as po_id, c.quantity as knitting_qnty, d.qnty, b.trans_id as trans_id, a.receive_date as receive_date, d.barcode_no 
		from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c
		left join pro_roll_details d on c.po_breakdown_id =d.po_breakdown_id and c.dtls_id=d.dtls_id and d.entry_form=2 and d.status_active=1
		where a.id=b.mst_id and c.dtls_id=b.id and a.item_category=13 and a.entry_form=2 and c.entry_form=2 and a.receive_basis=2 and b.status_active=1 and b.is_deleted=0 $prog_cond_for_rcv $store_cond
		group by a.id, b.id, c.id, d.id, a.booking_id,c.po_breakdown_id, c.quantity, d.qnty, b.trans_id, a.receive_date, d.barcode_no";
		// echo $prod_sql;die;
		...
		foreach($sql_prod as $row)
		{
			...
			{
				//if($check_did[$row[csf('did')]]=="")
				//{
					//$check_did[$row[csf('did')]]=$row[csf('did')];
		...
		//if($trans_po_cond != "" ||  $job_no_cond != "" || $buyer_id_cond != "" || $booking_no_cond_transRow != "" )
		//if(empty($program_cond_trans) && empty($booking_no_cond_program)) // if search by order,job
		if(empty($production_barcode_no_arr) ) 
		{
		...
		/*echo $production_barcode_cond_for_rcv.'empty';
		echo "<pre>";
		...
		{
			//echo count($production_id_arr);die;
			$all_production_id_arr = implode(",", $production_id_arr);
			$productionCond = ""; $production_cond_for_rcv = ""; $rollMst= ""; $roll_production_cond_for_rcv = "";
			if($db_type==2 && count($production_id_arr)>999)
			{
				...
				foreach($all_production_id_arr_chunk as $chunk_arr)
				{
					...
					$rollMst.=" b.mst_id in(".implode(",",$chunk_arr).") or ";
				}

				$production_cond_for_rcv.=" and (".chop($productionCond,'or ').")";
				$roll_production_cond_for_rcv.=" and (".chop($rollMst,'or ').")";

			}
			else
			{
				$production_cond_for_rcv=" and a.booking_id in($all_production_id_arr)";
				$roll_production_cond_for_rcv=" and b.mst_id in($all_production_id_arr)";
			}

			$recv_array=array();
			$sql_recv=sql_select("SELECT a.booking_id,c.po_breakdown_id as po_id,sum(c.quantity) as knitting_qnty, min(receive_date) as receive_date 
			from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c 
			where a.id=b.mst_id and c.dtls_id=b.id and a.item_category=13 and a.entry_form=22 and c.entry_form=22 and a.receive_basis=9 and b.status_active=1 and b.is_deleted=0 $production_cond_for_rcv $store_cond
			group by a.booking_id,c.po_breakdown_id");
			$minimum_date = "";
			foreach($sql_recv as $row)
			{
				...
				//$knitting_recv_date_array[$program_ref_arr[$row[csf('booking_id')]]][$row[csf('po_id')]] =$val[csf("receive_date")];
		...
		$barcode_arr = array();
		$barcodeData = sql_select("SELECT b.mst_id, b.barcode_no, b.po_breakdown_id, a.booking_id from inv_receive_master a, pro_roll_details b where a.id=b.mst_id and a.entry_form=2 and b.entry_form=2 and a.item_category=13 and a.receive_basis=2 $roll_production_cond_for_rcv $store_cond");
		foreach ($barcodeData as $row) 
		{
			...
			//$barcode_no_ref[$row[csf('barcode_no')]]["book"] =   $row[csf('booking_id')];
			$barcode_no_ref[$row[csf('barcode_no')]]["po"] =   $row[csf('po_breakdown_id')];
		}
		/*echo "<pre>";
		print_r($barcode_no_ref);die;*/
		//$production_barcode = array_filter($production_barcode);

		if(count($production_barcode_no_arr) > 0)
		{
			/*$all_production_barcode = implode(",", $production_barcode);
			$prodBar = ""; $prod_barcode_cond_for_rcv = "";
			if($db_type==2 && count($program_arr)>999)
			{
				$program_arr_chunk=array_chunk($production_barcode,999) ;
				foreach($program_arr_chunk as $chunk_arr)
				{
					$prodBar.=" barcode_no in(".implode(",",$chunk_arr).") or ";
				}

				$prod_barcode_cond_for_rcv.=" and (".chop($prodBar,'or ').")";

			}
			else
			{
				$prod_barcode_cond_for_rcv=" and barcode_no in($all_production_barcode)";
			}*/
			/*echo "<pre>";
			print_r($prod_barcode_cond_for_rcv);*/

			/*$delivery_qty_res = sql_select("SELECT sum(qnty) as qnty,a.booking_no,po_breakdown_id, min(receive_date) as receive_date 
			from pro_roll_details a, inv_receive_master b  
			where a.mst_id = b.id and b.entry_form = 58 and a.entry_form in (58) and a.status_active=1 and a.is_deleted=0 $production_barcode_cond_for_rcv group by a.booking_no, po_breakdown_id");*/

			$delivery_qty_res = sql_select("SELECT sum(b.qnty) as qnty,b.booking_no,b.po_breakdown_id, min(a.receive_date) as receive_date 
			from inv_receive_master a , pro_roll_details b
			where a.id=b.mst_id and b.entry_form = 58 and b.entry_form in (58) and b.status_active=1 and b.is_deleted=0 and $production_barcode_cond_for_rcv $store_cond
			group by b.booking_no, b.po_breakdown_id");
			...
			/*$delivery_qty_res = sql_select("SELECT barcode_no, qnty,entry_form,booking_no,po_breakdown_id 
			from pro_roll_details where entry_form in (61) and status_active=1 and is_deleted=0 and booking_without_order=0 and is_returned <> 1 $production_barcode_cond_for_rcv");*/
			$delivery_qty_res = sql_select("SELECT b.barcode_no, b.qnty, b.entry_form, b.booking_no, b.po_breakdown_id 
			from inv_grey_fabric_issue_dtls a, pro_roll_details  b
			where a.id=b.dtls_id and b.entry_form in (61) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.booking_without_order=0 and b.is_returned <> 1 
			$production_barcode_cond_for_rcv $store_cond2 ");
			foreach ($delivery_qty_res as  $val)
			{
				//$knitting_recv_qnty_array[$val[csf("booking_no")]][$val[csf("po_breakdown_id")]] = $val[csf("qnty")];
				$knit_issue_arr[$val[csf("booking_no")]][$val[csf("po_breakdown_id")]]["qnty"] += $val[csf("qnty")];
			}
		}
	}
	...
	/*if($trans_po_cond != "" ||  $job_no_cond != "" || $buyer_id_cond != "" || $booking_no_cond_transRow != "" )
	{
		//As transfer row order need not to have own program so transfer order cond have this other cond
		$trans_row_po_cond_sql =  sql_select("select e.id as po_id from wo_po_details_master a, wo_po_break_down e, wo_booking_dtls c where a.job_no = e.job_no_mst and e.id = c.po_break_down_id $trans_po_cond $job_no_cond $buyer_id_cond $booking_no_cond_transRow and a.status_active = 1 and e.status_active = 1 and c.status_active=1");
		foreach ($trans_row_po_cond_sql as $val)
		{
			$trans_po_arr[$val[csf("po_id")]] = $val[csf("po_id")];
		}
	}*/

	if($search_po_cond != "" ||  $job_no_cond != "" || $buyer_id_cond != "" || $booking_no_cond_transRow != "" || $internal_ref_cond_2 !="")
	{
		$trans_row_po_cond_sql =  sql_select("SELECT e.id as po_id from wo_po_details_master a, wo_po_break_down e, wo_booking_dtls c where a.job_no = e.job_no_mst and e.id = c.po_break_down_id and a.company_name =$company_name $search_po_cond $job_no_cond $buyer_id_cond $booking_no_cond_transRow $internal_ref_cond_2 and a.status_active = 1 and e.status_active = 1 and c.status_active=1 group by e.id");
		foreach ($trans_row_po_cond_sql as $val)
		{
			$trans_po_arr[$val[csf("po_id")]] = $val[csf("po_id")];
		}
	...
	$to_poids = implode(",", array_filter(array_unique($trans_po_arr)));
	if($to_poids !="")
	{
		//$to_poids=0;
		$to_pocond = $trns_to_po_cond = "";
		$from_pocond="";$from_pocond2="";$to_pocond2 =""; $transfer_all_po_cond=""; $transfer_all_po_cond2="";
		$yarnOrdCond = $yarn_order_cond = "";
		...
		if($db_type==2 && count($to_poids_arr)>999)
		{
			...
			foreach($to_poids_chunk as $chunk_arr)
			{
				...
				$to_pocond2.=" c.po_breakdown_id in(".implode(",",$chunk_arr).") or ";
				$from_pocond.=" a.from_order_id in(".implode(",",$chunk_arr).") or ";
				$from_pocond2.=" b.from_order_id in(".implode(",",$chunk_arr).") or ";
				$yarnOrdCond.=" b.po_breakdown_id in(".implode(",",$chunk_arr).") or ";

			}

			$trns_to_po_cond.=" and (".chop($to_pocond,'or ').")";
			$transfer_all_po_cond_from .= " and (" .chop($from_pocond,'or ').")";
			//$transfer_all_po_cond .= " and (" .chop($to_pocond,'or ')." or ". chop($from_pocond,'or ') .")";
			$transfer_all_po_cond2 .= " and (" .chop($to_pocond2,'or ').")";

			$yarn_order_cond.=" and (".chop($yarnOrdCond,'or ').")";

		}
		else
		{
			$trns_to_po_cond=" and a.to_order_id in($to_poids)";
			$transfer_all_po_cond_from = " and ( a.from_order_id in($to_poids))";
			// $transfer_all_po_cond = " and ( a.to_order_id in($to_poids) or a.from_order_id in($to_poids) )";
			$transfer_all_po_cond2 = " and ( c.po_breakdown_id in($to_poids))";

			$yarn_order_cond=" and b.po_breakdown_id in($to_poids)";
		}
		// echo $transfer_all_po_cond2;die;
		/*$transfer_sql="SELECT b.from_program,b.to_program, a.entry_form,
		...
		where a.id=b.mst_id  and c.dtls_id=b.id and c.trans_type in(5,6) and a.status_active=1 and a.is_deleted=0 and a.item_category=13 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and c.entry_form in (83,13,80) and a.transfer_criteria in (1,2,4,6) and  b.from_program>0 and b.to_program>0 and (c.is_sales is null or c.is_sales = 0 or c.is_sales = 2) $transfer_all_po_cond $trans_store_cond";*/ // and (B.FROM_STORE = 195 or B.TO_STORE=195)

		$from_transfer_sql="SELECT A.TRANSFER_SYSTEM_ID, b.from_program,b.to_program, a.entry_form, 
		c.quantity as item_transfer_out,
		a.from_order_id,a.to_order_id, b.from_store, B.TO_STORE, B.ACTIVE_DTLS_ID_IN_TRANSFER
		from inv_item_transfer_dtls b, inv_item_transfer_mst a,  order_wise_pro_details c 
		where a.id=b.mst_id and c.dtls_id=b.id  and a.id=b.mst_id  
		and c.trans_type=6 and a.status_active=1 and a.is_deleted=0 and a.item_category=13 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and c.entry_form in (83,13,80) and a.transfer_criteria in (1,2,4,6) and b.from_program>0 and b.to_program>0 and (c.is_sales is null or c.is_sales = 0 or c.is_sales = 2) $transfer_all_po_cond_from $from_store_cond  and  b.active_dtls_id_in_transfer =1";
		// echo $transfer_sql;
		$from_transfer_data_array=sql_select($from_transfer_sql); //and c.is_sales <> 1

		$transfer_qty_arr=array();
		foreach($from_transfer_data_array as $row_b)
		{
		...
		$to_transfer_sql="SELECT A.TRANSFER_SYSTEM_ID, b.from_program,b.to_program, a.entry_form, 
		c.quantity as item_transfer_in,
		a.from_order_id,a.to_order_id, b.from_store, B.TO_STORE, B.ACTIVE_DTLS_ID_IN_TRANSFER
		from inv_item_transfer_dtls b, inv_item_transfer_mst a,  order_wise_pro_details c 
		where a.id=b.mst_id and c.dtls_id=b.id  and a.id=b.mst_id  
		and c.trans_type=5 and a.status_active=1 and a.is_deleted=0 and a.item_category=13 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and c.entry_form in (83,13,80) and a.transfer_criteria in (1,2,4,6) and b.from_program>0 and b.to_program>0 and (c.is_sales is null or c.is_sales = 0 or c.is_sales = 2) $trns_to_po_cond $to_store_cond";
		// echo $transfer_sql;
		$to_transfer_data_array=sql_select($to_transfer_sql); //and c.is_sales <> 1

		// $transfer_qty_arr=array();
		foreach($to_transfer_data_array as $row_b)
		{
			if($row_b[csf("entry_form")] == "83" || $row_b[csf("entry_form")] == "13")
			{
				$transfer_qty_arr[$row_b[csf('to_program')]][$row_b[csf('to_order_id')]]['transfer_in']+=$row_b[csf('item_transfer_in')];
		...
		// ========================	Start =====================================
		/*$get_barcode_sql="SELECT (case when c.trans_type in(6) then d.qnty else 0 end) as item_transfer_out, c.trans_type, d.barcode_no,
		(case when c.trans_type in(5) then d.qnty else 0 end) as item_transfer_in, c.po_breakdown_id
		from inv_item_transfer_dtls b,inv_item_transfer_mst a,order_wise_pro_details c, pro_roll_details d 
		where a.id=b.mst_id and c.dtls_id=b.id and a.id=d.mst_id and b.id=d.dtls_id
		and c.trans_type in(5,6) and a.status_active=1 and a.is_deleted=0 and a.item_category=13 
		and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and c.entry_form in (82) and a.transfer_criteria in (1,4,6,2) 
		and (c.is_sales is null or c.is_sales = 0 or c.is_sales = 2) $transfer_all_po_cond2 $trans_store_cond";*/

		$from_get_barcode_sql="SELECT (case when c.trans_type in(6) then d.qnty else 0 end) as item_transfer_out, c.trans_type, d.barcode_no, c.po_breakdown_id
		from inv_item_transfer_dtls b,inv_item_transfer_mst a,order_wise_pro_details c, pro_roll_details d 
		where a.id=b.mst_id and c.dtls_id=b.id and a.id=d.mst_id and b.id=d.dtls_id
		and c.trans_type in(6) and a.status_active=1 and a.is_deleted=0 and a.item_category=13 
		and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and c.entry_form in (82) and a.transfer_criteria in (1,4,6,2) 
		and (c.is_sales is null or c.is_sales = 0 or c.is_sales = 2) $transfer_all_po_cond2 $from_store_cond";

		//echo $get_barcode_sql;die; // and (B.FROM_STORE = 195 or B.TO_STORE=195)
		$from_barcode_data_array=sql_select($from_get_barcode_sql);// and c.po_breakdown_id=d.po_breakdown_id
		$trans_barcode_arr=array();
		foreach($from_barcode_data_array as $row)
		{
			$transfer_qty_arr[$barcode_no_ref[$row[csf('barcode_no')]]["book"]][$row[csf('po_breakdown_id')]]['transfer_out']+=$row[csf('item_transfer_out')];
		}

		$to_get_barcode_sql="SELECT c.trans_type, d.barcode_no,
		(case when c.trans_type in(5) then d.qnty else 0 end) as item_transfer_in, c.po_breakdown_id
		from inv_item_transfer_dtls b,inv_item_transfer_mst a,order_wise_pro_details c, pro_roll_details d 
		where a.id=b.mst_id and c.dtls_id=b.id and a.id=d.mst_id and b.id=d.dtls_id
		and c.trans_type in(5) and a.status_active=1 and a.is_deleted=0 and a.item_category=13 
		and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and c.entry_form in (82) and a.transfer_criteria in (1,4,6,2) 
		and (c.is_sales is null or c.is_sales = 0 or c.is_sales = 2) $transfer_all_po_cond2 $to_store_cond";

		$to_barcode_data_array=sql_select($to_get_barcode_sql);// and c.po_breakdown_id=d.po_breakdown_id
		$trans_barcode_arr=array();
		foreach($to_barcode_data_array as $row)
		{
			$transfer_qty_arr[$barcode_no_ref[$row[csf('barcode_no')]]["book"]][$row[csf('po_breakdown_id')]]['transfer_in']+=$row[csf('item_transfer_in')];
		}
		/*echo "<pre>";
		...
		$yarn_lot_arr=array();
		$grey_qty_arr=array();
		$all_yarn_program_no = implode(",", $all_prog_for_issue);
		$yarnProgCond = ""; $all_yarn_program_no_cond = "";
		if($db_type==2 && count($all_prog_for_issue)>999)
		{
			$all_prog_for_issue_chunk=array_chunk($all_prog_for_issue,999) ;
			foreach($all_prog_for_issue_chunk as $chunk_arr)
			{
				$yarnProgCond.=" c.booking_id in(".implode(",",$chunk_arr).") or ";
			}

			$all_yarn_program_no_cond.=" and (".chop($yarnProgCond,'or ').")";

		}
		else
		{
			$all_yarn_program_no_cond=" and c.booking_id in($all_yarn_program_no)";
		}
		...
		where a.id=b.dtls_id and a.mst_id = c.id and c.receive_basis = 2 and b.entry_form =2 and c.entry_form = 2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $all_yarn_program_no_cond");
		//$yarn_order_cond
		...
		if(count($all_yarn_prod_id_arr) > 0)
		{
			...
			$supplier_yarn = return_library_array("select a.id, b.short_name from product_details_master a, lib_supplier b where  a.supplier_id = b.id $all_yarn_prod_id_cond and b.status_active = 1 and a.status_active=1","id","short_name");
		}
	}
	...
	//{
		...
		// echo "*".$transfer_all_po_cond."<br>=".$trns_to_po_cond."<br>=".$tran_job_no_cond."<br>==".$trans_po_cond."<br>";die;
		//$roll_maintained=return_field_value("fabric_roll_level","variable_settings_production","company_name =$company_name and variable_list=3 and item_category_id=13 and is_deleted=0 and status_active=1");
		//echo $roll_maintained;

		//if ($roll_maintained==2) // gross
		//{
			$trans_sql="SELECT b.to_program, $from_order_idCond, min(a.transfer_date) as transfer_date,
			sum(case when c.trans_type in(5) then c.quantity else 0 end) as item_transfer_in,a.to_order_id , e.po_number, d.job_no_prefix_num, d.buyer_name, e.grouping
			from inv_item_transfer_dtls b,inv_item_transfer_mst a, order_wise_pro_details c , wo_po_details_master d,wo_po_break_down e
			where a.id=b.mst_id and c.dtls_id=b.id and d.job_no = e.job_no_mst and a.to_order_id = e.id and c.trans_type in(5) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.item_category=13 and c.entry_form in (83,13) and a.entry_form in (83,13) and b.from_program>0 and b.to_program>0 and (c.is_sales <> 1 or c.is_sales is null) and a.transfer_criteria in (1,2,4) $trns_to_po_cond $tran_job_no_cond $trans_po_cond $program_cond_trans $buyer_id_cond_trans $internal_ref_cond_2 $to_store_cond
			group by b.to_program,a.to_order_id, e.po_number, d.job_no_prefix_num, d.buyer_name, e.grouping";
			//echo $trans_sql;die;  // B.TO_STORE=195
			...
			{
				/*if($trns_row_data[$row_b[csf('to_program')]] != ""){
					$trns_row_data[$row_b[csf('to_program')]].="##".$row_b[csf('to_order_id')].'!!!!'.$row_b[csf('item_transfer_in')]."!!!!".$row_b[csf('from_order_id')].'!!!!'.$row_b[csf('job_no_prefix_num')].'!!!!'.$row_b[csf('po_number')].'!!!!'.$row_b[csf('buyer_name')].'!!!!'.$row_b[csf('grouping')].'!!!!'.$row_b[csf('transfer_date')];
				}
				else{
					$trns_row_data[$row_b[csf('to_program')]]=$row_b[csf('to_order_id')].'!!!!'.$row_b[csf('item_transfer_in')]."!!!!".$row_b[csf('from_order_id')].'!!!!'.$row_b[csf('job_no_prefix_num')].'!!!!'.$row_b[csf('po_number')].'!!!!'.$row_b[csf('buyer_name')].'!!!!'.$row_b[csf('grouping')].'!!!!'.$row_b[csf('transfer_date')];
				}*/
		...
		//else // roll
		//{
			if($trans_po_cond != "" ||  $job_no_cond != "" || $buyer_id_cond != "" || $production_barcode_cond_for_rcv != "" )
			{
				$roll_trans_in_sql="SELECT $from_order_idCond, min(a.transfer_date) as transfer_date,
				sum(f.qnty) as item_transfer_in,b.to_order_id , e.po_number, d.job_no_prefix_num, d.buyer_name, e.grouping, f.barcode_no
				from inv_item_transfer_dtls b,inv_item_transfer_mst a, order_wise_pro_details c , wo_po_details_master d,wo_po_break_down e, pro_roll_details f
				where a.id=b.mst_id and c.dtls_id=b.id and d.job_no = e.job_no_mst and b.to_order_id = e.id and a.id=f.mst_id and b.id=f.dtls_id and c.trans_type in(5) and a.status_active=1 and a.is_deleted=0 
				and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.item_category=13 and c.entry_form in (82,83) and a.entry_form in (82,83) and c.is_sales <> 1 and a.transfer_criteria=4 $tran_job_no_cond $trans_po_cond_roll $buyer_id_cond_trans $internal_ref_cond_2 $production_barcode_cond_for_rcv $to_store_cond
				group by b.to_program,b.to_order_id, e.po_number, d.job_no_prefix_num, d.buyer_name, e.grouping,f.barcode_no";//$transfer_all_po_cond2   $production_barcode_cond_for_rcv
				//echo $roll_trans_in_sql;//die; // B.TO_STORE=195
				$roll_data_trans= sql_select($roll_trans_in_sql);

				if ($production_barcode_cond_for_rcv=="") 
				{
					// echo $production_barcode_cond_for_rcv.'==<br>**';
					//going to production f.barcode_no get program
					foreach ($roll_data_trans as $key => $row) 
					{
						$in_barcode_no_arr[$row[csf('barcode_no')]] = $row[csf('barcode_no')];
					}
					$in_barcode_no_arr = array_filter($in_barcode_no_arr);
					if(count($in_barcode_no_arr) > 0)
					{
						//echo count($in_barcode_no_arr);die;
						$all_in_barcode_no_arr = implode(",", $in_barcode_no_arr);
						$productionTrnasInBarcode = ""; $trans_in_production_barcode_cond_for_rcv = "";
						if($db_type==2 && count($in_barcode_no_arr)>999)
						{
							$all_in_barcode_no_arr_chunk=array_chunk($in_barcode_no_arr,999) ;
							foreach($all_in_barcode_no_arr_chunk as $chunk_arr)
							{
								$productionTrnasInBarcode.=" barcode_no in(".implode(",",$chunk_arr).") or ";
							}

							$trans_in_production_barcode_cond_for_rcv.=" and (".chop($productionTrnasInBarcode,'or ').")";
						}
						else
						{
							$trans_in_production_barcode_cond_for_rcv=" and barcode_no in($all_in_barcode_no_arr)";
						}
					}
					$barcode_arr = array();
					$barcodeData = sql_select("SELECT b.mst_id, b.barcode_no, b.po_breakdown_id, a.booking_id from inv_receive_master a, pro_roll_details b where a.id=b.mst_id and a.entry_form=2 and b.entry_form=2 and a.item_category=13 and a.receive_basis=2 $trans_in_production_barcode_cond_for_rcv $store_cond"); // a.STORE_ID=195
					foreach ($barcodeData as $row) 
					{
						//$production_barcode[$row[csf('barcode_no')]] =$row[csf('barcode_no')];
						$barcode_no_ref[$row[csf('barcode_no')]]["book"] =   $row[csf('booking_id')];
				...
				{
					/*$trns_row_data[$barcode_no_ref[$row[csf('barcode_no')]]["book"]][$row[csf('to_order_id')].'!!!!'.'from_order'.'!!!!'.$row[csf('job_no_prefix_num')].'!!!!'.$row[csf('po_number')].'!!!!'.$row[csf('buyer_name')].'!!!!'.$row[csf('grouping')].'!!!!'.$row[csf('transfer_date')]]['trans_qty_in']+=$row[csf('item_transfer_in')];
					$trns_row_data[$barcode_no_ref[$row[csf('barcode_no')]]["book"]][$row[csf('to_order_id')].'!!!!'.'from_order'.'!!!!'.$row[csf('job_no_prefix_num')].'!!!!'.$row[csf('po_number')].'!!!!'.$row[csf('buyer_name')].'!!!!'.$row[csf('grouping')].'!!!!'.$row[csf('transfer_date')]]['count']++;*/

					$trns_row_data[$barcode_no_ref[$row[csf('barcode_no')]]["book"]][$row[csf('to_order_id')].'!!!!'.'from_order'.'!!!!'.$row[csf('job_no_prefix_num')].'!!!!'.$row[csf('po_number')].'!!!!'.$row[csf('buyer_name')].'!!!!'.$row[csf('grouping')]]['trans_qty_in']+=$row[csf('item_transfer_in')];
					$trns_row_data[$barcode_no_ref[$row[csf('barcode_no')]]["book"]][$row[csf('to_order_id')].'!!!!'.'from_order'.'!!!!'.$row[csf('job_no_prefix_num')].'!!!!'.$row[csf('po_number')].'!!!!'.$row[csf('buyer_name')].'!!!!'.$row[csf('grouping')]]['count']++;

					$trns_row_prog_ref_data[$barcode_no_ref[$row[csf('barcode_no')]]["book"]] = $barcode_no_ref[$row[csf('barcode_no')]]["book"];

					//added in 03-01-2020
					$all_prog_for_issue[$barcode_no_ref[$row[csf('barcode_no')]]["book"]] = $barcode_no_ref[$row[csf('barcode_no')]]["book"];
				}
				unset($roll_data_trans);
			}
			if ($production_barcode_cond_for_rcv!="" || $trans_in_production_barcode_cond_for_rcv!="") 
			{
				$roll_issue="SELECT d.booking_no as program_no,b.remarks, d.qnty knitting_issue_qnty, d.po_breakdown_id
				from  inv_issue_master a, inv_grey_fabric_issue_dtls b , order_wise_pro_details c, pro_roll_details d
				where a.id=b.mst_id and b.id = c.dtls_id and a.id=d.MST_ID and b.id=D.DTLS_ID
				and c.trans_type = 2
				and a.item_category=13 and a.entry_form in (61) and c.entry_form in (61) and d.entry_form in (61)
				and a.status_active=1 and a.is_deleted=0
				and b.status_active=1 and b.is_deleted=0
				and c.status_active = 1 and c.is_deleted = 0
				and d.booking_no is not null $production_barcode_cond_for_rcv $trans_in_production_barcode_cond_for_rcv $store_cond3"; // b.STORE_NAME=195
				//echo $roll_issue;
				...
				unset($sql_roll_data);
				//echo "<pre>";print_r($knit_issue_arr);
			}
	...
	if($all_trans_prog)
	{
		...
		$progCond = $all_trans_prog_cond = ""; $all_program_for_trans_issue_cond=$progCond2=$progCond3=$all_yarn_program_no_cond2="";
		...
		if($db_type==2 && count($all_trans_prog_arr)>999)
		{
			...
			foreach($all_trans_prog_chunk as $chunk_prog)
			{
				...
				$progCond2.=" b.program_no in($chunk_prog_val) or ";
				$progCond3.=" c.booking_id in($chunk_prog_val) or ";
			}

			$all_trans_prog_cond.=" and (".chop($progCond,'or ').")";
			$all_program_for_trans_issue_cond.=" and (".chop($progCond2,'or ').")";
			$all_yarn_program_no_cond2.=" and (".chop($progCond3,'or ').")";

		}
		else
		{
			$all_trans_prog_cond=" and d.id in($all_trans_prog)";
			$all_program_for_trans_issue_cond=" and b.program_no in($all_trans_prog)";
			$all_yarn_program_no_cond2=" and c.booking_id in($all_trans_prog)";
			//$all_trans_prog3.=','.$all_trans_prog;
		}

		// =============================== Start
		if ($all_yarn_program_no_cond2!="") // gross transfered program yarn_lot_count
		{
			$yarn_production_wise_lot_count=sql_select("SELECT b.po_breakdown_id, a.yarn_lot, a.yarn_count as yarn_count, c.booking_id, a.id, a.yarn_prod_id
			from pro_grey_prod_entry_dtls a, order_wise_pro_details b,inv_receive_master c
			where a.id=b.dtls_id and a.mst_id = c.id and c.receive_basis = 2 and b.entry_form =2 and c.entry_form = 2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $all_yarn_program_no_cond2 $store_cond4"); // c.STORE_ID=195
			//$yarn_order_cond
		...
		$sql_arr = sql_select("SELECT a.job_no_prefix_num,c.is_sales, d.id as prog_no, d.program_date, a.job_no,a.job_no_prefix_num, a.company_name, a.buyer_name, b.id as po_id, d.machine_dia,d.fabric_dia, b.po_number, c.gsm_weight, c.program_qnty , d.stitch_length, d.color_id, e.booking_no, e.fabric_desc
		from ppl_planning_entry_plan_dtls c, ppl_planning_info_entry_dtls d, ppl_planning_info_entry_mst e,wo_po_details_master a, wo_po_break_down b
		where a.job_no=b.job_no_mst and e.id=d.mst_id and c.dtls_id=d.id and c.po_id=b.id
		and a.company_name=$company_name and c.is_sales <> 1 and e.is_sales <> 1 and a.status_active = 1 and a.is_deleted= 0 and b.status_active in (1,3) and b.is_deleted= 0 $all_trans_prog_cond and c.status_active = 1 and c.is_deleted= 0 and d.status_active = 1 and d.is_deleted= 0 and e.status_active = 1 and e.is_deleted= 0");
		//and b.status_active in (1,3) confirm by common team mamun
		$trns_data_arr=array();
	...
	$all_prog_for_issue = array_filter(array_unique($all_prog_for_issue));
	if(count($all_prog_for_issue)>0)
	{
		$progCond = $all_prog_no_for_issue_cond = $progCond2 = $progCond3 = $all_prog_no_for_issue_return_cond = $all_prog_no_for_issue_return_cond2 = "";
		$all_prog_no_for_issue = implode(",", $all_prog_for_issue);
		if($db_type==2 && count($all_prog_for_issue)>999)
		{
			$all_prog_for_issue_chunk=array_chunk($all_prog_for_issue,999) ;
			foreach($all_prog_for_issue_chunk as $chunk_prog)
			{
				$chunk_prog_val=implode(",",$chunk_prog);
				$progCond.=" b.program_no in($chunk_prog_val) or ";
				$progCond2.=" a.booking_id in($chunk_prog_val) or ";
				$progCond3.=" b.pi_wo_batch_no in($chunk_prog_val) or ";
			}

			$all_prog_no_for_issue_cond.=" and (".chop($progCond,'or ').")";
			$all_prog_no_for_issue_return_cond.=" and (".chop($progCond2,'or ').")";
			$all_prog_no_for_issue_return_cond2.=" and (".chop($progCond3,'or ').")";

		}
		else
		{
			$all_prog_no_for_issue_cond=" and b.program_no in($all_prog_no_for_issue)";
			$all_prog_no_for_issue_return_cond=" and a.booking_id in($all_prog_no_for_issue)";
			$all_prog_no_for_issue_return_cond2=" and b.pi_wo_batch_no in($all_prog_no_for_issue)";
		}
		...
		and b.status_active=1 and b.is_deleted=0
		and c.status_active = 1 and c.is_deleted = 0
		and b.program_no <> 0 and b.program_no is not null $all_prog_no_for_issue_cond $store_cond3");
		$idChkArr = array(); // b.STORE_NAME=195
		...
		$sql_iss_return=sql_select("SELECT c.po_breakdown_id, a.booking_no, c.quantity from inv_receive_master a, inv_transaction b, order_wise_pro_details c where a.id = b.mst_id and b.transaction_type =4 and b.id=c.trans_id and c.entry_form=51 and a.receive_basis=3 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $all_prog_no_for_issue_return_cond $store_cond5"); // b.STORE_ID=195
		...
		where a.id = b.mst_id and b.transaction_type=4 and b.id=c.trans_id and c.entry_form=84 and a.receive_basis=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $all_prog_no_for_issue_return_cond2 $store_cond5"; // b.STORE_ID=195
	...
	// =====================gross Transfer to Issue Start===========================
	$all_prog_for_issue = array_filter(array_unique($all_prog_for_issue));
	$all_trans_prog =array_filter(array_unique($trns_row_prog_ref_data));
	$arrDifProgram=array_diff($all_trans_prog, $all_prog_for_issue);
	$all_prog_trans_for_issue = array_filter(array_unique($arrDifProgram));
	if(count($all_prog_trans_for_issue)>0)
	{
		$progConds = $all_program_for_trans_issue_conds= "";
		$all_prog_no_for_issue = implode(",", $all_prog_trans_for_issue);
		if($db_type==2 && count($all_prog_trans_for_issue)>999)
		{
			$all_prog_for_issue_chunk=array_chunk($all_prog_trans_for_issue,999) ;
			foreach($all_prog_for_issue_chunk as $chunk_prog)
			{
				$chunk_prog_val=implode(",",$chunk_prog);
				$progConds.=" b.program_no in($chunk_prog_val) or ";
			}

			$all_program_for_trans_issue_conds.=" and (".chop($progConds,'or ').")";

		}
		else
		{
			$all_program_for_trans_issue_conds=" and b.program_no in($all_prog_no_for_issue)";
		}
	}
	// print_r($arrDif);die;
	/*$trans_program=array();
	foreach ($all_trans_prog as $key => $value) 
	{
		if (!isset($all_prog_for_issue[$value])) 
		{
		 	$trans_program[$value]=$value;
		} 
	}*/
	// print_r($trans_program);die;

	if ($all_program_for_trans_issue_conds!="") // gross Transfer to Issue
	{
		$sql_trans_issue_data=sql_select("SELECT b.program_no,b.remarks,c.quantity knitting_issue_qnty,c.po_breakdown_id,b.id
		from  inv_issue_master a,inv_grey_fabric_issue_dtls b , order_wise_pro_details c
		where a.id=b.mst_id and b.id = c.dtls_id
		and c.trans_type = 2
		and a.item_category=13 and a.entry_form in (16) and c.entry_form in (16)
		...
		and b.status_active=1 and b.is_deleted=0
		and c.status_active = 1 and c.is_deleted = 0 
		and b.program_no <> 0 and b.program_no is not null $all_program_for_trans_issue_conds $store_cond3");
		$idChkArr = array(); // $all_program_for_trans_issue_cond
		//$knit_issue_arr=array(); //problem is here and b.program_no!=19932 // b.STORE_NAME=195
		foreach($sql_trans_issue_data as $row)
		{
			$knit_issue_arr[$row[csf('program_no')]][$row[csf('po_breakdown_id')]]['qnty'] +=$row[csf('knitting_issue_qnty')];
			$knit_issue_arr[$row[csf('program_no')]]['knit_id']=$row[csf('knit_id')];
			$knit_issue_arr[$row[csf('program_no')]][$row[csf('po_breakdown_id')]]['remarks']=$row[csf('remarks')];
		}
		unset($sql_trans_issue_data);
	}
	// =====================gross Transfer to Issue End===========================
	...
	<fieldset style="width:2250px;">
		...
		<div style="width:2500px; overflow-y:scroll; max-height:450px;" id="scroll_body">
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="2480" class="rpt_table" id="tbl_list_search">
				<?
				
				$i=1;  $roll_arr=array(); $transfer_part_check_arr=array();
				foreach($nameArray as $row)
				{
					...
					$tot_balance=number_format($tot_balance,2,'.','');
					...
					if ((($get_upto_qnty == 1 && $tot_balance > $txt_qnty) || ($get_upto_qnty == 2 && $tot_balance < $txt_qnty) || ($get_upto_qnty == 3 && $tot_balance >= $txt_qnty) || ($get_upto_qnty == 4 && $tot_balance <= $txt_qnty) || ($get_upto_qnty == 5 && $tot_balance == $txt_qnty) || $get_upto_qnty == 0) && ($cbo_value_with==0 || ($cbo_value_with ==1 && $tot_balance>0 )))
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
							...
							<td align="right" width="80" title="<? echo $row[csf('po_id')].'='.$row[csf('prog_no')].'='.$row[csf('booking_no')]; ?>">
				...
				$roll_arr=array();
				//echo "<pre>";
				//print_r($transfer_part_check_arr);
...
if($action=="receive_grey_popup")
{
	...
	<fieldset style="width:1200px; margin-left:3px">
		...
		<div id="report_container" style="width:100%">
			...
			<table border="1" class="rpt_table" rules="all" width="1180" cellpadding="0" cellspacing="0" id="report_table_footer">
				<tfoot>
					<th width="30">&nbsp;</th>
					<th width="80" >&nbsp;</th>
					<th width="80">&nbsp;</th>
					...
					<th width="80" align="right" id="value_total_rcv_qty"><? echo number_format($total_balance,2); ?></th>
					<th width="80" >&nbsp;</th>
					<th width="80" >&nbsp;</th>
					<th width="80" >&nbsp;</th>
					<th width="80" >&nbsp;</th>
			...
			<div style="width:638px; overflow-y:scroll; max-height:250px;font-size:12px;" id="scroll_body">
				<table border="1" class="rpt_table" rules="all" width="620" cellpadding="0" cellspacing="0" id="table_body">
					<tbody>
						...
						$mrr_sql="SELECT a.knit_dye_source,a.knit_dye_company,a.issue_number,a.order_id,a.issue_date,a.challan_no,b.no_of_roll,b.rack,b.program_no, sum(c.quantity) as knitting_issue_qnty 
						from inv_issue_master a, inv_grey_fabric_issue_dtls b ,order_wise_pro_details c where a.id=b.mst_id and b.id =  c.dtls_id and a.item_category=13 and b.program_no='$prog_no' and c.po_breakdown_id in($po_id) and a.entry_form in (16,61) and c.entry_form in (16,61) and  a.issue_basis=3 and b.status_active=1 and b.is_deleted=0 $store_cond
						...
						SELECT a.knit_dye_source,a.knit_dye_company,a.issue_number,a.order_id,a.issue_date,a.challan_no,b.no_of_roll,b.rack,b.program_no, sum(d.qnty) as knitting_issue_qnty 
						from inv_issue_master a, inv_grey_fabric_issue_dtls b , pro_roll_details d 
						where a.id=b.mst_id  and a.id = d.mst_id and b.id = d.dtls_id and a.entry_form in (16,61) and d.entry_form in (16,61) and b.status_active=1 and b.is_deleted=0  and d.booking_no='$prog_no' and d.po_breakdown_id in($po_id)  $store_cond
						group by b.program_no, b.issue_qnty,a.issue_number,a.issue_date, a.challan_no, b.no_of_roll,b.rack,a.order_id, a.knit_dye_source, a.knit_dye_company";
		...
		<div id="report_container" style="width:100%">
			...
			<div style="width:560px; overflow-y:scroll; max-height:250px;font-size:12px; overflow-x:hidde;" id="scroll_body">
				<table border="1" class="rpt_table" rules="all" width="540" cellpadding="0" cellspacing="0" id="table_body">
					<tbody>
						<?
						$sql_data=("SELECT b.from_program,b.to_program,b.rack, b.shelf, b.transfer_qnty as transfer_qnty,a.from_order_id 
						from inv_item_transfer_dtls b, inv_item_transfer_mst a, order_wise_pro_details c 
						where a.id=b.mst_id  and c.dtls_id=b.id and c.trans_type=6 and a.status_active=1 and a.is_deleted=0 and a.item_category=13 and a.transfer_criteria in(2,4) and  b.from_program='$prog_no'  and a.from_order_id in($po_id) and b.from_program>0 and b.to_program>0 $store_cond");
						// echo $sql_data;
						$data_array=sql_select($sql_data);//$store_cond
						$transfer_qty_arr=array();
						foreach($data_array as $row_b)
						{
						...
						$iss_data=sql_select("SELECT b.rack, b.self, sum(c.quantity) as issue_qnty 
						from inv_issue_master a, inv_grey_fabric_issue_dtls b ,order_wise_pro_details c 
						where a.id=b.mst_id and b.id = c.dtls_id and a.company_id='$companyID' and a.item_category=13 and b.program_no='$prog_no' and c.po_breakdown_id in ($po_id) and a.entry_form in (16) and c.entry_form in (16) and a.issue_basis=3 and b.status_active=1 and b.is_deleted=0 $store_iss_cond
						group by b.rack,b.self");//$store_iss_cond
						...
						$sql_prod="SELECT a.id, a.booking_id,b.order_id, b.rack, b.self, sum(c.quantity) as recv_qnty, max(b.trans_id) as trans_id,a.roll_maintained 
						from inv_receive_master a, pro_grey_prod_entry_dtls b ,order_wise_pro_details c 
						where a.id=b.mst_id and b.id = c.dtls_id and c.entry_form=2 and a.item_category=13 and a.entry_form=2 and a.company_id='$companyID' and a.booking_id='$prog_no' and a.receive_basis=2 and b.status_active=1 and b.is_deleted=0  and c.po_breakdown_id in ($po_id) $store_rcv_cond2
						group by a.id,a.booking_id,b.order_id,b.rack,b.self,a.roll_maintained";//$store_rcv_cond
						// echo $sql_prod;
						...
						if($recv_id!="")
						{
							$sql_recv="SELECT b.rack, b.self, sum(c.quantity) as recv_qnty 
							from inv_receive_master a, pro_grey_prod_entry_dtls b  ,order_wise_pro_details c  
							where a.id=b.mst_id and b.id = c.dtls_id and a.item_category=13 and a.company_id='$companyID' and a.booking_id in($recv_id) and a.entry_form=22 and a.receive_basis=9 and c.po_breakdown_id in ($po_id)  and b.status_active=1 and b.is_deleted=0 $store_rcv_cond2
							group by b.rack, b.self"; //$store_rcv_cond2
							// echo $sql_recv;
						...
						$trans_in="SELECT b.from_program,b.to_program,b.rack, b.shelf, b.transfer_qnty as transfer_qnty,a.to_order_id 
						from inv_item_transfer_dtls b,inv_item_transfer_mst a,order_wise_pro_details c 
						where a.id=b.mst_id and c.dtls_id=b.id and c.trans_type=5 and a.status_active=1 and a.is_deleted=0 and a.item_category=13 and a.transfer_criteria in(2,4) and b.to_program='$prog_no'
						and a.to_order_id in($po_id) and b.from_program>0 and b.to_program>0 $to_store_cond";
						// echo $trans_in;
						$data_array_trans_in=sql_select($trans_in);
						foreach($data_array_trans_in as $rows)
						{
							$transfer_in_qty_arr[$rows[csf('rack')]][$rows[csf('shelf')]]['in_qnty']+=$rows[csf('transfer_qnty')];
							$rack_shelf_arr[$rows[csf('rack')]][$rows[csf('shelf')]]=$rows[csf('transfer_qnty')];
						}
						// echo '<pre>';print_r($transfer_in_qty_arr);

						// for roll receive start ----------------------------------------------
						$barcode_res = sql_select("SELECT  a.barcode_no from 
						pro_roll_details a, inv_receive_master b where a.entry_form = 2 and a.mst_id = b.id and a.booking_no = '$prog_no' and a.po_breakdown_id in ($po_id) and a.entry_form = 2 and a.status_active = 1 and a.is_deleted = 0 and b.company_id = $companyID");
						...
						if($all_productionBarcode) 
						{
							...
							and c.status_active=1 and c.is_deleted=0 and c.entry_form in(82) and a.entry_form in(82) and a.transfer_criteria in (1,2,4,6) and (c.is_sales is null or c.is_sales = 0 or c.is_sales = 2) $all_Barcode_cond 
							group by b.rack, b.shelf, c.entry_form"); //$store_cond
						...
						//echo "<pre>";print_r($iss_return_arr);
						// for roll receive end ----------------------------------------------

						$issue_return_sql = sql_select("SELECT c.quantity, b.rack, b.self 
						from inv_receive_master a, inv_transaction b, order_wise_pro_details c 
						where a.id = b.mst_id and b.transaction_type = 4 and b.id = c.trans_id and c.entry_form=51 and a.receive_basis=3  and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and a.company_id='$companyID' and a.booking_id = '$prog_no' and c.po_breakdown_id in ($po_id) $store_rcv_cond"); // $store_rcv_cond
						...
						$i=1;
						//echo "<pre>";print_r($rack_shelf_arr);
						foreach($rack_shelf_arr as $rack=>$data)
						{
							foreach($data as $shelf=>$qty)
							{
								...
								$trans_in=$transfer_in_qty_arr[$rack][$shelf]['in_qnty'];
								$tran_to_issue=$transfer_qty_issue_arr[$rack][$shelf]['from_qnty'];
								$tot_recv=$recv_qty+$tran_to_issue + $iss_return+$trans_in;
								$tot_issue=$iss_qty+$trans_from_recv;
								$stock_qty=$tot_recv-$tot_issue;
								?>
								<tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
									<td width="40"><p><? echo $i; ?>&nbsp;</p></td>
									<td width="100" title="<? echo $rack; ?>"><p><? echo $rackDetails[$rack]; ?>&nbsp;</p></td>
									<td width="100" title="<? echo $shelf; ?>"><p><? echo $selfDetails[$shelf]; ?>&nbsp;</p></td>
			...
			<div style="width:638px; overflow-y:scroll; max-height:250px;font-size:12px;" id="scroll_body">
				<table border="1" class="rpt_table" rules="all" width="620" cellpadding="0" cellspacing="0" id="table_body">
					<tbody>
						...
						and b.status_active=1 and b.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
						group by b.program_no, b.issue_qnty,a.issue_number,a.issue_date,a.challan_no,b.no_of_roll, b.rack,a.order_id, a.knit_dye_source,a.knit_dye_company";
				...
				<table border="1" class="rpt_table" rules="all" width="540" cellpadding="0" cellspacing="0" id="table_body">
					<tbody>
						...
						$sql_result=("SELECT b.from_program,b.to_program,b.rack, b.shelf, b.transfer_qnty as transfer_qnty,a.to_order_id from inv_item_transfer_dtls b,inv_item_transfer_mst a,order_wise_pro_details c where a.id=b.mst_id  and c.dtls_id=b.id and c.trans_type=6 and a.status_active=1 and a.is_deleted=0 and a.item_category=13 and a.transfer_criteria=4 and b.to_program='$prog_no' and a.to_order_id in($po_id) and b.from_program>0 and b.to_program>0 
							...
							sum(d.qnty) as transfer_qnty,
							case when c.entry_form=82 then b.from_order_id else 0 end as from_order_id
							from inv_item_transfer_dtls b,inv_item_transfer_mst a,order_wise_pro_details c, pro_roll_details d 
							where a.id=b.mst_id and c.dtls_id=b.id and a.id=d.mst_id and b.id=d.dtls_id and c.trans_type in(5) and a.status_active=1 and a.is_deleted=0 and a.item_category=13 and b.status_active=1 and b.is_deleted=0 
		...
		<div id="report_container" style="width:100%">
			...
			<div style="width:500px; overflow-y:scroll; max-height:250px;font-size:12px; overflow-x:hidden;" id="scroll_body">
				<table border="1" class="rpt_table" rules="all" width="500" cellpadding="0" cellspacing="0" id="table_body">
					...
					$job_array=array();
					$sqls="SELECT b.id,a.job_no,b.po_number from wo_po_details_master a,wo_po_break_down b where a.job_no=b.job_no_mst and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0   group by b.id,a.job_no,b.po_number ";
					...
					and c.entry_form in (13) and a.entry_form in (13) and a.transfer_criteria in (1,2,4) and  b.from_program>0 and b.to_program>0 and (c.is_sales <> 1 or c.is_sales is null) and b.to_program in($prog_no) and a.to_order_id in($po_id) $store_cond
					...
					sum(d.qnty) as qnty from inv_item_transfer_dtls b,inv_item_transfer_mst a,order_wise_pro_details c, pro_roll_details d
					where a.id=b.mst_id  and c.dtls_id=b.id and a.id=d.mst_id and b.id=d.dtls_id and c.trans_type in(5) and a.status_active=1 and a.is_deleted=0 and a.item_category=13
					and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
					and c.entry_form in (83,82) and a.entry_form in (83,82) and a.transfer_criteria in (1,2,4,6) and (c.is_sales is null or c.is_sales = 0 or c.is_sales = 2) and c.po_breakdown_id in($po_id) $production_barcode_cond_for_rcv $store_cond
	...
	<div>
		<fieldset style="width:550px; ">
			...
			<div id="report_container" style="width:100%">
				...
				<div style="width:550px; overflow-y:scroll; max-height:250px;font-size:12px; overflow-x:hidden;" id="scroll_body">
					<table border="1" class="rpt_table" rules="all" width="500" cellpadding="0" cellspacing="0" id="table_body">
						...
						union all 
						SELECT a.transfer_system_id as sys_num, a.transfer_date, 
						case when c.entry_form=82 then b.to_order_id when c.entry_form=83 then a.to_order_id else 0 end as to_order_id, null as booking_no, c.entry_form, sum(case when c.trans_type in(6) then c.quantity else 0 end) as qnty  from inv_item_transfer_dtls b,inv_item_transfer_mst a,order_wise_pro_details c, pro_roll_details d
						...
						and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
						and c.entry_form in (83,82) and a.entry_form in (83,82) and a.transfer_criteria in (1,2,4,6) and (c.is_sales is null or c.is_sales = 0 or c.is_sales = 2) and c.po_breakdown_id in($po_id) $production_barcode_cond_for_rcv $store_cond
						group by a.transfer_system_id, a.transfer_date, c.entry_form,b.to_order_id,a.to_order_id";
						// echo $data_array;
						$result = sql_select($data_array);
						foreach($result as $row)
						{
