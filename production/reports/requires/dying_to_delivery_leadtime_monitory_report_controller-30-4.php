...
//echo "DDDDDD";die;
	...
	//-----------------------===========For SubCon============Area ----------------------------
 $subcon_dyeing_sql_unload="SELECT a.id as mst_id,a.service_company,a.floor_id,a.service_source,a.insert_date,a.remarks,a.batch_id, a.result,a.batch_no,a.process_id,a.entry_form,a.start_hours,a.start_minutes,a.production_date as end_date,a.load_unload_id, a.batch_ext_no,a.process_start_date,a.process_end_date,a.end_minutes,a.end_hours,c.booking_no,c.batch_weight,c.update_date,c.insert_date as batch_date_time,c.batch_date,c.booking_without_order,c.dur_req_hr,c.dur_req_min,d.cust_style_ref,d.order_no,e.party_id as buyer_id from pro_batch_create_mst c,pro_batch_create_dtls b,pro_fab_subprocess a,subcon_ord_dtls d,subcon_ord_mst e where  c.id=b.mst_id and c.id=a.batch_id and d.id=b.po_id and d.job_no_mst=e.subcon_job and c.entry_form=36 and a.status_active=1 and c.status_active=1  and a.load_unload_id=2  $batch_year_cond $batch_cond3 $prod_comp_cond $prod_date_cond $buyer_cond order by a.id asc";
	...
	$sub_fin_fab_sql= sql_select("select c.insert_date,c.product_date, b.batch_id,b.product_qnty from  subcon_production_mst c,subcon_production_dtls b where c.id=b.mst_id  and c.status_active=1  and c.entry_form in(292)  and b.status_active=1 and b.is_deleted=0 $sub_batch_cond_for2 order by b.batch_id");
	...
	foreach($sub_fin_fab_sql as $row)
	{
		...
		$sub_fin_fab_qty_arr[$row[csf("batch_id")]][12]["receive_qnty"]+=$row[csf("product_qnty")];
	}
	unset($fin_fab_sql);
	$sub_fin_delivery_sql= sql_select("select c.insert_date,c.delivery_date, b.batch_id,b.delivery_qty from  subcon_delivery_dtls b,subcon_delivery_mst c where  b.mst_id=c.id  and c.status_active=1 and c.process_id in(4)  and b.status_active=1 and b.is_deleted=0 $sub_batch_cond_for2 order by b.batch_id");
	//echo "select c.insert_date,c.delivery_date, b.batch_id,b.delivery_qty from  subcon_delivery_dtls b,subcon_delivery_mst c where  b.mst_id=c.id  and c.status_active=1 and c.process_id in(4)  and b.status_active=1 and b.is_deleted=0 $sub_batch_cond_for2 order by b.batch_id";
	...
	foreach($sub_fin_delivery_sql as $row)
	{
		...
		$fin_fab_deli_qty_arr[$row[csf("batch_id")]][12]["current_delivery"]+=$row[csf("delivery_qty")];
	}
	...
	if($rpt_type==0)
	{
		if($rpt_type==0) $width=8250;else $width=500;
		?>
								...
								foreach($batch_wise_dying_arr as $batch_id=>$row)
								{
											...
											//	echo $batch_date_time.'='.$heat_date_time;
												...
												<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $ii; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $ii; ?>">
													<td width="20" class=""><? echo $ii++ ;?></td>
													<td width="80" title="Prod Date"><? echo change_date_format($row["end_date"]) ;?> </td>
													<td width="80" class=""><?  if($within_group==2) echo $booking_data_arr2[$row[("sales_order_no")]]["buyer_id"];else echo $booking_data_arr[$row[("booking_no")]]["buyer_id"];//echo $company_library[$row["w_company_id"]];?> </td>
													<td width="80" style="word-break:break-all" class=""><? echo $booking_data_arr2[$row[("sales_order_no")]]["style_ref_no"];?></td>
													<td width="100" class=""><? echo $row[("sales_order_no")];?></td>
													<td width="100" class=""><? echo $row[("booking_no")];//$row["batch_no"];//$booking_data_arr2[$row[("sales_order_no")]]["style_ref_no"];?></td>
													<? ?>
													<td width="80" class=""><? echo $row[("batch_no")];//if($within_group==1) echo $booking_data_arr[$row[("booking_no")]]["job_no"];?></td>
													<td width="80" class=""><? echo $row["extention_no"];//$booking_data_arr2[$row[("sales_order_no")]]["season"];?> </td>
													<td width="80" align="right"><? echo number_format($row["batch_qty"],0);?></td>
													<td width="80" class=""><? echo change_date_format($row["batch_date"]);?></td>
													<td width="80" class=""><? echo $row[("batch_hr_min")];//$batch_hr_min;?> </td>
													<td width="80" class=""><? echo change_date_format($heatset_start_date);?></td>
													<td width="80" class=""><? echo  $heatset_start_time;?></td>
													<td width="80" class="" title="<? echo $batch_id;?>"><? echo change_date_format($heatset_end_date);?></td>
													<td width="80" class=""><? echo $heatset_endtime;?></td>
													<td width="80" class="">
														<?
													...
													<td width="80" class=""><?  if($heatset_end_date!='') echo floor($batchtoheat_time_diff/60).":".$batchtoheat_time_diff%60;
													else echo " "; ?> </td>
													<td width="80" class=""><?  echo change_date_format($load_date);?> </td>
													<td width="80" class=""><?  echo $load_time;?> </td>
													<td width="80" class="">
														<? echo change_date_format($unload_date);?>
													</td>
													<td width="80" class=""><? echo $unload_time;?> </td>
													<td width="80" class=""><?  if($unload_date!="")  echo $batchtodying_days_remian=datediff('d',$row["batch_date"],$unload_date)-1;
														else echo "";;?> </td>
													<td width="80" class="" align="right"><? if($batch_hr_min!=0) echo floor($batchtodying_time_diff/60).":".$batchtodying_time_diff%60;;?></td>
													<td width="80" class="" title="<? echo $sliting_start_date;?>" align="right">
													...
													<td width="80" class="" title="H:M">
														<?
														echo $sliting_endtime;
														?>
													</td>
													<td width="80" title="Unload Date Time" class="">
														<?
														
														 if($sliting_end_date!="")  echo $dyingtosliting_days_remian=datediff('d',$unload_date,$sliting_end_date)-1;//
														?>
													</td>
													<td width="80" class="" title="Unload Date Time=<? echo $unload_date_time;?>">
														<?
														 if($sliting_end_date!=0) echo floor($dyingtosliting_time_diff/60).":".$dyingtosliting_time_diff%60;
														 else echo " "; 
														?> 
													</td>
													<td width="80" class=""><? echo $dyeing_result[$row["sliting_result"]];?></td>
                                                    
													<td width="80" class=""><? echo change_date_format($stenter_start_date);?></td>
                                                    <td width="80" class=""><? echo $stenter_start_time;?></td>
													<td width="80" class=""><? echo change_date_format($stenter_end_date);?></td>
                                                    <td width="80" class=""><? echo $stenter_endtime ;?></td>
													<td width="80" class=""><?   if($stenter_end_date!="")  echo $dyingtostener_days_diff=datediff('d',$unload_date,$stenter_end_date)-1; ;?></td>
													<td width="80"  title="Unload Date Time=<? echo $unload_date_time;?>"><?  if($stenter_end_date!=0) echo floor($dyingtostener_time_diff/60).":".$dyingtostener_time_diff%60;
														 else echo " "; ?></td>
                                                    
                                                    <td width="80" class=""><? echo $dyeing_result[$row["stenter_result"]]  ;?></td>
													<td width="80" class=""><? echo change_date_format($type_stenter_start_date);?></td>
                                                    <td width="80" class=""><? echo $type_stenter_start_time;?></td>
													<td width="80" class=""><? echo change_date_format($type_stenter_end_date) ;?></td>
                                                    <td width="80" class=""><? echo $type_stenter_endtime;?></td>
													<td width="80" class=""><? if($type_stenter_end_date!="")  echo $dyingtotype_stener_time_diff=datediff('d',$unload_date,$type_stenter_end_date)-1;?></td>
													<td width="80" class=""><?   if($type_stenter_end_date!=0) echo floor($type_dyingtotype_stener_time_diff/60).":".$type_dyingtotype_stener_time_diff%60;
														 else echo " ";?></td>
                                                    
                                                    <td width="80" class=""><? echo  $dyeing_result[$row["type_stenter_result"]];?></td>
                                                    
													<td width="80" class=""><? echo change_date_format($dry_start_date);?></td>
                                                    <td width="80" class=""><? echo $dry_starttime;?></td>
													<td width="80" class=""><? echo change_date_format($dry_end_date);?></td>
                                                    <td width="80" class=""><? echo  $dry_endtime;?></td>
													<td width="80" class=""><? if($dry_end_date!="")  echo $dyingtodry_days_remian=datediff('d',$unload_date,$dry_end_date)-1;
														else echo "";?></td>
													<td width="80" title="Unload Date Time=<? echo $unload_date_time;?>"><? if($dry_end_date!=0) echo floor($dyingtodry_time_diff/60).":".$dyingtodry_time_diff%60;
														 else echo " ";?></td>
                                                   
                                                    <td width="80" class=""><? echo $dyeing_result[$row["dry_result"]];?></td>
                                                    
													<td width="80" class=""><? echo change_date_format($typewise_dry_start_date);?></td>
                                                    <td width="80" class=""><? echo $typewise_dry_start_time;?></td>
													<td width="80" class=""><? echo change_date_format($typewise_dry_end_date);?></td>
                                                    <td width="80" class=""><? echo $typewise_dry_endtime;?></td>
													<td width="80" class=""><? if($typewise_dry_end_date!="")  echo $dyingtotype_dry_days_remian=datediff('d',$unload_date,$typewise_dry_end_date)-1;
														else echo ""; ;?></td>
													<td width="80" class=""><? if($typewise_dry_end_date!=0) 
													echo floor($dyingtotypedry_time_diff/60).":".$dyingtotypedry_time_diff%60; 
													else echo " "; ?></td>
                                                    <td width="80" class=""><? echo  $dyeing_result[$row["type_dry_result"]];?></td>
                                                    
													<td width="80" class=""><? echo change_date_format($comp_start_date);?></td>
                                                    <td width="80" class=""><? echo $comp_start_time  ;?></td>
													<td width="80" class=""><? echo change_date_format($comp_end_date);?></td>
                                                    <td width="80" class=""><? echo $comp_endtime;?></td>
													<td width="80" class=""><? if($comp_end_date!="")  echo $dyingtocomp_days_remian=datediff('d',$unload_date,$comp_end_date)-1;
														else echo ""; ;?></td>
													<td width="80" class=""><? if($comp_end_date!=0) echo floor($dyingtocomp_time_diff/60).":".$dyingtocomp_time_diff%60;
														 else echo " "; ?></td>
                                                    
                                                    <td width="80" class=""><? echo  $dyeing_result[$row["comp_result"]];?></td>
                                                    
                                                    
													<td width="80" class=""><? echo change_date_format($typewise_comp_start_date);?></td>
                                                    <td width="80" class=""><? echo $typewise_comp_start_time;?></td>
													<td width="80" class=""><? echo change_date_format($typewise_comp_end_date); ;?></td>
                                                    <td width="80" class=""><? echo $typewise_comp_endtime;?></td>
													<td width="80" class=""><?  if($typewise_comp_end_date!="")  echo $dyingtotype_comp_days_remian=datediff('d',$unload_date,$typewise_comp_end_date)-1;
														else echo ""; ;?></td>
													<td width="80" class=""><?  if($typewise_comp_end_date!=0) 
													echo floor($dyingtotypecomp_time_diff/60).":".$dyingtotypecomp_time_diff%60; 
													else echo " "; ;?></td>
                                                    
                                                    <td width="80" class=""><? echo $dyeing_result[$row["type_comp_result"]];?></td>
                                                    
													<td width="80" class=""><? echo change_date_format($typewise_brush_start_date) ;?></td>
                                                    <td width="80" class=""><? echo $typewise_brush_start_time ;?></td>
													<td width="80" class=""><? echo change_date_format($typewise_brush_end_date) ;?></td>
                                                    <td width="80" class=""><? echo $typewise_brush_endtime;?></td>
													<td width="80" class=""><? if($typewise_brush_end_date!="")  echo $dyingtotype_brush_days_remian=datediff('d',$unload_date,$typewise_brush_end_date)-1;
							;?></td>
													<td width="80" class=""><? if($typewise_brush_end_date!=0) 
													echo floor($dyingtotypebrush_time_diff/60).":".$dyingtotypebrush_time_diff%60; 
													else echo " "; ?></td>
                                                     <td width="80" class=""><? echo $dyeing_result[$row["type_brush_result"]];?></td>
                                                    
                                                  
                                                  <td width="80" class=""><? echo change_date_format($typewise_peach_start_date) ;?></td>
                                                    <td width="80" class=""><? echo $typewise_peach_start_time ;?></td>
													<td width="80" class=""><? echo change_date_format($typewise_peach_end_date) ;?></td>
                                                    <td width="80" class=""><? echo $typewise_peach_endtime;?></td>
													<td width="80" class=""><? if($typewise_peach_end_date!="")  echo $dyingtotype_peach_days_remian=datediff('d',$unload_date,$typewise_peach_end_date)-1;?></td>
													<td width="80" class=""><? if($typewise_peach_end_date!=0) 
													echo floor($dyingtotypepeach_time_diff/60).":".$dyingtotypepeach_time_diff%60; 
													else echo " "; ?></td>
                                                  
                                                    <td width="80" class=""><? echo $dyeing_result[$row["type_peach_result"]];?></td>
                                                    <td width="80" class=""><? echo change_date_format($fin_prod_date)  ;?></td>
													<td width="80" class=""><? echo $fintime_cal ;?></td>
                                                    <td width="80" class=""><?  if($fin_prod_date!="")  echo $dyingtofinprod_days_remian=datediff('d',$unload_date,$fin_prod_date)-1;?></td>
													
													<td width="80" class=""><?  if($fin_prod_date!=0) 
													...
													<td width="80" class=""><? echo change_date_format($fin_deli_date);?></td>
                                                      <td width="80" class=""><? echo $deli_fintime_cal ;?></td>
                                                    <td width="80" class=""><?  if($fin_deli_date!="")  echo $dyingtofindeli_days_remian=datediff('d',$unload_date,$fin_deli_date)-1;?></td>
													<td width="80" class=""><?  if($fin_deli_date!=0) 
													...
													<td width="80" class=""><?   if($fin_deli_date!="")  echo $dyingtosliting_days_remian=datediff('d',$row["batch_date"],$fin_deli_date)-1;?></td>
                                                    <td width="80" title="Batch Date Time=<? echo $batch_date_time;?>" class=""><?  if($fin_deli_date!=0) 
													echo floor($batchtoDelifinprod_time_diff/60).":".$batchtoDelifinprod_time_diff%60; 
													else echo " " ;?></td>
                                                    <td width="80" class=""><? if($fin_deli_date!="")  echo $dyingtodeli_days_remian=datediff('d',$unload_date,$fin_deli_date)-1 ;//echo "M".$unload_date_time.'='.$fin_deli_prod_date_time;?></td>
                                                  
													<td width=""  title="Unload Date Time=<? echo $unload_date;?>"><?  if($fin_deli_date!=0) 
                            ...
                            <td colspan="102"><b>Inbound Subcontract</b></td>
								...
								foreach($sub_batch_wise_dying_arr as $batch_id=>$row)
								{
												...
												<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trsub_<? echo $ii; ?>','<? echo $bgcolor;?>')" id="trsub_<? echo $ii; ?>">
													<td width="20" class=""><? echo $ii++ ;?></td>
													<td width="80" title="Prod Date"><? echo change_date_format($row["end_date"]) ;?> </td>
													<td width="80" class=""><?  echo $booking_data_arr[$batch_id]["buyer_id"];//echo $company_library[$row["w_company_id"]];?> </td>
													<td width="80" style="word-break:break-all" class=""><? echo $booking_data_arr2[$batch_id]["style_ref_no"];?></td>
													<td width="100" class=""><? //echo $row[("sales_order_no")];?></td>
													<td width="100" class=""><? echo $booking_data_arr2[$batch_id]["order_no"];;//$row["batch_no"];//$booking_data_arr2[$row[("sales_order_no")]]["style_ref_no"];?></td>
													<? ?>
													<td width="80" class=""><? echo $row[("batch_no")];//if($within_group==1) echo $booking_data_arr[$row[("booking_no")]]["job_no"];?></td>
													<td width="80" class=""><? echo $row["extention_no"];//$booking_data_arr2[$row[("sales_order_no")]]["season"];?> </td>
													<td width="80" align="right"><? echo number_format($row["batch_qty"],0);?></td>
													<td width="80" class=""><? echo change_date_format($row["batch_date"]);?></td>
													<td width="80" class=""><? echo $batch_hr_min;?> </td>
													<td width="80" class=""><? echo change_date_format($heatset_start_date);?></td>
													<td width="80" class=""><? echo  $heatset_start_time;?></td>
													<td width="80" class="" title="<? echo $batch_id;?>"><? echo change_date_format($heatset_end_date);?></td>
													<td width="80" class=""><? echo $heatset_endtime;?></td>
													...
													<td width="80" class=""><? echo change_date_format($stenter_end_date);?></td>
                                                    <td width="80" class=""><? echo $stenter_endtime ;?></td>
													<td width="80" class=""><?   if($stenter_end_date!="")  echo $dyingtostener_days=datediff('d',$unload_date,$stenter_end_date)-1; ;?></td>
													<td width="80" class=""><?  if($stenter_end_date!=0) echo floor($dyingtostener_time_diff/60).":".$dyingtostener_time_diff%60;
														 else echo " "; ?></td>
                                                    ...
                                                    <td width="80" class=""><? echo  $dyeing_result[$row["type_stenter_result"]];?></td>
													<td width="80" class=""><? echo change_date_format($dry_start_date);?></td>
                                                    <td width="80" class=""><? echo $dry_start_time;?></td>
													<td width="80" class=""><? echo change_date_format($dry_end_date);?></td>
                                                    <td width="80" class=""><? echo  $dry_endtime;?></td>
													<td width="80" class=""><? if($dry_end_date!="")  echo $dyingtodry_days_remian=datediff('d',$unload_date,$dry_end_date)-1;//
														else echo "";?></td>
													<td width="80" class=""><? if($dry_end_date!=0) echo floor($dyingtodry_time_diff/60).":".$dyingtodry_time_diff%60;
														 else echo " ";?></td>
							...
							;?></td>
                                                  ...
                                                  <td width="80" class=""><? echo change_date_format($typewise_peach_start_date) ;?></td>
													...
													echo floor($dyingtotypeDelifinprod_time_diff/60).":".$dyingtotypeDelifinprod_time_diff%60; 
                                                      ...
                                                      <td width="80" align="right"><? echo number_format($fin_deli_qnty_kg,2) ;?></td>
                                                    <td width="80" align="right"><? echo number_format($fin_deli_qnty_yds,2)  ;?></td>
													<td width="80" class=""><?   if($fin_deli_date!="")  echo $dyingtodeli_days_remian=datediff('d',$row["batch_date"],$fin_deli_date)-1;?></td>
                                                    <td width="80" class=""><?  if($fin_deli_date!=0) 
													echo floor($batchtoDelifinprod_time_diff/60).":".$batchtoDelifinprod_time_diff%60; 
													else echo " " ;?></td>
                                                    <td width="80" class=""><? if($fin_deli_date!="")  echo $dyingtodeli_days_remian=datediff('d',$unload_date,$fin_deli_date)-1 ;?></td>
                                                  
													<td width="" title="Unload Date Time=<? echo $unload_date;?>" class=""><?  if($fin_deli_date!=0) 
													echo floor($dyeingtoDelifinprod_time_diff/60).":".$dyeingtoDelifinprod_time_diff%60; 
													else echo " " ;;?></td>
