...
if($action==='report_generate')
{
	...
	if ($cbo_report_type == 2)
	{
		if($txt_date_from != '' && $txt_date_to != '' || $txt_job_no != '' || $txt_style_no != '')
		{
			...
			else $date_cond =" and c.ex_factory_date between '$txt_date_from' and '$txt_date_to'";

			$sqlEx="SELECT a.JOB_NO, c.PO_BREAK_DOWN_ID, c.EX_FACTORY_DATE from wo_po_details_master a, wo_po_break_down b, pro_ex_factory_mst c, pro_ex_factory_delivery_mst d where a.job_no=b.job_no_mst and b.id=c.po_break_down_id and c.delivery_mst_id=d.id and d.company_id='$cbo_company_name' and a.is_deleted=0 and b.status_active in(1,3) and b.is_deleted=0 and b.shiping_status=3 and c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1 $date_cond $job_no $style_no $buyer_cond $year_cond group by a.job_no, c.po_break_down_id, c.ex_factory_date order by a.job_no, c.ex_factory_date";
			$sqlEx_res=sql_select($sqlEx);
			$tot_rows=0; $jobNos='';$ex_factory_date_arr=array();
	...
	$sql_main="SELECT a.ID, a.JOB_NO_PREFIX_NUM, a.JOB_NO, a.COMPANY_NAME, a.BUYER_NAME, a.STYLE_REF_NO, b.PUB_SHIPMENT_DATE, b.id as PO_ID, b.PO_NUMBER, b.SHIPING_STATUS, b.po_quantity as ORDER_QTY, b.plan_cut as PLAN_CUT_QTY, sum(b.plan_cut*a.total_set_qnty) as PLAN_CUT_QTY_PCS, avg(b.excess_cut) as PLAN_CUT_PERCENT
		from wo_po_details_master a, wo_po_break_down b 
		where a.job_no=b.job_no_mst and a.is_deleted=0 and b.status_active in(1,3) and b.is_deleted=0 $company_cond $buyer_cond $year_cond $job_no $style_no $job_no_cond 
	...
	$sql_finishRecGreyUsedQty="SELECT d.JOB_NO_MST, a.RECV_NUMBER, a.RECEIVE_DATE, a.RECEIVE_BASIS, a.BOOKING_NO, a.KNITTING_SOURCE, a.CHALLAN_NO, a.KNITTING_COMPANY, b.BATCH_ID, b.PROD_ID, b.GSM, b.WIDTH, b.COLOR_ID, b.RACK_NO, b.grey_used_qty as GREY_USED_QTY, sum(CASE WHEN c.entry_form in(37,68,17) and c.trans_type=1 THEN c.quantity ELSE 0 END) AS FINISH_RECEIVE , sum(CASE WHEN c.entry_form in(37,68,17) and c.trans_type=1 THEN c.fab_meter_qnty ELSE 0 END) AS FAB_METER_QNTY
	from inv_receive_master a, pro_finish_fabric_rcv_dtls b, order_wise_pro_details c, wo_po_break_down d 
	where a.id=b.mst_id and b.id=c.dtls_id and c.po_breakdown_id=d.id and a.company_id=$cbo_company_name and a.entry_form in (37,68,17) and c.entry_form in (37,68,17) and c.trans_type=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active in(1,3) and d.is_deleted=0 $po_id_cond 
	...
	// SQL Cutting
	$sql_cutting="SELECT c.PO_BREAK_DOWN_ID, c.production_quantity as CUTTING_QTY,c.used_qty_kg as USED_KG,c.reject_qty_kg_break_down as REJECT_BREAK_DOWN from pro_garments_production_mst c where c.company_id=$cbo_company_name and c.production_type=1 and c.status_active=1 and c.is_deleted=0 $po_id_cond2";
	// echo $sql_cutting;
	...
	$table_width = 3920+80+80+80+80+80+80;
	ob_start();
		...
		<table class="rpt_table" border="1" rules="all" width="<? echo $table_width; ?>" cellpadding="0" cellspacing="0">
			<thead>
				<tr> 
					...
					<th style="background: #f5c6cb;" colspan="7">Yarn</th>
					<th style="background: #ffeeba;" colspan="5">Greige Fabric</th>
					<th style="background: #bee5eb;" colspan="6">Finish Fabric Information</th>
					<th style="background: #f5c6cb;" colspan="12">Fabric Store</th>
					<th style="background: #ffeeba;" colspan="11">Cutting</th>
				</tr>
				<tr>
					<th width="50">SL</th>
	                ...
	                <th width="80">Yarn Required</th>
	                <th width="80">Sample Booking Grey Qty</th>
	                <th width="80">Short Fabric Booking Yarn Qty</th>
	                <th width="80">Total Yarn Require Qty</th>
	                <th width="80">Yarn Allocation Qty</th>
	                ...
	                <th width="80">Finished Fabric Require Qty</th>
	                <th width="80">Finished Fabric Require Qty (MTR)</th>
	                ...
	                <th width="80">Fabric Stock %</th>             
	                <th width="80">Possible Cutting PCS</th>
	                ...
	                <th width="80">Return To Fabric Store</th>
	                <th>Last Ex-Factory Date</th>
				</tr>
			</thead>
		</table>
		<div style="width:<? echo $table_width+20; ?>px; overflow-y:scroll; max-height:300px" id="scroll_body">
		    <table class="rpt_table" border="1" rules="all" width="<? echo $table_width; ?>" cellpadding="0" cellspacing="0" id="table_body">
		        <tbody>
		        	...
		        	foreach ($job_alldata_arr as $job => $row)
		        	{
		        		...
		        		{						     		
			        		...
			        		$issue_qnty           = $yarn_issue_qnty-$yarn_issue_return;
			        		...
			        		$grey_used_qty        = $finishRecGreyUsedQty_arr[$job]['GREY_USED_QTY'];
							...
							$actual_cutt_cons  = ($finish_issue*12)/$cutting_qty;
				        	...
				        	<tr bgcolor="<? echo $bgcolor;  ?>" onclick="change_color('trds_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="trds_<? echo $i; ?>">
				        		...
				        		<td width="80"><p><? if ($exShipStatus == '3') echo 'Closed'; else echo 'Running'; ?></p></td>
				        		<td width="80" align="right"><p><? echo $row['ORDER_QTY']; ?></p></td>
				        		<td width="80" align="right"><p><? echo $row['PLAN_CUT_QTY']; ?></p></td>
				        		...
				        		<td width="80" align="right"><p><? echo number_format($yarn_allocation_qnty,2); ?></p></td>
				        		...
				        		<td width="80" align="right" title="(100*(Fabric Stock in Hand+Return to Fabric Store))/Finished Fabric Require Qty"><p><? echo number_format($fabric_stock_percent,2); ?></p></td>	
				        		<td width="80" align="right" title="Plan Cut Qty Pcs/Fin Fab Req*Fabric Issue To Cutting"><p><? echo number_format($possible_cutt_pcs); ?></p></td>
				        		<td width="80" align="right"><p><a href='#report_details' onClick="openmypage_popup('<? echo $cbo_company_name."__".$job; ?>','<? echo $jobwisepoId; ?>','cutting_popup','720px');"><? echo number_format($cutting_qty,2); ?></a></p></td>
				        		...
				        		<td width="80" align="right" title="(Fabric Issue To Cutting*12)/Cutting Qty"><p><? echo number_format($actual_cutt_cons,2); ?></p></td>
				        		...
				        		<td width="80" align="right"><p><a href='#report_details' onClick="openmypage_popup('<? echo $cbo_company_name."__".$job."__".$finishRecIssueReturn_prodID[$job]['PROD_ID']; ?>','<? echo $jobwisepoId; ?>','return_from_cuttting_popup','1200px');"><? echo number_format($fin_iss_retn_qnty,2); ?></a></p></td>

				        		<td align="center"><p><? echo change_date_format($ex_factory_date_arr[$job]['EX_FACTORY_DATE']); ?></p></td>
				        	</tr>
				        	<?
				        	$i++;
	    ...
	    <table class="rpt_table" border="1" rules="all" width="<? echo $table_width; ?>" cellpadding="0" cellspacing="0">
			<tfoot>
				<tr>
	        		...
	        		<th width="80" align="right" id="value_tot_yarn_allocation_qnty"><p><? echo number_format($tot_yarn_allocation_qnty,2); ?></p></th>
	        		...
	        		<th width="80" align="right" title="(100*(Total Fabric Stock in Hand+Total Return to Fabric Store))/Total Finished Fabric Require Qty"><p><? echo number_format($tot_fabric_stock_percent,2); ?></p></th>
	        		<th width="80" align="right" id="tot_possible_cutt_pcs"><p><? echo number_format($tot_possible_cutt_pcs,2); ?></p></th>
...
if ($action === 'yarn_issue_knitting_popup')
{
	...
	list($company_id, $job, $prod_id, $return_prod_id) = explode('__', $data);
            ...
            <table border="1" class="rpt_table" rules="all" width="<? echo $table_width; ?>" cellpadding="0" cellspacing="0" id="report_table_footer">
				<tfoot>
					<tr>
	                    ...
	                    <th width="80"></th>
	                    <th width="70"></th>
	                    <th width="120" align="right">Total Return:</th>
	                    <th width="80" align="right" id="value_total_issue_rtn_quantity"><? echo number_format($total_issue_rtn_quantity,2,'.','');?></th>
	                    <th width="120"></th>
	                    <th width="70"></th>
                    </tr>
					<tr>
						<th colspan="10" align="right">Total Issue:</th>
						<th align="right"><? echo number_format(($total_issue_quantity-$total_issue_rtn_quantity),2,'.','');?></th>
						<th colspan="2"></th>
					</tr>
				</tfoot>
            </table>
...
if($action==='finish_fabric_receive_popup')
{
	...
	list($company_id, $job, $finishRec_prodID, $finishRec_batchID, $finishRecIssueReturn_prodID) = explode('__', $data);
		...
		<div style="width:100%" id="report_container">     
            ...
            <div style="width:<? echo $table_width+20; ?>px; max-height:330px; overflow-y:scroll" id="scroll_body">
                <table border="1" class="rpt_table" rules="all" width="<? echo $table_width; ?>" cellpadding="0" cellspacing="0" id="table_body">
                    ...
                    $fab_process_type='';
                    //echo $order_id.'fdfgdg'.$finishRec_prodIDs.'sdgrg'.$finishRec_batchIDs;
                    if ($order_id != '' && $finishRec_prodIDs != '' && $finishRec_batchIDs != '')
                    {	
	                    $sql="SELECT a.RECV_NUMBER, a.RECEIVE_DATE, a.RECEIVE_BASIS, a.BOOKING_NO, a.KNITTING_SOURCE, a.CHALLAN_NO, a.KNITTING_COMPANY, a.INSERT_DATE, a.inserted_by as USER_ID, b.BATCH_ID, b.PROD_ID, b.GSM, b.WIDTH, b.COLOR_ID, b.RACK_NO, b.grey_used_qty as GREY_USED_QTY, sum(NVL(c.quantity, 0)) as QUANTITY 
        			...
        			foreach($result as $row)
                    {
                        ...
                        $process_loss = 100 - (($row['QUANTITY']/$row['GREY_USED_QTY'])*100);
                        ...
                        <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
		                    ...
		                    <td width="60" align="right"><p><? echo number_format($row['GREY_USED_QTY'],2); ?></p></td>
                    	...
                    	<?
                    	$i++;
                    	$total_grey_used_qty += $row['GREY_USED_QTY'];
	                ...
	                <tbody>
               		...
               		foreach ($sql_res as $row) 
               		{
						...
						<tr bgcolor="<?= $bgcolor; ?>" onClick="change_color('tr_<?= $i; ?>','<?= $bgcolor;?>')" id="tr_<? echo $i;?>">
                            ...
                            <td width="80" align="right"><?= $row['CUTQNTYPCS']; ?></td>
	                    ...
	                    $total_west_quantity += $break_down_ex[1];
	                    $total_cutpanel_quantity += $break_down_ex[2];
	                    $total_short_quantity += $break_down_ex[3];
	                    $total_cutting_pcs_quantity +=  $row['CUTQNTYPCS'];
