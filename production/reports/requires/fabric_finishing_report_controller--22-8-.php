...
if($action=="fabric_finishing_report")
{
	...
	$batch_data=sql_select("select a.batch_id,b.roll_no as roll_no_heat,b.prod_id,b.width_dia_type as dia_type,a.production_date,b.barcode_no,$rollID 
	...
	(CASE WHEN b.entry_page=35 and  a.load_unload_id in(2) THEN b.production_qty ELSE 0 END) AS unload_qty
	 from pro_fab_subprocess a, pro_fab_subprocess_dtls b where a.id=b.mst_id and a.entry_form in(32,30,35,31,48,33,34) and b.entry_page in(32,30,35,31,48,33,34) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $dates_com2 ");
	...
	if(($page_upto==7 || $page_upto>7) && $roll_maintained==1)
    {
        ...
        {       
		    ...
		    $batch_prod_qty_arr[$row[csf('batch_id')]][$row[csf('prod_id')]][$row[csf('dia_type')]][$row[csf('roll_id')]][$row[csf('roll_no_heat')]]['heat']+=$row[csf('heat_qty')];
			}
			
            $batch_prod_qty_arr[$row[csf('batch_id')]][$row[csf('prod_id')]][$row[csf('dia_type')]][$row[csf('roll_id')]][$row[csf('barcode_no')]]['sliting']+=$row[csf('sliting_qty')];
            $batch_prod_qty_arr[$row[csf('batch_id')]][$row[csf('prod_id')]][$row[csf('dia_type')]][$row[csf('roll_id')]][$row[csf('barcode_no')]]['drying']+=$row[csf('drying_qty')];
            $batch_prod_qty_arr[$row[csf('batch_id')]][$row[csf('prod_id')]][$row[csf('dia_type')]][$row[csf('roll_id')]][$row[csf('barcode_no')]]['stenter']+=$row[csf('stenter_qty')];
           if($page_upto==7 || $page_upto>7)
            {
			 if($row[csf('compact_qty')]>0)
			 {
		    $batch_prod_qty_arr[$row[csf('batch_id')]][$row[csf('prod_id')]][$row[csf('dia_type')]][$row[csf('roll_id')]][$row[csf('barcode_no')]]['compact']+=$row[csf('compact_qty')];   
			 }
			}
			else
			{
            ...
            $batch_prod_qty_arr[$row[csf('batch_id')]][$row[csf('prod_id')]][$row[csf('dia_type')]][$row[csf('roll_id')]][$row[csf('barcode_no')]]['special']+=$row[csf('special_qty')];
            ...
            $batch_prod_qty_arr2[$row[csf('batch_id')]][$row[csf('prod_id')]][$row[csf('dia_type')]]['compact']+=$row[csf('re_compact_qty')];
			 ...
			 $batch_prod_qty_arr2[$row[csf('batch_id')]][$row[csf('prod_id')]][$row[csf('dia_type')]]['heat']+=$row[csf('heat_qty')];
...
$sql="(
		...
		{
            $sql="(select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.po_id,b.prod_id,b.width_dia_type,$group_conct,c.job_no_mst,d.job_no_prefix_num,d.buyer_name,f.process_end_date,f.process_start_date,c.file_no,c.grouping,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.shift_name,f.machine_id,f.floor_id,f.remarks $sp_finish from pro_batch_create_dtls b,wo_po_break_down c,wo_po_details_master d,pro_batch_create_mst a,pro_fab_subprocess f where  f.batch_id=a.id and a.id=b.mst_id and f.entry_form=34 and  a.batch_against in(1,2,3) and b.po_id=c.id and d.job_no=c.job_no_mst $company_cond $working_company_cond   $dates_com $jobdata $batch_num $booking_num $buyerdata $order_no $year_cond $shift_cond $file_cond $ref_cond $color_name and a.entry_form=0  and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 GROUP BY b.po_id, b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type,c.job_no_mst, d.job_no_prefix_num, d.buyer_name, f.process_end_date,c.file_no,c.grouping,f.process_start_date,f.production_date,f.start_minutes,f.start_hours, f.end_hours, f.end_minutes,f.shift_name, f.machine_id,f.floor_id,f.remarks $finish_group)
            union
            (
                select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.po_id,b.prod_id,b.width_dia_type,null as po_number,null as po_id,null as job_no_mst,null as job_no_prefix_num,null as buyer_name,f.process_end_date,f.process_start_date,null as file_no,null as grouping,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.shift_name,f.machine_id,f.floor_id,f.remarks $sp_finish from pro_batch_create_dtls b,pro_batch_create_mst a,pro_fab_subprocess f  where a.id=b.mst_id and  f.batch_id=a.id and  f.batch_id=b.mst_id and f.entry_form=34 and  a.batch_against in(3)  $company_cond $working_company_cond   $dates_com $buyerdata_non_ord  $batch_num $booking_num $shift_cond $color_name and a.entry_form=0 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and b.po_id=0  GROUP BY b.po_id, b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type,f.process_end_date,f.process_start_date,f.production_date,f.start_minutes,f.start_hours, f.end_hours, f.end_minutes,f.shift_name, f.machine_id,f.floor_id,f.remarks $finish_group
            ) $order_by2";
		}
		else if($cbo_type==9)//  Stentering 
		{
            $sql="(select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.po_id,b.prod_id,b.width_dia_type,$group_conct,c.job_no_mst,d.job_no_prefix_num,d.buyer_name,f.process_end_date,f.process_start_date,c.file_no,c.grouping,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.shift_name,f.machine_id,f.floor_id,f.remarks $stenter from pro_batch_create_dtls b,wo_po_break_down c,wo_po_details_master d,pro_batch_create_mst a,pro_fab_subprocess f where  f.batch_id=a.id and a.id=b.mst_id and f.entry_form=48 and  a.batch_against in(1,2,3) and b.po_id=c.id and d.job_no=c.job_no_mst and  f.batch_id=b.mst_id $company_cond $working_company_cond  $dates_com $jobdata $batch_num $booking_num $buyerdata $order_no $year_cond $shift_cond $file_cond $ref_cond $color_name and a.entry_form=0 and f.re_stenter_no=0 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0 GROUP BY b.po_id, b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type,c.job_no_mst, d.job_no_prefix_num, d.buyer_name, f.process_end_date,f.process_start_date,c.file_no,c.grouping,f.production_date,f.start_minutes,f.start_hours, f.end_hours, f.end_minutes, f.shift_name,f.machine_id,f.floor_id,f.remarks $stenter_group)
            union
            (
            select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.po_id,b.prod_id,b.width_dia_type,null as po_number,null as po_id,null as job_no_mst,null as job_no_prefix_num,null as buyer_name,f.process_end_date,f.process_start_date,null as file_no,null as grouping,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.shift_name,f.machine_id,f.floor_id,f.remarks $stenter from pro_batch_create_dtls b,pro_batch_create_mst a,pro_fab_subprocess f  where   f.batch_id=a.id and f.batch_id=b.mst_id and a.id=b.mst_id and f.entry_form=48 $company_cond $working_company_cond  $dates_com $buyerdata_non_ord  $batch_num $booking_num  $shift_cond $color_name and a.entry_form=0 and f.re_stenter_no=0  and  a.batch_against in(3)  and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and b.po_id=0    GROUP BY b.po_id, b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type, f.process_end_date,f.process_start_date,f.production_date,f.start_minutes,f.start_hours, f.end_hours, f.end_minutes, f.shift_name,f.machine_id,f.floor_id,f.remarks $stenter_group ) $order_by2 
            ";
		}
		else if($cbo_type==11)//  Re Stentering 
		{
            $sql="(select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.po_id,b.prod_id,b.width_dia_type,$group_conct,c.job_no_mst,d.job_no_prefix_num,d.buyer_name,f.process_end_date,f.process_start_date,c.file_no,c.grouping,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.shift_name,f.machine_id,f.floor_id,f.remarks $stenter,f.re_stenter_no from pro_batch_create_dtls b,wo_po_break_down c,wo_po_details_master d,pro_batch_create_mst a,pro_fab_subprocess f  where   a.id=b.mst_id and f.entry_form=48 and  a.batch_against in(1,2,3) and b.po_id=c.id and d.job_no=c.job_no_mst and  f.batch_id=a.id $company_cond $working_company_cond   $dates_com $jobdata $batch_num $booking_num $buyerdata $order_no $year_cond $shift_cond $file_cond $ref_cond $color_name and a.entry_form=0   and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and f.re_stenter_no!=0 GROUP BY b.po_id, b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type,c.job_no_mst, d.job_no_prefix_num, d.buyer_name, f.process_end_date,f.process_start_date,c.file_no,c.grouping,f.production_date,f.start_minutes,f.start_hours, f.end_hours, f.end_minutes, f.shift_name,f.machine_id,f.floor_id,f.remarks,f.re_stenter_no $stenter_group)
            union
            (
            select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.po_id,b.prod_id,b.width_dia_type,null as po_number,null as po_id,null as job_no_mst,null as job_no_prefix_num,j.buyer_id as buyer_name,f.process_end_date,f.process_start_date,null as file_no,null as grouping,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.shift_name,f.machine_id,f.floor_id,f.remarks $stenter,f.re_stenter_no from pro_batch_create_dtls b,pro_batch_create_mst a,pro_fab_subprocess f where f.batch_id=a.id  and a.entry_form=0 and a.id=b.mst_id and f.entry_form=48 and  a.batch_against in(3) $company_cond $working_company_cond    $dates_com  $buyerdata_non_ord  $batch_num $booking_num $shift_cond $color_name   and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and b.po_id=0  and f.re_stenter_no!=0   GROUP BY b.po_id, b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type, f.process_end_date,f.process_start_date,f.production_date,f.start_minutes,f.start_hours, f.end_hours, f.end_minutes, f.shift_name,f.machine_id,f.floor_id,f.remarks,f.re_stenter_no $stenter_group ) $order_by2 
            ";
			//echo $sql;
		}
		...
		{
            $sql="(
			select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.prod_id,b.width_dia_type,$group_conct,c.job_no_mst,d.job_no_prefix_num,d.buyer_name,f.process_end_date,c.file_no,c.grouping,f.process_start_date,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.shift_name,f.machine_id,f.floor_id,f.remarks $compact,f.re_stenter_no  from pro_batch_create_dtls b,wo_po_break_down c,wo_po_details_master d,pro_batch_create_mst a,pro_fab_subprocess f,pro_fab_subprocess_dtls h where  a.id=b.mst_id and b.po_id=c.id and d.job_no=c.job_no_mst and f.batch_id=a.id and  f.batch_id=b.mst_id and f.id=h.mst_id and h.prod_id=b.prod_id and  f.entry_form=h.entry_page $company_cond $working_company_cond  and f.entry_form=33 and  a.entry_form=0  and f.re_stenter_no!=0 and  a.batch_against in(1,2,3)  $dates_com $jobdata $batch_num $booking_num $buyerdata $order_no $year_cond $shift_cond $file_cond $ref_cond $color_name and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 GROUP BY b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type,c.job_no_mst, d.job_no_prefix_num, d.buyer_name, f.process_end_date,f.process_start_date,f.production_date,f.start_minutes,f.start_hours, f.end_hours, f.end_minutes,f.shift_name, f.machine_id,f.floor_id,c.file_no,c.grouping,f.remarks,f.re_stenter_no $compact_group )
			union
			(
			select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.prod_id,b.width_dia_type,null as po_number,null as po_id,null as job_no_mst,null as job_no_prefix_num,null as buyer_name,f.process_end_date,null as file_no,null as grouping,f.process_start_date,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.shift_name,f.machine_id,f.floor_id,f.remarks $compact,f.re_stenter_no  from pro_batch_create_dtls b,pro_batch_create_mst a,pro_fab_subprocess f,pro_fab_subprocess_dtls h where a.id=b.mst_id and f.entry_form=33 and f.batch_id=b.mst_id and  f.batch_id=a.id and f.id=h.mst_id and h.prod_id=b.prod_id and  f.entry_form=h.entry_page $company_cond $working_company_cond   $dates_com $batch_num $booking_num $shift_cond $buyerdata_non_ord  $color_name and a.entry_form=0  and f.re_stenter_no!=0 and  a.batch_against in(3)  and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and b.po_id=0 GROUP BY b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type, f.process_end_date,f.process_start_date,f.production_date,f.start_minutes,f.start_hours, f.end_hours, f.end_minutes,f.shift_name, f.machine_id,f.floor_id,f.remarks,f.re_stenter_no $compact_group
			) $order_by2";
			//echo $sql;
		}	
		...
		{
            $sql_heat="(
			select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.po_id,b.prod_id,b.width_dia_type,$group_conct,c.job_no_mst,d.job_no_prefix_num,d.buyer_name,c.file_no,c.grouping,f.entry_form,f.process_end_date,f.process_start_date,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.shift_name,f.machine_id,f.floor_id,f.remarks $heat_set from pro_batch_create_dtls b,wo_po_break_down c,wo_po_details_master d,pro_batch_create_mst a,pro_fab_subprocess f where  f.batch_id=a.id and b.po_id=c.id and d.job_no=c.job_no_mst and a.id=b.mst_id and f.entry_form in(32) and  a.batch_against in(1,2,3)  $company_cond $working_company_cond $dates_com $jobdata $batch_num $booking_num $buyerdata $order_no $year_cond $shift_cond $file_cond $ref_cond $color_name and a.entry_form=0 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 GROUP BY b.po_id, b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type,c.job_no_mst, d.job_no_prefix_num, d.buyer_name,f.entry_form, f.process_end_date, f.process_start_date,f.production_date,f.start_minutes,f.start_hours,f.end_hours, f.end_minutes,c.file_no,c.grouping,f.shift_name,f.machine_id,f.floor_id,f.remarks $heat_group)
			union
			(
			select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.po_id,b.prod_id,b.width_dia_type,null as po_number,null as po_id,null as job_no_mst,null as job_no_prefix_num,null as buyer_name,null as file_no,null as grouping,f.entry_form,f.process_end_date,f.process_start_date,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.shift_name,f.machine_id,f.floor_id,f.remarks $heat_set from pro_batch_create_dtls b,pro_batch_create_mst a,pro_fab_subprocess f where a.id=b.mst_id and  f.batch_id=a.id and f.entry_form in(32) and a.batch_against in(1,2,3) $company_cond  $working_company_cond  $dates_com  $batch_num $booking_num $buyerdata_non_ord  $shift_cond $color_name and a.entry_form=0 and b.po_id=0  and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0   GROUP BY b.po_id, b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type,f.entry_form, f.process_end_date, f.process_start_date,f.production_date,f.start_minutes,f.start_hours,f.end_hours, f.end_minutes,f.shift_name,f.machine_id,f.floor_id,f.remarks $heat_group
			)  $order_by2 ";
            ...
            union
            (
            	select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.prod_id,b.width_dia_type,null as po_number,null as po_id,null as job_no_mst,null as job_no_prefix_num,null as buyer_name,null as file_no,null as grouping,f.shift_name,f.entry_form,f.process_end_date,f.process_start_date,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.machine_id,f.floor_id,f.remarks 
            $sliting_sq  from pro_batch_create_dtls b,pro_batch_create_mst a,pro_fab_subprocess f where a.id=b.mst_id and f.batch_id=a.id and f.entry_form in(30) and  a.batch_against in(3) $company_cond  $working_company_cond $dates_com $batch_num $booking_num  $shift_cond $buyerdata_non_ord  $color_name and a.entry_form=0 and b.po_id=0   and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0   GROUP BY b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type,f.shift_name,f.entry_form, f.process_end_date,f.process_start_date,f.production_date,f.start_minutes,f.start_hours, f.end_hours, f.end_minutes, f.machine_id,f.floor_id,f.remarks $sliting_group
            ) $order_by2 ";

            $sql_drying=" (
            select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.prod_id,b.width_dia_type,$group_conct,c.job_no_mst,d.job_no_prefix_num,d.buyer_name,c.file_no,c.grouping,f.shift_name,f.entry_form,f.process_end_date,f.process_start_date,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.machine_id,f.floor_id,f.remarks $drying  from pro_batch_create_dtls b,wo_po_break_down c,wo_po_details_master d,pro_batch_create_mst a,pro_fab_subprocess f where  f.batch_id=a.id  and a.id=b.mst_id and f.entry_form in(31) and  a.batch_against in(1,2,3) and b.po_id=c.id and d.job_no=c.job_no_mst and a.entry_form=0  $company_cond  $working_company_cond $dates_com $jobdata $batch_num $booking_num $buyerdata $order_no $year_cond $shift_cond $file_cond $ref_cond $color_name    and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 GROUP BY  b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type,c.job_no_mst, d.job_no_prefix_num, d.buyer_name,f.entry_form, f.process_end_date ,f.process_start_date,f.production_date,f.start_minutes,f.start_hours,f.end_hours, f.end_minutes,c.file_no,c.grouping,f.shift_name,f.floor_id, f.machine_id,f.remarks $drying_group)
            union
            (
            select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.prod_id,b.width_dia_type,null as po_number,null as po_id,null as job_no_mst,null as job_no_prefix_num,null as buyer_name,null as file_no,null as grouping,f.shift_name,f.entry_form,f.process_end_date,f.process_start_date,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.machine_id,f.floor_id,f.remarks $drying  from pro_batch_create_dtls b,pro_batch_create_mst a,pro_fab_subprocess f where  a.id=b.mst_id and f.batch_id=a.id  $company_cond  $working_company_cond   $dates_com  $buyerdata_non_ord  $batch_num $booking_num  $shift_cond $color_name and a.entry_form=0 and b.po_id=0 and   f.entry_form in(31) and  a.batch_against in(3) and  b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 GROUP BY b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type,f.entry_form, f.process_end_date ,f.process_start_date,f.production_date,f.start_minutes,f.start_hours,f.end_hours, f.end_minutes,f.shift_name, f.machine_id,f.floor_id,f.remarks $drying_group
            ) $order_by2 ";

            $sql_stentering="(select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.po_id,b.prod_id,b.width_dia_type,$group_conct,c.job_no_mst,d.job_no_prefix_num,d.buyer_name,c.file_no,c.grouping,f.shift_name,f.process_end_date,f.process_start_date,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.machine_id,f.floor_id,f.remarks $stenter  from pro_batch_create_dtls b,wo_po_break_down c,wo_po_details_master d,pro_batch_create_mst a,pro_fab_subprocess f where f.batch_id=a.id and a.id=b.mst_id and f.entry_form=48 and f.re_stenter_no=0 and  a.batch_against in(1,2,3) and b.po_id=c.id and d.job_no=c.job_no_mst  $company_cond  $working_company_cond $dates_com $jobdata $batch_num $booking_num $buyerdata $order_no $year_cond $shift_cond $file_cond $ref_cond $color_name and a.entry_form=0 and   b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0 GROUP BY b.po_id, b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type,c.job_no_mst, d.job_no_prefix_num, d.buyer_name,c.file_no,c.grouping,f.shift_name, f.process_end_date,f.process_start_date,f.production_date,f.start_minutes,f.start_hours, f.end_hours, f.end_minutes, f.machine_id,f.floor_id,f.remarks $stenter_group)
            union
            (
            select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.po_id,b.prod_id,b.width_dia_type,null as po_number,null as po_id,null as job_no_mst,null as job_no_prefix_num,null as buyer_name,null as file_no,null as grouping,f.shift_name,f.process_end_date,f.process_start_date,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.machine_id,f.floor_id,f.remarks $stenter  from pro_batch_create_dtls b,pro_batch_create_mst a,pro_fab_subprocess f where  f.batch_id=a.id  and a.id=b.mst_id and f.entry_form=48 and f.re_stenter_no=0 and  a.batch_against in(3) and b.po_id=0  $company_cond  $working_company_cond $dates_com  $batch_num $booking_num  $buyerdata_non_ord  $shift_cond $color_name and a.entry_form=0  and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0  GROUP BY b.po_id, b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type,f.shift_name, f.process_end_date,f.process_start_date,f.production_date,f.start_minutes,f.start_hours, f.end_hours, f.end_minutes, f.machine_id,f.floor_id,f.remarks $stenter_group
            ) $order_by2 ";

            $sql_compacting="(select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.po_id,b.prod_id,b.width_dia_type,$group_conct,c.job_no_mst,d.job_no_prefix_num,d.buyer_name,c.file_no,c.grouping,f.shift_name,f.entry_form,f.process_end_date,f.process_start_date,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.machine_id,f.floor_id,f.remarks $compact from pro_batch_create_dtls b,wo_po_break_down c,wo_po_details_master d,pro_batch_create_mst a,pro_fab_subprocess f where  b.po_id=c.id and d.job_no=c.job_no_mst and f.batch_id=a.id  and a.id=b.mst_id and f.entry_form in(33) and f.re_stenter_no=0 and  a.batch_against in(1,2,3) $company_cond  $working_company_cond $dates_com $jobdata $batch_num $booking_num $buyerdata $order_no $year_cond $color_name $shift_cond $file_cond $ref_cond and a.entry_form=0   and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 GROUP BY b.po_id, b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type,c.job_no_mst, d.job_no_prefix_num, d.buyer_name,f.entry_form, f.process_end_date,f.process_start_date,f.production_date,f.start_minutes,f.start_hours, f.end_hours,c.file_no,c.grouping,f.shift_name, f.end_minutes, f.floor_id,f.machine_id,f.remarks $compact_group)
            union
            (
            select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.po_id,b.prod_id,b.width_dia_type,null as po_number,null as po_id,null as job_no_mst,null as job_no_prefix_num,null as buyer_name,null as file_no,null as grouping,f.shift_name,f.entry_form,f.process_end_date,f.process_start_date,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.machine_id,f.floor_id,f.remarks $compact from pro_batch_create_dtls b,pro_batch_create_mst a,pro_fab_subprocess f where a.id=b.mst_id and  f.batch_id=a.id and f.entry_form in(33) and f.re_stenter_no=0 and  a.batch_against in(3) and b.po_id=0 $company_cond   $working_company_cond $dates_com $buyerdata_non_ord  $batch_num $booking_num  $color_name $shift_cond and a.entry_form=0  and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0   GROUP BY b.po_id, b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type,f.entry_form, f.process_end_date,f.process_start_date,f.production_date,f.start_minutes,f.start_hours, f.end_hours,f.shift_name, f.end_minutes,f.floor_id, f.machine_id,f.remarks $compact_group
            )  $order_by2 ";
            
            $sql_special="(
            select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.po_id,b.prod_id,b.width_dia_type,$group_conct,c.job_no_mst,d.job_no_prefix_num,d.buyer_name,c.file_no,c.grouping,f.shift_name,f.entry_form,f.process_end_date,f.process_start_date,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.machine_id,f.floor_id,f.remarks $sp_finish  from pro_batch_create_dtls b,wo_po_break_down c,wo_po_details_master d,pro_batch_create_mst a,pro_fab_subprocess f where a.id=b.mst_id and f.batch_id=a.id  and b.po_id=c.id and d.job_no=c.job_no_mst $company_cond   $working_company_cond  $dates_com $jobdata $batch_num $booking_num $buyerdata $order_no $year_cond $color_name $shift_cond $file_cond $ref_cond and a.entry_form=0 and f.entry_form in(34) and  a.batch_against in(1,2,3)  and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 GROUP BY b.po_id, b.item_description,a.company_id, a.id, a.batch_no,a.booking_no, a.batch_date,a.color_id, a.extention_no, a.batch_against,b.prod_id, b.width_dia_type,c.job_no_mst, d.job_no_prefix_num, d.buyer_name,f.entry_form, f.process_end_date,f.process_start_date,f.production_date,f.start_minutes,f.start_hours, f.end_hours,c.file_no,c.grouping,f.shift_name, f.end_minutes, f.machine_id,f.floor_id,f.remarks $finish_group)			
            union
            (
            select a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.po_id,b.prod_id,b.width_dia_type,null as po_number,null as po_id,null as job_no_mst,null as job_no_prefix_num,null as buyer_name,null as file_no,null as grouping,f.shift_name,f.entry_form,f.process_end_date,f.process_start_date,f.production_date as end_date,f.start_minutes,f.start_hours,f.end_hours,f.end_minutes,f.machine_id,f.floor_id,f.remarks $sp_finish  from pro_batch_create_dtls b,pro_batch_create_mst a,pro_fab_subprocess f where a.id=b.mst_id and  f.batch_id=a.id and f.entry_form in(34) and  a.batch_against in(1,2,3) and b.po_id=0 $company_cond  $working_company_cond  $dates_com  $batch_num $booking_num $buyerdata_non_ord  $color_name $shift_cond and a.entry_form=0 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0   GROUP BY b.po_id, b.item_description,a.company_id,a.id,a.batch_no,a.booking_no,a.batch_date,a.color_id,a.extention_no,a.batch_against,b.prod_id, b.width_dia_type,f.entry_form, f.process_end_date,f.process_start_date,f.production_date,f.start_minutes,f.start_hours, f.end_hours,f.shift_name, f.end_minutes, f.machine_id,f.floor_id,f.remarks $finish_group
            ) $order_by2 ";
		}
...
?>
 ...
 <table class="rpt_table" width="1920" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
<thead>
<tr>
    <th width="30">SL</th>
...
if($group_by!=0)
{
   ...
   // $heat_prod_qty=$batch_prod_qty_arr[$batch[csf('id')]][$batch[csf('prod_id')]][$batch[csf('width_dia_type')]]['heat'];
   			if ($roll_maintained==1 && $cbo_type!=0)
			 {
				 $heat_prod_qty=$batch_prod_qty_arr[$batch[csf('id')]][$batch[csf('prod_id')]][$batch[csf('width_dia_type')]][$batch[csf('roll_no')]][$batch[csf('roll_no_heat')]]['heat'];
			 }
			 else 
			 {
			 	 $heat_prod_qty=$batch_prod_qty_arr[$batch[csf('id')]][$batch[csf('prod_id')]][$batch[csf('width_dia_type')]]['heat'];
...
?>
 ...
 <table class="rpt_table" width="1900" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
<tfoot>
<tr>
    <th width="30">&nbsp;</th>
     <? if($group_by==2 || $group_by==0){ ?>
...
?>
 ...
 <table class="rpt_table" width="1835" cellpadding="0" cellspacing="0" border="1" rules="all" style="float: left;">
<thead>
<tr>
    <th width="30">SL</th>
     <? if($group_by==2 || $group_by==0){ ?>
...
<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
   ...
   <td width="80"><p><? //echo $floor_arr[$batch[csf('floor_id')]]; ?></p></td>
    ...
    <td align="right" width="70" >
        ...
        echo number_format($batch_prod_qty_arr[$batch[csf('id')]][$batch[csf('prod_id')]][$batch[csf('width_dia_type')]][$batch[csf('roll_no')]][$batch[csf('barcode_no')]]['sliting'],2);  
        }else
        {
           // echo $page_upto.'g';
...
//if ($roll_maintained==1 && $cbo_type!=0) {
if(($page_upto==7 || $page_upto>7) && $roll_maintained==1){
    $tot_prod_btq+=$batch_prod_qty_arr[$batch[csf('id')]][$batch[csf('prod_id')]][$batch[csf('width_dia_type')]][$batch[csf('roll_no')]][$batch[csf('barcode_no')]]['sliting'];
}
else
{
...
$grand_btq+=$batch[csf('batch_qnty')];
  //  if ($roll_maintained==1 && $cbo_type!=0) {
	  if(($page_upto==7 || $page_upto>7) && $roll_maintained==1){
        $grand_tot_prod_btq+=$batch_prod_qty_arr[$batch[csf('id')]][$batch[csf('prod_id')]][$batch[csf('width_dia_type')]][$batch[csf('roll_no')]][$batch[csf('barcode_no')]]['sliting'];
    }
    else
    {
...
<table class="rpt_table" width="1815" cellpadding="0" cellspacing="0" border="1" rules="all" style="float: left;">
<tfoot>
<tr>
    <th width="30">&nbsp;</th>
     <? if($group_by==2 || $group_by==0){ ?>
...
?>
 ...
 <table class="rpt_table" width="1835" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
<thead>
<tr>
    <th width="30">SL</th>
<? if($group_by==2 || $group_by==0){ ?>
   ...
   <td width="80"><p><? //echo $floor_arr[$batch[csf('floor_id')]]; ?></p></td>
    ...
    <td align="right" width="70" title="<? echo $batch[csf('batch_qnty')];  ?>"><? echo number_format($batch[csf('batch_qnty')],2);  ?></td>
       <td align="right" width="70" >
        ...
        //if ($roll_maintained==1 && $cbo_type!=0) {
		if(($page_upto==5 || $page_upto>5) && $roll_maintained==1){
            echo number_format($batch_prod_qty_arr[$batch[csf('id')]][$batch[csf('prod_id')]][$batch[csf('width_dia_type')]][$batch[csf('roll_no')]][$batch[csf('barcode_no')]]['drying'],2);
         }
         else
         {
...
//if ($roll_maintained==1 && $cbo_type!=0) {
	if(($page_upto==5 || $page_upto>5) && $roll_maintained==1){
    $tot_prod_btq+=$batch_prod_qty_arr[$batch[csf('id')]][$batch[csf('prod_id')]][$batch[csf('width_dia_type')]][$batch[csf('roll_no')]][$batch[csf('barcode_no')]]['drying'];
}
else
{
...
$grand_btq+=$batch[csf('batch_qnty')];
   // if ($roll_maintained==1 && $cbo_type!=0) {
	   if(($page_upto==5 || $page_upto>5) && $roll_maintained==1){
        $grand_tot_prod_btq+=$batch_prod_qty_arr[$batch[csf('id')]][$batch[csf('prod_id')]][$batch[csf('width_dia_type')]][$batch[csf('roll_no')]][$batch[csf('barcode_no')]]['drying'];
    }
    else
    {
...
<table class="rpt_table" width="1815" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
<tfoot>
<tr>
    <th width="30">&nbsp;</th>
   <? if($group_by==2 || $group_by==0){ ?>
...
?>
 ...
 <table class="rpt_table" width="1860" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
<thead>
<tr>
    <th width="30">SL</th>
     <? if($group_by==2 || $group_by==0){ ?>
   ...
   <td width="80"><p><? //echo $floor_arr[$batch[csf('floor_id')]]; ?></p></td>
    ...
    <td align="right" width="70" title="<? echo $batch[csf('batch_qnty')];  ?>"><p><? echo number_format($batch[csf('batch_qnty')],2);  ?></p></td>
     <td align="right" width="70" >
       ...
       // if ($roll_maintained==1 && $cbo_type!=0) {
		   if(($page_upto==4 || $page_upto>4) && $roll_maintained==1){
            echo number_format($batch_prod_qty_arr[$batch[csf('id')]][$batch[csf('prod_id')]][$batch[csf('width_dia_type')]][$batch[csf('roll_no')]][$batch[csf('barcode_no')]]['stenter'],2); 
        }
        else
        {
...
//if ($roll_maintained==1 && $cbo_type!=0) {
	 if(($page_upto==4 || $page_upto>4) && $roll_maintained==1){
    $tot_prod_btq+=$batch_prod_qty_arr[$batch[csf('id')]][$batch[csf('prod_id')]][$batch[csf('width_dia_type')]][$batch[csf('roll_no')]][$batch[csf('barcode_no')]]['stenter'];
}
else
{
...
$grand_btq+=$batch[csf('batch_qnty')];
    //if ($roll_maintained==1 && $cbo_type!=0) {
		 if(($page_upto==4 || $page_upto>4) && $roll_maintained==1){
        $grand_tot_prod_btq+=$batch_prod_qty_arr[$batch[csf('id')]][$batch[csf('prod_id')]][$batch[csf('width_dia_type')]][$batch[csf('roll_no')]][$batch[csf('barcode_no')]]['stenter'];
    }
    else
    {
...
?>
 ...
 <table class="rpt_table" width="1855" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
<thead>
<tr>
    <th width="30">SL</th>
...
$com_group_arr=$batch[csf('prod_id')].$batch[csf('id')].$batch[csf('machine_id')].$batch[csf('floor_id')].$batch[csf('shift_name')].$batch[csf('width_dia_type')].$batch[csf('end_date')];
  ...
  { 
	 ...
	 $prod_compact_qty=$batch_prod_qty_arr[$batch[csf('id')]][$batch[csf('prod_id')]][$batch[csf('width_dia_type')]][$batch[csf('roll_id')]][$batch[csf('barcode_no')]]['compact'];
    // echo number_format($prod_compact_qty,2).'A'; 
...
<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
   ...
   // if ($roll_maintained==1 && $cbo_type!=0) {
	    if(($page_upto==6 || $page_upto>6) && $roll_maintained==1){
        echo number_format($batch_prod_qty_arr[$batch[csf('id')]][$batch[csf('prod_id')]][$batch[csf('width_dia_type')]][$batch[csf('roll_no')]][$batch[csf('barcode_no')]]['special'],2); 
    }
    else{
...
$btq+=$batch[csf('batch_qnty')];
   if(($page_upto==6 || $page_upto>6) && $roll_maintained==1){
        $tot_prod_btq+=$batch_prod_qty_arr[$batch[csf('id')]][$batch[csf('prod_id')]][$batch[csf('width_dia_type')]][$batch[csf('roll_no')]][$batch[csf('barcode_no')]]['special'];
    }
    else
    {
...
?>
 ...
 <table class="rpt_table" width="1670" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
<thead>
<tr>
    <th width="30">SL</th>
...
?>
 ...
 <table class="rpt_table" width="1750" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
<thead>
<tr>
    <th width="30">SL</th>
    <? if($group_by==2 || $group_by==0){ ?>
...
?>
 ...
 <table class="rpt_table" width="1520" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
<thead>
<tr>
    <th width="30">SL</th>
       <? if($group_by==2 || $group_by==0){ ?>
...
//}
 ...
 <table class="rpt_table" width="1480" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
<tfoot>
<tr>
    <th width="30">&nbsp;</th>
    <? if($group_by==2 || $group_by==0){ ?>
...
<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
    ...
    <td align="right" width="70"><? echo number_format($heat_row[csf('batch_qnty')],2);  ?></td>
     <td align="right" width="70" ><? 
			...
			if(($page_upto==1 || $page_upto>1) && $roll_maintained==1)
			 {
				 echo number_format($batch_prod_qty_arr[$heat_row[csf('id')]][$heat_row[csf('prod_id')]][$heat_row[csf('width_dia_type')]][$heat_row[csf('roll_no')]]['heat'],2);
			 }
			 else
			 {
			 	 echo number_format($batch_prod_qty_arr[$heat_row[csf('id')]][$heat_row[csf('prod_id')]][$heat_row[csf('width_dia_type')]]['heat'],2);
...
$i++;
	if(($page_upto==1 || $page_upto>1) && $roll_maintained==1)
	 {
		 $tot_prod_qty_heat+=$batch_prod_qty_arr[$heat_row[csf('id')]][$heat_row[csf('prod_id')]][$heat_row[csf('width_dia_type')]][$heat_row[csf('roll_no')]]['heat'];
	 }
	 else
	 {
