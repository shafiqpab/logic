...
if($action=="report_generate") // Show Button
{
	...
	if ($order_type==0 || $order_type==1) // Self Order
	{
		...
		<table align="center" cellspacing="0" width="1920"  border="1" rules="all" class="rpt_table" >
			...
			and a.id=b.mst_id and a.machine_id=c.id  and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  
			...
			foreach(sql_select($sqls) as $vals)
			{
			 ...
			 $sql_result="SELECT a.id, a.batch_no, a.batch_id, a.batch_ext_no, a.machine_id,a.entry_form, a.load_unload_id, a.end_hours, a.end_minutes, a.result, a.remarks,a.production_date, 
			 ...
			 and a.id=b.mst_id and a.machine_id=c.id   and q.entry_form=0 and a.load_unload_id!=1 
			...
			foreach ($sql_dtls as $row)
			{
			  ...
			  $sql_result_sales="SELECT q.sales_order_no,a.id, a.batch_no, a.batch_id, a.batch_ext_no, a.machine_id,a.entry_form, a.load_unload_id, a.end_hours, a.end_minutes, a.result, a.remarks,a.production_date, 
	...
	if ($order_type==0 || $order_type==2) // Subcontract Order
	{
		...
		<table align="center" cellspacing="0" width="1920"  border="1" rules="all" class="rpt_table" >
			...
			<? 
			$sqls=" SELECT a.batch_id, a.batch_ext_no, a.machine_id,a.end_hours, a.end_minutes, a.process_end_date, a.remarks,a.start_hours,a.start_minutes
			from pro_fab_subprocess a, lib_machine_name c 
			where a.service_company=$cbo_company_id $location_id $batch_cond $floor_id_cond $machine_cond 
			and a.machine_id=c.id  and a.status_active=1 and a.is_deleted=0  and a.load_unload_id!=1 and c.status_active=1 and c.is_deleted=0  
 			group by a.batch_id, a.batch_ext_no, a.machine_id,a.end_hours, a.end_minutes, a.process_end_date, a.remarks,a.start_hours,a.start_minutes";
		  ...
		  foreach(sql_select($sqls) as $vals)
		  {
			...
			if ($order_no=="") $order_no_cond =""; else $order_no_cond =" and p.order_no=$txt_order_no ";
		  	...
		  	// Main query	
			$sql_result="SELECT a.id, a.batch_no, a.batch_id, a.batch_ext_no, a.machine_id, a.entry_form,a.load_unload_id, a.end_hours, a.end_minutes, a.result, a.remarks,a.production_date, a.water_flow_meter, b.batch_qnty, b.item_description, c.machine_no, c.prod_capacity, f.party_id, p.order_no, q.color_id, q.color_range_id, q.total_trims_weight, q.batch_weight,q.process_id ,a.process_id as process
			...
			where a.service_company=$cbo_company_id $order_no_cond $location_id $batch_cond $floor_id_cond $machine_cond $sql_cond $party_name_cond $batch_color_cond $color_range_cond and a.machine_id=c.id   and q.entry_form=36 and a.load_unload_id!=1 and a.batch_id=q.id and b.po_id=p.id  and p.job_no_mst= f.subcon_job and q.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 order by a.production_date, c.machine_no";
			...
			foreach ($sql_dtls as $row)
			{
				...
				$date_data_arr[$row[csf('production_date')]][$row[csf('machine_id')]][$row[csf('batch_id')]]['batch_no']=$row[csf('batch_no')];			
				$date_data_arr[$row[csf('production_date')]][$row[csf('machine_id')]][$row[csf('batch_id')]]['batch_id']=$row[csf('batch_id')];		
				$date_data_arr[$row[csf('production_date')]][$row[csf('machine_id')]][$row[csf('batch_id')]]['prod_qty']+=$row[csf('batch_qnty')];
				$date_data_arr[$row[csf('production_date')]][$row[csf('machine_id')]][$row[csf('batch_id')]]['batch_weight']=$row[csf('batch_weight')];	
					...
					foreach ($machine_data as $batch_id => $row) 
					{
						...
						<tr id="<? echo 'v_'.$i; ?>" onClick="change_color('v_<? echo $i; ?>','#FFFFFF')" bgcolor="<? echo $bgcolor; ?>">
							...
							<?
								...
								$load_hour = $load_time_array[$row['machine_id']][$row['batch_id']][$row['batch_ext_no']]["start_hours"];
								$load_minut = $load_time_array[$row['machine_id']][$row['batch_id']][$row['batch_ext_no']]["start_minutes"];
	...
	if ($order_type==0 || $order_type==3) // Sample Without Order
	{
		...
		<table align="center" cellspacing="0" width="1920"  border="1" rules="all" class="rpt_table" >
			...
			from pro_fab_subprocess a, pro_fab_subprocess_dtls b, lib_machine_name c 
			where a.service_company=$cbo_company_id $location_id $batch_cond $floor_id_cond $machine_cond  
			and a.id=b.mst_id and a.machine_id=c.id  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.load_unload_id!=1 
			group by  a.batch_id, a.batch_ext_no, a.machine_id,a.end_hours, a.end_minutes, a.process_end_date, a.remarks,a.start_hours,a.start_minutes ";
			//and a.entry_form=35
			$load_time_array=array();
			foreach(sql_select($sqls) as $vals)
			...
			foreach ($sql_dtls as $row)
			{
			 ...
			 from fabric_sales_order_mst p, pro_batch_create_mst q, pro_fab_subprocess a, pro_fab_subprocess_dtls b, lib_machine_name c 
			 where a.id=b.mst_id and a.machine_id=c.id  and q.entry_form=0 and a.load_unload_id!=1 
