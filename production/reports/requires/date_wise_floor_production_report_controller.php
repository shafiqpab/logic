...
{ 	 
	...
	group by d.serving_company,a.JOB_NO,c.po_break_down_id,d.floor_id,d.production_date order by d.production_date";
    //d.floor_id,echo $sql;die();//and a.JOB_NO='TIL-20-00004' 
...
$sql_pre = "select a.costing_per,b.job_no,b.cm_cost,b.freight,b.total_cost,b.comm_cost,b.commission,b.margin_pcs_set,b.incometax_cost,b.interest_cost from wo_pre_cost_dtls b, wo_pre_cost_mst a
		...
		$sqlOrder="select a.TOTAL_SET_QNTY,b.JOB_NO_MST,sum(b.ORDER_TOTAL) as FOV_VAL,sum(b.ORDER_QUANTITY) as ORDER_QUANTITY from WO_PO_DETAILS_MASTER a,wo_po_color_size_breakdown b where a.JOB_NO=b.JOB_NO_MST $job_con2 and b.IS_DELETED=0 and b.STATUS_ACTIVE = 1 group by a.TOTAL_SET_QNTY,b.JOB_NO_MST";
...
$sql_pre = "select a.COSTING_DATE,a.costing_per,b.job_no,b.cm_cost,b.freight,b.total_cost,b.comm_cost,b.commission,b.margin_pcs_set,b.incometax_cost,b.interest_cost from wo_pre_cost_dtls b, wo_pre_cost_mst a
		...
		//........................................
		$sqlOrder="select a.TOTAL_SET_QNTY,b.JOB_NO_MST,a.SET_SMV,a.GMTS_ITEM_ID,sum(b.ORDER_TOTAL) as FOV_VAL,sum(b.ORDER_QUANTITY) as ORDER_QUANTITY from WO_PO_DETAILS_MASTER a,wo_po_color_size_breakdown b where a.JOB_NO=b.JOB_NO_MST $job_con2 and b.IS_DELETED=0 and b.STATUS_ACTIVE = 1 group by a.TOTAL_SET_QNTY,a.SET_SMV,a.GMTS_ITEM_ID,b.JOB_NO_MST";
		$sqlOrder_result=sql_select($sqlOrder);
...
//echo array_sum($jobDataArr[FOB_VALUE][$rows[JOB_NO]]).'-'.$totalCost.')/'.array_sum($jobDataArr[PO_QUANTITY][$rows[JOB_NO]]);
	...
	//echo array_sum($jobDataArr[FOB_VALUE][$rows[JOB_NO]]).'-'.$totalCost.')/'.array_sum($jobDataArr[PO_QUANTITY][$rows[JOB_NO]]);
		...
		<tr bgcolor="<? echo $bgcolor; ?>">
            ...
            <td align="center"><?= $itemSMVData[$rows[ITEM_NUMBER_ID]];?></td>
			<td align="center"><?= $cm_cost_pcs*12 ;?></td>
			<td align="center" title="<?= $jobData[$rows[JOB_NO]][FOV_VAL].'-'.$totalCost.'/'.$jobData[$rows[JOB_NO]][ORDER_QUANTITY];?>"><?= number_format($cm_mergin_pcs*12,2);?></td>
