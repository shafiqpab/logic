...
{
	echo create_drop_down( "cbo_location", 110, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id='$data' order by location_name","id,location_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/floor_wise_sewing_wip_report_controller', this.value, 'load_drop_down_floor', 'floor_td' );get_php_form_data( this.value, 'eval_multi_select', 'requires/floor_wise_sewing_wip_report_controller' );",0 );     	 
}
...
{ 
	...
	// load_drop_down( 'requires/line_wise_hourly_production_report_controller', this.value+'_'+document.getElementById('cbo_location').value+'_'+document.getElementById('cbo_company_name').value+'_'+document.getElementById('txt_date').value, 'load_drop_down_line', 'line_td' );
...
if($action=="report_generate")
{
		...
		<style type="text/css">
			...
			.gd-color3
			{
                ...
                <div style="width:1100px;"> 
	                ...
	                // $production_mst_sql= "SELECT a.floor_id,a.sewing_line,a.prod_reso_allo,d.buyer_name,c.color_number_id,a.po_break_down_id,
					...
					// 	where a.production_type=b.production_type and a.production_type in(4,5) and c.id=b.color_size_break_down_id $prod_po_ids and b.production_type in(4,5) and a.id=b.mst_id and b.color_size_break_down_id=c.id and a.po_break_down_id=c.po_break_down_id and
					...
					// 	group by a.floor_id,a.sewing_line,a.prod_reso_allo,d.buyer_name,c.color_number_id,a.po_break_down_id,
         			// 	d.style_ref_no,e.po_number order by a.floor_id,a.sewing_line,d.buyer_name,e.po_number";
					 $production_mst_sql= "SELECT a.floor_id,a.sewing_line,a.prod_reso_allo,d.buyer_name,c.color_number_id,c.item_number_id,a.po_break_down_id,f.FLOOR_SERIAL_NO,c.order_quantity,
         				d.style_ref_no,e.po_number,e.grouping,			
						sum(CASE WHEN a.production_type =4 and production_date=$prod_date THEN b.production_qnty ELSE 0 END) AS today_sewingin_qnty,
						sum(CASE WHEN a.production_type =5 and production_date=$prod_date THEN b.production_qnty ELSE 0 END) AS today_sewing_out_qnty,
						sum(CASE WHEN a.production_type =4 and production_date<=$prod_date THEN b.production_qnty ELSE 0 END) AS sewingin_qnty_pre,
						sum(CASE WHEN a.production_type =5 and production_date<=$prod_date THEN b.production_qnty ELSE 0 END) AS sewingout_qnty_pre
						...
						a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $prod_con and d.job_no=e.job_no_mst AND e.id=a.po_break_down_id AND e.id=c.po_break_down_id and d.status_active=1 and d.is_deleted=0
						group by a.floor_id,a.sewing_line,a.prod_reso_allo,d.buyer_name,c.color_number_id,c.item_number_id,a.po_break_down_id,
	                ...
	                foreach ($prod_res as $key => $val) 
	                {
						...
						$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]][$val[csf('po_break_down_id')]]['grouping'] = $val[csf('grouping')];
						$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]][$val[csf('po_break_down_id')]]['order_quantity'] += $val[csf('order_quantity')];

                		$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]][$val[csf('po_break_down_id')]]['today_sewing_in'] += $val[csf('today_sewingin_qnty')];

	                	$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]][$val[csf('po_break_down_id')]]['today_sewing_out'] += $val[csf('today_sewing_out_qnty')];

	                	$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]][$val[csf('po_break_down_id')]]['pre_sewing_in'] += $val[csf('sewingin_qnty_pre')];

	                	$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]][$val[csf('po_break_down_id')]]['pre_sewing_out'] += $val[csf('sewingout_qnty_pre')];

	                	$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]][$val[csf('po_break_down_id')]]['buyer_name'] = $val[csf('buyer_name')];

	                	$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]][$val[csf('po_break_down_id')]]['style_ref_no'] = $val[csf('style_ref_no')];

	                	$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]][$val[csf('po_break_down_id')]]['po_number'] = $val[csf('po_number')];

						$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]][$val[csf('po_break_down_id')]]['item_number_id'] = $val[csf('item_number_id')];
	                            ...
	                            foreach ($production_data_arr as $floor_id => $floor_data) 
	                            {
	                            	...
	                            	?>
	                            		<tr class="gd-color" bgcolor="#dccdcd" style="font-weight: bold; text-align: left;">
	                            			<td colspan="15" align="left" width="1600"> <? echo $floor_library[$floor_id];?> </td>
	                            		</tr>
	                            	<?
                            		$sub_in = 0;
	                            	$sub_out = 0;
	...
	elseif($type==1) // show2 button
	{
		...
		// print_r($prod_po_ids);die;
			...
			from 
			pro_garments_production_mst a,pro_garments_production_dtls b ,wo_po_color_size_breakdown c, wo_po_details_master d, wo_po_break_down e, lib_prod_floor f
			where a.id=b.mst_id and b.color_size_break_down_id=c.id and c.job_id=d.id and d.id=e.job_id and a.po_break_down_id=c.po_break_down_id and c.id=b.color_size_break_down_id AND e.id=a.po_break_down_id $prod_po_ids and b.production_type in(4,5) and  a.floor_id = f.id and a.production_type in(4,5) and
			...
			$production_data_arr[$val[csf('floor_id')]][$slNo][$val[csf('sewing_line')]][$val[csf('buyer_name')]][$val[csf('job_no')]][$val[csf('po_break_down_id')]][$val[csf('color_number_id')]][$val[csf('size_id')]]['today_sewing_in'] += $val[csf('today_sewingin_qnty')];

			$production_data_arr[$val[csf('floor_id')]][$slNo][$val[csf('sewing_line')]][$val[csf('buyer_name')]][$val[csf('job_no')]][$val[csf('po_break_down_id')]][$val[csf('color_number_id')]][$val[csf('size_id')]]['today_sewing_out'] += $val[csf('today_sewing_out_qnty')];

			$production_data_arr[$val[csf('floor_id')]][$slNo][$val[csf('sewing_line')]][$val[csf('buyer_name')]][$val[csf('job_no')]][$val[csf('po_break_down_id')]][$val[csf('color_number_id')]][$val[csf('size_id')]]['pre_sewing_in'] += $val[csf('sewingin_qnty_pre')];

			$production_data_arr[$val[csf('floor_id')]][$slNo][$val[csf('sewing_line')]][$val[csf('buyer_name')]][$val[csf('job_no')]][$val[csf('po_break_down_id')]][$val[csf('color_number_id')]][$val[csf('size_id')]]['pre_sewing_out'] += $val[csf('sewingout_qnty_pre')];
															...
															foreach($color_data as $size_id => $val)
															{
																...
																// { 
																	...
																	<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_1nd<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_1nd<? echo $i; ?>" >
																		<td width="20"><? echo $i; ?></td>  
																		<? if($r==0){?>  
																		<td align="center" valign="middle" rowspan="<? echo $rowspan_arr[$floor_id][$line_id][$job_no];?>" width="90"><p><? echo $val['line_name']; //$line_library[$line_id]; ?></p></td>
																	...
																	$job_sub_in += $val['today_sewing_in'];
																	$job_sub_out += $val['today_sewing_out'];
																	$job_sub_in_pre += $val['pre_sewing_in'];
																	$job_sub_out_pre += $val['pre_sewing_out'];
																	$job_sub_wip += $wip;
	...
	else if($type==2) // show 3 button
	{
		...
		$prod_con .= ($hidden_po_id=="") ? "" : " and e.id in($hidden_po_id)";
		$prod_con .= ($txt_internal_ref=="") ? "" : " and e.grouping='$txt_internal_ref'";
		$prod_con .= ($txt_style_no=="") ? "" : " and d.style_ref_no='$txt_style_no'";
		...
		<style type="text/css">
			...
			.gd-color3
			{
                ...
                <div style="width:1100px;"> 
					...
					if(str_replace("'","",$prod_date)!="")
					{
						$po_sql= "SELECT DISTINCT a.po_break_down_id from pro_garments_production_mst a 
							where a.production_date=$prod_date and a.production_type in(4,5) and a.is_deleted=0 and a.status_active=1";	
						$production_po_ids="";
						$po_id_arr = [];
						$po_res = sql_select($po_sql);
						foreach ($po_res as $key => $val) 
					...
					// $con = connect();
					// execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ref_from=1 and ENTRY_FORM=76");
					// oci_commit($con);							
					// fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 76, 1, $po_id_arr, $empty_arr);//PO ID
					

					
					// print_r($prod_po_ids);die;
					$production_data_arr=array(); 
					 $production_mst_sql= "SELECT a.floor_id,a.sewing_line,a.prod_reso_allo,d.buyer_name,c.color_number_id,c.item_number_id,a.po_break_down_id,f.FLOOR_SERIAL_NO,
         				d.style_ref_no,e.po_number,e.grouping,			
						(CASE WHEN a.production_type =4 and production_date=$prod_date THEN b.production_qnty ELSE 0 END) AS today_sewingin_qnty,
						(CASE WHEN a.production_type =5 and production_date=$prod_date THEN b.production_qnty ELSE 0 END) AS today_sewing_out_qnty,
						(CASE WHEN a.production_type =4 and production_date<=$prod_date THEN b.production_qnty ELSE 0 END) AS sewingin_qnty_pre,
						(CASE WHEN a.production_type =5 and production_date<=$prod_date THEN b.production_qnty ELSE 0 END) AS sewingout_qnty_pre
						from 
						pro_garments_production_mst a,pro_garments_production_dtls b ,wo_po_color_size_breakdown c, wo_po_details_master d, wo_po_break_down e, lib_prod_floor f
						where a.id=b.mst_id and b.color_size_break_down_id=c.id AND c.po_break_down_id=e.id  and c.job_id=d.id AND e.id=a.po_break_down_id  and d.id=e.job_id and a.floor_id = f.id  and a.production_type in(4,5) and  
						a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $prod_con  and d.status_active=1 and d.is_deleted=0";
						// echo $production_mst_sql;die;
	                $prod_res = sql_select($production_mst_sql);
	                ...
	                foreach ($prod_res as $key => $val) 
	                {
						...
						$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]]['grouping'] = $val[csf('grouping')];

                		$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]]['today_sewing_in'] += $val[csf('today_sewingin_qnty')];

	                	$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]]['today_sewing_out'] += $val[csf('today_sewing_out_qnty')];

	                	$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]]['pre_sewing_in'] += $val[csf('sewingin_qnty_pre')];

	                	$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]]['pre_sewing_out'] += $val[csf('sewingout_qnty_pre')];

	                	$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]]['buyer_name'] = $val[csf('buyer_name')];

	                	$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]]['style_ref_no'] = $val[csf('style_ref_no')];

	                	$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]]['po_number'] = $val[csf('po_number')];

						$production_data_arr[$val[csf('floor_id')]][$slNo][$line_name][$val[csf('buyer_name')]][$val[csf('color_number_id')]]['item_number_id'] = $val[csf('item_number_id')];		                	                		                	
                    	...
                    	{                	
	                    	...
	                    	{	
	                    		...
	                    		foreach ($line_data as $buyer_id => $buyer_data) 
	                    		{
	                    		   	...
	                    		   	foreach ($buyer_data as $color_id =>$val) 
		                    		{
		                            			...
		                            			{													
														?> 
															<tr height="30" bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_1nd<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_1nd<? echo $i; ?>" >
																<td width="20"><? echo $i; ?></td>  
																<? if($r==0){?>  
																<td align="center" valign="middle" rowspan="<? echo $rowspan_arr[$floor_id][$line_id];?>" width="90"><p><? echo $line_id; ?></p></td>
																<?}?>
																<td valign="middle" width="150"><p><? echo $buyer_library[$buyer_id]; ?></p></td>
	            ...
	            <table width="700" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
	                ...
	                <tbody>
	                	<tr>
	                        ...
	                        <td align="center">
	                        	<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view ('<? echo $company_name; ?>'+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('cbo_year').value, 'search_list_view', 'search_div', 'floor_wise_sewing_wip_report_controller', 'setFilterGrid(\'tbl_list_search\',-1)');check_all_data();" style="width:80px;" />
	                    	</td>
	                    </tr>
	            	</tbody>
	           	</table>
	            ...
	            <table width="700" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
	                ...
	                <tbody>
	                	<tr>
	                        ...
	                        <td align="center">
	                        	<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view ('<? echo $company_name; ?>'+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('cbo_year').value, 'search_list_view', 'search_div', 'floor_wise_sewing_wip_report_controller', 'setFilterGrid(\'tbl_list_search\',-1)');" style="width:80px;" />
	                    	</td>
	                    </tr>
	            	</tbody>
	           	</table>
