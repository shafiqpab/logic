...
{ 
	...
	$lay_cond .= ($shipping_status != "") 	? " and b.shiping_status in($shipping_status)" : "";
	//======================================================================================
	...
	$gmt_prod_cond .= ($int_ref_no != "") 		? " and d.grouping like('%$int_ref_no%')" : "";
	$gmt_prod_cond .= ($shipping_status != "") 	? " and d.shiping_status in($shipping_status)" : "";	
	// echo $floor_group."*".$floor_name;die();
	if($floor_group!='0' && $floor_name=="")
	{
		$floor_group_sql=sql_select("SELECT   id  FROM lib_prod_floor where group_name ='$floor_group' and status_active=1 ");
		// echo "SELECT   id  FROM lib_prod_floor where group_name ='$floor_group' and status_active=1";die();
	...
	foreach ($lay_res as $val) 
	{
		...
		/*if($po_chk_array[$val['PO_ID']][$val['ITEM_ID']][$val['COUNTRY_ID']][$val['COLOR_ID']]=="")
		{
			$order_qty_array[$val['PO_ID']][$val['ITEM_ID']][$val['COUNTRY_ID']][$val['COLOR_ID']] += $val['ORDER_QUANTITY'];
			$po_chk_array[$val['PO_ID']][$val['ITEM_ID']][$val['COUNTRY_ID']][$val['COLOR_ID']]=="porimoni";
		}*/
	}
	...
	/==========================================================================================*/ 
	$sql_lay=" SELECT d.working_company_id as COMP_ID,d.LOCATION_ID,d.FLOOR_ID,f.ORDER_ID,e.gmt_item_id as ITEM_ID,f.COUNTRY_ID,e.color_id as COLOR_ID, (case when d.entry_date=$txt_production_date then f.size_qty else 0 end )  as TODAY_LAY,  f.size_qty as TOTAL_LAY
		from ppl_cut_lay_mst d, ppl_cut_lay_dtls e, ppl_cut_lay_bundle f
		where  d.id=e.mst_id and d.id=f.mst_id and e.id=f.dtls_id and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 $po_id_cond"; // and f.order_id in($poIds)  
	 // echo $sql_lay;die();
	$lay_res = sql_select($sql_lay);
	$lay_qty_array = array();
	foreach ($lay_res as $val) 
	...
	$sql_prod=" SELECT a.serving_company as COMP_ID, a.LOCATION,a.FLOOR_ID,e.JOB_NO_PREFIX_NUM, e.BUYER_NAME,e.STYLE_REF_NO,e.CLIENT_ID,e.id as JOB_ID,e.job_no_prefix_num as JOB_NO,d.id as PO_ID,d.PO_NUMBER,d.GROUPING,d.SHIPING_STATUS,c.color_number_id as COLOR_ID, a.production_date, c.po_break_down_id as order_id, c.item_number_id as ITEM_ID, c.COUNTRY_ID,c.country_ship_date as SHIP_DATE

		from  pro_garments_production_mst a,wo_po_color_size_breakdown c ,wo_po_break_down d,wo_po_details_master e 
		where a.po_break_down_id=c.po_break_down_id and a.po_break_down_id=d.id and c.po_break_down_id=d.id and e.id=d.job_id and e.id=c.job_id and c.item_number_id=a.item_number_id and c.country_id=a.country_id and d.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and c.status_active in(1,2,3) and c.is_deleted=0 and d.status_active in(1,2,3) and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and  a.production_date=$txt_production_date $gmt_prod_cond";		
	// echo $sql_prod;die;
	...
	foreach($po_arr_cond as $key=>$value)
	{
	   	if($pi==0)
	   	{
			$po_id_cond=" and ( a.po_break_down_id  in(".implode(",",$value).")"; 		
	   	}
	  	else //po_break_down_id
	   	{
			$po_id_cond.=" or a.po_break_down_id  in(".implode(",",$value).")";
	...
	$sql_prod=" SELECT a.serving_company as COMP_ID, a.LOCATION,a.FLOOR_ID, e.BUYER_NAME,e.id as JOB_ID,d.id as PO_ID,c.color_number_id as COLOR_ID, c.item_number_id as ITEM_ID, c.COUNTRY_ID,		
		(CASE WHEN   a.production_date=$txt_production_date and  b.production_type =1 and a.production_type =1 THEN b.production_qnty  ELSE 0 END) AS TODAY_CUTTING_QNTY,
		(CASE WHEN      b.production_type =1 and a.production_type =1 THEN b.production_qnty  ELSE 0 END) AS TOTAL_CUTTING_QNTY,
		(CASE WHEN   a.production_date=$txt_production_date and  b.production_type =4 and a.production_type =4 THEN b.production_qnty  ELSE 0 END) AS TODAY_INPUT_QNTY,
		(CASE WHEN      b.production_type =4 and a.production_type =4 THEN b.production_qnty  ELSE 0 END) AS TOTAL_INPUT_QNTY	
		...
		where a.id=b.mst_id and b.color_size_break_down_id=c.id and a.po_break_down_id=c.po_break_down_id and a.po_break_down_id=d.id and c.po_break_down_id=d.id and e.id=d.job_id and e.id=c.job_id and c.item_number_id=a.item_number_id and c.country_id=a.country_id and d.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and c.status_active in(1,2,3) and c.is_deleted=0 and d.status_active in(1,2,3) and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0  $po_id_cond";		
	// echo $sql_prod;die;
	$prod_res = sql_select($sql_prod);
	$prod_data = array();
	foreach ($prod_res as $val) 
	{
		$prod_data[$val['COMP_ID']][$val['LOCATION']][$val['FLOOR_ID']][$val['BUYER_NAME']][$val['JOB_ID']][$val['PO_ID']][$val['ITEM_ID']][$val['COUNTRY_ID']][$val['COLOR_ID']]['today_cutting'] += $val['TODAY_CUTTING_QNTY'];
		$prod_data[$val['COMP_ID']][$val['LOCATION']][$val['FLOOR_ID']][$val['BUYER_NAME']][$val['JOB_ID']][$val['PO_ID']][$val['ITEM_ID']][$val['COUNTRY_ID']][$val['COLOR_ID']]['total_cutting'] += $val['TOTAL_CUTTING_QNTY'];
		$prod_data[$val['COMP_ID']][$val['LOCATION']][$val['FLOOR_ID']][$val['BUYER_NAME']][$val['JOB_ID']][$val['PO_ID']][$val['ITEM_ID']][$val['COUNTRY_ID']][$val['COLOR_ID']]['today_input'] += $val['TODAY_INPUT_QNTY'];
		$prod_data[$val['COMP_ID']][$val['LOCATION']][$val['FLOOR_ID']][$val['BUYER_NAME']][$val['JOB_ID']][$val['PO_ID']][$val['ITEM_ID']][$val['COUNTRY_ID']][$val['COLOR_ID']]['total_input'] += $val['TOTAL_INPUT_QNTY'];
	...
	/==========================================================================================*/
	$po_id_cond = str_replace("a.po_break_down_id", "c.po_break_down_id", $po_id_cond);
	$sql_order=" SELECT c.po_break_down_id as PO_ID,C.item_number_id as ITEM_ID,c.COUNTRY_ID,C.color_number_id as COLOR_ID,c.order_quantity as ORDER_QUANTITY
		from wo_po_color_size_breakdown c
		where c.status_active in(1,2,3) and c.is_deleted=0 $po_id_cond";
				...
				<table cellspacing="0" cellpadding="0"  border="1" rules="all"  width="1900" class="rpt_table" align="left">
					<thead>
						<tr >
							...
							<th width="210" colspan="3">Cutting QC Qty</th>
														...
														foreach ($po_data as $item_id => $item_data) 
														{
															foreach ($item_data as $country_id => $country_data) 
															{
																foreach ($country_data as $color_id => $row) 
																{
																	...
																	$today_cutting = $prod_data[$com_id][$loc_id][$floor_id][$buyer_id][$job_id][$po_id][$item_id][$country_id][$color_id]['today_cutting'];
																	$total_cutting = $prod_data[$com_id][$loc_id][$floor_id][$buyer_id][$job_id][$po_id][$item_id][$country_id][$color_id]['total_cutting'];
																	$today_input = $prod_data[$com_id][$loc_id][$floor_id][$buyer_id][$job_id][$po_id][$item_id][$country_id][$color_id]['today_input'];
																	$total_input = $prod_data[$com_id][$loc_id][$floor_id][$buyer_id][$job_id][$po_id][$item_id][$country_id][$color_id]['total_input'];


																	if($job_no !="" || $order_no !="" || $int_ref_no !="")
																	{
																		if($today_lay>0 || $total_lay>0 || $today_cutting>0 || $today_input>0)
																		{
																			...
																			<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('trs_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="trs_<? echo $i; ?>">
																				...
																				<td align="right" width="70" ><? echo number_format($today_cutting,0);?></td>
																				<td align="right" width="70" ><? echo number_format($total_cutting,0);?></td>
																				<td align="right" width="70" ><? echo number_format(($total_cutting / $row['order_quantity'])*100,2);?></td>
																				<td align="right" width="70" ><? echo number_format($today_input,0);?></td>
																				<td align="right" width="70" ><? echo number_format($total_input,0);?></td>
																				<td align="right" width="70" ><? echo number_format(($total_input / $row['order_quantity'])*100,2);?></td>
																			...
																			$gr_today_cut_qty 	+= $today_cutting;
																			$gr_total_cut_qty 	+= $total_cutting;
																			$gr_today_in_qty 	+= $today_input;
																			$gr_total_in_qty 	+= $total_input;			
																		}
																	}
																	else
																	{
																		if($today_lay>0 || $today_cutting>0 || $today_input>0)
																		{
																			$bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF" ;
																			?>
																			<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('trs_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="trs_<? echo $i; ?>">
