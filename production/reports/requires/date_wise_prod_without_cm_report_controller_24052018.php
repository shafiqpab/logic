					...
					<tr>
						...
						<div style="clear:both;">
							<table width="1750" cellspacing="0" border="1" class="rpt_table" rules="all" id="" >
								<thead>
									<tr>
										...
										<th width="80"  style="word-wrap: break-word;word-break: break-all;" >Total Finish</th>
                            ...
                            <div style="overflow-y:scroll; max-height:225px; width:1768px" >
                                ...
                                $sql_dtls="SELECT a.location, a.company_id, a.serving_company, a.floor_id, a.sewing_line, a.po_break_down_id, a.production_date, a.item_number_id, a.production_type, a.production_source, a.embel_name, max(a.prod_reso_allo) as prod_reso_allo, sum(b.production_qnty) as production_quantity, sum(a.re_production_qty) as re_production_qty, sum(a.reject_qnty) as reject_qnty, sum(a.carton_qty) as carton_qty 
                                ...
                                group by a.location, a.company_id, a.serving_company, a.floor_id, a.sewing_line, a.po_break_down_id, a.production_date, a.item_number_id, a.production_type, a.production_source, a.embel_name
                               ...
                               $po_wise_unit_price_sql="SELECT a.buyer_name,b.id,b.unit_price from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $poIds_cond3 $all_exfac_poids  group by a.buyer_name,b.id,b.unit_price";
                                ...
                                ?>
                                    ...
                                    <tr> 
                                        <td style="word-wrap: break-word;word-break: break-all;" colspan="2" align="right">Total</td> 
                                        ...
                                        <td width="80" style="word-wrap: break-word;word-break: break-all;" align="right"><? echo number_format($sumFinish_qty); ?></td>
                                        <td width="80" style="word-wrap: break-word;word-break: break-all;" align="right"><? echo number_format($sum_exfact); ?></td>
                                        <td width="80" style="word-wrap: break-word;word-break: break-all;" align="right"><? echo number_format($sum_exfact_fob); ?></td>
                                        <td width="80" style="word-wrap: break-word;word-break: break-all;" align="right"><? echo number_format($sum_exfact_bal); ?></td>
			...
			if(str_replace("'","",trim($cbo_subcon))==2) //yes
			{
				...
				?>
					...
					<h5 style="width:800px;float: left; background-color:#FCF"><strong>Production-Subcontract Order (Inbound) Details</strong></h5>
						...
						<table width="1400" cellspacing="0" border="1" class="rpt_table" rules="all" id="table_header_2">
							<thead>
								<tr>
									<th width="30">Sl.</th>    
									<th width="100">Working Factory</th>
					...
					<div style="max-height:300px; overflow-y:scroll; width:1420px" id="scroll_body2">
						<table border="1"  class="rpt_table"  width="1400" rules="all" id="sub_list_view">
							  <? 
								...
								{
									 $order_sql= "select c.id, c.order_no, c.cust_style_ref, a.company_id, a.party_id, a.location_id, a.job_no_prefix_num, b.gmts_item_id, b.line_id, sum(CASE WHEN b.order_id=c.id THEN b.production_qnty ELSE 0 END) AS production_qnty,  b.production_date, b.prod_reso_allo from 											subcon_ord_mst a, subcon_gmts_prod_dtls b, subcon_ord_dtls c where 	 b.order_id=c.id and b.production_date between $txt_date_from and $txt_date_to  and a.company_id=$cbo_company_name 	and a.subcon_job=c.job_no_mst $sub_floor_name $sub_location_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by b.order_id, c.id, c.order_no, c.cust_style_ref, a.company_id, a.job_no_prefix_num, a.party_id, a.company_id, a.party_id, a.location_id, b.gmts_item_id, b.line_id, b.production_date, b.prod_reso_allo order by c.id";
								}
							   ...
							   // echo $order_sql;//die;
								...
								<table border="1" class="tbl_bottom"  width="1400" rules="all" id="report_table_footer2" >
									<tr>
										<td width="30"></td>   
										<td width="100"></td>
										<td width="100"></td>
	...
	<div>
		<div style="width:<? echo $div_width; ?>px" id="">
			...
			<table width="2080" cellspacing="0" border="1" rules="all" align="left">
				...
				if($type==1)
				{
					 $sql_dtls="SELECT a.job_no_prefix_num,a.job_no, a.buyer_name, a.style_ref_no, a.company_name, b.id, b.po_number, b.pub_shipment_date, b.grouping, b.file_no, c.po_break_down_id, c.production_date, c.item_number_id, c.prod_reso_allo, 
					...
					sum(case when c.production_type=7 and d.production_type=7   then c.re_production_qty else 0 end) as re_iron_qnty,
					...
					group by a.job_no_prefix_num,a.job_no, a.buyer_name, a.style_ref_no, a.company_name, b.id, b.po_number, b.pub_shipment_date, b.grouping, b.file_no, c.po_break_down_id, c.production_date, c.item_number_id, c.prod_reso_allo
					order by b.pub_shipment_date";
				}
				else
				{
					 $sql_dtls="SELECT a.job_no_prefix_num,a.job_no, a.buyer_name, a.style_ref_no, a.company_name, b.id, b.po_number, b.pub_shipment_date, b.grouping, b.file_no, c.location, c.floor_id, c.sewing_line, c.po_break_down_id, c.production_date, c.item_number_id, c.prod_reso_allo,c.production_source, 
					sum(case when c.production_type=1 and d.production_type=1 then d.production_qnty else 0 end) as cutting_qnty,
					sum(case when c.production_type=2 and d.production_type=2 and c.embel_name=1 then d.production_qnty else 0 end) as print_input_qnty,
					sum(case when c.production_type=3  and d.production_type=3 and c.embel_name=1 then d.production_qnty else 0 end) as print_output_qnty,
					sum(case when c.production_type=2  and d.production_type=2  and c.embel_name=2 then d.production_qnty else 0 end) as embro_input_qnty,
					...
					from wo_po_details_master a, wo_po_break_down b, pro_garments_production_mst c,pro_garments_production_dtls d 
					where a.job_no=b.job_no_mst and b.id=c.po_break_down_id and c.id=d.mst_id and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1  and d.is_deleted=0 and d.status_active=1  and a.company_name=$cbo_company_name $buyer_name $file_no_cond $internal_ref_cond $txt_date $floor_name $location_cond $garmentsNature
					group by a.job_no_prefix_num,a.job_no, a.buyer_name, a.style_ref_no, a.company_name, b.id, b.po_number, b.pub_shipment_date, b.grouping, b.file_no, c.location, c.floor_id, c.sewing_line, c.po_break_down_id, c.production_date, c.item_number_id, c.prod_reso_allo,c.production_source
					order by b.pub_shipment_date";
				}
