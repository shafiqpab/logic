...
//dyes B2
	...
	// Only for Self Batch -- Following Batch Progress Report
	 $sql_data_sub="SELECT a.id,a.batch_no,a.floor_id,c.entry_form,a.batch_against,(b.batch_qnty) as batch_qty, c.process_end_date as prod_date,a.batch_weight, a.working_company_id,b.prod_id,b.po_id,b.width_dia_type,a.color_id,a.booking_no,a.extention_no, b.item_description, a.booking_without_order 
		...
		where  a.id=b.mst_id  and c.batch_id=a.id and c.batch_id=b.mst_id and c.entry_form in(38) and a.is_sales=0 and c.load_unload_id=2 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and b.po_id>0 $batch_no_cond $date_cond  $workingCompany_cond2  $mc_cond $floor_cond
		order by a.id";		
	//echo $sql_data;
   ...
   $sql_data="SELECT a.id,a.batch_no,a.color_id,a.floor_id,c.entry_form,a.batch_against,(b.batch_qnty) as batch_qty,a.process_id, c.process_end_date as prod_date,a.batch_weight, a.working_company_id,b.prod_id,b.po_id,b.width_dia_type,a.color_id,a.booking_no,a.extention_no, b.item_description, a.booking_without_order,d.detarmination_id as deter_id 
		from pro_batch_create_mst a, pro_batch_create_dtls b,pro_fab_subprocess c ,product_details_master d
		where  a.id=b.mst_id  and c.batch_id=a.id and c.batch_id=b.mst_id and d.id=b.prod_id and c.entry_form in(35) and a.is_sales=0 and c.load_unload_id=2 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and b.po_id>0   $batch_no_cond $date_cond $mc_cond $floor_cond $workingCompany_cond 
		order by a.id";		
	//echo $sql_data;
...
// ========================================Start===========================================
 ...
 $sql_sub_fin=" SELECT  b.order_id,b.batch_id,b.product_qnty as fin_qty,a.product_type as process from subcon_production_mst a, subcon_production_dtls b where a.id=b.mst_id and a.entry_form=292  and a.product_type=4 $sub_batch_cond";
	...
	foreach($sql_sub_fin_result as $row)// for Finish Production
	{
		...
		$finish_data_arr[$row[csf('batch_id')]]['sub_finish_amt']+=$row[csf('fin_qty')]*$sub_order_wise_process_arr[$$row[csf('batch_id')]][$row[csf('process')]]['fin_rate'];
	}
				...
				foreach($prod_date_arr as $date_key=>$row)
				{
					//$total_booking_qty=$total_batch_qty=$total_finish_qty=$total_delivery_qty=$total_process_loss_qty=0;
								...
								<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trself_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="trself_<? echo $i; ?>">
									...
									<td width="100" align="right"><p><a href="##"  onClick="fnc_dyeing_popup('<? echo $date_key; ?>','<? echo $all_batch_idspopup; ?>','<? echo $working_company; ?>','dyes_chemical_cost_earn_popup',5)"><? echo number_format($tot_dye_chemical_cost,2); ?></a></p></td>
...
if ($action=="dyeing_earn_inhouse_popup")  // All Production Data popup dyeing_earn_inboundSub_popup
{
	...
	if($poIds !="")
	{
	  ...
	  $sql_po="SELECT a.buyer_name,b.id,b.job_no_mst,b.shipment_date,b.po_quantity,b.pub_shipment_date,b.grouping as ref_no,c.color_number_id as color_id,d.lib_yarn_count_deter_id as deter_id,f.cons_process,f.charge_unit,f.color_break_down from wo_po_break_down b,wo_po_details_master a,wo_po_color_size_breakdown c,wo_pre_cost_fabric_cost_dtls d,wo_pre_cos_fab_co_avg_con_dtls e,wo_pre_cost_fab_conv_cost_dtls f where  a.id=b.job_id and a.id=c.job_id and a.id=d.job_id and a.id=e.job_id and a.id=f.job_id and b.id=c.po_break_down_id and d.id=e.pre_cost_fabric_cost_dtls_id and c.po_break_down_id=e.po_break_down_id and c.item_number_id= d.item_number_id and c.color_number_id=e.color_number_id and c.size_number_id=e.gmts_sizes and d.id=f.fabric_description  and e.cons !=0    and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1   and f.cons_process not in(1,30,35) and d.is_deleted=0 and d.status_active=1 and e.is_deleted=0 and e.status_active=1 and f.is_deleted=0 and f.status_active=1 $po_cond";
	$sql_po_result = sql_select($sql_po);
	foreach ($sql_po_result as $val) 
	{
		$color_break_down=$val[csf('color_break_down')];
...
/*	$sql_data="SELECT a.id,a.batch_no,a.batch_date,a.color_id,a.floor_id,c.entry_form,a.batch_against,(b.batch_qnty) as batch_qty,c.process_id, c.process_end_date as prod_date,a.batch_weight, a.working_company_id,b.prod_id,b.po_id,b.width_dia_type,a.color_id,a.booking_no,a.extention_no, b.item_description, a.booking_without_order,d.detarmination_id as deter_id 
		...
		order by a.id";	*/
		 $sql_data_sub="SELECT a.id,a.batch_no,a.batch_date,a.floor_id,c.entry_form,a.batch_against,(b.batch_qnty) as batch_qty, c.process_end_date as prod_date,a.batch_weight, a.working_company_id,b.prod_id,b.po_id,b.width_dia_type,a.color_id,a.booking_no,a.extention_no, b.item_description, a.booking_without_order 
		from pro_batch_create_mst a, pro_batch_create_dtls b,pro_fab_subprocess c 
		where  a.id=b.mst_id  and c.batch_id=a.id and c.batch_id=b.mst_id and c.entry_form in(38) and a.is_sales=0 and c.load_unload_id=2 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and b.po_id>0 and a.id in($batch_id) and a.company_id=$company_id $date_cond
		order by a.id";		
	//echo $sql_data;
	$sub_nameArray=sql_select($sql_data_sub);
	foreach($sub_nameArray as $row)
        ...
        <caption><b> Issue Details:</b> </caption>
            ...
            <tbody>
                 <?
                        ...
                        {
                            if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
							$chemical_cost=$dyes_chemical_arr[$bId]['chemical_cost'];
							if($chemical_cost>0)
							{
                         ?>
                           <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trself_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="trself_<? echo $i; ?>">
                                ...
                                <td  width="80"><? echo change_date_format($row[('batch_date')]); ?></td>
                                 <td  width="80"><? echo $bId; ?></td>
                                <td  width="80" align="right"><? echo number_format($row[('self_qty')],0);?></td>
                                <td  width="80" align="right"><? echo number_format($chemical_cost,2); ?></td>
