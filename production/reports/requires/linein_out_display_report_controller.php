...
if($action=="load_drop_down_location") {   
	echo create_drop_down( 'cbo_location', 110, "select id,location_name from lib_location where status_active=1 and is_deleted=0 and company_id='$data' order by location_name", 'id,location_name', 1, '-- Select --', $selected, "load_drop_down( 'requires/linein_out_display_report_controller', document.getElementById('cbo_location').value, 'load_drop_down_floor', 'floor_td' ); get_php_form_data( document.getElementById('cbo_location').value, 'eval_multi_select', 'requires/linein_out_display_report_controller' ); load_drop_down( 'requires/linein_out_display_report_controller',document.getElementById('cbo_floor').value+'_'+document.getElementById('cbo_location').value+'_'+document.getElementById('cbo_company_name').value+'_'+document.getElementById('txt_date').value, 'load_drop_down_line', 'line_td' );", 0 );
}

if($action=="load_drop_down_floor") { //document.getElementById('cbo_floor').value
	echo create_drop_down( 'cbo_floor', 110, "select id,floor_name from lib_prod_floor where status_active=1 and is_deleted=0 and location_id='$data' and production_process=5 order by floor_name", 'id,floor_name', 1, '-- Select --', $selected, "load_drop_down( 'requires/linein_out_display_report_controller', document.getElementById('cbo_floor').value+'_'+document.getElementById('cbo_location').value+'_'+document.getElementById('cbo_company_name').value+'_'+document.getElementById('txt_date').value, 'load_drop_down_line', 'line_td' );",0 ); 
	exit();    	 
}
...
if($action=="report_generate") { 	
	/**
	 * some sample data in below info (development)
	 * prod. date: 24-11-2019
	 * company id: 1, 3 (Fashion Apparel Ltd., Dyes & Chemical [BD] Ltd.)
	 * location id: 18 (Banani)
	 * floor id: 126 (1st Floor)
	 * swing_line: 167 (1st line willforce)
	 * buyer: 249
	 * po_break_down_id: 34119
	 * po_number = 'FAL-19-00006'
	 */	
	$process = array( &$_POST );
	...
	if(str_replace("'","",trim($cbo_location)) != 0) {
		$condition .= " and a.location=$cbo_location";
	}

	if(str_replace("'","",trim($cbo_line)) != 0) {
		$condition .= " and a.sewing_line=$cbo_line";
	}

	if(str_replace("'","",trim($cbo_buyer_name)) != 0) {
		$condition .= " and b.buyer_name=$cbo_buyer_name";
	}

	if(str_replace("'","",trim($txt_job_no)) != "") {
		$condition .= " and b.job_no=$txt_job_no";
	}

	if(str_replace("'","",trim($txt_order_no)) != "") {
		$condition .= " and c.po_number=$txt_order_no";
	}

	if(str_replace("'","",trim($txt_style_no)) != "") {
		$condition .= " and b.style_ref_no=$txt_style_no";
	}

	if(str_replace("'","",trim($cbo_floor)) != "") {
		$condition .= " and a.floor_id in(" . str_replace("'","",trim($cbo_floor)) . ")";
	}
	...
	$sql_dataArray="SELECT a.id, a.line_number, a.floor_id, b.pr_date, b.target_per_hour, b.working_hour, b.man_power, b.style_ref_id, b.active_machine, b.operator,  b.helper, c.target_efficiency	from prod_resource_mst a,  prod_resource_dtls b, prod_resource_dtls_mast c where a.id=b.mst_id and b.mast_dtl_id=c.id and a.company_id=$company_id";
    ...
    <div style="width:100%">
        <div style="width:100%; font-weight:bold;"> <?php echo $floor_library[$cbo_floor]; ?>
        	<h3>Company Name : <? echo $company_library[str_replace("'","",$cbo_company_name)]; ?>
        		<br>Sewing Production Status (<? echo date("l, F d", strtotime(str_replace("'", "",  $txt_date))); ?>)
        	</h3>
        </div>
		...
		if($db_type==0)
		{
			...
			$sql.="	from pro_garments_production_mst a, wo_po_details_master b, wo_po_break_down c
					where a.company_id=$company_id $condition $txt_date_cond and a.entry_form=349 and a.production_type=13 and a.po_break_down_id=c.id and c.job_no_mst=b.job_no and a.status_active=1 and a.is_deleted=0
					group by a.prod_reso_allo, a.floor_id, a.sewing_line, b.job_no
		...
		if($db_type==0)
		{
			...
			WHERE a.po_break_down_id=d.id and d.job_no_mst=c.job_no and a.company_id=$company_id and a.po_break_down_id in($poIds) and a.entry_form in(348,349) and a.production_type in(12,13) and a.status_active=1 and c.status_active=1 and d.status_active=1
			...
			$sql_qcAlterSpotReject.=" from pro_garments_production_mst a, wo_po_details_master b, wo_po_break_down c
					where a.production_type=5 and a.po_break_down_id=c.id and c.job_no_mst=b.job_no and a.company_id=$company_id and a.po_break_down_id in($poIds) $txt_date_cond and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and b.status_active=1 group by a.floor_id, a.location, a.prod_reso_allo, a.sewing_line, a.po_break_down_id, b.job_no, a.item_number_id";
                ...
                <thead> 	 	 	 	 	 	
                    <tr>
                        ...
                        for($k=$hour; $k<=$last_hour; $k++)
						{
							...
							{	
								?>
	                        	<th width="3%" style="vertical-align:middle; word-break:break-all; <?php if (date('H') == $cur_hour) { ?> background-color: #F00; background-image: none; color: #FFF;<?php } ?>;"><?php  echo substr($start_hour_arr[$k],0,5); ?></th>
								<?
							}
							else
							{
                    ...
                    <tr>
                        ...
                        <th width="4%" style="vertical-align:middle; word-break:break-all;"><?php echo $totalGood_qty_new; ?></th>
						...
						foreach($gmts_val as $job_id=>$val)
						{
							foreach($val as $gmts_id => $row)
							{
								...
								<tr bgcolor="<?php echo $bgcolor; ?>" onclick="change_color('tr_1nd<?php echo $i; ?>','<?php echo $bgcolor; ?>')" id="tr_1nd<?php echo $i; ?>" >
									<td width="3%" bgcolor="<?php echo $frline_tdcolor; ?>" style="vertical-align:middle; word-break:break-all" align="center"><p style="color: <?php echo $frline_fontcolor; ?>; font-weight: bold;"><?php echo $line_name; ?></p></td>
									<td width="15%" style="vertical-align:middle; word-break:break-all" align="center" title='Bname=<?php echo $buyer_short_library[$row[("buyer_name")]];?> Job=<?php echo $row[("job_no_prefix_num")];?> Style=<?php echo $row[("style_ref_no")];?> Order=<?php echo $order_number;?> Item=<?php echo $garments_item[$gmts_id];?>'><p><?php echo $buyer_short_library[$row[("buyer_name")]].', '.$row[("job_no_prefix_num")].', '.$row[("style_ref_no")].', '.$order_number.', '.$garments_item[$gmts_id]; ?></p></td>


									<td width="3%" style="vertical-align:middle; word-break:break-all;" align="center" title="<?php echo 'Total Input='.$total_input.' and Total Output='.$total_output; ?>">
										<p>
										<?php
									...
									<td width="4%" style="vertical-align:middle; word-break:break-all" align="center"><?php echo $prod_resource_array[$row[('sewing_line')]][change_date_format($row[('production_date')])]['target_per_hour'].'<br/>'.$prod_resource_array[$row[('sewing_line')]][change_date_format($row[('production_date')])]['target_efficiency'];
									 $total_hterget += $prod_resource_array[$row[('sewing_line')]][change_date_format($row[('production_date')])]['target_per_hour'];  //h terget calculation
									    ?></td>
									...
									?>
	 	                            <td width="4%" style="vertical-align:middle; word-break:break-all" align="center"><?php echo $row[("good_qnty")]; ?></td>
	 	                            <td width="4%" style="vertical-align:middle; word-break:break-all" align="center"><?php  echo $prodAlterReject_Spot_arr[$flowre_id][$line_name][$job_id][$gmts_id]['good_qnty_new']; ?></td>
									...
									<td width="3%" style="vertical-align:middle; word-break:break-all" align="center">
										<?php echo $prod_resource_array[$row[('sewing_line')]][change_date_format($row[('production_date')])]['tpd']; ?>
									</td>															
