 ...
 //print_r($item_precost_smv_arr);
  ...
  and a.status_active=1 and a.is_deleted=0  and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and b.po_id>0 $cbo_company_cond   $buyerCond2  $dyeing_prod_date  order by a.id"; 		//and a.batch_against not in(2)
	...
	foreach($nameArray as $row)
    {
		...
		{
			$redyeing_batch_id_array[$row[csf('id')]] = $row[csf('id')];
		}
  ...
  and a.status_active=1 and a.is_deleted=0  and d.status_active=1 and d.is_deleted=0 and b.po_id>0 $cbo_company_cond $dyeing_prod_date
	...
	$total_amt=0;$dyeing_prod_self_amount_hnm_next=$dyeing_prod_self_qty_hnm_next=0;
	 foreach($nameArray as $row)
    {
	...
	$sql_subcon="SELECT a.currency_id,b.main_process_id,b.id,c.color_id,c.rate from subcon_ord_dtls b,subcon_ord_mst a,subcon_ord_breakdown c where a.subcon_job=b.job_no_mst and b.id=c.order_id and a.id=c.mst_id and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.rate>0 and b.main_process_id in(4,3) $sub_po_id_cond";
	
	$sql_subcon_res=sql_select($sql_subcon);
	$exchange_rate=80;
	foreach($sql_subcon_res as $row)
    {
		$sub_order_wise_arr[$row[csf('color_id')]][$row[csf('id')]]['process']  = $row[csf('main_process_id')];
		//$sub_order_wise_arr[$row[csf('id')]]['currency_id']  = $row[csf('currency_id')];
		if($row[csf('currency_id')]==1) //TK
		{ 
			$sub_order_wise_arr[$row[csf('color_id')]][$row[csf('id')]]['rate'] = $row[csf('rate')]/$exchange_rate;
		}
		else
		{
			$sub_order_wise_arr[$row[csf('color_id')]][$row[csf('id')]]['rate'] = $row[csf('rate')];
		}
	}
	//--------Subcon----
	...
	foreach($sub_nameArray as $row)
    {
			...
			{
			$sub_rate=$sub_order_wise_arr[$row[csf('color_id')]][$row[csf('po_id')]]['rate'];
    ...
    where a.id=b.mst_id and b.transaction_type=2  and  a.issue_purpose not in(13)  and a.entry_form=5 and a.batch_no  is not null  and   b.item_category in (5,6,7) $cbo_company_cond $buyerCond3 "); 
	

	$dyes_chemical_arr=array();
	$mm_chk=array();$mm=1;$others_tot_chemical_cost=$tot_chemical_cost=$subcon_total_chemical_cost=0;$buyer_others_tot_chemical_cost=0;
	foreach($sql_dyes_cost as $val)
	{
		...
		foreach($batchArr as $bid)
		{
			...
			if($all_batch_id==$bid && $all_batch_id!='')
			{
				//if($batchChkArr[$val[csf("batch_no")]]=='')
				//{
					...
					else
					{
						...
						if($buyer_id)
						{
						$dying_buyer_id_otherArr[$buyer_id]=$buyer_id;
						
						$others_tot_chemical_cost+=$val[csf("dyes_chemical_cost")]/$exchange_rate;
						$others_chemical_cost+=$val[csf("dyes_chemical_cost")]/$exchange_rate;
						$batcArr[$all_batch_id]=$all_batch_id;
						}
					}
					//$batchChkArr[$val[csf("batch_no")]]=766;
					
				//}
				//$subcon_total_chemical_cost+=$val[csf("dyes_chemical_cost")]/$exchange_rate;
			}
			$buyer_others_tot_chemical_cost+=$others_chemical_cost;
...
// echo implode(",",$batcArr);
	$sql_wash_dyes_cost =sql_select("select a.batch_no,a.issue_date,b.item_category,(b.cons_amount) as dyes_chemical_cost
    from inv_issue_master a, inv_transaction b
    where a.id=b.mst_id and b.transaction_type=2  and a.issue_purpose in(13)  and a.entry_form=5 and (a.batch_no  is null or a.batch_no=0)  and   b.item_category in (5,6,7) $cbo_company_cond $buyerCond3 $issue_dateCond"); 
	...
	$tot_wash_dyes_chemical_cost=0;$total_wash_dyes_chemical_cost=0;
	foreach($sql_wash_dyes_cost as $row)
	{
		...
		$total_wash_dyes_chemical_cost+=$row[csf("dyes_chemical_cost")]/$exchange_rate;
	}
	unset($sql_wash_dyes_cost);
	//echo $total_wash_dyes_chemical_cost.'sd';
	// ===============Knitting==================
		...
		<div id="scroll_body" align="center" style="height:auto; width:auto; margin:0 auto; padding:0;">
			...
			<table width="<? echo $table_width; ?> " border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
				...
				<thead>
				  ...
				  {
					$tot_dying=$dyeing_prod_self_amount_hnm_next+$dyeing_prod_self_amount;
					$tot_subcon=$subcon_prod_amount;
				   ...
				   $total_dyeingVal=($dyeing_prod_self_amount+$dyeing_prod_self_amount_hnm_next+$subcon_prod_amount)-$total_wash_dyes_chemical_cost;
				   $total_dyeingsubFinVal=$total_dyeingVal+$tot_special_finish_prod_amt;
				  ...
				  ?>
                    ...
                    <td width="100" align="right" title="Qty=<? echo $$dyeing_prod_self_qty_hnm_next;?>"  style="word-break: break-all;word-wrap: break-word;"><? echo number_format($dyeing_prod_self_qty_hnm_next,0);?></td>
                    ...
                    <td width="100"  align="right"style="word-break: break-all;word-wrap: break-word;"><a href="##"  onClick="fnc_dyeing_popup('<? echo $date_data_popup; ?>','<? echo $buyer_id; ?>','<? echo $cbo_company; ?>','dyeing_earn_inhouse_popup',3)"><? echo number_format($subcon_prod_amount,2); ?></a><? //echo number_format($dyeing_prod_self_amount+$subcon_prod_amount,0);?></td>
                     ...
                     <tr style="" bgcolor="<? echo $bgcolor2; ?>" onclick="change_color('tr_1ndWash','<? echo $bgcolor; ?>')" id="tr_1ndWash">
					<td width="250"  style="word-break: break-all;word-wrap: break-word;">Cost(Machine Wash,Lab Test):</td>
                    <td width="100" align="right"  style="word-break: break-all;word-wrap: break-word;"><? //echo number_format($subcon_prod_qty,0);?></td>
                    <td width="100"  align="right"style="word-break: break-all;word-wrap: break-word;"><a href="##"  onClick="fnc_dyeing_popup('<? echo $date_data_popup; ?>','<? echo $buyer_id; ?>','<? echo $cbo_company; ?>','dyeing_earn_inhouse_popup',4)"><? //echo number_format($total_wash_dyes_chemical_cost,2); ?></a><? echo number_format($total_wash_dyes_chemical_cost,0);?></td>
				   </tr>
                   
                     <tr>
                    <td style="word-break: break-all;" title="Tot Dyeing=$<? echo $tot_dying.',Subcon=$'.$tot_subcon;?>" align="right" width="250"><b>Total:</b></td>
                  ...
                  <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_1nd4','<? echo $bgcolor2; ?>')" id="tr_1nd4">
                   ...
                   // $total_dyeingVal=$tot_special_finish_prod_amt;
					?>
                     <tr style="">
                    <td style="word-break: break-all;" align="right" width="250"><b>Total Dyeing+Total Finish Income:</b></td>
...
if ($action=="dyeing_earn_inhouse_popup")  // All Production Data popup dyeing_earn_inboundSub_popup
{
	 ...
	 if($db_type==0)
			{
				...
				$issue_dateCond= " and a.issue_date between '".$start_date."' and '".$end_date."'";
			}
			else if($db_type==2)
			{
		 ...
		 // echo $type_id.'SDSD';die;
 		 ...
 		 $exchange_rate=80;
		  $re_sql_data="SELECT a.id,a.batch_no,a.color_id,a.floor_id,c.entry_form,a.batch_against,(b.batch_qnty) as batch_qty,c.process_id, c.process_end_date as prod_date,a.batch_weight, a.working_company_id,b.prod_id,b.po_id,b.width_dia_type,a.color_id,a.booking_no,a.extention_no, b.item_description, a.booking_without_order,d.detarmination_id as deter_id 
		from pro_batch_create_mst a, pro_batch_create_dtls b,pro_fab_subprocess c ,product_details_master d,wo_po_break_down e,wo_po_details_master f
		where  a.id=b.mst_id  and c.batch_id=a.id and c.batch_id=b.mst_id and d.id=b.prod_id and e.id=b.po_id and e.job_id=f.id and c.entry_form in(35)  and a.is_sales=0 and c.load_unload_id=2 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0  and c.status_active=1 and c.is_deleted=0
  and a.status_active=1 and a.is_deleted=0  and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and b.po_id>0  	 and a.batch_against  in(2)   $dyeing_prod_date $cbo_company_cond $buyerCondIn3 order by a.id"; 
  $re_nameArray=sql_select($re_sql_data);
	foreach($re_nameArray as $row)
    {
		$re_batch_id_array[$row[csf('id')]] = $row[csf('id')];
	}
  ...
  and a.status_active=1 and a.is_deleted=0  and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and b.po_id>0    $dyeing_prod_date $cbo_company_cond $buyerCondIn3 order by a.id"; 		//and a.batch_against not in(2)
    ...
    where a.id=b.mst_id and b.transaction_type=2  and  a.issue_purpose not in(13)  and a.entry_form=5 and a.batch_no  is not null  and   b.item_category in (5,6,7) $cbo_company_cond  $buyerCondIn  ");
			...
			$dyes_chemical_arr=array();
			$mm_chk=array();$mm=1;$others_tot_chemical_cost=$tot_chemical_cost_hnmNext=0;$others_tot_chemical_cost=0;$total_buyer_othersCost=0;$tot_buyer_othersCost=0;
			$re_tot_chemical_cost_hnmNext=$re_other_tot_chemical_cost=0;
			foreach($sql_dyes_cost as $val)
			{
				$batchArr=array_unique(explode(",",$val[csf("batch_no")]));
				$buyer_id=$val[csf("buyer_id")];
				
				$tot_chemical_cost_hnmNext=0;$re_other_tot_chemical_cost=0;
				foreach($batchArr as $bid)
				{
					$all_batch_id=$all_batch_id_array[$bid];//Dyeing only
					if($all_batch_id==$bid && $all_batch_id!='')
					{
						...
						else
						{
							...
							$tot_buyer_othersCost+=$val[csf("dyes_chemical_cost")]/$exchange_rate;
							$batcArr[$all_batch_id]=$all_batch_id;
							}
							
						}
					}
					$re_all_batch_id=$re_batch_id_array[$bid];//Re Dyeing only
					if($re_all_batch_id==$bid && $re_all_batch_id!='')
					{
					  if($buyer_id==1 || $buyer_id==10) // HnM and Next
						{
							 $re_tot_chemical_cost_hnmNextArr[$buyer_id]+=$val[csf("dyes_chemical_cost")]/$exchange_rate;
							 
							  $tot_chemical_cost_hnmNext+=$val[csf("dyes_chemical_cost")]/$exchange_rate;
						}
						else
						{
							 $re_other_tot_chemical_cost_hnmNextArr[$buyer_id]+=$val[csf("dyes_chemical_cost")]/$exchange_rate;
							  $re_other_tot_chemical_cost+=$val[csf("dyes_chemical_cost")]/$exchange_rate;
						}
					}
				}
				$re_tot_chemical_cost_hnmNext+=$tot_chemical_cost_hnmNext+$re_other_tot_chemical_cost;
			}
			//echo $re_tot_chemical_cost_hnmNext;
			// print_r($re_other_tot_chemical_cost_hnmNextArr);
			//echo count($batcArr).'sd';;
			//echo $re_tot_chemical_cost_hnmNext.'='.$re_other_tot_chemical_cost.',';
	...
	//echo $buyer_id.'='.$row[csf('id')].'<br>';
		...
		///////===========Sub Con ========================
		  if($type_id==3)
		  {
			...
			$re_sql_data_sub="SELECT a.id,a.batch_no,a.floor_id,c.entry_form,a.batch_against,(b.batch_qnty) as batch_qty, c.process_end_date as prod_date,a.batch_weight, a.working_company_id,b.prod_id,b.po_id,b.width_dia_type,a.color_id,a.booking_no,a.extention_no, b.item_description, a.booking_without_order 
			from pro_batch_create_mst a, pro_batch_create_dtls b,pro_fab_subprocess c 
			where  a.id=b.mst_id  and c.batch_id=a.id and c.batch_id=b.mst_id and c.entry_form in(38)   and a.is_sales=0 and c.load_unload_id=2 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and b.po_id>0 and a.batch_against in(2) $cbo_company_cond $dyeing_prod_date 
			order by a.id";		
		//echo $sql_data;
			$re_sub_nameArray=sql_select($re_sql_data_sub);
			foreach($sub_nameArray as $row)
			{
				$re_sub_batch_id_array[$row[csf('id')]] = $row[csf('id')];
			}
		...
		$sql_subcon="SELECT a.currency_id,a.party_id,b.main_process_id,b.id,c.color_id,c.rate from subcon_ord_dtls b,subcon_ord_mst a,subcon_ord_breakdown c where a.subcon_job=b.job_no_mst and b.id=c.order_id and a.id=c.mst_id and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.rate>0 and b.main_process_id in(4,3) $sub_po_id_cond";
		$sql_subcon_res=sql_select($sql_subcon);
			$exchange_rate=80;
		foreach($sql_subcon_res as $row)
		{
    ...
    where a.id=b.mst_id and b.transaction_type=2  and  a.issue_purpose not in(13)  and a.entry_form=5 and a.batch_no  is not null  and   b.item_category in (5,6,7) $cbo_company_cond  $buyerCondIn ");
			...
			foreach($sql_dyes_cost as $val)
			{
				...
				foreach($batchArr as $bid)
				{
					...
					if($all_batch_id==$bid)
					{
						//echo $val[csf("dyes_chemical_cost")].'sdd';
					$subcon_tot_chemical_costArr[$buyer_id]+=$val[csf("dyes_chemical_cost")]/$exchange_rate;
					}
					
					$re_all_batch_id=$re_sub_batch_id_array[$bid];//Re Dyeing only
					if($re_all_batch_id==$bid)
					{
						//echo $val[csf("dyes_chemical_cost")].'sdd';
					$re_subcon_tot_chemical_costArr[$buyer_id]+=$val[csf("dyes_chemical_cost")]/$exchange_rate;
					}
				}
			}
         ...
         <caption> <b> <? echo $head_type; ?> Details-(<? echo $dyeing_buyer;?>)</b></caption>
            ...
            <tbody>
                 <?
                      ...
                      <tr bgcolor="#CCCCCC"> 
                        <td align="right" colspan="2"><strong>Redyeing Cost Total</strong></td>
                        <td align="right"><? //echo number_format($tot_dyeing_qty,2); ?>&nbsp;</td>
                           <?
							//if($type_id!=3)
							//{
                        ...
                        <td align="right"><? 
						 if($type_id==1 || $type_id==2)
						 {
							echo number_format($re_tot_chemical_cost_hnmNext,2,'.','');
						 }
						 
						else  if($type_id==3)
						 {
							$re_tot_subcon_cost=array_sum($re_subcon_tot_chemical_costArr);
							echo number_format($re_tot_subcon_cost,2,'.','');
						 }
						 ?>&nbsp;</td>
                        <?
							//}
   ...
   $exchange_rate=80;
	 if($db_type==0)
			...
			else if($db_type==2)
			{
				...
				//==================================================Dyeing Prod===========.========================
				 if($company_id==0) $cbo_company_cond=""; else $cbo_company_cond=" and a.company_id=$company_id";
		$re_sql_data_special="SELECT e.id as dtls_id,a.id,a.batch_no,a.color_id,a.floor_id,c.entry_form,a.batch_against,(e.production_qty) as batch_qty,c.process_id,c.process_end_date as prod_date,a.batch_weight, a.working_company_id,b.prod_id,b.po_id,b.width_dia_type,a.color_id,a.booking_no,a.extention_no, b.item_description,c.entry_form, a.booking_without_order,d.detarmination_id as deter_id 
		from pro_batch_create_mst a, pro_batch_create_dtls b,pro_fab_subprocess c,pro_fab_subprocess_dtls e ,product_details_master d
		where  a.id=b.mst_id  and c.batch_id=a.id and c.batch_id=b.mst_id and d.id=b.prod_id and c.id=e.mst_id and e.prod_id=b.prod_id and e.prod_id=d.id  and c.entry_form in(32,48,33,34)  and a.is_sales=0 and a.batch_against in(2) and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0  and c.status_active=1 and c.is_deleted=0  and e.status_active=1 and e.is_deleted=0
  and a.status_active=1 and a.is_deleted=0  and d.status_active=1 and d.is_deleted=0 and b.po_id>0 $cbo_company_cond  $dyeing_prod_date
		order by a.id";		//and a.batch_against not in(2)
	//echo $sql_data;
	//$batch_against_chk=array(2);
	$re_nameArray_special=sql_select($re_sql_data_special);
	foreach($re_nameArray_special as $row)
    {
		$sp_re_id_array[$row[csf('id')]] = $row[csf('id')];
	}
  ...
  and a.status_active=1 and a.is_deleted=0  and d.status_active=1 and d.is_deleted=0 and b.po_id>0 $cbo_company_cond  $dyeing_prod_date
	...
	foreach ($sql_po_result as $val) 
	{
		...
		foreach($nameArray_special as $row)// for Finish Production 
		{
				...
				$sfin_entry_form_arr[$row[csf('id')]]['entry_form']=$row[csf('entry_form')];
				...
				if($special_fin_amt>0)
				{
					...
					if($dtls_chk_arr[$dtls_id]=="")
					{
						$all_batch_id_array[$row[csf('id')]]=$row[csf('id')];//Fin prod
    ...
    where a.id=b.mst_id and b.transaction_type=2  and  a.issue_purpose not in(13)  and a.entry_form=5 and a.batch_no  is not null  and   b.item_category in (5,6,7) $cbo_company_cond  $buyerCondIn ");
			...
			foreach($sql_dyes_cost as $val)
			{
				...
				foreach($batchArr as $bid)
				{ //$sp_re_id_array[$row[csf('id')]]
					$re_all_batch_id=$sp_re_id_array[$bid];//Dyeing only
					if($re_all_batch_id==$bid)
					{
						$entry_formId=$sfin_entry_form_arr[$bid]['entry_form'];
						//echo $val[csf("dyes_chemical_cost")].'sdd';
					$re_fin_dyes_chemical_costArr[$entry_formId]+=$val[csf("dyes_chemical_cost")]/$exchange_rate;
					}
				}
			}
			
		 
		 	//echo array_sum($re_fin_dyes_chemical_costArr); 
         ...
         <caption> <b>Finishing Details(<? echo implode(",",$special_buyer_arr);?>)</b></caption>
            <thead>
                <tr>
                 ...
                 <?
                        $i=1;$tot_dyeing_qty=$tot_dyeing_amount=$tot_fin_dyes_chemical_cost=0;
                        foreach($special_finish_prod_arr as $processId=>$processArr)
                        {
							...
							$prod_amt=$row[('amt')];
							if($prod_amt) $prod_amt=$prod_amt;else $prod_amt=0;
							$fin_dyes_chemical_cost=$fin_dyes_chemical_costArr[$processId][$buyerId];
							 
                         ?>
                           <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trself_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="trself_<? echo $i; ?>">
                                <td  width="20"><? echo $i; ?></td>
                                <td  width="200"><? echo $buyer_short_library[$buyerId]; ?></td>
                                <td  width="150"><? echo $entry_form[$processId]; ?></td>
						...
						$tot_dyeing_qty+=$prod_qty;
						$tot_dyeing_amount+=$prod_amt;
						$tot_fin_dyes_chemical_cost+=$fin_dyes_chemical_cost;
							}
                        }
						//array_sum($re_fin_dyes_chemical_costArr)
                        ?>
                        <tr bgcolor="#CCCCCC"> 
                        <td align="right" colspan="3"><strong>Total</strong></td>
                        <td align="right"><b><? echo number_format($tot_dyeing_qty,2); ?>&nbsp;</b></td>
                      
                        <td align="right"><b><? echo number_format($tot_dyeing_amount,2,'.',''); ?>&nbsp;</b></td>
                       
                       </tr>
                        <tr bgcolor="#CCCCCC"> 
                        <td align="right" colspan="3"><strong>Re dyeing Cost Total</strong></td>
                        <td align="left" colspan="2"><? echo number_format(array_sum($re_fin_dyes_chemical_costArr),2); ?>&nbsp;</td>
