...
if($batch_type==0)
	$batch_cond_batch="and a.entry_form in (0,36) and a.re_dyeing_from=0";
	else if($batch_type==1)
	$batch_cond_batch="and a.entry_form=0 ";
	else if($batch_type==2)
	$batch_cond_batch="and a.entry_form=36";
	else 
	$batch_cond_batch="and a.entry_form in(0,36) and a.batch_against in(2) and  a.re_dyeing_from!=0 ";
	...
	if ($report_type == 1)
	{
	    ...
	    execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_id and entry_form in (73)");
	     oci_commit($con);
		      ...
		      $sql_batch_dyeing="select a.id as batch_id,a.batch_no,a.extention_no,a.batch_against,f.process_end_date,f.result,f.in_charge_mst_id,f.reason_mst_id ,f.load_unload_id,c.first_name from  pro_batch_create_mst a ,pro_fab_subprocess f,lib_employee c where a.id=f.batch_id and f.in_charge_mst_id=c.id and f.entry_form=35 and  a.batch_against in(1) and f.result=1 and f.load_unload_id=2 and f.status_active=1 and f.is_deleted=0 and c.status_active=1 and c.is_deleted=0  and a.status_active=1 and a.working_company_id=$cbo_company_id $batch_cond_batch $prod_date_cond order by f.process_end_date asc";
	   ...
	   /*$sql_batch_dyeing_load="select a.re_dyeing_from as re_batch_id,a.batch_no,a.extention_no,a.batch_against,c.reason_type,f.process_end_date,f.result,f.reason_mst_id ,f.load_unload_id,f.process_end_date,f.batch_id,c.reason from  pro_batch_create_mst a ,pro_fab_subprocess f,lib_re_process_reason_entry c,gbl_temp_engine g where a.id=f.batch_id and f.reason_mst_id=c.id and a.re_dyeing_from=g.ref_val  and f.batch_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from=73    and f.entry_form=35 and  a.batch_against in(2) and f.load_unload_id=1 and f.status_active=1 and f.is_deleted=0 and c.status_active=1 and c.is_deleted=0  and a.status_active=1 and a.working_company_id=$cbo_company_id $batch_cond_batch  order by f.process_end_date asc";*/
		...
		// ======================================= Recipe Info Start ==================================
	    $sql_batch_recipe="select b.id as recipe_mst_id,b.dyeing_re_process,a.id as new_batch_id,a.re_dyeing_from as re_batch_id,a.batch_no,a.extention_no,a.batch_against,f.result,f.reason_mst_id ,f.load_unload_id,f.process_end_date,b.recipe_date,f.in_charge_mst_id from  pro_batch_create_mst a ,pro_recipe_entry_mst b ,pro_fab_subprocess f where   a.id=b.batch_id  and a.id=f.batch_id   and f.batch_id=b.batch_id    and b.entry_form=60  and f.load_unload_id=2  and b.dyeing_re_process in(1,2) and f.status_active=1 and f.is_deleted=0    and a.status_active=1 and a.working_company_id=$cbo_company_id $batch_cond_batch  $recipe_date_cond order by f.process_end_date asc";
		$sql_batch_recipe_result=sql_select($sql_batch_recipe);
		$recipeDateWise_batch_Arr=array();
	  foreach ($sql_batch_recipe_result as $row) 
	   {
		  $recipeDateWise_batch_Arr[$row[csf('in_charge_mst_id')]][$row[csf('process_end_date')]].=$row[csf('recipe_mst_id')].',';
	   }
	   // ======================================= Recipe RFT Info Start ==================================
	    $sql_batch_recipe_rft="select b.id as recipe_mst_id,b.dyeing_re_process,a.id as new_batch_id,a.re_dyeing_from as re_batch_id,a.batch_no,a.extention_no,a.batch_against,f.result,f.reason_mst_id ,f.load_unload_id,f.process_end_date,b.recipe_date,f.in_charge_mst_id from  pro_batch_create_mst a ,pro_recipe_entry_mst b ,pro_fab_subprocess f,gbl_temp_engine g where   a.id=b.batch_id  and a.id=f.batch_id   and f.batch_id=b.batch_id  and f.batch_id=g.ref_val and a.id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from=1 and g.entry_form=73   and b.entry_form=60  and f.load_unload_id=2  and f.result=1 and b.dyeing_re_process in(1,2) and f.status_active=1 and f.is_deleted=0    and a.status_active=1 and a.working_company_id=$cbo_company_id $batch_cond_batch  $recipe_date_cond order by f.process_end_date asc";
	   $sql_batch_recipe_result_rft=sql_select($sql_batch_recipe_rft);
	   $rft_recipeDateWise_batch_Arr=array();
					...
					<tbody id="scroll_body">
						...
						foreach($in_chargeData as $prod_date=>$row)
						{
							if ($i%2==0)  
								...
								{
									$recipeDateWise_batchArr=explode(",",$recipeDateWise_batch);
								...
								foreach($first_batchArr as $rft_batch)
								{
									...
									$rft_recipeDateWiseArrAll=implode(",",array_unique(explode(",",$rft_recipeDateWiseAll)));

									if(count($rft_recipeDateWiseArr)==1) //Signle Recipe found
									{
										...
										$tot_rft_recipe_batch_count+=count($rft_recipeDateWiseArr);
									}
									else
									{
							...
							<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i.$in_charge_id; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i.$in_charge_id; ?>" valign="top">
								...
								<td width="100"  align="center"title="No. of OK Batch/No. of Unload Batch [Excluding Re-Dyeing Batch]*100">
											<? echo number_format(($no_of_ok_batchTot/$no_of_unload_batch)*100,2); ?></td>
								...
								<td width="100" align="center" title="Topping RFT/Topping Batch*100"><? echo  number_format(($tot_rft_recipe_batch_count/$topping_recipeDateWise_batchTot)*100,2);; ?></td>
						...
						<tr style="text-align: center; background:#CCF;font-size:xx-large;">
							<td   align="right" colspan="2"><b>Total</b></td>
							<td width="80" align="center"><? echo number_format($tot_no_of_unload_batch, 0, '.', ''); ?></td>
							<td width="80" align="center"><? echo number_format($tot_no_of_ok_batchTot, 0, '.', ''); ?></td>
							<td width="80" title="No Of Ok/No Of Unload Batch*100"><? echo number_format(($tot_no_of_ok_batchTot/$tot_no_of_unload_batch)*100, 0, '.', ''); ?></td>
							<td width="80">&nbsp;</td>
							<td width="100"><? echo number_format($tot_topping_recipeDateWise_batchTot, 0, '.', ''); ?></td>
							<td width="80"><? echo number_format($tot_tot_rft_recipe_batch_count, 0, '.', ''); ?></td>
							 
							<td width="80"><? echo number_format(($tot_tot_rft_recipe_batch_count/$tot_topping_recipeDateWise_batchTot)*100, 0, '.', ''); ?></td>
							<td width="100">&nbsp;</td>
							<td width="">&nbsp;</td
