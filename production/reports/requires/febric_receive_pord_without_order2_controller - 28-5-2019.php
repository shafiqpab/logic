...
if($action=="report_generate")
{
	...
	//echo $str_cond;die;
	 $sql="select a.id as booking_id, a.booking_no, a.booking_no_prefix_num, $select_year as wo_year, a.is_short, a.po_break_down_id, a.fabric_source, a.is_approved, a.job_no, a.buyer_id, a.company_id, a.supplier_id, a.item_category, a.is_approved, b.sample_type, b.grey_fabric as grey_fabric_qnty, b.finish_fabric as finish_fabric_qty, b.fabric_color, b.fabric_description, b.style_id, b.composition, a.booking_date, b.lib_yarn_count_deter_id, a.booking_date, a.delivery_date
	...
	//-----------------------------------------------------------------------------------------------------
	//Finding Dtls ids of booking
	$booking_id=$sql_res[0][csf("booking_id")];
	$booking_no=$sql_res[0][csf("booking_no")];
	$dtls_ids_arr=array();
	$booking_wise_gray_data=array();

	$sql_dtls_ids_arr=sql_select("select b.id as dtls_id, a.roll_maintained from inv_receive_master a, pro_grey_prod_entry_dtls b where a.id=b.mst_id and a.booking_id='$booking_id' and a.booking_no='$booking_no' and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
	//echo "select b.id as dtls_id, a.roll_maintained from inv_receive_master a, pro_grey_prod_entry_dtls b where a.id=b.mst_id and a.booking_id='$booking_id' and a.booking_no='$booking_no' and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	
	$sql_ord_sample=sql_select("select b.id as dtls_id, a.to_order_id as booking_id from inv_item_transfer_mst a, inv_item_transfer_dtls b where a.id=b.mst_id and a.to_order_id='$booking_id' and a.entry_form=110 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
	if(count($sql_ord_sample)>0) $sql_dtls_ids_arr[0][csf('roll_maintained')]=1;
	
	$booking_wise_roll_check[$booking_no]=$sql_dtls_ids_arr[0][csf('roll_maintained')];

	if($booking_wise_roll_check[$booking_no]==1)// If it is roll lavel
	{
		//making dtls ids into string
		...
		//echo "select id, roll_no, barcode_no, po_breakdown_id, qnty,booking_no from pro_roll_details where dtls_id in ($dtls_ids) and booking_no='$booking_no' and entry_form=2 and status_active=1 and is_deleted=0";
		$prod_roll_dtls_arr=array();
		$sql_prod_roll_dtls_arr=sql_select("select id, roll_no, barcode_no, po_breakdown_id, qnty, booking_no from pro_roll_details where dtls_id in ($dtls_ids) and booking_no='$booking_no' and entry_form=2 and status_active=1 and is_deleted=0");
		...
		$booking_wise_gray_data[$booking_no]['barcodes']=$barcode_ids;
	...
	foreach($sql_res as $row)
	{
		...
		if(!in_array($row[csf("booking_id")],$temp_book_arr))
		{
			...
			//$buyer_wise_result[$row[csf("buyer_id")]]["yarn_issue"] +=$net_yarn_issue;
			//$buyer_wise_result[$row[csf("buyer_id")]]["knitting_total"] +=($grey_knit_production_arr[$row[csf("booking_id")]]+$grey_knit_receive_arr[$row[csf("booking_id")]]);
	...
	/*$sql_finish_store=sql_select("select a.booking_id,b.color_id,sum(case when a.receive_basis=9 then b.receive_qnty else 0 end) as production_qty,
	sum(case when a.receive_basis!=9 then b.receive_qnty else 0 end) as parcess_qty from  inv_receive_master a, pro_finish_fabric_rcv_dtls b 
	where a.id=b.mst_id  and a.entry_form=37 and a.booking_id>0 and a.booking_without_order=1 group by a.booking_id,b.color_id order by a.booking_id");
	foreach($sql_yarn_issue_rtn as $row)
	{
	...
	/*$sql_finish_store_prodction=sql_select("select c.booking_no_id,b.color_id,sum(case when a.receive_basis=9 then b.receive_qnty else 0 end) as production_qty from inv_receive_master a, pro_finish_fabric_rcv_dtls b, pro_batch_create_mst c where a.id=b.mst_id and a.entry_form=37  and b.batch_id=c.id  and c.batch_against=3 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0   group by c.booking_no_id,b.color_id ");
	
	foreach($sql_finish_store_prodction as $row)
	{
		$fin_store_rec_prod[$row[csf("booking_no_id")]][$row[csf("color_id")]]=$row[csf("production_qty")];
	}
	
	$sql_finish_store=sql_select("select a.booking_id, b.color_id,sum(b.receive_qnty ) as parcess_qty from  inv_receive_master a, pro_finish_fabric_rcv_dtls b 
	where a.id=b.mst_id  and a.entry_form=37 and a.receive_basis=2 and a.booking_id>0 and a.booking_without_order=1 group by a.booking_id,b.color_id");
	foreach($sql_yarn_issue_rtn as $row)
	{
		$fin_store_rec_purchase[$row[csf("booking_id")]][$row[csf("color_id")]]=$row[csf("parcess_qty")];
	}*/
	...
	//print_r($finishfab_product_arr);
	/*$sql_finish_prodction=sql_select("select max(a.receive_date) as receive_date , c.booking_no_id from inv_receive_master a,  pro_finish_fabric_rcv_dtls b,pro_batch_create_mst c where a.id=b.mst_id and b.batch_id=c.id and a.entry_form=7 and c.batch_against=3 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0   group by c.booking_no_id ");
	
	foreach($sql_finish_prodction as $row)
	{
		$finish_prodction_date[$row[csf("booking_no_id")]]=$row[csf("receive_date")];
	}*/
		//sum(case when a.receive_basis!=9 then b.receive_qnty else 0 end) as purchase_qty 			
	...
	//print_r($grey_knit_receive_prod_arr);
	/*if($all_delivery_id!="")
	{
		$sql_grey_knit_receive_roll=sql_select("select a.booking_id,a.entry_form, b.cons_quantity as receive_qty
		from inv_receive_master a, inv_transaction b 
		where a.id=b.mst_id and a.booking_without_order=1 and a.entry_form in(58) and b.transaction_type in(1) and a.booking_id in($all_delivery_id)");
	
		foreach($sql_grey_knit_receive_roll as $result)
		{
			$grey_knit_receive_arr[$delivery_book_id[$result[csf("booking_id")]]]+=$result[csf("receive_qty")];
		}
		unset($sql_grey_knit_receive_roll);
	}*/
	...
	//echo "select a.to_order_id, sum( case when a.transfer_criteria=7 and b.transaction_type=6 then b.cons_quantity else 0 end) as tranfer_out, sum( case when a.transfer_criteria=6 and b.transaction_type=5 then b.cons_quantity else 0 end) as tranfer_in from inv_item_transfer_mst a, inv_transaction b where a.id=b.mst_id   and a.item_category=13 and b.item_category=13 and b.transaction_type in(5,6) group by a.to_order_id order by a.to_order_id";
	
	//echo "select a.to_order_id, sum( case when a.transfer_criteria in (7,8) and b.transaction_type=6 then b.cons_quantity else 0 end) as tranfer_out, sum( case when a.transfer_criteria in(7,8) and b.transaction_type=5 then b.cons_quantity else 0 end) as tranfer_in from inv_item_transfer_mst a, inv_transaction b where a.id=b.mst_id   and a.item_category=13 and b.item_category=13 and b.transaction_type in(5,6) group by a.to_order_id order by a.to_order_id";
	
	//$sql_grey_knit_receive=sql_select("select a.to_order_id, sum( case when a.transfer_criteria in (7,8) and b.transaction_type=6 then b.cons_quantity else 0 end) as tranfer_out, sum( case when a.transfer_criteria in(6,8) and b.transaction_type=5 then b.cons_quantity else 0 end) as tranfer_in from inv_item_transfer_mst a, inv_transaction b where a.id=b.mst_id   and a.item_category=13 and b.item_category=13 and b.transaction_type in(5,6) group by a.to_order_id order by a.to_order_id");
	...
	/*echo "select a.booking_id, sum(b.cons_quantity) as issue_qty, max(a.issue_date) as issue_date
	from inv_issue_master a, inv_transaction b where a.id=b.mst_id and a.issue_basis in(0,1) and a.issue_purpose in(8,11) 
	and a.entry_form in(16,61) and b.transaction_type=2 $grey_del_pi_wo_batch_no_cond group by a.booking_id order by a.booking_id";*/
		/*echo "select a.booking_id, sum(b.issue_qnty) as issue_qty, max(a.issue_date) as issue_date
	...
	?>
	<table class="rpt_table" border="1" rules="all" width="4870" cellpadding="0" cellspacing="0">
		<thead>
			<tr>
				<th colspan="15">Booking Details</th>
	...
	<div style="width:4895px; overflow-y:scroll; max-height:300px" id="scroll_body">
    <table class="rpt_table" border="1" rules="all" width="4870" cellpadding="0" cellspacing="0" id="table_body">
        <tbody>
        <?
		$i=1;
		foreach($result_mst_array as $row)
		{
        	...
        	<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                ...
                <td width="80"><p><? if($row[("style_id")]!="") echo $style_reff_arr[$row[("style_id")]]['style_reff_no']; ?></p></td>
				...
				foreach($result_dtls_array[$row[("booking_id")]] as $dts_row)
				{
					if($m==1)
					{
                        ...
                        <td width="110" align="right">xxx<p>
                       ...
                       <td width="110" align="right"><p><? 
						...
						if ($booking_wise_roll_check[$row[("booking_no")]]==1) // If production is roll lavel
						{
	                    ...
	                    <td width="110" align="right" title="(Prod+Rec+Trans)-Issue">
							...
							if ($booking_wise_roll_check[$row[("booking_no")]]==1) // If production is roll lavel
							{
								$issueQty = $booking_wise_gray_data[$row[("booking_no")]]['gray_issue_qty'];
							}
							else
							{
	                    ...
	                    <td width="110" align="right"><p>
							...
							<? 
								if ($booking_wise_roll_check[$row[("booking_no")]]==1) //if production is roll level
								{
									...
									$dtls_tot_fin_prod_qty +=$booking_wise_gray_data[$row[("booking_no")]]['gray_fin_feb_prod_qty'];
	                        ...
	                        <td width="110" align="right"><p><? $fabric_store_available=$finish_prod_rece_store+$finish_parchase_rece_store; echo number_format($fabric_store_available,2); $dtls_tot_fabric_store_available +=$fabric_store_available;  ?></p></td>
					...
					else
					{
                        ...
                        <tr>
                            ...
                            <td width="110" align="right">xxx
	                            <a  href="##" onclick="openmypage('<? echo $row[("booking_id")]; ?>','dying_qty_popup','')">
	                            <?
	                            $dtls_dying_qty=$dyeing_check_arr[$row[("booking_id")]][$dts_row[("fabric_color")]]['batch_qnty'];
	                            echo number_format($dtls_dying_qty,2);
	                            $dtls_tot_dying_qty +=$dtls_dying_qty; 
	                            echo number_format($finishfab_product_arr[$row[("booking_id")]][$dts_row[("fabric_color")]],2); $dtls_tot_fin_prod_qty +=$finishfab_product_arr[$row[("booking_id")]][$dts_row[("fabric_color")]];  
	                            ?>
	                            </a>
	                            </p>
	                        </td>
                            <td width="110" align="right"><p><? 
                             $finish_balance=$dts_row[("finish_fabric_qty")]-$finishfab_product_arr[$row[("booking_id")]][$dts_row[("fabric_color")]]; echo number_format($finish_balance,2); $dtls_tot_fin_balance +=$finish_balance;  ?></p></td>
                            <td width="110" align="right"><p>
                            ...
                            $fabric_in_prod_floor=$finish_product_arr[$row[("booking_id")]][$dts_row[("fabric_color")]]-$finish_prodction_delivery_qty[$row[("booking_id")]];
                             echo number_format($fabric_in_prod_floor,2); $dtls_tot_fabric_in_prod_floor +=$fabric_in_prod_floor;  ?></p></td>
                            <td width="110" align="right"><p>
    ...
    <table class="rpt_table" border="1" rules="all" width="4870" cellpadding="0" cellspacing="0">
		<tfoot>
			<tr>
				...
				<th width="110"><? echo number_format($dtls_tot_fin_prod_qty,2); ?></th>
...
if($action=="yarn_issue")
{
	...
	<fieldset style="width:965px; margin-left:3px">
			<div id="report_container">
					...
					where a.id=d.mst_id and d.prod_id=c.id  and d.transaction_type=2 and d.item_category=1 and a.issue_basis=1 and a.issue_purpose=8 and d.transaction_type=2 and a.entry_form=3 and a.booking_id=$boking_id and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 group by a.booking_id, a.booking_no,a.issue_number, a.issue_date, a.challan_no, a.knit_dye_source, a.knit_dye_company,c.lot, c.yarn_type, c.id, c.product_name_details,d.brand_id";
					//echo $sql;die;
			...
			<table border="1" class="rpt_table" rules="all" width="970" cellpadding="0" cellspacing="0">
             ...
             <div style="width:990px; max-height:330px; overflow-y:scroll" id="scroll_body">
                 <table border="1" class="rpt_table" rules="all" width="970" cellpadding="0" cellspacing="0">
					...
					b.product_id, sum(b.current_delivery) as quantity 
					from inv_receive_master a, pro_grey_prod_delivery_dtls b,pro_grey_prod_delivery_mst c
					where c.id=b.mst_id and a.entry_form=2  and c.entry_form=53 and b.grey_sys_id=a.id and a.status_active=1 and a.is_deleted=0 and
					b.status_active=1 and b.is_deleted=0  and a.booking_without_order=1 and a.booking_id=$boking_id
					group by a.recv_number, a.receive_date, a.receive_basis, a.booking_no, a.knitting_source, a.challan_no, a.knitting_company, b.product_id";
                   ...
                   $result=sql_select($sql);
                    ...
                    ?>
                        <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
                            ...
                            <td width="110"><p><? echo $product_arr[$row[csf('product_id')]]; ?></p></td>
             ...
             <div style="width:990px; max-height:330px; overflow-y:scroll" id="scroll_body">
				...
				{	
					...
					foreach($sql_grey_booking as $val)
					{
						...
						b.machine_no_id, b.prod_id, sum(b.grey_receive_qnty) as quantity 
						from inv_receive_master a, pro_grey_prod_entry_dtls b
						where a.id=b.mst_id and a.booking_without_order=1 and a.id in (".$val[csf('id')].") and 
			...
			<table border="1" class="rpt_table" rules="all" width="950" cellpadding="0" cellspacing="0">
             ...
             <div style="width:977px; max-height:320px; overflow-y:scroll" id="scroll_body">
                 <table border="1" class="rpt_table" rules="all" width="950" cellpadding="0" cellspacing="0">
                    ...
                    {
                    	$sql="select e.grey_sys_number, e.determination_id, sum(f.qnty) as delivery_qty, a.receive_date,a.knitting_source,a.knitting_company from inv_receive_master a, pro_grey_prod_delivery_mst d, pro_grey_prod_delivery_dtls e, pro_roll_details f where d.id=e.mst_id and e.grey_sys_id=a.id and d.id=f.mst_id and e.id=f.dtls_id and f.barcode_no in ($barcode_ids)  and f.entry_form=67 and f.status_active=1 and f.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 group by  e.grey_sys_number, e.determination_id, a.receive_date, a.knitting_source, a.knitting_company";
                    }
