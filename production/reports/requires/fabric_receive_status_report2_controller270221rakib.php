...
{ 
 	...
 	if($template==1)
	{
		...
		{
			$jobIds_cond=" and job_id in ($poIds)";
		}
		...
		$yarn_sql="select job_no, count_id, copm_one_id, percent_one, copm_two_id, percent_two, type_id, sum(avg_cons_qnty) as qnty from wo_pre_cost_fab_yarn_cost_dtls where status_active=1 and is_deleted=0 $jobIds_cond group by job_no, count_id, copm_one_id, percent_one, copm_two_id, percent_two, type_id";
		...
		$sql_batch="select a.color_id, b.po_id, sum(b.batch_qnty) as batch_qnty from pro_batch_create_mst a, pro_batch_create_dtls b where a.id=b.mst_id and a.entry_form not in (36,74,17) and  a.batch_against not in(2) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $batch_po_cond group by a.color_id, b.po_id";
		//and a.entry_form!=36
...
if ($action=="yarn_issue_not_not_used") //Not used
{
    echo load_html_head_contents("Report Info", "../../../", 1, 1, '', '', '');
    extract($_REQUEST);
    $brand_array = return_library_array("select id, brand_name from lib_brand", "id", "brand_name");
			...
			<table border="1" class="rpt_table" rules="all" width="650" cellpadding="0" cellspacing="0">
             ...
             <div style="width:667px; max-height:320px; overflow-y:scroll" id="scroll_body">
                 <table border="1" class="rpt_table" rules="all" width="650" cellpadding="0" cellspacing="0">
                   ...
                   $sql="select a.batch_no, a.extention_no as ext_no, a.batch_date, a.color_id, sum(b.batch_qnty) as quantity from pro_batch_create_mst a, pro_batch_create_dtls b where a.id=b.mst_id and b.po_id in($ex_data[0]) and a.color_id='$ex_data[1]' and a.status_active=1 and a.entry_form not in (36,74,17)  and a.batch_against not in(2) and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id, a.batch_no, a.extention_no, a.batch_date, a.color_id";
                   //and a.entry_form!=36
                    $result=sql_select($sql);
        			foreach($result as $row)
                    {
