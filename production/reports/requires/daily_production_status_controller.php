...
/*	function return_field_value1( $field_name, $table_name, $query_cond, $return_fld_name, $new_conn )  // checked 3
{
	...
	*/	  
	...
	<fieldset style="width:650px;">
    	<label> <strong>Efficiency Summary:-</strong></label> 
       <table id="table_header_3" class="rpt_table" width="600" cellpadding="0" cellspacing="0" border="1" rules="all" style="background-color:#E3F2F1">
            <tr >
                <td width="200" >Last Week Factory Effciency</td>
            ...
            <tr >
                <td width="200" >Last Week Factory Perfomance</td>
...
<? 
	...
	if($prod_reso_allo[0]==1)
	{
		...
		foreach($dataArray_sql as $val)
		{
			...
			if(str_replace("'","",$cbo_no_prod_type)==1) // include no production line
			{
				$sewing_line_ids=$prod_reso_arr[$val[csf('id')]];
				$sl_ids_arr = explode(",", $sewing_line_ids);
				...
				$production_data_arr[$val[csf('floor_id')]][$val[csf('id')]]['prod_reso_allo']=1;
			}
			$prod_resource_array[$val[csf('id')]][$val[csf('pr_date')]]['man_power']=$val[csf('man_power')];
			$prod_resource_array[$val[csf('id')]][$val[csf('pr_date')]]['operator']=$val[csf('operator')];
			$prod_resource_array[$val[csf('id')]][$val[csf('pr_date')]]['helper']=$val[csf('helper')];
...
b.buyer_name  as buyer_name,b.style_ref_no,a.po_break_down_id, a.item_number_id,c.po_number as po_number,c.unit_price,c.file_no,c.grouping as ref,sum(a.production_quantity) as good_qnty,"; 
	...
	{
		$sql="select  a.company_id, a.location, a.floor_id, a.prod_reso_allo, a.production_date, a.sewing_line,b.buyer_name  as buyer_name,b.job_no,b.style_ref_no, a.po_break_down_id, a.item_number_id,c.po_number as po_number,c.file_no,c.unit_price,c.grouping as ref,sum(a.production_quantity) as good_qnty,a.remarks,"; 
		$first=1;
		for($h=$hour;$h<$last_hour;$h++)
		{
			$bg=$start_hour_arr[$h];
...
THEN production_quantity else 0 END) AS $prod_hour,";
	...
	foreach($production_serial_arr as $f_id=>$fname)
	{
		...
		//********************************************************************************************************************************************
			 ...
			 //***************************************************************************************************************************			  
				...
				$job_number=rtrim($production_data_arr[$f_id][$ldata]['job_no']);
				$job_number=implode(",",array_unique(explode(",",$job_number)));
	...
	//**************************** calclution total ***************************************************************************************
				...
				$html.='<td width="40">'.$i.'&nbsp;</td>
						...
						<td width="100"><p>'.$job_number.'&nbsp;</p></td>
						...
						<td align="right" width="60" style="mso-number-format:\@;">'.$item_smv.'</td>
       ...
       <table width="2200" cellpadding="0" cellspacing="0"> 
    	...
    	<?php $table_width=2400+($last_hour-$hour)*50; ?>
