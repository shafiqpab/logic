...
{
	$print_report_format_arr = return_library_array("select format_id,format_id from lib_report_template where template_name =$data and module_id=7 and report_id=71 and is_deleted=0 and status_active=1","format_id","format_id");
...
if($action=="report_generate")
{
	...
	if($cbo_company==0)
		...
		$cbo_company_cond=" and a.company_id=$cbo_company";
	...
	if($cbo_company==0)
		...
		$sub_company_cond=" and a.company_id=$cbo_company";
	...
	if($report_type==1)
	{
		$tbl_width=2320+count($shift_name)*205;
		...
		if($cbo_type==1 || $cbo_type==0)
		{
			...
			$job_data=sql_select("select c.job_no as sales_order,a.job_no, b.booking_no, $year_date,a.style_ref_no,a.insert_date from wo_po_details_master a,wo_booking_dtls b,fabric_sales_order_mst c where a.job_no=b.job_no and c.sales_booking_no=b.booking_no and a.company_name in($cbo_company_name,$cbo_working_company_id)");
			...
			$po_data=sql_select("select a.job_no, a.job_no_prefix_num, $year_field_sam as year, a.style_ref_no, b.id, b.po_number from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.company_name in($cbo_company_name,$cbo_working_company_id)");
						...
						<div style="width:660px; overflow-y:scroll; max-height:220px;" id="scroll_body">
							<table cellspacing="0" cellpadding="0" border="1" rules="all" width="640px" class="rpt_table" >
								<tbody>
									...
									. '<tr><th width="40">SL</th><th width="100">Buyer</th><th width="90">Inhouse</th><th width="90">Outbound-Subcon</th><th width="90">Sample Without Order</th><th width="100">Total</th></tr>'
									. '</thead></table>'
									. '<div style="width:570px; overflow-y:scroll; max-height:220px;" id="scroll_body">'
									. '<table cellspacing="0" cellpadding="0" border="1" rules="all" width="550px" class="rpt_table" >'
									. '<tbody>';
									//================================================================
									$sql_sample_sam="SELECT a.buyer_id, a.booking_no, sum(case when a.booking_without_order=1 and b.machine_no_id>0 $floor_id  then b.grey_receive_qnty end ) as sample_qty from inv_receive_master a, pro_grey_prod_entry_dtls b where a.entry_form=2 and a.item_category=13 and a.id=b.mst_id and a.receive_basis!=4 and a.buyer_id>0 $cbo_company_cond $company_working_cond and a.knitting_source like '$source'  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.booking_without_order in(1) $date_con $floor_id $buyer_cond group by a.buyer_id,a.booking_no ";
									// echo $sql_sample_sam;
									...
									$sql_sample_sam_out="SELECT a.buyer_id, a.booking_no, sum(case when a.booking_without_order=1 and b.machine_no_id=0 $floor_id  then b.grey_receive_qnty end ) as sample_qty from inv_receive_master a, pro_grey_prod_entry_dtls b where a.entry_form=2 and a.item_category=13 and a.id=b.mst_id and a.receive_basis!=4 $cbo_company_cond $company_working_cond and a.knitting_source=3  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.booking_without_order in(1) $date_con $floor_id $buyer_cond group by a.buyer_id,a.booking_no ";
									...
									foreach($sql_sample_samary_out as $inf)
									{
										...
										if($without_booking_no=='SMN')
										{
											$subcon_smn_buyer_samary[$inf[csf('buyer_id')]]+= $inf[csf('sample_qty')];
										}
										/*else if($without_booking_no=='SM')
										{
											$knit_buyer_samary[$row[csf('buyer_id')]]['with_qty']+= $row[csf('sample_qty')];
									...
									$sql_sample_sam_with="SELECT a.buyer_id, a.booking_no, sum(b.grey_receive_qnty) as with_ord_sample_qty from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c, wo_po_break_down e, wo_po_details_master f where a.id=b.mst_id and b.id=c.dtls_id and c.po_breakdown_id=e.id and e.job_no_mst=f.job_no and a.entry_form=2 and a.item_category=13 and c.entry_form=2 and a.buyer_id>0 and c.trans_type=1 and a.knitting_source=1 and a.booking_without_order=0 and b.machine_no_id>0 and a.receive_basis!=4 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and e.status_active=1 and e.is_deleted=0 $cbo_company_cond $company_working_cond $date_con $floor_id $buyer_cond $job_cond $order_cond $job_year_cond group by a.buyer_id,a.booking_no ";
									...
									//------------------------------------------07-09-2020 done by tofael----------------------------------------------------------

									// LC company wise variable check was there in details part  before 09-July-2020 which commented and picked up here to check in summary part also which is discussed with Mr. Mamun as he told whatever was done before here was ok don't need to change so summary and details query is syncronised with variable
									...
									/*if($roll_maintained_yesNo==1) // Yes
									{
										...
										$sql_qty.=" sum(case when a.knitting_source=1 then g.qnty else 0 end ) as qtyinhouse, sum(case when a.knitting_source=3 then g.qnty else 0 end ) as qtyoutbound from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c, lib_machine_name d, wo_po_break_down e,  wo_po_details_master f, pro_roll_details g
									...
									else // 2 No
									{
										...
										$sql_qty.=" sum(case when a.knitting_source=1 then c.quantity else 0 end ) as qtyinhouse, sum(case when a.knitting_source=3 then c.quantity else 0 end ) as qtyoutbound from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c, lib_machine_name d, wo_po_break_down e,  wo_po_details_master f
									...
									if($roll_maintained_yesNo==1) // Yes
									{
										...
										where c.po_breakdown_id=e.id and a.entry_form=2 and a.item_category=13 and a.id=b.mst_id and b.id=g.dtls_id and c.po_breakdown_id=g.po_breakdown_id and g.entry_form=2 and b.id=g.dtls_id and b.id=c.dtls_id and e.job_no_mst=f.job_no and c.entry_form=2 and c.trans_type=1 and a.knitting_source like '$source' and a.status_active=1 and a.is_deleted=0 and b.status_active=1  and a.receive_basis!=4 and b.is_deleted=0 and c.status_active=1 and c.is_sales=0 and c.is_deleted=0 and g.status_active=1 and g.is_deleted=0 $cbo_company_cond $company_working_cond $date_con $floor_id $buyer_cond $job_cond $order_cond $job_year_cond";
									}
									else // 2 No
									{
									...
									//SMN
									/*$sql_qty="SELECT a.buyer_id,a.booking_no, sum(case when a.knitting_source=1 and b.machine_no_id>0 $floor_id  then c.quantity end ) as qtyinhouse, sum(case when a.knitting_source=3 then c.quantity end ) as qtyoutbound
									from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c,wo_po_break_down e, wo_po_details_master f
									where a.id=b.mst_id and b.id=c.dtls_id and c.po_breakdown_id=e.id and e.job_no_mst=f.job_no and a.item_category=13 and a.buyer_id>0 and c.entry_form=2 and c.trans_type=1 and a.receive_basis!=4 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and e.status_active=1  and e.is_deleted=0 and a.knitting_source like '$source' $date_con $cbo_company_cond $company_working_cond $buyer_cond $job_cond $order_cond $job_year_cond and c.is_sales !=1
									group by a.buyer_id,a.booking_no";*/
									...
									foreach($knit_buyer_samary as $buyer_id=>$rows)
									{
										...
										$out_bound_qnty=0;
										$out_bound_qnty=$rows[('out_qty')]+$service_buyer_data[$buyer_id]+$subcon_smn_buyer_samary[$buyer_id];
										...
										<tr bgcolor="<? echo $bgcolor; ?>">
											...
											<td width="90" align="right" title="Qnty with SMN Outbound"><? echo number_format($with_out_qty,2,'.',''); $tot_summ=$rows[('in_qty')]+$out_bound_qnty+$with_out_qty+$with_qty; ?>&nbsp;</td>
										...
										<td width="100" align="right">'.  number_format($tot_summ,2,".","").'&nbsp;</td>
										</tr>';
						...
						<div style="width:280px; overflow-y:scroll; max-height:220px;" id="scroll_body">
							<table cellspacing="0" cellpadding="0" border="1" rules="all" width="260px" class="rpt_table" >
								<tbody>
									...
									and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0 $sub_company_cond $subcompany_working_cond $date_con_sub $floor_id_cond $buyer_id_cond $job_no_cond $order_no_cond $job_year_sub_cond
									group by a.party_id";*/
									$sql_inhouse_sub_summ="select a.party_id, sum(b.product_qnty) as qntysubshift
									from subcon_production_mst a, subcon_production_dtls b, lib_machine_name c, subcon_ord_dtls d, subcon_ord_mst e
						...
						<div style="width:280px; overflow-y:scroll; max-height:220px;" id="scroll_body">
							<table cellspacing="0" cellpadding="0" border="1" rules="all" width="260px" class="rpt_table" >
								<tbody>
									...
									where a.id=b.mst_id and b.id=d.dtls_id and d.po_breakdown_id=c.id and a.entry_form=2 and a.item_category=13 and a.knitting_source=1 and d.is_sales=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $cbo_company_cond $company_working_cond $date_con $floor_id $buyer_cond
									group by c.buyer_id ";
						...
						<div style="width:400px; overflow-y:scroll; max-height:220px;" id="scroll_body">
							<table cellspacing="0" cellpadding="0" border="1" rules="all" width="380px" class="rpt_table" >
								<tbody>
									...
									$total_prod_sammaryQty=$tot_qtyinhouse+$tot_qtyoutbound+$tot_with_ord_qty+$tot_without_ord_qty+$tot_qty_sub_summ+$tot_qty_sales_summ;
				...
				if($template==1)
				{
					...
					<div style="width:<? echo $tbl_width+460; ?>px; overflow-y:scroll; max-height:330px;" id="scroll_body">
						<table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $tbl_width+440; ?>" class="rpt_table" id="table_body">
							...
							//====================================================================================
							/*$sql_subcontract="select c.id,b.stitch_length, b.id, b.no_of_roll, a.recv_number_prefix_num, a.recv_number, a.receive_basis, a.knitting_source, a.knitting_company, a.receive_date, a.booking_id, a.booking_no, a.buyer_id, a.remarks,a.inserted_by,a.insert_date, b.prod_id, b.febric_description_id, b.gsm, b.width,  b.yarn_lot, b.yarn_count, b.brand_id, b.machine_no_id, b.machine_gg,b.machine_dia, b.floor_id $select_color, c.po_breakdown_id,  f.job_no_prefix_num, $year_field as year, e.po_number,e.file_no,e.grouping, sum(c.quantity) as outqntyshift,sum(c.quantity_pcs ) as outpcsshift
							from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c, wo_po_break_down e, wo_po_details_master f
							where c.po_breakdown_id=e.id and a.knitting_source=3 and a.item_category=13 and a.id=b.mst_id and b.id=c.dtls_id and e.job_no_mst=f.job_no and c.entry_form=2 and c.trans_type=1 and a.knitting_source=3 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_sales=0 and c.is_deleted=0 and a.receive_basis!=4 $cbo_company_cond $company_working_cond $date_con $buyer_cond $job_cond $order_cond $job_year_cond
							...
							from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c, wo_po_break_down e, wo_po_details_master f
							where c.po_breakdown_id=e.id and a.receive_basis=11 and a.item_category=13 and a.id=b.mst_id and b.id=c.dtls_id and e.job_no_mst=f.job_no and a.entry_form=22 and c.entry_form=22 and c.trans_type=1 and b.trans_id>0 and c.is_sales=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $cbo_company_cond $company_working_cond $date_con $buyer_cond $job_cond $order_cond $job_year_cond
							group by a.recv_number,b.stitch_length, a.receive_basis, a.receive_date, a.booking_id, b.prod_id, b.floor_id, b.yarn_lot, b.yarn_count, b.brand_id $group_color, f.job_no_prefix_num, f.insert_date, e.po_number,e.file_no,e.grouping, c.id, b.id, b.no_of_roll, a.recv_number_prefix_num, a.knitting_source, a.knitting_company, a.booking_no, a.buyer_id, a.remarks,a.inserted_by,a.insert_date, b.febric_description_id, b.gsm, b.width, b.machine_no_id, b.machine_gg,b.machine_dia, c.po_breakdown_id order by b.floor_id,a.receive_date";
							...
							//=====================================================================
							$sql_wout_order_smn="select b.id, a.recv_number_prefix_num, a.recv_number, a.receive_basis, a.knitting_source, a.knitting_company, a.receive_date, a.booking_id, a.booking_no, a.buyer_id, a.remarks,a.inserted_by,a.insert_date, b.prod_id, b.febric_description_id, b.gsm, b.width,  b.yarn_lot, b.yarn_count, b.stitch_length,b.brand_id, b.machine_no_id, b.machine_gg,b.machine_dia, b.floor_id $select_color, 0 as po_breakdown_id, 0 as machine_name, '' as job_no_mst, '' po_number, 0 as qntynoshift, sum(case when b.shift_name=0 then  b.no_of_roll end ) as rollnoshift	";
							if($roll_maintained_yesNo==1) // Yes
							{
								foreach($shift_name as $key=>$val)
								{
							...
							if (count($nameArray_inhouse)>0)
							{
								?>
								<tr  bgcolor="#CCCCCC">
									<td colspan="44" align="left" ><b>In-House</b></td>
								</tr>
								<?
							...
							if(count($nameArray_subcontract)>0)
							{
								?>
								<tr  bgcolor="#CCCCCC">
									<td colspan="44" align="left"><b>Outbound-Subcontract Production</b></td>
								</tr>
									...
									if(count($nameArray_service_receive)>0)
									{
										...
										foreach ($nameArray_service_receive as $row)
										{
											...
											if (count($nameArray_without_order)>0)
											{
												?>
												<tr  bgcolor="#CCCCCC">
													<td colspan="44" align="left" ><b>Sample Without Order</b></td>
												</tr>
												<?
												$html.="<tr  bgcolor='#CCCCCC'>
											...
											if (count($nameArray_without_order_smn)>0)
											{
												?>
												<tr  bgcolor="#CCCCCC">
													<td colspan="44" align="left" ><b>Sample Without Order Outbound</b></td>
												</tr>
												<?
												$html.="<tr  bgcolor='#CCCCCC'>
												...
												foreach ($nameArray_without_order_smn as $row)
												{
													...
													<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
														...
														<tr class="tbl_bottom">
															<td colspan="26" align="right"><b> Sample Without Order Outbound Total</b></td>
															<?
		...
		else if ($machine_wise_section==1)
		{
			...
			<fieldset style=" width:1050px;">
				...
				<table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center" id="summary_tab">
					...
					$machine_data=sql_select("select machine_entry_tbl_id, from_hour, from_minute, to_hour, to_minute, machine_idle_cause from  pro_cause_of_machine_idle where status_active=1 and machine_entry_tbl_id in(1,".implode(",", array_filter($machine_name_arr)).") and from_date between '$from_date' and '$machineTo_date' and to_date between '$from_date' and '$machineTo_date'  ");
					...
					foreach($machine_data as $row)
					{
						if($row[csf('from_hour')]!='')
						{
							...
							foreach ($shift_name as $key=>$shift)
							{
								...
								if($key != 3 && strtotime($from_date) == strtotime($row[csf('to_date')]))
								{
									if(strtotime($fromtime)>=$shift_from && strtotime($totime)<=$shift_to)
									{
										$machine_idle_cause[$machine_no][$key]['machine_idle_cause'][]=$row[csf('machine_idle_cause')];
	...
	else if($report_type==2) //Machine Wise
	{
		...
		if($cbo_type==1 || $cbo_type==0)
		{
			...
			if($cbo_company!=0) $lib_mc_cond.=" and company_id=$cbo_company";
							...
							if (count($nameArray_inhouse)>0)
							{
								...
								foreach ($nameArray_inhouse as $row)
								{
									...
									<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" valign="top">
										...
										<td width="70" align="center" title="<? if($within_group_id==2) echo "WithinGroup=No";else echo " ";?>"><p><? 
										if($within_group_id==2) echo " ";
										else echo implode(",",array_unique(explode(",",$row[csf('file_no')]))); ?></p></td>
										...
										<td width="80" align="center" style="max-width:80px"><P><? echo implode(",",array_unique(explode(",",$row[csf('yarn_lot')]))); ?>&nbsp;</P>
										</td>
										<td width="100"><P>
											<?
										...
										<td width="100"><P>
											...
											?>&nbsp;</P></td>
											<td width="60"  align="center" style="max-width:60px"><p><? echo  implode(",",array_unique(explode(",",$row[csf('stitch_length')]))); ?>&nbsp;</p></td>
	...
	else if($report_type==3) //Machine Wise 2
	{
		...
		if($cbo_type==1 || $cbo_type==0)
		{
			...
			unset($data_array);
			/*$booking_data=sql_select("select a.booking_type,a.booking_no  from wo_booking_mst a,wo_booking_dtls b where a.booking_no=b.booking_no and b.status_active=1 $cbo_company_cond");
		...
		<table cellpadding="0" cellspacing="0" width="<? echo $tbl_width; ?>">
			...
			<tr>
				<td align="center" width="100%" colspan="<? echo $ship_count+16; ?>" class="form_caption" style="font-size:16px"><? echo $company_arr[str_replace("'","",$cbo_company_name)]; ?></td>
			</tr>
			<tr>
		...
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1300px">
			...
			$sql_sample_sam_out="SELECT a.buyer_id, a.booking_no,a.floor, sum(case when a.booking_without_order=1 and b.machine_no_id=0 $floor_id  then b.grey_receive_qnty end ) as sample_qty from inv_receive_master a, pro_grey_prod_entry_dtls b where a.entry_form=2 and a.item_category=13 and a.id=b.mst_id and a.receive_basis!=4 $cbo_company_cond $company_working_cond and a.knitting_source=3  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.booking_without_order in(1) $date_con $floor_id $buyer_cond group by a.buyer_id,a.booking_no,a.floor ";
			// echo $sql_sample_sam_out;
			$sql_sample_samary_out=sql_select( $sql_sample_sam_out);
			// $subcon_smn_buyer_samary=array();
...
if($action=="report_generate_today")
{
	...
	if($cbo_company==0) $company_name_cond=""; else $company_name_cond=" and c.company_id=$cbo_company";
	...
	<fieldset style="width:<? echo $tbl_width+20; ?>px;">
		...
		if($cbo_type==1 || $cbo_type==0)
		{
			...
			if($cbo_type==2 || $cbo_type==0)
			{
				...
				$sql_inhouse_sub.="
				from subcon_ord_mst a, subcon_ord_dtls b, subcon_production_mst c, subcon_production_dtls d where c.company_id=$cbo_company_name and c.company_id=$cbo_working_company_id and a.subcon_job=b.job_no_mst and c.id=d.mst_id $job_ord_cond and c.product_type=2 $job_year_sub_cond $date_con_sub group by a.job_no_prefix_num, a.party_id, a.insert_date, d.order_id, b.order_no, b.cust_style_ref, d.cons_comp_id, d.gsm, d.dia_width, d.yarn_lot, d.stitch_len, d.color_id order by a.job_no_prefix_num DESC ";
