...
if($action=="party_popup")
{
	...
	$sql="select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name";
...
if($action=="report_generate")
{
	...
	if($type==3) //gmt prod sewing
	{
        ...
        foreach($tpd_data_arr as $row)
        {
		 ...
		 $dtls_sql="SELECT (b.po_quantity*c.total_set_qnty) as po_quantity, a.production_date, a.po_break_down_id as po_breakdown_id, a.item_number_id,c.buyer_name,a.company_id,c.id as job_id,
					...
					where a.po_break_down_id=b.id and b.job_id=c.id $location_con $cbo_company_cond_4 $company_working_cond_1 and a.production_date between '$date_from' and '$date_to' and a.is_deleted=0 and a.status_active=1 and b.status_active in(1,3) and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by b.po_quantity, c.total_set_qnty, a.production_date, a.po_break_down_id, a.item_number_id,c.buyer_name,a.company_id,c.id  order by a.production_date asc";
			...
			$job_id_arr = array();
			$po_id_arr = array();
			foreach($dtls_sql_result as $v)
			{
				$job_id_arr[$v['JOB_ID']] = $v['JOB_ID'];
				$po_id_arr[$v['PO_BREAKDOWN_ID']] = $v['PO_BREAKDOWN_ID'];
			}
			...
			if($smv_source==3)
			{
				$sql_item="SELECT b.id, a.sam_style, a.gmts_item_id from ppl_gsd_entry_mst a, wo_po_break_down b where b.job_no_mst=a.po_job_no and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active in(1,3) $po_id_cond";
			...
			else
			{
				...
				$sql_item="SELECT b.id, a.set_break_down, c.gmts_item_id, c.set_item_ratio, c.smv_pcs, c.smv_pcs_precost from wo_po_details_master a, wo_po_break_down b, wo_po_details_mas_set_details c where a.id=b.job_id and b.job_id=c.job_id $cbo_company_cond_2 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active in(1,3) $po_id_cond";
				// echo $sql_item;die;
				$resultItem=sql_select($sql_item);
				foreach($resultItem as $itemData)
				{
			...
			// $job_id_cond = where_con_using_array('job_id',0,$job_id_arr);
			...
			$sql = "SELECT job_no, cm_cost,FABRIC_COST,TRIMS_COST from wo_pre_cost_dtls where status_active=1 $job_id_cond ";
			$res = sql_select($sql);
			$fav_trims_cost_arr = array();
			foreach ($res as $v) 
			{
				$tot_cost_arr[$v['JOB_NO']] = $v['CM_COST'];
				$fav_trims_cost_arr[$v['JOB_NO']]['fab_cost'] = $v['FABRIC_COST'];
				$fav_trims_cost_arr[$v['JOB_NO']]['trims_cost'] = $v['TRIMS_COST'];
			}

			$fin_fab_avg_cons_arr = return_library_array("select job_no, AVG_FINISH_CONS from WO_PRE_COST_FABRIC_COST_DTLS where status_active=1 $job_id_cond ","job_no","AVG_FINISH_CONS");
			$acc_avg_cons_arr = return_library_array("select job_no, CONS_DZN_GMTS from WO_PRE_COST_TRIM_COST_DTLS where status_active=1 $job_id_cond ","job_no","CONS_DZN_GMTS");
			$acc_avg_rate_arr = return_library_array("select job_no, RATE from WO_PRE_COST_TRIM_COST_DTLS where status_active=1 $job_id_cond ","job_no","RATE");
			 ...
			 { 
				if(!array_key_exists( $row[csf("po_breakdown_id")],$po_id_array)){
					$job_array_summary[$row[csf("company_id")]][$row[csf("buyer_name")]]['po_qty']+=$row[csf("po_quantity")]; 
				...
				else
				{
				 ...
				 $cm_value=0; $cm_value_in=0; $cm_value_out=0; $sewing_qty_in=0; $sewing_qty_out=0;$sew_fab_used_amt = 0;$sew_acc_used_amt = 0;
				...
				$fin_fab_avg_cons_pcs = $fin_fab_avg_cons_arr[$job_no]/$dzn_qnty;
				// echo $fin_fab_avg_cons_arr[$job_no]."/".$dzn_qnty."<br>";
				$acc_avg_rate_pcs = $acc_avg_rate_arr[$job_no]/$acc_avg_cons_arr[$job_no];
				$sew_fab_used_amt = ($fav_trims_cost_arr[$job_no]['fab_cost']/$fin_fab_avg_cons_arr[$job_no])*$fin_fab_avg_cons_pcs*($sewing_qty_in+$sewing_qty_out);
				$sew_acc_used_amt = $acc_avg_rate_pcs*($sewing_qty_in+$sewing_qty_out);
				
				$prod_date_buyer_wise[$row[csf("company_id")]][$row[csf("buyer_name")]]['sew_fab_used_amt']+=$sew_fab_used_amt;
				$prod_date_buyer_wise[$row[csf("company_id")]][$row[csf("buyer_name")]]['sew_acc_used_amt']+=$sew_acc_used_amt;
			}
			...
			foreach($exfactory_res as $ex_row)
			{
				...
				$fin_fab_avg_cons_pcs = $fin_fab_avg_cons_arr[$job_no_ex]/$dzn_qnty;
				// echo $fin_fab_avg_cons_arr[$job_no_ex]."/".$dzn_qnty."<br>";
				$acc_avg_rate_pcs = $acc_avg_rate_arr[$job_no_ex]/$acc_avg_cons_arr[$job_no_ex];
				$exf_fab_used_amt = ($fav_trims_cost_arr[$job_no_ex]['fab_cost']/$fin_fab_avg_cons_arr[$job_no])*$fin_fab_avg_cons_pcs*$ex_row[csf("ex_factory_qnty")];
				$exf_acc_used_amt = $acc_avg_rate_pcs*$ex_row[csf("ex_factory_qnty")];
				
				$prod_date_buyer_wise[$ex_row[csf("company_name")]][$ex_row[csf("buyer_name")]]['exf_fab_used_amt']+=$exf_fab_used_amt;
				$prod_date_buyer_wise[$ex_row[csf("company_name")]][$ex_row[csf("buyer_name")]]['exf_acc_used_amt']+=$exf_acc_used_amt;
		...
		?>
        <table width="1700px " border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
            ...
            <thead>
            	<tr>
                   ...
                   <!-- <th width="100">Sew Output Target</th>-->
                    ...
                    <th width="100">Sewing Fab Used Amount</th>
                    <th width="100">Sewing Acce. Used Amount</th>
                    <th width="100">Sewing FOB Value</th>
                    <th width="100">Ex Factory</th>
                    <th width="100">Ex Factory CM Value</th>
                    <th width="100">Exfactory Fab Used Amount</th>
                    <th width="100">Ex-Factory Acce. Used Amount</th>
                    <th width="100">Ex Factory FOB Value</th>
                    <th>Sew Out to Ship Bal</th>
                </tr>
            </thead>
        </table>
        <div style="max-height:420px; overflow-y:scroll; width:1720px" id="scroll_body_summery">
       	<table cellspacing="0" border="1" class="rpt_table"  width="1700px" rules="all" id="scroll_body_summery" >
        <?
        // echo "<pre>";
        // print_r($prod_date);
        // echo "</pre>";
		...
		/*for($jk=0;$jk<$datediff;$jk++)
			...
			foreach($prod_date_buyer_wise as $companyKey =>$comp_value)
			{
				...
				<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('trs_<? echo $inc; ?>','<? echo $bgcolor; ?>')" id="trs_<? echo $inc; ?>">
					...
					<td align="right" width="100"><?=number_format($buyer_value['sew_fab_used_amt'],2);?></td>
					<td align="right" width="100"><?=number_format($buyer_value['sew_acc_used_amt'],2);?></td>
                   ...
                   <?php /*?> <td width="100" align="right"><? $exfactory_unitPrice= $prod_date_buyer_wise[$companyKey][$buyerKey]['ex_factory_smry_unitPrice']*$prod_date_buyer_wise[$companyKey][$buyerKey]['ex_factory_smry']; echo number_format($exfactory_unitPrice,2); $smry_tot_ex_factory_fob_val+=$exfactory_unitPrice; ?></td><?php */?>
				   	...
				   	<td align="right" width="100"><?=number_format($buyer_value['exf_fab_used_amt'],2);?></td>
					<td align="right" width="100"><?=number_format($buyer_value['exf_acc_used_amt'],2);?></td>
			...
			<?
			$smry_tot_sew_fab_used_amt += $buyer_value['sew_fab_used_amt'];
			$smry_tot_sew_acc_used_amt += $buyer_value['sew_acc_used_amt'];
			$smry_tot_exf_fab_used_amt += $buyer_value['exf_fab_used_amt'];
			$smry_tot_exf_acc_used_amt += $buyer_value['exf_acc_used_amt'];
			$inc++;
			}
		}
         ...
         <tfoot>
            <tr>
               ...
               <?php /*?> <th width="100"><? echo number_format($smry_tot_sewing_qnty,2); ?></th><?php */?>
				...
				<th width="100"><? echo number_format($smry_tot_sew_fab_used_amt,2); ?></th>
				<th width="100"><? echo number_format($smry_tot_sew_acc_used_amt,2); ?></th>
				...
				<th width="100"><? echo number_format($smry_tot_exf_fab_used_amt,2); ?></th>
				<th width="100"><? echo number_format($smry_tot_exf_acc_used_amt,2); ?></th>
        ...
        <p style="float:left"><strong>Details Part</strong></p>
            <thead>
            	...
            	<tr>
                    <th width="80" rowspan="3" valign="middle">Knitting </th>
	    ...
	    <table width="2410px " border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
	        <tfoot>
	            ...
	            <tr>
	                ...
	                <th width="90">Avg.</th>
	                <th width="80"><? 
					//echo number_format($total_knit/$count_knit,2); 
						$cv=$total_knit/$count_knit;
	...
	{
		$cbo_company=str_replace("'","",$cbo_company_id);
		$cbo_work_company=str_replace("'","",$cbo_working_company_id);
		$cbo_location=str_replace("'","",$cbo_location);
		$date_from=str_replace("'","",$txt_date_from);
		$date_to=str_replace("'","",$txt_date_to);
		if($cbo_company>0) $com_cond=" and a.company_id in($cbo_company)"; else $com_cond="";
		if($cbo_work_company>0) $work_com_cond=" and a.company_id in($cbo_work_company)"; else $work_com_cond="";
		if($cbo_work_company>0) $work_com_cond2=" and a.working_company_id in($cbo_work_company)"; else $work_com_cond2="";
		if($cbo_location>0) $loc_cond=" and a.location_id=$cbo_location"; else $loc_cond="";
		if($date_from!="" && $date_to!="") $date_cond=" and a.receive_date between '$date_from' and '$date_to'"; else $date_cond="";	

		$costing_per_arr = return_library_array("select job_no, costing_per from wo_pre_cost_mst","job_no","costing_per"); 
		$tot_cost_arr = return_library_array("select job_no, cm_cost from wo_pre_cost_dtls","job_no","cm_cost"); 

		$sql_day_target=sql_select("select target_date, sum(target_qnty) as qnty from  tna_grey_fab_target where status_active=1 and target_date between '$date_from' and '$date_to'  group by target_date");
		$target_qty_arr=array();
		foreach($sql_day_target as $row)
		{
			$target_qty_arr[change_date_format($row[csf("target_date")])]=$row[csf("qnty")];
		}
		
		if(str_replace("'", "", $cbo_location) !=0)
		{
			$loc=" and a.location_id= $cbo_location ";
		}
		
		if($cbo_company>0)
			$cbo_company_cond_1="and a.company_id in($cbo_company)";
		else
			$cbo_company_cond_1="and a.knitting_company in($cbo_working_company) $loc";
		
		$knitting_prod_sql=sql_select("select a.receive_date,
			sum(case when a.knitting_source=1 then b.grey_receive_qnty else 0 end) as knit_receive_inside,
			sum(case when a.knitting_source=3 then b.grey_receive_qnty else 0 end) as knit_receive_outside
			from inv_receive_master a, pro_grey_prod_entry_dtls b
			where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form=2 $cbo_company_cond_1 $date_cond 
		...
		// tommorow work
		if($cbo_company>0)
			$cbo_company_cond_2="and a.company_id in($cbo_company)";
		else
			$cbo_company_cond_2="and a.knit_dye_company in($cbo_working_company) $loc ";
		//$sql_grey_delivery=sql_select("select a.issue_date, sum(b.issue_qnty) as current_delivery from inv_issue_master a, inv_grey_fabric_issue_dtls b where a.id=b.mst_id and a.entry_form in(16,61) and a.issue_date between '$date_from' and '$date_to' $cbo_company_cond_2  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.issue_date order by a.issue_date");
		if($cbo_company>0)
		{
			$grey_comp_cond.=" and a.company_id in($cbo_company) ";
		}
		if($cbo_working_company>0)
		{
			$grey_comp_cond.=" and a.knitting_company in($cbo_working_company) ";
		}

		$sqls = "select  sum(b.current_delivery) current_delivery, a.delevery_date as issue_date from pro_grey_prod_delivery_mst a,pro_grey_prod_delivery_dtls b where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and a.is_deleted=0 and b.is_deleted=0 and a.entry_form=56 and a.delevery_date between '$date_from' and '$date_to' $grey_comp_cond group by a.delevery_date";

		$sql_grey_delivery=sql_select($sqls);
 		$grey_delivery_arr=array();
		foreach($sql_grey_delivery as $row)
		{
			$grey_delivery_arr[change_date_format($row[csf("issue_date")])]['current_delivery']=$row[csf("current_delivery")];
		}
		
		$sql_batch=sql_select("SELECT a.batch_date, sum(b.batch_qnty) as batch_qnty from pro_batch_create_mst a, pro_batch_create_dtls b where a.id=b.mst_id and a.batch_date between '$date_from' and '$date_to' $com_cond $work_com_cond2  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form=0 and a.batch_against!=3  group by a.batch_date order by a.batch_date");
		$batch_arr=array();
		foreach($sql_batch as $row)
		{
			$batch_arr[change_date_format($row[csf("batch_date")])]['batch_qnty']=$row[csf("batch_qnty")];
		}
		
		if($cbo_company>0)
			$cbo_company_cond_3="and a.company_id in($cbo_company)";
		else
			$cbo_company_cond_3="and a.service_company in($cbo_working_company)";
			
		//$sql_dyeing=sql_select("select a.process_end_date, sum(c.batch_qnty) as dyeing_qnty from pro_fab_subprocess a, pro_batch_create_mst b, pro_batch_create_dtls c where a.batch_id=b.id and b.id=c.mst_id and a.process_end_date between '$date_from' and '$date_to' $cbo_company_cond_3  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.entry_form=35 and a.load_unload_id=2  group by a.process_end_date order by a.process_end_date"); BATCH_WEIGHT

		$sql_dyeing=sql_select("
			select
				a.process_end_date, sum(b.batch_weight) as dyeing_qnty 
			from pro_fab_subprocess a,
				pro_batch_create_mst b
				--,pro_batch_create_dtls c 
			where
				a.batch_id=b.id
				--and b.id=c.mst_id
				and a.process_end_date between '$date_from' and '$date_to'
				$cbo_company_cond_3
		...
		{
			$dyeing_arr[change_date_format($row[csf("process_end_date")])]['dyeing_qnty']=$row[csf("dyeing_qnty")];
		}
		...
		$dyeingInBoundSql="
			SELECT
				a.process_end_date,
				--SUM(c.batch_qnty) AS inbound_qty
				SUM(b.batch_weight) AS inbound_qty
			FROM
				pro_fab_subprocess a
				INNER JOIN pro_batch_create_mst b ON a.batch_id = b.id
				--INNER JOIN pro_batch_create_dtls c ON b.id = c.mst_id
			WHERE
				a.status_active = 1 
				AND a.is_deleted = 0
				AND a.load_unload_id = 2
			...
			ORDER BY
				a.process_end_date
		";
		//echo $dyeingInBoundSql;
		$dyeingInBoundSqlResult=sql_select($dyeingInBoundSql);
		foreach($dyeingInBoundSqlResult as $row)
		{
			$dyeing_arr[change_date_format($row[csf("process_end_date")])]['inbound_qty']=$row[csf("inbound_qty")];
		}
		//end
		/*
		echo "select a.receive_date,
			sum(case when a.knitting_source=1 then b.receive_qnty else 0 end) as fin_receive_inside,
			sum(case when a.knitting_source=3 then b.receive_qnty else 0 end) as fin_receive_outside
			from inv_receive_master a, pro_finish_fabric_rcv_dtls b
			where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form=7 $cbo_company_cond_1 $date_cond
			group by a.receive_date
			order by a.receive_date";
		*/
		$finishing_prod_sql=sql_select("select a.receive_date,
			sum(case when a.knitting_source=1 then b.receive_qnty else 0 end) as fin_receive_inside,
			sum(case when a.knitting_source=3 then b.receive_qnty else 0 end) as fin_receive_outside
			from inv_receive_master a, pro_finish_fabric_rcv_dtls b
			where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form=7 $cbo_company_cond_1 $date_cond
			group by a.receive_date
			order by a.receive_date");
		//echo $knitting_prod_sql;
		$finishing_arr=array();
		foreach($finishing_prod_sql as $row)
		{
			$finishing_arr[change_date_format($row[csf("receive_date")])]['fin_receive_inside']=$row[csf("fin_receive_inside")];
			$finishing_arr[change_date_format($row[csf("receive_date")])]['fin_receive_outside']=$row[csf("fin_receive_outside")];
		}
		
		$sql_finish_delivery=sql_select("select a.issue_date, sum(b.issue_qnty) as current_delivery from inv_issue_master a,  inv_finish_fabric_issue_dtls b where a.id=b.mst_id and a.entry_form=18 and a.issue_date between '$date_from' and '$date_to' $cbo_company_cond_2  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.issue_date order by a.issue_date");
		
		$finish_delivery_arr=array();
		foreach($sql_finish_delivery as $row)
		{
			$finish_delivery_arr[change_date_format($row[csf("issue_date")])]['current_delivery']=$row[csf("current_delivery")];
		}
		
		//echo $cbo_location;die;
		$kintting_arr=array();
		foreach($knitting_prod_sql as $row)
		{
			$kintting_arr[change_date_format($row[csf("receive_date")])]['knit_receive_inside']=$row[csf("knit_receive_inside")];
			$kintting_arr[change_date_format($row[csf("receive_date")])]['knit_receive_outside']=$row[csf("knit_receive_outside")];
		}
		
		//for kintting in-bound
		$kinttingInBoundSql="
			SELECT
				a.product_date,
				--SUM(CASE WHEN a.knitting_source=1 AND a.entry_form = 159 THEN b.product_qnty ELSE 0 END) AS knitting_inbound_qty,
				--SUM(CASE WHEN a.knitting_source=1 AND a.entry_form = 292 THEN b.product_qnty ELSE 0 END) AS finishing_inbound_qty
				SUM(CASE WHEN a.entry_form = 159 THEN b.product_qnty ELSE 0 END) AS knitting_inbound_qty,
				SUM(CASE WHEN a.entry_form = 292 THEN b.product_qnty ELSE 0 END) AS finishing_inbound_qty
 			FROM
			...
			WHERE
				a.status_active = 1 
				AND a.is_deleted = 0
				AND a.entry_form IN(159, 292)
				AND b.status_active = 1 
				AND b.is_deleted = 0 
				".$cbo_company_cond_1." 
				AND a.product_date BETWEEN '".$date_from."' AND '".$date_to."'
			GROUP BY
				a.product_date
			ORDER BY
				a.product_date
		";
		//echo $kinttingInBoundSql;
		$kinttingInBoundSqlResult=sql_select($kinttingInBoundSql);
		foreach($kinttingInBoundSqlResult as $row)
		{
			$kintting_arr[change_date_format($row[csf("product_date")])]['inbound_qty'] = $row[csf("knitting_inbound_qty")];
			$finishing_arr[change_date_format($row[csf("product_date")])]['inbound_qty'] = $row[csf("finishing_inbound_qty")];
		}
		?>
        <table width="1590px" cellpadding="0" cellspacing="0" id="caption" align="center">
            <tr>
            <td align="center" width="100%" colspan="17" class="form_caption"><strong style="font-size:18px"><? if($cbo_company==0){ echo "Working Company Name:". $company_library[$cbo_working_company];} else{
				$com_arr=explode(",",str_replace("'","",$cbo_company));
	  			$comName="";
	  			foreach($com_arr as $comID)
	  			{
            ...
            <tr>  
               <td align="center" width="100%" colspan="17" class="form_caption" ><strong style="font-size:14px"><? echo $report_title; ?></strong></td>
            </tr>
            <tr>  
               <td align="center" width="100%" colspan="17" class="form_caption" ><strong style="font-size:14px"> <? echo "From : ".change_date_format(str_replace("'","",$txt_date_from))." To : ".change_date_format(str_replace("'","",$txt_date_to))."" ;?></strong></td>
            </tr>
            <tr>  
               <td width="100%" colspan="17" style="color:#FF0000; font-weight:bold;">Note: Without Receive Return & Issue Return.</td>
            </tr>  
        </table>
        <table width="1590px " border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
            <thead>
            	<tr>
                    <th width="30" rowspan="2" valign="middle">SL</th>
                    <th width="100" rowspan="2" valign="middle">Production Date</th>
                    <th width="100" rowspan="2" valign="middle">Day Target</th>
                    <th colspan="4">Knitting Section Production</th>
                    <th width="90" rowspan="2" valign="middle">Grey Delivery</th>
                    <th width="90" rowspan="2" valign="middle">Batching Done</th>
                    <th colspan="4">Dyeing Section Production</th>
                    <th colspan="4">Dyeing Finishing Production</th>
                    <th rowspan="2" valign="middle"  width="100">Fin Fab Delivery to Cut</th>
                </tr>
            	<tr>
                    <th width="90">In-House</th>
                    <th width="90">In-Bound</th>
                    <th width="90">Out-Bound</th>
                    <th width="90">Total</th>
                    <th width="90">In-House</th>
                    <th width="90">In-Bound</th>
                    <th width="90">Out-Bound</th>
        ...
        <div style="max-height:300px; overflow-y:scroll; width:1608px" id="scroll_body">
        <table cellspacing="0" border="1" class="rpt_table"  width="1590px" rules="all" id="scroll_body" >
        	<tbody>
            	<?
				$i=1;
				$kit_in=0;
				...
				for($j=0;$j<$datediff;$j++)
				{
					...
					$date_all=add_date(str_replace("'","",$txt_date_from),$j);
					$newdate =change_date_format($date_all);
					$tot_knit=$kintting_arr[$newdate]['knit_receive_inside']+$kintting_arr[$newdate]['inbound_qty']+$kintting_arr[$newdate]['knit_receive_outside'];
					$tot_dye=$dyeing_arr[$newdate]['dyeing_qnty']+$dyeing_arr[$newdate]['inbound_qty']+$finishing_arr[$newdate]['fin_receive_outside'];
					$tot_fin=$finishing_arr[$newdate]['fin_receive_inside']+$finishing_arr[$newdate]['inbound_qty']+$finishing_arr[$newdate]['fin_receive_outside'];
					?>
				<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                    	<td width="30"><? echo $i; ?></td>
                        <td width="100" align="center"><? echo $newdate; ?></td>
                        <td width="100" align="right">
						<? 
							echo number_format($target_qty_arr[$newdate],2); 
							$total_target_qnty+=$target_qty_arr[$newdate];
							if($target_qty_arr[$newdate]>0) $target_count++;   
						?>
                        </td>
                        <td width="90" align="right">
						<? 
							echo number_format($kintting_arr[$newdate]['knit_receive_inside'],2);
							$total_knit_inside+=$kintting_arr[$newdate]['knit_receive_inside'];
							if($kintting_arr[$newdate]['knit_receive_inside']>0) $kit_in++;   
						?>
                        </td>
                        <!-- for kintting in-bound-->
                        <td width="90" align="right">
						<? 
							echo number_format($kintting_arr[$newdate]['inbound_qty'],2);
							$total_knit_inbound+=$kintting_arr[$newdate]['inbound_qty'];
							if($kintting_arr[$newdate]['inbound_qty']>0) $kit_inbound++;   
						?>
                        </td>
                        <td width="90" align="right">
						<? 
							echo number_format($kintting_arr[$newdate]['knit_receive_outside'],2); 
							$total_knit_outside+=$kintting_arr[$newdate]['knit_receive_outside'];
							if($kintting_arr[$newdate]['knit_receive_outside']>0) $knit_out++;   
						?></td>
                        <td width="90" align="right">
						<? 
							echo number_format($tot_knit,2);
							$grand_tot_knit+=$tot_knit;
							if($tot_knit>0) $grand_knit++;   
						?>
                        </td>
                        <td width="90" align="right">
						<? 
							echo number_format($grey_delivery_arr[$newdate]['current_delivery'],2);
							$total_grey_delivery+=$grey_delivery_arr[$newdate]['current_delivery'];
							if($grey_delivery_arr[$newdate]['current_delivery']>0) $grey_del++;   
						?>
                        </td>
                        <td width="90" align="right">
						<? 
							echo number_format($batch_arr[$newdate]['batch_qnty'],2);
							$total_batch+=$batch_arr[$newdate]['batch_qnty'];
							if($batch_arr[$newdate]['batch_qnty']>0) $batch_count++;   
						?>
                        </td>
                        <td width="90" align="right">
						<? 
							echo number_format($dyeing_arr[$newdate]['dyeing_qnty'],2); 
							$total_dye_in+=$dyeing_arr[$newdate]['dyeing_qnty'];
							if($dyeing_arr[$newdate]['dyeing_qnty']>0) $dye_in++;   
						?>
                        </td>
                        <!-- for dyeing in-bound-->
                        <td width="90" align="right">
						<? 
							echo number_format($dyeing_arr[$newdate]['inbound_qty'],2); 
							$total_dye_inbound+=$dyeing_arr[$newdate]['inbound_qty'];
							if($dyeing_arr[$newdate]['inbound_qty']>0) $dye_inbound++;   
						?>
                        </td>
                        <td width="90" align="right">
						<? 
							echo number_format($finishing_arr[$newdate]['fin_receive_outside'],2); 
							$total_dye_out+=$finishing_arr[$newdate]['fin_receive_outside'];
							if($finishing_arr[$newdate]['fin_receive_outside']>0) $dye_out++;   
						?>
                        </td>
                        <td width="90" align="right">
						<? 
							echo number_format($tot_dye,2); 
							$grand_tot_dyeing+=$tot_dye;
							if($tot_dye>0) $grand_dye++;   
						?>
                        </td>
                        <td width="90" align="right">
						<? 
							echo number_format($finishing_arr[$newdate]['fin_receive_inside'],2);
							$total_finish_in+=$finishing_arr[$newdate]['fin_receive_inside'];
							if($finishing_arr[$newdate]['fin_receive_inside']>0) $fini_in++;   
						?></td>
                        <!-- for finishing in-bound-->
                        <td width="90" align="right">
						<? 
							echo number_format($finishing_arr[$newdate]['inbound_qty'],2);
							$total_finish_inbound+=$finishing_arr[$newdate]['inbound_qty'];
							if($finishing_arr[$newdate]['inbound_qty']>0) $fini_inbound++;   
						?></td>
                        <td width="90" align="right">
						<? 
							echo number_format($finishing_arr[$newdate]['fin_receive_outside'],2); 
							$total_finish_out+=$finishing_arr[$newdate]['fin_receive_outside'];
							if($finishing_arr[$newdate]['fin_receive_outside']>0) $fini_out++;   
						?>
                        </td>
                        <td width="90" align="right">
						<? 
							echo number_format($tot_fin,2); 
							$grand_tot_finish+=$tot_fin;
							if($tot_fin>0) $fin_tot_count++;   
						?>
                        </td>
                        <td align="right"  width="100">
						<? 
							echo number_format($finish_delivery_arr[$newdate]['current_delivery'],2);
							$total_finish_delivery+=$finish_delivery_arr[$newdate]['current_delivery'];
							if($finish_delivery_arr[$newdate]['current_delivery']>0) $fini_deve_count++;   
						?>
                        </td>
                    </tr>
                    <?
        ...
        <table width="1590px" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all">
        	<tfoot>
                <tr>
                    ...
                    <th width="100"><? echo number_format($total_target_qnty,2); ?></th>
                    <th width="90"><? echo number_format($total_knit_inside,2); ?></th>
                    <th width="90"><? echo number_format($total_knit_inbound,2); ?></th>
                    <th width="90"><? echo number_format($total_knit_outside,2); ?></th>
                    <th width="90"><? echo number_format($grand_tot_knit,2); ?></th>
                    <th width="90"><? echo number_format($total_grey_delivery,2); ?></th>
                    <th width="90"><? echo number_format($total_batch,2); ?></th>
                    <th width="90"><? echo number_format($total_dye_in,2); ?></th>
                    <th width="90"><? echo number_format($total_dye_inbound,2); ?></th>
                    <th width="90"><? echo number_format($total_dye_out,2); ?></th>
                    <th width="90"><? echo number_format($grand_tot_dyeing,2); ?></th>
                    <th width="90"><? echo number_format($total_finish_in,2); ?></th>
                    <th width="90"><? echo number_format($total_finish_inbound,2); ?></th>
                    <th width="90"><? echo number_format($total_finish_out,2); ?></th>
                    <th width="90"><? echo number_format($grand_tot_finish,2); ?></th>
                    <th width="100"><? echo number_format($total_finish_delivery,2); ?></th>
                </tr>
                <tr>
                    <th width="30"></th>
                    <th width="100">Average:</th>
                    <th width="100"><? 
					$target_avg=$total_target_qnty/$target_count; 
					if(is_infinite($target_avg) || is_nan($target_avg)){$target_avg=0;}
					echo number_format($target_avg,2); 
					?></th>
                    <th width="90"><? $kit_in_avg=$total_knit_inside/$kit_in; if(is_infinite($kit_in_avg) || is_nan($kit_in_avg)){$kit_in_avg=0;}echo number_format($kit_in_avg,2); ?></th>
                    <th width="90"><? $kit_inbound_avg=$total_knit_inbound/$kit_inbound; if(is_infinite($kit_inbound_avg) || is_nan($kit_inbound_avg)){$kit_inbound_avg=0;}echo number_format($kit_inbound_avg,2); ?></th>
                    <th width="90"><? $kit_out_avg=$total_knit_outside/$knit_out; if(is_infinite($kit_out_avg) || is_nan($kit_out_avg)){$kit_out_avg=0;} echo number_format($kit_out_avg,2); ?></th>
                    <th width="90"><? $grand_kint_avg=$grand_tot_knit/$grand_knit; if(is_infinite($grand_kint_avg) || is_nan($grand_kint_avg)){$grand_kint_avg=0;} echo number_format($grand_kint_avg,2); ?></th>
                    <th width="90"><? $grery_dev_avg=$total_grey_delivery/$grey_del;  if(is_infinite($grery_dev_avg) || is_nan($grery_dev_avg)){$grery_dev_avg=0;} echo number_format($grery_dev_avg,2); ?></th>
                    <th width="90"><? $batch_avg=$total_batch/$batch_count; if(is_infinite($batch_avg) || is_nan($batch_avg)){$batch_avg=0;} echo number_format($batch_avg,2); ?></th>
                    <th width="90"><? $dye_in_avg=$total_dye_in/$dye_in; if(is_infinite($dye_in_avg) || is_nan($dye_in_avg)){$dye_in_avg=0;} echo number_format($dye_in_avg,2); ?></th>
                    <th width="90"><? $dye_inbound_avg=$total_dye_inbound/$dye_inbound; if(is_infinite($dye_inbound_avg) || is_nan($dye_inbound_avg)){$dye_inbound_avg=0;} echo number_format($dye_inbound_avg,2); ?></th>
                    <th width="90"><? $dye_out_avg=$total_dye_out/$dye_out; if(is_infinite($dye_out_avg) || is_nan($dye_out_avg)){$dye_out_avg=0;} echo number_format($dye_out_avg,2); ?></th>
                    <th width="90"><? $grand_dye_avg=$grand_tot_dyeing/$grand_dye; if(is_infinite($grand_dye_avg) || is_nan($grand_dye_avg)){$grand_dye_avg=0;} echo number_format($grand_dye_avg,2); ?></th>
                    <th width="90"><? $fin_in_avg=$total_finish_in/$fini_in; if(is_infinite($fin_in_avg) || is_nan($fin_in_avg)){$fin_in_avg=0;} echo number_format($fin_in_avg,2); ?></th>
                    <th width="90"><? $fin_inbound_avg=$total_finish_inbound/$fini_inbound; if(is_infinite($fin_inbound_avg) || is_nan($fin_inbound_avg)){$fin_inbound_avg=0;} echo number_format($fin_inbound_avg,2); ?></th>
                    <th width="90"><? $fin_out_avg=$total_finish_out/$fini_out; if(is_infinite($fin_out_avg) || is_nan($fin_out_avg)){$fin_out_avg=0;} echo number_format($fin_out_avg,2); ?></th>
                    <th width="90"><? $garnd_fin_avg=$grand_tot_finish/$fin_tot_count;  if(is_infinite($garnd_fin_avg) || is_nan($garnd_fin_avg)){$garnd_fin_avg=0;} echo number_format($garnd_fin_avg,2); ?></th>
                    <th  width="100"><? $fin_delivery_avg=$total_finish_delivery/$fini_deve_count;  if(is_infinite($fin_delivery_avg) || is_nan($fin_delivery_avg)){$fin_delivery_avg=0;} echo number_format($fin_delivery_avg,2); ?></th>
                </tr>
            </tfoot> 
        </table>
        <?
	}
	else if($type==5)//On Finishing Qty
	{
	...
	else if($type==6) //Short Form  button
	{		
        ...
        <table width="2310px" cellpadding="0" cellspacing="0" id="caption" align="center">
            <tr>
            <td align="center" width="100%" colspan="22" class="form_caption"><strong style="font-size:18px">
			<? if($cbo_company==0){ echo "Working Company Name:". $company_library[$cbo_working_company];} else{ 
			$com_arr=explode(",",str_replace("'","",$cbo_company));
			$comName="";
			foreach($com_arr as $comID)
		...
		foreach($job_sql_result as $row)
		{
		 ...
		 if($cbo_working_company==0) $company_working_cond_1=""; else $company_working_cond_1=" and a.serving_company in($cbo_working_company)";
		  $dtls_sql="SELECT a.production_date, a.po_break_down_id as po_breakdown_id, a.item_number_id,c.buyer_name,a.company_id,
					sum(CASE WHEN a.production_type =1 $company_working_cond_1 THEN a.production_quantity END) AS cutting_qnty,
					sum(CASE WHEN a.production_type =1 and a.production_source=1 $company_working_cond_1 THEN a.production_quantity END) AS cutting_qnty_inhouse,
					sum(CASE WHEN a.production_type =1 and a.production_source=3 THEN a.production_quantity END) AS cutting_qnty_outbound, 
					
					sum(CASE WHEN a.production_type =3 and a.embel_name=1 $company_working_cond_1 THEN a.production_quantity END) AS printing_qnty,
					sum(CASE WHEN a.production_type =3 and a.embel_name=1 and a.production_source=1 $company_working_cond_1 THEN a.production_quantity END) AS printing_qnty_inhouse,
					sum(CASE WHEN a.production_type =3 and a.embel_name=1 and a.production_source=3 THEN a.production_quantity END) AS printing_qnty_outbound, 
					
					sum(CASE WHEN a.production_type =3 and a.embel_name=2 $company_working_cond_1 THEN a.production_quantity END) AS emb_qnty,
					sum(CASE WHEN a.production_type =3 and a.embel_name=2 and a.production_source=1 $company_working_cond_1 THEN a.production_quantity END) AS emb_qnty_inhouse,
					sum(CASE WHEN a.production_type =3 and a.embel_name=2 and a.production_source=3 THEN a.production_quantity END) AS emb_qnty_outbound,
					 
					sum(CASE WHEN a.production_type =5 $company_working_cond_1 THEN a.production_quantity END) AS sewing_qnty,
					sum(CASE WHEN a.production_type =5 and a.production_source=1 $company_working_cond_1 THEN a.production_quantity END) AS sewingout_qnty_inhouse,
					sum(CASE WHEN a.production_type =5 and a.production_source=3 THEN a.production_quantity END) AS sewingout_qnty_outbound, 
					
					 
					sum(CASE WHEN a.production_type =4 $company_working_cond_1 THEN a.production_quantity END) AS sewing_input_qnty,
					sum(CASE WHEN a.production_type =4 and a.production_source=1 $company_working_cond_1 THEN a.production_quantity END) AS sewing_input_qnty_inhouse,
					sum(CASE WHEN a.production_type =4 and a.production_source=3 THEN a.production_quantity END) AS sewing_input_qnty_outbound, 
					
					sum(CASE WHEN a.production_type =8 $company_working_cond_1 THEN a.production_quantity END) AS finish_qnty,
					sum(CASE WHEN a.production_type =8 and a.production_source=1 $company_working_cond_1 THEN a.production_quantity END) AS finish_qnty_inhouse, 
					sum(CASE WHEN a.production_type =8 and a.production_source=3 THEN a.production_quantity END) AS finish_qnty_outbound,
					sum(CASE WHEN a.production_type =8  THEN a.carton_qty END) AS carton_qty 
					from pro_garments_production_mst a, wo_po_break_down b,wo_po_details_master c 
					where a.po_break_down_id=b.id and c.job_no=b.job_no_mst  $location_con $cbo_company_cond_4 and a.production_date between '$date_from' and '$date_to' and a.is_deleted=0 and a.status_active=1 and b.status_active in(1,3) and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by a.production_date, a.po_break_down_id, a.item_number_id,c.buyer_name,a.company_id  order by a.production_date asc";
			//and a.production_date between '$date_from' and '$date_to'
			//and b.id in(29714,29715) 
			 //echo $dtls_sql;
			 $dtls_sql_result=sql_select($dtls_sql);
			 ...
			 foreach($dtls_sql_result as $row)
			 {
				...
				else
				{
				 ...
				 $prod_date[change_date_format($row[csf("production_date")])]['sewingout_value_outbound']+=$row[csf("sewingout_qnty_outbound")]*$job_array[$row[csf("po_breakdown_id")]]['unit_price'];
				  //$prod_date_buyer_wise[$row[csf("company_id")]][$row[csf("buyer_name")]]['sewing_fob_val_input_qnty']+=$row[csf("sewingout_qnty_inhouse")]*$job_array[$row[csf("po_breakdown_id")]]['unit_price'];
				  //$prod_date_buyer_wise[$row[csf("company_id")]][$row[csf("buyer_name")]]['sewingUnitprice']=$job_array[$row[csf("po_breakdown_id")]]['unit_price'];
				 ...
				 $prod_date[change_date_format($row[csf("production_date")])]['cm_value_in']+=$cm_value_in;
				 $prod_date[change_date_format($row[csf("production_date")])]['cm_value_out']+=$cm_value_out;
				// $prod_date_buyer_wise[$row[csf("company_id")]][$row[csf("buyer_name")]]['cm_value_in']+=$cm_value_in;
			}
			//print_r($prod_date_buyer_wise);
			 if(str_replace("'","",$cbo_location)==0){$location_con="";}else{$location_con=" and a.location_id=$cbo_location";}
		   	 //if($cbo_working_company==0) $company_working_cond_inbound=""; else $company_working_cond_inbound=" and a.company_id like '$cbo_working_company'";
			 $dtls_sql_inbound=sql_select("SELECT a.production_date,a.order_id,
					sum(CASE WHEN a.production_type =1 THEN a.production_qnty END) AS cutting_qnty_inbound,
					sum(CASE WHEN a.production_type =2 THEN a.production_qnty END) AS sewing_qnty_inbound,
					sum(CASE WHEN a.production_type =4 THEN a.production_qnty END) AS finish_qnty_inbound	
					from subcon_gmts_prod_dtls a
					where a.company_id in($cbo_working_company) $location_con and a.production_date between '$date_from' and '$date_to' and a.is_deleted=0 and a.status_active=1 and a.is_deleted=0 group by a.production_date,a.order_id order by a.production_date asc");
					
			$inbound_orderID="";		
			 foreach($dtls_sql_inbound as $row)
			 {
				 
				$inbound_orderID.=$row[csf("order_id")].',';
				 /*$production_date=date("Y-m-d", strtotime($row[csf('production_date')])); 
				 $prod_date[change_date_format($row[csf("production_date")])]['po_breakdown_id'].=$row[csf("po_breakdown_id")].",";
				 $prod_date[change_date_format($row[csf("production_date")])]['production_date']=$row[csf("production_date")];*/
				 $prod_date[change_date_format($row[csf("production_date")])]['cutting_qnty_inbound']+=$row[csf("cutting_qnty_inbound")];
				 $prod_date[change_date_format($row[csf("production_date")])]['sewing_qnty_inbound']+=$row[csf("sewing_qnty_inbound")];
				 $prod_date[change_date_format($row[csf("production_date")])]['finish_qnty_inbound']+=$row[csf("finish_qnty_inbound")];
			}
				//
			 	$inbound_orderIDs=chop($inbound_orderID,',');
				$inbound_carton_sql=sql_select("select c.production_date, 
				sum(case when c.production_type =1 and b.process_id=1 then b.total_carton_qnty end) as cutting_inbound_carton, 
				sum(case when c.production_type =2 and b.process_id=2 then b.total_carton_qnty end) as sewing_inbound_carton,
				sum(case when c.production_type =4 and b.process_id=4 then b.total_carton_qnty end) as finish_inbound_carton 
				from subcon_delivery_mst a, subcon_delivery_dtls b,subcon_gmts_prod_dtls c
				where a.id=b.mst_id and b.order_id=c.order_id 
				and b.order_id in($inbound_orderIDs) and c.production_date between '$date_from' and '$date_to' and a.status_active=1 and a.is_deleted=0 
				and c.status_active=1 and c.is_deleted=0 group by  c.production_date");
				
			foreach($inbound_carton_sql as $row)
			{
				 $prod_date[change_date_format($row[csf("production_date")])]['cutting_inbound_carton']+=$row[csf("cutting_inbound_carton")];
				 $prod_date[change_date_format($row[csf("production_date")])]['sewing_inbound_carton']+=$row[csf("sewing_inbound_carton")];
				 $prod_date[change_date_format($row[csf("production_date")])]['finish_inbound_carton']+=$row[csf("finish_inbound_carton")];
			}
			...
			sum(CASE WHEN a.entry_form!=85 THEN a.ex_factory_qnty ELSE 0 END)-sum(CASE WHEN a.entry_form=85 THEN a.ex_factory_qnty ELSE 0 END) as ex_factory_qnty
			from pro_ex_factory_mst a, wo_po_break_down b, wo_po_details_master c
			where a.po_break_down_id=b.id and b.job_no_mst=c.job_no $cbo_company_cond_6 $location_con and a.is_deleted=0 and a.status_active=1 and b.status_active in(1,3) and a.ex_factory_date between '$date_from' and '$date_to' group by a.ex_factory_date, a.po_break_down_id,c.company_name,c.buyer_name");
			...
			foreach($exfactory_res as $ex_row)
			{
				...
				$prod_date_buyer_wise[$ex_row[csf("company_name")]][$ex_row[csf("buyer_name")]]['ex_factory_smry_fobVal']+=$ex_row[csf("ex_factory_qnty")]*($job_array[$ex_row[csf("po_break_down_id")]]['unit_price']/$job_array[$ex_row[csf("po_break_down_id")]]['total_set_qnty']);*/
			...
			if($cbo_company==0) $cbo_company_yes=$cbo_working_company.'_'.'workingComp'; else $cbo_company_yes=$cbo_company.'_'.'mainComp';
		?>
        <table width="1670px " border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
            <thead>
            	<tr>
                    <th width="35" rowspan="4" valign="middle">SL</th>
                    <th width="90" rowspan="4" valign="middle">Production Date</th>
                    <th colspan="17">Shipping Status </th>
            	...
            	<tr>
                    <th rowspan="2" width="80">In House- Self Order</th>
                    <th rowspan="2" width="80">Inbound SubCont</th>
                    <th rowspan="2" width="80">Outbound Sub Contact</th>
                    <th rowspan="2" width="80">In house Total</th>
                    <th rowspan="2" width="80">In House- Self Order</th>
                    <th rowspan="2" width="80">Inbound SubCont</th>
                    <th rowspan="2" width="80">Outbound Sub Contact</th>
                    <th rowspan="2" width="80">In house Total</th>
		 ...
		 <div style="max-height:420px; overflow-y:scroll; width:1690px" id="scroll_body">
       	 <table cellspacing="0" border="1" class="rpt_table"  width="1670px" rules="all" id="scroll_body" >
         <?
			for($j=0;$j<$datediff;$j++)
			{
				if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";	
				...
				<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                   ...
                   <td width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'cutting_inbound')"><? echo number_format($prod_date[$newdate]['cutting_qnty_inbound'],2); ?></a><? $total_cutting_qnty_inbound+=$prod_date[$newdate]['cutting_qnty_inbound']; if($prod_date[$newdate]['cutting_qnty_inbound']>0) $cutting_qnty_inbound++; ?></td>
                   
                   
                    <td width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'cutting_subcontract')"><? echo number_format($prod_date[$newdate]['cutting_qnty_outbound'],2); ?></a><? $total_cutting_outbound+=$prod_date[$newdate]['cutting_qnty_outbound']; if($prod_date[$newdate]['cutting_qnty_outbound']>0) $cuting_out++; ?></td>
                     <td width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'cutting')"><? $totalCuttingQty=$prod_date[$newdate]['cutting_qnty']+$prod_date[$newdate]['cutting_qnty_inbound']; echo number_format($totalCuttingQty,2); ?></a><? $total_cutting+=$totalCuttingQty; if($prod_date[$newdate]['cutting_qnty']>0) $cuting++; ?></td>
                    
                    <td width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'sewingout_inhouse')"><? echo number_format($prod_date[$newdate]['sewingout_qnty_inhouse'],2); ?></a><?  $total_sew_inhouse+=$prod_date[$newdate]['sewingout_qnty_inhouse']; if($prod_date[$newdate]['sewingout_qnty_inhouse']>0) $sewing_in++; ?></td>
                   <td width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'sewing_inbound')"><? echo number_format($prod_date[$newdate]['sewing_qnty_inbound'],2); ?></a><? $total_sewing_qnty_inbound+=$prod_date[$newdate]['sewing_qnty_inbound']; if($prod_date[$newdate]['sewing_qnty_inbound']>0) $sewing_qnty_inbound++; ?></td>
                    <td width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'sewingout_subcontract')"><? echo number_format($prod_date[$newdate]['sewingout_qnty_outbound'],2); ?></a><? $total_sew_outbound+=$prod_date[$newdate]['sewingout_qnty_outbound']; if($prod_date[$newdate]['sewingout_qnty_outbound']>0) $sewing_out++; ?></td>
					<td width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'sewingout')"><? $totalSweingQty=$prod_date[$newdate]['sewing_qnty']+$prod_date[$newdate]['sewing_qnty_inbound']; echo number_format($totalSweingQty,2); ?></a><? $total_sew+=$totalSweingQty; if($prod_date[$newdate]['sewing_qnty']>0) $sewing++; ?></td>
                    
                    
                    <td width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'finish_inhouse')"><? echo number_format($prod_date[$newdate]['finish_qnty_inhouse'],2); ?></a><? $total_finishg_inhouse+=$prod_date[$newdate]['finish_qnty_inhouse']; if($prod_date[$newdate]['finish_qnty_inhouse']>0) $finish_in++; ?></td>
                    <td width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'finish_inbound')"><? echo number_format($prod_date[$newdate]['finish_qnty_inbound'],2); ?></a><? $total_finish_qnty_inbound+=$prod_date[$newdate]['finish_qnty_inbound']; if($prod_date[$newdate]['finish_qnty_inbound']>0) $finish_qnty_inbound++; ?></td>
                    <td width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'finish_subcontract')"><? echo number_format($prod_date[$newdate]['finish_qnty_outbound'],2); ?></a><? $total_finish_outbound+=$prod_date[$newdate]['finish_qnty_outbound']; if($prod_date[$newdate]['finish_qnty_outbound']>0) $finish_out++; ?></td>
                    <td width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'finish')"><? $totalFinishingQty=$prod_date[$newdate]['finish_qnty']+$prod_date[$newdate]['finish_qnty_inbound']; echo number_format($totalFinishingQty,2); ?></a><? $total_finish+=$totalFinishingQty; if($prod_date[$newdate]['finish_qnty']>0) $finish++; ?></td>
                    
                    
                    <td width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'carton')"><? $totalCarton=$prod_date[$newdate]['carton_qty']+$prod_date[$newdate]['cutting_inbound_carton']+$prod_date[$newdate]['sewing_inbound_carton']+$prod_date[$newdate]['finish_inbound_carton']; echo number_format($totalCarton,2); ?></a><? $total_carton+=$totalCarton; if($prod_date[$newdate]['carton_qty']>0) $carton++; ?></td>
        ...
        <tfoot>
            <tr>
                ...
                <th width="80"><? echo number_format($total_cutting_qnty_inbound,2); ?></th>
                
                <th width="80"><? echo number_format($total_cutting_outbound,2); ?></th>
                <th width="80"><? echo number_format($total_cutting,2); ?></th>
                <th width="80"><? echo number_format($total_sew_inhouse,2); ?></th>
                
                <th width="80"><? echo number_format($total_sewing_qnty_inbound,2); ?></th>
                
                <th width="80"><? echo number_format($total_sew_outbound,2); ?></th>
                <th width="80"><? echo number_format($total_sew,2); ?></th>
                <th width="80"><? echo number_format($total_finishg_inhouse,2); ?></th>
                
                <th width="80"><? echo number_format($total_finish_qnty_inbound,2); ?></th>
            ...
            <tr>
                <th width="35" >&nbsp;</th>
                <th width="90" >Avg.</th>
                <th width="80" ><? //echo number_format($total_knit/$count_knit,2); 
					$cv=$total_knit/$count_knit;
					if(is_infinite($cv) || is_nan($cv)){$cv=0;}
					echo number_format($cv,2); 
				?></th>
                ...
                <th width="80" ><? //echo number_format($total_emb/$embing,2);
					...
					echo number_format($cv,2); 
				 ?></th>
                <th width="80"><? //echo number_format($total_cutting_inhouse/$cuting_in,2);
					$cv=$total_cutting_inhouse/$cuting_in;
					if(is_infinite($cv) || is_nan($cv)){$cv=0;}
					echo number_format($cv,2); 
				 ?></th>
                
                <th width="80"><? //echo number_format($total_cutting_qnty_inbound/$cutting_qnty_inbound,2); 
					$cv=$total_cutting_qnty_inbound/$cutting_qnty_inbound;
					if(is_infinite($cv) || is_nan($cv)){$cv=0;}
					echo number_format($cv,2); 
				?></th>
                ...
                <th width="80"><? //echo number_format($total_sewing_qnty_inbound/$sewing_qnty_inbound,2);
					$cv=$total_sewing_qnty_inbound/$sewing_qnty_inbound;
					if(is_infinite($cv) || is_nan($cv)){$cv=0;}
					echo number_format($cv,2); 
				 ?></th>
                ...
                <th width="80"><? //echo number_format($total_finishg_inhouse/$finish_in,2);
                 ...
                 <th width="80"><? //echo number_format($total_finish_qnty_inbound/$finish_qnty_inbound,2); 
					$cv=$total_finish_qnty_inbound/$finish_qnty_inbound;
					if(is_infinite($cv) || is_nan($cv)){$cv=0;}
					echo number_format($cv,2); 
				 ?></th>
	...
	else if($type==7)//Gmt Prod Sew2 button
	{	
		$costing_per_arr = return_library_array("select job_no, costing_per from wo_pre_cost_mst","job_no","costing_per"); 
		$tot_cost_arr = return_library_array("select job_no, cm_cost from wo_pre_cost_dtls","job_no","cm_cost"); 
		// rehan for micro
		...
		$job_array=array(); 
		$job_sql="SELECT a.id, a.unit_price,b.buyer_name,b.company_name,sum(c.order_quantity) as po_quantity, b.job_no, b.total_set_qnty,b.set_smv from wo_po_break_down a, wo_po_details_master b ,wo_po_color_size_breakdown c where a.job_no_mst=b.job_no and a.is_deleted=0 and a.status_active in(1,3) and b.is_deleted=0 and b.status_active=1 and b.job_no=c.job_no_mst and a.id=c.po_break_down_id and b.is_deleted=0 and b.status_active=1 group by a.id, a.unit_price,b.buyer_name,b.company_name, b.job_no, b.total_set_qnty,b.set_smv";	
		
		$job_sql_result=sql_select($job_sql);
		foreach($job_sql_result as $row)
		{
			$job_array[$row[csf("id")]]['unit_price']=$row[csf("unit_price")];
			$job_array[$row[csf("id")]]['order_qntys']=$row[csf("po_quantity")];
			$job_array[$row[csf("id")]]['job_no']=$row[csf("job_no")];
			$job_array[$row[csf("id")]]['total_set_qnty']=$row[csf("total_set_qnty")];
			$job_array[$row[csf("id")]]['set_smv']=$row[csf("set_smv")];
			$job_array_summary[$row[csf("company_name")]][$row[csf("buyer_name")]]['po_qty']+=$row[csf("po_quantity")];
		 ...
		 if($cbo_working_company==0) $company_working_cond_1=""; else $company_working_cond_1=" and a.serving_company in($cbo_working_company)";
			...
			sum(CASE WHEN a.production_type =3 and a.embel_name=1 THEN e.production_qnty END) AS printing_qnty,
			sum(CASE WHEN a.production_type =3 and a.embel_name=1 and a.production_source=1 THEN e.production_qnty END) AS printing_qnty_inhouse,
			sum(CASE WHEN a.production_type =3 and a.embel_name=1 and a.production_source=3 THEN e.production_qnty END) AS printing_qnty_outbound, 
			
			sum(CASE WHEN a.production_type =3 and a.embel_name=2 THEN e.production_qnty END) AS emb_qnty,
			sum(CASE WHEN a.production_type =3 and a.embel_name=2 and a.production_source=1 THEN e.production_qnty END) AS emb_qnty_inhouse,
			sum(CASE WHEN a.production_type =3 and a.embel_name=2 and a.production_source=3 THEN e.production_qnty END) AS emb_qnty_outbound,
			...
			where a.po_break_down_id=b.id and c.id=b.job_id and b.id=d.po_break_down_id and d.id=e.color_size_break_down_id and a.id=e.mst_id  $location_con $cbo_company_cond_4 $company_working_cond_1 and a.production_date between '$date_from' and '$date_to' and a.is_deleted=0 and a.status_active=1 and b.status_active in(1) and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.is_deleted=0 and d.status_active=1 and e.is_deleted=0 and e.status_active=1 group by a.production_date, a.po_break_down_id, a.item_number_id,c.buyer_name,a.company_id  order by a.production_date asc";
					//echo $dtls_sql; die;
			 
			 $dtls_sql_result=sql_select($dtls_sql);
			 $prod_date=array();$po_id=""; $po_sewing_qty=array();
			 $po_wise_sewing_out_array=array();
			 if($comp && !$work_comp)
			 {
			 	foreach($dtls_sql_result as $row)
			 	{
			 		$po_wise_sewing_out_array[change_date_format($row[csf("production_date")])][$row[csf("po_breakdown_id")]]["sewing_out"]+=$row[csf("sewingout_qnty_inhouse")]+$row[csf("sewingout_qnty_outbound")];

			 		$production_date=date("Y-m-d", strtotime($row[csf('production_date')])); 
					 //	$po_wise_sewing_out_qntys[$row[csf('po_break_down_id')]]['sewing_out']+=$row[csf("sewingout_qnty_inhouse")]+$row[csf("sewingout_qnty_inhouse")];
			 		...
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewingout_qnty_outbound']+=$row[csf("sewingout_qnty_outbound")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewing_qnty']+=$row[csf("sewing_qnty")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewing_out_in_and_outbound']+=$row[csf("sewingout_qnty_inhouse")]+$row[csf("sewingout_qnty_inhouse")];

			 		$po_wise_sewinginhouse_and_outbound[$row[csf("po_break_down_id")]]['sewing_out_in_and_outbound']+=$row[csf("sewingout_qnty_inhouse")]+$row[csf("sewingout_qnty_inhouse")];



			 		$prod_date_buyer_wise[$row[csf("company_id")]][$row[csf("buyer_name")]]['cutting_qnty']+=$row[csf("cutting_qnty")];
			 		$prod_date_buyer_wise[$row[csf("company_id")]][$row[csf("buyer_name")]]['sewing_input_qnty']+=$row[csf("sewing_input_qnty")];
			 		$prod_date_buyer_wise[$row[csf("company_id")]][$row[csf("buyer_name")]]['sewing_qnty']+=$row[csf("sewing_qnty")];  
			 ...
			 else if(!$comp && $work_comp)
			 {
			 	$location_cons = str_replace('a.location', 'a.location_id', $location_con);
				$subcon_sql=" SELECT c.rate,c.smv, a.order_id as po_break_down_id, a.gmts_item_id as item_number_id, a.company_id,a.production_date,  b.party_id as buyer_name,
					sum( case when a.production_type=1 and d.production_type=1 then  d.prod_qnty else 0 end ) as cutting_qnty, 
					sum( case when a.production_type=1 and d.production_type=1 then  d.prod_qnty else 0 end ) as cutting_qnty_outbound,
					sum( case when a.production_type=7 and d.production_type=7 then  d.prod_qnty else 0 end ) as sewing_input_qnty,
					sum( case when a.production_type=2 and d.production_type=2 then  d.prod_qnty else 0 end ) as sewing_qnty, 
					sum( case when a.production_type=2 and d.production_type=2 then  d.prod_qnty else 0 end ) as sewingout_qnty_outbound,

					sum( case when a.production_type=4 and d.production_type=4 then  d.prod_qnty else 0 end ) as finish_qnty, 
					sum( case when a.production_type=4 and d.production_type=4 then  d.prod_qnty else 0 end ) as finish_qnty_outbound

					from subcon_gmts_prod_dtls a,subcon_gmts_prod_col_sz d, subcon_ord_mst b, subcon_ord_dtls c,subcon_ord_breakdown e
					where    a.id=d.dtls_id and a.order_id=c.id and c.job_no_mst=b.subcon_job and a.status_active=1 and a.is_deleted=0   and c.status_active=1 and c.is_deleted=0 and b.status_active=1 and b.is_deleted=0   and e.id=d.ord_color_size_id and e.order_id=c.id and a.order_id=e.order_id  and a.production_date between '$date_from' and '$date_to' $location_cons and a.company_id in($work_comp) group by c.rate,c.smv,a.order_id, a.gmts_item_id , a.company_id,a.production_date,  b.party_id ";
					foreach(sql_select($subcon_sql) as $row)
					{
						$prod_date[change_date_format($row[csf("production_date")])]['cutting_qnty_outbound']+=$row[csf("cutting_qnty_outbound")];
						...
						$prod_date[change_date_format($row[csf("production_date")])]['sewingout_qnty_outbound_pcs']+=($row[csf("sewingout_qnty_outbound")]*$row[csf("smv")]);

						$prod_date[change_date_format($row[csf("production_date")])]['finish_qnty_outbound']+=$row[csf("finish_qnty_outbound")];

						$prod_date[change_date_format($row[csf("production_date")])]['sewing_qnty_smv']+=$row[csf("sewing_qnty")]*$row[csf("smv")];

						$prod_date[change_date_format($row[csf("production_date")])]['sewingout_value_outbound']+=$row[csf("sewingout_qnty_outbound")]*$row[csf("rate")];
					}


			 	foreach($dtls_sql_result as $row)
			 	{
			 		$po_wise_sewing_out_array[change_date_format($row[csf("production_date")])][$row[csf("po_breakdown_id")]]["sewing_out"]+=$row[csf("sewingout_qnty_inhouse")];
			 		...
			 		$prod_date[change_date_format($row[csf("production_date")])]['po_breakdown_id'].=$row[csf("po_breakdown_id")].",";
			 		$prod_date[change_date_format($row[csf("production_date")])]['production_date']=$row[csf("production_date")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['printing_qnty']+=$row[csf("printing_qnty_inhouse")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['emb_qnty']+=$row[csf("emb_qnty_inhouse")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['cutting_qnty_inhouse']+=$row[csf("cutting_qnty_inhouse")];
			 		
			 		$prod_date[change_date_format($row[csf("production_date")])]['cutting_qnty']+=$row[csf("cutting_qnty_inhouse")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewingout_qnty_inhouse']+=$row[csf("sewingout_qnty_inhouse")];
			 		
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewing_qnty']+=$row[csf("sewingout_qnty_inhouse")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewing_out_in_and_outbound']+=$row[csf("sewingout_qnty_inhouse")]+$row[csf("sewingout_qnty_inhouse")];
			 		...
			 		$prod_date_buyer_wise[$row[csf("company_id")]][$row[csf("buyer_name")]]['cutting_qnty']+=$row[csf("cutting_qnty_inhouse")];
			 		$prod_date_buyer_wise[$row[csf("company_id")]][$row[csf("buyer_name")]]['sewing_input_qnty']+=$row[csf("sewing_input_qnty_inhouse")];
			 		$prod_date_buyer_wise[$row[csf("company_id")]][$row[csf("buyer_name")]]['sewing_qnty']+=$row[csf("sewingout_qnty_inhouse")];  

			 		$prod_date_buyer_wise[$row[csf("company_id")]][$row[csf("buyer_name")]]['finish_qnty']+=$row[csf("finish_qnty_inhouse")];
			 		$prod_date_buyer_wise[$row[csf("company_id")]][$row[csf("buyer_name")]]['buyer_name']=$row[csf("buyer_name")];
			 		...
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewing_qnty_smv']+=$row[csf("sewingout_qnty_inhouse")]*$item_smv;

			 		$prod_date[change_date_format($row[csf("production_date")])]['finish_qnty_inhouse']+=$row[csf("finish_qnty_inhouse")];
			 		
			 		$prod_date[change_date_format($row[csf("production_date")])]['finish_qnty']+=$row[csf("finish_qnty_inhouse")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['carton_qty']+=$row[csf("carton_qty")];
			 ...
			 else
			 {
			 	foreach($dtls_sql_result as $row)
			 	{
			 		...
			 		$prod_date[change_date_format($row[csf("production_date")])]['emb_qnty']+=$row[csf("emb_qnty")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['cutting_qnty_inhouse']+=$row[csf("cutting_qnty_inhouse")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['cutting_qnty_outbound']+=0;
			 		$prod_date[change_date_format($row[csf("production_date")])]['cutting_qnty']+=$row[csf("cutting_qnty")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewingout_qnty_inhouse']+=$row[csf("sewingout_qnty_inhouse")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewingout_qnty_outbound']+=0;
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewing_qnty']+=$row[csf("sewing_qnty")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewing_out_in_and_outbound']+=$row[csf("sewingout_qnty_inhouse")]+$row[csf("sewingout_qnty_inhouse")];
			 		...
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewingout_qnty_outbound_pcs']+=(0*$item_smv_array[$row[csf("po_breakdown_id")]][$row[csf("item_number_id")]]['smv_pcs']);
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewingout_qnty_inhouse_pcs']+=($row[csf("sewingout_qnty_inhouse")]*$item_smv_array[$row[csf("po_breakdown_id")]][$row[csf("item_number_id")]]['smv_pcs']);
			 		...
			 		$prod_date[change_date_format($row[csf("production_date")])]['finish_qnty_inhouse']+=$row[csf("finish_qnty_inhouse")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['finish_qnty_outbound']+=0;
			 		$prod_date[change_date_format($row[csf("production_date")])]['finish_qnty']+=$row[csf("finish_qnty")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['carton_qty']+=$row[csf("carton_qty")];

			 		$prod_date[change_date_format($row[csf("production_date")])]['sewingout_value_inhouse']+=$row[csf("sewingout_qnty_inhouse")]*$job_array[$row[csf("po_breakdown_id")]]['unit_price'];
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewingout_value_outbound']+=0*$job_array[$row[csf("po_breakdown_id")]]['unit_price'];
			 ...
			 //print_r($po_wise_sewing_out_array);die;
			 ...
			 if( $comp && !$work_comp  ) 
			 {
			 	...
			 	$company_working_cond_33=str_replace("a.knitting_company", "a.service_company", $company_working_cond_2);
			 	$dyeing_query="SELECT a.process_end_date as production_date, sum(b.production_qty) as dyeing_qnty from pro_fab_subprocess a, pro_fab_subprocess_dtls b where a.id=b.mst_id $cbo_company_cond_5 $company_working_cond_33   and a.process_end_date between '$date_from' and '$date_to' and a.entry_form=35  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and a.load_unload_id in(2) group by a.process_end_date";

			 	$dyeing_query_result=sql_select($dyeing_query);
			 	foreach( $dyeing_query_result as $dyeing_row)
			 	{
			 		$prod_date[change_date_format($dyeing_row[csf("production_date")])]['dyeing_qnty']+=$dyeing_row[csf("dyeing_qnty")];
			 	}



			 }
			 else if($work_comp && !$comp)
			 {
			 	$knited_query="SELECT a.receive_date as production_date, sum(b.grey_receive_qnty) as kniting_qnty from inv_receive_master a, pro_grey_prod_entry_dtls b where a.id=b.mst_id $cbo_company_cond_5  $company_working_cond_2 $location_con_rcv and a.receive_date between '$date_from' and '$date_to' and a.entry_form=2 and a.item_category=13 and a.knitting_source=1 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 group by a.receive_date";  
			 	...
			 	$knited_query_subcon="SELECT a.product_date as production_date, sum(b.product_qnty) as kniting_qnty from subcon_production_mst a, subcon_production_dtls b where a.id=b.mst_id $cbo_company_cond_5  $company_working_cond_2 $location_con_rcv and a.product_date between '$date_from' and '$date_to'  and a.is_deleted=0 and a.status_active=1 group by a.product_date";
			 	...
			 	$company_working_cond_33=str_replace("a.knitting_company", "a.service_company", $company_working_cond_2);
			 	$dyeing_query="SELECT a.process_end_date as production_date, sum(b.production_qty) as dyeing_qnty from pro_fab_subprocess a, pro_fab_subprocess_dtls b where a.id=b.mst_id $cbo_company_cond_5 $company_working_cond_33   and a.process_end_date between '$date_from' and '$date_to' and a.entry_form=35  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and a.load_unload_id in(2) and a.service_source=1 group by a.process_end_date";
			 	...
			 	$dyeing_query_subcon="SELECT a.product_date as production_date, 0 as dyeing_qnty from subcon_production_mst a, subcon_production_dtls b where a.id=b.mst_id $cbo_company_cond_5  $company_working_cond_2 $location_con_rcv and a.product_date between '$date_from' and '$date_to'  and a.is_deleted=0 and a.status_active=1  group by a.product_date"; // and a.entry_form=38 ( remove for hams) 
			 ...
			 $company_working_cond_5=str_replace("a.knitting_company", "a.company_id", $company_working_cond_2);
			 
			 $actual_resource_sql="SELECT b.pr_date ,sum(b.man_power) as man_power ,sum(b.helper) as helper ,sum(b.operator) as operator from prod_resource_mst a ,prod_resource_dtls b  where a.id=b.mst_id $cbo_company_cond_5 $company_working_cond_5 $location_con_rcv   and a.is_deleted=0    and b.is_deleted=0 group by b.pr_date";
			 foreach(sql_select($actual_resource_sql) as $keys=>$vals)  
			 {
			 	$resource_arrs[change_date_format($vals[csf("pr_date")])]["man_power"]=$vals[csf("man_power")];
			 	$resource_arrs[change_date_format($vals[csf("pr_date")])]["helper"]=$vals[csf("helper")];
			 	$resource_arrs[change_date_format($vals[csf("pr_date")])]["operator"]=$vals[csf("operator")];
			 }
			 $subcon_sewing="SELECT a.production_date as dates, sum(b.prod_qnty) as qntys,a.order_id from subcon_gmts_prod_dtls a   ,subcon_gmts_prod_col_sz b where a.id=b.dtls_id  and a.is_deleted=0 and  a.status_active=1 and a.production_type=2 $cbo_company_cond $company_working_cond_5 $location_con_rcv group by a.production_date,a.order_id  ";


 			 
 			 $subcon_sewing_array=array();

 			 foreach(sql_select($subcon_sewing)  as $keys_sub=>$val_sub)
 			 {
 			 	$subcon_sewing_array[change_date_format($val_sub[csf("dates")])]["qnty"]+=$val_sub[csf("qntys")];
 			 	$subcon_sewing_array_po_date[change_date_format($val_sub[csf("dates")])][$val_sub[csf("order_id")]]["qnty"]+=$val_sub[csf("qntys")];
 			 	if($subcon_sewing_array[change_date_format($val_sub[csf("dates")])]["order"]=="")
 			 	{
 			 		$subcon_sewing_array[change_date_format($val_sub[csf("dates")])]["order"]=$val_sub[csf("order_id")];
 			 	}
 			 	else
 			 	{
 			 		$subcon_sewing_array[change_date_format($val_sub[csf("dates")])]["order"].=','.$val_sub[csf("order_id")];
 			 ...
 			 $sub_con_unit_price=" SELECT a.currency_id,b.id,b.rate,b.order_rcv_date as order_date,b.order_uom as uom from subcon_ord_mst a, subcon_ord_dtls b where a.SUBCON_JOB=b.job_no_mst and a.status_active=1 and a.is_deleted=0 and  b.status_active=1 and b.is_deleted=0 group by a.currency_id,b.id,b.rate,b.order_rcv_date,b.order_uom";
 			 $sub_con_unit_price_arr=array();
 			 foreach(sql_select($sub_con_unit_price) as $keys=>$vals)
 			 {	 	
 			 		if($db_type==0)
 			 		{
 			 			$conversion_date=change_date_format($vals[csf("order_date")], "Y-m-d", "-",1);
 			 		}
 			 		else
 			 		{
 			 			$conversion_date=change_date_format($vals[csf("order_date")], "d-M-y", "-",1);
 			 		}
  			 		$sub_con_unit_price_arr[$vals[csf("id")]]["dates"]=$conversion_date; 			 
 			 		$sub_con_unit_price_arr[$vals[csf("id")]]["rate"]=$vals[csf("rate")];
  			 		$sub_con_unit_price_arr[$vals[csf("id")]]["currency_id"]=$vals[csf("currency_id")];
  			 		$sub_con_unit_price_arr[$vals[csf("id")]]["uom"]=$vals[csf("uom")];
 			 }
			 ...
			 if($cbo_company==0) $cbo_company_cond_6=""; else $cbo_company_cond_6=" and c.company_name in($cbo_company)";
			 if( str_replace("'", "",$cbo_working_company_id)==0) $cbo_delivery_com_cond=""; else $cbo_delivery_com_cond=" and d.delivery_company_id in($cbo_working_company)";//d.delivery_company_id=$cbo_working_company_id
			 if( str_replace("'", "",$cbo_working_company_id)>0 &&  str_replace("'", "",$cbo_location)>0 )
			 {
			...
			{
				$prod_date[change_date_format($ex_row[csf("ex_factory_date")])]['ex_factory']+=$ex_row[csf("ex_factory_qnty")];
				if($prod_date[change_date_format($ex_row[csf("ex_factory_date")])]['ex_factory_po']=="")
				{
					$prod_date[change_date_format($ex_row[csf("ex_factory_date")])]['ex_factory_po'].=$ex_row[csf("po_break_down_id")];
				}
				else
				{
					$prod_date[change_date_format($ex_row[csf("ex_factory_date")])]['ex_factory_po'].=','.$ex_row[csf("po_break_down_id")];
				...
				$po_wise_ex_fact[change_date_format($ex_row[csf("ex_factory_date")])][$ex_row[csf("po_break_down_id")]]['ex_fact_qnty']+=$ex_row[csf("ex_factory_qnty")];
			...
			if($cbo_company==0) $cbo_company_yes=$cbo_working_company.'_'.'workingComp'; else $cbo_company_yes=$cbo_company.'_'.'mainComp';
			?>
        	<table width="1250px " border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
      			<p style="float:left"><strong>Buyer Wise Summary Part</strong></p>
           			<thead>
            			<tr>
							<th width="30">SL</th>
       		...
       		<div style="max-height:420px; overflow-y:scroll; width:1270px" id="scroll_body_summery">
       			<table cellspacing="0" border="1" class="rpt_table"  width="1250px" rules="all" id="scroll_body_summery" >
			        <?
					$inc=1;
					$all_po_ids="";
					...
					foreach($prod_date_buyer_wise as $companyKey =>$comp_value)
					{
						foreach($comp_value as $buyerKey =>$buyer_value)
						{
							...
							<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('trs_<? echo $inc; ?>','<? echo $bgcolor; ?>')" id="trs_<? echo $inc; ?>">
								...
								<td width="100" align="right"><? $swingUnitPrice= $prod_date_buyer_wise[$companyKey][$buyerKey]['sewingUnitprice']*$prod_date_buyer_wise[$companyKey][$buyerKey]['sewing_qnty']; echo number_format($swingUnitPrice,2); $smry_tot_sewing_fob_val_input_qnty+=$swingUnitPrice; ?></td>
								<td width="100" align="right"><? echo number_format($prod_date_buyer_wise[$companyKey][$buyerKey]['ex_factory_smry'],2); $smry_tot_ex_factory+=$prod_date_buyer_wise[$companyKey][$buyerKey]['ex_factory_smry']; ?></td>
								<td width="100" align="right"><? echo number_format($prod_date_buyer_wise[$companyKey][$buyerKey]['ex_cm_value_in'],2); $smry_tot_ex_factory_val+=$prod_date_buyer_wise[$companyKey][$buyerKey]['ex_cm_value_in']; ?></td>
        ...
        <table width="2615px " border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
       	  <p style="float:left"><strong>Details Part</strong></p>
	       	 <thead>
	       	 	<tr>
	       	 		<th width="35" rowspan="4" valign="middle" style="word-wrap: break-word;word-break: break-all;">SL</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  width="100" rowspan="4" valign="middle">Production Date</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;" width="240" colspan="3">Textails Production </th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;" width="960" colspan="12">Garments Production </th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;" width="800" colspan="10">Order value and SMV </th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;" width="240" colspan="3">Ex-Factory Dtls</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;" width="160" colspan="2">CM Cost (On Sewing Qty)</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;" width="80" rowspan="4">Total CM Cost</th>

	       	 	</tr>
	       	 	<tr>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  width="80" rowspan="3" valign="middle">Knitting Prod. (Kg) </th> 
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  width="80" rowspan="3" valign="middle">Dyeing Prod. ( Kg.) </th>  
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  width="80" rowspan="3" valign="middle">All over Print. ( Kg.) </th>  
 	       	 		<th style="word-wrap: break-word;word-break: break-all;"  width="160" colspan="2" valign="middle">Emblishment (Pcs)</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  width="240" colspan="3" valign="middle">Cutting (Pcs)</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  width="240" colspan="3" valign="middle">Sewing (Pcs)</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  width="240" colspan="3" valign="middle">Finish (Pcs)</th>
 	       	 		<th style="word-wrap: break-word;word-break: break-all;"  width="80" rowspan="3">Carton</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  width="240" colspan="3">Order Value (On Sewing Qty)</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  width="560" colspan="7">SMV (On Sewing Qty)</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  width="80" rowspan="3" valign="middle">Ex-Factory Qty (Pcs)</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  width="80" rowspan="3" valign="middle">Ex-Factory Value (USD)</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  width="80" rowspan="3" valign="middle">Earning CM Cost (on Ex Fac.Value) (USD)</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  width="80" rowspan="3" valign="middle">In House (USD)</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  width="80" rowspan="3" valign="middle">Sub Con (In Bound) (USD)</th>
	       	 	...
	       	 	<tr>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  rowspan="2" width="80">Printing</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  rowspan="2" width="80">Emb.</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  rowspan="2" width="80">In House</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  rowspan="2" width="80">Sub Contact</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  rowspan="2" width="80">Total</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  rowspan="2" width="80">In House</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  rowspan="2" width="80">Sub Contact</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  rowspan="2" width="80">Total</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  rowspan="2" width="80">In House</th>
	       	 		...
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  rowspan="2" width="80">In House (USD)</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  rowspan="2" width="80">Sub Contact (USD)</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  rowspan="2" width="80">Total (USD)</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  rowspan="2" width="80">SAH Available</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  colspan="2" width="80">SAH Produced</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  rowspan="2" width="80">Helper</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  rowspan="2" width="80">Operator</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  rowspan="2" width="80">Total Manpower</th>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  rowspan="2" width="80">Efficiency</th>
	       	 		
	       	 	</tr>
	       	 	<tr>
	       	 		<th style="word-wrap: break-word;word-break: break-all;"  width="80">In House</th>
	       	 		<th  style="word-wrap: break-word;word-break: break-all;" width="80">Outbound Sub Con</th>
	       	 	</tr>
	       	 </thead>
        </table>
		<div style="max-height:420px; overflow-y:scroll; width:2635px" id="scroll_body">
	       	 <table cellspacing="0" border="1" class="rpt_table"  width="2615px" rules="all" id="scroll_body" >
		        	<?
		        	$totals_knit=0;
		        	$totals_dyeing=0;
		        	$total_helpers=0;
		        	$total_operator=0;
		        	$total_manpower=0;
		        	$total_earning_cost=0;
		        	$total_sewing_out_in_and_outbound=0;
		        	$total_sewing_out_inbound_subcon=0;
		        	$total_sewing_out_with_subcon=0;
		        	...
		        	$avg_manpower=0;
		        	$avg_earning_cost=0;
		        	$avg_sewing_out_in_and_outbound=0;
		        	$avg_sewing_out_inbound_subcon=0;
		        	$avg_sewing_out_with_subcon=0;
		        	$avg_sah_aval=0;
		        	$avg_sah_produced_in=0;
		        	$avg_sah_produced_out=0;
					...
					{
						$date_alls=add_date(str_replace("'","",$txt_date_from),$k);
						$newdates =change_date_format($date_alls);
						$po_ids=$prod_date[$newdates]['po_breakdown_id'];
						$po_ids.=$prod_date[$newdates]['ex_factory_po'];
						foreach( array_filter(array_unique(explode(",", chop($po_ids,","))))  as $po_vals)
						{
							if($po_vals)
							{
								$all_pos_array[trim($po_vals)]=trim($po_vals);
							}
						}
					}
					$all_po_id_in_arr=implode(",", $all_pos_array);
					if($all_po_id_in_arr)
					{
						 
						$condition->po_id("in($all_po_id_in_arr)"); 
					}  
					$condition->init();
					...
					for($j=0;$j<$datediff;$j++)
					{
						...
						?>
						<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
							<td style="word-wrap: break-word;word-break: break-all;" width="35" ><? echo $i; ?></td>
							<td style="word-wrap: break-word;word-break: break-all;" width="100"><? echo $newdate; $date=$date_all;  ?>&nbsp;</td>
							<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"; ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'grey_receive_qnty')"><?  echo number_format($prod_date[$newdate]['kniting_qnty'],2); ?></a><? $total_knit+=$prod_date[$newdate]['kniting_qnty'];   ?></td>
							<td  style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"; ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'dyeing_receive_qnty')"><?  echo number_format($prod_date[$newdate]['dyeing_qnty'],2); ?></a><? $total_knit+=$prod_date[$newdate]['dyeing_qnty'];    ?></td>
							<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"; ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'all_overprint_receive_qnty')"><?  echo number_format($prod_date[$newdate]['allover_qnty'],2); ?></a><? $total_knit+=$prod_date[$newdate]['allover_qnty'];  ?></td>
							
							<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'printreceived')"><? echo number_format($prod_date[$newdate]['printing_qnty'],2); ?></a> <? $total_print+=$prod_date[$newdate]['printing_qnty']; if($val['printing_qnty']>0) $printing++;  ?></td>
							<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'emb_qnty')"><? echo number_format($prod_date[$newdate]['emb_qnty'],2); ?></a><? $total_emb+=$prod_date[$newdate]['emb_qnty']; if($prod_date[$newdate]['emb_qnty']>0) $embing++; ?></td>

							<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'cutting_inhouse')"><? echo number_format($prod_date[$newdate]['cutting_qnty_inhouse'],2); ?></a><? $total_cutting_inhouse+=$prod_date[$newdate]['cutting_qnty_inhouse']; if($prod_date[$newdate]['cutting_qnty_inhouse']>0) $cuting_in++; ?></td>


							<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'cutting_subcontract')"><? echo number_format($prod_date[$newdate]['cutting_qnty_outbound'],2); ?></a><? $total_cutting_outbound+=$prod_date[$newdate]['cutting_qnty_outbound']; if($prod_date[$newdate]['cutting_qnty_outbound']>0) $cuting_out++; ?></td>
							<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'cutting')"><? echo number_format($prod_date[$newdate]['cutting_qnty'],2); ?></a><? $total_cutting+=$prod_date[$newdate]['cutting_qnty']; if($prod_date[$newdate]['cutting_qnty']>0) $cuting++; ?></td>


							<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'sewingout_inhouse')"><? echo number_format($prod_date[$newdate]['sewingout_qnty_inhouse'],2); ?></a><?  $total_sew_inhouse+=$prod_date[$newdate]['sewingout_qnty_inhouse']; if($prod_date[$newdate]['sewingout_qnty_inhouse']>0) $sewing_in++; ?></td>
							<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'sewingout_subcontract')"><? echo number_format($prod_date[$newdate]['sewingout_qnty_outbound'],2); ?></a><? $total_sew_outbound+=$prod_date[$newdate]['sewingout_qnty_outbound']; if($prod_date[$newdate]['sewingout_qnty_outbound']>0) $sewing_out++; ?></td>
							<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'sewingout')"><? echo number_format($prod_date[$newdate]['sewing_qnty'],2); ?></a><? $total_sew+=$prod_date[$newdate]['sewing_qnty']; if($prod_date[$newdate]['sewing_qnty']>0) $sewing++; ?></td>
							
							<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'finish_inhouse')"><? echo number_format($prod_date[$newdate]['finish_qnty_inhouse'],2); ?></a><? $total_finishg_inhouse+=$prod_date[$newdate]['finish_qnty_inhouse']; if($prod_date[$newdate]['finish_qnty_inhouse']>0) $finish_in++; ?></td>
							<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'finish_subcontract')"><? echo number_format($prod_date[$newdate]['finish_qnty_outbound'],2); ?></a><? $total_finish_outbound+=$prod_date[$newdate]['finish_qnty_outbound']; if($prod_date[$newdate]['finish_qnty_outbound']>0) $finish_out++; ?></td>
							<td  style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'finish')"><? echo number_format($prod_date[$newdate]['finish_qnty'],2); ?></a><? $total_finish+=$prod_date[$newdate]['finish_qnty']; if($prod_date[$newdate]['finish_qnty']>0) $finish++; ?></td>
							<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'carton')"><? echo number_format($prod_date[$newdate]['carton_qty'],2); ?></a><? $total_carton+=$prod_date[$newdate]['carton_qty']; if($prod_date[$newdate]['carton_qty']>0) $carton++; ?></td>


							<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'cm_value_popup')"><? $cm_value_in=$prod_date[$newdate]['cm_value_in']; echo number_format($cm_value_in,2); $total_cm_value_in+=$cm_value_in; if($cm_value_in>0) $cm_val_in++; ?></a>
							</td>
							<td style="word-wrap: break-word;word-break: break-all;"  width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'cm_value_popup')"><? $cm_value_out=$prod_date[$newdate]['cm_value_out']; echo number_format($cm_value_out,2); $total_cm_value_out+=$cm_value_out; if($cm_value_out>0) $cm_val_out++; 
								$cm_value_total=$cm_value_in+$cm_value_out;
							?></a>
							</td>
							...
							<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><? echo number_format($cm_value_total,2); ?></td>



							<td width="80" align="right"><? $swing_val_availble=$tsmvArr[$newdate]['smv']/60; echo number_format($swing_val_availble,2);  $total_swing_val_availble_value+=$swing_val_availble; ?></td>

							<td width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'smv_sewing_popup',1)"><? $swing_val_produced=$produce_qty; echo number_format($swing_val_produced,2);  $total_sewout_order_value_produced+=$swing_val_produced; ?></a></td>

							<td width="80" align="right"><? $sewing_out_bound=($prod_date[$newdate]['sewingout_qnty_outbound_pcs'])/60; $total_sewing_out_bound+=$sewing_out_bound; ?><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'smv_sewing_popup',3)"><? echo number_format($sewing_out_bound,2);?></a></td>

								<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><? echo $resource_arrs[$newdate]["helper"];?> </td>
								<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><? echo $resource_arrs[$newdate]["operator"];?> </td>
								<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><? echo $resource_arrs[$newdate]["man_power"];?> </td>

								 
								 
								<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><? echo number_format(($effiecy_aff_perc),2);  $total_effiecy_aff_perc+=$effiecy_aff_perc; 
								$po_id_ex=$prod_date[$newdate]['ex_factory_po'];

								?></td>
								<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id_ex."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'ex_factory_qty_popup')"><? $ex_factory_qty=$prod_date[$newdate]['ex_factory']; echo number_format($ex_factory_qty,2);  $total_ex_factory_qty+=$ex_factory_qty; if($ex_factory_qty>0) $ex_fac++;
									
									$total_cm_cost=0;
									$total_sewing_cm_costs=0;
									foreach( array_filter(array_unique(explode(",",chop($po_id_ex,",")))) as $p_id)
									{
										if($po_wise_ex_fact[$newdate][$p_id]['ex_fact_qnty'])
										{
											$total_cm_cost+=($other_costing_arr[$p_id]['cm_cost']/$job_array[$p_id]["order_qntys"])*$po_wise_ex_fact[$newdate][$p_id]['ex_fact_qnty'];
										}
									...
									foreach( array_filter(array_unique(explode(",",chop($po_id,",")))) as $p_id)
									{
										$costing_per=$costing_per_arr[$job_array[$p_id]['job_no']];
										$costing_cm=$cm_cost_arr[$job_array[$p_id]['job_no']];
										...
										//echo "$cm_cost_pcs a ";
										$cm_cost_pcs=number_format($cm_cost_pcs,2);
										$cm_cost_pcs=str_replace(",", "", $cm_cost_pcs);


										 
										//$total_sewing_cm_costs+=$po_wise_sewing_out_array[$newdate][$p_id]["sewing_out"]*($other_costing_arr[$p_id]['cm_cost']/$job_array[$p_id]["order_qntys"]);
										$total_sewing_cm_costs+=$po_wise_sewing_out_array[$newdate][$p_id]["sewing_out"]*$cm_cost_pcs;
										
									}


									$earnings_cost=$total_cm_cost;
									$sewing_cm_costs=$total_sewing_cm_costs;
									$sub_con_order=$subcon_sewing_array[$newdate]["order"];
									$subcon_total_value=0;
									foreach(explode(",", $sub_con_order) as $po_vals)
									{
										if($po_vals)
										{

											if($sub_con_unit_price_arr[$po_vals]["currency_id"]==1)
											{
												$currency_rate=set_conversion_rate( 2, $sub_con_unit_price_arr[$po_vals]["dates"] );
												if($sub_con_unit_price_arr[$po_vals]["uom"]==2)
												{
													$rates=($sub_con_unit_price_arr[$po_vals]["rate"]/$currency_rate)/12;
												}
												else
												{
													$rates=$sub_con_unit_price_arr[$po_vals]["rate"]/$currency_rate;
												}
												$rates=number_format($rates,2);
												$rates=str_replace(",", "", $rates);
												
												$subcon_total_value+=$rates*$subcon_sewing_array_po_date[$newdate][$po_vals]["qnty"];
								 ...
								 ?></a>  </td>
								<td  style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><? $ex_factory_value=$prod_date[$newdate]['ex_factory_val']; echo number_format($ex_factory_value,2);  $total_ex_factory_value+=$ex_factory_value; if($ex_factory_value>0) $ex_val++; ?></td>

								<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><? echo number_format(($earnings_cost),2); ?></td>


								<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><? echo $sewing_out_in_and_outbound=number_format($sewing_cm_costs,2).'';  ?> </td>
								
								
								<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><? echo $subcons_sewingout= number_format($subcon_total_value,2);  ?></td>
								<td  style="word-wrap: break-word;word-break: break-all;" width="80" align="right">
                                <a href="##" onclick="openmypage2(<? echo "'".$date."'"  ?>,<? echo "'".$cbo_company_yes."'"  ?>,<? echo "'".$po_id."'"  ?>,<? echo "'".str_replace("'","",$cbo_location)."'"  ?>,'total_cm_cost_popup_type_7')"><? echo $ttl_own_subcon_sewing= number_format(($sewing_cm_costs+$subcon_total_value),2);   ?></a>
                            </td>
								 

							</tr>
						<?
							$totals_knit+=$prod_date[$newdate]['kniting_qnty'];
							$totals_dyeing+=$prod_date[$newdate]['dyeing_qnty'];
							$total_helpers+= $resource_arrs[$newdate]["helper"];
							$total_operator+=$resource_arrs[$newdate]["operator"];
							$total_manpower+=$resource_arrs[$newdate]["man_power"];
							$total_sewing_out_inbound_subcon+=$subcon_total_value;
							$total_earning_cost+=$earnings_cost;
							$total_sewing_out_in_and_outbound+=$sewing_cm_costs;
							$total_sewing_out_with_subcon+=$sewing_cm_costs+$subcon_total_value;
							if($prod_date[$newdate]['kniting_qnty']){$avg_knit++;}
							if($prod_date[$newdate]['dyeing_qnty']){$avg_dyeing++;}
							if($resource_arrs[$newdate]["helper"]){$avg_helpers++;}
							if($resource_arrs[$newdate]["operator"]){$avg_operators++;}
							if($resource_arrs[$newdate]["man_power"]){$avg_manpower++;}							 
							if($earnings_cost){$avg_earning_cost++;}	
							//if($sewing_out_in_and_outbound){$avg_sewing_out_in_and_outbound++;}
							//if($subcon_sewingouts){$avg_sewing_out_inbound_subcon++;}
							//if($ttl_own_subcon_sewing){$avg_sewing_out_with_subcon++;}
							if($swing_val_availble){$avg_sah_aval++;}
							if($swing_val_produced){$avg_sah_produced_in++;}
							if($sewing_out_bound){$avg_sah_produced_out++;}

							if($sewing_cm_costs){$avg_sewing_out_in_and_outbound++;}
		        			if(str_replace("0.00", "",$subcon_total_value)){$avg_sewing_out_inbound_subcon++;}
		        			if(str_replace("0.00", "", $ttl_own_subcon_sewing)){ $avg_sewing_out_with_subcon++;}
		   ...
		   <table width="2615px " border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
		        <tfoot>
		            <tr>
		                <th style="word-wrap: break-word;word-break: break-all;" width="35">&nbsp;</th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="100">Total</th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? echo number_format($totals_knit,2); ?></th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? echo number_format($totals_dyeing,2); ?></th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? echo number_format($total_all_over_print,2); ?></th>
 		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? echo number_format($total_print,2); ?></th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? echo number_format($total_emb,2); ?></th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? echo number_format($total_cutting_inhouse,2); ?></th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? echo number_format($total_cutting_outbound,2); ?></th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? echo number_format($total_cutting,2); ?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? echo number_format($total_sew_inhouse,2); ?></th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? echo number_format($total_sew_outbound,2); ?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? echo number_format($total_sew,2); ?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? echo number_format($total_finishg_inhouse,2); ?></th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? echo number_format($total_finish_outbound,2); ?></th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? echo number_format($total_finish,2); ?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? echo number_format($total_carton,2); ?></th>
 		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? echo number_format($total_cm_value_in,2); ?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? echo number_format($total_cm_value_out,2); ?></th>
		                 <th style="word-wrap: break-word;word-break: break-all;"  width="80">&nbsp;</th>

		                 <th style="word-wrap: break-word;word-break: break-all;" width="80"><? echo number_format($total_swing_val_availble_value,2); ?></th>
		                 <th style="word-wrap: break-word;word-break: break-all;" width="80"><? echo number_format($total_sewout_order_value_produced,2); ?></th>
		                 <th style="word-wrap: break-word;word-break: break-all;" width="80"><?  echo number_format($total_sewing_out_bound,2); ?></th>
		                 
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><?  echo number_format($total_helpers,2); ?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><?  echo number_format($total_operator,2); ?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><?  echo number_format($total_manpower,2); ?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80">&nbsp;</th>
		                
 		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? echo number_format($total_ex_factory_qty,2); ?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? echo number_format($total_ex_factory_value,2); ?> </th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? echo number_format($total_earning_cost,2); ?> </th>
 		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? echo number_format($total_sewing_out_in_and_outbound,2); ?> </th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? echo number_format($total_sewing_out_inbound_subcon,2); ?> </th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? echo number_format($total_sewing_out_with_subcon,2); ?> </th>
		                 
		               
		            </tr>
		            <tr>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="35" >&nbsp;</th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="100" >Avg.</th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80" ><? //echo number_format($totals_knit/$avg_knit,2); 
							$cv=$totals_knit/$avg_knit;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80" ><? //echo number_format($totals_dyeing/$avg_dyeing,2); 
							$cv=$totals_dyeing/$avg_dyeing;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80" ><? //echo number_format($total_all_over_print/$count_print,2);  
							$cv=$total_all_over_print/$count_print;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80" ><? //echo number_format($total_print/$printing,2);  
							$cv=$total_print/$printing;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80" ><? //echo number_format($total_emb/$embing,2);  
							$cv=$total_emb/$embing;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? //echo number_format($total_cutting_inhouse/$cuting_in,2);  
							$cv=$total_cutting_inhouse/$cuting_in;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? //echo number_format($total_cutting_outbound/$cuting_out,2); 
							$cv=$total_cutting_outbound/$cuting_out;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						 ?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? //echo number_format($total_cutting/ $cuting,2);  
							$cv=$total_cutting/ $cuting;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? //echo number_format($total_sew_inhouse/$sewing_in,2);  
							$cv=$total_sew_inhouse/$sewing_in;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? //echo number_format($total_sew_outbound/$sewing_out,2);  
							$cv=$total_sew_outbound/$sewing_out;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? //echo number_format($total_sew/$sewing,2);  
							$cv=$total_sew/$sewing;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? //echo number_format($total_finishg_inhouse/$finish_in,2);  
							$cv=$total_finishg_inhouse/$finish_in;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? //echo number_format($total_finish_outbound/$finish_out,2);  
							$cv=$total_finish_outbound/$finish_out;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? //echo number_format($total_finish/$finish,2);  
							$cv=$total_finish/$finish;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? //echo number_format($total_carton/$carton,2);  
							$cv=$total_carton/$carton;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		               
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? //echo number_format($total_cm_value_in/$cm_val_in,2);  
							$cv=$total_cm_value_in/$cm_val_in;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? //echo number_format($total_cm_value_out/$cm_val_out,2);  
							$cv=$total_cm_value_out/$cm_val_out;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
	                	...
	                	<th width="80"><? //echo number_format($total_swing_val_availble_value/$avg_sah_aval,2);  
							$cv=$total_swing_val_availble_value/$avg_sah_aval;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
	                	<th width="80"><? //echo number_format($total_sewout_order_value_produced/$avg_sah_produced_in,2);  
							$cv=$total_sewout_order_value_produced/$avg_sah_produced_in;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
	                	<th width="80"><? //echo number_format($total_sewing_out_bound/$avg_sah_produced_out,2);  
							$cv=$total_sewing_out_bound/$avg_sah_produced_out;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? //echo number_format( $total_helpers/$avg_helpers,2) ; 
							$cv=$total_helpers/$avg_helpers;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? //echo number_format( $total_operator/$avg_operators,2);  
							$cv=$total_operator/$avg_operators;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? //echo number_format( $total_manpower/$avg_manpower,2);  
							$cv=$total_manpower/$avg_manpower;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? //echo number_format($total_sew_order_value/$ord_tot,2); ?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? //echo number_format($total_ex_factory_qty/$ex_fac,2); 
							$cv=$total_ex_factory_qty/$ex_fac;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						 ?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? //echo number_format($total_ex_factory_value/$ex_val,2); 
							$cv=$total_ex_factory_value/$ex_val;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						 ?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? //echo number_format($total_earning_cost/$avg_earning_cost,2); 
							$cv=$total_earning_cost/$avg_earning_cost;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						 ?></th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? //echo number_format($total_sewing_out_in_and_outbound/$avg_sewing_out_in_and_outbound,2); 
							$cv=$total_sewing_out_in_and_outbound/$avg_sewing_out_in_and_outbound;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						 ?></th>
		                <th style="word-wrap: break-word;word-break: break-all;"  width="80"><? //echo number_format($total_sewing_out_inbound_subcon/$avg_sewing_out_inbound_subcon,2); 
							$cv=$total_sewing_out_inbound_subcon/$avg_sewing_out_inbound_subcon;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						 ?></th>
		                <th  style="word-wrap: break-word;word-break: break-all;" width="80"><? //echo number_format($total_sewing_out_with_subcon/$avg_sewing_out_with_subcon,2); 
							$cv=$total_sewing_out_with_subcon/$avg_sewing_out_with_subcon;
							if(is_infinite($cv) || is_nan($cv)){$cv=0;}
							echo number_format($cv,2); 
						 ?></th>
	...
	else if($type==888)//F.Rcv & Gmts Prod button
	{
	  	if($cbo_company==0) $cbo_company_cond_1=""; else $cbo_company_cond_1="and company_name in($cbo_company)";
		...
		if(str_replace("'","",$cbo_location)==0)
		{
			$cboWorkingComCondLocation="";
		}
		else
		{
			$cboWorkingComCondLocation=" and b.location_id=$cbo_location";
		}
		$prod_date=array();
		$prod_line=array();
        $sql= "SELECT b.id,a.pr_date,a.operator,a.helper,a.man_power,a.working_hour,a.target_per_hour from prod_resource_dtls a, prod_resource_mst b where b.id=a.mst_id $cbo_company_cond_3 $cboWorkingComCondManpower $cboWorkingComCondLocation and a.pr_date between '$date_from' and '$date_to' and a.is_deleted=0 and b.is_deleted=0 order by b.id";
        // echo $sql;die();
		$tpd_data_arr=sql_select($sql);
        foreach($tpd_data_arr as $row)
        {
			$production_date=date("Y-m-d", strtotime($row[csf('pr_date')])); 
			// $tsmvArr[change_date_format($production_date)]['smv']=$row[csf('tsmv')]*60;
			$prod_date[change_date_format($production_date)]['operator']+=$row[csf("operator")];
			$prod_date[change_date_format($production_date)]['helper']+=$row[csf("helper")];
			$prod_date[change_date_format($production_date)]['man_power']+=$row[csf("operator")]+$row[csf("helper")];
			$prod_date[change_date_format($production_date)]['working_hour']+=$row[csf("working_hour")];
			$prod_date[change_date_format($production_date)]['target_per_hour']+=$row[csf("target_per_hour")]*$row[csf("working_hour")];
			$prod_date[change_date_format($production_date)]['tsmv']=$row[csf("man_power")]*$row[csf("working_hour")]*60;
			$prod_line[change_date_format($production_date)][$row[csf("ID")]] = $row[csf("ID")];

			// echo $row[csf("id")]."==".$row[csf("target_per_hour")]*$row[csf("working_hour")]."<br>";
        } 
		
		// echo "<pre>";print_r($prod_date);
        // ====================================== production data ============================
		if($cbo_company==0) $cbo_company_cond_4=""; else $cbo_company_cond_4=" and a.company_id in($cbo_company)";
		if($cbo_working_company==0) $company_working_cond_1=""; else $company_working_cond_1=" and a.serving_company in($cbo_working_company)";
		$dtls_sql="SELECT c.id as job_id,c.job_no,c.buyer_name, a.production_date,b.production_qnty,b.production_type,a.item_number_id,a.po_break_down_id from pro_garments_production_mst a, pro_garments_production_dtls b,wo_po_details_master c,wo_po_break_down d, wo_po_color_size_breakdown e where a.id=b.mst_id and a.po_break_down_id=d.id and b.color_size_break_down_id=e.id and c.id=d.job_id and d.id=e.po_break_down_id  $location_con $cbo_company_cond_4 $company_working_cond_1 and a.production_date between '$date_from' and '$date_to' and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and a.production_type in(1,4,5,8)";
		// echo $dtls_sql;die();
		$dtls_sql_result=sql_select($dtls_sql);
		$po_item_out_qty_arr = array();
		$po_id_arr = array();
		$po_wise_job_arr = array();
		$buyer_smry_arr = array();
		foreach($dtls_sql_result as $row)
		{	
			$prod_date[change_date_format($row[csf("production_date")])][$row[csf("production_type")]] += $row[csf("production_qnty")];
			$po_item_out_qty_arr[change_date_format($row[csf("production_date")])][$row[csf("po_break_down_id")]][$row[csf("item_number_id")]][$row[csf("production_type")]] += $row[csf("production_qnty")];
			$po_id_arr[$row[csf("po_break_down_id")]] = $row[csf("po_break_down_id")];
			$job_id_array[$row[csf("job_id")]] = $row[csf("job_id")];
			$po_wise_job_arr[$row[csf("po_break_down_id")]] = $row[csf("job_no")];

			$buyer_smry_arr[$row[csf("buyer_name")]][$row[csf("production_type")]] += $row[csf("production_qnty")];
			 
		}
		// echo "<pre>";print_r($po_item_out_qty_arr);die;
		$po_id_cond = where_con_using_array($po_id_arr,0,"b.id");
		$item_smv_array=array();
		//$smv_source=2;
		if($smv_source==3)
		{
			$sql_item="SELECT b.id, a.sam_style, a.gmts_item_id from ppl_gsd_entry_mst a, wo_po_break_down b where b.job_id=a.job_id and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active in(1,3) $po_id_cond";
			$resultItem=sql_select($sql_item);
			foreach($resultItem as $itemData)
			{
				$item_smv_array[$itemData[csf('id')]][$itemData[csf('gmts_item_id')]]=$itemData[csf('sam_style')];
		...
		{
			if($cbo_company==0) $cbo_company_cond_2=""; else $cbo_company_cond_2=" and a.company_name in($cbo_company)";
			$sql_item="SELECT b.id, a.set_break_down, c.gmts_item_id, c.set_item_ratio, c.smv_pcs, c.smv_pcs_precost from wo_po_details_master a, wo_po_break_down b, wo_po_details_mas_set_details c where b.job_id=a.id and b.job_id=c.job_id $cbo_company_cond_2 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active in(1,3) $po_id_cond";
			$resultItem=sql_select($sql_item);
			foreach($resultItem as $itemData)
			{
				$item_smv_array[$itemData[csf('id')]][$itemData[csf('gmts_item_id')]]['smv_pcs']=$itemData[csf('smv_pcs')];
		...
		$costing_per_arr = return_library_array("SELECT job_no, costing_per from wo_pre_cost_mst where status_active=1 $job_id_cond","job_no","costing_per"); 
		$tot_cost_arr = return_library_array("SELECT job_no, cm_cost from wo_pre_cost_dtls where status_active=1 $job_id_cond","job_no","cm_cost"); 
		// print_r($tot_cost_arr);
			
		// ============================= finish fab rcv ==============================
		if($cbo_company==0) $cbo_company_cond_5=""; else $cbo_company_cond_5=" and a.company_id in($cbo_company)";
		if($cbo_working_company==0) $company_working_cond_2=""; else $company_working_cond_2=" and a.company_id in($cbo_working_company)";
		$finish_query="SELECT a.receive_date as production_date, sum(b.receive_qnty) as finishing_qnty from  inv_receive_master a, pro_finish_fabric_rcv_dtls b where a.id=b.mst_id $cbo_company_cond_5 $company_working_cond_2 $location_con_rcv and a.receive_date between '$date_from' and '$date_to' and a.entry_form=7 and a.item_category=2 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 group by a.receive_date";
		...
		// ============================== ex-factory ===============================
		if($cbo_company==0) $cbo_company_cond_6=""; else $cbo_company_cond_6=" and c.company_name in($cbo_company)";			
		if($cbo_working_company==0) $company_working_cond_1=""; else $company_working_cond_1=" and f.delivery_company_id in($cbo_working_company)";			
		$ex_sql = "SELECT a.ex_factory_date, a.po_break_down_id,c.company_name,c.buyer_name,   
		sum(CASE WHEN a.entry_form!=85 THEN d.production_qnty ELSE 0 END)-sum(CASE WHEN a.entry_form=85 THEN d.production_qnty ELSE 0 END) as ex_factory_qnty,(b.unit_price/c.total_set_qnty) as unit_price
		from pro_ex_factory_mst a,pro_ex_factory_dtls d, wo_po_break_down b, wo_po_details_master c,wo_po_color_size_breakdown e,pro_ex_factory_delivery_mst f
		where f.id=a.delivery_mst_id and a.id=d.mst_id and  a.po_break_down_id=b.id and b.job_id=c.id and d.color_size_break_down_id=e.id and e.po_break_down_id=b.id $cbo_company_cond_6 $location_con $company_working_cond_1 and a.is_deleted=0 and a.status_active=1 and d.status_active=1 and e.status_active=1 and f.status_active=1  and a.ex_factory_date between '$date_from' and '$date_to' group by a.ex_factory_date, a.po_break_down_id,c.company_name,c.buyer_name,b.unit_price,c.total_set_qnty";
		// echo $ex_sql;
		$exfactory_res = sql_select($ex_sql);
		
		foreach($exfactory_res as $ex_row)
		{
			$prod_date[change_date_format($ex_row[csf("ex_factory_date")])]['ex_factory']+=$ex_row[csf("ex_factory_qnty")];
			$prod_date[change_date_format($ex_row[csf("ex_factory_date")])]['ex_factory_val']+=$ex_row[csf("ex_factory_qnty")]*$ex_row[csf("unit_price")];

			$buyer_smry_arr[$ex_row[csf("buyer_name")]][0] += $ex_row[csf("ex_factory_qnty")];
			$buyer_smry_arr[$ex_row[csf("buyer_name")]]['fob_val'] += $ex_row[csf("ex_factory_qnty")]*$ex_row[csf("unit_price")];
		}

		foreach ($po_item_out_qty_arr as $date_key => $date_data) 
		{
			foreach ($date_data as $po_key => $po_data) 
			{
				foreach ($po_data as $item_key => $row) 
				{
					$cm = $tot_cost_arr[$po_wise_job_arr[$po_key]];
					$prod_date[$date_key]['produce_min'] += $item_smv_array[$po_key][$item_key]['smv_pcs']*$row[5];
					// echo $item_smv_array[$po_key][$item_key]['smv_pcs'],"*".$row[5]."<br>";
					$prod_date[$date_key]['earn_cm'] += $cm*$row[5];
					// echo $cm."*".$row[5]."<br>";
		...
		// echo "<pre>";print_r($buyer_smry_arr);die();
		if($cbo_company==0) $cbo_company_yes=$cbo_working_company.'_'.'workingComp'; else $cbo_company_yes=$cbo_company.'_'.'mainComp';
		?>
		<table width="1260px" cellpadding="0" cellspacing="0" id="caption" align="center">
            <tr>
	            <td align="center" width="100%" colspan="22" class="form_caption"><strong style="font-size:18px">
				<? if($cbo_company==0){ echo "Working Company Name:". $company_library[$cbo_working_company];} else{ 
				$com_arr=explode(",",str_replace("'","",$cbo_company));
        ...
        <table width="1260px " border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
            <thead>
            	<tr>
                    <th width="60">Date</th>                   
                    <th width="80">Fabric Rcv Kg</th>            
                    <th width="80">Cutting Pcs</th>            
                    <th width="80">Input Pcs</th>            
                    <th width="80">Total Use Operator</th>            
                    <th width="80">Total Use Helper</th>            
                    <th width="80">Total Manpower</th>            
                    <th width="80">Avg. W/hrs.</th>            
                    <th width="80">Target Pcs</th>            
                    <th width="80">Achieved Sewing Pcs</th>            
                    <th width="80">Avg SMV</th>            
                    <th width="80">Target Eff. %</th>            
                    <th width="80">Achieved Eff. %</th>            
                    <th width="80">Earned CM $ USD</th>            
                    <th width="80">Finishing Pcs</th>            
                    <th width="80">Shipment Pcs</th> 
                </tr>               
            </thead>
        </table>
		 <div style="max-height:500px; overflow-y:scroll; width:1280px" id="scroll_body">
	       	 <table cellspacing="0" border="1" class="rpt_table"  width="1260px" rules="all" id="scroll_body" >
	       	 	<tbody>
	         	<?
	         	$i=1;
	         	$total_fab_rcv_qty = 0;
				...
				for($j=0;$j<$datediff;$j++)
				{
					...
					$po_id=$prod_date[$newdate]['po_breakdown_id'];
					$produce_qty=$prod_date[$newdate]['sewingout_qnty_inhouse_pcs']/60;
					$effiecy_aff_perc=$produce_qty/($prod_date[$newdate]['tsmv']/60)*100;

					$fin_fab_qty=$prod_date[$newdate]['finishing_qnty'];
					$cutting_qty=$prod_date[$newdate][1];
					$input_qty=$prod_date[$newdate][4];
					$output_qty=$prod_date[$newdate][5];
					$finishing_qty=$prod_date[$newdate][8];

					$operator=$prod_date[$newdate]['operator'];
					$helper=$prod_date[$newdate]['helper'];
					$man_power=$prod_date[$newdate]['man_power'];
					$working_hour=$prod_date[$newdate]['working_hour'];
					$avg_working_hr = $working_hour/count($prod_line[$newdate]);
					$target_per_hour=$prod_date[$newdate]['target_per_hour'];
					$avg_smv = ($output_qty>0) ? $prod_date[$newdate]['produce_min']/$output_qty : 0;
					$available_min = $man_power*$working_hour*60;

					// $target_effi = (($avg_smv*$target_per_hour*100)/($man_power*$working_hour*60));
					$target_effi = ($output_qty>0) ? (($output_qty/60)/($prod_date[$newdate]['tsmv']/60))*100 : 0;

					// echo "((".$output_qty."/60)/(".$prod_date[$newdate]['tsmv']."/60))*100<br>";

					$acv_effi = ($available_min>0) ? ($prod_date[$newdate]['produce_min']/$available_min)*100 : 0;
					// echo $prod_date[$newdate]['produce_min']."/".$available_min."<br>";
					$earn_cm=$prod_date[$newdate]['earn_cm'];

					$ex_factory=$prod_date[$newdate]['ex_factory'];
					?>
					<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
						<td align="center" width="60"><? echo $newdate; $date=$date_all; //change_date_format($date_all); ?>&nbsp;</td>
	                    <td align="right" width="80"><?=number_format($fin_fab_qty,2);?></td>            
	                    <td align="right" width="80"><?=number_format($cutting_qty,0);?></td>            
	                    <td align="right" width="80"><?=number_format($input_qty,0);?></td>            
	                    <td align="right" width="80"><?=number_format($operator,0);?></td>            
	                    <td align="right" width="80"><?=number_format($helper,0);?></td>            
	                    <td align="right" width="80"><?=number_format($man_power,0);?></td>            
	                    <td align="right" width="80"><?=number_format($avg_working_hr,2);?></td>            
	                    <td align="right" width="80"><?=number_format($target_per_hour,0);?></td>            
	                    <td align="right" width="80"><?=number_format($output_qty,0);?></td>            
	                    <td align="right" width="80"><?=number_format($avg_smv,2);?></td>            
	                    <td align="right" width="80"><?=number_format($target_effi,2);?></td>            
	                    <td align="right" width="80"><?=number_format($acv_effi,2);?></td>            
	                    <td align="right" width="80"><?=number_format($earn_cm,2);?></td>            
	                    <td align="right" width="80"><?=number_format($finishing_qty,0);?></td>            
	                    <td align="right" width="80"><?=number_format($ex_factory,0);?></td>
					</tr>
					<?
					$i++;
		         	$total_fab_rcv_qty += $fin_fab_qty;
		         	$total_cut_qty += $cutting_qty;
		         	$total_input_qty += $input_qty;
		         	$total_operatos += $operator;
		         	$total_helper += $helper;
		         	$total_manpower += $man_power;
		         	$total_wo_hr += $avg_working_hr;
		         	$total_target += $target_per_hour;
		         	$total_out_qty += $output_qty;
		         	$total_avg_smv += $avg_smv;
		         	$total_target_effi += $target_effi;
		         	$total_achv_effi += $acv_effi;
		         	$total_earn_cm += $earn_cm;
		         	$total_fin_qty += $finishing_qty;
		         	$total_ex_qty += $ex_factory;
				}
				?>
				</tbody>
	    	</table>
    	...
    	<table width="1260px " border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
	        <tfoot>
	            <tr>
	                ...
	                <th width="80"><?=number_format($total_fab_rcv_qty,2);?></th>            
	                <th width="80"><?=number_format($total_cut_qty,0);?></th>            
	                <th width="80"><?=number_format($total_input_qty,0);?></th>            
	                <th width="80"><?=number_format($total_operatos,0);?></th>            
	                <th width="80"><?=number_format($total_helper,0);?></th>            
	                <th width="80"><?=number_format($total_manpower,0);?></th>            
	                <th width="80"><?=number_format($total_wo_hr,2);?></th>            
	                <th width="80"><?=number_format($total_target,0);?></th>            
	                <th width="80"><?=number_format($total_out_qty,0);?></th>            
	                <th width="80"><?=number_format($total_avg_smv,2);?></th>            
	                <th width="80"><?=number_format($total_target_effi,2);?></th>            
	                <th width="80"><?=number_format($total_achv_effi,2);?></th>            
	                <th width="80"><?=number_format($total_earn_cm,2);?></th>            
	                <th width="80"><?=number_format($total_fin_qty,0);?></th>            
	                <th width="80"><?=number_format($total_ex_qty,0);?></th>
	            </tr>
	            <tr>                
	                <th width="60">Day Avg.</th>           
	                <th width="80"><?=number_format(($total_fab_rcv_qty/$datediff),2);?></th>            
	                <th width="80"><?=number_format(($total_cut_qty/$datediff),0);?></th>            
	                <th width="80"><?=number_format(($total_input_qty/$datediff),0);?></th>            
	                <th width="80"><?=number_format(($total_operatos/$datediff),0);?></th>            
	                <th width="80"><?=number_format(($total_helper/$datediff),0);?></th>            
	                <th width="80"><?=number_format(($total_manpower/$datediff),0);?></th>            
	                <th width="80"><?=number_format(($total_wo_hr/$datediff),2);?></th>            
	                <th width="80"><?=number_format(($total_target/$datediff),0);?></th>            
	                <th width="80"><?=number_format(($total_out_qty/$datediff),0);?></th>            
	                <th width="80"><?=number_format(($total_avg_smv/$datediff),2);?></th>            
	                <th width="80"><?=number_format(($total_target_effi/$datediff),2);?></th>            
	                <th width="80"><?=number_format(($total_achv_effi/$datediff),2);?></th>            
	                <th width="80"><?=number_format(($total_earn_cm/$datediff),2);?></th>            
	                <th width="80"><?=number_format(($total_fin_qty/$datediff),0);?></th>            
	                <th width="80"><?=number_format(($total_ex_qty/$datediff),0);?></th>
	            </tr>
	        </tfoot>
	    </table>

	    <div style="margin-top: 10px;">
	    	<table width="600px " border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
	    		<caption style="font-weight: bold;font-size: 20px;text-align: left;">Buyer Summary</caption>
	    		<thead>
	    			<tr>
	    		...
	    		<tbody>
	    			...
	    			$tot_ex = 0;
	    			$tot_fob = 0;
	    			foreach ($buyer_smry_arr as $b_key => $val) 
	    			{
	    				?>
		    			<tr>
		    				<td align="right" width="120"><?=$buyer_short_library[$b_key];?></td>
		    				<td align="right" width="80"><?=number_format($val[1],0);?></td>
		    				<td align="right" width="80"><?=number_format($val[4],0);?></td>
		    				<td align="right" width="80"><?=number_format($val[5],0);?></td>
		    				<td align="right" width="80"><?=number_format($val[8],0);?></td>
		    				<td align="right" width="80"><?=number_format($val[0],0);?></td>
		    				<td align="right" width="80"><?=number_format($val['fob_val'],2);?></td>
		    			</tr>
		    			<?
		    			$tot_cut += $val[1];
		    			$tot_in += $val[4];
	    		...
	    		<tfoot>
	    			<tr>
	    				...
	    				<th width="80"><?=number_format($tot_cut,0);?></th>
	    				<th width="80"><?=number_format($tot_in,0);?></th>
	    				<th width="80"><?=number_format($tot_out,0);?></th>
	    				<th width="80"><?=number_format($tot_fin,0);?></th>
	    				<th width="80"><?=number_format($tot_ex,0);?></th>
	    				<th width="80"><?=number_format($tot_fob,2);?></th>
	    			</tr>
	    		</tfoot>
	    	</table>
	    </div>
	...
	else if($type==8)//F.Rcv & Gmts Prod button
	{	
		$costing_per_arr = return_library_array("select job_no, costing_per from wo_pre_cost_mst","job_no","costing_per"); 
		$tot_cost_arr = return_library_array("select job_no, cm_cost from wo_pre_cost_dtls","job_no","cm_cost"); 
		// rehan for micro
		  ...
		  $dtls_sql="SELECT a.production_date, a.po_break_down_id as po_breakdown_id, a.item_number_id,c.buyer_name,a.company_id,
			 ...
			 if($comp && !$work_comp)
			 {
			 	foreach($dtls_sql_result as $row)
			 	{
			 		...
			 		$prod_date[change_date_format($row[csf("production_date")])]['cutting_qnty_outbound']+=$row[csf("cutting_qnty_outbound")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['cutting_qnty']+=$row[csf("cutting_qnty")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewing_input_qnty']+=$row[csf("sewing_input_qnty")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewingout_qnty_inhouse']+=$row[csf("sewingout_qnty_inhouse")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewingout_qnty_outbound']+=$row[csf("sewingout_qnty_outbound")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewing_qnty']+=$row[csf("sewing_qnty")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewing_out_in_and_outbound']+=$row[csf("sewingout_qnty_inhouse")]+$row[csf("sewingout_qnty_inhouse")];
			 ...
			 else if(!$comp && $work_comp)
			 {
				...
				$subcon_sql=" SELECT c.rate,c.smv, a.order_id as po_break_down_id, a.gmts_item_id as item_number_id, a.company_id,a.production_date,  b.party_id as buyer_name,
			 	...
			 	foreach($dtls_sql_result as $row)
			 	{
			 		...
			 		$prod_date[change_date_format($row[csf("production_date")])]['cutting_qnty']+=$row[csf("cutting_qnty_inhouse")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewing_input_qnty']+=$row[csf("sewing_input_qnty_inhouse")];
			 		$prod_date[change_date_format($row[csf("production_date")])]['sewingout_qnty_inhouse']+=$row[csf("sewingout_qnty_inhouse")];
			...
			// echo "<pre>"; print_r($prod_date);die;
			 ...
			 $actual_resource_sql="SELECT a.id,b.pr_date ,sum(b.man_power) as man_power ,sum(b.helper) as helper ,sum(b.operator) as operator,sum(b.working_hour) as working_hour,sum(b.target_per_hour) as target_per_hour from prod_resource_mst a ,prod_resource_dtls b  where a.id=b.mst_id $cbo_company_cond_5 $company_working_cond_5 $location_con_rcv   and a.is_deleted=0    and b.is_deleted=0 group by a.id,b.pr_date";
			 // echo $actual_resource_sql;
			 foreach(sql_select($actual_resource_sql) as $keys=>$vals)  
			 {
			 	$resource_arrs[change_date_format($vals[csf("pr_date")])]["man_power"]+=$vals[csf("helper")]+$vals[csf("operator")];
			 	$resource_arrs[change_date_format($vals[csf("pr_date")])]["helper"]+=$vals[csf("helper")];
			 	$resource_arrs[change_date_format($vals[csf("pr_date")])]["operator"]+=$vals[csf("operator")];
			 	$resource_arrs[change_date_format($vals[csf("pr_date")])]["working_hour"]+=$vals[csf("working_hour")];
			 	$resource_arrs[change_date_format($vals[csf("pr_date")])]["target_per_hour"]+=$vals[csf("working_hour")]*$vals[csf("target_per_hour")];

			 	$prod_line[change_date_format($vals[csf("pr_date")])][$vals[csf("ID")]] = $vals[csf("ID")];
			 }
			 $subcon_sewing="SELECT a.production_date as dates, sum(b.prod_qnty) as qntys,a.order_id from subcon_gmts_prod_dtls a   ,subcon_gmts_prod_col_sz b where a.id=b.dtls_id  and a.is_deleted=0 and  a.status_active=1 and a.production_type=2 $cbo_company_cond $company_working_cond_5 $location_con_rcv group by a.production_date,a.order_id  ";
			...
			if($cbo_company==0) $cbo_company_cond_5=""; else $cbo_company_cond_5=" and a.company_id in($cbo_company)";
			if($cbo_working_company==0) $company_working_cond_2=""; else $company_working_cond_2=" and a.company_id in($cbo_working_company)";
			$finish_query="SELECT a.receive_date as production_date, sum(b.receive_qnty) as finishing_qnty from  inv_receive_master a, pro_finish_fabric_rcv_dtls b where a.id=b.mst_id $cbo_company_cond_5 $company_working_cond_2 $location_con_rcv and a.receive_date between '$date_from' and '$date_to' and a.entry_form=37 and a.item_category=2 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 group by a.receive_date";
			// echo $finish_query;die();
			$finish_query_result=sql_select($finish_query);
			$count_finish=count($finish_query_result);
			foreach( $finish_query_result as $finish_row)
			...
			// where a.delivery_mst_id=d.id and  a.po_break_down_id=b.id and b.job_no_mst=c.job_no $cbo_company_cond_6 $cbo_delivery_com_cond $location_con $delv_location_con and a.is_deleted=0 and a.status_active=1 and b.status_active in(1,3) and a.ex_factory_date between '$date_from' and '$date_to' group by a.ex_factory_date, a.po_break_down_id,c.company_name,c.buyer_name";
			 ...
			 $exfactory_res = sql_select("SELECT a.ex_factory_date, a.po_break_down_id,c.company_name,c.buyer_name,   
			 	sum(CASE WHEN a.entry_form!=85 THEN a.ex_factory_qnty ELSE 0 END)-sum(CASE WHEN a.entry_form=85 THEN a.ex_factory_qnty ELSE 0 END) as ex_factory_qnty
			 	from pro_ex_factory_mst a, wo_po_break_down b, wo_po_details_master c,pro_ex_factory_delivery_mst d
			 	where a.delivery_mst_id=d.id and  a.po_break_down_id=b.id and b.job_id=c.id $cbo_company_cond_6 $cbo_delivery_com_cond $location_con $delv_location_con and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and a.ex_factory_date between '$date_from' and '$date_to' group by a.ex_factory_date, a.po_break_down_id,c.company_name,c.buyer_name");
        ...
        <table width="1280px" cellpadding="0" cellspacing="0" id="caption" align="center">
        	<tr>
        		<td align="center" width="100%" colspan="22" class="form_caption"><strong style="font-size:18px">
        			<? if($cbo_company==0){ echo "Working Company Name:". $company_library[$cbo_working_company];} else{ 
        				$com_arr=explode(",",str_replace("'","",$cbo_company));
        ...
        <table width="1260px " border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
       	  <p style="float:left;font-size: 16px"><strong>Details Part</strong></p>
	       	 <thead>
	       	 	<tr>
                    <th width="60"><p>Date</p></th>                   
                    <th width="80"><p>Fabric Rcv Kg</p></th>            
                    <th width="80"><p>Cutting Pcs</p></th>            
                    <th width="80"><p>Input Pcs</p></th>            
                    <th width="80"><p>Total Use Operator</p></th>            
                    <th width="80"><p>Total Use Helper</p></th>            
                    <th width="80"><p>Total Manpower</p></th>            
                    <th width="80"><p>Avg. W/hrs.</p></th>            
                    <th width="80"><p>Target Pcs</p></th>            
                    <th width="80"><p>Achieved Sewing Pcs</p></th>            
                    <th width="80"><p>Avg SMV</p></th>            
                    <th width="80"><p>Target Eff. %</p></th>            
                    <th width="80"><p>Achieved Eff. %</p></th>            
                    <th width="80"><p>Earned CM $ USD</p></th>            
                    <th width="80"><p>Finishing Pcs</p></th>            
                    <th width="80"><p>Shipment Pcs</p></th> 
	       	 	</tr>
	       	 </thead>
        </table>
		<div style="max-height:420px; overflow-y:scroll; width:1280px" id="scroll_body">
	       	 <table cellspacing="0" border="1" class="rpt_table"  width="1260px" rules="all" id="scroll_body" >
		        	<?
		        	$totals_knit=0;
		        	$totals_dyeing=0;
					...
					for($j=0;$j<$datediff;$j++)
					{
						...
						$working_hour=$resource_arrs[$newdate]['working_hour'];
						$target_per_hour=$resource_arrs[$newdate]['target_per_hour'];
						$avg_working_hr = (count($prod_line[$newdate])>0) ? $working_hour/count($prod_line[$newdate]) : 0;

						$avg_smv = ($prod_date[$newdate]['sewing_qnty']>0) ? $prod_date[$newdate]['sewing_qnty_smv']/$prod_date[$newdate]['sewing_qnty'] : 0;

						$available_min = $resource_arrs[$newdate]["man_power"]*$resource_arrs[$newdate]["working_hour"]*60;
						// $acv_effi = ($available_min>0) ? ($prod_date[$newdate]['sewing_qnty_smv']/$available_min)*100 : 0;
						$target_effi = ($resource_arrs[$newdate]["man_power"]*$avg_working_hr*60>0) ? (($target_per_hour*$avg_smv) / ($resource_arrs[$newdate]["man_power"]*$avg_working_hr*60))*100 : 0;

						$produce_qty=$prod_date[$newdate]['sewingout_qnty_inhouse_pcs']/60;
						$effiecy_aff_perc=($tsmvArr[$newdate]['smv']>0) ? $produce_qty/($tsmvArr[$newdate]['smv']/60)*100 : 0;
						// echo $produce_qty."/(".$tsmvArr[$newdate]['smv']."/60)*100<br>";

						if($prod_date[$newdate]['cutting_qnty']<1 || $prod_date[$newdate]['sewing_input_qnty']<1 || $prod_date[$newdate]['sewing_qnty']<1)
						{
							$bgcolor="red";
						}
						?>
						<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
							<td width="60"><? echo $newdate; $date=$date_all;  ?>&nbsp;</td>
							<td width="80" align="right"><? echo number_format($prod_date[$newdate]['finishing_qnty'],2); ?></td>

							<td width="80" align="right"><? echo number_format($prod_date[$newdate]['cutting_qnty'],0); ?></td>
							<td width="80" align="right"><? echo number_format($prod_date[$newdate]['sewing_input_qnty'],0); ?></td>

							<td width="80" align="right"><? echo $resource_arrs[$newdate]["operator"];?> </td>
							<td width="80" align="right"><? echo $resource_arrs[$newdate]["helper"];?> </td>
							<td width="80" align="right"><? echo $resource_arrs[$newdate]["man_power"];?> </td>
							<td width="80" align="right"><? echo number_format($avg_working_hr,2);?> </td>
							<td width="80" align="right"><? echo number_format($target_per_hour,0);?> </td>
							
							<td width="80" align="right"><? echo number_format($prod_date[$newdate]['sewing_qnty'],0); ?></td>
							
							<td width="80" align="right"><? echo number_format($avg_smv,2); ?></td>
							<td width="80" align="right"><? echo number_format(($target_effi),2); ?></td>	

							<td width="80" align="right"><? echo number_format(($effiecy_aff_perc),2); ?></td>

							<td  width="80" align="right"><? $total_cm_cost=0;
									$total_sewing_cm_costs=0;
									foreach( array_filter(array_unique(explode(",",chop($po_id_ex,",")))) as $p_id)
									{
										if($po_wise_ex_fact[$newdate][$p_id]['ex_fact_qnty'])
									...
									echo $ttl_own_subcon_sewing= number_format(($sewing_cm_costs+$subcon_total_value),2);   ?></td>
							
							
							<td  width="80" align="right"><? echo number_format($prod_date[$newdate]['finish_qnty'],2); ?></td>

							<td width="80" align="right"><? $ex_factory_qty=$prod_date[$newdate]['ex_factory']; echo number_format($ex_factory_qty,2); ?></td>							 

							</tr>
						<?
							$total_fab_rcv_qty+=$prod_date[$newdate]['finishing_qnty'];
							$total_cut_qty+=$prod_date[$newdate]['cutting_qnty'];
							$total_input_qty+=$prod_date[$newdate]['sewing_input_qnty'];
							$total_out_qty+=$prod_date[$newdate]['sewing_qnty'];
							$total_fin_qty+=$prod_date[$newdate]['finish_qnty'];
							$total_ex_qty+=$prod_date[$newdate]['ex_factory'];
							$total_helpers+= $resource_arrs[$newdate]["helper"];
							$total_operator+=$resource_arrs[$newdate]["operator"];
							$total_manpower+=$resource_arrs[$newdate]["man_power"];
							$total_target+=$target_per_hour;
							$total_sewing_out_inbound_subcon+=$subcon_total_value;
							$total_earn_cm+=$sewing_cm_costs+$subcon_total_value;
							$total_earning_cost+=$earnings_cost;
							$total_sewing_out_in_and_outbound+=$sewing_cm_costs;
							$total_sewing_out_with_subcon+=$sewing_cm_costs+$subcon_total_value;
							if($prod_date[$newdate]['kniting_qnty']){$avg_knit++;}
		        			...
		        			$tot_avg_working_hr += $avg_working_hr;
		        			$sewing_qnty_smv += $prod_date[$newdate]['sewing_qnty_smv'];
		        			$tot_produce_qty += $produce_qty;
		        			$tot_smv += $tsmvArr[$newdate]['smv'];

							$i++;
					}

					$total_avg_smv = ($total_out_qty>0) ? $sewing_qnty_smv/$total_out_qty : 0;
					$total_achv_effi = ($total_manpower*$tot_avg_working_hr*60>0) ? (($total_target*$total_avg_smv) / ($total_manpower*$tot_avg_working_hr*60))*100 : 0;

					$total_target_effi=($tot_smv>0) ? $tot_produce_qty/($tot_smv/60)*100 : 0;
					?>
	  		 </table>
	    	<table width="1260px " border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
		        <tfoot>
		            <tr>
		                ...
		                <th width="80"><?=number_format($total_cut_qty,0);?></th>            
		                <th width="80"><?=number_format($total_input_qty,0);?></th>            
		                <th width="80"><?=number_format($total_operator,0);?></th>            
		                <th width="80"><?=number_format($total_helpers,0);?></th>            
		                <th width="80"><?=number_format($total_manpower,0);?></th>            
		                <th width="80"><?//=number_format($total_wo_hr,2);?></th>            
		                <th width="80"><?=number_format($total_target,0);?></th>            
		                <th width="80"><?=number_format($total_out_qty,0);?></th>            
		            ...
		            <tr>                
		                ...
		                <th width="80"><?=number_format(($total_cut_qty/$datediff),0);?></th>            
		                <th width="80"><?=number_format(($total_input_qty/$datediff),0);?></th>            
		                <th width="80"><?=number_format(($total_operator/$datediff),0);?></th>            
		                <th width="80"><?=number_format(($total_helpers/$datediff),0);?></th>            
		                <th width="80"><?=number_format(($total_manpower/$datediff),0);?></th>            
		                <th width="80"><?=number_format(($total_wo_hr/$datediff),2);?></th>            
		                <th width="80"><?=number_format(($total_target/$datediff),0);?></th>            
		                <th width="80"><?=number_format(($total_out_qty/$datediff),0);?></th>            
		    ...
		    <table width="780px " border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all"> 
      			<p style="float:left"><strong>Buyer Wise Summary Part</strong></p>
           			<thead>
            			<tr>
							<th width="30">SL</th>
       		...
       		<div style="max-height:420px; overflow-y:scroll; width:800px" id="scroll_body_summery">
       			<table cellspacing="0" border="1" class="rpt_table"  width="780px" rules="all" id="scroll_body_summery" >
			        <?
					$inc=1;
					$all_po_ids="";
					...
					foreach($prod_date_buyer_wise as $companyKey =>$comp_value)
					{
						foreach($comp_value as $buyerKey =>$buyer_value)
						{
							...
							<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('trs_<? echo $inc; ?>','<? echo $bgcolor; ?>')" id="trs_<? echo $inc; ?>">
								...
								<td width="100" align="right"><? echo number_format($prod_date_buyer_wise[$companyKey][$buyerKey]['sewing_input_qnty'],2); $smry_tot_in_qnty+=$prod_date_buyer_wise[$companyKey][$buyerKey]['sewing_input_qnty']; ?></td>
	        		...
	        		<tfoot>
	            		<tr>
							...
							<th width="150">Total</th>
							<th width="100"><? echo number_format($smry_tot_cutting_qnty,2); ?></th>
							<th width="100"><? echo number_format($smry_tot_in_qnty,2); ?></th>
							<th width="100"><? echo number_format($smry_tot_sewing_qnty,2); ?></th>
							<th width="100"><? echo number_format($smry_tot_finish_qnty,2); ?></th>
							<th width="100"><? echo number_format($smry_tot_ex_factory,2); ?></th>
							<th width="100"><? echo number_format($smry_tot_ex_factory_fob_val,2); ?></th>
	  	...
	  	<table style="margin:0px auto;" cellspacing="0" cellpadding="0" border="1" rules="all" width="680px" class="rpt_table">
			...
			<tbody>
				...
				foreach($po_data as $po_id=>$buyer_data)
				{
					foreach($buyer_data as $buyer_id=>$rows)
					{
						...
						<tr>
                            ...
                            <td align="center"><? echo number_format($rows['qnty'],2); ?></td> 
				...
				<tr style="background-color: #E4E4E4;">
					...
					<td align="center"><b><? echo number_format($subTotal['qnty'],2); ?></b></td>
			...
			<tr style="background-color: #E4E4E4;">
				...
				<td align="center"><b><? echo number_format($grandTotal['qnty'],2); ?></b></td>
...
if($action=="cutting_inhouse") 
{
    ...
    <?
        ...
        foreach ($cutting_query_result as $row)  
        {
			...
			<tr bgcolor="<? echo $bgcolor;  ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
            	<td width="30px"><? echo $i; ?></td>
                <td width="70px" class="wrd_brk"><? echo $po_array[$row[csf('po_break_down_id')]]['job']; ?></td>
                <td width="100px" class="wrd_brk"><? echo $po_array[$row[csf('po_break_down_id')]]['po']; ?></td>
                <td width="100px" class="wrd_brk"><? echo $buyerArr[$po_array[$row[csf('po_break_down_id')]]['buyer']]; ?></td>
                <td width="120px" class="wrd_brk"><? echo $po_array[$row[csf('po_break_down_id')]]['style']; ?></td>
                <td width="90px" align="right"><? echo number_format($row[csf('production_quantity')],2); $total_cutting_qnty+=$row[csf('production_quantity')]; ?></td>
			</tr>
			<?
			$i++;
...
if($action=="cutting") 
{
    ...
    <?
        ...
        foreach ($cutting_query_result as $row)  
        {
			...
			<tr bgcolor="<? echo $bgcolor;  ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
            	<td width="30px"><? echo $i; ?></td>
                <td class="wrd_brk" width="70px"><? echo $po_array[$row[csf('po_break_down_id')]]['job']; ?></td>
                <td class="wrd_brk" width="100px"><? echo $po_array[$row[csf('po_break_down_id')]]['po']; ?></td>
                <td class="wrd_brk" width="100px"><? echo $buyerArr[$po_array[$row[csf('po_break_down_id')]]['buyer']]; ?></td>
                <td class="wrd_brk" width="120px"><? echo $po_array[$row[csf('po_break_down_id')]]['style'] ?></td>
                <td width="90px" align="right"><? echo number_format($row[csf('production_quantity')],2); $total_cutting_qnty+=$row[csf('production_quantity')]; ?></td>
			</tr>
			<?
			$i++;
        ...
        foreach ($cutting_query_result2 as $row)  
        {
			...
			<tr bgcolor="<? echo $bgcolor;  ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
            	<td width="30px"><? echo $i; ?></td>
                <td class="wrd_brk" width="70px"><? echo $po_array2[$row[csf('po_break_down_id')]]['job']; ?></td>
                <td class="wrd_brk" width="100px"><? echo $po_array2[$row[csf('po_break_down_id')]]['po']; ?></td>
                <td class="wrd_brk" width="100px"><? echo $buyerArr[$po_array2[$row[csf('po_break_down_id')]]['buyer']]; ?></td>
                <td class="wrd_brk" width="120px"><? echo $po_array2[$row[csf('po_break_down_id')]]['style'] ?></td>
                <td width="90px" align="right"><? echo number_format($row[csf('production_quantity')],2); $total_cutting_qnty+=$row[csf('production_quantity')]; ?></td>
			</tr>
			<?
			$i++;
    ...
    <?
        ...
        foreach ($exfactory_query_result as $row)  
        {
			...
			<tr bgcolor="<? echo $bgcolor;  ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
            	<td width="30px"><? echo $i; ?></td>
                <td class="wrd_brk" width="70px"><? echo $po_array[$row[csf('po_break_down_id')]]['job']; ?></td>
                <td class="wrd_brk" width="120px"><? echo $buyerArr[$po_array[$row[csf('po_break_down_id')]]['buyer']]; ?></td>
                <td class="wrd_brk" width="120px"><? echo $po_array[$row[csf('po_break_down_id')]]['style']; ?></td>
                <td class="wrd_brk" width="120px"><? echo $po_array[$row[csf('po_break_down_id')]]['po'];; ?></td>
    ...
    <?
        ...
        foreach ($ord_sew_inhouse_sql_result as $row)  
        {
			...
			<tr bgcolor="<? echo $bgcolor;  ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
            	<td width="30px"><? echo $i; ?></td>
                <td class="wrd_brk" width="70px"><? echo $po_array[$row[csf('po_break_down_id')]]['job']; ?></td>
                <td class="wrd_brk" width="110px"><? echo $buyerArr[$po_array[$row[csf('po_break_down_id')]]['buyer']]; ?></td>
                <td class="wrd_brk" width="110px"><? echo $po_array[$row[csf('po_break_down_id')]]['style']; ?></td>
                <td class="wrd_brk" width="110px"><? echo $po_array[$row[csf('po_break_down_id')]]['po'];; ?></td>
...
if($action=="smv_sewing_popup") 
{
    ...
    <?
        ...
        foreach ($ord_sew_inhouse_sql_result as $row)  
        {
			...
			<tr bgcolor="<? echo $bgcolor;  ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
            	<td width="30"><? echo $i; ?></td>
                <td class="wrd_brk" width="60"><? echo $po_array[$row[csf('po_break_down_id')]]['job']; ?></td>
                <td class="wrd_brk" width="50" align="right"><? echo $item_smv_array[$row[csf('po_break_down_id')]][$row[csf('item_number_id')]]['smv_pcs']; ?></td>
                <td class="wrd_brk" width="110"><? echo $buyerArr[$po_array[$row[csf('po_break_down_id')]]['buyer']]; ?></td>
                <td class="wrd_brk" width="110"><? echo $po_array[$row[csf('po_break_down_id')]]['style']; ?></td>
                <td class="wrd_brk" width="100"><? echo $garments_item[$row[csf('item_number_id')]]; ?></td>
                <td class="wrd_brk" width="110"><? echo $po_array[$row[csf('po_break_down_id')]]['po']; ?></td>
