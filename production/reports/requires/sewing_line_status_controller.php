...
{	
	...
	// echo $prod_start_hour.":00";
	/* $time1 = strtotime($prod_start_hour.":00");
	$time2 = strtotime(date('H:i:s'));
	$difference_hour = round(abs($time2 - $time1) / 3600,2); */
	...
	if(substr($global_start_lanch,0,2) < $time2)
	{
		...
		$difference_hour = (int) $time2 - $time1;
	...
	/										smv sorce 										/
	/===================================================================================== */
	if($smvsourcefor_charka==1)//ISD-23-06058
	{
		$jovCondForsmvWs=where_con_using_array($all_job_arr,0,"a.PO_JOB_NO");
		
		$sql_item="SELECT a.id, a.PO_JOB_NO, A.TOTAL_SMV, A.STYLE_REF, a.GMTS_ITEM_ID from PPL_GSD_ENTRY_MST a where a.BULLETIN_TYPE=4 and A.IS_DELETED=0 and A.STATUS_ACTIVE=1 and a.APPROVED=1 $jovCondForsmvWs
	...
	$td_width = round(($tbl_width-715)/$tot_td);
                	...
                	foreach($production_serial_arr as $floor_id=>$flr_data) 
                	{
	                	...
	                	foreach($flr_data as $sl=>$sl_data) 
	                	{
							...
							{   
								...
								{
									$difference_hour = $prod_resource_array[$l_id]['working_hour'];
									// $difference_hour = $line_last_prod_hour_array[$l_id]-1; // avoid lunch hour 
									// $difference_hour = $line_last_prod_hour_array[$l_id];
								}
			                   ...
			                   <tr bgcolor="<? echo $bgcolor;?>" id="tr_<?= $i;?>" onClick="change_color('tr_<?= $i; ?>','<?= $bgcolor; ?>')" title="<?=$title;?>">
				                    ...
				                    <td  bgcolor="<?=$color;?>" style="<?=$style;?>" align="right" width="60" title="<?="((".$line_tot."/(".$prod_resource_array[$l_id]['terget_hour']."*".$difference_hour."))*100)";?>">
									<?=($line_tot>0) ? number_format((($line_tot/($prod_resource_array[$l_id]['terget_hour']*$difference_hour))*100),0) : 0;?>%
								</td>                   
			            ...
			            <tr style="background: #1F1D36;text-align: right;">
	                        <td colspan="3" width="135"><?=$floorArr[$floor_id];?></td>
                ...
                <tfoot>
                   <tr style="background: #1F1D36;">
						...
						for($k=$hour; $k<=$last_hour; $k++)
						{
							...
							{
								?>
			                      <th align="right" width="<?=$td_width;?>"><? echo number_format($total_arr[$prod_hour],0); ?></th>
								<?	
							}
							/*elseif ($total_production[$prod_hour]>0 && $p>11) 
							{
