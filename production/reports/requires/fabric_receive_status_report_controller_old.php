...
{ 
 	...
 	if($template==1)
	{
		...
		if(str_replace("'","",trim($txt_file_no))!="") $file_no=" and LOWER(b.file_no) like LOWER('%".str_replace("'","",trim($txt_file_no))."%')"; else $file_no="";
		if(str_replace("'","",trim($txt_ref_no))!="") $ref_no=" and LOWER(b.grouping) like LOWER('%".str_replace("'","",trim($txt_ref_no))."%')"; else $ref_no="";
		...
		$yarn_sql="select job_no, count_id, copm_one_id, percent_one, copm_two_id, percent_two, type_id, sum(avg_cons_qnty) as qnty from wo_pre_cost_fab_yarn_cost_dtls where status_active=1 and is_deleted=0 group by job_no, count_id, copm_one_id, percent_one, copm_two_id, percent_two, type_id";//, sum(cons_qnty) as qnty
		...
		$dataArrayWo=array(); $colorWiseReqQty_arr=array();
		...
		$tot_fabric_available=0; $tot_fabric_left_over=0; $tot_knit_balance_qnty=0; $tot_grey_inhand=0; $tot_grey_issue_bl=0;
		...
		if($type==1)
		{
			...
			where a.job_no=b.job_no_mst and a.company_name='$company_name' and LOWER(b.po_number) like LOWER('$search_string') and b.shiping_status like '$shipping_status' and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $buyer_id_cond $job_no_cond $str_cond $str_cond_insert $year_cond $file_no $ref_no order by b.pub_shipment_date, a.job_no_prefix_num, b.id";
		}
		else
		{
			$table_width="4070"; $colspan="12";
			
			if($db_type==0)
			{
				$sql="select a.company_name, a.buyer_name, a.job_no_prefix_num, a.job_no, a.style_ref_no, a.gmts_item_id, a.order_uom, a.total_set_qnty as ratio, group_concat(b.id) as po_id, group_concat(b.po_number) as po_number, sum(b.po_quantity) as po_qnty, sum(b.plan_cut) as plan_cut 
				from wo_po_details_master a, wo_po_break_down b 
				where a.job_no=b.job_no_mst and a.company_name='$company_name' and LOWER(a.style_ref_no) like LOWER('$search_string') and b.shiping_status like '$shipping_status' and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $buyer_id_cond $job_no_cond $str_cond $str_cond_insert $year_cond group by a.job_no";
			}
			else
			{
				$sql="select a.company_name, a.buyer_name, a.job_no_prefix_num, a.job_no, a.style_ref_no, a.gmts_item_id, a.order_uom, a.total_set_qnty as ratio, LISTAGG(cast(b.id as varchar2(4000)), ',') WITHIN GROUP (ORDER BY b.id) as po_id, LISTAGG(cast(b.po_number as varchar2(4000)), ',') WITHIN GROUP (ORDER BY b.id) as po_number, sum(b.po_quantity) as po_qnty, sum(b.plan_cut) as plan_cut 
				from wo_po_details_master a, wo_po_break_down b 
        ...
        <fieldset style="width:<? echo $table_width+30; ?>px;">	
            ...
            <table class="rpt_table" border="1" rules="all" width="<? echo $table_width; ?>" cellpadding="0" cellspacing="0" id="table_header_1">
                <thead>
                    <tr>
                        ...
                        <th colspan="11">Grey Fabric Status</th>
                    ...
                    <tr>
                        ...
                        <th width="100">Balance<br/><font style="font-size:9px; font-weight:100">(Grey Req-(Yarn Issue+ Net Trans+ Purchase B.))</font></th>
                        <th width="100">Required<br/><font style="font-size:9px; font-weight:100">(As Per Booking)</font></th>
                        <th width="100">Grey Production</th>
                        <th width="100">Knitting Balance<br/><font style="font-size:9px; font-weight:100">( Req. As Booking - (Grey Prod. + Purchase B.))</font></th>
                        <th width="100">Grey Recv./ Purchase</th>
                        <th width="100">Net Transfer</th>
                        <th width="100">Grey Available</th>
                        <th width="100">Grey Balance</th>
                        <th width="100">Grey Issue</th>
                        <th width="100">Grey In Hand<br/><font style="font-size:9px; font-weight:100">(Grey Available - Grey Issue)</font></th>
                        <th width="100">Grey Issue Bal.<br/><font style="font-size:9px; font-weight:100">(Required-Grey Issue)</font></th>
            ...
            <?
				...
				$html.="</table>
					<table border='1' rules='all'>
						<thead>
							<tr>
								<th colspan='$colspan_excel_dtls'>Order Details</th>
								<th colspan='7'>Yarn Status</th>
								<th colspan='11'>Grey Fabric Status</th>
								<th colspan='11'>Finish Fabric Status</th>
							</tr>
				...
				$html.="<th>Count</th>
						...
						<th>Issued</th>
						<th>Net Transfer</th>
						<th>Balance<br/><font style='font-size:9px; font-weight:100'>(Grey Req-(Yarn Issue+ Net Trans+ Purchase B.))</font></th>
						<th>Required<br/><font style='font-size:9px; font-weight:100'>(As Per Booking)</font></th>
						<th>Grey Production</th>
						<th>Knitting Balance<br/><font style='font-size:9px; font-weight:100'>( Req. As Booking - (Grey Prod. + Purchase B.))</font></th>
						<th>Grey Recv./ Purchase</th>
						<th>Net Transfer</th>
						<th>Grey Available</th>
						<th>Grey Balance</th>
						<th>Grey Issue</th>
						<th>Grey In Hand<br/><font style='font-size:9px; font-weight:100'>(Grey Available - Grey Issue)</font></th>
                        <th>Grey Issue Bal.<br/><font style='font-size:9px; font-weight:100'>(Required-Grey Issue)</font></th>
						<th>Batch Qnty</th>
						<th>Fabric Color</th>
						<th>Required</th>
						<th>Dye Qnty</th>
            ...
            <div style="width:<? echo $table_width+20; ?>px; overflow-y:scroll; max-height:400px" id="scroll_body">
				...
				if($type==1)
				{
					...
					if($chk_no_boking==1)// check no booking 
					{
						...
						{
							$nobooking_check=array_filter(explode(",",substr($dataArrayWo[$row[csf('po_id')]],0,-1)));
							if(count($nobooking_check)<1)
							{
								...
								if(($cbo_discrepancy==1 && $grey_recv_qnty>$yarn_issued) || ($cbo_discrepancy==0))
								{
									...
									$required_qnty=0;$req_purc_qnty=0;$main_booking=''; $sample_booking=''; $main_booking_excel=''; $sample_booking_excel='';$all_book_prefix_no='';
										...
										foreach($dataArray as $woRow)
										{
											...
											if(!in_array($id,$booking_array))
											{
												...
												if($booking_type==4)
												{
													...
													$sample_booking.="<a href='##' style='color:#000' onclick=\"generate_worder_report('3','".$booking_no."','".$company_id."','".$row[csf('po_id')]."','".$item_category."','".$fabric_source."','".$job_no."','".$is_approved."','".$action_name."')\"><font style='font-weight:bold' $wo_color>".$booking_no."(".$system_date.")"."</font></a><br>";
												...
												else
												{
													...
													$main_booking.="<a href='##' style='color:#000' onclick=\"generate_worder_report('".$is_short."','".$booking_no."','".$company_id."','".$row[csf('po_id')]."','".$item_category."','".$fabric_source."','".$job_no."','".$is_approved."','".$action_name."')\"><font style='font-weight:bold' $wo_color>".$booking_no."(".$system_date.")".$pre."</font></a><br>";
									...
									if($tot_color>0)
									{
										...
										?>
											<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $k; ?>">
												...
												<td width="125"><p><? echo $display_font_color.$main_booking.$font_end; ?></p></td>
												<td width="125"><p><? echo $display_font_color.$sample_booking.$font_end; ?></p></td>
                                                <td width="50"><p><? echo $display_font_color.$all_book_prefix_no.$font_end; ?></p></td>
												<td width="100" align="center"><p><? echo $display_font_color.$row[csf('job_no')].$font_end; ?></p></td>
												<td width="120">
													<p>
												...
												<td width="40" onclick="openmypage_image('requires/fabric_receive_status_report_controller.php?action=show_image&job_no=<? echo $row[csf('job_no')] ?>','Image View')" style="cursor:pointer;"><p><img  src='../../<? echo $imge_arr[$row[csf('job_no')]]; ?>' height='25' width='40' /></p></td>
												...
												<td width="100" align="right">
													<? 
														if($z==1)
														{
															echo number_format($knit_balance_qnty,2);
															$html.=number_format($knit_balance_qnty,2);
														}
													?>
												</td>
												<? $html.="</td><td>"; ?>
												...
												<td width="100" align="right">
													<? 
														if($z==1)
														{
															echo number_format($grey_inhand_qnty,2);
															$html.=number_format($grey_inhand_qnty,2);
														}
													?>
												</td>
												<? $html.="</td><td>"; ?>
												<td width="100" align="right">
													<? 
														if($z==1)
														{
															echo number_format($grey_iss_balance_qnty,2);
															$html.=number_format($grey_iss_balance_qnty,2);
														}
													?>
												</td>
												<? $html.="</td><td>"; ?>
									...
									?>
										<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $k; ?>">
                                            ...
                                            <td width="50"><p><? echo $all_book_prefix_no; ?></p></td>
											<td width="100" align="center"><? echo $row[csf('job_no')]; ?></td>
											<td width="120">
												<p>
													<a href='#report_details' onclick="progress_comment_popup('<? echo $row[csf('job_no')]; ?>','<? echo $row[csf('po_id')]; ?>','<? echo $template_id; ?>','<? echo $tna_process_type; ?>');"><? echo $row[csf('po_number')];//$display_font_color.$row[csf('po_number')].$font_end;  ?></a>
												</p>
											</td>
											...
											<td width="100" align="right"><? echo number_format($knit_balance_qnty,2,'.',''); ?></td>
											...
											<td width="100" align="right"><a  href="##" onclick="open_febric_receive_status_order_wise_popup('<? echo $row[csf('po_id')]; ?>','grey_issue','')"><? echo number_format($grey_fabric_issue,2,'.',''); ?></a></td>
											<td width="100" align="right"><? echo number_format($grey_inhand_qnty,2,'.',''); ?></td>
											<td width="100" align="right"><? echo number_format($grey_iss_balance_qnty,2,'.',''); ?></td>
										...
										<?	
											...
											<td>".number_format($knit_balance_qnty,2)."</td>
											...
											<td>".number_format($grey_inhand_qnty,2)."</td>
											<td>".number_format($grey_iss_balance_qnty,2)."</td>
									...
									foreach($dataArray as $woRow)
									{
										...
										if(!in_array($id,$booking_array))
										{
											...
											else
											{
												...
												if($is_short==1) 
												{
													if($item_category==3) $pre="W"; else $pre="S";
													$action_name=$report_format_arr[$booking_print_arr[2]];
												}
												else 
												{
													if($item_category==3) $pre="W"; else $pre="M"; 
													$action_name=$report_format_arr[$booking_print_arr[1]];
												}
								...
								if($tot_color>0)
								{
									...
									?>
										<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $k; ?>">
											...
											<td width="100" align="right">
												<? 
													$color_wise_grey_qty=0;
													$color_wise_grey_qty=$colorWiseReqQty_arr[$row[csf('po_id')]][$key];
													echo number_format($color_wise_grey_qty,2,'.','');
													$html.=number_format($color_wise_grey_qty,2);
													
													$fin_fab_Requi_array[$row[csf('buyer_name')]]+=$color_wise_grey_qty;
													$tot_color_wise_req+=$color_wise_grey_qty; 
												?>
											</td>
											<? 
												$html.="</td><td>"; $html_short.="<td>"; 
				...
				else
				{
					...
					if($chk_no_boking==1)// check no booking 
					{
						foreach($nameArray as $row)
						{
							$check_job_po_id=explode(",",$row[csf('po_id')]);
								...
								if(($cbo_discrepancy==1 && $grey_recv_qnty>$yarn_issued) || ($cbo_discrepancy==0))
								{
									...
									if($tot_color>0)
									{
										...
										?>
											<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $k; ?>">
												...
												<td width="125"><? echo $display_font_color.$main_booking.$font_end; ?></td>
												<td width="125"><? echo $display_font_color.$sample_booking.$font_end; ?></td>
                                                <td width="50"><? echo $display_font_color.$all_book_prefix_no.$font_end; ?></td>
												<td width="100" align="center"><? echo $display_font_color.$row[csf('job_no')].$font_end; ?></td>
									...
									?>
										<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $k; ?>">
											...
											<td width="125"><? echo $main_booking; ?></td>
											<td width="125"><? echo $sample_booking; ?></td>
                                            <td width="50"><? echo $all_book_prefix_no; ?></td>
											<td width="100" align="center"><? echo $row[csf('job_no')]; ?></td>
					...
					else
					{
						foreach($nameArray as $row)
						{
							...
							$grey_purchase_qnty=0; $grey_recv_qnty=0; $grey_fabric_issue=0; $booking_data='';$all_book_prefix_no='';
							$job_po_id=explode(",",$row[csf('po_id')]);
							foreach($job_po_id as $po_id)
							{
								$yarn_descrip_data=$yarn_des_data[$po_id];
            ...
            <?
				...
				$html.="<th></th>
							...
							<th align='right'>".number_format($tot_knit_balance_qnty,2)."</th>
							...
							<th align='right'>".number_format($tot_grey_inhand,2)."</th>
							<th align='right'>".number_format($tot_grey_issue_bl,2)."</th>
            ...
            <table width="<? echo $table_width; ?>" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all">
                <tfoot>
                    <tr>
                        ...
                        <th width="100" id="value_tot_knit_bl"><? echo number_format($tot_knit_balance_qnty,2); ?></th>
                        ...
                        <th width="100" id="value_tot_grey_inhand"><? echo number_format($tot_grey_inhand,2); ?></th>
                        <th width="100" id="value_tot_grey_issue_bl"><? echo number_format($tot_grey_issue_bl,2); ?></th>
                ...
                <tr valign="top">
                            ...
                            <table width="1720" class="rpt_table" border="0" rules="all" align="center">
                                <thead>
                                    ...
                                    <tr align="center" id="date_td" style="visibility:hidden;border:none">
                                         ...
                                         <? $file=str_replace("'",'',$txt_file_no);if($file) echo 'File No: '.$file;?>
                                         </span>
                                         <font size="3"><? echo "From ". change_date_format($start_date). " To ". change_date_format($end_date);?></font>
                                         </th>
                                    </tr>
                                    <tr align="center">
                                        <th colspan="17">Buyer Level Summary</th>
                                ...
                                foreach($buyer_name_array as $key=>$value)
                                {
                                    ...
                                    $html.="<tr bgcolor='$bgcolor'>
                                            ...
                                            <td align='right'>".$order_qty_array[$key]."</td>
                                        ...
                                        ";
                                    ?>
                                    <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr2_<? echo $b_sl; ?>','<? echo $bgcolor; ?>')" id="tr2_<? echo $b_sl; ?>">
                                        <td width="40"><? echo $b_sl ;?></td>
                                        <td width="130"><? echo $value ;?></td>
                                        <td width="100" align="right"><? echo $order_qty_array[$key]; $order_qty_array_tot+=$order_qty_array[$key]; ?></td>
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <th width="100" align="right"><? echo $order_qty_array_tot; ?></th>
                    ...
                    <?
                        $html.="</tbody>
                                ...
                                <th align='right'>".$order_qty_array_tot."</th>
                    ...
                    <td>
                        <table width="600" class="rpt_table" border="1" rules="all">
                            ...
                            <tbody>
                                ...
                                <tr bgcolor="#CFF" style="font-weight:bold" onclick="change_color('tr3_3','#CFF')" id="tr3_3">
                                   <td>Total Yarn Balance</td>
                                ...
                                <tr bgcolor="#FFFFFF" onclick="change_color('tr3_4','#FFFFFF')" id="tr3_4">
                                    <td>Total Grey Fabric Required</td>
                                   <td align="right"><? echo number_format($tot_fabric_req,2); ?></td>
                                   <td>&nbsp;</td>
                                </tr>
                                <tr bgcolor="#FFFFFF" onclick="change_color('tr3_5','#FFFFFF')" id="tr3_5">
                                   <td>Total Grey Fabric Available</td>
                                ...
                                <tr bgcolor="#FFFFFF" onclick="change_color('tr3_6','#FFFFFF')" id="tr3_6">
                                   <td>Total Grey Fabric Issued To Dye</td>
                                ...
                                <tr bgcolor="#CFF" style="font-weight:bold" onclick="change_color('tr3_7','#CFF')" id="tr3_7">
                                   <td>Total Grey Fabric Issue Balance</td>
                                ...
                                <tr bgcolor="#FFFFFF" onclick="change_color('tr3_8','#FFFFFF')" id="tr3_8">
                                    <td>Total Batch Qnty</td>
                                    <td align="right"> <? echo number_format($tot_batch_qnty,2); ?></td>
                                    <td align="right">&nbsp;</td>
                                </tr>
                                <tr bgcolor="#CFF" style="font-weight:bold" onclick="change_color('tr3_9','#CFF')" id="tr3_9">
                                    <td>Total Batch Balance To Grey Required</td>
                                    <td align="right"> <? $tot_batch_balance=$tot_fabric_req-$tot_batch_qnty; echo number_format($tot_batch_balance,2); ?></td>
                                    <td align="right">&nbsp;</td>
                                </tr>
                                <tr bgcolor="#FFFFFF" onclick="change_color('tr3_10','#FFFFFF')" id="tr3_10">
                                    <td>Total Dye Qnty</td>
                                ...
                                <tr bgcolor="#CFF" style="font-weight:bold" onclick="change_color('tr3_11','#CFF')" id="tr3_11">
                                    <td>Total Dye Balance To Grey Required</td>
                                ...
                                <tr bgcolor="#FFFFFF" onclick="change_color('tr3_12','#FFFFFF')" id="tr3_12">
                                   <td>Total Finish Fabric Required</td>
                                   <td align="right"><? echo number_format($tot_color_wise_req,2); ?></td>
                                   <td>&nbsp;</td>
                                </tr>
                                <tr bgcolor="#FFFFFF" onclick="change_color('tr3_13','#FFFFFF')" id="tr3_13">
                                ...
                                <tr bgcolor="#CFF" style="font-weight:bold" onclick="change_color('tr3_14','#CFF')" id="tr3_14">
                                   <td>Total Finish Fabric Balance</td>
                                ...
                                <tr bgcolor="#FFFFFF" onclick="change_color('tr3_15','#FFFFFF')" id="tr3_15">
                                   <td>Total Finish Fabric Issued To Cut</td>
	...
	$filename="../../ext_resource/tmp_report/".$user_name."_".$name.".xls";
	$filename_short="../../ext_resource/tmp_report/".$user_name."_".$name."short.xls";
	echo "$total_data####$filename####$filename_short####1";
	exit();
...
{ 
 	...
 	if($template==1)
	{
		...
		$dataArrayTrans_issue=sql_select("select a.po_breakdown_id, b.color_id, a.quantity
								from order_wise_pro_details a, inv_grey_fabric_issue_dtls b  where a.dtls_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form in(16,61)");
		foreach($dataArrayTrans_issue as $row)
		{
			$grey_issue_qnty_arr[$row[csf('po_breakdown_id')]][$row[csf('color_id')]]+=$row[csf('quantity')];
		}
		...
		$dataArrayWo=array();
		$sql_wo="select b.po_break_down_id, a.id, a.booking_no, a.booking_no_prefix_num, a.insert_date, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.booking_no_prefix_num, a.job_no, a.is_short, a.is_approved, b.fabric_color_id, sum(b.fin_fab_qnty) as req_qnty, sum(b.grey_fab_qnty) as grey_req_qnty from wo_booking_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and a.item_category in(2,3,13) and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $color_cond_search group by b.po_break_down_id, a.id, a.booking_no, a.booking_no_prefix_num, a.insert_date, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.booking_no_prefix_num, a.job_no, a.is_short, a.is_approved, b.fabric_color_id";
		$resultWo=sql_select($sql_wo); $colorWiseReqQty_arr=array();
		foreach($resultWo as $woRow)
		{
			$dataArrayWo[$woRow[csf('po_break_down_id')]].=$woRow[csf('id')]."**".$woRow[csf('booking_no')]."**".$woRow[csf('insert_date')]."**".$woRow[csf('item_category')]."**".$woRow[csf('fabric_source')]."**".$woRow[csf('company_id')]."**".$woRow[csf('booking_type')]."**".$woRow[csf('booking_no_prefix_num')]."**".$woRow[csf('job_no')]."**".$woRow[csf('is_short')]."**".$woRow[csf('is_approved')]."**".$woRow[csf('fabric_color_id')]."**".$woRow[csf('req_qnty')]."**".$woRow[csf('grey_req_qnty')]."**".$woRow[csf('booking_no_prefix_num')].",";
			$colorWiseReqQty_arr[$woRow[csf('po_break_down_id')]][$woRow[csf('fabric_color_id')]]+=$woRow[csf('grey_req_qnty')];
        ...
        <fieldset style="width:<? echo $table_width+30; ?>px;">	
            ...
            <table class="rpt_table" border="1" rules="all" width="<? echo $table_width; ?>" cellpadding="0" cellspacing="0" id="table_header_1">
                <thead>
                    <tr>
                        <th colspan="<? echo $colspan; ?>">Order Details</th>
                        <th colspan="7">Yarn Status</th>
                        <th colspan="9">Grey Fabric Status</th>
                        <th>Fabric Color</th>
                        <th colspan="3">Grey Issue Status</th>
								...
								if(($cbo_discrepancy==1 && $grey_recv_qnty>$yarn_issued) || ($cbo_discrepancy==0))
								{
									...
									if(count($dataArray)>0)
									{
...

										{
											...
											$color_data_array[$fabric_color_id]+=$req_qnty;
											$color_grey_data_array[$fabric_color_id]+=$grey_req_qnty;
											$grey_fabric_issue+=$grey_issue_qnty_arr[$row[csf('po_id')]][$fabric_color_id];
									...
									if($tot_color>0)
									{
										...
										?>
											<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $k; ?>">
												...
												<td width="80"><p><? echo $display_font_color.$buyer_short_name_library[$row[csf('buyer_name')]].$font_end; ?></p></td>
												<td width="130"><p><a  href="##" onclick="open_febric_receive_status_order_wise_popup('<? echo $row[csf('po_id')]; ?>','grey_issue','')"><? echo $display_font_color.$row[csf('style_ref_no')].$font_end; ?></a></p></td>
												<td width="100"><p><? echo $display_font_color.$row[csf('file_no')].$font_end; ?></p></td>
												<td width="100"><p><? echo $display_font_color.$row[csf('grouping')].$font_end; ?></p></td>
												<td width="40" onclick="openmypage_image('requires/fabric_receive_status_report_controller.php?action=show_image&job_no=<? echo $row[csf('job_no')] ?>','Image View')" style="cursor:pointer;"><p><img  src='../../<? echo $imge_arr[$row[csf('job_no')]]; ?>' height='25' width='40' /></p></td>
												<td width="140"><p><? echo $display_font_color.$gmts_item.$font_end; ?></p></td>
												...
												<td width="100" align="right">
													<? 
														...
														echo number_format($color_wise_grey_qty,2,'.','');
														$html.=number_format($color_wise_grey_qty,2);
														$grey_fab_Requi_array[$row[csf('buyer_name')]]+=$color_wise_grey_qty;
														$tot_color_wise_grey_req+=$color_wise_grey_qty; 
													?>
												</td>
                                                <? $html.="</td><td>"; ?>
												<td width="100" align="right" title="Grey Issue Return Not handled In This Report"><a  href="##" onclick="open_febric_receive_status_order_wise_popup('<? echo $row[csf('po_id')]; ?>','grey_issue','<? echo $key; ?>')"><? echo number_format($grey_issue_qnty_arr[$row[csf('po_id')]][$key],2,'.',''); $html.=number_format($grey_issue_qnty_arr[$row[csf('po_id')]][$key],2); ?></a>
													<? 
														if($z==1)
														{
														?>
									...
									?>
										<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $k; ?>">
											...
											<td width="80"><p><? echo $buyer_short_name_library[$row[csf('buyer_name')]]; ?></p></td>
											<td width="130"><p><a  href="##" onclick="open_febric_receive_status_order_wise_popup('<? echo $row[csf('po_id')]; ?>','grey_issue','')"></a><? echo $row[csf('style_ref_no')]; ?></a></p></td>
											<td width="100"><p><? echo $row[csf('file_no')]; ?></p></td>
											<td width="100"><p><? echo $row[csf('grouping')]; ?></p></td>
											<td width="40" onclick="openmypage_image('requires/fabric_receive_status_report_controller.php?action=show_image&job_no=<? echo $row[csf("job_no")] ?>','Image View')" style="cursor:pointer;"><p><img  src='../../<? echo $imge_arr[$row[csf('job_no')]]; ?>' height='25' width='40' /></p></td>
											<td width="140"><p><? echo $gmts_item; ?></p></td>
							...
							if(($cbo_discrepancy==1 && $grey_recv_qnty>$yarn_issued) || ($cbo_discrepancy==0))
							{
								...
								?>
									<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $k; ?>">
										...
										<td width="80"><p><? echo $buyer_short_name_library[$row[csf('buyer_name')]]; ?></p></td>
										<td width="130"><p><a  href="##" onclick="open_febric_receive_status_order_wise_popup('<? echo $row[csf('po_id')]; ?>','grey_issue','')"><? echo $row[csf('style_ref_no')]; ?></a></p></td>
										<td width="100"><p><? echo $row[csf('file_no')]; ?></p></td>
										<td width="100"><p><? echo $row[csf('grouping')]; ?></p></td>
										<td width="40" onclick="openmypage_image('requires/fabric_receive_status_report_controller.php?action=show_image&job_no=<? echo $row[csf("job_no")] ?>','Image View')" style="cursor:pointer;"><p><img  src='../../<? echo $imge_arr[$row[csf('job_no')]]; ?>' height='25' width='40' /></p></td>
										<td width="140"><p><? echo $gmts_item; ?></p></td>
						...
						foreach($nameArray as $row)
						{
							...
							if(count($no_book_check_arr)<1)
							{
								...
								if(($cbo_discrepancy==1 && $grey_recv_qnty>$yarn_issued) || ($cbo_discrepancy==0))
								{
									...
									$booking_array=array(); $color_data_array=array();
									$required_qnty=0; $main_booking=''; $sample_booking=''; $main_booking_excel=''; $sample_booking_excel='';$grey_fabric_issue=0;$color_grey_fabric_issue=0;
									$dataArray=explode(",",substr($booking_data,0,-1));
									if(count($dataArray)>0)
									{
										foreach($dataArray as $woRow)
										{
											...
											foreach($job_po_id as $po_id)
											{
												$grey_fabric_issue+=$grey_issue_qnty_arr[$po_id][$fabric_color_id];
												$color_grey_fabric_issue+=$grey_issue_qnty_arr[$po_id][$fabric_color_id];
												
											}
											$color_grey_issue_arr[$fabric_color_id]=$color_grey_fabric_issue;
										}
									}
									else
									{
											...
											<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $k; ?>">
												...
												<td width="100" align="right">
													<? 
														...
														echo number_format($color_grey_data_array[$key],2,'.','');
														$html.=number_format($color_grey_data_array[$key],2);
														$grey_fab_Requi_array[$row[csf('buyer_name')]]+=$color_grey_data_array[$key];
														$tot_color_wise_grey_req+=$color_grey_data_array[$key];
													?>
												</td>
                                                <? $html.="</td><td>"; ?>
												<td width="100" align="right" title="Grey Issue Return Not handled In This Report"><a  href="##" onclick="open_febric_receive_status_order_wise_popup('<? echo $row[csf('po_id')]; ?>','grey_issue','<? echo $key; ?>')"><? echo number_format($color_grey_issue_arr[$key],2,'.',''); $html.=number_format($color_grey_issue_arr[$key],2); ?></a>
													<? 
														/*if($z==1)
														{
														?>
								...
								if(count($dataArray)>0)
								{
									foreach($dataArray as $woRow)
									{
										...
										if($item_category!=3)
										{
											...
											$color_grey_issue_arr[$fabric_color_id]+=$color_grey_fabric_issue;
											$grey_fabric_issue+=$color_grey_fabric_issue;
										}
            ...
            <?
				...
				$html.="<th></th>
							...
							<th align='right'>".number_format($tot_grey_issue_bl,2)."</th>							
							<th></th>
							<th align='right'>".number_format($tot_color_wise_grey_req,2)."</th>
							<th align='right'>".number_format($tot_grey_issue,2)."</th>
							<th align='right'>".number_format($tot_batch_qnty,2)."</th>
							<th align='right'>".number_format($tot_color_wise_req,2)."</th>
							<th align='right'>".number_format($tot_dye_qnty,2)."</th>
            ...
            <table width="<? echo $table_width; ?>" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all">
                <tfoot>
                    <tr>
                        ...
                        <th width="100" id="value_tot_grey_issue_bl"><? echo number_format($tot_grey_issue_bl,2); ?></th>                        
                        <th width="100"></th>
                        <th width="100" id="value_tot_color_wise_grey_req"><? echo number_format($tot_color_wise_grey_req,2); ?></th>
                        <th width="100" id="value_tot_grey_issue"><? echo number_format($tot_grey_issue,2); ?></th>
                        <th width="100" id="value_tot_batch"><? echo number_format($tot_batch_qnty,2); ?></th>
                        <th width="100" id="value_tot_fin_rec"><? echo number_format($tot_color_wise_req,2); ?></th>
                        <th width="100" id="value_tot_dye_qnty"><? echo number_format($tot_dye_qnty,2); ?></th>
	...
	$filename="../../ext_resource/tmp_report/".$user_name."_".$name.".xls";
	$filename_short="../../ext_resource/tmp_report/".$user_name."_".$name."short.xls";
	echo "$total_data####$filename####$filename_short####2";
	exit();
			...
			<table border="1" class="rpt_table" rules="all" width="860" cellpadding="0" cellspacing="0">
             ...
             <div style="width:877px; max-height:320px; overflow-y:scroll" id="scroll_body">
                 <table border="1" class="rpt_table" rules="all" width="860" cellpadding="0" cellspacing="0">
                    <?
                    $i=1; $issue_to='';
					if($color!="") $color_cond=" and b.color_id='$color'"; else $color_cond="";
