...
if($action=="report_generate")
{
	...
	$cbo_shift_source=str_replace("'","",$cbo_shift_source);
	...
	else if($report_type==2 && $cbo_shift_source ==1)
	{
		if($cbo_type==1 || $cbo_type==0)
		{
			if (str_replace("'","",$cbo_buyer_name)==0) $buyer_cond=''; else $buyer_cond=" and a.buyer_id=$cbo_buyer_name";
        ...
        <table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table">
			...
			if(str_replace("'","",$cbo_knitting_source)==0 || str_replace("'","",$cbo_knitting_source)==2)
			{
				...
				from subcon_production_mst a, subcon_production_dtls b, lib_machine_name c, subcon_ord_dtls d 
				where a.id=b.mst_id and b.machine_id=c.id and b.job_no=d.job_no_mst and d.id=b.order_id  and a.product_type=2 and d.status_active=1 and d.is_deleted=0 
				and a.status_active=1 and a.is_deleted=0 and a.company_id=$cbo_company_name $date_con_sub $floor_id $buyer_id_cond $job_no_cond $order_no_cond $job_year_sub_cond $sales_order_cond $booking_no_cond $within_group_cond
	...
	else if($report_type==2 && $cbo_shift_source ==2)
	{
		if($cbo_shift_source ==2)
		{
			//Creating shift array from library for knitting production insert time
			...
			foreach ($shift_duration_entry_arr as $value) 
			{
				$start_arr = explode(":",$value[csf("start_time")]);
				$start_hour = $start_arr[0]*1;
				$start_min = $start_arr[1]*1;

				$end_arr = explode(":",$value[csf("end_time")]);
				$end_hour = $end_arr[0]*1;
				$end_min = $end_arr[1]*1;

				if($start_hour < $end_hour)
				{
					$hour_diff_to_min = ($end_hour - $start_hour) * 60;
					if($start_min < $end_min)
					{
						$min_diff = ($end_min - $start_min);
						$hour_diff_to_min += $min_diff;
					}
					else if($start_min > $end_min)
					{
						$min_diff = ($start_min - $end_min);
						$hour_diff_to_min -= $min_diff;
					}
				}
				else if ($start_hour > $end_hour)
				{
					$hour_diff_to_min = ($end_hour + 24 - $start_hour) * 60;
					if($start_min < $end_min)
					{
						$min_diff = ($end_min - $start_min);
						$hour_diff_to_min += $min_diff;
				...
				//echo $hour_diff_to_min;die;
				if(strlen($start_min) == 1)
				{
					$start_min_str = "0".$start_min;
				}else{
					$start_min_str = $start_min;
				}

				$time_array[$start_hour][$start_min_str]= $value[csf("shift_name")];
				for ($i=1; $i <= $hour_diff_to_min; $i++) 
				{ 
					$start_min += 1;
					if($start_min  >59 )
					{
					...
					$time_array[$start_hour][$start_min_str]= $value[csf("shift_name")];
					//echo $i . " >>> " . $start_hour ."=". $start_min."<br>";
				}
				//die;
			}
		}
        ...
        <table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table">
			...
			foreach($nameArray_inhouse as $row)
			{
				...
				$exp_production_dtls_id = explode(",", $row[csf('dtls_id')]);
				foreach($exp_production_dtls_id as $pval)
				{
					$productionDtlsIdArr[$pval] = $pval;
				}
			}
			//echo "<pre>";
			//print_r($productionDtlsIdArr);
			...
			$production_dtls_sql = sql_select( "select id, grey_receive_qnty, insert_date from pro_grey_prod_entry_dtls where 1=1 ".where_con_using_array($productionDtlsIdArr, '0', 'id') ." and status_active=1 ");

			foreach ($production_dtls_sql as $val) {
				$production_dtls_arr[$val[csf('id')]]['insert_date'] = $val[csf('insert_date')];
				$production_dtls_arr[$val[csf('id')]]['qnty'] = $val[csf('grey_receive_qnty')];
			}
			...
			foreach ($nameArray_inhouse as $row)
			{
				...
				foreach ($exp_production_dtls_id as $val) 
				{
					$insert_hour =  date("G",strtotime($production_dtls_arr[$val]['insert_date']));
					$insert_min =  date("i",strtotime($production_dtls_arr[$val]['insert_date']));

					echo $time_array[$insert_hour][$insert_min];
					//$production_dtls_arr[$val]['insert_date'];
					//$production_dtls_arr[$val]['qnty'];
				}
