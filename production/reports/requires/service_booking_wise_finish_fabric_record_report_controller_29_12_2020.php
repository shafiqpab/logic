...
if($action=="report_generated")
{
	...
	if($cbo_process!=1)
	{
		...
		$fabric_isssueToProcess_sql =sql_select("select b.booking_no,b.booking_dtls_id, b.order_id,b.body_part_id,b.color_id,b.width as width,
		sum(case when a.entry_form=91 then b.batch_issue_qty else 0 end) as batch_issue_qty,
		sum(case when a.entry_form=92 then b.batch_issue_qty else 0 end) as batch_recv_qty,b.febric_description_id,a.entry_form    
											...
											foreach($poData as $fabrication_id => $fabricDAta)
											{
												...
												foreach($fabricDAta as $gmtsColorId => $row)
												{
													...
													<tr bgcolor="<? //echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
														...
														//echo $bookingNo."=".$po_id."=".$fabric_des_id_arr[$fabrication_id][$bookingNo]."=".$fabric_des_body_part_id_arr[$fabrication_id][$bookingNo]."=".$dia_width."=".$gmtsColorId."=".$dtls_id."<br/>";

														$fab_issue=$fabric_issue_recv_arr[$bookingNo][$po_id][$fabric_des_id_arr[$fabrication_id][$bookingNo]][$fabric_des_body_part_id_arr[$fabrication_id][$bookingNo]][$row["dia_width"]][$gmtsColorId][$row["dtls_id"]]['batch_issue_qty']; echo number_format($fab_issue,2,'.','');
														?></td>
														<td width="60" align="right">
															<? 
															...
															{
																$fab_recv=$fabric_issue_recv_arr[$bookingNo][$po_id][$fabric_des_id_arr[$fabrication_id][$bookingNo]][$fabric_des_body_part_id_arr[$fabrication_id][$bookingNo]][$row["dia_width"]][$gmtsColorId][$row["dtls_id"]]['batch_recv_qty'];
															}
											...
											foreach($poData as $fabrication_id => $row)
											{
												...
												//{
													...
													<tr bgcolor="<? //echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
														...
														$fab_issue=$fabric_issue_recv_arr[$bookingNo][$po_id][$fabric_des_id_arr[$fabrication_id][$bookingNo]][$fabric_des_body_part_id_arr[$fabrication_id][$bookingNo]][$row["dia_width"]]['batch_issue_qty']; echo number_format($fab_issue,2,'.','');
														?></td>
														<td width="60" align="right">
															<? 
																...
																$fab_recv=$fabric_issue_recv_arr[$bookingNo][$po_id][$fabric_des_id_arr[$fabrication_id][$bookingNo]][$fabric_des_body_part_id_arr[$fabrication_id][$bookingNo]][$row["dia_width"]]['batch_recv_qty'];
															
															
															//echo number_format($fab_recv,2,'.',''); 

															//echo $bookingNo."=".$po_id."=".$fabric_des_id_arr[$fabrication_id][$bookingNo]."=".$fabric_des_body_part_id_arr[$fabrication_id][$bookingNo]."=".$row["dia_width"]."=".$row["dtls_id"];
															?>
															<a href="##" onclick="openmypage_qnty('<? echo $bookingNo;?>','<? echo $po_id;?>','openmypage_receive','<? echo $fabric_des_id_arr[$fabrication_id][$bookingNo];?>','<? echo $fabric_des_body_part_id_arr[$fabrication_id][$bookingNo];?>','<? echo $row["dia_width"]; ?>','0','0','<? echo $date_from;?>','<? echo $date_to;?>','<? echo $row["process"];?>');"><? echo number_format($fab_recv,2,".",""); ?>
		...
		<div id="scroll_body" align="center">
			<?
				...
				if ($processId==31)
				{
					...
					and d.entry_form=37 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 
				...
				{
					$rcv_sql = sql_select("select b.booking_no,b.booking_dtls_id,a.receive_date as transaction_date, b.order_id,b.body_part_id,b.color_id,b.width as width,
					sum(case when a.entry_form=91 then b.batch_issue_qty else 0 end) as batch_issue_qty,
					sum(case when a.entry_form=92 then b.batch_issue_qty else 0 end) as batch_recv_qty,b.febric_description_id,a.entry_form    
					from inv_receive_mas_batchroll a, pro_grey_batch_dtls b where a.id=b.mst_id and a.entry_form in(91,92) and a.company_id=$companyID and b.booking_no='$booking_no' and b.order_id=$poID and b.febric_description_id=$fabricDescId and b.body_part_id=$bodyPartId and b.color_id=$gmts_color and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by  b.booking_no,b.booking_dtls_id,a.receive_date as transaction_date, b.order_id,b.febric_description_id,b.body_part_id ,b.color_id,b.width,a.entry_form");
				}
