...
{ 
	...
	if($type==1)
	{
		...
		foreach($sql_subcon as $val)
		{
			...
			if($production_data_arr[$val[csf('floor_id')]][$val[csf('sewing_line')]]['item_number_id']!="")
			{
				$production_data_arr[$val[csf('floor_id')]][$val[csf('sewing_line')]]['item_number_id'].="****".$val[csf('po_break_down_id')]."**".$val[csf('item_number_id')].'**'."subcon"; 
			}
			else
			{
				$production_data_arr[$val[csf('floor_id')]][$val[csf('sewing_line')]]['item_number_id']=$val[csf('po_break_down_id')]."**".$val[csf('item_number_id')].'**'."subcon"; 
		...
		foreach($production_data_arr as $f_id=>$fname)
		{
			...
			foreach($fname as $l_id=>$ldata)
			{
				...
				$html.='<td width="40">'.$i.'&nbsp;</td>
							...
							<td align="right" width="60" style="text-align:left;"><p>'.chop($set_smv_all,',').'</p></td>
	...
	<fieldset style="width:620px; ">
        ...
        <table border="1" class="rpt_table" rules="all" width="600" cellpadding="0" cellspacing="0" align="center">
			...
			for($h=$hour;$h<$last_hour;$h++)
			{
				...
				where a.id=d.mst_id and d.production_type=$pro_type and f.id=d.color_size_break_down_id and d.status_active=1 and d.is_deleted=0 and  a.production_type=$pro_type and a.po_break_down_id=c.id and c.job_no_mst=b.job_no and c.id=f.po_break_down_id and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and f.status_active=1 and f.is_deleted=0  and c.is_deleted=0  and a.serving_company=".$company_id."  and a.floor_id=".$floor_id." and a.sewing_line=".$sewing_line."  and a.po_break_down_id in(".$po_id.") and a.production_date='".$prod_date."'  group by b.style_ref_no, c.po_number,a.po_break_down_id order by  c.po_number ";
