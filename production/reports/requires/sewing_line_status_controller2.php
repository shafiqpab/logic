...
{	
	...
	else if($db_type==2)
	{
		...
		WHERE a.production_type=5 and a.status_active=1 and a.is_deleted=0 and a.id=d.mst_id and d.color_size_break_down_id=e.id and e.PO_BREAK_DOWN_ID=c.id and c.job_id=b.id and a.status_active=1 and a.is_deleted=0 and b.is_deleted=0 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 $company_name $location $floor $line $buyer_id_cond  $txt_date_from 
		GROUP BY b.job_no,b.job_no_prefix_num, a.company_id, a.location, a.floor_id,a.po_break_down_id,a.prod_reso_allo, a.production_date, a.sewing_line,b.buyer_name,b.style_ref_no,a.item_number_id,c.po_number,c.unit_price,c.file_no,c.grouping ,d.color_type_id,a.remarks,a.production_hour,c.shipment_date,e.color_number_id,e.country_id
		ORDER BY a.location,a.floor_id,a.sewing_line";
	}
	 //echo $sql;die;
	...
	{	
		$company_id_array[$val[csf('company_id')]] = $val[csf('company_id')];
	 	...
	 	if($production_data_arr[$val[csf('sewing_line')]]['job_no']!="")
		{
			...
			$production_data_arr[$val[csf('sewing_line')]]['buyer'].=",".$val[csf('buyer_name')];
	...
	$company_id_cond = where_con_using_array($company_id_array,0,"a.company_name");
	...
	{
		$sql_item="SELECT b.id, a.set_break_down, c.gmts_item_id, c.set_item_ratio, c.smv_pcs, c.smv_pcs_precost,c.smv_set from wo_po_details_master a,wo_po_break_down b, wo_po_details_mas_set_details c where b.job_id=a.id and b.job_id=c.job_id $company_id_cond $poIds_cond and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1";
	...
	<fieldset style="width:<? echo $tbl_width+20;?>px">
       ...
       <!-- <table width="<? echo $tbl_width;?>" cellpadding="0" cellspacing="0"> 
            <tr class="form_caption">
                <td colspan="<? echo $colspan;?>" align="center"><strong>Sewing Line Status Report &nbsp;</strong></td> 
            </tr>
            <tr class="form_caption">
        ...
        <br />    -->     
        <!-- <marquee width = "100%" behavior="alternate" direction="left"><? echo $companyArr[$company_id]; ?></marquee> -->
                ...
                <tr>
                   ...
                   // print_r($production_hour);
                	...
                	for($k=$hour; $k<=$last_hour; $k++)
					{
						...
						{
							?>
		                      <th title="<?=substr($start_hour_arr[$k],0,5)."-".substr($start_hour_arr[$k+1],0,5);?>" width="40" style="vertical-align:middle"><div class="block_div"><?  echo "H".$p; ?></div></th>
							<?	
						}
        ...
        <div style="width:<?= $tbl_width+20;?>px; max-height:560px; overflow-y:scroll;" id="scroll_body">
                	...
                	foreach($production_serial_arr as $floor_id=>$flr_data) 
                	{
	                	...
	                	foreach($flr_data as $sl=>$sl_data) 
	                	{
							...
							{   
								...
								$buyer_arr = array_unique(explode(",", $production_data_arr[$l_id]['buyer_name']));
								$buyer = implode(",", $buyer_arr);
								$style_arr = array_unique(explode(",", $production_data_arr[$l_id]['style_ref_no']));
								$styleName = implode(",", $style_arr);
								$po_number_arr = array_unique(explode(",", $production_data_arr[$l_id]['po_number']));
								$po_name = implode(",", $po_number_arr);
								$style = "";
			                   ...
			                   <tr bgcolor="<? echo $bgcolor;?>" id="tr_<?= $i;?>" onClick="change_color('tr_<?= $i; ?>','<?= $bgcolor; ?>')" title="<?=$title;?>">
									...
									<td width="75"><p style="<?=$style;?>"><?=$buyer;?></p></td>
									<td width="75"><p style="<?=$style;?>"><?=$styleName;?></p></td>
									<td width="75"><p style="<?=$style;?>"><?=$po_name;?></p></td>
				                    <td width="100"><p style="<?=$style;?>"><?=implode(", ", array_unique(explode(",", $item_name)));?></p></td>
									...
									for($k=$hour; $k<=$last_hour; $k++)
									{
										...
										{
											?>
						                      <td bgcolor="<?=$color;?>" align="right" width="40" id="<?=$new_style;?>" <?//=$title;?> >
						                      	<?//=$new_color;?>
						                      	<div style="<?=$style;?>">
			            ...
			            <tr style="background: #1F1D36;text-align: right;">
	                        <td colspan="6" width="135"><?=$floorArr[$floor_id];?></td>
							...
							for($k=$hour; $k<=$last_hour; $k++)
							{
								...
								{
									?>
				                      <td align="right" width="40"><? echo number_format($f_total_arr[$prod_hour],0); ?></td>
									<?	
								}
								/*elseif ($total_production[$prod_hour]>0 && $p>11) 
								{
		 ...
		 <table class="rpt_table" width="<?= $tbl_width;?>" cellpadding="0" cellspacing="0" border="1" rules="all" align="left">
                ...
                <tfoot>
                   <tr style="background: #1F1D36;">
	                    ...
	                    <th width="45"><?=number_format($gr_op,0);?></th>
	                    <th width="45"><?=number_format($gr_hp,0);?></th>
	                    <th width="45"><?=number_format($gr_mp,0);?></th>
	                    <th width="45"><?=number_format(($gr_tot_smv/$gr_tot_line),2);?></th>
	                    <th width="60"><?=number_format($gr_day_trgt,0);?></th>
	                    <th width="60"><?=number_format($gr_trgt_hr,0);?></th>
                        <?
                        $p = 1;
                        $total = 0;
						for($k=$hour; $k<=$last_hour; $k++)
						{
							...
							{
								?>
			                      <th align="right" width="40"><? echo number_format($total_arr[$prod_hour],0); ?></th>
								<?	
							}
							/*elseif ($total_production[$prod_hour]>0 && $p>11) 
							{
								?>
			                      <th align="right" width="70"><? echo $total_production[$prod_hour]; ?></th>
								<?
							}*/
							$p++;
                        ...
                        ?>
					    <th width="60"><?=number_format($total,0);?></th>                   
					    <th width="60"><?=number_format(($gr_tot_achv/$gr_tot_line),0);?>%</th> 
	                    <th width="60"></th> 
                    </tr>
                </tfoot>
            </table>
