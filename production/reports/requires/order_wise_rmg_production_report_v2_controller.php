...
{ 
	...
	{		
		...
		where b.job_no_mst=a.job_no  and b.id=c.po_break_down_id and a.job_no=c.job_no_mst and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0 and c.status_active in(1,2,3) and c.is_deleted=0 $str_po_cond $txt_style_ref_number_cond $internal_ref_cond_po $order_no_cond $order_id_cond
		...
		foreach($sql_po_result as $row)
		{
			...
			$main_array[$row[csf("id")]][$row[csf("color_number_id")]][$row[csf("size_number_id")]]['file_no'] = $row[csf("file_no")];
			$main_array[$row[csf("id")]][$row[csf("color_number_id")]][$row[csf("size_number_id")]]['grouping'] = $row[csf("grouping")];
		}
		// ======================================= FOR ROWSPAN ==============================
		$rowSpanArray = array();
		...
		foreach($sql_res as $vals)
		{
			...
			if($entry_form!=76)
			{
				$cut_lay_arrs[$vals[csf("order_id")]][$vals[csf("color_id")]][$vals[csf("size_id")]]+=$vals[csf("qty")];
			}
			else
			{
				$cut_lay_arrs[$vals[csf("order_id")]][$vals[csf("color_id")]][$vals[csf("size_id")]]+=$vals[csf("qty")];
        ...
        from pro_gmts_cutting_qc_dtls c 
        where c.status_active=1 $all_po_ids_cond
		...
		$sql_fin = "SELECT e.color_id, e.po_breakdown_id AS po_id, sum(e.quantity) as qnty
	  	FROM product_details_master a,  order_wise_pro_details e
       	WHERE a.id = e.prod_id and e.trans_id>0 and a.item_category_id=2 and e.trans_type=1 and e.entry_form in(37,68) and e.po_breakdown_id in($all_po_id) and a.status_active=1 and e.status_active=1 group by e.color_id, e.po_breakdown_id";//in(7,37,66,68,225)
       	$sql_fin_res = sql_select($sql_fin);
       	$fin_qnty_arr = array();
       	// $fin_qnty_arr2 = array();
       	foreach ($sql_fin_res as $key => $val) 
       	{
       		$fin_qnty_arr[$val[csf('po_id')]][$val[csf('color_id')]]=$val[csf('qnty')];
		...
		<div>
			...
			<table width="1580" cellspacing="0" border="1" class="rpt_table" rules="all" id="table_header_1">
				<thead>
					<tr>
						<th width="30">SL</th>   
						...
						<!-- <th width="50">FB. Con.</th> -->
						<th width="50">Fin. Fab. Req.</th>
						<th width="50">Fab. Rcv.</th>
						<th width="50">Fab. Bal.</th>
						<th width="60">Aricle No.</th>
						...
						<th width="50">%</th>                        
                        <th width="50">Cut. Bal.</th>
						<th width="50">Cut. QC</th>
						<th width="50">Reject</th> 

						<th width="50">Sent to Print</th>
						<th width="50">Print. Rcv.</th>
						<th width="50">Print Bal.</th>
						<th width="50">Sent to Emb</th>
						<th width="50">Emb. Rcv.</th> 

						<th width="50">Sew. Input</th>
						<th width="50">input Bal.</th>
						<th width="50">Sewing</th>
						<th width="50">Bal.</th>
						<th width="50">Pack. & Fin.</th>
						<th width="50">Bal.</th>
			...
			<div style="max-height:330px; overflow-y:scroll; width:1600px" id="scroll_body">
				<table border="1" class="rpt_table" width="1580" rules="all" id="table_body">
					<?
					$s=1;
					...
					{					
						...
						foreach ($po_data as $color_id => $color_data) 
						{
							...
							{	
								...
								$fab_fin_qnty	= $fin_qnty_arr[$po_id][$color_id];
								$order_qnty 	= $colo_size_ord_qnty_arr[$po_id][$color_id][$size_id];
								$cut_qnty 		= $cut_lay_arrs[$po_id][$color_id][$size_id];
								...
								$co_print_rej_qnty 	+= $print_reject;
                                ...
                                {
                                    ?>
                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>" style="word-break: break-all;word-wrap: break-word;" width="30"><? echo $s;?></td>
                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>" style="word-break: break-all;word-wrap: break-word;" width="60"><? echo $row['po_number']; ?></td>
                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>" style="word-break: break-all;word-wrap: break-word;" width="60"><? echo $row['file_no']; ?></td>
                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>" style="word-break: break-all;word-wrap: break-word;" width="60"><? echo $row['grouping']; ?></td>
                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>" style="word-break: break-all;word-wrap: break-word;" width="60"><? echo $buyer_short_library[$row['buyer_name']]; ?></td>
                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>" style="word-break: break-all;word-wrap: break-word;" width="60">
									<?
									$itemDetails = '';
                                    ...
                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>" style="word-break: break-all;word-wrap: break-word;" width="60"><? echo $color_Arr_library[$color_id]; ?></td>
                                    <!-- <td valign="middle" rowspan="<? //echo $rowSpanArray[$po_id][$color_id];?>" style="word-break: break-all;word-wrap: break-word;" width="50" align="right" ><? //echo $fin_con; ?></td> -->
                                    <td title="<? echo 'fin con = '.$fin_con.' , plan_cut='.$plan_cut;?>" valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>" style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><?  echo number_format($fab_req_qty); ?></td>
                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>" style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($fab_fin_qnty,2); ?></td>
                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>" style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($fab_req_qty - $fab_fin_qnty); ?></td>
                                    <?
									$s++;
                                    }
                                    ?>
                                    <td style="word-break: break-all;word-wrap: break-word;" width="60"><? echo $row['article_number']; ?></td>
                                    <td style="word-break: break-all;word-wrap: break-word;" width="30"><? echo $size_Arr_library[$size_id]; ?></td>
                                    
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($order_qnty,0); ?></td>
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($cut_qnty,0); ?></td>
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? $percent = ($cut_qnty / $order_qnty)*100; echo number_format($percent,2); ?>%</td>
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($cut_balance,0); ?></td>
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($cut_qc,0); ?></td>
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($cut_rej,0); ?></td>
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($print_issue,0); ?></td>
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($print_recive,0); ?></td>
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><?  echo number_format($print_reject,0); ?></td>
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($embel_issue,0); ?></td>
                                    
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($embel_rcv,0); ?></td>
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($sewing_in,0); ?></td>
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($input_bal,0); ?></td>
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($sewing_out,0); ?></td>
                            
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($sewing_bal,0); ?></td>
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($pack_finish,0); ?></td>
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($pack_fin_bal,0); ?></td>
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($ex_fact_qnty,0); ?></td>						
                                    <td style="word-break: break-all;word-wrap: break-word;" width="50" align="right"><? echo number_format($ex_fact_bal,0); ?></td>
                                </tr>
                                <? $color++;
							}
							?>
							<tr class="gd-color2">
								<td colspan="12" align="right">Color Wise Sub Total</td>
								<td align="right"><? echo number_format($co_order_qnty);?></td>
								<td align="right"><? echo number_format($co_cutting_qnty);?></td>
								<td align="right"></td>
								<td align="right"><? echo number_format($co_cut_bal_qnty);?></td>
								<td align="right"><? echo number_format($co_cut_qc_qnty);?></td>
						...
						?>
                        <tr class="gd-color3">
                            <td colspan="12" align="right">Po Wise Sub Total</td>
			...
			<div>
				<table border="1" class="rpt_table" width="1580" rules="all" id="report_table_footer" >
					<tr class="gd-color">
						<td width="30">&nbsp;</td>
                        <td width="60">&nbsp;</td>
                        <td width="60">&nbsp;</td>
						...
						<td width="50">&nbsp;</td>
						<td width="50">&nbsp;</td>
						<td width="60" align="right">Grand Total</td>
						<td width="30"></td>
						
						<td width="50" align="right"><? echo number_format($gr_order_qnty,0);?></td>
						<td width="50" align="right"><? echo number_format($gr_cutting_qnty,0);?></td>
						<td width="50" align="right"></td>
						<td width="50" align="right"><? echo number_format($gr_cut_bal_qnty,0);?></td>
						<td width="50" align="right"><? echo number_format($gr_cut_qc_qnty,0);?></td>
						<td width="50" align="right"><? echo number_format($gr_cut_rej_qnty,0);?></td>
						<td width="50" align="right"><? echo number_format($gr_printS_qnty,0);?></td>
						<td width="50" align="right"><? echo number_format($gr_printR_qnty,0);?></td>
						<td width="50" align="right"><? echo number_format($gr_print_rej_qnty,0);?></td>
						<td width="50" align="right"><? echo number_format($gr_embS_qnty,0);?></td>
						
						<td width="50" align="right"><? echo number_format($gr_embR_qnty,0);?></td>
						<td width="50" align="right"><? echo number_format($gr_input_qnty,0);?></td>
						<td width="50" align="right"><? echo number_format($gr_input_bal_qnty,0);?></td>
						<td width="50" align="right"><? echo number_format($gr_output_qnty,0);?></td>
				
						<td width="50" align="right"><? echo number_format($gr_out_bal_qnty,0);?></td>
						<td width="50" align="right"><? echo number_format($gr_pac_fin_qnty,0);?></td>
						<td width="50" align="right"><? echo number_format($gr_pac_fin_bal_qnty,0);?></td>
						<td width="50" align="right"><? echo number_format($gr_export_qnty,0);?></td>						
						<td width="50" align="right"><? echo number_format($gr_export_bal_qnty,0);?></td>
					</tr>
				</table>
			</div>
		</div>
	...
	//	echo $order_qnty_sql; die;
		...
		foreach($main_array as $po_id=>$po_data)
		{
			foreach($po_data as $color_id=>$color_data)
			{
				foreach($color_data as $size_id=>$row)
				{
					...
					$buyer_summery[$buyer_short_library[$row['buyer_name']]]['order_qnty']+=$colo_size_ord_qnty_arr[$po_id][$color_id][$size_id];
					$buyer_summery[$buyer_short_library[$row['buyer_name']]]['qty']+=$cut_lay_arrs[$po_id][$color_id][$size_id];
					$buyer_summery[$buyer_short_library[$row['buyer_name']]]['sQty']+=$gmts_prod_arr[$po_id][$color_id][$size_id]['sQty'];
					$buyer_summery[$buyer_short_library[$row['buyer_name']]]['soQty']+=$gmts_prod_arr[$po_id][$color_id][$size_id]['soQty'];
					$buyer_summery[$buyer_short_library[$row['buyer_name']]]['finish_qnty_inhouse']+=$gmts_prod_arr[$po_id][$color_id][$size_id]['finish_qnty_inhouse'];
					$buyer_summery[$buyer_short_library[$row['buyer_name']]]['ex_qty']+=$ex_factory_arr[$po_id][$color_id][$size_id]['ex_qty'];
		...
		<div>
			...
			<table width="1150" cellspacing="0" border="1" class="rpt_table" rules="all" id="table_header_1">
				<thead>
					<tr>
						<th width="40"><p>SL</p> </th>   
						<th width="100"><p>Buyer</p></th>
						<th width="90"><p>Order Qty</p></th>
						<th width="90"><p>Cutting Qty</p></th>
						<th width="100"><p>Order to Cutting %</p></th>
						<th width="60"><p>Input Qty</p></th>
						<th width="90"><p>Order to Input %</p></th>
						<th width="70"><p>Sewing Qty</p></th>
						<th width="110"><p>Order to Sew. Out. %</p></th>
						<th width="100"><p>Pack. & Fini. Qty</p></th>
						<th width="120"><p>Order to Pack. & Fin. %</p></th>
						<th width="80"><p>Shipment Qty</p></th>
						<th width="115"><p>Order to Shipment %</p></th>
						<th width="80">Cut to Ship %</th>
					 </tr>
				</thead>
			</table>
			<div style="max-height:330px;  width:1200px" >
				<table border="1" class="rpt_table" width="1150" rules="all">
					<?
					$i=1;
					foreach($buyer_summery as $buyer_name=>$rows)
					{
							...
							?>
								...
								<td width="100" valign="middle"><p><? echo $buyer_name;?></p></td>
								<td width="90" align="right"><p><? echo $rows['order_qnty'] ;?></p></td>
								<td width="90"align="right"><p><? echo $rows['qty'];?></p></td>
								<td width="100"align="right"><p><? $percent = ($rows['qty'] / $rows['order_qnty'] )*100; echo number_format($percent,2); ?>%</p></td>
								<td width="60" align="right"><p><?echo $rows['sQty'];?></p></td>
								<td width="90"align="right"><p><? $percent = ($rows['sQty'] / $rows['order_qnty'] )*100; echo number_format($percent,2); ?>%</p></td>
								<td width="70" align="right"><p><? echo $rows['soQty'];?></p></td>
								<td width="110" align="right"><p><? $percent = ($rows['soQty'] / $rows['order_qnty'] )*100; echo number_format($percent,2); ?>%</p></td>
								<td width="100" align="right"><p><? echo $rows['finish_qnty_inhouse']?></p></td>
								<td width="120" align="right"><p><? $percent = ($rows['finish_qnty_inhouse'] / $rows['order_qnty'] )*100; echo number_format($percent,2); ?>%</p></td>
								<td width="80" align="right"><p><? echo $rows['ex_qty'];?></p></td>
								<td width="115" align="right"><p><? $percent = ($rows['ex_qty'] / $rows['order_qnty'] )*100; echo number_format($percent,2); ?>%</p></p></td>
								<td width="80" align="right"><p><? $percent = ( $rows['ex_qty'] / $rows['qty']  )*100; echo number_format($percent,2); ?>%</p></td>
			 ...
			 <table width="1830" cellspacing="0" border="1" class="rpt_table" rules="all" id="table_header_1">
				<thead>
					<tr>
						...
						<th width="60"><p>Order Number</p></th>
						<th width="60"><p>File NO</p></th>
						<th width="60"> <p>Ref. NO</p></th>
						<th width="60"><p> Buyer Name</p></th>
						<th width="60"><p>Item Name</p></th>
						<th width="60"><p>Color</p></th>

						<th width="50"><p>Fin. Fab. Req.</p></th>
						<th width="50"><p>Fab. Rcv.</p></th>
						<th width="50"><p>Fab. Bal.</p></th>
						<th width="60"><p>Aricle No.</p></th>

						<th width="30"><p>Size</p></th>
						<th width="50"><p>Order Qty.</p> </th>

						<th width="50"><p>Cutting</p></th>
						<th width="50"><p>Order to Cutting%</p>
							</th>                        
                        <th width="50"><p>Cut. Bal.</p></th>
						<th width="50"><p>Cut. QC</p></th>
						<th width="50"><p>Reject</p></th> 

						<th width="50"><p>Sent to Print</p></th>
						<th width="50"><p>Print. Rcv.</p></th>
						<th width="50"><p>Print Bal.</p></th>
						<th width="50"><p>Sent to Emb</p></th>
						<th width="50"><p>Emb. Rcv.</p></th> 

						<th width="50"><p>Sew. Input</p></th>
						<th width="50"><p>input Bal.</p></th>
						<th width="50"><p>Order to Input %</p></th>
						<th width="50"><p>Sewing</p></th>
						<th width="50"><p>Bal.</p></th>
						<th width="50"><p>Order to Sew. Output %</p></th>
						<th width="50"><p>Pack. & Fin.</p></th>
						<th width="50"><p>Bal.</p></th>
						<th width="50"><p>Order to Pack. & Fin. %</p></th>
						
						<th width="50"><p>Export</p></th>
						<th width="50"><p>Balance</p></th>
					...
					{					
						...
						foreach ($po_data as $color_id => $color_data) 
						{
							...
							{	
                                ...
                                {
                                    ?>
                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>" width="30"><p><? echo $s;?></p></td>
                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>"  width="60"><p><? echo $row['po_number']; ?></p></td>
                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>"  width="60"><p><? echo $row['file_no']; ?></p></td>
                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>" width="60"><p><? echo $row['grouping']; ?></p></td>
                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>"  width="60"><p><? echo $buyer_short_library[$row['buyer_name']]; ?></p></td>
                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>"  width="60"><p>
									<?
									$itemDetails = '';
									foreach($row['item_number_id'] as $itemNumID)
									{
                                    ...
                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>" width="60"><p><? echo $color_Arr_library[$color_id]; ?></p></td>
                                   
                                    <td title="<? echo 'fin con = '.$fin_con.' , plan_cut='.$plan_cut;?>" valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>" width="50" align="right"><p><?  echo number_format($fab_req_qty); ?></p></td>

                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>" width="50" align="right"><p><? echo number_format($fab_fin_qnty,2); ?></p></td>

                                    <td valign="middle" rowspan="<? echo $rowSpanArray[$po_id][$color_id];?>" width="50" align="right"><p><? echo number_format($fab_req_qty - $fab_fin_qnty); ?></p></td>
                                    <?
									$s++;
                                    }
                                    ?>
                                    <td  width="60"><p><? echo $row['article_number']; ?></p></td>
                                    <td  width="30"><p><? echo $size_Arr_library[$size_id]; ?></p></td>
                                    ...
                                    <td width="50" align="right"><p><? echo number_format($cut_balance,0); ?></p></td>
                                    <td width="50" align="right"><p><? echo number_format($cut_qc,0); ?></p></td>
                                    <td width="50" align="right"><p><? echo number_format($cut_rej,0); ?></p></td>
                                    <td width="50" align="right"><p><? echo number_format($print_issue,0); ?></p></td>
                                    <td width="50" align="right"><p><? echo number_format($print_recive,0); ?></p></td>
                                    <td width="50" align="right"><p><?  echo number_format($print_reject,0); ?></p></td>
                                    <td  width="50" align="right"><p><? echo number_format($embel_issue,0); ?></p></td>
                                    
                                    <td width="50" align="right"><p><? echo number_format($embel_rcv,0); ?></p></td>
                                    <td width="50" align="right"><p><? echo number_format($sewing_in,0); ?></p></td>
                                    <td  width="50" align="right"><p><? echo number_format($input_bal,0); ?></p></td>
                                    ...
                                    <td width="50" align="right"><p><? echo number_format($sewing_bal,0); ?></p></td>
									<td width="50" align="right"><p><? $percent = ($sewing_out / $order_qnty)*100; echo number_format($percent,2) ?></p>%</td>
                                    <td width="50" align="right"><p><? echo number_format($pack_finish,0); ?></p></td>
                                    <td  width="50" align="right"><p><? echo number_format($pack_fin_bal,0); ?></p></td>
									<td  width="50" align="right"><p><?$percent = ($pack_finish / $order_qnty)*100; echo number_format($percent,2);?>%</p></td>
                                    <td  width="50" align="right"><p><? echo number_format($ex_fact_qnty,0); ?></p></td>						
                                    <td width="50" align="right"><p><? echo number_format($ex_fact_bal,0); ?></p></td>
									<td width="50" align="right"><p><?$percent = ($ex_fact_qnty / $order_qnty)*100; echo number_format($percent,2);?>%</p></td>
									<td width="50" align="right"><p><?$percent = ( $ex_fact_qnty/$cut_qnty)*100; echo number_format($percent,2);?>%</p></td>
                                </tr>
                                <? $color++;
							}
							?>
			...
			<div>
				<table border="1" class="rpt_table" width="1830" rules="all" id="report_table_footer" >
					<tr class="gd-color">
						<td width="30">&nbsp;</td>
                        <td width="60">&nbsp;</td>
                        <td width="60">&nbsp;</td>
