...
{ 
		...
		{ 
		   ...
		   for($j=0;$j<$countArr;$j++)
		   {
			   ...
			   if($item_id>0)
			   {
					...
					$rej_value=$gmts_prod_arr[$orderRes[csf("id")]][$item_id][1]['crQty']+$gmts_prod_arr[$orderRes[csf("id")]][$item_id][3]['embQty']+$gmts_prod_arr[$orderRes[csf("id")]][$item_id][5]['sorQty']+$gmts_prod_arr[$orderRes[csf("id")]][$item_id][7]['irnQty']+$gmts_prod_arr[$orderRes[csf("id")]][$item_id][8]['frQty'];
	...
	else if ($type==6)
	{
		...
		if($db_type==2)
		{
		$order_sql="select a.id, b.job_no_prefix_num, a.po_number, b.insert_date, a.job_no_mst,
		...
		{ 
		   $i++;
		                $cutting_qnty=0; $embl_recv_qnty=0;$sewingin_qnty=0;$sewingout_qnty=0;$iron_qnty=0;
						...
						$rcv_wash = 0;  $rcv_special =0;  $embl_recv_qty=0; $rej_value=0; $ex_factory_qnty=0;
					   ...
					   $po_country_data_arr[$orderRes[csf("id")]][$orderRes[csf("shipment_date")]][ex_factory_date][]=$ex_factory_arr[$orderRes[csf("id")]][$item_id][$c_id]['date'];
						...
						$po_country_data_arr[$orderRes[csf("id")]][$orderRes[csf("shipment_date")]][style_name]=$orderRes[csf("style_ref_no")];                        $po_country_data_arr[$orderRes[csf("id")]][$orderRes[csf("shipment_date")]][shipment_date]=$orderRes[csf("shipment_date")];
						...
						$po_country_data_arr[$orderRes[csf("id")]][$orderRes[csf("shipment_date")]][emblish_receive]+=$embl_recv_qnty;                        $po_country_data_arr[$orderRes[csf("id")]][$orderRes[csf("shipment_date")]][sewing_in]+=$sewingin_qnty;
            ...
            <table border="1" class="rpt_table" width="2350" rules="all" id="table_body">
				...
				foreach($po_country_data_arr as $po_id=>$po_value)
				{
					foreach($po_value as $c_date=>$date_value)
					{
						...
						{
						 $ex_date_arr[]=change_date_format($e_date);
						}
                       $shipping_status=0;
                       ...
                       if($shipping_status==1 || $shipping_status==2)
						{
						    ...
						    $days_remian=datediff("d",$date,$date_value[shipment_date]); 
							
							if($date_value[shipment_date] > $date) 
							{
								$color="";
							}
							else if($date_value[shipment_date] < $date) 
							{
								$color="red";
							}														
							else if($date_value[shipment_date] >= $date && $days_remian<=5 ) 
							{
								$color="orange";
							}
						} 
						...
						{
							$days_remian=datediff("d",$ex_date_arr[0],$date_value[shipment_date]);
							if($date_value[shipment_date] >= $ex_date_arr[0]) 
							{ 
								$color="green";
							}
							else if($date_value[shipment_date] < $ex_date_arr[0]) 
							{ 
								$color="#2A9FFF";
							}
					    ...
					    $days_remian=datediff("d",$date_value[shipment_date],$date); 
					 ...
					 ?>
					  <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_2nd<? echo $sl; ?>','<? echo $bgcolor; ?>')" id="tr_2nd<? echo $sl; ?>">
                            ...
                            <td width="80"><p><? echo date("Y",strtotime($date_value[insert_date])); ?></p></td>
                            ...
                            <td width="80" align="center"><p><a href="##" onclick="openmypage_country_ship_date(<? echo $po_id.",'".implode("*",$item_id_all)."','exfactoryCountry_shipdate','','',".$type.",'$country_id_all'"; ?>)"><? echo implode(",",$ex_date_arr); ?></a></p></td>
                            ...
                            <td width="80" align="right" '.$excess_bgcolor.'><? echo $actual_exces_cut; ?></td>
                            ...
                            <td width="80" align="right"><a href="##" onclick="openmypage_country_ship_date(<? echo $po_id.",'".implode("*",$item_id_all)."','country_shipdate_display_qty','8','',".$type.",'$country_id_all'"; ?>)"><? echo $date_value[finish_qty]; $total_finish_qty+=$date_value[finish_qty];?></td>
                            <td width="80" align="right"><? echo number_format($finish_status,2)." %"; ?></a></td>
        ...
        <div style="max-height:425px; overflow-y:scroll; width:518px;" id="scroll_body">
            ...
            <?
             ...
             $sql=sql_select("select sum(ex_factory_qnty) as ex_factory_qnty, ex_factory_date 		  
				  from pro_ex_factory_mst where po_break_down_id in ($po_break_down_id) and item_number_id in ($item_id) and country_id  in ($country_id) and status_active=1 and is_deleted=0 group by ex_factory_date"); 
           $i=1; 
            foreach($sql as $resultRow)
            {
                 if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
...
if ($action=="country_shipdate_display_qty") // popup
{
    ...
    <?
	if($country_id=='') $country_cond=""; else $country_cond=" and country_id in ($country_id) ";
	$item_id=implode(",",explode("*",$item_id));
 	?>
    <fieldset>
    <div style="margin-left:60px">
            ...
            <?
			 ...
			 else if ($production_type==4)
			 {
				 $sql=sql_select("select po_break_down_id,production_date,prod_reso_allo,sewing_line,production_source,sum(production_quantity) as production_quantity
				  from pro_garments_production_mst where po_break_down_id in ($po_break_down_id) and item_number_id in  ($item_id )and status_active=1 and is_deleted=0 and production_type=$production_type $location $floor $country_cond group by po_break_down_id,production_date,prod_reso_allo,sewing_line,production_source");
			 }
			 else
			 {
...
if ($action=="reject_qty")
{
	...
	if($cutting_variable==1)
	{
		...
		{
			if($reportType==5) $tbl_width=350; else $tbl_width=200;
			?>
