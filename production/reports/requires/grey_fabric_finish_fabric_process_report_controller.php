...
if($action == "create_booking_no_search_list_view")
{
	...
	//pro_batch_create_mst//wo_non_ord_samp_booking_mst
 	 ...
 	 $sql= "select a.id,a.booking_no_prefix_num as no_prefix_num,a.booking_no, a.booking_date, a.company_id, a.buyer_id, a.job_no, a.item_category, a.fabric_source, a.supplier_id, a.is_approved, a.ready_to_approved from wo_booking_mst a,wo_booking_dtls b where $company and a.booking_no=b.booking_no and a.booking_type=3 $buyer $booking_no $booking_date and a.status_active=1 and a.is_deleted=0   group by a.id,a.booking_no_prefix_num ,a.booking_no, a.booking_date, a.company_id, a.buyer_id, a.job_no, a.item_category, a.fabric_source, a.supplier_id, a.is_approved, a.ready_to_approved order by id Desc";
...
if($action=="report_generated")
{
	...
	$fabric_isssueToProcess_sql =sql_select("select b.booking_no,b.booking_dtls_id, b.order_id,b.body_part_id,b.color_id,b.width,
		sum(case when a.entry_form=91 then b.batch_issue_qty else 0 end) as batch_issue_qty,
		sum(case when a.entry_form=92 then b.batch_issue_qty else 0 end) as batch_recv_qty,b.febric_description_id    
		from inv_receive_mas_batchroll a, pro_grey_batch_dtls b where a.id=b.mst_id and a.entry_form in(91,92) and  b.order_id in($po_break_down_ids) $comp_cond $process_cond_2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by  b.booking_no,b.booking_dtls_id, b.order_id,b.febric_description_id,b.body_part_id ,b.color_id,b.width");
					...
					<?  
						...
						foreach($sql_serviceBooking as $row)
						{
							...
							<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
		                        ...
		                        <td width="80" align="center"><? echo $row[csf("booking_no_prefix_num")]; ?></td>
								...
								<td width="60" align="right"><? $fab_issue=$fabric_issue_recv_arr[$row[csf("booking_no")]][$row[csf("po_break_down_id")]][$fabric_des_id_arr[$row[csf("pre_cost_fabric_cost_dtls_id")]][$row[csf("booking_no")]]][$fabric_des_body_part_id_arr[$row[csf("pre_cost_fabric_cost_dtls_id")]][$row[csf("booking_no")]]][$row[csf("dia_width")]][$row[csf("gmts_color_id")]][$row[csf("dtls_id")]]['batch_issue_qty']; echo number_format($fab_issue,2,'.',''); 
									//echo $row[csf("booking_no")]."==".$row[csf("po_break_down_id")]."==".$fabric_des_id_arr[$row[csf("pre_cost_fabric_cost_dtls_id")]][$row[csf("booking_no")]]."==".$fabric_des_body_part_id_arr[$row[csf("pre_cost_fabric_cost_dtls_id")]][$row[csf("booking_no")]];

								?></td>
								<td width="60" align="right"><? $fab_recv=$fabric_issue_recv_arr[$row[csf("booking_no")]][$row[csf("po_break_down_id")]][$fabric_des_id_arr[$row[csf("pre_cost_fabric_cost_dtls_id")]][$row[csf("booking_no")]]][$fabric_des_body_part_id_arr[$row[csf("pre_cost_fabric_cost_dtls_id")]][$row[csf("booking_no")]]][$row[csf("dia_width")]][$row[csf("gmts_color_id")]][$row[csf("dtls_id")]]['batch_recv_qty']; echo number_format($fab_recv,2,'.',''); ?></td>
...
if($action=="report_generated_2")
{
	...
	$con = connect();
	$r_id3=execute_query("delete from tmp_booking_no where userid=$user_name");
	$r_id6=execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_name and entry_form in (993,994)");
	if($r_id3 && $r_id6)
	{
		oci_commit($con);
	}

	/* $sql_service_booking="select a.id,b.id as dtls_id,a.booking_no_prefix_num, b.job_no as job_no_prefix_num,a.booking_no,a.booking_date,a.company_id,a.buyer_id,b.po_break_down_id,b.dia_width,b.fabric_color_id,b.gmts_color_id,a.item_category,a.fabric_source,a.supplier_id,a.pay_mode,b.pre_cost_fabric_cost_dtls_id,b.process,sum(b.wo_qnty) as wo_qnty from wo_booking_mst a,wo_booking_dtls b where a.booking_no=b.booking_no and a.booking_type=3 and a.status_active=1 and a.is_deleted=0 and a.company_id='$company' $process_cond $booking_num $order_no_cond_search $order_id_cond $buyer_cond $supplier_cond $year_cond $booking_date_range_cond and b.wo_qnty>0 group by a.id,b.id,a.booking_no_prefix_num, b.job_no,a.booking_no,a.booking_date,a.company_id,a.buyer_id,b.po_break_down_id,b.dia_width,b.fabric_color_id,b.gmts_color_id,a.item_category,a.fabric_source,a.supplier_id,a.pay_mode,b.pre_cost_fabric_cost_dtls_id,b.process order by a.booking_no"; */


	$sql_service_booking="SELECT a.id,b.id as dtls_id,a.booking_no_prefix_num, b.job_no as job_no_prefix_num,a.booking_no,a.booking_date,a.company_id,a.buyer_id,b.po_break_down_id, b.dia_width,b.fabric_color_id,b.gmts_color_id,a.item_category,a.fabric_source,a.supplier_id,a.pay_mode,b.pre_cost_fabric_cost_dtls_id,b.process, c.gsm_weight,sum(b.wo_qnty) as wo_qnty
	from wo_booking_mst a,wo_booking_dtls b, wo_pre_cost_fabric_cost_dtls c, wo_pre_cost_fab_conv_cost_dtls d
	where a.id=b.booking_mst_id and a.booking_type=3 and a.status_active=1 and b.status_active=1
	and b.job_no=c.job_no and c.job_id=d.job_id and b.pre_cost_fabric_cost_dtls_id =d.id and c.id=d.fabric_description
	and a.is_deleted=0 and a.company_id='$company' $process_cond $booking_num $order_no_cond_search $order_id_cond $buyer_cond $supplier_cond $year_cond $booking_date_range_cond and b.wo_qnty>0 
	group by a.id,b.id,a.booking_no_prefix_num, b.job_no,a.booking_no, a.booking_date, a.company_id, a.buyer_id, b.po_break_down_id,b.dia_width,b.fabric_color_id, b.gmts_color_id,a.item_category,a.fabric_source,a.supplier_id, a.pay_mode, b.pre_cost_fabric_cost_dtls_id, b.process, c.gsm_weight
	order by a.booking_no";
	...
	foreach($sql_serviceBooking as $row)
	{
		...
		$all_pre_cost_id_arr[$row[csf("pre_cost_fabric_cost_dtls_id")]]=$row[csf("pre_cost_fabric_cost_dtls_id")];
	...
	fnc_tempengine("GBL_TEMP_ENGINE", $user_name, 993, 1,$all_po_id_arr, $empty_arr);//PO ID
	fnc_tempengine("GBL_TEMP_ENGINE", $user_name, 994, 1,$all_pre_cost_id_arr, $empty_arr);//Pre cost ID

	foreach ($all_booking_no_arr as $s_book) {
		$rID2=execute_query("insert into tmp_booking_no (userid, booking_no) values ($user_name,".$s_book.")");
	}
	if($rID2)
	{
		oci_commit($con);
 	...
 	$po_no_sql=sql_select("select a.id, a.po_number,b.style_ref_no from wo_po_break_down a, wo_po_details_master b, GBL_TEMP_ENGINE c where a.job_id=b.id and a.id=c.ref_val and c.entry_form=993 and c.user_id=$user_name and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
	//a.id in($po_break_down_ids) and 
	foreach($po_no_sql as $row)
	{
		$po_no_arr[$row[csf("id")]]['po_number']=$row[csf("po_number")];
		$po_no_arr[$row[csf("id")]]['style_ref_no']=$row[csf("style_ref_no")];
	...
	$fabric_isssueToProcess_sql =sql_select("SELECT b.booking_no, b.booking_dtls_id, b.order_id, b.body_part_id, c.gmts_color_id, c.fabric_color_id, b.width, b.gsm, b.process_id, 
	sum(case when a.entry_form=91 then b.batch_issue_qty else 0 end) as batch_issue_qty, 
	sum(case when a.entry_form=92 then b.batch_issue_qty else 0 end) as batch_recv_qty,
	sum(case when a.entry_form=92 then b.grey_used else 0 end) as grey_used,
	b.febric_description_id, b.fin_dia, b.fin_gsm    
	from inv_receive_mas_batchroll a, pro_grey_batch_dtls b left join wo_booking_dtls c on b.booking_dtls_id=c.id, GBL_TEMP_ENGINE d where a.id=b.mst_id and a.entry_form in(91,92) and b.order_id=d.ref_val and d.user_id=$user_name and d.entry_form=993 $comp_cond $process_cond_2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.booking_no,b.booking_dtls_id, b.order_id,b.febric_description_id,b.body_part_id, c.gmts_color_id,c.fabric_color_id,b.width, b.gsm, b.process_id, b.fin_dia, b.fin_gsm");
	
	
	foreach($fabric_isssueToProcess_sql as $row)
	{
		$fabric_issue_recv_arr[$row[csf("booking_no")]][$row[csf("order_id")]][$row[csf("febric_description_id")]][$row[csf("body_part_id")]][$row[csf("width")]][$row[csf("gsm")]][$row[csf("process_id")]][$row[csf("gmts_color_id")]][$row[csf("fabric_color_id")]][$row[csf("booking_dtls_id")]]['batch_issue_qty']=$row[csf("batch_issue_qty")];

		$fabric_issue_recv_arr[$row[csf("booking_no")]][$row[csf("order_id")]][$row[csf("febric_description_id")]][$row[csf("body_part_id")]][$row[csf("fin_dia")]][$row[csf("fin_gsm")]][$row[csf("process_id")]][$row[csf("gmts_color_id")]][$row[csf("fabric_color_id")]][$row[csf("booking_dtls_id")]]['batch_recv_qty']=$row[csf("batch_recv_qty")];

		$fabric_issue_recv_arr[$row[csf("booking_no")]][$row[csf("order_id")]][$row[csf("febric_description_id")]][$row[csf("body_part_id")]][$row[csf("fin_dia")]][$row[csf("fin_gsm")]][$row[csf("process_id")]][$row[csf("gmts_color_id")]][$row[csf("fabric_color_id")]][$row[csf("booking_dtls_id")]]['grey_used']=$row[csf("grey_used")];
	}

	//echo "<pre>";
	//print_r($fabric_issue_recv_arr);die;

	$fabric_Desc_sql= sql_select("SELECT a.id,b.lib_yarn_count_deter_id,c.booking_no,b.body_part_id  
		from wo_pre_cost_fab_conv_cost_dtls a, wo_pre_cost_fabric_cost_dtls b, wo_booking_dtls c, GBL_TEMP_ENGINE d, GBL_TEMP_ENGINE e where b.job_no=c.job_no and b.status_active=1 and c.is_deleted=0 and a.id in($pre_cost_fabric_cost_dtls_id) and a.fabric_description = b.id and c.booking_type = 3 and c.po_break_down_id=d.ref_val and d.user_id=$user_name and d.entry_form=993 and a.id=e.ref_val and e.entry_form=994 and e.user_id=$user_name group by a.id,b.lib_yarn_count_deter_id,c.booking_no,b.body_part_id order by c.booking_no");
	//and c.po_break_down_id in($po_break_down_ids)
	...
	$wo_pre_cost_fab_conv_cost_dtls_id=sql_select("SELECT c.job_no,c.id,c.fabric_description as fabric_description_id,c.cons_process,d.body_part_id, d.color_type_id, d.fabric_description as fabric_description, e.body_part_id as job_body_part_id, e.color_type_id as job_color_type_id, e.fabric_description as job_fabric_description
	from wo_booking_dtls b,wo_pre_cost_fab_conv_cost_dtls c 
	left join wo_pre_cost_fabric_cost_dtls d on c.fabric_description= d.id
	left join wo_pre_cost_fabric_cost_dtls e on c.job_no= e.job_no, tmp_booking_no f
	where c.id=b.pre_cost_fabric_cost_dtls_id and b.booking_no=f.booking_no and f.userid = $user_name
	group by c.job_no,c.id,c.fabric_description,c.cons_process, d.body_part_id, d.color_type_id, d.fabric_description, e.body_part_id, e.color_type_id, e.fabric_description");
	...
	foreach( $wo_pre_cost_fab_conv_cost_dtls_id as $row)
	{
		...
		{
			$fabric_description_array[$row[csf("id")]]=$body_part[$row[csf("body_part_id")]].', '.$color_type[$row[csf("color_type_id")]].', '.$row[csf("fabric_description")];
		}

		if($row[csf("fabric_description_id")] == 0)
		{
			$fabric_description_array[$row[csf("id")]].=$body_part[$row[csf("job_body_part_id")]].', '.$color_type[$row[csf("job_color_type_id")]].', '.$row[csf("job_fabric_description")]." ";
		}
	}
	...
	<div align="center">
		<fieldset style="width:1295px;">
			...
			<div align="left">
				<table width="1690" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table" align="left">            	
					<thead>
			        	<tr>
			                <th width="40">SL</th>
			                <th width="80">WO No</th>
				...
				<div style="width:1710px; overflow-y: scroll; max-height:380px;" id="scroll_body">
					<table width="1690" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table" align="left" id="table_body"> 
					<?  
						$i=1; $tot_fab_issue=0; $tot_fab_recv=0; $tot_processLoss=0; $tot_balance=0; $tot_wo_qnty=0;
						$construction_data_arr=array();
						...
						foreach($sql_serviceBooking as $row)
						{
							...
							<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr<? echo $i;?>','<? echo $bgcolor;?>')" id="tr<? echo $i;?>">
								<td width="40"><? echo $i; ?></td>
		                        <td width="80" align="center"><? echo $row[csf("booking_no_prefix_num")]; ?></td>
		                        <td width="120" class="word_wrap"><? 
		                        if($row[csf("pay_mode")]==3 || $row[csf("pay_mode")]==5)
								{ 
									echo $company_library[$row[csf("supplier_id")]] ;
								}
		                        else if($row[csf("pay_mode")]==1){
		                        ...
		                        <td width="120" class="word_wrap"><? echo $buyer_arr[$row[csf("buyer_id")]]; ?>&nbsp;</td>
		                        <td width="120" class="word_wrap"><p><? echo $po_no_arr[$row[csf("po_break_down_id")]]['po_number']; ?>&nbsp;</p></td>
		                        <td width="120" class="word_wrap"><p><? echo $po_no_arr[$row[csf("po_break_down_id")]]['style_ref_no']; ?>&nbsp;</p></td>
		                        <td width="60"><p><? echo $row[csf("dia_width")]; ?>&nbsp;</p></td>
		                        <td width="60"><p><? echo $row[csf("gsm_weight")]; ?>&nbsp;</p></td>
		                        <td width="100" class="word_wrap"><p><? echo $color_library[$row[csf("gmts_color_id")]]; ?>&nbsp;</p></td>
		                        <td width="100" class="word_wrap"><p><? echo $color_library[$row[csf("fabric_color_id")]]; ?>&nbsp;</p></td>
		                        <td width="100" class="word_wrap"><p><? echo $conversion_cost_head_array[$row[csf("process")]]; ?>&nbsp;</p></td>
								
		                        <td width="150" class="word_wrap"><p><? echo $fabric_description_array[$row[csf("pre_cost_fabric_cost_dtls_id")]]; ?>&nbsp;</p></td>
								<td width="60" align="right"><? echo number_format($row[csf("wo_qnty")],2,'.',''); ?></td>
								<td width="60" align="right">
									<? 
									$fab_issue=$fabric_issue_recv_arr[$row[csf("booking_no")]][$row[csf("po_break_down_id")]][$fabric_des_id_arr[$row[csf("pre_cost_fabric_cost_dtls_id")]][$row[csf("booking_no")]]][$fabric_des_body_part_id_arr[$row[csf("pre_cost_fabric_cost_dtls_id")]][$row[csf("booking_no")]]][$row[csf("dia_width")]][$row[csf("gsm_weight")]][$row[csf("process")]][$row[csf("gmts_color_id")]][$row[csf("fabric_color_id")]][$row[csf("dtls_id")]]['batch_issue_qty']; 
									
									echo number_format($fab_issue,2,'.',''); 
								?>
								</td>
								<td width="60" align="right">
									<? $issue_balance=($row[csf("wo_qnty")]-$fab_issue); echo number_format($issue_balance,2,'.',''); ?>
								</td>
								<td width="60" align="right">
									<?
										$grey_used=$fabric_issue_recv_arr[$row[csf("booking_no")]][$row[csf("po_break_down_id")]][$fabric_des_id_arr[$row[csf("pre_cost_fabric_cost_dtls_id")]][$row[csf("booking_no")]]][$fabric_des_body_part_id_arr[$row[csf("pre_cost_fabric_cost_dtls_id")]][$row[csf("booking_no")]]][$row[csf("dia_width")]][$row[csf("gsm_weight")]][$row[csf("process")]][$row[csf("gmts_color_id")]][$row[csf("fabric_color_id")]][$row[csf("dtls_id")]]['grey_used']; 
										echo number_format($grey_used,2,'.',''); 
									?>
								</td>
								<td width="60" align="right">
									<? 
									$fab_recv=$fabric_issue_recv_arr[$row[csf("booking_no")]][$row[csf("po_break_down_id")]][$fabric_des_id_arr[$row[csf("pre_cost_fabric_cost_dtls_id")]][$row[csf("booking_no")]]][$fabric_des_body_part_id_arr[$row[csf("pre_cost_fabric_cost_dtls_id")]][$row[csf("booking_no")]]][$row[csf("dia_width")]][$row[csf("gsm_weight")]][$row[csf("process")]][$row[csf("gmts_color_id")]][$row[csf("fabric_color_id")]][$row[csf("dtls_id")]]['batch_recv_qty']; 
				...
				<table width="1690" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table" align="left">               
			        <tfoot>
			            <tr>
			                ...
			                <th align="right" width="60" id="td_fab_woqnty_id"><? echo number_format($tot_wo_qnty,2,'.',''); ?></th>			                
			                <th align="right" width="60" id="td_fab_issue_id"><? echo number_format($tot_fab_issue,2,'.',''); ?></th>
			                <th align="right" width="60" id="td_fab_issue_id"><? echo number_format($tot_issue_balance,2,'.',''); ?></th>
			                <th align="right" width="60" id="td_fab_issue_id"><? echo number_format($tot_grey_used,2,'.',''); ?></th>
 							<th align="right" width="60" id="td_fab_recv_id"><? echo number_format($tot_fab_recv,2,'.',''); ?></th>			                
 							<th align="right" width="60" id="td_fab_processloss_id"><? echo number_format($tot_processLoss,2,'.',''); ?>
							</th>
