...
if($batch_type==0)
	...
	if ($report_type == 1)
	{
		...
		$sql_batch_dyeing="select a.id as batch_id,a.batch_no,a.extention_no,a.batch_against,f.process_end_date,f.result,f.in_charge_mst_id,f.reason_mst_id ,f.load_unload_id,c.first_name from  pro_batch_create_mst a ,pro_fab_subprocess f,lib_employee c where a.id=f.batch_id and f.in_charge_mst_id=c.id and f.entry_form=35 and  a.batch_against in(1,2) and f.result=1 and f.load_unload_id=2 and f.status_active=1 and f.is_deleted=0 and c.status_active=1 and c.is_deleted=0  and a.status_active=1 and a.working_company_id=$cbo_company_id $in_charge_cond $batch_cond_batch $prod_date_cond order by f.process_end_date asc";
	  ...
	  foreach ($sql_batch_recipe_result as $row) 
	   {
		  $recipeDateWise_batch_Arr[$row[csf('in_charge_mst_id')]][$row[csf('process_end_date')]][$row[csf('batch_no')]].=$row[csf('batch_no')].',';
		  
		   $OtherrecipeDateWise_batch_Arr[$row[csf('process_end_date')]][$row[csf('batch_no')]].=$row[csf('batch_no')].',';
	   }
	   // ======================================= Recipe RFT Info Start ==================================
	    /* $sql_batch_recipe_rft="select b.id as recipe_mst_id,b.dyeing_re_process,a.id as new_batch_id,a.re_dyeing_from as re_batch_id,a.batch_no,a.extention_no,a.batch_against,f.result,f.reason_mst_id ,f.load_unload_id,f.process_end_date,b.recipe_date,f.in_charge_mst_id from  pro_batch_create_mst a ,pro_recipe_entry_mst b ,pro_fab_subprocess f,gbl_temp_engine g where   a.id=b.batch_id  and a.id=f.batch_id   and f.batch_id=b.batch_id  and f.batch_id=g.ref_val and a.id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from=1 and g.entry_form=73  and b.dyeing_re_process=1  and b.entry_form=60  and f.load_unload_id=2  and f.result=1  and f.status_active=1 and f.is_deleted=0    and a.status_active=1 and a.working_company_id=$cbo_company_id $batch_cond_batch   order by f.process_end_date asc";*/
		 
		  $sql_batch_recipe_rft="select b.id as recipe_mst_id,b.dyeing_re_process,a.id as new_batch_id,a.re_dyeing_from as re_batch_id,a.batch_no,a.extention_no,a.batch_against,f.result,f.reason_mst_id ,f.load_unload_id,f.process_end_date,b.recipe_date,f.in_charge_mst_id from  pro_batch_create_mst a ,pro_recipe_entry_mst b ,pro_fab_subprocess f  where   a.id=b.batch_id  and a.id=f.batch_id   and f.batch_id=b.batch_id  and b.dyeing_re_process=1  and b.entry_form=60  and f.load_unload_id=2  and f.result in(1,2)  and f.status_active=1 and f.is_deleted=0    and a.status_active=1 and a.working_company_id=$cbo_company_id $batch_cond_batch  $prod_date_cond  order by f.process_end_date asc";
	 ...
	 foreach ($sql_batch_recipe_result_rft as $row) 
	  {
		 if($row[csf('extention_no')])
		 {
			...
			$other_rft_recipeDateWise_batch_Arr[$row[csf('process_end_date')]][$row[csf('batch_no')]].=$row[csf('recipe_mst_id')].',';
		 	$other_rft_recipeDateWise_batchNo_Arr[$row[csf('process_end_date')]][$row[csf('batch_no')]].=$row[csf('batch_no')].',';
		 }
		 else
		 {
			 $rft_recipeDateWise_batch_Arr[$row[csf('in_charge_mst_id')]][$row[csf('process_end_date')]][$row[csf('batch_no')]].=$row[csf('recipe_mst_id')].',';
					...
					<tbody id="scroll_body">
						...
						foreach($in_chargeData as $prod_date=>$row)
						{
							if ($i%2==0)  
								...
								$no_of_ok_batch=rtrim($reprocess_batch_idArr[$in_charge_id][$prod_date],',');//$others_reprocess_batch_idArr[$In_charge][$min_prod_date]
								$others_no_of_ok_batch=rtrim($others_reprocess_batch_idArr[$in_charge_id][$prod_date],',');
								 // echo $first_batch.'='.$first_batch_re.'<br>';
								if($others_no_of_ok_batch!="")
								{
									$others_no_of_ok_batchArr=array_unique(explode(",",$others_no_of_ok_batch));
									$others_no_of_ok_batchArr_tot=count($others_no_of_ok_batchArr);
								}
								...
								$rft_recipeDateWise_all="";$topping_not_ok_lot_recipe_all="";$tot_rft_recipe_batch_count='';$topping_recipeDateWise_batchTot="";
								foreach($first_batchArr as $rft_batch)
								{
								//echo $first_batch_re.'='.$rft_batch.',';
								
								if($first_batch_re!="")
								{
									$recipeDateWise_batch=rtrim($OtherrecipeDateWise_batch_Arr[$prod_date][$rft_batch],',');
									
									$rft_recipeDateWiseAll=chop($other_rft_recipeDateWise_batch_Arr[$prod_date][$rft_batch],',');
									$rft_BatchNoAll=chop($other_rft_recipeDateWise_batchNo_Arr[$prod_date][$rft_batch],',');
								...
								{
									$recipeDateWise_batch=rtrim($recipeDateWise_batch_Arr[$in_charge_id][$prod_date][$rft_batch],',');
							...
							<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i.$in_charge_id; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i.$in_charge_id; ?>" valign="top">
								...
								<td width="120" align="center" title="<?=$first_batch;?>"><? if($first_batch_re) echo $no_of_unload_batch=0;else echo $no_of_unload_batch; ?></td>
								<td width="100" align="center" title="<?=$first_batch;?>"><?   
								if($others_no_of_ok_batch) 
								{
									 $no_of_ok_batchTot=$no_of_unload_batch;
								}
								else { $no_of_ok_batchTot=$no_of_unload_batch-$no_of_ok_batchArr_tot; }
								if($first_batch_re) echo  $no_of_ok_batchTot=0;else echo $no_of_ok_batchTot; ?></td>
								<td width="100"  align="center"title="No. of OK Batch/No. of Unload Batch [Excluding Re-Dyeing Batch]*100">
											<? if($first_batch_re) echo ""; else echo fn_number_format(($no_of_ok_batchTot/$no_of_unload_batch)*100,2); ?></td>
								<td width="150" align="center"><? if($first_batch_re) echo $not_ok_lots=0; else echo $not_ok_lots; ?></td>
						...
						<tr style="text-align: center; background:#CCC;font-size:16px;">
							...
							<td width="80"><b><? echo fn_number_format(($tot_tot_rft_recipe_batch_count/$tot_topping_recipeDateWise_batchTot)*100, 2, '.', ''); ?></b></td>
							<td width="100">&nbsp;</td>
							<td width="">&nbsp;</td>
