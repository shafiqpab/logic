...
{ 	
	...
	$sql.=" FROM pro_garments_production_mst a, wo_po_break_down c, wo_po_details_master b
		where a.production_type in (5) and a.po_break_down_id=c.id and c.job_id=b.id and a.status_active=1 and a.is_deleted=0 $company_cond $floor_cond $line $txt_date_cond
	...
	if ($count_hour != 0) $trend=$totalProdQnty/$count_hour*16;
	...
	$dhu = ($totalGoodAlrSpotRejectQnty/$totalProdQnty)*100; //(qcQty/total_qty)*100-100;
	if ($count_hour != 0 && $operator != 0) $efficiency=(($totalProdQnty*$total_smv)/($operator*$count_hour*60))*100;
	...
	WHERE a.po_break_down_id=d.id and d.job_no_mst=c.job_no and a.status_active=1 and c.status_active=1 and d.status_active=1 and a.production_type in(4,5) and d.id in($order_ids) $company_cond $floor_cond $line
    		...
    		<table style="width: 100%; height: 10%; background-color: #16365c; overflow: hidden; clear: both; margin-left: 0; float: left;">
	    		...
	    		foreach($production_data as $line => $row)
				{
		    		for($h=$hour; $h<=$last_hour; $h++)
		    		{
						...
						if ($operator != 0) $actual_efficiency=(($row[$prod_hour]*$total_smv)/($operator*60))*100;
