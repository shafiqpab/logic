...
{	
	...
	{	
		...
		if($line_wise_job_info_array[$val[csf('sewing_line')]]['job_no']!="")
		{
			...
			$line_wise_job_info_array[$val[csf('sewing_line')]]['grouping'].="**".$val[csf('ref')];
	...
	// echo "<pre>"; print_r($line_wise_job_info_array);die();
    ...
    if($smv_source==3) // from gsd enrty
	{
		...
		$sql_item="SELECT a.id,a.APPLICABLE_PERIOD,a.BULLETIN_TYPE,A.TOTAL_SMV,A.STYLE_REF,a.GMTS_ITEM_ID  from PPL_GSD_ENTRY_MST a where a.APPLICABLE_PERIOD <= $txt_date and A.IS_DELETED=0 and A.STATUS_ACTIVE=1 $style_cond and a.APPROVED=1 
	...
	$tbl_width = 865+($last_hour - ($hour+1))*40;
                	...
                	foreach($production_serial_arr as $floor_id=>$flr_data) 
                	{
	                	...
	                	foreach($flr_data as $sl=>$sl_data) 
	                	{
							...
							{   
								...
								$int_ref = implode(",", array_unique(array_filter(explode("**", $line_wise_job_info_array[$l_id]['grouping']))));
			                   ...
			                   <tr bgcolor="<? echo $bgcolor;?>" id="tr_<?= $i;?>" onClick="change_color('tr_<?= $i; ?>','<?= $bgcolor; ?>')" title="<?=$title;?>">
				                    ...
				                    <td width="75"><p style="<?=$style;?>"><?=$int_ref;?></p></td>
				                    ...
				                    <td width="45" align="center"><p style="<?=$style;?>"><?=number_format($item_smv,2);?></p></td>
			            ...
			            <tr style="background: #1F1D36;text-align: right;">
	                        <td colspan="5"><?=$floorArr[$floor_id];?></td>
