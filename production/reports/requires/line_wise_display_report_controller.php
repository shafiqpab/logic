...
{ 	
	...
	if($db_type==0)
	{
		...
		for($h=$hour;$h<$last_hour;$h++)
        {
			...
			if($first==1)
			{
			 $sql.="sum(CASE WHEN a.production_hour<='$end' and a.production_type in(13,16) THEN production_quantity else 0 END) AS $prod_hour,";
			}
			else
			{
		    $sql.="sum(CASE WHEN a.production_hour>'$bg' and  a.production_hour<='$end' and a.production_type in(13,16) THEN production_quantity else 0 END) AS $prod_hour,";
			}
			$first=$first+1;
		}
		$prod_hour='prod_hour'.$last_hour;
		$sql.="sum(CASE WHEN  a.production_hour>'$start_hour_arr[$last_hour]' and a.production_hour<='$start_hour_arr[24]' and a.production_type in(13,16) THEN production_quantity else 0 END) AS $prod_hour";
	
		$sql.=" FROM pro_garments_production_mst a, wo_po_details_master b, wo_po_break_down c
			where a.entry_form in(349,355) and a.production_type in (13,16) and a.po_break_down_id=c.id and c.job_no_mst=b.job_no and a.status_active=1 and a.is_deleted=0 $company_name $line $txt_date_cond
	...
	else
	{
		...
		for($h=$hour;$h<$last_hour;$h++)
        {
			...
			if($first==1)
			{
			    $sql.=" sum(CASE WHEN TO_CHAR(a.production_hour,'HH24:MI')<='$end' and a.production_type in(13,16) THEN production_quantity else 0 END) AS $prod_hour,";
			}
			else
			{
		        $sql.=" sum(CASE WHEN TO_CHAR(a.production_hour,'HH24:MI')>'$bg' and  TO_CHAR(a.production_hour,'HH24:MI')<='$end' and a.production_type in(13,16) THEN production_quantity else 0 END) AS $prod_hour,";
			}
			$first=$first+1;
		}
		$prod_hour='prod_hour'.$last_hour;
		$sql.=" sum(CASE WHEN  TO_CHAR(a.production_hour,'HH24:MI')>'$start_hour_arr[$last_hour]' and TO_CHAR(a.production_hour,'HH24:MI')<='$start_hour_arr[24]' and a.production_type in(13,16) THEN production_quantity else 0 END) AS $prod_hour";
	...
	if($db_type==0)
	{
		...
		sum(case when a.production_type in(12,15) and a.production_date<='$txt_date' then a.production_quantity else 0 END) as total_sewing_input,
		sum(case when a.production_type in(13,16) and a.production_date<='$txt_date' then a.production_quantity else 0 END) as total_sewing_output, 
		sum(case when a.production_type in(12,15) and a.production_date='$txt_date' then a.production_quantity else 0 END) as today_sewing_input,
		sum(case when a.production_type in(12,15) and a.production_date<'$txt_date' then a.production_quantity else 0 END) as total_sewing_input_except_current_date,
		sum(case when a.production_type in(13,16) and a.production_date<'$txt_date' then a.production_quantity else 0 END) as total_sewing_output_except_current_date
		FROM pro_garments_production_mst a, wo_po_details_master c, wo_po_break_down d 
		WHERE a.po_break_down_id=d.id and d.job_no_mst=c.job_no and a.status_active=1 and c.status_active=1 and d.status_active=1 and a.entry_form in(348,349,354,355) and a.production_type in(12,13,15,16) $company_name $line 
