...
{ 
...

	{
...

		{
			...
			$sql="select a.job_no,a.job_quantity , a.company_name, a.buyer_name, a.style_ref_no, a.order_uom, a.gmts_item_id, a.set_break_down, a.total_set_qnty as ratio, group_concat(distinct(b.id)) as po_id, group_concat(concat_ws('**',b.id,b.po_number,b.po_quantity)) as po_data, c.item_number_id, c.location, c.floor_id, c.prod_reso_allo, c.sewing_line, sum(c.production_quantity) as qnty from wo_po_details_master a, wo_po_break_down b, pro_garments_production_mst c where a.job_no=b.job_no_mst and a.company_name='$comapny_id' and b.id=c.po_break_down_id and c.location like '$location'  and c.sewing_line like '$line' and production_date=$txt_date and production_type=5 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $buyer_id_cond $floor group by a.job_no,a.job_quantity , c.location, c.floor_id, c.prod_reso_allo, c.sewing_line order by c.location, c.floor_id, c.sewing_line";//, c.item_number_id
...

		{
			...
			$sql="select a.job_no,a.job_quantity , a.company_name, a.buyer_name, a.style_ref_no, a.order_uom, a.gmts_item_id, a.set_break_down, a.total_set_qnty as ratio, LISTAGG(b.id, ',') WITHIN GROUP (ORDER BY b.id) as po_id, LISTAGG(cast(b.id || '**' || b.po_number || '**' || b.po_quantity as varchar2(4000)), ',') WITHIN GROUP (ORDER BY b.id) as po_data, c.item_number_id, c.location, c.floor_id, c.prod_reso_allo, c.sewing_line, sum(c.production_quantity) as qnty from wo_po_details_master a, wo_po_break_down b, pro_garments_production_mst c where a.job_no=b.job_no_mst and a.company_name='$comapny_id' and b.id=c.po_break_down_id and c.location like '$location' and c.sewing_line like '$line' and production_date=$txt_date and production_type=5 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $buyer_id_cond $floor group by a.job_no, a.company_name, a.buyer_name, a.style_ref_no, a.order_uom, a.gmts_item_id, a.set_break_down, a.total_set_qnty, c.location, c.floor_id, c.prod_reso_allo, c.sewing_line, c.item_number_id,a.job_quantity  order by c.location, c.floor_id, c.sewing_line";	
...

		{
			...
			}//'.$floor_item_smv.'   '.$location_item_smv.'
			...
			$po_ids = array_unique(explode(",",$row[csf("po_id")]));//explode(",",$row[csf("po_id")]);
			...
			$today_smv=$prod_resource_array[$row[csf('sewing_line')]][$pr_date]['tsmv'];
...

	{
		...
		$sql="select a.id, a.location_id, a.floor_id, a.line_number, b.active_machine, b.pr_date, b.man_power, b.operator, b.helper, b.line_chief, b.target_per_hour, b.working_hour from prod_resource_mst a, prod_resource_dtls b where a.id=b.mst_id and a.company_id=$comapny_id and a.location_id like '$location' and a.id like '$line' and pr_date=$txt_date  $floor_con order by a.location_id, a.floor_id, a.line_number";
...
<script>
	...
	<fieldset style="width:970px; margin-left:5px">
		...
		<div id="report_container">
         ...
         <div style="width:970px; max-height:370px; overflow-y:scroll" id="scroll_body">
