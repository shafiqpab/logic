...
{ 
	...
	if($type==1)
	{
		...
		/*echo "select a.id, a.location_id, a.floor_id, a.line_number, b.active_machine, b.pr_date, b.man_power, b.operator, b.helper, b.line_chief, b.target_per_hour, b.working_hour from prod_resource_mst a, prod_resource_dtls b where a.id=b.mst_id and a.company_id=$comapny_id";die;*/
		
		/*echo "select a.id, a.location_id, a.floor_id, a.line_number, b.active_machine, b.pr_date, b.man_power, b.operator, b.helper,b.smv_adjust,b.smv_adjust_type, b.line_chief, b.target_per_hour, b.working_hour,c.from_date,c.to_date,c.capacity from prod_resource_mst a, prod_resource_dtls b,prod_resource_dtls_mast c where a.id=c.mst_id and c.id=b.mast_dtl_id and a.company_id=$comapny_id  and b.is_deleted=0 and c.is_deleted=0";die;*/
		...
		foreach( $result as $row )
		{
			...
			$mst_sql= "SELECT  
						...
						where  
							po_break_down_id in(".$row[csf("po_id")].") and item_number_id='".$row[csf("item_number_id")]."' and location='".$row[csf("location")]."' and floor_id='".$row[csf("floor_id")]."' and prod_reso_allo='".$row[csf("prod_reso_allo")]."' and sewing_line='".$row[csf("sewing_line")]."' and is_deleted=0 and status_active=1"; 
		   ...
		   // $no_of_days=return_field_value("count(id)","lib_capacity_calc_dtls","date_calc between '$fstinput_date' and $txt_date and day_status=1");
			...
			foreach($dataLibArray as $libRow)
			{
				...
				$smv_adjustmet_type=$prod_resource_array[$row[csf('sewing_line')]][$pr_date]['smv_adjust_type'];
				if(str_replace("'","",$smv_adjustmet_type)==1) $total_adjust=$prod_resource_array[$row[csf('sewing_line')]][$pr_date]['smv_adjust'];
				else if(str_replace("'","",$smv_adjustmet_type)==2) $total_adjust=($prod_resource_array[$row[csf('sewing_line')]][$pr_date]['smv_adjust'])*(-1);
			...
			$html.='<td width="40">'.$i.'</td>
					...
					<td width="75" align="right"><a href="##" onclick="openmypage('.$comapny_id.",'".$row[csf('po_id')]."',".$row[csf('item_number_id')].",".$row[csf('location')].",".$row[csf('floor_id')].",".$row[csf('sewing_line')].",".$txt_date.",'prev_qnty',4".",".$row[csf('prod_reso_allo')].')">'.$prev_input_qnty.'</a>&nbsp;</td>
					<td width="75" align="right"><a href="##" onclick="openmypage('.$comapny_id.",'".$row[csf('po_id')]."',".$row[csf('item_number_id')].",".$row[csf('location')].",".$row[csf('floor_id')].",".$row[csf('sewing_line')].",".$txt_date.",'prev_qnty',5".",".$row[csf('prod_reso_allo')].')">'.$prev_out_qnty.'</a>&nbsp;</td>
