...
if ($action == "report_generate")
{
	...
	else if($report_type==4) // show2
	{
	   ...
	   $sql_country_w="SELECT  dm.delivery_company_id, dm.delivery_location_id, m.id, m.po_break_down_id, m.foc_or_claim, m.ex_factory_date, m.ex_factory_qnty as ex_qnty, a.buyer_name,a.DEALING_MARCHANT, a.job_no_prefix_num,b.id as po_id,  a.job_no, a.style_ref_no, a.ship_mode, b.po_number, b.shipment_date, b.pub_shipment_date, b.shiping_status, c.country_ship_date, c.order_quantity, c.order_rate, c.order_total, c.shiping_status as cshiping_status, d.id as del_dtls_id, d.production_qnty, d.production_qnty*c.order_rate as ex_value, c.id as color_size $delayShortSelect
        from pro_ex_factory_delivery_mst dm, pro_ex_factory_mst m, wo_po_break_down b, wo_po_details_master a, wo_po_color_size_breakdown c,
        pro_ex_factory_dtls d
        where dm.id = m.delivery_mst_id and m.PO_BREAK_DOWN_ID=b.id and a.id = c.job_id and a.id = b.job_id and b.id = c.po_break_down_id and m.id = d.mst_id and d.color_size_break_down_id = c.id  and b.id=m.po_break_down_id $sql_cond and dm.delivery_company_id<>0 and m.entry_form<>85 and dm.delivery_company_id is not null and dm.status_active = 1 and d.status_active = 1  and d.is_deleted = 0  and dm.is_deleted = 0 and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0 and m.status_active = 1 and m.is_deleted = 0 
		order by m.id";
		
		//echo $sql_country_w;die;
    
	    $country_w_res = sql_select($sql_country_w);
	    $data_buyer_wise = array(); $qntyChkArr = array(); $result_array= array(); $jobPoWiseArr= array();$po_id_array= array(); $poIds='';
	    ...
	    foreach($country_w_res as  $row)
	    {
			...
			if($dtls_id_check[$row[csf("del_dtls_id")]]=="")
			{
				$dtls_id_check[$row[csf("del_dtls_id")]]=$row[csf("del_dtls_id")];
				$data_buyer_wise[$row[csf("buyer_name")]]["ex_qnty"] += $row[csf("production_qnty")];
       ...
       // print_r($job_id_array);
	    $order_qty_cond=where_con_using_array($po_id_array,0,"PO_BREAK_DOWN_ID");
		$order_qty_sql="SELECT PO_BREAK_DOWN_ID,ORDER_QUANTITY from WO_PO_COLOR_SIZE_BREAKDOWN where STATUS_ACTIVE=1 and IS_DELETED=0  $order_qty_cond ";
		//echo $order_qty_sql;die;
		$order_qty_arr=array();
		foreach (sql_select($order_qty_sql) as  $value) 
		{
			$order_qty_arr[$value['PO_BREAK_DOWN_ID']]['po_qty']+=$value['ORDER_QUANTITY'];
		}
						...
						else // Country Ship Date wise
						{
							...
							foreach($data_buyer_wise as $buyerId => $row)
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" <? echo $stylecolor; ?> onClick="change_color('se_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="se_<? echo $i; ?>">
                                    ...
                                    <td width="100" align="right"><? echo number_format($data_buyer_wise[$buyerId]["ex_val"]-$buyerInvoiceValue[$buyerId],2);?></td>
						...
						foreach($jobData as $delComp => $delCompData)
						{
							foreach($delCompData as $delLoca => $row)
							{
								...
								<tr bgcolor="<?=$bgcolor;?>" <?=$stylecolor;?> onClick="change_color('tr4_<?=$q;?>', '<?=$bgcolor;?>');" id="tr4_<?=$q;?>">
                                    ...
                                    <td width="100" align="right"><?=number_format($order_qty_arr[$poid]['po_qty'],2);?></td>
								...
								$totDtlsPoQty += $order_qty_arr[$poid]['po_qty'];
	...
	else if($report_type==5)  // Button show 3
	{
		if ($date_from && $date_to && $cbo_type != 4)
		{
			if ($db_type == 0) {
	   ...
	   $sql_country_w="SELECT  dm.delivery_company_id, dm.delivery_location_id, m.id, m.po_break_down_id, m.foc_or_claim, m.ex_factory_date, m.ex_factory_qnty as ex_qnty, a.buyer_name,a.DEALING_MARCHANT, a.job_no_prefix_num, a.job_no, a.style_ref_no, a.ship_mode, b.po_number, b.shipment_date, b.pub_shipment_date, b.shiping_status, c.country_ship_date, c.order_quantity, c.order_rate, c.order_total, c.shiping_status as cshiping_status, d.production_qnty, d.production_qnty*c.order_rate as ex_value, c.id as color_size,m.actual_po $delayShortSelect,d.id as dtls_id
        ...
        pro_ex_factory_dtls d
        where dm.id = m.delivery_mst_id and a.job_no = c.job_no_mst and a.job_no = b.job_no_mst and b.id = c.po_break_down_id and m.id = d.mst_id $sql_cond and d.color_size_break_down_id = c.id  and dm.delivery_company_id<>0 and m.entry_form<>85 and dm.delivery_company_id is not null and dm.status_active = 1 and dm.is_deleted = 0 and a.status_active = 1 and a.is_deleted = 0 and m.status_active = 1 and m.is_deleted = 0 and m.id in (select ap.mst_id from pro_ex_factory_actual_po_details ap where ap.is_deleted=0 and (ap.actual_po_id is not null or ap.actual_po_dtls_id is not null))  order by m.id";
		//echo $sql_country_w;die;

	    $country_w_res = sql_select($sql_country_w);
	    $data_buyer_wise = array(); $qntyChkArr = array(); $result_array= array(); $jobPoWiseArr= array(); $poIds='';
	    $exf_mst_id_arr=array();
	    $po_id_arrs=array();
	    foreach($country_w_res as  $row)
	    {
	    	$po_id_arrs[$row[csf('po_break_down_id')]]=$row[csf('po_break_down_id')];
			$exf_mst_id_arr[$row[csf('id')]]=$row[csf('id')];
		}
		//CLIENT_ID
		$po_id_cond=where_con_using_array($po_id_arrs,0,"b.id");
	    ...
	    $exf_mst_id_cond=where_con_using_array($exf_mst_id_arr,0,"b.mst_id");

	    $sql_actutal_po="SELECT a.acc_ship_date,b.mst_id FROM wo_po_acc_po_info a,pro_ex_factory_actual_po_details b,wo_po_acc_po_info_dtls c  where  ( a.id=b.actual_po_id or c.id = b.actual_po_dtls_id) and a.id = c.mst_id and c.is_deleted = 0 and  a.status_active=1 AND a.is_deleted=0 and  b.status_active=1 AND b.is_deleted=0 and a.acc_ship_date is not null $exf_mst_id_cond group by a.acc_ship_date,b.mst_id";
		//echo $sql_actutal_po;
	    $actutal_po_data=return_library_array(  $sql_actutal_po, "mst_id", "acc_ship_date"  );
	    ...
	    foreach($country_w_res as  $row)
	    {
			...
			$time_diff_buffer = strtotime($actutal_po_data[$row[csf("id")]]) - strtotime($row[csf("ex_factory_date")]);


			$time_diff = strtotime($actutal_po_data[$row[csf("id")]]) - strtotime($row[csf("ex_factory_date")]);
			$shipDate=$actutal_po_data[$row[csf("id")]];
			$shippingStatus=$row[csf("shiping_status")];
	        ...
	        if($time_diff_buffer > 0)
	        {
				...
				$result_array[$row[csf("delivery_company_id")]][$client_id][$row[csf("buyer_name")]]['early_dtls_id'] .= $row[csf("dtls_id")] .",";
	        ...
	        else if($time_diff_buffer < 0 )
	        {
				...
				$result_array[$row[csf("delivery_company_id")]][$client_id][$row[csf("buyer_name")]]['delay_dtls_id'] .= $row[csf("dtls_id")] .",";
	        ...
	        else if($time_diff_buffer == 0)
	        {
				...
				$result_array[$row[csf("delivery_company_id")]][$client_id][$row[csf("buyer_name")]]['ontime_dtls_id'] .= $row[csf("dtls_id")] .",";
	        ...
	        $result_array[$row[csf("delivery_company_id")]][$client_id][$row[csf("buyer_name")]]['client_name'] = $buyerArr[$client_id]["name"];
	        $result_array[$row[csf("delivery_company_id")]][$client_id][$row[csf("buyer_name")]]['client_id'] = $client_id;
			...
			<fieldset  style="width:1350px; float:left;">
				...
				<div style=" max-height:350px; width:1350px; overflow-y:scroll;" id="scroll_body2">
					<table style="width:1330px" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" >
						...
						foreach($result_array as $company_id => $company_data)
						{
							//$com_data = $company_data;
							foreach($company_data as $client_id => $client_wise_data)
							{
								$price[$client_id] = $buyerArr[$client_id]["name"];
							}
							array_multisort($price, SORT_ASC, $company_data);
							foreach($company_data as $client_id => $client_wise_data)
							{
								
								foreach($client_wise_data as $buyer_id => $row)
								{
									$mainArr[$company_id][$row['client_id']][$buyer_id] = $row;
						...
						foreach($mainArr as $company_id => $company_data)
						{
						...
						foreach($mainArr as $company_id => $company_data)
						{
							...
							foreach($company_data as $client_id => $client_wise_data)
							{
								...
								foreach($client_wise_data as $buyer_id => $row)
								{
									...
									$com_td_span = $company_row_span_arr[$company_id]+$client_t[$company_id];
									$po_arr =  array_filter(array_unique(explode(",",chop($row["po_id"],","))));
									$ex_return_qnty = 0;$ex_return_value=0;
									foreach($po_arr as $po_id)
									{
									...
									?>
	                                    ...
	                                    <td width="70" style="word-break: break-all;" align="right"><a href="##" onclick="openmypage_actual_popup('<?=$company_id;?>','<?=$client_id;?>','<?=$client_id;?>','early_popup','<?=chop($row['early_dtls_id'],",");?>');"><?=fn_number_format($row["early"], 0);?></a></td>
	                                    ...
	                                    <td width="70" style="word-break: break-all;" align="right"><a href="##" onclick="openmypage_actual_popup('<?=$company_id;?>','<?=$client_id;?>','<?=$client_id;?>','ontime_popup','<?=chop($row['ontime_dtls_id'],",");?>');"><?=fn_number_format($row["ontime"], 0);?></a></td>
	                                    ...
	                                    <td width="70" style="word-break: break-all;" align="right"><a href="##" onclick="openmypage_actual_popup('<?=$company_id;?>','<?=$client_id;?>','<?=$client_id;?>','delay_popup','<?=chop($row['delay_dtls_id'],",");?>');"><?=fn_number_format($row["delay"], 0);?></a></td>
	...
	else if($report_type==6)  // Button show 4
	{
		if ($date_from && $date_to && $cbo_type != 4)
		{
			if ($db_type == 0) {
		...
		$sql_country_w="SELECT  dm.delivery_company_id, dm.delivery_location_id, m.id, m.po_break_down_id, m.foc_or_claim, m.ex_factory_date, m.ex_factory_qnty as ex_qnty, a.buyer_name,a.DEALING_MARCHANT, a.job_no_prefix_num, a.job_no, a.style_ref_no, a.ship_mode, b.po_number, b.shipment_date, b.pub_shipment_date, b.shiping_status,m.actual_po $delayShortSelect,e.ex_fact_qty as production_qnty,f.unit_price*e.ex_fact_qty as  ex_value,g.acc_ship_date,e.id as ex_fact_acc_po_row
		from pro_ex_factory_delivery_mst dm, pro_ex_factory_mst m, wo_po_details_master a, wo_po_break_down b,pro_ex_factory_actual_po_details e,wo_po_acc_po_info_dtls f, wo_po_acc_po_info g
		where dm.id = m.delivery_mst_id  and a.id = b.job_id  and m.id=e.mst_id and e.actual_po_id = f.mst_id and e.actual_po_dtls_id=f.id and g.id=f.mst_id and f.po_break_down_id = b.id   $sql_cond   and dm.delivery_company_id<>0 and m.entry_form<>85 and dm.delivery_company_id is not null and dm.status_active = 1 and dm.is_deleted = 0 and a.status_active = 1 and a.is_deleted = 0 and m.status_active = 1 and m.is_deleted = 0 and (e.actual_po_id is not null or e.actual_po_dtls_id is not null) and e.is_deleted = 0  order by m.id";
		//echo $sql_country_w;die;
	   ...
	   $po_id_arrs=array();
		...
		$con = connect();
		execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ref_from=1 and ENTRY_FORM=36");
		oci_commit($con);
				
		fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 36, 1, $po_id_arrs, $empty_arr);//PO ID=1

		$client_id_arr=return_library_array( "select a.client_id,b.id from wo_po_details_master a, wo_po_break_down b,gbl_temp_engine g where a.id=b.job_id and b.id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in(1) and g.entry_form=36  and a.is_deleted=0 and b.is_deleted=0  order by id desc", "id", "client_id"  );
	    ...
	    foreach($country_w_res as  $row)
	    {
			...
			$time_diff = strtotime($acc_ship_date) - strtotime($row[csf("ex_factory_date")]);
			$shipDate=$acc_ship_date;
			$shippingStatus=$row[csf("shiping_status")];
	        ...
	        if($time_diff_buffer > 0)
	        {
	            $data_buyer_wise[$row[csf("buyer_name")]]["early"] += $row[csf("production_qnty")];
				$result_array[$row[csf("delivery_company_id")]][$client_id][$row[csf("buyer_name")]]['early'] += $row[csf("production_qnty")];
				$result_array[$row[csf("delivery_company_id")]][$client_id][$row[csf("buyer_name")]]['early_value'] += $row[csf("ex_value")];
				$result_array[$row[csf("delivery_company_id")]][$client_id][$row[csf("buyer_name")]]['early_dtls_id'][$row[csf("id")]] = $row[csf("id")] ;

				$jobPoWiseArr[$poStr][$row[csf("delivery_company_id")]][$client_id]['early']+= $row[csf("production_qnty")];
				$jobPoWiseArr[$poStr][$row[csf("delivery_company_id")]][$client_id]['early_value']+= ($row[csf("ex_value")] );
	        }
	        else if($time_diff_buffer < 0 )
	        {
	            $data_buyer_wise[$row[csf("buyer_name")]]["delay"] += $row[csf("production_qnty")];
				...
				$result_array[$row[csf("delivery_company_id")]][$client_id][$row[csf("buyer_name")]]['delay'] += $row[csf("production_qnty")];
				$result_array[$row[csf("delivery_company_id")]][$client_id][$row[csf("buyer_name")]]['delay_value'] += $row[csf("ex_value")];
				$result_array[$row[csf("delivery_company_id")]][$client_id][$row[csf("buyer_name")]]['delay_dtls_id'][$row[csf("id")]] = $row[csf("id")] ;

				$jobPoWiseArr[$poStr][$row[csf("delivery_company_id")]][$client_id]['delay']+= $row[csf("production_qnty")];
				$jobPoWiseArr[$poStr][$row[csf("delivery_company_id")]][$client_id]['delay_value']+= ($row[csf("ex_value")] );
	        }
	        else if($time_diff_buffer == 0)
	        {
	            $data_buyer_wise[$row[csf("buyer_name")]]["ontime"] += $row[csf("production_qnty")];
				...
				$result_array[$row[csf("delivery_company_id")]][$client_id][$row[csf("buyer_name")]]['ontime'] += $row[csf("production_qnty")];
				$result_array[$row[csf("delivery_company_id")]][$client_id][$row[csf("buyer_name")]]['ontime_value'] += $row[csf("ex_value")];
				$result_array[$row[csf("delivery_company_id")]][$client_id][$row[csf("buyer_name")]]['ontime_dtls_id'][$row[csf("id")]] = $row[csf("id")];

				$jobPoWiseArr[$poStr][$row[csf("delivery_company_id")]][$client_id]['ontime']+= $row[csf("production_qnty")];
				$jobPoWiseArr[$poStr][$row[csf("delivery_company_id")]][$client_id]['ontime_value']+= ($row[csf("ex_value")] );
	        }
		...
		$sqlQty="SELECT a.id, a.shipment_date, a.pub_shipment_date, b.country_ship_date, c.production_qnty
	        from wo_po_break_down a, wo_po_color_size_breakdown b, pro_ex_factory_dtls c,gbl_temp_engine g
	        where a.id = b.po_break_down_id and c.color_size_break_down_id = b.id and b.po_break_down_id=g.ref_val and c.po_break_down_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in(1) and g.entry_form=36 and a.status_active = 1 and a.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0 ";
		$sqlQty_res = sql_select($sqlQty); $totExQtyArr=array();
		foreach($sqlQty_res as  $row)
		{
			$shipDate="";
		...
		$invoiceSql = "select a.ID,a.BUYER_ID,a.LOCATION_ID,a.CLAIM_AMMOUNT,b.PO_BREAKDOWN_ID,b.CURRENT_INVOICE_VALUE  from COM_EXPORT_INVOICE_SHIP_MST a, COM_EXPORT_INVOICE_SHIP_DTLS b,gbl_temp_engine g where a.id=B.MST_ID and b.PO_BREAKDOWN_ID=g.ref_val    and g.user_id = ".$user_id." and g.ref_from in(1) and g.entry_form=36 ";
		$invoiceSqlResult = sql_select($invoiceSql); $invoiceValue=array();
		foreach($invoiceSqlResult as  $row)
		{
			$client_id=$client_id_arr[$row[PO_BREAKDOWN_ID]];
		...
		execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id."  and ENTRY_FORM=36");
		oci_commit($con);
		disconnect($con);
							...
							foreach($company_data as $client_id => $client_wise_data)
							{
								...
								foreach($client_wise_data as $buyer_id => $row)
								{
									...
									?>
	                                    ...
	                                    <td width="70" style="word-break: break-all;" align="right"><a href="##" onclick="openmypage_actual_popup('<?=$company_id;?>','<?=$client_id;?>','<?=$client_id;?>','early_popup','<?=implode(",", $row['early_dtls_id']);?>','actual_po_poup_for_button_4');"><?=fn_number_format($row["early"], 0);?></a></td>
	                                    <td width="50" style="word-break: break-all;" align="right"><?=fn_number_format($earlyPer, 2);?></td>
	                                    <td width="70" style="word-break: break-all;" align="right"><?=fn_number_format($row["early_value"] , 0);?></td>
	                                    <td width="50" style="word-break: break-all;" align="right"><?=fn_number_format(($row["early_value"] * 100) / $ex_value_after_return, 2);?></td>

	                                    <td width="70" style="word-break: break-all;" align="right"><a href="##" onclick="openmypage_actual_popup('<?=$company_id;?>','<?=$client_id;?>','<?=$client_id;?>','ontime_popup','<?=implode(",", $row['ontime_dtls_id']);?>','actual_po_poup_for_button_4');"><?=fn_number_format($row["ontime"], 0);?></a></td>
	                                    <td width="50"  style="word-break: break-all;" align="right"><?=fn_number_format($onTimePer, 2);?></td>
	                                    <td width="70" style="word-break: break-all;" align="right"><?=fn_number_format($row["ontime_value"] , 0);?></td>
	                                    <td width="50" style="word-break: break-all;" align="right"><?=fn_number_format(($row["ontime_value"] * 100) / $ex_value_after_return, 2);?></td>

	                                    <td width="70" style="word-break: break-all;" align="right"><a href="##" onclick="openmypage_actual_popup('<?=$company_id;?>','<?=$client_id;?>','<?=$client_id;?>','delay_popup','<?=implode(",", $row['delay_dtls_id']);?>','actual_po_poup_for_button_4');"><?=fn_number_format($row["delay"], 0);?></a></td>
	                                    <td width="50" style="word-break: break-all;" align="right"><?=fn_number_format($delayPer, 2);?></td>
	                                    <td width="70" style="word-break: break-all;" align="right"><?=fn_number_format($row["delay_value"] , 0);?></td>
	                                     <td width="50" style="word-break: break-all;" align="right"><?=fn_number_format(($row["delay_value"]  * 100) / $ex_value_after_return, 2);?></td>
									...
									$grand_claim_val+=array_sum($invoiceClimValue[$client_id][$buyer_id]);
									 ...
									 $sub_early+=$row["early_value"];
									 $sub_ontime+=$row["ontime_value"];
									 $sub_delay+=$row["delay_value"];
									 $tot_early+=$row["early_value"];
									 $tot_ontime+=$row["ontime_value"];
									 $tot_delay+=$row["delay_value"];
	...
	else if($report_type==7) // show5
	{
	   ...
	   $sql_country_w="SELECT  dm.delivery_company_id, dm.delivery_location_id,dm.challan_no,dm.lock_no,dm.transport_supplier, m.id, m.po_break_down_id, m.foc_or_claim, m.ex_factory_date, a.buyer_name,a.DEALING_MARCHANT, a.job_no_prefix_num,b.id as po_id, a.job_no, a.style_ref_no, a.ship_mode, b.po_number, b.shipment_date, b.pub_shipment_date, b.shiping_status,b.unit_price, c.country_ship_date, c.order_quantity, c.order_rate, c.order_total, c.shiping_status as cshiping_status, d.id as del_dtls_id, d.production_qnty, d.production_qnty*c.order_rate as ex_value, c.id as color_size $delayShortSelect,
	   	(case when m.entry_form = 85 then d.production_qnty else 0 end ) as return_qnty,
		(case when m.entry_form != 85 then d.production_qnty else 0 end ) as ex_qnty
        from pro_ex_factory_delivery_mst dm, pro_ex_factory_mst m, wo_po_break_down b, wo_po_details_master a, wo_po_color_size_breakdown c,
        pro_ex_factory_dtls d
        where dm.id = m.delivery_mst_id and m.PO_BREAK_DOWN_ID=b.id and a.id = c.job_id and a.id = b.job_id and b.id = c.po_break_down_id and m.id = d.mst_id and d.color_size_break_down_id = c.id $sql_cond and dm.delivery_company_id<>0  and dm.delivery_company_id is not null  and dm.status_active = 1 and dm.is_deleted = 0 and a.status_active = 1 and a.is_deleted = 0 and m.status_active = 1 and m.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0   and d.status_active = 1 and d.is_deleted = 0
		order by m.id";
	    ...
	    $country_w_res = sql_select($sql_country_w);
	    $data_buyer_wise = array(); $qntyChkArr = array(); $result_array= array(); $jobPoWiseArr= array(); $poIds='';$po_id_array= array();
		$k=1;
		
		//echo count($country_w_res);
	    foreach($country_w_res as  $row)
	    {
			...
			$result_array[$row[csf("delivery_company_id")]][$row[csf("buyer_name")]]['ex_qnty'] += $row[csf("ex_qnty")];
			$result_array[$row[csf("delivery_company_id")]][$row[csf("buyer_name")]]['return_qnty'] += $row[csf("return_qnty")];
	        $result_array[$row[csf("delivery_company_id")]][$row[csf("buyer_name")]]['ex_value'] += $row[csf("ex_value")];
			$result_array[$row[csf("delivery_company_id")]][$row[csf("buyer_name")]]['unit_price'] += $row[csf("unit_price")];
			...
			$result_array[$row[csf("delivery_company_id")]][$row[csf("buyer_name")]]['buyer_name'] = $row[csf("buyer_name")];
			
			
			$jobPoWiseArr[$poStr][$row[csf("delivery_company_id")]][$row[csf("delivery_location_id")]]['ex_qnty']+= $row[csf("ex_qnty")];
			$jobPoWiseArr[$poStr][$row[csf("delivery_company_id")]][$row[csf("delivery_location_id")]]['ex_value']+= $row[csf("ex_value")];
			$jobPoWiseArr[$poStr][$row[csf("delivery_company_id")]][$row[csf("delivery_location_id")]]['ex_date']=$row[csf("ex_factory_date")];
			$jobPoWiseArr[$poStr][$row[csf("delivery_company_id")]][$row[csf("delivery_location_id")]]['challan_no']=$row[csf("challan_no")];
			$jobPoWiseArr[$poStr][$row[csf("delivery_company_id")]][$row[csf("delivery_location_id")]]['lock_no']=$row[csf("lock_no")];
			$jobPoWiseArr[$poStr][$row[csf("delivery_company_id")]][$row[csf("delivery_location_id")]]['transport_supplier']=$row[csf("transport_supplier")];
			$jobPoWiseArr[$poStr][$row[csf("delivery_company_id")]][$row[csf("delivery_location_id")]]['unit_price']=$row[csf("unit_price")];
			$jobPoWiseArr[$poStr][$row[csf("delivery_company_id")]][$row[csf("delivery_location_id")]]['order_rate']+=$row[csf("order_rate")];
			$jobPoWiseArr[$poStr][$row[csf("delivery_company_id")]][$row[csf("delivery_location_id")]]['return_qnty']+=$row[csf("return_qnty")];
			$jobPoWiseArr[$poStr][$row[csf("delivery_company_id")]][$row[csf("delivery_location_id")]]['BUYER_NAME'] =$row[csf("BUYER_NAME")];
			
			$fob_arr[$row[csf("delivery_company_id")]] [$row[csf("buyer_name")]]['fob'] += $row[csf("unit_price")];
			$fob_arr [$row[csf("delivery_company_id")]] [$row[csf("buyer_name")]]['row'] += 1;
		...
		fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 36, 1, $po_id_arr, $empty_arr);//PO ID

		$sql_order_dtls=("SELECT a.po_break_down_id,a.order_quantity FROM wo_po_color_size_breakdown a,gbl_temp_engine b WHERE a.po_break_down_id=b.ref_val and b.user_id = $user_id and b.entry_form=36 and b.ref_from=1 and a.status_active=1 AND a.is_deleted=0");
		// echo $sql_order_dtls;die;
		$order_dtls_arr=array();
	
		foreach (sql_select($sql_order_dtls) as  $value) 
		{
			$order_dtls_arr[$value['PO_BREAK_DOWN_ID']]['po_qty']+=$value['ORDER_QUANTITY'];
		}
		//  echo "<pre>";print_r($order_dtls_arr);die;

		// foreach($jobPoWiseArr as $poid=>$podata)
		// {
		// 	foreach($podata as $cid=>$cdata)
		// 	{
		// 		foreach($cdata as $cid=>$cdata)
		// 		{
		// 			$postr_arr=explode('__',$poid);
		// 			$fob_arr [$postr_arr[0]]['unit_price'] +=$cdata['unit_price'];
		// 			$fob_arr [$postr_arr[0]]['row'] +=1;
		...
		if($db_type==2 && $tot_rows>1000)
		{
			...
			{
				$ids=implode(",",$ids);
				$poIds_country_cond.=" b.po_break_down_id in($ids) or ";
			}
			$poIds_country_cond=chop($poIds_country_cond,'or ');
			$poIds_country_cond.=")";
		}
		...
		{
			$poIds_country_cond=" and b.po_break_down_id in ($poIds)";
		}
		$sql_cond3="";
		$sql_cond3= " and b.ex_factory_date between '" . date("j-M-Y", strtotime($date_from)) . "' and '" . date("j-M-Y", strtotime($date_to)) . "'";
		$sql_res=("SELECT b.po_break_down_id,b.ex_factory_date, c.color_size_break_down_id, d.order_rate,
		(case when b.entry_form = 85 then c.production_qnty else 0 end ) as return_qnty,	
		 from pro_ex_factory_mst b, pro_ex_factory_dtls c, wo_po_color_size_breakdown d where b.id = c.mst_id and c.color_size_break_down_id = d.id and b.status_active=1 and b.is_deleted=0
        and c.status_active = 1 and c.is_deleted = 0  and d.status_active = 1 and d.is_deleted = 0 $sql_cond3 ".where_con_using_array(explode(",",$poIds),0,'b.PO_BREAK_DOWN_ID')."" );
		//echo $sql_res;die;
		$ex_return_qty_arr=array();
		foreach(sql_select($sql_res) as $row)
		{
			$ex_return_value =  $row[csf('return_qnty')]*$row[csf('order_rate')];
			$ex_return_qty_arr[$row[csf('po_break_down_id')]][$row[csf('color_size_break_down_id')]]['return_qty']+=$row[csf('return_qnty')];
			$ex_return_qty_arr[$row[csf('po_break_down_id')]][$row[csf('color_size_break_down_id')]]['ex_qnty']+=$row[csf('ex_qnty')];
			$ex_return_qty_arr[$row[csf('po_break_down_id')]][$row[csf('color_size_break_down_id')]]['order_rate']+=$row[csf('order_rate')];

			$ex_return_qty_arr[$row[csf('po_break_down_id')]][$row[csf('color_size_break_down_id')]]['return_value']=$ex_return_value;
			$ex_return_qty_arr[$row[csf('po_break_down_id')]]['color_size_list'].=$row[csf('color_size_break_down_id')].",";
		}
		//echo "<pre>";print_r($result_array);die;
		// $order_qty_cond=where_con_using_array($po_id_array,0,"PO_BREAK_DOWN_ID");
		// $order_qty_sql="SELECT PO_BREAK_DOWN_ID,ORDER_QUANTITY from WO_PO_COLOR_SIZE_BREAKDOWN where STATUS_ACTIVE=1 and IS_DELETED=0  $order_qty_cond ";
		...
		execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ref_from in (1,2) and ENTRY_FORM=36");
		oci_commit($con);
		disconnect($con);
		...
		<div>
			...
			<fieldset  style="width:650px; float:left;">
				<table style="width:630px" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" >
					<thead>
						<tr>
							<td colspan="12" align="center" style="font-size:15px; font-weight:bold;">Export Summary By Production Factory</td>
						</tr>
						<tr>
							<th width="150" rowspan="2">Production Factory</th>
							<th width="130" rowspan="2">Buyer</th>
							<th width="100" rowspan="2"><p>Export (Qty)</p></th>     
							<th width="100" rowspan="2"><p>Return (Qty)</p></th>         
							<th width="100" rowspan="2">FOB</th>
							<th width="100" rowspan="2">Export Value in USD</th>
				...
				<div style=" max-height:350px; width:650px; overflow-y:scroll;" id="scroll_body2">
				<table style="width:630px" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" >
					<?
					$buyer_row_span_arr = array();
					foreach($result_array as $company_id => $company_data)
					{
						...
						foreach($company_data as $buyer_id => $row)
						{
							...
							$buyer_row_span_arr[$company_id] =$buyer_row_span;
						}
					}
					...
					foreach($jobPoWiseArr as $jobRow => $jobData)
					{
						foreach($jobData as $delComp => $delCompData)
						{
							...
							{ 
								// echo "<pre>";
								// print_r($row);
								$total_num_of_fob[$delComp][$row['BUYER_NAME']]['FOBDATA'] +=  $row['unit_price'];
								$total_num_of_buyer[$delComp][$row['BUYER_NAME']]['FOB_BUYER'] +=  count($row['BUYER_NAME']);
					...
					foreach($result_array as $company_id => $company_data)
					{
						...
						{
							$y = 1;
							 $Fob_total = $total_num_of_fob[$delComp][$buyer_id]['FOBDATA'];
						     $CountBuyer    =   $total_num_of_buyer[$delComp][$buyer_id]['FOB_BUYER'];
							 
							 $avg_fob_final = $Fob_total /$CountBuyer ;
								if ($m % 2 == 0) $bgcolor = "#E9F3FF"; else $bgcolor = "#FFFFFF";
								$buyer_td_span = $buyer_row_span_arr[$company_id."*".$location_id];
								$po_arr =  array_filter(array_unique(explode(",",chop($row["po_id"],","))));
								$ex_return_qnty = 0;$ex_return_value=0;
								foreach($po_arr as $po_id)
								{
									...
									foreach($color_size_arr as $color_size_id)
									{
										...
										$ex_return_value +=  $ex_return_qty_arr[$po_id][$color_size_id]['return_value'];
										$fob += $ex_return_qty_arr[$po_id][$color_size_id]['order_rate'];
									}
								...
								$row["po_id"]; $row["color_size"];
								//$fob_arr[$row[csf("delivery_company_id")]] [$row[csf("buyer_name")]]['fob'] += $row[csf("unit_price")];
								$avg_fob=$fob_arr[$company_id][$buyer_id]['fob'] / $fob_arr[$company_id][$buyer_id]['row'];
								//print_r($fob_arr);die;
								//echo $avg_fob;

								$buyer_ex_qty=$row['ex_qnty'];
							
								$ex_value=  $row['ex_qnty'] * $avg_fob_final;

								?>
									<td width="130"><?=$buyerArr[$buyer_id]["name"];?></td>
									<td width="100" align="right"><a href="##" onclick="openmypage_ex_popup_one('<?=$company_id;?>','<?=$buyer_id;?>','','<?=$date_from;?>','<?=$date_to;?>','<?=$export_qnty;?>');"><?=$buyer_ex_qty;?></a></td>
									<td width="100" align="right"><?=number_format($row['return_qnty'], 2);?></td>
									<td width="100" align="right"><?=number_format($avg_fob_final, 2);?></td>
									<td width="100" align="right"><?=number_format($ex_value, 2);?></td>
								</tr>
								<?
						
								$grand_export_qnty += $buyer_ex_qty;
								$grand_return_qnty += $row['return_qnty'];
								$grand_ex_value += $ex_value_after_return;
								$gr_ex_value +=$ex_value;
					...
					<tr style="background-color:#e0e0e0; font-weight: bold">
						...
						<td align="right"><?=number_format($grand_export_qnty, 2);?></td>
						<td align="right"><?=number_format($grand_return_qnty, 2);?></td>
						<td align="right"><p></p></td>
						<td align="right"><?=number_format($gr_ex_value, 2);?></td>
					</tr>
				</table>
				</div>
			</fieldset>
				...
				<table style="width:1680px" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" >
					<thead>
						...
						<tr>
							...
							<th width="80">Transport Company</th>
							...
							<th width="90">Current Ex-Fact. Value</th>
							<th width="80">Excess Shipment Qty</th>
							<th>Ex-Fact Status</th>
						</tr>
					</thead>
				</table>
					...
					foreach($jobPoWiseArr as $jobRow => $jobData)
					{
						foreach($jobData as $delComp => $delCompData)
						{
							foreach($delCompData as $delLoca => $row)
							{
								...
								$totShipQty=$totExQtyArr[$poid][$shipDate];
								$return_val=$row["return_qnty"]*$row["unit_price"];
								$ex_qnty=($row["ex_qnty"]);
								//echo $ex_qnty;
								?>
								<tr bgcolor="<?=$bgcolor;?>" <?=$stylecolor;?> onClick="change_color('tr4_<?=$q;?>', '<?=$bgcolor;?>');" id="tr4_<?=$q;?>">
                                    ...
                                    <td width="70" style="word-break:break-all"><?=change_date_format($shipDate);?></td>
                                    <td width="100" align="right"><?=number_format($order_dtls_arr[$poid]['po_qty'],2);?></td>
                                    <td width="90" style="word-break:break-all"><?=$shipment_mode[$ship_mode];?></td>
                                    <td width="70" style="word-break:break-all"><?=change_date_format($row["ex_date"]);?></td>
									<td width="80" style="word-break:break-all"><?=$row["challan_no"];?></td>
									<td width="80" style="word-break:break-all"><?=$row["lock_no"];?></td>
									<td width="80" style="word-break:break-all"><?=$trans_com_arr[$row["transport_supplier"]];?></td>
									
									<td width="90" align="right"><?=number_format($ex_qnty)?></td>	
									<td width="90" align="right"><?=number_format($row["return_qnty"],2);?></td>							
									<td width="80" align="right"><?=number_format($row["unit_price"], 2);?></td>
									<td width="90" align="right"><?=number_format($return_val,2);?></td>		

									<td width="90" align="right"><?=number_format($row["ex_value"], 2);?></td>
									<td width="80" align="right">
									<?
									$a=$ex_qnty;
									$b=($order_dtls_arr[$poid]['po_qty']);
									if($a>$b){
										$c=$ex_qnty-$order_dtls_arr[$poid]['po_qty'];
										echo $c;
										$tot_ship_qnty +=$c;
									}
									else{
								...
								$totDtlsOntime+=$row["ontime"];
								$totDtlsDelay+=$row["delay"];
								$totDtlsPoQty += $order_dtls_arr[$poid]['po_qty'];
								$totDtlsExQty+=$ex_qnty;
								$totDtlsExVal+=$row["ex_value"];
								// echo $c."hhh"."<br>";
								$totDtlsShipQty+=$totShipQty;
							
								$gr_return_qnty+=$row["return_qnty"];
								$gr_return_value_qnty+=$return_val;
								$q++;
							}
						}
					}
				...
				<table style="width:1680px" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" >
					<tfoot>
                        <tr style="background-color:#e0e0e0; font-weight: bold">
							...
							<td width="80">&nbsp;</td>
							<td width="80">&nbsp;</td>
                            <td width="90" align="right" id="totDtlsExQty"><?=number_format($totDtlsExQty);?></td>
							<td width="90" align="right"><?=number_format($gr_return_qnty,2);?></td>
							<td width="80">&nbsp;</td>
							<td width="90" align="right"><?=number_format($gr_return_value_qnty,2);?></td>
							<td width="90" align="right" id="value_totDtlsExVal"><?echo number_format($totDtlsExVal);?></td>
							<td width="80" align="right"><?=number_format($tot_ship_qnty,2)?></td>
                            <td></td>
                        </tr>
					</tfoot>
				</table>
...
if($action == "ex_qnty_popup_one")
{
    echo load_html_head_contents("Country Order Dtls Info", "../../../", 1, 1,'','','');
     extract($_REQUEST);
    //$color_size = chop($color_size,",");
    $order_data=sql_select( "select b.po_number,b.grouping, b.id from wo_po_color_size_breakdown a, wo_po_break_down b where a.po_break_down_id = b.id"); //and a.id in ($color_size)
	$order_arr=array();
	foreach($order_data as $row){
		$order_arr[$row[csf("id")]]["order_no"]=$row[csf("po_number")];
		$order_arr[$row[csf("id")]]["internal_ref"]=$row[csf("grouping")];
    ...
    <fieldset style="width:670px; margin-left:3px">
		...
		$sql = "SELECT c.id as color_size_mst_id,c.country_id,c.po_break_down_id,d.color_size_break_down_id,d.production_qnty as production_qnty,c.size_number_id,c.color_number_id,dm.delivery_company_id
		from pro_ex_factory_delivery_mst dm,pro_ex_factory_mst m, wo_po_details_master a, wo_po_color_size_breakdown c, pro_ex_factory_dtls d
		where dm.id = m.delivery_mst_id and a.id = c.job_id and m.id = d.mst_id
		and d.color_size_break_down_id = c.id and dm.delivery_company_id <> 0 and m.entry_form <> 85 and dm.delivery_company_id is not null
		and dm.status_active = 1 and dm.is_deleted = 0 and a.status_active = 1 and a.is_deleted = 0 and m.status_active = 1 and m.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0 and d.status_active = 1 and d.is_deleted = 0
		$sql_cond 
		order by c.po_break_down_id,c.country_id";
		//echo $sql;die;
        ...
        $result = sql_select($sql);
			...
			foreach ($result as $row)
				...
				<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
					...
					<td width="100"><div style="word-wrap:break-word; width:100px"><? echo $color_library[$row[csf('color_number_id')]]; ?></div></td>
					<td width="100"><div style="word-wrap:break-word; width:100px"><? echo $size_library[$row[csf('size_number_id')]]; ?></div></td>
					<td align="right" title="<? echo "Export Qty=".$size_ex_qnty;?>">
						<p>
							<?
								
								echo number_format($row['PRODUCTION_QNTY'],2);
							?>
						</p>
					</td>
				</tr>
			<?
			$total += number_format($row['PRODUCTION_QNTY'],2,".","");
			$i++;
		}
		?>
		</tbody>
        ...
        <div id="report_div" align="center" style="overflow-y: 350px;width: 740px;">
            <table border="1" class="rpt_table" rules="all" width="720" cellpadding="0" cellspacing="0" align="center">
                <thead>
	                ...
	                <th width="100" style="word-wrap:break-word;">Color</th>
	                <th width="70" style="word-wrap:break-word;">Actual po qty.</th>
	                <th width="70" style="word-wrap:break-word;">Rate</th>
	                <th width="80" style="word-wrap:break-word;">Export Qnty</th>
	                <th  style="word-wrap:break-word;">Exporty Value</th>
                ...
                <tbody id="table_body">
                        ...
                        $con = connect();
						execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ref_from=2 and ENTRY_FORM=36");
						oci_commit($con);
								
						fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 36, 2, explode(",", $dtls_id), $empty_arr);//PO ID=1


                        //$cond = where_con_using_array(explode(",", $dtls_id),0,"a.id");
						...
						$sql = "SELECT d.job_no,
							       ...
							       c.id as ex_fact_acc_po_row,
							       d.acc_ship_date,
							        c.ex_fact_qty * e.unit_price AS ex_fact_val,
							        e.unit_price
							  FROM pro_ex_factory_mst a,
							       pro_ex_factory_actual_po_details c,
							       wo_po_acc_po_info d,
							       wo_po_acc_po_info_dtls e,gbl_temp_engine g
							 WHERE     c.mst_id = a.id
							       AND c.actual_po_id = d.id
							       and d.id = e.mst_id
							       ...
							       and a.is_deleted = 0
							        ...
							        and a.id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in(2) and g.entry_form=36
                        ...
                        execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id."  and ENTRY_FORM=36 and ref_from in(2) ");
						oci_commit($con);
						disconnect($con);
                        $i = 1;
                        $dtls_id_arr = array( );
							...
							if($types == "early_popup" && $time_diff_buffer > 0 )
							{
	                        	if(empty($ex_fact_acc_po_row_arr[$row[csf('ex_fact_acc_po_row')]]))
	                        	{
		                            ...
		                            <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
		                                ...
		                                <td width="100"><div style="word-wrap:break-word;"><? echo $color_library[$row[csf('gmts_color_id')]]; ?></div></td>
		                                <td width="70" align="right"><div style="word-wrap:break-word;justify-content: right;text-align: right;"><? echo number_format($row[csf('po_qty')]); ?></div></td>
		                                <td width="70" align="center"><div style="word-wrap:break-word;justify-content: right;text-align: center;"><? echo number_format($row[csf('unit_price')],4); ?></div></td>
		                                <td width="80" align="right">
		                                	<div style="word-wrap:break-word;justify-content: right;text-align: right;"><? echo number_format($row[csf('ex_fact_qty')]); ?></div>
		                                </td>
		                                <td  align="right">
		                                	<div style="word-wrap:break-word;justify-content: right;text-align: right;"><? echo number_format($row[csf('ex_fact_val')],2); ?></div>
		                                </td>
		                            </tr>
			                        <?
			                        $po_qty+=$row[csf('po_qty')];
			                        $ex_fact_qty+=$row[csf('ex_fact_qty')];
			                         $ex_fact_val+=$row[csf('ex_fact_val')];
			                        $i++;
			                    }
							}
							else if($types == "ontime_popup" && $time_diff_buffer == 0 )
                    ...
                    <tfoot>
		        	    <tr class="tbl_bottom">
		        	        ...
		        	        <td width="80" align="right"><? echo number_format($ex_fact_qty);?></td>
		        	        <td  align="right"><? echo number_format($ex_fact_val);?></td>
