...
if($action=="generate_report2") //Show 2
{
	...
	if ($cbo_prod_type>0 && $cbo_party_id>0) $cbo_prod_type_cond="and f.service_company in($cbo_party_id) "; else $cbo_prod_type_cond="";	
	if ($cbo_prod_type>0) $cbo_prod_source_cond="and f.service_source in($cbo_prod_type) "; else $cbo_prod_source_cond="";
	// if ($cbo_prod_type>0) $out_source_cond="and a.knitting_source in($cbo_prod_type) "; else $out_source_cond="";

	if ($cbo_result_name==0) $result_name_cond=""; else $result_name_cond="  and f.result='".$cbo_result_name."' ";
	if ($shift==0) $shift_name_cond=""; else $shift_name_cond="  and f.shift_name='".$shift."' ";
	if ($machine=="") $machine_cond=""; else $machine_cond =" and f.machine_id in ( $machine ) ";
	if ($machine=="") $out_machine_cond=""; else $out_machine_cond =" and b.machine_no_id in ( $machine ) ";
	if ($floor_name==0 || $floor_name=='')
	{
		$floor_id_cond="";$floor_id_cond2="";
	}
	...
	$hidden_booking_cond=""; $out_booking_cond="";
	if($booking_no!="")
	{
		$hidden_booking_cond="and a.booking_no like '%$booking_no%' ";
		$out_booking_cond="and d.sales_booking_no like '%$booking_no%' ";
	}
	//echo $hidden_booking_cond.'='.$booking_no;
	...
	if ($batch_no=="") $batch_num=""; else $batch_num="  and a.batch_no='".trim($batch_no)."' ";
	if ($batch_no=="") $batch_numCond=""; else $batch_numCond="  and e.batch_no='".trim($batch_no)."' ";
	...
	if ($cbo_prod_type>0 && $cbo_party_id>0) $out_party_cond="and a.knitting_company in($cbo_party_id) "; else $out_party_cond="";
	if ($working_company==0) $out_working_companyCond=""; else $out_working_companyCond=" and a.company_id=$working_company";
	if ($company==0) $out_lc_companyCond=""; else $out_lc_companyCond=" and d.buyer_id=$company and d.within_group=1";
	...
	if($txt_date_from && $txt_date_to)
	{
		if($db_type==0)
		{
			...
			if($date_search_type==1)
			{
				$dates_com=" and  f.process_end_date BETWEEN '$date_from' AND '$date_to' ";
				$dates_com2=" and  f.process_end_date BETWEEN '$date_start' AND '$date_end' ";
				$receive_date_cond=" and  a.receive_date BETWEEN '$date_start' AND '$date_end' ";
			}
			else
			{
				$dates_com=" and f.insert_date between '".$date_from."' and '".$date_to." 23:59:59' ";
				$dates_com2=" and f.insert_date between '".$date_start."' and '".$date_end." 23:59:59' ";
				$recv_insert_cond=" and a.insert_date between '".$date_start."' and '".$date_end." 23:59:59' ";
			}
		}
		elseif($db_type==2)
		{
			...
			if($date_search_type==1)
			{
				$dates_com="and  f.process_end_date BETWEEN '$date_from' AND '$date_to'";
				$dates_com2="and  f.process_end_date BETWEEN '$date_start' AND '$date_end'";
				$receive_date_cond="and  a.receive_date BETWEEN '$date_from' AND '$date_to'";
			}
			else
			{
				$dates_com=" and f.insert_date between '".$date_from."' and '".$date_to." 11:59:59 PM' ";
				$dates_com2=" and f.insert_date between '".$date_start."' and '".$date_end." 11:59:59 PM' ";
				$recv_insert_cond=" and a.insert_date between '".$date_from."' and '".$date_to." 11:59:59 PM' ";
			}
		}
	}
	...
	else if($cbo_type==2) // Dyeing Production Report
	{
		if(str_replace("'",'',$cbo_batch_type)==0 || str_replace("'",'',$cbo_batch_type)==1)
		{
			...
			from pro_batch_create_dtls b, fabric_sales_order_mst d, pro_fab_subprocess f, pro_batch_create_mst a 
			where f.batch_id=a.id $companyCond $workingCompany_name_cond2  $dates_com $po_job_cond $fsodata $buyerdata $batch_num $year_cond $result_name_cond $shift_name_cond $machine_cond $floor_id_cond $cbo_prod_type_cond $cbo_prod_source_cond $hidden_booking_cond and a.entry_form=0  and a.id=b.mst_id and f.batch_id=b.mst_id and f.entry_form=35 and f.load_unload_id=2 and a.batch_against in(1,11,2,3) and b.po_id=d.id and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and a.is_sales=1 and b.is_sales=1
			$order_by";
		}
		...
		if(str_replace("'",'',$cbo_batch_type)==0)
		{
			// inv_receive_master, inv_transaction, pro_finish_fabric_rcv_dtls, order_wise_pro_details, pro_batch_create_mst
			$outbound_sql="SELECT a.id, a.receive_date, a.knitting_company, b.batch_id, b.dia_width_type, b.color_id, b.fabric_description_id, b.gsm, b.width as dia, b.uom, c.quantity, c.prod_id, d.job_no as fso_no, d.within_group, d.style_ref_no, d.sales_booking_no, d.buyer_id, d.season, d.po_buyer, d.booking_id, e.batch_no
			from inv_receive_master a, pro_finish_fabric_rcv_dtls b, order_wise_pro_details c, fabric_sales_order_mst d, pro_batch_create_mst e
			where a.id=b.mst_id and b.id=c.dtls_id and c.po_breakdown_id=d.id and b.batch_id=e.id and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and a.entry_form=225 and a.knitting_source=3 and a.receive_basis=14  and c.trans_type=1 and c.is_sales=1 and c.entry_form=225 $out_lc_companyCond $out_working_companyCond $receive_date_cond $recv_insert_cond $fsodata $batch_numCond $out_buyer_cond $year_cond $out_machine_cond $out_party_cond $buyerdata $out_source_cond $out_booking_cond";// and d.id=2276 and c.po_breakdown_id=2276
			/*$sub_job_cond $suborder_no $result_name_cond $floor_id_cond $order_by*/
			// echo $outbound_sql;
			$outbound_sql_result=sql_select($outbound_sql);
			foreach($outbound_sql_result as $row)
			{
				$outbound_data_arr[$row[csf('batch_id')]][$row[csf('prod_id')]]['receive_date']=$row[csf('receive_date')];
				$outbound_data_arr[$row[csf('batch_id')]][$row[csf('prod_id')]]['within_group']=$row[csf('within_group')];
				$outbound_data_arr[$row[csf('batch_id')]][$row[csf('prod_id')]]['po_buyer']=$row[csf('po_buyer')];
				$outbound_data_arr[$row[csf('batch_id')]][$row[csf('prod_id')]]['buyer_id']=$row[csf('buyer_id')];
				$outbound_data_arr[$row[csf('batch_id')]][$row[csf('prod_id')]]['dyeing_company']=$row[csf('knitting_company')];
				$outbound_data_arr[$row[csf('batch_id')]][$row[csf('prod_id')]]['booking_no']=$row[csf('sales_booking_no')];
				$outbound_data_arr[$row[csf('batch_id')]][$row[csf('prod_id')]]['style_ref_no']=$row[csf('style_ref_no')];
				$outbound_data_arr[$row[csf('batch_id')]][$row[csf('prod_id')]]['fso_no']=$row[csf('fso_no')];
				$outbound_data_arr[$row[csf('batch_id')]][$row[csf('prod_id')]]['dia_width_type']=$row[csf('dia_width_type')];
				$outbound_data_arr[$row[csf('batch_id')]][$row[csf('prod_id')]]['color_id']=$row[csf('color_id')];
				$outbound_data_arr[$row[csf('batch_id')]][$row[csf('prod_id')]]['fabric_description_id']=$row[csf('fabric_description_id')];
				$outbound_data_arr[$row[csf('batch_id')]][$row[csf('prod_id')]]['season']=$row[csf('season')];
				$outbound_data_arr[$row[csf('batch_id')]][$row[csf('prod_id')]]['batch_no']=$row[csf('batch_no')];
				if ($row[csf('uom')]==27) 
				{
					$qnty_conversion = ($row[csf('quantity')] * 36 * $row[csf('dia')] * $row[csf('gsm')]) / (1550 * 1000);
				}
				else
				{
					$qnty_conversion = $row[csf('quantity')];
				}
				$outbound_data_arr[$row[csf('batch_id')]][$row[csf('prod_id')]]['quantity']+=$qnty_conversion;
				...
				$shade_matched_wise_summary[$buyer]["out"]+=$qnty_conversion;
				$out_buyer_wise_summary[$buyer]["out"]+=$qnty_conversion;
			}
		}
	}
	//echo $sql; 

	if($cbo_type==2) // Dyeing Production Report
	{
		/*$sql_booking_type="SELECT booking_no, booking_type, is_short,1 as with_order_status from wo_booking_mst where status_active=1 and is_deleted=0
		union all select booking_no, booking_type, is_short, 2 as with_order_status from wo_non_ord_samp_booking_mst where status_active=1 and is_deleted=0";
		$sql_booking_type_data=sql_select($sql_booking_type);
		...
		if(str_replace("'",'',$cbo_batch_type)==0 || str_replace("'",'',$cbo_batch_type)==1)
		{
			...
			foreach ($sql_prod_ref as $val) 
			{
				if($val[csf("load_unload_id")]==2)
				{
					...
					/*if($chkBatch2[$val[csf("batch_id")]] =="")
					{
						$batch_product_arr4[$val[csf("batch_id")]]['total_batch']++;
						$chkBatch2[$val[csf("batch_id")]] =$val[csf("batch_id")];
					}*/
				}
				else
				{
			...
			//print_r($sales_batch_arr);
		 	$sql_dyes = "SELECT a.id as recipe_id,a.batch_id,d.item_category_id,c.ratio from pro_recipe_entry_mst a, dyes_chem_issue_requ_dtls c,dyes_chem_requ_recipe_att b,product_details_master d 
		 	where b.recipe_id=a.id and c.mst_id=b.mst_id and d.id=c.product_id and d.item_category_id in(6) and c.item_category=6  and a.status_active=1 and c.status_active=1 and a.is_deleted=0 and b.status_active=1 $req_batch_cond_for_in"; 
			// echo $sql;die();
			...
			$add_tp_stri_batch_sql=sql_select("select  a.entry_form,a.batch_id, a.dyeing_re_process,b.prod_id,b.ratio,c.item_category_id from pro_recipe_entry_mst a,pro_recipe_entry_dtls b,product_details_master c 
			where a.id=b.mst_id and c.id=b.prod_id   and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and c.item_category_id in(5,7,6) $all_batch_no_cond2 ");
			...
			$shade_total_color_batch_qty=$shade_total_others_color_batch_qty=0;$section_wise_batch_qnty_total=0;
			...
			foreach($batchdata as $row)
			{
				...
				if($row[csf("extention_no")]>0 && $row[csf("result")]==1)
				{
					if($row[csf("process_id")]==147) //Re dyeing
					{
						if($dyingchkBatch[$row[csf("batch_id")]] =="")
						{
							...
							if ($total_redying_batch_qty>0) 
							{
								$total_redying_batch_arr[$row[csf("batch_id")]] =$row[csf("batch_id")];
							}
					...
					if($row[csf("process_id")]==148) //Re Wash...
					{
						if($dyinWgchkBatch[$row[csf("batch_id")]] =="")
						{
							...
							if ($total_rewash_batch_qty>0) 
							{
								$total_rewash_batch_arr[$row[csf("batch_id")]] =$row[csf("batch_id")];
							}
					...
					if($row[csf("process_id")]==294) //Stripping...
					{
						if($dyinStripchkBatch[$row[csf("batch_id")]] =="")
						{
							...
							if ($total_stripping_qnty>0) 
							{
								$total_stripping_batch_arr[$row[csf("batch_id")]] =$row[csf("batch_id")];
							}
				...
				if($row[csf('machine_id')] != 0) //$multi_batch_load_id==1 &&
				{
					...
					$total_running_machine_arr[$row[csf('machine_id')]]=$row[csf('machine_id')];
					//echo $row[csf('machine_id')].'d'.$machine_capacity_arr[$row[csf('machine_id')]].'<br>';
					$tot_machine_capacity_arr[$row[csf('batch_id')]]= $machine_capacity_arr[$row[csf('machine_id')]];
						
					$McfunctionalBatchArr[$row[csf("machine_id")]].=$row[csf("system_no")].',';
					//if($row[csf("machine_id")]==105) echo $row[csf("machine_id")].',';
				}
				...
				//echo $row[csf("extention_no")].'='.$row[csf("result")].'='.$color_typeId.'<br>';
				if($row[csf("result")]==1)
				{
					if($color_typeId==1 || $color_typeId==5) //Solid ,AOP
					{
						//echo $color_typeId.'D,';
						if($colorchkBatch[$row[csf("batch_id")]][$program_no][$color_typeId] =="")
						{
							...
							$colorchkBatch[$row[csf("batch_id")]][$program_no][$color_typeId]=$row[csf("batch_id")];

							/*if($total_color_batch_arr[$row[csf("batch_id")]] =="" && $batch_prod_tot!="")
							{
								// echo $row[csf("batch_id")].'==<br>';
								// $total_color_batch=$batch_product_arr4[$row[csf("batch_id")]]['total_batch'];
								$total_color_batch_arr[$row[csf("batch_id")]] =$row[csf("batch_id")];
							}*/
					...
					{
						if($colorchkBatch2[$row[csf("batch_id")]][$program_no][$color_typeId] =="")
						{
							...
							$batch_prod_tots=$batch_product_arr3[$row[csf("batch_id")]][$row[csf("prod_id")]][$color_typeId];
							$total_others_color_batch_qty += $batch_prod_tots;
							//echo $batch_prod_tots.'A,';
							$colorchkBatch2[$row[csf("batch_id")]][$program_no][$color_typeId] =$row[csf("batch_id")];

							/*if($total_others_color_batch[$row[csf("batch_id")]] =="" && $batch_prod_tots!="")
							{
								// echo $row[csf("batch_id")].'==<br>';
								// $total_color_batch=$batch_product_arr4[$row[csf("batch_id")]]['total_batch'];
								$total_others_color_batch[$row[csf("batch_id")]] =$row[csf("batch_id")];
							}*/
						}
				...
				if($row[csf("result")]==1)
				{
					if($color_typeId==1 || $color_typeId==5)
					{
						...
						if($colorchkBatchColorType[$row[csf("batch_id")]][$program_no][$color_typeId] =="")
						{
							...
							$colorchkBatchColorType[$row[csf("batch_id")]][$program_no][$color_typeId] =$row[csf("batch_id")];
						}
						
					}
					else
					{
						if($colorchkBatchColorType2[$row[csf("batch_id")]][$program_no][$color_typeId] =="")
						{
							//$batch_prod_tot=$sales_batch_product_arr[$row[csf("batch_id")]][$color_typeId];
							$batch_prod_tot=$batch_product_arr3[$row[csf("batch_id")]][$row[csf("prod_id")]][$color_typeId];
							...
							$colorchkBatchColorType2[$row[csf("batch_id")]][$program_no][$color_typeId] =$row[csf("batch_id")];
						}
					}
				}
				...
				if($row[csf("extention_no")]==0 && $row[csf("result")]==1 && $add_tp_stri_batch_arr[$row[csf("batch_id")]]==2)
				{
					...
					if($chkBatch_1[$row[csf("batch_id")]] =="")
					{
						...
						if(count($all_category_Arr)>=2 && ($chemical_arr_diff))
						{
							...
							if($total_dyes_chemical_adding_batch_arr[$row[csf("batch_id")]] =="" && $total_dyes_chemical_adding_qnty!="")
							{
								$total_dyes_chemical_adding_batch_arr[$row[csf("batch_id")]] =$row[csf("batch_id")];
							}
						}
						else if(in_array(6,$all_category_Arr)) //====Dyes =6
						{
							...
							if($total_dyes_adding_batch_arr[$row[csf("batch_id")]] =="" && $total_dyes_adding_qnty!="")
							{
								$total_dyes_adding_batch_arr[$row[csf("batch_id")]] =$row[csf("batch_id")];
							}
						}
						else if(in_array(5,$all_category_Arr) || in_array(7,$all_category_Arr)) //Dyes ,//Chemical =5,7 
						{
							...
							if($total_chemical_adding_batch_arr[$row[csf("batch_id")]] =="" && $total_chemical_adding_qnty!="")
							{
								$total_chemical_adding_batch_arr[$row[csf("batch_id")]] =$row[csf("batch_id")];
							}
						}
				...
				//$row[csf("extention_no")]>0 && 
				if($row[csf("result")]==1 && $add_tp_stri_batch_arr[$row[csf("batch_id")]]==1) //Topping
				{
					if($chkBatch_12[$row[csf("batch_id")]] =="")
					{
						...
						if($total_topping_batch_arr[$row[csf("batch_id")]] =="" && $total_topping_qnty!="")
						{
							$total_topping_batch_arr[$row[csf("batch_id")]] =$row[csf("batch_id")];
						}
					}
				}
				...
				if($chkBatch_444[$row[csf("batch_id")]] =="" && $batch_againstId!=2 && $row[csf("result")]==1)
				{
				...
				//	echo $row[csf("within_group")].'='.$row[csf("result")].'='.$booking_type_arr[$row[csf("booking_no")]].'='.$row[csf("booking_without_order")].'<br>';
				//shade Matched
				if($row[csf("result")]==1) 
				{
					if($row[csf("within_group")]==1)
					{
						...
						else if($booking_type_arr[$row[csf("booking_no")]]=="Sample" && $row[csf("result")]==1)
						{
							if($row[csf("booking_without_order")]==1)
							{
								//$shade_matched_wise_summary[$row[csf("po_buyer")]]["smn"]+=$row[csf("batch_qnty")];
								...
								if($row[csf("extention_no")]==0)
								{
									if($chkBatch_42[$row[csf("batch_id")]] == "")
									{
										...
										//echo $batch_product_arr2[$row[csf("batch_id")]]["batch_prod_tot"].'A <br>';
										$shade_matched_wise_summary[$row[csf("po_buyer")]]["smn"]+=$batch_product_arr2[$row[csf("batch_id")]]["batch_prod_tot"];
										$shade_matched_buyer_total +=$batch_product_arr2[$row[csf("batch_id")]]["batch_prod_tot"];
			...
			// echo count($total_others_color_batch).'Others Batch';
			// echo $total_others_color_batch_qty.'=';
			//print_r($tot_process_end_date_arr);
			// echo count($total_dyes_adding_batch_arr).'DD';

			$batch_row_span_arr =array();$ttl_batch_qty=0;$machine_check_array=array();
			$x=1;$total_others_color_batch_qty=$total_color_batch_qty=$shade_total_others_color_batch_qty=$shade_total_color_batch_qty=0;
			foreach ($data_array_batch as $batch_id => $batch_data_arr) 
			{
				$batch_row_span = 0;
				foreach ($batch_data_arr as $prod_id => $val) 
				{
					...
					$color_typeId=$val[("color_type")];
					$resultId=$val[("result")]; 
					$extention_no=$val[("extention_no")];
					if($resultId==1 && $extention_no==0 &&  $add_tp_stri_batch_arr[$batch_id]=="")
					{
						$ttl_batch_trim=0;
						if($TrimBtchChkkArr[$batch_id]=='')
						{
							$ttl_batch_trim=$ttl_batch_prod_tot_arr[$batch_id]['trims'];
							$TrimBtchChkkArr[$batch_id]=$batch_id;
							// $total_batch_trim++;
						}
						 
						if($color_typeId==1 || $color_typeId==5) //Solid ,AOP
						{
						  	$production_qnty=$val[("production_qnty")]+$ttl_batch_trim;
							//$batch_prod_tot=$batch_product_arr3[$row[csf("batch_id")]][$row[csf("prod_id")]][$color_typeId];
							$total_color_batch_qty += $production_qnty ;
							$shade_total_color_batch_qty+= $production_qnty;
							
							if($total_color_batch_arr[$batch_id] =="" && $production_qnty!="")
							{
								// echo $batch_id.'==<br>';
								// $total_color_batch=$batch_product_arr4[$batch_id]['total_batch'];
								$total_color_batch_arr[$batch_id]=$batch_id;
							}
						}
						else
						{
							...
							$production_qnty=$val[("production_qnty")]+$ttl_batch_trim;
							$total_others_color_batch_qty += $production_qnty;
							$shade_total_others_color_batch_qty+= $production_qnty;

							if($total_others_color_batch_arr[$batch_id] =="" && $production_qnty!="")
							{
								// echo $batch_id.'==<br>';
								// $total_color_batch=$batch_product_arr4[$row[csf("batch_id")]]['total_batch'];
								$total_others_color_batch_arr[$batch_id] =$batch_id;
							}
							//echo $batch_prod_tots.'A,';
							//$colorchkBatch2[$row[csf("batch_id")]][$program_no][$color_typeId] =$row[csf("batch_id")];
						}
					...
					if($batch_againstId==2 && $val[("result")]==1) //Color Range wise Reprocess  
					{
						$production_qnty=$val[("production_qnty")];
						$color_wise_batch_arr[$val[("color_range_id")]]["summary_batch_qnty"]+=$production_qnty;//$ttl_batch_prod_tot_arr[$batch_id]['prod_qty'];
						if($ttl_batch_prod_tot_arr[$batch_id]['trims']>0)
						{
							//$color_wise_batch_arr[$val[("color_range_id")]]["total_trims_weight"]+=$ttl_batch_prod_tot_arr[$batch_id]['trims'];
						}
					...
					else
					{
						...
						{
							$production_qnty=$val[("production_qnty")];
							$without_reprocess_color_wise_batch_arr[$val[("color_range_id")]]["summary_batch_qnty"]+=$production_qnty;//$ttl_batch_prod_tot_arr[$batch_id]['prod_qty'];
							if($ttl_batch_prod_tot_arr[$batch_id]['trims']>0)
							{
								//$without_reprocess_color_wise_batch_arr[$val[("color_range_id")]]["total_trims_weight"]+=$ttl_batch_prod_tot_arr[$batch_id]['trims'];
							}
		...
		//echo $total_color_batch_qty.'DD';
		// echo $total_color_batch.'DD';
		// echo count($total_color_batch_arr).'Color Batch,';
		// echo count($total_others_color_batch_arr).'Other batch';
		// echo $total_others_color_batch_qty.'=';
		...
		if(str_replace("'",'',$cbo_batch_type)==0 || str_replace("'",'',$cbo_batch_type)==2 )
		{
			...
			foreach($subbatchdata as $row)
			{
				...
				$subbatch_data_arr[$row[csf("batch_id")]][$row[csf("prod_id")]]["batch_against"] = $row[csf("batch_against")];
				$subbatch_data_arr[$row[csf("batch_id")]][$row[csf("prod_id")]]["add_tp_strip"] = $add_tp_stri_batch_arr[$row[csf("batch_id")]];
				//echo  $sub_batch_product_arr2[$row[csf("batch_id")]]["batch_prod_tot"].'BB';
				//echo  $row[csf("sub_batch_qnty")].'X';
				$multi_batch_load_id=$multi_batch_arr[$row[csf("batch_id")]];
				...
				$sub_ttl_batch_prod_tot_arr[$row[csf("batch_id")]]["prod_qty"]= $row[csf("sub_batch_qnty")];
				$sub_ttl_batch_prod_tot_arr[$row[csf("batch_id")]]["trims"]=$row[csf("total_trims_weight")];
				//echo $row[csf("extention_no")].'d';
				if($row[csf("result")]==1 && $row[csf("extention_no")]==0)
				{
					$total_others_color_batch_qty+=$row[csf("sub_batch_qnty")]; 
					$shade_total_others_color_batch_qty+= $row[csf("sub_batch_qnty")];

					if($total_others_color_batch_arr[$row[csf("batch_id")]] =="" && $row[csf("sub_batch_qnty")]!="")
					{
						// echo $row[csf("batch_id")].'==<br>';
						// $total_color_batch=$batch_product_arr4[$row[csf("batch_id")]]['total_batch'];
						$total_others_color_batch_arr[$row[csf("batch_id")]] =$row[csf("batch_id")];
					}
				}
				...
				*/
				...
				if($row[csf("extention_no")]==0 && $row[csf("result")]==1 && $add_tp_stri_batch_arr[$row[csf("batch_id")]]=="")
				{
					if($chkBatch_2[$row[csf("batch_id")]] =="")
					{
						//$total_rft_qnty += $batch_product_arr[$row[csf("batch_id")]]["batch_prod_tot"];
						$total_rft_qnty += $batch_product_arr2[$row[csf("batch_id")]]["batch_prod_tot"] + $row[csf("total_trims_weight")];
						// $total_rft_batch[$row[csf("batch_id")]]++;
						$chkBatch_2[$row[csf("batch_id")]] =$row[csf("batch_id")];
					}
				}
				$batch_againstId=$row[csf("batch_against")];
				if($chkBatch_4[$row[csf("batch_id")]] =="" && $batch_againstId!=2 && $row[csf("result")]==1)
				{
					$without_fabric_type_production_arr[$row[csf("fabric_type")]] += $batch_product_arr2[$row[csf("batch_id")]]["batch_prod_tot"] + $row[csf("total_trims_weight")];
					$without_fabric_type_production_total +=$batch_product_arr2[$row[csf("batch_id")]]["batch_prod_tot"] + $row[csf("total_trims_weight")];
					$chkBatch_4[$row[csf("batch_id")]] =$row[csf("batch_id")];
			...
			foreach ($subbatch_data_arr as $batch_id => $batch_data_arr) 
			{
				...
				foreach ($batch_data_arr as $prod_id=>$row) 
				{
					$sub_batch_row_span++;
					$sub_batch_qnty_arr[$batch_id]+= $row["production_qnty"]; 
					$batch_againstId=$row[("batch_against")]; 
					//echo $batch_againstId.'d';
					//$sub_ttl_batch_prod+=$row["production_qnty"]+$row["total_trims_weight"];//$sub_ttl_batch_prod_tot_arr[$batch_id]["trims"];
					$sub_buyer_batch_qnty_arr[$row["buyer_id"]]+= $row["production_qnty"]+$sub_ttl_batch_prod_tot_arr[$batch_id]["trims"];
					if($subbtchChkkArr[$batch_id]=='')
					{
						if($batch_againstId==2 && $row[("result")]==1)  //Reprocess
						{
							//$color_wise_batch_arr[$row[("color_range_id")]]["summary_batch_qnty"]+=$sub_batch_qnty_arr[$batch_id];//$sub_ttl_batch_prod_tot_arr[$batch_id]["prod_qty"];
							if($sub_ttl_batch_prod_tot_arr[$batch_id]["trims"]>0)
							{
						...
						$subbtchChkkArr[$batch_id]=$batch_id;
					}
					//echo $batch_againstId.'=='.$row[("result")].'f'; 
						if($batch_againstId==2 &&  $row[("result")]==1)  //Reprocess
						{
							$color_wise_batch_arr[$row[("color_range_id")]]["summary_batch_qnty"]+=$row["production_qnty"];//$sub_batch_qnty_arr[$batch_id];//$sub_ttl_batch_prod_tot_arr[$batch_id]["prod_qty"];
						...
						else
						{
							if($row[("result")]==1) 
							{
							$without_reprocess_color_wise_batch_arr[$row[("color_range_id")]]["summary_batch_qnty"]+=$row["production_qnty"];//$sub_batch_qnty_arr[$batch_id];//$sub_ttl_batch_prod_tot_arr[$batch_id]["prod_qty"];
							 // $tot_red+=$row["production_qnty"];
							$without_reprocess_color_wise_batch_arr[$row[("color_range_id")]]["batch_ids"].=$batch_id.',';
							}
	...
	// echo $total_others_color_batch_qty.'='.count($total_color_batch_arr).'='.count($total_others_color_batch_arr);
	...
	else if($cbo_type==2) // Dyeing Production Report
	{
		...
		<div>
			<fieldset style="width:1350px;">
				...
				<?
				if(str_replace("'",'',$cbo_batch_type)==0 || str_replace("'",'',$cbo_batch_type)==1) // Self Batch
				{
					if (count($batchdata)>0 || count($subbatchdata)>0)
					{
						...
						<!-- As Per Weight and As per number of Batch and Fabric Type wise breakdown without Reprocess Start -->
						<table style="margin:5px;" cellpadding="0"  width="300" cellspacing="0" align="left"  >
							<!-- As Per Weight -->
							<tr>
							  	<td>
									<table class="rpt_table" rules="all" border="1">
										<thead>
											<tr>
												<th colspan="2">As Per Weight</th>
											</tr>
										</thead>
										<tbody>
											<tr>	
												...
												<td align="right"  width="100"><?
												  ...
												  echo  number_format(($total_rft_qnty/($total_reprocess_qty + $total_rft_add))*100,2);
												  $total_per+= number_format(($total_rft_qnty/($total_reprocess_qty + $total_rft_add))*100,2);

												?></td>	
											</tr>
											<tr>															
												<td  width="150">Adding %</td> 
												<td align="right"  width="100"><? 
												echo  number_format(($total_adding/($total_reprocess_qty + $total_rft_add))*100,2); 
												$total_per+= number_format(($total_adding/($total_reprocess_qty + $total_rft_add))*100,2);
												?></td>	
											</tr>
											<tr>														
												<td  width="150">Topping %</td> 
												<td align="right"  width="100"><? 														
												  echo number_format(($total_topping_qnty	/($total_reprocess_qty + $total_rft_add))*100,2);
												   $total_per+= number_format(($total_topping_qnty	/($total_reprocess_qty + $total_rft_add))*100,2);
											
												?></td>	
											</tr>	
											<tr>														
												<td  width="150">Reprocess %</td> 
												<td align="right"  width="100"><? echo  number_format(($total_reprocess_qty	/($total_reprocess_qty + $total_rft_add))*100,2);
												$total_per+= number_format(($total_reprocess_qty/($total_reprocess_qty + $total_rft_add))*100,2);
												?></td>	
											</tr>														
										</tbody>
										<tfoot>
											<tr>
												<th>Total </th>
												<th><? echo number_format($total_per,0);?></th>
											</tr>
										</tfoot>
									</table>
							  	</td>
						   	...
						   	<!-- As Per Weight -->
						   	<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;</td></tr>
						   	<!-- As Per Number Of Batch -->
						   	<tr>
							  	<td>
									<table class="rpt_table" rules="all" border="1">
										<thead>
											<tr>
												<th colspan="2">As Per Number Of Batch</th>
											</tr>
										</thead>
										<tbody>
											<tr>	
												<?php
												$total_rft_batch=count($total_color_batch_arr)+count($total_others_color_batch_arr);
												$total_adding_batch=count($total_dyes_adding_batch_arr)+count($total_chemical_adding_batch_arr)+count($total_dyes_chemical_adding_batch_arr);
												$total_reprocess_batch=count($total_redying_batch_arr)+count($total_rewash_batch_arr)+count($total_stripping_batch_arr);
												$total_batch_count=$total_rft_batch+$total_adding_batch+count($total_topping_batch_arr)+$total_reprocess_batch
												?>
												<td width="150">RFT %</td> 
												<td align="right" width="100"><? echo number_format(($total_rft_batch/$total_batch_count)*100,2);
												  $total_percent+= ($total_rft_batch/$total_batch_count)*100;
												?></td>	
											</tr>
											<tr>															
												<td width="150">Adding %</td> 
												<td align="right" width="100"><? echo number_format(($total_adding_batch/$total_batch_count)*100,2);
												$total_percent+= ($total_adding_batch/$total_batch_count)*100;
												?></td>	
											</tr>
											<tr>														
												<td width="150">Topping %</td> 
												<td align="right"  width="100"><? echo number_format((count($total_topping_batch_arr)/$total_batch_count)*100,2);
												   $total_percent+= (count($total_topping_batch_arr)/$total_batch_count)*100;
												?></td>	
											</tr>	
											<tr>														
												<td width="150">Reprocess %</td> 
												<td align="right" width="100"><? echo number_format(($total_reprocess_batch/$total_batch_count)*100,2);
												$total_percent+= ($total_reprocess_batch/$total_batch_count)*100;
												?></td>	
											</tr>														
										</tbody>
										<tfoot>
											<tr>
												<th>Total </th>
												<th><? echo number_format($total_percent,0);?></th>
											</tr>
										</tfoot>
									</table>
							  	</td>
						   	...
						   	<!-- As Per Number Of Batch -->
						   	<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;</td></tr>
						   	<!-- Fabric Type Wise Breakdown Without Reprocess Start -->
						   	<tr>
							 	<td>
								<table class="rpt_table" rules="all" border="1">
									<thead>
						   	...
						   	<!-- Fabric Type Wise Breakdown Without Reprocess End -->
						</table>
						<!-- As Per Weight and As per number of Batch and Fabric Type wise breakdown without Reprocess End -->

						<!-- Re-Process Summary (Buyer Wise) Start -->
						...
						<!-- Re-Process Summary (Buyer Wise) End -->

						<!-- Outbound subcontract Start -->
						<table cellpadding="0" style="margin:5px;" width="200" cellspacing="0" align="left"  class="rpt_table" rules="all" border="1">
							<thead>
								<tr>
									<th colspan="2">Outbound Subcontract</th>
								</tr>
								<tr>
									<th width="100">Buyer</th>
									<th width="">Quantity (Kgs)</th>
								</tr>
							</thead>
							<tbody>
								<? 
								$sl=1;
								foreach($out_buyer_wise_summary as $buyer=> $row)
								{
									if ($sl%2==0)  $bgcolor_dyeing="#E9F3FF"; else $bgcolor_dyeing="#FFFFFF";
									?>
									<tr bgcolor="<? echo $bgcolor_dyeing;?>">
										<td align="center"><? echo $buyer_arr[$buyer];?></td>
										<td align="right"><? echo number_format($row['out'],2);?></td>
									</tr>
									<?
									$sl++;
									$outbound_buyer_wise_batch_total+=$row['out'];
								}
								?>
							</tbody>
							<tfoot>
								<tr>
									<th>Total </th>
									<th><? echo number_format($outbound_buyer_wise_batch_total,2);?></th>
								</tr>
							</tfoot>
						</table>
						<!-- Outbound subcontract End -->						

						<!-- Color Range wise Summary With Reprocess Start-->
						<table cellpadding="0" style="margin:5px;" width="300" cellspacing="0" align="left"  class="rpt_table" rules="all" border="1">
							<thead>
								<tr>
						...
						<!-- Color Range wise Summary With Reprocess End-->
                       
                       	<!-- Color Range wise Summary Without Reprocess Start -->
                        <table cellpadding="0" style="margin:5px;" width="300" cellspacing="0" align="left"  class="rpt_table" rules="all" border="1">
							<thead>
								<tr>
							...
							<tbody>
								<? 
								$sl=1;$tot_batch_qty=$tot_no_of_batch=$tot_total_trims_weight=$total_no_of_batch=0;
								//print_r($color_wise_batch_arr);ttl_batch_prod_tot
								foreach($without_reprocess_color_wise_batch_arr as $color_rang_id=> $val)
								{
									$out_total_color_qty+=$val['summary_batch_qnty']+$val['total_trims_weight'];
								}
								foreach($without_reprocess_color_wise_batch_arr as $color_rang_id=> $summ)
								{
									if ($sl%2==0)  $bgcolor_dyeing="#E9F3FF"; else $bgcolor_dyeing="#FFFFFF";
									$out_batch_ids=rtrim($summ[batch_ids],',');
									$out_batch_ids_arr=array_unique(explode(",",$out_batch_ids));
									$tot_total_trims_weight+=$summ['total_trims_weight'];
									//echo count($batch_ids_arr).',';
									//echo $val['total_trims_weight'].'D';
									$tot_prod_qty_summ=$summ['summary_batch_qnty']+$summ['total_trims_weight'];
									...
									<tr bgcolor="<? echo $bgcolor_dyeing;?>">
										...
										<td align="center"><? echo $color_range[$color_rang_id];?></td>
										<td align="right"><? echo number_format($tot_prod_qty_summ,2);?></td>
										<td align="right" title="Tot batch Qty=<? echo $out_total_color_qty;?>">
										<? echo number_format(($tot_prod_qty_summ/($out_total_color_qty))*100,2);?></td>
										<td align="right"><? echo count($out_batch_ids_arr);?></td>
									</tr>
									<?
									$sl++;
									$tot_batch_qty+=$tot_prod_qty_summ;
									$total_no_of_batch+=count($out_batch_ids_arr);
								}
								?>
							</tbody>
							<tfoot>
								<tr>
									<th colspan="2">Total </th>
									<th><? echo number_format($tot_batch_qty,2);?></th>
									<th><? echo number_format(($tot_batch_qty/($out_total_color_qty))*100,2);?></th>
									<th><? echo number_format($total_no_of_batch,0);?></th>
								</tr>
							</tfoot>
						</table>
						<!-- Color Range wise Summary Without Reprocess End -->
						...
						<table cellpadding="0"  width="700" cellspacing="0" align="left"  class="rpt_table" rules="all" border="1">
							<thead>
								<tr>
									<th colspan="9">Summary Total (Shade Match)</th>
								</tr>
								<tr>
									<th width="100">Buyer</th>
									<th width="100">Self batch</th> 
									...
									<th width="60">Trims Weight</th>
									<th width="60">Inbound Subcontract</th>
									<th width="60">Outbound Subcontract</th>
									<th width="100">Buyer Total</th>
									<th>%</th>
								</tr>
							</thead>
							<tbody>
								...
								foreach($shade_matched_wise_summary as $buyer=> $val)
								{
									...
									?>
									<tr bgcolor="<? echo $bgcolor_dyeing;?>">
										<td align="center" title="<?echo $buyer;?>"><? echo $buyer_arr[$buyer];?></td>
										<td align="right"><? echo number_format($val["self"],2); ?></td>
										<td align="right"><? echo number_format($val["sm"],2); ?></td>
										<td align="right"><? echo number_format($val["smn"],2); ?></td>
										<td align="right"><? echo number_format($val["trim"],2); ?></td>
										<td align="right"><? echo number_format($tot_buyer_match_shade_subcon,2); ?></td>
										<td align="right"><? echo number_format($val["out"],2); ?></td>
										<td align="right" title="Subcon=<? echo $tot_buyer_match_shade_subcon;?>"><? echo number_format($tot_buyer_match_shade,2);?></td>
										<td align="right"><? echo number_format(($tot_buyer_match_shade/$shade_matched_buyer_total)*100,2);?></td>
									</tr>
									<?
									$sl++;
								...
								<tr style="background-color: #eee;font-weight: bold;">
									...
									<td width="60" align="right"><? echo number_format($total_smn,2);?></td>
									<td width="60" align="right"><? echo number_format($total_trim,2);?></td>
									<td width="60" align="right"><? echo number_format($grand_total_subcon,2);?></td>
									<td width="60" align="right"><? echo number_format($total_outbound,2);?></td>
									<td width="100" align="right"><? echo number_format($total_tot_buyer_match_shade,2);?></td>
									<td align="right"><? echo number_format($total_summury_shade_matched,2);?></td>
								</tr>
							</tbody>
							<thead>
								<tr>
									<th colspan="9">Summary Total (Shade Not Match)</th>
								</tr>
							</thead>
							<tbody>
								<?
							...
							<tfoot>
								...
								<tr bgcolor="#EBEBEB">
									...
									<th align="right"><? echo number_format($grand_total_trim,2);?></th>
									<th align="right"><? echo number_format($grand_total_subcon,2);?></th>
									<th align="right"><? echo number_format($grand_total_out,2);?></th>
									<th align="right"><? echo number_format($grand_total_buyer,2);?></th>
									<th align="right"><? ?></th>
								</tr>
							</tfoot>
						...
						<!-- Inhouse, Inbound and Outbound subcontract Total Production Without Reprocess Start -->
						<table cellpadding="0" style="margin:5px;" width="200" cellspacing="0" align="left"  class="rpt_table" rules="all" border="1">
							<thead>
								<tr>
									<th colspan="2">Inhouse, Inbound and Outbound subcontract Total Production Without Reprocess</th>
								</tr>
							</thead>
							<tbody>
								<tr>
									<td width="150">Inhouse</td> 
									<td align="right"  width="100"><? echo number_format($total_rft_add,2,'.','');
									  $total_in_out_subc_qty+= number_format($total_rft_add,2,'.','');
									?></td>	
								</tr>
								<tr>															
									<td  width="150">Inbound Subcontract</td> 
									<td align="right" width="100"><? echo number_format($grand_total_subcon,2,'.',''); 
									$total_in_out_subc_qty+= number_format($grand_total_subcon,2,'.','');
									?></td>	
								</tr>
								<tr>														
									<td  width="150">Outbound Subcontract</td> 
									<td align="right"  width="100"><? echo number_format($outbound_buyer_wise_batch_total,2,'.','');
									   $total_in_out_subc_qty+= number_format($outbound_buyer_wise_batch_total,2,'.','');
									?></td>	
								</tr>														
							</tbody>
							<tfoot>
								<tr>
									<th>Total </th>
									<th><? echo number_format($total_in_out_subc_qty,2,'.','');?></th>
								</tr>
							</tfoot>
						</table>
						<!-- Inhouse, Inbound and Outbound subcontract Total Production Without Reprocess End -->

						<!-- Inhouse, Inbound and Outbound Subcontract Total Production With Reprocess Start -->
						<table cellpadding="0" style="margin:5px;" width="200" cellspacing="0" align="left"  class="rpt_table" rules="all" border="1">
							<thead>
								<tr>
									<th colspan="2">Inhouse, Inbound and Outbound Subcontract Total Production With Reprocess</th>
								</tr>
							</thead>
							<tbody>
								<tr>
									<td width="150">Inhouse</td> 
									<td align="right"  width="100"><? echo  number_format($total_reprocess_qty + $total_rft_add,2,'.','');
									  $total_in_out_subc_qty_with+= number_format($total_reprocess_qty + $total_rft_add,2,'.','');
									?></td>	
								</tr>
								<tr>															
									<td  width="150">Inbound Subcontract</td> 
									<td align="right" width="100"><? 
									echo number_format($grand_total_subcon,2,'.',''); 
									$total_in_out_subc_qty_with+= number_format($grand_total_subcon,2,'.','');
									?></td>	
								</tr>
								<tr>														
									<td  width="150">Outbound Subcontract</td> 
									<td align="right"  width="100"><?
									  echo number_format($outbound_buyer_wise_batch_total,2,'.','');
									   $total_in_out_subc_qty_with+= number_format($outbound_buyer_wise_batch_total,2,'.','');
									?></td>	
								</tr>														
							</tbody>
							<tfoot>
								<tr>
									<th>Total </th>
									<th><? echo number_format($total_in_out_subc_qty_with,2,'.','');?></th>
								</tr>
							</tfoot>
						</table>
						<!-- Inhouse, Inbound and Outbound Subcontract Total Production With Reprocess End -->
						...
						<!-- Shade wise Dyeing Category without Reprocess Start -->
                        <table cellpadding="0" style="margin:5px;"  width="300" cellspacing="0" align="left"  class="rpt_table" rules="all" border="1">
							<thead>
								<tr>
						...
						<!-- Shade wise Dyeing Category without Reprocess End -->

						<!-- Section wise Reprocess -->
                        <table cellpadding="0" style="margin:5px;"  width="300" cellspacing="0" align="left"  class="rpt_table" rules="all" border="1">
							...
							<tbody>
								<? 
								$sl=1;$tot_sectionQty=0;$total_sectionQty=0;
								foreach($section_wise_batch_qnty_arr as $sectionId=> $val)
								{
									$total_sectionQty+=$val;
								}
								foreach($section_wise_batch_qnty_arr as $sectionId=> $val)
								{
									if ($sl%2==0)  $bgcolor_dyeing="#E9F3FF"; else $bgcolor_dyeing="#FFFFFF";
									...
									<tr bgcolor="<? echo $bgcolor_dyeing;?>">
										...
										<td align="right"><? echo number_format($val,2);?></td>
										<td align="right"><? echo number_format(($val/$total_sectionQty)*100,2);?></td>
									</tr>
									<?
									$sl++;
									$tot_sectionQty+=$val;
							...
							<tfoot>
								<tr>
									<th>Total </th>
									<th><? echo number_format($tot_sectionQty,2);?></th>
									<th>100.00</th>
								</tr>
							</tfoot>
						</table>
						...
						<table cellpadding="0" style="margin:5px;"  width="160" cellspacing="0" align="left"  class="rpt_table" rules="all" border="1">
							<tbody bgcolor="#FFFFFF">
								...
								foreach($McfunctionalBatchArr as $m_id=>$tot_no)
								{
									$McfunctionalBatch=rtrim($tot_no,',');
									$tot_nos=array_unique(explode(",",$McfunctionalBatch));
									$total_machine_capacity+= $machine_capacity_arr[$m_id]*count($tot_nos);
									//echo $m_id.'='.$machine_capacity_arr[$m_id].'='.$tot_nos.'<br>';
								}
								//echo $total_machine_capacity;
								$tot_machine=count($total_machine_arr);
								//echo $tot_machine.'XXd';
								$running_machine=count($total_running_machine_arr);
								$stop_machine=$tot_machine-$running_machine;
								...
								$stop_machine_percent=(($stop_machine/$tot_machine)*100);
									$total_batch_weight=($ttl_batch_qty+$sub_ttl_batch_prod)*100;
									//$total_machine_capacity=($tot_machine_capacity+$sub_tot_machine_capacity)*1;
									//$NoOfBatch_no=count($NoOfBatchArr);
									//$mc_cal=$total_machine_capacity*$NoOfBatch_no;
								 //	echo $total_batch_weight.'vd'; 
									$avg_load_percent=($total_batch_weight/$total_machine_capacity)*$all_days_total_date;
							
								?>
								<tr>
									<td width="90" title="Total batch Weight(<? echo number_format($ttl_batch_qty,2).' + '.number_format($sub_ttl_batch_prod,2);?>)*100/(Total Running Machine Capacity*No of Batch(=<? echo $total_machine_capacity;?>)*Working Day(<? echo $all_days_total_date;?>))">Avg. Load% :</td>
									<td align="right"><? 
									echo number_format($avg_load_percent,2);
				...
				if(str_replace("'",'',$cbo_batch_type)==0 || str_replace("'",'',$cbo_batch_type)==2)//Inbound subc
				{
					if (count($subbatchdata)>0)
					{
						$group_by=str_replace("'",'',$cbo_group_by);
						if(str_replace("'",'',$cbo_batch_type)==2)
						{
							...
							<!-- Color Range wise Summary Start -->
							<table cellpadding="0"  width="300" cellspacing="0" align="left"  class="rpt_table" rules="all" border="1">
								<thead>
									<tr>
										<th colspan="5">Color Range wise Summary</th>
						...
						<!-- Inbound subcontract End -->
						<br/>
						<? 
					}
				}

				if(str_replace("'",'',$cbo_batch_type)==0) // Outbound Subcontract
				{
					?>
					<!-- Outbound subcontract Start -->
					<div align="left">
						<table class="rpt_table" width="1120" cellpadding="0" cellspacing="0" border="1" rules="all" align="center" id="table_header_1">
							<caption><b>Outbound Subcontract</b></caption>
							<thead>
								<tr>
									<th width="30">SL</th>
									...
									<th width="90">FSO No</th>
									<th width="70">Fabric Booking No</th>
									<th width="100">Booking Type</th>
									<th width="70">Fabrics Desc</th>
						...
						<div style=" max-height:350px; width:1140px; overflow-y:scroll;;" id="scroll_body">
							<table class="rpt_table" id="table_body" width="1120" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
								<tbody>
									<? 
									$total_qty=0;								
									$i=1;
									foreach($outbound_data_arr as $batch_id=>$batch_data_arr)
									{
										foreach ($batch_data_arr as $prod_id => $row) 
										{
											// $batch_td_span = $sub_batch_row_span_arr[$batch_id];
											...
											?>
											<tr bgcolor="<? echo $bgcolor_dyeing; ?>"  id="tr2_<? echo $i; ?>" onClick="change_color('tr2_<? echo $i; ?>','<? echo $bgcolor_dyeing; ?>')">
												<td width="30"><? echo $i; ?></td>
												<td width="80"><p>&nbsp;<? echo change_date_format($row['receive_date']);?></p></td>
												<td width="100" align="center"><p><? echo $supplier_name_arr[$row['dyeing_company']]; ?></p></td>
												...
												<td width="100" align="center"><p><? echo $row['style_ref_no']; ?></p></td>
												<td width="80" align="center"><p><? echo $row['season']; ?></p></td>
												<td width="90"><p><? echo $row['fso_no']; ?></p></td>
												<td width="70" align="center"><p><? echo $row['booking_no']; ?></p></td>
												<td width="100" style="word-wrap:break-word;"><? echo $booking_type_arr[$row['booking_no']]; ?></td>
												<td width="70"><p><? echo $composition_arr[$row['fabric_description_id']]; ?></p></td>
												<td width="50"><p><? echo $fabric_typee[$row['dia_width_type']]; ?></p></td>
												<td width="80"><p><? echo $color_library[$row['color_id']]; ?></p></td>
												<td width="80"><p><? echo $row['batch_no']; ?></p></td>
												<td width="" align="right"><p><? echo number_format($row['quantity'],2,'.',''); ?></p></td>
											</tr>
											<? 
											$i++;
								...
								<tfoot>
									<tr class="tbl_bottom">
										...
										<td width=""><p><? echo number_format($total_qty,2,'.',''); ?></p></td>
									</tr> 
								</tfoot>
							</table>
						</div>
...
yarn_comp_type1st,c.yarn_comp_percent1st, c.yarn_count_id,c.yarn_type,c.color,c.lot from ppl_yarn_requisition_entry a, pro_roll_details b, product_details_master c where b.entry_form=2 and b.receive_basis=2 and cast(a.knit_id as varchar2(4000)) = b.booking_no and a.prod_id = c.id and c.item_category_id=1 and a.status_active=1 and b.status_active=1 and c.status_active=1 $barcode_cond");
			...
			foreach ($data_array_batch as $batch_id => $batch_data_arr) 
			{
				...
				foreach ($batch_data_arr as $prod_id => $val) 
				{
				 ...
				 $multi_batch_load_id=$multi_batch_arr[$batch_id];
				 $batch_againstId=$val[csf("batch_againstId")];
				//echo $val["total_trims_weight"].'FD';   
				if($batch_againstId==2  && $val[("result")]==1) //Color Range wise Reprocess
				{
					$color_wise_batch_arr[$val[("color_range_id")]]["summary_batch_qnty"]+=$ttl_batch_prod_tot_arr[$batch_id]['prod_qty'];
