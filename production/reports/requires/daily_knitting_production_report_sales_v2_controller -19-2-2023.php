...
if ($action == "report_generate") 
{
	...
	if ($report_type == 1)
	{
		...
		<div align="left" style="background-color:#E1E1E1; color:#000; font-size:14px; font-family:Georgia, 'Times New Roman', Times, serif; width: 845px;"><strong><u><i>In-House + Outbound + Inbound [Knitting Production]</i></u></strong></div>
		<table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table">
			<thead>
				<tr>
					<th colspan="11">Job Summary (In-House + Outbound + Inbound)</th>
				</tr>
				<tr>
			...
			<tfoot>
				...
				<tr>
					<th colspan="5"><strong>In %</strong></th>
			...
			<table border="1" class="rpt_table" rules="all" width="370" cellpadding="0" cellspacing="0" align="center">
				...
				<tbody>
					...
					if ($mstData[0][csf('receive_basis')] == 2)
					{
						...
						if ($is_salesOrder == 1)
						{
							$query = "select a.id, a.roll_no, a.barcode_no, a.po_breakdown_id, a.qnty, b.job_no as po_number, a.dtls_id from pro_roll_details a, fabric_sales_order_mst b where a.po_breakdown_id=b.id and a.dtls_id=$dtls_id and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.barcode_no";
						}
						elseif($is_salesOrder == 2)
						{
							$query = "select a.id, a.roll_no, a.barcode_no, a.po_breakdown_id, a.qnty, b.booking_no as po_number, a.dtls_id from pro_roll_details a, wo_non_ord_samp_booking_mst b where a.po_breakdown_id=b.id and a.dtls_id=$dtls_id and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,3) and b.is_deleted=0 order by a.barcode_no";
					...
					else
					{
						...
						{
							$query = "select id,roll_no,barcode_no,po_breakdown_id,qnty,booking_no as po_number, dtls_id from pro_roll_details where dtls_id=$dtls_id and entry_form=2 and status_active=1 and is_deleted=0 order by barcode_no";
						}
						else
						{
							$query = "select a.id, a.roll_no, a.barcode_no, a.po_breakdown_id, a.qnty, b.po_number, a.dtls_id from pro_roll_details a, wo_po_break_down b where a.po_breakdown_id=b.id and a.dtls_id=$dtls_id and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,3) and b.is_deleted=0 order by a.barcode_no";
					...
					$ib_query = "select a.id, a.roll_no, a.barcode_no, a.po_breakdown_id, a.qnty, a.dtls_id from subcon_pro_roll_details a, subcon_ord_dtls b where a.po_breakdown_id=b.id and a.dtls_id=$dtls_id and a.entry_form=159 and a.status_active=1 and a.is_deleted=0 order by a.id";
					//echo $ib_query;
