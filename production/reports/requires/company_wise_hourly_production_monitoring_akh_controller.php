   ...
   if ($type == 0) 
	{ 
		...
		<table width="1290" cellpadding="0" cellspacing="0"> 
			...
			<table  id="talbe_1_gt" width="1290" cellpadding="0"  cellspacing="0" border="1" rules="all">
				<tr>              
					<th width="35"></th>
					<th width="60"></th>
					<th width="100"></th>
			...
			<table id="table_header_1" class="rpt_table" width="1290" cellpadding="0" cellspacing="0" border="1" rules="all">
				<thead>
					<tr height="50">
			...
			<div style="width:1310; max-height:400px; overflow-y:scroll" id="scroll_body">
				<table class="rpt_table" width="1290" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body">
					...
					foreach($prod_resource_array as $company_id=>$com_name)
					{
						...
						?>
						<tr bgcolor="#E8FFFF">
							<td width="" colspan="5"><strong>Company Name:<? echo $companyArr[$company_id]?></strong></td>
							<?
							for($k=$hour; $k<=$last_hour; $k++)
							{
								$prod_hour="prod_hour".substr($start_hour_arr[$k],0,2)."";
						...
						foreach($com_name as $lo_id=>$lo_name)
						{
							...
							foreach($lo_name as $f_id=>$fname)
							{
								...
								?>
								<tr bgcolor="#E8FFFF">
									<td width="" colspan="5"><strong>Floor Name:<? echo $floorArr[$f_id];?></strong></td>
									<?
									for($k=$hour; $k<=$last_hour; $k++)
									{
										$prod_hour="prod_hour".substr($start_hour_arr[$k],0,2)."";
								...
								foreach($fname as $resource_id=>$resource_data)
								{
									...
									if($cbo_no_prod_type==2 && $line_production_hour>0)
									{
										...
										<td width="70" onclick="openmypage_image('requires/company_wise_hourly_production_monitoring_akh_controller.php?action=show_image&job_no=<? echo $job_no;?>','Image View')">
									...
									{//echo "system";die;
										...
										<td width="100" align="center" class="wrd_brk"><p><? echo $production_data_arr[$f_id][$resource_id]['po_number'];?>&nbsp;</p></td>
										<td width="70" onclick="openmypage_image('requires/company_wise_hourly_production_monitoring_akh_controller.php?action=show_image&job_no=<? echo $job_no2;?>','Image View')">
										...
										<td  width="50"> 
											<?
												$sql_remark="select remarks,production_hour from pro_garments_production_mst where serving_company in(".$cbo_company_id.") and  floor_id=$f_id and sewing_line in($resource_id) and po_break_down_id in($order_no_total) and production_date=$txt_date and status_active=1 and is_deleted=0 group by remarks,production_hour order by production_hour";
												// echo $sql_remark;
												$sql_line_remark=sql_select($sql_remark);

												foreach($sql_line_remark as $data){
													if(trim($data[csf('remarks')])!=""){
														echo $data[csf('remarks')];
													}
												}
											?>
