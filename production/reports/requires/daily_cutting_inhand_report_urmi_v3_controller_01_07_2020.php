...
{ 
	...
	if($type==1)
	{
		...
		$sql_lay=" SELECT a.working_company_id, b.gmt_item_id, c.order_id, c.country_id, b.color_id,sum(case when a.entry_date=$txt_production_date then c.size_qty else 0 end )  as today_lay,  sum(c.size_qty )  as total_lay
		...
		group by a.working_company_id, b.gmt_item_id, c.order_id, c.country_id, b.color_id ";  
		...
		sum(CASE WHEN   a.production_date=$txt_production_date and  b.production_type =5 and a.production_type =5 THEN b.production_qnty  ELSE 0 END) AS today_output_qnty,
		sum(CASE WHEN      b.production_type =5 and a.production_type =5 THEN b.production_qnty  ELSE 0 END) AS total_output_qnty, 

		sum(CASE WHEN   a.production_date=$txt_production_date and  b.production_type =11 and a.production_type =11 THEN b.production_qnty  ELSE 0 END) AS today_poly_qnty,
		sum(CASE WHEN      b.production_type =11 and a.production_type =11 THEN b.production_qnty  ELSE 0 END) AS total_poly_qnty,
		sum(CASE WHEN   a.production_date=$txt_production_date and  b.production_type =11 and a.production_type =11 THEN b.reject_qty  ELSE 0 END) AS today_poly_rej_qnty,
		sum(CASE WHEN      b.production_type =11 and a.production_type =11 THEN b.reject_qty  ELSE 0 END) AS total_poly_rej_qnty,

		sum(CASE WHEN   a.production_date=$txt_production_date and  b.production_type =8 and a.production_type =8 THEN b.production_qnty  ELSE 0 END) AS today_pac_qnty,
		sum(CASE WHEN      b.production_type =8 and a.production_type =8 THEN b.production_qnty  ELSE 0 END) AS total_pac_qnty,

		sum(CASE WHEN a.production_date=$txt_production_date and b.is_rescan=0 and b.production_type =5 and a.production_type =5 THEN b.reject_qty ELSE 0 END)- sum(CASE WHEN a.production_date=$txt_production_date and b.is_rescan=1 and b.production_type =5 and a.production_type =5 THEN b.production_qnty ELSE 0 END) AS today_sewing_reject_qty,
		sum(CASE WHEN  b.production_type =5 and b.is_rescan=0 and a.production_type =5 THEN b.reject_qty ELSE 0 END)-sum(CASE WHEN  b.production_type =5 and b.is_rescan=1 and a.production_type =5 THEN b.production_qnty ELSE 0 END) AS total_sewing_reject

		from  pro_garments_production_dtls b,pro_garments_production_mst a,wo_po_color_size_breakdown c ,wo_po_break_down d 
		where a.id=b.mst_id and b.color_size_break_down_id=c.id and a.po_break_down_id=c.po_break_down_id and a.po_break_down_id=d.id and c.po_break_down_id=d.id and d.is_deleted=0 and d.shiping_status<>3 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and c.status_active in(1,2,3) and c.is_deleted=0   and a.serving_company in($cbo_work_company_name) $gmts_loc_floor_cond   $lay_order_cond_prod
		...
		foreach($production_sql_result as $row)
		{
		 ...
		 $ex_factory_sql="SELECT m.delivery_date, a.po_break_down_id as order_id, a.item_number_id, a.country_id, c.color_number_id as color_id, sum( case when a.ex_factory_date=$txt_production_date then  b.production_qnty else 0 end ) as today_ex_fac,  sum(   b.production_qnty  ) as total_ex_fac
		from pro_ex_factory_delivery_mst m, pro_ex_factory_mst a, pro_ex_factory_dtls b, wo_po_color_size_breakdown c,wo_po_break_down d
		where m.id=a.delivery_mst_id and a.id=b.mst_id and b.color_size_break_down_id=c.id and a.po_break_down_id=c.po_break_down_id and a.po_break_down_id=d.id and c.po_break_down_id=d.id and d.is_deleted=0  and a.status_active=1 and a.is_deleted=0  and c.status_active=1 and c.is_deleted=0 and m.status_active=1 and m.is_deleted=0 and m.entry_form!=85 and m.delivery_company_id in($cbo_work_company_name) $gmts_loc_floor_cond_exfac $lay_order_cond_prod 
		...
		if(count($row_generate_cond_array)>0)
		{
			 $sql_color_size="SELECT a.job_no, a.style_ref_no, a.buyer_name,a.client_id,b.shiping_status,  a.job_no_prefix_num,   b.po_number, c.po_break_down_id, c.item_number_id, c.country_id, c.color_number_id, c.country_ship_date, c.order_quantity 
				...
				where a.job_no=b.job_no_mst and b.id=c.po_break_down_id and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,2,3)  and b.is_deleted=0 and c.status_active in(1,2,3) and b.shiping_status<>3 and c.is_deleted=0 $po_id_conds $buyer_cond   $others_cond ";
		...
		<fieldset style="width:2920px;">
			<div style="width:2920px;">
				<table width="2920"  cellspacing="0"   >
				...
				<fieldset style="width:2920px; float:left;">
					<legend>Report Details Part</legend>
					<table cellspacing="0" cellpadding="0"  border="1" rules="all"  width="2900" class="rpt_table" align="left">
						<thead>
							<tr >
								...
								<th width="160" colspan="2">Lay Qty</th>
								<th width="160" colspan="2">Cutting Qty</th>
					...
					<div style="max-height:425px; overflow-y:scroll; width:2920px;" id="scroll_body">
						<table  border="1" class="rpt_table"  width="2900" rules="all" id="table_body" >
							<tbody>
								<?
 								$i=1;
												...
												foreach($item_data as $country_id=>$country_data)
												{
													foreach($country_data as $color_id=>$value)
													{
															...
															<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_2nd<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_2nd<? echo $i; ?>">
																...
																<td width="100"><p><? echo $buyer_short_library[$value["client"]]; ?>&nbsp;</p></td>
																...
																<td width="80" align="right"><? $poly_wip=($tot_sewing_out_qnty-$tot_poly_qnty)-$tot_poly_rej_qnty; echo number_format($poly_wip,0); $job_poly_wip+=$poly_wip; $buyer_poly_wip+=$poly_wip; $gt_poly_wip+=$poly_wip; $po_poly_wip+=$poly_wip;?></td>
					 ...
					 <table border="1" class="rpt_table"  width="2900" rules="all" style="margin-left: 2px;" align="left" id="">
						<tfoot>
							<tr>
