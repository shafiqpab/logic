...
if($action=="report_generate")
{
	...
	// $txt_challan_no_cond= ($txt_challan_no=="")? "": "and a.challan_no like '%".$txt_challan_no."%'";
	$challan_cond = ($txt_challan_no != "") ? " and f.sys_number_prefix_num=$txt_challan_no" : "";
	$txt_job_no_cond= ($txt_job_no=="")? "": "and d.job_no_prefix_num='".$txt_job_no."'";
	...
	if($db_type==0)
	{
		if($txt_date_from!="" && $txt_date_to!="") $date_cond="and a.production_date between '".change_date_format($txt_date_from,'yyyy-mm-dd')."' and '".change_date_format($txt_date_to,'yyyy-mm-dd')."' ";
	}
	else
	{
		if($txt_date_from!="" && $txt_date_to!="") $date_cond="and a.production_date between '".change_date_format($txt_date_from, "", "",1)."' and '".change_date_format($txt_date_to, "", "",1)."' ";
	}

	$dataSql="SELECT a.id, a.company_id, a.delivery_mst_id, a.production_type, a.production_source, a.serving_company, a.po_break_down_id, f.sys_number, a.embel_name, a.embel_type, sum(b.production_qnty) as production_qnty, d.buyer_name, d.job_no, d.style_ref_no, e.po_number,e.grouping,a.insert_date from pro_garments_production_mst a, pro_garments_production_dtls b, wo_po_color_size_breakdown c, wo_po_details_master d, wo_po_break_down e,pro_gmts_delivery_mst f where a.company_id='$company_id' $date_cond $txt_order_no_cond $txt_job_no_cond $txt_buyer_cond $internal_ref_cond $file_no_cond $challan_cond  and a.id=b.mst_id and a.production_type='4' and b.production_type='4' and b.color_size_break_down_id=c.id and e.id=a.po_break_down_id and e.id=c.po_break_down_id and a.delivery_mst_id=f.id AND d.id=e.job_id
	AND d.id=c.job_id and b.status_active=1 and b.is_deleted=0 and c.status_active IN(1,2,3) and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0 AND d.status_active=1 AND d.is_deleted=0 AND e.status_active=1
    AND e.is_deleted=0 group by a.id, a.company_id, a.delivery_mst_id, a.production_type, a.production_source, a.serving_company, a.po_break_down_id, a.embel_name, a.embel_type, d.buyer_name, d.job_no, d.style_ref_no, e.po_number,e.grouping,f.sys_number,a.insert_date order by a.id desc";
	//    echo $dataSql;die;
        ...
        <table width="100%" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all">
            <thead>
               <tr>
	                <th width="30"></th>
	                <th width="50">SL</th>
	                <th width="80">System Issue ID</th>
	                <th width="100">Input Challan No</th>
        	...
        	<tr bgcolor="<? echo $bgcolor; ?>">
        		<td>
        			...
        			<input type="hidden" id="companyId_<? echo $j; ?>" value="<? echo $selectResult[csf('company_id')]; ?>" />
        			<input type="hidden" id="deliveryBasisId_<? echo $j; ?>" value="<? echo $delivery_basis; ?>" />
        			<input type="hidden" id="deliveryMstId_<? echo $j; ?>" value="<? echo $selectResult[csf('delivery_mst_id')]; ?>" />
	            ...
	            <td><? echo $j; ?></td>
	            <td><p><? echo $selectResult[csf('id')]; ?></p></td>
	            <td><p><? echo $selectResult[csf('sys_number')]; ?></p></td>
	            <td><p><? echo $buyer_arr[$selectResult[csf('buyer_name')]]; ?></p></td>
	            ...
	            <td><p><? echo $selectResult[csf('po_number')]; ?></p></td>
	            <td><p><? echo $selectResult[csf('grouping')]; ?></p></td>
	            ...
	            <td ><p><? echo $selectResult[csf('insert_date')]; ?> </p> </td>

        	</tr>
        	<?
        		$j++;
				$tr_production_qnty+=$selectResult[csf('production_qnty')];
        	}
        	?>
        	</tbody>
			<tfoot>
				<th colspan="11">Total</th>
				<th><?  echo $tr_production_qnty;?></th>
				<th></th>
...
<div style="width:1230px;">
    <table width="750" cellspacing="0" align="center">
        ...
        <tr>
            ...
            ?>
        	<td><strong>Sewing Company <span style="float: right;">: &nbsp;</span> </strong></td>
        	<td>
        		<?
        	...
        	<td><strong> Source <span style="float: right;">: &nbsp;</span> </strong></td>
        	<td>
	...
	<div style="width:100%;">
        ...
        <br>
		 <?
            echo signature_table(26, $data[0], "1200px");
         ?>
	</div>
	</div>
<?
