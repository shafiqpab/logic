...
if($action=="report_generate") 
{
	...
	if ($txt_date_from != "" && $txt_date_to != "") {
		if ($db_type == 0) {
			$date_cond = " and c.country_ship_date between '" . change_date_format(trim($txt_date_from), "yyyy-mm-dd") . "' and '" . change_date_format(trim($txt_date_to), "yyyy-mm-dd") . "'";
		} else {
			$date_cond = " and c.country_ship_date between '" . change_date_format(trim($txt_date_from), '', '', 1) . "' and '" . change_date_format(trim($txt_date_to), '', '', 1) . "'";
		}
	} else {
		$date_cond = "";
	}
        ...
        <table class="rpt_table" cellspacing="0" cellpadding="0" border="1" rules="all" width="1450">
            <thead style="font-size:13px">
                <tr>
                    ...
                    <th width="70">First Inspaction Receive [Pcs]</th>
                    <th width="70">First Inspaction[Pcs]</th>
	...
	$consLbs=sql_select("select a.color_id, a.actual_consumption, b.job_no from ppl_size_set_consumption a, ppl_size_set_mst b where b.id=a.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.id ");
	$consLbs_arr=array();
	...
	foreach($proddate_sqlRes as $row)
	{
		...
		if($row[csf('production_type')]==53)
			$bundleissue_qty =($row[csf('production_qnty')] ) - ($reject_qty[$row[csf('bundle_no')]] + $alter_qty[$row[csf('bundle_no')]]+ $spt_qty[$row[csf('bundle_no')]]);
		if($row[csf('production_type')]==54)
			$proddate_arr[$row[csf('bundle_no')]][$row[csf('barcode_no')]]['linking _rec']=$row[csf('production_date')];
		...
		if($row[csf('production_type')]==76)//issue 1st insp
			$knitting_qty = ($row[csf('production_qnty')] ) - ($reject_qty[$row[csf('bundle_no')]] + $alter_qty[$row[csf('bundle_no')]]+ $spt_qty[$row[csf('bundle_no')]]);
		if($row[csf('production_type')]==77)//Receive 1st insp
			$ispfirst_rcv_qty = ($row[csf('production_qnty')] ) - ($reject_qty[$row[csf('bundle_no')]] + $alter_qty[$row[csf('bundle_no')]]+ $spt_qty[$row[csf('bundle_no')]]);
	}
	unset($proddate_sqlRes);
	
	// , SUM(i.reject_qty) as reject_qty, SUM(i.alter_qty) as alter_qty, SUM(i.spot_qty) as spot_qty, SUM(i.replace_qty) as replace_qty
	
	if($db_type==0) $year_field="YEAR(a.insert_date)"; else if($db_type==2) $year_field="to_char(a.insert_date,'YYYY')";
		$sql="SELECT a.job_no, a.job_no_prefix_num, a.buyer_name, a.style_ref_no as style, $year_field as year, b.id as po_id, b.po_number, c.item_number_id, c.country_ship_date, c.color_number_id, c.size_number_id, d.bundle_no, d.barcode_no, d.number_start, d.number_end, d.size_qty, f.cutting_no, f.entry_date,g.qc_pass_qty
		from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, ppl_cut_lay_bundle d, ppl_cut_lay_size e, ppl_cut_lay_mst f,pro_gmts_cutting_qc_dtls g
		where a.garments_nature=100 and g.color_size_id=c.id and a.job_no=b.job_no_mst and b.id=c.po_break_down_id and b.id=d.order_id and c.country_id=d.country_id and c.color_number_id=e.color_id and d.dtls_id=e.dtls_id and d.size_id=e.size_id and c.size_number_id=d.size_id and f.id=d.mst_id and f.id=e.mst_id 
		
		$companyCond $buyer_id_cond $jobCond $styleCond $poCond $po_ids_cond $lotratioCond $bundleNoCond $qcNoCond $date_cond and a.is_deleted=0 and b.is_deleted=0 and c.is_deleted=0 and d.is_deleted=0 and e.is_deleted=0 and f.is_deleted=0  and g.is_deleted=0 
		
		group by a.job_no, a.job_no_prefix_num, a.buyer_name, a.style_ref_no, a.insert_date, b.id, b.po_number, c.item_number_id, c.country_ship_date, c.color_number_id, c.size_number_id, d.bundle_no, d.barcode_no, d.number_start, d.number_end, d.size_qty, f.cutting_no, f.entry_date,g.qc_pass_qty order by d.barcode_no desc";
	//	 echo $sql; die;
	...
	<div style="width:1470px; overflow-y: scroll; max-height:350px" id="scroll_body">
        ...
        foreach($sql_res as $row)
		{
            ...
            <tr bgcolor="<?=$bgcolor;?>" onClick="change_color('tr_1nd<?=$i; ?>','<?=$bgcolor; ?>')" id="tr_1nd<?=$i; ?>" style="font-size:13px">
				...
				<td width="109" align="center" style="word-break:break-all"><br><?=$row[csf('style')]; ?></td>
                ...
                <td width="94" align="center" style="word-break:break-all"><br>&nbsp;<?=change_date_format($row[csf('country_ship_date')]); ?></td>
                ...
                <td width="94" align="center"><br><?= $knitting_qty ?>&nbsp;</td>          
                <td width="94" align="center"><br><?= $ispfirst_rcv_qty; ?>&nbsp;</td>                
                <td width="106" align="center"><br><?=$row[csf('qc_pass_qty')]; ?>&nbsp;</td>
                <td width="94" align="center"><br><?= $bundleissue_qty; ?>&nbsp;</td>            
                <td align="center"><br><?=$ispfirst_rcv_qty - $bundleissue_qty; ?>&nbsp;</td>
            </tr>
            <?
			$i++;
			$kniQty +=$knitting_qty;
			$firstInsRec +=$ispfirst_rcv_qty;
			$firstIns +=$row[csf('qc_pass_qty')];
			$issuDep +=$bundleissue_qty;
			$balance=$ispfirst_rcv_qty-$bundleissue_qty;
			$sizeQty +=$row[csf('size_qty')];
    ...
    <table class="tbl_bottom" cellspacing="0" cellpadding="0" border="1" rules="all" width="1450">
        <tr style="font-size:13px">
            ...
            <td width="80" align="center"><?php echo $sizeQty;?></td>
            <td width="94" align="center"><?= $kniQty;?></td>
            <td width="94" align="center"><?= $firstInsRec;?></td>
            <td width="106" align="center"><?= $firstIns ;?></td>
            <td width="94" align="center"><?= $issuDep;?></td>
...
if($action=="report_summery_generate") 
{
	...
	if($db_type==0) $year_field="YEAR(a.insert_date)"; else if($db_type==2) $year_field="to_char(a.insert_date,'YYYY')";
	$sql="SELECT a.job_no, a.job_no_prefix_num, a.buyer_name, a.style_ref_no as style, $year_field as year, b.id as po_id, b.po_number, c.item_number_id, c.country_ship_date,b.doc_sheet_qty, c.color_number_id, c.size_number_id, d.bundle_no, d.barcode_no, d.number_start, d.number_end, d.size_qty, f.cutting_no, f.entry_date,g.qc_pass_qty
	...
	,pro_gmts_cutting_qc_dtls g where a.garments_nature=100  and g.color_size_id=c.id and a.job_no=b.job_no_mst and b.id=c.po_break_down_id and b.id=d.order_id and c.country_id=d.country_id and c.color_number_id=e.color_id and d.dtls_id=e.dtls_id and d.size_id=e.size_id and c.size_number_id=d.size_id and f.id=d.mst_id and f.id=e.mst_id 
	
	$companyCond $buyer_id_cond $jobCond $styleCond $poCond $po_ids_cond $lotratioCond $bundleNoCond $qcNoCond $date_cond and a.is_deleted=0 and b.is_deleted=0 and c.is_deleted=0 and d.is_deleted=0 and e.is_deleted=0 and f.is_deleted=0  
	
	group by a.job_no, a.job_no_prefix_num, a.buyer_name, a.style_ref_no, a.insert_date, b.id, b.po_number, c.item_number_id, c.country_ship_date,b.doc_sheet_qty, c.color_number_id, c.size_number_id, d.bundle_no, d.barcode_no, d.number_start, d.number_end, d.size_qty, f.cutting_no, f.entry_date,g.qc_pass_qty order by d.barcode_no desc";
		 //echo $sql; die;
	$sql_res = sql_select($sql); $i=1;
	?>
	<div style="width:1470px; overflow-y: scroll; max-height:350px" id="scroll_body">
        ...
        foreach($sql_res as $row)
		{
            ...
            <tr bgcolor="<?=$bgcolor;?>" onClick="change_color('tr_1nd<?=$i; ?>','<?=$bgcolor; ?>')" id="tr_1nd<?=$i; ?>" style="font-size:13px">
                ...
                <td width="101" align="center" style="word-break:break-all"><br><?=$buyerArr[$row[csf('buyer_name')]]; ?></td>
				<td width="115" align="center" style="word-break:break-all"><br><?=$row[csf('style')]; ?></td>
                <td width="88" align="center"><br><?=$row[csf('job_no_prefix_num')]; ?></td>
                
             
                <td width="145" align="center" style="word-break:break-all"><br><?=$garments_item[$row[csf('item_number_id')]]; ?></td>
                <td width="102" align="center" style="word-break:break-all"><br>&nbsp;<?=change_date_format($row[csf('country_ship_date')]); ?></td>
                <td width="145" align="center" style="word-break:break-all;" ><br><?=$colorArr[$row[csf('color_number_id')]]; ?>&nbsp;</td>
                <td width="87" align="center" style="word-break:break-all"><br><?=$sizeArr[$row[csf('size_number_id')]]; ?>&nbsp;</td>				
                <td width="87" align="center"><br><?=$row[csf('doc_sheet_qty')]; ?></td>               
                <td width="102" align="center"><br><?= $knitting_qty; ?>&nbsp;</td>          
                <td width="102" align="center"><br><?= $ispfirst_rcv_qty; ?>&nbsp;</td>                
                <td width="114" align="center"><br><?=$row[csf('qc_pass_qty')]; ?>&nbsp;</td>
                <td width="102" align="center"><br><?= $bundleissue_qty; ?>&nbsp;</td>            
                <td align="center"><br><?= $ispfirst_rcv_qty-$bundleissue_qty; ?>&nbsp;</td>
            </tr>
            <?
			$i++;
			...
			$issuDep +=$bundleissue_qty;
			$balance=$ispfirst_rcv_qty-$bundleissue_qty;
			$sizeQty +=$row[csf('doc_sheet_qty')];
			$gsizeQty+=$row[csf('doc_sheet_qty')];
			$gsizeLbsQty+=$lbsQty;
    ...
    <table class="tbl_bottom" cellspacing="0" cellpadding="0" border="1" rules="all" width="1450">
        <tr style="font-size:13px">
            ...
            <td width="87" align="center" ><?=$sizeQty; ?></td>
            <td width="102"  align="center"><?= $kniQty ;?></td>
            <td width="102" align="center"><?= $firstInsRec ;?></td>
            <td width="114"  align="center"><?= $firstIns ?></td>
            <td width="102"  align="center"><?= $issuDep ?></td>
            <td  align="center"><?= $balance ?></td>
