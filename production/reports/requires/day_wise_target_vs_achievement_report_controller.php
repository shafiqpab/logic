...
{
	echo create_drop_down( "cbo_location_id", 150, "select id,location_name from lib_location where status_active=1 and is_deleted=0 and company_id='$data' order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down( 'requires/day_wise_target_vs_achievement_report_controller', this.value, 'load_drop_down_floor', 'floor_td' );",0 ); 
	exit();    	 
}

if ($action=="load_drop_down_floor")
{
	echo create_drop_down( "cbo_floor_id", 130, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process=5 order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "",0 );     	 	
	exit();    	 
}
...
if($action=="report_generate__")
{
	...
	{	
		...
		$floorLineArray[$val[csf('floor_id')]][$sewing_line] = $val[csf('floor_id')];

		if($dataArray[$val[csf('floor_id')]][$sewing_line][$val[csf('production_date')]]['item_number_id']!="")
		{
			$dataArray[$val[csf('floor_id')]][$sewing_line][$val[csf('production_date')]]['item_number_id'].="****".$val[csf('po_break_down_id')]."**".$val[csf('item_number_id')]; 
		}
		else
		{
			 $dataArray[$val[csf('floor_id')]][$sewing_line][$val[csf('production_date')]]['item_number_id']=$val[csf('po_break_down_id')]."**".$val[csf('item_number_id')]; 
	...
	$tbl_width = 200 + (count($date_range)*40);
	...
	<fieldset style="width:<? echo $tbl_width+20;?>px">
       <table width="<? echo $tbl_width;?>" cellpadding="0" cellspacing="0">
            ...
            <tr class="form_caption">
                <td colspan="<? echo $colspan;?>" align="center"><strong>Day Wise Target VS Achievement Report &nbsp;</strong></td> 
            </tr>
            <tr class="form_caption">
                <td colspan="<? echo $colspan;?>" align="center"><strong><? echo "Date:  ".change_date_format( str_replace("'","",trim($txt_date_from)) ); ?> To <? echo change_date_format( str_replace("'","",trim($txt_date_to)) ); ?></strong></td> 
            </tr>
        </table>
        <!-- ============================== heading part ============================== -->
        <div>
            		...
            		foreach ($floorLineArray as $f_key => $f_data) 
            		{
            			...
            			foreach ($f_data as $l_key => $line_date) 
            			{
            				...
            				<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>" style="font-size:11px">
            					<td rowspan="5" width="80"><? echo $l_key;?></td>
            					<td width="50">Target</td>
            					<?
            					$target = 0;
			                   	foreach ($date_range as $key => $value) 
			                  	{
			                  		// echo $f_key."=".$l_key."=".$value."<br>";
            			...
            			<tr style="background: #a8d5baff; font-weight: bold;">
	    					<td rowspan="5" width="50"><? echo $floorArr[$f_key];?><br>Total</td>
	    					<td width="50">Target</td>
	    					<?
	    					$floor_target = 0;
		                   	foreach ($date_range as $key => $value) 
        ...
        <div>
            <table class="rpt_table" width="<? echo $tbl_width;?>" cellpadding="0" cellspacing="0" border="1" rules="all" align="left">
            	<tfoot>            		
    				<tr>
    					<th rowspan="5" width="80">Total Achieved</th>
...
if($action=="report_generate") // show 
{
	...
	// echo "<pre>"; print_r($line_wise_style_count_arr);die();
...
if($action=="report_generate2")// actual smv
{
	?>
	<style type="text/css">		
		table tr th,table tr td
	...
	if($prod_reso_allo[0]==1)
	{
		...
		{			
			...
			$prod_resource_smv_array[$val[csf('id')]][$style_arr[$val[csf('po_id')]]][$val[csf('gmts_item_id')]][$val[csf('pr_date')]]=$val[csf('actual_smv')];	
		}
		...
		$sql_query="SELECT b.mst_id, b.pr_date,b.number_of_emp ,b.adjust_hour,b.total_smv,b.adjustment_source from prod_resource_mst a,prod_resource_smv_adj b  where a.id=b.mst_id  and a.company_id=$company_id $pr_date_con and a.is_deleted=0 and b.is_deleted=0 and b.is_deleted=0 and b.status_active=1 ";
		// echo $sql_query;
            					...
            					$line_total_efficiency_min = 0;
			                   	foreach ($date_range as $key => $value) 
			                  	{
									...
									foreach ($germents_item as $g_val) 
									{
										$po_garment_item=explode('**',$g_val);
										if($item_smv!='') $item_smv.='/';
										$item_smv.=$prod_resource_smv_array[$line_date][$po_garment_item[2]][$po_garment_item[1]][$value];
										$produce_minit+=$production_data_arr_qty2[$f_key][$l_key][$po_garment_item[0]][$value]['quantity']*$prod_resource_smv_array[$line_date][$po_garment_item[2]][$po_garment_item[1]][$value];

										// echo $production_data_arr_qty2[$f_key][$l_key][$po_garment_item[0]][$value]['quantity']."*".$prod_resource_smv_array[$line_date][$po_garment_item[2]][$po_garment_item[1]][$value]."<br>";
									...
									// echo $efficiency_min."=".$line_date."=".$value."<br>";

									// ================== for extra hour, leave ,sick out etc.. ===================
									$txtDate = str_replace("'", "", $value);
									if($line_wise_style_count_arr[$value][$f_key][$line_date]>1)
									{
										$mn_power = $prod_resource_smv_adj_array[$line_date][$txtDate][1]['number_of_emp'];
										if($line_wise_style_count_arr2[$value][$f_id][$line_date]>1)
										{
											$late_hr = $prod_resource_smv_adj_array[$line_date][$txtDate][5]['total_smv'];
											...
											$line_wise_style_count_arr2[$value][$f_id][$line_date]--;
										}
										else
										{
											
											$extra_hr = $prod_resource_smv_adj_array[$line_date][$txtDate][1]['total_smv'];
											$lunch_hr = $prod_resource_smv_adj_array[$line_date][$txtDate][2]['total_smv'];
											$sick_hr = $prod_resource_smv_adj_array[$line_date][$txtDate][3]['total_smv'];
											$leave_hr = $prod_resource_smv_adj_array[$line_date][$txtDate][4]['total_smv'];
									...
									else // for single line
									{
										...
										// echo $efficiency_min."=".$l_id."=".$style_key."=".$pr_date."=".$extra_hr."- (".$lunch_hr."+".$sick_hr."+".$leave_hr.")<br>";
...
if($action=="report_generate3")// show2
{
	?>
	<style type="text/css">		
		table tr th,table tr td
	...
	{	
		...
		$dataArray[$val[csf('floor_id')]][$sewing_line][$val[csf('production_date')]]['qty'] += $val[csf('good_qnty')];
		$floorLineArray[$val[csf('floor_id')]][$sewing_line] = $val[csf('sewing_line')];
		$date_and_line_qty_arr[$sewing_line][$val[csf('production_date')]] += $val[csf('good_qnty')];
		...
		$production_data_arr_qty[$val[csf('floor_id')]][$sewing_line][$val[csf('po_break_down_id')]][$val[csf('item_number_id')]][$val[csf('production_date')]]['quantity']+=$val[csf('good_qnty')];
		$production_data_arr_qty2[$val[csf('floor_id')]][$sewing_line][$val[csf('po_break_down_id')]][$val[csf('item_number_id')]][$val[csf('production_date')]]['quantity']+=$val[csf('good_qnty')];
	...
	// echo "<pre>"; print_r($date_and_line_qty_arr);die();
	...
	$date_range = get_date_range($date_from,$date_to);
	// echo "<pre>";print_r($floorLineArray);die();
	$tbl_width = 200 + (count($date_range)*40);
	$colspan = 3 + count($date_range);
            			...
            			foreach ($f_data as $l_key => $line_date) 
            			{
            				...
            				<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>" style="font-size:11px">
            					...
            					$target = 0;
			                  	...
			                  	{
			                  		// echo $f_key."=".$line_date."=".$value."<br>";
									if($date_and_line_qty_arr[$l_key][$value]>0)
									{
										if($line_wise_style_count_arr[$value][$f_key][$l_key]>1)
										{
											// $target = $prod_resource_array2[$f_key][$l_key][$value]['tpd'];
											$target = $prod_resource_array[$f_key][$l_key][$value]['tpd'];
											$floor_qty_arr[$f_key][$value]['target'] += $target;
											$grand_total_arr[$value]['target'] += $target;
											$line_tot_target += $target;
            					...
            					$deviation = 0;
			                   	...
			                   	foreach ($date_range as $key => $value) 
			                  	{
									if($date_and_line_qty_arr[$l_key][$value]>0)
									{
										...
										{
											// $target = $prod_resource_array2[$f_key][$line_date][$value]['tpd'];
											$target = $prod_resource_array[$f_key][$line_date][$value]['tpd'];
											$achivement = $dataArray[$f_key][$l_key][$value]['qty'];
											$deviation = $achivement - $target;			                  		
											$floor_qty_arr[$f_key][$value]['deviation'] += $deviation;
											$grand_total_arr[$value]['deviation'] += $deviation;
            					...
            					$achive_prsnt = 0;
			                   	foreach ($date_range as $key => $value) 
			                  	{
									if($date_and_line_qty_arr[$l_key][$value]>0)
									{
			                   		...
			                   		?>
			                   		<td title="(Prod Qty*Target)*100" align="right" width="40"><? echo number_format($achive_prsnt,2);?></td>
			                   		<?
			                   	}
			                   	$i++;
			                   	if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
            					...
            					$line_total_efficiency_min = 0;
			                   	foreach ($date_range as $key => $value) 
			                  	{
									...
									foreach ($germents_item as $g_val) 
									{
										...
										if($item_smv!='') $item_smv.='/';
										$item_smv.=$item_smv_array[$po_garment_item[0]][$po_garment_item[1]];
										$produce_minit+=$production_data_arr_qty2[$f_key][$l_key][$po_garment_item[0]][$po_garment_item[1]][$value]['quantity']*$item_smv_array[$po_garment_item[0]][$po_garment_item[1]];
										// echo $production_data_arr_qty2[$f_key][$l_key][$po_garment_item[0]][$po_garment_item[1]][$value]['quantity']."*".$item_smv_array[$po_garment_item[0]][$po_garment_item[1]]."<br>";
										...
										if(str_replace("'","",$actual_production_date)>str_replace("'","",$actual_date)) 
										{
											...
											else
											{
												if($date_and_line_qty_arr[$l_key][$value]>0)
												{
													if($line_wise_style_count_arr[$f_key][$l_key]>1)
													{
														if($total_eff_hour>$prod_resource_array[$l_key][$value]['working_hour'])
														{
															$total_eff_hour=$prod_resource_array[$l_key][$value]['working_hour'];
														}
													}
													else
													{
            						...
            						$line_total_efficiency_min += $efficiency_min;
			                   		?>
			                   		<td title="((Prod Qty*Item SMV)/(Manpower*Working Hour*60))*100" align="right" width="40"><? echo number_format($achive_effi,2);?></td>
			                   		<?
			                   	}
			                   	$line_tot_achive_effi = ($line_total_efficiency_min>0) ? ($line_total_produce_min / $line_total_efficiency_min)*100 : 0;
			                   	// echo $line_total_produce_min ."/". $line_total_efficiency_min."*10<br>";
