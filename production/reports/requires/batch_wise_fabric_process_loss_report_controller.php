...
if($action=="report_generate")
{
	...
	if($category==2)
	{
		...
		foreach($sql_dtls_result as $row)
		{
			...
			$data[$row[csf('knitting_source')]][$order_type][$uom][$batchNoAndExtention][$prodId][$row[csf('body_part_id')]]['booking_no'] 		= $row[csf('booking_no')];
		...
		if($salesOrder_id!="")
		{
			...
			$sql_batch=sql_select("SELECT b.body_part_id, a.id,sum(b.batch_qnty) as batch_qnty, a.batch_weight, a.total_trims_weight, a.sales_order_no, a.is_sales, a.color_id, a.batch_no, a.color_id, a.color_range_id, a.booking_no, a.extention_no, b.prod_id
			from pro_batch_create_mst a, pro_batch_create_dtls b, GBL_TEMP_ENGINE g 
			where a.id=b.mst_id and a.sales_order_id=g.ref_val and g.user_id=$user_name and g.entry_form=1090 and g.ref_from=3 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
			group by b.body_part_id,a.id,a.batch_weight,a.total_trims_weight,a.sales_order_no, a.is_sales,a.batch_no, a.color_id, a.color_range_id, a.booking_no, a.extention_no, b.prod_id");
			//$batch_conds 

			foreach($sql_batch as $row)
			{
				if($row[csf('extention_no')]==0 || $row[csf('extention_no')] =="")
				{
					$body_part_wise_batch_arr[$row[csf('batch_no')]][$row[csf('color_id')]][$row[csf('booking_no')]][$row[csf('body_part_id')]]['batch_qnty']+=$row[csf('batch_qnty')];
					$batch_no_arr[$row[csf('id')]]['batch_qnty']+=$row[csf('batch_qnty')];

					$batch_qnty_with_booking_color_batch[$row[csf('batch_no')]][$row[csf('color_id')]][$row[csf('booking_no')]]+=$row[csf('batch_qnty')];
				}
		...
		<div>
			<fieldset style="width:2880px;">
				...
				<table border="1" rules="all" class="rpt_table" cellpadding="0" cellspacing="0" width="2950" align="left" >
					<thead>
						<tr>
							<th width="30">SL</th> 
				...
				<div style="width:2970px; max-height:340px; overflow-y:scroll"  align="left" id="scroll_body">
					<table border="1" rules="all" class="rpt_table" cellpadding="0" cellspacing="0" width="2950" >
						<tbody>
							<?
											...
											foreach($batchNoExtData as $prodId=>$productionArr)
											{
												foreach($productionArr as $bodyPartId=> $row)
												{
													...
													if($cbo_batch_status==0 || $cbo_batch_status==2 || ($cbo_batch_status==1 && $incomple_batch_arr[$kniting_source_key][$order_type][$uomId][$batch_no]==""))
													{
														...
														$batch_sum_array[$kniting_source_key][$order_type][$uomId][$batch_no]['open_qc_pass_qty'] += $row['open_qc_pass_qty'];

														
														
														if($batch_id_bodypart_check[$batch_no."*".$row['color_id']."*".$row['booking_no']."*".$bodyPartId."*".$uomId]=="")
														{
															$batch_id_bodypart_check[$batch_no."*".$row['color_id']."*".$row['booking_no']."*".$bodyPartId."*".$uomId] = $row['batch_id'].'*'.$bodyPartId;

															$fabric_weight=$body_part_wise_batch_arr[$batch_no][$row['color_id']][$row['booking_no']][$bodyPartId]['batch_qnty'];
															$batch_sum_array[$kniting_source_key][$order_type][$uomId][$batch_no]['fab_wgt'] += $fabric_weight;
														}

														if($batch_id_book_color_check[$batch_no."*".$row['color_id']."*".$row['booking_no']."*".$uomId] =="")
														{
															$batch_id_book_color_check[$batch_no."*".$row['color_id']."*".$row['booking_no']] = $batch_no."*".$row['color_id']."*".$row['booking_no'];
															$batch_sum_array[$kniting_source_key][$order_type][$uomId][$batch_no]['batch_qnty'] = $batch_qnty_with_booking_color_batch[$batch_no][$row['color_id']][$row['booking_no']];
															//echo $batch_no."=".$uomId."=".$batch_qnty_with_booking_color_batch[$batch_no][$row['color_id']][$row['booking_no']]."<br>";
														}
								...
								foreach($kniting_source_key_val as $order_type=>$orderData)
								{
									...
									foreach($orderData as $uomId=>$uomData)
									{
										...
										<tr style="background-color:#4a7570;">
											<td colspan="30"><strong><? echo $unit_of_measurement[$uomId];?></strong> </td>
										</tr>
										<?
										foreach($uomData as $batchNoExt=>$batchNoExtData)
										{
											foreach($batchNoExtData as $prodId=>$productionArr)
											{
												foreach ($productionArr as $bodyPartId=>$row) 
												{
													...
													<tr bgcolor="<? echo $bgcolor; ?>"  onclick="change_color('tr_<?echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<?echo $i;?>">
														...
														<td width="100" class="wordwrap"><? echo $buyer_Arr[$row['buyer_id']];?></td>
														<td width="100" class="wordwrap"><? echo ($order_type==0)?$po_data_arr[$row['order_id']]['style_ref_no']:$sub_con_data_arr[$row["order_id"]]['cust_style_ref']; ?></td>
														<td width="100" class="wordwrap"><? echo ($order_type==0)?$batch_no_arr[$row['batch_id']]['booking_no']:$sub_con_data_arr[$row["order_id"]]['subcon_job'];?></td>
														<td width="100"><p><? echo $booking_type_arr[$po_data_arr[$row['order_id']]['booking_entry_form']];?></p></td>
														<td width="100" class="wordwrap"><? echo ($order_type==0)?$batch_no_arr[$row['batch_id']]['sales_order_no']:$sub_con_data_arr[$row["order_id"]]['order_no'];?></td>
														<td width="100" class="wordwrap"><? echo $batch_no=$batch_no_arr[$row['batch_id']]['batch_no'];?></td>
														<td width="100" class="wordwrap"><? echo $batch_no_arr[$row['batch_id']]['extention_no'];?></td>
														<td width="100" class="wordwrap"><? echo $body_part[$bodyPartId]; ?></td>
														<?
														if ($kniting_source_key==1) 
														{
															...
															{
																?>
																<td align="center" width="100" class="wordwrap"><? echo $constructtion_arr[$row['fab_desc_id']]; ?></td>
																<td  align="center" width="120" class="wordwrap"><? echo ($order_type==0)?$composition_arr[$row['fab_desc_id']]:$row['fabric_description'];?></td>
																<?
															}
														}
														else
														...
														//$batch_weight_from_table = $batch_no_arr[$row['batch_id']]['batch_qnty'];
														...
														<td width="100" class="wordwrap"><? echo $fabric_typee[$row['dia_width_type']]; ?></td>
														<td width="100"><? echo $row['gsm'];?></td>
														<td width="100" class="wordwrap"><? echo $color_arr[$row['color_id']];?></td>
														<td width="100" class="wordwrap"><? echo $color_range[$batch_no_arr[$curr_batch_id]['color_range_id']];?></td>
														<? 
														...
														if($batch_row_chk[$kniting_source_key][$order_type][$uomId][$batch_no] == "")
														{
															...
															$open_qc_pass_qty=$batch_sum_array[$kniting_source_key][$order_type][$uomId][$batch_no]['open_qc_pass_qty'];

															$total_qc_pass_qty +=$open_qc_pass_qty + $batch_sum_array[$kniting_source_key][$order_type][$uomId][$batch_no]['qc_pass_qty'];

															$total_yds_to_kg += $batch_sum_array[$kniting_source_key][$order_type][$uomId][$batch_no]['yds_to_kg'];

															$fab_wgt = $batch_sum_array[$kniting_source_key][$order_type][$uomId][$batch_no]['fab_wgt'];

															$batch_weight_from_table = $batch_sum_array[$kniting_source_key][$order_type][$uomId][$batch_no]['batch_qnty'];
															$total_batch_weight=$batch_weight_from_table+$total_trims_weight;

															//N.B : Fabric wgt comes from batch and body part wise BUT Total Batch Wtg comes only from batch
														?>
														<td width="100" align="right" rowspan="<? echo $row_count;?>"><p><? echo number_format($fab_wgt,2);?></p></td>
														...
														<td width="100" align="right" rowspan="<? echo $row_count;?>"><p><? echo number_format($open_qc_pass_qty,2);?></p></td>
														<?
														$total_unit_wise_open_tot_qty[$kniting_source_key][$order_type][$uomId] += $open_qc_pass_qty;
														}
														?>
														...
														?>
															...
															if($uomId==27)
															{
																$process_loss = 100- ($total_yds_to_kg /$total_batch_weight)*100; 
															}
															else
															{
														...
														if($batch_row_chk[$kniting_source_key][$order_type][$uomId][$batch_no] == "")
														{
															...
															if($yds_to_kg)
															{
																$process_loss_with_reprocess = 100 - ($yds_to_kg / $total_batch_weight) * 100 ;
																//{100-(Yds To Kg/Total Batch Weight x 100)}
															}
															else
															{
																$process_loss_with_reprocess = 100 - ($total_qc_pass_qty / $total_batch_weight) * 100 ;
																//100 - (147 / 231.7)*100
															}
