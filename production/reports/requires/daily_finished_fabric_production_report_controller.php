...
if($action=="report_generate")
{
	...
	if ($type==1) // Show Start
	{
		...
		<div>
	  	...
	  	<fieldset style="width:2970px;">
	        <table width="2020px" cellpadding="0" cellspacing="0" id="caption" align="center">
	            <tr>
	        ...
	        <table  width="3130" border="1" rules="all" class="rpt_table" cellpadding="0" cellspacing="0" >
	        	<thead>
	                <th width="40">SL</th>
	                ...
	                <th width="80">Fabric Process Loss %</th>
					<th width="80">Insert User</th>
	                <th width="">Remarks</th>
	            </thead>
	        </table>
	        <div style="width:3160px; overflow-y:scroll; max-height:350px;" id="scroll_body">
	        <table width="3130" border="1" rules="all" class="rpt_table" cellpadding="0" cellspacing="0" id="tbl_dyeing">
	        <?
	        $con = connect();
			...
			$roll_maintained=return_field_value("fabric_roll_level","variable_settings_production","company_name=$cbo_company and item_category_id=2 and variable_list=3 and is_deleted=0 and status_active=1");
			$insert_by=return_library_array( "Select id,user_name from user_passwd", "id", "user_name"  );
			if($roll_maintained==1) $roll_maintained=$roll_maintained; else $roll_maintained=0;

			// ================= For without roll lavel Start =======
			$sql_dtls="SELECT a.knitting_source, a.knitting_company, b.floor, a.recv_number, a.receive_date,a.location_id,b.order_id,b.batch_id,b.gsm,b.width,b.grey_used_qty,b.receive_qnty as qc_pass_qty,b.reject_qty,b.color_id,b.fabric_description_id,b.buyer_id , c.booking_no, c.booking_without_order, c.batch_no,c.extention_no,b.remarks,b.inserted_by,b.prod_id, c.batch_weight
			from inv_receive_master a, pro_finish_fabric_rcv_dtls b, pro_batch_create_mst c
			where  a.id=b.mst_id $lcCompany_cond and a.entry_form in(7,66) and b.batch_id = c.id and c.status_active =1 and a.item_category=2 and a.status_active=1 and a.is_deleted=0  $date_cond $unit_name $knitting_ref_cond  $all_booking_no_cond $batch_cond order by c.batch_no";
			//echo $sql_dtls;
			$order_ids = "";$nonOrdBooking= "";
			$sql_dtls_result=sql_select($sql_dtls);
			//print_r($sql_dtls_result);
			...
			foreach ($sql_dtls_result as $key => $row) 
			{
				...
				$production_data_arr[$row[csf('recv_number')]][$str_ref]['inserted_by']=$row[csf('inserted_by')];
				...
				$production_data_arr[$row[csf('recv_number')]][$str_ref]['batch_weight']=$row[csf('batch_weight')];
			...
			// echo $order_ids.'#<br>';
			// ================= For without roll lavel End =========

			// ================= For roll lavel Start ===============
			$sql_roll_dtls="SELECT a.knitting_source, a.knitting_company, b.floor, a.recv_number, a.receive_date,a.location_id,b.order_id,b.batch_id,b.gsm,b.width,b.receive_qnty as qc_pass_qty,b.reject_qty,b.color_id,b.fabric_description_id,b.buyer_id , c.booking_no, c.booking_without_order, c.batch_no,c.extention_no,b.remarks, b.barcode_no, b.prod_id, c.batch_weight
			from inv_receive_master a, pro_finish_fabric_rcv_dtls b, pro_batch_create_mst c
			where  a.id=b.mst_id $lcCompany_cond and a.entry_form in(7,66) and b.batch_id = c.id and c.status_active =1 and a.item_category=2 and a.status_active=1 and a.is_deleted=0 and b.barcode_no!=0 $date_cond $unit_name $knitting_ref_cond  $all_booking_no_cond $batch_cond order by c.batch_no";
			//echo $sql_roll_dtls;die;
			...
			where b.barcode_no=c.barcode_no and b.status_active=1 and c.userid=$user_name group by b.barcode_no, b.batch_qnty");
	        ...
	        foreach ($sql_roll_dtls_result as $key => $row) 
			{
				...
				if($row[csf('booking_without_order')]==1)
				{
					$greyUsedArr[$row[csf('recv_number')]][$row[csf('batch_id')]][$row[csf('prod_id')]]+=$row[csf('grey_used_qty')];
				}
				else
				{
					$greyUsedArr[$row[csf('recv_number')]][$row[csf('batch_id')]][$row[csf('order_id')]][$row[csf('prod_id')]]+=$batch_dtls_qty;;
				}
				$production_data_arr[$row[csf('recv_number')]][$str_ref]['knitting_source']=$row[csf('knitting_source')];
				$production_data_arr[$row[csf('recv_number')]][$str_ref]['knitting_company']=$row[csf('knitting_company')];
				$production_data_arr[$row[csf('recv_number')]][$str_ref]['floor']=$row[csf('floor')];
			...
			else
			{
			    ...
			    if($nonOrdBooking!="")
			    {
					...
					where a.booking_no=b.booking_no and b.style_id=c.id and b.entry_form_id=140 and a.booking_type=4 and a.item_category=2 and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0  and c.status_active=1 and c.is_deleted=0  $nonOrdBooking_chnk_cond
					group by b.booking_no, c.style_ref_no, a.grouping, b.style_id";//$ref_company_cond 
					...
					$pro_smn_req_qty_arr=array();
					$sql_smple_req="SELECT a.id, b.booking_no,  a.grouping, b.style_id, b.finish_fabric,b.grey_fabric, b.fabric_color, b.lib_yarn_count_deter_id
					...
					foreach( $sql_sample_req_result as $row )
					{
						...
						$pro_smn_req_qty_arr[$row[csf('booking_no')]][$row[csf('fabric_color')]]['grey_qty']+=$row[csf('grey_fabric')];
						$pro_smn_req_qty_arr[$row[csf('booking_no')]][$row[csf('fabric_color')]]['fin_fab_qnty']+=$row[csf('finish_fabric')];
					}
			    }
				// echo "<pre>";
				// print_r($pro_smn_req_qty_arr);
				...
				if($order_ids != "")
				{
					...
					$req_qty_arr=array();
					$pro_req_qty_arr = array();
					$sql_req="SELECT a.booking_no, a.po_break_down_id, a.fabric_color_id, a.gsm_weight, a.dia_width, b.lib_yarn_count_deter_id, sum(a.grey_fab_qnty) as req_qty, sum(a.fin_fab_qnty) as fin_fab_qnty 
					...
					foreach( $sql_req_result as $row )
					{
						...
						$pro_req_qty_arr[$row[csf('booking_no')]][$row[csf('fabric_color_id')]]['grey_qty']+=$row[csf('req_qty')];
						$pro_req_qty_arr[$row[csf('booking_no')]][$row[csf('fabric_color_id')]]['fin_fab_qnty']+=$row[csf('fin_fab_qnty')];
					}
				}
				//echo "<pre>";print_r($pro_req_qty_arr);//die;
			...
			foreach ($production_data_arr as $recv_number => $recv_number_data)
			{
				foreach($recv_number_data as $str_ref => $row)
				{
					...
					$po_qty='';
					$shipment_date='';
					$style='';$file_no=$int_ref_no='';$batch_qty="";$greyUsed_qty="";
					$grey_qty=0;
					$fin_fab_qnty=0;
					...
					foreach($order_number as $po_id)
					{
						if($po_id>0)
						{
							...
							$greyUsed_qty+=$greyUsedArr[$recv_number][$batch_id][$po_id][$prod_id];
						...
						else
						{
							...
							$greyUsed_qty+=$greyUsedArr[$recv_number][$batch_id][$prod_id];
						}
					}
					...
					if($row['booking_without_order']==0)
					{
						...
						$grey_qty+=$pro_req_qty_arr[$row['booking_no']][$row['color_id']]['grey_qty'];
						$fin_fab_qnty+=$pro_req_qty_arr[$row['booking_no']][$row['color_id']]['fin_fab_qnty'];
					}
					else
					{
						$req_qty=$smn_req_qty_arr[$row['booking_no']][$row['fabric_description_id']][$row['color_id']]['req_qty'];
						
						$grey_qty+=$pro_smn_req_qty_arr[$row['booking_no']][$row['color_id']]['grey_qty'];
						$fin_fab_qnty+=$pro_smn_req_qty_arr[$row['booking_no']][$row['color_id']]['fin_fab_qnty'];
					}

					if($fin_fab_qnty >0 )
					{
						$process_percent=(($grey_qty-$fin_fab_qnty)/$grey_qty)*100;
					}
					else
					{
						$process_percent = 0;
					...
					$batch_span = $batch_no_count[$batch_no]++;
					?>
		            ...
		            <tr  bgcolor="<? echo $bgcolor; ?>"  onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
		                ...
		                <td  width="100"><p><? // 	echo $buyer_library[$row['buyer_id']]; echo $buyer_name;
						?></p> </td>
		                ...
		                <td width="80" class="word_wrap_break"><p><? 
		                if($row['booking_without_order']==1)
		                { 
		                	echo $nonOrdBooking_arr[$row['booking_no']]; 
		                }
		                else
		                {
		                	echo $int_ref_no;
		                ...
		                <td width="100" class="word_wrap_break" title="<? echo $batch_id; ?>"><p><? echo $batch_no; ?></p>  </td>
		                ...
		                <td width="80"  align="right" title="greyUsed: <? echo $greyUsed_qty;?>" id="total_grey_qty"><p><? echo number_format($row['grey_used_qty'],2);?></p>  </td>
		                <td width="80"  align="right" id="total_rj_qty"><p><? echo number_format($row['reject_qty'],2);?></p>  </td>
		                <td width="80"  align="right" id="" title="<? echo 'grey='.$grey_qty.' finish='.$fin_fab_qnty;?>"><p><? echo number_format($process_percent,2);?></p>  </td>
						<td width="80" class="word_wrap_break"><p><?php echo $insert_by[$row['inserted_by']]; ?></p></td>
		    ...
		    <table class="rpt_table" width="3130" cellpadding="0" cellspacing="0" border="1" rules="all"  id="tbl_dyeing_footer" align="left">
		    <tfoot>
		        <tr>
		            <th width="40">&nbsp;</th>
		            <th width="100">&nbsp;</th>
		            ...
		            <th width="100"></th>
		            <th width="100"></th>
		            <th width="80" id=""><? echo number_format($total_batch_qty,2); ?></th>
		            <th width="80" id=""><? echo number_format($total_qc_qty,2); ?></th>
		            <th width="80" id=""><? echo number_format($tot_grey_used_qty,2); ?></th>
		            <th width="80" id=""><? echo number_format($total_rj_qty,2); ?></th>
		            <th width="80" id=""><? //echo number_format($total_rj_qty,2); ?></th>
		            <th width="80">&nbsp;</th>
					<th width="">&nbsp;</th>
		        </tr>
	...
	if ($type==2) // Show 2 Start
	{
		...
		<div>
	  	...
	  	<fieldset style="width:2610px;">
			...
			$sql_dtls="SELECT a.knitting_source, a.knitting_company, b.floor, a.recv_number, a.receive_date,a.location_id,b.order_id,b.batch_id,b.gsm,b.width,b.grey_used_qty,b.receive_qnty as qc_pass_qty,b.reject_qty, c.color_range_id, b.color_id,b.fabric_description_id,b.buyer_id , c.booking_no, c.booking_without_order, c.batch_no,c.extention_no,b.remarks,b.inserted_by 
			from inv_receive_master a, pro_finish_fabric_rcv_dtls b, pro_batch_create_mst c
			...
			group by a.knitting_source, a.knitting_company, b.floor, a.recv_number,a.receive_date,a.location_id,b.batch_id,b.order_id,b.gsm,b.width,b.grey_used_qty,b.receive_qnty,b.reject_qty, c.color_range_id, b.color_id,b.fabric_description_id,b.buyer_id , c.booking_no, c.booking_without_order, c.batch_no,c.extention_no,b.remarks,b.inserted_by
			order by c.batch_no, a.recv_number";
			...
			else
			{
				...
				if($order_ids != "")
				{
					...
					$order_sql=sql_select("select b.id, b.job_no_mst, b.po_number, b.po_quantity, b.file_no, b.grouping as int_ref_no, a.total_set_qnty, a.style_ref_no, a.buyer_name, a.style_owner from wo_po_details_master a, wo_po_break_down b where b.job_no_mst=a.job_no $lcCompany_cond_2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $all_po_ref_cond");
					...
					foreach($order_sql as $val)
					{
						...
						$job_no_arr[$val[csf('id')]]['file_no']=$val[csf('file_no')];
						$job_no_arr[$val[csf('id')]]['int_ref_no']=$val[csf('int_ref_no')];
						$job_no_arr[$val[csf('id')]]['style_owner']=$val[csf('style_owner')];
					}
			...
			<table  width="2940" border="1" rules="all" class="rpt_table" cellpadding="0" cellspacing="0" >
	        	<thead>
	                <th width="40">SL</th>
	                <th width="100">Working Company</th>
	                <th width="100">Working Floor</th>
	        ...
	        <div style="width:2980px; overflow-y:scroll; max-height:350px;" id="scroll_body">
	        <table width="2940" border="1" rules="all" class="rpt_table" cellpadding="0" cellspacing="0" id="tbl_dyeing1">

			<?
			$batch_no_count_arr=array();
			$insert_by=return_library_array( "Select id,user_name from user_passwd", "id", "user_name"  );
			foreach($sql_dtls_result as $row)
			{
				$batch_no_count_arr[$row[csf('batch_no')]]++;
			}
			// echo "<pre>"; print_r($batch_no_count_arr);
			$i=1;	
			foreach($sql_dtls_result as $row)
			{
			 	if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
				...
				foreach($order_number as $po_id)
				{
					if($po_id>0)
					{
						...
						$file_no.=$job_no_arr[$po_id]['file_no'].",";
						$int_ref_no.=$job_no_arr[$po_id]['int_ref_no'].",";
						$style_owner.=$job_no_arr[$po_id]['style_owner'].",";
				...
				$job_mst_id=array_unique(explode(",",$job_no));
				$job_num='';
				$style_owner=implode(',',array_unique(explode(',',chop($style_owner,','))));
				...
				$batch_count=$batch_no_count_arr[$batch_no]++;
				...
				{
					$grey_qty+=$pro_req_qty_arr[$row[csf('booking_no')]][$row[csf('color_id')]]['grey_qty'];
					$fin_fab_qnty+=$pro_req_qty_arr[$row[csf('booking_no')]][$row[csf('color_id')]]['fin_fab_qnty'];
				}
				else
				{
					$grey_qty+=$pro_smn_req_qty_arr[$row[csf('booking_no')]][$row[csf('color_id')]]['grey_qty'];
					$fin_fab_qnty+=$pro_smn_req_qty_arr[$row[csf('booking_no')]][$row[csf('color_id')]]['fin_fab_qnty'];
				}
	            ...
	            <tr  bgcolor="<? echo $bgcolor; ?>"  onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
	                ...
	                <td  width="120"><p><? echo $booking_no;?></p>  </td>
	                <td  width="120"><p><? echo $job_num;?></p>  </td>
					<td  width="100"><p><? echo $company_arr[$style_owner];?></p>  </td>
					...
					<td width="80"  align="right" id=""><p><? echo number_format($process_percent,2);?></p>  </td>
					<td width="80" class="word_wrap_break"><p><?php echo $insert_by[$row[csf('inserted_by')]]; ?></p></td>
	...
	if ($type==3) // Show 3 Start
	{
		...
		<div>
	  	...
	  	<fieldset style="width:2610px;">
			...
			/*$sql_dtls="SELECT a.knitting_source, a.knitting_company, b.floor, a.recv_number, a.receive_date,a.location_id,b.order_id,b.batch_id,b.gsm,b.width,sum(b.grey_used_qty) as grey_used_qty, sum(b.receive_qnty) as qc_pass_qty, sum(b.reject_qty) as reject_qty, c.color_range_id, b.color_id,b.fabric_description_id,b.buyer_id , c.booking_no, c.booking_without_order, c.batch_no,c.extention_no,b.remarks 
			from inv_receive_master a, pro_finish_fabric_rcv_dtls b, pro_batch_create_mst c
			where  a.id=b.mst_id $lcCompany_cond and a.entry_form in(7,66) and b.batch_id = c.id and c.status_active =1 and a.item_category=2 and a.status_active=1 and a.is_deleted=0  $date_cond $unit_name $knitting_ref_cond  $all_booking_no_cond $batch_cond
			group by a.knitting_source, a.knitting_company, b.floor, a.recv_number,a.receive_date,a.location_id,b.batch_id,b.order_id,b.gsm,b.width, c.color_range_id, b.color_id,b.fabric_description_id,b.buyer_id , c.booking_no, c.booking_without_order, c.batch_no,c.extention_no,b.remarks
			order by b.batch_id, b.fabric_description_id, b.gsm";*/
			$sql_dtls="SELECT a.knitting_source, a.knitting_company, b.floor, a.recv_number, a.receive_date,a.location_id,b.order_id,b.batch_id,b.gsm,b.width,b.grey_used_qty, b.receive_qnty as qc_pass_qty, b.reject_qty, c.color_range_id, b.color_id,b.fabric_description_id,b.buyer_id , c.booking_no, c.booking_without_order, c.batch_no,c.extention_no,b.remarks,b.inserted_by 
			from inv_receive_master a, pro_finish_fabric_rcv_dtls b, pro_batch_create_mst c
			where  a.id=b.mst_id $lcCompany_cond and a.entry_form in(7,66) and b.batch_id = c.id and c.status_active =1 and a.item_category=2 and a.status_active=1 and a.is_deleted=0  $date_cond $unit_name $knitting_ref_cond  $all_booking_no_cond $batch_cond
			group by a.knitting_source, a.knitting_company, b.floor, a.recv_number,a.receive_date,a.location_id,b.batch_id,b.order_id,b.gsm,b.width,b.grey_used_qty, b.receive_qnty, b.reject_qty, c.color_range_id, b.color_id,b.fabric_description_id,b.buyer_id , c.booking_no, c.booking_without_order, c.batch_no,c.extention_no,b.remarks,b.inserted_by
			order by b.batch_id, b.fabric_description_id, b.gsm";
			//$row[csf('batch_id')]][$order_id][$row[csf('gsm')]][$row[csf('fabric_description_id')]
			// echo $sql_dtls; 
			...
			<!-- =============== Summary Start ======================== -->
			<div style="padding-bottom:100px;">
				<!-- === Color Range Wise Finish Production Summary Start === -->
			 	<div style="float: left; margin-left: 20px;">
				 	<table width="370" border="1" cellpadding="0" cellspacing="0" rules="all" class="rpt_table">
						<thead>
							<tr>
			 	...
			 	<!-- === Color Range Wise Finish Production Summary End ===== -->

			 	<!-- ===== Buyer Wise Finish Production Summary Start ======= -->
			 	<div style="float: left; margin-left: 20px;">
		            <table width="370" border="1" cellpadding="0" cellspacing="0" rules="all" class="rpt_table">
						<thead>
							<tr>
			 	...
			 	<!-- ===== Buyer Wise Finish Production Summary End ========= -->
			 	<br/>
			</div>
			<!-- =============== Summary End ========================== -->

			<!-- =============== Details Data Show Start ======================== -->
			<table  width="2840" border="1" rules="all" class="rpt_table" cellpadding="0" cellspacing="0" >
	        	<thead>
	                <th width="40">SL</th>
	                <th width="100">Working Company</th>
	                <th width="100">Working Floor</th>
	        ...
	        <div style="width:2880px; overflow-y:scroll; max-height:350px;" id="scroll_body">
	        <table width="2840" border="1" rules="all" class="rpt_table" cellpadding="0" cellspacing="0" id="tbl_dyeing3">
			...
			foreach($sql_dtls_result as $row)
			{
	            ...
	            <tr  bgcolor="<? echo $bgcolor; ?>"  onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
	                ...
	                <td width="80" class="word_wrap_break"><p><? if($row[csf('booking_without_order')]==1){ echo $nonOrdBooking_arr[$row[csf('booking_no')]]; }else{echo $int_ref_no;} ?></p></td>
	                <td width="80" align="right"><p><? echo number_format($order_qty_arr_m);?></p>  </td>
	                <td width="250" class="word_wrap_break" title="Detar ID: <?echo $row[csf("fabric_description_id")].'Product ID: '.$row[csf("fabric_description_id")];?>"><p><? echo $composition_arr[$row[csf('fabric_description_id')]];?></p> </td>
	                <td width="60"> <p><? echo $row[csf('gsm')];?></p> </td>
	                <td width="60"> <p><? echo $row[csf('width')];?></p> </td>
	                <td width="100" class="word_wrap_break"><p><? echo $color_range[$row[csf('color_range_id')]];?></p> </td>
	                <td width="100" class="word_wrap_break"><p><? echo $color_library[$row[csf('color_id')]];?></p> </td>
	                <td width="100" class="word_wrap_break" title="<?echo $row[csf("batch_id")];?>"><p><? echo $batch_no; ?></p>  </td>
	                <td width="100"><p><? echo $extention_no; ?></p>  </td>

	                <?
					$groupData_1 = $row[csf("batch_id")] . $row[csf("order_id")] . $row[csf("gsm")] . $row[csf("fabric_description_id")];
					if (!in_array($groupData_1, $date_array_1)) {
					?>
						<td width="80" rowspan="<? echo $gData_1[$groupData_1]; ?>" align="right" title="<? echo 'Batch ID:'.$row[csf('batch_id')]; ?>"> <p><? echo number_format($batch_qty,2);$total_batch_qty+=$batch_qty;?></p>
						<?
	                ...
	                <td width="80"  align="right" id="total_grey_qty"><p><? echo number_format($row[csf('grey_used_qty')],2);?></p>  </td>
	                <td width="80"  align="right" id="total_rj_qty"><p><? echo number_format($row[csf('reject_qty')],2);?></p>  </td>
					<td width="80" align="right" title="{(grey used qnty-qc pass qnty)*100}/grey used qnty">
						<p><? 
						//echo number_format($process_percent,2);
						if ($row[csf('grey_used_qty')]>0) 
						{
							echo number_format(($row[csf('grey_used_qty')]-$row[csf('qc_pass_qty')])*100/$row[csf('grey_used_qty')],2);
						}
						else 
						{
							echo "0.00";						
