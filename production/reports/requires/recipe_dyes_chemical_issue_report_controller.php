	...
	if($txt_date_from && $txt_date_to) 
	{
		...
		//	echo $all_req_ids.'b,';
			...
			$item_group_arr=return_library_array( "select id,item_name from lib_item_group where item_category in (5,6,7) and status_active=1 and is_deleted=0",'id','item_name');
			 ...
			 c.item_group_id, d.sub_process,d.requ_no,a.req_no,b.batch_lot
			  from inv_issue_master a, inv_transaction b, product_details_master c, dyes_chem_issue_dtls d
			  where a.id=b.mst_id and b.id =d.trans_id and d.product_id=c.id and b.prod_id=c.id and b.transaction_type=2 and a.entry_form=5 and b.item_category in (5,6,7) $req_cond_for_in2  group by d.recipe_id,a.batch_no,d.product_id,b.batch_lot, c.item_group_id,d.requ_no,a.req_no, d.sub_process order by d.sub_process "; 
			 // echo $sql_isue_dtls;die;
			  ...
			  foreach($sql_result_issue as $row)
			  {
				  ...
				  if($row[csf('batch_lot')]!="")
				  {
				  $issue_data_arr[$row[csf('req_no')]][$row[csf('sub_process')]][$row[csf('prod_id')]][$row[csf('item_group_id')]][$row[csf('recipe_id')]]['batch_lot'].=$row[csf('batch_lot')].',';} 
				  $issue_req_chk_arr[$row[csf('req_no')]][$row[csf('sub_process')]]=$row[csf('req_no')]; 
			  }
		...
		foreach($prevRatioData as $row)
		{
		   ...
		   $sql_dtls = "select a.id as req_id,a.requ_no,a.requisition_date,a.requisition_basis, a.batch_id, a.recipe_id, b.id,
			...
			$result_issue_req=sql_select($sql_dtls);
				...
				$issue_found=$issue_req_chk_arr[$row[csf("req_id")]][$row[csf("sub_process")]];
				...
				$issue_req_arr[$req_rec_data][$row[csf("sub_process")]][$row[csf("prod_id")]][$row[csf("item_group_id")]]['lot']=$row[csf("lot")];
				 }
				}
                                ...
                                <?
									...
									//echo $batch_id.'ddds';
									 ...
									 foreach($req_data as $sub_process_id=>$process_data)
										...
										$total_liquor=0;$liquor_ratio=0;
										foreach($recipe_ids as $rid)
										{
										$total_liquor+=$recipe_ratio_arr[$sub_process_id][$rid]['total_liquor'];
										$liquor_ratio+=$recipe_ratio_arr[$sub_process_id][$rid]['liquor_ratio'];
										}
										...
										if (!in_array($sub_process_id_chk,$sub_process_array) )
											{
												?>  <tr bgcolor="#CCCCCC">
                                                <? //if($r==1) {?>
												<td colspan="15"  align="center" rowspan="<? //echo $sub_req_rowspan_arr[$req_nos][$sub_process_id]; ?>"><strong><? echo $dyeing_sub_process[$sub_process_id].',Liqur Ratio: &nbsp;'.number_format($liquor_ratio,2).', Total Liquor: &nbsp; '.number_format($total_liquor,2); ?></strong></td>
                                                <?
												 // }
												?>
												</tr> 
										 ...
										 foreach($process_data as $prod_id=>$prod_data)
                               			 {
											   foreach($prod_data as $item_id=>$row)
												...
												$batch_lot=rtrim($issue_data_arr[$req_id][$sub_process_id][$prod_id][$item_id][$recipe_id]['batch_lot'],',');
												...
												$batch_lot = implode(", ",array_unique(explode(",",$batch_lot)));
                                           ...
                                           if (!in_array($requ_no,$req_chk_arr) )
											{ 
												$f++;//batch_weight
												?>
                                             <td style="word-break:break-all" rowspan="<? //echo $req_rowspan_arr[$req_nos]; ?>" align="center" width="" title="Batch ID=<? echo $batch_id; ?>"><p><? echo "Recipe No:".$recipe_id.', Req. No:'.$requ_no.', Batch No:'.$batchNos.', Batch Wgt:'.number_format($batch_weight,2).', FSO No:'.$sales_order_no.', Booking No:'.$booking_nos.', Batch Color:'.$color_name.', Color Range:'.$color_range_name; ?></p></td>
                                             ...
                                             ?>
                                               <td class="" width="30"><? echo $r; ?></td>
                                                ...
                                                <td class="" width="60" title=""><p><? echo $batch_lot; ?></p></td>
