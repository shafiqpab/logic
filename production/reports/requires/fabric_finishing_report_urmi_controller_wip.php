...
if($action=="report_generate")
{
	...
	$sales_cond="";$sales_cond2="";
	if($fso_no) $sales_cond=" and d.job_no like '%$fso_no'";
	if($fso_no) $sales_cond2=" and c.sales_order_no like '%$fso_no'";
	//echo  $sales_cond.'d';

	if($cbo_buyer_name>0) $buyer_cond=" and (e.po_buyer=$cbo_buyer_name or e.buyer_id=$cbo_buyer_name)";
	if($cbo_buyer_name>0) $buyer_cond2=" and e.buyer_id=$cbo_buyer_name";
	...
	{ 
		...
		$get_batch_ids_from_subprocess_chk = sql_select("select max(a.id) as ids,a.batch_id,a.entry_form,a.result,a.next_process_id from pro_fab_subprocess a  ,pro_fab_subprocess_dtls b where a.id=b.mst_id  and a.status_active=1  and b.status_active=1 $prod_date_cond  $batch_cond3 group by a.batch_id,a.entry_form,a.next_process_id,a.result order by ids desc");
		//echo "select max(a.id) as ids,a.batch_id,a.entry_form,a.result,a.next_process_id from pro_fab_subprocess a  ,pro_fab_subprocess_dtls b where a.id=b.mst_id  and a.status_active=1  and b.status_active=1 $prod_date_cond  $batch_cond3 group by a.batch_id,a.entry_form,a.next_process_id,a.result order by ids desc";
		...
		foreach($get_batch_ids_from_subprocess_chk as $row)
		{
			...
			$batch_sub_process_arr2[$row[csf("ids")]][$row[csf("batch_id")]]['result_id'] = $row[csf("next_process_id")];
				//$batch_id_arr[$row[csf("batch_id")]] = $row[csf("batch_id")];
		}
		...
		/* $get_batch_ids_from_subprocess_sql_load = "select a.id,a.load_unload_id,a.entry_form,a.result,b.id batch_id,b.re_dyeing_from,b.extention_no,d.within_group,d.id fso_id,d.buyer_id,d.po_buyer
		...
		where d.id=c.po_id and c.mst_id=b.id and c.status_active=1 and a.load_unload_id>0  and d.status_active=1 $sales_cond $batch_cond $comp_cond2 $batch_ext_cond $production_batch_id_cond order by a.id desc";//group by b.id,b.extention_no,d.within_group,d.id,d.buyer_id,d.po_buyer

		$get_batch_ids_from_subprocess_load = sql_select($get_batch_ids_from_subprocess_sql_load);
		$k=0;
		foreach($get_batch_ids_from_subprocess_load as $row)
		{
			$batch_id_arr[$row[csf("batch_id")]] = $row[csf("batch_id")];
			...
			if($row[csf("id")]!="")
				$sub_process_id_arr[$row[csf("id")]]=$row[csf("id")];//re_dyeing_from

			if($row[csf("load_unload_id")]==2)
			{
				//if($load==0){ //|| $row[csf("re_dyeing_from")]>0
			...
			{
				if($k==0){ //|| $row[csf("re_dyeing_from")]>0
					$batch_max_process[$row[csf("batch_id")]] = ($row[csf("id")]=="")?0:$row[csf("id")];
				//echo $row[csf("id")].'B,';
				}
			}
		  ...
		  $get_batch_ids_from_subprocess_sql = "select max(a.id) as id,b.id as batch_id
		from fabric_sales_order_mst d,pro_batch_create_dtls c,pro_batch_create_mst b
		left join pro_fab_subprocess a on b.id=a.batch_id and a.status_active=1 $prod_comp_cond
		where d.id=c.po_id and c.mst_id=b.id and c.status_active=1  and d.status_active=1  $sales_cond $batch_cond $comp_cond2 $batch_ext_cond $production_batch_id_cond group by b.id order by id desc";//group by b.id,b.extention_no,d.within_group,d.id,d.buyer_id,d.po_buyer
		...
		foreach($get_batch_ids_from_subprocess as $row)
		{
			...
			$entry_formId=$batch_sub_process_arr2[$row[csf("id")]][$row[csf("batch_id")]]['entry_form'];
			$resultId=$batch_sub_process_arr2[$row[csf("id")]][$row[csf("batch_id")]]['result'];
			$process_resultId=$batch_sub_process_arr2[$row[csf("id")]][$row[csf("batch_id")]]['result_id'];
			
			$batch_sub_process_arr3[$row[csf("batch_id")]]['entry_form']=$entry_formId;
			$batch_sub_process_arr3[$row[csf("batch_id")]]['result']=$resultId;
			$batch_sub_process_arr3[$row[csf("batch_id")]]['process_result']=$process_resultId;
	...
	if($fso_no)
	{
		// $sales_cond2=" and c.sales_order_no like '%$fso_no'";
		  $get_batch_ids_from_subprocess_sql = "select max(a.id) as id,b.id as batch_id
		from fabric_sales_order_mst d,pro_batch_create_dtls c,pro_batch_create_mst b
		left join pro_fab_subprocess a on b.id=a.batch_id and a.status_active=1 $prod_comp_cond
		where d.id=c.po_id and c.mst_id=b.id and c.status_active=1  and d.status_active=1  $sales_cond $batch_cond $comp_cond2 $batch_ext_cond $production_batch_id_cond group by b.id order by id desc";
		$get_batch_ids_from_subprocess = sql_select($get_batch_ids_from_subprocess_sql);
			$i=0;
			foreach($get_batch_ids_from_subprocess as $row)
			{
				...
				if($row[csf("id")]=="")
				{
					//echo $row[csf("batch_id")].'='.$row[csf("id")].'<br>';
					if(in_array(33, $batch_process))
					{
						//$batch_id_arr[$row[csf("batch_id")]] = $row[csf("batch_id")];
					}
	...
	//echo $batch_id_cond.'='.$batch_cond2.'='.$batch_date_cond2;
	...
	from pro_batch_create_dtls d,pro_batch_create_mst c,fabric_sales_order_mst e
	where d.mst_id=c.id and e.id=d.po_id and c.sales_order_id=e.id and c.is_sales=1 and c.status_active=1 and d.status_active=1 and e.status_active=1 $batch_id_cond $batch_cond2 $batch_date_cond2 $batch_year_cond $comp_cond $batch_ext_cond2 $sales_cond2 $buyer_cond  $floor_cond
...
//echo $batch_cond_for_in.'FDF';
	...
	//echo "SELECT a.id,a.extention_no,a.re_dyeing_from from pro_batch_create_mst a where re_dyeing_from>0 $batch_cond_for_ext";
	...
	{
	      $dyeing_sql="SELECT a.id as mst_id,a.service_company,a.floor_id,a.entry_form,a.service_source,a.insert_date,a.remarks,a.batch_id, a.result,a.next_process_id,a.batch_no,a.process_id,a.entry_form,a.start_hours,a.start_minutes,a.production_date as end_date,a.load_unload_id, a.batch_ext_no,a.process_start_date,a.process_end_date,a.end_minutes,a.end_hours,b.production_qty,b.prod_id,b.no_of_roll,b.width_dia_type from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.status_active=1 and b.status_active=1 $batch_cond_for_in  order by a.id desc";
		$dyeing_data = sql_select($dyeing_sql);
		$cum_prod_qnty = 0;$next_process_arr_check_show=array(4,17,18,19,20,100);
		foreach($dyeing_data as $row )
		{
			//echo $row[csf("batch_id")].'='.$row[csf("production_qty")].'<br>';
			...
			else $process_result=$row[csf("next_process_id")];
			
			/*if($row[csf("next_process_id")]==12)
			{
				//If found Stentering...
				//echo $results.'x=<br>';
				$process_result= ($cbo_type>0 && $cbo_type!=$row[csf("next_process_id")])?"":12;
			}
			else if($row[csf("entry_form")]==48 && (in_array($row[csf("result")],$next_process_arr_check_show) && $row[csf("next_process_id")]==0))
			{
				//If found Stentering...
				$process_result= ($cbo_type>0 && $cbo_type!=$row[csf("next_process_id")])?"":12;
			}*/
			
		if($row[csf("next_process_id")]==13)
		{
			//If found Drying...
			$process_result= ($cbo_type>0 && $cbo_type!=$row[csf("next_process_id")])?"":13;
		}
		else if($row[csf("entry_form")]==31 && (in_array($row[csf("result")],$next_process_arr_check_show) && $row[csf("next_process_id")]==0))
		{
			//If found Drying...
			$process_result= ($cbo_type>0 && $cbo_type!=$row[csf("next_process_id")])?13:13;
			//$process_result=13;
		...
		if($row[csf("next_process_id")]==12)
		{
	 		//If found Stentering...
			//echo $results.'x=<br>';
			//echo "A".$row[csf("entry_form")];
			$process_result= ($cbo_type>0 && $cbo_type!=$row[csf("next_process_id")])?"":12;
		}
		else if($row[csf("entry_form")]==48 && (in_array($row[csf("result")],$next_process_arr_check_show) && $row[csf("next_process_id")]==0))
		{
			//If found Stentering...
			//echo  "B".$row[csf("entry_form")];
			 
			$process_result= ($cbo_type>0 && $cbo_type!=$row[csf("next_process_id")])?12:12;
			 
			//echo $process_result;
		}
		
		if($row[csf("next_process_id")]==14)
		{
	 		//If found Compacting...
			$process_result= ($cbo_type>0 && $cbo_type!=$row[csf("next_process_id")])?"":14;
		}
		//else if($sub_entry_form2==33 && (in_array($results_check,$next_process_arr_check_show) && $results==0) )
		else if($row[csf("entry_form")]==33 && (in_array($row[csf("result")],$next_process_arr_check_show) && $row[csf("next_process_id")]==0) )
		{
			//If found Compacting...
			$process_result= ($cbo_type>0 && $cbo_type!=$row[csf("next_process_id")])?14:14;
			//$process_result=14;
		}
		
		if($row[csf("next_process_id")]==15)
		{
			//If found Brush...
			$process_result= ($cbo_type>0 && $cbo_type!=$row[csf("next_process_id")])?"":15;
		}
		else if($row[csf("entry_form")]==34 && in_array(68,$prod_process_id)  && (in_array($row[csf("result")],$next_process_arr_check_show) && $results==0))
		{
			//If found Brush...
			$process_result= ($cbo_type>0 && $cbo_type!=$row[csf("next_process_id")])?15:15;
			//$process_result=15;
		}
		else if($row[csf("next_process_id")]==16)
		{
	 		//If found Peach..
			$process_result= ($cbo_type>0 && $cbo_type!=$row[csf("next_process_id")])?"":16;
		}
		else if($row[csf("entry_form")]==34  && in_array(67,$prod_process_id) && (in_array($row[csf("result")],$next_process_arr_check_show) && $row[csf("next_process_id")]==0))
		{
			//If found Peach...
			
			$process_result= ($cbo_type>0 && $cbo_type!=$row[csf("next_process_id")])?16:16;
			//$process_result=16;
		}
			...
			//if($row[csf("prod_id")]==102875)
				...
				{
					//if($row[csf("batch_id")]==64711) { //echo $row[csf("production_qty")].'='.$process_result.','; else echo '';
				$subpro_prod_qty_arr[$row[csf("mst_id")]][$process_result][$row[csf("batch_id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]+=$row[csf("production_qty")];
					//} 
				}
			//echo $process_result.'='.$row[csf("end_date")].'<br>';
			$subpro_prod_arr[$row[csf("mst_id")]][$process_result][$row[csf("batch_id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['floor_id']=$row[csf("floor_id")];
			$subpro_prod_arr[$row[csf("mst_id")]][$process_result][$row[csf("batch_id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['service_source']=$row[csf("service_source")];
			$subpro_prod_arr[$row[csf("mst_id")]][$process_result][$row[csf("batch_id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['process_end_date']=$row[csf("end_date")];
			$subpro_prod_arr[$row[csf("mst_id")]][$process_result][$row[csf("batch_id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['remarks']=$row[csf("remarks")];
			$subpro_prod_arr[$row[csf("mst_id")]][$process_result][$row[csf("batch_id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['no_of_roll']=$row[csf("no_of_roll")];
			$subpro_prod_arr[$row[csf("mst_id")]][$process_result][$row[csf("batch_id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['hr_min']=$row[csf("end_hours")].':'.$row[csf("end_minutes")];
			 
			
			$result_id_arr[$row[csf("mst_id")]] = $row[csf("next_process_id")];
			$result_check_arr[$row[csf("mst_id")]] = $row[csf("result")];
			$result_id_entry_form_arr[$row[csf("mst_id")]] = $row[csf("entry_form")];
			$process_id_form_arr[$row[csf("mst_id")]] = $row[csf("process_id")];//$row[csf("process_id")]
			
			if($row[csf("load_unload_id")]==2 || $row[csf("load_unload_id")]==1)
			{
	...
	$entry_form_chk=array(32 => '33',30 => '30', 31 => '13' , 48 => "12", 33 => '14',  34 => '15', 34 => '16');
	
	$chk_batch_qty=array();$prod_qty_arr=array();
	$subpro_prod_qty=0;$next_process_arr_check_show=array(4,17,18,19,20,100);
	foreach($result_batch as $row)
	{
		$process_ids=explode(",",$row[csf("process_id")]);
		$batch_max_process_id = $batch_max_process[$row[csf("id")]];
		
		//echo $batch_max_process_id.'<br>';
		$results = $result_id_arr[$batch_max_process_id];
		$process_id_prod = explode(",",$process_id_form_arr[$batch_max_process_id]);//$process_id_form_arr[$row[csf("mst_id")]]
		$results_check = $result_check_arr[$batch_max_process_id];//$result_check_arr[$row[csf("mst_id")]]
		//if($results_check) $results_check=$results_check;else $results_check=0;
		//if($results) $results=$results;else $results=0;
		
		
		$load_unload = $load_unload_arr[$batch_max_process_id];
		$is_shade_matched = $load_unload_shade_matched[$row[csf("id")]];

		$entry_form_id = $result_id_entry_form_arr[$batch_max_process_id];
		$has_extension = $batch_ext[$row[csf("id")]];
		
		$sub_entry_form=$batch_sub_process_arr[$row[csf("id")]]['entry_form'];
		 $sub_entry_form2=$batch_sub_process_arr3[$row[csf("id")]]['entry_form'];
		//echo $batch_max_process_id.'='.$entry_form_id.'='.$results.'='.$results_check.'<br>'; 
		
		//echo $process_id.'DD';
		$sub_result_id2=0;
		if($batch_sub_process_arr3[$row[csf("id")]]['result'])
		{
		
		$sub_result_id2=$batch_sub_process_arr3[$row[csf("id")]]['result'];
			//echo $row[csf("id")].'='.$sub_result_id2.'<br>';
		}
		$sub_result_id=$batch_sub_process_arr[$row[csf("id")]]['result'];
		//echo $results_check .'='.$results.'='.$load_unload.'='.$row[csf("id")].'<br>';die;
		 if(($cbo_type==0 || $cbo_type==30) && ($load_unload==2 && ($results_check==1 || $results_check==11 || $results_check==0)))
		{
			$process_id= 30;
			//echo $load_unload.'='.$process_id.'='.$results.',';
		}
		else  if(($cbo_type==0 || $cbo_type==30) && ($entry_form_id==30 && in_array($results_check,$next_process_arr_check_show) && $results==0))
		{
			$process_id= 30;
			//echo $load_unload.'AA'.$process_id.'='.$results.',';
		}
		else if(($cbo_type==0 || $cbo_type==33) && ($results_check==0 || $results_check=='' ) && $entry_form_id=="")
		{
	 		//If found Heatset..
			if(in_array(33, $process_ids) && $has_extension==""){
				$process_id= 33;
				$entry_form_id=32;
				//echo $row[csf("id")].'A'.$results_check.'='.$entry_form_id.'<br>';
			}
		} 
		else if(($cbo_type==0 || $cbo_type==33) && ($entry_form_id==32  && in_array($results_check,$next_process_arr_check_show) && $results==0))
		{
	 		//If found Heatset.. for Result Select  
			//echo $entry_form_id.'H'.$process_ids.'<br>';
		//	print_r($process_ids);
			if(in_array(33, $process_ids) && $has_extension==""){
				$process_id= 33;
				$entry_form_id=32;
				//echo $row[csf("id")].'B'.$results_check.'='.$entry_form_id.'<br>';
			}
		} 
		else $process_id=0; 
		//echo $process_id.'A'.$entry_form_id.'='.$results_check.'='.$results.'<br>'; 
		if($results==13)
		{
			//If found Drying...
			$process_id= ($cbo_type>0 && $cbo_type!=$results)?"":13;
		}
		else if($entry_form_id==31 && (in_array($results_check,$next_process_arr_check_show) && $results==0))
		{
			//If found Drying...
			if($cbo_type>0)
			{
				 $chkprocess_id=$cbo_type;
			}
			else
			{
				 $chkprocess_id="";
			}
			$process_id= ($cbo_type>0 && $cbo_type!=$results)?13:13;
			//$process_id=13;
		...
		if($results==12)
		{
			...
			//echo $process_id.'='.$cbo_type.'='.$results.'x<br>';
		//	if($row[csf("id")]=='96567') echo "P=".$row[csf("id")];else echo " ";
		}
		else if($entry_form_id==48 && (in_array($results_check,$next_process_arr_check_show) && $results==0))
		{
			//If found Stentering...
			//echo $results_check.'<br>';
			...
			$process_id= ($cbo_type>0 && $cbo_type!=$results)?12:12;
			//if($row[csf("id")]=='96567') echo "X=".$results.'='.$process_id;else echo " ";
			//$process_id=12;
			//if($row[csf("id")]=='96567')  echo $process_id.'='.$cbo_type.'='.$results.'<br>'; else echo " ";
		}
		...
		//else if($sub_entry_form2==33 && (in_array($results_check,$next_process_arr_check_show) && $results==0) )
		else if($entry_form_id==33 && (in_array($results_check,$next_process_arr_check_show) && $results==0) )
		{
			//If found Compacting...
			//echo $row[csf("id")]."<br>";
			//$process_id=14;
			...
			$process_id= ($cbo_type>0 && $cbo_type!=$results)?14:14;
			//$process_id=14;
		}
		//if(in_array(33, $process_ids) && $has_extension==""){
				//$process_id= 33;
				//$entry_form_id=32;
				//echo $row[csf("id")].'B'.$results_check.'='.$entry_form_id.'<br>';
			 //echo $process_id_prod.'='.$entry_form_id.'='.$results.'='.$results_check;
		if($results==15)
		{
			//If found Brush...
			$process_id= ($cbo_type>0 && $cbo_type!=$results)?"":15;
		}
		else if($entry_form_id==34 && in_array(68,$process_id_prod)  && (in_array($results_check,$next_process_arr_check_show) && $results==0))
		{
			//If found Brush...
			if($cbo_type>0)
			{
			...
			$process_id= ($cbo_type>0 && $cbo_type!=$results)?15:15;
			//$process_id= 15;
		}
		else if($results==16)
		{
		...
		else if($entry_form_id==34  && in_array(67,$process_id_prod) && (in_array($results_check,$next_process_arr_check_show) && $results==0))
		{
			//If found Peach...
			if($cbo_type>0)
			{
			...
			$process_id= ($cbo_type>0 && $cbo_type!=$results)?16:16;
			 //$process_id= 16;
		}
		//echo $process_id.'DD'.$row[csf("id")].'<br>';
		//if($cbo_type>0) $process_id=$cbo_type;else $process_id=0;
		//if($cbo_type>0) { $process_id=$cbo_type;}
		//else {$process_id="";$results="";}
		
		if($process_id!="" ){
			if($process_id==33 || $process_id==30 )
			{
			...
			else
			{
				if($cbo_type == 0)
				{
					$subpro_prod_qty=$subpro_prod_qty_arr[$batch_max_process_id][$process_id][$row[csf("id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]];
				}else
				{
					if($process_id==$cbo_type)
					{
						$subpro_prod_qty=$subpro_prod_qty_arr[$batch_max_process_id][$process_id][$row[csf("id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]];
					}
				}
				
				//echo $subpro_prod_qty.'='.$batch_max_process_id.'='.$process_id.'<br>';
			}

			$hr_min = $subpro_prod_arr[$batch_max_process_id][$process_id][$row[csf("id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['hr_min'];
			...
			else
			{
				...
				$prod_remarks 		= $subpro_prod_arr[$batch_max_process_id][$process_id][$row[csf("id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['remarks'];
				$process_end_date 	= $subpro_prod_arr[$batch_max_process_id][$process_id][$row[csf("id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['process_end_date'];
				$no_of_roll 		= $subpro_prod_arr[$batch_max_process_id][$process_id][$row[csf("id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['no_of_roll'];
				$floor_id 			= $subpro_prod_arr[$batch_max_process_id][$process_id][$row[csf("id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['floor_id'];
				//echo $process_id .'='.$no_of_roll.', ' ;
				//if($process_id==12) echo $row[csf("id")].'='.$process_end_date.'<br>';else echo "No";
			}
			...
			$service_source=$subpro_prod_arr[$batch_max_process_id][$process_id][$row[csf("id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['service_source'];
			if($cbo_type == 0)
			{
				...
				$batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]["width_dia_type"]=$row[csf("width_dia_type")];
				$batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]["result"]=$results_check;
				$batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]["no_of_roll"]=$no_of_roll;
			...
			$dyeing_result_arr = array(1,0);
			$next_process_arr_check=array(0,4,11);
			$next_process_arr_check_not=array(4,17,18,19,20,100);
			//echo $results_check.'='.$row[csf("id")].'<br> ';
			//if($results_check==0) $results_check=11;
			//if($results==0) $results=11;
			
			//echo $results_check.'='.$results.'='.$process_id.'='.$load_unload.'='.$entry_form_id.'='.$row[csf("id")].'<br>';
			if($load_unload==1 && $entry_form_id==35 && in_array(33, $process_ids))
			{
					//echo $load_unload.'='.$entry_form_id.', ';
				unset($batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]);
			}
			else if(($cbo_type==0 || $cbo_type==33) && $results_check==0 && $entry_form_id=="")//&& $entry_form_id==35
			{
				// echo 'h1';
				if(in_array(33, $process_ids) && $has_extension==""){
					unset($batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]);
			...
			else if(($cbo_type==0 || $cbo_type==33) && (in_array($sub_result_id2,$next_process_arr_check_not) && $results>0) && $sub_entry_form2==32)//HeatSet Complete Found
			{
				//echo $sub_entry_form2.'='.$sub_result_id2.'<br>';
				//echo "H2";
				if($sub_entry_form2)
				{
					//echo $row[csf("id")]."=D".$sub_entry_form2;
				if(in_array(33, $process_ids)){
					unset($batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]);
				}
				}
			}
			else if((in_array($results_check,$next_process_arr_check_not)) && $entry_form_id!=35 && ($entry_form_id) ){
				//echo  $row[csf("id")].'p'.$results_check.'='.$entry_form_id.'<br>';
				//unset($batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]);
			}
			else if((in_array($sub_result_id2,$next_process_arr_check_not) && $sub_result_id2>0 && $results>0) && $sub_entry_form2!=35 ){
				//echo $sub_result_id2.'g'.$row[csf("id")].'<br>';
				unset($batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]);
			}
			...
			else if($load_unload==2 && $entry_form_id==35 && (!in_array($results_check, $dyeing_result_arr)))
			{
					//echo $sub_result_id2.'X'.$row[csf("id")].'<br>';
				unset($batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]);
			}
			else if($load_unload==2 && $entry_form_id==35 && in_array($results_check, $dyeing_result_arr))
			{
					//echo $sub_result_id2.'C'.$row[csf("id")].'<br>';
			...
			if((in_array($results_check,$next_process_arr_check) && $results>0) && $entry_form_id==34 ){
				
				//echo $results_check.'T'.$row[csf("id")].'<br>';
				//unset($batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]);
			}
			if($results>0 && (in_array($results_check,$next_process_arr_check_not)) && $entry_form_id){
				
				//echo $results_check.'X'.$row[csf("id")].'<br>';
				//echo $row[csf("id")].'='."H2";
				unset($batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]);
			}
	...
	{
		if($rpt_type==0) $width=2850;else $width=500;
		?>
		<div>
			<style type="text/css">
				.alignment_css
			...
			<fieldset style="width:<? echo $width;?>px;">
				...
				if($rpt_type!=0)
				{
								...
								if(!empty($batch_data)){
									...
									foreach($batch_data as $batch_id=>$prod_data)
									{
										foreach($prod_data as $prod_id=>$row)
										{
											...
											$summary_wip_arr[$page_from_id]["no_of_roll"]+=$row["no_of_roll"];
											$summary_wip_arr[$page_from_id]["batch_qty"]+=$row["batch_qty"];
											$summary_wip_arr[$page_from_id]["batch_no"].=$batch_id.',';
										}
									}
							...
							<table align="left" class="rpt_table" id="table_body_sammary" width="490" cellpadding="0" cellspacing="0" border="1" rules="all">
								<tbody>
									...
									foreach($summary_wip_arr as $process_id=>$row)
									{
										...
										$batch_no=rtrim($row["batch_no"],',');
										$batch_nos_arr=array_unique(explode(",",$batch_no));
					...
					?>
					<table class="rpt_table" width="2860" cellpadding="0" cellspacing="0" border="1" rules="all" align="left">
						...
						<thead>
							<tr>
                                ...
                                <th width="100" class="alignment_css">Result</th>
								<th width="" class="alignment_css">Remarks</th>
							</tr>
						</thead>
					</table>
					<div style=" max-height:380px; width:2880px; overflow-y:scroll;" id="scroll_body">
						<table align="left" class="rpt_table" id="table_body" width="2860" cellpadding="0" cellspacing="0" border="1" rules="all">
							<tbody>
								<?
									...
									if(!empty($batch_data)){
										...
										foreach($batch_data as $batch_id=>$prod_data)
										{
											foreach($prod_data as $prod_id=>$row)
											{
												...
												?>
												<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $ii.$page_from_id; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $ii.$page_from_id; ?>">
													...
													<td width="80" title="<? echo $page_from_id;?>" class="alignment_css">
													...
													<td width="80" title="<? echo $unload_date_time.'='.$new_date_time_end;?>" class="alignment_css">
														 <?
                                                    ...
                                                    <td width="100" class="alignment_css"><? echo $dyeing_result[$row[('result')]] ;?></td>
...
if($action=="report_generate_not") //Not used 20-8-2020
{
	$process = array( &$_POST );
	extract(check_magic_quote_gpc( $process ));
	$all_condition="";
	...
	if($txt_date_from && $txt_date_to)
	{
		...
		foreach($get_batch_ids_from_subprocess_load as $row)
		{
		 ...
		 $get_batch_ids_from_subprocess_sql = "select a.id,a.load_unload_id,a.entry_form,a.result,b.id batch_id,b.extention_no,d.within_group,d.id fso_id,d.buyer_id,d.po_buyer
		from fabric_sales_order_mst d,pro_batch_create_dtls c,pro_batch_create_mst b
		left join pro_fab_subprocess a on b.id=a.batch_id and a.status_active=1 $prod_comp_cond
		where d.id=c.po_id and c.mst_id=b.id and c.status_active=1 and a.load_unload_id=0 and d.status_active=1 $sales_cond $batch_cond $comp_cond2 $batch_ext_cond $production_batch_id_cond order by a.id desc";//group by b.id,b.extention_no,d.within_group,d.id,d.buyer_id,d.po_buyer
...
if($action=="report_generate2")//Not used
{
	$process = array( &$_POST );
	extract(check_magic_quote_gpc( $process ));
	$all_condition="";
...
if($action=="report_generate222")//Not used
{
	$process = array( &$_POST );
	extract(check_magic_quote_gpc( $process ));
	$all_condition="";
	...
	if($txt_date_from && $txt_date_to)
	{
		...
		from pro_batch_create_dtls c,pro_batch_create_mst b
		left join pro_fab_subprocess a on b.id=a.batch_id and a.status_active=1 $prod_comp_cond
		where c.mst_id=b.id and c.status_active=1 and d.status_active=1 $batch_cond $comp_cond2 $batch_ext_cond $production_batch_id_cond order by a.id desc";//group by b.id,b.extention_no,d.within_group,d.id,d.buyer_id,d.po_buyer
		...
		where d.id=c.po_id and c.mst_id=b.id and c.status_active=1 and d.status_active=1 $sales_cond $batch_cond $comp_cond2 $batch_ext_cond
		order by a.id desc";*///group by b.id,b.extention_no,b.process_id,d.within_group,d.id,d.buyer_id,d.po_buyer
	$get_batch_ids_from_subprocess_sql = "select a.entry_form,a.id,b.id batch_id,b.extention_no
		from pro_batch_create_dtls c,pro_batch_create_mst b
		left join pro_fab_subprocess a on b.id=a.batch_id  and a.status_active=1 $prod_comp_cond
		where c.mst_id=b.id   and b.entry_form=36  and c.status_active=1 and b.status_active=1  $batch_cond $batch_ext_cond $sub_buyer_cond2 $production_batch_id_cond order by a.id desc";
...
*/
	...
	$sub_po_sql = "select a.id,b.id batch_id,b.extention_no,b.process_id,e.within_group,d.cust_style_ref,d.order_no,e.subcon_job,d.id po_id,e.party_id as buyer_id,c.prod_id,c.batch_qnty
		from subcon_ord_mst e,subcon_ord_dtls d,pro_batch_create_dtls c,pro_batch_create_mst b
		left join pro_fab_subprocess a on b.id=a.batch_id  and a.status_active=1
		where  d.job_no_mst=e.subcon_job and d.id=c.po_id and c.mst_id=b.id   and b.entry_form=36 and c.status_active=1 and d.status_active=1  $batch_cond $batch_ext_cond $sub_buyer_cond2 $batch_cond_for_in2
		order by a.id desc";
		foreach(sql_select($sub_po_sql) as $v)
		{
			$subcon_job_arr[$v[csf("batch_id")]]["style_ref_no"]=$v[csf("cust_style_ref")];
