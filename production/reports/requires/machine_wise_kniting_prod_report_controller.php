...
if($action=="report_generate")
{
	...
	$cbo_machine_status=str_replace("'","",$cbo_machine_status);
	...
	<div>
        <table width="<? echo $table_width; ?>" cellpadding="0" cellspacing="0" id="caption" align="center">
            <tr>
               <td align="center" width="100%" colspan="7" class="form_caption" ><strong style="font-size:16px"><? echo $company_library[$cbo_company]; ?></strong></td>
            </tr> 
            <tr>  
               <td align="center" width="100%" colspan="7" class="form_caption" ><strong style="font-size:13px"><? echo $report_title;  echo "; From : ".change_date_format(str_replace("'","",$txt_date_from))." To : ".change_date_format(str_replace("'","",$txt_date_to))."" ;?></strong></td>
            </tr>
        </table>
        <?
		...
		if (str_replace("'","",$cbo_location_id)==0) $location_cond=""; else $location_cond=" and b.location_id=$cbo_location_id";
		if (str_replace("'","",$txt_machine_id)==0 || str_replace("'","",$txt_machine_id)=='') $machine_cond=""; else $machine_cond=" and b.id in ( $machine_name )";
		if ($floor_name==0 || $floor_name=='') $floor_cond=""; else $floor_cond=" and b.floor_id in ( $floor_name )";
		...
		$floor_name=str_replace("'","",$cbo_floor_id);
		if ($cbo_location==0) $location_id =""; else $location_id =" and a.knitting_location_id=$cbo_location ";
		if ($cbo_location==0) $location_id2 =""; else $location_id2 =" and a.location_id=$cbo_location ";
		...
		$con = connect();
		execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_id and entry_form in (135)");
		oci_commit($con);

		// Knitting Production
		$sql_result="SELECT a.receive_date, b.machine_no_id, b.floor_id, sum(b.grey_receive_qnty) as grey_receive_qnty from inv_receive_master a, pro_grey_prod_entry_dtls b 
		where a.id=b.mst_id and a.entry_form=2 and a.item_category=13 and a.knitting_source=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.machine_no_id!=0 and a.knitting_company=$cbo_company $location_id $machine_no $receive_date 
		group by a.receive_date, b.machine_no_id, b.floor_id ";
		//echo $sql_result;
		...
		foreach ($sql_dtls as $row)
		{
			...
			$mc_id_arr[$row[csf('machine_no_id')]]=$row[csf('machine_no_id')];
		}
		
		// SubCon Knitting Production
		$sql_result_subcon="SELECT a.product_date, b.machine_id, sum(b.product_qnty) as product_qnty 
		from subcon_production_mst a, subcon_production_dtls b 
		where a.id=b.mst_id and a.product_type=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.machine_id!=0 and a.knitting_company=$cbo_company $location_id2 $machine_no_sub $receive_date_sub group by a.product_date, b.machine_id ";
		//echo $sql_result_subcon;
		...
		foreach ($sql_dtls_sub as $row)
		{
			...
			$mc_id_arr[$row[csf('machine_id')]]=$row[csf('machine_id')];
		}
		//var_dump ($date_data_sub_arr);
		fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 135, 1,$mc_id_arr, $empty_arr);
		...
		// Machine Entry
		if ($cbo_machine_status==1) // Production > j gulo production hoyce sei machine
		{
			$sql_machine_dtls="SELECT b.id, b.machine_no, b.brand, b.origin, b.prod_capacity, b.dia_width, b.floor_id 
			from GBL_TEMP_ENGINE g, lib_machine_name b
			where g.ref_val=b.id and g.user_id=$user_id and g.entry_form=135 and g.ref_from=1 and b.category_id=1 and b.company_id=$cbo_company_id and b.status_active=1 and b.is_deleted=0 $location_cond $floor_cond $machine_cond order by b.seq_no";
		}
		elseif ($cbo_machine_status==2) // Destruction > j gulo production hoy nai sei machine
		{
			$sql_machine_dtls="SELECT b.id, b.machine_no, b.brand, b.origin, b.prod_capacity, b.dia_width, b.floor_id 
			from lib_machine_name b
			where b.category_id=1 and b.company_id=$cbo_company_id and b.id not in(select g.ref_val from GBL_TEMP_ENGINE g where g.user_id=$user_id and g.entry_form=135 and g.REF_FROM=1) and b.status_active=1 and b.is_deleted=0 $location_cond $floor_cond $machine_cond order by b.seq_no";
		}
		else // All
		{
			$sql_machine_dtls="SELECT b.id, b.machine_no, b.brand, b.origin, b.prod_capacity, b.dia_width, b.floor_id 
			from lib_machine_name b
			where b.category_id=1 and b.company_id=$cbo_company_id and b.status_active=1 and b.is_deleted=0 $location_cond $floor_cond $machine_cond order by b.seq_no";
		}		
		...
		foreach($sql_machine as $row)//$machine_id=>$machine_val 
		{
			...
			<tr bgcolor="<? echo $bgcolor; ?>" style="height:35px;" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
				...
				<td width="80" title="<?=$machine_id;?>"><p><? echo $machine_no;?></p></td>
				<td width="70"><p><? echo $floor_library[$machine_floor]; ?></p></td>
				<td width="60"><p><? echo $machine_dia; ?></p></td>
