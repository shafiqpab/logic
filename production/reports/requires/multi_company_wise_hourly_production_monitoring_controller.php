...
{
	// $buttonIdArr = ['108#show', '893#line_wise_summary', '894#production_summary', '895#monthly_production_report', '896#monthly_production_summary', '897#monthly_production_summary2', '898#target_vs_achievement', '899#sewing_wip', '901#cm_wise_production_summary'];
	// $print_report_format_arr = get_report_button_array($data, 7, 308, $user_id,$buttonIdArr);
...
if($action=="print_button_setting")
{
	//echo $data;die;
	$buttonIdArr = ['108#show', '893#line_wise_summary', '894#production_summary', '895#monthly_production_report', '896#monthly_production_summary', '897#monthly_production_summary2', '898#target_vs_achievement', '899#sewing_wip', '901#cm_wise_production_summary'];
	$print_report_format_arr = get_report_button_array($data, 7, 308, $user_id,$buttonIdArr);
...
if($action=="report_generate") //2nd Button Start...
{
	...
	$hour=$start_time[0]*1; $minutes=$start_time[1]; $last_hour=23;
	...
	foreach(sql_select($dataArray_sql) as $val)
	{
		...
		$prod_resource_array[$val[csf('company_id')]][$val[csf('location_id')]][$val[csf('floor_id')]][$val[csf('id')]]['iron_man']=$val[csf('iron_man')];
		$prod_resource_array[$val[csf('company_id')]][$val[csf('location_id')]][$val[csf('floor_id')]][$val[csf('id')]]['qi']=$val[csf('qi')];
				...
				foreach($lo_name as $f_id=>$fname)
				{
					...
					//***************************************************************************************************************************
						...
						if($cbo_no_prod_type==2 && $line_production_hour>0)
						{
							...
							$helper=($resource_data['helper'] + $resource_data['iron_man'] + $resource_data['qi']);
						...
						if($cbo_no_prod_type==1)
						{
							...
							$floor_man_power+=$resource_data['man_power'];
							$floor_operator+=$resource_data['operator'];
							$floor_helper+=($resource_data['helper'] + $resource_data['iron_man'] + $resource_data['qi']);
							$floor_tgt_h+=$resource_data['terget_hour'];
							$floor_working_hour+=$resource_data['working_hour'];
							$eff_target_floor+=$eff_target;
							...
							$total_operator+=$resource_data['operator'];
							$total_man_power+=$resource_data['man_power'];
							$total_helper+=($resource_data['helper'] + $resource_data['iron_man'] + $resource_data['qi']);
							$total_capacity+=$resource_data['capacity'];
							$total_working_hour+=$resource_data['working_hour'];
							$gnd_total_tgt_h+=$resource_data['terget_hour'];
							$grand_total_terget+=$eff_target;
							...
							$company_operator+=$resource_data['operator'];
							$company_man_power+=$resource_data['man_power'];
							$company_helper+=($resource_data['helper'] + $resource_data['iron_man'] + $resource_data['qi']);
							$company_capacity+=$resource_data['capacity'];
							$company_working_hour+=$resource_data['working_hour'];
							$company_total_tgt_h+=$resource_data['terget_hour'];
							$company_total_terget+=$eff_target;
			...
			<table id="table_header_2" class="rpt_table" width="2300" cellpadding="0" cellspacing="0" border="1" rules="all">
				<thead>
					<tr height="50">
						...
						<th width="50"><p>Helper/Iron-Man/Qi</p></th>
			...
			<table id="table_header_1" class="rpt_table" width="<?echo $width_cal; ?>" cellpadding="0" cellspacing="0" border="1" rules="all">
				<thead>
					<tr height="50">
						...
						<th style="word-wrap:break-word; word-break: break-all;" width="50"><p>Helper/Iron-Man/Qi</p></th>
...
if($action=="report_generate_new")
{
	...
	else if ($type == 4) // Monthly Production Summary
	{
		...
		$production_sql = " SELECT c.production_date, c.serving_company, c.production_type, c.sewing_line, c.prod_reso_allo, a.buyer_name, a.style_ref_no,a.job_no, a.set_smv,a.total_set_qnty, a.avg_unit_price,c.item_number_id,
							SUM(CASE WHEN c.production_type=5 AND d.production_type=5 THEN d.production_qnty ELSE 0 END) AS sewing_output
							FROM wo_po_details_master a, wo_po_break_down b, pro_garments_production_mst c, pro_garments_production_dtls d
							WHERE
							    a.id = b.job_id
							...
							AND d.status_active = 1
							AND d.is_deleted = 0
							GROUP BY c.production_date, c.serving_company, c.production_type, c.sewing_line, c.prod_reso_allo , a.buyer_name, a.style_ref_no,a.job_no, a.set_smv,a.total_set_qnty,a.avg_unit_price,c.item_number_id";
        //echo $production_sql; die;
		$result_production_sql = sql_select($production_sql);
		...
		foreach($result_production_sql as $row)
		{
			...
			// echo $sewing_line_id."**".$lineSerialArr[$sewing_line_id]."**".$slNo."<br>";
				...
				$data_array[$row[csf('serving_company')]][$slNo][$row[csf('sewing_line')]][$row[csf('style_ref_no')]][$row[csf('item_number_id')]][date('d-M-Y',strtotime($row[csf('production_date')]))]['sewing_output'] += $row[csf('sewing_output')];
				$data_array[$row[csf('serving_company')]][$slNo][$row[csf('sewing_line')]][$row[csf('style_ref_no')]][$row[csf('item_number_id')]]['job_no'] = $row[csf('job_no')];
				$data_array[$row[csf('serving_company')]][$slNo][$row[csf('sewing_line')]][$row[csf('style_ref_no')]][$row[csf('item_number_id')]]['item_name'] .= $garments_item[$row[csf('item_number_id')]].",";
				$data_array[$row[csf('serving_company')]][$slNo][$row[csf('sewing_line')]][$row[csf('style_ref_no')]][$row[csf('item_number_id')]]['item_number_id'] .= $row[csf('item_number_id')].",";

				$line_style_wise_production_sql_array[$row[csf('serving_company')]][$row[csf('sewing_line')]][$row[csf('style_ref_no')]]['style_ref_no'][$row[csf('item_number_id')]] = $row[csf('style_ref_no')];

				// $line_style_wise_production_sql_array[$row[csf('serving_company')]][$row[csf('sewing_line')]][$row[csf('style_ref_no')]]['item_number_id'] = $row[csf('item_number_id')];

				$line_style_wise_production_sql_array[$row[csf('serving_company')]][$row[csf('sewing_line')]][$row[csf('style_ref_no')]][$row[csf('item_number_id')]]['buyer_name'] = $row[csf('buyer_name')];
				$line_style_wise_production_sql_array[$row[csf('serving_company')]][$row[csf('sewing_line')]][$row[csf('style_ref_no')]][$row[csf('item_number_id')]]['set_smv'] = $row[csf('set_smv')];
				$line_style_wise_production_sql_array[$row[csf('serving_company')]][$row[csf('sewing_line')]][$row[csf('style_ref_no')]][$row[csf('item_number_id')]]['total_set_qnty'] = $row[csf('total_set_qnty')];

				$line_style_wise_production_sql_array[$row[csf('serving_company')]][$row[csf('sewing_line')]][$row[csf('style_ref_no')]][$row[csf('item_number_id')]]['sewing_output'] += $row[csf('sewing_output')];
				$line_style_wise_production_sql_array[$row[csf('serving_company')]][$row[csf('sewing_line')]][$row[csf('style_ref_no')]][$row[csf('item_number_id')]]['cm_cost'] = $row[csf('cm_cost')];
				$line_style_wise_production_sql_array[$row[csf('serving_company')]][$row[csf('sewing_line')]][$row[csf('style_ref_no')]][$row[csf('item_number_id')]]['avg_unit_price'] = $row[csf('avg_unit_price')];
		...
		$job_cond = where_con_using_array($job_array,1,"job_no");
		$cm_cost_arr = return_library_array( "SELECT job_no,cm_cost from wo_pre_cost_dtls where status_active=1 $job_cond", "job_no", "cm_cost");
		// echo "SELECT job_no,cm_cost from wo_pre_cost_dtls where status_active=1 $job_cond"; die;
		//echo "<pre>";print_r($cm_cost_arr);die;

		$sql = "SELECT JOB_NO,GMTS_ITEM_ID,SMV_PCS from WO_PO_DETAILS_MAS_SET_DETAILS where SMV_PCS is not null $job_cond";
		$res = sql_select($sql);
		$item_smv_array = array();
		foreach ($res as $v)
		{
			$item_smv_array[$v['JOB_NO']][$v['GMTS_ITEM_ID']] = $v['SMV_PCS'];
		}
		// echo "<pre>";print_r($item_smv_array);die;
		$line_rowspan_array =array();
		$style_count_array =array();
			foreach($data_array as $comp_id => $com_value)
			{
				ksort($com_value);
				foreach($com_value as $serial_key=>$serial_value)
				{
					foreach($serial_value as $line_id => $line_value)
					{
						...
						{
							foreach($style_value as $item_key => $item_value)
							{
								$line_rowspan_array[$comp_id][$line_id]++;
								$style_count_array[$comp_id][$line_id][$style][$item_key]++;
						    }
						}
					}
				}
									...
									foreach($com_value as $serial_key=>$serial_value)
									{
										foreach($serial_value as $line_id => $line_value)
										{
											...
											foreach($line_value as $style => $style_value)
											{
												...
												{
													// echo "<pre>";
													//  print_r($style_value);die;
													$style_count[$comp_id] += $style_count_array[$comp_id][$line_id][$style][$item_key];
													 $set_smv = $line_style_wise_production_sql_array[$comp_id][$line_id][$style][$item_key]['set_smv'];

													$total_set_qnty=$line_style_wise_production_sql_array[$comp_id][$line_id][$style][$item_key]['total_set_qnty'];

													// $itm_id_arry = array_unique(array_filter(explode(",",$style_value['item_number_id'])));
													$itm_id_arr = array_unique(array_filter(explode(",",$item_key)));
													// echo "<pre>";print_r($item_value);
													//echo "<pre>";print_r($itm_id_arr);die;

													$itm_smv = "";
													// echo $style_value['job_no'] .'<br>';
													foreach ($itm_id_arr as $itm_key => $itm_id)
													{
														$itm_smv = ($itm_smv=="") ? $item_smv_array[$item_value['job_no']][ $itm_id] : ", ".$item_smv_array[$item_value['job_no']][$itm_id];
													}
													// echo"<pre>";
													...
													?>
														<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $k; ?>">
															<td width="100" align="left"><? echo $companyArr[$comp_id]; ?></td>
															<td width="100" align="left" title="<?=$line_id;?>">
																<?
															...
															<td width="100" align="left"><? echo $buyer_library[$line_style_wise_production_sql_array[$comp_id][$line_id][$style][$item_key]['buyer_name']]; ?></td>
															<td width="100" align="left"><? echo $style; ?></td>
															<td width="100" align="left"><? echo $garments_item[$item_key]; ?></td>
															<td width="40" align="right">
															<?
															echo number_format($itm_smv,2);
															$total_smv[$comp_id] += $itm_smv;
																//echo $total_smv."**"."ffff";
															?>
															</td>
																...
																<?
																	foreach($date_range_arr as $date_value){
																		...
																		<td width="70" align="right">
																			<? echo number_format($data_array[$comp_id][$serial_key][$line_id][$style][$item_key][$date_value]['sewing_output'],0) ;

																			$total_com_date_wise[$comp_id][$date_value]  += $data_array[$comp_id][$serial_key][$line_id][$style][$item_key][$date_value]['sewing_output'] ;
														...
														<td width="100" align="right"><? echo $total = $line_style_wise_production_sql_array[$comp_id][$line_id][$style][$item_key] ['sewing_output'];
															$total_com_wise[$comp_id]  += $line_style_wise_production_sql_array[$comp_id][$line_id][$style][$item_key]['sewing_output'];
															?>
														</td>
															<?
															if ($l==0)
														...
														<td width="100" align="right" title="cm=<?=$cm_cost_arr[$item_value['job_no']];?>">
															<?

															$cm_per_pcs = (($itm_smv/$set_smv)*$cm_cost_arr[$item_value['job_no']])/12;
															echo "$ ". number_format($cm_per_pcs,4);
															...
															$total_cm_per_pcs[$comp_id] += (($itm_smv/$set_smv)*$cm_cost_arr[$item_value['job_no']])/12;
															//echo $total_cm_per_pcs."**"."ffff";
															?>
														</td>
														<td width="100" align="right">
															<?
																$cm_per_pcs_data = number_format($cm_per_pcs,4);
																echo "$ ". number_format($total_cm_earn =  $total * $cm_per_pcs_data,4);
																$tot_total_cm_earn[$comp_id] +=  $total_cm_earn;
															?>
														</td>
														<td width="100" align="right">
															<?
															$fob = $line_style_wise_production_sql_array[$comp_id][$line_id][$style][$item_key]['avg_unit_price'];
											...
											?>
												<tr style="background: #8DAFDA;">
													<td colspan="5" align="center" style="font-weight: bold;"><? echo $companyArr[$comp_id]; ?> total =</td>
													...
													<td width="100"align="right" style="font-weight: bold;">
																...
																echo "$ ". number_format($total_avg_cm_per_pcs,4);
													...
													<td width="100"align="right" style="font-weight: bold;">
																<?
																echo "$ ". number_format($tot_total_cm_earn[$comp_id],4);
							...
							<tfoot>
								<tr style="background: gray;">
									...
									<?
									   ...
									   echo "$ ". number_format($grand_avg_total_cm_per_pcs,4);
									?>
								    </td>
									<td width="100" align="right" style="font-weight: bold;"><? echo "$ ". number_format($grand_tot_total_cm_earn,4); ?></td>
									<td width="100" align="right" style="font-weight: bold;">
									<?
	...
	else if ($type == 7) // Monthly Production Summary 2
	{
		$companyArr 	= return_library_array("select id,company_name from lib_company","id","company_name");
		$lineArr = return_library_array("select id,line_name from lib_sewing_line order by id","id","line_name");
		$prod_reso_line_arr=return_library_array( "select id, line_number from prod_resource_mst",'id','line_number');
		...
		$production_sql = " SELECT c.production_date, c.serving_company, c.production_type, c.sewing_line, c.prod_reso_allo, a.buyer_name, a.style_ref_no,a.job_no, a.set_smv,a.total_set_qnty, a.avg_unit_price,c.item_number_id,b.id,
							SUM(CASE WHEN c.production_type=5 AND d.production_type=5 THEN d.production_qnty ELSE 0 END) AS sewing_output
							FROM wo_po_details_master a, wo_po_break_down b, pro_garments_production_mst c, pro_garments_production_dtls d
							WHERE
							    a.id = b.job_id
							...
							AND d.status_active = 1
							AND d.is_deleted = 0
							GROUP BY c.production_date, c.serving_company, c.production_type, c.sewing_line, c.prod_reso_allo , a.buyer_name, a.style_ref_no,a.job_no, a.set_smv,a.total_set_qnty,a.avg_unit_price,c.item_number_id,b.id";
        //  echo $production_sql; die;
		$result_production_sql = sql_select($production_sql);
		...
		// echo "SELECT job_no,cm_cost from wo_pre_cost_dtls where status_active=1 $job_cond"; die;
		//echo "<pre>";print_r($cm_cost_arr);die;
        // echo "<pre>";print_r($po_ids);echo "</pre>";
		$sql = "SELECT JOB_NO,GMTS_ITEM_ID,SMV_PCS from WO_PO_DETAILS_MAS_SET_DETAILS where SMV_PCS is not null $job_cond";
		$res = sql_select($sql);
		$item_smv_array = array();
		foreach ($res as $v)
		...
		$cutting_sql="SELECT c.serving_company,c.po_break_down_id,
		              (CASE WHEN d.production_type =1 and c.production_date='$date_to' THEN d.production_qnty ELSE 0 END) AS cutting_today,
					  (CASE WHEN d.production_type =1 and c.production_date between '$date_from' and '$date_to'  THEN d.production_qnty ELSE 0 END) AS cutting_total,
					  (CASE WHEN d.production_type =8 and c.production_date='$date_to' THEN d.production_qnty ELSE 0 END) AS finsihing_today,
					  (CASE WHEN d.production_type =8 and c.production_date between '$date_from' and '$date_to'  THEN d.production_qnty ELSE 0 END) AS finishing_total
					  FROM pro_garments_production_mst c, pro_garments_production_dtls d
					  WHERE
					      c.id=d.mst_id
					  AND c.status_active=1
					  ...
					  AND d.status_active=1
					  AND d.is_deleted=0
					  AND c.production_date between '$date_from' and '$date_to'
					  and d.production_type in(1,8)
					   $pro_company_name
		              ";
		 	//    echo $cutting_sql; die();

			$cutting_sql_result=sql_select($cutting_sql);
			$prod_arr=array();

			foreach($cutting_sql_result as $val)
			{
				$prod_arr[$val[csf('serving_company')]]['cutting_today']+=$val[csf('cutting_today')];
				$prod_arr[$val[csf('serving_company')]]['cutting_total']+=$val[csf('cutting_total')];
				$prod_arr[$val[csf('serving_company')]]['finishing_today']+=$val['FINSIHING_TODAY'];
				$prod_arr[$val[csf('serving_company')]]['finishing_total']+=$val[csf('finishing_total')];
			}
			...
			<table id="table_header_1" class="rpt_table" width="<? echo $width ?>" cellpadding="0" cellspacing="0" border="1" rules="all">
				<thead>
					<tr height="50">
						...
						<th width="100">Line Total</th>
						<th width="100">CM Per Pcs</th>
						<th width="100">CM Per Dzn</th>
						<th width="100">Total CM Earn </th>
						<th width="100">Day Cutting </th>
						<th width="100">Total Cutting</th>
						<th width="100">Day Finishing</th>
						<th width="100">Total Finishing</th>
					...
					<table class="rpt_table" width="<? echo $width ?>" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body">
										...
										foreach($serial_value as $line_id => $line_value)
										{
											...
											foreach($line_value as $style => $style_value)
											{
												foreach($style_value as $item_key => $item_value)
												{
													// echo "<pre>";
													//  print_r($style_value);die;
													$r_span=$company_rowspan_array[$comp_id];
													...
													?>
														<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $k; ?>">
															...
															<td width="100" align="right" title="cm=<?=$cm_cost_arr[$item_value['job_no']];?>">
																...
																echo "$ ". number_format($cm_per_dzn,4);
																?>
															</td>
															<td width="100" align="right">
																<?
															...
															<?
															if(!in_array($comp_id,$comp_id_check))
															{
															 $comp_id_check[]=$comp_id;
															 ?>
														    <td width="100" align="right" valign="middle" rowspan="<? echo $r_span;?>">
															<?
                                                              echo $prod_arr[$comp_id]['cutting_today'];
															  $tot_cutting_today+= $prod_arr[$comp_id]['cutting_today'];

															?>
														    </td>
															<td width="100" align="right" valign="middle" rowspan="<? echo $r_span;?>">
															<?
                                                                echo $prod_arr[$comp_id]['cutting_total'];
																$tot_cutting_total+=$prod_arr[$comp_id]['cutting_total'];
															?>
															</td>
															<td width="100" align="right" valign="middle" rowspan="<? echo $r_span;?>">
															<?
                                                                 echo $prod_arr[$comp_id]['finishing_today'];
																 $tot_finishing_today+=$prod_arr[$comp_id]['finishing_today'];
															?>
															</td>
															<td width="100" align="right" valign="middle" rowspan="<? echo $r_span;?>">
															<?
															   echo $prod_arr[$comp_id]['finishing_total'];
															   $tot_finishing_total+=$prod_arr[$comp_id]['finishing_total'];
											...
											?>
												<tr style="background: #8DAFDA;">
													...
													<td width="100"align="right" style="font-weight: bold;">
														<?
                                                          echo number_format($tot_cutting_today,2);
														  $gr_tot_cutting_today+=$tot_cutting_today;
														?>
													</td>
													<td width="100"align="right" style="font-weight: bold;">
													    <?
														   echo number_format($tot_cutting_total,2);
														   $gr_tot_cutting_total+=$tot_cutting_total;

														?>
													</td>
													<td width="100"align="right" style="font-weight: bold;">
													    <?
                                                             echo number_format($tot_finishing_today,2);
															 $gr_tot_finishing_today+=$tot_finishing_today;
														?>
													</td>
													<td width="100"align="right" style="font-weight: bold;">
													    <?
														   echo number_format($tot_finishing_total,2);
														   $gr_tot_finishing_total+=$tot_finishing_total;
							...
							<tfoot>
								<tr style="background: gray;">
									...
									<td width="100" align="right" style="font-weight: bold;">
									<?
									  echo  number_format($gr_tot_cutting_today,2);
									?>
								    </td>
									<td width="100" align="right" style="font-weight: bold;"><?   echo  number_format($gr_tot_cutting_total,2); ?></td>
									<td width="100" align="right" style="font-weight: bold;"><?  echo  number_format($gr_tot_finishing_today,2);  ?></td>
									<td width="100" align="right" style="font-weight: bold;"><? echo  number_format($gr_tot_finishing_total,2); ?></td>
	...
	else if ($type == 5) // Target vs Achievement
	{
		...
		foreach($prod_resource_sql_result as $row){
			...
			$prod_resource_sql_array[$row[csf('company_id')]][$slNo][$row[csf('id')]][$newdate]['target_per_hour'] = $row[csf('target_per_hour')];
    ...
    else if ($type == 6) // Sewing WIP
	{
		...
		// echo $txt_ref_no;
		// if(str_replace("'", "", $txt_ref_no)) $style_cond="and  a.style_ref_no = $txt_ref_no"; else $style_cond="";
		// echo $style_cond; die;
				...
				foreach($serial_value as $sewing_line_id => $sewing_value)
				{
					...
					foreach($sewing_value as $po_id => $po_value)
					{
						foreach($po_value as $color_id => $color_value)
						{
							...
							$total_sew_output = $production_sql_array[$company_id][$sewing_line_id][$po_id][$color_id]['total_sewing_output'];
							$output_bal = $total_sew_output  - $production_sql_array[$company_id][$sewing_line_id][$po_id][$color_id]['total_sewing_input'];
	...
	else if ($type == 8) // Show 2
	{
		...
		if($company_id == "" || $company_id == 0) $pro_company_name=""; else $pro_company_name_one="and a.company_id in($company_id)";
		...
		$production_sql = " SELECT c.production_date, c.serving_company, c.production_type, c.sewing_line, c.prod_reso_allo, a.buyer_name, a.style_ref_no,a.job_no, a.set_smv,a.total_set_qnty, a.avg_unit_price,b.id as po_id,c.item_number_id,
							SUM(CASE WHEN c.production_type=5 AND d.production_type=5 THEN d.production_qnty ELSE 0 END) AS sewing_output
							FROM wo_po_details_master a, wo_po_break_down b, pro_garments_production_mst c, pro_garments_production_dtls d
							WHERE
							    a.id = b.job_id
		...
		foreach($result_production_sql as $row)
		{
			...
			// echo $sewing_line_id."**".$lineSerialArr[$sewing_line_id]."**".$slNo."<br>";
				...
				$date_wise_production_sql_array[date('d-M-Y',strtotime($row[csf('production_date')]))]['sewing_output'] += $row[csf('sewing_output')];
				$job_array[$row[csf('job_no')]] = $row[csf('job_no')];
				$style_array[$row[csf('style_ref_no')]] = $row[csf('style_ref_no')];
				$poWiseStyleArr[$row[csf('po_id')]]= $row[csf('style_ref_no')];
				$line_id_array[$row[csf('sewing_line')]]= $row[csf('sewing_line')];


		}
		//  echo "<pre>";print_r($data_array);die;
	    // echo "<pre>";print_r($poWiseStyleArr);die;
		//  echo "<pre>";print_r($line_style_wise_production_sql_array);die;
		$job_cond = where_con_using_array($job_array,1,"job_no");
		$cm_cost_arr = return_library_array( "SELECT job_no,cm_cost from wo_pre_cost_dtls where status_active=1 $job_cond", "job_no", "cm_cost");
		// echo "SELECT job_no,cm_cost from wo_pre_cost_dtls where status_active=1 $job_cond"; die;
		//echo "<pre>";print_r($cm_cost_arr);die;
        ...
        $style_cond=where_con_using_array($style_array,1,"style_ref_no");

        $cm_sql="SELECT a.style_ref_no,d.production_date,e.cm_cost from wo_po_details_master a,wo_po_break_down b,pro_garments_production_mst d,wo_pre_cost_dtls e WHERE a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and a.id=b.job_id and b.id=d.po_break_down_id and a.id=e.job_id and b.job_id=e.job_id $style_cond ";

		$tot_cm_sql=sql_select($cm_sql);
		$cm_arr=array();
		$cm_per_pcs=array();

		foreach($tot_cm_sql as $row)
		{
			$cm_arr[$row[csf('style_ref_no')]][date('d-M-Y',strtotime($row[csf('production_date')]))]['cm_cost']=$row[csf('cm_cost')];
            $cm_per_style[$row[csf('style_ref_no')]]['cm_cost']=$row[csf('cm_cost')];
		}
		//   echo '<pre>';
		//   print_r($cm_arr);
		//   echo '</pre>';

		/* $prod_resource_sql="SELECT a.id as line_id,b.pr_date, b.man_power, c.working_hour,c.target_per_line,c.po_id,b.target_per_hour,b.active_machine,
		c.gmts_item_id from prod_resource_mst a, prod_resource_dtls b, prod_resource_color_size c where a.id=b.mst_id and a.id=c.mst_id and b.pr_date between '$date_from' and '$date_to' $pro_company_name_one "; 
		
		foreach($total_prod_resource_sql as $row)
		{
			//    echo "<pre>";print_r($poWiseStyleArr);die;
			$prod_resource_array[$row[csf('line_id')]][$poWiseStyleArr[$row['PO_ID']]][$row[csf('gmts_item_id')]][date('d-M-Y',strtotime($row[csf('pr_date')]))]['target_per_hour']=$row[csf('target_per_line')]*$row[csf('working_hour')];
			$prod_resource_array[$row[csf('line_id')]][$poWiseStyleArr[$row['PO_ID']]][$row[csf('gmts_item_id')]][date('d-M-Y',strtotime($row[csf('pr_date')]))]['target_per_line']=$row[csf('target_per_line')];
			$prod_resource_array[$row[csf('line_id')]][$poWiseStyleArr[$row['PO_ID']]][$row[csf('gmts_item_id')]][date('d-M-Y',strtotime($row[csf('pr_date')]))]['working_hour']=$row[csf('working_hour')];
			$line_cost_array[$row[csf('line_id')]][$poWiseStyleArr[$row['PO_ID']]][$row[csf('gmts_item_id')]]['active_machine']=$row[csf('active_machine')];
		}
		*/

		$line_id_cond = where_con_using_array($line_id_array,0,"a.id");

		$prod_resource_sql="SELECT a.id as line_id,b.from_date,b.to_date, c.working_hour,c.target_per_line,c.po_id,b.target_per_hour,b.active_machine,
		c.gmts_item_id from prod_resource_mst a, PROD_RESOURCE_DTLS_MAST b, prod_resource_color_size c where a.id=b.mst_id and b.id=c.dtls_id  $pro_company_name_one $line_id_cond";//and b.from_date >= '$date_from' and b.to_date <='$date_to'
		// echo $prod_resource_sql;die;
		...
		foreach($total_prod_resource_sql as $row)
		{
			$diff = datediff('d',$row[csf('from_date')],$row[csf('to_date')]);
			for($j=0;$j<$diff;$j++)
        	{
				$date_all=add_date(str_replace("'","",$row[csf('from_date')]),$j);
				$newdate =change_date_format($date_all,'','',1);
				//    echo "<pre>";print_r($poWiseStyleArr);die;
				$prod_resource_array[$row[csf('line_id')]][$poWiseStyleArr[$row['PO_ID']]][$row[csf('gmts_item_id')]][$newdate]['target_per_hour']=$row[csf('target_per_line')]*$row[csf('working_hour')];
				$prod_resource_array[$row[csf('line_id')]][$poWiseStyleArr[$row['PO_ID']]][$row[csf('gmts_item_id')]][$newdate]['target_per_line']=$row[csf('target_per_line')]*$row[csf('working_hour')];
				$prod_resource_array[$row[csf('line_id')]][$poWiseStyleArr[$row['PO_ID']]][$row[csf('gmts_item_id')]][$newdate]['working_hour']=$row[csf('working_hour')];
				$line_cost_array[$row[csf('line_id')]][$poWiseStyleArr[$row['PO_ID']]][$row[csf('gmts_item_id')]]['active_machine']=$row[csf('active_machine')];
			}
		}
										...
										foreach($serial_value as $line_id => $line_value)
										{
											...
											foreach($line_value as $style => $style_value)
											{
												foreach($style_value as $item_key => $item_value)
												{
													...
													?>
														<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $k; ?>">
															<td width="100" align="left" style="word-break:break-all;"><? echo $companyArr[$comp_id]; ?></td>
															<td width="100" align="left" style="word-break:break-all;" title="<?=$line_id;?>">
																<?
																	if($line_sql_array[$comp_id][$line_id]['prod_reso_allo'] == 1)
																	{
																		$sewing_line=$prod_reso_line_arr[$line_id];
															...
															<td width="100" align="left" style="word-break:break-all;"><? echo $buyer_library[$line_style_wise_production_sql_array[$comp_id][$line_id][$style][$item_key]['buyer_name']]; ?></td>
															<td width="100" align="left" style="word-break:break-all;"><? echo $style; ?></td>
															<td width="100" align="left" style="word-break:break-all;"><? echo $garments_item[$item_key]; ?></td>
															<td width="40" align="right" style="word-break:break-all;">
															<?
															echo number_format($itm_smv,2);
															$total_smv[$comp_id] += $itm_smv;
																//echo $total_smv."**"."ffff";
															...
															?>
																<td width="70" align="right">
																<div align="right"><? echo $prod_resource_array[$line_id][$style][$item_key][$date_value]['target_per_hour'];  
																
																$comp_target_per_line[$comp_id][$date_value]+= $prod_resource_array[$line_id][$style][$item_key][$date_value]['target_per_hour'];

																$style_target_per_line[$style]+=$prod_resource_array[$line_id][$style][$item_key][$date_value]['target_per_hour'];
																
																
																?>&nbsp;</div>
																<hr class="style-three">
																<div align="right"><? echo number_format($item_value[$date_value]['sewing_output'],0);
																$comp_achieve_per_line[$comp_id][$date_value]+=$item_value[$date_value]['sewing_output']; 

																$style_achieve_per_line[$style]+=$item_value[$date_value]['sewing_output'];
																
																?>&nbsp;</div>
																<hr class="style-three">
																<div align="right"><? $variance=$prod_resource_array[$line_id][$style][$item_key][$date_value]['target_per_hour']-$item_value[$date_value]['sewing_output']; echo number_format($variance,0);
																$comp_variance[$comp_id][$date_value]+=$variance;
																$style_variance[$style]+=$variance
																
																
																
																?>&nbsp;</div>
																<hr class="style-three">
																<div align="right"><? $cm=$cm_arr[$style]      [$date_value]['cm_cost'];
																	$target_per_hour=$prod_resource_array[$line_id][$style][$item_key][$date_value]['target_per_line'];
																	$work_hour=$prod_resource_array[$line_id][$style][$item_key][$date_value]['working_hour'];
																	$total_hour=$target_per_hour;
																	$cm_per_day=$total_hour*$cm;
																	echo fn_number_format($cm_per_day,2);
																	$comp_cm_per_day[$comp_id][$date_value]+=$cm_per_day;
																	$style_cm_per_day[$style]+=$cm_per_day;

																?>&nbsp;</div>
																<hr class="style-three">
																<div align="right"><?
																$actual_output=$item_value[$date_value]['sewing_output'];
																$earned_cm=$actual_output*$cm; echo fn_number_format($earned_cm,2);

                                                                      
																$comp_cm_earned_cm[$comp_id][$date_value]+=$earned_cm;

																$style_cm_earnerd_cm[$style]+=$earned_cm;

																?>&nbsp;</div>
																<hr class="style-three">
																<div align="right"><?
																	$variance_cm=$cm_per_day-$earned_cm;
																	echo fn_number_format($variance_cm,2);

																	$comp_cm_variance_cm[$comp_id][$date_value]+=$variance_cm;
																	$style_cm_variance_cm[$style]+=$variance_cm;
																?>&nbsp;</div>
																<hr class="style-three">
																</td>
															<?
															...
															<td width="100" style="word-break:break-all;" >
															    <div align="right"><?=fn_number_format($style_target_per_line[$style],2);?></div>
																<hr class="style-three">
																<div align="right"><?=fn_number_format($style_achieve_per_line[$style],2);?></div>
																<hr class="style-three">
																<div align="right"><?=fn_number_format($style_variance[$style],2);?></div>
																<hr class="style-three">
																<div align="right"><?=fn_number_format($style_cm_per_day[$style],2);?></div>
																<hr class="style-three">
																<div align="right"><?=fn_number_format($style_cm_earnerd_cm[$style],2);?></div>
																<hr class="style-three">
																<div align="right"><?=fn_number_format($style_cm_variance_cm[$style],2);?></div>
																<hr class="style-three">
															...
															if ($l==0)
															{
																?>
																	...
																	<td width="100" align="right" valign="middle" rowspan="<? echo $line_rowspan_array[$comp_id][$line_id]; ?>" style="word-break:break-all;">
																		<?
																		echo number_format($line_sql_array[$comp_id][$line_id]['sewing_output']);
																		$total_line[$comp_id] += $line_sql_array[$comp_id][$line_id]['sewing_output'];
																		?>
														  ...
														  if ($l==0)
															{
																?>
																	...
																	<td width="100" align="right" valign="middle" rowspan="<? echo $line_rowspan_array[$comp_id][$line_id]; ?>" style="word-break:break-all;">
																		<?
																	     $line_cost=$line_cost_array[$line_id][$style][$item_key]['active_machine']*27;
																		 echo number_format($line_cost);
																		$total_line_cost[$comp_id] += $line_cost;
																		?>
																	</td>
																<?
														...
														<td width="100" align="right" title="cm=<?=$cm_cost_arr[$item_value['job_no']];?>" style="word-break:break-all;">
															<?

															$cm_per_pcs = $cm_per_style[$style]['cm_cost'];
															echo "$ ". number_format($cm_per_pcs,4);

															
															// echo $count_cm_per_pcs;
															// $total_cm_per_pcs[$comp_id] += $line_style_wise_production_sql_array[$comp_id][$line_id][$style]['cm_cost'];
															$total_cm_per_pcs[$comp_id] += $cm_per_pcs;
															//echo $total_cm_per_pcs."**"."ffff";
															?>
														</td>
														<td width="100" align="right" style="word-break:break-all;">
															<?
																$cm_per_pcs_data = number_format($cm_per_pcs,4);
																$total = $line_style_wise_production_sql_array[$comp_id][$line_id][$style][$item_key] ['sewing_output'];
																echo "$ ". number_format($total_cm_earn =  $total * $cm_per_pcs_data,4);
																$tot_total_cm_earn[$comp_id] +=  $total_cm_earn;
															?>
														</td>
														<td width="100" align="right" style="word-break:break-all;"><? echo "$".fn_number_format($line_style_wise_production_sql_array[$comp_id][$line_id][$style][$item_key] ['avg_unit_price'],2); ?></td>
														<td width="100" align="right" style="word-break:break-all;">
															<?
																$unit_price=$line_style_wise_production_sql_array[$comp_id][$line_id][$style][$item_key] ['avg_unit_price'];
																$total_fob = $unit_price*$total;
																$com_total_fob[$comp_id] += $total_fob;
																echo "$ ". number_format($total_fob,2);
															?>
														</td>
								...
								?>
									...
									<?
										foreach($date_range_arr as $date_value){
											?>
											<td width="70" align="right" style="font-weight: bold;">
											<div><?  echo number_format($comp_target_per_line[$comp_id][$date_value],2); $gr_comp_target_per_line[$date_value]+=$comp_target_per_line[$comp_id][$date_value]; $style_comp_target_per_line[$style]+=$comp_target_per_line[$comp_id][$date_value];?></div>
											<hr class="style-three">
											<div><?= number_format($comp_achieve_per_line[$comp_id][$date_value],2); $comp_line_total+=$comp_achieve_per_line[$comp_id][$date_value];
											$gr_achieve_per_line[$date_value]+=$comp_achieve_per_line[$comp_id][$date_value];

											$style_comp_achieve_per_line[$style]+=$comp_achieve_per_line[$comp_id][$date_value];
											
											?></div>
											<hr class="style-three">
											<div><?=number_format($comp_variance[$comp_id][$date_value],2);
											$gr_variance[$date_value]+=$comp_variance[$comp_id][$date_value];
											$style_comp_variance[$style]+=$comp_variance[$comp_id][$date_value];
											
											?></div>
											<hr class="style-three">
											<div><?=number_format($comp_cm_per_day[$comp_id][$date_value],2);
											$gr_comp_per_day[$date_value]+=$comp_cm_per_day[$comp_id][$date_value];
											$style_comp_per_day[$style]+=$comp_cm_per_day[$comp_id][$date_value];
											
											?></div>
											<hr class="style-three">
											<div><?=number_format($comp_cm_earned_cm[$comp_id][$date_value],2);
											$gr_comp_earned_cm[$date_value]+=$comp_cm_earned_cm[$comp_id][$date_value];
											$style_comp_earned_cm[$style]+=$comp_cm_earned_cm[$comp_id][$date_value];
											
											?></div>
											<hr class="style-three">
											<div><?=number_format($comp_cm_variance_cm[$comp_id][$date_value],2);
											$gr_cm_variance_cm[$date_value]+=$comp_cm_variance_cm[$comp_id][$date_value];
											$style_comp_cm_variance_cm[$style]+=$comp_cm_variance_cm[$comp_id][$date_value];
									...
									?>
									<td width="100">
									<div align="right"><?=number_format($style_comp_target_per_line[$style],2); $gr_style_comp_target_per_line+=$style_comp_target_per_line[$style]; ?></div>
									<hr class="style-three">
									<div align="right"><?=number_format($style_comp_achieve_per_line[$style],2);$gr_style_comp_achieve_per_line+=$style_comp_achieve_per_line[$style];; ?></div>
									<hr class="style-three">
									<div align="right"><?=number_format($style_comp_variance[$style],2); $gr_style_comp_variance+=$style_comp_variance[$style];?></div>
									<hr class="style-three">
									<div align="right"><?=number_format($style_comp_per_day[$style],2); $gr_style_per_day+=$style_comp_per_day[$style];?></div>
									<hr class="style-three">
									<div align="right"><?=number_format($style_comp_earned_cm[$style],2); $gr_style_comp_earned_cm+=$style_comp_earned_cm[$style];?></div>
									<hr class="style-three">
									<div align="right"><?=number_format($style_comp_cm_variance_cm[$style],2); $gr_style_comp_cm_variance_cm+=$style_comp_cm_variance_cm[$style]; ?></div>
									<hr class="style-three">
									</td>
									<td width="100" align="right" style="font-weight: bold;"><? echo number_format($total_line[$comp_id],0); $gr_line_total+=$total_line[$comp_id]; ?></td>
									<td width="100" align="right" style="font-weight: bold;"><? echo number_format($total_line_cost[$comp_id]); $gr_line_cost+=$total_line_cost[$comp_id]; ?></td>
									<td width="100" align="right" style="font-weight: bold;">
									<?
									  $per_cm=$tot_total_cm_earn[$comp_id]/$total_line[$comp_id];
									  echo number_format($per_cm,2);
									  $gr_per_cm+=$per_cm;
									?>
								    </td>
									<td width="100" align="right" style="font-weight: bold;"><? echo number_format($tot_total_cm_earn[$comp_id],2); $gr_total_cm_earn+=$tot_total_cm_earn[$comp_id];?></td>
									<td width="100" align="right" style="font-weight: bold;">
									<?
									?>
								    </td>
									<td width="100" align="right" style="font-weight: bold;"><? echo number_format($com_total_fob[$comp_id],2); $gr_fob+=$com_total_fob[$comp_id]; ?></td>
								</tr>
								<?
								...
								<tr style="background: gray;">
									...
									<?
										foreach($date_range_arr as $date_value){
											?>
											<td width="70" align="right" style="font-weight: bold;">
											<div><?  echo number_format($gr_comp_target_per_line[$date_value],2); ?></div>
											<hr class="style-three">
											<div><?= number_format($gr_achieve_per_line[$date_value],2);
											
											?></div>
											<hr class="style-three">
											<div><?=number_format($gr_variance[$date_value],2);
											
											?></div>
											<hr class="style-three">
											<div><?=number_format($gr_comp_per_day[$date_value],2);
											
											
											?></div>
											<hr class="style-three">
											<div><?=number_format($gr_comp_earned_cm[$date_value],2);
											
											
											?></div>
											<hr class="style-three">
											<div><?=number_format($gr_cm_variance_cm[$date_value],2);
									...
									?>
									<td width="100">
									<div align="right"><?=number_format($gr_style_comp_target_per_line,2);?></div>
									<hr class="style-three">
									<div  align="right"><?=number_format($gr_style_comp_achieve_per_line,2);?></div>
									<hr class="style-three">
									<div  align="right"><?=number_format($gr_style_comp_variance,2);?></div>
									<hr class="style-three">
									<div  align="right"><?=number_format($gr_style_per_day,2);?></div>
									<hr class="style-three">
									<div  align="right"> <?=number_format($gr_style_comp_earned_cm,2);?></div>
									<hr class="style-three">
									<div  align="right"><?=number_format($gr_style_comp_cm_variance_cm,2);?></div>
		
									</td>
									<td width="100" align="right" style="font-weight: bold;"><? echo number_format($gr_line_total,0); ?></td>
									<td width="100" align="right" style="font-weight: bold;"><? echo number_format($gr_line_cost); ?></td>
									<td width="100" align="right" style="font-weight: bold;">
									<?
									  echo number_format($gr_per_cm,2);
									?>
								    </td>
									<td width="100" align="right" style="font-weight: bold;"><? echo number_format($gr_total_cm_earn,2);?></td>
									<td width="100" align="right" style="font-weight: bold;">
									<?
									?>
								    </td>
									<td width="100" align="right" style="font-weight: bold;"><? echo number_format($gr_fob,2);  ?></td>
								</tr>
						    </tfoot>
					</table>
				</div>
