				...
				<table width="896" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
					...
					<tbody>
						<tr>
							...
							<td align="center">
								<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view (document.getElementById('cbo_company_name').value+'**'+document.getElementById('txt_fso_no').value+'**'+document.getElementById('txt_booking_no').value+'**'+document.getElementById('txt_style_no').value+'**'+document.getElementById('cbo_within_group').value+'**'+document.getElementById('cbo_year').value+'**'+document.getElementById('cbo_buyer_name').value, 'bookingnumbershow_search_list_view', 'search_div', 'fabric_finishing_report_urmi_subcon_controller_wip', 'setFilterGrid(\'list_view\',-1)');" style="width:100px;" />
							</td>
						</tr>
					</tbody>
				</table>
				...
				<table width="746" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
					...
					<tbody>
						<tr>
							...
							<td align="center">
								<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view ('<? echo $company_name; ?>'+'**'+document.getElementById('txt_batch_no').value+'**'+document.getElementById('txt_date_from').value+'**'+document.getElementById('txt_date_to').value, 'batchnumbershow_search_list_view', 'search_div', 'fabric_finishing_report_urmi_subcon_controller_wip', 'setFilterGrid(\'list_view\',-1)');" style="width:100px;" />
							</td>
						</tr>
						<tr>
							<tr>
...
if($action=="batchnumbershow_search_list_view")
{
	...
	$sql="SELECT id,batch_no,extention_no,batch_for,booking_no,color_id,batch_weight from pro_batch_create_mst where company_id=$company_name  and entry_form=36 and is_deleted = 0 $search_con $field_grpby ";
...
{
	echo create_drop_down( "cbo_location_id", 110, "select id,location_name from lib_location where status_active=1 and is_deleted=0 and company_id in($data)    order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down( 'requires/fabric_finishing_report_urmi_subcon_controller_wip', document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_floor', 'floor_td' );",0 );
	exit();
}
if ($action=="load_drop_down_floor")
{
	$ex_data = explode("_", $data);
	echo create_drop_down( "cbo_floor_id", 110, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and production_process in(3,4) and company_id in($ex_data[0]) and location_id in($ex_data[1]) order by floor_name","id,floor_name",1, "-- Select Floor --", $selected, "load_drop_down( 'requires/fabric_finishing_report_urmi_subcon_controller_wip',this.value, 'load_drop_down_machine', 'machine_td' );",0 );
	exit();
}
...
if($action=="report_generate2")
{
	...
	if($cbo_buyer_name) $buyer_id_cond=" and e.party_id=$cbo_buyer_name";else $buyer_id_cond="";
	if($cbo_location_id) $location_cond=" and b.location_id=$cbo_location_id";else $location_cond="";
	if($cbo_location_id) $location_cond2=" and a.location_id=$cbo_location_id";else $location_cond2="";
	if($cbo_location_id) $location_cond3=" and c.location_id=$cbo_location_id";else $location_cond3="";
	if($cbo_year) $batch_year_cond = " and to_char(c.insert_date,'YYYY')=$cbo_year";
	if($batch_number_show) $batch_cond=" and b.batch_no='$batch_number_show'";
	if($batch_extension) $batch_ext_cond=" and b.extention_no='$batch_extension'";
	if($batch_number_show) $batch_cond2=" and c.batch_no='$batch_number_show'";
	if($batch_number_show) $batch_cond3=" and a.batch_no='$batch_number_show'";
	...
	if($txt_date_from && $txt_date_to)
	{
		...
		$get_batch_ids_by_batch_date=sql_select("select entry_form, b.id,b.process_id from pro_batch_create_mst b where b.status_active=1 and b.entry_form=36 $batch_cond $batch_date_cond  ");
		//echo "select entry_form, b.id,b.process_id from pro_batch_create_mst b where b.status_active=1 and b.entry_form=36 $batch_cond $batch_date_cond ";
		...
		foreach($get_batch_ids_by_batch_date as $row)
		{
			...
			else
			{
				...
				//echo $row[csf("id")].'='.$row[csf("process_id")]."D, ";
			}
		}
		//print_r($production_batch_arr);

		$get_batch_ids_from_subprocess = sql_select("select a.batch_id from pro_fab_subprocess a where a.status_active=1 and a.entry_form=38 and a.load_unload_id=2 $prod_date_cond  $batch_cond3  group by a.batch_id");
		...
		$get_batch_ids_from_subprocess_chk = sql_select("select a.batch_id,a.entry_form,a.result from pro_fab_subprocess a where a.status_active=1  $prod_date_cond  $batch_cond3 group by a.batch_id,a.entry_form,a.result");
		//echo "select a.batch_id from pro_fab_subprocess a where a.status_active=1 and a.entry_form=38 and a.load_unload_id=2 $prod_date_cond $floor_cond2 $batch_cond3 group by a.batch_id";
		...
		if(empty($production_batch_arr))
		{
		 ...
		 $get_batch_ids_from_subprocess_sql = "select max(a.id) as id,b.id batch_id
		...
		where c.mst_id=b.id and c.status_active=1  $batch_cond  $floor_cond2 $comp_cond2 $batch_ext_cond  $production_batch_id_cond group by b.id order by id desc";//group by b.id,b.extention_no,d.within_group,d.id,d.buyer_id,d.po_buyer

		$get_batch_ids_from_subprocess = sql_select($get_batch_ids_from_subprocess_sql);
		if(count($get_batch_ids_from_subprocess)<=0) {echo "No Data Found";die;}
		$i=0;
		foreach($get_batch_ids_from_subprocess as $row)
		{
	...
	else 
	{
	   ...
	   $get_batch_ids_from_subprocess_sql = "select a.entry_form,a.result,a.load_unload_id,a.id,b.id batch_id,b.extention_no
		from pro_batch_create_dtls c,pro_batch_create_mst b
		left join pro_fab_subprocess a on b.id=a.batch_id   and a.status_active=1 $prod_comp_cond
		where c.mst_id=b.id  and b.entry_form=36  and c.status_active=1 and b.status_active=1  $batch_cond $batch_ext_cond $sub_buyer_cond2  $floor_cond2  $production_batch_id_cond order by a.id desc";
	...
	$batch_date_cond2 = (empty($batch_max_process))?$batch_date_cond2:"";
		 ...
		 $sql_query="select c.id ,c.process_id,c.batch_no,c.batch_date as batch_date,c.color_id,c.color_range_id,c.booking_no,c.company_id as working_company_id,c.sales_order_no,c.sales_order_id,c.id as batch_id,c.extention_no as batch_ext_no,c.remarks,c.floor_id,d.prod_id,d.width_dia_type,d.item_description as fabric_desc,sum(d.batch_qnty) as batch_qty,d.po_id,sum(d.roll_no) no_of_roll,d.fin_dia,d.gsm
	from pro_batch_create_dtls d,pro_batch_create_mst c,subcon_ord_mst e,subcon_ord_dtls f
	where d.mst_id=c.id and f.job_no_mst=e.subcon_job and f.id=d.po_id and c.entry_form=36  and c.status_active=1 and d.status_active=1 $batch_id_cond $batch_cond2 $batch_date_cond2 $batch_year_cond $comp_cond3 $batch_ext_cond2    $buyer_id_cond
	group by c.id,c.batch_no,c.process_id,c.color_id,c.color_range_id,c.batch_date,c.booking_no,c.company_id,c.sales_order_no, c.sales_order_id,c.id,c.extention_no,c.remarks,c.floor_id,d.prod_id,d.fin_dia,d.gsm,d.width_dia_type,d.item_description,d.po_id order by c.id";
	...
	foreach( $dyeing_unload_data as $row )
	{
		...
		$load_unload_time_arr[$row[csf("batch_id")]]["floor_id"]=$row[csf("floor_id")];
	...
	if($batch_cond_for_in!="")
	{
		...
		foreach($dyeing_data as $row )
		{
			...
			//echo $production_qty.'='.$row[csf("batch_id")].'='.$row[csf("prod_id")].'='.$row[csf("width_dia_type")].',<br>';
				$subpro_prod_qty_arr[$process_result][$row[csf("batch_id")]][$row[csf("prod_id")]]+=$production_qty;
			   }
			$subpro_prod_arr2[$process_result][$row[csf("batch_id")]]['floor_id']=$row[csf("floor_id")];
			...
			$subpro_prod_arr2[$process_result][$row[csf("batch_id")]]['process_end_date']=$row[csf("process_end_date")];
			...
			$subpro_prod_arr2[$process_result][$row[csf("batch_id")]]['hr_min']=$row[csf("end_hours")].':'.$row[csf("end_minutes")];
			
			$result_id_arr[$row[csf("mst_id")]] = $row[csf("result")];
		//	echo $row[csf("entry_form")].'f';
	...
	//echo $results."=".$load_unload."=".$batch_max_process_id."=".$row[csf("id")].'<br> ';
	...
	//echo $load_unload.'='.$results."=".$entry_form_id."=".$sub_result_id."=".$sub_entry_form."=".$process_id.', ';
		if(($cbo_type==0 || $cbo_type==30) && ($load_unload==2 && ($results==1 || $results==11 || $results==0)))
		{
			
			$process_id= 30;
			if($process_id!="") $process_id=$process_id;else $process_id="";
			//echo "X";
			///echo $row[csf("process_end_date")].'XSSSSSSSS';
		}
		...
		if($process_id!=""){
			...
			{
				$subpro_prod_qty=$subpro_prod_qty_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]];
				//echo $row[csf("id")].'='.$row[csf("prod_id")].'='.$row[csf("width_dia_type")];
			}
			

			$hr_min = $subpro_prod_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]['hr_min'][$row[csf("width_dia_type")]];
			...
			else
			{
				...
				$process_end_date 	= $subpro_prod_arr2[$process_id][$row[csf("id")]]['process_end_date'];
				$no_of_roll 		= $subpro_prod_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['no_of_roll'];
				$floor_id 			= $subpro_prod_arr2[$process_id][$row[csf("id")]]['floor_id'];
				$hr_min 			= $subpro_prod_arr2[$process_id][$row[csf("id")]]['hr_min'];
				//echo $row[csf("id")].'='.$hr_min.'<br>';
			}
...
*/
			...
			//echo $process_end_date.'D'.$row[csf("id")].'<br>';
			...
			$batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]["no_of_roll"]+=$row[csf("no_of_roll")];
		...
		//echo $process_id.'='.$results.'='.$entry_form_id.'='.$row[csf("id")].'<br>';
		
			
			if($load_unload==1 && $entry_form_id==38 && in_array(33, $process_ids))
			{
				//echo "A";
			...
			else if($results==11 && $entry_form_id!=38 ){
				unset($batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]);
			}
			...
			else if($load_unload==2 && $entry_form_id==38 && !in_array($results, $dyeing_result_arr))
			{
				//echo "A";
				unset($batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]);
			}
			else if($load_unload==2 && $entry_form_id==38 && in_array($results, $dyeing_result_arr))
			{
			...
			else if($results==11 && $entry_form_id==34 ){
				...
				//$summary_wip_arr[$process_id]["no_of_roll"]+=$row[csf("no_of_roll")];
	...
	foreach($batch_wip_arr as $page_from_id=>$batch_data)
	{
		foreach($batch_data as $batch_id=>$prod_data)
			{
				...
				{
					//$summary_wip_arr[$page_from_id]["no_of_roll"]+=$row[csf("no_of_roll")];
					//$summary_wip_arr[$page_from_id]["barcode_no"]+=$row[csf("barcode_no")];
				}
			}
	 ...
	 $sub_po_sql = "select a.id,b.id batch_id,b.extention_no,b.process_id,e.within_group,d.cust_style_ref,d.order_no,e.subcon_job,d.id po_id,e.party_id as buyer_id,c.prod_id,c.batch_qnty
		...
		left join pro_fab_subprocess a on b.id=a.batch_id  and a.status_active=1
		where  d.job_no_mst=e.subcon_job and d.id=c.po_id and c.mst_id=b.id   and b.entry_form=36 and c.status_active=1 and d.status_active=1  $batch_cond $batch_ext_cond $sub_buyer_cond2 $batch_cond_for_in3 $buyer_id_cond
		order by a.id desc";
			...
			<fieldset style="width:<? echo $width;?>px;">
				...
				if($rpt_type==1)
				{
						foreach($batch_wip_arr as $page_from_id=>$batch_data)
							...
							//print_r($summary_wip_arr).'SSSSSSSSS';
					...
					?>
					<table class="rpt_table" width="2250" cellpadding="0" cellspacing="0" border="1" rules="all" align="left">
						...
						<thead>
							<tr>
								...
								<th width="90" class="alignment_css">Order No</th>
					...
					<div style=" max-height:380px; width:2270px; overflow-y:scroll;" id="scroll_body">
						<table align="left" class="rpt_table" id="table_body" width="2250" cellpadding="0" cellspacing="0" border="1" rules="all">
							<tbody>
								<?
								$m=1;
								foreach($batch_wip_arr as $page_from_id=>$batch_data)
										...
										foreach($batch_data as $batch_id=>$prod_data)
										{
											foreach($prod_data as $prod_id=>$row)
											{
												...
												<!--	<td width="80" class="alignment_css"><?echo $row["service_source"] ;?> </td>-->
													...
													<td width="80" title="Floor Id=<? echo $row["floor_id"];?>" class="alignment_css"><? echo $floor_arr[$row["floor_id"]];?></td>
													...
													<td width="80" title="Entry Page=<? echo $page_from_id; ?>" class="alignment_css">
														<? if($page_from_id==32) 
														{ 
														 $last_process_date=change_date_format($row["batch_date"]);
														 $last_process_date_cal=$row["batch_date"];
														}
														else 
														{ 
														$last_process_date=change_date_format($row["process_end_date"]); echo $last_process_date;
														 $last_process_date_cal=$row["process_end_date"];
														}
														?>
													</td>
													<td width="80" title="HeatSet not show" class="alignment_css" title="H:M">
														<?
														$dyeing_date=change_date_format($load_unload_time_arr[$batch_id]["end_date"]);
														...
														$new_date_time_end=(change_date_format($row["process_end_date"]).' '.$row["process_end_time"].':'.'00');
														$new_date_time_end2=$dyeing_date.' '.$dyeing_time.':'.'00';
														?>
													</td>
													<td width="80" title="HeatSet and Sliting not show" class="alignment_css">
														<?
														$total_time_diff=datediff(n,$unload_date_time ,$new_date_time_end);
														$diff_time_days=$total_time_diff;
														
														$days_remian=datediff('d',$last_process_date_cal,$dyeing_date)-1;
														if($page_from_id==33 || $page_from_id==30) echo ""; else echo  abs($days_remian);?>
													</td>
													<td width="80" title="HeatSet and Sliting not show<? echo $unload_date_time.'='.$new_date_time_end;?>"  class="alignment_css">
														<?
														//echo $unload_date_time.'='.$new_date_time_end.'='.$dyeing_date;
...
if($action=="report_generate")//Not used
{
	$process = array( &$_POST );
	extract(check_magic_quote_gpc( $process ));
	$all_condition="";
