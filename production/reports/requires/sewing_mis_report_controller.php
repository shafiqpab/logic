...
{
    $print_report_format_arr = return_library_array("select format_id,format_id from lib_report_template where template_name =$data and module_id=7 and report_id=188 and is_deleted=0 and status_active=1","format_id","format_id");
	...
	<table cellspacing="0" width="200"  border="1" rules="all" class="rpt_table" align="center" >
		...
		<tbody>
			<tr>
				<td width="200">
			<? 
				echo create_drop_down( "cbo_copy_type", 200, array(1=>"One Copy",2=>"Four Copies"),"", 0, "-- Select Floor --", "", "" );
			?>
			</td>
			</tr>
			<tr>
				<td width="200" align="center">
					<input type="button" name="search1" id="search1" value="Close" onClick="parent.emailwindow.hide()" style="width:60px" class="formbutton" />
				</td>
				</tr>
		</tbody>
	</table>
...
if($action=="report_generate_month") 
{
	...
	// ======= for cumulative sewing qty =================
	$current_month_first_date=date("Y-m-01",$production_date_min);
	$start_date = date("01-M-Y",strtotime(str_replace("'", "", $txt_date)));
	...
	// =============================================== for inhouse prod =================================
	$sql_item="SELECT a.job_no, a.buyer_name, a.style_ref_no, b.id, a.set_break_down, c.gmts_item_id, c.set_item_ratio, c.smv_pcs, c.smv_pcs_precost,c.smv_set from wo_po_details_master a, wo_po_break_down b, wo_po_details_mas_set_details c where b.job_no_mst=a.job_no and b.job_no_mst=c.job_no and a.company_name in($manufacturing_company) and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1";
	// echo $sql_item;die;
	$resultItem=sql_select($sql_item);
	
	foreach($resultItem as $itemData)
	{
		$item_smv_array[$itemData[csf('job_no')]][$itemData[csf('gmts_item_id')]]=$itemData[csf('smv_pcs')];
		$po_buyer_arr[$itemData[csf('id')]]=$itemData[csf('buyer_name')];
		$po_style_arr[$itemData[csf('id')]]=$itemData[csf('style_ref_no')];
	}

	// ============================================== for subcontact prod ===========================================
	$sql_sub_item="SELECT  a.party_id as buyer_name, b.cust_style_ref, b.id from subcon_ord_mst a, subcon_ord_dtls b where a.id=b.mst_id and a.company_id in($manufacturing_company) and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and b.cust_style_ref != '0'";
	// echo $sql_sub_item;die;
	$sub_resultItem=sql_select($sql_sub_item);
	$sub_po_style_arr = array();
	$sub_po_buyer_arr = array();
	foreach($sub_resultItem as $itemData)
	{
		$sub_po_buyer_arr[$itemData[csf('id')]]=$itemData[csf('buyer_name')];
		$sub_po_style_arr[$itemData[csf('id')]]=$itemData[csf('cust_style_ref')];
	}
	// echo "<pre>";print_r($sub_po_style_arr);die();
	/*===============================================================================/
	...
	if($prod_reso_allo[0]==1)
	{
		...
		foreach($dataArray_sql as $val)
		{
			...
			$pr_date=$val[csf('pr_date')];
			$floor_id=$val[csf('floor_id')];
			if($poly_line==0)
			{
				$pr_date=date("d-M-Y",strtotime($pr_date));
				
				$prod_resource_array[$val[csf('id')]][$pr_date]['man_power']=$val[csf('man_power')];
				$prod_resource_array[$val[csf('id')]][$pr_date]['operator']=$val[csf('operator')];
				$prod_resource_array[$val[csf('id')]][$pr_date]['helper']=$val[csf('helper')];
				$prod_resource_array[$val[csf('id')]][$pr_date]['terget_hour']=$val[csf('target_per_hour')];
				//$prod_resource_array[$val[csf('id')]][$pr_date]['target_per_line']=$val[csf('target_per_line')];
				$prod_resource_array[$val[csf('id')]][$pr_date]['working_hour']=$val[csf('working_hour')];
				$prod_resource_array[$val[csf('id')]][$pr_date]['tpd']=$val[csf('target_per_hour')]*$val[csf('working_hour')];
				$prod_resource_array[$val[csf('id')]][$pr_date]['day_start']=$val[csf('from_date')];
				$prod_resource_array[$val[csf('id')]][$pr_date]['day_end']=$val[csf('to_date')];
				$prod_resource_array[$val[csf('id')]][$pr_date]['capacity']=$val[csf('capacity')];
				$prod_resource_array[$val[csf('id')]][$pr_date]['active_machine']=$val[csf('active_machine')];
				//$prod_resource_array[$val[csf('id')]][$pr_date]['active_operator']=$val[csf('active_operator')];
				//$prod_resource_array[$val[csf('id')]][$pr_date]['active_helper']=$val[csf('active_helper')];
				$prod_resource_array[$val[csf('id')]][$pr_date]['line_chief']=$val[csf('line_chief')];
				$prod_resource_array[$val[csf('id')]][$pr_date]['smv_adjust']=$val[csf('smv_adjust')];
				$prod_resource_array[$val[csf('id')]][$pr_date]['smv_adjust_type']=$val[csf('smv_adjust_type')];
				$prod_resource_array[$val[csf('id')]][$pr_date]['target_efficiency']=$val[csf('target_efficiency')];
				$prod_resource_array[$val[csf('id')]][$pr_date]['line_chief']=$val[csf('line_chief')];
				...
				$production_data_arr[$pr_date][$floor_id][$val[csf('id')]]['serial']=$slNo;
				$production_serial_arr[$pr_date][$floor_id][$val[csf('line_chief')]][$slNo][$val[csf('id')]]=$val[csf('id')];
				
				if($production_data_arr[$pr_date][$floor_id][$val[csf('id')]]['po_id']!="")
				{
					$production_data_arr[$pr_date][$floor_id][$val[csf('id')]][$val[csf('po_id')]]['po_id'].=",".$val[csf('po_id')];
					$production_data_arr[$pr_date][$floor_id][$val[csf('id')]]['style_ref'].=",".$po_style_arr[$val[csf('po_id')]]; 
				}
				else
				{
					$production_data_arr[$pr_date][$floor_id][$val[csf('id')]]['po_id']=$val[csf('po_id')];	
					$production_data_arr[$pr_date][$floor_id][$val[csf('id')]]['style_ref']=$po_style_arr[$val[csf('po_id')]];
				}
				
				if($production_data_arr[$pr_date][$floor_id][$val[csf('id')]]['item_number_id']!="")
				{
					$production_data_arr[$pr_date][$floor_id][$val[csf('id')]]['item_number_id'].="#".$val[csf('po_id')]."**".$val[csf('gmts_item_id')]."**".$val[csf('target_per_line')]; 
				}
				else
				{
					 $production_data_arr[$pr_date][$floor_id][$val[csf('id')]]['item_number_id']=$val[csf('po_id')]."**".$val[csf('gmts_item_id')]."**".$val[csf('target_per_line')]; 
				}
				if( $production_data_arr[$pr_date][$floor_id][$val[csf('id')]]['buyer_name']!="")
				{
					 $production_data_arr[$pr_date][$floor_id][$val[csf('id')]]['buyer_name'].=",".$po_buyer_arr[$val[csf('po_id')]]; 
				}
				else
				{
					 $production_data_arr[$pr_date][$floor_id][$val[csf('id')]]['buyer_name']=$po_buyer_arr[$val[csf('po_id')]]; 
				...
				$style_resource_array[$po_style_arr[$val[csf('po_id')]]][$pr_date][$floor_id][$val[csf('id')]]['target_per_line']+=$val[csf('target_per_line')];
				$style_resource_array[$po_style_arr[$val[csf('po_id')]]][$pr_date][$floor_id][$val[csf('id')]]['active_operator']+=$val[csf('active_operator')];
				$style_resource_array[$po_style_arr[$val[csf('po_id')]]][$pr_date][$floor_id][$val[csf('id')]]['active_helper']+=$val[csf('active_helper')];
				$style_resource_array[$po_style_arr[$val[csf('po_id')]]][$pr_date][$floor_id][$val[csf('id')]]['extra_worker']+=$val[csf('extra_worker')];
				$style_resource_array[$po_style_arr[$val[csf('po_id')]]][$pr_date][$floor_id][$val[csf('id')]]['style_working_hour']+=$val[csf('style_working_hour')];

				$production_data_arr[$pr_date][$floor_id][$val[csf('id')]]['prod_reso_allo']=1;
				$production_data_arr[$pr_date][$floor_id][$val[csf('id')]]['type_line']=2;
			} 
				// ================================ for subcon =====================
				$sstyle_resource_array[$sub_po_style_arr[$val[csf('po_id')]]][$pr_date][$floor_id][$val[csf('id')]]['target_per_line']+=$val[csf('target_per_line')];
				$sstyle_resource_array[$sub_po_style_arr[$val[csf('po_id')]]][$pr_date][$floor_id][$val[csf('id')]]['active_operator']+=$val[csf('active_operator')];
				$sstyle_resource_array[$sub_po_style_arr[$val[csf('po_id')]]][$pr_date][$floor_id][$val[csf('id')]]['active_helper']+=$val[csf('active_helper')];
				$sstyle_resource_array[$sub_po_style_arr[$val[csf('po_id')]]][$pr_date][$floor_id][$val[csf('id')]]['extra_worker']+=$val[csf('extra_worker')];
				$sstyle_resource_array[$sub_po_style_arr[$val[csf('po_id')]]][$pr_date][$floor_id][$val[csf('id')]]['style_working_hour']+=$val[csf('style_working_hour')];
		}
		// echo "<pre>"; print_r($style_resource_array);die();
		...
		$prod_resource_smv_adj_style_array = array();
		$prod_resource_smv_adj_style_title_array = array();
		$sql_query=sql_select("SELECT b.mst_id, b.pr_date,b.number_of_emp ,b.adjust_hour,b.total_smv,b.adjustment_source,b.style_ref_no from prod_resource_mst a,prod_resource_smv_adj b  where a.id=b.mst_id  and a.company_id=$comapny_id and b.pr_date=$txt_date and a.is_deleted=0 and b.is_deleted=0 and b.is_deleted=0 and b.status_active=1  $location_cond  $floor ");
		foreach($sql_query as $val)
		{
			$val[csf('pr_date')]=date("d-M-Y",strtotime($val[csf('pr_date')]));
			if($val[csf('adjustment_source')]==1) // for extra hour
			{
				$prod_resource_smv_adj_array[$val[csf('mst_id')]][$val[csf('pr_date')]][1]['number_of_emp']+=$val[csf('number_of_emp')];
				$prod_resource_smv_adj_array[$val[csf('mst_id')]][$val[csf('pr_date')]][1]['adjust_hour']+=$val[csf('adjust_hour')];
				$prod_resource_smv_adj_array[$val[csf('mst_id')]][$val[csf('pr_date')]][1]['total_smv']+=$val[csf('total_smv')];

				$prod_resource_smv_adj_style_array[$val[csf('mst_id')]][$val[csf('style_ref_no')]][$val[csf('pr_date')]][1]['number_of_emp']+=$val[csf('number_of_emp')];
				$prod_resource_smv_adj_style_array[$val[csf('mst_id')]][$val[csf('style_ref_no')]][$val[csf('pr_date')]][1]['adjust_hour']+=$val[csf('adjust_hour')];
				$prod_resource_smv_adj_style_array[$val[csf('mst_id')]][$val[csf('style_ref_no')]][$val[csf('pr_date')]][1]['total_smv']+=$val[csf('total_smv')];
			}
			else // for others
			{
				$prod_resource_smv_adj_array[$val[csf('mst_id')]][$val[csf('pr_date')]][2]['number_of_emp']+=$val[csf('number_of_emp')];
				$prod_resource_smv_adj_array[$val[csf('mst_id')]][$val[csf('pr_date')]][2]['adjust_hour']+=$val[csf('adjust_hour')];
				$prod_resource_smv_adj_array[$val[csf('mst_id')]][$val[csf('pr_date')]][2]['total_smv']+=$val[csf('total_smv')];
				$prod_resource_smv_adj_style_array[$val[csf('mst_id')]][$val[csf('style_ref_no')]][$val[csf('pr_date')]][2]['total_smv']+=$val[csf('total_smv')];
				
				$prod_resource_smv_adj_style_title_array[$val[csf('mst_id')]][$val[csf('style_ref_no')]][$val[csf('pr_date')]].=$val[csf('adjustment_source')].",";
			}
		}
		
		// echo "<pre>";print_r($prod_resource_smv_adj_style_array);die();
		/*===============================================================================/
		/							Actual resource time data							 /
		/============================================================================== */
		if($db_type==0)
		{
	...
	$lineHoursArr=array();
	$sewingOutputQtyArr=array();
	$reso_line_ids='';
	$all_po_id="";
	$all_po_id_arr=array();
	$lineWiseJobGmtsItemArr=array();
	$lineWiseStyleGmtsItemArr=array();
	...
	foreach($sql_resqlt as $val)
	{
		...
		$floor_id=$val[csf('floor_id')];
		$sewing_line=$val[csf('sewing_line')];
		$groupStr=$val[csf('job_no')].'__'.$val[csf('style_ref_no')].'__'.$val[csf('item_number_id')];
		if($poly_line==0)
		{
			if($lineSerialArr[$sewing_line_id]=="")
			{
			...
			//echo $prod_resource_array[$sewing_line][$prod_date]['line_chief']."**".$val[csf('pr_date')]."**".$sewing_line;die;
			$production_serial_arr[$prod_date][$floor_id][$prod_resource_array[$sewing_line][$prod_date]['line_chief']][$slNo][$sewing_line]=$sewing_line;
			$lineWiseStyleGmtsItemArr[$prod_date][$floor_id][$prod_resource_array[$sewing_line][$prod_date]['line_chief']][$slNo][$sewing_line]=$sewing_line;
			$lineWiseJobGmtsItemArr[$prod_date][$floor_id][$sewing_line].=$groupStr.'###';
			$line_start=$line_number_arr[$sewing_line][$prod_date]['prod_start_time'];
			if($line_start!="") 
			{ 
				$line_start_hour=substr($line_start,0,2); 
				if(substr($line_start_hour,0,1)==0) $line_start_hour=substr($line_start_hour,1,1);	
			}
			else $line_start_hour=$hour; 
			...
			for($h=$hour; $h<$last_hour; $h++)
			{
				...
				if(str_replace("'","",$actual_production_date)>=str_replace("'","",$actual_date)) 
				{
					...
					{
						$production_po_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]+=$val[csf($prod_hour)];
						$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line][$prod_hour]+=$val[csf($prod_hour)];
						
						$lineHoursArr[$groupStr][$prod_date][$floor_id][$sewing_line][$prod_hour]+=$val[csf($prod_hour)];
					} 	
				}
			...
			{	
				if( $h>=$line_start_hour && $h<=$actual_time)
				{
					...
					$production_po_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]+=$val[csf('prod_hour23')]; 
					$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line][$prod_hour]+=$val[csf('prod_hour23')]; 
					$lineHoursArr[$groupStr][$prod_date][$floor_id][$sewing_line][$prod_hour]+=$val[csf('prod_hour23')];    
				} 	
			}
			// echo $groupStr."==".$sewing_line."<br>";
			$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['prod_reso_allo']=$val[csf('prod_reso_allo')];
			$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['serial']=$slNo;  
			$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['buyer_name']=$val[csf('buyer_name')]; 
			$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['style_ref']=$val[csf('style_ref_no')];
			$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['type_line']=1;
			$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['po_id'].=$val[csf('po_break_down_id')].",";
			
			$fob_rate_data_arr[$prod_date][$floor_id][$sewing_line][$val[csf('po_break_down_id')]]['rate']=$val[csf('unit_price')]; 
			
			if($production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['item_number_id']!="")
			{
				$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['item_number_id'].="#".$val[csf('po_break_down_id')]."**".$val[csf('item_number_id')]; 
			}
			else
			{
				 $production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['item_number_id']=$val[csf('po_break_down_id')]."**".$val[csf('item_number_id')]; 
			...
			if($production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['po_id']!="")
			{
				$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['po_number'].=",".$val[csf('po_number')];
				$production_data_arr[$groupStr][$floor_id][$sewing_line]['file'].=",".$val[csf('file_no')];
				$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['ref'].=",".$val[csf('ref')]; 
			}
			else
			{
				$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['po_number']=$val[csf('po_number')];
				$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['file']=$val[csf('file_no')]; 
				$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['ref']=$val[csf('ref')]; 
			}
			
			$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['quantity']+=$val[csf('good_qnty')];
			$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['reject_qty']+=$val[csf('reject_qty')]+$val[csf('alter_qty')]+$val[csf('spot_qty')];
			$production_data_arr_qty[$groupStr][$prod_date][$floor_id][$sewing_line][$val[csf('item_number_id')]]['quantity']+=$val[csf('good_qnty')];//[$val[csf('po_break_down_id')]]
			
			$sewingOutputQtyArr[$val[csf('po_break_down_id')]][$val[csf('item_number_id')]][$sewing_line]+=$val[csf('good_qnty')];
    ...
    {
        $all_po_arr_chunk=array_chunk($all_po_id_arr,999) ;
        foreach($all_po_arr_chunk as $chunk_arr)
        {
            $chunk_arr_value=implode(",",$chunk_arr);
            $poCond.=" b.id in($chunk_arr_value) or ";
        }

        $poIds_cond.=" and (".chop($poCond,'or ').")";
    }
    else
    {
        $poIds_cond=" and b.id in(".implode(',', $all_po_id_arr).")";
    }

	$po_id_cond = str_replace("b.id", "po_break_down_id", $poIds_cond);
	$line_tot_prod_arr = return_library_array("select sewing_line,sum(production_quantity) as production_qnty from pro_garments_production_mst where status_active=1 and is_deleted=0 and production_date between '$start_date' and $txt_date $po_id_cond group by sewing_line","sewing_line","production_qnty");

	// echo "select sewing_line,sum(production_quantity) as production_qnty from pro_garments_production_mst where status_active=1 and is_deleted=0 and production_date between '$start_date' and $txt_date $po_id_cond group by sewing_line";die();
	// echo "<pre>";print_r($line_tot_prod_arr);die();
	/*===============================================================================/
	/							Subcon Gmts production data							 /
	/============================================================================== */
	if($db_type==0)
	{
	...
	{
		$sql_sub_contuct= "select b.subcon_job, a.company_id, a.location_id, a.floor_id, a.production_date, a.line_id, a.gmts_item_id, b.party_id as buyer_name, a.prod_reso_allo, a.order_id, c.order_no as po_number, c.cust_style_ref, avg(c.smv) as smv, a.prod_reso_allo, sum(a.production_qnty) as good_qnty,"; 
		$first=1;
		for($h=$hour;$h<$last_hour;$h++)
		{
			$bg=$start_hour_arr[$h];
		...
		$sql_sub_contuct.="sum(CASE WHEN TO_CHAR(a.hour,'HH24:MI')>'$start_hour_arr[$last_hour]' and TO_CHAR(a.hour,'HH24:MI')<='$start_hour_arr[24]' and a.production_type=2 THEN a.production_qnty else 0 END) AS prod_hour23 from subcon_ord_mst b, subcon_ord_dtls c, subcon_gmts_prod_dtls a where a.production_type=2 and a.order_id=c.id and c.job_no_mst=b.subcon_job and a.status_active=1 and a.is_deleted=0 and b.is_deleted=0 and c.is_deleted=0 and a.company_id=$comapny_id  and a.production_date between '".$last_month_start_date."' and  $txt_date $subcon_location $floor $subcon_line group by b.subcon_job, a.company_id, a.location_id, a.floor_id, a.order_id, a.production_date, a.line_id, a.gmts_item_id, b.party_id, c.order_no, c.cust_style_ref, a.prod_reso_allo order by a.location_id, a.floor_id, a.prod_reso_allo";
	}
	// echo $sql_sub_contuct;die();
	$sub_result=sql_select($sql_sub_contuct);
	$slineWiseJobGmtsItemArr=array();
	$slineWiseStyleGmtsItemArr=array();
	foreach($sub_result as $subcon_val)
	{
		$produDate=""; 
		$sewing_line="";
		$produDate=$subcon_val[csf('production_date')];
		$sewing_line=$subcon_val[csf('line_id')];
		$floor_id=$subcon_val[csf('floor_id')];
		$produDate=date("d-M-Y",strtotime($produDate));

		$jobGrp=$subcon_val[csf('subcon_job')].'__'.$subcon_val[csf('cust_style_ref')].'__'.$subcon_val[csf('gmts_item_id')];			
		$slineWiseJobGmtsItemArr[$produDate][$floor_id][$sewing_line].=$jobGrp.',';
		
		if($subcon_val[csf('prod_reso_allo')]==1) $sewing_line_id=$prod_reso_arr[$sewing_line];
		else $sewing_line_id=$sewing_line;
		...
		$slineWiseStyleGmtsItemArr[$produDate][$floor_id][$prod_resource_array[$sewing_line][$produDate]['line_chief']][$slNo][$sewing_line]=$sewing_line;
	}


	
	$sql="SELECT a.buyer_id, a.style_ref, a.gmts_item_id, b.operator_smv, b.helper_smv from ppl_gsd_entry_mst a, ppl_gsd_entry_dtls b where a.id=b.mst_id and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1";

	$sql_result=sql_select($sql); $ws_sewing_smvArr=array();
	foreach($sql_result as $row)
	{
		$smv=0;
		$smv=$row[csf('operator_smv')]+$row[csf('helper_smv')];
		$ws_sewing_smvArr[$row[csf('buyer_id')]][$row[csf('style_ref')]][$row[csf('gmts_item_id')]]+=$smv;
	}
	unset($sql_result);
 
	//var_dump($ws_sewing_smvArr[219]['482665 RAW-016.LD.JNR'][30]); 
	
	//$lineHoursArr[$groupStr][$prod_date][$floor_id][$sewing_line][$prod_hour]
	$lineActiveHoursArr=array();
	foreach($lineHoursArr as $jobStr=>$jobRow)
	{		
		foreach($jobRow as $proDate=>$dateRow)
		{
			foreach($dateRow as $floorid=>$floorRow)
			{
				foreach($floorRow as $sewingline=>$lineRow)
				{
					foreach($lineRow as $prodHour=>$qty)
					{
						if(($qty*1)>0)
						$lineActiveHoursArr[$jobStr][$proDate][$floorid][$sewingline]+=1;
					}
				}
			}
		}
	...
	// ===================================== getting first style of gmts prod =====================================

	$sql = "SELECT a.STYLE_REF_NO,c.SEWING_LINE from wo_po_details_master a,wo_po_break_down b,pro_garments_production_mst c where a.id=b.job_id and b.id=c.po_break_down_id and c.production_type=5 and c.production_date=$txt_date $poIds_cond group by a.STYLE_REF_NO,c.SEWING_LINE,c.production_hour order by c.production_hour asc";
	// echo $sql;die;
	$res = sql_select($sql);
	$first_prod_style_array = array();
	$first_style_chk_array = array();
	foreach ($res as $val) 
	{
		if($first_style_chk_array[$val['SEWING_LINE']]=="")
		{
			$first_prod_style_array[$val['SEWING_LINE']][$val['STYLE_REF_NO']] = $val['STYLE_REF_NO'];
			$first_style_chk_array[$val['SEWING_LINE']] = "porimoni";
		}
		
	}
	// echo "<pre>";print_r($first_prod_style_array);die();
	...
	$style_arr3 = array();
	$style_arr4 = array();
	foreach($lineWiseStyleGmtsItemArr[$txt_date] as $f_id=>$fname)
	{
		foreach($fname as $line_cheaf=>$line_cheaf_data)
		{
			foreach($line_cheaf_data as $sl=>$s_data)
			{
				...
				{
					$jobItemStrs=array_unique(array_filter(explode("###",$lineWiseJobGmtsItemArr[$txt_date][$f_id][$ldata])));
					foreach($jobItemStrs as $jobData)
					{
						$style_arr[$l_id]++;
						$style_arr2[$l_id]++;
						$style_arr3[$l_id]++;
	...
	foreach($slineWiseStyleGmtsItemArr[$txt_date] as $f_id=>$fname)
	{
		foreach($fname as $line_cheaf=>$line_cheaf_data)
		{
			foreach($line_cheaf_data as $sl=>$s_data)
			{
				foreach($s_data as $l_id=>$ldata)
				{
					$jobItemStrs=array_unique(array_filter(explode(",",$slineWiseJobGmtsItemArr[$txt_date][$f_id][$ldata])));
					foreach($jobItemStrs as $jobData)
					{
						$style_arr[$l_id]++;
						$style_arr2[$l_id]++;
	...
	$j=0; $smv_for_item=""; $line_floor_production=0; $line_total_production=0; $gnd_total_fob_val=0; $floor_serial_no=0; $line_cheaf_serial=0;
	...
	$rowspan_arr = array();
	$mr_rowspan_arr = array();
	
	$line_chief_rowspan_arr = array();
	$line_wise_tot_effi_arr = array();
		...
		foreach($fname as $line_cheaf=>$line_cheaf_data)
		{
			foreach($line_cheaf_data as $sl=>$s_data)
			{
				foreach($s_data as $l_id=>$ldata)
				{
					...
					{
						$rowspan_arr[$f_id]++;
						$mr_rowspan_arr[$f_id][$l_id]++;
						$line_chief_rowspan_arr[$f_id][$line_cheaf]++;
					

						// ========================== get line total efficiency ========================

						$jobNo=$styleRef=$itemNo="";
						$exjobData=explode("__",$jobData);
						
						$jobNo=$exjobData[0];
						$styleRef=$exjobData[1];
						$itemNo=$exjobData[2];
						//echo $itemNo.'<br>';
						$item_pos=array_unique(array_filter(explode(",",$production_data_arr[$jobData][$txt_date][$f_id][$ldata]['po_id'])));
						$order_no_total=""; $sewing_output=0; 
						foreach($item_pos as $poid)
						{
							if($order_no_total!="") $order_no_total.=",";
							$order_no_total.=$poid;
							$sewing_output+=$sewingOutputQtyArr[$poid][$itemNo][$ldata];
							
							$po_wise_target_min+=($item_smv_array[$poid][$itemNo]*1)*$po_val;
						}
						
						$buyer_name=""; $buyer_id=0;			
						$buyer_id=$production_data_arr[$jobData][$txt_date][$f_id][$ldata]['buyer_name'];
						$buyer_name=$buyerArr[$buyer_id];
						
						$garment_itemname=''; $item_smv=""; $smv_for_item=""; $produce_minit="";  $efficiency_min=0; $efficiency_min_summary=0; $po_wise_target_min=0; $index=''; $produce_minit=0;
						$garment_item_arr=array(); $item_id_arr=array();
						
						$style_ref=$styleRef;
						
						$wsSmv=$ws_sewing_smvArr[$buyer_id][$style_ref][$itemNo]*1;
						if($wsSmv>0) { $item_smv=$wsSmv; $smvTd=""; }
						else { $item_smv=$item_smv_array[$jobNo][$itemNo]; $smvTd="red"; }
						
						$produce_minit=$production_po_data_arr[$jobData][$txt_date][$f_id][$ldata]*$item_smv;
						$po_wise_target_min=$production_data_arr_qty[$jobData][$txt_date][$f_id][$ldata][$itemNo]['quantity']*$item_smv;
						//echo $produce_minit.'<br>';
						$gmtsItemName=$garments_item[$itemNo];						
						...
						$type_line=$production_data_arr[$jobData][$txt_date][$f_id][$ldata]['type_line'];
						//$prod_reso_allo=$line_data['prod_reso_allo'];
						//echo $production_data_arr[$jobData][$txt_date][$f_id][$ldata]['prod_reso_allo'];die;
						$sewing_line=''; $poly_line=0;
						
						if($production_data_arr[$jobData][$txt_date][$f_id][$ldata]['prod_reso_allo']==1)
						{
						...
						if($lunch_start!="") $lunch_start_hour=$lunch_start; 
						else $lunch_start_hour=$lunch_hour; 
							 //***********************************************************************************			  
						$production_hour=array();
						for($h=$hour;$h<=$last_hour;$h++)
						{
							 $prod_hour="prod_hour".substr($line_start_hour_arr[$h],0,2).""; 
							 $production_hour[$prod_hour]=$production_data_arr[$jobData][$txt_date][$f_id][$ldata][$prod_hour];
							 $floor_production[$prod_hour]+=$production_data_arr[$jobData][$txt_date][$f_id][$ldata][$prod_hour];
							 $total_production[$prod_hour]+=$production_data_arr[$jobData][$txt_date][$f_id][$ldata][$prod_hour];
						}
								
						$floor_production['prod_hour24']+=$production_data_arr[$jobData][$txt_date][$f_id][$ldata]['prod_hour23'];
						$total_production['prod_hour24']+=$production_data_arr[$jobData][$txt_date][$f_id][$ldata]['prod_hour23'];
						$production_hour['prod_hour24']=$production_data_arr[$jobData][$txt_date][$f_id][$ldata]['prod_hour23']; 
						
						if(str_replace("'","",$actual_production_date)==str_replace("'","",$actual_date)) 
						{
							if($type_line==2) $line_start=$production_data_arr[$jobData][$txt_date][$f_id][$ldata]['prod_start_time']; //No Profuction Line
							else $line_start=$line_number_arr[$ldata][$txt_date]['prod_start_time'];
							...
							for($lh=$line_start_hour;$lh<=$last_hour;$lh++)
							{
								...
								if($lh<$actual_time)
								{
									...
									$line_production_hour+=$production_data_arr[$jobData][$txt_date][$f_id][$ldata][$line_hour];
									$line_floor_production+=$production_data_arr[$jobData][$txt_date][$f_id][$ldata][$line_hour];
									$line_total_production+=$production_data_arr[$jobData][$txt_date][$f_id][$ldata][$line_hour];
							...
							if($type_line==2)
							{
								if($total_eff_hour>$line_data['working_hour']) $total_eff_hour=$line_data['working_hour'];
							}
							else
							{
								if($total_eff_hour>$prod_resource_array[$ldata][$txt_date]['working_hour']) $total_eff_hour=$prod_resource_array[$ldata][$txt_date]['working_hour'];
							}
						}
						if(str_replace("'","",$actual_production_date)<str_replace("'","",$actual_date)) 
						{
							for($ah=$hour;$ah<=$last_hour;$ah++)
							{
								...
								$line_production_hour+=$production_data_arr[$jobData][$txt_date][$f_id][$ldata][$prod_hour];
								$line_floor_production+=$production_data_arr[$jobData][$txt_date][$f_id][$ldata][$prod_hour];
								$line_total_production+=$production_data_arr[$jobData][$txt_date][$f_id][$ldata][$prod_hour];
							}
							
							$total_eff_hour=$prod_resource_array[$ldata][$txt_date]['working_hour'];	
						}
						
						if($sewing_day!="") $days_run= $diff=datediff("d",$sewing_day,$txt_date); else $days_run=0;
						//******************************* line effiecency***********************************************['']
						$current_wo_time=0;
						...
						$target_efficiency=0;
						// if($po_wise_target_min>0) $target_efficiency=($po_wise_target_min*$total_eff_hour);
						//die;
						...
						// $eff_target=$item_smv*$style_resource_array[$style_ref][$txt_date][$f_id][$ldata]['target_per_line'];
						...
						$extra_worker=$style_resource_array[$style_ref][$txt_date][$f_id][$ldata]['extra_worker'];
						//$action_machine=$prod_resource_array[$ldata][$txt_date]['active_operator'];	
						//$man_power=$prod_resource_array[$ldata][$txt_date]['man_power'];
						$action_machine=$style_resource_array[$style_ref][$txt_date][$f_id][$ldata]['active_operator'];
							
						//$operator=$prod_resource_array[$ldata][$txt_date]['operator'];
						$operator=$style_resource_array[$style_ref][$txt_date][$f_id][$ldata]['active_operator'];//$prod_resource_array[$ldata][$txt_date]['active_operator'];
						//$helper=$prod_resource_array[$ldata][$txt_date]['helper'];
						$helper=$style_resource_array[$style_ref][$txt_date][$f_id][$ldata]['active_helper'];//$prod_resource_array[$ldata][$txt_date]['active_helper'];
						$man_power=$operator+$helper;
						$terget_hour=$style_resource_array[$style_ref][$txt_date][$f_id][$ldata]['target_per_line'];	
						$capacity=$prod_resource_array[$ldata][$txt_date]['capacity'];
						//$working_hour=$prod_resource_array[$ldata][$txt_date]['working_hour'];
						$working_hour=$lineActiveHoursArr[$jobData][$txt_date][$f_id][$ldata];
						//$production_data_arr[$jobData][$txt_date][$f_id][$ldata][$prod_hour];

						$style_working_hour = $style_resource_array[$style_ref][$txt_date][$f_id][$ldata]['style_working_hour'];
						$target_per_line = $style_resource_array[$style_ref][$txt_date][$f_id][$ldata]['target_per_line'];
						$eff_target = $target_per_line*$style_working_hour;


						$target_efficiency=($item_smv*$eff_target);

						$efficiency_min += ($man_power*$working_hour*60);
						// $efficiency_min+=($man_power+$extra_worker)*($working_hour*60);

						// =========================== Extra hour smv add ====================================
						$total_smv = 0;
						$total_smv_others = 0;
						// echo $style_arr[$ldata]."<br>";
						/*if($style_arr3[$ldata]>1) // when multiple style running a line
						{
							if($style_arr4[$ldata]>1)
							{
								$total_smv = $prod_resource_smv_adj_array[$ldata][$txt_date][2]['total_smv'];
								if($pp==0)
								{
									$efficiency_min = $efficiency_min - $total_smv;
									$pp++;
								}
								// echo $efficiency_min ."-". $total_smv."<br>";
								// $extra_worker  = $prod_resource_smv_adj_array[$ldata][$txt_date][2]['number_of_emp'];
								$extra_worker  = 0;
								$extra_hour  = 0;
							...
							{
								$total_smv = $prod_resource_smv_adj_array[$ldata][$txt_date][1]['total_smv'];
								// $total_smv_others = $prod_resource_smv_adj_array[$ldata][$txt_date][1]['total_smv'];
								$efficiency_min = $efficiency_min + $total_smv;
								$extra_worker  = $prod_resource_smv_adj_array[$ldata][$txt_date][1]['number_of_emp'];
								$extra_hour  = $prod_resource_smv_adj_array[$ldata][$txt_date][1]['adjust_hour'];
							}
						}
						else
						{
							$total_smv = $prod_resource_smv_adj_array[$ldata][$txt_date][1]['total_smv'];
							$total_smv_others = $prod_resource_smv_adj_array[$ldata][$txt_date][2]['total_smv'];
							$efficiency_min = $efficiency_min + $total_smv -$total_smv_others;
							$extra_worker  = $prod_resource_smv_adj_array[$ldata][$txt_date][1]['number_of_emp'];
							$extra_hour  = $prod_resource_smv_adj_array[$ldata][$txt_date][1]['adjust_hour'];
						}*/
						...
						// =========================================================						
						$extra_worker  = $prod_resource_smv_adj_style_array[$ldata][$style_ref][$txt_date][1]['number_of_emp'];
						$extra_hour  = $prod_resource_smv_adj_style_array[$ldata][$style_ref][$txt_date][1]['adjust_hour'];
						$total_smv  = $prod_resource_smv_adj_style_array[$ldata][$style_ref][$txt_date][1]['total_smv'];

						$total_smv_others = $prod_resource_smv_adj_style_array[$ldata][$style_ref][$txt_date][2]['total_smv'];

						$efficiency_min = $efficiency_min + $total_smv -$total_smv_others;
						
						$line_efficiency=($produce_minit/$efficiency_min)*100;
						// echo "(".$produce_minit."/".$efficiency_min.")*100<br>";
						$line_wise_tot_effi_arr[$l_id] += $line_efficiency;
	...
	// echo "<pre>";print_r($production_serial_arr[$txt_date]);echo "</pre>";die();
	// echo "<pre>";print_r($line_chief_rowspan_arr);echo "</pre>";die();
	...
	$floor_smv=$floor_row=$floor_helper=$floor_tgt_h=$floor_days_run=$floor_working_hour=$line_floor_production=$floor_today_product=$floor_avale_minute=0;
	$total_operator=$total_helper=$gnd_hit_rate=0; $total_smv=$total_terget=$grand_total_product=$gnd_line_effi=0; $total_man_power=$gnd_avable_min=$gnd_product_min=0;
	$item_smv=$item_smv_total=$line_efficiency=$days_run=$total_working_hour=$gnd_total_tgt_h=$total_capacity=$grand_particulars_hour=0;
	$j=0; $smv_for_item=""; $line_floor_production=0; $line_total_production=0; $gnd_total_fob_val=0; $floor_serial_no=0; $line_cheaf_serial=0;
	
	$line_number_check_arr=array(); $total_production=array(); $floor_production=array(); $graph_line_arr=array(); $graph_line_reject_arr=array(); $floor_serial_data_arr=array(); $line_cheaf_serial_data_arr=array();
	$line_count_array = array();
	$pp = 0;
	foreach($production_serial_arr[$txt_date] as $f_id=>$fname)
	{
		ksort($fname); 
		$floor_line_num=0; $floot_line_total=0;
		$f = 0;
		...
		foreach($fname as $line_cheaf=>$line_cheaf_data)
		{
			...
			// echo $line_cheaf."=".$line_cheaf_total_row."<br>";
			$line_cheaf_row_count=0;
			foreach($line_cheaf_data as $sl=>$s_data)
			{				
				foreach($s_data as $l_id=>$ldata)
				{
					$tot_line_efficiency = 0;
					$l=0;
					if($i!=1)
					{
						if(!in_array($f_id, $check_arr))
						{
							// firest content============================================================================
							 $html.='<tr  class="tbl_bottom">
									...
									<td width="80">&nbsp;</td>
									<td width="60" align="center" title="(Prod. Qty*SMV)/Prod. Qty">'.number_format($floor_produc_min/$line_floor_production,2).'</td>
									...
									<td align="center" width="60">'.$floor_spend_min.'</td>
									<td align="center" width="60">'.$floor_extra_hour.'</td>
									<td align="center" width="60">'.$floor_particulars_hour.'</td>
									...
									<td align="center" width="70">'.number_format($floor_tgt_h,2).'</td>
									<td align="center" width="80">'.number_format($eff_target_floor,2).'</td>';
							  ...
							  $floor_working_hour=0; $line_floor_production=0; $floor_today_product=0; $floor_avale_minute=0; $floor_produc_min=0; $floor_efficency=0; $floor_man_power=0; $floor_capacity=0; $floor_sewing_input=0; $floor_action_machine=0; $floor_target_efficiency=0; $floor_extra_hour=0;$floor_particulars_hour=0; $floor_extra_worker=0;$floor_spend_min=0; $floor_target_efficiency_manual=0;
							  $j=0;
						}
					}
					//echo $l_id.'='.$ldata.'<br>';
					$j++;
					// echo $floor_line_num."kakku<br>";
					// $floor_line_num++;
					$germents_item_row=array();
					$jobItemStr=array_unique(array_filter(explode("###",$lineWiseJobGmtsItemArr[$txt_date][$f_id][$ldata])));
					$jobNo=$styleRef=$itemNo="";
					foreach($jobItemStr as $jobData)
					{
						$floor_line_num++;
						$jobNo=$styleRef=$itemNo="";
						$exjobData=explode("__",$jobData);
						...
						// echo $jobData."=".$txt_date."=".$f_id."=".$ldata."<br>";
						if($production_data_arr[$jobData][$txt_date][$f_id][$ldata]['prod_reso_allo']==1)
						{
							$line_number=explode(",",$prod_reso_arr[$ldata]);
							foreach($line_number as $val)
						...
						//$action_machine=$prod_resource_array[$ldata][$txt_date]['active_operator'];	
						//$man_power=$prod_resource_array[$ldata][$txt_date]['man_power'];
						$action_machine=$style_resource_array[$first_prod_style_array[$l_id][$style_ref]][$txt_date][$f_id][$ldata]['active_operator'];

						// echo $first_prod_style_array[$l_id][$style_ref]."=".$style_ref."=".$txt_date."=".$f_id."=".$l_id."<br>";
						...
						$man_power=$operator+$helper;
						// $terget_hour=$style_resource_array[$style_ref][$txt_date][$f_id][$ldata]['target_per_line'];	
						$terget_hour=($man_power*60*($target_efficiency_manual/100))/$item_smv;	
						...
						$total_smv_others = 0;
						// echo $style_arr[$ldata]."<br>";
						/*if($style_arr2[$ldata]>1) // when multiple style running a line
						{
							if($style_arr[$ldata]>1)
							{
								$total_smv = $prod_resource_smv_adj_array[$ldata][$txt_date][2]['total_smv'];
								if($pp==0)
								{
						...
						$particulars_hour = $prod_resource_smv_adj_style_array[$ldata][$style_ref][$txt_date][2]['total_smv'];
						$adjustment_source = $prod_resource_smv_adj_style_title_array[$ldata][$style_ref][$txt_date];
						$adjustment_source_arr = array_unique(array_filter(explode(",", $adjustment_source)));
						$particulars_title = "";
						foreach ($adjustment_source_arr as $source_id) 
						{
							if($source_id==2)
							{
								$particulars_title .= ($particulars_title=="") ? "Lunch Out" : " + Lunch Out";
							}
							elseif ($source_id==3) 
							{
								$particulars_title .= ($particulars_title=="") ? "Sick Out" : " + Sick Out";
							}
							elseif ($source_id==4) 
							{
								$particulars_title .= ($particulars_title=="") ? "Leave Out" : " + Leave Out";
							}
							elseif ($source_id==5) 
							{
								$particulars_title .= ($particulars_title=="") ? "Late In" : " + Late In";
							}
							elseif ($source_id==6) 
							{
								$particulars_title .= ($particulars_title=="") ? "General Out" : " + General Out";
							}
							elseif ($source_id==7) 
							{
								$particulars_title .= ($particulars_title=="") ? "NPT" : " + NPT";
							}
						}

						// $efficiency_min = $efficiency_min - $particulars_hour;
						
						$line_efficiency=($produce_minit/$efficiency_min)*100;
						$tot_line_efficiency += $line_efficiency;

						$spend_min = $working_hour*$man_power*60;

						$eff_target = ($efficiency_min*$terget_hour)/($man_power*60);
						$target_efficiency=($item_smv*$eff_target);
						
						$floor_capacity+=$prod_resource_array[$ldata][$txt_date]['capacity'];
						//$prod_resource_array[$ldata][$txt_date]['helper'];
						$floor_tgt_h+=$terget_hour;//$prod_resource_array[$ldata][$txt_date]['terget_hour'];	
						$floor_working_hour+=$working_hour;//$prod_resource_array[$ldata][$txt_date]['working_hour']; 
						...
						$floor_particulars_hour+=$particulars_hour;
						$floor_extra_worker+=$extra_worker;
						$floor_spend_min+=$spend_min;
						$floor_efficency=($floor_produc_min/$floor_avale_minute)*100;
						
						// $total_operator+=$operator;//$prod_resource_array[$ldata][$txt_date]['operator'];
						$total_man_power+=$man_power;//$prod_resource_array[$ldata][$txt_date]['man_power'];
						$total_helper+=$helper;//$prod_resource_array[$ldata][$txt_date]['helper'];
						$total_capacity+=$prod_resource_array[$ldata][$txt_date]['capacity'];
						$total_working_hour+=$working_hour;//$prod_resource_array[$ldata][$txt_date]['working_hour']; 
						$gnd_total_tgt_h+=$terget_hour;//$prod_resource_array[$ldata][$txt_date]['terget_hour'];
						$grand_extra_hour+=$extra_hour;
						$grand_particulars_hour+=$particulars_hour;
						$grand_extra_worker+=$extra_worker;
						$grand_spend_min+=$spend_min;
						...
						$po_id=rtrim($production_data_arr[$jobData][$txt_date][$f_id][$ldata]['po_id'],',');
						$po_id=array_unique(explode(",",$po_id));
						$style=rtrim($production_data_arr[$jobData][$txt_date][$f_id][$ldata]['style_ref'],',');
					
						$style=implode(",",array_unique(explode(",",$style)));
						$po_id=$production_data_arr[$jobData][$txt_date][$f_id][$ldata]['po_id'];
					   ...
					   //********************************* calclution floor total    *******************$txt_date],$sewing_day
						...
						if($l==0)
						{
							$grand_sewing_output+=$line_tot_prod_arr[$l_id];
							$floor_sewing_output+=$line_tot_prod_arr[$l_id];
						}
						...
						$extra_min = $extra_hour*$extra_worker*60;		
						...
						$html.='<td width="40" align="center">'.$i.'&nbsp;</td>
								<td width="100" align="center" title="'.$f_id.'">'.$floor_name.'&nbsp; </td>
								<td align="center" width="80" title="'.$line_cheaf.'='.$sl.'='.$ldata.'">'. $sewing_line.'&nbsp; </td>
								<td title="Buyer:'.$buyer_id.', Style:'.$style_ref.', Item:'.$itemNo.'" align="center" width="60" bgcolor="'.$smvTd.'"><p>'.number_format($item_smv,2).'</p></td>';
								if($l==0)
								{
									$html.='<td valign="middle" rowspan="'.$mr_rowspan_arr[$f_id][$l_id].'" align="center" width="60">'.$action_machine.'</td>									
								
									<td  valign="middle" rowspan="'.$mr_rowspan_arr[$f_id][$l_id].'" align="center" width="60">'.$operator.'</td>
								...
								if($l==0)
								{
									$html.='<td valign="middle" rowspan="'.$mr_rowspan_arr[$f_id][$l_id].'" align="center" width="60">'.$helper.'</td>									
								
									<td  valign="middle" rowspan="'.$mr_rowspan_arr[$f_id][$l_id].'" align="center" width="60">'.$man_power.'</td>
								...
								<td width="60" align="center">'.$spend_min.'</td>
								<td width="60" align="center" title="'.$extra_min.'">'.$extra_hour.'</td>';
								if($particulars_hour!="")
								{
									$html.='<td width="60" align="center" title="'.$particulars_title.'">-'.$particulars_hour.'</td>';
								}
								else
								{
									$html.='<td width="60" align="center" title="'.$particulars_title.'">'.$particulars_hour.'</td>';
								}
								$html.='<td align="center" width="100">'.$efficiency_min.'</td>
								<td width="80" align="center"><p>'.$buyer_name.'&nbsp;</p></td>
								
								<td width="100" align="center"><p>'.$style_ref.'&nbsp;</p></td>
								<td width="100" align="center"><p>'.$gmtsItemName.'&nbsp;</p></td> 
								...
								<td align="center" width="70">'. number_format($terget_hour,2).'</td>
								<td align="center" width="80" title="Target per line='.$target_per_line.'*Working hour='.$style_working_hour.'">'. number_format($eff_target,2).'</td>';
								$tday_line_qc_pass=0;
								for($k=$hour; $k<=$last_hour; $k++)
								{
									$prod_hour="prod_hour".substr($start_hour_arr[$k],0,2)."";
									
									if($start_hour_arr[$k]==$lunch_start_hour) $bg_color='background:yellow';
									else if($terget_hour>$production_hour[$prod_hour])
									{
										$bg_color='background:red';
										if($production_hour[$prod_hour]==0) $bg_color='';
									}
									else if($terget_hour<$production_hour[$prod_hour])
									{
								...
								$html.='<td align="center" width="80"><a href="##" onclick="openmypage('.$cbo_company_id.",'".$jobData."','main',".$f_id.",".$ldata.",'tot_prod','".$item_smv."','".$actual_time_hour."','".$line_start."','".$txt_date."'".')">'.$line_production_hour.'</a></td>';
									if($l==0)
									{
										$html.='<td rowspan="'.$mr_rowspan_arr[$f_id][$l_id].'" align="center" width="80">'.$line_tot_prod_arr[$l_id].'</td>';
									}

									if($line_efficiency<=$txt_parcentage) $html.='<td title="(Produce min='.$produce_minit.'/Spend min='.$efficiency_min.')*100" align="center" width="80" bgcolor="red">'.number_format($line_efficiency,2).'%</td>';
									else $html.='<td title="(Produce min='.$produce_minit.'/Spend min='.$efficiency_min.')*100" align="center" width="80">'.number_format($line_efficiency,2).'%</td>'; 
									if($l==0)
									{
										$line_efficiency2 = $line_wise_tot_effi_arr[$l_id]/$mr_rowspan_arr[$f_id][$l_id];
										$html.='<td title="(Tot Efi='.$line_wise_tot_effi_arr[$l_id].'/Tot Line='.$mr_rowspan_arr[$f_id][$l_id].')" valign="middle" rowspan="'.$mr_rowspan_arr[$f_id][$l_id].'" align="center" width="80">'.number_format($line_efficiency2,2).'</td>';
										$l++;
									}
									$html.='<td align="center" width="60" >'.number_format($target_efficiency,0).'</td>
											<td width="100" align="center"><a href="##" onclick="openmypage('.$cbo_company_id.",'".$jobData."','main',".$f_id.",".$ldata.",'tot_prod','".$item_smv."','".$actual_time_hour."','".$line_start."','".$txt_date."'".')">'.number_format($produce_minit,0).'</a></td>
											<td align="center" width="70" >'. $days_run.'</td>
											<td align="center" width="70" >'. number_format(($line_production_hour/$eff_target)*100,2).'%</td>';
									if($line_cheaf_row_count==0)
									{
										$line_cheaf_serial++;
										$html.='<td align="center" width="70" rowspan="'.$line_chief_rowspan_arr[$f_id][$line_cheaf].'" id="line_cheaf_sl'.$line_cheaf_serial.'" valign="middle" ></td>';

										$line_cheaf_row_count++;
									}
									// echo $floor_line_num."==".$f_id."<br>";
									if($f==0)
									{
										//echo $floot_line_total;die;
										$floor_serial_no++;
										$f++;
										$html.='<td align="center" width="80" rowspan="'.$rowspan_arr[$f_id].'"   id="line_floor_sl'.$floor_serial_no.'"  valign="middle" ></td>';
									}
									$html.='<td width="" id="remarks" title="'.$fob_rate.'" align="left"><a href="##" onclick="show_remarks_popup('.$ldata.",'".$style_ref."'".')">View</a></td>';
    ...
    <div>
    	...
    	/================================================================================ -->
    	<fieldset>
	        <label><strong>Daily Summary ( In House order ):-</strong></label>
	    	<?php $table_width=2460+($last_hour-$hour)*50; ?>
	        <table id="table_header_1" class="rpt_table" width="<?php echo $table_width; ?>" cellpadding="0" cellspacing="0" border="1" rules="all">
	            <thead>
	                 <tr height="50">
	                    ...
	                    <th width="60">Spent Minutes</th>
	                    <th width="60">Extra Hrs</th>
	                    <th width="60">Particulars Min O/I</th>
	                    <th width="100">Total Spent Minutes</th>
	                    ...
	                    <th width="80">Style Efficiency. %</th>
	                    <th width="80">Line Efficiency. %</th>
	            ...
	            <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body">
	                ...
	                <tfoot >
	                	 <tr>
	                        ...
	                        <th width="60" title="(Prod. Qty*SMV)/Prod. Qty"><? echo number_format(($gnd_product_min/$line_total_production),2); ?></th>
	                        ...
	                        <th align="center" width="60"><?php  echo $grand_spend_min; ?></th>
	                        <th align="center" width="60"><?php  echo $grand_extra_hour; ?></th>
	                        <th align="center" width="60"><?php  echo $grand_particulars_hour; ?></th>
	                        ...
	                        <th width="70"><?  echo number_format($gnd_total_tgt_h,2); ?></th>
	                        <th width="70"><?  echo number_format($total_terget,2); ?></th>
	                        <?
							for($k=$hour; $k<=$last_hour; $k++)
							{
								$prod_hour="prod_hour".substr($start_hour_arr[$k],0,2)."";
	                        ...
	                        <th align="center" width="80"><? echo number_format(($gnd_product_min*100)/$gnd_avable_min,2)."%"; ?></th>
	                        <th align="center" width="60"><? echo number_format($grand_target_efficiency,2); ?></th>
		...
		//echo $sql_sub_contuct;
		$sub_result=sql_select($sql_sub_contuct);
		$subcon_order_smv=array(); $slineWiseJobGmtsItemArr=array(); $sproduction_data_arr=array(); $slineHoursArr=array();
		foreach($sub_result as $subcon_val)
		{
			$produDate=""; $sewing_line="";
			$produDate=$subcon_val[csf('production_date')];
			$sewing_line=$subcon_val[csf('line_id')];
			$floor_id=$subcon_val[csf('floor_id')];
			$produDate=date("d-M-Y",strtotime($produDate));
			...
			$sproduction_data_arr[$jobGrp][$produDate][$floor_id][$sewing_line]['serial']=$slNo;
			$sproduction_serial_arr[$produDate][$floor_id][$prod_resource_array[$sewing_line][$produDate]['line_chief']][$slNo][$sewing_line]=$sewing_line;
			$sproduction_data_arr[$jobGrp][$produDate][$floor_id][$sewing_line]['prod_reso_allo']=$subcon_val[csf('prod_reso_allo')];
			$sproduction_data_arr[$jobGrp][$produDate][$floor_id][$sewing_line]['buyer_name']=$subcon_val[csf('buyer_name')]; 
			$sproduction_data_arr[$jobGrp][$produDate][$floor_id][$sewing_line]['order_id'].=$subcon_val[csf('order_id')].","; 
			
			$production_po_data_arr[$jobGrp][$produDate][$floor_id][$sewing_line][$subcon_val[csf('order_id')]]+=$subcon_val[csf('good_qnty')];
		
			if($sproduction_data_arr[$jobGrp][$produDate][$floor_id][$sewing_line]['po_number']!="")
			{
				$sproduction_data_arr[$jobGrp][$produDate][$floor_id][$sewing_line]['po_number'].=",".$subcon_val[csf('po_number')];
				$sproduction_data_arr[$jobGrp][$produDate][$floor_id][$sewing_line]['style_ref'].=",".$subcon_val[csf('cust_style_ref')];  
			}
			else
			{
				$sproduction_data_arr[$jobGrp][$produDate][$floor_id][$sewing_line]['po_number']=$subcon_val[csf('po_number')]; 
			...
			$sproduction_data_arr[$jobGrp][$produDate][$floor_id][$sewing_line]['type_line']=1; 
			$sproduction_data_arr[$jobGrp][$produDate][$floor_id][$sewing_line]['smv']=$subcon_val[csf('smv')];
			$sproduction_data_arr[$jobGrp][$produDate][$floor_id][$sewing_line]['quantity']+=$subcon_val[csf('good_qnty')];
			
			$line_start=$line_number_arr[$val[csf('line_id')]][$subcon_val[csf('production_date')]]['prod_start_time']	;
			if($line_start!="") 
			{ 
				$line_start_hour=substr($line_start,0,2); 
				if(substr($line_start_hour,0,1)==0)  $line_start_hour=substr($line_start_hour,1,1);	
			...
			{
				$prod_hour="prod_hour".substr($start_hour_arr[$h],0,2).""; 
				$slineHoursArr[$jobGrp][$produDate][$floor_id][$sewing_line][$prod_hour]+=$subcon_val[csf($prod_hour)];
				//echo str_replace("'","",$actual_production_date).'='.str_replace("'","",$actual_date).'<br>';
				$sproduction_data_arr[$jobGrp][$produDate][$floor_id][$sewing_line][$prod_hour]+=$subcon_val[csf($prod_hour)]; 
				if(str_replace("'","",$actual_production_date)>=str_replace("'","",$actual_date)) 
				{
					 if( $h>=$line_start_hour && $h<=$actual_time)
					 {
						//echo $prod_hour;
					 	$sproduction_po_data_arr[$jobGrp][$produDate][$floor_id][$sewing_line]+=$subcon_val[csf($prod_hour)];
					 } 
				}
				if(str_replace("'","",$actual_production_date)>=str_replace("'","",$actual_date)) 
				{
			...
			{	
				if( $h>=$line_start_hour && $h<=$actual_time)
				{
					$sproduction_po_data_arr[$jobGrp][$produDate][$floor_id][$sewing_line]+=$subcon_val[csf('prod_hour23')];
					$slineHoursArr[$jobGrp][$produDate][$floor_id][$sewing_line][$prod_hour]+=$subcon_val[csf('prod_hour23')];
				} 	
			}
			else 
			{
			...
			$sproduction_data_arr_qty[$jobGrp][$produDate][$floor_id][$sewing_line]['quantity']+=$subcon_val[csf('good_qnty')];
			
			//$sproduction_data_arr[$jobGrp][$produDate][$floor_id][$sewing_line]['prod_hour23']+=$subcon_val[csf('prod_hour23')];
		}
	
		
		$slineActiveHoursArr=array();
		foreach($slineHoursArr as $jobStr=>$jobRow)
		{
			foreach($jobRow as $proDate=>$dateRow)
			{
				foreach($dateRow as $floorid=>$floorRow)
				{
					foreach($floorRow as $sewingline=>$lineRow)
					{
						...
						{
							if(($qty*1)>0)
								$slineActiveHoursArr[$jobStr][$proDate][$floorid][$sewingline]+=1;
						}
					}
				}
			}
		...
		/*echo "<pre>";
		print_r($sproduction_po_data_arr); die;*/
		$shtml=""; 
		$i=1;
		
		$floor_line_num=0; $floor_name=""; $floor_smv=0; $floor_row=0; $floor_operator=0; $floor_helper=0; $floor_tgt_h=0; $floor_man_power=0; $floor_days_run=0; $eff_target_floor=0; $floor_working_hour=0; $line_floor_production=0; $floor_today_product=0; $floor_avale_minute=0; $floor_produc_min=0; $floor_efficency=0; $floor_man_power=0; $floor_capacity=0; $floor_sewing_input=0; $floor_action_machine=0; $floor_target_efficiency=0; $floor_extra_hour=0;$floor_particulars_hour=0; $floor_extra_worker=0; $floor_target_efficiency_manual=0;$grand_particulars_hour=0;$floor_sewing_output=0;$sgrand_particulars_hour=0;$sewing_output=0; $floor_production=array(); 
	foreach($production_serial_arr[$txt_date] as $f_id=>$fname)
	{
		ksort($fname); 
		$floor_line_num=0; $floot_line_total=0;
		...
		foreach($fname as $line_cheaf=>$line_cheaf_data)
		{
			...
			$line_cheaf_row_count=0;
			//$sproduction_serial_arr[$produDate][$floor_id][$prod_resource_array[$sewing_line][$produDate]['line_chief']][$slNo][$sewing_line]
			foreach($line_cheaf_data as $sl=>$s_data)
			{
				$line_cheaf_row_count++;
				...
				foreach($s_data as $l_id=>$ldata)
				{
						...
						if(!in_array($f_id, $check_arr))
						{
							// firest content============================================================================
							 $html.='<tr  class="tbl_bottom">
							  ...
							  $sfloor_serial_data_arr[$floor_serial_no]=number_format($floor_efficency,2);
							
							  $floor_line_num=0; $floor_name=""; $floor_smv=0; $floor_row=0; $floor_operator=0; $floor_helper=0; $floor_tgt_h=0; $floor_man_power=0; $floor_days_run=0; $eff_target_floor=0;
							  unset($floor_production);
							  $floor_working_hour=0; $line_floor_production=0; $floor_today_product=0; $floor_avale_minute=0; $floor_produc_min=0; $floor_efficency=0; $floor_man_power=0; $floor_capacity=0; $floor_sewing_input=0; $floor_action_machine=0; $floor_target_efficiency=0; $floor_extra_hour=0;$floor_spend_min=0;$floor_particulars_hour=0; $floor_extra_worker=0; $floor_target_efficiency_manual=0;
							  $j=0;
						}
					}
					...
					$j++;
					$floor_line_num++;
					$germents_item_row=array(); $jobItemStr="";
					//echo $txt_date.'='.$f_id.'='.$ldata;
					$jobItemStr=array_unique(array_filter(explode(",",$slineWiseJobGmtsItemArr[$txt_date][$f_id][$l_id])));
					// print_r($jobItemStr);
					$jobNo=$styleRef=$itemNo="";
					foreach($jobItemStr as $jobData)
					{
						$exjobData=$jobNo=$styleRef=$itemNo="";
						$exjobData=explode("__",$jobData);
						// print_r($exjobData);
						$jobNo=$exjobData[0];
						$styleRef=$exjobData[1];
						$itemNo=$exjobData[2];
						//echo $itemNo.'<br>';
						$item_pos=array_unique(array_filter(explode(",",$production_data_arr[$jobData][$txt_date][$f_id][$l_id]['po_id'])));
						$order_no_total=""; $sewing_output=0; 
						foreach($item_pos as $poid)
						{
							if($order_no_total!="") $order_no_total.=",";
						...
						$buyer_name=""; $buyer_id=0;			
						$buyer_id=$sproduction_data_arr[$jobData][$txt_date][$f_id][$l_id]['buyer_name'];
						$buyer_name=$buyerArr[$buyer_id];
						...
						//if($wsSmv>0) { $item_smv=$wsSmv; $smvTd=""; }
						//else { $item_smv=$item_smv_array[$jobNo][$itemNo]; $smvTd="red"; }
						$item_smv=$sproduction_data_arr[$jobData][$txt_date][$f_id][$l_id]['smv'];
						
						$po_wise_target_min=0;
						//$produce_minit=$production_po_data_arr[$jobData][$txt_date][$f_id][$l_id]*$item_smv;
						$po_wise_target_min=$sproduction_data_arr_qty[$jobData][$txt_date][$f_id][$l_id]['quantity']*$item_smv;
						//echo $produce_minit.'<br>';
						$gmtsItemName=$garments_item[$itemNo];
						
						$subcon_po_id=array_unique(explode(',',$sproduction_data_arr[$jobData][$txt_date][$f_id][$l_id]['order_id']));
						$subcon_order_id="";
						foreach($subcon_po_id as $sub_val)
						{
							$subcon_po_smv=explode(',',$sub_val); 
							
							//$produce_minit+=$sproduction_po_data_arr[$jobData][$txt_date][$f_id][$l_id][$sub_val]*$subcon_order_smv[$sub_val];
							if($subcon_order_id!="") $subcon_order_id.=",";
							$subcon_order_id.=$sub_val;
						}
						$produce_minit=$sproduction_data_arr_qty[$jobData][$txt_date][$f_id][$l_id]['quantity']*$item_smv;
						// $produce_minit=$sproduction_po_data_arr[$jobData][$txt_date][$f_id][$l_id]*$item_smv;
						
						$days_run=0; $line_production_hour=0; $sewing_output=0;
						
						
						$type_line=$sproduction_data_arr[$jobData][$txt_date][$f_id][$ldata]['type_line'];
						//$prod_reso_allo=$line_data['prod_reso_allo'];
						//echo $sproduction_data_arr[$jobData][$txt_date][$f_id][$ldata]['prod_reso_allo'];die;
						$sewing_line=''; $poly_line=0;
						
						if($sproduction_data_arr[$jobData][$txt_date][$f_id][$ldata]['prod_reso_allo']==1)
						{
							$line_number=explode(",",$prod_reso_arr[$ldata]);
							foreach($line_number as $val)
							{
						...
						{
							 $prod_hour="prod_hour".substr($line_start_hour_arr[$h],0,2).""; 
							 $production_hour[$prod_hour]=$sproduction_data_arr[$jobData][$txt_date][$f_id][$ldata][$prod_hour];
							 $floor_production[$prod_hour]+=$sproduction_data_arr[$jobData][$txt_date][$f_id][$ldata][$prod_hour];
							 $stotal_production[$prod_hour]+=$sproduction_data_arr[$jobData][$txt_date][$f_id][$ldata][$prod_hour];
						}
								
						$floor_production['prod_hour24']+=$sproduction_data_arr[$jobData][$txt_date][$f_id][$ldata]['prod_hour23'];
						$stotal_production['prod_hour24']+=$sproduction_data_arr[$jobData][$txt_date][$f_id][$ldata]['prod_hour23'];
						$production_hour['prod_hour24']=$sproduction_data_arr[$jobData][$txt_date][$f_id][$ldata]['prod_hour23']; 
						...
						if(str_replace("'","",$actual_production_date)==str_replace("'","",$actual_date)) 
						{
							...
							for($lh=$line_start_hour;$lh<=$last_hour;$lh++)
							{
								...
								if($lh<$actual_time)
								{
									$total_eff_hour=$total_eff_hour+1;;	
									$line_hour="prod_hour".substr($bg,0,2)."";
									$line_production_hour+=$sproduction_data_arr[$jobData][$txt_date][$f_id][$ldata][$line_hour];
									$line_floor_production+=$sproduction_data_arr[$jobData][$txt_date][$f_id][$ldata][$line_hour];
									$sline_total_production+=$sproduction_data_arr[$jobData][$txt_date][$f_id][$ldata][$line_hour];
									$actual_time_hour=$start_hour_arr[$lh+1];
								}
							}
						...
						if(str_replace("'","",$actual_production_date)<str_replace("'","",$actual_date)) 
						{
							...
							{
								$prod_hour="prod_hour".substr($start_hour_arr[$ah],0,2).""; 
								$line_production_hour+=$sproduction_data_arr[$jobData][$txt_date][$f_id][$ldata][$prod_hour];
								$line_floor_production+=$sproduction_data_arr[$jobData][$txt_date][$f_id][$ldata][$prod_hour];
								$sline_total_production+=$sproduction_data_arr[$jobData][$txt_date][$f_id][$ldata][$prod_hour];
							}
						...
						$extra_hour=$prod_resource_array[$ldata][$txt_date]['adjust_hour'];	
						// $extra_worker=$prod_resource_array[$ldata][$txt_date]['number_of_emp'];	
						$extra_worker=$sstyle_resource_array[$style_ref][$txt_date][$f_id][$ldata]['extra_worker'];
						//$action_machine=$prod_resource_array[$ldata][$txt_date]['active_machine'];	
						// $action_machine=$prod_resource_array[$ldata][$txt_date]['operator'];	
						$action_machine=$sstyle_resource_array[$style_ref][$txt_date][$f_id][$ldata]['active_operator'];
						// echo $style_ref."==".$txt_date."==".$f_id."==".$ldata."<br>";
						//$man_power=$prod_resource_array[$ldata][$txt_date]['man_power'];
							
						// $operator=$prod_resource_array[$ldata][$txt_date]['operator'];
						$operator=$sstyle_resource_array[$style_ref][$txt_date][$f_id][$ldata]['active_operator'];
						// $helper=$prod_resource_array[$ldata][$txt_date]['helper'];
						$helper=$sstyle_resource_array[$style_ref][$txt_date][$f_id][$ldata]['active_helper'];
						$man_power=$operator+$helper;
						$style_working_hour = $sstyle_resource_array[$style_ref][$txt_date][$f_id][$ldata]['style_working_hour'];
						$target_per_line = $sstyle_resource_array[$style_ref][$txt_date][$f_id][$ldata]['target_per_line'];
						$eff_target = $target_per_line*$style_working_hour;
						// $terget_hour=$prod_resource_array[$ldata][$txt_date]['terget_hour'];	
						$terget_hour=$sstyle_resource_array[$style_ref][$txt_date][$f_id][$ldata]['target_per_line'];	
						$capacity=$prod_resource_array[$ldata][$txt_date]['capacity'];
						//$working_hour=$prod_resource_array[$ldata][$txt_date]['working_hour'];
						$working_hour=$slineActiveHoursArr[$jobData][$txt_date][$f_id][$ldata];
						//$production_data_arr[$jobData][$txt_date][$f_id][$ldata][$prod_hour];
						...
						$total_smv = 0;
						$total_smv_others = 0;
						// echo $line_count_array[$ldata]."<br>";
						if($style_arr2[$ldata]>1) // when multiple style running a line
						{
							if($style_arr[$ldata]>1)
							{
						...
						$floor_capacity+=$prod_resource_array[$ldata][$txt_date]['capacity'];
						$floor_man_power+=$man_power;//$prod_resource_array[$ldata][$txt_date]['man_power'];
						$floor_operator+=$operator;//$prod_resource_array[$ldata][$txt_date]['operator'];
						$floor_helper+=$helper;//$prod_resource_array[$ldata][$txt_date]['helper'];
						...
						$stotal_operator+=$operator;//$prod_resource_array[$ldata][$txt_date]['operator'];
						$stotal_man_power+=$man_power;//$prod_resource_array[$ldata][$txt_date]['man_power'];
						$stotal_helper+=$helper;//$prod_resource_array[$ldata][$txt_date]['helper'];
						$total_capacity+=$prod_resource_array[$ldata][$txt_date]['capacity'];
						$stotal_working_hour+=$working_hour;//$prod_resource_array[$ldata][$txt_date]['working_hour']; 
						$sgnd_total_tgt_h+=$prod_resource_array[$ldata][$txt_date]['terget_hour'];
						$sgrand_extra_hour+=$extra_hour;
						$sgrand_spend_min+=$spend_min;
						$sgrand_particulars_hour+=$particulars_hour;
						$sgrand_extra_worker+=$extra_worker;
						$stotal_terget+=$eff_target;
						$sgrand_total_product+=$today_product;
						$sgnd_avable_min+=$efficiency_min;
						$sgnd_product_min+=$produce_minit;
						$sgnd_total_fob_val+=$fob_val;
						$sgnd_active_machine+=$action_machine; 
							
					
						$po_id=rtrim($sproduction_data_arr[$jobData][$txt_date][$f_id][$ldata]['po_id'],',');
						$po_id=array_unique(explode(",",$po_id));
						
						$po_id=$sproduction_data_arr[$jobData][$txt_date][$f_id][$ldata]['po_id'];
					
					
						$floor_target_efficiency_manual+=$target_efficiency_manual;
						$sgrand_target_efficiency_manual+=$target_efficiency_manual;
						$floor_target_efficiency+=$target_efficiency;
						$sgrand_target_efficiency+=$target_efficiency;
						$cbo_get_upto=str_replace("'","",$cbo_get_upto);
						$txt_parcentage=str_replace("'","",$txt_parcentage);
					   //********************************* calclution floor total    *******************$txt_date],$sewing_day
						$floor_name=$floorArr[$f_id];	
						...
						$floor_days_run+=$days_run;
						$floor_sewing_input+=$sewing_input;
						$sgrand_sewing_input+=$sewing_input;
						$sgrand_sewing_output+=$sewing_output;
						...
						if($line_efficiency<=$txt_parcentage) $efficiency_color="#FF0000"; else $efficiency_color="#FFFFFF";
						if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
						$shtml.="<tr bgcolor='$bgcolor' onclick=change_color('tr_$i','$bgcolor') id=tr_$i>";
						$shtml.='<td width="40" align="center">'.$i.'&nbsp;</td>
								<td width="100" align="center">'.$floor_name.'&nbsp; </td>
								<td align="center" width="80" title="'.$ldata.'">'. $sewing_line.'&nbsp; </td>
								<td align="center" width="60"><p>'.number_format($item_smv,2).'</p></td>
								...
								<td width="60" align="center">'.$particulars_hour.'</td>
								...
								<td align="center" width="80" title="Target per line='.$target_per_line.'*Working hour='.$style_working_hour.'">'. $eff_target.'</td>';
								$tday_line_qc_pass=0;
								for($k=$hour; $k<=$last_hour; $k++)
								{
									$prod_hour="prod_hour".substr($start_hour_arr[$k],0,2)."";
									...
									$shtml.='<td align="center" width="50"  style='.$bg_color.'>'.$production_hour[$prod_hour].'</td>';
								}
										
								$avarage_production_per_line=$line_production_hour/$working_hour;
								$shtml.='<td align="center" width="80"><a href="##" onclick="openmypage('.$cbo_company_id.",'".$jobData."','sub',".$f_id.",".$ldata.",'tot_prod','".$item_smv."','".$actual_time_hour."','".$line_start."','".$txt_date."'".')">'.$line_production_hour.'</a></td>
									<td align="center" width="80">'.$sewing_output.'</td>';

									if($line_efficiency<=$txt_parcentage) $shtml.='<td title="(Produce min='.$produce_minit.'/Spend min='.$efficiency_min.')*100" align="center" width="80" bgcolor="red">'.number_format($line_efficiency,2).'%</td>';
									else $shtml.='<td title="(Produce min='.$produce_minit.'/Spend min='.$efficiency_min.')*100" align="center" width="80">'.number_format($line_efficiency,2).'%</td>'; 

									
									$shtml.='<td align="center" width="80">'.number_format($line_efficiencysss,2).'%</td>
									<td align="center" width="60" >'.number_format($target_efficiency,0).'</td>
											<td width="100" align="center"><a href="##" onclick="openmypage('.$cbo_company_id.",'".$jobData."','sub',".$f_id.",".$ldata.",'tot_prod','".$item_smv."','".$actual_time_hour."','".$line_start."','".$txt_date."'".')">'.number_format($produce_minit,0).'</a></td>
											<td align="center" width="70" >'. $days_run.'</td>
											<td align="center" width="70" >'. number_format(($line_production_hour/$eff_target)*100,2).'%</td>';
									if($line_cheaf_row_count==1)
									{
										$line_cheaf_serial++;
										$shtml.='<td align="center" width="70" rowspan="'.$line_cheaf_total_row.'" id="line_cheaf_sl'.$line_cheaf_serial.'" valign="middle" ></td>';
									}
									// echo "string".$floor_line_num;
									// if($floor_line_num==1)
									// {
										//echo $floot_line_total;die;
										$floor_serial_no++;
										$shtml.='<td align="center" width="80" id="sline_floor_sl'.$floor_serial_no.'"  valign="middle" ></td>';
										//rowspan="'.$floot_line_total.'" 
									// }
									$shtml.='<td width="" title="'.$fob_rate.'" align="left">'.$production_data_arr[$f_id][$l_id]['remarks'].'</td>';
	...
	$shtml.='<tr  class="tbl_bottom">
					...
					for($k=$hour; $k<=$last_hour; $k++)
					{
						...
						$shtml.='<td align="center" width="50" style='.$bg_color.' >'. $floor_production[$prod_hour].'</td>';
					}
                            
				$avarage_production_per_floor=$line_floor_production/$floor_working_hour;
				$shtml.='<td align="center" width="80">'. $line_floor_production.'</td>
						<td align="center" width="80">'. $floor_sewing_output.'</td>
						<td align="center" width="80">'.number_format($floor_efficency,2).' %</td>
						<td align="center" width="80">'.number_format($floor_efficency,2).' %</td>
						<td align="center" width="60">'.number_format($floor_target_efficiency,0).'</td>
		...
		?>
        <fieldset>
        <label><strong>Daily Summary ( In-Bound Sub-Con Order ):-</strong></label>
    	<?php $table_width=2460+($last_hour-$hour)*50; ?>
        <table id="table_header_1" class="rpt_table" width="<?php echo $table_width; ?>" cellpadding="0" cellspacing="0" border="1" rules="all">
            <thead>
                 <tr height="50">
                	...
                	for($k=$hour+1; $k<=$last_hour+1; $k++)
					{
						?><th width="50" style="vertical-align:middle"><div class="block_div"><?=substr($start_hour_arr[$k],0,5); ?></div></th><?	
					}
                	?>
                   	<th width="80">Total Prd.</th>
                    <th width="80">Cumulative Prd.</th>
            ...
            <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body">
                ...
                <tfoot >
                	 <tr>
                        ...
                        <th width="80">&nbsp;</th>
                        <th width="60">Total</th>
                        <th width="60" title="(Prod. Qty*SMV)/Prod. Qty"><? echo number_format(($sgnd_product_min/$sline_total_production),2); ?></th>
                        <th width="60"><? echo $sgnd_active_machine;//$total_operator; ?></th>
                        <th width="60"><? echo $stotal_operator; ?></th>
                        
                        <th width="60"><? echo $stotal_helper; ?></th>
                        <th align="center" width="60"><? echo $stotal_man_power; ?></th>
                        
                        <th width="60"><? echo $stotal_working_hour; ?></th>
                         <th align="center" width="60"><?php  echo $sgrand_extra_worker; ?></th>
                        <th align="center" width="60"><?php  echo $sgrand_spend_min; ?></th>
                        <th align="center" width="60"><?php  echo $sgrand_extra_hour; ?></th>
                        <th align="center" width="60"><?php  echo $sgrand_particulars_hour; ?></th>
                        <th align="center" width="100"><? echo $sgnd_avable_min; ?></th>
                        <th width="80">&nbsp;</th>
                        <th width="100"></th>
                        <th width="70"><?  //echo number_format($total_terget/$total_working_hour,2); ?></th>
                        <th width="70"><?  //echo $total_terget; ?></th>
                        <th width="70"><?  echo number_format($sgrand_target_efficiency_manual/($i-1),2)."%"; ?></th>
                        <th width="70"><?  echo $sgnd_total_tgt_h; ?></th>
                        <th width="70"><?  echo $stotal_terget; ?></th>
                        <?
						for($k=$hour; $k<=$last_hour; $k++)
						{
							$prod_hour="prod_hour".substr($start_hour_arr[$k],0,2)."";
							?>
							<th align="center" width="50"><? echo $stotal_production[$prod_hour]; ?></th>
							<?	
						}
                        ?>
                        
                        <th align="center" width="80"><?  echo $sline_total_production; ?></th>
                        <th align="center" width="80"><?  echo $sgrand_sewing_output; ?></th>
                      
                        <th align="center" width="80"><? echo number_format(($sgnd_product_min*100)/$sgnd_avable_min,2)."%"; ?></th>
                        <th align="center" width="80"><? echo number_format(($sgnd_product_min*100)/$sgnd_avable_min,2)."%"; ?></th>

                        <th align="center" width="60"><? echo $sgrand_target_efficiency; ?></th>
                        <th align="center" width="100"><?  echo number_format($sgnd_product_min,0); ?></th>
                        
                        <th align="center" width="70"><? //echo number_format($grand_target_efficiency,2).""; ?></th>
                        <th align="center" width="70"><? echo number_format(($sline_total_production/$stotal_terget)*100,2)."%"; ?></th>
                        <th align="center" width="70"><? //echo number_format(($line_total_production/$total_terget)*100,2)."%"; ?></th>
                        <th align="center" width="70"><? //echo number_format(($line_total_production/$total_terget)*100,2)."%"; ?></th>
                        <th align="center" width=""><? //echo $gnd_avable_min; ?></th>
                   </tr>
    ...
    <br/>
    <? //print_r($sfloor_serial_data_arr);?>
    <script>
	...
	<?php foreach($sfloor_serial_data_arr as $floor_serial=>$floor_serial_data)
	{
	?>
		$("#sline_floor_sl<?php echo $floor_serial; ?>").text("<?php echo $floor_serial_data; ?>%");
	<?php
	}
	?>
...
if($action=="report_generate_show2") 
{
	...
	if($prod_reso_allo[0]==1)
	{
		...
		$dataArray_sql=sql_select("SELECT a.id, a.location_id, a.floor_id, a.line_number, b.active_machine, b.pr_date, b.man_power, b.operator, c.target_efficiency, b.helper, b.smv_adjust, b.smv_adjust_type, b.line_chief, b.target_per_hour, b.working_hour, c.from_date, c.to_date, c.capacity, sum(d.target_per_line) as target_per_line, sum(d.operator) as active_operator, sum(d.helper) as active_helper,sum(d.working_hour) as style_working_hour, d.po_id, d.gmts_item_id 
		from prod_resource_mst a, prod_resource_dtls b,prod_resource_dtls_mast c left join prod_resource_color_size d on (d.dtls_id=c.id and d.mst_id=c.mst_id) where a.id=c.mst_id and c.id=b.mast_dtl_id and a.id=b.mst_id and a.company_id=$comapny_id and b.pr_date between '$last_month_start_date' and $txt_date and a.is_deleted=0 and b.is_deleted=0 and c.is_deleted=0  $location_cond  $floor group by a.id, a.location_id, a.floor_id, a.line_number, b.active_machine, b.pr_date, b.man_power, b.operator, c.target_efficiency, b.helper, b.smv_adjust, b.smv_adjust_type, b.line_chief, b.target_per_hour, b.working_hour, c.from_date, c.to_date, c.capacity, d.po_id, d.gmts_item_id order by a.id desc ");// ,sum(d.actual_smv) as extra_worker
	...
	{
		$sql="SELECT a.production_hour, a.company_id, a.location, a.floor_id, a.prod_reso_allo, a.production_date, a.sewing_line, b.buyer_name  as buyer_name, b.job_no,b.job_no_prefix_num, b.style_ref_no, a.po_break_down_id, a.item_number_id, c.po_number as po_number, c.file_no, c.unit_price, c.grouping as ref, sum(a.production_quantity) as good_qnty,
 		sum(nvl(a.alter_qnty,0)) as alter_qty, sum(nvl(a.reject_qnty,0)) as reject_qty, sum(nvl(a.spot_qnty,0)) as spot_qty,"; 
		$first=1;
		for($h=$hour;$h<$last_hour;$h++)
		{
		...
		$sql.="sum(CASE WHEN TO_CHAR(a.production_hour,'HH24:MI')>'$start_hour_arr[$last_hour]' and TO_CHAR(a.production_hour,'HH24:MI')<='$start_hour_arr[24]' and a.production_type=5 THEN production_quantity else 0 END) AS prod_hour23 from  wo_po_details_master b, wo_po_break_down c, pro_garments_production_mst a where a.production_type=5 and a.po_break_down_id=c.id and c.job_id=b.id and  a.status_active=1 and a.is_deleted=0 and b.is_deleted=0 and c.is_deleted=0 $company_name $location $floor $line  and a.production_date between '".$last_month_start_date."' and  $txt_date $product_category_cond group by a.company_id, a.location, a.floor_id, a.po_break_down_id, a.prod_reso_allo, a.production_date, a.sewing_line, b.buyer_name, b.job_no,b.job_no_prefix_num, b.style_ref_no, a.item_number_id, c.po_number, c.unit_price, c.file_no, c.grouping,a.production_hour order by a.production_hour asc";
	...
	foreach($sql_resqlt as $val)
	{
		...
		if($poly_line==0)
		{
			...
			$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['serial']=$slNo;  
			$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['buyer_name']=$val[csf('buyer_name')]; 
			$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['job_no_prefix_num']=$val[csf('job_no_prefix_num')]; 
			$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['style_ref']=$val[csf('style_ref_no')];
			$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['type_line']=1;
			$production_data_arr[$groupStr][$prod_date][$floor_id][$sewing_line]['po_id'].=$val[csf('po_break_down_id')].",";
			...
			foreach($line_cheaf_data as $sl=>$s_data)
			{
				foreach($s_data as $l_id=>$ldata)
				{
					...
					foreach($jobItemStrs as $jobData)
					{
						...
						$buyer_name=""; $buyer_id=0;			
						$buyer_id=$production_data_arr[$jobData][$txt_date][$f_id][$ldata]['buyer_name'];
						$job_no_prefix_num=$production_data_arr[$jobData][$txt_date][$f_id][$ldata]['job_no_prefix_num'];
						$buyer_name=$buyerArr[$buyer_id];
					...
					foreach($jobItemStr as $jobData)
					{
					   ...
					   //********************************* calclution floor total    *******************$txt_date],$sewing_day
						...
						$html.='<td width="40" align="center">'.$i.'&nbsp;</td>
								...
								$html.='<td align="center" width="60">'.$helper.'</td>
								<td align="center" width="60">'.$man_power.'</td>
								<td width="60" align="center">'.$working_hour.'</td>
								<td width="60" align="center">'.$extra_worker.'</td>
								<td width="60" align="center">'.$spend_min.'</td>
								...
								$html.='<td align="center" width="100">'.$efficiency_min.'</td>
								<td width="80" align="center"><p>'.$buyer_name.'&nbsp;</p></td>
								<td width="80" align="center"><p>'.$job_no_prefix_num.'&nbsp;</p></td>
    ...
    <div>
    	...
    	<fieldset>
	        <label><strong>Daily Summary ( In House order ):-</strong></label>
	    	<?php $table_width=2540+($last_hour-$hour)*50; ?>
	        <table id="table_header_1" class="rpt_table" width="<?php echo $table_width; ?>" cellpadding="0" cellspacing="0" border="1" rules="all">
	            <thead>
	                 <tr height="50">
	                    <th width="40">SL</th>
...
if($action=="report_generate_show3") 
{
    ...
    <div>
    	...
    	<fieldset>
	        <label><strong>Daily Summary ( In House order ):-</strong></label>
	    	<?php $table_width=2390+($last_hour-$hour)*50; ?>
	        <table id="table_header_1" class="rpt_table" width="<?php echo $table_width; ?>" cellpadding="0" cellspacing="0" border="1" rules="all">
	            <thead>
	                 <tr height="50">
	                    <th width="40">SL</th>
	...
	$shtml.='<tr  class="tbl_bottom">
        ...
        <fieldset>
        <label><strong>Daily Summary ( In-Bound Sub-Con Order ):-</strong></label>
    	<?php $table_width=2310+($last_hour-$hour)*50; ?>
        <table id="table_header_1" class="rpt_table" width="<?php echo $table_width; ?>" cellpadding="0" cellspacing="0" border="1" rules="all">
            <thead>
                 <tr height="50">
                    <th width="40">SL</th>
...
if($action=="tot_prod")
{
	...
	<fieldset style="width:520px; ">
	  ...
	  <?
        ...
        //print_r($new_smv);	
		$actual_date=date("Y-m-d"); $subcon_production_data_arr=array();
	    $actual_production_date=date("Y-m-d",strtotime(str_replace("'","",$txt_date)));
		if($type=="main")
		{
			if($db_type==0)
			...
			foreach($sql_pop as $pro_val)
			{
				...
				$subcon_production_data_arr[$pro_val[csf('po_break_down_id')]][$pro_val[csf('po_number')]]['item_smv']=$item_smv;	
			}
		}
		...
		if($type=="sub")
		{
			if($db_type==0)
			{
				if(str_replace("'","",$actual_production_date)>=str_replace("'","",$actual_date))
				{
					...
					where a.production_type=2 and a.order_id=c.id  and a.status_active=1 and a.is_deleted=0  and a.company_id=".$company_id."  and a.floor_id=".$floor_id." and a.line_id=".$sewing_line." and c.job_no_mst='$jobNo' and a.gmts_item_id='$itemNo' and a.production_date='".$prod_date."'                         	   group by a.order_id, c.order_no,c.smv");
				}
				if(str_replace("'","",$actual_production_date)<str_replace("'","",$actual_date))
				{
					$sql_subcon=sql_select("select  
			...
			foreach($sql_subcon as $sub_val)
			{
				...
				$subcon_production_data_arr[$sub_val[csf('order_id')]][$sub_val[csf('po_number')]]['item_smv']=$item_smv;;	
			}	
		}	   
		//print_r($subcon_production_data_arr);
					
		$total_producd_min=0; $i=1; $total_qnty=0;
			...
			foreach($pop_val as $po_id=>$pop_val)
			{
			   if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";	
				...
				<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
					...
					<td align="right"><? echo number_format($pop_val['item_smv'],2); ?>&nbsp;</td>
					<td align="right"><? $total_po_qty+=$pop_val['po_qty']; echo $pop_val['po_qty']; ?>&nbsp;</td>
					<td align="right"> <? echo number_format($producd_min,2); ?>&nbsp;</td>
				</tr>
				<?
                $i++;
			}
...
if($action=="show_remarks_popup")
{
	echo load_html_head_contents("FOB Info", "../../../", 1, 1,'','','');
	extract($_REQUEST);

	$sql = "SELECT adjustment_source,remarks from PROD_RESOURCE_SMV_ADJ where mst_id=$mst_id and style_ref_no='$style' and status_active=1 and is_deleted=0 and remarks is not null";
	// echo $sql;
    ...
    <div style="width:380px;" align="center">
		...
		<div id="report_container">
			<table border="1" class="rpt_table" rules="all" width="380" cellpadding="0" cellspacing="0" align="center">
				<caption><strong>Remarks Details </strong></caption>
                <thead>
                	<th width="30">SL</th>
				...
				<tbody>
					<?php $sl=1; foreach ($data_array as $key => $value): ?>
						<tr>
						<td><?=$sl;?></td>
						<td><?=$smv_adjustment_head[$value['ADJUSTMENT_SOURCE']];?></td>
						<td><?=$value['REMARKS'];?></td>
					</tr>						
					<?php $sl++; endforeach ?>
				</tbody>
