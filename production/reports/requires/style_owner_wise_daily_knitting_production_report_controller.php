...
{
	$print_report_format_arr = return_library_array("select format_id,format_id from lib_report_template where template_name in($data) and module_id=7 and report_id=222 and is_deleted=0 and status_active=1","format_id","format_id");
	echo "print_report_button_setting('".implode(',',$print_report_format_arr)."');\n";
	exit();
}
if ($action=="load_drop_down_location")
{
	echo create_drop_down( "cbo_location_id", 140, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id='$data' order by location_name","id,location_name", 1, "--Select Location--", $selected, "load_drop_down( 'requires/style_owner_wise_daily_knitting_production_report_controller', document.getElementById('cbo_working_company_id').value+'_'+this.value, 'load_drop_down_floor', 'floor_td' );",0 );
	exit();     	 
}
				...
				<table width="770" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
					...
					<tbody>
						<tr>
							...
							<td align="center">
								<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view ('<? echo $companyID; ?>'+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('txt_date_from').value+'**'+document.getElementById('txt_date_to').value+'**'+'<? echo $yearID; ?>'+'**'+'<? echo $ordType; ?>', 'create_int_ref_search_list_view', 'search_div', 'style_owner_wise_daily_knitting_production_report_controller', 'setFilterGrid(\'tbl_list_search\',-1)');" style="width:100px;" />
							</td>
						</tr>
						<tr>
				...
				<table width="770" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
					...
					<tbody>
						<tr>
							...
							<td align="center">
								<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view ('<? echo $companyID; ?>'+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('txt_date_from').value+'**'+document.getElementById('txt_date_to').value+'**'+'<? echo $yearID; ?>'+'**'+'<? echo $ordType; ?>', 'create_order_no_search_list_view', 'search_div', 'style_owner_wise_daily_knitting_production_report_controller', 'setFilterGrid(\'tbl_list_search\',-1)');" style="width:100px;" />
							</td>
						</tr>
						<tr>
							<td colspan="5" height="20" valign="middle"><? echo load_month_buttons(1); ?></td>
				...
				<table width="570" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
					...
					<tbody>
						<tr>
							...
							<td align="center">
								<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view ('<? echo $companyID; ?>'+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+'<? echo $yearID; ?>'+'**'+'<? echo $ordType; ?>', 'create_job_no_search_list_view', 'search_div', 'style_owner_wise_daily_knitting_production_report_controller', 'setFilterGrid(\'tbl_list_search\',-1)');" style="width:100px;" />
							</td>
						</tr>
					</tbody>
				</table>
...
if($action=="report_generate")
{
	...
	if($style_owner_name==0)
	{
		$style_owner_cond="";
	}
	else
	{
		$style_owner_cond=" and a.style_owner=$style_owner_name";
		$style_owner_cond2=" and f.style_owner=$style_owner_name";
	}
	...
	if($report_type==1) // Prod. Wise
	{
		...
		if($cbo_type==1 || $cbo_type==0)
		{
			...
			$job_data=sql_select("select c.job_no as sales_order,a.job_no, b.booking_no, $year_date,a.style_ref_no,a.insert_date from wo_po_details_master a,wo_booking_dtls b,fabric_sales_order_mst c where a.job_no=b.job_no and c.sales_booking_no=b.booking_no and a.company_name in($cbo_company,$cbo_working_company_id) $style_owner_cond");
			...
			$po_data=sql_select("select a.job_no, a.job_no_prefix_num, $year_field_sam as year, a.style_ref_no, b.id, b.po_number from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.company_name in($cbo_company,$cbo_working_company_id) $style_owner_cond");
						...
						<div style="width:660px; overflow-y:scroll; max-height:220px;" id="scroll_body">
							<table cellspacing="0" cellpadding="0" border="1" rules="all" width="640px" class="rpt_table" >
								<tbody>
									...
									{
										$sql_sample_sam="SELECT a.buyer_id, a.booking_no, a.knitting_source, sum(case when a.booking_without_order=1 $floor_id  then b.grey_receive_qnty end ) as sample_qty from inv_receive_master a, pro_grey_prod_entry_dtls b where a.entry_form=2 and a.item_category=13 and a.id=b.mst_id and a.receive_basis!=4 and a.buyer_id>0 $cbo_company_cond $company_working_cond and a.knitting_source like '$source'  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.booking_without_order in(1) $date_con $floor_id $buyer_cond $location_cond group by a.buyer_id,a.booking_no, a.knitting_source ";
									}
									...
									else // 2 No
									{
										...
										where c.po_breakdown_id=e.id and a.entry_form=2 and a.item_category=13 and a.id=b.mst_id and b.id=c.dtls_id and e.job_no_mst=f.job_no and c.entry_form=2 and c.trans_type=1 and a.knitting_source like '$source' and a.status_active=1 and a.is_deleted=0 and b.status_active=1  and a.receive_basis!=4 and b.is_deleted=0 and c.status_active=1 and c.is_sales=0 and c.is_deleted=0 $cbo_company_cond $company_working_cond $date_con $floor_id $style_owner_cond2 $buyer_cond $job_cond $order_cond $job_year_cond $location_cond";
									}
					...
					<table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $tbl_width+440; ?>" class="rpt_table" id="table_body">
						...
						if (count($nameArray_inhouse)>0 || count($nameArray_subcontract)>0)//for avg.rate
						{
						    ...
						    if($job_no_pre)
						    {
								...
								where a.job_no=b.job_no and b.job_no=c.job_no and b.fabric_description=c.id and b.cons_process=1  $job_no_pre_cond $style_owner_cond
		...
		else if ($machine_wise_section==1)
		{
			...
			<fieldset style=" width:1050px;">
				...
				<table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center" id="summary_tab">
					...
					$ymcpacity_arr=return_library_array( "select d.machine_no,sum(c.quantity) as quantity
						from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c, lib_machine_name d, wo_po_break_down e,  wo_po_details_master f
						where a.id=b.mst_id and b.id=c.dtls_id and b.machine_no_id=d.id and c.po_breakdown_id=e.id and e.job_no_mst=f.job_no and a.entry_form=2 and a.item_category=13 and c.entry_form=2 and c.trans_type=1 and a.knitting_source=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $cbo_company_cond $company_working_cond $date_con_2 $floor_id $style_owner_cond2 $buyer_cond $job_cond $order_cond $job_year_cond $location_cond group by d.machine_no", "machine_no", "quantity" );
	...
	else if($report_type==6) //Machine Wise 3
	{
		...
		{
			// $sql_inhouse.=", (case when b.shift_name=$key then c.quantity else 0 end ) as shift".strtolower($val);
			$sql_inhouse.=", case when b.shift_name=$key then b.no_of_roll else 0 end as roll".strtolower($val)."
		...
		<div>
			...
			<div style="width:<? echo $tbl_width+20; ?>px; overflow-y:scroll; max-height:330px;" id="scroll_body">
				<table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $tbl_width; ?>" class="rpt_table" id="table_body">
					...
					and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 $date_con $job_year_cond $location_cond
...
{
	echo load_html_head_contents("Style Owner Wise Daily Knitting Production report", "../../", 1, 1,'','','');
	extract($_REQUEST);
