...
$sql="SELECT
	...
	where
		...
		$shift_con
		$date_con
		";//$date_con_to$date_con_from$division_con	
 //echo $sql;
...
$order_cond = where_con_using_array($order_id_arr,0,"b.id");
$po_number_arr = array();
$po_quantity_arr = array();
$style_arr = array();
$sql = "SELECT b.id, a.style_ref_no,b.po_number,b.po_quantity from wo_po_details_master a, wo_po_break_down b where a.id=b.job_id and b.status_active=1 $order_cond";
$res = sql_select($sql);
foreach ($res as $v) 
{
	$po_number_arr[$v['ID']] = $v['PO_NUMBER'];
	$po_quantity_arr[$v['ID']] += $v['PO_QUANTITY'];
	$style_arr[$v['ID']] = $v['STYLE_REF_NO'];
}


/*//for previous deduct by order
$po_id=rtrim($po_id,',');
 $sql="SELECT c.order_id,c.bill_qty,c.bill_safty_qty ,c.bill_safty_amount,c.gmt_item_id  FROM pro_weekly_wages_bill_mst a, pro_weekly_wages_bill_dtls b,pro_weekly_wages_order_brk c WHERE a.id=b.mst_id and a.id=c.weekly_wages_mst_id and b.id=weekly_wages_dtls_id and c.order_id in($po_id) and a.bill_for=20 $company_name $location_con $division_con $department_con $shift_con";
 $data_array=sql_select($sql);
	foreach ($data_array as $row)
	{  
		$keys=$row[csf("order_id")].$row[csf("gmt_item_id")];
		$total_previous_deducted_qty[$keys]+=$row[csf("bill_safty_qty")];
		$total_previous_deducted_amount[$keys]+=$row[csf("bill_safty_amount")];
	}


*/

$sql="SELECT production_quantity,po_break_down_id,serving_company,location,item_number_id from  pro_garments_production_mst where production_type = 1 $production_company $production_location $production_date_con and status_active=1 and is_deleted=0";
// echo $sql;
...
<table width="100%">
    <tr>
        <th colspan="15">
            ...
            <h2 style="font-size:25px; line-height:22px;">Cutting Wages Bill</h2>
        </th>
    </tr>
    <tr>
...
<div style="width:1509px; max-height:300px; overflow-y:auto;" id="scroll_body">
<table width="1490" border="1" class="rpt_table" rules="all" id="table_body">
    <tbody>
        <?
		$i=1;
		foreach($data_arr as $rows){
         $bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF";
			...
			$bill_qty_pcs = ($rows['bill_qty']*12);
			$tot_bill_qty_pcs += $bill_qty_pcs;
        ...
        <tr bgcolor="<? echo $bgcolor;?>" id="tr_<? echo $i;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" style="cursor:pointer;">
            ...
            <td width="100"><? echo $style_arr[$rows['order_id']];?></td>
            ...
            <td width="85" align="right"><? echo round($rows['previous_bill_qty']*1);?></td>
            <td width="90" align="right"><? echo round($pro_qty_arr[$rows['key']]);?></td>
            <td width="75" align="right"><? echo round($rows['bill_qty']);?></td>
            <td width="75" align="right"><? echo round($bill_qty_pcs);?></td>
...
<table width="100%" border="1" class="rpt_table" rules="all">
    <tfoot>
        <tr>
            ...
            <th width="75" id="tot_bill_qty_pcs" align="right"><? echo $tot_bill_qty_pcs;?></th>
            <th width="50"></th>
            <th width="80" id="tot_amount" align="right"><? echo round($tot_amount);?></th>
            <th></th>
        </tr>
        <tr>
            <th width="50" colspan="15" align="right">Deducted Qty: </th>
            <th width="80" align="right"><? echo $deducted_emp;?></th>
            <th></th>
        </tr>
        <tr>
            <th width="50" colspan="15" align="right">Net Amount: </th>
            <th width="80" align="right"><? echo round($tot_amount-$deducted_emp);?></th>
            <th></th>
        </tr>
     </tfoot>
</table>    
...
<?	
	echo signature_table(227, $cbo_company_name, "1490px");
