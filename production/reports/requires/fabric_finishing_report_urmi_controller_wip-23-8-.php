...
if($action=="report_generate")
{
	...
	if($txt_date_from && $txt_date_to)
	{
		...
		$get_batch_ids_from_subprocess = sql_select("select a.batch_id from pro_fab_subprocess a where a.status_active=1 and a.entry_form=35 and a.load_unload_id=2 $prod_date_cond $floor_cond2 $batch_cond3 group by a.batch_id");
	...
	where d.mst_id=c.id and c.is_sales=1 and c.status_active=1 and d.status_active=1 $batch_id_cond $batch_cond2 $batch_date_cond2 $batch_year_cond $comp_cond $batch_ext_cond2 $sales_cond2 $floor_cond
	...
	foreach($result_batch as $row)
	{
		...
		//echo $cbo_type.'='.$results.'='.$entry_form_id.'='.$entry_form_id;
