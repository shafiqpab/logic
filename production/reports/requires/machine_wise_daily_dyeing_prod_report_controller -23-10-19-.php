			...
			foreach ($self_date_data_arr as $p_date=>$prod_date_data) 
			{
				...
				foreach ($prod_date_data as $machine_id => $machine_data) 
				{
					...
					//$re_pro_qty = $re_process_qty;		
						...
						//	$array_result_arr=array(2,4); redyeing_needed shade_matched
							if (($row['result']==1 || $row['result']==2)  && $re_process_qty3=='')  //Shade match/Redyeing Needed/Fin Prod not Available
							{
										//echo $row['batch_ext_no'].'='.$row['redyeing_needed'].'<br>';
											if ($row['color_range_id']==4)
											{
												$white_batch_qty = $row['batch_qty']; 
											}
							...
							//echo $row['result'].'=='.$row['redyeing_needed'].'=='.$prod_fin_qnty2;
							...
							else if (($row['result']==1 || $row['result']==2) && ($row['redyeing_needed']==2 || $row['shade_matched']==1) && $re_process_qty3=='')//Shade match/Redyeing Needed/Fin Prod not Available 
							{
									...
									//else $actual_prod_qty=$row['batch_qty']+$row['total_trims_weight'];
									//echo  $row['result'].'='.$re_process_qty3.'<br>';
							}
							
						
						if (($row['result']==1 || $row['result']==2) && $row['batch_ext_no']>0 && $re_process_qty3>0 ) //Shade match/Redyeing Needed/Redying/Fin Prod Available
							{
							...
							<td width="80" valign="middle" align="right"><? echo $trims_weight=$row['total_trims_weight']; $total_trims_weight+=$trims_weight;?></td>
