...
{ 
	...
	foreach($data_result as $row)
	{
		$po_wise_prod_data_arr[$row[csf('buyer_name')]]['pub_date']	=$row[csf('pub_shipment_date')];
		$po_wise_prod_data_arr[$row[csf('buyer_name')]]['production_date']	=$row[csf('production_date')];
		$po_wise_prod_data_arr[$row[csf('buyer_name')]]['job_no']	=$row[csf('job_no')];
		$po_wise_prod_data_arr[$row[csf('buyer_name')]]['buyer']		=$row[csf('buyer_name')];
		$po_wise_prod_data_arr[$row[csf('buyer_name')]]['season_matrix']	=$row[csf('season_matrix')];
		$po_wise_prod_data_arr[$row[csf('buyer_name')]]['gmts_item_id']	=$row[csf('gmts_item_id')];
		$po_wise_prod_data_arr[$row[csf('buyer_name')]]['style_ref_no']	=$row[csf('style_ref_no')];
		$po_wise_prod_data_arr[$row[csf('buyer_name')]]['po_no']			=$row[csf('po_number')];
		$po_wise_prod_data_arr[$row[csf('buyer_name')]]['po_quantity']	=$row[csf('po_quantity')]*$row[csf('ratio')];
		$po_wise_prod_data_arr[$row[csf('buyer_name')]]['unit_price']	=$row[csf('unit_price')];
		$po_wise_prod_data_arr[$row[csf('buyer_name')]]['po_total_price']=$row[csf('po_total_price')];
		$po_wise_prod_data_arr[$row[csf('buyer_name')]]['production_quantity']+=$row[csf('production_quantity')];
		$po_wise_prod_data_arr[$row[csf('buyer_name')]]['produced_min']	+=$row[csf('produced_min')];
		$po_wise_prod_data_arr[$row[csf('buyer_name')]]['efficency_min']	+=$row[csf('efficency_min')];
		
		$po_wise_prod_data_arr[$row[csf('buyer_name')]]['total_target']	=$row[csf('total_target')];
		$po_wise_prod_data_arr[$row[csf('buyer_name')]]['po_id']			=$row[csf('po_id')];
		
		$job_wise_prod_data_arr[$row[csf('buyer_name')]]['row_span']	+=1;
		
		$job_wise_prod_data_arr[$row[csf('buyer_name')]]['job_quantity']=$row[csf('job_quantity')];
		$job_wise_prod_data_arr[$row[csf('buyer_name')]]['total_price']	=$row[csf('total_price')];
	...
	<fieldset style="width:100%;">	
		...
		$other_costing_arr=$other->getAmountArray_by_job(); 
		$j=1;
		foreach($po_wise_prod_data_arr as $buyerId=>$val)
		{
			...
			$costing_per	 	= $pre_cost_date_arr[$job_no]['costing_per'];
			$job_quantity	 	= $job_wise_prod_data_arr[$val['buyer']]['job_quantity'];
			$total_job_value 	= $job_wise_prod_data_arr[$val['buyer']]['total_price'];
			...
			//$interest_expense	= $pre_cost_date_arr[$job_no]['interest_expense']/100;
			//$income_tax			= $pre_cost_date_arr[$job_no]['income_tax']/100;
			//$interest_expense	= $pre_cost_date_arr3[$job_no][$date_key]['interest_expense']/100;
			//$income_tax			= $pre_cost_date_arr3[$job_no][$date_key]['income_tax']/100;
			/*$pre_cost_date_arr3[$row[csf('job_no')]]['cost_per_minute']	=$financial_para2[$date_key]['cost_per_minute'];
		$pre_cost_date_arr3[$row[csf('job_no')]]['interest_expense']	=$financial_para2[$date_key]['interest_expense'];
		$pre_cost_date_arr3[$row[csf('job_no')]]['income_tax']		=$financial_para2[$date_key]['income_tax'];*/
			...
			//$prod_sew_out_qty	= $gmt_prod_qty_arr[$val['po_id']]['sew_out_qty'];
			$produced_min		= $val['produced_min'];
			
			$pre_costing_date	= $pre_cost_date_arr[$val['job_no']]['costing_date'];
			
			//$cost_per_minute	= $pre_cost_date_arr[$val['job_no']]['cost_per_minute'];
			//$cost_per_minute	= $pre_cost_date_arr3[$val['job_no']][$date_key]['cost_per_minute'];
			...
			$prod_sew_out_qty	= $val['production_quantity'];
			$shipment_qty		= $exfactory_qty_arr[$val['po_id']]['ex_factory_qnty'];
			
			$cm_cost_earning_per_pcs=($po_cm_cost+$net_job_profit_value)/$job_quantity;
			$cm_cost_earning	= $cm_cost_earning_per_pcs*$prod_sew_out_qty;
			
			$tot_target			= $res_prod_data_arr[$val['po_id']]['target'];
			$shipment_cm_earning_value=$cm_cost_earning_per_pcs*$shipment_qty;
			if($db_type==0)
			{
				$conversion_date = change_date_format($pre_costing_date, "Y-m-d", "-",1);
			...
			$currency_rate	= set_conversion_rate($usd_id,$conversion_date );
			$row_span		= $job_wise_prod_data_arr[$val['buyer']]['row_span'];
			
			?>
			<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>">
				<td width="30"><? echo $i; ?></td>
				<td width="120"><? echo $buyer_arr[$buyerId]; //$buyer_arr[$val['buyer']]; ?></td>
				
				<td width="90" align="right" title="Produced Min/Production(Sewing Out)"><? 
					($prod_sew_out_qty=="0")? $avg_smv=$prod_sew_out_qty : $avg_smv=$produced_min/$prod_sew_out_qty;
					echo number_format($avg_smv,2); ?></td>
				
                <td  width="90" align="right"  title="Produced Min/ Available Min"><? 
					$tot_effecincy=$produced_min/$avaiable_spent_min; 
					echo number_format($tot_effecincy,2); ?>  </td>
				<td width="90" title="Sewing Out<? echo $prod_sew_out_qty; ?>" align="right"><? echo number_format($prod_sew_out_qty,2); ?></td>
				<td width="90" title="Available Min"  align="right" ><? echo $avaiable_spent_min; ?></td>
				<td width="90"  align="right"><?  echo $produced_min;  ?></td>
				<td  width="90" title="<? echo "CM (".$po_cm_cost.")+Net FOB Value(".number_format($net_job_profit_value,4).")/Job Qty(".$job_quantity."*Production";?>" align="right"><?  echo number_format($cm_cost_earning,2); ?></td>
				<td  width="90" align="right"><? 
				($val['unit_price']*$prod_sew_out_qty)>0 ? $cmPercent = ($cm_cost_earning/($val['unit_price']*$prod_sew_out_qty))*100 : $cmPercent = 0 ;
				echo number_format($cmPercent,2); 
				?></td>
				<td width="90"  title="Production(Sewing Out=<? echo $val['unit_price'];?>)*PO Unit price=<? echo $prod_sew_out_qty;?>"  align="right"><? $fob_earning=$val['unit_price']*$prod_sew_out_qty; echo number_format($fob_earning,2); ?></td>
				<td width="120"  title="<? echo "Cost Per Minutes=".$cost_per_minute."/Conv.Rate=".$currency_rate."";?>" align="right"><? 
				//$cm_cost=($avaiable_spent_min*$cost_per_minute)/$currency_rate; 
				$cm_cost=($cost_per_minute)/$currency_rate; 
				echo number_format($cm_cost,2); 
				?></td>
			</tr>
			<?
			$i++;
