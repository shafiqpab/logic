...
//  echo $job_poid;die;
	...
	$sizearr=return_library_array("SELECT id,size_name from lib_size ","id","size_name");  
	 if($type==1)
	 {
			...
			$sql_lay="SELECT d.size_id, d.marker_qty,c.gmt_item_id,c.color_id,d.order_id,e.buyer_name,a.job_no,e.style_ref_no,e.client_id from ppl_cut_lay_mst a,ppl_cut_lay_dtls c,ppl_cut_lay_size d ,wo_po_details_master e, wo_po_break_down f where e.job_no=f.job_no_mst and e.job_no=a.job_no and d.order_id=f.id and  a.id=c.mst_id  and a.id=d.mst_id and a.status_active=1  $date_cond_3 and a.WORKING_COMPANY_ID in ($cbo_work_company_name) $job_cond_2 $buyer_cond $job_year_cond order by a.job_no asc";
		...
		//	echo $sql_lay;
			...
			foreach($sql_lay_result as $row)
			{
				...
				$order_color_size_data[$row[csf("buyer_name")]][$row[csf("job_no")]][$row[csf("order_id")]][$row[csf("gmt_item_id")]][$row[csf("color_id")]][$row[csf("size_id")]]["cut_lay"]+=$row[csf("marker_qty")];
				if($l==0){
			...
			$sql_lay_prev="SELECT d.size_id, d.marker_qty,c.gmt_item_id,c.color_id,d.order_id,e.buyer_name,a.job_no from ppl_cut_lay_mst a,ppl_cut_lay_dtls c,ppl_cut_lay_size d ,wo_po_details_master e, wo_po_break_down f where e.job_no=f.job_no_mst and e.job_no=a.job_no and d.order_id=f.id and  a.id=c.mst_id  and a.id=d.mst_id and a.status_active=1  and a.WORKING_COMPANY_ID in ($cbo_work_company_name) $job_year_cond $d_l_order_cond $job_cond_2 $buyer_cond order by a.job_no asc";
					// echo $sql_lay_prev;
				...
				foreach($sql_lay_prev_result as $row)
				{
					...
					$order_color_size_data[$row[csf("buyer_name")]][$row[csf("job_no")]][$row[csf("order_id")]][$row[csf("gmt_item_id")]][$row[csf("color_id")]][$row[csf("size_id")]]["lay_prev_qnty"]+=$row[csf("marker_qty")];
				}
			
			$sewing_in_sql="SELECT a.production_date, c.po_break_down_id as order_id, c.item_number_id, d.buyer_name, c.color_number_id as color_id,c.size_number_id as size_id,
			...
			foreach($sewing_in_sql_result as $row)
			{
				...
				$order_color_size_data[$row[csf("buyer_name")]][$row[csf("job_no")]][$row[csf("order_id")]][$row[csf("item_number_id")]][$row[csf("color_id")]][$row[csf("size_id")]]["sewing_in"]+=$row[csf("sewing_in_qnty")];
			...
			$sewing_in_tot_sql="SELECT a.production_date, c.po_break_down_id as order_id, c.item_number_id, d.buyer_name, c.color_number_id as color_id,c.size_number_id as size_id,
			sum(CASE WHEN b.production_type =4 and a.production_type =4 THEN b.production_qnty ELSE 0 END) AS sewing_in_prev_qnty,d.job_no
			from  pro_garments_production_dtls b,pro_garments_production_mst a,wo_po_color_size_breakdown c ,wo_po_details_master d, wo_po_break_down e
			...
			d.job_no order by d.job_no asc";
			$sewing_in_tot_sql_data=sql_select($sewing_in_tot_sql);
			foreach($sewing_in_tot_sql_data as $row)
			{
				
				
				$order_color_size_data[$row[csf("buyer_name")]][$row[csf("job_no")]][$row[csf("order_id")]][$row[csf("item_number_id")]][$row[csf("color_id")]][$row[csf("size_id")]]["sewing_in_prev_qnty"]+=$row[csf("sewing_in_prev_qnty")];
			}



			$sewing_out_sql="SELECT a.production_date, c.po_break_down_id as order_id, c.item_number_id, d.buyer_name, c.color_number_id as color_id,c.size_number_id as size_id,			
			...
			foreach($sewing_out_sql_result as $row)
			{
				...
				$order_color_size_data[$row[csf("buyer_name")]][$row[csf("job_no")]][$row[csf("order_id")]][$row[csf("item_number_id")]][$row[csf("color_id")]][$row[csf("size_id")]]["sewing_out"]+=$row[csf("sewing_out_qnty")];
			...
			$sewing_out_tot_sql="SELECT a.production_date, c.po_break_down_id as order_id, c.item_number_id, d.buyer_name, c.color_number_id as color_id,c.size_number_id as size_id,			
			...
			foreach($poly_sql_result as $row)
			{
				...
				$order_color_size_data[$row[csf("buyer_name")]][$row[csf("job_no")]][$row[csf("order_id")]][$row[csf("item_number_id")]][$row[csf("color_id")]][$row[csf("size_id")]]["poly_qnty"]+=$row[csf("poly_qnty")];
			...
			foreach($poly_sql_data as $row)
			{
				...
				$order_color_size_data[$row[csf("buyer_name")]][$row[csf("job_no")]][$row[csf("order_id")]][$row[csf("item_number_id")]][$row[csf("color_id")]][$row[csf("size_id")]]["poly_prev_qnty"]+=$row[csf("poly_prev_qnty")];
			}

			


			$paking_finish_sql="SELECT c.po_break_down_id as order_id, c.item_number_id, c.color_number_id as color_id,c.size_number_id as size_id,	sum(CASE WHEN b.production_type =8 and a.production_type =8 THEN b.production_qnty ELSE 0 END) AS paking_finish_qnty,d.job_no, d.buyer_name,d.style_ref_no,d.client_id,c.order_quantity
	...
	//	echo 	$paking_finish_sql;
		...
		foreach($paking_finish_sql_result as $row)
		{
			if($pf==0){
				...
				$order_color_size_data[$row[csf("buyer_name")]][$row[csf("job_no")]][$row[csf("order_id")]][$row[csf("item_number_id")]][$row[csf("color_id")]][$row[csf("size_id")]]["paking_finish_qnty"]+=$row[csf("paking_finish_qnty")];
		...
		$paking_finish_tot_sql="SELECT c.po_break_down_id as order_id, c.item_number_id, c.color_number_id as color_id,c.size_number_id as size_id,	sum(CASE WHEN b.production_type =8 and a.production_type =8 THEN b.production_qnty ELSE 0 END) AS paking_finish_prev_qnty,d.job_no, d.buyer_name from  pro_garments_production_dtls b,pro_garments_production_mst a,wo_po_color_size_breakdown c ,wo_po_details_master d, wo_po_break_down e where  d.job_no=e.job_no_mst and d.job_no=c.job_no_mst and c.po_break_down_id=e.id and a.id=b.mst_id and b.color_size_break_down_id=c.id and a.po_break_down_id=c.po_break_down_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and c.status_active in(1,2,3) and c.is_deleted=0 and a.production_source=1 and b.production_type=8 and a.production_type=8   and a.serving_company in($cbo_work_company_name)   $d_pf_order_cond $job_cond $buyer_cond_2 $job_year_cond_2 group by  c.po_break_down_id, c.item_number_id, c.color_number_id,c.size_number_id,d.job_no, d.buyer_name order by d.job_no asc";
			...
			foreach($ex_factory_sql_result as $row)
			{
				...
				$order_color_size_data[$row[csf("buyer_name")]][$row[csf("job_no")]][$row[csf("order_id")]][$row[csf("item_number_id")]][$row[csf("color_id")]][$row[csf("size_id")]]["ex_fact_qnty"]+=$row[csf("ex_fact_qnty")];
			...
			$ex_factory_tot_sql="SELECT  a.po_break_down_id as order_id, a.item_number_id, c.color_number_id as color_id, sum(b.production_qnty) as ex_fact_prev_qnty ,c.size_number_id as size_id,d.job_no, d.buyer_name,d.style_ref_no
			...
			group by  a.po_break_down_id, a.item_number_id, c.color_number_id,c.size_number_id ,d.job_no, d.buyer_name,d.style_ref_no order by d.job_no asc";
			...
			foreach($ex_factory_tot_sql_data as $row)
			{
				...
				$order_color_size_data[$row[csf("buyer_name")]][$row[csf("job_no")]][$row[csf("order_id")]][$row[csf("item_number_id")]][$row[csf("color_id")]][$row[csf("size_id")]]["ex_fact_prev_qnty"]+=$row[csf("ex_fact_prev_qnty")];
			}
			...
			// print_r($production_data);die;
				...
				$sql_data=sql_select("SELECT  a.id,a.client_id as buyer_client,a.style_ref_no,c.order_quantity, c.color_number_id,a.buyer_name, c.item_number_id,c.size_number_id,a.job_no,c.po_break_down_id  FROM wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c where
				a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $job_cond_3 $buyer_cond_3 $job_year_cond_3 order by a.job_no asc");
    ...
    <div style="width:2480px;">
        <table width="2040" cellspacing="0">
            <tr class="form_caption" style="border:none;">
                <td colspan="31" align="center" style="border:none;font-size:14px; font-weight:bold"> Daily Cutting And
                    Input Inhand Report</td>
            </tr>
					...
					foreach($buyer_data as $job_no=>$job_data)
					{
							...
							foreach($order_data as $item_id=>$item_data)
							{
								foreach($item_data as $color_id=>$color_data)
								{
									foreach($color_data as $size_id=>$value)
									{
											...
											$client_id=$buyer_wise_data[$buyer_id]['client_id'];
                        ...
                        <tr bgcolor="<? echo $bgcolor; ?>"
                            ...
                            <td width="70" align="right">
                                <?
											...
											if($date_from !=="" && $date_to !=="" && $job_cond_2 =="" || $date_from !=="" && $date_to !=="" && $job_cond_2 !==""){
														echo $value["cut_lay"];
														// number_format($production_data[$order_id][$item_id][$color_id][$size_id]["lay_qnty"],0); 
														$job_lay_qnty+=$value["cut_lay"];
														$color_lay_qnty+=$value["cut_lay"];
														$item_lay_qnty+=$value["cut_lay"];
														$po_lay_qnty+=$value["cut_lay"];
														$buyer_lay_qnty+=$value["cut_lay"]; 
														$gt_lay_qnty+=$value["cut_lay"];
                            ...
                            <td width="70" align="right">
                                <?
														if($date_from !=="" && $date_to !=="" && $job_cond_2 =="" || $date_from !=="" && $date_to !=="" && $job_cond_2 !==""){
														echo $value["sewing_in"];

													// number_format($production_data[$order_id][$item_id][$color_id][$size_id]["sewing_in_qnty"],0); 								
													
													
													$job_sewing_in_qnty+=$value["sewing_in"];
													$color_sewing_in_qnty+=$value["sewing_in"];
													$item_sewing_in_qnty+=$value["sewing_in"];
													$po_sewing_in_qnty+=$value["sewing_in"];
													$buyer_sewing_in_qnty+=$$value["sewing_in"]; $gt_sewing_in_qnty+=$value["sewing_in"];
                            ...
                            <td width="70" align="right">
                                <p>
                                    <? 
												...
												// $tot_sewing_in_qnty=$production_data[$order_id][$item_id][$color_id][$size_id]["sewing_in_prev_qnty"]; echo number_format($tot_sewing_in_qnty,0); 
												$tot_sewing_in_qnty=$value["sewing_in_prev_qnty"];
                            ...
                            <td width="70" align="right">
                                <? 
															if($date_from !=="" && $date_to !=="" && $job_cond_2 =="" || $date_from !=="" && $date_to !=="" && $job_cond_2 !==""){
													echo $value["sewing_out"];
											
														// echo number_format($production_data[$order_id][$item_id][$color_id][$size_id]["sewing_out_qnty"],0); 
														
												$job_sewing_out_qnty+=$value["sewing_out"];
												 $color_sewing_out_qnty+=$value["sewing_out"];
												 $item_sewing_out_qnty+=$value["sewing_out"];
												 $po_sewing_out_qnty+=$value["sewing_out"];
												$buyer_sewing_out_qnty+=$value["sewing_out"]; 
												$gt_sewing_out_qnty+=$value["sewing_out"];
															}
												?>
                            </td>
                            <td width="70" align="right">
                                <? 
												...
												// $tot_sewing_out_qnty=$production_data[$order_id][$item_id][$color_id][$size_id]["sewing_out_prev_qnty"]; echo number_format($tot_sewing_out_qnty,0); 
												$tot_sewing_out_qnty=$value["sewing_out_prev_qnty"];
                            ...
                            <td width="70" align="right">
                                <? 
																if($date_from !=="" && $date_to !=="" && $job_cond_2 =="" || $date_from !=="" && $date_to !=="" && $job_cond_2 !==""){
													echo $value["poly_qnty"];
												// echo number_format($production_data[$order_id][$item_id][$color_id][$size_id]["poly_qnty"],0);
												 $job_poly_qnty+=$value["poly_qnty"];
												$color_poly_qnty+=$value["poly_qnty"];
												$item_poly_qnty+=$value["poly_qnty"];
												$po_poly_qnty+=$value["poly_qnty"];									
												$buyer_poly_qnty+=$value["poly_qnty"];
												 $gt_poly_qnty+=$value["poly_qnty"];}?>
                            </td>
                            <td width="70" align="right">
                                <? 
												echo $value["poly_prev_qnty"];
												// $tot_poly_qnty=$production_data[$order_id][$item_id][$color_id][$size_id]["poly_prev_qnty"]; echo number_format($tot_poly_qnty,0); 
												$tot_poly_qnty=$value["poly_prev_qnty"];
                            ...
                            <td width="70" align="right">
                                <?
															if($date_from !=="" && $date_to !=="" && $job_cond_2 =="" || $date_from !=="" && $date_to !=="" && $job_cond_2 !==""){
												echo $value["paking_finish_qnty"];
											
												// echo number_format($production_data[$order_id][$item_id][$color_id][$size_id]["paking_finish_qnty"],0); 

												$job_paking_finish_qnty+=$value["paking_finish_qnty"];
												$color_paking_finish_qnty+=$value["paking_finish_qnty"];
												$item_paking_finish_qnty+=$value["paking_finish_qnty"];
												$po_paking_finish_qnty+=$value["paking_finish_qnty"];
												$buyer_paking_finish_qnty+=$value["paking_finish_qnty"];
												$gt_paking_finish_qnty+=$value["paking_finish_qnty"];}?>
                            </td>
                            <td width="70" align="right">
                                <? 
													echo $value["paking_finish_prev_qnty"];
												// $tot_paking_finish_qnty=$production_data[$order_id][$item_id][$color_id][$size_id]["paking_finish_prev_qnty"]; echo number_format($tot_paking_finish_qnty,0); 
												$tot_paking_finish_qnty=$value["paking_finish_prev_qnty"];
                            ...
                            <td width="70" align="right">
                                <? 
																if($date_from !=="" && $date_to !=="" && $job_cond_2 =="" || $date_from !=="" && $date_to !=="" && $job_cond_2 !==""){
													echo $value["ex_fact_qnty"];
												// echo number_format($production_data[$order_id][$item_id][$color_id][$size_id]["ex_fact_qnty"],0); 
												
												$job_ex_fact_qnty+=$value["ex_fact_qnty"];
												$color_ex_fact_qnty+=$value["ex_fact_qnty"];
												$item_ex_fact_qnty+=$value["ex_fact_qnty"];
												$po_ex_fact_qnty+=$value["ex_fact_qnty"];
												 $buyer_ex_fact_qnty+=$value["ex_fact_qnty"];
												 $gt_ex_fact_qnty+=$value["ex_fact_qnty"];}?>
                            </td>
                            <td width="70" align="right">
                                <?
													echo $value["ex_fact_prev_qnty"];
												// $tot_ex_fact_qnty=$production_data[$order_id][$item_id][$color_id][$size_id]["ex_fact_prev_qnty"];
												// echo number_format($tot_ex_fact_qnty,0); 
												$tot_ex_fact_qnty=$value["ex_fact_prev_qnty"];
