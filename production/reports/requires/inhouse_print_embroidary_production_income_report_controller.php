...
{ 
	...
	$lib_color=return_library_array( "select id, color_name from  lib_color",'id','color_name');
		...
		//	=====================================print qty============================================
		 ...
		 $currency_data=sql_select("select id,company_id,currency,conversion_rate,marketing_rate,con_date from currency_conversion_rate where status_active=1 and is_deleted=0 and company_id=$company_id order by id desc ");

		//  echo "<pre>";
	//   print_r($currency_data);
		$currency=$currency_data[0][csf("conversion_rate")];

	$print_qty_data=sql_select("SELECT id, po_break_down_id,embel_name,production_source, item_number_id, country_id, production_date, production_quantity, reject_qnty, production_source, serving_company, embel_name,embel_type from pro_garments_production_mst where   embel_name='1' and production_type='3'and production_source=1 and status_active=1 and is_deleted=0 $date_cond order by id");
...
$emb_qty_data=sql_select("SELECT id, po_break_down_id,embel_name, item_number_id, production_date, production_quantity,embel_type from pro_garments_production_mst where embel_name='2' and production_type='3' and production_source=1 and status_active=1 and is_deleted=0 $date_cond order by id");
...
$job_wise_order_qty=sql_select("SELECT a.is_confirmed,  a.po_number,  a.po_received_date,  a.pub_shipment_date, b.company_name, b.buyer_name,  a.shipment_date,b.style_ref_no,  a.po_quantity,  a.unit_price,  a.po_total_price,  a.excess_cut,  a.plan_cut, ( a.pub_shipment_date- a.po_received_date) as date_diff, ( a.pub_shipment_date- a.factory_received_date) as facdate_diff,  a.status_active,  a.id,b.job_no from wo_po_break_down a,wo_po_details_master b where  a.JOB_NO_MST=b.job_no and  a.status_active=1 and a.is_deleted=0 $sql_cond $job_cond order by a.id ASC");
	...
	//================================issue qty ===========================
		...
		$issue_print_data=sql_select("SELECT a.id,a.issue_number, b.id as bid, b.cons_quantity, b.prod_id,c.item_category_id, c.item_group_id, b.order_id,c.avg_rate_per_unit
			from inv_issue_master a, inv_transaction b, product_details_master c where   b.prod_id=c.id and  c.item_category_id=22 and  a.entry_form=21 and a.id=b.mst_id  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $issue_date_cond ");
		foreach($issue_print_data as $row){
				...
				$order_issue_id_arr[$row[csf("order_id")]][$row[csf("id")]]=$row[csf("id")];
		}
...
foreach($data_array as $row ){
			...
			$order_wise_data_arr[$row[csf("id")]][$row[csf("emb_type")]]['embel_type']=$row[csf("embel_type")];
			$order_wise_data_arr[$row[csf("id")]][$row[csf("emb_type")]]['emb_rate']=$embl_rate_arr[$row[csf("job_no_mst")]][$row[csf("embel_type")]];;
			$order_wise_data_arr[$row[csf("id")]][$row[csf("emb_type")]]['emb_qty']=$emb_qty[$row[csf("id")]][$row[csf("embel_type")]];
					...
					if (!in_array($batch_no,$buyer_wise_qty_id))
						{ $z++;
							 $buyer_wise_qty_id[]=$batch_no;							
							  ...
							  $buyer_wise_order_arr[$row[csf("buyer_name")]][$row[csf("id")]]=$row[csf("id")];
							  
							  $buyer_wise_issue_id[$row[csf("buyer_name")]]['issue_id']=implode(",",$order_issue_id_arr[$row[csf("id")]]);
	...
	foreach ($order_wise_data_arr as $id => $emb_data) 
	{
		...
		{
			$buyer_wise_qty_arr[$row["buyer_name"]]['print_value'] +=($print_qty[$id][$emb_data_id]*$print_rate_arr[$row["job_no"]][$emb_data_id]);
			$buyer_wise_qty_arr[$row["buyer_name"]]['embr_value'] +=($emb_qty[$id][$emb_data_id]*$embl_rate_arr[$row["job_no"]][$emb_data_id]);
			$buyer_wise_qty_arr[$row["buyer_name"]]['embl_qty']+=$emb_qty[$id][$emb_data_id];
			$buyer_wise_qty_arr[$row["buyer_name"]]['print_qty']+=$print_qty[$id][$emb_data_id];
						
							  
		if($print_qty[$id][$emb_data_id] !=''){
			if($is==0){		
	        	...
	        	<table width="1180" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all" id="table_body" align="left"> 
		    		<tbody id="tbl_list_search" align="left">
		        		...
		        		foreach($buyer_wise_qty_arr as $buyer_id=>$row){
							...
							$buyer_order_id=implode(",",$buyer_wise_order_arr[$buyer_id]);
							?>
							<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>">
								...
								<td width="80" align="left"><a href="##"onclick="openmypage_order('<?=$row['issue_p_num']?>','<?=$buyer_order_id;?>','<?=$row['b_job_no'];?>','<?=$txt_date_from;?>','<?=$txt_date_to;?>','Issue_cost_PrintPopup')"><? echo number_format($row['issue_p_val'],0);?></a></td>
								<td width="80" align="left"><a href="##"onclick="openmypage_order('<?=$row['issue_embl_num']?>','<?=$buyer_order_id;?>','<?=$row['b_job_no'];?>','<?=$txt_date_from;?>','<?=$txt_date_to;?>','Issue_cost_EmblPopup')"><?echo number_format($row['issue_e_val'],0)?></a></td>
		        		...
		        		foreach ($order_wise_data_arr as $id => $emb_data){
							...
							<td width="80" align="left"><? echo $emblishment_print_type_arr[$row['emb_type']]?></td>
								<?}else if($row['emb_name']==2){?>
									<td width="80" align="left"><? echo $emblishment_embroy_type_arr[$row['embel_type']]?></td>						
								<?}?>
						
							
							<td width="80" align="left"><?  echo number_format($plan_cut,2);?></td>
							<td width="80" align="left"><? echo $print_qty[$id][$row['emb_type']];?></td>
							<td width="80" align="left"><? echo number_format($print_rate);?></td>
							<td width="80" align="left" title="<?= $print_qty[$id][$row['emb_type']]."*".$print_rate;?>"><? echo number_format($print_qty[$id][$row['emb_type']]*$print_rate,2);?></td>
								<?	if($row['emb_name']==2){?>
							<td width="80" align="left"><? echo number_format($row['emb_qty']);	?>	</td>
							<td width="80" align="left"><? echo number_format($row['emb_rate']);?></td>
							<td width="80" align="left" title="<?=$emb_qty[$id][$row['emb_type']]."*".$embl_rate;?>"><? echo number_format($row['emb_qty']*$row['emb_rate']);?></td>
							<?}else{?>
									<td width="80" align="left"><? echo ""?></td>		
									<td width="80" align="left"><? echo ""?></td>				
									<td width="80" align="left"><? echo ""?></td>								
								<?}?>
...
//   print_r($currency_data);
	...
	$issue_print_data=sql_select("SELECT a.id,a.issue_number,a.issue_date, b.id, b.store_id, b.cons_uom, b.cons_quantity, b.machine_category, b.machine_id, b.prod_id, b.location_id, b.department_id, b.section_id, c.item_category_id, c.item_group_id, b.order_id,c.avg_rate_per_unit,c.company_id,c.item_description	from inv_issue_master a, inv_transaction b, product_details_master c where   b.prod_id=c.id   and  c.item_category_id=22 and  a.entry_form=21 and a.id=b.mst_id and b.order_id in ($order_id) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $date_cond order by a.issue_date asc");
...
<div style="width:700px" align="center" id="details_reports"> 
    ...
    <table id="tbl_id" class="rpt_table" width="" border="1" rules="all" >
        ...
        <tr>
            ...
            <td><?  echo number_format($row[csf("cons_quantity")]);  ?></td>
            <td><?  echo number_format($rate,2);  ?></td>
	   ...
	   $tot_value+=$value;
		...
		<tr>
            ...
            <td></td>
            <td><b>Total Qty</b></td>
            <td><b><?  echo number_format($tot_qty,2);  ?></b></td>
            <td><b>Total Value</b></td>
            <td><b><? echo number_format($tot_value,2); ?></b></td>
        </tr>
    </table>
...
<div style="width:700px" align="center" id="details_reports"> 
 ...
 ?></td>
            ...
            <td><?  echo number_format($rate,2);  ?></td>
            <td><? echo number_format($value2); ?></td>
        </tr>
       <?
	     $tot_qty+=$row[csf("cons_quantity")];
		 $tot_value+=$value;
...
if ($action=='Issue_PrintPopup')
{
			...
			//echo $job_list;
			$print_qty_data=sql_select("SELECT id, po_break_down_id, item_number_id, country_id, production_date, production_quantity,  embel_name,embel_type from pro_garments_production_mst where   embel_name='1' and production_type='3' and production_source=1 and status_active=1 and is_deleted=0 and po_break_down_id in ($orderid) $date_cond order by id");
				...
				<thead>
					...
					$i=1;
						foreach($order_wise_data_arr as $emb_id =>$emb_data){
							...
							$amount=$print_qty[$row['id']][$row['type']]*$print_rate_arr[$row['job_no']][$row['type']];
							if($print_qty[$row['id']][$row['type']]!=''){
							?>
				<tr>
					<td><? echo  $i; ?></td>
					...
					<td><? echo number_format($print_qty[$row['id']][$row['type']],2);  ?></td>
					<td><?  echo number_format($print_rate_arr[$row['job_no']][$row['type']],2);  ?></td>          
					<td><?  echo number_format($amount,2);  ?></td>            
				</tr>
			<?
			$tot_qty +=$print_qty[$row['id']][$row['type']];
			$tot_amount +=$amount;
			$i++;	}
					}
				}?>
				<tr>
					...
					<td><b><?  echo number_format($tot_amount,2);  ?></b></td>            
				</tr>
			</table>
...
if ($action=='Issue_emblPopup')
{
	echo load_html_head_contents("Inhouse Print/Embroidary Production Income Report", "../../../", 1, 1,$unicode,'','');
			...
			embel_name,embel_type from pro_garments_production_mst where   embel_name='2' and production_type='3' and production_source=1 and status_active=1 
