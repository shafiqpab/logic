...
if($action=="report_generate_sheet_kal") //Monthly KPI PER% of KAL
{
	...
	foreach ($sql_po_plan_result as $val) 
	{
		...
		//===========Tejgoan Yarn End=================
		if($val['LOCATION_NAME']==3 && $val['COMPANY_ID']==2) //Location Ashulia//Kal 
		{
	 ...
	 //and b.id in(20325,20326,20327,20328,20329,20330)
	  ...
	  foreach ($sql_po_plan_result_ashulia as $val) 
	  {
		  ...
		  if($val['COMPANY_ID']==2) //Kal Ashulia
		  {
			...
			{
				$ashulia_plan_qty_array[$val['LOCATION_NAME']][$plandate][$val['POID']][$val['COMPANY_ID']]['qc_planQty']+=$val['QC_PLAN_QTY'];
			}
			if($val['INPUT_PLAN_QTY']>0 && $val['TASK_ID']==122 && $val['REF_FROM']==4) //Input
			{
				$ashulia_in_plan_qty_array[$val['LOCATION_NAME']][$plandate][$val['POID']][$val['COMPANY_ID']]['input_planQty']+=$val['INPUT_PLAN_QTY'];
			}
			if($val['OUT_PLAN_QTY']>0 && $val['TASK_ID']==86 && $val['REF_FROM']==5) //Output
			{
				$ashulia_out_plan_qty_array[$val['LOCATION_NAME']][$plandate][$val['POID']][$val['COMPANY_ID']]['out_planQty']+=$val['OUT_PLAN_QTY'];
			}
			if($val['FIN_PLAN_QTY']>0 && $val['TASK_ID']==88 && $val['REF_FROM']==6) //Fin
			{
				$ashulia_gFin_plan_qty_array[$val['LOCATION_NAME']][$plandate][$val['POID']][$val['COMPANY_ID']]['fin_planQty']+=$val['FIN_PLAN_QTY'];
			}
		 }
	  ...
	  from wo_po_details_master a,wo_po_break_down b, pro_garments_production_mst c where a.id=b.job_id and b.id=c.po_break_down_id  and a.location_name in(3,5)  and c.PRODUCTION_QUANTITY>0 and  b.status_active=1 and b.is_deleted=0  and  c.status_active=1 and c.is_deleted=0 and b.id in(20328,20329,20330) and c.production_date between '$startDate' and '$endDate' $company_conds order by c.PRODUCTION_DATE asc"; 
		...
		foreach ($sql_po_result_prod_ash as $val) 
		{
			...
			$Ash_com_poIdArr[$val['COMPANY_ID']][$val['LOCATION_NAME']][$val['POID']]=$val['GROUPING'];
			}
		}
	  ...
	  from wo_po_details_master a,wo_po_break_down b, pro_garments_production_mst c,gbl_temp_engine g where a.id=b.job_id and b.id=c.po_break_down_id   and a.location_name=3 
	   ...
	   //and b.id in(20325,20326,20327,20328,20329,20330)
		...
		foreach ($sql_po_result_prod_curr as $val) 
		{
			if($val['COMPANY_ID']==2) //Kal Ashulia 
			{
				...
				{
					$ashulia_plan_qty_array[$val['LOCATION_NAME']][$propddate][$val['POID']][$val['COMPANY_ID']]['prod_qc']+=$val['PROD_QC'];
				}
				if($val['PRODUCTION_TYPE']==4)
				{
					$ashulia_in_plan_qty_array[$val['LOCATION_NAME']][$propddate][$val['POID']][$val['COMPANY_ID']]['prod_in']+=$val['INPUT_PROD'];
				}
				if($val['PRODUCTION_TYPE']==5)
				{
					$ashulia_out_plan_qty_array[$val['LOCATION_NAME']][$propddate][$val['POID']][$val['COMPANY_ID']]['prod_out']+=$val['OUT_PROD'];
				}
				if($val['PRODUCTION_TYPE']==8)
				{
					$ashulia_gFin_plan_qty_array[$val['LOCATION_NAME']][$propddate][$val['POID']][$val['COMPANY_ID']]['prod_fin']+=$val['FIN_PROD'];
				}
		    }
...
//     echo "<pre>";
//  print_r($ashulia_plan_qty_array);
//  die;
	...
	foreach ($company_wise_arr as   $com_id) 
	{
		...
		{	
			 foreach ($fiscalMonth_arr as $year_mon => $val) 
			{
					...
					//============Ashulia Location============
					//==================Qc Prod=======================
					if(($unit_id==3 || $unit_id==5) && $com_id==2) //Kal
					{
						$diff_days=datediff('d',$from_date,$to_date);
						...
						for($j=0;$j<$diff_days;$j++)
						{
							...
							foreach($ashulia_plan_qty_array[$unit_id][$newdate_ash] as $poid=>$poData)
							{
								...
								if($planQty_qc>0)
								{
									...
									$po_planQtyQc_arr[$unit_id][$newdate_ash][$poid]=$planQty_qc+$po_planQtyQc_arr[$unit_id][$prev_date_planQc[$poid]][$poid];
								...
								if($planQty_qc>0 || $prod_qcQty>0)
								{
									...
									{
									//	if($poid==20326)  echo $newdate."=".$poid."=".$po_alloQty_arr[$prev_allocation_date[$poid]][$poid].'=A <br>';
										$po_prodQcQty_arr[$unit_id][$newdate_ash][$poid]=$po_prodQcQty_arr[$unit_id][$prev_prodQc_date[$poid]][$poid];
									...
									$planQtycal=0;
									$planQtycal=$ashulia_plan_qty_array[$unit_id][$newdate_ash][$poid][$com_id]['qc_planQty'];//Plan
									if($ashulia_plan_qty_array[$unit_id][$newdate_ash][$poid][$com_id]['prod_qc']!="" || $planQtycal>0)
									{
										//if($poid==20326)  echo $newdate."=".$poid."=".$alloQty."+".$po_alloQty_arr[$prev_allocation_date[$poid]][$poid]."=C <br />";
										$po_prodQcQty_arr[$unit_id][$newdate_ash][$poid]=$prod_qcQty+$po_prodQcQty_arr[$unit_id][$prev_prodQc_date[$poid]][$poid];
										$prev_prodQc_date[$poid] = $newdate_ash;
									}
								}
							} 
							...
							foreach($ashulia_in_plan_qty_array[$unit_id][$newdate_ash] as $poid=>$poData)
							{
								...
								{
									//  echo $newdate_ash."=".$poid."=".$planQty_qc."+".$po_planQtyQc_arr[$prev_date_planQc[$poid]][$poid]."<br />";
									$po_planQtyIn_arr[$unit_id][$newdate_ash][$poid]=$planQty_in+$po_planQtyIn_arr[$unit_id][$prev_date_planIn[$poid]][$poid];
								...
								if($planQty_in>0 || $prod_InQty>0)
								{
									...
									{
									//	if($poid==20326)  echo $newdate."=".$poid."=".$po_alloQty_arr[$prev_allocation_date[$poid]][$poid].'=A <br>';
										$po_prodInQty_arr[$unit_id][$newdate_ash][$poid]=$po_prodInQty_arr[$unit_id][$prev_prodIn_date[$poid]][$poid];
									...
									$planInQtycal=0;
									$planInQtycal=$ashulia_in_plan_qty_array[$unit_id][$newdate_ash][$poid][$com_id]['input_planQty'];//Plan
									if($ashulia_in_plan_qty_array[$unit_id][$newdate_ash][$poid][$com_id]['prod_in']!="" || $planInQtycal>0)
									{
										//if($poid==20326)  echo $newdate."=".$poid."=".$alloQty."+".$po_alloQty_arr[$prev_allocation_date[$poid]][$poid]."=C <br />";
										$po_prodInQty_arr[$unit_id][$newdate_ash][$poid]=$prod_InQty+$po_prodInQty_arr[$unit_id][$prev_prodIn_date[$poid]][$poid];
										$prev_prodIn_date[$poid] = $newdate_ash;
									}
								}
							} 
							...
							foreach($ashulia_out_plan_qty_array[$unit_id][$newdate_ash] as $poid=>$poData)
							{
								...
								{
									//  echo $newdate_ash."=".$poid."=".$planQty_qc."+".$po_planQtyQc_arr[$prev_date_planQc[$poid]][$poid]."<br />";
									$po_planQtyOut_arr[$unit_id][$newdate_ash][$poid]=$planQty_out+$po_planQtyOut_arr[$unit_id][$prev_date_planOut[$poid]][$poid];
								...
								if($planQty_out>0 || $prod_OutQty>0)
								{
									...
									{
									//	if($poid==20326)  echo $newdate."=".$poid."=".$po_alloQty_arr[$prev_allocation_date[$poid]][$poid].'=A <br>';
										$po_prodOutQty_arr[$unit_id][$newdate_ash][$poid]=$po_prodOutQty_arr[$unit_id][$prev_prodOut_date[$poid]][$poid];
									...
									$planOutQtycal=0;
									$planOutQtycal=$ashulia_out_plan_qty_array[$unit_id][$newdate_ash][$poid][$com_id]['out_planQty'];//Plan
									if($ashulia_out_plan_qty_array[$unit_id][$newdate_ash][$poid][$com_id]['prod_out']!="" || $planOutQtycal>0)
									{
										//if($poid==20326)  echo $newdate."=".$poid."=".$alloQty."+".$po_alloQty_arr[$prev_allocation_date[$poid]][$poid]."=C <br />";
										$po_prodOutQty_arr[$unit_id][$newdate_ash][$poid]=$prod_OutQty+$po_prodOutQty_arr[$unit_id][$prev_prodOut_date[$poid]][$poid];
										$prev_prodOut_date[$poid] = $newdate_ash;
									}
								}
							}
							//=============Gmts Fin=========================//ashulia_gFin_plan_qty_array
							 ...
							 foreach($ashulia_gFin_plan_qty_array[$unit_id][$newdate_ash] as $poid=>$poData)
							 {
								 ...
								 {
									 //  echo $newdate_ash."=".$poid."=".$planQty_qc."+".$po_planQtyQc_arr[$prev_date_planQc[$poid]][$poid]."<br />";
									 $po_planQtyFin_arr[$unit_id][$newdate_ash][$poid]=$planQty_fin+$po_planQtyFin_arr[$unit_id][$prev_date_planFin[$poid]][$poid];
								 ...
								 if($planQty_fin>0 || $prod_FinQty>0)
								 {
									 ...
									 {
									 //	if($poid==20326)  echo $newdate."=".$poid."=".$po_alloQty_arr[$prev_allocation_date[$poid]][$poid].'=A <br>';
										 $po_prodFinQty_arr[$unit_id][$newdate_ash][$poid]=$po_prodFinQty_arr[$unit_id][$prev_prodFin_date[$poid]][$poid];
									 ...
									 $planFinQtycal=0;
									 $planFinQtycal=$ashulia_gFin_plan_qty_array[$unit_id][$newdate_ash][$poid][$com_id]['fin_planQty'];//Plan
									 if($ashulia_gFin_plan_qty_array[$unit_id][$newdate_ash][$poid][$com_id]['prod_fin']!="" || $planFinQtycal>0)
									 {
										 //if($poid==20326)  echo $newdate."=".$poid."=".$alloQty."+".$po_alloQty_arr[$prev_allocation_date[$poid]][$poid]."=C <br />";
										 $po_prodFinQty_arr[$unit_id][$newdate_ash][$poid]=$prod_FinQty+$po_prodFinQty_arr[$unit_id][$prev_prodFin_date[$poid]][$poid];
										 $prev_prodFin_date[$poid] = $newdate_ash;
									 }
								 }
							 }
	 ...
	 //print_r($avg_tot_kpi_calArr);
	  ...
	  //==================Tejgoan Yarn Month Wise KPI==================
	   ...
	   foreach($Ash_com_poIdArr as $comp_id=>$comData)
		{
			foreach($comData as $unit_id=>$unitData)
			{
		 foreach($unitData as $poid=>$IR)
	     {
				...
				//	$company_kip_cal_Arr=array();//$po_planQtyArr=array();
							...
							for($j=0;$j<$diff_days;$j++)
							{
								...
								//==========Qc Prod and Qc Plan==============
								$planQcQtycal=0;
								$planQcQtycal=$ashulia_plan_qty_array[$unit_id][$day_all][$poid][$comp_id]['qc_planQty'];//alloQty
								//$ashulia_plan_qty_array[3][$plandate][$val['POID']][$val['COMPANY_ID']]['qc_planQty']
								
								if($planQcQtycal>0)
								{
									
									$mon_qc_qty=$po_prodQcQty_arr[$unit_id][$day_all][$poid];
									$qc_month_wise_kpiArr[$comp_id][$unit_id][$day_all]['qc_prod']+=$mon_qc_qty;
								}
								$qc_month_wise_kpiArr[$comp_id][$unit_id][$day_all]['qc_plan']+=$po_planQtyQc_arr[$unit_id][$day_all][$poid];

								//==========In Prod and In Plan==============
								$planInQtycal=0;
								$planInQtycal=$ashulia_in_plan_qty_array[$unit_id][$day_all][$poid][$comp_id]['input_planQty'];//alloQty
								//$ashulia_plan_qty_array[3][$plandate][$val['POID']][$val['COMPANY_ID']]['qc_planQty']
								
								if($planInQtycal>0)
								{
									$mon_in_qty=$po_prodInQty_arr[$unit_id][$day_all][$poid];
									$in_month_wise_kpiArr[$comp_id][$unit_id][$day_all]['in_prod']+=$mon_in_qty;
								}
								$in_month_wise_kpiArr[$comp_id][$day_all]['in_plan']+=$po_planQtyIn_arr[$day_all][$poid];

								//==========Out Qty Prod and Out Plan Qty==============
								$planOutQtycal=0;
								$planOutQtycal=$ashulia_out_plan_qty_array[$unit_id][$day_all][$poid][$comp_id]['out_planQty'];//alloQty
								//$ashulia_plan_qty_array[3][$plandate][$val['POID']][$val['COMPANY_ID']]['qc_planQty']
								//po_prodOutQty_arr
								if($planOutQtycal>0)
								{
									$mon_out_qty=$po_prodOutQty_arr[$unit_id][$day_all][$poid];
									$out_month_wise_kpiArr[$comp_id][$unit_id][$day_all]['out_prod']+=$mon_out_qty;
								}
								$out_month_wise_kpiArr[$comp_id][$unit_id][$day_all]['out_plan']+=$po_planQtyOut_arr[$unit_id][$day_all][$poid];

								//==========GMTS Fin Qty Prod and Fin Plan Qty==============
								$planFinQtycal=0;//po_prodFinQty_arr
								$planFinQtycal=$ashulia_gFin_plan_qty_array[$unit_id][$day_all][$poid][$comp_id]['fin_planQty'];
								//$ashulia_plan_qty_array[3][$plandate][$val['POID']][$val['COMPANY_ID']]['qc_planQty']
								//po_prodOutQty_arr
								if($planFinQtycal>0)
								{
									$mon_fin_qty=$po_prodFinQty_arr[$unit_id][$day_all][$poid];
									$fin_month_wise_kpiArr[$comp_id][$unit_id][$day_all]['fin_prod']+=$mon_fin_qty;
								}
								$fin_month_wise_kpiArr[$comp_id][$unit_id][$day_all]['fin_plan']+=$po_planQtyFin_arr[$unit_id][$day_all][$poid];
						 }
					  }
			   ...
			   $qc_month_kpi_per=$qc_month_prod/$qc_month_plan*100; 
				...
				if($day_chk<=$today)// as on today
				{
					...
					if($qc_month_prod>0 && $qc_month_plan>0)
					{
						...
						// echo  $monthYr.'='.$month_allo.'='. $month_plan.'*100<br>';
					$ash_qc_month_wise_kpiArr[$comp_id][$unit_id][$yr_month]+=$qc_month_kpi_per;
					$ash_month_wise_kpiArr[$comp_id][$unit_id][$day_key]['qc']+=$qc_month_kpi_per;
					}
					if($qc_month_plan>0)
					{
						$mon_grand_qc_event_kpi_perArr[$comp_id][$unit_id][$day_key]=1;

						$ash_num_of_plan_days[$comp_id][$unit_id][$day_key]++;
						$gbl_num_of_plan_daysArr[$comp_id][$unit_id][$yr_month]++;
					}
				}
			}
		   }
	   ...
	   foreach($ash_qc_month_wise_kpiArr as $comp_id=>$comData) //Month Wise KPI Percentage summary part
	   {
			foreach($comData as $unit_id=>$monData) //Month Wise KPI Percentage summary part
			{
				...
				{
					$ash_comp_avg_perArr[$comp_id][$unit_id]+=$per/$ash_num_of_plan_days[$comp_id][3][$monYr];
				}
			}
	   }
	   ...
	   //========****************===============
				...
				$monthYr=date("M-Y",strtotime($day_key));
				 ...
				 if($day_chk<=$today)// as on today
				 {
					 ...
					 if($in_month_prod>0 && $in_month_plan>0)
					 {
						 ...
						 // echo  $monthYr.'='.$month_allo.'='. $month_plan.'*100<br>';
					 $ash_in_month_wise_kpiArr[$comp_id][$unit_id][$yr_month]+=$in_month_kpi_per;
					 $ash_month_wise_kpiArr[$comp_id][$unit_id][$day_key]['in']+=$in_month_kpi_per;
					 }
					$qc_plan_day_chk= $qc_month_wise_kpiArr[$comp_id][$day_key]['qc_plan'];
					 if($in_month_plan>0)
					 {
						 $ash_in_num_of_plan_days[$comp_id][$unit_id][$yr_month]++;
						 $mon_grand_in_event_kpi_perArr[$comp_id][$unit_id][$day_key]=1;
					 }
		...
		{
			foreach($comData as $unit_id=>$comData) //Month Wise Out KPI Percentage summary part
		{
			foreach($comData as $day_key=>$row)
			{
				...
				$out_month_kpi_per=$out_month_prod/$out_month_plan*100; 
				 ...
				 if($day_chk<=$today)// as on today
				 {
					 ...
					 if($out_month_prod>0 && $out_month_plan>0)
					 {
						 ...
						 // echo  $monthYr.'='.$month_allo.'='. $month_plan.'*100<br>';
					 $ash_out_month_wise_kpiArr[$comp_id][$unit_id][$yr_month]+=$out_month_kpi_per;

					 $ash_month_wise_kpiArr[$comp_id][$unit_id][$day_key]['out']+=$out_month_kpi_per;
					 }
					 $qc_plan_day_chk= $qc_month_wise_kpiArr[$comp_id][$day_key]['qc_plan'];
					 ...
					 if($out_month_plan>0)
					 {
						 $ash_out_num_of_plan_days[$comp_id][$unit_id][$yr_month]++;
						 $mon_grand_out_event_kpi_perArr[$comp_id][$unit_id][$day_key]=1;
					 }
		...
		$fin_month_kpi_per=$fin_month_prod/$fin_month_plan*100; 
		 ...
		 if($day_chk<=$today)// as on today
		 {
			 ...
			 if($fin_month_prod>0 && $fin_month_plan>0)
			 {
				 ...
				 // echo  $monthYr.'='.$month_allo.'='. $month_plan.'*100<br>';
			 $ash_fin_month_wise_kpiArr[$comp_id][$unit_id][$yr_month]+=$fin_month_kpi_per;
			 $ash_month_wise_kpiArr[$comp_id][$unit_id][$day_key]['fin']+=$fin_month_kpi_per;
			 }
			 ...
			 if($fin_month_plan>0)
			 {
				 $ash_fin_num_of_plan_days[$comp_id][$unit_id][$yr_month]++;
				 $mon_grand_fin_event_kpi_perArr[$comp_id][$unit_id][$day_key]=1;
			 }
...
foreach($ash_gbl_comp_avg_perArr as $comp_id=>$comData) //Month Wise KPI Percentage summary part
{
	 foreach($comData as $unit_id=>$monData)  
	 {
		 foreach($monData as $day_key=>$val)  
		 {
			...
			// $event_qc_count=$mon_grand_qc_event_kpi_perArr[$comp_id][3][$monYr];//$mon_grand_qc_event_kpi_perArr
			// $event_in_count=$mon_grand_in_event_kpi_perArr[$comp_id][3][$monYr];
			// $event_out_count=$mon_grand_out_event_kpi_perArr[$comp_id][3][$monYr];
			// $event_fin_count=$mon_grand_fin_event_kpi_perArr[$comp_id][3][$monYr];
			// $event_mon_count= $event_qc_count+$event_in_count+$event_out_count+$event_fin_count;
					...
					$allo_Day_wise_calArr=array();//MAY-2023 JUN-2023
						...
						if($year_mon=='MAY-2023' || $year_mon=='JUN-2023'  || $year_mon=='JUL-2023')
						{
							for($j=0;$j<$diff_days;$j++)
							{
								...
								$po_planQty_tillCalculteArr[$day_count]+=$po_planQtyQc_arr[$day_count][$poid];
				 ?>
					 		 <td width="80"  title=" <?=$planQty_cal;  ?>"> <?=$po_planQtyQc_arr[$day_count][$poid]; ?> </td>
             <?
					...
					$allo_Day_wise_calArr=array();//MAY-2023 JUN-2023
						if($year_mon=='MAY-2023' || $year_mon=='JUN-2023'  || $year_mon=='JUL-2023')
						{
							for($j=0;$j<$diff_days;$j++)
							{
								...
								$alloQty_cal=0;
								if($po_planQtyQc_arr[$day_count][$poid]!=0)
								{
									$alloQty_cal=$po_prodQcQty_arr[$day_count][$poid];
								}
					...
					$allo_Day_wise_calArr=array();//MAY-2023 JUN-2023
						...
						if($year_mon=='MAY-2023' || $year_mon=='JUN-2023'  || $year_mon=='JUL-2023')
						{
							for($j=0;$j<$diff_days;$j++)
							{
								...
								//$alloQty=$plan_qty_array[1][$day_count][$poid][$comp_id]['alloQty'];
								$po_planQty_tillCalculteArr[$day_count]+=$po_planQtyIn_arr[$day_count][$poid];
				 ?>
					 		 <td width="80"  title=" <?=$planQty_cal;  ?>"> <?=$po_planQtyIn_arr[$day_count][$poid]; ?> </td>
             <?
					...
					$allo_Day_wise_calArr=array();//MAY-2023 JUN-2023
						if($year_mon=='MAY-2023' || $year_mon=='JUN-2023'  || $year_mon=='JUL-2023')
						{
							for($j=0;$j<$diff_days;$j++)
							{
								...
								$alloQty_cal=0;
								if($po_planQtyIn_arr[$day_count][$poid]!=0)
								{
									$alloQty_cal=$po_prodInQty_arr[$day_count][$poid];
								}
					...
					$allo_Day_wise_calArr=array();//MAY-2023 JUN-2023
						...
						if($year_mon=='MAY-2023' || $year_mon=='JUN-2023'  || $year_mon=='JUL-2023')
						{
							for($j=0;$j<$diff_days;$j++)
							{
								...
								$planQty_cal=$qc_plan_qty_array[1][$day_count][$poid][$comp_id]['planQty'];//alloQty
								//$alloQty=$plan_qty_array[1][$day_count][$poid][$comp_id]['alloQty'];
								$po_planQty_tillCalculteArr[$day_count]+=$po_planQtyOut_arr[$day_count][$poid];
				 ?>
					 		 <td width="80"  title=" <?=$planQty_cal;  ?>"> <?=$po_planQtyOut_arr[$day_count][$poid]; ?> </td>
             <?
					...
					$allo_Day_wise_calArr=array();//MAY-2023 JUN-2023
						if($year_mon=='MAY-2023' || $year_mon=='JUN-2023'  || $year_mon=='JUL-2023')
						{
							for($j=0;$j<$diff_days;$j++)
							{
								...
								$alloQty_cal=0;
								if($po_planQtyOut_arr[$day_count][$poid]!=0)
								{
									$alloQty_cal=$po_prodOutQty_arr[$day_count][$poid];
								}
					...
					$allo_Day_wise_calArr=array();//MAY-2023 JUN-2023
						...
						if($year_mon=='MAY-2023' || $year_mon=='JUN-2023'  || $year_mon=='JUL-2023')
						{
							for($j=0;$j<$diff_days;$j++)
							{
								...
								$planQty_cal=$qc_plan_qty_array[1][$day_count][$poid][$comp_id]['planQty'];//alloQty
								//$alloQty=$plan_qty_array[1][$day_count][$poid][$comp_id]['alloQty'];
								$po_planQty_tillCalculteArr[$day_count]+=$po_planQtyFin_arr[$day_count][$poid];
				 ?>
					 		 <td width="80"  title=" <?=$planQty_cal;  ?>"> <?=$po_planQtyFin_arr[$day_count][$poid]; ?> </td>
             <?
					...
					$allo_Day_wise_calArr=array();//MAY-2023 JUN-2023
						if($year_mon=='MAY-2023' || $year_mon=='JUN-2023'  || $year_mon=='JUL-2023')
						{
							for($j=0;$j<$diff_days;$j++)
							{
								...
								$alloQty_cal=0;
								if($po_planQtyFin_arr[$day_count][$poid]!=0)
								{
									$alloQty_cal=$po_prodFinQty_arr[$day_count][$poid];
								}
						...
						<? }
						  ...
						  <? }
							...
							if($unit_id!=6)  //ash_comp_avg_perArr
							{
								...
								{
									//$ash_comp_avg_perArr[$comp_id][$unit_id]
								$Ash_com_avg_per=$ash_comp_avg_perArr[$com_id][$unit_id];
								
								}
								if($com_id==2 && $unit_id==4) //Ashulia Kal Location
								{
									$unit_id-5;
									//$ash_comp_avg_perArr[$comp_id][$unit_id]
								$Ash_com_avg_per=$ash_comp_avg_perArr[$com_id][5];
								
								}

								$tot_ash_com_avg_per=$yarn_com_avg_per+$Ash_com_avg_per;
								?>
								<td align="center" title="Tot KPI/12">   <?  echo fn_number_format($tot_ash_com_avg_per/12,2).'%'; ?></td>
								<?
							...
							foreach ($fiscalMonth_arr as $year_mon => $val) 
							{
								...
								// ==============Ashulia****======= Kal=================
								if($com_id==2 && ($unit_id==3 || $unit_id==4))
								{
									if($unit_id==4) $unit_id=5;
									$qc_diff_days=$ash_num_of_plan_days[$com_id][$unit_id][$year_mon];
									 // echo $ash_qc_month_wise_kpiArr[$com_id][3][$year_mon].'='.$qc_diff_days . "<br>";
									$qc_month_wise_kpi_per =$ash_qc_month_wise_kpiArr[$com_id][$unit_id][$year_mon];
									$kpi_for_day=0;
									if($qc_month_wise_kpi_per>0)
									{
										$kpi_for_day=1;
									}

									$in_diff_days=$ash_in_num_of_plan_days[$com_id][$unit_id][$year_mon];
									$in_month_wise_kpi_per =$ash_in_month_wise_kpiArr[$com_id][$unit_id][$year_mon];
									if($in_month_wise_kpi_per>0)
									{
										$kpi_for_day+=1;
									}

									$out_diff_days=$ash_out_num_of_plan_days[$com_id][$unit_id][$year_mon];
									$out_month_wise_kpi_per =$ash_out_month_wise_kpiArr[$com_id][$unit_id][$year_mon];
									if($out_month_wise_kpi_per>0)
									{
										$kpi_for_day+=1;
									}
									//ash_fin_month_wise_kpiArr
									$fin_diff_days=$ash_fin_num_of_plan_days[$com_id][$unit_id][$year_mon];
									$fin_month_wise_kpi_per =$ash_fin_month_wise_kpiArr[$com_id][$unit_id][$year_mon];
									if($fin_month_wise_kpi_per>0)
									{
										$kpi_for_day+=1;
									}
									//ash_fin_month_wise_kpiArr
									// echo $kpi_for_day.'=DDDDDDDDDDDD';//$gbl_num_of_plan_daysArr[$com_id][3][$year_mon]
									$mon_days_count=$gbl_num_of_plan_daysArr[$com_id][$unit_id][$year_mon];

									// $event_qc_count=$mon_grand_qc_event_kpi_perArr[$com_id][3][$year_mon];//$mon_grand_qc_event_kpi_perArr
									// $event_in_count=$mon_grand_in_event_kpi_perArr[$com_id][3][$year_mon];
									// $event_out_count=$mon_grand_out_event_kpi_perArr[$com_id][3][$year_mon];
									// $event_fin_count=$mon_grand_fin_event_kpi_perArr[$com_id][3][$year_mon];
									// $event_mon_count= $event_qc_count+$event_in_count+$event_out_count+$event_fin_count;

									//$month_kpi_per =($qc_month_wise_kpi_per+$in_month_wise_kpi_per+$out_month_wise_kpi_per+$fin_month_wise_kpi_per)/$event_mon_count;
									$month_kpi_per =$ash_gbl_comp_mon_avg_perArr[$com_id][$unit_id][$year_mon];
...
if($action=="report_generate_by_year_ashulia_kal") //Unit Ashulia//Kal and Monthly Date Wise KPI PER%  
{
	...
	//$location_library = return_library_array("select id, location_name from lib_location","id", "location_name");// where company_id=$cbo_company_id
	// ========================= for Plan ======================
	  $sql_po_plan="SELECT a.company_name as COMPANY_ID,a.LOCATION_NAME,b.id as POID,b.GROUPING,c.TASK_ID, c.PLAN_QTY,c.PLAN_DATE, b.job_no_mst as JOB_NO from wo_po_details_master a,wo_po_break_down b, tna_plan_target c where a.id=b.job_id and b.id=c.po_id and a.location_name=$unit_id and c.task_id in(84,122,86,88)   and c.task_type=1 and c.plan_qty>0 and  b.status_active=1 and b.is_deleted=0 and b.id in(20328,20329,20330)   and c.PLAN_DATE between '$startDate' and '$endDate' $company_conds order by c.PLAN_DATE asc"; //and b.id in(20325,20326,20327,20328,20329,20330)
	$sql_po_plan_result = sql_select($sql_po_plan);
	foreach ($sql_po_plan_result as $val) 
	{
		 $company_wise_arr[$val['COMPANY_ID']]=$val['COMPANY_ID'];
	...
	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 89, 6, $plan_fin_poIdArr, $empty_arr);//PO ID Ref from=6
	 ...
	 $sql_po_plan_ashulia="SELECT a.company_name as COMPANY_ID,b.id as POID,c.PLAN_DATE,c.TASK_ID,g.REF_FROM, b.job_no_mst as JOB_NO,
	(case when  c.task_id=84  and g.ref_from=3  then c.PLAN_QTY else 0 end) as QC_PLAN_QTY,
	(case when  c.task_id=122  and g.ref_from=4  then c.PLAN_QTY else 0 end) as INPUT_PLAN_QTY,
	(case when  c.task_id=86  and g.ref_from=5  then c.PLAN_QTY else 0 end) as OUT_PLAN_QTY,
	(case when  c.task_id=88  and g.ref_from=6  then c.PLAN_QTY else 0 end) as FIN_PLAN_QTY 
	from wo_po_details_master a,wo_po_break_down b, tna_plan_target c,gbl_temp_engine g where a.id=b.job_id and b.id=c.po_id and a.location_name=$unit_id   and c.task_type=1 and c.plan_qty>0 and b.id=g.ref_val and c.po_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in(3,4,5,6) and g.entry_form=89  and  c.status_active=1 and c.is_deleted=0 and  b.status_active=1 and b.is_deleted=0 $company_conds and c.PLAN_DATE <= '$endDate' order by c.PLAN_DATE asc"; 
	 //and b.id in(20325,20326,20327,20328,20329,20330)
	  $sql_po_plan_result_ashulia = sql_select($sql_po_plan_ashulia);
	  ...
	  foreach ($sql_po_plan_result_ashulia as $val) 
	  {
		  ...
		  //echo $plandate.'<br>';;
			...
			if($val['QC_PLAN_QTY']>0 && $val['TASK_ID']==84 && $val['REF_FROM']==3) //QC
			{
				$ashulia_plan_qty_array[$unit_id][$plandate][$val['POID']][$val['COMPANY_ID']]['qc_planQty']+=$val['QC_PLAN_QTY'];
			}
			if($val['INPUT_PLAN_QTY']>0 && $val['TASK_ID']==122 && $val['REF_FROM']==4) //Input
			{
				$ashulia_in_plan_qty_array[$unit_id][$plandate][$val['POID']][$val['COMPANY_ID']]['input_planQty']+=$val['INPUT_PLAN_QTY'];
			}
			if($val['OUT_PLAN_QTY']>0 && $val['TASK_ID']==86 && $val['REF_FROM']==5) //Output
			{
				$ashulia_out_plan_qty_array[$unit_id][$plandate][$val['POID']][$val['COMPANY_ID']]['out_planQty']+=$val['OUT_PLAN_QTY'];
			}
			if($val['FIN_PLAN_QTY']>0 && $val['TASK_ID']==88 && $val['REF_FROM']==6) //Fin
			{
				$ashulia_gFin_plan_qty_array[$unit_id][$plandate][$val['POID']][$val['COMPANY_ID']]['fin_planQty']+=$val['FIN_PLAN_QTY'];
			}
	  ...
	  from wo_po_details_master a,wo_po_break_down b, pro_garments_production_mst c where a.id=b.job_id and b.id=c.po_break_down_id  and a.location_name=$unit_id  and c.PRODUCTION_QUANTITY>0 and  b.status_active=1 and b.is_deleted=0  and  c.status_active=1 and c.is_deleted=0 and b.id in(20328,20329,20330) and c.production_date between '$startDate' and '$endDate' $company_conds order by c.PRODUCTION_DATE asc"; 
		...
		fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 89, 7, $prod_poIdArr, $empty_arr);//PO ID Ref from=7
		  ...
		  $sql_po_prod_ashulia_curr="SELECT a.company_name as COMPANY_ID,b.id as POID,c.PRODUCTION_DATE,c.PRODUCTION_TYPE,g.REF_FROM, b.job_no_mst as JOB_NO,
	  (case when  c.production_type=1    then c.PRODUCTION_QUANTITY else 0 end) as PROD_QC,
	  (case when  c.production_type=4   then c.PRODUCTION_QUANTITY else 0 end) as INPUT_PROD,
	  (case when  c.production_type=5     then c.PRODUCTION_QUANTITY else 0 end) as OUT_PROD,
	  (case when  c.production_type=8     then c.PRODUCTION_QUANTITY else 0 end) as FIN_PROD 
	  from wo_po_details_master a,wo_po_break_down b, pro_garments_production_mst c,gbl_temp_engine g where a.id=b.job_id and b.id=c.po_break_down_id   and a.location_name=$unit_id 
	  and b.id=g.ref_val and c.po_break_down_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in(7) and g.entry_form=89
	   and c.PRODUCTION_QUANTITY>0  and  b.status_active=1 and b.is_deleted=0  and  c.status_active=1 and c.is_deleted=0 and c.production_date <= '$endDate' $company_conds order by c.PRODUCTION_DATE asc"; 
	   //and b.id in(20325,20326,20327,20328,20329,20330)
		$sql_po_result_prod_curr = sql_select($sql_po_prod_ashulia_curr);
		foreach ($sql_po_result_prod_curr as $val) 
		{
				...
				if($val['PRODUCTION_TYPE']==1)
				{
					$ashulia_plan_qty_array[$unit_id][$propddate][$val['POID']][$val['COMPANY_ID']]['prod_qc']+=$val['PROD_QC'];
				}
				if($val['PRODUCTION_TYPE']==4)
				{
					$ashulia_in_plan_qty_array[$unit_id][$propddate][$val['POID']][$val['COMPANY_ID']]['prod_in']+=$val['INPUT_PROD'];
				}
				if($val['PRODUCTION_TYPE']==5)
				{
					$ashulia_out_plan_qty_array[$unit_id][$propddate][$val['POID']][$val['COMPANY_ID']]['prod_out']+=$val['OUT_PROD'];
				}
				if($val['PRODUCTION_TYPE']==8)
				{
					$ashulia_gFin_plan_qty_array[$unit_id][$propddate][$val['POID']][$val['COMPANY_ID']]['prod_fin']+=$val['FIN_PROD'];
				}
		...
		<table border="1" rules="all" class="rpt_table" width="<? echo $tbl_width_unit;?>" cellpadding="0" cellspacing="0">
	        ...
	        <thead>
		        ...
		        <?
						...
						if($day_chk<=$today)// as on today
						{
							$ash_gbl_comp_avg_kip=$ash_gbl_comp_avg_perArr[$cbo_company_id][$unit_id][$date];
	        ...
	        <tfoot>
				...
				foreach ($fiscalMonth_arr as $year_mon => $val) 
	            {
					...
					$tot_mon_days=$gbl_num_of_plan_daysArr[$cbo_company_id][$unit_id][$year_mon];
