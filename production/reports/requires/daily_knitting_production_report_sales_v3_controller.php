					...
					<table cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
						<table cellpadding="0" cellspacing="0" width="600" border="1" rules="all" class="rpt_table">
							...
							<tr class="general">
								...
								<td align="center">
									<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_within_group').value+'_'+document.getElementById('hidden_yearID').value+'_'+document.getElementById('cbo_serach_type').value+'_'+'<? echo $companyID; ?>', 'create_sales_order_no_search_list', 'search_div', 'daily_knitting_production_report_sales_v3_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
								</td>
							</tr>
						</table>
...
if ($action == "create_sales_order_no_search_list") {
	...
	$sql = "SELECT a.id, $year_field, a.job_no_prefix_num, a.job_no, a.within_group, a.sales_booking_no,a.booking_date, a.buyer_id, a.style_ref_no, a.location_id from fabric_sales_order_mst a where a.status_active=1 and a.is_deleted=0 and a.company_id in($companyID) $within_group_cond $search_field_cond $sales_order_cond $year_cond order by a.id";
					...
					<div id="content_search_panel">
						<table cellpadding="0" cellspacing="0" width="100%" class="rpt_table" border="1" rules="all">
							...
							<tr>
								...
								<td align="center">
									<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_company_id').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value +'_'+document.getElementById('cbo_buyer').value+'_'+'<? echo $cbo_booking_type; ?>', 'create_booking_search_list_view', 'search_div', 'daily_knitting_production_report_sales_v3_controller', 'setFilterGrid(\'tbl_list_search\',-1, tableFilters);'); accordion_menu(accordion_h1.id,'content_search_panel','')" style="width:100px;" />
								</td>
							</tr>
							<tr>
						...
						<table cellpadding="0" cellspacing="0" width="600" border="1" rules="all" class="rpt_table">
							...
							<tr class="general">
								...
								<td align="center">
									...
									echo create_drop_down("cbo_serach_type", 150, $serach_type_arr, "", 0, "--Select--", 3, "", 0);
									?>
								</td>
								<td align="center">
									<input type="text" style="width:140px" class="text_boxes" name="txt_search_common" id="txt_search_common" placeholder="Write" />
								...
								<td align="center">
									<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_within_group').value+'_'+document.getElementById('hidden_yearID').value+'_'+document.getElementById('cbo_serach_type').value, 'create_internalref_no_search_list', 'search_div', 'daily_knitting_production_report_sales_v3_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
								</td>
							</tr>
						</table>
						<div style="margin-top:15px" id="search_div"></div>
...
if ($action == "report_generate") 
{
    ...
    $shift_id = str_replace("'", "", $cbo_shift);
	$shift_cond .= ($shift_id!=0) ? "and shift_name =$shift_id " : ""; 
	//var_dump($txt_date_to);
	if (str_replace("'", "", $cbo_working_company_name)>0) $working_company_cond = " and a.knitting_company=$cbo_working_company_name ";

	$sales_order_cond = ($sales_order_no != "") ? " and e.job_no like '%$sales_order_no%' " : "";
	//echo $cbo_type.'=='.$report_type;

	$vari_knit_charge_source_arr = sql_select("select editable from variable_order_tracking where company_name in($cbo_company_name) and variable_list=70 and status_active=1");
	...
	if ($report_type == 1) // Show
	{
		...
		$tbl_width = 2680 + $width + count($shift_name) * 60;
		ob_start();
		?>
			...
			<tbody>
				...
				if ($db_type == 0) {
					...
					foreach ($shift_name as $key => $val) {
						...
						$sql_inhouse .= ", sum(case when b.shift_name=$key then b.no_of_roll else 0 end) as noofrollshift" . strtolower($val);
					}
					$sql_inhouse .= " from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c, lib_machine_name d, wo_po_break_down e,  wo_po_details_master f 
						where a.id=b.mst_id and b.id=c.dtls_id and b.machine_no_id=d.id and e.job_no_mst=f.job_no and c.po_breakdown_id=e.id and a.entry_form=2 and a.item_category=13 and c.entry_form=2 and c.trans_type=1 and a.knitting_source=1 and a.company_id in($cbo_company_name) $working_company_cond and a.knitting_source like '$source' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $date_con $floor_id $buyer_cond $job_cond $order_cond $job_year_cond
					...
					$sql_inhouse .= " from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c,fabric_sales_order_mst e ,wo_non_ord_samp_booking_mst g where a.id=b.mst_id and b.id=c.dtls_id and c.po_breakdown_id=e.id and e.sales_booking_no=g.booking_no and g.status_active=1 and g.is_deleted=0 and a.entry_form=2 and a.item_category=13 and c.entry_form=2 and c.trans_type=1 and a.company_id in($cbo_company_name) $working_company_cond and a.knitting_source like '$source' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $date_con $floor_id $buyer_cond $sales_order_cond $booking_no_cond $within_group_cond $cust_buyer_cond group by b.kniting_charge, b.machine_no_id,a.receive_date,e.job_no,e.sales_booking_no, e.within_group,a.company_id,a.receive_basis,a.insert_date,a.inserted_by, a.booking_no, g.booking_type, g.entry_form_id, b.floor_id, a.knitting_source,a.knitting_company,c.is_sales,e.buyer_id,e.customer_buyer)";

					$sql_inhouse .= " union all (select b.kniting_charge, a.company_id,a.receive_basis,a.insert_date,a.inserted_by, a.receive_date, a.booking_no,999 as booking_type, 1 as is_order, null as entry_form, listagg((cast(a.remarks as varchar2(4000))),',') within group (order by a.remarks) as remarks, listagg((cast(b.id as varchar2(4000))),',') within group (order by b.id) as dtls_id, listagg((cast(b.prod_id as varchar2(4000))),',') within group (order by b.prod_id) as prod_id, listagg((cast(b.febric_description_id as varchar2(4000))),',') within group (order by b.febric_description_id) as febric_description_id, listagg((cast(b.body_part_id as varchar2(4000))),',') within group (order by b.body_part_id) as body_part_id, listagg((cast(b.gsm as varchar2(4000))),',') within group (order by b.gsm) as gsm, listagg((cast(b.width as varchar2(4000))),',') within group (order by b.width) as width, listagg((cast(b.yarn_lot as varchar2(4000))),',') within group (order by b.yarn_lot) as yarn_lot, listagg((cast(b.yarn_count as varchar2(4000))),',') within group (order by b.yarn_count) as yarn_count, listagg((cast(b.stitch_length as varchar2(4000))),',') within group (order by b.stitch_length) as stitch_length, listagg((cast(b.brand_id as varchar2(4000))),',') within group (order by b.brand_id) as brand_id, b.machine_no_id, b.floor_id as floor_id, listagg((cast(b.color_id as varchar2(4000))),',') within group (order by b.color_id) as color_id, listagg((cast(b.color_range_id as varchar2(4000))),',') within group (order by b.color_range_id) as color_range_id, listagg((cast(c.po_breakdown_id as varchar2(4000))),',') within group (order by c.po_breakdown_id) as po_breakdown_id, e.job_no,e.sales_booking_no,sum(b.reject_fabric_receive) as reject_qty,c.is_sales,e.buyer_id unit_id,e.within_group,a.knitting_source,a.knitting_company,e.buyer_id,e.customer_buyer,sum(case when b.shift_name=0 then c.quantity else 0 end) as without_shift,sum(case when b.shift_name=0 then b.no_of_roll else 0 end) as without_shift_noofroll, listagg((cast(b.yarn_prod_id as varchar2(4000))),',') within group (order by b.yarn_prod_id) as yarn_prod_id, sum(case when b.shift_name=1 then c.quantity else 0 end) as qntyshifta, sum(case when b.shift_name=1 then b.no_of_roll else 0 end) as noofrollshifta, sum(case when b.shift_name=2 then c.quantity else 0 end) as qntyshiftb,  sum(case when b.shift_name=2 then b.no_of_roll else 0 end) as noofrollshiftb, sum(case when b.shift_name=3 then b.no_of_roll else 0 end) as noofrollshiftc,sum(case when b.shift_name=3 then c.quantity else 0 end) as qntyshiftc";
				
					$sql_inhouse .= " from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c,fabric_sales_order_mst e  where a.id=b.mst_id and b.id=c.dtls_id and c.po_breakdown_id=e.id and e.within_group=2 and a.entry_form=2 and a.item_category=13 and c.entry_form=2 and c.trans_type=1 and a.company_id in($cbo_company_name) $working_company_cond and a.knitting_source like '$source' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $date_con $floor_id $buyer_cond $sales_order_cond $booking_no_cond $within_group_cond $cust_buyer_cond group by b.kniting_charge, b.machine_no_id,a.receive_date,e.job_no,e.sales_booking_no, e.within_group,a.company_id,a.receive_basis,a.insert_date,a.inserted_by, a.booking_no, b.floor_id, a.knitting_source,a.knitting_company,c.is_sales,e.buyer_id,e.customer_buyer)
				...
				if (str_replace("'", "", $cbo_knitting_source) == 0 || str_replace("'", "", $cbo_knitting_source) == 2) 
				{
					...
					$sql_inhouse_sub = " SELECT 999 as receive_basis,a.insert_date,a.inserted_by, a.product_date as receive_date, a.program_no as booking_no, 999 as booking_type, 1 as is_order, null as entry_form, listagg((cast(a.remarks as varchar2(4000))),',') within group (order by a.remarks) as remarks, listagg((cast(b.id as varchar2(4000))),',') within group (order by b.id) as dtls_id, listagg((cast(b.cons_comp_id as varchar2(4000))),',') within group (order by b.cons_comp_id) as prod_id, 0 as febric_description_id,
						...
						sum(case when b.shift=0 then b.no_of_roll else 0 end) as without_shift_noofroll,
						sum(case when b.shift=1 then b.no_of_roll else 0 end) as noofrollshifta,
						sum(case when b.shift=2 then b.no_of_roll else 0 end) as noofrollshiftb,
						sum(case when b.shift=3 then b.no_of_roll else 0 end) as noofrollshiftc,a.company_id,
						...
						and a.status_active=1 and a.is_deleted=0 and a.company_id in($cbo_company_name) $working_company_cond $date_con_sub $floor_id $buyer_id_cond $job_no_cond $order_no_cond $job_year_sub_cond $sales_order_cond $booking_no_cond $within_group_cond $cust_buyer_in_bound_cond
				...
				$machine_inhouse_array = $total_running_machine = $cust_buyer_wise_production_arr =  $cust_buyer_wise_machine = $buyer_wise_production_arr = $job_wise_machine = array();
				foreach ($nameArray_inhouse as $row) {
		...
		<fieldset style="width:<? echo $tbl_width + 20; ?>px;">
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $tbl_width+600; ?>" class="rpt_table" align="left"	>
				...
				<thead>
					<tr>
						...
						<th colspan="<? echo count($shift_name)+ count($shift_name); ?>">Production</th>
						<th width="100" rowspan="2">Shift Total</th>
						<th width="60" rowspan="2">No of roll</th>
						<?
						if ($vari_knit_charge_source == 2) {
						?>
			...
			<div style="width:<? echo $tbl_width+20+600; ?>px; overflow-y:scroll; max-height:350px;" id="scroll_body" align="left">
				<table align="left" cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $tbl_width+600; ?>" class="rpt_table" id="table_body_main">
								...
								foreach ($nameArray_inhouse as $row) {
									...
									if ($row[csf("knitting_source")] == 3) // Out-bound
									{
										...
										<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" valign="top">
											<td width="30" align="center"><? $km++;
																echo $km;  ?></td>
											<td width="60" align="center"><? echo ($row[csf('within_group')] == 1) ? "Yes" : "No"; ?></td>
											<td width="100" align="center"><? echo $knitting_party; ?></td>
											<td width="100" align="center"><? echo $floor_details[$row[csf('floor_id')]]; ?></td>
											<td width="90" align="center"><? echo $machine_arr[$row[csf('machine_no_id')]]; ?></td>
											...
											<td width="140" align="center"><strong>P:</strong><? echo $row[csf('booking_no')] . "<br /><strong>S:</strong>" . $row[csf('job_no')]; ?>
											</td>
											...
											<td width="100" align="center">
												<p style="word-break:break-all;"><? echo $body_partName; ?></p>
											</td>
											<td width="100">
												<P>
													<?
											...
											$row_tot_roll = 0;
											$row_tot_qnty = 0;
											$row_tot_no_of_roll = 0;
											foreach ($shift_name as $key => $val) {
												$row_tot_qnty += $row[csf('qntyshift' . strtolower($val))];
												$row_tot_no_of_roll += $row[csf('noofrollshift' . strtolower($val))];
											?>
												<td width="60" align="right" title="<? echo $key . "==" . $val; ?> "><? echo number_format($row[csf('qntyshift' . strtolower($val))], 2); ?> </td>
												<td width="60" align="right" title="<? echo $key . "==" . $val; ?> "><? echo number_format($row[csf('noofrollshift' . strtolower($val))], 2); ?> </td>
											<?
											...
											<td width="60" align="right">
												<? echo number_format($row_tot_no_of_roll, 2, '.', ''); ?> 
											</td>
											
											<?
											if ($vari_knit_charge_source == 2) {
											?>
												<td width="100"  align="right"><? echo number_format($row[csf('kniting_charge')], 2, '.', ''); ?></td>
												<td width="100"align="right"><? $charge_amount = $row_tot_qnty * $row[csf('kniting_charge')];
									...
									else // In-house
									{
										...
										?>
											<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" valign="top">
												...
												$km++;
												?>
												<td align="center" style="width: 30px;"><? echo $km; ?></td>
												<td width="60" align="center"><? echo ($row[csf('within_group')] == 1) ? "Yes" : "No"; ?></td>
												<td width="100" align="center">
												...
												$row_tot_no_of_roll = 0;
												foreach ($shift_name as $key => $val) {
													if($key==$shift_id){$row_tot_qnty += $row[csf('qntyshift' . strtolower($val))]; }elseif($shift_id==0){
														$row_tot_qnty += $row[csf('qntyshift' . strtolower($val))];
													}
													$row_tot_no_of_roll += $row[csf('noofrollshift' . strtolower($val))];
												?>
													<td width="60" align="right"><? 
													if($key==$shift_id){ echo number_format($row[csf('qntyshift' . strtolower($val))], 2);}elseif($shift_id==0){
														echo number_format($row[csf('qntyshift' . strtolower($val))], 2);
													}
													//echo number_format($row[csf('qntyshift' . strtolower($val))], 2); ?> </td>
													<td width="60" align="right"><? echo number_format($row[csf('noofrollshift' . strtolower($val))], 2); ?> </td>
												<?
													$grand_total_ship[$key] += $row[csf('qntyshift' . strtolower($val))];
													$inhouse_ship[$key] += $row[csf('qntyshift' . strtolower($val))];
												}
												...
												<td width="60" align="right">
													...
													<? echo number_format($row_tot_no_of_roll, 2, '.', ''); ?></td>
									...
									foreach ($nameArray_inhouse_subcon as $row) {
										...
										?>
											<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" valign="top">
												...
												<td width="140" align="center"><strong>P:</strong><? echo $row[csf('booking_no')] . "<br /><strong>S:</strong>" . $row[csf('job_no')]; ?></td>
												
												<td width="140" align="center"><? echo $order_nos; ?></td>
												<td width="100" align="center"><? //echo chop($internalRefArr[$row[csf("sales_booking_no")]]['grouping'],","); ?></td>
												<td width="70" align="center">
													<p><? echo $count; ?></p>
												</td>
												<td width="150" align="center">
												...
												foreach ($shift_name as $key => $val) {
													...
													$row_tot_no_of_roll += $row[csf('noofrollshift' . strtolower($val))];
												?>
													<td width="60" align="right"><? echo number_format($row[csf('qntyshift' . strtolower($val))], 2); ?> </td>
													<td width="60" align="right"><? echo number_format($row[csf('noofrollshift' . strtolower($val))], 2); ?> </td>
												<?
													$grand_total_ship[$key] += $row[csf('qntyshift' . strtolower($val))];
												...
												if ($vari_knit_charge_source == 2) {
												?>
													<td width="100" align="right"><? echo number_format($row[csf('rate')], 2, '.', ''); ?></td>
													<td width="100" align="right"><? echo number_format($row_tot_qnty * $row[csf('rate')], 2, '.', ''); ?></td>
												<?
												}
												?>
		    ...
		    <table class="rpt_table" width="<? echo $tbl_width+600; ?>" cellpadding="0" cellspacing="0" border="1" rules="all">
	            <tfoot>
	                <tr>
	                	<th width="30">&nbsp;</th>
						<th width="60">&nbsp;</th>
						...
						<?
							foreach ($shift_name as $key => $val) {
							?>
								<th width="60" class="word_wrap_break" id="value_shift_qnty_<? echo $key; ?>" style="text-align: right"><? echo number_format($grand_total_ship[$key], 2, '.', ''); ?></th>
								<th width="60">&nbsp;</th>
							<?
							}
						?>
						...
						<th width="100" class="word_wrap_break" id="value_shift_qnty_total" style="text-align: right"><? echo number_format($grand_tot_qnty + $outBoundWithoutShiftQnty, 2, '.', ''); ?></th>
						<th width="60">&nbsp;</th>
						<?
						if ($vari_knit_charge_source == 2) {
						?>
						...
						<th width="80" class="word_wrap_break" id="value_shift_reject_qnty" style="text-align: right"><? echo number_format($total_grand_rejectQnty, 2, '.', ''); ?></th>
						<th> &nbsp;</th>
					</tr>
	            </tfoot>
	        </table>
	...
	else // Show 2
	{
		...
		$width = 0;
		$colspan = 0;
		$tbl_width = 2280 + $width + count($shift_name) * 60;

		$con = connect();
		execute_query("DELETE from GBL_TEMP_ENGINE where user_id=$user_id and entry_form=169");
		oci_commit($con);
		...
		<table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table">
			...
			<tbody>
				...
				{
					$sql_inhouse = "select * from (
						(select a.company_id,a.receive_basis, a.receive_date, a.booking_no,nvl(f.booking_type, 1) booking_type, 1 as is_order, f.entry_form, listagg((cast(a.remarks as varchar2(4000))),',') within group (order by a.remarks) as remarks, listagg((cast(b.id as varchar2(4000))),',') within group (order by b.id) as dtls_id, listagg((cast(b.prod_id as varchar2(4000))),',') within group (order by b.prod_id) as prod_id, listagg((cast(b.febric_description_id as varchar2(4000))),',') within group (order by b.febric_description_id) as febric_description_id,listagg((cast(b.body_part_id as varchar2(4000))),',') within group (order by b.body_part_id) as body_part_id, listagg((cast(b.gsm as varchar2(4000))),',') within group (order by b.gsm) as gsm, listagg((cast(b.width as varchar2(4000))),',') within group (order by b.width) as width, listagg((cast(b.yarn_lot as varchar2(4000))),',') within group (order by b.yarn_lot) as yarn_lot, listagg((cast(b.yarn_count as varchar2(4000))),',') within group (order by b.yarn_count) as yarn_count, listagg((cast(b.stitch_length as varchar2(4000))),',') within group (order by b.stitch_length) as stitch_length, listagg((cast(b.brand_id as varchar2(4000))),',') within group (order by b.brand_id) as brand_id, b.machine_no_id, b.floor_id as floor_id, listagg((cast(b.color_id as varchar2(4000))),',') within group (order by b.color_id) as color_id, listagg((cast(b.color_range_id as varchar2(4000))),',') within group (order by b.color_range_id) as color_range_id, listagg((cast(c.po_breakdown_id as varchar2(4000))),',') within group (order by c.po_breakdown_id) as po_breakdown_id, e.job_no,e.sales_booking_no,e.booking_id,sum(b.reject_fabric_receive) as reject_qty,c.is_sales,e.buyer_id unit_id,e.within_group,a.knitting_source,a.knitting_company,e.buyer_id,e.customer_buyer,sum(case when b.shift_name=0 then c.quantity else 0 end) as without_shift,sum(case when b.shift_name=0 then b.no_of_roll else 0 end) as without_shift_noofroll, listagg((cast(b.yarn_prod_id as varchar2(4000))),',') within group (order by b.yarn_prod_id) as yarn_prod_id";
					foreach ($shift_name as $key => $val) 
					{
						$sql_inhouse .= ", sum(case when b.shift_name=$key then c.quantity else 0 end) as qntyshift" . strtolower($val);
						$sql_inhouse .= ", sum(case when b.shift_name=$key then b.no_of_roll else 0 end) as noofrollshift" . strtolower($val);
					...
					$sql_inhouse .= " from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c,fabric_sales_order_mst e,  wo_booking_mst f  where a.id=b.mst_id and b.id=c.dtls_id and c.po_breakdown_id=e.id and e.sales_booking_no=f.booking_no and a.entry_form=2 and a.item_category=13 and c.entry_form=2 and c.trans_type=1 and a.company_id in($cbo_company_name) $working_company_cond and a.knitting_source like '$source' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and f.status_active=1 and f.is_deleted=0 $date_con $floor_id $buyer_cond $sales_order_cond $booking_no_cond $within_group_cond $cust_buyer_cond 
					group by b.machine_no_id,a.receive_date,e.job_no,e.sales_booking_no,e.booking_id, e.within_group,a.company_id,a.receive_basis, a.booking_no, f.booking_type, f.entry_form, b.floor_id, a.knitting_source,a.knitting_company,c.is_sales,e.buyer_id,e.customer_buyer)";

					$sql_inhouse .= " union all  (select a.company_id,a.receive_basis, a.receive_date, a.booking_no,nvl(g.booking_type, 1) booking_type, 2 as is_order, g.entry_form_id as entry_form, listagg((cast(a.remarks as varchar2(4000))),',') within group (order by a.remarks) as remarks, listagg((cast(b.id as varchar2(4000))),',') within group (order by b.id) as dtls_id, listagg((cast(b.prod_id as varchar2(4000))),',') within group (order by b.prod_id) as prod_id, listagg((cast(b.febric_description_id as varchar2(4000))),',') within group (order by b.febric_description_id) as febric_description_id, listagg((cast(b.body_part_id as varchar2(4000))),',') within group (order by b.body_part_id) as body_part_id, listagg((cast(b.gsm as varchar2(4000))),',') within group (order by b.gsm) as gsm, listagg((cast(b.width as varchar2(4000))),',') within group (order by b.width) as width, listagg((cast(b.yarn_lot as varchar2(4000))),',') within group (order by b.yarn_lot) as yarn_lot, listagg((cast(b.yarn_count as varchar2(4000))),',') within group (order by b.yarn_count) as yarn_count, listagg((cast(b.stitch_length as varchar2(4000))),',') within group (order by b.stitch_length) as stitch_length, listagg((cast(b.brand_id as varchar2(4000))),',') within group (order by b.brand_id) as brand_id, b.machine_no_id, b.floor_id as floor_id, listagg((cast(b.color_id as varchar2(4000))),',') within group (order by b.color_id) as color_id, listagg((cast(b.color_range_id as varchar2(4000))),',') within group (order by b.color_range_id) as color_range_id, listagg((cast(c.po_breakdown_id as varchar2(4000))),',') within group (order by c.po_breakdown_id) as po_breakdown_id, e.job_no,e.sales_booking_no,e.booking_id,sum(b.reject_fabric_receive) as reject_qty,c.is_sales,e.buyer_id unit_id,e.within_group,a.knitting_source,a.knitting_company,e.buyer_id,e.customer_buyer,sum(case when b.shift_name=0 then c.quantity else 0 end) as without_shift,sum(case when b.shift_name=0 then b.no_of_roll else 0 end) as without_shift_noofroll, listagg((cast(b.yarn_prod_id as varchar2(4000))),',') within group (order by b.yarn_prod_id) as yarn_prod_id
						";
					...
					$sql_inhouse .= " from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c,fabric_sales_order_mst e ,wo_non_ord_samp_booking_mst g where a.id=b.mst_id and b.id=c.dtls_id and c.po_breakdown_id=e.id and e.sales_booking_no=g.booking_no and g.status_active=1 and g.is_deleted=0 and a.entry_form=2 and a.item_category=13 and c.entry_form=2 and c.trans_type=1 and a.company_id in($cbo_company_name) $working_company_cond and a.knitting_source like '$source' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $date_con $floor_id $buyer_cond $sales_order_cond $booking_no_cond $within_group_cond $cust_buyer_cond 
					group by b.machine_no_id,a.receive_date,e.job_no,e.sales_booking_no,e.booking_id, e.within_group,a.company_id,a.receive_basis, a.booking_no, g.booking_type, g.entry_form_id, b.floor_id, a.knitting_source,a.knitting_company,c.is_sales,e.buyer_id,e.customer_buyer)";

					$sql_inhouse .= " union all (select a.company_id,a.receive_basis, a.receive_date, a.booking_no,999 as booking_type, 1 as is_order, null as entry_form, listagg((cast(a.remarks as varchar2(4000))),',') within group (order by a.remarks) as remarks, listagg((cast(b.id as varchar2(4000))),',') within group (order by b.id) as dtls_id, listagg((cast(b.prod_id as varchar2(4000))),',') within group (order by b.prod_id) as prod_id, listagg((cast(b.febric_description_id as varchar2(4000))),',') within group (order by b.febric_description_id) as febric_description_id, listagg((cast(b.body_part_id as varchar2(4000))),',') within group (order by b.body_part_id) as body_part_id, listagg((cast(b.gsm as varchar2(4000))),',') within group (order by b.gsm) as gsm, listagg((cast(b.width as varchar2(4000))),',') within group (order by b.width) as width, listagg((cast(b.yarn_lot as varchar2(4000))),',') within group (order by b.yarn_lot) as yarn_lot, listagg((cast(b.yarn_count as varchar2(4000))),',') within group (order by b.yarn_count) as yarn_count, listagg((cast(b.stitch_length as varchar2(4000))),',') within group (order by b.stitch_length) as stitch_length, listagg((cast(b.brand_id as varchar2(4000))),',') within group (order by b.brand_id) as brand_id, b.machine_no_id, b.floor_id as floor_id, listagg((cast(b.color_id as varchar2(4000))),',') within group (order by b.color_id) as color_id, listagg((cast(b.color_range_id as varchar2(4000))),',') within group (order by b.color_range_id) as color_range_id, listagg((cast(c.po_breakdown_id as varchar2(4000))),',') within group (order by c.po_breakdown_id) as po_breakdown_id, e.job_no,e.sales_booking_no,e.booking_id,sum(b.reject_fabric_receive) as reject_qty,c.is_sales,e.buyer_id unit_id,e.within_group,a.knitting_source,a.knitting_company,e.buyer_id,e.customer_buyer,sum(case when b.shift_name=0 then c.quantity else 0 end) as without_shift,sum(case when b.shift_name=0 then b.no_of_roll else 0 end) as without_shift_noofroll, listagg((cast(b.yarn_prod_id as varchar2(4000))),',') within group (order by b.yarn_prod_id) as yarn_prod_id, sum(case when b.shift_name=1 then c.quantity else 0 end) as qntyshifta, sum(case when b.shift_name=1 then b.no_of_roll else 0 end) as noofrollshifta, sum(case when b.shift_name=2 then c.quantity else 0 end) as qntyshiftb,  sum(case when b.shift_name=2 then b.no_of_roll else 0 end) as noofrollshiftb, sum(case when b.shift_name=3 then b.no_of_roll else 0 end) as noofrollshiftc,sum(case when b.shift_name=3 then c.quantity else 0 end) as qntyshiftc";
				
					$sql_inhouse .= " from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c,fabric_sales_order_mst e  where a.id=b.mst_id and b.id=c.dtls_id and c.po_breakdown_id=e.id and e.within_group=2 and a.entry_form=2 and a.item_category=13 and c.entry_form=2 and c.trans_type=1 and a.company_id in($cbo_company_name) $working_company_cond and a.knitting_source like '$source' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $date_con $floor_id $buyer_cond $sales_order_cond $booking_no_cond $within_group_cond $cust_buyer_cond 
					group by b.machine_no_id,a.receive_date,e.job_no,e.sales_booking_no,e.booking_id, e.within_group,a.company_id,a.receive_basis, a.booking_no, b.floor_id, a.knitting_source,a.knitting_company,c.is_sales,e.buyer_id,e.customer_buyer)
						) order by knitting_source,customer_buyer,receive_date asc 
						";
				}
				// echo $sql_inhouse;
				...
				foreach ($nameArray_inhouse as $row) 
				{
					...
					$dtls_id = explode(",", $row[csf('dtls_id')]);
					foreach ($dtls_id as $key => $id) {
						$dtlsIdArr[$id] = $id;
					}

					$fso_ids = explode(",", $row[csf('po_breakdown_id')]);
					foreach ($fso_ids as $key => $fsoid) {
						$fsoIdArr[$fsoid] = $fsoid;
					}

					if ($row[csf("within_group")]==1) 
					{
						$booking_id_arr[$row[csf("booking_id")]]=$row[csf("booking_id")];//fao booking id
					}
				}
				// echo "<pre>"; print_r($dtlsIdArr);
				fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 169, 1,$fsoIdArr, $empty_arr);//fso Id
				fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 169, 2,$yarnProdIdArr, $empty_arr);//ProdId
				fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 169, 3,$booking_id_arr, $empty_arr);//ProdId
				fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 169, 4,$dtlsIdArr, $empty_arr);//production_dtlsId

				$product_details_array = array();
				$sql = "SELECT a.id, a.supplier_id, a.lot, a.current_stock, a.yarn_comp_type1st, a.yarn_comp_percent1st, a.yarn_comp_type2nd, a.yarn_comp_percent2nd, a.yarn_count_id, a.yarn_type, a.color, a.brand 
				from GBL_TEMP_ENGINE g, product_details_master a 
				where g.ref_val=a.id and g.user_id=$user_id and g.entry_form=169 and g.ref_from=2 and a.item_category_id=1 and a.status_active=1 and a.is_deleted=0";
				//echo $sql;
				$result = sql_select($sql);
				foreach ($result as $row)
				{
				...
				$yarn_type_arr = return_library_array("select id, yarn_type from product_details_master where 1=1 " . where_con_using_array($yarnProdIdArr, '0', 'id'), "id", "yarn_type");
				// echo "<pre>"; print_r($yarn_type_arr);

				// Booking Type start===================
				$job_from_fso =  sql_select("SELECT c.booking_no,c.booking_type,c.is_short, b.job_no_prefix_num,b.job_no, a.job_no as fso_no, d.short_booking_type 
				from GBL_TEMP_ENGINE g, fabric_sales_order_mst a, wo_booking_dtls c,wo_po_details_master b, wo_booking_mst d 
				where g.ref_val=a.id and g.user_id=$user_id and g.entry_form=169 and g.ref_from=1 and a.booking_id = c.booking_mst_id and c.job_no=b.job_no  and a.booking_id = d.id and c.booking_mst_id = d.id  and a.within_group=1
				union all
				select b.booking_no,4 as booking_type,0 as is_short,0 as job_no_prefix_num,null as job_no, a.job_no as fso_no, null as short_booking_type 
				from GBL_TEMP_ENGINE g, fabric_sales_order_mst a,wo_non_ord_samp_booking_mst b 
				where g.ref_val=a.id and g.user_id=$user_id and g.entry_form=169 and g.ref_from=1 and a.sales_booking_no=b.booking_no and a.within_group=1");
				$job_fso_chk=array();
				...
				// echo "<pre>"; print_r($cust_buyer_wise_production_arrr);die;
				...
				if(!empty($booking_no_arr))
				{
					...
					$bookingInternalRefSql =sql_select("SELECT a.booking_no,b.grouping 
					from GBL_TEMP_ENGINE g, wo_booking_mst x, wo_booking_dtls a,wo_po_break_down b 
					where g.ref_val=x.id and g.user_id=$user_id and g.entry_form=169 and g.ref_from=3 and x.id=a.booking_mst_id and a.po_break_down_id=b.id  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.grouping is not null and x.company_id in($cbo_company_name) group by a.booking_no,b.grouping");// $all_book_no_cond
				...
				// roll_delivery sql, sys_dtls_id is production_dtls_id
				$roll_delivery_sql="SELECT c.id as delivery_id , b.sys_dtls_id, b.barcode_num, b.current_delivery 
				from GBL_TEMP_ENGINE g, pro_grey_prod_delivery_dtls b, PRO_GREY_PROD_DELIVERY_MST c 
				WHERE g.ref_val=b.sys_dtls_id and g.user_id=$user_id and g.entry_form=169 and g.ref_from=4 and b.MST_ID=c.id and c.ENTRY_FORM=56  and B.ENTRY_FORM=56 
				and b.STATUS_ACTIVE=1 and b.is_deleted=0";
				// echo $roll_delivery_sql;die;
				$roll_delivery_sql_result = sql_select($roll_delivery_sql);
				foreach ($roll_delivery_sql_result as $key => $row) 
				{
					$delivery_roll_count_arr[$row[csf('sys_dtls_id')]]++;
					$delivery_roll_qty[$row[csf('sys_dtls_id')]] += $row[csf('current_delivery')];
					$delivery_roll_id_arr[$row[csf('sys_dtls_id')]] .= $row[csf('delivery_id')].',';
				}
				// echo "<pre>";print_r($delivery_roll_qty);

				$roll_recv_sql="SELECT a.DTLS_ID as production_dtls_id, b.DTLS_ID as recv_dtls_id, b.barcode_no, b.qnty, c.recv_number
				from GBL_TEMP_ENGINE g, PRO_ROLL_DETAILS a, PRO_ROLL_DETAILS b, INV_RECEIVE_MASTER c
				where g.ref_val=a.dtls_id and g.user_id=$user_id and g.entry_form=169 and g.ref_from=4 and a.id=b.roll_id and b.mst_id=c.id and c.entry_form=58 and a.entry_form=2 and b.entry_form=58 and a.STATUS_ACTIVE=1 and a.is_deleted=0 and b.STATUS_ACTIVE=1 and b.is_deleted=0 and c.STATUS_ACTIVE=1 and c.is_deleted=0";
				// echo $roll_recv_sql;die;
				$roll_recv_sql_result = sql_select($roll_recv_sql);
				foreach ($roll_recv_sql_result as $key => $row) 
				{
					$recv_roll_count_arr[$row[csf('production_dtls_id')]]++;
					$recv_roll_qty[$row[csf('production_dtls_id')]] += $row[csf('qnty')];
					$recv_roll_id_arr[$row[csf('production_dtls_id')]] .= $row[csf('production_dtls_id')].',';
				}
				// echo "<pre>";print_r($recv_roll_id_arr);
					...
					foreach ($job_data as $sales_booking_no => $sales_booking_data) 
					{
						foreach ($sales_booking_data as $buyer => $rows) 
						{
							...
							<tr bgcolor="<? echo $bgcolor; ?>">
								...
								<td align="center"><? echo $sales_booking_no; ?></td>
								<td align="center"> <p><? echo chop($internalRefArr[$sales_booking_no]['grouping'],","); ?></p>
								</td>
								<td align="center">
									<? 
									if($buyer_arr[$buyer] !=""){
								...
								<td align="center">
								<a href='#report_details' onClick="openmypage('<? echo $job_no; ?>','<? echo $sales_booking_no; ?>','<? echo $buyer; ?>','<? echo 1; ?>','machine_popup');"><? echo $total_no_machine; ?></a> </td>
								<td align="right" style="width: 45px;"><? echo number_format($rows[1][1][1][1], 2, '.', ''); ?></td>
								<td align="right" style="width: 45px;"><? echo number_format($rows[1][999][2][1], 2, '.', ''); ?></td>
								<td align="right"><? echo number_format($out_bound_qnty_wg_yes, 2, '.', ''); ?></td>
								<td align="right"><? echo number_format($out_bound_qnty_wg_no, 2, '.', ''); ?></td>
								<td align="right"><? echo number_format($in_bound_qnty, 2, '.', ''); ?></td>
								<td align="right"><? echo  number_format($tot_summ, 2, '.', ''); ?></td>
		...
		<fieldset style="width:<? echo $tbl_width + 20; ?>px;">
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $tbl_width+600; ?>" class="rpt_table" align="left"	>
				<thead>
					<tr>
						...
						<th width="60" rowspan="2">GSM</th>
						<th colspan="<? echo count($shift_name)+ count($shift_name); ?>">Production</th>
						<th width="80" rowspan="2">Shift Total</th>
						<th width="60" rowspan="2">No of roll</th>
						<th width="80" rowspan="2">Delivery to Store, (kg)</th>
						<th width="60" rowspan="2">No of Roll Delivery to Store</th>
						<th width="80" rowspan="2">Received by Store (Kg)</th>
						<th width="60" rowspan="2">Receive by Store ( Roll)</th>
						<th rowspan="2"> Remarks</th>
					</tr>
					<tr>
						<?
			...
			<div style="width:<? echo $tbl_width+20+600; ?>px; overflow-y:scroll; max-height:350px;" id="scroll_body" align="left">
				<table align="left" cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $tbl_width+600; ?>" class="rpt_table" id="table_body_main">
						...
						if ($cbo_type == 1 || $cbo_type == 0) 
						{
							if (count($nameArray_inhouse) > 0) // Knitting Production
							{
								$km = 0;
								$tot_reject_qty = 0;
								$machine_arr = return_library_array("select id, machine_no from lib_machine_name", "id", "machine_no");
								...
								foreach ($nameArray_inhouse as $row) 
								{
									...
									if ($row[csf("knitting_source")] == 3) // Out-bound
									{
										...
										$delivery_qty=0;$delivery_roll_count=0;$delivery_roll_id='';
										$recv_qty=0;$recv_roll_count=0;$recv_roll_id='';
										$production_dtls_id=array_unique(explode(",",$row[csf('dtls_id')]));
										foreach($production_dtls_id as $p_id)
										{
											$delivery_qty+=$delivery_roll_qty[$p_id];
											$delivery_roll_count+=$delivery_roll_count_arr[$p_id];
											$delivery_roll_id.=$delivery_roll_id_arr[$p_id].',';

											$recv_qty+=$recv_roll_qty[$p_id];
											$recv_roll_count+=$recv_roll_count_arr[$p_id];
											$recv_roll_id.=$recv_roll_id_arr[$p_id].',';
										}
										$delivery_roll_id =implode(",",array_filter(array_unique(explode(",", $delivery_roll_id))));
										$recv_roll_id =implode(",",array_filter(array_unique(explode(",", $recv_roll_id))));

										?>
										<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" valign="top">
											<td width="30" align="center"><? $km++; echo $km;  ?></td>
											<td width="70"align="center">
												<p><? if ($row[csf('receive_date')] != "" && $row[csf('receive_date')] != "0000-00-00") echo change_date_format($row[csf('receive_date')]); ?>&nbsp;</p>
											</td>
											<td width="100" align="center"><? echo $knitting_party; ?></td>
											...
											<td width="100" align="center" title="<?=$row[csf('sales_booking_no')];?>">
												<p><? echo $booking_type_arr[$row[csf('sales_booking_no')]]; ?></p>
											</td>
											<td width="100" align="center">
												<p><? echo chop($internalRefArr[$row[csf("sales_booking_no")]]['grouping'],","); ?></p>
											</td>
											...
											<td width="80" align="right" title="<?=$row[csf('dtls_id')];?>"><p><? echo number_format($delivery_qty, 2, '.', ''); ?>&nbsp;</p></td>
											<td width="60" align="right" title="<?=$delivery_roll_id;?>"><a href='#report_details' onClick="openmypageDeliveryRecv('<? echo $delivery_roll_id; ?>','<? echo 1; ?>','delivery_recv_qty_popup');"><? echo $delivery_roll_count; ?></a></td>
											<td width="80"  align="right"><p><? echo number_format($recv_qty, 2, '.', ''); ?></p></td>
											<td width="60" align="right" title="<?=$recv_roll_count;?>"><a href='#report_details' onClick="openmypageDeliveryRecv('<? echo $recv_roll_count; ?>','<? echo 2; ?>','delivery_recv_qty_popup');"><? echo $recv_roll_count; ?></a></td>
											<td><p><? echo $row[csf('remarks')]; ?>&nbsp;</p></td>
										</tr>
										<?
										$inhouse_tot_qty += $row_tot_qnty;
										$grand_tot_qnty += $row_tot_qnty;
										$grand_delivery_qty += $delivery_qty;
										$grand_recv_qty += $recv_qty;
										$outBoundWithoutShiftQnty += $row[csf('without_shift')];
										$i++;
									...
									else // In-house
									{
										if ($row[csf('machine_no_id')] != 0) 
										{
											...
											<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" valign="top">
												...
												?>
												<td width="80" align="right"><a href='#report_details' onClick="openmypageshift('<? echo $row[csf('dtls_id')]; ?>','<? echo 1; ?>','shift_qty_popup');"><? echo number_format($row_tot_qnty, 2, '.', ''); ?></a> </td>
												<td width="60" align="right"><? echo number_format($row_tot_no_of_roll, 2, '.', ''); ?></td>											
												...
												<td width="60" align="right" title="<?=$recv_roll_id;?>"><a href='#report_details' onClick="openmypageDeliveryRecv('<? echo $recv_roll_id; ?>','<? echo 2; ?>','delivery_recv_qty_popup');"><? echo $recv_roll_count; ?></a></td>
												<td><p><? echo $row[csf('remarks')]; ?>&nbsp;</p></td>
											</tr>
											<?
											$inhouse_tot_qty += $row_tot_qnty;
							...
							if (count($nameArray_inhouse_subcon) > 0) // Subcon Knitting Production
							{
								//$km = 0;
								$tot_reject_qty = 0;
								$machine_arr = return_library_array("select id, machine_no from lib_machine_name", "id", "machine_no");
								...
								{									
									if ($row[csf('machine_no_id')] != 0) 
									{
										...
										<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" valign="top">
											...
											<td width="100" align="center"><? echo $order_nos; ?></td>
											...
											<td width="60" align="right">
												<? echo number_format($row_tot_no_of_roll, 2, '.', ''); ?></td>											
											<td width="80" align="center"><p><? echo 'delivery_qty'; ?>&nbsp;</p></td>
											<td width="60" align="center"><p><? echo 'delivery_roll'; ?>&nbsp;</p></td>
											<td width="80"  align="right"><p><? echo 'recv_qty'; ?></p></td>
											<td width="60" align="right"><p><? echo 'Receive_Roll'; ?></p></td>
											<td><p><? echo $row[csf('remarks')]; ?>&nbsp;</p></td>
										</tr>
										<?
										$inbound_tot_qty += $row_tot_qnty;
		    ...
		    <table class="rpt_table" width="<? echo $tbl_width+600; ?>" cellpadding="0" cellspacing="0" border="1" rules="all">
	            <tfoot>
	                <tr>
						...
						?>
						<th width="80" class="word_wrap_break" id="value_shift_qnty_total" style="text-align: right"><? echo number_format($grand_tot_qnty + $outBoundWithoutShiftQnty, 2, '.', ''); ?></th>
						<th width="60">&nbsp;</th>
						<th width="80" class="word_wrap_break" id="value_delivery_qnty" style="text-align: right"><? echo number_format($grand_delivery_qty, 2, '.', ''); ?></th>
						<th width="60">&nbsp;</th>

						<th width="80" class="word_wrap_break" id="value_receive_qnty" style="text-align: right"><? echo number_format($grand_recv_qty, 2, '.', ''); ?></th>
						<th width="60">&nbsp;</th>
						<th> &nbsp;</th>
					</tr>
	            </tfoot>
...
if($action=="shift_qty_popup") 
{
	...
	?>
	<fieldset style="width:390px; margin-left:3px">
		<script>
			function print_window()
			{
				var w = window.open("Surprise", "#");
		...
		<div id="scroll_body" align="center">
			...
			<table border="1" class="rpt_table" rules="all" width="370" cellpadding="0" cellspacing="0" align="center">
				<thead>
					<tr>
						<th width="50">Sl</th>
				...
				<tbody>
					<?
					$mstData = sql_select("SELECT booking_no, booking_without_order, receive_basis from inv_receive_master a, pro_grey_prod_entry_dtls b where a.id=b.mst_id and b.id in($dtls_id) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
					...
					{						
						...
						if ($is_salesOrder == 1)
						{
							$query = "SELECT a.id, a.roll_no, a.barcode_no, a.po_breakdown_id, a.qnty, b.job_no as po_number, a.dtls_id 
							from pro_roll_details a, fabric_sales_order_mst b 
							where a.po_breakdown_id=b.id and a.dtls_id in($dtls_id) and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.barcode_no";
						}
						elseif($is_salesOrder == 2)
						{
							$query = "SELECT a.id, a.roll_no, a.barcode_no, a.po_breakdown_id, a.qnty, b.booking_no as po_number, a.dtls_id 
							from pro_roll_details a, wo_non_ord_samp_booking_mst b 
							where a.po_breakdown_id=b.id and a.dtls_id in($dtls_id) and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,3) and b.is_deleted=0 order by a.barcode_no";
						}
						else
						{
							$query = "SELECT a.id, a.roll_no, a.barcode_no, a.po_breakdown_id, a.qnty, b.po_number, a.dtls_id from pro_roll_details a, wo_po_break_down b 
							where a.po_breakdown_id=b.id and a.dtls_id in($dtls_id) and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,3) and b.is_deleted=0 order by a.barcode_no";
						}
					}
					else
					{
						...
						{
							$query = "SELECT id,roll_no,barcode_no,po_breakdown_id,qnty,booking_no as po_number, dtls_id from pro_roll_details where dtls_id in($dtls_id) and entry_form=2 and status_active=1 and is_deleted=0 order by barcode_no";
						}
						else
						{
							$query = "SELECT a.id, a.roll_no, a.barcode_no, a.po_breakdown_id, a.qnty, b.po_number, a.dtls_id from pro_roll_details a, wo_po_break_down b where a.po_breakdown_id=b.id and a.dtls_id in($dtls_id) and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,3) and b.is_deleted=0 order by a.barcode_no";
						}
					}
					// echo $query;
					...
					// echo "<pre>"; print_r($production_dtls_arr);
					...
					$ib_query = "SELECT a.id, a.roll_no, a.barcode_no, a.po_breakdown_id, a.qnty, a.dtls_id from subcon_pro_roll_details a, subcon_ord_dtls b where a.po_breakdown_id=b.id and a.dtls_id in($dtls_id) and a.entry_form=159 and a.status_active=1 and a.is_deleted=0 order by a.id";
					//echo $ib_query;
					...
					foreach ($ib_query_result as $row) 
					{
						...
						$production_qty_arr[$row[csf('barcode_no')]]['barcode_no'] = $row[csf('barcode_no')];
						$production_qty_arr[$row[csf('barcode_no')]]['qnty'] += $row[csf('qnty')];
						$inbound_production_dtls_arr[$row[csf('dtls_id')]] = $row[csf('dtls_id')];
					}

					$inbound_production_dtls_arr =array_filter($inbound_production_dtls_arr);

					if(count($inbound_production_dtls_arr)>0)
					{
						$production_dtls = implode(",", $inbound_production_dtls_arr);
						$all_production_dtls_cond=""; $production_dtls_Cond="";
						if($db_type==2 && count($inbound_production_dtls_arr)>999)
						{
							$inbound_production_dtls_arr_chunk=array_chunk($inbound_production_dtls_arr,999) ;
							foreach($yarn_issue_id_arr_chunk as $chunk_arr)
							{
								$chunk_arr_value=implode(",",$chunk_arr);
								$production_dtls_Cond.="  b.id in($chunk_arr_value) or ";
...
if($action=="delivery_recv_qty_popup") 
{
	echo load_html_head_contents("Production Info", "../../../", 1, 1,'','','');
	extract($_REQUEST);
		...
		<div id="scroll_body" align="center">
			<table border="1" class="rpt_table" rules="all" width="370" cellpadding="0" cellspacing="0" align="center">
				<thead>
					<tr>
						<th width="50">Sl</th>
						<th width="120"><? echo ($type==1) ? 'Delivery No' : "Reveive No" ; ?></th>
						<th width="100"> Roll No</th>
						<th >Qty </th>
					</tr>
				</thead>
				<tbody>
					<?
					if ($type==1) // roll delivery
					{
						$roll_delivery_sql="SELECT c.id as delivery_id, c.sys_number , b.sys_dtls_id, b.barcode_num as barcode_no, b.current_delivery as qnty
						from pro_grey_prod_delivery_dtls b, PRO_GREY_PROD_DELIVERY_MST c 
						WHERE b.MST_ID=c.id and c.ENTRY_FORM=56  and B.ENTRY_FORM=56 and c.id in($dtls_id)
						and b.STATUS_ACTIVE=1 and b.is_deleted=0";
						// echo $roll_delivery_sql;
						$sql_result = sql_select($roll_delivery_sql);
					}
					else // $type==2 is roll receive
					{
						$roll_recv_sql="SELECT a.DTLS_ID as production_dtls_id, b.DTLS_ID as recv_dtls_id, b.barcode_no, b.qnty, c.recv_number as sys_number
						from PRO_ROLL_DETAILS a, PRO_ROLL_DETAILS b, INV_RECEIVE_MASTER c
						where a.id=b.roll_id and b.mst_id=c.id and a.dtls_id in($dtls_id) and c.entry_form=58 and a.entry_form=2 and b.entry_form=58 and a.STATUS_ACTIVE=1 and a.is_deleted=0 and b.STATUS_ACTIVE=1 and b.is_deleted=0 and c.STATUS_ACTIVE=1 and c.is_deleted=0";
						// echo $roll_recv_sql;die;
						$sql_result = sql_select($roll_recv_sql);
					}
					...
					foreach($sql_result as $row)
					{
						...
						<tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
							<td align="center"><p><? echo $i; ?></p></td>
							<td align="center"><p><? echo $row[csf('sys_number')]; ?></p></td>
							<td align="center"><? echo $row[csf('barcode_no')]; ?></td>
							<td align="right"><p><? echo $row[csf('qnty')]; ?></p></td>
						</tr>
						<?

						$tot_delivery+=$row[csf('qnty')];
						$i++;
				...
				<tfoot>
					<tr class="tbl_bottom">
						...
						<td align="right" title=""><? echo $tot_delivery; ?>&nbsp;</td>
