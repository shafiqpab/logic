...
$rate_data=sql_select("select  a.id,a.emb_name,a.job_no,a.cons_dzn_gmts, c.rate, a.amount,b.costing_per,c.po_break_down_id  from wo_pre_cost_embe_cost_dtls a,wo_pre_cost_mst b,wo_pre_cos_emb_co_avg_con_dtls c where  a.id=c.pre_cost_emb_cost_dtls_id and emb_name in (1,2) and  a.job_no=b.job_no $job_cond2 and a.status_active=1 and a.is_deleted=0 order by a.id");
$rate_cal=1;
...
foreach($rate_data as $row){
		...
		$print_rate_arr[$row[csf("job_no")]][$row[csf("po_break_down_id")]]=$row[csf("rate")]/$rate_cal;
		//  $print_rate_arr[$row[csf("job_no")]]=$row[csf("job_no")];

		}elseif($row[csf("emb_name")]==2){
			$embl_rate_arr[$row[csf("job_no")]][$row[csf("po_break_down_id")]]=$row[csf("rate")]/$rate_cal;
...
$emb_qty_data=sql_select("SELECT id, po_break_down_id,embel_name, item_number_id, production_date, production_quantity from pro_garments_production_mst where embel_name='2' and production_type='3' and status_active=1 and is_deleted=0 $date_cond order by id");
...
//================================issue qty ===========================
		...
		from inv_issue_master a, inv_transaction b, product_details_master c where   b.prod_id=c.id and  c.item_category_id in(22,57) and  a.entry_form=21 and a.id=b.mst_id");
		...
		$issue_print_data=sql_select("select a.id,a.issue_number, b.id as bid, b.cons_quantity, b.prod_id,c.item_category_id, c.item_group_id, b.order_id,c.avg_rate_per_unit
			from inv_issue_master a, inv_transaction b, product_details_master c where   b.prod_id=c.id and  c.item_category_id=22 and  a.entry_form=21 and a.id=b.mst_id");
		...
		$issue_qty=sql_select("select a.id,a.issue_number, b.id as bid, b.cons_uom, b.cons_quantity,b.prod_id, c.item_category_id, c.item_group_id, b.order_id,c.AVG_RATE_PER_UNIT
			from inv_issue_master a, inv_transaction b, product_details_master c ,inv_receive_master  d  where d.id=b.mst_id and  b.prod_id=c.id and  c.item_category_id=57 and  a.entry_form=21 and a.id=b.mst_id");
		...
		//   print_r($item_desc_arr);
		//  echo "<br>";
		
	    								
 //   ================================ main query GMTS_ITEM_ID================================================= 

 $data_array=sql_select("select a.is_confirmed, a.po_number, a.po_received_date, a.pub_shipment_date, a.shipment_date, a.po_quantity, a.unit_price, a.po_total_price,a.id,a.job_no_mst,c.item_number_id,b.buyer_name,c.po_break_down_id,b.company_name,c.embel_name from wo_po_break_down a,wo_po_details_master b,pro_garments_production_mst c where a.JOB_NO_MST=b.job_no and c.po_break_down_id=a.id and a.status_active=1 and a.is_deleted=0 $sql_cond $date_cond2 order by a.id ASC");
 $po_number=0; $i=0; $m=0; $buyer_id=0; $z=0; $is=0;
 $buyer_wise_qty_id=array();
foreach($data_array as $row ){

			$order_wise_data_arr[$row[csf("id")]]['id']=$row[csf("id")];
			$order_wise_data_arr[$row[csf("id")]]['job_no']=$row[csf("job_no_mst")];
			$order_wise_data_arr[$row[csf("id")]]['po_number']=$row[csf("po_number")];
			// $order_wise_data_arr[$row[csf("id")]]['item_number_id']=$row[csf("item_number_id")];
			$order_wise_data_arr[$row[csf("id")]]['item_number_id'] =$item_desc_arr[$row[csf("id")]];
			// if($i==0){// 	$order_wise_data_arr[$row[csf("id")]]['item_number_id'] .=$item_desc_arr[$row[csf("id")]];
			// 	$i++;
			// }else{// 	$order_wise_data_arr[$row[csf("id")]]['item_number_id'].=",".$item_desc_arr[$row[csf("id")]];
			// }
						...
						{ $z++;
							 $buyer_wise_qty_id[]=$batch_no;							
								if($is==0){				
										...
										$buyer_wise_qty_arr[$row[csf("buyer_name")]]['order_id'] .=$row[csf("id")];
										$is++;									
								}else{									
									$buyer_wise_qty_arr[$row[csf("buyer_name")]]['issue_p_num'] .=",".$iss_print_issue_num_arr[$row[csf("id")]];	
									$buyer_wise_qty_arr[$row[csf("buyer_name")]]['issue_embl_num'] .=",".$iss_eble_issue_num_arr[$row[csf("id")]];
							  ...
							  $buyer_wise_qty_arr[$row[csf("buyer_name")]]['print_value'] +=($print_qty[$row[csf("id")]]*$print_rate_arr[$row[csf("job_no_mst")]][$row[csf("id")]]);
							  $buyer_wise_qty_arr[$row[csf("buyer_name")]]['embr_value'] +=($emb_qty[$row[csf("id")]]*$embl_rate_arr[$row[csf("job_no_mst")]][$row[csf("id")]]);
    ...
    print_r($order_wises_data_arr);
		    		...
		    		<tbody id="tbl_list_search" align="left">
		        		...
		        		foreach($buyer_wise_qty_arr as $buyer_id=>$row){
							...
							<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>">
								...
								<td width="80" align="left"><a href="##"onclick="openmypage_order('<?=$buyer_id?>','<?=$row['order_id'];?>','<?=$row['b_job_no'];?>','Issue_PrintPopup')"><? echo number_format($row['print_value'],0);?></a></td>
								<td width="60" align="center"><? echo $row['embl_qty'];?></td>
								<td width="100" align="left"><a href="##"onclick="openmypage_order('<?=$buyer_id?>','<?=$row['order_id'];?>','<?=$row['b_job_no'];?>','Issue_emblPopup')"><? echo $row['embr_value'];?></a></td>
								...
								<td width="80" align="left"><a href="##"onclick="openmypage_order('<?=$row['issue_p_num']?>','<?=$orderid_arr;?>','<?=$row['b_job_no'];?>','Issue_cost_PrintPopup')"><? echo $row['issue_p_val']?></a></td>
								<td width="80" align="left"><a href="##"onclick="openmypage_order('<?=$row['issue_embl_num']?>','<?=$orderid_arr;?>','<?=$row['b_job_no'];?>','Issue_cost_EmblPopup')"><?echo $row['issue_e_val']?></a></td>
								<td width="80" align="left" title="<?=$row['issue_p_val']."+".$row['issue_e_val'];?>"><? echo $row['issue_p_val']+$row['issue_e_val'];?></td>
								<td width="80" align="left"><? echo $row['print_value']-$row['issue_p_val'];?></td>
								<td width="80" align="left"><? echo $row['embr_value']-$row['issue_e_val'];?></td>
								<td width="80" align="left"><? echo $row['print_value']+$row['embr_value']-$row['issue_p_val']-$row['issue_e_val'];?></td>
		...
		<fieldset style="width:1090px;"> 	
	        ...
	        <!-- ========================== table body ========================== -->         
	        <div id="scroll_body" style="width:1100px;max-height:300px;overfllow-y:auto;">
	        	<table width="1090" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all" id="table_body1" align="left"> 
		    		<tbody id="tbl_list_search1" align="center">
		        		<?
		        		$t=1;
		        	
		        		foreach ($order_wise_data_arr as $id => $row) 
		        		{
							...
							$print_rate=$print_rate_arr[$row['job_no']][$row['id']];
							$embl_rate=$embl_rate_arr[$row['job_no']][$row['id']];
							...
							<td width="60" align="center"><?echo $style_ref;?></td>						
...
if ($action=='Issue_cost_PrintPopup')
{
	...
	$order_array=sql_select("select  a.po_number,a.id,a.job_no_mst,c.item_number_id from wo_po_break_down a,wo_po_details_master b,pro_garments_production_mst c where a.JOB_NO_MST=b.job_no and c.po_break_down_id=a.id and a.status_active=1 and a.is_deleted=0  and a.id in ($order_id) order by a.id ASC");
...
//   print_r($currency_data);
	...
	$issue_print_data=sql_select("select a.id,a.issue_number,a.issue_date, b.id, b.store_id, b.cons_uom, b.cons_quantity, b.machine_category, b.machine_id, b.prod_id, b.location_id, b.department_id, b.section_id, c.item_category_id, c.item_group_id, b.order_id,c.avg_rate_per_unit,c.company_id
			from inv_issue_master a, inv_transaction b, product_details_master c where   b.prod_id=c.id and b.id in($issueId)  and  c.item_category_id=22 and  a.entry_form=21 and a.id=b.mst_id and b.order_id in ($order_id)");
...
<div style="width:700px" align="center" id="details_reports"> 
    ...
    <table id="tbl_id" class="rpt_table" width="" border="1" rules="all" >
        ...
        <tr>
            ...
            <td><?echo implode(",",array_unique(explode(",", chop($order_arr[$row[csf("order_id")]],",")))); ?></td>
...
if ($action=='Issue_cost_EmblPopup')
{
	...
	$issue_print_data=sql_select("select a.id,a.issue_number,a.issue_date, b.id, b.store_id, b.cons_uom, b.cons_quantity, b.machine_category, b.machine_id, b.prod_id, b.location_id, b.department_id, b.section_id, c.item_category_id, c.item_group_id, b.order_id,c.avg_rate_per_unit,c.company_id
			from inv_issue_master a, inv_transaction b, product_details_master c where   b.prod_id=c.id and  c.item_category_id=57 and  a.entry_form=21 and a.id=b.mst_id and b.id in($issueId) and b.order_id in ($order_id)");
...
<div style="width:700px" align="center" id="details_reports"> 
    ...
    <table id="tbl_id" class="rpt_table" width="" border="1" rules="all" >
        ...
        <tr>
            ...
            <td><? echo $row[csf("issue_date")];  ?></td>
            <td><?  echo $lib_po_number[$row[csf("order_id")]];  ?></td>
            <td><?	echo implode(",",array_unique(explode(",", chop($order_arr[$row[csf("order_id")]],","))));
 ?></td>
            <td><?  echo $row[csf("cons_quantity")];  ?></td>
            <td><?  echo $rate;  ?></td>
            <td><? echo $value; ?></td>
...
if ($action=='Issue_PrintPopup')
{
	...
	$print_qty_data=sql_select("SELECT id, po_break_down_id, item_number_id, country_id, production_date, production_quantity,  embel_name from pro_garments_production_mst where   embel_name='1' and production_type='3' and status_active=1 and is_deleted=0 and po_break_down_id in ($orderid) order by id");
	...
	$rate_data=sql_select("select  a.id,a.emb_name,a.job_no,a.cons_dzn_gmts, c.rate, a.amount,b.costing_per,c.po_break_down_id  from wo_pre_cost_embe_cost_dtls a,wo_pre_cost_mst b,wo_pre_cos_emb_co_avg_con_dtls c where  a.id=c.pre_cost_emb_cost_dtls_id and emb_name=1 and  a.job_no=b.job_no and a.status_active=1 and a.is_deleted=0 and c.po_break_down_id in ($orderid) order by a.id");
	$rate_cal=1;
	$print_rate_arr=array();
	foreach($rate_data as $row){
					if($row[csf("costing_per")]=1){//For 1 Dzn
			...
			$print_rate_arr[$row[csf("po_break_down_id")]]=$row[csf("rate")]/$rate_cal;
    ...
    <table id="tbl_id" class="rpt_table" width="" border="1" rules="all" >
    	<thead>
        	<tr>
			 ...
			 <?
			 $i=1;
			 	foreach(array_unique($order_data_arr) as $id){
					 $amount=$print_qty[$id]*$print_rate_arr[$id];
					
					 ?>
        <tr>
			<td><? echo  $i; ?></td>
			<td><? echo $lib_buyer[$buyer_id]; ?></td>
        	<td><? echo $lib_po_number[$id]; ?></td>
            <td><? echo $print_qty[$id];  ?></td>
            <td><?  echo $print_rate_arr[$id];  ?></td>          
            <td><?  echo  $amount;  ?></td>            
        </tr>
       <?
	   $tot_qty +=$print_qty[$id];
	   $tot_amount +=$amount;
	   $i++;
		 }?>
		 <tr>
...
if ($action=='Issue_emblPopup')
{
	...
	foreach($order_data as $id){if($i==0){$i++;}elseif($i==1){$orderid.=$id;$i++;}else{$orderid .=",".$id;}}
	$order_data_arr=array();
	$print_qty_data=sql_select("SELECT id, po_break_down_id, item_number_id, country_id, production_date, production_quantity,  embel_name from pro_garments_production_mst where   embel_name='2' and production_type='3' and status_active=1 and is_deleted=0 and po_break_down_id in ($orderid) order by id");
	...
	$rate_data=sql_select("select  a.id,a.emb_name,a.job_no,a.cons_dzn_gmts, c.rate, a.amount,b.costing_per,c.po_break_down_id  from wo_pre_cost_embe_cost_dtls a,wo_pre_cost_mst b,wo_pre_cos_emb_co_avg_con_dtls c where  a.id=c.pre_cost_emb_cost_dtls_id and emb_name=2 and  a.job_no=b.job_no and a.status_active=1 and a.is_deleted=0 and c.po_break_down_id in ($orderid) order by a.id");
	$rate_cal=1;
	$print_rate_arr=array();
	foreach($rate_data as $row){
					if($row[csf("costing_per")]=1){//For 1 Dzn
...
<div style="width:600px" align="center" id="details_reports"> 
  	<legend>Issue Embleshment-$ pop up</legend>
    <table id="tbl_id" class="rpt_table" width="" border="1" rules="all" >
    	<thead>
        	<tr>
				<th width="20" align="center">SL </th>
