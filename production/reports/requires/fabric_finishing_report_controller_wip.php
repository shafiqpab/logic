...
$process_format=array(0=>"ALL", 30 => 'Slitting/Squeezing WIP', 13 => 'Drying WIP' , 12 => "Stentering WIP", 14 => 'Compacting WIP');
				...
				<table width="896" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
					...
					<tbody>
						<tr>
							<td>
								<?
								echo create_drop_down( "cbo_company_name", 120, "select id,company_name from lib_company where status_active=1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select Company --", 0, "load_drop_down('requires/fabric_finishing_report_controller_wip', this.value, 'load_drop_down_buyer_fso', 'buyer_td_fso' );" );
								?>
							...
							<td align="center">
								<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view (document.getElementById('cbo_company_name').value+'**'+document.getElementById('txt_fso_no').value+'**'+document.getElementById('txt_booking_no').value+'**'+document.getElementById('txt_style_no').value+'**'+document.getElementById('cbo_within_group').value+'**'+document.getElementById('cbo_year').value+'**'+document.getElementById('cbo_buyer_name').value, 'bookingnumbershow_search_list_view', 'search_div', 'fabric_finishing_report_controller_wip', 'setFilterGrid(\'list_view\',-1)');" style="width:100px;" />
							</td>
						</tr>
					</tbody>
				</table>
				...
				<table width="746" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
					...
					<tbody>
						<tr>
							...
							<td align="center">
								<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view ('<? echo $company_name; ?>'+'**'+document.getElementById('txt_batch_no').value+'**'+document.getElementById('txt_date_from').value+'**'+document.getElementById('txt_date_to').value, 'batchnumbershow_search_list_view', 'search_div', 'fabric_finishing_report_controller_wip', 'setFilterGrid(\'list_view\',-1)');" style="width:100px;" />
							</td>
						</tr>
						<tr>
							<tr>
...
{
	echo create_drop_down( "cbo_location_id", 110, "select id,location_name from lib_location where status_active=1 and is_deleted=0 and company_id in($data)    order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down( 'requires/fabric_finishing_report_controller_wip', document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_floor', 'floor_td' );",0 );
	exit();
}
if ($action=="load_drop_down_floor")
{
	$ex_data = explode("_", $data);
	echo create_drop_down( "cbo_floor_id", 110, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and production_process in(3,4) and company_id in($ex_data[0]) and location_id in($ex_data[1]) order by floor_name","id,floor_name",1, "-- Select Floor --", $selected, "load_drop_down( 'requires/fabric_finishing_report_controller_wip',this.value, 'load_drop_down_machine', 'machine_td' );",0 );
	exit();
}
...
if($action=="report_generate")
{
	...
	$booking_cond3="";$booking_cond2="";$booking_cond="";
	if($booking_no) $booking_cond=" and b.booking_no like '%$booking_no'";
	if($booking_no) $booking_cond2=" and d.booking_no like '%$booking_no'";
	if($booking_no) $booking_cond3=" and c.booking_no like '%$booking_no'";
...
//	echo  $booking_cond.'d';
	...
	{ 
		...
		$get_batch_ids_by_batch_date=sql_select("select entry_form, b.id,b.process_id from pro_batch_create_mst b where b.status_active=1 and entry_form in(0) $batch_cond $batch_date_cond $booking_cond   ");
		//echo "select entry_form, b.id,b.process_id from pro_batch_create_mst b where b.status_active=1 and entry_form in(0) $batch_cond $batch_date_cond  $floor_cond3 $booking_cond";
	...
	//	echo "select max(a.id) as ids,a.batch_id,a.entry_form,a.result from pro_fab_subprocess a  ,pro_fab_subprocess_dtls b where a.id=b.mst_id  and a.status_active=1  and b.status_active=1 $prod_date_cond  $batch_cond3 group by a.batch_id,a.entry_form,a.result order by ids desc";
		...
		where   c.mst_id=b.id and c.status_active=1    $batch_cond $comp_cond2 $batch_ext_cond $production_batch_id_cond $booking_cond group by b.id order by id desc";
		 
		//group by b.id,b.extention_no,d.within_group,d.id,d.buyer_id,d.po_buyer
	...
	if($booking_no)
	{
		...
		from pro_batch_create_dtls c,pro_batch_create_mst b
		left join pro_fab_subprocess a on b.id=a.batch_id and a.status_active=1 $prod_comp_cond
		where  c.mst_id=b.id and c.status_active=1   $booking_cond $batch_cond $comp_cond2 $batch_ext_cond $production_batch_id_cond group by b.id order by id desc";
		$get_batch_ids_from_subprocess = sql_select($get_batch_ids_from_subprocess_sql);
			$i=0;
			foreach($get_batch_ids_from_subprocess as $row)
			{
	...
	else
		{
		   ...
		   $get_batch_ids_from_subprocess_sql = "select a.id,b.id batch_id,b.extention_no,b.process_id,c.prod_id,c.batch_qnty
			from pro_batch_create_dtls c,pro_batch_create_mst b
			left join pro_fab_subprocess a on b.id=a.batch_id and a.status_active=1
			where  c.mst_id=b.id and c.status_active=1   $batch_cond $booking_cond $comp_cond2 $batch_ext_cond
...
*/
	...
	where d.mst_id=c.id  and c.status_active=1 and d.status_active=1  $batch_id_cond $booking_cond3  $batch_cond2 $batch_date_cond2 $batch_year_cond $comp_cond $batch_ext_cond2 $buyer_cond  $floor_cond
...
//echo $batch_cond_for_in.'FDF';
	...
	{
	    $dyeing_sql="SELECT a.id as mst_id,a.service_company,a.machine_id,a.shift_name,a.batch_ext_no,a.floor_id,a.service_source,a.insert_date,a.remarks,a.batch_id, a.result,a.batch_no,a.process_id,a.entry_form,a.start_hours,a.start_minutes,a.production_date as end_date,a.load_unload_id, a.batch_ext_no,a.process_start_date,a.process_end_date,a.end_minutes,a.end_hours,b.production_qty,b.prod_id,b.no_of_roll,b.width_dia_type from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.status_active=1 and b.status_active=1 $batch_cond_for_in  order by a.id asc";
		...
		foreach($dyeing_data as $row )
		{
			...
			$subpro_prod_arr[$process_result][$row[csf("batch_id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['shift_name']=$row[csf("shift_name")];
			$subpro_prod_arr[$process_result][$row[csf("batch_id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['mc']=$row[csf("machine_id")];
			...
			$subpro_prod_arr[$process_result][$row[csf("batch_id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['batch_ext_no']=$row[csf("batch_ext_no")];
	...
	foreach($result_batch as $row)
	{
		...
		else if(($cbo_type==0 || $cbo_type==33) && $results==0 && $entry_form_id=="")
		{
	 		//If found Peach..
		...
		if($process_id!=""){
			...
			$shift_name_id = $subpro_prod_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['shift_name'];
			$mc_id = $subpro_prod_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]][$row[csf("width_dia_type")]]['mc'];
		...
		//	echo $results.'='.$process_id.', ';
			...
			$batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]["shift"]=$shift_name_id;
			$batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]["mc"]=$mc_id;
			$batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]["remarks"]=$prod_remarks;
			...
			//echo $row[csf("id")].', ';
			//echo $cbo_type.'=='.$results.'=='.$has_extension.'=='.$process_id.'=='.$entry_form_id.'<br>';
			...
			else if(($cbo_type==0 || $cbo_type==33) && $results==0 && $entry_form_id=="")//&& $entry_form_id==35
			{
				//echo 'B';
				if(in_array(33, $process_ids) && $has_extension==""){
					unset($batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]);
			...
			else if(($cbo_type==0 || $cbo_type==33) && $sub_result_id2==11 && $sub_entry_form2=32)//HeatSet Complete Found
			{
			...
			else if($results==11 && $entry_form_id!=35 ){
				unset($batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]);
			}
			else if($sub_result_id2==11 && $sub_entry_form2!=35 ){
				//echo 'G';
				unset($batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]);
			}
			...
			else if($load_unload==2 && $entry_form_id==35 && !in_array($results, $dyeing_result_arr))
			{
					//echo 'A';
				unset($batch_wip_arr[$process_id][$row[csf("id")]][$row[csf("prod_id")]]);
			}
			...
			if($results==11 && $entry_form_id==34 ){
...
//$po_id_arr
$po_cond_ids=implode(",",$po_id_arr);
	$poIds=chop($po_cond_ids,','); $po_cond_for_in="";
	$po_ids=count(array_unique(explode(",",$po_cond_ids)));
	if($db_type==2 && $po_ids>1000)
	{
		$po_cond_for_in=" and (";
		$poIdsArr=array_chunk(explode(",",$poIds),999);
	...
	{
		$po_cond_for_in=" and c.id in($poIds)";
	}
	
	 $booking_data="SELECT a.buyer_id,c.po_number,c.id as po_id, a.booking_no,b.job_no,c.file_no,c.grouping as ref_no	from  wo_po_break_down c, wo_booking_mst a , wo_booking_dtls b where  b.po_break_down_id=c.id  and a.booking_no=b.booking_no and a.status_active=1 and b.status_active=1 and c.status_active=1 $po_cond_for_in group by  a.buyer_id,c.po_number,c.id, a.booking_no,b.job_no,c.file_no,c.grouping   ";
	foreach(sql_select($booking_data) as $v)
	{
		
		 
		//$booking_data_arr2[$v[csf("fso_no")]]["buyer_id"]=$buyer_list[$v[csf("buyer_id")]];
		$booking_data_arr[$v[csf("booking_no")]]["buyer_id"]=$buyer_list[$v[csf("buyer_id")]];
		$booking_data_arr[$v[csf("booking_no")]]["job_no"]=$v[csf("job_no")];
		$booking_data_arr[$v[csf("booking_no")]]["file_no"].=$v[csf("file_no")].',';
		$booking_data_arr[$v[csf("booking_no")]]["ref_no"].=$v[csf("ref_no")].',';
		$booking_data_arr[$v[csf("booking_no")]]["po_number"].=$v[csf("po_number")].',';
	...
	{
		if($rpt_type==0) $width=2100;else $width=500;
		?>
		<div>
			<style type="text/css">
				.alignment_css
			...
			<fieldset style="width:<? echo $width;?>px;">
				...
				if($rpt_type!=0)
				{
					...
					?>
					<table class="rpt_table" width="2100" cellpadding="0" cellspacing="0" border="1" rules="all" align="left">
						...
						<thead>
							<tr>
								...
								<th width="100" class="alignment_css">Booking No</th>
								<th width="80" class="alignment_css">File no</th>
								<th width="80" class="alignment_css">Ref. no</th>
								...
								<th width="80" class="alignment_css">Fabric Desc</th>
                                ...
                                <th width="80" class="alignment_css">Last Process Date</th>
                              ...
                              <!--  <th width="80" class="alignment_css">Yarn Lot</th>-->
								<th width="" class="alignment_css">Reprocess</th>
							</tr>
						</thead>
					</table>
					<div style=" max-height:380px; width:2120px; overflow-y:scroll;" id="scroll_body">
						<table align="left" class="rpt_table" id="table_body" width="2100" cellpadding="0" cellspacing="0" border="1" rules="all">
							<tbody>
								<?
									...
									if(!empty($batch_data)){
										...
										foreach($batch_data as $batch_id=>$prod_data)
										{
											foreach($prod_data as $prod_id=>$row)
											{
												if ($ii%2==0)
													...
													$bgcolor="#FFFFFF";
												$po_numbers=rtrim($booking_data_arr[$row[("booking_no")]]["po_number"],',');
												$po_numbers=implode(",",array_unique(explode(",",$po_numbers)));
												
												$file_no=rtrim($booking_data_arr[$row[("booking_no")]]["file_no"],',');
												$file_nos=implode(",",array_unique(explode(",",$file_no)));
												$ref_no=rtrim($booking_data_arr[$row[("booking_no")]]["ref_no"],',');
												$ref_nos=implode(",",array_unique(explode(",",$ref_no)));
												//echo $batch_id.'='.$row[("batch_ext_no")].'m';;
												...
												<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $ii.$page_from_id; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $ii.$page_from_id; ?>">
													...
													<td width="80" class="alignment_css"><? echo $machine_no_arr[$row["mc"]];?></td>
                                                    <td width="80" class="alignment_css"><? echo $floor_arr[$row["floor_id"]];?></td>
                                                    <td width="80" class="alignment_css"><? echo $shift_name[$row["shift"]];?></td>
													<td width="80" class="alignment_css"><? echo $booking_data_arr[$row[("booking_no")]]["buyer_id"];?></td>
                                                    
													<td width="80" class="alignment_css"><? echo $booking_data_arr[$row[("booking_no")]]["job_no"];?></td>
													<? ?>
                                                    <td width="100" class="alignment_css"><? echo $row["booking_no"];?></td>
													<td width="80" class="alignment_css"><?  echo $file_nos;?></td>
                                                    <td width="80" class="alignment_css"><?  echo $ref_nos;?></td>
                                                    <td width="80" class="alignment_css"><?  echo $po_numbers;?></td>
													...
													<td width="80" class="alignment_css">
														...
														echo $com_data_desc[0].','.$com_data_desc[1];
														$width_dia_type=$fabric_typee[$row["width_dia_type"]];
														?>
													</td>
                                                    <td width="80" class="alignment_css" title="<? echo $batch_id;?>"><? echo $com_data_desc[2];?></td>
													...
													<td width="80" class="alignment_css">
														...
														if($page_from_id==33) echo ""; else echo  abs($days_remian);?>
													</td>
													<td width="80" class="alignment_css">
														<?
														if($page_from_id==33) echo "";  else echo  abs ( floor( abs($diff_time_days/60)) )." H :".abs($diff_time_days%60)." M ";
														?>
													</td>
                                                   ...
                                                   <!-- <td width="80" class="alignment_css"><? //echo $dyeing_time ;?></td>-->
                                                    <td width="" class="alignment_css"><? if($row["batch_ext_no"]>0)  echo "Re Dyeing";else echo " "; ?></td>
										...
										if($tot_batch_qty>0){
											...
											<tr style="background-color:whitesmoke;font-size:12px; cursor:pointer; " onClick="change_color('tr3_<? echo $ii; ?>','<? echo $bgcolor; ?>')" id="tr3_<? echo $ii; ?>" >
												<td width="" colspan="16" class="alignment_css" align="right"><strong><? echo  $process_format[$page_from_id];?> Total=</strong></td>
