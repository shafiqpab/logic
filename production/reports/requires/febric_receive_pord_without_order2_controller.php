...
if($action=="report_generate")
{
	...
	$con = connect();
	$r_id1=execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_name." and ENTRY_FORM = 173");
	if($r_id1)
	{
		oci_commit($con);
        disconnect($con);
	...
	$BookingIdIdChk = array();
	$allBookingIdArr = array();
	$styleIdChk = array();
	$allSampReqBookingIdArr = array();
	foreach ($sql_res as $val)
	{
		...
		{
			if($BookingIdIdChk[$val[csf('style_id')]] == "")
			{
				$styleIdChk[$val[csf('style_id')]] = $val[csf('style_id')];
				$allSampReqBookingIdArr[$val[csf("style_id")]]=$val[csf("style_id")];
			}
		...
		if($BookingIdIdChk[$val[csf('booking_id')]] == "")
		{
			$BookingIdIdChk[$val[csf('booking_id')]] = $val[csf('booking_id')];
			$allBookingIdArr[$val[csf('booking_id')]] = $val[csf('booking_id')];
		}
	}
	...
	$allBookingIdArr = array_filter($allBookingIdArr);
	if(!empty($allBookingIdArr))
	{
		fnc_tempengine("GBL_TEMP_ENGINE", $user_name, 173, 1,$allBookingIdArr, $empty_arr); //recv id
		//die;
		$sql_dtls_ids="SELECT b.id as dtls_id, a.roll_maintained,a.booking_id from inv_receive_master a, pro_grey_prod_entry_dtls b, GBL_TEMP_ENGINE x where a.id=b.mst_id and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.booking_id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1
		union all
		SELECT b.id as dtls_id, a.roll_maintained,e.id as booking_id
		from inv_receive_master a, pro_grey_prod_entry_dtls b, ppl_planning_entry_plan_dtls c, wo_non_ord_samp_booking_mst e, GBL_TEMP_ENGINE x
		where a.id=b.mst_id  and a.booking_id=c.dtls_id and c.booking_no=e.booking_no and a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and e.id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1";
		...
		die;*/
		//echo $sql_dtls_ids;die;
		$sql_dtls_ids_arr=sql_select($sql_dtls_ids);
		...
		foreach ($sql_dtls_ids_arr as $val)
		{
			...
			$alldtlsIdArr[$val[csf('dtls_id')]] = $val[csf('dtls_id')];
		}
	}

	$allSampReqBookingIdArr = array_filter($allSampReqBookingIdArr);
	if(!empty($allSampReqBookingIdArr))
	{
		fnc_tempengine("GBL_TEMP_ENGINE", $user_name, 173, 2,$allSampReqBookingIdArr, $empty_arr); //recv id
		//die;

		$samp_req_color_arr="SELECT b.booking_no,c.sample_mst_id,d.color_id,d.fabric_color from wo_non_ord_samp_booking_dtls b,sample_development_fabric_acc c,sample_development_rf_color d, GBL_TEMP_ENGINE x where c.id=d.dtls_id and c.sample_mst_id=b.style_id  and b.entry_form_id=140 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and d.status_active=1 and c.sample_mst_id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=2 group by b.booking_no,c.sample_mst_id,d.color_id,d.fabric_color";//and d.fabric_color>0
		//echo $samp_req_color_arr;die;
		...
		$sample_req_sql="SELECT e.sample_mst_id, e.sample_prod_qty as sample_req_qty
		from sample_development_dtls e, GBL_TEMP_ENGINE x
		where  e.sample_mst_id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=2 and e.status_active=1
       	and e.is_deleted=0";
		//echo $sample_req_sql;die;
		$sample_req_sql_result=sql_select($sample_req_sql);
		$sample_req_qty_arr=array();
		foreach ($sample_req_sql_result as $val)
		{
			$sample_req_qty_arr[$val[csf('sample_mst_id')]]+=$val[csf('sample_req_qty')];
		}
	}

	//die;
	// echo '<pre>'; print_r($sample_req_qty_arr);


	$booking_id_array = array_filter($booking_id_array);
	if(!empty($booking_id_array))
	{
		fnc_tempengine("GBL_TEMP_ENGINE", $user_name, 173, 3,$booking_id_array, $empty_arr); //recv id
		//die;
		$dtls_trans_ids_arr=array();

		$sql_ord_sample=sql_select("SELECT b.id as dtls_id, a.to_order_id as booking_id from inv_item_transfer_mst a, inv_item_transfer_dtls b, GBL_TEMP_ENGINE x where a.id=b.mst_id and a.entry_form=110 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.to_order_id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=3");
		//echo "SELECT b.id as dtls_id, a.to_order_id as booking_id from inv_item_transfer_mst a, inv_item_transfer_dtls b, GBL_TEMP_ENGINE x where a.id=b.mst_id and a.entry_form=110 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.to_order_id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=3";
	...
	$alldtlsIdArr = array_filter($alldtlsIdArr);
	if(!empty($alldtlsIdArr))
	{
		fnc_tempengine("GBL_TEMP_ENGINE", $user_name, 173, 4,$alldtlsIdArr, $empty_arr); //recv id
		//die;
		$sql_prod_roll_dtls_arr=sql_select("SELECT a.id, a.roll_no, a.barcode_no, a.po_breakdown_id, a.qnty, a.booking_no from pro_roll_details a, GBL_TEMP_ENGINE x where a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and a.dtls_id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=4");//booking_no in($booking_no) and
		//echo "SELECT a.id, a.roll_no, a.barcode_no, a.po_breakdown_id, a.qnty, a.booking_no from pro_roll_details a, GBL_TEMP_ENGINE x where a.entry_form=2 and a.status_active=1 and a.is_deleted=0 and a.dtls_id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=4";
...
//die;
	...
	if($dtls_trans_ids!="")
	{
		fnc_tempengine("GBL_TEMP_ENGINE", $user_name, 173, 5,$dtls_trans_ids_arr, $empty_arr);
		
		
		$sql_trans_prod_roll_dtls_arr=sql_select("select a.id, a.roll_no, a.barcode_no, a.po_breakdown_id, a.qnty, a.booking_no from pro_roll_details a, GBL_TEMP_ENGINE b where 
		a.dtls_id=b.ref_val and b.user_id=$user_name and b.entry_form=173 and b.ref_from=5 and  a.entry_form=110 and a.status_active=1 and a.is_deleted=0");
...
//die;
	...
	{
		fnc_tempengine("GBL_TEMP_ENGINE", $user_name, 173, 6,$prod_roll_dtls_arr, $empty_arr);
	    
		$sqlRoll="select a.BOOKING_NO, a.PO_BREAKDOWN_ID, a.ENTRY_FORM, a.QNTY from pro_roll_details a, GBL_TEMP_ENGINE b where a.barcode_no=b.ref_val and b.user_id=$user_name and b.entry_form=173 and b.ref_from=6 and a.entry_form in (58,61,66,67,68,71) and a.status_active=1 and a.is_deleted=0";
		//echo $sqlRoll; die;
		$sqlRoll_result=sql_select($sqlRoll);
		foreach($sqlRoll_result as $row)
		{
			if($row['ENTRY_FORM']==58)//Gray fabric receive
			{
				$booking_wise_gray_data[$booking_id_to_booking_no_array[$row['PO_BREAKDOWN_ID']]]['gray_rcv_qty']+=$row['QNTY'];
			}
			if($row['ENTRY_FORM']==61)//Gray fabric issue
			{
				$booking_wise_gray_data[$booking_id_to_booking_no_array[$row['PO_BREAKDOWN_ID']]]['gray_issue_qty']+=$row['QNTY'];
			}
			if($row['ENTRY_FORM']==66)//Gray finish fabric production roll wise
			{
				$booking_wise_gray_data[$row['BOOKING_NO']]['gray_fin_feb_prod_qty']+=$row['QNTY'];
			}
			if($row['ENTRY_FORM']==67)//Finish fabric roll receive by store qty
			{
				$booking_wise_gray_data[$row['BOOKING_NO']]['fin_feb_delivery_store_qty']+=$row['QNTY'];
			}
			if($row['ENTRY_FORM']==68)//Finish fabric roll receive by store
			{
				$booking_wise_gray_data[$row['BOOKING_NO']]['fin_feb_rcb_by_store_qty']+=$row['QNTY'];
			}
			if($row['ENTRY_FORM']==71)//Finish fabric roll Issue
			{
				$booking_wise_gray_data[$row['BOOKING_NO']]['fin_feb_roll_issue_qty']+=$row['QNTY'];
			}
		}
		unset($sqlRoll_result);
		/*echo "<pre>";
		print_r($booking_wise_gray_data); die;*/
	}
	...
	$booking_id_check=array();$chkArr=array();
	foreach($sql_res as $row)
	{
		//echo $row[csf("finish_fabric_qty")]."=";
		$tot_rows++;
		...
		{
			// echo $row[csf("booking_id")].'<br>';
			/*if ($checkBookingID[$row[csf("booking_id")]]=="")
			{
				// echo $row[csf("booking_id")].'<br>';
				$checkBookingID[$row[csf("booking_id")]]=$row[csf("booking_id")];
				$result_mst_array[$row[csf("booking_id")]]["smn_qty"]=$sample_req_qty_arr[$row[csf("booking_id")]];
				$buyer_wise_result[$row[csf("buyer_id")]]["summary_smn_qty"]+=$sample_req_qty_arr[$row[csf("booking_id")]];
			}*/
			if ($checkBookingIDsss[$row[csf("style_id")]]=="")
			{
				$checkBookingIDsss[$row[csf("style_id")]]=$row[csf("style_id")];
				// echo $sample_req_qty_arr[$row[csf("style_id")]].'<br>';
				$result_mst_array[$row[csf("booking_id")]]["smn_qty"]=$sample_req_qty_arr[$row[csf("style_id")]];
				$buyer_wise_result[$row[csf("buyer_id")]]["summary_smn_qty"]+=$sample_req_qty_arr[$row[csf("style_id")]];
			}


			// echo $row[csf('fabric_color')].'===<br>';
				...
				{	if($fb_color>0)
					{
						...
						/*if ($checkBookingID1[$row[csf("booking_id")]][$fb_color]=="")
						{
							// echo $row[csf("booking_id")].'<br>';
							//echo $row[csf("booking_id")].']['.$fb_color.'*<br>';
							$checkBookingID1[$row[csf("booking_id")]][$fb_color]=$row[csf("booking_id")].$fb_color;
							//echo $sample_req_qty_arr[$row[csf("booking_id")]][$fb_color].'*<br>';
							//$result_mst_array[$row[csf("booking_id")]]["smn_qty"]=$sample_req_qty_arr[$row[csf("booking_id")]][$fb_color];
							//$buyer_wise_result[$row[csf("buyer_id")]]["summary_smn_qty"]=$sample_req_qty_arr[$row[csf("booking_id")]][$fb_color];
						}*/
			...
			else
			{
				...
				/*if ($checkBookingID[$row[csf("booking_id")]][$row[csf("fabric_color")]]=="")
				{
					//echo $row[csf("booking_id")].']['.$row[csf("fabric_color")].'=<br>';
					$checkBookingID[$row[csf("booking_id")]][$row[csf("fabric_color")]]=$row[csf("booking_id")].$row[csf("fabric_color")];
					//echo $sample_req_qty_arr[$row[csf("booking_id")]][$row[csf('fabric_color')]].'=<br>';
					//$result_mst_array[$row[csf("booking_id")]]["smn_qty"]=$sample_req_qty_arr[$row[csf("booking_id")]][$row[csf('fabric_color')]];
					//$buyer_wise_result[$row[csf("buyer_id")]]["summary_smn_qty"]=$sample_req_qty_arr[$row[csf("booking_id")]][$row[csf('fabric_color')]];
				}*/
		...
		{
			$result_mst_array[$row[csf("booking_id")]]["smn_qty"]=0;
			$buyer_wise_result[$row[csf("buyer_id")]]["summary_smn_qty"]=0;
	...
	// echo $program_no;
	$sql_yarn_issue=sql_select("SELECT a.booking_id, sum(b.cons_quantity) as issue_qty, min(a.issue_date) as issue_date from GBL_TEMP_ENGINE x, inv_issue_master a, inv_transaction b where x.ref_val=a.booking_id and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1 and a.id=b.mst_id and a.issue_basis=1 and a.issue_purpose=8 and a.item_category=1 and b.transaction_type=2 and a.entry_form=3  and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and a.booking_id>0 and a.company_id=$cbo_company_name group by a.booking_id
	union all
	SELECT e.id as booking_id,sum(b.cons_quantity) as issue_qty,min(a.issue_date) as issue_date
	from inv_issue_master a, inv_transaction b, ppl_yarn_requisition_entry c, ppl_planning_entry_plan_dtls d, wo_non_ord_samp_booking_mst e, GBL_TEMP_ENGINE x
	where a.id=b.mst_id and b.requisition_no=c.requisition_no and c.knit_id=d.dtls_id and d.booking_no=e.booking_no and e.id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1 and c.prod_id=b.prod_id and a.issue_basis=3 and a.issue_purpose in(1,8) and a.item_category=1 and b.transaction_type=2 and a.entry_form=3 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and a.company_id=$cbo_company_name group by e.id");
	...
	$sql_yarn_issue_rtn=sql_select("SELECT a.booking_id,sum(b.cons_quantity) as issue_rtn_qty from GBL_TEMP_ENGINE x,inv_receive_master a, inv_transaction b where x.ref_val=a.booking_id and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1 and a.id=b.mst_id and a.receive_basis=1 and a.item_category=1 and b.transaction_type=4 and a.entry_form=9 and a.booking_id>0 and a.booking_without_order=1  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.booking_id
	union all
	SELECT c.order_id as booking_id,sum(b.cons_quantity) as issue_rtn_qty
	from inv_receive_master a, inv_transaction b , ppl_yarn_requisition_breakdown c,GBL_TEMP_ENGINE x
	where a.id=b.mst_id
	and a.booking_id=c.requisition_id and c.item_id=b.prod_id and c.order_id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1
	...
	$sql_finish_store_pi=sql_select("select d.work_order_id as booking_id, b.color_id, sum(b.receive_qnty ) as parcess_qty from  inv_receive_master a, pro_finish_fabric_rcv_dtls b, com_pi_master_details c, com_pi_item_details d, GBL_TEMP_ENGINE x where a.id=b.mst_id and a.booking_id=c.id and c.id=d.pi_id and d.work_order_id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1 and a.entry_form=37 and a.receive_basis=1 and a.booking_id>0  group by d.work_order_id, b.color_id");
	//$pi_booking_id_cond
	...
	$sql_finish_store_prod=sql_select("SELECT c.booking_no_id as booking_id, c.batch_against, a.receive_basis, a.entry_form, b.color_id, max(a.receive_date) as receive_date, sum(b.receive_qnty) as production_qty from inv_receive_master a, pro_finish_fabric_rcv_dtls b, pro_batch_create_mst c,GBL_TEMP_ENGINE x where a.id=b.mst_id and b.batch_id=c.id and c.booking_no_id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1 and c.booking_without_order=1 and c.batch_against=3 and a.entry_form in (7,37) and c.booking_no_id>0 and a.receive_basis !=11 and a.status_active=1 and b.status_active=1  group by c.booking_no_id, b.color_id,  c.batch_against, a.receive_basis, a.entry_form 
	
	union all SELECT f.id as booking_id, d.batch_against, a.receive_basis, a.entry_form, b.color_id, max(a.receive_date) as receive_date, sum(b.receive_qnty) as production_qty from inv_receive_master a, pro_finish_fabric_rcv_dtls b, pro_batch_create_mst d, wo_non_ord_knitdye_booking_mst e, wo_non_ord_samp_booking_mst f,GBL_TEMP_ENGINE x where a.id=b.mst_id and b.batch_id=d.id and d.booking_without_order=1 and a.entry_form in (37) and d.booking_no=e.booking_no and a.receive_basis=11  and e.fab_booking_id=f.id and f.id=x.ref_val and e.fab_booking_id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1 and a.status_active=1 and b.status_active=1 group by f.id, b.color_id, d.batch_against, a.receive_basis, a.entry_form");
	//$finprod_booking_id_cond
	//$fin_service_booking_id_cond
	...
	from pro_grey_prod_delivery_mst a,pro_grey_prod_delivery_dtls b, pro_batch_create_mst c,GBL_TEMP_ENGINE x
	where a.id=b.mst_id and b.batch_id=c.id and c.booking_no_id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1 and  a.entry_form=54  and b.entry_form=54 and c.batch_against=3 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  group by c.booking_no_id,b.color_id  ");
	//$finprod_booking_id_cond
	...
	$sql_grey_knit_production=sql_select("SELECT a.id,a.booking_id, sum(b.grey_receive_qnty) as receive_qty, max(a.receive_date) as receive_date, null as program_no
	from GBL_TEMP_ENGINE x, inv_receive_master a, pro_grey_prod_entry_dtls b
	where x.ref_val=a.booking_id and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1 and a.id=b.mst_id and a.receive_basis=1 and a.booking_without_order=1 and a.entry_form=2 and a.booking_id>0  group by a.id,a.booking_id
	union all
	SELECT a.id,e.id as booking_id, sum(b.grey_receive_qnty) as receive_qty, max(a.receive_date) as receive_date, a.booking_id as program_no
	from inv_receive_master a, pro_grey_prod_entry_dtls b, ppl_planning_entry_plan_dtls c, wo_non_ord_samp_booking_mst e,GBL_TEMP_ENGINE x
	where a.id=b.mst_id and a.booking_id=c.dtls_id and c.booking_no=e.booking_no and e.id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1
	and a.receive_basis=2 and a.booking_without_order=1 and a.entry_form=2 and a.booking_id>0 group by a.id,e.id, a.booking_id ");
	//$grey_del_booking_id_cond
	//$for_plan_booking_id_cond
	...
	foreach($sql_grey_knit_production as $row)
	{
		...
		$program_no_arr[$row[csf("booking_id")]]['rcv_id'] .=$row[csf("id")].',';
		$program_no_arr[$row[csf("booking_id")]]['program_no'] .=$row[csf("program_no")].',';
		//$prod_booking_id_arr[$row[csf("id")]]=$row[csf("booking_id")];
		array_push($all_prog_no_arr,$row[csf("program_no")]);
	}
	...
	from GBL_TEMP_ENGINE x, inv_receive_master a,  pro_grey_prod_delivery_dtls b, pro_grey_prod_delivery_mst c
	where x.ref_val=a.booking_id and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1 and c.id=b.mst_id and a.entry_form=2 and c.entry_form in(53,56) and b.grey_sys_id=a.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.booking_without_order=1  group by a.booking_id, c.id
	union all
	...
	from inv_receive_master a, pro_grey_prod_delivery_dtls b, pro_grey_prod_delivery_mst c, ppl_planning_entry_plan_dtls d, wo_non_ord_samp_booking_mst e,GBL_TEMP_ENGINE x
	where c.id=b.mst_id and a.entry_form=2 and c.entry_form in(53,56) and b.grey_sys_id=a.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
	and a.booking_id=d.dtls_id and d.booking_no=e.booking_no and e.id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1
	...
	$sql_grey_knit_receive=sql_select("SELECT a.booking_id, a.entry_form, ( case when a.receive_basis <>9 then b.cons_quantity else 0 end) as receive_qty,( case when a.receive_basis in(9) then b.cons_quantity else 0 end) as production_qty
		...
		where a.id=b.mst_id and a.booking_id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1
		
		and a.booking_without_order=1 and a.entry_form in(2,22) and b.transaction_type in(1,4) and a.booking_id>0 ");
	...
	//echo "<pre>";print_r($grey_knit_receive_prod_arr);

	$sql_grey_booking=sql_select("select a.id,a.booking_id from inv_receive_master a,GBL_TEMP_ENGINE x where a.booking_id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1 and a.receive_basis=1 and a.booking_without_order=1 and a.entry_form=2 ");
	//$grey_del_booking_id_cond
	...
	from inv_item_transfer_mst a, inv_transaction b, GBL_TEMP_ENGINE x where a.id=b.mst_id and a.order_id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1 and a.item_category=13 and b.item_category=13 and b.transaction_type in(5,6)
	...
	from GBL_TEMP_ENGINE x,inv_issue_master a, inv_grey_fabric_issue_dtls b where x.ref_val=a.booking_no and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1 and a.id=b.mst_id and a.issue_basis in(0,1) and a.issue_purpose in(8,11)
	...
	{
		$sql_grey_issue=sql_select("SELECT a.booking_id, sum(b.issue_qnty) as issue_qty, max(a.issue_date) as issue_date
		from inv_issue_master a, inv_grey_fabric_issue_dtls b where a.id=b.mst_id and a.issue_basis in(3) and a.issue_purpose in(8,11)
		and a.entry_form in(16,61) ".where_con_using_array($all_prog_no_arr,0,'a.booking_id')."  group by a.booking_id order by a.booking_id");
		//$grey_del_pi_wo_batch_no_cond
		foreach($sql_grey_issue as $result)
		{
			$grey_issue_arr[$result[csf("booking_id")]]+=$result[csf("issue_qty")];
	...
	$sql_batch_qty=sql_select("select a.id, a.batch_no, a.booking_no_id, a.color_id, sum(b.batch_qnty) as batch_qnty, max(a.batch_date) as batch_date from GBL_TEMP_ENGINE x, pro_batch_create_mst a, pro_batch_create_dtls b where  x.ref_val=a.booking_no_id and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1 and  a.id=b.mst_id and a.booking_without_order=1  group by a.id,a.batch_no,a.booking_no_id ,a.color_id ");
	//$allBookingIds_cond
	...
	$sql_dyeing_qty=sql_select("select a.id, a.booking_no_id,a.color_id, max(a.batch_date) as  batch_date from GBL_TEMP_ENGINE x,pro_batch_create_mst a, pro_fab_subprocess c where x.ref_val=a.booking_no_id and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1 and c.batch_id=a.id and a.booking_without_order=1 and c.load_unload_id=2  group by a.id, a.booking_no_id, a.color_id ");
	//$allBookingIds_cond
	...
	$sql_finish_receive=sql_select("select a.booking_id, b.color_id,sum(b.receive_qnty) as receive_qty from GBL_TEMP_ENGINE x,inv_receive_master a, pro_finish_fabric_rcv_dtls b where x.ref_val=a.booking_id and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1 and a.id=b.mst_id and a.receive_basis !=9 and a.booking_without_order=1 and a.entry_form=37 and a.status_active=1 and a.is_deleted=0  group by a.booking_id,b.color_id");
	//$grey_del_booking_id_cond
	...
	$sql_cutting_issue=sql_select("SELECT c.booking_no_id, c.color_id, sum(b.issue_qnty) as issue_qty from inv_issue_master a, inv_finish_fabric_issue_dtls b, pro_batch_create_mst c, wo_non_ord_samp_booking_mst d,GBL_TEMP_ENGINE x where a.id=b.mst_id and b.batch_id=c.id and a.entry_form=18 and c.booking_no_id>0 and c.booking_without_order=1 and c.booking_no = d.booking_no and c.booking_no_id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1 group by c.booking_no_id,c.color_id union all select  f.id as booking_id,  d.color_id,  sum(b.issue_qnty) as issue_qty from inv_issue_master a, inv_finish_fabric_issue_dtls b, pro_batch_create_mst d, wo_non_ord_knitdye_booking_mst e, wo_non_ord_samp_booking_mst f,GBL_TEMP_ENGINE x where a.id=b.mst_id and b.batch_id=d.id and d.booking_without_order=1 and a.entry_form in (18) and d.booking_no=e.booking_no and d.booking_no_id>0 and e.fab_booking_id=f.id and f.id=x.ref_val and e.fab_booking_id=x.ref_val and x.user_id=$user_name and x.entry_form=173 and x.ref_from=1 group by f.id, d.color_id");
	...
	$con = connect();
	$r_id2=execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_name." and ENTRY_FORM=173");
	...
	<div style="width:2350px; margin-bottom:10px;">
		...
		<table class="rpt_table" border="1" rules="all" width="1700" cellpadding="0" cellspacing="0">
			<thead>
	        	<tr>
					<th  colspan="17" align="center">Buyer Level Summary</th>
				</tr>
	        	<tr>
					<th width="40">SL</th>
	                <th width="80">Buyer Name</th>
					<th width="100" title="Sample Requisition>Sample Req Qty">Sample Qty</th>
			...
			foreach($buyer_wise_result as $row_result)
			{
	        	...
	        	<tr bgcolor="<? echo $bgcolor; ?>">
	                ...
	                <td align="right"><? echo number_format($row_result["summary_smn_qty"],2); $buyer_tot_sample_qty+=$row_result["summary_smn_qty"]; ?></td>
	        ...
	        <tfoot>
	        	<tr>
					...
					<th align="right"><? echo number_format($buyer_tot_sample_qty,2); ?></th>
	...
	<table class="rpt_table" border="1" rules="all" width="5050" cellpadding="0" cellspacing="0">
		<thead>
			<tr>
				<th colspan="17">Booking Details</th>
	...
	<div style="width:5075px; overflow-y:scroll; max-height:300px" id="scroll_body">
    <table class="rpt_table" border="1" rules="all" width="5050" cellpadding="0" cellspacing="0" id="table_body">
        <tbody>
	        <?
			...
			foreach($result_mst_array as $bookingID => $row)
			{
	        	...
	        	<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
	                ...
	                <td width="65" align="center"><p><a href='##' style='color:#000' onclick="generate_order_report('<? echo $row['booking_no'];?> ','<? echo $row['company_id']; ?>','<? echo $row['is_approved'];?>','<? echo $action;?>','<? echo $row['entry_form_id'];?>','<? echo $row['style_id'];?>')"><? echo $row[("booking_no_prefix_num")]; ?></a></p></td>

	                <td width="80"><p><? echo $buyer_short_name_library[$row[("buyer_id")]]; ?></p></td>
	                <td width="100" align="right"><p><? echo number_format($row["smn_qty"],2); ?></p></td>
					...
					foreach($result_dtls_array[$row[("booking_id")]] as $dts_row)
					{
						if($m==1)
						{
	                        ...
	                        <td width="110" align="right"><p><a  href="##" onclick="openmypage('<? echo $row[("booking_id")]; ?>','yarn_issue','<? echo $row[("booking_no")]; ?>')">
							<?
								$net_yarn_issue=$yarn_issue_arr[$row[("booking_id")]]-$yarn_issue_rtn_arr[$row[("booking_id")]];
	                    	...
	                    	<?  //echo $prog_booking_id_array[$row[("booking_id")]];
	                    		...
	                    		{
									if($program_no_arr[$row["booking_id"]]['program_no'] !="")
									{
										$all_receive_id=implode(",", array_filter(array_unique(explode(",",$program_no_arr[$row["booking_id"]]['program_no']))));
										$id=$all_receive_id."_withoutRoll";
									}
									else
									{
							...
							else
							{
								...
								if($program_no_arr[$row["booking_id"]]['rcv_id'] !="")
								{
									$all_receive_id=array_filter(array_unique(explode(",",$program_no_arr[$row["booking_id"]]['rcv_id'])));
								}
								else
								{
		                    ...
		                    <td width="110" align="right"><p>
								...
								<?
		                            ...
		                            else
		                            {
										if($program_no_arr[$row["booking_id"]]['program_no'] !="")
										{
											$prog_no=implode(",", array_filter(array_unique(explode(",",$program_no_arr[$row["booking_id"]]['program_no']))));

											echo number_format($grey_issue_arr[$prog_no],2);
											$dtls_tot_gray_issue+=$grey_issue_arr[$prog_no];
											$buyer_wise_result[$row[("buyer_id")]]["grey_issue"] +=$grey_issue_arr[$prog_no];
										}
										else
										{
								...
								<td width="110" align="right" title="Entry Form=<? echo $row[("entry_form_id")].' booking_id='.$row[("booking_id")].' Color Id='.$dts_row[("fabric_color")];?>">
									<p>
								...
								<td width="110" align="right" title="<? echo "roll".$finish_roll_maintained;?>"><p>
									<?
										//if ($booking_wise_roll_check[$row[("booking_id")]]==1)
										if($finish_roll_maintained==1)
			                    		{
						...
						else
						{
	                        ...
	                        <tr>
	                            ...
	                            <td width="80">&nbsp;</td>
	                            <td width="100" title="sample qty">&nbsp;</td>
	                            <td width="80">&nbsp;</td>
	                            <td width="80">&nbsp;</td>
	                            <td width="80">&nbsp;</td>
	                            <td width="80">&nbsp;</td>
    ...
    <table class="rpt_table" border="1" rules="all" width="5050" cellpadding="0" cellspacing="0">
		<tfoot>
			<tr>
                ...
                <th width="65">&nbsp;</th>
                <th width="80">&nbsp;</th>
                <th width="100" id="value_dtls_tot_sample_qty"><? echo number_format($dtls_tot_sample_qty,2); ?></th>
                <th width="80">&nbsp;</th>
                <th width="80">&nbsp;</th>
                <th width="80">&nbsp;</th>
                <th width="80">&nbsp;</th>
	...
	<fieldset style="width:965px; margin-left:3px">
		<div id="report_container">
            <table border="1" class="rpt_table" rules="all" width="960" cellpadding="0" cellspacing="0">
				...
				where a.id=d.mst_id and d.prod_id=c.id and d.requisition_no=e.requisition_id and e.item_id=d.prod_id and c.id=e.item_id and a.issue_basis=3 and a.issue_purpose=1 and a.item_category=1 and d.transaction_type=2 and a.entry_form=3 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and e.order_id=$boking_id and e.booking_no='$booking_no'
				group by a.booking_id, a.booking_no,a.issue_number, a.issue_date, a.challan_no, a.knit_dye_source, a.knit_dye_company,c.lot, c.yarn_type, c.id, c.product_name_details,d.brand_id";*/
				...
				where a.id=d.mst_id and d.prod_id=c.id and d.requisition_no=e.requisition_no and e.knit_id=f.dtls_id and f.booking_no='$booking_no' and e.prod_id=d.prod_id and a.issue_basis=3 and a.issue_purpose in(1,8) and a.item_category=1 and d.transaction_type=2 and a.entry_form=3 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 group by a.booking_id, a.booking_no,a.issue_number, a.issue_date, a.challan_no, a.knit_dye_source, a.knit_dye_company,c.lot, c.yarn_type, c.id,
				...
				$booking_no_details=return_library_array( "select id, booking_no from wo_non_ord_samp_booking_mst where id in ($boking_id)", "id", "booking_no");
				if(!empty($result_out))
				{
					?>
	                <thead>
			...
			<div id="report_container">
				<table border="1" class="rpt_table" rules="all" width="970" cellpadding="0" cellspacing="0">
	             ...
	             <div style="width:990px; max-height:330px; overflow-y:scroll" id="scroll_body">
					...
					if($booking_type=="withoutRoll")
					{
						$sql_grey_booking=sql_select("select a.id,a.booking_id from inv_receive_master a where a.receive_basis in(1,2) and a.booking_without_order=1 and a.entry_form=2 and a.booking_id=$boking_id");
			...
			<div id="report_container">
				<table border="1" class="rpt_table" rules="all" width="860" cellpadding="0" cellspacing="0">
	             ...
	             <div style="width:877px; max-height:320px; overflow-y:scroll" id="scroll_body">
					...
					//echo $sql;
	                    $result=sql_select($sql);
						$batchidArr=array(); $productIdArr=array();
						foreach($result as $brow)
						{
							$batchidArr[$brow[csf('batch_id')]]=$brow[csf('batch_id')];
							$productIdArr[$brow[csf('prod_id')]]=$brow[csf('prod_id')];
						}
						
						$batch_details=return_library_array( "select id, batch_no from pro_batch_create_mst where 1=1 and id in (".implode(",",$batchidArr).")", "id", "batch_no");
						$product_details=return_library_array( "select id, product_name_details from product_details_master where 1=1 and id in (".implode(",",$productIdArr).")", "id", "product_name_details");
	        			foreach($result as $row)
	                    {
	                        if ($i%2==0)
	                            $bgcolor="#E9F3FF";
	             ...
	             <div style="width:877px; max-height:320px; overflow-y:scroll" id="scroll_body">
	                 <table border="1" class="rpt_table" rules="all" width="860" cellpadding="0" cellspacing="0">
	                    ...
	                    if($booking_type=="withoutRoll")
	                    {
							...
							where a.id=b.mst_id and a.entry_form=16 and a.booking_id=$id and a.issue_basis in(1,3) and a.issue_purpose=8 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
						...
						foreach($result as $brow)
						{
							$batchidArr[$brow[csf('batch_no')]]=$brow[csf('batch_no')];
						}
	...
	<script>
		...
		<fieldset style="width:980px; margin-left:3px">
			...
			/*	$sql_finish_prodction_delivery=sql_select("select max(d.delevery_date) as receive_date ,sum(e.current_delivery) as delivery_qty,c.booking_no_id from inv_receive_master a,  pro_finish_fabric_rcv_dtls b,pro_batch_create_mst c,pro_grey_prod_delivery_mst d,pro_grey_prod_delivery_dtls e where d.id=e.mst_id and e.grey_sys_id=a.id and d.entry_form=54  and e.entry_form=54 and a.id=b.mst_id and a.entry_form=7  and b.batch_id=c.id  and c.batch_against=3 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0   group by c.booking_no_id ");*/
						//echo $sql;
	                    $result=sql_select($sql);
						$nonBookingidArr=array();
						foreach($result as $brow)
						{
							$nonBookingidArr[$brow[csf('booking_no_id')]]=$brow[csf('booking_no_id')];
						}
						
						$booking_no_details=return_library_array( "select id, booking_no from wo_non_ord_samp_booking_mst where id in (".implode(",",$nonBookingidArr).")", "id", "booking_no");
	        			foreach($result as $row)
	                    {
	                        if ($i%2==0)
	                            $bgcolor="#E9F3FF";
