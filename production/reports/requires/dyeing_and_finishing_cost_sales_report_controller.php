...
if($action=="report_generate")
{
	...
	$con = connect();
    execute_query("delete from tmp_batch_or_iss where userid=$user_name");
    execute_query("delete from tmp_batch_id where userid=$user_name");
    execute_query("delete from tmp_booking_id where userid=$user_name");
    execute_query("delete from tmp_prog_no where userid=$user_name");
    execute_query("delete from tmp_barcode_no where userid=$user_name");
    oci_commit($con);

    if ($issue_type==2) // With issue for In-house and Inbound
	{
		if ($batch_number!="") 
		{
		...
		foreach($sql_chemical_issue as $val)
		{
			...
			foreach ($all_batch_no as $key => $batchId) 
			{
				...
				/*if( $batchId_check[$batchId] =="" )
		        {
		            $batchId_check[$batchId]=$batchId;
		            $batch_id = $batchId;
		            // echo "insert into tmp_batch_id (userid, batch_id) values ($user_name,$batch_id)";
		            execute_query("insert into tmp_batch_id (userid, batch_id) values ($user_name,$batch_id");
		        }*/
		        ...
		        if( $batchId_check[$batchId] =="" )
		        {
		            ...
		            $batch_id = $batchId;
		            // echo "insert into tmp_batch_id (userid, batch_id) values ($user_name,$batch_id)";
		            $r_ids=execute_query("insert into tmp_batch_id (userid, batch_id) values ($user_name,$batch_id)");
		        }
			}
		}
		oci_commit($con);
		...
		from pro_batch_create_dtls b, fabric_sales_order_mst d, pro_fab_subprocess f, pro_batch_create_mst a, tmp_batch_id c 
		where f.batch_id=a.id and a.id=b.mst_id and f.batch_id=b.mst_id and b.po_id=d.id and a.id=c.batch_id and c.userid=$user_name and a.company_id=$company $dates_com $fsodata $batch_num $year_cond and a.entry_form=0  and f.entry_form=35 and f.load_unload_id=2 and a.batch_against in(1,11,2,3) and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and a.is_sales=1 and b.is_sales=1 and f.result=1";
		...
		from pro_batch_create_dtls b, subcon_ord_dtls c, subcon_ord_mst d, pro_batch_create_mst a, pro_fab_subprocess f, tmp_batch_id c 
		where f.batch_id=a.id and f.batch_id=b.mst_id and a.id=b.mst_id and b.po_id=c.id and d.subcon_job=c.job_no_mst and a.id=c.batch_id and c.userid=$user_name and a.company_id=$company and a.entry_form=36 and f.entry_form=38 and f.load_unload_id=2 and a.batch_against in(1,11,2,3) and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and f.result=1 $dates_com $batch_num $fsodata $year_con";
		// echo $sql_subcon;die;
	}
	// echo "<pre>";print_r($issue_batch_id_arr);die;
	// echo "<pre>";print_r($issue_batch_cond);die;
	// ==========================================Inhouse Batch Start============================================
	if ($issue_type==1) // Without issue
	{
		$inhouse_batch_sql = "SELECT f.insert_date, a.batch_no, a.batch_weight, a.id as batch_id,a.sales_order_id, a.batch_against,a.color_id, a.color_range_id, a.extention_no, a.total_trims_weight, a.process_id as process_name, (b.batch_qnty) as batch_qnty, b.program_no,b.item_description, b.po_id, b.prod_id,b.body_part_id, b.width_dia_type, d.job_no_prefix_num, d.buyer_id, d.po_buyer, f.remarks, f.shift_name, f.production_date as process_end_date, f.process_end_date as production_date, f.end_hours,f.system_no, f.floor_id,f.multi_batch_load_id, f.hour_unload_meter, f.water_flow_meter, f.end_minutes, f.machine_id, f.load_unload_id, f.fabric_type,f.responsibility_id, f.result, a.booking_no, d.booking_without_order, d.season, d.job_no  as sales_no,d.style_ref_no , b.barcode_no, f.process_id, f.ltb_btb_id, d.within_group, d.booking_id
	...
	foreach($inhouse_batchdata as $row)
	{
        ...
        if( $program_no_check[$row[csf('program_no')]] =="" )
        {
            $program_no_check[$row[csf('program_no')]]=$row[csf('program_no')];
            $program_no = $row[csf('program_no')];
            execute_query("insert into tmp_prog_no (userid, prog_no) values ($user_name,$program_no)");
        }
        ...
        if( $barcode_no_check[$row[csf('barcode_no')]] =="" )
        {
            ...
            execute_query("insert into tmp_barcode_no (userid, barcode_no) values ($user_name,$barcode_no)");
        }
	}
	oci_commit($con);
	// echo $inhouse_batch_sql;die;
	...
	$sql_booking_type="SELECT a.booking_no, a.booking_type, a.is_short,1 as with_order_status 
	from wo_booking_mst a, tmp_booking_id b
	where a.id=b.booking_id and b.userid=$user_name and a.status_active=1 and a.is_deleted=0
	union all 
	select a.booking_no, a.booking_type, a.is_short, 2 as with_order_status
	from wo_non_ord_samp_booking_mst a, tmp_booking_id b
	where a.id=b.booking_id and b.userid=$user_name and a.status_active=1 and a.is_deleted=0";
	// echo $sql_booking_type;die;
	...
	{
		$sql_prog="SELECT b.id as program_id, b.id as program_no,a.color_type_id 
		FROM ppl_planning_entry_plan_dtls a, ppl_planning_info_entry_dtls b, tmp_prog_no c
		WHERE a.dtls_id=b.id and b.id=c.prog_no and a.dtls_id=c.prog_no and c.userid=$user_name  and  a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.is_sales=1";// $progId_batch_cond_for_in
		// echo $sql_prog;die;
	...
	if (!empty($all_batch)) // in-hounse batch // Problem done
	{
		...
		where a.id = b.mst_id and a.batch_id=c.mst_id and b.prod_id=c.prod_id and a.batch_id=d.batch_issue_id  and c.mst_id=d.batch_issue_id and d.type=1 and d.entry_form=0 and d.userid=$user_name and a.load_unload_id=2 and b.load_unload_id=2 and b.entry_page=35 and a.status_active = 1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and  c.is_deleted=0");// $all_batch_no_cond2
		...
		where a.id = b.mst_id and a.batch_id=c.batch_issue_id and c.type=1 and c.entry_form=0 and c.userid=$user_name and a.load_unload_id=2 and b.load_unload_id=2 and b.entry_page=35 and a.status_active = 1 and b.status_active=1"); // and a.batch_id in ($all_batch_ids)  $all_batch_no_cond2
	...
	if (!empty($all_barcode)) // Problem 
	{
		...
		// $yarn_lot_data=sql_select("SELECT b.barcode_no, a.prod_id,c. yarn_comp_type1st,c.yarn_comp_percent1st, c.yarn_count_id,c.yarn_type,c.color,c.lot from ppl_yarn_requisition_entry a, pro_roll_details b, product_details_master c where b.entry_form=2 and b.receive_basis=2 and cast(a.knit_id as varchar2(4000)) = b.booking_no and a.prod_id = c.id and c.item_category_id=1 and a.status_active=1 and b.status_active=1 and c.status_active=1 $barcode_cond");
		
		$yarn_lot_data=sql_select("SELECT b.barcode_no, a.prod_id,c.
		yarn_comp_type1st,c.yarn_comp_percent1st, c.yarn_count_id,c.yarn_type,c.color,c.lot 
		from tmp_prog_no d, ppl_yarn_requisition_entry a, pro_roll_details b, product_details_master c
		where d.prog_no=a.knit_id and cast(a.knit_id as varchar2(4000)) = b.booking_no and a.prod_id = c.id and d.userid=$user_name and b.entry_form=2 and b.receive_basis=2 and c.item_category_id=1 and a.status_active=1 and b.status_active=1 and c.status_active=1");
	...
	foreach($subbatchdata as $row) 
	{
		...
		if( $subcon_batch_id_check[$row[csf('batch_id')]] =="" )
        {
            $subcon_batch_id_check[$row[csf('batch_id')]]=$row[csf('batch_id')];
            $subcon_batch_id = $row[csf('batch_id')];
            // echo "insert into tmp_batch_or_iss (userid, batch_issue_id, batch_issue_no, type, entry_form) values ($user_name,$subcon_batch_id,'$subcon_batch_id',2,36)";
            $r_id2=execute_query("insert into tmp_batch_or_iss (userid, batch_issue_id, batch_issue_no, type, entry_form) values ($user_name,$subcon_batch_id,'$subcon_batch_id',2,36)");
        }
	}
	oci_commit($con);	
	// echo $sql_subcon;die;
	...
	if (!empty($all_sub_batch)) 
	{
		...
		from  pro_fab_subprocess a, pro_batch_create_dtls b, tmp_batch_or_iss c
		where a.batch_id =b.mst_id and a.batch_id=c.batch_issue_id and c.type=2 and c.entry_form=36 and c.userid=$user_name and a.load_unload_id=2 and a.load_unload_id=2 and a.entry_form=38 and a.status_active=1 and b.status_active=1 "); //  $all_sub_batch_no_cond2
	...
	and b.transaction_type=2 and a.entry_form=5 and a.issue_basis=5 and a.batch_no  is not null  and   b.item_category in (5,6,7)"); //  and c.sub_process!=92
	// and d.batch_no='RS-5-Batch' and d.sales_order_no='UG-FSOE-22-00009'
	$batch_based_dyes_chemical_arr=array(); $batch_based_data_arr=array();$batch_based_all_batch=array();
	...
	// echo '<pre>';print_r($batch_based_dyes_chemical_arr);die;

	oci_commit($con);
	unset($batch_based_sql);
	// echo "checking";die; //3

	/*$all_batch_based_ids= implode(",",array_filter(array_unique($batch_based_all_batch)));
	$batchCond = $batch_based_id_cond = ""; 
	$all_batch_based_arr=explode(",",$all_batch_based_ids);
	if($db_type==2 && count($all_batch_based_arr)>999)
	{
		$all_batch_chunk=array_chunk($all_batch_based_arr,999) ;
		foreach($all_batch_chunk as $chunk_arr)
		{
			$batchCond.=" a.id in(".implode(",",$chunk_arr).") or ";	
		}
		$batch_based_id_cond.=" and (".chop($batchCond,'or ').")";		
	}
	else
	{ 	
		$batch_based_id_cond=" and a.id in($all_batch_based_ids)";
	}*/
	// echo $batch_based_id_cond;die;

	if (!empty($batch_based_all_batch)) // batch_based_inhouse production
	{
		...
		from  pro_fab_subprocess a, pro_fab_subprocess_dtls b,pro_batch_create_dtls c, tmp_batch_or_iss d
		where a.id = b.mst_id and a.batch_id=c.mst_id and b.prod_id=c.prod_id and a.batch_id=d.batch_issue_id and c.mst_id=d.batch_issue_id and d.type=3 and d.entry_form=5 and d.userid=$user_name and a.load_unload_id in(2) and b.load_unload_id in(2) and b.entry_page=35 and a.status_active = 1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0");// $all_batch_no_cond2
		...
		from  tmp_batch_or_iss c, pro_fab_subprocess a, pro_fab_subprocess_dtls b
		where c.batch_issue_id=a.batch_id and a.id = b.mst_id and c.type=3 and c.entry_form=5 and c.userid=$user_name and a.load_unload_id in(2) and b.load_unload_id in(2) and b.entry_page=35 and a.status_active = 1 and b.status_active=1"); // and a.batch_id in ($all_batch_ids)  $all_batch_no_cond2
		...
		from tmp_batch_or_iss c, pro_batch_create_mst a, pro_batch_create_dtls b, fabric_sales_order_mst d, pro_fab_subprocess f
		where c.batch_issue_id=a.id and c.batch_issue_id=b.mst_id and c.batch_issue_id=f.batch_id and a.id=b.mst_id and a.id=f.batch_id  and b.mst_id=f.batch_id  and b.po_id=d.id and a.company_id=$company  $fsodata $batch_num $year_cond and a.entry_form=0 and c.type=3 and c.entry_form=5 and c.userid=$user_name and f.entry_form=35 and f.load_unload_id=2 and a.batch_against in(1,11,2,3) and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and a.is_sales=1 and b.is_sales=1 and f.result=1";
		/*$batch_based_inhouse_batch_sql = "SELECT f.insert_date, a.batch_no, a.batch_weight, a.id as batch_id,a.sales_order_id, a.batch_against,a.color_id, a.color_range_id, a.extention_no, a.total_trims_weight, a.process_id as process_name, b.batch_qnty as batch_qnty, b.program_no,b.item_description, b.po_id, b.prod_id,b.body_part_id, b.width_dia_type, d.job_no_prefix_num, d.buyer_id, d.po_buyer, f.remarks, f.shift_name, f.production_date as process_end_date, f.process_end_date as production_date, f.end_hours,f.system_no, f.floor_id,f.multi_batch_load_id, f.hour_unload_meter, f.water_flow_meter, f.end_minutes, f.machine_id, f.load_unload_id, f.fabric_type,f.responsibility_id, f.result, a.booking_no, d.booking_without_order, d.season, d.job_no as sales_no,d.style_ref_no , b.barcode_no, f.process_id, f.ltb_btb_id, d.within_group, d.booking_id 
		from pro_batch_create_mst a, pro_batch_create_dtls b, fabric_sales_order_mst d, pro_fab_subprocess f
		where a.id=b.mst_id and a.id=f.batch_id  and b.mst_id=f.batch_id  and b.po_id=d.id and a.company_id=$company  $fsodata $batch_num and a.entry_form=0 and f.entry_form=35 and f.load_unload_id=2 and a.batch_against in(1,11,2,3) and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and a.is_sales=1 and b.is_sales=1 and f.result=1 $batch_based_id_cond";*/
		// echo $batch_based_inhouse_batch_sql;die;
		$batch_based_inhouse_batchdata=sql_select($batch_based_inhouse_batch_sql);// $year_cond
	...
	if (!empty($all_batch)) // dying productionload_data
	{
		$load_time_data=sql_select("SELECT f.batch_id, f.water_flow_meter, f.batch_no, f.load_unload_id, f.process_end_date, f.end_hours, f.hour_load_meter, f.end_minutes 
		from pro_fab_subprocess f, tmp_batch_or_iss g 
		where f.batch_id=g.batch_issue_id and g.type in(1,3) and g.entry_form in(0,5) and g.userid=$user_name and f.load_unload_id=1 and f.entry_form=35 $companyCond1 and f.status_active=1  and f.is_deleted=0 $workingCompany_name_cond13");
	...
	if (!empty($all_sub_batch)) // dying productionload_data
	{
		$sub_load_time_data=sql_select("SELECT f.batch_id, f.water_flow_meter, f.batch_no, f.load_unload_id, f.process_end_date, f.end_hours, f.hour_load_meter, f.end_minutes 
		from pro_fab_subprocess f, tmp_batch_or_iss g 
		where f.batch_id=g.batch_issue_id and g.type=2 and g.entry_form=36 and g.userid=$user_name and f.load_unload_id=1 and f.entry_form=38 $companyCond1 and f.status_active=1  and f.is_deleted=0 $workingCompany_name_cond13");
    ...
    {
    	$batch_weight_arr[$batch_id] = $row["prod_qnty_tot"]+$row["total_trims_weight"];
    }
    foreach($subbatch_data_arr as $batch_id=>$row)
    {
    	$batch_weight_arr[$batch_id] = $row["production_qnty"]+$row["total_trims_weight"];
    }
    foreach($batch_based_data_arr as $batch_id=>$row)
    {
    	$batch_weight_arr[$batch_id] = $row["production_qnty"]+$row["total_trims_weight"];
    }
    // echo '<pre>';print_r($batch_weight_arr);
    $sql_dyes_cost =sql_select("SELECT a.batch_no, a.issue_basis, a.issue_purpose,c.sub_process,b.id as trans_id,b.item_category,b.cons_amount as dyes_chemical_cost
    from inv_issue_master a, inv_transaction b, dyes_chem_issue_dtls c
    where a.id=b.mst_id and a.id=c.mst_id and b.id=c.trans_id and b.transaction_type=2 and a.entry_form=5 and a.batch_no  is not null  and b.item_category in (5,6,7)"); //and b.item_category in (5,6,7,23)
    // and c.batch_id in('16027,16028','15880')
    // and c.batch_id in('155366,155352')
     // and c.batch_id in('155352,155366') and a.issue_basis=7

	
	$dyes_chemical_arr=array();
	$in_out_dyes_chemical_arr=array();
	foreach($sql_dyes_cost as $val)
	{
		...
		foreach ($all_batch_no as $key => $batchId) 
		{
			$tot_batch_weight+=$batch_weight_arr[$batchId];
		}
		foreach ($all_batch_no as $key => $batchId) 
		{
			// Total issue value / Total Batch weight * Single Batch weight.
			if($sub_process!=92)
			{
				$dyes_chemical_arr[$batchId][$val[csf("item_category")]]['chemical_cost']+=(($val[csf("dyes_chemical_cost")]/$tot_batch_weight)*$batch_weight_arr[$batchId]);
			}
			else
			{
				$dyes_chemical_arr[$batchId][$val[csf("item_category")]]['chemical_cost_finish']+=(($val[csf("dyes_chemical_cost")]/$tot_batch_weight)*$batch_weight_arr[$batchId]);
			}

			if ($val[csf("issue_basis")]==7) // Requisition
			{
				// Total issue value / Total Batch weight * Single Batch weight.
				if($sub_process!=92)
				{
					$in_out_dyes_chemical_arr[$batchId][$val[csf("item_category")]]['chemical_cost']+=(($val[csf("dyes_chemical_cost")]/$tot_batch_weight)*$batch_weight_arr[$batchId]);
				}
				else
				{
					$in_out_dyes_chemical_arr[$batchId][$val[csf("item_category")]]['chemical_cost_finish']+=(($val[csf("dyes_chemical_cost")]/$tot_batch_weight)*$batch_weight_arr[$batchId]);
				}
                    ...
                    <table class="rpt_table" id="table_body" width="<? echo $table_width;?>" cellpadding="0" cellspacing="0" border="1" rules="all">
                        <tbody>
                            ...
                            foreach($inhouse_batch_data_array as $batch_id=>$row)
                            {
								...
								$first_chemi_cost=$in_out_dyes_chemical_arr[$batch_id][5]['chemical_cost']+$in_out_dyes_chemical_arr[$batch_id][7]['chemical_cost'];
				                $first_dyeing_cost=$in_out_dyes_chemical_arr[$batch_id][6]['chemical_cost'];
				                $chemical_cost_finish=$in_out_dyes_chemical_arr[$batch_id][5]['chemical_cost_finish']+$in_out_dyes_chemical_arr[$batch_id][7]['chemical_cost_finish'];
                                ...
                                <tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')">
                                    ...
                                    <td width="100" rowspan="<? echo $batch_td_span?>" title="<? echo $load_date[$batch_id];?>">
									<div  style="word-wrap:break-word;">&nbsp;
									<? $load_t=$load_hr[$batch_id].':'.$load_min[$batch_id]; echo  ($load_date[$batch_id] == '0000-00-00' || $load_date[$batch_id] == '' ? '' : change_date_format($load_date[$batch_id])).' &nbsp; '.$load_t; ?>
									</div>
									</td>
                                    <td width="100"><? $hr=strtotime($unload_date,$load_t); $min=($row['end_minutes'])-($load_min[$batch_id]);
										echo  ($row['process_end_date'] == '0000-00-00' || $row['process_end_date'] == '' ? '' : change_date_format($row['process_end_date'])).' &nbsp; '.$unload_time=$row['end_hours'].':'.$row['end_minutes']; ?>
									</td> 
                                    <td width="70">
                                    	<? 
		                            ...
		                            <td width="70" align="right" title="Chemical Cost+Dyes Cost+Finishing Cost"><? echo number_format($chemical_cost_finish+$batch_chemical_price,4,".",""); 
		                            ?></td>
                ...
                <table class="rpt_table" width="<? echo $table_width;?>" cellpadding="0" cellspacing="0" border="1" rules="all">
                    <tfoot>
                        <tr>
                            ...
                            <th width="70"><? echo number_format($total_batch_chemical_price+$tot_re_dying_dyes_chemical_cost+$tot_re_dying_chemical_cost,4); ?></th>
                            <th width="">&nbsp;</th>
                        </tr>
                    </tfoot>
                </table>
                    ...
                    <table class="rpt_table" id="table_body2" width="<? echo $table_width;?>" cellpadding="0" cellspacing="0" border="1" rules="all">
                        <tbody>
                            ...
                            foreach($subbatch_data_arr as $batch_id => $row)
                            {
								...
								$sub_first_chemi_cost=$in_out_dyes_chemical_arr[$batch_id][5]['chemical_cost']+$in_out_dyes_chemical_arr[$batch_id][7]['chemical_cost'];
				                $sub_first_dyeing_cost=$in_out_dyes_chemical_arr[$batch_id][6]['chemical_cost'];
				                $sub_chemical_cost_finish=$in_out_dyes_chemical_arr[$batch_id][5]['chemical_cost_finish']+$in_out_dyes_chemical_arr[$batch_id][7]['chemical_cost_finish'];
	                            ...
	                            <tr bgcolor="<? echo $bgcolor; ?>" id="tr2_<? echo $j; ?>" onClick="change_color('tr2_<? echo $j; ?>','<? echo $bgcolor; ?>')">
                                    ...
                                    <td align="right" width="100" title="Batch Weight/Machine Capacity*100"><? echo number_format(($batch_qnty/$machine_capacity_arr[$row['machine_id']]*100),2);  ?></td>
                                    <td width="100"><p style="word-wrap:break-word;" title="<? echo $sub_load_date[$batch_id];?>"><? $load_t=$sub_load_hr[$batch_id].':'.$sub_load_min[$batch_id];echo  ($sub_load_date[$batch_id] == '0000-00-00' || $sub_load_date[$batch_id] == '' ? '' : change_date_format($sub_load_date[$batch_id])).' &nbsp; '.$load_t;?></p></td>
                                    <td width="100"><? $hr=strtotime($unload_date,$load_t); $min=($row['end_minutes'])-($sub_load_min[$batch_id]);
										echo  ($row['process_end_date'] == '0000-00-00' || $row['process_end_date'] == '' ? '' : change_date_format($row['process_end_date'])).' &nbsp; '.$unload_time=$row['end_hours'].':'.$row['end_minutes']; ?></td>
                                    <td width="70">
                                    	<?     
										$new_date_time_unload=($unload_date.' '.$unload_time.':'.'00');
                ...
                <table class="rpt_table" width="<? echo $table_width;?>" cellpadding="0" cellspacing="0" border="1" rules="all">
                    <tfoot>
                        <tr>
                            ...
                            <th width="70"><? echo number_format($total_sub_batch_chemical_price+$tot_re_dying_dyes_chemical_cost+$tot_sub_re_dying_chemical_cost,4); ?></th>
                            <th width="">&nbsp;</th>
                        </tr>
                    </tfoot>
                </table>
                    ...
                    <table class="rpt_table" id="table_body3" width="<? echo $table_width;?>" cellpadding="0" cellspacing="0" border="1" rules="all">
                        <tbody>
                            ...
                            foreach($batch_based_data_arr as $batch_id => $row)
                            {
								...
								if ($batch_based_batch_weight=="") $batch_based_batch_weight=0;
								$water_cons_unload=$row['water_flow_meter'];
								$water_cons_load=$water_flow_arr[$batch_id];
								$load_hour_meter=$load_hour_meter_arr[$batch_id];
	                            ...
	                            <tr bgcolor="<? echo $bgcolor; ?>" id="tr3_<? echo $j; ?>" onClick="change_color('tr3_<? echo $j; ?>','<? echo $bgcolor; ?>')">
		                            ...
		                            <td width="90" align="right" title="Tot Chem + Dyes Cost (Tk)"><?$batch_based_batch_chemical_price=$batch_based_first_chemi_cost+$batch_based_first_dyeing_cost; 
		                            	...
		                            	$total_batch_based_batch_chemical_price+=$batch_based_batch_chemical_price;?>
		                            </td>
		                            <td width="90" align="right" title="Tot Chemical+Dyes Cost/Batch Weight"><?
		                            if ($batch_based_batch_weight!=0) 
		                            {
		                            	echo number_format($batch_based_batch_chemical_price/$batch_based_batch_weight,4,".","");
		                            }
		                            else
		                            {
		                            	echo '0.0000';
					                ...
					                ?></td>
		                            <td width="90" align="right" title="Tot Finishing Cost/Tot Batch Weight"><? 
		                            if ($batch_based_batch_weight!=0) 
		                            {
		                            	echo number_format($batch_based_chemical_cost_finish/$batch_based_batch_weight,4,".","");  $tot_batch_based_re_dying_dyes_cost+=$batch_based_chemical_cost_finish/$batch_based_batch_weight;
		                            }
		                            else
		                            {
		                            	echo '0.0000';
		                            ...
		                            <td width="70" align="right" title="Chemical Cost+Dyes Cost+Finishing Cost"><? echo number_format($batch_based_chemical_cost_finish+$batch_based_batch_chemical_price,4,".",""); 
		                            $tot_batch_based_chemical_cost_finish+=$batch_based_chemical_cost_finish;?></td>
								...
								$total_batch_based_batch_weight+=$batch_based_batch_weight;
		                    }
                            ?>
                        </tbody>
                    </table>
                ...
                <table class="rpt_table" width="<? echo $table_width;?>" cellpadding="0" cellspacing="0" border="1" rules="all">
                    <tfoot>
                        <tr>
                            ...
                            <th width="70"><? echo number_format($total_batch_based_batch_chemical_price+$tot_batch_based_re_dying_dyes_chemical_cost+$tot_batch_based_chemical_cost_finish,4); ?></th>
                            <th width="">&nbsp;</th>
                        </tr>
                    </tfoot>
                </table>
                        ...
                        <tbody>
                            ...
                            foreach($independent_based_data_arr as $batch_id => $row)
                            {
	                            ...
	                            <tr bgcolor="<? echo $bgcolor; ?>" id="tr4_<? echo $j; ?>" onClick="change_color('tr4_<? echo $j; ?>','<? echo $bgcolor; ?>')">
		                            ...
		                            <td width="70" align="right" title="Chemical Cost+Dyes Cost+Finishing Cost"><? echo number_format($independent_based_chemical_cost_finish+$independent_based_batch_chemical_price,4,".",""); ?></td>
		                            <td width=""></td>
                                </tr>
                                <?
                                $j++;
                ...
                <table class="rpt_table" width="<? echo $table_width;?>" cellpadding="0" cellspacing="0" border="1" rules="all">
                    <tfoot>
                        <tr>
                            ...
                            <th width="90"></th>
                            <th width="70"><? echo number_format($total_independent_based_batch_chemical_price+$tot_independent_based_re_dying_dyes_chemical_cost+$tot_independent_based_re_dying_chemical_cost,4); ?></th>
                            <th width="">&nbsp;</th>
                        </tr>
                    </tfoot>
                </table>
                        ...
                        <tbody>
                            ...
                            foreach($machine_wash_data_arr as $batch_id => $row)
                            {
	                            ...
	                            <tr bgcolor="<? echo $bgcolor; ?>" id="tr5_<? echo $j; ?>" onClick="change_color('tr5_<? echo $j; ?>','<? echo $bgcolor; ?>')">
					                ...
					                ?></td>
		                            <td width="90" align="right" title="Tot Finishing Cost"></td>
		                            <td width="70" align="right" title="Chemical Cost+Dyes Cost+Finishing Cost"><? echo number_format($machine_wash_chemical_cost_finish+$machine_wash_batch_chemical_price,4,".",""); ?></td>
                                    <td width=""></td>
                                </tr>
                                <?
                                $j++;
                ...
                <table class="rpt_table" width="<? echo $table_width;?>" cellpadding="0" cellspacing="0" border="1" rules="all">
                    <tfoot>
                        <tr>
                            ...
                            <th width="90"></th>
                            <th width="70"><? echo number_format($total_machine_wash_batch_chemical_price+$tot_machine_wash_re_dying_dyes_chemical_cost+$tot_machine_wash_re_dying_chemical_cost,4); ?></th>
                            <th width="">&nbsp;</th>
                        </tr>
                    </tfoot>
                </table>
