...
if($action=="report_generate")
{
    ...
    if($cbo_type)
    {
        if($cbo_type==67 || $cbo_type==68  || $cbo_type==35   || $cbo_type==194)$all_condition.=" and d.process_id like '%$cbo_type%'";
        else   $all_condition.=" and d.entry_form=$cbo_type";
    }
    if($cbo_company_name)$all_condition.=" and d.service_company in($cbo_company_name)";
    if($cbo_floor_id)$all_condition.=" and d.floor_id in($cbo_floor_id)";
    if($cbo_machine_id)$all_condition.=" and d.machine_id in($cbo_machine_id)";
    //if($cbo_buyer_name)$all_condition.=" and a.po_buyer in($cbo_buyer_name)";
    
    //if($cbo_year)$all_condition.=" and to_char(a.insert_date,'YYYY') =$cbo_year";
    if($cbo_year)$all_condition.=" and a.insert_date >= to_timestamp('".$cbo_year."-01-01', 'YYYY-MM-DD') and a.insert_date <= to_timestamp('".$cbo_year."-12-31', 'YYYY-MM-DD')";  
    
    if($fso_no)$all_condition.=" and a.job_no_prefix_num=$fso_no";
    if($cbo_source)$all_condition.=" and d.service_source=$cbo_source";
    if($batch_number)$all_condition.=" and c.id in($batch_number)";
    if($batch_number_show)$all_condition.=" and c.batch_no in('$batch_number_show')";
    if($booking_number_show)$all_condition.=" and a.job_no_prefix_num='$booking_number_show'";
    ...
    if($txt_date_from && $txt_date_to)
    {
        ...
        $batch_id_arr = return_library_array( "select batch_id,batch_id from  pro_fab_subprocess where status_active=1 and is_deleted=0 and process_end_date BETWEEN '$date_from' AND '$date_to' ",'batch_id','batch_id'); 
        $batch_id_cond = where_con_using_array($batch_id_arr,0,"d.batch_id");
...
/*	 $sql_batch="SELECT f.id as batch_dtls_id,f.prod_id,d.batch_ext_no as process_batch_ex,d.entry_form, c.id as batch_id,f.body_part_id,f.item_description as fabric_desc,d.batch_no,d.batch_ext_no,f.batch_qnty as batch_qty FROM pro_batch_create_mst c ,pro_batch_create_dtls f,fabric_sales_order_mst a,pro_fab_subprocess d Where   c.id=f.mst_id and a.id=c.sales_order_id and c.id=d.batch_id and a.SALES_BOOKING_NO=c.booking_no  and a.status_active=1 and c.status_active=1 and d.status_active=1 and d.entry_form in(32,47,30,31,48,33,34) $all_condition order by c.id asc "; 
    ...
    $main_query="SELECT f.id as batch_dtls_id,d.batch_ext_no as process_batch_ex,e.roll_id, d.previous_process, d.process_id,d.re_stenter_no AS re_process1,
     e.id as dtls_id,  a.booking_type, d.entry_form, c.id as batch_id,  a.job_no,a.within_group,a.sales_booking_no, booking_id,a.buyer_id,a.style_ref_no,a.season, a.po_buyer,a.po_job_no,a.booking_type, f.body_part_id, f.body_part_id  ,e.prod_id as determination_id,f.item_description as fabric_desc,e.gsm as gsm_weight,e.dia_width ,e.width_dia_type,b.color_id,b.status_active as deleted_status,b.color_range_id,d.batch_no,d.batch_ext_no, d.ltb_btb_id,d.water_flow_meter,d.production_date as process_end_date,d.temparature,d.stretch,d.over_feed,d.feed_in,d.pinning,d.speed_min,d.end_hours,d.end_minutes,d.machine_id,d.floor_id,d.load_unload_id,d.result,d.multi_batch_load_id,d.shift_name,process_end_date as production_date,d.remarks, d.chemical_name,d.length_shrinkage,d.width_shrinkage,d.spirality,d.process_start_date,d.start_minutes,d.start_hours,d.service_source,d.service_company,d.received_chalan,d.issue_chalan,d.issue_challan_mst_id,d.fabric_type,d.re_stenter_no,d.booking_no,d.hour_load_meter,d.hour_unload_meter,d.system_no,d.recv_number_prefix,d.recv_number_prefix_num,d.recv_number,d.receive_date, f.prod_id,f.batch_qnty as batch_qty, e.production_qty,e.roll_no, e.no_of_roll, d.advanced_prod_qty, d.result, d.steam FROM fabric_sales_order_mst a,fabric_sales_order_dtls b ,pro_batch_create_mst c ,pro_fab_subprocess d ,pro_fab_subprocess_dtls e,pro_batch_create_dtls f Where a.id=b.mst_id and b.color_id=c.color_id and a.id=c.sales_order_id      and c.id=d.batch_id and d.id=e.mst_id and c.id=f.mst_id and b.mst_id=f.po_id   and e.prod_id=f.prod_id  and a.status_active=1 and b.status_active in(1,0) and c.status_active=1 and d.status_active=1 and e.status_active=1 and d.entry_form in(32,47,30,31,48,33,34) $all_condition $batch_id_cond order by d.id asc ";

    //and a.SALES_BOOKING_NO=c.booking_no
	 
    //echo $main_query;die;



    $sql_data=sql_select($main_query); 
    /*echo "<pre>";
    print_r($sql_data);
    die;*/
    ...
    $main_array=array(); 
    $all_booking=array(); 
    $all_batch=array(); $chk_batch_qty=array();
    $summery='';
    $entry='';
    ...
    { 
        ...
        if($cbo_buyer_name)
        {
            if($row[csf("within_group")]==1 && $row[csf("po_buyer")]!=$cbo_buyer_name)continue;
            if($row[csf("within_group")]==2 && $row[csf("buyer_id")]!=$cbo_buyer_name)continue;
        }
		//$process_end_date=max($row[csf("process_end_date")],$row[csf("process_end_date")]);
		// if($row[csf("entry_form")]==30) echo $process_end_date.'D';
		
		$sales_booking_no=explode("-",$row[csf("sales_booking_no")]);
		$non_booking_no=$sales_booking_no[1];
	   //if($row[csf("entry_form")]==30) echo	$row[csf("entry_form")].'='.$row[csf("batch_qty")].'<br>';
        //SMN
        ...
        //if($row["re_stenter_no"]>0)$re_work_production_qty=$row[csf("production_qty")];
        ...
        if($fab_dlts_id_chk[$row[csf("batch_id")]][$row[csf("dtls_id")]]=="")
        {
            ...
            else if($row[csf("previous_process")]==3) {$chemical_qty=$row[csf("production_qty")];$ext_production_qty=0;}
            ...
            else
            {
                ...
                else 
                {
                    ...
                    if($row[csf("re_stenter_no")]==0 || $row[csf("re_stenter_no")]=="") 
                    {
            ...
            $summary_arr[$row[csf("entry_form")]]["no_of_roll"]+=1;
           
        }
       
        if(isset($row[csf("advanced_prod_qty")])){

           
          if($entry !==$row[csf("entry_form")]){
            $summery =$row[csf("entry_form")]."**".$row[csf("advanced_prod_qty")];
            $entry=$row[csf("entry_form")];
         
            $summary_arr[$row[csf("entry_form")]]["advanced_prod_qty"] +=$row[csf("advanced_prod_qty")];
            $dtls_arr_pro_qty[$row[csf("machine_id")]][$row[csf("shift_name")]]["advanced_prod_qty"] +=$row[csf("advanced_prod_qty")];
          }
        ...
        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["deleted_status"]=$row[csf("deleted_status")];
		
		$main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["remarks"]=$row[csf("remarks")];
		
		  $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["prod_id"]=$row[csf("prod_id")];
        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["po_job_no"]=$row[csf("po_job_no")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["re_stenter_no"]=$row[csf("re_stenter_no")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["previous_process"]=$row[csf("previous_process")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["process_id"]=$row[csf("process_id")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["re_process1"]=$row[csf("re_process1")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["re_process2"]=$row[csf("re_process2")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["booking_type"]=$row[csf("booking_type")];


        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["width_shrinkage"]=$row[csf("width_shrinkage")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["chemical_name"]=$row[csf("chemical_name")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["length_shrinkage"]=$row[csf("length_shrinkage")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["stretch"]=$row[csf("stretch")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["over_feed"]=$row[csf("over_feed")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["feed_in"]=$row[csf("feed_in")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["pinning"]=$row[csf("pinning")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["speed_min"]=$row[csf("speed_min")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["no_of_roll"]=$row[csf("no_of_roll")];
        



        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["process_end_date"]=$row[csf("process_end_date")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["process_end_time"]=$row[csf("end_hours")].':'.$row[csf("end_minutes")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["process_start_date"]=$row[csf("process_start_date")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["process_start_time"]=$row[csf("start_hours")].':'.$row[csf("start_minutes")];


        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["batch_no"]=$row[csf("batch_no")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["temparature"]=$row[csf("temparature")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["sales_booking_no"]=$row[csf("sales_booking_no")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["style_ref_no"]=$row[csf("style_ref_no")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["season"]=$row[csf("season")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["within_group"]=$row[csf("within_group")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["po_buyer"]=$row[csf("po_buyer")];
       // $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["po_job_no"]=$row[csf("po_job_no")];
        $comp_arr=explode(",",$row[csf("fabric_desc")]);
        
        if($main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["fabric_desc"]=="")
        {
            $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["fabric_type"].=$comp_arr[0];
            $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["fabric_desc"].=$comp_arr[1];
            $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["body_part_id"].=$body_part_arr[$row[csf("body_part_id")]];
            $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["gsm_weight"].=$row[csf("gsm_weight")];

            $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["width_dia_type"].=$fabric_typee[$row[csf("width_dia_type")]];

            $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["dia"].=$row[csf("dia_width")];



            $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["advanced_prod_qty"]=$row[csf("advanced_prod_qty")];
            $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["steam"]=$row[csf("steam")];
            $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["result"]=$row[csf("result")];
        ...
        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["floor_id"]=$row[csf("floor_id")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["buyer_id"]=$row[csf("buyer_id")];

        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["shift_name"]=$row[csf("shift_name")];
        $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["color_range_id"]=$row[csf("color_range_id")];
        if(! $qnty_chk_arr[$row[csf("dtls_id")]])
        {   
            // $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["batch_qty"]+=$row[csf("batch_qty")];

            $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["production_qty"]+=$row[csf("production_qty")];
        ...
        else if(! $qnty_chk_arr_ex_prod[$row[csf("batch_id")]][$row[csf("dtls_id")]]  &&  $row[csf("batch_ext_no")])
        {
           //$main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["batch_qty_ex"]+=$row[csf("batch_qty")];
            ...
            $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["production_qty_ex"]+=$row[csf("production_qty")];
            $qnty_chk_arr_ex_prod[$row[csf("batch_id")]][$row[csf("dtls_id")]]=$row[csf("dtls_id")]; 
        }

        if(! $roll_count[$row[csf("entry_form")]][$row[csf("production_date")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("body_part_id")]][$row[csf("re_stenter_no")]][$row[csf("roll_no")]][$row[csf("dtls_id")]] )
        {
            //echo $row[csf("batch_no")];
            $roll_count2[$row[csf("entry_form")]][change_date_format($row[csf("production_date")])][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("body_part_id")]]+=1;
            $roll_count[$row[csf("entry_form")]][$row[csf("production_date")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("body_part_id")]][$row[csf("re_stenter_no")]][$row[csf("roll_no")]][$row[csf("dtls_id")]]=420;
        }
	    ...
	    if($dup_chk_entry_arr2[$row[csf("entry_form")]][$row[csf("batch_dtls_id")]]=="")
        {
            ...
            $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["batch_qty"]+=$batch_qty;

            //echo $row[csf("entry_form")]."=".$row[csf("machine_id")]."=".$row[csf("job_no")]."=".$row[csf("style_ref_no")]."=".$row[csf("body_part_id")]."=".$row[csf("determination_id")]."=".$row[csf("color_id")]."=".$row[csf("batch_id")]."=".$row[csf("batch_ext_no")]."=".$row[csf("re_stenter_no")]."=".$row[csf("service_company")]."=".$row[csf("service_source")]."=".$row[csf("production_date")]."=".$batch_qty."<br/>";
		   
	  // $batch_chk_val=$row[csf("batch_dtls_id")];
		if(!in_array($qnty_chk_arr_batch[$batch_chk_val],$chk_batch_qty))
       {   
		 //echo  $row[csf("batch_qty")].'<br>';
		  // $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["batch_qty"]+=$row[csf("batch_qty")];
		   
		   //if($row[csf("entry_form")]==30) echo $row[csf("batch_qty")].',';
             
          // $chk_batch_qty[]=$batch_chk_val;
		   //$qnty_chk_arr_batch[$row[csf("batch_dtls_id")]]=$row[csf("batch_dtls_id")];
        }
		
          if(! $qnty_chk_arr_batch_ex[$row[csf("batch_dtls_id")]] &&  $row[csf("batch_ext_no")])
        {
         //echo "V";
		  // $main_array[$row[csf("entry_form")]][$row[csf("machine_id")]][$row[csf("job_no")]][$row[csf("style_ref_no")]][$row[csf("body_part_id")]][$row[csf("determination_id")]][$row[csf("color_id")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("service_company")]][$row[csf("service_source")]][$row[csf("production_date")]]["batch_qty_ex"]+=$row[csf("batch_qty")];
		   
           // $qnty_chk_arr_batch_ex[$row[csf("batch_dtls_id")]]=$row[csf("batch_dtls_id")]; 
        }
		  //$mc_qnty_arr_batch[$row[csf("entry_form")]][$row[csf("batch_id")]][$row[csf("machine_id")]][$row[csf("fabric_desc")]]['batch_qty']=$row[csf("batch_qty")];

    }
    unset($sql_data);
    // echo "<pre>";
    // print_r($summary_arr);die;
  
    $all_booking_nos="'".implode("','",$all_booking)."'";
	$all_booking_nos_non="'".implode("','",$all_booking_non)."'";
    $all_batch_id="'".implode("','",$all_batch)."'";

    $all_batch_id_cond = where_con_using_array($all_batch,0,"batch_id");
    $process_seq_sql="SELECT    batch_id,  process_id , machine_id  FROM pro_fab_subprocess Where  status_active=1 $all_batch_id_cond  order by  id asc ";
    //echo $process_seq_sql;die;
    ...
    $dyeing_sql="SELECT batch_id, batch_no, batch_ext_no,production_date as process_end_date,end_minutes,end_hours from pro_fab_subprocess where status_active=1 and   entry_form = 35 and load_unload_id = 2 $all_batch_id_cond" ;
...
//production_date
    $dyeing_sql_1="SELECT batch_id, batch_no, batch_ext_no,process_end_date,end_minutes,end_hours from pro_fab_subprocess where status_active=1 and   entry_form = 35 and load_unload_id = 1 $all_batch_id_cond" ;
    foreach(sql_select($dyeing_sql_1) as $row )
    {
       if(strtotime($row[csf("process_end_date")]))
	   {
	    $dyeing_data_1[$row[csf("batch_id")]][$row[csf("batch_ext_no")]]["date"]=$row[csf("process_end_date")];
	   }
        $dyeing_data_1[$row[csf("batch_id")]][$row[csf("batch_ext_no")]]["time"]=$row[csf("end_hours")].":".$row[csf("end_minutes")];
    }



    $booking_data_arr=array();
    $all_booking_nos_non_cond = where_con_using_array($all_booking_non,0,"booking_no");
    $non_booking_sql="SELECT buyer_id,booking_no,booking_type from wo_non_ord_samp_booking_mst where status_active=1 and   booking_type = 4 $all_booking_nos_non_cond" ;
    ...
    $all_booking_nos_cond = where_con_using_array($all_booking,1,"a.booking_no");	
    $booking_data="SELECT a.buyer_id, a.booking_no ,a.short_booking_type ,a.booking_type,a.is_short,b.division_id from wo_booking_mst a,wo_booking_dtls b where a.booking_no=b.booking_no and a.status_active=1 and b.status_active=1 $all_booking_nos_cond group by  a.buyer_id, a.booking_no ,a.short_booking_type ,a.booking_type,a.is_short,b.division_id ";
   ...
   // echo $main_query;
    ...
    if($operation==0)
    {
     ...
     ?>
          ...
          <style type="text/css">
            ...
            <fieldset style="width:4420px;">
                 
                <table class="rpt_table" width="4400" cellpadding="0" cellspacing="0" border="1" rules="all" align="left">
                    <thead>
                        <tr>
                            ...
                            <th width="80" class="alignment_css">Booking No</th> 
                            ...
                            <th width="80" class="alignment_css">FSO No</th> 
                            <th width="80" class="alignment_css">Batch No</th> 
                            <th width="80" class="alignment_css">Extn. No</th> 
                            <th width="80" class="alignment_css">Body Part</th> 
                            ...
                            <th width="80" class="alignment_css">Advance  <br>Prod. Qty</th> 
                            ...
                            <th width="80" class="alignment_css">Result</th> 
                            ...
                            <th width="80" class="alignment_css">Steam</th> 
                ...
                <div style=" max-height:380px; width:4420px; overflow-y:scroll;" id="scroll_body">
                    <table class="rpt_table" id="table_body" width="4400" cellpadding="0" cellspacing="0" border="1" rules="all" align="">
                     <tbody>
                    <?
                     $ii=1;   

                     foreach($main_array as $entry_form=>$machine_data)
                     {
                        ...
                        ?>
                        <tr>
                            <th colspan="55" align="left">
                                <strong><?
                                  if($entry_form==35 || $entry_form==67 || $entry_form==68   || $entry_form==194) {
                                        echo $conversion_cost_head_array[$entry_form];
                                   } 

                                   else 
                                   {
                                       echo $process_format[$entry_form];
                                   }
                                   ?>
                        ...
                        {  
                            ...
                            foreach($job_data as $job_no=>$style_data)
                            {
                                foreach($style_data as $style_id=>$body_part_data)
                                {

                                    foreach($body_part_data as $body_part_id=>$determination_data)
                                    {  
                                        foreach($determination_data as $determination_id=>$color_data)
                                        {
                                                        ...
                                                        foreach($re_stenter_no_data as $re_stenter_no=>$service_company_data)
                                                        {
                                                            foreach($service_company_data as $service_company=>$service_source_data)
                                                            {
                                                                foreach($service_source_data as $service_source=>$production_date_data)
                                                                {
                                                                    ...
                                                                    {
                                                                        //echo $body_part_id." 333 <br>";
                                                                        /*$serial=$process_seq_arr[$batch_id][$machine_id];
                                                                        $serial_arr=explode(",", $serial);

                                                                        $serial_arr_new=array();
                                                                        $serial_arr_final=array();
                                                                        foreach($serial_arr as $v)
                                                                        {
                                                                            if($row["process_id"]==$v) break;
                                                                            $serial_arr_new[$v]=$v;
                                                                        }

                                                                        foreach($serial_arr_new as $val)
                                                                        {
                                                                            if($val==67 || $val==68 || $val==69 )
                                                                            {
                                                                                if( $val==69)
                                                                                {
                                                                                    $serial_arr_final[67]=67;
                                                                                    $serial_arr_final[68]=68;
                                                                                }
                                                                                else
                                                                                {
                                                                                      $serial_arr_final[$val]=$val;
                                                                                }
                                                                        ...
                                                                        //print_r($serial_arr_final);
                                                                        //$dynamic_process=end($serial_arr_final);
                                                                        $batch_qty=0;
                                                                        $production_qty=0;
                                                                        $production_qty_ex=0;
                                                                        $re_work_production_qty=0;
                                                                        ...
                                                                        if($row["previous_process"]==1)
                                                                        {
                                                                           ...
                                                                           $advanced_prod_qty=$row["advanced_prod_qty"];
                                                                           $production_qty=0;
                                                                           $re_work_production_qty=0;
                                                                           $production_qty_ex=0;
                                                                        }
                                                                        ...
                                                                        $entry_form_advanced_prod_qty +=$advanced_prod_qty;

                                                                        if($batch_qty || $brush_qty || $peach_qty || $chemical_qty || $advanced_prod_qty || $aop_qty || $production_qty ||  $batch_qty_ex ||  $production_qty_ex || $re_work_production_qty || 1==1  )
                                                                        {
                                                                            if ($ii%2==0)  
                                                                                $bgcolor="#E9F3FF";
                                                                            else
																			...
																			if($deleted_status==0)
																			{
																				...
																				$deleted_colorMsg="Deleted Found";
																			}
																			else { $deleted_color="";
																			$deleted_colorMsg="";
																			}
                                                                            ...
                                                                            <tr bgcolor="<? echo $bgcolor; ?>"   onClick="change_color('tr_<? echo $ii; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $ii; ?>">
                                                                             ...
                                                                             <td width="80" class="alignment_css"><? if($row["within_group"]==1) echo $buyer_arr[$booking_data_arr[$row["sales_booking_no"]]["buyer_id"]];  else echo $buyer_arr[$row["buyer_id"]];?></td>
                                                                             <td width="80" class="alignment_css"><? echo $row["style_ref_no"];?></td>
                                                                             <td width="80" class="alignment_css"><? if($row["within_group"]==1) echo $row["po_job_no"]; //else echo $job_no;?></td>
                                                                             <td width="80" class="alignment_css"><? echo $row["season"];?> </td>
                                                                             <td width="80" class="alignment_css"><? echo $row["sales_booking_no"];?></td>
                                                                             <td width="80" class="alignment_css"><? echo $booking_data_arr[$row["sales_booking_no"]]["type"];?></td>
                                                                             <td width="80" class="alignment_css"><? echo $booking_data_arr[$row["sales_booking_no"]]["short_booking_type"];?> </td>
                                                                             <td width="80" class="alignment_css"><? echo $booking_data_arr[$row["sales_booking_no"]]["division"];?></td>
                                                                             <td width="80" class="alignment_css"><? echo $job_no;?></td>
                                                                             <td width="80" title="<? echo $deleted_colorMsg;?>" class="alignment_css"><p style="color:<? echo $deleted_color; ?>"><? echo $row["batch_no"];?></p></td>
                                                                             <td width="80" class="alignment_css"><? echo $ext_no;?></td>
                                                                             <td width="80" class="alignment_css"><? echo implode(",",array_unique( explode(",", $row["body_part_id"])));?></td>
                                                                             <td width="80" class="alignment_css"><?
                                                                                 ...
                                                                                 <td width="80" class="alignment_css"><? echo number_format($aop_qty,2); ?></td>
                                                                                    ...
                                                                                    <?
                                                                                    if($s_new!==$service_source){?>
                                                                                     <td width="80" class="alignment_css"><? echo $row["advanced_prod_qty"]; ?></td>
                                                                                     <?
                                                                                        $s_new=$service_source;
                                                                                    }else{?>
                                                                                 <td width="80" class="alignment_css"><? echo ''; ?></td>
                                                                                 <?}?>
                                                                             
                                                                                 <td width="80" class="alignment_css"><?  echo $row["re_process1"];    ?></td>
                                                                                 <td width="80" class="alignment_css"><? echo $roll_count2[$entry_form][change_date_format($production_date)][$batch_id][$ext_no][$row["re_stenter_no"]][$body_part_id];?> </td>
                                                                                 ...
                                                                                 <td width="80" class="alignment_css"> 
                                                                                     ...
                                                                                     $dyeing_date_1=$dyeing_data_1[$batch_id][$ext_no]["date"];
                                                                                     $dyeing_time_1=$dyeing_data_1[$batch_id][$ext_no]["time"];
                                                                                    ...
                                                                                    $new_date_time_end1=$dyeing_date_1.' '.$dyeing_time_1.':'.'00';
                                                                                    ...
                                                                                    //  $total_time_ex=datediff(n,$new_date_time_end2 ,$new_date_time_start);
                                                                                     $total_time_ex=datediff(n,$new_date_time_end1 ,$new_date_time_end2);
																					 $days_remian=datediff('d',$new_date_time_end1,$new_date_time_end2)-1;
                                                                                    echo  abs ( floor(abs($total_time/60)))." H :".abs($total_time%60)." M ";  
                                                                                    

                                                                                    // echo $new_date_time_end2.'='.$new_date_time_end1.',';
                                                                                   /*if($dyeing_date && $dyeing_time )
                                                                                   {

                                                                                   
                                                                                        $datetime1= new DateTime($row["process_start_date"]." ".$row["process_start_time"].":00");
                                                                                        $datetime2= new DateTime($dyeing_date." ".$dyeing_time.":00");
                                                                                        //$datetime2 = new DateTime('2011-01-03 17:13:00');
                                                                                        $interval = $datetime1->diff($datetime2);
                                                                                       // echo $interval->format(' %h h %i m');
                                                                                        $time_used=$interval->format(' %a ');
                                                                                    }*/
                                                                                    $exe_days=   round($total_time_ex/60/24);
                                                                                    ...
                                                                                    <td width="80" class="alignment_css"><? echo  $days_remian;//abs($exe_days);?></td>
                                                                                    <td width="80" class="alignment_css" title="<? echo $new_date_time_end2.'='.$new_date_time_end1;?>"><?  echo  abs ( floor( abs($total_time_ex/60)) )." H :".abs($total_time_ex%60)." M ";  ?></td>
                                                                                    <td width="80" class="alignment_css"><? echo $dyeing_result[$row["result"]];  ?></td>
                                                                                    ...
                                                                                    <td width="80" class="alignment_css"><?if($entry_form==33){ echo $row["steam"];}?></td>
                                ...
                                ?>
                                    <tr style="background-color:whitesmoke; " onClick="change_color('tr2_<? echo $ii; ?>','<? echo $bgcolor; ?>')" id="tr2_<? echo $ii; ?>" style="font-size:12px; cursor:pointer;" >
                                     <td width="2080" colspan="26" class="alignment_css" align="right"><strong><? echo  $machine_arr[$machine_id];?> Total=</strong></td>
                                     ...
                                     <td colspan="24" width="1680" class="alignment_css"> </td>
                             ...
                             ?>
                                    <tr style="background-color:whitesmoke; " onClick="change_color('tr3_<? echo $ii; ?>','<? echo $bgcolor; ?>')" id="tr3_<? echo $ii; ?>" style="font-size:12px; cursor:pointer;" >
                                     <td width="2080" colspan="26" class="alignment_css" align="right"><strong><? echo  $process_format[$entry_form];?> Total=</strong></td>
    ...
    else if($operation==1)
    {
        ...
        $dtls_width=230+(count($head_arr)*100);
        $dtls_width2=270+(count($head_arr)*100);
        ...
        <br> <br>
                ...
                <table class="rpt_table" width="1200" cellpadding="0" cellspacing="0" border="1" rules="all" align="left">
                    <thead>
                        <tr>
                            ...
                            <th width="100" class="alignment_css">Advance <br>Production Qty</th>  
                ...
                <div style=" max-height:380px; width:1220px; overflow-y:scroll;" id="scroll_body">
                    <table class="rpt_table" id="table_body" width="1200" cellpadding="0" cellspacing="0" border="1" rules="all" align="">
                        <tbody>
                            ...
                            foreach($head_arr as $entry_form=>$row)
                            {
                                ...
                                <tr>
                                    ...
                                    <td width="100" align="center"><? echo $batch_qty=number_format($summary_arr[$entry_form]["batch_qty"],2);?></td>
                                    <td width="100" align="center"><? echo $production_qty=number_format($summary_arr[$entry_form]["production_qty"],2);?></td>
                                    <td width="100" align="center"><? echo $ext_production_qty=number_format($summary_arr[$entry_form]["ext_production_qty"],2);?></td>
                                    <td width="100" align="center"><? echo $re_work_production_qty=number_format($summary_arr[$entry_form]["re_work_production_qty"],2);?></td>
                                    <td width="100" align="center"><? echo $brush_qty=number_format($summary_arr[$entry_form]["brush_qty"],2);?></td>
                                    <td width="100" align="center"><? echo $peach_qty=number_format($summary_arr[$entry_form]["peach_qty"],2);?></td>
                                    <td width="100" align="center"><? echo $aop_qty=number_format($summary_arr[$entry_form]["aop_qty"],2);?></td>
                                    <td width="100" align="center"><? echo $chemical_qty=number_format($summary_arr[$entry_form]["chemical_qty"],2);?></td>
                                    <td width="100" align="center"><? echo  $advanced_prod_qty=number_format($summary_arr[$entry_form]["advanced_prod_qty"],2);?></td>
                                    <td width="100" align="center"><? echo $summary_arr[$entry_form]["no_of_batch"] ;?></td>
                                    <td width="100" align="center"><? echo $summary_arr[$entry_form]["no_of_roll"] ;?></td>
                                ...
                                $gr_advanced_prod_qty+=str_replace(",","",$advanced_prod_qty);
                            ...
                            <tr>
                                    ...
                                    <td width="100" align="center"><? echo number_format($gr_advanced_prod_qty,2);?></td>
            ...
            <fieldset style=" float: left; width:<? echo $dtls_width2;?>px;">
                ...
                <table class="rpt_table" width="<? echo $dtls_width ;?>" cellpadding="0" cellspacing="0" border="1" rules="all" align="left">
                    <thead>
                        <tr>
                            <th width="160" class="alignment_css">M/C</th> 
                            <th width="60" class="alignment_css">Shift</th> 
                            <?
                                foreach($head_arr as $entry_form=>$vv )

                                {
                                    if($entry_form==35 || $entry_form==67 || $entry_form==68   || $entry_form==194)
                                    {
                                        ...
                                        echo "<th width='100' class='alignment_css'>$conversion_cost_head_array[$entry_form]</th> ";
                                   } 

                                   else 
                                   {
                                    echo "<th width='100' class='alignment_css'>$process_format[$entry_form]</th> ";
                            ...
                            <th width="200" class="alignment_css">Advance Prod. Qty</th> 

                        </tr>
                    </thead>
                </table>
                <div style=" max-height:500px; width:<? echo $dtls_width2;?>px; overflow-y:scroll;" id="scroll_body_3">
                    <table class="rpt_table" id="table_body_5" width="<? echo $dtls_width;?>" cellpadding="0" cellspacing="0" border="1" rules="all" align="left">
                        <tbody>
                            <?
                            ...
                            foreach($dtls_arr_head as $machine_id=>$shift_data)
                            {
                                foreach($shift_data as $shift_id=>$row)
                                {
                                    ...
                                    <tr>
                                        ...
                                        {
                                            $vals=$dtls_arr_qnty[$machine_id][$shift_id][$entry_form];
                                            ?>
                                            <td width="100" align="center"><? echo   number_format($vals,2);?></td> 

                                            <?
                                            $process_total_arr[$entry_form]+=$vals;
                                        }


                                        
                                        ?>
                                      <td width="200" align="center"><?echo  number_format($dtls_arr_pro_qty[$machine_id][$shift_id]["advanced_prod_qty"],2);?></td> 

                                    </tr>

                                    <?
                                     $adqty_sum += $dtls_arr_pro_qty[$machine_id][$shift_id]["advanced_prod_qty"];
                                }
                            }
                            ...
                            <tr>
                                        ...
                                        { ?>
                                            <td width="100" align="center"><? echo  number_format($process_total_arr[$entry_form],2);?></td> 

                                            <?
                                           
                                        }
                                        ?>
                                        <td width="200" align="center"><?= number_format($adqty_sum,2);?> </td>
