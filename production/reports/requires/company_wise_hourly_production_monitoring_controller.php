...
if ($action=="load_drop_down_location")
{
	...
	order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down( 'requires/company_wise_hourly_production_monitoring_controller', this.value, 'load_drop_down_floor', 'floor_td' );get_php_form_data( this.value, 'eval_multi_select', 'requires/company_wise_hourly_production_monitoring_controller' );",0 ); 
	exit();    	 
}
...
if($action=="report_generate") //2nd Button Start...
{
	...
	else $buyer_id_cond=" and b.buyer_name=$cbo_buyer_name";
	...
	if($poIds_cond!="")	
	{
		...
		// echo $sql_item_rate."==".$poIds_cond;die();
	...
	if($db_type==0)
	{
		$sql_sub_contuct= "SELECT  a.company_id, a.location_id, a.floor_id, a.production_date, a.line_id, b.subcon_job, b.party_id  as buyer_name, a.order_id, c.order_no as po_number, c.cust_style_ref, a.gmts_item_id, max(c.smv) as smv, c.rate,
	...
	foreach($sub_result as $subcon_val)
	{
		...
		$fob_rate_data_arr[$subcon_val[csf('floor_id')]][$subcon_val[csf('line_id')]][$subcon_val[csf('order_id')]]['rate']=$subcon_val[csf('rate')]; 
		
		if($production_data_arr[$subcon_val[csf('floor_id')]][$subcon_val[csf('line_id')]]['item_number_id']!="")
		{
			$production_data_arr[$subcon_val[csf('floor_id')]][$subcon_val[csf('line_id')]]['item_number_id'].="****".$subcon_val[csf('order_id')]."**".$subcon_val[csf('gmts_item_id')]."**".$subcon_val[csf('subcon_job')]; 
		}
		else
		{
			 $production_data_arr[$subcon_val[csf('floor_id')]][$subcon_val[csf('line_id')]]['item_number_id']=$subcon_val[csf('order_id')]."**".$subcon_val[csf('gmts_item_id')]."**".$subcon_val[csf('subcon_job')]; 
		...
		$production_data_arr[$subcon_val[csf('floor_id')]][$subcon_val[csf('line_id')]]['prod_hour23']+=$subcon_val[csf('prod_hour23')];
		$production_data_arr_qty[$subcon_val[csf('floor_id')]][$subcon_val[csf('line_id')]][$subcon_val[csf('order_id')]][$subcon_val[csf('gmts_item_id')]]['quantity']+=$subcon_val[csf('good_qnty')];
		
		if($all_subpo_id=="") $all_subpo_id=$subcon_val[csf('order_id')]; else $all_subpo_id.=",".$subcon_val[csf('order_id')];
	}
	
	$subpo_ids=count(array_unique(explode(",",$all_subpo_id)));
	$subpo_numIds=chop($all_subpo_id,','); $subpoIds_cond="";
	if($all_subpo_id!='' || $all_subpo_id!=0)
	{
		if($db_type==2 && $subpo_ids>1000)
		{
			$subpoIds_cond=" and (";
			$subpoIdsArr=array_chunk(explode(",",$subpo_numIds),990);
			foreach($subpoIdsArr as $subids)
			{
				$subids=implode(",",$subids);
				$subpoIds_cond.=" b.id in ($subids) or ";
			}
			$subpoIds_cond=chop($subpoIds_cond,'or ');
			$subpoIds_cond.=")";
		}
		else
		{
			$subpoIds_cond=" and b.id in ($all_subpo_id)";
		}
	}

	if($subpoIds_cond!="")
	{
		$sql_subitem_rate="SELECT b.id, c.item_id, c.qnty, c.amount from subcon_ord_dtls b, subcon_ord_breakdown c where b.id=c.order_id and b.job_no_mst=c.job_no_mst and  b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 $subpoIds_cond";
		$subresultRate=sql_select($sql_subitem_rate);
		
		foreach($subresultRate as $row)
		{
			$item_po_array[$row[csf('id')]][$row[csf('item_id')]]['qty']+=$row[csf('qnty')];
			$item_po_array[$row[csf('id')]][$row[csf('item_id')]]['amt']+=$row[csf('amount')];
		}
	}
	
	$ressub_input_output=sql_select("SELECT a.company_id, a.location_id, a.floor_id, a.line_id, a.order_id, a.production_type, a.production_date, a.production_qnty from subcon_gmts_prod_dtls a where a.production_type in (2,7) and a.order_id in($all_subpo_id) and a.status_active=1 and a.is_deleted=0"); 
		
	foreach($ressub_input_output as $i_val)
	{
		if($i_val[csf('production_type')]==7)
		{
			if($input_output_po_arr[$i_val[csf('company_id')]][$i_val[csf('floor_id')]][$i_val[csf('line_id')]][$i_val[csf('order_id')]]['input_date']!='')
			{
				if(strtotime($input_output_po_arr[$i_val[csf('company_id')]][$i_val[csf('floor_id')]][$i_val[csf('line_id')]][$i_val[csf('order_id')]]['input_date'])>strtotime($i_val[csf('production_date')]))
				{
					$input_output_po_arr[$i_val[csf('company_id')]][$i_val[csf('floor_id')]][$i_val[csf('line_id')]][$i_val[csf('order_id')]]['input_date']=$i_val[csf('production_date')];
				}
			}
			else
			{
			...
			$input_output_po_arr[$i_val[csf('company_id')]][$i_val[csf('floor_id')]][$i_val[csf('line_id')]][$i_val[csf('order_id')]]['input']+=$i_val[csf('production_qnty')];
			
			if(change_date_format($i_val[csf('production_date')])==$search_prod_date)
			{
				$input_po_arr[$i_val[csf('company_id')]][$i_val[csf('floor_id')]][$i_val[csf('line_id')]][$i_val[csf('order_id')]][change_date_format($i_val[csf('production_date')])]+=$i_val[csf('production_qnty')];
			}
		}
		else
		{
			$input_output_po_arr[$i_val[csf('company_id')]][$i_val[csf('floor_id')]][$i_val[csf('line_id')]][$i_val[csf('order_id')]]['output']+=$i_val[csf('production_qnty')];
		}
	}
	...
	if ($type == 0) // show button
	{
		...
		foreach($prod_resource_array as $company_id=>$com_name)
		{
			...
			$html.='<tr  bgcolor="#E8FFFF">
						<td width="" colspan="33"><strong>Company Name:'.$companyArr[$company_id].'</strong></td>';
						for($k=$hour; $k<=$last_hour; $k++)
						{
			...
			$floor_html.='<tr  bgcolor="#CFCFA0">
						<td colspan="19"><strong>Company Name:'.$companyArr[$company_id].'</strong></td>';
			...
			foreach($com_name as $lo_id=>$lo_name)
			{
				...
				foreach($lo_name as $f_id=>$fname)
				{
					foreach($fname as $resource_id=>$resource_data)
					{
						...
						foreach($germents_item as $g_val)
						{
							...
							if($floor_line_chk[$f_id][$resource_id]=="")
							{
								$today_input+=$production_data_arr[$f_id][$resource_id]['input_qnty'];
								// echo $production_data_arr[$f_id][$resource_id]['input_qnty']."<br>";
								$floor_line_chk[$f_id][$resource_id] = $resource_id;
							}
							...
							$fob_rate.'='.$item_po_array[$po_garment_item[0]][$po_garment_item[1]]['amt'].'/'.$item_po_array[$po_garment_item[0]][$po_garment_item[1]]['qty'].'<br>';
						...
						if(is_nan($line_cm_value)){ $line_cm_value=0; }
				...
				//********************************************************************************************************
					 ...
					 //***********************************************************************************************			  
						...
						if($cbo_no_prod_type==2 && $line_production_hour>0)
						{
						   ...
						   //********************************* calclution floor total    ****************************************************$pr_date],$sewing_day
							...
							$html.='<td width="40">'.$i.'&nbsp;</td>
									...
									<td align="right" width="70">'.$capacity.'</td>
									...
									<td align="right" width="80">'.change_date_format($min_input_date).'</td>
									...
									<td width="100" align="right">'.number_format($efficiency_min,2).'</td>
									...
									$html.='<td align="right" width="70">'.$resource_data['machine'].'</td>
						...
						if($cbo_no_prod_type==1) // with subcon production
						{
							$current_wo_time=0;
							if($current_date==$search_prod_date)
							{
						   ...
						   //********************************* calclution floor total    ****************************************************$pr_date],$sewing_day
							...
							$html.='<td width="40">'.$i.'&nbsp;</td>
									<td width="80">'.$floor_name.'&nbsp; </td>
									<td align="center" width="80" title="'.$resource_id.'">'. $sewing_line.'&nbsp; </td>
									<td width="80"><p>'.$buyer_name.'&nbsp;</p></td>
									<td width="140"><p>'.$production_data_arr[$f_id][$resource_id]['po_number'].'&nbsp;</p></td>
									<td width="100"><p>'.$style_button.'&nbsp;</p></td>
									<td width="120" style="word-wrap:break-word; word-break: break-all;">'.$garment_itemname.'</td>
					...
					if($cbo_no_prod_type==2 && $line_floor_production>0)
					{
						$html.='<tr  bgcolor="#B6B6B6">
								...
								<td align="right" width="70">'.$floor_total_machine.'</td>
							...
							$floor_html.='<td align="right" width="80">'. $floor_total_input.'</td>
										<td align="right" width="80">'. $floor_total_output.'</td>
										<td align="right" width="80">'. $floor_total_wip.'</td>
										...
										for($k=$hour; $k<=$last_hour; $k++)
										{
											$prod_hour="prod_hour".substr($start_hour_arr[$k],0,2)."";
											if($k==$last_hour) $hourwidth=''; else $hourwidth="50";
											if($start_hour_arr[$k]==$global_start_lanch) $styleBcolor='style=" background-color:#FFFF66"'; else $styleBcolor='';
											$floor_html.='<td align="right" width='.$hourwidth.' '.$styleBcolor.' >'. $floor_production[$prod_hour].'</td>';
										}	
			...
			$html.='<tr class="tbl_bottom">
					...
					<td align="right" width="60"></td>
					<td align="right" width="70">'.$company_capacity.'</td>
					...
					<td align="right" width="100">'. number_format($company_avable_min,2).'</td>
					...
					<td align="right" width="70">'.$company_total_machine.'</td>
			...
			$floor_html.='<tr class="tbl_bottom">
						<td width="120" colspan="2">Company Total</td>
						<td width="70" align="right">'. $company_total_tgt_h.'</td>
						<td width="70" align="right">'.$company_capacity.'</td>
			...
			$floor_html.='<td align="right" width="80">'. $company_total_input.'</td>
						...
						for($k=$hour; $k<=$last_hour; $k++)
						{
							...
							if($k==$last_hour) $hourwidth=''; else $hourwidth="50";
							if($start_hour_arr[$k]==$global_start_lanch) $styleBcolor='style=" background-color:#FFFF66"'; else $styleBcolor='';
							$floor_html.='<td align="right" width='.$hourwidth.' '.$styleBcolor.' >'. $company_production[$prod_hour].'</td>';
						}
			$floor_html.='</tr>';	
		...
		<fieldset style="width:2530px">
	       <table width="2200" cellpadding="0" cellspacing="0"> 
	        ...
	        <label> <strong>Report Sumarry:-</strong></label> 
	          <table id="table_header_2" class="rpt_table" width="<?=$ftbwitdh+1440; ?>px" cellpadding="0" cellspacing="0" border="1" rules="all">
	            <thead>
	                <tr height="50">
	                    <th width="40">SL</th>
	                	...
	                	for($k=$hour; $k<=$last_hour; $k++)
						{
							if($k==$last_hour) $hourwidth=''; else $hourwidth='50';
						?>
	                    	<th width="<?=$hourwidth; ?>" style="vertical-align:middle"><div class="block_div"><?=substr($start_hour_arr[$k],0,5); ?></div></th>
						<?	
						}
	                	?>
	                </tr>
	        ...
	        <!--</table>
	        <div style="width:<?//=$ftbwitdh+1458; ?>px; max-height:400px; overflow-y:scroll" id="scroll_body">
	           <table class="rpt_table" width="<?//=$ftbwitdh+1440; ?>" cellpadding="0" cellspacing="0" border="1" rules="all" id="">-->
	            ...
	            <tfoot>
	                   <tr>
							...
							for($k=$hour; $k<=$last_hour; $k++)
							{
								...
								if($k==$last_hour) $hourwidth=''; else $hourwidth='50';
							?>
							  <th width="<?=$hourwidth; ?>" ><?=$total_production[$prod_hour];   ?></th>
							<?	
							}
	                		?>
	                   </tr>
	        ...
	        <table id="table_header_1" class="rpt_table" width="3650" cellpadding="0" cellspacing="0" border="1" rules="all">
	            <thead>
	                <tr height="50">
	                    <th width="40">SL</th>
	                    <th width="80">Floor Name</th>
	        ...
	        <div style="width:3650; max-height:400px; overflow-y:scroll" id="scroll_body">
	         		...
	         		foreach($prod_resource_array as $company_id=>$com_name)
					{
						...
						<tr bgcolor="#E8FFFF">
							...
							for($k=$hour; $k<=$last_hour; $k++)
							{
								...
								<td align="right" width=<? echo $hourwidth; ?> style=" <?=$bg_color; ?>"></td>
							<?	
							}
							?>									
						</tr>
    ...
    <fieldset style="width:1000px; ">
		...
		<div id="report_container" align="center">
			<table border="1" class="rpt_table" rules="all" width="500" cellpadding="0" cellspacing="0" align="center">
				...
				$sql_pop="select  c.po_number, a.po_break_down_id, sum(a.production_quantity) as good_qnty from pro_garments_production_mst a, wo_po_break_down c where a.production_type=4 and a.po_break_down_id=c.id and a.status_active=1 and a.is_deleted=0 and a.serving_company=".$company_id." and a.po_break_down_id in(".$po_id.")  and a.sewing_line=".$sewing_line." and a.floor_id=".$floor." group by c.po_number, a.po_break_down_id  
				union all 
				select c.order_no as po_number, a.order_id as po_break_down_id, sum(a.production_qnty) as good_qnty from subcon_gmts_prod_dtls a, subcon_ord_dtls c where a.production_type=7 and a.order_id=c.id and a.status_active=1 and a.is_deleted=0 and a.company_id=".$company_id." and a.order_id in(".$po_id.")  and a.line_id=".$sewing_line." and a.floor_id=".$floor." group by c.order_no, a.order_id order by  po_number";//and a.production_date='".$production_date."'
			...
			//echo $sql_pop;die;	
                ...
                <?
				$sql_color_size="select  c.color_number_id,c.size_number_id ,a.po_break_down_id, sum(b.production_qnty) as good_qnty,d.po_number from pro_garments_production_mst a, pro_garments_production_dtls b, wo_po_color_size_breakdown c,wo_po_break_down d where a.id=b.mst_id and b.production_type=4 and a.production_type=4 and a.po_break_down_id=c.po_break_down_id and b.color_size_break_down_id=c.id and a.status_active=1 and a.is_deleted=0 and a.serving_company=".$company_id." and a.po_break_down_id in(".$po_id.")  and a.sewing_line=".$sewing_line." and a.floor_id=".$floor." and d.id=c.po_break_down_id group by c.color_number_id, c.size_number_id, a.po_break_down_id,d.po_number  
				union all 
				select  c.color_id as color_number_id, c.size_id as size_number_id, a.order_id as po_break_down_id, sum(b.prod_qnty) as good_qnty,d.order_no as po_number from subcon_gmts_prod_dtls a, subcon_gmts_prod_col_sz b, subcon_ord_breakdown c,subcon_ord_dtls d where a.id=b.dtls_id and b.production_type=7 and a.production_type=7 and a.order_id=c.order_id and b.ord_color_size_id=c.id and a.status_active=1 and a.is_deleted=0 and a.company_id=".$company_id." and a.order_id in(".$po_id.") and a.line_id=".$sewing_line." and a.floor_id=".$floor." and d.id=c.order_id  group by c.color_id, c.size_id, a.order_id,d.order_no order by po_break_down_id";// and a.production_date='".$production_date."'
			...
			//echo $sql_color_size;die;	
				...
				foreach($sql_color_size_result as $cs_val)
				{
					...
					$color_size_qty_arr[$cs_val[csf('po_break_down_id')]][$cs_val[csf('color_number_id')]]['po_number']=$cs_val[csf('po_number')];
					...
					foreach($po_value as $color_id=>$color_value)
					{
					  	...
					  	?>
                         	<tr style="font:'Arial Narrow';" align="center" bgcolor="<? echo $bgcolor;?>" onclick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $k; ?>">
                                ...
                                <td width="180" style="word-wrap:break-word; word-break: break-all; text-align:center" ><? echo $color_value['po_number']; ?></td>
    ...
    <fieldset style="width:1000px; ">
		...
		<div id="report_container" align="center">
			<table border="1" class="rpt_table" rules="all" width="500" cellpadding="0" cellspacing="0" align="center">
				...
				union all 
				select  c.color_id as color_number_id, c.size_id as size_number_id, a.order_id as po_break_down_id, sum(b.prod_qnty) as good_qnty from subcon_gmts_prod_dtls a, subcon_gmts_prod_col_sz b, subcon_ord_breakdown c where a.id=b.dtls_id and b.production_type=2 and a.production_type=2 and a.order_id=c.order_id and b.ord_color_size_id=c.id and a.status_active=1 and a.is_deleted=0 and a.company_id=".$company_id." and a.order_id in(".$po_id.") and a.line_id=".$sewing_line." and a.floor_id=".$floor."  group by c.color_id, c.size_id, a.order_id order by po_break_down_id";
    ...
    <fieldset style="width:1000px; ">
		...
		<div id="report_container" align="center">
			<table border="1" class="rpt_table" rules="all" width="500" cellpadding="0" cellspacing="0" align="center">
				...
				union all
				select c.order_no as po_number,a.order_id as po_break_down_id, sum( case when a.production_type=7 THEN a.production_qnty ELSE 0 END)  as input_qty ,sum( case when a.production_type=2 THEN a.production_qnty ELSE 0 END)  as output_qty from subcon_gmts_prod_dtls a, subcon_ord_dtls c where a.order_id=c.id and a.status_active=1 and a.is_deleted=0 and a.company_id=".$company_id." and a.order_id in(".$po_id.")  and a.line_id=".$sewing_line." and a.floor_id=".$floor."  group by c.order_no,a.order_id
				order by  po_number ";
			//echo $sql_pop;die;
				$sql_result=sql_select($sql_pop);
				$k=1;$total_amount=0;$total_prod_qty=0;
				...
				$sql_color_size="select CAST(a.production_type AS int) AS production_type,c.color_number_id,c.size_number_id ,a.po_break_down_id, b.production_qnty  as good_qnty,d.po_number from pro_garments_production_mst a, pro_garments_production_dtls b, wo_po_color_size_breakdown c,wo_po_break_down d where a.id=b.mst_id and b.production_type in (4,5) and a.production_type in (4,5) and a.po_break_down_id=c.po_break_down_id and b.color_size_break_down_id=c.id and a.status_active=1 and a.is_deleted=0 and a.serving_company=".$company_id." and a.po_break_down_id in(".$po_id.")  and a.sewing_line=".$sewing_line." and a.floor_id=".$floor." and d.id=c.po_break_down_id 
				union all 
				select a.production_type, c.color_id as color_number_id, c.size_id as size_number_id, a.order_id as po_break_down_id, b.prod_qnty as good_qnty,d.order_no as po_number from subcon_gmts_prod_dtls a, subcon_gmts_prod_col_sz b, subcon_ord_breakdown c,subcon_ord_dtls d where a.id=b.dtls_id and b.production_type in (2,7) and a.production_type in (2,7) and a.order_id=c.order_id and b.ord_color_size_id=c.id and a.status_active=1 and a.is_deleted=0 and a.company_id=".$company_id." and a.order_id in(".$po_id.")  and a.line_id=".$sewing_line." and a.floor_id=".$floor." and d.id=c.order_id order by po_break_down_id ";
			...
			//echo $sql_color_size;	
				...
				foreach($sql_color_size_result as $cs_val)
				{
					if($cs_val[csf('production_type')]==7) $cs_val[csf('production_type')]=4;
					if($cs_val[csf('production_type')]==2) $cs_val[csf('production_type')]=5;
					$size_arr[$cs_val[csf('size_number_id')]]=$cs_val[csf('size_number_id')];
				...
				foreach($color_size_qty_arr as $po_id=>$po_value)
				{
					...
					foreach($po_value as $color_id=>$color_value)
					{
					  	...
					  	?>
                         	<tr style="font:'Arial Narrow';" align="center" bgcolor="<? echo $bgcolor;?>" onclick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $k; ?>">
                                <td width="30" rowspan="<?php // echo $order_color_arr[$po_id]; ?> "><? echo $r; ?></td>
                                <td width="150" style="word-wrap:break-word; word-break: break-all; text-align:center" ><? echo $color_value['po_number']; ?></td>
    ...
    <fieldset style="width:1000px; ">
		...
		<div id="report_container" align="center">
			<table border="1" class="rpt_table" rules="all" width="500" cellpadding="0" cellspacing="0" align="center">
				...
				union all 
				select c.order_no as po_number, a.order_id as po_break_down_id, sum(a.production_qnty) as good_qnty from subcon_gmts_prod_dtls a, subcon_ord_dtls c where a.production_type=7 and a.order_id=c.id and a.status_active=1 and a.is_deleted=0 and a.company_id=".$company_id." and a.order_id in(".$po_id.") and a.production_date='".$production_date."' and a.line_id=".$sewing_line." and a.floor_id=".$floor." group by c.order_no, a.order_id order by  po_number ";
			...
			union all 
			SELECT  c.color_id as color_number_id, c.size_id as size_number_id, a.order_id as po_break_down_id, sum(b.prod_qnty) as good_qnty,d.ORDER_NO as po_number from subcon_gmts_prod_dtls a, subcon_gmts_prod_col_sz b, subcon_ord_breakdown c,subcon_ord_dtls d where a.id=b.dtls_id and b.production_type=7 and a.production_type=7 and a.order_id=c.order_id and b.ord_color_size_id=c.id and a.status_active=1 and a.is_deleted=0 and a.company_id=".$company_id." and a.order_id in(".$po_id.") and a.production_date='".$production_date."' and a.line_id=".$sewing_line." and a.floor_id=".$floor." and d.id=c.order_id group by c.color_id, c.size_id, a.order_id,d.order_no order by po_break_down_id";
			...
			//echo $sql_color_size;die;	
				...
				{
					$size_arr[$cs_val[csf('size_number_id')]]=$cs_val[csf('size_number_id')];
					$color_size_qty_arr[$cs_val[csf('po_break_down_id')]][$cs_val[csf('color_number_id')]][$cs_val[csf('size_number_id')]]['good_qnty']+=$cs_val[csf('good_qnty')];
					$color_size_qty_arr[$cs_val[csf('po_break_down_id')]][$cs_val[csf('color_number_id')]]['po_number']=$cs_val[csf('po_number')];
					...
					foreach($po_value as $color_id=>$color_value)
					{
					  	...
					  	?>
                         	<tr style="font:'Arial Narrow';" align="center" bgcolor="<? echo $bgcolor;?>" onclick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $k; ?>">
								...
								{
								?>
									<td width="70" align="right"><?php echo $color_value[$sid]['good_qnty'] ;?></td>
								<?php
								}
								?>
                                <td width="100" align="right"><?php echo $order_color_arr[$po_id][$color_id] ;?></td>
    ...
    <fieldset style="width:1000px; ">
		...
		<div id="report_container" align="center">
			<table border="1" class="rpt_table" rules="all" width="500" cellpadding="0" cellspacing="0" align="center">
				...
				$sql_color_size="select  c.color_number_id,c.size_number_id ,a.po_break_down_id, sum(b.production_qnty)  as good_qnty,d.po_number from pro_garments_production_mst a, pro_garments_production_dtls b, wo_po_color_size_breakdown c,wo_po_break_down d where a.id=b.mst_id and b.production_type=5 and a.production_type=5 and a.po_break_down_id=c.po_break_down_id and b.color_size_break_down_id=c.id and a.status_active=1 and a.is_deleted=0 and a.serving_company=".$company_id." and a.po_break_down_id in(".$po_id.") and a.production_date='".$production_date."' and a.sewing_line=".$sewing_line." and a.floor_id=".$floor." and d.id=c.po_break_down_id  group by c.color_number_id,c.size_number_id ,a.po_break_down_id,d.po_number  
				union all 
				select  c.color_id as color_number_id, c.size_id as size_number_id, a.order_id as po_break_down_id, sum(b.prod_qnty) as good_qnty,d.order_no as po_number from subcon_gmts_prod_dtls a, subcon_gmts_prod_col_sz b, subcon_ord_breakdown c,subcon_ord_dtls d where a.id=b.dtls_id and b.production_type=2 and a.production_type=2 and a.order_id=c.order_id and b.ord_color_size_id=c.id and a.status_active=1 and a.is_deleted=0 and a.company_id=".$company_id." and a.order_id in(".$po_id.") and a.production_date='".$production_date."' and a.line_id=".$sewing_line." and a.floor_id=".$floor." and d.id=c.order_id group by c.color_id, c.size_id, a.order_id,d.order_no order by po_break_down_id";
					...
					foreach($po_value as $color_id=>$color_value)
					{
					  	...
					  	?>
                         	<tr style="font:'Arial Narrow';" align="center" bgcolor="<? echo $bgcolor;?>" onclick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $k; ?>">
								...
								?>
                                <td align="right"><?php echo $order_color_arr[$po_id][$color_id] ;?></td>
                        	</tr>
						<?
						$r++;
					}
		...
		<div id="report_container">
			<table border="1" class="rpt_table" rules="all" width="500" cellpadding="0" cellspacing="0" align="center">
                ...
                <?
						...
						UNION ALL 
						select b.id,c.item_id as item_number_id,c.qnty as order_quantity,c.amount as order_total from SUBCON_ORD_DTLS b,SUBCON_ORD_BREAKDOWN c where b.id=c.order_id and c.order_id in(".$po_id.") and b.is_deleted=0 and b.status_active=1  and c.is_deleted=0 and c.status_active=1
						";
						// echo $sql_item_rate;
						$resultRate=sql_select($sql_item_rate);
						$item_po_array=array();
		                ...
		                sum(CASE WHEN  a.production_type=5 THEN a.production_quantity else 0 END)  as good_qnty 
						from pro_garments_production_mst a, wo_po_details_master b, wo_po_break_down c
						where a.production_type=5 and a.po_break_down_id=c.id and c.job_no_mst=b.job_no and a.status_active=1 and a.is_deleted=0 and a.serving_company in(".$company_id.")  and a.floor_id=".$floor_id." and a.sewing_line=".$sewing_line."  and a.po_break_down_id in(".$po_id.") and a.production_date='".$prod_date."' group by c.po_number,a.po_break_down_id,a.item_number_id
						UNION ALL 
						SELECT c.order_no as po_number, a.order_id as po_break_down_id,a.GMTS_ITEM_ID as item_number_id,avg(c.rate) as unit_price,  sum(a.production_qnty) as good_qnty from subcon_gmts_prod_dtls a, subcon_ord_dtls c where a.production_type=2 and a.order_id=c.id and a.status_active=1 and a.is_deleted=0 and a.company_id=".$company_id." and a.order_id in(".$po_id.") and a.production_date='".$prod_date."' and a.line_id=".$sewing_line."  group by c.order_no, a.order_id,a.gmts_item_id order by  po_number
						";
						// echo $sql_pop;die;
						$sql_result=sql_select($sql_pop);
						$k=1;$total_amount=0;$total_prod_qty=0;
