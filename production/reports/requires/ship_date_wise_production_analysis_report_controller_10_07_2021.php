...
{ 
	...
	if ($po_ids != '')
	{
		...
		{
			$po_id_cond = ' and (';
			$poIdArr = array_chunk($po_ids,999);
			foreach($poIdArr as $ids)
			{
				$ids = implode(',',$ids);
				$po_id_cond .= " d.id in($ids) or ";
			}
			$po_id_cond = rtrim($po_id_cond2,'or ');
			$po_id_cond .= ')';
		}
		else
		{
			$po_ids = implode(',', $po_ids);
			$po_id_cond=" and d.id in ($po_ids)";
		}
	}
	...
	where a.id=b.delivery_mst_id and b.id=k.mst_id and k.color_size_break_down_id=c.id and c.po_break_down_id=d.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and k.status_active=1 and k.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active in(1,2,3) and d.is_deleted=0 $company_cond2 $work_company_cond $po_id_cond
	...
	<div width="<?= $details_table_width; ?>">
		        ...
		        <tbody>
		        	...
		        	foreach($buyer_summay_arr as $buyer_id=>$buyer_data)
					{
			        	foreach ($buyer_data as $buyer_client_id=>$value) 
			        	{
			        		...
			        		$shipment_qty = $value['AIR_QTY_FOC'] + $value['AIR_QTY_CLAIM'] + $value['SEA_QTY'];
			        		$excess_short_qty = $shipment_qty - $order_qty;
			        		if ($excess_short_qty > 0) $excess_qty = $excess_short_qty;
			        		else if ($excess_short_qty < 0) $short_qty = $excess_short_qty;
				        	...
				        	<tr bgcolor="<?= $bgcolor;  ?>" onclick="change_color('trds_<?= $i; ?>','<?= $bgcolor; ?>')" id="trds_<?= $i; ?>">
				        		...
				        		<td width="100" align="right" title="AIR Qty FOC+AIR Qty Claim+SEA Qty"><p><?= number_format($shipment_qty,0); ?></p></td>
	    ...
	    <table width="<?= $summary_table_width; ?>" class="rpt_table" border="1" rules="all" cellpadding="0" cellspacing="0">
			<tfoot>
				<tr>
					...
					<th width="100" align="right" id="tot_shipment_qty_id"><p><?= number_format(($tot_air_qty_foc+$tot_air_qty_claim+$tot_sea_qty),0); ?></p></th>
		...
		<div style="width:<?= $details_table_width+20; ?>"px; overflow-y:auto; max-height:300px" id="scroll_body_details">
		    <table width="<?= $details_table_width; ?>" class="rpt_table" border="1" rules="all" cellpadding="0" cellspacing="0" id="table_body_details">
		        <tbody>
		        	<?
		        	$i=1;
								...
								foreach($order_data as $item_id=>$item_data)
								{
									foreach($item_data as $country_id=>$country_data)
									{
										foreach($country_data as $color_id=>$value)
										{
											...
											$shipment_qty = $air_qty_foc+$air_qty_claim+$sea_qty;
											...
											$ttl_shipment_qty = $ttl_air_qty_foc+$ttl_air_qty_claim+$ttl_sea_qty;
								        	...
								        	<tr bgcolor="<?= $bgcolor;  ?>" onclick="change_color('trdsw_<?= $i; ?>','<?= $bgcolor; ?>')" id="trdsw_<?= $i; ?>">
									        	...
									        	<td width="80" align="right" title="AIR Qty FOC+AIR Qty Claim+SEA Qty"><p><?= number_format($shipment_qty,0); ?></p></td>
		...
		<table width="<?= $details_table_width; ?>" border="1" class="rpt_table" rules="all" cellpadding="0" cellspacing="0">
			<tfoot>
				<tr>
		        	<th width="50"><p>&nbsp;</p></th>
		        	<th width="150"><p>&nbsp;</p></th>
