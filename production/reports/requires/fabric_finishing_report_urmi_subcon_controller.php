        ...
        if($fab_dlts_id_chk[$row[csf("batch_id")]][$row[csf("dtls_id")]]=="")
        {
         ...
         //   if($row[csf("batch_ext_no")])$ext_production_qty=$row[csf("production_qty")];
			$summary_arr[$row[csf("entry_form")]]["no_of_roll"]+=$row[csf("no_of_roll")];
			 if($row[csf("re_stenter_no")]>0) { $re_work_production_qty=$row[csf("production_qty")];$production_qty=0;}
			 else { $re_work_production_qty=0;}
...
//echo $row[csf("buyer_name")].'D';
       ...
       // if($roll_count[$row[csf("entry_form")]][$row[csf("production_date")]][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("fabric_desc")]][$row[csf("re_stenter_no")]][$row[csf("roll_no")]][$row[csf("dtls_id")]]=="")
	    if($fab_dlts_id_chk2[$row[csf("batch_id")]][$row[csf("dtls_id")]]=="")
        {
            ...
            $roll_count2[$row[csf("entry_form")]][change_date_format($row[csf("production_date")])][$row[csf("batch_id")]][$row[csf("batch_ext_no")]][$row[csf("re_stenter_no")]][$row[csf("prod_id")]]+=$row[csf("no_of_roll")];
																	 ...
																	 //echo  $body_prod_rowspan.'B,<br>';
																	    if($batch_qty ||$brush_qty || $peach_qty || $chemical_qty || $aop_qty || $production_qty ||  $batch_qty_ex ||  $production_qty_ex || $re_work_production_qty || 1==1  )
                                                                        {
                                                                            ...
                                                                            <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trsub_<? echo $kk.$entry_form_id; ?>','<? echo $bgcolor;?>')" id="trsub_<? echo $kk.$entry_form_id; ?>">
																			 ...
																			 $fabric_typeD=ltrim($row["fabric_type"],',');
																			 $fabric_desc=ltrim($row["fabric_desc"],',');
																			 $gsm_weight=ltrim($row["gsm_weight"],',');
																			 $dia=ltrim($row["dia"],',');
																			 $width_dia_type=ltrim($row["width_dia_type"],',');
                                                                                 $com_data=array_unique( explode(",",$fabric_typeD)) ;
                                                                                 $com_data_desc=array_unique( explode(",",$fabric_desc)) ;
                                                                                 echo implode(",",$com_data);?> </td>
                                                                                 <td width="80" class="alignment_css"><? echo implode(",",$com_data_desc);?> </td>
                                                                                 <td width="80" class="alignment_css"><? echo implode(",",array_unique( explode(",", $gsm_weight)));   ?> </td>
                                                                                 <td width="80" class="alignment_css"><? echo implode(",",array_unique( explode(",", $dia)));  ;?> </td>
                                                                                 <td width="80" class="alignment_css"><? echo implode(",",array_unique( explode(",", $width_dia_type)));  ;?> </td>
                                                                               ...
                                                                               //  if($p==1)
																				...
																				//}
                                                                                 ...
                                                                                 <td width="80" class="alignment_css" title="No of Roll=<? echo $prod_no_of_roll;?>"><? echo $roll_count2[$entry_form_id][change_date_format($production_date)][$batch_id][$ext_no][$row["re_stenter_no"]][$prod_id];?> </td>
