...
if($action=="generate_report2") //Show 2
{
	...
	if($cbo_type==2) 
	{
		if(str_replace("'",'',$cbo_batch_type)==0 || str_replace("'",'',$cbo_batch_type)==1)
		{
			...
			where a.id = b.mst_id and a.load_unload_id in(1,2) and b.load_unload_id in(1,2) and b.entry_page=35 and a.status_active = 1 and b.status_active=1 $all_batch_no_cond2");
			...
			$add_tp_stri_batch_sql=sql_select("SELECT  a.entry_form,a.batch_id, a.dyeing_re_process,b.prod_id,b.ratio,c.item_category_id 
				...
				if($row[csf("extention_no")]>0 && $row[csf("result")]==1)
				{
					...
					if($row[csf("process_id")]==148) //Re Wash...
					{
						if($dyinWgchkBatch[$row[csf("batch_id")]] =="")
						{
							...
							{
								$total_rewash_batch[$row[csf("batch_id")]] =$row[csf("batch_id")];
							}
			...
			<fieldset style="width:1350px;">
				...
				if(str_replace("'",'',$cbo_batch_type)==0 || str_replace("'",'',$cbo_batch_type)==1)
				{
					if (count($batchdata)>0 || count($subbatchdata)>0)
					{
						...
						<table style="margin:5px;" cellpadding="0"  width="300" cellspacing="0" align="left"  >
							...
							<tr>
							  	<td>
							  		<table cellpadding="0" cellspacing="0"  class="rpt_table" rules="all" border="1">
							  			<thead>
											<tr>
						   	...
						   	<!-- As per number of Batch -->
						   	<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;</td></tr>
						   	<!-- Fabric type wise breakdown without Reprocess -->
						   	<tr>
							  	<td>
							  		<table cellpadding="0" cellspacing="0"  class="rpt_table" rules="all" border="1">
										<thead>
                       ...
                       <!-- Color Range wise Summary Without Reprocess Start -->
						...
						?>
						<div align="left">							
							<div style=" max-height:350px; width:3340px; overflow-y:scroll;">
							</div>
						</div>
						<br/>
						<? 
				...
				if(str_replace("'",'',$cbo_batch_type)==0 || str_replace("'",'',$cbo_batch_type)==2)
				{
					if (count($subbatchdata)>0)
					{
                        ...
                        ?>
						<div align="left">
							<div style=" max-height:350px; width:2860px; overflow-y:scroll;">
							</div>
						</div>
						<br/>
						<? 
