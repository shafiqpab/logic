...
{
	echo create_drop_down( "cbo_location", 130, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id in($data) order by location_name","id,location_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/floor_wise_sewing_monitoring_report_controller', this.value+'_'+$data, 'load_drop_down_floor', 'floor_td' );get_php_form_data( this.value, 'eval_multi_select', 'requires/floor_wise_sewing_monitoring_report_controller' );",0 );  
	// load_floor($data);   	 
}
			...
			foreach($location_data as $floor_id=>$floor_data)
			{
				...
				foreach($floor_data as $resource_id=>$resource_data)
				{
					...
					if($cbo_no_prod_type==1 && $line_floor_production>0)
					{
						...
						$floor_html.='<td style="word-wrap:break-word; word-break: break-all;" width="90">'.$sewing_line.'&nbsp;</td>
									...
									<td width="100" align="right">'. number_format($cm_valu_lc,2).'</td>
				...
				if($location_floor_prod_arr[$location_id][$floor_id] > 0)
				{
					$floor_html.='<tr bgcolor="#becbc9" style="font-weight:bold">
						...
						<td align="right"></td>
						<td align="right"></td>
						<td align="right">'.number_format($floor_wise_cm_value,2).'</td>
						<td align="right">'.number_format($floor_wise_cm_valu_lc,2).'</td>
						<td align="right">'.number_format($floor_wise_fob_value,2).'</td>
					</tr>';
				}
			}
		}							
	      	...
	      	<table width="890" cellspacing="0" border="1" class="rpt_table" rules="all" id="table_header_1">
	            <thead> 	 	 	 	 	 	
	                <tr>  
	                    ...
	                    <th width="100">Sewing CM Value BOM</th>
	                    <th width="100">Sewing CM Value LC</th>
	           ...
	           <?  echo $floor_html; ?> 
	            <tfoot>
	                   <tr style="font-weight:bold;color: #000000">
	                        ...
	                        <th style="font-weight:bold;color: #000000" align="right" width="100"></th>
	                        <th style="font-weight:bold;color: #000000" align="right" width="100"><? echo number_format($floor_cm_value,2); ?></th>
	                        <th style="font-weight:bold;color: #000000" align="right" width="100"><? echo number_format($floor_cm_valu_lc,2); ?></th>
	                        <th style="font-weight:bold;color: #000000" align="right" width="100"><? echo number_format($floor_fob_value,2); ?></th>
	                   </tr>
	               </tfoot>
	          </table>
	        </div>
