				...
				<table width="570" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
					...
					<tbody>
						<tr>
							...
							<td align="center">
								<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view ('<? echo $companyID; ?>'+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+'<? echo $cbo_year_id; ?>'+'**'+'<? echo $cbo_month_id; ?>', 'create_job_no_search_list_view', 'search_div', 'knitting_program_wise_grey_fab_stock_report_controller_test_temp', 'setFilterGrid(\'tbl_list_search\',-1)');" style="width:100px;" />
							</td>
						</tr>
					</tbody>
				</table>
...
if($action=="report_generate")
{
	...
	where a.job_no=b.job_no_mst and e.id=d.mst_id and c.dtls_id=d.id and c.po_id=b.id and a.company_name=$company_name $buyer_id_cond $po_cond $job_no_cond $program_no_cond $date_cond $booking_no_cond $internal_ref_cond and c.is_sales !=1 and d.is_sales !=1  and a.buyer_name= f.id and f.status_active=1 and f.is_deleted=0
	...
	foreach ($nameArray as $val)
	{
		...
		if(!$prog_id_check[$val[csf('prog_no')]])
		{
		    $prog_id_check[$val[csf("prog_no")]]=$val[csf("prog_no")];
		...
		if(!$po_id_check[$val[csf('po_id')]])
		{
		    ...
		    $PoNO = $val[csf("po_id")];
		    $rID2=execute_query("insert into tmp_poid (userid, poid) values ($user_name,$PoNO)");
		}
	}
	...
	if($rID && $rID2)
	{
		...
		echo "failed to insert into temp table";die;
	}
	...
	if(count($program_arr)>0)
	{
		...
		where a.id=b.mst_id and c.dtls_id=b.id and a.item_category=13 and a.entry_form=2 and c.entry_form=2 and a.receive_basis=2 and b.status_active=1 and b.is_deleted=0 $prog_cond_for_rcv
		group by a.id, b.id, c.id, d.id, a.booking_id,c.po_breakdown_id, c.quantity, d.qnty, b.trans_id, a.receive_date, d.barcode_no";*/

		$prod_sql="SELECT a.id, b.id as bid, c.id as cid, d.id as did, a.booking_id,c.po_breakdown_id as po_id, c.quantity as knitting_qnty, d.qnty, b.trans_id as trans_id, a.receive_date as receive_date, d.barcode_no 
		from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c
		left join pro_roll_details d on c.po_breakdown_id =d.po_breakdown_id and c.dtls_id=d.dtls_id and d.entry_form=2 and d.status_active=1, tmp_prog_no d
		where a.id=b.mst_id and c.dtls_id=b.id and a.item_category=13 and a.entry_form=2 and c.entry_form=2 and a.receive_basis=2 and b.status_active=1 and b.is_deleted=0 and a.booking_id=d.prog_no and d.userid= $user_name
		group by a.id, b.id, c.id, d.id, a.booking_id,c.po_breakdown_id, c.quantity, d.qnty, b.trans_id, a.receive_date, d.barcode_no";
		...
		if(count($production_id_arr) > 0)
		{
			...
			$recv_array=array();
			$sql_recv=sql_select("select a.booking_id,c.po_breakdown_id as po_id,sum(c.quantity) as knitting_qnty, min(receive_date) as receive_date from inv_receive_master a, pro_grey_prod_entry_dtls b,order_wise_pro_details c where a.id=b.mst_id and c.dtls_id=b.id and a.item_category=13 and a.entry_form=22 and c.entry_form=22 and a.receive_basis=9 and b.status_active=1 and b.is_deleted=0 $production_cond_for_rcv group by a.booking_id,c.po_breakdown_id");
			$minimum_date = "";
			foreach($sql_recv as $row)
			{
				...
				if(!$po_id_check[$row[csf('po_id')]])
				{
				    $po_id_check[$row[csf("po_id")]]=$row[csf("po_id")];
				    $PoNO = $row[csf("po_id")];
				    $rID2=execute_query("insert into tmp_poid (userid, poid) values ($user_name,$PoNO)");
				}
			}
		...
		/*$delivery_qty_res = sql_select("select sum(qnty) as qnty,a.booking_no,po_breakdown_id, min(receive_date) as receive_date from pro_roll_details a, inv_receive_master b  where a.mst_id = b.id and b.entry_form = 58 and a.entry_form in (58) and a.status_active=1 and a.is_deleted=0 $prog_cond_for_rcv_roll group by a.booking_no,po_breakdown_id");*/

		$delivery_qty_res = sql_select("select sum(qnty) as qnty,a.booking_no,po_breakdown_id, min(receive_date) as receive_date from pro_roll_details a, inv_receive_master b, tmp_prog_no c where a.mst_id = b.id and b.entry_form = 58 and a.entry_form in (58) and a.status_active=1 and a.is_deleted=0 and cast(a.booking_no as varchar2(30))= cast(c.prog_no as varchar2(30)) and c.userid= $user_name group by a.booking_no,po_breakdown_id");

		//echo "select sum(qnty) as qnty,a.booking_no,po_breakdown_id, min(receive_date) as receive_date from pro_roll_details a, inv_receive_master b, tmp_prog_no c where a.mst_id = b.id and b.entry_form = 58 and a.entry_form in (58) and a.status_active=1 and a.is_deleted=0 and cast(a.booking_no as varchar2(30))= cast(c.prog_no as varchar2(30)) and c.userid= $user_name group by a.booking_no,po_breakdown_id";die;
	...
	foreach($data_trans as $row_b)
	{
		...
		if(!$prog_id_check[$row_b[csf('to_program')]])
		{
		    $prog_id_check[$row_b[csf("to_program")]]=$row_b[csf("to_program")];
		    $ProgNO = $row_b[csf("to_program")];
		    $rID=execute_query("insert into tmp_prog_no (userid, prog_no) values ($user_name,$ProgNO)");
		}

		if(!$po_id_check[$row_b[csf('to_order_id')]])
		{
		    $po_id_check[$row_b[csf("to_order_id")]]=$row_b[csf("to_order_id")];
		    $PoNO = $row_b[csf("to_order_id")];
		    $rID2=execute_query("insert into tmp_poid (userid, poid) values ($user_name,$PoNO)");
		}
	...
	$roll_trans_in_sql="SELECT b.from_order_id, min(a.transfer_date) as transfer_date, sum(c.quantity) as item_transfer_in, b.to_order_id, e.po_number, d.job_no_prefix_num, d.buyer_name, e.grouping , h.id as program_no from inv_item_transfer_mst a, inv_item_transfer_dtls b, order_wise_pro_details c , wo_po_details_master d,wo_po_break_down e, pro_roll_details f, pro_roll_details g, ppl_planning_info_entry_dtls h, lib_buyer i
	where a.entry_form in (82,83) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
	and a.id=b.mst_id and c.trans_type in(5) and c.entry_form in (82,83) and c.is_sales <> 1 and b.id=c.dtls_id and c.po_breakdown_id=e.id and b.to_order_id = e.id
	and d.id= e.job_id
	and a.id=f.mst_id and b.id=f.dtls_id  
	and g.entry_form=2 and f.barcode_no = g.barcode_no and g.receive_basis=2 and cast(g.booking_no as varchar2(30)) = cast(h.id as varchar2(30))
	$date_cond_roll $proram_no_cond_roll $tran_job_no_cond $trans_po_cond_roll $buyer_id_cond_trans $internal_ref_cond_2
	  
	and c.status_active=1 and c.is_deleted=0 and a.item_category=13  and d.buyer_name= i.id and i.status_active=1 and i.is_deleted=0
	group by b.to_program, b.from_order_id, b.to_order_id, e.po_number, d.job_no_prefix_num, d.buyer_name, e.grouping, h.id";
	...
	foreach($roll_data_trans as $row)
	{
		...
		if(!$prog_id_check[$row[csf('program_no')]])
		{
		    $prog_id_check[$row[csf("program_no")]]=$row[csf("program_no")];
		    $ProgNO = $row[csf("program_no")];
		    $rID=execute_query("insert into tmp_prog_no (userid, prog_no) values ($user_name,$ProgNO)");
		}
	...
	{
		$for_issue_po_ids = implode(",", $for_issue_po_arr);
		$forIssuePoId = ""; $for_issue_po_id_cond = "";
		if($db_type==2 && count($for_issue_po_arr)>999)
		{
			$for_issue_po_arr_chunk=array_chunk($for_issue_po_arr,999) ;
			foreach($for_issue_po_arr_chunk as $chunk_arr)
			{
				$forIssuePoId.=" c.po_breakdown_id in(".implode(",",$chunk_arr).") or ";
			}

			$for_issue_po_id_cond.=" and (".chop($forIssuePoId,'or ').")";
		}
		else
		{
			$for_issue_po_id_cond=" and c.po_breakdown_id in($for_issue_po_ids)";
		}

		/*$roll_issue="SELECT b.remarks, d.qnty knitting_issue_qnty, d.po_breakdown_id, b.id, d.barcode_no, g.booking_no as program_no from inv_issue_master a, inv_grey_fabric_issue_dtls b, order_wise_pro_details c, pro_roll_details d left join pro_roll_details g on  d.barcode_no = g.barcode_no and g.entry_form=2 and g.receive_basis=2 where a.id=b.mst_id and b.id = c.dtls_id and a.id=d.mst_id and b.id=d.dtls_id and c.trans_type = 2 and a.item_category=13 and a.entry_form in (61) and c.entry_form in (61) and d.entry_form in (61) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active = 1 and c.is_deleted = 0 $for_issue_po_id_cond";*/

		$roll_issue="SELECT b.remarks, d.qnty knitting_issue_qnty, d.po_breakdown_id, b.id, d.barcode_no, g.booking_no as program_no from inv_issue_master a, inv_grey_fabric_issue_dtls b, order_wise_pro_details c, pro_roll_details d left join pro_roll_details g on  d.barcode_no = g.barcode_no and g.entry_form=2 and g.receive_basis=2, tmp_poid e where a.id=b.mst_id and b.id = c.dtls_id and a.id=d.mst_id and b.id=d.dtls_id and c.trans_type = 2 and a.item_category=13 and a.entry_form in (61) and c.entry_form in (61) and d.entry_form in (61) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active = 1 and c.is_deleted = 0 and c.po_breakdown_id=e.poid and e.userid= $user_name";
		...
		{
			//Splited barcode no 
			$issue_barcode_arr = implode(",", $issue_barcode_arr);
			if($db_type==2 && count($issue_barcode_arr)>999)
			{
				$issue_barcode_arr_chunk=array_chunk($issue_barcode_arr,999) ;
				$barcode_cond = " and (";

				foreach($issue_barcode_arr_chunk as $chunk_arr)
				{
					$barcode_cond.=" c.barcode_no in(".implode(",",$chunk_arr).") or ";
				}
			...
			{
				$barcode_cond=" and c.barcode_no in($issue_barcode_arr)";
			}

			$split_chk_sql = sql_select("select b.barcode_no, c.qnty, d.booking_no as program_no, c.po_breakdown_id from pro_roll_split b, pro_roll_details c, pro_roll_details d where b.entry_form = 75 and b.split_from_id=c.roll_split_from and b.barcode_no=d.barcode_no and d.entry_form=2 and d.receive_basis=2 and b.status_active = 1 and c.status_active = 1 $barcode_cond");
		...
		/*$sql_data=sql_select("SELECT b.program_no,b.remarks,c.quantity knitting_issue_qnty,c.po_breakdown_id,b.id
			...
			and b.program_no <> 0 and b.program_no is not null $for_issue_po_id_cond ");*/
		$sql_data=sql_select("SELECT b.program_no,b.remarks,c.quantity knitting_issue_qnty,c.po_breakdown_id,b.id
		from  inv_issue_master a,inv_grey_fabric_issue_dtls b , order_wise_pro_details c, tmp_poid d
		where a.id=b.mst_id and b.id = c.dtls_id
		and c.trans_type = 2 and a.item_category=13 and a.entry_form in (16) and c.entry_form in (16)
		and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active = 1 and c.is_deleted = 0
		and b.program_no <> 0 and b.program_no is not null and c.po_breakdown_id=d.poid and d.userid=$user_name");
			$idChkArr = array();
		...
		//$roll_iss_rtn_sql="SELECT c.po_breakdown_id, b.pi_wo_batch_no, c.quantity from inv_receive_master a, inv_transaction b, order_wise_pro_details c where a.id = b.mst_id and b.transaction_type=4 and b.id=c.trans_id and c.entry_form=84 and a.receive_basis=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $for_issue_po_id_cond";
		$roll_iss_rtn_sql="SELECT c.po_breakdown_id, b.pi_wo_batch_no, c.quantity from inv_receive_master a, inv_transaction b, order_wise_pro_details c, tmp_poid d where a.id = b.mst_id and b.transaction_type=4 and b.id=c.trans_id and c.entry_form=84 and a.receive_basis=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.po_breakdown_id=d.poid and d.userid=$user_name";
		...
		and c.entry_form in (83,13,80) and a.transfer_criteria in (1,4,6) and  b.from_program>0 and b.to_program>0 and (c.is_sales is null or c.is_sales = 0 or c.is_sales = 2) $for_issue_po_id_cond";*/
		$transfer_sql="SELECT b.from_program,b.to_program, a.entry_form,
		(case when c.trans_type in(6)  then c.quantity else 0 end) as item_transfer_out,
		(case when c.trans_type in(5) then c.quantity else 0 end) as item_transfer_in,
		a.from_order_id,a.to_order_id
		from inv_item_transfer_dtls b,inv_item_transfer_mst a,order_wise_pro_details c, tmp_poid d
		where a.id=b.mst_id  and c.dtls_id=b.id and c.trans_type in(5,6) and a.status_active=1 and a.is_deleted=0 and a.item_category=13
		and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
		and c.entry_form in (83,13,80) and a.transfer_criteria in (1,4,6) and  b.from_program>0 and b.to_program>0 and (c.is_sales is null or c.is_sales = 0 or c.is_sales = 2) and c.po_breakdown_id=d.poid and d.userid=$user_name";
		//echo $transfer_sql;
		...
		/*$get_barcode_sql="SELECT (case when c.trans_type in(6) then d.qnty else 0 end) as item_transfer_out, c.trans_type, d.barcode_no,e.booking_no as program_no,
	    ...
	    and (c.is_sales is null or c.is_sales = 0 or c.is_sales = 2) $for_issue_po_id_cond ";*/

	    $get_barcode_sql="SELECT (case when c.trans_type in(6) then d.qnty else 0 end) as item_transfer_out, c.trans_type, d.barcode_no,e.booking_no as program_no,
	    (case when c.trans_type in(5) then d.qnty else 0 end) as item_transfer_in, c.po_breakdown_id
	    from inv_item_transfer_dtls b,inv_item_transfer_mst a,order_wise_pro_details c, pro_roll_details d, pro_roll_details e, tmp_poid f
	    where a.id=b.mst_id and c.dtls_id=b.id and a.id=d.mst_id and b.id=d.dtls_id and d.barcode_no= e.barcode_no and e.entry_form=2 and e.receive_basis=2
	    and c.trans_type in(5,6) and a.status_active=1 and a.is_deleted=0 and a.item_category=13 
	    and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and c.entry_form in (82) and a.transfer_criteria in (1,4,6,2) 
	    and (c.is_sales is null or c.is_sales = 0 or c.is_sales = 2) and c.po_breakdown_id=f.poid and f.userid=$user_name ";
		...
		if(!empty($all_prog_for_issue))
		{
			...
			if($db_type==2 && count($all_prog_for_issue)>999)
			{
				...
				foreach($all_prog_for_issue_chunk as $chunk_arr)
				{
					...
					$progCond.=" d.id in(".implode(",",$chunk_arr).") or ";
				}
			...
			else
			{
				...
				$all_trans_prog_cond=" and d.id in($all_yarn_program_no)";
			}
			...
			$yarn_production_wise_lot_count=sql_select("SELECT b.po_breakdown_id, a.yarn_lot, a.yarn_count as yarn_count, c.booking_id, a.id, a.yarn_prod_id
			from pro_grey_prod_entry_dtls a, order_wise_pro_details b,inv_receive_master c, tmp_prog_no d
			where a.id=b.dtls_id and a.mst_id = c.id and c.receive_basis = 2 and b.entry_form =2 and c.entry_form = 2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and c.booking_id=d.prog_no and d.userid=$user_name");
			//$yarn_order_cond
			...
			/*$sql_arr = sql_select("SELECT d.id as prog_no, d.program_date, d.machine_dia, d.fabric_dia, c.gsm_weight, c.program_qnty, d.stitch_length, d.color_id, e.booking_no, e.fabric_desc from ppl_planning_entry_plan_dtls c, ppl_planning_info_entry_dtls d, ppl_planning_info_entry_mst e where e.id=d.mst_id and c.dtls_id=d.id and c.is_sales <> 1 and e.is_sales <> 1 and c.status_active =1 and c.is_deleted= 0 and d.status_active =1 and d.is_deleted= 0 and e.status_active = 1 and e.is_deleted= 0 $all_trans_prog_cond group by d.id, d.program_date, d.machine_dia, d.fabric_dia, c.gsm_weight, c.program_qnty , d.stitch_length, d.color_id, e.booking_no, e.fabric_desc");*/

			$sql_arr = sql_select("SELECT d.id as prog_no, d.program_date, d.machine_dia, d.fabric_dia, c.gsm_weight, c.program_qnty, d.stitch_length, d.color_id, e.booking_no, e.fabric_desc from ppl_planning_entry_plan_dtls c, ppl_planning_info_entry_dtls d, ppl_planning_info_entry_mst e, tmp_prog_no f where e.id=d.mst_id and c.dtls_id=d.id and c.is_sales <> 1 and e.is_sales <> 1 and c.status_active =1 and c.is_deleted= 0 and d.status_active =1 and d.is_deleted= 0 and e.status_active = 1 and e.is_deleted= 0 and d.id=f.prog_no and f.userid=$user_name group by d.id, d.program_date, d.machine_dia, d.fabric_dia, c.gsm_weight, c.program_qnty , d.stitch_length, d.color_id, e.booking_no, e.fabric_desc");
				...
				<table border="1" class="rpt_table" rules="all" width="540" cellpadding="0" cellspacing="0" id="table_body">
					...
					if($all_productionBarcode) 
					{
						...
						$rollWiseRcv =  sql_select("SELECT  a.qnty as recv_qnty, c.rack, c.self, a.id
							...
							and a.entry_form = 58 and a.status_active = 1 and a.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0 and b.company_id = $companyID $all_Barcode_cond");
						...
						where a.mst_id = b.id and b.id = c.mst_id and a.dtls_id =c.id and a.entry_form=61 and b.entry_form = 61 and a.status_active=1 and a.is_deleted = 0  and a.booking_no='$prog_no' and a.po_breakdown_id in($po_id)");// $all_Barcode_cond
						...
						and c.status_active=1 and c.is_deleted=0 and c.entry_form in(82) and a.entry_form in(82) and a.transfer_criteria in (1,2,4,6) and (c.is_sales is null or c.is_sales = 0 or c.is_sales = 2) $all_Barcode_cond group by b.rack, b.shelf, c.entry_form");
						...
						where a.id=b.mst_id and b.id=c.trans_id and a.id=d.mst_id and b.transaction_type=4 and c.entry_form=84 and d.entry_form=84 and a.receive_basis=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.booking_no='$prog_no' and d.po_breakdown_id in($po_id)";
						// echo $roll_iss_rtn_sql;
						$roll_iss_rtn_data=sql_select($roll_iss_rtn_sql);
						foreach($roll_iss_rtn_data as $row)
						{
				...
				<table border="1" class="rpt_table" rules="all" width="540" cellpadding="0" cellspacing="0" id="table_body">
					<tbody>
						...
						$sql_result=("SELECT b.from_program,b.to_program,b.rack, b.shelf, b.transfer_qnty as transfer_qnty,a.to_order_id from inv_item_transfer_dtls b,inv_item_transfer_mst a,order_wise_pro_details c where a.id=b.mst_id  and c.dtls_id=b.id and c.trans_type=6 and a.status_active=1 and a.is_deleted=0 and a.item_category=13 and a.transfer_criteria=4 and b.to_program='$prog_no' and a.to_order_id in($po_id) and b.from_program>0 and b.to_program>0 
							...
							sum(case when c.trans_type in(5) then c.quantity else 0 end) as transfer_qnty,
		...
		<div id="report_container" style="width:100%">
			...
			<div style="width:500px; overflow-y:scroll; max-height:250px;font-size:12px; overflow-x:hidden;" id="scroll_body">
				<table border="1" class="rpt_table" rules="all" width="500" cellpadding="0" cellspacing="0" id="table_body">
					...
					and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
					and c.entry_form in (13) and a.entry_form in (13) and a.transfer_criteria=4 and  b.from_program>0 and b.to_program>0 and c.is_sales <> 1 and b.to_program in($prog_no) and a.to_order_id in($po_id)
					...
					sum(case when c.trans_type in(5) then c.quantity else 0 end) as qnty from inv_item_transfer_dtls b,inv_item_transfer_mst a,order_wise_pro_details c, pro_roll_details d, pro_roll_details g, ppl_planning_info_entry_dtls h
					...
					and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
					and c.entry_form in (83,82) and a.entry_form in (83,82) and a.transfer_criteria in (1,2,4,6) and (c.is_sales is null or c.is_sales = 0 or c.is_sales = 2) and c.po_breakdown_id in($po_id) $production_barcode_cond_for_rcv and d.barcode_no=g.barcode_no and g.entry_form=2 and g.receive_basis=2 and cast(g.booking_no as varchar2(30)) = cast(h.id as varchar2(30)) and h.id=$prog_no
		...
		<div id="report_container" style="width:100%">
			...
			<div style="width:698px; overflow-y:scroll; max-height:250px;font-size:12px; overflow-x:hidden;" id="scroll_body">
				<table border="1" class="rpt_table" rules="all" width="680" cellpadding="0" cellspacing="0" id="table_body">
					...
					where a.id = b.mst_id and b.transaction_type =4 and b.id=c.trans_id and c.entry_form=84 and a.receive_basis=0 and b.pi_wo_batch_no = $prog_no and c.po_breakdown_id=$po_id";
			...
			<div id="report_container" style="width:100%">
				...
				<div style="width:500px; overflow-y:scroll; max-height:250px;font-size:12px; overflow-x:hidden;" id="scroll_body">
					<table border="1" class="rpt_table" rules="all" width="500" cellpadding="0" cellspacing="0" id="table_body">
						...
						and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
						and c.entry_form in (13,80) and a.entry_form in (13,80) and a.transfer_criteria in (4,6) and  b.from_program>0 and b.to_program>0 and (c.is_sales is null or c.is_sales = 0 or c.is_sales = 2) and b.from_program in($prog_no) and a.from_order_id in($po_id)
						...
						and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
						and c.entry_form in (83,82) and a.entry_form in (83,82) and a.transfer_criteria in (1,2,4,6) and (c.is_sales is null or c.is_sales = 0 or c.is_sales = 2) and c.po_breakdown_id in($po_id) $production_barcode_cond_for_rcv
