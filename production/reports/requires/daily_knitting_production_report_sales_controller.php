				...
				<div id="content_search_panel" >
					<table cellpadding="0" cellspacing="0" width="100%" class="rpt_table" border="1" rules="all">
						...
						<tr>
							...
							<td align="center">
								<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_company_id').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value +'_'+document.getElementById('cbo_buyer').value+'_'+<? echo $cbo_booking_type; ?>, 'create_booking_search_list_view', 'search_div', 'daily_knitting_production_report_sales_controller', 'setFilterGrid(\'tbl_list_search\',-1, tableFilters);'); accordion_menu(accordion_h1.id,'content_search_panel','')" style="width:100px;" />
							</td>
						</tr>
						<tr>
...
if($action=="report_generate")
{
	...
	$vari_knit_charge_source_arr = sql_select("select editable from variable_order_tracking where company_name in($cbo_company) and variable_list=70 and status_active=1");
	...
	if($report_type==1)	
	{
						...
						<div style="width:570px; overflow-y:scroll; max-height:220px;" id="scroll_body">
							<table cellspacing="0" cellpadding="0" border="1" rules="all" width="550px" class="rpt_table" >
								<tbody>
									...
									where a.entry_form=22 and a.receive_basis=11 and a.item_category=13 and a.id=b.mst_id $cbo_company_cond $company_working_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $date_con $buyer_cond group by a.buyer_id"); // and a.company_id=$cbo_company_name
						...
						<div style="width:280px; overflow-y:scroll; max-height:220px;" id="scroll_body">
							<table cellspacing="0" cellpadding="0" border="1" rules="all" width="260px" class="rpt_table" >
								<tbody>
									...
									$cbo_company_cond $company_working_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0  and e.status_active=1 and e.is_deleted=0 $date_con_sub $floor_id_cond $buyer_id_cond $job_no_cond $order_no_cond $job_year_sub_cond
									group by a.party_id";//and a.company_id=$cbo_company_name
		...
		<fieldset style="width:<? echo $tbl_width+20; ?>px;">
							...
							if (count($nameArray_inhouse)>0)
							{
								...
								foreach ($nameArray_inhouse as $row)
								{
									...
									if(!in_array($row[csf('floor_id')],$floor_array))
									{
										...
										?>
										<tr><td colspan="37" style="font-size:14px" bgcolor="#CCCCAA">&nbsp;<b><? if($row[csf('floor_id')]==0 ||$row[csf('floor_id')]=="") echo "Without Floor"; else echo $floor_details[$row[csf('floor_id')]]; ?></b></td></tr>
										<?
									...
									<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>"> 
										<td width="40" align="center" valign="middle" id="chk_hide_dtls">
											...
											<td width="55"><p>AA<? echo $knitting_party; ?>&nbsp;</p></td>
									...
									foreach ($nameArray_subcontract as $row)
									{
										...
										<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>"> 
											<td width="40" align="center" valign="middle">
												...
												<td width="30"><? echo $i; ?></td>
												<td width="55"><p>BB<? echo $knitting_party; ?>&nbsp;</p></td>
												<td width="60"><p>&nbsp;<? echo $row[csf('machine_name')]; ?></p></td>
											...
											foreach ($nameArray_service_receive as $row)
											{
												...
												<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>"> 
													<td width="40" align="center" valign="middle">
														...
														<input type="hidden" id="source_<? echo $i; ?>" name="source[]" value="<? echo "3"; ?>" />
														<td width="30"><? echo $i; ?></td>
														<td width="55"><p>CC<? echo $knitting_party; ?>&nbsp;</p></td>
														<td width="60"><p>&nbsp;<? echo $row[csf('machine_name')]; ?></p></td>
														<td align="center" width="60"><p><? echo $po_sub_array[$row[csf('po_breakdown_id')]]['job_no']; ?></p></td>
												...
												if (count($nameArray_without_order)>0)
												{
													...
													foreach ($nameArray_without_order as $row)
													{
														...
														if(!in_array($row[csf('floor_id')],$floor_array))
														{
															...
															?>
															<tr>
																<td colspan="35" style="font-size:14px" bgcolor="#CCCCAA">
																	<b><? ($row[csf('floor_id')]==0 ||$row[csf('floor_id')]=="")? "Without Floor": $floor_details[$row[csf('floor_id')]]; ?></b>
																</td>
															</tr>
															<?	
														...
														<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>"> 
															<td width="40" align="center" valign="middle" id="">
																...
																<td width="30"><? echo $i; ?></td>
																<td width="55"><p>DD<? echo $knitting_party; ?>&nbsp;</p></td>
																<td width="60"><p>&nbsp;<? echo $row[csf('machine_name')]; ?></p></td>
																<td align="center" width="60"><p>&nbsp;<? echo $row[csf('job_no_prefix_num')]; ?></p></td>
																<td width="70">&nbsp;</td>
																<td width="70">&nbsp;</td>
			...
			if($cbo_type==2 || $cbo_type==0)
			{
				...
				where a.id=b.mst_id and b.machine_id=c.id and b.order_id=d.id and e.subcon_job=d.job_no_mst and a.product_type=2 
				$cbo_company_cond $company_working_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0  and e.status_active=1 and e.is_deleted=0 $date_con_sub $floor_id_cond $buyer_id_cond $job_no_cond $order_no_cond $job_year_sub_cond
				group by b.id, a.prefix_no_num, a.product_no, a.product_date, a.party_id, a.remarks, b.cons_comp_id, b.gsm, b.dia_width, b.dia_width_type,  b.yarn_lot, b.yrn_count_id, b.stitch_len, b.brand, b.machine_id, b.floor_id, b.color_id, b.order_id, c.machine_no, e.job_no_prefix_num, e.insert_date, d.order_no, d.cust_style_ref, c.seq_no order by b.floor_id, a.product_date, c.seq_no"; //and a.company_id=$cbo_company_name
	        ...
	        <fieldset style=" width:750px;">
	            ...
	            <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center" id="summary_tab">
	                ...
	                $ymcpacity_arr=return_library_array( "SELECT d.machine_no,sum(c.quantity) as quantity  from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c, lib_machine_name d, wo_po_break_down e,  wo_po_details_master f where c.po_breakdown_id=e.id and a.entry_form=2 and a.item_category=13 and a.id=b.mst_id and b.id=c.dtls_id and e.job_no_mst=f.job_no and c.entry_form=2 and c.trans_type=1 and a.knitting_source=1 and a.knitting_source like '$source' and b.machine_no_id=d.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $cbo_company_cond $company_working_cond $location_cond $date_con_2 $floor_id $buyer_cond $job_cond $order_cond $job_year_cond group by d.machine_no", "machine_no", "quantity" );// and a.company_id=$cbo_company_name
	    
	                $ymcpacityWO_arr=return_library_array( "SELECT d.machine_no,sum(b.grey_receive_qnty) as quantity  from inv_receive_master a, pro_grey_prod_entry_dtls b, lib_machine_name d where a.entry_form=2 and a.item_category=13 and a.id=b.mst_id  and a.knitting_source=1  and a.knitting_source like '$source' and b.machine_no_id=d.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.booking_without_order=1 $cbo_company_cond $company_working_cond $location_cond $date_con_2 $floor_id  $buyer_cond group by d.machine_no", "machine_no", "quantity" );
	                // and a.company_id=$cbo_company_name
	...
	else if($report_type==3) //Machine Wise 2
	{
		...
		if($cbo_type==1 || $cbo_type==0)
		{
			...
			and a.company_id = $company_name $program_cond_trans $booking_no_cond_program $libas_date_cond_2");*/
		
			$knit_plan_arr=array();
			$plan_data=sql_select("select b.id,b.knitting_source, b.color_range,c.machine_id,b.program_qnty as prog_qty,c.distribution_qnty as program_qnty, b.stitch_length, b.machine_dia, b.machine_gg from ppl_planning_info_entry_dtls b left join ppl_planning_info_machine_dtls c on  b.id=c.dtls_id and c.status_active=1 where  b.status_active=1 ");
			//echo "select b.id, b.color_range,c.machine_id,c.distribution_qnty as program_qnty, b.stitch_length, b.machine_dia, b.machine_gg from ppl_planning_info_entry_dtls b,ppl_planning_info_machine_dtls c where b.id=c.dtls_id and b.status_active=1 and c.status_active=1";
			foreach($plan_data as $row)
			{
				$knit_plan_arr[$row[csf('id')]]['cr']=$row[csf('color_range')];
				$knit_plan_arr[$row[csf('id')]]['sl']=$row[csf('stitch_length')]; 
				...
				if($row[csf('knitting_source')]==1)
				{
					$machine_wise_plan_arr[$row[csf('id')]][$row[csf('machine_id')]]['program_qnty']+=$row[csf('program_qnty')]; 
				}
				else
				{
					$machine_wise_plan_arr[$row[csf('id')]]['program_qnty']+=$row[csf('prog_qty')]; 
				}
		...
		$tbl_width=2800+$width+count($shift_name)*60;
		ob_start();
		?>
		<table cellpadding="0" cellspacing="0" width="<? echo $tbl_width; ?>">
			<tr>
        ...
        <table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table">
			...
			if($db_type==0)
			{
				...
				group by b.kniting_charge, a.company_id,a.receive_basis, a.receive_date, a.booking_no, a.challan_no, b.machine_no_id,d.brand, b.floor_id, d.seq_no, d.machine_no 
			...
			if(str_replace("'","",$cbo_knitting_source)==0 || str_replace("'","",$cbo_knitting_source)==2)
			{
				...
				listagg((cast(nvl(b.color_id,0) as varchar2(4000))),',') within group (order by nvl(b.color_id,0)) as color_id,
				 listagg((cast(b.color_range as varchar2(4000))),',') within group (order by b.color_range) as color_range_id, listagg((cast(b.order_id as varchar2(4000))),',') within group (order by b.order_id) as po_breakdown_id, listagg((cast(d.order_no as varchar2(4000))),',') within group (order by d.order_no) as order_nos, d.job_no_mst as job_no, null as sales_booking_no,sum(b.reject_qnty) as reject_qty,0 as is_sales, a.party_id as unit_id,0 as within_group,  2 as knitting_source, a.knitting_company,a.party_id as buyer_id,
				...
				and a.status_active=1 and a.is_deleted=0  $company_working_cond $cbo_company_cond $date_con_sub $floor_id_cond $buyer_id_cond $job_no_cond $order_no_cond $job_year_sub_cond $sales_order_cond $booking_no_cond $within_group_cond
				group by a.product_date,a.knitting_source,a.knitting_company, b.machine_id, b.floor_id, d.job_no_mst, a.party_id,a.company_id 
			...
			$get_fso_rate = sql_select("SELECT mst_id, job_no_mst, body_part_id, process_seq_main from fabric_sales_order_dtls where status_active=1 and is_deleted=0");
			$fso_knitting_rate_arr=array(); //  and mst_id=2255  and job_no_mst='OG-FSOE-21-00090'
			foreach ($get_fso_rate as $fso_row)
			{
				$process_seq_main=explode(',', $fso_row[csf("process_seq_main")]);
				foreach ($process_seq_main as $key => $value) 
				{
					$process_rate_arr=explode('_',  $value);
					$process=$process_rate_arr[0];
					$rate=$process_rate_arr[2];
					if ($process==1) 
					{
						// echo $process.'=='.$rate.'<br>';
						$fso_knitting_rate_arr[$fso_row[csf("job_no_mst")]][$body_part[$fso_row[csf("body_part_id")]]] = $rate;
					}
					
				}
			}
			// echo "<pre>";print_r($fso_knitting_rate_arr);die;
			...
			foreach ($nameArray_inhouse as $row)
			{
				...
				$buyer_wise_production_arr[$buyer_id][$row[csf("knitting_source")]][$row[csf("booking_type")]][$row[csf("within_group")]][$row[csf("is_order")]] += ($row[csf("qntyshifta")]+$row[csf("qntyshiftb")]+$row[csf("qntyshiftc")]);
				if($row[csf('receive_basis')]==2) //receive_basis
				{
					$prog_booking_nos .="'".$row[csf("booking_no")]."'". ",";
				}
				$booking_nos .= $row[csf("booking_no")] . ",";
			}
			$prog_booking_nos=rtrim($prog_booking_nos,',');
			$prog_booking_nos=implode(",",array_unique(explode(",",$prog_booking_nos)));
			//Prod Qty Start here
			$sql_inhouse2="select * from (
				(select a.company_id,a.booking_no, b.machine_no_id,sum(case when b.shift_name=0 then c.quantity else 0 end) as without_shift";
				foreach($shift_name as $key=>$val)
				{
					$sql_inhouse2.=", sum(case when b.shift_name=$key then c.quantity else 0 end) as qntyshift".strtolower($val);
				}
				$within_group_cond = ($cbo_within_group != 0)?" and e.within_group=$cbo_within_group" : "";
				...
				$sql_inhouse2.=" from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c,fabric_sales_order_mst e,  wo_booking_mst f  where a.id=b.mst_id and b.id=c.dtls_id and c.po_breakdown_id=e.id and e.sales_booking_no=f.booking_no and a.entry_form=2 and a.item_category=13 and c.entry_form=2 and c.trans_type=1  $company_working_cond $cbo_company_cond and a.knitting_source like '$source' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and a.booking_no in($prog_booking_nos) and a.receive_date<='$to_date' $floor_id $buyer_cond $sales_order_cond $booking_no_cond $within_group_cond $entry_form_cond  group by a.company_id,a.booking_no, b.machine_no_id)";

				$sql_inhouse2 .= " union all  (select a.company_id,a.booking_no, b.machine_no_id,sum(case when b.shift_name=0 then c.quantity else 0 end) as without_shift";
				foreach($shift_name as $key=>$val)
				{
					$sql_inhouse2.=", sum(case when b.shift_name=$key then c.quantity else 0 end) as qntyshift".strtolower($val);
				}
				...
				$sql_inhouse2.=" from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c,fabric_sales_order_mst e ,wo_non_ord_samp_booking_mst g where a.id=b.mst_id and b.id=c.dtls_id and c.po_breakdown_id=e.id and e.sales_booking_no=g.booking_no and g.status_active=1 and g.is_deleted=0 and a.entry_form=2 and a.item_category=13 and c.entry_form=2 and c.trans_type=1  $company_working_cond $cbo_company_cond and a.knitting_source like '$source' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.booking_no in($prog_booking_nos)  and a.receive_date<='$to_date' $floor_id $buyer_cond $sales_order_cond $booking_no_cond $within_group_cond $entry_form_cond  group by a.company_id,a.booking_no, b.machine_no_id)";
				
				$sql_inhouse2.=" union all (select a.company_id,a.booking_no, b.machine_no_id,sum(case when b.shift_name=0 then c.quantity else 0 end) as without_shift, sum(case when b.shift_name=1 then c.quantity else 0 end) as qntyshifta, sum(case when b.shift_name=2 then c.quantity else 0 end) as qntyshiftb, sum(case when b.shift_name=3 then c.quantity else 0 end) as qntyshiftc";
				if($cbo_booking_type > 0)
				{	
					$entry_form_cond = " and a.id=0";
				}
				...
				$sql_inhouse2.=" from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c,fabric_sales_order_mst e  where a.id=b.mst_id and b.id=c.dtls_id and c.po_breakdown_id=e.id and e.within_group=2 and a.entry_form=2 and a.item_category=13 and c.entry_form=2 and c.trans_type=1  $company_working_cond $cbo_company_cond and a.knitting_source like '$source' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.booking_no in($prog_booking_nos)  and a.receive_date<='$to_date' $floor_id $buyer_cond $sales_order_cond $booking_no_cond $within_group_cond $entry_form_cond  group by a.company_id,a.booking_no, b.machine_no_id)
				) order by machine_no_id  
				";
				//echo $sql_inhouse2;
				//prob Qty End
			//echo $sql_inhouse2.'DD';
			$sql_inhouse2_result=sql_select( $sql_inhouse2);
			foreach ($sql_inhouse2_result as $row)
			{
				$prog_wise_prod_qty[$row[csf('machine_no_id')]][$row[csf('booking_no')]]['qty']=$row[csf('qntyshifta')]+$row[csf('qntyshiftb')]+$row[csf('qntyshiftc')];
			}
			unset($sql_inhouse2_result);
			$subconJobNos="";
			//print_r($prog_wise_prod_qty);
			foreach ($nameArray_inhouse_subcon as $row)
			{
				if($row[csf('machine_no_id')] != 0)
				{
				...
				$buyer_id =$buyer_arr[$row[csf("buyer_id")]];// ($row[csf("within_group")]==1)?$booking_arr[$row[csf("sales_booking_no")]]:$buyer_arr[$row[csf("buyer_id")]];
				$buyer_wise_production_arr[$buyer_id][$row[csf("knitting_source")]][$row[csf("booking_type")]][$row[csf("within_group")]][$row[csf("is_order")]] += ($row[csf("qntyshifta")]+$row[csf("qntyshiftb")]+$row[csf("qntyshiftc")]);
				$subconJobNos.="'".$row[csf("job_no")]."',";
				
			}
			$subconJobNos=chop($subconJobNos,",");

			$sql_subConProgram = sql_select("select b.id as program_no,a.job_no,a.buyer_id,a.gsm_weight,a.dia,a.subcon_order_id,b.color_id,b.machine_id,b.machine_dia,b.machine_gg  from subcon_planning_mst a,subcon_planning_dtls b where a.id=b.mst_id and a.job_no in($subconJobNos) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
			foreach($sql_subConProgram as $datas)
			{
				$progranNoArr[$datas[csf("job_no")]][$datas[csf("buyer_id")]][$datas[csf("gsm_weight")]][$datas[csf("dia")]][$datas[csf("color_id")]][$datas[csf("machine_dia")]][$datas[csf("machine_gg")]]['programNo']=$datas[csf("program_no")];
			}
			//print_r($progranNoArr);
			//echo "<pre>";
			//print_r($buyer_wise_production_arr);die;
			$sql_subConProgram_machine = sql_select("select b.id as program_no,a.job_no,a.buyer_id,a.gsm_weight,a.dia,a.subcon_order_id,b.color_id,b.machine_id,b.machine_dia,b.machine_gg ,c.distribution_qnty
			from subcon_planning_mst a,subcon_planning_dtls b,subcon_planning_machine_dtls c  
			where a.id=b.mst_id and a.id=c.mst_id and b.id=c.dtls_id and b.machine_id=c.machine_id and a.job_no in($subconJobNos) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
			foreach($sql_subConProgram_machine as $datas)
			{
				$progranMachineDistriQntyArr[$datas[csf("job_no")]][$datas[csf("buyer_id")]][$datas[csf("gsm_weight")]][$datas[csf("dia")]][$datas[csf("color_id")]][$datas[csf("machine_dia")]][$datas[csf("machine_gg")]][$datas[csf("machine_id")]][$datas[csf("program_no")]]['distribution_qnty']=$datas[csf("distribution_qnty")];
			}

			//echo "<pre>";
			//print_r($progranMachineDistriQntyArr);die;
            ...
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $tbl_width; ?>" class="rpt_table" id="table_head" >
                <thead>
                    <tr>
                        ...
                        <th width="70" rowspan="2">Receive Challan No</th>
                        <th width="100" rowspan="2">Style Name</th>
						<th width="100" rowspan="2">Body Part</th>
                        <th width="140" rowspan="2">Booking Type</th>
                        <th width="140" rowspan="2">Booking No</th>
                        ...
                        //}
                        ?>
                        <th width="100" rowspan="2">Target Qty</th>
                        <th width="100" rowspan="2">Total Production Qty </th>
                        <th width="100" rowspan="2">Balance Production Qty</th>
                ...
                <tbody id="scroll_body" >
                    <?
                    $total_grand_rejectQnty=$grand_tot_program_qnty=$grand_bal_prod_tot_qnty=0;
					$p=1;$q=1; $z=1;
                    if($cbo_type==1 || $cbo_type==0)
                    {
						...
						if (count($nameArray_inhouse)>0)
                        {
                          // echo $row[csf("knitting_source")].'D'.$row[csf("body_part_id")];
                            ...
                            foreach ($nameArray_inhouse as $row)
                            {
								...
								if($row[csf("knitting_source")]==3) // Outbound Subcontract
								{
									if($p==1)
									{
										?>
                                        <tr class="tbl_bottom">
	                                        ...
	                                        //}
	                                        ?>
                                            <td align="right"><? echo number_format($program_qnty_tot_qty,2,'.',''); ?></td>
                                   			<td align="right"><? echo number_format($inhouse_mc_prod_tot_qnty,2,'.',''); ?></td>
                                   			<td align="right"><? echo number_format($inhouse_bal_mc_prod_qty,2,'.',''); ?></td>
										...
										$inhouse_ship=array();$inhouse_mc_prod_ship=array();$inhouse_tot_qty=$machine_total_inhouser=$grand_reject_qty=$program_qnty_tot_qty=$inhouse_mc_prod_tot_qnty=$inhouse_bal_mc_prod_qty=$inhouse_tot_charge_amount=0;
                                    ...
                                    if($row[csf('receive_basis')]==2)
                                    {
										...
										$program_qnty=$machine_wise_plan_arr[$row[csf('booking_no')]]['program_qnty'];
                                    }
                                    else
                                    {
									...
									$fso_knitting_rate=$fso_knitting_rate_arr[$row[csf("job_no")]][$body_partName];
                                    ...
                                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" valign="top">
                                        ...
                                        <td align="center"><p><? if($row[csf('within_group')]==1){echo $company_arr[$row[csf('unit_id')]];}else{echo $company_arr[$row[csf('company_id')]];} ?></p></td>
                                        <td width="70" align="center"><p><? echo $buyer_id; ?></></td>
                                       
                                        <td align="center"><strong>P:</strong><? echo $row[csf('booking_no')] . "<br /><strong>S:</strong>" . $row[csf('job_no')]; ?>
                                        </td>
                                        <td width="70" align="center"><p><? echo $row[csf('challan_no')]; ?></></td>
                                        <td width="100" align="center"><p><? echo $row[csf('style_ref_no')]; ?></></td>
										<td width="100" align="center"><p><? echo $body_partName; ?></></td>
                                        <td align="center">
                                            <? 
                                            ...
                                            {
                                                echo $booking_type_arr[$row[csf('entry_form')]];
                                            }
                                            
                                            ?>
                                        </td>
                                        <td align="center"><p><? echo $row[csf('sales_booking_no')]; ?></p></td>
                                        <td align="center"><p><? echo $count; ?></p></td>
                                        <td><P>
										<?
                                        ...
                                        ?>&nbsp;
										 ...
										 <td align="center"><p><? echo $yarn_type_name; ?></p></td>
                                        <td align="center"><P><? echo implode(",",array_unique(explode(",",$row[csf('yarn_lot')]))); ?>&nbsp;</P></td>
                                        <td><P>
										<?
                                        ...
                                        ?>&nbsp;</P></td>
                                        <td align="center"><p><? echo  implode(",",array_unique(explode(",",$row[csf('stitch_length')]))); ?>&nbsp;</p></td>
                                        <td width="60"  align="center"><p><? echo  implode(",",array_unique(explode(",",$row[csf('width')])));?>&nbsp;</p></td>
                                        <td width="60"  align="center"><p><? echo  implode(",",array_unique(explode(",",$row[csf('gsm')])));?>&nbsp;</p></td>
                                        <?
                                        $row_tot_roll=0; 
                                        $row_tot_qnty=0;  $mc_prod_tot_qnty=0; 
                                        foreach($shift_name as $key=>$val)
                                        {
                                            $row_tot_qnty+=$row[csf('qntyshift'.strtolower($val))]; 
											if($row[csf('receive_basis')]==2)
												{
													if($row[csf('qntyshift'.strtolower($val))]>0)
													{
													$mc_prod_tot_qnty+=$row[csf('qntyshift'.strtolower($val))];
													//$inhouse_mc_prod_ship[$key]+=$row[csf('qntyshift'.strtolower($val))];
													}
												}
                                            ?>
										...
										$mc_prod_tot_qnty=$prog_wise_prod_qty[$row[csf('machine_no_id')]][$row[csf('booking_no')]]['qty'];
                                        ?>
                                        <td align="right" ><? echo number_format($row_tot_qnty,2,'.',''); ?></td>

                                        <?
                                        //if ($vari_knit_charge_source==2) 
                                        //{
                                        	?>
                                        	<td align="right" ><? echo $fso_knitting_rate; //number_format($row[csf('kniting_charge')],2,'.',''); ?></td>
                                        	<td align="right" ><? $charge_amount=$row_tot_qnty*$fso_knitting_rate; 
                                        ...
                                        <td align="right" ><? echo number_format($program_qnty,2,'.',''); ?></td>
                                        <td align="right" ><? echo number_format($mc_prod_tot_qnty,2,'.',''); ?></td>
                                        <td align="right"  title="Target Qty-Prod MC Qty"><? $bal_mc_prod_qty=$program_qnty-$mc_prod_tot_qnty; echo number_format($bal_mc_prod_qty,2,'.',''); ?></td>
										<?
                                        if($temp_arr[$row[csf('machine_no_id')]][$row[csf('receive_date')]]=="" )
                                        {
                                            ...
                                            ?>
                                            <td valign="top" align="right" rowspan="<? echo $machine_inhouse_array[$row[csf('machine_no_id')]][$row[csf('receive_date')]]; ?>"><? //echo number_format($machine_inhouse_qty[$row[csf('machine_no_id')]],2,'.',''); ?> </td>
                                            <?
										...
										<?
                                        $inhouse_tot_qty+=$row_tot_qnty; $inhouse_bal_mc_prod_qty+=$bal_mc_prod_qty;
                                        $inhouse_tot_charge_amount+=$charge_amount;
										$program_qnty_tot_qty+=$program_qnty;$inhouse_mc_prod_tot_qnty+=$mc_prod_tot_qnty;
                                        $grand_tot_qnty+=$row_tot_qnty;  $grand_tot_program_qnty+=$program_qnty;
										$grand_tot_mc_qnty+=$mc_prod_tot_qnty; 
								...
								else // In-house
								{
									...
									if($row[csf('machine_no_id')] != 0)
									{
										...
										{
											$machine_dia_gage=$knit_plan_arr[$row[csf('booking_no')]]['machine_dia']." X ".$knit_plan_arr[$row[csf('booking_no')]]['machine_gg'];
											$program_qnty=$machine_wise_plan_arr[$row[csf('booking_no')]][$row[csf('machine_no_id')]]['program_qnty'];
											//echo $program_qnty.'='.$row[csf('booking_no')].'='.$row[csf('machine_no_id')].'<br>';
										}
										else
										{
											$machine_dia_gage=$machine_details[$row[csf('machine_no_id')]]['dia_width']." X ".$machine_details[$row[csf('machine_no_id')]]['gauge'];
										...
										<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" valign="top">
											...
											<td align="center" style="width: 40px;"><? echo $km; ?></td>
												<td align="center"><? echo ($row[csf('within_group')]==1)?"Yes":"No"; ?></td>
												<td align="center"><p><? echo $knitting_party; ?></p></td>
												<td align="center" title="Rec Basis=<? echo $row[csf('receive_basis')].',MC ID='.$row[csf('machine_no_id')];?>"><p><? echo $machine_arr[$row[csf('machine_no_id')]]; ?></p></td>
												<td align="center"><p><? echo $machine_brand; ?></p></td>
											<td align="center"><p><? if($row[csf('receive_date')]!="" && $row[csf('receive_date')]!="0000-00-00") echo change_date_format($row[csf('receive_date')]); ?>&nbsp;</p></td>
											<td align="center"><p><? echo $machine_dia_gage; ?></p></td>
											<td align="center"><p><? if($row[csf('within_group')]==1){echo $company_arr[$row[csf('unit_id')]];}else{echo $company_arr[$row[csf('company_id')]];} ?></p></td>
											<td width="70" align="center"><p><? echo $buyer_id; ?></></td>
	                                        ...
	                                        ?>
                                            <td align="right" ><? echo number_format($program_qnty,2,'.',''); ?></td>
                                            <td align="right" title="<? echo $row[csf('receive_basis')];?>" ><? echo number_format($mc_prod_tot_qnty,2,'.',''); ?></td>
                                            <td align="right" title="Target Qty-Prod MC Qty" ><? $bal_prod_qty=$program_qnty-$mc_prod_tot_qnty;echo number_format($bal_prod_qty,2,'.',''); ?></td>
											<?
											if($temp_arr[$row[csf('machine_no_id')]][$row[csf('receive_date')]]=="" )
											{
												$temp_arr[$row[csf('machine_no_id')]][$row[csf('receive_date')]]=$row[csf('receive_date')];														
												?>
												<td valign="top" align="right" rowspan="<? echo $machine_inhouse_array[$row[csf('machine_no_id')]][$row[csf('receive_date')]]; ?>"><? echo number_format($machine_inhouse_qty[$row[csf('machine_no_id')]][$row[csf('receive_date')]],2,'.',''); ?> </td>
												<?
												
												$grand_machine_total+=$machine_inhouse_qty[$row[csf('machine_no_id')]][$row[csf('receive_date')]];
												$machine_total_inhouser+=$machine_inhouse_qty[$row[csf('machine_no_id')]][$row[csf('receive_date')]];
											}
											?>
											<td align="right"><p><? echo number_format($row[csf('reject_qty')],2,'.',''); ?></p></td>
											<td><p><? echo $row[csf('remarks')]; ?>&nbsp;</p></td>
											...
											<?
											$inhouse_tot_qty+=$row_tot_qnty;$program_qnty_tot_qty+=$program_qnty;$inhouse_mc_prod_tot_qnty+=$mc_prod_tot_qnty;
											$inhouse_tot_charge_amount+=$charge_amount;
											$grand_tot_qnty+=$row_tot_qnty;$grand_bal_prod_tot_qnty+=$bal_prod_qty; $inhouse_bal_mc_prod_qty+=$bal_prod_qty;
											$grand_tot_program_qnty+=$program_qnty;  $grand_tot_mc_qnty+=$mc_prod_tot_qnty; 
											$grand_tot_charge_amount+=$charge_amount;
											$grand_reject_qty += $row[csf('reject_qty')];
											$i++;
							...
							<tr class="tbl_bottom">
                                ...
                                <td align="right"><? echo number_format($program_qnty_tot_qty,2,'.',''); ?></td>
                                <td align="right"><? echo number_format($inhouse_mc_prod_tot_qnty,2,'.',''); ?></td>
                                <td align="right"><? $inhouse_bal_prod_tot_qnty=$program_qnty_tot_qty-$inhouse_mc_prod_tot_qnty;
												echo number_format($inhouse_bal_prod_tot_qnty,2,'.',''); ?></td>
								<td align="right"><? //echo number_format($machine_total_inhouser,2,'.',''); ?></td>
								<td align="right"><? echo number_format($grand_reject_qty,2,'.',''); ?></td>
								<td>&nbsp;</td>
							</tr>	
						...
						if($cbo_booking_type==0) // In-Bound Subcontract
						{
							if (count($nameArray_inhouse_subcon)>0)
	                        {
	                            ...
	                            foreach ($nameArray_inhouse_subcon as $row)
	                            {
									...
									if($row[csf('machine_no_id')] != 0)
									{
										...
										$knitting_party=$company_arr[$row[csf('knitting_company')]];
											...
											$knitting_party="&nbsp;";
										$main_gsm=implode(",",array_unique(explode(",",$row[csf('gsm')])));
										$main_diaWidth=implode(",",array_unique(explode(",",$row[csf('width')])));
										$main_color=implode(",",array_unique(explode(",",$row[csf('color_id')])));

										$sub_programNo= $progranNoArr[$row[csf("job_no")]][$row[csf("buyer_id")]][$main_gsm][$main_diaWidth][$main_color][$machine_dia][$machine_gg]['programNo'];
										$macineDistributeQnty= $progranMachineDistriQntyArr[$row[csf("job_no")]][$row[csf("buyer_id")]][$main_gsm][$main_diaWidth][$main_color][$machine_dia][$machine_gg][$row[csf('machine_no_id')]][$sub_programNo]['distribution_qnty'];
										...
										<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" valign="top">
	                                        ...
	                                        <td align="center">&nbsp;</td>
											 <td align="center"><? echo $knitting_party; ?></td>
	                                        <td align="center"><p><? echo $machine_arr[$row[csf('machine_no_id')]]; ?></p></td>
	                                        <td align="center"><p><? echo $machine_brand; ?></p></td>
											<td align="center"><p><? if($row[csf('receive_date')]!="" && $row[csf('receive_date')]!="0000-00-00") echo change_date_format($row[csf('receive_date')]); ?>&nbsp;</p></td>
											<td align="center"><p><? echo $machine_dia_gage; ?></p></td>
											<td align="center"><p><? echo $company_arr[$row[csf('company_id')]];?></p></td>
											<td width="70" align="center"><p><? echo $buyer_id; ?></p></td>
											<td align="center"><? 
											//echo $row[csf("job_no")]."**".$row[csf("buyer_id")]."**".$main_gsm."**".$main_diaWidth."**".$main_color."**".$machine_dia."**".$machine_gg."===";
											echo "P:". $sub_programNo."<br/>"; echo $row[csf('job_no')]; ?>&nbsp;</P></td>
											<td width="70" align="center"><p></p></td>
											<td width="100" align="center"><p><? //echo $buyer_id; ?></p></td>
                                            <td width="100" align="center"><p><? //echo $buyer_id; ?></p></td>
											<td align="center">&nbsp;</td>
											<td align="center"><? echo $order_nos;?></td>
											<td align="center"><p><? echo $count; ?></p></td>
											<td><P><? $brand_data=implode(",",array_unique(explode(",",$row[csf('brand_id')])));
											echo $brand_data; ?>&nbsp; </P></td>
											<td align="center"><p><? //echo $count; ?></p></td>
											<td align="center"><P><? echo implode(",",array_unique(explode(",",$row[csf('yarn_lot')]))); ?>&nbsp;</P></td>
											<td colspan="2"><P>
											<?
	                                        ...
	                                        //{
	                                        	...
	                                        	<td align="right" ><? echo number_format($row[csf('rate')]*$row_tot_qnty,2,'.',''); ?></td>
	                                        	<?
	                                        //}
	                                        ?>
                                            <td align="right" ><? echo number_format($macineDistributeQnty,2,'.',''); ?></td>
                                            <td align="right" ><? echo number_format($row_tot_qnty,2,'.',''); ?></td>
                                            <td align="right" ><? echo number_format($macineDistributeQnty-$row_tot_qnty,2,'.',''); ?></td>
											<?
											if($temp_arr[$row[csf('machine_no_id')]][$row[csf('receive_date')]]=="" )
											{
												$temp_arr[$row[csf('machine_no_id')]][$row[csf('receive_date')]]=$row[csf('receive_date')];														
												...
												$grand_machine_total+=$machine_inhouse_qty[$row[csf('machine_no_id')]][$row[csf('receive_date')]];
												$machine_total_inbound+=$machine_inhouse_qty[$row[csf('machine_no_id')]][$row[csf('receive_date')]];
											}
											?>
											<td align="right"><p><? echo number_format($row[csf('reject_qty')],2,'.',''); ?></p></td>
											<td><p><? echo $row[csf('remarks')]; ?>&nbsp;</p></td>
	                                    ...
	                                    $inbound_tot_amount+=$row[csf('rate')]*$row_tot_qnty;
	                                    $grand_tot_charge_amount+=$row[csf('rate')]*$row_tot_qnty; 
										$inboundreject_qty += $row[csf('reject_qty')];
										$balQntyMcDistri_qty += $macineDistributeQnty-$row_tot_qnty;
	                                    $grand_reject_qty += $row[csf('reject_qty')];
	                                    $tot_macineDistributeQnty += $macineDistributeQnty;
	                                    $grand_macineDistributeQnty += $macineDistributeQnty;
	                                    $i++;
									}
									$z++;
	                            }
								...
								<tr class="tbl_bottom">
	                                ...
	                                //}
	                                ?>
                                    <td align="right"><? echo number_format($tot_macineDistributeQnty,2,'.',''); ?></td>
                                    <td align="right"><? echo number_format($inbound_tot_qty,2,'.',''); ?></td>
                                    <td align="right"><? echo number_format($balQntyMcDistri_qty,2,'.',''); ?></td>
                                    
									<td align="right"><? echo number_format($machine_total_inbound,2,'.',''); ?></td>
                ...
                <tfoot>
                    <tr>
                        ...
                        //}
                        ?>
                        <th><? echo number_format($grand_tot_program_qnty+$grand_macineDistributeQnty,2,'.',''); ?></th>
                         <th><? echo number_format($grand_tot_mc_qnty+$inbound_tot_qty,2,'.',''); ?></th>
                        <th><? echo number_format(($grand_tot_program_qnty-$grand_tot_mc_qnty+$balQntyMcDistri_qty),2,'.',''); ?></th>
                        <th align="right"><? echo number_format($grand_machine_total,2,'.',''); ?></th>
	...
	else //Machine Wise
	{
		...
		$tbl_width=2700+$width+count($shift_name)*60;
		ob_start();
		?>
		<table cellpadding="0" cellspacing="0" width="<? echo $tbl_width; ?>">
			<tr>
		...
		<table>
			...
			$machine_inhouse_array=$machine_inhouse_subcon_array=$machine_inhouse_subcon_qty=$machine_outbound_array=$machine_inhouse_qty=$machine_outbound_qty=$total_running_machine=$buyer_wise_production_arr=array();
			foreach ($nameArray_inhouse as $row)
			{
				if($row[csf('machine_no_id')] != 0)
				{
				...
				if($row[csf("knitting_source")]==3)
				{
					...
					$machine_outbound_array[$row[csf('machine_no_id')]][$row[csf('receive_date')]]++;
					foreach($shift_name as $key=>$val)
					{
						$machine_outbound_qty[$row[csf('machine_no_id')]][$row[csf('receive_date')]]+=$row[csf('qntyshift'.strtolower($val))];
					}
				}

				$buyer_id = ($row[csf("within_group")]==1)?$booking_arr[$row[csf("sales_booking_no")]]:$buyer_arr[$row[csf("buyer_id")]];
				//$buyer_wise_production_arr[$buyer_id][$row[csf("knitting_source")]][$row[csf("booking_type")]][$row[csf("within_group")]] += ($row[csf("qntyshifta")]+$row[csf("qntyshiftb")]+$row[csf("qntyshiftc")]+$row[csf("without_shift")]);
				$buyer_wise_production_arr[$buyer_id][$row[csf("knitting_source")]][$row[csf("booking_type")]][$row[csf("within_group")]][$row[csf("is_order")]] += ($row[csf("qntyshifta")]+$row[csf("qntyshiftb")]+$row[csf("qntyshiftc")]+$row[csf("without_shift")]);
				$booking_nos .= $row[csf("booking_no")] . ",";
			...
			foreach ($nameArray_inhouse_subcon as $row)
			{
				if($row[csf('machine_no_id')] != 0)
				{
					...
					$machine_inhouse_subcon_array[$row[csf('machine_no_id')]][$row[csf('receive_date')]]++;
					foreach($shift_name as $key=>$val)
					{
						$machine_inhouse_subcon_qty[$row[csf('machine_no_id')]][$row[csf('receive_date')]]+=$row[csf('qntyshift'.strtolower($val))];
					}
				}
			...
			/*foreach($buyer_wise_production_arr as $buyer => $rows)
			{
				if ($k%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
				$out_bound_qnty=0;
				$out_bound_qnty_wg_yes=$rows[3][1][1]+$service_buyer_data[$buyer];
				$out_bound_qnty_wg_no=$rows[3][1][2]+$service_buyer_data[$buyer];						
				$sample_with_order=$rows[1][4][1]+$rows[3][4][1];
				$tot_summ=$rows[1][1][1]+$rows[1][1][2]+$rows[3][1][1]+$rows[3][1][2]+$subcon_buyer_samary[$buyer]+$sample_with_order;
				//$buyer_id = ($row[csf("within_group")]==1)?$booking_arr[$row[csf("sales_booking_no")]]:$row[csf("buyer_id")];
				?>
				<tr bgcolor="<? echo $bgcolor; ?>">
					<td align="center" style="width: 40px;"><? echo $k; ?></td>
					<td align="center" title="PO Buyer-<? echo $buyer;?>"><? echo $buyer; ?></td>
					<td align="right" style="width: 45px;"><? echo number_format($rows[1][1][1],2,'.',''); ?></td>
					<td align="right" style="width: 45px;"><? echo number_format($rows[1][1][2],2,'.',''); ?></td>
					<td align="right"><? echo number_format($out_bound_qnty_wg_yes,2,'.',''); ?></td>
					<td align="right"><? echo number_format($out_bound_qnty_wg_no,2,'.',''); ?></td>
					<td align="right"><? echo  number_format($sample_with_order,2,'.',''); ?></td>
					<td align="right"><? echo number_format($subcon_buyer_samary[$buyer],2,'.','');?></td>
					<td align="right"><? echo  number_format($tot_summ,2,'.',''); ?></td>
				</tr>
				<?
				$tot_qtyinhouse_wg_yes += $rows[1][1][1];
				$tot_qtyinhouse_wg_no += $rows[1][1][2];
				$tot_qtyoutbound_wg_yes += $out_bound_qnty_wg_yes;
				$tot_qtyoutbound_wg_no += $out_bound_qnty_wg_no;
				$tot_sample_with_order += $sample_with_order;
				$total_summ += $tot_summ;
			...
			<tr>
				<td width="640" style="margin:5px;">
			        <div align="left" style="background-color:#E1E1E1; color:#000; font-size:14px; font-family:Georgia, 'Times New Roman', Times, serif; width: 725px;"><strong><u><i>In-House + Outbound + Inbound [Knitting Production]</i></u></strong></div>
			        <div style="width:725px; overflow-y:scroll; max-height:220px;" id="scroll_body1">
			        <table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table">
			            <thead>
			                <tr>
			                    <th colspan="10">Knit Production Summary (In-House + Outbound + Inbound)</th>
			    ...
			    <td width="660">
			    	<div align="left" style="background-color:#E1E1E1; color:#000; width:580px; font-size:14px; font-family:Georgia, 'Times New Roman', Times, serif;"><strong><u><i>Floor Wise Knitting Production Summary (In-House + Outbound + SubCon)</i></u></strong></div>
            ...
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $tbl_width; ?>" class="rpt_table" id="table_head" >
                <thead>
                    <tr>
                        ...
                        <th width="60" rowspan="2">GSM</th>
                        <th colspan="<? echo count($shift_name); ?>">Production</th>
                        <th width="100" rowspan="2">Without Shift(Outbound)</th>
                        <th width="100" rowspan="2">Shift Total</th>
                        <?
                        if ($vari_knit_charge_source==2) 
                        {
                       ...
                       // echo count($nameArray_inhouse);
						if (count($nameArray_inhouse)>0)
                        {
                          // echo $row[csf("knitting_source")].'D'.$row[csf("body_part_id")];
                            ...
                            foreach ($nameArray_inhouse as $row)
                            {
								...
								if($row[csf("knitting_source")]==3) // Out-bound
								{
									if($p==1)
									{
										?>
                                        <tr class="tbl_bottom">
                                            <td colspan="24" align="right"><b>In-house Total(with order)</b></td>
                                            <?
                                            foreach($shift_name as $key=>$val)
                                            {
                                                ?>
                                        ...
                                        <tr  bgcolor="#CCCCCC">
                                            <td colspan="<? echo $ship_count+31+$colspan; ?>" align="left" ><b>Outbound Subcontract</b></td>
                                        </tr>
                                        <?
                                        $total_grand_rejectQnty+=$grand_reject_qty;
										$inhouse_ship=array();$inhouse_tot_qty=$machine_total_inhouser=$grand_reject_qty=$inhouse_tot_charge_amount=0;
									}
                                    ...
                                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" valign="top">
                                        ...
                                        <td align="center"><p><? if($row[csf('within_group')]==1){echo $company_arr[$row[csf('unit_id')]];}else{echo $company_arr[$row[csf('company_id')]];} ?></p></td>
                                        <td width="70" align="center"><p><? echo $buyer_id; ?></></td>
                                        <td align="center"><? echo $row[csf('booking_no')] . "<br /><strong>S:</strong>" . $row[csf('job_no')]; ?>
                                        </td>
										<td width="100" align="center"><p><? echo $body_partName; ?></></td>
                                        <td align="center">
                                            <? 
                                        ...
                                        ?>
                                        <td align="right" ><? echo number_format($row[csf('without_shift')],2,'.',''); ?></td>
                                        <td align="right" ><? echo number_format($row_tot_qnty+$row[csf('without_shift')],2,'.',''); ?></td>
                                        <?
                                        if ($vari_knit_charge_source==2) 
                                        {
                                        	?>
                                        ...
                                        ?>
										<?
                                        if($temp_outbound_arr[$row[csf('machine_no_id')]][$row[csf('receive_date')]]=="" )
                                        {
                                            $temp_outbound_arr[$row[csf('machine_no_id')]][$row[csf('receive_date')]]=$row[csf('receive_date')];														
                                            ?>
                                            <td valign="top" align="right" rowspan="<? echo $machine_outbound_array[$row[csf('machine_no_id')]][$row[csf('receive_date')]]; ?>"><? //echo number_format($machine_outbound_qty[$row[csf('machine_no_id')]],2,'.',''); ?></td>
                                            <?
                                            
                                            //$grand_machine_total+=$machine_outbound_qty[$row[csf('machine_no_id')]];
                                           // $machine_total_inhouser+=$machine_outbound_qty[$row[csf('machine_no_id')]];
                                        }
                                        ?>
                                        <td width="100" align="center"><p><? echo $lib_userArr[$row[csf('inserted_by')]]; ?>&nbsp;</p></td>
								...
								else // In-house
								{
									if($q==1)
									{
										?>
                                        <tr  bgcolor="#CCCCCC">
                                            <td colspan="<? echo $ship_count+31+$colspan; ?>" align="left" ><b>In-House</b></td>
                                        </tr>
                                        <?
									}
									if($row[csf('machine_no_id')] != 0)
									{
										...
										<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" valign="top">
	                                        ...
	                                        ?>
											<?
											if($temp_inhouse_arr[$row[csf('machine_no_id')]][$row[csf('receive_date')]]=="" )
											{
												$temp_inhouse_arr[$row[csf('machine_no_id')]][$row[csf('receive_date')]]=$row[csf('receive_date')];														
												?>
												<td valign="top" align="right" rowspan="<? echo $machine_inhouse_array[$row[csf('machine_no_id')]][$row[csf('receive_date')]]; ?>"><? echo number_format($machine_inhouse_qty[$row[csf('machine_no_id')]][$row[csf('receive_date')]],2,'.',''); ?> </td>
												<?
							...
							?> 
							<tr class="tbl_bottom">
								<td colspan="24" align="right"><b>Outbound Total(with order)</b></td>
								<?
								foreach($shift_name as $key=>$val)
								{
									?>
								...
								?>
								<td align="right" title="without shift"><? echo number_format($outBoundWithoutShiftQnty,2,'.',''); ?></td>
							...
							if (count($nameArray_inhouse_subcon)>0)
	                        {
	                            ...
	                            foreach ($nameArray_inhouse_subcon as $row)
	                            {
									if($z==1)
									{
										?>
										<tr  bgcolor="#CCCCCC">
											<td colspan="<? echo $ship_count+31+$colspan; ?>" align="left" ><b>In-Bound Subcontract</b></td>
										</tr>
										<?
									}
									if($row[csf('machine_no_id')] != 0)
									{
										...
										$machine_brand = $machine_details[$row[csf('machine_no_id')]]['brand'];
										if($row[csf('knitting_source')]==1 || $row[csf('knitting_source')]==2)
											$knitting_party=$company_arr[$row[csf('knitting_company')]];
										else if($row[csf('knitting_source')]==3)
											$knitting_party=$supplier_arr[$row[csf('knitting_company')]];
										else
										...
										<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" valign="top">
											...
											<td align="center"><p><? echo $company_arr[$row[csf('company_id')]];?></p></td>
											<td width="70" align="center"><p><? echo $buyer_id; ?></></td>
											<td align="center"><? echo $row[csf('job_no')]; ?>&nbsp;</P></td>
											<td width="100" align="center"><p><? //echo $buyer_id; ?></></td>
											<td align="center">&nbsp;</td>
											<td align="center"><? echo $order_nos;?></td>
											<td align="center"><p><? echo $count; ?></p></td>
											<td><P><? $brand_data=implode(",",array_unique(explode(",",$row[csf('brand_id')])));
	                                        ...
	                                        ?>
											<?
											if($temp_subcon_arr[$row[csf('machine_no_id')]][$row[csf('receive_date')]]=="" )
											{
												$temp_subcon_arr[$row[csf('machine_no_id')]][$row[csf('receive_date')]]=$row[csf('receive_date')];														
												?>
												<td valign="top" align="right" rowspan="<? echo $machine_inhouse_subcon_array[$row[csf('machine_no_id')]][$row[csf('receive_date')]]; ?>"><? echo number_format($machine_inhouse_subcon_qty[$row[csf('machine_no_id')]][$row[csf('receive_date')]],2,'.',''); ?> </td>
												<?
												
												$grand_machine_total+=$machine_inhouse_subcon_qty[$row[csf('machine_no_id')]][$row[csf('receive_date')]];
												$machine_total_inbound+=$machine_inhouse_subcon_qty[$row[csf('machine_no_id')]][$row[csf('receive_date')]];
											}
											?>
                                            <td width="100" align="center"><p><? echo $lib_userArr[$row[csf('inserted_by')]]; ?>&nbsp;</p></td>
	                                  	  	<td width="100" align="center"><p><? echo $row[csf('insert_date')]; ?>&nbsp;</p></td>
								...
								?> 
								<tr class="tbl_bottom">
									<td colspan="24" align="right"><b>Inbound Total</b></td>
									<?
									foreach($shift_name as $key=>$val)
									{
										?>
                ...
                <tfoot>
                    <tr>
                        ...
                        ?>
                        <th align="right" title="without shift"><? echo number_format($outBoundWithoutShiftQnty,2,'.',''); ?></th>
