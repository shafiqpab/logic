            ...
            <table width="800" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr class="general">
                        ...
                        <td align="center">
                        	<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view (document.getElementById('cbo_company_name').value+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+'<? echo $cbo_year; ?>', 'job_popup_search_list_view', 'search_div', 'color_wise_rmg_production_status_controller', 'setFilterGrid(\'table_body2\',-1)');" style="width:100px;" />
                    </td>
                    </tr>
            	</tbody>
           	</table>
...
{ 
	...
	if($type==1) // show button
	{
		...
		$sql_cond .= ($working_company_id !=0) ? " and d.serving_company=$working_company_id" : "";
		...
		where a.id=b.job_id and b.id=c.po_break_down_id and d.po_break_down_id=b.id and d.po_break_down_id=c.po_break_down_id and d.id=e.mst_id and c.id=e.color_size_break_down_id and a.id=c.job_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1  and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1  and e.is_deleted=0  $sql_cond and d.production_type in(1,2,3,4,5,8)
		group by a.job_no, a.company_name, a.buyer_name, a.style_ref_no, b.id, b.po_number,  c.item_number_id,c.color_number_id,d.serving_company,d.location,d.production_type,d.sewing_line,d.prod_reso_allo,b.grouping order by b.id desc";
		...
		foreach($sql_res as $vals)
		{
			...
			$data_array[$vals["STYLE_REF_NO"]][$vals["JOB_NO"]][$vals["PO_ID"]][$vals["ITEM_ID"]][$vals["COLOR_ID"]]["grouping"] = $vals["GROUPING"];
			...
			{			 
				...
				$chk_arr[$vals["STYLE_REF_NO"]][$vals["JOB_NO"]][$vals["PO_ID"]][$vals["ITEM_ID"]][$vals["COLOR_ID"]] = "kakku";
			}
			...
			<div>		
		        <table width="<? echo $tbl_width;?>" cellpadding="0" cellspacing="0"> 
		            <tr class="form_caption">
		            	<td colspan="<? echo $colspan;?>" align="center"><strong>Color Wise RMG Production Status</strong></td> 
		            </tr>
		            <tr class="form_caption">
									...
									foreach ($item_data as $color_id => $row) 
									{
										...
										if($row['total_cutting'] >0 || $row['today_cutting'] >0 || $row['today_sewing_input'] >0 || $row['today_sewing_output'] >0 || $row['today_issue_to_wash'] >0 || $row['today_rcv_frm_wash'] >0)
										{
											...
											<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>" style="font-size:11px">
												...
												<td rowspan="<? echo $rowspan[$po_id][$item_id];?>" width="100"><? echo $row['grouping'];?></td>
	...
	elseif ($type==3) // short button
	{
		...
		sum(c.order_quantity) as ORDER_QUANTITY,b.grouping, 
		sum(case when d.production_type=1 and e.production_type=1 and d.production_date<=$txt_production_date then e.production_qnty else 0 end ) as TOTAL_CUTTING ,
		sum(case when d.production_type=2 and e.production_type=2 and d.embel_name=3 and d.production_date<=$txt_production_date then e.production_qnty else 0 end ) as TOTAL_ISSUE_TO_WASH ,
		sum(case when d.production_type=3 and e.production_type=3 and d.embel_name=3 and d.production_date<=$txt_production_date then e.production_qnty else 0 end ) as TOTAL_RCV_FRM_WASH ,
		...
		from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c,pro_garments_production_mst d,pro_garments_production_dtls e
		where a.id=b.job_id and b.id=c.po_break_down_id and d.po_break_down_id=b.id and d.po_break_down_id=c.po_break_down_id and d.id=e.mst_id and c.id=e.color_size_break_down_id and a.id=c.job_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1  and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1  and e.is_deleted=0  $sql_cond and d.production_type in(1,2,3,4,5,8)
		group by a.job_no, a.company_name, a.buyer_name, a.style_ref_no, b.id, b.po_number,  c.item_number_id,c.color_number_id,d.serving_company,d.location,d.sewing_line,d.prod_reso_allo,b.grouping  order by b.id desc";
		...
		foreach($sql_res as $vals)
		{
			...
			$data_array[$vals["STYLE_REF_NO"]][$vals["JOB_NO"]][$vals["PO_ID"]][$vals["ITEM_ID"]][$vals["COLOR_ID"]][$sewing_line]["job_no"] = $vals["JOB_NO"];		
			$data_array[$vals["STYLE_REF_NO"]][$vals["JOB_NO"]][$vals["PO_ID"]][$vals["ITEM_ID"]][$vals["COLOR_ID"]][$sewing_line]["grouping"] = $vals["GROUPING"];		 
	...
	elseif ($type==5) // Cutting status button
	{
		...
		from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c,pro_garments_production_mst d,pro_garments_production_dtls e
		where a.id=b.job_id and b.id=c.po_break_down_id and d.po_break_down_id=b.id and d.po_break_down_id=c.po_break_down_id and d.id=e.mst_id and c.id=e.color_size_break_down_id and a.id=c.job_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1  and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1  and e.is_deleted=0  $sql_cond and d.production_type in(1,4) and e.production_qnty>0
		group by a.job_no, a.company_name, a.buyer_name, a.style_ref_no, b.id, b.po_number,c.item_number_id,c.color_number_id,d.serving_company,d.location,d.production_type,d.remarks,d.sewing_line,d.prod_reso_allo,b.grouping";
		...
		foreach($sql_res as $vals)
		{
			...
			else
			{
				...
				$data_array_without_line[$vals["STYLE_REF_NO"]][$vals["JOB_NO"]][$vals["PO_ID"]][$vals["ITEM_ID"]][$vals["COLOR_ID"]][$sewing_line]["grouping"] = $vals["GROUPING"];			
			...
			// if($chk_arr[$vals["STYLE_REF_NO"]][$vals["JOB_NO"]][$vals["PO_ID"]][$vals["ITEM_ID"]][$vals["COLOR_ID"]]=="")
			$others_date_array[$vals["STYLE_REF_NO"]][$vals["JOB_NO"]][$vals["PO_ID"]][$vals["ITEM_ID"]][$vals["COLOR_ID"]][$sewing_line]["today_cutting"]+=$vals["TODAY_CUTTING"];
			$others_date_array[$vals["STYLE_REF_NO"]][$vals["JOB_NO"]][$vals["PO_ID"]][$vals["ITEM_ID"]][$vals["COLOR_ID"]][$sewing_line]["total_cutting"]+=$vals["TOTAL_CUTTING"];			
			
			$po_id_array[$vals["PO_ID"]] = $vals["PO_ID"];
		}
		// echo "<pre>";print_r($others_date_array);die;
		$poIds = implode(",", $po_id_array);
										...
										foreach ($color_data as $line_name => $row) 
										{
											if($others_date_array[$style][$job][$po_id][$item_id][$color_id]['total_cutting']>0 || $row['total_sewing_input']>0 || $row['today_sewing_input']>0)
											{
												...
												<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>" style="font-size:11px">
													...
													<td valign="middle" rowspan="<? echo $rowspan[$po_id][$item_id];?>" width="100"><p><? echo $row['grouping'];?></p></td>
										...
										foreach ($color_data as $line_name => $row) 
										{
											if($others_date_array[$style][$job][$po_id][$item_id][$color_id]['total_cutting']>0 || $row['total_sewing_input']>0 || $row['today_sewing_input']>0)
											{
												...
												<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>" style="font-size:11px">
													...
													<td valign="middle" rowspan="<? echo $rowspan_without_line[$po_id][$item_id];?>" width="100"><p><? echo $row['grouping'];?></p></td>
