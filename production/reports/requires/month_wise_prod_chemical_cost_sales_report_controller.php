...
//die;
	...
	//echo "SELECT a.MST_ID ,a.REQUISITION_NO,b.BATCH_NO,a.ITEM_CATEGORY,a.PROD_ID,a.CONS_QUANTITY,a.CONS_AMOUNT from inv_transaction a,inv_issue_master b where a.mst_id=b.id and b.entry_form=5 and b.is_deleted=0 and b.status_active=1 and a.status_active=1 and a.transaction_type=2 $requisition_id_cond   ";
	...
	// print_r($issue_item_cat_amt_dtls_arr);// count($batch_all_id_array);die;
...
//	$company_cond = str_replace("a.company_id", "c.company_id", $company_id);	
	...
	if ($w_companyID==0) $wo_company_idcond =""; else $wo_company_idcond =" and p.working_company_id='$w_companyID'";
	
	
	  $sql = "(SELECT p.id as recipe_id,p.buyer_id,p.total_liquor,p.recipe_type,p.surplus_solution,p.pickup,a.avg_rate_per_unit, p.batch_id, p.entry_form,p.batch_qty,c.batch_no, a.id, a.item_category_id, a.item_group_id, a.sub_group_name, a.item_description, a.item_size,a.current_stock, a.unit_of_measure,b.prod_id, b.sub_process_id, b.dose_base, b.ratio, b.seq_no,b.sub_seq, b.new_batch_weight, b.new_total_liquor,b.liquor_ratio as liquor_ratio_dtls,b.total_liquor as total_liquor_dtls, b.item_lot, b.store_id ,b.comments
		from pro_recipe_entry_mst p, product_details_master a, pro_recipe_entry_dtls b,pro_batch_create_mst c  
		...
		union 
		(
		SELECT p.id as recipe_id,p.buyer_id,p.total_liquor,p.recipe_type,p.surplus_solution,p.pickup,0 as avg_rate_per_unit, p.batch_id, p.entry_form,p.batch_qty,c.batch_no, b.prod_id as id, null as item_category_id, null as item_group_id, null as sub_group_name, null as item_description, null as item_size,null as current_stock, null as unit_of_measure,b.prod_id, b.sub_process_id, b.dose_base, b.ratio, b.seq_no,b.sub_seq, b.new_batch_weight, b.new_total_liquor,b.liquor_ratio as liquor_ratio_dtls,b.total_liquor as total_liquor_dtls, b.item_lot, b.store_id ,b.comments
			from pro_recipe_entry_mst p, pro_recipe_entry_dtls b,pro_batch_create_mst c  
	...
	foreach ($res as $row) 
	{
		...
		$batch_job_buyer_arr[$row[csf('batch_id')]]['buyer_name']=$row[csf('buyer_id')];
	...
	<div style="width:1160px; margin-left:30px" id="report_div">
     <!--<div style="width:870px;" align="center"><input  type="button" value="Print Preview" onClick="print_window()" style="width:100px"  class="formbutton"/></div>-->
    <div style="width:1160px; font-family:'Arial Narrow'; font-size:14px;">Total Dyes & Chemical Cost Dtls</div>
    <table align="center" cellspacing="0" width="1160" border="1" rules="all" class="rpt_table" >
        <thead align="center">
    	   <tr>
                <th width="20">SL</th>
		...
		//$req_no=$issue_data_arr[$batch_id]['REQUISITION_NO'];
			$req_no=rtrim($batch_req_id_array[$batch_id],',');
			$req_noArr=array_unique(explode(",",$req_no));
			$batch_req_noArr=array();
			foreach($req_noArr as $rid)
			{
				$batch_req_noArr[]=$batch_req_no_array[$rid];
			}
		$ISSUE_NUMBERArr=rtrim($issue_data_arr[$batch_id]['ISSUE_NUMBER'],',');
		$ISSUE_NUMBERs=implode(",",array_unique(explode(",",$ISSUE_NUMBERArr)));
		//$requ_no_arr[$batch_id]['req_no'];
		//echo $requ_no_arr[$req_no]['req_no'].', ';
		$issue_wgt_perQty=0;
		foreach(array_unique(explode(",",$ISSUE_NUMBERArr)) as $issueId)
		{
		$issue_wgt_perQty+=$issue_data_arr[$issueId];
		}
		
		//$requ_noss=rtrim($batch_req_no_array[$req_no],','); 
		$requ_nos=implode(",",$batch_req_noArr);
	//	$requ_nos=implode(", ",array_unique(explode(",",$requ_no)));
		
		$ref_no=rtrim($batch_ref_no_arr[$batch_id]['ref_no'],',');
				
		$issue_dateAll=rtrim($issue_data_arr[$batch_id]['ISSUE_DATE'],',');
		$issue_dateAllArr=array_unique(explode(",",$issue_dateAll));
		$issue_date=min($issue_dateAllArr);
		...
		// if($batch_id==372795) echo $job_no.'='.$buyer_id;
		}
		else
		{
			$buyer_id=$batch_job_buyer_arr[$batch_id]['buyer_name'];	
		...
		$ref_nos=implode(", ",array_unique(explode(",",$ref_no)));
			...
			<tbody>
				<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('trDys_<? echo $i; ?>','<? echo $bgcolor;?>')" id="trDys_<? echo $i; ?>">
					<td align="center"><? echo $i; ?></td>
					<td align="center"><p><? echo $ISSUE_NUMBERs; ?></p></td>
