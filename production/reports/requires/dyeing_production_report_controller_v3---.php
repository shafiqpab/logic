...
// machine_no || '-' || brand as machine_name
	...
	$sql_batch_id=sql_select("select f.batch_id,f.shift_name,f.fabric_type,t.prod_id,t.production_qty from  pro_batch_create_mst a ,pro_fab_subprocess f,lib_machine_name g,pro_fab_subprocess_dtls t where a.id=f.batch_id and f.id=t.mst_id and g.id=f.machine_id  and f.entry_form=35 and  a.batch_against in(1,2,3) and t.entry_page=35 and f.load_unload_id=2 and f.status_active=1 and f.is_deleted=0  and a.status_active=1 $unload_batch_cond2 $dates_com $cbo_prod_type_cond $result_name_cond $shift_name_cond $machine_cond $floor_id_cond $companyCond $workingCompany_name_cond2");
	
	//echo "select f.batch_id,f.shift_name,f.fabric_type,t.prod_id,t.production_qty from  pro_batch_create_mst a ,pro_fab_subprocess f,lib_machine_name g,pro_fab_subprocess_dtls t where a.id=f.batch_id and f.id=t.mst_id and g.id=f.machine_id  and f.entry_form=35 and  a.batch_against in(1,2) and t.entry_page=35 and f.load_unload_id=2 and f.status_active=1 and f.is_deleted=0  and a.status_active=1 $unload_batch_cond2 $dates_com $cbo_prod_type_cond $result_name_cond $shift_name_cond $machine_cond $floor_id_cond $companyCond $workingCompany_name_cond2";die;
	...
	foreach($sql_batch_id as $row_batch)
	{
		...
		{
		$fabric_batch_arr_withOutshift[$row_batch[csf('batch_id')]][$row_batch[csf('fabric_type')]][$row_batch[csf('shift_name')]]['prod_qty']+=$row_batch[csf('production_qty')];
		}
		$tot_row++;
	}//echo $batch_id;die;
	...
	else
	{
		...
		if($cbo_type==2)//   For Order Wise Dyeing Production
		{
			if(str_replace("'",'',$cbo_batch_type)==0 || str_replace("'",'',$cbo_batch_type)==1)
			{
			 ...
			 union
			 (
			 	select a.working_company_id,a.company_id,a.batch_against,a.batch_no, a.batch_weight,a.id, a.color_id, a.extention_no,a.total_trims_weight, SUM(b.batch_qnty) AS batch_qnty,b.item_description, b.prod_id, b.width_dia_type,null as po_number ,null as po_id,null as grouping, null as file_no, null as job_no_prefix_num,h.buyer_id as buyer_name,f.remarks,f.shift_name, f.production_date as process_end_date,f.process_end_date as production_date,
    f.end_hours, f.floor_id, f.hour_unload_meter,f.water_flow_meter, f.end_minutes, f.machine_id,f.load_unload_id, f.fabric_type,f.result,a.booking_no,a.booking_without_order,g.seq_no from pro_batch_create_dtls b, pro_batch_create_mst a, pro_fab_subprocess f, lib_machine_name g,wo_non_ord_samp_booking_mst h where h.booking_no=a.booking_no $companyCond  $booking_no_cond $workingCompany_name_cond2 and f.batch_id=a.id and f.batch_id=b.mst_id  and a.entry_form=0 and g.id=f.machine_id and a.id=b.mst_id and f.entry_form=35 and f.load_unload_id=2 and a.batch_against in(1,2) and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0  and  f.status_active=1 and f.is_deleted=0 $dates_com  $batch_num  $color_name  $buyerdata2 $result_name_cond $shift_name_cond $machine_cond $floor_id_cond $cbo_prod_type_cond $cbo_prod_source_cond GROUP BY a.working_company_id,a.company_id,a.batch_no,a.batch_against, a.batch_weight,a.id,h.buyer_id, a.color_id, a.extention_no, b.item_description, b.prod_id, b.width_dia_type,g.seq_no,
	...
	if($cbo_type==2) // Dyeing Production
	{
		...
		{
			//echo $sql;
			$batchdata=sql_select($sql);$batch_aganist_arr=array(2,3);
			...
			foreach($batchdata as $row)
			{
				...
				$batch_detail_arr[$row[csf('id')]][$row[csf('prod_id')]]['shift_wise_qty']=$row[csf('batch_qnty')];//$shift_wise_prod_arr[$row[csf('id')]][$row[csf('fabric_type')]][$row[csf('prod_id')]]['prod_qty'];//buyer_trims_self_arr
				
				$fabric_withOutshift=$fabric_batch_arr_withOutshift[$row[csf('id')]][$row[csf('fabric_type')]][$row[csf('shift_name')]]['prod_qty'];
				$batch_shift_arr[$row[csf('fabric_type')]][$row[csf('shift_name')]]['without_shift_qty']=$fabric_withOutshift;
				if($shift_fabric_batch_arr[$row[csf('fabric_type')]][$row[csf('shift_name')]]>0)
				{
				$batch_all_shift_arr[$row[csf('fabric_type')]][$row[csf('shift_name')]]['all_shift_qty']=$row[csf('batch_qnty')];//$shift_fabric_batch_arr[$row[csf('fabric_type')]][$row[csf('shift_name')]];//$shift_wise_prod_arr[$row[csf('id')]][$row[csf('fabric_type')]][$row[csf('prod_id')]]['prod_qty'];
				}
				
				
				if($row[csf('batch_against')]==2)//batch_qnty
				{
					$buyer_re_process_arr[$row[csf('buyer_name')]]['re_dyeing_qty']+=$row[csf('batch_qnty')];//$shift_wise_prod_arr[$row[csf('id')]][$row[csf('fabric_type')]][$row[csf('prod_id')]]['prod_qty'];;
					$buyer_re_process_arr[$row[csf('buyer_name')]]['trim']+=$row[csf('total_trims_weight')];
					$total_re_dyeing_qty+=$row[csf('batch_qnty')]+$row[csf('total_trims_weight')];
					//$shift_wise_prod_arr[$row[csf('id')]][$row[csf('fabric_type')]][$row[csf('prod_id')]]['prod_qty'];
					if($row[csf('result')]==1)
					{
						$total_shadematched_shelf_reprocess_qty+=$row[csf('batch_qnty')];//$shift_wise_prod_arr[$row[csf('id')]][$row[csf('fabric_type')]][$row[csf('prod_id')]]['prod_qty'];;
					...
					{
						$buyer_shadematched_arr[$row[csf('buyer_name')]]['batch_qty']+=$row[csf('batch_qnty')];//$shift_wise_prod_arr[$row[csf('id')]][$row[csf('fabric_type')]][$row[csf('prod_id')]]['prod_qty']; 
						$buyer_shadematched_arr[$row[csf('buyer_name')]]['trim']+=$row[csf('total_trims_weight')];
						$total_shadematched_qty+=$row[csf('batch_qnty')];//$shift_wise_prod_arr[$row[csf('id')]][$row[csf('fabric_type')]][$row[csf('prod_id')]]['prod_qty']+$row[csf('total_trims_weight')];
						if($row[csf('batch_against')]!=2)//batch_qnty
						{
						$total_shadematched_shelf_qty+=$row[csf('batch_qnty')];//$shift_wise_prod_arr[$row[csf('id')]][$row[csf('fabric_type')]][$row[csf('prod_id')]]['prod_qty'];
		...
		if(str_replace("'",'',$cbo_batch_type)==0 || str_replace("'",'',$cbo_batch_type)==2)
		{
			...
			foreach($sql_subcon_data as $row)
			{
				...
				$total_subcon_summary+=$row[csf('sub_batch_qnty')];
					...
					if($row[csf('batch_against')]==2)//batch_qnty
					{
					$buyer_re_process_arr[$row[csf('buyer_name')]]['re_dyeing_qty']+=$row[csf('sub_batch_qnty')];
					$buyer_re_process_arr[$row[csf('buyer_name')]]['trim']+=$row[csf('total_trims_weight')];
					$total_re_dyeing_qty+=$row[csf('sub_batch_qnty')]+$row[csf('total_trims_weight')];
					}
				...
				//$subcon_batch_detail_arr[$row[csf('fabric_type')]][$row[csf('shift_name')]]['result']=$row[csf('result')];
				
			}
	//print_r($subcon_batch_detail_arr);
		}
		if(str_replace("'",'',$cbo_batch_type)==0 || str_replace("'",'',$cbo_batch_type)==3) //Batch Aganist - Sample
		{
			...
			foreach($batchdata_sam as $row)
			{
				...
				//$samp_batch_detail_arr[$row[csf('fabric_type')]][$row[csf('shift_name')]]['result']=$row[csf('result')];
				//echo $shift_wise_prod_arr[$row[csf('id')]][$row[csf('fabric_type')]][$row[csf('prod_id')]]['prod_qty'].'=';
				$samp_batch_detail_arr[$row[csf('id')]][$row[csf('prod_id')]]['shift_wise_qty']+=$row[csf('batch_qnty')];//$shift_wise_prod_arr[$row[csf('id')]][$row[csf('fabric_type')]][$row[csf('prod_id')]]['prod_qty'];
				...
				if($row[csf('batch_against')]==2)//batch_qnty
					...
					$buyer_re_process_arr[$row[csf('buyer_name')]]['re_dyeing_qty']+=$row[csf('batch_qnty')];//$shift_wise_prod_arr[$row[csf('id')]][$row[csf('fabric_type')]][$row[csf('prod_id')]]['prod_qty'];
					$buyer_re_process_arr[$row[csf('buyer_name')]]['trim']+=$row[csf('total_trims_weight')];
					$total_re_dyeing_qty+=$row[csf('re_dyeing_qty')]+$row[csf('total_trims_weight')];
					}
				...
				if($row[csf('result')]==1)
					{
						...
						if($row[csf('batch_against')]!=2)//batch_qnty
						{
						$total_shadematched_samp_qty+=$row[csf('batch_qnty')];//$shift_wise_prod_arr[$row[csf('id')]][$row[csf('fabric_type')]][$row[csf('prod_id')]]['prod_qty'];
	...
	if($cbo_type==2) //  Dyeing Production
	{
		...
		<?
			...
			<td width="450" valign="top" align="left" style="margin:5px" colspan="5" >
				<table style="width:450px;border:1px solid #000;" align="center"  class="rpt_table" rules="all" >
                           ...
                           <tbody>
                             <?
								...
								//echo array_sum($buyer_re_process_arr);
								 ...
								 $total_trim_qty=$total_shadematched_shelf_trim_qty+$total_shadematched_subcon_trim_qty+$total_shadematched_samp_trim_qty;
							...
							//$title_hd="Self Qty($total_shadematched_shelf_reprocess_qty),SubconQty($total_shadematched_subcon_re_qty),SampleQty($total_shadematched_samp_re_qty)";
                             ...
                             <tr bgcolor="<? echo $bgcolor; ?>" style="cursor:pointer;">
                               <td align="right"><? echo number_format($total_shadematched_shelf_qty,0); ?></td>
                               ...
                               <td align="right"><? echo number_format($total_trim_qty,0,'.',''); ?></td>
                               <td align="right"> <? echo number_format($total_shadematched_subcon_qty,0,'.',''); ?></td>
							   <td align="right"><? $grandtotal_shade_match_qty=$total_shadeMatch_qty+$tot_reprocess_qty;
                           ...
                           <tfoot>
                             <tr bgcolor="#CCCCCC" style="font-weight:bold;">
                              <th align="left"><b><? echo number_format(($total_shadematched_shelf_qty/$grandtotal_shade_match_qty)*100,0,'.','').'%';?></b></th>
			 ...
			 <td  valign="top" width="230">
                 <table cellpadding="0"  width="230" style="margin:5px" cellspacing="0" align="left"  class="rpt_table" rules="all" border="1">
						...
						<td  valign="top">
                 		<table cellpadding="0"  width="400" style="margin:5px" cellspacing="0" align="left"  class="rpt_table" rules="all" border="1">
							<thead>
								<tr>
								...
								<tr>
									...
									<?
									 ...
									 ?>
									<th>Without Shift Qty </th>
									<th>Total Prod. Qty. </th>
								</tr>
							</thead>
							<tbody>
							<?
								 $k=1;	$total_batch_qty=0;
								  ...
								  ?>
                                   <tr bgcolor="<? echo $bgcolor; ?>" style="cursor:pointer;">
									  ...
									  $tot_shift_wise=0;
										foreach ($shift_name as $key => $value) {
										$shift_wise_qty=$batch_all_shift_arr[$typekey][$key]['all_shift_qty'];
											?>
											<td><? echo number_format($shift_wise_qty,0); ?></td>
											<?
											$tot_shift_arr[$key]+=$shift_wise_qty;
											$tot_shift_wise+=$shift_wise_qty;
											//$with_out_shift+=$fabric_batch_arr_withOutshift[$typekey][0]['prod_qty'];
										}
										$with_out_shift=$batch_shift_arr[$typekey][0]['without_shift_qty'];
									 ?>
                                       <td align="right" title="<? echo $with_out_shift?>"><? echo number_format($with_out_shift,0); ?></td>
									   <td align="right" title=""><? echo number_format($tot_shift_wise+$with_out_shift,0); ?></td>
								  </tr>
						 ...
						 <td  valign="top">
							...
							$st=1;$total_batch_qty_shade=0;$tot_batch_per_shade=0;
								//echo array_sum($buyer_re_process_arr);
							foreach($buyer_shadematched_arr as $key=>$val)
							{ 
							...
							?>
								  <tr bgcolor="<? echo $bgcolor; ?>" style="cursor:pointer;">
									  ...
									  <td title="Total Shade<? echo $total_shadematched_qty;?>"><?  echo number_format(($total_shade/$total_shadematched_qty)*100,0).'%'; ?></td>
							  </tr>
							  <?
							  $st++;
							  $total_batch_qty_shade+=$val["batch_qty"];
							...
							<tfoot>
								...
								<th> Total</th>
								 ...
								 <th width="30"><? echo number_format((($total_batch_qty_trim+$total_batch_qty_shade)/$total_shadematched_qty)*100,0); ?></th>
								</tr>
							</tfoot>
						</table>
				   		</td>
                           ...
                           <tbody>
							...
							foreach($buyer_re_process_arr as $key=>$val)
							{
                             ...
                             <tr bgcolor="<? echo $bgcolor; ?>" style="cursor:pointer;">
                               ...
                               <td align="right" title="Batch and Trims Qty"><? echo number_format($val['re_dyeing_qty']+$trims_qty_sum,0,'.',''); $total_batch_qty_reproc+=$val['re_dyeing_qty']+$trims_qty_sum; ?></td>
		 ...
		 if(str_replace("'",'',$cbo_batch_type)==0 || str_replace("'",'',$cbo_batch_type)==1)
		 {
			...
			<div style=" max-height:350px; width:1470px; overflow-y:scroll; float:left; clear:both;" id="scroll_body">
            <table class="rpt_table" id="table_body" width="1450" cellpadding="0" cellspacing="0" border="1" rules="all" align="left">
                <tbody>
                <?
				$tot_sum_trims_qnty=0;$tot_shadeMatchQty=0;
              ...
              foreach($batch_detail_arr as $batch_id=>$prod_data)
			  {
				foreach($prod_data as $prod_id=>$batch)
				{
					...
					$dyeing_prod_qty=$batch[('batch_qnty')];//$batch[('shift_wise_qty')];//$shift_wise_prod_arr[$batch_id][$batch[('fabric_type')]][$prod_id]['prod_qty'];
					$batch_no=$batch[('id')];
				if (!in_array($batch_no,$trims_check_array))
					...
					if($batch['result']==1 && (!in_array($batch[('batch_against')],$batch_aganist_arr)))
					{
						$tot_shadeMatchQty+=$batch[('batch_qnty')];
					}
					...
					<tr bgcolor="<? echo $bgcolor_dyeing; ?>"  id="tr_<? echo $i; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor_dyeing; ?>')">
                        ...
                        <td align="right" width="70"><? echo number_format($dyeing_prod_qty,0);  ?></td>
			...
			<table class="rpt_table" width="1460" id="report_table_footer" cellpadding="0" cellspacing="0" border="1" rules="all">
				 <tfoot>
                <tr>
					...
					<th width="80"></th>
					<th width="90"></th>
					<th width="40" title="Shade Match=<? echo $tot_shadeMatchQty;?>">Total:</th>
					<th width="70" id="total_batch_qty"><? echo number_format($grand_total_batch_qty,0); ?></th>
					<th width="70"  id="total_batch_trim_qty"><? echo number_format($tot_trims_qnty,0); ?></th>
					
					<th width="75"><? //echo $tot_shadeMatchQty;?></th>
					<th width="75"></th>
					<th width="60"></th>
					<th width="100"></th>
					<th width="50"></th>
                ...
                <tr>
					...
					<th width="70"  id="total_batch_qty_grand"><? echo number_format($grand_total_batch_qty+$tot_trims_qnty,0); ?></th>
					<th width="70"></th>
		...
		?>
		 	<div align="left" style="float:left; clear:both;">
			 <table class="rpt_table" width="1460" cellpadding="0" cellspacing="0" border="1" rules="all" align="left" id="table_header_2">
			...
			<div style=" max-height:350px; width:1480px; overflow-y:scroll; float:left; clear:both;" id="scroll_body_sub">
            <table class="rpt_table" id="table_body2" width="1460" cellpadding="0" cellspacing="0" border="1" rules="all" align="left">
                <tbody>
                <?
              ...
              foreach($subcon_batch_detail_arr as $batch_id=>$prod_data)
			  {
				...
				if (!in_array($batch_no,$sub_trims_check_array))
					...
					<tr bgcolor="<? echo $bgcolor_sub_dyeing; ?>"  id="trsub_<? echo $i; ?>" onClick="change_color('trsub_<? echo $i; ?>','<? echo $bgcolor_sub_dyeing; ?>')">
                        ...
                        <td align="right" width="70"><? echo number_format($sub_dyeing_prod_qty,0);  ?></td>
			...
			<table class="rpt_table" width="1460" id="report_table_footer" cellpadding="0" cellspacing="0" border="1" rules="all">
                ...
                <tr>
					...
					<th width="80"></th>
					<th width="80"></th>
					<th width="90">Trims</th>
					<th width="40">Total:</th>
					<th width="70"  id="total_batch_trim_qty2"><? echo number_format($sub_tot_trims_qnty,0); ?></th>
					<th width="70"></th>
                ...
                <tr>
					...
					<th width="90">Grand</th>
					<th width="40">Total:</th>
					<th width="70"  id="total_batch_qty2"><? echo number_format($sub_grand_total_batch_qty+$sub_tot_trims_qnty,0); ?></th>
					<th width="70"></th>
		...
		if(str_replace("'",'',$cbo_batch_type)==0 || str_replace("'",'',$cbo_batch_type)==3) // Sample Production
		{
		 ...
		 <div align="left" style="float:left; clear:both;">
			 ...
			 <table class="rpt_table" width="1460" cellpadding="0" cellspacing="0" border="1" rules="all" align="left" id="table_header_3">
			...
			<div style=" max-height:350px; width:1480px; overflow-y:scroll; float:left; clear:both;" id="scroll_body_samp">
            <table class="rpt_table" id="table_body3" width="1460" cellpadding="0" cellspacing="0" border="1" rules="all" align="left">
                <tbody>
                <?
				$tot_sum_trims_qnty=0;
              ...
              foreach($samp_batch_detail_arr as $batch_id=>$prod_data)
			  {
				foreach($prod_data as $prod_id=>$samp_batch)
				{
					...
					$samp_dyeing_prod_qty=$samp_batch[('shift_wise_qty')];//$shift_wise_prod_arr[$batch_id][$batch[('fabric_type')]][$prod_id]['prod_qty'];
				...
				if (!in_array($batch_no,$samp_trims_check_array))
					...
					<tr bgcolor="<? echo $bgcolor_sam_dyeing; ?>"  id="trsam_<? echo $i; ?>" onClick="change_color('trsam_<? echo $i; ?>','<? echo $bgcolor_sam_dyeing; ?>')">
                        ...
                        <td align="right" width="70"><? echo number_format($samp_dyeing_prod_qty,0);  ?></td>
					...
					$total_water_cons_load+=$water_cons_load;
					$total_water_cons_unload+=$water_cons_unload;
					$samp_tot_trims_qnty+=$tot_trim_qty;
					$trims_summary+=$tot_trim_qty;
			...
			<table class="rpt_table" width="1460" id="report_table_footer" cellpadding="0" cellspacing="0" border="1" rules="all">
                ...
                <tr>
					...
					<th width="90">Trims</th>
					<th width="40">Total:</th>
					<th width="70"  id="total_batch_trim_qty3"><? echo number_format($samp_tot_trims_qnty,0); ?></th>
					<th width="70"></th>
                ...
                <tr>
					...
					<th width="90">Grand</th>
					<th width="40">Total:</th>
					<th width="70"  id="total_batch_qty3"><? echo number_format($samp_grand_total_batch_qty+$samp_tot_trims_qnty,0); ?></th>
					<th width="70"></th>
