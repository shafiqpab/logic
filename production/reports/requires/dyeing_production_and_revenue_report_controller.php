...
if ($action == "report_generate")
{
	...
	if (str_replace("'", "", trim($txt_date_from)) != "" && str_replace("'", "", trim($txt_date_to)) != "")
	{
        ...
        $date_cond3 = " AND F.PRODUCTION_DATE BETWEEN " . trim($txt_date_to) . " AND " . trim($txt_date_to) . "";
...
/* 
    ...
    $sql_batch_today = "SELECT  A.BATCH_NO, A.BATCH_WEIGHT, A.ID AS BATCH_ID, F.MACHINE_ID,A.ENTRY_FORM
    FROM PRO_BATCH_CREATE_DTLS B, FABRIC_SALES_ORDER_MST D, PRO_FAB_SUBPROCESS F, PRO_BATCH_CREATE_MST A 
    WHERE F.BATCH_ID=A.ID  AND A.ENTRY_FORM=0  AND A.ID=B.MST_ID AND F.BATCH_ID=B.MST_ID AND A.WORKING_COMPANY_ID='$companyId' $date_cond3 $sales_no_cond AND F.ENTRY_FORM=35 AND F.LOAD_UNLOAD_ID=2 AND A.BATCH_AGAINST IN(1,11,2,3) AND B.PO_ID=D.ID AND B.STATUS_ACTIVE=1 AND B.IS_DELETED=0 AND A.STATUS_ACTIVE=1 AND A.IS_DELETED=0 AND F.STATUS_ACTIVE=1 AND F.IS_DELETED=0 AND A.IS_SALES=1 AND B.IS_SALES=1 AND F.RESULT=1 AND A.EXTENTION_NO IS NULL ".where_con_using_array($machine_ids_arr,0,'F.MACHINE_ID')."
    ...
    FROM PRO_BATCH_CREATE_DTLS B, SUBCON_ORD_DTLS C, SUBCON_ORD_MST D, PRO_BATCH_CREATE_MST A, PRO_FAB_SUBPROCESS F
    WHERE F.BATCH_ID=A.ID AND F.BATCH_ID=B.MST_ID  AND A.ENTRY_FORM=36 AND A.ID=B.MST_ID AND D.COMPANY_ID='$companyId' $date_cond3 AND F.ENTRY_FORM=38 AND F.LOAD_UNLOAD_ID=2 AND A.BATCH_AGAINST IN(1,11,2,3) AND B.PO_ID=C.ID AND D.SUBCON_JOB=C.JOB_NO_MST AND B.STATUS_ACTIVE=1 AND B.IS_DELETED=0 AND A.STATUS_ACTIVE=1 AND A.IS_DELETED=0 AND F.STATUS_ACTIVE=1 AND F.IS_DELETED=0 AND F.RESULT=1 AND A.EXTENTION_NO IS NULL ".where_con_using_array($machine_ids_arr,0,'F.MACHINE_ID')."  GROUP BY A.ID, A.BATCH_NO,F.MACHINE_ID,A.ENTRY_FORM, A.BATCH_WEIGHT";
    
    //echo $sql_batch_today; 
    $batch_rslt_today = sql_select($sql_batch_today);

    $all_batch_id_cond=array();
    $duplicate_batch_id_check=array();
    foreach ($batch_rslt_today as $row) 
	{
   ...
   // var_dump($all_batch_id_cond);
    ...
    foreach ($batch_rslt_today as $row) 
	{
        if($row['ENTRY_FORM']==0)
        {
            if($duplicate_batch_today_check[$row['BATCH_ID']] == '')
            {
               $duplicate_batch_today_check[$row['BATCH_ID']] = $row['BATCH_ID'];
        ...
        {
            if($duplicate_sub_batch_today_check[$row['BATCH_ID']] == '')
            {
                $duplicate_sub_batch_today_check[$row['BATCH_ID']] = $row['BATCH_ID'];
    ...
    //var_dump( $first_chemi_cost);

    $sql_batch_with_out_ext = "SELECT  A.BATCH_NO, A.BATCH_WEIGHT, A.ID AS BATCH_ID, F.MACHINE_ID,A.ENTRY_FORM,A.EXTENTION_NO
    FROM PRO_BATCH_CREATE_DTLS B, FABRIC_SALES_ORDER_MST D, PRO_FAB_SUBPROCESS F, PRO_BATCH_CREATE_MST A 
    ...
    GROUP BY A.ID, A.BATCH_NO,F.MACHINE_ID,A.ENTRY_FORM, A.BATCH_WEIGHT,A.EXTENTION_NO
    UNION ALL 
    SELECT A.BATCH_NO, A.BATCH_WEIGHT, A.ID AS BATCH_ID, F.MACHINE_ID,A.ENTRY_FORM,A.EXTENTION_NO
    FROM PRO_BATCH_CREATE_DTLS B, SUBCON_ORD_DTLS C, SUBCON_ORD_MST D, PRO_BATCH_CREATE_MST A, PRO_FAB_SUBPROCESS F
    WHERE F.BATCH_ID=A.ID AND F.BATCH_ID=B.MST_ID  AND A.ENTRY_FORM=36 AND A.ID=B.MST_ID AND D.COMPANY_ID='$companyId' $date_cond2 AND F.ENTRY_FORM=38 AND F.LOAD_UNLOAD_ID=2 AND A.BATCH_AGAINST IN(1,11,2,3) AND B.PO_ID=C.ID AND D.SUBCON_JOB=C.JOB_NO_MST AND B.STATUS_ACTIVE=1 AND B.IS_DELETED=0 AND A.STATUS_ACTIVE=1 AND A.IS_DELETED=0 AND F.STATUS_ACTIVE=1 AND F.IS_DELETED=0 AND F.RESULT=1  AND A.EXTENTION_NO IS NULL ".where_con_using_array($machine_ids_arr,0,'F.MACHINE_ID')."  GROUP BY A.ID, A.BATCH_NO,F.MACHINE_ID,A.ENTRY_FORM, A.BATCH_WEIGHT,A.EXTENTION_NO";
    
    //echo $sql_batch_with_out_ext;
    $batch_with_out_ext_rslt = sql_select($sql_batch_with_out_ext);

    $batch_info_arr = array();
    $duplicate_batch_ext_check = array();
    $duplicate_sub_batch_ext_check = array();

    foreach ($batch_with_out_ext_rslt as $row) 
	{
        if($row['ENTRY_FORM']==0)
        {
            if($duplicate_batch_ext_check[$row['BATCH_ID']] == '')
            {
                $duplicate_batch_ext_check[$row['BATCH_ID']] = $row['BATCH_ID'];
        ...
        {
            if($duplicate_sub_batch_ext_check[$row['BATCH_ID']] == '')
            {
                $duplicate_sub_batch_ext_check[$row['BATCH_ID']] = $row['BATCH_ID'];
                $batch_info_arr[$row['MACHINE_ID']]['batch_weight'] += $row['BATCH_WEIGHT'];
                $batch_info_arr[$row['MACHINE_ID']]['no_of_batch']++;
            }
        }
	...
	unset($batch_with_out_ext_rslt);
    
    $sql_batch = "SELECT  A.BATCH_NO, A.BATCH_WEIGHT, A.ID AS BATCH_ID, F.MACHINE_ID,A.ENTRY_FORM,F.SYSTEM_NO
    FROM PRO_BATCH_CREATE_DTLS B, FABRIC_SALES_ORDER_MST D, PRO_FAB_SUBPROCESS F, PRO_BATCH_CREATE_MST A 
    WHERE F.BATCH_ID=A.ID  AND A.ENTRY_FORM=0  AND A.ID=B.MST_ID AND F.BATCH_ID=B.MST_ID AND A.WORKING_COMPANY_ID='$companyId' $date_cond3 $sales_no_cond AND F.ENTRY_FORM=35 AND F.LOAD_UNLOAD_ID=2 AND A.BATCH_AGAINST IN(1,11,2,3) AND B.PO_ID=D.ID AND B.STATUS_ACTIVE=1 AND B.IS_DELETED=0 AND A.STATUS_ACTIVE=1 AND A.IS_DELETED=0 AND F.STATUS_ACTIVE=1 AND F.IS_DELETED=0 AND A.IS_SALES=1 AND B.IS_SALES=1  ".where_con_using_array($machine_ids_arr,0,'F.MACHINE_ID')."
    GROUP BY A.ID, A.BATCH_NO,F.MACHINE_ID,A.ENTRY_FORM, A.BATCH_WEIGHT,F.SYSTEM_NO
    UNION ALL 
    SELECT A.BATCH_NO, A.BATCH_WEIGHT, A.ID AS BATCH_ID, F.MACHINE_ID,A.ENTRY_FORM,F.SYSTEM_NO
    FROM PRO_BATCH_CREATE_DTLS B, SUBCON_ORD_DTLS C, SUBCON_ORD_MST D, PRO_BATCH_CREATE_MST A, PRO_FAB_SUBPROCESS F
    WHERE F.BATCH_ID=A.ID AND F.BATCH_ID=B.MST_ID  AND A.ENTRY_FORM=36 AND A.ID=B.MST_ID AND D.COMPANY_ID='$companyId' $date_cond3 AND F.ENTRY_FORM=38 AND F.LOAD_UNLOAD_ID=2 AND A.BATCH_AGAINST IN(1,11,2,3) AND B.PO_ID=C.ID AND D.SUBCON_JOB=C.JOB_NO_MST AND B.STATUS_ACTIVE=1 AND B.IS_DELETED=0 AND A.STATUS_ACTIVE=1 AND A.IS_DELETED=0 AND F.STATUS_ACTIVE=1 AND F.IS_DELETED=0 ".where_con_using_array($machine_ids_arr,0,'F.MACHINE_ID')."  GROUP BY A.ID, A.BATCH_NO,F.MACHINE_ID,A.ENTRY_FORM, A.BATCH_WEIGHT,F.SYSTEM_NO";
  ...
  // $batch_ids_arr = array();
    ...
    foreach ($batch_rslt as $row) 
	{
        if($row['ENTRY_FORM']==0)
        {
            if($duplicate_batch_check[$row['BATCH_ID']] == '')
            {
               ...
               $batch_weight_per += $row['BATCH_WEIGHT'];
              // array_push($batch_ids_arr,$row['BATCH_ID']);
    ...
    //var_dump($batch_ids_arr);

    $sql_batch_tdace = "SELECT A.ENTRY_FORM,  A.ID AS BATCH_ID
    FROM PRO_BATCH_CREATE_DTLS B, FABRIC_SALES_ORDER_MST D, PRO_FAB_SUBPROCESS F, PRO_BATCH_CREATE_MST A 
    WHERE F.BATCH_ID=A.ID  AND A.ENTRY_FORM=0  AND A.ID=B.MST_ID AND F.BATCH_ID=B.MST_ID AND A.WORKING_COMPANY_ID='$companyId' $date_cond3 $sales_no_cond AND F.ENTRY_FORM=35 AND F.LOAD_UNLOAD_ID=2 AND A.BATCH_AGAINST IN(1,11,2,3) AND B.PO_ID=D.ID AND B.STATUS_ACTIVE=1 AND B.IS_DELETED=0 AND A.STATUS_ACTIVE=1 AND A.IS_DELETED=0 AND F.STATUS_ACTIVE=1 AND F.IS_DELETED=0 AND A.IS_SALES=1 AND B.IS_SALES=1  ".where_con_using_array($machine_ids_arr,0,'F.MACHINE_ID')."
    GROUP BY  A.ENTRY_FORM,A.ID
    UNION ALL 
    SELECT A.ENTRY_FORM, A.ID AS BATCH_ID
    FROM PRO_BATCH_CREATE_DTLS B, SUBCON_ORD_DTLS C, SUBCON_ORD_MST D, PRO_BATCH_CREATE_MST A, PRO_FAB_SUBPROCESS F
    WHERE F.BATCH_ID=A.ID AND F.BATCH_ID=B.MST_ID  AND A.ENTRY_FORM=36 AND A.ID=B.MST_ID AND D.COMPANY_ID='$companyId' $date_cond3 AND F.ENTRY_FORM=38 AND F.LOAD_UNLOAD_ID=2 AND A.BATCH_AGAINST IN(1,11,2,3) AND B.PO_ID=C.ID AND D.SUBCON_JOB=C.JOB_NO_MST AND B.STATUS_ACTIVE=1 AND B.IS_DELETED=0 AND A.STATUS_ACTIVE=1 AND A.IS_DELETED=0 AND F.STATUS_ACTIVE=1 AND F.IS_DELETED=0 ".where_con_using_array($machine_ids_arr,0,'F.MACHINE_ID')."  GROUP BY A.ENTRY_FORM,A.ID";

    //LISTAGG(CAST(A.ID AS VARCHAR(4000)),',') WITHIN GROUP (ORDER BY A.ID) AS BATCH_ID
    //echo  $sql_batch_tdace;
    $sql_batch_tdace_rslt = sql_select($sql_batch_tdace);

    $batch_ids_arr = array();
    $batch_weight_per=0;
    foreach ($sql_batch_tdace_rslt as $row) 
	{
        if($row['ENTRY_FORM']==0)
        {
            if($duplicate_batch_check1[$row['BATCH_ID']] == '')
            {
                $duplicate_batch_check1[$row['BATCH_ID']] = $row['BATCH_ID'];

               $batch_id_arr[]=$row['BATCH_ID'];
            }
        }
        else  if($row['ENTRY_FORM']==36)
        {
            if($duplicate_sub_batch_check1[$row['BATCH_ID']] == '')
            {
                $duplicate_sub_batch_check1[$row['BATCH_ID']] = $row['BATCH_ID'];
    ...
    where a.id=b.mst_id and a.id=c.mst_id and b.id=c.trans_id and b.transaction_type=2 and a.entry_form=5 and a.batch_no  is not null  and   b.item_category in (5,6,7) and a.issue_purpose in(10,11,13,14,34,35,40,42,52)
    group by a.batch_no,b.item_category,c.sub_process  "); 
	 
	$dyes_chemical_arr=array();
	foreach($sql_dyes_cost as $val)
	{
        $dyes_chemical_arr[$val[csf("batch_no")]]['chemical_cost']+=$val[csf("dyes_chemical_cost")];
...
//echo "select batch_no from inv_issue_master  where  entry_form=5 and status_active=1 and is_deleted=0 and batch_no is not null"; die;
    ...
    //var_dump($issue_single_batch_arr);

    $t_exp = 0;
    foreach(array_unique($issue_single_batch_arr) as $b_id)
	{
        $t_exp +=$dyes_chemical_arr[$b_id]['chemical_cost'];
    }
    ...
    // UNION ALL 
    // SELECT A.ID AS TRANS_ID, A.TRANSACTION_TYPE, A.MST_ID AS REC_ISSUE_ID, A.TRANSACTION_DATE, A.ITEM_CATEGORY, 0 AS RECEIVE_QTY, A.CONS_QUANTITY AS ISSUE_QTY, A.CONS_UOM, B.ID AS PROD_ID, B.ITEM_GROUP_ID, B.SUB_GROUP_NAME, B.ITEM_DESCRIPTION, B.ITEM_CODE, A.CONS_RATE, A.CONS_AMOUNT, A.INSERTED_BY, A.ORDER_QNTY, A.ORDER_RATE, C.TRANSFER_SYSTEM_ID AS RECV_ISSUE_NUM, 0 AS BOOKING_ID, NULL AS BOOKING_NO, C.CHALLAN_NO AS CHALLAN_NO, 0 AS KNITTING_SOURCE, 0 AS KNITTING_COMPANY, 0 AS CURRENCY_ID, 0 AS RECEIVE_ISSUE_PURPOSE, NULL AS REQ_NO, NULL AS BATCH_NO, NULL AS CURRENCY_ID, NULL AS EXCHANGE_RATE, NULL AS BATCH_ID, NULL AS RECIPE_ID, 3 AS TYPE 
    // FROM INV_TRANSACTION A, PRODUCT_DETAILS_MASTER B, INV_ITEM_TRANSFER_MST C 
    // WHERE A.PROD_ID=B.ID AND A.MST_ID=C.ID AND A.COMPANY_ID=$companyId AND A.TRANSACTION_TYPE IN(6) AND C.ENTRY_FORM IN(55)  
	...
	if ($presentationType == 1)
	{
		...
		<fieldset style="width:100%;">
            ...
            <table cellspacing="0" cellpadding="3" border="1" rules="all"  class="rpt_table" id="scroll_body">
                ...
                <?
                $machine = array();
                foreach ($print_data as $mid => $midval) 
                {
                    foreach ($midval as $mno => $row) 
                    {
                        //$mc_arr = explode("-",$row["machine_no"]);
                        $mc_arr = preg_split('/[-,;\s]+/', $row['machine_no'], -1, PREG_SPLIT_NO_EMPTY);
                        $machine[$mc_arr[0]][$mc_arr[1]][$mid]['cap'] =  $row["prod_capacity"];
                        $machine[$mc_arr[0]][$mc_arr[1]][$mid]['no'] = $row["machine_no"];
                    }
                }
                //var_dump( $machine);
                ...
                ?>
                    ...
                    <tr>
                        ...
                        <? 
                        foreach ($machine as $f => $fval) 
                        {
                            ksort($fval);
                            foreach ($fval as $s => $sval) 
                            {
                                foreach ($sval as $m_id => $rows){ ?>
                        
                                    <td align="center" width="150" bgcolor="#E9F3FF" style="font: 14px tahoma;">
                                    <?
                                     
                                        $data = preg_split('/[-,;\s]+/', $rows['no'], -1, PREG_SPLIT_NO_EMPTY);
                   ...
                   <tr>
                        ...
                        foreach ($machine as $f => $fval) 
                        {
                            ...
                            {
                                foreach ($sval as $m_id => $rows){ ?>
                                    <td align="center" width="150" style="font: 14px tahoma;">
                                    <p style="width:80px;">
                                    <?
                                    $data = preg_split('/[-,;\s]+/', $rows['no'], -1, PREG_SPLIT_NO_EMPTY);
                   ...
                   <tr>
                        ...
                        foreach ($machine as $f => $fval) 
                        {
                            ...
                            {
                                foreach ($sval as $m_id => $rows){ ?>
                                    <td align="center" width="150" bgcolor="#E9F3FF">
                                    <?=$rows['cap'];
                                     $tot_prod_capacity +=$rows['cap'];
                                    ?>
                                    </td>
                                <?
                                }
            ...
            <div style="margin-top: 70px">
                ...
                <table cellpadding="0" width="300" cellspacing="0" align="left" class="rpt_table" rules="all" border="1">
                    ...
                    <tbody>
                        <tr bgcolor="#E9F3FF" >
                            <td style="font: 14px tahoma;">Inhouse </td>
                ...
                // {
                    ...
                    <table cellpadding="0" width="500" cellspacing="0" align="left" class="rpt_table" rules="all" border="1">
                        ...
                        <tbody>
                            <tr bgcolor="#E9F3FF" >
                                <td style="font: 14px tahoma;">Upto yesterday Production of &nbsp;<?  if($txt_date_from!="") echo $month_name; ?>-<?  if($txt_date_from!="") echo $year_name; ?> [Inhouse]	 </td>
                                <td align="right" style="font: 14px tahoma;">
                            ...
                            <tr bgcolor="#E9F3FF" >
                                <td style="font: 14px tahoma;">Upto yesterday Production of &nbsp;<?  if($txt_date_from!="") echo $month_name; ?>-<?  if($txt_date_from!="") echo $year_name; ?> [Inbound Subcontract]	 </td>
                                <td align="right" style="font: 14px tahoma;">
                    ...
                    <table cellpadding="0" width="300" cellspacing="0" align="left" class="rpt_table" rules="all" border="1">
                        ...
                        <tbody>
                            <tr bgcolor="#E9F3FF" >
                                ...
                                if($tot_y_production)
                                {
                                    echo number_format($tot_y_production/$no_of_days,2);
                                }
                                else
                                {
                                    echo "0.00";
           ...
           //echo  $sql_batch_fc;
            ...
            if($txt_date_from1!="")
            {
                ...
                if($date_distance1==1 && $fist_day_of_month1>1)
                {
                    ...
                    FROM PRO_BATCH_CREATE_DTLS B, SUBCON_ORD_DTLS C, SUBCON_ORD_MST D, PRO_BATCH_CREATE_MST A, PRO_FAB_SUBPROCESS F
                    WHERE F.BATCH_ID=A.ID AND F.BATCH_ID=B.MST_ID  AND A.ENTRY_FORM=36 AND A.ID=B.MST_ID AND D.COMPANY_ID='$companyId'  AND F.ENTRY_FORM=38 AND F.LOAD_UNLOAD_ID=2 AND A.BATCH_AGAINST IN(1,11,2,3) AND B.PO_ID=C.ID AND D.SUBCON_JOB=C.JOB_NO_MST AND B.STATUS_ACTIVE=1 AND B.IS_DELETED=0 AND A.STATUS_ACTIVE=1 AND A.IS_DELETED=0 AND F.STATUS_ACTIVE=1 AND F.IS_DELETED=0 AND F.RESULT=1 AND A.EXTENTION_NO IS NULL AND F.PRODUCTION_DATE<'".$txt_date_from1."' $sql_cond  ".where_con_using_array($machine_ids_arr1,0,'F.MACHINE_ID')."  GROUP BY A.ID, A.BATCH_NO,F.MACHINE_ID,A.ENTRY_FORM, A.BATCH_WEIGHT,F.FABRIC_TYPE,A.COLOR_RANGE_ID";
                   ...
                   //echo $sql_batch_up;
                    ...
                    if ($db_type == 0) 
                    {
                        $date_cond_up = "and FROM_DATE = '" . change_date_format(date('Y-m-01')) . "' and TO_DATE ='" . change_date_format(date('Y-m-t'), "yyyy-mm-dd") . "'";
                    } 
                    else 
                    {
                        $date_cond_up = "and FROM_DATE = '" . change_date_format(date('Y-m-01'), '', '', 1) . "' and TO_DATE ='" . change_date_format(date('Y-m-t'), '', '', 1) . "'";
                    }

                    $sql_d_rate_chart_up = "SELECT COMPANY_ID, FABRIC_TYPE_ID, COLOR_RANGE_ID, RATE, EXCHANGE_RATE, FROM_DATE, TO_DATE 
                    FROM LIB_DYEING_RATE_CHART_DTLS WHERE COMPANY_ID='$companyId' AND ENTRY_FORM=540 AND STATUS_ACTIVE=1 AND IS_DELETED=0
                    ".where_con_using_array($fabric_type_arr_up,0,'fabric_type_id')." $date_cond_up";
                    ...
                    $sql_batch_wo_ext_up = "SELECT A.ENTRY_FORM, A.ID AS BATCH_ID
                    FROM PRO_BATCH_CREATE_DTLS B, FABRIC_SALES_ORDER_MST D, PRO_FAB_SUBPROCESS F, PRO_BATCH_CREATE_MST A 
                    ...
                    //echo $sql_batch_wo_ext_up;

                    $batch_wo_ext_up_rslt = sql_select($sql_batch_wo_ext_up);
                    $duplicate_batch_up_wo_ext_check = array();
                    $batch_id_up_wo_ext = array();
                    foreach ($batch_wo_ext_up_rslt as $row) 
                    {
                        if($duplicate_batch_up_wo_ext_check[$row['BATCH_ID']] == '')
                        {
                            $duplicate_batch_up_wo_ext_check[$row['BATCH_ID']] = $row['BATCH_ID'];
                           // array_push( $batch_id_up_wo_ext, $row['BATCH_ID']);
                           $batch_id_up_wo_ext[]=$row['BATCH_ID'];
                        }
                    }

                    //================================

                    $sql_dyes_cost_up =sql_select("select a.batch_no,c.sub_process,b.item_category,sum(b.cons_amount) as dyes_chemical_cost
                    from inv_issue_master a, inv_transaction b,dyes_chem_issue_dtls c
                    where a.id=b.mst_id and a.id=c.mst_id and b.id=c.trans_id and b.transaction_type=2 and a.entry_form=5 and a.batch_no  is not null  and   b.item_category in (5,6,7) and a.issue_purpose in(10,11,13,14,34,35,40,42,52)
                    group by a.batch_no,b.item_category,c.sub_process  "); 
                     
                    $dyes_chemical__up_arr=array();
                    foreach($sql_dyes_cost_up as $val)
                    {
                        $dyes_chemical__up_arr[$val[csf("batch_no")]]['chemical_cost']+=$val[csf("dyes_chemical_cost")];
                    }
                    ...
                    //*********************************************************************************************************************************************************
                     if($db_type==0)
                     {
                         $sql_dtls_up =sql_select("select batch_no from inv_issue_master  where  entry_form=5 and status_active=1 and is_deleted=0 and batch_no<>'' order by batch_no");
                     }
                     else
                     {
                         $sql_dtls_up =sql_select("select batch_no from inv_issue_master  where  entry_form=5 and status_active=1 and is_deleted=0 and batch_no is not null order by batch_no");
                     }
                      
                
                
                //echo "select batch_no from inv_issue_master  where  entry_form=5 and status_active=1 and is_deleted=0 and batch_no is not null"; die;
                    foreach($sql_dtls_up as $inf)
                    {
                        if(strpos($inf[csf("batch_no")], ",")==true)
                        {
                            $multi_batch_ids=explode(",",$inf[csf("batch_no")]);
                            foreach($multi_batch_ids as $multi_batch)
                            {
                                $multi_batch_up_chk[$multi_batch]=$multi_batch;
                            }
                        }
                    }
                    ...
                    foreach($sql_dtls_up as $inf)
                    {
                        if(strpos($inf[csf("batch_no")], ",")==true)
                        {
                            ...
                            foreach($multi_batch_id as $m_batch)
                            { 
                                if(in_array($m_batch,$batch_id_up_wo_ext)) {$issue_multi_batch_temp_arr[]=$m_batch;  $batch_match+=$chk_multi_cond;  }
                            }
                            if($batch_match==$total_batch_id) 
                            { 
                                //echo "A";
                                $issue_multi_batch_up_arr[]=$inf[csf("batch_no")]; 
                                $multi_single_batch=array_merge($multi_single_batch, $issue_multi_batch_temp_arr); 
                            }
                        }
                        else
                        {
                            //echo $inf[csf("batch_no")].'X';
                            if(in_array($inf[csf("batch_no")],$batch_id_up_wo_ext) && $multi_batch_up_chk[$inf[csf("batch_no")]]=="") 
                            { 
                                $issue_single_batch_up_arr[$inf[csf("batch_no")]]=$inf[csf("batch_no")];
                            } 
                        }
                    }
                    //var_dump($issue_single_batch_arr);
                    ...
                    $y_exp = 0;
                    foreach(array_unique($issue_single_batch_up_arr) as $b_id)
                    {
                        $y_exp +=$dyes_chemical__up_arr[$b_id]['chemical_cost'];
                    }
                
                    foreach(array_unique($issue_multi_batch_up_arr) as $b_id)
                    {
                        $y_exp +=$dyes_chemical__up_arr[$b_id]['chemical_cost'];
                    }
                     ...
                     $g_tot_tk=0;$g_uya_tk=0;$g_uta_tk=0;
                        ...
                        { 
                            ...
                            <? 
                                ...
                                { 
                                    ...
                                    <td align="right"  style="font: 14px tahoma;">
                                        ...
                                        if($v_c_range !='Sample')
                                        {
                                            $batch_weight = $batch_arr[$k_f_type][$v_c_range]['batch_weight'];
                                            echo number_format($batch_weight,2);

                                        }
                                        else
                                        {
                                            $batch_weight =  $batch_smn_arr[$k_f_type]['batch_weight'];
                                            echo number_format($batch_weight,2);
                                        }
                            ...
                            ?>
                                ...
                                if($tot_tk+$uya_tk>0)
                                {
                                    echo number_format($tot_tk+$uya_tk,2); 
                                }
                                else
                                {
                                    echo '0.00';
                                }
                               $g_uta_tk +=($tot_tk+$uya_tk); 
                                ?></td>
                        ...
                        <tr>
                            ...
                            <td align="right"  style="font: 14px tahoma;"><b>
                                <? 
                              
                                if($g_uya_tk>0)
                                {
                                    echo number_format( $g_tot_tk,2);
                                }
                                else
                                {
                                    echo '0.00';
                    ...
                    <tbody>
                        <tr bgcolor="#E9F3FF" >
                            ...
                            <td align="right" style="font: 14px tahoma;"> 
                                ...
                                echo number_format($t_exp,4,".","");
                                $g_tot_chemical_cost_finish +=$t_exp;
                                ?>
                            </td>
                        </tr>
                        <tr bgcolor="#E9F3FF" >
                            ...
                            <td align="right" style="font: 14px tahoma;">
                                ...
                                echo number_format($y_exp,4,".","");
                                $g_tot_chemical_cost_finish_up +=$y_exp;
                                ?>
                            </td>
                        </tr>
                        <tr bgcolor="#E9F3FF" >
                            ...
                            <td align="right" style="font: 14px tahoma;"> 
                                <?
                                $utdace = ($g_tot_chemical_cost_finish+$g_tot_chemical_cost_finish_up);
                                echo number_format($utdace,4,".","");
                                ?>
                            </td>
                        </tr>
                    </tbody>
                ...
                <table cellpadding="0" width="400" cellspacing="0" align="left" class="rpt_table" rules="all" border="1">
                    ...
                    <tbody>
                        <tr bgcolor="#E9F3FF" >
                            ...
                            <td align="right" style="font: 14px tahoma;"> 
                            <?
                            if($total_in_out_subc_qty)
                            {
                                echo number_format($t_exp/$total_in_out_subc_qty,4,".","");
                            }
                            else
                            {
                                echo '0.0000';
                        ...
                        <tr bgcolor="#E9F3FF" >
                            ...
                            if($tot_y_production)
                            {
                                //echo $tot_y_production;
                                echo number_format($y_exp/$tot_y_production,4,".","");  
                            }
                            else
                            {
                                echo '0.0000';
