...
if ($action=="load_drop_down_floor_line")
{
	...
	if($prod_reso_allo==1)
	{
		...
		else
		{
			...
			if( $explode_data[0]!=0 ) $cond = " and a.floor_id in($explode_data[3])";
    ...
    //    echo "<pre>"; print_r($line_array);die;
		echo create_drop_down( "cbo_line_id", 130,$line_array,"", 0, "-- Select --", $selected, "",0,0 );
	}
	else
	{
		...
		echo create_drop_down( "cbo_line_id", 130, "select id,line_name from lib_sewing_line where is_deleted=0 and status_active=1 and floor_name!=0 $cond order by line_name","id,line_name", 0, "-- Select --", $selected, "",0,0 );
	}
	exit();
}
...
{ 
	...
	foreach($sew_sql_result as $row)
	{
		...
		$sew_line_defect_type[$row[csf('defect_type_id')]][$row[csf('defect_point_id')]]=$row[csf('defect_point_id')];
		...
		// print_r($sew_line_defect_type);
		 
	}
	
	//  echo"<pre>"; print_r($sew_line_defect_type);
	if($type_id==1)
	{
	...
	$sql_prod_sew="SELECT a.floor_id,a.location as wk_location,a.prod_reso_allo,a.sewing_line,a.serving_company,b.production_qnty, e.id as po_id,f.style_ref_no,f.buyer_name from pro_garments_production_mst a,pro_garments_production_dtls b, wo_po_break_down e, wo_po_details_master f where  a.id=b.mst_id and a.po_break_down_id=e.id and e.job_id=f.id  and a.production_type=5 and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0  and e.status_active in(1,2,3) and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0  $date_cond $work_company_cond $company_id_cond $location_id_cond $wk_location_id_cond $floor_cond $sewing_line_cond $int_ref_cond $style_ref_cond";
	//  echo $sql_prod_sew;
	...
	foreach($sew_prod_sql_result as $row)
	{
		...
		$comp_unit=$comp_short.'-'.$floor_name_arr[$row[csf('floor_id')]];
		$unit_wise_arr[$comp_unit]['auditQty']+=$row[csf('production_qnty')];
		
		$sew_line_prod_qty_arr[$row[csf('serving_company')]][$row[csf('floor_id')]][$sewing_line][$row[csf('style_ref_no')]]['production_quantity']+=$row[csf('production_qnty')];
	}
				...
				<table width="<?= $td_width; ?>" class="rpt_table" border="1" rules="all" cellpadding="0" align="left" cellspacing="0" id="table_body_summary">
					...
					foreach($unit_data as $line_id=>$line_data)
					{
						...
						?>
							...
							<td width="75"><div style="word-wrap:break-word;"><p><? echo $company_short_arr[$company_id]; ?></p></div></td>
							<td width="80"><div style="word-wrap:break-word;"><p><? echo $floor_name_arr[$unit_id]; ?></p></div></td>
							<td width="70" title="<? echo $line_id;?>"><div style="word-wrap:break-word;"><? echo $lineArr[$line_id]; ?></div></td>
							<td width="100"><div style="word-wrap:break-word;"><p><? echo $buyer_library[$row[('buyer_name')]]; ?></p></div></td>
							<td width="100"><div style="word-wrap:break-word;"><p><? echo $style_ref; ?></p></div></td>
							<? //sew_fin_spot_defect_type
						$line_frontbackWest_defect_qty_arr=array();
						foreach($sew_line_defect_type[1] as $type_id=>$defect_type)
						{
							$alt_defect_qty= $sew_line_qty_arr[$company_id][$unit_id][$line_id][$style_ref][1][$type_id]['defect_qty'];
							//echo $alt_defect_qty.'='.$line_id;
							...
							$fronBackWest_defect_qty=$alt_defect_qty;
						?>
						<td  width="45"  style="" align="right"><p><?  echo $fronBackWest_defect_qty;   ?></p></td>
						...
						foreach($sew_line_defect_type[7] as $measure_type_id=>$measurement_discrep_type)
						{
							$mesurement_qty=$sew_line_qty_arr[$company_id][$unit_id][$line_id][$style_ref][7][$measure_type_id]['defect_qty'];;
						?>
						<td  width="45" class="" align="right" style=""><p><?  echo $mesurement_qty;   ?></p></td>
						...
						<td width="70" align="right"><p><? $tot_delectQty=$line_frontbackWest_defect_qty_arr[$line_id]+$line_measure_defect_qty_arr[$line_id];
						echo $tot_delectQty;?></p></td>
						<td width="70" align="right"> <p><? echo $prod_qty;?></p></td>
						<td width="70" align="right" title="Total Defect Qty/Audit qty*100" ><p><? 
						...
						echo number_format($line_percent,2);?></p></td>
	...
	//   $sew_fin_alter_defect_Arr="1,4,6,9,12,17,21,25,32,36,41,42,43,44,45,46,47,48,49,65,66,67,68,69";
	//   $sew_fin_alter_defect_typeChk=explode(",",$sew_fin_alter_defect_Arr);
	//   print_r($sew_fin_alter_defect_type);
	//   print_r($sew_fin_spot_defect_type);
	  foreach($sew_fin_alter_defect_type as $key=>$val)
	  {
		 // echo "=A".$key.',';
		  if(in_array($key,$sew_fin_alter_defect_typeChk))
		  {
			  // echo "=B".$key.',';
			  $sew_fin_alter_defect_typeArr[$key]=$key;
		  }
	  }
	  ...
	  {
		//   echo "=A".$key.',';
		  if(in_array($key,$sew_fin_spot_defect_typeChk))
		  {
			//    echo "=B".$key.',';
			  $sew_fin_spot_defect_typeArr[$key]=$key;
		  }
	  }
	//  print_r($sew_line_defect_type[2]);
	   $tot_rows=count($sew_line_defect_type[1])+count($sew_line_defect_type[2])+count($sew_line_defect_type[7]);
		$td_width=750+(45*$tot_rows);
		$row_span=6+count($sew_line_defect_type[1])+count($sew_line_defect_type[2]);	
		// echo $row_span.'DDDDDDDDDD';;
		asort($sew_line_defect_type[1]);
		asort($sew_line_defect_type[2]);
	    asort($sew_line_defect_type[7]);
		?>
		<div width="<?= $td_width; ?>">
		     <style type="text/css">
				.block_div { 
			...
			<table width="<?= $td_width; ?>" class="rpt_table" border="1" rules="all" cellpadding="0" cellspacing="0" align="left">
				<thead>
					<tr>
					<?
					if(count($sew_line_defect_type[1])|| count($sew_line_defect_type[2])>0){?>
					<th colspan="<? echo $row_span;?>" >QUALITY DEFECT</th>
					<?}?>
					<? if(count($sew_line_defect_type[7])>0){?>
					<th colspan="<?echo $row?>">MEASUREMENT DISCREPANCY</th>
					<?}?>
					<th width="70">&nbsp;</th>
					<th width="70">&nbsp;</th>
					<th width="70">&nbsp;</th>
					...
					<tr height="110">
						...
						foreach($sew_line_defect_type[1] as $type_id=>$defect_type)
						{
							...
							?>
							<th  width="45" style="vertical-align:middle;" title="<? echo $id;?>"><div class="block_div" style=""><p><?  echo $sew_fin_alter_defect_type[$type_id];  ?></p></div></th>
							<?
							$id++;
							
						}
						foreach($sew_line_defect_type[2] as $type_id=>$defect_type)
						{
							
							?>
							<th  width="45" style="vertical-align:middle;" title="<? echo $id;?>"><div class="block_div" style=""><p><?  echo $sew_fin_spot_defect_type[$type_id];  ?></p></div></th>
							<?
							$id++;
						...
						{
							?>
							<th   width="45"  style="vertical-align:middle;height:auto;" title="<? echo $id;?>"><div class="block_div"  style=""><?  echo $measurement_discrep_type;   ?></div></th>
			...
			<table width="<?= $td_width; ?>" class="rpt_table" border="1" rules="all" cellpadding="0" align="left" cellspacing="0" id="table_body_summary">
					...
					foreach($unit_data as $line_id=>$line_data)
					{
						...
						foreach($sew_line_defect_type[1] as $type_id=>$defect_type)
						{
							...
							$fronBackWest_defect_qty=$alt_defect_qty;
						?>
						<td  width="45"  style="" align="right"><?  echo $fronBackWest_defect_qty;   ?></td>
						...
						foreach($sew_line_defect_type[2] as $type_id=>$defect_type)
						{
							$spot_defect_qty= $sew_line_qty_arr[$company_id][$unit_id][$line_id][$style_ref][2][$type_id]['defect_qty'];
							
							
						?>
						<td  width="45"  style="" align="right"><?  echo $spot_defect_qty;   ?></td>
						
						

						<?
						$spot_defect_qty_arr[$type_id]+=$spot_defect_qty;
						$line_spot_defect_qty_arr[$line_id]+=$spot_defect_qty;
						}
						...
						foreach($sew_line_defect_type[7] as $measure_type_id=>$measurement_discrep_type)
						{
							$mesurement_qty=$sew_line_qty_arr[$company_id][$unit_id][$line_id][$style_ref][7][$measure_type_id]['defect_qty'];;
						?>
						<td  width="45" class="" align="right" style=""><?  echo $mesurement_qty;   ?></td>
						...
						<td width="70" align="right"><? $tot_delectQty=$line_frontbackWest_defect_qty_arr[$line_id]+$line_spot_defect_qty_arr[$line_id]+$line_measure_defect_qty_arr[$line_id];
					...
					<tr class="tbl_bottom">
						...
						{
						?>
						<td  width="45" style="" title="DefectType=<? echo $type_id; ?>"  align="right"><?  echo $frontbackWest_defect_qty_arr[$type_id];  ?></td>
						<?
						$sew_defect_top6_arr[$sew_line_defect_type[1][$type_id]]=$frontbackWest_defect_qty_arr[$type_id];
						}
						foreach($sew_line_defect_type[2] as $type_id=>$defect_type)
						{
						?>
						<td  width="45" style="" title="DefectType=<? echo $type_id; ?>"  align="right"><?  echo $spot_defect_qty_arr[$type_id];  ?></td>
						<?
						// $sew_defect_top6_arr[$sew_line_defect_type[2][$type_id]]=$spot_defect_qty_arr[$type_id];
						}
						...
						{
							?>
							<td  width="45" class="" align="right" style=""><?  echo $mesurement_defect_qty_arr[$measure_type_id];   ?></td>
							<?
							$sew_defect_topMeasure_arr[$sew_line_defect_type[7][$measure_type_id]]=$mesurement_defect_qty_arr[$measure_type_id];
						}
						?>
				...
				<tr>
					...
					<tr>
						...
						foreach($sew_defect_top6_arr2 as $key_type=>$top_defect_qty)
						{
							...
							if($t<=6)
							{
								$top_six_percent_graph_arr[$sew_fin_alter_defect_type[$key_type]]=($top_defect_qty/$total_adult_qty)*100;
							?>
							<tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('trtop_<? echo $t; ?>','<? echo $bgcolor;?>')" id="trtop_<? echo $t;?>">
							<td  align="center"><? echo $t;?></td>
							<td  align="center"><? echo $sew_fin_alter_defect_type[$key_type];?></td>
							<td  align="right"><? echo $total_adult_qty;?></td>
							<td  align="right"><? echo $top_defect_qty;?></td>
							<td  align="right" title="Defect Qty/Audit Qty*100"><? echo number_format((($top_defect_qty/$total_adult_qty)*100),2); ?></td>
							</tr>
