...
{ 
	...
	if($type==1)
	{
		...
		else if($db_type==2)
		{
			...
			$sql_query="select  a.company_id, a.location, a.floor_id, a.prod_reso_allo, a.production_date, a.sewing_line, b.buyer_name as buyer_name, b.style_ref_no, b.set_break_down,b.set_smv, a.po_break_down_id, e.item_number_id, c.po_number as po_number,
				sum(d.production_qnty) as good_qnty";
				$first=1;
				...
				$sql_query.=", sum(CASE WHEN $production_hour>'$start_hour_arr[$last_hour]' and $production_hour<='$start_hour_arr[24]' and d.production_type=11 THEN d.production_qnty else 0 END) AS good_24 from pro_garments_production_mst a,pro_garments_production_dtls d, wo_po_details_master b, wo_po_break_down c,wo_po_color_size_breakdown e
				...
				group by a.company_id, a.location, a.floor_id, a.po_break_down_id, a.prod_reso_allo, a.production_date, a.sewing_line, b.buyer_name, b.style_ref_no, b.set_break_down,b.set_smv, e.item_number_id, c.po_number order by a.location, a.floor_id, a.po_break_down_id";
		...
		foreach($sql as $val)
		{
			...
			{
				$production_data_arr[$val[csf('floor_id')]][$val[csf('sewing_line')]]['item_number_id'].="****".$val[csf('po_break_down_id')]."**".$val[csf('item_number_id')]."**".$val[csf('style_ref_no')]; 
			}
			else
			{
				$production_data_arr[$val[csf('floor_id')]][$val[csf('sewing_line')]]['item_number_id']=$val[csf('po_break_down_id')]."**".$val[csf('item_number_id')]."**".$val[csf('style_ref_no')]; 
			...
			for($iii=1; $iii<=24; $iii++){
				...
				if($iii<10){$iii='0'.$iii;}
				if($val[csf("good_".$iii)]){
					$production_data_arr[$val[csf('floor_id')]][$val[csf('sewing_line')]]['production_hour']+=1;
				}
			}
		...
		foreach($production_data_arr as $f_id=>$fname)
		{
			...
			foreach($fname as $l_id=>$ldata)
			{
				...
				foreach ($germents_item as $value) 
				{
					...
					// echo $production_data_arr2[$po_id][$f_id][$l_id][$item_id]."<br>";
					if($production_data_arr2[$po_id][$f_id][$l_id][$item_id] !="")
					{
						...
						foreach ($break_down_smv_arr as $smv) 
						{
							...
							//echo $set_smv[2]."<br>";
							//echo $item_id."==".$set_smv[0]."<br>";
							//$setSmv = number_format($set_smv[2],2);
				...
				foreach($germents_item as $g_val)
				{
					...
					if(!isset($chk_item_array[$style_ref][$item_id]))
					{	
						if($garment_itemname!='') $garment_itemname.=',';
							$garment_itemname.=$garments_item[$po_garment_item[1]];
					}	
					$chk_item_array[$style_ref][$item_id] = $po_garment_item[1];
		...
		$html.='<tr  bgcolor="#B6B6B6">
			...
			<td align="right" width="80">'.number_format($floor_ttl_tgt,0).'</td>
			<td align="right" width="80">'.$floor_today_product.'</td>
			<td align="right" width="80">'.number_format(($floor_today_product-$floor_ttl_tgt),2).'</td>
			<td align="right" width="100">'.$floor_avale_minute.'</td>
			<td align="right" width="100">'.number_format($floor_produc_min,2).'</td>
			<td align="right" width="60">'.number_format(($floor_today_product/$floor_ttl_tgt)*100,2).'%</td>
			<td align="right" width="90">'.number_format($floor_efficency,2).' %</td>
		...
		<fieldset style="width:2680px">
			...
			<table id="table_header_1" class="rpt_table" width="2730" cellpadding="0" cellspacing="0" border="1" rules="all">
				<thead>
					<tr height="50">
						<th width="40">SL</th>
						<th width="80">Floor Name</th>
			...
			<div style="width:2747px; max-height:400px; overflow-y:scroll" id="scroll_body_2">
				<table class="rpt_table" width="2730" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body">
					<? echo $html;  ?>
					<tfoot>
						<tr>
							<th width="40">&nbsp;</th>
	...
	else if($type==2)
	{
		...
		else if($db_type==2)
		{
			...
			$sql_query="SELECT  a.company_id, a.location, a.floor_id, a.prod_reso_allo, a.production_date, a.sewing_line, b.buyer_name as buyer_name, b.style_ref_no,b.set_break_down, b.set_smv, a.po_break_down_id, a.item_number_id, c.po_number as po_number,
				sum(d.production_qnty) as good_qnty";
				$first=1;
				for($h=$hour;$h<$last_hour;$h++)
				{
				...
				FROM pro_garments_production_mst a,pro_garments_production_dtls d, wo_po_details_master b, wo_po_break_down c,wo_po_color_size_breakdown e
				where a.production_type=5 and d.production_type=5 and a.id=d.mst_id and   a.po_break_down_id=c.id and c.job_no_mst=b.job_no and a.po_break_down_id=e.po_break_down_id and d.color_size_break_down_id=e.id and b.job_no=e.job_no_mst and c.id=e.po_break_down_id and  a.status_active=1 and a.is_deleted=0 and c.is_deleted=0 and d.status_active=1 and c.is_deleted=0 and c.status_active in(1,2,3)  and e.status_active in(1,2,3) and e.is_deleted=0   $company_name $buyer $location $floor $line $txt_date_from 
				group by a.company_id, a.location, a.floor_id, a.po_break_down_id, a.prod_reso_allo, a.production_date, a.sewing_line, b.buyer_name, b.style_ref_no, b.set_break_down,b.set_smv, a.item_number_id, c.po_number order by a.location, a.floor_id, a.po_break_down_id";
				//echo $h;
		}
		...
		foreach($production_data_arr as $f_id=>$fname)
		{
			...
			//echo $total_adjustment.'+('.$prod_resource_array[$l_id][$pr_date]['man_power'].')*'.$cla_cur_time.'*60#';
				...
				$html.='<td width="40">'.$i.'&nbsp;</td>
							...
							<td align="right" width="80"><a href="##" onclick="openmypage('."'".$cbo_company_id."'".",'".$order_no_total."',".$f_id.",".$l_id.",'tot_prod','".$smv_for_item."',".$txt_date.',5)">'.$today_product.'</a></td>
							
							<td align="right" width="80">'.number_format(($today_product-$eff_target),0).'</td>
							<td align="right" width="100">'.number_format($efficiency_min,2).'</td>
							<td align="right" width="100"><a href="##" onclick="openmypage('."'".$cbo_company_id."'".",'".$order_no_total."',".$f_id.",".$l_id.",'tot_prod','".$smv_for_item."',".$txt_date.',5)">'.number_format($produce_minit,2).'</a></td>
...
if($action=="tot_prod")
{
	...
	<fieldset style="width:620px; ">
        ...
        <table border="1" class="rpt_table" rules="all" width="600" cellpadding="0" cellspacing="0" align="center">
			...
			<?
			$production_hour="TO_CHAR(production_hour,'HH24:MI')";
            $sql_pop="select b.style_ref_no, c.po_number, a.po_break_down_id, sum(d.production_qnty) as good_qnty";
			$first=1;
			for($h=$hour;$h<$last_hour;$h++)
			{
				$bg=$start_hour_arr[$h];
				...
				where a.id=d.mst_id and d.production_type=$pro_type and f.id=d.color_size_break_down_id and d.status_active=1 and d.is_deleted=0 and  a.production_type=$pro_type and a.po_break_down_id=c.id and c.job_no_mst=b.job_no and c.id=f.po_break_down_id and a.status_active=1 and a.is_deleted=0 and  c.status_active in(1,2,3) and c.is_deleted=0 and f.status_active in(1,2,3) and f.is_deleted=0  and c.is_deleted=0  and a.serving_company=".$company_id."  and a.floor_id=".$floor_id." and a.sewing_line=".$sewing_line."  and a.po_break_down_id in(".$po_id.") and a.production_date='".$prod_date."'  group by b.style_ref_no, c.po_number,a.po_break_down_id order by  c.po_number ";
				...
				$sql_query_subcon="SELECT  c.cust_style_ref as style_ref_no, 00 as po_break_down_id,c.smv as sub_smv, c.order_no as po_number,
				sum(d.prod_qnty) as good_qnty";
				$first=1;
				...
				group by c.cust_style_ref, c.order_no,c.smv";
			...
			foreach($sql_res as $pop_val)
			{
				...
				if($pop_val[csf('po_break_down_id')]=="00")
				{
					$item_smv_pop=$pop_val[csf('sub_smv')];
				}
