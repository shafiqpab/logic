...
function hs_chart_mm(gtype,cData,dataTitle){
    ...
    $('#container'+gtype).highcharts({
        ...
        title: {
            text: 'Wash & Measurement '+dataTitle,
			style: {
				 fontSize: '16px',
				 fontWeight: 'bold'
			  }
...
<script>
	...
	{
		$("#txt_job_no").val("");
		var page_link='requires/date_wise_finishing_defect_report_controller.php?action=job_popup';
	...
	{
		$("#txt_order_no").val("");
		var page_link='requires/date_wise_finishing_defect_report_controller.php?action=order_popup';
	...
	function fn_generate_report(type)
	{
		...
		freeze_window(3);
		http.open("POST","requires/date_wise_finishing_defect_report_controller.php",true);
	...
	function openmypage_challan_popup(company_id,work_comp_ids,order_id,job_no,buyer_id,location_ids,floor_ids,txt_date_from,txt_date_to,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/date_wise_finishing_defect_report_controller.php?data='+data+'&action='+action, 'Channan Details', 'width=630px,height=400px,center=1,resize=0,scrolling=0','../');
	}
	...
	function fn_on_change()
	{
		...
		load_drop_down( 'requires/date_wise_finishing_defect_report_controller', cbo_company, 'load_drop_down_location', 'td_lc_location' );
	...
	function fn_on_change_wk()
	{
		...
		//var cbo_working_company_name = $("#cbo_working_company_name").val();
		load_drop_down( 'requires/date_wise_finishing_defect_report_controller', wk_company_name, 'load_drop_down_location_wk', 'location_wk_td' );
	...
	function fn_on_change_floor()
	{
		...
		//var cbo_working_company_name = $("#cbo_working_company_name").val();
		load_drop_down( 'requires/date_wise_finishing_defect_report_controller', wk_location, 'load_drop_down_floor', 'floor_wk_td' );
	...
	function fn_on_change_line()
	{
		...
		//var cbo_working_company_name = $("#cbo_working_company_name").val();
		load_drop_down( 'requires/date_wise_finishing_defect_report_controller', wk_floor_name, 'load_drop_down_floor_line', 'line_wk_td' );
      ...
      //  var style_owner = $("#cbo_style_owner").val();
		...
		var lc_location = $("#cbo_lc_location").val();
			...
			else
			{
				...
				//alert(company_name);
		//var page_link='requires/date_wise_finishing_defect_report_controller.php?action=order_no_search_popup&work_company='+work_company+'&company_name='+company_name+'&lc_location='+lc_location+'&type='+type;
		var page_link='requires/date_wise_finishing_defect_report_controller.php?action=order_no_search_popup&company_name='+company_name+'&lc_location='+lc_location+'&type='+type;
...
<body onLoad="set_hotkey();">
	...
	<h3 style="width:1350px; margin-top:20px;" align="center" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3>
        		...
        		<table class="rpt_table" width="1350" cellpadding="0" cellspacing="0" align="center" border="1" rules="all">
	                <thead>                    
	                    <tr>
	                        ...
	                        <th width="150" class="">W. Company</th>
...
<script> 
	...
	set_multiselect('cbo_wk_location','0','0','','0');
