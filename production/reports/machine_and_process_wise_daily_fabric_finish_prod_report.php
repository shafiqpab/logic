...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Machine and Process Wise Daily Fabric Finishing Production Report", "../../", 1, 1,$unicode,1,1);
?>	
<script>
...
var permission = '<? echo $permission; ?>';	
	...
	function openmypage_machine()
	{
		if(form_validation('cbo_company_id','Company Name')==false)
		{
		 ...
		 // alert(data);
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/machine_and_process_wise_daily_fabric_finish_prod_report_controller.php?action=machine_no_popup&data='+data,'Machine Name Popup', 'width=470px,height=420px,center=1,resize=0','../')
	...
	function fn_report_generated(operation)
	{
		...
		freeze_window(3);
		http.open("POST","requires/machine_and_process_wise_daily_fabric_finish_prod_report_controller.php",true);
	...
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/machine_and_process_wise_daily_fabric_finish_prod_report_controller.php?machine_id='+machine_id+'&date='+date+'&action='+action, 'Cause of Machine Idle', 'width=600px,height=450px,center=1,resize=0,scrolling=0','../');
	}
	...
	$(function()
		{
			$('#txt_batch_color').keyup(function() 
			{
				...
				$.ajax({
					url: 'requires/machine_and_process_wise_daily_fabric_finish_prod_report_controller.php',
					type: 'POST',
	...
	function openmypage_fabricBooking()
	{
		...
		{ 	
			...
			var page_link = 'requires/machine_and_process_wise_daily_fabric_finish_prod_report_controller.php?cbo_company_id='+cbo_company_id+'&action=fabricBooking_popup';
            ...
            <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" align="center" rules="all" border="1">
                ...
                <tbody>
                    <tr>
                        <td> 
							<?
								echo create_drop_down( "cbo_company_id", 100, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/machine_and_process_wise_daily_fabric_finish_prod_report_controller', this.value, 'load_drop_down_location', 'location_td' );load_drop_down( 'requires/machine_and_process_wise_daily_fabric_finish_prod_report_controller',document.getElementById('cbo_order_type').value+'_'+this.value, 'load_drop_down_buyer', 'buyer_td' );" );
						    ?>
						</td>
						<td id="location_td">
						    <? 
                        ...
                        <td>
                            <? 
                            	...
                            	echo create_drop_down( "cbo_order_type", 100, $order_type,"", 1, "-- Select Type --", 0, "load_drop_down( 'requires/machine_and_process_wise_daily_fabric_finish_prod_report_controller',this.value+'_'+document.getElementById('cbo_company_id').value, 'load_drop_down_buyer', 'buyer_td' );order_type();");
                            ?>
                        </td>
                        <td id="buyer_td">
                        	<? 
