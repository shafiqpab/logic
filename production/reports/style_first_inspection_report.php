<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Style First Inspection Report
Functionality	:	
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Style First Inspection Report","../../", 1, 1, $unicode,1); 
?>	
<script>
		...
		var tableFilters_1 = 
		 {
			...
			col_operation: {
			id:["value_sizelbs_td_1","value_kni_td_1","value_first_in_rec_td_1","value_first_ins_td_1","value_issue_td_1","value_bal_td_1"],
			col: [8,9,10,11,12,13],
			operation: ["sum","sum","sum","sum","sum","sum"],
			write_method: ["innerHTML","innerHTML","innerHTML","innerHTML","innerHTML","innerHTML"]
		 }
	...
	function generate_report(type)
	{
		...
		if(type==2){
			...
			var data="action=report_summery_generate"+get_submitted_data_string('cbo_company_id*cbo_buyer_id*txt_job_no*txt_style_ref_no*hiden_order_id*txt_date_from*txt_date_to*txt_qr_no',"../../")+'&report_title='+report_title+'&type='+type;
		}else{

		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_buyer_id*txt_job_no*txt_style_ref_no*hiden_order_id*txt_date_from*txt_date_to*txt_qr_no',"../../")+'&report_title='+report_title+'&type='+type;
		}
		
		// alert(data); return;
		freeze_window(3);
		http.open("POST","requires/style_first_inspection_report_controller.php",true);
	...
	function openmypage_job_no() // For Line number
	{
		...
		var page_link='requires/style_first_inspection_report_controller.php?action=openJobNoPopup&companyID='+company+'&buyer_name='+buyer;  
	...
	function openmypage_order_no() 
	{
		...
		var buyer = $("#cbo_buyer_id").val();
		var job_no = $("#txt_job_no").val();
		var page_link='requires/style_first_inspection_report_controller.php?action=order_no_popup&lc_company='+lc_company+'&buyer='+buyer+'&job_no='+job_no;  
	...
	{ 
		...
		var page_link='requires/style_first_inspection_report_controller.php?action=lotratio_popup&company_id='+company_id; 
	...
	function fnc_bundelDtls(companyid,bundleNo,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/style_first_inspection_report_controller.php?companyid='+companyid+'&bundleNo='+bundleNo+'&action='+action, 'Details View', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../');
	}
             ...
             <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    ...
                    <input name="txt_date_to" id="txt_date_to" class="datepicker"  style="width:55px"  placeholder="To Date"  ></td>
                        <td><input type="button" name="search1" id="search1" value="Details" onClick="generate_report(1);" style="width:70px" class="formbutton" />
						<input type="button" name="search2" id="search2" value="Summery" onClick="generate_report(2);" style="width:70px" class="formbutton" />
						</td>
                    </tr>
                </tbody>
            </table>
