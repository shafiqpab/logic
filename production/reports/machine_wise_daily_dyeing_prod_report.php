...
Updated by 		: 	Shafiq	
Update date		: 	21-01-2020 (machine wise button)	   
QC Performed BY	:		
QC Date			:	
Comments		:
*/
...
var permission = '<? echo $permission; ?>';	
	...
	function openmypage_machine()
	{
		if(form_validation('cbo_company_id','Company Name')==false)
		{
		 ...
		 // alert(data);
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/machine_wise_daily_dyeing_prod_report_controller.php?action=machine_no_popup&data='+data,'Machine Name Popup', 'width=470px,height=420px,center=1,resize=0','../')
	...
	function fn_report_generated(operation)
	{
		...
		// alert(datediff);
		if(datediff !='1' && operation=='5')
		{
			alert('Production date can not mix, Date range will same.');
			$('#txt_date_from').focus();
			return;
		}
		...
		if (operation==5) 
		{
			var data="action=report_generate3"+get_submitted_data_string('cbo_company_id*cbo_location_id*cbo_order_type*cbo_buyer_name*txt_booking_no*txt_order_no*txt_batch*hidden_color_id*txt_color_range*cbo_floor_id*txt_machine_name*txt_machine_id*txt_date_from*txt_date_to',"../../")+'&report_title='+report_title+'&datediff='+datediff;
		}
		
		freeze_window(3);
		http.open("POST","requires/machine_wise_daily_dyeing_prod_report_controller.php",true);
	...
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/machine_wise_daily_dyeing_prod_report_controller.php?machine_id='+machine_id+'&date='+date+'&action='+action, 'Cause of Machine Idle', 'width=600px,height=450px,center=1,resize=0,scrolling=0','../');
	}
	...
	$(function()
		{
			$('#txt_batch_color').keyup(function() 
			{
				...
				$.ajax({
					url: 'requires/machine_wise_daily_dyeing_prod_report_controller.php',
					type: 'POST',
	...
	function openmypage_fabricBooking()
	{
		...
		{ 	
			...
			var page_link = 'requires/machine_wise_daily_dyeing_prod_report_controller.php?cbo_company_id='+cbo_company_id+'&action=fabricBooking_popup';
            ...
            <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" align="center" rules="all" border="1">
                ...
                <tbody>
                    <tr>
                        <td> 
							<?
								echo create_drop_down( "cbo_company_id", 100, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/machine_wise_daily_dyeing_prod_report_controller', this.value, 'load_drop_down_location', 'location_td' );load_drop_down( 'requires/machine_wise_daily_dyeing_prod_report_controller',document.getElementById('cbo_order_type').value+'_'+this.value, 'load_drop_down_buyer', 'buyer_td' );" );
						    ?>
						</td>
						<td id="location_td">
						    <? 
                        ...
                        <td>
                            <? 
                            	$order_type = array(1=>'Self Order', 2=>'Subcontract Order', 3=>'Sample Without Order');
                            	echo create_drop_down( "cbo_order_type", 100, $order_type,"", 1, "-- Select Type --", 0, "load_drop_down( 'requires/machine_wise_daily_dyeing_prod_report_controller',this.value+'_'+document.getElementById('cbo_company_id').value, 'load_drop_down_buyer', 'buyer_td' );order_type();");
                            ?>
                        </td>
                        <td id="buyer_td">
                        	<? 
                ...
                <tr>
					...
					<td align="center" colspan="4">
                        ...
                        <input type="button" id="show_button" class="formbutton" style="width:90px" value="Machine Wise" onClick="fn_report_generated(5)" />&nbsp;
