...
<script>
	...
	function fn_report_generated(type)
	{
		...
		if(txt_file_no=="" && txt_job_no=="" && txt_order_no=="" && txt_inter_ref=="" && txt_barcode_no=="" && txt_style_ref_no=="" && txt_batch_no=="" && txt_booking_no =="")
		{
		...
		if(type==1)
		{
           /*  if(txt_file_no=="" && txt_job_no=="" && txt_order_no=="" && txt_inter_ref=="" && txt_barcode_no=="" && txt_style_ref_no=="" && txt_booking_no =="" && txt_batch_no!="")
            {
                alert("Please Select Report2");
                return;
            } */
	  		var report_title=$( "div.form_caption" ).html();
	  		var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_file_no*txt_job_no*txt_style_ref_no*txt_order_no*txt_inter_ref*txt_barcode_no*hdn_batch_no*txt_batch_no*cbo_year*txt_booking_no*booking_without_order',"../../")+'&report_title='+report_title;
		}
		else
		{
			var report_title=$( "div.form_caption" ).html();
	...
	function openmypage_booking()
	{
		...
		{ 	
			...
			var title = 'Roll Position Tracking Report';	
			var page_link = 'requires/roll_position_tracking_report_controller.php?cbo_company_id='+cbo_company_id+'&cbo_buyer_name='+cbo_buyer_name+'&action=booking_popup';		  
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1050px,height=470px,center=1,resize=1,scrolling=0','../');		
			emailwindow.onclose=function()
			{
				document.getElementById('txt_booking_no').value = this.contentDoc.getElementById("selected_booking_no").value;
				document.getElementById('booking_without_order').value = this.contentDoc.getElementById("booking_without_order").value;
			}
		}
	}
	...
	function fnc_reset(){
		reset_form('knitDyeingLoadReport_1','report_container*report_container2','','','');
		$('#booking_without_order').val(0);	
	}
	function print_report_button_setting(company){
		var report_ids = return_ajax_request_value(company, 'get_report_id', 'requires/roll_position_tracking_report_controller');
		$("#show_button").hide();
		$("#show_button2").hide();
		...
		for (var k=0; k<report_id.length; k++)
		{
			...
			if(report_id[k]==256) $("#show_button2").show();
		}
	}
</script>
</head>
             ...
             <fieldset style="width:1060px;">
                 <table class="rpt_table" width="1060" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        <thead>
                            ...
                            <th colspan="2"><input type="reset" name="res" id="res" value="Reset" onClick="fnc_reset()" class="formbutton" style="width:60px" /></th>
                        </thead>
                        <tbody>
                            <tr class="general">
                               <td><? echo create_drop_down( "cbo_company_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "print_report_button_setting(this.value);load_drop_down( 'requires/roll_position_tracking_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" ); ?></td>
                                <td id="buyer_td"><? echo create_drop_down( "cbo_buyer_name", 140, $blank_array,"", 1, "-- Select Buyer --", $selected, "",0,"" ); ?></td>
                                <td>
                                     <input type="text" name="txt_booking_no" id="txt_booking_no" class="text_boxes" placeholder="Browse" onDblClick="openmypage_booking();" style="width:90px" readonly />
                                      <input type="hidden" name="booking_without_order" id="booking_without_order" class="text_boxes"  onDblClick="" style="width:90px" value="0" />
                                </td>
                                <td>
								<? 
                                ...
                                <td><input type="text" name="txt_job_no" id="txt_job_no" class="text_boxes" placeholder="Write" style="width:80px" /></td>
                                 
                                <td><input type="text" name="txt_style_ref_no" id="txt_style_ref_no" class="text_boxes" placeholder="Write" style="width:90px" /></td>
                                <td><input type="text" name="txt_order_no" id="txt_order_no" class="text_boxes" placeholder="Write" style="width:90px" /></td>
                                <td><input type="text" name="txt_file_no" id="txt_file_no" class="text_boxes" placeholder="Write" style="width:80px" /></td>
                                <td><input type="text" name="txt_inter_ref" id="txt_inter_ref" class="text_boxes" placeholder="Write" style="width:90px" /></td>
                                <td><input type="text" name="txt_batch_no" id="txt_batch_no" class="text_boxes" placeholder="Write/Browse" onDblClick="openmypage_batchNo();" style="width:90px" />
                                      <input type="hidden" name="hdn_batch_no" id="hdn_batch_no" class="text_boxes"  onDblClick="" style="width:90px" />
                                </td>
                                <td><input type="text" name="txt_barcode_no" id="txt_barcode_no" class="text_boxes" placeholder="Write" style="width:90px" /></td>
                                <td><input type="button" id="show_button" class="formbutton" style="width:60px;display:none;" value="Show" onClick="fn_report_generated(1)" /></td>
                                <td><input type="button" id="show_button2" class="formbutton" style="width:60px;display:none;" value="Report 2" onClick="fn_report_generated(2)" /></td>
                            </tr>
                        </tbody>
                    </table>
                </fieldset>
