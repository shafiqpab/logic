...
<script>
	...
	function open_style_ref()
	{
		...
		var page_link='requires/style_wise_dhu_report_urmi_controller.php?action=style_wise_search&company='+company+'&buyer='+buyer+'&job_no='+job_no+'&job_id='+job_id+'&cbo_year='+cbo_year; 
	...
	function open_order_no()
	{
		...
		var cbo_year = $("#cbo_year").val();
		var page_link='requires/style_wise_dhu_report_urmi_controller.php?action=order_wise_search&company='+company+'&buyer='+buyer+'&style_id='+style_id+'&job_id='+job_id+'&job_no='+job_no+'&cbo_year='+cbo_year; 
	...
	function open_job_no()
	{
		...
		var buyer=$("#cbo_buyer_name").val();
		var cbo_year = $("#cbo_year").val();	
		var page_link='requires/style_wise_dhu_report_urmi_controller.php?action=job_wise_search&company='+company+'&buyer='+buyer+'&cbo_year='+cbo_year; 
		var title="Search Order Popup";
	...
	function generate_report(report_type)
	{
		...
		if (report_type == 3){
			...
			if( form_validation('cbo_company_name*cbo_wo_company_name*txt_date_from*txt_date_to','Company Name*Working Company*Date Range*Date Range')==false )
			{
				return;
			}
		}
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_wo_company_name*cbo_location*cbo_floor*cbo_floor_group_name*cbo_buyer_name*txt_int_ref*txt_job_no*txt_style_ref*txt_po_no*cbo_production_status*txt_date_from*txt_date_to',"../../")+'&report_title='+report_title+'&report_type='+report_type;
		freeze_window(3);
		http.open("POST","requires/style_wise_dhu_report_urmi_controller.php",true);
	...
	function mypopup(data,action,width,height,title)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/style_wise_dhu_report_urmi_controller.php?data='+data+'&action='+action, title, 'width='+popup_width+','+'height='+popup_height+',center=1,resize=0,scrolling=0','../');
	}	
	...
	{
		var popup_width=width;
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/style_wise_dhu_report_urmi_controller.php?company_id='+company_id+'&action='+action+'&insert_date='+insert_date+'&jobnumber_prefix='+jobnumber_prefix, 'Detail Veiw', 'width='+popup_width+', height=400px,center=1,resize=0,scrolling=0','../');
	}
	...
	function openmypage_order(company_id,order_id,order_number,insert_date,type,action,width,height)
	{
		var popup_width=width;
		var popup_height=height;
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/style_wise_dhu_report_urmi_controller.php?company_id='+company_id+'&action='+action+'&insert_date='+insert_date+'&order_id='+order_id+'&order_number='+order_number+'&type='+type, 'Detail Veiw', 'width='+popup_width+','+'height='+popup_height+',center=1,resize=0,scrolling=0','../');
	}	
	...
	function job_no_popup(type)
	{
		...
		var page_link='requires/style_wise_dhu_report_urmi_controller.php?action=job_wise_search&company='+company+'&buyer='+buyer+'&cbo_year='+cbo_year+'&type='+type+'&txt_job_no='+txt_job_no; 
		var title="Search Order Popup";
		...
		<div style="width:100%;" align="center" id="content_search_panel">
			<form id="dateWiseProductionReport_1"  autocomplete="off">    
				<fieldset style="width:1600px;">
					<table class="rpt_table" width="1580px" cellpadding="0" cellspacing="0" align="center" rules="all">
						<thead>                    
							<tr>
						...
						<tbody>
							<tr class="general">
								<td id="company_td"> 
									<?
									echo create_drop_down( "cbo_company_name", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/style_wise_dhu_report_urmi_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
									?>
								</td>
                                
								<td width="100"> 
									<?
										echo create_drop_down( "cbo_wo_company_name", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/style_wise_dhu_report_urmi_controller', this.value, 'load_drop_down_location', 'location_td' );" );
									?>
								</td>                   
								<td width="100" id="location_td">
									<? 
										echo create_drop_down( "cbo_location", 100, $blank_array,"", 1, "-- Select --", $selected, " load_drop_down( 'requires/style_wise_dhu_report_urmi_controller', this.value, 'load_drop_down_floor', 'floor_td' );", 1, "" );
									?>
								</td>
								...
								<td>
									...
									$production_status = array(1 => "Cutting", 2 => "Sewing", 3 => "Iron");
										echo create_drop_down( "cbo_production_status", 150, $production_status,"", 0, "-- Production Status --", 0, "",0 );
									?>
								</td>
								<td align="center">
								...
								<td>
									<input type="button" id="show_button1" class="formbutton"   value="Summary" onClick="generate_report(1)" />
									<input type="button" id="show_button2" class="formbutton"   value="Details" onClick="generate_report(2)" />
									<input type="button" id="show_button3" class="formbutton"   value="Summary2" onClick="generate_report(3)" />
