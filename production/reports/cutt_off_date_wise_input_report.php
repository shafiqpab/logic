...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../logout.php";
	...
	function generate_report(type)
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_buyer_id*txt_job_no*txt_style_ref_no*txt_order_no*txt_date_from*txt_date_to*hiden_order_id',"../../")+'&report_title='+report_title+'&type='+type;
		
		// alert(data); return;
		
		freeze_window(3);
		http.open("POST","requires/cutt_off_date_wise_input_report_controller.php",true);
	...
	{		
		var page_link='requires/cutt_off_date_wise_input_report_controller.php?action=open_prod_popup&data='+param;  
	...
	function openmypage_job_no() // For Line number
	{
		...
		var page_link='requires/cutt_off_date_wise_input_report_controller.php?action=openJobNoPopup&lc_company='+company+'&buyer='+buyer;  
	...
	function openmypage_style_ref() 
	{
		...
		var buyer = $("#cbo_buyer_id").val();
		var page_link='requires/cutt_off_date_wise_input_report_controller.php?action=style_ref_no_popup&lc_company='+lc_company+'&buyer='+buyer;  
	...
	function openmypage_order_no() 
	{
		...
		var lc_company = $("#cbo_company_id").val();	
		var buyer = $("#cbo_buyer_id").val();
		var page_link='requires/cutt_off_date_wise_input_report_controller.php?action=order_no_popup&lc_company='+lc_company+'&buyer='+buyer;  
