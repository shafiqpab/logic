...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../logout.php";
	...
	function generate_report(type)
	{
		...
		if(type==1)
		{
			var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_location_id*cbo_floor_id*cbo_line*hidden_line_id*cbo_buyer_name*txt_date',"../../")+'&report_title='+report_title;
		}
		else if(type==2)
		{
			var data="action=report_generate_date"+get_submitted_data_string('cbo_company_id*cbo_location_id*cbo_floor_id*cbo_line*hidden_line_id*cbo_buyer_name*txt_date',"../../")+'&report_title='+report_title;
		}
		else
		{
			var data="action=report_generate3"+get_submitted_data_string('cbo_company_id*cbo_location_id*cbo_floor_id*cbo_line*hidden_line_id*cbo_buyer_name*txt_date',"../../")+'&report_title='+report_title;
		}
		freeze_window(3);
		http.open("POST","requires/hourly_production_monitoring_report_gross_controller.php",true);
 ...
 function open_job_no()
	 {
	    ...
	    var page_link='requires/hourly_production_monitoring_report_gross_controller.php?action=job_wise_search&company='+company+'&location='+location+'&floor_id='+floor_id+'&txt_date='+txt_date; 
	...
	function show_remarks_popup(search_string)
	{
		let popup_width='550px'; 
		let action = "remarks_popup";
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/hourly_production_monitoring_report_gross_controller.php?search_string='+search_string+'&action='+action, 'Remarks Veiw', 'width='+popup_width+', height=270px,center=1,resize=0,scrolling=0','../');
	}
         ...
         <fieldset style="width:1090px;">
             <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
             	<thead>
                    ...
                    <th width="230"><input type="reset" name="res" id="res" value="Reset" style="width:100px" class="formbutton" onClick="reset_form('LineWiseProductivityAnalysis_1','report_container','','','')" /></th>
                </thead>
                <tbody>
                    <tr class="general">
                       <td>
							<? 
                                echo create_drop_down( "cbo_company_id", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", 0, "load_drop_down( 'requires/hourly_production_monitoring_report_gross_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );load_drop_down( 'requires/hourly_production_monitoring_report_gross_controller', this.value, 'load_drop_down_location', 'location_td' ); get_php_form_data( this.value, 'company_wise_report_button_setting','requires/hourly_production_monitoring_report_gross_controller' );" );
                            ?>                            
                        </td>
                         <td>
                        ...
                        <td id="line_td">
                                    <input type="text" id="cbo_line"  name="cbo_line"  style="width:140px" class="text_boxes" onDblClick="open_job_no()" placeholder="Browse"  readonly/>                                    <input type="hidden" id="hidden_line_id" name="hidden_line_id" />
                        </td>
                        <td id="buyer_td_id"> 
                            <?
                        ...
                        <td>
                            <input type="button" name="Show" id="Show" value="Show" onClick="generate_report(1)" style="width:55px;display:none;" class="formbutton" />
                            <input type="button" name="Show2" id="Show2" value="Show2" onClick="generate_report(2)" style="width:55px;display:none;" class="formbutton"/>
                            <input type="button" name="Show3" id="Show3" value="Show3" onClick="generate_report(3)" style="width:55px;display:none;" class="formbutton"/>
                        </td>
                    </tr>
                </tbody>
            </table>
