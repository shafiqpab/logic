...
//--------------------------------------------------------------------------------------------------------------------
	...
	<script type="text/javascript">
		...
		function fnc_send_printer_text()
		{
			...
			var barcode_number='';
			var batch_id = $("#hdnBatchId").val();
			var company_id = $("#cmbCompanyName").val();
			batch_number 	= $("#txtBatchNo").val();
			barcode_number  = $("#txtBarcodeNo").val();
			if(batch_number == '' && barcode_number == '')
			{
			...
			// var data = $("#hdnRollIds").val() + "***" + batch_number + "***" + barcode_number;
			var data = company_id + "***" + barcode_number;
			var url = return_ajax_request_value( data, "report_barcode_text_file", "requires/grey_fabric_barcode_sticker_export_report_controller");
			// alert(url);
			window.open("requires/"+trim(url)+".zip","##");
		}
		...
		function fnc_barcode_code128()
		{
			...
			var company_id = $("#cmbCompanyName").val();
			var barcode_number  = $("#txtBarcodeNo").val();
			...
			var response = return_global_ajax_value(company_id, 'check_barcode_generate', '', 'requires/grey_fabric_barcode_sticker_export_report_controller');

			if(response ==1)
			{
				var data = company_id + "***" + barcode_number;
				var url = return_ajax_request_value( data, "print_barcode_one_128_v2", "requires/grey_fabric_barcode_sticker_export_report_controller");
				//alert(url);return;
				window.open(trim(url),"##");
			}
			else
			{
				alert('Variable settings is external device for barcode Please change if need.');
				return;
			}
		...
		function getBarcodeStickerInfo(batch_id, batch_number, barcode_number) {
			var company_id = $("#cmbCompanyName").val();
			var data="action=get_batch_barcodes&batch_id=" + batch_id + "&barcode_number=" + barcode_number + "&batch_number=" + batch_number+ "&company_id=" + company_id;
...
<body onLoad="set_hotkey();">
	<div style="width:100%;" align="center">
		...
		<form name="dailyYarnStatusReport_1" id="dailyYarnStatusReport_1">
			<h3 style="width:680px; margin-top:10px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu(this.id,'content_search_panel','')">-Search Panel</h3>
					...
					<table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                        	<tr>
                        		<td>
                        			<?
                        			echo create_drop_down( "cmbCompanyName", 140, "select id,company_name from lib_company where status_active=1 and is_deleted=0 and core_business not in(3)  order by company_name","id,company_name", 1, "-- Select Company --", 0, "" );
                        			?>
                        		</td>
                                ...
                                <td>
                                 ...
                                 <td>
									 ...
									 <input type="button" id="btn_barcode_128" name="btn_barcode_128" value="Barcode 128 v2" class="formbutton" onClick="fnc_barcode_code128(1)"/>
