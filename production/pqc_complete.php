...
function fnc_company_check(val)
{
	if(val==1)
	{
		...
		{
			get_php_form_data(document.getElementById('cbo_company_name').value,'production_process_control','requires/pqc_complete_controller' );
		}
	}
	else
	{
...
function openmypage(page_link,title)
{
	...
	//			load_drop_down( 'requires/trimming_complete_controller',company_id, 'load_drop_down_location', 'location_td' );
			get_php_form_data(company_id,'load_variable_settings','requires/pqc_complete_controller');
			get_php_form_data(company_id,'load_variable_settings_reject','requires/pqc_complete_controller');
			console.log('variable_reject');
			childFormReset();//child from reset
			console.log('reset');
			get_php_form_data(po_id+'**'+item_id+'**'+country_id, "populate_data_from_search_popup", "requires/pqc_complete_controller" );
			...
			{
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+job_num+'**'+variableSettingsReject, "color_and_size_level", "requires/pqc_complete_controller" );
			}
			else
			{
			...
			show_list_view(po_id+'**'+item_id+'**'+country_id,'show_dtls_listview','cutting_production_list_view','requires/pqc_complete_controller','setFilterGrid(\'tbl_list_search\',-1)');
			show_list_view(po_id,'show_country_listview','list_view_country','requires/pqc_complete_controller','');
 			set_button_status(0, permission, 'fnc_cutting_update_entry',0);
 			load_drop_down( 'requires/pqc_complete_controller', po_id, 'load_drop_down_color_type', 'color_type_td');
 				release_freezing();
	...
	if ( form_validation('cbo_company_name*txt_order_no*cbo_cutting_company*txt_trim_date','Company Name*Order No*Cutting Comapny*Trim Date')==false )
	{
	...
	else
		{
			...
			//alert(data); return;
 			...
 			http.open("POST","requires/pqc_complete_controller.php",true);
...
function cutting_update_entry_Reply_info()
{
 	if(http.readyState == 4)
	{
		...
		else if(reponse[0]==0)//insert
		{
			...
			show_list_view(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id,'show_dtls_listview','cutting_production_list_view','requires/pqc_complete_controller','setFilterGrid(\'tbl_list_search\',-1)');
			 	//cbo_produced_by*
			reset_form('','','txt_trim_qty*txt_reject_qty*txt_alter_qnty*txt_spot_qnty*hidden_break_down_html*hidden_colorSizeID*txt_remark*txt_mst_id','','','txt_trim_date*txt_reporting_hour');
 			get_php_form_data(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id, "populate_data_from_search_popup", "requires/pqc_complete_controller" );

			if(variableSettings!=1) {
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject, "color_and_size_level", "requires/pqc_complete_controller" );
			}
			else
			{
				$("#txt_trim_qty").removeAttr("readonly");
...
function openmypage_bandle( row_id, field_id, colsize)
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/pqc_complete_controller.php?action=bundle_preparation&cutting_qnty='+$('#colSize_'+field_id).val()+'&fld_id='+field_id+'&colsize='+colsize+'&row_id='+row_id+'&ext_data='+ext_data+'&txt_mst_id='+txt_mst_id+'&company_name='+company_name+'&hid_job_num='+hid_job_num+'&hidden_po_break_down_id='+hidden_po_break_down_id,'Bundle Preparation', 'width=900px,height=350px,center=1,resize=1,scrolling=0','');
...
function put_country_data(po_id, item_id, country_id, po_qnty, plan_qnty)
{
	...
	if(variableSettings!=1)
	{
		get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+job_num, "color_and_size_level","requires/pqc_complete_controller");
	}
	else
	{
		$("#txt_trim_qty").removeAttr("readonly");
	...
	show_list_view(po_id+'**'+item_id+'**'+country_id,'show_dtls_listview','cutting_production_list_view','requires/pqc_complete_controller','');
	set_button_status(0, permission, 'fnc_cutting_update_entry',0);
	release_freezing();
}
...
{
	if(str==1)
	 	return_ajax_request_value(1, "print_report_bundle_barcode", "requires/pqc_complete_controller");
	else
		return_ajax_request_value(1, "print_report_operation_barcode", "requires/pqc_complete_controller");
...
function openmypage_defectQty(type)
{
	...
	else
	{
		...
		var page_link = 'requires/pqc_complete_controller.php?hidden_po_break_down_id='+hidden_po_break_down_id+'&txt_mst_id='+txt_mst_id+'&save_data='+save_data+'&defect_qty='+defect_qty+'&all_defect_id='+all_defect_id+'&type='+type+'&action=defect_data';
...
function check_produced_by(val)
{
	...
	{
		var response=return_global_ajax_value( po_id+'**'+item_id, 'piece_rate_order_cheack', '', 'requires/pqc_complete_controller');
...
function fnc_workorder_search(supplier_id)
{
	...
	load_drop_down( 'requires/pqc_complete_controller', company+"_"+supplier_id+"_"+po_break_down_id, 'load_drop_down_workorder', 'workorder_td' );
	//alert($('#cbo_cutting_company option').size())
}

function fnc_workorder_rate(data,id)
{
	get_php_form_data(data+"_"+id, "populate_workorder_rate", "requires/pqc_complete_controller" );
}
...
function show_print_report(type,title)
{
	...
	else
	{
		...
		window.open("requires/pqc_complete_controller.php?data=" + data+'&action='+action, true );
                return;
	}
}
...
function fn_chk_next_process_qty(tableName,index,sizeId) // for color and size level
{
	...
	$.ajax({
		url: 'requires/pqc_complete_controller.php',
		type: 'POST',
		data: data,
                        ...
                        <table width="100%">
                            ...
                            <tr>
            					...
            					<input type="hidden" name="hidden_preceding_process" id="hidden_preceding_process">
								 ...
								 <td width="150">
									 <input name="txt_order_no" placeholder="Double Click to Search" onDblClick="openmypage('requires/pqc_complete_controller.php?action=order_popup&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value,'Order Search')" id="txt_order_no" class="text_boxes" style="width:130px " readonly />
...
<!--                                   <td width="130" onClick="show_report(1)" class="must_entry_caption">Company </td>-->
                                   ...
                                   <td width="100"   class="must_entry_caption">Company </td>
                                    ...
                                    <td width="150">
                                        <?
                                            echo create_drop_down( "cbo_company_name", 140, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, " get_php_form_data(this.value,'load_variable_settings','requires/pqc_complete_controller'); get_php_form_data(this.value,'load_variable_settings_reject','requires/pqc_complete_controller');",1 );
                                        ?>
                              ...
                              <tr>
                                     ...
                                     <td width="150">
                                         <?
                                            echo create_drop_down( "cbo_source", 140, $knitting_source,"", 1, "-- Select Source --", $selected, "fnc_company_check(this.value);load_drop_down( 'requires/pqc_complete_controller', this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_cutt_company', 'cutt_company_td' );dynamic_must_entry_caption(this.value);",0,'1,3' );
                                         ?>
                                     </td>
                                     <td width="100" class="must_entry_caption">PQC.  Company</td>
