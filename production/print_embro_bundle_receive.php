...
<script>
	...
	<?
	if($_SESSION['logic_erp']['mandatory_field'][536]!="")
	{
		$mandatory_field_arr= json_encode( $_SESSION['logic_erp']['mandatory_field'][536] );
	...
	{
		var report_ids = return_global_ajax_value( data, 'load_report_format', '', 'requires/print_embro_bundle_receive_controller');
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				{ 
					...
					if(delivery_basis==3)
					{
						var barcode_nos=return_global_ajax_value(all_data[0], 'bundle_nos_update', '', 'requires/print_embro_bundle_receive_controller');
						//alert(bundle_nos);
						var response_data=return_global_ajax_value_post(trim(barcode_nos)+"**0**"+all_data[0]+"**"+cbo_company_name+"**"+$('#txt_issue_challan_id').val(), 'populate_bundle_data_update', '', 'requires/print_embro_bundle_receive_controller');
	...
	*/
	...
	//generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val(),'embrodary_color_wise_print5','requires/print_embro_bundle_receive_controller');
	...
	function fnc_issue_print_embroidery_entry(operation)
	{
		...
		if(operation==0 || operation==1 || operation==2)
		{
			...
			if('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][536]); ?>') 
			{
				if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][536]); ?>','<? echo implode('*',$_SESSION['logic_erp']['mandatory_message'][536]); ?>')==false) {return;}
			}
			...
			if(delivery_basis==3)
			{
				...
				$("#tbl_details").find('tbody tr').each(function()
				{
					...
					try 
					{
						...
						dataString+='&bundleNo_' + j + '=' + bundleNo + '&orderId_' + j + '=' + orderId + '&gmtsitemId_' + j + '=' + gmtsitemId + '&countryId_' + j + '=' + countryId + '&colorId_' + j + '=' + colorId + '&sizeId_' + j + '=' + sizeId + '&colorSizeId_' + j + '=' + colorSizeId + '&qty_' + j + '=' + qty + '&dtlsId_' + j + '=' + dtlsId + '&rejectQty_' + j + '=' + rejectQty + '&cutNo_' + j + '=' + cutNo+ '&replaceQty_' + j + '=' + replacetQty+'&actualReject_' + j + '=' + actualReject+ '&barcodeNo_' + j + '=' + barcodeNo+ '&isRescan_' + j + '=' + isRescan;
					}
					catch(e) 
					{
	...
	function fnc_issue_print_embroidery_Reply_info()
	{
		if(http.readyState == 4) 
		{
			...
			else if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
			{
				...
				$('#txt_embl_company').attr('disabled','true');
	...
	
	{
		...
		$('#txt_embl_company').removeAttr('disabled');
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				{ 
					var tot_row=$('#tbl_details tbody tr').length; 
					if( (tot_row*1)<1)
					get_php_form_data(hidden_bundle_nos+"_"+hidden_challan_no,'load_mst_data_bundle_scan','requires/print_embro_bundle_receive_controller');
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				{ 
					var tot_row=$('#tbl_details tbody tr').length; 
					if( (tot_row*1)<1)
					get_php_form_data(hidden_bundle_nos+"_"+hidden_challan_id,'load_mst_data_bundle_scan','requires/print_embro_bundle_receive_controller');
	...
	function fnc_rescan_bundle(bundle_no,hidden_challan_id)
	{
		...
		var response_data=return_global_ajax_value(bundle_no+"**"+row_num+"**"+hidden_challan_id+"**"+$('#cbo_company_name').val(), 'populate_bundle_data_rescan', '', 'requires/print_embro_bundle_receive_controller');
		//alert(222)
		$('#tbl_details tbody').prepend(response_data);
		var tot_row=$('#tbl_details tbody tr').length; 
		$('#txt_tot_row').val(tot_row);
	...
	{
		var challan_duplicate=return_ajax_request_value( barcode_no+"__"+$('#cbo_company_name').val(),"challan_duplicate_check", "requires/print_embro_bundle_receive_controller");
		...
		else
		{
			...
			if(n>0)
			{
				...
				var page_link='requires/print_embro_bundle_receive_controller.php?action=bundle_popup_single&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value;
				...
				emailwindow.onclose=function()
				{
					...
					{ 
						...
						var tot_row=$('#tbl_details tbody tr').length; 
						if( (tot_row*1)<1)
							get_php_form_data(single_bundle_info[3]+"_"+single_bundle_info[0],'load_mst_data_bundle_scan','requires/print_embro_bundle_receive_controller');
	...
	function create_scan_row(barcode_nos,challan_id)
	{
		...
		var response_data=return_global_ajax_value(barcode_nos+"**"+row_num+"**"+challan_id+"**"+$('#cbo_company_name').val(), 'populate_bundle_data', '', 'requires/print_embro_bundle_receive_controller');
		$('#tbl_details tbody').prepend(response_data);
		var tot_row=$('#tbl_details tbody tr').length; 
	...
	function create_row(bundle_nos)
	{
		...
		var row_num=$('#tbl_details tbody tr').length; 
		var issue_challan_id=$("#txt_issue_challan_id").val();
		var print_issue_challan_scan=$("#txt_print_issue_challan_scan").val();
		var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"**"+issue_challan_id+"**"+$('#cbo_company_name').val()+"**"+print_issue_challan_scan, 'populate_bundle_data', '', 'requires/print_embro_bundle_receive_controller');
		$('#tbl_details tbody').prepend(response_data);
		var tot_row=$('#tbl_details tbody tr').length; 
		$('#txt_tot_row').val(tot_row);
		Qty=0;
	...
	function scan_challan_no(chanal_no)
	{
		...
		if(delivery_basis==3)
		{
			...
			{ 
				...
				get_php_form_data(bundle_nos+"_"+challan_id,'load_mst_data_bundle_scan','requires/print_embro_bundle_receive_controller');
				//get_php_form_data(bundle_nos+"_"+0,'load_mst_data','requires/print_embro_bundle_receive_controller');
				create_row(bundle_nos);
	...
	// ====================== printing challan========================
	function openmypage_print_issue_challan()
	{
		var title = 'Challan Selection Form';
		var delivery_basis=$('#delivery_basis').val();
		var cbo_company_name=$('#cbo_company_name').val();		
		var page_link = 'requires/print_embro_bundle_receive_controller.php?delivery_basis='+delivery_basis+'&cbo_company_name='+cbo_company_name+'&action=print_isssue_challan_no_popup';
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				$("#txt_floor_id").val(all_data[5]);
				$("#txt_organic").val(all_data[7]);
				$("#txt_print_issue_challan_scan").val(all_data[12]);
				if(delivery_basis==3)
				{
					//var hidden_bundle_nos=this.contentDoc.getElementById("hidden_bundle_nos").value;//po id
					var bundle_nos=return_global_ajax_value(all_data[0], 'bundle_nos', '', 'requires/print_embro_bundle_receive_controller');
	...
	$('#txt_print_issue_challan_scan').live('keydown', function(e) {
		if (e.keyCode === 13) {
			e.preventDefault();
			scan_print_challan_no(this.value); 
		}
	});

	function scan_print_challan_no(chanal_no)
	{
		var delivery_basis=$('#delivery_basis').val();
		if(delivery_basis==3)
		{
	...
	function calculate_qcpasss_rescan(id)
	{
		...
		var qc_qty=$("#qty_"+id).val()*1;

		if(replace_qty>reject_qty)
		{
			alert('Replace qty can not over reject qty.');
			$("#replaceQty_"+id).val('');
			return;
		}
		
		if(replace_qty>prod_qty)
		{
			$("#replaceQty_"+id).val(prod_qty);
			$("#qty_"+id).val(prod_qty);
			$("#QcQty_"+id).text(prod_qty);
		}
		else
		{
			// alert('ok');
			$("#qty_"+id).val(replace_qty+prod_qty-reject_qty);
			$("#QcQty_"+id).text(replace_qty+prod_qty-reject_qty);
		}
	}
	...
	{
		var challan_duplicate=return_ajax_request_value( bundle_no+"__"+$('#cbo_company_name').val(),"challan_duplicate_check_resacn", "requires/print_embro_bundle_receive_controller");
		var ex_challan_duplicate=challan_duplicate.split("_");
		...
		if(n>0)
		{
			...
			emailwindow.onclose=function()
			{
				...
				{ 
					...
					var tot_row=$('#tbl_details tbody tr').length; 
					if( (tot_row*1)<1)
						get_php_form_data(bundle_no+"_"+single_bundle_info[0],'load_mst_data_bundle_scan','requires/print_embro_bundle_receive_controller');
	...
	function create_rescan_row(bundle_nos,challan_id)
	{
		freeze_window(5);
		var row_num=$('#txt_tot_row').val();
		var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"**"+challan_id+"**"+$('#cbo_company_name').val(), 'populate_bundle_data_rescan', '', 'requires/print_embro_bundle_receive_controller');
		$('#tbl_details tbody').prepend(response_data);
		var tot_row=$('#tbl_details tbody tr').length; 
		$('#txt_tot_row').val(tot_row);
		release_freezing();
	...
	function pop_entry_reject(row_id)
	{
		...
		var page_link='requires/print_embro_bundle_receive_controller.php?action=reject_qty_popup&actual_infos='+actual_infos;
		var title='Reject Record Info';

		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=400px,height=430px,center=1,resize=1,scrolling=0','');
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0]
	...
	function show_cost_details()
	{
		...
		var page_link='requires/print_embro_bundle_receive_controller.php?action=show_cost_details&sys_id='+system_id;
            ...
            <fieldset>
                <table width="100%">
                    ...
                    <tr>
                        ...
                        <td>
                            <? 
                            	echo create_drop_down( "cbo_company_name", 180, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "get_php_form_data(this.value,'load_variable_settings','requires/print_embro_bundle_receive_controller');load_drop_down( 'requires/print_embro_bundle_receive_controller', this.value, 'load_drop_down_body_part', 'body_part_td' );get_php_form_data(this.value,'load_variable_settings_for_working_company','requires/print_embro_bundle_receive_controller');working_com_fnc();load_html();fnc_load_report_format(this.value);" );
                            ?>
                         ...
                         <td id="embel_name_td">
                            ...
                            <td class="must_entry_caption" id="issue_challan_td">Print Prod Issue Challan No </td>
                         <td id="embel_name_td">
                            <input type="text" id="txt_print_issue_challan_scan" name="txt_print_issue_challan_scan" class="text_boxes" placeholder="Scan/Browse"  style=" width:166px;" onDblClick="openmypage_print_issue_challan()"/>
                         </td>
                    </tr>
                     <tr>
                    ...
                    <tr>
                          ...
                          <td id="emb_company_td">
                              <input name="txt_embl_company" id="txt_embl_company" class="text_boxes" type="text" style="width:167px" readonly  />
                              <input name="txt_embl_company_id" id="txt_embl_company_id" class="text_boxes" type="hidden" style="width:167px" />
                          </td>
                          <td>Location</td>
                          <td id="location_td">
                                 <input name="txt_location_name" id="txt_location_name" class="text_boxes" type="text" style="width:167px"  readonly />
                              <input name="txt_location_id" id="txt_location_id" class="text_boxes" type="hidden" style="width:167px" />
                          </td>
                          <td>Floor</td>
                         <td id="floor_td">
                             <input name="txt_floor_name" id="txt_floor_name" class="text_boxes" type="text" style="width:167px" readonly />
					...
					<tr>
						<td>Get Entry Date</td>                                              
						<td> 
							<input type="text" name="get_entry_date" id="get_entry_date" class="datepicker" style="width:167px">
						</td>
						<td>Manual Chalan No</td>                                              
						<td><input type="text" name="txt_manual_challan_no" id="txt_manual_challan_no" class="text_boxes" style="width:167px"></td>
						<td>Get Entry No</td>                                              
						<td> 
                    ...
                    <tr>     
                        ...
                        <td id="id_remarks2" colspan="3" style="display:none"> 
                           <input type="text" name="txt_remark_bundle" id="txt_remark_bundle" class="text_boxes" style="width:455px" title="450 Characters Only." />
                        </td>
					</tr>					
                </table>
                </fieldset> <br />
                ...
                <div id="bundle_list_view">
                    ...
                    <div style="width:970px; max-height:250px; overflow-y:scroll" align="left">    
						...
						<table  cellspacing="0" border=""  id="tbl_details_1" class="rpt_table">      
							<tfoot>
								<tr>									
									<th width="30"></th>
									<th width="80"></th>
									...
									<th width="90"></th>
									<th width="100"></th>
									<th width="80">Tot Bundel</th>
									<th width="80" id="no_of_bundel"></th>
									<th width="60">Gtms.Qty</th>
               	...
               	<table cellpadding="0" cellspacing="1" width="1050px">
               		<tr>
                        <td align="center" colspan="9" valign="middle" class="button_container">
                            ...
                            ?>
                               ...
                               <input type="button" id="btn_print2" name="btn_print2"   style="width:120px;"  class="formbutton" value="Size Wise Print2"  onClick="generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val(),'embrodary_color_wise_print2','requires/print_embro_bundle_receive_controller');"/>
                               
                                <input type="button" id="btn_print5" name="btn_print5"   style="width:120px;"  class="formbutton" value="Print5"  onClick="generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val(),'embrodary_color_wise_print5','requires/print_embro_bundle_receive_controller');"/>

								<input type="button" id="btn_print6" name="btn_print6"   style="width:120px;"  class="formbutton" value="Print6"  onClick="generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val(),'embrodary_color_wise_print6','requires/print_embro_bundle_receive_controller');"/>

								<input type="button" id="btn_print7" name="btn_print7"   style="width:120px;"  class="formbutton" value="Print7"  onClick="generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val(),'embrodary_color_wise_print_7','requires/print_embro_bundle_receive_controller');"/>
