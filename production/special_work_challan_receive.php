...
<script>
    ...
    function openmypage_sysNo()
    {
        ...
        var page_link = 'requires/special_work_challan_receive_controller.php?cbo_company_name=' + cbo_company_name + '&action=challan_no_popup';
        ...
        } else
        {
            ...
            emailwindow.onclose = function ()
            {
                ...
                if (mst_id != "")
                {
                    ...
                    get_php_form_data(all_data[0], "populate_data_from_challan_popup", "requires/special_work_challan_receive_controller");

                    var delivery_basis = $('#delivery_basis').val();
                    if (delivery_basis == 3)
                    {
                        var bundle_nos = return_global_ajax_value(all_data[0], 'bundle_nos_update', '', 'requires/special_work_challan_receive_controller');
                       	var issue_challan_id=$("#txt_issue_challan_id").val();
                        var response_data = return_global_ajax_value(trim(bundle_nos)+"**0**"+all_data[0]+"**"+cbo_company_name+"**"+issue_challan_id, 'populate_bundle_data_update', '', 'requires/special_work_challan_receive_controller');
                    ...
                    {
                        show_list_view(all_data[1], 'show_country_listview', 'list_view_country', 'requires/special_work_challan_receive_controller', '');
                        show_list_view(all_data[0], 'show_dtls_listview', 'printing_production_list_view', 'requires/special_work_challan_receive_controller', '');
                    }
    ...
    {
        window.open("requires/special_work_challan_receive_controller.php?data=" + data + '&action=' + action, true);
    }
    ...
    function fnc_issue_print_embroidery_entry(operation)
    {
        ...
        var working_company_mandatory = return_global_ajax_value(company_id, 'load_variable_settings_for_working_company', '', 'requires/special_work_challan_receive_controller');
        ...
        if (operation == 4)
        {
            ...
            generate_report_file($('#cbo_company_name').val() + '*' + $('#txt_system_id').val() + '*' + $('#delivery_basis').val() + '*' + report_title, 'emblishment_issue_print', 'requires/special_work_challan_receive_controller');
            return;
        }
        ...
        if (operation == 0 || operation == 1 || operation == 2)
        {
            ...
            if (delivery_basis == 3)
            {
                ...
                $("#tbl_details").find('tbody tr').each(function ()
                {
                    ...
                    try
                    {
                        ...
                        dataString += '&bundleNo_' + j + '=' + bundleNo + '&orderId_' + j + '=' + orderId + '&gmtsitemId_' + j + '=' + gmtsitemId + '&countryId_' + j + '=' + countryId + '&colorId_' + j + '=' + colorId + '&sizeId_' + j + '=' + sizeId + '&colorSizeId_' + j + '=' + colorSizeId + '&qty_' + j + '=' + qty + '&dtlsId_' + j + '=' + dtlsId + '&rejectQty_' + j + '=' + rejectQty + '&cutNo_' + j + '=' + cutNo+ '&replaceQty_' + j + '=' + replacetQty+ '&barcodeNo_' + j + '=' + barcodeNo+ '&isRescan_' + j + '=' + isRescan+ '&dtlsId_' + j + '=' + dtlsId;
                    } catch (e)
                    {
            ...
            http.open("POST", "requires/special_work_challan_receive_controller.php", true);
        ...
        if (http.readyState == 4)
        {
			...
			else if (reponse[0] == 0 || reponse[0] == 1 || reponse[0] == 2)
            {
                ...
                } else
                {
                    ...
                    show_list_view(reponse[1], 'show_dtls_listview', 'printing_production_list_view', 'requires/special_work_challan_receive_controller', '');
...
//new need
    ...
    function put_country_data(delivery_id, mst_id, country_id, item_id, order_id)
    {
        ...
        get_php_form_data(delivery_id + '**' + item_id + '**' + $('#cbo_embel_name').val() + '**' + country_id + '**' + mst_id, "populate_data_from_search_popup", "requires/special_work_challan_receive_controller");
    ...
    function openmypage_bundle(page_link, title)
    {
        ...
        } else
        {
			...
			var issue_challan_id=$("#txt_issue_challan_id").val();
            emailwindow = dhtmlmodal.open('EmailBox', 'iframe', page_link + '&bundleNo=' + bundleNo, title, 'width=990px,height=370px,center=1,resize=0,scrolling=0', '')
            emailwindow.onclose = function ()
            {
                var theform = this.contentDoc.forms[0];
                var hidden_bundle_nos = this.contentDoc.getElementById("hidden_bundle_nos").value;//po id
				var hidden_challan_no=this.contentDoc.getElementById("hidden_issue_challan_id").value;//challan No
				
                if (hidden_bundle_nos!="")
				{ 
					var tot_row=$('#tbl_details tbody tr').length; 
					if( (tot_row*1)<1)
					get_php_form_data(hidden_bundle_nos+"_"+hidden_challan_no,'load_mst_data_bundle_scan','requires/special_work_challan_receive_controller');
	...
	function create_scan_row(barcode_nos,challan_id)
	{
		...
		var response_data=return_global_ajax_value(barcode_nos+"**"+row_num+"**"+challan_id+"**"+$('#cbo_company_name').val(), 'populate_bundle_data', '', 'requires/special_work_challan_receive_controller');
		$('#tbl_details tbody').prepend(response_data);
		var tot_row=$('#tbl_details tbody tr').length; 
		$('#txt_tot_row').val(tot_row);
		release_freezing();
	...
	{
		var challan_duplicate=return_ajax_request_value( barcode_no+"__"+$('#cbo_company_name').val(),"challan_duplicate_check", "requires/special_work_challan_receive_controller");
		...
		else
		{
			...
			if(n>0)
			{
				...
				var page_link='requires/special_work_challan_receive_controller.php?action=bundle_popup_single&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value;
				...
				emailwindow.onclose=function()
				{
					...
					{ 
						...
						var tot_row=$('#tbl_details tbody tr').length; 
						if( (tot_row*1)<1)
							get_php_form_data(single_bundle_info[3]+"_"+single_bundle_info[0],'load_mst_data_bundle_scan','requires/special_work_challan_receive_controller');
    ...
    function create_row(bundle_nos)
    {
        ...
        var row_num = $('#txt_tot_row').val();
		var issue_challan_id=$("#txt_issue_challan_id").val();
        var response_data = return_global_ajax_value(bundle_nos+"**"+row_num+"**"+issue_challan_id+"**"+$('#cbo_company_name').val(), 'populate_bundle_data', '', 'requires/special_work_challan_receive_controller');
        $('#tbl_details tbody').prepend(response_data);
        var tot_row = $('#tbl_details tbody tr').length;
        $('#txt_tot_row').val(tot_row);
        release_freezing();
    ...
    function openmypage_issue_challan()
    {
        ...
        var delivery_basis = $('#delivery_basis').val();
        var cbo_company_name = $('#cbo_company_name').val();
        var page_link = 'requires/special_work_challan_receive_controller.php?delivery_basis=' + delivery_basis + '&cbo_company_name=' + cbo_company_name + '&action=isssue_challan_no_popup';
        ...
        emailwindow.onclose = function ()
        {
            ...
            if (mst_id != "")
            {
                ...
                if (delivery_basis == 3)
                {
                    ...
                    var bundle_nos = return_global_ajax_value(all_data[0], 'bundle_nos', '', 'requires/special_work_challan_receive_controller');
                    if (bundle_nos != "")
                    {
                        //get_php_form_data(hidden_bundle_nos,'load_mst_data','requires/special_work_challan_receive_controller');
                        $('#txt_tot_row').val(0);
                        $('#tbl_details tbody tr').remove();
                ...
                {
                    show_list_view(all_data[0], 'show_country_listview', 'list_view_country', 'requires/special_work_challan_receive_controller', '');
                    release_freezing();
                }
            }
    ...
    function scan_challan_no(chanal_no)
    {
        ...
        if (delivery_basis == 3)
        {
            ...
            if (chanal_no != "")
            {
                ...
                if (totalRow > 0) {
                    alert("Scan Bundle Found.");
                    return;
                }

                var challan_id = return_global_ajax_value(chanal_no, 'get_challan_id', '', 'requires/special_work_challan_receive_controller');
                var bundle_nos = return_global_ajax_value(challan_id, 'bundle_nos', '', 'requires/special_work_challan_receive_controller');


                get_php_form_data(bundle_nos, 'load_mst_data', 'requires/special_work_challan_receive_controller');
                create_row(bundle_nos);
                $("#txt_issue_challan_scan").val('');
            }
        } else
        {
            var response_data = return_global_ajax_value(chanal_no + "**" + cbo_cut_panel_basis, 'check_bundle_data', '', 'requires/special_work_challan_receive_controller');
            ...
            show_list_view(all_data[3], 'show_dtls_listview', 'order_list_view', 'requires/special_work_challan_receive_controller', '');
        }
    }
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				{ 
					var tot_row=$('#tbl_details tbody tr').length; 
					if( (tot_row*1)<1)
					get_php_form_data(hidden_bundle_nos+"_"+hidden_challan_id,'load_mst_data_bundle_scan','requires/special_work_challan_receive_controller');
	...
	function fnc_rescan_bundle(bundle_no,hidden_challan_id)
	{
		...
		freeze_window(5);
		var row_num=$('#txt_tot_row').val();
		var response_data=return_global_ajax_value(bundle_no+"**"+row_num+"**"+hidden_challan_id+"**"+$('#cbo_company_name').val(), 'populate_bundle_data_rescan', '', 'requires/special_work_challan_receive_controller');
		//alert(222)
		$('#tbl_details tbody').prepend(response_data);
		var tot_row=$('#tbl_details tbody tr').length; 
		$('#txt_tot_row').val(tot_row);
	...
	{
		var challan_duplicate=return_ajax_request_value( bundle_no+"__"+$('#cbo_company_name').val(),"challan_duplicate_check_resacn", "requires/special_work_challan_receive_controller");
		var ex_challan_duplicate=challan_duplicate.split("_");
		...
		if(n>0)
		{
			...
			emailwindow.onclose=function()
			{
				...
				{ 
					...
					var tot_row=$('#tbl_details tbody tr').length; 
					if( (tot_row*1)<1)
						get_php_form_data(bundle_no+"_"+single_bundle_info[0],'load_mst_data_bundle_scan','requires/special_work_challan_receive_controller');
	...
	function create_rescan_row(bundle_nos,challan_id)
	{
		freeze_window(5);
		var row_num=$('#txt_tot_row').val();
		var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"**"+challan_id+"**"+$('#cbo_company_name').val(), 'populate_bundle_data_rescan', '', 'requires/special_work_challan_receive_controller');
		$('#tbl_details tbody').prepend(response_data);
		var tot_row=$('#tbl_details tbody tr').length; 
		$('#txt_tot_row').val(tot_row);
		release_freezing();
    ...
    function pop_entry_reject(row_id)
	{
		...
		var page_link='requires/special_work_challan_receive_controller.php?action=reject_qty_popup&actual_infos='+actual_infos;
                    ...
                    <fieldset>
                        <table width="100%">
                            <tr>
                                ...
                                <td colspan="3"> 
                                    <input name="txt_challan_no" id="txt_challan_no" class="text_boxes" type="text" style="width:180px" onDblClick="openmypage_sysNo()" placeholder="Double click to search" />
                            ...
                            <tr>
                                ...
                                <td>
                                    <?
                                    echo create_drop_down("cbo_company_name", 180, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name", "id,company_name", 1, "-- Select --", $selected, "get_php_form_data(this.value,'load_variable_settings','requires/special_work_challan_receive_controller'); load_drop_down( 'requires/special_work_challan_receive_controller', this.value, 'load_drop_down_body_part', 'body_part_td' ); working_com_fnc(); load_html();");//get_php_form_data(this.value,'load_variable_settings_for_working_company','requires/special_work_challan_receive_controller');
                                    ?>
                                ...
                                <td id="embel_name_td">
                                    <input type="text" id="txt_issue_challan_scan" name="txt_issue_challan_scan" class="text_boxes" placeholder="Scan/Browse"  style=" width:180px;" onDblClick="openmypage_issue_challan()"/>
                                </td>
                                <td class="must_entry_caption">Delivery Date</td>
                                <td> 
                                    <input type="text" name="txt_issue_date" id="txt_issue_date" value="<? echo date("d-m-Y") ?>" class="datepicker" style="width:180px;"  />
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td id="embro_type_td" width="170">
                                    <?
                                    echo create_drop_down("cbo_embel_type", 180, $emblishment_spwork_type, "", 1, "--- Select Printing ---", $selected, "", 1);
                                    ?>
                                </td>
                            ...
                            <tr>
                                ...
                                <td id="emb_company_td">
                                    <input name="txt_embl_company" id="txt_embl_company" class="text_boxes" type="text" style="width:180px"  placeholder="Dispay" readonly/>
                                    <input name="txt_embl_company_id" id="txt_embl_company_id" class="text_boxes" type="hidden" style="width:167px" />
                                </td>
                                <td>Location</td>
                                <td id="location_td">
                                    <input name="txt_location_name" id="txt_location_name" class="text_boxes" type="text" style="width:180px"  placeholder="Dispay" readonly/>
                                    <input name="txt_location_id" id="txt_location_id" class="text_boxes" type="hidden" style="width:180px" />
                                </td>
                                <td>Floor</td>
                                <td id="floor_td">
                                    <input name="txt_floor_name" id="txt_floor_name" class="text_boxes" type="text" style="width:180px"  placeholder="Dispay" readonly/>
                            ...
                            <tr>
                                ...
                                <td>Organic</td>
                                <td>
                                    <input name="txt_organic" id="txt_organic" class="text_boxes" type="text" style="width:180px" readonly />
                                </td>
                                <td>Body Part</td>
                                <td id="body_part_td">
                                    <?
                                ...
                                <td>
                                    <?
                                    echo create_drop_down("cbo_working_company_name", 180, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name", "id,company_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/special_work_challan_receive_controller', $('#cbo_working_company_name').val(), 'load_drop_down_working_location', 'working_location_td' );");
                                    ?>
                                </td>
                                ...
                                <fieldset>
                                    ...
                                    <table  cellpadding="0" cellspacing="2" width="100%">
                                        <tr>
                                            ...
                                            <td colspan="3" width="110"> 
                                                <input name="txt_bundle_no" placeholder="Browse/Write/Scan" onDblClick="openmypage_bundle('requires/special_work_challan_receive_controller.php?action=bundle_popup&company=' + document.getElementById('cbo_company_name').value + '&garments_nature=' + document.getElementById('garments_nature').value, 'Bundle Search')" id="txt_bundle_no" class="text_boxes" style="width:212px" />
                                            </td>
                                             <td width="100" class="must_entry_caption" id="td_caption">Re-Scan Barcode</td>
                                           <td colspan="2" width="110"> 
                                               <input name="txt_bundle_rescan" placeholder="Browse/Write/Scan" onDblClick="openmypage_bundle_rescan('requires/special_work_challan_receive_controller.php?action=bundle_popup_rescan&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value,'Search Bundle For Rescan')"  id="txt_bundle_rescan" class="text_boxes" style="width:212px" />
                                           </td>
                                        </tr>
                                    </table>
                                </fieldset>
                    ...
                    <table cellpadding="0" cellspacing="1" width="100%">
                        <tr>
                            <td align="center" colspan="9" valign="middle" class="button_container">
                                ...
                                <input type="button" id="btn_print" name="btn_print"   style="width:100px;"  class="formbutton" value="Special Work"  onClick="generate_report_file($('#cbo_company_name').val() + '*' + $('#txt_system_id').val() + '*' + $('#delivery_basis').val(), 'bundle_receive_special_work', 'requires/special_work_receive_controller');"/>
                            </td>
                            <td>&nbsp;</td>				
                        </tr>
                    </table>
