<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Fabric Sales Order Entry For Woven
Functionality	:
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Fabric Sales Order Entry For Woven", "../", 1, 1, '', '', '');
...
<script>
	...
	function openmypage_fabricBooking() {
		...
		if (form_validation('cbo_company_id', 'Company') == false) {
			...
			var page_link = 'requires/fabric_sales_order_entry_woven_controller.php?cbo_company_id=' + cbo_company_id + '&action=fabricBooking_popup';
			...
			emailwindow.onclose = function () {
                ...
                load_drop_down('requires/fabric_sales_order_entry_woven_controller', document.getElementById('cbo_within_group').value + '_' + data[7], 'load_drop_down_dealing_merchant', 'team_td');

                $('#cbo_dealing_merchant').val(data[8]);
                $('#is_apply_last_update').val(0);

                show_list_view(data[1], 'show_fabric_details', 'order_details_container', 'requires/fabric_sales_order_entry_woven_controller', '');
                total_amount_cal();
				total_fin_grey_cal();
                if(data[15] == ""){
    ...
    function pending_booking_data_dtls(data_str, is_approved) 
    {
        ...
        {
            var response=return_global_ajax_value( data_str, 'check_approvl_necessity_setup', '', 'requires/fabric_sales_order_entry_woven_controller');
            if(response==1)
            {
                //$('#hidden_booking_data').val(data);
                //parent.emailwindow.hide();
    ...
    function active_inactive() {
		...
		// var response=trim(return_global_ajax_value( $('#cbo_company_id').val(), 'is_validate', '', 'requires/fabric_sales_order_entry_woven_controller'));
    	...
    	reset_form('', 'approval_msg_td', 'txt_booking_no*txt_booking_no_id*txt_delivery_date*txt_style_ref*cbo_currency*cbo_team_leader*cbo_dealing_merchant*txt_season*cboBodyPart_1*cboColorType_1*txtFabricDesc_1*fabricDescId_1*txtFabricGsm_1*txtFabricDia_1*cboGsmType_1*txtCuttableWidth_1*txtColor_1*colorId_1*txtFinishQty_1*txtAvgRate_1*txtAmount_1*txtProcessLoss_1*txtGreyQty_1*cboWorkScope_1*updateIdDtls_1*txtBookingQnty_1', '', '$(\'#tbl_item_details tbody tr:not(:first)\').remove();', '');
    	...
    	if (within_group == 1) {
    		...
    		$('#cboGsmType_1').attr('disabled', 'disabled');
            $('#txtCuttableWidth_1').attr('disabled', 'disabled');
    		...
    		$('#cboGsmType_1').removeAttr('disabled', 'disabled');
            $('#txtCuttableWidth_1').removeAttr('disabled', 'disabled');
    	...
    	if (company_id == 0) {
    		...
    		load_drop_down('requires/fabric_sales_order_entry_woven_controller', within_group + '_' + company_id, 'load_drop_down_buyer', 'buyer_td');
            load_drop_down('requires/fabric_sales_order_entry_woven_controller', within_group + '_' + company_id, 'load_drop_down_cust_buyer', 'cust_buyer_td');
    	}
    }
    ...
    function add_break_down_tr(i) {
    	...
    	if (within_group != 1) {
    		...
    		if (row_num != i) {
    			...
    			$('#txtBookingQnty_' + i).removeAttr("readonly").removeAttr("onKeyUp").attr("onKeyUp", "calculate_amount(" + i + ");calculate_fin_qty(" + i + ");copy_process_loss(" + i + ");consumtion_calculate(" + i + ");");
                ...
                $('#cboConsUom_' + i).removeAttr("onchange").attr("onchange", "consumtion_calculate(" + i + ");");
    ...
    function fnc_fabric_sales_order_entry(operation) {
    	...
    	if(within_group == 2){
    		...
    		var response=trim(return_global_ajax_value( data, 'is_booking_duplicate', '', 'requires/fabric_sales_order_entry_woven_controller'));
    		if(response == "invalid"){
    	...
    	if(within_group == 2 && operation == 1){
    		if(booking_no != hdn_booking_no){
    			...
    			var response=trim(return_global_ajax_value( data, 'is_booking_used_in_plan', '', 'requires/fabric_sales_order_entry_woven_controller'));
    			if(response == "invalid"){
        ...
        {
            var response=trim(return_global_ajax_value( $('#txt_job_no').val(), 'is_booking_revised', '', 'requires/fabric_sales_order_entry_woven_controller'));
    	...
    	if (operation == 4) {
    		...
    		if (within_group == 1) {
    			window.open("requires/fabric_sales_order_entry_woven_controller.php?data=" + data + '&action=fabric_sales_order_print', true);
    		} else {
    			window.open("requires/fabric_sales_order_entry_woven_controller.php?data=" + data + '&action=fabric_sales_order_print2', true);
    		}
		...
		if (operation == 6) {
    		...
    		if (within_group == 1) {
    			window.open("requires/fabric_sales_order_entry_woven_controller.php?data=" + data + '&action=fabric_sales_order_print_yes_6', true);
    		} else {
    	...
    	$("#tbl_item_details").find('tbody tr').each(function () {
    		...
    		var cboGsmType = $(this).find('select[name="cboGsmType[]"]').val();
            var txtCuttableWidth = $(this).find('input[name="txtCuttableWidth[]"]').val();
            ...
            console.log(finishQty * 1);
            console.log('isMainProcessMandatory='+ isMainProcessMandatory +',isSubProcessMandatory='+isSubProcessMandatory+',iseditable='+iseditable);
    		if (finishQty * 1 > 0) {
				...
				if((fabricDesc == "" || fabricGsm == "" || fabricDia == "" || cboColorRange == 0 || (colorId == 0 && txtColor =="")) && k == 0){
    ...
    function fnc_fabric_sales_order_entry_Reply_info() {
    	...
    	if (http.readyState == 4) {
    		...
    		if ((response[0] == 0 || response[0] == 1)) {
    			...
    			show_list_view(response[1] + "**" + color_from_library + "**" + cbo_within_group + "**" + company_id, 'show_fabric_details_update', 'order_details_container', 'requires/fabric_sales_order_entry_woven_controller', '');
    ...
    function fnc_fabric_yarn_dtls_entry(operation) {
    	...
    	$("#table_yarn_details").find('tbody tr').each(function () {
            ...
            var cboFabricSource = $(this).find('select[name="cboFabricSource[]"]').val();
            var txtConstructionY = $(this).find('input[name="txtConstructionY[]"]').val();
    		j++;
    		dataString += '&fabricDescIdY' + j + '=' + fabricDescIdY + '&txtFabricGsmY' + j + '=' + txtFabricGsmY + '&cboColorRangeY' + j + '=' + cboColorRangeY + '&txtGreyQtyY' + j + '=' + txtGreyQtyY + '&yarnData' + j + '=' + yarnData+ '&cboFabricSource' + j + '=' + cboFabricSource+ '&txtConstructionY' + j + '=' + txtConstructionY;
    ...
    function fnc_fabric_yarn_dtls_entry_Reply_info() {
    	if (http.readyState == 4) {
            ...
            var datas = return_global_ajax_value(response[1], 'yarn_details', '', 'requires/fabric_sales_order_entry_woven_controller');
    ...
    function fabric_sales_order_print3() {
    	var data = $('#cbo_company_id').val() + '*' + $('#txt_booking_no_id').val() + '*' + $('#txt_booking_no').val() + '*' + $('#txt_job_no').val() + '*' + $("div.form_caption").html();
    	window.open("requires/fabric_sales_order_entry_woven_controller.php?data=" + data + '&action=fabric_sales_order_print3', true);
    	return;
    }
    ...
    function fabric_sales_order_print_kds2() {
        ...
        window.open("requires/fabric_sales_order_entry_woven_controller.php?data=" + data + '&action=fabric_sales_order_print_kds2', true);
        return;
        /*if (within_group == 1) {
            window.open("requires/fabric_sales_order_entry_woven_controller.php?data=" + data + '&action=fabric_sales_order_print_kds2', true);
        } else {
    ...
    function fabric_sales_order_print4() {
        ...
        // window.open("requires/fabric_sales_order_entry_woven_controller.php?data=" + data + '&action=fabric_sales_order_print4', true);
        // return;
    ...
    function fabric_sales_order_print5() {
        ...
        var within_group=$('#cbo_within_group').val()
            if (within_group == 2) {
                window.open("requires/fabric_sales_order_entry_woven_controller.php?data=" + data + '&action=fabric_sales_order_print5', true);
            } else {
    ...
    function func_send_mail() 
    {
        ...
        var page_link = 'requires/fabric_sales_order_entry_woven_controller.php?action=send_mail_popup';
    ...
    function openmypage_fabricDescription(i) {
    	...
    	var page_link = 'requires/fabric_sales_order_entry_woven_controller.php?action=fabricDescription_popup';
    	...
    	emailwindow.onclose = function () {
            ...
            var hiddenFabricDia = this.contentDoc.getElementById("hiddenFabricDia").value;
            var hiddenGsmType = this.contentDoc.getElementById("hiddenGsmType").value;
            var hiddenCuttableWidth = this.contentDoc.getElementById("hiddenCuttableWidth").value;

    		$('#txtFabricDesc_' + i).val(theename);
    		$('#fabricDescId_' + i).val(theemail);
    		$('#txtFabricGsm_' + i).val(theegsm);
    		$('#cboColorRange_' + i).val(theecolorrange);
            $('#txtFabricDia_' + i).val(hiddenFabricDia);
            $('#cboGsmType_' + i).val(hiddenGsmType);
            $('#txtCuttableWidth_' + i).val(hiddenCuttableWidth);
    	}
    }

    function openmypage_color(i) {
    	var title = 'Color Info';
    	var page_link = 'requires/fabric_sales_order_entry_woven_controller.php?action=color_popup';
    ...
    function calculate_grey_qty(i) 
    {
        ...
        else 
        {
            ...
            if(cbo_within_group ==1 && textile_sales_maintain==1 )
            {
                //will not change
 ...
 function openmypage_jobNo() {
 	...
 	if (form_validation('cbo_company_id', 'Company') == false) {
 		...
 		var page_link = 'requires/fabric_sales_order_entry_woven_controller.php?cbo_company_id=' + cbo_company_id + '&action=jobNo_popup';
 		...
 		emailwindow.onclose = function () {
            ...
            get_php_form_data(job_id+'_'+booking_data[1], "populate_data_from_sales_order", "requires/fabric_sales_order_entry_woven_controller");

            var cbo_within_group = $('#cbo_within_group').val();
			var company_id = $('#cbo_company_id').val();
            show_list_view(job_id + "**" + color_from_library + "**" + cbo_within_group + "**" + company_id, 'show_fabric_details_update', 'order_details_container', 'requires/fabric_sales_order_entry_woven_controller', '');
            total_amount_cal();
			total_fin_grey_cal();
...
function show_fabric_yarn_details(update_id) {
        //show_list_view(update_id,'yarn_details','yarn_details_list_view','requires/fabric_sales_order_entry_woven_controller','',0);
        var datas = return_global_ajax_value(update_id, 'yarn_details', '', 'requires/fabric_sales_order_entry_woven_controller');
        var yarn_datas = trim(datas).split("##")
        $('#yarn_details_list_view').html(yarn_datas[0]);
     ...
     function apply_last_update() {
     	...
     	if (within_group == 1) {
     		var approved_data = trim(return_global_ajax_value(txt_booking_no, 'check_booking_approval', '', 'requires/fabric_sales_order_entry_woven_controller'));
     		...
     		if (approved != 1) {
     			...
     			var response=return_global_ajax_value( data_for_setup, 'check_approvl_necessity_setup_revised', '', 'requires/fabric_sales_order_entry_woven_controller');
     			if( approved==3){
     				if(response!=1){
     					alert("Approved Booking First.");
     					return;
     		...
     		freeze_window(5);
     		show_list_view(update_id + '**' + txt_booking_no, 'show_fabric_details_last_update', 'order_details_container', 'requires/fabric_sales_order_entry_woven_controller', '');
     		total_amount_cal();
     ...
     function show_change_bookings() {
        ...
        {
     	  show_list_view($('#cbo_company_id').val(), 'show_change_bookings', 'list_change_booking_nos', 'requires/fabric_sales_order_entry_woven_controller', 'setFilterGrid(\'tbl_list_search_revised\',-1);');
        }
     }
     ...
     function show_change_pending_bookings() 
     {
        ...
        {
            show_list_view($('#cbo_company_id').val(), 'show_change_pending_bookings', 'list_change_pending_booking_nos', 'requires/fabric_sales_order_entry_woven_controller', 'setFilterGrid(\'tbl_list_search_pending_booking\',-1);');
        }
     }

     function btn_load_change_bookings(){
     	var count = trim(return_global_ajax_value("", 'btn_load_change_bookings', '', 'requires/fabric_sales_order_entry_woven_controller'));
     	if(count > 0){
     ...
     function set_form_data(data) {
        ...
        var approved_data = trim(return_global_ajax_value(booking_no, 'check_booking_approval', '', 'requires/fabric_sales_order_entry_woven_controller'));
        ...
        if (approved != 1) {
           ...
           /* if (approved != 1) {
            ...
            load_drop_down('requires/fabric_sales_order_entry_woven_controller', cbo_company_id, 'load_drop_down_location', 'location_td');
            get_php_form_data(cbo_company_id, 'process_loss_method', 'requires/fabric_sales_order_entry_woven_controller');

            var color_from_library = $('#color_from_library').val();

            get_php_form_data(job_id, "populate_data_from_sales_order", "requires/fabric_sales_order_entry_woven_controller");

            var within_group = $('#cbo_within_group').val();
            var company_id = $('#cbo_company_id').val();
            show_list_view(job_id + "**" + color_from_library + "**" + within_group + "**" + company_id, 'show_fabric_details_update', 'order_details_container', 'requires/fabric_sales_order_entry_woven_controller', '');
            total_amount_cal();
    ...
    function generate_worder_report(type, txt_booking_no, cbo_company_name, txt_order_no_id, cbo_fabric_natu, cbo_fabric_source, txt_job_no, id_approved_id, booking_entry_form, is_short, action, revised_no="") 
    {
        ...
        get_php_form_data(cbo_company_name+'_'+booking_entry_form, 'print_report_setting_action', 'requires/fabric_sales_order_entry_woven_controller');
    ...
    {
        get_php_form_data(company_id+'_'+entry_form, 'print_report_setting_action', 'requires/fabric_sales_order_entry_woven_controller');
...
function openmypage_process(incr_id)
{
	...
	var page_link = 'requires/fabric_sales_order_entry_woven_controller.php?txt_process_id='+txt_process_id+'&process_seq='+txt_process_seq+'&action=process_name_popup';
...
function openmypage_process_main_____________________bk__________(incr_id)
{
    ...
    var title = 'Process Name Selection Form';
    var page_link = 'requires/fabric_sales_order_entry_woven_controller.php?txt_process_id='+txt_process_id+'&process_seq='+txt_process_seq+'&txtProcessSeqSub='+txtProcessSeqSub+'&action=process_name_popup_main';
...
function openmypage_process_main(incr_id)
{
    ...
    var page_link = 'requires/fabric_sales_order_entry_woven_controller.php?txt_process_id='+txt_process_id+'&process_seq='+txt_process_seq+'&txtProcessSeqSub='+txtProcessSeqSub+'&cbo_within_group='+cbo_within_group+'&cbo_buyer_name='+cbo_buyer_name+'&action=process_name_popup_main';
...
function call_print_button_for_mail(mail_address,mail_body){
	...
	get_php_form_data($('#txt_booking_no').val()+'__seperate__'+mail_address+'__seperate__'+mail_body, 'auto_mail_send', 'requires/fabric_sales_order_entry_woven_controller');
}
	...
	<body onLoad="set_hotkey(); btn_load_change_bookings();">
		<div style="width:100%;" align="center">
			...
			<fieldset style="width:1445px;">
				<legend>Fabric Sales Order Entry For Woven</legend>
				<form name="fabricOrderEntry_1" id="fabricOrderEntry_1">
						...
						<fieldset style="width:970px;">
							<table width="920" align="center" border="0">
								...
								<tr>
									...
									<td width="190">
										<?
										echo create_drop_down("cbo_company_id", 162, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name", 'id,company_name', 1, '--Select Company--', 0, "load_drop_down( 'requires/fabric_sales_order_entry_woven_controller', this.value, 'load_drop_down_location', 'location_td' );get_php_form_data( this.value,'process_loss_method' ,'requires/fabric_sales_order_entry_woven_controller'); active_inactive(); get_php_form_data( this.value, 'company_wise_report_button_setting','requires/fabric_sales_order_entry_woven_controller' )", '', '', '', '', '');
                                        //load_drop_down( 'requires/fabric_sales_order_entry_woven_controller', document.getElementById('cbo_within_group').value+'_'+this.value, 'load_drop_down_buyer', 'buyer_td' );
										?>
									...
									<td>
										<?
										echo create_drop_down("cbo_within_group", 162, $yes_no, "", 0, "--  --", 2, "active_inactive();");
                                        //load_drop_down( 'requires/fabric_sales_order_entry_woven_controller', this.value+'_'+document.getElementById('cbo_company_id').value+'_'+'_80', 'load_drop_down_buyer', 'buyer_td' );
										?>
									</td>
								...
								<tr>
									...
									<td>
										<?
										echo create_drop_down("cbo_team_leader", 162, "select id,team_leader_name from lib_marketing_team where status_active =1 and is_deleted=0 order by team_leader_name", "id,team_leader_name", 1, "-- Select Team Leader--", $selected, "load_drop_down( 'requires/fabric_sales_order_entry_woven_controller', document.getElementById('cbo_within_group').value+'_'+this.value, 'load_drop_down_dealing_merchant', 'team_td' );", 1);
										?>
									</td>
								...
								<tr>
                                    ...
                                    <td colspan="2">
                                        ...
                                        terms_condition(547,'txt_job_no','../');
                                        ?>
                                    </td>
								</tr>
                                <tr>
                                    <td>Order Status</td>
                                    <td><?=create_drop_down( "cbo_order_status", 162, $order_status, 0, "", $selected,"", "" ); ?></td>
                                    <td colspan="4" id="approval_msg_td" style="font-size:18px; color:#F00"  valign="top" align="center"></td>
                                </tr>
							</table>
						</fieldset>
					</div>
                    ...
                    style="max-height:300px; width:210px; overflow:auto; float:left; padding-top:5px; margin-top:5px; position:relative;"></div>
					<div align="left">
						<fieldset style="width:2059px; margin-top:10px">
							<legend>
                            Fabric Details 
							...
							<table cellpadding="0" cellspacing="0" width="2049" class="rpt_table" border="1" rules="all"
							id="tbl_item_thead">
							<thead>
								<th width="35" style="word-break:break-all">SL</th>
								<th width="90" style="word-break:break-all">Garment Item</th>
								<th width="90" style="word-break:break-all">Body Part</th>
								<th width="80" style="word-break:break-all">Color Type</th>
								<th width="152" style="word-break:break-all" class="must_entry_caption">Fabric Description</th>
								<th width="57" class="must_entry_caption">Weight</th>
                                <th width="90" style="word-break:break-all">Weight Type</th>
								<th width="52" style="word-break:break-all" class="must_entry_caption">Full<br>Width</th>
                                <th width="52" style="word-break:break-all">Cutable Width</th>
								
								<th width="87" style="word-break:break-all">Color</th>
								<th width="90" style="word-break:break-all" class="must_entry_caption">Color Range</th>
								<th width="60" style="word-break:break-all">Cons. UOM</th>
								<th width="80" style="word-break:break-all">Booking Qty.</th>
								<th width="57" style="word-break:break-all" class="must_entry_caption">Avg. Price</th>
								<th width="72" style="word-break:break-all" class="must_entry_caption">Amount</th>
								<th width="60" style="word-break:break-all">UOM</th>
								<th width="67" style="word-break:break-all" class="must_entry_caption">Finish Qty.</th>
								<th width="52" style="word-break:break-all">Process <br>Loss %</th>
								<th width="67" style="word-break:break-all">Grey Qty.</th>
								<th width="90" style="word-break:break-all">Work Scope</th>
                                <th width="100" style="word-break:break-all" id="main_process">Main Process &nbsp; <input type="checkbox" checked id="processChkMain" name="processChkMain"/></th>
								<th width="100" style="word-break:break-all" id="main_process">Sub Process &nbsp;<input type="checkbox" checked id="processChk" name="processChk"/></th>
								<th width="100" style="word-break:break-all">Remarks</th>
								<th width="100" style="word-break:break-all">Barcode No</th>
								<th></th>
							</thead>
						</table>
						<div style="width:2049px; max-height:260px; overflow-y:scroll;" id="list_container_batch"
						align="left">
						<table cellpadding="0" cellspacing="0" width="2029" class="rpt_table" border="1" rules="all"
						id="tbl_item_details">
						<tbody id="order_details_container">
							<tr class="general" id="tr_1">
								<td width="35" id="slTd_1">
									<span>1</span>
									<input type="hidden" name="txtSerial[]" id="txtSerial_1" class="text_boxes" value="1" readonly/>
								</td>
								<td width="90">
									<?
									echo create_drop_down("cboGarmItemId_1", 70, $garments_item, "", 1, "- Select -", $row[csf('item_number_id')], "", "", "", "", "", "", "", "", "garmItemId[]");
									?>
								</td>
								<td width="90">
									<?
									echo create_drop_down("cboBodyPart_1", 70, $body_part, "", 1, "- Select -", 0, "", "1", "", "", "", "", "", "", "cboBodyPart[]");
									?>
								</td>
								<td width="80">
									<?
									echo create_drop_down("cboColorType_1", 60, $color_type, "", 1, "- Select -", 0, "", "1", "", "", "", "", "", "", "cboColorType[]");
									?>
								</td>
								<td width="152">
                                ...
                                <td width="90">
									<?
									echo create_drop_down("cboGsmType_1", 75, $fabric_weight_type, "", 1, "-- Select --", 0, "", "1", "", "", "", "", "", "", "cboGsmType[]");
									?>
								</td>
								<td width="52">
								...
								<td width="87">
									...
									style="width:70px" placeholder="Display" readonly/>
								...
								<td width="60">
									<?
									echo create_drop_down("cboConsUom_1", 50, $unit_of_measurement, "", 0, "", "27", "refresh_fields(1);consumtion_calculate(1)", "1", "12,27,23", "", "", "", "", "", "cboConsUom[]");
									?>
								</td>
								<td width="80">
									<input type="text" name="txtBookingQnty[]" id="txtBookingQnty_1" class="text_boxes_numeric"
									style="width:60px"
									onKeyUp="calculate_amount(1); calculate_fin_qty(1);copy_process_loss(1);consumtion_calculate(1);"
									readonly/>
								</td>
								<td width="57">
									<input type="text" name="txtAvgRate[]" id="txtAvgRate_1" class="text_boxes_numeric"
									style="width:40px" onKeyUp="calculate_amount(1);" readonly/>
								</td>
								<td width="72">
								...
								<td width="60">
									<?
									echo create_drop_down("cboUom_1", 50, $unit_of_measurement, "", 0, "", "27", "consumtion_calculate(1)", "1", "12,27,23", "", "", "", "", "", "cboUom[]");
									?>
								</td>
								<td width="67">
								...
								<td width="90">
									<?
									echo create_drop_down("cboWorkScope_1", 80, $item_category, "", 1, "-- Select --", 3, "", "0", "3,14", "", "", "", "", "", "cboWorkScope[]");
									?>
								</td>
                                <td width="100">
	...
	<form name="fabricOrderEntry_2" id="fabricOrderEntry_2">
		<fieldset style="width:840px; margin-top:10px">
			...
			<thead>
                ...
                <th width="100" align="right" class="must_entry_caption">Fabric Source</td>
