...
<script>
	...
	function openmypage_po()
	{
		...
		{ 	
			...
			emailwindow.onclose=function()
			{
				...
				var html=''; var html2=''; var num_row=$('#scanning_tbl tbody tr').length;
				var total_prog_booking_qty=0; var total_req_qty=0; var total_balance_qty=0;
				if(num_row>1)
				{
					num_row--;
					total_prog_booking_qty+=$('#total_prog_booking_qty').text().replace(",","")*1;
					total_req_qty+=$('#total_req_qty').text().replace(",","")*1;
					total_balance_qty+=$('#total_balance_qty').text().replace(",","")*1;
				...
				for(var k=0; k<data.length; k++)
				{
					...
					/* 
					0=>buyerId,	1=>poId,2=>programBookingId,3=>deterId,4=>colorId,5=>countId,6=>reqQty,7=>receiveBasisId,8=>receiveBasis,9=>program booking,10=>Buyer,11=>Job No,12=>Style Ref ,13=>Order No,14=>Construction,15=>Composition,16=>GSM,17=>Dia,18=>Color,19=>Prod. Id,20=>File No,21=>Ref. No ,22=>progBookNo,23=>totReqnQty,24=>balance
					*/
					total_prog_booking_qty+=row_data[6]*1;
					total_req_qty+=row_data[24]*1;
					total_balance_qty+=row_data[25]*1;

					var html=html+'<tr bgcolor="'+bgcolor+'" id="tr_'+num_row+'"><td width="30" align="center">'+num_row+'</td><td width="80" style="word-break:break-all;">'+row_data[13]+'</td><td width="100" style="word-break:break-all;">'+row_data[12]+'</td><td width="70" style="word-break:break-all;">'+row_data[22]+'</td><td width="60" style="word-break:break-all;">'+row_data[20]+'</td><td width="55" style="word-break:break-all;">'+row_data[10]+'</td><td width="80" id="job'+num_row+'">'+row_data[11]+'</td><td width="75" style="word-break:break-all;">'+row_data[15]+'</td><td width="100" style="word-break:break-all;">'+row_data[16]+'</td><td width="40" id="gsm'+num_row+'">'+row_data[17]+'</td><td width="40" id="dia'+num_row+'">'+row_data[18]+'</td><td width="70" style="word-break:break-all;">'+row_data[19]+'</td><td width="70" align="right">'+row_data[6]+'</td><td width="70" align="right">'+row_data[24]+'</td><td width="70" align="right" id="totBalQty'+num_row+'">'+row_data[25]+'</td><td width="80" align="center"><input type="text" value="" class="text_boxes_numeric" style="width:65px" id="reqsnQty'+num_row+'" name="reqsnQty[]" onkeyup="fnc_count_total_qty();fnc_validation_qty('+num_row+');""/></td><td width="90" align="center"><input type="text" value="" class="text_boxes" style="width:75px" id="remarks'+num_row+'" name="remarks[]"/></td><td width="65" id="programNo'+num_row+'">'+program_no+'</td><td width="90" id="bookingNo'+num_row+'">'+booking_no+'</td><td width="70" style="display:none"></td><td width="70" style="display:none" id="lot'+num_row+'"></td><td id="prodId'+num_row+'">'+row_data[20]+'<input type="hidden" value="'+row_data[0]+'" id="buyerId'+num_row+'" name="buyerId[]"/><input type="hidden" value="'+row_data[1]+'" id="poId'+num_row+'" name="poId[]"/><input type="hidden" value="'+row_data[3]+'" id="deterId'+num_row+'" name="deterId[]"/><input type="hidden" value="'+row_data[4]+'" id="colorId'+num_row+'" name="colorId[]"/><input type="hidden" value="'+row_data[5]+'" id="countId'+num_row+'" name="countId[]"/><input type="hidden" value="'+row_data[2]+'" id="programBookingId'+num_row+'" name="programBookingId[]"/><input type="hidden" value="'+row_data[7]+'" id="receiveBasis'+num_row+'" name="receiveBasis[]"/><input type="hidden" value="" id="dtlsId'+num_row+'" name="dtlsId[]"/></td></tr>';
					
				}
				
					var html2='<tr><td colspan="12" style="text-align:right; font-weight:bold;">'+'Total'+'</td></td><td id="total_prog_booking_qty" style="text-align:right; font-weight:bold;">'+number_format(total_prog_booking_qty,2,'.','')+'</td></td><td id="total_req_qty" style="text-align:right; font-weight:bold;">'+number_format(total_req_qty,2,'.','')+'</td></td><td id="total_balance_qty" style="text-align:right; font-weight:bold;">'+number_format(total_balance_qty,2,'.','')+'</td></td><td width="80" align="center"><input type="text" class="text_boxes_numeric" style="width:65px" id="total_blnc_qty_td_id" name="" readonly/></td></tr>';
	...
	function fnc_validation_qty(rowsl)
	{
	...
	{
		var ww = window.open("requires/fabric_requisition_for_batch_entry_controller.php?data=" + data+'&action='+action, true);
	}
		...
		<div align="center" style="width:100%;">
            <fieldset style="width:690px;">
                ...
                <table cellpadding="0" cellspacing="2" width="650">
                    ...
                    <tr>
                        <td align="right" colspan="3"><b>Order Number</b></td>
                        <td colspan="3">
			...
			<fieldset style="width:1430px;text-align:left">
				<table cellpadding="0" width="1410" cellspacing="0" border="1" id="scanning_tbl_top" class="rpt_table" rules="all">
                    <thead>
                        ...
                        <th width="70" style="display:none">Yarn Count</th>
                        <th width="70" style="display:none">Yarn Lot</th>
                        <th>Prod. Id</th>
                    </thead>
                 </table>
                ...
                <br>
                <table width="1325" cellpadding="0" cellspacing="0" border="1" id="" rules="all">
                    <tr>
