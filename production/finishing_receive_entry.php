...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("FINISHING RECEIVE ENTRY","../", 1, 1, $unicode,'','');
?>	
<script>
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			if (po_id!="")
			{
				...
				get_php_form_data(po_id+'**'+item_id+'**'+country_id+'**'+$('#hidden_preceding_process').val()+'**'+pack_type, "populate_data_from_search_popup", "requires/finishing_receive_entry_controller" );
				...
				if(variableSettings!=1){ 
					get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'**'+variableSettings, "color_and_size_level", "requires/finishing_receive_entry_controller" ); 
				...
				show_list_view(po_id+'**'+item_id+'**'+country_id+'**'+prod_reso_allo+'**'+pack_type+'**'+variableSettings,'show_dtls_listview','list_view_container','requires/finishing_receive_entry_controller','setFilterGrid(\'tbl_list_search\',-1)');
				show_list_view(po_id,'show_country_listview','list_view_country','requires/finishing_receive_entry_controller','');		
...
function fnc_wash_output_entry(operation)
{
	...
	if(operation==4)
	{
		 ...
		 print_report( $('#cbo_company_name').val()+'*'+$('#txt_mst_id').val()+'*'+report_title+'*'+$("#wash_production_variable").val(), "wash_output_print", "requires/finishing_receive_entry_controller" ) 
		 return;
	}
	else if(operation==0 || operation==1 || operation==2)
	{
		...
		else
		{
			...
			var data="action=save_update_delete&operation="+operation+"&colorIDvalue="+colorIDvalue+"&colorIDvalueRej="+colorIDvalueRej+get_submitted_data_string('garments_nature*cbo_company_name*cbo_country_name*wash_production_variable*wash_production_variable_rej*hidden_po_break_down_id*hidden_colorSizeID*cbo_buyer_name*txt_style_no*cbo_item_name*txt_order_qty*cbo_source*cbo_wash_company*cbo_location*cbo_floor*txt_wash_date*cbo_produced_by*cbo_wash_line*txt_reporting_hour*txt_super_visor*txt_wash_qty*txt_reject_qnty*txt_alter_qnty*txt_challan*txt_remark*txt_input_quantity*txt_cumul_wash_qty*txt_yet_to_wash*hidden_break_down_html*txt_mst_id*prod_reso_allo*txt_spot_qnty*save_data*defect_type_id*save_dataSpot*defectSpot_type_id*hidden_currency_id*hidden_exchange_rate*hidden_piece_rate*cbo_work_order*txt_pack_type',"../");
 			// alert (data); return;
 			http.open("POST","requires/finishing_receive_entry_controller.php",true);
 	...
 	if(http.readyState == 4) 
	{
		...
		else if(reponse[0]==0)
		{
			...
			show_msg(trim(reponse[0]));
 			show_list_view(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id+'**'+prod_reso_allo+'**'+pack_type+'**'+variableSettings,'show_dtls_listview','list_view_container','requires/finishing_receive_entry_controller','setFilterGrid(\'tbl_list_search\',-1)');
			...
			get_php_form_data(po_id+'**'+$('#cbo_item_name').val()+'**'+country_id+'**'+$('#hidden_preceding_process').val()+'**'+pack_type, "populate_data_from_search_popup", "requires/finishing_receive_entry_controller" );
 			if(variableSettings!=1) { 
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject+'**'+$('#hidden_preceding_process').val()+'**'+pack_type, "color_and_size_level", "requires/finishing_receive_entry_controller" ); 
			}
			else
			{
				$("#txt_wash_qty").removeAttr("readonly");
...
function put_country_data(po_id, item_id, country_id, po_qnty, plan_qnty, pack_type)
{
	...
	show_list_view(po_id+'**'+item_id+'**'+country_id+'**'+prod_reso_allo+'**_'+'**'+variableSettings,'show_dtls_listview','list_view_container','requires/finishing_receive_entry_controller','');
	set_button_status(0, permission, 'fnc_wash_output_entry',1,0);
	release_freezing();
}
...
function check_produced_by(val)
{
	...
	{
		var response=return_global_ajax_value( po_id+'**'+item_id, 'piece_rate_order_cheack', '', 'requires/finishing_receive_entry_controller');
...
function openmypage_defectQty(type)
{
	...
	else
	{
		...
		var page_link = 'requires/finishing_receive_entry_controller.php?hidden_po_break_down_id='+hidden_po_break_down_id+'&txt_mst_id='+txt_mst_id+'&save_data='+save_data+'&defect_qty='+defect_qty+'&all_defect_id='+all_defect_id+'&type='+type+'&action=defect_data';
...
function fnc_workorder_search(supplier_id)
{
	...
	load_drop_down( 'requires/finishing_receive_entry_controller', company+"_"+supplier_id+"_"+po_break_down_id, 'load_drop_down_workorder', 'workorder_td' ); 
	//alert($('#cbo_cutting_company option').size())
}

function fnc_workorder_rate(data,id)
{
	get_php_form_data(data+"_"+id, "populate_workorder_rate", "requires/finishing_receive_entry_controller" );
}
...
function fnc_company_check(val)
{
	if(val==1)
	{
		...
		{
			get_php_form_data(document.getElementById('cbo_wash_company').value,'production_process_control','requires/finishing_receive_entry_controller' );
		}
	}
	else
	{
		get_php_form_data(document.getElementById('cbo_company_name').value,'production_process_control','requires/finishing_receive_entry_controller' );
	}
 }
...
function fnc_loadvariable(val)
{
	...
	$("#txt_qty_source").val('');
	get_php_form_data( $("#cbo_company_name").val()+'**'+val,'load_variable_settings','requires/finishing_receive_entry_controller');
}
                    ...
                    <table width="100%" border="0">
                        <tr>
                        	...
                        	<td class="must_entry_caption">Source</td>
                             <td>
								 <?
                                 echo create_drop_down( "cbo_source", 150, $knitting_source,"", 1, "-- Select Source --", $selected, "fnc_company_check(this.value); load_drop_down( 'requires/finishing_receive_entry_controller', this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_wash_output', 'sew_company_td' ); dynamic_must_entry_caption(this.value);", 0, '1,3' );
                           ...
                           <input type="hidden" name="hidden_preceding_process" id="hidden_preceding_process">
                         	 <td class="must_entry_caption">Finishing Company</td>
                         ...
                         <tr>
                            ...
                            <td width="150">
								<input name="txt_order_no" placeholder="Double Click to Search" id="txt_order_no" onDblClick="openmypage('requires/finishing_receive_entry_controller.php?action=order_popup&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value+'&production_company='+document.getElementById('cbo_wash_company').value+'&hidden_variable_cntl='+document.getElementById('hidden_variable_cntl').value+'&hidden_preceding_process='+document.getElementById('hidden_preceding_process').value,'Order Search')"  class="text_boxes" style="width:140px " readonly>
