...
if ($action=="load_drop_down_floor")
{
	...
	//echo "select a.id, a.floor_name from lib_prod_floor a, lib_machine_name b where a.id=b.floor_id and b.category_id=4 and b.company_id='$ex_data[0]' and b.status_active=1 and b.is_deleted=0  $location_cond group by a.id, a.floor_name order by a.floor_name";
	...
	if ($order_type==0 || $order_type==1) // Self Order
	{
		...
		<? 
			...
			$sql_result="SELECT a.id, a.batch_no, a.batch_id, a.batch_ext_no, a.machine_id, a.load_unload_id, a.end_hours, a.end_minutes, a.result, a.remarks,a.production_date, 
			 ...
			 order by a.production_date, q.process_id, c.machine_no";
...
if($action=="report_generate2") // Show Button 2, Summary
{ 
	$process = array( &$_POST );
	extract(check_magic_quote_gpc( $process )); 
		...
		<div style="margin-left:6px;">
			<table width="600"   border="0" cellpadding="0" cellspacing="0" >
			 <tr>
               ...
               <td>
               <table width="300"  style=" margin:5px;" id="tdtag" style="" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all">
				<thead>
				  <tr>
				  ...
				  <tr>
				    ...
				    <td id="tdtag" style="background: #E9F3FF;"><strong><?php 
					$tot_actual_prod_qty=$tot_self_actual_prod_qty+$subcon_total_prod_qty+$sample_total_prod_qty;
	...
	if ($order_type==0 || $order_type==1) // Self Order
	{
				...
				foreach ($prod_date_data as $machine_id => $machine_data) 
				{
					...
					//echo $self_fin_prod_check."=";
						...
						<tr id="<? echo 'v_'.$i; ?>" onClick="change_color('v_<? echo $i; ?>','#FFFFFF')" bgcolor="<? echo $bgcolor; ?>">
							<?							
								if ($r==0) 
								{
									...
									<td rowspan="<? if (isset($idle_data_arr[$p_date][$machine_id])) 
									{ 
										echo $self_rowspan_arr[$p_date][$machine_id]+1;
									}
									else
									{
                            ...
                            <td width="80" valign="middle" title="Fabric + Trims " align="right"><? echo $row['batch_qty']+$row['total_trims_weight'];?></td>
						...
						//if (($row['result']==1 || $row['result']==2) && $row['batch_ext_no']>0 && $re_process_qty3>0 ) //Shade match/Redyeing Needed/Re Process /Fin Prod Available
							...
							{
								$self_incomplete_qty=$row['batch_qty'];
							}
							...
							?>
							<td width="60" valign="middle" align="right"><? echo $avarage_batch_qty; $other_mcw_wise_total+=$avarage_batch_qty;?></td>
							<td width="60" valign="middle" align="right"><? echo $white_batch_qty; $white_mcw_wise_total+=$white_batch_qty;?></td> 
							<td width="60" valign="middle" align="right"><? echo $wash_batch_qty; $wash_mcw_wise_total+=$wash_batch_qty;?></td> 
							<td width="60" valign="middle" align="right"><? echo $re_pro_qty; $total_pro_qty+=$re_pro_qty;?></td>
							<td width="60" valign="middle" align="right"><? echo $trims_weight; $total_trims_weight+=$trims_weight;?></td>
							<td width="70" valign="middle" align="right"><? echo $total_self_production=$row['batch_qty']+$row['total_trims_weight'];//($avarage_batch_qty+$white_batch_qty+$wash_batch_qty+$re_pro_qty+$trims_weight); 
							$total_self_production_qty+=$total_self_production; ?></td>
                            <td width="70" valign="middle" align="right"><? echo $actual_prod_qty; $total_actual_prod_qty+=$actual_prod_qty;?></td>
							<td width="70" valign="middle" align="right"><? echo $self_incomplete_qty;//if($row['incomplete_result']==4) echo $total_self_incomplete_qty=$row['batch_qty']; $total_self_incomplete_qty+=$self_incomplete_qty; ?></td>
					...
					<tr class="tbl_bottom">
						...
						<td><strong></strong></td>
						<td><strong><? echo $mcw_wise_total_batch; ?></strong></td>
						<td colspan="5" align="right"><b>Total:</b></td>
						<td><strong><? echo $other_mcw_wise_total;?></strong></td>
						<td><strong><? echo $white_mcw_wise_total;?><strong></td>
						<td><strong><? echo $wash_mcw_wise_total;?><strong></td>
						<td><strong><? echo $total_pro_qty;?><strong></td>
						<td><strong><? echo $total_trims_weight;?><strong></td>
                        <td><strong><? echo $total_self_production_qty;?><strong></td>
						<td><strong><? echo $total_actual_prod_qty;?><strong></td>
						<td><strong><? echo $$total_self_incomplete_qty;?><strong></td>
						<td colspan="11"></td>
					</tr>
					<?
					$i++;
					...
					<tr class="tbl_bottom">
						...
						<td><strong></strong></td>
						<td><strong><? echo $date_wise_total_batch;?></strong></td>
						<td colspan="5" align="right"><b>Date wise Total:</b></td>
						<td><strong><? echo $other_date_wise_total;?></strong></td>
						<td><strong><? echo $white_date_wise_total;?></strong></td>
						<td><strong><? echo $wash_date_wise_total;?></strong></td>
						<td><strong><? echo $pro_qty_date_wise_total;?></strong></td>
						<td><strong><? echo $trims_weight_date_wise_total;?></strong></td>
						<td><strong><? echo $date_wise_total_self_production;?></strong></td>
                        <td><strong><? echo $date_wise_total_self_actual_production;?></strong></td>
						<td><strong><? echo $date_wise_total_self_incomplete_qty;?></strong></td>
						<td colspan="11"></td>
					</tr>
					<?
					$other_grnd_total+=$other_date_wise_total;
	    ...
	    <tfoot>
	        <tr class="tbl_bottom">
				...
				<td width="100"></td>
				<td width="80"><strong><? echo $grnd_total_batch;?></strong>&nbsp;</td>
	        	<td width="100"></td>
	        	<td width="120"></td> 
                <td width="80"></td>
	        	<td width="50"></td> 
	        	<td width="100" align="right"><strong>Grand Total :</strong></td>
	            <td width="60"><strong><? echo $other_grnd_total;?></strong>&nbsp;</td>
	            <td width="60"><strong><? echo $white_grnd_total;?></strong></td>
	            <td width="60"><strong><? echo $wash_grnd_total;?></strong></td>
	            <td width="60"><strong><? echo $pro_qty_grnd_total;?></strong></td>
	            <td width="60"><strong><? echo $trims_weight_grnd_total;?></strong></td>
	            <td width="70"><strong><? echo $self_production_grnd_total;?></strong></td>
                 <td width="70"><strong><? echo $self_actual_production_grnd_total;?></strong></td>
	            <td width="70"><strong><? echo $incomplete_qty_grnd_total;?></strong></td>
	            <td colspan="11"></td>
	        </tr> 
	...
	if ($order_type==0 || $order_type==2) // Subcontract Order
	{
			...
			foreach ($subcon_date_data_arr as $p_date=>$prod_date_data) 
			{
				...
				foreach ($prod_date_data as $machine_id => $machine_data) 
				{
						...
						<tr id="<? echo 'vs_'.$i; ?>" onClick="change_color('vs_<? echo $i; ?>','#FFFFFF')" bgcolor="<? echo $bgcolor; ?>">
							<?
								if ($r==0) 
								{
									...
									<td rowspan="<? if (isset($idle_data_arr[$p_date][$machine_id])) 
									{ 
										echo $subcon_rowspan_arr[$p_date][$machine_id]+1;
									}
									else
									{
										echo $subcon_rowspan_arr[$p_date][$machine_id];
									}
									?>" width="30" valign="middle"><? echo $i;?></td>
									<td rowspan="<? if (isset($idle_data_arr[$p_date][$machine_id])) 
									{ 
                            ...
                            <td width="70" valign="middle" title="Fabric + Trims" align="right"><? echo $row['batch_qnty']+$row['total_trims_weight'];?></td> 
							...
							{
									$sub_actual_prod_qty =$row['batch_qnty'];
							}
							...
							?>
							<td width="60" valign="middle" align="right"><? echo $subcon_other_pro_qty; $subcon_other_mcw_wise_total+=$subcon_other_pro_qty;?></td>
							<td width="60" valign="middle" align="right"><? echo $subcon_white_pro_qty; $subcon_white_mcw_wise_total+=$subcon_white_pro_qty;?></td>
							<td width="60" valign="middle" align="right"><? echo $subcon_wash_pro_qty; $subcon_wash_mcw_wise_total+=$subcon_wash_pro_qty;?></td>
							<td width="60" valign="middle" align="right"><? echo $pro_qty; $subcon_pro_qty_total+=$pro_qty;?></td>
							<td width="60" valign="middle" align="right"><? echo $trims_weight; $subcon_trims_weight_total+=$trims_weight;?></td>
							<td width="70" valign="middle" align="right"><? echo $total_production=$row['batch_qnty']+$row['total_trims_weight'];//$subcon_other_pro_qty+$subcon_white_pro_qty+$subcon_wash_pro_qty+$pro_qty+$trims_weight+$incomplete_qty+$sub_other_qty; 
								$total_subc_production_qty+=$total_production;?></td>
                             <td width="70" valign="middle" align="right" title="BatchID=<? echo $batch_id;?>"><? echo $sub_actual_prod_qty; $subcon_actual_prod_qty_total+=$sub_actual_prod_qty;?></td>
							<td width="70" valign="middle"  align="right"><? echo $incomplete_qty;//if($row['incomplete_result']==4) echo $incomplete_qty=$row['batch_qnty']; $total_subc_incomplete_qty+=$incomplete_qty; ?></td>
                            <? if($row['result']==1) { $resultBgcolor = "green";} else if($row['result']==2) {$resultBgcolor = "yellow";} else{$resultBgcolor = "#FFFFFF";} ?>
                            <td width="110" valign="middle" bgcolor="<? echo $resultBgcolor; ?>"><div style="word-wrap:break-word; width:110px;"><? echo $dyeing_result[$row['result']];?></div></td>
					...
					<tr class="tbl_bottom">
						...
						<td width="100"></td>
						<td><strong><? echo $subcon_mc_wise_total_batch;?></strong></td>
						<td colspan="5" align="right"><b>Total:</b></td>
						<td><strong><? echo $subcon_other_mcw_wise_total;?></strong></td>
						<td><strong><? echo $subcon_white_mcw_wise_total;?><strong></td>
						<td><strong><? echo $subcon_wash_mcw_wise_total;?><strong></td>
						<td><strong><? echo $subcon_pro_qty_total;?><strong></td>
						<td><strong><? echo $subcon_trims_weight_total;?><strong></td>
						<td><strong><? echo $total_subc_production_qty;?><strong></td>
                        <td><strong><?  echo $subcon_actual_prod_qty_total;?><strong></td>
						<td><strong><? echo $total_subc_incomplete_qty;?><strong></td>
						<td colspan="11"></td>
					</tr>
					<?
					$i++;
					...
					<tr class="tbl_bottom">
						...
						<td width="100"></td>
						<td><strong><? echo $subcon_date_wise_total_batch;?></strong></td>
						<td colspan="5" align="right"><b>Date wise Total:</b></td>
						<td><strong><? echo $subcon_other_date_wise_total;?></strong></td>
						<td><strong><? echo $subcon_white_date_wise_total;?></strong></td>
						<td><strong><? echo $subcon_wash_date_wise_total;?></strong></td>
						<td><strong><? echo $subcon_pro_qty_date_wise_total;?></strong></td>
						<td><strong><? echo $subcon_trims_weight_date_wise_total;?></strong></td>
						<td><strong><? echo $date_wise_total_subc_production;?></strong></td>
                        <td><strong><? echo $date_wise_total_subc_actual_production;?></strong></td>
						<td><strong><? echo $date_wise_total_subc_incomplete_qty;?></strong></td>
						<td colspan="11"></td>
					</tr>
					<?
					$subcon_other_grnd_total+=$subcon_other_date_wise_total;
	    ...
	    <tfoot>
	        <tr class="tbl_bottom">
	        	...
	        	<td width="100"></td>
	        	<td width="100"><strong><? echo $subcon_grnd_total_batch;?></strong>&nbsp;</td>
	        	<td width="100"></td>
	        	<td width="100"></td>
                <td width="70"></td>
	        	<td width="50"></td>
	        	<td width="100" align="right"><strong>Grand Total :</strong></td>
	            <td width="60"><strong><? echo $subcon_other_grnd_total;?></strong>&nbsp;</td>
	            <td width="60"><strong><? echo $subcon_white_grnd_total;?></strong></td>
	            <td width="60"><strong><? echo $subcon_wash_grnd_total;?></strong></td>
	            <td width="60"><strong><? echo $subcon_pro_qty_grnd_total;?></strong></td>
	            <td width="60"><strong><? echo $subcon_trims_weight_grnd_total;?></strong></td>
	            <td width="70"><strong><? echo $subc_production_grnd_total;?></strong></td>
                <td width="70"><strong><? echo $subcon_grnd_total_actual_batch;?></strong></td>
	            <td width="70"><strong><? echo $subc_incomplete_qty_grnd_total;?></strong></td>
	...
	if ($order_type==0 || $order_type==3) // Sample Without Order
	{
			...
			foreach ($date_data_arr as $p_date=>$prod_date_data) 
			{
				...
				foreach ($prod_date_data as $machine_id => $machine_data) 
				{
					...
					foreach ($machine_data as $batch_id => $row) 
					{
						...
						<tr id="<? echo 'vSamp_'.$i; ?>" onClick="change_color('vSamp_<? echo $i; ?>','#FFFFFF')" bgcolor="<? echo $bgcolor; ?>">
                           ...
                           <td  width="80" rowspan="<? //echo $samp_rowspan_arr[$p_date][$machine_id][$row['system_no']];?>" valign="middle" style="word-wrap:break-word; word-break: break-all;"><? echo $sample_functional_batch=$row['system_no']; 
							...
							<td width="120" valign="middle"><div style="word-wrap:break-word; width:120px;"><? echo $row['booking_no'];?></div></td>
                           <td width="70" valign="middle" align="right" title="Fabric + Trims" ><? echo $row['batch_qty']+$row['total_trims_weight'];?></td>
							...
							if($row['incomplete_result']==4) 
							{
								$samp_incomplete_qty=$row['batch_qty'];
							}
							else $samp_incomplete_qty=0;
							$total_samp_incomplete_qty+=$incomplete_qty;
							if($samp_actual_prod_qty>0) $trims_weight=$row['total_trims_weight'];else $trims_weight="";
							 
							?>
							<td width="60" valign="middle" align="right"><? echo $other_pro_qty; $other_mcw_wise_total+=$other_pro_qty;?></td>
							<td width="60" valign="middle" align="right"><? echo $white_pro_qty; $white_mcw_wise_total+=$white_pro_qty;?></td>
							<td width="60" valign="middle" align="right"><? echo $wo_wash_pro_qty; $wo_wash_mcw_wise_total+=$wo_wash_pro_qty;?></td>
							<td width="60" valign="middle" align="right"><? echo $samp_pro_qty; $wo_pro_qty_total+=$samp_pro_qty;?></td>
							<td width="60" valign="middle"><? echo $trims_weight; $wo_trims_weight_total+=$trims_weight;?></td>
							<td width="70" valign="middle" align="right"><? echo $total_production=$row['batch_qty']+$row['total_trims_weight'];//($other_pro_qty+$white_pro_qty+$wo_wash_pro_qty+$samp_pro_qty+$trims_weight+$samp_incomplete_qty); 
							$total_samp_production+=$total_production; ?></td>
                            <td width="70" valign="middle" align="right"><? echo $samp_actual_prod_qty; $samp_actual_pro_qty_total+=$samp_actual_prod_qty;?></td>
							<td width="70" valign="middle"><? echo $incomplete_qty;//if($row['result']==4) echo $incomplete_qty=$row['batch_qty'];
					...
					<tr class="tbl_bottom">
						...
						<td></td>
						<td><strong><? echo $sm_mcw_wise_total_batch;?></strong></td>
						<td colspan="5" align="right"><b>Total:</b></td>
						<td><strong><? echo $other_mcw_wise_total;?></strong></td>
						<td><strong><? echo $white_mcw_wise_total;?><strong></td>
						<td><strong><? echo $wo_wash_mcw_wise_total;?><strong></td>
						<td><strong><? echo $wo_pro_qty_total;?><strong></td>
						<td><strong><? echo $wo_trims_weight_total;?><strong></td>
						<td><strong><? echo $total_samp_production;?><strong></td>
                        <td><strong><? echo $samp_actual_pro_qty_total;?><strong></td>
						<td><strong><? echo $total_samp_incomplete_qty;?><strong></td>
						<td colspan="11"></td>
					...
					<tr class="tbl_bottom">
						...
						<td></td>
						<td><strong><? echo $sm_date_wise_total_batch;?></strong></td>
						<td colspan="5" align="right"><b>Date wise Total:</b></td>
						<td><strong><? echo $other_date_wise_total;?></strong></td>
						<td><strong><? echo $white_date_wise_total;?></strong></td>
						<td><strong><? echo $wash_date_wise_total;?></strong></td>
						<td><strong><? echo $wo_pro_qty_date_wise_total;?></strong></td>
						<td><strong><? echo $wo_trims_weight_date_wise_total;?></strong></td>
						<td><strong><? echo $date_wise_total_samp_production;?></strong></td>
                        <td><strong><?  echo $wo_pro_qty_actual_date_wise_total;?></strong></td>
						<td><strong><? echo $date_wise_total_samp_incomplete_qty;?></strong></td>
						<td colspan="11"></td>
					</tr>
					<?
					$other_grnd_total+=$other_date_wise_total;
	    ...
	    <tfoot>
	        <tr class="tbl_bottom">
	        	...
	        	<td width="100"></td>
	        	<td width="100"><strong><? echo $sm_grnd_total_batch;?></strong>&nbsp;</td> 
	        	<td width="120"></td> 
	        	<td width="120"></td>
                <td width="70"></td> 
	        	<td width="50"></td>
	            ...
	            <td width="60"><strong><? echo $white_grnd_total;?></strong></td>
	            <td width="60"><strong><? echo $wash_grnd_total;?></strong></td>
	            <td width="60"><strong><? echo $wo_pro_qty_grnd_total;?></strong></td>
	            <td width="60"><strong><? echo $wo_trims_weight_grnd_total;?></strong></td>
	            <td width="70"><strong><? echo $grnd_total_samp_production;?></strong></td>
                <td width="70"><strong><? echo $actual_wo_pro_qty_grnd_total;?></strong></td>
	            <td width="70"><strong><? echo $grnd_total_samp_incomplete_qty;?></strong></td>
	            <td colspan="11"></td>
	        </tr> 
	        <tr class="tbl_bottom">
	        	<td colspan="2" align="right"><b>Avg Batch Per M/C:</b></td>  
...
if($action=="report_generate3") // Show Finish 2, Summary
{ 
	$user_id = $_SESSION['logic_erp']["user_id"];
	//echo $user_id; die;
	$process = array( &$_POST );
	...
	$batch_color=str_replace("'","",$hidden_color_id);
	$batch_color_txt=str_replace("'","",$txt_batch_color);
	$txt_color_range=str_replace("'","",$txt_color_range);
	$floor_name=str_replace("'","",$cbo_floor_id);
	$date_from=str_replace("'","",$txt_date_from);
	$date_to=str_replace("'","",$txt_date_to);
	...
	if ($batch=="") $batch_cond =""; else $batch_cond =" and a.batch_no='$batch' ";
	if ($cbo_process==0 || $cbo_process=='') $cbo_process_cond =" "; else $cbo_process_cond =" and a.process_id in '$cbo_process' ";
	//if ($cbo_process==0 || $cbo_process=='') $cbo_process_cond =" and a.process_id in (33,63,65,68,90,93,94,100,127,156,155,179,200)"; else $cbo_process_cond =" and a.process_id in($cbo_process) ";
	if ($order_type==0) $order_type_cond =""; else $order_type_cond =" and a.order_type=$order_type ";
	if ($cbo_location==0 || $cbo_location=='') $location_id =""; else $location_id =" and c.location_id=$cbo_location ";
	if ($cbo_company==0 || $cbo_company=='') $cbo_company_cond =""; else $cbo_company_cond =" and a.service_company=$cbo_company ";
	if ($floor_name==0 || $floor_name=='') $floor_id_cond=""; else $floor_id_cond=" and a.floor_id=$cbo_floor_id";
	if ($machine_name=='' || $machine_name==0) $machine_cond=" "; else $machine_cond =" and a.machine_id in ($machine_name) ";
	if ($buyer_name==0  || $buyer_name=='') $buyer_name_cond =""; else $buyer_name_cond =" and p.buyer_id=$buyer_name ";
	if ($booking_no=="") $booking_no_cond =""; else $booking_no_cond =" and c.booking_no=$txt_booking_no ";
	if ($batch_color=="") $batch_color_cond =""; else $batch_color_cond =" and c.color_id=$batch_color ";
	if ($batch_color_txt=="") $batch_color_txt_cond =""; else $batch_color_txt_cond =" and l.color_name like '%$batch_color_txt%' ";
	if ($txt_color_range==0 || $txt_color_range=="") $color_range_cond =""; else $color_range_cond =" and q.color_range_id=$txt_color_range ";
	...
	if($db_type==0)
	{
		if( $date_from!="" && $date_to!="" )
		{
			$prod_date_cond .= " and a.process_end_date between '".change_date_format($date_from,'yyyy-mm-dd')."' and '".change_date_format($date_to,'yyyy-mm-dd')."'";
		}
	}
	else if($db_type==2)
	{
		if($date_from!="" && $date_to!="") 
		{
			$prod_date_cond .= " and a.process_end_date between '".change_date_format($date_from,'','',1)."' and '".change_date_format($date_to,'','',1)."'";
		}
	}
	...
	$company_short_library=return_library_array( "select id,company_short_name from lib_company", "id", "company_short_name"  );
	$floor_library=return_library_array( "select id,floor_name from  lib_prod_floor", "id", "floor_name"  );
	$batch_weight=return_library_array( "select a.id,  a.batch_weight from pro_batch_create_mst a", "id", "batch_weight"  );
	
		$no_machone_sqls="select b.mst_id, a.batch_id,  a.batch_no,  a.batch_ext_no,  a.process_id,  a.process_end_date,  a.machine_id,  a.floor_id,  a.production_date,  a.service_company,
					  sum(b.batch_qty) as batch_qty, sum(b.production_qty) as production_qty,  b.const_composition,  b.gsm,  b.dia_width,  a.water_flow_meter,  a.temparature,  a.end_hours,  a.end_minutes,  a.result,  a.shift_name,
					  a.process_start_date,  a.start_minutes,  a.start_hours,  a.fabric_type,  a.incomplete_result,  c.batch_date,  c.booking_no,  c.booking_without_order,
					  c.batch_against,  c.color_id,  c.batch_weight,  a.entry_form,  a.load_unload_id,  a.remarks,  l.color_name,  m.buyer_id
					  from pro_fab_subprocess a left join pro_fab_subprocess_dtls b on a.id = b.mst_id left join pro_batch_create_mst c
					  on a.batch_id = c.id inner join lib_color l on c.color_id = l.id left join  wo_booking_mst m on c.booking_no = m.booking_no
					  where a.status_active = 1 and b.status_active = 1  and a.process_id not in(31) and (a.machine_id is null or a.machine_id=0)
					  $batch_color_txt_cond $booking_no_cond $machine_cond $batch_cond $cbo_process_cond $cbo_company_cond $floor_id_cond $prod_date_cond 
					  group by b.mst_id, a.batch_id,  a.batch_no,  a.batch_ext_no,  a.process_id,  a.process_id,  a.process_end_date,  a.machine_id,  a.floor_id,  a.production_date,  a.service_company,
					  b.const_composition,  b.gsm,  b.dia_width,  a.water_flow_meter,  a.temparature,  a.end_hours,  a.end_minutes,  a.result,  a.shift_name,  a.process_start_date,
					  a.start_minutes,  a.start_hours,  a.fabric_type,  a.incomplete_result,  c.batch_date,  c.booking_no,  c.booking_without_order,  c.batch_against,  c.color_id,  c.batch_weight,  a.entry_form,  a.load_unload_id,
					  a.remarks,  l.color_name,  m.buyer_id	order by a.production_date, a.process_id, a.batch_no,  a.machine_id,c.booking_no";
				  
		//echo $no_machone_sqls; //die;
		
		//echo $self_sqls; //die;
		$count_no_machone_sqls=count($no_machone_sqls);
		$no_machone_sqls_result=sql_select($no_machone_sqls);
		
		//echo $count_no_machone_sqls;
		$no_machone_array=array();
		foreach( $no_machone_sqls_result as $rrow )
		{
			$no_machone_array[$rrow[csf('process_id')]][$rrow[csf('batch_id')]]['process_id'] = $rrow[csf('process_id')];
			$no_machone_array[$rrow[csf('process_id')]][$rrow[csf('batch_id')]]['production_qty'] += $rrow[csf('production_qty')];
			$no_machone_array[$rrow[csf('process_id')]][$rrow[csf('batch_id')]]['batch_no'] = $rrow[csf('batch_no')];
			//$no_machone_array[$rrow[csf('process_id')]][$rrow[csf('batch_id')]]['batch_no'] = $rrow[csf('batch_no')];				
		}			

		$self_sqls="select b.mst_id, a.batch_id,  a.batch_no,  a.batch_ext_no,  a.process_id,  a.process_end_date,  a.machine_id,  a.floor_id,  a.production_date,  a.service_company,
					  sum(b.batch_qty) as batch_qty, sum(b.production_qty) as production_qty,  b.const_composition,  b.gsm,  b.dia_width,  a.water_flow_meter,  a.temparature,  a.end_hours,  a.end_minutes,  a.result,  a.shift_name,
					  a.process_start_date,  a.start_minutes,  a.start_hours,  a.fabric_type,  a.incomplete_result,  c.batch_date,  c.booking_no,  c.booking_without_order,
					  c.batch_against,  c.color_id,  c.batch_weight,  a.entry_form,  a.load_unload_id,  a.remarks,  l.color_name,  m.buyer_id
					  from pro_fab_subprocess a left join pro_fab_subprocess_dtls b on a.id = b.mst_id left join pro_batch_create_mst c
					  on a.batch_id = c.id inner join lib_color l on c.color_id = l.id left join  wo_booking_mst m on c.booking_no = m.booking_no
					  where a.status_active = 1 and b.status_active = 1  and a.process_id not in '31' and (a.machine_id is not null OR a.machine_id !=0)
					  $batch_color_txt_cond $booking_no_cond $machine_cond $batch_cond $cbo_process_cond $cbo_company_cond $floor_id_cond $prod_date_cond 
					  group by b.mst_id, a.batch_id,  a.batch_no,  a.batch_ext_no,  a.process_id,  a.process_id,  a.process_end_date,  a.machine_id,  a.floor_id,  a.production_date,  a.service_company,
					  b.const_composition,  b.gsm,  b.dia_width,  a.water_flow_meter,  a.temparature,  a.end_hours,  a.end_minutes,  a.result,  a.shift_name,  a.process_start_date,
					  a.start_minutes,  a.start_hours,  a.fabric_type,  a.incomplete_result,  c.batch_date,  c.booking_no,  c.booking_without_order,  c.batch_against,  c.color_id,  c.batch_weight,  a.entry_form,  a.load_unload_id,
		...
		//echo $self_sqls; //die;
		$count_data=count($self_sqls);
		$sql_result=sql_select($self_sqls);
		unset($pro_array);
		//echo $count_data;
		$pro_array=array();
		foreach( $sql_result as $rows )
		{ 
			//if($rows[csf('process_id')]==68) echo $rows[csf('process_id')].', ';
			if($rows[csf('machine_id')]==0 || $rows[csf('machine_id')]==''){$machine_id = 5000;} else {$machine_id=$rows[csf('machine_id')];}
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['machine_id'] = $machine_id;
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['batch_id'] = $rows[csf('batch_id')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['process_id'] = $rows[csf('process_id')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['process_start_date'] = $rows[csf('process_start_date')];			
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['batch_qty']+=$rows[csf('batch_qty')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['batch_no'] = $rows[csf('batch_no')];
			
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['result'] = $rows[csf('result')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['booking_no'] = $rows[csf('booking_no')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['production_date'] = $rows[csf('production_date')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['gsm'] = $rows[csf('gsm')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['dia_width'] = $rows[csf('dia_width')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['const_composition'] = $rows[csf('const_composition')];
			
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['start_minutes'] = $rows[csf('start_minutes')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['start_hours'] = $rows[csf('start_hours')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['process_end_date'] = $rows[csf('process_end_date')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['end_minutes'] = $rows[csf('end_minutes')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['end_hours'] = $rows[csf('end_hours')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['production_qty']+=$rows[csf('production_qty')];			
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['color_id'] = $rows[csf('color_id')];
			
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['floor_id'] = $rows[csf('floor_id')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['batch_ext_no'] = $rows[csf('batch_ext_no')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['shift_name'] = $rows[csf('shift_name')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['temparature'] = $rows[csf('temparature')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['water_flow_meter'] = $rows[csf('water_flow_meter')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['fabric_type'] = $rows[csf('fabric_type')];
			$pro_array[$rows[csf('process_id')]][$machine_id][$rows[csf('batch_id')]][$rows[csf('process_end_date')]]['remarks'] = $rows[csf('remarks')];
			
			$process_array_sum[$rows[csf('process_id')]]['process_id'] = $rows[csf('process_id')];
			$process_array_sum[$rows[csf('process_id')]]['production_qty'] += $rows[csf('production_qty')];		

			 
		}
		
		//echo "<pre>";print_r($pro_array); //die;
		ob_start();
	?>
	
	<br>
	<div align="center"><p align = "center"> <font size="16"><strong> Daily Dyeing Finishing Report </strong></p></div><br><br>
	<div align="center">
		<table width="1610px" > 
			<tr>
				<td>
					<table width="410px" align="left" class="rpt_table"> 
						<thead>
							<tr>
								<th width="30">Sl</th>
								<th width="80">Process id</th>
						...
						<? 
							$s=1;
							foreach ($process_array_sum as $pid => $pval)             
							{	
								...
								?>
									<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $s; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $s;?>">	
										<td align="center" width="30"><? echo $s; ?></td> 
										<td align="center" width="40"><? echo $pval['process_id']; ?></td>
										<td style="word-wrap: break-word;word-break: break-all;" width="80"><? echo $conversion_cost_head_array[$pval['process_id']]; ?></td>
										<td align="right" width="80"><? echo  $pval['production_qty']; ?></td>
									</tr>
								<?
								
								$prod_qty_tot += $pval['production_qty'];
								$s++;
							}	
						?>
							<tfoot>
								<tr bgcolor="#dddddd">  
										...
										<td  align="right"><strong><? echo $prod_qty_tot; ?></strong></td>  							
								</tr>
							</tfoot>
					</table>
				</td>
			
				<td width="100px"> No Machine Production : <br>
					<table width="410px" align="left" class="rpt_table"> 
						<thead>
							<tr>
								<th width="30">Sl</th>
						...
						<? 
							$s=1;
							foreach ($no_machone_array as $proid => $proval)             
							{	
								foreach ($proval as $bid => $pvalues)             
								{	
									if ($s%2==0) $bgcolor="#e9f3ff";else $bgcolor="#ffffff";
									?>
										<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $s; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $s;?>">	
											<td align="center" width="30"><? echo $s; ?></td> 
											<td align="center" width="40"><? echo $conversion_cost_head_array[$pvalues['process_id']]; ?></td>
											<td style="word-wrap: break-word;word-break: break-all;" width="80"><? echo $pvalues['batch_no']; ?></td>
											<td align="right" width="80"><? echo  $pvalues['production_qty']; ?></td>
										</tr>
									<?
									
									$prod_qty_total += $pvalues['production_qty'];
									$s++;
								}
							}	
						?>
							<tfoot>
								<tr bgcolor="#dddddd">  
										...
										<td ></td>
										<td align="right"><strong>Grand total :</strong></td>
										<td  align="right"><strong><? echo $prod_qty_total; ?></strong></td>  							
								</tr>
							</tfoot>
					</table>
				</td>
				...
				<td valign="top" align="right">
						<table align="right" width="400px" class="rpt_table">
							<tr>
									<th align="right"><strong>Production date : </strong></th>	
									<th align="left"><strong><? echo "   ".$date_from."   to   ".$date_to;?></strong></th>				
							</tr>
							<tr>
									<th  align="right"><strong>Reporting date : </strong></th>	
									<th  align="left"><strong><? echo date("d/m/y") ;?></strong></th>				
							</tr>
						</table>
				</td>
			</tr>
	...
	<div align="center">
		<table width="1610px" border="1" cellpadding="0" cellspacing="0" class="rpt_table" id="table_header_1" rules="all"> 
			<thead>
				<tr>
	                <th width="30">Sl</th>
					...
					<th width="80">Proces start</th>
					<th width="50">Start time</th>
                    <th width="80">Proces end</th>
	    ...
	    <div align="center" style="width:1610px; overflow-y:scroll; max-height:500px;" id="scroll_body">
			<table align="center" cellspacing="0" width="1610px"  border="1" rules="all" class="rpt_table"  id="table_body">
				<? $sl=1;$time=0;
				$total = array();
					foreach ($pro_array as $pdid => $pval)             
					{						
						foreach ($pval as $mid => $mval)             
						{
							foreach ($mval as $batch_id => $batch_val)             
							{								
								foreach ($batch_val as $bdate => $fval)             
								{
									

									if ($sl%2==0) $bgcolor="#e9f3ff"; else $bgcolor="#ffffff";
									if($fval[('end_hours')]!=''|| $fval[('end_minutes')]!='')
									{	 
										$strstart = $fval[('process_start_date')]." ".$fval[('start_hours')].":".$fval[('start_minutes')].":"."00";
										$strend   = $fval[('production_date')]." ".$fval[('end_hours')].":".$fval[('end_minutes')].":"."00";										
									...
									<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $sl; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $sl;?>">	
										...
										<td width="80"><? echo $fval['process_end_date']; ?></td>
										<td style="word-wrap: break-word;word-break: break-all;" width="120"><? echo $fval['booking_no']; ?></td>
										<td width="60"><? echo $fval['batch_no']; ?></td>
										<td style="word-wrap: break-word;word-break: break-all;" width="80"><? echo $conversion_cost_head_array[$fval['process_id']]; ?></td>
										<td style="word-wrap: break-word;word-break: break-all;" width="120"><? echo $color_library[$fval['color_id']]; ?></td>							
										<td width="50" align="center"><? echo $fval['batch_ext_no']; ?></td>
										<td width="80">
										<?
											
													echo $machine_arr[$fval['machine_id']];
																						
										?>
										
										</td>
										<td width="80"><? echo $fval['process_start_date']; ?></td>
										<td width="50"><? if($fval['start_hours']<12) $t="am"; else $t="pm";
														echo $fval['start_hours'].":".$fval['start_minutes']." ".$t; ?></td>
										<td width="80"><? echo $fval['production_date']; ?></td>
										<td width="50"><? if($fval['end_hours']<12) $r="am"; else $r="pm";
														echo $fval['end_hours'].":".$fval['end_minutes']." ".$r; ?></td>
										<td align="center" width="50" bgcolor="<? if($dtestart>$dteend || $dtestart==$dteend) echo "#ecccdd";else echo "#ffffff";	?>">
										<?
											if($fval[('end_hours')]!=''|| $fval[('end_minutes')]!='')
											{
												$strstart = $fval['process_start_date']." ".$fval['start_hours'].":".$fval['start_minutes'].":"."00";
												...
												{		
													...
													print $dtediff->format("%h:%i")." h";
													$tt = $dtediff->format("%H:%I:%S");
													$total[]= "'".$tt."'";
													$config[$pdid]['total'] = implode(', ', $total);
													$grand['GT'] = implode(', ', $total);
													//$total = ['00:02:55','00:07:56','01:03:32','01:13:34','02:13:44','03:08:53','03:13:54'];
												}
												else echo " WR ";
											}
											else echo " WR ";
										...
										<td align="center" width="50"><? echo $fval['gsm']; ?></td>
										<td align="center" width="80"><? echo $fval['dia_width']; ?></td>
										<td style="word-wrap: break-word;word-break: break-all;" width="200"><? echo $fval['const_composition']; ?></td>
										<td align="right" width="80"><? echo $batch_weight[$batch_id]; //$fval['batch_qty']; ?></td>
										<td align="right" width="80"><? echo $fval['production_qty']; ?></td>
										<td width="80"><? echo $dyeing_result[$fval['result']]; ?></td>
										<td style="word-break: break-all;word-wrap: break-word;"><? echo $fval['remarks']; ?></td>
									</tr>
									<?
									
									$sl++;
									$time++;
									$process_total_batch_qty[$pdid] += $batch_weight[$batch_id];
									$process_prod_total_qty[$pdid] += $fval['production_qty'];
									$batch_qty += $batch_weight[$batch_id];
									$total_qty += $fval['batch_qty'];
									$prod_qty += $fval['production_qty'];
								}
							}
						}
					...
					?>
						<tr bgcolor="#dddddd">  
							...
							<td  align="right"><strong><? echo $process_total_batch_qty[$pdid];?><? //echo $total_qty; ?></strong></td>    
							<td  align="right"><strong><? echo $process_prod_total_qty[$pdid];?><? //echo $total_qty; ?></strong></td>    									
				...
				<tfoot>
	                <tr bgcolor="#dddddd">  
							<td colspan = "15"></td>
						    <td align="right"><strong>Grand total :</strong></td>
							<td  align="right"><strong><? echo $batch_qty; ?></strong></td>  
							<td  align="right"><strong><? echo $prod_qty; ?></strong></td>  							
							<td><strong>Grand Total Time :</strong></td>
							<td><strong><? 
											

													$sum_g = strtotime('00:00:00');
													$sum2_g=0;
													foreach ($grand as $v){

														$sum1_g=strtotime($v)-$sum_g;

														$sum2_g = $sum2_g+$sum1_g;
													}

													$sum3_g=$sum_g+$sum2_g;

													echo date("H:i:s",$sum3_g);
...
if ($action == "fabricBooking_popup") 
{
	...
	<script>
		...
		function company_validation()
		{
			...
			{
				 show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('cbo_lc_company_id').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('cbo_year').value, 'create_booking_search_list_view', 'search_div', 'machine_wise_daily_dyeing_finishing_prod_report_controller', 'setFilterGrid(\'tbl_list_search\',-1);'); 
			}
		}
