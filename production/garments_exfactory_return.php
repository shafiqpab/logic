...
<script>
	...
	function fnc_exFactory_entry(operation)
	{
		...
		{
			 var report_title=$( "div.form_caption" ).html();
			 print_report( $('#cbo_company_name').val()+'*'+$('#txt_return_id').val()+'*'+$('#txt_ex_factory_date').val()+'*'+report_title, "garments_exfactory_print2", "requires/garments_exfactory_return_controller" );
			 release_freezing();
            ...
            <legend>Production Module</legend>
                ...
                <table width="100%" border="0">
                    ...
                    <tr>
                        ...
                        <td width="170">
                            <?
                            echo create_drop_down( "cbo_company_name", 172, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", '', "load_drop_down( 'requires/garments_exfactory_return_controller', this.value, 'load_drop_down_location', 'location_td' );get_php_form_data(this.value,'load_variable_settings','requires/garments_exfactory_return_controller');get_php_form_data(this.value,'production_process_control','requires/garments_exfactory_return_controller');",0 ); ?>
                        	...
                        	<input type="hidden" id="txt_source" />
					...
					<tr>
					<td align="right">Delivery Company</td>
						 <td>
						 <?
                            echo create_drop_down( "cbo_delivery_company_id", 172, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", '', "",1 ); ?>
                        </td>
						
					<td align="right">Delivery Location</td>
						 <td>
						 <? echo create_drop_down( "cbo_delivery_location_id", 172,"select id,location_name from lib_location comp where status_active =1 and is_deleted=0","id,location_name", 1, "-- Select Location --", $selected, "",1 );?>
                        </td>
						
					<td align="right">Remarks</td>
						 <td>
                        	<input type="text" name="remarks" id="remarks" class="text_boxes" style="width:160px;" maxlength="100" placeholder="Write your comments"  />
                        </td>
					</tr>
                </table>
                </fieldset>
                ...
                <table cellpadding="0" cellspacing="1" width="930">
                    <tr>
                        <td align="center" colspan="10" valign="middle" class="button_container">
                            ...
                            ?>
							<input id="Print2" class="formbutton" type="button" value="Print2" name="Print2" onclick="fnc_exFactory_entry(5)" style="width:80px;">
