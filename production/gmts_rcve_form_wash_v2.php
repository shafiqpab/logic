...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Garments Receive From Wash v2 Info", "../", 1, 1, $unicode, '', '');
...
<script>
	...
	{
		if (form_validation('cbo_company_name*cbo_buyer_name*cbo_source', 'Company Name*Buyer*source') == false) 
		{
			return;
		}
		...
		emailwindow.onclose=function()
		{
			...
			get_php_form_data(company_id,'load_variable_settings','requires/gmts_rcve_from_wash_v2_controller');
			get_php_form_data(company_id,'load_variable_settings_reject','requires/gmts_rcve_from_wash_v2_controller');
			// alert(item_id);
			if (po_id!="")
			{
				freeze_window(5);
				...
				get_php_form_data(po_id+'**'+item_id+'**'+$('#cbo_embel_name').val()+'**'+country_id+'**'+source, "populate_data_from_search_popup", "requires/gmts_rcve_from_wash_v2_controller" );

				load_drop_down( 'requires/gmts_rcve_from_wash_v2_controller', $('#cbo_embel_name').val()+'**'+$('#hidden_po_break_down_id').val(), 'load_drop_down_embro_issue_type', 'embro_type_td');

				get_php_form_data($('#hidden_po_break_down_id').val()+'**'+$('#cbo_item_name').val()+'**'+$('#sewing_production_variable').val()+'**'+$('#styleOrOrderWisw').val()+'**'+$('#cbo_embel_name').val()+'**'+country_id+'**'+$('#embro_production_variable').val()+'**'+source, 'color_and_size_level', 'requires/gmts_rcve_from_wash_v2_controller' );
				...
				/* show_list_view(po_id+'**'+item_id+'**'+$('#cbo_embel_name').val()+'**'+country_id+'**1','show_dtls_listview','printing_production_list_view','requires/gmts_rcve_from_wash_v2_controller','');
				setFilterGrid("tbl_search",-1); */
				show_list_view(po_id,'show_country_listview','list_view_country','requires/gmts_rcve_from_wash_v2_controller','');
				set_button_status(0, permission, 'fnc_rcve_print_embroidery_entry',1,0);
				release_freezing();
			}
	...
	function put_country_data(po_id, item_id, country_id, po_qnty, plan_qnty)
	{
		...
		get_php_form_data(po_id+'**'+item_id+'**'+$('#cbo_embel_name').val()+'**'+country_id, "populate_data_from_search_popup", "requires/gmts_rcve_from_wash_v2_controller" );

		let variableSettings=$('#sewing_production_variable').val();
		let styleOrOrderWisw=$('#styleOrOrderWisw').val();

		if(variableSettings==1) $("#txt_rcve_qty").removeAttr("readonly");
		else $('#txt_rcve_qty').attr('readonly','readonly');

		if(variableSettings!=1)
		{
			get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+$("#cbo_embel_name").val()+'**'+country_id, "color_and_size_level", "requires/gmts_rcve_from_wash_v2_controller" );
		}

		set_button_status(0, permission, 'fnc_rcve_print_embroidery_entry',1,0);
		release_freezing();
	}
	function fnc_rcve_print_embroidery_entry(operation)
	{
		let source=$("#cbo_source").val();
		if(operation==4)
		{
			var report_title=$( "div.form_caption" ).html();
			print_report( $('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+report_title+'*'+$("#sewing_production_variable").val(), "wash_receive_print", "requires/gmts_rcve_from_wash_v2_controller" )
			return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			if('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][648]);?>'){
				if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][648]);?>','<? echo implode('*',$_SESSION['logic_erp']['field_message'][648]);?>')==false)
				{
			...
			if ( form_validation('cbo_company_name*cbo_location_name*txt_rcve_date*cbo_source*cbo_emb_company*txt_order_no*txt_rcve_qty*cbo_embel_type','Company Name*PO Location*Issue Date*Source*Wash Company*Order No*Issue Quantity*Embel. Type')==false )
			{
				return;
			}
			else
			{
				/* if(source==1)
				{
					if ( form_validation('cbo_wash_location*cbo_floor','Wash Location*Floor')==false )
					{
						return;
					}
				}
				...
				{
					$("input[name=colorSizeRej]").each(function(index, element) {
						// console.log( "inside loop"+ $(this).val());
						if( $(this).val()!='' )
						{
							if(k==0)
							{
				...
				// console.log("Rej"+colorIDvalueRej);  return;
				let data="action=save_update_delete&operation="+operation+"&colorIDvalue="+colorIDvalue+"&colorIDvalueRej="+colorIDvalueRej+get_submitted_data_string('txt_system_no*txt_system_id*cbo_company_name*garments_nature*cbo_location_name*txt_rcve_date*cbo_source*cbo_emb_company*cbo_wash_location*txt_remark*cbo_received_location*cbo_floor*hidden_po_break_down_id*txt_rcve_qty*cbo_embel_type*cbo_country_name*cbo_item_name*cbo_buyer_name*txt_style_no*txt_job_no*txt_ir*txt_country_ship_date*txt_pack_type*txt_cut_qty*txt_sewing_qty*txt_cum_rcve_qty*sewing_production_variable*hidden_colorSizeID*txt_yet_to_rcve*txt_mst_id*txt_reject_qty',"../");
				//alert (data);return;
				freeze_window(operation);
				http.open("POST","requires/gmts_rcve_from_wash_v2_controller.php",true);
				http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
				http.send(data);
				http.onreadystatechange = fnc_rcve_print_embroidery_Reply_info;
			}
		}
	}

	function fnc_rcve_print_embroidery_Reply_info()
	{
		if(http.readyState == 4)
		{
			...
			if(reponse[0]==15)
			{
				 setTimeout('fnc_rcve_print_embroidery_entry('+ reponse[1]+')',8000);
			}
			...
			show_list_view(delevary_mst_id,'show_dtls_listview_from_sys_popup','printing_production_list_view','requires/gmts_rcve_from_wash_v2_controller','');
			setFilterGrid("tbl_search",-1);
			reset_form('','','txt_rcve_qty*txt_reject_qty');
			get_php_form_data(po_id+'**'+$("#cbo_item_name").val()+'**'+$('#cbo_embel_name').val()+'**'+country_id, "populate_data_from_search_popup", "requires/gmts_rcve_from_wash_v2_controller" );
			set_button_status(0, permission, 'fnc_rcve_print_embroidery_entry',1,1);
			if(variableSettings!=1) {
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+$("#cbo_embel_name").val()+'**'+country_id, "color_and_size_level", "requires/gmts_rcve_from_wash_v2_controller" );
			}
			else
			...
			if(reponse[0]==1 || reponse[0]==2) set_button_status(0, permission, 'fnc_rcve_print_embroidery_entry',1,1);
			release_freezing();
			...
			{ 
				$("#txt_rcve_date").attr('disabled','disabled');
				...
				$("#cbo_received_location").attr('disabled','disabled');
			...
			if(reponse[4]==2)//Delete Operation
			{
				...
				reset_form('washReceive_1','list_view_country','','txt_rcve_date,<?=$date?>','childFormReset()')
			}
			release_freezing();
		}
	}
	...
	{
		//alert(data); return;
		get_php_form_data(data,'populate_receive_form_data','requires/gmts_rcve_from_wash_v2_controller');
	...
	function reve_id_popup()
	{
		...
		let page_link = 'requires/gmts_rcve_from_wash_v2_controller.php?action=system_number_popup&company_id=' + company_id+'&source='+source+'&buyer_id='+buyer;
		...
		emailwindow.onclose=function()
		{
			...
			get_php_form_data(responseDataArr[0], "populate_mst_form_data", "requires/gmts_rcve_from_wash_v2_controller" );

			show_list_view(responseDataArr[0],'show_dtls_listview_from_sys_popup','printing_production_list_view','requires/gmts_rcve_from_wash_v2_controller','');
			setFilterGrid("tbl_search",-1);
			// printing_production_list_view
		...
		<div style="width:1010px; float:left" align="center">
			<form name="washReceive_1" id="washReceive_1" autocomplete="off">
				<fieldset style="width:1010px;">
					<legend>Gmts. Receive From Wash V2</legend>
					<fieldset>
						<table width="1000px" border="0">
							<tr>
								<td align="right" colspan="3">Receive ID</td>
								<td colspan="3">
							...
							<tr>
								...
								<td width="210">
									<? echo create_drop_down( "cbo_source", 170, $knitting_source,"", 1, "-- Select Source --", 1, "load_drop_down( 'requires/gmts_rcve_from_wash_v2_controller', this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_wash_company', 'emb_company_td' );", 0, '1,3' ); ?>
								</td>
							...
							<tr>
								...
								<td width="210">
									<?
										...
										echo create_drop_down( "cbo_received_location", 170, $sending_sql,"id,location_name", 1, "-- Select Sending Location --", $selected, "load_drop_down( 'requires/gmts_rcve_from_wash_v2_controller', this.value, 'load_drop_down_floor', 'floor_td' );" );
									?>
								</td>
								...
								<fieldset>
									...
									<table cellpadding="0" cellspacing="2" width="100%">
										<tr>
											...
											<td>
												<input name="txt_order_no" id="txt_order_no" placeholder="Double Click to Search" onDblClick="openmypage('requires/gmts_rcve_from_wash_v2_controller.php?action=order_popup&company='+$('#cbo_company_name').val()+'&garments_nature='+$('#garments_nature').val()+'&hidden_variable_cntl='+$('#hidden_variable_cntl').val()+'&hidden_preceding_process='+$('#hidden_preceding_process').val()+'&sewing_production_variable='+$('#sewing_production_variable').val()+'&buyer_id='+$('#cbo_buyer_name').val()+'&source='+$('#cbo_source').val(),'Order Search')" class="text_boxes" style="width:150px " readonly />
					...
					<table cellpadding="0" cellspacing="1" width="100%">
						<tr>
							<td align="center" colspan="6" valign="middle" class="button_container">
								<?
									echo load_submit_buttons($permission, "fnc_rcve_print_embroidery_entry", 0, 1, "reset_form('washReceive_1','list_view_country','','txt_rcve_date,".$date."','childFormReset()')", 1);
								?>
