<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Hang Tag Entry
...
function openmypage(page_link,title)
{
	...
	if ( form_validation('cbo_company_name*cbo_source*cbo_hang_tag_company','Company Name*Production Source*Production Company')==false )
	{
		return;
	}
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			if (po_id!="")
			{
				...
				get_php_form_data(po_id+'**'+item_id+'**'+country_id+'**'+$('#hidden_preceding_process').val(), "populate_data_from_search_popup", "requires/hang_tag_entry_controller" );
				...
				if(variableSettings!=1){ 
					get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject+'**'+$('#hidden_preceding_process').val(), "color_and_size_level", "requires/hang_tag_entry_controller" ); 
				}
				else
				{
					$("#txt_hang_tag_qty").removeAttr("readonly");
				}
				...
				show_list_view(po_id+'**'+item_id+'**'+country_id,'show_dtls_listview','list_view_container','requires/hang_tag_entry_controller','setFilterGrid(\'tbl_list_search\',-1)');
				show_list_view(po_id+'_'+preceding_process,'show_country_listview','list_view_country','requires/hang_tag_entry_controller','');
...
function fnc_iron_input(operation)
{
	if(operation==4)
	{
		 ...
		 print_report( $('#cbo_company_name').val()+'*'+$('#txt_mst_id').val()+'*'+report_title, "iron_output_print", "requires/hang_tag_entry_controller" ) 
		 return;
	}
	else if(operation==0 || operation==1 || operation==2)
	{
 		if ( form_validation('cbo_company_name*txt_order_no*cbo_hang_tag_company*txt_hang_tag_date*cbo_location*cbo_floor','Company Name*Order No*Hang Tag Company*Input Date*Location*Floor')==false )
		{
			return;
		}		
		else
		{
			if($('#txt_hang_tag_qty').val()<1&&$('#txt_re_hang_tag_qty').val()<1&&$('#txt_reject_qnty').val()<1)
			{
			...
			if(date_compare($('#txt_hang_tag_date').val(), current_date)==false)
			{
 	...
 	if(http.readyState == 4) 
	{
		...
		{ 
			...
			show_msg(trim(reponse[0]));
 			show_list_view(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id,'show_dtls_listview','list_view_container','requires/hang_tag_entry_controller','setFilterGrid(\'tbl_list_search\',-1)');
			reset_form('','','txt_hang_tag_qty*txt_re_hang_tag_qty*txt_reject_qnty*txt_challan*txt_remark*txt_sewing_quantity*txt_cumul_hang_tag_qty*txt_yet_to_hang_tag*hidden_break_down_html*txt_mst_id','','','txt_hang_tag_date*txt_reporting_hour');
			get_php_form_data(po_id+'**'+$('#cbo_item_name').val()+'**'+country_id+'**'+$('#hidden_preceding_process').val(), "populate_data_from_search_popup", "requires/hang_tag_entry_controller" );
 			if(variableSettings!=1) { 
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject+'**'+$('#hidden_preceding_process').val(), "color_and_size_level", "requires/hang_tag_entry_controller" ); 
			}
			else
...
{
	//txt_hang_tag_date  txt_reporting_hour cbo_time txt_hang_tag_qty txt_remark txt_sewing_quantity txt_cumul_hang_tag_qty txt_yet_to_hang_tag
	reset_form('','','txt_reporting_hour*txt_hang_tag_qty*txt_re_hang_tag_qty*txt_challan*txt_remark*txt_sewing_quantity*txt_cumul_hang_tag_qty*txt_yet_to_hang_tag*hidden_break_down_html*txt_mst_id','','');
 	$('#txt_sewing_quantity').attr('placeholder','');//placeholder value initilize
	$('#txt_cumul_hang_tag_qty').attr('placeholder','');//placeholder value initilize
	$('#txt_yet_to_hang_tag').attr('placeholder','');//placeholder value initilize
...
function fn_total_reiron(tableName,index) // for color and size level
{
	...
	$("#txt_re_hang_tag_qty").val(totalValRej);
}
...
function fnc_company_check(val)  
{
	if(val==1)
	{
		if($("#cbo_company_name").val()==0)
		{
			...
			$("#cbo_source").val(0);
			$("#cbo_hang_tag_company").val(0);
			return;
		}
		else
		{
			get_php_form_data(document.getElementById('cbo_hang_tag_company').value,'production_process_control','requires/hang_tag_entry_controller' );
		}
	}
	else
	{
		get_php_form_data(document.getElementById('cbo_company_name').value,'production_process_control','requires/hang_tag_entry_controller' );
	}
 }
...
function fn_colorlevel_total(index) //for color level
{
	...
	$("#txt_hang_tag_qty").val( $("#total_color").val() );
} 
...
function fn_colorReiron_total(index) //for color level
{
	...
	$("#txt_re_hang_tag_qty").val( $("#total_color_reiron").val() );
}
...
function put_country_data(po_id, item_id, country_id, po_qnty, plan_qnty)
{
	...
	show_list_view(po_id+'**'+item_id+'**'+country_id,'show_dtls_listview','list_view_container','requires/hang_tag_entry_controller','');
	set_button_status(0, permission, 'fnc_iron_input',1,0);
	release_freezing();
}
...
function fnc_workorder_search(supplier_id)
{
	...
	if( form_validation('cbo_company_name*txt_order_no*cbo_hang_tag_company','Company Name*Order No*Hang Tag Company')==false )
	{
		return;
	}
	...
	load_drop_down( 'requires/hang_tag_entry_controller', company+"_"+supplier_id+"_"+po_break_down_id, 'load_drop_down_workorder', 'workorder_td' ); 
	//alert($('#cbo_cutting_company option').size())
}

function fnc_workorder_rate(data,id)
{
	get_php_form_data(data+"_"+id, "populate_workorder_rate", "requires/hang_tag_entry_controller" );
}
</script>
</head>
  				...
  				<fieldset>
                    ...
                    <table width="100%" border="0">
                        <tr>
                            ...
                            <td>                                
								<? 
								echo create_drop_down( "cbo_company_name", 170, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "get_php_form_data(this.value,'load_variable_settings','requires/hang_tag_entry_controller');" );
								?>	
							...
							<td width="" class="must_entry_caption">&nbsp;Source</td>
                             <td width="">
								 <?
								 echo create_drop_down( "cbo_source", 170, $knitting_source,"", 1, "-- Select Source --", $selected, "fnc_company_check(this.value);load_drop_down( 'requires/hang_tag_entry_controller', this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_source', 'iron_company_td' );dynamic_must_entry_caption(this.value);", 0, '1,3' );
								 ?>
                             </td>
                          ...
                          <td width="" class="must_entry_caption">Hang Tag Company</td>
                             <td id="iron_company_td" width="170">
								 <?
                                 echo create_drop_down( "cbo_hang_tag_company", 170, $blank_array,"", 1, "-- Select --", $selected, "" );
                                 ?>
						     </td>
                            ...
                            <td width="170">
                                <input name="txt_order_no" placeholder="Double Click to Search" id="txt_order_no" onDblClick="openmypage('requires/hang_tag_entry_controller.php?action=order_popup&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value+'&production_company='+document.getElementById('cbo_hang_tag_company').value+'&hidden_variable_cntl='+document.getElementById('hidden_variable_cntl').value+'&hidden_preceding_process='+document.getElementById('hidden_preceding_process').value,'Order Search')"  class="text_boxes" style="width:160px " readonly />
                    ...
                    <tr>
                            ...
                            <legend>New Entry</legend>
                                 <table  cellpadding="0" cellspacing="2" width="100%">
                                 	<tr>
                                    	...
                                    	<tr>
                                            <td width="120">Hang Tag Output Date</td>
                                             <td width=""> 
                                              	<input name="txt_hang_tag_date" id="txt_hang_tag_date" class="datepicker" type="text" value="<? echo date("d-m-Y")?>" style="width:100px;"  />
                                            </td>
                                    	</tr>
                                         <tr>
                               ...
                               <input name="txt_reporting_hour" id="txt_reporting_hour" class="text_boxes" style="width:100px" placeholder="24 Hour Format" onBlur="fnc_valid_time(this.value,'txt_reporting_hour');" onKeyUp="fnc_valid_time(this.value,'txt_reporting_hour');" onKeyPress="return numOnly(this,event,this.id);" maxlength="8" />
                                     ...
                                     <tr>  
                                         <td width="" class="must_entry_caption">Hang Tag. Output Qty</td> 
                                         <td width=""> 
                                            <input name="txt_hang_tag_qty" id="txt_hang_tag_qty" class="text_boxes_numeric"  style="width:100px" readonly  />
                                     ...
                                     <tr>  
                                         <td width="">Re-Hang Tag Qty</td> 
                                         <td width=""> 
                                            <input name="txt_re_hang_tag_qty" id="txt_re_hang_tag_qty" class="text_boxes_numeric"  style="width:100px" />
                                        </td>
                                    </tr>
                                    <tr>  
                        ...
                        <td width="25%" valign="top">
                            ...
                            <legend>Display</legend>
                                <table  cellpadding="0" cellspacing="2" width="100%" >
                                    <tr>
                                        <td width="140" id="caption_msg_id">Iron Output Qty</td>
                                        <td>
                                    ...
                                    <tr>
                                        <td width=""> Cumul. Hang Tag. Qty</td>
                                        <td>
                                         <input type="text" name="txt_cumul_hang_tag_qty" id="txt_cumul_hang_tag_qty" class="text_boxes_numeric" style="width:80px"  readonly disabled />
                                        </td>
                                    </tr>
                                     <tr>
                                        <td width="">Yet to Hang Tag Output</td>
                                        <td>
                                         <input type="text" name="txt_yet_to_hang_tag" id="txt_yet_to_hang_tag" class="text_boxes_numeric" style="width:80px" readonly disabled />
                                        </td>
                                    </tr>
                                </table>
                            </fieldset>	
                     ...
                     <tr>
		   				<td align="center" colspan="9" valign="middle" class="button_container">
							<?
							$date=date('d-m-Y');
                            echo load_submit_buttons( $permission, "fnc_iron_input", 0, 1,"reset_form('ironoutput_1','list_view_country','','txt_hang_tag_date,".$date."','childFormReset()')",1); 
                            ?>
