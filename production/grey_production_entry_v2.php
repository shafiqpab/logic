...
//--------------------------------------------------------------------------------------------------------------------
		...
		if (form_validation('cbo_company_id*cbo_knitting_source*txt_booking_no','Company*Knitting Source*Booking/Knit Plan')==false)
		{
		    ...
		    emailwindow.onclose=function()
		    {
				...
				if(theemail!="")
				{
					...
					show_list_view(theename,'show_service_booking_programlist','list_program_wise_fabric_desc_container','requires/grey_production_entry_controller_v2','');
					release_freezing();
				}
			}
		}
				...
				if(theemail!="")
				{
					...
					else
					{
						...
						if($('#process_costing_maintain').val()*1!=1)
						{
							...
							$('#txt_color_type_id').val(data[25]);
							$('#txt_color_type').val(data[26]);
	...
	function openmypage_po()
	{
		...
		var txt_receive_qnty_pcs = $('#txt_receive_qnty_pcs').val();
		...
		var txt_operator_name=$('#txt_operator_name').val();
		var txt_operator_id=$('#txt_operator_id').val();
		var txt_id_card=$('#txt_id_card').val();
		...
		emailwindow.onclose=function()
		{
			...
			var txt_operator_id=this.contentDoc.getElementById("txt_operator_id").value;
			var txt_operator_name=this.contentDoc.getElementById("txt_operator_name").value;

			$('#txt_operator_id').val(txt_operator_id);
			$('#txt_operator_name').val(txt_operator_name);
			if(weight_scale_type*1 !=0)
			{
				$('#txt_operator_name').attr("disabled","disabled");
			}
			else
			{
				$('#txt_operator_name').removeAttr("disabled");
			}
			...
			var program_no=	$('#txt_booking_no').val();
			var program_machine_number=	$('#cbo_machine_name').val();
			var txt_receive_qnty=	$('#txt_receive_qnty').val();

			if($('#process_costing_maintain').val()*1 ==1 ||  $('#var_rate_mandatory').val()==1 )
			{
				get_php_form_data(program_no+ '_' + cbo_company_id + '_' + program_machine_number + '_' + $("#txt_receive_date").val() + '_' + txt_receive_qnty + '_' + $('#process_costing_maintain').val()*1 + '_' + $('#var_rate_mandatory').val(), "load_yarn_lot_popup_data", "requires/grey_production_entry_controller_v2" );
			}
	...
	function fnc_grey_production_entry(operation)
	{
		...
		if($("#txt_operator_id").val() =="" || $("#txt_operator_id").val() ==0)
		{
			alert("Operator Name not found for save/update event.");
			fnc_reset();
			return;
		}
		...
		if($('#txt_receive_qnty').val()==0 && $('#txt_reject_fabric_recv_qnty').val()==0)
		{
			...
			{
				var operation_id_previours = $('#txt_id_card').val();
				alert("Please Fill up Grey Receive Qnty Value");
				$('#txt_id_card').val(operation_id_previours);
				fnc_reset();
				return;
			}
	...
	function fnc_grey_production_entry_Reply_info()
	{
		if(http.readyState == 4)
		{
			...
			//var auto_pdf_print=1;
			if(weight_scale_type==2 && auto_barcode_generate !=2)
			{
				var data="***"+reponse[4]+"*********"+reponse[1];
				var pdf_path=return_ajax_request_value(data, "auto_create_and_print_pdf_file", "requires/grey_production_entry_controller_v2");
				//alert(api_data);
				printPDF(pdf_path);
				fnc_reset();
				$('#txt_booking_no').focus();
			...
			else if(reponse[0]==0 && auto_barcode_generate !=2)
			{
				var data="***"+reponse[4]+"*********"+reponse[1];
	...
	function norsel_print_manual()
	{
		$('#update_dtls_id').val();
		$('#update_id').val();

		if($('#update_dtls_id').val() =="" || $('#update_id').val()=="")
		{
			alert("Print will work only update mode");
			return;
		}

		var data="***"+$('#update_dtls_id').val()+"*********"+$('#update_id').val();
		var api_data=return_ajax_request_value(data, "report_barcode_text_file_for_api", "requires/grey_production_entry_controller_v2");
	}
			...
			emailwindow.onclose=function()
			{
				...
				if(trim(grey_recv_id)!="")
				{
					...
					if(cbo_receive_basis==2 && knitting_source==3 && txt_service_booking!="")
					{
						...
						show_list_view(txt_service_booking,'show_service_booking_programlist','list_program_wise_fabric_desc_container','requires/grey_production_entry_controller_v2','');
						$("#cbo_knitting_source").attr("disabled",'true');
						$("#cbo_knitting_company").attr("disabled",'true');
					}
	...
	function fnc_operator_name()
	{
		...
		{
			var page_link='requires/grey_production_entry_controller_v2.php?cbo_company_id='+cbo_company_id+'&booking_id='+booking_id+'&receive_basis='+receive_basis+'&cbo_location_name='+cbo_location_name+'&cbo_knitting_company='+cbo_knitting_company+'&cbo_knitting_source='+cbo_knitting_source+'&action=operator_name_popup';
	...
	function fnc_barcode_code128(type)
	{
		...
		if(type==1){
			...
			var url=return_ajax_request_value(data, "direct_print_barcode_5", "requires/grey_production_entry_controller_v2");
		}else if(type==15){
			var url=return_ajax_request_value(data, "print_barcode_ccl", "requires/grey_production_entry_controller_v2");
		}else if(type==16){
			var url=return_ajax_request_value(data, "direct_print_barcode_6", "requires/grey_production_entry_controller_v2");
		}else if(type==17){
			var url=return_ajax_request_value(data, "print_barcode_one_128_v5", "requires/grey_production_entry_controller_v2");
		} else{
	...
	$('#txt_booking_no').live('keydown', function (e)
	{
    	if (e.keyCode === 13)
		{
    		...
    		if(cbo_receive_basis==2)
    		{
				...
				{
					//For SSL Scale
					var reponse=trim(txt_booking_no).split('-');
					if(reponse[0]=="EMP")
					{
						...
						.done(function( data ) {
							//N.B. To reset weight data this api is called
						});
				...
				var program_machine_number='';
				var program_machine_arr =trim(txt_booking_no).split('/');
				if(program_machine_arr.hasOwnProperty(1))
				{
					program_machine_number = program_machine_arr[1];
					txt_booking_no=program_machine_arr[0];
				}
				...
				$("#txt_color_type_id").val(data[38]);
				$("#txt_color_type").val(data[39]);
				...
				{
					$('#cbo_buyer_name').attr('disabled','disabled');
					get_php_form_data(theemail+ '_' + cbo_company_id + '_' + program_machine_number, "populate_data_from_booking2", "requires/grey_production_entry_controller_v2" );
					$("#cbo_knitting_source").attr("disabled",'true');
					$("#cbo_knitting_company").attr("disabled",'true');
					//$("#cbo_location_name").attr("disabled",'true');
				...
				if($('#process_costing_maintain').val()*1==1)
				{
					disable_enable_fields('cbo_com_location_name*cbo_company_id*cbo_com_location_name*cbo_location_name*txt_shift_name*txt_stitch_length*cbo_floor_id*txt_color*cbo_color_range*txt_brand*cbo_uom*cbo_floor*cbo_room*txt_rack*txt_shelf*txt_roll_no',1); //cbo_store_name
					//length*cbo_machine_name*txt_gsm*txt_width*txt_service_booking
				}
				else
				{
					disable_enable_fields('cbo_com_location_name*cbo_company_id*cbo_com_location_name*cbo_location_name*txt_shift_name*txt_stitch_length*cbo_floor_id*txt_color*txt_yarn_lot*cbo_color_range*show_textcbo_yarn_count*txt_brand*cbo_uom*cbo_floor*cbo_room*txt_rack*txt_shelf*txt_roll_no',1); //cbo_store_name
					//length*cbo_machine_name*txt_gsm*txt_width*txt_service_booking
				}

				if(auto_barcode_generate == 2 || $('#body_part_type_id').val()==40 || $('#body_part_type_id').val() ==50 || weight_scale_type*1==0)
				{
					//N.B. body part type "40=>flat knit" and "50=>cuff" then quantity popup will open to give size
				...
				else
				{
					...
					.done(function( data ) {
						...
						if (obj != null && obj !== undefined)
						{
							...
							//get_php_form_data(receive_basis+'**'+cbo_company_id+'**'+booking_no+'**'+dtls_id+'**'+all_po_id+'**'+roll_maintained+'**'+production_control+'**'+barcode_generation+'**'+save_data_stylewise+'**'+txt_receive_qnty+'**'+txt_receive_qnty_pcs+'**'+prev_distribution_method+'**'+cbo_body_part+'**'+txt_gsm+'**'+txt_width+'**'+fabric_desc_id+'**'+txt_deleted_id+'**'+txt_reject_fabric_recv_qnty+'**'+booking_without_order+'**'+booking_id+'**'+cbo_com_location_name+'**'+cbo_store_name+'**'+fabric_store_auto_update+'**'+store_update_upto+'**'+api_weight, "load_yarn_lot_popup_data", "requires/grey_production_entry_controller_v2" );

							if($('#process_costing_maintain').val()*1 ==1 ||  $('#var_rate_mandatory').val()==1 ){
								get_php_form_data(theemail+ '_' + cbo_company_id + '_' + program_machine_number + '_' + $("#txt_receive_date").val() + '_' + api_weight + '_' + $('#process_costing_maintain').val()*1 + '_' + $('#var_rate_mandatory').val(), "load_yarn_lot_popup_data", "requires/grey_production_entry_controller_v2" );
							}
	...
	function get_po_popup_data(api_weight)
	{
		...
		get_php_form_data(receive_basis+'**'+cbo_company_id+'**'+booking_no+'**'+dtls_id+'**'+all_po_id+'**'+roll_maintained+'**'+production_control+'**'+barcode_generation+'**'+save_data_stylewise+'**'+txt_receive_qnty+'**'+txt_receive_qnty_pcs+'**'+prev_distribution_method+'**'+cbo_body_part+'**'+txt_gsm+'**'+txt_width+'**'+fabric_desc_id+'**'+txt_deleted_id+'**'+txt_reject_fabric_recv_qnty+'**'+booking_without_order+'**'+booking_id+'**'+cbo_com_location_name+'**'+cbo_store_name+'**'+fabric_store_auto_update+'**'+store_update_upto+'**'+api_weight+'**'+weight_scale_type, "populate_data_from_po_popup_stylewise", "requires/grey_production_entry_controller_v2" );
					...
					<fieldset style="width:930px">
						<table cellpadding="0" cellspacing="2" width="100%" id="master_table">
							...
							<tr>
								...
								<td width="150">
									<input type="text" name="txt_booking_no" id="txt_booking_no" class="text_boxes" style="width:140px"  placeholder="Double Click to Search" onDblClick="openmypage_fabricBooking();" >
							...
							<tr>
								...
								<td>Internal Ref</td>
								<td>
									<input type="text" name="txt_internal_ref" id="txt_internal_ref" class="text_boxes" style="width:140px;" placeholder="Display" disabled="" readonly/>
								</td>
							</tr>
						</table>
					</fieldset>
								...
								<fieldset>
									...
									<table cellpadding="0" cellspacing="2" width="100%">
										...
										<tr>
											...
											<td>
												<input type="text" name="txt_gsm" id="txt_gsm" class="text_boxes_numeric" style="width:60px;" />
												<input type="text" name="txt_old_gsm" id="txt_old_gsm" class="text_boxes" style="width:47px;" disabled="disabled" readonly placeholder="Old gsm"/>
										...
										<tr>
											...
											<td>
												<input type="text" name="txt_width" id="txt_width" class="text_boxes" style="width:60px;text-align:right;" />
												<input type="text" name="txt_old_dia" id="txt_old_dia" class="text_boxes" style="width:47px;" disabled="disabled" readonly placeholder="Old dia"/>
										...
										<tr>
											<td>Color Type</td>
											<td>
												<input type="text" name="txt_color_type" id="txt_color_type" class="text_boxes" style="width:120px;" readonly/>
												<input type="hidden" name="txt_color_type_id" id="txt_color_type_id" />
											</td>
											<td>Rack</td>
											<!--<td>
                ...
                <tr>
                	<td colspan="6" align="center" class="button_container">
                		<?
							...
							<input type="button" value="Norsel print manual" class="formbutton" style=" width:150px; margin-right: 100px;" onClick="norsel_print_manual()"/>
                        </td>
                    </tr>
                </table>
