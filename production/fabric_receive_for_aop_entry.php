...
function openmypage_order(page_link,title)
{
	...
	emailwindow.onclose=function()
	{
		...
		if (id.value!="")
		{
			reset_form('','booking_list_view','txt_order_no_id*txt_order_no*cbo_currency*txt_exchange_rate*cbo_pay_mode*txt_receive_date*cbo_supplier_name*txt_attention*txt_delivery_date*cbo_source*txt_booking_no','txt_receive_date,<? echo date("d-m-Y"); ?>');
			freeze_window(5);
			document.getElementById('txt_order_no_id').value=id.value;
			document.getElementById('txt_order_no').value=po.value;
			get_php_form_data( id.value, "populate_order_data_from_search_popup", "requires/fabric_receive_for_aop_entry_controller" );
...
function calculate_amount(i)
{
	...
	var rcvQnty=(document.getElementById('txtRcvqnty_'+i).value)*1;
	//alert(rcvQnty);
	if(is_short==2)
	{
		var balance = (document.getElementById('txtRcvqnty_'+i).placeholder)*1;
		//alert(balance);
		if(rcvQnty>balance){
			alert("No Balance Quantity"); 
			document.getElementById('txtAmount_'+i).value='';
			document.getElementById('txtRcvqnty_'+i).value='';
	...
	//alert(txt_rate);
	var txt_amount=rcvQnty*txt_rate;
...
{
	//alert(1);
	var total_qty=0; var total_amount=0; var total_issue_qty=0; 
	$('table#tbl_dtls_emb tbody tr').each(function()
	{
		var txtWoqnty 		= $(this).find('input[name="txtWoqnty[]"]').val()*1;
		var txtRcvqnty 		= $(this).find('input[name="txtRcvqnty[]"]').val()*1;
		var txtAmount 		= $(this).find('input[name="txtAmount[]"]').val()*1;
		total_issue_qty 	+= txtWoqnty;
		total_qty 			+= txtRcvqnty;
		total_amount 		+= txtAmount;
		//alert(txtWoqnty);
	});
	//alert(total_qty); 
	document.getElementById('total_issue_qty').value 	=total_issue_qty.toFixed(2);
	document.getElementById('total_qty').value 			=total_qty.toFixed(2);
...
function fnc_generate_booking()
{
	...
	else
	{
		...
		http.open("POST","requires/fabric_receive_for_aop_entry_controller.php",true);
...
function openmypage_booking(page_link,title)
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1550px,height=400px,center=1,resize=1,scrolling=0','')
	emailwindow.onclose=function()
	{
		var theform=this.contentDoc.forms[0];
		var all_issue_id=this.contentDoc.getElementById("all_issue_id").value;
		var all_sub_dtls_id=this.contentDoc.getElementById("all_sub_dtls_id").value;
		var all_within_group=this.contentDoc.getElementById("all_within_group").value;
		var all_entry_form=this.contentDoc.getElementById("all_entry_form").value;
		//alert(all_within_group);
		document.getElementById('within_group').value=all_within_group;

		if (all_issue_id!="")
		{
			//alert(theemail.value);
			var data='1'+'**'+all_issue_id+'**'+all_sub_dtls_id+'**'+0+'**'+cbo_company_id+'**'+all_entry_form;
		 	get_php_form_data( all_issue_id+'**'+all_entry_form, "populate_issue_data_from_search_popup", "requires/fabric_receive_for_aop_entry_controller" );
		 	show_list_view(data, 'generate_aop_booking','booking_list_view','requires/fabric_receive_for_aop_entry_controller','$(\'#hide_fabric_description\').val(\'\')');
		 	set_all_onclick();
			calculate_total_amount();
			//alert("mmm");
	   		//set_button_status(1, permission, 'fnc_trims_booking',1);
		    //show_list_view(document.getElementById('txt_booking_no').value, 'fabric_detls_list_view','data_panel','requires/fabric_receive_for_aop_entry_controller','');

		    //get_php_form_data( document.getElementById('cbo_company_name').value, 'company_wise_report_button_setting','requires/fabric_receive_for_aop_entry_controller' );
		}
	}
}

function openmypage_receive(page_link,title)
{
	var cbo_company_id=document.getElementById('cbo_company_name').value;
	var cbo_supplier_name=document.getElementById('cbo_supplier_name').value;
	var page_link=page_link+'&cbo_company_id='+cbo_company_id+'&cbo_supplier_name='+cbo_supplier_name;
	...
	emailwindow.onclose=function()
	{
		...
		{
			//alert(theemail.value);
			//reset_form('servicebooking_1','booking_list_view','','txt_receive_date,<? echo date("d-m-Y"); ?>');
		 	get_php_form_data( theemail_booking.value, "populate_receive_data_from_search_popup", "requires/fabric_receive_for_aop_entry_controller" );
			//alert("mmm");
	   		//set_button_status(1, permission, 'fnc_trims_booking',1);
			...
			set_all_onclick();
			calculate_total_amount();
			set_button_status(1, permission, 'fnc_service_booking_dtls',1);
...
function fnc_trims_booking( operation ){
	...
	{
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_system_id*cbo_company_name*txt_receive_date*cbo_receive_purpose*cbo_source*cbo_supplier_name*txt_party_location*txt_order_no*hid_order_id*hid_issue_id*cbo_is_short*txt_remarks*update_id*txt_rcv_challan_no*buyer_id*within_group',"../");
	}
...
function fnc_service_booking_dtls( operation )
{
	...
	if(operation==4)
	{
		...
		print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+report_title, "receive_print", "requires/fabric_receive_for_aop_entry_controller") 
		//return;
		show_msg("3");
	}
	else
	{
		...
		var txt_party_location 		= $('#txt_party_location').val();
		var txt_order_no 			= $('#txt_order_no').val();
		var hid_order_id 			= $('#hid_order_id').val();
		var hid_issue_id 			= $('#hid_issue_id').val();
		var cbo_is_short 			= $('#cbo_is_short').val();
		var txt_remarks 			= $('#txt_remarks').val();
		var update_id 				= $('#update_id').val();
		var txt_rcv_challan_no 		= $('#txt_rcv_challan_no').val();
		var buyer_id 				= $('#buyer_id').val();
		var within_group 			= $('#within_group').val();
		...
		$('table#tbl_dtls_emb tbody tr').each(function()
		{
			...
			var hndIssueDtlsId 	= $(this).find('input[name="hndIssueDtlsId[]"]').val();
			var txtRcvqnty 		= $(this).find('input[name="txtRcvqnty[]"]').val();
			var prodId 			= $(this).find('input[name="prodId[]"]').val();
			var IssueEntryForm  = $(this).find('input[name="IssueEntryForm[]"]').val();
			
			if(txtWoqnty==0 || txtWoqnty=='')
			{	 
				alert('Please Fill up receive Qty ');	
			...
			if(check_field==0)
			{
				//alert(check_field);
				j++;
				data_all += "&cboProcessType_" + j + "='" + cboProcessType + "'&txtWoqnty_" + j + "='" + txtWoqnty + "'&txtRate_" + j + "='" + txtRate + "'&txtAmount_" + j + "='" + txtAmount + "'&txtNumberRoll_" + j + "='" + txtNumberRoll  + "'&txtRemarks_" + j + "='" + txtRemarks + "'&soDtlsId_" + j + "='" + soDtlsId+ "'&batchDtlsId_" + j + "='" + batchDtlsId+ "'&batchId_" + j + "='" + batchId + "'&hiddenid_" + j + "='" + hiddenid+ "'&hndIssueDtlsId_" + j + "='" + hndIssueDtlsId+ "'&txtRcvqnty_" + j + "='" + txtRcvqnty+ "'&prodId_" + j + "='" + prodId+ "'&IssueEntryForm_" + j + "='" + IssueEntryForm + "'";
		...
		var data="action=save_update_delete_dtls&operation="+operation+'&total_row='+i+'&txt_system_id='+txt_system_id+'&cbo_company_name='+cbo_company_name+'&txt_receive_date='+txt_receive_date+'&cbo_receive_purpose='+cbo_receive_purpose+'&cbo_source='+cbo_source+'&cbo_supplier_name='+cbo_supplier_name+'&txt_party_location='+txt_party_location+'&txt_order_no='+txt_order_no+'&hid_order_id='+hid_order_id+'&hid_issue_id='+hid_issue_id+'&cbo_is_short='+cbo_is_short+'&txt_remarks='+txt_remarks+'&update_id='+update_id+'&txt_rcv_challan_no='+txt_rcv_challan_no+'&buyer_id='+buyer_id+'&within_group='+within_group+'&cbo_currency_id='+cbo_currency_id+data_all; 

		//var data="action=save_update_delete_dtls&operation="+operation+'&total_row='+i+'&txt_order_no='+txt_order_no+'&hid_order_id='+hid_order_id+'&update_id='+update_id+'&txtDeletedId='+txtDeletedId+data_all+data; 
...
function check_exchange_rate()
{
	...
	var booking_date = $('#txt_receive_date').val();
	var cbo_company_name = $('#cbo_company_name').val();
	var response=return_global_ajax_value( cbo_currercy+"**"+booking_date+"**"+cbo_company_name, 'check_conversion_rate', '', 'requires/fabric_receive_for_aop_entry_controller');
...
function fnc_fab_booking(page_link,title)
{
	...
	emailwindow.onclose=function()
	{
		...
		if (theemail.value!="")
		{
			//reset_form('fabricbooking_1','booking_list_view','','txt_receive_date,<? //echo date("d-m-Y"); ?>');
		//	get_php_form_data( theemail.value, "populate_data_from_search_popup", "requires/fabric_receive_for_aop_entry_controller" );
			//check_month_setting();
    ...
    <!--<div id="content_search_panel" >-->
        <form name="servicebooking_1"  autocomplete="off" id="servicebooking_1">
            <fieldset style="width:950px;">
                <legend>Fabric Receive For AOP</legend>
                <table  width="900" cellspacing="2" cellpadding="0" border="0">
                    <tr>
                        <td align=""></td>
                        <td align=""></td>
                        <td  width="130" height="" align="right" class="must_entry_caption">Receive ID</td>              <!-- 11-00030  -->
                        <td  width="170" >
                        <input class="text_boxes" type="text" style="width:160px" onDblClick="openmypage_receive('requires/fabric_receive_for_aop_entry_controller.php?action=receive_popup','receive Search')" readonly placeholder="Double Click" name="txt_system_id" id="txt_system_id"/>
                        </td>
                        <td align=""></td>
                        <td align=""></td>
                    </tr>
                    <tr>
                        <td width="170" align="right" class="must_entry_caption">Company Name</td>
                        <td><? 
                        //load_drop_down( 'requires/fabric_receive_for_aop_entry_controller', this.value, 'load_drop_down_supplier', 'supplier_td' );
                        ...
                        <td width="130" align="right">receive Date</td>
                        <td width="170"><input class="datepicker" type="text" style="width:160px" name="txt_receive_date" id="txt_receive_date" value="<? echo date("d-m-Y")?>" /></td>
                        <td width="130" align="right">Receive Basis</td>              <!-- 11-00030  -->
                        <td width="170" ><? echo create_drop_down( "cbo_receive_purpose", 172, $receive_basis_arr,"", 1, "-- Select Source --", "17", "",1,'11,14,17,18'); ?></td>
                    </tr>
                    <tr>
                    	<td class="must_entry_caption" align="right"><strong>Issue ID / WO</strong></td>
                        <td>
                    		<input class="text_boxes" type="text" style="width:160px" name="txt_order_no" id="txt_order_no" onDblClick="openmypage_booking('requires/fabric_receive_for_aop_entry_controller.php?action=issue_wo_popup','Issue Search')" readonly placeholder="Double Click" />
                            <input type="hidden" name="hid_order_id" id="hid_order_id">
                            <input type="hidden" name="hid_issue_id" id="hid_issue_id">
                    	...
                    	<td width="130" align="right">Service Source </td>              <!-- 11-00030  -->
                    ...
                    <tr>
                        ...
                        <td class="must_entry_caption" align="right">Currency</td>
                        <td><?php echo create_drop_down( "cbo_currency_id", 172,$currency,'',0,'',2,0,0); ?></td>
                        <td align="right">Receive Challan No</td>
                        <td><input class="text_boxes" type="text" style="width:160px"  name="txt_rcv_challan_no" id="txt_rcv_challan_no"/></td>
                    ...
                    <tr>
                        ...
                        <td colspan="3"><input class="text_boxes" type="text" style="width:98%;"  name="txt_remarks" id="txt_remarks"/></td>
                    </tr>
            ...
            <fieldset style="width:950px;">
                <table cellpadding="0" cellspacing="2" border="0" class="rpt_table" rules="all" id="tbl_dtls_emb">
                    ...
                    <tfoot>
				    	...
				    	<th align="right" ><input type="text" name="total_issue_qty" id="total_issue_qty" class="text_boxes_numeric" style="width:60px" readonly /></td>
                ...
                <table width="1550" cellspacing="2" cellpadding="0" border="0">
                    <tr>
                        <td align="center" colspan="15" valign="middle" class="button_container">
                        	<? echo  load_submit_buttons( $permission, "fnc_service_booking_dtls", 0,0 ,"reset_form('servicebooking_1','booking_list_view*data_panel','','txt_receive_date,".$date."',$endis,'cbo_source*cbo_company_name*cbo_supplier_name')",1) ;
                        	//load_submit_buttons( $permission, "fnc_service_booking_dtls", 0,1 ,"reset_form('','booking_list_view*data_panel','','','','')",2) ; ?>
                        </td>
                    </tr>   
                </table>
...
function fnc_process_data(){
	//if (form_validation('cbo_company_name*txt_system_id*txt_order_no','Company*receive ID*Work Order')==false){
	if (form_validation('cbo_company_name*txt_system_id*txt_order_no','Company*receive ID*Work Order')==false){
		return;
	}
	else{
	    ...
	    var page_link='requires/fabric_receive_for_aop_entry_controller.php?action=fabric_search_popup';
...
function set_data(po_id,fabric_cost_id,precost_conver_id,booking_id){
		...
		var cbo_currency=document.getElementById('cbo_currency').value;
		//alert(cbo_is_short);
		show_list_view(fabric_cost_id+'**'+document.getElementById('txt_order_no_id').value+'**'+document.getElementById('txt_booking_no').value+'**'+document.getElementById('cbo_level').value+"**"+precost_conver_id+"**"+cbo_is_short+"**"+cbo_currency, 'show_aop_booking','booking_list_view','requires/fabric_receive_for_aop_entry_controller','$(\'#hide_fabric_description\').val(\'\')');
		set_button_status(1, permission, 'fnc_service_booking_dtls',2);
}
