<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Issue To Distribution Point
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Issue To Distribution Point","../../", 1, 1, $unicode,'','');
...
<script>
	...
	{ 
		var title = 'Issue To Distribution Point Info';	
		var page_link = 'requires/issue_to_distribution_controller.php?action=system_number_popup';
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				$('#txt_wo_no').val(ex_data[15]);
				$('#txt_update_id').val(ex_data[0]);
				$('#txt_system_no').val(ex_data[1]);
				$('#cbo_company_name').val(ex_data[2]);
				load_drop_down( 'requires/issue_to_distribution_controller', ex_data[2]+'_'+1, 'load_drop_down_location', 'location_td' );
				$('#cbo_location').val(ex_data[3]);
				$('#cbo_source').val(ex_data[4]);
				fnc_load_party(1);
				
				$('#cbo_distribution_company').val(ex_data[5]);
				fnc_load_party(2);
				$('#cbo_distribution_location').val(ex_data[6]);
				
				if(ex_data[4]==1) var location=ex_data[6]; else  var location=ex_data[3];
				load_drop_down('requires/issue_to_distribution_controller', location, 'load_drop_down_floor', 'floor_td' );
				$('#cbo_floor').val(ex_data[7]);
				//$('#txt_job_id').val(ex_data[8]);
				$('#txt_job_no').val(ex_data[8]);
				$('#txt_issue_date').val(ex_data[9]);
				
				$('#cbo_sending_location').val(ex_data[10]);
				$('#txt_challan').val(ex_data[11]);
				$('#txt_remark').val(ex_data[12]);
				$('#txt_style_ref').val(ex_data[13]);
				$('#cbo_buyer_name').val(ex_data[14]);
   
				fnc_dtls_data_load(ex_data[8],ex_data[0]);
				dynamic_must_entry_caption(ex_data[4]);
				
				set_button_status(1, permission, 'fnc_issue_distribution_entry',1,0);
				release_freezing();
			}
		}
	}//end function
	
	function fnc_issue_distribution_entry(operation)
	{
		if(operation==2)
		{
		...
		if(operation==4)
		{
			var report_title=$("div.form_caption").html();
			 print_report( $('#cbo_company_name').val()+'*'+$('#txt_update_id').val()+'*'+report_title, "issueto_distribution_print", "requires/issue_to_distribution_controller" )
			return;
		}
		
		else if(operation==0 || operation==1 || operation==2)
		{  
			if ( form_validation('cbo_company_name*cbo_location*cbo_source*cbo_distribution_company*txt_job_no*txt_issue_date','Company Name*Location*Source*Embel.Company*Job No*Issue Date')==false )
			{
				return;
			}
			else
			{
				if(source==1)
				{
					if ( form_validation('cbo_distribution_location*cbo_floor','W. Location*Floor')==false )
					{
						return;
					}
				}
				...
				for (var i=1; i<=tot_row; i++)
				{
					...
					{
						k++;
						data_str+="&txtIssueQty_" + k + "='" + $('#txtIssueQty_'+i).val()+"'"+"&txtpoid_" + k + "='" + $('#txtpoid_'+i).val()+"'"+"&txtDtlsData_" + k + "='" + $('#txtDtlsData_'+i).val()+"'"+"&txtColorSizeid_" + k + "='" + $('#txtColorSizeid_'+i).val()+"'"+"&txtDtlsUpId_" + k + "='" + $('#txtDtlsUpId_'+i).val()+"'";
					}
				}
				if(k==0)
				{
					alert("Please input Issue Qty. (PCS).");
					return;
				}
				//alert(data_str)
	
				var data="action=save_update_delete&operation="+operation+'&tot_row='+k+get_submitted_data_string('garments_nature*txt_system_no*cbo_company_name*cbo_location*cbo_source*cbo_distribution_company*cbo_distribution_location*cbo_floor*txt_job_no*txt_style_ref*cbo_buyer_name*txt_issue_date*cbo_sending_location*txt_challan*txt_remark*txt_job_id*txt_update_id*txt_wo_id',"../../")+data_str;
				// alert (data); return;
				freeze_window(operation);
				http.open("POST","requires/issue_to_distribution_controller.php",true);
				http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
				http.send(data);
				http.onreadystatechange = fnc_issue_distribution_entry_reply_info;
			}
		}
	}
	
	function fnc_issue_distribution_entry_reply_info()
	{
		if(http.readyState == 4)
		{
			...
			if(reponse[0]==15) 
			{ 
				 setTimeout('fnc_issue_distribution_entry('+ reponse[1]+')',4000); 
			}
	...
	function fnc_total_calculate(value,i)
	{
		...
		{
			//alert("Qnty Excceded");
			var confirm_value=confirm("Issue qty Excceded by Plan Qty. (PCS). Press cancel to proceed otherwise press ok.");
			if(confirm_value!=0)
			{
				$("#txtIssueQty_"+i).val('');
			}			
		...
		var tot_row=$('#tbl_details tr').length-1;
		//alert(rowCount)
		math_operation( "txtTotIssueQty", "txtIssueQty_", "+", tot_row );
		//fnc_total_calculate();
	}
	...
	function fnc_dtls_data_load(job_no,uid)
	{
		...
		var list_view_orders = return_global_ajax_value( cbo_company_id+'***'+job_no+'***'+uid, 'order_details', '', 'requires/issue_to_distribution_controller');
		if(list_view_orders!='')
		{
	...
	function fnc_load_party(type)
	{
		...
		var working_company = $('#cbo_distribution_company').val();
		var location_name = $('#cbo_location').val();
		
		if(source==1 && type==1)
		{
			load_drop_down( 'requires/issue_to_distribution_controller', company+'_'+1, 'load_drop_down_working_com', 'emb_company_td' );
		}
		else if(source==3 && type==1)
		{
			load_drop_down( 'requires/issue_to_distribution_controller', company+'_'+3, 'load_drop_down_working_com', 'emb_company_td' );
		}
		else if(source==1 && type==2)
		{
			load_drop_down( 'requires/issue_to_distribution_controller', working_company+'_'+2, 'load_drop_down_location', 'working_location_td' ); 
		} 
	}
	function openmypage_woNo()
	{
		...
		if (form_validation('cbo_company_name*cbo_source*txt_job_no','Company*Source*Job No')==false)
		{
			return;
		}
		else
		{			
			if (form_validation('cbo_company_name','cbo_source','txt_job_no','Service Company','Source','Job No')==false)
			{
				return;
			}	
			var page_link='requires/issue_to_distribution_controller.php?company_id='+cbo_company_id+'&cbo_service_source='+cbo_service_source+'&txt_job_no='+txt_job_no+'&action=work_order_popup';
...
<div style="width:100%;">
    ...
    <div style="width:1050px; float:left" align="center">
 		<fieldset style="width:1050px;">
            <legend>Issue To Distribution Point</legend>
            <form name="issuetodistribution_1" id="issuetodistribution_1" method="" autocomplete="off" >
                <fieldset>
                    <table width="100%">
                        <tr>
                            <td colspan="4" align="right"><b>ISSUE NO : </b></td>
                        ...
                        <tr>
                            ...
                            <td width="140"><? echo create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "--Select--", $selected, "load_drop_down( 'requires/issue_to_distribution_controller', this.value+'_'+1, 'load_drop_down_location', 'location_td'); fnc_load_party(1,document.getElementById('cbo_source').value);","" ); ?></td>
                            ...
                            <td id="emb_company_td"><? echo create_drop_down( "cbo_distribution_company", 130, $blank_array,"", 1, "-W. Company-", $selected, "" ); ?></td>
                        </tr>
                        <tr>
                            <td id="locations">W. Location</td>
                            <td id="working_location_td"><? echo create_drop_down( "cbo_distribution_location", 130, $blank_array,"", 1, "-- Select --", $selected, "" ); ?></td>
                            ...
                            <td><input type="text" name="txt_job_no" id="txt_job_no" class="text_boxes" style="width:120px" onDblClick="openmypage_job('requires/issue_to_distribution_controller.php?action=job_popup', 'Job/Order Selection Form');" placeholder="Browse" readonly /></td>
                            <td>Style Ref.</td>
                        ...
                        <tr>
                            ...
                            <td>Remarks</td>
                            <td colspan="3"><input type="text" name="txt_remark" id="txt_remark" class="text_boxes" style="width:120px" title="450 Characters Only." /></td>
                ...
                <fieldset style="width:1050px">
                    <legend>Issue Details List</legend>
                    <div>
                            ...
                            <table cellpadding="0" width="1030" cellspacing="0" border="1" class="rpt_table" rules="all" id="tbl_item_details">      
                                <tbody id="tbl_details">
                                	<tr bgcolor="#E9F3FF">
                                        ...
                                        <td width="80">&nbsp;</td>
                                        <td align="right">
                                        	<input type="text" name="txtIssueQty_1" id="txtIssueQty_1" class="text_boxes_numeric" style="width:70px;" value="" onBlur="fnc_total_calculate(this.value,1);" />
                        ...
                        <table cellpadding="0" width="1050" cellspacing="0" border="1" class="rpt_table" rules="all">      
                            <tbody>
                                <tr class="tbl_bottom" name="tr_btm" id="tr_btm">
                                    ...
                                    <td><input type="text" name="txtTotIssueQty" id="txtTotIssueQty" class="text_boxes_numeric" style="width:80px;" value="" readonly /></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <table cellpadding="0" cellspacing="1" width="1050">
                        <tr>
                            <td align="center" colspan="14" valign="middle" class="button_container">
                                <?
                                    $date=date('d-m-Y');
                                    echo load_submit_buttons( $permission, "fnc_issue_distribution_entry", 0,1 , "reset_form('issuetodistribution_1','','', 'txt_issue_date,".$date."','')",1); 
                                ?>
