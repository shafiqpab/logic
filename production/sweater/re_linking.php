<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create for Re linking 
...
function fnc_company_check(val)
{
	if(val==1)
	{
		...
		{
			get_php_form_data(document.getElementById('cbo_sewing_company').value+'**'+document.getElementById('garments_nature').value,'production_process_control','requires/re_linking_controller' );
		}
	}
	else
	{
		get_php_form_data(document.getElementById('cbo_company_name').value+'**'+document.getElementById('garments_nature').value,'production_process_control','requires/re_linking_controller' );
	}
}
...
function openmypage(page_link,title)
{
	...
	//{
		...
		emailwindow.onclose=function()
		{
		   ...
		   // load_drop_down( 'requires/re_linking_controller',company_id, 'load_drop_down_location', 'location_td' );
			get_php_form_data(company_id,'load_variable_settings','requires/re_linking_controller');
			if (po_id!="")
			{
				//freeze_window(5);
				...
				fnc_company_check(3);
				get_php_form_data(po_id+'**'+item_id+'**'+country_id+'**'+$('#hidden_preceding_process').val(), "populate_data_from_search_popup", "requires/re_linking_controller" );
				...
				if(variableSettings!=1){
					get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+$('#hidden_preceding_process').val()+'**'+garments_nature, "color_and_size_level", "requires/re_linking_controller" );
				}
				else
				{
				...
				show_list_view(po_id+'**'+item_id+'**'+country_id+'**'+prod_reso_allo,'show_dtls_listview','list_view_container','requires/re_linking_controller','setFilterGrid(\'tbl_list_search\',-1)');
				show_list_view(po_id,'show_country_listview','list_view_country','requires/re_linking_controller','setFilterGrid(\'country_list_search\',-1)');
				...
				var data=po_id+'**'+sewing_date;
 	 			var response_data = return_global_ajax_value( data, 'plan_data_action', '', 'requires/re_linking_controller');
...
function fnc_sewing_input_entry(operation)
{
	...
	if(operation==4)
	{
		 ...
		 // alert(master_ids);
		 print_report( $('#cbo_company_name').val()+'*'+master_ids+'*'+report_title, "sewing_input_print", "requires/re_linking_controller" )
		 return;
	}
	else if(operation==5)
	{
		 ...
		 var report_title=$( "div.form_caption" ).html();
		 // alert(master_ids);
		 print_report( $('#cbo_company_name').val()+'*'+master_ids+'*'+report_title, "sewing_input_print2", "requires/re_linking_controller" )
		 return;
	}
	else if(operation==0 || operation==1 || operation==2) {
		if (form_validation('cbo_company_name*txt_order_no*cbo_item_name*cbo_source*cbo_sewing_company*txt_sewing_date*txt_input_qnty', 'Company Name*Order No*Item Name*Source*Sewing Company*Sewing Date*Input Quantity') == false) {
 	...
 	if(http.readyState == 4)
	{
		...
		else if(reponse[0]==0)
		{
			...
			show_msg(trim(reponse[0]));
 			show_list_view(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id+'**'+prod_reso_allo,'show_dtls_listview','list_view_container','requires/re_linking_controller','setFilterGrid(\'tbl_list_search\',-1)');
			reset_form('','','txt_input_qnty*hidden_break_down_html*hidden_colorSizeID*txt_remark*cbo_sewing_line*txt_man_cutting_no*txt_mst_id','','');//'txt_sewing_date,<? echo date("d-m-Y"); ?>'
			get_php_form_data(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id+'**'+$('#hidden_preceding_process').val(), "populate_data_from_search_popup", "requires/re_linking_controller" );
		...
		else if(reponse[0]==1)
		{
			...
			show_msg(trim(reponse[0]));
			show_list_view(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id+'**'+prod_reso_allo,'show_dtls_listview','list_view_container','requires/re_linking_controller','setFilterGrid(\'tbl_list_search\',-1)');
			reset_form('','','txt_input_qnty*hidden_break_down_html*hidden_colorSizeID*txt_remark*cbo_sewing_line*txt_man_cutting_no*txt_mst_id','','');
			get_php_form_data(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id+'**'+$('#hidden_preceding_process').val(), "populate_data_from_search_popup", "requires/re_linking_controller" );
		...
		else if(reponse[0]==2)
		{
			...
			show_msg(trim(reponse[0]));
			show_list_view(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id+'**'+prod_reso_allo,'show_dtls_listview','list_view_container','requires/re_linking_controller','setFilterGrid(\'tbl_list_search\',-1)');
			reset_form('','','txt_input_qnty*hidden_break_down_html*hidden_colorSizeID*txt_remark*cbo_sewing_line*txt_mst_id','txt_sewing_date,<? echo date("d-m-Y"); ?>','');
			get_php_form_data(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id+'**'+$('#hidden_preceding_process').val(), "populate_data_from_search_popup", "requires/re_linking_controller" );
		...
		load_drop_down( 'requires/re_linking_controller',all_data, 'load_drop_down_sewing_line_floor', 'sewing_line_td' );
...
{
 	reset_form('','list_view_container','txt_input_qnty*cbo_sewing_line*hidden_break_down_html*hidden_colorSizeID*txt_receive_qnty*txt_cumul_input_qty*txt_yet_to_input','','');
...
{
	//alert(id); return;
	get_php_form_data(id,'populate_input_form_data','requires/re_linking_controller');
}
...
function fnc_all_system_id()
{
	...
	var page_link="requires/re_linking_controller.php?action=all_system_id_popup&po_id="+po_id;
...
function fn_chk_next_process_qty(tableName,index,sizeId) // for color and size level
{
	...
	$.ajax({
		url: 'requires/re_linking_controller.php',
		type: 'POST',
		data: data,
            ...
            <form name="sewinginput_1" id="sewinginput_1" autocomplete="off" >
            	<fieldset>
            		<table width="100%">
            			<tr>
            				...
            				<td width="">
            					<input name="txt_order_no" placeholder="Double Click to Search" onDblClick="openmypage('requires/re_linking_controller.php?action=order_popup&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value,'Order Search')" id="txt_order_no" class="text_boxes" style="width:100px " readonly /><input type="button"   class="formbutton" onclick="fnc_all_system_id();"  style="width: 77px" value="View Sys.Ch.">
            				...
            				<td width="">
            					<?
            					echo create_drop_down( "cbo_source", 180, $knitting_source,"", 1, "-- Select Source --", $selected, "fnc_company_check(this.value);load_drop_down( 'requires/re_linking_controller', this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_sewing_input', 'sew_company_td' );get_php_form_data(this.value,'line_disable_enable','requires/re_linking_controller');dynamic_must_entry_caption(this.value);", 0, '1,3' );
            					?>
            				</td>
            			...
            			<tr>
            				...
            				<td>
            					<?
            					echo create_drop_down( "cbo_company_name", 190, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "get_php_form_data(this.value,'load_variable_settings','requires/re_linking_controller');",1 );
            					?>
            				...
            				<td width="150">
            					<?
            					echo create_drop_down( "cbo_buyer_name", 180, "select id,buyer_name from lib_buyer where is_deleted=0 and status_active=1 order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "",1,0 );
            					?>
            				</td> 
            			</tr>
            			<tr> 
            				...
            				<td width="150">
            					<input name="txt_style_no" id="txt_style_no" class="text_boxes" style="width:180px " disabled  readonly />
            				</td>
            				<td width="100" class="must_entry_caption"> Item </td>
            				<td width="180">
            					<?
            					echo create_drop_down( "cbo_item_name", 180, $garments_item,"", 1, "-- Select --", $selected, "",1,0 );
            					?>
            				</td>  
            			</tr>
            			<tr> 
            				...
            				<td width="190" id="location_td">
            					<?
            					echo create_drop_down( "cbo_location", 190,$blank_array,"", 1, "-- Select Location --", $selected, "" );
            					?>
            				</td>
            				<td width="100" id="floors">Floor</td>
            				<td width="180" id="floor_td">
            					<?
				...
				<table cellpadding="0" cellspacing="1" width="100%">
						...
						<td width="30%" valign="top">
							<fieldset>
								...
								<table  cellpadding="0" cellspacing="2" width="100%">
									...
									<tr>
											<td width="" class="must_entry_caption">Linking Quantity</td>
											<td width="">
									...
									<tr>
											<td width="" class="must_entry_caption">Challan No/Sys. Chln.</td>
											<td>
											<input type="text" name="txt_iss_id" id="txt_iss_id" class="text_boxes" style="width:100px" readonly="" />
											</td>
									</tr>
									<tr>
											<td width="120">Manual Cut.No</td>
											<td>
											<input type="text" name="txt_man_cutting_no" id="txt_man_cutting_no" class="text_boxes" style="width:100px" />
											</td>
									</tr>
									<tr>
										<td width="">Remarks</td>
						...
						<td width="20%" valign="top">
							<fieldset>
								...
								<table  cellpadding="0" cellspacing="1" width="100%" >
									...
									<tr>
										<td width="150">Yet to Re-Linking</td>
										<td>
					...
					<tr>
						...
						<?
						$date=date('d-m-Y');
						echo load_submit_buttons( $permission, "fnc_sewing_input_entry", 0, 0,"reset_form('sewinginput_1','list_view_country','','txt_sewing_date,".$date."','childFormReset()')",1);
						?> 
