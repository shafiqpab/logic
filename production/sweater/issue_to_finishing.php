...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Issue To Finishing", "../../", 1, 1, $unicode, '', '');
...
<script>
	...
	{		
			...
			else
			{
				...
				http.open("POST", "requires/issue_to_finishing_controller.php", true);
	...
	function fnc_washReceive_entry_Reply_info()
	{
		if(http.readyState == 4) 
		{
			...
			{ 
				...
				// show_list_view(reponse[3],'show_dtls_listview','wash_receive_list_view','requires/issue_to_finishing_controller','setFilterGrid(\'tbl_list_search\',-1)');
				show_list_view(system_id,'show_listview','wash_receive_list_view','requires/issue_to_finishing_controller','setFilterGrid(\'tbl_list_search\',-1)');
				...
				// get_php_form_data(po_id+'**'+$('#cbo_item_name').val()+'**'+country_id+'**'+$('#hidden_preceding_process').val()+'**'+pack_type, "populate_data_from_search_popup", "requires/issue_to_finishing_controller" );
				
					
				
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+txt_job_no+'**'+$('#hidden_preceding_process').val()+'**'+pack_type, "color_and_size_level", "requires/issue_to_finishing_controller" );
				release_freezing();
	...
	// 	get_php_form_data(po_id + '**' + item_id + '**' + country_id + '**' + $('#hidden_preceding_process').val() + '**' + $('#txt_mst_id').val() + '**2**' + $('#sewing_production_variable').val() + '**' + $('#variable_is_controll').val()+ '**' + ship_date+ '**' + pack_type, "populate_data_from_search_popup", "requires/issue_to_finishing_controller");

	// 	var variableSettings = $('#sewing_production_variable').val();
	// 	var styleOrOrderWisw = $('#styleOrOrderWisw').val();
	// 	if (variableSettings != 1) {
	// 		get_php_form_data(po_id + '**' + item_id + '**' + variableSettings + '**' + styleOrOrderWisw + '**' + country_id + '**' + $('#hidden_preceding_process').val() + '**' + $('#variable_is_controll').val()+ '**' + ship_date+ '**' + pack_type, "color_and_size_level", "requires/issue_to_finishing_controller");
	// 	} else {
	...
	function delivery_sys_popup() {
		...
		/*var page_link='requires/issue_to_finishing_controller.php?action=sys_surch_popup&company='+company;*/
		var page_link = 'requires/issue_to_finishing_controller.php?action=sys_surch_popup&company_id=' + company_id;
		...
		emailwindow.onclose = function() {
			...
			if (delivery_id != "") {
				...
				load_drop_down( 'requires/issue_to_finishing_controller', ex_data[2]+'_'+1, 'load_drop_down_location', 'location_td' );
				...
				load_drop_down('requires/issue_to_finishing_controller', location, 'load_drop_down_floor', 'floor_td' );
				...
				get_php_form_data(delivery_mst_id, "populate_mst_form_data", "requires/issue_to_finishing_controller");
				
				show_list_view(delivery_mst_id,'show_listview','wash_receive_list_view','requires/issue_to_finishing_controller','setFilterGrid(\'tbl_list_search\',-1)');
	...
	function openmypage(page_link,title)
{
	...
	//			load_drop_down( 'requires/trimming_complete_controller',company_id, 'load_drop_down_location', 'location_td' );
			get_php_form_data(company_id,'load_variable_settings','requires/issue_to_finishing_controller');
			//get_php_form_data(company_id,'load_variable_settings_reject','requires/issue_to_finishing_controller');
			//console.log('variable_reject');
			console.log('reset');
			get_php_form_data(po_id+'**'+item_id+'**'+country_id+'**'+$('#hidden_preceding_process').val(), "populate_data_from_search_popup", "requires/issue_to_finishing_controller" );
			...
			if(variableSettings!=1)
			{
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+job_num+'**'+$('#hidden_preceding_process').val()+'**'+garments_nature, "color_and_size_level", "requires/issue_to_finishing_controller" );
			}
			// else
			// {
			...
			show_list_view(po_id+'**'+item_id+'**'+country_id,'show_dtls_listview','requires/issue_to_finishing_controller','setFilterGrid(\'tbl_list_search\',-1)');
			show_list_view(po_id,'show_country_listview','list_view_country','requires/issue_to_finishing_controller','');
 			set_button_status(0, permission, 'fnc_cutting_update_entry',0);
 			load_drop_down( 'requires/issue_to_finishing_controller', po_id, 'load_drop_down_color_type', 'color_type_td');
 				release_freezing();
	...
	function fnc_load_party(type)
	{
		...
		if(source==1 && type==1)
		{
			load_drop_down( 'requires/issue_to_finishing_controller', company+'_'+1, 'load_drop_down_working_com', 'wash_company_td' );
		}
		else if(source==3 && type==1)
		{
			load_drop_down( 'requires/issue_to_finishing_controller', company+'_'+3, 'load_drop_down_working_com', 'wash_company_td' );
		}
		else if(source==1 && type==2)
		{
			load_drop_down( 'requires/issue_to_finishing_controller', working_company+'_'+2, 'load_drop_down_location', 'working_location_td' ); 
		} 
	}
</script>
</head>
					...
					<fieldset>
						<table width="1180px" border="0">
							...
							<tr>
								...
								<td width="140"><? echo create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "--Select--", $selected, "load_drop_down( 'requires/issue_to_finishing_controller', this.value+'_'+1, 'load_drop_down_location', 'location_td'); fnc_load_party(1,document.getElementById('cbo_source').value);get_php_form_data(this.value,'load_variable_settings','requires/issue_to_finishing_controller');",0); ?>
							...
							<tr>
								...
								<td>
									 <input name="txt_order_no" placeholder="Double Click to Search" onDblClick="openmypage('requires/issue_to_finishing_controller.php?action=order_popup&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value,'Order Search')" id="txt_order_no" class="text_boxes" style="width:100px " readonly />
						...
						<tr>
							<td width="30%" valign="top">
								<fieldset>
									...
									<table  width="350px">
										<tr>
											<td width="100" >Total Wash. Qty</td>
											<td>
