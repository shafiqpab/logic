<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create print Technical Attachment Complete
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Technical Attachment Complete","../../", 1, 1, $unicode,'','');
...
<script>
	...
	<?php
	if($_SESSION['logic_erp']['data_arr'][712]){
		echo "var field_level_data= " . json_encode($_SESSION['logic_erp']['data_arr'][712]). ";\n";
	}
	?>
	...
	{
		var title = 'Atch Info';	
		var page_link = 'requires/technical_attachment_complete_controller.php?action=system_number_popup';
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				load_drop_down( 'requires/technical_attachment_complete_controller', ex_data[2]+'_'+1, 'load_drop_down_location', 'location_td' );
				$('#cbo_location').val(ex_data[3]);
				$('#cbo_source').val(ex_data[4]);
				fnc_load_party(1);
				
				$('#cbo_poly_company').val(ex_data[5]);
				fnc_load_party(2);
				$('#cbo_poly_location').val(ex_data[6]);
				
				if(ex_data[4]==1) var location=ex_data[6]; else  var location=ex_data[3];
				load_drop_down('requires/technical_attachment_complete_controller', location, 'load_drop_down_floor', 'floor_td' );
				$('#cbo_floor').val(ex_data[7]);
				$('#txt_job_id').val(ex_data[8]);
				$('#txt_job_no').val(ex_data[9]);
				$('#txt_poly_date').val(ex_data[10]);
				load_drop_down( 'requires/technical_attachment_complete_controller', ex_data[7]+'_'+ex_data[3]+'_'+ex_data[10], 'load_drop_down_poly_line_floor', 'poly_line_td' );
				$('#cbo_poly_line').val(ex_data[11]);
				...
				get_php_form_data(ex_data[2], "production_process_control", "requires/technical_attachment_complete_controller" );
				
				set_button_status(1, permission, 'fnc_poly_entry',1,0);
				release_freezing();
			}
		}
	}//end function
	
	function fnc_poly_entry(operation)
	{
		if(operation==2)
		{
		...
		if(operation==4)
		{
			// var report_title=$("div.form_caption").html();
			 //print_report( $('#cbo_company_name').val()+'*'+master_ids+'*'+report_title, "emblishment_issue_print", "requires/technical_attachment_complete_controller" )
			// return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			if ( form_validation('cbo_company_name*cbo_location*cbo_source*cbo_poly_company*txt_job_no*txt_poly_date*cbo_poly_line*txt_reporting_hour','Company Name*Location*Source*Atch Company*Job No*Atch Date*Atch Line No*Reporting Hour')==false )
			{
				return;
			}
			else
			{
				if(source==1)
				{
					if ( form_validation('cbo_poly_location*cbo_floor','Atch Location*Floor')==false )
					{
						return;
					}
				}
				...
				{
					alert("Atch Date Can not Be Greater Than Current Date");
					return;
				}
				...
				var data="action=save_update_delete&operation="+operation+'&tot_row='+k+get_submitted_data_string('garments_nature*txt_system_no*cbo_company_name*cbo_location*cbo_source*cbo_poly_company*cbo_poly_location*cbo_floor*txt_job_no*txt_style_ref*cbo_buyer_name*txt_poly_date*txt_reporting_hour*cbo_poly_line*txt_challan*txt_remark*txt_job_id*txt_update_id',"../../")+data_str;
				//alert (data); return;
				freeze_window(operation);
				http.open("POST","requires/technical_attachment_complete_controller.php",true);
				http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
				http.send(data);
				http.onreadystatechange = fnc_poly_entry_reply_info;
			}
		}
	}
	
	function fnc_poly_entry_reply_info()
	{
		if(http.readyState == 4)
		{
			...
			if(reponse[0]==15) 
			{ 
				 setTimeout('fnc_poly_entry('+ reponse[1]+')',4000); 
			}
	...
	function fnc_total_calculate(value,i)
	{
		...
		if(((value*1)+pre_iss_qty)>placeholder_value)
		{
			...
			{
				var confirm_value=confirm("Atch qty Excceded by Plan Qty. (PCS). Press cancel to proceed otherwise press ok.");
				if(confirm_value!=0)
				{
					$("#txtQty_"+i).val('');
				}	
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				if (theemail!="")
				{
					...
					get_php_form_data(cbo_company_name, "production_process_control", "requires/technical_attachment_complete_controller" );
					release_freezing();
				}
			}
		}
	...
	function fnc_dtls_data_load(job_no,uid)
	{
		...
		var list_view_orders = return_global_ajax_value( cbo_company_id+'***'+job_no+'***'+uid, 'order_details', '', 'requires/technical_attachment_complete_controller');
		if(list_view_orders!='')
		{
	...
	function fnc_load_party(type)
	{
		...
		var working_company = $('#cbo_poly_company').val();
		var location_name = $('#cbo_location').val();
		
		if(source==1 && type==1)
		{
			load_drop_down( 'requires/technical_attachment_complete_controller', company+'_'+1, 'load_drop_down_working_com', 'poly_company_td' );
		}
		else if(source==3 && type==1)
		{
			load_drop_down( 'requires/technical_attachment_complete_controller', company+'_'+3, 'load_drop_down_working_com', 'poly_company_td' );
		}
		else if(source==1 && type==2)
		{
			load_drop_down( 'requires/technical_attachment_complete_controller', working_company+'_'+2, 'load_drop_down_location', 'working_location_td' ); 
		} 
	}
...
<div style="width:100%;">
    ...
    <div style="width:1050px; float:left" align="center">
 		<fieldset style="width:1050px;">
            <legend>Technical Atch Info.</legend>
            <form name="polyentry_1" id="polyentry_1" method="" autocomplete="off" >
                <fieldset>
                    <table width="100%">
                        <tr>
                            <td colspan="4" align="right"><b>Atch NO : </b></td>
                        ...
                        <tr>
                            <td width="100" class="must_entry_caption">L.C Company</td>
                            <td width="140"><? echo create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "--Select--", $selected, "load_drop_down( 'requires/technical_attachment_complete_controller', this.value+'_'+1, 'load_drop_down_location', 'location_td'); fnc_load_party(1,document.getElementById('cbo_source').value);setFieldLevelAccess(this.value);","" ); ?></td>
                            ...
                            <td id="poly_company_td"><? echo create_drop_down( "cbo_poly_company", 130, $blank_array,"", 1, "--Select--", $selected, "" ); ?></td>
                        </tr>
                        <tr>
                            <td id="locations">W.C Location</td>
                            <td id="working_location_td"><? echo create_drop_down( "cbo_poly_location", 130, $blank_array,"", 1, "-- Select --", $selected, "" ); ?></td>
                            ...
                            <td><input type="text" name="txt_job_no" id="txt_job_no" class="text_boxes" style="width:120px" onDblClick="openmypage_job('requires/technical_attachment_complete_controller.php?action=job_popup', 'Job/Order Selection Form');" placeholder="Browse" readonly /></td>
                            <td>Style Ref.</td>
                        ...
                        <tr>
                            ...
                            <td class="must_entry_caption">Atch Date</td>
                            <td><input type="text" name="txt_poly_date" id="txt_poly_date" value="<? echo date("d-m-Y")?>" class="datepicker" style="width:120px;" onChange="load_drop_down( 'requires/technical_attachment_complete_controller',document.getElementById('cbo_floor').value+'_'+document.getElementById('cbo_location').value+'_'+this.value, 'load_drop_down_poly_line_floor', 'poly_line_td' );"  /></td>
                            <td class="must_entry_caption">Atch Line No</td> 
                            <td id="poly_line_td"><? echo create_drop_down( "cbo_poly_line", 130, $blank_array,"", 1, "Select Line", $selected, "",1,0 ); ?></td>
                ...
                <fieldset style="width:1200px">
                    <legend>Technical Atch Details List</legend>
                    <div>
                        <table cellpadding="0" width="1200" cellspacing="0" border="1" class="rpt_table" rules="all">
                            <thead>
                                <tr>
                                    ...
                                    <th width="70">Re-Atch Qty. (PCS)</th>
                        ...
                        <table cellpadding="0" width="1200" cellspacing="0" border="1" class="rpt_table" rules="all">      
                            <tbody>
                                <tr class="tbl_bottom" name="tr_btm" id="tr_btm">
                                    ...
                                    <td width="70"><input type="text" name="txtTotReIrnQty" id="txtTotReIrnQty" class="text_boxes_numeric" style="width:55px;" value="" readonly /></td>
                    ...
                    <table cellpadding="0" cellspacing="1" width="1200">
                        <tr>
                            <td align="center" colspan="18" valign="middle" class="button_container">
                                <?
                                    $date=date('d-m-Y');
                                    echo load_submit_buttons( $permission, "fnc_poly_entry", 0,0 , "reset_form('polyentry_1','','', 'txt_poly_date,".$date."','')",1); 
                                ?>
