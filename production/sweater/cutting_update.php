...
<?php
if($_SESSION['logic_erp']['data_arr'][681]){
	echo "var field_level_data= " . json_encode($_SESSION['logic_erp']['data_arr'][681]). ";\n";
}
?>
...
function fnc_company_check(val)
{
	if(val==1)
	{
		...
		{
			get_php_form_data(document.getElementById('cbo_work_order').value,'production_process_control','requires/sewing_input_controller' );
		}
	}
	else
	{
...
function openmypage(page_link,title)
{
	...
	//			load_drop_down( 'requires/cutting_update_controller',company_id, 'load_drop_down_location', 'location_td' );
			...
			get_php_form_data(po_id+'**'+item_id+'**'+country_id, "populate_data_from_search_popup", "requires/cutting_update_controller" );
			...
			{
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+job_num+'**'+variableSettingsReject, "color_and_size_level", "requires/cutting_update_controller" );
			}
			else
			{
...
function fnc_cutting_update_entry(operation)
{
	...
	else
	{
		...
		if('<? echo implode('*', $_SESSION['logic_erp']['mandatory_field'][681]);?>')
        {
            if (form_validation('<? echo implode('*', $_SESSION['logic_erp']['mandatory_field'][681]);?>','<? echo implode('*', $_SESSION['logic_erp']['mandatory_message'][681]);?>')==false)
            { 
                release_freezing();
                return;
            }
 	...
 	if(http.readyState == 4)
	{
		...
		else if(reponse[0]==0)//insert
		{
			...
			reset_form('','','txt_cutting_qty*txt_reject_qty*save_data*txt_reject_qty_kg*txt_used_qty_kg*txt_cutting_no*hidden_break_down_html*hidden_colorSizeID*txt_remark*txt_mst_id','','','txt_cutting_date*txt_reporting_hour');
 			get_php_form_data(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id, "populate_data_from_search_popup", "requires/cutting_update_controller" );

			if(variableSettings!=1) {
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject, "color_and_size_level", "requires/cutting_update_controller" );
			}
			else
			{
				$("#txt_cutting_qty").removeAttr("readonly");
...
function put_country_data(po_id, item_id, country_id, po_qnty, plan_qnty)
{
	...
	if(variableSettings!=1)
	{
		get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+job_num, "color_and_size_level","requires/cutting_update_controller");
	}
	else
	{
		$("#txt_cutting_qty").removeAttr("readonly");
...
function openmypage_rejectQty(type)
{
	...
	function fnc_wo_no()
	{
		if ( form_validation('cbo_company_name*cbo_source*cbo_cutting_company','Company Name*Production Source*Cutting Company')==false )
		{
			return;
		}
		else
		{
			...
			var page_link="requires/cutting_update_controller.php?action=wo_no_popup&company_id="+company_id+'&service_company_id='+service_company_id+'&txt_job_no='+txt_job_no;
                        ...
                        <table width="100%">
                            <tr>
                                ...
                                <td width="130"   class="must_entry_caption">Company </td>
                                    <td width="170">
                                        <?
                                        echo create_drop_down( "cbo_company_name", 210, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, " get_php_form_data(this.value,'load_variable_settings','requires/cutting_update_controller'); get_php_form_data(this.value,'load_variable_settings_reject','requires/cutting_update_controller');",1 );
                                        ?>
                              ...
                              <tr>
                                    ...
                                    <td width="170">
                                        <?
                                        echo create_drop_down( "cbo_source", 210, $knitting_source,"", 1, "-- Select Source --", $selected, "fnc_company_check(this.value);load_drop_down( 'requires/cutting_update_controller', this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_cutt_company', 'cutt_company_td' );dynamic_must_entry_caption(this.value);",0,'1,3' );
                                        ?>
                                    </td>
                              </tr>
                              <tr>
                               ...
                               <tr>
                                    ...
                                    <td>
                                        <input type="text" name="txt_wo_no" id="txt_wo_no" class="text_boxes" style="width:200px" placeholder="Browse" onDblClick="fnc_wo_no();" readonly disabled />
                                        <input type="hidden" name="txt_wo_id" id="txt_wo_id" class="text_boxes" style="width:50px" />
                                    </td>
                              </tr>
                        </table>
                        </fieldset>
                        <table><tr><td colspan="6" height="5"id="workorder_rate_td" style=" color:red; font-size:12px"></td></tr></table>
