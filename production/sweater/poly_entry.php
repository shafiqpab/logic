<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create print Poly Entry
...
<script>
	...
	{
		var title = 'Poly Info';	
		var page_link = 'requires/poly_entry_controller.php?action=system_number_popup';
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				load_drop_down( 'requires/poly_entry_controller', ex_data[2]+'_'+1, 'load_drop_down_location', 'location_td' );
				...
				load_drop_down('requires/poly_entry_controller', location, 'load_drop_down_floor', 'floor_td' );
				$('#cbo_floor').val(ex_data[7]);
				$('#txt_job_id').val(ex_data[8]);
				$('#txt_job_no').val(ex_data[9]);
				$('#txt_poly_date').val(ex_data[10]);
				load_drop_down( 'requires/poly_entry_controller', ex_data[7]+'_'+ex_data[3]+'_'+ex_data[10], 'load_drop_down_poly_line_floor', 'poly_line_td' );
				...
				get_php_form_data(ex_data[2], "production_process_control", "requires/poly_entry_controller" );
	...
	function fnc_poly_entry(operation)
	{
		...
		if(operation==4)
		{
			// var report_title=$("div.form_caption").html();
			 //print_report( $('#cbo_company_name').val()+'*'+master_ids+'*'+report_title, "emblishment_issue_print", "requires/poly_entry_controller" )
			// return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			if ( form_validation('cbo_company_name*cbo_location*cbo_source*cbo_poly_company*txt_job_no*txt_poly_date*cbo_poly_line*txt_reporting_hour','Company Name*Location*Source*Poly Company*Job No*Issue Date*Poly Line No*Reporting Hour')==false )
			{
				return;
			}
			else
			{
				if(source==1)
				{
					if ( form_validation('cbo_poly_location*cbo_floor','Poly Location*Floor')==false )
					{
						return;
					}
				}
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				if (theemail!="")
				{
					...
					get_php_form_data(cbo_company_name, "production_process_control", "requires/poly_entry_controller" );
					release_freezing();
				}
			}
		}
	...
	function fnc_dtls_data_load(job_no,uid)
	{
		...
		var list_view_orders = return_global_ajax_value( cbo_company_id+'***'+job_no+'***'+uid, 'order_details', '', 'requires/poly_entry_controller');
		if(list_view_orders!='')
		{
	...
	function fnc_load_party(type)
	{
		...
		if(source==1 && type==1)
		{
			load_drop_down( 'requires/poly_entry_controller', company+'_'+1, 'load_drop_down_working_com', 'poly_company_td' );
		}
		else if(source==3 && type==1)
		{
			load_drop_down( 'requires/poly_entry_controller', company+'_'+3, 'load_drop_down_working_com', 'poly_company_td' );
		}
		else if(source==1 && type==2)
		{
			load_drop_down( 'requires/poly_entry_controller', working_company+'_'+2, 'load_drop_down_location', 'working_location_td' ); 
		} 
	}
...
<div style="width:100%;">
    ...
    <div style="width:1050px; float:left" align="center">
 		<fieldset style="width:1050px;">
            <legend>Poly Info.</legend>
            <form name="polyentry_1" id="polyentry_1" method="" autocomplete="off" >
                <fieldset>
                    <table width="100%">
                        <tr>
                            <td colspan="4" align="right"><b>Poly NO : </b></td>
                        ...
                        <tr>
                            ...
                            <td width="140"><? echo create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "--Select--", $selected, "load_drop_down( 'requires/poly_entry_controller', this.value+'_'+1, 'load_drop_down_location', 'location_td'); fnc_load_party(1,document.getElementById('cbo_source').value);","" ); ?></td>
                            ...
                            <td width="100" class="must_entry_caption">Poly Company</td>
                            <td id="poly_company_td"><? echo create_drop_down( "cbo_poly_company", 130, $blank_array,"", 1, "-Poly Company-", $selected, "" ); ?></td>
                        </tr>
                        <tr>
                            <td id="locations">Poly Location</td>
                            ...
                            <td><input type="text" name="txt_job_no" id="txt_job_no" class="text_boxes" style="width:120px" onDblClick="openmypage_job('requires/poly_entry_controller.php?action=job_popup', 'Job/Order Selection Form');" placeholder="Browse" readonly /></td>
                            <td>Style Ref.</td>
                        ...
                        <tr>
                            ...
                            <td><input type="text" name="txt_poly_date" id="txt_poly_date" value="<? echo date("d-m-Y")?>" class="datepicker" style="width:120px;" onChange="load_drop_down( 'requires/poly_entry_controller',document.getElementById('cbo_floor').value+'_'+document.getElementById('cbo_location').value+'_'+this.value, 'load_drop_down_poly_line_floor', 'poly_line_td' );"  /></td>
                ...
                <fieldset style="width:1200px">
                    <legend>Poly Details List</legend>
                    <div>
                        <table cellpadding="0" width="1200" cellspacing="0" border="1" class="rpt_table" rules="all">
                            <thead>
                                <tr>
                                    ...
                                    <th width="70">Re-Poly Qty. (PCS)</th>
                                    <th>QC Pass Qty. (PCS)</th>
                                </tr>
                            </thead>
                        </table>
                        <div  style="width:1200px;max-height:250px;overflow-y:scroll"  align="left">    
                            <table cellpadding="0" width="1182" cellspacing="0" border="1" class="rpt_table" rules="all" id="tbl_item_details">      
