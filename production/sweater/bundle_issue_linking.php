<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Bundle Issue To Linking
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Bundle Issue To Linking","../../", 1, 1, $unicode,'','');
?>
...
<script>
	...
	{
		var challan_duplicate=return_ajax_request_value( barcode_no,"challan_duplicate_check", "requires/bundle_issue_linking_controller");
	 ...
	 $('#txt_bundle_no').live('keydown', function(e) {
        if (e.keyCode === 13)
        {
			if ( form_validation('cbo_company_name*cbo_working_company','Company Name*Working Company')==false )
			{
				return;
			}
	...
	function create_row(bundle_nos, vscan, hidden_source_cond)
    {
        ...
        var response_data = return_global_ajax_value(bundle_nos + "**" + row_num + "****" + $('#cbo_company_name').val() + "**" + vscan + "**" + hidden_source_cond, 'populate_bundle_data', '', 'requires/bundle_issue_linking_controller');
	...
	function openmypage_bundle(page_link,title)
	{
		...
		var page_link='	requires/bundle_issue_linking_controller.php?action=bundle_popup&company_id='+document.getElementById('cbo_company_name').value+'&bundleNo='+bundleNo;
	...
	function fnc_issue_linking_entry(operation)
	{
		if(operation==4)
		{
			var report_title=$( "div.form_caption" ).html();
			generate_report_file( $('#cbo_company_name').val()+'*'+$('#txt_system_no').val()+'*'+$('#txt_update_id').val()+'*'+report_title, 'bundle_issue_linking_print', 'requires/bundle_issue_linking_controller' );
			return;
		}
		
		if (form_validation('cbo_company_name*cbo_location*cbo_source*cbo_working_company*cbo_working_location*cbo_floor*txt_issue_date','LC Company*LC Location*Source*W. Company*WC. Location*Floor*Issue Date')==false )
		{
			return;
		}
		...
		if(operation==0 || operation==1 || operation==2)
		{
			...
			var data="action=save_update_delete&operation="+operation+'&tot_row='+j+get_submitted_data_string('txt_system_no*txt_update_id*cbo_company_name*cbo_location*cbo_source*cbo_working_company*cbo_working_location*cbo_floor*txt_issue_date*txt_challan_no*txt_job_no*txt_remarks*garments_nature',"../../")+dataString;
			
			freeze_window(operation);
			//alert(data); return;
			http.open("POST","requires/bundle_issue_linking_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
			http.onreadystatechange = fnc_issue_linking_entry_reply_info;
		}
	}
  
	function fnc_issue_linking_entry_reply_info()
	{
		...
		{		
			...
			if(reponse[0]==15) 
			{ 
				 setTimeout('fnc_issue_linking_entry('+ reponse[1]+')',4000); 
			}
			else if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
			{
				...
				set_button_status(1, permission, 'fnc_issue_linking_entry',1,1);	
			}
			if(reponse[0]!=15)
			{
			  release_freezing();
	...
	{
		window.open("requires/bundle_issue_linking_controller.php?data=" + data+'&action='+action, true );
	}
	...
	function openmypage_sysNo()
	{
		...
		var page_link = 'requires/bundle_issue_linking_controller.php?action=system_number_popup';
			
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link+'&cbo_company_name='+cbo_company_name, title, 'width=1020px,height=390px,center=1,resize=0,scrolling=0','../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0];
			...
			{ 
				...
				fnc_load_party(1);
				load_drop_down( 'requires/bundle_issue_linking_controller', ex_data[2]+'_'+1, 'load_drop_down_location', 'location_td' );
				...
				load_drop_down('requires/bundle_issue_linking_controller', location, 'load_drop_down_floor', 'floor_td' );
				$('#cbo_floor').val(ex_data[7]);
				$('#txt_issue_date').val(ex_data[8]);
				$('#txt_challan_no').val(ex_data[9]);
				$('#txt_remarks').val(ex_data[10]);
	
				var response_data = return_global_ajax_value(ex_data[0]+"**"+ ex_data[2], 'populate_bundle_data_update', '', 'requires/bundle_issue_linking_controller');
				...
				var tot_row=$('#tbl_details tbody tr').length; 
				$('#hidden_row_number').val(tot_row);
				set_button_status(1, permission, 'fnc_issue_linking_entry',1,0);
				release_freezing();
			}
		}
	}//end function
	...
	function pageReset()
	{
		// document.getElementById('txt_system_no').value='';
		location.reload();
	}
	...
	function fnc_load_party(type)
	{
		...
		if(source==1 && type==1)
		{
			load_drop_down( 'requires/bundle_issue_linking_controller', company+'_'+1, 'load_drop_down_working_com', 'working_com' );
		}
		else if(source==2 && type==1)
		{
			load_drop_down( 'requires/bundle_issue_linking_controller', company+'_'+2, 'load_drop_down_working_com', 'working_com' );
		}
		else if(source==1 && type==2)
		{
			load_drop_down( 'requires/bundle_issue_linking_controller', working_company+'_'+2, 'load_drop_down_location', 'working_location_td' ); 
		} 
	}
	...
	function location_select()
	{
		...
		if($('#cbo_working_location option').length==2)
		{
			if($('#cbo_working_location option:first').val()==0)
			{
				...
				load_drop_down('requires/bundle_issue_linking_controller', $('#cbo_working_location option:last').val(), 'load_drop_down_floor', 'floor_td' );
			}
		}
		...
		if($('#cbo_floor option').length==2)
		{
			...
			{
				$('#cbo_floor').val($('#cbo_floor option:last').val());
			}
		}
		else if($('#cbo_floor option').length==1)
		{
			$('#cbo_floor').val($('#cbo_floor option:last').val());
		}	
	}
...
<body onLoad="set_hotkey()">
 <div style="width:100%;">
    ...
    <div style=" float:left" align="left"> 
        ...
        <form name="bundleissuelinking_1" id="bundleissuelinking_1" method="" autocomplete="off" >
			<fieldset style="width:1050px;">
        		<legend>Master Part</legend>
                <table width="1050">
                    <tr>
                        <td align="right" colspan="4"><b>ISSUE NO</b></td>
                        <td colspan="4"> 
                    ...
                    <tr>
                        ...
                        <td width="140"><? echo create_drop_down( "cbo_company_name",130, "select id, company_name from lib_company comp  where status_active =1 and  is_deleted=0  $company_cond  order by company_name","id,company_name", 1, "-- Select --", $selected,"load_drop_down( 'requires/bundle_issue_linking_controller', this.value+'_'+1, 'load_drop_down_location', 'location_td' ); location_select(); fnc_load_party(1);","" ); ?></td>
                    ...
                    <tr>
                     ...
                     <tr>
                        <td>Remarks</td>
                        <td colspan="7"><input name="txt_remarks" id="txt_remarks" class="text_boxes" type="text" style="width:850px" /></td>
                     </tr>
                </table>
			</fieldset><br />
                ...
                <table cellpadding="0" cellspacing="1" width="1100">
                    <tr>
                        <td align="center" colspan="9" valign="middle" class="button_container">
                            <?
                                $date=date('d-m-Y');
                                echo load_submit_buttons( $permission, "fnc_issue_linking_entry", 0,1 , "reset_form('bundleissuelinking_1','dtls_body','', 'txt_issue_date,".$date."','pageReset();')",1); 
                            ?>
...
<script>fnc_load_party(1);</script>
