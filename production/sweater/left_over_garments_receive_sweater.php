...
<script>
	...
	function fnc_left_over_gmts_input_Reply_info()
	{
	 	if(http.readyState == 4) 
		{
			...
			{ 
				...
				show_list_view(response[1]+'**'+country_maintain+'**'+leftover_source,'show_dtls_listview_system','list_view_container','requires/left_over_garments_receive_sweater_controller','setFilterGrid(\'tbl_list_search\',-1)');	
				show_list_view(po_id+'**'+company_name+'**'+location_name+'**'+order_type+'**'+buyer_name+'**'+goods_type+'**'+working_company_name+'**'+working_location_name+'**'+country_maintain+'**'+leftover_source,'show_country_listview','list_view_country','requires/left_over_garments_receive_sweater_controller');
			...
			else if(response[0]==2)
			{
				...
				show_list_view(response[1]+'**'+country_maintain+'**'+leftover_source,'show_dtls_listview_system','list_view_container','requires/left_over_garments_receive_sweater_controller','setFilterGrid(\'tbl_list_search\',-1)');
				show_list_view(response[4]+'**'+company_name+'**'+location_name+'**'+order_type+'**'+buyer_name+'**'+goods_type+'**'+working_company_name+'**'+working_location_name,'show_country_listview','list_view_country','requires/left_over_garments_receive_sweater_controller');
	...
	function openmypage( )
	{
		...
		var page_link = 'requires/left_over_garments_receive_sweater_controller.php?action=order_popup&company='+$('#cbo_company_name').val()+'&location_name='+$('#cbo_location_name').val()+'&order_type='+$('#cbo_order_type').val()+'&goods_type='+$('#cbo_goods_type').val()+'&garments_nature='+$('#garments_nature').val()+'&buyer_name='+$('#cbo_buyer_name').val()+'&store_name='+$('#cbo_store_name').val()+'&working_company_name='+$('#cbo_working_company_name').val()+'&working_location_name='+$('#cbo_working_location_name').val()+'&country_maintain_variable='+$('#country_maintain_variable').val()+'&leftover_source='+$('#leftover_source').val()+'&sewing_production_variable='+$('#sewing_production_variable').val()+'&wo_company_and_location_mandatory='+$('#wo_company_and_location_mandatory').val();
		...
		emailwindow.onclose=function()
		{
			...
			get_php_form_data(company_id,'load_variable_settings','requires/left_over_garments_receive_sweater_controller');			
			...
			{				
				...
				get_php_form_data(po_id+'**'+country_id+'**'+company_id+'**'+gmts_item+'**'+$('#cbo_goods_type').val()+'**'+variableSettings+'**'+$('#cbo_order_type').val()+'**'+location_name+'**'+$('#garments_nature').val()+'**'+country_maintain+'**'+leftover_source+'**'+working_company_name+'**'+working_location_name+'**'+wo_com_loc_mandatory, "color_and_size_level_left_over", "requires/left_over_garments_receive_sweater_controller" );

				var goodsType = document.getElementById('cbo_goods_type').value;

				if(goodsType != 1) {
					show_list_view(po_id+'**'+gmts_item+'**'+country_id+'**'+company_id+'**'+country_maintain+'**'+leftover_source,'show_dtls_listview','list_view_container','requires/left_over_garments_receive_sweater_controller','setFilterGrid(\'tbl_list_search\',-1)');
				}
				show_list_view(po_id+'**'+company_name+'**'+location_name+'**'+order_type+'**'+buyer_name+'**'+goods_type+'**'+working_company_name+'**'+working_location_name+'**'+country_maintain+'**'+leftover_source,'show_country_listview','list_view_country','requires/left_over_garments_receive_sweater_controller');
	...
	function put_country_data(po_id, item_id, country_id, lft_qnty)
	{
		...
		get_php_form_data(po_id+'**'+item_id+'**'+country_id+'**'+variableSettingsCountry+'**'+leftover_source, "populate_data_from_search_popup", "requires/left_over_garments_receive_sweater_controller" );
		...
		if(goodsType != 1) {
			show_list_view(po_id+'**'+item_id+'**'+countryId+'**'+company_id+'**'+leftover_source,'show_dtls_listview','list_view_container','requires/left_over_garments_receive_sweater_controller','setFilterGrid(\'tbl_list_search\',-1)');
		}
	...
	function get_left_over_balance(po_id,country_id,gmts_item,company,location,wo_company,wo_location,country_maintain,leftover_source)
	{
		...
		$.ajax({
			type:'POST',
			url: 'requires/left_over_garments_receive_sweater_controller.php',
			data: data,
			cache: false,
	...
	function system_number_popup()
	{
		...
		{*/
			...
			var page_link = 'requires/left_over_garments_receive_sweater_controller.php?action=system_number_popup&company='+$('#cbo_company_name').val()+'&location_name='+$('#cbo_location_name').val()+'&leftover_date='+$('#txt_leftover_date').val()+'&order_type='+$('#cbo_order_type').val()+'&goods_type='+$('#cbo_goods_type').val();
			...
			emailwindow.onclose=function()
			{
				...
				get_php_form_data(responseDataArr[0], "populate_mst_form_data", "requires/left_over_garments_receive_sweater_controller" );

				var country_maintain = $("#country_maintain_variable").val();
				var leftover_source = $("#leftover_source").val();
				
				show_list_view(responseDataArr[0]+'**'+country_maintain+'**'+leftover_source,'show_dtls_listview_system','list_view_container','requires/left_over_garments_receive_sweater_controller','setFilterGrid(\'tbl_list_search\',-1)');
	...
	function check_exchange_rate(curr_id)
	{
		...
		var response=return_global_ajax_value( cbo_currercy+"**"+booking_date, 'check_conversion_rate', '', 'requires/left_over_garments_receive_sweater_controller');
	...
	function populate_selected_data(dtls_id,mst_id,po_id,country_id,item_id,variableSettings)
	{
		...
		//{ 
			...
			show_list_view(po_id+'**'+company_name+'**'+location_name+'**'+order_type+'**'+buyer_name+'**'+goods_type+'**'+working_company_name+'**'+working_location_name+'**'+country_maintain_variable+'**'+leftover_source+'**'+wo_com_loc_mandatory,'show_country_listview','list_view_country','requires/left_over_garments_receive_sweater_controller');
	...
	function left_over_gmts_receive_print()
	{
		...
		else
		{
			...
			print_report( $('#cbo_company_name').val()+'*'+$('#txt_mst_id').val()+'*'+report_title+'*'+$('#txt_system_no').val()+'*'+$('#cbo_order_type').val()+'*'+rowCount+receiveAmounts, "left_over_gmts_receive_print", "requires/left_over_garments_receive_sweater_controller");
		}
  				...
  				<fieldset>
                    <table width="100%" border="0">
                        ...
                        <tr>
                            ...
                            <td width="180">                                
                            <? 
                            echo create_drop_down( "cbo_company_name", 170, "select company_name,id from lib_company comp where is_deleted=0  and status_active=1 $company_cond  order by company_name",'id,company_name', 1, '--- Select Company ---', 0, "load_drop_down( 'requires/left_over_garments_receive_sweater_controller', this.value, 'load_drop_down_location', 'location' );load_drop_down( 'requires/left_over_garments_receive_sweater_controller',this.value, 'load_drop_down_store_name', 'store_name_td');get_php_form_data(this.value,'load_variable_settings','requires/left_over_garments_receive_sweater_controller');" );
                            ?>	
                        ...
                        <tr>
                            ...
                            <!-- <td width="110" class="must_entry_caption">Working Floor</td>
