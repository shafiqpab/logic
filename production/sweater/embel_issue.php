<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create print Embel. Issue
...
<script>
	...
	<?php
	if($_SESSION['logic_erp']['data_arr'][326]){
		echo "var field_level_data= " . json_encode($_SESSION['logic_erp']['data_arr'][326]). ";\n";
	}
	?>
	...
	{ 
		var title = 'Embellishment Issue Info';	
		var page_link = 'requires/embel_issue_controller.php?action=system_number_popup';
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				load_drop_down( 'requires/embel_issue_controller', ex_data[2]+'_'+1, 'load_drop_down_location', 'location_td' );
				...
				load_drop_down('requires/embel_issue_controller', location, 'load_drop_down_floor', 'floor_td' );
				$('#cbo_floor').val(ex_data[7]);
				$('#txt_job_id').val(ex_data[8]);
				$('#txt_job_no').val(ex_data[9]);
				$('#txt_issue_date').val(ex_data[10]);
				load_drop_down( 'requires/embel_issue_controller', ex_data[9], 'load_drop_down_embl_name', 'embel_name_td' );
				$('#cbo_embel_name').val(ex_data[11]);
				load_drop_down( 'requires/embel_issue_controller', ex_data[11]+'**'+ex_data[9], 'load_drop_down_embro_issue_type', 'embro_type_td');
				...
				set_button_status(1, permission, 'fnc_embel_issue_entry',1,0);
				release_freezing();
			}
		}
	}//end function
	
	function fnc_embel_issue_entry(operation)
	{
		if(operation==2)
		{
		...
		if(operation==4)
		{
			var report_title=$("div.form_caption").html();
			 print_report( $('#cbo_company_name').val()+'*'+$('#txt_update_id').val()+'*'+report_title, "emblishment_issue_print", "requires/embel_issue_controller" )
			return;
		}
		
		else if(operation==0 || operation==1 || operation==2)
		{  
			if ( form_validation('cbo_company_name*cbo_location*cbo_source*cbo_emb_company*txt_job_no*txt_issue_date*cbo_embel_name*cbo_embel_type','Company Name*Location*Source*Embel.Company*Job No*Issue Date*Embel. Name* Embel. Type')==false )
			{
				return;
			}
			else
			{
				...
				var data="action=save_update_delete&operation="+operation+'&tot_row='+k+get_submitted_data_string('garments_nature*txt_system_no*cbo_company_name*cbo_location*cbo_source*cbo_emb_company*cbo_emb_location*cbo_floor*txt_job_no*txt_style_ref*cbo_buyer_name*txt_issue_date*cbo_embel_name*cbo_embel_type*cbo_sending_location*txt_challan*txt_remark*txt_job_id*txt_update_id*txt_wo_id',"../../")+data_str;
				// alert (data); return;
				freeze_window(operation);
				http.open("POST","requires/embel_issue_controller.php",true);
				http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
				http.send(data);
				http.onreadystatechange = fnc_embel_issue_entry_reply_info;
			}
		}
	}
	
	function fnc_embel_issue_entry_reply_info()
	{
		if(http.readyState == 4)
		{
			...
			if(reponse[0]==15) 
			{ 
				 setTimeout('fnc_embel_issue_entry('+ reponse[1]+')',4000); 
			}
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				if (theemail!="")
				{
					...
					load_drop_down( 'requires/embel_issue_controller', ex_data[1], 'load_drop_down_embl_name', 'embel_name_td' );
	...
	function fnc_dtls_data_load(job_no,embl_name,embl_type,uid)
	{
		...
		var list_view_orders = return_global_ajax_value( cbo_company_id+'***'+job_no+'***'+embl_name+'***'+embl_type+'***'+uid, 'order_details', '', 'requires/embel_issue_controller');
		if(list_view_orders!='')
		{
	...
	function fnc_load_party(type)
	{
		...
		if(source==1 && type==1)
		{
			load_drop_down( 'requires/embel_issue_controller', company+'_'+1, 'load_drop_down_working_com', 'emb_company_td' );
		}
		else if(source==3 && type==1)
		{
			load_drop_down( 'requires/embel_issue_controller', company+'_'+3, 'load_drop_down_working_com', 'emb_company_td' );
		}
		else if(source==1 && type==2)
		{
			load_drop_down( 'requires/embel_issue_controller', working_company+'_'+2, 'load_drop_down_location', 'working_location_td' ); 
		} 
	}
	function openmypage_woNo()
	{
			...
			{			
				...
				var page_link='requires/embel_issue_controller.php?company_id='+cbo_company_id+'&cbo_service_source='+cbo_service_source+'&txt_job_no='+txt_job_no+'&cbo_embel_name='+cbo_embel_name+'&action=work_order_popup';
...
<div style="width:100%;">
    ...
    <div style="width:1050px; float:left" align="center">
 		<fieldset style="width:1050px;">
            <legend>Embel. Issue Info.</legend>
            <form name="printembro_1" id="printembro_1" method="" autocomplete="off" >
                <fieldset>
                    <table width="100%">
                        ...
                        <tr>
                            ...
                            <td width="140"><? echo create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "--Select--", $selected, "load_drop_down( 'requires/embel_issue_controller', this.value+'_'+1, 'load_drop_down_location', 'location_td'); fnc_load_party(1,document.getElementById('cbo_source').value);setFieldLevelAccess(this.value);","" ); ?></td>
                        ...
                        <tr>
                            ...
                            <td><input type="text" name="txt_job_no" id="txt_job_no" class="text_boxes" style="width:120px" onDblClick="openmypage_job('requires/embel_issue_controller.php?action=job_popup', 'Job/Order Selection Form');" placeholder="Browse" readonly /></td>
                            <td>Style Ref.</td>
                        ...
                        <tr>
                            ...
                            <td id="embel_name_td"><? echo create_drop_down( "cbo_embel_name", 130, $emblishment_name_array,"", 1, "-Select Embel.Name-", $selected, "load_drop_down( 'requires/embel_issue_controller', this.value+'**'+document.getElementById('txt_job_no').value, 'load_drop_down_embro_issue_type', 'embro_type_td');","","1,2,4,5,99" ); ?></td>
                ...
                <fieldset style="width:1050px">
                    <legend>Embel. Issue Details List</legend>
                    <div>
                    ...
                    <table cellpadding="0" cellspacing="1" width="1050">
                        <tr>
                            <td align="center" colspan="14" valign="middle" class="button_container">
                                <?
                                    $date=date('d-m-Y');
                                    echo load_submit_buttons( $permission, "fnc_embel_issue_entry", 0,1 , "reset_form('printembro_1','','', 'txt_issue_date,".$date."','')",1); 
                                ?>
