<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Get Up Complete V2
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Get Up Complete V2","../../", 1, 1, $unicode,'','');
...
<script>
	...
	{
		var title = 'Get Up Entry Info';	
		var page_link = 'requires/getup_complete_v2_controller.php?action=system_number_popup';
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				load_drop_down( 'requires/getup_complete_v2_controller', ex_data[2]+'_'+1, 'load_drop_down_location', 'location_td' );
				...
				load_drop_down('requires/getup_complete_v2_controller', location, 'load_drop_down_floor', 'floor_td' );
				$('#cbo_floor').val(ex_data[7]);
				$('#txt_job_id').val(ex_data[8]);
				$('#txt_job_no').val(ex_data[9]);
				$('#txt_getup_date').val(ex_data[10]);
				...
				get_php_form_data(ex_data[2], "production_process_control", "requires/getup_complete_v2_controller" );
				
				set_button_status(1, permission, 'fnc_getup_entry',1,0);
				release_freezing();
			}
		}
	}//end function
	
	function fnc_getup_entry(operation)
	{
		...
		if(operation==4)
		{
			...
			//print_report( $('#cbo_company_name').val()+'*'+master_ids+'*'+report_title, "emblishment_issue_print", "requires/getup_complete_v2_controller" );
			//release_freezing();
			//return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			if ( form_validation('cbo_company_name*cbo_location*cbo_source*cbo_working_company*txt_job_no*txt_getup_date*txt_reporting_hour','Company Name*Location*Source*Working Company*Job No*Get Up Date*Reporting Hour')==false )
			{
				release_freezing();
				return;
			}
			else
			{
				if(source==1)
				{
					if ( form_validation('cbo_working_location*cbo_floor','Working Location*Floor')==false )
					{
						release_freezing();
						return;
					}
				...
				if(date_compare($('#txt_getup_date').val(), current_date)==false)
				{
					alert("Get Up Date Can not Be Greater Than Current Date");
					release_freezing();
					return;
				}
				...
				var data="action=save_update_delete&operation="+operation+'&tot_row='+k+get_submitted_data_string('garments_nature*txt_system_no*cbo_company_name*cbo_location*cbo_source*cbo_working_company*cbo_working_location*cbo_floor*txt_job_no*txt_style_ref*cbo_buyer_name*txt_getup_date*txt_reporting_hour*txt_challan*txt_remark*txt_job_id*txt_update_id',"../../")+data_str;
				//alert (data); return;
				
				http.open("POST","requires/getup_complete_v2_controller.php",true);
				http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
				http.send(data);
				http.onreadystatechange = fnc_getup_entry_reply_info;
			}
		}
	}
	
	function fnc_getup_entry_reply_info()
	{
		if(http.readyState == 4)
		{
			...
			if(reponse[0]==15) 
			{ 
				 setTimeout('fnc_getup_entry('+ reponse[1]+')',4000); 
			}
	...
	function fnc_total_calculate(value,i)
	{
		...
		if(((value*1)+pre_iss_qty)>placeholder_value)
		{
			...
			{
				var confirm_value=confirm("Get Up Qty Excceded by Plan Qty. (PCS). Press cancel to proceed otherwise press ok.");
				if(confirm_value!=0)
				{
					$("#txtQty_"+i).val('');
				}	
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				if (theemail!="")
				{
					...
					get_php_form_data(cbo_company_name, "production_process_control", "requires/getup_complete_v2_controller" );
					release_freezing();
				}
			}
		}
	...
	function fnc_dtls_data_load(job_no,uid)
	{
		...
		var list_view_orders = return_global_ajax_value( cbo_company_id+'***'+job_no+'***'+uid, 'order_details', '', 'requires/getup_complete_v2_controller');
		if(list_view_orders!='')
		{
	...
	function fnc_load_party(type)
	{
		...
		if(source==1 && type==1)
		{
			load_drop_down( 'requires/getup_complete_v2_controller', company+'_'+1, 'load_drop_down_working_com', 'working_company_td' );
		}
		else if(source==3 && type==1)
		{
			load_drop_down( 'requires/getup_complete_v2_controller', company+'_'+3, 'load_drop_down_working_com', 'working_company_td' );
		}
		else if(source==1 && type==2)
		{
			load_drop_down( 'requires/getup_complete_v2_controller', working_company+'_'+2, 'load_drop_down_location', 'working_location_td' ); 
		} 
	}
...
<div style="width:100%;">
    ...
    <div style="width:1050px; float:left" align="center">
 		<fieldset style="width:1050px;">
            <legend>Get Up Complete Info.</legend>
            <form name="getupentry_1" id="getupentry_1" method="" autocomplete="off" >
                <fieldset>
                    <table width="100%">
                        <tr>
                            <td colspan="4" align="right"><b>Get Up NO : </b></td>
                            <td colspan="4"><input name="txt_system_no" id="txt_system_no" class="text_boxes" type="text" style="width:120px" onDblClick="openmypage_sysNo();" placeholder="Browse" readonly/>
            					...
            					<input type="hidden" id="txt_user_lebel" value="<?=$level; ?>" />
                            </td>
                        </tr>
                        <tr>
                            <td width="100" class="must_entry_caption">Lc. Company</td>
                            <td width="140"><?=create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "--Select--", $selected, "load_drop_down( 'requires/getup_complete_v2_controller', this.value+'_'+1, 'load_drop_down_location', 'location_td'); fnc_load_party(1,document.getElementById('cbo_source').value);","" ); ?></td>
                            <td width="100" class="must_entry_caption">Lc. Com. Location</td>
                            <td width="140" id="location_td"><?=create_drop_down( "cbo_location", 130, $blank_array,"", 1, "--Select Location--", $selected, "" ); ?></td>
                            <td width="100" class="must_entry_caption">Source</td>
                            <td width="140"><?=create_drop_down("cbo_source", 130, $knitting_source,"", 1, "--Select Source--", $selected, "fnc_load_party(1,this.value); dynamic_must_entry_caption(this.value);", 0, '1,3'); ?></td>
                            <td width="100" class="must_entry_caption">W. Company</td>
                            <td id="working_company_td"><?=create_drop_down( "cbo_working_company", 130, $blank_array,"", 1, "-W. Company-", $selected, ""); ?></td>
                        </tr>
                        <tr>
                            <td id="locations">WC Location</td>
                            <td id="working_location_td"><?=create_drop_down( "cbo_working_location", 130, $blank_array,"", 1, "-- Select --", $selected, ""); ?></td>
                            <td id="floors">Floor</td>
                            <td id="floor_td"><?=create_drop_down( "cbo_floor", 130, $blank_array,"", 1, "--Select Floor--", $selected, ""); ?></td>
                            <td class="must_entry_caption">Job No</td>
                            <td><input type="text" name="txt_job_no" id="txt_job_no" class="text_boxes" style="width:120px" onDblClick="openmypage_job('requires/getup_complete_v2_controller.php?action=job_popup', 'Job/Order Selection Form');" placeholder="Browse" readonly /></td>
                            <td>Style Ref.</td>
                            <td><input type="text" name="txt_style_ref" id="txt_style_ref" class="text_boxes" style="width:120px" disabled /></td>
                        </tr>
                        <tr>
                        	<td>Buyer Name</td>
                            <td><?=create_drop_down( "cbo_buyer_name", 130, "select id, buyer_name from lib_buyer where is_deleted=0 and status_active=1 order by buyer_name","id,buyer_name", 1, "--Select Buyer--", $selected, "",1,0); ?></td>
                            <td class="must_entry_caption">Get Up Date</td>
                            <td><input type="text" name="txt_getup_date" id="txt_getup_date" value="<?=date("d-m-Y")?>" class="datepicker" style="width:120px;" /></td>
                        ...
                        <tr>
                            <td>Remarks</td>
                            <td colspan="7"><input type="text" name="txt_remark" id="txt_remark" class="text_boxes" style="width:605px" title="450 Characters Only." /></td>
                        </tr>
                    </table>
                </fieldset><br />
                    
                <fieldset style="width:1200px">
                    <legend>Get Up Complete Details List</legend>
                    <div>
                        <table cellpadding="0" width="1200" cellspacing="0" border="1" class="rpt_table" rules="all">
                            <thead>
                                <tr>
                                    ...
                                    <th width="70">ReGetUp Qty. [PCS]</th>
                    ...
                    <table cellpadding="0" cellspacing="1" width="1200">
                        <tr>
                            <td align="center" colspan="18" valign="middle" class="button_container">
                                <?
                                    $date=date('d-m-Y');
                                    echo load_submit_buttons( $permission, "fnc_getup_entry", 0,0 , "reset_form('getupentry_1','','', 'txt_getup_date,".$date."','')",1); 
                                ?>
