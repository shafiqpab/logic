...
if ($action=="load_drop_down_location")
{
	...
	else { $dropdown_name="cbo_working_location";  $load_fnc="load_drop_down('requires/bundle_issue_to_first_inspection_controller', this.value, 'load_drop_down_floor', 'floor_td' );"; }
...
{
    echo create_drop_down("cbo_floor", 110, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process=5 order by floor_name", "id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/bundle_issue_to_first_inspection_controller', $('#cbo_working_company').val()+'_'+$('#cbo_working_location').val()+'_'+this.value+'_'+$('#txt_issue_date').val(), 'load_drop_down_line', 'line_td' );", 0);
    exit();
}
...
if ($action == "populate_bundle_data") 
{
    ...
    $scanned_bundle_arr = return_library_array("select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=76 and b.status_active=1 and b.is_deleted=0 $scnbundle_nos_cond", 'bundle_no', 'bundle_no');
	...
	{ 
		...
		if(trim($scanned_bundle_arr[$row[csf('bundle_no')]])=="")
		{
			...
			<tr bgcolor="<?=$bgcolor; ?>" id="tr_<?=$i;?>"> 
                ...
                <td width="70" style="word-break:break-all;" align="center"><?=$company_short_arr[$row[csf('serving_company')]]; ?></td>
				<td width="90" style="word-break:break-all;" align="center"><?=$floorArr[$row[csf('floor_id')]]; ?></td>
...
if($action=="populate_mst_data")
{
    ...
    where a.production_type=51 and b.production_type=51 and a.id=b.delivery_mst_id and b.barcode_no='$data[0]' and a.company_id='$data[1]' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	$sql_pop_res=sql_select($sql_pop);
	foreach($sql_pop_res as $row)
    {
		echo $row[csf('company_id')].'_'.$row[csf('location_id')].'_'.$row[csf('production_source')].'_'.$row[csf('working_company_id')].'_'.$row[csf('working_location_id')].'_'.$row[csf('floor_id')].'_'.$row[csf('production_source')];
	}
	exit();
}
            ...
            <fieldset style="width:810px;">
                ...
                <table cellpadding="0" cellspacing="0" width="550" border="1" rules="all" class="rpt_table">
                    ...
                    <tr class="general">
                        ...
                        <td>
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view(document.getElementById('txt_order_no').value+'_'+'<? echo $company_id; ?>'+'_'+document.getElementById('txtbundle_no').value+'_'+'<? echo $bundleNo; ?>'+'_'+document.getElementById('txt_job_no').value+'_'+document.getElementById('cbo_job_year').value+'_'+document.getElementById('txt_style_ref').value+'_'+document.getElementById('txt_lot_ratio_no').value+'_'+document.getElementById('txt_qr_code').value+'_'+$('#is_exact').is(':checked'), 'create_bundle_search_list_view', 'search_div', 'bundle_issue_to_first_inspection_controller', 'setFilterGrid(\'tbl_list_search\',-1);reset_hide_field();');" style="width:100px;"/>
                        </td>
                    </tr>
                </table>
...
if ($action == "create_bundle_search_list_view") 
{
   ...
   // $scanned_bundle_arr = return_library_array("select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=55 and  b.status_active=1 and b.is_deleted=0", 'bundle_no', 'bundle_no');
    ...
    $scanned_bundle_arr = return_library_array("select bundle_no, bundle_no from pro_garments_production_dtls where production_type=76 and cut_no='".$tmp_cut."' and status_active=1 and is_deleted=0", 'bundle_no', 'bundle_no');
    foreach (explode(",", $selectedBuldle) as $bn) {
        $scanned_bundle_arr[$bn] = $bn;
    }

    $scanne=sql_select( "select b.bundle_no, sum(b.production_qnty) as production_qnty,a.sewing_line from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=76  and b.status_active=1 and b.is_deleted=0 $cutCon_a group by b.bundle_no,a.sewing_line");
    ...
    // echo $cutting_no;
    $last_operation=gmt_production_validation_script( 76, 1,'', $cutting_no, $production_squence);
    //$last_operation=gmt_production_validation_script( 4, 1 );
    ...
    <div style="width:850px; max-height:210px; overflow-y:scroll" id="list_container_batch" align="left">    
        ...
        <?
            ...
            foreach($last_operation as  $item_id=>$operation_cond)
            {
				...
				$qrCond $orderCon $bndlCon $yearCond $jobCon $cutCon $styleCond
				and a.status_active=1 and a.is_deleted=0 and a.production_type=51 group by c.cut_no, c.bundle_no, d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.bundle_no, e.po_number,c.barcode_no order by c.cut_no, length(c.bundle_no) asc, c.bundle_no asc";
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
        ...
        $preIssueRec_sql="select c.barcode_no, c.bundle_no, c.production_type from pro_garments_production_mst a, pro_garments_production_dtls c where a.id=c.mst_id and a.production_type in (76,77) and c.bundle_no in ($bundle) and c.production_type in (76,77) and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 "; //and (c.is_rescan=0 or c.is_rescan is null)
        $preIssueRec_sql_result = sql_select($preIssueRec_sql); $duplicate_bundle=array(); $bundleRecArr=array();
        foreach ($preIssueRec_sql_result as $row)
        {
			if($row[csf('production_type')]==76)//Previous Issue      
            	$duplicate_bundle[$row[csf('barcode_no')]]=$row[csf('bundle_no')];
			else if($row[csf('production_type')]==77)//Insp Rec
				$bundleRecArr[$row[csf('barcode_no')]]=$row[csf('bundle_no')];
        }
		unset($preIssueRec_sql_result);
		...
		if (str_replace("'", "", $txt_update_id) == "")
		{
			...
			$new_sys_number = explode("*", return_next_id_by_sequence("", "pro_gmts_delivery_mst",$con,1,$cbo_company_name,'BIFI',0,date("Y",time()),0,0,76,0,0 ));
			$field_array_delivery = "id, sys_number_prefix, sys_number_prefix_num, sys_number, company_id, production_type, production_source, delivery_date, challan_no, remarks, entry_form, inserted_by, insert_date, status_active, is_deleted";
			...
			$data_array_delivery="(".$mst_id.",'".$new_sys_number[1]."','".(int)$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",76,".$cbo_source.",".$txt_issue_date.",'".$challanNo."',".$txt_remarks.",375,".$user_id.",'".$pc_date_time."',1,0)";
		...
		{ 	
			...
			if($duplicate_bundle[$$bundleNo]=='' && $bundleRecArr[$$bundleNo]=="")
            {
		...
		foreach($mstArr as $orderId=>$orderData)
		{
			foreach($orderData as $gmtsItemId=>$gmtsItemIdData)
			{
				foreach($gmtsItemIdData as $countryId=>$qty)
				{
					...
					$data_array_mst.="(".$id.",".$mst_id.",".$garments_nature.",".$cbo_company_name.",".$cbo_source.",".$txt_issue_date.",'".$challan_no."',".$txt_remarks.",".$orderId.", ".$gmtsItemId.",".$countryId.",".$qty.",76,3,375,".$user_id.",'".$pc_date_time."',1,0)";
		...
		foreach($dtlsArr as $bundle_no=>$qty)
		{
			...
			$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",76,'".$dtlsArrColorSize[$bundle_no]."','".$qty."','".$cutNoArr[$bundle_no]."','".$bundle_no."',375,'".$bundleBarcodeArr[$bundle_no]."',1,0)"; 
		}
		$flag=1;
		if (str_replace("'", "", $txt_update_id) == "")
		{
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$preIssueRec_sql="select c.delivery_mst_id, c.barcode_no, c.bundle_no, c.production_type from pro_garments_production_mst a, pro_garments_production_dtls c where a.id=c.mst_id and a.production_type in (76,77) and c.bundle_no in ($bundle) and c.production_type in (76,77) and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 "; //and (c.is_rescan=0 or c.is_rescan is null)
        $preIssueRec_sql_result = sql_select($preIssueRec_sql); $duplicate_bundle=array(); $bundleRecArr=array();
        foreach ($preIssueRec_sql_result as $row)
        {
			if($row[csf('production_type')]==76 && $row[csf('delivery_mst_id')]!=$mst_id)//Previous Issue      
            	$duplicate_bundle[$row[csf('barcode_no')]]=$row[csf('bundle_no')];
			if($row[csf('production_type')]==77)//Insp Rec
				$bundleRecArr[$row[csf('barcode_no')]]=$row[csf('bundle_no')];
        }
		unset($preIssueRec_sql_result);
		//echo "10**"; print_r($bundleRecArr); die;
		
       /* $receive_sql="select c.barcode_no, c.bundle_no from pro_garments_production_mst a, pro_garments_production_dtls c where a.id=c.mst_id and a.production_type=76 and c.bundle_no in ($bundle) and c.production_type=76 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and a.delivery_mst_id!=$mst_id and c.delivery_mst_id!=$mst_id)"; 
		...
		$issue_data_arr=production_data($mst_id,76);
		...
		$flag=1;
		$delete = execute_query("update pro_garments_production_mst set status_active=0,is_deleted=1,updated_by=$user_id,update_date='".$pc_date_time."' WHERE delivery_mst_id=$mst_id and production_type=76 and status_active=1 and is_deleted=0");
		if($delete==1 && $flag==1) $flag=1; else $flag=0;
		$delete_dtls = execute_query("update pro_garments_production_dtls set status_active=0, is_deleted=1 WHERE delivery_mst_id=$mst_id and production_type=76 and status_active=1 and is_deleted=0");
...
{
  	echo load_html_head_contents("Bundle Issue To First Inspection Info","../../../", 1, 1, '','1','');
	extract($_REQUEST);
	?>
	<script>
    ...
    <table width="840" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
        <thead>
            <tr>                	 
        ...
        <tbody>
            <tr class="general">                    
                ...
                <td><input name="txt_qr_code" id="txt_qr_code" class="text_boxes" style="width:70px" placeholder="Write" /></td>
                <td>
                <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('txt_issue_no').value+'_'+document.getElementById('txt_job_search').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_order_search').value+'_'+document.getElementById('cbo_job_year').value+'_'+document.getElementById('txt_style_search').value+'_'+document.getElementById('txt_qr_code').value, 'create_system_search_list_view', 'search_div', 'bundle_issue_to_first_inspection_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" />				
                </td>
            </tr>
            <tr>                  
...
if($action=="create_system_search_list_view")
{
	...
	if(trim($qrcode)=="") $qrCodeCond=""; else $qrCodeCond=" and e.barcode_no ='".trim($qrcode)."'"; 
	...
	$sql_pop="SELECT a.id, a.sys_number_prefix_num, a.sys_number, a.company_id, a.location_id, a.production_source, a.delivery_date, a.challan_no, a.remarks, c.job_no, c.style_ref_no, $year, d.po_number
    ...
    where c.garments_nature=100 and a.entry_form=375 and b.entry_form=375 and a.id=b.delivery_mst_id and b.id=e.mst_id and b.po_break_down_id=d.id and c.job_no=d.job_no_mst $conpany_cond $job_cond $sql_cond $order_cond $system_cond $stylecond $jyearCond $qrCodeCond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and e.status_active=1 and e.is_deleted=0
	group by a.id, a.sys_number_prefix_num, a.sys_number, a.company_id, a.location_id, a.production_source, a.delivery_date, a.challan_no, a.remarks, c.job_no, c.style_ref_no, a.insert_date, d.po_number
	 order by a.id DESC";
	//echo $sql_pop;
    ...
    <div style="width:700px; max-height:270px;overflow-y:scroll;" >	 
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="680" class="rpt_table" id="list_view">
        <tbody>
            ...
            {  
                if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
                ?>
                <tr bgcolor="<?=$bgcolor; ?>" onClick="js_set_value('<?=$row[csf('id')].'_'.$row[csf('sys_number')].'_'.$row[csf('company_id')].'_'.$row[csf('production_source')].'_'.change_date_format($row[csf('delivery_date')]).'_'.$row[csf('challan_no')].'_'.$row[csf('remarks')]; ?>')" style="cursor:pointer" >
                    <td width="30" align="center"><?=$i; ?></td>
...
if($action=="populate_bundle_data_update")
{
	...
	$sql_bundle="select barcode_no from pro_garments_production_dtls where delivery_mst_id='$mst_id' and production_type=76 and status_active=1 and is_deleted=0";
    ...
    $recForInspection=return_library_array( "select c.bundle_no, c.barcode_no from pro_gmts_delivery_mst a, pro_garments_production_dtls c where a.id=c.delivery_mst_id and c.production_type=77 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $bundle_nos_cond",'bundle_no','barcode_no');
	
	$output_bundles=return_library_array( "select c.bundle_no, sum(c.production_qnty) as issue_qty from pro_gmts_delivery_mst a, pro_garments_production_dtls c where  a.id=c.delivery_mst_id and c.production_type=76 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $bundle_nos_cond group by c.bundle_no",'bundle_no','issue_qty');
    $year_field="";
    if($db_type==0) $year_field="YEAR(f.insert_date)"; else if($db_type==2)$year_field="to_char(f.insert_date,'YYYY')";
	...
	from pro_garments_production_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name='$company_id' and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.delivery_mst_id='$mst_id' and a.production_type=76 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and d.status_active in(1,2,3) and d.is_deleted=0 and e.status_active in(1,2,3) and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 $bundle_nosCond  order by c.cut_no, length(c.bundle_no) asc, c.bundle_no asc";
     //echo $sql;// die;
	$result = sql_select($sql);
	...
	{ 
		if(trim($scanned_bundle_arr[$row[csf('bundle_no')]])=="" || $mst_id!="")
		{
			...
			if($recForInspection[$row[csf('bundle_no')]]=="") $removeBtnAct=""; else $removeBtnAct="display:none";
			?>
			<tr bgcolor="<?=$bgcolor; ?>" id="tr_<?=$i;?>"> 
				<td width="30" align="center"><?=$i; ?></td>
				<td width="80" id="bundle_<?=$i; ?>" title="<?=$row[csf('barcode_no')]; ?>"><?=$row[csf('bundle_no')]; ?></td>
				...
				<td width="80" style="word-break:break-all;" align="left"><?=$country_arr[$row[csf('country_id')]]; ?></td>
				<td id="button_1" align="center">
					<input type="button" id="decrease_<?=$i; ?>" name="decrease[]" style="width:30px;<?=$removeBtnAct; ?>" class="formbuttonplasminus" value="-" onClick="fn_deleteRow(<?=$i; ?>);"  />
...
if($action=="issue_to_1st_inspection_print")
{
    extract($_REQUEST);
    $data=explode('*',$data);
    //print_r ($data);
...
<div style="width:1030px;">
   ...
   from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name='$data[0]' and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.delivery_mst_id='$data[2]' and a.production_type=76 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and d.status_active in(1,2,3) and d.is_deleted=0 and e.status_active in(1,2,3) and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0
    ...
    <?=signature_table(188, $data[0], "1000px"); ?>  
    </div>
