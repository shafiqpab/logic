...
if($action=='populate_data_from_yarn_issue')
{
	...
	{ 
		...
		if($new_conn!="")
		{
			...
			//$supemployee_name=return_library_array( "select id_card_no, (first_name||' '||middle_name|| '  ' || last_name) as emp_name from hrm_employee where id_card_no='".$row[csf("supervisor_id")]."' and status_active=1 and is_deleted=0",'id_card_no','emp_name',$new_conn);
