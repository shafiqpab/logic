...
{
	echo create_drop_down( "cbo_location_name", 170, "select location_name,id from lib_location where is_deleted=0  and status_active=1 and company_id='$data' order by location_name",'id,location_name', 1, '--- Select Location ---', 0, "load_drop_down( 'requires/left_over_garments_receive_sweater_controller',document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_store_name', 'store_name_td');" );
}
...
{
	echo create_drop_down( "cbo_location_name", 120, "select location_name,id from lib_location where is_deleted=0  and status_active=1 and company_id='$data' order by location_name",'id,location_name', 1, '--- Select Location ---', 0, "load_drop_down( 'left_over_garments_receive_sweater_controller', document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_store_name2', 'cbo_store_name' )" );
	exit();
}
...
if($action=="create_po_search_list_view")
{
	...
	$is_wo_com_loc_mandatory = $ex_data[16];
	...
	if($order_type==1) // Self Order Type
	{
		...
		if(trim($garments_nature)!='0')
		{
			$sql_cond .= " and a.garments_nature='$garments_nature'";
		}
	}
	...
	if($order_type==1) // Self Order Type
	{
		...
		$sql_ex_fact = "SELECT a.po_break_down_id as po_id, sum(a.ex_factory_qnty) as ex_fact_qty from pro_ex_factory_mst a where a.status_active=1 $poCond group by a.po_break_down_id";
		 //echo $sql_ex_fact;
		...
		$sql_sewing_out = "SELECT d.po_break_down_id as po_id,sum($prod_qty_source) as sewing_out_qty from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c, pro_garments_production_mst d, pro_garments_production_dtls e where a.id=b.job_id and a.id=c.job_id and b.id=c.po_break_down_id and d.id=e.mst_id and b.id=d.po_break_down_id  and e.production_type in($prod_type_id) and d.production_type=e.production_type and a.status_active=1 and b.status_active in(1,3) and d.status_active=1 and e.status_active=1 and c.status_active in(1,3)  $sql_cond group by d.po_break_down_id";//b.shiping_status=1
		 //echo $sql_sewing_out;
		...
		if( $goods_type ==1 || $goods_type == 2 )	//Goods Type [Good GMT In Hand , Damage GMT]
		{
			...
			$sql= "SELECT a.job_no, a.style_ref_no, a.buyer_name, a.gmts_item_id, a.location_name, a.currency_id, a.company_name, b.po_number, b.id, b.pub_shipment_date, b.shipment_date, b.unit_price as order_rate,c.item_number_id $country_select
            ...
            GROUP BY a.job_no, a.style_ref_no, a.buyer_name, a.gmts_item_id, a.location_name, a.currency_id, a.company_name, b.po_number, b.id, b.pub_shipment_date, b.shipment_date,b.unit_price, c.item_number_id $group_by
		...
		if($goods_type==2) 
		{
			$sql_sewing_out = "SELECT a.po_break_down_id as po_id, a.item_number_id as item_id, a.country_id, $prod_qty_source as sewing_out_qty
			from pro_garments_production_mst a, pro_garments_production_dtls b
			where a.id=b.mst_id and a.po_break_down_id in($left_over_po_id) and a.item_number_id in($left_over_item_id) and a.company_id=$company  and b.production_type in($prod_type_id) and a.production_type=b.production_type and a.status_active=1 and b.status_active=1 and a.garments_nature=100 $wo_com_loc_cond
...
if($action=="get_left_over_balance")
{
	...
	if($order_type==1)// self order
	{
		...
		if($goods_type==2) 
		{
			...
			from pro_garments_production_mst a, pro_garments_production_dtls b
			where a.id=b.mst_id and a.po_break_down_id=$po_id $country_cond and a.item_number_id=$gmts_item and b.production_type in($prod_type_id) and a.production_type=b.production_type and a.status_active=1 and b.status_active=1 and a.garments_nature=100 and a.company_id=$company $wo_com_loc_cond";
		} 
		else 
		{
...
if($action=="system_number_popup")
{
	...
	<script>
		$(document).ready(function(e) {
            //$("#txt_search_common").focus();
			load_drop_down( 'left_over_garments_receive_sweater_controller','<? echo $company; ?>', 'load_drop_down_location2', 'location_td' );
        });
	        ...
	        <table ellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
	            ...
	            <tbody>
	                <tr>
	                    <td>
	                    <?
	                    echo create_drop_down( "cbo_company_name", 170, "select company_name,id from lib_company comp where is_deleted=0  and status_active=1 order by company_name",'id,company_name', 1, '--- Select Company ---',$company, "load_drop_down( 'left_over_garments_receive_sweater_controller', this.value, 'load_drop_down_location2', 'location_td' );");?>
	                    </td>
	                    <td id="location_td">
	                    <?
	...
	<div style="width:100%;max-height:180px; overflow:y-scroll" id="iron_list_view" align="left">
		...
		<?php
			...
			if($db_type==0)
			{
				...
				where b.status_active=1 and b.is_deleted=0 $po_cond and a.id=b.mst_id order by b.id";//,wo_po_color_size_breakdown d   and  c.color_size_break_down_id = d.id
			}
		    if($db_type==2)
			{
...
if($action=="show_country_listview")
{
	...
	if($order_type==1)
	{
		...
		// ============================= GETTING DELIVERY QTY ============================
		if($goods_type==2) {
			$sql_delivery = "SELECT b.from_po_id as po_id,b.country_id,b.item_number_id as item_id, b.production_quantity as delivery_qty from pro_gmts_delivery_mst a, pro_gmts_delivery_dtls b where a.id=b.mst_id and b.from_po_id=$po_id and a.company_id=$company_name and a.location_id=$location_name  and b.production_type=10 and a.status_active=1 and b.status_active=1 $wo_com_loc_cond group by b.from_po_id,b.country_id,b.item_number_id";
		} else {
			$sql_delivery = "SELECT b.from_po_id as po_id,b.country_id,b.item_number_id as item_id,sum(b.production_quantity) as delivery_qty from pro_gmts_delivery_mst a, pro_gmts_delivery_dtls b where a.id=b.mst_id and b.from_po_id=$po_id and a.company_id=$company_name and a.location_id=$location_name and b.production_type=10 and a.status_active=1 and b.status_active=1 $wo_com_loc_cond group by b.from_po_id,b.country_id,b.item_number_id";
		}
		$sql_delivery_res = sql_select($sql_delivery);
...
if($action=="populate_data_from_search_popup")
{
 	...
 	foreach($res as $result)
	{
		...
		echo "$('#txt_style_name').val('".$result[csf('style_ref_no')]."');\n";
 		if($qty_source!=0)
 		{
 			echo "$('#dynamic_msg').html('Total Cut Quantity');\n";
   			...
   			if($sewing_level==1)
			{
				$dataArray=sql_select("SELECT SUM(CASE WHEN a.production_type=$qty_source   THEN a.production_quantity END) as totalreceive,SUM(CASE WHEN a.production_type=4   THEN a.production_quantity ELSE 0 END) as totalinput from pro_garments_production_mst a WHERE  a.po_break_down_id=".$result[csf('id')]." and a.item_number_id='$item_id' $country_cond and a.status_active=1 and a.is_deleted=0 and a.garments_nature=100");
			}
		...
		if($qty_source==0)
		{
			...
			$total_produced = return_field_value("sum(b.production_qnty)","pro_garments_production_mst a,pro_garments_production_dtls b","a.id = b.mst_id and a.po_break_down_id=".$result[csf('id')]." and a.item_number_id='$item_id' $country_cond and a.production_type=4 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.production_type=4 and a.status_active=1 and a.garments_nature=100");
			if($sewing_level==1)
			{
				$total_produced = return_field_value("sum(production_quantity)","pro_garments_production_mst "," po_break_down_id=".$result[csf('id')]." and  item_number_id='$item_id' $country_cond2 and  production_type=4 and  is_deleted=0   and  status_active=1 and garments_nature=100");
			}
...
if($action=="populate_mst_form_data")
{
	...
	//echo $sql.";\n";
	$result =sql_select($sql);
	echo"load_drop_down( 'requires/left_over_garments_receive_sweater_controller', ".$result[0][csf('company_id')].", 'load_drop_down_store_name', 'store_name_td' );\n";
	echo"load_drop_down( 'requires/left_over_garments_receive_sweater_controller', ".$result[0][csf('company_id')].", 'load_drop_down_location', 'cbo_location_name' );\n";

	echo"load_drop_down( 'requires/left_over_garments_receive_sweater_controller', ".$result[0][csf('working_company_id')].", 'load_drop_down_working_location', 'working_location' );\n";
...
if($action=="color_and_size_level_left_over")
{
	...
	$prod_type_id = ($leftover_source==1) ? '4,5,55' : 11;
	$prod_qty_source = ($goodsType==2) ? "b.reject_qnty" : "a.production_qnty";
			...
			else //color and size level
			{
				if($order_type==1) // Self Order
				{
					...
					else
					{
						...
						// echo $rcv_qnty."-(".$exfac_qnty."+".$pre_rcv_qnty.")";
					}
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$new_sys_number=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'LGR', date("Y",time()), 5, "select sys_number_prefix,sys_number_prefix_num from pro_leftover_gmts_rcv_mst where company_id=$cbo_company_name $mrr_cond order by id DESC ", "sys_number_prefix", "sys_number_prefix_num" ));
		 ...
		 $field_array_mst="id, sys_number_prefix, sys_number_prefix_num, sys_number, company_id, location, leftover_date, order_type, buyer_name, store_name, exchange_rate, goods_type,working_company_id,working_location_id,garments_nature,remarks, inserted_by, insert_date";
		...
		else
		{
			if(str_replace("'","",$txt_mst_id)=='')
			{
				$data_array_mst="(".$id.",'".$new_sys_number[1]."','".$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",".$cbo_location_name.",".$txt_leftover_date.",".$cbo_order_type.",".$cbo_buyer_name.",".$cbo_store_name.",".$exchange_rate.",".$cbo_goods_type.",".$cbo_working_company_name.",".$cbo_working_location_name.",".$garments_nature.",".$txt_remark.",".$user_id.",'".$pc_date_time."')";
		...
		$dtlsData = sql_select("SELECT a.color_size_break_down_id,
								sum(CASE WHEN a.production_type in(4,5,55) then a.production_qnty ELSE 0 END) as production_qnty,
								...
								where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name $country_cond and a.color_size_break_down_id!=0 and a.production_type in(4,5,55,7) and b.garments_nature=100
								...
								foreach($itme_data as $color_id=>$color_data)
								{
									foreach($color_data as $size_id=>$size_data)
									{
										...
										<tr bgcolor="<? echo $bgcolor; ?>">
											...
											<td><? echo $size_data["room_no"]; ?></td>bdt_amount
