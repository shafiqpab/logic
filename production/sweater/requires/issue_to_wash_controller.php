...
if ($action=="load_drop_down_location")
{
	...
	else { $dropdown_name="cbo_working_location";  $load_fnc="load_drop_down('requires/issue_to_wash_controller', this.value, 'load_drop_down_floor', 'floor_td' );"; }
...
{
    echo create_drop_down("cbo_floor", 110, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process=5 order by floor_name", "id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/issue_to_wash_controller', $('#cbo_working_company').val()+'_'+$('#cbo_working_location').val()+'_'+this.value+'_'+$('#txt_issue_date').val(), 'load_drop_down_line', 'line_td' );", 0);
    exit();
}
...
if ($action == "populate_bundle_data") 
{
    ...
    $scanned_bundle_arr = return_library_array("select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=78 and b.status_active=1 and b.is_deleted=0 $scnbundle_nos_cond", 'bundle_no', 'bundle_no');
	...
	{ 
		...
		if(trim($scanned_bundle_arr[$row[csf('bundle_no')]])=="")
		{
			...
			<tr bgcolor="<?=$bgcolor; ?>" id="tr_<?=$i;?>"> 
                ...
                <td width="70" style="word-break:break-all;" align="center"><?=$company_short_arr[$row[csf('sending_location')]]; ?></td>
...
if($action=="populate_mst_data")
{
    ...
    where a.entry_form=320 and b.entry_form=320 and a.id=b.delivery_mst_id and b.barcode_no='$data[0]' and a.company_id='$data[1]' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	//echo $sql_pop;
	$sql_pop_res=sql_select($sql_pop);
	foreach($sql_pop_res as $row)
    {
		echo $row[csf('company_id')].'_'.$row[csf('location_id')].'_'.$row[csf('production_source')];
	}
	exit();
}
...
if ($action == "create_bundle_search_list_view") 
{
   ...
   // $scanned_bundle_arr = return_library_array("select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=55 and  b.status_active=1 and b.is_deleted=0", 'bundle_no', 'bundle_no');
	...
	else
	{
		...
		if($job_no!='') $jobCon=" and f.job_no_prefix_num like '%$job_no%'";
		if($bndl_no!='') $bndlCon=" and c.bundle_no like '%$bndl_no%'";
		if($style!='') $styleCond=" and f.style_ref_no like '%$style%'";
	}
	if($cut_no!= '') $tmp_cut=trim($company_short_arr[$company].'-'.$syear.'-'.str_pad($cut_no,6,"0",STR_PAD_LEFT));
	
	$yearCond="";
	
	if($jyear!=0 && ($job_no!='' || $styleCond!='' || $ex_data[0]!=''))
	{
		if($db_type==0) $yearCond="YEAR(f.insert_date)=$jyear"; else if($db_type==2) $yearCond="and to_char(f.insert_date,'YYYY')=$jyear";
	}
    
    $scanned_bundle_arr = return_library_array("select bundle_no, bundle_no from pro_garments_production_dtls where production_type=78 and cut_no='".$tmp_cut."' and status_active=1 and is_deleted=0", 'bundle_no', 'bundle_no');
	//echo "select bundle_no, bundle_no from pro_garments_production_dtls where production_type=78 and cut_no='".$tmp_cut."' and status_active=1 and is_deleted=0";
    foreach (explode(",", $selectedBuldle) as $bn) {
        $scanned_bundle_arr[$bn] = $bn;
    }

    $scanne=sql_select( "select b.bundle_no, sum(b.production_qnty) as production_qnty,a.sewing_line from pro_garments_production_mst a, pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=78 and b.status_active=1 and b.is_deleted=0 $cutCon_a group by b.bundle_no,a.sewing_line");
    ...
    // echo $cutting_no;
    $last_operation=gmt_production_validation_script( 78, 1,'', $cutting_no, $production_squence);
    //$last_operation=gmt_production_validation_script( 4, 1 );
    ...
    <div style="width:850px; max-height:210px; overflow-y:scroll" id="list_container_batch" align="left">    
        ...
        <?
            ...
            foreach($last_operation as  $item_id=>$operation_cond)
            {
				...
				$qrCond $orderCon $bndlCon $yearCond $jobCon $cutCon $styleCond
				and a.status_active=1 and a.is_deleted=0 and a.production_type=56 group by c.cut_no, c.bundle_no, d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.bundle_no, e.po_number,c.barcode_no order by c.cut_no, length(c.bundle_no) asc, c.bundle_no asc";
...
if ($action=="save_update_delete")
{
	...
	{
		$rec_number=return_field_value("sys_number", "pro_gmts_delivery_mst"," issue_challan_id=$txt_update_id and status_active=1 and is_deleted=0 and production_type=79 and entry_form=384");
		if($iss_number){
			echo "washRec**".str_replace("'","",$txt_update_id)."**".$rec_number;
			die;
		}
	}
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
        ...
        $receive_sql="select c.barcode_no, c.bundle_no from pro_garments_production_mst a, pro_garments_production_dtls c where a.id=c.mst_id and a.production_type=78 and c.bundle_no in ($bundle) and c.production_type=78 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 "; //and (c.is_rescan=0 or c.is_rescan is null)
		...
		if (str_replace("'", "", $txt_update_id) == "")
		{
			...
			$new_sys_number = explode("*", return_next_id_by_sequence("", "pro_gmts_delivery_mst",$con,1,$cbo_company_name,'BIW',0,date("Y",time()),0,0,78,0,0 ));
			...
			$data_array_delivery="(".$mst_id.",'".$new_sys_number[1]."','".(int)$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",78,".$cbo_source.",".$txt_issue_date.",'".$challanNo."',".$txt_remarks.",383,".$user_id.",'".$pc_date_time."',1,0)";
		...
		foreach($mstArr as $orderId=>$orderData)
		{
			foreach($orderData as $gmtsItemId=>$gmtsItemIdData)
			{
				foreach($gmtsItemIdData as $countryId=>$qty)
				{
					...
					$data_array_mst.="(".$id.",".$mst_id.",".$garments_nature.",".$cbo_company_name.",".$cbo_source.",".$txt_issue_date.",'".$challan_no."',".$txt_remarks.",".$orderId.", ".$gmtsItemId.",".$countryId.",".$qty.",78,3,383,".$user_id.",'".$pc_date_time."',1,0)";
		...
		foreach($dtlsArr as $bundle_no=>$qty)
		{
			...
			$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",78,'".$dtlsArrColorSize[$bundle_no]."','".$qty."','".$cutNoArr[$bundle_no]."','".$bundle_no."',383,'".$bundleBarcodeArr[$bundle_no]."',1,0)"; 
		}
		$flag=1;
		if (str_replace("'", "", $txt_update_id) == "")
		{
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
        ...
        $bundle="'".implode("','",$bundleCheckArr)."'";
        $receive_sql="select c.barcode_no, c.bundle_no from pro_garments_production_mst a, pro_garments_production_dtls c where a.id=c.mst_id and a.production_type=78 and c.bundle_no in ($bundle) and c.production_type=78 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and a.delivery_mst_id!=$mst_id and c.delivery_mst_id!=$mst_id)"; 
        $receive_result = sql_select($receive_sql);
        foreach ($receive_result as $row)
        {           
            $duplicate_bundle[$row[csf('bundle_no')]]=$row[csf('bundle_no')];
        }
		$non_delete_arr=production_validation($mst_id,79);
		$issue_data_arr=production_data($mst_id,78);
		...
		$flag=1;
		$delete = execute_query("update pro_garments_production_mst set status_active=0,is_deleted=1,updated_by=$user_id,update_date='".$pc_date_time."' WHERE delivery_mst_id=$mst_id and production_type=78 and status_active=1 and is_deleted=0");
		if($delete==1 && $flag==1) $flag=1; else $flag=0;
		$delete_dtls = execute_query("update pro_garments_production_dtls set status_active=0, is_deleted=1 WHERE delivery_mst_id=$mst_id and production_type=78 and status_active=1 and is_deleted=0");
...
if($action=="create_system_search_list_view")
{
    ...
    where c.garments_nature=100 and a.entry_form=383 and b.entry_form=383 and a.id=b.delivery_mst_id and b.po_break_down_id=d.id and c.job_no=d.job_no_mst $conpany_cond $job_cond $sql_cond $order_cond $system_cond $stylecond $jyearCond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.id DESC";
	//echo $sql_order;
...
if($action=="populate_bundle_data_update")
{
	...
	$sql_bundle="select barcode_no from pro_garments_production_dtls where delivery_mst_id='$mst_id' and production_type=78 and status_active=1 and is_deleted=0";
	...
	$sql_knit="select c.barcode_no, a.serving_company, a.floor_id from pro_garments_production_mst a, pro_garments_production_dtls c where a.id=c.mst_id and a.production_type=56 and c.production_type=56 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $bundle_nos_cond";
...
<div style="width:1030px;">
    <table width="1000" cellspacing="0" align="left">
        ...
        <tr>
            ...
            <td width="110"><strong>W. Company:</strong></td>
            <td><?=$company_library[$dataArray[0][csf('working_company_id')]]; ?></td>
        </tr>
        <tr>
            <td><strong>WC. Location:</strong></td>
            <td><?=$location_library[$dataArray[0][csf('working_location_id')]]; ?></td>
            <td><strong>Floor :</strong></td>
            <td><? //=$floor_library[$dataArray[0][csf('floor_id')]]; ?></td>
   ...
   from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name='$data[0]' and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.delivery_mst_id='$data[2]' and a.production_type=78 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and d.status_active in(1,2,3) and d.is_deleted=0 and e.status_active in(1,2,3) and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0
    ...
    <?=signature_table(192, $data[0], "1000px"); ?>  
    </div>
