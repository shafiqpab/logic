...
if($action=="load_machine")
{
    if($db_type==2)
    {
        $sql="select (id || ':' || machine_no) as machine_name from lib_machine_name where category_id=1  and status_active=1 and is_deleted=0 and is_locked=0  order by seq_no";
    }
    else
    {
...
if ($action == "load_multiple_drop_down") 
{
    $data = explode("_", $data);
    $company_id = $data[1];
    //$company_id
    ...
    echo "****";
    echo create_drop_down( "cbo_working_location", 140, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id='$data[1]' order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down('requires/bundle_issue_to_knitting_floor_test_controller', this.value, 'load_drop_down_floor', 'floor_td' );",1 );

    echo "****";
    echo create_drop_down( "cbo_location", 140, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id='$data[1]' order by location_name","id,location_name", 1, "-- Select Location --", $selected, "",1 );
    echo "****";
    echo create_drop_down( "cbo_floor", 140, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data[2]' and production_process in (2) order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "",1 );
    exit();
}
                            ...
                            foreach($job_bodypart_id as $id=>$name)
                            {
                                ...
                                if($id!=14)
                                {
                                    ...
                                    <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value(<? echo $i;?>)"> 
                                        ...
                                        <td><p><? echo $body_part_type_string; ?></p></td>
                                    </tr>
                                    <?
                                    $i++;
                                }
                    ...
                    <td align="center">
                        <input 
                            ...
                            onClick="show_list_view (
                                        ...
                                        '<? echo trim($lot_ratio,','); ?>'+'_'+
                                        '<? echo trim($bodypart_ids,','); ?>'+'_'+
                                        '<? echo trim($bodypart_type,','); ?>',
                                        'create_bundle_search_list_view',
                                        'search_div',
                                        'bundle_issue_to_knitting_floor_test_controller',
...
if($action=='populate_data_from_issue')
{
    ...
    { 
        ...
        echo "load_drop_down( 'requires/bundle_issue_to_knitting_floor_test_controller', ".($row[csf("production_source")])."+'_'+".($row[csf("company_id")]).", 'load_drop_down_knitting_com','knitting_com');";
        ...
        echo "load_drop_down( 'requires/bundle_issue_to_knitting_floor_test_controller', ".$row[csf("working_location_id")].", 'load_drop_down_floor', 'floor_td' );";
        echo "document.getElementById('cbo_floor').value                    = '".($row[csf("floor_id")])."';\n";
        echo "load_drop_down( 'requires/bundle_issue_to_knitting_floor_test_controller', '".$row[csf('company_id')]."', 'load_drop_down_lc_location', 'location_td' );\n";
        ...
        $style_ref_no=return_field_value(
                                    "style_ref_no",
                                    "wo_po_details_master",
                                    " job_no='".($row[csf("job_no_mst")])."' and status_active=1 and is_deleted=0");
        ...
        $employee_name=return_field_value(
                                    "(first_name||' '||middle_name|| '  ' || last_name) as emp_name",
                                    "hrm_employee",
                                    " id_card_no='".($row[csf("supervisor_id")])."' and status_active=1 and is_deleted=0",
                                    "emp_name",$new_conn);
...
if($action=='populate_data_from_yarn_lot')
{
    ...
    { 
        ...
        echo "load_drop_down( 'requires/bundle_issue_to_knitting_floor_test_controller', ".($row[csf("source")])."+'_'+".($row[csf("company_id")]).", 'load_drop_down_knitting_com','knitting_com');";
        
        echo "document.getElementById('cbo_working_company').value          = '".$row[csf("working_company_id")]."';\n";
        
        echo "load_location();\n";
        echo "document.getElementById('cbo_working_location').value                     = '".$row[csf("location_id")]."';\n";
        echo "load_drop_down( 'requires/bundle_issue_to_knitting_floor_test_controller', ".$row[csf("location_id")].", 'load_drop_down_floor', 'floor_td' );";
        echo "document.getElementById('cbo_floor').value  = '".($row[csf("floor_id")])."';\n";
        echo "load_drop_down( 'requires/bundle_issue_to_knitting_floor_test_controller', '".$row[csf('company_id')]."', 'load_drop_down_lc_location', 'location_td' );\n";
...
function get_bundle_list_view($bundle_no,$bodypart_type)
{
        ...
        <table cellpadding="0" width="<?php echo $div_width;?>" cellspacing="0" border="1" class="rpt_table" rules="all">
            ...
            <thead>
                <tr>
                    ...
                    <th width="80" rowspan="3" class="must_entry_caption">MC No<input type="checkbox" id="all_check" onclick="check_all('all_check')"></th>
...
function get_yarn_list_view($bundle,$cutting_no,$operator_id)
{
    $color_library=return_library_array( "select id,color_name from lib_color", "id", "color_name"  );
    
    $data=explode("_",$data);
    $operator_id=$data[2];
    $sql_cut=sql_select("select a.job_no, a.size_set_no, b.color_id, b.roll_data, a.id, b.id as dtls_id from ppl_cut_lay_mst a, ppl_cut_lay_dtls b where a.cutting_no='".$cutting_no."' and a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 ");
    $job_no             =$sql_cut[0][csf("job_no")];
    $color_id           =$sql_cut[0][csf("color_id")];
    $mst_id             =$sql_cut[0][csf("id")];
    $dtls_id            =$sql_cut[0][csf("dtls_id")];
    ...
    $sql_returnable=sql_select(" select yarn_color, lot_ratio_no, required_qty, (required_qty-issue_qty) as returnable_qty from pro_gmts_knitting_issue_dtls where operator_id=$operator_id and lot_ratio_no='".$cutting_no."' and production_type=50 and status_active=1 and is_deleted=0");
...
if($action=='populate_all_list_view')
{
    $data = explode("_", $data);
    $color_library=return_library_array( "select id,color_name from lib_color", "id", "color_name"  );
    $size_library=return_library_array( "select id,size_name from lib_size", "id", "size_name"  );
...
{
    $data = explode("_", $data);
    $data_array=sql_select("SELECT a.id, a.company_id, a.source, a.working_company_id , a.location_id, a.cutting_no, a.floor_id, job_no , size_set_no from ppl_cut_lay_mst a, ppl_cut_lay_bundle b where a.id=b.mst_id and b.barcode_no='$data[0]'  and a.entry_form=253 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
    
    foreach ($data_array as $row)
    { 
        /*if ($row[csf("source")] == 1) 
        {
            $wo_company =  create_drop_down("cbo_working_company", 140, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0  order by comp.company_name", "id,company_name", 1, "--Select Knit Company--", $company_id, "load_location();", 1);
        } 
        else if ($row[csf("source")] == 3) 
        {
            $wo_company =  create_drop_down("cbo_working_company", 140, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=20 and a.status_active=1 group by a.id,a.supplier_name order by a.supplier_name", "id,supplier_name", 1, "--Select Knit Company--", 0, "load_location();");
        } 
        else 
        {
            $wo_company =  create_drop_down("cbo_working_company", 140, $blank_array, "", 1, "--Select Knit Company--", 0, "load_location();");
        }
        $wo_location =  create_drop_down( "cbo_working_location", 140, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id=".$row[csf(company_id)]." order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down('requires/bundle_issue_to_knitting_floor_test_controller', this.value, 'load_drop_down_floor', 'floor_td' );",1 );

        
        $wo_floor = create_drop_down( "cbo_floor", 140, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id=".$row[csf(location_id)]." and production_process in (2) order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "",1 );

        echo "document.getElementById('knitting_com').innerHTML = '".$wo_company."';\n";
        echo "document.getElementById('location_td').innerHTML = '".$wo_location."';\n";
        echo "document.getElementById('floor_td').innerHTML = '".$wo_floor."';\n";*/
        ...
        echo "document.getElementById('cbo_company_name').value             = '".$row[csf("company_id")]."';\n";
        echo "document.getElementById('cbo_source').value                   = '".$row[csf("source")]."';\n";
        echo "load_drop_down_multiple( 'requires/bundle_issue_to_knitting_floor_test_controller', ".($row[csf("source")])."+'_'+".($row[csf("company_id")])."+'_'+".($row[csf("location_id")]).", 'load_multiple_drop_down','knitting_com*working_location_td*location_td*floor_td');";
        // echo "load_drop_down( 'requires/bundle_issue_to_knitting_floor_test_controller', ".($row[csf("source")])."+'_'+".($row[csf("company_id")]).", 'load_drop_down_knitting_com','knitting_com');";
        ...
        echo "document.getElementById('cbo_location').value                   = '".$location_id."';\n";
        // echo "document.getElementById('bundle_list_view').innerHTML           = '".get_bundle_list_view($data[0],$data[1])."';\n";
        // echo "document.getElementById('yarn_list_view').innerHTML           = '".get_yarn_list_view($data[0],$row[CUTTING_NO],$data[3])."';\n";
        ...
        <table cellpadding="0"width="<?php echo $div_width;?>"cellspacing="0"border="1"class="rpt_table"rules="all">
            ...
            <thead>
                <tr>
                    ...
                    <th width="100" rowspan="3">Bundle No</th>
                    <th width="100" rowspan="3">Barcode No</th>
                    <th width="80" rowspan="3" class="must_entry_caption">MC No</th>
                    <th width="120" rowspan="3"> G. Color</th>
                    <th width="50"  rowspan="3">Size</th>
                    <th width="60"  rowspan="3">Bundle Qty.(Pcs)</th>
                    <th width="100" colspan="2">RMG No.</th>
        ...
        <?php  
            ...
            ?>
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="tr_<? echo $i; ?>" > 
                    ...
                    <td width="80" align="right"><p><?php $returanable_qty = $selectResult[csf('returanable_qty')]/454;echo number_format($returanable_qty,4,".",""); ?></p></td>
                    ...
                    <tr class="general">
                        <td align="center">
                            <?
                                ...
                                echo create_drop_down( "cbo_company_name",
                                                        ...
                                                        $selected, 
                                                        "load_drop_down(     
                                                                        'bundle_issue_to_knitting_floor_test_controller', this.value, 
                        ...
                        <td>
                            ...
                            onClick="show_list_view ( 
                                                        ...
                                                        'search_div', 
                                                        'bundle_issue_to_knitting_floor_test_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />                
                        </td>
                    </tr> 
               </table>
...
{
    if($new_conn!=""){
        echo create_drop_down("cbo_location_name",135,"select id,location_name from lib_location where company_id=$data and status_active=1 and is_deleted=0","id,location_name",1,"-- Select Location --",$selected,"load_drop_down('bundle_issue_to_knitting_floor_test_controller', this.value,'load_drop_down_division','division_td_hrm');","","","","","","",$new_conn );
    }else{
        echo create_drop_down("cbo_location_name",135,"select id,location_name from lib_location where company_id=$data and status_active=1 and is_deleted=0","id,location_name",1,"-- Select Location --",$selected, "load_drop_down('bundle_issue_to_knitting_floor_test_controller', this.value,'load_drop_down_division','division_td_hrm');","","","","","","" );
    }
    exit();
}
...
{
    if($new_conn!=""){
        echo create_drop_down("cbo_division_name",135,"select id,division_name from lib_division where location_id=$data and status_active=1 and is_deleted=0","id,division_name",1,"-- Select Division --",$selected,"load_drop_down('bundle_issue_to_knitting_floor_test_controller',this.value,'load_drop_down_department','department_td_hrm');","","","","","","",$new_conn );
    }else{
        echo create_drop_down("cbo_division_name",135,"select id,division_name from lib_division where location_id=$data and status_active=1 and is_deleted=0","id,division_name",1,"-- Select Division --",$selected,"load_drop_down('bundle_issue_to_knitting_floor_test_controller',this.value,'load_drop_down_department','department_td_hrm');","","","","","","" );
    }
    exit();
}
    ...
    <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
                ...
                <tbody>
                    <tr class="general">                    
                        ...
                        <td>
                             <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_job_prifix').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_po_no').value+'_'+document.getElementById('txt_style_no').value, 'create_job_search_list_view', 'search_div', 'bundle_issue_to_knitting_floor_test_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" />                
                        </td>
                </tr>
                    <tr>                  
    ...
    <table cellspacing="0" width="1000"  border="1" rules="all" class="rpt_table" style=" margin-top:20px;" >
        ...
        <?php
            ...
            foreach($dataArrayKnitting as $val)
            {
                ...
                $returnable_quantity+=$val[csf('returanable_qty')];
               // $issue_quantity+=$val[csf('issue_qty')];
                $j++;
            }
        ...
        <tfooter bgcolor="#eee" align="left">
            ...
            <th width="100" align="center"><?php echo number_format($returnable_quantity,4); ?></th>
...
if($action=="challan_duplicate_check")
{
    ...
    $search_lot_no=return_field_value(
                                    "a.cutting_no as cutting_no",
                                    "ppl_cut_lay_mst a, ppl_cut_lay_bundle  b",
                                    "b.barcode_no='".$data."' and a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0",
                                    "cutting_no");  

    $result=sql_select("select a.sys_number,b.bundle_no from pro_gmts_delivery_mst a,pro_garments_production_dtls b where a.id=b.delivery_mst_id and b.production_type=50 and a.status_active=1 and a.embel_name=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $bundle_nos_cond group by a.sys_number,b.bundle_no");
