...
if ($action=="load_drop_down_location")
{
	...
	else { $dropdown_name="cbo_working_location";  $load_fnc="load_drop_down('requires/linking_output_controller', this.value, 'load_drop_down_floor', 'floor_td' );"; }
...
{
    echo create_drop_down("cbo_floor", 130, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process=5 order by floor_name", "id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/linking_output_controller', $('#cbo_working_company').val()+'_'+$('#cbo_working_location').val()+'_'+this.value+'_'+$('#txt_output_date').val(), 'load_drop_down_line', 'line_td' );", 0);
    exit();
}
...
if ($action == "populate_bundle_data") 
{
    ...
    $scanned_bundle_arr = return_library_array("select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=56 and b.status_active=1 and b.is_deleted=0 $scnbundle_nos_cond", 'bundle_no', 'bundle_no');
...
if($action=="populate_mst_data")
{
    ...
    where a.entry_form=321 and b.entry_form=321 and a.id=b.delivery_mst_id and b.barcode_no='$data[0]' and a.company_id='$data[1]' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	...
	foreach($sql_pop_res as $row)
    {
		...
		echo "load_drop_down( 'requires/linking_output_controller', '".$row[csf("working_company_id")]."_".$row[csf("working_location_id")]."_".$row[csf("floor_id")]."_".$data[2]."', 'load_drop_down_line', 'line_td' );\n";
...
if ($action == "create_bundle_search_list_view") 
{
  ...
  // echo $tmp_cut;
   $scanned_bundle_arr = return_library_array("select bundle_no, bundle_no from pro_garments_production_dtls where production_type=56 and cut_no='".$tmp_cut."' and status_active=1 and is_deleted=0", 'bundle_no', 'bundle_no');
    foreach (explode(",", $selectedBuldle) as $bn) {
        $scanned_bundle_arr[$bn] = $bn;
    }

    $scanne=sql_select( "select b.bundle_no, sum(b.production_qnty) as production_qnty,a.sewing_line from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=56 and b.status_active=1 and b.is_deleted=0 $cutCon_a group by b.bundle_no,a.sewing_line");
...
if ($action=="save_update_delete")
{
	...
	{
		$rec_number=return_field_value("sys_number", "pro_gmts_delivery_mst"," issue_challan_id=$txt_update_id and status_active=1 and is_deleted=0 and production_type=57 and entry_form=320");
		if($iss_number){
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
        ...
        $receive_sql="select c.barcode_no, c.bundle_no from pro_garments_production_mst a, pro_garments_production_dtls c where a.id=c.mst_id and a.production_type=56 and c.bundle_no in ($bundle) and c.production_type=56 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 "; //and (c.is_rescan=0 or c.is_rescan is null)
		...
		if (str_replace("'", "", $txt_update_id) == "")
		{
			...
			$new_sys_number = explode("*", return_next_id_by_sequence("", "pro_gmts_delivery_mst",$con,1,$cbo_company_name,'OLK',0,date("Y",time()),0,0,56,0,0 ));
			...
			$data_array_delivery="(".$mst_id.",'".$new_sys_number[1]."','".(int)$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",56,".$cbo_location.",".$cbo_source.",".$cbo_working_company.",".$cbo_working_location.",".$cbo_floor.",".$cbo_line_no.",".$txt_output_date.",'".$challanNo."',".$txt_remarks.",322,".$user_id.",'".$pc_date_time."',1,0)";
		...
		else 
		{
			...
			$data_array_delivery = "".$cbo_location."*".$cbo_source."*".$cbo_working_company."*".$cbo_working_location."*".$cbo_floor."*".$cbo_line_no."*".$txt_output_date."*'".$challanNo."'*".$txt_remarks."*".$user_id."*'".$pc_date_time."'";
		...
		if($db_type==2) 
		{
			$txt_reporting_hour=str_replace("'","",$txt_output_date)." ".str_replace("'","",$txt_reporting_hour);
		...
		foreach($dtlsArr as $bundle_no=>$qty)
		{
			...
			$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",56,'".$dtlsArrColorSize[$bundle_no]."','".$qty."','".$dtlsArrRej[$bundle_no]."','".$dtlsArrAlt[$bundle_no]."','".$dtlsArrSpot[$bundle_no]."','".$dtlsArrRep[$bundle_no]."','".$cutNoArr[$bundle_no]."','".$bundle_no."',322,'".$bundleBarcodeArr[$bundle_no]."','".$bundleRescanArr[$bundle_no]."',1,0)"; 
		}
		$flag=1;
		if (str_replace("'", "", $txt_update_id) == "")
		{
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
        ...
        $bundle="'".implode("','",$bundleCheckArr)."'";
        $receive_sql="select c.barcode_no, c.bundle_no from pro_garments_production_mst a, pro_garments_production_dtls c where a.id=c.mst_id and a.production_type=56 and c.bundle_no in ($bundle) and c.production_type=56 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and a.delivery_mst_id!=$mst_id and c.delivery_mst_id!=$mst_id)"; 
        $receive_result = sql_select($receive_sql);
        foreach ($receive_result as $row)
        {           
            $duplicate_bundle[$row[csf('bundle_no')]]=$row[csf('bundle_no')];
        }
		$non_delete_arr=production_validation($mst_id,57);
		$issue_data_arr=production_data($mst_id,56);
		...
		$flag=1;
		$delete = execute_query("update pro_garments_production_mst set status_active=0,is_deleted=1,updated_by=$user_id,update_date='".$pc_date_time."' WHERE delivery_mst_id=$mst_id and production_type=56 and status_active=1 and is_deleted=0");
		if($delete==1 && $flag==1) $flag=1; else $flag=0;
		$delete_dtls = execute_query("update pro_garments_production_dtls set status_active=0, is_deleted=1 WHERE delivery_mst_id=$mst_id and production_type=56 and status_active=1 and is_deleted=0");
...
{
  	echo load_html_head_contents("Linking Output Info","../../../", 1, 1, '','1','');
	extract($_REQUEST);
	?>
	<script>
	...
	?>
     <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="985" >
        <thead>
            ...
            <th width="60">Output Year</th>
            <th width="60">Output No</th>
...
if($action=="populate_bundle_data_update")
{
	...
	$sql_bundle="select barcode_no from pro_garments_production_dtls where delivery_mst_id='$mst_id' and production_type=55 and status_active=1 and is_deleted=0";
	...
	from pro_garments_production_mst a,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name='$company_id' and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.delivery_mst_id='$mst_id' and a.production_type=56 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and d.status_active in(1,2,3) and d.is_deleted=0 and e.status_active in (1,2,3) and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 $bundle_nosCond  order by c.cut_no, length(c.bundle_no) asc, c.bundle_no asc";
     echo $sql;// die;
	$result = sql_select($sql);
	$count=count($result);
	$i=$count;
	...
	{ 
		if(trim($scanned_bundle_arr[$row[csf('bundle_no')]])=="" || $mst_id!="")
		{
			...
			<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i;?>"> 
				...
				<td width="100" style="word-break:break-all;" align="left"><? echo $color_arr[$row[csf('color_number_id')]]; ?></td>
				<td width="60" style="word-break:break-all;" align="left">&nbsp;<? echo $size_arr[$row[csf('size_number_id')]]; ?></td>
				<td width="65" id="prodQty_<? echo $i; ?>" align="right"><? echo ($row[csf('production_qnty')]+$row[csf('reject_qty')]+$row[csf('alter_qty')]+$row[csf('spot_qty')])-$row[csf('replace_qty')]; //$qty; ?>&nbsp;</td>
