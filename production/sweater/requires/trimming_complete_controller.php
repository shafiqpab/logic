...
if ($action=="load_drop_down_location")
{
	...
	$res=sql_select($sql);
	$sl=$selected;
	echo create_drop_down( "cbo_location", 140, $sql,"id,location_name", 1, "-- Select Location --", $sl, "load_drop_down( 'requires/trimming_complete_controller', this.value, 'load_drop_down_floor', 'floor_td' );",0 );
	exit();
}

if ($action=="load_drop_down_floor")
{
	echo create_drop_down( "cbo_floor", 140, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process=5 order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/trimming_complete_controller', this.value+'_'+document.getElementById('cbo_location').value, 'load_drop_down_poly_line_floor', 'poly_line_td' );",0 );
	exit();
}
...
if($action=="production_process_control")
{
    ...
    $control_and_preceding=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=50 and page_category_id=111 and company_name='$data'");
...
if($action=="load_drop_down_cutt_company")
{
	...
	if($data==3)
	{
		...
		echo create_drop_down( "cbo_cutting_company", 140, "select id,company_name from lib_company where is_deleted=0 and status_active=1 $company_credential_cond order by company_name","id,company_name", 1, "--- Select ---", "", "load_drop_down( 'requires/trimming_complete_controller', this.value, 'load_drop_down_location', 'location_td' );fnc_company_check(this.value);",0,0 );
	else
...
if($action=="order_popup")
{
	...
	// onkeydown="if (event.keyCode == 13) document.getElementById('btn_show').click()"
			...
			{
				load_drop_down( 'trimming_complete_controller',document.getElementById('company_search_by').value,'load_drop_down_buyer', 'search_by_td' );
	...
	<div align="center" style="width:100%;" >
        ...
        <table width="990" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
             <thead>
...
if($action=="create_po_search_list_view")
{
	...
	{
		$sql = "SELECT b.id,a.order_uom,a.buyer_name,b.grouping,b.file_no,a.company_name,a.total_set_qnty,a.set_break_down, a.job_no,a.style_ref_no,a.gmts_item_id,a.location_name,b.shipment_date,b.po_number,b.po_quantity,b.plan_cut,a.insert_date
			...
			$sql_cond $sql_shipment_year_cond group by b.id,a.order_uom,a.buyer_name,b.grouping,b.file_no,a.company_name,a.total_set_qnty,a.set_break_down, a.job_no,a.style_ref_no,a.gmts_item_id,a.location_name,b.shipment_date,b.po_number,b.po_quantity,b.plan_cut, b.t_year order by b.shipment_date";
	}
	else
	{
 		$sql = "SELECT b.id,a.order_uom,a.buyer_name,b.grouping,b.file_no,a.company_name,a.total_set_qnty,a.set_break_down, a.job_no,a.style_ref_no,a.gmts_item_id,a.location_name,b.shipment_date,b.po_number,b.po_quantity ,b.plan_cut
			...
			a.garments_nature=$garments_nature
			$sql_cond $sql_shipment_year_cond order by b.shipment_date";
	}
	// echo $sql;die;
    ...
    <div style="width:1140px;">
     	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="100%" class="rpt_table">
            <thead>
                ...
                <th width="70">Trim. Qty</th>
                <th >Balance</th>
            </thead>
     	</table>
     </div>
     <div style="width:1140px; max-height:240px;overflow-y:scroll;" >
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1120" class="rpt_table" id="tbl_po_list" >
			<?
			$i=1;
				...
				for($k=0;$k<$numOfItem;$k++)
				{
					...
					foreach($country as $country_id)
					{
                        ...
                        <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value(<? echo $row[csf("id")];?>,'<? echo $grmts_item;?>','<? echo $po_qnty;?>','<? echo $trim_qnty;?>','<? echo $country_id;?>','<? echo$job_num;?>','<? echo $company_id;?>');" >
                            ...
                            <td width="130"><p><?php echo $row[csf("po_number")]; ?></p></td>
                            <td width="90" style="word-break:break-all"><p><?php echo $job_num; ?></p></td>
                            ...
                            <td width="70"><p><?php echo $row[csf("grouping")]; ?></p></td>
                            ...
                            <td width="70" align="right"><?php echo $trim_qnty; ?>&nbsp;</td>
...
if($action=="populate_data_from_search_popup")
{
	...
	else if($preceding_process==1) $qty_source=1; //knitting Complete
	else if($preceding_process==100) $qty_source=100; //First Inspection
 	...
 	foreach($res as $result)
	{
		...
		echo "$('#txt_job_no').val('".$result[csf('job_no')]."');\n";
 		...
 		if($qty_source==0)
 		{
			...
			$total_produced = return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=".$result[csf('id')]." and item_number_id='$item_id' and country_id='$country_id' and production_type=111 and is_deleted=0");
		...
		{
			$dataArray=sql_select("SELECT SUM(CASE WHEN a.production_type=$qty_source and b.production_type=$qty_source THEN b.production_qnty END) as totalreceive,SUM(CASE WHEN a.production_type=111 and b.production_type=111  THEN b.production_qnty ELSE 0 END) as totalinput from pro_garments_production_mst a,pro_garments_production_dtls b WHERE a.id=b.mst_id and  a.po_break_down_id=".$result[csf('id')]." and a.item_number_id='$item_id' and a.country_id='$country_id' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
...
if($action=="color_and_size_level")
{
		...
		if($qty_source!=0)
		{
			...
			if( $variableSettings==2 ) // color level
			{
				...
				{
					$sql = "SELECT a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty,sum( CASE WHEN b.production_type=$qty_source then b.production_qnty ELSE 0 END) as production_qnty,sum( CASE WHEN b.production_type=111 then b.production_qnty ELSE 0 END) as cur_production_qnty
			...
			else if( $variableSettings==3 ) //color and size level
			{
				...
				$dtlsData = sql_select("SELECT a.color_size_break_down_id,
											...
											sum(CASE WHEN a.production_type=111 then a.production_qnty ELSE 0 END) as cur_production_qnty
		...
		else // when variable setting not set=============================
		{
			if( $variableSettings==2 ) // color level
			{
				if($db_type==0)
	...
	<style type="text/css">
		...
		<div style="width:1005px;max-height:250px; overflow-y:auto; overflow-x: auto;" id="sewing_production_list_view" align="left">
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1000" class="rpt_table"  align="left">
				 ...
				 <thead>
	            	<tr>
		                ...
		                <th width="100" align="center">Trim. Line</th>
			...
			<?php
				...
				{
					$sql="SELECT a.id,a.po_break_down_id,a.item_number_id,a.production_date,a.production_quantity,a.reject_qnty,a.production_source,TO_CHAR(a.production_hour,'HH24:MI') as production_hour,a.serving_company,a.location,a.floor_id,a.country_id, min(b.id) as color_id,a.supervisor,a.sewing_line,a.challan_no,a.spot_qnty,a.alter_qnty,a.wo_order_no from pro_garments_production_mst a,wo_po_color_size_breakdown b where a.po_break_down_id='$po_id' and a.po_break_down_id=b.po_break_down_id and a.item_number_id='$item_id' and a.country_id='$country_id' and a.production_type='111' and a.status_active=1 and a.is_deleted=0 group by a.id,a.po_break_down_id,a.item_number_id,a.production_date,a.production_quantity,a.reject_qnty,a.production_source,a.production_hour,a.serving_company,a.location,a.floor_id,a.country_id,a.supervisor,a.sewing_line,a.challan_no,a.spot_qnty,a.alter_qnty,a.wo_order_no order by a.production_date";
			...
			?>
				...
				<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data('<? echo $selectResult[csf('id')]."_".$selectResult[csf('color_id')]; ?>','populate_cutting_form_data','requires/trimming_complete_controller');" >
					...
					<td  align="center"><?php echo $selectResult[csf('wo_order_no')]; ?></td>
...
if($action=="populate_cutting_form_data")
{
  	...
  	foreach($sqlResult as $result)
	{
		...
		echo "load_drop_down( 'requires/trimming_complete_controller', ".$result[csf('production_source')].", 'load_drop_down_cutt_company', 'cutt_company_td' );\n";
		echo "$('#cbo_cutting_company').val('".$result[csf('serving_company')]."');\n";
		echo "load_drop_down( 'requires/trimming_complete_controller',".$result[csf('serving_company')].", 'load_drop_down_location', 'location_td' );";
		echo "$('#cbo_location').val('".$result[csf('location')]."');\n";
		echo "load_drop_down( 'requires/trimming_complete_controller', ".$result[csf('location')].", 'load_drop_down_floor', 'floor_td' );\n";
		echo "$('#cbo_floor').val('".$result[csf('floor_id')]."');\n";

		echo "load_drop_down( 'requires/trimming_complete_controller', ".$result[csf('po_break_down_id')].", 'load_drop_down_color_type', 'color_type_td' );\n";
		echo "$('#cbo_color_type').val('".$color_type_val[0][csf("color_type_id")]."');\n";

		if($result[csf('production_source')]==3)
		{
			echo "load_drop_down( 'requires/trimming_complete_controller', '".$result[csf('company_id')]."_".$result[csf('serving_company')]."_".$result[csf('po_break_down_id')]."', 'load_drop_down_workorder', 'workorder_td' );\n";
		...
		echo "$('#txt_spot_qnty').val('".$result[csf('spot_qnty')]."');\n";
  		...
  		echo "load_drop_down( 'requires/trimming_complete_controller', '".$result[csf('floor_id')]."_".$result[csf('location')]."', 'load_drop_down_poly_line_floor', 'poly_line_td' );\n";
  		
  		//load_drop_down( 'requires/trimming_complete_controller', this.value+'_'+document.getElementById('cbo_location').value, 'load_drop_down_poly_line_floor', 'poly_line_td' );
		...
		echo "$('#txt_yet_cut').val('".$result[csf('yet_to_produced')]."');\n"; */
			...
			$defect_sql=sql_select("select id, po_break_down_id, defect_type_id, defect_point_id, defect_qty from pro_gmts_prod_dft where mst_id='".$result[csf('id')]."' and status_active=1 and is_deleted=0 and production_type='111'");
		...
		$control_and_preceding=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=50 and page_category_id=112 and company_name=$company_id");
	    if(count($control_and_preceding)>0)
	    {
		...
		{
			$dataArray=sql_select("SELECT SUM(CASE WHEN a.production_type=$qty_source and b.production_type=$qty_source THEN b.production_qnty END) as totalreceive,SUM(CASE WHEN a.production_type=111 and b.production_type=111  THEN b.production_qnty ELSE 0 END) as totalinput from pro_garments_production_mst a,pro_garments_production_dtls b WHERE a.id=b.mst_id and  a.po_break_down_id=".$result[csf('po_break_down_id')]." and a.item_number_id=".$result[csf('item_number_id')]." and a.country_id=".$result[csf('country_id')]." and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
			foreach($dataArray as $row)
			{
		...
		else
		{
			if( $variableSettings!=1 ) // gross level
			{
				...
				if( $variableSettings==2 ) // color level
				{
					...
					{
						$sql = "SELECT a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty,sum(CASE WHEN a.production_type=$qty_source then b.production_qnty else 0 end) as production_qnty,sum(CASE WHEN a.production_type=111 then b.production_qnty else 0 end) as cur_production_qnty, sum(b.reject_qty) as reject_qty
					from wo_po_color_size_breakdown a left join pro_garments_production_dtls b on a.id=b.color_size_break_down_id and b.production_type in($qty_source,111)
					where a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' and a.is_deleted=0 and a.status_active=1 group by a.item_number_id, a.color_number_id order by  a.color_number_id";
				...
				else if( $variableSettings==3 ) //color and size level
				{
					/*$sql = "select id, item_number_id, size_number_id, color_number_id, order_quantity, plan_cut_qnty, (select sum(CASE WHEN pro_garments_production_dtls.color_size_break_down_id=wo_po_color_size_breakdown.id then production_qnty ELSE 0 END) from pro_garments_production_dtls where is_deleted=0 and  	production_type=1 ) as production_qnty
						...
						$dtlsData = sql_select("SELECT a.color_size_break_down_id,
											sum(CASE WHEN a.production_type=$qty_source then a.production_qnty ELSE 0 END) as production_qnty,
											sum(CASE WHEN a.production_type=111 then a.production_qnty ELSE 0 END) as cur_production_qnty,
											sum(CASE WHEN a.production_type=111 then a.reject_qty ELSE 0 END) as reject_qty
											from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id   and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id <> 0 and a.production_type in(1) group by a.color_size_break_down_id");
				...
				foreach($colorResult as $color)
				{
					...
					else //color and size level
					{
						...
						$colorHTML .='<tr><td>'.$size_library[$color[csf("size_number_id")]].'</td><td><input type="hidden" name="bundlemst" id="bundle_mst_'.$color[csf("color_number_id")].($i+1).'" value="'.$bundle_mst_data.'"  class="text_boxes_numeric" style="width:100px"  ><input type="hidden" name="bundledtls" id="bundle_dtls_'.$color[csf("color_number_id")].($i+1).'"  class="text_boxes_numeric" style="width:100px" value="'.$bundle_dtls_data.'" ><input type="text" name="colorSize" id="colSize_'.$color[csf("color_number_id")].($i+1).'"  class="text_boxes_numeric" style="width:50px" placeholder="'.($cut_qnty-$cur_cut_qnty+$amount).'" onblur="fn_total('.$color[csf("color_number_id")].','.($i+1).');fn_chk_next_process_qty('.$color[csf("color_number_id")].','.($i+1).','.$color[csf("size_number_id")].')" onkeyup="" value="'.$amount.'" ><input type="hidden" name="colorSizeUpQty" id="colSizeUpQty_'.$color[csf("color_number_id")].($i+1).'" value="'.$amount.'" ><input type="text" name="colorSizeRej" id="colSizeRej_'.$color[csf("color_number_id")].($i+1).'"  class="text_boxes_numeric" style="width:50px" placeholder="Rej. Qty" onblur="fn_total_rej('.$color[csf("color_number_id")].','.($i+1).')" value="'.$rej_qnty.'" '.$disable.'></td><td><input type="text" name="button" value="'.$po_color_size_qnty_arr[$color[csf("color_number_id")]][$color[csf("size_number_id")]].'" readonly="" disabled="" class="text_boxes_numeric" style="size:30px;"  /></td></tr>';
						//$colorWiseTotal += $amount;
						 $bundle_dtls_data="";
						 $bundle_dtls_data="";
					}
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$sewing_production_variable)==2)//color level wise
		{
			...
			foreach($rowEx as $rowE=>$val)
			{
				...
				if($j==0)$data_array = "(".$dtls_id.",".$id.",111,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."',".$cbo_color_type.")";
				else $data_array .= ",(".$dtls_id.",".$id.",111,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."',".$cbo_color_type.")";
				//$dtls_id=$dtls_id+1;
 				$j++;
			}
 		}
		...
		if(str_replace("'","",$sewing_production_variable)==3)//color and size wise
		{
		   ...
		   $color_all=implode(",",$color_all_id);
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				if($j==0)$data_array = "(".$dtls_id.",".$id.",111,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$cbo_color_type.")";
				else $data_array .= ",(".$dtls_id.",".$id.",111,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$cbo_color_type.")";
				//$dtls_id=$dtls_id+1;
 				$j++;
			}
		...
		{		 
			...
			foreach($defect_array as $key=>$val)
			{
				...
				$defectPointId=$key;
				$defect_qty=$val;
				$data_array_defect.="(".$dft_id.",".$id.",111,".$hidden_po_break_down_id.",".$defect_type_id.",".$defectPointId.",'".$defect_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			} 
		}
		...
		{		 
			...
			foreach($defectSpt_array as $keysp=>$valsp)
			{
				...
				$defectsp_qty=$valsp;
				$data_array_defectsp.="(".$dftSp_id.",".$id.",111,".$hidden_po_break_down_id.",'".str_replace("'", "", $defectSpot_type_id)."','".$defectspPointId."','".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			} 
		}
  	...
  	else if ($operation==1) // Update Here------------------------------------------------------
	{
		...
		$data_array1="".$cbo_company_name."*".$txt_challan_no."*".$cbo_source."*".$cbo_cutting_company."*".$cbo_location."*".$txt_trim_date."*".$txt_trim_qty."*111*".$sewing_production_variable."*".$cutting_production_variable_reject."*".$txt_reporting_hour."*".$txt_remark."*".$txt_super_visor."*".$cbo_poly_line."*".$cbo_floor."*".$txt_reject_qty."*".$txt_alter_qnty."*".$txt_spot_qnty."*".$txt_cumul_cutting."*".$txt_yet_cut."*".$txt_wo_id."*".$txt_wo_no."*".$hidden_currency_id."*".$hidden_exchange_rate."*'".$rate."'*'".$amount."'*".$user_id."*'".$pc_date_time."'";
		...
		if(str_replace("'","",$sewing_production_variable)!=1 && str_replace("'","",$txt_mst_id)!='')// check is not gross level
		{
			...
			if(str_replace("'","",$sewing_production_variable)==2)//color level wise
			{
				...
				foreach($rowEx as $rowE=>$val)
				{
					...
					if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",111,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."',".$cbo_color_type.")";
					else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",111,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."',".$cbo_color_type.")";
					//$dtls_id=$dtls_id+1;
					$j++;
				}
			}
			...
			if(str_replace("'","",$sewing_production_variable)==3)//color and size wise
			{
				...
				foreach($rowEx as $rowE=>$valE)
				{
					...
					if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",111,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$cbo_color_type.")";
					else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",111,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$cbo_color_type.")";
					//$dtls_id=$dtls_id+1;
					$j++;
				}
			}
		...
		{		 
			...
			foreach($defect_array as $key=>$val)
			{
				...
				$defectPointId=$key;
				$defect_qty=$val;
				$data_array_defect.="(".$dft_id.",".$txt_mst_id.",111,".$hidden_po_break_down_id.",".$defect_type_id.",".$defectPointId.",'".$defect_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			} 
		}
		if($data_array_defect!="")
		{
			...
			$query3=execute_query("DELETE FROM pro_gmts_prod_dft WHERE mst_id=$txt_mst_id and defect_type_id=1 and production_type=111");
			$defectQ=sql_insert("pro_gmts_prod_dft",$field_array_defect,$data_array_defect,1);
		}
		$defectSpot=true;
		$data_array_defectsp="";
		...
		{		 
			...
			foreach($defectSpt_array as $keysp=>$valsp)
			{
				...
				$defectspPointId=$keysp;
				$defectsp_qty=$valsp;
				$data_array_defectsp.="(".$dftSp_id.",".$txt_mst_id.",111,".$hidden_po_break_down_id.",'".str_replace("'", "", $defectSpot_type_id)."','".$defectspPointId."','".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			} 
		}
		...
		if($data_array_defectsp!="")
		{
			...
			$query4=execute_query("DELETE FROM pro_gmts_prod_dft WHERE mst_id=$txt_mst_id and defect_type_id=2 and production_type=111");
			$defectSpot=sql_insert("pro_gmts_prod_dft",$field_array_defectsp,$data_array_defectsp,0);
		}
	...
	else if ($operation==2)  // Delete Here----------------------------------------------------------
	{
		...
		$con = connect();
		if($db_type==0)	{ mysql_query("BEGIN"); }
		$next_process_sql=sql_select("SELECT po_break_down_id from pro_garments_production_mst where status_active=1 and is_deleted=0 and production_type = 111 and po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name and country_id=$cbo_country_name");
		$next_process_sql2=sql_select("SELECT po_break_down_id from pro_cut_delivery_order_dtls where status_active=1 and is_deleted=0 and production_type = 111 and po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name and country_id=$cbo_country_name");
...
if($action=="bundle_preparation")
{
	...
	function calculate_bundle( pcs_per_bund )
	{
		...
		var list_view_bundle = return_global_ajax_value( data, 'load_php_dtls_form', '', 'trimming_complete_controller');
	...
	function fnc_cutting_bundle_entry(operation)
	{
		...
		http.open("POST","trimming_complete_controller.php",true);
	...
	function fnc_operation_sticker_report()
	{
		...
		//alert(data);
		var url=return_ajax_request_value(data, "print_report_operation_barcode", "trimming_complete_controller");
		window.open(url,"##");
	}
	...
	function fnc_bundle_report()
	{
		...
		data=data+"***"+$('#rpt_data' ).val()+"***"+colsize;
		var url=return_ajax_request_value(data, "print_report_bundle_barcode", "trimming_complete_controller");
		window.open(url,"##");
	}
	...
	function fnc_operation_report()
	{
		...
		data=data+"***"+$('#rpt_data' ).val()+"***"+colsize;
		var url=return_ajax_request_value(data, "print_report_operation", "trimming_complete_controller");
		//alert(url);return;
		window.open(url,"##");
	}
	...
	function reset_pcs_per_bundle()
	{
	  ...
	  <script>
	  	get_php_form_data(<? echo $txt_mst_id; ?>+"**"+<? echo $hidden_po_break_down_id; ?>+"**"+document.getElementById('row_id').value, "populate_data_from_bundle", "trimming_complete_controller" );
	 	calculate_bundle(document.getElementById('txt_pcs_per_bendle').value);
	  </script>
	  <script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
	  </html>
