...
if ($action=="load_drop_down_location")
{
	...
	else if($data[1]==2) { $dropdown_name="cbo_working_location"; $load_function="load_drop_down( 'requires/getup_complete_v2_controller', this.value, 'load_drop_down_floor', 'floor_td' );"; }
...
if ($action=="load_drop_down_working_com")
{
	...
	if($data[1]==1)
	{
		echo create_drop_down( "cbo_working_company", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-W. Company-", $data[2], "$load_function");
	}
	else
	{
		echo create_drop_down( "cbo_working_company", 130, "select a.id, a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=23 and a.status_active=1 group by a.id,a.supplier_name order by a.supplier_name","id,supplier_name", 1, "-W. Company-", $data[2], "" );
	}	
	exit();	 
}
...
if ($action=="order_details")
{
	...
	foreach ($sql_result as $row)
	{
		...
		?>
        <tr bgcolor="<?=$bgcolor; ?>">
            <td width="20" align="center"><?=$k; ?></td>
            <td width="110" style="word-break:break-all"><?=$row[csf("po_number")]; ?></td>
            <td width="60"><?=change_date_format($row[csf("pub_shipment_date")]); ?></td>
            ...
            <td width="60"><?=change_date_format($row[csf("country_ship_date")]); ?></td>
            ...
            <td width="70" align="right" id="planCutQty_<?=$k; ?>"><?=$row[csf("plan_cut_qnty")]; ?></td>
            
            <td width="70"><input type="text" name="txtAltQty_<?=$k; ?>" id="txtAltQty_<?=$k; ?>" class="text_boxes_numeric" style="width:53px;" value="<?=$qtyAlt; ?>" onBlur="fnc_total_calculate(this.value,<?=$k; ?>);" /></td>
            <td width="70"><input type="text" name="txtSpot_<?=$k; ?>" id="txtSpot_<?=$k; ?>" class="text_boxes_numeric" style="width:53px;" value="<?=$qtySpt; ?>" onBlur="fnc_total_calculate(this.value,<?=$k; ?>);" /></td>
            <td width="70"><input type="text" name="txtRjtQty_<?=$k; ?>" id="txtRjtQty_<?=$k; ?>" class="text_boxes_numeric" style="width:53px;" value="<?=$qtyRej; ?>" onBlur="fnc_total_calculate(this.value,<?=$k; ?>);" /></td>
            <td width="70"><input type="text" name="txtReIrnQty_<?=$k; ?>" id="txtReIrnQty_<?=$k; ?>" class="text_boxes_numeric" style="width:53px;" value="<?=$qtyRePoly; ?>" onBlur="fnc_total_calculate(this.value,<?=$k; ?>);" /></td>
            
            <td>
            	<input type="text" name="txtQty_<?=$k; ?>" id="txtQty_<?=$k; ?>" class="text_boxes_numeric" style="width:43px;" value="<?=$qty; ?>" placeholder="<?=$placeholderQty; ?>" pre_issue_qty="<?=$old_qty; ?>" onBlur="fnc_total_calculate(this.value,<?=$k; ?>);" />
                <input type="hidden" name="txtpoid_<?=$k; ?>" id="txtpoid_<?=$k; ?>" style="width:30px" class="text_boxes" value="<?=$row[csf("id")]; ?>" />
                <input type="hidden" name="txtColorSizeid_<?=$k; ?>" id="txtColorSizeid_<?=$k; ?>" style="width:30px" class="text_boxes" value="<?=$row[csf("cid")]; ?>" />
                <input type="hidden" name="txtDtlsData_<?=$k; ?>" id="txtDtlsData_<?=$k; ?>" style="width:30px" class="text_boxes" value="<?=$dtlsData; ?>" />
                <input type="hidden" name="txtDtlsUpId_<?=$k; ?>" id="txtDtlsUpId_<?=$k; ?>" style="width:30px" class="text_boxes" value="<?=$dtls_id; ?>" />
            </td>
        </tr>
		<?
	}
...
if($action=="system_number_popup")
{
    ...
    <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
        <table width="850" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
            ...
            <tbody>
                <tr class="general">                    
                    <td><?=create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select Company --","", ""); ?></td>
...
if($action=="create_system_search_list_view")
{
    ...
    where c.garments_nature=100 and a.entry_form=677 and b.entry_form=677 and a.id=b.delivery_mst_id and b.po_break_down_id=d.id and c.job_no=d.job_no_mst $conpany_cond $job_cond $sql_cond $order_cond $system_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
	...
	?>
     <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="985" >
        <thead>
            ...
            <th width="60">Get Up Year</th>
            <th width="60">Get Up No</th>
            <th width="120">Working Company</th>
            <th width="120">Working Location</th>
            <th width="80">Floor</th>
            <th width="70">Get Up Date</th>
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if (str_replace("'","",$txt_update_id)=="")
		{
			...
			$new_sys_number = explode("*", return_next_id_by_sequence("", "pro_gmts_delivery_mst",$con,1,$cbo_company_name,'GTU',0,date("Y",time()),0,0,113,0,0 ));
			
			$field_array_delivery = "id, sys_number_prefix, sys_number_prefix_num, sys_number, company_id, production_type, location_id, production_source, working_company_id, working_location_id, floor_id, job_id, job_no, delivery_date, challan_no, remarks, entry_form, inserted_by, insert_date, status_active, is_deleted";
			$mst_id = return_next_id_by_sequence("pro_gmts_delivery_mst_seq", "pro_gmts_delivery_mst", $con );
				
			$data_array_delivery="(".$mst_id.",'".$new_sys_number[1]."','".(int)$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",113,".$cbo_location.",".$cbo_source.",".$cbo_working_company.",".$cbo_working_location.",".$cbo_floor.",".$txt_job_id.",".$txt_job_no.",".$txt_getup_date.",".$txt_challan.",".$txt_remark.",677,".$user_id.",'".$pc_date_time."',1,0)";
		}
		else
		{
			$mst_id = str_replace("'","",$txt_update_id);
			
			$field_array_delivery = "location_id*production_source*working_company_id*working_location_id*floor_id*job_id*job_no*delivery_date*challan_no*remarks*updated_by*update_date";
			$data_array_delivery = "".$cbo_location."*".$cbo_source."*".$cbo_working_company."*".$cbo_working_location."*".$cbo_floor."*".$txt_job_id."*".$txt_job_no."*".$txt_getup_date."*".$txt_challan."*".$txt_remark."*".$user_id."*'".$pc_date_time."'";
		}
		...
		if($db_type==2) 
		{
			$txt_reporting_hour=str_replace("'","",$txt_getup_date)." ".str_replace("'","",$txt_reporting_hour);
			$txt_reporting_hour="to_date('".$txt_reporting_hour."','DD MONTH YYYY HH24:MI:SS')";
		}
		
		$field_array_mst="id, delivery_mst_id, garments_nature, company_id, location, production_source, floor_id, production_date, production_hour, challan_no, remarks, po_break_down_id, item_number_id, country_id, serving_company, production_quantity, alter_qnty, spot_qnty, reject_qnty, re_production_qty, production_type, entry_break_down_type, entry_form, inserted_by, insert_date, status_active, is_deleted";
		$data_array_mst="";
		...
		foreach($mstArr as $orderId=>$orderData)
		{
			foreach($orderData as $gmtsItemId=>$gmtsItemIdData)
			{
				foreach($gmtsItemIdData as $countryId=>$qtyData)
				{
					...
					if($db_type==2)
					{
						$data_array_mst.=" INTO pro_garments_production_mst (".$field_array_mst.") VALUES(".$id.",".$mst_id.",".$garments_nature.",".$cbo_company_name.",".$cbo_location.",".$cbo_source.",".$cbo_floor.",".$txt_getup_date.",".$txt_reporting_hour.",".$txt_challan.",".$txt_remark.",".$orderId.", ".$gmtsItemId.",".$countryId.",".$cbo_working_company.",".$qtyData['qc'].",".$qtyData['alt'].",".$qtyData['spt'].",".$qtyData['rej'].",".$qtyData['reirn'].",113,3,677,".$user_id.",'".$pc_date_time."',1,0)";
					}
					else
					{
						if($data_array_mst!="") $data_array_mst.=",";
						$data_array_mst.="(".$id.",".$mst_id.",".$garments_nature.",".$cbo_company_name.",".$cbo_location.",".$cbo_source.",".$cbo_floor.",".$txt_getup_date.",".$txt_reporting_hour.",".$txt_challan.",".$txt_remark.",".$orderId.", ".$gmtsItemId.",".$countryId.",".$cbo_working_company.",".$qtyData['qc'].",".$qtyData['alt'].",".$qtyData['spt'].",".$qtyData['rej'].",".$qtyData['reirn'].",113,3,677,".$user_id.",'".$pc_date_time."',1,0)";
					}
		...
		foreach($dtlsArr as $colorSizeid=>$qtyStr)
		{
			...
			$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",113,'".$colorSizeid."','".$qtyStr['qc']."','".$qtyStr['alt']."','".$qtyStr['spt']."','".$qtyStr['rej']."','".$qtyStr['reirn']."',677,1,0)"; 
		}
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$field_array_up="location*production_source*floor_id*production_date*production_hour*challan_no*remarks*po_break_down_id*item_number_id*country_id*serving_company*production_quantity*alter_qnty*spot_qnty*reject_qnty*re_production_qty*updated_by*update_date";
			...
			if($orderId)
			{
				foreach ($orderData as $gmtsItemId => $gmtsItemIdData)
				{
					foreach ($gmtsItemIdData as $countryId =>$qtyData) 
					{
						...
						{
							$data_array_up[$gmtProdId] =explode("*",("".$cbo_location."*".$cbo_source."*".$cbo_floor."*".$txt_getup_date."*".$txt_reporting_hour."*".$txt_challan."*".$txt_remark."*".$orderId."*".$gmtsItemId."*".$countryId."*".$cbo_working_company."*'".$qtyData['qc']."'*'".$qtyData['alt']."'*'".$qtyData['spt']."'*'".$qtyData['rej']."'*'".$qtyData['reirn']."'*'".$user_id."'*'".$pc_date_time."'"));
							$id=$gmtProdId;
							$is_up=1;
							$id_arr[]=$gmtProdId;
						}
		...
		$dtlsrDelete = execute_query("update pro_garments_production_dtls set status_active=0, is_deleted=1 WHERE delivery_mst_id=$mst_id and production_type=113 and status_active=1 and is_deleted=0");
...
<div style="width:930px;">
    ...
    <table align="right" cellspacing="0" width="900" >
        ...
        <br>
		 <?=signature_table(26, $data[0], "900px"); ?>
	</div>
	</div>
	<?
    exit();
