...
{ 
	...
	echo create_drop_down( "cbo_country_id",100, "SELECT a.id,a.country_name from lib_country a inner join wo_po_color_size_breakdown b on a.id=b.country_id left join week_of_year c on c.week_date=b.country_ship_date $con_week  where $con  and b.status_active=1 and b.is_deleted=0 group by a.id,a.country_name order by a.country_name","id,country_name", 1, "--Select--", "", "get_php_form_data( document.getElementById('cbo_order_id').value+','+document.getElementById('cbo_week_no').value+','+this.value+','+document.getElementById('cbo_inspection_level').value, 'set_po_qnty_ship_date', 'requires/buyer_inspection_controller')","","","","","","" );
}
...
if ($action=="order_search_popup")
{
    ...
    <table width="1100" cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center" rules="all">
        <thead>                	 
            ...
            <th width="90">Internal Ref No.</th>
        ...
        <tr class="general">
            <td> <input type="hidden" id="selected_job">
                <?=create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", '',"load_drop_down( 'buyer_inspection_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" ); ?>
           </td>
           <td id="buyer_td"><?=create_drop_down( "cbo_buyer_name", 172, $blank_array,'', 1, "-- Select Buyer --" ); ?></td>
           <td id="search_td"><input name="txt_order" id="txt_order"  class="text_boxes" style="width:80px"></td>
           <td id="search_td"><input name="txt_file" id="txt_file"  class="text_boxes" style="width:80px"></td>
           <td id="search_td"><input name="txt_inter_ref" id="txt_inter_ref"  class="text_boxes" style="width:80px"></td>
           <td><input name="txt_style_ref" id="txt_style_ref"  class="text_boxes" style="width:80px"></td>
...
if($action=="create_po_search_list_view")
{
	...
	$data=explode('_',$data);
	//var_dump($data);
	//echo "setFieldLevelAccess($data[0]);\n";
	...
	function fnc_close( )
		{
			...
			for(var i=1; i<=rowCount; i++)
			{
				...
				if(breck_down_data=="")
				{
					breck_down_data+=$('#itemId_'+i).val()+'_'+($('#colorId_'+i).val()*1)+'_'+($('#colorQty_'+i).val()*1)+'_'+($('#itemColorInsQty_'+i).val()*1);
				}
				else
				{
					breck_down_data+="----"+$('#itemId_'+i).val()+'_'+($('#colorId_'+i).val()*1)+'_'+($('#colorQty_'+i).val()*1)+'_'+($('#itemColorInsQty_'+i).val()*1);
		...
		<tbody>
			...
			if($hidden_ins_data)
			{
				...
				{	
					...
					<tr>
						<td><? echo $i ;?></td>
						<td><? echo $garments_item[$val[0]];?>
						<input type="hidden" id="itemId_<? echo $i;?>" name="itemColorInsQty<? echo $i;?>" value="<? echo $val[0];?>">
						</td>
						<td><? echo $color_arr[$val[1]];?>
							<input type="hidden" id="colorId_<? echo $i;?>" name="colorId_<? echo $i;?>" value="<? echo $val[1];?>">
						</td>
						<td align="right"><? echo $val[csf("qnty")];?>
								<input type="text" id="colorQty_<? echo $i;?>" name="colorQty_<? echo $i;?>" disabled="" readonly value="<? echo $val[2];?>">
						</td>
						<td><input type="text_boxes_numeric" id="itemColorInsQty_<? echo $i;?>" name="itemColorInsQty_<? echo $i;?>" value="<? echo $val[3]?>"></td>
					</tr>
					<?
					$i++;
				}
			...
			else
			{
				...
				{				
					...
					<tr>
						<td><? echo $i ;?></td>
						<td><? echo $garments_item[$val[csf("item_number_id")]];?>
						<input type="hidden" id="itemId_<? echo $i;?>" name="itemColorInsQty<? echo $i;?>" value="<? echo $val[csf("item_number_id")];?>">
						</td>
						<td><? echo $color_arr[$val[csf("color_number_id")]];?>
							
							<input type="hidden" id="colorId_<? echo $i;?>" name="colorId_<? echo $i;?>" value="<? echo $val[csf("color_number_id")];?>">
						</td>
						<td align="right"><? echo $val[csf("qnty")];?>
								<input type="hidden" id="colorQty_<? echo $i;?>" name="colorQty_<? echo $i;?>" value="<? echo $val[csf("qnty")];?>">
						</td>
						<td><input type="text_boxes_numeric" id="itemColorInsQty_<? echo $i;?>" name="itemColorInsQty_<? echo $i;?>" value=""></td>
					</tr>
					<?
					$i++;
				}
...
if($action=="set_po_qnty_ship_date")
{
	list($order_id,$week_id,$country_id,$inspection_level,$update_id)=explode(",",$data);
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$field_array="id,job_no,ins_reason,all_data,po_break_down_id,actual_po_id,inspection_company,source, working_company, 
   working_location, working_floor,inspected_by,week_id,country_id,inspection_date,inspection_qnty,inspection_status,inspection_level,inspection_cause,comments,inserted_by,insert_date";
...
if($action=="show_active_listview")
{
	...
	$po_number=return_library_array( "select id,po_number from wo_po_break_down where status_active in(1,2,3) and job_no_mst='$data'  and is_deleted=0", "id", "po_number");
...
if($action=="populate_inspection_details_form_data")
{
	...
	foreach ($data_array as $row)
	{
		...
		echo "document.getElementById('cbo_working_floor').value = '".$row[csf("working_floor")]."';\n"; 
	 	...
	 	echo "get_php_form_data('".$row[csf("po_break_down_id")].",".$row[csf("week_id")].",".$row[csf("country_id")].",".$row[csf("inspection_level")].",".$row[csf("id")]."','set_po_qnty_ship_date', 'requires/buyer_inspection_controller');\n";
