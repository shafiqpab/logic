...
if ($action == "load_variable_settings") {
	...
	$control_and_preceding = sql_select("select is_control, preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=50 and page_category_id=120 and company_name='$data'");
	...
	$qty_source = 0;
	if ($preceding_process == 8) $qty_source = 8; //Packing And Finishing
	...
	<body>
		<div align="center" style="width:100%;">
			<form name="searchorderfrm_1" id="searchorderfrm_1" autocomplete="off">
				<table width="1150" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
					<thead>
...
if ($action == "create_delivery_search_list") 
{
	...
	if($txt_challan_no=="" && $po_no=="" && $job_no=="" && $txt_date_from=="" && $txt_date_to=="")
	{
	...
	if ($db_type == 0) 
	{
		...
		where a.id=b.delivery_mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form!=85 $sql_cond $po_cond $buyerCond $job_cond
	...
	else 
	{
		...
		where a.id=b.delivery_mst_id and c.id=b.po_break_down_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form!=85 $sql_cond $po_cond $buyerCond $job_cond
	...
	$order_sql = sql_select("SELECT a.id,a.job_no_mst, a.po_number,(a.po_quantity) as po_quantity,a.shiping_status  from wo_po_break_down a,GBL_TEMP_ENGINE tmp where a.id=tmp.ref_val and tmp.entry_form=71  and tmp.user_id=$user_id and tmp.ref_from=1 and a.status_active=1");
...
if ($action == "show_dtls_listview_mst") 
{
	...
	<div style="width:1200px;max-height:180px; overflow:y-scroll" id="sewing_production_list_view" align="left">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" id="details_table">
			...
			$style_sql = "SELECT a.style_ref_no,b.id,b.po_number,b.po_quantity from wo_po_details_master a, wo_po_break_down b,GBL_TEMP_ENGINE tmp where a.id=b.job_id and b.id=tmp.ref_val and tmp.entry_form=71  and tmp.user_id=$user_id and tmp.ref_from=1";
...
if ($action == "populate_data_from_search_popup") 
{
	...
	$ex_fac_qty = return_field_value("sum(b.production_qnty) as production_qnty", "pro_ex_factory_mst a,pro_ex_factory_dtls b ", " a.id=b.mst_id and a.id='$ex_mst_id'  and a.status_active=1 and a.entry_form<>85 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0", "production_qnty");
	...
	foreach ($res as $result) 
	{
		...
		if ($qty_source != 0) 
		{
			...
			else 
			{
				...
				{
					$finish_qty = sql_select("SELECT sum(case when b.production_type='$qty_source' then b.production_qnty else 0 end) as production_qnty,(sum( CASE WHEN b.trans_type=5 and b.production_type=10 THEN b.production_qnty ELSE 0 END)-sum( CASE WHEN b.trans_type=6 and b.production_type=10 THEN b.production_qnty ELSE 0 END)) as trans_qnty from pro_garments_production_mst a ,pro_garments_production_dtls b where  a.id=b.mst_id and a.po_break_down_id=" . $result[csf('id')] . " and a.item_number_id='$item_id' and b.production_type in($qty_source,10) and a.country_id='$country_id' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
				}
				// receive from fin gmts order to order transfer
...
if ($action == "color_and_size_level") 
{
	...
	if($variableSettings==4)// actual po wise
	{
		exit();
	}
	...
	//echo $qty_source.'-'.$variableSettings;
	if ($qty_source != 0) {
		if ($variableSettings == 2) // color level
		{
			...
			else 
			{
				...
				{
					$sql = "SELECT a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty,
					(sum(CASE WHEN b.production_type='$qty_source' and c.purpose_id not in(2,3) then b.production_qnty ELSE 0 END) - sum(case when b.production_type=83 and c.purpose_id not in(2,3) then b.production_qnty else 0 end)) as production_qnty,
		...
		else if ($variableSettings == 3) //color and size level
		{
			...
			else
			{
				...
				from pro_garments_production_dtls a,pro_garments_production_mst b,wo_po_color_size_breakdown c where a.status_active=1 and a.mst_id=b.id and c.id=a.color_size_break_down_id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id<>0 and a.production_type=10 $ship_date_cond3 $pack_type_cond3 group by a.color_size_break_down_id");
			...
			// pre($color_size_pro_qnty_array); die;
			$sql_exfac = sql_select("SELECT a.item_number_id,a.color_number_id,a.size_number_id,sum(case when m.entry_form<>85 then  ex.production_qnty else 0 end ) - sum(case when m.entry_form=85 then  ex.production_qnty else 0 end ) as ex_production_qnty from wo_po_color_size_breakdown a,pro_ex_factory_mst m
	                    ...
	                    and a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' $ship_date_cond $pack_type_cond and a.is_deleted=0 and a.status_active=1 group by a.item_number_id, a.color_number_id, a.size_number_id");
			foreach ($sql_exfac as $row_exfac) {
	...
	else // if preceding process =0 in variable setting then plan cut quantity will show
	{
		if ($variableSettings == 2) // color level
		{
			...
			$dtlsData = sql_select("SELECT a.color_number_id, sum(case when m.entry_form<>85 then b.production_qnty else 0 end ) - sum(case when m.entry_form=85 then b.production_qnty else 0 end ) as production_qnty
				from wo_po_color_size_breakdown a ,pro_ex_factory_mst m, pro_ex_factory_dtls b where a.po_break_down_id=m.po_break_down_id and m.id=b.mst_id and m.status_active=1 and m.is_deleted=0 and  a.id=b.color_size_break_down_id
				and  a.po_break_down_id='$po_id' and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 group by a.color_number_id");
			...
			from wo_po_color_size_breakdown a
			where a.po_break_down_id='$po_id' and a.is_deleted=0 and a.status_active=1 group by a.color_number_id order by a.color_number_id"; //color_number_id, id		
		...
		else if ($variableSettings == 3) //color and size level
		{
			...
			$dtlsData = sql_select("SELECT a.color_size_break_down_id,
										...
										from pro_ex_factory_dtls a,pro_ex_factory_mst b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id<>0 and a.is_deleted=0 and a.status_active=1  group by a.color_size_break_down_id");
	...
	if ($qty_source != 0) {
		foreach ($colorResult as $color) {
			...
			else //color and size level
			{
				...
				$colorHTML .= '<tr><td>' . $size_library[$color[csf("size_number_id")]] . '</td><td><input type="text" name="colorSize" id="colSize_' . $color[csf("color_number_id")] . ($i + 1) . '"  class="text_boxes_numeric" style="width:100px" placeholder="' . ($pro_qnty - $exfac_qnty) . '" onkeyup="fn_total(' . $color[csf("color_number_id")] . ',' . ($i + 1) . ')"></td></tr>';
			}
			$i++;
		}
	}
...
?>
	...
	<div id="scroll_body" style="width:450px; max-height:450px; overflow-x:hidden;  overflow-y:auto;">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="430" class="rpt_table" id="tbl_body_1">
			<?
			$i = 1;
			...
			foreach ($sqlResult as $row) {
				...
				<tr bgcolor="<?=$bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="put_country_data(<?=$row[csf('po_break_down_id')].",".$row[csf('item_number_id')].",".$row[csf('country_id')].",".$row[csf('order_qnty')].",".$row[csf('plan_cut_qnty')].",'".$row[csf('country_ship_date')]."','" .$row[csf('pack_type')]."'"; ?>);">
					...
					<td width="50" style="word-break:break-all"><?=$cut_up_array[$row[csf('cutup')]]; ?>&nbsp;</td>
					...
					<td align="right" style="word-break:break-all"><?=$row[csf('order_qnty')]; ?></td>
				</tr>
				<? $i++;
			}
			?>
...
if ($action == "populate_exfactory_form_data")
{
	...
	*/
	...
	$control_and_preceding = sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=50 and page_category_id=120 and company_name='$company_id'");
	$preceding_process = $control_and_preceding[0][csf("preceding_page_id")];
	// echo "select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=50 and page_category_id=120 and company_name='$company_id'";die;
	echo "$('#hidden_variable_cntl').val('" . $control_and_preceding[0][csf('is_control')] . "');\n";


	$qty_source = 0;
	// if ($preceding_process == 120) $qty_source = 8; //Packing And Finishing
	$qty_source = $preceding_process;
			...
			if ($qty_source != 0) 
			{
				if ($variableSettings != 1) // gross level
				{
					...
					else if ($variableSettings == 3) //color and size level
					{
						...
						$sql_exfac = sql_select("SELECT a.item_number_id,a.color_number_id,a.size_number_id,SUM (case when m.entry_form!=85 then ex.production_qnty else 0 end) - SUM (case when m.entry_form=85 then ex.production_qnty else 0 end) as ex_production_qnty from wo_po_color_size_breakdown a
								...
								where a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' $ship_date_cond $pack_type_cond and a.is_deleted=0 and a.status_active in(1,2,3) group by a.item_number_id, a.color_number_id, a.size_number_id  order by a.item_number_id, a.color_number_id, a.size_number_id");
						foreach ($sql_exfac as $row_exfac) {
							$ex_fac_value[$row_exfac[csf("item_number_id")]][$row_exfac[csf("color_number_id")]][$row_exfac[csf("size_number_id")]] = $row_exfac[csf("ex_production_qnty")];
						}
					...
					foreach ($colorResult as $color) {
						...
						else //color and size level
						{
							...
							$colorHTML .= '<tr><td>' . $size_library[$color[csf("size_number_id")]] . '</td><td><input type="text" name="colorSize" id="colSize_' . $color[csf("color_number_id")] . ($i + 1) . '"  class="text_boxes_numeric" style="width:100px" placeholder="' . ($pro_qnty - $exfac_qnty + $amount) . '" onkeyup="fn_total(' . $color[csf("color_number_id")] . ',' . ($i + 1) . ')" value="' . $amount . '"' . $disabled . '></td></tr>';
...
if ($action == "save_update_delete") 
{
	...
	$is_control = return_field_value("is_control", "variable_settings_production", "company_name=$cbo_company_name and variable_list=50 and page_category_id=120");
	...
	if ($operation == 0) // Insert Here----------------------------------------------------------
	{
		...
		$country_exfactory_qty = return_field_value("sum(case when entry_form !=85 then ex_factory_qnty else 0 end) - sum(case when entry_form =85 then ex_factory_qnty else 0 end) as ex_factory_qnty", "pro_ex_factory_mst", "po_break_down_id=$hidden_po_break_down_id and country_id=$cbo_country_name $pack_type_cond $country_ship_date_cond and status_active=1 and is_deleted=0");
		...
		$prodData = sql_select("SELECT c.size_number_id, c.color_number_id,sum(a.production_qnty) as production_qnty
								...
								union all 
								select c.size_number_id,c.color_number_id,(sum( CASE WHEN b.trans_type=5 THEN a.production_qnty ELSE 0 END)-sum( CASE WHEN b.trans_type=6 THEN a.production_qnty ELSE 0 END)) as production_qnty
		...
		$sql_exfac = sql_select("SELECT c.size_number_id, c.color_number_id, sum(case when b.entry_form<>85 then a.production_qnty else 0 end )- sum(case when b.entry_form=85 then a.production_qnty else 0 end ) as ex_production_qnty
							...
							group by c.size_number_id,c.color_number_id");
		$ex_fac_data = array();
		$ex_fac_color_size_data = array();
		foreach ($sql_exfac as $row_exfac) {
		...
		if (str_replace("'", "", $sewing_production_variable) == 3) //color and size wise
		{
			...
			foreach ($rowEx as $rowE => $valE) {
				...
				if($colSizeID_arr[$index]!="")
				{
					...
					if ($j == 0) $data_array = "(" . $dtls_id . "," . $id . ",'" . $colSizeID_arr[$index] . "','" . $colorSizeValue . "','" . $cost_of_fab_per_pcs . "','" . $cost_per_pcs . "','".$cut_fab_cost."','" . $cut_oh . "','" . $print_cost . "','" . $emb_cost . "','" . $wash_cost . "','" . $sew_oh . "','" . $sew_trims_cost . "','" . $fin_oh . "','" . $fin_trims_cost . "')";
					else $data_array .= ",(" . $dtls_id . "," . $id . ",'" . $colSizeID_arr[$index] . "','" . $colorSizeValue . "','" . $cost_of_fab_per_pcs . "','" . $cost_per_pcs . "','".$cut_fab_cost."','" . $cut_oh . "','" . $print_cost . "','" . $emb_cost . "','" . $wash_cost . "','" . $sew_oh . "','" . $sew_trims_cost . "','" . $fin_oh . "','" . $fin_trims_cost . "')";
					$dtls_id = $dtls_id + 1;
					$j++;
				}
			}
		...
		// echo "10**insert into pro_ex_factory_mst ($field_array1) values $data_array1";die;
		$rID = sql_insert("pro_ex_factory_mst", $field_array1, $data_array1, 1);
		$DeliveryrID = true;
		...
		//release lock table
		//check_table_status( $_SESSION['menu_id'],0);
		// echo "10**$rID && $DeliveryrID && $dtlsrID && $sts_ex_mst && $sts_country && $sts_ex && $invoiceID && $actualPoId";die;
	...
	else if ($operation == 1) // Update Here End------------------------------------------------------
	{
		...
		if ($is_gate_passed != "") {
			...
			$rID = false;
			$rID2 = false;
			$field_array1 = "invoice_no*lc_sc_no*updated_by*update_date";
			$data_array1 = "'" . $invoice_id . "'*'" . $lcsc_id . "'*" . $user_id . "*'" . $pc_date_time . "'";
		...
		$country_exfactory_qty = return_field_value("sum(case when entry_form !=85 then ex_factory_qnty else 0 end) - sum(case when entry_form =85 then ex_factory_qnty else 0 end) as ex_factory_qnty", "pro_ex_factory_mst", "po_break_down_id=$hidden_po_break_down_id and country_id=$cbo_country_name $pack_type_cond $country_ship_date_cond and status_active=1 and is_deleted=0 and id<>$txt_mst_id");
		$country_exfactory_qty = $country_exfactory_qty + str_replace("'", "", $txt_ex_quantity);
		...
		if (str_replace("'", "", $sewing_production_variable) != 1 && str_replace("'", "", $txt_mst_id) != '') // check is not gross level
		{
			...
			if (str_replace("'", "", $sewing_production_variable) == 3) //color and size wise
			{
				...
				foreach ($rowEx as $rowE => $valE) {
					...
					if($colSizeID_arr[$index]!="")
					{
						if ($j == 0) $data_array = "(" . $dtls_id . "," . $txt_mst_id . ",'" . $colSizeID_arr[$index] . "','" . $colorSizeValue . "','" . $cost_of_fab_per_pcs. "','" . $cost_per_pcs. "','" . $cut_fab_cost . "','" . $cut_oh . "','" . $print_cost . "','" . $emb_cost . "','" . $wash_cost . "','" . $sew_oh . "','" . $sew_trims_cost . "','" . $fin_oh . "','" . $fin_trims_cost . "')";
						else $data_array .= ",(" . $dtls_id . "," . $txt_mst_id . ",'" . $colSizeID_arr[$index] . "','" . $colorSizeValue . "','" . $cost_of_fab_per_pcs. "','" . $cost_per_pcs. "','" . $cut_fab_cost . "','" . $cut_oh . "','" . $print_cost . "','" . $emb_cost . "','" . $wash_cost . "','" . $sew_oh . "','" . $sew_trims_cost . "','" . $fin_oh . "','" . $fin_trims_cost . "')";
						$dtls_id = $dtls_id + 1;
						$j++;
					}
				}
...
if ($action == "actual_po_action_popup") 
{
	...
	<body>
		...
		<div style="width:690px; max-height:400px; overflow-y:scroll" id="list_container_batch" align="left">
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="670" class="rpt_table" id="tbl_list_search">
				<?
				$country_library = return_library_array("select id,country_name from lib_country", "id", "country_name");
				...
				$i = 1;
				if($acc_po_entry_controll==2)
				{
					$sql = "SELECT a.id,a.acc_po_no,a.acc_ship_date,a.id as dtls_id,a.acc_PO_QTY as acc_po_qty,a.gmts_item,a.gmts_color_id,a.country_id,a.gmts_size_id,a.acc_po_qty,a.acc_ship_date
					FROM wo_po_acc_po_info a WHERE a.status_active=1 and a.is_deleted=0 and a.po_break_down_id in($po_id)
					order by a.acc_ship_date,a.id,a.country_id,a.gmts_item,a.gmts_color_id,a.gmts_size_id";
				}
				else
				{
			...
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="850" class="rpt_table" id="tbl_list_search">
				...
				foreach ($result as $row) 
				{
					...
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i; ?>">
						...
						<td width="60" align="right"><input type="text" class="text_boxes_numeric" id="acc_po_qty" name="acc_po_qty" value="<?=$act_po_data_ar[$row[csf('dtls_id')]];?>" style="width:50px" onKeyUp="fn_set_grnd_total();" placeholder="<?=$yet_to_delv_qty;?>"></td>
...
if ($action == "ExFactoryPrintSonia") {
	...
	$delivery_mst_sql = sql_select("SELECT id, sys_number, transport_supplier, driver_name, truck_no, dl_no, lock_no, destination_place,challan_no,sys_number_prefix_num,sys_number,mobile_no,do_no,gp_no,forwarder,forwarder_2,delivery_company_id,delivery_location_id,delivery_floor_id,attention,remarks,delivery_date,depo_details,TO_CHAR(insert_date,'HH24:MI') as delv_time
	from pro_ex_factory_delivery_mst where id=$data[1] and entry_form!=85");
...
if ($action == "ex_factory_print_new9") {
	extract($_REQUEST);
	$data = explode('*', $data);
	$id_ref = str_replace("'", "", $data[4]);
	$show_hide_delv_info = str_replace("'", "", $data[5]);
