...
if ($action=="load_drop_down_location")
{
	...
	else if($data[1]==2) { $dropdown_name="cbo_poly_location"; $load_function="load_drop_down( 'requires/technical_attachment_complete_controller', this.value, 'load_drop_down_floor', 'floor_td' );"; }
...
{
 	echo create_drop_down( "cbo_floor", 130, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process in (7,8,9) order by floor_name","id,floor_name", 1, "--Select Floor--", $selected, "load_drop_down( 'requires/technical_attachment_complete_controller', this.value+'_'+document.getElementById('cbo_location').value+'_'+document.getElementById('txt_poly_date').value, 'load_drop_down_poly_line_floor', 'poly_line_td' );",0 );
	exit();
}
...
if ($action=="load_drop_down_working_com")
{
	...
	if($data[1]==1)
	{
		echo create_drop_down( "cbo_poly_company", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-Atch Company-", $data[2], "$load_function");
	}
	else
	{
		echo create_drop_down( "cbo_poly_company", 130, "select a.id, a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=23 and a.status_active=1 group by a.id,a.supplier_name order by a.supplier_name","id,supplier_name", 1, "-Atch Company-", $data[2], "" );
	}	
	exit();	 
}
    ...
    <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            ...
            <tbody>
                <tr class="general">                    
                    ...
                    <td>
                    	<input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('txt_poly_no').value+'_'+document.getElementById('txt_job_search').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_order_search').value+'_'+document.getElementById('txt_style_ref').value, 'create_system_search_list_view', 'search_div', 'technical_attachment_complete_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />				
                    </td>
                </tr>
                <tr>                  
...
if($action=="create_system_search_list_view")
{
    ...
    where c.garments_nature=100 and a.entry_form=712 and b.entry_form=712 and a.id=b.delivery_mst_id and b.po_break_down_id=d.id and c.job_no=d.job_no_mst $conpany_cond $job_cond $sql_cond $order_cond $system_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if (str_replace("'","",$txt_update_id)=="")
		{
			...
			$new_sys_number = explode("*", return_next_id_by_sequence("", "pro_gmts_delivery_mst",$con,1,$cbo_company_name,'ATTC',0,date("Y",time()),0,0,93,0,0 ));
			...
			$data_array_delivery="(".$mst_id.",'".$new_sys_number[1]."','".(int)$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",93,".$cbo_location.",".$cbo_source.",".$cbo_poly_company.",".$cbo_poly_location.",".$cbo_floor.",".$txt_job_id.",".$txt_job_no.",".$txt_poly_date.",".$cbo_poly_line.",".$txt_challan.",".$txt_remark.",712,".$user_id.",'".$pc_date_time."',1,0)";
		}
		else
		{
		...
		foreach($mstArr as $orderId=>$orderData)
		{
			foreach($orderData as $gmtsItemId=>$gmtsItemIdData)
			{
				foreach($gmtsItemIdData as $countryId=>$qtyData)
				{
					...
					if($db_type==2)
					{
						$data_array_mst.=" INTO pro_garments_production_mst (".$field_array_mst.") VALUES(".$id.",".$mst_id.",".$garments_nature.",".$cbo_company_name.",".$cbo_location.",".$cbo_source.",".$cbo_poly_line.",".$cbo_floor.",".$txt_poly_date.",".$txt_reporting_hour.",".$txt_challan.",".$txt_remark.",".$orderId.", ".$gmtsItemId.",".$countryId.",".$cbo_poly_company.",".$qtyData['qc'].",".$qtyData['alt'].",".$qtyData['spt'].",".$qtyData['rej'].",".$qtyData['reirn'].",93,3,712,".$user_id.",'".$pc_date_time."',1,0)";
					}
					else
					{
		...
		foreach($dtlsArr as $colorSizeid=>$qtyStr)
		{
			...
			$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",93,'".$colorSizeid."','".$qtyStr['qc']."','".$qtyStr['alt']."','".$qtyStr['spt']."','".$qtyStr['rej']."','".$qtyStr['reirn']."',712,1,0)"; 
		}
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$dtlsrDelete = execute_query("update pro_garments_production_dtls set status_active=0, is_deleted=1 WHERE delivery_mst_id=$mst_id and production_type=93 and status_active=1 and is_deleted=0");
