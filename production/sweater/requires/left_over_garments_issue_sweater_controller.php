...
{ 
	echo create_drop_down( "cbo_location_name", 120, "select location_name,id from lib_location where is_deleted=0  and status_active=1 and company_id='$data' order by location_name",'id,location_name', 1, '--- Select Location ---', 0, "load_drop_down( 'left_over_garments_issue_sweater_controller', this.value, 'load_drop_down_store_name2', 'cbo_store_name' );" );     	 
}
...
if($action=="create_po_search_list_view")
{
	...
	switch ($txt_search_by) 
	{
		...
		case 4:
			...
			break;
		case 6:
			$sql_cond = " and c.grouping like '%".trim($txt_search_common)."%'";
			break;		
		default:
			$sql_cond = " and b.order_no like '%".trim($txt_search_common)."%'";
			break;
	...
	$sql ="SELECT a.company_id, a.location, b.po_break_down_id, b.order_no, b.style_ref_no, b.item_number_id,a.store_name, sum(b.total_left_over_receive) as rcv_qty $country_select from pro_leftover_gmts_rcv_mst a, pro_leftover_gmts_rcv_dtls b, wo_po_break_down c where a.id=b.mst_id and b.po_break_down_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $sql_cond group by a.company_id, a.location, b.po_break_down_id, b.order_no, b.style_ref_no, b.item_number_id,a.store_name $country_select";
...
{	
	...
	$dataEx = explode("**", $data);
	// ============================= GETTING RECEIVE QTY ============================
	$sqlrRcv ="SELECT b.po_break_down_id as po_id, b.item_number_id as item_id, b.country_id, sum(b.total_left_over_receive) as rcv_qnty from pro_leftover_gmts_rcv_mst a, pro_leftover_gmts_rcv_dtls b where a.id=b.mst_id and b.po_break_down_id=$dataEx[0] and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.po_break_down_id, b.item_number_id, b.country_id";
	...
	$sqlLeft ="SELECT b.po_break_down_id as po_id, b.item_number_id as item_id, b.country_id, sum(b.total_issue) as issue_qnty from pro_leftover_gmts_issue_mst a, pro_leftover_gmts_issue_dtls b where a.id=b.mst_id and b.po_break_down_id=$dataEx[0] and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.po_break_down_id, b.item_number_id, b.country_id";
	// echo $sql;
...
if($action=="system_number_popup")
{
	...
	<script>
		$(document).ready(function(e) {
            //$("#txt_search_common").focus();
			load_drop_down( 'left_over_garments_issue_sweater_controller','<? echo $company; ?>', 'load_drop_down_location2', 'location_td' );
        });
	        		...
	        		<td align="center" width="100%">
	                    <table ellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
	        				...
	        				<tr>
	                        	<td>                                
									<? 
									echo create_drop_down( "cbo_company_name", 170, "select company_name,id from lib_company comp where is_deleted=0  and status_active=1 order by company_name",'id,company_name', 1, '--- Select Company ---',$company, "load_drop_down( 'left_over_garments_issue_sweater_controller', this.value, 'load_drop_down_location2', 'location_td' );");?>
								</td>
	                             <td id="location_td">
									 <?
...
if($action=="populate_mst_form_data")
{
	...
	$result =sql_select($sql);
	echo"load_drop_down( 'requires/left_over_garments_issue_sweater_controller', '".$result[0][csf('company_id')]."_".$result[0][csf('location')]."', 'load_drop_down_store_name', 'store_name_td' );\n"; 
...
if($action=="get_left_over_balance")
{
	...
	$country_cond = ($country_maintain==1) ? " and b.country_id=$country_id" : "";
	$sql_leftover = "SELECT sum(b.total_left_over_receive) as leftover_qty from pro_leftover_gmts_rcv_mst a, pro_leftover_gmts_rcv_dtls b where a.id=b.mst_id and b.po_break_down_id=$po_id $country_cond $order_type_cond and b.item_number_id=$gmts_item and a.status_active=1 and b.status_active=1";
	...
	$country_cond = ($country_maintain==1) ? " and b.country_id=$country_id" : "";
	$sql_leftover = "SELECT sum(b.total_issue) as leftover_qty from pro_leftover_gmts_issue_mst a, pro_leftover_gmts_issue_dtls b where a.id=b.mst_id and b.po_break_down_id=$po_id $country_cond and b.item_number_id=$gmts_item and a.status_active=1 and b.status_active=1 ";
	$sql_leftover_res = sql_select($sql_leftover);
...
if($action=="color_and_size_level_left_over")
{
		...
		where a.id=b.mst_id and b.po_break_down_id='$poId'  and b.item_number_id=$item_id $country_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
...
if ($action=="save_update_delete")
{
	...
	else if ($operation==2)  // Delete Here---------------------------------------------------------- 
	{
		...
		$dtls_data = sql_select("SELECT id from pro_leftover_gmts_issue_dtls where mst_id=$txt_mst_id and status_active=1 and is_deleted=0");
		$no_of_details_data  = count($dtls_data);
		if ($no_of_details_data == 1 )  //Only 1 details row
		{
			$rID = sql_delete("pro_leftover_gmts_issue_mst","updated_by*update_date*status_active*is_deleted","".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*0*1",'id ',$txt_mst_id);
		}
		
		 $rID1 = sql_delete("pro_leftover_gmts_issue_dtls","updated_by*update_date*status_active*is_deleted","".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*0*1",'id*mst_id ',"$hidden_dtls_id*$txt_mst_id");
		
		if($sewing_production_variable !=1)
		{
			$rID2 = sql_delete("pro_leftover_gmts_issue_clr_sz","status_active*is_deleted","0*1",'mst_id*dtls_id',"$txt_mst_id*$hidden_dtls_id");	
		}
		...
		if($db_type==2 || $db_type==1 )
		{
			if($sewing_production_variable !=1)
			{
				...
				{
					oci_commit($con);   
					echo "2**".str_replace("'","",$txt_mst_id)."**".str_replace("'","",$txt_system_no)."**".str_replace("'","",$cbo_company_name)."**".($no_of_details_data-1);
