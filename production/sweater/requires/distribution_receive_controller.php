...
{
	echo create_drop_down( "cbo_location", 210, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id='$data'  $location_credential_cond order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down( 'requires/distribution_receive_controller', this.value, 'load_drop_down_floor', 'floor_td' );",0 );
	exit();
}

if ($action=="load_drop_down_floor")
{
	echo create_drop_down( "cbo_floor", 210, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process=2 order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/distribution_receive_controller', document.getElementById('cbo_cutting_company').value+'_'+document.getElementById('cbo_location').value+'_'+this.value, 'load_drop_down_table', 'table_td' );",0 );
	exit();
}
...
if($action=="load_drop_down_cutt_company")
{
	...
	if($data==3)
	{
		...
		echo create_drop_down( "cbo_cutting_company", 210, "select id,company_name from lib_company where is_deleted=0 and status_active=1 $company_credential_cond order by company_name","id,company_name", 1, "--- Select ---", "", "load_drop_down( 'requires/distribution_receive_controller', this.value, 'load_drop_down_location', 'location_td' );fnc_company_check(this.value);",0,0 );
	else
...
if($action=="order_popup")
{
	...
	<script>
		...
		function search_populate(str)
		{
			...
			{
				load_drop_down( 'distribution_receive_controller',document.getElementById('company_search_by').value,'load_drop_down_buyer', 'search_by_td' );
...
if($action=="create_po_search_list_view")
{
	...
	$total_cut_qty_arr=sql_select("SELECT po_break_down_id, item_number_id, country_id, sum(production_quantity) as production_quantity from pro_garments_production_mst where status_active=1 and is_deleted=0 and production_type=116 $po_id_cond group by po_break_down_id, item_number_id, country_id");
...
if($action=="populate_data_from_search_popup")
{
 	...
 	foreach($res as $result)
	{
		...
		$total_produced = return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=".$result[csf('id')]." and item_number_id='$item_id' and country_id='$country_id' and production_type=116 and is_deleted=0");
...
if($action=="color_and_size_level")
{
		...
		else // by default color and size level
		{
			...
			$dtlsData = sql_select("SELECT a.color_size_break_down_id,
										(CASE WHEN a.production_type=1 then a.production_qnty ELSE 0 END) as kint_qnty,
										(CASE WHEN a.production_type=116 then a.production_qnty ELSE 0 END) as dis_qnty
										from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in(1,116)");
 		...
 		foreach($colorResult as $color)
		{
			if( $variableSettings==2 ) // color level
			{
				$kint_qnty = $color_size_qnty_array[$color[csf('item_number_id')]][$color[csf('color_number_id')]]['kint_qnty'];
				$dis_qnty = $color_size_qnty_array[$color[csf('item_number_id')]][$color[csf('color_number_id')]]['dis_qnty'];

				$colorHTML .='<tr><td>'.$color_library[$color[csf("color_number_id")]].'</td><td><input type="text" name="txt_color" id="colSize_'.($i+1).'" style="width:60px"  class="text_boxes_numeric" placeholder="Qty"'.($kint_qnty - $dis_qnty).'" onkeyup="fn_colorlevel_total('.($i+1).')"></td><td><input type="text" name="txtColSizeKint" id="txtColSizeKint_'.($i+1).'" style="width:60px"  class="text_boxes_numeric" placeholder="Kint.QTY") '.$disable.'"></td></tr>';
		...
		<div style="width:1140px;max-height:250px; overflow-y:auto; overflow-x: auto;" id="sewing_production_list_view" align="left">
			...
			<?php
				...
				foreach(sql_select($sqlResult) as $selectResult){
					...
					$fnc_var="get_php_form_data('".$tot_data."','populate_cutting_form_data','requires/distribution_receive_controller');";
			?>
...
if($action=="populate_cutting_form_data")
{
	...
	$sqlResult =("SELECT id,company_id, po_break_down_id, item_number_id, challan_no, production_source, produced_by, production_date, production_quantity, $production_hour_cond , entry_break_down_type, break_down_type_rej, serving_company, location, floor_id, reject_qnty,cut_no, used_qty_kg, reject_qty_kg, reject_qty_kg_break_down, remarks, total_produced, yet_to_produced, country_id,wo_order_id,currency_id,exchange_rate,rate, table_no from pro_garments_production_mst where id='$data[0]' and production_type='116' and status_active=1 and is_deleted=0 order by id");

	$po_id=$sqlResult[0][csf("po_break_down_id")];
	$color_type_val=sql_select("SELECT b.color_type_id  from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and a.production_type=116 and b.production_type=116 and a.status_active=1 and b.status_active=1 and  a.id='$data[0]' group by b.color_type_id ");
    $sqls_col_size="SELECT color_number_id,size_number_id, sum(plan_cut_qnty) as qnty from wo_po_color_size_breakdown where po_break_down_id=$po_id and status_active=1 and is_deleted=0 group by color_number_id,size_number_id";
	foreach(sql_select($sqls_col_size) as $key=>$value)
	{
		$po_color_size_qnty_arr[$value[csf("color_number_id")]][$value[csf("size_number_id")]] +=$value[csf("qnty")];
  	...
  	foreach(sql_select($sqlResult) as $result)
	{
		...
		echo "load_drop_down( 'requires/distribution_receive_controller', ".$result[csf('production_source')].", 'load_drop_down_cutt_company', 'cutt_company_td' );\n";
		echo "$('#cbo_cutting_company').val('".$result[csf('serving_company')]."');\n";
		echo "load_drop_down( 'requires/distribution_receive_controller',".$result[csf('serving_company')].", 'load_drop_down_location', 'location_td' );";
		echo "$('#cbo_location').val('".$result[csf('location')]."');\n";
		echo "load_drop_down( 'requires/distribution_receive_controller', ".$result[csf('location')].", 'load_drop_down_floor', 'floor_td' );\n";
		echo "$('#cbo_floor').val('".$result[csf('floor_id')]."');\n";
		echo "load_drop_down( 'requires/distribution_receive_controller', '".$result[csf('serving_company')]."_".$result[csf('location')]."_".$result[csf('floor_id')]."');\n";
		//echo "$('#cbo_table').val('".$result[csf('table_no')]."');\n";

		// echo "load_drop_down( 'requires/distribution_receive_controller', ".$result[csf('po_break_down_id')].", 'load_drop_down_color_type', 'color_type_td' );\n";
		...
		{
			echo "load_drop_down( 'requires/distribution_receive_controller', '".$result[csf('company_id')]."_".$result[csf('serving_company')]."_".$result[csf('po_break_down_id')]."');\n";

			//load_drop_down( 'requires/distribution_receive_controller', company+"_"+supplier_id+"_"+po_break_down_id, 'load_drop_down_workorder', 'workorder_td' );
	...
	//	echo "$('#cbo_produced_type').val('".$result[csf('produced_by')]."');\n";
		...
		$total_produced = return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=".$result[csf('po_break_down_id')]." and item_number_id=".$result[csf('item_number_id')]." and country_id=".$result[csf('country_id')]." and production_type=116 and is_deleted=0");
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if($db_type==0)
		{
			$data_array2="(".$id.",".$garments_nature.",".$cbo_company_name.",".$txt_challan_no.",".$hidden_po_break_down_id.", ".$cbo_item_name.", ".$cbo_country_name.", ".$cbo_source.",".$cbo_cutting_company.",".$cbo_location.",".$txt_cutting_date.",".$txt_cutting_qty.",116,".$sewing_production_variable.",".$txt_reporting_hour.",".$txt_remark.",".$cbo_floor.",".$txt_cumul_cutting.",".$txt_yet_cut.",".$hidden_currency_id.",".$hidden_exchange_rate.",'".$rate."','".$amount."',".$user_id.",'".$pc_date_time."')";
		}
		else
		{
		...
		if(str_replace("'","",$sewing_production_variable)==2)//color level wise
		{
			...
			foreach($rowEx as $rowE=>$val)
			{
				...
				if($j==0)$data_array = "(".$dtls_id.",".$id.",116,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1].")";
				else $data_array .= ",(".$dtls_id.",".$id.",116,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1].")";
				//$dtls_id=$dtls_id+1;
 				$j++;
			}
 		}
		   ...
		   $color_all=implode(",",$color_all_id);
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				$index = $sizeID.$color_arr[$colorID].$colorID;
				 ...
				 if($j==0)$data_array = "(".$dtls_id.",".$id.",116,'".$colSizeID_arr[$index]."','".$colorSizeValue."')";
				 else $data_array .= ",(".$dtls_id.",".$id.",116,'".$colSizeID_arr[$index]."','".$colorSizeValue."')";
				 $j++;
			}
  	...
  	else if ($operation==1) // Update Here------------------------------------------------------
	{
		...
		$data_array1="".$cbo_company_name."*".$txt_challan_no."*".$cbo_source."*".$cbo_cutting_company."*".$cbo_location."*".$txt_cutting_date."*".$txt_cutting_qty."*116*".$sewing_production_variable."*".$txt_reporting_hour."*".$txt_remark."*".$cbo_floor."*".$txt_cumul_cutting."*".$txt_yet_cut."*".$hidden_currency_id."*".$hidden_exchange_rate."*'".$rate."'*'".$amount."'*".$user_id."*'".$pc_date_time."'";
		...
		if(str_replace("'","",$sewing_production_variable)!=1 && str_replace("'","",$txt_mst_id)!='')// check is not gross level
		{
			...
			if(str_replace("'","",$sewing_production_variable)==2)//color level wise
			{
				...
				foreach($rowEx as $rowE=>$val)
				{
					...
					if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",116,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."')";
					else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",116,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."')";
					//$dtls_id=$dtls_id+1;
					$j++;
				}
			}
			...
			if(str_replace("'","",$sewing_production_variable)==3)//color and size wise
			{
				...
				foreach($rowEx as $rowE=>$valE)
				{
					...
					if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",116,'".$colSizeID_arr[$index]."','".$colorSizeValue."')";
					else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",116,'".$colSizeID_arr[$index]."','".$colorSizeValue."')";
					$dtls_id=$dtls_id+1;
					$j++;
				}
			}
	...
	else if ($operation==2)  // Delete Here----------------------------------------------------------
	{
		...
		$next_process_sql=sql_select("SELECT po_break_down_id from pro_garments_production_mst where status_active=1 and is_deleted=0 and production_type <> 116 and po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name and country_id=$cbo_country_name");
		$next_process_sql2=sql_select("SELECT po_break_down_id from pro_cut_delivery_order_dtls where status_active=1 and is_deleted=0 and production_type <> 116 and po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name and country_id=$cbo_country_name");
