...
if ($action=="load_drop_down_location")
{
	...
	else { $dropdown_name="cbo_working_location";  $load_fnc="load_drop_down('requires/receive_in_wash_controller', this.value, 'load_drop_down_floor', 'floor_td' );"; }
...
{
    echo create_drop_down("cbo_floor", 110, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process=5 order by floor_name", "id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/receive_in_wash_controller', $('#cbo_working_company').val()+'_'+$('#cbo_working_location').val()+'_'+this.value+'_'+$('#txt_issue_date').val(), 'load_drop_down_line', 'line_td' );", 0);
    exit();
}
...
if ($action == "populate_bundle_data") 
{
	...
	if($issue_no!="")
	{
		$issue_id = return_field_value("id", "PRO_GMTS_DELIVERY_MST", "SYS_NUMBER='$issue_no' and status_active=1 and is_deleted=0");
		$ex_data[5] = $issue_id;
	}
	
	/*if($vscan==0)
	{
		$mst_sql=sql_select("select a.sys_number, a.id from pro_gmts_delivery_mst a where a.sys_number='$ex_data[0]' and a.production_type=78 and a.status_active=1 and a.is_deleted=0  group by a.sys_number, a.id");
		//echo "select a.sys_number, a.id from pro_gmts_delivery_mst a where a.sys_number='$ex_data[0]' and a.production_type=78 and a.status_active=1 and a.is_deleted=0  group by a.sys_number, a.id";
	...
	if($ex_data[5]!="")
	{
		$sql_bundle="SELECT barcode_no from pro_garments_production_dtls where delivery_mst_id in ($ex_data[5]) and production_type=78 and status_active=1 and is_deleted=0";
    ...
    $scanned_bundle_arr = return_library_array("SELECT b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=79 and b.status_active=1 and b.is_deleted=0 $scnbundle_nos_cond", 'bundle_no', 'bundle_no');
...
if($action=="populate_mst_data")
{
    ...
    where a.entry_form=384 and b.entry_form=384 and a.id=b.delivery_mst_id and b.barcode_no='$data[0]' and a.company_id='$data[1]' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
			...
			<legend></legend>           
	            <table cellpadding="0" cellspacing="0" width="550" border="1" rules="all" class="rpt_table">
	                ...
	                <tr class="general">
	                    ...
	                    <td>
	                        <input type="text"name="txt_lot_no"id="txt_lot_no"style="width:120px"class="text_boxes"value="<?php if($ratio_prifix) echo $ratio_prifix*1; ?>" />
	                    </td>  		
	                    <td>
	                    	<input type="text"name="txt_challan_no"id="txt_challan_no"style="width:120px"class="text_boxes" />
	                    </td>  		
	            		<td align="center">
	                     	<input type="button"name="button2"class="formbutton"value="Show"onClick="show_list_view (document.getElementById('cbo_company_name').value+'_'+ document.getElementById('txt_challan_no').value+'_'+ document.getElementById('txt_job_no').value+'_'+ document.getElementById('txt_lot_no').value+'_'+ document.getElementById('cbo_lot_year').value, 'create_issue_callan_search_list_view', 'search_div', 'receive_in_wash_controller', 'setFilterGrid(\'tbl_list_search\',-1);')"style="width:100px;" />
	                     </td>
	                </tr>
	           </table>
...
if ($action == "create_bundle_search_list_view") 
{
   ...
   // $scanned_bundle_arr = return_library_array("select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=55 and  b.status_active=1 and b.is_deleted=0", 'bundle_no', 'bundle_no');
    ...
    $scanned_bundle_arr = return_library_array("select bundle_no, bundle_no from pro_garments_production_dtls where production_type=79 and cut_no='".$tmp_cut."' and status_active=1 and is_deleted=0", 'bundle_no', 'bundle_no');
    foreach (explode(",", $selectedBuldle) as $bn) {
        $scanned_bundle_arr[$bn] = $bn;
    }

    $scanne=sql_select( "select b.bundle_no, sum(b.production_qnty) as production_qnty,a.sewing_line from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=79  and b.status_active=1 and b.is_deleted=0 $cutCon_a group by b.bundle_no,a.sewing_line");
    ...
    // echo $cutting_no;
    $last_operation=gmt_production_validation_script( 79, 1,'', $cutting_no, $production_squence);
    //$last_operation=gmt_production_validation_script( 4, 1 );
    ...
    <div style="width:850px; max-height:210px; overflow-y:scroll" id="list_container_batch" align="left">    
        ...
        <?
            ...
            foreach($last_operation as  $item_id=>$operation_cond)
            {
				...
				$qrCond $orderCon $bndlCon $yearCond $jobCon $cutCon $styleCond
				and a.status_active=1 and a.is_deleted=0 and a.production_type=79 group by c.cut_no, c.bundle_no, d.po_break_down_id, d.job_no_mst, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.bundle_no, e.po_number,c.barcode_no order by c.cut_no, length(c.bundle_no) asc, c.bundle_no asc";
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
        ...
        $receive_sql="select c.barcode_no, c.bundle_no from pro_garments_production_mst a, pro_garments_production_dtls c where a.id=c.mst_id and a.production_type=79 and c.bundle_no in ($bundle) and c.production_type=79 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 "; //and (c.is_rescan=0 or c.is_rescan is null)
		...
		if (str_replace("'", "", $txt_update_id) == "")
		{
			...
			$new_sys_number = explode("*", return_next_id_by_sequence("", "pro_gmts_delivery_mst",$con,1,$cbo_company_name,'BRW',0,date("Y",time()),0,0,79,0,0 ));
			...
			$data_array_delivery="(".$mst_id.",'".$new_sys_number[1]."','".(int)$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",".$hidden_issue_id.",79,".$cbo_source.",".$txt_issue_date.",'".$challanNo."',".$txt_remarks.",384,".$user_id.",'".$pc_date_time."',1,0)";
		...
		foreach($mstArr as $orderId=>$orderData)
		{
			foreach($orderData as $gmtsItemId=>$gmtsItemIdData)
			{
				foreach($gmtsItemIdData as $countryId=>$qty)
				{
					...
					$data_array_mst.="(".$id.",".$mst_id.",".$garments_nature.",".$cbo_company_name.",".$cbo_source.",".$txt_issue_date.",'".$challan_no."',".$txt_remarks.",".$orderId.", ".$gmtsItemId.",".$countryId.",".$qty.",79,3,384,".$user_id.",'".$pc_date_time."',1,0)";
		...
		foreach($dtlsArr as $bundle_no=>$qty)
		{
			...
			$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",79,'".$dtlsArrColorSize[$bundle_no]."','".$qty."','".$cutNoArr[$bundle_no]."','".$bundle_no."',384,'".$bundleBarcodeArr[$bundle_no]."',1,0)"; 
		}
		$flag=1;
		if (str_replace("'", "", $txt_update_id) == "")
		{
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$issue_data_arr=production_data($mst_id,79);
		...
		$flag=1;
		$delete = execute_query("update pro_garments_production_mst set status_active=0,is_deleted=1,updated_by=$user_id,update_date='".$pc_date_time."' WHERE delivery_mst_id=$mst_id and production_type=79 and status_active=1 and is_deleted=0");
		if($delete==1 && $flag==1) $flag=1; else $flag=0;
		$delete_dtls = execute_query("update pro_garments_production_dtls set status_active=0, is_deleted=1 WHERE delivery_mst_id=$mst_id and production_type=79 and status_active=1 and is_deleted=0");
...
{
  	echo load_html_head_contents("Bundle Receive in Wash [Sweater] Info","../../../", 1, 1, '','1','');
	extract($_REQUEST);
	?>
	<script>
...
if($action=="create_system_search_list_view")
{
	...
	if(str_replace("'","",$issue_no)=="") $issue_nocond=""; else $issue_nocond=" and a.sys_number_prefix_num='".str_replace("'","",$issue_no)."'";
    ...
    where c.garments_nature=100 and a.entry_form=384 and b.entry_form=384 and a.id=b.delivery_mst_id and b.po_break_down_id=d.id and c.job_no=d.job_no_mst $conpany_cond $issue_nocond $job_cond $sql_cond $order_cond $system_cond $stylecond $jyearCond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.id DESC";
	//echo $sql_order;	
	$sql_pop_res=sql_select($sql_pop);
	$issue_challan_id_arr = array();
	foreach ($sql_pop_res as $v) 
	{
		$issue_challan_id_arr[$v['ISSUE_CHALLAN_ID']] = $v['ISSUE_CHALLAN_ID'];
	}
	$issue_challan_id_cond = where_con_using_array($issue_challan_id_arr,0,"id");
	$issueSys_arr = return_library_array("SELECT id,sys_number from pro_gmts_delivery_mst where entry_form=383 and status_active=1 and is_deleted=0 $issue_challan_id_cond", 'id', 'sys_number');
...
if($action=="populate_bundle_data_update")
{
	...
	$sql_bundle="select barcode_no from pro_garments_production_dtls where delivery_mst_id='$mst_id' and production_type=79 and status_active=1 and is_deleted=0";
	$sql_bundle_res=sql_select($sql_bundle); $bundleNo=""; $bundle_count=0;
	foreach($sql_bundle_res as $row)
	{
		$bundle_count++;
...
if($action=="receive_in_wash_print")
{
    extract($_REQUEST);
    $data=explode('*',$data);
    //print_r ($data);
...
<div style="width:1030px;">
   ...
   from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name='$data[0]' and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.delivery_mst_id='$data[2]' and a.production_type=79 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and d.status_active in(1,2,3) and d.is_deleted=0 and e.status_active in(1,2,3) and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0
    ...
    <?=signature_table(193, $data[0], "1000px"); ?>  
    </div>
