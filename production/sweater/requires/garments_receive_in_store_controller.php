...
if ($action=="load_drop_down_location")
{
	...
	else if($data[1]==2) { $dropdown_name="cbo_working_location"; $load_function="load_drop_down( 'requires/garments_receive_in_store_controller', this.value, 'load_drop_down_floor', 'floor_td' );"; }
...
if ($action=="order_details")
{
	...
	where a.id=b.po_break_down_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.job_no_mst='$jobno'";
	//echo $sql_dtls;
...
{
  	echo load_html_head_contents("Garments Receive In Store Info","../../../", 1, 1, '','1','');
	extract($_REQUEST);
	?>
	<script>
		function js_set_value( strCon ) 
    ...
    <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            ...
            <tbody>
                <tr class="general">                    
                    ...
                    <td>
                    	<input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('txt_issue_no').value+'_'+document.getElementById('txt_job_search').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_order_search').value+'_'+document.getElementById('txt_style_ref').value, 'create_system_search_list_view', 'search_div', 'garments_receive_in_store_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />				
                    </td>
                </tr>
                <tr>                  
...
if($action=="create_system_search_list_view")
{
    ...
    where c.garments_nature=100 and a.entry_form=679 and b.entry_form=679 and a.id=b.delivery_mst_id and b.po_break_down_id=d.id and c.job_no=d.job_no_mst $conpany_cond $job_cond $sql_cond $order_cond $system_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if (str_replace("'","",$txt_update_id)=="")
		{
			...
			$new_sys_number = explode("*", return_next_id_by_sequence("", "pro_gmts_delivery_mst",$con,1,$cbo_company_name,'GRS',0,date("Y",time()),0,0,90,0,0 ));
			// txt_wo_id
			...
			$data_array_delivery="(".$mst_id.",'".$new_sys_number[1]."','".(int)$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",90,".$cbo_location.",".$cbo_source.",".$cbo_working_company.",".$cbo_working_location.",".$cbo_floor.",".$txt_job_id.",".$txt_job_no.",".$txt_receive_date.",'".$sending_company."','".$sending_location."',".$txt_challan.",".$txt_remark.",679,".$user_id.",'".$pc_date_time."',1,0)";
		}
		else
		{
			...
			$data_array_delivery = "".$cbo_location."*".$cbo_source."*".$cbo_working_company."*".$cbo_working_location."*".$cbo_floor."*".$txt_job_id."*".$txt_job_no."*".$txt_receive_date."*".$sending_company."*".$sending_location."*".$txt_challan."*".$txt_remark."*".$user_id."*'".$pc_date_time."'";
		}
		
		$mstArr=array(); $dtlsArr=array();
		for($j=1;$j<=$tot_row;$j++)
		{
			$txtIssueQty 		="txtReceiveQty_".$j;
			$txtpoid 			="txtpoid_".$j;
		...
		foreach($mstArr as $orderId=>$orderData)
		{
			foreach($orderData as $gmtsItemId=>$gmtsItemIdData)
			{
				foreach($gmtsItemIdData as $countryId=>$qty)
				{
					...
					$data_array_mst.="(".$id.",".$mst_id.",".$garments_nature.",".$cbo_company_name.",".$cbo_location.",".$cbo_source.",'".$sending_company."','".$sending_location."',".$cbo_floor.",".$txt_receive_date.",".$txt_challan.",".$txt_remark.",".$orderId.", ".$gmtsItemId.",".$countryId.",".$cbo_working_company.",".$qty.",90,3,679,".$user_id.",'".$pc_date_time."',1,0,".$txt_wo_id.")";
		...
		foreach($dtlsArr as $colorSizeid=>$qty)
		{
			...
			$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",90,'".$colorSizeid."','".$qty."',679,1,0)"; 
		}
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
			...
			if($orderId)
			{
				foreach ($orderData as $gmtsItemId => $gmtsItemIdData)
				{
					foreach ($gmtsItemIdData as $countryId => $qty) 
					{
						...
						{
							$data_array_up[$gmtProdId] =explode("*",("".$cbo_location."*".$cbo_source."*'".$sending_company."'*'".$sending_location."'*".$cbo_floor."*".$txt_receive_date."*".$txt_challan."*".$txt_remark."*".$orderId."*".$gmtsItemId."*".$countryId."*".$cbo_working_company."*".$qty."*'".$user_id."'*'".$pc_date_time."'"));
							$id=$gmtProdId;
							$is_up=1;
							$id_arr[]=$gmtProdId;
						}
		...
		$dtlsrDelete = execute_query("update pro_garments_production_dtls set status_active=0, is_deleted=1 WHERE delivery_mst_id=$mst_id and production_type=90 and status_active=1 and is_deleted=0");
...
if($action=="garments_receive_store_print") // Print 2 Start.
{
	extract($_REQUEST);
	$data=explode('*',$data);
	...
	$sql="SELECT a.id, a.company_id, a.challan_no, a.po_break_down_id , a.item_number_id, a.country_id, a.production_source, a.serving_company, a.location, a.embel_name, a.embel_type, a.production_quantity, a.production_type, a.remarks, a.floor_id,a.body_part_info,b.sys_number from pro_garments_production_mst a,pro_gmts_delivery_mst b where a.production_type=90 and a.delivery_mst_id=$mst_id and b.id=a.delivery_mst_id and a.status_active=1 and a.is_deleted=0 ";
//	echo $sql;die;
	...
	$sql_color_type=sql_select("SELECT color_type_id from pro_garments_production_dtls where status_active=1 and is_deleted=0 and delivery_mst_id=$mst_id and production_type=90");
    ...
    <table width="900" cellspacing="0" align="right">
        ...
        <tr>
            ...
            <td width="125"><strong>Issue ID:</strong></td> <td width="175px"><? echo $dataArray[0][csf('sys_number')]; ?></td>
