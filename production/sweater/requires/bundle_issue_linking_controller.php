...
if ($action=="load_drop_down_location")
{
	...
	else { $dropdown_name="cbo_working_location";  $load_fnc="load_drop_down('requires/bundle_issue_linking_controller', this.value, 'load_drop_down_floor', 'floor_td' ); location_select();"; }
...
if ($action=="load_drop_down_working_com")
{
	...
	if($data[1]==1) $load_function="fnc_load_party(2); location_select();";
	else $load_function="";
...
{
    echo create_drop_down("cbo_floor", 130, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process=5 order by floor_name", "id,floor_name", 1, "-- Select Floor --", $selected, "", 0);
        ...
        <form name="searchwofrm" id="searchwofrm">
            <fieldset style="width:810px;">
                ...
                <table cellpadding="0" cellspacing="0" width="550" border="1" rules="all" class="rpt_table">
                    ...
                    <tr class="general">
                        <td><? echo create_drop_down( "cbo_cut_year", 70, $year,'', "", '-- Select --',date("Y",time()), "",'','','','' ); ?></td>               
...
if ($action == "create_bundle_search_list_view") 
{
  ...
  // echo $tmp_cut;
   $scanned_bundle_arr = return_library_array("select bundle_no, bundle_no from pro_garments_production_dtls where production_type=53 and cut_no='".$tmp_cut."' and status_active=1 and is_deleted=0", 'bundle_no', 'bundle_no');
    ...
    // echo $cutting_no;
    $last_operation=gmt_production_validation_script( 53, 1,'', $cutting_no, $production_squence);
    //$last_operation=gmt_production_validation_script( 4, 1 );
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
        ...
        $receive_sql="select c.barcode_no, c.bundle_no from pro_garments_production_mst a, pro_garments_production_dtls c where a.id=c.mst_id and a.production_type=53 and c.bundle_no in ($bundle) and c.production_type=53 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 "; //and (c.is_rescan=0 or c.is_rescan is null)
		...
		if (str_replace("'", "", $txt_update_id) == "")
		{
			...
			$new_sys_number = explode("*", return_next_id_by_sequence("", "pro_gmts_delivery_mst",$con,1,$cbo_company_name,'ILK',0,date("Y",time()),0,0,53,0,0 ));
			$field_array_delivery = "id, sys_number_prefix, sys_number_prefix_num, sys_number, company_id, production_type, location_id, production_source, working_company_id, working_location_id, floor_id, delivery_date, challan_no, remarks, entry_form, inserted_by, insert_date, status_active, is_deleted";
			...
			$data_array_delivery="(".$mst_id.",'".$new_sys_number[1]."','".(int)$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",53,".$cbo_location.",".$cbo_source.",".$cbo_working_company.",".$cbo_working_location.",".$cbo_floor.",".$txt_issue_date.",'".$challanNo."',".$txt_remarks.",319,".$user_id.",'".$pc_date_time."',1,0)";
		...
		else 
		{
			...
			$field_array_delivery = "location_id*production_source*working_company_id*working_location_id*floor_id*delivery_date*challan_no*remarks*updated_by*update_date";
			$data_array_delivery = "".$cbo_location."*".$cbo_source."*".$cbo_working_company."*".$cbo_working_location."*".$cbo_floor."*".$txt_issue_date."*'".$challanNo."'*".$txt_remarks."*".$user_id."*'".$pc_date_time."'";
		...
		foreach($mstArr as $orderId=>$orderData)
		{
			foreach($orderData as $gmtsItemId=>$gmtsItemIdData)
			{
				foreach($gmtsItemIdData as $countryId=>$qty)
				{
					...
					$data_array_mst.="(".$id.",".$mst_id.",".$garments_nature.",".$cbo_company_name.",".$cbo_location.",".$cbo_source.",".$cbo_working_company.",".$cbo_working_location.",".$cbo_floor.",".$txt_issue_date.",'".$challan_no."',".$txt_remarks.",".$orderId.", ".$gmtsItemId.",".$countryId.",".$qty.",53,3,319,".$user_id.",'".$pc_date_time."',1,0)";
		...
		foreach($dtlsArr as $bundle_no=>$bundle_data)
		{
			...
			$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",53,'".$dtlsArrColorSize[$bundle_no]."','".$bundle_data['qc_pass']."','".$cutNoArr[$bundle_no]."','".$bundle_no."',319,'".$bundleBarcodeArr[$bundle_no]."',1,0)"; 
		}
		$flag=1;
		if (str_replace("'", "", $txt_update_id) == "")
		{
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
        ...
        $bundle="'".implode("','",$bundleCheckArr)."'";
        $receive_sql="select c.barcode_no, c.bundle_no from pro_garments_production_mst a, pro_garments_production_dtls c where a.id=c.mst_id and a.production_type=53 and c.bundle_no in ($bundle) and c.production_type=53 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and a.delivery_mst_id!=$mst_id and c.delivery_mst_id!=$mst_id)"; 
        $receive_result = sql_select($receive_sql);
        foreach ($receive_result as $row)
        {           
            $duplicate_bundle[$row[csf('bundle_no')]]=$row[csf('bundle_no')];
        }
		$non_delete_arr=production_validation($mst_id,54);
		$issue_data_arr=production_data($mst_id,53);
		...
		foreach ($dtlsArr as $bundle_no => $qty)
		{
			if($bundle_no)
			{
				...
				$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",53,'".$dtlsArrColorSize[$bundle_no]."','".$qty['qc_pass']."','".$cutNoArr[$bundle_no]."','".$bundle_no."',319,'".$bundleBarcodeArr[$bundle_no]."',1,0)"; 
			}
		} 
		$flag=1;
		$delete = execute_query("update pro_garments_production_mst set status_active=0,is_deleted=1,updated_by=$user_id,update_date='".$pc_date_time."' WHERE delivery_mst_id=$mst_id and production_type=53 and status_active=1 and is_deleted=0");
		if($delete==1 && $flag==1) $flag=1; else $flag=0;
		$delete_dtls = execute_query("update pro_garments_production_dtls set status_active=0, is_deleted=1 WHERE delivery_mst_id=$mst_id and production_type=53 and status_active=1 and is_deleted=0");
...
if($action=="system_number_popup")
{
    ...
    <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            ...
            <tbody>
                <tr class="general">                    
                    <td><? echo create_drop_down( "cbo_company_name", 130, "select id, company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond  order by company_name","id,company_name", 1, "-- Select Company --",$cbo_company_name, ""); ?></td>
                    ...
                    <td align="center">
                    <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('txt_issue_no').value+'_'+document.getElementById('txt_job_search').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_order_search').value, 'create_system_search_list_view', 'search_div', 'bundle_issue_linking_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />				
                    </td>
                </tr>
                <tr>                  
...
if($action=="populate_bundle_data_update")
{
	...
	$sql_bundle="select barcode_no from pro_garments_production_dtls where delivery_mst_id='$mst_id' and production_type=53 and status_active=1 and is_deleted=0";
...
if($action=="bundle_issue_linking_print")
{
    extract($_REQUEST);
    $data=explode('*',$data);
    //print_r ($data);
...
<div style="width:1030px;">
   ...
   from pro_garments_production_mst a, pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f where f.company_name='$data[0]' and a.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_no_mst=f.job_no and c.delivery_mst_id='$data[2]' and a.production_type=53 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and d.status_active in(1,2,3) and d.is_deleted=0 and e.status_active in(1,2,3) and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0
    ...
    <?=signature_table(183, $data[0], "1000px"); ?>  
    </div>
