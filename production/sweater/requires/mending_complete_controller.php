...
if ($action=="load_drop_down_location")
{
	...
	$res=sql_select($sql);
	$sl=$selected;
	echo create_drop_down( "cbo_location", 140, $sql,"id,location_name", 1, "-- Select Location --", $sl, "load_drop_down( 'requires/mending_complete_controller', this.value, 'load_drop_down_floor', 'floor_td' );",0 );
...
{
	echo create_drop_down( "cbo_floor", 140, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process=5 order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/mending_complete_controller', this.value+'_'+document.getElementById('cbo_location').value, 'load_drop_down_poly_line_floor', 'poly_line_td' );",0 );
	exit();
}
...
if($action=="production_process_control")
{
    ...
    $control_and_preceding=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=50 and page_category_id=112 and company_name='$data'");
...
if($action=="load_drop_down_cutt_company")
{
	...
	if($data==3)
	{
		...
		echo create_drop_down( "cbo_cutting_company", 140, "select id,company_name from lib_company where is_deleted=0 and status_active=1 $company_credential_cond order by company_name","id,company_name", 1, "--- Select ---", "", "load_drop_down( 'requires/mending_complete_controller', this.value, 'load_drop_down_location', 'location_td' );fnc_company_check(this.value);",0,0 );
	else
...
if($action=="order_popup")
{
	...
	// onkeydown="if (event.keyCode == 13) document.getElementById('btn_show').click()"
			...
			{
				load_drop_down( 'mending_complete_controller',document.getElementById('company_search_by').value,'load_drop_down_buyer', 'search_by_td' );
...
if($action=="create_po_search_list_view")
{
	...
	$total_cut_qty_arr=sql_select("SELECT po_break_down_id, item_number_id, country_id, sum(production_quantity) as production_quantity from pro_garments_production_mst where status_active=1 and is_deleted=0 and production_type=112 $po_id_cond group by po_break_down_id, item_number_id, country_id");
 	...
 	foreach($res as $result)
	{
		...
		echo "$('#txt_job_no').val('".$result[csf('job_no')]."');\n";
 		...
 		if($qty_source==0)
 		{
			...
			$total_produced = return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=".$result[csf('id')]." and item_number_id='$item_id' and country_id='$country_id' and production_type=112 and is_deleted=0");
		...
		{
			$dataArray=sql_select("SELECT SUM(CASE WHEN a.production_type=$qty_source and b.production_type=$qty_source THEN b.production_qnty END) as totalreceive,SUM(CASE WHEN a.production_type=112 and b.production_type=112  THEN b.production_qnty ELSE 0 END) as totalinput from pro_garments_production_mst a,pro_garments_production_dtls b WHERE a.id=b.mst_id and  a.po_break_down_id=".$result[csf('id')]." and a.item_number_id='$item_id' and a.country_id='$country_id' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
		...
		if($qty_source==0)
		{
			...
			else if( $variableSettings==3 ) //color and size level
			{
				...
				$dtlsData = sql_select("SELECT a.color_size_break_down_id,
											...
											from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in(112) group by a.color_size_break_down_id");
		...
		else
		{
			...
			else if( $variableSettings==3 ) //color and size level
			{
				$dtlsData = sql_select("SELECT a.color_size_break_down_id,
											...
											sum(CASE WHEN a.production_type=112 then a.production_qnty ELSE 0 END) as cur_production_qnty
		...
		<div style="width:1005px;max-height:250px; overflow-y:auto; overflow-x: auto;" id="sewing_production_list_view" align="left">
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1000" class="rpt_table"  align="left">
				 ...
				 <thead>
	            	<tr>
		                <th width="20">SL</th>
		                <th width="110" align="center">Item Name</th>
		                ...
		                <th width="80" align="center">Mend. Line</th>
			...
			?>
				...
				<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data('<? echo $selectResult[csf('id')]."_".$selectResult[csf('color_id')]; ?>','populate_cutting_form_data','requires/mending_complete_controller');" >
					...
					<td  align="center"><?php echo $selectResult['WO_ORDER_NO']; ?></td>
...
if($action=="populate_cutting_form_data")
{
	...
	$sqlResult =sql_select("SELECT id,company_id, po_break_down_id, item_number_id, challan_no, production_source, produced_by, production_date, production_quantity, $production_hour_cond , entry_break_down_type, break_down_type_rej, serving_company, location, floor_id, reject_qnty, remarks, total_produced,alter_qnty,spot_qnty, yet_to_produced, country_id,wo_order_id,wo_order_no,currency_id,exchange_rate,rate,supervisor,sewing_line from pro_garments_production_mst where id='$data[0]' and production_type='112' and status_active=1 and is_deleted=0 order by id");

	$po_id=$sqlResult[0][csf("po_break_down_id")];
	
	$color_type_val=sql_select("SELECT b.color_type_id  from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and a.production_type=112 and b.production_type=112 and a.status_active=1 and b.status_active=1 and  a.id='$data[0]' group by b.color_type_id ");
    $sqls_col_size="SELECT color_number_id,size_number_id, sum(plan_cut_qnty) as qnty from wo_po_color_size_breakdown where po_break_down_id=$po_id and status_active=1 and is_deleted=0 group by color_number_id,size_number_id";
	foreach(sql_select($sqls_col_size) as $key=>$value)
	{
		$po_color_size_qnty_arr[$value[csf("color_number_id")]][$value[csf("size_number_id")]] +=$value[csf("qnty")];
	...
	{
		echo "$('#txt_trim_date').val('".change_date_format($result[csf('production_date')])."');\n";


		echo "$('#cbo_source').val('".$result[csf('production_source')]."');\n";
		echo "load_drop_down( 'requires/mending_complete_controller', ".$result[csf('production_source')].", 'load_drop_down_cutt_company', 'cutt_company_td' );\n";
		echo "$('#cbo_cutting_company').val('".$result[csf('serving_company')]."');\n";
		echo "load_drop_down( 'requires/mending_complete_controller',".$result[csf('serving_company')].", 'load_drop_down_location', 'location_td' );";
		echo "$('#cbo_location').val('".$result[csf('location')]."');\n";
		echo "load_drop_down( 'requires/mending_complete_controller', ".$result[csf('location')].", 'load_drop_down_floor', 'floor_td' );\n";
		echo "$('#cbo_floor').val('".$result[csf('floor_id')]."');\n";

		echo "load_drop_down( 'requires/mending_complete_controller', ".$result[csf('po_break_down_id')].", 'load_drop_down_color_type', 'color_type_td' );\n";
		...
		{
			echo "load_drop_down( 'requires/mending_complete_controller', '".$result[csf('company_id')]."_".$result[csf('serving_company')]."_".$result[csf('po_break_down_id')]."', 'load_drop_down_workorder', 'workorder_td' );\n";
		...
		echo "$('#txt_spot_qnty').val('".$result[csf('spot_qnty')]."');\n";
  		...
  		echo "load_drop_down( 'requires/mending_complete_controller', '".$result[csf('floor_id')]."_".$result[csf('location')]."', 'load_drop_down_poly_line_floor', 'poly_line_td' );\n";
  		
  		//load_drop_down( 'requires/mending_complete_controller', this.value+'_'+document.getElementById('cbo_location').value, 'load_drop_down_poly_line_floor', 'poly_line_td' );
		...
		echo "$('#txt_yet_cut').val('".$result[csf('yet_to_produced')]."');\n"; */
			...
			$defect_sql=sql_select("select id, po_break_down_id, defect_type_id, defect_point_id, defect_qty from pro_gmts_prod_dft where mst_id='".$result[csf('id')]."' and status_active=1 and is_deleted=0 and production_type='112'");
		...
		{
			$dataArray=sql_select("SELECT SUM(CASE WHEN a.production_type=$qty_source and b.production_type=$qty_source THEN b.production_qnty END) as totalreceive,SUM(CASE WHEN a.production_type=112 and b.production_type=112  THEN b.production_qnty ELSE 0 END) as totalinput from pro_garments_production_mst a,pro_garments_production_dtls b WHERE a.id=b.mst_id and  a.po_break_down_id=".$result[csf('po_break_down_id')]." and a.item_number_id=".$result[csf('item_number_id')]." and a.country_id=".$result[csf('country_id')]." and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
			foreach($dataArray as $row)
			{
				...
				else if( $variableSettings==3 ) //color and size level
				{
					/*$sql = "select id, item_number_id, size_number_id, color_number_id, order_quantity, plan_cut_qnty, (select sum(CASE WHEN pro_garments_production_dtls.color_size_break_down_id=wo_po_color_size_breakdown.id then production_qnty ELSE 0 END) from pro_garments_production_dtls where is_deleted=0 and  	production_type=1 ) as production_qnty
						...
						$dtlsData = sql_select("SELECT a.color_size_break_down_id,
											...
											sum(CASE WHEN a.production_type=$qty_source then a.production_qnty ELSE 0 END) as production_qnty,
											sum(CASE WHEN a.production_type=112 then a.production_qnty ELSE 0 END) as cur_production_qnty,
											sum(CASE WHEN a.production_type=112 then a.reject_qty ELSE 0 END) as reject_qty
											from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id   and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id <> 0 and a.production_type in($qty_source, 112) group by a.color_size_break_down_id");
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$sewing_production_variable)==2)//color level wise
		{
			...
			foreach($rowEx as $rowE=>$val)
			{
				...
				if($j==0)$data_array = "(".$dtls_id.",".$id.",112,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."',".$cbo_color_type.")";
				else $data_array .= ",(".$dtls_id.",".$id.",112,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."',".$cbo_color_type.")";
				//$dtls_id=$dtls_id+1;
 				$j++;
			}
 		}
		...
		if(str_replace("'","",$sewing_production_variable)==3)//color and size wise
		{
		   ...
		   $color_all=implode(",",$color_all_id);
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				if($j==0)$data_array = "(".$dtls_id.",".$id.",112,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$cbo_color_type.")";
				else $data_array .= ",(".$dtls_id.",".$id.",112,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$cbo_color_type.")";
				//$dtls_id=$dtls_id+1;
 				$j++;
			}
		...
		{		 
			...
			foreach($defect_array as $key=>$val)
			{
				...
				$defectPointId=$key;
				$defect_qty=$val;
				$data_array_defect.="(".$dft_id.",".$id.",112,".$hidden_po_break_down_id.",".$defect_type_id.",".$defectPointId.",'".$defect_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			} 
		}
		...
		{		 
			...
			foreach($defectSpt_array as $keysp=>$valsp)
			{
				...
				$defectsp_qty=$valsp;
				$data_array_defectsp.="(".$dftSp_id.",".$id.",112,".$hidden_po_break_down_id.",'".str_replace("'", "", $defectSpot_type_id)."','".$defectspPointId."','".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			} 
		}
  	...
  	else if ($operation==1) // Update Here------------------------------------------------------
	{
		...
		$data_array1="".$cbo_company_name."*".$txt_challan_no."*".$cbo_source."*".$cbo_cutting_company."*".$cbo_location."*".$txt_trim_date."*".$txt_trim_qty."*112*".$sewing_production_variable."*".$cutting_production_variable_reject."*".$txt_reporting_hour."*".$txt_remark."*".$txt_super_visor."*".$cbo_poly_line."*".$cbo_floor."*".$txt_reject_qty."*".$txt_alter_qnty."*".$txt_spot_qnty."*".$txt_cumul_cutting."*".$txt_yet_cut."*".$txt_wo_id."*".$txt_wo_no."*".$hidden_currency_id."*".$hidden_exchange_rate."*'".$rate."'*'".$amount."'*".$user_id."*'".$pc_date_time."'";
		...
		if(str_replace("'","",$sewing_production_variable)!=1 && str_replace("'","",$txt_mst_id)!='')// check is not gross level
		{
			...
			if(str_replace("'","",$sewing_production_variable)==2)//color level wise
			{
				...
				foreach($rowEx as $rowE=>$val)
				{
					...
					if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",112,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."',".$cbo_color_type.")";
					else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",112,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."',".$cbo_color_type.")";
					//$dtls_id=$dtls_id+1;
					$j++;
				}
			}
			...
			if(str_replace("'","",$sewing_production_variable)==3)//color and size wise
			{
				...
				foreach($rowEx as $rowE=>$valE)
				{
					...
					if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",112,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$cbo_color_type.")";
					else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",112,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$cbo_color_type.")";
					//$dtls_id=$dtls_id+1;
					$j++;
				}
			}
		...
		{		 
			...
			foreach($defect_array as $key=>$val)
			{
				...
				$defectPointId=$key;
				$defect_qty=$val;
				$data_array_defect.="(".$dft_id.",".$txt_mst_id.",112,".$hidden_po_break_down_id.",".$defect_type_id.",".$defectPointId.",'".$defect_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			} 
		}
		if($data_array_defect!="")
		{
			...
			$query3=execute_query("DELETE FROM pro_gmts_prod_dft WHERE mst_id=$txt_mst_id and defect_type_id=1 and production_type=112");
			$defectQ=sql_insert("pro_gmts_prod_dft",$field_array_defect,$data_array_defect,1);
		}
		$defectSpot=true;
		$data_array_defectsp="";
		...
		{		 
			...
			foreach($defectSpt_array as $keysp=>$valsp)
			{
				...
				$defectspPointId=$keysp;
				$defectsp_qty=$valsp;
				$data_array_defectsp.="(".$dftSp_id.",".$txt_mst_id.",112,".$hidden_po_break_down_id.",'".str_replace("'", "", $defectSpot_type_id)."','".$defectspPointId."','".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			} 
		}
		...
		if($data_array_defectsp!="")
		{
			...
			$query4=execute_query("DELETE FROM pro_gmts_prod_dft WHERE mst_id=$txt_mst_id and defect_type_id=2 and production_type=112");
			$defectSpot=sql_insert("pro_gmts_prod_dft",$field_array_defectsp,$data_array_defectsp,0);
		}
	...
	else if ($operation==2)  // Delete Here----------------------------------------------------------
	{
		...
		$con = connect();
		if($db_type==0)	{ mysql_query("BEGIN"); }
		$next_process_sql=sql_select("SELECT po_break_down_id from pro_garments_production_mst where status_active=1 and is_deleted=0 and production_type = 112 and po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name and country_id=$cbo_country_name");
		$next_process_sql2=sql_select("SELECT po_break_down_id from pro_cut_delivery_order_dtls where status_active=1 and is_deleted=0 and production_type = 112 and po_break_down_id=$hidden_po_break_down_id and item_number_id=$cbo_item_name and country_id=$cbo_country_name");
...
if($action=="bundle_preparation")
{
	...
	function calculate_bundle( pcs_per_bund )
	{
		...
		var list_view_bundle = return_global_ajax_value( data, 'load_php_dtls_form', '', 'mending_complete_controller');
	...
	function fnc_cutting_bundle_entry(operation)
	{
		...
		http.open("POST","mending_complete_controller.php",true);
	...
	function fnc_operation_sticker_report()
	{
		...
		//alert(data);
		var url=return_ajax_request_value(data, "print_report_operation_barcode", "mending_complete_controller");
		window.open(url,"##");
	}
	...
	function fnc_bundle_report()
	{
		...
		data=data+"***"+$('#rpt_data' ).val()+"***"+colsize;
		var url=return_ajax_request_value(data, "print_report_bundle_barcode", "mending_complete_controller");
		window.open(url,"##");
	}
	...
	function fnc_operation_report()
	{
		...
		data=data+"***"+$('#rpt_data' ).val()+"***"+colsize;
		var url=return_ajax_request_value(data, "print_report_operation", "mending_complete_controller");
		//alert(url);return;
		window.open(url,"##");
	}
	...
	function reset_pcs_per_bundle()
	{
	  ...
	  <script>
	  	get_php_form_data(<? echo $txt_mst_id; ?>+"**"+<? echo $hidden_po_break_down_id; ?>+"**"+document.getElementById('row_id').value, "populate_data_from_bundle", "mending_complete_controller" );
	 	calculate_bundle(document.getElementById('txt_pcs_per_bendle').value);
	  </script>
	  <script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
	  </html>
