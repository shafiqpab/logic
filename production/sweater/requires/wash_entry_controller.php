...
if ($action=="load_variable_settings")
{
	...
	echo "$('#wash_production_variable').val(0);\n";
	...
	{
		echo "$('#wash_production_variable').val(".$result[csf("sewing_production")].");\n";
	...
	echo "$('#wash_production_variable_rej').val(0);\n";
	...
	{
		echo "$('#wash_production_variable_rej').val(".$result[csf("sewing_production")].");\n";
	}
	...
	$variable_is_control=return_field_value("is_control","variable_settings_production","company_name=$working_company and variable_list=50 and page_category_id=103","is_control");
	echo "document.getElementById('variable_is_controll').value='".$variable_is_control."';\n";
	
	$variable_qty_source_wash=return_field_value("qty_source_wash","variable_settings_production","company_name=$working_company and variable_list=42","qty_source_wash");
	echo "document.getElementById('txt_qty_source').value='".$variable_qty_source_wash."';\n";
...
{
	echo create_drop_down( "cbo_location", 150, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id='$data' order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down( 'requires/wash_entry_controller', this.value, 'load_drop_down_floor', 'floor_td'); load_drop_down( 'requires/wash_entry_controller', document.getElementById('cbo_location').value+'_'+document.getElementById('prod_reso_allo').value+'_'+document.getElementById('txt_wash_date').value, 'load_drop_down_wash_output_line', 'wash_line_td'); get_php_form_data(document.getElementById('cbo_source').value, 'line_disable_enable','requires/wash_entry_controller');");
	exit();
}

if ($action=="load_drop_down_floor")
{
	echo create_drop_down( "cbo_floor", 150, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process in (7) order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/wash_entry_controller', this.value+'_'+document.getElementById('cbo_location').value+'_'+document.getElementById('prod_reso_allo').value+'_'+document.getElementById('txt_wash_date').value, 'load_drop_down_wash_line_floor', 'wash_line_td' );",0 );  
	exit();   	 
}
...
{
	if($data==1)
		echo "disable_enable_fields('cbo_wash_line',0,'','');\n";
	else
	{
		echo "$('#cbo_wash_line').val(0);\n";
		echo "disable_enable_fields('cbo_wash_line',1,'','');\n";	
	}
	exit();
}

if($action=="load_drop_down_wash_output")
{
	...
	if($data==3)
	{
		if($db_type==0)
		{
			echo create_drop_down( "cbo_wash_company", 150, "select id,supplier_name from lib_supplier where status_active=1 and is_deleted=0 and find_in_set(22,party_type) order by supplier_name","id,supplier_name", 1, "--- Select ---", $selected, "fnc_loadvariable($('#cbo_company_name').val()); fnc_workorder_search(this.value);",0,0 );
		}
		else
		{	
			echo create_drop_down( "cbo_wash_company", 150, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=22 and a.status_active=1 group by a.id,a.supplier_name order by a.supplier_name","id,supplier_name", 1, "--- Select ---", $selected, " fnc_loadvariable($('#cbo_company_name').val()); fnc_workorder_search(this.value);",0,0 );
		}
	}
	else if($data==1)
	{
 		echo create_drop_down( "cbo_wash_company", 150, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1 $company_cond order by company_name","id,company_name", 1, "--- Select ---", "",  "fnc_loadvariable(this.value); load_drop_down( 'requires/wash_entry_controller', this.value, 'load_drop_down_location', 'location_td' ); fnc_company_check(document.getElementById('cbo_source').value);",0,0 ); 
		
	}
 	else
	{
		echo create_drop_down( "cbo_wash_company", 150, $blank_array,"", 1, "--- Select ---", $selected, "",0,0 );
	}
	exit();
}
...
if($action=="load_drop_down_wash_output_line")
{
	//echo $data; die;
	$explode_data = explode("_",$data);
	$location = $explode_data[0];
	$prod_reso_allocation = $explode_data[1];
	$txt_wash_date = $explode_data[2];
	...
	if($prod_reso_allocation==1)
	{
		...
		else
		{
			if($db_type==0)
			{
				$line_data=sql_select("select a.id, a.line_number from prod_resource_mst a, prod_resource_dtls b where a.id=b.mst_id and b.pr_date='".change_date_format($txt_wash_date,'yyyy-mm-dd')."' and a.location_id='$location' and a.is_deleted=0 and b.is_deleted=0 group by a.id order by a.prod_resource_num");
			}
			if($db_type==2 || $db_type==1)
			{	
				//echo "select a.id, a.line_number from prod_resource_mst a, prod_resource_dtls b where a.id=b.mst_id and b.pr_date='".date("j-M-Y",strtotime($txt_wash_date))."' and a.is_deleted=0 and b.is_deleted=0 and a.location_id='$location' group by a.id, a.line_number";
				$line_data=sql_select("select a.id, a.line_number from prod_resource_mst a, prod_resource_dtls b where a.id=b.mst_id and b.pr_date='".date("j-M-Y",strtotime($txt_wash_date))."' and a.is_deleted=0 and b.is_deleted=0 and a.location_id='$location' group by a.id, a.line_number, a.prod_resource_num order by a.prod_resource_num");
			}
		}
		...
		echo create_drop_down( "cbo_wash_line", 110,$line_array,"", 1, "--- Select ---", $selected, "",0,0 );		
	}
	else
	{
		echo create_drop_down( "cbo_wash_line", 110, "select id, line_name from lib_sewing_line where is_deleted=0 and status_active=1 and location_name='$location' and location_name!=0 order by sewing_line_serial","id,line_name", 1, "Select Line", $selected, "" );
	}
	exit();
}

if($action=="load_drop_down_wash_line_floor")
{
	$explode_data = explode("_",$data);	
	$prod_reso_allocation = $explode_data[2];
	$txt_wash_date = $explode_data[3];
	$cond="";
	...
	if($prod_reso_allocation==1)
	{
		...
		else
		{
			...
			if($db_type==0)
			{
				$line_data=sql_select("select a.id, a.line_number from prod_resource_mst a, prod_resource_dtls b where a.id=b.mst_id and b.pr_date='".change_date_format($txt_wash_date,'yyyy-mm-dd')."' and a.is_deleted=0 and b.is_deleted=0 $cond group by a.id order by a.prod_resource_num");
			}
			if($db_type==2 || $db_type==1)
			{	
				$line_data=sql_select("select a.id, a.line_number from prod_resource_mst a, prod_resource_dtls b where a.id=b.mst_id and b.pr_date='".date("j-M-Y",strtotime($txt_wash_date))."' and a.is_deleted=0 and b.is_deleted=0 $cond group by a.id, a.line_number, a.prod_resource_num order by a.prod_resource_num");
			}
		}
	...
	else
	{
		...
		echo create_drop_down( "cbo_wash_line", 110, "select id,line_name from lib_sewing_line where  is_deleted=0 and status_active=1 and floor_name!=0 $cond order by sewing_line_serial","id,line_name", 1, "--- Select ---", $selected, "",0,0 );
	}
	exit();
}
...
if($action=="create_po_search_list_view")
{
	...
	$variable_qty_source_wash=return_field_value("qty_source_wash","variable_settings_production","company_name=$company and variable_list=42","qty_source_wash");
	$sql_cond="";
	...
	{	
 		$sql = "SELECT b.id, a.order_uom, a.buyer_name, a.company_name, a.total_set_qnty, a.set_break_down, a.job_no, a.style_ref_no, a.gmts_item_id, a.location_name, b.shipment_date, b.po_number,b.file_no,b.grouping, b.po_quantity, b.plan_cut
			...
			where a.id = b.job_id and a.status_active=1 and a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0 and a.garments_nature=$garments_nature $sql_cond order by b.shipment_date DESC";
	}
	...
	$country_sql=sql_select( "SELECT po_break_down_id, item_number_id, country_id, pack_type, sum(order_quantity) as po_qty, sum(plan_cut_qnty) as plan_cut_qty from wo_po_color_size_breakdown where status_active in(1,2,3) and is_deleted=0 $po_id_cond group by po_break_down_id, item_number_id, country_id, pack_type"); 
	...
	/*if($variable_qty_source_wash==1) $qty_source=5; //Sweing Output
	else if($variable_qty_source_wash==2) $qty_source=7;//Iron Output*/
	...
	else if($preceding_process==103) $qty_source=11;//wash Entry*/
	// default source for wash from sewing output

	$qty_source=5;// its is fixed for wash according to urmi instruction
	$total_in_qty_data_arr=array();
    $sql= "select a.po_break_down_id, a.item_number_id, a.country_id, sum(a.production_quantity) as production_quantity, a.pack_type from pro_garments_production_mst a where   a.status_active=1 and a.is_deleted=0 and  a.production_type='$qty_source'  and a.serving_company=$production_company group by a.po_break_down_id, a.item_number_id, a.country_id, a.pack_type ";
    //echo $sql;
...
if($action=="populate_data_from_search_popup")
{
	...
	$qty_source=0;
	if($preceding_process==111) $qty_source=111; //Triming 
	else if($preceding_process==112) $qty_source=112;//Mending 
  	...
  	foreach($res as $result)
	{
		...
		echo "$('#txt_style_no').val('".$result[csf('style_ref_no')]."');\n";
   		...
   		if($qty_source!=0)
   		{
   			$dataArray=sql_select("SELECT SUM(CASE WHEN a.production_type='$qty_source'  THEN a.production_quantity END) as totalinput,SUM(CASE WHEN a.production_type=3 and a.embel_name=3  THEN a.production_quantity ELSE 0 END) as totalwash from pro_garments_production_mst a WHERE  a.po_break_down_id=".$result[csf('id')]."  and a.item_number_id='$item_id' and a.country_id='$country_id' $pack_type_cond and a.status_active=1 and a.is_deleted=0");
	 		foreach($dataArray as $row)
			{  
				echo "$('#txt_input_quantity').val('".$row[csf('totalinput')]."');\n";
				echo "$('#txt_cumul_wash_qty').val('".$row[csf('totalwash')]."');\n";
				$yet_to_produced = $row[csf('totalinput')]-$row[csf('totalwash')];
				echo "$('#txt_yet_to_wash').val('".$yet_to_produced."');\n";
			}
   		}
...
if($action=="color_and_size_level")
{
	...
	if(!$variableSettings)$variableSettings=3;
	...
	if($qty_source !=0)
	{
	 	...
	 	if( $variableSettings==2 ) // color level
	 	{
			if($db_type==0)
			{
				$sql = "SELECT id, item_number_id, size_number_id, color_number_id, order_quantity, sum(plan_cut_qnty) as plan_cut_qnty, (select sum(CASE WHEN pdtls.color_size_break_down_id=wo_po_color_size_breakdown.id then pdtls.production_qnty ELSE 0 END) from pro_garments_production_dtls pdtls where pdtls.production_type='$qty_source' and pdtls.is_deleted=0 ) as production_qnty, (select sum(CASE WHEN cur.color_size_break_down_id=wo_po_color_size_breakdown.id then cur.production_qnty ELSE 0 END) from pro_garments_production_dtls cur where cur.production_type=3 and cur.is_deleted=0 ) as cur_production_qnty 
			...
			else
			{
				$sql = "SELECT a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty,
						...
						sum(CASE WHEN c.production_type=3 and c.embel_name=3 then b.reject_qty ELSE 0 END) as reject_qty
	...
	else //if( $variableSettings==3) color and size level (if not lib set, defaulst this;)
	{
		...
		$dtlsData = sql_select("select a.color_size_break_down_id,
									...
									sum(CASE WHEN a.production_type=3 and b.embel_name=3 then a.production_qnty ELSE 0 END) as cur_production_qnty ,
									sum(CASE WHEN a.production_type=3 and b.embel_name=3 then a.reject_qty ELSE 0 END) as reject_qty
	...
	else // if preceding process =0 in variable setting then plan cut quantity will show
	{
		if( $variableSettings==2 ) // color level
		{
			...
			else
			{
				...
				from wo_po_color_size_breakdown a left join pro_garments_production_dtls b on a.id=b.color_size_break_down_id and b.production_type=3
		...
		{				
			...
			$dtlsData = sql_select("select a.color_size_break_down_id,
										sum(CASE WHEN a.production_type=3 and b.embel_name=3 then a.production_qnty ELSE 0 END) as production_qnty
		...
		else // by default color and size level
		{
			...
			$dtlsData = sql_select("select a.color_size_break_down_id,
										sum(CASE WHEN a.production_type=3 and b.embel_name=3 then a.production_qnty ELSE 0 END) as production_qnty
										from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in(3) group by a.color_size_break_down_id");
 ...
 if($action=="show_dtls_listview")
{
	$company_arr=return_library_array( "select id, company_name from lib_company",'id','company_name');
	$supplier_arr=return_library_array( "select id, supplier_name from lib_supplier",'id','supplier_name');
	$wash_line_arr=return_library_array( "select id, line_name from lib_sewing_line",'id','line_name');
	...
	$pack_type = $dataArr[4];
	$variable_settings = $dataArr[5];
	if($pack_type=='' || $pack_type=="_") $pack_type_cond=""; else $pack_type_cond=" and pack_type='$pack_type'";
	?>	 
     <div style="width:100%;">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="100%" class="rpt_table">
            <thead>
	...
	<div style="width:100%;max-height:180px; overflow:y-scroll" id="wash_production_list_view" align="left">
		...
		<?php  
			...
			if($db_type==2)
			{
				...
				{
					$sqls="SELECT a.id,a.po_break_down_id,a.item_number_id,a.country_id, a.production_date, sum(b.production_qnty) as production_quantity, a.alter_qnty, a.spot_qnty, a.reject_qnty, a.production_source, a.serving_company, a.sewing_line, a.supervisor, a.location, a.prod_reso_allo, TO_CHAR(a.production_hour,'HH24:MI') as prod_hour,a.challan_no,a.wo_order_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and  a. po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' and a.production_type='3' and b.production_type='3' and a.embel_name=3 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $pack_type_cond group by a.id,a.po_break_down_id,a.item_number_id,a.country_id, a.production_date, a.alter_qnty, a.spot_qnty, a.reject_qnty, a.production_source, a.serving_company, a.sewing_line, a.supervisor, a.location, a.prod_reso_allo,  a.production_hour ,a.challan_no,a.wo_order_no order by TO_CHAR(a.production_hour,'DD-MON-YYYY HH24:MI')"; 
				}
				else
				{
					$sqls="SELECT a.id,a.po_break_down_id,a.item_number_id,a.country_id, a.production_date, sum(a.production_quantity) as production_quantity, a.alter_qnty, a.spot_qnty, a.reject_qnty, a.production_source, a.serving_company, a.sewing_line, a.supervisor, a.location, a.prod_reso_allo, TO_CHAR(a.production_hour,'HH24:MI') as prod_hour,a.challan_no,a.wo_order_no from pro_garments_production_mst a where a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' and a.production_type='3' and a.embel_name=3 and a.status_active=1 and a.is_deleted=0 $pack_type_cond group by a.id,a.po_break_down_id,a.item_number_id,a.country_id, a.production_date, a.alter_qnty, a.spot_qnty, a.reject_qnty, a.production_source, a.serving_company, a.sewing_line, a.supervisor, a.location, a.prod_reso_allo,  a.production_hour ,a.challan_no,a.wo_order_no order by TO_CHAR(a.production_hour,'DD-MON-YYYY HH24:MI')"; 
				}
			...
			else 
			{
				if($variable_settings !=1)
				{
					$sqls="SELECT a.id,a.po_break_down_id,a.item_number_id,a.country_id, a.production_date, b.production_qnty as production_quantity, a.alter_qnty, a.spot_qnty, a.reject_qnty, a.production_source, a.serving_company, a.sewing_line, a.supervisor, a.location, a.prod_reso_allo, TIME_FORMAT( a.production_hour, '%H:%i' ) as prod_hour,a.challan_no,a.wo_order_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' and a.production_type='3'  and a.embel_name=3 and b.production_type='3' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $pack_type_cond group by a.id,a.po_break_down_id,a.item_number_id,a.country_id, a.production_date, a.alter_qnty, a.spot_qnty, a.reject_qnty, a.production_source, a.serving_company, a.sewing_line, a.supervisor, a.location, a.prod_reso_allo,   a.production_hour,a.challan_no,a.wo_order_no order by a.production_date, a.production_hour";
				}
				else
				{
					$sqls="SELECT a.id,a.po_break_down_id,a.item_number_id,a.country_id, a.production_date, a.production_quantity as production_quantity, a.alter_qnty, a.spot_qnty, a.reject_qnty, a.production_source, a.serving_company, a.sewing_line, a.supervisor, a.location, a.prod_reso_allo, TIME_FORMAT( a.production_hour, '%H:%i' ) as prod_hour,a.challan_no,a.wo_order_no from pro_garments_production_mst a where a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' and a.production_type='3'  and a.embel_name=3 and b.production_type='3' and a.status_active=1 and a.is_deleted=0 and $pack_type_cond group by a.id,a.po_break_down_id,a.item_number_id,a.country_id, a.production_date, a.alter_qnty, a.spot_qnty, a.reject_qnty, a.production_source, a.serving_company, a.sewing_line, a.supervisor, a.location, a.prod_reso_allo,   a.production_hour,a.challan_no ,a.wo_order_no order by a.production_date, a.production_hour";
				}
			...
			foreach($sqlResult as $selectResult){
				...
				if($selectResult[csf('prod_reso_allo')]==1)
				{
					...
					{
						if($wash_line=='') $wash_line=$wash_line_arr[$val]; else $wash_line.=",".$wash_line_arr[$val];
					}
				}
				else $wash_line=$wash_line_arr[$selectResult[csf('sewing_line')]];
				
  		?>
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data('<? echo $selectResult[csf('id')].'**'.$variable_settings; ?>','populate_input_form_data','requires/wash_entry_controller');" > 
                ...
                <td width="50" align="center"><p><? echo $wash_line; ?></p></td>
				...
				<td width="90" align="center"><p><? echo $selectResult['WO_ORDER_NO']; ?></p></td>
			</tr>
			<?php
			$i++;
			}
...
if($action=="show_country_listview")
{
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="400" class="rpt_table">
		...
		<? 
		$issue_qnty_arr=sql_select("select a.po_break_down_id, a.item_number_id, a.country_id, a.pack_type, a.production_quantity as cutting_qnty from pro_garments_production_mst a where  a.po_break_down_id='$data' and a.production_type=3 and a.embel_name=3 and a.status_active=1 and a.is_deleted=0 ");
...
if($action=="populate_input_form_data")
{
	...
	$variable_qty_source_wash=return_field_value("qty_source_wash","variable_settings_production","company_name=$company_id and variable_list=42","qty_source_wash");
	$control_and_preceding=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=50 and page_category_id=3 and company_name='$company_id'");  
	...
	if($preceding_process==111) $qty_source=111; //Triming 
		...
		{ 
			echo "$('#txt_wash_date').val('".change_date_format($result[csf('production_date')])."');\n";
			echo "$('#txt_wash_qty').val('".$result[csf('production_quantity')]."');\n";
			echo "$('#cbo_source').val('".$result[csf('production_source')]."');\n";
			echo "load_drop_down( 'requires/wash_entry_controller', ".$result[csf('production_source')].", 'load_drop_down_wash_output', 'sew_company_td' );\n";
			echo "$('#cbo_wash_company').val('".$result[csf('serving_company')]."');\n";
			echo "load_drop_down( 'requires/wash_entry_controller',".$result[csf('serving_company')].", 'load_drop_down_location', 'location_td' );";

			echo "$('#cbo_location').val('".$result[csf('location')]."');\n";
			echo "load_drop_down( 'requires/wash_entry_controller', ".$result[csf('location')].", 'load_drop_down_floor', 'floor_td' );\n";
			echo "$('#cbo_floor').val('".$result[csf('floor_id')]."');\n";
			
			echo "load_drop_down( 'requires/wash_entry_controller', document.getElementById('cbo_floor').value+'_'+document.getElementById('cbo_location').value+'_'+document.getElementById('prod_reso_allo').value+'_'+document.getElementById('txt_wash_date').value, 'load_drop_down_wash_line_floor', 'wash_line_td' );\n";
			
			echo "$('#cbo_wash_line').val('".$result[csf('sewing_line')]."');\n";
			echo "get_php_form_data(".$result[csf('production_source')].",'line_disable_enable','requires/wash_entry_controller');\n";
			...
			if($result[csf('production_source')]==3)
			{
				...
				$preceding_process= $control_and_preceding[0][csf("preceding_page_id")];
				echo "$('#hidden_variable_cntl').val('".$control_and_preceding[0][csf('is_control')]."');\n";
 	 			echo "load_drop_down( 'requires/wash_entry_controller', '".$result[csf('company_id')]."_".$result[csf('serving_company')]."_".$result[csf('po_break_down_id')]."', 'load_drop_down_workorder', 'workorder_td' );\n";
			...
			if($qty_source!=0)
			{
				$dataArray=sql_select("select SUM(CASE WHEN a.production_type='$qty_source' and b.production_type='$qty_source' THEN  b.production_qnty END) as totalinput,SUM(CASE WHEN a.production_type=3 and a.embel_name=3 and b.production_type=3  THEN b.production_qnty ELSE 0 END) as totalwash from pro_garments_production_mst a,pro_garments_production_dtls b WHERE a.id=b.mst_id and a.po_break_down_id=".$result[csf('po_break_down_id')]." and a.item_number_id=".$result[csf('item_number_id')]." and a.country_id=".$result[csf('country_id')]." $pack_typeCond and a.production_source=".$result[csf('production_source')]." and a.serving_company=".$result[csf('serving_company')]." and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
		  		foreach($dataArray as $row)
				{  
					echo "$('#txt_input_quantity').val('".$row[csf('totalinput')]."');\n";
					echo "$('#txt_cumul_wash_qty').val('".$row[csf('totalwash')]."');\n";
			...
			$defect_sql=sql_select("select id, po_break_down_id, defect_type_id, defect_point_id, defect_qty from pro_gmts_prod_dft where mst_id='".$result[csf('id')]."' and status_active=1 and is_deleted=0 and production_type='3'");
			...
			echo "$('#txt_sys_chln').val('".$result[csf('id')]."');\n";
	 		echo "set_button_status(1, permission, 'fnc_wash_output_entry',1,1);\n";
				...
				{ 
					...
					if( $variableSettings==2 ) // color level
					{
						if($db_type==0)
						{
							...
							$sql = "SELECT id, item_number_id, size_number_id, color_number_id, order_quantity, sum(plan_cut_qnty) as plan_cut_qnty, (select sum(CASE WHEN pdtls.color_size_break_down_id=wo_po_color_size_breakdown.id then pdtls.production_qnty ELSE 0 END) from pro_garments_production_dtls pdtls where pdtls.production_type='$qty_source' and pdtls.is_deleted=0 ) as production_qnty, (select sum(CASE WHEN cur.color_size_break_down_id=wo_po_color_size_breakdown.id then cur.production_qnty ELSE 0 END) from pro_garments_production_dtls cur where cur.production_type=3 and cur.is_deleted=0) as cur_production_qnty, (select sum(CASE WHEN cur.color_size_break_down_id=wo_po_color_size_breakdown.id then cur.reject_qty ELSE 0 END) from pro_garments_production_dtls cur where cur.production_type=3 and cur.is_deleted=0 ) as reject_qty
							from wo_po_color_size_breakdown
					...
					else if( $variableSettings==3 ) //color and size level
					{
							$dtlsData = sql_select("SELECT a.color_size_break_down_id,
												...
												sum(CASE WHEN a.production_type=3 and b.embel_name=3 then a.production_qnty ELSE 0 END) as cur_production_qnty,
												sum(CASE WHEN a.production_type=3 and b.embel_name=3 then a.reject_qty ELSE 0 END) as reject_qty
												from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id and b.id=$id and a.color_size_break_down_id!=0 and a.production_type in($qty_source,3) $pack_cond group by a.color_size_break_down_id ");
				...
				{ 
					...
					if( $variableSettings==2 ) // color level
					{
						if($db_type==0)
						{
							...
							$sql = "select id, item_number_id, size_number_id, color_number_id, order_quantity, sum(plan_cut_qnty) as plan_cut_qnty, (select sum(CASE WHEN pro_garments_production_dtls.color_size_break_down_id=wo_po_color_size_breakdown.id then production_qnty ELSE 0 END) from pro_garments_production_dtls where is_deleted=0 and  	production_type=3 ) as production_qnty, (select sum(CASE WHEN pro_garments_production_dtls.color_size_break_down_id=wo_po_color_size_breakdown.id then reject_qty ELSE 0 END) from pro_garments_production_dtls where is_deleted=0 and production_type=3 ) as reject_qty
							from wo_po_color_size_breakdown
					...
					else if( $variableSettings==3 ) //color and size level
					{
							...
							$dtlsData = sql_select("select a.color_size_break_down_id,
												...
												sum(a.reject_qty) as reject_qty
												from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id and b.id=$id and a.color_size_break_down_id!=0 and a.production_type=3 and  b.embel_name=3  group by a.color_size_break_down_id");
							//and b.id='$data'
                          //echo $dtlsData;die;
							foreach($dtlsData as $row)
							{				  
							...
							$sql = "select id, item_number_id, size_number_id, color_number_id, order_quantity, plan_cut_qnty 
							from wo_po_color_size_breakdown
							where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' and is_deleted=0 and status_active in(1) order by color_number_id,size_order"; 
					...
					foreach($colorResult as $color)
					{
						...
						else //color and size level
						{
							...
							$colorHTML .='<tr><td>'.$size_library[$color[csf("size_number_id")]].'</td><td><input type="hidden" name="bundlemst" id="bundle_mst_'.$color[csf("color_number_id")].($i+1).'" value="'.$bundle_mst_data.'"  class="text_boxes_numeric" style="width:100px"  ><input type="hidden" name="bundledtls" id="bundle_dtls_'.$color[csf("color_number_id")].($i+1).'"  class="text_boxes_numeric" style="width:100px" value="'.$bundle_dtls_data.'" ><input type="text" name="colorSize" id="colSize_'.$color[csf("color_number_id")].($i+1).'"  class="text_boxes_numeric" style="width:50px" placeholder="'.($cut_qnty+$amount).'" onblur="fn_total('.$color[csf("color_number_id")].','.($i+1).')" value="'.$cut_qnty.'" ><input type="text" name="colorSizeRej" id="colSizeRej_'.$color[csf("color_number_id")].($i+1).'"  class="text_boxes_numeric" style="width:50px" placeholder="Rej. Qty" onblur="fn_total_rej('.$color[csf("color_number_id")].','.($i+1).')" value="'.$rej_qnty.'" '.$disable.'></td><td><input type="button" name="button" value="Click For Bundle" class="formbutton" style="size:30px;" onclick="openmypage_bandle('.$color[csf("id")].','.$color[csf("color_number_id")].($i+1).','.$tmp_col_size.');" /></td></tr>';				
							//$colorWiseTotal += $amount;
							 $bundle_dtls_data="";
							 $bundle_dtls_data="";
						}
...
if ($action=="save_update_delete")
{
	...
	if(!str_replace("'","",$wash_production_variable)) $wash_production_variable=3;
	...
	//$variable_qty_source_wash=return_field_value("qty_source_wash","variable_settings_production","company_name=$cbo_company_name and variable_list=42","qty_source_wash");
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		{
			$txt_wash_qty=str_replace("'","",$txt_wash_qty);
			$country_wash_input_qty=return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=$hidden_po_break_down_id and production_type=4 and country_id=$cbo_country_name and status_active=1 and is_deleted=0");
			
			$country_wash_output_qty=return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=$hidden_po_break_down_id and production_type=5 and country_id=$cbo_country_name and status_active=1 and is_deleted=0");
			
			//echo $country_wash_output_qty .'<'. $country_iron_qty.'+'.$txt_iron_qty;die;
			if($country_wash_input_qty < $country_wash_output_qty+$txt_wash_qty)
			{
				echo "25**0";
				disconnect($con);
				die;
		...
		$field_array1="id, garments_nature, company_id, challan_no, po_break_down_id, item_number_id, country_id, pack_type, production_source, serving_company, location, produced_by, production_date, production_quantity, production_type,embel_name, entry_break_down_type, break_down_type_rej, sewing_line, supervisor, production_hour, remarks, floor_id, alter_qnty, reject_qnty, total_produced, yet_to_produced, prod_reso_allo, spot_qnty, wo_order_id, wo_order_no, currency_id, exchange_rate, rate, amount, inserted_by, insert_date";
		
		$rate=str_replace("'","",$hidden_piece_rate);
		if($rate!="") { $amount=$rate*str_replace("'","",$txt_wash_qty);}
		
		if($db_type==0)
		{
			$data_array1="(".$id.",".$garments_nature.",".$cbo_company_name.",".$txt_challan.",".$hidden_po_break_down_id.",".$cbo_item_name.",".$cbo_country_name.",".$txt_pack_type.",".$cbo_source.",".$cbo_wash_company.",".$cbo_location.",".$cbo_produced_by.",".$txt_wash_date.",".$txt_wash_qty.",3,3,".$wash_production_variable.",".$wash_production_variable_rej.",".$cbo_wash_line.",".$txt_super_visor.",".$txt_reporting_hour.",".$txt_remark.",".$cbo_floor.",".$txt_alter_qnty.",".$txt_reject_qnty.",".$txt_cumul_wash_qty.",".$txt_yet_to_wash.",".$prod_reso_allo.",".$txt_spot_qnty.",".$txt_wo_id.",".$txt_wo_no.",".$hidden_currency_id.",".$hidden_exchange_rate.",'".$rate."','".$amount."',".$user_id.",'".$pc_date_time."')";
		}
		else
		{
			$txt_reporting_hour=str_replace("'","",$txt_wash_date)." ".str_replace("'","",$txt_reporting_hour);
		...
		$dtlsData = sql_select("select a.color_size_break_down_id,
										...
										where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name and b.country_id=$cbo_country_name and a.color_size_break_down_id!=0 and a.production_type in($qty_source,3) $pack_type_cond
		...
		if(str_replace("'","",$wash_production_variable)==2)//color level wise
		{		
			...
			foreach($rowEx as $rowE=>$val)
			{
				...
				if($is_control==1 && $user_level!=2)
				{
					if($colorSizeNumberIDArr[1]>0)
					{
						...
						{
							echo "35**Production Quantity Not Over wash Input Qnty";
		...
		if(str_replace("'","",$wash_production_variable)==3)//color and size wise
		{		
		...
		{		 
			...
			foreach($defect_array as $key=>$val)
			{
				...
				$defectPointId=$key;
				$defect_qty=$val;
				$data_array_defect.="(".$dft_id.",".$id.",3,".$hidden_po_break_down_id.",".$defect_type_id.",".$defectPointId.",'".$defect_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			} 
		}
		...
		{		 
			...
			foreach($defectSpt_array as $keysp=>$valsp)
			{
				...
				$defectsp_qty=$valsp;
				$data_array_defectsp.="(".$dftSp_id.",".$id.",3,".$hidden_po_break_down_id.",".$defectSpot_type_id.",".$defectspPointId.",'".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			} 
		}
		...
		if(str_replace("'","",$wash_production_variable)==2 || str_replace("'","",$wash_production_variable)==3)
		{
		...
		if($db_type==0)
		{	  
			if(str_replace("'","",$wash_production_variable)!=1)
			{
				if($rID && $dtlsrID)
				{
					mysql_query("COMMIT");  
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		/*if($is_control==1 && $user_level!=2)
		{
			...
			$country_wash_output_qty=return_field_value("sum(production_quantity)","pro_garments_production_mst","po_break_down_id=$hidden_po_break_down_id and production_type=5 and country_id=$cbo_country_name and status_active=1 and is_deleted=0 and id <> $txt_mst_id");
			
			// echo $country_wash_input_qty .'<'. $country_wash_output_qty.'+'.$txt_wash_qty;die;
			if($country_wash_input_qty < $country_wash_output_qty+$txt_wash_qty)
			{
				echo "25**0";
				disconnect($con);
	...
	$sql_result=sql_select("select b.bundle_no from  pro_garments_production_mst a,pro_garments_production_dtls b where  a.id=b.mst_id and a.id =$txt_mst_id and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1");
		...
		if(count($check_bundle_no) > 0){echo "101**".implode(',',$check_bundle_no);exit();}
		...
		if($db_type==2)
		{
 		...
 		$field_array1="pack_type*production_source*serving_company*location*produced_by*production_date*production_quantity*production_type*entry_break_down_type* break_down_type_rej*sewing_line*supervisor*production_hour*challan_no*remarks*floor_id*reject_qnty*alter_qnty*total_produced*yet_to_produced*prod_reso_allo*spot_qnty*wo_order_id*wo_order_no*currency_id *exchange_rate*rate*amount*updated_by*update_date";
		$rate=str_replace("'","",$hidden_piece_rate);
		if($rate!="") {  $amount=$rate*str_replace("'","",$txt_sewing_qty);}
		else {$amount="";}
		
		$data_array1="".$txt_pack_type."*".$cbo_source."*".$cbo_wash_company."*".$cbo_location."*".$cbo_produced_by."*".$txt_wash_date."*".$txt_wash_qty."*3*".$wash_production_variable."*".$wash_production_variable_rej."*".$cbo_wash_line."*".$txt_super_visor."*".$txt_reporting_hour."*".$txt_challan."*".$txt_remark."*".$cbo_floor."*".$txt_reject_qnty."*".$txt_alter_qnty."*".$txt_cumul_wash_qty."*".$txt_yet_to_wash."*".$prod_reso_allo."*".$txt_spot_qnty."*".$txt_wo_id."*".$txt_wo_no."*".$hidden_currency_id."*".$hidden_exchange_rate."*'".$rate."'*'".$amount."'*".$user_id."*'".$pc_date_time."'";
		...
		if(str_replace("'","",$wash_production_variable)!=1 && str_replace("'","",$txt_mst_id)!='' ) // check is not gross level
		{
			...
			$dtlsData = sql_select("select a.color_size_break_down_id,
										...
										sum(CASE WHEN a.production_type=3 and b.embel_name=3 then a.production_qnty ELSE 0 END) as cur_production_qnty 
										from pro_garments_production_dtls a,pro_garments_production_mst b 
										where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name and b.country_id=$cbo_country_name and a.color_size_break_down_id!=0 and a.production_type in($qty_source,3) and b.id!=$txt_mst_id $pack_type_cond
										group by a.color_size_break_down_id");
			$color_pord_data=array();							
			foreach($dtlsData as $row)
			{				  
		...
		{		 
			...
			foreach($defect_array as $key=>$val)
			{
				...
				$defectPointId=$key;
				$defect_qty=$val;
				$data_array_defect.="(".$dft_id.",".$txt_mst_id.",3,".$hidden_po_break_down_id.",".$defect_type_id.",".$defectPointId.",'".$defect_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			} 
		}
		if($data_array_defect!="")
		{
			...
			$query3=execute_query("DELETE FROM pro_gmts_prod_dft WHERE mst_id=$txt_mst_id and defect_type_id=1 and production_type=3");
			$defectQ=sql_insert("pro_gmts_prod_dft",$field_array_defect,$data_array_defect,1);
		}
		$defectSpot=true;
		$data_array_defectsp="";
		...
		{		 
			...
			foreach($defectSpt_array as $keysp=>$valsp)
			{
				...
				$defectspPointId=$keysp;
				$defectsp_qty=$valsp;
				$data_array_defectsp.="(".$dftSp_id.",".$txt_mst_id.",3,".$hidden_po_break_down_id.",".$defectSpot_type_id.",".$defectspPointId.",'".$defectsp_qty."',".$user_id.",'".$pc_date_time."')";
				$i++;
			} 
		}
		...
		if($data_array_defectsp!="")
		{
			...
			$query4=execute_query("DELETE FROM pro_gmts_prod_dft WHERE mst_id=$txt_mst_id and defect_type_id=2 and production_type=3");
			$defectSpot=sql_insert("pro_gmts_prod_dft",$field_array_defectsp,$data_array_defectsp,0);
		}
...
if($action=="wash_output_print")
{
	...
	$wash_library=return_library_array( "select id, line_name from  lib_sewing_line", "id", "line_name"  );
...
<div style="width:930px;">
    <table width="900" cellspacing="0" align="right">
        ...
        <tr>
            <td><strong>wash Line: </strong></td><td><? echo $wash_library[$line_data_variable[$dataArray[0][csf('sewing_line')]]]; ?></td>
...
if($action=="production_process_control")
{
    ...
    $control_and_preceding=sql_select("select is_control,preceding_page_id from variable_settings_production where status_active=1 and is_deleted=0 and variable_list=50 and page_category_id=3 and company_name='$data'");  
