...
if ($action=="load_drop_down_location")
{
	...
	else { $dropdown_name="cbo_working_location";  $load_fnc="load_drop_down('requires/bundle_trimming_controller', this.value, 'load_drop_down_floor', 'floor_td' );"; }
...
{
    echo create_drop_down("cbo_floor", 130, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process=5 order by floor_name", "id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/bundle_trimming_controller', $('#cbo_working_company').val()+'_'+$('#cbo_working_location').val()+'_'+this.value+'_'+$('#txt_trim_date').val(), 'load_drop_down_line', 'line_td' );", 0);
    exit();
}
...
if ($action == "populate_bundle_data") 
{
    ...
    $scanned_bundle_arr = return_library_array("select b.bundle_no, b.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=111 and b.status_active=1 and b.is_deleted=0 $scnbundle_nos_cond", 'bundle_no', 'bundle_no');
...
if($action=="populate_mst_data")
{
    ...
    where a.entry_form=322 and b.entry_form=322 and a.id=b.delivery_mst_id and b.barcode_no='$data[0]' and a.company_id='$data[1]' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	...
	foreach($sql_pop_res as $row)
    {
		...
		echo "load_drop_down( 'requires/bundle_trimming_controller', '".$row[csf("working_company_id")]."_".$row[csf("working_location_id")]."_".$row[csf("floor_id")]."_".$data[2]."', 'load_drop_down_line', 'line_td' );\n";
...
if ($action == "create_bundle_search_list_view") 
{
  ...
  // echo $tmp_cut;
   $scanned_bundle_arr = return_library_array("select bundle_no, bundle_no from pro_garments_production_dtls where production_type=111 and cut_no='".$tmp_cut."' and status_active=1 and is_deleted=0", 'bundle_no', 'bundle_no');
    foreach (explode(",", $selectedBuldle) as $bn) {
        $scanned_bundle_arr[$bn] = $bn;
    }

    $scanne=sql_select( "select b.bundle_no, sum(b.production_qnty) as production_qnty,a.sewing_line from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and b.production_type=111 and b.status_active=1 and b.is_deleted=0 $cutCon_a group by b.bundle_no,a.sewing_line");
    ...
    // echo $cutting_no;
    $last_operation=gmt_production_validation_script( 56, 1,'', $cutting_no, $production_squence);
    //$last_operation=gmt_production_validation_script( 4, 1 );
...
if ($action=="save_update_delete")
{
	...
	{
		$rec_number=return_field_value("sys_number", "pro_gmts_delivery_mst"," issue_challan_id=$txt_update_id and status_active=1 and is_deleted=0 and production_type=89 and entry_form=320");
		if($iss_number){
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if (str_replace("'", "", $txt_update_id) == "")
		{
			...
			$new_sys_number = explode("*", return_next_id_by_sequence("", "pro_gmts_delivery_mst",$con,1,$cbo_company_name,'TRM',0,date("Y",time()),0,0,111,0,0 ));
			...
			$data_array_delivery="(".$mst_id.",'".$new_sys_number[1]."','".(int)$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",111,".$cbo_location.",".$cbo_source.",".$cbo_working_company.",".$cbo_working_location.",".$cbo_floor.",".$cbo_line_no.",".$txt_trim_date.",'".$challanNo."',".$txt_remarks.",671,".$user_id.",'".$pc_date_time."',1,0)";
		...
		else 
		{
			...
			$data_array_delivery = "".$cbo_location."*".$cbo_source."*".$cbo_working_company."*".$cbo_working_location."*".$cbo_floor."*".$cbo_line_no."*".$txt_trim_date."*'".$challanNo."'*".$txt_remarks."*".$user_id."*'".$pc_date_time."'";
		...
		if($db_type==2) 
		{
			$txt_reporting_hour=str_replace("'","",$txt_trim_date)." ".str_replace("'","",$txt_reporting_hour);
		...
		foreach($dtlsArr as $bundle_no=>$qty)
		{
			...
			$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",111,'".$dtlsArrColorSize[$bundle_no]."','".$qty."','".$dtlsArrRej[$bundle_no]."','".$dtlsArrAlt[$bundle_no]."','".$dtlsArrSpot[$bundle_no]."','".$dtlsArrRep[$bundle_no]."','".$cutNoArr[$bundle_no]."','".$bundle_no."',671,'".$bundleBarcodeArr[$bundle_no]."','".$bundleRescanArr[$bundle_no]."',1,0)"; 
		}
		$flag=1;
		if (str_replace("'", "", $txt_update_id) == "")
		{
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$non_delete_arr=production_validation($mst_id,89);
		$issue_data_arr=production_data($mst_id,111);
		...
		$flag=1;
		$delete = execute_query("update pro_garments_production_mst set status_active=0,is_deleted=1,updated_by=$user_id,update_date='".$pc_date_time."' WHERE delivery_mst_id=$mst_id and production_type=111 and status_active=1 and is_deleted=0");
		if($delete==1 && $flag==1) $flag=1; else $flag=0;
		$delete_dtls = execute_query("update pro_garments_production_dtls set status_active=0, is_deleted=1 WHERE delivery_mst_id=$mst_id and production_type=111 and status_active=1 and is_deleted=0");
...
{
  	echo load_html_head_contents("Trimming Info","../../../", 1, 1, '','1','');
	extract($_REQUEST);
	?>
	<script>
...
if($action=="populate_bundle_data_update")
{
	...
	$sql_bundle="select barcode_no from pro_garments_production_dtls where delivery_mst_id='$mst_id' and production_type=56 and status_active=1 and is_deleted=0";
