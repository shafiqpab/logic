...
if ($action=="load_drop_down_location")
{
	...
	else if($data[1]==2) { $dropdown_name="cbo_emb_location"; $load_function="load_drop_down( 'requires/embel_issue_controller', this.value, 'load_drop_down_floor', 'floor_td' );"; }
...
if($action=="load_drop_down_embl_name")
{
	...
	echo create_drop_down( "cbo_embel_name", 130, $emblishment_name_array,"", 1, "-Select Embel.Name-", $selected, "load_drop_down( 'requires/embel_issue_controller', this.value+'**'+document.getElementById('txt_job_no').value, 'load_drop_down_embro_issue_type', 'embro_type_td');","","$emb_name" );
	exit();
}
...
{
  	echo load_html_head_contents("Embellishment Issue Info","../../../", 1, 1, '','1','');
	extract($_REQUEST);
	?>
	<script>
		function js_set_value( strCon ) 
    ...
    <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            ...
            <tbody>
                <tr class="general">                    
                    ...
                    <td>
                    	<input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('txt_issue_no').value+'_'+document.getElementById('txt_job_search').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_order_search').value+'_'+document.getElementById('txt_style_ref').value, 'create_system_search_list_view', 'search_div', 'embel_issue_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />				
                    </td>
                </tr>
                <tr>                  
...
if($action=="create_system_search_list_view")
{
    ...
    where c.garments_nature=100 and a.entry_form=326 and b.entry_form=326 and a.id=b.delivery_mst_id and b.po_break_down_id=d.id and c.job_no=d.job_no_mst $conpany_cond $job_cond $sql_cond $order_cond $system_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if (str_replace("'","",$txt_update_id)=="")
		{
			...
			$new_sys_number = explode("*", return_next_id_by_sequence("", "pro_gmts_delivery_mst",$con,1,$cbo_company_name,'EMBI',0,date("Y",time()),0,0,63,0,0 ));
			// txt_wo_id
			...
			$data_array_delivery="(".$mst_id.",'".$new_sys_number[1]."','".(int)$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",63,".$cbo_location.",".$cbo_source.",".$cbo_emb_company.",".$cbo_emb_location.",".$cbo_floor.",".$txt_job_id.",".$txt_job_no.",".$txt_issue_date.",".$cbo_embel_name.",".$cbo_embel_type.",'".$sending_company."','".$sending_location."',".$txt_challan.",".$txt_remark.",326,".$user_id.",'".$pc_date_time."',1,0)";
		}
		else
		{
			...
			$data_array_delivery = "".$cbo_location."*".$cbo_source."*".$cbo_emb_company."*".$cbo_emb_location."*".$cbo_floor."*".$txt_job_id."*".$txt_job_no."*".$txt_issue_date."*".$cbo_embel_name."*".$cbo_embel_type."*".$sending_company."*".$sending_location."*".$txt_challan."*".$txt_remark."*".$user_id."*'".$pc_date_time."'";
		}
		...
		$field_array_mst="id, delivery_mst_id, garments_nature, company_id, location, production_source, sending_location, sending_company, floor_id, production_date, challan_no, remarks, po_break_down_id, item_number_id, country_id, serving_company, embel_name, embel_type, production_quantity, production_type, entry_break_down_type, entry_form, inserted_by, insert_date, status_active, is_deleted, wo_order_id";
		$data_array_mst="";
		...
		foreach($mstArr as $orderId=>$orderData)
		{
			foreach($orderData as $gmtsItemId=>$gmtsItemIdData)
			{
				foreach($gmtsItemIdData as $countryId=>$qty)
				{
					...
					$data_array_mst.="(".$id.",".$mst_id.",".$garments_nature.",".$cbo_company_name.",".$cbo_location.",".$cbo_source.",'".$sending_company."','".$sending_location."',".$cbo_floor.",".$txt_issue_date.",".$txt_challan.",".$txt_remark.",".$orderId.", ".$gmtsItemId.",".$countryId.",".$cbo_emb_company.",".$cbo_embel_name.",".$cbo_embel_type.",".$qty.",63,3,326,".$user_id.",'".$pc_date_time."',1,0,".$txt_wo_id.")";
		...
		foreach($dtlsArr as $colorSizeid=>$qty)
		{
			...
			$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",63,'".$colorSizeid."','".$qty."',326,1,0)"; 
		}
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$field_array_up="location*production_source*sending_location*sending_company*floor_id*production_date*challan_no*remarks*po_break_down_id*item_number_id*country_id*serving_company*embel_name*embel_type*production_quantity*updated_by*update_date";
			...
			if($orderId)
			{
				foreach ($orderData as $gmtsItemId => $gmtsItemIdData)
				{
					foreach ($gmtsItemIdData as $countryId => $qty) 
					{
						...
						{
							$data_array_up[$gmtProdId] =explode("*",("".$cbo_location."*".$cbo_source."*'".$sending_company."'*'".$sending_location."'*".$cbo_floor."*".$txt_issue_date."*".$txt_challan."*".$txt_remark."*".$orderId."*".$gmtsItemId."*".$countryId."*".$cbo_emb_company."*".$$cbo_embel_name."*".$cbo_embel_type."*".$qty."*'".$user_id."'*'".$pc_date_time."'"));
							$id=$gmtProdId;
							$is_up=1;
							$id_arr[]=$gmtProdId;
						}
		...
		$dtlsrDelete = execute_query("update pro_garments_production_dtls set status_active=0, is_deleted=1 WHERE delivery_mst_id=$mst_id and production_type=63 and status_active=1 and is_deleted=0");
...
if($action=="emblishment_issue_print") // Print 2 Start.
{
	extract($_REQUEST);
	$data=explode('*',$data);
...
//	echo $sql;die;
	...
	$sql_color_type=sql_select("SELECT color_type_id from pro_garments_production_dtls where status_active=1 and is_deleted=0 and delivery_mst_id=$mst_id and production_type=63");
