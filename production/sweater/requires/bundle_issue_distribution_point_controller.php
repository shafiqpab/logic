...
if($action=="show_dtls_listview_bundle")
{
	...
	<div style="width:<?php echo $div_width;?>px;max-height:250px;overflow-y:scroll" align="left"> 
		...
		<?php   
			...
			$sqlResult =sql_select("select b.* , a.gmt_item_id, a.color_id, c.machine_id, c.bundle_qty, c.production_qnty, c.color_size_break_down_id from ppl_cut_lay_dtls a, ppl_cut_lay_bundle b, pro_garments_production_dtls c where $barcode_cond and c.production_type=52 and a.id=b.dtls_id and b.barcode_no=c.barcode_no and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0");
			//echo "select b.* , a.gmt_item_id, a.color_id, c.machine_id, c.bundle_qty, c.production_qnty, c.color_size_break_down_id from ppl_cut_lay_dtls a, ppl_cut_lay_bundle b, pro_garments_production_dtls c where $barcode_cond and c.production_type=52 and c.bundle_no=b.bundle_no and a.id=b.dtls_id  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
...
if($action=="populate_bundle_data")
{
	...
	$sqlResult =sql_select("select b.* , a.gmt_item_id, a.color_id, c.machine_id, c.bundle_qty, c.production_qnty, c.color_size_break_down_id
					...
					where c.production_type=52 and a.id=b.dtls_id and b.barcode_no=c.barcode_no $barcode_cond $oldbarcode_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0");
...
if($action=="challan_duplicate_check")
{
	...
	if($exdata[1]==1)
	{
		$firstinssql="select b.BARCODE_NO from pro_gmts_cutting_qc_mst a, pro_garments_production_dtls b where a.id=b.delivery_mst_id and a.CUTTING_QC_NO='$exdata[0]' and a.production_type=52 and b.production_type=52 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
		//echo $firstinssql;
    ...
    where a.garments_nature=100 and f.PRODUCTION_TYPE=86 and a.id=f.delivery_mst_id and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0 $bundle_nos_cond group by a.cutting_qc_no, f.barcode_no");
...
if($action=='populate_data_from_barcode')
{
	...
	{ 
		...
		echo "load_drop_down( 'requires/bundle_issue_distribution_point_controller', ".($row[csf("production_source")])."+'_'+".($row[csf("company_id")]).", 'load_drop_down_knitting_com','knitting_com');";
		...
		echo "load_drop_down( 'requires/bundle_issue_distribution_point_controller', ".$row[csf("service_location")].", 'load_drop_down_floor', 'floor_td' );";
		
		echo "document.getElementById('cbo_floor').value  					= '".($row[csf("floor_id")])."';\n";
		echo "load_drop_down( 'requires/bundle_issue_distribution_point_controller', '".$row[csf('company_id')]."', 'load_drop_down_lc_location', 'location_td' );\n";
...
if($action=="populate_bundle_data_rescan")
{
	...
	$sqlResult =sql_select("select b.* ,a.gmt_item_id, a.color_id, c.machine_id, c.bundle_qty, b.size_qty, c.color_size_break_down_id
									from ppl_cut_lay_dtls a, ppl_cut_lay_bundle b, pro_garments_production_dtls c 
									where $barcode_cond and c.production_type=86 and b.mst_id=$mst_id and b.dtls_id=$dtls_id and c.bundle_no=b.bundle_no and a.id=b.dtls_id and a.color_id=$color_id  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
...
if($action=="show_dtls_listview_bundle_rescan2")
{
	...
	//echo "SELECT b.bundle_no,b.barcode_no,b.order_id,b.size_id,b.country_id , a.gmt_item_id, sum(c.production_qnty) as production_qnty, c.color_size_break_down_id from ppl_cut_lay_dtls a, ppl_cut_lay_bundle b, pro_garments_production_dtls c where $barcode_cond and c.production_type=52 and b.mst_id=$mst_id and b.dtls_id=$dtls_id and c.bundle_no=b.bundle_no and a.id=b.dtls_id and a.color_id=$color_id  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.bundle_no,b.barcode_no,b.order_id,b.size_id,b.country_id , a.gmt_item_id, c.color_size_break_down_id";
	foreach ($sqlResult as $key => $value) 
	{
		$input_qty_arr[$val[csf('barcode_no')]]+=$val[csf('production_qnty')];
		$input_barcode_arr[]=$val[csf('barcode_no')];
...
if($action=="load_mst_data")
{
	...
	if(count($sql_mst_data)>0)
	{
		...
		{
			echo "load_drop_down( 'requires/bundle_issue_distribution_point_controller', '".$val[csf('company_id')]."', 'load_drop_down_lc_location', 'location_td' );\n";

			echo "load_drop_down( 'requires/bundle_issue_distribution_point_controller', '".$val[csf('serving_company')]."', 'load_drop_down_location', 'working_location_td' );\n";

			echo "load_drop_down( 'requires/bundle_issue_distribution_point_controller','".$val[csf('service_location')]."', 'load_drop_down_floor', 'floor_td');\n";
			...
			echo "$('#txt_issue_date').val('".change_date_format($val[csf('cutting_qc_date')])."');\n";
		}
	}
	else
...
if($action=="bundle_popup")
{
			...
			<legend></legend>           
	            <table cellpadding="0" cellspacing="0" width="550" border="1" rules="all" class="rpt_table">
	                ...
	                <tr class="general">
	            		...
	            		<td align="center">
	                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view (document.getElementById('cbo_company_name').value+'_'+document.getElementById('bundle_no').value+'_'+'<? echo trim($bundleNo,','); ?>'+'_'+document.getElementById('txt_job_no').value+'_'+document.getElementById('txt_lot_no').value+'_'+document.getElementById('cbo_lot_year').value+'_'+'<? echo trim($lot_ratio,','); ?>','create_bundle_search_list_view','search_div','bundle_issue_distribution_point_controller','setFilterGrid(\'tbl_list_search\',-1);reset_hide_field();')" style="width:100px;" />
	                     </td>
	                </tr>
	           </table>
	...
	<script>
		...
		{
			show_list_view (document.getElementById('cbo_company_name').value+'_'+document.getElementById('bundle_no').value+'_'+'<? echo trim($bundleNo,','); ?>'+'_'+document.getElementById('txt_job_no').value+'_'+document.getElementById('txt_lot_no').value+'_'+document.getElementById('cbo_lot_year').value+'_'+'<? echo trim($lot_ratio,','); ?>','create_bundle_search_list_view','search_div','bundle_issue_distribution_point_controller','setFilterGrid(\'tbl_list_search\',-1);reset_hide_field();');
		}
	</script>
	</html>
	<?
...
if($action=="create_bundle_search_list_view")
{
	...
	$scanned_bundle_arr=return_library_array("SELECT a.bundle_no, a.bundle_no from pro_garments_production_mst b, pro_garments_production_dtls a
												...
												where  b.id=a.mst_id and  a.production_type=86 and b.production_type=86 and b.status_active=1 and b.is_deleted=0 $bundle_no_cond $cutCon ", 'bundle_no', 'bundle_no');
	...
	<div style=" width:1020px; max-height:210px; overflow-y:scroll" id="list_container_batch" align="left">
        ...
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1000" class="rpt_table" id="tbl_list_search">  
			...
			$sql="select b.job_no_mst ,a.cut_no,b.color_number_id, b.item_number_id ,b.size_number_id , a.bundle_no, b.po_break_down_id  , b.country_id ,   c.size_qty ,a.barcode_no,c.mst_id,$group_field
                 ...
                 where  
             		c.barcode_no=a.barcode_no and  a.production_type=52 and a.color_size_break_down_id=b.id  $selected_bundle_cond  $jobCon $cutCon $bundle_no_cond and b.status_active=1 and  b.is_deleted=0 and  a.status_active=1 and  a.is_deleted=0 
			...
			<legend></legend>           
	            <table cellpadding="0" cellspacing="0" width="550" border="1" rules="all" class="rpt_table">
	                ...
	                <tr class="general">
	            		...
	            		<td align="center">
	                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view (document.getElementById('cbo_company_name').value+'_'+document.getElementById('bundle_no').value+'_'+'<? echo trim($bundleNo,','); ?>'+'_'+document.getElementById('txt_job_no').value+'_'+document.getElementById('txt_lot_no').value+'_'+document.getElementById('cbo_lot_year').value+'_'+'<? echo trim($lot_ratio,','); ?>','create_bundle_rescan_search_list_view','search_div','bundle_issue_distribution_point_controller','setFilterGrid(\'tbl_list_search\',-1);reset_hide_field();')" style="width:100px;" />
	                     </td>
	                </tr>
	           </table>
	           <div style="width:100%; margin-top:5px; margin-left:10px" id="search_div" align="left"></div>
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
       ...
       $receive_sql="SELECT c.barcode_no, c.bundle_no from pro_garments_production_mst a, pro_garments_production_dtls c where a.id=c.mst_id and a.production_type=86 and c.barcode_no in ($barcode) and c.production_type=86 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 ";
		...
		$data_arra_cutt_mst="(".$qc_id.", 100, '".$new_system_id[1]."', ".(int)$new_system_id[2].", '".$new_system_id[0]."',".$txt_lot_ratio.",".$txt_job_no.",".$cbo_location.",".$cbo_floor.",".$cbo_company_name.",".$txt_issue_date.",".$cbo_source.",".$cbo_working_company.",".$cbo_working_location.",86,".$txt_remarks.",".$user_id.",'".$pc_date_time."',1,0)";
		...
		foreach($mstArr as $orderId=>$orderData)
		{
			foreach($orderData as $gmtsItemId=>$gmtsItemIdData)
			{
				foreach($gmtsItemIdData as $countryId=>$qty)
				{
					...
					$data_array_mst.="(".$id.",".$qc_id.",".$txt_lot_ratio.",".$cbo_company_name.",100,".$txt_rec_challan_no.",".$orderId.",".$gmtsItemId.",".$countryId.",".$cbo_source.",".$cbo_working_company.",".$cbo_location.",".$txt_issue_date.",".$qty.",86,3,".$txt_remarks.",".$cbo_floor.",".$user_id.",'".$pc_date_time."',1,0)";
		...
		foreach($dtlsArr as $bundle_no=>$bundle_data)
		{
			...
			$data_array_dtls.= "(".$dtls_id.",".$qc_id.",".$gmtsMstId.",86,'".$dtlsArrColorSize[$bundle_no]."','".$bundle_data['qc_pass']."',".$txt_lot_ratio.",'".$bundle_no."','".$bundleBarcodeArr[$bundle_no]."','".$bundleRescanArr[$bundle_no]."',1,0)"; 
		}
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
        ...
        $barcode="'".implode("','",$barcodeCheckArr)."'";
        $receive_sql="SELECT c.barcode_no,c.bundle_no from pro_garments_production_mst a,pro_garments_production_dtls c where a.id=c.mst_id and a.production_type=86 and c.barcode_no  in ($barcode)  and c.production_type=86 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and a.delivery_mst_id!=$mst_id and c.delivery_mst_id!=$mst_id and (c.is_rescan=0 or c.is_rescan is null)"; 
        $receive_result = sql_select($receive_sql);
        foreach ($receive_result as $row)
        {           
            $duplicate_bundle[$row[csf('barcode_no')]]=$row[csf('barcode_no')];
		...
		$data_arra_cutt_mst=$txt_issue_date."*".$txt_remarks."*".$user_id."*'".$pc_date_time."'";
		$mstArr=array(); $dtlsArr=array(); $colorSizeArr=array(); $mstIdArr=array();
		...
		foreach($mstArr as $orderId=>$orderData)
		{
			foreach($orderData as $gmtsItemId=>$gmtsItemIdData)
			{
				foreach($gmtsItemIdData as $countryId=>$qc_qty)
				{
					$id= return_next_id_by_sequence(  "pro_gar_production_mst_seq", "pro_garments_production_mst", $con );
					if($data_array_mst!="") $data_array_mst.=",";
					$data_array_mst.="(".$id.",".$mst_id.",".$txt_lot_ratio.",".$cbo_company_name.",".$garments_nature.",'".$challan_no."',".$orderId.",".$gmtsItemId.",".$countryId.",".$cbo_source.",".$cbo_working_company.",".$cbo_location.",".$txt_issue_date.",".$qc_qty.",86,3,".$txt_remarks.",".$cbo_floor.",".$user_id.",'".$pc_date_time."',1,0)";
					$mstIdArr[$orderId][$gmtsItemId][$countryId]=$id;
				}
			}
		}
		...
		foreach($dtlsArr as $bundle_no=>$bundle_data)
		{
			...
			$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",  "pro_garments_production_dtls", $con );
			$data_array_dtls.= "(".$dtls_id.",".$mst_id.",".$gmtsMstId.",86,'".$dtlsArrColorSize[$bundle_no]."','".$bundle_data['qc_pass']."',".$txt_lot_ratio.",'".$bundle_no."','".$bundleBarcodeArr[$bundle_no]."','".$bundleRescanArr[$bundle_no]."',1,0)"; 
		}
		$flag=$delete =$delete_dtls =$delete_qc =$rID_mst_qc=$rID_dtls=$rID=$dtlsrID=1;
		
		$delete = execute_query("update pro_garments_production_mst set status_active=0,is_deleted=1,updated_by=$user_id,update_date='".$pc_date_time."' WHERE delivery_mst_id=$mst_id and production_type=86 and status_active=1 and is_deleted=0");
		//echo "10**update pro_garments_production_mst set status_active=0,is_deleted=1,updated_by=$user_id,update_date='".$pc_date_time."' WHERE delivery_mst_id=$mst_id and production_type=86 and status_active=1 and is_deleted=0"; die;
		if($delete==1) $flag=1; else $flag=0;
		$delete_dtls = execute_query("update pro_garments_production_dtls set status_active=0,is_deleted=1 WHERE delivery_mst_id=$mst_id and production_type=86 and status_active=1 and is_deleted=0");
...
if($action=='populate_data_from_qc')
{
	...
	{ 
		...
		echo "document.getElementById('cbo_location').value 				= '".$row[csf("location_id")]."';\n";
		echo "document.getElementById('txt_issue_date').value 				= '".change_date_format($row[csf("cutting_qc_date")])."';\n"; 
		exit();
	}
}

if($action=="firstins_number_popup")
{
...
?>
	<script>
		...
		{
		document.getElementById('hidd_firstinspmst_id').value=strCon;
		parent.emailwindow.hide();
		}
    </script>
</head>
            ...
            <tbody>
                <tr class="general">                    
                    ...
                    <td>
                        ...
                        <input type="hidden" id="hidd_firstinspmst_id" name="hidd_firstinspmst_id" />
                    </td>
...
if($action=="create_first_insp_list_view")
{
    $ex_data = explode("_",$data);
	$company = $ex_data[0];	
...
if($action=='populate_data_from_firstIns')
{
...
<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            ...
            <tbody>
                  <tr class="general">                    
                        ...
                        <td><input name="txt_date_to" id="txt_date_to" class="datepicker" style="width:60px" placeholder="To Date" /></td>
                        <td>
                               <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('txt_cut_no').value+'_'+document.getElementById('txt_job_search').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_cut_qc').value+'_'+document.getElementById('txt_order_search').value+'_'+document.getElementById('txt_qr_search').value, 'create_system_search_list_view', 'search_div', 'bundle_issue_distribution_point_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" />				
                        </td>
                 </tr>
        		 <tr>                  
                    <td align="center" valign="middle" colspan="9"><? echo load_month_buttons(1); ?></td>
...
if($action=="create_system_search_list_view")
{
	...
	$sql_order="SELECT a.id, a.cutting_no, a.cut_qc_prefix_no, a.cutting_qc_no, a.table_no, a.job_no, a.cutting_qc_date, c.job_no_prefix_num, b.cut_num_prefix_no, $year, d.po_number
    FROM pro_gmts_cutting_qc_mst a, ppl_cut_lay_mst b, ppl_cut_lay_dtls e, wo_po_details_master c, wo_po_break_down d, pro_garments_production_dtls f
    where a.garments_nature=100 and a.production_type=86 and a.cutting_no=b.cutting_no and b.job_no=c.job_no and c.job_no=d.job_no_mst and a.id=f.delivery_mst_id $conpany_cond $cut_cond $job_cond $sql_cond $order_cond $system_cond $barcode_no and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and b.id=e.mst_id group by a.id, a.cutting_no, a.cut_qc_prefix_no, a.cutting_qc_no, a.table_no, a.job_no, a.cutting_qc_date, c.job_no_prefix_num, b.cut_num_prefix_no, a.insert_date, d.po_number order by a.id DESC";
//echo $sql_order;die;
