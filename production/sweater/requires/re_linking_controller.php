...
{
	echo create_drop_down( "cbo_location", 180, "select id,location_name from lib_location where status_active=1 and is_deleted=0 and company_id='$data' $location_credential_cond order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down( 'requires/re_linking_controller', this.value, 'load_drop_down_floor', 'floor_td' );load_drop_down( 'requires/re_linking_controller', this.value+'_'+$('#cbo_sewing_company').val()+'_'+document.getElementById('prod_reso_allo').value+'_'+document.getElementById('txt_sewing_date').value, 'load_drop_down_sewing_line', 'sewing_line_td' );",0 );
}

if ($action=="load_drop_down_floor")
{
	echo create_drop_down( "cbo_floor", 140, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process in (5) order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "get_php_form_data(document.getElementById('cbo_source').value,'line_disable_enable','requires/re_linking_controller'); load_drop_down( 'requires/re_linking_controller', this.value+'_'+document.getElementById('cbo_location').value+'_'+document.getElementById('prod_reso_allo').value+'_'+document.getElementById('txt_sewing_date').value+'_'+document.getElementById('cbo_sewing_company').value, 'load_drop_down_sewing_line_floor', 'sewing_line_td' );",0 );
}
...
if($action=="load_drop_down_sewing_input")
{
	...
	if($data==3)
	{
		...
		else
		{
  		...
  		echo create_drop_down( "cbo_sewing_company", 140, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1 $company_cond $company_credential_cond order by company_name","id,company_name", 1, "--- Select ---", "", "load_drop_down( 'requires/re_linking_controller', this.value, 'load_drop_down_location', 'location_td' );fnc_company_check(document.getElementById('cbo_source').value);",0,0 );
 	else
...
if ($action=="order_popup")
{
	...
	<script>
		...
		function search_populate(str)
		{
			...
			{
				load_drop_down( 're_linking_controller',document.getElementById('company_search_by').value,'load_drop_down_buyer', 'search_by_td' );
                ...
                <tr>
                    <td align="center" width="100%">
                        <table ellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                             <thead>
							    <tr>
									<th colspan="9"><? echo create_drop_down( "cbo_string_search_type", 130, $string_search_type,'', 1, "-- Select --",4 ); ?></th>
								</tr>
...
if($action=="create_po_search_list_view")
{
	...
	$sql = "SELECT a.id as job_no, b.id,b.shipment_date, b.po_number, a.buyer_name, a.style_ref_no,b.file_no, a.gmts_item_id,b.po_quantity,b.grouping,d.production_quantity,e.po_number_acc
			from wo_po_details_master a, wo_po_break_down b, wo_pre_cost_embe_cost_dtls c,pro_garments_production_mst d,wo_po_break_down_vw e
			where
			a.id = b.job_id and a.id = c.job_id and b.id = d.po_break_down_id and a.job_no = e.job_no_mst and d.production_type=4   and a.status_active=1 and b.status_active=1 and c.status_active=1  and d.status_active=1 and a.is_deleted=0  and b.is_deleted=0 and c.is_deleted=0  and d.is_deleted=0 $sql_cond $sql_shipment_year_cond order by b.shipment_date desc"; 
			// 
	// echo $sql;
	// die;	
	...
	foreach($result as $v)
	{
		$list_data[$v[csf('job_no')]] [$v[csf('po_number')]] [$v[csf('style_ref_no')]] ['id'] = $v[csf('id')] ;
		$list_data[$v[csf('job_no')]] [$v[csf('po_number')]] [$v[csf('style_ref_no')]] ['shipment_date'] = $v[csf('shipment_date')] ;
		$list_data[$v[csf('job_no')]] [$v[csf('po_number')]] [$v[csf('style_ref_no')]] ['buyer_name'] = $v[csf('buyer_name')] ;
		$list_data[$v[csf('job_no')]] [$v[csf('po_number')]] [$v[csf('style_ref_no')]] ['file_no'] = $v[csf('file_no')] ;
		$list_data[$v[csf('job_no')]] [$v[csf('po_number')]] [$v[csf('style_ref_no')]] ['gmts_item_id'] = $v[csf('gmts_item_id')] ;
		$list_data[$v[csf('job_no')]] [$v[csf('po_number')]] [$v[csf('style_ref_no')]] ['po_quantity'] = $v[csf('po_quantity')] ;
		$list_data[$v[csf('job_no')]] [$v[csf('po_number')]] [$v[csf('style_ref_no')]] ['grouping'] = $v[csf('grouping')] ;
		$list_data[$v[csf('job_no')]] [$v[csf('po_number')]] [$v[csf('style_ref_no')]] ['production_qty'] = $v[csf('production_quantity')] ;
		$list_data[$v[csf('job_no')]] [$v[csf('po_number')]] [$v[csf('style_ref_no')]] ['acc_po'] = $v[csf('po_number_acc')] ;
	}
	// echo '<pre>';
	// print_r($list_data);
	// die;
	...
	<style>
     ...
     <div style="width:1170px;">
     	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1170" width="100%" class="rpt_table">
            <thead>
                <th width="30">SL</th>
                ...
                <th width="60">Re-linking Qty</th> 
                <th>Balance</th>
            </thead>
     	</table>
     </div>
     <div style="width:1170px; max-height:240px;overflow-y:scroll;" >
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1150" class="rpt_table" id="tbl_po_list">
			<?
			$i=1;
            foreach( $list_data as $job_no =>$data  )
            {  
				foreach($data as $po_number => $v)
				{
					foreach ($v as $style_ref => $row) 
					{
						...
						for($k=0;$k<$numOfItem;$k++)
						{
							...
							}else
							{
								...
								{ 
									...
									?>
										<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value(<?= $row['id']?>,<?= $row['gmts_item_id'] ?>,<?= $row['po_quantity']; ?>)" >
											<td width="30" align="center"><?= $i; ?></td>
											<td width="65" align="center"><?= change_date_format($row[ "shipment_date"]);?></td>
											<td width="100" class="w-b-all"><?= $po_number; ?></td>
											<td width="80" class="w-b-all"><?= $row['acc_po'] ?></td> 
											<td width="100" class="w-b-all"><?= $buyer_arr[$row[ "buyer_name"]]; ?></td>
											<td width="110" class="w-b-all"><?= $style_ref ?></td>
											<td width="90" class="w-b-all"><?= $job_no; ?></td>
											<td width="70" class="w-b-all"><?= $row["file_no"]; ?></td>
											<td width="70" class="w-b-all"><?= $row["grouping"]; ?></td>
											<td width="100" class="w-b-all"><?=$garments_item [$row['gmts_item_id']];?></td>
											<td width="80" class="w-b-all"><?=  $country_library[$country_id]; ?></td>
											<td width="70" align="right"><?= $row['po_quantity']; ?></td> 
											<td width="60" align="right"><?=  $row['production_qty'] ?></td>
											<td align="right"><?=  $row['po_quantity'] -  $row['production_qty'] ?></td>
										</tr>
									<?
									$i++;
								}
...
if($action=="color_and_size_level")
{
		...
		if($qty_source!=0)
		{
			if($garments_nature==100) // for sweater
			{
				if( $variableSettings==2 ) // color level
				{
						if($db_type==0)
						{
							$sql = "SELECT id, item_number_id, size_number_id, color_number_id, order_quantity, sum(plan_cut_qnty) as plan_cut_qnty, (select sum(CASE WHEN pdtls.color_size_break_down_id=wo_po_color_size_breakdown.id then pdtls.production_qnty ELSE 0 END) from $dtls_table pdtls where pdtls.production_type='$qty_source' and pdtls.is_deleted=0 ) as production_qnty, (select sum(CASE WHEN cur.color_size_break_down_id=wo_po_color_size_breakdown.id then cur.production_qnty ELSE 0 END) from $dtls_table cur where cur.production_type=115 and cur.is_deleted=0 ) as cur_production_qnty
						...
						else
						{
							$sql = "SELECT a.item_number_id, a.color_number_id, sum(a.order_quantity) as order_quantity, sum(a.plan_cut_qnty) as plan_cut_qnty,
									...
									sum(CASE WHEN c.production_type=115 then b.production_qnty ELSE 0 END) as cur_production_qnty
				...
				{ 
					...
					$dtlsData = sql_select("SELECT a.color_size_break_down_id,
												...
												sum(CASE WHEN a.production_type=115 then a.production_qnty ELSE 0 END) as cur_production_qnty
												from $dtls_table a,$mst_table b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in($qty_source,115) group by a.color_size_break_down_id"); 
			...
			else // for knit and woven
			{
				...
				else if( $variableSettings==3 ) //color and size level
				{
					...
					if($qty_source=="9")
					{
						...
						$dtlsData = sql_select("SELECT a.color_size_break_down_id,
											sum(CASE WHEN a.production_type=115 then a.production_qnty ELSE 0 END) as cur_production_qnty
											from pro_garments_production_dtls a,pro_garments_production_mst b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in(115) group by a.color_size_break_down_id");
						foreach($dtlsData as $row)
						{
	 						$color_size_qnty_array[$row[csf('color_size_break_down_id')]]['rcv']= $row[csf('cur_production_qnty')];
						}
		...
		else // if preceding process =0 in variable setting then plan cut quantity will show
		{
			if( $variableSettings==2 ) // color level
			{
				...
				$sql = "SELECT id, item_number_id, size_number_id, color_number_id, order_quantity, sum(plan_cut_qnty) as plan_cut_qnty, (select sum(CASE WHEN $dtls_table.color_size_break_down_id=wo_po_color_size_breakdown.id then production_qnty ELSE 0 END) from $dtls_table where is_deleted=0 and production_type=115 ) as production_qnty
					from wo_po_color_size_breakdown
					where po_break_down_id='$po_id' and item_number_id='$item_id' and country_id='$country_id' and is_deleted=0 and status_active=1 group by color_number_id";
				}
				else
				{
					...
					from wo_po_color_size_breakdown a left join $dtls_table b on a.id=b.color_size_break_down_id and b.production_type=115
					where a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' and a.is_deleted=0 and a.status_active=1 group by a.item_number_id, a.color_number_id";
			...
			else if( $variableSettings==3 ) //color and size level
			{
				...
				$dtlsData = sql_select("select a.color_size_break_down_id,
											sum(CASE WHEN a.production_type=115 then a.production_qnty ELSE 0 END) as production_qnty
			...
			else // by default color and size level
			{
				...
				$dtlsData = sql_select("select a.color_size_break_down_id,
											sum(CASE WHEN a.production_type=155 then a.production_qnty ELSE 0 END) as production_qnty
											from $dtls_table a,$mst_table b where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id' and a.color_size_break_down_id!=0 and a.production_type in(115) group by a.color_size_break_down_id");
	...
	<div style="width:100%;max-height:180px; overflow:y-scroll" id="sewing_production_list_view" align="left">
		...
		<?php
			...
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" >
                ...
                <td width="120" align="center" onClick="fnc_load_from_dtls(<? echo $selectResult[csf('id')]; ?>);"><? echo $location_arr[$selectResult[csf('location')]];; ?></td>
                <td align="center" onClick="fnc_load_from_dtls(<? echo $selectResult[csf('id')]; ?>);"><?  echo $selectResult[csf('challan_no')]; ?></td>
			</tr>
			<?php
			$i++;
			}
...
if($action=="populate_input_form_data")
{
	...
	foreach($sqlResult as $result)
	{
		...
		echo "load_drop_down( 'requires/re_linking_controller', ".$result[csf('production_source')].", 'load_drop_down_sewing_input', 'sew_company_td' );\n";
		echo "$('#cbo_sewing_company').val('".$result[csf('serving_company')]."');\n";
		echo "load_drop_down( 'requires/re_linking_controller',".$result[csf('serving_company')].", 'load_drop_down_location', 'location_td' );";
		echo "$('#cbo_location').val('".$result[csf('location')]."');\n";
		echo "load_drop_down( 'requires/re_linking_controller', ".$result[csf('location')].", 'load_drop_down_floor', 'floor_td' );\n";
		echo "$('#cbo_floor').val('".$result[csf('floor_id')]."');\n";
		echo "load_drop_down( 'requires/re_linking_controller', document.getElementById('cbo_floor').value+'_'+$('#cbo_location').val()+'_'+document.getElementById('prod_reso_allo').value+'_'+document.getElementById('txt_sewing_date').value+'_'+document.getElementById('cbo_sewing_company').value, 'load_drop_down_sewing_line_floor', 'sewing_line_td' );\n";

		echo "$('#cbo_sewing_line').val('".$result[csf('sewing_line')]."');\n";
		echo "get_php_form_data(".$result[csf('production_source')].",'line_disable_enable','requires/re_linking_controller');\n";
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$txt_challan_no=(str_replace("'", "", $txt_challan)==0)? $id : $txt_challan;
   		$field_array1="id, garments_nature, company_id,man_cutt_no, po_break_down_id, item_number_id, country_id, production_source, serving_company, location, production_date,produced_by, production_quantity, production_type, entry_break_down_type, sewing_line, remarks, floor_id, total_produced, yet_to_produced, prod_reso_allo, inserted_by, insert_date";
		$data_array1="(".$id.",".$garments_nature.",".$cbo_company_name.",".$txt_man_cutting_no.",".$hidden_po_break_down_id.",".$cbo_item_name.",".$cbo_country_name.",".$cbo_source.",".$cbo_sewing_company.",".$cbo_location.",".$txt_sewing_date.",".$cbo_produced_by.",".$txt_input_qnty.",115,".$sewing_production_variable.",".$cbo_sewing_line.",".$txt_remark.",".$cbo_floor.",".$txt_cumul_input_qty.",".$txt_yet_to_input.",'".$prod_reso_allo."',".$user_id.",'".$pc_date_time."')";
		...
		else
		{
			$dtlsData = sql_select("select a.color_size_break_down_id,
										...
										sum(CASE WHEN a.production_type=115 then a.production_qnty ELSE 0 END) as cur_production_qnty
										from pro_garments_production_dtls a,pro_garments_production_mst b
										where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name and b.country_id=$cbo_country_name and a.color_size_break_down_id!=0 and a.production_type in(4,115)
										group by a.color_size_break_down_id");
		}
		...
		if(str_replace("'","",$sewing_production_variable)==2)//color level wise
		{
			...
			foreach($rowEx as $rowE=>$val)
			{
				...
				if($colSizeID_arr[$colorSizeNumberIDArr[0]]!="")
				{
					...
					if($j==0)$data_array = "(".$dtls_id.",".$id.",115,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."')";
					else $data_array .= ",(".$dtls_id.",".$id.",115,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."')";
					//$dtls_id=$dtls_id+1;
	 				$j++;
	 			}
	 			else
		...
		if(str_replace("'","",$sewing_production_variable)==3)//color and size wise
		{
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				if($colSizeID_arr[$index]!="")
				{
					//4 for Sewing Input Entry
					$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",  "pro_garments_production_dtls", $con );
					if($j==0)$data_array = "(".$dtls_id.",".$id.",115,'".$colSizeID_arr[$index]."','".$colorSizeValue."')";
					else $data_array .= ",(".$dtls_id.",".$id.",115,'".$colSizeID_arr[$index]."','".$colorSizeValue."')";
					//$dtls_id=$dtls_id+1;
	 				$j++;
 				}
	 			else
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$data_array1="".$cbo_source."*".$cbo_sewing_company."*".$cbo_location."*".$txt_man_cutting_no."*".$txt_sewing_date."*".$cbo_produced_by."*".$txt_input_qnty."*115*".$sewing_production_variable."*".$cbo_sewing_line."*".$txt_challan."*".$txt_remark."*".$cbo_floor."*".$txt_cumul_input_qty."*".$txt_yet_to_input."*'".$prod_reso_allo."'*".$user_id."*'".$pc_date_time."'";
		...
		if(str_replace("'","",$sewing_production_variable)!=1 && str_replace("'","",$txt_mst_id)!='' ) // check is not gross level
		{
			...
			else
			{
				$dtlsData = sql_select("select a.color_size_break_down_id,
										...
										sum(CASE WHEN a.production_type=115 then a.production_qnty ELSE 0 END) as cur_production_qnty
										from pro_garments_production_dtls a,pro_garments_production_mst b
										where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name and b.country_id=$cbo_country_name and a.color_size_break_down_id!=0 and a.production_type in(4,115) and b.id!=$txt_mst_id
										group by a.color_size_break_down_id");
			}
			...
			if(str_replace("'","",$sewing_production_variable)==2)//color level wise
			{
				...
				foreach($rowEx as $rowE=>$val)
				{
					...
					if($colSizeID_arr[$colorSizeNumberIDArr[0]]!="")
					{
						//4 for Sewing Input Entry
						$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",  "pro_garments_production_dtls", $con );
						if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",115,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."')";
						else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",115,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."')";
						//	$dtls_id=$dtls_id+1;
						$j++;
					}
					else
			...
			if(str_replace("'","",$sewing_production_variable)==3)//color and size wise
			{
				...
				foreach($rowEx as $rowE=>$valE)
				{
					...
					if($colSizeID_arr[$index]!="")
					{
						...
						//4 for Sewing Input Entry
						$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",  "pro_garments_production_dtls", $con );
						if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",115,'".$colSizeID_arr[$index]."','".$colorSizeValue."')";
						else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",115,'".$colSizeID_arr[$index]."','".$colorSizeValue."')";
						//$dtls_id=$dtls_id+1;
						$j++;
					}
					else
		...
		$dtlsrUpdate = execute_query("update pro_garments_production_dtls set status_active=0,is_deleted=1 where mst_id=$txt_mst_id",0);
		// $dtlsrUpdate = execute_query("delete from pro_garments_production_dtls where mst_id=$txt_mst_id",0);
