...
if ($action=="load_drop_down_location")
{
	...
	else if($data[1]==2) { $dropdown_name="cbo_wash_location"; $load_function="load_drop_down( 'requires/issue_to_finishing_controller', this.value, 'load_drop_down_floor', 'floor_td' );"; }
...
if ($action == "create_delivery_search_list") {
    ...
    where c.garments_nature=100 and a.entry_form=626 and a.id=b.delivery_mst_id and b.po_break_down_id=d.id and c.job_no=d.job_no_mst $conpany_cond $job_cond $sql_cond $order_cond $system_cond $buyer_cond $style_ref_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
	...
	<div style="width:1200px;max-height:180px; overflow:y-scroll" id="sewing_production_list_view" align="left">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" id="details_table">
			...
			foreach ($sqlResult as $selectResult) {
				...
				//$order_num_arr
			?>
				<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_exfactory_form_data','requires/issue_to_finishing_controller');get_php_form_data('<? echo $selectResult[csf('po_break_down_id')]; ?>+**+<? echo $selectResult[csf('item_number_id')]; ?>+**+<? echo $selectResult[csf('country_id')]; ?>'+'**'+$('#hidden_preceding_process').val()+'**'+$('#txt_mst_id').val()+'**1'+'**'+$('#sewing_production_variable').val()+'**'+$('#variable_is_controll').val()+'**'+$('#txt_country_ship_date').val()+'**'+$('#txt_pack_type').val(),'populate_data_from_search_popup','requires/issue_to_finishing_controller');">
...
if($action=="order_popup")
{
	...
	// onkeydown="if (event.keyCode == 13) document.getElementById('btn_show').click()"
			...
			{
				load_drop_down( 'issue_to_finishing_controller',document.getElementById('company_search_by').value,'load_drop_down_buyer', 'search_by_td' );
...
if($action=="create_po_search_list_view")
{
	...
	$total_cut_qty_arr=sql_select("SELECT po_break_down_id, item_number_id, country_id, sum(production_quantity) as production_quantity from pro_garments_production_mst where status_active=1 and is_deleted=0 and production_type=119 $po_id_cond group by po_break_down_id, item_number_id, country_id");
 	...
 	foreach($res as $result)
	{
		...
		echo "$('#cbo_item_name').val('".$result[csf('gmts_item_id')]."');\n";
 		echo "$('#txt_trimming_qty').attr('placeholder','');\n";//initialize quatity input field
			...
			$dataArray=sql_select("SELECT SUM(CASE WHEN a.production_type=$qty_source and b.production_type=$qty_source THEN b.production_qnty END) as totalinput,SUM(CASE WHEN a.production_type=119 and b.production_type=119  THEN b.production_qnty ELSE 0 END) as totalreceive from pro_garments_production_mst a,pro_garments_production_dtls b WHERE a.id=b.mst_id and  a.po_break_down_id=".$result[csf('id')]." and a.item_number_id='$item_id' and a.country_id='$country_id' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
			// echo $dataArray;die;
...
if($action=="color_and_size_level")
{
		...
		//echo "logic123_".$variableSettings;
			...
			if( $variableSettings==2 ) // color level
			{
					...
					from wo_po_color_size_breakdown a left join pro_garments_production_dtls b on a.id=b.color_size_break_down_id and b.production_type=119
			...
			else if( $variableSettings==3 ) //color and size level
			{
				$dtlsData = sql_select("SELECT a.color_size_break_down_id,
											...
											sum(CASE WHEN a.production_type=119 then a.production_qnty ELSE 0 END) as cur_production_qnty
...
if($action=="populate_mst_form_data")
{
	...
	echo"load_drop_down( 'requires/issue_to_finishing_controller', '".$location_parameter."', 'load_drop_down_working_com', 'wash_company_td' );\n";
	echo"load_drop_down( 'requires/issue_to_finishing_controller', ".$result[0][csf('working_company_id')].", 'load_drop_down_location2', 'working_location_td' );\n";

	echo"load_drop_down( 'requires/issue_to_finishing_controller', ".$result[0][csf('working_location_id')].", 'load_drop_down_floor', 'cbo_floor' );\n";
...
if($action=="populate_input_form_data")
{
		...
		SUM(CASE WHEN a.production_type=119 THEN a.production_quantity ELSE 0 END) as totalreceive
		from pro_garments_production_mst a,pro_gmts_delivery_mst b where b.id='$data[0]' and a.id='$data[1]'  and a.delivery_mst_id=b.id and a.production_type in(111,119) and a.status_active=1 and a.is_deleted=0 group by  b.id, a.company_id, a.garments_nature, a.po_break_down_id, a.item_number_id, a.challan_no, a.country_id,  a.pack_type, a.production_source, a.serving_company, a.sewing_line, a.location, a.produced_by, a.embel_name,  a.production_date, a.production_quantity, a.production_source, a.production_type, a.entry_break_down_type,  a.remarks, a.floor_id,  a.total_produced, a.yet_to_produced,b.location_id  order by b.id");
		//  echo $sqlResult	;die;
	...
	{ 
		...
		echo "load_drop_down( 'requires/issue_to_finishing_controller', ".$result[csf('production_source')].", 'load_drop_down_source', '' );\n";
		echo "load_drop_down( 'requires/issue_to_finishing_controller',".$result[csf('serving_company')].", 'load_drop_down_location2', 'working_location_td' );";
		...
		echo "load_drop_down( 'requires/issue_to_finishing_controller', ".$result[csf('location')].", 'load_drop_down_floor', 'floor_td' );\n";
		...
		SUM(CASE WHEN production_type=119 THEN production_quantity ELSE 0 END) as totalreceive		 
...
if($action=="show_listview")
{
	...
	<div style="width:100%;max-height:180px; overflow:y-scroll" id="wash_list_view" align="left">
		...
		<?php  
 		...
 		?>
			<tbody>
				<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data('<? echo $selectResult[csf('id')].'_'.$selectResult[csf('mst_id')]; ?>','populate_input_form_data','requires/issue_to_finishing_controller');" > 
   ...
   //echo "10**".$qty_source;
	...
	if ($operation == 0) // Insert Here----------------------------------------------------------
	{
		...
		if (str_replace("'", "", $txt_system_id) == "") 
		{
			...
			$new_sys_number = explode("*", return_next_id_by_sequence("", "pro_gmts_delivery_mst",$con,1,$cbo_company_name,'ITF',626,date("Y",time()),0,0,119,0,0 ));


			$field_array_delivery = "id, sys_number_prefix, sys_number_prefix_num, sys_number, company_id, production_type,location_id, production_source, working_company_id, working_location_id, floor_id, job_no, delivery_date,challan_no, remarks, entry_form, inserted_by, insert_date, status_active, is_deleted";
			$data_array_delivery="(".$mst_id.",'".$new_sys_number[1]."','".(int)$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",119,".$cbo_location.",".$cbo_source.",".$cbo_wash_company.",".$cbo_wash_location.",".$cbo_floor.",".$txt_job_no.",".$txt_issue_date.",".$txt_challan.",".$txt_remark.",626,".$user_id.",'".$pc_date_time."',1,0)";
		} else {
		...
		$field_array1="id, garments_nature, company_id, challan_no, po_break_down_id, item_number_id, country_id, production_source, serving_company, location, production_date, production_quantity, production_type,entry_break_down_type, remarks, floor_id,total_produced,yet_to_produced,delivery_mst_id, inserted_by, insert_date"; 

		$data_array1="(".$id.",".$garments_nature.",".$cbo_company_name.",".$txt_challan.",".$hidden_po_break_down_id.",".$cbo_item_name.",".$cbo_country_name.",".$cbo_source.",".$cbo_wash_company.",".$cbo_wash_location.",".$txt_issue_date.",".$txt_receive_qty.",119,".$sewing_production_variable.",".$txt_remark.",".$cbo_floor.",".$txt_cumul_quantity.",".$txt_yet_quantity.",".$mst_id.",".$user_id.",'".$pc_date_time."')";
		...
		$dtlsData = sql_select("SELECT a.color_size_break_down_id,
										...
										where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name and b.country_id=$cbo_country_name and a.color_size_break_down_id<>0 and a.production_type in($qty_source,119) $pack_type_cond
		...
		{		
			...
			foreach($rowEx as $rowE=>$val)
			{
				...
				if($j==0)$data_array = "(".$dtls_id.",".$id.",119,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$mst_id."')";
				else $data_array .= ",(".$dtls_id.",".$id.",119,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$mst_id."')";
				//$dtls_id=$dtls_id+1;							
 				$j++;								
			}
 		}//color level wise
		...
		{		
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				//8 for Garments Finishing Entry
				$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",   "pro_garments_production_dtls", $con );
				if($j==0)$data_array = "(".$dtls_id.",".$id.",119,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."','".$mst_id."')";
				else $data_array .= ",(".$dtls_id.",".$id.",119,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."','".$mst_id."')";
				//$dtls_id=$dtls_id+1;
 				$j++;
			}
		}//color and size wise
	...
	else if ($operation==1) // Update Here 
	{
		...
		if(str_replace("'","",$sewing_production_variable)!=1 && str_replace("'","",$txt_mst_id)!='' ) //  not gross level
		{
			...
			$dtlsData = sql_select("SELECT a.color_size_break_down_id,
										...
										sum(CASE WHEN a.production_type=119 then a.production_qnty ELSE 0 END) as cur_production_qnty 
										from pro_garments_production_dtls a,pro_garments_production_mst b 
										where a.status_active=1 and a.mst_id=b.id and b.po_break_down_id=$hidden_po_break_down_id and b.item_number_id=$cbo_item_name and b.country_id=$cbo_country_name and a.color_size_break_down_id<>0 and a.production_type in($qty_source,119) and b.id !=$txt_mst_id 
										group by a.color_size_break_down_id");
			$color_pord_data=array();							
			foreach($dtlsData as $row)
			{				  
			...
			{		
				...
				foreach($rowEx as $rowE=>$val)
				{
					...
					$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",  "pro_garments_production_dtls", $con );
					if($j==0)$data_array = "(".$dtls_id.",".$txt_mst_id.",119,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."','".$txt_system_id."')";
					else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",119,'".$colSizeID_arr[$colorSizeNumberIDArr[0]]."','".$colorSizeNumberIDArr[1]."','".$rejQtyArr[$colorSizeNumberIDArr[0]]."','".$txt_system_id."')";
					$j++;								
				}
			}
			...
			{		
				...
				foreach($rowEx as $rowE=>$valE)
				{
					...
					//8 for Garments Finishing Entry
					$dtls_id = return_next_id_by_sequence(  "pro_gar_production_dtls_seq",   "pro_garments_production_dtls", $con );
					if($j==0)$data_array_up = "(".$dtls_id.",".$txt_mst_id.",119,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."','".$txt_system_id."')";
					else $data_array .= ",(".$dtls_id.",".$txt_mst_id.",119,'".$colSizeID_arr[$index]."','".$colorSizeValue."','".$rejQtyArr[$index]."','".$txt_system_id."')";
					//$dtls_id=$dtls_id+1;
					$j++;
				}
			}
		...
		if(str_replace("'","",$sewing_production_variable)!=1)
		{
			...
			{
				oci_rollback($con);
				echo "10**".str_replace("'","",$hidden_po_break_down_id)."**".$rID."**".$dtlsrDelete."**".$dtlsrID."**".$challanrID;
			}
		}
		else
		{
