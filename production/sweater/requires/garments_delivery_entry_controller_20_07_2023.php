...
if ($action == "load_variable_settings") 
{
	...
	// echo $control_and_preceding;die;
...
if ($action == "color_and_size_level") 
{
	...
	else // if preceding process =0 in variable setting then plan cut quantity will show
	{
		if ($variableSettings == 2) // color level
		{
			...
			from wo_po_color_size_breakdown a
			where a.po_break_down_id='$po_id' and a.is_deleted=0 and a.status_active in(1,2,3) group by a.color_number_id order by a.color_number_id"; //color_number_id, id		
		...
		else if ($variableSettings == 3) //color and size level
		{
			...
			from wo_po_color_size_breakdown a
			where a.po_break_down_id='$po_id' and a.item_number_id='$item_id' and a.country_id='$country_id' $ship_date_cond $pack_type_cond and a.is_deleted=0 and a.status_active in(1,2,3) order by a.color_number_id,a.size_order"; //color_number_id, id
...
if ($action == "populate_exfactory_form_data")
{
	...
	*/
	$con = connect();
	$sqlRes = sql_select("SELECT po_break_down_id,item_number_id,country_id	from pro_ex_factory_mst where id='$data' and status_active=1 and entry_form<>85 and is_deleted=0");
