...
if($action=="create_bundle_search_list_view")
{
	...
	if ($lot_no != '') 
	{
		$cutCon_a = " and b.cut_no = '".trim($company_short_arr[$company].'-'.$syear.'-'.str_pad($lot_no,6,"0",STR_PAD_LEFT))."'";
		$cutCon = " and a.cutting_no = '".trim($company_short_arr[$company].'-'.$syear.'-'.str_pad($lot_no,6,"0",STR_PAD_LEFT))."'";
	...
	$scanned_bundle_arr=return_library_array("SELECT 
												...
												where 
													a.id=b.mst_id and 
													b.production_type  in (51) 
													$cutCon_a  and b.bundle_no like '$bundle_no' and 
...
if($action=='populate_data_from_yarn_lot_bundle')
{
	...
	$sql = sql_select("SELECT b.id as issue_id,a.operator_id,a.body_part_ids,b.SYS_NUMBER from PRO_GARMENTS_PRODUCTION_DTLS a,PRO_GMTS_DELIVERY_MST b where a.delivery_mst_id=b.id and a.status_active=1 and b.status_active=1 and a.barcode_no='$data' and a.production_type=50");
	foreach ($sql as $v) 
	{
		$bdpart_name = "";
		$bdpart_id_arr = explode(",",$v[csf("body_part_ids")]);
		foreach ($bdpart_id_arr as $key => $val) 
		{
			$bdpart_name .= ($bdpart_name=="") ? $time_weight_panel[$val] : ",".$time_weight_panel[$val];
		}
		/* $opsql = sql_select("select (first_name||' '||middle_name|| '  ' || last_name) as emp_name from hrm_employee where status_active=1 and is_deleted=0 and id_card_no='".$v[csf("operator_id")]."'",1,$new_conn);
		$emp_name = $opsql[0]['EMP_NAME']; */
		...
		$op_id = $v[csf("operator_id")];
		
		echo "document.getElementById('txt_issue_no').value 			= '".$v[csf("sys_number")]."';\n";
		echo "document.getElementById('txt_issue_id').value 			= '".$v[csf("issue_id")]."';\n";
		echo "document.getElementById('txt_operator_id').value 			= '".$v[csf("operator_id")]."';\n";
		echo "document.getElementById('txt_operation_name').value 		= '".$emp_arr[$op_id]."';\n";
		echo "document.getElementById('txt_bodyPart_id').value 		= '".$v[csf("body_part_ids")]."';\n";
		echo "document.getElementById('txt_bodypart_name').value 		= '".$bdpart_name."';\n";
	}
	die;
}
