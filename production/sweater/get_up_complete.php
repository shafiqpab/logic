...
<?php
if($_SESSION['logic_erp']['data_arr'][726]){
	echo "var field_level_data= " . json_encode($_SESSION['logic_erp']['data_arr'][726]). ";\n";
}
?>
...
function openmypage(page_link,title)
{
	...
	//			load_drop_down( 'requires/trimming_complete_controller',company_id, 'load_drop_down_location', 'location_td' );
			...
			console.log('reset');
			get_php_form_data(po_id+'**'+item_id+'**'+country_id+'**'+$('#hidden_preceding_process').val(), "populate_data_from_search_popup", "requires/get_up_complete_controller" );
			...
			{
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+job_num+'**'+variableSettingsReject+'**'+$('#hidden_preceding_process').val(), "color_and_size_level", "requires/get_up_complete_controller" );
			}
			else
			{
 	...
 	if(http.readyState == 4)
	{
		...
		else if(reponse[0]==0)//insert
		{
			...
			reset_form('','','txt_trim_qty*txt_reject_qty*txt_alter_qnty*txt_spot_qnty*hidden_break_down_html*hidden_colorSizeID*txt_remark*txt_mst_id','','','txt_trim_date*txt_reporting_hour');
 			get_php_form_data(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id+'**'+$('#hidden_preceding_process').val(), "populate_data_from_search_popup", "requires/get_up_complete_controller" );
