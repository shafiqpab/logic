...
function create_row(bundle_nos,vscan,hidden_cutting_no,bodypart_ids,issue_id)
{
	if(vscan==1)
	{
		...
		show_list_view(hidden_cutting_no+'_'+bundle_nos+'_'+bodypart_ids+'_'+issue_id, 'show_dtls_listview_from_issue', 'bundle_list_view', 'requires/bundle_receive_from_knitting_floor_controller', '');
...
function fnc_total_receive_qty()
{
	...
	$("#tbl_yarn_details").find('tbody tr').each(function()
	{
		...
		var actual_percentage=color_percentage/total_percentage;
		var color_qty		=(total_bundle_qty*actual_percentage);
		$(this).find("td:eq(5)").text(color_qty.toFixed(4));
	});
...
{
	var total_receive=0; var total_return=0; var total_wastage=0;

	$("#tbl_yarn_details").find('tbody tr').each(function()
	{
		var receive_qty  	=$(this).find("td:eq(5)").text()*1;
...
function fnc_receive_from_knitting_entry(operation)
{
	...
	if(operation==0 || operation==1 || operation==2)
	{
		...
		$("#tbl_yarn_details").find('tbody tr').each(function()
		{
			...
			var wastage	 		=$(this).find("td:eq(7)").text()*1;				
		...
		var data="action=save_update_delete&operation="+operation+
							'&tot_row='+j+
							'&yarn_color_row='+k+
							get_submitted_data_string('cbo_floor*txt_receive_date*cbo_source*txt_operator_id*cbo_working_company*cbo_working_location*cbo_company_name*cbo_location*cbo_bodypart_type*txt_challan_no*txt_system_id*txt_lot_ratio*txt_job_no*txt_remarks*garments_nature*txt_issue_id*txt_bodyPart_id*txt_size_set_no',"../")+dataString+dataString1;
...
function openmypage_sysNo()
{
	...
	emailwindow.onclose=function()
	{
		...
		{ 
			...
			show_list_view($("#txt_lot_ratio").val()+'_'+$("#txt_system_id").val(), 'show_dtls_yarn_listview_update', 'yarn_list_view', 'requires/bundle_receive_from_knitting_floor_controller', '');
        		...
        		<legend>Master Part</legend>
                	<table width="920">
                    	...
                    	<tr>
                        	...
                        	<td width="80" class="must_entry_caption" id="td_caption">Re-Scan Bar.</td>
                        	<td  width="110"> 
                        		<input 
	                    ...
	                    <tr>
                    		...
                    		<td width="80" class="must_entry_caption">Issue ID Qty.</td>
	                        <td width="110"> 
	                        	<input 
	                            	name="txt_issue_qty" 
	                                id="txt_issue_qty" 
