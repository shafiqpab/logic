<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Bundle Issue To Distribution Point
Functionality	:	
JS Functions	:
Created by		:	Kausar
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Bundle Issue To Distribution Point","../../", 1, 1, $unicode,'','');
?>
...
$('#txt_first_insp_no').live('keydown', function(e) {
	if (e.keyCode === 13) 
	{
		e.preventDefault();
		var txt_first_insp_no=trim($('#txt_first_insp_no').val().toUpperCase());
		var flag=1;
		...
		if(flag==1)
		{
			fnc_duplicate_inspno(txt_first_insp_no);
		}
	}
});

function fnc_duplicate_inspno(first_insp_no)
{
	var challan_duplicate=return_ajax_request_value( first_insp_no+'_'+1,"challan_duplicate_check", "requires/bundle_issue_distribution_point_controller");
...
{
	var challan_duplicate=return_ajax_request_value( barcode_no+'_'+0,"challan_duplicate_check", "requires/bundle_issue_distribution_point_controller");
	var ex_challan_duplicate=challan_duplicate.split("_");
...
function create_row(bundle_nos,vscan)
{
	...
	if(tot_row==0)
	{
		get_php_form_data( $("#txt_lot_ratio").val()+'_'+bundle_nos, "populate_data_from_barcode", "requires/bundle_issue_distribution_point_controller");
		show_list_view( $("#txt_lot_ratio").val()+'_'+bundle_nos, 'show_dtls_listview_bundle', 'bundle_list_view', 'requires/bundle_issue_distribution_point_controller','');
	...
	else
	{
		...
		var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"**"+vscan+"**"+hidden_cutting_no+"**"+formbarcodeno, 'populate_bundle_data', '', 'requires/bundle_issue_distribution_point_controller');
...
function openmypage_bundle(page_link,title)
{
	...
	var page_link='requires/bundle_issue_distribution_point_controller.php?action=bundle_popup&company_id='+document.getElementById('cbo_company_name').value+'&lot_ratio='+document.getElementById('txt_lot_ratio').value+'&bundleNo='+bundleNo;
...
function openmypage_bundle_rescan(page_link,title)
{
	...
	var title='Bundle Search';
	var page_link='requires/bundle_issue_distribution_point_controller.php?action=bundle_popup_rescan&company_id='+document.getElementById('cbo_company_name').value+'&lot_ratio='+document.getElementById('txt_lot_ratio').value+'&bundleNo='+bundleNo;
...
function create_row_rescan(bundle_nos,vscan)
{
	...
	if(tot_row==0)
	{
		// alert(bundle_nos)
		get_php_form_data( $("#txt_lot_ratio").val()+'_'+bundle_nos,"populate_data_from_barcode", "requires/bundle_issue_distribution_point_controller");
		show_list_view( $("#txt_lot_ratio").val()+'_'+bundle_nos, 'show_dtls_listview_bundle_rescan','bundle_list_view','requires/bundle_issue_distribution_point_controller','');
		$("#hidden_row_number").val($("#tbl_details").find('tbody tr').length);	
	}
	else
	{
		...
		var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"**"+vscan+"**"+hidden_cutting_no, 'show_dtls_listview_bundle_rescan2', '', 'requires/bundle_issue_distribution_point_controller');
		$('#tbl_details tbody').append(response_data);
		$("#hidden_row_number").val($("#tbl_details").find('tbody tr').length);
...
function fnc_duplicate_bundle_rescan(bundle_no)
{
	...
	var challan_duplicate=return_ajax_request_value( bundle_no+"__"+$('#cbo_company_name').val(),"qty_rescan_check", "requires/bundle_issue_distribution_point_controller");	
	...
	else
	{
		...
		{
			var page_link='requires/bundle_issue_distribution_point_controller.php?action=bundle_popup_line_select&data='+ex_challan_duplicate[2]+"&company_id="+$('#cbo_company_name').val();
			...
			emailwindow.onclose=function()
			{
				...
				//alert(tot_row)	
				if( (tot_row*1) <2)
					get_php_form_data(bundle_no+"__"+ex_challan_duplicate[4]+"__"+$('#txt_issue_date').val(),'load_mst_data','requires/bundle_issue_distribution_point_controller');
...
function load_location()
{
	...
	if(cbo_source==1)
	{
		load_drop_down( 'requires/bundle_issue_distribution_point_controller',cbo_working_company, 'load_drop_down_location', 'working_location_td' );
	}
	else
	{
...
function fnc_minusRow(id)
{
     ...
     var is_barcode_receive=return_global_ajax_value(barcode_no,'check_if_barcode_receive','','requires/bundle_issue_distribution_point_controller');
...
function fnc_issue_print_embroidery_entry(operation)
{
	...
	if ( form_validation('cbo_floor*txt_issue_date*cbo_source*cbo_working_company*cbo_working_location*cbo_company_name*cbo_location',
	...
	if(operation==0 || operation==1 || operation==2)
	{
		...
		var data="action=save_update_delete&operation="+operation+'&tot_row='+j+get_submitted_data_string('txt_system_no*txt_system_id*txt_lot_ratio*txt_lot_ratio_id*txt_job_no*txt_first_insp_no*cbo_company_name*cbo_location*txt_issue_date*txt_rec_challan_no*cbo_source*cbo_working_company*cbo_working_location*cbo_floor*txt_remarks*txt_barcode_no*txt_barcode_rescan*hidden_row_number*garments_nature',"../../")+dataString;
		
		http.open("POST","requires/bundle_issue_distribution_point_controller.php",true);
...
function openmypage_firstInsSysNo()
{
	var title = 'First Inspection No Selection Form';	
	var page_link = 'requires/bundle_issue_distribution_point_controller.php?action=firstins_number_popup';
		
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1050px,height=390px,center=1,resize=0,scrolling=0','../')
	emailwindow.onclose=function()
	{
		var theform=this.contentDoc.forms[0];
		var mst_id=this.contentDoc.getElementById("hidd_firstinspmst_id").value;//po id
		
		if(mst_id!="")
		{ 
			freeze_window(5);
			//reset_form('bndlissuedistributionpoint_1','list_view_country*breakdown_td_id','','','txt_issue_date,<?// echo date("d-m-Y"); ?>','');

			get_php_form_data(mst_id, "populate_data_from_firstIns", "requires/bundle_issue_distribution_point_controller"); 
			if (mst_id!="")
			{ 
				var challan_duplicate=return_ajax_request_value( mst_id+'_'+1,"challan_duplicate_check", "requires/bundle_issue_distribution_point_controller");
			...
			//show_list_view($("#txt_first_insp_no").val()+'_'+$("#cbo_company_name").val(), 'show_dtls_listview_update', 'bundle_list_view', 'requires/bundle_issue_distribution_point_controller', '');
...
function openmypage_sysNo()
{
	...
	var page_link = 'requires/bundle_issue_distribution_point_controller.php?action=system_number_popup';
	...
	emailwindow.onclose=function()
	{
		...
		{ 
			...
			get_php_form_data(mst_id, "populate_data_from_qc", "requires/bundle_issue_distribution_point_controller"); 
			show_list_view($("#txt_lot_ratio").val()+'_'+$("#txt_system_id").val(), 'show_dtls_listview_update', 'bundle_list_view', 'requires/bundle_issue_distribution_point_controller', '');
    ...
    <div style=" float:left" align="left"> 
        ...
        <form name="bndlissuedistributionpoint_1" id="bndlissuedistributionpoint_1" method="" autocomplete="off" >
			<fieldset style="width:1050px;">
        		<legend>Bundle Issue To Distribution Point</legend>
                <table width="1130">
                    <tr>
                    ...
                    <tr>
                        <td width="80" class="must_entry_caption" id="td_caption">First Insp. No</td>
                        <td width="130">
                        	<input name="txt_first_insp_no" placeholder="Browse/Write/Scan" onDblClick="openmypage_firstInsSysNo();" id="txt_first_insp_no" class="text_boxes" style="width:120px" /></td>
                        ...
                        <td width="130"><input  type="text" name="txt_issue_date" id="txt_issue_date" value="<?=date("d-m-Y"); ?>" class="datepicker" style="width:120px;" /></td> 
                    ...
                    <tr>
                        ...
                        <td colspan="2">
                        	<input name="txt_barcode_rescan" placeholder="Browse/Write/Scan" onDblClick="openmypage_bundle_rescan('requires/bundle_issue_distribution_point_controller.php?action=bundle_popup_rescan&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value,'Search Bundle For Rescan');" id="txt_barcode_rescan" class="text_boxes" style="width:120px" />
                        </td>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
               	...
               	<table cellpadding="0" cellspacing="1" width="100%">
               		<tr>
                        <td align="center" colspan="9" valign="middle" class="button_container">
                            <?
								$date=date('d-m-Y');
                                echo load_submit_buttons( $permission, "fnc_issue_print_embroidery_entry", 0,1 , "reset_form('bndlissuedistributionpoint_1','bundle_list_view','', 'txt_issue_date,".$date."','pageReset();')",1); 
                            ?>
