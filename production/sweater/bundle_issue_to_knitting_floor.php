...
function create_row(bundle_nos,vscan,hidden_cutting_no,bodypart_type,body_part_id)
{
	...
	if(tot_row==0)
	{
		if(vscan=="Browse")
		{
			get_php_form_data(
								...
								"requires/bundle_issue_to_knitting_floor_controller"
							 );
			show_list_view(
		...
		var all_machine=trim(return_global_ajax_value( $("#cbo_floor").val(), 'load_machine', '', 'requires/bundle_issue_to_knitting_floor_controller'));
	...
	else
	{
		...
		var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"**"+bodypart_type+"**"+$("#txt_bodyPart_id").val()+"**"+hidden_cutting_no, 'populate_bundle_data', '', 'requires/bundle_issue_to_knitting_floor_controller');
...
function openmypage_bundle(page_link,title)
{
	...
	if ( form_validation('txt_style_no*txt_bodypart_name*txt_operator_id','Style No*Body Part*Operator ID')==false )
	{
		return;
	}
	...
	var page_link='	requires/bundle_issue_to_knitting_floor_controller.php?action=bundle_popup&lot_ratio='+
					...
					'&style_no='+document.getElementById('txt_style_no').value+
					'&bundleNo='+bundleNo;
	...
	emailwindow=dhtmlmodal.open('EmailBox', 
								...
								'&bundleNo='+bundleNo, 
								title, 
								'width=1090px,height=370px,center=1,resize=0,scrolling=0',
								'../')
...
{
	if(document.getElementById('txt_style_no').value=="")
	{
		alert("Please select style no");
		document.getElementById('txt_style_no').focus();
		return;
	}
	var title='Body Part Search';
	var page_link='	requires/bundle_issue_to_knitting_floor_controller.php?action=bpdypart_popup&txt_bodyPart_id='+
					...
					'&txt_job_no='+document.getElementById('txt_job_no').value;
...
function load_location()
{
	...
	if(cbo_source==1)
	{
		load_drop_down( 'requires/bundle_issue_to_knitting_floor_controller',cbo_working_company, 'load_drop_down_location', 'working_location_td' );
	}
	else
	{
...
{
	var challan_duplicate=return_ajax_request_value( barcode_no,"challan_duplicate_check", "requires/bundle_issue_to_knitting_floor_controller");
...
{
	if(document.getElementById('txt_bodypart_name').value=="")
	{
		alert("Please select Body Part");
		document.getElementById('txt_bodypart_name').focus();
		return;
	}
	var page_link='requires/bundle_issue_to_knitting_floor_controller.php?action=operator_popup';
	...
	{			
		...
		var bodypart_id = document.getElementById('txt_bodyPart_id').value;
		show_list_view(style+'__'+bodypart_id+'__'+employee_data[1],
							'show_rest_of_bundle',
							'unreceived_bundle_list_view',
							'requires/bundle_issue_to_knitting_floor_controller',
							''
						  );
...
$('#txt_operator_id').live('keydown', function(e) {
	...
	if ( form_validation('txt_bodypart_name*txt_style_no','Body Part*Style')==false )
	{
		$('#txt_operator_id').val('');
		return;
	}
	...
	{
		e.preventDefault();
		var emp_code=trim($('#txt_operator_id').val().toUpperCase());
		var style = document.getElementById('txt_style_no').value;
		var bodypart_id = document.getElementById('txt_bodyPart_id').value;
		show_list_view(style+'__'+bodypart_id+'__'+emp_code,
							'show_rest_of_bundle',
							'unreceived_bundle_list_view',
							'requires/bundle_issue_to_knitting_floor_controller',
							''
...
{
	var page_link='requires/bundle_issue_to_knitting_floor_controller.php?action=supervisor_popup';
...
function fnc_issue_print_embroidery_entry(operation)
{
	...
	if(operation==4)
	{
		...
		generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#txt_lot_ratio').val()+'*'+$('#txt_job_no').val()+'*'+report_title, 'knitting_issue_print', 'requires/bundle_issue_to_knitting_floor_controller');
		release_freezing();
		return;
	}
	
	if ( form_validation('cbo_floor*txt_issue_date*cbo_source*txt_operator_id*txt_operation_name*txt_sup_name*cbo_working_company*cbo_working_location*cbo_company_name*cbo_location*cbo_bodypart_type*txt_style_no',
		'Floor*Issue Date*Source*Operator ID*Operator Name*W. Company*WC. Location*LC Company*Location*Bodypart Type*Style No')==false )
	{
		release_freezing();
		return;
	}
...
function openmypage_style()
{
	...
	var page_link = 'requires/bundle_issue_to_knitting_floor_controller.php?&action=job_search_popup';
...
function openmypage_sysNo()
{
	...
	var page_link = 'requires/bundle_issue_to_knitting_floor_controller.php?action=challan_no_popup';
...
{
	window.open("requires/bundle_issue_to_knitting_floor_controller.php?data=" + data+'&action='+action, true );
}
        ...
        <form name="printembro_1" id="printembro_1" method="" autocomplete="off" >
			<fieldset style="width:1000px;">
        		<legend>Master Part</legend>
                	<table width="980">
                        <tr>
                        	<td class="must_entry_caption">Style No</td>
                        	<td  width=""> 
	                        	<input  
	                            	name="txt_style_no" 
	                                placeholder="Browse" 
                    	...
                    	<tr>
	                        ...
	                        <td width="110" class="must_entry_caption" id="td_caption">Knit Sup ID</td>
	                        <td  width=""> 
	                        	<input 
	                            	name="hidden_sup_id" 
	                                placeholder="Browse/Scan" 
                        ...
                        <tr>
	                        <td width="110" class="must_entry_caption" id="td_caption">Knit Sup Name</td>
	                        <td  width=""> 
	                        	<input 
	                            	name="txt_sup_name" 
	                                id="txt_sup_name" 
            ...
            <fieldset style="">
				<legend>Bundle List</legend>
                	...
                	<!-- ========================== bundle issue list view ======================== -->

               		<fieldset>
                    	<div id="unreceived_bundle_list_view">
