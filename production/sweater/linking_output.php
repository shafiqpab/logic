<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Linking Output
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Linking Output","../../", 1, 1, $unicode,'','');
?>
...
<script>
	...
	{
		var challan_duplicate=return_ajax_request_value( barcode_no,"challan_duplicate_check", "requires/linking_output_controller");
    ...
    {
        var challan_duplicate = return_ajax_request_value(bundle_no, "challan_duplicate_check", "requires/linking_output_controller");
        //var ex_challan_duplicate = challan_duplicate.split("_");
        ...
        else
        {
			...
			if( (tot_row*1) <2)
			{
				get_php_form_data(bundle_no+'_'+$('#cbo_company_name').val()+'_'+$('#txt_output_date').val(), "populate_mst_data", "requires/linking_output_controller");
	...
	function create_row(bundle_nos, vscan, hidden_source_cond)
    {
        ...
        var response_data = return_global_ajax_value(bundle_nos + "**" + row_num + "****" + $('#cbo_company_name').val() + "**" + vscan + "**" + hidden_source_cond, 'populate_bundle_data', '', 'requires/linking_output_controller');
	...
	function openmypage_bundle(page_link,title)
	{
		...
		var page_link='	requires/linking_output_controller.php?action=bundle_popup&company_id='+document.getElementById('cbo_company_name').value+'&bundleNo='+bundleNo;
	...
	function fnc_linking_output_entry(operation)
	{
		if(operation==4)
		{
		...
		if (form_validation('cbo_company_name*cbo_location*cbo_source*cbo_working_company*cbo_working_location*cbo_floor*txt_output_date*txt_reporting_hour','LC Company*LC Location*Source*W. Company*WC. Location*Floor*Output Date*Reporting Hour')==false )
		{
			return;
		}
		...
		if(operation==0 || operation==1 || operation==2)
		{
			...
			//alert(data); return;
			http.open("POST","requires/linking_output_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
			http.onreadystatechange = fnc_linking_output_entry_reply_info;
		}
	}
  
	function fnc_linking_output_entry_reply_info()
	{
		...
		{		
			...
			if(reponse[0]==15) 
			{ 
				 setTimeout('fnc_linking_output_entry('+ reponse[1]+')',4000); 
			}
			else if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
			{
				...
				set_button_status(1, permission, 'fnc_linking_output_entry',1,1);	
			}
			if(reponse[0]!=15)
			{
			  release_freezing();
	...
	function openmypage_sysNo()
	{
		...
		var page_link = 'requires/linking_output_controller.php?action=system_number_popup&data='+company_id;
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				fnc_load_party(1);
				load_drop_down( 'requires/linking_output_controller', ex_data[2]+'_'+1, 'load_drop_down_location', 'location_td' );
				...
				load_drop_down('requires/linking_output_controller', location, 'load_drop_down_floor', 'floor_td' );
				$('#cbo_floor').val(ex_data[7]);
				$('#txt_output_date').val(ex_data[8]);
				
				load_drop_down( 'requires/linking_output_controller', ex_data[5]+'_'+ex_data[6]+'_'+ex_data[7]+'_'+ex_data[8], 'load_drop_down_line', 'line_td');
				...
				var response_data = return_global_ajax_value(ex_data[0]+"**"+ ex_data[2], 'populate_bundle_data_update', '', 'requires/linking_output_controller');
				...
				var tot_row=$('#tbl_details tbody tr').length; 
				$('#hidden_row_number').val(tot_row);
				set_button_status(1, permission, 'fnc_linking_output_entry',1,0);
				release_freezing();
			}
		}
	}//end function
	...
	{
		window.open("requires/linking_output_controller.php?data=" + data+'&action='+action, true );
	}
	...
	function fnc_load_party(type)
	{
		...
		if(source==1 && type==1)
		{
			load_drop_down( 'requires/linking_output_controller', company+'_'+1, 'load_drop_down_working_com', 'working_com' );
		}
		else if(source==2 && type==1)
		{
			load_drop_down( 'requires/linking_output_controller', company+'_'+2, 'load_drop_down_working_com', 'working_com' );
		}
		else if(source==1 && type==2)
		{
			load_drop_down( 'requires/linking_output_controller', working_company+'_'+2, 'load_drop_down_location', 'working_location_td' ); 
		} 
	}
...
<body onLoad="set_hotkey()">
 <div style="width:100%;">
    ...
    <div style=" float:left" align="left"> 
        ...
        <form name="linkingoutput_1" id="linkingoutput_1" method="" autocomplete="off" >
			<fieldset style="width:1050px;">
        		<legend>Linking Output</legend>
                <table width="1050">
                    <tr>
                        <td align="right" colspan="4"><b>Output Challan No</b></td>
                        <td colspan="4"> 
                    ...
                    <tr>
                        ...
                        <td width="140"><? echo create_drop_down( "cbo_company_name",130, "select id, company_name from lib_company comp  where status_active =1 and  is_deleted=0  $company_cond  order by company_name","id,company_name", 1, "-- Select --", $selected,"load_drop_down( 'requires/linking_output_controller', this.value+'_'+1, 'load_drop_down_location', 'location_td' ); fnc_load_party(1);location_select()","" ); ?></td>
                    ...
                    <tr>
                        ...
                        <td class="must_entry_caption">Output Date</td>
            ...
            <fieldset style="width:1280px">
				<legend>Linking Output Bundle List</legend>
                ...
                <table cellpadding="0" cellspacing="1" width="1280">
                    <tr>
                        <td align="center" colspan="19" valign="middle" class="button_container">
                            <?
                                $date=date('d-m-Y');
                                echo load_submit_buttons( $permission, "fnc_linking_output_entry", 0,1 , "reset_form('linkingoutput_1','bundle_list_view','', 'txt_output_date,".$date."','pageReset();')",1); 
                            ?>
