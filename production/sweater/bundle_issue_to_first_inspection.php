<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Bundle Issue To First Inspection
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Bundle Issue To First Inspection","../../", 1, 1, $unicode,'','');
?>
...
<script>
    ...
    {
        var challan_duplicate = return_ajax_request_value(bundle_no, "challan_duplicate_check", "requires/bundle_issue_to_first_inspection_controller");
        ...
        else
        {
			...
			if( (tot_row*1) <1)
			{
				...
				var response_mstdata = return_global_ajax_value(bundle_no[0]+'_'+$('#cbo_company_name').val(), 'populate_mst_data', '', 'requires/bundle_issue_to_first_inspection_controller');
				if(response_mstdata!="")
				{
					var ex_data=response_mstdata.split('_');
					//$('#cbo_company_name').val(ex_data[0]);
					//alert(ex_data[0]+'-'+$('#cbo_company_name').val())
					$('#cbo_source').val(ex_data[2]);
					//fnc_load_party(1);
					load_drop_down( 'requires/bundle_issue_to_first_inspection_controller', ex_data[0]+'_'+1, 'load_drop_down_location', 'location_td' );
					...
					//load_drop_down('requires/bundle_issue_to_first_inspection_controller', location, 'load_drop_down_floor', 'floor_td' );
					...
					//load_drop_down( 'requires/bundle_issue_to_first_inspection_controller', ex_data[3]+'_'+ex_data[4]+'_'+ex_data[5]+'_'+$('#txt_issue_date').val(), 'load_drop_down_line', 'line_td' );
				}
			}
        }
        $('#txt_bundle_no').val('');
	...
	function create_row(bundle_nos, vscan, hidden_source_cond)
    {
        ...
        var response_data = return_global_ajax_value(bundle_nos + "**" + row_num + "****" + $('#cbo_company_name').val() + "**" + vscan + "**" + hidden_source_cond, 'populate_bundle_data', '', 'requires/bundle_issue_to_first_inspection_controller');
	...
	function openmypage_bundle(page_link,title)
	{
		...
		var page_link='	requires/bundle_issue_to_first_inspection_controller.php?action=bundle_popup&company_id='+document.getElementById('cbo_company_name').value+'&bundleNo='+bundleNo;
	...
	function fnc_issue_first_inspection_entry(operation)
	{
		...
		if(operation==4)
		{
			...
			generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_no').val()+'*'+$('#txt_update_id').val()+'*'+report_title, 'issue_to_1st_inspection_print', 'requires/bundle_issue_to_first_inspection_controller');
			release_freezing();
			return;
		}
		...
		if(operation==0 || operation==1 || operation==2)
		{
			...
			var data="action=save_update_delete&operation="+operation+'&tot_row='+j+get_submitted_data_string('txt_system_no*txt_update_id*cbo_company_name*cbo_location*cbo_source*txt_issue_date*txt_challan_no*txt_job_no*txt_remarks*garments_nature',"../../")+dataString;
			
			//alert(data);release_freezing(); return;
			http.open("POST","requires/bundle_issue_to_first_inspection_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
			http.onreadystatechange = fnc_issue_first_inspection_entry_reply_info;
		}
	}
  
	function fnc_issue_first_inspection_entry_reply_info()
	{
		...
		{		
			...
			if(reponse[0]==15) 
			{ 
				 setTimeout('fnc_issue_first_inspection_entry('+ reponse[1]+')',4000); 
			}
			else if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
			{
				...
				fnc_total_calculate();
				set_button_status(1, permission, 'fnc_issue_first_inspection_entry',1,1);	
			}
			if(reponse[0]!=15)
			{
			  release_freezing();
	...
	function openmypage_sysNo()
	{
		...
		var page_link = 'requires/bundle_issue_to_first_inspection_controller.php?action=system_number_popup&company_id='+document.getElementById('cbo_company_name').value;
			
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=870px,height=390px,center=1,resize=0,scrolling=0','../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0];
			...
			{ 
				...
				//fnc_load_party(1);
				//load_drop_down( 'requires/bundle_issue_to_first_inspection_controller', ex_data[2]+'_'+1, 'load_drop_down_location', 'location_td' );
				...
				//load_drop_down( 'requires/bundle_issue_to_first_inspection_controller', ex_data[5]+'_'+ex_data[6]+'_'+ex_data[7]+'_'+ex_data[8], 'load_drop_down_line', 'line_td');
				...
				var response_data = return_global_ajax_value(ex_data[0]+"**"+ ex_data[2], 'populate_bundle_data_update', '', 'requires/bundle_issue_to_first_inspection_controller');
	...
	{
		window.open("requires/bundle_issue_to_first_inspection_controller.php?data=" + data+'&action='+action, true );
	}
	...
	function fnc_load_party(type)
	{
		...
		if(source==1 && type==1)
		{
			load_drop_down( 'requires/bundle_issue_to_first_inspection_controller', company+'_'+1, 'load_drop_down_working_com', 'working_com' );
		}
		else if(source==2 && type==1)
		{
			load_drop_down( 'requires/bundle_issue_to_first_inspection_controller', company+'_'+2, 'load_drop_down_working_com', 'working_com' );
		}
		else if(source==1 && type==2)
		{
			load_drop_down( 'requires/bundle_issue_to_first_inspection_controller', working_company+'_'+2, 'load_drop_down_location', 'working_location_td' ); 
		} 
	}
...
<body onLoad="set_hotkey()">
 <div style="width:100%;">
    ...
    <div style=" float:left" align="left"> 
        <form name="issuefirstinspection_1" id="issuefirstinspection_1" method="" autocomplete="off" >
			<fieldset style="width:1050px;">
        		<legend>Bundle Issue To First Inspection</legend>
                <table width="1050">
                    <tr>
                    	<td width="80" class="must_entry_caption">LC Company</td>
                        <td width="120"><? echo create_drop_down( "cbo_company_name",110, "select id, company_name from lib_company comp  where status_active =1 and  is_deleted=0  $company_cond  order by company_name","id,company_name", 1, "-- Select --", $selected,"","" ); //load_drop_down( 'requires/bundle_issue_to_first_inspection_controller', this.value+'_'+1, 'load_drop_down_location', 'location_td' ); fnc_load_party(1)?></td>
                        <td width="100">Sys. Issue No</td>
                        <td width="120"> 
                        ...
                        <td width="120" id="location_td"><? echo create_drop_down( "cbo_location",  110, $blank_array, "",  1, "-- Select Location --", $selected, "",1 ); ?></td>
            ...
            <fieldset style="width:1205px">
				<legend>Bundle Issue To First Inspection Bundle List</legend>
                ...
                <table cellpadding="0" cellspacing="1" width="1205">
                    <tr>
                        <td align="center" valign="middle" class="button_container">
                            <?
                                $date=date('d-m-Y');
                                echo load_submit_buttons( $permission, "fnc_issue_first_inspection_entry", 0,1 , "reset_form('issuefirstinspection_1','bundle_list_view','', 'txt_issue_date,".$date."','pageReset();')",1); 
                            ?>
