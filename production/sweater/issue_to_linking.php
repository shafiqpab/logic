...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Issue To Linking","../../", 1, 1, $unicode,'','');
...
function openmypage(page_link,title)
{
	...
	emailwindow.onclose=function()
	{
		...
		if (po_id!="")
		{
			...
			get_php_form_data(company_id,'load_variable_settings','requires/issue_to_linking_controller');

			childFormReset();//child from reset
			get_php_form_data(po_id+'**'+item_id+'**'+country_id, "populate_data_from_search_popup", "requires/issue_to_linking_controller" );
			...
			{
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+job_num, 'color_and_size_level',"requires/issue_to_linking_controller" );
			}
			else
			{
			...
			show_list_view(po_id+'**'+item_id+'**'+country_id,'show_dtls_listview','cutting_production_list_view','requires/issue_to_linking_controller','setFilterGrid(\'tbl_list_search\',-1)');
			show_list_view(po_id,'show_country_listview','list_view_country','requires/issue_to_linking_controller','');
	...
	if ( form_validation('cbo_company_name*txt_order_no*cbo_cutting_company*txt_cutting_date','Company Name*Order No*Cutting Comapny*Cutting Date')==false )
	{
	...
	else
		{
			...
			//alert(data); return;
 			...
 			http.open("POST","requires/issue_to_linking_controller.php",true);
...
function cutting_update_entry_Reply_info()
{
 	if(http.readyState == 4)
	{
		...
		else if(reponse[0]==0)//insert
		{
			...
			show_list_view(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id,'show_dtls_listview','cutting_production_list_view','requires/issue_to_linking_controller','setFilterGrid(\'tbl_list_search\',-1)');
			 	//cbo_produced_by*
			reset_form('','','txt_cutting_qty*hidden_break_down_html*hidden_colorSizeID*txt_remark*txt_mst_id','','','txt_cutting_date*txt_reporting_hour');
 			get_php_form_data(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id, "populate_data_from_search_popup", "requires/issue_to_linking_controller" );

			if(variableSettings!=1) {
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id, "color_and_size_level", "requires/issue_to_linking_controller" );
			}
			else
			{
				$("#txt_cutting_qty").removeAttr("readonly");
		...
		else if(reponse[0]==1)//update
		{
			...
			if(variableSettings!=1) {
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id,  "requires/issue_to_linking_controller" );
			}
			else
			{
				$("#txt_cutting_qty").removeAttr("readonly");
...
function put_country_data(po_id, item_id, country_id, po_qnty, plan_qnty)
{
	...
	show_list_view(po_id+'**'+item_id+'**'+country_id,'show_dtls_listview','cutting_production_list_view','requires/issue_to_linking_controller','');
	set_button_status(0, permission, 'fnc_cutting_update_entry',1);
	release_freezing();
}
...
// 	else
// 	{
// 		var response=return_global_ajax_value( po_id+'**'+item_id, 'piece_rate_order_cheack', '', 'requires/issue_to_linking_controller');
...
function fn_chk_next_process_qty(tableName,index,sizeId) // for color and size level
{
	...
	$.ajax({
		url: 'requires/issue_to_linking_controller.php',
		type: 'POST',
		data: data,
                        ...
                        <table width="100%">
                            ...
                            <tr>
            					...
            					<input type="hidden" name="hidden_preceding_process" id="hidden_preceding_process">
								 ...
								 <td width="170">
									 <input name="txt_order_no" placeholder="Double Click to Search" onDblClick="openmypage('requires/issue_to_linking_controller.php?action=order_popup&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value,'Order Search')" id="txt_order_no" class="text_boxes" style="width:200px " readonly />
                                ...
                                <td width="130"   class="must_entry_caption">Company </td>
                                    ...
                                    <td width="170">
                                        <?
                                            echo create_drop_down( "cbo_company_name", 210, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, " get_php_form_data(this.value,'load_variable_settings','requires/issue_to_linking_controller');",1 );
                                        ?>
                              ...
                              <tr>
                                     ...
                                     <td width="170">
                                         <?
                                            echo create_drop_down( "cbo_source", 210, $knitting_source,"", 1, "-- Select Source --", $selected, "fnc_company_check(this.value);load_drop_down( 'requires/issue_to_linking_controller', this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_cutt_company', 'cutt_company_td' );dynamic_must_entry_caption(this.value);",0,'1,3' );
                                         ?>
                                     </td>
                              </tr>
                              <tr>
