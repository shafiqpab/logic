...
function create_row(bundle_nos,vscan)
{
	...
	{
		//alert(bundle_nos)
			get_php_form_data( $("#txt_lot_ratio").val()+'_'+bundle_nos, "populate_data_from_barcode", "requires/bundle_knitting_qc_controller");
			show_list_view( $("#txt_lot_ratio").val()+'_'+bundle_nos, 'show_dtls_listview_bundle', 'bundle_list_view', 'requires/bundle_knitting_qc_controller','');
...
function openmypage_bundle(page_link,title)
{
	...
	var page_link='requires/bundle_knitting_qc_controller.php?action=bundle_popup&company_id='+document.getElementById('cbo_company_name').value+'&lot_ratio='+document.getElementById('txt_lot_ratio').value+'&bundleNo='+bundleNo;
...
function openmypage_bundle_rescan(page_link,title)
{
	...
	var title='Bundle Search';
	var page_link='requires/bundle_knitting_qc_controller.php?action=bundle_popup_rescan&company_id='+document.getElementById('cbo_company_name').value+'&lot_ratio='+document.getElementById('txt_lot_ratio').value+'&bundleNo='+bundleNo;
...
function create_row_rescan(bundle_nos,vscan)
{
	...
	if(tot_row==0)
	{
		// alert(bundle_nos)
		get_php_form_data( $("#txt_lot_ratio").val()+'_'+bundle_nos,"populate_data_from_barcode", "requires/bundle_knitting_qc_controller");
		show_list_view( $("#txt_lot_ratio").val()+'_'+bundle_nos, 'show_dtls_listview_bundle_rescan','bundle_list_view','requires/bundle_knitting_qc_controller','');
		$("#hidden_row_number").val($("#tbl_details").find('tbody tr').length);	
	}
	else
	{
		...
		var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"**"+vscan+"**"+hidden_cutting_no, 'show_dtls_listview_bundle_rescan2', '', 'requires/bundle_knitting_qc_controller');
		$('#tbl_details tbody').append(response_data);
		$("#hidden_row_number").val($("#tbl_details").find('tbody tr').length);
		calculate_defect_qty();
		calculate_reject_qty();
...
function fnc_duplicate_bundle_rescan(bundle_no)
{
	...
	var challan_duplicate=return_ajax_request_value( bundle_no+"__"+$('#cbo_company_name').val(),"qty_rescan_check", "requires/bundle_knitting_qc_controller");	
	...
	else
	{
		...
		{
			var page_link='requires/bundle_knitting_qc_controller.php?action=bundle_popup_line_select&data='+ex_challan_duplicate[2]+"&company_id="+$('#cbo_company_name').val();
			...
			emailwindow.onclose=function()
			{
				...
				//alert(tot_row)	
				if( (tot_row*1) <2)
					get_php_form_data(bundle_no+"__"+ex_challan_duplicate[4]+"__"+$('#txt_issue_date').val(),'load_mst_data','requires/bundle_knitting_qc_controller');
...
function defect_qty_popup(row_id,bundle_id,bundle_qnty)
{
	var actual_infos=$("#actual_defect_"+row_id).val();
	
	var page_link='requires/bundle_knitting_qc_controller.php?action=defect_qty_popup&actual_infos='+actual_infos+'&lot_ratio='+$("#txt_lot_ratio").val()+'&bundle_mst_id='+bundle_id+'&bundle_qnty='+bundle_qnty;
	var title='Defect Record Info';
	
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=600px,height=400px,center=1,resize=1,scrolling=0','../');
	emailwindow.onclose=function()
	{
		var theform=this.contentDoc.forms[0]
		var actual_infos=this.contentDoc.getElementById("actual_defect_infos").value; 
		var actual_qty=this.contentDoc.getElementById("actual_defect_qty").value;
		var breakdown_defect=this.contentDoc.getElementById("hidden_breakdown_defect").value;
		$("#actual_defect_"+row_id).val(actual_infos); 
		$("#txt_defect_qty_"+row_id).val(actual_qty); 
		$("#hidden_breakdown_defect_"+row_id).val(breakdown_defect); 
...
function calculate_defect_qty()
{
	...
	$("#tbl_details").find('tbody tr').each(function()
	{
		...
		var reject_qty=$(this).find('input[name="txt_reject_qty[]"]').val()*1;
		var replace_qty=$(this).find('input[name="txt_replace_qty[]"]').val()*1;
		var qcpass_qty=bundle_qty-defect_qty-reject_qty+replace_qty;
		total_qc_qty+=qcpass_qty;
		$(this).find("td:eq(9)").text(qcpass_qty);
...
function reject_qty_popup(row_id,bundle_id,bundle_qnty)
{
	var actual_infos=$("#actual_reject_"+row_id).val();
	
	var page_link='requires/bundle_knitting_qc_controller.php?action=reject_qty_popup&actual_infos='+actual_infos+'&lot_ratio='+$("#txt_lot_ratio").val()+'&bundle_mst_id='+bundle_id+'&bundle_qnty='+bundle_qnty;
	...
	emailwindow.onclose=function()
	{
		...
		var breakdown_reject=this.contentDoc.getElementById("hidden_breakdown_reject").value;
		$("#actual_reject_"+row_id).val(actual_infos); 
		$("#txt_reject_qty_"+row_id).val(actual_qty); 
		$("#hidden_breakdown_reject_"+row_id).val(breakdown_reject); 
		calculate_reject_qty();
	}
}

function calculate_reject_qty()
{
...
function openmypage_inspec_operator()
{
	...
	{				
		...
		var txt_qc_date = $("#txt_qc_date").val();
		get_php_form_data(employee_data[1]+'**'+txt_qc_date, "get_tot_loss_min", "requires/bundle_knitting_qc_controller");
	}
}//end function
...
function fnc_issue_print_embroidery_entry(operation)
{
	...
	if(operation==0 || operation==1 || operation==2)
	{
		...
		$("#tbl_details").find('tbody tr').each(function()
		{
			...
			var reject_qty 		=$(this).find('input[name="txt_reject_qty[]"]').val();			
			var replace_qty 	=$(this).find('input[name="txt_replace_qty[]"]').val();
			var color_size_id 	=$(this).find('input[name="txt_colorsize_id[]"]').val();
			var actual_defect 	=$(this).find('input[name="actual_defect[]"]').val();	
			var breakdown_defect=$(this).find('input[name="hidden_breakdown_defect[]"]').val();	
			var breakdown_reject=$(this).find('input[name="hidden_breakdown_reject[]"]').val();	
			...
			var qcpass_qty 		=$(this).find("td:eq(9)").text();		
			...
			try 
			{
				...
				dataString+='&bundleNo_' + j + '=' + bundleNo + 
							...
							'&reject_qty_' + j + '=' + reject_qty + 
							'&replace_qty_' + j + '=' + replace_qty + 
							'&actual_defect_' + j + '=' + actual_defect +  
							'&hidden_breakdown_defect_' + j + '=' + breakdown_defect +  	
							'&hidden_breakdown_reject_' + j + '=' + breakdown_reject +  			
		...
		var data="action=save_update_delete&operation="+operation+'&tot_row='+j+get_submitted_data_string('cbo_floor*txt_qc_date*cbo_source*txt_insp_opr_id*txt_operator_id*hidden_insp_sup_id*cbo_working_company*cbo_working_location*cbo_company_name*cbo_location*txt_system_no*txt_system_id*txt_lot_ratio*txt_job_no*txt_loss_min*txt_remarks*garments_nature',"../../")+dataString;
        ...
        <form name="printembro_1" id="printembro_1" method="" autocomplete="off" >
			<fieldset style="width:1050px;">
                ...
                <table width="1130">
                    ...
                    <tr>
                        <td width="80" class="must_entry_caption" id="td_caption">QR Code Scan</td>
                        <td width="130" >
                        	<input name="txt_barcode_no" placeholder="Browse/Write/Scan" onDblClick="openmypage_bundle();" id="txt_barcode_no" class="text_boxes" style="width:120px" />
                        </td>
                        <td width="100" class="must_entry_caption" id="td_caption">Re-Scan QR Code</td>
                        <td width="130">
                        	<input name="txt_barcode_rescan" placeholder="Browse/Write/Scan" onDblClick="openmypage_bundle_rescan('requires/bundle_knitting_qc_controller.php?action=bundle_popup_rescan&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value,'Search Bundle For Rescan');" id="txt_barcode_rescan" class="text_boxes" style="width:120px" />
                        </td>
                        <td width="80">Knit OP ID</td>
                        <td width="130"><input name="txt_operator_id" id="txt_operator_id" class="text_boxes" type="text" style="width:120px" disabled /></td>
                        <td width="90">Knit Operator</td>
                        <td width="130"><input name="txt_operator_name" id="txt_operator_name" class="text_boxes" type="text" style="width:120px" disabled /></td>
                    ...
                    <tr>
                        ...
                        <td id="location_td"><?=create_drop_down( "cbo_location",  130, $blank_array, "",  1, "-- Select Location --", $selected, "",1 ); ?></td>
                        ...
                        <td width="130" id="floor_td"><?=create_drop_down( "cbo_floor", 130, $blank_array,"", 1,"-- Select Floor --", $selected, "",1 ); ?></td> 
                    </tr>
                    <tr>
                        <td class="must_entry_caption">QC Date</td>
                        <td width="110"><input  type="text" name="txt_qc_date" id="txt_qc_date" value="<?=date("d-m-Y"); ?>" class="datepicker" style="width:120px;" /></td> 
                        <td width="80" class="must_entry_caption">Insp Opr ID</td>
                        <td width="110"><input onDblClick="openmypage_inspec_operator();" onKeyDown="fnc_employee_id_scanner(event,this.value,'txt_insp_opr_id*txt_insp_opr_name')" type="text" name="txt_insp_opr_id" id="txt_insp_opr_id" value="" class="text_boxes" style="width:120px;" placeholder="Browse/Scan" /></td> 
                        <td width="80" class="must_entry_caption">Insp Opr Name</td>
                        <td width="110"><input type="text" name="txt_insp_opr_name" id="txt_insp_opr_name" value="" class="text_boxes" style="width:120px;" disabled  /></td> 
                        <td width="80" class="must_entry_caption">Insp Sup Name</td>
                        <td width="110">
                        	<input onDblClick="openmypage_inspec_supp();" onKeyDown="fnc_employee_id_scanner(event,this.value,'hidden_insp_sup_id*txt_insp_sup_name');" type="text" name="txt_insp_sup_name" id="txt_insp_sup_name" value="" class="text_boxes" style="width:120px;" placeholder="Browse/Scan" />
                        	<input name="hidden_insp_sup_id" id="hidden_insp_sup_id" class="text_boxes" type="hidden" disabled />
                      	</td>  
                        <td width="80" class="">Loss Min</td>
                        <td width="110">
                        	<input type="text" name="txt_loss_min" id="txt_loss_min" value="" class="text_boxes_numeric" style="width:50px;"/>&nbsp;<input type="text" name="txt_show_loss_min" id="txt_show_loss_min" value="" class="text_boxes_numeric" style="width:50px; " readonly disabled="disabled" />                        
                      	</td> 
                    </tr>
                    <tr>
	                    <td class="must_entry_caption">Remarks</td>
	                    <td colspan="9"> 
	                    	<input name="txt_remarks" id="txt_remarks" class="text_boxes" type="text" style="width:1030px" /> 
	                    </td>
                    </tr>
                </table>
			</fieldset> <br />
				...
				<legend>Bundle List</legend>
                	<fieldset style="">
                    	<div id="bundle_list_view">
	                        <table cellpadding="0" width="1275" cellspacing="0" border="1" class="rpt_table" rules="all">
	                            <thead>
	                                <th width="30">SL</th>
	                        ...
	                        <div  style="width:1230px;max-height:250px;overflow-y:scroll"  align="left">    
	                            <table cellpadding="0" width="1275" cellspacing="0" border="1" class="rpt_table" rules="all" id="tbl_details">      
	                                <tbody>
