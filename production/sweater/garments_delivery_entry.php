		...
		emailwindow.onclose = function() {
			...
			if (po_id != "") {
				...
				if (variableSettings != 1) {
					get_php_form_data(po_id + '**' + item_id + '**' + variableSettings + '**' + styleOrOrderWisw + '**' + country_id + '**' + $('#hidden_preceding_process').val() + '**' + $('#variable_is_controll').val()+ '**' + ship_date+ '**' + pack_type, "color_and_size_level", "requires/garments_delivery_entry_controller");
				} else {
					$("#txt_ex_quantity").removeAttr("readonly");
				}
				change_shipping_status(0);
				show_list_view(po_id, 'show_country_listview', 'list_view_country', 'requires/garments_delivery_entry_controller', '');
	...
	function fnc_exFactory_entry_Reply_info() {
		if (http.readyState == 4) {
			...
			if (reponse[0] == 15) {
				...
				if (variableSettings != 1) {
					get_php_form_data(po_id + '**' + item_id + '**' + variableSettings + '**' + styleOrOrderWisw + '**' + country_id + '**' + $('#hidden_preceding_process').val() + '**' + $('#variable_is_controll').val()+ '**' + $('#txt_country_ship_date').val()+ '**' + $('#txt_pack_type').val(), "color_and_size_level", "requires/garments_delivery_entry_controller");
				} else {
					$("#txt_ex_quantity").removeAttr("readonly");
				}
				...
				reset_form('', '', 'txt_mst_id*txt_ex_quantity*hidden_actual_po*cbo_shipping_mode*txt_add_info*hidden_add_info*txt_actual_po*hidden_break_down_html*hidden_colorSizeID*txt_total_carton_qnty*txt_invoice_no*txt_lc_sc_no*txt_ctn_qnty*txt_transport_com*shipping_status*txt_remark*txt_finish_quantity*txt_cumul_quantity*txt_yet_quantity*txt_job_no*txt_style_no*txt_shipment_date*txt_order_qty*cbo_item_name*cbo_country_name*txt_order_no', '', '');
				if (variableSettings != 1) {
					get_php_form_data(po_id + '**' + item_id + '**' + variableSettings + '**' + styleOrOrderWisw + '**' + country_id + '**' + $('#hidden_preceding_process').val() + '**' + $('#variable_is_controll').val(), "color_and_size_level", "requires/garments_delivery_entry_controller");
				} else {
					$("#txt_ex_quantity").removeAttr("readonly");
				}
				$('#txt_lc_sc_no').attr('placeholder', ''); //placeholder value initilize
