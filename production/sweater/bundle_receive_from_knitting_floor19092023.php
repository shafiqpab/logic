...
function fnc_check_knit_qty(id){
	$("#tbl_details").find('tbody tr').each(function()
	{
		...
		if(bundle_qty<receive_qty)
		{  
			//$(this).find('input[name="txt_knit_qty[]"]').val('');
			alert("Knit Qty[Pcs] can not be greater than Bundle Qty[Pcs]");
			$(this).find('input[name="txt_knit_qty[]"]').val(bundle_qty);
		}
	});	
 }
...
function fnc_receive_from_knitting_entry(operation)
{
	...
	if(form_validation('cbo_floor*txt_receive_date*cbo_source*txt_operator_id*cbo_working_company*cbo_working_location*cbo_company_name*cbo_location*txt_bodypart_name', 'Floor*Issue Date*Source*Operator ID*W. Company*WC. Location*LC Company*Location*Body Part')==false )
	{
		release_freezing();
		return;
	}
...
function fnc_issue_to_knitting_Reply_info()
{
	...
	{		
		...
		if(trim(reponse[0])=='insIssue'){
			alert("Bundle Issue To First Inspection Found :"+trim(reponse[2])+"\n So Update/Delete Not Possible.")
			release_freezing();
			return;
		}	 
		if(reponse[0]==15) 
        ...
        <form name="printembro_1" id="printembro_1" method="" autocomplete="off" >
			<fieldset style="width:950px;">
        		<legend>Master Part</legend>
                	<table width="920">
                        <tr>
                            <td align="right" colspan="4"><b>Receive ID</b></td>
                            <td colspan="4"> 
                            	<input 
                                	name="txt_challan_no" 
                                    id="txt_challan_no" 
