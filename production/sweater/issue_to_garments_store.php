<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Issue To Garments Store
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Issue To Garments Store","../../", 1, 1, $unicode,'','');
...
<script>
	...
	{ 
		var title = 'Issue To Garments Store Info';	
		var page_link = 'requires/issue_to_garments_store_controller.php?action=system_number_popup';
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				load_drop_down( 'requires/issue_to_garments_store_controller', ex_data[2]+'_'+1, 'load_drop_down_location', 'location_td' );
				...
				load_drop_down('requires/issue_to_garments_store_controller', location, 'load_drop_down_floor', 'floor_td' );
				...
				set_button_status(1, permission, 'fnc_issue_to_garments_store_entry',1,0);
				release_freezing();
			}
		}
	}//end function
	
	function fnc_issue_to_garments_store_entry(operation)
	{
		...
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_name').val()+'*'+$('#txt_update_id').val()+'*'+report_title, "issue_to_garments_store_print", "requires/issue_to_garments_store_controller");
			release_freezing();
			return;
		}
		...
		{  
			if ( form_validation('cbo_company_name*cbo_location*cbo_source*cbo_working_company*txt_job_no*txt_issue_date','Company Name*Location*Source*W. Company*Job No*Issue Date')==false )
			{
				release_freezing();
				return;
			}
			else
			{
				...
				var data="action=save_update_delete&operation="+operation+'&tot_row='+k+get_submitted_data_string('garments_nature*txt_system_no*cbo_company_name*cbo_location*cbo_source*cbo_working_company*cbo_working_location*cbo_floor*txt_job_no*txt_style_ref*cbo_buyer_name*txt_issue_date*cbo_sending_location*txt_challan*txt_remark*txt_job_id*txt_update_id*txt_wo_id',"../../")+data_str;
				// alert (data); return;
				
				http.open("POST","requires/issue_to_garments_store_controller.php",true);
				http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
				http.send(data);
				http.onreadystatechange = fnc_issue_to_garments_store_entry_reply_info;
			}
		}
	}
	
	function fnc_issue_to_garments_store_entry_reply_info()
	{
		if(http.readyState == 4)
		{
			...
			if(reponse[0]==15) 
			{ 
				 setTimeout('fnc_issue_to_garments_store_entry('+ reponse[1]+')',4000); 
			}
	...
	function fnc_dtls_data_load(job_no,uid)
	{
		...
		var list_view_orders = return_global_ajax_value( cbo_company_id+'***'+job_no+'***'+uid, 'order_details', '', 'requires/issue_to_garments_store_controller');
		if(list_view_orders!='')
		{
	...
	function fnc_load_party(type)
	{
		...
		if(source==1 && type==1)
		{
			load_drop_down( 'requires/issue_to_garments_store_controller', company+'_'+1, 'load_drop_down_working_com', 'working_company_td' );
		}
		else if(source==3 && type==1)
		{
			load_drop_down( 'requires/issue_to_garments_store_controller', company+'_'+3, 'load_drop_down_working_com', 'working_company_td' );
		}
		else if(source==1 && type==2)
		{
			load_drop_down( 'requires/issue_to_garments_store_controller', working_company+'_'+2, 'load_drop_down_location', 'working_location_td' ); 
		} 
	}
	...
	function openmypage_woNo()
	{
		...
		{			
			...
			var page_link='requires/issue_to_garments_store_controller.php?company_id='+cbo_company_id+'&cbo_service_source='+cbo_service_source+'&txt_job_no='+txt_job_no+'&action=work_order_popup';
...
<div style="width:100%;">
    ...
    <div style="width:1050px; float:left" align="center">
 		<fieldset style="width:1050px;">
            <legend>Issue To Garments Store Info.</legend>
            <form name="issuetogarmentsstore_1" id="issuetogarmentsstore_1" method="" autocomplete="off" >
                <fieldset>
                    <table width="100%">
                        <tr>
                        ...
                        <tr>
                            ...
                            <td width="140"><? echo create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "--Select--", $selected, "load_drop_down( 'requires/issue_to_garments_store_controller', this.value+'_'+1, 'load_drop_down_location', 'location_td'); fnc_load_party(1,document.getElementById('cbo_source').value);","" ); ?></td>
                        ...
                        <tr>
                            ...
                            <td><input type="text" name="txt_job_no" id="txt_job_no" class="text_boxes" style="width:120px" onDblClick="openmypage_job('requires/issue_to_garments_store_controller.php?action=job_popup', 'Job/Order Selection Form');" placeholder="Browse" readonly /></td>
                            <td>Style Ref.</td>
                ...
                <fieldset style="width:1050px">
                    <legend>Issue To Garments Store Details List</legend>
                    <div>
                    ...
                    <table cellpadding="0" cellspacing="1" width="1050">
                        <tr>
                            <td align="center" colspan="14" valign="middle" class="button_container">
                                <?
                                    $date=date('d-m-Y');
                                    echo load_submit_buttons( $permission, "fnc_issue_to_garments_store_entry", 0,1 , "reset_form('issuetogarmentsstore_1','','', 'txt_issue_date,".$date."','')",1); 
                                ?>
