...
<?php
if($_SESSION['logic_erp']['data_arr'][685]){
	echo "var field_level_data= " . json_encode($_SESSION['logic_erp']['data_arr'][685]). ";\n";
}
?>
...
function fnc_wash_output_entry(operation)
{
	...
	else if(operation==0 || operation==1 || operation==2)
	{
		...
		else
		{
			...
			if('<? echo implode('*', $_SESSION['logic_erp']['mandatory_field'][685]);?>')
			{
				if (form_validation('<? echo implode('*', $_SESSION['logic_erp']['mandatory_field'][685]);?>','<? echo implode('*', $_SESSION['logic_erp']['mandatory_message'][685]);?>')==false)
				{
					release_freezing();
					return;
				}
...
function fnc_company_check(val)
{
	...
	function fnc_wo_no()
	{
		if ( form_validation('cbo_company_name*cbo_source*cbo_wash_company','Company Name*Production Source*Wash Company')==false )
		{
			return;
		}
		else
		{
			...
			var service_company_id=$("#cbo_wash_company").val();
			var txt_job_no=$("#txt_job_no").val();
			
			var title = 'Service WO Selection Popup';
			
			var page_link="requires/wash_entry_controller.php?action=wo_no_popup&company_id="+company_id+'&service_company_id='+service_company_id+'&txt_job_no='+txt_job_no;
                ...
                <fieldset>
                    <table width="100%" border="0">
                        <tr>
                            ...
                            <td width="150">                                
								<? 
                                 echo create_drop_down( "cbo_company_name", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "fnc_loadvariable(this.value);setFieldLevelAccess(this.value);" );
                                ?>
                ...
                <table cellpadding="0" cellspacing="1" width="100%">
                    <tr>
                    	<td width="35%" valign="top">
                            <fieldset>
                            <legend id="workorder_rate_id" style=" color:red; font-size:12px">New Entry</legend>
                                <table  cellpadding="0" cellspacing="1" width="100%">
                                	<tr>
