<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Bundle Wise Mending
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Bundle Wise Mending","../../", 1, 1, $unicode,'','');
?>
...
<script>
    ...
    {
        var challan_duplicate = return_ajax_request_value(bundle_no, "challan_duplicate_check", "requires/bundle_mending_controller");
        ...
        else
        {
			...
			if( (tot_row*1) <2)
			{
				get_php_form_data(bundle_no+'_'+$('#cbo_company_name').val()+'_'+$('#txt_mending_date').val(), "populate_mst_data", "requires/bundle_mending_controller");
	...
	function create_row(bundle_nos, vscan, hidden_source_cond)
    {
        ...
        var response_data = return_global_ajax_value(bundle_nos + "**" + row_num + "****" + $('#cbo_company_name').val() + "**" + vscan + "**" + hidden_source_cond, 'populate_bundle_data', '', 'requires/bundle_mending_controller');
	...
	function openmypage_bundle(page_link,title)
	{
		...
		var page_link='	requires/bundle_mending_controller.php?action=bundle_popup&company_id='+document.getElementById('cbo_company_name').value+'&bundleNo='+bundleNo;
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				get_php_form_data(hidden_bundle_nos+'_'+$('#cbo_company_name').val()+'_'+$('#txt_mending_date').val(), "populate_mst_data", "requires/bundle_mending_controller");
	...
	function fnc_mending_entry(operation)
	{
		...
		if (form_validation('cbo_company_name*cbo_location*cbo_source*cbo_working_company*cbo_working_location*cbo_floor*txt_mending_date*txt_reporting_hour','LC Company*LC Location*Source*W. Company*WC. Location*Floor*Mending Date*Reporting Hour')==false )
		{
			release_freezing();
			return;
		}
		...
		if(operation==0 || operation==1 || operation==2)
		{
			...
			var data="action=save_update_delete&operation="+operation+'&tot_row='+j+get_submitted_data_string('txt_system_no*txt_update_id*cbo_company_name*cbo_location*cbo_source*cbo_working_company*cbo_working_location*cbo_floor*cbo_line_no*txt_mending_date*txt_reporting_hour*txt_challan_no*txt_job_no*txt_remarks*garments_nature',"../../")+dataString;
			
			//alert(data); release_freezing(); return;
			http.open("POST","requires/bundle_mending_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
			http.onreadystatechange = fnc_mending_entry_reply_info;
		}
	}
  
	function fnc_mending_entry_reply_info()
	{
		...
		{		
			...
			if(reponse[0]==15) 
			{ 
				 setTimeout('fnc_mending_entry('+ reponse[1]+')',4000); 
			}
			else if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
			{
				...
				set_button_status(1, permission, 'fnc_mending_entry',1,0);	
			}
			if(reponse[0]!=15)
			{
			  release_freezing();
	...
	function openmypage_sysNo()
	{
		...
		var page_link = 'requires/bundle_mending_controller.php?action=system_number_popup&data='+company_id;
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				fnc_load_party(1);
				load_drop_down( 'requires/bundle_mending_controller', ex_data[2]+'_'+1, 'load_drop_down_location', 'location_td' );
				...
				load_drop_down('requires/bundle_mending_controller', location, 'load_drop_down_floor', 'floor_td' );
				$('#cbo_floor').val(ex_data[7]);
				$('#txt_mending_date').val(ex_data[8]);
				
				load_drop_down( 'requires/bundle_mending_controller', ex_data[5]+'_'+ex_data[6]+'_'+ex_data[7]+'_'+ex_data[8], 'load_drop_down_line', 'line_td');
				...
				var response_data = return_global_ajax_value(ex_data[0]+"**"+ ex_data[2], 'populate_bundle_data_update', '', 'requires/bundle_mending_controller');
	...
	{
		window.open("requires/bundle_mending_controller.php?data=" + data+'&action='+action, true );
	}
	...
	function fnc_load_party(type)
	{
		...
		if(source==1 && type==1)
		{
			load_drop_down( 'requires/bundle_mending_controller', company+'_'+1, 'load_drop_down_working_com', 'working_com' );
		}
		else if(source==2 && type==1)
		{
			load_drop_down( 'requires/bundle_mending_controller', company+'_'+2, 'load_drop_down_working_com', 'working_com' );
		}
		else if(source==1 && type==2)
		{
			load_drop_down( 'requires/bundle_mending_controller', working_company+'_'+2, 'load_drop_down_location', 'working_location_td' ); 
		} 
	}
...
<body onLoad="set_hotkey()">
 <div style="width:100%;">
    ...
    <div style=" float:left" align="left"> 
        ...
        <form name="mending_1" id="mending_1" method="" autocomplete="off" >
			<fieldset style="width:1050px;">
        		<legend>Mending</legend>
                <table width="1050">
                    <tr>
                        <td align="right" colspan="4"><b>Mending Challan No</b></td>
                        <td colspan="4"> 
                    ...
                    <tr>
                        ...
                        <td width="140"><? echo create_drop_down( "cbo_company_name",130, "select id, company_name from lib_company comp  where status_active =1 and  is_deleted=0  $company_cond  order by company_name","id,company_name", 1, "-- Select --", $selected,"load_drop_down( 'requires/bundle_mending_controller', this.value+'_'+1, 'load_drop_down_location', 'location_td' ); fnc_load_party(1);location_select()","" ); ?></td>
                    ...
                    <tr>
                        ...
                        <td class="must_entry_caption">Mending Date</td>
                        <td><input type="text" name="txt_mending_date" id="txt_mending_date" value="<? echo date("d-m-Y")?>" class="datepicker" style="width:120px;" /></td> 
                     </tr>
                     <tr>
                     	<td class="must_entry_caption">Rep. Hour</td>
                         <td> 
            ...
            <fieldset style="width:1280px">
				<legend>Mending Bundle List</legend>
                    ...
                    <table cellpadding="0" cellspacing="2" width="1280">
                        <tr>
                            ...
                            <td align="left" width="240"> 
                                <input name="txt_bundle_no" id="txt_bundle_no" class="text_boxes" style="width:212px" placeholder="Browse / Write / Scan" onDblClick="openmypage_bundle('requires/bundle_mending_controller.php?action=bundle_popup&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value, 'Bundle Search');" />
                            </td>
                            <td width="100" class="must_entry_caption" align="right">Re-Scan Barcode</td>
                            <td> 
                               <input name="txt_bundle_rescan" placeholder="Browse / Write / Scan" onDblClick="openmypage_bundle_rescan( 'requires/bundle_mending_controller.php?action=bundle_popup_rescan&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value,'Search Bundle For Rescan')" id="txt_bundle_rescan" class="text_boxes" style="width:212px" />
                            </td>
                        </tr>
                    </table>
                ...
                <table cellpadding="0" cellspacing="1" width="1280">
                    <tr>
                        <td align="center" colspan="19" valign="middle" class="button_container">
                            <?
                                $date=date('d-m-Y');
                                echo load_submit_buttons( $permission, "fnc_mending_entry", 0,0 , "reset_form('mending_1','bundle_list_view','', 'txt_mending_date,".$date."','pageReset();')",1); 
                            ?>
