	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			if (po_id!="")
			{
				...
				show_list_view(po_id+'**'+item_id+'**'+country_id+'**'+prod_reso_allo+'**'+pack_type+'**'+variableSettings,'show_dtls_listview','list_view_container','requires/poly_entry_controller','setFilterGrid(\'tbl_list_search\',-1)');
...
function fnc_poly_output_entry(operation)
{
	...
	else if(operation==0 || operation==1 || operation==2)
	{
		...
		else
		{
			...
			var data="action=save_update_delete&operation="+operation+"&colorIDvalue="+colorIDvalue+"&colorIDvalueRej="+colorIDvalueRej+get_submitted_data_string('garments_nature*cbo_company_name*cbo_country_name*poly_production_variable*poly_production_variable_rej*hidden_po_break_down_id*hidden_colorSizeID*cbo_buyer_name*txt_style_no*cbo_item_name*txt_order_qty*cbo_source*cbo_poly_company*cbo_location*cbo_floor*txt_poly_date*cbo_produced_by*cbo_poly_line*txt_reporting_hour*cbo_color_type*txt_super_visor*txt_poly_qty*txt_reject_qnty*txt_alter_qnty*txt_challan*txt_remark*txt_input_quantity*txt_cumul_poly_qty*txt_yet_to_poly*hidden_break_down_html*txt_mst_id*prod_reso_allo*txt_spot_qnty*save_data*defect_type_id*save_dataSpot*defectSpot_type_id*save_dataReject*defectReject_type_id*hidden_currency_id*hidden_exchange_rate*hidden_piece_rate*cbo_work_order*txt_pack_type',"../");
 			// alert (data); return;
 	...
 	if(http.readyState == 4) 
	{
		...
		else if(reponse[0]==0)
		{
			...
			show_msg(trim(reponse[0]));
 			show_list_view(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id+'**'+prod_reso_allo+'**'+pack_type+'**'+variableSettings,'show_dtls_listview','list_view_container','requires/poly_entry_controller','setFilterGrid(\'tbl_list_search\',-1)');
...
function put_country_data(po_id, item_id, country_id, po_qnty, plan_qnty, pack_type)
{
	...
	show_list_view(po_id+'**'+item_id+'**'+country_id+'**'+prod_reso_allo+'**_'+'**'+variableSettings,'show_dtls_listview','list_view_container','requires/poly_entry_controller','');
	set_button_status(0, permission, 'fnc_poly_output_entry',1,0);
	release_freezing();
}
                    ...
                    <table width="100%" border="0">
                        <tr>
                           ...
                           <input type="hidden" name="hidden_preceding_process" id="hidden_preceding_process">
                         	 <td class="must_entry_caption">Attachment Company</td>
                    	...
                    	<td width="35%" valign="top">
                            ...
                            <legend>New Entry</legend>
                                <table  cellpadding="0" cellspacing="1" width="100%">
                                    ...
                                    <tr>
                                    	<td class="must_entry_caption">Attachment Date</td>
                                         <td colspan="2"> 
                                     ...
                                     <tr>
                                        <td class="must_entry_caption">Attachment Line No</td> 
                                   ...
                                   <tr>
                                     ...
                                     <td><input type="button" name="btn" id="btn" class="formbuttonplasminus" value="Rej Defect" style="width:60px" onClick="openmypage_defectQty(3);"/></td>
                                   </tr>
                                   <tr>
