...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Grey febric delivery","../", 1, 1, $unicode,1,1); 
...
var tableFilters = 
	 {
		col_55: "none",
		col_operation: {
		id: ["value_total_prod_qntyinpcs","value_total_delivery_qntyinpcs","value_total_balance_delivery_qntyinpcs","value_current_qntyinpcs_val","value_total_stock","value_gt_tot_delivery","value_total_balance"], 
...
function generate_list()
{
	...
	return;
		...
		var data="action=list_generate"+get_submitted_data_string("cbo_company_id*cbo_location_id*cbo_buyer_id*txt_prog_no*cbo_year*txt_job_no*txt_style*txt_ord_no*txt_date_from*txt_date_to*cbo_status*update_mst_id*hidden_receive_id*hidden_product_id*hidden_order_id*cbo_order_type*cbo_knitting_source","../");
	freeze_window(4);
...
function new_window()
{
	...
	$('#tbl_header thead').find('th:nth-child(21),th:nth-child(22),th:nth-child(23)').hide();
	$('#table_body tbody').find('td:nth-child(21),td:nth-child(22),td:nth-child(23)').hide();
	$('#tbl_footer tfoot').find('th:nth-child(5),th:nth-child(6),th:nth-child(7)').hide();
	$('#tbl_footer tfoot').find('th:nth-child(4)').attr("width","80");
	$('#tbl_footer tfoot').find('th:nth-child(3)').attr("width","80");
	$('#tbl_footer tfoot').find('th:nth-child(2)').attr("width","80");
...
'<html><head><title></title><link rel="stylesheet" href="../css/style_common.css" type="text/css"/></head><body>'+document.getElementById('report_print').innerHTML+'</body</html>');
	...
	$('#tbl_header thead').find('th:nth-child(21),th:nth-child(22),th:nth-child(23)').show();
	$('#table_body tbody').find('td:nth-child(21),td:nth-child(22),td:nth-child(23)').show();
	$('#tbl_footer tfoot').find('th:nth-child(5),th:nth-child(6),th:nth-child(7)').show();
	$('#tbl_footer tfoot').find('th:nth-child(4)').removeAttr().attr("width","70");
	$('#tbl_footer tfoot').find('th:nth-child(3)').removeAttr().attr("width","70");
	$('#tbl_footer tfoot').find('th:nth-child(2)').removeAttr().attr("width","70");
...
function fnc_prod_delivery(operation)
{
	...
	if(operation==4) // Print
	{
		...
		print_report(program_ids+'_'+company+'_'+from_date+'_'+to_date+'_'+product_ids+'_'+order_ids+'_'+location+'_'+buyer+'_'+update_mst_id+'_'+delivery_date+'_'+Challan_no+'_'+cbo_order_type+'_'+report_title, "delivery_challan_print", "requires/grey_feb_delivery_entry_controller" ) ;
	}
	else if(operation==5) // Print(short)
	{
		var program_ids=product_ids=order_ids="";
		var total_tr=$('#table_body tbody tr').length-1;
		var company=$('#cbo_company_id').val();
		...
		print_report(program_ids+'_'+company+'_'+from_date+'_'+to_date+'_'+product_ids+'_'+order_ids+'_'+location+'_'+buyer+'_'+update_mst_id+'_'+delivery_date+'_'+Challan_no+'_'+cbo_order_type+'_'+report_title, "delivery_challan_print_2", "requires/grey_feb_delivery_entry_controller" ) ;
	}
	else if(operation==7) // Print 3
	{
		var program_ids=product_ids=order_ids="";
		var total_tr=$('#table_body tbody tr').length-1;
		var company=$('#cbo_company_id').val();
		...
		print_report(program_ids+'_'+company+'_'+from_date+'_'+to_date+'_'+product_ids+'_'+order_ids+'_'+location+'_'+buyer+'_'+update_mst_id+'_'+delivery_date+'_'+Challan_no+'_'+cbo_order_type+'_'+report_title, "delivery_challan_print_4", "requires/grey_feb_delivery_entry_controller" ) ;
	}
	else if(operation==6) // Print 2
	{
		var program_ids=product_ids=order_ids="";
		var total_tr=$('#table_body tbody tr').length-1;
		var company=$('#cbo_company_id').val();
		...
		print_report(program_ids+'_'+company+'_'+from_date+'_'+to_date+'_'+product_ids+'_'+order_ids+'_'+location+'_'+buyer+'_'+update_mst_id+'_'+delivery_date+'_'+Challan_no+'_'+cbo_order_type+'_'+report_title, "delivery_challan_print_3", "requires/grey_feb_delivery_entry_controller" ) ;
	}
	else if(operation==8) // Print(short2)
	{
		var program_ids=product_ids=order_ids="";
		var total_tr=$('#table_body tbody tr').length-1;
		var company=$('#cbo_company_id').val();
		...
		print_report(program_ids+'_'+company+'_'+from_date+'_'+to_date+'_'+product_ids+'_'+order_ids+'_'+location+'_'+buyer+'_'+update_mst_id+'_'+delivery_date+'_'+Challan_no+'_'+cbo_order_type+'_'+report_title, "delivery_challan_print_short2", "requires/grey_feb_delivery_entry_controller" ) ;
	}
	else
	{
		var details_data=""
		...
		for(var i=1; i<=total_row; i++)
		{
			...
			{
				details_data +='hidesysid_'+i+'*'+'hidesysnum_'+i+'*'+'hideprodid_'+i+'*'+'hidejob_'+i+'*'+'hideorder_'+i+'*'+'hideconstruction_'+i+'*'+'hidecomposition_'+i+'*'+'hidegsm_'+i+'*'+'hidedia_'+i+'*'+'txtcurrentdelivery_'+i+'*'+'hiddendtlsid_'+i+'*'+'txtcurrentRejdelivery_'+i+'*'+'txtroll_'+i+'*'+'txt_remarks_'+i+'*'+'hideprogrum_'+i+'*'+'hidefindtls_'+i+'*'+'hidtotalqtyTd_'+i+'*'+'txtcurrentdelivery_qntyinpcs_'+i+'*'+'hidesize_'+i+'*';
			}
		}
		//alert(details_data);return;
		// hidesysid_, hideprodid_, hideorder_
...
function fnc_prod_delivery_response()
{
	if(http.readyState == 4) 
	{
		...
		{
			//alert(reponse[1]);
			$('#txt_sys_num').val(reponse[2]);
			$('#update_mst_id').val(reponse[1]);
...
function open_mypage()
{
	...
	emailwindow.onclose=function()
	{
		...
		if(trim(ex_data[0])!="")
		{
			$('#update_mst_id').val(ex_data[0]);
			$('#txt_sys_num').val(ex_data[3]);
			$('#txt_delevery_date').val(ex_data[4]);
...
function sum_all_td()
{
	...
	{
		ttl_sum+=$('#hiddenCurrentVal_'+k).val()*1;
	}
	$('#total_current_val').text(ttl_sum);	 
}
function sum_all_td_pcs()
{
	var total_tr =$('#table_body tbody tr').length-1;
	var ttl_sum=0;	var k;
	for(k=1;k<=total_tr;k++)
	{
		ttl_sum+=$('#hiddenCurrentValQntyInPcs_'+k).val()*1;
	}
	$('#value_current_qntyinpcs_val').text(ttl_sum);	 
}
/*function setHideval( i )
{
	var dev_qty=$('#txtcurrentdelivery_'+i).val()*1;
	//alert(dev_qty+" txtcurrentdelivery qty ");
	//console.log(dev_qty);
	...
	{
		//$('#total_current_val').text(($('#total_current_val').text()*1)-($('#hiddenCurrentVal_'+i).val()*1));		
		//$('#total_current_val').text(number_format((($('#total_current_val').text()*1)+(dev_qty*1)),2));		
		$('#hiddenCurrentVal_'+i).val(dev_qty);		
	}
	sum_all_td();	
}*/

/*function setHideval___( i,receive_id, prod_id, po_breakdown_id )
{
	//alert(i+'='+receive_id+'='+prod_id+'='+po_breakdown_id); 
	var formData = 'receive_id='+receive_id+'&prod_id='+prod_id+'&po_breakdown_id='+po_breakdown_id+'&action=current_delv_good_qty';
	$.ajax({
		url: 'requires/grey_feb_delivery_entry_controller.php',
		type: 'POST',
		data: formData,
		success: function (response) {
			//alert(response);
			var dev_qty=$('#txtcurrentdelivery_'+i).val()*1+response*1;
			//var prod_qty=$('#totalqtyTd_'+i).text();
			var prod_qty=$('#hidtotalqtyTd_'+i).val();
			if((dev_qty*1)>(prod_qty*1))
			{	
				alert("Delivery Quantity Must be Less Then Blance Quantity");
				$('#txtcurrentdelivery_'+i).val("");
...
function setHidevalPcs( i )
{
	var dev_qty=$('#txtcurrentdelivery_qntyinpcs_'+i).val()*1;
	var prod_qty=$('#hidtotalqtyInPcsTd_'+i).val();
	if((dev_qty*1)>(prod_qty*1))
	{
		alert("Delivery Quantity In Pcs Must be Less Then Blance Quantity");
		$('#txtcurrentdelivery_qntyinpcs_'+i).val("");
	}
	else
	{
		$('#hiddenCurrentValQntyInPcs_'+i).val(dev_qty);		
	}
	sum_all_td_pcs();	
}
function setHideval( i )
...
function total_reject(i)
{
	var reject_qty=($('#txtcurrentRejdelivery_'+i).val()*1);
	var hidereject=($('#hidden_current_rej_val_'+i).val()*1);
	if(hidereject>0)
	{
		$('#total_reject').text($('#total_reject').text()-$('#hidden_current_rej_val_'+i).val());
	}
	$('#total_reject').text(number_format((($('#total_reject').text()*1)+reject_qty),2));
	$('#hidden_current_rej_val_'+i).val(reject_qty);
}
...
<body onLoad="set_hotkey();">
    ...
    <? echo load_freeze_divs ("../",$permission);  ?><br />    		 
        ...
        <fieldset style="width:1320px;">
            ...
            <table class="" width="1320" cellpadding="0" cellspacing="0" rules="all" border="1">
                <tr>
                    <td align="right" width="200" class="must_entry_caption">Delivery Date&nbsp;:</td>
                    <td width="170">
       ...
       <!-- <h3 align="left" id="accordion_h1" style="width:1120px" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3>--> 
            <div id="content_search_panel" style="width:1300px" align="left">      
            <table class="rpt_table" width="1300" cellpadding="0" cellspacing="0" rules="all" border="1">
                <thead>
					...
					<th width="120">Kniting Source</th>
                ...
                <tbody>
                    <tr>
                        <td>
                            <? 
                                echo create_drop_down( "cbo_company_id", 100, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "--Select Company--", $selected , "load_drop_down( 'requires/grey_feb_delivery_entry_controller', this.value, 'load_drop_down_location', 'location_td' );load_drop_down( 'requires/grey_feb_delivery_entry_controller', this.value, 'load_drop_down_buyer_form', 'buyer_td' );" );
                            ?>                            
                        </td>
                        <td id="location_td">
                            <? 
                                echo create_drop_down( "cbo_location_id",100,$blank_array,"", 1, "--Select Location--", $selected, "","","","","","",2);
                            ?>
                        </td>
                        <td id="buyer_td">
                            <? 
                        ...
                        <td >
                            <input type="text" name="txt_style" id="txt_style" class="text_boxes" style="width:85px;" />
                        </td>
                        <td >
                            <input type="text" name="txt_ord_no" id="txt_ord_no" class="text_boxes" style="width:85px;" />
                        </td>
						<td>
                            <? 
                               	echo create_drop_down("cbo_knitting_source",120,$knitting_source,"", 1, "-- Select --", 0,"",0,'1,3');
                            ?>
                        </td>
                        <td>
                        ...
                        <td>
                            <? 
                                ...
                                echo create_drop_down( "cbo_status", 80, $delevery_status,"", 0, "", 1 );
                            ?>
                        </td>
                        <td>
                            <? 
                                $order_status=array(1=>"With Order",2=>"Without Order");
                                echo create_drop_down( "cbo_order_type", 80, $order_status,"", 0, "", 1 );
                            ?>
                        </td>
                        <td>
                    ...
                    <tr>
                        <td colspan="14" align="center" valign="bottom"><? echo load_month_buttons(1);  ?></td>
                    </tr>
              </tbody>
            </table> 
        </div>
