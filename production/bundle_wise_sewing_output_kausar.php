...
function openmypage_bundle(page_link,title)
{
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				get_php_form_data(hidden_bundle_nos,'load_mst_data','requires/bundle_wise_sewing_output_controller');
...
$('#txt_bundle_no').live('keydown', function(e) {
	if (e.keyCode === 13) 
	{
		...
		var flag=1;
			...
			if(flag==1)
			{
			get_php_form_data(txt_bundle_no,"challan_duplicate_check", "requires/bundle_wise_sewing_output_controller");
			get_php_form_data(txt_bundle_no,'load_mst_data','requires/bundle_wise_sewing_output_controller');	
...
function create_row(bundle_nos)
{
	...
	var response_data=return_global_ajax_value(bundle_nos+"**"+row_num, 'populate_bundle_data', '', 'requires/bundle_wise_sewing_output_controller');
	$('#tbl_details tbody').prepend(response_data);
	var tot_row=$('#tbl_details tbody tr').length; 
	$('#txt_tot_row').val(tot_row);
	release_freezing();
