...
<script>
	...
	function check_batch()
	{
		...
		if(batch_no!="")
		{
			...
			var response=return_global_ajax_value( batch_no, 'check_batch_deying', '', 'requires/special_finish_controller');
	...
	function fnc_pro_fab_subprocess( operation )
	{
		...
		//alert(data_all);return;
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('cbo_company_id*txt_special_fin*txt_batch_no*txt_batch_ID*hidden_batch_id*txt_process_id*txt_process_name*txt_process_end_date*txt_process_date*txt_end_hours*txt_end_minutes*cbo_machine_name*cbo_result_name*txt_remarks*txt_ext_id*txt_update_id*cbo_floor*cbo_shift_name*txt_process_start_date*txt_start_hours*txt_start_minutes*txt_recevied_chalan*cbo_service_company*cbo_service_source*txt_issue_chalan*roll_maintained*txt_roll_id*txt_issue_mst_id*txt_booking_no*txt_trims_weight*hidden_exchange_rate*hidden_currency*txt_temparature*txt_speed*txt_feed*txt_steam*cbo_next_process*txt_advance_prod*re_checkbox',"../../")+data_all+'&total_row='+row_num;
		//alert (data);return;
                    ...
                    <table cellpadding="0" cellspacing="2" width="100%" id="main_tbl">
                        ...
                        <tr>
                         <td class="must_entry_caption" width="130">Company</td>
                            <td>
                                <?
									$comp_sql="SELECT comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name";
