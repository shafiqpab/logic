...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../../logout.php";
	...
	function openmypage_batchnum()
	{
		...
		//else
		//{
			var page_link='requires/aop_wash_controller.php?cbo_company_id='+cbo_company_id+'&action=batch_number_popup';
			...
			emailwindow.onclose=function()
			{
				...
				if(batch_data!="")
				{
					...
					freeze_window(5);
					get_php_form_data(batch_id+'_'+is_sales, "populate_data_from_batch", "requires/aop_wash_controller" );
					show_list_view(batch_id+'_'+roll_maintained,'show_fabric_desc_listview','list_fabric_desc_container','requires/aop_wash_controller','');
					show_list_view( batch_id,'show_dtls_list_view','list_container','requires/aop_wash_controller','');
					release_freezing();
				}
				get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/aop_wash_controller' );
				//get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/aop_wash_controller' );
			}
		//}
	}
	...
	function openmypage_servicebook()
	{
		...
		var page_link='requires/aop_wash_controller.php?cbo_company_id='+cbo_company_id+'&supplier_id='+supplier_id+'&process_id='+process_id+'&action=service_booking_popup';
	...
	function check_batch()
	{
		...
		if(batch_no!="")
		{
			...
			var response=return_global_ajax_value( cbo_company_id+"**"+batch_no, 'check_batch_no', '', 'requires/aop_wash_controller');
			...
			else
			{
				...
				get_php_form_data(response[1]+'_'+response[3], "populate_data_from_batch", "requires/aop_wash_controller" );
				show_list_view(response[1]+'_'+roll_maintained,'show_fabric_desc_listview','list_fabric_desc_container','requires/aop_wash_controller','');
				show_list_view(response[1],'show_dtls_list_view','list_container','requires/aop_wash_controller','');
				get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/aop_wash_controller' );
	...
	function check_issue_challan() //Issue Challan
	{
		...
		if(issue_chalan!="")
		{
			...
			var response=return_global_ajax_value( cbo_company_id+"**"+issue_chalan, 'check_issue_challan_no', '', 'requires/aop_wash_controller');
			var response=response.split("_");
			...
			else
			{
				...
				//$('#hidden_batch_id').val(response[1]);
				//var update_id_dtl=$('#txt_update_id').val(); 
				get_php_form_data(response[1], "populate_data_from_data", "requires/aop_wash_controller" );
				var row_num=$('#tbl_item_details tbody tr').length-1;
				//alert(row_num);return;
				
				var hidden_roll_id = $('#txt_roll_id').val();
				show_list_view(hidden_roll_id+'_'+batch_id,'issue_show_fabric_desc_listview','list_fabric_desc_container','requires/aop_wash_controller','');
	...
	function fnc_pro_fab_subprocess( operation )
	{
		...
		if (operation==0)
		{
			...
			if('<?php echo implode('*',$_SESSION['logic_erp']['mandatory_field'][424]);?>'){
				if (form_validation('<?php echo implode('*',$_SESSION['logic_erp']['mandatory_field'][424]);?>','<?php echo implode('*',$_SESSION['logic_erp']['field_message'][424]);?>')==false)
				{
					return;
				}
			}
		...
		//alert(row_num);return;
			...
			if((page_upto*1==1 || page_upto*1>1) && roll_maintained==1  ) // && roll_maintained==1 
			{
				...
				for (var i=1; i<=row_num; i++)
				{
					...
					data_all+=get_submitted_data_string('txtconscomp_'+i+'*txtgsm_'+i+'*txtbodypart_'+i+'*txtdiawidth_'+i+'*txtbatchqnty_'+i+'*txtprodid_'+i+'*updateiddtls_'+i+'*txtdiawidthID_'+i+'*txtroll_'+i+'*txtproductionqty_'+i+'*rollid_'+i+'*checkRow_'+i+'*txtrate_'+i+'*txtamount_'+i+'*txtafterknitdensity_'+i,"../../",i);
				}
  ...
  http.open("POST","requires/aop_wash_controller.php",true);
  ...
  http.onreadystatechange = fnc_pro_fab_subprocess_response;
	 ...
	 function fnc_pro_fab_subprocess_response()
	 {
		if(http.readyState == 4) 
		{
			...
			show_list_view(document.getElementById('txt_batch_ID').value+'_'+document.getElementById('roll_maintained').value,'show_fabric_desc_listview','list_fabric_desc_container','requires/aop_wash_controller','');
			//show_list_view(document.getElementById('txt_batch_ID').value,'show_fabric_desc_listview','list_fabric_desc_container','requires/aop_wash_controller','');
			show_list_view( document.getElementById('txt_batch_ID').value,'show_dtls_list_view','list_container','requires/aop_wash_controller','');
	...
	var cbo_company_id = $('#cbo_company_id').val();
	//var roll_maintained=$('#roll_maintained').val();
	//var response=return_global_ajax_value( cbo_company_id+"**"+str, 'check_batch_no_scan', '', 'requires/aop_wash_controller');
	var response=return_global_ajax_value( cbo_company_id+"**"+str, 'check_batch_no_scan', '', 'requires/aop_wash_controller');
	//var response=response.split("_");
	...
	/*if(response[0]==0)
		...
		{
	$('#hidden_batch_id').val(response[1]);
	get_php_form_data(response[1]+'_'+roll_maintained, "populate_data_from_batch", "requires/aop_wash_controller" );
	show_list_view(response[1]+'_'+roll_maintained,'show_fabric_desc_listview','list_fabric_desc_container','requires/aop_wash_controller','');
	get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/aop_wash_controller' );
...
$('#txt_batch_no').live('keydown', function(e) {
	...
	function openmypage_issue_challan()
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/aop_wash_controller.php?cbo_company_id='+cbo_company_id+'&action=issue_challan_popup','Issue Popup', 'width=880px,height=350px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function()
		{
			...
			if(issue_id!="")
			{
				get_php_form_data(issue_id, "populate_data_from_data", "requires/aop_wash_controller");
				//show_list_view(issue_id,'show_fabric_desc_listview_issue','list_fabric_desc_container','requires/aop_wash_controller','');
				var row_num=$('#tbl_item_details tbody tr').length-1;
				//alert(row_num);return;
				var issue_roll = $('#txt_roll_id').val();
				var update_roll_arr=new Array();
	...
	{ 
		...
		//alert(com);
		get_php_form_data($('#cbo_company_id').val(),'roll_maintained','requires/aop_wash_controller' );
		var roll_maintained=$('#roll_maintained').val();
		var page_upto=$('#page_upto').val();
		//alert(page_upto);
		if((page_upto*1==1 || page_upto*1>1) && roll_maintained==1  ) // && roll_maintained==1 
	 ...
	 function check_issue_challan_scan(str) //Issue Challan
	{
		...
		if(issue_chalan!="")
		{
			...
			else
			{
				...
				get_php_form_data(response[1], "populate_data_from_data", "requires/aop_wash_controller" );
				var hidden_roll_id = $('#txt_roll_id').val();
				show_list_view(batch_id+'_'+hidden_roll_id,'issue_show_fabric_desc_listview','list_fabric_desc_container','requires/aop_wash_controller','');
				$('#cbo_service_source').focus();
			}
	 ...
	 function check_issue_challan_scan(str) //Issue Challan Scan
	{
		...
		if(issue_chalan!="")
		{
			var response=return_global_ajax_value( cbo_company_id+"**"+issue_chalan, 'check_issue_challan_no_scan', '', 'requires/aop_wash_controller');
			var response=response.split("_");			
			if(response[0]==0)
			{
				alert('Issue Challan  not found.');
			...
			else
			{
				...
				get_php_form_data(response[1], "populate_data_from_data", "requires/aop_wash_controller" );
				var hidden_roll_id = $('#txt_roll_id').val();
				show_list_view(batch_id+'_'+hidden_roll_id+'_'+cbo_company_id,'issue_show_fabric_desc_listview','list_fabric_desc_container','requires/aop_wash_controller','');
				$('#cbo_service_source').focus();
			}
                    ...
                    <table cellpadding="0" cellspacing="2" width="100%" id="main_tbl">
                        ...
                        <tr>
                         <td class="must_entry_caption" width="130">Company</td>
                            <td>
                                <?
                                    //load_drop_down('requires/aop_wash_controller', this.value, 'load_drop_floor', 'floor_td' );
									echo create_drop_down( "cbo_company_id", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name desc","id,company_name",1, "--Select Company--",$selected, "roll_maintain();" );
                                ?>
                        ...
                        <td>
							<?
                                echo create_drop_down( "cbo_service_source", 135, $knitting_source, "", 1, "-- Select --", $selected, "load_drop_down( 'requires/aop_wash_controller',this.value+'**'+$('#cbo_company_id').val(),'load_drop_down_knitting_com','dyeing_company_td' );search_populate(this.value);","","1,3" );
                            ?>
                        </td>
                        ...
                        <tr>
                            ...
                            <td>
                               <?
								   echo create_drop_down( "cbo_sub_process", 135, $conversion_cost_head_array,"", 0, "-- Select --", 0, "","","193,209" );
                               ?>
