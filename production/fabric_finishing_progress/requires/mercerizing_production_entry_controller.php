...
if ($action=="load_drop_floor")
{
	...
	echo create_drop_down( "cbo_floor", 135, "select id,floor_name from lib_prod_floor where status_active=1 and is_deleted=0 and company_id='$data[0]' and production_process=4 order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected,"load_drop_down( 'requires/mercerizing_production_entry_controller', document.getElementById('cbo_service_company').value+'**'+this.value, 'load_drop_machine', 'machine_td' );" );     	 
	exit();
}
if($action=="load_drop_down_knitting_com")
{
	...
	if($data[0]==1)
	{
		echo create_drop_down( "cbo_service_company", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name",1, "-- Select --", 0, "load_drop_down('requires/mercerizing_production_entry_controller', this.value, 'load_drop_floor', 'floor_td' );","" );
	}
	else if($data[0]==3)
	{	
...
{ 	
	...
	{  	//if($row[csf('batch_against')])
		...
		//echo "load_drop_down( 'requires/mercerizing_production_entry_controller', '".$row[csf("company_id")]."', 'load_drop_floor', 'floor_td' );\n";
		...
		foreach($sql_batch as $r_batch)
		{
			...
			echo "load_drop_down( 'requires/mercerizing_production_entry_controller', document.getElementById('cbo_company_id').value+'**'+".$r_batch[csf("floor_id")].", 'load_drop_machine', 'machine_td' );\n";
...
if($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		if(str_replace("'","",$update_id)=="")
		{
			...
			$data_array="(".$id.",".$cbo_company_id.",".$txt_batch_no.",".$txt_batch_ID.",".$cbo_sub_process.",".$txt_process_end_date.",".$txt_process_date.",".$txt_temparature.",".$txt_stretch.",".$txt_feed.",".$txt_feed_in.",".$txt_pinning.",".$txt_speed.",".$txt_process_start_date.",".$txt_start_hours.",".$txt_start_minutes.",".$txt_end_hours.",".$txt_end_minutes.",".$cbo_machine_name.",".$cbo_floor.",618,".$cbo_shift_name.",".$txt_remarks.",".$txt_issue_chalan.",".$cbo_service_source.",".$cbo_service_company.",".$txt_issue_mst_id.",".$txt_booking_no.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		if(str_replace("'","",$roll_maintained)==1)
		{
			for($i=1;$i<=$total_row;$i++)
			{
				...
				$data_array_dtls.="(".$id_dtls.",".$mst_update_id.",618,".$$prod_id.",".$$txtconscomp.",".$$txtgsm.",".$$txtbodypart.",".$$txtdiawidthID.",".$$txtbatchqnty.",".$$txtroll.",".$$rollid.",".$$txtproductionqty.",".$$txtrate.",".$$txtamount.",".$hidden_currency.",".$hidden_exchange_rate.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				$id_dtls=$id_dtls+1;
				}
		...
		else
		{
			for($i=1;$i<=$total_row;$i++)
			{
				...
				if(str_replace("'","",$$checked_tr)==1)
				{
					...
					// $Itemprod_id=str_replace("'","",$$prod_id);
					if($data_array_dtls!="") $data_array_dtls.=","; 
					$data_array_dtls.="(".$id_dtls.",".$mst_update_id.",618,".$$prod_id.",".$$txtconscomp.",".$$txtgsm.",".$$txtbodypart.",".$$txtdiawidthID.",".$$txtbatchqnty.",".$$txtroll.",".$$txtproductionqty.",".$$txtrate.",".$$txtamount.",".$hidden_currency.",".$hidden_exchange_rate.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
					$id_dtls=$id_dtls+1;
					//print_r($data_array_dtls);die;
				}
			}
...
if($action=="populate_data_from_data")
{
	...
	{		
		...
		echo "load_drop_down( 'requires/mercerizing_production_entry_controller', ".$row[csf("dyeing_source")]."+'**'+".$row[csf("company_id")].", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
            ...
            <tbody>
            	<? foreach($result as $row){  
					...
					$roll_no=$row[csf("no_of_roll")];
								...
								?>
                	<tr bgcolor="<? echo $bgcolor; ?>" onClick='show_list_view("<? echo $row[csf("id")];?>","child_form_input_data","list_fabric_desc_container","requires/mercerizing_production_entry_controller");get_php_form_data("<? echo $row[csf("id")];?>","mst_id_child_form_input_data","requires/mercerizing_production_entry_controller")'  style="cursor:pointer" >
...
if($action=="child_form_input_data")
{
	...
	$sql_result=sql_select("select a.id,b.id as dtls_id,b.prod_id,b.const_composition,b.gsm,b.dia_width,b.width_dia_type,b.batch_qty,b.production_qty,b.no_of_roll,b.roll_no,b.roll_id,b.rate,b.amount from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.id=$data and a.entry_form=618 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1");
...
{
	$sql_result=sql_select("SELECT a.id,a.company_id,a.batch_id,a.service_source,a.service_company,a.received_chalan,a.issue_chalan,issue_challan_mst_id,a.process_end_date,a.production_date,a.process_start_date,a.process_id,a.end_hours,a.end_minutes,a.start_minutes,a.temparature,a.stretch,a.over_feed,a.feed_in,a.pinning,a.speed_min,a.start_hours,a.floor_id,a.machine_id,a.shift_name,a.remarks,a.booking_no,b.currency_id,b.exchange_rate from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.id=$data and entry_form=618  and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0");
  	...
  	$trims_weight=return_field_value("total_trims_weight","pro_batch_create_mst","company_id =".$sql_result[0][csf('company_id')]." and id=".$sql_result[0][csf('batch_id')]." and is_deleted=0 and status_active=1");
		...
		{
		echo "load_drop_down('requires/mercerizing_production_entry_controller', '".$sql_result[0][csf('service_company')]."', 'load_drop_floor', 'floor_td' );\n";
		}
		echo "load_drop_down( 'requires/mercerizing_production_entry_controller', document.getElementById('cbo_service_company').value+'**'+".$sql_result[0][csf("floor_id")].", 'load_drop_machine', 'machine_td' );\n";
		...
		echo "load_drop_down( 'requires/mercerizing_production_entry_controller', ".$sql_result[0][csf("service_source")]."+'**'+".$sql_result[0][csf("company_id")].", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
        ...
        <table  cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all" width="900">
            ...
            <tbody>
                <tr>
                    ...
                    <? 
                   		echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 and id=".$cbo_company_id." order by company_name","id,company_name",1, "-- Select Company --", '', "load_drop_down( 'mercerizing_production_entry_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
                    ?>
                    </td>
                    ...
                    <td align="center">
                    	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_search_category').value+'_'+document.getElementById('txt_booking_no').value+'_'+'<?php echo $supplier_id."_".$process_id; ?>', 'create_booking_search_list_view', 'search_div', 'mercerizing_production_entry_controller', 'setFilterGrid(\'table_body\',-1)')" style="width:100px;" />
                    </td>
                </tr>
                <tr>
