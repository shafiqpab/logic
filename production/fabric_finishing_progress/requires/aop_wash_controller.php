...
if ($action=="load_drop_floor")
{
	...
	echo create_drop_down( "cbo_floor", 135, "select id,floor_name from lib_prod_floor where status_active=1 and is_deleted=0 and company_id='$data[0]' and production_process=3 order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected,"load_drop_down( 'requires/aop_wash_controller', document.getElementById('cbo_service_company').value+'**'+this.value, 'load_drop_machine', 'machine_td' );" );     	 
	exit();
}
if($action=="load_drop_down_knitting_com")
{
	...
	if($data[0]==1)
	{
		echo create_drop_down( "cbo_service_company", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name",1, "-- Select --", 0, "load_drop_down('requires/aop_wash_controller', this.value, 'load_drop_floor', 'floor_td' );","" );
	}
	else if($data[0]==3)
	{	
...
if ($action=="load_drop_machine")
{
	...
	if($db_type==2)
	{
	echo create_drop_down( "cbo_machine_name", 135, "select id,machine_no || '-' || brand as machine_name from lib_machine_name where category_id in(2) and company_id=$com and floor_id=$floor and status_active=1 and is_deleted=0 and is_locked=0 order by machine_name","id,machine_name", 1, "-- Select Machine --", $selected, "get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+this.value, 'populate_data_from_machine', 'requires/aop_wash_controller' );","" );
	}
	else if($db_type==0)
	{
	echo create_drop_down( "cbo_machine_name", 135, "select id,concat(machine_no, '-', brand) as machine_name from lib_machine_name where category_id in(2) and company_id=$com and floor_id=$floor and status_active=1 and is_deleted=0 and is_locked=0 order by machine_name","id,machine_name", 1, "-- Select Machine --", $selected, "get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+this.value, 'populate_data_from_machine', 'requires/aop_wash_controller' );","" );
	}
	exit();
...
{ 
	...
	$ex_data=explode('**',$data);
	 ...
	 $sql_res="select id, floor_id, machine_group from lib_machine_name where id=$ex_data[2] and category_id in(2) and company_id=$ex_data[0] and  floor_id=$ex_data[1] and status_active=1 and is_deleted=0 ";
...
{ 	
	...
	a.color_id,a.color_range_id,a.insert_date,a.dur_req_hr,a.dur_req_min, a.booking_without_order, sum(b.batch_qnty) as batch_qnty, $select_list from pro_batch_create_mst a,pro_batch_create_dtls b where a.id=b.mst_id  and a.entry_form in(0,36,563) and a.id='$batch_id_ex' $select_group_row";
	}
	else
	{
	...
	a.color_id,a.color_range_id,a.insert_date, a.booking_without_order, sum(b.batch_qnty) as batch_qnty, $select_list from pro_batch_create_mst a,pro_batch_create_dtls b where a.id='$batch_id_ex' and a.entry_form in(0,36,563)  and a.id=b.mst_id  $pop_batch";	
	}
	//echo $sql_re;
	...
	{  	//if($row[csf('batch_against')])
		...
		//echo "load_drop_down( 'requires/aop_wash_controller', '".$row[csf("company_id")]."', 'load_drop_floor', 'floor_td' );\n";
		//echo "load_drop_down( 'requires/aop_wash_controller', '".$row[csf("company_id")]."', 'roll_maintained', 'roll_maintained' );\n";
		...
		foreach($sql_batch as $r_batch)
		{
			...
			//echo "load_drop_down( 'requires/aop_wash_controller', document.getElementById('cbo_company_id').value+'**'+".$r_batch[csf("floor_id")].", 'load_drop_machine', 'machine_td' );\n";
            ...
            <table cellpadding="0" cellspacing="0" width="850" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td align="center" id="search_by_td">				
                     ...
                     <td align="center">	
                    	<?
                       		 echo create_drop_down( "cbo_service_source", 135, $knitting_source, "", 1, "-- Select --", $selected, "load_drop_down( 'aop_wash_controller',this.value+'**'+$cbo_company_id,'load_drop_down_knitting_com','dyeing_company_td' );","","1,3" );
						?>
                    </td> 
            		...
            		<td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $cbo_company_id; ?>, 'create_challan_search_list_view', 'search_div', 'aop_wash_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                     </td>
                </tr>
                <tr>
...
if($action=="populate_data_from_data")
{
	...
	{		
		...
		echo "load_drop_down( 'requires/aop_wash_controller', ".$row[csf("dyeing_source")]."+'**'+".$row[csf("company_id")].", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
...
$fabric_desc_arr = array();
   ...
   //if((page_upto*1==1 || page_upto*1>1) && roll_maintained==1  ) // && roll_maintained==1 
	...
	{
		$i=1;	
		$sql_data=("SELECT a.entry_form,b.width_dia_type,a.company_id,b.item_description,b.gsm,b.fin_dia,b.width_dia_type,b.prod_id,b.roll_no,b.roll_id, sum(b.batch_qnty) as batch_qnty from pro_batch_create_dtls b,pro_batch_create_mst a where b.mst_id='$batch_id'  and a.id=b.mst_id and  a.entry_form in(0,36,563)  and b.status_active=1 and b.is_deleted=0   group by b.id,b.item_description,b.width_dia_type,b.gsm,b.fin_dia,b.prod_id,a.entry_form,b.roll_id,b.roll_no,a.company_id");
		//echo $sql_data;
		...
		if(count($sql_result)>0)
		{
			...
			{
				$fabric_roll_arr[$row[csf('id')]]['roll']=$row[csf('roll_no')];
				$fabric_roll_arr[$row[csf('id')]]['barcode_no']=$row[csf('barcode_no')];
			}
				...
				<tr class="general" id="row_<? echo $i; ?>">
	             <td style="float: left;" width="40" id="sl_<? echo $i; ?>"><input type="checkbox" id="checkRow_<? echo $i; ?>" name="checkRow[]"  checked> &nbsp; &nbsp;<? echo $i; ?></td>
	                ...
	                <td><input type="text" name="txtafterknitdensity_<? echo $i; ?>" id="txtafterknitdensity_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px;" value="<? echo $head_densityData_batch;  ?>"/></td> 
	                 ...
	                 <td><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes_numeric" value="<? echo $roll_no;?>" style="width:35px;"   /> <input type="hidden" name="rollid_<? echo $i; ?>" id="rollid_<? echo $i; ?>" style="width:50px;" value="<? echo $row[csf('roll_id')];?>" class="text_boxes_numeric" />	</td>
			...
			<tr class="general" id="row_<? echo $i; ?>">
	            <td width="60" id="sl_<? echo $i; ?>"><input type="checkbox" id="checkRow_<? echo $i; ?>" name="checkRow[]"  checked> &nbsp; &nbsp;<? echo $i; ?></td>
	                ...
	                <td><input type="text" name="txtafterknitdensity_<? echo $i; ?>" id="txtafterknitdensity_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px;" value="" /></td> 
	                 ...
	                 <td><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes_numeric" value="<? //echo $roll_no;?>" style="width:60px;"   /> <input type="hidden" name="rollid_<? echo $i; ?>" id="rollid_<? echo $i; ?>" style="width:35px;" value="<? //echo $row[csf('roll_id')];?>" class="text_boxes_numeric" />	</td>
	...
	else
	{
		...
		//{
		   ...
		   $data=("SELECT a.entry_form,b.width_dia_type,b.item_description,b.gsm,b.fin_dia,b.width_dia_type,b.prod_id,count(b.roll_no) as no_of_roll, sum(b.batch_qnty) as batch_qnty from pro_batch_create_mst a, pro_batch_create_dtls b 
		   	where b.mst_id='$batch_id'  and a.id=b.mst_id and a.entry_form in(0,36,563) and b.status_active=1 and b.is_deleted=0 group by a.entry_form,b.width_dia_type,b.gsm,b.fin_dia,b.item_description,b.width_dia_type,b.prod_id");
		...
		if(($page_upto_id==1 || $page_upto_id>1) && str_replace("'","",$roll_maintained)==1)
		{
			for($i=1;$i<=$total_row;$i++)
			{
				...
				if(str_replace("'","",$$checked_tr)==1)
				{
					...
					$data_array_dtls.="(".$id_dtls.",".$mst_update_id.",424,'".$Itemprod_id."',".$$txtconscomp.",".$$txtgsm.",".$$txtbodypart.",".$$txtdiawidthID.", ".$$txtbatchqnty.",".$$txtroll.",".$$rollid.",".$$txtproductionqty.",".$$txtrate.",".$$txtamount.",".$$txtafterknitdensity.",".$hidden_currency.",".$hidden_exchange_rate.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
					$id_dtls=$id_dtls+1;
				}
			}
		}
...
if($action=="show_dtls_list_view")
{
	...
	//echo $sql;
	$result = sql_select($sql);*/
	$sql_dtls = "select a.id,b.const_composition AS const_composition,b.gsm as gsm ,b.roll_no AS roll_no,b.dia_width as dia_width,b.width_dia_type as width_dia_type,b.roll_id as roll_id,(b.batch_qty) as batch_qty,(b.production_qty) as production_qty,(b.no_of_roll) as no_of_roll from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$data and a.entry_form=424 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1";
	...
	{
		$fab_dtls_arr[$row[csf("id")]]['batch_qty']+=$row[csf("batch_qty")];
		$fab_dtls_arr[$row[csf("id")]]['const_composition'].=$row[csf("const_composition")].',';
		$fab_dtls_arr[$row[csf("id")]]['roll_no'].=$row[csf("roll_no")].',';
		$fab_dtls_arr[$row[csf("id")]]['dia_width'].=$row[csf("dia_width")].',';
		$fab_dtls_arr[$row[csf("id")]]['roll_id'].=$row[csf("roll_id")].',';
		$fab_dtls_arr[$row[csf("id")]]['width_dia_type'].=$row[csf("width_dia_type")].',';
		$fab_dtls_arr[$row[csf("id")]]['no_of_roll']+=$row[csf("no_of_roll")];
		$fab_dtls_arr[$row[csf("id")]]['production_qty']+=$row[csf("production_qty")];

		
	}
	//print_r($fab_dtls_arr);
	$i=1;
	$total_batch_qty=0;
	$total_prod_qty=0;
	?> 
        ...
        <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" width="800" rules="all">
            ...
            <tbody>
				...
				$roll_maintained=return_field_value("fabric_roll_level","variable_settings_production","company_name ='$company_id' and item_category_id=50 and variable_list=3 and is_deleted=0 and status_active=1");
						$page_upto_id=return_field_value("page_upto_id","variable_settings_production","company_name ='$company_id' and item_category_id=50 and variable_list=3 and is_deleted=0 and status_active=1");
            	$checkBatch = array();
            	foreach($fab_dtls_arr as $mst_id=>$row){  
					...
					$roll_no=$row[("no_of_roll")];
								...
								?>
                	<tr bgcolor="<? echo $bgcolor; ?>" onClick='show_list_view("<? echo $mst_id;?>","child_form_input_data","list_fabric_desc_container","requires/aop_wash_controller");get_php_form_data("<? echo $mst_id;?>","mst_id_child_form_input_data","requires/aop_wash_controller")'  style="cursor:pointer" >
	...
	//echo $company_id;
		...
		foreach($sql_result as $row)
		{
			...
			<tr class="general" id="row_<? echo $i; ?>">
                ...
                <td><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes_numeric" value="<? echo $roll_no;?>" style="width:35px;"    /> <input type="hidden" name="rollid_<? echo $i; ?>" id="rollid_<? echo $i; ?>" style="width:35px;" value="<? echo $roll_id;?>" class="text_boxes_numeric" />	</td>
...
{
	//print($data);
	$sql_result=sql_select("SELECT a.chemical_name, a.id,a.company_id,a.batch_id,a.service_source,a.service_company,a.received_chalan,a.issue_chalan,issue_challan_mst_id,a.process_end_date, a.production_date,a.process_start_date,a.process_id,a.end_hours,a.end_minutes,a.start_hours,a.start_minutes,a.temparature,a.stretch, a.over_feed, a.feed_in,a.pinning,a.speed_min,a.floor_id,a.machine_id,a.result,a.shift_name,a.remarks,a.booking_no,b.currency_id,b.exchange_rate from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.id=$data and entry_form=424  and a.status_active=1 and a.is_deleted=0 group by a.chemical_name, a.id, a.service_source, a.service_company,a.received_chalan, a.issue_chalan, issue_challan_mst_id, a.process_end_date, a.production_date, a.process_start_date ,a.process_id, a.end_hours,a.end_minutes, a.start_hours, a.temparature,a.stretch,a.over_feed,a.feed_in,a.pinning,a.speed_min,a.floor_id,a.machine_id,a.result, a.company_id,a.batch_id, a.shift_name, a.remarks,a.start_minutes,a.booking_no,b.currency_id,b.exchange_rate");
  ...
  $trims_weight=return_field_value("total_trims_weight","pro_batch_create_mst","company_id =".$sql_result[0][csf('company_id')]." and id=".$sql_result[0][csf('batch_id')]." and is_deleted=0 and status_active=1");
		...
		//echo "document.getElementById('cbo_service_source').value	= ".$sql_result[0][csf('service_source')].";\n";
			...
			if($service_source==1)
			{
			echo "load_drop_down( 'requires/aop_wash_controller', '".$sql_result[0][csf("service_company")]."', 'load_drop_floor', 'floor_td' );\n";
			...
			echo "load_drop_down( 'requires/aop_wash_controller', document.getElementById('cbo_service_company').value+'**'+".$sql_result[0][csf("floor_id")].", 'load_drop_machine', 'machine_td' );\n";
			echo "$('#cbo_service_source').val(".$sql_result[0][csf("service_source")].");\n";
			echo "load_drop_down( 'requires/aop_wash_controller', ".$sql_result[0][csf("service_source")]."+'**'+".$sql_result[0][csf("company_id")].", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
			...
			echo "load_drop_down( 'requires/aop_wash_controller', ".$sql_result[0][csf("service_company")]."+'**'+".$sql_result[0][csf("floor_id")].", 'load_drop_machine', 'machine_td' );\n";
        ...
        <table  cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all" width="900">
            ...
            <tbody>
                <tr>
                    ...
                    <? 
                   		echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 and id=".$cbo_company_id." order by company_name","id,company_name",1, "-- Select Company --", '', "load_drop_down( 'aop_wash_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
                    ?>
                    </td>
                    ...
                    <td align="center">
                    	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_search_category').value+'_'+document.getElementById('txt_booking_no').value+'_'+document.getElementById('txt_job_no').value+'_'+'<?php echo $supplier_id."_".$process_id; ?>', 'create_booking_search_list_view', 'search_div', 'aop_wash_controller', 'setFilterGrid(\'table_body\',-1)')" style="width:100px;" />
                    </td>
                </tr>
                <tr>
