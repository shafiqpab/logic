...
if ($action=="load_drop_floor")
{
	...
	echo create_drop_down( "cbo_floor", 135, "select id,floor_name from lib_prod_floor where status_active=1 and is_deleted=0 and company_id='$data[0]' and production_process=4  order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected,"load_drop_down( 'requires/de_oiling_controller', document.getElementById('cbo_service_company').value+'**'+this.value, 'load_drop_machine', 'machine_td' );" );     	 
	exit();
}
...
if($action=="load_drop_down_knitting_com")
{
	...
	if($data[0]==1)
	{
		echo create_drop_down( "cbo_service_company", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name",1, "-- Select --", 0, "load_drop_down('requires/dry_slitting_controller', this.value, 'load_drop_floor', 'floor_td' );","" );
	}
	else if($data[0]==3)
	{	
...
{ 
	...
	{ 
		...
		//echo "load_drop_down( 'requires/dry_slitting_controller', '".$row[csf("company_id")]."', 'load_drop_floor', 'floor_td' );\n";
		...
		echo "document.getElementById('cbo_process_id').value 			= '".$row[csf("process_id_batch")]."';\n";
...
process_start_date,start_hours,start_minutes,end_hours,end_minutes,machine_id,floor_id,process_id,production_date,shift_name,remarks from pro_fab_subprocess where entry_form=310 and batch_id='".$row[csf("id")]."' ");
		...
		foreach($sql_batch as $r_batch)
		{
			...
			echo "document.getElementById('cbo_process_id').value 				= '".$r_batch[csf("process_id")]."';\n";
			...
			echo "load_drop_down( 'requires/dry_slitting_controller', document.getElementById('cbo_company_id').value+'**'+".$r_batch[csf("floor_id")].", 'load_drop_machine', 'machine_td' );\n";
...
/*	$prodData=sql_select("select id,detarmination_id, item_description, gsm, dia_width, product_name_details from product_details_master where item_category_id=13");
	...
	$sql_result=sql_select("SELECT a.entry_form,b.width_dia_type,b.item_description, sum(b.batch_qnty) as batch_qty,b.prod_id,b.barcode_no,b.roll_id,b.roll_no from pro_batch_create_dtls b,pro_batch_create_mst a  where b.mst_id='$batch_id'   and a.id=b.mst_id and a.id='$batch_id' and  a.entry_form in(0,36) and b.status_active=1 and b.is_deleted=0  group by b.id,b.item_description,b.width_dia_type,b.prod_id,a.entry_form,b.roll_id,b.barcode_no,b.roll_no");
	$i=1;
	$b_qty=0;
	//if($roll_maintained==1)
	if(($page_upto_id==7 || $page_upto_id>7) && $roll_maintained==1)
	...
	?>
    	<tr class="general" id="row_<? echo $i; ?>">
            ...
            <td title="<? echo $cons_comps; ?>"><input type="text" name="txtconscomp_<? echo $i; ?>" id="txtconscomp_<? echo $i; ?>" class="text_boxes" style="width:200px;" value="<? echo $cons_comps; ?>" disabled/></td>
	...
	else
	{
     ...
     <tr>
         <td colspan="2" align="left"><b style="float:left"><input type="checkbox" name="checkbox_id" id="checkbox_id" onClick="fnc_check()" style="width:50px;" class="text_boxes" readonly />Check/Uncheck:</b> </td>
         <td colspan="4" align="right"><b>Sum:</b> <? //echo $b_qty; ?> </td>
        ...
        <td align="right"><input type="text" name="total_production_qnty" id="total_production_qnty" class="text_boxes_numeric" style="width:60px" value="<? echo number_format($prod_qty,2); ?>"  readonly /></td>
        <td align="right"></td>           
...
{ 
	$sql_result=sql_select("select b.id, b.gsm,b.width_dia_type,b.dia_width,b.const_composition,b.batch_qty,b.production_qty,b.no_of_roll,b.roll_no, b.prod_id,b.rate,b.amount from pro_fab_subprocess_dtls b,pro_fab_subprocess a where a.id=b.mst_id and a.batch_id='$batch_id' and b.entry_page=323 and a.status_active=1 and a.is_deleted=0");
	//echo "select b.id, b.gsm,b.width_dia_type,b.dia_width,b.const_composition,b.batch_qty,b.production_qty,b.no_of_roll,b.roll_no, b.prod_id,b.rate,b.amount from pro_fab_subprocess_dtls b,pro_fab_subprocess a where a.id=b.mst_id and a.batch_id='$batch_id' and b.entry_page=34 and a.status_active=1 and a.is_deleted=0";
	$i=1;
	$b_qty=0;
	if(count($sql_result)>0)
	{
		...
		?>
			<tr class="general" id="row_<? echo $i; ?>">
				<td><? echo $i; ?></td>
				<td title="Desc=<? echo $cons_comps; ?>"><input type="text" name="txtconscomp_<? echo $i; ?>" id="txtconscomp_<? echo $i; ?>" class="text_boxes" style="width:200px;" value="<? echo $cons_comps//$row[csf('const_composition')]; ?>" disabled/></td>
				...
				<td><input type="text" name="txtdiawidth_<? echo $i; ?>" id="txtdiawidth_<? echo $i; ?>" class="text_boxes" style="width:70px;" value="<? echo $fabric_typee[$row[csf('width_dia_type')]];?>" disabled/>
				 ...
				 <td><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes" style="width:60px;" value="<? echo $row[csf('no_of_roll')]; ?>"/>
				 <input type="hidden" name="txtbarcode_<? echo $i; ?>" id="txtbarcode_<? echo $i; ?>" class="text_boxes" style="width:65px;" value="<? echo $barcode; ?>"/>
				 </td>
				<td><input type="text" name="txtbatchqnty_<? echo $i; ?>" id="txtbatchqnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo $row[csf('batch_qty')]; ?>" disabled/>
                <input type="hidden" name="txtdeterid_<? echo $i; ?>" id="txtdeterid_<? echo $i; ?>"  value="<? echo $fabric_desc_arr[$row[csf('prod_id')]]['detarmination_id'];?>" />
		...
		<?
			/*if(!in_array($row[csf('no_of_roll')], $checkBatchRoll))
			{
				$b_qty+= $row[csf('batch_qty')];
				$checkBatchRoll[] = $row[csf('no_of_roll')];
			}*/
			$b_qty+= $row[csf('batch_qty')];
	...
	{
		 $result=sql_select("select a.entry_form,b.width_dia_type,b.item_description, sum(b.batch_qnty) as batch_qty,b.prod_id,count(b.roll_no ) as roll_no from pro_batch_create_dtls b,pro_batch_create_mst a  where b.mst_id='$batch_id'  and a.id=b.mst_id and  a.entry_form in(0,36) and b.status_active=1 and b.is_deleted=0 group by a.entry_form,b.width_dia_type,b.item_description,b.prod_id");
	...
	?>
    	<tr class="general" id="row_<? echo $i; ?>">
         	<td><? echo $i; ?></td>
            <td title="Desc=<? echo $cons_comps; ?>"><input type="text" name="txtconscomp_<? echo $i; ?>" id="txtconscomp_<? echo $i; ?>" class="text_boxes" style="width:200px;" value="<? echo $cons_comps; ?>" disabled/></td>
            <td><input type="text" name="txtgsm_<? echo $i; ?>" id="txtgsm_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px;" value="<? echo $gsm; ?>" /></td>
            <td><input type="text" name="txtbodypart_<? echo $i; ?>" id="txtbodypart_<? echo $i; ?>" class="text_boxes" style="width:50px;" value="<? echo  $dia_width; //$row[csf('width_dia_type')]; ?>" disabled/></td>
            <td><input type="text" name="txtdiawidth_<? echo $i; ?>" id="txtdiawidth_<? echo $i; ?>" class="text_boxes" style="width:70px;" value="<? echo $fabric_typee[$row[csf('width_dia_type')]];?>" disabled/>
            <input type="hidden" name="txtdiawidthID_<? echo $i; ?>" id="txtdiawidthID_<? echo $i; ?>" value="<? echo $row[csf('width_dia_type')];?>" readonly />
...
if($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		{
		$field_array="id,company_id,batch_no,batch_id,batch_ext_no,process_id,process_end_date,production_date,process_start_date,start_hours,start_minutes,end_hours,end_minutes,machine_id,floor_id,entry_form,shift_name,remarks,issue_chalan,service_source,service_company,received_chalan,issue_challan_mst_id,booking_no,inserted_by,insert_date";
		$id=return_next_id( "id", " pro_fab_subprocess", 1 ) ;
		$data_array="(".$id.",".$cbo_company_id.",".$txt_batch_no.",".$txt_batch_ID.",".$txt_ext_id.",".$cbo_process_id.",".$txt_process_end_date.",".$txt_process_date.",".$txt_process_start_date.",".$txt_start_hours.",".$txt_start_minutes.",".$txt_end_hours.",".$txt_end_minutes.",".$cbo_machine_name.",".$cbo_floor.",323,".$cbo_shift_name.",".$txt_remarks.",".$txt_issue_chalan.",".$cbo_service_source.",".$cbo_service_company.",".$txt_recevied_chalan.",".$txt_issue_mst_id.",".$txt_booking_no.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		if(($page_upto_id==7 || $page_upto_id>7) && str_replace("'","",$roll_maintained)==1)
		{
			...
			for($i=1;$i<=$total_row;$i++)
			{
				...
				if(str_replace("'","",$$checked_tr)==1)
				{
					...
					$data_array_dtls.="(".$id_dtls.",".$mst_update_id.",323,".$Itemprod_id.",".$$txtconscomp.",".$$txtgsm.",".$$txtbodypart.",".$$txtdiawidthID.",".$$txtbatchqnty.",".$$txtroll.",".$$txtbarcode.",".$$rollid.",".$$txtproductionqty.",".$$txtrate.",".$$txtamount.",".$hidden_currency.",".$hidden_exchange_rate.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
					$id_dtls=$id_dtls+1;
		...
		else
		{
			...
			for($i=1;$i<=$total_row;$i++)
			{
				...
				$Itemprod_id=str_replace("'","",$$prod_id);
				if($data_array_dtls!="") $data_array_dtls.=","; 
				$data_array_dtls.="(".$id_dtls.",".$mst_update_id.",323,".$Itemprod_id.",".$$txtconscomp.",".$$txtgsm.",".$$txtbodypart.",".$$txtdiawidthID.",".$$txtbatchqnty.",".$$txtroll.",".$$txtproductionqty.",".$$txtrate.",".$$txtamount.",".$hidden_currency.",".$hidden_exchange_rate.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				$id_dtls=$id_dtls+1;
				//print_r($data_array_dtls);die;
			}	
		}
	...
	{ 
		...
		$field_array_update="company_id*batch_no*service_source*service_company*received_chalan*issue_chalan*issue_challan_mst_id*batch_id*batch_ext_no* process_id*process_end_date*production_date*process_start_date*start_hours*start_minutes*end_hours*end_minutes*machine_id*floor_id*shift_name*remarks*booking_no*updated_by*update_date";
		$data_array_update="".$cbo_company_id."*".$txt_batch_no."*".$cbo_service_source."*".$cbo_service_company."*".$txt_recevied_chalan."*".$txt_issue_chalan."*".$txt_issue_mst_id."*".$txt_batch_ID."*".$txt_ext_id."*".$cbo_process_id."*".$txt_process_end_date."*".$txt_process_date."*".$txt_process_start_date."*".$txt_start_hours."*".$txt_start_minutes."*".$txt_end_hours."*".$txt_end_minutes."*".$cbo_machine_name."*".$cbo_floor."*".$cbo_shift_name."*".$txt_remarks."*".$txt_booking_no."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
...
if($action=="populate_data_from_data")
{
	...
	{		
		...
		echo "load_drop_down( 'requires/dry_slitting_controller', ".$row[csf("dyeing_source")]."+'**'+".$row[csf("company_id")].", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
            ...
            <table cellpadding="0" cellspacing="0" width="850" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td align="center" id="search_by_td">				
                     ...
                     <td align="center">	
                    	<?
                       		 echo create_drop_down( "cbo_service_source", 135, $knitting_source, "", 1, "-- Select --", $selected, "load_drop_down( 'dry_slitting_controller',this.value+'**'+$cbo_company_id,'load_drop_down_knitting_com','dyeing_company_td' );","","1,3" );
						?>
                    </td> 
            		...
            		<td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $cbo_company_id; ?>+'_'+document.getElementById('cbo_service_source').value+'_'+document.getElementById('cbo_service_company').value, 'create_challan_search_list_view', 'search_div', 'dry_slitting_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                     </td>
                </tr>
                <tr>
...
if($action=="show_dtls_list_view")
{
	...
	if($db_type==2)
	{
		$sql = "SELECT a.id,a.re_stenter_no,b.const_composition AS const_composition,b.gsm as gsm ,b.roll_no AS roll_no,b.dia_width as dia_width,b.width_dia_type as width_dia_type,b.roll_id as roll_id,(b.batch_qty) as batch_qty,(b.production_qty) as production_qty,(b.no_of_roll) as no_of_roll from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$batch_id and a.entry_form=323 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $re_stener_cond ";
	}
	else
	{
		 $sql = "SELECT a.id,a.re_stenter_no,group_concat(b.const_composition) AS const_composition,group_concat(b.gsm) as gsm,group_concat(b.roll_no) as roll_no ,group_concat(b.dia_width)  as dia_width,group_concat(b.width_dia_type)  as width_dia_type,group_concat(b.roll_id) as roll_id,sum(b.batch_qty) as batch_qty,sum(b.production_qty) as production_qty,sum(b.no_of_roll) as no_of_roll from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$batch_id and a.entry_form=323 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1  group by a.id,a.re_stenter_no";	
	}
	// echo $sql;die();
	$dtls_arr = array();
	$check_arr = array();
            ...
            <tbody>
            	...
            	{ 
	            	...
	            	{ 					
	                	...
	                	<tr bgcolor="<? echo $bgcolor; ?>" onClick='show_list_view("<? echo $row["id"];?>","child_form_input_data","list_fabric_desc_container","requires/dry_slitting_controller");get_php_form_data("<? echo $row["id"];?>","mst_id_child_form_input_data","requires/dry_slitting_controller");'  style="cursor:pointer" >
	                        ...
	                        <td align="right" width="80"><p><? echo $row["batch_qty"]; ?></p></td>
	                        <td align="right" width="80"><p><? echo $row["production_qty"]; ?></p></td>
	                   </tr>
	                	<? 
	                	$i++;
	                } 
            	...
            	<tfoot>
                    ...
                    <th><? echo $total_batch_qty; ?></th><th><? echo $total_prod_qty; ?></th>
                </tfoot>
            </tbody>
        </table>
    <?
...
if($action=="show_dtls_list_viewss")
{
	...
	if($db_type==2)
	{
		$sql = "SELECT a.id,a.batch_id,listagg(cast(b.const_composition as varchar2(4000)),',') within group (order by b.const_composition) AS const_composition,listagg(b.gsm,',') within group (order by b.gsm ) as gsm ,listagg(cast(b.roll_no as varchar2(4000)),',') within group (order by b.roll_no) AS roll_no,listagg(b.dia_width,',') within group (order by b.dia_width ) as dia_width,listagg(b.width_dia_type,',') within group (order by b.width_dia_type ) as width_dia_type,listagg(b.roll_id,',') within group (order by b.roll_id ) as roll_id,sum(b.batch_qty) as batch_qty,sum(b.production_qty) as production_qty,sum(b.no_of_roll) as no_of_roll from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$data and a.entry_form=310 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1  group by a.id,a.batch_id";
	}
	else
	{
		$sql = "SELECT a.id,a.batch_id,group_concat(b.const_composition) AS const_composition,group_concat(b.gsm) as gsm,group_concat(b.roll_no) as roll_no ,group_concat(b.dia_width)  as dia_width,group_concat(b.width_dia_type)  as width_dia_type,group_concat(b.roll_id) as roll_id,sum(b.batch_qty) as batch_qty,sum(b.production_qty) as production_qty,sum(b.no_of_roll) as no_of_roll from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$data and a.entry_form=310 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1  group by a.id";	
	}
	$result = sql_select($sql);
	$i=1;
	$total_batch_qty=0;
            ...
            <tbody>
            	...
            	foreach($result as $row){  
					...
					$roll_no=$row[csf("no_of_roll")];
								...
								?>
                	<tr bgcolor="<? echo $bgcolor; ?>" onClick='show_list_view("<? echo $row[csf("id")];?>","child_form_input_data","list_fabric_desc_container","requires/dry_slitting_controller");get_php_form_data("<? echo $row[csf("id")];?>","mst_id_child_form_input_data","requires/dry_slitting_controller")'  style="cursor:pointer" >
                        <td width="30"><? echo $i; ?></td>
...
if($action=="child_form_input_data")
{
	...
	$sql_result=sql_select("select a.id,b.id as dtls_id,b.prod_id,b.const_composition,b.gsm,b.dia_width,b.width_dia_type,b.batch_qty,b.production_qty,b.no_of_roll,b.barcode_no,b.roll_no,b.roll_id,b.rate,b.amount from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.id=$data and a.entry_form=323 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1");
	//if(count($sql_result)>0)
	///{
	$roll_maintained=return_field_value("fabric_roll_level","variable_settings_production","company_name ='$company_id' and item_category_id=50 and variable_list=3 and is_deleted=0 and status_active=1");
	$page_upto_id=return_field_value("page_upto_id","variable_settings_production","company_name =$company_id and item_category_id=50 and variable_list=3 and is_deleted=0 and status_active=1");
		...
		$fabric_desc_arr=array();
		$prodData=sql_select("select id,detarmination_id from product_details_master where item_category_id=13 and id in(".implode(',',$prod_idArr).")");
		foreach($prodData as $row)
		{
		$fabric_desc_arr[$row[csf('id')]]['detarmination_id']=$row[csf('detarmination_id')];
		}
	  ...
	  <tr>
        ...
        <td align="right"><input type="text" name="total_production_qnty" id="total_production_qnty" class="text_boxes_numeric" style="width:50px" value="<? echo $tot_production_qty; ?>" readonly  /></td>
        <td align="right"></td>           
...
{
	  $sql_result=sql_select("SELECT a.id,a.batch_id,a.company_id,a.service_source,a.service_company,a.received_chalan,a.issue_chalan,issue_challan_mst_id,a.process_end_date,a.production_date,a.process_start_date,a.process_id,a.end_hours,a.end_minutes,a.start_minutes,a.start_hours,a.floor_id,a.machine_id,a.shift_name,a.remarks,a.booking_no,b.currency_id,b.exchange_rate from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.id=$data and entry_form=323 and a.status_active=1 and a.is_deleted=0 ");
...
//echo  $sql_batch=("select id,batch_no,width_dia_type,process_end_date,process_start_date,start_hours,start_minutes,end_hours,end_minutes,machine_id,floor_id,process_id,temparature,stretch,over_feed,feed_in,pinning,speed_min,chemical_name,production_date,shift_name,remarks from pro_fab_subprocess where entry_form=31 and batch_id='$data' ");
 //echo  $sql_result[0][csf("process_id")];die;
			...
			*/
			...
			echo "document.getElementById('cbo_process_id').value 				= '".$sql_result[0][csf("process_id")]."';\n";
			...
			{
			echo "load_drop_down('requires/dry_slitting_controller', '".$sql_result[0][csf('service_company')]."', 'load_drop_floor', 'floor_td' );\n";
			}
		
			echo "load_drop_down( 'requires/dry_slitting_controller', document.getElementById('cbo_service_company').value+'**'+".$sql_result[0][csf("floor_id")].", 'load_drop_machine', 'machine_td' );\n";
			...
			echo "load_drop_down( 'requires/dry_slitting_controller', ".$sql_result[0][csf("service_source")]."+'**'+".$sql_result[0][csf("company_id")].", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
        ...
        <table  cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all" width="900">
            ...
            <tbody>
                <tr>
                    ...
                    <td align="center">
                    	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_search_category').value+'_'+document.getElementById('txt_booking_no').value+'_'+'<?php echo $supplier_id."_".$process_id; ?>', 'create_booking_search_list_view', 'search_div', 'dry_slitting_controller', 'setFilterGrid(\'table_body\',-1)')" style="width:100px;" />
                    </td>
                </tr>
                <tr>
