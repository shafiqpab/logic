...
if( $_SESSION['logic_erp']['user_id'] == "" ) header("location:login.php");
	...
	if($action=="company_wise_report_button_setting")
	{
		...
		$print_report_format=return_field_value("format_id"," lib_report_template","template_name ='".$data."'  and module_id=7 and report_id=253 and is_deleted=0 and status_active=1");
		$print_report_format_arr=explode(",",$print_report_format);

		echo "$('#print_report').hide();\n";
		echo "$('#print_report2').hide();\n";
		echo "$('#print_report3').hide();\n";
		echo "$('#print_multiple_subcon').hide();\n";

		if($print_report_format != "")
		{
			foreach($print_report_format_arr as $id)
			{
				if($id==115){echo "$('#print_report').show();\n";}
				if($id==116){echo "$('#print_report2').show();\n";}
				if($id==136){echo "$('#print_report3').show();\n";}
				if($id==859){echo "$('#print_multiple_subcon').show();\n";}
			}
		}
		else
		{
			echo "$('#print_report').show();\n";
			echo "$('#print_report2').show();\n";
			echo "$('#print_report3').show();\n";
			echo "$('#print_multiple_subcon').show();\n";
		}
	...
	if($action=="load_drop_down_knitting_com_new")
	{
		...
		{
			$sql="SELECT DISTINCT a.id, a.supplier_name FROM lib_supplier a, lib_supplier_party_type b, lib_supplier_tag_company c  WHERE   a.id = b.supplier_id AND b.supplier_id = c.supplier_id AND a.status_active = 1   GROUP BY a.id, a.supplier_name UNION ALL  SELECT DISTINCT c.id, c.supplier_name FROM lib_supplier_tag_company a, lib_supplier_party_type b, lib_supplier  c, inv_receive_mas_batchroll  d  WHERE   c.id = b.supplier_id AND a.supplier_id = b.supplier_id AND c.id = d.dyeing_source	 AND c.status_active IN (1, 3) AND c.is_deleted = 0 GROUP BY c.id, c.supplier_name ORDER BY supplier_name";
			//echo $sql ;
			// $sql = "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and a.status_active=1 group by a.id,a.supplier_name order by a.supplier_name";

			echo create_drop_down( "cbo_service_company", 152, "$sql","id,supplier_name", 1, "-- Select --", 0, "fnc_reset_form(2)" );
		}
		else
		{
			echo create_drop_down( "cbo_service_company", 152, $blank_array,"",1, "-- Select --", 0, "fnc_reset_form(2)" );
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$field_array_dtls="id, mst_id, batch_id, prod_id, body_part_id, febric_description_id, gsm, width, color_id, width_dia_type, process_id, batch_wgt, roll_no, batch_issue_qty, buyer_id, job_no, order_id, fin_dia, fin_gsm, remarks,
		outbound_batchname,booking_no,rate,booking_date,booking_without_order,booking_dtls_id, color_type_id, is_sales, inserted_by,insert_date";
		$all_detailsId='';
		for($j=1;$j<=$tot_row;$j++)
		{
			...
			if($$issueQty!="")
			{
				...
				$data_array_dtls.="(".$dtls_id.",".$id.",'".$$batchId."','".$$productId."','".$$bodyparyId."','".$$deterId."','".$$gsm."','".$$dia."','".$$colorId."','".$$widthTypeId."','".$$process_id."','".$$batchWgt."','".$$txtRollNo."','".$$issueQty."','".$$buyerId."','".$$jobNo."','".$$orderId."','".$$finDia."','".$$finGsm."','".$$remarks."','".$$outBoundBatchNo."','".$$bookingNo."','".$$woRate."','".$booking_date."','".$booking_without_order."','".$$bookingDtls."','".$$colorTypeId."','".$$isSales."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
	...
	else if ($operation==1)   // Update Here
	{
		...
		//$dtls_id=return_next_id("id", "pro_grey_batch_dtls", 1);
		$field_array_dtls="id, mst_id, batch_id, prod_id, body_part_id, febric_description_id, gsm, width, color_id, width_dia_type, process_id, batch_wgt, roll_no, batch_issue_qty, buyer_id, job_no, order_id, fin_dia, fin_gsm, remarks,
		outbound_batchname,booking_no,rate,booking_date,booking_without_order,booking_dtls_id, color_type_id, inserted_by, insert_date";
		...
		for($j=1;$j<=$tot_row;$j++)
		{
			...
			$bookingNo="bookingNo_".$j;
			$bookingDtlsID="bookingDtls_".$j;

			$bookingDtlsID=str_replace("'","",$$bookingDtlsid);
			$booking_dtls_id_arr[$bookingDtlsID]=$bookingDtlsID;
		}

		// issue
		$sql_prev="select a.recv_number,b.batch_issue_qty as batch_issue_qty, b.booking_no, b.color_id, b.body_part_id, b.febric_description_id from inv_receive_mas_batchroll a,pro_grey_batch_dtls b where a.id=b.mst_id and a.entry_form=91 and a.status_active=1 and b.status_active=1 and b.booking_dtls_id in (". implode(',',$booking_dtls_id_arr) .") " ;
		//echo "10**".$sql_prev; die;
		$Prev_nameArray=sql_select( $sql_prev );
		$recv_number_array=array();$prev_tot_batch_issue_qty_arr=array();
		foreach($Prev_nameArray as $row)
		{
			$prev_tot_batch_issue_qty_arr[$row[csf('booking_no')]][$row[csf('color_id')]][$row[csf('body_part_id')]][$row[csf('febric_description_id')]]+=$row[csf('batch_issue_qty')];
			$recv_number_array[$row[csf('booking_no')]][$row[csf('color_id')]][$row[csf('body_part_id')]][$row[csf('febric_description_id')]][$row[csf('recv_number')]]=$row[csf('recv_number')];
		}

		// receive + issue return
		$sql_prev_recv="SELECT a.recv_number,b.batch_issue_qty as batch_issue_qty, b.booking_no, b.color_id, b.body_part_id, b.febric_description_id, a.entry_form from inv_receive_mas_batchroll a,pro_grey_batch_dtls b where a.id=b.mst_id and a.entry_form in (92,554) and a.status_active=1 and b.status_active=1 and b.booking_dtls_id in (". implode(',',$booking_dtls_id_arr) .") " ;
		$Prev_recv_nameArray=sql_select( $sql_prev_recv );
		$prev_tot_batch_recv_qty_arr=array();$recv_number_mrr_array=array();
		foreach($Prev_recv_nameArray as $row)
		{
			$recv_number_mrr_array[$row[csf('booking_no')]][$row[csf('color_id')]][$row[csf('body_part_id')]][$row[csf('febric_description_id')]][$row[csf('recv_number')]]=$row[csf('recv_number')];

			if($row[csf('entry_form')]==554)
			{
				//( issue return )
				$prev_tot_batch_issue_return_qty_arr[$row[csf('booking_no')]][$row[csf('color_id')]][$row[csf('body_part_id')]][$row[csf('febric_description_id')]]+=$row[csf('batch_issue_qty')];
			}
			else
			{
				//( receive  )
				$prev_tot_batch_recv_qty_arr[$row[csf('booking_no')]][$row[csf('color_id')]][$row[csf('body_part_id')]][$row[csf('febric_description_id')]]+=$row[csf('batch_issue_qty')];
			}
		}
		...
		for($j=1;$j<=$tot_row;$j++)
		{
			...
			$txtRollNo="txtRollNo_".$j;
			$issueQty="txtIssueQty_".$j;
			$hdnIssueQnty="hiddenTxtIssueQty_".$j;
			$remarks="txtRemarks_".$j;
			$widthTypeId="widthTypeId_".$j;
			...
			if($$issueQty!="")
			{
				...
				$curr_hiddenIssueQty=str_replace("'","",$$hdnIssueQnty);
				...
				//$prev_tot_batch_issue_return_qty_arr[$row[csf('booking_no')]][$row[csf('color_id')]][$row[csf('body_part_id')]][$row[csf('febric_description_id')]]

				$prev_tot_batch_issue_qty = $prev_tot_batch_issue_qty_arr[$$bookingNo][$color_Id][$bodypary_Id][$deter_Id] - $prev_tot_batch_issue_return_qty_arr[$$bookingNo][$color_Id][$bodypary_Id][$deter_Id];
				if(!empty($recv_number_array[$$bookingNo][$color_Id][$bodypary_Id][$deter_Id]))
				{
					$recv_numbers = implode(',',$recv_number_array[$$bookingNo][$color_Id][$bodypary_Id][$deter_Id]);
					$recv_number_arr[$recv_numbers]=$recv_numbers;
				}
				...
				/* $sql_prev_recv="select a.recv_number,b.batch_issue_qty as batch_issue_qty from inv_receive_mas_batchroll a,pro_grey_batch_dtls b where a.id=b.mst_id and a.entry_form=92 and a.status_active=1 and b.status_active=1 and b.booking_no='".$$bookingNo."' and b.booking_dtls_id=".$bookingDtlsId." and b.color_id='".$color_Id."' and body_part_id=".$bodypary_Id." and febric_description_id=".$deter_Id." " ;
				$Prev_recv_nameArray=sql_select( $sql_prev_recv );
				$prev_tot_batch_recv_qty=0;$recv_number_mrr_arr=array();
				foreach($Prev_recv_nameArray as $row)
				{
					$prev_tot_batch_recv_qty+=$row[csf('batch_issue_qty')];
					$recv_number_mrr_arr[$row[csf('recv_number')]]=$row[csf('recv_number')];
				} */
				//echo "10**$prev_tot_batch_recv_qty"."===".$curr_issueQty; die;
				//if(($prev_tot_batch_recv_qty>$curr_issueQty)){
				//echo "10**".$prev_tot_batch_issue_qty."+".$curr_issueQty."-".$curr_hiddenIssueQty;

				$prev_tot_batch_recv_qty=0;$recv_number_mrr_arr=array();
				$prev_tot_batch_recv_qty= $prev_tot_batch_recv_qty_arr[$$bookingNo][$color_Id][$bodypary_Id][$deter_Id];
				if(!empty($recv_number_mrr_array[$$bookingNo][$color_Id][$bodypary_Id][$deter_Id]))
				{
					$recv_number_mrrs = implode(',',$recv_number_mrr_array[$$bookingNo][$color_Id][$bodypary_Id][$deter_Id]);
					$recv_number_mrr_arr[$recv_numbers]=$recv_number_mrrs;
				}

				$totalIssue=($prev_tot_batch_issue_qty+$curr_issueQty)-$curr_hiddenIssueQty;

				//echo "10**".$totalIssue."<".$prev_tot_batch_recv_qty;
				if(($totalIssue<$prev_tot_batch_recv_qty)){
					echo "PreviousRecv**".str_replace("'","",$$bookingNo)."**".implode(", ",$recv_number_arr)."**".$prev_tot_batch_recv_qty."**".$$batchWgt;
					disconnect($con);
					die;
				}
	...
	else if ($operation==2) // Delete Here
	{
		...
		$sql = "select a.recv_number as recv_number from inv_receive_mas_batchroll a,pro_grey_batch_dtls b where a.id=b.mst_id  and a.entry_form in(554) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.issue_id=$update_id group by a.recv_number";
		$data_array = sql_select($sql);
		$recv_number=$data_array[0][csf('recv_number')];
		if ($recv_number!="")
		{
			echo "13**Fabric Issue Return. Found=" .$recv_number;
			disconnect($con);
			die;
		}
...
if($action=="grey_item_details_update")
{
	...
	$sql=sql_select("select b.id, b.mst_id, b.prod_id, b.body_part_id, b.febric_description_id, b.gsm, b.width, b.buyer_id, b.job_no, b.order_id, b.width_dia_type, b.color_range_id, b.batch_id,a.dyeing_source, a.batch_no, b.color_id, b.process_id, b.batch_wgt, b.batch_issue_qty, b.outbound_batchname, b.fin_dia, b.fin_gsm, b.width_dia_type,b.roll_no,a.receive_basis, b.booking_no,b.booking_dtls_id, b.booking_date,b.booking_without_order,b.rate, b.remarks, c.fabric_color_id, b.color_type_id, b.is_sales from pro_grey_batch_dtls b left join wo_booking_dtls c on b.booking_dtls_id=c.id and b.is_sales!=1, inv_receive_mas_batchroll a where b.mst_id=$datas[0] and b.mst_id=a.id and a.entry_form=91 and b.status_active=1 and b.is_deleted=0 order by b.id"  );
	...
	$color_arr = return_library_array("select id, color_name from lib_color","id","color_name");// where id in($color_id_all)
	$buyer_arr = return_library_array("select id, buyer_name from lib_buyer","id","buyer_name");
	


	$composition_arr=array(); $constructtion_arr=array();
	$sql_deter="select a.id, a.construction, b.copmposition_id, b.percent from lib_yarn_count_determina_mst a, lib_yarn_count_determina_dtls b where a.id=b.mst_id and a.id in($febric_description_id_all)  and b.status_active=1 and a.status_active=1 group by a.id, a.construction, b.copmposition_id, b.percent ";
    ...
    $previousReturn=sql_select("SELECT a.batch_issue_qty,a.booking_no,a.booking_dtls_id,a.id
    from pro_grey_batch_dtls a, inv_receive_mas_batchroll b
    where a.mst_id = b.id  and a.booking_dtls_id in($booking_dtls_id) $without_current_return and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.entry_form = 554 and b.issue_id=$datas[0]");
	...
	{
		$smn_info_sql = "SELECT b.id,c.fabric_color FROM wo_non_ord_knitdye_booking_mst a, wo_non_ord_knitdye_booking_dtl b, wo_non_ord_samp_booking_dtls c where a.id=b.mst_id and a.fab_booking_id=c.booking_mst_id and a.company_id = $data[1] and b.id in ($booking_dtls_id) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
		union all
		select b.id,c.fabric_color from wo_non_ord_aop_booking_mst a, wo_non_ord_aop_booking_dtls b, wo_non_ord_samp_booking_dtls c where a.id=b.wo_id and a.fab_booking_no=c.booking_no and a.company_id = $data[1] and b.id in ($booking_dtls_id) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.fabric_description=c.id";
	...
	foreach($sql as $val)
	{
		...
		$balance = $val[csf("batch_wgt")] - ($previousIssueArrNew[$val[csf('booking_no')]][$val[csf('booking_dtls_id')]] + $previousReturnArrNew[$val[csf('booking_no')]][$val[csf('booking_dtls_id')]]);
		...
		else if($val[csf("booking_without_order")] == 1)
		{
			$bookingType = 2;
		}
		else
		...
		<tr id="tr_<? echo $i; ?>" align="center" valign="middle">
			...
			<td style="word-break:break-all;" width="90" id="colorTypeNO_<? echo $i; ?>"><? echo $color_type[$val[csf("color_type_id")]]; ?></td>
			...
			<td style="word-break:break-all;" width="70" id="color_<? echo $i; ?>">
				<? 
					if($val[csf('is_sales')]!=1)
					{
			...
			<td style="word-break:break-all;" width="70" id="fabColor_<? echo $i; ?>" >
				<?
					...
					else if($val[csf('booking_without_order')] !=1)
					{
						...
						echo $color_arr[$smn_info_arr[$val[csf('booking_dtls_id')]]['fabric_color']];
					}
				?>
			</td>
			<td style="word-break:break-all;" width="70" id="diaType_<? echo $i; ?>"><? echo $fabric_typee[$val[csf("width_dia_type")]]; ?></td>
			...
			<td style="word-break:break-all;" width="100" id="internalRef_<? echo $i; ?>" align="left"><? echo implode(",",array_unique(explode(",",chop($internalRef_arr[$val[csf("order_id")]],',')))); ?></td>
			...
			<td id="button_1" align="center">
				...
				<input type="hidden" name="colorTypeId[]" id="colorTypeId_<? echo $i; ?>" value="<? echo $val[csf("color_type_id")]; ?>"/>
				...
				<input type="hidden" name="widthTypeId[]" id="widthTypeId_<? echo $i; ?>" value="<? echo $val[csf("width_dia_type")]; ?>"/>
				...
				<input type="hidden" name="bookingType[]" id="bookingType_<? echo $i; ?>" value="<? echo $bookingType; ?>"/>
					...
					<input type="hidden" name="hiddenTxtIssueQty[]" id="hiddenTxtIssueQty_<? echo $i; ?>" value="<? echo $val[csf("batch_issue_qty")]; ?>"/>
...
if($action=="populate_data_from_data")
{
	...
	foreach($res as $row)
	{
		...
		echo "load_drop_down( 'requires/fabric_issue_to_finishing_process_controller', ".$row[csf("dyeing_source")]."+'**'+".$row[csf("company_id")].", 'load_drop_down_knitting_com_new', 'dyeing_company_td' );\n";
						...
						<table  cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
							...
							<tr>
								<td> <input type="hidden" id="selected_booking">
									<?
									echo create_drop_down( "cbo_company_mst", 120, "select id,company_name from lib_company comp where status_active=1 $company_cond order by company_name","id,company_name",1, "-- Select Company --", "".$company_id."", "load_drop_down( 'fabric_issue_to_finishing_process_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );",1);
									?>
								</td>
								<td>
									<?php
									...
									{
										echo create_drop_down( "cbo_supplier_name", 122, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and a.status_active=1 group by a.id,a.supplier_name order by a.supplier_name","id,supplier_name", 1, "-- Select --", "".$supplier_id."", "",1 );
										//and b.party_type in (21,24,25)
									}
									else
									{
								...
								<td>
									<input name="txt_fso_no" id="txt_fso_no" class="text_boxes" style="width:70px">
								</td>
								<td>
...
if ($action=="create_booking_search_list_view")
{
    ...
    $supplier_id_from_front=$data[14];
    ...
    if($db_type==0)
    {
		$booking_year_cond=" and SUBSTRING_INDEX(a.insert_date, '-', 1)=$data[15]";
    	if ($data[2]!="" &&  $data[3]!="") $booking_date  = "and a.booking_date  between '".change_date_format($data[2], "yyyy-mm-dd", "-")."' and '".change_date_format($data[3], "yyyy-mm-dd", "-")."'"; else $booking_date ="";
    }

    if($db_type==2)
    {
		$booking_year_cond=" and to_char(a.insert_date,'YYYY')=$data[15]";
    	if ($data[2]!="" &&  $data[3]!="") $booking_date  = "and a.booking_date  between '".change_date_format($data[2], "yyyy-mm-dd", "-",1)."' and '".change_date_format($data[3], "yyyy-mm-dd", "-",1)."'"; else $booking_date ="";

		if ($data[2]!="" &&  $data[3]!="") $wo_date_cond  = "and c.wo_date  between '".change_date_format($data[2], "yyyy-mm-dd", "-",1)."' and '".change_date_format($data[3], "yyyy-mm-dd", "-",1)."'"; else $wo_date_cond ="";
		
    }
    //echo $data[8];
    if($data[6]==1)
    {
    	if (str_replace("'","",$data[5])!="") $booking_cond=" and a.booking_no_prefix_num='$data[5]' $booking_year_cond "; else  $booking_cond="";
    	if (str_replace("'","",$data[5])!="") $booking_cond_nonOrder=" and a.wo_no_prefix_num='$data[5]'    "; else  $booking_cond_nonOrder="";
    	if (str_replace("'","",$data[5])!="") $sales_wo_no_cond=" and c.do_number_prefix_num='$data[5]' and to_char(c.insert_date,'YYYY')=$data[15]  "; else  $sales_wo_no_cond="";
    ...
    if($data[6]==4 || $data[6]==0)
    {
    	...
    	if (str_replace("'","",$data[5])!="") $sales_wo_no_cond=" and c.do_number_prefix_num like '%$data[5]%' and to_char(c.insert_date,'YYYY')=$data[15] "; else  $sales_wo_no_cond="";
	...
	$variable_setting=return_field_value("yes_no_permission","variable_settings_production","company_name='$data[0]' and variable_list=77","yes_no_permission");
	$variable_cond="";
	if($variable_setting==1){
		$variable_cond="and a.is_approved=1";
	}

	if($is_sales==1)
	{
		if ($data[1]!=0) $buyer_cond=" and ((e.buyer_id='$data[1]' and e.within_group=2) or (e.po_buyer='$data[1]' and e.within_group=1))"; else $buyer_cond="";

		$sql="SELECT c.id as wo_id, c.do_no as booking_no, c.company_id, e.within_group, e.buyer_id, e.po_buyer, c.wo_date as booking_date, e.job_no, g.grouping, g.file_no, e.style_ref_no, c.dyeing_compnay_id as supplier_id, g.po_number, e.po_job_no, c.process_id as process, d.wo_qty, d.id as wo_dtls_id, 696 as type   
		from dyeing_work_order_mst c, dyeing_work_order_dtls d, fabric_sales_order_mst e 
		left join wo_booking_dtls f on e.sales_booking_no=f.booking_no and f.status_active=1
		left join wo_po_break_down g on f.po_break_down_id=g.id
		where c.entry_form=696 and c.id=d.mst_id and d.FSO_ID=e.id and c.company_id=$data[0] and c.dyeing_compnay_id='$data[7]' $buyer_cond and d.status_active=1 and c.status_active=1 $wo_date_cond $sales_wo_no_cond 
		order by c.id";


	}
    else if($data[10]!="" || $data[11]!="" || $data[12]!="")
    {
    	...
    	where a.booking_no = b.booking_no  and b.po_break_down_id=d.id  and $company $buyer $booking_date $style_cond $file_cond $po_number_cond $internal_ref_cond and  a.booking_type=3 and  a.status_active=1 and a.is_deleted=0 and b.job_no=c.job_no  $booking_cond $job_cond  $supplier $variable_cond and b.process in (1,2,25,26,31,32,33,35,36,37,38,40,60,62,63,64,65,66,67,68,69,70,71,73,82,83,84,85,88,89,90,91,93,94,100,129,135,136,145,156,127,155,154,159,158,199,196,34,195,193,198,169,160,202,205,209,210,211,212,219,220,224,225,226,227,231,161,171,213,214,241,250,254,255,263,277,278,279,288,826,289,268,318,341,397,412,413,417,287,430,434,422,435,429,442,234,456,471,472,475,477,478,479,481,379)
    	$preBookingNos_1 and a.ref_closing_status=0 
    	group by a.id,a.booking_no_prefix_num, a.booking_no, c.job_no, a.booking_date, a.company_id, a.buyer_id, a.po_break_down_id, a.item_category, a.fabric_source, c.job_no_prefix_num,c.style_ref_no, a.supplier_id,a.process,d.file_no,d.po_number,d.grouping  	order by booking_date desc";
    }
    else
	{
    	 $sql= "SELECT a.id, a.booking_no_prefix_num, a.booking_no, a.booking_date, a.company_id, a.buyer_id, c.job_no_prefix_num,c.style_ref_no, c.job_no, a.po_break_down_id, a.process, a.item_category, a.fabric_source, a.supplier_id, 1 as type ,sum(b.wo_qnty) as wo_qnty,d.file_no,d.po_number,d.grouping
    	from wo_booking_mst a, wo_booking_dtls b, wo_po_details_master c ,wo_po_break_down d
    	where a.booking_no = b.booking_no  and b.po_break_down_id=d.id  and $company $buyer $booking_date $style_cond $file_cond $po_number_cond $internal_ref_cond and  a.booking_type=3 and  a.status_active=1 and a.is_deleted=0 and b.job_no=c.job_no  $booking_cond $job_cond  $supplier $variable_cond and b.process in (1,2,25,26,31,32,33,35,36,37,38,40,60,62,63,64,65,66,67,68,69,70,71,73,82,83,84,85,88,89,90,91,93,94,100,129,135,136,145,156,127,155,154,159,158,196,198,34,195,193,199,169,160,202,205,209,210,211,212,219,220,224,225,226,161,227,171,213,214,231,241,205,254,255,263,277,278,279,288,826,289,268,318,341,397,412,413,417,287,422,430,434,435,429,442,234,456,471,472,475,477,478,479,322,379,481)
    	$preBookingNos_1 and a.ref_closing_status=0 
	...
	if($is_sales==1)
	{
		foreach($result as $row)
		{
			if($dtls_qnty_chk[$row[csf('dtls_id')]]=="")
			{
				$dtls_qnty_chk[$row[csf('dtls_id')]]=$row[csf('dtls_id')];
				$woQtyArr[$row[csf('booking_no')]]+=$row[csf('wo_qty')];
			}
			...
			$wo_data_arr[$row[csf('booking_no')]]['wo_id']=$row[csf('wo_id')];
			$wo_data_arr[$row[csf('booking_no')]]['booking_no']=$row[csf('booking_no')];
			$wo_data_arr[$row[csf('booking_no')]]['buyer_id']=$buyer_id;
			$wo_data_arr[$row[csf('booking_no')]]['booking_date']=$row[csf('booking_date')];
			$wo_data_arr[$row[csf('booking_no')]]['fso_no']=$row[csf('job_no')];
			$wo_data_arr[$row[csf('booking_no')]]['file_no'].=$row[csf('file_no')].',';
			$wo_data_arr[$row[csf('booking_no')]]['grouping'].=$row[csf('grouping')].',';
			$wo_data_arr[$row[csf('booking_no')]]['po_no'].=$row[csf('po_number')].',';
			$wo_data_arr[$row[csf('booking_no')]]['job_no'].=$row[csf('po_job_no')].',';
			$wo_data_arr[$row[csf('booking_no')]]['company_id']=$row[csf('company_id')];
			$wo_data_arr[$row[csf('booking_no')]]['style_ref_no']=$row[csf('style_ref_no')];
			$wo_data_arr[$row[csf('booking_no')]]['supplier_id']=$row[csf('supplier_id')];
			$wo_data_arr[$row[csf('booking_no')]]['process']=$row[csf('process')];
			$wo_data_arr[$row[csf('booking_no')]]['type']=$row[csf('type')];
		}
	}
	else
	{
    	...
    	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1270" class="rpt_table" id="tbl_list_search" >
    		<tbody>
				...
				if($is_sales==1)
				{
					...
					foreach($wo_data_arr as $wo_no=>$row)
					{
						$total_issueQty= $issueQtyArr[$row['booking_no']];

						$total_woQty=$woQtyArr[$row['booking_no']];
						$total_woQty=number_format($total_woQty,2,'.','');
						$total_issueQty=number_format($total_issueQty,2,'.','');
						...
						if($balance > 0)
						{
							...
							if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
							$booking_format_arr=explode("-", $row['booking_no']);
							...
							?>
							<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $row['wo_id']."_".$row['booking_no']."_".$job_no."_".$row['type']."_".$row['booking_date']."_".$balance."_".$booking_format."_".$process; ?>');">
							<?
							
							?>
								<td width="40"><? echo $i; ?></td>
								<td width="150"><p><? echo $row['booking_no']; ?></p></td>
								<td width="80"><p><? echo change_date_format($row['booking_date']); ?></p></td>
								<td width="80"><p><? echo $comp[$row['company_id']]; ?></p></td>

								<td width="100"><p><? echo $buyer_arr[$row['buyer_id']]; ?></p></td>
								<td width="70"><p><? echo  $job_no;?></p></td>
								<td width="70"><p><? echo implode(",",array_unique(explode(",",chop($row['fso_no'],',')))); ?></p></td>


								<td width="70"><p><? echo implode(",",array_unique(explode(",",chop($row['grouping'],',')))); ?></p></td>
								<td width="70"><p><? echo implode(",",array_unique(explode(",",chop($row['file_no'],',')))); ?></p></td>

								<td width="110"><p><? echo $row['style_ref_no']; ?></p></td>
								<td width="150"><p><? echo implode(",",array_unique(explode(",",chop($row['po_no'],',')))); ?></p></td>
								<td width="150"><p><? echo "Service- Fabric"; ?></p></td>

								<td align="center"><? echo $suplier[$row[csf('supplier_id')]]; ?></td>
							</tr>
							<?
							$i++;
...
if ($action=="fabric_search_popup")
{
	...
	if($type == 1)
	{
		...
		$sql_batch="SELECT a.batch_no,a.id as batch_id,a.color_id,a.process_id,b.id as dtls_id,b.item_description,b.body_part_id,b.prod_id,b.po_id,b.width_dia_type,b.batch_qnty ,d.detarmination_id as deter_id
		from pro_batch_create_mst a,pro_batch_create_dtls b,pro_fab_subprocess c,product_details_master d
		where a.id=b.mst_id  and a.id=c.batch_id and c.batch_id=b.mst_id and d.id=b.prod_id and c.load_unload_id=2 and a.entry_form=0 and b.po_id in($po_ids) and b.batch_qnty>0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.id";
	...
	if($booking_type == 1)
	{
		  ...
		  $sql= "SELECT a.id as aid,c.id as dtls_id, a.body_part_id as body_part_id,a.color_type_id as color_type_id,a.fabric_description,a.gsm_weight,a.width_dia_type,a.construction,a.lib_yarn_count_deter_id,b.id as bid,b.cons_process as cons_process, c.id as id,c.job_no,c.po_break_down_id,c.booking_no as booking_no,c.pre_cost_fabric_cost_dtls_id as pre_cost_fabric_cost_dtls_id,c.dia_width as dia_width, c.wo_qnty as wo_qnty,c.rate,c.amount as amount,c.gmts_color_id as gmts_color_id, c.fabric_color_id,c.printing_color_id, c.process,c.uom,null as buyer_id
	...
	else
	{
		...
		where a.id = b.wo_id and b.fab_booking_no = c.booking_no and b.fabric_description = c.id and  a.status_active=1 and a.is_deleted=0 and  b.status_active=1 and b.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 and a.wo_no = '$bookingNo'  and b.wo_qty>0
		union all
		SELECT a.id,b.id as dtls_id,c.body_part as body_part_id,c.color_type_id,b.gmts_color as gmts_color_id,  a.prefix_num as booking_no_prefix_num, a.booking_no, c.lib_yarn_count_deter_id,  a.company_id, a.buyer_id, null as job_no_prefix_num,null as style_ref_no, null as job_no, null as po_break_down_id,b.wo_qty,  b.gsm as gsm_weight,b.dia as dia_width,b.rate, b.process_id as process, 0 as item_category, b.fabric_source, a.supplier_id,b.uom,b.fab_des_id, 2 as type from wo_non_ord_knitdye_booking_mst a,  wo_non_ord_knitdye_booking_dtl b, wo_non_ord_samp_booking_dtls c where a.id = b.mst_id and b.fab_des_id =  c.id  and b.status_active = 1 and b.is_deleted = 0 and a.booking_no = '$bookingNo' and b.wo_qty>0";
 	}
	//echo $sql;
	//=========================================================
	...
	foreach ($result_po_batch as $row)
	{
		...
		//{
			...
			$po_batch_wise_data_qty_arr[$row[csf('batch_id')]]['batch_qnty']+= $row[csf('batch_qnty')];
  		...
  		<table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" width="1000">
            ...
            <?
				...
				foreach($results as $row)
				{
					...
					if($row[csf("fabric_color_id")]=='' || $row[csf("fabric_color_id")]==0) $row[csf("fabric_color_id")]=$row[csf("printing_color_id")];

					$color_typeid=0; 
					$color_typeid=$booking_color_type_arr[$row[csf("po_break_down_id")]][$row[csf("body_part_id")]][$row[csf("fabric_color_id")]][$deter_id];
					 

					if($row[csf("body_part_id")]=='') $row[csf("body_part_id")]=0;else $row[csf("body_part_id")]=$row[csf("body_part_id")];

					$batch_id=$po_batch_data_qty_arr[$row[csf('po_break_down_id')]][$row[csf('body_part_id')]][$row[csf('fabric_color_id')]][$deter_id]['batch_id'];
					$batch_no=$po_batch_data_qty_arr[$row[csf('po_break_down_id')]][$row[csf('body_part_id')]][$row[csf('fabric_color_id')]][$deter_id]['batch_no'];
//echo $row[csf('po_break_down_id')].'='.$row[csf('body_part_id')].'='.$row[csf('fabric_color_id')].'='.$deter_id.'<br>';
					$batch_qnty=0;$color_previousIssue_qty=0;
					$batch_qnty=$po_batch_data_qty_arr[$row[csf('po_break_down_id')]][$row[csf('body_part_id')]][$row[csf('fabric_color_id')]][$deter_id]['batch_qnty'];
					if($batch_qnty==0 || $batch_qnty=="") $batch_qnty=0;else $batch_qnty=$batch_qnty;

					$color_previousIssue_qty=$previousIssueArrNew2[$row[csf('po_break_down_id')]][$row[csf('body_part_id')]][$row[csf('fabric_color_id')]][$deter_id];
					...
					$batch_id_val=rtrim($batch_id,',');
					$batch_id_array=array_unique(explode(',',$batch_id_val));
					//var_dump($batch_id_array);
					//print_r($batch_id_array);
					$batch_ids=implode(',',array_unique(explode(',',$batch_id_array))) ;
					...
					if($cons_process==35)
					{
						foreach ($batch_id_array as $batch_id_key => $batchId)
						{
							$dtl_data=$row[csf("dtls_id")].'_'.$batchId;
							...
							{
							 	$batch_qnty=0;
							 	$batch_qnty=$po_batch_wise_data_qty_arr[$batchId]['batch_qnty'];
							}
							if($batchId!="" && $cons_process==35)
							{ //AOP
								?>
								<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i; ?>" onClick="js_set_value(<? echo $i; ?>)">
									...
									<td align="center"  width="100" title="<? echo $batchId; ?>"><? echo $batch_no_arr[$batchId]; ?></td>
			                   	...
			                   	<?
					   			$i++;
								$batch_no_chk[$batch_no_arr[$batchId]]=$batch_no_arr[$batchId];
							}
						}
					} //AOP chk end
					else
					{
						foreach ($batch_id_array as $batch_id_key => $batchId)
						{
							...
							<td align="center"  width="100"><? echo $batch_no_arr[$batchId];//$batch_no; ?></td>
							<td align="center"  width="100"><? echo $body_part[$row[csf("body_part_id")]]; ?></td>
							<td align="center" style="word-wrap: break-word;"><? echo $fabric_des; ?>&nbsp;</td>
							<td align="center"  width="50"><? echo $row[csf("gsm_weight")]; ?>&nbsp;</td>
							<td  align="center" width="50" ><? echo $row[csf("dia_width")]; ?>&nbsp;</td>
	                        ...
	                        <td align="center" width="80" ><? echo number_format($batch_qnty,2); ?>&nbsp;</td>
	                   		...
	                   		<?
					   		$i++;
							$batch_no_chk[$batch_no_arr[$batchId]]=1;
						}
					}
				}
...
if($action=="check_booking_no")
{
	...
	$sql = "SELECT a.id, a.booking_no_prefix_num, a.booking_no, a.booking_date, a.company_id, a.supplier_id, 1 as type
	...
	and b.process in (25,26,31,33,35,36,37,40,60,62,63,64,65,66,67,68,69,70,71,73,82,83,84,85,88,89,90,91,93,94,129,135,136,145,156,159,34,195,198,169,160,161,205,210,211,212,219,220,224,225,226,227,213,214,241,205,231,254,255,263,277,278,279,288,289,268,318,341,397,412,413,417,287,422,430,429,434,435,442,456,471,472,475,477,478,479,481,379) and a.booking_no ='$book_ref' and a.company_id = '$company_id' and a.supplier_id = '$supplier_id'
	group by a.id, a.booking_no_prefix_num, a.booking_no, a.booking_date, a.company_id, a.supplier_id

	union all
	select a.id,a.wo_no_prefix_num as booking_no_prefix_num, a.wo_no as booking_no, a.booking_date, a.company_id, a.supplier_id, 2 as type
	from wo_non_ord_aop_booking_mst a , wo_non_ord_aop_booking_dtls b
	...
	group by a.id,a.wo_no_prefix_num, a.wo_no, a.booking_date, a.company_id, a.supplier_id

	union all
	SELECT c.id, c.do_number_prefix_num as booking_no_prefix_num, c.do_no as booking_no, c.wo_date as booking_date, c.company_id, c.dyeing_compnay_id as supplier_id, 696 as type
	from dyeing_work_order_mst c, dyeing_work_order_dtls d, fabric_sales_order_mst e 
	left join wo_booking_dtls f on e.sales_booking_no=f.booking_no and f.status_active=1
	left join wo_po_break_down g on f.po_break_down_id=g.id
	where c.entry_form=696 and c.id=d.mst_id and d.FSO_ID=e.id and c.company_id='$company_id' and c.dyeing_compnay_id='$supplier_id' and d.status_active=1 and c.status_active=1 and c.do_no = '$book_ref'
	group by c.id, c.do_number_prefix_num, c.do_no, c.wo_date, c.company_id, c.dyeing_compnay_id
...
if($action=="subcon_issue_print")
{
	...
	$sql = "SELECT  b.batch_id,b.prod_id, b.order_id, b.buyer_id, b.color_id, b.body_part_id, b.febric_description_id, b.process_id, b.fin_dia, b.fin_gsm, b.roll_id, 0 as barcode_no, b.batch_issue_qty as qnty, b.roll_no , b.booking_no, b.booking_dtls_id, b.outbound_batchname, b.booking_without_order, b.gsm, b.width, c.fabric_color_id, b.color_type_id from inv_receive_mas_batchroll a, pro_grey_batch_dtls b, wo_booking_dtls c where a.id=b.mst_id and a.id=$update_id and c.id = b.booking_dtls_id and a.entry_form=91 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
   //echo $sql;die;
	...
	$composition_arr=array();
	$sql_deter="select a.id, a.construction, b.copmposition_id, b.percent from lib_yarn_count_determina_mst a, lib_yarn_count_determina_dtls b where a.id=b.mst_id and a.id in($febric_description_id_all) group by a.id, a.construction, b.copmposition_id, b.percent";
	$data_array=sql_select($sql_deter);
	foreach( $data_array as $row )
	{
		if(array_key_exists($row[csf('id')],$composition_arr))
			...
			<tr class="form_caption">
			   ...
			   //die;
				...
				//0.0112 sec
						...
						$buyer_arr_aop=return_library_array( "select wo_no, buyer_id from wo_non_ord_aop_booking_mst where   wo_no in($booking_no_all)", "wo_no", "buyer_id");
						$buyer_arr_KnitArr=return_library_array( "select booking_no, buyer_id from WO_NON_ORD_KNITDYE_BOOKING_MST where booking_no in($booking_no_all)", "booking_no", "buyer_id");
				//============================
								...
								<td align="center"><? echo $color_type[$row[csf('color_type_id')]]; ?>&nbsp;</td>
									...
									<? //echo $buyer_array[$job_array[$row[csf('order_id')]]['buyer_id']]."<hr/>".$job_array[$row[csf('order_id')]]['style_ref_no']."<hr/>".$job_array[$row[csf('order_id')]]['job']."<hr/>".$job_array[$row[csf('order_id')]]['po']; ?>
                                        <table cellspacing="0" width="100%"  border="1" rules="all" style="border:hidden;">
                                           <tr><td><?
		                                    if ($buyer_array[$job_array[$row[csf('order_id')]]['buyer_id']]=="") {
		                                		 $buyer_name=$buyer_array[$buyer_arr_aop[$row[csf('booking_no')]]];
												 if($buyer_name=="") $buyer_name=$buyer_array[$buyer_arr_KnitArr[$row[csf('booking_no')]]];
		                                	}else{
		                                		 $buyer_name=$buyer_array[$job_array[$row[csf('order_id')]]['buyer_id']];
		                                	}
		                                	echo $buyer_name;
                                			//$buyer_array[$job_array[$row[csf('order_id')]]['buyer_id']];   ?>&nbsp;</td></tr>
...
if($action=="subprocess_issue_print")
{
	...
	$sql = "SELECT  b.batch_id,b.prod_id, b.order_id, b.buyer_id, b.color_id, c.fabric_color_id, b.body_part_id, b.febric_description_id, b.process_id, b.fin_dia,b.booking_without_order, b.fin_gsm,b.gsm,b.width, b.roll_id, 0 as barcode_no, b.batch_issue_qty as qnty, roll_no, b.outbound_batchname,b.booking_no,b.booking_dtls_id, b.remarks, b.color_type_id from inv_receive_mas_batchroll a, pro_grey_batch_dtls b, wo_booking_dtls c where a.id=b.mst_id and a.id=$update_id and b.booking_dtls_id=c.id and a.entry_form=91 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.batch_issue_qty>0 and nvl(b.booking_without_order,0)=0
	union all
	SELECT b.batch_id,b.prod_id, b.order_id, b.buyer_id, b.color_id, d.fabric_color as fabric_color_id, b.body_part_id,
b.febric_description_id, b.process_id, b.fin_dia,b.booking_without_order, b.fin_gsm,b.gsm,b.width, b.roll_id, 0 as barcode_no, b.batch_issue_qty as qnty, roll_no, b.outbound_batchname,b.booking_no,b.booking_dtls_id, b.remarks, b.color_type_id
from inv_receive_mas_batchroll a, pro_grey_batch_dtls b, wo_non_ord_aop_booking_dtls c, wo_non_ord_samp_booking_dtls d
where a.id=b.mst_id and a.id=$update_id and b.booking_dtls_id=c.id and c.fabric_description=d.id and a.entry_form=91 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.batch_issue_qty>0 and b.booking_without_order=1
union all
SELECT b.batch_id,b.prod_id, b.order_id, b.buyer_id, b.color_id, d.fabric_color as fabric_color_id, b.body_part_id,
b.febric_description_id, b.process_id, b.fin_dia,b.booking_without_order, b.fin_gsm,b.gsm,b.width, b.roll_id, 0 as barcode_no, b.batch_issue_qty as qnty, roll_no, b.outbound_batchname,b.booking_no,b.booking_dtls_id, b.remarks, b.color_type_id
from inv_receive_mas_batchroll a, pro_grey_batch_dtls b, wo_non_ord_knitdye_booking_dtl c, wo_non_ord_samp_booking_dtls d
where a.id=b.mst_id and a.id=$update_id and b.booking_dtls_id=c.id and c.FAB_DES_ID=d.id and a.entry_form=91 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.batch_issue_qty>0 and b.booking_without_order=1 ";
	...
	$buyer_array=return_library_array( "select id, short_name from lib_buyer", "id", "short_name");
	$buyer_code_array=return_library_array( "select id, remark from lib_buyer", "id", "remark");
	$country_arr=return_library_array( "select id, country_name from  lib_country", "id", "country_name"  );
	...
	$job_array=array();
	 ...
	 $job_sql="select a.buyer_name, a.job_no, a.style_ref_no, b.id, b.po_number,b.grouping, c.internal_ref from wo_po_details_master a, wo_po_break_down b left join wo_order_entry_internal_ref c on b.job_no_mst=c.job_no where a.job_no=b.job_no_mst and b.id in($order_id_all)";
	$job_sql_result=sql_select($job_sql);
	foreach($job_sql_result as $row)
	{
		$job_array[$row[csf('id')]]['job']=$row[csf('job_no')];
		...
		if($row[csf('internal_ref')])
		{
			$job_array[$row[csf('id')]]['internal_ref']=$row[csf('internal_ref')];
		}
		else{
			$job_array[$row[csf('id')]]['internal_ref']=$row[csf('grouping')];
		}
	...
	$yarn_lot_sql="select a.id, a.recv_number, a.company_id, a.receive_basis, a.booking_id, a.booking_no, a.knitting_source, a.challan_no,b.id, b.body_part_id, b.trans_id,
	b.prod_id, b.febric_description_id, b.no_of_roll, b.gsm, b.width, b.uom, b.yarn_lot,b.yarn_prod_id, b.yarn_count, b.order_id,c.po_id,d.job_no_mst as job_no	,c.fabric_desc,c.determination_id
	 from inv_receive_master a,pro_grey_prod_entry_dtls b,ppl_planning_entry_plan_dtls c,wo_po_break_down d  where a.id=b.mst_id  and c.dtls_id=a.booking_id and c.po_id=d.id and c.po_id in ($order_id_all)";
	$yarn_lot_data=sql_select($yarn_lot_sql);
	$yarn_lot_data_arr=array();
	foreach($yarn_lot_data as $val){
		$yarn_lot_data_arr[$val[csf('job_no')]][$val[csf('po_id')]][$val[csf('body_part_id')]][$val[csf('determination_id')]][$val[csf('gsm')]]['yarn_lot']=$val[csf('yarn_lot')];

	}

	//======================================end============yarn lot=====================================14-11-2021==================================

	// echo "<pre>";
	// print_r($yarn_lot_data_arr);
	...
	$composition_arr=array();
	 $sql_deter="select a.id, a.construction, b.copmposition_id, b.percent from lib_yarn_count_determina_mst a, lib_yarn_count_determina_dtls b where a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and a.id in($febric_description_id_all) ";
	$data_array=sql_select($sql_deter);
	foreach( $data_array as $row )
	{
		if(array_key_exists($row[csf('id')],$composition_arr))
	...
	$feb_des_data = sql_select("select b.id as dtls_id, a.wo_no as booking_no,b.fabric_description as fab_des_id,b.fabric_source
		from wo_non_ord_aop_booking_mst a,wo_non_ord_aop_booking_dtls b where a.id = b.wo_id and a.company_id = $company and  a.status_active=1 and a.is_deleted=0  and b.status_active = 1 and a.wo_no in($booking_no_all)
		union all
		select b.id as dtls_id, a.booking_no, b.fab_des_id,b.fabric_source
		from wo_non_ord_knitdye_booking_mst a,  wo_non_ord_knitdye_booking_dtl b
		where a.id = b.mst_id and a.company_id = $company and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and a.booking_no in($booking_no_all) and b.fab_des_id in($febric_description_id_all)"); //
	...
	$fab_des_id_all=implode(",",array_unique(array_filter(explode(",",chop($fab_des_ids_all,",")))));
	if($fab_des_id_all!=""){$fab_des_id_cond="and a.id in($fab_des_id_all)";}else{$fab_des_id_cond="";}
	if($fab_des_id_all!=""){$fab_des_id_cond_2="and c.id in($fab_des_id_all)";}else{$fab_des_id_cond_2="";}

	if($booking_without_order_status!=0 && $fab_des_id_all!="")
	{
		$sql_non_order=sql_select("select a.id,a.body_part,a.fabric_description,fabric_color,a.gmts_color,a.gsm_weight,a.dia_width from wo_non_ord_samp_booking_mst b,wo_non_ord_samp_booking_dtls a where a.booking_no=b.booking_no and a.status_active=1 and a.is_deleted=0  $fab_des_id_cond order by a.id"); //and a.id in($febric_description_id_all)
	...
	if($fab_des_id_all!="")
	{
		$sql_order_feb=sql_select("select c.id,c.from_prod_id from inv_item_transfer_mst a,inv_transaction b,inv_item_transfer_dtls c where a.id= b.mst_id and a.id=c.mst_id and a.transfer_criteria=6 and c.item_category=13  and a.status_active=1 and a.is_deleted=0  $fab_des_id_cond_2 order by c.id"); //and c.id in($febric_description_id_all)
	...
	<div>
		...
		<table cellspacing="0" width="1300"  border="1" rules="all" class="rpt_table" >
			...
			<tbody>
				...
				$buyer_arr_aop=return_library_array( "select wo_no, buyer_id from wo_non_ord_aop_booking_mst", "wo_no", "buyer_id");
				$buyer_arr_aop=return_library_array( "select booking_no, buyer_id from wo_non_ord_knitdye_booking_mst", "booking_no", "buyer_id");
				...
				foreach($result as $row)
				{
					$feb_des_id = $feb_des_array[$row[csf("booking_no")]][$row[csf("booking_dtls_id")]]["feb_des_id"];
					$feb_des_source = $feb_des_array[$row[csf("booking_no")]][$row[csf("booking_dtls_id")]]["fabric_source"];
					$job_no=$job_array[$row[csf('order_id')]]['job'];
					$po_id=$job_array[$row[csf('order_id')]]['po_id'];
					?>
					<tr>
						<td align="center" valign="middle"><? echo $i; ?></td>
						<td align="center"><? echo $row[csf("booking_no")]; ?></td>
						...
						<td align="center"><? echo $color_type[$row[csf('color_type_id')]]; ?>&nbsp;</td>
							...
							<td style="word-break:break-all;"><? echo $color_arr[$row[csf("color_id")]];?>&nbsp;</td>
							<td style="word-break:break-all;"><? echo $color_arr[$row[csf("fabric_color_id")]];?>&nbsp;</td>
							<td style="word-break:break-all;"><? echo $yarn_lot_data_arr[$job_no][$po_id][$row[csf('body_part_id')]][$row[csf("febric_description_id")]][$row[csf('gsm')]]['yarn_lot'];?>&nbsp;</td>
							<td style="word-break:break-all;"><? echo $conversion_cost_head_array[$row[csf("process_id")]]; ?>&nbsp;</td>
							<td style="word-break:break-all;" align="right"><? if($row[csf("qnty")]!=0){ echo $row[csf("qnty")];} ?></td>
										...
										?>
											<tr>
		                                		<td><?
				                                	if ($buyer_array[$job_array[$row[csf('order_id')]]['buyer_id']]=="") {
				                                		$buyer_name=$buyer_array[$buyer_arr_aop[$row[csf('booking_no')]]];
				                                		$buyer_code=$buyer_code_array[$buyer_arr_aop[$row[csf('booking_no')]]];
				                                	}else{
				                                		$buyer_name=$buyer_array[$job_array[$row[csf('order_id')]]['buyer_id']];
				                                		$buyer_code=$buyer_code_array[$job_array[$row[csf('order_id')]]['buyer_id']];
				                                	}
													if($buyer_code !="" || $buyer_name !="")
													{
														echo $buyer_code;
													}


				                                	  //$buyer_array[$job_array[$row[csf('order_id')]]['buyer_id']]; ?>&nbsp;
			                                	</td>
	                                		</tr>
	                                		<tr><td><? echo $job_array[$row[csf('order_id')]]['style_ref_no']; ?>&nbsp;</td></tr>
...
if($action=="subprocess_issue_print3")
{
	extract($_REQUEST);
	$data=explode('*',$data);
	...
	$sql = "SELECT  b.batch_id,b.prod_id, b.order_id, b.buyer_id, b.color_id, b.body_part_id, b.febric_description_id, b.process_id, b.fin_dia,b.booking_without_order, b.fin_gsm,b.gsm,b.width, b.roll_id, 0 as barcode_no, b.batch_issue_qty as qnty, roll_no, b.outbound_batchname,b.booking_no,b.booking_dtls_id, b.remarks, c.fabric_color_id,c.yarn_count,c.brand, b.color_type_id from inv_receive_mas_batchroll a, pro_grey_batch_dtls b, wo_booking_dtls c where a.id=b.mst_id and a.id=$update_id and b.booking_dtls_id=c.id and a.entry_form=91 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.batch_issue_qty>0";
		...
		<table width="1220" cellspacing="0">
			...
			<tr class="form_caption">
                <td colspan="6" align="center" style="font-size:14px">
                    ...
                    ?>
                   <span style="margin-left: -200px;float: right;"> <b> Print Time : <? echo $date=date("F j, Y, g:i a"); ?></b> </span>
                </td>
			</tr>
            <tr>
                <!--<td colspan="6" align="center" style="font-size:18px"><strong><u><? //echo $data[3]; ?> Challan</u></strong></td>-->
		...
		<table cellspacing="0" width="1220"  border="1" rules="all" class="rpt_table" >
			<thead bgcolor="#dddddd" align="center">
				<tr>
					...
					<th width="150">Remarks (Item Discription)</th>
				</tr>
			...
			<tbody>
				...
				foreach($result as $row)
				{
					...
					<tr>
						...
						<td align="center"><? echo $job_array[$row[csf('order_id')]]['style_ref_no']; ?>&nbsp;</td>
						<td align="center"><? echo $job_array[$row[csf('order_id')]]['po']; ?>&nbsp;</td>

						<td ><? echo $row[csf("booking_no")]; ?>&nbsp;</td>
						<td ><? echo $row[csf('outbound_batchname')]; ?>&nbsp;</td>
						<td style="word-break:break-all;" align="center"><? echo $body_part[$row[csf('body_part_id')]]; ?> &nbsp;</td>
						<td align="center"><? echo $color_type[$row[csf('color_type_id')]]; ?>&nbsp;</td>
						<td style="word-break:break-all;" title="DeterId=<? echo $row[csf("febric_description_id")];?>">
						 <?
							//echo "order = ".$row[csf('order_id')]." , des id = ".$row[csf("febric_description_id")]." , source = ".$dataArray[0][csf('dyeing_source')];
...
if($action=="subprocess_issue_print4")
{
	extract($_REQUEST);
	$data=explode('*',$data);
	...
	$sql = "SELECT  b.batch_id,b.prod_id, b.order_id, b.buyer_id, b.color_id, b.body_part_id, b.febric_description_id, b.process_id, b.fin_dia,b.booking_without_order, b.fin_gsm,b.gsm,b.width, b.roll_id, 0 as barcode_no, b.batch_issue_qty as qnty, roll_no, b.outbound_batchname,b.booking_no,b.booking_dtls_id, b.remarks, c.fabric_color_id,c.yarn_count,c.brand from inv_receive_mas_batchroll a, pro_grey_batch_dtls b, wo_booking_dtls c where a.id=b.mst_id and a.id=$update_id and b.booking_dtls_id=c.id and a.entry_form=91 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.batch_issue_qty>0";
	...
	$booking_sql="SELECT a.id,a.booking_no,b.po_break_down_id FROM wo_booking_mst a, wo_booking_dtls b WHERE a.id = b.booking_mst_id AND b.po_break_down_id in($order_id_all) and a.entry_form in(86,88,108,118,119,608) group by a.id,a.booking_no,b.po_break_down_id";
	...
	{
		$booking_array[$row[csf('po_break_down_id')]]['booking_no']=$row[csf('booking_no')];
	}
	unset($booking_sql_result);
	...
	<div>
		...
		<br>
		<table cellspacing="0" width="1420"  border="1" rules="all" class="rpt_table" >
			<thead bgcolor="#dddddd" align="center">
				<tr>
					<th width="30">SL</th>
					<th width="100">Booking No</th>
					<th width="140">Work Order No</th>
					<th width="50">No. of Roll</th>
					<th width="100">Batch No </th>
					<th width="120">Body Part</th>
					<th width="170">Const./Compo</th>
					<th width="120">Y.count</th>
			...
			<tbody>
				...
				foreach($result as $row)
				{
					...
					<tr>
						<td align="center" valign="middle"><? echo $i; ?></td>
						<td align="center"><? echo $booking_array[$row[csf('order_id')]]['booking_no']; ?></td>
						<td align="center"><? echo $row[csf("booking_no")]; ?></td>
						<td align="center"><? echo $row[csf('roll_no')]; ?>&nbsp;</td>
						<td align="center"><? echo $row[csf('outbound_batchname')]; ?>&nbsp;</td>
						...
						<td style="word-break:break-all;">
							...
							<td style="word-break:break-all;" align="center"><? echo $row[csf("yarn_count")]; ?> &nbsp;</td>
							<td style="word-break:break-all;" align="center"><? echo $row[csf("brand")]; ?> &nbsp;</td>
							<td style="word-break:break-all;"><? echo $row[csf('gsm')]; ?>&nbsp;</td>
							<td style="word-break:break-all;"><? echo $row[csf('width')]; ?>&nbsp;</td>
...
if($action=="subprocess_issue_print_3")
{
    ...
    $PNG_WEB_DIR = 'requires/qrcode_image/';
	...
	//if ($order_wise_booking_all!="") {
		$sql = "SELECT  a.recv_number_prefix_num, a.recv_number,b.batch_id,b.prod_id, b.order_id, b.buyer_id, b.color_id, b.body_part_id, b.febric_description_id, b.process_id, b.fin_dia,b.booking_without_order, b.fin_gsm,b.gsm,b.width, b.roll_id, 0 as barcode_no, b.batch_issue_qty as qnty, b.roll_no, b.outbound_batchname,b.booking_no,b.booking_dtls_id,b.remarks, c.fabric_color_id, b.color_type_id from inv_receive_mas_batchroll a, pro_grey_batch_dtls b, wo_booking_dtls c where a.id=b.mst_id and c.id=b.booking_dtls_id and a.recv_number in('".$issue_number."') and a.entry_form=91 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.recv_number asc ";
	...
	$body_part_ids="";$roll_ids="";$order_ids="";$prod_ids="";$color_ids="";$booking_nos="";$febric_description_ids="";$barcode_nos="";$rcv_num_prefix_num="";
	foreach($results as $row)
	{
		$booking_without_order_status=$row[csf('booking_without_order')];
		$body_part_ids.=$row[csf('body_part_id')].',';
		...
		$febric_description_ids.="'".$row[csf('febric_description_id')]."'".',';
		$barcode_nos.="'".$row[csf('barcode_no')]."'".',';
		$rcv_num_prefix_num.=$row[csf('recv_number_prefix_num')].',';
	...
	$febric_description_id_all=chop($febric_description_ids,",");
	$barcode_no_all=chop($barcode_nos,",");
	$rcv_num_prefix_num_all = implode(',',array_unique(explode(',', chop($rcv_num_prefix_num,","))));
	$recv_number_all = implode(',',array_unique(explode(',', chop($recv_number,","))));
	...
	$filename = $PNG_TEMP_DIR.md5('fifp').'.png';
	QRcode::png($recv_number_all, $filename, $errorCorrectionLevel, $matrixPointSize, 2);
	...
	<div style="height:auto;">
		...
		<table width="1500" cellspacing="0" id="summaryTbl">
			<tr>
				<td colspan="2"><img src="<? echo $PNG_WEB_DIR.basename($filename);?>" height="100" width=""></td>
			...
			<tr class="form_caption">
                <td colspan="6" align="center" style="font-size:24px">  <span style="margin-left: -170px; ">
					<?
                    $nameArray=sql_select( "select plot_no,level_no,road_no,block_no,country_id,province,city,zip_code,email,website from lib_company where id=$company");
                    foreach ($nameArray as $result)
                    {
			...
			<td colspan="6" align="center" >
				<span style="margin-bottom: 150px; font-size:20px"><? echo $recv_number_all;?>
				</span>
			</td>
            </tr>
            <tr>
			...
			<tr>
				...
				if($showItem==1)
				{
					$showItemHeading="Job/Internal Ref.No";
				}
				else
				{
					$showItemHeading="Buyer/Style/Job/Internal Ref.No/Order";
				}
		...
		<table cellspacing="0" width="1500"  border="1" rules="all" class="rpt_table main_tbl">
			...
			<thead bgcolor="#dddddd" align="center">
				<tr>
					...
					<!-- <th width="50">UOM</th> -->
					<th width="80"><? echo $showItemHeading; ?></th>
					<? if($showItem ==1){?>
			...
			<tbody>
				...
				foreach($result as $row)
				{
					...
					<tr id="mainTr">
									...
									else
									{
										?>
											<tr><td align="center"><?
			                                	...
			                                	echo $buyer_name;
			                                	//echo $buyer_array[$job_array[$row[csf('order_id')]]['buyer_id']]; ?>&nbsp;</td></tr>
						...
						<? if($showItem ==1){?>
						<td align="center" style="word-wrap: break-word;">
						<? echo substr($row[csf("remarks")],0,45); ?>
						</td>
						<? } ?>
					</tr>
					<?
...
if($action == "populate_wo_data")
{
	...
	if($booking_type == 696)
	{
		$sql="SELECT a.id as dtls_id, a.mst_id,a.issue_date,a.fabric_desc as lib_yarn_count_deter_id,a.machine_dia as dia_width,a.body_part_id,a.machine_gg as gsm_weight,a.stitch_length,a.color_range,a.color_id,a.shade,a.proccess_loss, a.issue_qnty,a.wo_qty,a.rate,a.amount,a.remark_text,a.issue_no,a.yarn,a.issue_id, a.fso_id, a.color_type as color_type_id, a.dia_type as width_dia_type, c.job_no, c.sales_booking_no, c.style_ref_no, c.within_group, c.buyer_id, c.po_buyer, b.wo_basis, b.process_id as process, 12 as uom
		from dyeing_work_order_dtls a, dyeing_work_order_mst b, fabric_sales_order_mst c
		where a.MST_ID=b.id and a.fso_id=c.id and b.entry_form=696 and a.mst_id=$booking_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
		//echo $sql;die;
		$result=sql_select($sql);
		...
		$fso_ref_sql=sql_select("SELECT a.id, c.grouping FROM fabric_sales_order_mst a, wo_booking_dtls b, wo_po_break_down c  
		WHERE a.SALES_BOOKING_NO=b.booking_no and b.po_break_down_id=c.id and a.status_active = 1 and a.is_deleted = 0 and b.status_active=1 and b.is_deleted=0 and a.id in (".implode(',',$all_fso_arr).")");
		...
		foreach($fso_ref_sql as $row)
		{
			$po_details_array[$row[csf("id")]]['internalRef'] .=$row[csf("grouping")].',';
		}

		$previousIssueArrNew = array();
		$previousIssueRes=sql_select("SELECT a.batch_issue_qty,a.booking_no, a.booking_dtls_id  from pro_grey_batch_dtls a, inv_receive_mas_batchroll b where a.mst_id=b.id and a.booking_no ='$booking_no' and a.status_active=1 and a.is_deleted=0 and b.entry_form = 91");

		foreach($previousIssueRes as $row)
		{
			$previousIssueArrNew[$row[csf('booking_no')]][$row[csf('booking_dtls_id')]]+=$row[csf('batch_issue_qty')];
		}
		unset($previousIssueRes);

		$previousIssueReturnArrNew = array();
		$issueReturnSql=sql_select("SELECT a.batch_issue_qty,a.booking_no, a.booking_dtls_id from pro_grey_batch_dtls a, inv_receive_mas_batchroll b where a.mst_id=b.id and a.booking_no ='$booking_no' and a.status_active=1 and a.is_deleted=0 and b.entry_form = 554");

		foreach($issueReturnSql as $row3)
		{
			$previousIssueReturnArrNew[$row3[csf('booking_no')]][$row3[csf('booking_dtls_id')]]['issueReturnQnty']+=$row3[csf('batch_issue_qty')];
		}
		unset($issueReturnSql);
		...
		foreach ($result as  $row) 
		{
			...
			{
				$buyer_id= $row[csf("po_buyer")];
				$buyer_name= $buyer_name_array[$row[csf("po_buyer")]];
			}
			else
			{
				$buyer_id= $row[csf("buyer_id")];
				$buyer_name= $buyer_name_array[$row[csf("buyer_id")]];
			}

			$fabric_color=$row[csf("color_id")];
			$fso_no=$row[csf("job_no")];

			$internalRef = implode(",",array_unique(explode(",",chop($po_details_array[$row[csf("fso_id")]]['internalRef'],','))));

			$previousIssueReturn=$previousIssueReturnArrNew[$booking_no][$row[csf('dtls_id')]]['issueReturnQnty'];
			$balance = number_format($row[csf("wo_qty")],4,'.','') - ($previousIssueArrNew[$booking_no][$row[csf('dtls_id')]]-$previousIssueReturn);

			$wo_data .= $row[csf("width_dia_type")]."**".$body_part[$row[csf("body_part_id")]]."**".$row[csf("gsm_weight")]."**".$row[csf("dia_width")]."**".$fabric_des."**".$color_arr[$gmts_color]."**".$row[csf("job_no")]."**".$row[csf("fso_id")]."**".number_format($row[csf("wo_qty")],4,'.','')."**".$fabric_color."**".$row[csf("body_part_id")]."**".$row[csf("lib_yarn_count_deter_id")]."**".$row[csf("rate")]."**".$row[csf("process")]."**".$fabric_typee[$row[csf("width_dia_type")]]."**".$row[csf("dtls_id")]."**".$balance."**".$previousIssueArrNew[$booking_no][$row[csf('dtls_id')]]."**".$row[csf('uom')]."**".$buyer_name."**".$po_details_array[$row[csf("po_break_down_id")]]['job_no']."**".$fso_no."**".$buyer_id."**".$buyer_id_non_ord."**".$buyer_name_non_ord."**".$batch_no."**".$priv_current_qty."**".$batch_qnty."**".$batch_id."**".$internalRef."**".$color_arr[$fabric_color]."**".$row[csf("color_type_id")]."**".$color_type[$row[csf("color_type_id")]]."#";
		}
	...
	else
	{
		...
		{
			$sql ="SELECT a.id,b.id as dtls_id,c.body_part as body_part_id,c.color_type_id, b.printing_color_id as gmts_color_id, c.fabric_color as fabric_color_id, a.wo_no_prefix_num as booking_no_prefix_num, a.wo_no as booking_no,c.lib_yarn_count_deter_id,  a.company_id, a.buyer_id, null as job_no_prefix_num,null as style_ref_no, null as job_no, null as po_break_down_id,b.wo_qty as wo_qnty,b.aop_gsm as gsm_weight,b.aop_dia as dia_width, b.rate, 35 as process, 0 as item_category, a.aop_source as fabric_source, a.supplier_id,b.uom,b.fabric_description as fab_des_id, 2 as type
			from wo_non_ord_aop_booking_mst a,wo_non_ord_aop_booking_dtls b ,wo_non_ord_samp_booking_dtls c
			...
			union all
			SELECT a.id,b.id as dtls_id,c.body_part as body_part_id,c.color_type_id,b.gmts_color as gmts_color_id, c.fabric_color as fabric_color_id, a.prefix_num as booking_no_prefix_num, a.booking_no, c.lib_yarn_count_deter_id,  a.company_id, a.buyer_id, null as job_no_prefix_num,null as style_ref_no, null as job_no, null as po_break_down_id,b.wo_qty,  b.gsm as gsm_weight,b.dia as dia_width,b.rate, b.process_id as process, 0 as item_category, b.fabric_source, a.supplier_id,b.uom,b.fab_des_id, 2 as type from wo_non_ord_knitdye_booking_mst a,  wo_non_ord_knitdye_booking_dtl b, wo_non_ord_samp_booking_dtls c where a.id = b.mst_id and b.fab_des_id =  c.id  and b.status_active = 1 and b.is_deleted = 0 and a.booking_no = '$booking_no' and b.wo_qty>0";
		...
		if($booking_type==1 && $po_id_cond!="")
		{
			...
			foreach($data_array_sql as $row)
			{
				...
				$po_details_array[$row[csf("po_id")]]['year']=date("Y",strtotime($row[csf("insert_date")]));
				$po_details_array[$row[csf("po_id")]]['po_number']=$row[csf("po_number")];
				$po_details_array[$row[csf("po_id")]]['internalRef']=$row[csf("grouping")];
			}
		}
		//=========================================================
		...
		$previousIssueArrNew = array();
		$previousIssueRes=sql_select("select a.batch_issue_qty,a.booking_no, a.booking_dtls_id,a.id,a.order_id,a.body_part_id,a.color_id,a.febric_description_id as deter_id,a.batch_id  from pro_grey_batch_dtls a, inv_receive_mas_batchroll b
			where a.mst_id = b.id and a.booking_no ='$booking_no' and a.status_active=1 and a.is_deleted=0 and b.entry_form = 91");
		foreach($previousIssueRes as $row2)
		{

			$booking_dtls_id_arr[$row2[csf("booking_dtls_id")]]=$row2[csf("booking_dtls_id")];
			$order_id_arr[$row2[csf("order_id")]]=$row2[csf("order_id")];
			$body_part_id_arr[$row2[csf("body_part_id")]]=$row2[csf("body_part_id")];
			if($row2[csf("batch_id")]!=""){$batch_id_arr[$row2[csf("batch_id")]]=$row2[csf("batch_id")];}
		...
		$booking_dtls_ids=implode(',', $booking_dtls_id_arr);
		$order_ids=implode(',', $order_id_arr);
		$body_part_ids=implode(',', $body_part_id_arr);
		$batch_ids=implode(',', $batch_id_arr);

		$previousIssueReturnArrNew = array();
		$issueReturnSql=sql_select("select a.batch_issue_qty,a.booking_no, a.booking_dtls_id,a.id,a.order_id,a.body_part_id,a.color_id,a.febric_description_id as deter_id from pro_grey_batch_dtls a, inv_receive_mas_batchroll b
			where a.mst_id = b.id and a.booking_no ='$booking_no' and a.status_active=1 and a.is_deleted=0 and b.entry_form = 554 and a.order_id in($order_ids) and a.booking_dtls_id in($booking_dtls_ids) and a.body_part_id in($body_part_ids) and a.batch_id in($batch_ids)" );
		...
		//print_r($previousIssueReturnArrNew);
		...
		{
			$previousIssueReturn=$previousIssueReturnArrNew[$booking_no][$row[csf('dtls_id')]]['issueReturnQnty'];
			$balance = number_format($row[csf("wo_qnty")],4,'.','') - ($previousIssueArrNew[$booking_no][$row[csf('dtls_id')]]-$previousIssueReturn);
			$balance_qty = $row[csf("wo_qnty")]-($previousIssueArrNew[$booking_no][$row[csf('dtls_id')]]-$previousIssueReturn);
			...
			{*/
				//echo ",B";
				$wo_data .= $row[csf("width_dia_type")]."**".$body_part[$row[csf("body_part_id")]]."**".$row[csf("gsm_weight")]."**".$row[csf("dia_width")]."**".$fabric_des."**".$color_arr[$gmts_color]."**".$row[csf("job_no")]."**".$row[csf("po_break_down_id")]."**".number_format($row[csf("wo_qnty")],4,'.','')."**".$gmts_color."**".$row[csf("body_part_id")]."**".$row[csf("lib_yarn_count_deter_id")]."**".$row[csf("rate")]."**".$row[csf("process")]."**".$fabric_typee[$row[csf("width_dia_type")]]."**".$row[csf("dtls_id")]."**".$balance."**".$previousIssueArrNew[$booking_no][$row[csf('dtls_id')]]."**".$row[csf('uom')]."**".$po_details_array[$row[csf("po_break_down_id")]]['buyer_name']."**".$po_details_array[$row[csf("po_break_down_id")]]['job_no']."**".$po_details_array[$row[csf("po_break_down_id")]]['po_number']."**".$po_details_array[$row[csf("po_break_down_id")]]['buyer_id']."**".$buyer_id_non_ord."**".$buyer_name_non_ord."**".$batch_no."**".$priv_current_qty."**".$batch_qnty."**".$batch_id."**".$po_details_array[$row[csf("po_break_down_id")]]['internalRef']."**".$color_arr[$fabric_color]."**".$row[csf("color_type_id")]."**".$color_type[$row[csf("color_type_id")]]."#";
			//}
			//else $wo_data="";
		}
	}
...
if($action == "populate_wo_data2")
{
	...
	foreach($previousIssueRes as $row2)
	{
		...
		$previousIssueArrNew[$row2[csf('booking_no')]][$row2[csf('booking_dtls_id')]][$row2[csf('batch_id')]]+=$row2[csf('batch_issue_qty')];
		$previousIssueArrNew2[$row2[csf('order_id')]][$row2[csf('body_part_id')]][$row2[csf('color_id')]][$deter_id]+=$row2[csf('batch_issue_qty')];
	}
	...
	$po_arr_ids=implode(',', $po_arr);
	$batchDataArrNew = array();
	$sql_po_batch = "SELECT a.id,b.batch_no,b.color_id,b.id as batch_id,b.booking_no_id,b.booking_no,b.booking_without_order,b.extention_no,a.po_id,a.prod_id,a.item_description,a.body_part_id, a.width_dia_type,(a.batch_qnty) as batch_qnty from pro_batch_create_dtls a, pro_batch_create_mst b ,pro_fab_subprocess c where a.mst_id=b.id  and c.batch_id=b.id and c.load_unload_id=2 and a.po_id in($po_arr_ids) and b.id in($batch_ids) and a.status_active=1 and b.status_active=1 and c.status_active=1 order by a.id";
	// echo $sql_po_batch;die;
	$result_po_batch=sql_select($sql_po_batch);
	foreach ($result_po_batch as $row)
	{
		$fab_des=explode(",",$row[csf('item_description')]);
	...
	//echo "<pre>"; print_r($po_batch_data_arr);die;
	// echo $sql;
	...
	foreach($result as $row)
	{
		...
		$batch_qnty=0;$color_previousIssue_qty=0;
		$batch_qnty=$po_batch_data_qty_arr[$row[csf('po_break_down_id')]][$row[csf('body_part_id')]][$row[csf('fabric_color_id')]][$deter_id]['batch_qnty'];
		$batch_id=$po_batch_data_arr[$row[csf('po_break_down_id')]][$row[csf('fabric_color_id')]]['batch_id'];
		$batch_no=$po_batch_data_arr[$row[csf('po_break_down_id')]][$row[csf('fabric_color_id')]]['batch_no'];
		$batch_id_val=rtrim($batch_id,',');
		$batch_id_array=array_unique(explode(',',$batch_id_val));
		//var_dump($batch_id_array);
		foreach ($batch_id_array as $batch_id_key => $batchId)
		{
			$batch_number=$batch_no_arr[$batchId];
			if($batch_qnty==0 || $batch_qnty=="") $batch_qnty=0;else $batch_qnty=$batch_qnty;
			if($batch_qnty>0)
			{
				$batch_qnty=0;
			...
			$balance_qty = $batch_qnty-$previousIssueArrNew[$booking_no][$row[csf('dtls_id')]][$batchId];

			$color_previousIssue_qty=$previousIssueArrNew2[$row[csf('po_break_down_id')]][$row[csf('body_part_id')]][$row[csf('fabric_color_id')]][$deter_id];
			$priv_current_qty=$previousIssueArrNew[$booking_no][$row[csf('dtls_id')]][$batchId];
			//echo $priv_current_qty.'='.$batch_qnty.'='.$color_previousIssue_qty.'='.$balance_qty.'<br>';

			//	echo $batch_no.'GG'.$batch_id;
			if($row[csf("process")]==35) //AOP
			{
				// echo $row[csf('dtls_id')].'<br>';
				$balance = number_format($batch_qnty,4,'.','') - $previousIssueArrNew[$booking_no][$row[csf('dtls_id')]][$batchId];
				$batch_qnty=$batch_qnty;
			}
			else
			{ //For All
				$balance = number_format($row[csf("wo_qnty")],4,'.','') - $previousIssueArrNew[$booking_no][$row[csf('dtls_id')]][$batchId];
			...
			if($booking_type == 1)
			{
				...
				if ($row[csf("gmts_color_id")]=="")
				{
					$gmts_color=$row[csf("fabric_color_id")];
				}
				else
				{
					$gmts_color=$row[csf("gmts_color_id")];
			...
			$wo_data .= $row[csf("width_dia_type")]."**".$body_part[$row[csf("body_part_id")]]."**".$row[csf("gsm_weight")]."**".$row[csf("dia_width")]."**".$fabric_des."**".$color_arr[$gmts_color]."**".$row[csf("job_no")]."**".$row[csf("po_break_down_id")]."**".number_format($batch_qnty,4,'.','')."**".$gmts_color."**".$row[csf("body_part_id")]."**".$row[csf("lib_yarn_count_deter_id")]."**".$row[csf("rate")]."**".$row[csf("process")]."**".$fabric_typee[$row[csf("width_dia_type")]]."**".$row[csf("dtls_id")]."**".$balance."**".$previousIssueArrNew[$booking_no][$row[csf('dtls_id')]][$batchId]."**".$row[csf('uom')]."**".$po_details_array[$row[csf("po_break_down_id")]]['buyer_name']."**".$po_details_array[$row[csf("po_break_down_id")]]['job_no']."**".$po_details_array[$row[csf("po_break_down_id")]]['po_number']."**".$po_details_array[$row[csf("po_break_down_id")]]['buyer_id']."**".$buyer_id_non_ord."**".$buyer_name_non_ord."**".$batch_number."**".$priv_current_qty."**".$batch_qnty."**".$batchId."**".$po_details_array[$row[csf("po_break_down_id")]]['internalRef']."**".$color_arr[$fabric_color]."#";
		}
...
if($action=="issue_multy_number_popup")
{
	...
	{
		if(document.getElementById('chk_getpass').value==0) document.getElementById('chk_getpass').value=1;
		else document.getElementById('chk_getpass').value=0;
	}
 </script>
			...
			<table width="930" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
				<thead>
					<tr>
						...
						<th width="80">Gate Pass Pending</th>
				...
				<tbody>
					<tr>
						...
						<td align="center">
							<input type="checkbox" value="1" onClick="set_checkvalue()" id="chk_getpass" checked>
						</td>
						<td align="center">
							<input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $company; ?>+'_'+document.getElementById('cbo_service_company_name').value+'_'+<? echo $source; ?>+'_'+<? echo $service_company; ?>+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('chk_getpass').value, 'create_multy_issue_search_list_view', 'search_div', 'fabric_issue_to_finishing_process_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />
						</td>
					</tr>
					<tr>
...
if($action=="create_multy_issue_search_list_view")
{
	...
	if($check_type==1)
	{
		$sql = "SELECT a.id, $year_field a.recv_number_prefix_num,a.recv_number, a.dyeing_source, a.company_id,a.dyeing_company, a.receive_date, b.booking_without_order,sum(b.batch_issue_qty) as batch_issue_qty, b.booking_no
		from inv_receive_mas_batchroll  a,pro_grey_batch_dtls b where a.company_id=$company and a.dyeing_company=$cbo_service_company_name and a.entry_form=91 and a.status_active=1 and a.is_deleted=0 and a.id = b.mst_id and a.recv_number not in(select challan_no from inv_gate_pass_mst where status_active=1 and is_deleted=0 and challan_no is not null) and b.status_active=1 and b.is_deleted=0 and a.dyeing_source=$source $search_field_cond $date_cond  $year_cond
		group by a.id, a.insert_date, a.recv_number_prefix_num, a.recv_number, a.dyeing_source,a.company_id, a.dyeing_company, a.receive_date,b.booking_without_order, b.booking_no
		order by a.id";
	}
	else
	{
		$sql = "SELECT a.id, $year_field a.recv_number_prefix_num,a.recv_number, a.dyeing_source, a.company_id,a.dyeing_company, a.receive_date, b.booking_without_order,sum(b.batch_issue_qty) as batch_issue_qty, b.booking_no
		from inv_receive_mas_batchroll  a,pro_grey_batch_dtls b, inv_gate_pass_mst c  where a.company_id=$company and a.dyeing_company=$cbo_service_company_name and a.entry_form=91 and a.status_active=1 and a.is_deleted=0 and a.id = b.mst_id and a.recv_number = c.challan_no and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.dyeing_source=$source $search_field_cond $date_cond  $year_cond
		group by a.id, a.insert_date, a.recv_number_prefix_num, a.recv_number, a.dyeing_source,a.company_id, a.dyeing_company, a.receive_date,b.booking_without_order, b.booking_no
		order by a.id";
	}
	//echo $sql;
	...
	$arr=array(1=>$supplier_arr,4=>$company_arr);
	echo create_list_view("list_view", "WO, Supplier,Challan/System No, Year, Company Name, Issue Date, Issue Qty","100,100,100,40,150,100","750","230",0, $sql , "js_set_value", "recv_number,dyeing_source,id", "1", 1, "0,dyeing_company,0,0,company_id,0,0", $arr, "booking_no,dyeing_company,recv_number,year,company_id,receive_date,batch_issue_qty","","","0,0,0,0,0,3,1","",1) ;	//
...
if($action=="subprocess_issue_print5")
{
	extract($_REQUEST);
	$data=explode('*',$data);
	...
	$sql = "SELECT  b.batch_id,b.prod_id, b.order_id, b.buyer_id, b.color_id, b.body_part_id, b.febric_description_id, b.process_id, b.fin_dia,b.booking_without_order, b.fin_gsm,b.gsm,b.width, b.roll_id, b.batch_issue_qty as qnty, roll_no, b.outbound_batchname,b.booking_no,b.booking_dtls_id, b.remarks, cast(c.fabric_color_id as varchar2(100)) as fabric_color_id, b.color_type_id,b.is_sales from inv_receive_mas_batchroll a, pro_grey_batch_dtls b, wo_booking_dtls c where a.id=b.mst_id and a.id=$update_id and b.booking_dtls_id=c.id and a.entry_form=91 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.batch_issue_qty>0 and b.is_sales is null
	union all
	SELECT  b.batch_id,b.prod_id, b.order_id, b.buyer_id, b.color_id, b.body_part_id, b.febric_description_id, b.process_id, b.fin_dia,b.booking_without_order, b.fin_gsm,b.gsm,b.width, b.roll_id, b.batch_issue_qty as qnty, roll_no, b.outbound_batchname,b.booking_no,b.booking_dtls_id, b.remarks, b.color_id as fabric_color_id, b.color_type_id,b.is_sales from inv_receive_mas_batchroll a, pro_grey_batch_dtls b, dyeing_work_order_dtls c where a.id=b.mst_id and a.id=$update_id and b.booking_dtls_id=c.id and a.entry_form=91 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.batch_issue_qty>0 and b.is_sales=1";

	//echo $sql;die;
	$results=sql_select($sql);

	$body_part_ids="";$roll_ids="";$order_ids="";$fso_ids="";$prod_ids="";$color_ids="";$booking_nos="";$febric_description_ids="";$barcode_nos="";
	foreach($results as $row)
	{
		$booking_without_order_status=$row[csf('booking_without_order')];
		$body_part_ids.=$row[csf('body_part_id')].',';
	...
	if($order_id_all !="")
	{
		...
		foreach($job_sql_result as $row)
		{
			...
			$job_array[$row[csf('id')]]['po']=$row[csf('po_number')];
			$job_array[$row[csf('id')]]['internal_ref']=$row[csf('internal_ref')];
			$job_array[$row[csf('id')]]['ir_no']=$row[csf('grouping')];
		}
	}

	if($fso_id_all !="")
	{
		$fso_ref_sql=sql_select("SELECT a.id, a.job_no, c.grouping, a.style_ref_no, a.within_group, a.po_buyer, a.buyer_id FROM fabric_sales_order_mst a, wo_booking_dtls b, wo_po_break_down c  
		WHERE a.SALES_BOOKING_NO=b.booking_no and b.po_break_down_id=c.id and a.status_active = 1 and a.is_deleted = 0 and b.status_active=1 and b.is_deleted=0 and a.id in ($fso_id_all)");
		...
		foreach($fso_ref_sql as $row)
		{
			...
			$job_array[$row[csf('id')]]['ir_no'].=$row[csf("grouping")].',';
			$job_array[$row[csf('id')]]['style_ref_no']=$row[csf("style_ref_no")];
			$job_array[$row[csf('id')]]['buyer_id']=$buyer_id;
		}
	}
	...
	<div>
		...
		<table cellspacing="0" width="1220"  border="1" rules="all" class="rpt_table" >
			<thead bgcolor="#dddddd" align="center">
				<tr>
					...
					<th width="140">Buyer</th>
					<th width="140">Style</th>
					<th width="140">Service WO No</th>
					<th width="180">Int.Ref </th>
                    <th width="150">Batch No </th>
					<th width="120">Body Part</th>
					<th width="100">Color Type</th>
					<th width="170">Const./Compo</th>
			...
			<tbody>
				...
				foreach($result as $row)
				{
					...
					$ir_no = implode(",",array_unique(explode(",",chop($job_array[$row[csf('order_id')]]['ir_no'],','))));
					?>
				...
				<tfoot>
					<tr>
						...
						<th align="right"><? if($total_weight!=0){echo number_format($total_weight,2);} ?></th>
						<th><? echo number_format($total_roll,2);?></th>
						<th>&nbsp;</th>
						<th>&nbsp;</th>
					</tr>
				</tfoot>
		...
		<? echo signature_table(145, $company, "1220px",'','5px'); ?>
		<script type="text/javascript" src="../../../js/jquery.js"></script>
		<script type="text/javascript" src="../../../js/jquerybarcode.js"></script>
		<script>
			function generateBarcode( valuess )
