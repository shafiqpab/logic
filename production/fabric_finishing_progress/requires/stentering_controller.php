...
if($action=="load_drop_down_knitting_com_new")
{
	...
	{	
		...
		echo create_drop_down( "cbo_service_company", 135, "$sql","id,supplier_name", 1, "-- Select --", 0, "load_drop_down('requires/stentering_controller', this.value, 'load_drop_floor', 'floor_td' );" );
	}
	else
	{
		echo create_drop_down( "cbo_service_company", 135, $blank_array,"",1, "-- Select --", 0, "" );
...
if($action=="create_batch_search_list_view")
{
	...
	if($search_type==1)
	{
		if ($batch_no!='') { $batch_cond=" and a.batch_no='$batch_no'";$batch_cond2=" and b.batch_no='$batch_no'";} else { $batch_cond="";$batch_cond2="";}
	}
	else if($search_type==4 || $search_type==0)
	{
		if ($batch_no!='') { $batch_cond=" and a.batch_no like '%$batch_no%'";$batch_cond2=" and b.batch_no like '%$batch_no%'";} else { $batch_cond="";$batch_cond2="";}
	}
	else if($search_type==2)
	{
		if ($batch_no!='') { $batch_cond=" and a.batch_no like '$batch_no%'"; $batch_cond2=" and b.batch_no like '$batch_no%'";} else {$batch_cond="";$batch_cond2="";}
	...
	{	
		...
		{ 
			$batch_date_con = " and a.batch_date between '".change_date_format($start_date, "mm-dd-yyyy", "-",1)."' and '".change_date_format($end_date, "mm-dd-yyyy", "-",1)."'";
	...
	//echo $sql;//die;
	 $sql = "select a.id, a.batch_no, a.batch_date, a.batch_weight, a.booking_no, a.extention_no,a.sales_order_id, a.color_id, a.batch_against, a.re_dyeing_from,a.is_sales from pro_batch_create_mst a  where a.batch_for in(0,1) and a.batch_against<>4 and a.status_active=1 and a.is_deleted=0 $search_field_cond_batch $batch_date_con $batch_cond  order by a.id desc"; 
...
$nameArray_prod=sql_select( $sql_prod );
				...
				foreach ($nameArray as $selectResult)
				{
					...
					if($unload_batch_dyeing_chk_arr[$selectResult[csf('id')]]>0) //Unload check
					{
					$po_no='';
					...
					else
					{
						...
						//{
							...
							{
								//if($unload_batch_dyeing_chk_arr[$selectResult[csf('id')]]>0) //Unload check
								if($unload_batch_dyeing_chk_arr[$row[csf('id')]]>0 && $row[csf('re_dyeing_from')]==0 || 1==1)
								{
...
if($action=="populate_restenter_from_data") 
{
	$ex_data=explode("_",$data);
		...
		//$tot_batch_qty=$tot_prod_prev_qty=0;
		 ...
		 $re_stenter=return_field_value("max(re_stenter_no) as re_stenter_no"," pro_fab_subprocess","batch_id=".$ex_data[0]." and is_deleted=0 and status_active=1 and entry_form=48","re_stenter_no");
...
if($action=="populate_data_from_data")
{
	...
	{		
		...
		echo "load_drop_down( 'requires/stentering_controller', ".$row[csf("dyeing_source")]."+'**'+".$row[csf("company_id")].", 'load_drop_down_knitting_com_new', 'dyeing_company_td' );\n";
...
{ 
	...
	else if($db_type==2) $select_group_row="and  rownum<=1 group by a.id,a.sales_order_id,a.batch_no,a.batch_weight,a.color_id, 
	...
	else if($db_type==2) $pop_batch=" group by a.id,a.batch_no,a.sales_order_id, a.batch_weight,batch_date,a.color_id,a.color_range_id,a.insert_date, a.booking_without_order,a.company_id,a.process_id,a.entry_form,a.booking_no,a.total_trims_weight  order by a.id";
	...
	if($batch_no!=''){ 
	$data_array=sql_select("SELECT a.id,a.batch_no,a.sales_order_id,a.entry_form,a.batch_date,a.total_trims_weight, a.batch_weight,MAX(a.extention_no) as extention_no,a.company_id,a.process_id as process_id_batch,a.booking_no, 
	...
	{
	$data_array=sql_select("SELECT a.id,a.batch_no,a.sales_order_id,a.entry_form, a.batch_weight,Max(a.extention_no) as extention_no,a.company_id,a.batch_date,a.total_trims_weight,a.process_id as process_id_batch,a.booking_no,
...
{	
	...
	*/
	...
	$sql_result=sql_select("SELECT a.entry_form,b.width_dia_type,b.gsm,b.fin_dia,b.item_description, sum(b.batch_qnty) as batch_qty,b.prod_id,b.barcode_no,b.roll_id,b.roll_no 
	from pro_batch_create_dtls b,pro_batch_create_mst a  
	where b.mst_id='$batch_id'  and b.width_dia_type in(1,3) and a.id=b.mst_id and a.id='$batch_id' and  a.entry_form in(0,36) and b.status_active=1 and b.is_deleted=0  
	group by b.id,b.item_description,b.gsm,b.fin_dia,b.width_dia_type,b.prod_id,a.entry_form,b.roll_id,b.barcode_no,b.roll_no order by b.item_description");
	$i=1;
	//	 echo $page_upto_id.'='.$roll_maintained;
	//if($roll_maintained==1)
	if(($page_upto_id==4 || $page_upto_id>4) && $roll_maintained==1)
	{
		...
		if(count($sql_result)>0)
		{
			...
			foreach($sql_result as $row)
			{
				...
				//$roll_no=$fabric_roll_arr[$row[csf('roll_id')]]['roll'];
				//$barcode=$fabric_roll_arr[$row[csf('roll_id')]]['barcode'];
				$barcode=$fabric_roll_arr[$row[csf('roll_id')]]['barcode_no'];
				$roll_no=$fabric_roll_arr[$row[csf('roll_id')]]['roll'];		
			?>
	    	<tr class="general" id="row_<? echo $i; ?>">
	       		<td width="40" id="sl_<? echo $i; ?>"><input type="checkbox" id="checkRow_<? echo $i; ?>" name="checkRow[]"  checked /> &nbsp; &nbsp;<? echo $i; ?></td>
	...
	else 
	{
		...
		{	
			...
			$result=sql_select("SELECT a.id as batch_id,a.entry_form,b.width_dia_type,b.gsm,b.fin_dia,b.item_description, sum(b.batch_qnty) as batch_qty,b.prod_id,count(b.roll_no) as roll_no from pro_batch_create_dtls b,pro_batch_create_mst a  where b.mst_id='$batch_id'  and b.width_dia_type in(1,2,3) and a.id=b.mst_id and  a.entry_form in(0,36) and b.status_active=1 and b.is_deleted=0 group by a.id,a.entry_form,b.width_dia_type,b.gsm,b.fin_dia,b.item_description,b.prod_id order by b.item_description ");
			...
			foreach($result as $row)
			{
				...
				if($row[csf('entry_form')]==36)
					...
					//echo $row[csf('batch_id')].'d'.$row[csf('prod_id')].'d'.$row[csf('roll_no')].'d'.$row[csf('width_dia_type')].'d'.$dia_width.'d'.$gsm;
					if(!$prod_prev_qty) $prod_prev_qty=$row[csf('batch_qty')];
				?>
		    	<tr class="general" id="row_<? echo $i; ?>">
		        	<td><input type="checkbox" id="checkRow_<? echo $i; ?>" name="checkRow[]"  checked /> &nbsp; &nbsp;<? echo $i; ?></td>
		            <td><input type="text" name="txtconscomp_<? echo $i; ?>" id="txtconscomp_<? echo $i; ?>" class="text_boxes" style="width:170px;" value="<? echo $cons_comps; ?>" disabled/></td>
		            ...
		            <td><input type="text" name="txtproductionqty_<? echo $i; ?>" id="txtproductionqty_<? echo $i; ?>" onKeyUp="calculate_production_qnty();"  value="<? echo $prod_prev_qty; ?>"  class="text_boxes_numeric" style="width:60px;"/></td>
		            <td><input type="text" name="txtrate_<? echo $i; ?>" id="txtrate_<? echo $i; ?>" class="text_boxes_numeric" style="width:50px;" readonly/></td>
					<td><input type="text" name="txtamount_<? echo $i; ?>" id="txtamount_<? echo $i; ?>" class="text_boxes_numeric" style="width:70px;"  readonly/> </td>
		        </tr>
		    	<?
...
if($action=="save_update_delete")
{
	...
	{ 
		...
		$field_array_update="company_id*batch_no*previous_process*service_source*service_company*received_chalan*issue_chalan*issue_challan_mst_id*re_stenter_no*batch_id*batch_ext_no*process_id*production_date*process_start_date*start_hours*start_minutes*end_hours*end_minutes*machine_id*floor_id*temparature 	*stretch*over_feed*feed_in*pinning*speed_min*process_end_date*chemical_name*next_process_id*advanced_prod_qty*is_re_dyeing*shift_name*remarks*booking_no*result*updated_by*update_date";
		$data_array_update="".$cbo_company_id."*".$txt_batch_no."*".$cbo_previous_process."*".$cbo_service_source."*".$cbo_service_company."*".$txt_recevied_chalan."*".$txt_issue_chalan."*".$txt_issue_mst_id."*".$txt_restenter_no."*".$txt_batch_ID."*".$txt_ext_id."*".$cbo_sub_process."*".$txt_process_date."*".$txt_process_start_date."*".$txt_start_hours."*".$txt_start_minutes."*".$txt_end_hours."*".$txt_end_minutes."*".$cbo_machine_name."*".$cbo_floor."*".$txt_temparature."*".$txt_stretch."*".$txt_feed."*".$txt_feed_in."*".$txt_pinning."*".$txt_speed."*".$txt_process_end_date."*".$txt_chemical."*".$cbo_next_process."*".$txt_advance_prod."*".$re_checkbox."*".$cbo_shift_name."*".$txt_remarks."*".$txt_booking_no."*".$cbo_result_name."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
...
$sql_re_stent=sql_select("select max(re_stenter_no) as re_stenter_no_max,min(re_stenter_no) as re_stenter_no_min from pro_fab_subprocess where entry_form=48 and batch_id='".$sql_result[0][csf('batch_id')]."' and status_active = 1 and is_deleted = 0");
//echo "select max(re_stenter_no) as re_stenter_no_max,min(re_stenter_no) as re_stenter_no_min from pro_fab_subprocess where batch_id='".$sql_result[0][csf('batch_id')]."'  and status_active = 1 and is_deleted = 0";
...
if ($action=="create_booking_search_list_view")
{
	...
	$sql_booking= sql_select("select f.lib_yarn_count_deter_id,d.pre_cost_fabric_cost_dtls_id,d.po_break_down_id,sum(d.amount) as amount, sum(d.wo_qnty) as wo_qnty,d.booking_no  from wo_pre_cost_fab_conv_cost_dtls e,wo_pre_cost_fabric_cost_dtls f, wo_booking_dtls d,wo_booking_mst a, wo_po_details_master b where e.job_no=f.job_no and f.id=e.fabric_description and e.id=d.pre_cost_fabric_cost_dtls_id and d.booking_no=a.booking_no and a.job_no=b.job_no and a.booking_type=3 and a.supplier_id=$supplier_id and  a.status_active=1 and a.is_deleted=0 and d.process=$process_id $sql_cond group by d.po_break_down_id,d.booking_no,d.pre_cost_fabric_cost_dtls_id,f.lib_yarn_count_deter_id ");
	...
	$sql_po=sql_select("select b.id, a.job_no_prefix_num,b.po_number from wo_po_details_master a, wo_po_break_down b where a.id=b.job_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.id in(".implode(",",$poIdArr).")");
	foreach($sql_po as $val)
	{
	$job_no_arr[$val[csf('id')]]=$val[csf('job_no_prefix_num')];
