...
if($action=="populate_restenter_from_data_old")
{
...
if($action=="populate_restenter_from_data") 
{
	$ex_data=explode("_",$data);
		...
		//$tot_batch_qty=$tot_prod_prev_qty=0;
		 ...
		 $re_stenter=return_field_value("max(re_stenter_no) as re_stenter_no"," pro_fab_subprocess","batch_id=".$ex_data[0]." and is_deleted=0 and status_active=1 and entry_form=31","re_stenter_no");
...
if($action=="create_batch_search_list_view")
{
  ...
  $sql_prod = "select b.id, b.batch_no, a.result from pro_batch_create_mst b,pro_fab_subprocess a  where b.id=a.batch_id and a.entry_form in(35,38) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and a.load_unload_id=2  $batch_date_con2  $batch_cond2 order by b.id desc"; 
$nameArray_prod=sql_select( $sql_prod );
	...
	$sql = "SELECT id, batch_no, batch_date, batch_weight, booking_no, extention_no,sales_order_id, color_id, batch_against, re_dyeing_from,is_sales from pro_batch_create_mst where batch_for in(0,1) and batch_against<>4 and status_active=1 and is_deleted=0 $search_field_cond_batch $batch_date_con $batch_cond order by id desc"; 
	//echo $sql;//die;
	?>
    <div>
        ...
        <div style="width:770px; overflow-y:scroll; max-height:230px;" id="buyer_list_view" align="center">
            ...
            <?
				...
				foreach ($nameArray as $selectResult)
				{
					...
					?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $selectResult[csf('id')].'_'.$is_sales.'_'.$selectResult[csf('batch_no')]; ?>')"> 
					...
					else
					{
						...
						foreach($dataArray as $row)
						{
							...
							if($unload_batch_dyeing_chk_arr[$row[csf('id')]]>0 && $row[csf('re_dyeing_from')]==0)
							{
								$po_no=implode(",",array_unique(explode(",",$po_num[$selectResult[csf('id')]]['po_no'])));
								?>
...
if(count($sales_idArr)>0)
 ...
 $re_dryer_no=return_field_value("re_stenter_no","pro_fab_subprocess","batch_id ='$batch_id' and entry_form=31  and is_deleted=0 and status_active=1");
	foreach ($data_array as $row)
	{ 
		...
		if($re_dryer_no==0 || $re_dryer_no=="") $re_dryer_no=0;else $re_dryer_no=$re_dryer_no;
	...
	//	echo "load_drop_down( 'requires/drying_stentering_controller', '".$row[csf("company_id")]."', 'load_drop_floor', 'floor_td' );\n";
		echo "document.getElementById('txt_restenter_no').value 				= '".$re_dryer_no."';\n";
...
{	
	...
	$sql_result=sql_select("SELECT a.entry_form,b.width_dia_type,b.item_description,b.gsm,b.fin_dia, sum(b.batch_qnty) as batch_qty,b.prod_id,b.barcode_no,b.roll_id,b.roll_no 
	from pro_batch_create_dtls b,pro_batch_create_mst a  
	where b.mst_id='$batch_id'  and b.width_dia_type in(1,2,3) and a.id=b.mst_id and a.id='$batch_id' and  a.entry_form in(0,36) and b.status_active=1 and b.is_deleted=0  
	group by b.id,b.item_description,b.gsm,b.fin_dia,b.width_dia_type,b.prod_id,a.entry_form,b.barcode_no,b.roll_id,b.roll_no order by b.item_description asc ");
	$i=1;
...
if($action=="save_update_delete")
{
	...
	//echo "10**select a.batch_id,max(a.id) as mst_id from pro_fab_subprocess a where  a.batch_id=$txt_batch_ID and a.company_id=$cbo_company_id and a.status_active=1 $up_id_cond group by a.batch_id ";die;
		...
		//echo "10**select a.batch_id,a.end_hours,a.end_minutes,a.process_end_date,a.production_date from pro_fab_subprocess a where  a.batch_id=$txt_batch_ID and a.company_id=$cbo_company_id and a.id in($last_mst_id) and a.id!=$updateId  and a.status_active=1  ";die;
		foreach($batch_last_process as $row)
		{
			$previ_process_end_date= $row[csf('production_date')];
			$previ_end_hours= $row[csf('end_hours')];
	...
	if ($operation==0)  // Insert Here
	{
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");  
				echo "0**".str_replace("'","",$mst_update_id)."**".str_replace("'","",$txt_restenter_no);
			}
			else
			{
	...
	{ 
		...
		if(count($data_array_up)>0)
			{
				...
				//echo "10**".$$updateiddtls.'_'.$roll_maintained."**".bulk_update_sql_statement("pro_fab_subprocess_dtls", "id",$field_array_up,$data_array_up,$id_arr );die;
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");  
				echo "1**".str_replace("'","",$update_id)."**".str_replace("'","",$txt_restenter_no);
			}
			else
			{
				mysql_query("ROLLBACK"); 
...
if($action=="show_dtls_list_view")
{
	...
	if($db_type==2)
	{
		...
		$sql = "SELECT a.batch_id, a.re_stenter_no,a.id,b.const_composition,b.gsm ,b.roll_no ,b.dia_width,b.width_dia_type, b.roll_id ,b.batch_qty,b.production_qty,b.no_of_roll, b.prod_id, b.barcode_no
		from pro_fab_subprocess a,pro_fab_subprocess_dtls b 
		where a.id=b.mst_id and a.batch_id=$data and a.entry_form=31 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1  ";	
	}
	else
	{
		...
		from pro_fab_subprocess a,pro_fab_subprocess_dtls b 
		where a.id=b.mst_id and a.batch_id=$data and a.entry_form=31 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1  group by a.re_stenter_no,a.id";	
	}
	// echo $sql;
	$result = sql_select($sql);
	$sql_data_arr=array();
	foreach($result as $row)
	{
		$sql_data_arr[$row[csf("id")]]["re_stenter_no"]=$row[csf("re_stenter_no")];
		$sql_data_arr[$row[csf("id")]]["const_composition"].=$row[csf("const_composition")].",";
		$sql_data_arr[$row[csf("id")]]["gsm"].=$row[csf("gsm")].",";
		$sql_data_arr[$row[csf("id")]]["roll_no"].=$row[csf("roll_no")].",";
		$sql_data_arr[$row[csf("id")]]["dia_width"].=$row[csf("dia_width")].",";
		$sql_data_arr[$row[csf("id")]]["width_dia_type"].=$row[csf("width_dia_type")].",";
		$sql_data_arr[$row[csf("id")]]["roll_id"].=$row[csf("roll_id")].",";
		$sql_data_arr[$row[csf("id")]]["batchId"]=$row[csf("batch_id")];
		
		if ($checkBatch2[$row[csf("batch_id")]][$row[csf("barcode_no")]][$row[csf("prod_id")]][$row[csf("gsm")]][$row[csf("dia_width")]][$row[csf("width_dia_type")]]=="") 
		{
			$sql_data_arr[$row[csf("id")]]["tot_batch_qty"]+=$row[csf("batch_qty")];
			$checkBatch2[$row[csf("batch_id")]][$row[csf("barcode_no")]][$row[csf("prod_id")]][$row[csf("gsm")]][$row[csf("dia_width")]][$row[csf("width_dia_type")]]=$row[csf("batch_id")].'='.$row[csf("barcode_no")].'='.$row[csf("prod_id")].'='.$row[csf("gsm")].'='.$row[csf("dia_width")].'='.$row[csf("width_dia_type")];
		}
		$sql_data_arr[$row[csf("id")]]["batch_qty"]+=$row[csf("batch_qty")];
		$sql_data_arr[$row[csf("id")]]["production_qty"]+=$row[csf("production_qty")];
		$sql_data_arr[$row[csf("id")]]["no_of_roll"]+=$row[csf("no_of_roll")];
	}
	//echo "<pre>"; print_r($checkBatch2); echo "</pre>";
	$i=1;
	$total_batch_qty=0;
	$total_prod_qty=0;
	?>     	
    <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" width="800" rules="all">
        ...
        <tbody>
        	<?
				...
				$roll_maintained=return_field_value("fabric_roll_level","variable_settings_production","company_name ='$company_id' and item_category_id=50 and variable_list=3 and is_deleted=0 and status_active=1");
        	//$checkBatch = array();
        	foreach ($sql_data_arr as $key => $row) 
        	{
				if ($i%2==0)$bgcolor="#E9F3FF";						
				else $bgcolor="#FFFFFF";
			

				/*if(!in_array($row['batch_id'], $checkBatch))
				{
					$total_batch_qty +=	$row["batch_qty"];
					$checkBatch[] =	$row['batch_id'];
				}*/
				...
				{
					$roll_no=$row["no_of_roll"];
				}
				?>
            	<tr bgcolor="<? echo $bgcolor; ?>" onClick='show_list_view("<? echo $key;?>","child_form_input_data","list_fabric_desc_container","requires/drying_stentering_controller");get_php_form_data("<? echo $key;?>","mst_id_child_form_input_data","requires/drying_stentering_controller")'  style="cursor:pointer" >
                    <td width="30"><? echo $i; ?></td>
                    <td width="80"><p><? echo chop($cons_composition_cond,","); ?></p></td>
                    <td width="80"><p><? echo chop($gsm_cond,","); ?></p></td>
                    <td width="70"><p><? echo chop($dia_width_cond,","); ?></p></td>
                    <td width="130"><p><? echo   chop($dia_type,",");//$row[csf("width_dia_type")] ; ?></p></td>
                    <td width="100"><p><? echo $row['re_stenter_no'];  ?></p></td>
                    <td width="80" align="right"><p><? echo number_format($row["batch_qty"],2); ?></p></td>
                    <td width="80" align="right"><p><? echo number_format($row["production_qty"],2); ?></p></td>
               </tr>
            	<? 
            	$i++; 
            	$total_batch_qty +=	$row["tot_batch_qty"];
        	} 
        	?>
        	<tfoot>
...
{
	$sql_result=sql_select("SELECT a.re_stenter_no,a.id,a.company_id,a.batch_id,a.service_source,a.next_process_id,a.advanced_prod_qty,a.is_re_dyeing,a.service_company,a.received_chalan,a.issue_chalan,issue_challan_mst_id, a.process_end_date,a.production_date,a.process_start_date,a.previous_process,a.batch_no,a.process_id,a.end_hours,a.end_minutes,a.start_hours,a.temparature,a.stretch,a.over_feed,a.feed_in,a.pinning,a.speed_min,a.floor_id,a.machine_id,a.result,a.shift_name,a.remarks,a.booking_no,b.currency_id,b.exchange_rate from pro_fab_subprocess a, pro_fab_subprocess_dtls b where a.id=b.mst_id and a.id=$data and entry_form=31 ");
	...
	$sql_re_stent=sql_select("select max(re_stenter_no) as re_stenter_no_max,min(re_stenter_no) as re_stenter_no_min from pro_fab_subprocess where entry_form=31 and batch_no='".$sql_result[0][csf('batch_no')]."' and status_active = 1 and is_deleted = 0");
    $re_dry_no_max=$sql_re_stent[0][csf('re_stenter_no_max')];
   // $re_slitting_no_min=$sql_re_stent[0][csf('re_stenter_no_min')];

	if($re_dry_no_max)
	{
		$re_dry_from = $re_dry_no_max;
	}else{
		$re_dry_from = "0";
	}
	...
	echo "document.getElementById('txt_restenter_no').value	= '".$re_dry_from."';\n";
	echo "document.getElementById('re_stenter_from').value	= '".$re_dry_from."';\n";
	...
	echo "load_drop_down( 'requires/drying_stentering_controller', ".$sql_result[0][csf("service_source")]."+'**'+".$sql_result[0][csf("company_id")].", 'load_drop_down_knitting_com_new', 'dyeing_company_td' );\n";
