...
if ($action == "load_drop_floor") {
	...
	echo create_drop_down("cbo_floor", 135, "select id,floor_name from lib_prod_floor where status_active=1 and is_deleted=0 and company_id='$data[0]'  and production_process=3  order by floor_name", "id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/dyeing_production_for_woven_textile_entry_controller', document.getElementById('cbo_service_company').value+'**'+this.value, 'load_drop_machine', 'machine_td' );");
	exit();
}
...
if ($action == "create_batch_search_list_view") {
	...
	if ($batch_type == 0)
		$search_field_cond_batch = "and a.entry_form in (0,36,563)"; 
	else if ($batch_type == 1)
		$search_field_cond_batch = "and a.entry_form=563";//and a.entry_form=0
...
if ($action == 'populate_data_from_batch') {
	...
	//and a.id not in(select batch_id from pro_fab_subprocess where load_unload_id in(1) and entry_form=35 and status_active=1)
		if ($batch_no != '') {
			...
			$sql_re = "select a.id as id,a.double_dyeing,a.batch_no,b.is_sales, a.entry_form,a.company_id,a.working_company_id, a.batch_weight,Max(a.extention_no) as extention_no,a.process_id as process_id_batch, a.color_id, a.booking_without_order,a.booking_no, a.sales_order_no,
				...
				a.id='$batch_id'   and a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and a.status_active=1  and a.is_deleted=0  $select_field1";
			}
			else //For Trim Batch
			{
	...
	if($load_unload_arr[$row[csf("id")]]=="")
	{
		...
		echo "load_drop_down( 'requires/dyeing_production_for_woven_textile_entry_controller', " . $inhouse . "+'**'+" . $row[csf("working_company_id")] . ", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
		...
		echo "load_drop_down( 'requires/dyeing_production_for_woven_textile_entry_controller', '" . $row[csf("working_company_id")] . "', 'load_drop_floor', 'floor_td' );\n";
		echo "fnc_mc_load();\n";
		if($floor_id>0)
		{
		...
		if ($salesOrder == 1) {
			...
			if($floor_id>0)
			{
			echo "load_drop_down( 'requires/dyeing_production_for_woven_textile_entry_controller', $working_company_id+'**'+$floor_id, 'load_drop_machine', 'machine_td' );\n";
			}
			if($dyeing_machine>0)
			{
			echo "document.getElementById('cbo_machine_name').value 			= '" . $dyeing_machine . "';\n";
		...
		foreach ($sql_batch_d as $dyeing_d) {//$minute=str_pad($r_batch[csf("end_minutes")],2,'0',STR_PAD_LEFT);
			...
			echo "load_drop_down( 'requires/dyeing_production_for_woven_textile_entry_controller', " . $dyeing_d[csf("service_source")] . "+'**'+" . $dyeing_d[csf("company_id")] . ", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
			echo "$('#cbo_service_company').val(" . $dyeing_d[csf("service_company")] . ");\n";
			echo "load_drop_down( 'requires/dyeing_production_for_woven_textile_entry_controller', '" . $dyeing_d[csf("service_company")] . "', 'load_drop_floor', 'floor_td' );\n";
	...
	foreach ($sql_batch as $r_batch) {
		if ($load_unload == 1) //Load
		{
			...
			echo "load_drop_down( 'requires/dyeing_production_for_woven_textile_entry_controller', '" . $r_batch[csf("service_company")] . "', 'load_drop_floor', 'floor_td' );\n";
			...
			echo "load_drop_down( 'requires/dyeing_production_for_woven_textile_entry_controller', " . $r_batch[csf("service_source")] . "+'**'+" . $r_batch[csf("company_id")] . ", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
			...
			echo "load_drop_down( 'requires/dyeing_production_for_woven_textile_entry_controller', document.getElementById('cbo_service_company').value+'**'+" . $r_batch[csf("floor_id")] . ", 'load_drop_machine', 'machine_td' );\n";
		...
		else if ($load_unload == 2) //Unload
		{
			...
			//echo "load_drop_down( 'requires/dyeing_production_for_woven_textile_entry_controller', '".$r_batch[csf("process_id")]."', 'load_drop_sub_process', 'sub_process_td' );\n";
...
if ($action == 'populate_data_from_batch2') {
	...
	foreach ($data_array as $row) {
		...
		$sql_batch_d = sql_select("select id,service_source,system_no,service_company,received_chalan,issue_chalan,issue_challan_mst_id,company_id,batch_id,batch_no,process_end_date,end_hours,end_minutes,machine_id,floor_id,process_id,ltb_btb_id,remarks,hour_load_meter from
					pro_fab_subprocess where batch_id='$batch_id' and id=$update_mst_id   and load_unload_id=1 and status_active=1 and is_deleted=0 ");
		foreach ($sql_batch_d as $dyeing_d) {//$minute=str_pad($r_batch[csf("end_minutes")],2,'0',STR_PAD_LEFT);
			echo "document.getElementById('txt_dying_started').value = '" . change_date_format($dyeing_d[csf("process_end_date")]) . "';\n";
			echo "document.getElementById('txt_dying_end_load').value = '" . str_pad($dyeing_d[csf("end_hours")], 2, '0', STR_PAD_LEFT) . ':' . str_pad($dyeing_d[csf("end_minutes")], 2, '0', STR_PAD_LEFT) . "';\n";
			echo "$('#txt_issue_chalan').val('" . $dyeing_d[csf("issue_chalan")] . "');\n";
	...
	if ($load_unload == 1) {
		...
		$sql_batch = sql_select(
			"select id,batch_no,company_id,system_no,batch_id,service_source,service_company,received_chalan,issue_chalan,issue_challan_mst_id,process_end_date,load_unload_id,end_hours,end_minutes,machine_id,floor_id,process_id,
			ltb_btb_id,water_flow_meter,result,remarks,dyeing_type_id,hour_load_meter,multi_batch_load_id	from pro_fab_subprocess where batch_id='$batch_id' and id=$update_mst_id   and load_unload_id in(1) and status_active=1 and is_deleted=0  ");
	} else if ($load_unload == 2) {
		$sql_batch = sql_select("select id,batch_no,company_id,system_no,service_source,service_company,received_chalan,issue_chalan,issue_challan_mst_id,batch_id,process_end_date,load_unload_id,end_hours,end_minutes,machine_id,floor_id,process_id,ltb_btb_id,water_flow_meter,result,remarks,dyeing_type_id,hour_unload_meter,shift_name,responsibility_id,fabric_type,production_date from pro_fab_subprocess where batch_id='$batch_id'   and load_unload_id in(2,1)  and status_active=1 and is_deleted=0 $select_group_row1");
...
//echo $company_id.'=='.$double_dyeing.'='.$page_upto_id.'='.$roll_maintained_id;
	if ($db_type == 0) {
		if ($roll_maintained_id == 1) {
			$yarn_lot_data = sql_select("select  b.po_id, b.prod_id, group_concat(d.yarn_lot) as yarn_lot, group_concat(d.yarn_count) as yarn_count, group_concat(d.brand_id) as brand_id
						...
						where a.id=b.mst_id and b.roll_id=c.id and c.dtls_id=d.id and d.mst_id=e.id  and a.id=$batch_id and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and e.entry_form in(2,22,563)
			...
			$yarn_lot_data = sql_select("select  b.po_id, b.prod_id, group_concat(d.yarn_lot) as yarn_lot, group_concat(d.yarn_count) as yarn_count, group_concat(d.brand_id) as brand_id
						...
						where a.id=b.mst_id and b.prod_id=d.prod_id and d.mst_id=e.id  and a.id=$batch_id and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and e.entry_form in(2,22,563)
						group by  b.po_id, b.prod_id");
		}
	} else if ($db_type == 2) {
		...
		if ($roll_maintained_id == 1) {
			...
			$yarn_lot_data = sql_select("select  b.po_id, b.prod_id, LISTAGG(CAST(d.yarn_lot AS VARCHAR2(4000)), ',') WITHIN GROUP (ORDER BY d.yarn_lot) as yarn_lot, LISTAGG(CAST(d.yarn_count AS VARCHAR2(4000)), ',') WITHIN GROUP (ORDER BY d.yarn_count) as yarn_count,LISTAGG(CAST(d.brand_id AS VARCHAR2(4000)), ',') WITHIN GROUP (ORDER BY d.brand_id) as brand_id
						from pro_batch_create_mst a,pro_batch_create_dtls b, pro_grey_prod_entry_dtls d,  inv_receive_master e
						where a.id=b.mst_id  and d.mst_id=e.id and b.prod_id=d.prod_id  and a.id=$batch_id and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and e.entry_form in(2,22,563)
						group by b.po_id, b.prod_id");
		}
	}
	foreach ($yarn_lot_data as $rows) {
		...
		$max_load_id = return_field_value("max(id) as maxload_id", "pro_fab_subprocess", "batch_id =$batch_id and entry_form=616  and load_unload_id in(1) and is_deleted=0 and status_active=1","maxload_id");
		$max_unload_id = return_field_value("max(id)  as maxload_id", "pro_fab_subprocess", "batch_id =$batch_id and entry_form=616  and load_unload_id in(2) and is_deleted=0 and status_active=1","maxload_id");

		if($max_load_id!="")
		{
		 $load_insert = ("select a.load_unload_id,b.production_qty,b.roll_id,b.no_of_roll,b.prod_id,b.gsm,b.dia_width,b.barcode_no,b.width_dia_type from  pro_fab_subprocess a, pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$batch_id and a.entry_form=616  and a.load_unload_id in(1) and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.is_deleted=0 and b.mst_id in($max_load_id) ");
		...
		if($max_unload_id!="")
		{
		 $unload_insert = ("select a.load_unload_id,b.production_qty,b.roll_id,b.prod_id,b.gsm,b.dia_width,b.barcode_no,b.width_dia_type from  pro_fab_subprocess a, pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$batch_id and a.entry_form=616  and a.load_unload_id in(2) and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.is_deleted=0  ");//and b.mst_id in($max_unload_id)
  ...
  //echo $entry_form.'='.$page_upto_id.'='.$roll_maintained;
	...
	{
		if (($page_upto_id == 2 || $page_upto_id > 2) && $roll_maintained == 1) {
			  $sql_insert = ("select b.production_qty,b.roll_id,b.prod_id from  pro_fab_subprocess a, pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$batch_id and a.entry_form=616  and b.roll_id>0 $load_unload_cond and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.is_deleted=0");
			...
			else if ($db_type == 2) $select_group = "group by b.id,b.item_description,b.width_dia_type,a.entry_form,b.prod_id,b.po_id,b.roll_no,b.roll_id,b.barcode_no,b.gsm";
			//   $result = "select a.entry_form,b.width_dia_type,b.prod_id,b.po_id,b.barcode_no as batch_barcode,b.roll_no as batch_rollno,b.roll_id ,b.item_description,b.width_dia_type, sum(b.batch_qnty) as batch_qnty,c.roll_no,c.barcode_no from pro_batch_create_dtls b,pro_batch_create_mst a,pro_roll_details c where b.mst_id=$batch_id and a.id=b.mst_id and b.roll_id=c.id   and b.status_active=1 and b.is_deleted=0  $roll_id_cond $select_group";
		  $result = "select a.entry_form,b.width_dia_type,b.prod_id,b.po_id,b.barcode_no as batch_barcode,b.roll_no as batch_rollno,b.roll_id ,b.item_description,b.width_dia_type, sum(b.batch_qnty) as batch_qnty,b.gsm  from pro_batch_create_dtls b,pro_batch_create_mst a where b.mst_id=$batch_id and a.id=b.mst_id    and b.status_active=1 and b.is_deleted=0  $roll_id_cond $select_group";
			...
			if($max_unload_id!="")
				{
				  $unload_insert = ("select a.load_unload_id,b.production_qty,b.roll_id,b.prod_id,b.gsm,b.dia_width,b.barcode_no,b.width_dia_type from  pro_fab_subprocess a, pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$batch_id   and a.load_unload_id in(2) and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.is_deleted=0 ");
				$unload_result = sql_select($unload_insert);
				foreach ($unload_result as $row) {
							//$unload_qty_arr[$row[csf('prod_id')]][$row[csf('barcode_no')]][$row[csf('gsm')]][$row[csf('width_dia_type')]]= $row[csf('production_qty')];
			...
			$batch_id_search = sql_select("select a.batch_id from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$batch_id and company_id=$company_id and entry_form=616 and a.status_active=1 and b.status_active=1 $load_unload_cond");
			...
			if ($db_type == 0) $select_group = " group by b.id,b.item_description";
			else if ($db_type == 2) $select_group = "group by b.item_description,b.width_dia_type,a.entry_form,b.prod_id,b.po_id,b.gsm";
			//echo "sdsd";
			//   $sql_result = "select a.entry_form,b.width_dia_type,b.prod_id,b.po_id,count(b.roll_no) as no_of_roll ,b.item_description,b.width_dia_type, sum(b.batch_qnty) as batch_qnty from pro_batch_create_dtls b,pro_batch_create_mst a,pro_roll_details c where b.mst_id=$batch_id and a.id=b.mst_id and b.roll_id=c.id   and b.status_active=1 and b.is_deleted=0 $batch_insert_cond $select_group";

			  $sql_result = "select a.entry_form,b.width_dia_type,b.prod_id,b.po_id,count(b.roll_no) as no_of_roll ,b.item_description,b.width_dia_type, sum(b.batch_qnty) as batch_qnty,b.gsm from pro_batch_create_dtls b,pro_batch_create_mst a  where b.mst_id=$batch_id and a.id=b.mst_id     and b.status_active=1 and b.is_deleted=0 $batch_insert_cond $select_group";
			...
			if(count($result)==0)
			{
				if ($db_type == 0) $select_group = " group by b.id,b.item_description";
				 ...
				 $sql_result ="select a.entry_form,b.width_dia_type,b.prod_id,b.po_id,count(b.roll_no) as no_of_roll ,b.item_description,b.width_dia_type, sum(b.batch_qnty) as batch_qnty,0 as roll_no, 0 as barcode_no from pro_batch_create_dtls b,pro_batch_create_mst a where b.mst_id=$batch_id and a.id=b.mst_id  and b.status_active=1 and b.is_deleted=0 $batch_insert_cond $select_group";
				$result=sql_select($sql_result);
			}
	...
	else //Trim Batch Start.
	{
			...
			if ($db_type == 0) $select_group = " group by b.id,b.item_description";
				...
				$sql_result ="select a.entry_form, 0 as prod_id,0 as po_id,0 as no_of_roll,b.remarks,b.item_description, 0 as width_dia_type, sum(b.trims_wgt_qnty) as batch_qnty,0 as roll_no, 0 as barcode_no from pro_batch_trims_dtls b,pro_batch_create_mst a where a.id=b.mst_id and   b.mst_id=$batch_id   and b.status_active=1 and b.is_deleted=0 $batch_insert_cond group by a.entry_form,b.remarks,b.item_description";
				$result=sql_select($sql_result);
...
if ($action == "save_update_delete") {
	...
	if ($operation == 0)  // Insert Here
	{
		...
		if (str_replace("'", '', $cbo_load_unload) == 2) {
			//if (str_replace("'", '', $txt_process_id) == 31) {
					$sql_data = "select id, batch_id from pro_fab_subprocess where  company_id=" . $cbo_company_id . " and  batch_id=" . $txt_batch_ID . " and load_unload_id=1 and entry_form=616 and is_deleted=0 and status_active=1";
			...
			//}
					...
					$sql_loadunload="select id,load_unload_id, batch_id from pro_fab_subprocess where  company_id=".$cbo_company_id." and  batch_id=".$txt_batch_ID." and entry_form=616 and is_deleted=0 and status_active=1   order by id desc";
			...
			if($multi_dyeing==2) // is Multi no
			{
				...
				{
					$sql_unload="select id, batch_id from pro_fab_subprocess where  company_id=".$cbo_company_id." and  batch_id=".$txt_batch_ID." and load_unload_id=2 and entry_form=616 and is_deleted=0 and status_active=1";
		...
		//if($updateId) $up_id_cond="and a.id!=$updateId";else $up_id_cond="";
				...
				if($multi_dyeing==2 && $cbo_dyeing_type==1) // is Multi no //CBP Dyeing
					{
						$sql_load="select id, batch_id from pro_fab_subprocess where  company_id=".$cbo_company_id." and  batch_id=".$txt_batch_ID." and load_unload_id=1 and entry_form=616 and is_deleted=0 and status_active=1";
					...
					$sql_loadunload="select id,load_unload_id, batch_id,machine_id from pro_fab_subprocess where  company_id=".$cbo_company_id." and  batch_id=".$txt_batch_ID."  and entry_form=616 and is_deleted=0 and status_active=1   order by id desc";//and load_unload_id=1 
					$loadunload_data_array=sql_select($sql_loadunload);
		...
		if ($cbo_load_unload == "'1'") {
			...
			$data_array = "(" . $id . "," . $cbo_company_id . "," . $system_no . "," . $cbo_service_source . "," . $cbo_service_company . "," . $txt_recevied_chalan . "," . $txt_issue_chalan . "," . $txt_issue_mst_id . "," . $txt_batch_no . "," . $txt_batch_ID . "," . $txt_ext_id . ",31," . $cbo_ltb_btb . "," . $txt_water_flow . "," . $txt_process_start_date . "," . $txt_start_hours . "," . $txt_start_minutes . "," . $cbo_machine_name . "," . $cbo_floor . "," . $cbo_load_unload . ",616," . $hidden_double_dyeing . "," . $txt_remarks . "," . $cbo_dyeing_type . "," . $txt_load_meter . "," . $cbo_yesno . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
			...
			if (($page_upto_id == 2 || $page_upto_id > 2) && str_replace("'", "", $roll_maintained) == 1) {
				...
				for ($i = 1; $i <= $total_row; $i++) {
					...
					if (str_replace("'", "", $$checked_tr) == 1) {
						...
						$data_array_dtls .= "(" . $id_dtls . "," . $mst_update_id . ",616," . $$prod_id . "," . $$txtconscomp . "," . $$txtgsm . "," . $$txtdiawidth . "," . $$txtdiatypeID . "," . $$txtbatchqnty . "," . $$txtroll . "," . $$txtbarcode . ",1," . $$rollid . "," . $$txtproductionqty . "," . $$txtremark . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
						$id_dtls = $id_dtls + 1;
					}
				...
				for ($i = 1; $i <= $total_row; $i++) {
					...
					$Itemprod_id = str_replace("'", "", $$prod_id);
					if ($data_array_dtls != "") $data_array_dtls .= ",";
					$data_array_dtls .= "(" . $id_dtls . "," . $mst_update_id . ",616," . $$prod_id . "," . $$txtconscomp . "," . $$txtgsm . "," . $$txtdiawidth . "," . $$hiddendiatypeid_ . "," . $$txtbatchqnty . "," . $$txtroll . ",1," . $$txtproductionqty . "," . $$txtremark . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
					$id_dtls = $id_dtls + 1;
					//print_r($data_array_dtls);die;
				}
			...
			if (($page_upto_id == 2 || $page_upto_id > 2) && str_replace("'", "", $roll_maintained) == 1) {
				...
				for ($i = 1; $i <= $total_row; $i++) {
					...
					if (str_replace("'", "", $$checked_tr) == 1) {
						...
						$Itemprod_id = str_replace("'", "", $$prod_id);
						if ($data_array_dtls != "") $data_array_dtls .= ",";
						$data_array_dtls .= "(" . $id_dtls . "," . $mst_update_id . ",616," . $$prod_id . "," . $$txtconscomp . "," . $$txtgsm . "," . $$txtdiawidth . "," . $$txtdiatypeID . "," . $$txtbatchqnty . "," . $$txtroll . "," . $$txtbarcode . ",2," . $$rollid . "," . $$txtproductionqty . "," . $$txtremark . "," . $$txtrate . "," . $$txtamount . "," . $hidden_currency . "," . $hidden_exchange_rate . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
						$id_dtls = $id_dtls + 1;
					}
					//print_r($data_array_dtls);die;
				}
				...
				for ($i = 1; $i <= $total_row; $i++) {
					...
					$Itemprod_id = str_replace("'", "", $$prod_id);
					if ($data_array_dtls != "") $data_array_dtls .= ",";
					$data_array_dtls .= "(" . $id_dtls . "," . $mst_update_id . ",616," . $$prod_id . "," . $$txtconscomp . "," . $$txtgsm . "," . $$txtdiawidth . "," . $$hiddendiatypeid . "," . $$txtbatchqnty . "," . $$txtroll . ",2," . $$txtproductionqty . "," . $$txtrate . "," . $$txtamount . "," . $hidden_currency . "," . $hidden_exchange_rate . "," . $$txtremark . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
					$id_dtls = $id_dtls + 1;
					//print_r($data_array_dtls);die;
				}
	...
	else if ($operation == 1)   // Update Here
	{
		...
		$cbo_machine_id=str_replace("'","",$cbo_machine_name);
		//if($updateId) $up_id_cond="and a.id!=$updateId";else $up_id_cond="";
		$batch_prod=sql_select("select a.batch_id,max(a.id) as mst_id from pro_fab_subprocess a where  a.batch_id=$txt_batch_ID and a.entry_form=616 and a.company_id=$cbo_company_id and a.status_active=1  group by a.batch_id ");
		$last_mst_id=0;
		foreach($batch_prod as $row)
		{
			$last_mst_id= $row[csf('mst_id')];
		}
		$batch_last_process=sql_select("select a.batch_id,a.load_unload_id,a.result,a.end_minutes,a.process_end_date,a.production_date from pro_fab_subprocess a where  a.batch_id=$txt_batch_ID and a.company_id=$cbo_company_id and a.id in($last_mst_id) and a.status_active=1  ");
	...
	//	echo "10**=".$double_dyeing_id.'='.$load_unloadId.'='.$last_load_unload_id;die;
		...
		if ($cbo_load_unload == "'1'") {
			...
			$data_array_update = "" . $cbo_service_source . "*" . $cbo_service_company . "*" . $txt_recevied_chalan . "*" . $txt_issue_chalan . "*" . $txt_issue_mst_id . "*" . $txt_batch_no . "*" . $txt_batch_ID . "*" . $txt_ext_id . "*" . $txt_process_id . "*" . $cbo_ltb_btb . "*" . $txt_water_flow . "*" . $txt_process_start_date . "*" . $txt_start_hours . "*" . $txt_start_minutes . "*" . $cbo_machine_name . "*" . $cbo_floor . "*" . $cbo_load_unload . "*616*" . $txt_remarks . "*" . $cbo_dyeing_type . "*" . $txt_load_meter . "*" . $cbo_yesno . "*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'";//
			...
			if (($page_upto_id == 2 || $page_upto_id > 2) && str_replace("'", "", $roll_maintained) == 1) {
				...
				for ($i = 1; $i <= $total_row; $i++) {
					...
					if (str_replace("'", "", $$checkRowTd) == 1) {
						...
						if (str_replace("'", '', $$updateiddtls) == "") {
							...
							$data_array_dtls_insert .= "(" . $id_dtls . "," . $update_id . ",616," . $$prod_id . "," . $$txtconscomp . "," . $$txtgsm . "," . $$txtdiawidth . "," . $$hiddendiatypeid . "," . $$txtbatchqnty . "," . $$txtroll . "," . $$txtbarcode . ",1," . $$rollid . "," . $$txtprodqnty . "," . $$txtremark . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
							$id_dtls = $id_dtls + 1;
						}
					} else {
						if (str_replace("'", '', $$updateiddtls) != "") {
				...
				for ($i = 1; $i <= $total_row; $i++) {
					...
					if (str_replace("'", '', $$updateiddtls) != "") {
						...
						if ($data_array_dtls_insert != "") $data_array_dtls_insert .= ",";
						$data_array_dtls_insert .= "(" . $id_dtls . "," . $update_id . ",616," . $$prod_id . "," . $$txtconscomp . "," . $$txtgsm . "," . $$txtdiawidth . "," . $$hiddendiatypeid . "," . $$txtbatchqnty . "," . $$txtroll . ",1," . $$rollid . "," . $$txtprodqnty . "," . $$txtremark . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
						$id_dtls = $id_dtls + 1;
					}
				}
			}
	...
	$data_array_update = "" . $cbo_service_source . "*" . $cbo_service_company . "*" . $txt_recevied_chalan . "*" . $txt_issue_chalan . "*" . $txt_issue_mst_id . "*" . $txt_batch_no . "*" . $txt_batch_ID . "*" . $txt_ext_id . "*" . $txt_process_id . "*" . $cbo_ltb_btb . "*" . $txt_water_flow . "*" . $txt_process_end_date . "*" . $txt_end_hours . "*" . $txt_end_minutes . "*" . $cbo_machine_name . "*" . $cbo_floor . "*" . $cbo_load_unload . "*" . $cbo_result_name . "*616*" . $txt_remarks . "*" . $cbo_dyeing_type . "*" . $txt_unload_meter . "*" . $cbo_shift_name . "*" . $cbo_fabric_type . "*" . $txt_process_date . "*" . $txt_booking_no . "*" . $cbo_responsibility . "*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "' ".$field_data_arr."";
			...
			if (($page_upto_id == 2 || $page_upto_id > 2) && str_replace("'", "", $roll_maintained) == 1) {
				...
				for ($i = 1; $i <= $total_row; $i++) {
					...
					if (str_replace("'", "", $$checkRowTd) == 1) {
						...
						if (str_replace("'", '', $$updateiddtls) == "") {
							...
							if ($data_array_dtls_insert != "") $data_array_dtls_insert .= ",";
							$data_array_dtls_insert .= "(" . $id_dtls . "," . $update_id . ",616," . $$prod_id . "," . $$txtconscomp . "," . $$txtgsm . "," . $$txtdiawidth . "," . $$hiddendiatypeid . "," . $$txtbatchqnty . "," . $$txtroll . "," . $$txtbarcode . ",2," . $$rollid . "," . $$txtrate . "," . $$txtamount . "," . $$txtprodqnty . "," . $$txtremark . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
							$id_dtls = $id_dtls + 1;
						}
				...
				for ($i = 1; $i <= $total_row; $i++) {
					...
					if (str_replace("'", '', $$updateiddtls) == "") {
						if ($data_array_dtls_insert != "") $data_array_dtls_insert .= ",";
						$data_array_dtls_insert .= "(" . $id_dtls . "," . $update_id . ",616," . $$prod_id . "," . $$txtconscomp . "," . $$txtgsm . "," . $$txtdiawidth . "," . $$hiddendiatypeid . "," . $$txtbatchqnty . "," . $$txtroll . ",2," . $$rollid . "," . $$txtrate . "," . $$txtamount . "," . $$txtprodqnty . "," . $$txtremark . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
						$id_dtls = $id_dtls + 1;
					}
...
if ($action == "check_batch_no_load") {
	...
	$process = $data[3];
	//if ($process == 31) {
		$sql = "select id, batch_id from pro_fab_subprocess where  company_id='" . trim($data[0]) . "' and  batch_id='" . trim($data[2]) . "' and load_unload_id=1 and entry_form=616 and is_deleted=0 and status_active=1";
		$data_array = sql_select($sql);
		if (count($data_array) > 0) {
...
if ($action == "check_batch_no_for_machine") {
 ...
 $sql = "select  a.batch_no as batch_no,a.batch_id from pro_fab_subprocess a,pro_fab_subprocess_dtls b  where  a.id=b.mst_id and a.service_company='" . trim($data[0]) . "'  and a.machine_id='$data[3]' and a.service_source in(1)  and a.load_unload_id=1 and a.entry_form=616 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1    group by  a.batch_no,a.batch_id";
            ...
            <fieldset>
                <table cellpadding="0" cellspacing="2" width="100%" id="main_tbl">
                    ...
                    <tr>
                        ...
                        <td>
							<?
							echo create_drop_down("cbo_company_id", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name", "id,company_name", 1, "--Select Company--", $selected, "load_drop_down('requires/dyeing_production_for_woven_textile_entry_controller', this.value, 'load_drop_floor', 'floor_td' );roll_maintain();", "", "", "", "", "");
							?>
                    ...
                    <tr>
                        ...
                        <td>
							<?
							//fnc_company_val();
							echo create_drop_down("cbo_service_source", 135, $knitting_source, "", 1, "-- Select --", $selected, "load_drop_down( 'requires/dyeing_production_for_woven_textile_entry_controller',this.value+'**'+$('#cbo_company_id').val(),'load_drop_down_knitting_com','dyeing_company_td' );search_populate(this.value);", "", "1,3");
							?>
                        </td>
                    ...
                    <tr>
                        ...
                        <td>
							<?
							echo create_drop_down( "txt_process_id", 135, $conversion_cost_head_array,"", 1, "-- Select --", 0, "","","31" );
							?>
        ...
        <fieldset onLoad="process_check(document.getElementById('txt_process_id').value);">
            <table cellpadding="0" cellspacing="2" width="100%" id="main_tbl">
                ...
                <tr>
                    ...
                    <td>
						<?
						echo create_drop_down("cbo_company_id", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "--Select Company--", $selected, "load_drop_down('requires/dyeing_production_for_woven_textile_entry_controller', this.value, 'load_drop_floor', 'floor_td' );roll_maintain();");
						?>
                        <input type="hidden" name="txt_update_id" id="txt_update_id" style="width:100px;"
                               class="text_boxes" readonly/>
                        <input type="hidden" name="roll_maintained" id="roll_maintained" style="width:100px;"
                ...
                <tr>
                    ...
                    <td id="sub_process_td">
						...
						echo create_drop_down( "txt_process_id", 135, $conversion_cost_head_array,"", 0, "-- Select --", 0, "process_check(this.value)","","31" );
						?>
                        <!-- <input type="text" name="txt_process_name" id="txt_process_name" class="text_boxes" style="width:122px;" placeholder="Double Click To Search" onDblClick="openmypage_process();" readonly />
						<input type="hidden" name="txt_process_id" id="txt_process_id" />-->
                    </td>
...
if ($action == "load_drop_down_knitting_com") {
	...
	if ($data[0] == 1) {
		...
		echo create_drop_down("cbo_service_company", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "-- Select --", "", "load_drop_down('requires/dyeing_production_for_woven_textile_entry_controller', this.value, 'load_drop_floor', 'floor_td' );load_batch_working_company(this.value);fnc_mc_load()", "");
	} else if ($data[0] == 3) {
...
if ($action == "populate_data_from_data") {
	...
	foreach ($res as $row) {
		...
		echo "load_drop_down( 'requires/dyeing_production_for_woven_textile_entry_controller', " . $row[csf("dyeing_source")] . "+'**'+" . $row[csf("company_id")] . ", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
            ...
            <fieldset style="width:860px; margin-left:2px">
                <table cellpadding="0" cellspacing="0" width="850" border="1" rules="all" class="rpt_table">
                    ...
                    <tr class="general">
                        ...
                        <td align="center">
							<?
							echo create_drop_down("cbo_service_source", 135, $knitting_source, "", 1, "-- Select --", $selected, "load_drop_down( 'dyeing_production_for_woven_textile_entry_controller',this.value+'**'+$cbo_company_id,'load_drop_down_knitting_com','dyeing_company_td' );", "", "1,3");
							?>
                        </td>
                        <td id="dyeing_company_td">
							<?
                        ...
                        <td align="center">
                            <input type="button" name="button2" class="formbutton" value="Show"
                                   onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $cbo_company_id; ?>, 'create_challan_search_list_view', 'search_div', 'dyeing_production_for_woven_textile_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1);')"
                                   style="width:100px;"/>
                        </td>
                    </tr>
                    <tr>
    ...
    <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" width="800" rules="all">
		...
		<? foreach ($result as $row) {
            ...
            <tr bgcolor="<? echo $bgcolor; ?>"
                onClick='show_list_view("<? echo $batch_id . '_' . $load_unload . '_' . $po_id_arr . '_' . $row[csf("id")]; ?>","child_form_input_data","list_fabric_desc_container","requires/dyeing_production_for_woven_textile_entry_controller");get_php_form_data("<? echo $row[csf("id")]; ?>","mst_id_child_form_input_data","requires/dyeing_production_for_woven_textile_entry_controller")'
...
if ($action == "child_form_input_data") {
	...
	$company_id = return_field_value("company_id", "pro_fab_subprocess", "batch_id='$batch_id' and entry_form=616 and is_deleted=0 and status_active=1");
	...
	if ($db_type == 0) {
		if ($roll_maintained_id == 1) {
			$yarn_lot_data = sql_select("select  b.po_id, b.prod_id, group_concat(d.yarn_lot) as yarn_lot, group_concat(d.yarn_count) as yarn_count, group_concat(d.brand_id) as brand_id
						from pro_batch_create_mst a,pro_batch_create_dtls b, pro_roll_details c, pro_grey_prod_entry_dtls d,  inv_receive_master e
						where a.id=b.mst_id and b.roll_id=c.id and c.dtls_id=d.id and d.mst_id=e.id  and a.id=$batch_id and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0  
						group by  b.po_id, b.prod_id");
		} else {
			$yarn_lot_data = sql_select("select  b.po_id, b.prod_id, group_concat(d.yarn_lot) as yarn_lot, group_concat(d.yarn_count) as yarn_count, group_concat(d.brand_id) as brand_id
						from pro_batch_create_mst a,pro_batch_create_dtls b, pro_grey_prod_entry_dtls d,  inv_receive_master e
						where a.id=b.mst_id and b.prod_id=d.prod_id and d.mst_id=e.id and a.id=$batch_id and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0  
						group by  b.po_id, b.prod_id");
		}
	} else if ($db_type == 2) {
		...
		if ($roll_maintained_id == 1) {
			...
			$yarn_lot_data = sql_select("select  b.po_id, b.prod_id, LISTAGG(CAST(d.yarn_lot AS VARCHAR2(4000)), ',') WITHIN GROUP (ORDER BY d.yarn_lot) as yarn_lot, LISTAGG(CAST(d.yarn_count AS VARCHAR2(4000)), ',') WITHIN GROUP (ORDER BY d.yarn_count) as yarn_count,LISTAGG(CAST(d.brand_id AS VARCHAR2(4000)), ',') WITHIN GROUP (ORDER BY d.brand_id) as brand_id
						from pro_batch_create_mst a,pro_batch_create_dtls b, pro_grey_prod_entry_dtls d,  inv_receive_master e
						where a.id=b.mst_id  and d.mst_id=e.id and b.prod_id=d.prod_id  and a.id=$batch_id and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0  
						group by b.po_id, b.prod_id");
		}
	}
	foreach ($yarn_lot_data as $rows) {
	...
	$sql_result = sql_select("select a.id,b.id as dtls_id,b.prod_id,b.const_composition,b.remarks,b.gsm,b.dia_width, b.width_dia_type, b.batch_qty, b.production_qty,b.barcode_no, b.roll_no, b.no_of_roll,b.roll_id,b.rate,b.amount from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.id=$mst_id and a.entry_form=616 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $load_unload_cond");
...
//echo $data."<br>";
	...
	if ($load_unload == 1) {
		...
		$sql = "select a.id,a.company_id,a.batch_id,a.batch_no,sum(b.batch_qty) as batch_qty,sum(b.production_qty) as production_qty, $dyeing_date_con  from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.system_no=$system_no and a.entry_form=616 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $load_unload_cond  group by a.id,a.batch_id,a.batch_no,a.company_id,$dyeing_date_con order by a.id";
		//echo $sql;
	} else {
    ...
    <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" width="800" rules="all">
		...
		<? foreach ($result as $row) {
            ...
            <tr bgcolor="<? echo $bgcolor; ?>"
                onClick='show_list_view("<? echo $batch_id . '_' . $load_unload . '_' . $po_id_arr . '_' . $row[csf("id")]. '_' . $entry_form_no; ?>","child_form_input_data","list_fabric_desc_container","requires/dyeing_production_for_woven_textile_entry_controller");get_php_form_data("<? echo $load_unload . '_' . $row[csf("batch_id")] . '_' . $row[csf("batch_no")] . '_' . $row[csf("company_id")]. '_' . $entry_form_no. '_' . $row[csf("id")]; ?>","populate_data_from_batch2","requires/dyeing_production_for_woven_textile_entry_controller")'
                style="cursor:pointer">
                <td width="30"><? echo $i; ?></td>
            ...
            <table cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all" width="900">
                ...
                <tr>
                    <td align="center"><input type="hidden" id="selected_booking">
						<?
						echo create_drop_down("cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 and id=" . $cbo_company_id . " order by company_name", "id,company_name", 1, "-- Select Company --", '', "load_drop_down( 'dyeing_production_for_woven_textile_entry_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
						?>
                    </td>
                    ...
                    <td align="center">
                        <input type="button" name="button2" class="formbutton" value="Show"
                               onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_search_category').value+'_'+document.getElementById('txt_booking_no').value+'_'+'<?php echo $supplier_id . "_" . $process_id; ?>', 'create_booking_search_list_view', 'search_div', 'dyeing_production_for_woven_textile_entry_controller', 'setFilterGrid(\'table_body\',-1)')"
                               style="width:100px;"/>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td align="center">
                        <input type="button" name="btn_show" class="formbutton" value="Show"
                               onClick="show_list_view ( document.getElementById('cbo_company_id').value+'_'+document.getElementById('txt_system_no').value+'_'+document.getElementById('txt_batch_no').value+'_'+'<? echo $load_unload; ?>', 'create_sys_search_list_view', 'search_div', 'dyeing_production_for_woven_textile_entry_controller', 'setFilterGrid(\'list_view\',-1)')"
                               style="width:100px;"/>
                    </td>
                </tr>
                <tr>
...
if ($action == "create_sys_search_list_view") {
	...
	$sql = "select a.entry_form,a.batch_for,a.batch_against,a.extention_no,b.service_source,b.service_company,b.id,b.batch_no,b.company_id,b.load_unload_id,b.batch_id,b.system_no,b.process_end_date,b.end_hours,b.end_minutes,b.production_date
			from  pro_batch_create_mst a,pro_fab_subprocess b where a.id=b.batch_id and a.entry_form in (35,563) and b.status_active=1 and b.is_deleted=0 $com_cond $sys_cond $batch_no_cond  $load_unload_cond $user_working_comp_cond order by b.id desc";
	//echo $sql;
	$sql2=sql_select($sql);
	foreach ($sql2 as $row) {
	...
	$sql_last_prod="select max(b.id) as mst_id,b.batch_id
			from  pro_batch_create_mst a,pro_fab_subprocess b where a.id=b.batch_id and a.entry_form in (35,563) and b.status_active=1 and b.is_deleted=0 $com_cond $sys_cond $batch_no_cond  $load_unload_cond $user_working_comp_cond group by b.batch_id order by mst_id desc";
...
if ($action == "dyeing_pro_print") {
	...
	$sales_order_result = sql_select("select id,job_no,within_group,sales_booking_no,style_ref_no, customer_buyer from fabric_sales_order_mst where status_active=1 and is_deleted=0");
	$sales_arr = array();
    ...
    <div style="width:800px;" align="center">
        <table width="780" cellspacing="0" align="center" border="0">
            ...
            <tr>
                ...
                <td width="100"><strong>Ref. Qty:</strong></td>
