...
if ($action == "load_batch_machine") {
	...
	$sql_res = "select a.machine_id,max(a.id) as  mst_id from pro_fab_subprocess a,lib_machine_name b where a.machine_id=b.id and b.id=$ex_data[1] and a.load_unload_id=1 and a.company_id=$ex_data[0] and a.machine_id=$ex_data[1] and a.multi_batch_load_id=1 and a.status_active=1 and a.is_deleted=0 $flood_cond group by a.machine_id";
	//echo $sql_res;die;
	$nameArray = sql_select($sql_res);
	$mst_id=$nameArray[0][csf('mst_id')];

	$sql_res_unload = "select a.machine_id,a.batch_id,a.process_end_date,a.floor_id,a.end_hours,a.end_minutes from pro_fab_subprocess a,lib_machine_name b where a.machine_id=b.id   and a.load_unload_id=2 and a.company_id=$ex_data[0]  and a.machine_id=$ex_data[1] and a.status_active=1 and a.is_deleted=0";
	...
	$sql_res_mst = "select a.machine_id,a.batch_id,a.process_end_date,a.floor_id,a.end_hours,a.end_minutes from pro_fab_subprocess a,lib_machine_name b where a.machine_id=b.id and a.id=$mst_id and a.load_unload_id=1 and a.company_id=$ex_data[0]  and a.machine_id=$ex_data[1] and a.multi_batch_load_id=1 and a.status_active=1 and a.is_deleted=0";
...
if ($action == "create_batch_search_list_view") {
	...
	else
	{
		...
		from pro_batch_create_mst a,pro_fab_subprocess b
	...
	if(!empty($all_batch_no_arr))
	{
        ...
        if ($db_type == 2)
        {
			if ($batch_type == 0 || $batch_type == 2) {
				...
				$sql_po = sql_select("select b.mst_id, a.job_no_mst, a.order_no as   a.order_no as po_no from  subcon_ord_dtls a, pro_batch_create_dtls b where a.id=b.po_id and  b.status_active=1 and b.is_deleted=0 $all_batch_no_cond  ");
			} else {
				...
				$sql_po = sql_select("select b.mst_id, a.job_no_mst,a.po_number as po_no from wo_po_break_down a, pro_batch_create_dtls b where a.id=b.po_id and  b.status_active=1 and b.is_deleted=0 $all_batch_no_cond  ");
			}
		}
		if ($db_type == 0)
		{
						...
						if ($selectResult[csf('re_dyeing_from')] == 0 || $selectResult[csf('re_dyeing_from')]>0)
						{
							if($load_unload_arr[$selectResult[csf('id')]]=="")
							{
								if($is_sales == 1){
									...
									$po_no = implode(", ", array_unique(explode(",", rtrim($po_num[$selectResult[csf('id')]]['po_no'],','))));
									$job_no= implode(", ",array_unique(explode(",",rtrim($po_num[$selectResult[csf('id')]]['job_no_mst'],','))));
								}
								?>
...
if ($action == 'populate_data_from_batch')
{
	...
	//echo $entry_form.'DDD';
	//$is_sales = $ex_data[3];
	$double_dyeing=0;$total_trims_weight=0;
	$batch_data =sql_select("select company_id,working_company_id,batch_date,entry_form,double_dyeing,dyeing_machine,floor_id,total_trims_weight from pro_batch_create_mst where id ='$batch_id' and is_deleted=0 and status_active=1");
	foreach ($batch_data as $row)
	{
		...
		$dyeing_machine = $row[csf('dyeing_machine')];
	...
	$data_chk=sql_select($last_load);
	$chk_load=$chk_unload=0;
	foreach ($data_chk as $row)
	{
		$chk_load_unload_id= $row[csf('load_unload_id')];
		if($chk_load_unload_id==1)
		{
			$chk_load=1;
		}
		if($chk_load_unload_id==2)
		{
			$chk_unload=2;
		}
	}
...
//	echo $load_unload_id.'=A';
	echo "document.getElementById('batch_type').innerHTML = '';\n";
	if($load_unload==1 && $load_unload_id==1)
	{
		echo "document.getElementById('batch_type').innerHTML = '<b>Already load, unload please</b>';\n";die;
	}
	if($load_unload==2 && $load_unload_id==2)
	{
		echo "document.getElementById('batch_type').innerHTML = '<b>Already Unload</b>';\n";die;
	}

	//echo $chk_load.'='.$chk_unload.'='.$multi_dyeing.'=A';
	if($chk_load==1 && $chk_unload==2 && $multi_dyeing==2)
	{

		echo "document.getElementById('batch_type').innerHTML = '<b>Already Load and unload Found.</b>';\n";die;
	}
	...
	foreach ($sql_sales_job as $sales_job_row)
	{
		...
		$po_job_no=$sales_job_row[csf('po_job_no')];
	...
	{
	  	//echo $row[csf("working_company_id")].'=XXXXXXXXXXX';
		...
		echo "document.getElementById('txt_batch_date').value 			= '" . change_date_format($batch_date) . "';\n";
		 echo "document.getElementById('txt_hidden_service_company').value 	= '" .$working_company_id. "';\n";
		...
		if($load_unload_arr[$row[csf("id")]]=="")
		{
			...
			{
				echo "$('#txt_reason_name').attr('disabled',false);\n";
			}
			...
			if ($row[csf("entry_form")] == 36)
			{
				...
				/*$result_job = sql_select("SELECT $select_listagg_subcon, b.subcon_job as job_no_mst, b.party_id as buyer_name from  subcon_ord_dtls a, subcon_ord_mst b
				where a.job_no_mst=b.subcon_job  and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 $pro_cond group by b.subcon_job, b.party_id");*/
				$result_job = sql_select("SELECT a.order_no as po_no, b.subcon_job as job_no_mst, b.party_id as buyer_name from  subcon_ord_dtls a, subcon_ord_mst b
				where a.job_no_mst=b.subcon_job  and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 $pro_cond");
				foreach ($result_job as $row)
				{
					$all_po_no[$row[csf("po_no")]]=$row[csf("po_no")];
				}
			}
			else
			{
				$batch_type = "<b> SELF ORDER BATCH </b>";
				/*$result_job = sql_select("SELECT $select_listagg, a.job_no_mst, b.buyer_name
				from wo_po_break_down a, wo_po_details_master b
				where a.job_id=b.id  and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0  $pro_cond $select_f_group");*/
				$result_job = sql_select("SELECT a.po_number as po_no, a.file_no as file_no, a.grouping as ref_no, a.job_no_mst, b.buyer_name
				from wo_po_break_down a, wo_po_details_master b
				where a.job_id=b.id  and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 $pro_cond");// $pro_cond
				foreach ($result_job as $row)
				{
					$all_po_no[$row[csf("po_no")]]=$row[csf("po_no")];
					if ($row[csf("file_no")]!="")
					{
						$all_file_no[$row[csf("file_no")]]=$row[csf("file_no")];
					}
					if ($row[csf("ref_no")]!="")
					{
						$all_ref_no[$row[csf("ref_no")]]=$row[csf("ref_no")];
					}
				}
			}
			...
			if($load_unload==2)
			{
				//echo $row[csf("extention_no")].'=AAAAAAAAAAAAA';
			...
			$pro_id2 = chop(implode(",", $all_po_no),",");
			$file_no = chop(implode(",", $all_file_no),",");
			$ref_no =  chop(implode(",", $all_ref_no),",");
			...
			if ($salesOrder == 1)
			{
				if($within_group == 1)
				{
					...
					{
						$fso_to_ref_sql=sql_select("SELECT b.job_no, c.grouping, c.file_no from fabric_sales_order_mst a, wo_booking_dtls b, wo_po_break_down c
						where a.booking_id=b.booking_mst_id and b.po_break_down_id=c.id and b.booking_no ='".$row[csf('booking_no')]."' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by b.job_no, c.grouping, c.file_no");
						$ref_no='';$ref_no="";$file_no="";
						foreach ($fso_to_ref_sql as $key => $val)
						{
							$ref_no.=$val[csf('grouping')].',';
							$file_no.=$val[csf('file_no')].',';
						}
						$ref_no = implode(",", array_unique(explode(",", chop($ref_no,","))));
						$file_no = implode(",", array_unique(explode(",", chop($file_no,","))));
						$job_nos = $fso_to_ref_sql[0][csf('job_no')];
			...
			if($floor_id>0)
			{
				echo "load_drop_down( 'requires/dyeing_production_controller', $working_company_id+'**'+$floor_id, 'load_drop_machine', 'machine_td' );\n";
			}
			if($dyeing_machine>0)
			{
				echo "document.getElementById('cbo_machine_name').value 			= '" . $dyeing_machine . "';\n";
	...
	else if ($db_type == 2) $select_group_row1 = " and  rownum>=1 order by id";//order by id desc limit 0,1
	if ($load_unload == 1)
	{
	...
	else if ($load_unload == 2)
	{
		...
		// echo "select id,batch_no,company_id,system_no,service_source,service_company,received_chalan,issue_chalan,issue_challan_mst_id,batch_id,process_end_date,load_unload_id,end_hours,end_minutes,machine_id,floor_id,process_id,ltb_btb_id,water_flow_meter,result,remarks,dyeing_type_id,hour_unload_meter,shift_name,responsibility_id,fabric_type,production_date from pro_fab_subprocess where batch_id='$batch_id' and entry_form=35 and load_unload_id in(2,1) and status_active=1 and is_deleted=0 $select_group_row1";
	}
	$service_company=$sql_batch[0][csf('service_company')];
	//echo $service_company.'=AAAAAAAAAAA';
	if($service_company)
	{
		$service_source=$sql_batch[0][csf('service_source')];
		$service_company=$sql_batch[0][csf('service_company')];
		$company_id=$sql_batch[0][csf('company_id')];
		$floor_id=$sql_batch[0][csf('floor_id')];

		echo "$('#cbo_service_company').val(" . $service_company . ");\n";
		echo "$('#cbo_service_source').val(" . $service_source . ");\n";
		echo "$('#txt_hidden_service_company').val(" . $working_company_id . ");\n";
		//echo $service_company.'='.$floor_id.'=';

		echo "load_drop_down( 'requires/dyeing_production_controller', $service_company+'**'+" . $floor_id . ", 'load_drop_machine', 'machine_td' );\n";
	}
...
if ($action == 'populate_data_from_batch2') {
	...
	$roll_maintained = return_field_value("fabric_roll_level", "variable_settings_production", "company_name ='$company_id' and item_category_id=50 and variable_list=3 and is_deleted=0 and status_active=1");
	$page_upto_id = return_field_value("page_upto_id", "variable_settings_production", "company_name ='$company_id' and item_category_id=50 and variable_list=3 and is_deleted=0 and status_active=1");
	$color_arr = return_library_array("select id, color_name from lib_color where status_acive=1", 'id', 'color_name');
	...
	$in_charge_arr = return_library_array("select b.id, b.first_name from lib_employee b,pro_fab_subprocess f where f.in_charge_mst_id=b.id  and f.batch_id=$batch_id and f.status_active=1 and f.is_deleted=0 and b.status_active=1 and b.is_deleted=0", 'id', 'first_name');
	$reason_arr = return_library_array("select b.id, b.reason from lib_re_process_reason_entry b,pro_fab_subprocess f where f.reason_mst_id=b.id and f.batch_id=$batch_id and f.status_active=1 and f.is_deleted=0 and b.status_active=1 and b.is_deleted=0", 'id', 'reason');
	...
	foreach ($data_array as $row) {
		...
		$sql_batch_d = sql_select("select id,service_source,system_no,service_company,received_chalan,issue_chalan,issue_challan_mst_id,company_id,batch_id,batch_no,process_end_date,end_hours,end_minutes,machine_id,floor_id,process_id,ltb_btb_id,remarks,hour_load_meter from
					pro_fab_subprocess where batch_id='$batch_id' and entry_form=35 and load_unload_id=1 and status_active=1 and is_deleted=0 order by id desc ");
	...
	if ($load_unload == 1) {
		...
		$sql_batch = sql_select(
			"select id,batch_no,company_id,system_no,batch_id,service_source,service_company,received_chalan,issue_chalan,issue_challan_mst_id,process_end_date,load_unload_id,end_hours,end_minutes,machine_id,floor_id,process_id,
			ltb_btb_id,water_flow_meter,result,remarks,reason_mst_id,dyeing_type_id,hour_load_meter,multi_batch_load_id	from pro_fab_subprocess where batch_id='$batch_id' and id=$update_mst_id and entry_form=35 and load_unload_id in(1) and status_active=1 and is_deleted=0  ");
	} else if ($load_unload == 2) {
		$sql_batch = sql_select("select id,batch_no,company_id,system_no,service_source,service_company,received_chalan,issue_chalan,issue_challan_mst_id,batch_id,process_end_date,load_unload_id,end_hours,end_minutes,machine_id,floor_id,process_id,ltb_btb_id,water_flow_meter,result,remarks,dyeing_type_id,hour_unload_meter,shift_name,responsibility_id,in_charge_mst_id,fabric_type,production_date from pro_fab_subprocess where batch_id='$batch_id'  and entry_form=35 and load_unload_id in(2,1)  and status_active=1 and is_deleted=0 $select_group_row1");
	...
	foreach ($sql_batch as $row) {
		if($update_mst_id==$row[csf("id")])
		{
			...
			if($row[csf("load_unload_id")]>0)
			{
				$dyeing_load_unload_id=$row[csf("load_unload_id")];
			}
			if($row[csf("process_end_date")]>0)
			{
				$process_end_date=$row[csf("process_end_date")];
			}
			if($row[csf("end_hours")]>0)//end_hours,end_minutes
			{
				$end_hours=$row[csf("end_hours")];
			}
			if($row[csf("end_minutes")]>0)
			{
				$end_minutes=$row[csf("end_minutes")];
			}
			if($row[csf("responsibility_id")]=='') $row[csf("responsibility_id")]=0;
			$responsibility_id=$row[csf("responsibility_id")];
			if($row[csf("in_charge_mst_id")]>0)
			{
				$in_charge_mst_id=$row[csf("in_charge_mst_id")];
			}
			if($row[csf("shift_name")]=='') $row[csf("shift_name")]=0;
				$shift_id=$row[csf("shift_name")];
			if($row[csf("production_date")])
			{
				$production_date=$row[csf("production_date")];
			}
			if($row[csf("hour_unload_meter")])
			{
				$hour_unload_meter=$row[csf("hour_unload_meter")];
			}
			if($row[csf("remarks")])
			{
			...
			$dyeing_type_id=0;
			if($row[csf("dyeing_type_id")])
			{
				$dyeing_type_id=$row[csf("dyeing_type_id")];
			}
			if($row[csf("water_flow_meter")])
			{
				$water_flow_meter=$row[csf("water_flow_meter")];
			}
			if($row[csf("ltb_btb_id")]=='') $row[csf("ltb_btb_id")]=0;
			$ltb_btb_id=$row[csf("ltb_btb_id")];
			if($row[csf("fabric_type")]=='') $row[csf("fabric_type")]=0;

				$fabric_type=$row[csf("fabric_type")];
				$process_id=$row[csf("process_id")];
		}
	}
	// echo $fabric_type.'=XD='.$dyeing_load_unload_id;

	echo "load_drop_down( 'requires/dyeing_production_controller', " . $service_source . "+'**'+" . $company_id . ", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
	echo "load_drop_down( 'requires/dyeing_production_controller', $service_company+'**'+" . $floor_id . ", 'load_drop_machine', 'machine_td' );\n";
	...
	foreach ($sql_batch as $r_batch) {
		if ($load_unload == 1) //Load
		{
			...
			echo "document.getElementById('txt_water_flow').value	= '" . $r_batch[csf("water_flow_meter")] . "';\n";
			echo "document.getElementById('cbo_yesno').value	= '" . $r_batch[csf("multi_batch_load_id")] . "';\n";
			echo "document.getElementById('txt_reason_id').value	= '" . $r_batch[csf("reason_mst_id")] . "';\n";
			echo "document.getElementById('txt_reason_name').value	= '" . $reason_arr[$r_batch[csf("reason_mst_id")]] . "';\n";
			$minute = str_pad($r_batch[csf("end_minutes")], 2, '0', STR_PAD_LEFT);
			$hour = str_pad($r_batch[csf("end_hours")], 2, '0', STR_PAD_LEFT);
			echo "document.getElementById('txt_start_minutes').value	= '" . $minute . "';\n";
			echo "document.getElementById('txt_start_hours').value	= '" . $hour . "';\n";
		...
		//	echo "load_drop_down( 'requires/dyeing_production_controller', " . $r_batch[csf("service_source")] . "+'**'+" . $r_batch[csf("company_id")] . ", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
			...
			//echo "load_drop_down( 'requires/dyeing_production_controller', document.getElementById('cbo_service_company').value+'**'+" . $r_batch[csf("floor_id")] . ", 'load_drop_machine', 'machine_td' );\n";
			echo "document.getElementById('cbo_floor').value = '" . $floor_id . "';\n";
			echo "document.getElementById('cbo_machine_name').value = '" . $machine_id . "';\n";
			echo "document.getElementById('txt_remarks').value	= '" . $r_batch[csf("remarks")] . "';\n";
			echo "document.getElementById('cbo_dyeing_type').value	= '" . $r_batch[csf("dyeing_type_id")] . "';\n";
			if ($update_mst_id != 0) {
				//echo "document.getElementById('txt_system_no').value 			= '".$r_batch[csf("system_no")]."';\n";
		...
		{
			if ($dyeing_load_unload_id == 2) {
				echo "document.getElementById('txt_update_id').value 		= '" . $update_mst_id . "';\n";
				echo "document.getElementById('txt_batch_no').value 			= '" . $r_batch[csf("batch_no")] . "';\n";
				echo "document.getElementById('txt_process_end_date').value = '" . ($dyeing_load_unload_id == 1 ? "" : change_date_format($process_end_date)) . "';\n";
				echo "document.getElementById('txt_process_date').value = '" . ($dyeing_load_unload_id == 1 ? "" : change_date_format($production_date)) . "';\n";
				echo "document.getElementById('cbo_shift_name').value	= '" . $shift_id . "';\n";
				echo "document.getElementById('cbo_responsibility').value	= '" . $responsibility_id . "';\n";
				echo "document.getElementById('txt_in_charge_id').value	= '" . $in_charge_mst_id . "';\n";
				echo "document.getElementById('txt_in_charge').value	= '" . $in_charge_arr[$in_charge_mst_id] . "';\n";

				echo "document.getElementById('cbo_fabric_type').value	= '" . $fabric_type . "';\n";
				echo "document.getElementById('txt_unload_meter').value	= '" . $hour_unload_meter . "';\n";
				echo "document.getElementById('txt_water_flow').value	= '" . ($dyeing_load_unload_id == 1 ? "" : $r_batch[csf("water_flow_meter")]) . "';\n";
				echo "document.getElementById('cbo_ltb_btb').value	= '" . $ltb_btb_id . "';\n";
			}
			$process_name = '';
			...
			echo "document.getElementById('txt_process_id').value 				= '" . $process_id . "';\n";
			echo "process_check('" . $process_id . "');\n";
			//echo "document.getElementById('txt_process_name').value 				= '".$process_name."';\n";
			if ($dyeing_load_unload_id == 2) {
				$minute = str_pad($end_minutes, 2, '0', STR_PAD_LEFT);
				$hour = str_pad($end_hours, 2, '0', STR_PAD_LEFT);
				echo "document.getElementById('txt_end_minutes').value	= '" . $minute . "';\n";
				echo "document.getElementById('txt_end_hours').value	= '" . $hour . "';\n";
			}
			echo "$('#txt_issue_chalan').val('" . $r_batch[csf("issue_chalan")] . "');\n";
			...
			echo "document.getElementById('cbo_machine_name').value = '" . $machine_id . "';\n";
			//echo "document.getElementById('cbo_ltb_btb').value	= '".$r_batch[csf("ltb_btb_id")]."';\n";
			echo "document.getElementById('cbo_dyeing_type').value	= '" . $dyeing_type_id . "';\n";
			if ($dyeing_load_unload_id == 2) {
				if($result_id)
				{
				echo "document.getElementById('cbo_result_name').value	= '" . $result_id . "';\n";
				}
				echo "document.getElementById('txt_remarks').value	= '" . ($dyeing_load_unload_id == 1 ? "" : $remarks) . "';\n";
			}
			//echo "document.getElementById('txt_remarks').value	= '".$r_batch[csf("remarks")]."';\n";
			echo "$('#cbo_service_source').attr('disabled',true);\n";
			echo "$('#cbo_service_company').attr('disabled',true);\n";
...
if ($action == 'show_fabric_desc_listview')
{
	...
	$double_dyeing=0;
	//echo $process_id.'XX';
	$batch_data =sql_select("select company_id,double_dyeing,process_id from pro_batch_create_mst where id ='$batch_id' and entry_form in(0,136) and is_deleted=0 and status_active=1");
	foreach ($batch_data as $row)
	{
		$company_id = $row[csf('company_id')];
		$double_dyeing = $row[csf('double_dyeing')];
	...
	//$company_id = return_field_value("company_id", "pro_batch_create_mst", "id ='$batch_id' and is_deleted=0 and status_active=1");
	$roll_maintained = return_field_value("fabric_roll_level", "variable_settings_production", "company_name ='$company_id' and item_category_id=50 and variable_list=3 and is_deleted=0 and status_active=1");
	$page_upto_id = return_field_value("page_upto_id", "variable_settings_production", "company_name =$company_id and item_category_id=50 and variable_list=3 and is_deleted=0 and status_active=1 and page_upto_id>0");
	...
	if ($db_type == 0) {
		...
		if ($roll_maintained_id == 1) {
			...
			// 			where a.id=b.mst_id and b.roll_id=c.id and c.dtls_id=d.id and d.mst_id=e.id  and a.id=$batch_id and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and e.entry_form in(2,22)
			// 			group by b.po_id, b.prod_id");
			$yarn_lot_data = sql_select("SELECT  b.po_id, b.prod_id, d.yarn_lot  as yarn_lot, d.yarn_count   as yarn_count, d.brand_id   as brand_id
			from pro_batch_create_mst a,pro_batch_create_dtls b, pro_roll_details c, pro_grey_prod_entry_dtls d,  inv_receive_master e
			where a.id=b.mst_id and b.roll_id=c.id and c.dtls_id=d.id and d.mst_id=e.id  and a.id=$batch_id and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and e.entry_form in(2,22)");
		} else {
	...
	foreach ($yarn_lot_data as $rows)
	{
		...
		if($rows[csf('yarn_lot')])
		{
			$yarn_lot_arr[$rows[csf('prod_id')]][$rows[csf('po_id')]]['lot'] .= $rows[csf('yarn_lot')].',';
		}
		if($rows[csf('brand_id')])
		{
			$yarn_lot_arr[$rows[csf('prod_id')]][$rows[csf('po_id')]]['yarn_count'] .= $rows[csf('brand_id')].',';
		}
		if($rows[csf('yarn_count')])
		{
			$yarn_lot_arr[$rows[csf('prod_id')]][$rows[csf('po_id')]]['brand_id'] .= $rows[csf('yarn_count')].',';
		}
	}
	...
	if($max_load_id!="")
	{
		$load_insert = ("SELECT a.load_unload_id,b.production_qty,b.roll_id,b.no_of_roll,b.prod_id,b.gsm,b.dia_width,b.barcode_no,b.width_dia_type, b.batch_qty from  pro_fab_subprocess a, pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$batch_id and a.entry_form=35  and a.load_unload_id in(1) and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.is_deleted=0 and b.mst_id in($max_load_id) ");
		// echo $load_insert;
	...
	if($entry_form!=136)
	{
		if (($page_upto_id == 2 || $page_upto_id > 2) && $roll_maintained == 1) {
			...
			foreach ($result_data as $row) {
				...
				$brand =   chop($yarn_lot_arr[$row[csf('prod_id')]][$row[csf('po_id')]]['brand_id'],',');
				$lots =  chop($yarn_lot_arr[$row[csf('prod_id')]][$row[csf('po_id')]]['lot'],',');
				$lot = implode(", ",array_unique(explode(",", $lots)));
				$lot =chop($lot,',');
				...
				$y_count_id = rtrim($yarn_lot_arr[$row[csf('prod_id')]][$row[csf('po_id')]]['yarn_count'].',');
				if ($db_type == 0) {
				...
				if($load_unload==2)
				{
					if($dyeing_type_id==2) //CBP
					{
						...
						//echo $load_prod_qnty.'A';
					}
					else
					{
						//echo $row[csf('batch_barcode')].'='.$gsm.'<br>';
						$load_prod_qnty=$load_qty_arr[$row[csf('prod_id')]][$row[csf('batch_barcode')]][$gsm][$row[csf('width_dia_type')]];
				...
				foreach ($result as $row)
				{
					...
					if($prod_qnty>0) //zero Qty check start
					{
						...
						<tr class="general" id="row_<? echo $i; ?>">
							...
							<td><input type="text" name="txtconscomp_<? echo $i; ?>" id="txtconscomp_<? echo $i; ?>" class="text_boxes" style="width:170px;" value="<? echo $cons_comps; ?>" disabled/></td>
							<td><input type="text" name="txtgsm_<? echo $i; ?>" id="txtgsm_<? echo $i; ?>" class="text_boxes" style="width:40px;" value="<? echo trim($gsm); ?>" readonly/></td>
							<td><input type="text" name="txtdiawidth_<? echo $i; ?>" id="txtdiawidth_<? echo $i; ?>" class="text_boxes" style="width:40px;" value="<? echo $dia_width; ?>" disabled  /></td>
							<td><input type="text" name="txtdiatype_<? echo $i; ?>" id="txtdiatype_<? echo $i; ?>" class="text_boxes" style="width:70px;" value="<? echo $fabric_typee[$row[csf('width_dia_type')]]; ?>" disabled/>
							<input type="hidden" name="hiddendiatypeid_<? echo $i; ?>" id="hiddendiatypeid_<? echo $i; ?>" value="<? echo $row[csf('width_dia_type')]; ?>" readonly/>
							...
							<td><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes"   style="width:40px;" value="<? echo $roll_no; ?>" readonly/> <input type="hidden" name="rollid_<? echo $i; ?>" id="rollid_<? echo $i; ?>" style="width:50px;" value="<? echo $row[csf('roll_id')]; ?>" class="text_boxes_numeric" readonly/>
							</td>
							<td><input type="text" name="txtbatchqnty_<? echo $i; ?>" id="txtbatchqnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px; background:<? echo $batch_filed; ?>" onKeyUp="calculate_production_qnty2();" value="<? echo number_format($row[csf('batch_qnty')], 2, ".", ""); ?>" <? echo $batch_filed_read ?>/>
							</td>
							<td><input type="text" name="txtprodqnty_<? echo $i; ?>" id="txtprodqnty_<? echo $i; ?>"class="text_boxes_numeric" style="width:60px;" onKeyUp="calculate_production_qnty();" value="<? echo number_format($prod_qnty, 2, ".", ""); ?>" <? echo $readdata ?> />
				...
				foreach ($result as $row)
				 {
					...
					<tr class="general" id="row_<? echo $i; ?>">
						...
						<td><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes"   style="width:40px;" value="<? echo $roll_no; ?>" readonly/>
...
if ($action == 'issue_show_fabric_desc_listview') {
	...
	foreach ($result as $row) {
		...
		$y_count_id =chop($yarn_lot_arr[$row[csf('prod_id')]][$row[csf('po_id')]]['yarn_count'],',');
		$count_id = array_unique(explode("**", $y_count_id));
		//print_r( $count_id).'aziz';
		//array_unique(explode(',',$y_count));
		$yarn_count_value = '';
		...
		if (($page_upto_id == 2 || $page_upto_id > 2) && $roll_maintained == 1) {
			$roll_no = $row[csf('roll_no')];//$fabric_roll_arr[$row[csf('roll_id')]]['roll'];
...
if ($action == "save_update_delete") {
	...
	$load_dying_started = str_replace("'", "", $txt_dying_started);
	$load_dying_started_chk = strtotime($load_dying_started);
	...
	$sql_batch=sql_select("select batch_no,batch_date from pro_batch_create_mst where id =$batch_id and is_deleted=0 and status_active=1");
	foreach($sql_batch as $row)
	{
	...
	if (str_replace("'", '', $cbo_load_unload) == 2) //unLoad
	{
		...
		else if($unload_process_start_date_chk<$load_dying_started_chk) //unload_process_start_date_chk
		{
			echo "23**Prod Date is found backdate  than loading Start date=".date('d-m-Y',$load_dying_started_chk).'='.date('d-m-Y',$unload_process_start_date_chk).'='.$load_dying_started_chk;
			disconnect($con);
			die;
		}
	}
	...
	if ($operation == 0)  // Insert Here
	{
		...
		if (str_replace("'", '', $cbo_load_unload) == 2) {
			...
			//}
					...
					$sql_loadunload="select id,load_unload_id, batch_id from pro_fab_subprocess where  company_id=".$cbo_company_id." and  batch_id=".$txt_batch_ID." and entry_form=35 and is_deleted=0 and status_active=1   order by id desc";
		...
		//if($updateId) $up_id_cond="and a.id!=$updateId";else $up_id_cond="";
				...
				if($multi_dyeing==2 && $cbo_dyeing_type==1) // is Multi no //CBP Dyeing
					...
					$sql_loadunload="select id,load_unload_id, batch_id,machine_id from pro_fab_subprocess where  company_id=".$cbo_company_id." and  batch_id=".$txt_batch_ID."  and entry_form=35 and is_deleted=0 and status_active=1  and machine_id=$cbo_machine_id  order by id desc";//and load_unload_id=1
					$loadunload_data_array=sql_select($sql_loadunload);
		...
		$page_upto_id = return_field_value("page_upto_id", "variable_settings_production", "company_name =$cbo_company_id and item_category_id=50 and variable_list=3 and is_deleted=0 and status_active=1 and page_upto_id>0");

		//echo "10**=".$page_upto_id.'D';die;
		...
		if ($cbo_load_unload == "'1'") {
			...
			$field_array = "id,company_id,system_no,service_source,service_company,received_chalan,issue_chalan,issue_challan_mst_id,batch_no,batch_id,batch_ext_no,process_id,ltb_btb_id,water_flow_meter,process_end_date,end_hours,end_minutes,machine_id,floor_id,load_unload_id,entry_form,multi_dyeing_id,remarks,dyeing_type_id,hour_load_meter,reason_mst_id,multi_batch_load_id,inserted_by,insert_date";
			$data_array = "(" . $id . "," . $cbo_company_id . "," . $system_no . "," . $cbo_service_source . "," . $cbo_service_company . "," . $txt_recevied_chalan . "," . $txt_issue_chalan . "," . $txt_issue_mst_id . "," . $txt_batch_no . "," . $txt_batch_ID . "," . $txt_ext_id . "," . $txt_process_id . "," . $cbo_ltb_btb . "," . $txt_water_flow . "," . $txt_process_start_date . "," . $txt_start_hours . "," . $txt_start_minutes . "," . $cbo_machine_name . "," . $cbo_floor . "," . $cbo_load_unload . ",35," . $hidden_double_dyeing . "," . $txt_remarks . "," . $cbo_dyeing_type . "," . $txt_load_meter . "," . $txt_reason_id ."," . $cbo_yesno . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
	...
	//	echo "10**".$rID."**".$rID2."**".$flag.'**'.$load_unload_id."**".$msg;die;
		if ($db_type == 0) {
			if ($flag == 1) {
				mysql_query("COMMIT");
	...
	else if ($operation == 1)   // Update Here
	{
		...
		$cbo_machine_id=str_replace("'","",$cbo_machine_name);
		//if($updateId) $up_id_cond="and a.id!=$updateId";else $up_id_cond="";
		$batch_prod=sql_select("select a.batch_id,max(a.id) as mst_id from pro_fab_subprocess a where  a.batch_id=$txt_batch_ID and a.entry_form=35 and a.company_id=$cbo_company_id and a.status_active=1  and a.machine_id=$cbo_machine_id group by a.batch_id ");
		$last_mst_id=0;
		foreach($batch_prod as $row)
		{
			$last_mst_id= $row[csf('mst_id')];
		}
		$batch_prod_mc=sql_select("select a.batch_id,max(a.id) as mst_id from pro_fab_subprocess a where  a.batch_id=$txt_batch_ID and a.entry_form=35 and a.company_id=$cbo_company_id and a.status_active=1 and a.load_unload_id=2  group by a.batch_id ");
		$unload_found="";
		foreach($batch_prod_mc as $row)
		{
			$unload_found= $row[csf('mst_id')];
		}
		if($load_unloadId==1 && $unload_found!="")
		{
			echo "14**Unload Found,Update not allowed.";
			disconnect($con);
			die;
		}

		$batch_last_process=sql_select("select a.batch_id,a.load_unload_id,a.result,a.end_minutes,a.process_end_date,a.production_date from pro_fab_subprocess a where  a.batch_id=$txt_batch_ID and a.company_id=$cbo_company_id and a.id in($last_mst_id) and a.machine_id=$cbo_machine_id and a.status_active=1  ");
		...
		//  $msg='Please load correct machine which already unloaded='.$sql_loadunload;
		// 		echo "101**".$msg;
		// 		disconnect($con);
		// 		die;
	...
	//	echo "10**=".$double_dyeing_id.'='.$load_unloadId.'='.$last_load_unload_id;die;
		...
		if ($cbo_load_unload == "'1'") {
			$field_array_update = "service_source*service_company*received_chalan*issue_chalan*issue_challan_mst_id*batch_no*batch_id*batch_ext_no*process_id*ltb_btb_id*water_flow_meter*process_end_date*end_hours*end_minutes*machine_id*floor_id*load_unload_id*entry_form*remarks*dyeing_type_id*hour_load_meter*reason_mst_id*multi_batch_load_id*updated_by*update_date";
			$data_array_update = "" . $cbo_service_source . "*" . $cbo_service_company . "*" . $txt_recevied_chalan . "*" . $txt_issue_chalan . "*" . $txt_issue_mst_id . "*" . $txt_batch_no . "*" . $txt_batch_ID . "*" . $txt_ext_id . "*" . $txt_process_id . "*" . $cbo_ltb_btb . "*" . $txt_water_flow . "*" . $txt_process_start_date . "*" . $txt_start_hours . "*" . $txt_start_minutes . "*" . $cbo_machine_name . "*" . $cbo_floor . "*" . $cbo_load_unload . "*35*" . $txt_remarks . "*" . $cbo_dyeing_type . "*" . $txt_load_meter . "*" . $txt_reason_id . "*" . $cbo_yesno . "*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'";//
	...
	else if ($operation == 2)   // Delete Here
	{
		$con = connect();
		$update_id=str_replace("'", "", $txt_update_id);
		$next_transaction_found = return_field_value("batch_no", "pro_fab_subprocess", "batch_id =$txt_batch_ID and entry_form!=35 and is_deleted=0 and status_active=1","batch_no");
		if($next_transaction_found!="")
		{
			echo "17**Next Transaction Found,Delete not allowed.";
			disconnect($con);die;
		}

		$field_array="updated_by*update_date*status_active*is_deleted";
		$data_array="".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*'0'*'1'";
		$rID=sql_update("pro_fab_subprocess",$field_array,$data_array,"id",$update_id,0);
		if ($rID)
			$flag = 1;
		else
			$flag = 0;
		...
		$rID_dtls=sql_update("pro_fab_subprocess_dtls",$field_array_dtls,$data_array_dtls,"mst_id",$update_id,0);
		if($flag==1)
		{
			if ($rID_dtls) $flag = 1;else $flag = 0;
		}
		//echo "10**=".$rID.'='.$rID_dtls.'='.$flag.'=A='.$update_id;die;
//
		if ($flag == 1)
		{
			oci_commit($con);
			echo "2**" . str_replace("'", "", $update_id) . "**" . str_replace("'", "", $txt_batch_ID) . "**" .  str_replace("'", "", $txt_system_no) . "**" . str_replace("'", "", $cbo_load_unload);
		}
		else
		{
			oci_rollback($con);
			echo "10**".str_replace("'",'',$update_id)."**".str_replace("'",'',$txt_batch_ID);
		}
...
if ($action == "check_batch_no_for_machine") {
 ...
 //$back_date_cond="and a.process_end_date>='30-Oct-2022'";
            ...
            <fieldset>
                <table cellpadding="0" cellspacing="2" width="100%" id="main_tbl">
                    ...
                    <tr>
                        ...
                        <td>
							<?
							echo create_drop_down("cbo_company_id", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name", "id,company_name", 1, "--Select Company--", $selected, "load_drop_down('requires/dyeing_production_controller', this.value, 'load_drop_floor', 'floor_td' );roll_maintain();", "", "", "", "", "");
							?>
                    ...
                    <tr>
                        ...
                        <td id="floor_td">
							<?
							echo create_drop_down("cbo_floor", 130, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 order by floor_name", "id,floor_name", 1, "-- Select Floor --", $selected, "", 0, "", "", "", "", 4);
							?>
                        </td>
                    <tr>
                    <tr>
					...
					<tr>
                        <td id="">Reason Name</td>
						<td>
						<input type="text" name="txt_reason_name" id="txt_reason_name" placeholder="Browse" onDblClick="fnc_reason_name(); " class="text_boxes" style="width:120px" disabled="disabled" readonly>
						<input type="hidden" name="txt_reason_id" id="txt_reason_id" class="text_boxes" style="width:40px" disabled="disabled">
						</td>
					</tr>
                    <tr>
                        ...
                        <td>
							<?
							echo create_drop_down("cbo_yesno", 80, $yes_no, "", 0, "-- Select--", 1, "fnc_load_data(this.value);", 0, "", "", "", "");
							?>
                        </td>
                        </tr>
            ...
            <table cellpadding="0" cellspacing="2" width="100%" id="main_tbl">
                ...
                <tr>
				 ...
				 <tr>
                        <td id="">In-Charge</td>
						<td>
						<input type="text" name="txt_in_charge" id="txt_in_charge" placeholder="Browse" onDblClick="fnc_incharge_name(); " class="text_boxes" style="width:120px" readonly >
...
if ($action == "reason_name_popup") {
	echo load_html_head_contents("Reason Name Info", "../../../", 1, 1, '', '', '');
	extract($_REQUEST);
	?>
	<script>
	...
	<input type="hidden" name="reason_hdn" id="reason_hdn" value=""/>
	<?
		$reason_type_arr = array(1=>"In-Charge",2=>"Others");
		$section_arr = array(1=>"Knitting",2=>"Dyeing", 3=>"Finishing", 4=>"Sewing");
		$arr=array(0=>$section_arr,2=>$reason_type_arr);
	 	$sql = "select id, reason,reason_type,section from lib_re_process_reason_entry where  section=2  and status=1 and status_active=1 and is_deleted=0 order by reason";

	echo create_list_view("tbl_list_search", "Section,Reason Name,Reason Type", "150,100,100", "370", "160", 0, $sql, "js_set_value", "id,reason", "", 1, "section,0,reason_type", $arr, "section,reason,reason_type", "", 'setFilterGrid("tbl_list_search",-1);', '0', '', 0);
	exit();
}
if ($action == "incharge_name_popup") {
	...
	<?
		...
		$sql = "select id, first_name from lib_employee where  in_charge like '%2%' and status_active=1 and is_deleted=0 order by first_name";
...
if ($action == "load_drop_down_knitting_com") {
	...
	if ($data[0] == 1) {
		...
		echo create_drop_down("cbo_service_company", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "-- Select --", "", "load_drop_down('requires/dyeing_production_controller', this.value, 'load_drop_floor', 'floor_td' );load_batch_working_company(this.value);fnc_mc_load()", "");
	} else if ($data[0] == 3) {
...
if ($action == "child_form_input_data") {
	...
	foreach ($yarn_lot_data as $rows) {
		...
		if($rows[csf('brand_id')])
		{
			$yarn_lot_arr[$rows[csf('prod_id')]][$rows[csf('po_id')]]['brand_id'] .=$rows[csf('brand_id')].',';
		}
		if($rows[csf('yarn_count')])
		{
	...
	//----------------------------------------------------------------------------
	$prodData = sql_select("select distinct b.id,detarmination_id from product_details_master b,pro_batch_create_dtls c where c.prod_id=b.id and c.mst_id=$batch_id and b.item_category_id=13  and c.is_deleted=0 and c.status_active=1");
	...
	foreach ($sql_result as $row) {
		...
		$brand = chop($yarn_lot_arr[$prod_id][$po_id]['brand_id'],',');
		$lot =chop($yarn_lot_arr[$prod_id][$po_id]['lot'],',');
		$brand_id = explode(',', $brand);
		$brand_value = "";
		foreach ($brand_id as $val) {
			if ($val > 0) {
		...
		$y_count_id = chop($yarn_lot_arr[$prod_id][$po_id]['yarn_count'],',');
		if ($db_type == 0) {
			$count_id = array_unique(explode(",", $y_count_id));
		} else {
			$count_id = array_unique(explode(",", $y_count_id));
        ...
        <tr class="general" id="row_<? echo $i; ?>">
            <td width="60" id="sl_<? echo $i; ?>">
                ...
                &nbsp; &nbsp;</td>
            <td><input type="text" name="txtconscomp_<? echo $i; ?>" id="txtconscomp_<? echo $i; ?>" class="text_boxes" style="width:170px;" value="<? echo $cons_comps; ?>" disabled/></td>
            <td><input type="text" name="txtgsm_<? echo $i; ?>" id="txtgsm_<? echo $i; ?>" class="text_boxes_numeric"  style="width:40px;" value="<? echo trim($gsm); ?>" readonly/></td>
            ...
            <td><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes_numeric" value="<? echo $roll_no; ?>" style="width:40px;" <? echo $readonly; ?> readonly />
...
//echo $data."<br>";
	...
	//$po_id_arr = return_field_value("po_id", "pro_batch_create_dtls", "mst_id ='$batch_id' and is_deleted=0 and status_active=1");
	//$company_id = return_field_value("company_id", "pro_batch_create_mst", "id ='$batch_id' and is_deleted=0 and status_active=1");
	$sql_batch=sql_select("select COMPANY_ID,PO_ID from pro_batch_create_mst where id ='$batch_id' and is_deleted=0 and status_active=1");
	foreach ($sql_batch as $row)
	{
    ...
    <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" width="800" rules="all">
		...
		<? foreach ($result as $row) {
            ...
            <tr bgcolor="<? echo $bgcolor; ?>"
                onClick='show_list_view("<? echo $batch_id . '_' . $load_unload . '_' . $po_id_arr . '_' . $row[csf("id")]. '_' . $entry_form_no; ?>","child_form_input_data","list_fabric_desc_container","requires/dyeing_production_controller");get_php_form_data("<? echo $load_unload . '_' . $row[csf("batch_id")] . '_' . $row[csf("batch_no")] . '_' . $row[csf("company_id")]. '_' . $entry_form_no. '_' . $row[csf("id")]; ?>","populate_data_from_batch2","requires/dyeing_production_controller")'
                style="cursor:pointer">
                <td width="30"><? echo $i; ?></td>
...
if ($action == "create_booking_search_list_view") {
	...
	$sql_po=sql_select("select b.id, c.job_no_prefix_num from wo_po_details_master c, wo_po_break_down b,wo_booking_dtls d,wo_booking_mst a where c.id=b.job_id and b.id=d.po_break_down_id and a.booking_no=d.booking_no and a.job_no=b.job_no_mst  and a.booking_type=3 and a.supplier_id=$supplier_id and  a.status_active=1 and a.is_deleted=0 and d.process=$process_id $sql_cond  and  a.status_active=1 and a.is_deleted=0");
	foreach ($sql_booking as $row) {
		$job_no_arr[$row[csf('id')]]=$row[csf('job_no_prefix_num')];
...
if ($action == "create_sys_search_list_view") {
	...
	$sql = "select a.entry_form,a.batch_for,a.batch_against,a.extention_no,b.service_source,b.service_company,b.id,b.batch_no,b.company_id,b.load_unload_id,b.batch_id,b.system_no,b.process_end_date,b.end_hours,b.end_minutes,b.production_date
			from  pro_batch_create_mst a,pro_fab_subprocess b where a.id=b.batch_id and b.entry_form=35 and b.status_active=1 and b.is_deleted=0 $com_cond $sys_cond $batch_no_cond  $load_unload_cond $user_working_comp_cond order by b.id desc";
	//echo $sql;
	$sql2=sql_select($sql);
	foreach ($sql2 as $row) {
	...
	$sql_last_prod="select max(b.id) as mst_id,b.batch_id
			from  pro_batch_create_mst a,pro_fab_subprocess b where a.id=b.batch_id and b.entry_form=35 and b.status_active=1 and b.is_deleted=0 $com_cond $sys_cond $batch_no_cond  $load_unload_cond $user_working_comp_cond group by b.batch_id order by mst_id desc";
...
if ($action == "dyeing_pro_print") {
	...
	$batch_id=$dataArr1[0][csf('batch_id')];
	$sales_order_result = sql_select("select b.id,b.job_no,b.within_group,b.sales_booking_no,b.style_ref_no, b.customer_buyer from fabric_sales_order_mst b,pro_batch_create_mst d where b.id=d.sales_order_id and status_active=1 and is_deleted=0 and d.id in($batch_id)");
	$sales_arr = array();
	...
	//print_r($dataArr1);
	$qry_rslt = "select a.job_no,a.buyer_name,a.style_ref_no,b.po_number,c.mst_id as batch_id,d.color_id, c.po_id,c.is_sales from wo_po_details_master a,wo_po_break_down b,pro_batch_create_dtls c,pro_batch_create_mst d where c.mst_id=d.id and a.id=b.job_id and b.id=c.po_id and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.id in($batch_id)";
	//echo $qry_rslt;
	...
	foreach ($dataArr2 as $row) {
		...
		{
			$po_batch_data[$row[csf('batch_id')]]['po'] = $sales_arr[$row[csf("po_id")]]["sales_order_no"];
			$po_batch_data[$row[csf('batch_id')]]['style'] = $sales_arr[$row[csf("po_id")]]["style_ref_no"];
			$po_batch_data[$row[csf('batch_id')]]['buyer'] = $sales_arr[$row[csf("po_id")]]["customer_buyer"];
		}
		else
		{
	...
	foreach ($yarn_lot_data as $rows) {
		...
		$yarn_countArr[$rows[csf('yarn_count')]]=$rows[csf('yarn_count')];
		$yarn_lottArr[$rows[csf('yarn_lot')]]=$rows[csf('yarn_lot')];
		$yarn_brandArr[$rows[csf('brand_id')]]=$rows[csf('brand_id')];
	...
	$brand_ids = implode(",",$yarn_brandArr);//array_unique(explode(',', $yarn_lot_data[0][csf('brand_id')]));
	$yarn_lot = implode(",",$yarn_lottArr);
	$style_ref = "";
	$fabric_type = "";
	$buyer_name = "";
	$po_nmbr = "";
