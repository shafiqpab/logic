...
if ($action=="load_drop_floor")
{
	...
	echo create_drop_down( "cbo_floor", 135, "select id,floor_name from lib_prod_floor where status_active=1 and is_deleted=0 and company_id='$data[0]'  and production_process=4 order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected,"load_drop_down( 'requires/slitting_squeezing_controller', document.getElementById('cbo_service_company').value+'**'+this.value, 'load_drop_machine', 'machine_td' );" );     	 
	exit();
}
...
if($action=="load_drop_down_knitting_com")
{
	...
	if($data[0]==1)
	{
		echo create_drop_down( "cbo_service_company", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name",1, "-- Select --", 0, "load_drop_down('requires/slitting_squeezing_controller', this.value, 'load_drop_floor', 'floor_td' );","" );
	}
	else if($data[0]==3)
	{	
...
if($action=="create_batch_search_list_view")
{
	...
	{
		echo "<div align='center'> <b>Please write Batch no/Date Ranage</b>.</div>";die;
	}
	...
	if($db_type==2)
		...
		$batch_date_con3 = " and c.batch_date between '".change_date_format($start_date, "mm-dd-yyyy", "-",1)."' and '".change_date_format($end_date, "mm-dd-yyyy", "-",1)."'";
		}
		else 
		{ 
			$batch_date_con ="";$batch_date_con2 ="";$batch_date_con3 ="";
		}
		...
		if($batch_type==0 || $batch_type==2)
			...
			$sql_po=sql_select("select b.mst_id, a.job_no_mst,group_concat(distinct a.order_no )  as po_no from  subcon_ord_dtls a, pro_batch_create_dtls b,pro_batch_create_mst c where a.id=b.po_id and c.id=b.mst_id and  b.status_active=1 and b.is_deleted=0 and  a.status_active=1 and a.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 $batch_date_con3 group by b.mst_id,a.job_no_mst");
			}
			else
			{
				
			$sql_po=sql_select("select b.mst_id, a.job_no_mst,group_concat( distinct a.po_number)  as po_no from wo_po_break_down a, pro_batch_create_dtls b,pro_batch_create_mst c where a.id=b.po_id and c.id=b.mst_id and  c.status_active=1 and c.is_deleted=0 and  b.status_active=1 and b.is_deleted=0 $batch_date_con3  group by b.mst_id,a.job_no_mst");
	...
	$resultChkArr=array(1,2,3,4,5,6); //and b.result in(2,3,4,5,6)
    $sql_prod = "select a.id, a.batch_no, b.result from pro_batch_create_mst a,pro_fab_subprocess b  where a.id=b.batch_id and b.entry_form in(35,38) and b.status_active=1 and b.is_deleted=0  and b.load_unload_id=2  and b.status_active=1 and b.is_deleted=0 $batch_cond2 $batch_date_con2 order by a.id desc"; 
$nameArray_prod=sql_select( $sql_prod );
	foreach ($nameArray_prod as $row)
	{
		 // 2,3,4,5,6
		 if(in_array($row[csf('result')],$resultChkArr))
		 {
	...
	$sql = "select id, batch_no, batch_date, batch_weight, booking_no, extention_no, color_id, batch_against, re_dyeing_from,sales_order_id, is_sales from pro_batch_create_mst where batch_for in(0,1) and batch_against<>4 and status_active=1 and is_deleted=0 $search_field_cond_batch $batch_date_con $batch_cond order by id desc"; 
	...
	foreach ($nameArray as $selectResult)
	{
		...
		{
			$is_sales_flag=1;
			$sales_id_arr[$selectResult[csf('sales_order_id')]]= $selectResult[csf('sales_order_id')];
		}
	}
            ...
            <?
				...
				foreach ($nameArray as $selectResult)
				{
					...
					{	
						...
						if($unload_batch_dyeing_chk_arr[$selectResult[csf('id')]]>0) //Unloaded --issue id-10471
						{
					...
					else
					{
						...
						{
							if($unload_batch_dyeing_chk_arr[$row[csf('id')]]>0 && $row[csf('re_dyeing_from')]==0) //Unloaded
							{
								$po_no=implode(",",array_unique(explode(",",$po_num[$selectResult[csf('id')]]['po_no'])));
...
if($action=='populate_data_from_batch')
{
	...
	else if($db_type==2) $select_group_row="group by a.entry_form,a.sales_order_id,a.batch_no,a.batch_weight,a.color_id, 
	...
	else if($db_type==2) $pop_batch=" group by a.batch_no, a.batch_weight,batch_date,a.color_id,a.color_range_id, a.booking_without_order,a.company_id,a.process_id,a.entry_form,a.booking_no,a.total_trims_weight";
	...
	if($batch_no!='')
	{ 
		$sql_re="SELECT MAX(a.id) as id,a.batch_no,a.entry_form,a.sales_order_id,a.batch_date,a.company_id,a.total_trims_weight, a.batch_weight,Max(a.extention_no) as extention_no,a.process_id as process_id_batch,a.color_id,a.color_range_id,max(a.insert_date) as insert_date, a.booking_without_order,a.booking_no, sum(b.batch_qnty) as batch_qnty, $select_list from pro_batch_create_mst a,pro_batch_create_dtls b where a.batch_no='$batch_no' and a.entry_form in(0,36)  and a.id=b.mst_id  and b.status_active=1  $select_group_row ";
	}
	else
	{	 
		$sql_re="SELECT a.company_id,a.total_trims_weight,MAX(a.id)as id,a.batch_no,a.sales_order_id,a.entry_form, a.batch_weight,Max(a.extention_no) as extention_no,a.batch_date,a.process_id as process_id_batch, a.booking_no,
	a.color_id,a.color_range_id,Max(a.insert_date) as insert_date, a.booking_without_order, sum(b.batch_qnty) as batch_qnty, $select_list from pro_batch_create_mst a,pro_batch_create_dtls b where a.id='$batch_id'  and a.id=b.mst_id and a.entry_form in(0,36)  and b.status_active=1 $pop_batch";	
	}
	$data_array=sql_select($sql_re);
	$sales_idArr=array();
	foreach ($data_array as $row)
	{
		$sales_idArr[$row[csf('sales_order_id')]]=$row[csf('sales_order_id')]; 
		$company_id=$row[csf('company_id')]; 
	}
if(count($sales_idArr)>0)
 {
   $sql_sales_job=array();
   $sql_sales_job=sql_select("SELECT a.buyer_id, b.job_no as job_no_mst,b.booking_no,f.job_no as sales_order_no,f.within_group from wo_booking_mst a, wo_booking_dtls b, wo_po_break_down c, fabric_sales_order_mst f where a.booking_no=b.booking_no and b.booking_no=f.sales_booking_no and b.po_break_down_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and f.id in(".implode(",",$sales_idArr).") group by b.job_no,b.booking_no,f.job_no,f.within_group,a.buyer_id");
   ...
   $sql_sales_job2=sql_select("SELECT  sales_booking_no as  booking_no, job_no as sales_order_no ,buyer_id from   FABRIC_SALES_ORDER_MST  
	where status_active=1 and within_group=2  and id in(".implode(",",$sales_idArr).") group by sales_booking_no , job_no,buyer_id ");

   foreach ($sql_sales_job2 as $sales_job_row) {
	   $sales_job_arr2[$sales_job_row[csf('booking_no')]]["sales_order_no"] = $sales_job_row[csf('sales_order_no')];
	   $sales_job_arr2[$sales_job_row[csf('booking_no')]]["buyer_id"] = $sales_job_row[csf('buyer_id')];
   }
	unset($sql_sales_job2);
 }
	...
	$re_slitting_no=return_field_value("re_stenter_no","pro_fab_subprocess","batch_id ='$batch_id' and entry_form=30  and is_deleted=0 and status_active=1");
	...
	foreach ($data_array as $row)
	{
		...
		echo "document.getElementById('txt_trims_weight').value	= '".$total_trims_weight."';\n";
	...
	//	echo "load_drop_down( 'requires/slitting_squeezing_controller', '".$row[csf("company_id")]."', 'load_drop_floor', 'floor_td' );\n";
		...
		foreach($sql_batch_d as $dyeing_d)
		{
			...
			if($dyeing_d[csf("load_unload_id")]==2)
			{
			echo "document.getElementById('txt_unload_end_date').value = '".change_date_format($dyeing_d[csf("production_date")])."';\n";
			echo "document.getElementById('txt_unload_end_time').value = '".$dyeing_d[csf("end_hours")].':'.$dyeing_d[csf("end_minutes")]."';\n";
			}
		}
...
if($action=='show_fabric_desc_listview')
{
	...
	else if($db_type==2)
	{
		...
		foreach ($yarn_lot_data as $rows) 
		{
			...
			if($rows[csf('yarn_lot')])
			{
				$yarn_lot_arr_prodwise[$rows[csf('prod_id')]]['lot'] .= $rows[csf('yarn_lot')].',';
			}
			if($rows[csf('brand_id')])
			{
				$yarn_lot_arr_prodwise[$rows[csf('prod_id')]]['yarn_count'] .= $rows[csf('brand_id')].',';
			}
			if($rows[csf('yarn_count')])
			{
				$yarn_lot_arr_prodwise[$rows[csf('prod_id')]]['brand_id'] .= $rows[csf('yarn_count')].',';
			}
		}
		...
		{ 
			...
			<tr class="general" id="row_<? echo $i; ?>">
				...
				<input type="hidden" name="txtdiawidthID_<? echo $i; ?>" id="txtdiawidthID_<? echo $i; ?>" value="<? echo $row[csf('width_dia_type')];?>" readonly />
                 ...
                 <td>
                  <input type="text" name="txtbarcode_<? echo $i; ?>" id="txtbarcode_<? echo $i; ?>" class="text_boxes_numeric" style="width:65px;" value="<? echo $barcode;?>"/>
                 </td>
	...
	//	$batch_id_search=sql_select("select a.batch_id from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$ex_data[0] and company_id=$ex_data[1] and entry_form=30");
		...
		//$sql_result=sql_select("SELECT a.id as mst_id, b.id, b.gsm,b.width_dia_type,b.dia_width,b.const_composition,b.batch_qty,b.production_qty, b.roll_no,b.no_of_roll,b.prod_id,b.rate, b.amount from pro_fab_subprocess_dtls b,pro_fab_subprocess a where a.id=b.mst_id and a.batch_id='$batch_id' and b.entry_page=30 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and a.re_stenter_no=$re_slitting_no  order by a.id desc,b.id asc ");
		 ...
		 $sql_result=sql_select("SELECT a.id as batch_id,a.entry_form,b.width_dia_type,b.gsm,b.fin_dia,b.item_description, sum(b.batch_qnty) as batch_qnty,b.prod_id,count(b.roll_no) as  roll_no from pro_batch_create_dtls b,pro_batch_create_mst a where b.mst_id=$batch_id and a.id=b.mst_id and  a.entry_form in(0,36) and b.status_active=1 and b.is_deleted=0 group by a.id,a.entry_form,b.width_dia_type,b.gsm,b.fin_dia,b.item_description,b.prod_id  order by b.item_description ");
		...
		if(count($sql_result)>0)
		{
			...
			if(!$prod_prev_qty) $prod_prev_qty=$row[csf('batch_qnty')];
				...
				<tr class="general" id="row_<? echo $i; ?>">
					...
					<td>
						<input type="text" name="txtbarcode_<? echo $i; ?>" id="txtbarcode_<? echo $i; ?>" class="text_boxes_numeric" style="width:65px;" value="<? echo $barcode;?>" readonly />
					</td>
					<td><input type="text" name="txtbatchqnty_<? echo $i; ?>" id="txtbatchqnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:50px;" value="<? echo number_format($row[csf('batch_qnty')],2,'.',''); ?>" disabled/>
						<input type="hidden" name="txtprodid_<? echo $i; ?>" id="txtprodid_<? echo $i; ?>"  value="<? echo $row[csf('prod_id')];?>" />
						<input type="hidden" name="txtdeterid_<? echo $i; ?>" id="txtdeterid_<? echo $i; ?>"  value="<? echo $fabric_desc_arr[$row[csf('prod_id')]]['detarmination_id'];?>" />
...
if($action=='show_fabric_issue_listview')
{
	...
	foreach($prollData as $row)
	{
		...
		$roll_id_cond="";
		if(count($inserted_roll)>0) $roll_id_cond=" and b.roll_id not in (".implode(",",$inserted_roll).")";
		$sql_result=sql_select("select a.entry_form,b.width_dia_type,a.company_id,b.item_description,b.prod_id,b.barcode_no,b.roll_no,b.roll_id,b.po_id,  b.batch_qnty as batch_qnty from pro_batch_create_dtls b,pro_batch_create_mst a where b.mst_id='$batch_id' $roll_id_cond and b.roll_id  in($hidden_roll_id) and a.id=b.mst_id and  a.entry_form in(0,36) and b.status_active=1 and b.is_deleted=0  ");
		$batch_roll_id=array();
		$i=1;
		...
		$sql_result=sql_select("select a.entry_form,b.width_dia_type,a.company_id,b.item_description,b.prod_id,b.barcode_no,b.roll_no,b.roll_id,b.po_id,  b.batch_qnty as batch_qnty from pro_batch_create_dtls b,pro_batch_create_mst a where b.mst_id='$batch_id' $roll_id_cond and a.id=b.mst_id and  a.entry_form in(0,36) and b.status_active=1 and b.is_deleted=0  ");
		}
		foreach($sql_result as $row)
		{ 
			if($row[csf('entry_form')]==36)
...
if($action=="save_update_delete")
{
	...
	//	$batch_id_search=sql_select("select a.batch_id from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$txt_batch_ID and company_id=$cbo_company_id and entry_form=30");
		...
		$field_array="id,company_id,service_source,service_company,received_chalan,issue_chalan,issue_challan_mst_id ,batch_no,re_stenter_no,batch_id,batch_ext_no,
        ...
        shift_name,chemical_name,next_process_id,advanced_prod_qty,is_re_dyeing,remarks,booking_no,temparature,steam,over_feed,speed_min,inserted_by,insert_date";
		$id=return_next_id( "id", " pro_fab_subprocess", 1 ) ;
		$data_array="(".$id.",".$cbo_company_id.",".$cbo_service_source.",".$cbo_service_company.",".$txt_recevied_chalan.",".$txt_issue_chalan.",".$txt_issue_mst_id.",".$txt_batch_no.",".$txt_reslitting_no.",".$txt_batch_ID.",".$txt_ext_id.",".$cbo_sub_process.",".$txt_process_start_date.",".$txt_process_end_date.",".$txt_process_date.",".$txt_start_hours.",".$txt_start_minutes.",".$txt_end_hours.",".$txt_end_minutes.",".$cbo_machine_name.",".$cbo_result_name.",".$cbo_floor.",30,".$cbo_shift_name.",".$txt_chemical.",".$cbo_next_process.",".$txt_advance_prod.",".$re_checkbox.",".$txt_remarks.",".$txt_booking_no.",".$txt_temparature.",".$txt_steam.",".$txt_feed.",".$txt_speed.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		if(($page_upto_id==3 || $page_upto_id>3) && str_replace("'","",$roll_maintained)==1)
		{
			...
			for($i=1;$i<=$total_row;$i++)
			{
				...
				$data_array_dtls.="(".$id_dtls.",".$mst_update_id.",30,".$Itemprod_id.",".$$txtconscomp.",".$$txtgsm.",".$$txtbodypart.",".$$txtdiawidthID.",".$$txtbatchqnty.",".$$txtroll.",".$$txtbarcode.",".$$rollid.",".$$txtproductionqty.",".$$txtrate.",".$$txtamount.",".$hidden_currency.",".$hidden_exchange_rate.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				$id_dtls=$id_dtls+1;
				}
		...
		else
		{
			...
			for($i=1;$i<=$total_row;$i++)
			{
				...
				if(str_replace("'","",$$checked_tr)==1)
				{
					...
					$Itemprod_id=str_replace("'","",$$prod_id);
					if($data_array_dtls!="") $data_array_dtls.=","; 
					$data_array_dtls.="(".$id_dtls.",".$mst_update_id.",30,".$Itemprod_id.",".$$txtconscomp.",".$$txtgsm.",".$$txtbodypart.",".$$txtdiawidthID.",".$$txtbatchqnty.",".$$txtroll.",".$$txtproductionqty.",".$$txtrate.",".$$txtamount.",".$hidden_currency.",".$hidden_exchange_rate.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
					$id_dtls=$id_dtls+1;
					//print_r($data_array_dtls);die;
			   }
			}
	...
	{ 
		...
		$field_array_update="company_id*service_source*service_company*received_chalan*issue_chalan*issue_challan_mst_id *batch_no*re_stenter_no*batch_id*batch_ext_no*process_id*process_end_date*production_date*process_start_date*start_hours*start_minutes*end_hours*end_minutes*machine_id*result*floor_id*entry_form*shift_name*chemical_name*next_process_id*advanced_prod_qty*is_re_dyeing*remarks*booking_no*temparature*steam*over_feed*speed_min*updated_by*update_date";
		$data_array_update="".$cbo_company_id."*".$cbo_service_source."*".$cbo_service_company."*".$txt_recevied_chalan."*".$txt_issue_chalan."*".$txt_issue_mst_id."*".$txt_batch_no."*".$txt_reslitting_no."*".$txt_batch_ID."*".$txt_ext_id."*".$cbo_sub_process."*".$txt_process_end_date."*".$txt_process_date."*".$txt_process_start_date."*".$txt_start_hours."*".$txt_start_minutes."*".$txt_end_hours."*".$txt_end_minutes."*".$cbo_machine_name."*".$cbo_result_name."*".$cbo_floor."*30*".$cbo_shift_name."*".$txt_chemical."*".$cbo_next_process."*".$txt_advance_prod."*".$re_checkbox."*".$txt_remarks."*".$txt_booking_no."*".$txt_temparature."*".$txt_steam."*".$txt_feed."*".$txt_speed."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
...
if($action=="show_dtls_list_view")
{
	...
	$color_name_arr=return_library_array( "select id, color_name from lib_color where status_active=1 and color_name is not null",'id','color_name');
	if($db_type==0)
	{
		
		// group_concat(roll_id)  as roll_id 
	 	$sql = "SELECT a.id,group_concat(b.roll_no) AS roll_no,group_concat(b.const_composition) AS const_composition,group_concat(b.gsm) as gsm ,group_concat(b.dia_width) as dia_width,group_concat(b.width_dia_type) as width_dia_type,sum(b.batch_qty) as batch_qty,sum(b.production_qty) as production_qty,sum(b.no_of_roll) as no_of_roll from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$ex_data[0] and entry_form=30 group by a.id";
	}
	else
	{
		$sql = "SELECT a.id,a.batch_id,a.re_stenter_no,b.roll_no AS roll_no,b.const_composition AS const_composition, b.gsm as gsm ,b.dia_width as dia_width,b.width_dia_type as width_dia_type, b.roll_id as roll_id,(b.batch_qty) as batch_qty,(b.production_qty) as production_qty,(b.no_of_roll) as no_of_roll from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$ex_data[0] and entry_form=30 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0  order by b.roll_no";	
	...
	foreach($result as $key=>$val)
	{
		...
		$dtls_arr[$val[csf('batch_id')]][$val[csf('id')]]['re_stenter_no'] += $val[csf('re_stenter_no')];
		// echo $i++;
	}
	// echo "<pre>";
	...
	?> 
        ...
        <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" width="850" rules="all">
        	<thead>
            	<tr>
                	<th>SL</th>
            ...
            <tbody>
            	...
            	foreach($dtls_arr as $batch_id=>$id_row){  
					...
					$const_composition=chop($row['const_composition'],',');
					$const_composition=implode(",",array_unique(explode(",",$const_composition)));
					$dia_width=chop($row['dia_width'],',');
					$dia_width=implode(",",array_unique(explode(",",$dia_width)));

					?>
                	<tr bgcolor="<? echo $bgcolor; ?>" onClick='show_list_view("<? echo $row["id"]."_".$ex_data[1];?>","child_form_input_data","list_fabric_desc_container","requires/slitting_squeezing_controller");get_php_form_data("<? echo $ex_data[0]."_".$ex_data[2];?>", "populate_data_from_batch", "requires/slitting_squeezing_controller" );get_php_form_data("<? echo $row["id"];?>","mst_id_child_form_input_data","requires/slitting_squeezing_controller")'  style="cursor:pointer" >
                        <td width="30"><? echo $i; ?></td>
                        <td width="80"><p><? echo $const_composition; ?></p></td>
                        <td width="80"><p><? echo chop($gsm,','); ?></p></td>
                        <td width="70"><p><? echo chop($dia_width,','); ?></p></td>
...
if($action=="child_form_input_data")
{
	...
	$bat_pro=sql_select("select a.id as batch_id,b.batch_qnty,b.po_id,b.prod_id from pro_batch_create_mst a,pro_batch_create_dtls b,pro_fab_subprocess c where  a.id=b.mst_id  and c.batch_id=a.id and c.id=$ex_data[0] and a.is_deleted=0 and a.status_active=1  and b.batch_qnty>0");
 //echo "select a.id as batch_id,b.batch_qnty,b.po_id,b.prod_id from pro_batch_create_mst a,pro_batch_create_dtls b where  a.id=b.mst_id and a.id=".$batch_id." and a.is_deleted=0 and a.status_active=1  and b.batch_qnty>0";
	...
	//{
		...
		foreach($sql_result as $row)
		{
            ...
            <td><input type="text" name="txtlot_<? echo $i; ?>" id="txtlot_<? echo $i; ?>" class="text_boxes_numeric" style="width:55px;"  value="<? echo $yarn_lot_arr[$prod_id][$po_id]['lot'];?>" disabled /></td>
            <td><input type="text" name="txtyarncount_<? echo $i; ?>" id="txtyarncount_<? echo $i; ?>" class="text_boxes_numeric" style="width:50px;"  value="<? echo $yarn_count_value;?>"  disabled /></td>
            <td><input type="text" name="txtbrand_<? echo $i; ?>" id="txtbrand_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px;"  value="<? echo $brand_value;?>"  disabled/></td>
...
if($action=="mst_id_child_form_input_data")
{
   ...
   from pro_fab_subprocess a where a.id=$data and a.entry_form=30 and a.status_active=1 and a.is_deleted=0");
 ...
 $is_re_dyeing=$sql_result[0][csf("is_re_dyeing")];
   ...
   $sql_re_stent=sql_select("select max(re_stenter_no) as re_stenter_no_max,min(re_stenter_no) as re_stenter_no_min from pro_fab_subprocess where entry_form=30 and batch_no='".$sql_result[0][csf('batch_no')]."' and status_active = 1 and is_deleted = 0");
   ...
   // $re_slitting_no_min=$sql_re_stent[0][csf('re_stenter_no_min')];
	...
	echo "load_drop_down( 'requires/slitting_squeezing_controller',".$sql_result[0][csf("service_source")]."+'**'+ document.getElementById('cbo_company_id').value, 'load_drop_down_knitting_com_new', 'dyeing_company_td' );\n";
...
//	echo "$('#txt_reslitting_no').attr('readonly','readonly');\n";
	...
	{
	echo "load_drop_down('requires/slitting_squeezing_controller', '".$sql_result[0][csf('service_company')]."', 'load_drop_floor', 'floor_td' );\n";
	}
	echo "load_drop_down( 'requires/slitting_squeezing_controller', document.getElementById('cbo_service_company').value+'**'+".$sql_result[0][csf("floor_id")].", 'load_drop_machine', 'machine_td' );\n";
	        ...
	        <table  cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all" width="900">
	            ...
	            <tbody>
	                <tr>
	                    ...
	                    <? 
	                   		echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 and id=".$cbo_company_id." order by company_name","id,company_name",1, "-- Select Company --", '', "load_drop_down( 'slitting_squeezing_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
	                    ?>
	                    </td>
	                    ...
	                    <td align="center">
	                    	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_search_category').value+'_'+document.getElementById('txt_booking_no').value+'_'+'<?php echo $supplier_id."_".$process_id; ?>', 'create_booking_search_list_view', 'search_div', 'slitting_squeezing_controller', 'setFilterGrid(\'table_body\',-1)')" style="width:100px;" />
	                    </td>
	                </tr>
	                <tr>
