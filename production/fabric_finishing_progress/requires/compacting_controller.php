...
if($action=="load_drop_down_knitting_com_new")
{
	...
	{	
		...
		echo create_drop_down( "cbo_service_company", 135, "$sql","id,supplier_name", 1, "-- Select --", 0, "load_drop_down('requires/compacting_controller', this.value, 'load_drop_floor', 'floor_td' );" );
	}
	else
	{
		echo create_drop_down( "cbo_service_company", 135, $blank_array,"",1, "-- Select --", 0, "" );
...
{	
	...
	$sql_result=sql_select("SELECT a.entry_form,b.width_dia_type,b.gsm,b.fin_dia,b.item_description, sum(b.batch_qnty) as batch_qty,b.prod_id,b.roll_id,b.barcode_no,b.roll_no 
	from pro_batch_create_dtls b,pro_batch_create_mst a  
	where b.mst_id='$batch_id'   and a.id=b.mst_id and a.id='$batch_id' and  a.entry_form in(0,36) and b.status_active=1 and b.is_deleted=0  
	group by b.id,b.item_description,b.width_dia_type,b.gsm,b.fin_dia,b.prod_id,a.entry_form,b.roll_id,b.barcode_no,b.roll_no order by b.item_description");
	$i=1;
	$b_qty=0;
	//if($roll_maintained==1)
	...
	else // With Roll End
	{
		...
		//echo "SELECT a.id as batch_id,a.entry_form,b.width_dia_type,b.gsm,b.fin_dia,b.item_description, sum(b.batch_qnty) as batch_qnty,b.prod_id,count(b.roll_no) as  roll_no from pro_batch_create_dtls b,pro_batch_create_mst a where b.mst_id=$batch_id and a.id=b.mst_id and  a.entry_form in(0,36) and b.status_active=1 and b.is_deleted=0 group by a.id,a.entry_form,b.width_dia_type,b.gsm,b.fin_dia,b.item_description,b.prod_id  order by b.item_description ";
...
if($action=="show_dtls_list_view")
{
	...
	if($db_type==2)
	{
		...
		group by  a.re_stenter_no,a.previous_process, a.batch_id,a.id,b.const_composition,b.gsm,b.roll_no,b.dia_width,b.width_dia_type,b.roll_id, b.prod_id, b.barcode_no";
	}
	else
	{
		...
		from pro_fab_subprocess a,pro_fab_subprocess_dtls b 
		where a.id=b.mst_id and a.batch_id=$batch_id and a.entry_form=33 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $re_stener_cond  group by  a.re_stenter_no,a.previous_process,a.id";	
	}
	// echo $sql;
	...
	foreach($result as $key=>$val)
	{
		...
		if ($checkBatch2[$val[csf('batch_id')]][$val[csf("barcode_no")]][$val[csf("prod_id")]][$val[csf("gsm")]][$val[csf("dia_width")]][$val[csf("width_dia_type")]]=="") 
		{
			$dtls_arr[$val[csf('id')]][$val[csf('batch_id')]]['tot_batch_qty'] += $val[csf('batch_qty')];
			$checkBatch2[$val[csf('batch_id')]][$val[csf("barcode_no")]][$val[csf("prod_id")]][$val[csf("gsm")]][$val[csf("dia_width")]][$val[csf("width_dia_type")]]=[$val[csf('batch_id')]].'='.$val[csf("barcode_no")].'='.$val[csf("prod_id")].'='.$val[csf("gsm")].'='.$val[csf("dia_width")].'='.$val[csf("width_dia_type")];
		}
		// echo $i++;
	}
	// echo "<pre>";
...
{
	$sql_result=sql_select("SELECT a.previous_process, a.id,a.batch_no,a.batch_id,a.re_stenter_no,a.company_id,a.next_process_id,a.advanced_prod_qty,a.is_re_dyeing, a.service_source, a.service_company, a.length_shrinkage, a.width_shrinkage, a.spirality, a.received_chalan, a.issue_chalan, a.issue_challan_mst_id, a.process_end_date, a.production_date, a.process_start_date, a.process_id, a.end_hours, a.end_minutes, a.start_hours, a.start_minutes,a.result, a.temparature,a.steam, a.stretch, a.over_feed, a.feed_in, a.pinning, a.speed_min, a.floor_id, a.machine_id, a.shift_name, a.remarks 
	...
	echo "document.getElementById('txt_restenter_no').value	= '".$re_stenter_from."';\n";
	...
	echo "load_drop_down( 'requires/compacting_controller', ".$sql_result[0][csf("service_source")]."+'**'+".$sql_result[0][csf("company_id")].", 'load_drop_down_knitting_com_new', 'dyeing_company_td' );\n";
