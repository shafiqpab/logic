...
if ($action=="load_drop_floor")
{
	...
	echo create_drop_down( "cbo_floor", 135, "select id,floor_name from lib_prod_floor where status_active=1 and is_deleted=0 and company_id='$data[0]' and production_process=4  order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected,"load_drop_down( 'requires/special_finish_controller', document.getElementById('cbo_service_company').value+'**'+this.value, 'load_drop_machine', 'machine_td' );" );     	 
	exit();
}
...
if($action=="load_drop_down_knitting_com")
{
	...
	if($data[0]==1)
	{
		...
		echo create_drop_down( "cbo_service_company", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name",1, "-- Select --", 0, "load_drop_down('requires/special_finish_controller', this.value, 'load_drop_floor', 'floor_td' );","" );
	}
	else if($data[0]==3)
	{	
...
{ 
	...
	$ex_data=explode('**',$data);
	//print_r($ex_data);
	$sql_res="select id, floor_id, machine_group from lib_machine_name where id=$ex_data[1] and category_id=4 and company_id=$ex_data[0] and status_active=1 and is_deleted=0 ";
	$nameArray=sql_select($sql_res);
	foreach ($nameArray as $row)
	{	
	echo "document.getElementById('txt_machine_no').value 			= '".$floor_arr[$row[csf("floor_id")]]."';\n";
...
{	//and company_id='".trim($data[0])."'
	//$data=explode("**",$data);
	$sql="select id, batch_id,batch_no from pro_fab_subprocess where batch_no='".trim($data)."' and  entry_form in(35,38) and load_unload_id in(2) and is_deleted=0 and status_active=1";
...
if($action=="create_batch_search_list_view")
{
	...
	$search_type =$data[5];
	 ...
	 if($search_type==1)
	{
		if ($batch_no!='') { $batch_cond=" and batch_no='$batch_no'";$batch_cond2=" and b.batch_no='$batch_no'";} else { $batch_cond="";$batch_cond2="";}
	}
	else if($search_type==4 || $search_type==0)
	{
		if ($batch_no!='') { $batch_cond=" and batch_no like '%$batch_no%'";$batch_cond2=" and b.batch_no like '%$batch_no%'";} else { $batch_cond="";$batch_cond2="";}
	}
	...
	if($db_type==2)
		...
		{ 
			...
			$batch_date_con2 = " and b.batch_date between '".change_date_format($start_date, "mm-dd-yyyy", "-",1)."' and '".change_date_format($end_date, "mm-dd-yyyy", "-",1)."'";
		}
		...
		if($batch_type==0 || $batch_type==2)
			...
			$sql_po=sql_select("SELECT b.mst_id, a.job_no_mst,listagg(cast(a.order_no as varchar2(4000)),',') within group (order by a.order_no) as po_no from  subcon_ord_dtls a, pro_batch_create_dtls c,pro_batch_create_mst b where a.id=c.po_id and b.id=c.mst_id and  b.status_active=1 and b.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 $batch_date_con2  $batch_cond2 group by c.mst_id,a.job_no_mst");
			}
			else
			{
			$sql_po=sql_select("SELECT b.mst_id, a.job_no_mst,listagg(cast(a.po_number as varchar2(4000)),',') within group (order by a.po_number) as po_no from wo_po_break_down a, pro_batch_create_dtls c,pro_batch_create_mst b  where a.id=c.po_id and b.id=c.mst_id and  b.status_active=1 and b.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 $batch_date_con2  $batch_cond2 group by c.mst_id,a.job_no_mst");
			}
		}
	...
	$sql = "SELECT working_company_id,id, batch_no, batch_date, batch_weight, booking_no, extention_no, color_id, batch_against, re_dyeing_from,is_sales from pro_batch_create_mst where batch_for in(0,1) and batch_against<>4 and status_active=1 and is_deleted=0 $search_field_cond_batch $batch_date_con $batch_cond order by id desc"; 
	//echo $sql;//die;
	?>
    <div>
        ...
        <div style="width:770px; overflow-y:scroll; max-height:230px;" id="buyer_list_view" align="center">
            ...
            <?
				...
				$sql_sales_job2=sql_select("SELECT  sales_booking_no as  booking_no, job_no as sales_order_no  from   FABRIC_SALES_ORDER_MST   where status_active=1 and within_group=2 $SalesId_cond2 group by sales_booking_no , job_no ");
				...
				$sql_prod = "select b.id, b.batch_no, a.result from pro_batch_create_mst b,pro_fab_subprocess a  where b.id=a.batch_id and a.entry_form in(35,38) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and a.load_unload_id=2  $batch_date_con2  $batch_cond2  $batchId_cond3 order by b.id desc"; 
	$nameArray_prod=sql_select( $sql_prod );
				foreach ($nameArray_prod as $row)
				{
					 $unload_batch_dyeing_chk_arr[$row[csf('id')]]=$row[csf('id')];
				}
				foreach ($nameArray as $selectResult)
				{
					...
					$po_no = '';
				if($unload_batch_dyeing_chk_arr[$selectResult[csf('id')]]>0) //Unloaded check
			    {
					if ($selectResult[csf('re_dyeing_from')] == 0 || 1==1) {
						...
						?>
						<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $selectResult[csf('id')].'_'.$is_sales.'_'.$selectResult[csf('working_company_id')]; ?>')"> 
					...
					/*else
					{
							   ...
							   $sql_re= "SELECT working_company_id,is_sales,id, batch_no, batch_date, batch_weight, booking_no,MAX(extention_no) as extention_no, color_id, batch_against, re_dyeing_from from pro_batch_create_mst where  batch_for in(0,1) and entry_form in(0,36) and batch_against<>4 and status_active=1 and is_deleted=0 and  id=".$selectResult[csf('id')]." group by working_company_id,is_sales,id, batch_no, batch_date, batch_weight, booking_no , color_id, batch_against, re_dyeing_from ";
								...
								foreach($dataArray as $row)
								{
									if($row[csf('re_dyeing_from')]==0 || 1==1)
									{
										...
										?>
										<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $row[csf('id')].'_'.$row[csf('is_sales')].'_'.$row[csf('working_company_id')]; ?>')"> 
											<td width="40" align="center"><? echo $i; ?></td>	
...
{ 
	...
	$sql_sales_job2=sql_select("SELECT  sales_booking_no as  booking_no, job_no as sales_order_no ,buyer_id,within_group from   FABRIC_SALES_ORDER_MST   where status_active=1   group by sales_booking_no , job_no,buyer_id ,within_group");

	foreach ($sql_sales_job2 as $sales_job_row) {
		 
		$sales_job_arr2[$sales_job_row[csf('booking_no')]]["sales_order_no"] = $sales_job_row[csf('sales_order_no')];
		$sales_job_arr2[$sales_job_row[csf('booking_no')]]["buyer_id"] = $sales_job_row[csf('buyer_id')];
		$sales_job_arr2[$sales_job_row[csf('booking_no')]]["within_group"] = $sales_job_row[csf('within_group')];
	...
	else if($db_type==2) $pop_batch=" group by  a.working_company_id,a.id,a.batch_no, a.batch_weight,batch_date,a.color_id,a.color_range_id,a.insert_date, a.company_id,a.process_id,a.booking_without_order,a.entry_form,a.booking_no,a.total_trims_weight order by a.id";
	...
	if($batch_no!='')
		{ 
			$data_array=sql_select("SELECT a.working_company_id, MAX(a.id) as id,a.batch_no,a.entry_form,a.batch_date,a.company_id,a.total_trims_weight, a.batch_weight,MAX(a.extention_no) as extention_no,a.process_id as process_id_batch, a.color_id,a.color_range_id,a.insert_date, a.booking_without_order,a.booking_no, sum(b.batch_qnty) as batch_qnty, $select_list from pro_batch_create_mst a,pro_batch_create_dtls b where a.id='$batch_id' and a.entry_form in(0,36)  and a.id=b.mst_id $select_group_row");
		}
		else
		{
		$data_array=sql_select("SELECT a.working_company_id, a.id,a.batch_no,a.entry_form,a.total_trims_weight, a.batch_weight,Max(a.extention_no) as extention_no,a.batch_date,a.company_id,a.process_id as process_id_batch,  a.color_id,a.color_range_id,a.insert_date, a.booking_without_order,a.booking_no, sum(b.batch_qnty) as batch_qnty, $select_list from pro_batch_create_mst a,pro_batch_create_dtls b where a.id='$batch_id'   and a.entry_form in(0,36)  and a.id=b.mst_id $pop_batch");	
		}	
	...
	{ 
		...
		echo "document.getElementById('cbo_service_source').value 				= '1';\n";
		if($row[csf("entry_form")]==36)
		{
		echo "document.getElementById('cbo_service_company').value 				= '".$row[csf("company_id")]."';\n";
		echo "load_drop_down( 'requires/special_finish_controller', '".$row[csf("company_id")]."', 'load_drop_floor', 'floor_td' );\n";
		}
		else
		{
		echo "document.getElementById('cbo_service_company').value 				= '".$row[csf("working_company_id")]."';\n";
		echo "load_drop_down( 'requires/special_finish_controller', '".$row[csf("working_company_id")]."', 'load_drop_floor', 'floor_td' );\n";
		}
		echo "document.getElementById('cbo_company_id').value 				= '".$row[csf("company_id")]."';\n";
		...
		$within_group=$sales_job_arr2[$row[csf('booking_no')]]["within_group"];
		if ($is_sales == 1) {
			if($within_group == 1){
		...
		$sql_batch_d=sql_select("select id,batch_id,batch_no,process_end_date,end_hours,end_minutes,machine_id,floor_id,process_id,production_date,remarks from pro_fab_subprocess where batch_id='$batch_id' and entry_form=35 and load_unload_id=2");
	...
	//	echo "document.getElementById('txt_process_id').value 			= '".$row[csf("process_id_batch")]."';\n";
		...
		$sql_batch=sql_select("select id,batch_id,batch_no,process_end_date,
	process_start_date,start_hours,start_minutes,end_hours,end_minutes,machine_id,floor_id,process_id,production_date,shift_name,remarks from pro_fab_subprocess where entry_form=34 and batch_id='".$row[csf("id")]."' ");
		...
		foreach($sql_batch as $r_batch)
		{
			...
			echo "document.getElementById('txt_process_id').value 				= '';\n";
			echo "document.getElementById('txt_process_name').value 				= '';\n";
			echo "$('#txt_process_name').attr('disabled',false);\n";	
			...
			echo "load_drop_down( 'requires/special_finish_controller', document.getElementById('cbo_company_id').value+'**'+".$r_batch[csf("floor_id")].", 'load_drop_machine', 'machine_td' );\n";
...
{	
	$ex_data=explode('_',$data);
	$batch_id=$ex_data[0];
	$process_id=$ex_data[2];
	$extention_no=$ex_data[3];
	if($extention_no) $extention_no_cond=" and batch_ext_no='$extention_no'" ;
	...
	if($db_type==0)
	{
		$grp_con="group_concat(po_id) as po_id";
	}
	else
	{
		$grp_con="listagg((cast(po_id as varchar2(4000))),',') within group (order by po_id) as po_id";
	}
	$po_ids=return_field_value("$grp_con","pro_batch_create_dtls","mst_id ='$batch_id' and is_deleted=0 and status_active=1","po_id");
	if($po_ids!="") $po_ids_con="and po_breakdown_id in($po_ids)";else $po_ids_con="";
	if(($page_upto_id==6 || $page_upto_id>6) && $roll_maintained==1)
	{
		$rollData=sql_select("select id,roll_no,barcode_no from pro_roll_details where  status_active=1 and is_deleted=0 $po_ids_con");
	...
	$sql_result=sql_select("SELECT a.entry_form,b.width_dia_type,b.gsm,b.fin_dia,b.item_description, sum(b.batch_qnty) as batch_qty,b.prod_id,b.barcode_no,b.roll_id,b.roll_no from pro_batch_create_dtls b,pro_batch_create_mst a  where b.mst_id='$batch_id'   and a.id=b.mst_id and a.id='$batch_id' and  a.entry_form in(0,36) and b.status_active=1 and b.is_deleted=0    group by b.item_description,b.width_dia_type,b.prod_id,a.entry_form,b.roll_id,b.gsm,b.fin_dia,b.barcode_no,b.roll_no order by b.item_description");
	$i=1;
	$b_qty=0;
	//if($roll_maintained==1)
	if(($page_upto_id==6 || $page_upto_id>6) && $roll_maintained==1)
	{
		if(count($sql_result)>0)
		{
			...
			foreach($sql_result as $row)
			{
		    	...
		    	<tr class="general" id="row_<? echo $i; ?>">
		            ...
		            <td><input type="text" name="txtgsm_<? echo $i; ?>" id="txtgsm_<? echo $i; ?>" class="text_boxes" style="width:40px;" value="<? echo $gsm; ?>" /></td>
		            <td><input type="text" name="txtbodypart_<? echo $i; ?>" id="txtbodypart_<? echo $i; ?>" class="text_boxes" style="width:50px;" value="<? echo  $dia_width; ?>" disabled/></td> 
		            <td><input type="text" name="txtdiawidth_<? echo $i; ?>" id="txtdiawidth_<? echo $i; ?>" class="text_boxes" style="width:70px;" value="<? echo $fabric_typee[$row[csf('width_dia_type')]];?>" disabled/>
		             ...
		             <td><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo $row[csf('roll_no')];//$roll_no;//$row[csf('roll_no')];?>" readonly /> <input type="hidden" name="rollid_<? echo $i; ?>" id="rollid_<? echo $i; ?>" style="width:50px;" value="<? echo $row[csf('roll_id')];?>" class="text_boxes_numeric" /></td>
		    	...
		    	<?
		    	/*if(!in_array($row[csf('roll_no')], $checkBatchRoll))
		    	{
					$b_qty+= $row[csf('batch_qty')];
					$checkBatchRoll[]=$row[csf('roll_no')];
		    	}*/
				$b_qty+= $row[csf('batch_qty')];
		...
		else
		{
			...
			<tr class="general" id="row_<? echo $i; ?>">
	            ...
	            <td><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes_numeric" value="<? //echo $row[csf('roll_no')];?>" style="width:60px;"/ readonly>
		...
		?>
	     <tr>
		    <td colspan="6" align="right"><b style=" float:left">Check/Uncheck<input type="checkbox" style="width:30px" id="allcheckbox" name="allcheckbox[]"  onClick="checkbox_all(this.id);" checked  />&nbsp;&nbsp;&nbsp;OpenWith Check/Uncheck<input type="checkbox" style="width:30px" id="allcheckbox2" name="allcheckbox2[]"  onClick="checkbox_all2(2);"   />&nbsp;&nbsp;&nbsp;Tubular Check/Uncheck<input type="checkbox" style="width:30px" id="allcheckbox3" name="allcheckbox3[]"  onClick="checkbox_all3(3);"   /></b><b>Sum:</b> <? //echo $b_qty; ?> </td>
	        ...
	        <td align="right"><input type="text" name="total_production_qnty" id="total_production_qnty" class="text_boxes_numeric" style="width:60px" value="<? echo number_format($b_qty,2,'.',''); ?>"  readonly /></td>
	        <td align="right"></td>           
	...
	{ 
			...
			$result=sql_select("SELECT a.entry_form,b.width_dia_type,b.gsm,b.fin_dia,b.item_description, sum(b.batch_qnty) as batch_qty,b.prod_id,count(b.roll_no ) as roll_no from pro_batch_create_dtls b,pro_batch_create_mst a  where b.mst_id='$batch_id'  and a.id=b.mst_id and  a.entry_form in(0,36) and b.status_active=1 and b.is_deleted=0 group by a.entry_form,b.width_dia_type,b.item_description,b.gsm,b.fin_dia,b.prod_id  order by b.item_description");
			$b_qty=0;
			foreach($result as $row)
			{
				...
				?>
		    	<tr class="general" id="row_<? echo $i; ?>">
		         	<td width="60" id="sl_<? echo $i;?>"><? echo $i; ?>&nbsp; &nbsp;<input type="checkbox" id="checkRow_<? echo $i; ?>" name="checkRow[]"  checked /> </td>
		            <td><input type="text" name="txtconscomp_<? echo $i; ?>" id="txtconscomp_<? echo $i; ?>" class="text_boxes" style="width:200px;" value="<? echo $cons_comps; ?>" disabled/></td>
		            <td><input type="text" name="txtgsm_<? echo $i; ?>" id="txtgsm_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px;" value="<? echo $gsm; ?>" /></td>
		            <td><input type="text" name="txtbodypart_<? echo $i; ?>" id="txtbodypart_<? echo $i; ?>" class="text_boxes" style="width:50px;" value="<? echo  $dia_width; //$row[csf('width_dia_type')]; ?>" disabled/></td>
		            <td><input type="text" name="txtdiawidth_<? echo $i; ?>" id="txtdiawidth_<? echo $i; ?>" class="text_boxes" style="width:70px;" value="<? echo $fabric_typee[$row[csf('width_dia_type')]];?>" disabled/>
		            ...
		            <td><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo $row[csf('roll_no')];?>"/></td>
		            <td><input type="text" name="txtbatchqnty_<? echo $i; ?>" id="txtbatchqnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo $row[csf('batch_qty')]; ?>" disabled/>
		            <input type="hidden" name="txtdeterid_<? echo $i; ?>" id="txtdeterid_<? echo $i; ?>"  value="<? echo $fabric_desc_arr[$row[csf('prod_id')]]['detarmination_id'];?>" />
		             <input type="hidden" name="txtprodid_<? echo $i; ?>" id="txtprodid_<? echo $i; ?>"  value="<? echo $row[csf('prod_id')];?>" />
					<input type="hidden" name="updateiddtls_<? echo $i; ?>" id="updateiddtls_<? echo $i; ?>" class="text_boxes" value="<? //echo $row[csf('id')];?>" readonly />
	 	...
	 	?>
	  	<tr>
	        ...
	        <td><input type="text" name="total_production_qnty" id="total_production_qnty" value="<? echo number_format($pr_qty,2,'.',''); ?>" class="text_boxes_numeric" style="width:60px" readonly /></td>
	        <td align="right"></td>           
...
if($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		{
		$field_array="id,company_id,re_stenter_no,batch_no,batch_id,batch_ext_no,process_id,process_end_date,production_date,process_start_date,start_hours,start_minutes,end_hours,end_minutes,machine_id,result,floor_id,entry_form,shift_name,remarks,issue_chalan,service_source,service_company,received_chalan,issue_challan_mst_id,booking_no,temparature,steam,over_feed,next_process_id,advanced_prod_qty,is_re_dyeing,speed_min,inserted_by,insert_date";
		$id=return_next_id( "id", " pro_fab_subprocess", 1 ) ;
		$data_array="(".$id.",".$cbo_company_id.",".$txt_special_fin.",".$txt_batch_no.",".$txt_batch_ID.",".$txt_ext_id.",".$txt_process_id.",".$txt_process_end_date.",".$txt_process_date.",".$txt_process_start_date.",".$txt_start_hours.",".$txt_start_minutes.",".$txt_end_hours.",".$txt_end_minutes.",".$cbo_machine_name.",".$cbo_result_name.",".$cbo_floor.",34,".$cbo_shift_name.",".$txt_remarks.",".$txt_issue_chalan.",".$cbo_service_source.",".$cbo_service_company.",".$txt_recevied_chalan.",".$txt_issue_mst_id.",".$txt_booking_no.",".$txt_temparature.",".$txt_steam.",".$txt_feed.",".$cbo_next_process.",".$txt_advance_prod.",".$re_checkbox.",".$txt_speed.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		//print_r($data_array);die;
		//echo "insert into pro_fab_subprocess values $field_array,$data_array,'id',$id,0)";
		...
		if(($page_upto_id==6 || $page_upto_id>6) && str_replace("'","",$roll_maintained)==1)
		{
		...
		if(($page_upto_id==6 || $page_upto_id>6) && str_replace("'","",$roll_maintained)==1)
		{
			...
			for($i=1;$i<=$total_row;$i++)
			{
				...
				if(str_replace("'","",$$checked_tr)==1)
				{
					...
					$data_array_dtls.="(".$id_dtls.",".$mst_update_id.",34,".$Itemprod_id.",".$$txtconscomp.",".$$txtgsm.",".$$txtbodypart.",".$$txtdiawidthID.",".$$txtbatchqnty.",".$$txtroll.",".$$txtbarcode.",".$$rollid.",".$$txtproductionqty.",".$$txtrate.",".$$txtamount.",".$hidden_currency.",".$hidden_exchange_rate.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
					$id_dtls=$id_dtls+1;
		...
		{
			//$field_array_dtls="id,mst_id,entry_page,prod_id,const_composition,gsm,dia_width,width_dia_type,batch_qty,roll_no,production_qty,inserted_by,insert_date";
			//echo $total_row;die;
			for($i=1;$i<=$total_row;$i++)
			{
				$checked_tr="checkRow_".$i;
				if(str_replace("'","",$$checked_tr)==1)
				{
					...
					$Itemprod_id=str_replace("'","",$$prod_id);
					if($data_array_dtls!="") $data_array_dtls.=","; 
					$data_array_dtls.="(".$id_dtls.",".$mst_update_id.",34,".$Itemprod_id.",".$$txtconscomp.",".$$txtgsm.",".$$txtbodypart.",".$$txtdiawidthID.",".$$txtbatchqnty.",".$$txtroll.",".$$txtproductionqty.",".$$txtrate.",".$$txtamount.",".$hidden_currency.",".$hidden_exchange_rate.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
					$id_dtls=$id_dtls+1;
					//print_r($data_array_dtls);die;
				}
			}	
	...
	{ 
		...
		$field_array_update="company_id*batch_no*service_source*service_company*re_stenter_no*received_chalan*issue_chalan*issue_challan_mst_id*batch_id*batch_ext_no* process_id*process_end_date*production_date*process_start_date*start_hours*start_minutes*end_hours*end_minutes*machine_id*result*floor_id*entry_form*shift_name*remarks*booking_no*temparature*steam*over_feed*next_process_id*advanced_prod_qty*is_re_dyeing*speed_min*updated_by*update_date";
		$data_array_update="".$cbo_company_id."*".$txt_batch_no."*".$cbo_service_source."*".$cbo_service_company."*".$txt_special_fin."*".$txt_recevied_chalan."*".$txt_issue_chalan."*".$txt_issue_mst_id."*".$txt_batch_ID."*".$txt_ext_id."*".$txt_process_id."*".$txt_process_end_date."*".$txt_process_date."*".$txt_process_start_date."*".$txt_start_hours."*".$txt_start_minutes."*".$txt_end_hours."*".$txt_end_minutes."*".$cbo_machine_name."*".$cbo_result_name."*".$cbo_floor."*34*".$cbo_shift_name."*".$txt_remarks."*".$txt_booking_no."*".$txt_temparature."*".$txt_steam."*".$txt_feed."*".$cbo_next_process."*".$txt_advance_prod."*".$re_checkbox."*".$txt_speed."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		// print_r($data_array_delete);die;
		// echo "10**".bulk_update_sql_statement("pro_fab_subprocess_dtls", "id",$field_array_up,$data_array_up,$id_arr );die();
		// echo "10**".bulk_update_sql_statement("pro_fab_subprocess_dtls", "id",$field_array_delete,$data_array_delete,$id_arr_delete );die();
...
if($action=="process_name_popup")
{
	...
	<script>
		...
		function js_set_value( str ) 
		{
			...
			if (id.indexOf(',') > -1)
			{
				alert("Please don't mixed multiple processes simultaneously");
				//toggle( document.getElementById( 'search' + str ), '#FFFFCC' );
				//return;
			}
            ...
            <div style="width:350px; overflow-y:scroll; max-height:280px;" id="buyer_list_view" align="center">
                ...
                <?
                    ...
                    foreach($conversion_cost_head_array as $id=>$name)
                    {
                    	...
                    	if($id==35 || $id==67 || $id==68 || $id==88  || $id==100 || $id==194 || $id==201 || $id==202 || $id==203 || $id==204 || $id==60 || $id==64 || $id==206 || $id==209 || $id==127 || $id==256 || $id==128 || $id==268 || $id==295 || $id==296 || $id==394 || $id==396 || $id==221 || $id==438)
                    	{
...
if($action=="populate_data_from_data")
{
	...
	{		
		...
		echo "load_drop_down( 'requires/special_finish_controller', ".$row[csf("dyeing_source")]."+'**'+".$row[csf("company_id")].", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
...
if($action=='issue_show_fabric_desc_listview')//Grey Issue Subcon
{
	...
	if(count($sql_result)>0)
	{
		...
		foreach($sql_result as $row)
		{
			...
			?>
			<tr class="general" id="row_<? echo $i; ?>">
             <td width="60" id="sl_<? echo $i; ?>"><input type="checkbox" disabled="" id="checkRow_<? echo $i; ?>" name="checkRow[]" checked="checked"  > &nbsp; &nbsp;<? echo $i; ?></td>
				<td><input type="text" name="txtconscomp_<? echo $i; ?>" id="txtconscomp_<? echo $i; ?>" class="text_boxes" style="width:200px;" value="<? echo $cons_comps; ?>" disabled/></td>
				<td><input type="text" name="txtgsm_<? echo $i; ?>" id="txtgsm_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px;" value="<? echo $gsm; ?>" /></td>
				<td><input type="text" name="txtbodypart_<? echo $i; ?>" id="txtbodypart_<? echo $i; ?>" class="text_boxes" style="width:50px;" value="<? echo  $dia_width; //$row[csf('width_dia_type')]; ?>" disabled/></td> 
				<td><input type="text" name="txtdiawidth_<? echo $i; ?>" id="txtdiawidth_<? echo $i; ?>" class="text_boxes" style="width:70px;" value="<? echo $fabric_typee[$row[csf('width_dia_type')]];?>" disabled/>
                 ...
                 <td><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes_numeric" value="<? echo $roll_no;?>"  style="width:60px;" <? echo $readonly; ?> /> <input type="hidden" name="rollid_<? echo $i; ?>" id="rollid_<? echo $i; ?>" style="width:50px;" value="<? echo $row[csf('roll_id')];?>" class="text_boxes_numeric" />	</td>
	...
	foreach($sql_result as $row)
		...
		?>
			<tr class="general" id="row_<? echo $i; ?>">
             <td width="60" id="sl_<? echo $i; ?>"><input type="checkbox" disabled="" id="checkRow_<? echo $i; ?>" name="checkRow[]"  checked /> &nbsp; &nbsp;<? echo $i; ?></td>
				<td><input type="text" name="txtconscomp_<? echo $i; ?>" id="txtconscomp_<? echo $i; ?>" class="text_boxes" style="width:200px;" value="<? echo $cons_comps; ?>" disabled/></td>
				<td><input type="text" name="txtgsm_<? echo $i; ?>" id="txtgsm_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px;" value="<? echo $gsm; ?>" /></td>
				<td><input type="text" name="txtbodypart_<? echo $i; ?>" id="txtbodypart_<? echo $i; ?>" class="text_boxes" style="width:50px;" value="<? echo  $dia_width; //$row[csf('width_dia_type')]; ?>" disabled/></td> 
				<td><input type="text" name="txtdiawidth_<? echo $i; ?>" id="txtdiawidth_<? echo $i; ?>" class="text_boxes" style="width:70px;" value="<? echo $fabric_typee[$row[csf('width_dia_type')]];?>" disabled/>
		...
		<tr class="general" id="row_<? echo $i; ?>">
             <td width="60" id="sl_<? echo $i; ?>"><input type="checkbox" disabled="" id="checkRow_<? echo $i; ?>" name="checkRow[]"  checked /> &nbsp; &nbsp;<? echo $i; ?></td>
				...
				<td><input type="text" name="txtdiawidth_<? echo $i; ?>" id="txtdiawidth_<? echo $i; ?>" class="text_boxes" style="width:70px;" value="<? //echo $fabric_typee[$row[csf('width_dia_type')]];?>" disabled/>
                 ...
                 <td><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes_numeric" value="<? //echo $roll_no;?>" style="width:60px;" readonly /> <input type="hidden" name="rollid_<? echo $i; ?>" id="rollid_<? echo $i; ?>" style="width:50px;" value="<? //echo $row[csf('roll_id')];?>" class="text_boxes_numeric" />	</td>
				<td><input type="text" name="txtbatchqnty_<? echo $i; ?>" id="txtbatchqnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? //echo number_format($row[csf('batch_qnty')],2); ?>" disabled/>
                <input type="hidden" name="txtdeterid_<? echo $i; ?>" id="txtdeterid_<? echo $i; ?>"  value="<? //echo $fabric_desc_arr[$row[csf('prod_id')]]['detarmination_id'];?>" />
				 <input type="hidden" name="txtprodid_<? echo $i; ?>" id="txtprodid_<? echo $i; ?>"  value="<? //echo $row[csf('prod_id')];?>" />
				  <input type="hidden" name="updateiddtls_<? echo $i; ?>" id="updateiddtls_<? echo $i; ?>" class="text_boxes" value="<? //echo $row[csf('id')];?>" readonly />
	...
	<? }
	 ...
	 <tr>
        ...
        <td><input type="text" name="total_production_qnty" id="total_production_qnty" class="text_boxes_numeric" style="width:60px" readonly /> </td>
        <td align="right"></td>           
        <td align="right"><input type="text" name="total_amount" id="total_amount" class="text_boxes_numeric" style="width:70px" readonly /> </td>
     </tr>
	<?
            ...
            <table cellpadding="0" cellspacing="0" width="850" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td align="center" id="search_by_td">				
                     ...
                     <td align="center">	
                    	<?
                       		 echo create_drop_down( "cbo_service_source", 135, $knitting_source, "", 1, "-- Select --", $selected, "load_drop_down( 'special_finish_controller',this.value+'**'+$cbo_company_id,'load_drop_down_knitting_com','dyeing_company_td' );","","1,3" );
						?>
                    </td> 
            		...
            		<td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $cbo_company_id; ?>+'_'+document.getElementById('cbo_service_source').value+'_'+document.getElementById('cbo_service_company').value, 'create_challan_search_list_view', 'search_div', 'special_finish_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                     </td>
                </tr>
                <tr>
...
if($action=="show_dtls_list_view")
{
	...
	$process_id = $ex_data[2];
	$process_cond="";
	if($process_id) $process_cond=" and a.process_id='$process_id'";
	...
	if($db_type==2) //$process_cond //$process_cond
	{
		$sql = "SELECT a.process_id, a.id,a.re_stenter_no,b.const_composition AS const_composition,b.gsm as gsm ,b.roll_no AS roll_no,b.dia_width as dia_width,b.width_dia_type as width_dia_type,b.roll_id as roll_id,sum(b.batch_qty) as batch_qty,sum(b.production_qty) as production_qty,sum(b.no_of_roll) as no_of_roll from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$batch_id and a.entry_form=34 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1   group by a.process_id,a.id,a.re_stenter_no,b.const_composition,b.gsm,b.roll_no,b.dia_width,b.width_dia_type,b.roll_id";
	}
	else
	{
		 $sql = "SELECT a.process_id,a.id,a.re_stenter_no,group_concat(b.const_composition) AS const_composition,group_concat(b.gsm) as gsm,group_concat(b.roll_no) as roll_no ,group_concat(b.dia_width)  as dia_width,group_concat(b.width_dia_type)  as width_dia_type,group_concat(b.roll_id) as roll_id,sum(b.batch_qty) as batch_qty,sum(b.production_qty) as production_qty,sum(b.no_of_roll) as no_of_roll from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$batch_id and a.entry_form=34 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1    group by a.process_id,a.id,a.re_stenter_no";	
	}
	// echo $sql;die();
	$dtls_arr = array();
	$check_arr =array();
	...
	foreach($result as $key=>$val)
	{
		...
		$dtls_arr[$val[csf('id')]][$val[csf('batch_id')]]['process_id']  = $val[csf('process_id')];
            ...
            <tbody>
            	...
            	{ 
	            	...
	            	{ 					
	                	...
	                	<tr bgcolor="<? echo $bgcolor; ?>" onClick='show_list_view("<? echo $row["id"];?>","child_form_input_data","list_fabric_desc_container","requires/special_finish_controller");get_php_form_data("<? echo $row["id"];?>","mst_id_child_form_input_data","requires/special_finish_controller");'  style="cursor:pointer" >
	                        ...
	                        <td align="right" width="80"><p><? echo $conversion_cost_head_array[$row["process_id"]]; ?></p></td>
	                        <td align="right" width="80"><p><? echo $row["re_stenter_no"]; ?></p></td>
	                   </tr>
	                	<? 
	                	$i++;
	                } 
...
if($action=="child_form_input_data")
{
	...
	$sql_result=sql_select("SELECT a.id,b.id as dtls_id,b.prod_id,b.const_composition,b.gsm,b.dia_width,b.width_dia_type,b.batch_qty,b.production_qty,b.no_of_roll,b.barcode_no,b.roll_no,b.roll_id,b.rate,b.amount from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.id=$data and a.entry_form=34 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 order by b.const_composition");
	//if(count($sql_result)>0)
	///{
	$roll_maintained=return_field_value("fabric_roll_level","variable_settings_production","company_name ='$company_id' and item_category_id=50 and variable_list=3 and is_deleted=0 and status_active=1");
	$page_upto_id=return_field_value("page_upto_id","variable_settings_production","company_name =$company_id and item_category_id=50 and variable_list=3 and is_deleted=0 and status_active=1");
			 ...
			 ?>
              &nbsp; &nbsp;<? echo $i; ?></td>
				...
				<td><input type="text" name="txtdiawidth_<? echo $i; ?>" id="txtdiawidth_<? echo $i; ?>" class="text_boxes" style="width:70px;" value="<? echo $fabric_typee[$width_dia_type];?>" disabled/>
                 ...
                 <td><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes_numeric" value="<? echo $roll_no;?>" style="width:60px;" <? echo $readonly; ?>  /> <input type="hidden" name="rollid_<? echo $i; ?>" id="rollid_<? echo $i; ?>" style="width:50px;" value="<? echo $roll_id;?>" class="text_boxes_numeric" />	</td>
...
{
	  $sql_result=sql_select("SELECT  a.re_stenter_no, a.id,a.batch_id,a.company_id,a.service_source,a.service_company,a.next_process_id,a.advanced_prod_qty,a.is_re_dyeing,a.received_chalan,a.issue_chalan,issue_challan_mst_id,a.process_end_date,a.production_date,a.process_start_date,a.process_id,a.end_hours,a.end_minutes,a.start_minutes, a.temparature, a.steam,a.stretch, a.speed_min,a.over_feed, a.speed_min,a.start_hours,a.floor_id,a.machine_id,a.result,a.shift_name,a.remarks,a.booking_no,b.currency_id,b.exchange_rate from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.id=$data and entry_form=34 and a.status_active=1 and a.is_deleted=0 ");
...
//echo  $sql_batch=("select id,batch_no,width_dia_type,process_end_date,process_start_date,start_hours,start_minutes,end_hours,end_minutes,machine_id,floor_id,process_id,temparature,stretch,over_feed,feed_in,pinning,speed_min,chemical_name,production_date,shift_name,remarks from pro_fab_subprocess where entry_form=31 and batch_id='$data' ");
 //echo  $sql_result[0][csf("process_id")];die;
			...
			echo "document.getElementById('txt_special_fin').value	= '".$sql_result[0][csf('re_stenter_no')]."';\n";
			...
			*/
			...
			echo "$('#txt_process_name').attr('disabled',true);\n";
			...
			{
			echo "load_drop_down('requires/special_finish_controller', '".$sql_result[0][csf('service_company')]."', 'load_drop_floor', 'floor_td' );\n";
			}
		
			echo "load_drop_down( 'requires/special_finish_controller', document.getElementById('cbo_service_company').value+'**'+".$sql_result[0][csf("floor_id")].", 'load_drop_machine', 'machine_td' );\n";
			...
			echo "load_drop_down( 'requires/special_finish_controller', ".$sql_result[0][csf("service_source")]."+'**'+".$sql_result[0][csf("company_id")].", 'load_drop_down_knitting_com_new', 'dyeing_company_td' );\n";
        ...
        <table  cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all" width="900">
            ...
            <tbody>
                <tr>
                    ...
                    <? 
                   		echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 and id=".$cbo_company_id." order by company_name","id,company_name",1, "-- Select Company --", '', "load_drop_down( 'singeing_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
                    ?>
                    </td>
                    ...
                    <td align="center">
                    	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_search_category').value+'_'+document.getElementById('txt_booking_no').value+'_'+'<?php echo $supplier_id."_".$process_id; ?>', 'create_booking_search_list_view', 'search_div', 'special_finish_controller', 'setFilterGrid(\'table_body\',-1)')" style="width:100px;" />
                    </td>
                </tr>
                <tr>
...
if($action=="process_wise_re_special_no")
{
	$data=explode("**",$data);
	$txt_special_fin=return_field_value("count(id)","PRO_FAB_SUBPROCESS","process_id ='$data[1]' and batch_id='$data[0]' and status_active=1 and entry_form=34");
	 
	echo "document.getElementById('txt_special_fin').value= '".$txt_special_fin."';\n";
