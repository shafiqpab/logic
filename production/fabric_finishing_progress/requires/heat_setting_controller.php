...
if($action=='show_fabric_desc_listview')
{
   ...
   //if((page_upto*1==1 || page_upto*1>1) && roll_maintained==1  ) // && roll_maintained==1 
	...
	if(($page_upto_id==1 || $page_upto_id>1) && $roll_maintained==1)
	{
		...
		$prollData=sql_select("select id,roll_no,barcode_no from pro_roll_details where  entry_form in(2,22) and status_active=1 and is_deleted=0");
	...
	if(($page_upto_id==1 || $page_upto_id>1) && $roll_maintained==1)
	{
		...
		$sql_data=("SELECT a.entry_form,b.width_dia_type,a.company_id,b.item_description,b.gsm,b.fin_dia,b.width_dia_type,b.prod_id,b.barcode_no,b.roll_no,b.roll_id, sum(b.batch_qnty) as batch_qnty from pro_batch_create_dtls b,pro_batch_create_mst a where b.mst_id='$batch_id'  and a.id=b.mst_id and  a.entry_form in(0,36)  and b.status_active=1 and b.is_deleted=0   group by b.id,b.item_description,b.width_dia_type,b.gsm,b.fin_dia,b.prod_id,a.entry_form,b.roll_id,b.roll_no,a.company_id,b.barcode_no");
		//echo $sql_data;
		...
		if(count($sql_result)>0)
		{
			...
			$head_densityData=sql_select("SELECT roll_id,knitting_density from pro_qc_result_mst where status_active=1 and is_deleted=0 and roll_id in($rollIs)");
			...
			<tr class="general" id="row_<? echo $i; ?>">
	            <td width="60" id="sl_<? echo $i; ?>"><input type="checkbox" id="checkRow_<? echo $i; ?>" name="checkRow[]"  checked> &nbsp; &nbsp;<? echo $i; ?></td>
	                ...
	                <td><input type="text" name="txtafterknitdensity_<? echo $i; ?>" id="txtafterknitdensity_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px;" value="" /></td> 
	                 ...
	                 <td><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes_numeric" value="<? //echo $roll_no;?>" style="width:60px;" readonly />
					  <input type="hidden" name="rollid_<? echo $i; ?>" id="rollid_<? echo $i; ?>" style="width:35px;" value="<? //echo $row[csf('roll_id')];?>" class="text_boxes_numeric" />
...
if($action=="save_update_delete")
{
	...
	$check_batch_no=sql_select("select batch_no from pro_batch_create_mst a where a.id=$hidden_batch_id and a.company_id=$cbo_company_id and a.status_active=1");

	if(str_replace("'", "", $txt_batch_no) != $check_batch_no[0][csf("batch_no")])
	{
		echo "13**Batch no does not match with batch id";
		die;
	}
	...
	$batch_prod=sql_select("select a.batch_id,max(a.id) as mst_id from pro_fab_subprocess a where  a.batch_id=$hidden_batch_id and a.company_id=$cbo_company_id and a.status_active=1 $up_id_cond group by a.batch_id ");
	//echo "10**select a.batch_id,max(a.id) as mst_id from pro_fab_subprocess a where  a.batch_id=$hidden_batch_id and a.company_id=$cbo_company_id and a.status_active=1 $up_id_cond group by a.batch_id ";die;
		$last_mst_id=0;
		...
		if ($operation==0)
		{
		$batch_last_process=sql_select("select a.batch_id,a.end_hours,a.end_minutes,a.process_end_date,a.production_date from pro_fab_subprocess a where  a.batch_id=$hidden_batch_id and a.company_id=$cbo_company_id and a.id in($last_mst_id) and a.status_active=1  ");
		//echo "10**select a.batch_id,a.end_hours,a.end_minutes,a.process_end_date,a.production_date from pro_fab_subprocess a where  a.batch_id=$hidden_batch_id and a.company_id=$cbo_company_id and a.id in($last_mst_id) and a.status_active=1  ";die;
		...
		{
		$batch_last_process=sql_select("select a.batch_id,a.end_hours,a.end_minutes,a.process_end_date,a.production_date from pro_fab_subprocess a where  a.batch_id=$hidden_batch_id and a.company_id=$cbo_company_id and a.id in($last_mst_id) and a.id!=$updateId and a.status_active=1  ");
		...
		//echo "13**Back date time not allowed"."**".$end_date_time."**".$previ_end_date_time;
			//disconnect($con);die;
...
//cbo_result_name
		...
		{
			$field_array_dtls="id, mst_id, entry_page, prod_id, const_composition, gsm,dia_width,width_dia_type, batch_qty,roll_no,barcode_no,roll_id,production_qty, rate,amount,after_heatset_density,currency_id,exchange_rate,inserted_by, insert_date";
		}
		else
		{
		...
		if(($page_upto_id==1 || $page_upto_id>1) && str_replace("'","",$roll_maintained)==1)
		{
			for($i=1;$i<=$total_row;$i++)
			{
				...
				if(str_replace("'","",$$checked_tr)==1)
				{
					...
					$data_array_dtls.="(".$id_dtls.",".$mst_update_id.",32,".$Itemprod_id.",".$$txtconscomp.",".$$txtgsm.",".$$txtbodypart.",".$$txtdiawidthID.", ".$$txtbatchqnty.",".$$txtroll.",".$$txtbarcode.",".$$rollid.",".$$txtproductionqty.",".$$txtrate.",".$$txtamount.",".$$txtafterknitdensity.",".$hidden_currency.",".$hidden_exchange_rate.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
					$id_dtls=$id_dtls+1;
				}
			}
		}
	...
	{ 
		...
		$field_array_update="company_id*batch_no*service_source*service_company*received_chalan*issue_chalan*issue_challan_mst_id*batch_id*batch_ext_no*process_id*process_end_date*production_date*temparature*stretch*over_feed*feed_in*pinning*speed_min*process_start_date*start_hours*start_minutes*end_hours*end_minutes*machine_id*result*floor_id*chemical_name*next_process_id*advanced_prod_qty*is_re_dyeing*shift_name*remarks*booking_no*updated_by*update_date";
			
		$data_array_update="".$cbo_company_id."*".$txt_batch_no."*".$cbo_service_source."*".$cbo_service_company."*".$txt_recevied_chalan."*".$txt_issue_chalan."*".$txt_issue_mst_id."*".$hidden_batch_id."*".$txt_ext_id."*".$cbo_sub_process."*".$txt_process_end_date."*".$txt_process_date."*".$txt_temparature."*".$txt_stretch."*".$txt_feed."*".$txt_feed_in."*".$txt_pinning."*".$txt_speed."*".$txt_process_start_date."*".$txt_start_hours."*".$txt_start_minutes."*".$txt_end_hours."*".$txt_end_minutes."*".$cbo_machine_name."*".$cbo_result_name."*".$cbo_floor."*".$txt_chemical."*".$cbo_next_process."*".$txt_advance_prod."*".$re_checkbox."*".$cbo_shift_name."*".$txt_remarks."*".$txt_booking_no."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		if(($page_upto_id==1 || $page_upto_id>1) && str_replace("'","",$roll_maintained)==1)
		{
			$field_array_up="mst_id*gsm*production_qty*roll_no*barcode_no*roll_id*rate*amount*after_heatset_density*currency_id*exchange_rate*updated_by*update_date";
		   ...
		   for($i=1; $i<=$total_row; $i++)
			{
				...
				if(str_replace("'","",$$checkRowTd)==1)					
				{
					...
					$data_array_up[str_replace("'",'',$$updateiddtls)] =explode("*",("".$update_id."*".$$txtgsm."*".$$txtproductionqty."*".$$txtroll."*".$$txtbarcode."*".$$rollid."*".$$txtrate."*".$$txtamount."*".$$txtafterknitdensity."*".$hidden_currency."*".$hidden_exchange_rate."*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'"));
				}
				else
				{
	...
	else if ($operation==2)   // Delete Here
	{
	   ...
	   $con = connect();
		...
		$update_id=str_replace("'",'',$txt_update_id);
		$flag=20;
		$rID=execute_query( "update  pro_fab_subprocess set status_active=0,is_deleted=1,update_date='".$pc_date_time."',updated_by=".$_SESSION['logic_erp']['user_id']."  where  id =$update_id ",1);
		if($rID) $flag=1; else $flag=20;
		
		$rID2=execute_query( "update  pro_fab_subprocess_dtls set status_active=0,is_deleted=1,update_date='".$pc_date_time."',updated_by=".$_SESSION['logic_erp']['user_id']."   where  mst_id =$update_id ",1);
...
{
	//print($data);
	$sql_result=sql_select("SELECT a.chemical_name, a.id,a.company_id,a.batch_id,a.service_source,a.service_company,a.next_process_id,a.advanced_prod_qty,a.is_re_dyeing,a.received_chalan,a.issue_chalan,issue_challan_mst_id,a.process_end_date, a.production_date,a.process_start_date,a.process_id,a.end_hours,a.end_minutes,a.start_hours,a.start_minutes,a.temparature,a.stretch, a.over_feed, a.feed_in,a.pinning,a.speed_min,a.floor_id,a.machine_id,a.result,a.shift_name,a.remarks,a.booking_no,b.currency_id,b.exchange_rate from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.id=$data and entry_form=32  and a.status_active=1 and a.is_deleted=0 group by a.chemical_name,a.is_re_dyeing,a.next_process_id,a.advanced_prod_qty, a.id, a.service_source, a.service_company,a.received_chalan, a.issue_chalan, issue_challan_mst_id, a.process_end_date, a.production_date, a.process_start_date ,a.process_id, a.end_hours,a.end_minutes, a.start_hours, a.temparature,a.stretch,a.over_feed,a.feed_in,a.pinning,a.speed_min,a.floor_id,a.machine_id,a.result, a.company_id,a.batch_id, a.shift_name, a.remarks,a.start_minutes,a.booking_no,b.currency_id,b.exchange_rate");
  ...
  $trims_weight=return_field_value("total_trims_weight","pro_batch_create_mst","company_id =".$sql_result[0][csf('company_id')]." and id=".$sql_result[0][csf('batch_id')]." and is_deleted=0 and status_active=1");
		...
		//echo "document.getElementById('cbo_service_source').value	= ".$sql_result[0][csf('service_source')].";\n";
			...
			echo "load_drop_down( 'requires/heat_setting_controller', ".$sql_result[0][csf("service_source")]."+'**'+".$sql_result[0][csf("company_id")].", 'load_drop_down_knitting_com_new', 'dyeing_company_td' );\n";
