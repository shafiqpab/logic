...
if ($action=="load_drop_floor")
{
	...
	echo create_drop_down( "cbo_floor", 135, "select id,floor_name from lib_prod_floor where status_active=1 and is_deleted=0 and company_id='$data[0]' and production_process=4 order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected,"load_drop_down( 'requires/scouring_production_entry_controller', document.getElementById('cbo_service_company').value+'**'+this.value, 'load_drop_machine', 'machine_td' );" );     	 
	exit();
}
if($action=="load_drop_down_knitting_com")
{
	...
	if($data[0]==1)
	{
		echo create_drop_down( "cbo_service_company", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name",1, "-- Select --", 0, "load_drop_down('requires/scouring_production_entry_controller', this.value, 'load_drop_floor', 'floor_td' );","" );
	}
	else if($data[0]==3)
	{	
...
if($action=="create_batch_search_list_view")
{
	...
	if($batch_type==0)
		...
		$search_field_cond_batch="and entry_form in (563)";
...
{ 	
	...
	$roll_maintained=return_field_value("fabric_roll_level","variable_settings_production","company_name ='$company_id' and item_category_id=2 and variable_list=3 and is_deleted=0 and status_active=1");
	...
	a.booking_without_order,a.batch_date,a.color_range_id,a.insert_date,a.batch_for,a.dur_req_hr,a.dur_req_min,a.company_id,a.process_id,a.entry_form,a.booking_no,a.total_trims_weight ";
	if($db_type==0) $pop_batch="order by a.id";
	else if($db_type==2) $pop_batch=" group by a.id,a.batch_no,a.dur_req_hr,a.dur_req_min, a.batch_weight,batch_date,a.color_id,a.color_range_id,a.insert_date,a.batch_for,a.process_id,a.company_id, a.booking_without_order ,a.entry_form,a.booking_no,a.total_trims_weight order by a.id";
	...
	a.color_id,a.color_range_id,a.insert_date,a.dur_req_hr,a.dur_req_min, a.booking_without_order, sum(b.batch_qnty) as batch_qnty, $select_list from pro_batch_create_mst a,pro_batch_create_dtls b where a.id=b.mst_id  and a.entry_form in(563) and a.id='$batch_id_ex' $select_group_row");
	}
	else
	{
		$data_array=sql_select("SELECT a.id,a.batch_no,a.entry_form,a.total_trims_weight,a.batch_for,a.company_id,a.process_id as process_id_batch, a.batch_weight,Max(a.extention_no) as extention_no,a.batch_date,a.booking_no, 
	a.color_id,a.color_range_id,a.insert_date,a.dur_req_hr,a.dur_req_min, a.booking_without_order, sum(b.batch_qnty) as batch_qnty, $select_list from pro_batch_create_mst a,pro_batch_create_dtls b where a.id='$batch_id_ex' and a.entry_form in(563)  and a.id=b.mst_id  $pop_batch");	
	}
	...
	{  	//if($row[csf('batch_against')])
		...
		//echo "load_drop_down( 'requires/scouring_production_entry_controller', '".$row[csf("company_id")]."', 'load_drop_floor', 'floor_td' );\n";
		...
		//print_r($insert_t);
		echo "document.getElementById('txt_batch_duration').value 		= '".$duration_hr.':'. $duration_min."';\n";
		...
		foreach($sql_batch as $r_batch)
		{
			...
			echo "load_drop_down( 'requires/scouring_production_entry_controller', document.getElementById('cbo_company_id').value+'**'+".$r_batch[csf("floor_id")].", 'load_drop_machine', 'machine_td' );\n";
...
if($action=='show_fabric_desc_listview')
{
	...
	$i=1;
	$sql_result=sql_select("select a.entry_form,b.width_dia_type,b.fin_dia,b.gsm,b.item_description, sum(b.batch_qnty) as batch_qty,b.prod_id,b.roll_id,b.roll_no from pro_batch_create_dtls b,pro_batch_create_mst a  where b.mst_id='$batch_id'   and a.id=b.mst_id and a.id='$batch_id' and  a.entry_form in(0,36,563) and b.status_active=1 and b.is_deleted=0 and b.roll_id not in(select b.roll_id from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id='$batch_id'    and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 ) group by b.id,b.item_description,b.width_dia_type,b.prod_id,a.entry_form,b.fin_dia,b.gsm,b.roll_id,b.roll_no");
	//echo "select a.entry_form,b.width_dia_type,b.fin_dia,b.gsm,b.item_description, sum(b.batch_qnty) as batch_qty,b.prod_id,b.roll_id,b.roll_no from pro_batch_create_dtls b,pro_batch_create_mst a  where b.mst_id='$batch_id'   and a.id=b.mst_id and a.id='$batch_id' and  a.entry_form in(0,36) and b.status_active=1 and b.is_deleted=0 and b.roll_id not in(select b.roll_id from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id='$batch_id' and a.entry_form=47 and b.entry_page=47 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 ) group by b.id,b.item_description,b.width_dia_type,b.prod_id,a.entry_form,b.fin_dia,b.gsm,b.roll_id,b.roll_no";
	...
	if(count($sql_result)>0)
	{
		...
		{
			$rollArray[$row[csf('roll_id')]] = $row[csf('roll_id')];
		}
		$rollIs = implode(",", $rollArray);
		$fabric_roll_arr=array();
		$prollData=sql_select("select id,roll_no,barcode_no from pro_roll_details where  status_active=1 and is_deleted=0 and id in($rollIs)");
	...
	if(($page_upto_id==8 || $page_upto_id>8) && $roll_maintained==1)
	{
		if(count($sql_result)>0)
		{
			foreach($sql_result as $row)
			{
				if($row[csf('entry_form')]==563)
				{
					...
					//echo $row[csf('roll_no')].'DDDDDDDDDD';;
				}
				else
				{
					$cons_comps='';
				...
				<tr class="general" id="row_<? echo $i; ?>">
					...
					<td ><input type="text" name="txtbodypart_<? echo $i; ?>" id="txtbodypart_<? echo $i; ?>" class="text_boxes" style="width:40px;" value="<? echo  $dia_width; ?>" disabled/></td> 
					<td ><input type="text" name="txtdiawidth_<? echo $i; ?>" id="txtdiawidth_<? echo $i; ?>" class="text_boxes" style="width:70px;" value="<? echo $fabric_typee[$row[csf('width_dia_type')]];?>" disabled/>
					 ...
					 <td ><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes_numeric" style="width:35px;" value="<? echo $roll_no; ?>" <? echo $readonly; ?> /> <input type="hidden" name="rollid_<? echo $i; ?>" id="rollid_<? echo $i; ?>" style="width:50px;" value="<? echo $row[csf('roll_id')];?>" class="text_boxes_numeric" /></td>
		...
		else
		{
		    ...
		    <tr class="general" id="row_<? echo $i; ?>">
				...
				<td><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes_numeric" value="<? //echo $row[csf('roll_no')];?>" style="width:35px;" <? echo $readonly; ?> />
        ...
        <tr>
            ...
            <td align="right"><input type="text" name="total_batch_qnty" id="total_batch_qnty" class="text_boxes_numeric" style="width:60px" value="<? echo number_format($b_qty,2); ?>" readonly /></td> <td align="right"><input type="text" name="total_production_qnty" id="total_production_qnty" class="text_boxes_numeric" style="width:60px" value="<? echo number_format($prod_qty,2); ?>"   readonly /></td>
            <td align="right"></td>           
	...
	else
	{
		...
		$i=1;	
		$sql_result=sql_select("select b.id, b.gsm,b.width_dia_type,b.dia_width,b.const_composition,b.production_qty,b.no_of_roll,b.roll_no,b.batch_qty, b.prod_id, b.rate,b.amount from pro_fab_subprocess_dtls b,pro_fab_subprocess a where a.id=b.mst_id and a.batch_id='$batch_id' and a.entry_form=613  and a.status_active=1 and a.is_deleted=0");
		if(count($sql_result)>0)
		{
			foreach($sql_result as $row)
			{
				...
				<tr class="general" id="row_<? echo $i; ?>">
					...
					<td title="<? echo $row[csf('width_dia_type')];?>"><input type="text" name="txtdiawidth_<? echo $i; ?>" id="txtdiawidth_<? echo $i; ?>" class="text_boxes" style="width:70px;" value="<? echo $fabric_typee[$row[csf('width_dia_type')]];?>" disabled/>
	                ...
	                <td><input type="text" name="txtproductionqty_<? echo $i; ?>" id="txtproductionqty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo $row[csf('production_qty')]; ?>" onKeyUp="calculate_production_qnty();"/></td>
			    ...
			    <?
				$b_qty+=$row[csf('batch_qty')];
				$tot_production_qty+=$row[csf('production_qty')];
		...
		{
			$result=sql_select("select a.entry_form, b.width_dia_type,b.roll_no,b.fin_dia,b.gsm,b.item_description,b.width_dia_type,b.prod_id, sum(b.batch_qnty) as batch_qnty from pro_batch_create_dtls b,pro_batch_create_mst a  where mst_id='$batch_id' and a.id=b.mst_id and  a.entry_form in(0,36,563) and b.status_active=1 and b.is_deleted=0  group by a.entry_form, b.width_dia_type,b.item_description,b.width_dia_type,b.roll_no,b.fin_dia,b.gsm,b.prod_id");
			//echo "select a.entry_form, b.width_dia_type,b.roll_no,b.fin_dia,b.gsm,b.item_description,b.width_dia_type,b.prod_id, sum(b.batch_qnty) as batch_qnty from pro_batch_create_dtls b,pro_batch_create_mst a  where mst_id='$batch_id' and a.id=b.mst_id and  a.entry_form in(0,36) and b.status_active=1 and b.is_deleted=0  group by a.entry_form, b.width_dia_type,b.item_description,b.width_dia_type,b.roll_no,b.fin_dia,b.gsm,b.prod_id";
			foreach($result as $row)
			{
				...
				<tr class="general" id="row_<? echo $i; ?>">
					...
					<td><input type="text" name="txtdiawidth_<? echo $i; ?>" id="txtdiawidth_<? echo $i; ?>" class="text_boxes" style="width:70px;" value="<? echo $fabric_typee[$row[csf('width_dia_type')]];?>" disabled/>
	                 ...
	                 <td><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes_numeric" style="width:35px;" value="<? echo $roll_no;?>"/>  <input type="hidden" name="rollid_<? echo $i; ?>" id="rollid_<? echo $i; ?>" style="width:50px;" value="<? //echo $row[csf('roll_id')];?>" class="text_boxes_numeric" /></td>
	                ...
	                <td><input type="text" name="txtproductionqty_<? echo $i; ?>" id="txtproductionqty_<? echo $i; ?>" class="text_boxes_numeric" value="<? echo  number_format($row[csf('batch_qnty')],2,".",""); ?>" style="width:60px;" onKeyUp="calculate_production_qnty();"/></td>
	                <td><input type="text" name="txtrate_<? echo $i; ?>" id="txtrate_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" readonly/></td>
	            	<td><input type="text" name="txtamount_<? echo $i; ?>" id="txtamount_<? echo $i; ?>" class="text_boxes_numeric" style="width:70px;"  readonly/> </td>
				</tr>
			    <?
				$b_qty+= $row[csf('batch_qnty')];
				$tot_production_qty+=$row[csf('batch_qnty')];
				$i++;
			}
		}
		?>
		<tr>
	     	<td colspan="6" align="right"><b style=" float:left">Check/Uncheck<input type="checkbox" style="width:30px" id="allcheckbox" name="allcheckbox[]"  onClick="checkbox_all(this.id);" checked  /></b><b>Sum:</b> <? //echo $b_qty; ?> </td>
	        <td align="right"><b><input type="text" name="total_batch_qnty" id="total_batch_qnty" class="text_boxes_numeric" style="width:60px" value="<? echo number_format($b_qty,2); ?>" readonly /> </b></td><td><input type="text" name="total_production_qnty" id="total_production_qnty" class="text_boxes_numeric" style="width:60px"  readonly /></td>
	        <td align="right"></td>           
...
if($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		{
			$field_array="id,company_id,batch_no,batch_id,process_id,process_end_date,production_date,temparature,stretch,over_feed,feed_in,pinning,speed_min,process_start_date,start_hours,start_minutes,end_hours,end_minutes,machine_id,floor_id,entry_form,shift_name,remarks,issue_chalan,service_source,service_company,issue_challan_mst_id,booking_no,inserted_by,insert_date";
			$id=return_next_id( "id", "pro_fab_subprocess", 1 ) ;
			$data_array="(".$id.",".$cbo_company_id.",".$txt_batch_no.",".$txt_batch_ID.",".$cbo_sub_process.",".$txt_process_end_date.",".$txt_process_date.",".$txt_temparature.",".$txt_stretch.",".$txt_feed.",".$txt_feed_in.",".$txt_pinning.",".$txt_speed.",".$txt_process_start_date.",".$txt_start_hours.",".$txt_start_minutes.",".$txt_end_hours.",".$txt_end_minutes.",".$cbo_machine_name.",".$cbo_floor.",617,".$cbo_shift_name.",".$txt_remarks.",".$txt_issue_chalan.",".$cbo_service_source.",".$cbo_service_company.",".$txt_issue_mst_id.",".$txt_booking_no.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			//print_r($data_array);die;
			$mst_update_id=str_replace("'","",$id);	
		}
		if(str_replace("'","",$roll_maintained)==1)
		{
			$field_array_dtls="id,mst_id,entry_page,prod_id,const_composition,gsm,dia_width,width_dia_type,batch_qty,roll_no,roll_id,production_qty, rate, amount, currency_id,exchange_rate,inserted_by,insert_date";
		}
		else
		{
		...
		if(str_replace("'","",$roll_maintained)==1)
		{
			for($i=1;$i<=$total_row;$i++)
			{
				...
				$data_array_dtls.="(".$id_dtls.",".$mst_update_id.",617,".$$prod_id.",".$$txtconscomp.",".$$txtgsm.",".$$txtbodypart.",".$$txtdiawidthID.",".$$txtbatchqnty.",".$$txtroll.",".$$rollid.",".$$txtproductionqty.",".$$txtrate.",".$$txtamount.",".$hidden_currency.",".$hidden_exchange_rate.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				$id_dtls=$id_dtls+1;
				}
		...
		else
		{
			for($i=1;$i<=$total_row;$i++)
			{
				...
				if(str_replace("'","",$$checked_tr)==1)
				{
					...
					// $Itemprod_id=str_replace("'","",$$prod_id);
					if($data_array_dtls!="") $data_array_dtls.=","; 
					$data_array_dtls.="(".$id_dtls.",".$mst_update_id.",617,".$$prod_id.",".$$txtconscomp.",".$$txtgsm.",".$$txtbodypart.",".$$txtdiawidthID.",".$$txtbatchqnty.",".$$txtroll.",".$$txtproductionqty.",".$$txtrate.",".$$txtamount.",".$hidden_currency.",".$hidden_exchange_rate.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
					$id_dtls=$id_dtls+1;
					//print_r($data_array_dtls);die;
				}
			}
	...
	{ 
		...
		$field_array_update="company_id*batch_no*service_source*service_company*issue_chalan*issue_challan_mst_id*batch_id*process_id*process_end_date*production_date*temparature*stretch*over_feed*feed_in*pinning*speed_min*process_start_date*start_hours*start_minutes*end_hours*end_minutes*machine_id*floor_id*shift_name*remarks*booking_no*updated_by*update_date";
		$data_array_update="".$cbo_company_id."*".$txt_batch_no."*".$cbo_service_source."*".$cbo_service_company."*".$txt_issue_chalan."*".$txt_issue_mst_id."*".$txt_batch_ID."*".$cbo_sub_process."*".$txt_process_end_date."*".$txt_process_date."*".$txt_temparature."*".$txt_stretch."*".$txt_feed."*".$txt_feed_in."*".$txt_pinning."*".$txt_speed."*".$txt_process_start_date."*".$txt_start_hours."*".$txt_start_minutes."*".$txt_end_hours."*".$txt_end_minutes."*".$cbo_machine_name."*".$cbo_floor."*".$cbo_shift_name."*".$txt_remarks."*".$txt_booking_no."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
    ...
    //print_r($data_array);die;
		...
		if(str_replace("'","",$roll_maintained)==1)
		{
			$field_array_up="mst_id*gsm*production_qty*roll_no*roll_id*rate*amount*currency_id*exchange_rate*updated_by*update_date";
		...
		{
			$field_array_up="mst_id*gsm*production_qty*roll_no*rate*amount*currency_id*exchange_rate*updated_by*update_date";
			$field_array_delete="updated_by*update_date*status_active*is_deleted";
			for($i=1; $i<=$total_row; $i++)
			{
				$prod_id="txtprodid_".$i;
...
{
	$data=explode("**",$data);
	$sql="select id, batch_no,company_id, is_sales from pro_batch_create_mst where batch_no='".trim($data[1])."' and entry_form in(0,36,563) and is_deleted=0 and status_active=1 order by id desc";
...
if($action=="populate_data_from_data")
{
	...
	{		
		...
		echo "load_drop_down( 'requires/scouring_production_entry_controller', ".$row[csf("dyeing_source")]."+'**'+".$row[csf("company_id")].", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
...
$sql_result=sql_select("select a.entry_form,b.width_dia_type,a.company_id,b.item_description,b.width_dia_type,b.prod_id,b.roll_no,b.roll_id, sum(b.batch_qnty) as batch_qnty from pro_batch_create_dtls b,pro_batch_create_mst a where  a.id=b.mst_id and   a.entry_form in(0,36,563) and b.status_active=1 and b.is_deleted=0  and b.roll_id in($hidden_roll_id) and b.mst_id=$batch_id and b.roll_id not in(select b.roll_id from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id='$batch_id' and a.entry_form=47 and b.status_active=1 and b.is_deleted=0 )  group by b.id,b.item_description,b.width_dia_type,b.prod_id,a.entry_form,b.roll_id,b.roll_no,a.company_id");
	if(count($sql_result)>0)
	{
	...
	{ 
		$sql=("select a.entry_form,b.width_dia_type,a.company_id,b.item_description,b.width_dia_type,b.prod_id,b.roll_no,b.roll_id, sum(b.batch_qnty) as batch_qnty from pro_batch_create_dtls b,pro_batch_create_mst a where  a.id=b.mst_id  and a.entry_form in(0,36,563) and b.status_active=1 and b.is_deleted=0  and b.mst_id=$batch_id and b.roll_id not in(select b.roll_id from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.batch_id='$batch_id'  and b.status_active=1 and b.is_deleted=0 )  group by b.id,b.item_description,b.width_dia_type,b.prod_id,a.entry_form,b.roll_id,b.roll_no,a.company_id");
	...
	foreach($sql_result as $row)
		{
			//$desc=explode(",",$row[csf('item_description')]);
				...
				$roll_maintained=return_field_value("fabric_roll_level","variable_settings_production","company_name ='$compamy' and item_category_id=2 and variable_list=3 and is_deleted=0 and status_active=1");
	...
	<? }
	 ...
	 <tr>
        ...
        <td align="right"><b><input type="text" name="total_batch_qnty" id="total_batch_qnty" class="text_boxes_numeric" value="<? echo number_format($b_qty,2); ?>" style="width:60px" readonly />  </b></td><td><input type="text" name="total_production_qnty" id="total_production_qnty" class="text_boxes_numeric" style="width:60px" value="<? echo number_format($tot_production_qty,2); ?>" readonly /> </td>
        <td align="right"></td>           
        <td align="right"><input type="text" name="total_amount" id="total_amount" class="text_boxes_numeric" style="width:70px" readonly /> </td>
     </tr>
	<?
            ...
            <tbody>
            	<? foreach($result as $row){  
					...
					$roll_no=$row[csf("no_of_roll")];
								...
								?>
                	<tr bgcolor="<? echo $bgcolor; ?>" onClick='show_list_view("<? echo $row[csf("id")];?>","child_form_input_data","list_fabric_desc_container","requires/scouring_production_entry_controller");get_php_form_data("<? echo $row[csf("id")];?>","mst_id_child_form_input_data","requires/scouring_production_entry_controller")'  style="cursor:pointer" >
...
if($action=="child_form_input_data")
{
	...
	$sql_result=sql_select("select a.id,b.id as dtls_id,b.prod_id,b.const_composition,b.gsm,b.dia_width,b.width_dia_type,b.batch_qty,b.production_qty,b.no_of_roll,b.roll_no,b.roll_id,b.rate,b.amount from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.id=$data and a.entry_form=617 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1");
	...
	<tr>
      	<td colspan="6" align="right"><b>Sum:</b> <? //echo $b_qty; ?> </td>
        <td align="right"><b><input type="text" name="total_batch_qnty" id="total_batch_qnty" class="text_boxes_numeric" style="width:60px" value="<? echo number_format($tot_batch_qty,2); ?> " readonly /><? //echo $tot_batch_qty; ?> </b></td> <td align="right"><input type="text" name="total_production_qnty" id="total_production_qnty" class="text_boxes_numeric" style="width:50px" value="<? echo $tot_production_qty; ?>" readonly  /></td>
        <td align="right"></td>           
...
{
	$sql_result=sql_select("SELECT a.id,a.company_id,a.batch_id,a.service_source,a.service_company,a.received_chalan,a.issue_chalan,issue_challan_mst_id,a.process_end_date,a.production_date,a.process_start_date,a.process_id,a.end_hours,a.end_minutes,a.start_minutes,a.temparature,a.stretch,a.over_feed,a.feed_in,a.pinning,a.speed_min,a.start_hours,a.floor_id,a.machine_id,a.shift_name,a.remarks,a.booking_no,b.currency_id,b.exchange_rate from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.id=$data and entry_form=617  and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0");
  	$variable_production_roll=return_field_value("fabric_roll_level", "variable_settings_production", "variable_list=3 and item_category_id=2 and status_active=1 and is_deleted=0 and company_name=".$sql_result[0][csf('company_id')]."");
  	...
  	$trims_weight=return_field_value("total_trims_weight","pro_batch_create_mst","company_id =".$sql_result[0][csf('company_id')]." and id=".$sql_result[0][csf('batch_id')]." and is_deleted=0 and status_active=1");
		...
		//echo $variable_production_roll;die;
		...
		//echo "set_multiselect('cbo_sub_process','0','1','".$sql_result[0][csf('process_id')]."','0');\n";
		$minute=''; $hour='';
		...
		{
		echo "load_drop_down('requires/scouring_production_entry_controller', '".$sql_result[0][csf('service_company')]."', 'load_drop_floor', 'floor_td' );\n";
		}
		echo "load_drop_down( 'requires/scouring_production_entry_controller', document.getElementById('cbo_service_company').value+'**'+".$sql_result[0][csf("floor_id")].", 'load_drop_machine', 'machine_td' );\n";
		...
		echo "load_drop_down( 'requires/scouring_production_entry_controller', ".$sql_result[0][csf("service_source")]."+'**'+".$sql_result[0][csf("company_id")].", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
        ...
        <table  cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all" width="900">
            ...
            <tbody>
                <tr>
                    ...
                    <? 
                   		echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 and id=".$cbo_company_id." order by company_name","id,company_name",1, "-- Select Company --", '', "load_drop_down( 'scouring_production_entry_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
                    ?>
                    </td>
                    ...
                    <td align="center">
                    	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_search_category').value+'_'+document.getElementById('txt_booking_no').value+'_'+'<?php echo $supplier_id."_".$process_id; ?>', 'create_booking_search_list_view', 'search_div', 'scouring_production_entry_controller', 'setFilterGrid(\'table_body\',-1)')" style="width:100px;" />
                    </td>
                </tr>
                <tr>
