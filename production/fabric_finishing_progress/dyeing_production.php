...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../../logout.php";
	...
	function openmypage_servicebook()
	{
		...
		var page_link='requires/dyeing_production_controller.php?cbo_company_id='+cbo_company_id+'&supplier_id='+supplier_id+'&process_id='+process_id+'&action=service_booking_popup';
	...
	function openmypage_batchnum()
	{
		...
		{
			page_link='requires/dyeing_production_controller.php?action=batch_number_popup&cbo_company_id='+cbo_company_id+'&batch_no='+batch_no+'&load_unload='+load_unload;

			//var page_link='requires/dyeing_production_controller.php?cbo_company_id='+cbo_company_id+'&action=batch_number_popup'+'&load_unload=load_unload';
			...
			emailwindow.onclose=function()
			{
				...
				if(batch_datas!="")
				{
					...
					freeze_window(5);
					//get_php_form_data(document.getElementById('cbo_load_unload').value+'_'+response[1]+'_'+batch_no+'_'+response[2], "populate_data_from_batch", "requires/dyeing_production_controller" );
					...
					get_php_form_data(document.getElementById('cbo_load_unload').value+'_'+batch_id+'_'+batch_no+'_'+is_sales+'_'+entry_form_no, "populate_data_from_batch", "requires/dyeing_production_controller" );
					...
					if(txt_ext_id!="")
					{
						$('#cbo_responsibility').attr('disabled',false);
					}
					else{
						$('#cbo_responsibility').attr('disabled',true);
					}
					...
					show_list_view(batch_id+'_'+roll_maintained+'_'+document.getElementById('cbo_load_unload').value+'_'+document.getElementById('txt_process_id').value+'_'+entry_form_no+'_'+hidden_double_dyeing+'_'+hidden_result_id+'_'+cbo_dyeing_type,'show_fabric_desc_listview','list_fabric_desc_container','requires/dyeing_production_controller','');
					$('#cbo_company_id').attr('disabled','disabled');

					//show_list_view(batch_id+'_'+document.getElementById('cbo_load_unload').value,'show_dtls_batch_list_view','list_container','requires/dyeing_production_controller','');
					release_freezing();
				}
				get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/dyeing_production_controller' );
	...
	function check_batch()
	{
		...
		if(batch_no!="")
		{
			...
			var response=return_global_ajax_value( cbo_company_id+"**"+batch_no, 'check_batch_no', '', 'requires/dyeing_production_controller');
			...
			else
			{
				...
				//alert(response[1]);
				//get_php_form_data(response[1], "populate_data_from_batch2", "requires/dyeing_production_controller" );
				get_php_form_data(document.getElementById('cbo_load_unload').value+'_'+response[1]+'_'+batch_no+'_'+response[2]+'_'+response[3], "populate_data_from_batch", "requires/dyeing_production_controller" );
				...
				//show_list_view(response[1]+'_'+document.getElementById('cbo_load_unload').value+'_'+document.getElementById('txt_system_no').value,'show_dtls_list_view','list_container','requires/dyeing_production_controller','');
				//show_list_view(response[1]+'_'+document.getElementById('cbo_load_unload').value+'_'+document.getElementById('txt_system_no').value,'show_dtls_batch_list_view','list_container','requires/dyeing_production_controller','');
				get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/dyeing_production_controller' );
	...
	function fnc_pro_fab_subprocess( operation ) //Save Update Here...
	{
		if(operation==4)// print
		{
			...
			print_report( $('#cbo_company_id').val()+'*'+$('#txt_system_no').val()+'*'+$('#cbo_load_unload').val()+'*'+$('#txt_batch_ID').val()+'*'+report_title, "dyeing_pro_print", "requires/dyeing_production_controller")
			//alert(print_report);
			return;
		}
		...
		if (document.getElementById('cbo_load_unload').value==1) //Loading Here
		{
			...
			{
			var response2=return_global_ajax_value(cbo_company_id+"**"+batch_no+"**"+batch_id+"**"+machine_no,'check_for_shade_matched','','requires/dyeing_production_controller');
			var response2=response2.split("_");
			}
			//alert(1);
			var response=return_global_ajax_value( service_company+"**"+batch_no+"**"+batch_id+"**"+machine_no, 'check_batch_no_for_machine', '', 'requires/dyeing_production_controller');
			var response=response.split("_");
			if(service_source!=3)
			{
			...
			if(operation==0)
			{
				...
				else
				{
					if(yesno==2)
					{
						...
						/*var response3=return_global_ajax_value( process_start_date+"**"+cbo_floor+"**"+machine_no+"**"+update_id, 'machine_load_status', '', 'requires/dyeing_production_controller');
					...
					else if(yesno==1)
					{
						...
						{
							var date_response=trim(return_global_ajax_value( functional_batch+"**"+process_start_date, 'check_process_start_date', '', 'requires/dyeing_production_controller'));
			...
			else
			{
				...
				var data="action=save_update_delete&operation="+operation+get_submitted_data_string('cbo_load_unload*txt_process_start_date*txt_start_hours*txt_start_minutes*cbo_company_id*txt_batch_no*txt_batch_ID*hidden_batch_id*txt_process_id*cbo_machine_name*txt_remarks*txt_ext_id*txt_update_id*cbo_floor*cbo_ltb_btb*txt_water_flow*cbo_yesno*cbo_service_source*cbo_service_company*txt_recevied_chalan*txt_issue_chalan*txt_issue_mst_id*roll_maintained*txt_load_meter*txt_system_no*txt_entry_form_no*hidden_double_dyeing*hidden_result_id*cbo_dyeing_type*txt_reason_id',"../../")+data_all+'&total_row='+row_num;
				  //alert (data);return;
			 	 freeze_window(operation);
			 	 http.open("POST","requires/dyeing_production_controller.php",true);
			 	 http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		...
		if (document.getElementById('cbo_load_unload').value==2) //unloading here
		{
			...
			//var date_time_response=return_global_ajax_value( cbo_company_id+"**"+batch_no+"**"+batch_id+"**"+dyeing_end_date+"**"+dyeing_started_date+"**"+unload_hr_min+"**"+load_hr_min, 'check_date_time', '', 'requires/dyeing_production_controller');
			...
			var load_end_hour_minute_data=return_global_ajax_value( $('#hidden_batch_id').val(), 'get_load_end_hour_minute', '', 'requires/dyeing_production_controller');
			...
			//var end_minute=$('#txt_end_minutes').val()*1;
				...
				{
					var date_response=trim(return_global_ajax_value( functional_batch+"**"+process_start_date, 'check_process_production_date', '', 'requires/dyeing_production_controller'));
					...
					//alert(process_st_date);
						...
						if(production_date!=prod_date && prod_date!="")
						{
							...
							{
							alert("Production Date Must be Same in Functional Batch No "+functional_batch); return;
							}
						}
			...
			var cbo_company_id = $('#cbo_company_id').val();
			var batch_id = $('#hidden_batch_id').val();
			var response=return_global_ajax_value( cbo_company_id+"**"+batch_no+"**"+batch_id+"**"+process_id, 'check_batch_no_load', '', 'requires/dyeing_production_controller');
			var response=response.split("_");
			if(response[0]==0)
				{
					//alert(process_id);
	...
	function fnc_pro_fab_subprocess_response()
	{
		if(http.readyState == 4)
		{
			...
			//show_list_view(document.getElementById('txt_batch_ID').value+'_'+document.getElementById('roll_maintained').value+'_'+document.getElementById('cbo_load_unload').value,'show_fabric_desc_listview','list_fabric_desc_container','requires/dyeing_production_controller','');
			//alert(response[3]);
			show_list_view( response[2]+'_'+response[4]+'_'+response[3]+'_'+document.getElementById('txt_entry_form_no').value,'show_dtls_batch_list_view','list_container','requires/dyeing_production_controller','');
	...
	function load_list_view (str)
	{
		...
		show_list_view(str,'on_change_data','load_unload_container','../fabric_finishing_progress/requires/dyeing_production_controller','');
		...
		else
		{
			...
			document.getElementById('ltb_ltb_caption').innerHTML='BTB LTB';
			$('#ltb_ltb_caption').css('color','blue');
			document.getElementById('production_date_td').innerHTML='Production Date';
	...
	var batch_no=$('#txt_batch_no').val();
	var cbo_company_id = $('#cbo_company_id').val();
	//var response=return_global_ajax_value( cbo_company_id+"**"+str, 'check_batch_no_scan', '', 'requires/dyeing_production_controller');
	//var response=response.split("_");
	check_batch();
	...
	/*if(response[0]==0)
	...
	else
		{
			$('#hidden_batch_id').val(response[1]);
			get_php_form_data(document.getElementById('cbo_load_unload').value+'_'+response[1]+'_'+batch_no+'_'+response[2], "populate_data_from_batch", "requires/dyeing_production_controller" );
			show_list_view(response[1],'show_fabric_desc_listview','list_fabric_desc_container','requires/dyeing_production_controller','');
			get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/dyeing_production_controller' );
			$('#cbo_company_id').focus();
		}*/
	}
...
function openmypage_process()
	{
		...
		var page_link = 'requires/dyeing_production_controller.php?txt_process_id='+txt_process_id+'&action=process_name_popup';
	...
	function openmypage_process_unload()
	{
		var txt_process_id = $('#txt_process_id').val();
		var title = 'Process Name Selection Form';
		var page_link = 'requires/dyeing_production_controller.php?txt_process_id='+txt_process_id+'&action=process_name_popup_unload';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=400px,height=370px,center=1,resize=1,scrolling=0','../');
		emailwindow.onclose=function()
		{
		var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
	...
	function roll_maintain()
	{
		...
		//alert(com);
		get_php_form_data($('#cbo_company_id').val(),'roll_maintained_setting','requires/dyeing_production_controller' );
		var roll_maintained=$('#roll_maintained').val();
		var page_upto=$('#page_upto').val();
	...
	function openmypage_issue_challan()
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/dyeing_production_controller.php?cbo_company_id='+cbo_company_id+'&action=issue_challan_popup','Issue Popup', 'width=880px,height=350px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
			...
			if(issue_id!="")
			{
				get_php_form_data(issue_id, "populate_data_from_data", "requires/dyeing_production_controller");
	...
	function check_issue_challan_scan(str) //Issue Challan
	{
		...
		if(issue_chalan!="")
		{
			var response=return_global_ajax_value( cbo_company_id+"**"+issue_chalan, 'check_issue_challan_no', '', 'requires/dyeing_production_controller');
			var response=response.split("_");
			...
			else
			{
				...
				get_php_form_data(response[1], "populate_data_from_data", "requires/dyeing_production_controller" );
				var hidden_roll_id = $('#txt_roll_id').val();
				show_list_view(document.getElementById('txt_batch_ID').value+'_'+document.getElementById('roll_maintained').value+'_'+document.getElementById('cbo_load_unload').value,'issue_show_fabric_desc_listview','list_fabric_desc_container','requires/dyeing_production_controller','');
	 ...
	 function check_issue_challan_scan(str) //Issue Challan Scan
	{
		...
		if(issue_chalan!="")
		{
			var response=return_global_ajax_value( cbo_company_id+"**"+issue_chalan, 'check_issue_challan_no_scan', '', 'requires/dyeing_production_controller');
			var response=response.split("_");
			if(response[0]==0)
			{
				alert('Issue Challan  not found.');
			...
			else
			{
				...
				get_php_form_data(response[1], "populate_data_from_data", "requires/dyeing_production_controller" );
				//var hidden_roll_id = $('#txt_roll_id').val();
			//	show_list_view(batch_id+'_'+hidden_roll_id+'_'+cbo_company_id,'issue_show_fabric_desc_listview','list_fabric_desc_container','requires/dyeing_production_controller','');
				$('#cbo_service_source').focus();
			}
	...
	function load_batch_working_company(service_company)
	{
		...
		//
			...
			var load_unload=$('#cbo_load_unload').val();
			var hidden_service_company=$('#txt_hidden_service_company').val()*1;
			//alert(service_company+'='+hidden_service_company);
			if(service_source==1)
			{
					if(hidden_service_company!=service_company)
					{
						...
						load_drop_down( 'requires/dyeing_production_controller', hidden_service_company, 'load_drop_floor', 'floor_td' );
					}
					else
					{
...
function open_syspopup(page_link,title)
{
	...
	var load_unload = $("#cbo_load_unload").val();
	var batch_no = $("#txt_batch_no").val();
	page_link='requires/dyeing_production_controller.php?action=sys_popup&company='+company+'&batch_no='+batch_no+'&load_unload='+load_unload+'&system_no='+system_no;
	...
	emailwindow.onclose=function()
	{
		...
		if (sysNumber!="")
		{
			...
			show_list_view( sysNumber[2]+'_'+sysNumber[4]+'_'+sysNumber[1]+'_'+sysNumber[5],'show_dtls_batch_list_view','list_container','requires/dyeing_production_controller','');			//function disable_enable_fields( flds, operation, loop_flds, loop_leng )
...
function fnc_load_data(type_id)
{
		...
		if(type_id==1)
		{
		get_php_form_data($('#cbo_company_id').val()+'**'+$('#cbo_machine_name').val()+'**'+$('#cbo_floor').val(),'load_batch_machine','requires/dyeing_production_controller' );
		}
		/*else
		{
...
function fnc_mc_load()//
{
	...
	if(floor_length==1)
	{
		load_drop_down( 'requires/dyeing_production_controller', document.getElementById('cbo_service_company').value+'**'+document.getElementById('cbo_floor').value, 'load_drop_machine', 'machine_td' );
	}
}
function fnc_reason_name()
	{
		var cbo_company_id = $('#cbo_company_id').val();
		...
		if (form_validation('cbo_company_id*cbo_service_company*txt_batch_no*txt_ext_id','Company*Knitting Company*Batch No*Ext. No')==false)
		{
			return;
		}
		else
		{
			var page_link='requires/dyeing_production_controller.php?cbo_company_id='+cbo_company_id+'&txt_batch_no='+txt_batch_no+'&action=reason_name_popup';
			var title='Reason Info From Library';

			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=380px,height=250px,center=1,resize=1,scrolling=0','');
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0];
				var reason_hdn=this.contentDoc.getElementById("reason_hdn").value;
				if(trim(reason_hdn)!="")
				{
					reason_data=reason_hdn.split("_");
					reason_id=reason_data[0];
					reason_name=reason_data[1];
					freeze_window(5);
					$('#txt_reason_name').val(reason_name);
					$('#txt_reason_id').val(reason_id);
					release_freezing();
				}
			}
		}
...
function fnc_incharge_name()
	{
		...
		if (form_validation('txt_batch_no*cbo_company_id*cbo_service_company','Batch No*Company*Knitting Company')==false)
		{
			return;
		}
		else
		{
			var page_link='requires/dyeing_production_controller.php?cbo_company_id='+cbo_company_id+'&txt_batch_no='+txt_batch_no+'&action=incharge_name_popup';
			...
			emailwindow.onclose=function()
			{
				...
				if(trim(reason_hdn)!="")
				{
					...
					reason_name=reason_data[1];
					freeze_window(5);
					$('#txt_in_charge').val(reason_name);
					$('#txt_in_charge_id').val(reason_id);
					release_freezing();
				}
			}
		}
