...
<script>
	...
	*/
	...
	function openmypage_booking()
	{
		...
		{
			var booking_no  = $(this).children('td').eq(18).html();
	...
	function fnc_grey_roll_receive_from_subcon( operation )
	{
		...
		if(operation==5)
		{
			...
			var report_title=$( "div.form_caption" ).html();
			generate_report_file( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title,'fabric_receive_print2','requires/aop_roll_receive_entry_controller');
			return;
		}
		...
		$("#scanning_tbl").find('tbody tr').each(function()
		{
			...
			if(txtReceiveQty>0)
			{
				...
				var greyUsedQtyHidden=$(this).find('input[name="txtGreyUsedQtyHidden[]"]').val();
				...
				var woorder_qty=$(this).find("td:eq(12)").text();
				var jobNo=$(this).find('input[name="hiddnJobNo[]"]').val();
				var dia=$(this).find("td:eq(7)").text();
				var dtlsId=$(this).find('input[name="dtlsId[]"]').val();
				var bookingDtlsId=$(this).find('input[name="bookingDtlsId[]"]').val();
				var hidIssQty=$(this).find('input[name="hidIssQty[]"]').val();
				var colorTypeId=$(this).find('input[name="colorTypeId[]"]').val();
				var isSales=$(this).find('input[name="isSales[]"]').val();
				var txtserviceIssueNo=$(this).find('input[name="txtserviceIssueNo[]"]').val();
				var txtserviceIssueId=$(this).find('input[name="txtserviceIssueId[]"]').val();
				if(operation==0)
				{
				...
				j++;
				dataString+='&cboProcess_' + j + '=' + cboProcess + '&determinationId_' + j + '=' + determinationId + '&buyerId_' + j + '=' + buyerId + '&txtRate_' + j + '=' + txtRate + '&orderId_' + j + '=' + orderId + '&txtBatchNo_' + j + '=' + txtBatchNo+ '&txtinHouseBatchNo_' + j + '=' + txtInhouseBatchNo + '&jobNo_' + j + '=' + jobNo + '&colorId_' + j + '=' + colorId + '&dtlsId_' + j + '=' + dtlsId + '&txtAmount_' + j + '=' + txtAmount + '&dia_' + j + '=' + dia + '&bodypartId_' + j + '=' + bodypartId + '&txtReceiveQty_' + j + '=' + txtReceiveQty+ '&tr_' + j + '=' + id+ '&currencyId_' + j + '=' + currencyId+ '&exchangeRate_' + j + '=' + exchangeRate+ '&bookingId_' + j + '=' + bookingId+ '&bookingNo_' + j + '=' + bookingNo+ '&workorderNo_' + j + '=' + woorder_qty+ '&finDia_' + j + '=' + finDia+ '&finGsm_' + j + '=' + finGsm+ '&bookWithoutOrder_' + j + '=' + bookWithoutOrder+ '&greyUsed_' + j + '=' + greyUsed + '&hidIssQty_' + j + '=' + hidIssQty + '&bookingDtlsId_' + j + '=' + bookingDtlsId + '&colorTypeId_' + j + '=' + colorTypeId + '&greyUsedQtyHidden_' + j + '=' + greyUsedQtyHidden+ '&isSales_' + j + '=' + isSales+ '&txtserviceIssueNo_' + j + '=' + txtserviceIssueNo+ '&txtserviceIssueId_' + j + '=' + txtserviceIssueId;
			}
		});

		if('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][92]);?>'){
				if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][92]);?>','<? echo implode('*',$_SESSION['logic_erp']['field_message'][92]);?>')==false)
				{
		...
		var data="action=save_update_delete&operation="+operation+'&tot_row='+j+get_submitted_data_string('txt_receive_no*cbo_company_id*cbo_service_source*cbo_service_company*txt_boe_mushak_challan_no*txt_boe_mushak_challan_date*txt_receive_date*txt_receive_challan*update_id',"../../")+dataString;
		//alert(data)
	...
	function fnc_reset_form()
	{
		...
		var html='<tr id="tr_1" align="center" valign="middle"><td width="25" id="sl_1"></td><td width="70" id="batchNo_1"><input type="text" id="txtBatchNo_1" name="txtBatchNo[]"  style=" width:60px" class="text_boxes"/></td><td width="70" id="inHouseBatchNo_1" style="display:none"><input type="text" id="txtinHouseBatchNo_1" name="txtinHouseBatchNo[]"  style=" width:60px" class="text_boxes"/></td><td style="word-break:break-all;" width="80" id="bodyPart_1"></td><td style="word-break:break-all;" width="80" id="colorTypeNO_1"></td><td style="word-break:break-all;" width="120" id="cons_1" align="left"></td><td style="word-break:break-all;" width="50" id="dia_1"></td><td style="word-break:break-all;" width="50" id="gsm_1"></td><td style="word-break:break-all;" width="70" id="color_1"></td><td style="word-break:break-all;" width="70" id="fabColor_1"></td><td width="120" align="right" id=""><? echo create_drop_down( "cboProcess_1", 120, $conversion_cost_head_array,"", 1, "-- Select Process --", "", "","","","","","","","","cboProcess[]" );?></td><td style="word-break:break-all;" width="60" id="woQty_1"></td><td width="60" align="center" id=""><input type="text" id="totalreceiveQty_1" name="totalreceiveQty[]"  style=" width:40px" class="text_boxes_numeric" readonly/></td><td width="60" align="center" id=""><input type="text" id="txtReceiveQty_1" name="txtReceiveQty[]"  style=" width:40px" class="text_boxes_numeric"/></td><td width="60" align="center" id=""><input type="text" id="txtRate_1" name="txtRate[]"  style=" width:40px" class="text_boxes_numeric" readonly/></td><td width="70" align="center" id=""><input type="text" id="txtAmount_1" name="txtAmount[]"  style=" width:55px" class="text_boxes_numeric"/></td><td width="70" align="center" id=""><input type="text" id="txtgreyUsed_1" name="txtgreyUsed[]"  style=" width:55px" class="text_boxes_numeric"/><input type="hidden" id="totalgreyusedQty_1" name="totalgreyusedQty[]" class="text_boxes_numeric" readonly/></td><td style="word-break:break-all;" width="90" id="bookingNo_1" align="left"></td><td style="word-break:break-all;" width="60" id="buyer_1"></td><td style="word-break:break-all;" width="80" id="job_1"></td><td style="word-break:break-all;" width="100" id="order_1" align="left"></td><td style="word-break:break-all;" width="100" id="internalRef_1" align="left"></td><td style="word-break:break-all;" width="" id="currency_1" align="left"><? echo create_drop_down( "currencyId_1", 65, $currency,"", 1, "Select", "", "","","","","","","","","currencyId[]" ); ?><input type="hidden" name="hiddnJobNo[]" id="hiddnJobNo_<? echo $i; ?>" value=""/><input type="hidden" name="progBookPiId[]" id="progBookPiId_1"/><input type="hidden" name="orderId[]" id="orderId_1"/><input type="hidden" name="colorId[]" id="colorId_1"/><input type="hidden" name="dtlsId[]" id="dtlsId_1"/><input type="hidden" name="bodypartId[]" id="bodypartId_1"/><input type="hidden" name="colorTypeId[]" id="colorTypeId_1"/><input type="hidden" name="buyerId[]" id="buyerId_1"/><input type="hidden" name="determinationId[]" id="determinationId_1"/><input type="hidden" name="currencyId[]" id="currencyId_1"/><input type="hidden" name="exchangeRate[]" id="exchangeRate_1"/><input type="hidden" name="txtBookingNo[]" id="txtBookingNo_1"/><input type="hidden" name="finDia[]" id="finDia_1"/><input type="hidden" name="finGsm[]" id="finGsm_1"/></td></tr>';
	...
	function validation_grey_used(id)
	{
		var txt_grey_used_qnty = $("#txtgreyUsed_"+id).attr("placeholder")*1;
		var tot_grey_used = $("#totalgreyusedQty_"+id).val()*1;
		var curr_greyused = $("#txtgreyUsed_"+id).val()*1;
		var hidden_curr_grey_used = $("#txtGreyUsedQtyHidden_"+id).val()*1;
			
		var txt_variable_checked= $('#txt_variable_checked').val()*1;
		var txt_variable_percent= $('#txt_variable_percent').val()*1;

		// for grey used
		if(hidden_curr_grey_used != 0)
		{
			var totRcvGreyUsed = (tot_grey_used + curr_greyused ) - hidden_curr_grey_used ;
		}else
		{
			var totRcvGreyUsed =tot_grey_used + curr_greyused;
		}

		if($("#cboProcess_"+id).val()*1!==35)
		{
			if(totRcvGreyUsed > txt_grey_used_qnty)
			{
				//alert(totRcvGreyUsed +'>'+ txt_grey_used_qnty);
				$("#txtgreyUsed_"+id).val($("#txtGreyUsedQtyHidden_"+id).val()*1);
			}

		}
		else if($("#cboProcess_"+id).val()*1==35)
		{
			/*if(txt_variable_checked==1) // comment for crm 22738
			{
				var greyused_qnty_perc=(txt_grey_used_qnty*txt_variable_percent)/100;
				// alert(totRcvGreyUsed +'>'+ txt_grey_used_qnty+greyused_qnty_perc+' =perc');
				if(totRcvGreyUsed > txt_grey_used_qnty+greyused_qnty_perc)
				{
			...
			{
				// alert(totRcvGreyUsed +'>'+ txt_grey_used_qnty+' =F');
				if(totRcvGreyUsed > txt_grey_used_qnty)
				{
					$("#txtReceiveQty_"+id).val($("#txtReceiveQtyHidden_"+id).val()*1);
					$("#txtgreyUsed_"+id).val($("#txtGreyUsedQtyHidden_"+id).val()*1);
	...
	{	
		...
		// alert($("#cboProcess_"+id).val());35
		if($("#cboProcess_"+id).val()*1!==35)
		{
			$("#hidIssQty_"+id).val(issue_qnty*1);
		...
		{
			// alert(txt_variable_checked);1
			if(txt_variable_checked==1)
			{
				var issue_qnty_perc=(issue_qnty*txt_variable_percent)/100;
				//alert(totRcv +'>'+ issue_qnty+'+'+issue_qnty_perc);
				var iss_qty=issue_qnty+issue_qnty_perc;
				$("#hidIssQty_"+id).val(iss_qty*1);
...
<body onLoad="set_hotkey();">
    <div align="center" style="width:100%;">
        ...
        <form name="rollscanning_1" id="rollscanning_1"  autocomplete="off"  >
            <fieldset style="width:910px;">
                ...
                <table cellpadding="0" cellspacing="2" width="900">
                    ...
                    <tr>
                        ...
                        <td>
                            <? 
                                echo create_drop_down( "cbo_company_id", 152, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name","id,company_name", 1, "-- Select --", 0, "check_variable_setting();",0 );
                        ...
                        <td>
							<?
                                echo create_drop_down( "cbo_service_source", 152, $knitting_source, "", 1, "-- Select --", 0, "load_drop_down( 'requires/fabric_service_receive_controller',this.value+'**'+$('#cbo_company_id').val(),'load_drop_down_knitting_com','dyeing_company_td' );","","1,3" );
                            ?>
                        </td>
                    ...
                    <tr>
                        ...
                        <td>
                        	<input type="text" name="txt_woorder_no" id="txt_woorder_no" class="text_boxes" style="width:140px;" placeholder="Browse/scan" onDblClick="openmypage_booking();"  />
			...
			<br>
			<fieldset style="width:1600px;text-align:left">
				<style>
                    #scanning_tbl tr td
                    {
				...
				<table cellpadding="0" width="1760" cellspacing="0" border="1" id="scanning_tbl_top" class="rpt_table" rules="all">
                    <thead>
                    	<th width="25">SL</th>
                        <th width="100">Fso Issue Challan</th>
                 ...
                 <div style="width:1780px; max-height:250px; overflow-y:scroll" align="left">
                 	<table cellpadding="0" cellspacing="0" width="1760" border="1" id="scanning_tbl" rules="all" class="rpt_table">
                    	<tbody id="scanning_tbody">
                        	<tr id="tr_1" align="center" valign="middle">
                                <td width="25" id="sl_1"></td>
								<td style="word-break:break-all;" width="100" id="serviceIssueNo_1"></td>
                                ...
                                <td style="word-break:break-all;" width="80" id="colorTypeNO_1"></td>
                                ...
                                <td width="70" align="center" id=""><input type="text" id="txtgreyUsed_1" name="txtgreyUsed[]"  style=" width:55px" class="text_boxes_numeric" placeholder=""/><input type="hidden" name="txtGreyUsedQtyHidden_1" name="txtGreyUsedQtyHidden[]"/><input type="hidden" id="totalgreyusedQty_1" name="totalgreyusedQty[]"  style=" width:40px" class="text_boxes_numeric" readonly/></td>
                                ...
                                <td style="word-break:break-all;" width="100" id="currency_1" align="left">
									<? 
                                    ...
                                    <input type="hidden" name="hidIssQty[]" id="hidIssQty_1"/>
                                    <input type="hidden" name="privCurrentQty[]" id="privCurrentQty_1"/>
                                    <input type="hidden" name="batchtQty[]" id="batchtQty_1"/>
                                    <input type="hidden" name="txtserviceIssueNo[]" id="txtserviceIssueNo_1"/>
                                    <input type="hidden" name="txtserviceIssueId[]" id="txtserviceIssueId_1"/>
                                </td>
                            </tr>
                        </tbody>
                	</table>
                ...
                <table width="1200" cellpadding="0" cellspacing="0" border="1" id="" rules="all">
                    <tr>
                        <td align="center" class="button_container">
                            ...
                            ?>
							<input type="button" id="print_report" name="print_report" style="width:60px" class="formbuttonplasminus" value="Print 2" onClick="fnc_grey_roll_receive_from_subcon(5);" />
                        </td>
                    </tr>  
                </table>
			</fieldset>
