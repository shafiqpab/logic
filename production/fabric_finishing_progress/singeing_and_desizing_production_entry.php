<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Singeing and Desizing Production
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Singeing and Desizing Production Entry Info","../../", 1, 1, "",'1','');
...
function scan_batchnumber(str)
{
	...
	//var response=return_global_ajax_value( cbo_company_id+"**"+str, 'check_batch_no_scan', '', 'requires/singeing_and_desizing_production_entry_controller');
	...
	get_php_form_data(response[1]+'_'+str, "populate_data_from_batch", "requires/singeing_and_desizing_production_entry_controller" );
	var cbo_company_id = $('#cbo_company_id').val();
	
	show_list_view(response[1]+'_'+cbo_company_id,'show_fabric_desc_listview','list_fabric_desc_container','requires/singeing_and_desizing_production_entry_controller','');
	get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/singeing_and_desizing_production_entry_controller' );
	var roll_variable = $('#roll_maintained').val();
	show_list_view( response[1]+"_"+roll_variable,'show_dtls_list_view','list_container','requires/singeing_and_desizing_production_entry_controller','');
	//$('#txt_batch_no').focus();
...
function openmypage_batchnum()
{
		...
		$("#txt_batch_no").val('');
		var page_link='requires/singeing_and_desizing_production_entry_controller.php?cbo_company_id='+cbo_company_id+'&action=batch_number_popup';
		...
		//alert(batch_ids[1])
			if(batch_ids!="")
			{
				...
				var response=return_global_ajax_value( batch_no, 'check_batch_deying', '', 'requires/singeing_and_desizing_production_entry_controller');
				var response=response.split("_");
				//alert(response[0]);
				// if(response[0]==0)
				// {
				...
				// 	return;
				// }
				get_php_form_data(batch_ids[0]+'_'+batch_ids[2]+'_'+batch_no+'_'+batch_ids[2], "populate_data_from_batch", "requires/singeing_and_desizing_production_entry_controller" );
				check_re_sltting();
			
				var cbo_company_id = $('#cbo_company_id').val();
				show_list_view(batch_ids[0]+'_'+cbo_company_id,'show_fabric_desc_listview','list_fabric_desc_container','requires/singeing_and_desizing_production_entry_controller','');
				var roll_variable = $('#roll_maintained').val();
				show_list_view( batch_ids[0]+"_"+roll_variable+'_'+batch_ids[2],'show_dtls_list_view','list_container','requires/singeing_and_desizing_production_entry_controller','');
...
function openmypage_servicebook()
{
	...
	var page_link='requires/singeing_and_desizing_production_entry_controller.php?cbo_company_id='+cbo_company_id+'&supplier_id='+supplier_id+'&process_id='+process_id+'&action=service_booking_popup';
...
function check_batch()
{
	...
	if(batch_no!="")
	{
		...
		var response_res=return_global_ajax_value( batch_no, 'check_batch_deying_result', '', 'requires/singeing_and_desizing_production_entry_controller');
		...
		var response=return_global_ajax_value( cbo_company_id+"**"+batch_no, 'check_batch_no', '', 'requires/singeing_and_desizing_production_entry_controller');
		var response=response.split("_");
		if(response[0]==0)
		{
		...
		else
		{
			...
			//get_php_form_data(response[1], "populate_data_from_batch", "requires/singeing_and_desizing_production_entry_controller" );
			var re_slitting_no=$('#txt_reslitting_no').val();
			get_php_form_data(response[1]+'_'+response[2]+'_'+batch_no+'_'+re_slitting_no, "populate_data_from_batch", "requires/singeing_and_desizing_production_entry_controller" );
			check_re_sltting();
			var re_slitting_no=$('#txt_reslitting_no').val();
			var cbo_company_id = $('#cbo_company_id').val();
			var txt_batch_id= $('#txt_batch_ID').val();
			show_list_view(txt_batch_id+'_'+cbo_company_id+'_'+re_slitting_no,'show_fabric_desc_listview','list_fabric_desc_container','requires/singeing_and_desizing_production_entry_controller','');
			var roll_variable = $('#roll_maintained').val();
			show_list_view( txt_batch_id+"_"+roll_variable+'_'+response[2],'show_dtls_list_view','list_container','requires/singeing_and_desizing_production_entry_controller','');
			get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/singeing_and_desizing_production_entry_controller' );
...
function fnc_pro_fab_subprocess( operation )
{
    ...
    if((page_upto*1==3 || page_upto*1>3) && roll_maintained*1==1  )
	{
		for (var i=1; i<=row_num; i++)
		{
			...
			data_all+=get_submitted_data_string('txtconscomp_'+i+'*txtgsm_'+i+'*txtbodypart_'+i+'*txtdiawidth_'+i+'*txtsalesqnty_'+i+'*txtprodid_'+i+'*updateiddtls_'+i+'*txtdiawidthID_'+i+'*txtroll_'+i+'*txtproductionqty_'+i+'*txtproductionqty_'+i+'*txtheat_set_density_'+i+'*txtyarncount_'+i+'*rollid_'+i+'*checkRow_'+i+'*txtrate_'+i+'*txtamount_'+i+'*txtknitting_density_'+i,"../../",i);
		}
	}
	else
	{		
...
function fnc_pro_fab_subprocess_response()
{
	if(http.readyState == 4) 
	{
		...
		else if(reponse[0]==0 || reponse[0]==1)
		{
			...
			//var cbo_company_id = $('#cbo_company_id').val();
			//var roll_maintained=$('#roll_maintained').val();
			//show_list_view(batch_id+'_'+cbo_company_id,'show_fabric_desc_listview','list_fabric_desc_container','requires/singeing_and_desizing_production_entry_controller','');

			show_list_view( batch_id+"_"+reponse[1]+"_"+is_sales,'show_dtls_list_view','list_container','requires/singeing_and_desizing_production_entry_controller','');
	...
	function openmypage_process()
	{
		...
		var page_link = 'requires/singeing_and_desizing_production_entry_controller.php?txt_process_id='+txt_process_id+'&action=process_name_popup';
	...
	function openmypage_issue_challan()
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/singeing_and_desizing_production_entry_controller.php?cbo_company_id='+cbo_company_id+'&action=issue_challan_popup','Issue Popup', 'width=780px,height=350px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
			...
			if(issue_id!="")
			{
				...
				get_php_form_data(issue_id, "populate_data_from_data", "requires/singeing_and_desizing_production_entry_controller");
				var hidden_roll_id = $('#txt_roll_id').val();
				show_list_view(batch_id+'_'+hidden_roll_id+'_'+cbo_company_id,'show_fabric_issue_listview','list_fabric_desc_container','requires/singeing_and_desizing_production_entry_controller','');
	...
	function check_issue_challan_scan(str) //Issue Challan
	{
		...
		if(issue_chalan!="")
		{
			var response=return_global_ajax_value( cbo_company_id+"**"+issue_chalan, 'check_issue_challan_no_scan', '', 'requires/singeing_and_desizing_production_entry_controller');
			var response=response.split("_");			
			if(response[0]==0)
			{
			...
			else
			{
				...
				get_php_form_data(response[1], "populate_data_from_data", "requires/singeing_and_desizing_production_entry_controller" );
				var hidden_roll_id = $('#txt_roll_id').val();
				show_list_view(batch_id+'_'+hidden_roll_id+'_'+cbo_company_id,'show_fabric_issue_listview','list_fabric_desc_container','requires/singeing_and_desizing_production_entry_controller','');
			}
	...
	function check_issue_challan() //Issue Challan
	{
		...
		if(issue_chalan!="")
		{
			var response=return_global_ajax_value( cbo_company_id+"**"+issue_chalan, 'check_issue_challan_no', '', 'requires/singeing_and_desizing_production_entry_controller');
			var response=response.split("_");
	...
	{ 
		...
		get_php_form_data($('#cbo_company_id').val(),'roll_maintained','requires/singeing_and_desizing_production_entry_controller' );
		var roll_maintained=$('#roll_maintained').val();
		var page_upto=$('#page_upto').val();
...
function check_re_sltting()
	{
		...
		get_php_form_data(batch_id+'_'+txt_reslitting_no, "populate_restenter_from_data", "requires/singeing_and_desizing_production_entry_controller" );
            ...
            <tr>
                <td width="21%" valign="top">
                    ...
                    <table cellpadding="0" cellspacing="2" width="100%" id="main_tbl">
                        ...
                        <tr> 
							<td width="90">Sales Number</td>
                             <td width="110"> <input type="text" name="txt_sales_number" id="txt_sales_number" class="text_boxes" style="width:100px;" readonly  />
                            </td> 
                        </tr>
						<tr> 
                            <td width="">Re Sliting No.</td>
                            <td id="re_stenter_td">
                                <input type="text" name="txt_reslitting_no" id="txt_reslitting_no" class="text_boxes_numeric" style="width:122px;"  value="0" onChange="check_re_sltting();" readonly /> 
                        ...
                        <tr>
                         <td class="must_entry_caption" width="130">Company</td>
                            <td>
                                <?
                                   //load_drop_down('requires/singeing_and_desizing_production_entry_controller', this.value, 'load_drop_floor', 'floor_td' );
                        ...
                        <td>
							<?
                                echo create_drop_down( "cbo_service_source", 135, $knitting_source, "", 1, "-- Select --", $selected, "load_drop_down( 'requires/singeing_and_desizing_production_entry_controller',this.value+'**'+$('#cbo_company_id').val(),'load_drop_down_knitting_com','dyeing_company_td' );change_css(this.value);","","1,3" );
                            ?>
                        </td>
                        ...
                        <tr>
                            ...
                            <td>
                               <?
								   echo create_drop_down( "cbo_sub_process", 135, $conversion_cost_head_array,"", 0, "-- Select --", 0, "","","463" );
                               ?>
                        ...
                        <tr>
						 ...
						 <tr>
                            <td class="">Pechar In</td>
                            <td>
                                <input type="text" name="txt_pechar_in" id="txt_pechar_in" class="text_boxes_numeric" style="width:30px;" />Pechar Out <input type="text" name="txt_pechar_out" id="txt_pechar_out" class="text_boxes_numeric" style="width:30px;"   />
                            </td>
                        </tr>
						<tr>
                            <td class="">Pinning</td>
                            <td>
                                <input type="text" name="txt_feed" id="txt_feed" class="text_boxes_numeric" style="width:30px;" />Speed(M/Min) <input type="text" name="txt_steam" id="txt_steam" class="text_boxes_numeric" style="width:30px;"   />
                            </td>
                        </tr>
                           <td>Floor</td>
                            <td id="floor_td">
                                     ...
                                     <table width="940" align="left" class="rpt_table" rules="all" id="tbl_item_details">
                                        <thead>
                                            <tr>
												...
												<th width="65">Knitting Density</th>
                                                <th width="65">Heat Set Density</th>
                                       ...
                                       <tbody id="list_fabric_desc_container">
										...
										<td>
                                            <input type="text" name="txtknitting_density_1" id="txtknitting_density_1" class="text_boxes_numeric" style="width:65px;" />
                                          </td>
                                                                          
                                          <td><input type="text" name="txtheat_set_density_<? echo $i; ?>" id="txtheat_set_density_<? echo $i; ?>" class="text_boxes_numeric" style="width:50px;" value="<? echo number_format($row[csf('batch_qnty')],2); ?>" disabled/>
                                        ...
                                        <td><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes_numeric" style="width:35px;" value="<? echo $row[csf('roll_no')];?>"/>
										 ...
										 <td><input type="text" name="txtsalesqnty_<? echo $i; ?>" id="txtsalesqnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:55px;"  value="<? echo $lot;?>" disabled /></td>
                                        ...
                                        <td><input type="text" name="txtamount_1" id="txtamount_1" class="text_boxes_numeric" style="width:70px;"  readonly/> 
									
										<input type="hidden" name="txtyarncount_<? echo $i; ?>" id="txtyarncount_<? echo $i; ?>" class="text_boxes_numeric" style="width:50px;"  value="<? echo $yarn_count_value;?>"  disabled />
										<input type="hidden" name="txtbrand_<? echo $i; ?>" id="txtbrand_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px;"  value="<? echo $brand_value;?>"  disabled/>
									</td>
                						</tr>
                                       </tbody>
                                  </table>
