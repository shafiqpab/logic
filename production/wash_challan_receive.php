...
function openmypage_sysNo()
{
	...
	var page_link = 'requires/wash_challan_receive_controller.php?cbo_company_name='+cbo_company_name+'&action=challan_no_popup';
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				get_php_form_data(all_data[0], "populate_data_from_challan_popup", "requires/wash_challan_receive_controller" );
				
				var delivery_basis=$('#delivery_basis').val();
				if(delivery_basis==3)
				{
					var bundle_nos=return_global_ajax_value(all_data[0], 'bundle_nos_update', '', 'requires/wash_challan_receive_controller');
					//alert(bundle_nos);
					var response_data=return_global_ajax_value(trim(bundle_nos)+"**0**"+all_data[0]+"**"+cbo_company_name, 'populate_bundle_data_update', '', 'requires/wash_challan_receive_controller');
				...
				{
					show_list_view(all_data[1],'show_country_listview','list_view_country','requires/wash_challan_receive_controller','');
					show_list_view(all_data[0],'show_dtls_listview','printing_production_list_view','requires/wash_challan_receive_controller','');
				}
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				get_php_form_data(po_id+'**'+item_id+'**'+$('#cbo_embel_name').val()+'**'+country_id, "populate_data_from_search_popup", "requires/wash_challan_receive_controller" );
				...
				else
				{
					...
					get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+$("#cbo_embel_name").val()+'**'+country_id, "color_and_size_level", "requires/wash_challan_receive_controller" ); 
				}
				
				show_list_view(po_id,'show_country_listview','list_view_country','requires/wash_challan_receive_controller','');	
...
function generate_report_file(data,action,page)
	{
		...
		window.open("requires/wash_challan_receive_controller.php?data=" + data+'&action='+action, true );
	}
...
function fnc_issue_print_embroidery_entry(operation)
{
	...
	var working_company_mandatory=return_global_ajax_value(company_id, 'load_variable_settings_for_working_company', '', 'requires/wash_challan_receive_controller');
	...
	if(operation==4)
	{
		...
		generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val()+'*'+report_title, 'emblishment_issue_print', 'requires/wash_challan_receive_controller');
		return;
	}
		...
		if(delivery_basis==3)
		{
			...
			$("#tbl_details").find('tbody tr').each(function()
			{
				...
				try 
				{
					...
					dataString+='&bundleNo_' + j + '=' + bundleNo + '&orderId_' + j + '=' + orderId + '&gmtsitemId_' + j + '=' + gmtsitemId + '&countryId_' + j + '=' + countryId + '&colorId_' + j + '=' + colorId + '&sizeId_' + j + '=' + sizeId + '&colorSizeId_' + j + '=' + colorSizeId + '&qty_' + j + '=' + qty + '&dtlsId_' + j + '=' + dtlsId + '&rejectQty_' + j + '=' + rejectQty + '&cutNo_' + j + '=' + cutNo;
				}
				catch(e) 
				{
 	...
 	if(http.readyState == 4) 
	{
		...
		else if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
		{
			...
			else
			{
				...
				show_list_view(reponse[1],'show_dtls_listview','printing_production_list_view','requires/wash_challan_receive_controller','');
...
function put_country_data(delivery_id, mst_id, country_id, item_id,order_id)
{
	...
	get_php_form_data(delivery_id+'**'+item_id+'**'+$('#cbo_embel_name').val()+'**'+country_id+'**'+mst_id, "populate_data_from_search_popup", "requires/wash_challan_receive_controller" );
...
function openmypage_bundle(page_link,title)
{
	...
	{
		var challan_duplicate=return_ajax_request_value( bundle_no+"__"+$('#cbo_company_name').val(),"challan_duplicate_check", "requires/wash_challan_receive_controller");
		...
		else
		{
			...
			if( (tot_row*1) <1)
				get_php_form_data(txt_bundle_no,'load_mst_data','requires/wash_challan_receive_controller');	
		}
		$('#txt_bundle_no').val('');
	}
...
function create_row(bundle_nos)
{
	...
	var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"****"+$('#cbo_company_name').val(), 'populate_bundle_data', '', 'requires/wash_challan_receive_controller');
	$('#tbl_details tbody').prepend(response_data);
	var tot_row=$('#tbl_details tbody tr').length; 
	$('#txt_tot_row').val(tot_row);
	release_freezing();
...
function openmypage_issue_challan()
	{
		...
		var delivery_basis=$('#delivery_basis').val();
		var cbo_company_name=$('#cbo_company_name').val();		
		var page_link = 'requires/wash_challan_receive_controller.php?delivery_basis='+delivery_basis+'&cbo_company_name='+cbo_company_name+'&action=isssue_challan_no_popup';
		...
		emailwindow.onclose=function()
		{
			...
			{ 
					...
					if(delivery_basis==3)
					{
						...
						var bundle_nos=return_global_ajax_value(all_data[0], 'bundle_nos', '', 'requires/wash_challan_receive_controller');
						if (bundle_nos!="")
						{ 
							//get_php_form_data(hidden_bundle_nos,'load_mst_data','requires/wash_challan_receive_controller');
							$('#txt_tot_row').val(0);
							$('#tbl_details tbody tr').remove();
					...
					{
						show_list_view(all_data[0],'show_country_listview','list_view_country','requires/wash_challan_receive_controller','');	
						release_freezing();
					}
				}
 ...
 function scan_challan_no(chanal_no)
 {
	...
	if(delivery_basis==3)
	{
		...
		{ 
			...
			var challan_id=return_global_ajax_value(chanal_no, 'get_challan_id', '', 'requires/wash_challan_receive_controller');
			var bundle_nos=return_global_ajax_value(challan_id, 'bundle_nos', '', 'requires/wash_challan_receive_controller');

			get_php_form_data(bundle_nos,'load_mst_data','requires/wash_challan_receive_controller');
			create_row(bundle_nos);
	...
	{
	var response_data=return_global_ajax_value(chanal_no+"**"+cbo_cut_panel_basis, 'check_bundle_data', '', 'requires/wash_challan_receive_controller'); 
	...
	show_list_view(all_data[3],'show_dtls_listview','order_list_view','requires/wash_challan_receive_controller','');
	}
 }
            ...
            <fieldset>
                <table width="100%">
                    ...
                    <tr>
                        ...
                        <td>
                            <? 
                            	echo create_drop_down( "cbo_company_name", 180, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "get_php_form_data(this.value, 'load_variable_settings', 'requires/wash_challan_receive_controller'); working_com_fnc(); load_html();" );//get_php_form_data(this.value,'load_variable_settings_for_working_company','requires/wash_challan_receive_controller');
                            ?>
                     ...
                     <tr>
                         ...
                         <td id="embel_name_td">
                             <? 
							 	echo create_drop_down( "cbo_embel_name", 180, $emblishment_name_array,"", 1, "-- Select Embel.Name --", $selected, "",1,3 );
                             ?>
                         </td>
                         <td class="must_entry_caption">Embel. Type</td>
                         <td id="embro_type_td" width="170">
                              <? 
								echo create_drop_down( "cbo_embel_type", 180, $emblishment_wash_type,"", 1, "--- Select Printing ---", $selected, "",1 );  
                             ?>
                         </td>
                    ...
                    <tr>
                     ...
                     <td id="working_company">Working Company</td>
                        <td>
                        	<? 
								echo create_drop_down( "cbo_working_company_name", 180, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select --",$selected,"load_drop_down( 'requires/wash_challan_receive_controller', $('#cbo_working_company_name').val(), 'load_drop_down_working_location', 'working_location_td' );" );
                            ?>
                        </td>
               	...
               	<table cellpadding="0" cellspacing="1" width="100%">
               		<tr>
                        <td align="center" colspan="9" valign="middle" class="button_container">
                            ...
                            <input type="button" id="btn_print" name="btn_print"   style="width:120px;"  class="formbutton" value="Print3"  onClick="generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val(),'wash_receive_challan_print','requires/print_embro_bundle_receive_controller');"/>
                        </td>
                        <td>&nbsp;</td>				
                    </tr>
               	</table>
