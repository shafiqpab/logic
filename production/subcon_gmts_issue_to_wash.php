<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create SubCon Garments Issue to Wash
...
<script>
	...
	{ 
		...
		emailwindow.onclose=function()
		{
			...
			get_php_form_data(company_id,'load_variable_settings','requires/subcon_gmts_issue_to_wash_controller');
			...
			if (po_id)
			{
				...
				get_php_form_data(po_id+'**'+item_id+'**'+$('#cbo_embel_name').val(), "populate_data_from_search_popup", "requires/subcon_gmts_issue_to_wash_controller" );

				get_php_form_data($('#hidden_po_break_down_id').val()+'**'+$('#cbo_item_name').val()+'**'+$('#sewing_production_variable').val()+'**'+$('#styleOrOrderWisw').val()+'**'+$('#cbo_embel_name').val(), 'color_and_size_level', 'requires/subcon_gmts_issue_to_wash_controller' ); 
	...
	function fnc_issue_print_embroidery_entry(operation)
	{
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+report_title+'*'+$("#sewing_production_variable").val(), "wash_issue_print", "requires/subcon_gmts_issue_to_wash_controller" ) 
			return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{

			if ( form_validation('cbo_company_name*cbo_location_name*txt_issue_date*cbo_emb_company*cbo_wash_location*txt_order_no*txt_issue_qty','Company Name*PO Location*Issue Date*Wash Company*Wash Location*Order No*Issue Quantity')==false )
			{
				return;
			}
			else
			{
				...
				let data="action=save_update_delete&operation="+operation+"&colorIDvalue="+colorIDvalue+get_submitted_data_string('txt_system_no*txt_system_id*cbo_company_name*garments_nature*cbo_location_name*txt_issue_date*cbo_emb_company*cbo_wash_location*txt_remark*hidden_po_break_down_id*txt_issue_qty*cbo_item_name*cbo_party_name*txt_style_no*txt_job_no*txt_delivery_date*txt_cut_qty*txt_sewing_qty*txt_cum_issue_qty*sewing_production_variable*hidden_colorSizeID*yet_to_issue*txt_mst_id',"../");
				//alert (data);return;
				freeze_window(operation);
				http.open("POST","requires/subcon_gmts_issue_to_wash_controller.php",true);
	...
	function fnc_issue_print_embroidery_Reply_info()
	{
		if(http.readyState == 4)
		{
			...
			show_list_view(system_mst_id,'show_dtls_listview_from_sys_popup','printing_production_list_view','requires/subcon_gmts_issue_to_wash_controller','');
			setFilterGrid("tbl_search",-1);
			reset_form('','','txt_issue_qty');
			set_button_status(0, permission, 'fnc_issue_print_embroidery_entry',1,1);
			get_php_form_data(po_id+'**'+$("#cbo_item_name").val()+'**'+$('#cbo_embel_name').val()+'**'+country_id, "populate_data_from_search_popup", "requires/subcon_gmts_issue_to_wash_controller" );

			if(variableSettings!=1) {
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+$("#cbo_embel_name").val()+'**'+country_id, "color_and_size_level", "requires/subcon_gmts_issue_to_wash_controller" );
			}
			else
			{
				$("#txt_issue_qty").removeAttr("readonly"); 
			...
			{ 
				...
				reset_form('washoutput_1','','','txt_issue_date,<?=$date?>','childFormReset()')
			}else{
	...
	{
		//alert(data); return;
		get_php_form_data(data,'populate_issue_form_data','requires/subcon_gmts_issue_to_wash_controller');
	}
	...
	function issue_id_popup()
	{
		...
		let page_link = 'requires/subcon_gmts_issue_to_wash_controller.php?action=system_number_popup&company_id=' + company_id+'&party_name='+ party_name;
		...
		emailwindow.onclose=function()
		{
			...
			get_php_form_data(responseDataArr[0], "populate_mst_form_data", "requires/subcon_gmts_issue_to_wash_controller" );			
			
			show_list_view(responseDataArr[0],'show_dtls_listview_from_sys_popup','printing_production_list_view','requires/subcon_gmts_issue_to_wash_controller','');
			$("#cbo_company_name").attr("disabled","disabled");
			$("#cbo_location_name").attr("disabled","disabled");
			$("#cbo_party_name").attr("disabled","disabled");
			setFilterGrid("tbl_search",-1);
		...
		<div style="width:1010px; float:left" align="center">
			<form name="washoutput_1" id="washoutput_1" autocomplete="off">
				<fieldset style="width:1010px;">
					<legend>SubCon Gmts. Issue to Wash</legend>
					<fieldset>
						<table width="1000px" border="0">
							...
							<tr>
								...
								<td width="210">
									...
									echo create_drop_down("cbo_company_name", 170, $company_sql, "id,company_name", 1, "-- Select Company --", '', "load_drop_down( 'requires/subcon_gmts_issue_to_wash_controller', this.value, 'load_drop_down_po_location', 'location_td' );load_drop_down( 'requires/subcon_gmts_issue_to_wash_controller', this.value, 'load_drop_down_party_name', 'party_td' );", 0); ?>
								...
								<fieldset>
									...
									<table cellpadding="0" cellspacing="2" width="100%">
										<tr>
											...
											<td>
												<input name="txt_order_no" id="txt_order_no" placeholder="Double Click to Search" onDblClick="openmypage('requires/subcon_gmts_issue_to_wash_controller.php?action=order_popup&company='+$('#cbo_company_name').val()+'&garments_nature='+$('#garments_nature').val()+'&hidden_variable_cntl='+$('#hidden_variable_cntl').val()+'&hidden_preceding_process='+$('#hidden_preceding_process').val()+'&sewing_production_variable='+$('#sewing_production_variable').val()+'&buyer_id='+$('#cbo_party_name').val(),'Order Search')" class="text_boxes" style="width:150px " readonly />
					...
					<table cellpadding="0" cellspacing="1" width="100%">
						<tr>
							<td align="center" colspan="6" valign="middle" class="button_container">
								<?
									echo load_submit_buttons($permission, "fnc_issue_print_embroidery_entry", 0, 1, "reset_form('washoutput_1','','','txt_issue_date,".$date."','childFormReset()')", 1);
								?>
