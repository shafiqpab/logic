...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../../logout.php";
	...
	function openmypage_batchnum()
	{
		...
		{*/
			var page_link='requires/de_oiling_controller.php?cbo_company_id='+cbo_company_id+'&action=batch_number_popup';
			...
			emailwindow.onclose=function()
			{
				...
				{
					freeze_window(5);
					get_php_form_data(batch_id, "populate_data_from_batch", "requires/de_oiling_controller" );
					show_list_view(batch_id+'_'+roll_maintained,'show_fabric_desc_listview','list_fabric_desc_container','requires/de_oiling_controller','');
					//show_list_view( batch_id,'show_dtls_list_view','list_container','requires/de_oiling_controller','');
					show_list_view(document.getElementById('txt_batch_ID').value,'show_dtls_list_view','list_container','requires/de_oiling_controller','');
					
					release_freezing();
				}
				get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/de_oiling_controller' );
			}
		//}
	}
	...
	function openmypage_servicebook()
	{
		...
		var page_link='requires/de_oiling_controller.php?cbo_company_id='+cbo_company_id+'&supplier_id='+supplier_id+'&process_id='+process_id+'&action=service_booking_popup';
	...
	function check_batch()
	{
		...
		if(batch_no!="")
		{
			...
			var response=return_global_ajax_value( cbo_company_id+"**"+batch_no, 'check_batch_no', '', 'requires/de_oiling_controller');
			var response=response.split("_");
			if(response[0]==0)
			{
				alert('Batch no not found.');
				$('#txt_batch_no').val('');$('#txt_batch_ID').val('');
				$('#hidden_batch_id').val(''); 
				//$('#cbo_company_id').val(''); 
				$('#txt_update_id').val(''); 
				$('#cbo_sub_process').val('');
			...
			else
			{
				...
				get_php_form_data(response[1]+'_'+response[2], "populate_data_from_batch", "requires/de_oiling_controller" );
				show_list_view(response[1]+'_'+roll_maintained,'show_fabric_desc_listview','list_fabric_desc_container','requires/de_oiling_controller','');
				show_list_view( response[1],'show_dtls_list_view','list_container','requires/de_oiling_controller','');
				get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/de_oiling_controller' );
	...
	function fnc_pro_fab_subprocess( operation )
	{
		...
		if( form_validation('cbo_company_id*txt_batch_no*cbo_service_source*cbo_service_company*txt_recevied_chalan*txt_process_end_date*txt_process_start_date*txt_process_date*txt_batch_ID','Company*Batch No*Service Source*Service Company*Recevied Challan*Production Date*Process Start  Date*Process End Date*Batch ID')==false )
			...
			if('<?php echo implode('*',$_SESSION['logic_erp']['mandatory_field'][310]);?>'){
				if (form_validation('<?php echo implode('*',$_SESSION['logic_erp']['mandatory_field'][310]);?>','<?php echo implode('*',$_SESSION['logic_erp']['field_message'][310]);?>')==false)
				{
					return;
				}
			}
	...
	//alert(data_all);return;
	  ...
	  http.open("POST","requires/de_oiling_controller.php",true);
	...
	function fnc_pro_fab_subprocess_response()
	{
		if(http.readyState == 4) 
		{
			...
			show_list_view(document.getElementById('txt_batch_ID').value+'_'+document.getElementById('roll_maintained').value,'show_fabric_desc_listview','list_fabric_desc_container','requires/de_oiling_controller','');
			show_list_view(document.getElementById('txt_batch_ID').value,'show_dtls_list_view','list_container','requires/de_oiling_controller','');
			if(reponse[0]==2)
			{
...
function scan_batchnumber(str) //bar code scan
{
	var cbo_company_id = $('#cbo_company_id').val();
	var roll_maintained=$('#roll_maintained').val();
	//var response=return_global_ajax_value( cbo_company_id+"**"+str, 'check_batch_no_scan', '', 'requires/de_oiling_controller');
	//var response=response.split("_");
	...
	$('#hidden_batch_id').val(response[1]);
	get_php_form_data(response[1]+'_'+roll_maintained, "populate_data_from_batch", "requires/de_oiling_controller" );
	show_list_view(response[1]+'_'+roll_maintained,'show_fabric_desc_listview','list_fabric_desc_container','requires/de_oiling_controller','');
	get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/de_oiling_controller' );
...
function openmypage_process()
	{
		...
		var page_link = 'requires/de_oiling_controller.php?txt_process_id='+txt_process_id+'&action=process_name_popup';
	...
	function check_issue_challan() //Issue Challan
	{
		...
		if(issue_chalan!="")
		{
			...
			var response=return_global_ajax_value( cbo_company_id+"**"+issue_chalan, 'check_issue_challan_no', '', 'requires/de_oiling_controller');
			var response=response.split("_");
			...
			else
			{
				...
				get_php_form_data(response[1], "populate_data_from_data", "requires/de_oiling_controller" );
				var row_num=$('#tbl_item_details tbody tr').length-1;
				//alert(row_num);return;
				
				var hidden_roll_id = $('#txt_roll_id').val();
				show_list_view(hidden_roll_id+'_'+batch_id,'issue_show_fabric_desc_listview','list_fabric_desc_container','requires/de_oiling_controller','');
	...
	function openmypage_issue_challan()
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/de_oiling_controller.php?cbo_company_id='+cbo_company_id+'&action=issue_challan_popup','Issue Popup', 'width=880px,height=350px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
			...
			if(issue_id!="")
			{
				get_php_form_data(issue_id, "populate_data_from_data", "requires/de_oiling_controller");
	...
	{ 
		...
		//alert(com);
		get_php_form_data($('#cbo_company_id').val(),'roll_maintained_data','requires/de_oiling_controller' );
		var roll_maintained=$('#roll_maintained').val();
		var page_upto=$('#page_upto').val();
		if((page_upto*1==7 || page_upto*1>7) && roll_maintained==1  ) // && roll_maintained==1 
		{
	 ...
	 function check_issue_challan_scan(str) //Issue Challan Scan
	{
		...
		if(issue_chalan!="")
		{
			var response=return_global_ajax_value( cbo_company_id+"**"+issue_chalan, 'check_issue_challan_no_scan', '', 'requires/de_oiling_controller');
			var response=response.split("_");			
			if(response[0]==0)
			{
				alert('Issue Challan  not found.');
			...
			else
			{
				...
				get_php_form_data(response[1], "populate_data_from_data", "requires/de_oiling_controller" );
				var hidden_roll_id = $('#txt_roll_id').val();
				show_list_view(batch_id+'_'+hidden_roll_id+'_'+cbo_company_id,'issue_show_fabric_desc_listview','list_fabric_desc_container','requires/de_oiling_controller','');
				$('#cbo_service_source').focus();
			}
                    ...
                    <table cellpadding="0" cellspacing="2" width="100%" id="main_tbl">
                        ...
                        <tr>
                         <td class="must_entry_caption" width="130">Company</td>
                            <td>
                                <?
								//load_drop_down('requires/de_oiling_controller', this.value, 'load_drop_floor', 'floor_td' );
                        ...
                        <td>
							<?
                                echo create_drop_down( "cbo_service_source", 135, $knitting_source, "", 1, "-- Select --", $selected, "load_drop_down( 'requires/de_oiling_controller',this.value+'**'+$('#cbo_company_id').val(),'load_drop_down_knitting_com','dyeing_company_td' );search_populate(this.value);","","1,3" );
                            ?>
                        </td>
                        ...
                        <tr>
                            ...
                            <td>
                               <?
								 echo create_drop_down( "cbo_process_id", 135, $conversion_cost_head_array,"", 0, "-- Select --", 0, "","","179" );
                               ?>
