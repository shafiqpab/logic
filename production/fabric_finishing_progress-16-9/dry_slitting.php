<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Dry Slitting Entry
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("De Oiling  Entry Info","../../", 1, 1, "",'1','');
?>
<script>
...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../../logout.php";
	...
	function openmypage_batchnum()
	{
		...
		{*/
			var page_link='requires/dry_slitting_controller.php?cbo_company_id='+cbo_company_id+'&action=batch_number_popup';
			...
			emailwindow.onclose=function()
			{
				...
				{
					freeze_window(5);
					get_php_form_data(batch_id, "populate_data_from_batch", "requires/dry_slitting_controller" );
					show_list_view(batch_id+'_'+roll_maintained,'show_fabric_desc_listview','list_fabric_desc_container','requires/dry_slitting_controller','');
					//show_list_view( batch_id,'show_dtls_list_view','list_container','requires/dry_slitting_controller','');
					show_list_view(document.getElementById('txt_batch_ID').value,'show_dtls_list_view','list_container','requires/dry_slitting_controller','');
					
					release_freezing();
				}
				get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/dry_slitting_controller' );
			}
		//}
	}
	...
	function openmypage_servicebook()
	{
		...
		var page_link='requires/dry_slitting_controller.php?cbo_company_id='+cbo_company_id+'&supplier_id='+supplier_id+'&process_id='+process_id+'&action=service_booking_popup';
	...
	function check_batch()
	{
		...
		$('#hidden_batch_id').val('');$('#txt_batch_ID').val('');
		var cbo_company_id = $('#cbo_company_id').val();
		var roll_maintained=$('#roll_maintained').val();
		if(batch_no!="")
		{
			...
			var response=return_global_ajax_value( cbo_company_id+"**"+batch_no, 'check_batch_no', '', 'requires/dry_slitting_controller');
			...
			else
			{
				...
				get_php_form_data(response[1]+'_'+response[2], "populate_data_from_batch", "requires/dry_slitting_controller" );
				show_list_view(response[1]+'_'+roll_maintained,'show_fabric_desc_listview','list_fabric_desc_container','requires/dry_slitting_controller','');
				show_list_view( response[1],'show_dtls_list_view','list_container','requires/dry_slitting_controller','');
				get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/dry_slitting_controller' );
	...
	function fnc_pro_fab_subprocess( operation )
	{
		...
		if (operation==0)
		{
			...
			if('<?php echo implode('*',$_SESSION['logic_erp']['mandatory_field'][323]);?>'){
				if (form_validation('<?php echo implode('*',$_SESSION['logic_erp']['mandatory_field'][323]);?>','<?php echo implode('*',$_SESSION['logic_erp']['field_message'][323]);?>')==false)
				{
					return;
				}
			}
		...
		else
		{
			...
			for (var i=1; i<=row_num; i++)
			{
				...
				data_all+=get_submitted_data_string('txtconscomp_'+i+'*txtgsm_'+i+'*txtbodypart_'+i+'*txtdiawidth_'+i+'*txtbatchqnty_'+i+'*txtprodid_'+i+'*updateiddtls_'+i+'*txtdiawidthID_'+i+'*txtroll_'+i+'*txtproductionqty_'+i+'*txtrate_'+i+'*txtamount_'+i,"../../",i);
			} 
			//alert( data_all);return;
		}
		
	//alert(data_all);return;
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('cbo_company_id*txt_batch_no*txt_batch_ID*hidden_batch_id*cbo_process_id*txt_process_end_date*txt_process_date*txt_end_hours*txt_end_minutes*cbo_machine_name*txt_remarks*txt_ext_id*txt_update_id*cbo_floor*cbo_shift_name*txt_process_start_date*txt_start_hours*txt_start_minutes*txt_recevied_chalan*cbo_service_company*cbo_service_source*txt_issue_chalan*roll_maintained*txt_roll_id*txt_issue_mst_id*txt_booking_no*txt_trims_weight*hidden_exchange_rate*hidden_currency',"../../")+data_all+'&total_row='+row_num;
		//alert (data);return;
	  freeze_window(operation);
	  http.open("POST","requires/dry_slitting_controller.php",true);
	...
	function fnc_pro_fab_subprocess_response()
	{
		if(http.readyState == 4) 
		{
			...
			show_list_view(document.getElementById('txt_batch_ID').value+'_'+document.getElementById('roll_maintained').value,'show_fabric_desc_listview','list_fabric_desc_container','requires/dry_slitting_controller','');
			show_list_view(document.getElementById('txt_batch_ID').value,'show_dtls_list_view','list_container','requires/dry_slitting_controller','');
			if(reponse[0]==2)
			{
...
function scan_batchnumber(str) //bar code scan
{
	var cbo_company_id = $('#cbo_company_id').val();
	var roll_maintained=$('#roll_maintained').val();
	//var response=return_global_ajax_value( cbo_company_id+"**"+str, 'check_batch_no_scan', '', 'requires/dry_slitting_controller');
	//var response=response.split("_");
	...
	$('#hidden_batch_id').val(response[1]);
	get_php_form_data(response[1]+'_'+roll_maintained, "populate_data_from_batch", "requires/dry_slitting_controller" );
	show_list_view(response[1]+'_'+roll_maintained,'show_fabric_desc_listview','list_fabric_desc_container','requires/dry_slitting_controller','');
	get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/dry_slitting_controller' );
...
function openmypage_process()
	{
		...
		var page_link = 'requires/dry_slitting_controller.php?txt_process_id='+txt_process_id+'&action=process_name_popup';
	...
	function check_issue_challan() //Issue Challan
	{
		...
		if(issue_chalan!="")
		{
			...
			var response=return_global_ajax_value( cbo_company_id+"**"+issue_chalan, 'check_issue_challan_no', '', 'requires/dry_slitting_controller');
			var response=response.split("_");
			...
			else
			{
				...
				get_php_form_data(response[1], "populate_data_from_data", "requires/dry_slitting_controller" );
				var row_num=$('#tbl_item_details tbody tr').length-1;
				//alert(row_num);return;
				
				var hidden_roll_id = $('#txt_roll_id').val();
				show_list_view(hidden_roll_id+'_'+batch_id,'issue_show_fabric_desc_listview','list_fabric_desc_container','requires/dry_slitting_controller','');
	...
	function openmypage_issue_challan()
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/dry_slitting_controller.php?cbo_company_id='+cbo_company_id+'&action=issue_challan_popup','Issue Popup', 'width=880px,height=350px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
			...
			if(issue_id!="")
			{
				get_php_form_data(issue_id, "populate_data_from_data", "requires/dry_slitting_controller");
				var row_num=$('#tbl_item_details tbody tr').length-1;
				//alert(row_num);return;
				var issue_roll = $('#txt_roll_id').val();
				var update_roll_arr=new Array();
	...
	{ 
		...
		//alert(com);
		get_php_form_data($('#cbo_company_id').val(),'roll_maintained_data','requires/dry_slitting_controller' );
		var roll_maintained=$('#roll_maintained').val();
		var page_upto=$('#page_upto').val();
		if((page_upto*1==7 || page_upto*1>7) && roll_maintained==1  ) // && roll_maintained==1 
		{
	 ...
	 function check_issue_challan_scan(str) //Issue Challan Scan
	{
		...
		if(issue_chalan!="")
		{
			var response=return_global_ajax_value( cbo_company_id+"**"+issue_chalan, 'check_issue_challan_no_scan', '', 'requires/dry_slitting_controller');
			var response=response.split("_");			
			if(response[0]==0)
			{
				alert('Issue Challan  not found.');
			...
			else
			{
				...
				get_php_form_data(response[1], "populate_data_from_data", "requires/dry_slitting_controller" );
				var hidden_roll_id = $('#txt_roll_id').val();
				show_list_view(batch_id+'_'+hidden_roll_id+'_'+cbo_company_id,'issue_show_fabric_desc_listview','list_fabric_desc_container','requires/dry_slitting_controller','');
				$('#cbo_service_source').focus();
			}
	...
	function fnc_check()
	{
		var row_num=$('#tbl_item_details tbody tr').length-1;
			//alert(row_num);
			var checked_id=$('#checkbox_id').prop('checked');
			//alert(checked_id);
			for (var k=1; k<=row_num; k++)
			{ //alert(k);
				if(checked_id==true)
                    ...
                    <table cellpadding="0" cellspacing="2" width="100%" id="main_tbl">
                        ...
                        <tr>
                         <td class="must_entry_caption" width="130">Company</td>
                            <td>
                                <?
								//load_drop_down('requires/dry_slitting_controller', this.value, 'load_drop_floor', 'floor_td' );
                        ...
                        <td>
							<?
                                echo create_drop_down( "cbo_service_source", 135, $knitting_source, "", 1, "-- Select --", $selected, "load_drop_down( 'requires/dry_slitting_controller',this.value+'**'+$('#cbo_company_id').val(),'load_drop_down_knitting_com','dyeing_company_td' );search_populate(this.value);","","1,3" );
                            ?>
                        </td>
                        ...
                        <tr>
                            ...
                            <td>
                               <?
								 echo create_drop_down( "cbo_process_id", 135, $conversion_cost_head_array,"", 0, "-- Select --", 0, "","","200" );
                               ?>
                            </td>
                        </tr>
                        <tr>
                                ...
                                <fieldset>
                                    <table width="830" align="left" id="tbl_body1">
                                        <tr>
                                            ...
                                            <td width="100">Dyeing En Date</td>
                                            <td>
