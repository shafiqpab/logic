...
if($action=="mst_id_child_form_input_data")
{
	$sql_result=sql_select("SELECT a.id,a.company_id,a.batch_id,a.service_source,a.service_company,a.received_chalan,a.issue_chalan,issue_challan_mst_id,a.process_end_date,a.production_date,a.process_start_date,a.process_id,a.end_hours,a.end_minutes,a.start_minutes,a.temparature,a.stretch,a.over_feed,a.feed_in,a.pinning,a.speed_min,a.start_hours,a.floor_id,a.machine_id,a.shift_name,a.remarks,a.booking_no,b.currency_id,b.exchange_rate from pro_fab_subprocess a,pro_fab_subprocess_dtls b where a.id=b.mst_id and a.id=$data and entry_form=47  and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0");
  	...
  	$trims_weight=return_field_value("total_trims_weight","pro_batch_create_mst","company_id =".$sql_result[0][csf('company_id')]." and id=".$sql_result[0][csf('batch_id')]." and is_deleted=0 and status_active=1");
		...
		echo "load_drop_down( 'requires/singeing_controller', ".$sql_result[0][csf("service_source")]."+'**'+".$sql_result[0][csf("company_id")].", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
