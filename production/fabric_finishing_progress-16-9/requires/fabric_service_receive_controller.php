    ...
    <div>
        ...
        <table cellspacing="0" width="1030"  border="1" rules="all" class="rpt_table" >
            ...
            <?
                ...
                $sql="select id,mst_id,outbound_batchname,booking_no,booking_id,body_part_id,febric_description_id,width,color_id, process_id,wo_qty, batch_issue_qty,rate,amount,currency_id,exchange_rate,buyer_id, job_no, order_id from pro_grey_batch_dtls where mst_id=$update_id order by id " ;
                //echo $sql;
                $i=1;
            ...
            <tfoot>
                ...
                <th width=""> </th>
                <th width="">Total</th>
                <th width="" align="right"><?php echo $total_wo;?></th>
                <th width="" align="right"><?php echo $total_rece;?></th>
                ...
                <table  cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
                    ...
                    <tr>
                        <td> <input type="hidden" id="selected_booking">
                            <? 
                                echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 $company_cond order by company_name","id,company_name",1, "-- Select Company --", "".$company_id."", "load_drop_down( 'fabric_service_receive_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );",1);
                            ?>
                        </td>
                        <td>
                            <?php 
...
if ($action=="create_booking_search_list_view")
{
   ...
   // $po_no=return_library_array( "select id, po_number from wo_po_break_down",'id','po_number');
    ...
    where $company $buyer $booking_date $style_cond and  a.booking_type=3 and  a.status_active=1 and a.is_deleted=0 and e.job_no=c.job_no and a.booking_no = e.booking_no   $booking_cond $job_cond  $supplier $po_ids_cond and a.booking_no = b.booking_no and d.id = b.mst_id and d.entry_form = 91 $preBookingNos_1
...
if ($action=="fabric_detls_list_view")
{
    ...
    if($type==1)
    {
        ...
        foreach( $wo_pre_cost_fab_conv_cost_dtls_id as $row_wo_pre_cost_fab_conv_cost_dtls_id)
        {
            if($row_wo_pre_cost_fab_conv_cost_dtls_id[csf("fabric_description")]!=0)
            {
				...
				$color_type_arr[$job_no][$fabric_description_row[csf("body_part_id")]].=$fabric_description_row[csf("color_type_id")].',';
				
            }
            
        }

       $sql_tmp = "select a.pre_cost_fabric_cost_dtls_id, a.po_break_down_id,a.color_type, a.gmts_color_id as fabric_color_id, a.process , a.job_no, a.description, c.rate, c.batch_wgt as workorder_qty, c.batch_issue_qty,c.booking_dtls_id,c.body_part_id, a.dia_width, a.fin_gsm, b.currency_id, b.exchange_rate , c.id as batch_dtls_id,c.booking_no,c.outbound_batchname
        ...
        $previousReceiveRes=sql_select("select a.batch_issue_qty,a.order_id,a.booking_no,a.body_part_id,a.color_id,a.febric_description_id as deter_id,a.booking_dtls_id,a.id  from pro_grey_batch_dtls a, inv_receive_mas_batchroll b  
        ...
        foreach ($tmp_dataArray as $booking_dtls_id => $row) 
        {
            ...
            $previous_receive_qty2=$previousReceiveArrNew2[$row[('po_break_down_id')]][$body_id][$row[('fabric_color_id')]][$fabric_des];
            $chk_balance_qty = $row['batch_issue_qty']-$previous_receive_qty2;
            $po_batch_qty=$po_batch_data_qty_arr[$row[('po_break_down_id')]][$body_id][$row[('fabric_color_id')]][$fabric_des]['batch_qnty'];
    ...
    foreach( $data_array as $row )
    {
     ...
     where b.id=c.wo_id and c.fabric_description=a.id and b.wo_no='$booking_no' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0
      and  b.wo_no = d.booking_no and d.mst_id = e.id and e.entry_form = 91
            union all 
        select d.body_part_id as body_part, d.febric_description_id as lib_yarn_count_deter_id, d.color_id as fabric_color, d.process_id as process , null as fabric_description, null as job_no, null as po_break_down_id, null as description,b.buyer_id, d.fin_dia as dia_width, d.fin_gsm as fin_gsm, b.currency_id, b.exchange_rate, d.id as batch_dtls_id, d.batch_issue_qty, d.batch_wgt  as workorder_qty, d.booking_dtls_id, b.booking_no,d.rate,d.outbound_batchname
        from wo_non_ord_knitdye_booking_mst b,wo_non_ord_knitdye_booking_dtl c, pro_grey_batch_dtls d,inv_receive_mas_batchroll e 
        where   b.id = c.mst_id and c.id = d.booking_dtls_id and  d.mst_id = e.id and e.entry_form = 91 and b.booking_no = d.booking_no and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0  and b.booking_no  ='$booking_no'
      order by batch_dtls_id";
...
{
    $sql = "select id, company_id, recv_number, dyeing_source, dyeing_company, receive_date,challan_no from inv_receive_mas_batchroll where id=$data and entry_form=92";
    //echo $sql;
    $res = sql_select($sql);    
    foreach($res as $row)
    {       
...
if ($action=="save_update_delete")
{
    ...
    { 
      ...
      $id= return_next_id_by_sequence("INV_RCV_MAS_BATC_PK_SEQ", "inv_receive_mas_batchroll", $con);
	   $new_mrr_number = explode("*", return_next_id_by_sequence("INV_RCV_MAS_BATC_PK_SEQ", "inv_receive_mas_batchroll",$con,1,$cbo_company_id,'FSR',92,date("Y",time()) ));
        ...
        $field_array="id,recv_number_prefix,recv_number_prefix_num,recv_number,entry_form, company_id, dyeing_source,batch_id, dyeing_company, receive_date,challan_no, inserted_by, insert_date";
        if(str_replace("'","",$txt_receive_challan)=="") $txt_receive_challan=$new_mrr_number[0];
        $data_array="(".$id.",'".$new_mrr_number[1]."',".$new_mrr_number[2].",'".$new_mrr_number[0]."',92,".$cbo_company_id.",".$cbo_service_source.","."0".",".$cbo_service_company.",".$txt_receive_date.",'".str_replace("'","",$txt_receive_challan)."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
        
      //  $dtls_id=return_next_id("id", "pro_grey_batch_dtls", 1);
        $field_array_dtls="id, mst_id, outbound_batchname, booking_no, booking_id, booking_without_order, body_part_id, febric_description_id, width, color_id, process_id, wo_qty, batch_issue_qty, rate, amount, currency_id, exchange_rate, buyer_id, job_no, order_id, fin_dia, fin_gsm,grey_used,booking_dtls_id,  inserted_by, insert_date";     
        $all_detailsId='';
        for($j=1;$j<=$tot_row;$j++)
        {   
           ...
           // echo $$currencyId;die;
            ...
            if($$receiveQty!="")
            {
                ...
                $data_array_dtls.="(".$dtls_id.",".$id.",'".$$batchName."','".$$bookingNo."','".$$bookingId."','".$$bookWithoutOrder."','".$$bodyparyId."','".$$deterId."','".$$dia."','".$$colorId."','".$$process_id."','".$$workorderNo."','".$$receiveQty."','".$$txtRate."','".$$txtAmount."','".$$currencyId."','".$$exchangeRate."','".$$buyerId."','".$$jobNo."','".$$orderId."','".$$finDia."','".$$finGsm."','".$$greyUsed."','".$$bookingDtlsId."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
    ...
    { 
        ...
        $field_array="dyeing_source*dyeing_company*receive_date*challan_no*updated_by*update_date";
        $data_array=$cbo_service_source."*".$cbo_service_company."*".$txt_receive_date."*".$txt_receive_challan."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
        ...
        {   
            ...
            if($$receiveQty!="")
            {
                if($$update_dtls!="")
                {
                    ...
                    $data_array_update_dtls[str_replace("'","",$$update_dtls)]=explode("*",("'".$$batchName."'*'".$$receiveQty."'*'".$$txtRate."'*'".$$txtAmount."'*'".$$greyUsed."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
                    $all_detailsId.=$$trId."__".str_replace("'","",$$update_dtls).",";
                }
                else
                {
                    if($data_array_dtls!="") $data_array_dtls.=",";
                    $data_array_dtls.="(".$dtls_id.",".$update_id.",'".$$batchName."','".$$bookingNo."','".$$bookingId."','".$$bookWithoutOrder."','".$$bodyparyId."','".$$deterId."','".$$dia."','".$$colorId."','".$$process_id."','".$$receiveQty."','".$$txtRate."','".$$txtAmount."','".$$currencyId."','".$$exchangeRate."','".$$buyerId."','".$$jobNo."','".$$orderId."','".$$finDia."','".$$finGsm."','".$$greyUsed."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
                    $all_detailsId.=$$trId."__".$dtls_id.",";
                   // $dtls_id = $dtls_id+1;
                }
            }
