...
if ($action=="batch_number_popup")
{
	...
	?>
	<script>
		function js_set_value(id,batch_no,is_sales)
		{ 
			var data_id=id+'_'+batch_no+'_'+is_sales;
            ...
            <?
				...
				foreach ($nameArray as $selectResult)
				{
					...
					{	
						...
						//$po_no=implode(",",array_unique(explode(",",$po_num[$selectResult[csf('id')]]['po_no'])));
						?>
						<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $selectResult[csf('id')]; ?>','<? echo $selectResult[csf('batch_no')];?>','<? echo $is_sales;?>')"> 
...
if($action=='populate_data_from_batch')
{
	...
	{	 
		$sql_re="SELECT a.company_id,a.total_trims_weight,MAX(a.id)as id,a.batch_no,a.entry_form, a.batch_weight,Max(a.extention_no) as extention_no,a.batch_date,a.process_id as process_id_batch, a.booking_no,
	...
	{ 
		...
		if(!$row[csf("total_trims_weight")])$row[csf("total_trims_weight")]=0;
	...
	//	echo "load_drop_down( 'requires/slitting_squeezing_controller', '".$row[csf("company_id")]."', 'load_drop_floor', 'floor_td' );\n";
		...
		$sql_batch_d=sql_select("select id,batch_no,process_end_date,end_hours,end_minutes,machine_id,floor_id,process_id,remarks from pro_fab_subprocess where
		batch_id='$batch_id' and entry_form=35 and load_unload_id=1");
		...
		if(count($sql_result)>0)
		{
			...
			foreach($sql_result as $row)
			{
				...
				<tr class="general" id="row_<? echo $i; ?>">
					...
					<td width="40" id="sl_<? echo $i; ?>"><input type="checkbox" id="checkRow_<? echo $i; ?>" name="checkRow[]"  checked><? echo $i; ?></td>
					<td title="<? echo  $row[csf('const_composition')]; ?>"><input type="text" name="txtconscomp_<? echo $i; ?>" id="txtconscomp_<? echo $i; ?>" class="text_boxes" style="width:145px;" value="<? echo  $row[csf('const_composition')]; ?>" disabled/></td>
					...
					<td><input type="text" name="txtproductionqty_<? echo $i; ?>" id="txtproductionqty_<? echo $i; ?>" class="text_boxes_numeric" value="<? echo number_format($row[csf('production_qty')],2,'.',''); ?>" style="width:50px;" onKeyUp="calculate_production_qnty();" /></td>
...
if($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$field_array="id,company_id,service_source,service_company,received_chalan,issue_chalan,issue_challan_mst_id ,batch_no,batch_id,batch_ext_no,
        ...
        shift_name,chemical_name,remarks,booking_no,inserted_by,insert_date";
		$id=return_next_id( "id", " pro_fab_subprocess", 1 ) ;
		$data_array="(".$id.",".$cbo_company_id.",".$cbo_service_source.",".$cbo_service_company.",".$txt_recevied_chalan.",".$txt_issue_chalan.",".$txt_issue_mst_id.",".$txt_batch_no.",".$txt_batch_ID.",".$txt_ext_id.",".$cbo_sub_process.",".$txt_process_start_date.",".$txt_process_end_date.",".$txt_process_date.",".$txt_start_hours.",".$txt_start_minutes.",".$txt_end_hours.",".$txt_end_minutes.",".$cbo_machine_name.",".$cbo_result_name.",".$cbo_floor.",30,".$cbo_shift_name.",".$txt_chemical.",".$txt_remarks.",".$txt_booking_no.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");  
				echo "0**".$id."**".str_replace("'","",$roll_maintained);
			}
			else
			{
				mysql_query("ROLLBACK"); 
				echo "10**".$id."**".str_replace("'","",$roll_maintained);
			}
		}
	...
	{ 
		...
		$field_array_update="company_id*service_source*service_company*received_chalan*issue_chalan*issue_challan_mst_id *batch_no*batch_id*batch_ext_no*process_id*process_end_date*production_date*process_start_date*start_hours*start_minutes*end_hours*end_minutes*machine_id*result*floor_id*entry_form*shift_name*chemical_name*remarks*booking_no*updated_by*update_date";
		$data_array_update="".$cbo_company_id."*".$cbo_service_source."*".$cbo_service_company."*".$txt_recevied_chalan."*".$txt_issue_chalan."*".$txt_issue_mst_id."*".$txt_batch_no."*".$txt_batch_ID."*".$txt_ext_id."*".$cbo_sub_process."*".$txt_process_end_date."*".$txt_process_date."*".$txt_process_start_date."*".$txt_start_hours."*".$txt_start_minutes."*".$txt_end_hours."*".$txt_end_minutes."*".$cbo_machine_name."*".$cbo_result_name."*".$cbo_floor."*30*".$cbo_shift_name."*".$txt_chemical."*".$txt_remarks."*".$txt_booking_no."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");  
				echo "1**".$update_id."**".str_replace("'","",$roll_maintained);
			}
			else
			{
				mysql_query("ROLLBACK"); 
				echo "10**".$update_id."**".str_replace("'","",$roll_maintained);
			}
		}
		if($db_type==2 || $db_type==1 )
		{	
...
{
   $sql_result=sql_select("SELECT a.id, a.company_id,a.service_source, a.service_company, a.received_chalan, a.issue_chalan, a.issue_challan_mst_id, a.process_end_date, a.production_date, a.process_start_date, a.batch_id,a.process_id, a.end_hours, a.end_minutes, a.start_hours, a.start_minutes, a.temparature, a.stretch, a.over_feed, a.feed_in, a.pinning, a.speed_min, a.floor_id, a.machine_id,a.result, a.shift_name,a.chemical_name, a.remarks,a.booking_no 
  ...
  $trims_weight=return_field_value("total_trims_weight","pro_batch_create_mst","company_id =".$sql_result[0][csf('company_id')]." and id=".$sql_result[0][csf('batch_id')]." and is_deleted=0 and status_active=1");
	...
	echo "load_drop_down( 'requires/slitting_squeezing_controller',".$sql_result[0][csf("service_source")]."+'**'+ document.getElementById('cbo_company_id').value, 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
