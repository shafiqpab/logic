		...
		if(batch_no!="")
		{
			...
			else
			{
				$('#hidden_batch_id').val(response[1]);
					...
					//else
					//{ 
				get_php_form_data(response[1]+'_'+response[2], "populate_data_from_batch", "requires/stentering_controller" );
				check_re_stenter();
	...
	function fnc_pro_fab_subprocess_response()
	{
		if(http.readyState == 4) 
		{
			...
			if(reponse[0]==0)
			{
				...
				reset_form('slittingsqueezing_1','list_container','','','$(\'#list_fabric_desc_container tr:not(:first)\').remove();','txt_batch_ID*txt_job_no*txt_buyer*txt_order_no*txt_color*txt_mc_group*txt_machine_no*txt_slitting_time*txt_slitting_date*txt_ext_id');
				release_freezing();
			}
			else if(reponse[0]==1)
			{
...
function openmypage_process()
	...
	{
		var batch_id=$('#hidden_batch_id').val();
		get_php_form_data(batch_id, "populate_restenter_from_data", "requires/stentering_controller" );
		//txt_restenter_no
