...
function openmypage_batchnum()
{
		...
		//alert(batch_ids[1])
			if(batch_ids!="")
			{
				...
				get_php_form_data(batch_ids[0]+'_'+batch_ids[2], "populate_data_from_batch", "requires/slitting_squeezing_controller" );
...
{
	var batch_no=$('#txt_batch_no').val();$('#txt_batch_ID').val('');
	var cbo_company_id = $('#cbo_company_id').val();
	if(batch_no!="")
	{
		var response=return_global_ajax_value( batch_no, 'check_batch_deying', '', 'requires/slitting_squeezing_controller');
		...
		else
		{
			...
			get_php_form_data(response[1]+'_'+response[2]+'_'+batch_no, "populate_data_from_batch", "requires/slitting_squeezing_controller" );
			var cbo_company_id = $('#cbo_company_id').val();
			var txt_batch_id= $('#txt_batch_ID').val();
			show_list_view(txt_batch_id+'_'+cbo_company_id,'show_fabric_desc_listview','list_fabric_desc_container','requires/slitting_squeezing_controller','');
			var roll_variable = $('#roll_maintained').val();
                ...
                <td width="73%" valign="top">
                    <table cellpadding="0" cellspacing="1" width="100%" border="0" align="left">
                        ...
                        <tr>
                            <td  valign="top">
                                <fieldset>
                                    <table width="800" align="left" id="tbl_body1">
                                        <tr>
