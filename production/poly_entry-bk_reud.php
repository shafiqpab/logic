...
<?
$data_arr= json_encode( $_SESSION['logic_erp']['data_arr'][164] );
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			if (po_id!="")
			{
				...
				get_php_form_data(po_id+'**'+item_id+'**'+country_id+'**'+$('#hidden_preceding_process').val()+'**'+pack_type, "populate_data_from_search_popup", "requires/poly_entry_controller" );
				...
				if(variableSettings!=1){ 
					get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject+'**'+$('#hidden_preceding_process').val()+'**'+pack_type, "color_and_size_level", "requires/poly_entry_controller" ); 
				...
				show_list_view(po_id+'**'+item_id+'**'+country_id+'**'+prod_reso_allo+'**'+pack_type,'show_dtls_listview','list_view_container','requires/poly_entry_controller','setFilterGrid(\'tbl_list_search\',-1)');
				show_list_view(po_id,'show_country_listview','list_view_country','requires/poly_entry_controller','');		
...
function fnc_poly_output_entry(operation)
{
	...
	else if(operation==0 || operation==1 || operation==2)
	{
		...
		else
		{
			if($("#cbo_source").val()==1)
			{
				if($('#cbo_sewing_floor').is('[disabled=disabled]'))
				{
					var vlidationPolyLine=form_validation('cbo_location','Location')==false
				}
				else
				{
					var vlidationPolyLine=form_validation('cbo_location*cbo_sewing_floor*cbo_poly_line','Location*Floor*Line')==false
				}
					if(vlidationPolyLine)
					{
						return;
					}
			}
 	...
 	if(http.readyState == 4) 
	{
		...
		else if(reponse[0]==0)
		{
			...
			show_msg(trim(reponse[0]));
 			show_list_view(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id+'**'+prod_reso_allo+'**'+pack_type,'show_dtls_listview','list_view_container','requires/poly_entry_controller','setFilterGrid(\'tbl_list_search\',-1)');
			reset_form('','','cbo_produced_by*cbo_poly_line*txt_reporting_hour*txt_super_visor*txt_poly_qty*txt_reject_qnty*txt_alter_qnty*txt_remark*txt_input_quantity*txt_cumul_poly_qty*txt_yet_to_poly*hidden_break_down_html*txt_mst_id*txt_spot_qnty*save_data*defect_type_id*all_defect_id*save_dataSpot*allSpot_defect_id*defectSpot_type_id','txt_poly_date,<? echo date("d-m-Y"); ?>','');
			get_php_form_data(po_id+'**'+$('#cbo_item_name').val()+'**'+country_id+'**'+$('#hidden_preceding_process').val()+'**'+pack_type, "populate_data_from_search_popup", "requires/poly_entry_controller" );
 			if(variableSettings!=1) { 
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject+'**'+$('#hidden_preceding_process').val()+'**'+pack_type, "color_and_size_level", "requires/poly_entry_controller" ); 
			}
			else
...
function put_country_data(po_id, item_id, country_id, po_qnty, plan_qnty, pack_type)
{
	...
	show_list_view(po_id+'**'+item_id+'**'+country_id+'**'+prod_reso_allo,'show_dtls_listview','list_view_container','requires/poly_entry_controller','');
	set_button_status(0, permission, 'fnc_poly_output_entry',1,0);
	release_freezing();
}
                    ...
                    <table width="100%" border="0">
                        <tr>
                           ...
                           <input type="hidden" name="hidden_preceding_process" id="hidden_preceding_process">
                         	 <td class="must_entry_caption">Poly Company</td>
                        ...
                        <tr>
                        	<td class="must_entry_caption">Sew. Floor</td>
                             <td id="floor_td">
								 <? 
                                 echo create_drop_down( "cbo_sewing_floor", 150, $blank_array,"", 1, "-- Select Floor --", $selected, "" );
                                 ?>
                             </td>
                            ...
                            <td> Fini. Floor</td>
                          	<td id="fini_floor_td">
								 <? 
                                 echo create_drop_down( "cbo_finishing_floor", 150, $blank_array,"", 1, "-- Select Finishing Floor --", $selected, "" );
                                 ?>
                           </td>
