...
<script>
	...
	{ 
		...
		emailwindow.onclose=function()
		{
			...
			if(reqn_id!="")
			{
				...
				{
					var action='populate_list_view_for_sales';
				}
				else
				{
					var action='populate_list_view';
				}

				var list_view = trim(return_global_ajax_value(reqn_id, action, '', 'requires/fabric_requisition_for_batch_entry_controller_3'));
	...
	{
		var cbo_company_id = $('#cbo_company_id').val();
		var hdn_is_sales = $('#hidden_is_sales').val();
		...
		{ 	
			...
			var page_link ='requires/fabric_requisition_for_batch_entry_controller_3.php?company_id='+cbo_company_id+'&hdn_is_sales='+hdn_is_sales+'&action=po_popup';
			...
			emailwindow.onclose=function()
			{
				...
				// alert(update_id +'!=""&&'+   hdn_is_sales+'!='+hidden_is_sales);
				if (update_id!="" && (hdn_is_sales!=hidden_is_sales))
				{
					alert('Sales Order and Order Mixed not allowed');return;
				}
				if (hidden_is_sales==1) 
				{
					var data="action=populate_details_data_for_sales&type=1"+ get_submitted_data_string('cbo_company_id*cbo_location_name*hidden_data*update_id',"../");
				}
				else
				{
	...
	function generate_report2(program_id)
	{ 
		var company_id = $('#cbo_company_id').val();
		var path='../';
		var page = 'planning_info_entry';
		print_report( company_id+'*'+program_id+'*'+path+'*'+page, "print", "../prod_planning/requires/yarn_requisition_entry_controller" ) 
	}

	function open_mypage(program_id,po_id,prod_id,body_part_id,color_id,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/fabric_requisition_for_batch_entry_controller_3.php?companyID='+companyID+'&program_id='+program_id+'&po_id='+po_id+'&prod_id='+prod_id+'&body_part_id='+body_part_id+'&color_id='+color_id+'&hdn_is_sales='+hdn_is_sales+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../../');
	}

	function openmypage_collar_cuff(i,program_id,po_id,prod_id,body_part_id,color_id,bookingNo) 
	{
    	var cbo_company_id = $('#cbo_company_id').val();
        var update_id = $('#update_id').val();
        var data_string = $('#txt_collar_cuff_string'+i).val();
    	...
    	var page_link = 'requires/fabric_requisition_for_batch_entry_controller_3.php?action=collar_cuff_qnty_popup&cbo_company_id=' + cbo_company_id + '&program_id=' + program_id + '&po_id=' + po_id + '&prod_id=' + prod_id + '&body_part_id=' + body_part_id + '&color_id=' + color_id + '&update_id=' + update_id + '&bookingNo=' + bookingNo + '&save_data=' + data_string;

    	emailwindow = dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=690px,height=270px,center=1,resize=1,scrolling=0', '');

    	emailwindow.onclose = function () {
    		var theform = this.contentDoc.forms[0];
    		var save_data = this.contentDoc.getElementById("hidden_save_string_data").value;
    		var tot_qnty = this.contentDoc.getElementById("hidden_tot_qnty").value;

    		$('#txt_collar_cuff_qnty'+i).val(tot_qnty);
    		$('#txt_collar_cuff_string'+i).val(save_data);
    	}
    }
	...
	function fnc_fabric_requisition_for_batch( operation )
	{
		...
		{
			var report_title=$( "div.form_caption" ).html();
			generate_report_file( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_requisition_no').val()+'*'+report_title,'print_fab_req_for_batch_2','requires/fabric_requisition_for_batch_entry_controller_3');
			return;
		}
		...
		var data="action=save_update_delete&operation="+operation+'&tot_row='+j+get_submitted_data_string('cbo_company_id*cbo_location_name*txt_requisition_date*txt_requisition_no*txt_deleted_id*update_id*hidden_is_sales',"../")+dataString;
	...
	function fnc_fabric_requisition_for_batch_Reply_info()
	{
		if(http.readyState == 4) 
		{
			...
			if((response[0]==0 || response[0]==1))
			{
				...
				var list_view = trim(return_global_ajax_value(response[1], action, '', 'requires/fabric_requisition_for_batch_entry_controller_3'));
				$("#scanning_tbl tbody").html(list_view);
				$('#cbo_company_id').attr('disabled',true);
				fnc_count_total_qty();
				setFilterGrid("scanning_tbl",-1);
    ...
    {	//alert(1);
        ...
        $("#scanning_tbl").find('tbody tr').each(function () 
        {
        	if( $(this).find('input[name="bookintQty[]"]').val() != undefined)
        	{
...
<body onLoad="set_hotkey();">
    ...
    <form name="requisitionEntry_1" id="requisitionEntry_1"> 
		<div align="center" style="width:100%;">
			...
			<br>
			<fieldset style="width:2305px;text-align:left">
				<table cellpadding="0" width="2285" cellspacing="0" border="1" id="scanning_tbl_top" class="rpt_table" rules="all">
                    <thead>
                    	<th width="30">SL</th>
                        ...
                        <th width="80">Collar Cuff Qnty</th>
                        <th width="90">Reqn. Qty. (Kg)</th>
                        <th width="90">Net Issue</th>
                        <th width="90">Remarks</th>
                        <th width="60">Prod. Id</th>
                        <th width="50">&nbsp;</th>
                    </thead>
                 </table>
                 <div style="width:2305px; max-height:250px; overflow-y:scroll" align="left">
                 	<table cellpadding="0" cellspacing="0" width="2285" border="1" id="scanning_tbl" rules="all" class="rpt_table">
                    	<tbody id="tbl_tbody">
                ...
                <br>
                <table width="1905" cellpadding="0" cellspacing="0" border="1" id="" rules="all">
                    <tr>
                        <td align="center" class="button_container">
                            ...
                            ?>
                            <input type="button" class="formbutton" id="Print2" style="width:120px;" value="Print 2" onClick="fnc_fabric_requisition_for_batch(5)" />

							<input type="hidden" id="hidden_data" name="hidden_data" />
							<input type="hidden" id="txt_deleted_id" name="txt_deleted_id" />
							<input type="hidden" id="hidden_is_sales" name="hidden_is_sales" />
                        </td>
                    </tr>  
                </table>
			</fieldset>
