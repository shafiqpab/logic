...
<script>
	...
	function fn_order()
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1055px,height=420px,center=1,resize=1,scrolling=0','../');
		emailwindow.onclose=function()
		{
			...
			var list_view_po =return_global_ajax_value( hidden_job_no+"__"+hidden_color_id+"__"+cbo_trim_type+"__"+company+"__"+hdn_data_generate_level, 'cut_item_details', '', 'requires/trims_issue_requisition_v2_controller');
			...
			if(hdn_data_generate_level==2)
			{
				...
				var all_po_id =return_global_ajax_value( hidden_job_no, 'job_wise_po_data', '', 'requires/trims_issue_requisition_v2_controller');
				fn_item_details(all_po_id, hidden_color_id, 0);
			}
	...
	function fn_item_details(po_id, gmst_color_id, mst_id)
	{
		var store_id = $("#cbo_store_name").val();
		var hdn_reqsn_data_source_variable = $("#hdn_reqsn_data_source_variable").val();
		var store_id = $("#cbo_store_name").val();
		...
		if(mst_id==0)
		{
			var list_view_po =return_global_ajax_value( po_id+"__"+gmst_color_id+"__"+mst_id+"__"+hdn_reqsn_data_source_variable+"__"+store_id, 'product_details', '', 'requires/trims_issue_requisition_v2_controller');
			set_button_status(0, permission, 'fnc_requisition_entry',1,1);
		}
		else
		{
			var list_view_po =return_global_ajax_value( po_id+"__"+gmst_color_id+"__"+mst_id+"__"+hdn_reqsn_data_source_variable+"__"+store_id, 'product_details_update_input', '', 'requires/trims_issue_requisition_v2_controller');
	...
	function fnc_requisition_entry(operation)
	{
		if(operation==4)
		{
			 ...
			 print_report( $('#cbo_working_company').val()+'*'+$('#cbo_working_location').val()+'*'+$('#update_id').val()+'*'+report_title, "garments_exfactory_print", "requires/trims_issue_requisition_v2_controller" )
		...
		if(operation==5)
		{
			...
			var report_title=$( "div.form_caption" ).html();
			print_report( $('#cbo_working_company').val()+'*'+$('#cbo_working_location').val()+'*'+$('#update_id').val()+'*'+report_title, "garments_exfactory_print_job", "requires/trims_issue_requisition_v2_controller" )
		...
		if(operation==0 || operation==1 || operation==2)
		{
			...
			else
			{
				...
				$("#tbl_item_details").find('tbody tr').each(function()
				{
					...
					var reruired_qnty=trim($('#tdRrequiredQnty_'+i).html())*1;
					var prev_req_qnty=trim($('#tdPrevReqQnty_'+i).html())*1;
					var bookQnty=trim($('#tdBookQnty_'+i).html())*1;
					...
					var stock_qnty=trim($('#tdStockQnty_'+i).html())*1;
					var store_rcv_qnty=trim($('#tdStoreRcvQnty_'+i).html())*1;
					...
					{
						j++;
						dataString+='&job_no' + j + '=' + job_no + '&gmts_color_id' + j + '=' + gmts_color_id + '&order_id' + j + '=' + order_id + '&book_dtls_id' + j + '=' + book_dtls_id + '&pre_cost_dtls_id' + j + '=' + pre_cost_dtls_id+ '&item_group' + j + '=' + item_group + '&itemdescription' + j + '=' + itemdescription+ '&cbouom' + j + '=' + cbouom + '&bookQnty' + j + '=' + bookQnty + '&reqQnty' + j + '=' + reqQnty+ '&txtRemarks' + j + '=' + txtRemarks+ '&updateId' + j + '=' + updateId+ '&reruired_qnty' + j + '=' + reruired_qnty+ '&prev_req_qnty' + j + '=' + prev_req_qnty+ '&prodId' + j + '=' + prodId+ '&stock_qnty' + j + '=' + stock_qnty+ '&store_rcv_qnty' + j + '=' + store_rcv_qnty;
					}
					i++;
				});
				
				var data="action=save_update_delete&operation="+operation+"&row_num="+j+get_submitted_data_string('txt_req_no*update_id*cbo_working_company*cbo_working_location*cbo_floor_name*cbo_sewing_line*txt_req_date*txt_delivery_date*cbo_trim_type*txt_remarks*cbo_store_name',"../")+dataString;
	...
	function fnc_requisition_entry_Reply_info()
	{
	 	if(http.readyState == 4)
		{
			...
			else if(reponse[0]==0 || reponse[0]==1)
			{
				...
				show_list_view(trim(reponse[1]),'product_details_update','requisition_details_container_update','requires/trims_issue_requisition_v2_controller','');
	...
	function return_system_popup() //Return PopUp
	{
		...
		emailwindow.onclose=function()
		{
			...
			if(return_id !="")
			{
				...
				show_list_view(return_id,'product_details_update','requisition_details_container_update','requires/trims_issue_requisition_v2_controller','');
				set_button_status(1, permission, 'fnc_requisition_entry',1,1);
			}
		}
	}
	...
	function fn_variable_data(com_id)
	{
		...
		$("#hdn_reqsn_basis_variable").val(trim(prod_variable_data[0]));
		$("#hdn_reqsn_data_source_variable").val(trim(prod_variable_data[1]));
		$("#hdn_data_generate_level").val(trim(prod_variable_data[2]));
		//cbo_trim_type
    ...
    <div style="width:1450px; float:left">
        ...
        <fieldset style="width:1400px;">
            <legend>Requisition Master</legend>
                ...
                <table cellpadding="0" cellspacing="1" width="1320" class="rpt_table" border="1" rules="all" id="tbl_item_details" align="left">
                    <thead>
                    	<tr>
                            <th width="30">SL</th>
                    ...
                    <tr>
                        <td align="center" colspan="10" valign="middle" class="button_container">
                            ...
                            ?>
                             ...
                             <input type="button" id="btn_print2" name="btn_print2"  style="width:80px;"  class="formbutton" value="Print2"  onClick="fnc_requisition_entry(5);"/>
                             <input type="button" id="btn_print3" name="btn_print3"  style="width:80px;"  class="formbutton" value="Print3"  onClick="fnc_requisition_entry(6);"/>
