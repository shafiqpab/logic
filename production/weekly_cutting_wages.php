<?
/*-------------------------------------------- Comments
Purpose			: 	This form created Weekly Cutting wages
...
Updated by 		: 		
Update date		: 		   
QC Performed BY	: Rana,Sujon,Nasir		 
QC Date			:	
Comments		:
*/ 
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Weekly Cutting Wages Entry Info","../", 1, 1, "",'1','');
...
<script>
	...
	{ 
		...
		else
		{
			...
			var page_link = 'requires/weekly_cutting_wages_controller.php?cbo_company_id='+cbo_company_id+'&action=systemId_popup';
			...
			emailwindow.onclose=function()
			{
				...
				get_php_form_data(mst_id[0], "populate_cutting_wages_mst_form_data", "requires/weekly_cutting_wages_controller" );
				show_list_view(mst_id[0],'show_weekly_wages_bill_listview','list_container','requires/weekly_cutting_wages_controller','');
				
				disable_enable_fields('cbo_company_id*cbo_final_bill*cbo_location*cbo_division*cbo_department*cbo_shift*txt_week_date_from*txt_week_date_to',1);
	...
	function openpage_emp_id()
	{
		...
		else
		{
			...
			var page_link = 'requires/weekly_cutting_wages_controller.php?cbo_company_id='+cbo_company_id+'&cbo_bill_for='+cbo_bill_for+'&action=employee_info_popup';
			...
			emailwindow.onclose=function()
			{
				...
				get_php_form_data(emp_id, "populate_emp_info", "requires/weekly_cutting_wages_controller");
			}
		}
	}
	...
	function openpage_search_order()
	{
		...
		if (form_validation('cbo_company_id*cbo_bill_for*cbo_final_bill*cbo_location*txt_week_date_from*txt_emp_name','Company*Bill for*Finall bill*Location*Week Date*Service Provider')==false)
		{
			return;
		}
		else
		{
			...
			var page_link = 'requires/weekly_cutting_wages_controller.php?action=order_info_popup&mst_data='+cbo_company_id+'_'+cbo_bill_for+'_'+txt_week_date_from+'_'+txt_week_date_to+'_'+cbo_location+'_'+cbo_shift+'_'+txt_emp_code+'&txt_order_id='+txt_order_id+'&txt_gmt_item_id='+txt_gmt_item_id;
			...
			emailwindow.onclose=function()
			{
				...
				//var data=hidden_order_id+'_'+hidden_job_id+'_'+hidden_rate+'_'+hidden_rate_var_id+'_'+hidden_style_ref+'_'+hidden_item_id+'_'+cbo_company_id+'_'+cbo_location+'_'+cbo_bill_for+'_'+cbo_final_bill+'_'+txt_week_date_from+'_'+txt_week_date_to+'_'+txt_emp_code+'_'+dtls_update_id;
				var data=hidden_order_id+'**'+hidden_job_id+'**'+hidden_rate+'**'+hidden_rate_var_id+'**'+hidden_style_ref+'**'+hidden_item_id+'**'+cbo_company_id+'**'+cbo_location+'**'+cbo_bill_for+'**'+cbo_final_bill+'**'+txt_week_date_from+'**'+txt_week_date_to+'**'+txt_emp_code+'**'+dtls_update_id;
				
				
				get_php_form_data(data, "populate_order_data", "requires/weekly_cutting_wages_controller");
			}
		}
	}
	...
	{
		window.open("requires/weekly_cutting_wages_controller.php?data=" + data+'&action='+action, true );
	}

	
	function fnc_weekly_wages_bill_entry( operation )
	{
		if(operation==4)
		{
		...
		if(operation==0 || operation==1 || operation==2)
		{
			//show_msg('13');
			//return;
			if( form_validation('cbo_company_id*cbo_bill_for*cbo_final_bill*cbo_location*txt_week_date_from*txt_week_date_to*txt_emp_name*txt_order_no','Company*Bill For* Final Bill*Location*From Date*To Date*Emp Card No*Order No')==false )
			{
				return;
			}	
	
			var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_system_id*update_id*dtls_update_id*cbo_company_id*cbo_bill_for*cbo_final_bill*cbo_location*cbo_division*cbo_department*cbo_shift*txt_week_date_from*txt_week_date_to*txt_emp_code*txt_emp_name*txt_order_id*cbo_buyer_name*txt_style_ref_no*txt_gmt_item_name*txt_appv_wo_order_qty*cbo_wages_rate_variables*txt_cutting_bill_qty*txt_deducted_qty_hidden*hidden_order_break_down*txt_wo_rate*txt_amount*tex_emp_deduct_amount*txt_net_amount*txt_previous_bill_qty*txt_yet_to_bill_qty*cbo_cutting_bill_uom*cbo_wo_rate_uom*cbo_previous_bill_qty_uom*cbo_yet_to_bill_qty_uom',"../");
			
			freeze_window(operation);
			http.open("POST","requires/weekly_cutting_wages_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
			http.onreadystatechange = fnc_weekly_wages_bill_entry_reponse;
	
		}
	}
	
	
	function fnc_weekly_wages_bill_entry_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			if(reponse[0]==0 || reponse[0]==1)
			{
				...
				show_list_view(reponse[1],'show_weekly_wages_bill_listview','list_container','requires/weekly_cutting_wages_controller','');
	...
	function fnc_enable(){
		disable_enable_fields('cbo_company_id*cbo_final_bill*cbo_location*cbo_division*cbo_department*cbo_shift*txt_week_date_from*txt_week_date_to',0);
	set_button_status(0, permission, 'fnc_weekly_wages_bill_entry',1);	
	}
	...
	function fnc_net_amount(deducted){
		...
		var deducted = deducted.replace(',', '');
		var amount = amount.replace(',', '');
		var deducte_amount = parseFloat(amount)-parseFloat(deducted);
		$('#txt_net_amount').val(deducte_amount);	
	}
...
<? echo load_freeze_divs ("../",$permission); ?>
    <form name="weekly_wages_bill_1" id="weekly_wages_bill_1" autocomplete="off" >
        <fieldset>
            <legend>&nbsp;Weekly Cutting Wages</legend>
				<fieldset>
						...
						<table cellpadding="0" cellspacing="2" width="100%">
							<tr>
								...
								<input type="hidden" id="dtls_update_id" name="dtls_update_id" />
									...
									<input type="hidden" id="cbo_bill_for" name="cbo_bill_for" value="20"/>
								</td>
							</tr>
							<tr>
									<td colspan="7">&nbsp;</td>
								...
								<tr>
									...
									<td width="170">
										<?
										// echo create_drop_down( "cbo_company_id", 160, $company_arr,"", 1, "--Select Company--", 0, "","","1,3" );
										echo create_drop_down( "cbo_company_id", 160, $company_arr,"", 1, "--Select Company--", 0, "load_drop_down('requires/weekly_cutting_wages_controller', this.value, 'load_drop_down_location', 'location_td' );","","1,3" );
										?>
									</td>
									<td width="130" class="must_entry_caption" align="right">Final Bill</td>
									<td width="170">
										<?= create_drop_down( "cbo_final_bill", 160, $yes_no,"", 1, "--Select Final Bill--", 2, "" ); ?>
									</td>
									<td width="130" class="must_entry_caption" align="right">Location</td>	
									<td width="170" id="location_td">
										<?= create_drop_down( "cbo_location", 160, $location_details,"", 1, "--Select Location--", 0, "" );?>
								</td>	
								</tr>
								<tr>
									<td width="130" align="right">Division Name</td>
									<td width="170">
										<?= create_drop_down( "cbo_division", 160, $division_details,"", 1, "--Select Division--", 0, "" ); ?>
									</td>
									<td width="130" align="right">Department Name</td>
									<td width="170">
										<?= create_drop_down( "cbo_department", 160, $department_details,"", 1, "--Select Department--", 0, "" );?>
									</td>
									<td width="130" align="right">Shift</td>
									<td width="170">
										<?= create_drop_down( "cbo_shift", 160, $shift_name,"", 1, "--Select Shift--", 1, "" );?>
									</td>
								</tr>
								<tr>
                ...
                <fieldset>
                    ...
                    <table cellpadding="0" cellspacing="2" width="850" border="0">
						<tr>
							<td width="140" align="right">Service Provider</td>
							<td width="170">
                    			<input type="hidden" name="txt_emp_card_no" id="txt_emp_card_no" />
                                <input type="hidden" name="txt_emp_code" id="txt_emp_code">
                                <input type="text" name="txt_emp_name" id="txt_emp_name" class="text_boxes" style="width:150px"  tabindex="13" placeholder="Double Click to Search" readonly onDblClick="openpage_emp_id()" />
                    		</td>
						...
						<tr>
                            ...
                            <td width="170">
								<?= create_drop_down( "cbo_buyer_name", 160, "select buyer_name,id from lib_buyer where is_deleted=0  and status_active=1 order by buyer_name","id,buyer_name", 1, "--Select Buyer--", 0, "",1 );?>
                            </td>
                            <td width="120" align="right">Cutting/Bill Qty</td>
							<td width="100">
                            ...
                            <td width="60">
								<?= create_drop_down( "cbo_cutting_bill_uom", 60, $unit_of_measurement,"",1, "--Select--", "","",1,"1,2" );?>
                            </td>                  
						</tr>
						<tr>
							...
							<td width="100">
                                <input type="text" name="txt_wo_rate" id="txt_wo_rate" class="text_boxes" tabindex="23" style="width:107px;text-align:right" disabled />
                            </td>
                            <td width="60">
								<?= create_drop_down( "cbo_wo_rate_uom", 60,$unit_of_measurement,"",1, "--Select--", "","",1,"1,2" );?>
                            </td> 
                    	</tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td width="170" id="item_group_td_id">
								<?= create_drop_down( "cbo_wages_rate_variables", 160,$color_type,"", 1, "--Rate Variable--", 0, "" ,1);?>
                            </td>
                        ...
                        <tr>
                            ...
                            <td width="60">
								<?= create_drop_down( "cbo_previous_bill_qty_uom",60,$unit_of_measurement,"",1, "--Select--", "","",1,"1,2" );
                                ?>
                            </td>
                    	</tr>		
                        <tr>
                            ...
                            <td width="60">
								<?= create_drop_down( "cbo_yet_to_bill_qty_uom", 60,$unit_of_measurement,"",1, "--Select--", "","",1,"1,2" );?>
                            </td>
                        </tr>
                        <tr><td colspan="7" height="8"></td></tr>
                		<tr>
                    		<td colspan="7" align="center" class="button_container">
								<?= load_submit_buttons($permission, "fnc_weekly_wages_bill_entry", 0,1,"reset_form('weekly_wages_bill_1','list_container','','','fnc_enable()','cbo_bill_for')",1);?>
                    		</td>
                 		</tr>
					</table>        
                </fieldset>      
