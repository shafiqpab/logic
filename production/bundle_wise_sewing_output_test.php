...
<script>
	...
	function openmypage_sysNo()
	{
		...
		var page_link = 'requires/bundle_wise_sewing_output_test_controller.php?cbo_company_name='+cbo_company_name+'&cbo_source='+cbo_source+'&cbo_serving_company='+cbo_serving_company+'&action=challan_no_popup';
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				{ 
					...
					get_php_form_data(mst_id, "populate_data_from_challan_popup", "requires/bundle_wise_sewing_output_test_controller" );
					
					var delivery_basis=$('#delivery_basis').val();
					if(delivery_basis==3)
					{
						var bundle_nos=return_global_ajax_value(mst_id, 'bundle_nos', '', 'requires/bundle_wise_sewing_output_test_controller');
						var response_data=return_global_ajax_value(trim(bundle_nos)+"**0**"+mst_id+"**"+cbo_company_name, 'populate_bundle_data_update', '', 'requires/bundle_wise_sewing_output_test_controller');
					...
					{
						show_list_view(mst_id,'show_dtls_listview','printing_production_list_view','requires/bundle_wise_sewing_output_test_controller','');
					}
					release_freezing();
				}
			}
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				{ 
					...
					get_php_form_data(po_id+'**'+item_id+'**'+$('#cbo_embel_name').val()+'**'+country_id, "populate_data_from_search_popup", "requires/bundle_wise_sewing_output_test_controller" );
					...
					else
					{
						...
						get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+$("#cbo_embel_name").val()+'**'+country_id, "color_and_size_level", "requires/bundle_wise_sewing_output_test_controller" ); 
					}
					
					show_list_view(po_id,'show_country_listview','list_view_country','requires/bundle_wise_sewing_output_test_controller','');	
	...
	{
		window.open("requires/bundle_wise_sewing_output_test_controller.php?data=" + data+'&action='+action, true );
	}

	function fnc_sewing_bundle_output_entry(operation)
	{
		var company_id=$('#cbo_company_name').val();
		/*var working_company_mandatory=return_global_ajax_value(company_id, 'load_variable_settings_for_working_company', '', 'requires/bundle_wise_sewing_output_test_controller');
		...
		if(operation==4)
		{
			...
			generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val()+'*'+report_title, 'emblishment_issue_print', 'requires/bundle_wise_sewing_output_test_controller'); 
			 return;
		}
		else if(operation==5)
		{
			...
			'emblishment_issue_print_2', 'requires/bundle_wise_sewing_output_test_controller');
			return;
		}
		...
		if(operation==0 || operation==1 || operation==2)
		{
			...
			http.open("POST","requires/bundle_wise_sewing_output_test_controller.php",true);
		...
		if(http.readyState == 4) 
		{
			...
			else if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
			{
				...
				else
				{
					...
					show_list_view(reponse[1],'show_dtls_listview','printing_production_list_view','requires/bundle_wise_sewing_output_test_controller','');
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				{					
					...
					if( (tot_row*1) <1)
						get_php_form_data(hidden_bundle_nos+"__"+hidden_bundle_line+"__"+$('#txt_issue_date').val(),'load_mst_data','requires/bundle_wise_sewing_output_test_controller');
	...
	function fnc_duplicate_bundle(bundle_no)
	{
		...
		var rcv_date = $('#txt_issue_date').val();
		var challan_duplicate=return_ajax_request_value( bundle_no+"__"+$('#cbo_company_name').val()+"__"+$('#cbo_line_no').val()+"__"+row_num+"__"+rcv_date,"challan_duplicate_check", "requires/bundle_wise_sewing_output_test_controller");
		...
		else
		{
			...
			{
				var page_link='requires/bundle_wise_sewing_output_test_controller.php?action=bundle_popup_line_select&data='+ex_challan_duplicate[5]+'&company_id='+$('#cbo_company_name').val();
				...
				emailwindow.onclose=function()
				{
					...
					if( (tot_row*1) <2)
						get_php_form_data(bundle_no+"__"+ex_challan_duplicate[4]+"__"+$('#txt_issue_date').val(),'load_mst_data','requires/bundle_wise_sewing_output_test_controller');
				}
			}
			else
			{
				// create_row(bundle_no);
				// var vline_no=$('#cbo_line_no').val();			
				
				// var row_num=$('#txt_tot_row').val();
				// var response_data_msg=return_global_ajax_value(bundle_nos+"**"+row_num+"****"+$('#cbo_company_name').val()+"**"+vline_no, 'populate_bundle_data_check', '', 'requires/bundle_wise_sewing_output_test_controller');
				if(ex_challan_duplicate[1]==2)
				{
					alert('All Bundle must be under Selected Company/line. Please Check');
					release_freezing();
					return;
				}
				else if(ex_challan_duplicate[1]==3)
				{
				...
				// var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"****"+$('#cbo_company_name').val()+"**"+$('#cbo_line_no').val(), 'populate_bundle_data', '', 'requires/bundle_wise_sewing_output_test_controller');
				// alert(ex_challan_duplicate[2]);
				$('#tbl_details tbody').prepend(ex_challan_duplicate[2]);
				var tot_row=$('#tbl_details tbody tr').length; 
				$('#txt_tot_row').val(tot_row);
				release_freezing();
	...
	function create_row_rescan(bundle_nos,vline_no,is_defect)
	{
		...
		var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"****"+$('#cbo_company_name').val()+"**"+$('#cbo_line_no').val()+"**"+is_defect, 'populate_bundle_data_rescan', '', 'requires/bundle_wise_sewing_output_test_controller');
		$('#tbl_details tbody').prepend(response_data);
		var tot_row=$('#tbl_details tbody tr').length; 
		$('#txt_tot_row').val(tot_row);
		release_freezing();
	...
	function fnc_duplicate_bundle_rescan(bundle_no)
	{
		...
		var challan_duplicate=return_ajax_request_value( bundle_no+"__"+$('#cbo_company_name').val(),"qty_rescan_check", "requires/bundle_wise_sewing_output_test_controller");
		...
		else
		{
			...
			{
				var page_link='requires/bundle_wise_sewing_output_test_controller.php?action=bundle_popup_line_select&data='+ex_challan_duplicate[2]+"&company_id="+$('#cbo_company_name').val();
	...
	function pop_entry_reject(row_id)
	{
		...
		var page_link='requires/bundle_wise_sewing_output_test_controller.php?action=reject_qty_popup&actual_infos='+actual_infos;
	...
	function pop_entry_spot(row_id)
	{
		...
		var page_link='requires/bundle_wise_sewing_output_test_controller.php?action=spot_qty_popup&actual_infos='+actual_infos;
	...
	function change_mode(source_id)
	{
		if(source_id==1)
		{
			get_php_form_data($('#cbo_company_name').val(),'load_variable_settings','requires/bundle_wise_sewing_output_test_controller'); get_php_form_data($('#cbo_company_name').val(),'load_variable_settings_for_working_company','requires/bundle_wise_sewing_output_test_controller'); load_html();
				
		}
		else
		{
			get_php_form_data($('#cbo_company_name').val(),'load_variable_settings','requires/bundle_wise_sewing_output_test_controller'); get_php_form_data($('#cbo_emb_company').val(),'load_variable_settings_for_working_company','requires/bundle_wise_sewing_output_test_controller'); load_html();
                ...
                <table width="100%">
                    ...
                    <tr>
                        ...
                        <td>
                          ...
                          <td>
                              <? 
                              	echo create_drop_down( "cbo_source", 180, $knitting_source,"", 1, "-- Select Source --", $selected, "load_drop_down( 'requires/bundle_wise_sewing_output_test_controller', this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_embro_issue_source', 'emb_company_td' );load_drop_down( 'requires/bundle_wise_sewing_output_test_controller', $('#cbo_company_name').val(), 'load_drop_down_location', 'location_td' );change_mode(this.value);", 0, '1,3' );
                              ?>
                          </td>
                    ...
                    <tr>
                         ...
                         <td> 
                            <input name="txt_reporting_hour" id="txt_reporting_hour" class="text_boxes" style="width:167px" placeholder="24 Hour Format" onBlur="fnc_valid_time(this.value,'txt_reporting_hour');" onKeyUp="fnc_valid_time(this.value,'txt_reporting_hour');" onKeyPress="return numOnly(this,event,this.id);" maxlength="8" />
                         </td>
                    </tr>
                    <tr style="display:none;">
                     <td id="working_company">Working Company</td>
                        <td>
                        	<? 
								echo create_drop_down( "cbo_working_company_name", 180, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select --",$selected,"load_drop_down( 'requires/bundle_wise_sewing_output_test_controller', $('#cbo_working_company_name').val(), 'load_drop_down_working_location', 'working_location_td' );" );
                            ?>
                        </td>
                    ...
                    <tr>
                    	<td>Remarks</td>
                        <td colspan="5">
                            <input name="txt_remark" id="txt_remark" class="text_boxes" type="text" style="width:94%" />
                        </td>
                    </tr>
                                       ...
                                       <table  cellpadding="0" cellspacing="2" width="100%">
                                          <tr>
                                               ...
                                               <td colspan="3" width="110"> 
                                                   <input name="txt_order_no" placeholder="Double Click to Search" onDblClick="openmypage('requires/bundle_wise_sewing_output_test_controller.php?action=order_popup&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value,'Order Search')" id="txt_order_no" class="text_boxes" style="width:212px" readonly />
                              ...
                              <legend>New Entry</legend>
                               <table  cellpadding="0" cellspacing="2" width="100%">
                                  <tr>
                                       ...
                                       <td colspan="3" width="110"> 
                                           <input name="txt_bundle_no" placeholder="Browse/Write/Scan" onDblClick="openmypage_bundle('requires/bundle_wise_sewing_output_test_controller.php?action=bundle_popup&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value,'Bundle Search')" id="txt_bundle_no" class="text_boxes" style="width:212px" />
                                       </td>
                                       <td width="100" class="must_entry_caption" id="td_caption">Re-Scan Barcode</td>
                                       <td colspan="2" width="110"> 
                                           <input name="txt_bundle_rescan" placeholder="Browse/Write/Scan" onDblClick="openmypage_bundle_rescan('requires/bundle_wise_sewing_output_test_controller.php?action=bundle_popup_rescan&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value,'Search Bundle For Rescan')"  id="txt_bundle_rescan" class="text_boxes" style="width:212px" />
                                       </td>
                                    </tr>
                                </table>
                            </fieldset>
