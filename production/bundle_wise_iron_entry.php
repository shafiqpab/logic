...
function openmypage_sysNo()
{
	...
	var page_link = 'requires/bundle_wise_iron_entry_controller.php?cbo_company_name='+cbo_company_name+'&action=challan_no_popup';
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				get_php_form_data(mst_id, "populate_data_from_challan_popup", "requires/bundle_wise_iron_entry_controller" );
				
				var delivery_basis=$('#delivery_basis').val();
				if(delivery_basis==3)
				{
					var bundle_nos=return_global_ajax_value(mst_id, 'bundle_nos', '', 'requires/bundle_wise_iron_entry_controller');
					var response_data=return_global_ajax_value(trim(bundle_nos)+"**0**"+mst_id+"**"+cbo_company_name, 'populate_bundle_data_update', '', 'requires/bundle_wise_iron_entry_controller');
					...
					set_button_status(1, permission, 'fnc_iron_bundle_entry',1,0);
				}
				else
				{
					show_list_view(mst_id,'show_dtls_listview','printing_production_list_view','requires/bundle_wise_iron_entry_controller','');
				}
				release_freezing();
			}
		}
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				get_php_form_data(po_id+'**'+item_id+'**'+$('#cbo_embel_name').val()+'**'+country_id, "populate_data_from_search_popup", "requires/bundle_wise_iron_entry_controller" );
				...
				else
				{
					...
					get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+$("#cbo_embel_name").val()+'**'+country_id, "color_and_size_level", "requires/bundle_wise_iron_entry_controller" ); 
				}
				
				show_list_view(po_id,'show_country_listview','list_view_country','requires/bundle_wise_iron_entry_controller','');	
				set_button_status(0, permission, 'fnc_iron_bundle_entry',1,0);
				release_freezing();
			}
		}
	}//end else
...
function generate_report_file(data,action,page)
	{
		window.open("requires/bundle_wise_iron_entry_controller.php?data=" + data+'&action='+action, true );
	}


function fnc_iron_bundle_entry(operation)
{
	var company_id=$('#cbo_company_name').val();
	/*var working_company_mandatory=return_global_ajax_value(company_id, 'load_variable_settings_for_working_company', '', 'requires/bundle_wise_iron_entry_controller');
		...
		if(delivery_basis==3)
		{
			...
			$("#tbl_details").find('tbody tr').each(function()
			{
				...
				try 
				{
					...
					dataString+='&bundleNo_' + j + '=' + bundleNo + '&orderId_' + j + '=' + orderId + '&gmtsitemId_' + j + '=' + gmtsitemId + '&countryId_' + j + '=' + countryId + '&colorId_' + j + '=' + colorId + '&sizeId_' + j + '=' + sizeId + '&colorSizeId_' + j + '=' + colorSizeId + '&qty_' + j + '=' + qty + '&dtlsId_' + j + '=' + dtlsId + '&rejectQty_' + j + '=' + rejectQty + '&replaceQty_' + j + '=' + replaceQty + '&cutNo_' + j + '=' + cutNo;
				}
				catch(e) 
				{
...
function fnc_issue_print_embroidery_Reply_info()
{
 	if(http.readyState == 4) 
	{
		...
		if(reponse[0]==15) 
		{ 
			 setTimeout('fnc_iron_bundle_entry('+ reponse[1]+')',8000); 
		}
		...
		else if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
		{
			...
			if(delivery_basis==3)
			{
				set_button_status(1, permission, 'fnc_iron_bundle_entry',1,1);
			}
			else
			{
				...
				show_list_view(reponse[1],'show_dtls_listview','printing_production_list_view','requires/bundle_wise_iron_entry_controller','');
				set_button_status(0, permission, 'fnc_iron_bundle_entry',1,1);
			}
		}
		release_freezing();
 	}
...
function fn_colorlevel_total(index) //for color level
{
	...
	{
		var challan_duplicate=return_ajax_request_value( bundle_no+"__"+$('#cbo_company_name').val(),"challan_duplicate_check", "requires/bundle_wise_iron_entry_controller");
		...
		else
		{
			...
			if( (tot_row*1) <1)
				get_php_form_data(bundle_no,'load_mst_data','requires/bundle_wise_iron_entry_controller');	
			else
				get_php_form_data(bundle_no+"__"+$('#cbo_line_no').val(),'load_mst_data','requires/bundle_wise_iron_entry_controller');
		}
		$('#txt_bundle_no').val('');
	}
...
function create_row(bundle_nos)
{
	...
	var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"****"+$('#cbo_company_name').val()+"**"+$('#cbo_line_no').val(), 'populate_bundle_data', '', 'requires/bundle_wise_iron_entry_controller');
	$('#tbl_details tbody').prepend(response_data);
	var tot_row=$('#tbl_details tbody tr').length; 
	$('#txt_tot_row').val(tot_row);
	release_freezing();
...
{ 
	...
	var qc_qty=(prodQty-rejectQty)+replaceQty;
	
	if(prodQty<qc_qty)
	{
		qc_qty=qc_qty=(prodQty-rejectQty);
		$("#replaceQty_"+id).val('');
	}
	
	if(rejectQty>=prodQty)
	{
		$("#rejectQty_"+id).val('');
	}
	else
            ...
            <fieldset style="width:930px; margin:0 auto;">
                <table width="100%">
                    ...
                    <tr>
                        ...
                        <td>
                            <? 
                            	echo create_drop_down( "cbo_company_name", 180, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "get_php_form_data(this.value,'load_variable_settings','requires/bundle_wise_iron_entry_controller');get_php_form_data(this.value,'load_variable_settings_for_working_company','requires/bundle_wise_iron_entry_controller'); load_html();" );
                            ?>
                          ...
                          <td>
                              <? 
                              	echo create_drop_down( "cbo_source", 180, $knitting_source,"", 1, "-- Select Source --", $selected, "load_drop_down( 'requires/bundle_wise_iron_entry_controller', this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_embro_issue_source', 'emb_company_td' );load_drop_down( 'requires/embroidery_delivery_entry_controller', $('#cbo_company_name').val(), 'load_drop_down_location', 'location_td' );", 0, '1,3' );
                              ?>
                          </td>
                          <td class="must_entry_caption">Iron Company</td>
                    ...
                    <tr>
                         <td class="must_entry_caption">Iron Date</td>
                         <td> 
                     ...
                     <td id="working_company">Working Company</td>
                        <td>
                        	<? 
								echo create_drop_down( "cbo_working_company_name", 180, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select --",$selected,"load_drop_down( 'requires/bundle_wise_iron_entry_controller', $('#cbo_working_company_name').val(), 'load_drop_down_working_location', 'working_location_td' );" );
                            ?>
                        </td>
                ...
                <div id="bundle_list_view" style="display:none">
                	<table cellpadding="0" width="1080" cellspacing="0" border="1" class="rpt_table" rules="all">
                        <thead>
                            <th width="30">SL</th>
                    ...
                    <div style="width:1080px; max-height:250px; overflow-y:scroll" align="left">    
                    	<table cellpadding="0" width="1060" cellspacing="0" border="1" class="rpt_table" rules="all" id="tbl_details">      
                            <tbody>
                            </tbody>
                        </table>
                	</div>
               	...
               	<table cellpadding="0" cellspacing="1" width="100%">
               		<tr>
                        <td align="center" colspan="9" valign="middle" class="button_container">
                            <?
								$date=date('d-m-Y');
                                echo load_submit_buttons( $permission, "fnc_iron_bundle_entry", 0,1 ,"reset_form('printembro_1','list_view_country','','txt_issue_date,".$date."','pageReset();')",1); 
                            ?>
