...
function openmypage_booking(page_link,title)
{
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			var theemail_sales_order_id=this.contentDoc.getElementById("sales_order_id");
			var theemail_sales_order_type=this.contentDoc.getElementById("sales_order_type");
			//alert(theemail_within_group.value);
			document.getElementById('within_group').value=theemail_within_group.value;
			document.getElementById('sales_order_type').value=theemail_sales_order_type.value;

			if (theemail_booking.value!="")
			{
				//alert(theemail.value);
				var data=theemail_booking.value+'_'+theemail_job.value+'_'+theemail_within_group.value+'_'+theemail_sales_order_id.value+'_'+theemail_sales_order_type.value;
				var data_action='';
				/*if(theemail_within_group.value==2 || sales_order_type.value==1){
					data_action='populate_sales_data_from_search_popup'; 
				}else{
					data_action='populate_data_from_search_popup';
				}*/

				data_action='populate_all_data_from_search_popup';
			 	get_php_form_data( data, data_action, "requires/fabric_issue_for_aop_entry_controller" );
				//alert("mmm");
...
function fnc_trims_booking( operation ){
	...
	if (form_validation('cbo_company_name*txt_order_no*cbo_supplier_name*cbo_currency_id','Company Name*Work Order*Service Company*Currency')==false){
		release_freezing();
		return;
	}
	else
	{
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_system_id*cbo_company_name*txt_issue_date*cbo_issue_purpose*cbo_source*cbo_supplier_name*txt_party_location*txt_order_no*hid_order_id*txt_gate_pass_no*cbo_is_short*txt_remarks*update_id*cbo_currency_id*txt_attention*txt_delivery_date*txt_vehical_no*txt_driver_name*txt_dl_no*txt_transport*txt_cell_no*buyer_id*within_group*cbo_lc_company_name*hid_fab_booking*sales_order_type',"../");
	}
	...
	else
	{
		...
		$('table#tbl_dtls_emb tbody tr').each(function()
		{
			...
			if(check_field==0)
			{
				//alert(check_field);
				j++;
				data_all += "&cboProcessType_" + j + "='" + cboProcessType + "'&txtWoqnty_" + j + "='" + txtWoqnty + "'&txtRate_" + j + "='" + txtRate + "'&txtAmount_" + j + "='" + txtAmount + "'&txtNumberRoll_" + j + "='" + txtNumberRoll  + "'&txtRemarks_" + j + "='" + txtRemarks + "'&soDtlsId_" + j + "='" + soDtlsId+ "'&batchDtlsId_" + j + "='" + batchDtlsId+ "'&batchId_" + j + "='" + batchId + "'&hiddenid_" + j + "='" + hiddenid+ "'&prodId_" + j + "='" + prodId + "'";
...
function fnc_service_booking_dtls_reponse()
{
	if(http.readyState == 4)
	{
		...
		if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
		{
			...
			var data='2'+'**'+update_id+'**'+within_group;
			show_list_view(data, 'generate_aop_booking','booking_list_view','requires/fabric_issue_for_aop_entry_controller','$(\'#hide_fabric_description\').val(\'\')');
...
function fnc_multi_print()
{
	...
	var supplier_name = $("#cbo_supplier_name").val();	
	var page_link='requires/fabric_issue_for_aop_entry_controller.php?action=multi_issue_number_popup&company='+company+'&supplier_name='+supplier_name; 
	...
	{
		var theform=this.contentDoc.forms[0]; 
		var issue_numbers=this.contentDoc.getElementById("hidden_issue_number").value; // mrr number
		var issue_ids=this.contentDoc.getElementById("hidden_issue_id").value; // mrr number
		var within_group=this.contentDoc.getElementById("hidden_within_group_number").value; // order number
		var report_title=$( "div.form_caption" ).html();
		//issue_ids=issue_ids+','+$("#update_id").val();
		//alert(issue_ids);
		print_report( $('#cbo_company_name').val()+'*'+issue_ids+'*'+report_title+'*'+within_group+'*'+issue_numbers, "multi_issue_print", "requires/fabric_issue_for_aop_entry_controller") ;
		return;
...
function fnc_load_address(val){
	var response=return_global_ajax_value(val, 'return_supplier_address', '', 'requires/fabric_issue_for_aop_entry_controller');
	$('#txt_party_location').val(response);
}
                ...
                <table  width="900" cellspacing="2" cellpadding="0" border="0">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <input type="hidden" name="sales_order_type" id="sales_order_type">
                        </td>
                    ...
                    <tr>
                        ...
                        <td colspan="3"><input class="text_boxes" type="text" style="width:160px"  name="txt_remarks" id="txt_remarks"/></td>
                    </tr>
                    <tr>
                ...
                <table width="1480" cellspacing="2" cellpadding="0" border="0">
                    <tr>
                        <td align="center" colspan="14" valign="middle" class="button_container">
                        	...
                        	<input type="button" name="print3" value="Print Multi Issue Number" id="print3" class="formbutton" style="width: 180px;" onClick="fnc_multi_print()" />
                        </td>
...
function fnc_process_data(){
	...
	if (form_validation('cbo_company_name*txt_system_id*txt_order_no','Company*Issue ID*Work Order')==false){
		...
		var hid_fab_booking=document.getElementById('hid_fab_booking').value;
		var is_update=1;
		/*if(update_id==''){
			var is_update=1;
		}else{
		...
		page_link=page_link+'&txt_order_no='+txt_order_no+'&hid_order_id='+hid_order_id+'&cbo_is_short='+cbo_is_short+'&cbo_company_name='+cbo_company_name+'&cbo_supplier_name='+cbo_supplier_name+'&buyer_id='+buyer_id+'&within_group='+within_group+'&sales_order_type='+sales_order_type+'&hid_fab_booking='+hid_fab_booking;
		...
		emailwindow.onclose=function(){
			...
			var theemail3=this.contentDoc.getElementById("txtBatchId");
			var theemail4=this.contentDoc.getElementById("txtBatchDtlsId");
			var txtSoDtlsId=theemail.value;
			var txtPreCostDtlsId=theemail2.value;
			var txtBatchMstId=theemail3.value;
			var txtBatchDtlsId=theemail4.value;

			var row = $("#tbl_dtls_emb tbody tr").length;
			var rowNo = $("#tbl_dtls_emb tbody tr:last").attr('id');
			var valuesLastRow = $("#tbl_dtls_emb tbody tr:last").find('input[name=hiddenid_'+rowNo+']').val();
			//alert(row);
			if(row>1 || (row==1 && valuesLastRow!='' ) && theemail4.value!=""){
				row = row*1+1;
				
				var data=is_update+'**'+txtSoDtlsId+'**'+txtPreCostDtlsId+'**'+txtBatchMstId+'**'+txt_order_no+'**'+within_group+'**'+sales_order_type+'**'+hid_fab_booking+'**'+txtBatchDtlsId+'**'+row;
				//alert(data);
				//1**,**undefined,undefined**12361,12813**UG-FSOE-21-00008**2**0**230221**28788,29952**4
				var responseHtml = return_ajax_request_value(data, 'generate_aop_booking', 'requires/fabric_issue_for_aop_entry_controller');
				//alert(responseHtml);return;
				$("#tbl_dtls_emb tbody").append(responseHtml);
			}
			else{
				if (theemail4.value!=""){
