...
function openmypage_sysNo()
{
	...
	var page_link = 'requires/wash_embro_delivery_entry_controller.php?cbo_company_name='+cbo_company_name+'&action=challan_no_popup';
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				get_php_form_data(mst_id, "populate_data_from_challan_popup", "requires/wash_embro_delivery_entry_controller" );
				
				var delivery_basis=$('#delivery_basis').val();
				if(delivery_basis==3)
				{
					var bundle_nos=return_global_ajax_value(mst_id, 'bundle_nos', '', 'requires/wash_embro_delivery_entry_controller');
					var response_data=return_global_ajax_value(trim(bundle_nos)+"**0**"+mst_id+"**"+cbo_company_name, 'populate_bundle_data', '', 'requires/wash_embro_delivery_entry_controller');
				...
				{
					show_list_view(mst_id,'show_dtls_listview','printing_production_list_view','requires/wash_embro_delivery_entry_controller','');
				}
				release_freezing();
			}
		}
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				get_php_form_data(po_id+'**'+item_id+'**'+$('#cbo_embel_name').val()+'**'+country_id, "populate_data_from_search_popup", "requires/wash_embro_delivery_entry_controller" );
				...
				else
				{
					...
					get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+$("#cbo_embel_name").val()+'**'+country_id, "color_and_size_level", "requires/wash_embro_delivery_entry_controller" ); 
				}
				
				show_list_view(po_id,'show_country_listview','list_view_country','requires/wash_embro_delivery_entry_controller','');	
...
function generate_report_file(data,action,page)
	{
		window.open("requires/wash_embro_delivery_entry_controller.php?data=" + data+'&action='+action, true );
	}
...
function fnc_issue_print_embroidery_entry(operation)
{
	...
	var working_company_mandatory=return_global_ajax_value(company_id, 'load_variable_settings_for_working_company', '', 'requires/wash_embro_delivery_entry_controller');
	...
	if(operation==4)
	{
		...
		generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val()+'*'+report_title, 'emblishment_issue_print', 'requires/wash_embro_delivery_entry_controller'); 
		 return;
	}
	else if(operation==5)
	{
		var report_title=$( "div.form_caption" ).html();
		generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val()+'*'+report_title, 'emblishment_issue_print_2', 'requires/wash_embro_delivery_entry_controller'); 
		 return;
	}
	else if(operation==6)
	{
		var report_title=$( "div.form_caption" ).html();
		generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val()+'*'+report_title, 'emblishment_issue_print_3', 'requires/wash_embro_delivery_entry_controller'); 
		 return;
	}
	else if(operation==7)
	{
		...
		var report_title=$( "div.form_caption" ).html();
		generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val()+'*'+report_title, 'emblishment_issue_print_4', 'requires/wash_embro_delivery_entry_controller'); 
		 return;
	}
	...
	if(operation==0 || operation==1 || operation==2)
	{
		...
		if(delivery_basis==3)
		{
			...
			$("#tbl_details").find('tbody tr').each(function()
			{
				...
				try 
				{
					...
					dataString+='&bundleNo_' + j + '=' + bundleNo + '&orderId_' + j + '=' + orderId + '&gmtsitemId_' + j + '=' + gmtsitemId + '&countryId_' + j + '=' + countryId + '&colorId_' + j + '=' + colorId + '&sizeId_' + j + '=' + sizeId + '&colorSizeId_' + j + '=' + colorSizeId + '&qty_' + j + '=' + qty + '&dtlsId_' + j + '=' + dtlsId + '&cutNo_' + j + '=' + cutNo;
				}
				catch(e) 
				{
		...
		http.open("POST","requires/wash_embro_delivery_entry_controller.php",true);
 	...
 	if(http.readyState == 4) 
	{
		...
		else if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
		{
			...
			else
			{
				...
				show_list_view(reponse[1],'show_dtls_listview','printing_production_list_view','requires/wash_embro_delivery_entry_controller','');
...
function openmypage_bundle(page_link,title)
{
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			var arr = hidden_job_no.split(',');
    
			var job_no = arr.filter(function(value, index, self) { 
	...
	{
		var challan_duplicate=return_ajax_request_value( bundle_no+"__"+$('#cbo_company_name').val(),"challan_duplicate_check", "requires/wash_embro_delivery_entry_controller");
...
function create_row(bundle_nos)
{
	...
	var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"****"+$('#cbo_company_name').val(), 'populate_bundle_data', '', 'requires/wash_embro_delivery_entry_controller');
	//$job_no = 'FAL-23-01220';
	var job_no = $("#job").val();
	// var embel_type=return_global_ajax_value(job_no+"**"+row_num, 'populate_embel_type', '', 'requires/wash_embro_delivery_entry_controller');

    // $('#embro_type_td').html(embel_type);
	load_drop_down( 'requires/wash_embro_delivery_entry_controller', job_no+'**', 'populate_embel_type', 'embro_type_td' );
            ...
            <fieldset>
                <table width="100%">
                    ...
                    <tr>
                        ...
                        <td>
                            <? 
                            	echo create_drop_down( "cbo_company_name", 180, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "get_php_form_data(this.value,'load_variable_settings','requires/wash_embro_delivery_entry_controller');get_php_form_data(this.value,'load_variable_settings_for_working_company','requires/wash_embro_delivery_entry_controller');working_com_fnc();load_html();" );
                            ?>
                         ...
                         <td id="embro_type_td" width="170">
                             <? 
								echo create_drop_down( "cbo_embel_type", 180, $emblishment_wash_type,"", 1, "--- Select wash ---", $selected, "" );  
                             ?>
                         </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                     ...
                     <td id="working_company">Working Company</td>
                        <td>
                        	<? 
								echo create_drop_down( "cbo_working_company_name", 180, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select --",$selected,"load_drop_down( 'requires/wash_embro_delivery_entry_controller', $('#cbo_working_company_name').val(), 'load_drop_down_working_location', 'working_location_td' );" );
                            ?>
                        </td>
               	...
               	<table cellpadding="0" cellspacing="1" width="100%">
               		<tr>
                        <td align="center" colspan="9" valign="middle" class="button_container">
                            ...
                            <input id="Print3" class="formbutton" type="button" style="width:80px;" onClick="fnc_issue_print_embroidery_entry(6)" name="print3" value="Print 3">
                             <input id="Print4"  class="formbutton" type="button" style="width:80px;" onClick="fnc_issue_print_embroidery_entry(7)" name="print4" value="Print 4" >
