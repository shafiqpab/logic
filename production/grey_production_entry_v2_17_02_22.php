...
//--------------------------------------------------------------------------------------------------------------------
	...
	function fnc_grey_production_entry_Reply_info()
	{
		if(http.readyState == 4) 
		{
			...
			if(reponse[0]==0)
			{
				...
				'<html><head><title></title><link rel="stylesheet" href="../css/style_common.css" type="text/css" media="print"/></head><body>'+api_data+'</body</html>');
				d.close(); 
	...
	function load_machine(machine_id=0)
	{
		...
		if(cbo_knitting_source==1)
		{
			load_drop_down( 'requires/grey_production_entry_controller_v2',cbo_knitting_company+'_'+cbo_floor_id+'_'+machine_id, 'load_drop_machine', 'machine_td' );
		}
		else
		{
			load_drop_down( 'requires/grey_production_entry_controller_v2',cbo_company_id+'_'+cbo_floor_id+'_'+machine_id, 'load_drop_machine', 'machine_td' );
		}
	...
	function out_bound_machine_load(source)
	{
		if(source ==3)
		{
			load_drop_down( 'requires/grey_production_entry_controller_v2', $('#cbo_company_id').val(), 'load_drop_machine_out_bound', 'machine_td');
	...
	$('#txt_booking_no').live('keydown', function (e) 
	{
    	if (e.keyCode === 13)
		{
    		...
    		if(cbo_receive_basis==2)
    		{
				...
				$.post( "requires/grey_production_entry_controller_v2.php", { action: "load_weight_machine_data"})
				.done(function( data ) {
					var obj = JSON.parse(data);
					var api_weight=10; 
					if(api_weight){
						//$( "#txtGreyQnty_" +i).val( obj.weight );
						var knit_operator=11;
						var knit_operator_name='Mohammad';
	...
	function fnc_reset()
	{
		reset_form('greyproductionentry_1','list_container_knitting*list_fabric_desc_container*roll_details_list_view*list_program_wise_fabric_desc_container','','cbo_receive_basis,2','disable_enable_fields(\'cbo_company_id\');set_receive_basis();');
