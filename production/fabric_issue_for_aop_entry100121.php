...
function fnc_trims_booking( operation ){
	...
	/*if (form_validation('cbo_company_name*txt_order_no*cbo_buyer_name','Company Name*Work Order*Buyer Name')==false)
	{
		release_freezing();
		return;
	}
	...
	{
		data_all=data_all+get_submitted_data_string('txt_system_id*cbo_company_name*txt_issue_date*cbo_issue_purpose*cbo_source*cbo_buyer_name*cbo_party_location*txt_order_no*hid_order_id*txt_attention*txt_delivery_date*txt_vehical_no*txt_driver_name*txt_dl_no*txt_transport*txt_mobile_no*txt_gate_pass_no*cbo_is_short*txt_remarks*update_id',"../");
	}*/
	var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_system_id*cbo_company_name*txt_issue_date*cbo_issue_purpose*cbo_source*cbo_buyer_name*cbo_party_location*txt_order_no*hid_order_id*txt_gate_pass_no*cbo_is_short*txt_remarks*update_id*txt_attention*txt_delivery_date*txt_vehical_no*txt_driver_name*txt_dl_no*txt_transport*txt_cell_no',"../");
...
function fnc_trims_booking_reponse(){
	if(http.readyState == 4)
	{
		 ...
		 /*if(reponse[0]==2)
		 {
			show_msg(trim(reponse[0]));
			set_button_status(0, permission, 'fnc_trims_booking',1);
			reset_form('','data_panel*booking_list_view','txt_booking_no*cbo_company_name*cbo_buyer_name*cbo_currency*txt_exchange_rate*txt_issue_date*txt_delivery_date*cbo_pay_mode*cbo_source*cbo_supplier_name*txt_attention','txt_issue_date,<? echo date("d-m-Y"); ?>');
		 }
	...
	else
	{
		...
		$('table#tbl_dtls_emb tbody tr').each(function()
		{
			...
			if(check_field==0)
			{
				//alert(check_field);
				j++;
				data_all += "&cboProcessType_" + j + "='" + cboProcessType + "'&txtWoqnty_" + j + "='" + txtWoqnty + "'&txtRate_" + j + "='" + txtRate + "'&txtAmount_" + j + "='" + txtAmount + "'&txtNumberRoll_" + j + "='" + txtNumberRoll  + "'&txtRemarks_" + j + "='" + txtRemarks + "'&soDtlsId_" + j + "='" + soDtlsId+ "'&batchDtlsId_" + j + "='" + batchDtlsId + "'&hiddenid_" + j + "='" + hiddenid + "'";
				i++;
				//alert(data_all);	
			}
		});
		var data="action=save_update_delete_dtls&operation="+operation+'&total_row='+i+'&txt_order_no='+txt_order_no+'&hid_order_id='+hid_order_id+'&update_id'+update_id+data_all; 
		//alert(data);
		http.open("POST","requires/fabric_issue_for_aop_entry_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
            ...
            <fieldset style="width:950px;">
                ...
                <table  width="900" cellspacing="2" cellpadding="0" border="0">
                    ...
                    <tr>
                        ...
                        <td><?  echo create_drop_down( "cbo_company_name", 172, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name", "id,company_name",1, "-- Select Company --", $selected, "load_drop_down( 'requires/fabric_issue_for_aop_entry_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );","","" ); ?></td>
                        ...
                        <td width="170" ><? echo create_drop_down( "cbo_issue_purpose", 172, $yarn_issue_purpose,"", 1, "-- Select Source --", "52", "","" ); ?></td>
                    </tr>
                    <tr>
                    	<td width="130" align="right"> Source </td>              <!-- 11-00030  -->
                        <td width="170" ><? echo create_drop_down( "cbo_source", 172, $source,"", 1, "-- Select Source --", "3", "","" ); ?></td>
                        <td align="right" class="must_entry_caption">Service Company</td>
                        <td id="buyer_td"><? echo create_drop_down( "cbo_buyer_name", 172, "select id,buyer_name from lib_buyer where status_active =1 and is_deleted=0 order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "",1,"" ); ?></td>
                        ...
                        <td id="td_party_location" align="right">Service Company Address</td>
                        <td id="party_location_td"><? echo create_drop_down( "cbo_party_location", 172, $blank_array,"", 1, "-- Select Location --", $selected, "",1 ); ?></td>
                    	...
                    	<td width="100%" colspan="6">
	                    	<table width="100%" border="0">
	                    		<tr>
			                        ...
			                        <?
			                        $endis = "disable_enable_fields( 'cbo_company_name*cbo_buyer_name', 0 )";
			                        echo load_submit_buttons( $permission, "fnc_trims_booking", 0,0 ,"reset_form('servicebooking_1','booking_list_view*data_panel*pdf_file_name','','txt_issue_date,".$date."',$endis,'cbo_source*txt_attention*txt_delivery_date*cbo_company_name*cbo_buyer_name')",1) ;
			                        ?>
			                        </td>
	                    		</tr>
	                    	</table>
            ...
            <fieldset style="width:950px;">
                ...
                <table cellpadding="0" cellspacing="2" border="0" class="rpt_table" rules="all" id="tbl_dtls_emb">
                    ...
                    <tfoot>
				    	...
				    	<th align="right"><? echo $blaamount; ?></td>
				    	<th align="right"><? echo $blaamount; ?></td>
				    	<th align="right">&nbsp;</td>
				    	<th align="right">&nbsp;</td>
				    	<th align="right">&nbsp;</td>
...
function fnc_process_data(){
	...
	if (form_validation('cbo_company_name','Company')==false){
		...
		//page_link=page_link+'&garments_nature='+garments_nature+'&cbo_booking_month='+cbo_booking_month+'&cbo_booking_year='+cbo_booking_year+'&cbo_company_name='+cbo_company_name+'&cbo_buyer_name='+cbo_buyer_name+'&cbo_currency='+cbo_currency+'&cbo_is_short='+cbo_is_short;
		page_link=page_link+'&txt_order_no='+txt_order_no+'&hid_order_id='+hid_order_id+'&cbo_is_short='+cbo_is_short+'&cbo_company_name='+cbo_company_name+'&cbo_buyer_name='+cbo_buyer_name;
		...
		emailwindow.onclose=function(){
			...
			if (theemail.value!=""){
				...
				var txtBatchDtlsId=theemail3.value;
				var data='1'+'**'+txtSoDtlsId+'**'+txtPreCostDtlsId+'**'+txtBatchDtlsId+'**'+txt_order_no;
				//alert(txt_order_no);
				show_list_view(data, 'generate_aop_booking','booking_list_view','requires/fabric_issue_for_aop_entry_controller','$(\'#hide_fabric_description\').val(\'\')');
			}
		}
