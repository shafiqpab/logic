		...
		if(delivery_basis==3)
		{
			...
			$("#tbl_details").find('tbody tr').each(function()
			{
				...
				try 
				{
					...
					dataString+='&bundleNo_' + j + '=' + bundleNo + '&orderId_' + j + '=' + orderId + '&gmtsitemId_' + j + '=' + gmtsitemId + '&countryId_' + j + '=' + countryId + '&colorId_' + j + '=' + colorId + '&sizeId_' + j + '=' + sizeId + '&colorSizeId_' + j + '=' + colorSizeId + '&qty_' + j + '=' + qty + '&dtlsId_' + j + '=' + dtlsId + '&rejectQty_' + j + '=' + rejectQty + '&cutNo_' + j + '=' + cutNo+ '&replaceQty_' + j + '=' + replacetQty;
				}
				catch(e) 
				{
...
function pageReset()
{
	...
	function openmypage_bundle(page_link,title)
	{
		...
		else
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link+'&bundleNo='+bundleNo+'&issue_challan_id='+issue_challan_id, title, 'width=890px,height=370px,center=1,resize=0,scrolling=0','')
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0];
...
{
	var challan_duplicate=return_ajax_request_value( bundle_no+"__"+$('#cbo_company_name').val(),"challan_duplicate_check", "requires/print_embro_bundle_receive_controller");
		...
		if(n>0)
		{
			...
			emailwindow.onclose=function()
			{
				...
				{ 
					...
					var tot_row=$('#tbl_details tbody tr').length; 
					if( (tot_row*1)<1)
						get_php_form_data(single_bundle_info[2]+"_"+single_bundle_info[0],'load_mst_data_bundle_scan','requires/print_embro_bundle_receive_controller');
					
					create_scan_row(bundle_no,single_bundle_info[0]);
				}
			}
...
function create_scan_row(bundle_nos,challan_id)
{
	freeze_window(5);
	var row_num=$('#txt_tot_row').val();
	var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"**"+challan_id+"**"+$('#cbo_company_name').val(), 'populate_bundle_data', '', 'requires/print_embro_bundle_receive_controller');
	$('#tbl_details tbody').prepend(response_data);
	var tot_row=$('#tbl_details tbody tr').length; 
	$('#txt_tot_row').val(tot_row);
	release_freezing();
...
function create_row(bundle_nos)
{
	...
	var row_num=$('#txt_tot_row').val();
	var issue_challan_id=$("#txt_issue_challan_id").val();
	var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"**"+issue_challan_id+"**"+$('#cbo_company_name').val(), 'populate_bundle_data', '', 'requires/print_embro_bundle_receive_controller');
	$('#tbl_details tbody').prepend(response_data);
	var tot_row=$('#tbl_details tbody tr').length; 
	$('#txt_tot_row').val(tot_row);
	release_freezing();
...
function openmypage_issue_challan()
{
 ...
 function scan_challan_no(chanal_no)
 {
	...
	function calculate_qcpasss_rescan(id)
	{
		...
		{
			$("#qty_"+id).val(replace_qty);
			$("#QcQty_"+id).text(replace_qty);
		}
	}
	...
	if (e.keyCode === 13) 
	{
		...
		if(flag==1)
		{
			fnc_duplicate_scan_bundle(txt_bundle_no);
		}
	}
});

function fnc_duplicate_scan_bundle(bundle_no)
{
	...
	if(n>0)
	{
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				if( (tot_row*1)<1)
					get_php_form_data(single_bundle_info[2]+"_"+single_bundle_info[0],'load_mst_data_bundle_scan','requires/print_embro_bundle_receive_controller');
				create_rescan_row(single_bundle_info[2],single_bundle_info[0]);
			}
		}
                              ...
                              <legend>New Entry</legend>
                               <table  cellpadding="0" cellspacing="2" width="100%">
                                  <tr>
                                       ...
                                       <td width="80" class="must_entry_caption" id="td_caption">Re-Scan Bundle</td>
                                       <td colspan="2" width="110"> 
               		...
               		<tr>
                        <td align="center" colspan="9" valign="middle" class="button_container">
                            ...
                            ?>
                               ...
                               <input type="button" id="btn_print" name="btn_print"   style="width:120px;"  class="formbutton" value="Size Wise Print2"  onClick="generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val(),'embrodary_color_wise_print2','requires/print_embro_bundle_receive_controller');"/>
