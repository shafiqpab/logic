...
<script>
	...
	function openmypage_sysNo()
	{
		...
		var page_link = 'requires/bundle_wise_sewing_output_controller.php?cbo_company_name='+cbo_company_name+'&action=challan_no_popup';
	...
	function fnc_issue_print_embroidery_Reply_info()
	{
		if(http.readyState == 4) 
		{
			...
			if(reponse[0]==15) 
			{ 
				 setTimeout('fnc_sewing_bundle_output_entry('+ reponse[1]+')',8000); 
			}
	...
	function openmypage_bundle(page_link,title)
	{
		...
		else
		{
			var tot_row=$('#tbl_details tbody tr').length; 
			var line_id=0;	
			if( (tot_row*1) <1) line_id=0; else line_id=$('#cbo_line_no').val();
	...
	{
		var challan_duplicate=return_ajax_request_value( bundle_no+"__"+$('#cbo_company_name').val(),"challan_duplicate_check", "requires/bundle_wise_sewing_output_controller");
		...
		else
		{
			...
			if( (tot_row*1) <1)
				get_php_form_data(bundle_no,'load_mst_data','requires/bundle_wise_sewing_output_controller');	
			else
				get_php_form_data(bundle_no+"__"+$('#cbo_line_no').val(),'load_mst_data','requires/bundle_wise_sewing_output_controller');
		}
		$('#txt_bundle_no').val('');
	}
	...
	function create_row(bundle_nos)
	{
		...
		var response_data_msg=return_global_ajax_value(bundle_nos+"**"+row_num+"****"+$('#cbo_company_name').val()+"**"+$('#cbo_line_no').val(), 'populate_bundle_data_check', '', 'requires/bundle_wise_sewing_output_controller');
	...
	function change_mode(source_id)
	{
		...
		{
			get_php_form_data($('#cbo_emb_company').val(),'load_variable_settings','requires/bundle_wise_sewing_output_controller'); get_php_form_data($('#cbo_emb_company').val(),'load_variable_settings_for_working_company','requires/bundle_wise_sewing_output_controller'); load_html();
