<?
/*-------------------------------------------- Comments
Purpose			: 	This form will represent finish qc result
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("finish Production QC Result","../", 1, 1, $unicode,1,1); 
?>	
<script>
...
function grey_receive_popup()
{
	...
	{ 	
		var page_link='requires/finish_production_qc_result_controller.php?cbo_company_id='+cbo_company_id+'&action=grey_receive_popup_search';
		var title='Finish Production Form';
		...
		emailwindow.onclose=function()
		{
			...
			{
				freeze_window(5);
				get_php_form_data(grey_recv_id, "populate_data_from_grey_recv", "requires/finish_production_qc_result_controller" );
...
function  generate_report(rptType)
{
	...
	var dataString = "&cbo_company_id="+cbo_company_id+"&cbo_search_category="+cbo_search_category+"&txt_booking_no="+txt_booking_no+"&txt_batch_no="+txt_batch_no+"&cbo_year="+cbo_year+"&txt_date_from="+txt_date_from+"&txt_date_to="+txt_date_to+"&txt_barcode="+txt_barcode+"&txt_recieved_number="+txt_recieved_number;


	var data="action=generate_report"+dataString;
	freeze_window(5);
	http.open("POST","requires/finish_production_qc_result_controller.php",true);
...
function fn_knit_defect(prod_dtls_id,barcode_no)
{
	...
	else
	{
		...
		var page_link='requires/finish_production_qc_result_controller.php?update_dtls_id='+prod_dtls_id+'&roll_maintained='+roll_maintained+'&company_id='+company_id+'&barcode_no='+barcode_no+'&action=finish_defect_popup';
...
<div style="width:100%;" align="center">
   ...
   <h3 align="left" id="accordion_h1" style="width:930px" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3>
    ...
    <div style="width:930px;" align="center" id="content_search_panel">
        <fieldset style="width:100%;">
                ...
                <tr class="general">
                    <td>
                        <?
                        	echo create_drop_down( "cbo_company_id", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 order by comp.company_name","id,company_name", 1, "--Select Company--", $selected, "get_php_form_data( this.value,'roll_maintained' ,'requires/finish_production_qc_result_controller');" );
                        ?>  
					...
					<td>
						<input type="text" name="txt_batch_no" id="txt_batch_no" class="text_boxes" style="width:100px">
					</td>
...
function openmypage_barcode()
	{ 
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/finish_production_qc_result_controller.php?company_id='+company_id+'&action=barcode_popup','Barcode Popup', 'width=1240px,height=380px,center=1,resize=1,scrolling=0','')
		emailwindow.onclose=function()
		{
