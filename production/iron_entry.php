<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create print Iron Entry
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Iron Entry","../", 1, 1, $unicode,'','');
...
<script>
	...
	function openmypage_sysNo()
	{
		...
		var page_link = 'requires/iron_entry_controller.php?action=system_number_popup&cbo_company_name='+cbo_company_name;
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				load_drop_down( 'requires/iron_entry_controller', ex_data[2]+'_'+1, 'load_drop_down_location', 'location_td' );
				...
				load_drop_down('requires/iron_entry_controller', location, 'load_drop_down_floor', 'floor_td' );
				...
				$('#txt_table_no').val(ex_data[18]);
	
				fnc_dtls_data_load(ex_data[9],ex_data[0]);
				
				get_php_form_data(ex_data[2], "production_process_control", "requires/iron_entry_controller" );
	...
	function fnc_iron_entry(operation)
	{
		...
		if(operation==4)
		{
			// var report_title=$("div.form_caption").html();
			 //print_report( $('#cbo_company_name').val()+'*'+master_ids+'*'+report_title, "emblishment_issue_print", "requires/iron_entry_controller" )
			// return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			else
			{
				...
				var data="action=save_update_delete&operation="+operation+'&tot_row='+k+get_submitted_data_string('garments_nature*txt_system_no*cbo_company_name*cbo_location*cbo_source*cbo_iron_company*cbo_iron_location*cbo_floor*txt_job_no*txt_style_ref*cbo_buyer_name*txt_iron_date*txt_reporting_hour*cbo_sending_location*txt_challan*txt_remark*txt_job_id*txt_update_id*txt_wo_id*txt_table_no',"../")+data_str;
				//alert (data); return;
				freeze_window(operation);
				http.open("POST","requires/iron_entry_controller.php",true);
	...
	function fnc_total_calculate(value,i)
	{
		...
		var rejtQty = $("#txtRjtQty_"+i).val()*1;
		var tot_row=$('#tbl_details tr').length-1;
		//alert(placeholder_value);
		if(((value*1)+pre_iss_qty+rejtQty)>placeholder_value)
		{
			...
			{
				var confirm_value=confirm("QC qty Excceded by Prod Qty. (PCS). Press cancel to proceed otherwise press ok.");
				if(confirm_value!=0)
				{
					$("#txtQty_"+i).val('');
				}	
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				if (theemail!="")
				{
					...
					get_php_form_data(cbo_company_name, "production_process_control", "requires/iron_entry_controller" );
					release_freezing();
				}
			}
		}
	...
	function fnc_dtls_data_load(job_no,uid)
	{
		...
		var list_view_orders = return_global_ajax_value( cbo_company_id+'***'+job_no+'***'+uid, 'order_details', '', 'requires/iron_entry_controller');
		if(list_view_orders!='')
		{
	...
	function fnc_load_party(type)
	{
		...
		if(source==1 && type==1)
		{
			load_drop_down( 'requires/iron_entry_controller', company+'_'+1, 'load_drop_down_working_com', 'iron_company_td' );
		}
		else if(source==3 && type==1)
		{
			load_drop_down( 'requires/iron_entry_controller', company+'_'+3, 'load_drop_down_working_com', 'iron_company_td' );
		}
		else if(source==1 && type==2)
		{
			load_drop_down( 'requires/iron_entry_controller', working_company+'_'+2, 'load_drop_down_location', 'working_location_td' ); 
		} 
	}
	...
	function calculate_qcpasss(qty, i)
	{ 
		//alert(23);
		var placeholder_value = $("#txtQty_"+i).attr('placeholder')*1;
		var qcQty=$("#txtQty_"+i).val()*1;
		var tot_row=$('#tbl_details tr').length-1;
		if(qcQty>0)
		{		
			if(placeholder_value < qty)
			{
				alert(`Total reject qty can not over production qnty prod qty=(${placeholder_value}). `);
				$("#txtRjtQty_"+i).val(0);
				// return;
			}
			else
			{
				$("#txtQty_"+i).val(qcQty-qty);
				// $("#txtRjtQty_"+i).val(0);
				// return;
			}
		}
		...
		{
			// $("#txtQty_"+i).attr('placeholder',placeholder_value-qty);
		}
		math_operation( "txtTotQty", "txtQty_", "+", tot_row );
		math_operation( "txtTotRjtQty", "txtRjtQty_", "+", tot_row );
	}
	...
	function fnc_wo_no()
	{
		...
		else
		{
			...
			var page_link="requires/iron_entry_controller.php?action=wo_no_popup&company_id="+company_id+'&service_company_id='+service_company_id+'&txt_job_no='+txt_job_no;
	...
	{
		var system_id=$("#txt_update_id").val();
		if(system_id=="")
		{
			alert('System No Required!');
			return;
		}

		var page_link='requires/iron_entry_controller.php?action=show_cost_details&sys_id='+system_id;
 		...
 		<fieldset style="width:1050px;">
            ...
            <form name="ironentry_1" id="ironentry_1" method="" autocomplete="off" >
                <fieldset>
                    <table width="100%">
                        <tr>
                            <td colspan="4" align="right"><b>Sys NO : </b></td>
                            <td colspan="4">
                        ...
                        <tr>
                            ...
                            <td width="140"><? echo create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "--Select--", $selected, "load_drop_down( 'requires/iron_entry_controller', this.value+'_'+1, 'load_drop_down_location', 'location_td'); fnc_load_party(1,document.getElementById('cbo_source').value);get_php_form_data(this.value,'load_variable_settings','requires/iron_entry_controller');","" ); ?></td>
                            ...
                            <td id="iron_company_td"><? echo create_drop_down( "cbo_iron_company", 130, $blank_array,"", 1, "-Embel. Company-", $selected, "" ); ?></td>
                        </tr>
                        <tr>
                            ...
                            <td><input type="text" name="txt_job_no" id="txt_job_no" class="text_boxes" style="width:120px" onDblClick="openmypage_job('requires/iron_entry_controller.php?action=job_popup', 'Job/Order Selection Form');" placeholder="Browse" readonly /></td>
                            <td>Style Ref.</td>
                        ...
                        <tr>
                            ...
                            <td class="must_entry_caption">Date</td>
                        ...
                        <tr>
                            ...
                            <td id="servicewo_td">Table /Line No</td>
            				<td id="table_td">
							<? echo create_drop_down( "txt_table_no", 130, "select id,TABLE_NAME from lib_table_entry where status_active =1 and is_deleted=0 and TABLE_TYPE=2 order by TABLE_SEQUENCE","id,TABLE_NAME", 1, "-Select Table/Line No-", $selected, "" ); ?>
                            <!-- <input type="text" name="txt_table_no" id="txt_table_no" class="text_boxes_numeric" style="width:120px" /> -->
                            </td>
                            <td>Remarks</td>
                            <td colspan="3"><input type="text" name="txt_remark" id="txt_remark" class="text_boxes" style="width:362px" title="450 Characters Only." /> <input type="hidden" name="txt_wo_id" id="txt_wo_id" class="text_boxes" style="width:50px" /></td>
                        </tr>
                        <tr>
