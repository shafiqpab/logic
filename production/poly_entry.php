<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Poly Entry
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			if (po_id!="")
			{
				...
				var poly_line=$('#cbo_poly_line').val();
				
				childFormReset();//child from reset
				get_php_form_data(po_id+'**'+item_id+'**'+country_id+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'**'+poly_line+'**'+country_ship_date, "populate_data_from_search_popup", "requires/poly_entry_controller" );
				...
				var poly_company=$('#cbo_poly_company').val();
				if(variableSettings!=1){ 
					get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'****'+country_ship_date, "color_and_size_level", "requires/poly_entry_controller"); 
				...
				{
					$('#prod_qty').text('Sewing Input');
				}
				else if(preceding_process==29)
				{
					$('#prod_qty').text('Sewing Output');
				}
				else if(preceding_process==30)
				{
					$('#prod_qty').text('Iron Output');
				}
				else if(preceding_process==31)
				{
					$('#prod_qty').text('Packing And Finishing');
				}
				else if(preceding_process==103)
				{
					$('#prod_qty').text('Poly Entry');
				}
				else if(preceding_process==256)
				{
					$('#prod_qty').text('Hang Tag Qty');
				}
				
				
				load_drop_down( 'requires/poly_entry_controller', po_id, 'load_drop_down_color_type', 'color_type_td' );
				var prod_reso_allo=$('#prod_reso_allo').val();
				show_list_view(po_id+'**'+item_id+'**'+country_id+'**'+prod_reso_allo+'**'+pack_type+'**'+variableSettings+'**'+country_ship_date,'show_dtls_listview','list_view_container','requires/poly_entry_controller','setFilterGrid(\'tbl_list_search\',-1)');
				show_list_view(po_id+'**'+poly_company,'show_country_listview','list_view_country','requires/poly_entry_controller','');		
...
function fnc_poly_output_entry(operation)
{
	...
	if(operation==4)
	{
		 ...
		 print_report( $('#cbo_company_name').val()+'*'+$('#txt_mst_id').val()+'*'+report_title+'*'+$("#poly_production_variable").val(), "poly_output_print", "requires/poly_entry_controller" ) 
		 return;
	}
	else if(operation==0 || operation==1 || operation==2)
	{
		...
		if ( form_validation('cbo_company_name*txt_order_no*cbo_source*cbo_poly_company*txt_poly_date*txt_reporting_hour*txt_challan','Company Name*Order No*Source*Poly Company*Poly Date*Reporting Hour*txt_challan')==false )
		{
			return;
		}		
		else
		{
			...
			var i=0;  var k=0; l=0;var colorIDvalue=''; var colorIDvalueRej='';var colorIDvalueRejWithDefect='';
			...
			else if(variableSettingsReject==3)//color and size level
			{
				...
				// ========================================
				$("input[name=actual_reject").each(function(index, element) 
				{
					if( $(this).val()!='' )
					{
						if(l==0)
						{
							colorIDvalueRejWithDefect = colorList[l]+"**"+$(this).val();
						}
						else
						{
							colorIDvalueRejWithDefect += "***"+colorList[l]+"**"+$(this).val();
			...
			// alert(colorIDvalueRejWithDefect);
			...
			var data="action=save_update_delete&operation="+operation+"&colorIDvalue="+colorIDvalue+"&colorIDvalueRej="+colorIDvalueRej+"&colorIDvalueRejWithDefect="+colorIDvalueRejWithDefect+get_submitted_data_string('garments_nature*cbo_company_name*cbo_country_name*poly_production_variable*poly_production_variable_rej*hidden_po_break_down_id*hidden_colorSizeID*cbo_buyer_name*txt_style_no*cbo_item_name*txt_order_qty*cbo_source*cbo_poly_company*cbo_location*cbo_floor*txt_poly_date*cbo_produced_by*cbo_poly_line*txt_reporting_hour*cbo_color_type*txt_super_visor*txt_poly_qty*txt_reject_qnty*txt_alter_qnty*txt_challan*txt_remark*txt_input_quantity*txt_cumul_poly_qty*txt_yet_to_poly*hidden_break_down_html*txt_mst_id*prod_reso_allo*txt_spot_qnty*save_data*defect_type_id*save_dataSpot*defectSpot_type_id*save_dataReject*defectReject_type_id*hidden_currency_id*hidden_exchange_rate*hidden_piece_rate*cbo_work_order*txt_pack_type*country_ship_date*txt_internal_ref',"../");
 			// alert (data); return;
...
function fnc_poly_output_entry_Reply_info()
{
 	if(http.readyState == 4) 
	{
		...
		else
		{
			...
			show_list_view(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id+'**'+prod_reso_allo+'**'+pack_type+'**'+variableSettings+'**'+country_ship_date,'show_dtls_listview','list_view_container','requires/poly_entry_controller','setFilterGrid(\'tbl_list_search\',-1)');
			...
			get_php_form_data(po_id+'**'+$('#cbo_item_name').val()+'**'+country_id+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'**'+poly_line+'**'+country_ship_date, "populate_data_from_search_popup", "requires/poly_entry_controller" );
			if(variableSettings!=1) { 
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'**'+poly_line+'**'+country_ship_date, "color_and_size_level", "requires/poly_entry_controller" ); 
			}
			else
			{
				$("#txt_poly_qty").removeAttr("readonly");
...
function fn_total_rej(tableName,index) // for color and size level
{
    ...
    var qc_value = $("#colSize_"+tableName+index).val();
    if(qc_value=="")
    {
    	// alert('QC qty is required. Please enter minimum qc qty like 0(zero) ');
...
function put_country_data(po_id, item_id, country_id, po_qnty, plan_qnty, pack_type, ship_date)
{
	...
	$("#cbo_country_name").val(country_id);
	$("#txt_pack_type").val(pack_type);
	$("#country_ship_date").val(ship_date);
	$("#selected_proudction_company").val($('#cbo_poly_company').val());
	var poly_line=$('#cbo_poly_line').val();
	
	childFormReset();//child from reset
	get_php_form_data(po_id+'**'+item_id+'**'+country_id+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'**'+poly_line+'**'+ship_date, "populate_data_from_search_popup", "requires/poly_entry_controller" );
	...
	if(variableSettings!=1)
	{ 
		get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'**'+poly_line+'**'+ship_date, "color_and_size_level", "requires/poly_entry_controller" ); 
		$("#txt_poly_qty").attr("readonly","readonly");
	}
	else
	{
	...
	show_list_view(po_id+'**'+item_id+'**'+country_id+'**'+prod_reso_allo+'**'+'**'+variableSettings,'show_dtls_listview','list_view_container','requires/poly_entry_controller','setFilterGrid(\'tbl_list_search\',-1)');
	set_button_status(0, permission, 'fnc_poly_output_entry',1,0);
	release_freezing();
}
...
function check_produced_by(val)
{
	...
	{
		var response=return_global_ajax_value( po_id+'**'+item_id, 'piece_rate_order_cheack', '', 'requires/poly_entry_controller');
...
function openmypage_defectQty(type)
{
	...
	else
	{
		...
		else if (type==3)
		{
			...
			title = 'Reject Qty Info';
		}
		
		var page_link = 'requires/poly_entry_controller.php?hidden_po_break_down_id='+hidden_po_break_down_id+'&txt_mst_id='+txt_mst_id+'&save_data='+save_data+'&defect_qty='+defect_qty+'&all_defect_id='+all_defect_id+'&type='+type+'&action=defect_data';
...
function fnc_workorder_search(supplier_id)
{
	...
	load_drop_down( 'requires/poly_entry_controller', company+"_"+supplier_id+"_"+po_break_down_id, 'load_drop_down_workorder', 'workorder_td'); 
	//alert($('#cbo_cutting_company option').size())
}

function fnc_workorder_rate(data,id)
{
	get_php_form_data(data+"_"+id, "populate_workorder_rate", "requires/poly_entry_controller" );
}
...
function fnc_company_check(val)
{
	if(val==1)
	{
		...
		{
			get_php_form_data(document.getElementById('cbo_poly_company').value,'production_process_control','requires/poly_entry_controller' );
		}
	}
	else
	{
		get_php_form_data(document.getElementById('cbo_company_name').value,'production_process_control','requires/poly_entry_controller' );
	}
 }
...
function fnc_loadvariable(val)
{
	...
	$("#txt_qty_source").val('');
	get_php_form_data( $("#cbo_company_name").val()+'**'+val,'load_variable_settings','requires/poly_entry_controller');
}
...
function pop_entry_reject(row_id)
{
	...
	var page_link='requires/poly_entry_controller.php?action=reject_qty_popup&actual_infos='+actual_infos;
	...
	emailwindow.onclose=function()
	{
		...
		$("#colSizeRej_"+row_id).val(actual_qty);

		var reject_qnty = ($("#txt_reject_qnty").val()*1)+(actual_qty*1);

		$("#txt_reject_qnty").val(reject_qnty);
		// calculate_qcpasss(row_id);

		//alert(actual_infos+'=='+actual_qty) 
	}
}
	function load_poly_company(source_id)
	{
		if($('#cbo_poly_company option').length==2)
		{
			if($('#cbo_poly_company option:first').val()==0)
			{
				var cbo_poly_company_id=$('#cbo_poly_company option:last').val();
				$('#cbo_poly_company').val(cbo_poly_company_id);
				if(source_id==1)
				{
					fnc_loadvariable(cbo_poly_company_id); 
					load_drop_down( 'requires/poly_entry_controller', cbo_poly_company_id, 'load_drop_down_location', 'location_td' ); 
					fnc_company_check(source_id);
				}
				if(source_id==3)
				{
					fnc_loadvariable($('#cbo_company_name').val()); 
					fnc_workorder_search(cbo_poly_company_id);
				}
			}
		}
	}
	...
	{
		let variableSettings=$('#poly_production_variable').val();
		let variableSettingsReject=$('#poly_production_variable_rej').val();
		...
		let pack_type=$("#txt_pack_type").val();
		let country_ship_date=$("#country_ship_date").val();

		if(variableSettings!=1)
		{
			get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject+'**'+$('#txt_qty_source').val()+'**'+pack_type+'**'+sewing_line+'**'+country_ship_date, "color_and_size_level", "requires/poly_entry_controller" ); 
                ...
                <fieldset>
                    <table width="100%" border="0">
                        <tr>
                            ...
                            <td width="150">                                
								<? 
                                 echo create_drop_down( "cbo_company_name", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "fnc_loadvariable(this.value);" );
                                ?>
                                ...
                                <input type="hidden" id="country_ship_date" value="0" />
                        	...
                        	<td class="must_entry_caption">Source</td>
                             <td>
								 <?
                                 echo create_drop_down( "cbo_source", 150, $knitting_source,"", 1, "-- Select Source --", $selected, "fnc_company_check(this.value); load_drop_down( 'requires/poly_entry_controller', this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_poly_output', 'sew_company_td' ); get_php_form_data(this.value,'line_disable_enable','requires/poly_entry_controller');dynamic_must_entry_caption(this.value);load_poly_company(this.value);", 0, '1,3' );
								 //get_php_form_data($('#cbo_company_name').val()+'**'+this.value+'**'+$('#hidden_po_break_down_id').val()+'**'+$('#cbo_item_name').val()+'**'+$('#cbo_country_name').val()+'**'+$('#txt_pack_type').val(), 'display_bl_qnty', 'requires/poly_entry_controller');
                                 ?>
                             </td>
                           ...
                           <input type="hidden" name="hidden_preceding_process" id="hidden_preceding_process">
                         	 <td class="must_entry_caption">WO. Company</td>
                         ...
                         <tr>
                            ...
                            <td width="150">
								<input name="txt_order_no" placeholder="Double Click to Search" id="txt_order_no" onDblClick="openmypage('requires/poly_entry_controller.php?action=order_popup&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value+'&production_company='+document.getElementById('cbo_poly_company').value+'&hidden_variable_cntl='+document.getElementById('hidden_variable_cntl').value+'&hidden_preceding_process='+document.getElementById('hidden_preceding_process').value,'Order Search')"  class="text_boxes" style="width:140px " readonly>
                        ...
                        <tr>
                        	<td id="floors">Floor</td>
							 ...
							 <td>IR/IB</td>
                            <td>
                                <input name="txt_internal_ref" id="txt_internal_ref" class="text_boxes" style="width:140px " disabled readonly>	
							</td>
                    ...
                    <tr>
                            ...
                            <legend>New Entry</legend>
                                <table  cellpadding="0" cellspacing="1" width="100%">
                                   ...
                                   <tr>
                                     ...
                                     <td>
                                     	<!-- <input type="button" name="btn" id="btn" class="formbuttonplasminus" value="Rej Defect" style="width:60px" onClick="openmypage_defectQty(3);"/> -->
                                     </td>
                                   </tr>
                                   <tr>
                         ...
                         <td width="22%" valign="top">
                            ...
                            <legend>Display</legend>
                                <table  cellpadding="0" cellspacing="2" width="100%" >
                                    <tr>
                                        <td width="100" id="prod_qty">Input Quantity</td>
                                        <td>
                     ...
                     <tr>
		   				<td align="center" colspan="9" valign="middle" class="button_container">
							<?
								$date=date('d-m-Y');
								echo load_submit_buttons( $permission, "fnc_poly_output_entry", 0,1,"reset_form('polyoutput_1','list_view_country','','txt_poly_date,".$date."*txt_challan,0','childFormReset()')",1); 
                            ?>
