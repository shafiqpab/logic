...
<script>
	...
	<? 
	if($_SESSION['logic_erp']['mandatory_field'][537]!="")
	{
		$mandatory_field_arr= json_encode( $_SESSION['logic_erp']['mandatory_field'][537] );
	...
	{
		var report_ids = return_global_ajax_value( data, 'load_report_format', '', 'requires/embrodary_challan_receive_controller');
	...
	{   
		...
		else
		{
			...
			for (var k=0; k<report_id.length; k++)
			{
				...
				else if(report_id[k]==165)
				{
					$("#btn_print").show();
				}
				else if(report_id[k]==166)
				{
					$("#btn_print2").show();
				}
	...
	function openmypage_sysNo()
	{
		...
		var page_link = 'requires/embrodary_challan_receive_controller.php?cbo_company_name='+cbo_company_name+'&action=challan_no_popup';
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				{ 
					...
					get_php_form_data(all_data[0], "populate_data_from_challan_popup", "requires/embrodary_challan_receive_controller" );
					
					var delivery_basis=$('#delivery_basis').val();
					if(delivery_basis==3)
					{
						var bundle_nos=return_global_ajax_value(all_data[0], 'bundle_nos_update', '', 'requires/embrodary_challan_receive_controller');
						
						var response_data=return_global_ajax_value(trim(bundle_nos)+"**"+$("#txt_issue_challan_id").val()+"**"+all_data[0]+"**"+cbo_company_name, 'populate_bundle_data_update', '', 'requires/embrodary_challan_receive_controller');
					...
					{
						show_list_view(all_data[1],'show_country_listview','list_view_country','requires/embrodary_challan_receive_controller','');
						show_list_view(all_data[0],'show_dtls_listview','printing_production_list_view','requires/embrodary_challan_receive_controller','');
					}
					$("#btn_print").removeClass("formbutton_disabled");
					$("#btn_print").addClass("formbutton");
					
					$("#btn_print2").removeClass("formbutton_disabled");
					$("#btn_print2").addClass("formbutton");
					$("#btn_print5").removeClass("formbutton_disabled");
					$("#btn_print5").addClass("formbutton");
					$("#btn_print6").removeClass("formbutton_disabled");
					$("#btn_print6").addClass("formbutton");
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				{ 
					...
					get_php_form_data(po_id+'**'+item_id+'**'+$('#cbo_embel_name').val()+'**'+country_id, "populate_data_from_search_popup", "requires/embrodary_challan_receive_controller" );
					...
					else
					{
						...
						get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+$("#cbo_embel_name").val()+'**'+country_id, "color_and_size_level", "requires/embrodary_challan_receive_controller" ); 
					}
					
					show_list_view(po_id,'show_country_listview','list_view_country','requires/embrodary_challan_receive_controller','');	
	...
	*/
	function generate_report_file(data,action,page)
		{
			window.open("requires/embrodary_challan_receive_controller.php?data=" + data+'&action='+action, true );
		}
	...
	function fnc_issue_print_embroidery_entry(operation)
	{
		...
		var working_company_mandatory=return_global_ajax_value(company_id, 'load_variable_settings_for_working_company', '', 'requires/embrodary_challan_receive_controller');
		...
		if(operation==4)
		{
			...
			generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val()+'*'+report_title, 'emblishment_issue_print', 'requires/embrodary_challan_receive_controller');
			return;
		}
		...
		{
			var delivery_basis=$('#delivery_basis').val();
			var cbo_source=parseInt($('#cbo_source').val());
			if(cbo_source==3)
			{
				if('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][537]); ?>') 
				{
					if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][537]); ?>','<? echo implode('*',$_SESSION['logic_erp']['mandatory_message'][537]); ?>')==false) {return;}
				}
			}
			...
			if(delivery_basis==3)
			{
				...
				$("#tbl_details").find('tbody tr').each(function()
				{
					...
					var replacetQty=$(this).find('input[name="replaceQty[]"]').val();
					var actualReject=$(this).find('input[name="actual_reject[]"]').val();
					...
					try 
					{
						...
						dataString+='&bundleNo_' + j + '=' + bundleNo + '&orderId_' + j + '=' + orderId + '&gmtsitemId_' + j + '=' + gmtsitemId + '&countryId_' + j + '=' + countryId + '&colorId_' + j + '=' + colorId + '&sizeId_' + j + '=' + sizeId + '&colorSizeId_' + j + '=' + colorSizeId + '&qty_' + j + '=' + qty + '&dtlsId_' + j + '=' + dtlsId + '&rejectQty_' + j + '=' + rejectQty +'&replaceQty_' + j + '=' + replacetQty+'&actualReject_' + j + '=' + actualReject+ '&cutNo_' + j + '=' + cutNo+ '&barcodeNo_' + j + '=' + barcodeNo+ '&isRescan_' + j + '=' + isRescan+ '&dtlsId_' + j + '=' + dtlsId; 
					}
					catch(e) 
					{
				...
				var data="action=save_update_delete&operation="+operation+'&tot_row='+j+get_submitted_data_string('garments_nature*cbo_company_name*sewing_production_variable*cbo_embel_name*cbo_embel_type*cbo_source*txt_embl_company_id*txt_location_id*txt_floor_id*txt_issue_date*txt_boe_mushak_challan_no*txt_boe_mushak_challan_date*txt_organic*txt_system_id*delivery_basis*txt_challan_no*txt_issue_challan_id*cbo_body_part*cbo_working_company_name*cbo_working_location*txt_remark_bundle*get_entry_no*get_entry_date',"../")+dataString;
			}
			else
			{
	...
	function fnc_issue_print_embroidery_Reply_info()
	{
		if(http.readyState == 4) 
		{
			...
			else if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
			{
				...
				$('#txt_location_name').attr('disabled','true');
				$('#txt_floor_name').attr('disabled','true');
				var delivery_basis=$('#delivery_basis').val();
				if(delivery_basis==3)
				{
					// show_list_view(reponse[1],'show_cost_listview','printing_cost_list_view','requires/print_embro_bundle_receive_controller','');
					set_button_status(1, permission, 'fnc_issue_print_embroidery_entry',1,1);
				}
				else
				{
					reset_form('printembro_1','breakdown_td_id','','','txt_issue_date,<? echo date("d-m-Y"); ?>','cbo_company_name*sewing_production_variable*styleOrOrderWisw*cbo_embel_name*cbo_embel_type*cbo_source*txt_embl_company*txt_embl_company_id**cbo_knitting_source*cbo_location*cbo_floor*txt_boe_mushak_challan_no*txt_boe_mushak_challan_date*txt_organic*txt_issue_date*txt_challan_no*txt_system_id*delivery_basis*sewing_production_variable*embro_production_variable');
					show_list_view(reponse[1],'show_dtls_listview','printing_production_list_view','requires/embrodary_challan_receive_controller','');
	...
	function put_country_data(delivery_id, mst_id, country_id, item_id,order_id)
	{
		...
		get_php_form_data(delivery_id+'**'+item_id+'**'+$('#cbo_embel_name').val()+'**'+country_id+'**'+mst_id, "populate_data_from_search_popup", "requires/embrodary_challan_receive_controller" );
	...
	function pageReset()
	{
		...
		$('#cbo_embel_type').removeAttr('disabled');
		$('#cbo_source').removeAttr('disabled');
		$('#cbo_emb_company').removeAttr('disabled');
		$('#cbo_embel_name').removeAttr('disabled');
		$('#txt_location_name').removeAttr('disabled');
		$('#txt_floor_name').removeAttr('disabled');
	...
	function openmypage_bundle(page_link,title)
	{
		...
		else
		{
			...
			var issue_challan_id=$("#txt_issue_challan_id").val();
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link+'&bundleNo='+bundleNo+'&issue_challan_id='+issue_challan_id, title, 'width=990px,height=370px,center=1,resize=0,scrolling=0','')
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0];
				var hidden_bundle_nos=this.contentDoc.getElementById("hidden_bundle_nos").value;//po id
				var hidden_challan_no=this.contentDoc.getElementById("hidden_issue_challan_id").value;//bundle No
				...
				{ 
					...
					if( (tot_row*1) <1)
						get_php_form_data(hidden_bundle_nos+"_"+hidden_challan_no,'load_mst_data_bundle_scan','requires/embrodary_challan_receive_controller');
					create_scan_row(hidden_bundle_nos,hidden_challan_no);
				}
			}
		}//end else
	}//end function
	...
	{
		var challan_duplicate=return_ajax_request_value( bundle_no+"__"+$('#cbo_company_name').val(),"challan_duplicate_check", "requires/embrodary_challan_receive_controller");
	...
	{
		var challan_duplicate=return_ajax_request_value( barcode_no+"__"+$('#cbo_company_name').val(),"challan_duplicate_check", "requires/embrodary_challan_receive_controller");
		var ex_challan_duplicate=challan_duplicate.split("_");
		if(ex_challan_duplicate[0]==2) 
		{
			var alt_str=ex_challan_duplicate[1].split("*");
		...
		{
			var n = ex_challan_duplicate[1].search(",");
			if(n>0)
			{
	
				var page_link='requires/embrodary_challan_receive_controller.php?action=bundle_popup_single&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value;
				var title="Search Bundle";
				emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link+'&bundle_info='+ex_challan_duplicate[1], title, 'width=400px,height=180px,center=1,resize=0,scrolling=0','')
				emailwindow.onclose=function()
				{
					var theform=this.contentDoc.forms[0];
					var hidden_bundle_info=this.contentDoc.getElementById("hidden_bundle_info").value;//po id
					
					if (hidden_bundle_info!="")
					{ 
						single_bundle_info=hidden_bundle_info.split("*");
						if(($("#txt_issue_challan_id").val()*1)>0)
						{
							if(($("#txt_issue_challan_id").val()*1)!=single_bundle_info[0])
							{
						...
						var tot_row=$('#tbl_details tbody tr').length; 
						if( (tot_row*1)<1)
							get_php_form_data(single_bundle_info[3]+"_"+single_bundle_info[0],'load_mst_data_bundle_scan','requires/embrodary_challan_receive_controller');
						
						create_scan_row(barcode_no,single_bundle_info[0]);
					}
				}
			}
			else
			{
				...
				//create_row(bundle_no);
				single_bundle_info=ex_challan_duplicate[1].split("*");
				if(($("#txt_issue_challan_id").val()*1)>0 && (single_bundle_info[0]*1)>0)
				{
					if(($("#txt_issue_challan_id").val()*1)!=single_bundle_info[0])
					{
						alert("Challan Mix Not Allowed");
	...
	function create_scan_row(barcode_nos,challan_id)
	{
		/*freeze_window(5);
		var row_num=$('#txt_tot_row').val();
		var response_data=return_global_ajax_value(barcode_nos+"**"+row_num+"**"+challan_id+"**"+$('#cbo_company_name').val(), 'populate_bundle_data', '', 'requires/embrodary_challan_receive_controller');
		$('#tbl_details tbody').prepend(response_data);
		var tot_row=$('#tbl_details tbody tr').length; 
		$('#txt_tot_row').val(tot_row);
		release_freezing();*/
		...
		var data = barcode_nos+"**"+row_num+"**"+challan_id+"**"+$('#cbo_company_name').val();
		// var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"****"+$('#cbo_company_name').val()+"**"+vscan+"**"+hidden_source_cond, 'populate_bundle_data', '', 'requires/embroidery_delivery_entry_controller');

		$.ajax({
			type        : 'POST',
			url         : 'requires/embrodary_challan_receive_controller.php',
			...
			success 	: function(data){
				...
				if (orderIdZs !="" )
				{ 
					load_drop_down( 'requires/embrodary_challan_receive_controller', $('#cbo_embel_name').val()+'*'+orderIdZs, 'load_drop_down_embl_type', 'embro_type_td' );
				}
				//alert(orderIdZs);
				release_freezing();	
			}
	...
	function create_row(bundle_nos)
	{
		...
		var row_num=$('#txt_tot_row').val();
		var issue_challan_id=$("#txt_issue_challan_id").val();
		var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"**"+issue_challan_id+"**"+$('#cbo_company_name').val(), 'populate_bundle_data', '', 'requires/embrodary_challan_receive_controller');
		$('#tbl_details tbody').prepend(response_data);
		var tot_row=$('#tbl_details tbody tr').length; 
		$('#txt_tot_row').val(tot_row);
		release_freezing();
		...
		freeze_window(5);*/
		let vscan='';
		let hidden_source_cond='';

		var row_num=$('#txt_tot_row').val();
		var action='populate_bundle_data';
		var data = bundle_nos+"**"+row_num+"****"+$('#cbo_company_name').val()+"**"+vscan+"**"+hidden_source_cond;
		// var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"****"+$('#cbo_company_name').val()+"**"+vscan+"**"+hidden_source_cond, 'populate_bundle_data', '', 'requires/embroidery_delivery_entry_controller');
	...
	function openmypage_issue_challan()
	{
		...
		var delivery_basis=$('#delivery_basis').val();
		var cbo_company_name=$('#cbo_company_name').val();		
		var page_link = 'requires/embrodary_challan_receive_controller.php?delivery_basis='+delivery_basis+'&cbo_company_name='+cbo_company_name+'&action=isssue_challan_no_popup';
		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				if(delivery_basis==3)
				{
					var bundle_nos=return_global_ajax_value(all_data[0], 'bundle_nos', '', 'requires/embrodary_challan_receive_controller');
					if (bundle_nos!="")
					{ 
						//get_php_form_data(bundle_nos,'load_mst_data','requires/embrodary_challan_receive_controller');
						$('#txt_tot_row').val(0);
						$('#tbl_details tbody tr').remove();
				...
				{
					show_list_view(all_data[0],'show_country_listview','list_view_country','requires/embrodary_challan_receive_controller','');	
					release_freezing();
				}				
			}
		}//end else
	...
	function scan_challan_no(chanal_no)
	{
		...
		if(delivery_basis==3)
		{
			...
			{ 
				...
				var challan_id=return_global_ajax_value(chanal_no, 'get_challan_id', '', 'requires/embrodary_challan_receive_controller');
				var bundle_nos=return_global_ajax_value(challan_id, 'bundle_nos', '', 'requires/embrodary_challan_receive_controller');
				get_php_form_data(bundle_nos+"_"+challan_id,'load_mst_data_bundle_scan','requires/embrodary_challan_receive_controller');
		...
		{
			var response_data=return_global_ajax_value(chanal_no+"**"+cbo_cut_panel_basis, 'check_bundle_data', '', 'requires/embrodary_challan_receive_controller'); 
			...
			show_list_view(all_data[3],'show_dtls_listview','order_list_view','requires/embrodary_challan_receive_controller','');
		}
	}
	function calculate_qcpasss_rescan(id)
	{
		var replace_qty=$("#replaceQty_"+id).val()*1;
		var prod_qty=$("#prod_qty_"+id).val()*1;
		var reject_qty=$("#rejectQty_"+id).val()*1;
		var qc_qty=prod_qty-reject_qty+replace_qty;
		if(replace_qty>qc_qty)
		{
			$("#replaceQty_"+id).val("");
			$("#qty_"+id).val("");
			$("#QcQty_"+id).text("");
	...
	function calculate_qcpasss(id)
	{
		var reject_qty=$("#rejectQty_"+id).val()*1;
		var prod_qty=$("#prod_qty_"+id).val()*1;
		var replac_qty=$("#replaceQty_"+id).val()*1;
		var qc_qty=prod_qty-reject_qty+replac_qty;
	...
	function openmypage_bundle_rescan(page_link,title)
	{
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0];
				var hidden_bundle_nos=this.contentDoc.getElementById("hidden_bundle_nos").value;//po id
				var hidden_challan_id=this.contentDoc.getElementById("hidden_issue_challan_id").value;//bundle No
				if(hidden_bundle_nos!="")
				{ 
					var tot_row=$('#tbl_details tbody tr').length; 
					if( (tot_row*1)<1)
					get_php_form_data(hidden_bundle_nos+"_"+hidden_challan_id,'load_mst_data_bundle_scan','requires/embrodary_challan_receive_controller');
					fnc_rescan_bundle(hidden_bundle_nos,hidden_challan_id);
				}
			}
		}//end else
	}//end function	

	function fnc_rescan_bundle(bundle_no,hidden_challan_id)
	{
		
		freeze_window(5);
		var row_num=$('#txt_tot_row').val();
		var response_data=return_global_ajax_value(bundle_no+"**"+row_num+"**"+hidden_challan_id+"**"+$('#cbo_company_name').val(), 'populate_bundle_data_rescan', '', 'requires/embrodary_challan_receive_controller');
		//alert(222)
		$('#tbl_details tbody').prepend(response_data);
		var tot_row=$('#tbl_details tbody tr').length; 
		$('#txt_tot_row').val(tot_row);
	...
	$('#txt_bundle_rescan').live('keydown', function(e) {
		...
		if (e.keyCode === 13) 
		{
			...
			if(flag==1)
			{
				fnc_duplicate_rescan_bundle(txt_bundle_no);
			}
		}
	});
		
	function fnc_duplicate_rescan_bundle(bundle_no)
	{
		var challan_duplicate=return_ajax_request_value( bundle_no+"__"+$('#cbo_company_name').val(),"challan_duplicate_check_resacn", "requires/embrodary_challan_receive_controller");
		var ex_challan_duplicate=challan_duplicate.split("_");
		if(ex_challan_duplicate[0]==1)
		{
			alert("No Data Found.");
			return;
		}
		...
		if(n>0)
		{
			...
			emailwindow.onclose=function()
			{
				...
				{ 
					...
					var tot_row=$('#tbl_details tbody tr').length; 
					if( (tot_row*1)<1)
						get_php_form_data(bundle_no+"_"+single_bundle_info[0],'load_mst_data_bundle_scan','requires/embrodary_challan_receive_controller');
					create_rescan_row(bundle_no,single_bundle_info[0]);
				}
			}
		}
		else
	...
	function create_rescan_row(bundle_nos,challan_id)
	{
		freeze_window(5);
		var row_num=$('#txt_tot_row').val();
		var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"**"+challan_id+"**"+$('#cbo_company_name').val(), 'populate_bundle_data_rescan', '', 'requires/embrodary_challan_receive_controller');
		$('#tbl_details tbody').prepend(response_data);
		var tot_row=$('#tbl_details tbody tr').length; 
		$('#txt_tot_row').val(tot_row);
		release_freezing();
	}
	function pop_entry_reject(row_id)
	{
		var actual_infos=$("#actual_reject_"+row_id).val();

		var page_link='requires/embrodary_challan_receive_controller.php?action=reject_qty_popup&actual_infos='+actual_infos;
	...
	function show_cost_details()
	{
		...
		var page_link='requires/embrodary_challan_receive_controller.php?action=show_cost_details&sys_id='+system_id;
            ...
            <fieldset>
                <table width="100%">
                    ...
                    <tr>
                        ...
                        <td>
                            <? 
                            	echo create_drop_down( "cbo_company_name", 180, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "get_php_form_data(this.value,'load_variable_settings','requires/embrodary_challan_receive_controller'); load_drop_down( 'requires/embrodary_challan_receive_controller', this.value, 'load_drop_down_body_part', 'body_part_td'); working_com_fnc(); load_html();fnc_load_report_format(this.value);" );//get_php_form_data(this.value,'load_variable_settings_for_working_company','requires/embrodary_challan_receive_controller');
                            ?>
                     ...
                     <tr>
                         ...
                         <td id="embel_name_td">
                             <? 
							 	echo create_drop_down( "cbo_embel_name", 180, $emblishment_name_array,"", 0, "-- Select Embel.Name --", $selected, "",1,2 );
                             ?>
                         </td>
                         <td class="must_entry_caption">Embel. Type</td>
                         <td id="embro_type_td" width="170">
                              <? 
								echo create_drop_down( "cbo_embel_type", 180, $emblishment_embroy_type,"", 1, "--- Select Embroidery ---", $selected, "",1 );  
                             ?>
                         </td>
                    ...
                    <tr>
                        ...
                        <td id="body_part_td">
                        	<? 
								echo create_drop_down( "cbo_working_company_name", 180, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select --",$selected,"load_drop_down( 'requires/embrodary_challan_receive_controller', $('#cbo_working_company_name').val(), 'load_drop_down_working_location', 'working_location_td' );" );
                            ?>
                        </td>
                        </tr>
                        <tr>
							...
							<td> 
								<input type="text" name="txt_boe_mushak_challan_no" id="txt_boe_mushak_challan_no" class="text_boxes" style="width:167px">
							</td>
							<td>BOE/Mushak Challan Date</td>                                              
							<td> 
								<input type="text" name="txt_boe_mushak_challan_date" id="txt_boe_mushak_challan_date" class="datepicker" style="width:167px">
							</td>
					...
					<td>Gate Entry No </td>                                              
							<td> 
								<input type="text" name="get_entry_no" id="get_entry_no" class="text_boxes" style="width:167px">
							</td>
							<td>Gate Entry Date</td>                                              
							<td> 
								<input type="text" name="get_entry_date" id="get_entry_date" class="datepicker" style="width:167px">
							</td>
						<td id="id_remarks1" style="display:none;">Remarks</td> 
                        <td id="id_remarks2" style="display:none;" colspan="3"> 
                           <input type="text" name="txt_remark_bundle" id="txt_remark_bundle" class="text_boxes" style="width:155px" title="450 Characters Only." />
                        </td>
					</tr>
                </table>
                </fieldset> 
                               ...
                               <table  cellpadding="0" cellspacing="2" width="100%">
                                  <tr>
                                       ...
                                       <td colspan="2" width="110"> 
                                           <input name="txt_bundle_no" placeholder="Browse/Write/Scan" onDblClick="openmypage_bundle('requires/embrodary_challan_receive_controller.php?action=bundle_popup&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value,'Bundle Search')" id="txt_bundle_no" class="text_boxes" style="width:212px" />
                                       </td>
                                       <td width="100" class="must_entry_caption" id="td_caption">Re-Scan Barcode</td>
                                       <td colspan="2" width="110"> 
                                           <input name="txt_bundle_rescan" placeholder="Browse/Write/Scan" onDblClick="openmypage_bundle_rescan('requires/embrodary_challan_receive_controller.php?action=bundle_popup_rescan&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value,'Search Bundle For Rescan')"  id="txt_bundle_rescan" class="text_boxes" style="width:212px" />
                                       </td>
									   <td>
                ...
                <div id="bundle_list_view" >
                	<table cellpadding="0" width="970" cellspacing="0" border="1" class="rpt_table" rules="all">
                        <thead>
                            <th width="30">SL</th>
                            ...
                            <th width="50">Rep. Qty.</th>
                            <th width="50">Qc. Pass Qty.</th>
                            <th></th>
                        </thead>
              		</table>
                    <div style="width:970px; max-height:250px; overflow-y:scroll" align="left">    
                    	<table cellpadding="0" width="950" cellspacing="0" border="1" class="rpt_table" rules="all" id="tbl_details">      
                            <tbody>
                            </tbody>
                        </table>
						<table  cellspacing="0" border="1"  id="tbl_details_1">      
							<tfoot>
								<tr>
									...
									<th width="90"></th>
									<th width="100"></th>
									<th width="80" style="font-size:25px ;"><b>Num. of Bundel</b></th>
									<th width="80" id="no_of_bundel"></th>
									<th width="60" style="font-size:25px ;"><b>Gtms.Qty</b></th>
									<th width="50" id="gtms_qty"></th>
									<th width="50"></th>
									<th width="50"></th>
									<th width="50"></th>
									<th></th>
               		...
               		<tr>
                        <td align="center" colspan="9" valign="middle" class="button_container">
                            ...
                            ?>
                             ...
                             <input type="button" id="btn_print2" name="btn_print2" style="width:120px;"  class="formbutton_disabled" value="Size Wise Print 2"  onClick="generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val(),'embrodary_color_wise_print_2','requires/print_embro_bundle_receive_controller');"/>
                              <input type="button" id="btn_print5" name="btn_print5" style="width:120px;"  class="formbutton_disabled" value="Print 5"  onClick="generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val(),'embrodary_color_wise_print_5','requires/print_embro_bundle_receive_controller');"/>

							  <input type="button" id="btn_print6" name="btn_print6" style="width:120px;"  class="formbutton_disabled" value="Print 6"  onClick="generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val(),'embrodary_color_wise_print_6','requires/print_embro_bundle_receive_controller');"/>
