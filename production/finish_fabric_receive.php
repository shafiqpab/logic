...
// Note: Microfiber live and Dev
// Issue id: 11151
// If not found Dia
// 1# remove prod_id of grouping from side list view when browse batch no
// 2# remove where condition of Prod_id from load_drop_down_body_part action and add some prod information in where condition
// 3# in po_popup remove prod_id grouping from main query for save and unsaved mode 
...
<script>
	...
	<?
	if(!is_null($_SESSION['logic_erp']['data_arr'][7]))
	{
		$data_arr= json_encode( $_SESSION['logic_erp']['data_arr'][7]); 
	...
	function set_production_basis()
	{
		...
		else if(production_basis == 5)
		{
			...
			$('#txt_batch_no').attr("placeholder","Browse");
	...
	function openmypage_systemid()
	{
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				reset_form('finishFabricEntry_1','list_container_finishing*list_fabric_desc_container','','','','roll_maintained*fabric_store_auto_update*process_costing_maintain*textile_sales_maintain');
	...
	function openmypage_po()
	{
		...
		emailwindow.onclose=function()
		{
			...
			$('#hidden_grey_used_po').val(roundN(tot_grey_qnty,2));
	...
	function fnc_finish_production( operation )
	{
		...
		if(operation==5)
		{
			...
			var report_title=$( "div.form_caption" ).html();
			print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_system_id').val()+'*'+report_title, "finish_fab_production_print2", "requires/finish_fabric_receive_controller")
			show_msg("3");
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			if('<?php echo implode('*',$_SESSION['logic_erp']['mandatory_field'][7]);?>')
			{
				if (form_validation('<?php echo implode('*',$_SESSION['logic_erp']['mandatory_field'][7]);?>','<?php echo implode('*',$_SESSION['logic_erp']['field_message'][7]);?>')==false)
				{
			...
			if(is_sales == 1 && sales_order_type==2 && textile_sales_maintain ==2)
			{
				//N. B.  No need for rate, amount validation for (service sales order) and (textile sales maintain variable NO)
			}
			else
			{
				if(is_sales == 1)
					...
					if($("#process_costing_maintain").val()==1)
					{
						if($("#cbo_production_basis").val() ==5)
						{
							...
							if($("#batch_booking_without_order").val()!=1)
							{
								...
								if($("#txt_grey_used").val()!=$("#hidden_grey_used_po").val())
								{
									alert("Please Select Grey Used Qty again");
									return;
								}
	...
	function set_form_data(data) //
	{
		...
		load_drop_down('requires/finish_fabric_receive_controller', prod_id+"**"+txt_batch_id+"**"+data[0]+"**"+data[1]+"**"+data[2]+"**"+encodeURIComponent(data[3])+"**"+data[2]+"**"+encodeURIComponent(data[3])+"**"+data[5]+"**"+data[6]+"**"+data[7], 'load_drop_down_body_part','body_part_td');
		...
		get_php_form_data(txt_batch_id+'_'+data[1]+ '_'+ $('#cbo_body_part').val() + '_' + data[5] + '_' + data[2] + '_' + encodeURIComponent(data[3]), "populate_data_from_batch", "requires/finish_fabric_receive_controller" );
	...
	function check_batch(data,extention_no)
	{
		...
		if(production_basis==5)
		{
			...
			if(batch_id==0)
			{
				...
				reset_form('','list_fabric_desc_container','txt_batch_no*txt_batch_id*txt_batch_qty*batch_booking_without_order*is_sales*txt_fabric_desc*fabric_desc_id*txt_color*txt_production_qty*buyer_name*buyer_id*txt_dia_width*txt_gsm*all_po_id*save_data*distribution_method_id*txt_batch_qnty*txt_total_received*txt_yet_receive','','','');
				return;
			}
			else
			{
	...
	function proces_costing_popup()
	{
		...
		var txt_production_date=$('#txt_production_date').val();
		var update_dtls_id=$('#update_dtls_id').val();
		var update_id=$('#update_id').val();
		...
		var txt_batch_id = $('#txt_batch_id').val();
		var txt_process_id = $('#txt_process_id').val();
		var hidden_grey_used_po = $('#hidden_grey_used_po').val();
		...
		var page_link='requires/finish_fabric_receive_controller.php?recieve_basis='+recieve_basis+'&booking_id='+service_booking_id+'&booking_without_order='+service_booking_without_order+'&fabric_description_id='+fabric_description_id+'&txt_receive_qnty='+txt_receive_qnty+'&txt_job_no='+txt_job_no+'&all_po_id='+all_po_id+'&txt_receive_date='+txt_production_date+'&update_dtls_id='+update_dtls_id+'&update_id='+update_id+'&kitting_charge='+kitting_charge+'&kitting_charge_2nd='+kitting_charge_2nd+'&save_data='+save_data+'&name_color='+name_color+'&is_sales='+hdn_is_sales+'&txt_batch_id='+txt_batch_id + '&cbo_company_id='+cbo_company_id + '&txt_process_id=' + "'" + txt_process_id + "'" + '&booking_no='+ txt_service_booking + '&txt_gsm='+ txt_gsm+ '&txt_dia_width='+ txt_dia_width+ '&hidden_grey_used_po='+ hidden_grey_used_po + '&action=yarn_lot_popup';
...
<body onLoad="set_hotkey()">
	<div style="width:100%;" align="center">
		...
		<form name="finishFabricEntry_1" id="finishFabricEntry_1" autocomplete="off" >
					...
					<table cellpadding="0" cellspacing="1" width="810" border="0">
								...
								<fieldset>
									...
									<table cellpadding="0" cellspacing="2" width="100%">
										<tr>
											...
											<td>
												<input type="text" name="txt_batch_no" id="txt_batch_no" class="text_boxes" style="width:120px;" placeholder="Browse" readonly onDblClick="openmypage_batchnum();" onChange="check_batch(this.value,document.getElementById('txt_batch_extantion').value);" />
											...
											<td>
												<input type="text" name="txt_production_qty" id="txt_production_qty" class="text_boxes_numeric" placeholder="Double  Click" style="width:130px;" ondblclick="openmypage_po()" readonly />
											</td>
										</tr>
										<tr>
										...
										<tr>
											...
											<td>Batch GSM /Grey GSM /Old GSM</td>
											<td>
												<input type="text" name="txt_gsm_show" id="txt_gsm_show" class="text_boxes" style="width:130px;" placeholder="Display" disabled readonly/>
											</td>
										</tr>
										<tr>
											<td>Dia/Width</td>
											...
											<td>Batch dia /Grey dia /Old dia</td>
											<td>
												<input type="text" name="txt_dia_width_show" id="txt_dia_width_show" class="text_boxes" style="width:130px;" maxlength="10" placeholder="Display" disabled readonly />
											</td>
										</tr>
										<tr>
											<td>Buyer</td>
										...
										<tr>
											...
											<td>
												...
												<input type="hidden" name="hidden_grey_used_po" id="hidden_grey_used_po" />
											</td>
										...
										<tr>
											<td>Batch Status</td>
											<td>
												<? echo create_drop_down( "cbo_batch_status", 180,$batch_status_array,"", 0, "--  --", 1, "",0 ); ?>
											</td>
											<td>No Of Roll</td>
											<td>
						...
						<tr>
							<td align="center" colspan="4" class="button_container">
								...
								<input type="hidden" name="textile_sales_maintain" id="textile_sales_maintain" readonly class="text_boxes">
								<input type="hidden" name="sales_order_type" id="sales_order_type" readonly class="text_boxes">

								<input type="button" name="print_2" id="print_2" value="Print 2" onClick="fnc_finish_production(5)" style="width:100px;" class="formbutton" />
							</td>
						</tr>
					</table>
			...
			<div id="list_fabric_desc_container" style="width:500px; overflow:auto; float:left; padding-top:5px; margin-top:5px; position:relative;"></div>
		</form>
	</div>
</body>
