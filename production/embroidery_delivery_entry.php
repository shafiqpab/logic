...
function openmypage_sysNo()
{
	...
	var page_link = 'requires/embroidery_delivery_entry_controller.php?cbo_company_name='+cbo_company_name+'&action=challan_no_popup';
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			if(mst_id!="")
			{
				...
				get_php_form_data(mst_id, "populate_data_from_challan_popup", "requires/embroidery_delivery_entry_controller" );

				var delivery_basis=$('#delivery_basis').val();
				if(delivery_basis==3)
				{
					// var bundle_nos=return_global_ajax_value(mst_id, 'bundle_nos', '', 'requires/embroidery_delivery_entry_controller');
					var response_data=return_global_ajax_value("0**0**"+mst_id+"**"+$('#cbo_company_name').val(), 'populate_bundle_data_update', '', 'requires/embroidery_delivery_entry_controller');
					// alert(response_data);
				...
				{
					show_list_view(mst_id,'show_dtls_listview','printing_production_list_view','requires/embroidery_delivery_entry_controller','');
				}
				release_freezing();
			}
		}
...
{
	var report_ids = return_global_ajax_value( data, 'load_report_format', '', 'requires/embroidery_delivery_entry_controller');
	...
	if(work_order_material_auto_receive==1)
	{
		...
		$('#txt_embroidery_order_no').attr('disabled',false);
		$('#printing_work_oder').css('color','blue');
 	}
	else
	{
		$('#txt_embroidery_order_no').attr('disabled','true');
...
function openmypage(page_link,title)
{
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			if (po_id!="")
			{
				...
				get_php_form_data(po_id+'**'+item_id+'**'+$('#cbo_embel_name').val()+'**'+country_id, "populate_data_from_search_popup", "requires/embroidery_delivery_entry_controller" );
				...
				else
				{
					...
					get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+$("#cbo_embel_name").val()+'**'+country_id, "color_and_size_level", "requires/embroidery_delivery_entry_controller" );
				}

				show_list_view(po_id,'show_country_listview','list_view_country','requires/embroidery_delivery_entry_controller','');
	...
	{
		window.open("requires/embroidery_delivery_entry_controller.php?data=" + data+'&action='+action, true );
	}

function fnc_issue_print_embroidery_entry(operation)
{
	var company_id=$('#cbo_company_name').val();
	var working_company_mandatory=return_global_ajax_value(company_id, 'load_variable_settings_for_working_company', '', 'requires/embroidery_delivery_entry_controller');
	let work_order_material_auto_receive = $("#work_order_material_auto_receive").val()*1;
	if(working_company_mandatory==1)
	{
 	...
 	if(work_order_material_auto_receive==1)
	{
			if($('#txt_embroidery_order_no').val()=="")
			{
				release_freezing();
	...
	{
		var report_title=$( "div.form_caption" ).html();
		generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val()+'*'+report_title, 'emblishment_issue_print_2', 'requires/print_embro_delivery_entry_controller');
		return;
	}
	else if(operation==6)
	{
		var report_title=$( "div.form_caption" ).html();
		generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val()+'*'+report_title, 'emblishment_issue_print_3', 'requires/print_embro_delivery_entry_controller');
		return;
	}
	else if(operation==7)
	{
		...
		if($('#cbo_company_name').val()==0) {
			...
			alert("Select Company name first"); return;

			}else if($('#txt_challan_no').val()=="") {

			alert("Pls, browse challan No"); return;
 	...
 	if(http.readyState == 4)
	{
		...
		else if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
		{
			...
			else
			{
				...
				show_list_view(reponse[1],'show_dtls_listview','printing_production_list_view','requires/embroidery_delivery_entry_controller','');
...
function openmypage_bundle(page_link,title)
{
	...
	if(work_order_material_auto_receive==1)
	{
		...
		if ( form_validation('txt_embroidery_order_no','Embroidery Work Order')==false )
		{
		return;
		}
 	}
	...
	else
	{
		...
		emailwindow.onclose=function()
		{
			...
			{
				//fnc_duplicate_bundle(hidden_bundle_nos,"Browse",hidden_source_cond);
...
function create_row(bundle_nos,vscan,hidden_source_cond)
{
	...
	$.ajax({
	    type        : 'POST',
	    url         :  'requires/embroidery_delivery_entry_controller.php',
	    ...
	    success 	: function(data){
			...
			if (orderIdZs !="" )
			{
				load_drop_down( 'requires/embroidery_delivery_entry_controller', $('#cbo_embel_name').val()+'*'+orderIdZs, 'load_drop_down_embl_type', 'embro_type_td' );
			}
			//alert(orderIdZs);
			release_freezing();
	    }
...
{
	var challan_duplicate=return_ajax_request_value( bundle_no+"__"+$('#cbo_company_name').val(),"challan_duplicate_check", "requires/embroidery_delivery_entry_controller");
...
function create_rescanrow(bundle_nos,vscan,hidden_source_cond)
{
	...
	if(error==0)
	{
		freeze_window(5);
		var row_num=$('#txt_tot_row').val();
		var response_data=return_global_ajax_value(bundle_nos+"**"+row_num+"****"+$('#cbo_company_name').val()+"**"+vscan+"**"+hidden_source_cond, 'populate_bundle_data_rescan', '', 'requires/embroidery_delivery_entry_controller');
		$('#tbl_details tbody').prepend(response_data);
		var tot_row=$('#tbl_details tbody tr').length;
		$('#txt_tot_row').val(tot_row);
		release_freezing();
...
$('#txt_bundle_rescan').live('keydown', function(e) {
	...
	function show_cost_details()
	{
		...
		var page_link='requires/embroidery_delivery_entry_controller.php?action=show_cost_details&sys_id='+system_id;
...
function openmypage_order()
{
	...
	{
		var title = 'Embrodary Order No. Pop-up';
		var page_link = 'requires/embroidery_delivery_entry_controller.php?company='+company+'&party_name='+party_name+'&action=embroidery_order_popup';
		...
		emailwindow.onclose=function()
		{
			...
			if (theemaildata!="")
			{
				...
				$('#txt_embroidery_order_no').val(ex_data[1]);
            ...
            <fieldset>
                <table width="100%">
                    ...
                    <tr>
                        ...
                        <td>
                            <?
							  	//echo create_drop_down( "cbo_company_name", 180, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/embroidery_delivery_entry_controller', this.value, 'load_drop_down_location', 'location_td' );get_php_form_data(this.value,'load_variable_settings','requires/embroidery_delivery_entry_controller');load_drop_down( 'requires/embroidery_delivery_entry_controller', this.value, 'load_drop_down_body_part', 'body_part_td' );load_html();" );
                            	echo create_drop_down( "cbo_company_name", 180, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "get_php_form_data(this.value,'load_variable_settings','requires/embroidery_delivery_entry_controller'); load_drop_down( 'requires/embroidery_delivery_entry_controller', this.value, 'load_drop_down_body_part', 'body_part_td' ); working_com_fnc(); load_html();fnc_load_report_format(this.value);" );//get_php_form_data(this.value,'load_variable_settings_for_working_company','requires/embroidery_delivery_entry_controller');
                            ?>
                         ...
                         <td id="embro_type_td" width="170">
                             <?
								echo create_drop_down( "cbo_embel_type", 180, $emblishment_embroy_type,"", 1, "--- Select Embroidery ---", $selected, "" );
                             ?>
                         </td>
                    </tr>
                    <tr>
                          ...
                          <td>
                              <?
                              	echo create_drop_down( "cbo_source", 180, $knitting_source,"", 1, "-- Select Source --", $selected, "load_drop_down( 'requires/embroidery_delivery_entry_controller', this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_embro_issue_source', 'emb_company_td' );load_drop_down( 'requires/embroidery_delivery_entry_controller', $('#cbo_company_name').val(), 'load_drop_down_location', 'location_td' );", 0, '1,3' );
                              ?>
                          </td>
                    ...
                    <tr>
                        ...
                        <td id="body_part_td">
                        	<?
								echo create_drop_down( "cbo_working_company_name", 180, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select --",$selected,"load_drop_down( 'requires/embroidery_delivery_entry_controller', $('#cbo_working_company_name').val(), 'load_drop_down_working_location', 'working_location_td' );" );
                            ?>
                        </td>
					...
					<tr>
						...
						<td  id="printing_work_oder"><strong>Work Order</strong></td>
                        <td><input name="txt_embroidery_order_no" id="txt_embroidery_order_no" type="text"  class="text_boxes" style="width:170px" placeholder="Browse" onDblClick="openmypage_order();" disabled />
                                       ...
                                       <table  cellpadding="0" cellspacing="2" width="100%">
                                          <tr>
                                               ...
                                               <td colspan="3" width="110">
                                                   <input name="txt_order_no" placeholder="Double Click to Search" onDblClick="openmypage('requires/embroidery_delivery_entry_controller.php?action=order_popup&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value,'Order Search')" id="txt_order_no" class="text_boxes" style="width:212px" readonly />
                    	...
                    	<td>
                            <fieldset>
                              <legend>New Entry</legend>
                               <table  cellpadding="0" cellspacing="2" width="100%">
                                  <tr>
                                       <td width="100" class="must_entry_caption" id="td_caption">Barcode No</td>
                                       <td colspan="2" width="110">
                                           <input name="txt_bundle_no" placeholder="Browse/Write/Scan" onDblClick="openmypage_bundle('requires/embroidery_delivery_entry_controller.php?action=bundle_popup&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value,'Bundle Search')" id="txt_bundle_no" class="text_boxes" style="width:212px" />
                                       </td>
                                        <td width="100" class="must_entry_caption" id="td_caption">Re-Scan Barcode</td>
                                       <td colspan="2" width="110">
                                           <input name="txt_bundle_rescan" placeholder="Browse/Write/Scan" onDblClick="openmypage_bundle_rescan('requires/embroidery_delivery_entry_controller.php?action=bundle_popup_rescan&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value,'Search Bundle For Rescan')"  id="txt_bundle_rescan" class="text_boxes" style="width:212px" />
                                       </td>
									   <td>
               	...
               	<table cellpadding="0" cellspacing="1" width="100%">
               		<tr>
                        <td align="center" colspan="9" valign="middle" class="button_container">
                            <?
								$date=date('d-m-Y');
                                echo load_submit_buttons( $permission, "fnc_issue_print_embroidery_entry", 0,1 ,"reset_form('printembro_1','list_view_country*printing_production_list_view','','txt_issue_date,".$date."*cbo_embel_name,2','disable_enable_fields(\'cbo_company_name*cbo_source*cbo_emb_company*cbo_location*cbo_floor\',0);pageReset();')",0);
                            ?>
