...
<script>
	...
	function openmypage_po()
	{
		...
		else
		{
			...
			var page_link ='requires/ready_to_sewing_entry_controller.php?company_id='+cbo_company_id+'&cbo_buyer_name='+cbo_buyer_name+'&cbo_year='+cbo_year+'&action=po_popup';
			...
			emailwindow.onclose=function()
			{
				...
				{
					freeze_window(5);
					show_list_view( order_id+'**'+0+'**'+job_no, 'order_details_list', 'sewing_dtls_container', 'requires/ready_to_sewing_entry_controller', '' ) ;
	...
	function fnc_ready_to_sewing( operation )
	{
		...
		var data="action=save_update_delete&operation="+operation+'&tot_row='+j+get_submitted_data_string('txt_sewing_no*update_id*cbo_company_id*cbo_buyer_name*txt_sewing_date*txt_order_no*txt_order_id' ,"../")+dataString;
		//alert(data);return;
            ...
            <fieldset style="width:660px;">
                ...
                <table cellpadding="0" cellspacing="2" width="650" id="tbl_mst">
                    ...
                    <tr>
                        ...
                        <td width="150">
                            <? 
                                echo create_drop_down( "cbo_company_id", 132, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", 0, "load_drop_down( 'requires/ready_to_sewing_entry_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );",0 );
                            ?>
                        </td>
