...
http://202.4.105.82/erp/production/reports/company_location_wise_display_dashboard.php?url=Comfit Composite Knit Ltd./Unit -02/
*/
require_once('../../includes/common.php');
$url_title = ltrim($_SERVER['PHP_SELF'], '/');
$url = rtrim($_GET['url'], '/');
$url = explode('/', $url);
$url_company_name = $url[0];
...
<script> 
    ...
    function fn_efficiency_board(efficiency)
    {
    ...
    $(document).ready(function() {
        var cbo_company_name = '<? echo $url_company_name; ?>';
        var cbo_location_name = '<? echo $url_location_name; ?>';
        var txt_date = '<? echo date('d-M-Y'); ?>';
        //var txt_date = '29-Jan-2020';

        var data="action=report_generate"+"&max_height='"+max_height+"'"+"&txt_date='"+txt_date+"'"+"&cbo_company_name='"+cbo_company_name+"'"+"&cbo_location_name='"+cbo_location_name+"'";

        var numSuccessfullAjaxCallCount = 0;
        var ajax_call = function(){
            $.ajax({
                type:'POST',
                url:'requires/company_location_wise_display_dashboard_controller.php',
                data:data,
                /*beforeSend:function(){
                    $('#loading').fadeIn('fast');
                },*/
                success:function(response){
                    ...
                    fn_efficiency_board(efficiency);                
    ...
    {  
        ...
        var data="action=report_generate"+"&max_height='"+max_height+"'"+"&txt_date='"+txt_date+"'"+"&cbo_company_name='"+cbo_company_name+"'"+"&cbo_location_name='"+cbo_location_name+"'";
        //alert(data);
        http.open("POST","requires/company_location_wise_display_dashboard_controller.php",true);
