<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Dyeing Fin Recipe Entry
Functionality	:
JS Functions	:
Created by		:	Aziz
...
//    });
	...
	function show_details()
	{
		...
		if(val==2)
		{
			var list_view_orders = return_global_ajax_value( cbo_company_id+'**'+cbo_sub_process+'******'+cbo_store_name+'**'+variable_lot, 'item_details', '', 'requires/dyeing_fin_recipe_entry_controller');
			if(list_view_orders!='')
			{
				//$("#tbl_list tr").remove();
		...
		//show_list_view(cbo_company_id+'**'+cbo_sub_process, 'item_details', 'list_container_recipe_items', 'requires/dyeing_fin_recipe_entry_controller','setFilterGrid("tbl_list_search",-1)');
	...
	{
		var variable_recipe = $('#variable_recipe').val();
		if(variable_recipe!=2)
		{
			alert('Recipe Maintain must be seperate Recipe');
			return;
		}
		...
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_labdip_no').val()+'*'+report_title, "recipe_entry_print", "requires/dyeing_fin_recipe_entry_controller")
			//return;
			show_msg("3");
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			if( form_validation('cbo_working_company_id*txt_batch_no*cbo_company_id*txt_recipe_date*cbo_order_source*txt_color*txt_total_liquor_ratio*cbo_store_name','Batch*Working Company*Company*Recipe Date*Order Source*Color*Total Liquor*Store')==false )
			{
				return;
			}
			...
			if(operation==2){
				...
				var response = return_global_ajax_value( cbo_company_id+'**'+update_id_check+'**'+cbo_sub_process+'**'+txt_batch_id, 'item_req_check_data', '', 'requires/dyeing_fin_dyeing_fin_recipe_entry_controller');
			...
			if(copy_val==1) //Copy Item
			{
				var cbo_company_id = $('#cbo_working_company_id').val();
				var update_id_check = $('#update_id_check').val();
				/*var response = return_global_ajax_value( cbo_company_id+'**'+update_id_check, 'item_stock_details', '', 'requires/dyeing_fin_recipe_entry_controller');
			...
			if(copy_val==2)
			{
				...
				var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_labdip_no*cbo_company_id*cbo_working_company_id*cbo_location*txt_recipe_date*cbo_order_source*txt_booking_order*txt_booking_id*txt_recipe_des*txt_batch_id*cbo_buyer_name*cbo_method*txt_color*cbo_color_range*txt_liquor*txt_batch_ratio*txt_liquor_ratio*txt_booking_type*cbo_sub_process*txt_batch_weight*txt_remarks*copy_id*update_id*txt_total_liquor_ratio*txt_liquor_ratio_dtls*surpls_solution*txt_recipe_serial_no*txt_copy_from*cbo_store_name*txt_subprocess_seq*txt_trough_water',"../")+data_all+'&total_row='+i+'&delete_cause='+delete_cause;
				//alert(data);return;
			}
			else if(copy_val==1)
			{
				var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_labdip_no*cbo_company_id*cbo_working_company_id*cbo_location*txt_recipe_date*cbo_order_source*txt_booking_order*txt_booking_id*txt_recipe_des*txt_batch_id*cbo_buyer_name*cbo_method*txt_color*cbo_color_range*txt_liquor*txt_batch_ratio*txt_liquor_ratio*txt_booking_type*cbo_sub_process*txt_subprocess_remarks*txt_batch_weight*txt_remarks*copy_id*update_id_check*txt_total_liquor_ratio*txt_liquor_ratio_dtls*surpls_solution*txt_recipe_serial_no*txt_copy_from*cbo_store_name*txt_subprocess_seq*txt_trough_water',"../")+'&delete_cause='+delete_cause;
			}
			//alert (data);return;
			freeze_window(operation);
			http.open("POST","requires/dyeing_fin_recipe_entry_controller.php",true);
	...
	function fnc_recipe_entry_Reply_info()
	{
		if(http.readyState == 4)
		{
			...
			if((reponse[0]==0 || reponse[0]==1 || reponse[0]==2))
			{
				...
				{
				show_list_view(reponse[1], 'recipe_item_details', 'recipe_items_list_view', 'requires/dyeing_fin_recipe_entry_controller', '' ) ;
				setFilterGrid('tbl_list_search',-1);
				...
				$('#btn_recipe_calc3').removeAttr('disabled','disabled').attr('class','formbutton');
    ...
    function fn_recipe_calc(operation)
    {
        if(operation=="btn_recipe_calc")
		{
			...
			var report_title=$( "div.form_caption" ).html();
			print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_labdip_no').val()+'*'+$('#txt_yarn_lot').val()+'*'+$('#txt_brand').val()+'*'+$('#txt_count').val()+'*'+0+'*'+$('#surpls_solution').val()+'*'+$('#txt_batch_id').val()+'*'+$('#cbo_sub_process').val()+'*'+report_title, "recipe_entry_print_2", "requires/dyeing_fin_recipe_entry_controller")
			//return;
			show_msg("3");
		}
		else if(operation=="btn_recipe_calc3")
		{
            var report_title=$( "div.form_caption" ).html();
			print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_labdip_no').val()+'*'+report_title, "recipe_entry_print3", "requires/dyeing_fin_recipe_entry_controller")
			//return;
			show_msg("3");
		}
    }
	...
	function openmypage_sysNo()
	{
		...
		var page_link = 'requires/dyeing_fin_recipe_entry_controller.php?cbo_company_id='+cbo_company_id+'&cbo_working_company_id='+cbo_working_company_id+'&action=systemid_popup';
		...
		emailwindow.onclose=function()
		{
			...
			if(update_id!="")
			{
				freeze_window(5);
				$('#list_container_recipe_items').html('');
				get_php_form_data(update_id, "populate_data_from_search_popup", "requires/dyeing_fin_recipe_entry_controller" );
				var sub_process = $('#cbo_sub_process').val();
				//alert(sub_process);
				/*if(!(sub_process==93 || sub_process==94 || sub_process==95 || sub_process==96 || sub_process==97 || sub_process==98))
				{
				show_list_view(update_id, 'recipe_item_details', 'recipe_items_list_view', 'requires/dyeing_fin_recipe_entry_controller', '' ) ;
				setFilterGrid('tbl_list_search',-1);
				reset_form('','','copy_id*copy_sub_process*cbo_sub_process','','');
				set_button_status(0, permission, 'fnc_recipe_entry',1,1);
				$('#copy_id').removeAttr('disabled','disabled');
	...
	/*function openmypage_booking()
	{
		...
		else
		{
			...
			var page_link = 'requires/dyeing_fin_recipe_entry_controller.php?cbo_company_id='+cbo_company_id+'&cbo_buyer_name='+cbo_buyer_name+'&action=booking_popup';
 ...
 //alert(process_remark);
		...
		var cbo_store_name = $('#cbo_store_name').val();
		var variable_lot = $('#variable_lot').val();
		show_list_view(cbo_company_id+'**'+sub_process_id+"**"+update_id+"**"+copy_val+"**"+cbo_store_name+"**"+variable_lot, 'item_details', 'list_container_recipe_items', 'requires/dyeing_fin_recipe_entry_controller', '');
		setFilterGrid('tbl_list_search',-1);
	...
	function openmypage_itemLot(id)
	{
		...
		else
		{
			...
			var page_link = 'requires/dyeing_fin_recipe_entry_controller.php?cbo_company_id='+cbo_company_id+'&txt_prod_id='+txt_prod_id+'&txt_item_lot='+txt_item_lot+'&action=itemLot_popup';
	...
	function caculate_tot_liquor()
	{
		...
		var pickup_per=$('#txt_liquor_ratio_dtls').val()*1;
		...
		var tot_liquor_dtls=(batch_weight)*(pickup_per/100);
		if(pickup_per>0)
		{
	...
	function openmypage_batchNo()
	{
		...
		else
		{
			...
			var page_link = 'requires/dyeing_fin_recipe_entry_controller.php?cbo_company_id='+cbo_company_id+'&action=batch_popup';
			...
			emailwindow.onclose=function()
			{
				...
				if(batch_id!="")
				{
					...
					get_php_form_data(cbo_company_id+'**'+batch_val[0]+'**'+batch_val[1], "load_data_from_batch", "requires/dyeing_fin_recipe_entry_controller" );
					caculate_tot_liquor();
					release_freezing();
				}
			}
	...
	function copy_check(type)
	{
		...
		if(type==1)
		{
				...
				var response = return_global_ajax_value( working_company_id+'**'+recipe_date, 'recipe_previ_copy_check', '', 'requires/dyeing_fin_recipe_entry_controller');
				if(response=="")
				{
			...
			$("#list_container_recipe_items").html('');
			show_list_view($('#txt_sys_id').val(), 'recipe_item_details', 'recipe_items_list_view', 'requires/dyeing_fin_recipe_entry_controller', '' ) ;
			$('#update_id').val('');
			$('#txt_sys_id').val('');
			$('#txt_recipe_date').val('');
			$('#txt_labdip_no').val('');
	...
	function subprocess_change(sub_process)
	{
		...
		get_php_form_data(cbo_company_id+'**'+sub_process+'**'+update_id_check, "ratio_data_from_dtls", "requires/dyeing_fin_recipe_entry_controller" );
	}
	...
	function rcv_variable_check(str)
	{
		...
		var lots_variable=return_global_ajax_value( company_id, 'populate_data_lib_data', '', 'requires/dyeing_fin_recipe_entry_controller');
	...
	{
		var company_id=$('#cbo_working_company_id').val();
		get_php_form_data(company_id, "populate_stock_data", "requires/dyeing_fin_recipe_entry_controller" );
	}
	function recipe_variable_check()
	{
		var company_id=$('#cbo_working_company_id').val();
		get_php_form_data(company_id, "populate_recipe_data", "requires/dyeing_fin_recipe_entry_controller" );
	}
				...
				<table width="910" align="center" border="0">
                	...
                	<tr>
                        ...
                        <td>
                         ...
                         <td>
                            <?
							//load_drop_down('requires/dyeing_fin_recipe_entry_controller', this.value, 'load_drop_down_location', 'location_td' );load_drop_down('requires/dyeing_fin_recipe_entry_controller', this.value, 'load_drop_down_buyer', 'buyer_td_id' );
                                echo create_drop_down( "cbo_working_company_id", 152, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "--Select Company--", 0, "load_drop_down('requires/dyeing_fin_recipe_entry_controller', this.value, 'load_drop_down_location', 'location_td' );rcv_variable_check(2);stock_variable_check();load_room_rack_self_bin('requires/dyeing_fin_recipe_entry_controller*5_6_7_23', 'store','store_td', this.value);recipe_variable_check(this.value);" );
                            ?>
                    ...
                    <tr>
						...
						<td>Remarks</td>
                        <td colspan="5">
                            <input type="text" name="txt_remarks" id="txt_remarks" class="text_boxes" value="" style="width:720px;" />
                        </td>
                   </tr>
             </table>
			</fieldset>
            ...
            <legend>Item Details</legend>
 				<table cellpadding="0" cellspacing="0" border="0" width="100%">
                	<tr>
                    	...
                    	<td width="150">
							<?
                                echo create_drop_down( "cbo_sub_process", 100, $dyeing_sub_process,"", 1, "-- Select Sub Process --", 0, "reset_div(); subprocess_change(this.value);","","92,126,132,133,134,135,138","","",'70');
                            ?>
                        </td>
                        ...
                        <td class="must_entry_caption" width="70">Pick Up(%)</td>
                        <td class="must_entry_caption" >
							 <input type="text" name="txt_liquor_ratio_dtls" id="txt_liquor_ratio_dtls" class="text_boxes_numeric" value=""  onBlur="caculate_tot_liquor()" style="width:45px;" />
						</td>
						<td class="must_entry_caption" width="70">Trough Water(Ltr)</td>
						<td class="must_entry_caption">
							 <input type="text" name="txt_trough_water" id="txt_trough_water" class="text_boxes_numeric" value="50" style="width:50px;"  />
						</td>
						<td class="must_entry_caption" width="70">Total Liquor(ltr)</td>
						<td class="must_entry_caption">
							 <input type="text" name="txt_total_liquor_ratio" id="txt_total_liquor_ratio" class="text_boxes_numeric" value="" style="width:50px;" readonly />
						</td>
            ...
            <table width="910">
            	<tr>
                    <td colspan="4" align="center" class="button_container">
						...
						<input type="button" name="btn_recipe_calc3" id="btn_recipe_calc3" class="formbutton_disabled" value="Print 3" style=" width:100px" onClick="fn_recipe_calc(this.id);" disabled="disabled">
