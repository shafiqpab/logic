...
function search_party()
{
	...
	{
		var page_link = 'requires/left_over_garments_issue_urmi_controller.php?action=search_party&company='+$('#cbo_company_name').val();
...
function issu_purpose(issue)
{
	...
	if (issue==1) { //Buyer
		...
		//load_drop_down( 'requires/left_over_garments_issue_urmi_controller', this.value, 'load_drop_down_buyer', 'cbo_buyer_name' );
...
function openmypage(page_link,title)
{
	...
	else
	{
		...
		var page_link = 'requires/left_over_garments_issue_urmi_controller.php?action=order_popup&company='+$('#cbo_company_name').val()+'&location_name='+$('#cbo_location_name').val()+'&order_type='+$('#cbo_order_type').val()+'&goods_type='+$('#cbo_goods_type').val()+'&garments_nature='+$('#garments_nature').val()+'&store_name='+$('#cbo_store_name').val()+'&party_id='+$('#hidden_party_id').val()+'&country_maintain_variable='+$('#country_maintain_variable').val()+'&leftover_source='+$('#leftover_source').val()+'&wo_com='+$('#cbo_working_company_name').val()+'&wo_loc='+$('#cbo_working_location_name').val()+'&wo_floor='+$('#cbo_working_floor_name').val();
		...
		emailwindow.onclose=function()
		{
			...
			//load_drop_down( 'requires/left_over_garments_issue_urmi_controller', company_id+'_'+location_name, 'load_drop_down_store_name', 'store_name_td' ); 
			...
			var wo_com = $('#cbo_working_company_name').val();
			var wo_loc = $('#cbo_working_location_name').val();
			var wo_floor = $('#cbo_working_floor_name').val();
			
			//get_php_form_data(dtlsId+'**'+mstId, "color_and_size_level_left_over","requires/left_over_garments_issue_urmi_controller" );
			get_php_form_data(po_id+"**"+company_id+"**"+location_name+"**"+country_name+"**"+item_id+"**"+country_maintain+"**"+leftover_source+"**"+variableSettings+"**"+goods_type+"**"+wo_com+"**"+wo_loc+"**"+wo_floor, "color_and_size_level_left_over","requires/left_over_garments_issue_urmi_controller" );
			show_list_view(po_id+"**"+country_maintain+"**"+leftover_source+"**"+goods_type,'show_country_listview','list_view_country','requires/left_over_garments_issue_urmi_controller');
			get_left_over_balance(po_id,country_name,item_id,country_maintain);
...
function put_country_data(po_id, item_id, country_id, issue_qnty)
{
	freeze_window(5);
	...
	get_php_form_data(po_id+'**'+item_id+'**'+country_id+'**'+country_maintain+"**"+leftover_source, "populate_data_from_search_popup", "requires/left_over_garments_issue_urmi_controller" );
	...
	{
		// get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id, "color_and_size_level_left_over", "requires/left_over_garments_issue_urmi_controller" );
		get_php_form_data(po_id+"**"+company_id+"**"+location_name+"**"+country_id+"**"+item_id+'**'+country_maintain+"**"+leftover_source+"**"+variableSettings+"**"+goods_type+"**"+wo_com+"**"+wo_loc+"**"+wo_floor, "color_and_size_level_left_over","requires/left_over_garments_issue_urmi_controller" );
	}
	else
	{
	...
	// show_list_view(po_id+'**'+item_id+'**'+country_id,'show_dtls_listview','list_view_container','requires/left_over_garments_issue_urmi_controller','setFilterGrid(\'tbl_list_search\',-1)');
	reset_form('','','txt_total_issue*txt_remark','','');
	set_button_status(0, permission, 'fnc_left_over_gmts_input',1,0);
	release_freezing();
}
...
function fnc_left_over_gmts_input(operation)
{
	...
	{
 		if ( form_validation('cbo_company_name*cbo_order_type*cbo_goods_type*cbo_issue_purpose*cbo_store_name*cbo_working_company_name*cbo_working_location_name*txt_order_no*txt_total_issue','Company Name*Order Type*Goods Type*Issue Purpose*Order No*Issue Qty')==false )
		{
			return;
		}		
		else
...
function fnc_left_over_gmts_input_Reply_info()
{
 	...
 	if(http.readyState == 4) 
	{
		...
		{ 
			...
			//$("#cbo_floor_name").attr("disabled","disabled");
			$("#txt_remark").attr("disabled","disabled");
			show_list_view(response[1]+'**'+country_maintain+'**'+leftover_source,'show_dtls_listview','list_view_container','requires/left_over_garments_issue_urmi_controller','setFilterGrid(\'tbl_list_search\',-1)');
			show_list_view(po_id+'**'+country_maintain+'**'+leftover_source+'**'+goods_type,'show_country_listview','list_view_country','requires/left_over_garments_issue_urmi_controller');	
...
function system_number_popup()
{
	...
	{*/
		...
		var page_link = 'requires/left_over_garments_issue_urmi_controller.php?action=system_number_popup&company='+$('#cbo_company_name').val()+'&location_name='+$('#cbo_location_name').val()+'&issue_date='+$('#txt_issue_date').val()+'&order_type='+$('#cbo_order_type').val()+'&goods_type='+$('#cbo_goods_type').val()+'&country_maintain_variable='+$('#country_maintain_variable').val()+'&leftover_source='+$('#leftover_source').val();
		...
		emailwindow.onclose=function()
		{
			...
			get_php_form_data(responseDataArr[0], "populate_mst_form_data", "requires/left_over_garments_issue_urmi_controller" )
			show_list_view(responseDataArr[0]+'**'+country_maintain+'**'+leftover_source,'show_dtls_listview','list_view_container','requires/left_over_garments_issue_urmi_controller','setFilterGrid(\'tbl_list_search\',-1)');
			// show_list_view(responseDataArr[0],'show_country_listview','list_view_country','requires/left_over_garments_issue_urmi_controller');
			...
			reset_form('','','hidden_dtls_id*cbo_buyer_name*hidden_receive_dtls_id*txt_order_no*txt_po_id*txt_style_name*cbo_item_name*cbo_country_name*txt_total_issue*txt_remark2*cbo_currency*txt_fob_rate*txt_issue_amount*txt_bdt_amount*cbo_room_no*cbo_rack_no*cbo_shelf_no*cbo_bin_no','','');
...
function check_exchange_rate(curr_id)
{
	...
	var response=return_global_ajax_value( cbo_currercy+"**"+booking_date, 'check_conversion_rate', '', 'requires/left_over_garments_issue_urmi_controller');
...
{
	var currency=$('#cbo_currency_mst').val()*1;
...
function populate_selected_data(dtls_id,mst_id,po_id,country_id,item_id,variableSettings,country_maintain,leftover_source)
{
	//freeze_window(5);
	release_freezing();
	reset_form('','','hidden_dtls_id*cbo_buyer_name*hidden_receive_dtls_id*txt_order_no*txt_po_id*txt_style_name*cbo_item_name*cbo_country_name*txt_total_issue*txt_sale_rate*txt_remark2*cbo_currency*txt_fob_rate*txt_issue_amount*txt_bdt_amount*cbo_room_no*cbo_rack_no*cbo_shelf_no*cbo_bin_no','','');	
	...
	get_php_form_data(dtls_id+"**"+mst_id+"**"+po_id+"**"+country_id+"**"+item_id+"**"+variableSettings+'**'+country_maintain+'**'+leftover_source+'**'+goods_type,"color_and_size_level_left_over_update", "requires/left_over_garments_issue_urmi_controller" );
	get_left_over_balance(po_id,country_id,item_id,country_maintain);
	...
	{ 
		...
		get_php_form_data(po_id+'**'+country_id+'**'+item_id+'**'+dtls_id+'**'+mst_id+'**'+variableSettings+'**'+$('#cbo_order_type').val(), "color_and_size_level_left_over_update", "requires/left_over_garments_issue_urmi_controller" ); 
...
function left_over_gmts_issue_print()
{
	...
	else
	{
		...
		print_report( $('#cbo_company_name').val()+'*'+$('#txt_mst_id').val()+'*'+report_title+'*'+$('#txt_system_no').val()+'*'+$('#cbo_order_type').val(), "left_over_gmts_issue_print", "requires/left_over_garments_issue_urmi_controller");
	}
...
function left_over_gmts_issue_print_2()
{
	...
	print_report( $('#cbo_company_name').val()+'*'+$('#txt_mst_id').val()+'*'+report_title+'*'+$('#txt_system_no').val()+'*'+$('#cbo_order_type').val()+'*'+showRate, 'left_over_gmts_issue_print_2', 'requires/left_over_garments_issue_urmi_controller');
}
...
{	
	...
	// alert(filed_value +'> ('+placeholder_value+'+'+filed_update_value+')');
	
	if(filed_value > (placeholder_value+filed_update_value))
	{
...
function fn_colorlevel_total(index) //for color level
{
	...
	function get_left_over_balance(po_id,country_id,gmts_item,country_maintain)
	{
		...
		var data = 'po_id='+po_id+'&country_id='+country_id+'&gmts_item='+gmts_item+'&country_maintain='+country_maintain+'&order_type='+order_type+'&goods_type='+goods_type+'&wo_com='+wo_com+'&wo_loc='+wo_loc+'&wo_floor='+wo_floor+'&action=get_left_over_balance';	
		// var data = {"po_id":po_id,"gmts_item":gmts_item,"country_id":country_id,"action":get_left_over_balance};	
		// alert(data);
		$.ajax({
			type:'POST',
			url: 'requires/left_over_garments_issue_urmi_controller.php',
			data: data,
			cache: false,
  				...
  				<fieldset>
                    <table width="100%" border="0">
                        ...
                        <tr>
                            ...
                            <td width="180">                                
                            <? 
                            echo create_drop_down( "cbo_company_name", 170, "select company_name,id from lib_company comp where is_deleted=0  and status_active=1 $company_cond  order by company_name",'id,company_name', 1, '--- Select Company ---', 0, "load_drop_down( 'requires/left_over_garments_issue_urmi_controller', this.value, 'load_drop_down_store_name', 'store_name_td' );load_drop_down( 'requires/left_over_garments_issue_urmi_controller', this.value, 'load_drop_down_location', 'location' );get_php_form_data(this.value,'load_variable_settings','requires/left_over_garments_issue_urmi_controller');" );
                            ?>
