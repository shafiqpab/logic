...
<script>
		...
		if(operation==0 || operation==1 || operation==2)
		{
			...
			if(delivery_basis==3)
			{
				...
				$("#tbl_details").find('tbody tr').each(function()
				{
					...
					var cboHoldBundle=$(this).find('select[name="cboHoldBundle[]"]').val();
	...
	function openmypage_bundle(page_link,title)
	{
		...
		else
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link+'&bundleNo='+bundleNo+'&line_id='+line_id, title, 'width=1010px,height=370px,center=1,resize=0,scrolling=0','')
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0];
	...
	function fnc_duplicate_bundle(bundle_no) // 18/05/2022
	{
	...
	function fnc_duplicate_bundle__bk(bundle_no)
	{
		freeze_window(5);
		var row_num=$('#txt_tot_row').val();
		var rcv_date = $('#txt_issue_date').val();
		var challan_duplicate=return_ajax_request_value( bundle_no+"__"+$('#cbo_company_name').val()+"__"+$('#cbo_line_no').val()+"__"+row_num+"__"+rcv_date,"challan_duplicate_check", "requires/bundle_wise_sewing_output_controller");
		...
		else
		{
			...
			if( (ex_challan_duplicate[3]*1)>1 &&  $('#cbo_line_no').val()==0 ) //(ex_challan_duplicate[3]*1)>1 
			{
				var page_link='requires/bundle_wise_sewing_output_controller.php?action=bundle_popup_line_select&data='+ex_challan_duplicate[5]+'&company_id='+$('#cbo_company_name').val();
	...
	function openmypage_bundle_rescan(page_link,title)
	{
		...
		else
		{
			var tot_row=$('#tbl_details tbody tr').length; 
			var line_id_val=$('#cbo_line_no').val();
			let is_defect=$('#is_defect').is(':checked');
			var line_id=0;	
			if( (tot_row*1) <1 && (line_id_val*1)==0) line_id=0; else line_id=$('#cbo_line_no').val();
			var bundleNo='';
			$("#tbl_details").find('tbody tr').each(function()
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link+'&bundleNo='+bundleNo+'&line_id='+line_id+'&is_defect='+is_defect, title, 'width=990px,height=370px,center=1,resize=0,scrolling=0','')
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0];
				var hidden_bundle_nos=this.contentDoc.getElementById("hidden_bundle_nos").value;//po id
				...
				if(is_defect==false)
				{
					is_defect=this.contentDoc.getElementById("is_defect").checked;//po id
				}
				// alert(is_defect);
		...
		else
		{
			...
			if( (ex_challan_duplicate[3]*1)>1 && $('#cbo_line_no').val()==0)
			{
				...
				emailwindow.onclose=function()
				{
					...
					if( (tot_row*1) <2)
						get_php_form_data(bundle_no+"__"+ex_challan_duplicate[4]+"__"+$('#txt_issue_date').val(),'load_mst_data','requires/bundle_wise_sewing_output_controller');
					create_row_rescan(bundle_no,hidden_bundle_nos,is_defect);
			...
			else
			{
				var tot_row=$('#tbl_details tbody tr').length; 
					...
					if( (tot_row*1) <2)
						get_php_form_data(bundle_no+"__"+ex_challan_duplicate[4]+"__"+$('#txt_issue_date').val(),'load_mst_data','requires/bundle_wise_sewing_output_controller');
					create_row_rescan(bundle_no,$('#cbo_line_no').val(),is_defect);
	...
	function change_mode(source_id)
	{
		if(source_id==1)
		{
			get_php_form_data($('#cbo_company_name').val(),'load_variable_settings','requires/bundle_wise_sewing_output_controller'); 
			// get_php_form_data($('#cbo_company_name').val(),'load_variable_settings_for_working_company','requires/bundle_wise_sewing_output_controller'); 
			load_html();
		...
		{
			get_php_form_data($('#cbo_company_name').val(),'load_variable_settings','requires/bundle_wise_sewing_output_controller');
			// get_php_form_data($('#cbo_emb_company').val(),'load_variable_settings_for_working_company','requires/bundle_wise_sewing_output_controller'); 
			load_html();
	...
	function show_cost_details()
	{
		...
		var page_link='requires/bundle_wise_sewing_output_controller.php?action=show_cost_details&sys_id='+system_id;
	...
	function fnSetHoldReason()
	{
		...
		$("#tbl_details").find('tbody tr').each(function()
		{
			if($('#cboHoldBundle_'+j).val() > 0 && reason_id ==0 )
			{
				reason_id = $('#cboHoldBundle_'+j).val();
			}
			j++;
		})
		// ============== set reason ============		
		$("#tbl_details").find('tbody tr').each(function()
		{
			$('#cboHoldBundle_'+m).val(reason_id);
			m++;
		})
            ...
            <fieldset style="width:930px; margin:0 auto;">
                <table width="100%">
                    ...
                    <tr>
                        ...
                        <td>
                            <? 
                            	echo create_drop_down( "cbo_company_name", 180, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "" );
                            ?>
                          ...
                          <td>
                              <? 
                              	echo create_drop_down( "cbo_source", 180, $knitting_source,"", 1, "-- Select Source --", $selected, "load_drop_down( 'requires/bundle_wise_sewing_output_controller', this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_wo_company', 'emb_company_td' );change_mode(this.value);", 0, '1,3' );
                              ?>
                          </td>
                ...
                <div id="bundle_list_view">
                	...
                	<table cellpadding="0" width="1335" cellspacing="0" border="1" class="rpt_table" rules="all">
                        <thead>
                            <th width="30">SL</th>
                            ...
                            <th width="50">Hold <input type="checkbox" value="1" name="hold_bundle" id="hold_bundle" onClick="fnSetHoldReason()"></th>
                            <th></th>
                        </thead>
              		</table>
                    <div style="width:1335px; max-height:250px; overflow-y:auto" align="left">    
                    	<table cellpadding="0" width="1325" cellspacing="0" border="1" class="rpt_table" rules="all" id="tbl_details">      
                            <tbody>
                            </tbody>
                        </table>
                	</div>
