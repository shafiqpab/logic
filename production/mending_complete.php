...
function openmypage(page_link,title)
{
	...
	//			load_drop_down( 'requires/trimming_complete_controller',company_id, 'load_drop_down_location', 'location_td' );
			...
			console.log('reset');
			get_php_form_data(po_id+'**'+item_id+'**'+country_id, "populate_data_from_search_popup", "requires/mending_complete_controller" );
			...
			{
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+job_num+'**'+variableSettingsReject, "color_and_size_level", "requires/mending_complete_controller" );
			}
			else
			{
 	...
 	if(http.readyState == 4)
	{
		...
		else if(reponse[0]==0)//insert
		{
			...
			reset_form('','','txt_trim_qty*txt_reject_qty*txt_alter_qnty*txt_spot_qnty*hidden_break_down_html*hidden_colorSizeID*txt_remark*txt_mst_id','','','txt_trim_date*txt_reporting_hour');
 			get_php_form_data(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id, "populate_data_from_search_popup", "requires/mending_complete_controller" );

			if(variableSettings!=1) {
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject, "color_and_size_level", "requires/mending_complete_controller" );
			}
			else
			{
				$("#txt_trim_qty").removeAttr("readonly");
...
function put_country_data(po_id, item_id, country_id, po_qnty, plan_qnty)
{
	...
	if(variableSettings!=1)
	{
		get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+job_num, "color_and_size_level","requires/mending_complete_controller");
	}
	else
	{
		$("#txt_trim_qty").removeAttr("readonly");
