...
<?php   
if($_SESSION['logic_erp']['data_arr'][722]){
	echo "var field_level_data= " . json_encode($_SESSION['logic_erp']['data_arr'][722]). ";\n";
}
?>
		...
		emailwindow.onclose=function()
		{
			...
			if (po_id!="")
			{
				...
				if(variableSettings!=1){
					get_php_form_data(po_id+'**'+item_id+'**'+variableSettings+'**'+styleOrOrderWisw+'**'+country_id+'**'+variableSettingsReject+'**'+txt_job_no+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'**'+ship_date, "color_and_size_level", "requires/finishing_entry_controller");
				...
				{
					get_php_form_data(po_id+'**'+item_id+'**'+country_id+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'**'+ship_date, "populate_data_from_search_popup", "requires/finishing_entry_controller" );
				...
				var data = po_id+'**'+item_id+'**'+country_id+'**'+txt_job_no+'**'+pack_type+'**'+ship_date;
				...
				else if(preceding_process==116)
				{
					$('#prod_qty').text('Finishing Input');
				}
				else if(preceding_process==250)
				{
					$('#prod_qty').text('Poly Entry');
				}
				...
				//  show_list_view(po_id+'**'+item_id+'**'+country_id+'**'+txt_job_no+'**'+pack_type,'show_all_listview','list_view_container','requires/finishing_entry_controller','setFilterGrid(\'tbl_list_search\',-1)');
		...
		if ( form_validation('cbo_company_name*txt_order_no*cbo_finish_company*txt_finishing_date','Company Name*Order No*Finishing Company*Finishing Date')==false )
		{
		...
		else
		 {
			...
			if(variableSettingsReject != sewing_production_variable){
				alert("Please check your variable settings");
				return;
			}
...
function fnc_finishing_entry_Reply_info()
{
 	if(http.readyState == 4) 
	{
		...
		var ship_date=$("#hidden_ship_date").val();
		...
		{
			gmts_fin  = reponse[1]*1;
			buyer_ins = reponse[2]*1;
			transfer_out = reponse[3]*1;
			let msg   = ''; 
			if (gmts_fin) 
			{
				msg += 'Garments Delevary,'; 
			}
			if (buyer_ins) 
			{
				msg += 'Buyer Inspection,';
			} 
			if (transfer_out) 
			{
				msg += 'Transfer Out'; 
		...
		else 
		{
			...
			// show_list_view(po_id+'**'+$("#cbo_item_name").val()+'**'+country_id+'**'+txt_job_no+'**'+pack_type,'show_all_listview','list_view_container','requires/finishing_entry_controller','setFilterGrid(\'tbl_list_search\',-1)');
			// show_list_view(po_id,'show_cost_listview','printing_cost_list_view','requires/finishing_entry_controller','');
			reset_form('','','txt_finishing_qty*txt_carton_qty*txt_alter_qnty*txt_spot_qnty*txt_reject_qnty*txt_challan*txt_remark*txt_finish_input_qty*txt_cumul_finish_qty*txt_yet_to_finish*hidden_break_down_html*txt_mst_id*save_data*defect_type_id*all_defect_id*save_dataSpot*allSpot_defect_id*defectSpot_type_id*save_dataRej*defectRej_type_id*allRej_defect_id*hidden_actual_po*txt_actual_po','','','txt_finishing_date*txt_reporting_hour');
			...
			{
				get_php_form_data(po_id+'**'+$('#cbo_item_name').val()+'**'+country_id+'**'+$('#hidden_preceding_process').val()+'**'+pack_type+'**'+ship_date, "populate_data_from_search_popup", "requires/finishing_entry_controller" );
				$("#txt_finishing_qty").removeAttr("readonly");
			}
...
function put_country_data(po_id, item_id, country_id, po_qnty, plan_qnty,pack_type,ship_date)
{
	...
	show_list_view(po_id+'**'+item_id+'**'+country_id+'**'+txt_job_no,'show_dtls_listview','list_view_container','requires/finishing_entry_controller','setFilterGrid(\'tbl_list_search\',-1)');
	// show_list_view(po_id+'**'+item_id+'**'+country_id+'**'+txt_job_no,'show_all_listview','list_view_container','requires/finishing_entry_controller','');
	set_button_status(0, permission, 'fnc_finishing_entry',1,0);
	release_freezing();
}
...
function openmypage_woNo()
	{
		...
		if (form_validation('cbo_company_name*cbo_source*txt_order_no','Company*Source*Service Company*Order No')==false)
		{
	  	...
	  	{			
			...
			var page_link='requires/finishing_entry_controller.php?company_id='+cbo_company_id+'&cbo_service_source='+cbo_service_source+'&po_order_id='+po_order_id+'&po_order_no='+po_order_no+'&action=service_booking_popup';
	...
	function actual_po_popup() 
	{
		...
		var item_id = $("#cbo_item_name").val();
		var act_po_id = $("#hidden_actual_po").val();
		var page_link = "requires/finishing_entry_controller.php?action=actual_po_action&po_id=" + po_id + "&item_id=" + item_id+ "&act_po_id=" + act_po_id;
		...
		{
			emailwindow = dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=930px,height=430px,center=1,resize=0,scrolling=0', '')
			emailwindow.onclose = function() 
			{
				var theform = this.contentDoc.forms[0];
				...
				if (data_string != "") 
				{
					...
					$("#txt_finishing_qty").val(po_qty);
					$("#txt_reject_qnty").val(po_rej_qty);
				} 
				else 
				{
					$("#txt_actual_po").val('');
	...
	function load_finish_company(source_id)
	{
		if($('#cbo_finish_company option').length==2)
		{
			if($('#cbo_finish_company option:first').val()==0)
			{
				var cbo_finish_company_id=$('#cbo_finish_company option:last').val();
				$('#cbo_finish_company').val(cbo_finish_company_id);
				if(source_id==1)
				{
					load_drop_down( 'requires/finishing_entry_controller', cbo_finish_company_id, 'load_drop_down_location', 'location_td' );fnc_company_check(source_id);
				}
			}
		}
	}	
	...
	function show_cost_details()
	{
		...
		var page_link='requires/finishing_entry_controller.php?action=show_cost_details&sys_id='+system_id;
...
<div style="width:100%;">
	<? echo load_freeze_divs ("../",$permission);  ?>
 	<div style="width:900px; float:left" align="center">
 		<!-- <div style="text-align: center;color: red;font-weight: bold;font-size: 20px;">Do not take update. Please be patient.</div> -->
            ...
            <form name="finishingentry_1" id="finishingentry_1" autocomplete="off" >
                ...
                <table width="100%" border="0">
                    ...
                    <tr>
                        ...
                        <td><? echo create_drop_down( "cbo_company_name", 170, "select id,company_name from lib_company where status_active =1 and is_deleted=0 and core_business not in(3) $company_credential_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "get_php_form_data(this.value,'load_variable_settings','requires/finishing_entry_controller');setFieldLevelAccess(this.value);" ); ?>	 
                        ...
                        <td><? echo create_drop_down( "cbo_source", 170, $knitting_source,"", 1, "-- Select Source --", $selected, "fnc_company_check(this.value);load_drop_down( 'requires/finishing_entry_controller', this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_source', 'finishing_td' );dynamic_must_entry_caption(this.value);load_finish_company(this.value);", 0, '1,3' ); ?></td>
                ...
                <tr>
                 ...
                 <tr>    
                     ...
                     <tr>  
                        ...
                        <td>Pack Type</td>
                        <td><input name="txt_pack_type" id="txt_pack_type" class="text_boxes" style="width:155px " disabled readonly>
						<input type="hidden" name="hidden_ship_date" id="hidden_ship_date" class="text_boxes" style="width:155px " disabled readonly></td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>WO NO</td>
                        <td>
                            <input type="text" name="txt_wo_no" id="txt_wo_no" class="text_boxes" style="width:155px;" placeholder="Browse/Write/scan" onDblClick="openmypage_woNo();" />
                ...
                <table cellpadding="0" cellspacing="1" width="100%">
                    <tr>
                        ...
                        <td width="25%" valign="top">
                            ...
                            <table  cellpadding="0" cellspacing="2" width="100%" >
                                <tr>
                                    <td width="120" id="prod_qty"> Iron Qty</td> 
                ...
                <div style="width:930px; margin-top:5px;max-height:300px;overflow-y:auto;"  id="list_view_container" align="center"></div>
