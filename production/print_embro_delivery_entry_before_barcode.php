		...
		emailwindow.onclose=function()
		{
			...
			{ 
				...
				if(delivery_basis==3)
				{
					...
					var response_data=return_global_ajax_value(trim(bundle_nos)+"**0**"+mst_id+"**"+$('#cbo_company_name').val(), 'populate_bundle_data_update', '', 'requires/print_embro_delivery_entry_controller');
...
function fnc_issue_print_embroidery_entry(operation)
{
	...
	{
		var report_title=$( "div.form_caption" ).html();
		generate_report_file($('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#delivery_basis').val()+'*'+report_title, 'emblishment_issue_print_4', 'requires/print_embro_delivery_entry_controller');
		return;
	}
		...
		if(delivery_basis==3)
		{
			...
			$("#tbl_details").find('tbody tr').each(function()
			{
				...
				try 
				{
					...
					dataString+='&bundleNo_' + j + '=' + bundleNo + '&orderId_' + j + '=' + orderId + '&gmtsitemId_' + j + '=' + gmtsitemId + '&countryId_' + j + '=' + countryId + '&colorId_' + j + '=' + colorId + '&sizeId_' + j + '=' + sizeId + '&colorSizeId_' + j + '=' + colorSizeId + '&qty_' + j + '=' + qty + '&dtlsId_' + j + '=' + dtlsId + '&cutNo_' + j + '=' + cutNo+ '&isRescan_' + j + '=' + isrescan;
				}
				catch(e) 
				{
            ...
            <fieldset>
                <table width="100%">
                    ...
                    <tr>
                        ...
                        <td>
                            <? 
								echo create_drop_down( "cbo_company_name", 180, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "get_php_form_data(this.value,'load_variable_settings','requires/print_embro_delivery_entry_controller'); load_drop_down( 'requires/print_embro_delivery_entry_controller', this.value, 'load_drop_down_body_part', 'body_part_td' ); working_com_fnc(); load_html();" );//get_php_form_data(this.value,'load_variable_settings_for_working_company','requires/print_embro_delivery_entry_controller');
                            ?>
