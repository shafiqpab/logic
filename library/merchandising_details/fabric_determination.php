...
<script type="text/javascript">
	...
	function fnc_fabric_count_determination( operation )
	{
		...
		for (var i=1; i<=row_num; i++)
		{
			...
			{
				data_all=data_all+get_submitted_data_string('cbo_fabric_nature*txttype*txtconstruction*txtweight*cboweighttype*txtdesign*txtfabricref*txtrdno*cbocolortype*cbo_status*update_mst_id*txt_full_width*txt_shrinkage_l*txt_shrinkage_w*txt_cutable_width*cbocompone_'+i+'*percentone_'+i+'*cbocountcotton_'+i+'*cbotypecotton_'+i+'*updateid_'+i,"../../");
			}
		}
		//alert(data_all)
		var data="action=save_update_delete&operation="+operation+'&total_row='+row_num+data_all;
		
		http.open("POST","requires/fabric_determination_controller.php",true);
	...
	function fnc_fabric_count_determination_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			else
			{
				//alert(reponse[0]);
				show_msg(trim(reponse[0]));
				show_list_view(reponse[1],'search_list_view','yarn_count_container','../merchandising_details/requires/fabric_determination_controller','setFilterGrid("list_view",-1)');
	...
	{
		var page_link="requires/fabric_determination_controller.php?action=composition_popup&inc="+inc;
