...
<script>
	...
	function update_year(val)
	{
		...
		//alert(data);
		var list_view_capacity = return_global_ajax_value( data, 'load_php_dtls_form', '', 'requires/yearly_linking_capacity_calculation_sweater_controller');
        ...
        if(particularId == 1 || particularId == 2 || particularId == 3 || particularId == 5 || particularId == 7 || particularId == 9){
            machinesArray.map((mid)=>{
                ...
                for(m=1;m<=12;m++){
                    ...
                    var multiplyQty_12 = multiplyQty_11 / (selectVal_1 ? selectVal_1 : 1 );
    ...
    function fnc_capacity_calculation( operation )
	{
		...
		http.open("POST","requires/yearly_linking_capacity_calculation_sweater_controller.php",true);
        ...
        <table cellpadding="0" cellspacing="2" width="100%">
            <tr>
                ...
                <td align="left">
                    ...
                    <?
                        echo create_drop_down( "cbo_company_id",160,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "--Select Company--", $selected, "load_drop_down( 'requires/yearly_linking_capacity_calculation_sweater_controller', this.value, 'load_drop_down_location', 'location_td');load_drop_down( 'requires/yearly_linking_capacity_calculation_sweater_controller', this.value, 'load_drop_down_machine_gauge', 'machine_gauge_td');make_multi_select();","","","","","",2); //location_select();
                    ?>
                </td>
                <td >Location </td>
            ...
            <tr>
                ...
                <td width="130" class="must_entry_caption">Machine Gauge/Brand</td>
                <td width="100" > <!-- id="machine_gauge_td" -->
                    <?php 
                        $machinesArray = array(1=>"Chunky Gauge- Up to 7 GG",2=>"Fine Gauge- Above 7 GG");
                        echo create_drop_down( "cbo_machine_gauge", 160, $machinesArray,"", 1, "-- Select gauge--", $selected, "","","" );
                    ?>
                </td>
