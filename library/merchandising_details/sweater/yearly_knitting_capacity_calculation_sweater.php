...
<script>
	...
	function update_year(val)
	{
		...
		//alert(data);
		var list_view_capacity = return_global_ajax_value( data, 'load_php_dtls_form', '', 'requires/yearly_knitting_capacity_calculation_sweater_controller');
    ...
    function multiplyData(idStr, machinesIdStr){
        ...
        if(particularId == 1 || particularId == 2 || particularId == 3 || particularId == 5 || particularId == 7 || particularId == 9){
            machinesArray.map((mid)=>{
                ...
                var particulaTotal_8 = 0; var particulaTotal_10 = 0;
    ...
    function fnc_capacity_calculation( operation )
	{
		...
		http.open("POST","requires/yearly_knitting_capacity_calculation_sweater_controller.php",true);
        ...
        <table cellpadding="0" cellspacing="2" width="100%">
            <tr>
                ...
                <td align="left">
                    ...
                    <?
                        echo create_drop_down( "cbo_company_id",160,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "--Select Company--", $selected, "load_drop_down( 'requires/yearly_knitting_capacity_calculation_sweater_controller', this.value, 'load_drop_down_location', 'location_td');load_drop_down( 'requires/yearly_knitting_capacity_calculation_sweater_controller', this.value, 'load_drop_down_machine_gauge', 'machine_gauge_td');make_multi_select();","","","","","",2); //location_select();
                    ?>
                </td>
                <td >Location </td>
            ...
            <tr>
                ...
                <td width="100" id="machine_gauge_td">
                    <?php 
                        echo create_drop_down( "cbo_machine_gauge", 160, $blank_array,"", 1, "-- Select gauge--", $selected, "","","" );
                    ?>
                </td>
