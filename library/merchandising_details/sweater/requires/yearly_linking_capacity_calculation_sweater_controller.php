...
if ($action=="load_php_dtls_form")
{
    ...
    $capacitySQL = "SELECT A.id, A.COMPANY_ID, A.LOCATION_ID, A.BASIC_SMV, A.EFFI_PERCENT, A.SMOOTHING_PERCENT, A.MACHINE_GAUGE, A.YEAR,
                    ...
                    WHERE A.COMPANY_ID=$company_id AND A.ENTRY_FORM=737 AND A.YEAR=$year AND A.STATUS_ACTIVE=1 AND A.IS_DELETED=0 AND B.STATUS_ACTIVE=1 AND B.IS_DELETED=0";
    //echo $capacitySQL;
    ...
    ?>
        <table cellpadding="0" border="1" cellspacing="0" width="1500" class="rpt_table" rules="all" id="myTable">
            ...
            <tbody>
                <?php 
                    // Machine array is static for linking calculation....
                    ...
                    $attribute_arr = array(1=>"Machine QTY", 2=>"Working Day", 3=>"Working Hours", 4=>"Working Min", 5=>"Smoothing (%)", 6=>"Available Min", 7=>"Efficency (%)", 8=>"Capacity Min", 9=>"Avg SMV", 10=>"Capacity in Pcs", 11=> "Prod/Day", 12=> "Prod/MC");
                    ...
                    foreach($machinesArray as $machineId => $group)
                    {
                        ...
                        foreach($attribute_arr as $particularId => $attribute)
                        {
                            ...
                            ?>
                            <tr style="<? echo $i==12 || $i%12==0 ? 'border-bottom: 3px solid green' : '';?>">
                                <? if($particularId == 1) { ?>
...
if($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here==================================================
	{
		...
		$data_array_mst="(".$mst_id.",".$company_id.",".$location_id.",".$basic_smv.",".$effi_percent.",".$smoothing_percent.",'".$machine_gauge."',".$cbo_year.", 737)";
