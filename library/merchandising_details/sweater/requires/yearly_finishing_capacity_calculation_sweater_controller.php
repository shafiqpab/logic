...
if ($action=="load_php_dtls_form")
{
    ...
    $capacitySQL = "SELECT A.ID, A.COMPANY_ID, A.LOCATION_ID, A.BASIC_SMV, A.EFFI_PERCENT, A.SMOOTHING_PERCENT, A.MACHINE_GAUGE, A.YEAR,
                    ...
                    WHERE A.COMPANY_ID=$company_id AND A.ENTRY_FORM=738 AND A.YEAR=$year AND A.STATUS_ACTIVE=1 AND A.IS_DELETED=0 AND B.STATUS_ACTIVE=1 AND B.IS_DELETED=0";
    //echo $checkSQL;
...
if($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here==================================================
	{
		...
		$data_array_mst="(".$mst_id.",".$company_id.",".$location_id.",".$basic_smv.",".$effi_percent.",".$smoothing_percent.",'".$machine_gauge."',".$cbo_year.", 738)";
