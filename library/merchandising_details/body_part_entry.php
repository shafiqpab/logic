...
<script>
    ...
    function fnc_body_part_entry(operation)
    {
		...
		var response=return_global_ajax_value( update_id, 'is_used_body_part', '', 'requires/body_part_entry_controller');
		if(response == 1){
        ...
        if (form_validation('body_part_full_name*body_part_short_name*body_part_type*txt_entry_page*cbo_status', 'Body Part Full Name*Body Part Short Name*Body Part Type*Tag Entry Page*Status') == false)
        {
            return;
        }
        else
    ...
    function fnc_body_part_entry_reponse()
    {
		...
		{
			$('#body_part_full_name').removeAttr('disabled','disabled');
			$('#body_part_short_name').removeAttr('disabled','disabled');
			$('#body_part_type').removeAttr('disabled','disabled');
