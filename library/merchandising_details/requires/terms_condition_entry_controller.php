...
if ($action == "load_php_data_to_form") {
	...
	$nameArray = sql_select("select terms,is_default,tag_buyer, page_id,id,terms_prefix from  lib_terms_condition where id='$data'");
	foreach ($nameArray as $inf) {
		...
		if ($inf[csf("is_default")] == 1) {
			...
			foreach ($tag_buyer_arr as $val) {
				if ($buyer_name == "") $buyer_name = $buyer_arr[$val];
				else $buyer_name .= "," . $buyer_arr[$val];
			}
		...
		foreach ($tag_page_arr as $val) {
			if ($page_name == "") $page_name = $entry_form[$val];
			else $page_name .= "," . $entry_form[$val];
		}
...
if ($action == "save_update_delete") {
	...
	if ($operation == 0)  // Insert Here
	{
		...
		$field_array = "id,terms,page_id,is_default,tag_buyer,terms_prefix";
		...
		for ($i = 0; $i < count($tag_page); $i++) {
			...
			$data_array .= "$add_comma(" . $id . "," . trim($txt_terms_condition) . "," . $tag_page[$i] . "," . $cbo_status . "," . $txt_tag_buyer_id . "," . $txt_terms_prefix . ")";
			$id++;
		}
		$id_lib_terms_tag_buyer = return_next_id("id", "lib_terms_tag_buyer", 1);
		$data_array_buyer = "";
		$tag_buyer = explode(',', str_replace("'", "", $txt_tag_buyer_id));
		for ($i = 0; $i < count($tag_buyer); $i++) {
			if ($i == 0) $add_comma = "";
			else $add_comma = ",";
			$data_array_buyer .= "$add_comma(" . $id_lib_terms_tag_buyer . "," . $id . "," . $tag_buyer[$i] . ")";
			$id_lib_terms_tag_buyer++;
		}
		$field_array_buyer = "id, page_id, buyer_id";
		$rID = sql_insert("lib_terms_condition", $field_array, $data_array, 1);
		$rID_1 = sql_insert("lib_terms_tag_buyer", $field_array_buyer, $data_array_buyer, 1);
	...
	} else if ($operation == 1)   // Update Here
	{
		...
		for ($i = 0; $i < count($tag_page); $i++) {
			if (in_array($tag_page[$i], $page_id_arr)) {
				$rID1 = sql_multirow_update("lib_terms_condition", "terms*terms_prefix*tag_buyer", $txt_terms_condition . "*" . $txt_terms_prefix . "*" . $txt_tag_buyer_id, "id", $update_id, 0);
			} else {
				if ($i == 0) $add_comma = "";
				else $add_comma = ",";
				$data_array .= "$add_comma(" . $id . "," . trim($txt_terms_condition) . "," . $tag_page[$i] . "," . $cbo_status . "," . $txt_tag_buyer_id . "," . $txt_terms_prefix . ")";
			...
			for ($i = 0; $i < count($tag_buyer); $i++) {
				...
				$data_array_buyer .= "$add_comma(" . $id_lib_terms_tag_buyer . "," . $update_id . "," . $tag_buyer[$i] . ")";
				$id_lib_terms_tag_buyer++;
			}
		}
		$field_array_buyer = "id,page_id,buyer_id";
	...
	} else if ($operation == 2)   // Delete Here
	{
		...
		if (count($booking_no_result) > 0) {
			...
			echo "15**Data Found " . $booking_no_all;
			die;
		} else {
						...
						<table cellspacing="0" cellpadding="0" border="1" rules="all" width="332" class="rpt_table" id="tbl_list_search">
							...
							foreach ($entry_form as $id => $name) {
								...
								if (in_array($id, $hidden_page_id)) {
									if ($buyer_row_id == "") $buyer_row_id = $i;
									else $buyer_row_id .= "," . $i;
								}
