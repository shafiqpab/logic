...
{
		$arr=array (1=>$entry_form,2=>$is_deleted);
		echo  create_list_view ( "list_view", "Terms,Page,Status", "250,150,50","500","220",0, "select    terms,is_default, page_id,id from lib_terms_condition where page_id>0 order by id asc", "get_php_form_data", "id", "'load_php_data_to_form'", 1, "0,page_id,is_default", $arr , "terms,page_id,is_default", "../merchandising_details/requires/terms_condition_entry_controller", 'setFilterGrid("list_view",-1,tableFilters);' ) ;
...
if ($action=="load_php_data_to_form")
{
	...
	$nameArray=sql_select( "select terms,is_default, page_id,id from  lib_terms_condition where id='$data'" );
...
{  
	...
	if ($operation==0)  // Insert Here
	{
		...
		$field_array="id,terms,page_id,is_default";
		...
		for($i=0; $i<count($tag_page); $i++)
		{
			...
			$data_array.="$add_comma(".$id.",".trim($txt_terms_condition).",".$tag_page[$i].",".$cbo_status.")";
			$id++;
		}
	...
	else if ($operation==1)   // Update Here
	{
		...
		if($db_type==0)
		{
			...
			for($i=0; $i<count($tag_page); $i++)
			{
				...
				{
					$rID1=sql_multirow_update("lib_terms_condition","terms",$txt_terms_condition,"id",$update_id,0);
				}
				else
				{
					if($i==0) $add_comma=""; else $add_comma=",";
