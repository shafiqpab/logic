...
{//"select id,location_name from lib_location where company_id='$data' and status_active =1 and is_deleted=0 order by location_name" id,location_name
	 echo create_drop_down( "cbo_location_id", 160, $empty,"", 1, "--Select Location--", $selected, "","1","","","","",3 );		 
}
...
if($action=="return_mstid")
{
  ...
  $mst_id="";
 $sql_mst=sql_select("SELECT distinct a.id FROM lib_min_lead_time_mst a, lib_min_lead_time_dtls b WHERE a.id=b.mst_id AND a.company_id=$data[0]  AND a.year_id=$data[1] AND a.month_id=$data[2] AND a.location_id=$data[3]");
...
if($action=="show_dtls_list_view")
{
	...
	$cbo_month=str_replace("'","",$data[2]);
	 /*$sql_result=sql_select("SELECT mid, bid,min_allocation,buyer_name,status_active,updid FROM (
SELECT a.id as mid,  b.buyer_id AS bid,b.min_allocation,c.buyer_name,c.status_active, b.id as updid 
FROM lib_min_lead_time_mst a, lib_min_lead_time_dtls b, lib_buyer c  
...
WHERE FIND_IN_SET($data[0],tag_company) AND FIND_IN_SET(1||3||21||90,party_type) ) AS tabl GROUP BY bid");*/
	...
	if($data[1] !="")
	 {
	 $allo_percent_arr=return_library_array( "SELECT  b.buyer_id,b.min_allocation FROM lib_min_lead_time_mst a, lib_min_lead_time_dtls b WHERE a.id=b.mst_id AND a.id=$data[1]",'buyer_id','min_allocation');
	$update_id_arr=return_library_array( "SELECT  b.buyer_id,b.id FROM lib_min_lead_time_mst a, lib_min_lead_time_dtls b WHERE a.id=b.mst_id AND a.id=$data[1]",'buyer_id','id');
	 }
	...
	?>
	 <table cellpadding="0" cellspacing="0" border="1" width="450" id="tbl_allocation" class="rpt_table" rules="all">
				<thead>
					...
					<th align="center">Min Lead Time (Day) <input type="checkbox" id="copy_check" accesskey="c" /> Copy</th>
				</thead>
				<tbody>
                <?
				...
				?>
					...
					<tr bgcolor="<? echo $bgcolor; ?>">
                        ...
                        <? if($row[csf("status_active")]==2){?><td  align="center"><input type="text" id="txt_min_mint_allocation_<? echo $i; ?>" name="txt_min_mint_allocation_<? echo $i; ?>" class="text_boxes_numeric" value="<? echo $allo_percent_arr[$row[csf("id")]]; ?>" style="width:100px" readonly="readonly" /> </td> <? } else { ?>
						<td  align="center"><input type="text" id="txt_min_mint_allocation_<? echo $i; ?>" name="txt_min_mint_allocation_<? echo $i; ?>" onchange="total_value(<? echo $i; ?>);copy_value_all(<? echo $i; ?>,this.value);" class="text_boxes_numeric" value="<? echo $allo_percent_arr[$row[csf("id")]]; ?>" style="width:100px" /> </td><? } ?>
                        <!--<td  align="center"><input type="text" id="txt_min_mint_allocation_<? //echo $i; ?>" name="txt_min_mint_allocation_<? //echo $i; ?>" onchange="total_value(<? //echo $i; ?>)" class="text_boxes_numeric" value="<? //echo $allo_percent_arr[$row[csf("id")]]; ?>" style="width:100px" /> </td><? //} ?>-->
					</tr>
                    <? }  ?>
                    </tbody>
                    <tfoot>
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		{
			$mst_id= return_next_id("id","lib_min_lead_time_mst",1);
		...
		//	$rID=sql_insert("lib_min_lead_time_mst",$field_array_mst,$data_array_mst,0);
		...
		else
		{
			...
			//$rID=sql_update("lib_min_lead_time_mst",$field_array_mst,$data_array_mst,"id",$update_id,0);
		}
		$dtls_id=return_next_id( "id", "lib_min_lead_time_dtls",1);
		$field_array_dtls="id,mst_id,buyer_id,min_allocation,inserted_by,insert_date,status_active,is_deleted";
		
		for($i=1; $i<=$tot_row_buyer; $i++)
		{
			$txt_buyer= "buyer_id_".$i;
			$txt_min_mint_allocation_parcentage="txt_min_mint_allocation_".$i;
			...
			$data_array_dtls.="(".$dtls_id.",".$mst_id.",".$$txt_buyer.",".$$txt_min_mint_allocation_parcentage.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			$dtls_id=$dtls_id+1;
			}
		}
		...
		$rID1=sql_insert("lib_min_lead_time_dtls",$field_array_dtls,$data_array_dtls,1);
	...
	else if ($operation==1)   // Update Here=============================================================================
	{
		...
		$field_array_dtls="min_allocation*updated_by*update_date";
		$dtls_id=return_next_id( "id", "lib_min_lead_time_dtls", 1 );
		$mst_id=str_replace("'",'',$update_id);
		$field_array_dtls_in="id,mst_id,buyer_id,min_allocation,inserted_by,insert_date,status_active,is_deleted";
		$coma=0;
		for($i=1; $i<=$tot_row_buyer; $i++)
		{
			$txt_buyer= "buyer_id_".$i;
			$txt_min_allocation="txt_min_mint_allocation_".$i;
			$update_id_dtls="update_id_dtls_".$i;
			
			if(str_replace("'",'',$$update_id_dtls)!="")
			{
				$id_arr[]=str_replace("'",'',$$update_id_dtls);
				$data_array_dtls[str_replace("'",'',$$update_id_dtls)] =explode(",",("".$$txt_min_allocation.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."'"));
			}
			else
			{
					if ($coma!=0) $data_array_dtls_in.=",";
						$data_array_dtls_in.="(".$dtls_id.",".$mst_id.",".$$txt_buyer.",".$$txt_min_allocation.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
						$dtls_id=$dtls_id+1;
						$coma++;
		...
		$rID=sql_update("lib_min_lead_time_mst",$field_array_mst,$data_array_mst,"id",$update_id,0);
		$rID1=execute_query(bulk_update_sql_statement("lib_min_lead_time_dtls","id", $field_array_dtls,$data_array_dtls,$id_arr),1);
		if($data_array_dtls_in !="")
		{
		   $rID2=sql_insert("lib_min_lead_time_dtls",$field_array_dtls_in,$data_array_dtls_in,1);
...
{
	$qry_result=sql_select( "select id,mst_id from  lib_min_lead_time_dtls where mst_id='$data'");
...
<div style="width:1190px; height:auto">
    <table align="right" cellspacing="0" width="1190px"  border="1" rules="all" class="rpt_table_qty_allocation" id="tbl_month_pce" >
        ...
        <?
			...
			$sql_con="SELECT  a.company_id,b.buyer_id,b.min_allocation";
			for($i=1;$i<=12;$i++)
			{
				 $sql_con .= ",SUM(CASE WHEN d.month_id =".$i. " THEN (d.capacity_month_pcs)   END) AS 'capa$i',SUM(CASE WHEN d.month_id =".$i. " THEN (d.capacity_month_pcs* b.min_allocation)/100   END) AS 'sum$i'";
				
			}
			$sql_con .= " FROM lib_min_lead_time_mst a,lib_min_lead_time_dtls b, lib_capacity_calc_mst c,  lib_capacity_year_dtls d
WHERE a.id=b.mst_id AND a.year_id=c.year AND d.month_id=a.month_id AND  a.location_id=c.location_id AND c.id=d.mst_id AND a.company_id=$data[0]  AND a.year_id=$data[1] AND a.month_id=$data[3] AND a.location_id=$data[4]  GROUP BY b.buyer_id";
		...
		foreach( $sql_data as $row)
		{
			...
			$total_allocation = $total_allocation+$row[csf("min_allocation")];
		...
		?>
            ...
            <tr bgcolor="<? echo $bgcolor ; ?>">
                ...
                <td align="right"><? echo $row[csf("min_allocation")]; ?>%</td> 
            </tr>
...
<br />
    ...
    <table align="right" cellspacing="0" width="1190px"  border="1" rules="all" class="rpt_table_qty_allocation" id="tbl_month_munite" >
        ...
        <tbody>
			...
			for($i=1;$i<=12;$i++)
			{
				 $sql_con .= ",SUM(CASE WHEN d.month_id =".$i. " THEN (d.capacity_month_min)   END) AS 'capa$i',SUM(CASE WHEN d.month_id =".$i. " THEN (d.capacity_month_min* b.min_allocation)/100   END) AS 'sum$i' ";
