...
if ($action=="search_list_view")
{
	...
	$sql="select id, fab_nature_id, construction, gsm_weight, status_active, color_range_id, stich_length, process_loss, sequence_no,fabric_composition_id,rd_no,supplier_reference from  TABLE_SAKIB_TEST where is_deleted=0 and entry_form=184 order by id";
...
{
	$nameArray=sql_select("select fab_nature_id, construction, gsm_weight, fab_composition, color_range_id, fabric_composition_id, status_active, stich_length, process_loss, sequence_no, id, supplier_reference, rd_no, hs_code, count_range, fabric_construction_id from TABLE_SAKIB_TEST where id='$data'");
	$lib_group_short=return_library_array( "select id,group_short_name from lib_group where id=1 and status_active=1", "id", "group_short_name");
	$lib_fabric_composition=return_library_array( "select id,fabric_composition_name from lib_fabric_composition where status_active=1", "id", "fabric_composition_name");
	$lib_fabric_construction=return_library_array( "select id,fabric_construction_name from lib_fabric_construction where status_active=1", "id", "fabric_construction_name");
	$group_short_name=$lib_group_short[1];
	...
	function fnc_process_loss_entry(operation)
		{
			...
			http.open("POST","yarn_count_determination_test_controller.php",true);
...
if ($action=="save_update_delete_process_loss")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$rID_mst=sql_update("TABLE_SAKIB_TEST",$field_array_mst,$data_array_mst,"id","".$mst_id."",0);
...
if ($action=="save_update_delete")
{
	...
	$duplicate_sql=sql_select("SELECT a.id, b.id as dtls_id,  a.fabric_construction_id, b.copmposition_id, b.percent, b.count_id, b.type_id from TABLE_SAKIB_TEST a join lib_yarn_count_determina_dtls b on a.id=b.mst_id where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form=184 and a.fabric_construction_id=$fab_construction_id $comp_cond $updateid_cond");
	...
	if ($operation==0)  // Insert Here
	{
		...
		$id=return_next_id( "id", "TABLE_SAKIB_TEST", 1 ) ;
		$field_array1= "id, fab_nature_id,  construction, gsm_weight, color_range_id, stich_length, process_loss, sequence_no, rd_no, supplier_reference, fabric_composition_id, fabric_construction_id, entry_form, hs_code, count_range, inserted_by, insert_date, status_active, is_deleted";
		$data_array1="(".$id.",".$cbo_fabric_nature.",'".$txtconstruction."',".$txtgsmweight.",".$cbocolortype.",".$stichlength.",".$processloss.",".$txt_sequence.",".$txt_rd_number.",".$txt_supplier_reference.",".$fab_composition_id.",".$fab_construction_id.",184,$txt_hs_code,".$cbo_count_range.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_status.",'0')";
		...
		$rID=sql_insert("TABLE_SAKIB_TEST",$field_array1,$data_array1,0);
	...
	else if ($operation==1)   // Update Here
	{
		...
		$construction=str_replace("'",'',$txtconstruction);
		$txtconstruction=str_replace($str_rep,' ',$construction);
		$field_array1= "fab_nature_id*construction*gsm_weight*color_range_id*stich_length*process_loss*sequence_no*rd_no*supplier_reference*fabric_composition_id*fabric_construction_id*hs_code*count_range*updated_by*update_date*status_active*is_deleted*is_posted_sql";
		$data_array1="".$cbo_fabric_nature."*'".$txtconstruction."'*".$txtgsmweight."*".$cbocolortype."*".$stichlength."*".$processloss."*".$txt_sequence."*".$txt_rd_number."*".$txt_supplier_reference."*".$fab_composition_id."*".$fab_construction_id."*".$txt_hs_code."*".$cbo_count_range."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$cbo_status."*'0'*'2'";
			...
			if($is_used!=1)
			{
				$rID=sql_update("TABLE_SAKIB_TEST",$field_array1,$data_array1,"id","".$update_mst_id."",0);
			}			
			$rID_1=sql_insert("lib_yarn_count_determina_dtls",$field_array2,$data_array2,1);
	...
	else if ($operation==2) // Delete Here
	{
		...
		$rID=sql_delete("TABLE_SAKIB_TEST",$field_array1,$data_array1,"id","".$update_mst_id."",1);
