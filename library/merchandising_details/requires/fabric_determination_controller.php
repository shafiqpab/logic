...
if ($action=="search_list_view")
{
	...
	if (count($data_array)>0)
	{
		...
		echo  create_list_view ( "list_view", "Fab Nature,Fabric Ref,RD NO,Sys. Code,Type,Construction,Design,Weight,Weight Type,Color Range,Full Width,Cutable Width,Composition,Insert By,Status", "100,100,50,100,80,100,100,50,50,50,50,50,300,100,60","1470","350",0, $sql, "get_php_form_data", "id", "'load_php_data_to_form'",1, "fab_nature_id,0,0,0,id,0,0,weight_type,color_range_id,0,0,id,inserted_by,status_active", $arr , "fab_nature_id,fabric_ref,rd_no,type,id,construction,design,gsm_weight,weight_type,color_range_id,full_width,cutable_width,id,inserted_by,status_active", "requires/fabric_determination_controller",'setFilterGrid("list_view",-1);','0,0,0,0,0,0,0,2,0,0,0,0,0,0,0') ;*/
				...
				//print_r($sysCodeArr);				
				// $sql="select id,fab_nature_id, type, rd_no, construction, gsm_weight, weight_type, design, fabric_ref, color_range_id,inserted_by,status_active,full_width,cutable_width from  lib_yarn_count_determina_mst where is_deleted=0 and entry_form=426 order by id DESC";				
				// $arr=array (0=>$item_category, 3=>$sysCodeArr, 8=>$fabric_weight_type, 9=>$color_range,12=>$composition_arr,13=>$user_arr, 14=>$row_status);
				// echo  create_list_view ( "list_view", "Fab Nature,Fabric Ref,RD NO,Sys Code,Type,Construction,Design,Weight,Weight Type,Color Range,Full Width,Cutable Width,Composition,Insert By,Status", "100,100,50,80,100,100,100,50,50,50,50,50,300,100,50","1450","350",0, $sql, "get_php_form_data", "id", "'load_php_data_to_form'",1, "fab_nature_id,0,0,id,0,0,0,0,weight_type,color_range_id,0,0,id,inserted_by,status_active", $arr , "fab_nature_id,fabric_ref,rd_no,id,type,construction,design,gsm_weight,weight_type,color_range_id,full_width,cutable_width,id,inserted_by,status_active", "requires/fabric_determination_controller",'setFilterGrid("list_view",-1);','0,0,0,0,0,0,0,2,0,0,0,0,0,0,0') ;
				...
				echo  create_list_view ( "list_view", "Fab Nature,Fabric Ref,RD NO,Sys Code,Type,Construction,Design,Weight,Weight Type,Color Range,Full Width,Cutable Width,Shrinkage L %,Shrinkage W %,Composition,Insert By,Status", "100,100,50,80,100,100,100,50,50,50,50,50,60,60,300,100,50","1570","350",0, $sql, "get_php_form_data", "id", "'load_php_data_to_form'",1, "fab_nature_id,0,0,id,0,0,0,0,weight_type,color_range_id,0,0,0,0,id,inserted_by,status_active", $arr , "fab_nature_id,fabric_ref,rd_no,id,type,construction,design,gsm_weight,weight_type,color_range_id,full_width,cutable_width,shrinkage_l,shrinkage_w,id,inserted_by,status_active", "requires/fabric_determination_controller",'setFilterGrid("list_view",-1);','0,0,0,0,0,0,0,2,0,0,0,0,0,0,0') ;
...
{
	$nameArray=sql_select("select id,fab_nature_id, type, construction, gsm_weight, weight_type, design, fabric_ref, rd_no, color_range_id, entry_form, inserted_by,insert_date,status_active,is_deleted,full_width,cutable_width,shrinkage_l,shrinkage_w from lib_yarn_count_determina_mst where id='$data' and entry_form=426");
	$lib_group_short=return_library_array( "select id,group_short_name from lib_group where id=1 and status_active=1", "id", "group_short_name");	
...
if($action =="show_detail_form")
{
	...
	<table width="100%" border="0" id="tbl_yarn_count" cellpadding="0" cellspacing="0" class="rpt_table" rules="all" >
        <thead>
            <tr><th width="150" class="must_entry_caption"><font color="blue">Composition</font></th> <th width="50" class="must_entry_caption"><font color="blue">%</font></th> <th width="150">Count</th><th width="150">Type</th> <th width="">  </th> 
            </tr>
        </thead>
        <tbody>
			<?
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$duplicate_query=sql_select("SELECT id from lib_yarn_count_determina_mst where fab_nature_id=$cbo_fabric_nature and type='$txttype' and construction='$txtconstruction' and gsm_weight=$txtweight and weight_type=$cboweighttype and design='$txtdesign' and fabric_ref='$txtfabricref' and color_range_id=$cbocolortype and entry_form=426 and is_deleted=0");
		...
		$duplicate_data=sql_select("SELECT id from lib_yarn_count_determina_mst where rd_no='$txtrdno' and entry_form=426 and is_deleted=0");
		if (count($duplicate_data)>0)
		{
			...
			$field_array1= "id, fab_nature_id, type, construction, gsm_weight, weight_type, design, fabric_ref, rd_no, color_range_id, entry_form, full_width, cutable_width,shrinkage_l,shrinkage_w, inserted_by, insert_date, status_active, is_deleted";
			$data_array1="(".$id.",".$cbo_fabric_nature.",'".$txttype."','".$txtconstruction."',".$txtweight.",".$cboweighttype.",'".$txtdesign."','".$txtfabricref."','".$txtrdno."',".$cbocolortype.",426,".$txt_full_width.",".$txt_cutable_width.",'".str_replace("'", "", $txt_shrinkage_l)."','".str_replace("'", "", $txt_shrinkage_w)."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_status.",'0')";
	...
	else if ($operation==1)   // Update Here
	{
		...
		$inqueryNo=return_field_value("a.system_number as sys_no", "wo_quotation_inquery a, wo_quotation_inquery_fab_dtls b", " a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form=434 and constraction=".$update_mst_id."" ,"sys_no");
		...
		$duplicate_query=sql_select("SELECT id from lib_yarn_count_determina_mst where fab_nature_id=$cbo_fabric_nature and type='$txttype' and construction='$txtconstruction' and gsm_weight=$txtweight and weight_type=$cboweighttype and design='$txtdesign' and fabric_ref='$txtfabricref' and color_range_id=$cbocolortype and entry_form=426 and is_deleted=0 and id<>$update_mst_id");
		if (count($duplicate_query)>0)
		{
			echo "11**0"; disconnect($con); die;
		}
		...
		$duplicate_data=sql_select("SELECT id from lib_yarn_count_determina_mst where rd_no=$txtrdno and entry_form=426 and is_deleted=0 and id<>$update_mst_id");
		if (count($duplicate_data)>0)
		{
			...
			$field_array1= "fab_nature_id*type*construction*gsm_weight*weight_type*design*fabric_ref*rd_no*color_range_id*full_width*cutable_width*shrinkage_l*shrinkage_w*updated_by*update_date*status_active*is_deleted";

			$data_array1="".$cbo_fabric_nature."*'".$txttype."'*'".$txtconstruction."'*".$txtweight."*".$cboweighttype."*'".$txtdesign."'*'".$txtfabricref."'*'".$txtrdno."'*".$cbocolortype."*".$txt_full_width."*".$txt_cutable_width."*'".str_replace("'", "", $txt_shrinkage_l)."'*'".str_replace("'", "", $txt_shrinkage_w)."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$cbo_status."*'0'";				
			$id_dtls=return_next_id( "id", "lib_yarn_count_determina_dtls", 1 ) ;
