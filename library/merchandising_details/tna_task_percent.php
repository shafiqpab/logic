...
{
	if ( form_validation('txt_task_name*txt_start_percent*txt_end_percent','Task Name*Start Percent*End Percent')==false )
	{
		return;
	}	
	else
	{
		...
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_task_name*txt_start_percent*txt_end_percent*cbo_row_status*update_id*txt_notice_before*cbo_buyer_name',"../../");
		//alert(data);
		freeze_window(operation);
		http.open("POST","requires/tna_task_percent_controller.php",true);
...
function fnc_tna_task_entry_reponse()
{
	if(http.readyState == 4) 
	{
		...
		show_list_view('','report_settings_tna_task','list_view_report_settings','requires/tna_task_percent_controller','setFilterGrid("tbl_task_list",-1)');
		...
		<form name="tnataskentry_1" id="tnataskentry_1" method="" autocomplete="off">  <!------------------ Start Form ------------>
			...
			<table width="500" cellspacing="2" cellpadding="0" border="0">
            	 ...
            	 <tr style="display:none">
					...
					<td colspan="1">
					<?
                    echo create_drop_down( "cbo_task_catagory", 180, $tna_task_catagory,"", 1, "-- Select Catagory--", 0, "" );
                    ?>                     
					</td>
				</tr>
                <tr>
					...
					<td id="task_name_td" colspan="1">
						<? echo create_drop_down( "txt_task_name", 380, "select task_name,task_short_name from  lib_tna_task","task_name,task_short_name", 1, "-- Select Task --", $selected, "",0 );	 ?>				
					</td> 
                </tr>
                <tr>
                    <td width="120" >Buyer Name</td>
					<td colspan="1">
                    <? echo create_drop_down( "cbo_buyer_name", 380, "select id,buyer_name from  lib_buyer","id,buyer_name", 1, "-- Select Buyer --", $selected, "",0 );	 ?>			
				...
				<tr style="display:none">
					...
					<td colspan="1"> 
					<?
					echo create_drop_down( "cbo_module_name", 180, "select m_mod_id,main_module from main_module where status=1 order by mod_slno","m_mod_id,main_module", 1, "-- Select Module--", $selected, "load_drop_down( 'requires/tna_task_percent_controller', this.value, 'load_drop_down_page_link','page_link_td');" );
                    ?>                     
                    </td>
                 </tr>
                 <tr style="display:none">
                ...
                <tr>
					<td width="120">Start Percent</td>
					<td id="">
					 <input type="text" name="txt_start_percent" id="txt_start_percent" class="text_boxes_numeric" style="width:160px" />
                     &nbsp;&nbsp;Notice Before&nbsp;<input type="text" id="txt_notice_before" name="txt_notice_before" class="text_boxes_numeric" style="width:100px">
					</td>
				</tr>
                <tr>
					<td width="120">End Percent</td>
					<td id="">
					 <input type="text" name="txt_end_percent" id="txt_end_percent" class="text_boxes_numeric" style="width:160px" />
					</td>
				</tr>
                 <tr>
					...
					//echo create_drop_down( "cbo_row_status", 210, $row_status,"", 1, "-- Select status--", $selected, "",0 );	
					 echo create_drop_down( "cbo_row_status", 170, $row_status, 0, "", 1, "" ); 
					?> 
					</td>
				</tr>
                 <tr>
