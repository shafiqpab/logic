...
<script>
	...
	function fnc_terms_condition(operation) {
		...
		if (form_validation(validationField, 'Terms & Condition*Tag Page Name') == false) {
			...
			var data = "action=save_update_delete&operation=" + operation + get_submitted_data_string('txt_terms_condition*txt_terms_condition_more*txt_terms_condition_hdn*cbo_status*txt_tag_page*txt_tag_page_id*txt_tag_buyer*txt_tag_buyer_id*update_id*txt_terms_prefix', "../../");
	...
	function openmypage_tag_buyer() {
		var is_disable = $("#txt_tag_page").attr('isused') * 1;
		//alert(is_diable);
		...
		var title = 'Tag Buyer Selection Form';
		var page_link = 'requires/terms_condition_entry_controller.php?action=buyer_name_popup&txt_tag_buyer_id=' + txt_tag_buyer_id + '&is_disable=' + is_disable;
		emailwindow = dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=400px,height=370px,center=1,resize=1,scrolling=0', '../');
		emailwindow.onclose = function() {
			var theform = this.contentDoc.forms[0] //("search_order_frm"); //Access the form inside the modal window
				...
				<form name="colorinfo_1" id="colorinfo_1" autocomplete="off">
					<table cellpadding="0" cellspacing="2" width="500px">
						...
						<tr>
							...
							<td>
								<input type="text" name="txt_tag_buyer" id="txt_tag_buyer" class="text_boxes" style="width:92%;" placeholder="Click To Search" onClick="openmypage_tag_buyer();" readonly />
