...
$production_process_freact=array(1=>"Yarn",2=>"Knitting",3=>"AOP",4=>"Cutting",5=>"Printing",6=>"Printing Received",7=>"Embroidery",8=>"Sweing In",9=>"Sewing Out",10=>"Wash",11=>"Iron Output",12=>"Gmts Finishing",13=>"Ex-Factory",14=>"Dyeing",15=>"Grey To Dye",16=>"Fabric Available",17=>"Grey Production");

$gmt_prod_id_map[50]=50;//Yarn Issue
$gmt_prod_id_map[55]=55;//Grey Production
//$gmt_prod_id_map[3]=5;
...
$gmt_prod_id_map[65]=65;//Dyeing
$gmt_prod_id_map[60]=60;//Grey To Dye
$gmt_prod_id_map[70]=70;//Fabric Available
...
if ( $action=="save_update_delete" )
{
	...
	if ( $cbo_fr_integrtion==0 )
	{
		...
		{
			if(trim( $received_date)=="") $received_date=date('d-M-Y', strtotime('-2 months')); else $received_date=date("d-M-Y",strtotime($received_date));//$received_date=change_date_format($received_date, "yyyy-mm-dd", "-",1);
		}
		// Products file Data
		...
		$sql_po_data=sql_select($sql_po); $product_size_arr=array(); $orders_arr=array(); $orders_data_arr=array(); $order_size_arr=array(); $prod_up_arr=array(); $po_wise_job_arr=array();
		foreach($sql_po_data as $porow)
		{
			...
			$po_wise_job_arr[$porow[csf('bid')]]=$porow[csf('job_no')];
		...
		//print_r($orders_arr_tmp['FKTL-16-00855']); die;
		unset($sql_po_data);
		...
		where $job_cond_in and d.job_no=e.job_no   and d.is_deleted=0 and d.status_active=1 and e.is_deleted=0 and e.status_active=1 ";
		...
		unset($sql_po_data);
		 ...
		 $sql_po="select  a.po_break_down_id, d.booking_no 
		from wo_booking_mst d, WO_BOOKING_DTLS a
		where $job_cond_ind and d.booking_type=1 and d.is_short=2 and d.job_no=a.job_no and d.is_deleted=0 and d.status_active=1  group by a.po_break_down_id, d.booking_no   ";
						...
						//{  
							foreach($color_data as $color_id=>$other_val)
							{
								if($other_val['order_qty']>1)
								{
									...
									foreach($po_id_arr as $pid)
									{
										$newpoid_arr[trim($pid)]=$str_po;
									}
									$po_id_arrs=implode("",array_unique($po_id_arr));
									...
									$txt .=str_pad($str_po,0," ")."\t".str_pad($old_str_po,0," ")."\t".str_pad($buyer_style,0," ")."\t".str_pad($buyer_name_array[$ft_data_arr[$jobno][buyer_name]],0," ")."\t".str_pad(date("d/m/Y",strtotime($shipdate)),0," ")."\t".str_pad($other_val['plan_cut'],0," ")."\t".str_pad($other_val['order_qty'],0," ")."\t".str_pad($str,0," ")."\t".str_pad($fob,0," ")."\t".str_pad($style_ref_arr[$jobno],0," ")."\t".str_pad($ft_data_arr_book[$po_id_arrs]['booking_no'],0," ")."\t".str_pad($orders_data_arr[$po_id]['season'],0," ")."\t".str_pad($ssts,0," ")."\t".str_pad($extShipDate,0," ")."\t\r\n";
								}
							}
						//}
					}
		...
		//echo "0**"; print_r($newpoid_arr); die;
		$txt="";
		...
		$inv_prod_fabric_arr=array();
		//$yarn_issue_array=array(); $gp_qty_array=array(); $greyToDyeIssueQty = array();
		$inv_sql=sql_select("select max(a.transaction_date) as prod_date, b.po_breakdown_id, b.entry_form, sum(b.quantity) as quantity from inv_transaction a, order_wise_pro_details b, tmp_poid d where a.id = b.trans_id and b.po_breakdown_id=d.poid and b.entry_form in (2,3,16,19,61) and b.status_active=1 and b.is_deleted=0 and b.is_sales !=1 group by b.po_breakdown_id, b.entry_form");
		
		foreach($inv_sql as $row_inv)
		{
			if($row_inv[csf('entry_form')]==2)
			{
				//$gp_qty_array[$row_inv[csf('po_breakdown_id')]][grey_prod]=1;//P^WC:55
				//$inv_prod_fabric_arr[$row_inv[csf('po_breakdown_id')]][2]+=$row_inv[csf('quantity')];
				
				$production_qty[$newpoid_arr[$row_inv[csf("po_breakdown_id")]]][55][0][qnty]=$row_inv[csf("quantity")];
				$production_qty[$newpoid_arr[$row_inv[csf("po_breakdown_id")]]][55][0][pdate]=$row_inv[csf("prod_date")];
			}
			else if($row_inv[csf('entry_form')]==3)
			{
				//$yarn_issue_array[$row_inv[csf('po_breakdown_id')]][yarn_iss]=1;//P^WC:50
				//$inv_prod_fabric_arr[$row_inv[csf('po_breakdown_id')]][1]+=$row_inv[csf('quantity')];
				$production_qty[$newpoid_arr[$row_inv[csf("po_breakdown_id")]]][50][0][qnty]=$row_inv[csf("quantity")];
				$production_qty[$newpoid_arr[$row_inv[csf("po_breakdown_id")]]][50][0][pdate]=$row_inv[csf("prod_date")];
			}
			else if($row_inv[csf('entry_form')]==16 || $row_inv[csf('entry_form')]==19 || $row_inv[csf('entry_form')]==61)
			{
				//$greyToDyeIssueQty[$row_inv[csf('po_breakdown_id')]][greyto_dye]=1;//P^WC:60
				//$inv_prod_fabric_arr[$row_inv[csf('po_breakdown_id')]][15]+=$row_inv[csf('quantity')];
				$production_qty[$newpoid_arr[$row_inv[csf("po_breakdown_id")]]][60][0][qnty]=$row_inv[csf("quantity")];
				$production_qty[$newpoid_arr[$row_inv[csf("po_breakdown_id")]]][60][0][pdate]=$row_inv[csf("prod_date")];
			}
		}
		unset($inv_sql);
		
		$dyeing_qnty_arr=array();
		$dyeing_qty_sql=sql_select( "select b.po_id, sum(b.batch_qnty) as batch_qnty, max(c.process_end_date) as prod_date from pro_batch_create_mst a,pro_batch_create_dtls b, pro_fab_subprocess c, tmp_poid d where a.id=b.mst_id and a.id=c.batch_id and b.po_id=d.poid and c.load_unload_id=2 and c.entry_form=35 and a.batch_against<>2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by b.po_id");
		foreach($dyeing_qty_sql as $row_dye)
		{
			$production_qty[$newpoid_arr[$row_dye[csf("po_id")]]][65][0][qnty]=$row_dye[csf("batch_qnty")];
			$production_qty[$newpoid_arr[$row_dye[csf("po_id")]]][65][0][pdate]=$row_dye[csf("prod_date")];
		}
		unset($dyeing_qty_sql);
		
		//$fabrics_avl_qnty_arr=array();
		$auto_fab_sotre_update=2;
		if($auto_fab_sotre_update==2)
		{
			$fabrics_avl_qnty_sql=sql_select( "select max(a.transaction_date) as prod_date, b.po_breakdown_id, sum(b.quantity) as quantity from inv_transaction a, order_wise_pro_details b, tmp_poid d where a.id = b.trans_id and b.po_breakdown_id=d.poid and b.entry_form=37 and b.status_active=1 and b.is_deleted=0 group by b.po_breakdown_id");
			
		}
		else
		{
			$fabrics_avl_qnty_sql=sql_select( "select max(a.transaction_date) as prod_date, b.po_breakdown_id, sum(b.quantity) as quantity from inv_transaction a, order_wise_pro_details b, tmp_poid d where a.id = b.trans_id and b.po_breakdown_id=d.poid and b.entry_form=7 and b.status_active=1 and b.is_deleted=0 group by b.po_breakdown_id
			union all
			select max(a.transaction_date) as prod_date, b.po_breakdown_id, sum(b.quantity) as quantity from order_wise_pro_details b, inv_transaction a, tmp_poid d where  b.trans_id = a.id and b.po_breakdown_id=d.poid and b.entry_form=37 and a.receive_basis in (2,11) and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0  group by b.po_breakdown_id");
		
		}
		
		foreach($fabrics_avl_qnty_sql as $row_fav)
		{
			//$fabrics_avl_qnty_arr[$po_wise_job_arr[$row_fav[csf('po_breakdown_id')]]][fab_avai]=1;//P^WC:70
			//$inv_prod_fabric_arr[$row_fav[csf('po_breakdown_id')]][16]+=$row_fav[csf('quantity')];
			$production_qty[$newpoid_arr[$row_fav[csf("po_breakdown_id")]]][70][0][qnty]=$row_fav[csf("quantity")];
			$production_qty[$newpoid_arr[$row_fav[csf("po_breakdown_id")]]][70][0][pdate]=$row_fav[csf("prod_date")];
		}
		unset($fabrics_avl_qnty_sql);
		...
		$prod_sql="SELECT a.po_break_down_id, a.country_id, a.item_number_id, a.company_id, a.floor_id, a.sewing_line, a.production_type, a.production_date as production_date, b.color_size_break_down_id, b.production_qnty AS production_quantity, a.embel_name from pro_garments_production_mst a, pro_garments_production_dtls b, tmp_col_po_id c where a.production_type in (1,3,4,5,8,7) and a.id=b.mst_id and a.is_deleted=0 and a.status_active=1 and b.color_size_break_down_id=c.col_po_id and c.userid='$user_id' order by  production_date ASC,b.color_size_break_down_id asc"; //$poIds_cond_prod tmp_poid (userid, poid)  and a.embel_name in (1,2,3,4,5) 
		//echo $prod_sql; die;
