...
if ( $action=="save_update_delete" )
{
	...
	if ( $cbo_fr_integrtion==0 )
	{
		...
		if( $db_type==0) $shipment_date=" and b.po_received_date >= '$received_date' "; else if($db_type==2) $shipment_date=" and b.po_received_date >= '$received_date' ";
		...
		from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id and a.company_name!=6 '.$shipment_date.$shiping_status.' and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and c.is_deleted=0 order by a.id';
		...
		foreach($sql_po_data as $porow)
		{
			...
			$po_str=$porow['bid'].'_'.$pub_ship_date.'_'.$porow['is_confirmed'].'_'.$porow['status_active'].'_'.$porow['projected_po_id'].'_'.$porow['factory_received_date'];
		...
		foreach($production_qty as $oid=>$prodctn)
		{
			...
			foreach($prodctn as $prod_typ=>$prodtypedata)
			{
				...
				foreach($prodtypedata as $line=>$othersdata)
				{
					...
					if($line==0) $linestr=""; else $linestr=$erplinenamearr[$line];//(int)$line_name_res[$line]; 
