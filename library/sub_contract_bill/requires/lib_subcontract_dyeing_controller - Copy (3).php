...
if ($action=="list_view_subcon_dying_charge")
{
	...
	$arr=array (0=>$company_arr,2=>$process_type,3=>$conversion_cost_head_array,4=>$color_library_arr, 5=> $color_range, 6=>$fabric_typee,8=>$unit_of_measurement,9=>$production_process,11=>$buyer_arr,12=>$row_status);

	echo create_list_view ( "list_view", "Company Name,Const. Compo.,Process Type,Process Name,Color,Color Range,Width/Dia type,In House Rate,UOM,Rate type,Cust. Rate,Buyer,Status", "90,130,70,70,70,70,80,60,40,60,70,80,50","1040","250",1, "select id, comapny_id, const_comp, process_type_id, process_id, color_id, color_range_id, width_dia_id, in_house_rate, uom_id, rate_type_id ,customer_rate, buyer_id, status_active from lib_subcon_charge where status_active!=0 and is_deleted=0 and rate_type_id in (3,4,7,6)", "get_php_form_data", "id","'load_php_data_to_form'", 1, "comapny_id,0,process_type_id,process_id,color_id,color_range_id,width_dia_id,0,uom_id,rate_type_id,0,buyer_id,status_active", $arr, "comapny_id,const_comp,process_type_id,process_id,color_id,color_range_id,width_dia_id,in_house_rate,uom_id,rate_type_id,customer_rate,buyer_id,status_active","requires/lib_subcontract_dyeing_controller", 'setFilterGrid("list_view",-1);','0,0,0,0,0,0,0,2,0,0,2,0,0');
	exit();
}
...
{
	$color_library_arr=return_library_array( "select id,color_name from lib_color where status_active =1 and is_deleted=0", "id", "color_name");
	$nameArray=sql_select( "select id, comapny_id, const_comp, cons_comp_id, process_type_id, process_id, color_id, width_dia_id, in_house_rate, uom_id, rate_type_id, customer_rate, buyer_id,status_active,color_range_id from lib_subcon_charge where id='$data'" );
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)
	{
		...
		else
		{
			...
			$field_array="id, comapny_id, const_comp, cons_comp_id, process_type_id, process_id, color_id, width_dia_id, in_house_rate, uom_id, rate_type_id, customer_rate, buyer_id,color_range_id, inserted_by, insert_date, status_active, is_deleted";
			$data_array="(".$id.",".$cbo_company_id.",".$text_const_compo.",".$txt_cons_comp_id.",".$cbo_process_type.",".$cbo_process_id.",'".$color_tbl_id."',".$cbo_dia_width.",".$text_in_house_rate.",".$cbo_uom.",".$cbo_rate_type.",".$txt_customer_rate.",".$cbo_buyer_id.",".$cbo_color_range_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_status.",'0')";
	...
	else if ($operation==1)   // Update Here
	{
		...
		else
		{
			...
			$field_array="comapny_id*const_comp*cons_comp_id*process_type_id*process_id*color_id*width_dia_id*in_house_rate*uom_id*rate_type_id*customer_rate*buyer_id*color_range_id*updated_by*update_date*status_active*is_deleted";
			$data_array="".$cbo_company_id."*".$text_const_compo."*".$txt_cons_comp_id."*".$cbo_process_type."*".$cbo_process_id."*".$color_tbl_id."*".$cbo_dia_width."*".$text_in_house_rate."*".$cbo_uom."*".$cbo_rate_type."*".$txt_customer_rate."*".$cbo_buyer_id."*".$cbo_color_range_id."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$cbo_status."*'0'";
            ...
            <tbody>
                ...
                ?>
                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="js_set_value('<? echo $row[csf("id")].'***'.$row[csf("construction")].', '.chop($composition_arr[$row[csf("id")]],",");//.'***'.$row[csf("gsm_weight")] ?>')" id="tr_<? echo $i; ?>">
                        ...
                        <td width="150"><? echo chop($composition_arr[$row[csf("id")]],",") ; ?></td>
                        <td width="80"><? echo $row[csf("sequence_no")]; ?></td>
                        <td width="50"><? echo $row_status[$row[csf("status_active")]]; ?></td>
