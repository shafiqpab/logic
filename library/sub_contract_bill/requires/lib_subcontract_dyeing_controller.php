...
{
	$color_library_arr=return_library_array( "select id,color_name from lib_color where status_active =1 and is_deleted=0", "id", "color_name");
	$nameArray=sql_select( "select id, comapny_id, const_comp, cons_comp_id, process_type_id, process_id, color_id, width_dia_id, in_house_rate, uom_id, rate_type_id, customer_rate, buyer_id,status_active,color_range_id,dia_range,gsm_range,efficiency_range from lib_subcon_charge where id='$data'" );
	foreach ($nameArray as $inf)
	{
		...
		echo "document.getElementById('cbo_color_range_id').value  	= '".$inf[csf("color_range_id")]."';\n"; 
		echo "document.getElementById('text_dia_range').value  		= '".$inf[csf("dia_range")]."';\n"; 
		echo "document.getElementById('txt_gsm_range').value  		= '".$inf[csf("gsm_range")]."';\n"; 
		echo "document.getElementById('text_efficiency_range').value  	= '".$inf[csf("efficiency_range")]."';\n"; 
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)
	{
		...
		if(trim(str_replace("'","",$txt_color))!="")
		{
			if (!in_array(trim(str_replace("'", "", $txt_color)),$new_array_color,TRUE))
			{
				$color_tbl_id = return_id( trim(str_replace("'", "", $txt_color)), $color_library_arr, "lib_color", "id,color_name","0");
				$new_array_color[$color_tbl_id]=trim(str_replace("'", "", $txt_color));
			}
			else $color_tbl_id =  array_search(trim(str_replace("'", "", $txt_color)), $new_array_color,TRUE);
		}
		else $color_tbl_id=0;
		...
		{
			//cbo_color_range_id color_range_id  
			$id=return_next_id( "id", "lib_subcon_charge", 1 ) ; 
			
			$field_array="id, comapny_id, const_comp, cons_comp_id, process_type_id, process_id, color_id, width_dia_id, in_house_rate, uom_id, rate_type_id, customer_rate, buyer_id,color_range_id,dia_range,gsm_range,efficiency_range, inserted_by, insert_date, status_active, is_deleted";
			$data_array="(".$id.",".$cbo_company_id.",".$text_const_compo.",".$txt_cons_comp_id.",".$cbo_process_type.",".$cbo_process_id.",'".$color_tbl_id."',".$cbo_dia_width.",".$text_in_house_rate.",".$cbo_uom.",".$cbo_rate_type.",".$txt_customer_rate.",".$cbo_buyer_id.",".$cbo_color_range_id.",".$text_dia_range.",".$txt_gsm_range.",".$text_efficiency_range.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_status.",'0')";
	...
	else if ($operation==1)   // Update Here
	{
		...
		else
		{
			...
			$field_array="comapny_id*const_comp*cons_comp_id*process_type_id*process_id*color_id*width_dia_id*in_house_rate*uom_id*rate_type_id*customer_rate*buyer_id*color_range_id*dia_range*gsm_range*efficiency_range*updated_by*update_date*status_active*is_deleted";
			$data_array="".$cbo_company_id."*".$text_const_compo."*".$txt_cons_comp_id."*".$cbo_process_type."*".$cbo_process_id."*".$color_tbl_id."*".$cbo_dia_width."*".$text_in_house_rate."*".$cbo_uom."*".$cbo_rate_type."*".$txt_customer_rate."*".$cbo_buyer_id."*".$cbo_color_range_id."*".$text_dia_range."*".$txt_gsm_range."*".$text_efficiency_range."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$cbo_status."*'0'";
...
if($action=="const_comp_popup")
{
	...
	/*$sql="SELECT a.id, a.fab_nature_id, a.construction, a.gsm_weight, a.color_range_id, a.stich_length, a.process_loss,a.sequence_no, b.copmposition_id, b.percent, b.count_id, b.type_id, a.status_active,b.status_active 
	from  lib_yarn_count_determina_mst a,  lib_yarn_count_determina_dtls b 
	where a.id=b.mst_id and a.is_deleted=0 and b.is_deleted=0 order by b.id";*/
	 $sql="SELECT a.id, a.fab_nature_id, a.construction, a.gsm_weight, a.color_range_id, a.stich_length, a.process_loss,a.sequence_no, a.status_active
	from  lib_yarn_count_determina_mst a,  lib_yarn_count_determina_dtls b 
	where a.id=b.mst_id and a.is_deleted=0  group by  a.id, a.fab_nature_id, a.construction, a.gsm_weight, a.color_range_id, a.stich_length, a.process_loss,a.sequence_no,a.status_active  order by a.id";
    ...
    <div style="width:885; max-height:350px; overflow-y:scroll">
        <table cellpadding="0" width="865" class="rpt_table" rules="all" border="1" id="table_body">
            <tbody>
                ...
                ?>
                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="js_set_value('<? echo $row[csf("id")].'***'.$row[csf("construction")].', '.chop($composition_arr[$row[csf("id")]],",");//.'***'.$row[csf("gsm_weight")] ?>')" id="tr_<? echo $i; ?>">
                        <td width="25"><? echo $i; ?></td>
                        <td style="word-break:break-all"  width="80"><? echo $business_nature_arr[$row[csf("fab_nature_id")]]; ?></td>
                        <td style="word-break:break-all"  width="120"><? echo $row[csf("construction")]; ?></td>
                        <td style="word-break:break-all"  width="100"><? echo $row[csf("gsm_weight")]; ?></td>
                        <td style="word-break:break-all"  width="90"><? echo $color_range[$row[csf("color_range_id")]]; ?></td>
                        <td style="word-break:break-all"  width="80"><? echo $row[csf("stich_length")]; ?></td>
                        <td style="word-break:break-all"  width="80"><? echo $row[csf("process_loss")]; ?></td>
                        <td style="word-break:break-all"  width="150"><? echo chop($composition_arr[$row[csf("id")]],",") ; ?></td>
                        <td style="word-break:break-all"  width="80"><? echo $row[csf("sequence_no")]; ?></td>
                        <td style="word-break:break-all"  width="50"><? echo $row_status[$row[csf("status_active")]]; ?></td>
