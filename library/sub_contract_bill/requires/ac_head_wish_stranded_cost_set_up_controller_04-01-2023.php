		...
		<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="750">
            ...
            <tbody>
                <tr bgcolor="<?php echo $bgcolor?>" >
                    ...
                    <td align="right"><?php echo number_format($row["COST_PER_MIN"],8,'.','');?></td>
                    <td align="right"><?php echo number_format($row["COST_PER_PC"],8,'.','');?></td>
                </tr>
            </tbody>
            <?php 
            $total_amount+=$row["AMMOUNT"];
            $total_cost_min+=$row["COST_PER_MIN"];
            $total_cost_pc+=$row["COST_PER_PC"];
            }			
            ?>
            <tfoot>
                <tr>
                    ...
                    <th><?php echo number_format($total_cost_min,8,'.','');?></th>
                    <th><?php echo number_format($total_cost_pc,8,'.','');?></th>
                 </tr>
            </tfoot>
		</table>
	</div>
...
if ($action=="load_ac_head_tbl")
{
                  ...
                  <table cellspacing="0" cellpadding="0" border="1" rules="all" width="350" class="rpt_table" id="table_list_dtls">
					...
					foreach($account_head as $value)
					{
						...
						<tr  bgcolor="<? echo $bgcolor; ?>">
							...
							<input type="hidden" id="txt_capacity_min_<? echo $i;?>" value="<? echo $all_prv_data[$value["ID"]]["COST_PER_PC"]?>" style="width: 70px;" name="txt_capacity_min[]" class="text_boxes_numeric">
							<input type="hidden" id="txt_capacity_pcs_<? echo $i;?>" value="<? echo $all_prv_data[$value["ID"]]["COST_PER_MIN"]?>" style="width: 70px;" name="txt_capacity_pcs[]" class="text_boxes_numeric">						
						</td>
						</tr>
						<?
					}
	...
	else if($process_typ ==2)
	{
		...
		where a.id=b.mst_id and a.comapny_id=$company_id and a.year=$year and b.month_id=$month and  a.status_active = 1 and a.is_deleted = 0 and  b.status_active = 1 and b.is_deleted = 0");
