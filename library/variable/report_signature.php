...
<script>
	...
	{  
		...
		// 	  return;
		// }
		if(form_validation('cbo_company_name*cbo_report_name','Company Name*Report List')==false)
		{
			return;
		}
		else
		{ 
			...
			for(var i=1;i<=tot_row;i++)
			{
				...
				{
					j++;
					data_all+="&txtDesignation_" + j + "='" + $('#txtDesignation_'+i).val()+"'"+"&txtName_" + j + "='" + $('#txtName_'+i).val()+"'"+"&txtSequenceNo_" + j + "='" + $('#txtSequenceNo_'+i).val()+"'"+"&txtActivities_" + j + "='" + $('#txtActivities_'+i).val()+"'"+"&txtUser_" + j + "='" + $('#txtUser_'+i).val()+"'"+"&txtGroupBy_" + j + "='" + $('#txtgroupBy_'+i).val()+"'";
				}
			}
			...
			var data="action=save_update_delete&operation="+operation+get_submitted_data_string('cbo_company_name*cbo_report_name*cbo_template_id*cbo_prepared_by',"../../")+data_all+'&tot_row='+j;
			
			freeze_window(operation);
			http.open("POST","requires/report_signature_controller.php",true);
	...
	function fnc_report_signature_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			{
				show_list_view(response[1]+'_'+response[2]+'_'+response[4], 'signature_details', 'list_view_report_signature', 'requires/report_signature_controller', '' ) ;
			}
	...
	function check_company() 
	{
		...
		show_list_view(document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_report_name').value+'_'+document.getElementById('cbo_template_id').value, 'signature_details', 'list_view_report_signature', 'requires/report_signature_controller', '' ) ;	
            ...
            <form name="reportsignature_1" id="reportsignature_1" autocomplete="off">	
      			<table width="800" cellspacing="0" cellpadding="0" border="1" rules="all">
                	<tr>
                        ...
                        <td>
						<?
						echo create_drop_down( "cbo_company_name", 150, "SELECT comp.id, comp.company_name FROM lib_company comp WHERE comp.status_active=1 AND comp.is_deleted=0 $company_cond ORDER BY comp.company_name","id,company_name", 1, "-- Select Company --", 0, "reset_form('','list_view_report_signature','','');" );
						?>                          
                        </td>
                        <td class="must_entry_caption">Report List</td>
                        <td>
						<?
						echo create_drop_down( "cbo_report_name", 150, $report_signeture_list,'', 0, '', 0, "check_company();");
						?>
                        </td>
