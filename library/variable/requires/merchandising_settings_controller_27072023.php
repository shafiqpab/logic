...
if ($action=="on_change_data")
{
            ...
            <div style="width:400px; min-height:20px; max-height:250px;" id="variable_list_cont2" align="center">
                <table cellspacing="0" width="100%" >
                    <tr>
                        ...
                        <td width="190"><? echo create_drop_down( "cbo_consumption_basis", 170, $consumtion_basis,'', 1, '---- Select ----', $nameArray[0][csf('consumption_basis')], "" ); ?>
                        </td>
                    </tr>
                </table>
            </div>
		...
		<fieldset>
            ...
            <div style="width:400px; min-height:20px; max-height:250px;" id="variable_list_cont2" align="center">
                <table cellspacing="0" width="100%" border="1" class="rpt_table" rules="all">
                    ...
                    <tbody>
                    <?
                    $default = array(1=>'3',2=>'4',3=>'6',4=>'7'); $i=1;
                    if(count($nameArray)==0)
                    {
						foreach($default as $key=>$id)
						{
							...
							?>
							<tr id="tr_<? echo $key; ?>">
                                <td width="110" align="center"><? echo create_drop_down( "cbo_rate_type".$key, 100, $production_process,'', 1, "--Select Type--",$id , "","","3,4,6,7"); ?></td>
                                <td align="center">
						...
						foreach($nameArray as $row)
						{
							...
							?>
							<tr id="tr_<? echo $i; ?>">
								<td width="110" align="center"><? echo create_drop_down( "cbo_rate_type".$i, 100, $production_process,'', 1, "--Select Type--",$row[csf('rate_type')] , "","","3,4,6,7"); ?></td>
								<td align="center">
            ...
            <div style="width:600px; min-height:20px; max-height:250px;" id="variable_list_cont2" align="center">
                <table cellspacing="0" width="100%" class="rpt_table">
                    <thead>
                        ...
                        <th width="150" align="left">Exceed Qty Level Above Exceed Budget Qty(%)</th>
                    </thead>
                    <?
                    $i=1; $id_req=array(4);
                    foreach($item_category as $key=>$value)
                    {
						if (in_array($key,$id_req))
						{
							$resultRow= sql_select("select id,item_category_id, exeed_budge_qty,exeed_budge_amount,amount_exceed_level,exceed_qty_level from variable_order_tracking where company_name='$company_id' and item_category_id='$key' and variable_list=26 order by id");
	...
	if($type==27 || $type==57 || $type==58 || $type==84)//27 Commercial Cost Predefined Method-Pre-Costing, 57 Currier Cost Predefined Method, 58 Commercial Cost Predefined Method-Price Quotation, 84 Commercial Cost Predefined Method-QC
	{
		if($type==27)
		{
			$caption_th="Commercial Cost Predefined Method-Pre-Costing"; $setting_type=27; $setting_method="1,2,3,5,6,7,9";
		}
		else if( $type==57)
		{
			$caption_th="Currier Cost Predefined Method"; $setting_type=57; $setting_method="1,2,3";
		}
		else if( $type==58)
		{
			$caption_th="Commercial Cost Predefined Method-Price Quotation"; $setting_type=58;
			if($garments_nature == 3) $setting_method="1,2,4,5,6,7"; else $setting_method="1,2,4";
		}
		else if( $type==84)
		{
			$caption_th="Commercial Cost Predefined Method-QC"; $setting_type=84;
			if($garments_nature == 3) $setting_method="1,2,8"; else $setting_method="0";
		}
		
		$nameArray=sql_select( "select commercial_cost_method, id, commercial_cost_percent, editable from variable_order_tracking where company_name='$company_id' and variable_list=$setting_type order by id" );
		...
		<fieldset>
            ...
            <div style="width:800px; min-height:20px; max-height:250px;" id="variable_list_cont2" align="center">
                <table cellspacing="0" width="100%" >
                    <tr>
                        <td width="300" align="center" id="cbo_commercial_cost_method_td"><?=$caption_th; ?>  Cost</td>
                        <td width="190"><?=create_drop_down( "cbo_commercial_cost_method", 190, $commercial_cost_predefined_method,'', 1, '---- Select ----', $nameArray[0][csf('commercial_cost_method')], "","",$setting_method ); ?></td>
                        <td width="80" align="center">Percent</td>
                        ...
                        <td width="80" align="center">Editable</td>
                        <td width="100"><? echo create_drop_down( "cbo_editable", 100, $yes_no,'', 1, '---- Select ----', $nameArray[0][csf('editable')], "" ); ?></td>
                    </tr>
                </table>
            </div>
            <div style="width:500px; min-height:20px; max-height:250px;" id="variable_list_cont2" align="center">
	...
	{
		$nameArray=sql_select( "select tna_process_type,id from  variable_order_tracking where company_name='$company_id' and variable_list=31 order by id" );
		if(count($nameArray)>0)$is_update=1;else $is_update=0;
		?>
		<fieldset>
            ...
            <div style="width:400px; min-height:20px; max-height:250px;" id="variable_list_cont2" align="center">
                <table cellspacing="0" width="100%" >
                    <tr>
                        <td width="100">Tna Process type</td>
                        <td width="190">
	...
	{
		$trims_sup_buyer_tag=array(1=>"When no relation between Item and Supplier",2=>"When  relation between Item and Supplier" ,3=>"When  relation amoung Item,Supplier and Buyer");
            ...
            <div style="width:400px; min-height:20px; max-height:250px;" id="variable_list_cont2" align="center">
                <table cellspacing="0" width="100%" >
                    <tr>
                        ...
                        <td width="190">
							<?
                            $bom_cost_control_source_arr=array(1=>"Quick Costing",2=>"Price Quotation",3=>"Without Control",4=>"Quick Costing[WVN]",5=>"Short Quotation V2",6=>"Short Quotation V3",7=>"Short Quotation [Sweater]");
					...
					if(count($nameArray)==0)
                    {
						foreach($itemCatArr as $id)
						{
							...
							<tr id="tr_<? echo $i; ?>">
                                ...
                                <td align="center">
									<? 
                                    if ($id != 8) echo create_drop_down( "cbo_source_id".$i, 150, $sourceArr,'', 1, '--Select--', $selected, "" );
                    ...
                    {                        
						foreach($nameArray as $row)
						{
							...
							<tr id="tr_<? echo $i; ?>">
								...
								<td align="center">
									<? 
                                    if ($row[csf('ITEM_CATEGORY_ID')] != 8) echo create_drop_down( "cbo_source_id".$i, 150, $sourceArr,'', 1, '--Select--', $row[csf('EXCUT_SOURCE')], "" ); 
	...
	{
		$nameArray=sql_select( "select id, excut_source,exceed_qty_level from variable_order_tracking where company_name='$company_id' and variable_list=74 order by id" );
		if(count($nameArray)>0)$is_update=1;else $is_update=0;
		?>
		<fieldset>
            <legend>Sample delivery date calculation:</legend>
            <div style="width:400px; min-height:20px; max-height:250px;" id="variable_list_cont2" align="center">
                <table cellspacing="0" width="100%"  border="1" class="rpt_table" border="1" rules="all" >
                    <tr>
                    <td width="50">1</td>
		...
		else
		{
			...
			elseif(str_replace("'","", $cbo_variable_list_wo)==74)
            {
               //exceed_qty_level excut_source
			    $field_array="id,company_name,excut_source,exceed_qty_level,variable_list,inserted_by,insert_date,status_active,is_deleted";
                $data_array ="(".$id.",". $cbo_company_name_wo.",".$txt_without_emblish.",".$txt_with_emblish.",". $cbo_variable_list_wo.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";                
            }
            ...
            }elseif(str_replace("'","", $cbo_variable_list_wo)==94)
            {
	...
	else if ($operation==1)   // Update Here
	{
		...
		else if(str_replace("'","", $cbo_variable_list_wo)==76)//Budget UnApproved Validation
		{
			...
			for ($i=1;$i<=$total_row;$i++)
            {
				...
				//$data_array .="(".$id.",". $cbo_company_name_wo.",".$$embellishmentName.",". $cbo_variable_list_wo.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		...
		elseif(str_replace("'", "", $cbo_variable_list_wo)==74) //
        {
            $field_array="company_name*excut_source*exceed_qty_level*variable_list*updated_by*update_date";           

            $data_array ="". $cbo_company_name_wo."*".$txt_without_emblish."*".$txt_with_emblish."*". $cbo_variable_list_wo."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
