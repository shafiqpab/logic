...
{ 
	...
	{ 
		...
		$data_array="";
		$field_array="id,report_id,template_id,user_id, prepared_by, designation, name,activities, sequence_no, inserted_by, insert_date";
		...
		for($i=1;$i<=$tot_row; $i++)
		{
			...
			$data_array.="$add_comma(".$id.",".$cbo_report_name.",".$cbo_template_id.",".$$txtUser.",".$cbo_prepared_by.",".$$txtDesignation.",".$$txtName.",".$$txtActivities.",".$$txtSequenceNo.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";       
		}
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");
				echo "0**".str_replace("'","")."**".str_replace("'","",$cbo_report_name)."**1"."**".str_replace("'","",$cbo_template_id);
			}
			else
			{
				mysql_query("ROLLBACK"); 
				echo "5**".str_replace("'","",$cbo_report_name)."**0"."**".str_replace("'","",$cbo_template_id);
			}
		}
		...
		if($db_type==2 || $db_type==1 )
		{
			...
			{   
			    oci_commit($con); 
				echo "0**".str_replace("'","",$cbo_report_name)."**1"."**".str_replace("'","",$cbo_template_id);
			}
			else
			{ 
			     oci_rollback($con);
	...
	{ 
		...
		//$cbo_template_id
		$delete=execute_query("delete from variable_settings_signature where report_id=$cbo_report_name and template_id=$cbo_template_id ",1);
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");
				echo "1**".str_replace("'","",$cbo_report_name)."**1"."**".str_replace("'","",$cbo_template_id);
			}
			else
			{
				mysql_query("ROLLBACK"); 
				echo "6**"."**".str_replace("'","",$cbo_report_name)."**1"."**".str_replace("'","",$cbo_template_id);
			}
		}
	...
	{ 
		...
		$delete=execute_query("delete from variable_settings_signature where report_id=$cbo_report_name",1);
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");
				echo "2**".str_replace("'","",$cbo_report_name)."**0";
			}
			else
			{
				mysql_query("ROLLBACK"); 
				echo "7**".str_replace("'","",$cbo_report_name)."**1";
			}
		}
	...
	?>
		<table class="rpt_table" width="800" cellspacing="0" cellpadding="0" border="1" rules="all" id="tbl_report">
			...
			<tbody>
				<?
				$i=0; $button_status=0;
				$nameArray=sql_select( "select designation,company_id, name,template_id, sequence_no,activities,user_id,prepared_by from variable_settings_signature where report_id='$report_id' and company_id=0 and template_id in($template_id) and status_active=1 and is_deleted=0" );

            //    $sql= "select designation,company_id, name,template_id, sequence_no,activities,user_id,prepared_by from variable_settings_signature where report_id='$report_id' and company_id=0 and template_id in($template_id) and status_active=1 and is_deleted=0";
            //    echo $sql;die();
				...
				?>
					<tr class="general" id="row_1">
						...
						<td>
							<input type="text" name="txtSequenceNo[]" id="txtSequenceNo_1" class="text_boxes_numeric" value="1" style="width:80px;"/>
						</td>
						 <td>
				...
				else
				{
					...
					?>
                        ...
                        <td>
							...
							<td>
								<input type="text" name="txtSequenceNo[]" id="txtSequenceNo_<? echo $i; ?>" class="text_boxes_numeric" value="<? echo $row[csf('sequence_no')]; ?>" style="width:80px;"/>
							</td>
							 <td>
