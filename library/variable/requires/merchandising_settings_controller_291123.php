...
if ($action=="on_change_data")
{
	...
	if($type==27 || $type==57 || $type==58 || $type==84)//27 Commercial Cost Predefined Method-Pre-Costing, 57 Currier Cost Predefined Method, 58 Commercial Cost Predefined Method-Price Quotation, 84 Commercial Cost Predefined Method-QC
	{
		...
		$nameArray=sql_select("select id, commercial_cost_method, commercial_cost_percent, editable, tna_integrated as buyer_id, profit_calculative as brand_id, copy_quotation as based_on from variable_order_tracking where $variableCond order by id");
		//echo  "select id, commercial_cost_method, commercial_cost_percent, editable, tna_integrated as buyer_id, profit_calculative as brand_id from variable_order_tracking where $variableCond order by id";
...
if ($action=="save_update_delete_material_control")
{
	...
	if ($operation==0)  // Insert Here
	{
        ...
        {
            if (is_duplicate_field( "company_name", "variable_order_tracking", "company_name=$cbo_company_name_wo and variable_list=$cbo_variable_list_wo and item_category_id=".$item_category_id[$i]." and status_active=1 and is_deleted=0 " ) == 1)
            {
                echo 11; die;
            }
            else
...
if ($action=="save_update_delete")
{
   ...
   extract(check_magic_quote_gpc( $process ));
	...
	if ($operation==0)  // Insert Here
	{
		...
		else if(str_replace("'","", $cbo_variable_list_wo)==94)
		{
		...
		{
			$field_array="id, company_name, variable_list, commercial_cost_method, tna_integrated, profit_calculative, copy_quotation, editable, commercial_cost_percent, inserted_by, insert_date, status_active, is_deleted";
			$data_array ="(".$id.",". $cbo_company_name_wo.",". $cbo_variable_list_wo.",". $cbo_commercial_cost_method.",". $cbo_buyer_id.",". $cbo_brand_id.",". $cbo_based_on.",".$cbo_editable.",".$txt_commercial_cost_percent.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		}
	...
	else if ($operation==1)   // Update Here
	{
        ...
        {
            $field_array="company_name*variable_list*commercial_cost_method*tna_integrated*profit_calculative*copy_quotation*editable*commercial_cost_percent*updated_by*update_date";
            $data_array ="". $cbo_company_name_wo."*". $cbo_variable_list_wo."*". $cbo_commercial_cost_method."*". $cbo_buyer_id."*". $cbo_brand_id."*". $cbo_based_on."*".$cbo_editable."*'". $txt_commercial_cost_percent."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
            $rID=sql_update("variable_order_tracking", $field_array, $data_array,"id","".$update_id."",1);               
        }
        else if(str_replace("'","", $cbo_variable_list_wo)==31)
        {
