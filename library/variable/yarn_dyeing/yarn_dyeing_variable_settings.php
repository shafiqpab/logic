...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("yarn Dyeing Tracking Variable Settings", "../../../", 1, 1,$unicode,'','');
?>
...
var permission='<? echo $permission; ?>';
	...
	function fnc_production_variable_settings(operation)
	{
		...
		if (cbo_variable_list_production==1)
		{
			...
			http.open("POST","requires/yarn_dyeing_variable_settings_controller.php", true);
		...
		{
			var cbo_yd_last_process	= escape(document.getElementById('cbo_yd_last_process').value);
			var cbo_yd_process	= escape(document.getElementById('cbo_yd_process').value);
 			var data="action=save_update_delete&operation="+operation+get_submitted_data_string('cbo_company_name_production*cbo_variable_list_production*cbo_yd_last_process*cbo_yd_process*update_id',"../../../");
			//alert(data)
			//freeze_window(operation);
			http.open("POST","requires/yarn_dyeing_variable_settings_controller.php", true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	...
	function fnc_load_change_array(value)
	{
       load_drop_down( 'requires/yarn_dyeing_variable_settings_controller', value+'_'+2, 'load_drop_down_process', 'yarn_process' ) 
 	}
...
<body  onLoad="set_hotkey();">
	<div align="center" style="width:100%;">
        ...
        <fieldset style="width:850px;">
            <legend> Yarn Dyeing Variable Settings</legend>
            <form name="productionVariableSettings" id="productionVariableSettings" >
                    <table  width="850px" cellspacing="2" cellpadding="0" border="0">
                        <tr>
                            ...
                            <td width="300">
                                <?
                                    echo create_drop_down( "cbo_company_name_production", 250, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, '--- Select Company ---', 0, "show_list_view(document.getElementById('cbo_variable_list_production').value+'_'+this.value,'on_change_data','variable_settings_container','../yarn_dyeing/requires/yarn_dyeing_variable_settings_controller','');" );
                                ?>
                            </td>
                            <td width="150" align="center" class="must_entry_caption"><strong>Variable List</strong></td>
                            <td width="300">
                                <?
                                echo create_drop_down( "cbo_variable_list_production", 250, $yarn_dyeing_module_arr,'', '1', '---- Select ----', '',"show_list_view(this.value+'_'+document.getElementById('cbo_company_name_production').value,'on_change_data','variable_settings_container','../yarn_dyeing/requires/yarn_dyeing_variable_settings_controller','');",'','','','',''); //data, action, div, path, extra_func
                                ?>
                            </td>
                        </tr>
                    </table>
