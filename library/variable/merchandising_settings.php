...
function fnc_order_tracking_variable_settings( operation )
{
	...
	// alert(document.getElementById('cbo_variable_list_wo').value*1);return;
	var cbo_sales_year_started_date=""; var cbo_tna_integrated=""; var cbo_profit_calculative=""; var cbo_consumption_basis=""; var cbo_copy_quotation="";var cbo_short_booking="";
	...
	else if (document.getElementById('cbo_variable_list_wo').value*1==100)
	{
		
		if ( form_validation('cbo_company_name_wo*cbo_short_booking','Company Name*Short Bookoing')==0 )
		{
			return;
		}
		else
		{					
			nocache = Math.random();
			//var short_booking_td	= escape(document.getElementById('short_booking_td').innerHTML);
			eval(get_submitted_variables('cbo_company_name_wo*cbo_variable_list_wo*cbo_short_booking*update_id'));
			var data="action=save_update_delete_trims_booking&operation="+operation+get_submitted_data_string('cbo_company_name_wo*cbo_variable_list_wo*cbo_short_booking*update_id',"../../");
	...
	} // end cbo_variable_list_wo type 100
	else if (document.getElementById('cbo_variable_list_wo').value*1==101)
	{
		
		if ( form_validation('cbo_company_name_wo*cbo_color_update_afb','Company Name*Color Update After Batch')==0 )
		{
			return;
		}
		else
		{					
			nocache = Math.random();
			//var short_booking_td	= escape(document.getElementById('short_booking_td').innerHTML);
			eval(get_submitted_variables('cbo_company_name_wo*cbo_variable_list_wo*cbo_color_update_afb*update_id'));
			var data="action=save_update_delete_color_update_after_batch&operation="+operation+get_submitted_data_string('cbo_company_name_wo*cbo_variable_list_wo*cbo_color_update_afb*update_id',"../../");
	...
	} // end cbo_variable_list_wo type 101
	else if (document.getElementById('cbo_variable_list_wo').value*1==102)
	{
		
		if ( form_validation('cbo_company_name_wo*cbo_fabric_change_af_knitting','Company Name*Color Update After Batch')==0 )
		{
			return;
		}
		else
		{					
			nocache = Math.random();
			//var short_booking_td	= escape(document.getElementById('short_booking_td').innerHTML);
			eval(get_submitted_variables('cbo_company_name_wo*cbo_variable_list_wo*cbo_fabric_change_af_knitting*update_id'));
			var data="action=save_update_delete_color_update_fabric_change_af_knitting&operation="+operation+get_submitted_data_string('cbo_company_name_wo*cbo_variable_list_wo*cbo_fabric_change_af_knitting*update_id',"../../");
	...
	} // end cbo_variable_list_wo type 102
	...
	else if (document.getElementById('cbo_variable_list_wo').value*1==19 || document.getElementById('cbo_variable_list_wo').value*1==103)
	{
	...
	else if (document.getElementById('cbo_variable_list_wo').value*1==27 || document.getElementById('cbo_variable_list_wo').value*1==57 || document.getElementById('cbo_variable_list_wo').value*1==58 || document.getElementById('cbo_variable_list_wo').value*1==84)
	{
		...
		{					
			...
			var cbo_buyer_id=0; var cbo_brand_id=0; var cbo_based_on=0; var cbo_compulsory	=0;
			if(document.getElementById('cbo_variable_list_wo').value*1==27 || document.getElementById('cbo_variable_list_wo').value*1==57)//ISD-23-12374
			{
				var cbo_buyer_id	= $("#cbo_buyer_id").val();
				var cbo_brand_id	= $("#cbo_brand_id").val();
				var cbo_based_on	= $("#cbo_based_on").val();
				if(document.getElementById('cbo_variable_list_wo').value*1==27 )var cbo_compulsory	= $("#cbo_compulsory").val()*1;
			}
			
			//eval(get_submitted_variables('cbo_company_name_wo*cbo_variable_list_wo*update_id'));
			var data="action=save_update_delete&operation="+operation+"&sales_year_started="+cbo_commercial_cost_method_td+'&cbo_commercial_cost_method='+cbo_commercial_cost_method+'&txt_commercial_cost_percent='+txt_commercial_cost_percent+'&cbo_editable='+cbo_editable+'&cbo_buyer_id='+cbo_buyer_id+'&cbo_brand_id='+cbo_brand_id+'&cbo_compulsory='+cbo_compulsory+'&cbo_based_on='+cbo_based_on+get_submitted_data_string('cbo_company_name_wo*cbo_variable_list_wo*update_id',"../../");
			freeze_window(operation);
			http.open("POST","requires/merchandising_settings_controller.php", true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
	...
	else if (document.getElementById('cbo_variable_list_wo').value*1==31)
	{
		...
		else
		{
			...
			eval(get_submitted_variables('cbo_company_name_wo*cbo_variable_list_wo*update_id'));
			var data="action=save_update_delete&operation="+operation+"&tna_process_type="+txt_tna_process_type+get_submitted_data_string('cbo_company_name_wo*cbo_variable_list_wo*update_id*cbo_based_on',"../../");
	...
	else if (document.getElementById('cbo_variable_list_wo').value*1==74)
	{
		...
		var auto_acknowledge = $("#cbo_auto_acknowledge").val()*1;
		...
		{				
			...
			var cbo_auto_acknowledge = $("#cbo_auto_acknowledge").val();
			eval(get_submitted_variables('cbo_company_name_wo*cbo_variable_list_wo*update_id'));
			var data="action=save_update_delete&operation="+operation+"&txt_without_emblish="+txt_without_emblish+"&txt_with_emblish="+txt_with_emblish+"&cbo_auto_acknowledge="+cbo_auto_acknowledge+get_submitted_data_string('cbo_company_name_wo*cbo_variable_list_wo*update_id',"../../");
	...
	else if (document.getElementById('cbo_variable_list_wo').value*1==94 || document.getElementById('cbo_variable_list_wo').value*1==104) //cbo_po_current_date
	{
	...
	else if (document.getElementById('cbo_variable_list_wo').value*1==97)
	{
		if ( form_validation('cbo_company_name_wo','Company Name')==0 )
		{
			return;
		...
		{					
			nocache = Math.random();
			var data="action=save_update_delete_color_sensivity&operation="+operation+get_submitted_data_string('cbo_company_name_wo*cbo_variable_list_wo*cbo_color_sensivity*cbo_editable_id*update_id',"../../");
			//alert(data);
			freeze_window(operation);
			http.open("POST","requires/merchandising_settings_controller.php", true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	...
	else if (document.getElementById('cbo_variable_list_wo').value*1==98) // Consumption-Required Calculation
	{
		if ( form_validation('cbo_company_name_wo*cbo_required_calculation_method','Company Name*Thread Consumption Calculation Method')==0 )
		{
			return;
		}
		else
		{				
			...
			var cbo_required_calculation_method = $("#cbo_required_calculation_method").val();
			var cbo_thread_length_calculation_method = $("#cbo_thread_length_calculation_method").val();
			//alert(cbo_thread_consumption_calculation_method);return;
			
			var data="action=save_update_delete&operation="+operation+"&cbo_thread_length_calculation_method="+cbo_thread_length_calculation_method+"&cbo_required_calculation_method="+cbo_required_calculation_method+get_submitted_data_string('cbo_company_name_wo*cbo_variable_list_wo*update_id',"../../");
			//alert(data);
			freeze_window(operation);
			http.open("POST","requires/merchandising_settings_controller.php", true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	...
	else if (document.getElementById('cbo_variable_list_wo').value*1==99)
	{
		if ( form_validation('cbo_company_name_wo*cbo_percentage_calculation','Company Name*Percentage Calculation')==0 )
		{
			return;
		}
		else
		{					
			nocache = Math.random();
			var percentage_calculation	= $("#cbo_percentage_calculation").val();
			eval(get_submitted_variables('cbo_company_name_wo*cbo_variable_list_wo*update_id'));
			var data="action=save_update_delete&operation="+operation+"&percentage_calculation="+percentage_calculation+get_submitted_data_string('cbo_company_name_wo*cbo_variable_list_wo*update_id',"../../");
			freeze_window(operation);
			http.open("POST","requires/merchandising_settings_controller.php", true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
...
function fnc_order_tracking_variable_settings_reponse()
{
	if(http.readyState == 4) 
	{
		...
		if(reponse[0]!=10)
		{
			reset_form('ordertrackingvariablesettings_1','variable_settings_container*buyer_brand_wise_commercial_currier_container','');
		}
		release_freezing();
	}
}	
...
function fn_show_details()
{
	
	var company_name_wo=$('#cbo_company_name_wo').val();
	var variable_list_wo=$('#cbo_variable_list_wo').val();
	var garments_nature=$('#garments_nature').val();
 
	
	if(variable_list_wo==27 || variable_list_wo==57)
	{
		show_list_view(variable_list_wo+'_'+company_name_wo+'_'+garments_nature,'on_change_data','variable_settings_container','requires/merchandising_settings_controller','');
		show_list_view(variable_list_wo+'_'+company_name_wo+'_'+garments_nature,'buyer_brand_wise_commercial_currier_list_view','buyer_brand_wise_commercial_currier_container','requires/merchandising_settings_controller','');
	}
	else
	{
		show_list_view(variable_list_wo+'_'+company_name_wo+'_'+garments_nature,'on_change_data','variable_settings_container','requires/merchandising_settings_controller','');
...
function fnc_valuetype(val)
{
	if(val==2) $("#tdvaltype").text("Fix Amount"); else $("#tdvaltype").text("Percent");
}
		...
		<form name="ordertrackingvariablesettings_1" id="ordertrackingvariablesettings_1" autocomplete="off">	
            <table width="750" cellspacing="2" cellpadding="0" border="0">
                <tr>
                    ...
                    <td width="250"><? echo create_drop_down( "cbo_company_name_wo", 250, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name", 'id,company_name', 1, '--- Select Company ---', '', 'fn_show_details();', '' ); ?></td>
                    <td width="200" align="center">Variable List</td>
                    <td width="250" class="must_entry_caption">
                        <? asort($order_tracking_module);
                            echo create_drop_down( "cbo_variable_list_wo", 250, $order_tracking_module,'', '1', '---- Select ----', '','fn_show_details();',''); ?>
                    </td>
                </tr>
            </table>
            <div style="width:895px; float:left; min-height:40px; margin:auto" align="center" id="variable_settings_container"></div>
            
            <div id="buyer_brand_wise_commercial_currier_container"></div>
		</form>	
	</fieldset>
    </div>
 </body>
