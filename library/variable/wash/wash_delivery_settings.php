...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Wash Variable Settings", "../../../", 1, 1,$unicode,'','');
?>
...
var permission='<? echo $permission; ?>';
	...
	function fnc_delivery_variable_settings(operation)
	{
		...
		if (cbo_variable_list_delivery==1)
		{
			...
			http.open("POST","requires/wash_delivery_settings_controller.php", true);
		...
		else if(cbo_variable_list_delivery==4)
		{
...
<body  onLoad="set_hotkey();">
	<div align="center" style="width:100%;">
        ...
        <fieldset style="width:850px;">
            <legend>Wash Delivery Variable Settings</legend>
            <form name="deliveryVariableSettings" id="deliveryVariableSettings" >
                    <table  width="850px" cellspacing="2" cellpadding="0" border="0">
                        <tr>
                            ...
                            <td width="300">
                                <?
                                    echo create_drop_down( "cbo_company_name_delivery", 250, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, '--- Select Company ---', 0, "show_list_view(document.getElementById('cbo_variable_list_delivery').value+'_'+this.value,'on_change_data','variable_settings_container','../wash/requires/wash_delivery_settings_controller','');" );
                                ?>
                            </td>
                            <td width="150" align="center" class="must_entry_caption">Variable List</td>
                            <td width="300">
                                <?
                                $wash_delivery_module = array(1 =>"Wash Delivery",2 =>"Wash Production",3 =>"Wash Bill. Qty. Control",4 =>"Wash Material Rec. Gmt.");
                                //echo create_drop_down( "cbo_variable_list_delivery", 250, $wash_delivery_module,'', 1, '--- Select ---', 0, "" );


                               echo create_drop_down( "cbo_variable_list_delivery", 250, $wash_delivery_module,'', 1, '---- Select ----', 0,"show_list_view(this.value+'_'+document.getElementById('cbo_company_name_delivery').value,'on_change_data','variable_settings_container','../wash/requires/wash_delivery_settings_controller','');",'','','','',''); //data, action, div, path, extra_func
                                ?>
                            </td>
                        </tr>
                    </table>
