...
echo load_html_head_contents("Item Creation Info", "../../", 1, 1,$unicode,1,1);
...
<script>
	...
	function openmypage_subgroup()
	{
		...
		else
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/item_creation_controller.php?category='+category+'&item_group_id='+item_group_id+'&item_group_name='+item_group_name+'&action=sub_group_popup','Search Group Code', 'width=800px,height=400px,center=1,resize=0,scrolling=0','../')
			emailwindow.onclose=function()
			{
	...
	function fn_company()
	{
		...
		else
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/item_creation_controller.php?cbo_company_name='+cbo_company_name+'&txt_copy_to_company_id='+txt_copy_to_company_id+'&action=company_popup','Copy Company', 'width=400px,height=400px,center=1,resize=0,scrolling=0','../')
			emailwindow.onclose=function()
			{
	...
	function fn_subprocess()
	{
		...
		else
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/item_creation_controller.php?cbo_company_name='+cbo_company_name+'&txt_subprocess_id='+txt_subprocess_id+'&action=subprocess_popup','Subprocess Popup', 'width=400px,height=400px,center=1,resize=0,scrolling=0','../')
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
				var party_id=this.contentDoc.getElementById("hidden_party_id").value;	 //Access form field with id="emailfield"
...
<div align="center">
	<form name="excelImport_1" id="excelImport_1" action="item_creation_import_excel.php" enctype="multipart/form-data" method="post">
    	<p style="font-size: 18px; text-align:center; color: red; font-weight:bold;">Excel Upload is not ready For Dyes Chemical Category.</p>
			...
			<table cellpadding="3" cellspacing="3">
	            <tr>
	                ...
	                <td width="170"><input type="hidden" name="set_id" id="set_id" >                                 
						<?
							echo create_drop_down( "cbo_item_category", 155, $item_category,"", "1", "--- Select---", 0, "fn_list_show(this.value,$('#cbo_company_name').val(),'0');","","$item_cat_cond","","","1,2,3,12,13,14,24,25,28,30,31,42,43,71,72,73,74,75,76,77,78,79,80,81,82,83,86,95,96,98,100,101,102,103,104,105,108,109,112" );
	            ...
	            <tr>
	                ...
	                <td> 
	                <Input type="text" name="txt_subgroup_code" id="txt_subgroup_code" style="width:145px" class="text_boxes" onDblClick="openmypage_subgroup()" placeholder="Browse" readonly />
	                <input type="hidden" id="item_sub_group_id" name="item_sub_group_id" />
	                </td>
	                <td align="right">Sub Group Name</td>
	                <td><Input type="text" name="txt_subgroup_name" id="txt_subgroup_name"  style="width:145px" class="text_boxes" placeholder="Display" readonly></td>
	            ...
	            <tr>
	                ...
	                <td>
						<?
							echo create_drop_down( "cbo_order_uom", 100, $unit_of_measurement,"", "1", "--- Select---", 0, "","0" );
	            ...
	            <tr>
	                ...
	                <td id="cons_td"> 
						<?
							echo create_drop_down( "cbo_cons_uom", 100, $unit_of_measurement,"", "1", "--- Select---", 0, "","0" );
	                    ?>&nbsp;<span style="color:red">Lower UOM</span>
	                </td>
	                <td align="right" class="must_entry_caption">Conversion Factor</td>
	                <td><Input type="text" name="txt_conversion_factor" id="txt_conversion_factor" style="width:145px" class="text_boxes_numeric"></td>
	            ...
	            <tr>
					...
					<td><? echo create_drop_down( "cbo_ordUOMDecPlaceQnt", 145, $dec_place_other_item, "", 1, 0, 0, "" ); ?></td>
                    <td align="right">Ord UOM Dec Place Rate</td>
					<td><? echo create_drop_down( "cbo_ordUOMDecPlaceRate", 145, $dec_place_other_item, "", 1, 0, 0, "" ); ?></td>
					<td align="right">Ord UOM Dec Place Amt</td>
					<td><? echo create_drop_down( "cbo_ordUOMDecPlaceAmt", 145, $dec_place_other_item, "", 1, 0, 0, "" ); ?></td>
	            </tr>
	            <tr>
	            	<td align="right">Cons UOM Dec Place Qnt</td>
					<td><? echo create_drop_down( "cbo_consUOMDecPlaceQnt", 145, $dec_place_other_item, "", 1, 0, 0, "" ); ?></td>
                    <td align="right">Cons UOM Dec Place Rate</td>
					<td><? echo create_drop_down( "cbo_consUOMDecPlaceRate", 145, $dec_place_other_item, "", 1, 0, 0, "" ); ?></td>
					<td align="right">Cons UOM Dec Place Amt</td>
					<td><? echo create_drop_down( "cbo_consUOMDecPlaceAmt", 145, $dec_place_other_item, "", 1, 0, 0, "" ); ?></td>					
	            </tr>
	            ...
	            <tr>
	                ...
	                <td>
						<input type="text" name="txt_subprocess" id="txt_subprocess" style="width:145px" class="text_boxes" onDblClick="fn_subprocess()" placeholder="browse" readonly disabled />
	                    <input type="hidden" name="txt_subprocess_id" id="txt_subprocess_id" />
					</td>
	                ...
	                <td>
						<input type="text" name="txt_copy_to_company" id="txt_copy_to_company" style="width:145px" class="text_boxes" onDblClick="fn_company()" placeholder="browse" readonly />
	                    <input type="hidden" name="txt_copy_to_company_id" id="txt_copy_to_company_id" />
					</td>					
	            </tr>
                <tr>
					...
					<td align="right">Use for</td>
	                <td>
						<? $use_for=array(1=>"Lab",2=>"Machine Wash",3=>" Finishing Recipe");
							echo create_drop_down( "cbo_use_for", 155, $use_for,'', 0, '--Select--', 1 );            
						?>
	                </td>
	            </tr>
	            <tr>
...
<script>
	...
	set_multiselect('cbo_use_for','0','0','','__set_buyer_status__../item_details/requires/item_creation_controller');
