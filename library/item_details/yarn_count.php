...
function fnc_yarn_count_info( operation )
{
	...
	{
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_yarn_count*cbo_status*txt_sequence*update_id',"../../");
		
		freeze_window(operation);
		 
		http.open("POST","requires/yarn_count_controller.php",true);
...
function fnc_yarn_count_info_reponse()
{
	if(http.readyState == 4) 
	{
		...
		else
		{
			...
			show_msg(reponse[0]);
			show_list_view(reponse[1],'search_list_view','yarn_count_list_view','../item_details/requires/yarn_count_controller','setFilterGrid("list_view",-1)');
		...
		<form name="yarncountinfo_1" id="yarncountinfo_1">	
			<table cellpadding="0" cellspacing="2" width="100%">
			 	<tr>
					...
					<td colspan="3">
						<input type="text" name="txt_yarn_count" id="txt_yarn_count" class="text_boxes" style="width:245px" maxlength="15" title="Maximum 15 Character" />						
					</td>
				</tr>	
				<tr>
