	...
	<div align="center" style="width:780px" >
		<fieldset style="width:780px">
			<form name="order_popup_1"  id="order_popup_1">
				...
				echo  create_list_view ( "list_view", "Item Catagory,Item Group Name,Sub Group Code,Sub Group Name", "200,200,100","750","320",0, $sql, "js_set_value", "id,sub_group_code,sub_group_name", "", 1, "item_category_id,item_group_id,0,0", $arr , "item_category_id,item_group_id,sub_group_code,sub_group_name", "item_creation_controller", 'setFilterGrid("list_view",-1);','0,0,0,0' );
				?>
			<input type="hidden" id="item_id" />
			</form>
		</fieldset>
...
if ($action=="load_php_data_to_form")
{
	...
	*/
	...
	$nameArray = sql_select("SELECT use_for from use_for_lab_machine_finishing where prod_id=$data");
	$use_for_data = "";
	foreach ($nameArray as $row) {
		$use_for_data .= $row[csf("use_for")] . ",";
	}
	$use_for_data = rtrim($use_for_data, ",");
	echo "set_multiselect('cbo_use_for','0','1','".($use_for_data)."','__set_buyer_status__../item_details/requires/item_creation_controller');\n"; 
}
...
if ($action=="save_update_delete")
{
	...
	if($txt_copy_to_company_id!="") $txt_copy_to_company_id=$txt_copy_to_company_id.",".str_replace("'","",$cbo_company_name); 
	//else $txt_copy_to_company_id=str_replace("'","",$cbo_company_name);
	...
	if ($operation==0)  // Insert Here=======================================================
	{
		...
		// echo "10**".$cbo_use_for;disconnect($con);die;
		
		/*==============================================================================/
		/			Item creation for all company when checkbox checked					/
		/============================================================================= */
		// echo "10**$copy_item";die();
		//echo "11** gg $txt_copy_to_company_id $duplicate_cond";die;
		if($txt_copy_to_company_id != "")
		{
			...
			if($db_type==2)
			{
				...
				if(str_replace("'","",$txt_model_name)=='') $duplicate_cond .=" and model is null"; else $duplicate_cond.=" and model=$txt_model_name";
				...
				if(str_replace("'","",$cbo_order_uom)=='') $duplicate_cond .=" and order_uom is null"; else $duplicate_cond.=" and order_uom=$cbo_order_uom";
				
				 $duplicate_item= is_duplicate_field("id","product_details_master","company_id in($company_ids) and item_category_id=$cbo_item_category and item_group_id=$item_group_id  $duplicate_cond and is_deleted=0 and entry_form <> 24");
			}
			else
			{
				$duplicate = is_duplicate_field("id","product_details_master","company_id in($company_ids) and item_category_id=$cbo_item_category and item_group_id=$item_group_id and sub_group_name=$txt_subgroup_name and item_description=$txt_description and item_size=$txt_item_size and model=$txt_model_name and is_deleted=0 and entry_form <> 24");
				$duplicate_item= is_duplicate_field("id","product_details_master","company_id in($company_ids) and item_category_id=$cbo_item_category and item_group_id=$item_group_id and sub_group_name=$txt_subgroup_name and item_number=$txt_item_no and item_code=$txt_item_code and is_deleted=0 and entry_form <> 24");
			}
			
			if(str_replace("'","",$txt_item_no)=='')  $duplicate_item='';
			if($duplicate==1)
			{
				echo "11**Duplicate Product is Not Allow.";
				disconnect($con);
				die;
			}
			if($duplicate_item==1)
			{
				echo "11**Duplicate Item Number is Not Allow.";
				disconnect($con);
				die;
			}

			$item_group_arr=return_library_array( "select id,item_name from lib_item_group",'id','item_name');
			$productname = $item_group_arr[str_replace("'", '', $item_group_id)]." ".str_replace("'", '', $txt_description);
			if(str_replace("'", '', $txt_item_size)!="") $productname .=" ".str_replace("'", '', $txt_item_size);

			$id = return_next_id_by_sequence("PRODUCT_DETAILS_MASTER_PK_SEQ", "product_details_master", $con);

			$field_array="id,company_id,item_category_id,entry_form,item_group_id,item_sub_group_id,sub_group_code,sub_group_name,item_code,item_description,product_name_details,item_size,re_order_label,minimum_label,maximum_label,unit_of_measure,order_uom,conversion_factor,item_account,item_number,inserted_by,insert_date,status_active,brand_name,model,origin,fixed_asset,order_uom_decimal_point_qty,order_uom_decimal_point,order_uom_decimal_point_amt,cons_uom_decimal_point_qty,cons_uom_decimal_point,cons_uom_decimal_point_amt,subprocess_id,is_deleted";

			$txt_description=htmlentities($txt_description);
			$data_array="";$j=0;
			...
			{ //txt_subprocess_id
				...
				if($j==0)
				{
					$data_array="(".$id.",".$com_id.",".$cbo_item_category.",".$entry_form_lib.",".$item_group_id.",".$item_sub_group_id.",".$txt_subgroup_code.",".$txt_subgroup_name.",".$txt_item_code.",".$txt_description.",'".$productname."',".$txt_item_size.",".$txt_reorder_label.",".$txt_min_label.",".$txt_max_label.",".$cbo_cons_uom.",".$cbo_order_uom.",".$txt_conversion_factor.",".$txt_item_account.",".$txt_item_no.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_status.",".$txt_brand.",".$txt_model_name.",".$cbo_origin.",".$cbo_fixed_asset.",".$cbo_ordUOMDecPlaceQnt.",".$cbo_ordUOMDecPlaceRate.",".$cbo_ordUOMDecPlaceAmt.",".$cbo_consUOMDecPlaceRate.",".$cbo_consUOMDecPlaceQnt.",".$cbo_consUOMDecPlaceAmt.",".$txt_subprocess_id.",0)";
				}
				else 
				{
					$data_array .=",(".$id.",".$com_id.",".$cbo_item_category.",".$entry_form_lib.",".$item_group_id.",".$item_sub_group_id.",".$txt_subgroup_code.",".$txt_subgroup_name.",".$txt_item_code.",".$txt_description.",'".$productname."',".$txt_item_size.",".$txt_reorder_label.",".$txt_min_label.",".$txt_max_label.",".$cbo_cons_uom.",".$cbo_order_uom.",".$txt_conversion_factor.",".$txt_item_account.",".$txt_item_no.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_status.",".$txt_brand.",".$txt_model_name.",".$cbo_origin.",".$cbo_fixed_asset.",".$cbo_ordUOMDecPlaceQnt.",".$cbo_ordUOMDecPlaceRate.",".$cbo_ordUOMDecPlaceAmt.",".$cbo_consUOMDecPlaceQnt.",".$cbo_consUOMDecPlaceRate.",".$cbo_consUOMDecPlaceAmt.",".$txt_subprocess_id.",0)";
				}
 				$j++;
			}

			$cbo_use_for=explode(',',str_replace("'","",$cbo_use_for));
			for($i=0; $i<count($cbo_use_for); $i++)
			{
				if($use_for_id=="") $use_for_id=return_next_id( "id", "use_for_lab_machine_finishing", 1 ); else $use_for_id=$use_for_id+1;
				if($i==0) $add_comma=""; else $add_comma=",";
				$data_array2.="$add_comma(".$use_for_id.",".$id.",".$cbo_use_for[$i].")";
			}
			$field_array2="id, prod_id, use_for";

			$rID=sql_insert("product_details_master",$field_array,$data_array,0);
			$rID4=sql_insert("use_for_lab_machine_finishing",$field_array2,$data_array2,1);
			if($db_type==0)
			{
				if($rID && $rID2)
				{
		...
		else
		{
			if($db_type==2)
			{
				...
				$duplicate_item= is_duplicate_field("id","product_details_master","company_id=$cbo_company_name and item_category_id=$cbo_item_category and item_group_id=$item_group_id and order_uom=$cbo_order_uom $duplicate_cond and is_deleted=0 and entry_form <> 24");
				//ORDER UOM ADDED BY JAHID HASAN
			}
			else
			{

				$duplicate = is_duplicate_field("id","product_details_master","company_id=$cbo_company_name and item_category_id=$cbo_item_category and item_group_id=$item_group_id and sub_group_name=$txt_subgroup_name and item_description=$txt_description and item_size=$txt_item_size and model=$txt_model_name and is_deleted=0 and entry_form <> 24");
				$duplicate_item= is_duplicate_field("id","product_details_master","company_id=$cbo_company_name and item_category_id=$cbo_item_category and item_group_id=$item_group_id and sub_group_name=$txt_subgroup_name and item_number=$txt_item_no and item_code=$txt_item_code and is_deleted=0 and entry_form <> 24");
			}
			
			//if(str_replace("'","",$txt_item_no)=='')  $duplicate_item=''; // COMMENTED BY JAHID HASAN
			if($duplicate==1)
			{
			...
			if($duplicate_item==1)
			{
				echo "11**Duplicate Item Number is Not Allow in Same Return Number.";
				disconnect($con);
				die;
			}
			...
			$field_array="id,company_id,item_category_id,entry_form,item_group_id,item_sub_group_id,sub_group_code,sub_group_name,item_code,item_description,product_name_details,item_size,re_order_label,minimum_label,maximum_label,unit_of_measure,order_uom,conversion_factor,item_account,item_number,inserted_by,insert_date,status_active,brand_name,model,origin,fixed_asset,order_uom_decimal_point_qty,order_uom_decimal_point,order_uom_decimal_point_amt,cons_uom_decimal_point_qty,cons_uom_decimal_point,cons_uom_decimal_point_amt,subprocess_id,is_deleted";
			$data_array="(".$id.",".$cbo_company_name.",".$cbo_item_category.",".$entry_form_lib.",".$item_group_id.",".$item_sub_group_id.",".$txt_subgroup_code.",".$txt_subgroup_name.",".$txt_item_code.",".$txt_description.",'".$productname."',".$txt_item_size.",".$txt_reorder_label.",".$txt_min_label.",".$txt_max_label.",".$cbo_cons_uom.",".$cbo_order_uom.",".$txt_conversion_factor.",".$txt_item_account.",".$txt_item_no.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_status.",".$txt_brand.",".$txt_model_name.",".$cbo_origin.",".$cbo_fixed_asset.",".$cbo_ordUOMDecPlaceQnt.",".$cbo_ordUOMDecPlaceRate.",".$cbo_ordUOMDecPlaceAmt.",".$cbo_consUOMDecPlaceQnt.",".$cbo_consUOMDecPlaceRate.",".$cbo_consUOMDecPlaceAmt.",".$txt_subprocess_id.",0)";
			//echo $data_array;die;

			if($cbo_use_for !='1' || $cbo_use_for !='2' || $cbo_use_for !='3'){
				
				if($use_for_id=="") $use_for_id=return_next_id( "id", "use_for_lab_machine_finishing", 1 ); else 
				$use_for_id=$use_for_id+1;
				if($i==0) $add_comma=""; else $add_comma=",";
				$data_array2.="$add_comma(".$use_for_id.",".$id.",0)";
			...
			$rID2=sql_insert("use_for_lab_machine_finishing",$field_array2,$data_array2,1);
			if($db_type==0)
			{
				if($rID )
				{
	...
	else if ($operation==1)   // Update Here==========================================================================================
	{
		...
		if($db_type==2)
		{
			...
			if(str_replace("'","",$txt_item_size)=='') $duplicate_cond .=" and item_size is null"; else $duplicate_cond.=" and item_size=$txt_item_size";
			if(str_replace("'","",$txt_model_name)=='') $duplicate_cond .=" and model is null"; else $duplicate_cond.=" and model=$txt_model_name";
			//$duplicate = is_duplicate_field("id","product_details_master","company_id=$cbo_company_name and item_category_id=$cbo_item_category and item_group_id=$item_group_id and id<>$update_id $duplicate_cond and is_deleted=0 and entry_form <> 24");
			//$duplicate_cond='';
			if(str_replace("'","",$txt_subgroup_name)=='') $duplicate_cond.=" and sub_group_name is null"; else $duplicate_cond.=" and sub_group_name=$txt_subgroup_name";
			if(str_replace("'","",$txt_item_no)=='') $duplicate_cond .=" and item_number is null"; else $duplicate_cond.=" and item_number=$txt_item_no";
			if(str_replace("'","",$txt_item_code)=='') $duplicate_cond .=" and item_code is null"; else $duplicate_cond.=" and item_code=$txt_item_code";
			...
			if(str_replace("'","",$cbo_item_category)==5 || str_replace("'","",$cbo_item_category)==6 || str_replace("'","",$cbo_item_category)==7 || str_replace("'","",$cbo_item_category)==23)
			{
				$update_id_sql="select ID from product_details_master where company_id=$cbo_company_name and item_category_id=$cbo_item_category and item_group_id=$item_group_id $duplicate_cond and is_deleted=0 and entry_form <> 24";
				//echo "10**".$update_id_sql;die;
				$update_id_sql_result=sql_select($update_id_sql);
				$all_up_id=array();
				foreach($update_id_sql_result as $val)
				{
					$all_up_id[$val["ID"]]=$val["ID"];
				}
				$duplicate_item= is_duplicate_field("id","product_details_master","company_id=$cbo_company_name and item_category_id=$cbo_item_category and item_group_id=$item_group_id and order_uom=$cbo_order_uom $duplicate_cond and is_deleted=0 and id not in(".implode(",",$all_up_id).") and entry_form <> 24");			

			}
			else
			{
				
				$duplicate_item= is_duplicate_field("id","product_details_master","company_id=$cbo_company_name and item_category_id=$cbo_item_category and item_group_id=$item_group_id and order_uom=$cbo_order_uom $duplicate_cond and is_deleted=0 and id<>$update_id and entry_form <> 24");
			}
		...
		else
		{
			...
			//$duplicate = is_duplicate_field("id","product_details_master","company_id=$cbo_company_name and item_category_id=$cbo_item_category and item_group_id=$item_group_id and id<>$update_id and sub_group_name=$txt_subgroup_name and item_description=$txt_description and item_size=$txt_item_size and model=$txt_model_name and is_deleted=0 and entry_form <> 24");
			$duplicate_item= is_duplicate_field("id","product_details_master","company_id=$cbo_company_name and item_category_id=$cbo_item_category and item_group_id=$item_group_id and id<>$update_id and sub_group_name=$txt_subgroup_name and item_description=$txt_description and item_size=$txt_item_size and model=$txt_model_name and is_deleted=0 and item_number=$txt_item_no and item_code=$txt_item_code and is_deleted=0 and id<>$update_id and entry_form <> 24");
		}
		...
		if($duplicate_item==1)
		{
			echo "11**Duplicate Product is Not Allows.";
			disconnect($con);
			die;
		}
		...
		if(str_replace("'", '',$cbo_status)!=1)
		{
			if(str_replace("'","",$cbo_item_category)==5 || str_replace("'","",$cbo_item_category)==6 || str_replace("'","",$cbo_item_category)==7 || str_replace("'","",$cbo_item_category)==23)
			{
				$prod_stock=return_field_value("current_stock","product_details_master","id in(".implode(",",$all_up_id).") and status_active=1 and is_deleted=0","current_stock");
			}
			else
			{
				$prod_stock=return_field_value("current_stock","product_details_master","id=".$update_id." and status_active=1 and is_deleted=0","current_stock");
			}
		...
		$item_status=str_replace("'", '',$cbo_status);
		if($item_status==2)
		{
			if(str_replace("'","",$cbo_item_category)==5 || str_replace("'","",$cbo_item_category)==6 || str_replace("'","",$cbo_item_category)==7 || str_replace("'","",$cbo_item_category)==23)
			{
				$item_transaction_check=return_field_value("id","inv_transaction","id in(".implode(",",$all_up_id).") and status_active=1 and is_deleted=0","id");
			}
			else
			{
				$item_transaction_check=return_field_value("id","inv_transaction","id=".$update_id." and status_active=1 and is_deleted=0","id");
			}
			if($item_transaction_check>0)
			{
				echo "11**Transaction Found, Update Not Allow.";
				disconnect($con);die;
			}
		}
		...
		if($com_copy_id !="")
		{
			$prev_item_sql="select sub_group_name, item_description, item_size, model, item_number, item_code from product_details_master where id=$update_id";
			$prev_item_sql_result=sql_select($prev_item_sql);
			$com_subgroup_name=$prev_item_sql_result[0][csf("sub_group_name")];
			$com_description=$prev_item_sql_result[0][csf("item_description")];
			$com_item_size=$prev_item_sql_result[0][csf("item_size")];
			$com_model=$prev_item_sql_result[0][csf("model")];
			$com_item_number=$prev_item_sql_result[0][csf("item_number")];
			$com_item_code=$prev_item_sql_result[0][csf("item_code")];
			
			if(trim(str_replace("'","",$txt_subgroup_name)) != trim($com_subgroup_name) || trim(str_replace("'","",$txt_description)) != trim($com_description) || trim(str_replace("'","",$txt_item_size)) != trim($com_item_size) || trim(str_replace("'","",$txt_model_name)) != trim($com_model) || trim(str_replace("'","",$txt_item_no)) != trim($com_item_number) || trim(str_replace("'","",$txt_item_code)) != trim($com_item_code))
			{
		...
		if($txt_copy_to_company_id !="")
		{
			...
			if(str_replace("'","",$txt_item_size)=='') $duplicate_cond .=" and item_size is null"; else $duplicate_cond.=" and item_size=$txt_item_size";
			if(str_replace("'","",$txt_model_name)=='') $duplicate_cond .=" and model is null"; else $duplicate_cond.=" and model=$txt_model_name";
			$another_com_item_sql="select company_id from product_details_master where item_category_id=$cbo_item_category and item_group_id=$item_group_id and is_deleted=0 and id<>$update_id $duplicate_cond ";
			...
			//$company_ids = $txt_copy_to_company_id;
			//$company_id_array = explode(",",$txt_copy_to_company_id);
			$field_array_insert="id,company_id,item_category_id,entry_form,item_group_id,item_sub_group_id,sub_group_code,sub_group_name,item_code,item_description,product_name_details,item_size,re_order_label,minimum_label,maximum_label,unit_of_measure,order_uom,conversion_factor,item_account,item_number,inserted_by,insert_date,status_active,brand_name,model,origin,fixed_asset,order_uom_decimal_point_qty,order_uom_decimal_point,order_uom_decimal_point_amt,cons_uom_decimal_point_qty,cons_uom_decimal_point,cons_uom_decimal_point_amt,subprocess_id,is_deleted";
			$j=0;
			foreach($company_id_array as $com_id=>$val_com_id)
			{
				$id = return_next_id_by_sequence("PRODUCT_DETAILS_MASTER_PK_SEQ", "product_details_master", $con);
				if($j==0)
				{
					$data_array_insert="(".$id.",".$com_id.",".$cbo_item_category.",".$entry_form_lib.",".$item_group_id.",".$item_sub_group_id.",".$txt_subgroup_code.",".$txt_subgroup_name.",".$txt_item_code.",".$txt_description.",'".$productname."',".$txt_item_size.",".$txt_reorder_label.",".$txt_min_label.",".$txt_max_label.",".$cbo_cons_uom.",".$cbo_order_uom.",".$txt_conversion_factor.",".$txt_item_account.",".$txt_item_no.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_status.",".$txt_brand.",".$txt_model_name.",".$cbo_origin.",".$cbo_fixed_asset.",".$cbo_ordUOMDecPlaceQnt.",".$cbo_ordUOMDecPlaceRate.",".$cbo_ordUOMDecPlaceAmt.",".$cbo_consUOMDecPlaceQnt.",".$cbo_consUOMDecPlaceRate.",".$cbo_consUOMDecPlaceAmt.",".$txt_subprocess_id.",0)";
				}
				else 
				{
					$data_array_insert .=",(".$id.",".$com_id.",".$cbo_item_category.",".$entry_form_lib.",".$item_group_id.",".$item_sub_group_id.",".$txt_subgroup_code.",".$txt_subgroup_name.",".$txt_item_code.",".$txt_description.",'".$productname."',".$txt_item_size.",".$txt_reorder_label.",".$txt_min_label.",".$txt_max_label.",".$cbo_cons_uom.",".$cbo_order_uom.",".$txt_conversion_factor.",".$txt_item_account.",".$txt_item_no.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_status.",".$txt_brand.",".$txt_model_name.",".$cbo_origin.",".$cbo_fixed_asset.",".$cbo_ordUOMDecPlaceQnt.",".$cbo_ordUOMDecPlaceRate.",".$cbo_ordUOMDecPlaceAmt.",".$cbo_consUOMDecPlaceQnt.",".$cbo_consUOMDecPlaceRate.",".$cbo_consUOMDecPlaceAmt.",".$txt_subprocess_id.",0)";
			...
			for($i=0; $i<count($cbo_use_for); $i++)
			{
				if($use_for_id=="") $use_for_id=return_next_id( "id", "use_for_lab_machine_finishing", 1 ); else $use_for_id=$use_for_id+1;
				if($i==0) $add_comma=""; else $add_comma=",";
				$data_array2.="$add_comma(".$use_for_id.",".$update_id.",".$cbo_use_for[$i].")";
			}
			$field_array2="id, prod_id, use_for";
		}
				

		//*order_uom*conversion_factor
		//*".$cbo_order_uom."*".$txt_conversion_factor."
		//echo "10** insert into product_details_master ($field_array_insert) values $data_array_insert";oci_rollback($con);disconnect($con);die;
		//*".$cbo_order_uom_decimal_point."*".$cbo_cons_uom_decimal_point."
		$rID=$rID_insert=$rID_update=true;
		if(str_replace("'","",$cbo_item_category)==5 || str_replace("'","",$cbo_item_category)==6 || str_replace("'","",$cbo_item_category)==7 || str_replace("'","",$cbo_item_category)==23)
		{
			$cbo_use_for=explode(',',str_replace("'","",$cbo_use_for));
			for($i=0; $i<count($cbo_use_for); $i++)
			...
			$field_array="company_id*item_category_id*entry_form*item_sub_group_id*sub_group_code*sub_group_name*item_code*item_description*product_name_details*item_size*re_order_label*minimum_label*maximum_label*unit_of_measure*item_account*item_number*updated_by*update_date*status_active*brand_name*model*origin*fixed_asset*subprocess_id*is_deleted";
			if (!empty($all_up_id))
			{
				foreach($all_up_id as $up_id)
				{
					$update_id_arr[]=$up_id;
					$data_array[$up_id]=explode("*",("".$cbo_company_name."*".$cbo_item_category."*".$entry_form_lib."*".$item_sub_group_id."*".$txt_subgroup_code."*".$txt_subgroup_name."*".$txt_item_code."*".$txt_description."*'".$productname."'*".$txt_item_size."*".$txt_reorder_label."*".$txt_min_label."*".$txt_max_label."*".$cbo_cons_uom."*".$txt_item_account."*".$txt_item_no."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$cbo_status."*".$txt_brand."*".$txt_model_name."*".$cbo_origin."*".$cbo_fixed_asset."*".$txt_subprocess_id."*0"));
					
				}
				//$data_array="".$cbo_company_name."*".$cbo_item_category."*".$entry_form_lib."*".$item_sub_group_id."*".$txt_subgroup_code."*".$txt_subgroup_name."*".$txt_item_code."*".$txt_description."*'".$productname."'*".$txt_item_size."*".$txt_reorder_label."*".$txt_min_label."*".$txt_max_label."*".$cbo_cons_uom."*".$txt_item_account."*".$txt_item_no."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$cbo_status."*".$txt_brand."*".$txt_model_name."*".$cbo_origin."*".$cbo_fixed_asset."*".$txt_subprocess_id."*0";
				
				$rID=execute_query(bulk_update_sql_statement("product_details_master","id",$field_array,$data_array,$update_id_arr));

				$rID3=execute_query( "delete from use_for_lab_machine_finishing where prod_id = $update_id",0);
			    $rID2=sql_insert("use_for_lab_machine_finishing",$field_array2,$data_array2,1);
			}

			$dyes_prod_stock=return_field_value("current_stock","product_details_master","id=$update_id and status_active=1 and is_deleted=0","current_stock");				
				//echo "10**$update_id";die;
			if ($dyes_prod_stock==0){
				$data_array_update="".$cbo_company_name."*".$cbo_item_category."*".$entry_form_lib."*".$item_sub_group_id."*".$txt_subgroup_code."*".$txt_subgroup_name."*".$txt_item_code."*".$txt_description."*'".$productname."'*".$txt_item_size."*".$txt_reorder_label."*".$txt_min_label."*".$txt_max_label."*".$cbo_cons_uom."*".$txt_item_account."*".$txt_item_no."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$cbo_status."*".$txt_brand."*".$txt_model_name."*".$cbo_origin."*".$cbo_fixed_asset."*".$txt_subprocess_id."*0";
				$rID_update=sql_update("product_details_master",$field_array,$data_array_update,"id","".$update_id."",0);
		...
		{
			$rID_insert=sql_insert("product_details_master",$field_array_insert,$data_array_insert,0);
		...
		if($db_type==0)
		{
			if($rID && $rID_update && $rID_insert && $rID3 && $rID2)
			{
				mysql_query("COMMIT");
