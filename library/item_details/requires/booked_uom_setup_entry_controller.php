...
if($action=="load_drop_down_subsection")
{
	...
	else if($data[0]==5) $subID='6,7,8,9,10,11';
	else if($data[0]==9) $subID='12,13';
	else $subID='0';
	//echo $data[0]."**".$subID;
	...
	if(count($data_array) > 0)
	{
		foreach ($data_array as $row) 
		{
			...
			<tr id="row_<? echo $tblRow; ?>" align="center">
	            <td width="150"><? echo create_drop_down( "cboSection_".$tblRow,150, $trims_section,"", 1, "-- Select Section --",$row[csf('section_id')],"load_sub_section(<? echo $tblRow; ?>)",0,'','','','','','',"cboSection[]"); ?>
	            </td>
	            <td id="subSectionTd_<? echo $tblRow; ?>"><? echo create_drop_down( "cboSubSection_".$tblRow, 150, $trims_sub_section,"", 1, "-- Select Sub-Section --",$row[csf('sub_section_id')],'',0,'','','','','','',"cboSubSection[]"); ?></td>
