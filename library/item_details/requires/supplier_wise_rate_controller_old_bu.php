...
if($action == "get_search_rate"){
	...
	// $query_for_supplier_rate = "select sr.rate, supplier.supplier_name, supplier.id supplier_id, item_description.id itd from lib_supplier_wise_rate sr join lib_supplier supplier on supplier.id = sr.supplier_id join lib_item_details item_description on sr.item_details_id = item_description.id where sr.item_details_id in (".str_replace("'","",$search_item_description).") and sr.item_category_id = $search_item_category and sr.item_group_id = $search_item_group ";

	// var_dump(sql_select($query_for_supplier_rate)); die();
	$i = 1;
...
<? 
	...
	foreach ($serch_item_library as $search_item_id => $search_item_value) {?>
		<input id="submit" class="formbutton" value="Edit" name="submit" onclick="updateSupplierRate()" style="width:60px; float:right" type="button">
      <span style="float:left; color:Blue; font-weight:bold"><? echo  $search_item_value;?></span>
          ...
          <thead>
            ...
            <tbody>
              <tr>
                  ...
                  <td width=200>
                  	<? 
                  		 echo  create_drop_down( "item-supplier-".$search_item_id."_1", 200, $supplier_library,'', 1, "-- Select Item Supplier --", 0, "" );
                  	?>
                  </td>
                  <td width=200>
                  	<input type="text" class="text_boxes" name="item-supplier-rate_1" id="item-supplier-rate-<? echo $search_item_id;?>_1" placeholder="rate">
                  </td>
                  <td>
                            <input type="button" id="increaselabtest_<? echo $i; ?>" style="width:30px" class="formbutton" value="+" onClick="add_break_down_tr_supplier_rate(<? echo $i; ?>, <? echo $search_item_id; ?> )"/>
                            <input type="button" id="decreaselabtest_<? echo $i; ?>" style="width:30px" class="formbutton" value="-" onClick="javascript:fn_deletebreak_down_tr(<? echo $i; ?> ,<? echo $search_item_id; ?>  );"/> 
                  </td> 
              </tr>
            </tbody>
