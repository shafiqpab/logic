...
{
	$nameArray=sql_select( "select id,yarn_count,sequence_no,status_active,woven_dying_type,count_value from  lib_yarn_count where id='$data'" );
	foreach ($nameArray as $inf)
	{
		...
		echo "document.getElementById('cbo_woven_dying_type').value  = '".($inf[csf("woven_dying_type")])."';\n";
		echo "document.getElementById('txt_count_value').value  = '".($inf[csf("count_value")])."';\n";
...
if ($action=="save_update_delete")
{
	...
	$txt_count_value 		= str_replace("'","",$txt_count_value);
	$cbo_woven_dying_type 	= str_replace("'","",$cbo_woven_dying_type);
	...
	if ($operation==0)  // Insert Here
	{
		...
		else
		{
			...
			$field_array="id,yarn_count,sequence_no,woven_dying_type,count_value,inserted_by,insert_date,status_active,is_deleted";
			$data_array="(".$id.",".$txt_yarn_count.",".$txt_sequence.",".$cbo_woven_dying_type.",".$txt_count_value.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_status.",0)"; 
	...
	else if ($operation==1)   // Update Here
	{
		...
		else
		{
			...
			$field_array="yarn_count*sequence_no*woven_dying_type*count_value*updated_by*update_date*status_active*is_deleted";
			$data_array="".$txt_yarn_count."*".$txt_sequence."*".$cbo_woven_dying_type."*".$txt_count_value."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$cbo_status."*0";
