	...
	if (move_uploaded_file($source_excel, $targetzip)) 
	{
		...
		for ($i = 1; $i <= $excel->sheets[0]['numRows']; $i++) 
		{
			...
			else
			{
				...
				$all_data_arr[$i][21]['stock_qty']=trim($stock_qty);
				$all_data_arr[$i][22]['rate']=trim($rate);
				...
				$all_data_arr[$i][29]['manuf_date']=date("d-M-Y", strtotime(str_replace("'", "", trim($manuf_date))));
				$all_data_arr[$i][30]['exp_date']=date("d-M-Y", strtotime(str_replace("'", "", trim($exp_date))));
			}
		}
		...
		foreach($all_data_arr as $column_val)
		{
			...
			if ($column_val[23]['supplier'] !="" || $stock_qty !=""){
				if ($supplier_id==""){
			...
			if ($column_val[24]['currency_val'] !="" || $stock_qty !=""){
				if ($currency_id==""){
			...
			if ($column_val[25]['store_name'] !="" || $stock_qty !=""){
				if ($store_name_id==""){
			...
			if ($column_val[26]['pay_mode_val'] !="" || $stock_qty !=""){
				if ($pay_mode_id==""){
			...
			if ($column_val[27]['source_val'] !="" || $stock_qty !=""){
				if ($source_id==""){
			...
			{				
				if ($item_ategory_id==5 || $item_ategory_id==6 || $item_ategory_id==7 || $item_ategory_id==23) //Dyes Chemical Category
				{
					...
					if ($variable_lot==1) //With Lot
					{
						...
						{
							$dyesprod_keys="";
							$dyesprod_keys=trim($company_id).'**'.trim($item_ategory_id).'**'.trim($item_group_id).'**'.trim($sub_group_name_val).'**'.trim($item_description).'**'.trim($item_size).'**'.trim($model).'**'.trim($item_number).'**'.trim($item_code).'**'.trim($lot);

							// Duplicate Product Check
							if (array_key_exists($dyesprod_keys, $dyesduplicate_product_data_arr))
							{
								$duplicate_dyesprod_keys=trim($column_val[1]['company'])."**".trim($column_val[2]['item_ategory_val'])."**".trim($column_val[3]['item_group'])."**".trim($sub_group_name_val)."**".trim($item_description)."**".trim($item_size)."**".trim($model)."**".trim($item_number)."**".trim($item_code)."**".trim($lot);
				...
				else
				{
					...
					if ($product_id != "") //update item with stock
					{
						...
						$update_product_data[$product_id]=explode("*",("".$currentStock."*".$avgRate."*".$StockValue."*".$supplier_id."*".$prod_id_update_from_excel."*".$store_name_id."*".$user_id."*'".$pc_date_time."'"));
						...
						if ($data_array_receive_with_stock_existingProd != '') $data_array_receive_with_stock_existingProd .=",";
						$data_array_receive_with_stock_existingProd .= "(".$id.",'".$new_recv_number[1]."','".$new_recv_number[2]."','".$new_recv_number[0]."',20,".$company_id.",".$receive_basis.",'".$receive_date."','".$store_name_id."','".$exchange_rate."','".$currency_id."','".$supplier_id."','".$pay_mode_id."','".$source_id."','".$user_id."','".$pc_date_time."')";

						$con_ile=$con_ile_cost=0;
						$cbo_floor=$cbo_room=$txt_rack=$txt_shelf=$cbo_bin=0;

						if ($data_array_trans_with_stock_existingProd != '') $data_array_trans_with_stock_existingProd .=",";
						$data_array_trans_with_stock_existingProd .= "(".$dtlsid.",".$id.",".$receive_basis.",".$company_id.",".$supplier_id.",".$product_id.",".$item_ategory_id.",1,'".$receive_date."','".$store_name_id."','".$order_uom_id."','".$txt_receive_qty."','".$txt_rate."','".$ile."','".$ile_cost."','".$txt_amount."','".$cons_uom_id."','".$con_quantity."','".$cons_rate."','".$con_ile."','".$con_ile_cost."','".$con_amount."','".$con_quantity."','".$con_amount."','".$cbo_floor."','".$cbo_room."','".$txt_rack."','".$txt_shelf."','".$cbo_bin."','".$manuf_date."','".$exp_date."','".$user_id."','".$pc_date_time."',1)";
					}
					else //new item insert with stock
					{
						$prod_keys="";
						$prod_keys=trim($company_id).'**'.trim($item_ategory_id).'**'.trim($item_group_id).'**'.trim($sub_group_name_val).'**'.trim($item_description).'**'.trim($item_size).'**'.trim($model).'**'.trim($item_number).'**'.trim($item_code);
						//echo $prod_keys.'system';
				
						// Duplicate Product Check
						if (array_key_exists($prod_keys, $duplicate_product_data_arr))
						{
							$duplicate_prod_keys=trim($column_val[1]['company'])."**".trim($column_val[2]['item_ategory_val'])."**".trim($column_val[3]['item_group'])."**".trim($sub_group_name_val)."**".trim($item_description)."**".trim($item_size)."**".trim($model)."**".trim($item_number)."**".trim($item_code);
						...
						if ($data_array_receive_with_stock != '') $data_array_receive_with_stock .=",";
						$data_array_receive_with_stock .= "(".$id.",'".$new_recv_number[1]."','".$new_recv_number[2]."','".$new_recv_number[0]."',20,".$company_id.",".$receive_basis.",'".$receive_date."','".$store_name_id."','".$exchange_rate."','".$currency_id."','".$supplier_id."','".$pay_mode_id."','".$source_id."','".$user_id."','".$pc_date_time."')";
						...
						if ($data_array_trans_with_stock != '') $data_array_trans_with_stock .=",";
						$data_array_trans_with_stock .= "(".$dtlsid.",".$id.",".$receive_basis.",".$company_id.",".$supplier_id.",".$prod_id.",".$item_ategory_id.",1,'".$receive_date."','".$store_name_id."','".$order_uom_id."','".$txt_receive_qty."','".$txt_rate."','".$ile."','".$ile_cost."','".$txt_amount."','".$cons_uom_id."','".$con_quantity."','".$cons_rate."','".$con_ile."','".$con_ile_cost."','".$con_amount."','".$con_quantity."','".$con_amount."','".$cbo_floor."','".$cbo_room."','".$txt_rack."','".$txt_shelf."','".$cbo_bin."','".$manuf_date."','".$exp_date."','".$user_id."','".$pc_date_time."',1)";


						if ($data_array_prod_with_stock != '') $data_array_prod_with_stock .=",";
						$data_array_prod_with_stock .= "(".$prod_id.",".$entry_form_lib.",".$company_id.",".$item_ategory_id.",".$item_group_id.",'".$item_sub_group_id."','".$sub_group_code_val."','".$sub_group_name_val."','".$item_code."','".$item_description."','".$productname."','".$item_size."','".$re_order_level."','".$min_level."','".$max_level."','".$cons_uom_id."','".$order_uom_id."','".$conversion_factor."','".$item_account."','".$item_number."','".$brand."','".$model."','".$origin_id."','".$fixed_asset_id."','".$con_quantity."','".$cons_rate."','".$con_amount."','".$supplier_id."','".$store_name_id."',1,0,'".$user_id."','".$pc_date_time."',1)";
					}
				}
			}
		...
		// insert new item without stock
		if ($data_array_prod_without_stock != ""){
			$rID=sql_insert("product_details_master",$field_array_prod,$data_array_prod_without_stock,0);
			if ($flag==1){
				if ($rID) $flag=1; else $flag=0;
			}
		}

		// insert new item with stock
		if ($data_array_prod_with_stock != ""){
			$rID2=sql_insert("product_details_master",$field_array_prod,$data_array_prod_with_stock,0);
			if ($flag==1){
				if ($rID2) $flag=1; else $flag=0;
			}
		}
		
		if ($data_array_receive_with_stock != ""){
			$rID3=sql_insert("inv_receive_master",$field_array_receive,$data_array_receive_with_stock,0);
			if ($flag==1){
				if ($rID3) $flag=1; else $flag=0;
			}
		}
		
		if ($data_array_trans_with_stock != ""){
			$rID4=sql_insert("inv_transaction",$field_array_trans,$data_array_trans_with_stock,0);
			if ($flag==1){
				if ($rID4) $flag=1; else $flag=0;
			}
		}
		
		// update new item with stock
		if ($data_array_receive_with_stock_existingProd != ""){
			$rID5=sql_insert("inv_receive_master",$field_array_receive,$data_array_receive_with_stock_existingProd,0);
			if ($flag==1){
				if ($rID5) $flag=1; else $flag=0;
			}
		}
		
		if ($data_array_trans_with_stock_existingProd != ""){
			$rID6=sql_insert("inv_transaction",$field_array_trans,$data_array_trans_with_stock_existingProd,0);
			if ($flag==1){
				if ($rID6) $flag=1; else $flag=0;
			}
		}

		if ($data_array_receive_chemical_dyes_with_lot != ""){
			$rID7=sql_insert("inv_receive_master",$field_array_receive_with_lot,$data_array_receive_chemical_dyes_with_lot,0);
			if ($flag==1){
				if ($rID7) $flag=1; else $flag=0;
			}
		}

		if ($data_array_trans_chemical_dyes_with_lot != ""){
			$rID8=sql_insert("inv_transaction",$field_array_trans_with_lot,$data_array_trans_chemical_dyes_with_lot,0);
			if ($flag==1){
				if ($rID8) $flag=1; else $flag=0;
			}
		}

		if ($data_array_receive_chemicaldyes_with_lot != ""){
			$rID9=sql_insert("inv_receive_master",$field_array_receive_with_lot,$data_array_receive_chemicaldyes_with_lot,0);
			if ($flag==1){
				if ($rID9) $flag=1; else $flag=0;
			}
		}

		if ($data_array_prod_chemicaldyes_with_lot != ""){
			$rID10=sql_insert("product_details_master",$field_array_prod_with_lot,$data_array_prod_chemicaldyes_with_lot,0);
			if ($flag==1){
				if ($rID10) $flag=1; else $flag=0;
			}
		}

		if ($data_array_trans_chemicaldyes_with_lot != ""){
			$rID11=sql_insert("inv_transaction",$field_array_trans_with_lot,$data_array_trans_chemicaldyes_with_lot,0);
			if ($flag==1){
				if ($rID11) $flag=1; else $flag=0;
			}
		}

		if ($data_array_store_with_lot != ""){
			$rID12=sql_insert("inv_store_wise_qty_dtls",$field_array_store_with_lot,$data_array_store_with_lot,0);
			if ($flag==1){
