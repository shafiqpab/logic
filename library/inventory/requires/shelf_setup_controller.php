...
{
	echo create_drop_down( "cbo_location_name", 262, "select location_name,id from lib_location where company_id='$data' and is_deleted=0  and status_active=1  order by location_name",'id,location_name', 1, '--- Select Location ---', 0, "load_drop_down( 'requires/shelf_setup_controller', this.value, 'load_drop_down_store_by_location', 'store_td' )"  );
}
if ($action=="load_drop_down_store_by_location")
{
	echo create_drop_down( "cbo_store_name", 262, "select store_name,id from lib_store_location where location_id='$data' and is_deleted=0  and status_active=1  order by store_name",'id,store_name', 1, '--- Select Store ---', 0, "load_drop_down( 'requires/shelf_setup_controller', this.value, 'load_drop_down_floor_by_store', 'floor_td' )"  );
}
if ($action=="load_drop_down_floor_by_store")
{
	echo create_drop_down( "cbo_floor_id", 262, "select a.floor_room_rack_name,a.floor_room_rack_id from lib_floor_room_rack_mst a,lib_floor_room_rack_dtls b where a.floor_room_rack_id=b.floor_id and b.store_id='$data' and a.is_deleted=0  and a.status_active=1 group by a.floor_room_rack_name,a.floor_room_rack_id",'floor_room_rack_id,floor_room_rack_name', 1, '--- Select Floor ---', 0, "load_drop_down( 'requires/shelf_setup_controller', this.value, 'load_drop_down_room_by_floor', 'room_td' )"  );
}
if ($action=="load_drop_down_room_by_floor")
{
	echo create_drop_down( "cbo_room_id", 262, "select a.floor_room_rack_name,a.floor_room_rack_id from lib_floor_room_rack_mst a,lib_floor_room_rack_dtls b where a.floor_room_rack_id=b.room_id and b.floor_id='$data' and a.is_deleted=0  and a.status_active=1 group by a.floor_room_rack_name,a.floor_room_rack_id",'floor_room_rack_id,floor_room_rack_name', 1, '--- Select Room ---', 0, "load_drop_down( 'requires/shelf_setup_controller', this.value, 'load_drop_down_rack_by_room', 'rack_td' )"  );
}
if ($action=="load_drop_down_rack_by_room")
{
	echo create_drop_down( "cbo_rack_id", 262, "select a.floor_room_rack_name,a.floor_room_rack_id from lib_floor_room_rack_mst a,lib_floor_room_rack_dtls b where a.floor_room_rack_id=b.rack_id and b.room_id='$data' and a.is_deleted=0  and a.status_active=1 group by a.floor_room_rack_name,a.floor_room_rack_id",'floor_room_rack_id,floor_room_rack_name', 1, '--- Select Rack ---', 0, ""  );
}


if ($action=="inventoryShelf_list_view")
{
...
if ($action=="load_php_data_to_form")//load list view data to the form
{
	...
	$nameArray=sql_select( "select a.company_id,b.location_id,a.floor_room_rack_name,a.status_active,b.serial_no, a.floor_room_rack_id,b.store_id,b.floor_id,b.room_id,rack_id from lib_floor_room_rack_mst a, lib_floor_room_rack_dtls b where a.floor_room_rack_id=b.shelf_id and a.status_active in(1,2,3) and a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0 and a.floor_room_rack_id='$data[0]' and b.floor_room_rack_dtls_id='$data[1]'" );
	foreach ($nameArray as $inf)
	{
		echo "load_drop_down( 'requires/shelf_setup_controller', '".($inf[csf("company_id")])."', 'load_drop_down_location', 'location' );\n";
		echo "load_drop_down( 'requires/shelf_setup_controller', '".($inf[csf("location_id")])."', 'load_drop_down_store_by_location', 'store_td' );\n";
		echo "load_drop_down( 'requires/shelf_setup_controller', '".($inf[csf("store_id")])."', 'load_drop_down_floor_by_store', 'floor_td' );\n";
		echo "load_drop_down( 'requires/shelf_setup_controller', '".($inf[csf("floor_id")])."', 'load_drop_down_room_by_floor', 'room_td' );\n";
		echo "load_drop_down( 'requires/shelf_setup_controller', '".($inf[csf("room_id")])."', 'load_drop_down_rack_by_room', 'rack_td' );\n";
		...
		echo "document.getElementById('txt_shelf').value  = '".($inf[csf("floor_room_rack_name")])."';\n";


		echo "document.getElementById('txt_shelf_sequence').value  = '".($inf[csf("serial_no")])."';\n";
		echo "document.getElementById('cbo_status').value  = '".($inf[csf("status_active")])."';\n";
		echo "document.getElementById('update_id').value  = '".($inf[csf("floor_room_rack_id")])."';\n";
		echo "set_button_status(1, '".$_SESSION['page_permission']."', 'fnc_inventory_shelf_info',1);\n";
	}
   ...
   select b.from_store,b.to_store,b.floor_id,b.to_floor_id,b.room,b.to_room,b.rack,b.to_rack,shelf,to_shelf,b.bin_box,b.to_bin_box from inv_item_transfer_mst a, inv_item_transfer_dtls b where a.id=b.mst_id and b.item_category in(2,13) and (a.company_id=$companyID or a.to_company=$companyID) and (a.location_id=$locationID  or  a.to_location_id=$locationID) and (b.from_store=$storeID or b.to_store=$storeID) and (b.floor_id=$floorID or b.to_floor_id=$floorID) and (b.room=$roomID or b.to_room=$roomID) and (b.rack=$rackID or b.to_rack=$rackID) and (b.shelf=$shelfID or b.to_shelf=$shelfID) group by  b.from_store,b.to_store,b.floor_id,b.to_floor_id,b.room,b.to_room,b.rack,b.to_rack,shelf,to_shelf,b.bin_box,b.to_bin_box ");
	...
	else
	{
		...
		echo "$('#txt_shelf').removeAttr('disabled')" . ";\n";
	}
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		else
		{
			...
			$data_array="(".$id.",".$cbo_company_name.",".$txt_shelf.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_status.",0)";

			$id_dtls=return_next_id( "floor_room_rack_dtls_id", "lib_floor_room_rack_dtls", 1 ) ;
			$field_array_dtls="floor_room_rack_dtls_id,shelf_id,floor_id,room_id,rack_id,company_id,location_id,store_id,serial_no,inserted_by,insert_date,status_active,is_deleted";
			$data_array_dtls="(".$id_dtls.",".$id.",".$cbo_floor_id.",".$cbo_room_id.",".$cbo_rack_id.",".$cbo_company_name.",".$cbo_location_name.",".$cbo_store_name.",".$txt_shelf_sequence.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_status.",0)";
	...
	else if ($operation==1)   // Update Here
	{
		...
		if (is_duplicate_field( "a.floor_room_rack_name", "lib_floor_room_rack_mst a,lib_floor_room_rack_dtls b", "a.company_id=$cbo_company_name and b.location_id=$cbo_location_name and b.store_id=$cbo_store_name and b.floor_id=$cbo_floor_id and b.room_id=$cbo_room_id and b.rack_id=$cbo_rack_id  and a.floor_room_rack_id=b.shelf_id and a.floor_room_rack_name=$txt_shelf and a.floor_room_rack_id!=$update_id and a.is_deleted=0" ) == 1)
		...
		else
		{
			...
			$data_array="".$cbo_company_name."*".$txt_shelf."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$cbo_status."*0";

			$field_array_dtls="rack_id*room_id*floor_id*company_id*location_id*store_id*serial_no*updated_by*update_date*status_active*is_deleted";
			$data_array_dtls="".$update_id."*".$cbo_room_id."*".$cbo_floor_id."*".$cbo_company_name."*".$cbo_location_name."*".$cbo_store_name."*".$txt_shelf_sequence."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$cbo_status."*0";
...
if($action=="print_barcode") // for Palmal Group
{
	//echo "1000".$data;die;
	$shelf_dtls_arr=array_unique(array_filter(explode(",",$data)));


	// VARIABLE_SETTINGS_INVENTORY
	...
	$con = connect();
	$r_id1 = execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_id and ref_from=1 and entry_form in (160)");
	if($r_id1)
	{
		oci_commit($con);
		disconnect($con);
	}
	foreach ($shelf_dtls_arr as $row)
	{

		$all_shelf_dtls_arr[$row]=$row;
	}


	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 160, 1,$all_shelf_dtls_arr, $empty_arr);
	//die;
	$i 		= 1;
	$html 	= '';

	// $sql= "SELECT a.company_id, a.floor_room_rack_name, b.floor_room_rack_dtls_id, b.store_id, b.floor_id, b.room_id,b.rack_id, 
	// concat(concat(b.floor_room_rack_dtls_id, '***'), a.floor_room_rack_name) as shelfinfo from lib_floor_room_rack_mst a, lib_floor_room_rack_dtls b, GBL_TEMP_ENGINE c where a.floor_room_rack_id=b.shelf_id and b.floor_room_rack_dtls_id=c.ref_val and c.user_id=$user_id and c.ref_from=1 and c.entry_form=160 and a.status_active in(1,2,3) and a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0 order by b.floor_room_rack_dtls_id";
	...
	execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_id and ref_from=1 and entry_form in (160)");
    oci_commit($con);
	disconnect($con);
	...
	{
		if(!empty($row['BIN_ID'])){
			$shelfinfo = $row['FLOOR_ROOM_RACK_DTLS_ID'].'***'.$floor_room_rack_library[$row['BIN_ID']]."/".$floor_room_rack_library[$row['SHELF_ID']]."/".$floor_room_rack_library[$row['RACK_ID']];
		}
		elseif(!empty($row['SHELF_ID'])){
			$shelfinfo = $row['FLOOR_ROOM_RACK_DTLS_ID'].'***'.$floor_room_rack_library[$row['ROOM_ID']]."/".$floor_room_rack_library[$row['RACK_ID']]."/".$floor_room_rack_library[$row['SHELF_ID']];
		}
		elseif(!empty($row['RACK_ID'])){
			$shelfinfo = $row['FLOOR_ROOM_RACK_DTLS_ID'].'***'.$floor_room_rack_library[$row['ROOM_ID']]."/".$floor_room_rack_library[$row['RACK_ID']]."/".$floor_room_rack_library[$row['FLOOR_ID']];
		}
		elseif(!empty($row['ROOM_ID'])){
			$shelfinfo = $row['FLOOR_ROOM_RACK_DTLS_ID'].'***'.$floor_room_rack_library[$row['ROOM_ID']]."/".$floor_room_rack_library[$row['FLOOR_ID']]."/".$floor_room_rack_library[$row['STORE_ID']];
		}


		// $filename = $PNG_TEMP_DIR.md5($row[csf("shelfinfo")]).'.png';
		$filename = $PNG_TEMP_DIR.md5($shelfinfo).'.png';
		QRcode::png($shelfinfo, $filename, $errorCorrectionLevel, $matrixPointSize, 2);
		...
		<table  cellpadding="0" cellspacing="0" >
			<tr>
				...
				<div id="div_'.$i.'"><img src="'.$PNG_WEB_DIR.basename($filename).'" height="80" width=""></div>
				</td>
			</tr>
			<tr>
				<td width="" align="center"><span style="font-weight:bold">Store : </span>'. $store_lib[$row[csf("store_id")]].'</td>
			</tr>
			<tr>
				<td width="" align="center"><span style="font-weight:bold">Floor : </span>'.$floor_room_rack_library[$row[csf('floor_id')]].'</td>
			</tr>
			<tr>
				<td width="" align="center"><span style="font-weight:bold">Room : </span>'.$floor_room_rack_library[$row[csf('room_id')]].'</td>
			</tr>
			<tr>
				<td width="" align="center"><span style="font-weight:bold">Rack : </span>'.$floor_room_rack_library[$row[csf('rack_id')]].'</td>
			</tr>
			<tr>
				<td width="" align="center"><span style="font-weight:bold">Shelf : </span>'.$row[csf('floor_room_rack_name')].'</td>
			</tr>
			<tr style="text-align: center;vertical-align: middle;padding: 0;">
				<td align="center"><barcode code="'.$shelfinfo.'" type="C39" size="0.5"  height="2.0" /></td>
			</tr>
		</table>';
...
if($action=="print_barcode2") // for Charka Group
{
	//echo "1000".$data;die;
	$shelf_dtls_arr=array_unique(array_filter(explode(",",$data)));
	...
	$mpdf = new mPDF('',    // mode - default ''
					array(76.8,101.6),		// array(65,210),    // format - A4, for example, default ''
	...
	$con = connect();
	$r_id1 = execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_id and ref_from=2 and entry_form in (160)");
	if($r_id1)
	{
		oci_commit($con);
		disconnect($con);
	...
	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 160, 2,$all_shelf_dtls_arr, $empty_arr);
	//die;
	$i 		= 1;
	$html 	= '';

	$sql= "SELECT a.company_id, a.floor_room_rack_name, b.floor_room_rack_dtls_id, b.store_id, b.floor_id, b.room_id,b.rack_id, concat(concat(b.floor_room_rack_dtls_id, '***'), a.floor_room_rack_name) as shelfinfo from lib_floor_room_rack_mst a, lib_floor_room_rack_dtls b, gbl_temp_engine c where a.floor_room_rack_id=b.shelf_id  and b.floor_room_rack_dtls_id=c.ref_val and c.user_id=$user_id and c.ref_from=2 and c.entry_form=160 and a.status_active in(1,2,3) and a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0 order by b.floor_room_rack_dtls_id";
	//echo $sql;die;

	$dataArray = sql_select($sql);

	execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_id and ref_from=2 and entry_form in (160)");
    oci_commit($con);
	disconnect($con);
	...
	{
		$filename = $PNG_TEMP_DIR.md5($row[csf("shelfinfo")]).'.png';
		QRcode::png($row[csf("shelfinfo")], $filename, $errorCorrectionLevel, $matrixPointSize, 2);
		...
		<table  cellpadding="0" cellspacing="0" >
			...
			<tr>
				<td align="center"><span style="font-weight:bold">Rack : </span>'.$floor_room_rack_library[$row[csf('rack_id')]].'</td>
			</tr>
			<tr>
				<td align="center" style="font-weight:bold"><span >Shelf : </span>'.$row[csf('floor_room_rack_name')].'</td>
			</tr>
			<tr style="width:100%, text-align: center;vertical-align: middle;padding: 0;">
				<td align="center"><barcode code="'.$row[csf("shelfinfo")].'" type="C39" size="0.5"  height="2.0" /></td>
			</tr>
