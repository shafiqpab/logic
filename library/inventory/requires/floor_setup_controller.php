...
{
	echo create_drop_down( "cbo_location_name", 262, "select location_name,id from lib_location where company_id='$data' and is_deleted=0  and status_active=1  order by location_name",'id,location_name', 1, '--- Select Location ---', 0, "load_drop_down( 'requires/floor_setup_controller', this.value, 'load_drop_down_store_by_location', 'store_td' )"  );
}
if ($action=="load_drop_down_store_by_location")
{
	...
	{
		echo create_drop_down( "cbo_store_name", 262, "select store_name,id from lib_store_location where location_id='$data' and is_deleted=0  and status_active=1  order by store_name",'id,store_name', 1, '--- Select Store ---', 0, ""  );
	}
}

if ($action=="inventoryFloor_list_view")
{
...
{
	$data = explode("_",$data);
	$nameArray=sql_select( "select a.company_id,b.location_id,a.floor_room_rack_name,a.status_active,b.serial_no, a.floor_room_rack_id,b.store_id from lib_floor_room_rack_mst a, lib_floor_room_rack_dtls b where a.floor_room_rack_id=b.floor_id and a.status_active in(1,2,3) and a.is_deleted=0 and b.status_active in(1,2,3) and b.is_deleted=0 and a.floor_room_rack_id='$data[0]' and b.floor_room_rack_dtls_id='$data[1]'" );
	foreach ($nameArray as $inf)
	{
		echo "load_drop_down( 'requires/floor_setup_controller', '".($inf[csf("company_id")])."', 'load_drop_down_location', 'location' );\n";
		echo "load_drop_down( 'requires/floor_setup_controller', '".($inf[csf("location_id")])."', 'load_drop_down_store_by_location', 'store_td' );\n";
   ...
   select b.from_store,b.to_store,b.floor_id,b.to_floor_id,b.room,b.to_room,b.rack,b.to_rack,shelf,to_shelf,b.bin_box,b.to_bin_box from inv_item_transfer_mst a, inv_item_transfer_dtls b where a.id=b.mst_id and b.item_category in(2,13) and (a.company_id=$companyID or a.to_company=$companyID) and (a.location_id=$locationID  or  a.to_location_id=$locationID) and (b.from_store=$storeID or b.to_store=$storeID)  and (b.floor_id=$floorID or b.to_floor_id=$floorID) group by  b.from_store,b.to_store,b.floor_id,b.to_floor_id,b.room,b.to_room,b.rack,b.to_rack,shelf,to_shelf,b.bin_box,b.to_bin_box ");
	if($checkPrevTransFloorArr[0][csf('floor_id')]>0 || $checkPrevTransFloorArr[0][csf('to_floor_id')]>0)
	{
		...
		echo "$('#txt_floor').attr('disabled','disabled')" . ";\n";
	}
	else
	{
		...
		echo "$('#txt_floor').removeAttr('disabled')" . ";\n";
	} 
}
...
if ($action=="save_update_delete")
{
	...
	{
		if (is_duplicate_field( "a.floor_room_rack_name", "lib_floor_room_rack_mst a,lib_floor_room_rack_dtls b", "a.company_id=$cbo_company_name and b.location_id=$cbo_location_name and b.store_id=$cbo_store_name and a.floor_room_rack_id=b.floor_id and a.floor_room_rack_name=$txt_floor and a.is_deleted=0" ) == 1)
		{
			echo "11**0"; die;
		}
		else
		{
			...
			$data_array="(".$id.",".$cbo_company_name.",".$txt_floor.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_status.",0)";

			$id_dtls=return_next_id( "floor_room_rack_dtls_id", "lib_floor_room_rack_dtls", 1 ) ;
			$field_array_dtls="floor_room_rack_dtls_id,floor_id,company_id,location_id,store_id,serial_no,inserted_by,insert_date,status_active,is_deleted";
			$data_array_dtls="(".$id_dtls.",".$id.",".$cbo_company_name.",".$cbo_location_name.",".$cbo_store_name.",".$txt_floor_sequence.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_status.",0)";
	...
	else if ($operation==1)   // Update Here
	{
		...
		if (is_duplicate_field( "a.floor_room_rack_name", "lib_floor_room_rack_mst a,lib_floor_room_rack_dtls b", "a.company_id=$cbo_company_name and b.location_id=$cbo_location_name and b.store_id=$cbo_store_name and a.floor_room_rack_id=b.floor_id and a.floor_room_rack_name=$txt_floor  and  a.floor_room_rack_id!=$update_id and a.is_deleted=0" ) == 1)
		...
		else
		{
			...
			$data_array="".$cbo_company_name."*".$txt_floor."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$cbo_status."*0";

			$field_array_dtls="floor_id*company_id*location_id*store_id*serial_no*updated_by*update_date*status_active*is_deleted";
			$data_array_dtls="".$update_id."*".$cbo_company_name."*".$cbo_location_name."*".$cbo_store_name."*".$txt_floor_sequence."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$cbo_status."*0";
