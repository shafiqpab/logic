...
$plan_manual_update_task_arr=return_library_array("select task_id,task_id from tna_manual_permission where is_plan_manual=1  and company_id=$cbo_company_name","task_id","task_id");
   ...
   //echo $sql; 
	...
	if($db_type==0)
	{
		...
		{
			if ($i!=$c) $sql .="max(CASE WHEN CONCAT(a.task_number) = '".$id."' THEN concat(a.actual_start_date,'_',a.actual_finish_date,'_',a.task_start_date,'_',a.task_finish_date,'_',a.notice_date_start,'_',a.notice_date_end,'_',a.remarks,'_',a.id,'_',a.task_number,'_',a.plan_start_flag,'_',a.plan_finish_flag,'_',a.COMMIT_START_DATE,'_',a.COMMIT_END_DATE)  END ) as status$id, ";
			else $sql .="max(CASE WHEN CONCAT(a.task_number) = '".$id."' THEN concat(a.actual_start_date,'_',a.actual_finish_date,'_',a.task_start_date,'_',a.task_finish_date,'_',a.notice_date_start,'_',a.notice_date_end,'_',a.remarks,'_',a.id,'_',a.task_number,'_',a.plan_start_flag,'_',a.plan_finish_flag,'_',a.COMMIT_START_DATE,'_',a.COMMIT_END_DATE)  END ) as status$id ";
			$i++;
		}
    	...
    	<thead>
        	<tr>
                ...
                <?
					...
					{
						$i++;
						if(count($tna_task_array)==$i) echo '<th width="160" colspan="2" title="'.$task_name.'='.$tna_task_array[$task_name].'">'. $key.'</th>'; else echo '<th width="160" colspan="2" title="'.$task_name.'='.$tna_task_array[$task_name].'">'.$key.'</th>';
					}
					echo '</tr><tr>';
...
*/		 if($row[csf('po_number_id')]==0)
		...
		?>
        		<tr bgcolor="<? echo $bgcolor; ?>" style="vertical-align:middle; cursor:pointer;" height="25" onClick="change_color('tr_<? echo $h;?>','<? echo $bgcolor;?>')" id="tr_<? echo $h; ?>">
                    ...
                    <td width="70" rowspan="4"><p><? echo $buyer_name[$wo_po_details_master[$row[csf('po_number_id')]]['buyer_name']]; ?></p></td>
                    ...
                    <td width="90" rowspan="4" align="right">
						<?
                    ...
                    <td width="40" rowspan="4" align="center"><? echo number_format($wo_po_details_master[$row[csf('po_number_id')]]['set_smv'],2); ?></td>
                    
                    <td rowspan="4" title="<? echo $row[csf('job_no')]; ?>"><p><? echo $wo_po_details_master[$row[csf('po_number_id')]]['style_ref_no']; ?></p></td>
                    
                     <td width="40" rowspan="4" align="center"><? echo $wo_po_details_master[$row[csf('po_number_id')]]['job_no_prefix_num']; ?></td>
                     <td rowspan="4" width="60" align="center"><? echo implode(',',$booking_no_arr[$row[csf('po_number_id')]]); ?></td>
                    ...
                    <td width="100" rowspan="4" title="<? echo " PO. Rec. Date: ".change_date_format($row[csf('po_receive_date')]); echo ",\n Pub Ship Date: ".$row[csf('pub_shipment_date')]; echo ",\n Prev. Pub Ship Date: ".$row[csf('pub_shipment_date_prev')];echo ",\n Insert Date: ".$row[csf('insert_date')].",\n Shipment Date: ".$row[csf('shipment_date')];?>"><div style="width:98px; word-break:break-all">
					<? echo "<a href='#report_details' style='color:#990000' onclick= \"order_update_log_popup('".$row[csf('job_no')]."','".$row[csf('po_number_id')]."');\">".change_date_format($row[csf('shipment_date')])."</a>"."<br>"." ".$lead_timee."<br>"." PO Lead Time:".$po_lead_time;  ?></div>
                    </td>
                    
                    <td width="80" rowspan="4" align="center"><? echo date("d-m-Y",strtotime($row[csf('insert_date')])); ?></td>
                    <td width="90" rowspan="4" align="center">
					<? 
                    ...
                    <td width="90" rowspan="4" align="center" id="approval_status_<?= $row[csf('po_number_id')];?>">
					<? 
                ...
                <?
					...
					echo '<tr style="cursor:pointer" onClick="change_color(\'commitment_'.$h.'\',\''.$bgcolor.'\')" id="commitment_'.$h.'"><td width="90"><b>Commitment</b></td>';
					$i=0;
					 foreach($tna_task_id as $vid=>$key)
					 {
						...
						if( $new_data[7]!="") $function="onclick='update_tna_process(3,$new_data[7],".$row[csf('po_number_id')].")'";  else $function="";
						$bgcolor1=""; $bgcolor="";
						
						if($actual_manual_update_task_arr[$vid]==''){$function="";}
						if($row[APPROVED]==1){$function="onclick='approvedAlertMessage()'";}
						
						if($new_data[11]== "" || $new_data[11]=="0000-00-00"){$bgcolor="";}else{$bgcolor="#fc9d02";}
						if($new_data[12]== "" || $new_data[12]=="0000-00-00"){$bgcolor1="";}else{$bgcolor1="#fc9d02";}
						
						$idd=$row[csf('job_no')]."".$row[csf('po_number_id')]."".$key;
						if(count($tna_task_id)==$i)
							echo '<td id="commitment_1'.$vid.$row[csf('po_number_id')].'" align="center" title="Click Here to Edit Date" id="'.$idd.'1" '.$function.' width="80" bgcolor="'.$bgcolor.'">'.($new_data[11]== "" || $new_data[11]=="0000-00-00" ? "" : change_date_format($new_data[11])).'</td><td id="commitment_2'.$vid.$row[csf('po_number_id')].'" align="center" id="'.$idd.'2" title="Click Here to Edit Date" '.$function.' bgcolor="'.$bgcolor1.'" title="'.$new_data[6].'">'.($new_data[12]== "" || $new_data[12]=="0000-00-00" ? "" : change_date_format($new_data[12])).'</td>';
							
						else
							echo '<td id="commitment_1'.$vid.$row[csf('po_number_id')].'" align="center" id="'.$idd.'1" title="Click Here to Edit Date"  '.$function.' width="80" bgcolor="'.$bgcolor.'">'.($new_data[11]== "" || $new_data[11]=="0000-00-00" ? "" : change_date_format($new_data[11])).'</td><td id="commitment_2'.$vid.$row[csf('po_number_id')].'" align="center" id="'.$idd.'2" title="Click Here to Edit Date" '.$function.' width="80" bgcolor="'.$bgcolor1.'" title="'.$new_data[6].'">'.($new_data[12]== "" || $new_data[12]=="0000-00-00" ? "" : change_date_format($new_data[12])).'</td>';
					...
					echo '<tr style="cursor:pointer" onClick="change_color(\'actula_'.$h.'\',\''.$bgcolor.'\')" id="actula_'.$h.'"><td width="90"><b>Actual</b></td>';
					$i=0;
					 foreach($tna_task_id as $vid=>$key)
					 {
					...
					echo '<tr title="Delay/Early By Calculate on Commitment Date if found. Other wise Plan Date" style="cursor:pointer" onClick="change_color(\'delay_'.$h.'\',\''.$bgcolor.'\')" id="delay_'.$h.'"><td width="90"><b>Delay/Early By</b></td>';
					$j=0;
					foreach($tna_task_id as $vid=>$key)
					{
						 $j++;
						...
						{
							if($new_data[11]!= ""){$new_data[2]=$new_data[11];}//Replace With COMMIT_START_DATE
						...
						{
							if($new_data[12]!= ""){$new_data[3]=$new_data[12];}//Replace With COMMIT_END_DATE 
...
if($action=="update_tna_progress_comment_style")
{
	...
	$lead_time_array=return_library_array("select task_template_id,lead_time from tna_task_template_details where task_type=1 group by lead_time,task_template_id","task_template_id",'lead_time');
...
?> 
    ...
    <script>
		...
		function new_window()
		{
			var url = 'tna_report_controller.php?job_no=<? echo $job_no;?>&po_id=<? echo $po_id;?>&template_id=<? echo $template_id;?>&tna_process_type=<? echo $tna_process_type;?>&action=tna_progress_comment_print_style&permission=<? echo $permission;?>';
			window.open(url, "MY PAGE");
		}
		
		function new_excel()
		{
			var url = 'tna_report_controller.php?job_no=<? echo $job_no;?>&po_id=<? echo $po_id;?>&template_id=<? echo $template_id;?>&tna_process_type=<? echo $tna_process_type;?>&action=tna_progress_comment_print_style&permission=<? echo $permission;?>&isExcel=1';
			window.open(url, "MY PAGE");
		}
...
if($action=="save_update_delete")
{
	...
	{		
		...
		if($update_tna_type==1)
		{
			...
			$field_array1="task_type*task_start_date*task_finish_date*commit_start_date*commit_end_date".$field;
			$data_array1="1*".$txt_plan_start_date."*".$txt_plan_finish_date."*".$txt_commitment_start_date."*".$txt_commitment_end_date.$data."";
			...
			else
			{
				...
				$field_array="id, template_id,task_number, job_no, po_number_id, task_start_date, task_finish_date, plan_start_flag,plan_finish_flag,commit_start_date,commit_end_date,status_active,is_deleted,task_type";
				$data_array="(".$hid.",".$htmp_id.",".$htask_id.",'".str_replace("'","",$hjob_id)."',".$hpo_id.",".$txt_plan_start_date.",".$txt_plan_finish_date.",".$start_flag.",".$finish_flag.",".$txt_commitment_start_date.",".$txt_commitment_end_date.",'1','0',1)";
		...
		else
		{
			...
			$field_array="actual_start_date*actual_finish_date*actual_start_flag*actual_finish_flag*commit_start_date*commit_end_date";
			$data_array="".$txt_actual_start_date."*".$txt_actual_finish_date."*".$start."*".$finish."*".$txt_commitment_start_date."*".$txt_commitment_end_date."";
			...
			if(str_replace("'","",$hmid))
			{
				$hfield_array="task_type*actual_start_date*actual_finish_date*commit_start_date*commit_end_date";
				$hdata_array="1*".$txt_actual_start_date."*".$txt_actual_finish_date."*".$txt_commitment_start_date."*".$txt_commitment_end_date."";
			...
			{
				$hid=return_next_id( "id", "tna_plan_actual_history", 1 ) ;
				$field_array="id, template_id,task_number,job_no, po_number_id, actual_start_date, actual_finish_date,commit_start_date,commit_end_date, status_active, is_deleted,task_type";
				$data_array="(".$hid.",".$htmp_id.",".$htask_id.",'".str_replace("'","",$hjob_id)."',".$hpo_id.",".$txt_actual_start_date.",".$txt_actual_finish_date.",".$txt_commitment_start_date.",".$txt_commitment_end_date.",'1','0',1)";
				$rID=sql_insert("tna_plan_actual_history",$field_array,$data_array,1);
...
if ($action=="save_update_delete_progress_comments")
{
	...
	{ 
		...
		$rID=execute_query("delete from tna_progress_comments where order_id=$orderid");
...
//----------------------------------------------------------------------------------------
	...
	$lead_time_array=return_library_array("select task_template_id,lead_time from tna_task_template_details  where task_type=1 group by lead_time","task_template_id",'lead_time');
