...
$menual_update_task_arr=array(7,10,12,15,17,32,46,47,80,101,110,120,121,131,177,178,183,240,241,242,243,244,245,246,247,249);
	
	$mod_sql= sql_select("select a.id,a.task_catagory,a.task_name,a.task_short_name,a.task_type,a.completion_percent,a.task_sequence_no,a.task_group,b.task_template_id,b.lead_time 
...
$plan_manual_update_task_arr=return_library_array("select task_id,task_id from tna_manual_permission where is_plan_manual=1  and company_id=$cbo_company_name","task_id","task_id");
  ...
  <tr>
        ...
        <td rowspan="2" align="center" valign="middle"><?= $job_no;?></td>
...
foreach($taskChunkArr as $row){
	...
	if($i==1){
		//$html.=$htmlArr['head'];
		 echo '
				...
				<tr>
					...
					$opupFunction='<a href="javascript:fnTaskPopup(\''.$row[$i1].'*'.$job_no.'\',305)">'.$taskNameArr[$row[$i1]].'</a>';
		...
		echo '<tr>
				...
				$opupFunction='<a href="javascript:fnTaskPopup(\''.$row[$i5].'*'.$job_no.'\',305)">'.$taskNameArr[$row[$i5]].'</a>';
		...
		//$html.=$htmlArr['right'];
		 echo '<tr>';
				...
				$opupFunction='<a href="javascript:fnTaskPopup(\''.$row[$i3].'*'.$job_no.'\',305)">'.$taskNameArr[$row[$i3]].'</a>';
		...
		//$html.=$htmlArr['footer'];
		 ...
		 echo '<tr>';
				...
				$opupFunction='<a href="javascript:fnTaskPopup(\''.$row[$i4].'*'.$job_no.'\',305)">'.$taskNameArr[$row[$i4]].'</a>';
		 ...
		 echo '<tr>
				...
				for($i6=($dataShowParLine-1);$i6>=0;$i6--){
					$opupFunction='<a href="javascript:fnTaskPopup(\''.$row[$i6].'*'.$job_no.'\',305)">'.$taskNameArr[$row[$i6]].'</a>';
	...
	{		
		...
		else
		{
			...
			else
			{
				...
				$data_array="(".$hid.",".$htmp_id.",".$htask_id.",'".str_replace("'","",$hjob_id)."',".$hpo_id.",".$txt_actual_start_date.",".$txt_actual_finish_date.",'1','0',3)";
				$rID=sql_insert("tna_plan_actual_history",$field_array,$data_array,1);
			}
			//history process end-----------------------------------------;
