...
{
	$print_report_format=return_field_value("format_id","lib_report_template","template_name ='".$data."' and module_id=3 and report_id=250 and is_deleted=0 and status_active=1");
	echo "print_report_button_setting('$print_report_format');\n"; 
die;
}

$lead_time=return_library_array("select task_template_id,lead_time from tna_task_template_details where task_type=6 group by task_template_id,lead_time","task_template_id","lead_time");
...
if($action=="generate_woven_tna_report")
{
	...
	if( $tna_process_type==2 ){//Parcent base;
		...
		from lib_tna_task a where a.is_deleted = 0 and a.status_active=1 $task_group_con $whereCon order by a.task_sequence_no asc";
	}
	else
	{
		...
		from lib_tna_task a,tna_task_template_details b where a.task_name=b.tna_task_id and b.task_type=6 and a.task_type=6 $task_group_con  $whereCon and a.is_deleted = 0 and a.status_active=1 and b.is_deleted = 0 and b.status_active=1 order by a.task_sequence_no asc";
	}
   ...
   //echo $sql; 
	...
	$tna_process_task_sql = "SELECT a.TNA_TASK_ID as TASK_NUMBER FROM TNA_TASK_TEMPLATE_DETAILS a WHERE a.IS_DELETED=0 and a.STATUS_ACTIVE=1 and a.task_type = 6 ".where_con_using_array($templateArr,0,'a.TASK_TEMPLATE_ID')." GROUP BY a.TNA_TASK_ID";
        	...
        	<tr>
                ...
                <?
					...
					foreach($tna_task_array as $task_name=>$key)
					{
						...
						$i++;
						if(count($tna_task_array)==$i) echo '<th style="background:'.$td_bgcolor.';" width="160" colspan="2" title="'.$task_name.'='.$tna_task_name[$task_name].'">'. $key." <br><small>".$tna_task_group_by_id[$task_name]."</small> </th>"; else echo '<th style="background:'.$td_bgcolor.';" width="160" colspan="2" title="'.$task_name.'='.$tna_task_name[$task_name].'">'.$key." <br><small>".$tna_task_group_by_id[$task_name]."</small> </th>";
					}
					echo '</tr><tr>';
	...
	foreach ($data_sql as $row)
	{
		...
		?>
        		<tr bgcolor="<? echo $bgcolor; ?>" style="vertical-align:middle; cursor:pointer;" height="25" onClick="change_color('tr_<? echo $h;?>','<? echo $bgcolor;?>')" id="tr_<? echo $h; ?>">
                    ...
                    <td rowspan="3" title="<? echo $row[csf('job_no')]; ?>">
						<p><? //echo $wo_po_details_master[$row[csf('po_number_id')]]['style_ref_no']; ?></p>

						<?
							echo "<a href='#report_details' style='color:#990000' onclick= \"report_generate_popup('".$row[csf('job_no')]."','".$row[csf('po_number_id')]."','".$row[csf('template_id')]."','".$tna_process_type."','job_style_wise_fabric_and_trims_analysis_report');\"><p>".$wo_po_details_master[$row[csf('po_number_id')]]['style_ref_no']."</p></a>";
					...
					foreach($tna_task_id as $vid=>$key)
					{
						...
						if ( $new_approval_arr[$row[csf('job_no')]][$key]=="") {
							$new_data=explode("_",$row[csf('status').$key]); 
						}
						else {
							$new_data=explode("_",$new_approval_arr[$row[csf('job_no')]][$key]);
						}
					    // print_r($tna_task_id);die;
						...
						if(in_array($vid,$tast_id_arr))
						{
							...
							if(count($tna_task_id)==$i)
								echo '<td id="plan_1'.$vid.$row[csf('po_number_id')].'" align="center" '.$psc.' width="80" '.$function.'>a'.($new_data[2]== "" || $new_data[2]=="0000-00-00" ? "<span style='color:#FF0000'> N/A </span>" : change_date_format($new_data[2])).'</td><td id="plan_2'.$vid.$row[csf('po_number_id')].'" align="center" '.$pfc.' '.$function.'> '.($new_data[3]== "N/A"  || $new_data[3]=="0000-00-00"? "" : change_date_format($new_data[3])).'</td>';
							 else
								echo '<td id="plan_1'.$vid.$row[csf('po_number_id')].'" align="center" '.$psc.'  width="80" '.$function.'>aaa'.($new_data[2]== "" || $new_data[2]=="0000-00-00" ? "<span style='color:#FF0000'> N/A </span>" : change_date_format($new_data[2])).'</td><td id="plan_2'.$vid.$row[csf('po_number_id')].'"  align="center" '.$pfc.'width="80" '.$function.'> '.($new_data[3]== ""  || $new_data[3]=="0000-00-00"? "<span style='color:#FF0000'> N/A </span>" : change_date_format($new_data[3])).'</td>';
						}
						else
						{
...
if($action=="generate_style_wise_report")
{
	...
	from lib_tna_task a,tna_task_template_details b where a.task_name=b.tna_task_id and b.task_type=6 and a.is_deleted = 0 and a.status_active=1 and b.is_deleted = 0 and b.status_active=1 order by a.task_sequence_no asc");
	...
	else if(str_replace("'","",$cbo_search_type)==2){
		if(str_replace("'","",$txt_date_from)=="" && str_replace("'","",$txt_date_to)=="") $date_range=""; else $date_range=" and b.po_received_date between $txt_date_from and $txt_date_to";
	}
	else if(str_replace("'","",$cbo_search_type)==3){
		if(str_replace("'","",$txt_date_from)=="" && str_replace("'","",$txt_date_to)=="") $date_range=""; else $date_range=" and c.country_ship_date between $txt_date_from and $txt_date_to";
	...
	else
	{
		...
		//-------------------------------
		$sql .=" from  tna_process_mst a, wo_po_break_down b where a.po_number_id=b.id $sql_order_con $sql_job_con $shipment_status_con and b.status_active=1 and b.po_quantity>0 $order_status_cond  and a.task_type=6  group by a.job_no,a.task_number,b.pub_shipment_date order by a.job_no"; 
	}
...
if($action=="update_tna_progress_comment_style")
{
	...
	$execution_time_sql= sql_select("select for_specific, tna_task_id, execution_days from tna_task_template_details where task_type=6");
...
?> 
    ...
    <script>
		...
		function fnc_progress_comments_entry(operation)
		{
			...
			http.open("POST","woven_tna_report_controller.php",true);
		...
		function fnc_progress_comments_Reply_info()
		{
			if(http.readyState == 4) 
			{
				...
				$.post('woven_tna_report_controller.php?job_no='+'<? echo $job_no; ?>'+'&po_id='+<? echo $po_id; ?>+'&template_id='+<? echo $template_id; ?>+'&tna_process_type='+<? echo $tna_process_type; ?>,
				{ 
		...
		function autoRefresh_div()
		 {
			  $("#auto_id").load("woven_tna_report_controller.php");
		  }
		...
		{	
			...
			var page_link = 'woven_tna_report_controller.php?data='+txtcomments+'&action=comments_popup';
		...
		function new_window()
		{
			var url = 'woven_tna_report_controller.php?job_no=<? echo $job_no;?>&po_id=<? echo $po_id;?>&template_id=<? echo $template_id;?>&tna_process_type=<? echo $tna_process_type;?>&action=tna_progress_comment_print_style&permission=<? echo $permission;?>';
			window.open(url, "MY PAGE");
		}
		
		function new_excel()
		{
			var url = 'woven_tna_report_controller.php?job_no=<? echo $job_no;?>&po_id=<? echo $po_id;?>&template_id=<? echo $template_id;?>&tna_process_type=<? echo $tna_process_type;?>&action=tna_progress_comment_print_style&permission=<? echo $permission;?>&isExcel=1';
			window.open(url, "MY PAGE");
		}
...
if ($action=="save_update_delete_progress_comments_style")
{
		...
		foreach(array_unique(explode(',',$orderid)) as $pid){
			for($i=1;$i<=$tot_row; $i++)
			{
				...
				if(str_replace("'","",$$txtcomments)!="" || str_replace("'","",$$txtmercomments)!="" || str_replace("'","",$$txtresponsible)!="")
				{
					...
					$data_array_comments.="(".$id.",".$jobno.",".$pid.",".$tamplateid_end.",".$$taskid.",".$$txtresponsible.",".$$txtcomments.",".$$txtmercomments.",6,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
					$id=$id+1;
				}
			}
		}
	...
	{ 
		...
		$rID=execute_query("delete from tna_progress_comments where tamplate_id in($tamplateid) and order_id in($orderid) and task_type=6");
		$rIDs=sql_insert("tna_progress_comments",$field_array_comments,$data_array_comments,1);
...
if($action=="generate_tna_with_commitment_report")
{
	...
	$mod_sql= sql_select("select id,task_catagory,task_name,task_short_name,task_type,completion_percent from lib_tna_task where is_deleted = 0 and status_active=1 and task_type=6 order by task_sequence_no asc");
	$tna_task_array=array();
	$tna_task_id=array();
	$tna_task_cat=array();
	$tna_task_name_arr=array();
 ...
 //echo $sql; 
	...
	if($db_type==0)
	{
		$tast_tmp_id_arr=return_library_array("select task_template_id, group_concat(tna_task_id) as tna_task_id  from tna_task_template_details where task_type=6 group by task_template_id","task_template_id",'tna_task_id');
	}
	else
	{
		$tast_tmp_id_arr=return_library_array("select task_template_id, listagg(cast(tna_task_id as varchar(4000)),',') within group(order by tna_task_id) as tna_task_id  from tna_task_template_details where task_type=6 group by task_template_id","task_template_id",'tna_task_id');
	}
...
{ 
	...
	$actcual_status = (in_array($user_id, explode(',', $permission_sql_res[0]['ACTCUAL_USER_ID']))  || empty($permission_sql_res[0]['ACTCUAL_USER_ID']) )?1:0;
...
{		
	...
	//actual..........
		var actual_start_date='<? echo change_date_format($tna_result[0][csf('actual_start_date')]);?>';
		var curr_actual_start_date=$('#txt_actual_start_date').val();
		var history_actual_start_date='<? echo change_date_format($tna_history[0][csf('actual_start_date')]);?>';
		
		var actual_finish_date='<? echo change_date_format($tna_result[0][csf('actual_finish_date')]);?>';
		var curr_actual_finish_date=$('#txt_actual_finish_date').val();
		var history_actual_finish_date='<? echo change_date_format($tna_history[0][csf('actual_finish_date')]);?>';

		var actual_start_flag = 0;var actual_finish_flag = 0;
		if(actual_start_date!=curr_actual_start_date){actual_start_flag=1;}
		if((history_actual_start_date!=curr_actual_start_date) && history_actual_start_date){ actual_start_flag=1;}
		
		if(actual_finish_date!=curr_actual_finish_date){actual_finish_flag=1;}
		if((history_actual_finish_date!=curr_actual_finish_date) && history_actual_finish_date){ actual_finish_flag=1;}

		 
		var data="action=save_update_delete&operation="+operation+'&start_flag='+start_flag+'&finish_flag='+finish_flag+'&actual_start_flag='+actual_start_flag+'&actual_finish_flag='+actual_finish_flag+get_submitted_data_string('txt_actual_start_date*txt_actual_finish_date*txt_update_tna_id*txt_plan_start_date*txt_plan_finish_date*txt_update_tna_type*txt_plan_actual_history*txt_commitment_start_date*txt_commitment_end_date',"../../../");
		//alert (data);return;
		freeze_window(operation);
		http.open("POST","woven_tna_report_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
...
function fnc_tna_actual_date_update_reponse()
{
    ...
    <table width="600" cellspacing="0" cellpadding="0" class="rpt_table">
        ...
        <tr>
         ...
         <tr>
        	<td align="right">Actual Start Date-1</td>
            <td>
            ...
            <td align="right">Actual Finish Date-2</td>
            <td>
...
<script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
<script>
	// disabled for next days in calnader
	//$(".datepicker#txt_actual_start_date, .datepicker#txt_actual_finish_date").datepicker( "destroy" )
	// $( ".datepicker#txt_actual_start_date, .datepicker#txt_actual_finish_date" ).datepicker({
	// 	dateFormat: 'dd-mm-yy',
	// 	changeMonth: true,
	// 	changeYear: true,
	// 	maxDate: new Date()
...
if($action=="save_update_delete")
{
	...
	{		
		...
		if($update_tna_type==1)
		{
			...
			if($actual_start_flag==1){$field.="*actual_start_flag";$data.="*1";}
			if($actual_finish_flag==1){$field.="*actual_finish_flag";$data.="*1";}
			
			
			$field_array1="task_type*task_start_date*task_finish_date".$field;
			$data_array1="6*".$txt_plan_start_date."*".$txt_plan_finish_date.$data."";
			...
			else
			{
				...
				$data_array="(".$hid.",".$htmp_id.",".$htask_id.",'".str_replace("'","",$hjob_id)."',".$hpo_id.",".$txt_plan_start_date.",".$txt_plan_finish_date.",".$start_flag.",".$finish_flag.",'1','0',6,".$user_id.",'".$pc_date_time."')";
		...
		else if($update_tna_type==3)
		{
			...
			else
			{
				...
				$data_array="(".$hid.",".$htmp_id.",".$htask_id.",'".str_replace("'","",$hjob_id)."',".$hpo_id.",".$txt_plan_start_date.",".$txt_plan_finish_date.",".$txt_commitment_start_date.",".$txt_commitment_end_date.",".$start_flag.",".$finish_flag.",'1','0',6,".$user_id.",'".$pc_date_time."')";
				$rID=sql_insert("tna_plan_actual_history",$field_array,$data_array,1);
		...
		else
		{
			...
			if(str_replace("'","",$hmid))
			{
				$hfield_array="task_type*actual_start_date*actual_finish_date*actual_start_flag*actual_finish_flag*update_by*update_date";
				$hdata_array="6*".$txt_actual_start_date."*".$txt_actual_finish_date."*".$start."*".$finish."*".$user_id."*'".$pc_date_time."'";
				$rID=sql_update("tna_plan_actual_history",$hfield_array,$hdata_array,"id",str_replace("'","",$hmid),1);
			}
			else
			{
				$hid=return_next_id( "id", "tna_plan_actual_history", 1 ) ;
				$field_array="id, template_id,task_number,job_no, po_number_id, actual_start_date, actual_finish_date,actual_start_flag,actual_finish_flag, status_active, is_deleted,task_type,update_by,update_date";
				$data_array="(".$hid.",".$htmp_id.",".$htask_id.",'".str_replace("'","",$hjob_id)."',".$hpo_id.",".$txt_actual_start_date.",".$txt_actual_finish_date.",".$start.",".$finish.",'1','0',6,".$user_id.",'".$pc_date_time."')";
				$rID=sql_insert("tna_plan_actual_history",$field_array,$data_array,1);
...
?> 
    ...
    <script>
		...
		function autoRefresh_div()
		 {
			  $("#auto_id").load("woven_tna_report_controller.php");// a function which will load data from other file after x seconds
		  }
		...
		function new_window()
		{
			var url = 'woven_tna_report_controller.php?job_no=<? echo $job_no;?>&po_id=<? echo $po_id;?>&template_id=<? echo $template_id;?>&tna_process_type=<? echo $tna_process_type;?>&action=tna_progress_comment_print&permission=<? echo $permission;?>';
			window.open(url, "MY PAGE");
		}
		...
		function new_excel()
		{
			...
			var url = 'woven_tna_report_controller.php?job_no=<? echo $job_no;?>&po_id=<? echo $po_id;?>&template_id=<? echo $template_id;?>&tna_process_type=<? echo $tna_process_type;?>&action=tna_progress_comment_print&permission=<? echo $permission;?>&isExcel=1';
			window.open(url, "MY PAGE");
		}
...
//----------------------------------------------------------------------------------------
	...
	$lead_time_array=return_library_array("select task_template_id,lead_time from tna_task_template_details  where task_type=6 group by lead_time","task_template_id",'lead_time');
	...
	$execution_time_sql= sql_select("select tna_task_id, execution_days from tna_task_template_details  where task_type=6");
	foreach ($execution_time_sql as $row_execution_time)
	{
...
$buyer_short_name_arr = return_library_array("SELECT short_name,id FROM  lib_buyer WHERE is_deleted = 0 and status_active=1 order by id asc","id","short_name");
  ...
  //echo $sql; 
	...
	else
	{
		...
		//-------------------------------
		$sql .=" from  tna_process_mst a, wo_po_break_down b where a.po_number_id=b.id $sql_order_con $sql_job_con $shipment_status_con and b.status_active=1 and b.po_quantity>0 $order_status_cond  and a.task_type=6  group by a.job_no,a.task_number order by a.job_no"; 
	}
	$data_sql_2= sql_select($sql);
	    //echo $sql;die;
...
$leader_library=return_library_array( "select id, team_leader_name from lib_marketing_team", "id", "team_leader_name");
	//TEMPLATE_ID
	$taskSql =sql_select("select MAX(TEMPLATE_ID)  AS TEMPLATE_ID,JOB_NO,MIN(TASK_START_DATE) AS TASK_START_DATE,MIN(TASK_FINISH_DATE) AS TASK_FINISH_DATE,MIN(ACTUAL_START_DATE) AS ACTUAL_START_DATE,MIN(ACTUAL_FINISH_DATE) AS ACTUAL_FINISH_DATE,MIN(COMMIT_START_DATE) AS COMMIT_START_DATE,MIN(COMMIT_END_DATE) AS COMMIT_END_DATE from tna_process_mst where JOB_NO='$job_no' and TASK_NUMBER='$task_id' and task_type=6 GROUP BY JOB_NO"); 
	//echo "select max(template_id)  as template_id,JOB_NO,MIN(TASK_START_DATE) AS TASK_START_DATE,MIN(TASK_FINISH_DATE) AS TASK_FINISH_DATE,MIN(ACTUAL_START_DATE) AS ACTUAL_START_DATE,MIN(ACTUAL_FINISH_DATE) AS ACTUAL_FINISH_DATE,MIN(COMMIT_START_DATE) AS COMMIT_START_DATE,MIN(COMMIT_END_DATE) AS COMMIT_END_DATE from tna_process_mst where JOB_NO='$job_no' and TASK_NUMBER='$task_id' and task_type=6 GROUP BY JOB_NO";
	$template_id=$taskSql[0][TEMPLATE_ID];
	//echo $template_id.'SS';
	//echo "select JOB_NO,MIN(TASK_START_DATE) AS TASK_START_DATE,MIN(TASK_FINISH_DATE) AS TASK_FINISH_DATE,MIN(ACTUAL_START_DATE) AS ACTUAL_START_DATE,MIN(ACTUAL_FINISH_DATE) AS ACTUAL_FINISH_DATE,MIN(COMMIT_START_DATE) AS COMMIT_START_DATE,MIN(COMMIT_END_DATE) AS COMMIT_END_DATE from tna_process_mst where JOB_NO='$job_no' and TASK_NUMBER='$task_id' and task_type=6 GROUP BY JOB_NO";
	
	$task_short_arr = return_library_array("select task_name,task_short_name from  lib_tna_task where STATUS_ACTIVE=1 and IS_DELETED=0","task_name","task_short_name");
	$sql_tna_comment=sql_select("select max(a.id) as id,a.mer_comments  from tna_progress_comments a,wo_po_break_down b where  b.id=a.order_id and a.task_id='$task_id' and b.job_no_mst='$job_no'  and a.task_type=6 group by a.mer_comments");
	//echo "select max(a.id) as id,a.mer_comments  from tna_progress_comments a,wo_po_break_down b where  b.id=a.order_id and a.task_id='$task_id' and b.job_no_mst='$job_no'  and a.task_type=6 group by a.mer_comments";
...
if($action=="generate_menual_date_change_report")
{
	...
	if( $tna_process_type==2 ){//Parcent base;
		...
		from lib_tna_task a where a.is_deleted = 0 and a.status_active=1 order by a.task_sequence_no asc";
	}
	else
	{
...
if($action=="generate_woven_tna_report_v2")
{
...
if($action=="task_his_dtls")
{
   ...
   foreach($dataArray as $sql_row)
   {
	...
	//-------------------------------------------------Fabric Booking
	$bookingFbSql ="select b.PRE_COST_FABRIC_COST_DTLS_ID,a.CURRENCY_ID,a.BOOKING_NO,a.BOOKING_DATE,a.PAY_MODE,a.SOURCE,a.SUPPLIER_ID,a.ITEM_CATEGORY,b.CONSTRUCTION,b.COPMPOSITION,b.GSM_WEIGHT,b.DIA_WIDTH, b.GREY_FAB_QNTY,b.FIN_FAB_QNTY, b.AMOUNT,B.PO_BREAK_DOWN_ID from WO_BOOKING_MST a, WO_BOOKING_dtls b where a.booking_no=b.BOOKING_NO and a.item_category=3   and b.JOB_NO='".$order_sql_res[0]['JOB_NO']."' and COMPANY_ID='".$order_sql_res[0]['COMPANY_NAME']."'  and B.PO_BREAK_DOWN_ID=$po_id AND a.IS_DELETED =0 and a.STATUS_ACTIVE=1 AND b.IS_DELETED =0 and b.STATUS_ACTIVE=1";
	 // echo $bookingFbSql;
	...
	<table border="1" rules="all" class="rpt_table">
		...
		<tbody>
			<?
			$i=1;
			foreach($booking_fb_data_arr['DATA'][3][3] as $key=>$row){
	...
	<table border="1" rules="all" class="rpt_table">
		...
		<tbody>
			<?
			$i=1;
			foreach($booking_fb_data_arr['DATA'][3][1] as $key=>$row){
				$row['AMOUNT']=$booking_fb_data_arr['AMOUNT'][$key];
				$row['WO_QNTY']=$booking_fb_data_arr['WO_QNTY'][$key];
				$row['REQ_WO_QNTY']=$booking_fb_data_arr['REQ_WO_QNTY'][$key];
				$row['DESCRIPTION']=$fabric_description_dtls_array[$row['PRE_COST_FABRIC_COST_DTLS_ID']];
	...
	<table border="1" rules="all" class="rpt_table">
		...
		<tbody>
			<?
			$i=1;
			foreach($finish_fab_rec_data_arr['DATA'][1] as $row){
			?>
			<tr>
				<td><?=$i;?></td>	
				<td><?=$row['BOOKING_NO'];?></td>	
...
if($action=="job_style_wise_fabric_and_trims_analysis_report"){
	...
	let fn_report_generated=(action)=>{
		...
		var data = "action="+action+"&cbo_company_id='<?=$cbo_company_name;?>'&cbo_buyer_id='0'&txt_style=''&txt_style_id=''&txt_order_no=''&txt_order_no_id='<?=$po_id;?>'&txt_date_from=''&txt_date_to=''&cbo_year='<?=date('Y');?>'&cbo_brand_id='0'&cbo_season_id='0'&cbo_season_year='0'&report_title=Job/Style Wise Fabric And Trims Analysis Report&operation=2";
		$('#report_container2').html('Wait loading...');
		http.open("POST","../../../order/reports/requires/job_or_style_wise_fabric_and_trims_analysis_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
