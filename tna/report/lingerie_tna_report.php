...
function fnc_generate_report_main( operation,rpt_type)
{
	...
	else if(rpt_type==1)
	{
		...
		action_type="generate_lingerie_tna_report";
	} 
	else if(rpt_type==2)
	{
	...
	else if(rpt_type==10)
	{
			...
			action_type="generate_lingerie_tna_report_v2";
	}
	else if(rpt_type==11)
	{
			if ( form_validation('cbo_company_name*txt_job_no','Company Name*Job No')==false )
	...
	http.open("POST","requires/lingerie_tna_report_controller.php",true);
...
{ 
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/lingerie_tna_report_controller.php?type='+type+'&mid='+id+'&po_id='+po_id+'&action=edit_update_tna'+'&permission='+permission, "TNA Update", 'width=640px,height=240px,center=1,resize=1,scrolling=0','../')
...
function progress_comment_popup(job_no,po_id,template_id,tna_process_type)
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/lingerie_tna_report_controller.php?job_no='+job_no+'&po_id='+po_id+'&template_id='+template_id+'&tna_process_type='+tna_process_type+'&action=update_tna_progress_comment'+'&permission='+permission, "TNA Progress Comment", 'width=1040px,height=460px,center=1,resize=1,scrolling=0','../')
...
function order_update_log_popup(job_no,po_id)
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/lingerie_tna_report_controller.php?job_no='+job_no+'&po_id='+po_id+'&action=po_update_history'+'&permission='+permission, "PO Update History", 'width=640px,height=300px,center=1,resize=1,scrolling=0','../')
...
function openmypage_task()
{
	...
	var page_link='requires/lingerie_tna_report_controller.php?action=task_surch&company='+company+'&tna_task='+tna_task+'&tna_task_id='+tna_task_id+'&tna_task_id_no='+tna_task_id_no+'&cbo_task_group='+cbo_task_group;   
...
function progress_comment_popup_style(job_no,po_id,template_id,tna_process_type)
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/lingerie_tna_report_controller.php?job_no='+job_no+'&po_id='+po_id+'&template_id='+template_id+'&tna_process_type='+tna_process_type+'&action=update_tna_progress_comment_style'+'&permission='+permission, "TNA Progress Comment", 'width=1040px,height=460px,center=1,resize=1,scrolling=0','../')
	emailwindow.onclose=function()
	{
		var theform=this.contentDoc.forms[0];
		var theemail=this.contentDoc.getElementById("selected_booking");
...
{ 
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/lingerie_tna_report_controller.php?task_data_str='+taskDataStr+'&action='+action+'&permission='+permission, "TNA Task Dtls", 'width='+width+'px,height=240px,center=1,resize=1,scrolling=0','../')
	emailwindow.onclose=function()
	{
		var theform=this.contentDoc.forms[0];
		var theemail=this.contentDoc.getElementById("selected_booking");
...
{ 
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/lingerie_tna_report_controller.php?task_data_str='+taskDataStr+'&action='+action+'&permission='+permission, "TNA Task Dtls", 'width='+width+'px,height=470px,center=1,resize=1,scrolling=0','../')
	emailwindow.onclose=function()
	{
		var theform=this.contentDoc.forms[0];
		var theemail=this.contentDoc.getElementById("selected_booking");
	...
	<fieldset style="width:1330px; text-align:left">
    	<table width="1330" class="rpt_table" border="1" rules="all" cellpadding="0" cellspacing="0">
           ...
           <tr class="general">
                <td align="center">
                	<?
					   echo create_drop_down( "cbo_company_name", 120, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, " load_drop_down( 'requires/lingerie_tna_report_controller', this.value, 'load_drop_down_buyer', 'buyer_td' ); get_php_form_data(this.value, 'set_print_button', 'requires/lingerie_tna_report_controller' ); " );
						?> 
                </td>
                 <td>
                	<?
           ...
           <tr>
           		<td colspan="15" height="40" valign="middle" align="center">
                    ...
                    <input type="button" class="formbutton" style="width:140px;" value="Style Follow Up Wvn" onClick="fnc_generate_report_main(3,9)" id="btn_style_wise_wvn" />
                
                
                	<!--Button action 10 for knit asis live. Don't active in devlop-->
                    <input type="button" class="formbutton" style="width:100px;" value="Report V2" onClick="fnc_generate_report_main(3,10)" id="btn_report_generate_10"/>
                    <input type="button" class="formbutton" style="width:150px;" value="Style Follow Up Short" onClick="fnc_generate_report_main(3,11)" id="btn_report_generate_11"/>
