<?
/*-------------------------------------------- Comments
Purpose			: 	This Report will create TNA Plan In Quantity Multy task
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("TNA Plan In Quantity Multy Task","../../", 1, 1, $unicode,'','');
...
function fnc_generate_report( operation)
{
	...
	if ( form_validation('cbo_company_name*tna_task_id*txt_date_from*txt_date_to','Company Name*Task Name*Date From*Date To')==false )
	{
		return;
	}
	
	var data="action=generate_report_summary&"+get_submitted_data_string('cbo_company_name*tna_task_id*cbo_buyer_name*cbo_job_year*txt_job_no*txt_order_no*cbo_shipment_status*cbo_order_status*txt_date_from*txt_date_to',"../../");
	freeze_window(operation);
	http.open("POST","requires/tna_plan_in_qnty_multi_task_report_controller.php",true);
...
function fnc_generate_report_main_reponse()
{
	if(http.readyState == 4) 
	{
		...
		$("#report_container_summary").html(reponse[0]);  
...
function generate_task_wise(str){
	var startEndDateArr=str.split('__');
		document.getElementById('task_id').value=startEndDateArr[0];
		document.getElementById('start_date').value=startEndDateArr[1];
		document.getElementById('end_date').value=startEndDateArr[2];
		fnc_generate_report_dtls(1);
}

function fnc_generate_report_dtls( operation)
{
	var job_no=$('#txt_job_no').val();
	var order_no=$('#txt_order_no').val();
	if ( form_validation('cbo_company_name*tna_task_id*txt_date_from*txt_date_to','Company Name*Task Name*Date From*Date To')==false )
	...
	var data="action=generate_report&"+get_submitted_data_string('cbo_company_name*task_id*cbo_buyer_name*cbo_job_year*txt_job_no*txt_order_no*cbo_shipment_status*cbo_order_status*start_date*end_date',"../../");
	freeze_window(operation);
	http.open("POST","requires/tna_plan_in_qnty_multi_task_report_controller.php",true);
	http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	http.send(data);
	http.onreadystatechange = fnc_generate_report_dtls_reponse;
}

function fnc_generate_report_dtls_reponse()
{
	if(http.readyState == 4) 
	{
		var reponse=trim(http.responseText).split('****');
...
{ 
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/tna_plan_in_qnty_multi_task_report_controller.php?type='+type+'&mid='+id+'&po_id='+po_id+'&action=edit_update_tna'+'&permission='+permission, "TNA Update", 'width=640px,height=240px,center=1,resize=1,scrolling=0','../')
...
function progress_comment_popup(job_no,po_id,template_id,tna_process_type)
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/tna_plan_in_qnty_multi_task_report_controller.php?job_no='+job_no+'&po_id='+po_id+'&template_id='+template_id+'&tna_process_type='+tna_process_type+'&action=update_tna_progress_comment'+'&permission='+permission, "TNA Progress Comment", 'width=1040px,height=460px,center=1,resize=1,scrolling=0','../')
...
function openmypage_task()
{
	...
	var page_link='requires/tna_plan_in_qnty_multi_task_report_controller.php?action=task_surch&company='+company+'&tna_task='+tna_task+'&tna_task_id='+tna_task_id+'&tna_task_id_no='+tna_task_id_no;  
...
<div align="center"> 
	...
	<fieldset style="width:1160px; text-align:left">
    	<table width="1150" class="rpt_table" border="1" rules="all" cellpadding="0" cellspacing="0">
        	<thead>
            	<tr>
                    ...
                    <th width="130">Task</th>
                    <th width="130" id="buyer_caption">Buyer Name</th>
                    ...
                    <th><input type="reset" id="btn_rest" value="Reset" onClick="reset_form('','report_container*print_button','cbo_company_name*tna_task_id*cbo_buyer_name*cbo_job_year*txt_job_no*txt_order_no*cbo_shipment_status*cbo_order_status*txt_date_from*txt_date_to','','','');" style="width:100px" class="formbutton" /></th>
               </tr>
            </thead>
           <tr class="general">
                <td align="center">
                <?
                	echo create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, " load_drop_down( 'requires/tna_plan_in_qnty_multi_task_report_controller', this.value, 'load_drop_down_buyer', 'buyer_td' ); " );
                ?> 
                </td>
                <td align="center">
                <input style="width:130px;"  name="txt_taks_name" id="txt_taks_name"  ondblclick="openmypage_task()"  class="text_boxes" placeholder="Browse" readonly/>   
                ...
                <td align="center"><input type="button" class="formbutton" style="width:100px" value="Show" onClick="fnc_generate_report(1)" id="btn_report_generate"/></td>
        ...
        <div style="margin-top:5px" id="report_container_summary"></div>
