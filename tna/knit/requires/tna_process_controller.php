...
function gbl_temp_engine_join($user_id,$entry_form_id,$joinField){
	return " and gbl.ENTRY_FORM=$entry_form_id and gbl.USER_ID=$user_id and gbl.REF_VAL=$joinField";
}
...
if ($action == "tna_process") {
	...
	$sql = "SELECT COMPANY_NAME,TNA_INTEGRATED,TNA_PROCESS_TYPE,TEXTILE_TNA_PROCESS_BASE,TNA_PROCESS_START_DATE,VARIABLE_LIST FROM  variable_order_tracking WHERE status_active =1 and is_deleted = 0 and variable_list in(14,31,43)";
	//echo $sql;die;
	$resultRes = sql_select($sql);
	$variable_settings = array();
	foreach ($resultRes as $row) {
		if($row['VARIABLE_LIST'] == 14){
			$variable_settings[$row['VARIABLE_LIST']][$row['COMPANY_NAME']] = $row['TNA_INTEGRATED'];
		}
		else if($row['VARIABLE_LIST'] == 31){
			$variable_settings[$row['VARIABLE_LIST']][$row['COMPANY_NAME']] = $row['TNA_PROCESS_TYPE'];
			$is_pub_shipment_date_arr[$row['VARIABLE_LIST']][$row['COMPANY_NAME']] = $row['TEXTILE_TNA_PROCESS_BASE'];
		}
		else if($row['VARIABLE_LIST'] == 43){
			$variable_settings[$row['VARIABLE_LIST']][$row['COMPANY_NAME']] = $row['TNA_PROCESS_START_DATE'];
		}
	...
	else if ($tna_process_type == 1) //Templae base;
	{
		...
		foreach ($result as $row) {
			$tna_task_name[$row['ID']] = $row['TASK_NAME'];
			$tna_task_details[$row['TASK_NAME']]['task_catagory'] = $row['TASK_CATAGORY'];
			$tna_task_details[$row['TASK_NAME']]['id'] = $row['ID'];
			$tna_task_details[$row['TASK_NAME']]['task_name'] = $row['TASK_NAME'];
			$tna_task_details[$row['TASK_NAME']]['task_short_name'] = $row['TASK_SHORT_NAME'];
			$tna_task_details[$row['TASK_NAME']]['task_type'] = $row['TASK_TYPE'];
			$tna_task_details[$row['TASK_NAME']]['module_name'] = $row['MODULE_NAME'];
			$tna_task_details[$row['TASK_NAME']]['link_page'] = $row['LINK_PAGE'];
			$tna_task_details[$row['TASK_NAME']]['penalty'] = $row['PENALTY'];
			$tna_task_details[$row['TASK_NAME']]['completion_percent'] = $row['COMPLETION_PERCENT'];

			$tna_task_seq_arr[$row['TASK_NAME']] = $row['TASK_SEQUENCE_NO'];
		}
	}
	unset($result);
	...
	foreach ($company_array as $cbo_company) {
		...
		$tna_process_type = $variable_settings[31][$cbo_company];
		$tna_process_start_date = $variable_settings[43][$cbo_company];
		...
		foreach ($result as $row) {
			if ($template[$row['TASK_TEMPLATE_ID']] == '') {
				...
				if ($row['FOR_SPECIFIC'] == 0) {
					...
					if (!in_array($row['FOR_SPECIFIC'], $tna_template_spc)) {
						$j = 0;
						$tna_template_spc[] = $row['FOR_SPECIFIC'];
					}
					$tna_template_buyer[$row['FOR_SPECIFIC']][$j]['lead'] = $row['LEAD_TIME'];
					$tna_template_buyer[$row['FOR_SPECIFIC']][$j]['id'] = $row['TASK_TEMPLATE_ID'];

					$tna_template_company_arr[$row['COMPANY_ID']][$row['FOR_SPECIFIC']][$j]['lead'] = $row['LEAD_TIME'];
					$tna_template_company_arr[$row['COMPANY_ID']][$row['FOR_SPECIFIC']][$j]['id'] = $row['TASK_TEMPLATE_ID'];
			...
			$active_task_arr[$row['TASK_NAME']] = $row['TASK_NAME'];
        ...
        $is_pub_shipment_date = $is_pub_shipment_date_arr[31][$cbo_company]; 
		...
		foreach($po_data_chunk_array as $data_array){
			...
			$job_id_array = array();$tmp_po_array = array();//$job_no_array = array();
			foreach ($data_array as $row) {
				$job_id_array[$row['JOB_ID']] = $row['JOB_ID'];
				$tmp_po_array[$row['ID']] = $row['ID'];
				$job_no_array[$row['JOB_NO_MST']] = $row['JOB_NO_MST'];
			}
			...
			$trims_data = $trims->getAmountArray_by_orderSourceidAndItemidType();
			...
			$con = connect();
			execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ENTRY_FORM=$eform",1);
			oci_commit($con);
			fnc_tempengine("GBL_TEMP_ENGINE", $user_id, $eform, 1, $order_id_array, $empty_arr);
			//disconnect($con);
			//---------------------------------end;
			...
			$pi_sql = " select  b.ORDER_ID,MAX(a.PI_DATE) AS MAX_DATE,MIN(a.PI_DATE) AS MIN_DATE, SUM(b.QUANTITY) AS QTY  from com_pi_master_details A,com_pi_item_details B,GBL_TEMP_ENGINE gbl where a.id=b.pi_id  and a.ITEM_CATEGORY_ID = 4 and a.IMPORTER_ID = $cbo_company and A.status_active=1 and A.is_deleted=0 and b.status_active=1 and b.is_deleted=0 " . gbl_temp_engine_join($user_id,$eform,'b.ORDER_ID') . " GROUP BY b.ORDER_ID";
			 //echo $pi_sql;die;
			...
			$sql = "select a.BODY_PART_ID,a.FABRIC_SOURCE,a.job_no,a.COLOR_TYPE_ID,b.PO_BREAK_DOWN_ID,a.id as pre_cost_fabric_cost_dtls_id,a.item_number_id,a.fab_nature_id,b.color_size_table_id, b.color_number_id,b.gmts_sizes as size_number_id, b.cons, b.requirment,a.rate FROM wo_pre_cost_fabric_cost_dtls a, wo_pre_cos_fab_co_avg_con_dtls b,GBL_TEMP_ENGINE gbl WHERE a.job_no=b.job_no and  a.id=b.pre_cost_fabric_cost_dtls_id " . gbl_temp_engine_join($user_id,$eform, 'b.po_break_down_id') . " and a.status_active=1 and a.is_deleted=0 order by a.id ";
			...
			//Trims Booking-------------------
			$trimsBookingSql = "SELECT a.SOURCE,c.TRIM_TYPE,b.PO_BREAK_DOWN_ID,max(a.BOOKING_DATE) as MAX_BOOKING_DATE,min(a.BOOKING_DATE) as MIN_BOOKING_DATE,sum(b.WO_QNTY) as WO_QNTY  FROM  WO_BOOKING_MST a,WO_BOOKING_DTLS b,LIB_ITEM_GROUP c,GBL_TEMP_ENGINE gbl WHERE b.TRIM_GROUP=c.id and a.BOOKING_NO=b.BOOKING_NO and b.is_deleted = 0 AND b.status_active = 1 and b.BOOKING_TYPE in(2,8) and c.TRIM_TYPE in(1,2) and c.ITEM_CATEGORY=4 " . gbl_temp_engine_join($user_id,$eform, 'b.PO_BREAK_DOWN_ID') . "  group by b.PO_BREAK_DOWN_ID,a.SOURCE,c.TRIM_TYPE";
			...
			$sql = "select a.color_type_id,b.po_break_down_id,sum((b.requirment/b.PCS)*c.PLAN_CUT_QNTY) as requirment from wo_pre_cost_fabric_cost_dtls a,wo_pre_cos_fab_co_avg_con_dtls b,WO_PO_COLOR_SIZE_BREAKDOWN c,GBL_TEMP_ENGINE gbl WHERE c.id=b.COLOR_SIZE_TABLE_ID and a.id=b.pre_cost_fabric_cost_dtls_id and a.company_id=$cbo_company " . gbl_temp_engine_join($user_id,$eform, 'b.po_break_down_id') . "  and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 and c.status_active =1 and c.is_deleted=0 group by a.color_type_id,b.po_break_down_id";
			...
			//Sample Part..............
			$sql = "select a.PO_BREAK_DOWN_ID,count(a.sample_type_id) as tot_sample_type_id from wo_po_sample_approval_info a,GBL_TEMP_ENGINE gbl WHERE a.sample_type_id in(18,45) and a.status_active =1 and a.is_deleted=0  " . gbl_temp_engine_join($user_id,$eform, 'a.po_break_down_id') . " group by a.po_break_down_id";
			...
			$sql = "select a.EMB_NAME,b.PO_BREAK_DOWN_ID,sum(b.amount) as AMOUNT from wo_pre_cost_embe_cost_dtls a,wo_pre_cos_emb_co_avg_con_dtls b,GBL_TEMP_ENGINE gbl WHERE a.id=b.PRE_COST_EMB_COST_DTLS_ID  and a.STATUS_ACTIVE=1 and a.IS_DELETED=0 and b.STATUS_ACTIVE=1 and b.IS_DELETED=0  " . gbl_temp_engine_join($user_id,$eform, 'b.po_break_down_id') . " group by a.EMB_NAME,b.PO_BREAK_DOWN_ID";
			...
			$sql = "SELECT a.TASK_NUMBER,a.PO_NUMBER_ID,a.ACTUAL_START_DATE,a.ACTUAL_FINISH_DATE,a.TASK_START_DATE,a.TASK_FINISH_DATE ,a.PLAN_START_FLAG,a.PLAN_FINISH_FLAG FROM tna_plan_actual_history a,GBL_TEMP_ENGINE gbl WHERE a.status_active =1 and a.is_deleted=0 and a.task_type=1 " . gbl_temp_engine_join($user_id,$eform, 'a.po_number_id') . " order by a.id";
			...
			$sql = "SELECT a.ID,a.job_no,a.PO_NUMBER_ID,a.task_category,a.TASK_NUMBER,a.ACTUAL_START_DATE,a.ACTUAL_FINISH_DATE,a.TEMPLATE_ID FROM tna_process_mst a,GBL_TEMP_ENGINE gbl WHERE a.status_active =1 and a.is_deleted = 0  and a.task_type=1 " . gbl_temp_engine_join($user_id,$eform, 'a.po_number_id') . "";
			...
			$sample_sql = "SELECT a.id as CID,a.PO_BREAK_DOWN_ID,SAMPLE_TYPE,a.approval_status_date as MAXDATE,a.approval_status_date as MINDATE FROM wo_po_sample_approval_info  a, lib_sample b,wo_po_color_size_breakdown c,GBL_TEMP_ENGINE gbl WHERE sample_type in (2,3,4,7,8,9,10,11,12,13,14,20) and c.id=a.color_number_id and a.po_break_down_id=c.po_break_down_id and a.job_no_mst=c.job_no_mst and c.status_active=1 AND c.is_deleted = 0 and b.id=a.SAMPLE_TYPE_ID and a.is_deleted = 0 and a.status_active=1 and b.is_deleted = 0 and b.BUSINESS_NATURE=2 and b.status_active=1 and c.status_active=1  and c.is_deleted = 0 " . gbl_temp_engine_join($user_id,$eform, 'a.PO_BREAK_DOWN_ID') . " ";
			//echo $sample_sql;die;
			...
			$sample_sub_sql = "SELECT count(a.id) as CID,a.PO_BREAK_DOWN_ID,sample_type,max(submitted_to_buyer) as MAXDATE,min(submitted_to_buyer) as MINDATE FROM wo_po_sample_approval_info  a, lib_sample b,wo_po_color_size_breakdown c,GBL_TEMP_ENGINE gbl WHERE sample_type in (2,3,4,7,8,9,10,11,12,13,14,20) and c.id=a.color_number_id and a.po_break_down_id=c.po_break_down_id and a.job_no_mst=c.job_no_mst and c.status_active=1 AND c.is_deleted = 0 and b.id=a.SAMPLE_TYPE_ID and a.is_deleted = 0 and a.status_active=1 and b.is_deleted = 0 and b.status_active=1 and b.BUSINESS_NATURE=2 " . gbl_temp_engine_join($user_id,$eform, 'a.PO_BREAK_DOWN_ID') . " and a.submitted_to_buyer is not null group by b.sample_type,a.po_break_down_id order by po_break_down_id asc";
			...
			$sample_app_sql = "SELECT count(a.id) as CID,a.PO_BREAK_DOWN_ID,sample_type,max(approval_status_date) as MAXDATE,min(approval_status_date) as MINDATE FROM wo_po_sample_approval_info  a, lib_sample b,wo_po_color_size_breakdown c,GBL_TEMP_ENGINE gbl WHERE sample_type in (2,3,4,7,8,9,10,11,12,13,14,20) and c.id=a.color_number_id and a.po_break_down_id=c.po_break_down_id and a.job_no_mst=c.job_no_mst and c.status_active=1 AND c.is_deleted = 0 and b.id=a.SAMPLE_TYPE_ID and a.is_deleted = 0 and a.status_active=1 and b.is_deleted = 0 and b.status_active=1 and a.APPROVAL_STATUS=3  and b.BUSINESS_NATURE=2 " . gbl_temp_engine_join($user_id,$eform, 'a.PO_BREAK_DOWN_ID') . " group by b.sample_type,a.po_break_down_id order by po_break_down_id asc";
			...
			$sql_knit_finish_rec = "select a.PO_BREAKDOWN_ID,c.booking_id, c.RECEIVE_BASIS,d.SOURCE,  sum(a.quantity) as QTY,min(c.RECEIVE_DATE) MINDATE, max(c.RECEIVE_DATE) MAXDATE from order_wise_pro_details a,pro_finish_fabric_rcv_dtls b,inv_receive_master c,COM_PI_MASTER_DETAILS d,GBL_TEMP_ENGINE gbl where d.id=c.BOOKING_ID and c.RECEIVE_BASIS=1 and a.dtls_id=b.id and b.mst_id=c.id  " . gbl_temp_engine_join($user_id,$eform, 'a.po_breakdown_id') . " and a.entry_form=37 and a.status_active=1 and a.is_deleted=0  group by a.po_breakdown_id,c.booking_id,c.RECEIVE_BASIS,d.SOURCE union all select a.PO_BREAKDOWN_ID,c.booking_id, c.RECEIVE_BASIS,d.SOURCE,  sum(a.quantity) as QTY,min(c.RECEIVE_DATE) MINDATE, max(c.RECEIVE_DATE) MAXDATE from order_wise_pro_details a,pro_finish_fabric_rcv_dtls b,inv_receive_master c,wo_booking_mst d,GBL_TEMP_ENGINE gbl where d.id=c.BOOKING_ID and c.RECEIVE_BASIS in( 2,11) and a.dtls_id=b.id and b.mst_id=c.id  " . gbl_temp_engine_join($user_id,$eform, 'a.po_breakdown_id') . " and a.entry_form=37 and a.status_active=1 and a.is_deleted=0  group by a.po_breakdown_id,c.booking_id,c.RECEIVE_BASIS,d.SOURCE";
			...
			$sql = "select b.PO_BREAK_DOWN_ID, sum(b.qnty) as qnty, min(a.allocation_date) as mindate, max(a.INSERT_DATE) as maxdate from inv_material_allocation_mst a, inv_material_allocation_dtls b ,GBL_TEMP_ENGINE gbl
	where a.id=b.mst_id and b.item_category=1 and  a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  " . gbl_temp_engine_join($user_id,$eform, 'b.po_break_down_id') . "  group by b.po_break_down_id";
	//echo $sql;die;

			$result = sql_select($sql);
			foreach ($result as $row) {
				$tna_task_update_data[$row['PO_BREAK_DOWN_ID']][48]['max_start_date'] =  date("d-M-Y", strtotime($row[csf("maxdate")]));//$row[csf("maxdate")];
			...
			$yarnReqSql = "select b.ORDER_ID, sum(b.REQUISITION_QTY) as QTY,min(a.REQUISITION_DATE) as MIN_DATE,max(a.REQUISITION_DATE) as MAX_DATE  from PPL_YARN_REQUISITION_ENTRY a,PPL_YARN_REQUISITION_BREAKDOWN b,GBL_TEMP_ENGINE gbl where a.REQUISITION_NO=b.REQUISITION_ID and b.status_active=1 AND b.is_deleted=0  " . gbl_temp_engine_join($user_id,$eform, 'b.ORDER_ID') . " group by b.ORDER_ID";
			...
			$sql_task = "SELECT count(a.id) as cid, a.po_break_down_id, max(a.submitted_to_buyer) as max_submitted_to_buyer,min(a.submitted_to_buyer) as min_submitted_to_buyer,max(a.approval_status_date) as max_approval_status_date,min(a.approval_status_date) as min_approval_status_date, LISTAGG(a.color_name_id, ',') WITHIN GROUP (ORDER BY a.color_name_id) AS color_name_id FROM wo_po_lapdip_approval_info a,GBL_TEMP_ENGINE gbl WHERE a.is_deleted = 0 and a.status_active=1 " . gbl_temp_engine_join($user_id,$eform, 'a.po_break_down_id') . "  group by a.po_break_down_id order by a.po_break_down_id asc";
			 //echo $sql_task;die;
			...
			$sql_task = "SELECT a.id as cid,a.po_break_down_id ,a.submitted_to_buyer,a.approval_status_date FROM wo_po_lapdip_approval_info a,GBL_TEMP_ENGINE gbl WHERE a.is_deleted = 0 and a.status_active=1  " . gbl_temp_engine_join($user_id,$eform, 'a.po_break_down_id') . " ";
			//echo $sql_task;die;
			...
			$sql_task = "SELECT count(a.id) as cid,po_break_down_id,max(submitted_to_buyer) as max_submitted_to_buyer,min(submitted_to_buyer) as min_submitted_to_buyer,max(approval_status_date) as max_approval_status_date,min(approval_status_date) as min_approval_status_date,trim_type FROM lib_item_group b, wo_po_trims_approval_info a,GBL_TEMP_ENGINE gbl WHERE b.id=a.accessories_type_id and a.is_deleted = 0 and a.status_active=1 and b.is_deleted = 0 and b.status_active=1 " . gbl_temp_engine_join($user_id,$eform, 'po_break_down_id') . " group by po_break_down_id,trim_type order by po_break_down_id asc";
			// echo $sql_task;die;
			...
			$sql_task = "SELECT count(a.id) as CID,PO_BREAK_DOWN_ID,trim_type FROM lib_item_group b, wo_po_trims_approval_info a,GBL_TEMP_ENGINE gbl
		WHERE b.id=a.accessories_type_id and approval_status_date is not null  and a.is_deleted = 0 and a.status_active=1   and b.is_deleted = 0 and b.status_active=1 " . gbl_temp_engine_join($user_id,$eform, 'po_break_down_id') . " group by po_break_down_id,trim_type order by po_break_down_id asc";
			 //echo $sql_task;die;
			...
			$sql_task = "SELECT count(a.id) as cid,a.po_break_down_id,max(a.submitted_to_buyer) as max_submitted_to_buyer,min(a.submitted_to_buyer) as min_submitted_to_buyer,max(a.approval_status_date) as max_approval_status_date,min(a.approval_status_date) as min_approval_status_date,a.embellishment_id FROM wo_po_embell_approval a,GBL_TEMP_ENGINE gbl WHERE is_deleted = 0 and status_active=1 " . gbl_temp_engine_join($user_id,$eform, 'a.po_break_down_id') . " group by a.po_break_down_id,a.embellishment_id order by a.po_break_down_id asc";
			//echo $sql_task;die;
			...
			$sql_task = "SELECT count(a.id) as cid,po_break_down_id FROM wo_po_embell_approval a,GBL_TEMP_ENGINE gbl WHERE approval_status_date is not null   and is_deleted = 0 and status_active=1 " . gbl_temp_engine_join($user_id,$eform, 'po_break_down_id') . " group by po_break_down_id,embellishment_id order by po_break_down_id asc";
			...
			$sql = "SELECT distinct (po_break_down_id) as  po_break_down_id,color_type_id FROM wo_pre_cost_fabric_cost_dtls a, wo_pre_cos_fab_co_avg_con_dtls b,GBL_TEMP_ENGINE gbl WHERE a.id=b.pre_cost_fabric_cost_dtls_id and b.cons>0 and a.color_type_id in (2,3,4,6,5) and a.status_active =1 and a.is_deleted=0 " . gbl_temp_engine_join($user_id,$eform, 'b.po_break_down_id') . "";
			//echo $sql;die;
			...
			$sql_task = "SELECT A.ITEM_CATEGORY,A.BOOKING_TYPE,A.SOURCE ,B.PO_BREAK_DOWN_ID,0 as IS_APPROVED, sum(wo_qnty) as TFB_QNTY, sum(b.amount/nullif(b.exchange_rate,0))  as TFB_AMOUNT, sum(grey_fab_qnty) as GFB_QNTY, min(a.booking_date) as START_DATE, max(a.booking_date) as END_DATE FROM  wo_booking_mst a, wo_booking_dtls b,GBL_TEMP_ENGINE gbl WHERE a.booking_no=b.booking_no and a.item_category in ( 2,4,12,3 ) and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 and (a.is_short=2 or a.ITEM_FROM_PRECOST=1) " . gbl_temp_engine_join($user_id,$eform, 'b.po_break_down_id') . "  group by b.po_break_down_id,a.item_category,a.booking_type,a.SOURCE union all SELECT 0 as ITEM_CATEGORY,0 as BOOKING_TYPE,0 as SOURCE ,B.PO_BREAK_DOWN_ID,a.IS_APPROVED, sum(wo_qnty) as TFB_QNTY, sum(b.amount/nullif(b.exchange_rate,0))  as TFB_AMOUNT, sum(grey_fab_qnty) as GFB_QNTY, min(a.booking_date) as START_DATE,  max(a.booking_date) as END_DATE FROM  wo_booking_mst a, wo_booking_dtls b,GBL_TEMP_ENGINE gbl WHERE a.booking_no=b.booking_no and a.item_category in ( 2,4,12,3 ) and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 and (a.is_short=2 or a.ITEM_FROM_PRECOST=1) " . gbl_temp_engine_join($user_id,$eform, 'b.po_break_down_id') . " and A.IS_APPROVED=1  group by b.po_break_down_id,A.IS_APPROVED";
			// echo $sql_task;die;
				...
				if ($row['IS_APPROVED'] == 1) {
					...
					if ($row['SOURCE'] == 1) {
						...
						if ($row[csf("item_category")] == 2) {
							$tna_task_update_data[$row[csf("po_break_down_id")]][276]['max_start_date'] = $row[csf("end_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][276]['min_start_date'] = $row[csf("start_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][276]['doneqnty'] += $row[csf("gfb_qnty")];

						} else if ($row[csf("item_category")] == 3) {
							$tna_task_update_data[$row[csf("po_break_down_id")]][302]['max_start_date'] = $row[csf("end_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][302]['min_start_date'] = $row[csf("start_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][302]['doneqnty'] += $row['GFB_QNTY'];
						} else if ($row[csf("item_category")] == 4) {
							$tna_task_update_data[$row[csf("po_break_down_id")]][278]['max_start_date'] = $row[csf("end_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][278]['min_start_date'] = $row[csf("start_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][278]['doneqnty'] += $row['TFB_QNTY'];
						} else if ($row[csf("item_category")] == 12) {
							$tna_task_update_data[$row[csf("po_break_down_id")]][33]['max_start_date'] = $row[csf("end_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][33]['min_start_date'] = $row[csf("start_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][33]['doneqnty'] += $row['TFB_QNTY'];
						}
	

						if ($row[csf("booking_type")] == 1 && $row[csf("item_category")] == 2) {
							$tna_task_update_data[$row[csf("po_break_down_id")]][31]['max_start_date'] = $row[csf("end_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][31]['min_start_date'] = $row[csf("start_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][31]['doneqnty'] += $row[csf("gfb_qnty")];
						} 
						else if ($row[csf("booking_type")] == 4 && $row[csf("item_category")] == 2) {
							// $tna_task_update_data[$row[csf("po_break_down_id")]][30]['max_start_date'] = $row[csf("end_date")];
							// $tna_task_update_data[$row[csf("po_break_down_id")]][30]['min_start_date'] = $row[csf("start_date")];
							// $tna_task_update_data[$row[csf("po_break_down_id")]][30]['doneqnty'] += $row[csf("gfb_qnty")];
						} 
						else if ($row[csf("booking_type")] == 4 && $row[csf("item_category")] == 3) {
							$tna_task_update_data[$row[csf("po_break_down_id")]][269]['max_start_date'] = $row[csf("end_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][269]['min_start_date'] = $row[csf("start_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][269]['doneqnty'] += $row[csf("gfb_qnty")];
						}
						//echo $tsktype;die;
					} 
					else 
					{
						if ($row[csf("item_category")] == 2) {
							...
							$tna_task_update_data[$row[csf("po_break_down_id")]][305]['doneqnty'] += $row[csf("gfb_qnty")];
						...
						elseif ($row[csf("item_category")] == 3) {
							$tna_task_update_data[$row[csf("po_break_down_id")]][34]['max_start_date'] = $row[csf("end_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][34]['min_start_date'] = $row[csf("start_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][34]['doneqnty'] += $row[csf("gfb_qnty")];
						}
						 elseif ($row[csf("item_category")] == 4) {
							...
							$tna_task_update_data[$row[csf("po_break_down_id")]][307]['doneqnty'] += $row['TFB_AMOUNT'];
							//.....................................end;

							$tna_task_update_data[$row[csf("po_break_down_id")]][32]['max_start_date'] = $row[csf("end_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][32]['min_start_date'] = $row[csf("start_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][32]['doneqnty'] += $row['TFB_AMOUNT'];

						} 
						elseif ($row[csf("item_category")] == 12) {
							$tna_task_update_data[$row[csf("po_break_down_id")]][33]['max_start_date'] = $row[csf("end_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][33]['min_start_date'] = $row[csf("start_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][33]['doneqnty'] += $row['TFB_QNTY'];
						}
						...
						if ($row[csf("booking_type")] == 1 && $row[csf("item_category")] == 2) {
							$tna_task_update_data[$row[csf("po_break_down_id")]][31]['max_start_date'] = $row[csf("end_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][31]['min_start_date'] = $row[csf("start_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][31]['doneqnty'] += $row['GFB_QNTY'];
						} 
						elseif ($row[csf("booking_type")] == 4 && $row[csf("item_category")] == 2) {
							// $tna_task_update_data[$row[csf("po_break_down_id")]][30]['max_start_date'] = $row[csf("end_date")];
							// $tna_task_update_data[$row[csf("po_break_down_id")]][30]['min_start_date'] = $row[csf("start_date")];
							// $tna_task_update_data[$row[csf("po_break_down_id")]][30]['doneqnty'] += $row['GFB_QNTY'];	
						} 
						else if ($row[csf("booking_type")] == 4 && $row[csf("item_category")] == 3) {
							$tna_task_update_data[$row[csf("po_break_down_id")]][269]['max_start_date'] = $row[csf("end_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][269]['min_start_date'] = $row[csf("start_date")];
							$tna_task_update_data[$row[csf("po_break_down_id")]][269]['doneqnty'] += $row['GFB_QNTY'];
						}
					}
			...
			$sql_task = "SELECT a.PO_BREAK_DOWN_ID,0 as IS_APPROVED, sum(wo_qnty) as TFB_QNTY, sum(b.amount/nullif(b.exchange_rate,0))  as TFB_AMOUNT, sum(grey_fab_qnty) as GFB_QNTY, min(a.booking_date) as MIN_DATE, max(a.booking_date) as MAX_DATE FROM  wo_booking_mst a, wo_booking_dtls b,GBL_TEMP_ENGINE gbl WHERE a.booking_no=b.booking_no and a.item_category=2 and A.BOOKING_TYPE=4 and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 and (a.is_short=2 or a.ITEM_FROM_PRECOST=1) " . gbl_temp_engine_join($user_id,$eform, 'b.po_break_down_id') . "  group by a.po_break_down_id";
			//echo $sql_task;die;
			$result = sql_select($sql_task);
			$purchase_update_task = array();
			foreach ($result as $row) 
			{
				foreach(explode(',',$row['PO_BREAK_DOWN_ID']) as $PO_ID){
					$tna_task_update_data[$PO_ID][30]['max_start_date'] = $row['MAX_DATE'];
					$tna_task_update_data[$PO_ID][30]['min_start_date'] = $row['MIN_DATE'];
					$tna_task_update_data[$PO_ID][30]['doneqnty'] += $row['GFB_QNTY'];
					$tna_task_update_data[$PO_ID][30]['reqqnty'] += $row['GFB_QNTY'];
				}
			...
			// print_r($tna_task_update_data[601][30]);die;


			//print_r($purchase_update_task); die;
			// Inventory Update Data

			$sql = "SELECT b.po_breakdown_id,b.entry_form, min(a.transaction_date) mindate, max(a.transaction_date) maxdate, sum(quantity) as prod_qntry FROM inv_transaction a,  order_wise_pro_details b,GBL_TEMP_ENGINE gbl where  b.trans_id=a.id and b.entry_form in (2,7,22,37,3,58,18,17,61,68,19 ) and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 " . gbl_temp_engine_join($user_id,$eform, 'b.po_breakdown_id') . " group by b.po_breakdown_id,b.entry_form order by b.po_breakdown_id";
			...
			$yarnIssueYDSql = "SELECT b.PO_BREAKDOWN_ID, min(a.transaction_date) MINDATE, max(a.transaction_date) MAXDATE, sum(quantity) as PROD_QNTRY FROM inv_transaction a,  order_wise_pro_details b,GBL_TEMP_ENGINE gbl where  b.trans_id=a.id and b.entry_form=3 and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 " . gbl_temp_engine_join($user_id,$eform, 'b.po_breakdown_id') . " and b.ISSUE_PURPOSE=2 group by b.po_breakdown_id";
			...
			$sql = "SELECT a.ORDER_UOM,b.po_breakdown_id, min(a.transaction_date) mindate, max(a.transaction_date) maxdate, sum(quantity) as prod_qntry,d.trim_type FROM inv_transaction a,  order_wise_pro_details b, product_details_master c , lib_item_group d,GBL_TEMP_ENGINE gbl where a.prod_id=c.id and b.trans_id=a.id and c.item_group_id=d.id and d.trim_type in (1,2) and b.entry_form in ( 24 )  and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 and c.status_active =1 and c.is_deleted=0 and d.status_active =1 and d.is_deleted=0 " . gbl_temp_engine_join($user_id,$eform, 'b.po_breakdown_id') . " group by a.ORDER_UOM,b.po_breakdown_id,d.trim_type union all SELECT 0 ORDER_UOM,b.po_breakdown_id, min(a.transaction_date) mindate, max(a.transaction_date) maxdate, 0 as prod_qntry,d.trim_type 
		FROM inv_transaction a,  order_wise_pro_details b, product_details_master c , lib_item_group d,GBL_TEMP_ENGINE gbl
		where a.prod_id=c.id and b.trans_id=a.id and c.item_group_id=d.id and d.trim_type in (1,2) and b.entry_form in ( 24 )  and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 and c.status_active =1 and c.is_deleted=0 and d.status_active =1 and d.is_deleted=0 " . gbl_temp_engine_join($user_id,$eform, 'b.po_breakdown_id') . " group by b.po_breakdown_id,d.trim_type";
			...
			$sewing_finish_trims_sql = "SELECT e.SOURCE,b.PO_BREAKDOWN_ID, min(a.transaction_date) MINDATE, max(a.transaction_date) MAXDATE, sum(quantity) as QTY,d.TRIM_TYPE FROM inv_transaction a,  order_wise_pro_details b, product_details_master c , lib_item_group d,inv_receive_master e,GBL_TEMP_ENGINE gbl where e.id=a.mst_id and a.prod_id=c.id and b.trans_id=a.id and c.item_group_id=d.id and d.trim_type in (1,2) and b.entry_form=24 " . gbl_temp_engine_join($user_id,$eform, 'b.po_breakdown_id') . "  and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 and c.status_active =1 and c.is_deleted=0 and d.status_active =1 and d.is_deleted=0 and e.status_active =1 and e.is_deleted=0 group by b.po_breakdown_id,d.trim_type,e.source order by b.po_breakdown_id"; //echo $sewing_finish_trims_sql;die;
			...
			$sql = "SELECT b.po_breakdown_id,b.entry_form, min(a.receive_date) mindate, max(a.receive_date) maxdate, sum(quantity) as prod_qntry FROM inv_receive_master a,  order_wise_pro_details b, pro_finish_fabric_rcv_dtls c,GBL_TEMP_ENGINE gbl where  c.mst_id=a.id and c.id=b.dtls_id and b.entry_form in ( 7 ) and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 and c.status_active =1 and c.is_deleted=0 " . gbl_temp_engine_join($user_id,$eform, 'b.po_breakdown_id') . " group by b.po_breakdown_id,b.entry_form order by b.po_breakdown_id";
			...
			$sql = "SELECT b.po_breakdown_id,b.entry_form, min(a.receive_date) mindate, max(a.receive_date) maxdate, sum(quantity) as prod_qntry FROM inv_receive_master a,  order_wise_pro_details b, pro_grey_prod_entry_dtls c,GBL_TEMP_ENGINE gbl where  c.mst_id=a.id and c.id=b.dtls_id and b.entry_form in ( 2 )  and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 and c.status_active =1 and c.is_deleted=0 " . gbl_temp_engine_join($user_id,$eform, 'b.po_breakdown_id') . " group by b.po_breakdown_id,b.entry_form order by b.po_breakdown_id";
			...
			$sql = "select b.po_id, sum(b.batch_qnty) as dye_qnty, min(c.process_end_date) mindate, max(c.process_end_date) maxdate from pro_batch_create_mst a,pro_batch_create_dtls b, pro_fab_subprocess c,GBL_TEMP_ENGINE gbl where a.id=b.mst_id and a.id=c.batch_id and c.load_unload_id=2 and c.entry_form=35 and a.batch_against<>2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 " . gbl_temp_engine_join($user_id,$eform, 'b.po_id') . " group by b.po_id";
			//echo $sql;die;
			...
			$sql = "SELECT po_break_down_id,min(inspection_date) as mind,max(inspection_date) as maxd,sum(inspection_qnty) as sumtot FROM pro_buyer_inspection,GBL_TEMP_ENGINE gbl WHERE status_active =1 and is_deleted = 0 " . gbl_temp_engine_join($user_id,$eform, 'po_break_down_id') . " group by po_break_down_id";
			...
			$sql = "SELECT po_break_down_id,min(ex_factory_date) as mind,max(ex_factory_date) as maxd,sum(ex_factory_qnty) as sumtot FROM  pro_ex_factory_mst,GBL_TEMP_ENGINE gbl WHERE status_active =1 and is_deleted = 0 " . gbl_temp_engine_join($user_id,$eform, 'po_break_down_id') . " group by po_break_down_id";
			...
			$sql = "SELECT  b.po_breakdown_id, min(a.submit_date) as mind,max(a.submit_date) as maxd, sum(b.current_invoice_qnty) as current_invoice_qnty FROM  com_export_doc_submission_mst a,com_export_invoice_ship_dtls b, com_export_doc_submission_invo c,GBL_TEMP_ENGINE gbl WHERE a.id=c.doc_submission_mst_id and c.invoice_id=b.mst_id and b.current_invoice_qnty>0  and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 and c.status_active =1 and c.is_deleted=0 " . gbl_temp_engine_join($user_id,$eform, 'b.po_breakdown_id') . " group by b.po_breakdown_id";
			...
			$sql = "select a.po_break_down_id,min(a.production_date) as mind,max(a.production_date) as maxd,a.production_type,sum(b.production_qnty) as production_quantity,a.embel_name,b.color_type_id from pro_garments_production_mst a,pro_garments_production_dtls b,GBL_TEMP_ENGINE gbl where  a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 " . gbl_temp_engine_join($user_id,$eform, 'a.po_break_down_id') . " group by a.po_break_down_id,a.production_type,b.color_type_id,a.embel_name";
			...
			$yrecSql = "SELECT c.PO_BREAKDOWN_ID, SUM (c.quantity) QTY,  MAX (a.receive_date) AS MAX_RECEIVE_DATE, MIN (a.receive_date) AS MIN_RECEIVE_DATE FROM inv_receive_master a INNER JOIN inv_transaction b ON a.id = b.mst_id INNER JOIN order_wise_pro_details c ON b.id = c.trans_id INNER JOIN product_details_master d ON c.prod_id = d.id INNER JOIN GBL_TEMP_ENGINE gbl ON c.po_breakdown_id=gbl.REF_VAL WHERE c.trans_type = 1  AND b.item_category = 1  AND b.transaction_type = 1 AND a.status_active = 1 AND a.is_deleted = 0  AND b.status_active = 1 AND b.is_deleted = 0 AND c.status_active = 1 AND c.is_deleted = 0   GROUP BY c.po_breakdown_id";
			...
			$sql = "select a.entry_form,b.order_id, max(a.receive_date) as  maxd, min(a.receive_date) as mind  from inv_receive_mas_batchroll  a, pro_grey_batch_dtls b,GBL_TEMP_ENGINE gbl where a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and a.entry_form in (91,92)  " . gbl_temp_engine_join($user_id,$eform, 'b.order_id') . " group by a.entry_form,b.order_id";
			...
			$sql_task = "select min(a.REQUISITION_DATE) as start_date, max(a.REQUISITION_DATE) as end_date ,d.id as po_breakdown_id,e.sample_type,sum(b.sample_prod_qty) as sample_prod_qty from sample_development_mst a,sample_development_dtls b,wo_po_details_master c,wo_po_break_down d, lib_sample e,GBL_TEMP_ENGINE gbl where e.id=b.sample_name and a.quotation_id=c.id and c.job_no=d.job_no_mst and a.id=b.sample_mst_id and b.entry_form_id=117 and e.sample_type in(2,12,13) and e.BUSINESS_NATURE=2 and b.is_deleted=0 and b.status_active=1   " . gbl_temp_engine_join($user_id,$eform, 'd.id') . " group by d.id ,e.sample_type order by d.id asc";
			...
			$sql_task = "SELECT b.po_break_down_id,  sum(wo_qnty) as tfb_qnty, sum(b.amount/b.exchange_rate)  as tfb_amount, sum(grey_fab_qnty) as gfb_qnty, min(a.booking_date) as start_date, max(a.booking_date) as end_date, a.item_category,c.sample_type FROM  wo_booking_mst a, wo_booking_dtls b,lib_sample c,GBL_TEMP_ENGINE gbl WHERE c.id=b.sample_type and a.booking_no=b.booking_no and a.is_short=2 and c.sample_type=2 and c.BUSINESS_NATURE=2 and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0  " . gbl_temp_engine_join($user_id,$eform, 'b.po_break_down_id') . " group by b.po_break_down_id,item_category,c.sample_type order by b.po_break_down_id asc";
			...
			$sql = "select d.id as po_breakdown_id,b.sample_prod_qty,e.ex_factory_qty,min(e.delivery_date) as start_date,max(e.delivery_date) as end_date from sample_development_mst a,sample_development_dtls b,wo_po_details_master c,wo_po_break_down d ,sample_ex_factory_dtls e,GBL_TEMP_ENGINE gbl where a.id=e.sample_development_id and a.quotation_id=c.id and c.job_no=d.job_no_mst and a.id=b.sample_mst_id and b.entry_form_id=117  and b.is_deleted=0 and b.status_active=1  " . gbl_temp_engine_join($user_id,$eform, 'd.id') . "  group by d.id ,b.sample_prod_qty,e.ex_factory_qty order by d.id asc";
			...
			$program_sql = "select b.COLOR_TYPE_ID,b.BODY_PART_ID,b.PO_ID, sum(b.program_qnty) as QNTY from ppl_planning_entry_plan_dtls b,GBL_TEMP_ENGINE gbl where b.status_active=1 and b.is_deleted=0  " . gbl_temp_engine_join($user_id,$eform, 'b.po_id') . " group by b.COLOR_TYPE_ID,b.BODY_PART_ID,b.PO_ID";
			...
			// $sql = "select b.BODY_PART_ID,c.po_breakdown_id,sum(c.qnty) as qty,min(a.receive_date) as start_date,max(a.receive_date) as end_date from inv_receive_master a,pro_grey_prod_entry_dtls b,pro_roll_details c,GBL_TEMP_ENGINE gbl where a.id=b.mst_id and a.id=c.mst_id and b.id=c.dtls_id  and c.roll_used=1 and c.entry_form=2 and c.status_active=1 and c.is_deleted=0 " . gbl_temp_engine_join($user_id,$eform, 'c.po_breakdown_id') . " group by b.BODY_PART_ID,c.PO_BREAKDOWN_ID UNION ALL select b.BODY_PART_ID,c.PO_BREAKDOWN_ID, SUM (c.QUANTITY) AS qty, MIN (a.receive_date) AS start_date,  MAX (a.receive_date) AS end_date from inv_receive_master a,PRO_GREY_PROD_ENTRY_DTLS b,ORDER_WISE_PRO_DETAILS c,GBL_TEMP_ENGINE gbl where a.id = b.MST_ID and b.TRANS_ID = c.TRANS_ID and a.IS_DELETED=0 and b.IS_DELETED=0 and b.IS_DELETED=0 and a.STATUS_ACTIVE=1 and b.STATUS_ACTIVE=1 and c.STATUS_ACTIVE=1 " . gbl_temp_engine_join($user_id,$eform, 'c.po_breakdown_id') . " group by b.BODY_PART_ID,c.PO_BREAKDOWN_ID";
			//echo $sql;die;

			$sql = "select b.BODY_PART_ID,c.po_breakdown_id,sum(c.qnty) as qty,min(a.receive_date) as start_date,max(a.receive_date) as end_date from inv_receive_master a,pro_grey_prod_entry_dtls b,pro_roll_details c,GBL_TEMP_ENGINE gbl where a.id=b.mst_id and a.id=c.mst_id and b.id=c.dtls_id  and c.roll_used=1 and c.entry_form=2 and c.status_active=1 and c.is_deleted=0 " . gbl_temp_engine_join($user_id,$eform, 'c.po_breakdown_id') . " group by b.BODY_PART_ID,c.PO_BREAKDOWN_ID UNION ALL select b.BODY_PART_ID,c.PO_BREAKDOWN_ID, SUM (c.QUANTITY) AS qty, MIN (a.receive_date) AS start_date,  MAX (a.receive_date) AS end_date from inv_receive_master a,PRO_GREY_PROD_ENTRY_DTLS b,ORDER_WISE_PRO_DETAILS c,GBL_TEMP_ENGINE gbl where a.id = b.MST_ID  AND c.dtls_ID=b.id and a.IS_DELETED=0 and b.IS_DELETED=0 and b.IS_DELETED=0 and a.STATUS_ACTIVE=1 and b.STATUS_ACTIVE=1 and c.STATUS_ACTIVE=1 " . gbl_temp_engine_join($user_id,$eform, 'c.po_breakdown_id') . " group by b.BODY_PART_ID,c.PO_BREAKDOWN_ID"; // and b.TRANS_ID = c.TRANS_ID  and c.entry_form=2
			...
			$sql = "select b.BODY_PART_ID,c.po_breakdown_id,sum(c.qnty) as reqqnty, sum(c.qc_pass_qnty) as doneqnty, 0 as returnable_qnty,min(a.receive_date) as start_date,max(a.receive_date) as end_date from inv_receive_master a,pro_finish_fabric_rcv_dtls b,pro_roll_details c,GBL_TEMP_ENGINE gbl where a.id=b.mst_id and a.id=c.mst_id and b.id=c.dtls_id  and c.entry_form=68 and c.status_active=1 and c.is_deleted=0  " . gbl_temp_engine_join($user_id,$eform, 'c.po_breakdown_id') . " group by b.BODY_PART_ID,c.po_breakdown_id union all select b.BODY_PART_ID,a.po_breakdown_id,sum(0) as reqqnty, sum(a.quantity) as doneqnty, sum(a.returnable_qnty) as returnable_qnty,min(c.receive_date) as start_date,max(c.receive_date) as end_date from order_wise_pro_details a,pro_finish_fabric_rcv_dtls b,inv_receive_master c,GBL_TEMP_ENGINE gbl where a.dtls_id=b.id and b.mst_id=c.id " . gbl_temp_engine_join($user_id,$eform, 'a.po_breakdown_id') . "and a.entry_form in(37,7)  and a.status_active=1 and a.is_deleted=0 group by b.BODY_PART_ID,a.po_breakdown_id";
			// echo $sql;die;
			...
			$po_con = gbl_temp_engine_join($user_id,$eform, 'b.wo_po_break_down_id');

			$sql = "select max(a.contract_date) as max_date,min(a.contract_date) as min_date,b.wo_po_break_down_id,sum(b.attached_qnty) as qty  from com_sales_contract a,com_sales_contract_order_info b,GBL_TEMP_ENGINE gbl where a.id=b.com_sales_contract_id $po_con  and a.convertible_to_lc in(2,3) group by b.wo_po_break_down_id union all select  max(a.lc_date) as max_date,min(a.lc_date) as min_date,b.wo_po_break_down_id,sum(b.attached_qnty) as qty  from com_export_lc a,com_export_lc_order_info b,GBL_TEMP_ENGINE gbl where a.id=b.com_export_lc_id $po_con group by b.wo_po_break_down_id";
			...
			$pre_cost_sql = "select a.APPROVED,b.id as PO_ID,max(c.APPROVED_DATE) as MAX_DATE,MIN(c.APPROVED_DATE) as MIN_DATE from WO_PRE_COST_MST a,WO_PO_BREAK_DOWN b,APPROVAL_HISTORY c,GBL_TEMP_ENGINE gbl where a.job_no=b.job_no_mst and a.id=c.mst_id and c.entry_form=15  and a.IS_DELETED=0 and a.STATUS_ACTIVE =1 and b.IS_DELETED=0 and b.STATUS_ACTIVE=1 and a.APPROVED>0 " . gbl_temp_engine_join($user_id,$eform, 'b.id') . " GROUP BY a.APPROVED,b.id union all select a.APPROVED,b.id as PO_ID,max(c.APPROVED_DATE) as MAX_DATE,MIN(c.APPROVED_DATE) as MIN_DATE from WO_PRE_COST_MST a,WO_PO_BREAK_DOWN b,co_com_pre_costing_app_his c,GBL_TEMP_ENGINE gbl where a.job_no=b.job_no_mst and a.id=c.mst_id and c.entry_form=11  and a.IS_DELETED=0 and a.STATUS_ACTIVE =1 and b.IS_DELETED=0 and b.STATUS_ACTIVE=1 and a.APPROVED>0 " . gbl_temp_engine_join($user_id,$eform, 'b.id') . " GROUP BY a.APPROVED,b.id";
			...
			$pre_cost_sql = "select b.id as PO_ID,max(a.APPROVED_DATE) as MAX_DATE,MIN(a.APPROVED_DATE) as MIN_DATE from WO_PRE_COST_MST a,WO_PO_BREAK_DOWN b,GBL_TEMP_ENGINE gbl where a.job_no=b.job_no_mst and a.IS_DELETED=0 and a.STATUS_ACTIVE =1 and a.READY_TO_APPROVED=1 and a.APPROVED=1  and b.IS_DELETED=0 and b.STATUS_ACTIVE=1 " . gbl_temp_engine_join($user_id,$eform, 'b.id') . " GROUP BY b.id";
			...
			//  document_approval_info
			$approval_sql = "select a.PO_BREAK_DOWN_ID, max(a.APPROVAL_STATUS_DATE) as MAX_DATE,MIN(a.APPROVAL_STATUS_DATE) as MIN_DATE from WO_PO_DOCUMENT_APPROVAL_INFO a,GBL_TEMP_ENGINE gbl where  a.IS_DELETED=0 and a.STATUS_ACTIVE =1" .gbl_temp_engine_join($user_id,$eform, 'a.PO_BREAK_DOWN_ID') . " GROUP BY a.PO_BREAK_DOWN_ID
			union all 
			select a.PO_ID as PO_BREAK_DOWN_ID, max(a.INSERT_DATE) as MAX_DATE,MIN(a.INSERT_DATE) as MIN_DATE from gmts_production_confirmation a,GBL_TEMP_ENGINE gbl where " .gbl_temp_engine_join($user_id,$eform, 'a.PO_ID') . " AND a.CONFIRM_STATUS = 1 AND a.STATUS_ACTIVE = 1 group by a.PO_ID";
			// echo $approval_sql;die;

			$res_approval_sql = sql_select($approval_sql);
			foreach ($res_approval_sql as $row) {
				if($row['MAX_DATE'] && $row['MIN_DATE']){
					$tna_task_update_data[$row['PO_BREAK_DOWN_ID']][131]['max_start_date'] = date("d-M-Y", strtotime($row['MAX_DATE']));
					$tna_task_update_data[$row['PO_BREAK_DOWN_ID']][131]['min_start_date'] = date("d-M-Y", strtotime($row['MIN_DATE']));
					$tna_task_update_data[$row['PO_BREAK_DOWN_ID']][131]['doneqnty'] = 1;
					$tna_task_update_data[$row['PO_BREAK_DOWN_ID']][131]['reqqnty'] = 1;
					$to_process_task[$row['PO_BREAK_DOWN_ID']][131] = 131;
				}
			}
			unset($res_approval_sql);



			
			//Only for FSO TNA Process............Sales order basis.................................
			if($gross_level == 1){
						
				$yarnIssueSql = "select d.id as PO_ID,a.PO_JOB_NO as JOB_NO,sum(b.QUANTITY) as REQ_QTY,max(c.TRANSACTION_DATE) as MAX_DATE,MIN(c.TRANSACTION_DATE) as MIN_DATE from FABRIC_SALES_ORDER_MST a,ORDER_WISE_PRO_DETAILS b,INV_TRANSACTION c,WO_PO_BREAK_DOWN d,GBL_TEMP_ENGINE gbl where a.id=b.PO_BREAKDOWN_ID and b.TRANS_ID=c.id and b.PROD_ID = c.PROD_ID and d.JOB_NO_MST=a.PO_JOB_NO " . gbl_temp_engine_join($user_id,$eform, 'd.id') . " and b.TRANS_TYPE=2  and c.TRANSACTION_TYPE=2 and b.ISSUE_PURPOSE=1 and b.IS_SALES=1 and c.ITEM_CATEGORY=1 and c.RECEIVE_BASIS=3 AND a.STATUS_ACTIVE = 1 AND b.IS_DELETED = 0 AND b.STATUS_ACTIVE = 1 AND c.IS_DELETED = 0 AND c.STATUS_ACTIVE = 1 group by d.id,a.PO_JOB_NO";
				//echo $yarnIssueSql;die;

				$yarnIssueSqlRes = sql_select($yarnIssueSql);
				$job_wise_yarn_issue_req_qty_arr = array();$job_wise_yarn_issue_done_qty_arr = array();
				foreach ($yarnIssueSqlRes as $row) {
					$job_wise_yarn_issue_req_qty_arr[$row['JOB_NO']][$row['PO_ID']] = $tna_task_update_data[$row['PO_ID']][50]['reqqnty'];
					$job_wise_yarn_issue_done_qty_arr[$row['JOB_NO']][$row['PO_ID']] = $row['REQ_QTY'];
					...
					$tna_task_update_data[$row['PO_ID']][50]['doneqnty'] = $row['REQ_QTY'];
				}
				foreach($job_wise_yarn_issue_done_qty_arr as $jobId => $reqQtyArr){ 
					if( array_sum($job_wise_yarn_issue_req_qty_arr[$jobId]) > end($reqQtyArr)){
						foreach($reqQtyArr as $pid => $preq){
							$tna_task_update_data[$pid][50]['max_start_date'] = '';
						}
					}
				}


				//print_r($job_wise_yarn_issue_done_qty_arr);die;

				$yarn_allocation_sql = "SELECT c.id as PO_ID,b.PO_JOB_NO as JOB_NO, sum(a.qnty) AS ALLOCAT_QNTY,max(a.ALLOCATION_DATE) as MAX_DATE,MIN(a.ALLOCATION_DATE) as MIN_DATE  FROM inv_material_allocation_mst a, FABRIC_SALES_ORDER_MST b,WO_PO_BREAK_DOWN c,GBL_TEMP_ENGINE gbl WHERE a.PO_BREAK_DOWN_ID = b.id and c.JOB_NO_MST=b.PO_JOB_NO AND a.status_active=1 AND a.is_deleted=0 AND b.status_active=1 AND b.is_deleted=0 AND c.status_active=1 AND c.is_deleted=0 and a.IS_SALES=1 " . gbl_temp_engine_join($user_id,$eform, 'c.id') . " group by c.id,b.PO_JOB_NO";
				
				$yarn_allocation_sql_res = sql_select($yarn_allocation_sql);
				$job_wise_allocation_req_qty_arr = array();$job_wise_allocation_done_qty_arr = array();
				foreach ($yarn_allocation_sql_res as $row) {
					$job_wise_allocation_req_qty_arr[$row['JOB_NO']][$row['PO_ID']] = $tna_task_update_data[$row['PO_ID']][48]['reqqnty'];
					$job_wise_allocation_done_qty_arr[$row['JOB_NO']][$row['PO_ID']] = $row['ALLOCAT_QNTY'];
					...
					$tna_task_update_data[$row['PO_ID']][48]['doneqnty'] = $row['ALLOCAT_QNTY'];
				}

				foreach($job_wise_allocation_done_qty_arr as $jobId => $reqQtyArr){ 
					if( array_sum($job_wise_allocation_req_qty_arr[$jobId]) > end($reqQtyArr)){
						foreach($reqQtyArr as $pid => $preq){
							$tna_task_update_data[$pid][48]['max_start_date'] = '';
						}
					}
				}
				...
				$knitting_pro_sql = "select e.id as PO_ID,d.PO_JOB_NO AS JOB_NO,max(a.RECEIVE_DATE) as MAX_DATE,MIN(a.RECEIVE_DATE) as MIN_DATE, sum(c.quantity) as QNTY from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c,FABRIC_SALES_ORDER_MST d,WO_PO_BREAK_DOWN e,GBL_TEMP_ENGINE gbl where d.id=c.po_breakdown_id and d.PO_JOB_NO=e.job_no_mst " . gbl_temp_engine_join($user_id,$eform, 'e.id') . " and a.id=b.mst_id and b.id=c.dtls_id and a.ITEM_CATEGORY=13 and a.entry_form =2 and c.entry_form =2 and a.RECEIVE_BASIS=2 and a.KNITTING_SOURCE=1   and a.status_active =1 and b.status_active =1 and c.status_active =1 group by e.id, d.PO_JOB_NO";
				//echo $knitting_pro_sql;die;

				$knitting_pro_sql_res = sql_select($knitting_pro_sql);
				$job_wise_kniting_req_qty_arr = array();$job_wise_kniting_done_qty_arr = array();
				foreach ($knitting_pro_sql_res as $row) {
					$job_wise_kniting_req_qty_arr[$row['JOB_NO']][$row['PO_ID']] = $tna_task_update_data[$row['PO_ID']][60]['reqqnty'];
					$job_wise_kniting_done_qty_arr[$row['JOB_NO']][$row['PO_ID']] = $row['QNTY'];
					...
					$tna_task_update_data[$row['PO_ID']][60]['doneqnty'] = $row['QNTY'];
				}

				foreach($job_wise_kniting_done_qty_arr as $jobId => $reqQtyArr){ 
					if( array_sum($job_wise_kniting_req_qty_arr[$jobId]) > end($reqQtyArr)){
						foreach($reqQtyArr as $pid => $preq){
							$tna_task_update_data[$pid][60]['max_start_date'] = '';
						}
					}
				}


				$knit_gray_pro_rcv_sql ="select e.id as PO_ID,d.PO_JOB_NO AS JOB_NO,max(a.RECEIVE_DATE) as MAX_DATE,MIN(a.RECEIVE_DATE) as MIN_DATE, sum(c.QUANTITY) as QNTY from INV_RECEIVE_MASTER a,PRO_GREY_PROD_ENTRY_DTLS b,ORDER_WISE_PRO_DETAILS c,FABRIC_SALES_ORDER_MST d,WO_PO_BREAK_DOWN e,GBL_TEMP_ENGINE gbl where a.id=b.mst_id and b.id=c.DTLS_ID and c.PO_BREAKDOWN_ID=d.id and d.PO_JOB_NO=e.job_no_mst " . gbl_temp_engine_join($user_id,$eform, 'e.id') . " AND c.ENTRY_FORM in(2,22,58) AND c.TRANS_TYPE = 1 AND c.IS_SALES = 1 and a.STATUS_ACTIVE=1 and a.IS_DELETED=0 and b.STATUS_ACTIVE=1 and b.IS_DELETED=0 and c.STATUS_ACTIVE=1 and c.IS_DELETED=0 and d.STATUS_ACTIVE=1 and d.IS_DELETED=0 group by e.id,d.PO_JOB_NO ";
				// echo $knit_gray_pro_rcv_sql;die;
				$knit_gray_pro_rcv_sql_res = sql_select($knit_gray_pro_rcv_sql);
				$job_wise_kniting_pro_rcv_qty_arr = array();$job_wise_kniting_pro_rcv_done_qty_arr = array();
				foreach ($knit_gray_pro_rcv_sql_res as $row) {
					$job_wise_kniting_pro_rcv_qty_arr[$row['JOB_NO']][$row['PO_ID']] = $tna_task_update_data[$row['PO_ID']][72]['reqqnty'];
					$job_wise_kniting_pro_rcv_done_qty_arr[$row['JOB_NO']][$row['PO_ID']] = $row['QNTY'];
					...
					$tna_task_update_data[$row['PO_ID']][72]['doneqnty'] = $row['QNTY'];
				}

				foreach($job_wise_kniting_pro_rcv_done_qty_arr as $jobId => $reqQtyArr){ 
					if( array_sum($job_wise_kniting_pro_rcv_qty_arr[$jobId]) > end($reqQtyArr)){
						foreach($reqQtyArr as $pid => $preq){
							$tna_task_update_data[$pid][72]['max_start_date'] = '';
						}
					}
				}

				$daying_production_sql ="SELECT e.id as PO_ID,d.PO_JOB_NO AS JOB_NO, SUM (b.batch_qnty) AS DYE_QNTY, MIN (c.process_end_date) MAX_DATE, MAX (c.process_end_date) MIN_DATE FROM pro_batch_create_mst a, pro_batch_create_dtls b, pro_fab_subprocess   c, FABRIC_SALES_ORDER_MST d,WO_PO_BREAK_DOWN e,GBL_TEMP_ENGINE gbl WHERE a.id = b.mst_id AND a.id = c.batch_id and b.PO_ID=d.id and d.PO_JOB_NO=e.JOB_NO_MST and b.IS_SALES=1 AND c.load_unload_id = 2 AND c.entry_form = 35 AND a.batch_against <> 2 AND a.status_active = 1 AND a.is_deleted = 0 AND b.status_active = 1 AND b.is_deleted = 0 AND c.status_active = 1 AND c.is_deleted = 0 AND d.status_active = 1 AND d.is_deleted = 0 AND e.status_active = 1 AND e.is_deleted = 0 " . gbl_temp_engine_join($user_id,$eform, 'e.id') . " GROUP BY e.id,d.PO_JOB_NO";
				 //echo $daying_production_sql;die;
				
				$daying_production_sql_res = sql_select($daying_production_sql);
				$job_wise_daying_pro_qty_arr = array();$job_wise_daying_pro_done_qty_arr = array();
				foreach ($daying_production_sql_res as $row) {
					$job_wise_daying_pro_qty_arr[$row['JOB_NO']][$row['PO_ID']] = $tna_task_update_data[$row['PO_ID']][61]['reqqnty'];
					$job_wise_daying_pro_done_qty_arr[$row['JOB_NO']][$row['PO_ID']] = $row['DYE_QNTY'];
					...
					$tna_task_update_data[$row['PO_ID']][61]['doneqnty'] = $row['DYE_QNTY'];
				}

				foreach($job_wise_daying_pro_done_qty_arr as $jobId => $reqQtyArr){ 
					if( array_sum($job_wise_daying_pro_qty_arr[$jobId]) > end($reqQtyArr)){
						foreach($reqQtyArr as $pid => $preq){
							$tna_task_update_data[$pid][61]['max_start_date'] = '';
						}
					}
				}

			}//end if;
			//...............................Only for FSO TNA Process end;
			...
			foreach ($po_order_details as $row)  // Non Process Starts Here
			{
				...
				if ($precost_fabric_source_by_po_arr[$row['po_id']] == 2) {
					foreach (array(48, 50, 60, 72, 61, 9, 10, 212, 73, 179, 323,64) as $vid) {
			...
			// print_r($tna_task_update_data[82596][131]);die;//51659,51660		
			//---------------------------------------------------------------------------------------------------------
			$insert_string = array();
			...
			$current .= "TNA-PROCESS:: CID: ".$cbo_company.", UID: ".$user_id.", DT: ".date("d-m-Y h:i:s a",time())."\n";
...
?>
	...
	<script>
		...
		function generate_list_view(dataStr) {
			...
			show_list_view(dataArr[0] + '_' + dataArr[1] + '_' + dataArr[2] + '_' + dataArr[3] + '_' + dataArr[4] + '_' + dataArr[5] + '_' + dataArr[6] + '_' + dataArr[7] + '_' + dataArr[8] + '_' + dataArr[9], 'ponumber_search_list_view', 'search_div', 'tna_process_controller', 'setFilterGrid(\'tbl_list_search\',-1)');
		}
	</script>
						...
						<td align="center">
							<table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
								...
								<thead>
									...
									<th width="170" colspan="2">Ship Date Range</th>
								...
								<tr>
									<td>
										...
										<?
										echo create_drop_down("cbo_company_mst", 100, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name", "id,company_name", 1, "-- Select Company --", $company, "load_drop_down( 'tna_process_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
										?>
									</td>
									<td id="buyer_td">
										<?
...
?>
	<table width="970" align="left" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" id="">
		<thead>
	...
	<div style="width:990px; max-height:230px; float:left; overflow-y:scroll">
		<table width="970" align="left" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" id="tbl_list_search">
			<?
