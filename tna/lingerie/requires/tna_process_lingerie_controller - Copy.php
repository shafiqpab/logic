...
if( $cbo_company<1 ){$company_array=return_library_array( "select id,id from lib_company where is_deleted = 0 and status_active = 1",'id','id' );}
...
*/	
	...
	foreach( $company_array as $cbo_company )
	{
		...
		{
			$remain_days=datediff( "d", date("Y-m-d",strtotime($row[PO_RECEIVED_DATE])), date("Y-m-d",strtotime($row[SHIPMENT_DATE])) );
			...
			{
				$template_missing_po[]=$row[PO_NUMBER];
				//This array for missiong PO Auto mail send..............
				$template_missing_po_mail_data_arr[]=array(
					'job_no_mst'		=> $row[JOB_NO_MST],
			...
			{ 
				...
				foreach($tna_task_name as $vid)
				{
					$to_process_task[$row[ID]][$vid]=$vid;
					//$tna_task_details[84][task_name]
				}
				...
				// Purchase
				foreach($fabricdata['knit']['grey'][$row[ID]] as $sourceId=>$sourceUomRows){
					if($sourceId==1){
						$tna_task_update_data[$row[ID]][276]['reqqnty']=array_sum($fabricdata['knit']['grey'][$row[ID]][$sourceId]);
						$to_process_task[$row[ID]][276]=276;
					}
					else{
						$tna_task_update_data[$row[ID]][31]['reqqnty']+=array_sum($fabricdata['knit']['grey'][$row[ID]][$sourceId]);
						$to_process_task[$row[ID]][31]=31;
						
						$tna_task_update_data[$row[ID]][305]['reqqnty']+=array_sum($fabricdata['knit']['grey'][$row[ID]][$sourceId]);
						$to_process_task[$row[ID]][305]=305;
				...
				/*
				if( array_sum($fabricdata['woven']['grey'][$row[ID]])>0) // Woven Purchase
				{
							$tna_task_update_data[$row[ID]][31]['reqqnty']+=array_sum($fabricdata['woven']['grey'][$row[ID]]);
				    $to_process_task[$row[ID]][31]=31;
				}
				*/

				
				if( array_sum($fabricdata_production['knit']['grey'][$row[ID]])>0)
				{
					$tna_task_update_data[$row[ID]][60]['reqqnty']+=array_sum($fabricdata_production['knit']['grey'][$row[ID]]);
					$to_process_task[$row[ID]][60]=60;
					$tna_task_update_data[$row[ID]][72]['reqqnty']+=array_sum($fabricdata_production['knit']['grey'][$row[ID]]);
					$to_process_task[$row[ID]][72]=72;
					
					$tna_task_update_data[$row[ID]][61]['reqqnty']+=array_sum($fabricdata_production['knit']['grey'][$row[ID]]);
					$to_process_task[$row[ID]][61]=61;
					

					
					$tna_task_update_data[$row[ID]][31]['reqqnty']+=array_sum($fabricdata_production['knit']['grey'][$row[ID]]);
					$to_process_task[$row[ID]][31]=31;
				
					
					$tna_task_update_data[$row[ID]][74]['reqqnty']+=array_sum($fabricdata_production['knit']['grey'][$row[ID]]);
					$to_process_task[$row[ID]][74]=74;
				}
				
				
				if( array_sum($fabricdata_production['knit']['finish'][$row[ID]])>0)
				{
					$tna_task_update_data[$row[ID]][64]['reqqnty']+=array_sum($fabricdata_production['knit']['finish'][$row[ID]]);
					$to_process_task[$row[ID]][64]=64;
				}
				
				if( count($trims_data[$row[ID]])>0 )
				{
					foreach($trims_data[$row[ID]] as $source_id=>$dataRows){
						if($source_id==1){
							$tna_task_update_data[$row[ID]][278]['reqqnty']+=$trims_data[$row[ID]][$source_id];
							$to_process_task[$row[ID]][278]=278;
						}
						else{
							$tna_task_update_data[$row[ID]][32]['reqqnty']+=$trims_data[$row[ID]][$source_id];
							$to_process_task[$row[ID]][32]=32;
							
							$tna_task_update_data[$row[ID]][307]['reqqnty']+=$trims_data[$row[ID]][$source_id];
							$to_process_task[$row[ID]][307]=307;
							
						}
					}
				}
				
				//print_r($fabricdata['woven']['grey'][$row[ID]]);die;
				
				
				foreach($fabricdata['woven']['grey'][$row[ID]] as $sourceId=>$sourceUomRows){
					if($sourceId==1){
						$tna_task_update_data[$row[ID]][302]['reqqnty']=array_sum($fabricdata['woven']['grey'][$row[ID]][$sourceId]);
						$to_process_task[$row[ID]][302]=302;
					}
					else{
						$tna_task_update_data[$row[ID]][34]['reqqnty']+=array_sum($fabricdata['woven']['grey'][$row[ID]][$sourceId]);
						$to_process_task[$row[ID]][34]=34;
					}
...
/*				if( array_sum($fabricdata['woven']['grey'][$row[ID]])>0)// Woven Purchase
				{
					$tna_task_update_data[$row[ID]][34]['reqqnty']=array_sum($fabricdata['woven']['grey'][$row[ID]]);
					$to_process_task[$row[ID]][34]=34;
				}
*/				
				...
				if( $yarndata[$row[ID]]>0)
				{
					$tna_task_update_data[$row[ID]][50]['reqqnty']=$yarndata[$row[ID]];
					$to_process_task[$row[ID]][50]=50;
					$tna_task_update_data[$row[ID]][48]['reqqnty']=$yarndata[$row[ID]];
					$to_process_task[$row[ID]][48]=48;
					$tna_task_update_data[$row[ID]][45]['reqqnty']=$yarndata[$row[ID]];
					$to_process_task[$row[ID]][45]=45;
					$tna_task_update_data[$row[ID]][46]['reqqnty']=$yarndata[$row[ID]];
					$to_process_task[$row[ID]][46]=46;
					$tna_task_update_data[$row[ID]][47]['reqqnty']=$yarndata[$row[ID]];
					$to_process_task[$row[ID]][47]=47;
					
				}
				
				if( $wash_data[$row[ID]]>0)
				{
					$tna_task_update_data[$row[ID]][89]['reqqnty']=$wash_data[$row[ID]];
					$to_process_task[$row[ID]][89]=89;
					$tna_task_update_data[$row[ID]][90]['reqqnty']=$wash_data[$row[ID]];
					$to_process_task[$row[ID]][90]=90;
				}
				
				if( $emblishment_data[$row[ID]]>0)
				{
					$tna_task_update_data[$row[ID]][85]['reqqnty']=$emblishment_data[$row[ID]];
					$to_process_task[$row[ID]][85]=85;
				}
				...
				//$order_id_array[$i]=$row[ID];
				$order_id_array[$row[ID]]=$row[ID];
				$po_order_template[$row[ID]]=  $template_id; 
				$po_order_details[$row[ID]]['po_received_date']=$row[PO_RECEIVED_DATE];
				$po_order_details[$row[ID]]['shipment_date']=$row[SHIPMENT_DATE];
				$po_order_details[$row[ID]]['job_no_mst']=$row[csf("job_no_mst")];
				$po_order_details[$row[ID]]['po_quantity']=$row[csf("po_quantity")];
				$po_order_details[$row[ID]]['po_quantity_pcs']=$row[csf("po_quantity_pcs")];
				$po_order_details[$row[ID]]['template_id']=$template_id;
				$po_order_details[$row[ID]]['po_id']=$row[ID];
				$po_order_details[$row[ID]]['is_confirmed']=$row[csf("is_confirmed")];
				$po_order_details[$row[ID]]['tna_task_from_upto']=$row[csf("tna_task_from_upto")];
				//$to_process_task
				$tna_task_update_data[$row[ID]][80]['max_start_date']=$row[csf("pp_meeting_date")];
				$tna_task_update_data[$row[ID]][80]['min_start_date']=$row[csf("pp_meeting_date")];
				//184=PP Meeting AOP;
				$tna_task_update_data[$row[ID]][184]['max_start_date']=$row[csf("pp_meeting_date")];
				$tna_task_update_data[$row[ID]][184]['min_start_date']=$row[csf("pp_meeting_date")];
				//185=PP Meeting YD;
				$tna_task_update_data[$row[ID]][185]['max_start_date']=$row[csf("pp_meeting_date")];
				$tna_task_update_data[$row[ID]][185]['min_start_date']=$row[csf("pp_meeting_date")];
				
				
				//$tna_task_update_data[$row[ID]][80]['noofval']=1;	
				$tna_task_update_data[$row[ID]][80]['doneqnty']=1;
				$tna_task_update_data[$row[ID]][80]['reqqnty']=1; 
				$tna_task_update_data[$row[ID]][1]['doneqnty']=1;
				$tna_task_update_data[$row[ID]][1]['reqqnty']=1; 
				$tna_task_update_data[$row[ID]][1]['max_start_date']=$row[PO_RECEIVED_DATE];
				$tna_task_update_data[$row[ID]][1]['min_start_date']=$row[PO_RECEIVED_DATE];
				$to_process_task[$row[ID]][1]=1;
...
//****************************************Actual TNA Process Start*****************************************
		...
		$costing_per_sql = "select job_no,costing_per from wo_pre_cost_mst a where 1=1 ".where_con_using_array($job_no_array,1,'a.job_no')." ";
		...
		$sql="select a.FABRIC_SOURCE,a.job_no,a.color_type_id,b.po_break_down_id,a.id as pre_cost_fabric_cost_dtls_id,a.item_number_id,a.fab_nature_id,b.color_size_table_id, b.color_number_id,b.gmts_sizes as size_number_id, b.cons, b.requirment,a.rate 
			...
			WHERE a.job_no=b.job_no and  a.id=b.pre_cost_fabric_cost_dtls_id ".where_con_using_array($order_id_array,0,'b.po_break_down_id')." and a.status_active=1 and a.is_deleted=0 order by a.id ";
			...
			if( $row[REQUIRMENT]>0)
			{
				...
				{//solid..
					//269=> "Sample Fabric Booking To Be Issued Woven",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][269]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][269]=269;
					
					//150	=> "PPS Fabric Issue",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][150]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][150]=150;
					
					//37  => "PPS Making",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][37]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][37]=37;
					//60 => "Gray Fabric Production To Be Done",	
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][60]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][60]=60;
					//73 => "Finished fabric to be in-house",
						//$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][73]['reqqnty']+=$row[REQUIRMENT];
						
						
						foreach($req_fin_fab_qty_arr[$row[csf('po_break_down_id')]] as $source_id=>$dataRows){
							if($source_id==1){
								$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][277]['reqqnty']+=round(array_sum($req_fin_fab_qty_arr[$row[csf('po_break_down_id')]][$source_id]));
								$to_process_task[$row[PO_BREAK_DOWN_ID]][277]=277;
							}
							else{
								$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][73]['reqqnty']+=round(array_sum($req_fin_fab_qty_arr[$row[csf('po_break_down_id')]][$source_id]));
								$to_process_task[$row[PO_BREAK_DOWN_ID]][73]=73;
								
								$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][306]['reqqnty']+=round(array_sum($req_fin_fab_qty_arr[$row[csf('po_break_down_id')]][$source_id]));
								$to_process_task[$row[PO_BREAK_DOWN_ID]][306]=306;
							}
					...
					//131 => "Production File Handover",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][131]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][131]=131;
					//130 => "Trim Card Handover",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][130]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][130]=130;
					//80	=> "PP Meeting To Be Conducted",
						//$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][80]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][80]=80;
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][80]['doneqnty']=1;
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][80]['reqqnty']=1; 
						
					//84=> "Cutting To Be Done",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][84]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][84]=84;
					//86	=> "Sewing To Be Done",	
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][86]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][86]=86;
					
					//51	=> "Yarn Send for Dyeing",	
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][51]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][51]=51;

					//165	=> "Yarndip Requisition",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][165]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][165]=165;
					//166	=> "Yarndip Submit To Buyer",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][166]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][166]=166;
					//167	=> "Yarndip Approval",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][167]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][167]=167;
						
					//203	=> "Grey Fab Issue",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][203]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][203]=203;
						
					//212=> "Knitting production Solid",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][212]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][212]=212;
						
						
					//256=> "YD Knit down Submission",
					    $tna_task_update_data[$row[PO_BREAK_DOWN_ID]][256]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][256]=256;
						
					//257=> "YD Knit down Approval", 
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][257]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][257]=257;

						$SolidPoArr[$row[PO_BREAK_DOWN_ID]]=$row[PO_BREAK_DOWN_ID];
				...
				else
				{
					...
					//169	=> "PPS Fabrics Issue (YD)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][169]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][169]=169;
					//171	=> "PPS Making (YD)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][171]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][171]=171;
					//174	=> "PPS Submit (YD)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][174]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][174]=174;
					//175	=> "PPS Approval (YD)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][175]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][175]=175;
					//177	=> "Trim Card Handover (YD)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][177]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][177]=177;
					//178	=> "Knitting production (YD)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][178]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][178]=178;
					//180	=> "Finish Fabrics Inhouse (YD)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][180]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][180]=180;
					//181	=> "Production File Handover(YD)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][181]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][181]=181;
					//185	=> "PP Meeting (YD)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][185]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][185]=185;
					//187	=> "Cutting Production (YD)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][187]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][187]=187;
					//191	=> "Sewing Production(YD)"
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][191]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][191]=191;
					//165	=> "Yarndip Requisition",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][165]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][165]=165;
					//166	=> "Yarndip Submit To Buyer",
					...
					//257=> "YD Knit down Approval", 
						...
						$to_process_task[$row[PO_BREAK_DOWN_ID]][258]=258;
						//259=> "Bulk Hanger Approval",	
						$to_process_task[$row[PO_BREAK_DOWN_ID]][259]=259;
						
						
						$YDPoArr[$row[PO_BREAK_DOWN_ID]]=$row[PO_BREAK_DOWN_ID];
					}
					//5,7 AOP
					else if(($row[COLOR_TYPE_ID]==5) || ($row[COLOR_TYPE_ID]==7)){
					//168	=> "PPS Fabrics Issue (AOP)"
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][168]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][168]=168;
					//170	=> "PPS Making (AOP)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][170]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][170]=170;
					//172	=> "PPS Submit (AOP)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][172]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][172]=172;
					//173	=> "PPS Approval (AOP)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][173]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][173]=173;
					//176	=> "Trim Card Handover (AOP)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][176]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][176]=176;
					//179	=> "Finish Fabrics Inhouse (AOP)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][179]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][179]=179;
					//184	=> "PP Meeting (AOP)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][184]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][184]=184;
					//186	=> "Cutting Production (AOP)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][186]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][186]=186;
					//190	=> "Sewing Production(AOP)",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][190]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][190]=190;
						
					//60 => "Gray Fabric Production To Be Done",	
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][60]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][60]=60;
					//62 => "Fabric Send for AOP",	
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][62]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][62]=62;
						
						
					//254=> "AOP Strike Off Submission",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][254]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][254]=254;
					//255=> "AOP Strike Off Approval",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][255]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][255]=255;
					...
					//255=> "AOP Strike Off Approval",
						...
						$AOPPoArr[$row[PO_BREAK_DOWN_ID]]=$row[PO_BREAK_DOWN_ID];
						
					}//solid..
					else if(($row[COLOR_TYPE_ID]==1) || ($row[COLOR_TYPE_ID]==20) || ($row[COLOR_TYPE_ID]==25) || ($row[COLOR_TYPE_ID]==26) || ($row[COLOR_TYPE_ID]==27) || ($row[COLOR_TYPE_ID]==28) || ($row[COLOR_TYPE_ID]==29) || ($row[COLOR_TYPE_ID]==30) || ($row[COLOR_TYPE_ID]==31) || ($row[COLOR_TYPE_ID]==34)  || ($row[COLOR_TYPE_ID]==35)  || ($row[COLOR_TYPE_ID]==36)  || ($row[COLOR_TYPE_ID]==37) || ($row[COLOR_TYPE_ID]==''))
					{
					//150	=> "PPS Fabric Issue",
						$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][150]['reqqnty']+=$row[REQUIRMENT];
						$to_process_task[$row[PO_BREAK_DOWN_ID]][150]=150;
...
//****************************************************************************************		
			...
			$sql = "SELECT task_number,po_number_id,actual_start_date,actual_finish_date,task_start_date,task_finish_date ,plan_start_flag,plan_finish_flag FROM tna_plan_actual_history WHERE task_type=4 ".where_con_using_array($order_id_array,0,'po_number_id')." and status_active =1 and is_deleted=0 order by id";
 	 //echo $sql;die;
	...
	// TNA Processs Data List for TNA UPDATE
		...
		$sql = "SELECT id,job_no,po_number_id,task_category,task_number,actual_start_date,actual_finish_date,template_id FROM tna_process_mst WHERE status_active =1 and is_deleted = 0  and task_type=4 ".where_con_using_array($order_id_array,0,'po_number_id')."";
 ...
 WHERE     sample_type IN (2,3,4,7,8,9,10,11,12,13,14) AND c.id = a.color_number_id AND a.po_break_down_id = c.po_break_down_id AND a.job_no_mst = c.job_no_mst  AND c.status_active = 1 AND c.is_deleted = 0  AND b.id = a.SAMPLE_TYPE_ID AND a.is_deleted = 0  AND a.status_active = 1 AND b.is_deleted = 0 AND b.status_active = 1 ".where_con_using_array($job_no_array,1,'a.job_no_mst')."";
...
where a.id=b.mst_id and b.item_category=1 and  a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0   ".where_con_using_array($order_id_array,0,'b.po_break_down_id')." group by b.po_break_down_id";
	...
	// LABDIP Approval Data for Update
		...
		$sql_task = "SELECT count(id) as cid, po_break_down_id, max(submitted_to_buyer) as max_submitted_to_buyer,min(submitted_to_buyer) as min_submitted_to_buyer,max(approval_status_date) as max_approval_status_date,min(approval_status_date) as min_approval_status_date, LISTAGG(color_name_id, ',') WITHIN GROUP (ORDER BY color_name_id) AS color_name_id
		 FROM wo_po_lapdip_approval_info WHERE is_deleted = 0 and status_active=1 ".where_con_using_array($job_no_array,1,'job_no_mst')." group by po_break_down_id order by po_break_down_id asc";
		...
		$sql_task = "SELECT id as cid,po_break_down_id ,submitted_to_buyer,approval_status_date FROM wo_po_lapdip_approval_info WHERE is_deleted = 0 and status_active=1 ".where_con_using_array($job_no_array,1,'a.job_no_mst')."";
	...
	WHERE b.id=a.accessories_type_id and a.is_deleted = 0 and a.status_active=1 and b.is_deleted = 0 and b.status_active=1 ".where_con_using_array($job_no_array,1,'a.job_no_mst')." group by po_break_down_id,trim_type order by po_break_down_id asc";
		//echo $sql_task;die;
		$result = sql_select( $sql_task );
	...
	WHERE b.id=a.accessories_type_id and approval_status_date is not null  and a.is_deleted = 0 and a.status_active=1   and b.is_deleted = 0 and b.status_active=1 ".where_con_using_array($job_no_array,1,'a.job_no_mst')." group by po_break_down_id,trim_type order by po_break_down_id asc";
		//echo $sql_task;die;
	...
	// Embelishment Approval Data for Update
		...
		$sql_task = "SELECT count(id) as cid,po_break_down_id,max(submitted_to_buyer) as max_submitted_to_buyer,min(submitted_to_buyer) as min_submitted_to_buyer,max(approval_status_date) as max_approval_status_date,min(approval_status_date) as min_approval_status_date,embellishment_id
		 FROM wo_po_embell_approval WHERE is_deleted = 0 and status_active=1 ".where_con_using_array($job_no_array,1,'job_no_mst')." group by po_break_down_id,embellishment_id order by po_break_down_id asc";
			  //echo $sql_task;die;
		...
		$sql_task = "SELECT count(id) as cid,po_break_down_id FROM wo_po_embell_approval WHERE approval_status_date is not null   and is_deleted = 0 and status_active=1 ".where_con_using_array($job_no_array,1,'a.job_no_mst')." group by po_break_down_id,embellishment_id order by po_break_down_id asc";
		...
		foreach($data_array as $row)
		{
			if($row[COLOR_TYPE_ID]==5) { $app=62; $sub=63; }
			else { $app=51; $sub=52; }
	...
	where e.id=a.mst_id and a.prod_id=c.id and b.trans_id=a.id and c.item_group_id=d.id and d.trim_type in (1,2) and b.entry_form=24 ".where_con_using_array($order_id_array,0,'b.po_break_down_id')."  and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 and c.status_active =1 and c.is_deleted=0 and d.status_active =1 and d.is_deleted=0 and e.status_active =1 and e.is_deleted=0 group by b.po_breakdown_id,d.trim_type,e.source order by b.po_breakdown_id"; //echo $sewing_finish_trims_sql;die;
	...
	// Inspection Data for Update
		...
		$sql = "SELECT po_break_down_id,min(inspection_date) as mind,max(inspection_date) as maxd,sum(inspection_qnty) as sumtot FROM pro_buyer_inspection WHERE status_active =1 and is_deleted = 0 ".where_con_using_array($job_no_array,1,'job_no')." group by po_break_down_id";
			//echo $sql;die;
		$result = sql_select( $sql );
	...
	// Doc Submisiion	
		...
		foreach( $result as $row ) 
		{
			$tna_task_update_data[$row[PO_BREAKDOWN_ID]][121]['max_start_date']=$row[csf("maxd")];
			$tna_task_update_data[$row[PO_BREAKDOWN_ID]][121]['min_start_date']=$row[csf("mind")];
			$tna_task_update_data[$row[PO_BREAKDOWN_ID]][121]['doneqnty']=$row[csf("current_invoice_qnty")];
			$tna_task_update_data[$row[PO_BREAKDOWN_ID]][121]['reqqnty']=$po_order_details[$row[csf("po_breakdown_id")]]['po_quantity'];
	...
	// Garments Production Data for Update .................................................... 
		...
		foreach( $result as $row ) 
		{
			...
			if( $gross_level==1 ){
				//84=Cutting Prod.;
				if ($row[PRODUCTION_TYPE]==1) $tsktype=84;
				//85=Embellishment
				else if ($row[PRODUCTION_TYPE]==3 && $to_process_task[$row[PO_BREAK_DOWN_ID]][85]==85){
					$tsktype=85;
				}
				//86=Sewing Prod.
				else if ($row[PRODUCTION_TYPE]==5) $tsktype=86;

				if ($row[PRODUCTION_TYPE]==3 && $row[csf("embel_name")]==1){
					$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][267]['max_start_date']=$row[csf("maxd")];
					$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][267]['min_start_date']=$row[csf("mind")];
					$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][267]['doneqnty']+=$row[PRODUCTION_QUANTITY];
				}
				else if ($row[PRODUCTION_TYPE]==3 && $row[csf("embel_name")]==2){
					$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][268]['max_start_date']=$row[csf("maxd")];
					$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][268]['min_start_date']=$row[csf("mind")];
					$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][268]['doneqnty']+=$row[PRODUCTION_QUANTITY];
				}
			...
			else
			{
				...
				if(($row[COLOR_TYPE_ID]==2) || ($row[COLOR_TYPE_ID]==3) || ($row[COLOR_TYPE_ID]==4) || ($row[COLOR_TYPE_ID]==6) || ($row[COLOR_TYPE_ID]==32) || ($row[COLOR_TYPE_ID]==33)){
					...
					//187=Cutting Prod YD
					if ($row[PRODUCTION_TYPE]==1) $tsktype=187;
					//189=Embellishmen YD
					else if ($row[PRODUCTION_TYPE]==3 && $to_process_task[$row[PO_BREAK_DOWN_ID]][189]==189){
						$tsktype=189;
					}
					//191=Sewing Prod YD
					else if ($row[PRODUCTION_TYPE]==5) $tsktype=191;
	
				}
				//5,7 AOP
				else if(($row[COLOR_TYPE_ID]==5) || ($row[COLOR_TYPE_ID]==7)){
					//186=Cutting Pro AOP
					if ($row[PRODUCTION_TYPE]==1) $tsktype=186;
					//188=Embellish AOP
					else if ($row[PRODUCTION_TYPE]==3 && $to_process_task[$row[PO_BREAK_DOWN_ID]][188]==188){
						$tsktype=188;
					}
					//190=Sewing Prod AOP
					else if ($row[PRODUCTION_TYPE]==5) $tsktype=190;
				}
				elseif(($row[COLOR_TYPE_ID]==1) || ($row[COLOR_TYPE_ID]==20) || ($row[COLOR_TYPE_ID]==25) || ($row[COLOR_TYPE_ID]==26) || ($row[COLOR_TYPE_ID]==27) || ($row[COLOR_TYPE_ID]==28) || ($row[COLOR_TYPE_ID]==29) || ($row[COLOR_TYPE_ID]==30) || ($row[COLOR_TYPE_ID]==31) || ($row[COLOR_TYPE_ID]==34)  || ($row[COLOR_TYPE_ID]==35)  || ($row[COLOR_TYPE_ID]==36)  || ($row[COLOR_TYPE_ID]==37) || ($row[COLOR_TYPE_ID]==''))
				{
					//84=Cutting Prod.;
					if ($row[PRODUCTION_TYPE]==1) $tsktype=84;
					//85=Embellishment
			...
			if ($row[PRODUCTION_TYPE]==4) $tsktype=122;
			else if ($row[PRODUCTION_TYPE]==7) $tsktype=87;
			else if ($row[PRODUCTION_TYPE]==8) $tsktype=88; 
			else if ($row[PRODUCTION_TYPE]==10) $tsktype=87;
			else if ($row[PRODUCTION_TYPE]==11) $tsktype=91;			
			
			else if($row[csf("embel_name")]==3 && $row[PRODUCTION_TYPE]==2) $tsktype=89;
			else if($row[csf("embel_name")]==3 && $row[PRODUCTION_TYPE]==3) $tsktype=90;
			
			
			$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][$tsktype]['max_start_date']=$row[csf("maxd")];
			$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][$tsktype]['min_start_date']=$row[csf("mind")];
			$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][$tsktype]['doneqnty']+=$row[PRODUCTION_QUANTITY];
			$tna_task_update_data[$row[PO_BREAK_DOWN_ID]][$tsktype]['reqqnty']=$po_order_details[$row[PO_BREAK_DOWN_ID]]['po_quantity_pcs'];
...
//Yarn Daying---------------------------------------------
		...
		where b.job_no=d.job_no_mst and  a.id=b.mst_id and b.prod_id=c.id and b.transaction_type=1 and b.item_category=1 and a.entry_form=1 and b.status_active=1 and b.is_deleted=0 ".where_con_using_array($job_no_array,1,'b.job_no')." group by d.id order by d.id asc";
		//echo $sql_task;die;
		$result = sql_select( $sql_task );
		$embelishment_update_task = array(); 
		foreach( $result as $row ) 
...
//new...........................................start;
			...
			where 
			e.id=b.sample_name and a.quotation_id=c.id and c.job_no=d.job_no_mst and a.id=b.sample_mst_id and b.entry_form_id=117 and e.sample_type in(2,12,13) and b.is_deleted=0 and b.status_active=1  ".where_con_using_array($order_id_array,0,'d.id')." group by d.id ,e.sample_type order by d.id asc";
		$result = sql_select( $sql_task );
		$purchase_update_task = array(); 
		foreach( $result as $row ) 
		{ 
		...
		FROM  wo_booking_mst a, wo_booking_dtls b,lib_sample c WHERE c.id=b.sample_type and a.booking_no=b.booking_no and a.is_short=2 and c.sample_type=2 and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 ".where_con_using_array($order_id_array,0,'b.po_break_down_id')." group by b.po_break_down_id,item_category,c.sample_type order by b.po_break_down_id asc";
...
union all
select  max(a.lc_date) as max_date,min(a.lc_date) as min_date,b.wo_po_break_down_id,sum(b.attached_qnty) as qty  from com_export_lc a,com_export_lc_order_info b where a.id=b.com_export_lc_id  ".where_con_using_array($order_id_array,0,'b.wo_po_break_down_id')." group by b.wo_po_break_down_id";
...
//$tna_task_seq_arr[$row[csf('task_name')]]=$row[csf('task_sequence_no')];
	...
	foreach( $po_order_details____off as $row )  // Non Process Starts Here
	{
			...
			{  
				...
				if($to_process_task[$row[po_id]][$row_task[task_name]]!="")
				{
					...
					{ 
						...
						$data_array_tna_process .="('$mst_id','$row[template_id]','$row[job_no_mst]','$row[po_id]','$row[po_received_date]','$row[shipment_date]','4','$row_task[task_name]','$target_date','$start_date','$finish_date','$notice_date_start','$notice_date_end','$date','$row_task[sequence_no]',$plan_start_flag,$plan_finish_flag,1,0,4)";
						
						$insert_string[] ="('$mst_id','$row[template_id]','$row[job_no_mst]','$row[po_id]','$row[po_received_date]','$row[shipment_date]','4','$row_task[task_name]','$target_date','$start_date','$finish_date','$notice_date_start','$notice_date_end','$date','$row_task[sequence_no]',$plan_start_flag,$plan_finish_flag,1,0,4)";
