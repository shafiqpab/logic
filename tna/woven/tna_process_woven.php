...
function fnc_tna_process( operation )
{
	...
	// alert(data)
	http.open("POST","requires/tna_process_woven_controller.php",true);
...
function open_popup()
{
	...
	var page_link='requires/tna_process_woven_controller.php?action=search_po_number&company='+company+'&buyer='+buyer+"&is_manual_process=1"; 
		...
		<form name="tnaprocess_1" id="tnaprocess_1">	
			<table cellpadding="0" cellspacing="2" width="100%">
			 	<tr>
					...
					<td colspan="1">
					   <?
                           echo create_drop_down( "cbo_company", 170,"select id,company_name from lib_company where status_active=1 and is_deleted=0 order by company_name","id,company_name", 1, "-- All company --", 0, "load_drop_down( 'requires/tna_process_woven_controller', this.value, 'load_drop_down_buyer', 'buyer_td')" );
                       ?>                     
					</td>
