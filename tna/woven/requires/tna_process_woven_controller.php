...
// $tba_color_id = return_field_value("textile_tna_process_base", "variable_order_tracking", "company_name ='TBA'");
// $sql = "SELECT "
// variable_order_tracking
// //  company_name*variable_list*tna_process_type*textile_tna_process_base*updated_by*update_date


 

$tna_process_based_arr=array();
$na_process_based_sql=sql_select("SELECT a.id, a.COMPANY_NAME, a.TEXTILE_TNA_PROCESS_BASE FROM variable_order_tracking a WHERE VARIABLE_LIST=31");
foreach($na_process_based_sql as $rows)
{
	$tna_process_based_arr[$rows['COMPANY_NAME']] = $rows['TEXTILE_TNA_PROCESS_BASE']; 
}
...
{ 
	...
	$sql = "select task_name,completion_percent,task_sequence_no from  lib_tna_task where is_deleted = 0 and status_active=1 and task_type=6 order by task_sequence_no asc";
	...
	foreach( $company_array as $cbo_company )
	{
		...
		{
			$sql = "SELECT id,task_catagory,task_name,task_short_name,task_type,module_name,link_page,penalty,completion_percent,task_sequence_no FROM lib_tna_task WHERE is_deleted = 0 and status_active=1 and task_type=6 order by task_sequence_no";
			$result = sql_select( $sql ) ;
		...
		foreach($po_data_chunk_array as $data_array){
			...
			if ($is_delete == 1) {
				$con = connect();
				$rid = execute_query("delete FROM tna_process_mst WHERE  task_type=6 and po_number_id in(".implode(',',$tmp_po_array).")", 1);
				oci_commit($con);
				disconnect($con);
			}
			...
			if ($delete_history_process == 1) {
				$con = connect();
				$rid = execute_query("update TNA_PLAN_ACTUAL_HISTORY set STATUS_ACTIVE=0,IS_DELETED=1 WHERE  task_type=6  and po_number_id in(".implode(',',$tmp_po_array).")", 1);
				oci_commit($con);
				disconnect($con);
			}
			//End TNA Delete..............................................
		...
		foreach($data_array as $row)
		{
			...
			{ 
				//print_r($fabricdata['woven']['finish']);echo 11;die;
				
				foreach($fabricdata['woven']['finish'][$row['ID']] as $sourceId=>$sourceUomRows){
					if($sourceId==1){
						$tna_task_update_data[$row['ID']][276]['reqqnty']=array_sum($fabricdata['woven']['grey'][$row['ID']][$sourceId]);
						$to_process_task[$row['ID']][276]=276;
					}
					else{
						$tna_task_update_data[$row['ID']][31]['reqqnty']+=array_sum($fabricdata['woven']['grey'][$row['ID']][$sourceId]);
						$to_process_task[$row['ID']][31]=31;
						
						$tna_task_update_data[$row['ID']][305]['reqqnty']+=array_sum($fabricdata['woven']['grey'][$row['ID']][$sourceId]);
				...
				if( array_sum($fabricdata_production['woven']['grey'][$row['ID']])>0)
				{
					$tna_task_update_data[$row['ID']][60]['reqqnty']+=array_sum($fabricdata_production['woven']['grey'][$row['ID']]);
					$to_process_task[$row['ID']][60]=60;
					$tna_task_update_data[$row['ID']][72]['reqqnty']+=array_sum($fabricdata_production['woven']['grey'][$row['ID']]);
					$to_process_task[$row['ID']][72]=72;
					
					$tna_task_update_data[$row['ID']][61]['reqqnty']+=array_sum($fabricdata_production['woven']['grey'][$row['ID']]);
					$to_process_task[$row['ID']][61]=61;
					

					
					$tna_task_update_data[$row['ID']][31]['reqqnty']+=array_sum($fabricdata_production['woven']['grey'][$row['ID']]);
					$to_process_task[$row['ID']][31]=31;
				
					
					$tna_task_update_data[$row['ID']][74]['reqqnty']+=array_sum($fabricdata_production['woven']['grey'][$row['ID']]);
					$to_process_task[$row['ID']][74]=74;
				}
				
				
				if( array_sum($fabricdata_production['woven']['finish'][$row['ID']])>0)
				{
					$tna_task_update_data[$row['ID']][64]['reqqnty']+=array_sum($fabricdata_production['woven']['finish'][$row['ID']]);
				...
				if( count($trims_data[$row['ID']])>0 )
				{
					foreach($trims_data[$row['ID']] as $source_id=>$dataRows){
						foreach($dataRows as $item_id=>$dataRow){
							...
							if($source_id==1){ 
								if($trims_data[$row['ID']][$source_id][$item_id][2]){
									$tna_task_update_data[$row['ID']][278]['reqqnty']+=$trims_data[$row['ID']][$source_id][$item_id][2];
								...
								if($trims_data[$row['ID']][$source_id][$item_id][2]){
									$tna_task_update_data[$row['ID']][32]['reqqnty']+=$trims_data[$row['ID']][$source_id][$item_id][2];
									$to_process_task[$row['ID']][32]=32;
									$tna_task_update_data[$row['ID']][307]['reqqnty']+=$trims_data[$row['ID']][$source_id][$item_id][2];
				...
				foreach($fabricdata['woven']['grey'][$row['ID']] as $sourceId=>$sourceUomRows){
					if($sourceId==1){
						...
						$tna_task_update_data[$row['ID']][311]['reqqnty']+=array_sum($fabricdata['woven']['grey'][$row['ID']][$sourceId]);
						$to_process_task[$row['ID']][311]=311;
					}
				}
             /*	if( array_sum($fabricdata['woven']['grey'][$row['ID']])>0)// Woven Purchase
				{
    			...
    			*/
				...
				$job_no_array[$row[csf("job_no_mst")]]= $row["JOB_NO_MST"];
				...
				$po_order_details[$row['ID']]['job_no_mst']=$row["JOB_NO_MST"];
				$po_order_details[$row['ID']]['po_quantity']=$row["PO_QUANTITY"];
				$po_order_details[$row['ID']]['po_quantity_pcs']=$row["PO_QUANTITY_PCS"];
				$po_order_details[$row['ID']]['template_id']=$template_id;
				$po_order_details[$row['ID']]['po_id']=$row['ID'];
				$po_order_details[$row['ID']]['is_confirmed']=$row["IS_CONFIRMED"];
				$po_order_details[$row['ID']]['tna_task_from_upto']=$row["TNA_TASK_FROM_UPTO"];
				//$to_process_task
				$tna_task_update_data[$row['ID']][80]['max_start_date']=$row['PP_MEETING_DATE'];
				$tna_task_update_data[$row['ID']][80]['min_start_date']=$row['PP_MEETING_DATE'];
				//184=PP Meeting AOP;
				$tna_task_update_data[$row['ID']][184]['max_start_date']=$row['PP_MEETING_DATE'];
				$tna_task_update_data[$row['ID']][184]['min_start_date']=$row['PP_MEETING_DATE'];
				//185=PP Meeting YD;
				$tna_task_update_data[$row['ID']][185]['max_start_date'] = $row['PP_MEETING_DATE'];
				$tna_task_update_data[$row['ID']][185]['min_start_date'] = $row['PP_MEETING_DATE'];
				
				if($row['PACK_HANDOVER_DATE']){
					$tna_task_update_data[$row['ID']][354]['doneqnty']=1;
					$tna_task_update_data[$row['ID']][354]['reqqnty']=1; 
					$tna_task_update_data[$row['ID']][354]['max_start_date']=$row["PACK_HANDOVER_DATE"];
					$tna_task_update_data[$row['ID']][354]['min_start_date']=$row["PACK_HANDOVER_DATE"];	
				}
     ...
     //**************************Actual TNA Process Start**********************
		...
		foreach($data_arr as $row)
		{
			...
			$set_item_ratio=$gmtsitem_ratio_array[$row['JOB_NO']][$row['ITEM_NUMBER_ID']];
			
			$requirment=$cons_arr[$row['JOB_NO']][$row['ID']][$row['PRE_COST_DTLS_ID']][$row['ITEM_NUMBER_ID']][$row[csf('color_number_id')]][$row['SIZE_NUMBER_ID']][$row['COLOR_TYPE_ID']]['requirment'];
			
			
			$cons=$cons_arr[$row['JOB_NO']][$row['ID']][$row['PRE_COST_DTLS_ID']][$row['ITEM_NUMBER_ID']][$row[csf('color_number_id')]][$row['SIZE_NUMBER_ID']][$row['COLOR_TYPE_ID']]['cons'];
			
			
			$req_fin_fab_qty_arr[$row['ID']][$row['SOURCE_ID']][$row['COLOR_TYPE_ID']] +=(($row[csf("plan_cut_qnty")]/($costing_per_qty*$set_item_ratio))*$cons);
			$req_grey_fab_qty[$row['ID']][$row['SOURCE_ID']][$row['COLOR_TYPE_ID']] +=(($row[csf("plan_cut_qnty")]/($costing_per_qty*$set_item_ratio))*$requirment);
		}
		...
		//print_r($to_process_task);die;
		//print_r($tna_task_update_data[63956][277]);die;		
    ...
    //Emb Part budget check......................................................
		...
		foreach( $result as $row ) 
		{
			...
			if( $gross_level==1 )
			{//solid..
				$tna_task_update_data[$row["PO_BREAK_DOWN_ID"]][85]['reqqnty']+=$row["AMOUNT"];
				$to_process_task[$row["PO_BREAK_DOWN_ID"]][85]=85;
    ...
    //****************************************************************************************		
		...
		$sql = "SELECT TASK_NUMBER,PO_NUMBER_ID,ACTUAL_START_DATE,ACTUAL_FINISH_DATE,TASK_START_DATE,TASK_FINISH_DATE ,PLAN_START_FLAG,PLAN_FINISH_FLAG,ACTUAL_START_FLAG,ACTUAL_FINISH_FLAG FROM tna_plan_actual_history WHERE status_active =1 and is_deleted=0 and task_type=6 ".where_con_using_array($order_id_array,0,'po_number_id')." order by id";
		//echo $sql;die;
		...
		foreach( $result as $row ) 
		{
			...
			$tna_updated_date[$row['PO_NUMBER_ID']][$row['TASK_NUMBER']]['actualtartflag']=$row['ACTUAL_START_FLAG'];
			$tna_updated_date[$row['PO_NUMBER_ID']][$row['TASK_NUMBER']]['actualfinishflag']=$row['ACTUAL_FINISH_FLAG'];
		}
		
		
	// TNA Processs Data List for TNA UPDATE
		$sql = "SELECT id,job_no,po_number_id,task_category,task_number,actual_start_date,actual_finish_date,template_id FROM tna_process_mst WHERE status_active =1 and is_deleted = 0  and task_type=6 ".where_con_using_array($order_id_array,0,'po_number_id')."";
		...
		{
			$con = connect();
			$rid=execute_query("delete FROM tna_process_mst WHERE task_type=6 and po_number_id in ( ".implode(",",$changed_templates)." )",1);
	...
	// Sample Approval Update Data
		...
		$sql_task = "SELECT count(a.id) as cid,a.po_break_down_id,sample_type,max(submitted_to_buyer) as max_submitted_to_buyer,min(submitted_to_buyer) as min_submitted_to_buyer,max(approval_status_date) as max_approval_status_date,min(approval_status_date) as min_approval_status_date
			 FROM wo_po_sample_approval_info  a, lib_sample b,wo_po_color_size_breakdown c WHERE sample_type in (2,3,4,7,8,9,10,11,9,5,13,14,12) and b.BUSINESS_NATURE = 3 and c.id=a.color_number_id and a.po_break_down_id=c.po_break_down_id and a.job_no_mst=c.job_no_mst and c.status_active=1 AND c.is_deleted = 0 and b.id=a.SAMPLE_TYPE_ID and a.is_deleted = 0 and a.status_active=1 and b.is_deleted = 0 and b.status_active=1";
		$sql_task .= where_con_using_array(explode(',',$po_ids),0,'a.po_break_down_id');
		...
		foreach( $result as $row ) 
		{
            ...
            WHERE sample_type IN (2,3,4,7,8,9,10,11,12,13,14) and b.BUSINESS_NATURE = 3 AND c.id = a.color_number_id AND a.po_break_down_id = c.po_break_down_id AND a.job_no_mst = c.job_no_mst  AND c.status_active = 1 AND c.is_deleted = 0  AND b.id = a.SAMPLE_TYPE_ID AND a.is_deleted = 0  AND a.status_active = 1 AND b.is_deleted = 0 AND b.status_active = 1";
			$sql_task .= where_con_using_array($order_id_array,0,'c.po_break_down_id');
			//echo $sql_task;die;
			// $p=1;
			// foreach($job_no_list_arr as $job_no_process)
			// {
		...
		union all
		SELECT a.PO_BREAKDOWN_ID, null as booking_id, null as RECEIVE_BASIS,d.SOURCE, sum(a.quantity) as QTY, min(c.TRANSFER_DATE) MINDATE, max(c.TRANSFER_DATE) MAXDATE from order_wise_pro_details a, inv_item_transfer_dtls b, inv_item_transfer_mst c, wo_booking_mst d
		where a.dtls_id=b.id and b.mst_id=c.id and b.to_ord_book_id=d.id ".where_con_using_array($order_id_array,0,'a.po_breakdown_id')." and a.entry_form=258 and c.entry_form=258 and a.trans_type=5 and a.status_active=1 and a.is_deleted=0 group by a.po_breakdown_id,d.SOURCE";
		...
		$sql_task .=where_con_using_array(explode(',',$po_ids),0,'po_break_down_id');
		...
		//print_r($tna_task_update_data[69345][10]);die;
	...
	// Embelishment Approval Data for Update
		...
		//$job_no_list_arr=array_chunk(array_unique(explode(",",$job_no_list)),999);
		$sql_task = "SELECT count(id) as cid,po_break_down_id,max(submitted_to_buyer) as max_submitted_to_buyer,min(submitted_to_buyer) as min_submitted_to_buyer,max(approval_status_date) as max_approval_status_date,min(approval_status_date) as min_approval_status_date,embellishment_id FROM wo_po_embell_approval WHERE is_deleted = 0 and status_active=1";
		$sql_task .=where_con_using_array(explode(',',$po_ids),0,'po_break_down_id');
		...
		{ 
			...
			if($row['SOURCE']==1){
				...
				if ($row[csf("item_category")]==2 || $row[csf("item_category")]==3) { 
					$qnty=$row[csf("gfb_qnty")]; 

				 
					$tna_task_update_data[$row[csf("po_break_down_id")]][31]['doneqnty']+=$qnty;
					...
					$tna_task_update_data[$row[csf("po_break_down_id")]][34]['doneqnty']+=$qnty;
					...
					$tna_task_update_data[$row[csf("po_break_down_id")]][311]['doneqnty']+=$qnty;
					$tna_task_update_data[$row[csf("po_break_down_id")]][311]['max_start_date']=$row[csf("end_date")];
					$tna_task_update_data[$row[csf("po_break_down_id")]][311]['min_start_date']=$row[csf("start_date")];
		...
		//print_r($tna_task_update_data[][276]); die;
	...
	where e.id=a.mst_id and a.prod_id=c.id and b.trans_id=a.id and c.item_group_id=d.id and d.trim_type in (1,2) and b.entry_form=24 ".where_con_using_array(explode(",",$po_ids),0,'b.po_breakdown_id')."  and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 and c.status_active =1 and c.is_deleted=0 and d.status_active =1 and d.is_deleted=0 and e.status_active =1 and e.is_deleted=0 group by b.po_breakdown_id,d.trim_type,e.source order by b.po_breakdown_id"; //echo $sewing_finish_trims_sql;die;
		...
		foreach($sewing_finish_trims_sql_result as $row)
		{
			if($row['TRIM_TYPE'] == 1){//Sewing Trims
				if($row['SOURCE'] == 1){//Forain..........
					$tna_task_update_data[$row['PO_BREAKDOWN_ID']][301]['max_start_date']=$row['MAXDATE'];
					$tna_task_update_data[$row['PO_BREAKDOWN_ID']][301]['min_start_date']=$row['MINDATE'];
					$tna_task_update_data[$row['PO_BREAKDOWN_ID']][301]['doneqnty']=$row['QTY'];
				}
				else{//Local..........
					$tna_task_update_data[$row['PO_BREAKDOWN_ID']][310]['max_start_date']=$row['MAXDATE'];
					$tna_task_update_data[$row['PO_BREAKDOWN_ID']][310]['min_start_date']=$row['MINDATE'];
					$tna_task_update_data[$row['PO_BREAKDOWN_ID']][310]['doneqnty']=$row['QTY'];
				}
			}
			else{//Finishing Trims
				if($row['SOURCE'] == 1){//Forain..........
					$tna_task_update_data[$row['PO_BREAKDOWN_ID']][300]['max_start_date']=$row['MAXDATE'];
					$tna_task_update_data[$row['PO_BREAKDOWN_ID']][300]['min_start_date']=$row['MINDATE'];
					$tna_task_update_data[$row['PO_BREAKDOWN_ID']][300]['doneqnty']=$row['QTY'];
				}
				else{//Local..........
					$tna_task_update_data[$row['PO_BREAKDOWN_ID']][309]['max_start_date']=$row['MAXDATE'];
					$tna_task_update_data[$row['PO_BREAKDOWN_ID']][309]['min_start_date']=$row['MINDATE'];
	...
	//print_r($tna_task_update_data[67794][322]);die;


	//woven precost app...........................................end;
	...
	foreach( $po_order_details as $row )  // Non Process Starts Here
	{
		...
		foreach($tna_task_seq_arr as $taskId=>$seq)
		{
			...
			//forain/local......................
			foreach(array(311,302,301,310,306,277,278,279,270,307,300,309) as $vid)
			{
 ...
 //---------------------------------------------------------------------------------------------------------
		...
		$field_array_tna_process="id,template_id,job_no,po_number_id,po_receive_date,shipment_date,task_category,task_number,target_date,task_start_date,task_finish_date,notice_date_start,notice_date_end,process_date,sequence_no,plan_start_flag,plan_finish_flag,ACTUAL_START_FLAG,ACTUAL_FINISH_FLAG,status_active,is_deleted,task_type";
			...
			{  
				...
				if($to_process_task[$row['po_id']][$row_task['task_name']]!="")
				{
					...
					{ 
						...
						$actual_start_flag=$tna_updated_date[$row['po_id']][$row_task['task_name']]['actualtartflag']*1;
						$actual_finish_flag=$tna_updated_date[$row['po_id']][$row_task['task_name']]['actualfinishflag']*1;
						
						
						//$data_array_tna_process .="('$mst_id','$row[template_id]','$row[job_no_mst]','$row[po_id]','$row[po_received_date]','$row[shipment_date]','1','$row_task[task_name]','$target_date','$start_date','$finish_date','$notice_date_start','$notice_date_end','$date','$row_task[sequence_no]',$plan_start_flag,$plan_finish_flag,1,0,6)";
						
						$insert_string[] ="('$mst_id','$row[template_id]','$row[job_no_mst]','$row[po_id]','$row[po_received_date]','$row[shipment_date]','1','$row_task[task_name]','$target_date','$start_date','$finish_date','$notice_date_start','$notice_date_end','$date','$row_task[sequence_no]',$plan_start_flag,$plan_finish_flag,$actual_start_flag,$actual_finish_flag,1,0,6)";
...
<script>
		...
		function generate_list_view(dataStr){
			...
			show_list_view ( dataArr[0]+'_'+dataArr[1]+'_'+dataArr[2]+'_'+dataArr[3]+'_'+dataArr[4]+'_'+dataArr[5]+'_'+dataArr[6]+'_'+dataArr[7]+'_'+dataArr[8], 'ponumber_search_list_view', 'search_div', 'tna_process_woven_controller', 'setFilterGrid(\'tbl_list_search\',-1)');
		}
            	...
            	<table cellspacing="0" cellpadding="0"  border="1" rules="all" class="rpt_table" align="center">
        			...
        			<tr>
                        ...
                        <input type="hidden" id="garments_nature" value="<? echo $garments_nature; ?>">
							<? 
								echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", $company,"load_drop_down( 'tna_process_woven_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
							?>
                    </td>
                   	<td id="buyer_td">
                     <? 
...
<script>
	...
	load_drop_down( 'tna_process_woven_controller', <? echo $company;?>, 'load_drop_down_buyer', 'buyer_td' );
