...
if ( $action=="tna_process" )
{
	...
	foreach( $company_array as $cbo_company )
	{
		...
		else 
		{
			...
			{
				$con = connect();
				$rid=execute_query("delete FROM tna_process_mst WHERE task_type=2 and PO_NUMBER_ID in (".implode(",",explode(',',$txt_booking_no)).")",1);
		...
		if($textile_tna_process_base==2){//Sales Order Base
			...
			$sql = "select a.id as JOB_ID,a.DELIVERY_DATE,a.booking_approval_date as BOOKING_DATE,a.JOB_NO,a.BUYER_ID,b.id as PO_ID,b.COLOR_TYPE_ID,b.COLOR_ID,b.FABRIC_DESC,b.DETERMINATION_ID,b.GSM_WEIGHT,b.DIA,sum(b.finish_qty) as FIN_FAB_QNTY,sum(b.grey_qty) as GREY_FAB_QNTY
			
            from fabric_sales_order_mst a,fabric_sales_order_dtls b  
			WHERE a.job_no=b.job_no_mst and b.is_deleted = 0 and b.status_active=1 and a.is_deleted = 0 and a.status_active=1 and a.delivery_date >'$tna_process_start_date' and company_id=".$cbo_company." $buyer_cond $booking_con
			group by a.id,a.booking_approval_date,a.delivery_date,a.job_no,a.buyer_id,b.id,b.COLOR_TYPE_ID,b.COLOR_ID,b.FABRIC_DESC,b.DETERMINATION_ID,b.GSM_WEIGHT,b.DIA ORDER BY a.delivery_date asc";			
		...
		foreach($data_array as $row)
		{
			...
			//color type wise req qty...................................start;
				...
				if( $gross_level==1 ){
					...
					if( in_array($row[COLOR_TYPE_ID],array(2,3,4,6,32,33,44,47,48,63))){
						...
						$req_qty_by_job_po[52][$row[JOB_NO]][$row[PO_ID]]+=$row[GREY_FAB_QNTY];
			...
			//color type wise req qty.....................................end;	
				...
				$tna_task_update_data[$row[PO_ID]][61]['reqqnty']+=$row[GREY_FAB_QNTY];
				...
				if(in_array($row[COLOR_TYPE_ID],array(5,7))){//AOP
...
$sql="SELECT a.JOB_NO,b.id as PO_ID,B.GREY_QTY, SUM(f.GREY_RECEIVE_QNTY) AS GREY_RECEIVE_QNTY,MAX(e.RECEIVE_DATE) AS MAX_DATE,MIN(e.RECEIVE_DATE) AS MIN_DATE  from fabric_sales_order_mst a,fabric_sales_order_dtls b,ppl_planning_entry_plan_dtls d,INV_RECEIVE_MASTER e,PRO_GREY_PROD_ENTRY_DTLS f where a.id=b.mst_id and a.id=d.po_id and e.id=f.mst_id and d.dtls_id=e.BOOKING_ID AND f.BODY_PART_ID=b.BODY_PART_ID  and  b.GSM_WEIGHT=f.GSM  and e.RECEIVE_BASIS=2   and a.company_id=$cbo_company ".where_con_using_array($booking_id_arr,0,'b.id')." GROUP BY a.JOB_NO,b.id,B.GREY_QTY ";// and d.dtls_id=11724
	...
	foreach($knit_gray_data_array as $row)
	{
		...
		$tmpData[DONE_QTY][$row[JOB_NO]][$row[PO_ID]]=$row[QTY];
		$tmpData[REQU_QTY][$row[JOB_NO]][$row[PO_ID]]=$row[GREY_QTY];
	}

	foreach($tmpData[JOB_NO] as $poId=>$jobNo){
		$reqQty=array_sum($tmpData[REQU_QTY][$jobNo]);
...
//Kniting Production...................................................................................start;	
  ...
  // echo $sql;die;
	...
	$sql="select e.FEBRIC_DESCRIPTION_ID,c.id as JOB_ID,d.FABRIC_DESC,e.GSM as GSM_WEIGHT, e.WIDTH as DIA,d.id as PO_ID,d.COLOR_TYPE_ID,min(a.receive_date) MIN_DATE,max(a.receive_date) MAX_DATE, sum(b.qnty) QTY 
...
group by e.FEBRIC_DESCRIPTION_ID,c.id,d.FABRIC_DESC,e.GSM, e.WIDTH,d.id,d.color_type_id";
			//echo $sql;die;  
...
//Daying Production................................................................start;	
	...
	// print_r($tna_task_update_data[15594][61]);die;
	  ...
	  //echo $sql;die;
		...
		foreach($fin_fab_data_array as $row)
		{
			if($row[csf("entry_form")]==7 and $row[csf("receive_basis")]==5){
				$tna_task_update_data[$row[csf("job_no")]][64]['max_start_date']=$row[MAX_DATE];
				$tna_task_update_data[$row[csf("job_no")]][64]['min_start_date']=$row[MIN_DATE]; 
				$tna_task_update_data[$row[csf("job_no")]][64]['doneqnty']=$row[csf("qty")];
			}
			else if($row[csf("entry_form")]==37 and $row[csf("receive_basis")]==10){
				$tna_task_update_data[$row[csf("job_no")]][73]['max_start_date']=$row[MAX_DATE];
				$tna_task_update_data[$row[csf("job_no")]][73]['min_start_date']=$row[MIN_DATE]; 
...
//echo $sql;die;
		...
		foreach($yarn_transfer_data_array as $row)
		{
			...
			$tmpData[JOB_NO][$row[PO_ID]]=$row[FSO_NO];
		...
		foreach($tmpData[JOB_NO] as $poId=>$jobNo){
			$reqQty=array_sum($tmpData[JOB_QTY][$jobNo]);
...
//---------------------------------------------------------------------------------------------------------
	...
	// print_r($tna_task_update_data);die;
	//print_r($tna_process_details[15594]);die;//15592,15593,15594	15592,15593,15594
 	//print_r($tna_task_update_data[15593][60]);die;
	$insert_string=array();
...
if ($action=="ponumber_search_list_view")
{
 ...
 ?>
		...
		<?
			...
			{
				/*$sql = "select a.within_group,b.id,a.company_id,a.booking_date,a.delivery_date,a.job_no as booking_no,a.buyer_id,$lead_time
				from fabric_sales_order_mst a, FABRIC_SALES_ORDER_DTLS b
				WHERE a.id=b.mst_id and a.is_deleted = 0 and a.status_active=1 and company_id=".$company." $buyer_con $booking_no_con $date_cond
				ORDER BY a.delivery_date asc";*/
