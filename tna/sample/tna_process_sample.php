...
function fnc_tna_process( operation )
{
	...
	// alert(data)
	http.open("POST","requires/tna_process_sample_controller.php",true);
...
function open_popup()
{
	...
	var page_link='requires/tna_process_sample_controller.php?action=search_po_number&company='+company+'&buyer='+buyer+"&is_manual_process=1"; 
		...
		<form name="tnaprocess_1" id="tnaprocess_1">	
			<table cellpadding="0" cellspacing="2" width="100%">
			 	<tr>
					...
					<td colspan="1">
					   <?
                           echo create_drop_down( "cbo_company", 170,"select id,company_name from lib_company where status_active=1 and is_deleted=0 order by company_name","id,company_name", 1, "-- All company --", 0, "load_drop_down( 'requires/tna_process_sample_controller', this.value, 'load_drop_down_buyer', 'buyer_td')" );
                       ?>                     
					</td>
			   ...
			   <tr>
                <tr>
					...
					<td colspan="3">
				       <input type="text" name="txt_ponumber" id="txt_ponumber" class="text_boxes" placeholder="Browse" style="width:428px" onClick="open_popup()"/> 
