...
$buyer_name = return_library_array("SELECT buyer_name,id FROM  lib_buyer WHERE is_deleted = 0 and status_active=1 order by id asc","id","buyer_name");
...
if($action=="generate_tna_report")
{
	...
	//$menual_update_task_arr=array(133,125,18,165,166,167,130,176,177,131,181,134,71,132);
	...
	if($txt_job_no=="") $txt_job_no=""; else $txt_job_no=" and b.job_no_prefix_num ='$txt_job_no'";
	...
	if($db_type==0)
	{
		$sql ="select b.buyer_id,b.within_group,a.po_number_id, a.job_no, a.shipment_date,max(b.delivery_date) as pub_shipment_date, a.template_id, a.po_receive_date,b.insert_date,";
		$i=1;
		...
		$sql .=" from  tna_process_mst a, fabric_sales_order_mst b where a.po_number_id=b.id  and b.status_active=1  and a.task_type=2 and b.company_id=$cbo_company_id $txt_job_no $txt_order_no $date_range  group by b.buyer_id,b.within_group,a.po_number_id,a.job_no,a.template_id,a.shipment_date,b.insert_date order by a.shipment_date,a.po_number_id,a.job_no"; 
	}
	else
	{
		$sql ="select b.buyer_id,b.within_group,a.po_number_id, a.job_no, max(a.shipment_date) as shipment_date,max(b.delivery_date) as pub_shipment_date,a.template_id, max(a.po_receive_date) as po_receive_date,b.insert_date,";
		$i=1;
   ...
   <div style="margin:0 1%;">
    ...
    <table width="<? echo $width+140; ?>" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all">
    	<thead>
        	<tr>
                ...
                <th rowspan="2">Sales Order/ Booking No</th>
		...
		?>
        		<tr bgcolor="<? echo $bgcolor; ?>" style="vertical-align:middle" height="25" onClick="change_color('tr_<? echo $h;?>','<? echo $bgcolor;?>')" id="tr_<? echo $h; ?>">
                    <td width="40" rowspan="3" align="center"><? echo $kid++;?></td>
                     ...
                     <td rowspan="3"><? echo $row[csf('job_no')]; ?></td>
    ...
    <div style="width:<? echo $width+140; ?>px;" align="left">
         <table width="100%" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all">
            <tfoot>
                <th colspan="<? echo (count($tna_task_id)*2)+10;?>">&nbsp;</th>
            </tfoot>
        </table>
    </div>
...
if($action=="edit_update_tna")
{
	...
	extract($_REQUEST);
		...
		$sql="select a.within_group,a.style_ref_no,a.sales_booking_no,a.job_no,a.buyer_id as buyer_name,b.buyer_id as po_buyer  from fabric_sales_order_mst a,wo_booking_mst b where a.sales_booking_no=b.booking_no and a.id=$booking_id";
		$result=sql_select($sql);
		
		$tna= "select template_id,task_number,task_start_date,task_finish_date,actual_start_date,actual_finish_date,plan_start_flag,plan_finish_flag,commit_start_date,commit_end_date from  tna_process_mst where id=$mid  and task_type=2";
...
function fnc_tna_actual_date_update_reponse()
{
    ...
    <table width="600" cellspacing="0" cellpadding="0" class="rpt_table">
        ...
        <tr>
            ...
            <input type="hidden" id="txt_plan_actual_history" name="txt_plan_actual_history"  value="<? echo $tna_history[0][csf('id')].'_'.$tna_result[0][csf('template_id')].'_'.$tna_result[0][csf('task_number')].'_'.$booking_id.'_'.$result[0][csf('job_no')]; ?>" />
