...
.group-7{background-image: linear-gradient(rgb(173, 112, 255) 10%, rgb(123, 170, 214) 96%)!important;}
...
<?
	...
	$lead_time_array=return_library_array("select task_template_id,lead_time from tna_task_template_details where is_deleted=0 and status_active=1  group by lead_time,task_template_id","task_template_id",'lead_time');
	if($db_type==0)
	{
        	...
        	<tr>
                ...
                <?
					...
					foreach($tna_task_group_array as $task_arr)
					{
						...
						{
							echo '<th width="160" colspan="2">'.$tna_task_short_array[$key].'</th>';
						}
					}
					echo '</tr><tr>';
    ...
    <div style="width:<? echo $width+140; ?>px;" align="left">
         <table width="100%" border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all">
            <tfoot>
                ...
                <th width="80"></th>
                <th width="80"></th>
                <th colspan="<? echo (count($tna_task_id)*2)+1;?>"></th>
            </tfoot>
        </table>
    </div>
...
if($action=="generate_task_wise_report")
{
	...
	$lead_time_array=return_library_array("select task_template_id,lead_time from tna_task_template_details where is_deleted=0 and status_active=1 group by lead_time","task_template_id",'lead_time');
	$txt_job_no=str_replace("'","",$txt_job_no);
	if($txt_job_no=="") $txt_job_no=""; else $txt_job_no=" and a.job_no_prefix_num ='$txt_job_no'";
	$txt_order_no=str_replace("'","",$txt_order_no);
	if($txt_order_no=="") $txt_order_no=""; else $txt_order_no=" and b.po_number ='$txt_order_no'";
...
if($action=="edit_update_tna")
{
	...
	if( $type==1 )
		...
		$tna= "select template_id,task_number,task_start_date,task_finish_date,actual_start_date,actual_finish_date from  tna_process_mst where id=$mid ";
...
function fnc_tna_actual_date_update( operation )
{
	...
	//var dataString = 'txt_actual_start_date*txt_actual_finish_date';
		...
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_actual_start_date*txt_actual_finish_date*txt_update_tna_id*txt_plan_start_date*txt_plan_finish_date*txt_update_tna_type*txt_plan_actual_history',"../../");
		//alert (data);return;
	...
	{		
		...
		if($update_tna_type==1)
		{
			...
			else
			{
				...
				$field_array="id, template_id,task_number, job_no, po_number_id, task_start_date, task_finish_date, status_active,is_deleted";
				$data_array="(".$hid.",".$htmp_id.",".$htask_id.",'".str_replace("'","",$hjob_id)."',".$hpo_id.",".$txt_plan_start_date.",".$txt_plan_finish_date.",'1','0')";
