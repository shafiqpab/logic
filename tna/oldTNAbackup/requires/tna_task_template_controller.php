...
if($action=="task_name_search")
{
   ...
   // $task_id_arr=return_library_array("select tna_task_id,tna_task_id from  tna_task_template_details where task_template_id =$system_id and status_active=1 and is_deleted = 0",'tna_task_id','tna_task_id');
	...
	$sql="select id, task_name, task_short_name , task_catagory, module_name, link_page,penalty,row_status,task_type from lib_tna_task where is_deleted=0 and row_status=1 $selected_ids $where_con order by task_sequence_no";
	...
	?>
		...
		?>
         		...
         		<tr style="cursor:pointer" onClick="js_set_value(<? echo $i; ?>);<? if ( $is_single==1 ) { echo "parent.emailwindow.hide();"; } ?>" id="search<? echo $i;?>">
                    ...
                    <td width="130"><? echo $lapdip_task_name[$data[csf('task_type')]]; ?></td>
...
if ($action=="show_list_view_template")
{
	...
	$data_array= sql_select("select id,task_template_id,lead_time,material_source,total_task,tna_task_id,deadline,execution_days,notice_before,sequence_no,for_specific,dependant_task,status_active from  tna_task_template_details where task_template_id in (".$data.") and status_active > 0 and is_deleted=0 order by sequence_no"); 
	...
	?>
		...
		?>
				<tr>
                    ...
                    <td>
                        <input name="txtdeadline_<? echo $i; ?>" onFocus="set_all_onclick();" value="<? echo $row[csf("deadline")]; ?>" type="text" id="txtdeadline_<? echo $i; ?>" style="width:90px" class="text_boxes_numeric"/>
                    </td>
                    <td>
...
{
	echo create_drop_down( "cbo_buyer_name", 172, "select a.id,a.buyer_name from lib_buyer a, lib_buyer_tag_company b where a.status_active =1 and a.is_deleted=0 and b.buyer_id=a.id and b.tag_company='$data' and a.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "" );
} 
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		if (is_duplicate_field( "id", "tna_task_template_details", "for_specific=$cbo_buyer_specific and lead_time=$txt_lead_time and task_type=$cbo_task_type" ) == true)
		{
			echo "11**".$temp_id."**".$rID;
			die;
		}
		...
		$field_array="id,task_template_id,lead_time,material_source,task_type,total_task,dependant_task,tna_task_id,deadline,execution_days,notice_before,sequence_no,for_specific,is_deleted,status_active,inserted_by,insert_date";
		 ...
		 for ($i=1;$i<=str_replace("'","",$txt_total_task); $i++)
		 {
			 ...
			 //echo "0**".$$cbotaskshortname."=";
			  if (str_replace("'","",$$cbotaskshortname)!="" && str_replace("'","",$$txtdeadline)!="" && str_replace("'","",$$txtexecutiondays)!="")
			  {
				...
				$data_array .="(".$id.",".$temp_id.",".$txt_lead_time.",".$cbo_material_source.",".$cbo_task_type.",".$txt_total_task.",'".str_replace("'","",$$hiddendependtaskid)."',".$$cbotaskshortname.",".$$txtdeadline.",".$$txtexecutiondays.",".$$txtnoticebefore.",".$$txtsequenceno.",".$cbo_buyer_specific.",0,".$$cbostatus.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				 
				$id=$id+1;
			  }
		 }
		 //echo "0**"."**insert into tna_task_template_details (".$field_array.") values ".$data_array; die;
	...
	else if ($operation==1)   // Update Here
	{
		...
		*/
		$field_array_save="id,task_template_id,lead_time,material_source,task_type,total_task,dependant_task,tna_task_id,deadline,execution_days,notice_before,sequence_no,for_specific,is_deleted,status_active,inserted_by,insert_date";
		$field_array_upd="lead_time*material_source*task_type*total_task*dependant_task*tna_task_id*deadline*execution_days*notice_before*sequence_no*for_specific*is_deleted*status_active*updated_by*update_date";
		$k=0;
		$data_array_save="";
		$id=return_next_id( "id", " tna_task_template_details", 1 ) ;
		for ($i=1;$i<=str_replace("'","",$txt_total_task);$i++)
		 {
			 ...
			 if (str_replace("'","",$$cbotaskshortname)!="" && str_replace("'","",$$cbotaskshortname)!=0 && str_replace("'","",$$txtdeadline)!="" && str_replace("'","",$$txtexecutiondays)!="")
			 {
				 ...
				 {	
					 ...
					 $not_delete_id[$id]=$id;
					 $data_array_save .="(".$id.",".$txt_system_id.",".$txt_lead_time.",".$cbo_material_source.",".$cbo_task_type.",".$txt_total_task.",'".str_replace("'","",$$hiddendependtaskid)."',".$$cbotaskshortname.",".$$txtdeadline.",".$$txtexecutiondays.",".$$txtnoticebefore.",".$$txtsequenceno.",".$cbo_buyer_specific.",0,".$$cbostatus.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				 ...
				 else
				 {
					 ...
					 $data_array_upd[str_replace("'",'',$$updateid)] =explode("*",("".$txt_lead_time."*".$cbo_material_source."*".$cbo_task_type."*".$txt_total_task."*'".str_replace("'","",$$hiddendependtaskid)."'*".$$cbotaskshortname."*".$$txtdeadline."*".$$txtexecutiondays."*".$$txtnoticebefore."*".$$txtsequenceno."*".$cbo_buyer_specific."*0*".$$cbostatus."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
...
if ($action=="task_template_list_view")
{
	...
	extract($_REQUEST);
	list($buyer_id,$search_type,$search_value,$type,$material_source)=explode('**',$data);
...
<body>
    <div align="center" style="width:100%;" >
     ...
     <? 
	 		if($db_type==0)
			{
				$sql= "select task_template_id,lead_time,material_source,total_task,for_specific,task_type from tna_task_template_details where status_active=1 and is_deleted=0 $search_con  group by task_template_id";
			}
			else
			{
				$sql= "select task_template_id,lead_time,material_source,total_task,for_specific,task_type from tna_task_template_details where status_active=1 and is_deleted=0 $search_con  group by task_template_id,lead_time,material_source,total_task,for_specific,task_type";
			}
			 //echo $sql;
...
{
	$data_array=sql_select("select task_template_id,lead_time,material_source,total_task,for_specific,task_type from  tna_task_template_details where task_template_id='$data' and is_deleted=0");
	foreach ($data_array as $row)
	{
