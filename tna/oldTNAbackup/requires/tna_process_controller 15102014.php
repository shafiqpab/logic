...
if ( $action=="tna_process" )
{
	...
	{ 
		$strcond=""; 
		if($is_delete==1)	execute_query("delete FROM tna_process_mst",1) ;  
	}
	else 
	{
		if($is_delete==1)	execute_query("delete FROM tna_process_mst WHERE  po_number_id in ( $txt_ponumber_id )"); 
	} 
	...
	if ( $txt_ponumber_id=="" )
	{
		...
		{									
			$sql = "SELECT (pub_shipment_date) as shipment_date,job_no_mst,po_received_date,b.id,po_quantity,a.buyer_name,a.garments_nature,b.po_number FROM wo_po_break_down b, wo_po_details_master a WHERE b.is_deleted = 0 and b.status_active=1 and a.job_no=b.job_no_mst and to_char(b.shipment_date)!='0000-00-00' and to_char(b.po_received_date)!='0000-00-00'   ORDER BY b.shipment_date asc";
		}
	}
	else
	{
	...
	//echo "SELECT id,job_no,po_number_id,task_category,task_number,actual_start_date,actual_finish_date FROM   tna_process_mst WHERE job_no in ( $job_no_list ) and status_active =1 and is_deleted = 0";die; 
...
//echo "SELECT po_break_down_id,sum(grey_fab_qnty) as grey_fab_qnty FROM wo_booking_dtls WHERE po_break_down_id in ( $po_ids ) and status_active =1 and is_deleted = 0 group by po_break_down_id";die;
	if($db_type==0)
	{
	...
	else
	{
		...
		$grey_fab_arr=array_chunk(array_unique(explode(",",$po_ids)),999);
		
		$sql = "SELECT po_break_down_id,sum(grey_fab_qnty) as grey_fab_qnty FROM wo_booking_dtls WHERE status_active =1 and is_deleted = 0";
		
		$p=1;
		foreach($grey_fab_arr as $grey_fab_list)
		{
			if($p==1) $sql .=" and (po_break_down_id in(".implode(',',$grey_fab_list).")"; else  $sql .=" or po_break_down_id in(".implode(',',$grey_fab_list).")";
	...
	//print_r(count($po_wise_booking_qnty));die;
...
// Sample Approval Update Data
	...
	{
	 	$sample_app_arr=array_chunk(array_unique(explode(",",$job_no_list)),999);
		
		$sql_task = "SELECT po_break_down_id,sample_type_id,max(submitted_to_buyer) as max_submitted_to_buyer,min(submitted_to_buyer) as min_submitted_to_buyer,max(approval_status_date) as max_approval_status_date,min(approval_status_date) as min_approval_status_date FROM wo_po_sample_approval_info WHERE status_active =1 and is_deleted = 0";
		
		$p=1;
		foreach($sample_app_arr as $sample_app_list)
		{
			if($p==1) $sql_task .=" and (job_no_mst in(".implode(',',$sample_app_list).")"; else  $sql_task .=" or job_no_mst in(".implode(',',$sample_app_list).")";
			
			$p++;
		}
		$sql_task .=")";
		
		$sql_task .=" group by sample_type_id,po_break_down_id order by po_break_down_id asc";
...
// LABDIP Approval Data for Update
	...
	{
	 	$lapdip_app_arr=array_chunk(array_unique(explode(",",$job_no_list)),999);
		
		$sql_task = "SELECT po_break_down_id,max(submitted_to_buyer) as max_submitted_to_buyer,min(submitted_to_buyer) as min_submitted_to_buyer,max(approval_status_date) as max_approval_status_date,min(approval_status_date) as min_approval_status_date FROM wo_po_lapdip_approval_info WHERE status_active =1 and is_deleted = 0";
		
		$p=1;
		foreach($lapdip_app_arr as $lapdip_app_list)
		{
			if($p==1) $sql_task .=" and (job_no_mst in(".implode(',',$lapdip_app_list).")"; else  $sql_task .=" or job_no_mst in(".implode(',',$lapdip_app_list).")";
...
WHERE b.id=a.accessories_type_id and job_no_mst in ( $job_no_list ) and a.is_deleted = 0 and a.status_active=1 group by po_break_down_id,trim_type order by job_no_mst,po_break_down_id asc";
	...
	else
	{
		...
		$trims_app_arr=array_chunk(array_unique(explode(",",$job_no_list)),999);
		
		$sql_task = "SELECT po_break_down_id,max(submitted_to_buyer) as max_submitted_to_buyer,min(submitted_to_buyer) as min_submitted_to_buyer,max(approval_status_date) as max_approval_status_date,min(approval_status_date) as min_approval_status_date,trim_type FROM lib_item_group b, wo_po_trims_approval_info a WHERE b.id=a.accessories_type_id and a.is_deleted = 0 and a.status_active=1";
		
		$p=1;
		foreach($trims_app_arr as $trims_app_list)
		{
			if($p==1) $sql_task .=" and (job_no_mst in(".implode(',',$trims_app_list).")"; else  $sql_task .=" or job_no_mst in(".implode(',',$trims_app_list).")";
...
// Embelishment Approval Data for Update
	...
	else
	{
	 	...
	 	$embell_app_arr=array_chunk(array_unique(explode(",",$job_no_list)),999);
		
		$sql_task = "SELECT po_break_down_id,max(submitted_to_buyer) as max_submitted_to_buyer,min(submitted_to_buyer) as min_submitted_to_buyer,max(approval_status_date) as max_approval_status_date,min(approval_status_date) as min_approval_status_date,embellishment_id FROM wo_po_embell_approval WHERE is_deleted = 0 and status_active=1";
		
		$p=1;
		foreach($embell_app_arr as $embell_app_list)
		{
			if($p==1) $sql_task .=" and (job_no_mst in(".implode(',',$embell_app_list).")"; else  $sql_task .=" or job_no_mst in(".implode(',',$embell_app_list).")";
	...
	//print_r($embelishment_update_task); die;	
...
// Purchase/Booking  Data for Update 2- FB, 4 -Trims, 12- Service
	...
	{
		$wo_booking_arr=array_chunk(array_unique(explode(",",$po_ids)),999);
		
		$sql_task = "SELECT b.po_break_down_id, min(a.booking_date) as start_date, max(a.booking_date) as end_date, a.item_category,a.booking_type
	FROM  wo_booking_mst a, wo_booking_dtls b WHERE a.booking_no=b.booking_no and a.item_category in ( 2,4,12,3 )";
		
		$p=1;
		foreach($wo_booking_arr as $wo_booking_list)
		{
			if($p==1) $sql_task .=" and (b.po_break_down_id in(".implode(',',$wo_booking_list).")"; else  $sql_task .=" or b.po_break_down_id in(".implode(',',$wo_booking_list).")";
...
FROM inv_transaction a,  order_wise_pro_details b where  b.trans_id=a.id and b.entry_form in ( 2,7 ) and b.po_breakdown_id in ( $po_ids ) group by b.po_breakdown_id,b.entry_form order by b.po_breakdown_id";
	...
	else
	{
		...
		$inv_update_arr=array_chunk(array_unique(explode(",",$po_ids)),999);
		
		$sql = "SELECT b.po_breakdown_id,b.entry_form, min(a.transaction_date) mindate, max(a.transaction_date) maxdate, sum(quantity) as prod_qntry 
FROM inv_transaction a,  order_wise_pro_details b where  b.trans_id=a.id and b.entry_form in ( 2,7 )";
		
		$p=1;
		foreach($inv_update_arr as $inv_update_list)
		{
			if($p==1) $sql .=" and (b.po_breakdown_id in(".implode(',',$inv_update_list).")"; else  $sql .=" or b.po_breakdown_id in(".implode(',',$inv_update_list).")";
...
where a.prod_id=b.id and b.trans_id=a.id and c.item_group_id=d.id and d.trim_type in (1,2) and b.entry_form in ( 24 ) and b.po_breakdown_id in ( $po_ids ) group by b.po_breakdown_id,d.trim_type order by b.po_breakdown_id";
	...
	{
		$tran_update_arr=array_chunk(array_unique(explode(",",$po_ids)),999);
...
FROM inv_transaction a,  order_wise_pro_details b, product_details_master c , lib_item_group d where  a.prod_id=c.id and b.trans_id=a.id and c.item_group_id=d.id and d.trim_type in (1,2) and b.entry_form in ( 24 )";
		
		$p=1;
		foreach($tran_update_arr as $tran_update_list)
		{
			if($p==1) $sql .=" and (b.po_breakdown_id in(".implode(',',$tran_update_list).")"; else  $sql .=" or b.po_breakdown_id in(".implode(',',$tran_update_list).")";
...
FROM inv_receive_master a,  order_wise_pro_details b, pro_finish_fabric_rcv_dtls c where  c.mst_id=a.id and c.id=b.dtls_id and b.entry_form in ( 7 ) and b.po_breakdown_id in ( $po_ids ) group by b.po_breakdown_id,b.entry_form order by b.po_breakdown_id";
	...
	else
	{
		...
		$fabric_pro_arr=array_chunk(array_unique(explode(",",$po_ids)),999);
...
FROM inv_receive_master a,  order_wise_pro_details b, pro_finish_fabric_rcv_dtls c where  c.mst_id=a.id and c.id=b.dtls_id and b.entry_form in ( 7 )";
		...
		$p=1;
		foreach($fabric_pro_arr as $fabric_pro_list)
		{
			if($p==1) $sql .=" and (b.po_breakdown_id in(".implode(',',$fabric_pro_list).")"; else  $sql .=" or b.po_breakdown_id in(".implode(',',$fabric_pro_list).")";
	...
	//print_r( $fabric_prod_transaction_update); die;
...
FROM inv_receive_master a,  order_wise_pro_details b, pro_grey_prod_entry_dtls c where  c.mst_id=a.id and c.id=b.dtls_id and b.entry_form in ( 2 ) and b.po_breakdown_id in ( $po_ids ) group by b.po_breakdown_id,b.entry_form order by b.po_breakdown_id";
	...
	else
	{
		...
		$grey_recv_arr=array_chunk(array_unique(explode(",",$po_ids)),999);
...
FROM inv_receive_master a,  order_wise_pro_details b, pro_grey_prod_entry_dtls c where c.mst_id=a.id and c.id=b.dtls_id and b.entry_form in ( 2 )";
		...
		$p=1;
		foreach($grey_recv_arr as $grey_recv_list)
		{
			if($p==1) $sql .=" and (b.po_breakdown_id in(".implode(',',$grey_recv_list).")"; else  $sql .=" or b.po_breakdown_id in(".implode(',',$grey_recv_list).")";
...
FROM  pro_dyeing_update_mst a,  order_wise_pro_details b, pro_dyeing_update_dtls c where  c.id=b.dtls_id and a.id=c.mst_id and b.entry_form in ( 6 ) and b.po_breakdown_id in ( $po_ids ) group by b.po_breakdown_id,b.entry_form order by b.po_breakdown_id";
	...
	else
	{
		...
		$pro_batch_arr=array_chunk(array_unique(explode(",",$po_ids)),999);
		
		$sql=" SELECT c.po_id,6 as entry_form,sum(batch_qnty) as batch_weight, "; 
		$sql .=" min(CASE WHEN a.load_unload_id =1 THEN a.process_end_date END) AS process_start_date,  max(CASE WHEN a.load_unload_id =2 THEN a.process_end_date END) AS process_end_date"; 
		$sql .=" from pro_fab_subprocess a, pro_batch_create_mst b, pro_batch_create_dtls c where a.batch_id=b.id and b.id=c.mst_id";
		
		$p=1;
		foreach($pro_batch_arr as $pro_batch_list)
		{
			if($p==1) $sql .=" and (c.po_id in(".implode(',',$pro_batch_list).")"; else  $sql .=" or c.po_id in(".implode(',',$pro_batch_list).")";
			
			$p++;
		}
		$sql .=")";
		
		$sql .=" group by c.po_id";
	}
	//echo $sql;die;
	$data_array=sql_select($sql);
	foreach($data_array as $row)
...
// Inspection Data for Update
	...
	else
	{
		...
		$inspection_arr=array_chunk(array_unique(explode(",",$job_no_list)),999);
		...
		$p=1;
		foreach($inspection_arr as $inspection_list)
		{
			if($p==1) $sql .=" and (job_no in(".implode(',',$inspection_list).")"; else  $sql .=" or job_no in(".implode(',',$inspection_list).")";
	...
	//print_r( $inspection_status_array); die;
...
// Ex-factory Data for Update 
	...
	else
	{
		...
		$ex_factory_arr=array_chunk(array_unique(explode(",",$po_ids)),999);
		...
		$p=1;
		foreach($ex_factory_arr as $ex_factory_list)
		{
			if($p==1) $sql .=" and (po_break_down_id in(".implode(',',$ex_factory_list).")"; else  $sql .=" or po_break_down_id in(".implode(',',$ex_factory_list).")";
	...
	//print_r( $exfactory_status_array); die;
	...
	{
	 	$export_doc_subm_arr=array_chunk(array_unique(explode(",",$po_ids)),999);
	...
	FROM  com_export_doc_submission_mst a,com_export_invoice_ship_dtls b, com_export_doc_submission_invo c WHERE a.id=c.doc_submission_mst_id and c.invoice_id=b.mst_id and b.current_invoice_qnty>0";
		
		$p=1;
		foreach($export_doc_subm_arr as $export_doc_subm_list)
		{
			if($p==1) $sql .=" and (b.po_breakdown_id in(".implode(',',$export_doc_subm_list).")"; else  $sql .=" or b.po_breakdown_id in(".implode(',',$export_doc_subm_list).")";
	...
	else
	{
		...
		$export_proceed_real_arr=array_chunk(array_unique(explode(",",$po_ids)),999);
		
		$sql = "SELECT b.po_breakdown_id,max(a.received_date) maxd,min(a.received_date) mind FROM com_export_proceed_realization a, com_export_invoice_ship_dtls b WHERE a.invoice_bill_id=b.mst_id and a.is_invoice_bill=2";
		
		$p=1;
		foreach($export_proceed_real_arr as $export_proceed_real_list)
		{
			if($p==1) $sql .=" and (b.po_breakdown_id in(".implode(',',$export_proceed_real_list).")"; else  $sql .=" or b.po_breakdown_id in(".implode(',',$export_proceed_real_list).")";
	...
	{
		$proceed_real_sub_arr=array_chunk(array_unique(explode(",",$po_ids)),999);
		
		$sql = "SELECT b.po_breakdown_id,max(a.received_date) maxd,min(a.received_date) mind FROM com_export_proceed_realization a, com_export_invoice_ship_dtls b,  com_export_doc_submission_invo c WHERE a.invoice_bill_id=c.doc_submission_mst_id and a.is_invoice_bill=1 and c.invoice_id=b.mst_id";
		
		$p=1;
		foreach($proceed_real_sub_arr as $proceed_real_sub_list)
		{
			if($p==1) $sql .=" and (b.po_breakdown_id in(".implode(',',$proceed_real_sub_list).")"; else  $sql .=" or b.po_breakdown_id in(".implode(',',$proceed_real_sub_list).")";
...
// Garments Production Data for Update  
	...
	else
	{
		...
		$garments_production_arr=array_chunk(array_unique(explode(",",$po_ids)),999);
		
		$sql = "SELECT po_break_down_id, min(production_date) as mind,max(production_date) as maxd, production_type,sum(production_quantity) as production_quantity FROM  pro_garments_production_mst WHERE ";
		
		$p=1;
		foreach($garments_production_arr as $garments_production_list)
		{
			if($p==1) $sql .=" (po_break_down_id in(".implode(',',$garments_production_list).")"; else  $sql .=" or po_break_down_id in(".implode(',',$garments_production_list).")";
			
			$p++;
		}
		$sql .=")";
		
		$sql .=" group by po_break_down_id,production_type";
	}
		...
		foreach( $tna_task_name_tmp[5] as $fid=>$value)
		{
			if( $tna_task_template_task[$po_order_template[$poid]][5][$value]!=""  )
			{
				...
				else // Update  submitted_to_buyer   approval_status_date
				{
					...
					{
						if ( $sample_approval_update[$poid][$tna_task_name[$value]['task_name']]['min_start_date']!="0000-00-00" || $sample_approval_update[$poid][$tna_task_name[$value]['task_name']]['min_start_date']!="" ) $start_date=$sample_approval_update[$poid][$tna_task_name[$value]['task_name']]['min_start_date']; else { if($db_type==0) $start_date="0000-00-00"; else $start_date=""; }
						
						if ( $sample_approval_update[$poid][$tna_task_name[$value]['task_name']]['max_start_date']!="0000-00-00" || $sample_approval_update[$poid][$tna_task_name[$value]['task_name']]['max_start_date']!="" ) $finish_date=$sample_approval_update[$poid][$tna_task_name[$value]['task_name']]['max_start_date']; else { if($db_type==0) $finish_date="0000-00-00"; else $finish_date=""; } 
		...
		foreach($tna_task_name_tmp[6] as $fid=>$value)
		{
			if( $tna_task_template_task[$po_order_template[$poid]][6][$value]!=""  )
			{
				...
				else // Update  submitted_to_buyer   approval_status_date
				{
					...
					if( $tna_task_name[$value]['task_type']==1 )  // submission
					{
						if ( $labdip_update_task[$poid]['min_start_date']!="0000-00-00" || $labdip_update_task[$poid]['min_start_date']!="" ) $start_date=$labdip_update_task[$poid]['min_start_date']; else { if($db_type==0) $start_date="0000-00-00"; else $start_date=""; }
						
						if ( $labdip_update_task[$poid]['max_start_date']!="0000-00-00" || $labdip_update_task[$poid]['max_start_date']!="" ) $finish_date=$labdip_update_task[$poid]['max_start_date']; else { if($db_type==0) $finish_date="0000-00-00"; else $finish_date=""; }
					...
					{
						if ( $labdip_update_task[$poid]['min_approval_status_date']!="0000-00-00" || $labdip_update_task[$poid]['min_approval_status_date']!="" ) $start_date=$labdip_update_task[$poid]['min_approval_status_date']; else { if($db_type==0) $start_date="0000-00-00"; else $start_date=""; }
						
						if ( $labdip_update_task[$poid]['max_approval_status_date']!="0000-00-00" || $labdip_update_task[$poid]['max_approval_status_date']!="" ) $finish_date=$labdip_update_task[$poid]['max_approval_status_date']; else { if($db_type==0) $finish_date="0000-00-00"; else $finish_date=""; } 
					...
					{
						if ( $labdip_update_task[$poid]['min_start_date']!="0000-00-00" || $labdip_update_task[$poid]['min_start_date']!="" ) $start_date=$labdip_update_task[$poid][$tna_task_name[$value]['task_name']]['min_start_date']; else { if($db_type==0) $start_date="0000-00-00"; else $start_date=""; }
		...
		foreach($tna_task_name_tmp[7] as $fid=>$value)
		{
			if( $tna_task_template_task[$po_order_template[$poid]][7][$value]!=""  )
			{
				...
				else // Update  min submitted_to_buyer ,max submitted_to_buyer,   same approval_status_date
				{
					...
					if( $tna_task_name[$value]['task_type']==1 )  // submission
					{
						if ( $trims_update_task[$poid][$tna_task_name[$value]['task_name']]['min_start_date']!="0000-00-00" || $trims_update_task[$poid][$tna_task_name[$value]['task_name']]['min_start_date']!="" ) $start_date=$trims_update_task[$poid][$tna_task_name[$value]['task_name']]['min_start_date']; else { if($db_type==0) $start_date="0000-00-00"; else $start_date=""; }
						
						if ( $trims_update_task[$poid][$tna_task_name[$value]['task_name']]['max_start_date']!="0000-00-00" || $trims_update_task[$poid][$tna_task_name[$value]['task_name']]['max_start_date']!="" ) $finish_date=$trims_update_task[$poid][$tna_task_name[$value]['task_name']]['max_start_date']; else { if($db_type==0) $finish_date="0000-00-00"; else $finish_date=""; }
					...
					else if( $tna_task_name[$value]['task_type']==2 )
					{
						...
						if ( $trims_update_task[$poid][$tna_task_name[$value]['task_name']]['min_approval_status_date']!="0000-00-00" || $trims_update_task[$poid][$tna_task_name[$value]['task_name']]['min_approval_status_date']!="" ) $start_date=$trims_update_task[$poid][$tna_task_name[$value]['task_name']]['min_approval_status_date']; else { if($db_type==0) $start_date="0000-00-00"; else $start_date=""; }
						
						if ( $trims_update_task[$poid][$tna_task_name[$value]['task_name']]['max_approval_status_date']!="0000-00-00" || $trims_update_task[$poid][$tna_task_name[$value]['task_name']]['max_approval_status_date']!="" ) $finish_date=$trims_update_task[$poid][$tna_task_name[$value]['task_name']]['max_approval_status_date']; else { if($db_type==0) $finish_date="0000-00-00"; else $finish_date=""; }
		...
		foreach($tna_task_name_tmp[8] as $fid=>$value)
		{
			if( $tna_task_template_task[$po_order_template[$poid]][8][$value]!=""  )
			{
				...
				else // Update  submitted_to_buyer   approval_status_date
				{
					...
					if( $tna_task_name[$value]['task_type']==1 )  // submission
					{
						if ( $embelishment_update_task[$poid][$tna_task_name[$value]['task_name']]['min_start_date']!="0000-00-00" || $embelishment_update_task[$poid][$tna_task_name[$value]['task_name']]['min_start_date']!="" ) $start_date=$embelishment_update_task[$poid][$tna_task_name[$value]['task_name']]['min_start_date']; else { if($db_type==0) $start_date="0000-00-00"; else $start_date=""; }
					...
					else if( $tna_task_name[$value]['task_type']==2 )
					{
						...
						if ( $embelishment_update_task[$poid][$tna_task_name[$value]['task_name']]['max_approval_status_date']!="0000-00-00" || $embelishment_update_task[$poid][$tna_task_name[$value]['task_name']]['max_approval_status_date']!="" ) $finish_date=$embelishment_update_task[$poid][$tna_task_name[$value]['task_name']]['max_approval_status_date']; else { if($db_type==0) $finish_date="0000-00-00"; else $finish_date=""; }
					...
					{
						if ( $embelishment_update_task[$poid][$tna_task_name[$value]['task_name']]['min_start_date']!="0000-00-00" || $embelishment_update_task[$poid][$tna_task_name[$value]['task_name']]['min_start_date']!="" ) $start_date=$embelishment_update_task[$poid][$tna_task_name[$value]['task_name']][$tna_task_name[$value]['task_name']]['min_start_date']; else { if($db_type==0) $start_date="0000-00-00"; else $start_date=""; }
		...
		foreach($tna_task_name_tmp[20] as $fid=>$value)
		{
			if( $tna_task_template_task[$po_order_template[$poid]][20][$value]!=""  )
			{
				...
				else // Update
				{
					...
					if ( $inventory_transaction_update[$poid][$inv_array_mapping[$tna_task_name[$value]['task_name']]]['start_date']!="0000-00-00" || $inventory_transaction_update[$poid][$inv_array_mapping[$tna_task_name[$value]['task_name']]]['start_date']!="" )
						$start_date=$inventory_transaction_update[$poid][$inv_array_mapping[$tna_task_name[$value]['task_name']]]['start_date']; 
					//else { if($db_type==0) $start_date="0000-00-00"; else $start_date=""; }
					
					if ( $inventory_transaction_update[$poid][$inv_array_mapping[$tna_task_name[$value]['task_name']]]['end_date']!="0000-00-00" || $inventory_transaction_update[$poid][$inv_array_mapping[$tna_task_name[$value]['task_name']]]['end_date']!="" ) $finish_date=$inventory_transaction_update[$poid][$inv_array_mapping[$tna_task_name[$value]['task_name']]]['end_date']; 
					//else { if($db_type==0) $finish_date="0000-00-00"; else $finish_date=""; }
		...
		foreach( $tna_task_name_tmp[26] as $fid=>$value )
		{
			if( $tna_task_template_task[$po_order_template[$poid]][26][$value]!=""  )
			{
				...
				else // Update
				{
					...
					if ( $gmts_prod_array[$poid][$gmts_prod_array_mapping[$tna_task_name[$value]['task_name']]]['min']!="0000-00-00" || $gmts_prod_array[$poid][$gmts_prod_array_mapping[$tna_task_name[$value]['task_name']]]['min']!="" ) $start_date=$gmts_prod_array[$poid][$gmts_prod_array_mapping[$tna_task_name[$value]['task_name']]]['min']; else { if($db_type==0) $start_date="0000-00-00"; else $start_date=""; }
					...
					{
						if( $tna_process_details[$process_id_up]['finish']!="0000-00-00" || $tna_process_details[$process_id_up]['finish']!="" ) $finish_date=$tna_process_details[$process_id_up]['finish'];
						else $finish_date=($gmts_prod_array[$poid][$gmts_prod_array_mapping[$tna_task_name[$value]['task_name']]]['max'] == "" ? $blank_date : $gmts_prod_array[$poid][$gmts_prod_array_mapping[$tna_task_name[$value]['task_name']]]['max']); 
					}
	...
	//echo $data_array_tna_process;die;
		...
		if( $data_array_tna_process!="" ) 
		{	
			$tna_pro_array=array_chunk(array_unique(explode(")",$data_array_tna_process)),2);
