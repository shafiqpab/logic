...
{ 
	...
	unset($job_no_array);
		...
		if($db_type==0)
		{
			$sql = "SELECT task_number,po_number_id,actual_start_date,actual_finish_date,task_start_date,task_finish_date FROM tna_plan_actual_history WHERE po_number_id in ( $po_ids ) and status_active =1 and is_deleted=0 ";
		}
		else
		{
			$job_no_list_arr=array_chunk(array_unique(explode(",",$po_ids)),999);
		
			$sql = "SELECT task_number,po_number_id,actual_start_date,actual_finish_date,task_start_date,task_finish_date FROM   tna_plan_actual_history WHERE  ";
			$p=1;
...
where a.id=b.mst_id and b.item_category=1 and  a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  ";
		...
		else
		{
			...
			$sql_task = "SELECT count(a.id) as cid,po_break_down_id,sample_type	 FROM wo_po_sample_approval_info  a, lib_sample b WHERE sample_type in  (2,3,4,7,8,9,10,11) and b.id=a.SAMPLE_TYPE_ID and approval_status_date is not null  and a.is_deleted = 0 and a.status_active=1 and b.is_deleted = 0 and b.status_active=1";
...
//-------------------------
	 ...
	 //print_r($template_wise_task[17]);die;
			...
			{  
				...
				if($to_process_task[$row[po_id]][$row_task[task_name]]!="")
				{
					...
					{ 
						...
						$insert_string[] ="('$mst_id','$row[template_id]','$row[job_no_mst]','$row[po_id]','$row[po_received_date]','$row[shipment_date]','1','$row_task[task_name]','$target_date','$start_date','$finish_date','$notice_date_start','$notice_date_end','$date','$row_task[sequence_no]',1,0)";
	  ...
	  //if($row_task[task_name]==52) { echo $tna_task_update_data[$row[po_id]][$row_task[task_name]]['noofapproved'].'!='.$tna_task_update_data[$row[po_id]][$row_task[task_name]]['noofval']; die; }
