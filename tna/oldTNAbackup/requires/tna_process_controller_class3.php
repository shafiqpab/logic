...
//****************************************************************************************		
	...
	// Sample Approval Update Data
		...
		foreach( $result as $row ) 
		{
			...
			else if ($row[csf("sample_type")]==12) { $sub=93;  $appr=94;  $sub=193;  $appr=194;}
...
where a.id=b.mst_id and b.item_category=1 and  a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  ";
	...
	// Garments Production Data for Update .................................................... 
		...
		{		 
			...
			$sql = "select a.po_break_down_id,min(a.production_date) as start_date,max(a.production_date) as end_date,a.production_type,sum(b.production_qnty) as production_qnty,a.embel_name,b.color_type_id from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and a.po_break_down_id in($po_ids) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
...
//new...........................................start;
		...
		{
			/*$sql_task = "SELECT b.po_break_down_id,  sum(wo_qnty) as tfb_qnty, sum(b.amount/b.exchange_rate)  as tfb_amount, sum(grey_fab_qnty) as gfb_qnty, min(a.booking_date) as start_date, max(a.booking_date) as end_date, a.item_category,a.booking_type
		FROM  wo_booking_mst a, wo_booking_dtls b WHERE b.po_break_down_id in ( $po_ids ) and a.booking_no=b.booking_no and a.is_short=2 and a.booking_type=7 and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 group by b.po_break_down_id,item_category,a.booking_type order by b.po_break_down_id asc";*/
		...
		{ 
			...
			//if($AOPandYDPOArr[$row[csf("po_breakdown_id")]]==''){
...
where c.company_name=3 and a.quotation_id=c.id and c.job_no=d.job_no_mst and a.id=b.sample_development_id and b.entry_form_id=132 and a.id=1201 and b.is_deleted=0 and b.status_active=1 group by d.id,b.ex_factory_qty order by d.id asc";*/
	...
	//print_r($AOPandYDPOArr);die;
	
	
	$sample_delivery_arr = sql_select($sql); 
	foreach($sample_delivery_arr as $row)
	{	
		//if($AOPandYDPOArr[$row[csf("po_breakdown_id")]]=='' && $row[csf("po_breakdown_id")]!=''){
...
//---------------------------------------------------------------------------------------------------------
	   ...
	   //print_r($template_wise_task[38]);die;
