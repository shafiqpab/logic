...
if ( $action=="tna_process" )
{
	...
	foreach( $company_array as $cbo_company )
	{
		...
		if($textile_tna_process_base==2){//Sales Order Base
			...
			$sql = "select a.id as booking_id,a.delivery_date,a.insert_date as booking_date,a.job_no as booking_no,a.buyer_id,b.color_type_id  ,sum(b.finish_qty) as fin_fab_qnty,sum(b.grey_qty) as grey_fab_qnty
			...
			group by a.id,a.insert_date,a.delivery_date,a.job_no,a.buyer_id,b.color_type_id ORDER BY a.delivery_date asc";
			    //echo $sql;die;
...
//Kniting Production...................................................................................start;	
		  /*$sql="select c.job_no booking_no,min(a.receive_date) min_date,max(a.receive_date) max_date,sum(b.qnty) qty from inv_receive_master a ,pro_roll_details b ,fabric_sales_order_mst c where a.id=b.mst_id and b.po_breakdown_id=c.id and a.receive_basis=2 and a.entry_form=2 and b.entry_form=2 and a.item_category=13  and a.status_active=1 and b.status_active=1 and c.status_active=1";*/
	...
	where a.id=b.mst_id and b.po_breakdown_id=c.id and a.receive_basis=2 and a.entry_form=2 and b.entry_form=2 and a.item_category=13  and a.status_active=1   and d.status_active=1 and b.status_active=1 and c.status_active=1 and a.knitting_company=$cbo_company
	  and c.id=d.mst_id"; 
		  ...
		  /*$sql="select a.booking_no,c.color_type_id,max(a.receive_date) max_date,min(a.receive_date) min_date,sum(b.grey_receive_qnty) qty  from inv_receive_master a, pro_grey_prod_entry_dtls b ,fabric_sales_order_dtls c
where a.id=b.mst_id and a.booking_id=c.mst_id and b.body_part_id=c.body_part_id and b.febric_description_id=c.determination_id and  a.entry_form=2 and a.item_category=13 and a.receive_basis=4 and a.knitting_company=$cbo_company and a.status_active=1 and b.status_active=1 and c.status_active=1
";
		  
			$p=1;
			foreach($job_no_list_arr as $job_no_process)
			{
				if($p==1){$sql .=" and (a.booking_id in(".implode(',',$job_no_process).")";}
				else{$sql .=" or a.booking_id in(".implode(',',$job_no_process).")";}
				$p++;
			}
			$sql .=")";
			$sql .=" group by  a.booking_no,c.color_type_id";
			  //echo $sql;die;
		$sql .="
		union all
select d.booking_no,d.color_type_id,max(a.receive_date) max_date,min(a.receive_date) min_date,sum(b.grey_receive_qnty) qty  from inv_receive_master a, pro_grey_prod_entry_dtls b ,ppl_planning_info_entry_dtls c,ppl_planning_info_entry_mst d
where a.id=b.mst_id and a.booking_id=c.id and c.mst_id=d.id  and  a.entry_form=2 and a.item_category=13 and a.receive_basis=2 and a.knitting_company=$cbo_company  and a.status_active=1 and b.status_active=1";
			$p=1;
			foreach($job_no_list_arr as $job_no_process)
			{
				if($p==1){$sql .=" and (a.booking_id in(".implode(',',$job_no_process).")";}
			...
			$sql .=")";
			$sql .=" group by d.booking_no,d.color_type_id";*/
...
//Batch Creation...................................................................................start;	
		...
		foreach($batchy_data_array as $row)
		{
			...
			$tna_task_update_data[$row[csf("sales_order_no")]][61]['reqqnty']+=$row[csf("qty")];
		}
...
/*select c.sales_order_no,max(a.process_end_date) max_date,min(a.process_end_date) min_date,sum(b.production_qty) as qty
 from pro_fab_subprocess a, pro_fab_subprocess_dtls b, pro_batch_create_mst c 
  ...
  and  c.status_active=1 and c.is_deleted=0 and c.sales_order_id=6072 and c.batch_no = '57754'  group by c.sales_order_no */
	...
	where  a.batch_no=b.batch_no and b.id=c.mst_id and b.load_unload_id = 2  and a.is_sales=1 and b.service_company=$cbo_company 
	";
...
from inv_receive_master a, pro_finish_fabric_rcv_dtls b,pro_batch_create_mst c  where a.id=b.mst_id  and a.item_category=2 and a.entry_form in(7,37) and b.batch_id=c.id and b.is_sales=1 and a.receive_basis in(5,10) and a.knitting_company=$cbo_company ";
			$p=1;
			foreach($job_no_list_arr as $job_no_process)
			{
		...
		foreach($fin_fab_data_array as $row)
		{
			if($row[csf("entry_form")]==7 and $row[csf("receive_basis")]==5){
				...
				$tna_task_update_data[$row[csf("job_no")]][239]['reqqnty']+=$row[csf("qty")];
			}
		}
...
//Yarn Issue................................................................start;	
	...
	where a.id=b.mst_id and b.job_no=c.job_no  and b.item_category=1  and a.company_id=$cbo_company";
			$p=1;
			foreach($job_no_list_arr as $job_no_process)
			{
				if($p==1){$sql .=" and (c.id in(".implode(',',$job_no_process).")";}
...
//Yarn receive................................................................start;	
	...
	where a.id=b.mst_id and b.job_no=c.job_no  and a.item_category=1 and b.item_category=1  and a.company_id=$cbo_company";
			$p=1;
			foreach($job_no_list_arr as $job_no_process)
			{
				if($p==1){$sql .=" and (c.id in(".implode(',',$job_no_process).")";}
...
$sql = "select  b.job_no, sum(b.req_quantity) req_qty, sum(b.supplier_order_quantity) qty ,max(a.wo_date) max_date,min(a.wo_date) min_date
...
//Yarn Rec................................................................start;	
	...
	where b.pi_wo_batch_no=d.pi_id and d.work_order_no=e.wo_number and e.id=f.mst_id and e.entry_form=144 $job_con and b.item_category=1 and b.transaction_type=1 and b.receive_basis=1 and e.pay_mode=2
	group by f.job_no
	union all
	...
	where a.id=b.mst_id and a.booking_id=e.id and e.id=f.mst_id and e.entry_form=144 $job_con and b.item_category=1 and b.transaction_type=1 and a.receive_basis=2 and a.entry_form=1 and e.pay_mode <>2
	group by f.job_no";
		...
		foreach($yarn_rec_data_array as $row)
		{
			...
			$tna_task_update_data[$row[csf("job_no")]][47]['reqqnty']=$row[csf("req_qty")];
		}
...
$job_con .=")"; 
	...
	$ffdtgSql="select b.fso_id,max(TO_CHAR(b.insert_date ,'DD-MM-YYYY')) max_date,min(TO_CHAR(b.insert_date ,'DD-MM-YYYY')) min_date , sum(b.delivery_qnty) qty from pro_fin_deli_multy_challan_mst a,pro_fin_deli_multy_challa_dtls b where a.id=b.mst_id and a.entry_form=231 and b.entry_form=231 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $job_con 
	 group by b.fso_id";		
...
//print_r($tna_task_update_data);die;
	...
	if($db_type==1 || $db_type==2 )
	{
		...
		if(count($process_id_up_array>0)) 
		{
			...
			for ($i=0;$i<$count;$i++)
		 	{
				//echo bulk_update_sql_statement( "tna_process_mst", "id", $field_array_tna_process_up, $data_array_tna_up[$i], array_values($id_up_array[$i] ));echo '####';
        	...
        	<td align="center" width="100%">
            	<table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <thead>                	 
                        ...
                        <th width="120" ><input type="checkbox" value="0" onClick="set_checkvalue()" id="chk_none_order">None Order</th>           
                    </thead>
        			<tr>
                    	<td> 
...
if ($action=="ponumber_search_list_view")
{
 ...
 if($buyer==0 && $booking=='' && str_replace("'","",$start_date)==''){
	 echo "<h1>Please Select Booking Date</h1>";exit();
 }
