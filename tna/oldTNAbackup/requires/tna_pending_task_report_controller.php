...
/*	if($db_type==0){
		$str_cond	=" and insert_date between '".$prev_date."' and '".$current_date."'";
	}
	else
	{
		$str_cond	=" and insert_date between '".$prev_date."' and '".$current_date." 11:59:59 PM'";
	}
*/
 ...
 // var_dump($dealing_marchant_data_arr); 
		...
		$sql ="select job_id,order_id,task_id,comments";
		...
		{
			$commentsArr[$rows[csf('job_id')]][$rows[csf('task_id')]]=$rows[csf('comments')];
...
<table width="1020">
    <tr>
        <th><strong>Completion Pending Of ( Date :<? echo change_date_format($prev_date);?> )</strong></th>
    </tr>
    ...
    <tr>
    	<td>
    		...
    		<table width="100%" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all">
                <thead>
                <tr>
                    <td colspan="9"><strong>Dealing Merchant : <? echo $dealing_marchant[$dealing_marchant_id]; ?></strong>, <strong> Mobile :<? echo $marchant_contact_no[$dealing_marchant_id]; ?></strong></td>
                </tr>
                <tr>
        		...
        		<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $dealing_marchant_id.$i;?>','<? echo $bgcolor;?>')" id="tr_<? echo $dealing_marchant_id.$i; ?>">
                    ...
                    <td><? echo $commentsArr[$row_dtls[csf('job_no')]][$row_dtls[csf('task_number')]]; ?></td>
                </tr>
				<? 
				}
