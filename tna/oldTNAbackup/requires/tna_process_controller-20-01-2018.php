...
{ 
	...
	foreach( $company_array as $cbo_company )
	{
	  ...
	  //print_r($tna_task_details);die;	
		...
		//$conversiondata=$conversion->getQtyArray_by_orderAndProcess();
...
//****************************************xxxxxxxxxxxxxxxxxxxxx*****************************************
		...
		foreach( $result as $row ) 
		{
			...
			if( $row[csf("requirment")]>0)
			{
				...
				//167	=> "Yarndip Approval",
					...
					$tna_task_update_data[$row[csf("po_break_down_id")]][60]['reqqnty']=0;
					$to_process_task[$row[csf("po_break_down_id")]][60]=0;
					
					//73	=> "Finished fabric to be in-house",
					$tna_task_update_data[$row[csf("po_break_down_id")]][73]['reqqnty']=0;
					$to_process_task[$row[csf("po_break_down_id")]][73]=0;
					
					//85 => "Print/Emb To Be Done",
					$tna_task_update_data[$row[csf("po_break_down_id")]][85]['reqqnty']=0;
					$to_process_task[$row[csf("po_break_down_id")]][85]=0;
					//86	=> "Sewing To Be Done",	
					$tna_task_update_data[$row[csf("po_break_down_id")]][86]['reqqnty']=0;
					$to_process_task[$row[csf("po_break_down_id")]][86]=0;
				...
				elseif(($row[csf("color_type_id")]==1) || ($row[csf("color_type_id")]==20) || ($row[csf("color_type_id")]==25) || ($row[csf("color_type_id")]==26) || ($row[csf("color_type_id")]==27) || ($row[csf("color_type_id")]==28) || ($row[csf("color_type_id")]==29) || ($row[csf("color_type_id")]==30) || ($row[csf("color_type_id")]==31))
				{
		...
		if($db_type==0)
		{
			$sql="select po_break_down_id,count(sample_type_id) as tot_sample_type_id from wo_po_sample_approval_info WHERE sample_type_id=45 and status_active =1 and is_deleted=0 and po_break_down_id in($po_ids) group by po_break_down_id";
		}
		else
		{
			$po_id_list_arr=array_chunk(array_unique(explode(",",$po_ids)),999);
		
			$sql = "select po_break_down_id,count(sample_type_id) as tot_sample_type_id from wo_po_sample_approval_info WHERE sample_type_id=45 and status_active =1 and is_deleted=0  ";
			$p=1;
			foreach($po_id_list_arr as $po_id_process)
			{
		...
		foreach( $result as $row ) 
		{
			...
			if($AOPandYDPOArr[$row[csf("po_break_down_id")]]==''){
			//159 => "Size Set Sample Making",
				$tna_task_update_data[$row[csf("po_break_down_id")]][159]['reqqnty']=$row[csf("tot_sample_type_id")];
				$to_process_task[$row[csf("po_break_down_id")]][159]=159;
			}
			if($AOPPoArr[$row[csf("po_break_down_id")]]!=''){
			...
			if($YDPoArr[$row[csf("po_break_down_id")]]!=''){
...
//****************************************************************************************		
	...
	// Sample Approval Update Data
		...
		$sql_task = "SELECT count(a.id) as cid,po_break_down_id,sample_type,max(submitted_to_buyer) as max_submitted_to_buyer,min(submitted_to_buyer) as min_submitted_to_buyer,max(approval_status_date) as max_approval_status_date,min(approval_status_date) as min_approval_status_date
		 FROM wo_po_sample_approval_info  a, lib_sample b WHERE sample_type in (2,3,4,7,8,9,10,11,9,5,12) and b.id=a.SAMPLE_TYPE_ID and a.is_deleted = 0 and a.status_active=1 and b.is_deleted = 0 and b.status_active=1";
		...
		foreach( $result as $row ) 
		{
			...
			else if ($row[csf("sample_type")]==12) { $sub=93;  $appr=94; }
		...
		if($db_type==0)
		{
			$sql_task = "SELECT a.id as cid,po_break_down_id,sample_type,submitted_to_buyer,approval_status_date	 FROM wo_po_sample_approval_info  a, lib_sample b WHERE job_no_mst in ( $job_no_list ) and sample_type in  (2,3,4,7,8,9,10,11,12) and b.id=a.SAMPLE_TYPE_ID  and a.is_deleted = 0 and a.status_active=1  and b.is_deleted = 0 and b.status_active=1";
		}
		else
		{
			$job_no_list_arr=array_chunk(array_unique(explode(",",$job_no_list)),999);
			$sql_task = "SELECT a.id as cid,po_break_down_id,sample_type,submitted_to_buyer,approval_status_date FROM wo_po_sample_approval_info  a, lib_sample b WHERE sample_type in  (2,3,4,7,8,9,10,11,12) and b.id=a.SAMPLE_TYPE_ID  and a.is_deleted = 0 and a.status_active=1 and b.is_deleted = 0 and b.status_active=1";
...
//-------------------------
	 ...
	 //print_r($template_wise_task[17]);die;
		...
		$field_array_tna_process="id,template_id,job_no,po_number_id,po_receive_date,shipment_date,task_category,task_number,target_date,task_start_date,task_finish_date,notice_date_start,notice_date_end,process_date,sequence_no,plan_start_flag,plan_finish_flag,status_active,is_deleted";
			...
			{  
				...
				if($to_process_task[$row[po_id]][$row_task[task_name]]!="")
				{
					...
					{ 
						...
						$data_array_tna_process .="('$mst_id','$row[template_id]','$row[job_no_mst]','$row[po_id]','$row[po_received_date]','$row[shipment_date]','1','$row_task[task_name]','$target_date','$start_date','$finish_date','$notice_date_start','$notice_date_end','$date','$row_task[sequence_no]',$plan_start_flag,$plan_finish_flag,1,0)";
						
						$insert_string[] ="('$mst_id','$row[template_id]','$row[job_no_mst]','$row[po_id]','$row[po_received_date]','$row[shipment_date]','1','$row_task[task_name]','$target_date','$start_date','$finish_date','$notice_date_start','$notice_date_end','$date','$row_task[sequence_no]',$plan_start_flag,$plan_finish_flag,1,0)";
