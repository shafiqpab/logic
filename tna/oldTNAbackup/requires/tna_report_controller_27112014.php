        		...
        		<tr bgcolor="<? echo $bgcolor; ?>" style="vertical-align:middle" height="25" onClick="change_color('tr_<? echo $h;?>','<? echo $bgcolor;?>')" id="tr_<? echo $h; ?>">
                    ...
                    <td width="120"  rowspan="3" title="<? echo $row[csf('job_no')]; ?>"><p><? echo $wo_po_details_master[$row[csf('job_no')]][csf('style_ref_no')]; ?></p></td>
                     ...
                     <? 
						...
						{
							$lead_timee=$lead_time[$row[csf('template_id')]];
						}
						else
						{
                    ...
                    <td width="100" rowspan="3" title="<? echo "Lead Time: ".$lead_timee."; "." PO. Rec. Date: ".change_date_format($row[csf('po_receive_date')]); ?>"><? echo change_date_format($row[csf('shipment_date')]);  ?></td>
                    <td width="90">Plan</td>
                <?
...
function fnc_tna_actual_date_update_reponse()
{
    ...
    <table width="600" cellspacing="0" cellpadding="0" class="rpt_table">
        ...
        <tr>
            ...
            <td>
            	<input type="text" <? if($type==2) echo "disabled='disabled'";  ?> name="txt_plan_start_date" id="txt_plan_start_date" class="datepicker" style="width:100px" value="<? echo change_date_format($tna_result[0][csf('task_start_date')]); ?>" />
            </td>
            
            <td align="right">Plan Finish Date</td>
            <td>
            	<input type="text" <? if($type==2) echo "disabled='disabled'";  ?> name="txt_plan_finish_date" id="txt_plan_finish_date" class="datepicker" style="width:100px"  value="<? echo change_date_format($tna_result[0][csf('task_finish_date')]) ?>"/>
            </td>
        </tr>
         ...
         <tr>
            ...
            <td>
            	<input type="text" <? if($type==1) echo "disabled='disabled'";  ?> name="txt_actual_start_date" id="txt_actual_start_date" class="datepicker" style="width:100px" value="<? echo change_date_format($tna_result[0][csf('actual_start_date')]) ?>" />
            </td>
            <td align="right">Actual Finish Date</td>
            <td>
            	<input type="text" <? if($type==1) echo "disabled='disabled'";  ?> name="txt_actual_finish_date" id="txt_actual_finish_date" class="datepicker" style="width:100px" value="<? echo change_date_format($tna_result[0][csf('actual_finish_date')]); ?>" />
            </td>
        </tr>
...
<body onLoad="set_hotkey()">
    ...
    <table style="width: 1030px;">
        <tr>
            <td>
                <div style="width: 1020px;font-size:12px;">
                <table width="1000" border="1" rules="all" class="rpt_table">
                    <thead>
                    	<tr align="center">
    ...
    <table style="width:1030px;">
        <tr>
            <td>    
                <div style="width: 1020px;overflow-y: scroll; max-height:180px;font-size:12px;" id="scroll_body2">
                <table width="1000px" border="1" rules="all" class="rpt_table" id="comments_tbl">
                	<tbody>
						<?php
                        $i=0;
                        ...
                        <tr bgcolor="<? echo $trcolor; ?>">
                            ...
                            <td><Input name="txtcomments[]" class="text_boxes" ID="txtcomments_<?php echo $i; ?>" value="<?php  echo $comments_array[$key]; ?>" onDblClick="openmypage(<?php echo $i; ?>); return false" style="width:147px;" autocomplete="off" readonly /></td>
                        </tr>
                        <?
                        }
                        ?>
