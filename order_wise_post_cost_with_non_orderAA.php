...
// Fabric Purch ============================
		...
		foreach($fabPurArr_non_acl as $booking_no=>$fabPur_book ){
			foreach($fabPur_book as $cat=>$fabPur_acl ){
				...
				if($cat==2){
						...
						$fabPurArr['acl']['knit']['non_qty']+=($fabPur_acl['qty']*$exfactoryQtyArr[$po_id])/$po_information_arr[$po_id]['order_quantity'];
						if($fabPur_acl['qty']>0){
							$fabPurArr['acl']['knit']['non_amount']+=($fabPur_acl['amount']*$exfactoryQtyArr[$po_id])/$po_information_arr[$po_id]['order_quantity'];
						 
					}
				}
				if($cat==3){
					 
					$fabPurArr['acl']['woven']['non_qty']+=($fabPur_acl['qty']*$exfactoryQtyArr[$po_id])/$po_information_arr[$po_id]['order_quantity'];
					if($fabPur_acl['qty']>0){
						$fabPurArr['acl']['woven']['non_amount']+=($fabPur_acl['amount']*$exfactoryQtyArr[$po_id])/$po_information_arr[$po_id]['order_quantity'];
					      }
				}
			} //Cat
		}
	...
	// Fabric Purch End ============================
		...
		// Knitting Bill non
		$sql_outb_non = "select  b.receive_qty,b.order_id,b.rate,b.amount,c.product_name_details as descrip from subcon_outbound_bill_mst a, subcon_outbound_bill_dtls b, product_details_master c where a.id=b.mst_id and b.item_id=c.id and a.process_id=2 and a.bill_Date between '".$form_date."' and '".$to_date."' and a.bill_for=3 and  a.is_deleted=0 and a.status_active=1 and  b.is_deleted=0 and b.status_active=1";
	...
	// Fabric Dye Finish  ==============================
		...
		foreach($data_finish as $row_finish){
			...
			if($cancelled_po==1)
			{
			$finishData_temp[$row_finish[csf('job_no')]][$index]['qty']=array_sum($finishQtyArr[$row_finish[csf('job_no')]][$row_finish[csf('id')]][$row_finish[csf('cons_process')]]);
			$finishData_temp[$row_finish[csf('job_no')]][$index]['amount']=array_sum($finishAmtArr[$row_finish[csf('job_no')]][$row_finish[csf('id')]][$row_finish[csf('cons_process')]]);
			}
			else //Cancel PO
			{
			 $finishData_temp_can[$row_finish[csf('job_no')]][$index]['qty']=array_sum($finishQtyArr[$row_finish[csf('job_no')]][$row_finish[csf('id')]][$row_finish[csf('cons_process')]]);
			$finishData_temp_can[$row_finish[csf('job_no')]][$index]['amount']=array_sum($finishAmtArr[$row_finish[csf('job_no')]][$row_finish[csf('id')]][$row_finish[csf('cons_process')]]);
			}
		}
		unset($data_finish);
		...
		// Non Bill Dying Finishing
			$sql_non_dying = "select  sum(b.receive_qty) as receive_qty,b.order_id,sum(b.amount) as amount from subcon_outbound_bill_mst a, subcon_outbound_bill_dtls b, product_details_master c where a.id=b.mst_id and b.item_id=c.id and a.process_id=4  and a.bill_Date between '".$form_date."' and '".$to_date."' and  a.is_deleted=0 and a.status_active=1 and a.bill_for=3 and  b.is_deleted=0 and b.status_active=1 group by b.order_id";
		//echo $sql;die;
	...
	// Fabric AOP  ==============================
		...
		$sql = "select a.id, a.job_no,a.item_number_id, a.body_part_id, a.fab_nature_id, a.color_type_id, a.fabric_description, a.gsm_weight,a.avg_cons, a.fabric_source, a.rate, a.amount,a.avg_finish_cons,a.status_active,b.cons_process from wo_pre_cost_fabric_cost_dtls a, wo_pre_cost_fab_conv_cost_dtls b,wo_po_break_down d where a.job_id=b.job_id and a.job_id=d.job_id and a.id=b.fabric_description and  d.id in(".implode(",",$exfactory_po_arr).")  and b.cons_process=35";
		...
		foreach($data_aop as $row_aop){
			...
			$finishData_temp[$row_aop[csf('job_no')]][$index]['qty']+=array_sum($aopQtyArr[$row_aop[csf('job_no')]][$row_aop[csf('id')]][$row_aop[csf('cons_process')]]);
			$finishData_temp[$row_aop[csf('job_no')]][$index]['amount']+=array_sum($aopAmtArr[$row_aop[csf('job_no')]][$row_aop[csf('id')]][$row_aop[csf('cons_process')]]);
		}
...
unset($finishData_temp);
	...
	// Wash Cost ============================
		...
		$sql = "selectb.id as po_id, a.id, a.job_no, a.emb_name,a.emb_type, a.cons_dzn_gmts,a.rate, a.amount,a.status_active from wo_pre_cost_embe_cost_dtls a,wo_po_break_down b  where a.job_id=b.job_id and  b.id in(".implode(",",$exfactory_po_arr).")  and emb_name =3 and status_active=1";
		$data_array=sql_select($sql);
		foreach($data_array as $row){
			$cancelled_po=$po_information_arr[$row[csf('po_id')]]['cancelled_status'];
			if($cancelled_po==1)
