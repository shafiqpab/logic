...
if($action=="get_system_approved")
{
	$data_arr=explode("*",$data);
	// echo $data_arr;die;
	$sql_data=sql_select("SELECT is_approved from wo_booking_mst where status_active=1 and is_deleted=0 and booking_no='$data_arr[1]'");
	$is_approved=0;
...
if($action=="terms_condition_popup")
{
	...
	extract($_REQUEST);
	// echo $sys_id.'='.$page_id.'='.$fso_id.'='.$buyer_id;
	if($sys_id!='')
	{
	...
	{		
			var approved=0;
			var page_id=$('#txt_entry_form').val();
			var sys_id=$('#txt_booking_no').val();
			if(page_id==87){
				var approved = return_ajax_request_value(page_id+'*'+sys_id, 'get_system_approved', 'terms_condition_controller');
				if(approved==1){
					alert('This System Is Approved So Update/Delete Not Possible');
		    		return;
				}
			}
		    var row_num=$('#tbl_termcondi_details tr').length-1;
			...
			for (var i=1; i<=row_num; i++)
			{
				...
				data_all=data_all+get_submitted_data_string('txt_booking_no*txt_entry_form*termscondition_'+i+'*termsprefix_'+i,"../../",i);
			}
		...
		emailwindow.onclose=function()
		{
			...
			if (theemail.value!="")
			{
				...
				for(var i=0;i<data.length;i++)
				{
					...
					+ '<td>'+counter+'</td><td><input type="text" name="termsprefix_'+counter+'" class="text_boxes" id="termsprefix_'+counter+'"  style="width:95%;" value="'+terms_prefix[i]+'"/> </td> <td><input type="text" name="termscondition_'+counter+'" class="text_boxes" id="termscondition_'+counter+'"  style="width:95%;" value="'+data[i]+'"/></td><td><input type="button" class="formbutton" id="increase_'+counter+'"  style="width:30px;" value="+" onClick="add_break_down_tr('+counter+')"/><input type="button" class="formbutton" id="decrease_'+counter+'"  style="width:30px;" value="-" onClick="javascript:fn_deletebreak_down_tr('+counter+')"/></td>'+ '</tr>');
				}
			}
		}
	}
	...
	<div align="center" style="width:100%;" >
		...
		<fieldset>
	        <form id="termscondi_1" autocomplete="off">
			   ...
			   <input type="hidden" id="cbo_buyer_name" name="cbo_buyer_name" value="<? echo str_replace("'","",$buyer_id) ?>"/>
	        	<table width="750" cellspacing="0" class="rpt_table" border="0" id="tbl_termcondi_details" rules="all">
	            	<thead>
	                	<tr>
	                    	<th width="50">Sl</th>
	                    	<th width="100">Terms Prefix</th>
	                ...
	                <tbody>
					 ...
					 //echo $page_id;
						...
						$booking_is_app=0;$disable_check="";$is_approved=0;
						if($page_id==87) //Only For Multiple Trim Booking V2-Knit as per JK /RS
						{
							$data_array_is_app=sql_select("select is_approved,ready_to_approved from  wo_booking_mst where booking_no='".str_replace("'","",$sys_id)."' and entry_form = $page_id");
							//echo "select   is_approved,ready_to_approved from  wo_booking_mst where booking_no='".str_replace("'","",$sys_id)."' and entry_form = $page_id";
							foreach( $data_array_is_app as $row )
							{
								$booking_is_app=$row[csf('ready_to_approved')];
								$is_approved=$row[csf('is_approved')];
							}
							if($booking_is_app==1) 
							{ 
							//$readonly_check="readonly";
						...
						//echo $readonly_check.'d';;
						$data_array=sql_select("select id, terms,terms_prefix from  wo_booking_terms_condition where booking_no='".str_replace("'","",$sys_id)."' and entry_form = $page_id order by id");
							...
							foreach( $data_array as $row )
							{
								...
								?>
	                            	<tr id="settr_1" align="center">
	                                    ...
	                                    <td>
                                         <input type="text" id="termsprefix_<? echo $i;?>"   name="termsprefix_<? echo $i;?>" style="width:95%"  class="text_boxes"  value="<? echo $row[csf('terms_prefix')]; ?>" <? echo $readonly_check;?>   /></td>
                                        <td>
						...
						else
						{
							...
							$data_array=sql_select("select id, terms,terms_prefix,tag_buyer from  lib_terms_condition where is_default=1 and page_id=$page_id order by id asc");
							//echo $data_array;die;
						   // print_r( $data_array);die;
							...
							foreach( $data_array as $row )
							{
								if($buyer_id != 'undefined'){
									$tag_buyer_arr=explode(",", $row['TAG_BUYER']);
									if(!in_array($buyer_id, $tag_buyer_arr)){
										continue;
									}
								}
								// print_r($tag_buyer_arr);
								$i++;
								?>
	                    		<tr id="settr_1" align="center">
	                                <td><?= $i;?> </td>

	                                <td><input type="text" id="termsprefix_<?= $i;?>" name="termsprefix_<?= $i;?>" style="width:95%"  class="text_boxes"  value="<?= $row['TERMS_PREFIX']; ?>" <?= $readonly_check;?>/></td>

                                    <td><input type="text" id="termscondition_<?= $i;?>" name="termscondition_<?= $i;?>" style="width:95%"  class="text_boxes"  value="<?= $row['TERMS']; ?>" <?= $readonly_check;?>/></td>

	                                <td><input type="button" id="increase_<?= $i; ?>" style="width:30px" class="formbutton" value="+" onClick="add_break_down_tr(<?= $i; ?> );" <?= $disable_check; ?>  <?= $readonly_check; ?> />

	                                <input type="button" id="decrease_<?= $i; ?>" style="width:30px" class="formbutton" value="-" onClick="javascript:fn_deletebreak_down_tr(<?= $i; ?> );" <?= $disable_check;?>/></td>
	                            </tr>
	                    		<? 
							}
						} 
	            ...
	            <table width="650" cellspacing="0" class="" border="0">
	            	<tr>
                        ...
                        <?
						//echo $page_id.'='.$booking_is_app;
						
                        if($page_id!=87 || $booking_is_app!=1) 
						{ 
						?>
						...
						<td align="center" height="15" width="50%">  
                           ...
                           <input type="hidden" id="is_approve_id" name="is_approve_id" value="<? echo $booking_is_app; ?>"/>
				        </td>
	                </tr>
	            </table>
	            <table width="650" cellspacing="0" class="" border="0">
    ...
    <script> 
    var is_app=$('#is_approve_id').val();
	//alert(is_app); For JK issue Id=24810
	if(is_app==1)
	{
		...
		$('#Refresh1').removeClass('formbutton').addClass('formbutton_disabled');
	}
    </script>
	</html>
	<?
...
if($action=="extra_terms_popup")
{
	...
	var prefix_selected=new Array();
	function js_set_value(counter,id,terms,prefix)
	{
		...
		else
		{
			...
			prefix_selected.splice( i,1 );
		}
		
		var ids = '';
		var termCon='';var termPrefix='';
		for( var i = 0; i < selected_id.length; i++ ) {
			ids += selected_id[i] + ',';
			termCon+=selected_item[i]+ ',';
			termPrefix+=prefix_selected[i]+ ',';
		}
		ids = ids.substr( 0, ids.length - 1 );
		termCon = termCon.substr( 0, termCon.length - 1 );
		termPrefix = termPrefix.substr( 0, termPrefix.length - 1 );
		$('#terms_breck_down').val( JSON.stringify(selected_item) );
		$('#txt_pre_cost_dtls_id').val( ids );
		$('#terms_prefix').val( termPrefix );
	}
    </script>
	...
	<fieldset>
        ...
        <input style="width:60px;" type="text" class="text_boxes"  name="terms_prefix" id="terms_prefix" /> 
	    ...
	    <tbody>
		<?  $i=1;
			$data_array=sql_select("select id, terms,terms_prefix from  lib_terms_condition where is_default=0 and page_id=$page_id");
			foreach( $data_array as $row )
			{
				 if ($i%2==0)  $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
			
				?>
		       <tr style="text-decoration:none; cursor:pointer" bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i;?>" onClick="js_set_value(<? echo $i ;?>,'<? echo $row[csf('id')];?>','<? echo $row[csf('terms')];?>','<? echo $row[csf('terms_prefix')];?>')">
		        <td width="40">
		       <? echo $i;?>
		        </td>
...
if($action=="save_update_delete")
{
	...
	if ($operation==0 || $operation==1)  // Insert/Update Here
	{
		...
		if  ( check_table_status( $_SESSION['menu_id'], 1 )==0 ) { echo "15**0"; die;}		
		 ...
		 $field_array="id,booking_no,entry_form,terms,terms_prefix";
		 for ($i=1;$i<=$total_row;$i++)
		 {
			 $termscondition="termscondition_".$i;
			 $termsprefix="termsprefix_".$i;
			if ($i!=1) $data_array .=",";
			$termscondition=str_replace("'","",$$termscondition);
			
			$data_array .="(".$id.",".$txt_booking_no.",".$txt_entry_form.",'".$termscondition."',".$$termsprefix.")";
			$id=$id+1;
		 }
		 //echo  $data_array;
