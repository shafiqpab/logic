<?
/*********************Comments******************
* Purpose			: 	This form will create Wash Dyes And Chemical Issue Requisition  
* Functionality	:                                                                           
* JS Functions	:                                                                           
* Created by		:	Kausar                                                               	
...
echo load_html_head_contents("Wash Dyes And Chemical Issue Requisition ","../../", 1, 1, $unicode,1,1);
$today=date('d-m-Y'); 
//--------------------------------------------------------------------------------------------------------------------
...
<script>
	...
	function openmypage_recipe()
	{
		...
		{ 	
			...
			var page_link = 'requires/wash_chemical_dyes_issue_req_controller.php?cbo_company_id='+cbo_company_id+'&recipe_id='+txt_recipe_id+'&action=recipe_popup';
			...
			emailwindow.onclose=function()
			{
				...
				if(erecipedata[0]!="")
				{
					...
					$('#txtBatchNo').val(erecipedata[8]);
					$('#txtMachineNo').val(erecipedata[9]);
					$('#txtBatchQty').val(erecipedata[10]);
					$('#hidden_operation_id').val(erecipedata[11]);
					$('#txtoperation').val(erecipedata[12]);
					$('#cbo_store_name').val(erecipedata[13]);
					$('#cbo_store_name').attr("disabled",true);
					
					show_list_view(cbo_company_id+"**"+erecipedata[0]+"**"+0, 'item_details', 'list_container_recipe_items', 'requires/wash_chemical_dyes_issue_req_controller', '');
	...
	function apply_last_update()
	{
		...
		{
			freeze_window(5);
			get_php_form_data(recipe_id, "populate_data_from_recipe_popup", "requires/wash_chemical_dyes_issue_req_controller" );
			var subprocess_id=return_global_ajax_value(recipe_id, 'get_subprocess_id', '', 'requires/wash_chemical_dyes_issue_req_controller');
			show_list_view(cbo_company_id+'**'+subprocess_id+"**"+recipe_id,'item_details','list_container_recipe_items','requires/wash_chemical_dyes_issue_req_controller','../');
			$('#is_apply_last_update').val(1);
			release_freezing();
		} 
	}
	...
	function fnc_chemical_dyes_issue_requisition(operation)
	{
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+$('#txt_recipe_id').val()+'*'+report_title, "chemical_dyes_issue_requisition_print", "requires/wash_chemical_dyes_issue_req_controller" );
			return;
		}
		if(operation==5)
		{
			...
			var report_title=$( "div.form_caption" ).html();
			print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+$('#txt_recipe_id').val()+'*'+report_title, "chemical_dyes_issue_requisition_print2", "requires/wash_chemical_dyes_issue_req_controller" );
			return;
		}
		...
		*/
			if( form_validation('cbo_company_name*cbo_location_name*txt_requisition_date*txt_recipe_no','Company Name*Location*Requisition Date*Recipe No')==false )
			{
				return;
			}
			var mst_data=get_submitted_data_string('txt_req_no*update_id*cbo_company_name*cbo_location_name*cbo_floor_name*txt_requisition_date*cbo_receive_basis*txt_recipe_id*txt_order_id*txt_job_no*is_apply_last_update*txtbuyerPoId*hidden_operation_id*txtoperation*txtRemarks*cbo_store_name',"../../");
			...
			http.open("POST","requires/wash_chemical_dyes_issue_req_controller.php",true);
	...
	{	
		...
		{   
			...
			if((reponse[0]==0 || reponse[0]==1))
			{
				...
				show_list_view(cbo_company_id+"**"+recipe_id+"**"+reponse[2], 'item_details', 'list_container_recipe_items', 'requires/wash_chemical_dyes_issue_req_controller', '');
	...
	function open_requisitionpopup()
	{
		...
		var page_link='requires/wash_chemical_dyes_issue_req_controller.php?action=requisition_popup&company='+company; 
		...
		emailwindow.onclose=function()
		{
			...
			if(reqnId!="")
			{
				...
				get_php_form_data(company+"**"+recipe_id+"**"+reqnId, "populate_data_from_data", "requires/wash_chemical_dyes_issue_req_controller");
				show_list_view(company+"**"+recipe_id+"**"+reqnId, 'item_details', 'list_container_recipe_items', 'requires/wash_chemical_dyes_issue_req_controller', '');
				//fnc_req_calculate(2,0);
			}
		}
	}
            ...
            <legend>Dyes And Chemical Issue Requisition</legend>
                <table width="810" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                    ...
                    <tr>
                        ...
                        <td width="155"><? echo create_drop_down( "cbo_company_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-Select Company-", $selected, "rcv_basis_reset(); load_drop_down( 'requires/wash_chemical_dyes_issue_req_controller', this.value, 'load_drop_down_location', 'location_td');get_php_form_data( this.value, 'company_wise_report_button_setting','requires/wash_chemical_dyes_issue_req_controller');",""); ?>
                        </td>
                    ...
                    <tr>
                        ...
                        <td ><input type="text" name="txt_requisition_date" id="txt_requisition_date" class="datepicker" style="width:150px;" value="<? echo date("d-m-Y");?>" placeholder="Select Date" /></td>
                    ...
                    <tr>
                    	<td>Batch No</td>
                        <td>
                            <input class="text_boxes" type="text" name="txtBatchNo" id="txtBatchNo"readonly placeholder="Display" style="width:150px;" /> 
                        </td>
                    	<td>Machine No</td>
                        <td>
                            <input class="text_boxes" type="text" name="txtMachineNo" id="txtMachineNo" readonly placeholder="Display" style="width:140px;" /> 
                        </td>
                    	<td>Gmts Qty. (Pcs)</td>
                        <td><input name="txtBatchQty" id="txtBatchQty" type="text" class="text_boxes_numeric" style="width:140px" readonly placeholder="Display" />
                        </td>
                    ...
                    <tr>
                    	<td>Order No</td>
                        <td>
                            <input class="text_boxes" type="text" name="txt_order_no" id="txt_order_no" placeholder="Display" style="width:150px;" readonly /> 
                    	...
                    	<td>Buyer Po</td>
                        <td><input name="txtbuyerPo" id="txtbuyerPo" type="text" class="text_boxes" style="width:140px" readonly placeholder="Display" />
                    ...
                    <tr>
                    	<td>Operation</td>
                    	<td>
							<input name="txtoperation" id="txtoperation" type="text" class="text_boxes" style="width:150px" readonly placeholder="Display" />
                        	<input type="hidden" name="hidden_operation_id" id="hidden_operation_id" readonly />
                    	</td>
                    ...
                    <tr>
                    	<td>Remarks</td>
                    	<td colspan="5">
                    		<input name="txtRemarks" id="txtRemarks" type="text" class="text_boxes" style="width:98%" />
                    	</td>
                    </tr>
                ...
                <table cellpadding="0" cellspacing="1" width="100%">
                    ...
                    <tr>
                        ...
                        <input type="hidden" name="update_dtls_id" id="update_dtls_id" readonly>
							...
							<input type="button" name="print" id="print" value="Print" onClick="fnc_chemical_dyes_issue_requisition(4)" style="width:100px;display:none;" class="formbuttonplasminus" />
							<input type="button" name="print2" id="print2" value="Print2" onClick="fnc_chemical_dyes_issue_requisition(5)" style="width:100px;" class="formbuttonplasminus" />
