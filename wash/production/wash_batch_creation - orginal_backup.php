...
<script>
	...
	function load_color_list(rowNo)
	{
		...
		//alert(poId+"**"+cboItem);
		show_list_view(poId+"*"+cboItem+"*"+rowNo,'show_color_listview','list_color','requires/wash_batch_creation_controller','');
	}
 
	function color_set_value(color,batch_blnc,rowNo,within_group,party_id,buyer_po_id,gmts_type)
	{
	...
	function openmypage_po(row_no)
	{
		...
		var page_link = 'requires/wash_batch_creation_controller.php?cbo_company_id='+cbo_company_id+'&buyer_id='+buyer_id+'&color_name='+color_name+'&tot_row='+tot_row+'&action=po_popup';
	...
	{
		window.open("requires/wash_batch_creation_controller.php?data=" + data+'&action='+action, true );
	}
	...
	function fnc_batch_creation(operation)
	{
		if(operation==4)
		{
			 ...
			 generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title,'batch_card_print','requires/wash_batch_creation_controller');
			 return;
		}
		...
		if( form_validation('cbo_batch_against*cbo_company_id*txt_batch_date*txt_batch_weight*cbo_operation','Batch Against*Company*Batch Date*Batch Weight*operation')==false )
		{
			return;
		}
		...
		for (var i=1; i<=row_num; i++)
		{
			...
			data_all+=get_submitted_data_string('poId_'+i+'*cboItem_'+i+'*txtGmtsQty_'+i+'*txtBatchQnty_'+i+'*buyerPoId_'+i+'*updateIdDtls_'+i,"../../",2);
		}
		
		//alert(data_all);return;
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_batch_sl_no*cbo_batch_against*cbo_gmts_type*cbo_sub_operation*cbo_company_id*batch_no_creation*txt_batch_number*txt_batch_date*txt_batch_weight*txt_ext_no*txt_batch_color*cbo_color_range*txt_organic*txt_process_id*txt_du_req_hr*txt_du_req_min*update_id*machine_id*txt_remarks*cbo_shift*txt_operator*txt_supervisor*buyer_id*cbo_within_group*cbo_operation',"../../")+data_all+'&total_row='+row_num+'&txt_deleted_id='+txt_deleted_id;

		freeze_window(operation);
		//alert(data); return;
		http.open("POST","requires/wash_batch_creation_controller.php",true);
	...
	function fnc_batch_creation_Reply_info()
	{
		if(http.readyState == 4) 
		{
			...
			else if((reponse[0]==0 || reponse[0]==1))
			{
				...
				show_list_view(batch_against+'**'+batch_for+'**'+reponse[1],'batch_details','batch_details_container','requires/wash_batch_creation_controller','');
	...
	function openmypage_batchNo()
	{
		...
		{ 	
			...
			var page_link = 'requires/wash_batch_creation_controller.php?cbo_company_id='+cbo_company_id+'&batch_against='+batch_against+'&action=batch_popup';
			
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=865px,height=370px,center=1,resize=1,scrolling=0','../');
			...
			emailwindow.onclose=function()
			{
				...
				//load_drop_down( 'requires/wash_batch_creation_controller', this.value, 'load_drop_down_sub_operation', 'sub_operation');
				...
				{
					freeze_window(5);
					get_php_form_data(batch_against+'**'+batch_for+'**'+batch_id, "populate_data_from_search_popup", "requires/wash_batch_creation_controller" );
				    show_list_view(batch_against+'**'+batch_for+'**'+batch_id,'batch_details','batch_details_container','requires/wash_batch_creation_controller','');
					release_freezing();
					$('#txt_deleted_id').val('');
					calculate_batch_qnty();
				} 
	...
	function load_color_list_update(data)
	{
		//alert(data); return;
		var data=data.split("*");
		show_list_view(data[0]+"*"+data[1]+"*"+"0*"+data[2],'show_color_listview','list_color','requires/wash_batch_creation_controller','');
	}
	...
	function openmypage_process()
	{
		...
		var page_link = 'requires/wash_batch_creation_controller.php?txt_process_id='+txt_process_id+'&action=process_name_popup&cbo_batch_against='+cbo_batch_against;
	...
	function fn_machine_seach()
	{
		...
		{ 	
			...
			var page_link = 'requires/wash_batch_creation_controller.php?cbo_company_id='+cbo_company_id+'&action=machineNo_popup&cbo_batch_against='+cbo_batch_against;
            ...
            <fieldset style="width:640px;">
                <table width="630" align="center" border="0">
                    ...
                    <td width="110" class="must_entry_caption">Gmts Type</td>
                        <td><? 
							echo create_drop_down( "cbo_gmts_type", 170, $wash_gmts_type_array,"", 1, "-- Select Location --", $selected, "",1 ); ?></td>
                        <td  style="display:none" width="110" class="must_entry_caption">Batch Against</td>
                        <td style="display:none"><? echo create_drop_down( "cbo_batch_against", 172, $batch_against,"",0, '--- Select ---', 1, "validate_check(this.value)",'','6','','','',1 ); ?></td>
                    ...
                    <tr>
                        ...
                        <td><input type="text" name="txt_batch_number" id="txt_batch_number" class="text_boxes" style="width:160px;" placeholder="Double Click To Edit" onDblClick="openmypage_batchNo()" tabindex="5" /></td>
                        <td>Operator</td>
                    ...
                    <tr>
                        <td>Color Range</td>
                        <td><? echo create_drop_down( "cbo_color_range", 172, $color_range,"",1, "-- Select --", 0, "",'','','','','',9); ?></td>
                        <td>Organic</td>
                        <td><input type="text" name="txt_organic" id="txt_organic" class="text_boxes" style="width:160px;" tabindex="10" /></td>
                    </tr>
                    <tr>
                    	<td>Process Name</td>
                    	<td><? echo create_drop_down( "txt_process_id",172, $wash_type,"",0, "-- Select --",0,"1",1); ?></td>
                    ...
                    <tr>
                       ...
                       <td>
                        ...
                        <td><? echo create_drop_down( "cbo_operation",172, $wash_operation_arr,"",1, "-- Select --",0,"load_drop_down( 'requires/wash_batch_creation_controller', this.value, 'load_drop_down_sub_operation', 'sub_operation'),set_multiselect('cbo_sub_operation','0','0','','0')"); ?></td>
                    </tr>
                    <tr>
                    	<td>Sub Operation</td>
                        <td id="sub_operation"><? echo create_drop_down( "cbo_sub_operation",172, $wash_sub_operation_arr,"","", "", 0, "",'','','','','',9); ?></td>
                    	<td>Remarks</td>
                        <td colspan="4"><input type="text" name="txt_remarks" id="txt_remarks" class="text_boxes" tabindex="15" style="width:94%;" /></td>
                    </tr>
                 </table>
            </fieldset>                 
                ...
                <table cellpadding="0" cellspacing="0" width="640" class="rpt_table" border="1" rules="all" id="tbl_item_details">
                    ...
                    <tbody id="batch_details_container">
                        <tr class="general" id="tr_1">
                            ...
                            <td><? echo create_drop_down( "cboItem_1", 180, $garments_item,"", 1, "-- Select Item --", 0, "load_color_list(1)" ); ?></td>
                            <td>
