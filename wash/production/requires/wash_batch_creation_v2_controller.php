...
if ($action=="load_drop_down_buyer")
{
	...
	if($data[1]==1)
	{
		echo create_drop_down( "cbo_party_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", "", "load_drop_down( 'wash_batch_creation_v2_controller', this.value+'_'+$within_group, 'load_drop_down_buyer_buyer', 'buyer_buye_td');");
	}
	else
	{
...
if($action=="po_popup")
{
	...
	if($update_id != "") $update_id = $update_id ;	
	else $update_id  = 0;
	...
	?>
		<script>
			...
			function js_set_value( po_id,po_no,gmts_item_id,buyer_id,buyer_po_id,buyer_po_no,color_id,job_no,within_group,po_qnty,buyer_style_ref,order_uom,operation_type,rewash_qty,batch_color,gmts_type,batch_no,batch_id,batch_dtls_id)
			{
				...
				document.getElementById('rewash_qty').value=rewash_qty;
				document.getElementById('batch_color').value=batch_color;
				document.getElementById('gmts_type').value=gmts_type;
			...
			function fnc_load_party(within_group) 
			{
				...
				load_drop_down( 'wash_batch_creation_v2_controller', company+'_'+within_group, 'load_drop_down_buyer_buyer', 'buyer_buye_td'); 
				load_drop_down( 'wash_batch_creation_v2_controller', company+'_'+within_group, 'load_drop_down_buyer', 'buyer_td' );
			}
		</script>
	</head>
				...
				<table cellpadding="0" cellspacing="0" width="930" class="rpt_table" align="center">
                    ...
                    <tr>
						...
						<th>
                            ...
                            <input type="hidden" name="rewash_qty" id="rewash_qty" value="">
                            <input type="hidden" name="batch_color" id="batch_color" value="">
                            <input type="hidden" name="gmts_type" id="gmts_type" value="">
						</th> 
                        </tr>
					</thead>
					<tr class="general">
						...
						<td id="buyer_td">
                          ...
                          <td id="buyer_buye_td">
                         <? 
                          echo create_drop_down( "txt_buyer_buyer_no", 125, $blank_array,"", 1, "Select buyer", $selected, "load_drop_down( 'wash_batch_creation_v2_controller', this.value+'_'+document.getElementById('cbo_within_group').value, 'load_drop_down_buyer_buyer', 'buyer_buye_td'); ",1,"" ); ?>
                        </td>
						<td><?
   ...
   if($within_group==1)
   {
	...
	if($batch_against==12)
	{
	...
	else if($batch_against==11)
 	{
		 ...
		 $production_qty_sql="select a.color_id,a.operation_type, b.id, b.po_id, b.prod_id,b.roll_no as batch_qnty,b.buyer_po_id,d.qcpass_qty as production_qty,d.rewash_qty,c.recipe_id from pro_batch_create_mst a, pro_batch_create_dtls b,subcon_embel_production_mst c,subcon_embel_production_dtls d  where  a.entry_form=316 and c.entry_form=301 and  a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  and d.status_active=1 and d.is_deleted=0 and a.id=c.recipe_id and c.id=d.mst_id and a.company_id=$company_id and a.operation_type=$operation  $batch_cond  order by b.id";  // and a.id= $batch_id  
		$production_data_array=sql_select($production_qty_sql); 
		$production_type_array=array();
		foreach($production_data_array as $row)
		{
			$production_type_array[$row[csf('operation_type')]][$row[csf('po_id')]]['production_qty']+=$row[csf('production_qty')];
			$production_type_array[$row[csf('operation_type')]][$row[csf('po_id')]]['rewash_qty']+=$row[csf('rewash_qty')];	
		}
		unset($production_data_array);
		
		
		   $sql = "select a.subcon_job as job_no, a.within_group, a.party_id as buyer_name,b.buyer_po_id,b.buyer_po_no, b.order_uom, b.gmts_item_id as gmts_item_id, b.gmts_color_id as color_id, b.id, b.order_no as po_number, b.order_quantity as po_qnty,b.buyer_style_ref, a.job_no_prefix_num,b.party_buyer_name,a.gmts_type
		...
		group by a.subcon_job,a.within_group, a.party_id,b.buyer_po_id,b.buyer_po_no, b.order_uom, b.gmts_item_id, b.gmts_color_id, b.id, b.order_no, b.order_quantity,b.buyer_style_ref, a.job_no_prefix_num,b.party_buyer_name,a.gmts_type
		order by b.id desc";
...
//echo $sql;
	...
	{
	 /*$operation_type_sql="select a.color_id,a.operation_type, b.id, b.po_id, b.prod_id, b.roll_no, b.batch_qnty, b.buyer_po_id from pro_batch_create_mst a, pro_batch_create_dtls b where  b.po_id in(".implode(',', $ord_dtls_array).") and a.entry_form=543 and  a.id=b.mst_id and b.status_active=1 and b.is_deleted=0  order by b.id";*/
	 
	 $operation_type_sql="select a.color_id,a.operation_type, b.id, b.po_id, b.prod_id, b.roll_no, b.batch_qnty, b.buyer_po_id from pro_batch_create_mst a, pro_batch_create_dtls b where   a.company_id=$company_id and a.entry_form=543 and  a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  order by b.id";
	 $data_array=sql_select($operation_type_sql); 
	}
	
	 //$operation_type_sql="select a.color_id,a.operation_type, b.id, b.po_id, b.prod_id, b.roll_no, b.batch_qnty, b.buyer_po_id from pro_batch_create_mst a, pro_batch_create_dtls b where  b.po_id in(".implode(',', $ord_dtls_array).") and a.entry_form=543 and  a.id=b.mst_id and b.status_active=1 and b.is_deleted=0  order by b.id";
	//$data_array=sql_select($operation_type_sql); 
	$operation_type_array=array();$batch_qnty_array=array();
	...
	foreach($data_array as $row)
	{
		...
		$batch_qnty_array[$row[csf('operation_type')]][$row[csf('po_id')]]['batch_qnty']+=$row[csf('roll_no')];		
	}
        ...
        <div style="width:970px; overflow-y:scroll; max-height:260px;" id="buyer_list_view" align="center">
            ...
            <?
				...
				foreach ($nameArray as $selectResult)
				{
					...
					$prev_batch_qnty=$batch_qnty_array[$operation][$selectResult[csf('id')]]['batch_qnty'];
					
					//echo $prev_batch_qnty; die;
					
					$rewash_qty=$production_type_array[$operation][$selectResult[csf('id')]]['rewash_qty']-$prev_batch_qnty;
				...
				?>
                    <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value(<? echo $selectResult[csf('id')]; ?>,'<? echo $selectResult[csf('po_number')]; ?>','<? echo $selectResult[csf('gmts_item_id')]; ?>','<? echo $selectResult[csf('buyer_name')]; ?>','<? echo $selectResult[csf('buyer_po_id')]; ?>','<? echo $selectResult[csf('buyer_po_no')]; ?>','<? echo $selectResult[csf('color_id')]; ?>','<? echo $selectResult[csf('job_no')]; ?>','<? echo $selectResult[csf('within_group')]; ?>','<? echo $selectResult[csf('po_qnty')]; ?>','<? echo $selectResult[csf('buyer_style_ref')]; ?>','<? echo $selectResult[csf('order_uom')]; ?>','<? echo $operation_type; ?>','<? echo $rewash_qty; ?>','<? echo $color_arr[$selectResult[csf('color_id')]]; ?>','<? echo $selectResult[csf('gmts_type')]; ?>')"> 
                        ...
                        <td width="100" style="word-break:break-all"><?php echo $buyer; ?></td>
                         <td width="100" style="word-break:break-all"><?php echo $party_buyer; ?></td>
                        <td width="100" style="word-break:break-all" title="<? echo $selectResult[csf('buyer_po_id')]; ?>"><? echo $selectResult[csf('buyer_po_no')]; ?></td>
                        <td width="100" style="word-break:break-all"><? if($selectResult[csf('within_group')]==1){echo $buyer_po_arr[$selectResult[csf('buyer_po_id')]]['style'];}else { echo $selectResult[csf('buyer_style_ref')] ;} ?></td>
                        <td width="70" align="right"><? echo $selectResult[csf('po_qnty')]; ?></td> 
                        <td width="70" align="right"><? echo $rewash_qty; ?></td> 
...
if ($action=="save_update_delete")
{
	...
	{ 
		...
		if(str_replace("'","",$txt_batch_color)!="")
		{
			...
			{
				$color_id = return_id( str_replace("'","",$txt_batch_color), $color_arr, "lib_color", "id,color_name","543");
		...
		if(str_replace("'","",$update_id)=="")
		{
			...
			{
				if(is_duplicate_field( "batch_no", "pro_batch_create_mst", "batch_no=$txt_batch_number and $extention_no_cond and entry_form=543 and  status_active=1 and  is_deleted=0" )==1)
				{
		...
		else
		{
			...
			if($batch_no_creation!=1)
			{
				if(is_duplicate_field( "batch_no", "pro_batch_create_mst", "batch_no=$txt_batch_number and $extention_no_cond and entry_form=543 and id<>$update_id and  status_active=1 and  is_deleted=0" )==1)
				{
					//check_table_status( $_SESSION['menu_id'],0);
					echo "11**0"; 
					disconnect($con); die;			
...
if ($action=="populate_data_from_search_popup")
{
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_drop_down( 'requires/wash_batch_creation_v2_controller', '".$row[csf("operation_type")]."', 'load_drop_down_sub_operation', 'sub_operation');\n";
		...
		echo "load_drop_down( 'requires/wash_batch_creation_v2_controller', document.getElementById('cbo_location_name').value+'_'+document.getElementById('cbo_company_id').value, 'load_drop_down_floor', 'floor_td');\n";
...
if( $action == 'batch_details' ) 
{
	...
	$cbo_company_id=trim($data[7]);
	
	//echo $operation_type_id; die;
	...
	foreach($po_data_array as $poRow)
	{
    	...
    	$po_item_color_qty[$poRow[csf("id")]][$poRow[csf("gmts_item_id")]][$poRow[csf("gmts_color_id")]]["uom"]=$poRow[csf("order_uom")];
		 ...
		 $production_qty_sql="select a.color_id,a.operation_type, b.id, b.po_id, b.prod_id,b.roll_no as batch_qnty,b.buyer_po_id,d.qcpass_qty as production_qty,d.rewash_qty,c.recipe_id from pro_batch_create_mst a, pro_batch_create_dtls b,subcon_embel_production_mst c,subcon_embel_production_dtls d  where  a.entry_form=316 and c.entry_form=301 and  a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  and d.status_active=1 and d.is_deleted=0 and a.id=c.recipe_id and c.id=d.mst_id and a.company_id=$cbo_company_id and a.operation_type=$operation_type_id  and b.po_id=$po_id order by b.id";  // and a.id= $batch_id  
		$production_data_array=sql_select($production_qty_sql); 
		$production_type_array=array();
		foreach($production_data_array as $row)
		{
	 ...
	 {
 		 $batch_qty_arr=array();
		 $batch_dtls_sql="select a.color_id, b.po_id, b.prod_id,c.mst_id,c.job_no_mst,c.buyer_po_no,c.buyer_po_id, sum(b.roll_no) as roll_no from pro_batch_create_mst a, pro_batch_create_dtls b,subcon_ord_dtls c where a.entry_form=543 and a.id=b.mst_id and  b.po_id=c.id  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  and a.batch_against in (11) group by b.po_id, b.prod_id,a.color_id,c.mst_id,c.job_no_mst,c.buyer_po_no,c.buyer_po_id";
	...
	if ($po_id !="") $po_id_cond = " and b.po_id in (".$po_id.")";
	 ...
	 if($batch_against ==11)
	 {
  		 ...
  		 $operation_type_sql="select a.color_id,a.operation_type, b.id, b.po_id, b.prod_id,b.roll_no as batch_qnty, b.buyer_po_id from pro_batch_create_mst a, pro_batch_create_dtls b where a.entry_form=543 and  a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0   $po_id_cond  $color_id_cond and a.batch_against in (11) and a.id!=$batch_id order by b.id";  
	...
	//echo "select a.id as batch_id,a.batch_against,a.color_id,a.operation_type, b.id, b.po_id, b.prod_id, b.roll_no, b.batch_qnty, b.buyer_po_id,c.job_no_mst,c.buyer_po_no,c.buyer_style_ref from pro_batch_create_mst a, pro_batch_create_dtls b,subcon_ord_dtls c  where b.mst_id=$batch_id and a.entry_form=543 and  a.id=b.mst_id and  b.po_id=c.id and b.status_active=1 and b.is_deleted=0  and c.status_active=1 and c.is_deleted=0  order by b.id"; die;
	...
	/*
	echo "select a.id as batch_id,a.batch_against,a.color_id,a.operation_type, b.id, b.po_id, b.prod_id, b.roll_no, b.batch_qnty, b.buyer_po_id,c.job_no_mst,c.buyer_po_no,c.buyer_style_ref from pro_batch_create_mst a, pro_batch_create_dtls b,subcon_ord_dtls c  where $batch_cond and a.entry_form=543 and  a.id=b.mst_id and  b.po_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and c.status_active=1 and c.is_deleted=0  order by b.id";*/
	...
	if ($data_array[0][csf('batch_against')] == 11)
	{
		...
		foreach ($data_array as $row)
		{
			...
			foreach($item_array as $item)
			{
		 	...
		 	$prevBatchQty=$currBatchQty=$chkgmtsqty=0;
		   ...
		   if($currentoperationtypeId==1)
			...
			$total_rewash_qty=$production_type_array[$row[csf('operation_type')]][$row[csf('po_id')]]['rewash_qty'];
			
			//echo $total_rewash_qty; 
			
			//$totalBatchQty=($prevBatchQty)+$row[csf('roll_no')];
			
			$totalBatchQty=($prevBatchQty);
			$chkgmtsqty=$total_rewash_qty-$totalBatchQty;
			//echo $prv_first_wash_qty."==".$row[csf('roll_no')]; die;
...
*/
	///////////////////////////// end previous backup
	 ...
	 where a.entry_form=543 and a.id=b.mst_id and  b.po_id=c.id  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by b.po_id, b.prod_id,a.color_id,c.mst_id,c.job_no_mst,c.buyer_po_no,c.buyer_po_id";
	$batchArray = sql_select($batch_dtls_sql);
	foreach ($batchArray as $value) 
	{
		$batch_qty_arr[$value[csf("job_no_mst")]][$value[csf("buyer_po_no")]][$value[csf("color_id")]]+=$value[csf("roll_no")];
...
//echo $ColorId."mahbub";die;
  ...
  $operation_type_sql="select a.color_id,a.operation_type, b.id, b.po_id, b.prod_id,b.roll_no as batch_qnty, b.buyer_po_id from pro_batch_create_mst a, pro_batch_create_dtls b where  b.po_id in($poId) and a.color_id in($ColorId)  and a.entry_form=543 and  a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  order by b.id";
...
/*  $operation_type_sql="select a.color_id,a.operation_type, b.id, b.po_id, b.prod_id,b.roll_no as batch_qnty, b.buyer_po_id from pro_batch_create_mst a, pro_batch_create_dtls b where  b.po_id in($poId) and a.color_id in($ColorId)  and a.entry_form=543 and  a.id=b.mst_id and b.status_active=1 and b.is_deleted=0  order by b.id";*/ 
