...
//////////////// start previous backup 
	...
	$data_array=sql_select("select a.color_id, b.id, b.po_id, b.prod_id, b.roll_no, b.batch_qnty, b.buyer_po_id,c.job_no_mst,c.buyer_po_no from pro_batch_create_mst a, pro_batch_create_dtls b,subcon_ord_dtls c  where b.mst_id=$batch_id and a.entry_form=316 and  a.id=b.mst_id and  b.po_id=c.id and b.status_active=1 and b.is_deleted=0  and c.status_active=1 and c.is_deleted=0  order by b.id"); 
	...
	foreach($data_array as $row)
	{
		...
		$prevBatchQty=$batch_qty_arr[$row[csf("job_no_mst")]][$row[csf("buyer_po_no")]][$row[csf("color_id")]];
		...
		<tr class="general" id="tr_<? echo $tblRow; ?>">
            ...
            <td><input name="txtbuyerPo_<? echo $tblRow; ?>" id="txtbuyerPo_<? echo $tblRow; ?>" type="text" class="text_boxes" value="<? echo $row[csf('buyer_po_no')]; ?>" style="width:80px" placeholder="Display" readonly />
			<td>
	...
	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="330" class="rpt_table color_tble" style="float: left;">
		...
		foreach ($nameArray as $selectResult) 
		{
			...
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i; ?>" onClick="color_set_value('<? echo $color_arr[$selectResult[csf('color_id')]]; ?>','<? echo $batch_balance; ?>','<? echo $rowNum; ?>','<? echo $selectResult[csf('within_group')]; ?>','<? echo $selectResult[csf('party_id')]; ?>','<? echo $selectResult[csf('buyer_po_id')]; ?>','<? echo $selectResult[csf('gmts_type')]; ?>','<? echo $selectResult[csf('buyer_po_no')]; ?>')"> 
				<td align="center" title="<? echo $selectResult[csf("job_no_mst")]."==".$selectResult[csf("buyer_po_no")]."==".$selectResult[csf("color_id")] ?>"><? echo $i; ?> </td>
