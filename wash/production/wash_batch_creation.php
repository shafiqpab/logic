...
<script>
	...
	<?
		if(isset($_SESSION['logic_erp']['data_arr'][316]))
		{
		  $data_arr=json_encode($_SESSION['logic_erp']['data_arr'][316] );
	...
	function load_color_list(rowNo)
	{
		...
		if(batch_against!=11)
		{
			show_list_view(poId+"*"+cboItem+"*"+rowNo+"*"+hiddenOperationType+"*"+hiddenColorId+"*"+PoNo+"*"+hiddenbatchno+"*"+hiddenbatchid+"*"+hiddenbatchdtlsid+"*"+cbo_operation_type+"*"+update_id+"*"+batch_against,'show_color_listview','list_color','requires/wash_batch_creation_controller','');
	...
	function openmypage_po(row_no)
	{
		...
		var page_link = 'requires/wash_batch_creation_controller.php?cbo_company_id='+cbo_company_id+'&buyer_id='+buyer_id+'&color_name='+color_name+'&tot_row='+tot_row+'&batch_against='+batch_against+'&action=po_popup';
	...
	function fnc_batch_creation(operation)
	{
		if(operation==4)
		{
			...
			// generate_report_file($('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#cbo_within_group').val(),'batch_card_print','requires/wash_batch_creation_controller');

			var report_title=$( "div.form_caption" ).html();
			print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_batch_sl_no').val()+'*'+$('#txt_batch_number').val()+'*'+$('#txt_ext_no').val()+'*'+report_title+'*'+$('#cbo_within_group').val(), "batch_card_print", "requires/wash_batch_creation_controller") 
			return;
		}
	...
	function openmypage_batchNo()
	{
		...
		{ 	
			...
			//alert(po_id);return;
				...
				{
					freeze_window(5);
					get_php_form_data(batch_against+'**'+batch_for+'**'+batch_id+'**'+po_id+'**'+operation_type_id+'**'+batch_color_id+'**'+operation_type+'**'+unloaded_batch+'**'+ext_from+'**'+cbo_company_id+'**'+batch_no, "populate_data_from_search_popup", "requires/wash_batch_creation_controller" );
				    show_list_view(batch_against+'**'+batch_for+'**'+batch_id+'**'+po_id+'**'+operation_type_id+'**'+batch_color_id+'**'+operation_type,'batch_details','batch_details_container','requires/wash_batch_creation_controller','');
					release_freezing();
					$('#txt_deleted_id').val('');
					calculate_batch_qnty();
					load_color_list(1);
	...
	function load_color_list_update(data)
	{
		...
		if(batch_against!=11)
		{
			show_list_view(data[0]+"*"+data[1]+"*"+"0*"+data[2]+"*"+batch_against,'show_color_listview','list_color','requires/wash_batch_creation_controller','');
			change_delivery_issue_captions();
		}
	}
	...
	function validate_check(str)
	{
	 ...
	 function val_roundup()
	 {
        if($('#round_down').is(':checked')){
            $( "input[name='txtGmtsQty[]']" ).each(function (index){
                ...
                <table width="730" align="center" border="0">
                    <tr>
                   		<td width="110" class="must_entry_caption">Batch Against</td>
								<td>
									...
									echo create_drop_down( "cbo_batch_against", 172, $batch_against,"",6, '--- Select ---',1, "",'','6,11,12','','','',1 );
                    ...
                    <tr>
                       ...
                       <td>
                        ...
                        <td><? echo create_drop_down( "cbo_operation",172, $wash_operation_arr,"",1, "-- Select --",0,"load_drop_down( 'requires/wash_batch_creation_controller', this.value, 'load_drop_down_sub_operation', 'sub_operation'),set_multiselect('cbo_sub_operation','0','0','','0'),list_wash_operation(1)",1); ?>
                ...
                <table cellpadding="0" cellspacing="0" width="740" class="rpt_table" border="1" rules="all" id="tbl_item_details">
                    ...
                    <tbody id="batch_details_container">
                        <tr class="general" id="tr_1">
                            ...
                            <td>
                                <input type="text" name="txtGmtsQty[]" id="txtGmtsQty_1" class="text_boxes_numeric" onKeyUp="calculate_batch_qnty();" style="width:75px" />
