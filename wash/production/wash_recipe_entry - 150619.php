<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Recipe Entry For Wash 
Functionality	:	
JS Functions	:
Created by		:	Kausar 
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Wash Recipe Entry Info", "../../", 1, 1,$unicode,1);
...
<script>
	...
	function fnc_openmypage_po()
	{
		...
		{ 
			...
			var title = 'Wash Order Selection Form';	
			
			var page_link = 'requires/wash_recipe_entry_controller.php?action=washorder_popup';
			...
			emailwindow.onclose=function()
			{
				...
				if(estr_data[9]!="")
				{
					...
					load_drop_down( 'requires/wash_recipe_entry_controller', estr_data[6], 'load_drop_down_embl_type', 'embl_type_td');
	...
	{
		load_drop_down( 'requires/wash_recipe_entry_controller', company+'_'+val, 'load_drop_down_buyer', 'buyer_td_id' );
	}
	...
	function show_details()
	{
		...
		var list_view_orders = return_global_ajax_value( cbo_company_id, 'item_details', '', 'requires/wash_recipe_entry_controller');
		if(list_view_orders!='')
		{
	...
	function fnc_recipe_entry(operation)
	{
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title, "recipe_entry_print", "requires/wash_recipe_entry_controller") 
			//return;
			show_msg("3");
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			if(recipe_for!=3)
			{
				if( form_validation('cbo_recipe_for*cbo_company_id*txt_recipe_date*cbo_within_group*txt_po_color*txt_multi_color','Recipe For*Company*Recipe Date*Within Group*Color*Color')==false )
				{
					return;
				}
			}
			...
			{
				if( form_validation('cbo_recipe_for*cbo_company_id*txt_recipe_date*txt_multi_color*cbo_store_id*cbo_item_category*txt_item_group','Recipe For*Company*Recipe Date*Color*Store*Item Category*Item Group')==false )
				{
					return;
				}
			}
			...
			for(var j=1; j<=row_num; j++)
			{
				...
				{
					i++;
					data_all+="&txt_seqno_" + i + "='" + $('#txt_seqno_'+j).val()+"'"+"&product_id_" + i + "='" + $('#product_id_'+j).text()+"'"+"&txt_comments_" + i + "='" + $('#txt_comments_'+j).val()+"'"+"&txt_ratio_" + i + "='" + $('#txt_ratio_'+j).val()+"'"+"&updateIdDtls_" + i + "='" + $('#updateIdDtls_'+j).val()+"'";
	...
	function openmypage_sysNo()
	{
		...
		{ 	
			...
			var page_link = 'requires/wash_recipe_entry_controller.php?cbo_company_id='+cbo_company_id+'&action=systemid_popup';
	...
	function fnc_item_details(color_data)
	{
		...
		show_list_view(cbo_company_id+"**"+color_id+"**"+update_id, 'item_details', 'list_container_recipe_items', 'requires/wash_recipe_entry_controller', '');
		setFilterGrid('tbl_list_search',-1);
	...
	function openmypage_item_group()
	{
		...
		else
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/wash_recipe_entry_controller.php?category='+category+'&action=itemgroup_popup','Search Group Name', 'width=580px,height=400px,center=1,resize=0,scrolling=0','../')
			emailwindow.onclose=function()
			{
	...
	function fnc_tot_ratio_calculate()
	{
		...
		$('#txt_total_ratio').val( total_ratio );
	}
			...
			<fieldset style="width:920px;">
                <table width="910" align="center" border="0">
                    ...
                    <tr>
                    	...
                    	<td>Remarks</td>
                        <td colspan="2"><input type="text" name="txt_remarks" id="txt_remarks" class="text_boxes" value="" style="width:250px;" /></td>
            ...
            <legend>Item Details</legend>
 				<table cellpadding="1" cellspacing="1" border="0" width="100%">
                	<tr>
                        ...
                        <td width="80" align="center"><strong>Store</strong></td>
 					...
 					<tr>
                        ...
                        <td align="center" id="store_td"><? echo create_drop_down( "cbo_store_id", 70, $blank_array,"", "1", "-Store-", 0, "","1","","","","","","" );  ?></td>
 						<td align="center"><? echo create_drop_down( "cbo_item_category", 130, $item_category,"", "1", "--Select--", 0, "","1","5,6,7","","","","","" );  ?></td>
                        <td align="center">
                    ...
                    <tr>
                    	<td colspan="4" align="right"><strong>Total Ratio :</strong></td>
