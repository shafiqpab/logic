...
<script>
	...
	subprocessForWashArr=['86','87','88','89','90','91','92','93','94','95'];
	...
	function show_details()
	{
		...
		var list_view_orders = return_global_ajax_value( cbo_company_name+'**'+cbo_sub_process+'**'+cbo_store_id+'******'+variable_lot, 'item_details', '', 'requires/wash_recipe_entry_controller');
	...
	function color_row(tr_id)
	{
		...
		var issue_qty=$('#hiddden_issue_qty_'+tr_id).val()*1;
		var update_id=$('#update_id').val()*1;
		var sub_process=$('#cbo_sub_process').val();
		
		if(update_id!="")
		{
			if(txt_ratio<issue_qty)
			{
				alert("Dosage Qty. Can not be Less than Issue Qty."+" ("+ issue_qty +")");
	...
	function fnc_recipe_entry(operation)
	{
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+$('#txt_labdip_no').val()+'*'+report_title, "recipe_entry_print", "requires/wash_recipe_entry_controller") 
			//return;
			show_msg("3");
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			if( form_validation('txt_batch_no*cbo_company_name*cbo_location*txt_recipe_date*txt_color','Batch*Company*Location*Recipe Date*Color')==false )
			{
				return;
			}
			...
			if(copy_val==1) //Copy Item
			{
				...
				/*var response_data = return_global_ajax_value( cbo_company_name+'**'+update_id_check, 'item_stock_details', '', 'requires/wash_recipe_entry_controller');
				var response=response_data.split("_");
				//alert(response[0]);
				if(response[0]==1)
				{
			...
			//alert(check_pro); return; 
			 ...
			 if(check_pro==2)
			 {
				...
				for(var j=1; j<=row_num; j++)
				{
					...
					{
						i++;
						data_all+="&txt_seqno_" + i + "='" + $('#txt_seqno_'+j).val()+"'"+"&product_id_" + i + "='" + $('#product_id_'+j).text()+"'"+"&txt_item_lot_" + i + "='" + $('#txt_item_lot_'+j).val()+"&txt_comments_" + i + "='" + $('#txt_comments_'+j).val()+"'"+"&txt_ratio_" + i + "='" + $('#txt_ratio_'+j).val()+"'"+"&updateIdDtls_" + i + "='" + $('#updateIdDtls_'+j).val()+"'"+"&hiddden_issue_qty_" + i + "='" + $('#hiddden_issue_qty_'+j).val()+"'";
			...
			{ 
				...
				var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_labdip_no*cbo_company_name*cbo_location*cbo_floor_name*txt_recipe_date*cbo_order_source*txt_booking_order*txt_booking_id*txt_recipe_des*txt_batch_id*cbo_party_name*cbo_method*txt_color*cbo_color_range*txt_liquor*txt_batch_ratio*txt_liquor_ratio*txt_booking_type*cbo_sub_process*txt_batch_weight*txt_remarks*copy_id*update_id*txt_total_liquor_ratio*txt_liquor_ratio_dtls*txt_time*txt_temparature*txt_ph*cbo_store_id*cbo_item_category*item_group_id*hidd_group_code*hidd_cons_uom*hidd_order_uom*txt_subgroup_name*txt_description*txt_item_size*hidd_newprod_id*cbo_within_group*txt_sys_id*txt_copy_from*cbo_gmts_item',"../../")+data_all+'&total_row='+i;
				// alert (data+'=='+copy_val); return;
				
				
			}
			else if(copy_val==1)
			{
				//var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_labdip_no*cbo_company_name*cbo_location*txt_recipe_date*cbo_order_source*txt_booking_order*txt_booking_id*txt_recipe_des*txt_batch_id*cbo_party_name*cbo_method*txt_color*cbo_color_range*txt_liquor*txt_batch_ratio*txt_liquor_ratio*txt_booking_type*cbo_sub_process*txt_subprocess_remarks*txt_batch_weight*txt_remarks*copy_id*update_id_check*txt_total_liquor_ratio*txt_liquor_ratio_dtls*txt_time*txt_temparature*txt_ph*cbo_store_id*cbo_item_category*item_group_id*hidd_group_code*hidd_cons_uom*hidd_order_uom*txt_subgroup_name*txt_description*txt_item_size*hidd_newprod_id*cbo_within_group*txt_sys_id*txt_copy_from',"../../")+data_all+'&total_row='+i;
				var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_labdip_no*cbo_company_name*update_id_check*cbo_location*cbo_floor_name*txt_recipe_date*cbo_order_source*txt_booking_order*txt_booking_id*txt_recipe_des*txt_batch_id*cbo_party_name*cbo_method*txt_color*cbo_color_range*txt_liquor*txt_batch_ratio*txt_liquor_ratio*txt_booking_type*cbo_sub_process*txt_batch_weight*txt_remarks*copy_id*update_id*txt_total_liquor_ratio*txt_liquor_ratio_dtls*txt_time*txt_temparature*txt_ph*cbo_store_id*cbo_item_category*item_group_id*hidd_group_code*hidd_cons_uom*hidd_order_uom*txt_subgroup_name*txt_description*txt_item_size*hidd_newprod_id*cbo_within_group*txt_sys_id*txt_copy_from*txt_subprocess_remarks*cbo_gmts_item',"../../");
				
			}
			
			freeze_window(operation);
			http.open("POST","requires/wash_recipe_entry_controller.php",true);
	...
	function fnc_recipe_entry_Reply_info()
	{
		if(http.readyState == 4) 
		{
			...
			if((reponse[0]==0 || reponse[0]==1))
			{
				...
				var sub_process = $('#cbo_sub_process').val();
				show_list_view(reponse[1], 'recipe_item_details', 'recipe_items_list_view', 'requires/wash_recipe_entry_controller', '' ) ;
				setFilterGrid('tbl_list_search',-1);
	...
	function openmypage_sysNo()
	{
		...
		{ 	
			...
			var page_link = 'requires/wash_recipe_entry_controller.php?cbo_company_name='+cbo_company_name+'&action=systemid_popup';
			  
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1210px,height=390px,center=1,resize=1,scrolling=0','../');
			...
			emailwindow.onclose=function()
			{
				...
				if(update_id!="")
				{
					freeze_window(5);
					$('#list_container_recipe_items').html('');
					get_php_form_data(update_id, "populate_data_from_search_popup", "requires/wash_recipe_entry_controller" );
					var sub_process = $('#cbo_sub_process').val();
					
					show_list_view(update_id, 'recipe_item_details', 'recipe_items_list_view', 'requires/wash_recipe_entry_controller', '' ) ;
					setFilterGrid('tbl_list_search',-1);
					load_drop_down('requires/wash_recipe_entry_controller',1, 'load_drop_down_type', 'type_td');
					//reset_form('','','copy_id*copy_sub_process*cbo_sub_process*txt_subprocess_remarks','','');
					set_button_status(0, permission, 'fnc_recipe_entry',1,1);
					$('#copy_id').removeAttr('disabled','disabled');			
					release_freezing();
	...
	{
		//sub_process_id=sub_process_id*1;
	 ...
	 if(subprocessForWashArr.indexOf(sub_process_id) !== -1)  //For Wash/Migration
	  ...
	  else
	  {
		...
		//alert(sub_process_id);
		show_list_view(cbo_company_name+'**'+sub_process_id+"**"+store_id+"**"+update_id+"**"+copy_val+"**"+variable_lot, 'item_details', 'list_container_recipe_items', 'requires/wash_recipe_entry_controller', '');
		setFilterGrid('tbl_list_search',-1);
		//$('#cbo_sub_process').attr('disabled','disabled');
		$('#cbo_company_name').attr('disabled','disabled');
		$('#cbo_store_id').attr('disabled','disabled');
	...
	function openmypage_itemLot(id)
	{
		...
		{ 	
			...
			var page_link = 'requires/wash_recipe_entry_controller.php?cbo_company_name='+cbo_company_name+'&txt_prod_id='+txt_prod_id+'&txt_item_lot='+txt_item_lot+'&action=itemLot_popup';
	...
	function openmypage_batchNo()
	{
		...
		{ 	
			...
			var page_link = 'requires/wash_recipe_entry_controller.php?cbo_company_name='+cbo_company_name+'&action=batch_popup';
			...
			emailwindow.onclose=function()
			{
				...
				if(batch_id!="")
				{
					...
					get_php_form_data(cbo_company_name+'**'+batch_val[0], "load_data_from_batch", "requires/wash_recipe_entry_controller" );
					var process_id_chk = $('#process_id_chk').val();
	...
	{  
		...
		{
			$("#list_container_recipe_items").html('');
			show_list_view($('#update_id').val(), 'recipe_item_details', 'recipe_items_list_view', 'requires/wash_recipe_entry_controller', '' ) ;
			$('#update_id').val('');
			$('#txt_sys_id').val('');
			$('#txt_recipe_date').val('');
			$('#txt_labdip_no').val('');
	...
	function subprocess_change(sub_process)
	{
		...
		get_php_form_data(cbo_company_name+'**'+sub_process+'**'+update_id_check, "ratio_data_from_dtls", "requires/wash_recipe_entry_controller" );
	}
	function fnResetForm()
	{
		//alert(33);
		$("#cbo_company_name").attr("disabled",false); 
		//disable_enable_fields(\'cbo_company_name*cbo_company_name\',0)
	...
	function fnc_load_party(type,within_group)
	{
		...
		{
			load_drop_down( 'requires/wash_recipe_entry_controller', company+'_'+1, 'load_drop_down_buyer', 'buyer_td_id' );
		}
		else if(within_group==2 && type==1)
		{
			load_drop_down( 'requires/wash_recipe_entry_controller', company+'_'+2, 'load_drop_down_buyer', 'buyer_td_id' );
			$('#txt_order_no').removeAttr('onDblClick','onDblClick');
		}
		else if(within_group==1 && type==2)
		{
			load_drop_down( 'requires/wash_recipe_entry_controller', party_name+'_'+2, 'load_drop_down_location', 'party_location_td' ); 
		} 
	}
	...
	function location_select()
	{
		...
		load_drop_down( 'requires/wash_recipe_entry_controller', document.getElementById('cbo_location').value+'_'+document.getElementById('cbo_company_name').value, 'load_drop_down_floor', 'floor_td');	
	}
			...
			<fieldset style="width:920px;">
				<table width="910" align="center" border="0">
                	...
                	<tr>
                        <td>Labdip No</td>
                        <td>
                            <input type="text" name="txt_labdip_no" id="txt_labdip_no" class="text_boxes" style="width:150px;" />
                        </td>
                        ...
                        <td>
                            <input type="text" name="txt_batch_no" id="txt_batch_no" class="text_boxes" style="width:70px;" placeholder="Double click to search" onDblClick="openmypage_batchNo();"  readonly/><input type="text" name="txt_ext_no" id="txt_ext_no" class="text_boxes_numeric" style="width:58px;"  placeholder="Ext. No." /><input type="hidden" name="txt_batch_weight" id="txt_batch_weight" class="text_boxes_numeric" style="width:58px;"  /><input type="hidden" name="txt_batch_id" id="txt_batch_id" style="width:40px;" />
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <?
                               // echo create_drop_down( "cbo_company_name", 152, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "--Select Company--", 0, "load_drop_down('requires/wash_recipe_entry_controller', this.value, 'load_drop_down_location', 'location_td' );") );

								echo create_drop_down( "cbo_company_name", 160, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/wash_recipe_entry_controller', this.value, 'load_drop_down_location', 'location_td'); location_select(); fnc_load_party(1,document.getElementById('cbo_within_group').value); load_drop_down('requires/wash_recipe_entry_controller', this.value+'__'+document.getElementById('cbo_location').value, 'load_drop_down_store', 'store_td');get_php_form_data( this.value, 'company_wise_report_button_setting','requires/wash_recipe_entry_controller');");
                            ?>
                        ...
                        <td >Floor/Unit</td>
						<td id="floor_td"><? echo create_drop_down( "cbo_floor_name", 150, $blank_array,"", 1, "-- Select Floor --", $selected, "" ); ?></td>
                        
                    </tr>
                    <tr>
                    	<td class="must_entry_caption">Recipe Date</td>
                        <td>
                        	<input type="text" name="txt_recipe_date" id="txt_recipe_date" class="datepicker" style="width:150px;" tabindex="6" />
                        </td>
                        <td>Within Group</td>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" name="txt_color" id="txt_color" class="text_boxes" value="" style="width:150px;" tabindex="10" disabled />
                    ...
                    <tr>
                    	<td>Order No.</td>
                        <td>
                            <input type="text" name="txt_order" id="txt_order" class="text_boxes" value="" style="width:150px;" disabled />
                        ...
                        <td>Gmts Item</td>
                        <td>
                            <?php echo create_drop_down('cbo_gmts_item', 150, $garments_item, '', 1, '-- Select --', '', '', 1, ''); ?>
                        </td>
                   ...
                   <tr>
                   		<td>Remarks</td>
                        <td>
                            <input type="text" name="txt_remarks" id="txt_remarks" class="text_boxes" value="" style="width:150px;" />
                        </td>
                   </tr>
             </table>
			</fieldset>                 
            ...
            <legend>Item Details</legend>
 				<table cellpadding="0" cellspacing="0" border="0" width="100%">
 					...
 					<tr>
                        ...
                        <td width="80" id="type_td">
							<?
                                echo create_drop_down( "cbo_sub_process", 80, $blank_array,"", 1, "-- Select Wash Type --", 0, "","","","","",'70');
                            ?>
                         </td>
                        ...
                        <td>
                        	<input type="text" name="txt_total_liquor_ratio" id="txt_total_liquor_ratio" class="text_boxes_numeric" value="" style="width:80px;"  />
                        </td>
            ...
            <table width="910">
            	<tr>
                    <td colspan="4" align="center" class="button_container">
						<? //reset_form('recipeEntry_1','list_container_recipe_items*recipe_items_list_view','','copy_id*2','disable_enable_fields(\'cbo_company_name*cbo_company_name\',0)');$('#cbo_company_name').attr('disabled','false');
