...
if ($action == "batch_popup") 
{
    ...
    <script>
		...
		function search_by(val)
		{
			...
			else if(val==2) $('#search_by_td').html('Job No');
			else if(val==3) $('#search_by_td').html('Style Ref.');
		}
    </script>
    </head>
    <body>
        ...
        <fieldset style="width:750px;margin-left:0px;">
            <form name="searchorderfrm_1" id="searchorderfrm_1" autocomplete="off">
                <table cellpadding="0" cellspacing="0" border="1" rules="all" width="750" class="rpt_table">
                    ...
                    <tr class="general">
                        <td><?  $search_by_arr=array(1=>"Batch No",2=>"Job No",3=>"Style Ref.");
...
if ($action == "create_batch_search_list_view") 
{
	...
	if ($search_type == 1) 
	{
		if($search_common!="")
		{
			...
			else if($search_by==2) $search_com_cond="and c.job_no_prefix_num='$search_common'";
			else if ($search_by==3) $search_com_cond=" and c.style_ref_no='$search_common' ";
		}
	} 
	else if ($search_type == 4 || $search_type == 0) 
	{
		if($search_common!="")
		{
			...
			else if($search_by==2) $search_com_cond="and c.job_no_prefix_num like '%$search_common%'";
			else if ($search_by==3) $search_com_cond=" and c.style_ref_no like '%$search_common%'";
		}
	} 
	else if ($search_type == 2) 
	{
	...
	//$po_sql ="Select style_ref_no, job_no, buyer_name from wo_po_details_master where status_active=1";
	$po_sql_res=sql_select($po_sql);
	foreach ($po_sql_res as $row)
	{
		$buyer_po_arr[$row[csf("id")]]['style']=$row[csf("style_ref_no")];
		...
		$buyer_po_arr[$row[csf("id")]]['buyer_name']=$buyer_arr[$row[csf("buyer_name")]];
	}
	unset($po_sql_res);*/
	
	$production_qty_sql="select a.color_id,a.operation_type, b.id, b.po_id, b.prod_id,b.roll_no as batch_qnty,b.buyer_po_id,d.qcpass_qty as production_qty,d.rewash_qty,c.recipe_id from pro_batch_create_mst a, pro_batch_create_dtls b,subcon_embel_production_mst c,subcon_embel_production_dtls d  where  a.entry_form=316 and  a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  and d.status_active=1 and d.is_deleted=0 and a.id=c.recipe_id and c.id=d.mst_id and a.company_id=$company_id $batch_cond  order by b.id"; // and a.id= $batch_id  
	...
	else if($db_type==2) $poid_cond="listagg(b.po_id,',') within group (order by b.po_id)";
	
	$sql = "select a.id, a.batch_no, a.extention_no, a.operation_type, a.company_id, a.batch_weight, a.batch_date, a.color_id, a.entry_form, sum(b.qty_pcs) as qty_pcs, $poid_cond as po_id, c.style_ref_no, c.job_no, c.buyer_name from pro_bundle_batch_mst a, pro_bundle_batch_dtls b, wo_po_details_master c, wo_po_break_down d where a.id=b.mst_id and c.job_no=d.job_no_mst and d.id=b.po_id and a.working_company=$company_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form in (389) $batch_cond $date_cond $search_com_cond and a.id not in(select poid from tmp_poid where userid=$user_id) group by a.id, a.batch_no, a.extention_no, a.company_id, a.batch_weight, a.batch_date, a.color_id, a.entry_form, a.operation_type, c.style_ref_no, c.job_no, c.buyer_name order by a.id DESC";
	//echo $sql;
            ...
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="682" class="rpt_table" id="list_view">
				...
				foreach ($nameArray as $row) 
				{
					...
					$order_id = array_unique(explode(",", $row[csf('po_id')]));
 					$styleRef=$jobNo=$buyerName="";
					/*foreach ($order_id as $idpo)
					{
						if($jobNo=="") $jobNo=$buyer_po_arr[$idpo]['job_no']; else $jobNo.= ",".$buyer_po_arr[$idpo]['job_no'];
						if($styleRef=="") $styleRef=$buyer_po_arr[$idpo]['style']; else $styleRef.= ",".$buyer_po_arr[$idpo]['style'];
						if($buyerName=="") $buyerName=$buyer_po_arr[$idpo]['buyer_name']; else $buyerName.= ",".$buyer_po_arr[$idpo]['buyer_name'];
						
						//$qty+=$order_arr[$idpo]['qty'];
					}	
					
					$jobNo=implode(", ",array_unique(explode(",",$jobNo)));
					$styleRef=implode(", ",array_unique(explode(",",$styleRef)));
					$buyerName=implode(", ",array_unique(explode(",",$buyerName)));	*/
					...
					$str=$row[csf('id')].'___'.$row[csf('batch_no')].'___'.$jobNo.'___'.$styleRef.'___'.$buyerName.'___'.$row[csf('company_id')].'___'.$row[csf('operation_type')];
					
					if($batch_balance_qty>0)
					{
						?>
						<tr bgcolor="<?=$bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<?=$i; ?>" onClick="js_set_value('<?=$str; ?>')">
							<td width="30"><?=$i; ?></td>
							<td width="70" style="word-break:break-all;"><?=$row[csf('batch_no')]; ?></td>
							<td width="40"><?=$row[csf('extention_no')]; ?>&nbsp;</td>
							<td width="80" style="word-break:break-all;"><?=$wash_operation_arr[$row[csf('operation_type')]]; ?>&nbsp;</td>
							<td width="90" style="word-break:break-all;"><?=$color_arr[$row[csf('color_id')]]; ?></td>
							<td width="80" align="right"><?=$row[csf('batch_weight')]; ?></td>
							<td width="80" align="right">&nbsp;<?=$row[csf('qty_pcs')]; ?></td>
							<td width="70" align="center"><?=change_date_format($row[csf('batch_date')]); ?></td>
...
if($action=="order_details")
{
	...
	$buyer_po_arr=array();
	$po_sql ="Select a.style_ref_no, a.buyer_name, b.id, b.po_number from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.job_no='$jobno'";
	$po_sql_res=sql_select($po_sql);
	foreach ($po_sql_res as $row)
	{
		$buyer_po_arr[$row[csf("id")]]['style']=$row[csf("style_ref_no")];
	...
	//echo "<pre>";
	//print_r($data_arr);
	$sql = "select a.id, a.batch_no, a.process_id, a.extention_no, a.operation_type, a.batch_weight, a.batch_date, a.color_id, sum(b.qty_pcs) as qty_pcs, b.po_id, sum(b.qty_gm) as qty_gm, c.job_no_mst, c.id as po_id, c.po_number, d.id as color_size_id, d.country_id, d.item_number_id, d.color_number_id, d.size_number_id, d.plan_cut_qnty
	from pro_bundle_batch_mst a, pro_bundle_batch_dtls b, wo_po_break_down c, wo_po_color_size_breakdown d where a.id=b.mst_id and b.po_id=c.id and c.id=d.po_break_down_id and c.job_no_mst=d.job_no_mst and d.id=b.colorsizeid and a.id=$batch_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id, a.batch_no, a.process_id, a.extention_no, a.operation_type, a.batch_weight, a.batch_date, a.color_id, b.po_id, c.job_no_mst, c.id, c.po_number, d.id, d.country_id, d.item_number_id, d.color_number_id, d.size_number_id, d.plan_cut_qnty";
	//echo $sql; die;
	$prod_data_arr=sql_select($sql);
	
	$production_qty_sql="select a.color_id,a.operation_type, b.id, b.po_id, b.prod_id,b.roll_no as batch_qnty,b.buyer_po_id,d.qcpass_qty as production_qty from pro_batch_create_mst a, pro_batch_create_dtls b,subcon_embel_production_mst c,subcon_embel_production_dtls d  where  a.entry_form=393 and  a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  and d.status_active=1 and d.is_deleted=0 and a.id=c.recipe_id and c.id=d.mst_id  and a.id= $batch_id   order by b.id";
	$production_data_array=sql_select($production_qty_sql); 
	$production_type_array=array();
	foreach($production_data_array as $row)
	{
	...
	foreach($prod_data_arr as $row)
	{
		...
		if($update_id!=0) $production_qty=$total_production_qty-$qcpass_qty;
		else $production_qty=$total_production_qty;
		
		//echo $data_arr[$row[csf('color_size_id')]]['qcpass_qty']."=="; die;
		?>
		<tr bgcolor="<?=$bgcolor; ?>" name="tr[]" id="tr_<?=$i;?>">
			<td align="center"><?=$i; ?></td>
            <td style="word-break:break-all"><?=$row[csf("po_number")]; ?>&nbsp;</td>
            <td style="word-break:break-all"><?=$country_arr[$row[csf("country_id")]]; ?>&nbsp;</td>
            <td style="word-break:break-all"><?=$garments_item[$row[csf('item_number_id')]]; ?>&nbsp;</td>
            <td style="word-break:break-all"><?=$color_arr[$row[csf('color_number_id')]]; ?>&nbsp;</td>
            <td style="word-break:break-all"><?=$size_arrey[$row[csf('size_number_id')]]; ?>&nbsp;</td>
            <td style="word-break:break-all"><? //=$process_name; ?>&nbsp;</td>
            <td style="word-break:break-all" align="right"><?=$row[csf("qty_pcs")]; ?>&nbsp;</td>
            <td style="word-break:break-all" align="right"><?=$production_qty; ?>&nbsp;
             <input type="hidden" name="txtPrevProdQty[]" id="txtPrevProdQty_<?=$i;?>" class="text_boxes_numeric" style="width:80px" value="<?=$production_qty; ?>"  />
             <input type="hidden" name="txtbatchQty[]" id="txtbatchQty_<?=$i;?>" class="text_boxes_numeric" style="width:80px" value="<?=$row[csf("qty_pcs")]; ?>"  />
            </td>
			<td align="right"><input type="text" name="txtProdQty[]" id="txtProdQty_<?=$i;?>" class="text_boxes_numeric" style="width:80px" placeholder="Write" value="<?=$qcpass_qty; ?>" onBlur="fnc_total_calculate();" onKeyUp="fnc_production_qty_validation();" /></td>
            <td><input type="text" name="txtRejQty[]" id="txtRejQty_<?=$i;?>" class="text_boxes_numeric" style="width:60px" placeholder="Write" value="<?=$rej_qty; ?>" onBlur="fnc_total_calculate();" /></td>
            <td align="right"><input type="text" name="txtReWashQty[]" id="txtReWashQty_<?=$i;?>" class="text_boxes_numeric" style="width:60px" placeholder="Write" value="<?=$rewash_qty; ?>" onBlur="fnc_total_calculate();" /></td>
            <td>
            	<input type="text" name="txtRemarks[]" id="txtRemarks_<?=$i;?>" class="text_boxes" style="width:50px" placeholder="Write" value="<?=$remarks; ?>" />
                <input type="hidden" name="updateIdDtls[]" id="updateIdDtls_<?=$i;?>" style="width:50px" value="<?=$upid; ?>" />
                <input type="hidden" name="txtPoId[]" id="txtPoId_<?=$i;?>" style="width:50px" value="<?=$row[csf('po_id')]; ?>" />
				<input type="hidden" name="colorSizeId[]" id="colorSizeId_<?=$i;?>" style="width:50px" value="<?=$row[csf('color_size_id')]; ?>" />
            </td>
		</tr>
		<?
		$i++;
                    ...
                    <table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                ...
                                <td><input type="text" name="txt_search_string" id="txt_search_string" class="text_boxes" style="width:90px" placeholder="" /></td>
...
if($action=="create_production_no_list_view")
{
	...
	$company_id=$data[0];
	$location_id=$data[1];
	$prod_no=str_replace("'","",$data[2]);
	$recipe_no=str_replace("'","",$data[3]);
	$date_from=$data[4];
	$date_to=$data[5];
	$search_type=$data[6];
	...
	$job_cond=""; $style_cond=""; $po_cond=""; $search_com_cond=""; $batch_cond="";
	//$entry_form_cond='';
	...
	if($search_type==1)
	{
		if($search_str!="")
		{
			...
			else if ($search_by==6) $batch_cond="and a.batch_no = '$search_str'";
		}
	}
	else if($search_type==4 || $search_type==0)
	{
		if($search_str!="")
		{
			...
			else if ($search_by==6) $batch_cond="and a.batch_no like '%$search_str%'"; 
		}
	}
	else if($search_type==2)
	{
	...
	if($batch_cond!="" && $search_by==6)
	{
		$batch_sql ="Select a.id as batch_id, a.batch_no from pro_bundle_batch_mst a where 1=1 $batch_cond";
		...
		if(count($batch_ids)<1) { echo "Batch Not Found."; disconnect($con); die; }
		...
		if($db_type==2 && count($batch_ids)>=999)
		{
			...
			foreach($chunk_arr as $val)
			{
				...
				if($batchid_cond=="")
				{
					$batchid_cond.=" and ( a.recipe_id in ( $ids) ";
				}
				else
				{
					$batchid_cond.=" or a.recipe_id in ( $ids) ";
				}
			}
			$batchid_cond.=")";
		}
	...
	if($db_type==0) $id_cond="group_concat(b.id)"; else if($db_type==2) $id_cond="listagg(b.id,',') within group (order by b.id)";
	...
	if(trim($prod_no)!="") $system_no_cond=" and a.sys_no like '%".trim($prod_no)."'"; else $system_no_cond="";
	if($recipe_no!="") $recipe_no_cond=" and a.sys_no like '%".trim($recipe_no)."%'"; else $recipe_no_cond="";
	
	//$batch_arr = return_library_array("select id, batch_no from pro_batch_create_mst", 'id', 'batch_no');
	$batchData=sql_select("select id, batch_no, operation_type, sub_operation from pro_bundle_batch_mst where status_active=1 and is_deleted=0 ");
	...
	$po_sql ="Select style_ref_no,buyer_name, job_no from wo_po_details_master where status_active = 1";
	$po_sql_res=sql_select($po_sql);
	foreach ($po_sql_res as $row)
	{
		$buyer_po_arr[$row[csf("job_no")]]['style']=$row[csf("style_ref_no")];
		//$buyer_po_arr[$row[csf("id")]]['po']=$row[csf("po_number")];
		$buyer_po_arr[$row[csf("job_no")]]['buyer']=$buyerArr[$row[csf("buyer_name")]];
	}
	unset($po_sql_res);
	?>
	<body>
		<div align="center">
			<fieldset style="width:780px;margin-left:10px">
				<form name="searchprocessfrm_1"  id="searchprocessfrm_1" autocomplete="off">
					...
					<div style="width:780px; overflow-y:scroll; max-height:300px;">
						<table cellspacing="0" cellpadding="0" border="1" rules="all" width="760" class="rpt_table" id="tbl_list_search" >
							<?
							 $sql="SELECT a.id, a.prefix_no_num, a.sys_no, a.location_id, a.serv_company, a.recipe_id, a.job_no, b.po_id as order_id, a.floor_id, a.machine_id, a.product_date, b.production_date, b.qcpass_qty from subcon_embel_production_mst a, subcon_embel_production_dtls b where a.id=b.mst_id and a.entry_form=393 and a.status_active = 1 and a.is_deleted = 0 and a.company_id='$company_id' $batchid_cond $location_cond $system_no_cond $recipe_no_cond $po_idsCond $date_cond order by a.id DESC";// $date_cond
							//echo $sql; die;
							$sql_res=sql_select($sql);
							...
							foreach($sql_res as $row)
							{
								...
								$str_data="";
								$str_data=$row[csf('id')].'***'.$row[csf('sys_no')].'***'.$row[csf('location_id')].'***'.$row[csf('recipe_id')].'***'.$batch_no.'***'.$row[csf('job_no')].'***'.$row[csf('serv_company')].'***'.$buyer_po_arr[$row[csf("job_no")]]['style'].'***'.$buyer_po_arr[$row[csf("job_no")]]['buyer'].'***'.$prod_data_arr[$row[csf('id')]]['production_date'].'***'.$prod_data_arr[$row[csf('id')]]['production_hour'].'***'.$prod_data_arr[$row[csf('id')]]['operator_name'].'***'.$prod_data_arr[$row[csf('id')]]['shift_id'].'***'.$row[csf('floor_id')].'***'.$row[csf('machine_id')].'***'.$operation_type.'***'.$sub_operation;
								
								?>
								<tr bgcolor="<?=$bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<?=$i;?>" onClick="js_set_value('<?=$str_data; ?>');"> 
									<td width="30" align="center"><?=$i; ?></td>	
									<td width="120" align="center"><?=$row[csf('prefix_no_num')]; ?></td>
                                    <td width="70"><?=change_date_format($row[csf('production_date')]); ?>&nbsp;</td>
                                    <td width="100"><?=$batch_no; ?></td>
                                    <td width="100"><?=$row[csf('qcpass_qty')]; ?></td>
                                    <td width="120"><?=$row[csf('job_no')]; ?></td>
                                    <td width="110" style="word-break:break-all"><?=$buyer_po_arr[$row[csf("job_no")]]['buyer']; ?></td>
                                    <td style="word-break:break-all"><?=$buyer_po_arr[$row[csf("job_no")]]['style']; ?>&nbsp;</td>
								</tr>
								<?
								$i++;
							}
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   // Insert Here==============================================================
	{
		...
		$new_return_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '', 'SWP', date("Y",time()), 5, "select id,prefix_no,prefix_no_num from  subcon_embel_production_mst where company_id=$cbo_company_id and entry_form=393 $year_cond=".date('Y',time())." order by id desc ", "prefix_no", "prefix_no_num" ));
		
		$id=return_next_id( "id", "subcon_embel_production_mst", 1) ; 
		$field_array="id, prefix_no, prefix_no_num, sys_no, company_id, location_id, serv_company, recipe_id, job_no, floor_id, machine_id, inserted_by, insert_date, status_active, is_deleted, entry_form";
		$data_array="(".$id.",'".$new_return_no[1]."','".$new_return_no[2]."','".$new_return_no[0]."',".$cbo_company_id.",".$cbo_location.",".$cbo_buyer_name.",".$txt_batch_id.",".$txt_job_no.",".$cbo_floor_id.",".$cbo_machine_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,393)";
		
		$production_no=$new_return_no[0];
		
		$data_array_dtls=""; $issave=1;
		$id_dtls=return_next_id( "id", "subcon_embel_production_dtls", 1 ) ;
		$field_array_dtls="id, mst_id, color_size_id, po_id, job_no, production_date, production_hour, qcpass_qty, reje_qty, rewash_qty, operator_name, shift_id, remarks, inserted_by, insert_date, status_active, is_deleted";
		//echo "10**"; //die; 
		for($i=1;$i<=$total_row;$i++)
		{
			...
			if($db_type==0)
			{
				...
				$data_array_dtls.="(".$id_dtls.",".$id.",'".$$colorSizeId."','".$$txtPoId."',".$txt_job_no.",".$txt_prod_date.",".$txt_reporting_hour.",'".$$txtProdQty."','".$$txtRejQty."','".$$txtReWashQty."',".$txt_super_visor.",".$cboShift.",'".$$txtRemarks."','".$_SESSION['logic_erp']['user_id']."','".$pc_date_time."',1,0)"; 
			...
			else
			{
				...
				if($data_array_dtls!="") $data_array_dtls.=",";
				$data_array_dtls.="INSERT INTO subcon_embel_production_dtls (".$field_array_dtls.") VALUES (".$id_dtls.",".$id.",'".$$colorSizeId."','".$$txtPoId."',".$txt_job_no.",".$txt_prod_date.",".$txtreporting_hour.",'".$$txtProdQty."','".$$txtRejQty."','".$$txtReWashQty."',".$txt_super_visor.",".$cboShift.",'".$$txtRemarks."','".$_SESSION['logic_erp']['user_id']."','".$pc_date_time."',1,0)"; 
				$id_dtls=$id_dtls+1;
	...
	else if ($operation==1)   // Update Here============================================================
	{
		...
		$qc_no=return_field_value( "sys_no", "subcon_embel_production_mst"," recipe_id=$update_id and status_active=1 and is_deleted=0 and entry_form=393");
		if($qc_no){
		...
		$field_array_update="location_id*recipe_id*job_no*floor_id*machine_id*updated_by*update_date";
		$data_array_update="".$cbo_location."*".$txt_batch_id."*".$txt_job_no."*".$cbo_floor_id."*".$cbo_machine_id."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"; 		
		
		$data_array_dtls_update="";
		$field_array_dtls_update="color_size_id*po_id*job_no*production_date*production_hour*qcpass_qty*reje_qty*rewash_qty*operator_name*shift_id*remarks*updated_by*update_date";
		...
		for($i=1;$i<=$total_row;$i++)
		{
			...
			if( $updateIds != "")
			{
				...
				$data_array_dtls_update[$updateIds] = explode("*",("'".$$colorSizeId."'*".$$txtPoId."*".$txt_job_no."*".$txt_prod_date."*".$txtreportingHour."*'".$$txtProdQty."'*'".$$txtRejQty."'*'".$$txtReWashQty."'*".$txt_super_visor."*".$cboShift."*'".$$txtRemarks."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
				$issave=1;
				...
				{
					if($data_array_dtls!="") $data_array_dtls.=",";
					$data_array_dtls.="(".$id_dtls.",".$update_id.",'".$$colorSizeId."',".$$txtPoId.",".$txt_job_no.",".$txt_prod_date.",".$txtreportingHour.",'".$$txtProdQty."','".$$txtRejQty."','".$$txtReWashQty."',".$txt_super_visor.",".$cboShift.",'".$cboShift."','".$$txtRemarks."','".$_SESSION['logic_erp']['user_id']."','".$pc_date_time."',1,0)"; 
				...
				else
				{
					...
					if($data_array_dtls!="") $data_array_dtls.=",";
					$data_array_dtls.="INSERT INTO subcon_embel_production_dtls (".$field_array_dtls.") VALUES (".$id_dtls.",".$update_id.",'".$$colorSizeId."',".$$txtPoId.",".$txt_job_no.",".$txt_prod_date.",".$txtreporting_hour.",'".$$txtProdQty."','".$$txtRejQty."','".$$txtReWashQty."',".$txt_super_visor.",".$cboShift.",'".$$txtRemarks."','".$_SESSION['logic_erp']['user_id']."','".$pc_date_time."',1,0)"; 
					$id_dtls=$id_dtls+1;
					$id_arr_pro[]=$id_dtls;
					if($issave==1)
					{
		...
		//echo "10**".$data_array_dtls;disconnect($con); die;
		$flag=1;
		if($issave==1)
		{
