...
<script>
	...
	function fnc_job_no()
	{
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				if(ex_data[4]==6){var search_type=ex_data[4];}else{var search_type=0;}
				if(ex_data[5]!=''){var delivery_id=ex_data[5];}else{var delivery_id=0;}
				//$('#txt_delivery_id').val(delivery_id);
				if(delivery_id!="")
				{
				 	$('#txt_delivery_id').val(delivery_id);
				}
				var list_view_orders = return_global_ajax_value( 0+'**'+ex_data[0]+'**'+1+'**'+exchange_rate+'**'+search_type+'**'+delivery_id, 'load_php_dtls_form', '', 'requires/wash_bill_issue_controller');
				if(list_view_orders!='')
				{
	...
	function fnc_embl_bill_issue( operation )
	{
		...
		if ( form_validation('cbo_company_name*cbo_party_name*txt_bill_date*txtJob_no*cbo_currency*txt_exchange_rate', 'Company Name*Party*Bill Date*Job No*currency*exchange rate')==false )
		{
			return;
		}
		else
		{
			if(operation==4)
			{
				...
				print_report( $('#cbo_company_name').val()+'*'+$('#txt_update_id').val()+'*'+report_title+'*'+$('#txtJob_no').val(), "embl_bill_issue_print", "requires/wash_bill_issue_controller") 
				//return;
				show_msg("3");
			}
			else
			{
				...
				var data_str=get_submitted_data_string('txt_bill_no*cbo_company_name*cbo_location_name*cbo_within_group*cbo_party_name*cbo_party_location*txt_bill_date*txt_remarks*txtJob_no*txt_update_id*is_posted_account*txt_exchange_rate*cbo_currency*txt_delivery_id',"../../");
	...
	function fnc_embl_bill_issue_response()
	{
		if(http.readyState == 4) 
		{
			...
			if(response[0]==0 || response[0]==1)
			{
				...
				var list_view_orders = return_global_ajax_value( response[1]+'**'+$('#txtJob_no').val()+'**'+2+'**'+exchange_rate+'**'+search_type+'**'+delivery_id, 'load_php_dtls_form', '', 'requires/wash_bill_issue_controller');
				if(list_view_orders!='')
				{
					$("#dtls_tbody tr").remove();
					$("#dtls_tbody").append(list_view_orders);
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				if (bill_data[0]!="")
				{
					...
					var list_view_orders = return_global_ajax_value( bill_data[0]+'**'+bill_data[8]+'**'+1+'**'+exchange_rate+'**'+search_type+'**'+delivery_id, 'load_php_dtls_form', '', 'requires/wash_bill_issue_controller');
					if(list_view_orders!='')
					{
						$("#dtls_tbody tr").remove();
						$("#dtls_tbody").append(list_view_orders);
                ...
                <table width="990" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                    ...
                    <tr>
                        ...
                        <td class="must_entry_caption">Exchange Rate</td>
               			 <td><input type="text" name="txt_exchange_rate" id="txt_exchange_rate" style="width:130px" class="text_boxes_numeric" onBlur="fnc_domestic_calculation(this.value); fnc_total_calculate();" /></td>
                    </tr>
                    <tr>
                        <td>Remarks</td>
