	...
	<body onLoad="fnc_load_party_order_popup(<? echo $data[0];?>,<? echo $data[3];?>,<? echo $data[2];?>)">
                ...
                <table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            ...
                            <td>
								<?
									$search_by_arr=array(1=>"Wash Job No",2=>"W/O No",3=>"Buyer Job",4=>"Buyer Po",5=>"Buyer Style",6=>" Delivery No ");
        ...
        <script>
        	...
        	function showList() {
        		...
        		show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_party_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_type').value+'_'+document.getElementById('txt_search_string').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+<? echo $data[3];?>, 'create_job_search_list_view', 'search_div', 'wash_bill_issue_controller', 'setFilterGrid(\'tbl_po_list\',-1)');
        		isValidated = false;
        	}
        </script>
	</body>
...
{	
	...
	else if($db_type==2)
	{
	 ...
	 $sql= "select a.id, a.subcon_job, a.job_no_prefix_num, $ins_year_cond as year, a.company_id, a.location_id, a.party_id, a.receive_date, a.order_no, a.delivery_date, $color_id_str as color_id, $buyer_po_id_cond as buyer_po_id ,b.buyer_po_no,b.buyer_style_ref ,b.party_buyer_name
	 ...
	 group by a.id, a.subcon_job, a.job_no_prefix_num, a.insert_date, a.company_id, a.location_id, a.party_id, a.receive_date, a.order_no, a.delivery_date,b.buyer_po_no,b.buyer_style_ref,b.party_buyer_name 
	 order by a.id DESC";
     ...
     <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="885" >
        ...
        <tbody>
            ...
            {  
                ...
                ?>
                <tr bgcolor="<? echo $bgcolor; ?>" onClick="js_set_value('<? echo $row[csf('subcon_job')].'***'.$row[csf('order_no')].'***'.$buyer_style.'***'.$buyer_name.'***'.$search_by.'***'.$search_str; ?>')" style="cursor:pointer" >
	...
	foreach ($sql_result as $row)
	{
		...
		else
		{
			...
			if($row[csf("order_uom")]==2)
			{
				...
				$Rate=number_format($Rateformat,4);
			}
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$txt_update_id)=="")
		{
			...
			$field_array="id, prefix_no, prefix_no_num, bill_no, company_id, location_id, party_source, party_id, party_location_id, bill_date, process_id, remarks,currency,exchange_rate,delivery_id, inserted_by, insert_date, status_active, is_deleted";
			$data_array="(".$id.",'".$new_bill_no[1]."','".$new_bill_no[2]."','".$new_bill_no[0]."',".$cbo_company_name.",".$cbo_location_name.",".$cbo_within_group.",".$cbo_party_name.",".$cbo_party_location.",".$txt_bill_date.",'".$bill_process_id."',".$txt_remarks.",".$cbo_currency.",".$txt_exchange_rate.",".$txt_delivery_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)"; 
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");  
				echo "0**".str_replace("'","",$id)."**".$return_no."**".str_replace("'", '',$txt_exchange_rate)."**".str_replace("'", '',$txt_delivery_id);
			}
			else
			{
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$field_array="location_id*party_source*party_id*party_location_id*bill_date*remarks*currency*exchange_rate*updated_by*update_date";
		$data_array="".$cbo_location_name."*".$cbo_within_group."*".$cbo_party_name."*".$cbo_party_location."*".$txt_bill_date."*".$txt_remarks."*".$cbo_currency."*".$txt_exchange_rate."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");  
				echo "1**".str_replace("'","",$txt_update_id)."**".$return_no."**".str_replace("'", '',$txt_exchange_rate)."**".str_replace("'", '',$txt_delivery_id);
			}
			else
			{
				mysql_query("ROLLBACK"); 
...
if($action=="create_bill_search_list_view")
{
	...
	if ( $spo_ids!="") $orderidsCond=" and b.id in ($spo_ids)"; else $orderidsCond="";
	 ...
	 $sql= "select a.id, a.bill_no, a.prefix_no_num, $insert_date_cond as year, a.location_id, a.bill_date, a.party_id, a.party_source, a.party_location_id, a.remarks, $order_id_cond as order_id, $buyer_po_id_cond as buyer_po_id, $delivery_id_cond as delivery_id,a.is_posted_account,a.currency, a.exchange_rate,a.delivery_id as type_delivery_id   from subcon_inbound_bill_mst a, subcon_inbound_bill_dtls b where a.id=b.mst_id and a.status_active=1 and a.process_id=304 $company $buyer_cond $withinGroup $bill_date $bill_id_cond $spo_idsCond $po_idsCond  group by a.id, a.bill_no, a.prefix_no_num, a.insert_date, a.location_id, a.bill_date, a.party_id, a.party_source, a.party_location_id, a.remarks,a.is_posted_account,a.currency, a.exchange_rate,a.delivery_id order by a.id DESC";
	//echo $sql;
...
if($action=="embl_bill_issue_print")
{
	...
	$sql_mst="select id, prefix_no_num, bill_no, location_id, bill_date, party_id, party_source, process_id, party_location_id, remarks, currency, exchange_rate
    ...
    <div style="width:930px;">
		...
		group by a.id,a.company_id, a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id, b.buyer_po_id, b.gmts_item_id, b.order_uom, b.gmts_color_id, b.order_quantity, d.delivery_prefix_num,d.id, d.delivery_date, e.id , e.delivery_qty, f.id, f.rate, f.amount,f.domestic_amount, f.remarks,b.buyer_po_no,b.buyer_style_ref
		)
		union all
		(
			select a.id,a.company_id, a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id as po_id, b.buyer_po_id, b.gmts_item_id, $process_type_cond as process_type, b.order_uom, b.gmts_color_id as color_id, b.order_quantity as qnty, d.delivery_prefix_num,d.id as delivery_mst_id, d.delivery_date, e.id as delivery_id, e.delivery_qty, 0 as upid, 0 as rate, 0 as amount,0 as domestic_amount, null as remarks,b.buyer_po_no,b.buyer_style_ref
		from subcon_ord_mst a, subcon_ord_dtls b, subcon_ord_breakdown c, subcon_delivery_mst d, subcon_delivery_dtls e
		where a.entry_form=295 and a.subcon_job=b.job_no_mst and a.id=b.mst_id and d.id=e.mst_id and b.id=e.order_id and e.bill_status!=1 and d.entry_form=303 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and b.id=c.mst_id and a.subcon_job='$job_no'
