...
<script>
	...
	var permission='<? echo $permission; ?>';
	 ...
	 <?
 if ($_SESSION['logic_erp']['data_arr'][303]){
	$data_arr= json_encode( $_SESSION['logic_erp']['data_arr'][303] );
 ...
 ?>
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				var ex_data=theemail.split('***')
 				//var StyleRef = ex_data[2].replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, '');
	...
	function fnc_embl_delivery( operation )
	{
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_name').val()+'*'+$('#txt_update_id').val()+'*'+$("#txtJob_no").val()+'*'+report_title+'*'+$("#cbo_template_id").val(), "embl_delivery_entry_print", "requires/wash_delivery_controller") 
			//return;
			show_msg("3");
		}
		else if(operation==5)
		{
			...
			print_report( $('#cbo_company_name').val()+'*'+$('#txt_update_id').val()+'*'+$("#txtJob_no").val()+'*'+report_title+'*'+$("#cbo_template_id").val(), "embl_delivery_entry_print2", "requires/wash_delivery_controller") 
			//return;
			show_msg("3");
		}		
		...
		else
		{
			...
			if(within_group==1)
			{
				if ( form_validation('cbo_company_name*cbo_location_name*cbo_party_name*cbo_deli_party_name*cbo_deli_party_location*txt_delivery_date*txtJob_no', 'Company Name*Location*Party*Delivery Party*Del.Party Location*Delivery Date*Job No')==false )
				{
					return;
				}
			}
			...
			{
				if ( form_validation('cbo_company_name*cbo_location_name*cbo_party_name*txt_delivery_date*txtJob_no', 'Company Name*Location*Party*Delivery Date*Job No')==false )
				{
					return;
				}
			}		
			...
			var data_str=get_submitted_data_string('txt_delv_no*cbo_company_name*cbo_location_name*cbo_floor_name*cbo_party_name*cbo_party_location*cbo_deli_party_name*cbo_deli_party_location*txt_delivery_date*cbo_within_group*txt_remarks*txtJob_no*txt_update_id*txt_update_details_id*cboshipingStatus*txt_variable_status',"../../");
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				if (emb_data[0]!="")
				{
					...
					load_drop_down( 'requires/wash_delivery_controller', emb_data[2]+'_'+document.getElementById('cbo_company_name').value, 'load_drop_down_floor', 'floor_td');
					$('#cbo_floor_name').val(emb_data[15]);
	...
	function location_select()
	{
		...
		load_drop_down( 'requires/wash_delivery_controller', document.getElementById('cbo_location_name').value+'_'+document.getElementById('cbo_company_name').value, 'load_drop_down_floor', 'floor_td');
	}
 ...
 function val_roundup()
	 {
        if($('#round_down').is(':checked')){
            $( "input[name='txtCurrDelv[]']" ).each(function (index){
			...
			<legend>Wash Delivery</legend>
                <table width="1090" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                    ...
                    <tr>
                        ...
                        <td><? echo create_drop_down( "cbo_deli_party_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "--Select Party--", $selected, "load_drop_down( 'requires/wash_delivery_controller', this.value+'_'+3, 'load_drop_down_location', 'dparty_location_td'); fnc_load_party(3,document.getElementById('cbo_within_group').value);"); ?></td>
                ...
                <table style="width:1100px;" cellpadding="0" cellspacing="2" border="1" class="rpt_table" rules="all">
                    <thead class="form_table_header">
                    	<tr>
                            ...
                            <th><?php   echo create_drop_down( "cboshipingStatus", 50, $shipment_status,"", 0, "-- Select --", 2, "",1,'2,3','','','','' );	  ?></th>
                             <th  class="receiv_id" style="display:none"></th>
                	    </tr>
                	    <tr>
	                        <th width="30">SL</th>
                            <th width="90">Buyer PO</th>
                            <th width="90">Buyer Style Ref.</th>
                    ...
                    <tbody id="dtls_tbody">
                    	<tr bgcolor="#FFFFFF">
                    		...
                    		<td>&nbsp;</td>
                    		<td>&nbsp;</td>
                    		<td align="center"><input type="text" name="txtCurrDelv[]" id="txtCurrDelv_1" class="text_boxes_numeric" style="width:55px;" onBlur="fnc_production_qty_ability(this.value,1); fnc_total_calculate();" /></td>
                    		...
                    		<td align="center"><input type="text" name="txtRemarks_1" id="txtRemarks_1" class="text_boxes" style="width:60px;" />
                                ...
                                <input type="hidden" name="txtorderlastsequence_1" id="txtorderlastsequence_1" style="width:80px" class="text_boxes" value="" />
                 				<input type="hidden" name="txtorderlastprocess_1" id="txtorderlastprocess_1" style="width:80px" class="text_boxes" value="" />
                                <input type="hidden" name="txtorderlastsequencebreakid_1" id="txtorderlastsequencebreakid_1" style="width:80px" class="text_boxes" value="" />
                            </td> 
                    	</tr>
                    </tbody> 
                    <tfoot>
