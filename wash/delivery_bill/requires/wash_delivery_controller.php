...
{	
   ...
   b.buyer_buyer from subcon_ord_mst a, subcon_ord_dtls b where a.subcon_job=b.job_no_mst and a.entry_form=295 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	...
	else if($db_type==2)
	{
		...
		//$buyer_style_cond="listagg(cast(b.buyer_style_ref as varchar2(500)),',') within group (order by b.buyer_style_ref)";
		
		$buyer_po_no_cond="rtrim(xmlagg(xmlelement(e,b.buyer_po_no,',').extract('//text()') order by b.buyer_po_no).GetClobVal(),',')";
		
		$buyer_style_cond="rtrim(xmlagg(xmlelement(e,b.buyer_style_ref,',').extract('//text()') order by b.buyer_style_ref).GetClobVal(),',')";
     ...
     <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="885" >
        ...
        <tbody>
            ...
            {  
				...
				//$buyer_style=implode(",",array_unique(explode(",",trim($row[csf('buyer_style')]))));
				$buyer_style=implode(",",array_unique(explode(",",trim($buyer_style))));
...
if($action=="populate_data_from_data")
{
   ...
   b.party_buyer_name as buyer_buyer from subcon_ord_mst a, subcon_ord_dtls b where a.subcon_job=b.job_no_mst and a.entry_form=295 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	...
	$sql= "select a.id, a.delivery_no, a.delivery_prefix_num, $insert_date_cond as year, a.location_id, a.floor_id,a.within_group, a.party_id, a.party_location, a.deli_party, a.deli_party_location, a.delivery_date, a.job_no, $wo_cond as order_id, $buyer_po_id_cond as buyer_po_id, $delivery_status_cond as delivery_status,b.id as details_id,job_order_status from subcon_delivery_mst a, subcon_delivery_dtls b 
	...
	group by a.id, a.delivery_no, a.delivery_prefix_num, a.insert_date, a.location_id, a.floor_id, a.within_group, a.party_id, a.party_location, a.deli_party, a.deli_party_location, a.delivery_date, a.job_no,b.id,job_order_status order by a.id DESC ";
	//echo $sql; 
	...
	foreach($result as $row)
	{
		...
		//echo "$('#cboshipingStatus').val('".$job_arr[$row[csf("job_order_status")]]."');\n";
...
if($action=="load_php_dtls_form")
{
	...
	if($update_id>0)
	{
		...
		$embellishment_type_result=sql_select( "select id, mst_id, description, process, embellishment_type, rate,prod_sequence_no from subcon_ord_breakdown where job_no_mst in ('$job_number')  and  status_active=1 and  is_deleted=0 order by id");
	}
	else
	{
		...
		$embellishment_type_result=sql_select( "select id, mst_id, description, process, embellishment_type, rate,prod_sequence_no from subcon_ord_breakdown where job_no_mst='$jobno'  and  status_active=1 and  is_deleted=0 order by id");
	}
	...
	if($update_id>0)
	{
		...
		a.entry_form in (316,543)  and b.po_id=c.po_id and c.mst_id=d.id  and  d.job_no='$job_number'  and  d.entry_form=301 and a.id=d.recipe_id and a.is_deleted=0 
	...
	else
	{
		...
		from pro_batch_create_mst a, pro_batch_create_dtls b,subcon_embel_production_dtls c ,subcon_embel_production_mst d 
		where a.id=b.mst_id and a.process_id='1'  and  a.status_active=1 and 
		a.entry_form in (316,543) and b.po_id=c.po_id and c.mst_id=d.id  and  d.job_no='$jobno'  and  d.entry_form=301 and a.id=d.recipe_id and a.is_deleted=0 
		"; //group by a.id,b.po_id,a.operation_type,c.qcpass_qty,a.process_id
	}
	...
	if($update_id>0)
	{
	  $sql_del="select a.id, a.mst_id, a.order_id, a.delivery_qty, a.remarks, a.bill_status, a.sort_qty, a.reject_qty,b.job_order_status from subcon_delivery_dtls a, subcon_delivery_mst b where b.id=a.mst_id and  a.id in ($dtlsID) and b.entry_form=303 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	}
	else
	{
		$sql_del="select a.id, a.mst_id, a.order_id, a.delivery_qty, a.remarks, a.bill_status, a.sort_qty, a.reject_qty,b.job_order_status from subcon_delivery_dtls a, subcon_delivery_mst b where b.id=a.mst_id and b.job_no='$jobno' and b.entry_form=303 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	}
	...
	foreach ($sql_del_res as $row)
	{
		if($update_id>0)
		{
			...
			$updtls_data_arr[$row[csf("order_id")]]['job_order_status']=$row[csf("job_order_status")];
	...
	if($db_type==0) $process_type_cond="group_concat(c.process,'*',c.embellishment_type)";
		...
		if($update_id>0)
		{
			...
			group by a.id, a.company_id, a.within_group, a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, b.buyer_po_no, b.buyer_style_ref,a.delivery_date, b.id, b.buyer_po_id, b.gmts_item_id, b.order_uom, b.gmts_color_id, b.gmts_size_id, b.order_quantity,e.delivery_qty
			order by b.gmts_size_id ASC";
		}
		else
		{
			$sql_job="select a.id, a.company_id, a.within_group, a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id as po_id, b.buyer_po_id, b.buyer_po_no,b.buyer_style_ref, b.gmts_item_id, 3 as main_process_id, b.order_uom, b.gmts_color_id as color_id, b.gmts_size_id, b.order_quantity as qnty, $process_type_cond as process_type
				...
				group by a.id, a.company_id, a.within_group, a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, b.buyer_po_no,b.buyer_style_ref, a.delivery_date, b.id, b.buyer_po_id, b.gmts_item_id, b.order_uom, b.gmts_color_id, b.gmts_size_id, b.order_quantity
				order by b.gmts_size_id ASC";
		}
	...
	if($variable_status==2)
	{
	 ...
	 $sql_delv_return= "select  b.order_id,b.delivery_qty from subcon_delivery_mst a, subcon_delivery_dtls b 
where a.id=b.mst_id and a.entry_form='360' and  b.entry_form='360' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.job_no='$jobno' and  b.next_process=1 ";
	...
	foreach ($sql_result as $row)
	{
		...
		$quantity=0; $dtlsup_id=""; $balanceQty=0; $preDelv_qty=0; $orderQty=0; $remarks=""; $bill_status=0; $sort_qty=0; $reject_qty=0;$qc_qty=0;
		...
		//print_r($newArr);
			//1==31==2==5242==10532mahbub
		//echo $breakdownprodsequence_no."==".$breakdownembellishmenttype."==".$breakdownprocess."==".$breakdownmst_id."==".$breakdownid."mahbub";
		...
		else
		{
			...
			///echo $breakdownprocess."----";
			
			if($breakdownprocess==2)
			{
				
				//echo "<pre>";
				//print_r($dryqcdtls_data_arr);
		...
		if($update_id>0)
		{
			...
			$preDelv_qty=$pre_total_updtls_data_arr[$row[csf("po_id")]]['qty']-$delv_return_qty;
			///$pre_qty_arr[$row[csf("po_id")]]['qty'];
			
			$job_order_status=$updtls_data_arr[$row[csf("po_id")]]['job_order_status'];
		}
		else
		{
			$preDelv_qty=$pre_qty_arr[$row[csf("po_id")]]['qty']-$delv_return_qty;
			
		   $job_order_status=$updtls_data_arr[$row[csf("po_id")]]['job_order_status'];
		   if($job_order_status!=""){$job_order_status=$job_order_status;}else{$job_order_status=2;}
		...
		//echo "<pre>";
		//print_r($newArr);	echo $breakdownprodsequence_no."==".$breakdownembellishmenttype."==".$breakdownprocess."==".$breakdownmst_id."==".$breakdownid."mahbub"; die;
		?>
        <tr bgcolor="<? echo $bgcolor; ?>" >
            ...
            <td align="right" id="txtsumqcqty_<? echo $k; ?>"  ><? echo $qc_qty; ?>&nbsp;</td>
			<? if($variable_status==2) { ?><td align="right"><? echo $rcv_return_qty; ?>&nbsp;</td><? } ?>
            <td align="right" title="<? echo $delv_return_qty; ?>"><? echo $preDelv_qty; ?>&nbsp;</td>
            <td align="right" title="Job Wise Delivery Status : <? echo $shipment_status[$job_order_status];?>" ><? echo $balanceQty; ?>&nbsp;</td>
            <td title="<? if($bill_status==1) echo "Already Bill Issued."; else echo "";?>"><input type="text" name="txtCurrDelv[]" id="txtCurrDelv_<? echo $k; ?>" class="text_boxes_numeric" style="width:55px;" value="<? echo $quantity; ?>" onBlur="fnc_production_qty_ability(this.value,<? echo $k; ?>); fnc_total_calculate();" placeholder="<? echo $balanceQty; ?>" pre_delv_qty="<? echo $preDelv_qty; ?>" delv_qty="<? echo $quantity; ?>" <? echo $disable; ?> /></td>
            ...
            <td><input type="text" name="txtRemarks_<? echo $k; ?>" id="txtRemarks_<? echo $k; ?>" class="text_boxes" style="width:60px;" value="<? echo $remarks; ?>" />
                ...
                <input type="hidden" name="txtorderlastsequence_<? echo $k; ?>" id="txtorderlastsequence_<? echo $k; ?>" style="width:80px" class="text_boxes" value="<? echo $breakdownprodsequence_no; ?>" />
                <input type="hidden" name="txtorderlastprocess_<? echo $k; ?>" id="txtorderlastprocess_<? echo $k; ?>" style="width:80px" class="text_boxes" value="<? echo $breakdownprocess; ?>" />
                <input type="hidden" name="txtorderlastsequencebreakid_<? echo $k; ?>" id="txtorderlastsequencebreakid_<? echo $k; ?>" style="width:80px" class="text_boxes" value="<? echo $breakdownid; ?>" />
            </td> 
        </tr>
	<?	
	}
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		//---------------Check Duplicate Job in Same Mrr number ------------------------//
		$duplicate = is_duplicate_field("b.id","subcon_delivery_mst a, subcon_delivery_dtls b","a.id=b.mst_id and a.id=$txt_update_id and a.job_no=$txtJob_no and a.entry_form = 303 and a.status_active=1 and b.status_active=1"); 
		
		//echo "10**".$duplicate; die;
		if($duplicate==1) 
		{
			echo "20**Duplicate Job is Not Allow in Same MRR Number.";
		...
		if(str_replace("'","",$txt_update_id)=="")
		{
			...
			$field_array_mst="id, delivery_prefix, delivery_prefix_num, delivery_no, company_id, location_id, floor_id, within_group, party_id, party_location, deli_party, deli_party_location, delivery_date, remarks, job_no,variable_status, entry_form, inserted_by, insert_date, status_active, is_deleted";
			
			$data_array_mst="(".$id.",'".$new_sys_number[1]."','".$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",".$cbo_location_name.",".$cbo_floor_name.",".$cbo_within_group.",".$cbo_party_name.",".$cbo_party_location.",".$cbo_deli_party_name.",".$cbo_deli_party_location.",".$txt_delivery_date.",".$txt_remarks.",".$txtJob_no.",".$txt_variable_status.",303,".$user_id.",'".$pc_date_time."',1,0)";
		...
		else
		{
			...
			$field_array_update_mst="location_id*floor_id*within_group*party_id*party_location*deli_party*deli_party_location*delivery_date*remarks*job_no*updated_by*update_date";
			$data_array_update_mst="".$cbo_location_name."*".$cbo_floor_name."*".$cbo_within_group."*".$cbo_party_name."*".$cbo_party_location."*".$cbo_deli_party_name."*".$cbo_deli_party_location."*".$txt_delivery_date."*".$txt_remarks."*".$txtJob_no."*".$user_id."*'".$pc_date_time."'";
		}
		
		$data_arr_dtls="";
		$id_dtls=return_next_id("id", "subcon_delivery_dtls", 1); $po_wise_qty_arr=array(); $color_size_arr=array();$sum_qc_qty_arr=array(); $sum_CurrDelv_arr=array();
		$field_array_dtls="id, mst_id, buyer_po_id, order_id, remarks, color_size_id, delivery_qty, sort_qty, reject_qty,order_last_sequence,order_last_process,order_sequnce_break_id, inserted_by, insert_date, status_active, is_deleted";
		...
		for($i=1; $i<= str_replace("'","",$total_row); $i++)
		{
			...
			$txtsumqcqty="txtsumqcqty_".$i;
			
			$txtorderlastsequence="txtorderlastsequence_".$i;
			$txtorderlastprocess="txtorderlastprocess_".$i;
			$txtorderlastsequencebreakid="txtorderlastsequencebreakid_".$i;
			 
			if($data_arr_dtls!="") $data_arr_dtls.=",";
			$data_arr_dtls.="(".$id_dtls.",".$id.",".$$txtbuyerPoId.",".$$txtpoid.",".$$txtRemarks.",".$$txtColorSizeid.",".$$txtCurrDelv.",".$$txtsort.",".$$txtreject.",".$$txtorderlastsequence.",".$$txtorderlastprocess.",".$$txtorderlastsequencebreakid.",'".$user_id."','".$pc_date_time."',1,0)"; 
			...
			$sum_qc_qty_arr[str_replace("'","",$txtJob_no)]+=str_replace("'","",$$txtsumqcqty);
			$sum_CurrDelv_arr[str_replace("'","",$txtJob_no)]+=str_replace("'","",$$txtCurrDelv);
		...
		//*************************************************// sum qc qty job wise  start 
		
		   		$variable_status=return_field_value("variable_dtls","variable_setting_wash","company_name=$cbo_company_name and variable_list =1 and is_deleted = 0 and status_active = 1");
				
				
		        if($variable_status==2)
				{
 					 	//////////////receive
					$receive_qty=return_field_value("sum(quantity)","sub_material_dtls a, sub_material_mst b","b.id=a.mst_id and b.embl_job_no=$txtJob_no and b.trans_type=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
					$receive_return_qty=return_field_value("sum(quantity)","sub_material_dtls a, sub_material_mst b","b.id=a.mst_id and b.embl_job_no=$txtJob_no and b.entry_form=372 and b.trans_type=3  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
 				//////////////////////////////
			
				}
				else
				{
						 $embellishment_type_result=sql_select( "select id, mst_id, description, process, embellishment_type, rate,prod_sequence_no,job_no_mst from subcon_ord_breakdown where job_no_mst=$txtJob_no order by id");
						$prod_sequence_no_arr=array();
						foreach ($embellishment_type_result as $row)
						{
							$prod_sequence_no_arr[$row[csf("mst_id")]][$row[csf("prod_sequence_no")]]=$row;
						...
						for($t=1; $t<= str_replace("'","",$total_row); $t++)
						{
								$txtpoid="txtpoid_".$t;
								$breakdownprocess=$newArr[str_replace("'","",$$txtpoid)]['PROCESS'];
								if($breakdownprocess==2)
								{
									$qc_dry_qty=return_field_value("sum(qcpass_qty)","subcon_embel_production_mst a, subcon_embel_production_dtls b"," a.id=b.mst_id and a.entry_form=342 and  a.job_no=$txtJob_no and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
								}
								else if($breakdownprocess==3)
								{
									$qc_dry_qty=return_field_value("sum(qcpass_qty)","subcon_embel_production_mst a, subcon_embel_production_dtls b"," a.id=b.mst_id and a.entry_form=342 and  a.job_no=$txtJob_no and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
								...
								{
									$qc_wet_qty=return_field_value("sum(qcpass_qty)","pro_batch_create_mst a, pro_batch_create_dtls b,subcon_embel_production_dtls c ,subcon_embel_production_mst d ","  a.id=b.mst_id and a.process_id='1'  and  a.status_active=1 and 
		a.entry_form in (316,543) and b.po_id=c.po_id and c.mst_id=d.id  and  d.job_no=$txtJob_no  and  d.entry_form=301 and a.id=d.recipe_id and a.is_deleted=0 
		group by a.id,b.po_id,a.operation_type,c.qcpass_qty,a.process_id");
				...
				////////////////////////////delevery
				$sum_qc_qty=$sum_qc_qty_arr[str_replace("'","",$txtJob_no)];
			    $sum_CurrDelv_qty=$sum_CurrDelv_arr[str_replace("'","",$txtJob_no)];
 				$prev_delivery_qty=return_field_value("sum(delivery_qty)","subcon_delivery_mst a, subcon_delivery_dtls b"," a.id=b.mst_id  and a.entry_form=303 and a.job_no=$txtJob_no and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0");
				$delv_return_qty=return_field_value("sum(delivery_qty)","subcon_delivery_mst a, subcon_delivery_dtls b ","a.id=b.mst_id and a.entry_form='360' and  b.entry_form='360' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.job_no=$txtJob_no and  b.next_process=1");
  				$total_delivery_qty=($prev_delivery_qty+$sum_CurrDelv_qty)-$delv_return_qty;
  				//////////////////////////////////////////////
				...
				if($variable_status==2)
				{
 					$balace_rev_qty=$receive_qty-$receive_return_qty;
					$balanceQty=($balace_rev_qty-$total_delivery_qty);
			
				}
				else
				{
					$balanceQty=($qc_wet_qty-$total_delivery_qty);
				}
				
		//echo "10**";
		//echo "<pre>";
		//echo $qc_wet_qty; die;
 		//*************************************************//  sum qc qty job wise  end
 
		if($balanceQty>0) $job_order_status=2; else $job_order_status=$job_order_status=3;
		
		//if($total_delivery_qty>=$sum_qc_qty) $job_order_status=3; else $job_order_status=str_replace("'","",$cboshipingStatus);
 		/*echo "10**";
		echo "<pre>";
		echo $job_order_status; die;
		echo print_r($sum_qc_qty_arr); die;*/
		...
		if(str_replace("'","",$txt_update_id)=="")
		{
			...
			//$job_status=execute_query("update subcon_delivery_mst set job_order_status=$job_order_status where id=$id",1);
			$job_status=execute_query("update subcon_delivery_mst set job_order_status=$job_order_status where job_no=$txtJob_no and entry_form=303",1);
			
		if($job_status==1 && $flag==1) $flag=1; else $flag=0;
		...
		else
		{
			...
			//$job_status=execute_query("update subcon_delivery_mst set job_order_status=$job_order_status where id=$txt_update_id",1);
		...
		//echo "10**$rID**$rID1**$sts_po**$sts_mst**$job_status**$flag"; disconnect($con); die;
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$field_array_update_mst="location_id*floor_id*within_group*party_id*party_location*deli_party*deli_party_location*delivery_date*remarks*job_no*variable_status*updated_by*update_date";
		$data_array_update_mst="".$cbo_location_name."*".$cbo_floor_name."*".$cbo_within_group."*".$cbo_party_name."*".$cbo_party_location."*".$cbo_deli_party_name."*".$cbo_deli_party_location."*".$txt_delivery_date."*".$txt_remarks."*".$txtJob_no."*".$txt_variable_status."*".$user_id."*'".$pc_date_time."'";
		...
		$field_arr_up="buyer_po_id*order_id*remarks*color_size_id*delivery_qty*sort_qty*reject_qty*order_last_sequence*order_last_process*order_sequnce_break_id*updated_by*update_date*status_active*is_deleted";
		
		$sum_qc_qty_arr=array(); $sum_CurrDelv_arr=array();
		...
		for($i=1; $i<= str_replace("'","",$total_row); $i++)
		{
			...
			{
				$data_arr_up[str_replace("'","",$$txtDtlsUpdateId)]=explode("*",("".$$txtbuyerPoId."*".$$txtpoid."*".$$txtRemarks."*".$$txtColorSizeid."*".$$txtCurrDelv."*".$$txtsort."*".$$txtreject."*".$$txtorderlastsequence."*".$$txtorderlastprocess."*".$$txtorderlastsequencebreakid."*".$user_id."*'".$pc_date_time."'*1*0"));
		...
		group by a.id,b.po_id,a.operation_type,c.qcpass_qty,a.process_id");
				...
				$prev_delivery_qty=return_field_value("sum(delivery_qty)","subcon_delivery_mst a, subcon_delivery_dtls b"," a.id=b.mst_id  and a.id!=$txt_update_id  and a.entry_form=303 and a.job_no=$txtJob_no and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0");
				$delv_return_qty=return_field_value("sum(delivery_qty)","subcon_delivery_mst a, subcon_delivery_dtls b ","a.id=b.mst_id and a.entry_form='360' and  b.entry_form='360' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.job_no=$txtJob_no and  b.next_process=1");
				 $sum_qc_qty=$sum_qc_qty_arr[str_replace("'","",$txtJob_no)];
			     $sum_CurrDelv_qty=$sum_CurrDelv_arr[str_replace("'","",$txtJob_no)];
				 $total_delivery_qty=($prev_delivery_qty+$sum_CurrDelv_qty)-$delv_return_qty;
		...
		//echo "10**".$balanceQty; die;
		
		
		/*$sum_qc_qty=$sum_qc_qty_arr[str_replace("'","",$txtJob_no)];
 		$sum_CurrDelv_qty=$sum_CurrDelv_arr[str_replace("'","",$txtJob_no)];
		
		$total_delivery_qty=$prev_delivery_qty+$sum_CurrDelv_qty;
		$del_sql_dtls="Select id, bill_status from subcon_delivery_dtls where mst_id in ($id) and status_active=1 and is_deleted=0";//
		...
		$distance_delete_id=implode(',',array_diff($all_dtls_id_arr,$id_arr_delv));
		  ...
		  if($distance_delete_id>0)
		  {
				$job_order_status=2; 
		  }
		  else
		  {
			   if($total_delivery_qty>=$sum_qc_qty)
				{
					$job_order_status=3; 
				} 
				else
		...
		//echo "10**";
		//echo "<pre>";
		//echo $sum_qc_qty."==".$sum_CurrDelv_qty."==".$prev_delivery_qty; die;
		//echo print_r($sum_qc_qty_arr); die;
		...
		if(str_replace("'",'',$txt_update_id)!="")// importwnt commoent first update work  subcon_delivery_dtls mst_id wise  status_active=0 is_deleted= 1
		{
			//echo "10**".$txt_update_id; disconnect($con); die;
			...
			//echo "10**".bulk_update_sql_statement("subcon_delivery_dtls", "mst_id", $field_array_del, $data_array_del, $txt_update_id); disconnect($con); die;
			$rID3=sql_update("subcon_delivery_dtls",$field_array_del,$data_array_del,"mst_id",$txt_update_id,0);
			//$rID3=execute_query(bulk_update_sql_statement( "subcon_delivery_dtls", "mst_id", $field_array_del,$data_array_del,$txt_update_id),1);
			if($rID3==1 && $flag==1) $flag=1; else $flag=0;
		}
		
		
		
		if($data_arr_up!="") // importwnt commoent 2nd update work  subcon_delivery_dtls id wise  status_active=1 is_deleted= 0 because 0 qty row not save update  and $rID3 after  $rID1 
		{
		...
		//echo "10**".$distancedeleteid; disconnect($con); die;
	...
	else if ($operation==2)  // Delete Here---------------------------------------------------------- 
	{
		...
		$prev_delivery_qty=return_field_value("sum(delivery_qty)","subcon_delivery_mst a, subcon_delivery_dtls b"," a.id=b.mst_id  and a.id!=$txt_update_id  and a.entry_form=303 and a.job_no=$txtJob_no");
		$total_delivery_qty=$prev_delivery_qty+$sum_CurrDelv_qty;

		if($total_delivery_qty>=$sum_qc_qty) $job_order_status=3; else $job_order_status=str_replace("'","",$cboshipingStatus);*/
		
		$job_status=execute_query("update subcon_delivery_mst set job_order_status=2 where job_no=$txtJob_no and entry_form=303",1);
		if($job_status==1 && $flag==1) $flag=1; else $flag=0;
		////////////////////////
...
if($action=="create_delivery_search_list_view")
{
   ...
   b.buyer_buyer from subcon_ord_mst a, subcon_ord_dtls b where a.subcon_job=b.job_no_mst and a.entry_form=295 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
	...
	group by a.id, a.delivery_no, a.delivery_prefix_num, a.insert_date, a.location_id, a.floor_id, a.within_group, a.party_id, a.party_location, a.deli_party, a.deli_party_location, a.remarks, a.delivery_date, a.job_no order by a.id DESC ";
	//echo $sql; 
	$result = sql_select($sql);
	?>
    <div>
     ...
     <div style="width:820px; max-height:270px;overflow-y:scroll;" >	 
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="800" class="rpt_table" id="tbl_po_list">
            ...
            foreach($result as $row)
            {
				...
				if($row[csf("within_group")]==1) $party_name=$comp[$row[csf("party_id")]]; else $party_name=$party_arr[$row[csf("party_id")]];
				$str_data="";
				$str_data=$row[csf('id')].'***'.$row[csf('delivery_no')].'***'.$row[csf('location_id')].'***'.$row[csf('within_group')].'***'.$row[csf('party_id')].'***'.change_date_format($row[csf('delivery_date')]).'***'.$row[csf('remarks')].'***'.$row[csf('job_no')].'***'.$order_no.'***'.$buyer_style.'***'.$buyer_po.'***'.implode(",",$delivery_status).'***'.$row[csf('party_location')].'***'.$row[csf('deli_party')].'***'.$row[csf('deli_party_location')].'***'.$row[csf('floor_id')];
				?>
...
{
	extract($_REQUEST);
	list($company_id, $update_id, $job_no, $report_title, $cbo_template_id) = explode("*", $data);


//echo $cbo_template_id; die;
	...
	{
		 $sql_main="SELECT a.id, a.company_id, a.delivery_no, a.location_id, a.within_group, a.party_id, a.delivery_date, a.job_no, a.remarks, b.delivery_qty, b.order_id, b.remarks as dtls_remarks, d.subcon_job, d.order_no, c.buyer_style_ref,c.party_buyer_name as buyer_buyer, c.gmts_item_id, c.gmts_color_id as color_id, c.gmts_size_id, c.buyer_po_id, c.buyer_po_no, e.job_no, f.grouping
		from subcon_delivery_mst a, subcon_delivery_dtls b, subcon_ord_dtls c, subcon_ord_mst d, wo_po_details_master e,wo_po_break_down f
		where a.id=b.mst_id and b.order_id=c.id and c.mst_id=d.id and a.entry_form=303 and a.id='$update_id' and e.job_no=f.job_no_mst and c.buyer_po_id=f.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 ORDER BY c.gmts_size_id ASC";  //and d.subcon_job='$job_no'
	} 
	else 
	{
		 $sql_main="SELECT a.id, a.company_id, a.delivery_no, a.location_id, a.within_group, a.party_id, a.delivery_date, a.job_no, a.remarks, b.delivery_qty, b.order_id, b.remarks as dtls_remarks, d.subcon_job, d.order_no, c.buyer_style_ref,c.party_buyer_name as buyer_buyer, c.gmts_item_id, c.gmts_color_id as color_id, c.gmts_size_id, c.buyer_po_id, c.buyer_po_no, '' as job_no, '' as grouping
		from subcon_delivery_mst a, subcon_delivery_dtls b, subcon_ord_dtls c, subcon_ord_mst d 
		where a.id=b.mst_id and b.order_id=c.id and c.mst_id=d.id and a.entry_form=303 and a.id='$update_id' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1   ORDER BY c.gmts_size_id ASC";  //and d.subcon_job='$job_no'
	}
    ...
    <div style="width:1600px; font-size:20px">
            ...
            <tr>
                ...
                <td>
                    <table width="800" cellspacing="0" align="center">
						...
						<tr><td>&nbsp;</td></tr>
                        <tr>
                            <td align="center" style="font-size:x-large;"><strong><u><? echo $report_title.' Challan'; ?></u></strong></td>
                        </tr>
                    </table>
                </td>
            </tr>
        ...
        <br>
        <div style="width:100%;">
            <table align="right" cellspacing="1" cellpadding="1" width="1600" border="1" rules="all" class="rpt_table">
                <thead bgcolor="#dddddd" align="center"><!-- style="font-size:12px"-->
                    <th width="50">SL</th>
                    <th width="130">Buyer PO</th>
                    <th width="130">Buyer Style Ref</th>
                    <th width="130">IR No.</th>
                    <th width="130">Buyer Job No.</th>
				...
				foreach ($dataArray as $row) 
				{
                    ...
                    <tr bgcolor="<? echo $bgcolor; ?>">
                        ...
                        <td width="130"><p><? echo $row[csf('grouping')]; ?></p></td>
                        ...
                        <td width="50" align="center"><p><? echo $size_arrey[$row[csf('gmts_size_id')]]; ?></p></td>
            ...
            <br>
			<? echo signature_table(181, $company_id, "1600px",$cbo_template_id, 20); 
 			?>
        </div>
    </div>
	<?
...
if($action=="embl_delivery_entry_print2")
{
	...
	$sql_main="SELECT a.id, a.company_id, a.delivery_no, a.location_id, a.party_location, a.within_group, a.party_id, a.delivery_date, a.job_no, a.remarks, b.delivery_qty, b.order_id, b.remarks as dtls_remarks, d.subcon_job, d.order_no, c.buyer_style_ref,c.party_buyer_name as buyer_buyer, c.gmts_item_id, c.gmts_color_id as color_id, c.buyer_po_id, c.buyer_po_no
		from subcon_delivery_mst a, subcon_delivery_dtls b, subcon_ord_dtls c, subcon_ord_mst d
		where a.id=b.mst_id and b.order_id=c.id and c.mst_id=d.id and a.entry_form=303 and a.id='$update_id'  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	$dataArray = sql_select($sql_main);// and d.subcon_job='$job_no'
	        ...
	        <div style="width:100%;">
	            <table align="right" cellspacing="1" cellpadding="1" width="1000" border="1" rules="all" class="rpt_table">
					...
					<tr>
						...
						<td colspan="5" align="left"><strong><? echo number_to_words($tot_delivery_qty); ?> Pcs</strong></td>
					</tr>
	            </table>	         
	            <br>
				
				<div>
					<? echo signature_table(181, $company_id, "1000px",$cbo_template_id, 20); ?>
				</div>
	        </div>
	    </div>
...
if($action="default_html")
{
	...
	<tr bgcolor="#FFFFFF">
		...
		<td>&nbsp;</td>
		<td>&nbsp;</td>
		<td align="center"><input type="text" name="txtCurrDelv[]" id="txtCurrDelv_1" class="text_boxes_numeric" style="width:45px;" onBlur="fnc_production_qty_ability(this.value,1); fnc_total_calculate();" /></td>
