...
{	
	...
	else if($db_type==2)
	{
	 ...
	 $sql= "select a.id, a.subcon_job, a.job_no_prefix_num, $ins_year_cond as year, a.company_id, a.location_id, a.party_id, a.receive_date, a.order_no, a.delivery_date, $color_id_str as color_id, $buyer_po_id_cond as buyer_po_id  
     ...
     <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="885" >
        ...
        <tbody>
            ...
            {  
                ...
                ?>
                <tr bgcolor="<? echo $bgcolor; ?>" onClick="js_set_value('<? echo $row[csf('subcon_job')].'***'.$row[csf('order_no')].'***'.$buyer_style.'***'.$buyer_name; ?>')" style="cursor:pointer" >
...
?>	
	...
	$sql= "select a.id, a.delivery_no, a.delivery_prefix_num, sum(b.delivery_qty) as delivery_qty, $insert_date_cond as year, a.location_id, a.within_group, a.party_id, a.party_location, a.deli_party, a.deli_party_location, a.delivery_date, $wo_cond as order_id,$do_cond as dtls_id, $buyer_po_id_cond as buyer_po_id, $delivery_status_cond as delivery_status,c.job_no_mst from subcon_delivery_mst a, subcon_delivery_dtls b , subcon_ord_dtls c 
	
	where a.id=b.mst_id and a.entry_form='303' and  a.id='$data' and b.order_id=c.id  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id, a.delivery_no, a.delivery_prefix_num, a.insert_date, a.location_id, a.within_group, a.party_id, a.party_location, a.deli_party, a.deli_party_location, a.delivery_date,c.job_no_mst order by a.id DESC ";
	
	
	
			//$sql="select a.id,a.delivery_date,a.location_id,a.challan_no,b.id as dtls_id,b.order_id,b.item_id,b.process_id,b.delivery_qty from  subcon_delivery_mst a,  subcon_delivery_dtls b where a.id=b.mst_id and  a.id='$data' and a.status_active=1 and a.is_deleted=0 group by a.id,a.delivery_date,a.location_id,a.challan_no,b.id as dtls_id,b.order_id,b.item_id,b.process_id,b.delivery_qty order by a.id";
...
if($action=="populate_data_from_data")
{
	...
	foreach($result as $row)
	{
		...
		echo "$('#txtBuyerName').val('".$buyer_po."');\n";
...
if($action=="load_php_dtls_form")
{
	...
	$qcdtls_data_arr=array();
		...
		if($update_id!=0)
		{
		  $sql_del="select a.id, a.mst_id, a.order_id, a.delivery_qty, a.remarks, a.bill_status, a.sort_qty, a.reject_qty from subcon_delivery_dtls a, subcon_delivery_mst b where b.id=a.mst_id and  a.id not in ($dtlsID) and b.entry_form=303 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	...
	if($db_type==0) $process_type_cond="group_concat(c.process,'*',c.embellishment_type)";
		...
		if($update_id!=0)
		{
			 ...
			 $sql_job="select a.id, a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id as po_id, b.buyer_po_id, b.gmts_item_id, 3 as main_process_id, b.order_uom, b.gmts_color_id as color_id, b.gmts_size_id, b.order_quantity as qnty, $process_type_cond as process_type,e.delivery_qty
			...
			group by a.id, a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id, b.buyer_po_id, b.gmts_item_id, b.order_uom, b.gmts_color_id, b.gmts_size_id, b.order_quantity,e.delivery_qty
			order by b.id ASC";
		}
		else
		{
			$sql_job="select a.id, a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id as po_id, b.buyer_po_id, b.gmts_item_id, 3 as main_process_id, b.order_uom, b.gmts_color_id as color_id, b.gmts_size_id, b.order_quantity as qnty, $process_type_cond as process_type
		...
		group by a.id, a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id, b.buyer_po_id, b.gmts_item_id, b.order_uom, b.gmts_color_id, b.gmts_size_id, b.order_quantity
		order by b.id ASC";
			}
	...
	foreach ($sql_result as $row)
	{
		...
		$quantity=0; $dtlsup_id=""; $balanceQty=0; $preDelv_qty=0; $orderQty=0; $remarks=""; $bill_status=0; $sort_qty=0; $reject_qty=0;
			...
			if($update_id!="")
			{
				$preDelv_qty=$updtls_data_arr[$row[csf("po_id")]]['qty'];///$pre_qty_arr[$row[csf("po_id")]]['qty'];
			}
			else
			{
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$txt_update_id)=="")
		{
			...
			$field_array_mst="id, delivery_prefix, delivery_prefix_num, delivery_no, company_id, location_id, within_group, party_id, party_location, deli_party, deli_party_location, delivery_date, remarks, job_no, entry_form, inserted_by, insert_date, status_active, is_deleted";
			
			$data_array_mst="(".$id.",'".$new_sys_number[1]."','".$new_sys_number[2]."','".$new_sys_number[0]."',".$cbo_company_name.",".$cbo_location_name.",".$cbo_within_group.",".$cbo_party_name.",".$cbo_party_location.",".$cbo_deli_party_name.",".$cbo_deli_party_location.",".$txt_delivery_date.",".$txt_remarks.",".$txtJob_no.",303,".$user_id.",'".$pc_date_time."',1,0)";
		...
		//echo "10**$rID**$rID1**$sts_po**$sts_mst**$flag"; die;
...
if($action=="embl_delivery_entry_print")
{
	...
	$sql="select id, prefix_no_num, sys_no, location_id, recipe_id, job_no, order_id from subcon_embel_production_mst where entry_form=303 and status_active = 1 and is_deleted = 0 and company_id='$company_id' $location_cond $system_no_cond $recipe_no_cond order by id DESC";
	
	$sql_mst = "select id, delivery_no, company_id, location_id, within_group, party_id, delivery_date, job_no, remarks from subcon_delivery_mst where entry_form=303 and id='$data[1]'";
    ...
    <div style="width:930px; font-size:6px">
        ...
        <div style="width:100%;">
            <table align="right" cellspacing="0" width="930" border="1" rules="all" class="rpt_table" style="font-size:12px">
				...
				$sql= "select  a.id, a.subcon_job, b.id as order_id, b.order_no, b.buyer_po_id, b.main_process_id, b.gmts_item_id, b.embl_type, b.body_part, c.id as color_size_id, c.color_id, c.size_id, c.qnty as qty, d.delivery_qty, d.sort_qty, d.reject_qty, d.remarks  from subcon_ord_mst a, subcon_ord_dtls b, subcon_ord_breakdown c, subcon_delivery_dtls d where a.subcon_job=b.job_no_mst and b.id=c.mst_id and c.id=d.color_size_id 
							
				
				and a.entry_form=295 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and a.company_id='$data[0]' and a.subcon_job='$job_no' and d.mst_id='$data[1]' order by c.id ASC";
				//echo $sql; die;
					...
					if (!in_array($row[csf("order_no")],$order_array) )
					{
						...
						?>
							<tr bgcolor="#dddddd">
                                ...
                                <td colspan="4" align="left" style="font-size:12px"><b>Buyer Style: <? echo $buyer_po_arr[$row[csf("buyer_po_id")]]['style']; ?>;</b></td>
							</tr>
						<?
            ...
            <br>
			<? echo signature_table(155, $com_id, "930px"); ?>
        </div>
    </div>
	<?
	exit();
