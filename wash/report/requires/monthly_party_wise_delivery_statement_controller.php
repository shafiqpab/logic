...
{ 
	...
	$return_sql = "select a.id, a.job_no, sum(b.delivery_qty) as delivery_return_qty, b.order_id, c.job_no_mst, c.gmts_color_id, c.gmts_item_id, b.id as dtls_id, a.delivery_no, a.delivery_date, b.remarks
    			from subcon_delivery_mst a, subcon_delivery_dtls b, subcon_ord_dtls c
   				where a.id = b.mst_id and a.entry_form = '360' and b.order_id = c.id $company_name $party_con $delivery_date $search_com_cond and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0
				group by a.id, c.job_no_mst, c.gmts_color_id, c.gmts_item_id, a.delivery_no, a.delivery_date, a.job_no, b.id, b.order_id,
       				b.remarks
				order by a.id desc";
    ...
    foreach ($delivery_result as $row) {
 	...
 	foreach ($return_result as $row) {
    	...
    	if( isset($return_arr[$row[csf('job_no_mst')]]) )
	   	{
    		$return_arr[$row[csf('job_no_mst')]][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['delivery_return_qty']+=$row[csf('delivery_return_qty')];
    	} else {
    		$return_arr[$row[csf('job_no_mst')]][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['delivery_return_qty']=$row[csf('delivery_return_qty')];
    	}
    ...
    foreach($order_result as $row)
	 {
	 	$job_dtls_id_arr[] = $row[csf('ord_dtls_id')];
    ...
    //unset($order_result);
	    ...
	    $job_dtls_id_arr = array_unique($job_dtls_id_arr);
		  ...
		  $break_con=where_con_using_array($job_dtls_id_arr,0,"e.mst_id");
		 ...
		 $order_break_sql = "select  e.mst_id ,$process_type_cond as  process from subcon_ord_breakdown e  where   e.status_active=1 and e.is_deleted=0 $break_con group by e.mst_id"; 
		 //echo $order_break_sql; die;
		  $break_order_result = sql_select($order_break_sql);
		  $process_break_id_arr = array();
	     foreach($break_order_result as $row)
		 {
	    	$process_break_id_arr[$row[csf('mst_id')]]['process_id_arr'] = $row[csf('process')];
		 }

		 /*echo "<pre>";
		 print_r($process_break_id_arr); die;*/


		
		$receeive_qty_array=array();
		$sql_receeive="Select b.job_dtls_id,c.job_no_mst,a.subcon_date,b.quantity as receive_qnty, c.gmts_color_id,c.gmts_item_id from subcon_ord_dtls c, sub_material_mst a, sub_material_dtls b where a.id=b.mst_id and a.entry_form=296 and b.job_dtls_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
        ...
        {
        	$total_balance_quantity=0; $total_delivery_qty_dzn=0; $total_delivery_amount_usd=0;$total_receeive_qty=0; $total_return_quantity =0; $total_actual_quantity =0;
			 $k=1;
			 $sl=1;
        ?>
        ...
        <thead>
            ...
            <th width="110" >Delivery Qty. (pcs)</th>
            <th width="110" >Delivery Return (pcs)</th>
            <th width="110" >Actual Delivery (pcs)</th>
			...
			{	
				...
				{	
					...
					{								
						...
						$return_qty = $return_arr[$row_data['job_no']][$row_data['gmts_color_id']][$row_data['gmts_item_id']]['delivery_return_qty'];

						$actual_delivery_qty = $delivery_qty-$return_qty;
						
						if($row_data['order_uom']==1)
						{
							$rate=$row_data['rate']*12;
							$delivery_qty_dzn=$actual_delivery_qty/12;
							$delivery_amount_usd=($actual_delivery_qty/12)*$rate;
						}
					...
					$ex_process=array_unique(explode(",",$process_break_id_arr[$row_data['job_dtls_id']]['process_id_arr']));  
				   // $ex_process=array_unique(explode(",",$row_data['process'])); 
					...
					<tr bgcolor="<? echo $bgcolor; ?>" <? echo $stylecolor; ?> onClick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $k; ?>">
	                    ...
	                    <td width="110"  align="right"><p><a href='#report_details' onClick="openmypage_receive('<? echo $row_data['job_no']; ?>','<? echo $row_data['gmts_color_id']; ?>','<? echo $row_data['gmts_item_id'];?>','<? echo $row_data['order_no']; ?>','<? echo $party_id;?>','<? echo $row_data['buyer_style_ref'];?>','receive_popup','delivery_date_wise');"><? echo number_format($receeive_qty,2);  ?> </a></p></td>
						<td width="110"  align="right"><p><a href='#report_details' onClick="openmypage_delivery('<? echo $row_data['job_no']; ?>','<? echo $row_data['gmts_color_id']; ?>','<? echo $row_data['gmts_item_id'];?>','<? echo $row_data['order_no']; ?>','<? echo $party_id;?>','<? echo $row_data['buyer_style_ref']; ?>','delivery_popup','delivery_date_wise');"><? echo number_format($delivery_qty,2);  ?> </a></p></td>
						<td width="110"  align="right"><p><?php echo number_format($return_qty,2); ?></p></td>
						<td width="110"  align="right"><p><?php echo number_format($actual_delivery_qty,2); ?></p></td>
					...
					$sl++;
					 ...
					 $total_actual_quantity+=$actual_delivery_qty;
        ...
        <tr>
            ...
            <td align="right"><? echo number_format($total_return_quantity,2); ?></td>
            <td align="right"><? echo number_format($total_actual_quantity,2); ?></td>
        ...
        <?
         ...
         $grand_total_return_quantity+=$total_return_quantity;
		 $grand_total_actual_quantity+=$total_actual_quantity;
        ...
        <tr>
            ...
            <td align="right"><? echo number_format($grand_total_return_quantity,2); ?></td>
            <td align="right"><? echo number_format($grand_total_actual_quantity,2); ?></td>
...
{ 
 ...
 /* $order_sql = "select c.rate,c.job_no_mst, c.gmts_item_id, c.gmts_color_id, c.order_no, c.buyer_po_no, c.buyer_style_ref, d.party_id, c.buyer_buyer, c.party_buyer_name,c.id as ord_dtls_id, c.order_uom, c.order_quantity as order_quantity,g.quantity as receive_qnty,c.remarks,$process_type_cond as process
		...
		*/
		   ...
		   $order_sql = "select  c.rate,c.job_no_mst, c.gmts_item_id, c.gmts_color_id, c.order_no, c.buyer_po_no, c.buyer_style_ref, d.party_id, c.buyer_buyer, c.party_buyer_name,c.id as ord_dtls_id, c.order_uom, c.order_quantity as order_quantity,g.quantity as receive_qnty,c.remarks
  		from sub_material_mst f, sub_material_dtls g,subcon_ord_dtls c,subcon_ord_mst d
 		where  f.id=g.mst_id  and f.entry_form=296  and g.job_dtls_id=c.id  and c.mst_id = d.id $receive_company_name $search_com_cond $within_group $receive_date $party_con and  f.status_active=1 and f.is_deleted=0 and g.status_active=1 and g.is_deleted=0"; 
				...
				{	
				   ...
				   // $ex_process=array_unique(explode(",",$row_data['process'])); 
					...
					<tr bgcolor="<? echo $bgcolor; ?>" <? echo $stylecolor; ?> onClick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $k; ?>">
	                    ...
	                    <td width="130" align="center"><? echo $garments_item[$row_data['gmts_item_id']];   ?></td>
	                    <td width="130" align="center"><? echo $sub_process_name;//chop($embellishment_type,','); ?></td>
	                    <td width="110"  align="right"><p><a href='#report_details' onClick="openmypage_receive('<? echo $row_data['job_no']; ?>','<? echo $row_data['gmts_color_id']; ?>','<? echo $row_data['gmts_item_id'];?>','<? echo $row_data['order_no']; ?>','<? echo $party_id;?>','<? echo $row_data['buyer_style_ref'];?>','receive_popup','receive_date_wise');"><? echo number_format($receeive_qty,2);  ?> </a></p></td>
						<td width="110"  align="right"><p><a href='#report_details' onClick="openmypage_delivery('<? echo $row_data['job_no']; ?>','<? echo $row_data['gmts_color_id']; ?>','<? echo $row_data['gmts_item_id'];?>','<? echo $row_data['order_no']; ?>','<? echo $party_id;?>','<? echo $row_data['buyer_style_ref']; ?>','delivery_popup','receive_date_wise');"><? echo number_format($delivery_qty,2);  ?> </a></p></td>
						<td width="110"  align="right"><?  echo number_format($delivery_qty_dzn,2);   ?></td>
	                    <td width="110"  align="right"><?  echo number_format($rate,4);   ?></td>
	                    <td width="110"  align="right"><?  echo number_format($delivery_amount_usd,2);   ?></td>
						<td width="200"  align="center"></td>
...
if($action=="report_generate_receive_delivery_date")
{ 
	...
	$party_com_arr=return_library_array( "SELECT id, company_name from lib_company where status_active =1 and is_deleted=0",'id','company_name');
	$party_buyer_arr=return_library_array( "SELECT id, buyer_name from lib_buyer where status_active =1 and is_deleted=0",'id','buyer_name');
	$search_str=trim(str_replace("'","",$txt_search_string));
	$search_type =str_replace("'","",$cbo_type);
		if($search_str!="")
		{
    ...
    <div id="mstDiv" align="center">
	  ...
	  $receive_sql = "select  c.job_no_mst, c.gmts_item_id, c.gmts_color_id, c.order_no,c.buyer_style_ref, d.party_id,g.quantity as receive_qnty,d.within_group
  		from sub_material_mst f, sub_material_dtls g,subcon_ord_dtls c,subcon_ord_mst d
 		where  f.id=g.mst_id  and f.entry_form=296  and g.job_dtls_id=c.id  and c.mst_id = d.id $receive_company_name $search_com_cond $within_group $receive_date $party_con and  f.status_active=1 and f.is_deleted=0 and g.status_active=1 and g.is_deleted=0"; 
   	//echo $order_sql;die;
  	$receive_result = sql_select($receive_sql);
  	// echo $order_sql;
	 $receive_data_array = array();
     foreach($receive_result as $row)
	 {
 		
		$receive_data_array[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$row[csf('job_no_mst')]][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['receive_qnty']+=$row[csf('receive_qnty')];
		
		//$receive_data_array[$row[csf('ord_dtls_id')]]['receive_qnty']+=$row[csf('receive_qnty')];
		//$receive_data_data[$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['receive_qnty']+=$row[csf('receive_qnty')];
     }
	
	
	
	
	//print_r($receive_data_array); die;
	

   $delivery_sql = "select a.id, a.job_no, sum(b.delivery_qty) as delivery_qty, b.order_id, c.job_no_mst, c.gmts_color_id, c.gmts_item_id, b.id as dtls_id, a.delivery_no, a.delivery_date, b.remarks
    			from subcon_delivery_mst a, subcon_delivery_dtls b, subcon_ord_dtls c
   				where a.id = b.mst_id and a.entry_form = '303' and b.order_id = c.id $company_name $delivery_date  $search_com_cond and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0  group by a.id, c.job_no_mst, c.gmts_color_id, c.gmts_item_id, a.delivery_no, a.delivery_date, a.job_no, b.id, b.delivery_qty, b.order_id,
       				b.remarks
				order by a.id desc";  //$delivery_date
	...
	$order_con=where_con_using_array($order_id_arr,0,"c.id");
	 ...
	 $receeive_qty_array=array();
		...
		/* $order_sql = "select  c.rate,c.job_no_mst, c.gmts_item_id, c.gmts_color_id, c.order_no, c.buyer_po_no, c.buyer_style_ref, d.party_id, c.buyer_buyer, c.party_buyer_name,c.id as ord_dtls_id, c.order_uom, c.order_quantity as order_quantity,d.within_group
  		from sub_material_mst f, sub_material_dtls g,subcon_ord_dtls c,subcon_ord_mst d
 		where  f.id=g.mst_id  and f.entry_form=296  and f.embl_job_no = c.job_no_mst
and f.trans_type = 1  and d.entry_form=295 and g.job_dtls_id=c.id  and c.mst_id = d.id  $receive_company_name $search_com_cond $within_group $receive_date $party_con and  f.status_active=1 and f.is_deleted=0 and g.status_active=1 and g.is_deleted=0 group by c.rate,c.job_no_mst, c.gmts_item_id, c.gmts_color_id, c.order_no, c.buyer_po_no, c.buyer_style_ref, d.party_id, c.buyer_buyer, c.party_buyer_name,c.id, c.order_uom, c.order_quantity,d.within_group
		union all
		select  c.rate,c.job_no_mst, c.gmts_item_id, c.gmts_color_id, c.order_no, c.buyer_po_no, c.buyer_style_ref, d.party_id, c.buyer_buyer, c.party_buyer_name,c.id as ord_dtls_id, c.order_uom, c.order_quantity as order_quantity,d.within_group
  		from subcon_delivery_mst a, subcon_delivery_dtls b,subcon_ord_dtls c,subcon_ord_mst d
 		where  a.id=b.mst_id  and a.entry_form=303 and d.entry_form=295  and b.order_id=c.id  and c.mst_id = d.id   $company_name $search_com_cond $within_group $delivery_date $party_con and  a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by c.rate,c.job_no_mst, c.gmts_item_id, c.gmts_color_id, c.order_no, c.buyer_po_no, c.buyer_style_ref, d.party_id, c.buyer_buyer, c.party_buyer_name,c.id, c.order_uom, c.order_quantity,d.within_group"; */
		
		
		  $order_sql = "select  c.rate,c.job_no_mst, c.gmts_item_id, c.gmts_color_id, c.order_no,c.buyer_style_ref, d.party_id, c.party_buyer_name,c.id as ord_dtls_id, c.order_uom,d.within_group
  		from sub_material_mst f, sub_material_dtls g,subcon_ord_dtls c,subcon_ord_mst d
 		where  f.id=g.mst_id  and f.entry_form=296  and f.embl_job_no = c.job_no_mst
and f.trans_type = 1  and d.entry_form=295 and g.job_dtls_id=c.id  and c.mst_id = d.id  $receive_company_name $search_com_cond $within_group $receive_date $party_con and  f.status_active=1 and f.is_deleted=0 and g.status_active=1 and g.is_deleted=0 group by c.rate,c.job_no_mst, c.gmts_item_id, c.gmts_color_id, c.order_no,  c.buyer_style_ref, d.party_id, c.party_buyer_name,c.id, c.order_uom,d.within_group
		union
		select  c.rate,c.job_no_mst, c.gmts_item_id, c.gmts_color_id, c.order_no, c.buyer_style_ref, d.party_id, c.party_buyer_name,c.id as ord_dtls_id, c.order_uom,d.within_group
  		from subcon_delivery_mst a, subcon_delivery_dtls b,subcon_ord_dtls c,subcon_ord_mst d
 		where  a.id=b.mst_id  and a.entry_form=303 and d.entry_form=295  and a.job_no = c.job_no_mst  and b.order_id=c.id  and c.mst_id = d.id   $company_name $search_com_cond $within_group $delivery_date $party_con and  a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by c.rate,c.job_no_mst, c.gmts_item_id, c.gmts_color_id, c.order_no, c.buyer_style_ref, d.party_id,c.party_buyer_name,c.id, c.order_uom,d.within_group"; 
	...
	$job_dtls_id_arr = array();
     foreach($order_result as $row)
	 {
    	...
    	// echo "$job_no <br>";
    //	$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['id']=$delivery_data[$row[csf('ord_dtls_id')]]['id'];
    	$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['party_buyer_name']=$row[csf('party_buyer_name')];
    	$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['order_no']=$row[csf('order_no')];
    	$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['job_no']=$row[csf('job_no_mst')];
		$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['within_group']=$row[csf('within_group')];//$delivery_data[$row[csf('ord_dtls_id')]]['job_no'];
    	$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['gmts_item_id']=$row[csf('gmts_item_id')];    	
    	$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['rate']=$row[csf('rate')];
    	$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['buyer_style_ref']=$row[csf('buyer_style_ref')];
    	$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['gmts_color_id']=$row[csf('gmts_color_id')];
    	//$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['remarks']=$row[csf('remarks')];//$delivery_data[$row[csf('ord_dtls_id')]]['remarks'];
    	$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['party_id']=$row[csf('party_id')];
    	//$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['details_id']=$delivery_data[$row[csf('ord_dtls_id')]]['details_id'];
    	$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['job_dtls_id'].=$row[csf('ord_dtls_id')].',';
    	//$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['embellishment_type'].=$row[csf('embellishment_type')].',';
    	//$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['process']=$row[csf('process')];
    	$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['order_uom']=$row[csf('order_uom')];
		//$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['order_quantity']+=$row[csf('order_quantity')];
		$do_all_data[$row[csf('within_group')]][$row[csf('party_id')]][$row[csf('buyer_style_ref')]][$job_no][$row[csf('gmts_color_id')]][$row[csf('gmts_item_id')]]['receive_qnty']+=$row[csf('receive_qnty')];
		$job_dtls_id_arr[] = $row[csf('ord_dtls_id')];
    ...
    $pi_sql = "select a.pi_number, b.work_order_no, b.color_id, b.booking from com_export_pi_mst a, com_export_pi_dtls b, tmp_job_no d where a.id=b.pi_id and a.status_active=1 and a.is_deleted=0 and b.work_order_no is not null and d.userid=$user_id and d.job_no=b.work_order_no group by a.pi_number, b.work_order_no, b.color_id, b.booking";

    $pi_data_arr = sql_select($pi_sql);

    $pi_no_arr = array();

    foreach($pi_data_arr as $data)
    {
    	if($pi_no_arr[$data[csf('work_order_no')]][$data[csf('booking')]][$data[csf('color_id')]]=='')
    	{
    		$pi_no_arr[$data[csf('work_order_no')]][$data[csf('booking')]][$data[csf('color_id')]] .= $data[csf('pi_number')];
    	}
    	else
    	{
    		$pi_no_arr[$data[csf('work_order_no')]][$data[csf('booking')]][$data[csf('color_id')]] .= ", ".$data[csf('pi_number')];
	...
	//print_r($do_all_data); die;
     ...
     foreach($break_order_result as $row)
	 {
		...
		//$party_com_arr=return_library_array( "SELECT id, company_name from lib_company where status_active =1 and is_deleted=0",'id','company_name');
	//	$party_buyer_arr=return_library_array( "SELECT id, buyer_name from lib_buyer where status_active =1 and is_deleted=0",'id','buyer_name');
	  foreach($do_all_data as $within_group_id=>$within_group_data) //$pay_term
      { 

        foreach($within_group_data as $party_id=>$party_data) //$pay_term
        {
        	$total_balance_quantity=0; $total_delivery_qty_dzn=0; $total_delivery_amount_usd=0;$total_receeive_qty=0;
			 $k=1;
			 $sl=1;
        ...
        <table style="width:1300px" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all">
        <tr>
       	 <td colspan="12" style="font-size:24px"><b>Party Name: <? if($within_group_id==1)
		 { echo $party_com_arr[$party_id];}
			 else if($within_group_id==2){ echo $party_buyer_arr[$party_id];}  ?></b></td>
        </tr>
        </table>               	
        <table style="width:1300px" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" id="scroll_body" >
        <thead>
            ...
            <th width="130" >Type of Wash</th>
             <th width="110" >Receive Qty. (pcs)</th>
            <th width="110" >Delivery Qty. (pcs)</th>
            <th width="110" >Rate (pcs) USD</th>
            <th width="110" >Delivery Amount USD</th>
            <th width="110" >Export Proforma Invoice</th>
        </thead>
        <tbody>
        <?
			...
			{	
				...
				{	
				  ...
				  // $ex_process=array_unique(explode(",",$process_break_id_arr[$row_data['job_dtls_id']]['process_id_arr'])); 
					  ...
					  $ex_process='';
 					  $job_dtls_id=chop($row_data['job_dtls_id'],",");
					  $job_dtls_ids=array_unique(explode(",",$job_dtls_id));
						if($job_dtls_ids!='')
						{
							/*foreach($job_dtls_ids as $val)
							{
								if($ex_process=="") $ex_process=$process_break_id_arr[$val]; else $ex_process.=",".$process_break_id_arr[$val];
							}*/
							
							for ($i=0; $i <count($job_dtls_ids) ; $i++) 
							{ 
								//$ex_process.=$process_break_id_arr[$job_dtls_ids[$i]].",";
								$ex_process.=$process_break_id_arr[$job_dtls_ids[$i]]['process_id_arr'];
					  ...
					  //$ex_process_name=implode(",",array_unique(explode(",",$ex_process)));
					//echo $ex_process;
				    $ex_process_name=array_unique(explode(",",$ex_process)); 
					$process_name=""; $sub_process_name="";
					foreach($ex_process_name as $process_data)
					{
						$ex_process_type=explode("*",$process_data);
						$process_id=$ex_process_type[0];
						$type_id=$ex_process_type[1];
					...
					$sub_process_name=implode(",",array_unique(explode(",",$sub_process_name)));
						...
						{
							$rate=$row_data['rate']*12;
							$ratepcs=$row_data['rate'];
							$delivery_qty_dzn=$delivery_qty/12;
							$delivery_amount_usd=($delivery_qty)*$ratepcs;
						}
						if($row_data['order_uom']==2)
						{
							
							$ratepcs=$row_data['rate']/12;
							$rate=$row_data['rate'];
							$delivery_qty_dzn=$delivery_qty/12;
							$delivery_amount_usd=($delivery_qty)*$ratepcs;
						}
						...
						$receeive_qty=$receive_data_array[$within_group_id][$party_id][$buyer_style_ref_id][$job_no_id][$gmts_color_id][$gmts_item_id]['receive_qnty'];
					?>
					...
					<tr bgcolor="<? echo $bgcolor; ?>" <? echo $stylecolor; ?> onClick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $k; ?>">
	                    ...
	                    <td width="130" align="center"><? echo $garments_item[$row_data['gmts_item_id']];   ?></td>
	                    <td width="130" align="center"><? echo $sub_process_name;//chop($embellishment_type,','); ?></td>
	                    <td width="110"  align="right"><p><a href='#report_details' onClick="openmypage_receive('<? echo $row_data['job_no']; ?>','<? echo $row_data['gmts_color_id']; ?>','<? echo $row_data['gmts_item_id'];?>','<? echo $row_data['order_no']; ?>','<? echo $party_id;?>','<? echo $row_data['buyer_style_ref'];?>','receive_delivery_popup');"><? echo number_format($receeive_qty,2);  ?> </a></p></td>
						<td width="110"  align="right"><p><a href='#report_details' onClick="openmypage_delivery('<? echo $row_data['job_no']; ?>','<? echo $row_data['gmts_color_id']; ?>','<? echo $row_data['gmts_item_id'];?>','<? echo $row_data['order_no']; ?>','<? echo $party_id;?>','<? echo $row_data['buyer_style_ref']; ?>','delivery_receive_popup');"><? echo number_format($delivery_qty,2);  ?> </a></p></td>
	                    <td width="110"  align="right"><?  echo number_format($ratepcs,4);   ?></td>
	                    <td width="110"  align="right"><?  echo number_format($delivery_amount_usd,2);   ?></td>
	                    <td width="110"  align="center"><?  echo $pi_no_arr[$row_data['job_no']][$row_data['order_no']][$row_data['gmts_color_id']];   ?></td>
					</tr>
					<?
	        		$k++;
					$sl++;
					...
					<tbody>
						...
						foreach($result as $row)
						{
							...
							<tr>
                                ...
                                <td width="100" title="<? echo $row[csf("sys_no")];?>"><? echo change_date_format($row[csf("subcon_date")]);?></td>
...
if($action=="receive_delivery_popup")
{
	echo load_html_head_contents("Report Info", "../../../", 1, 1,'','','');
	extract($_REQUEST);
	//echo $po_id;
					...
					<tbody>
						...
						foreach($result as $row)
						{
							...
							<tr>
								<td width="30"><? echo $i;?></td>
 								<td width="100" title="<? echo $row[csf("sys_no")];?>"><? echo $row[csf("sys_no")];?></td>
                                <td width="80" title="<? echo $row[csf("sys_no")];?>"><? echo change_date_format($row[csf("subcon_date")]);?></td>
								<td width="80"><? echo $row[csf("year")];?></td>
								<td width="100"><? echo $party_arr[$row[csf("party_id")]]; ?></td>
								<td  align="right"><? echo number_format($row[csf("quantity")],2);?></td>
							</tr>
							<?
 ...
 if($action=="delivery_popup")
{
	...
	if($db_type==2)
	{
		...
		where a. entry_form = 296 and a.trans_type=1 and a.is_deleted = 0 and a.id=b.mst_id and  b.job_dtls_id=c.id  and c.mst_id=d.id and a.embl_job_no='$job_no' and  a.company_id=$companyID  and c.gmts_item_id='$gmts_item_id' and  c.gmts_color_id='$gmts_color_id' and c.order_no ='$order_no'  and c.buyer_style_ref='$buyer_style_ref' and a.party_id='$party_id'  order by a.subcon_date desc";*/
			...
			if($type=='receive_date_wise')
			{
				$subcon_date ='';
			}
        
		
			$do_sql="select a.id,a.delivery_no, a.delivery_date,a.party_id, a.job_no,$ins_year_cond as year, b.id as details_id ,b.delivery_qty, b.order_id,b.remarks,c.gmts_item_id,c.gmts_color_id,c.order_no,b.remarks, a.delivery_date
			from subcon_delivery_mst a, subcon_delivery_dtls b,subcon_ord_dtls c,subcon_ord_mst d
			where a.entry_form = 303  and a.id=b.mst_id and  b.order_id=c.id and c.mst_id=d.id and a.job_no='$job_no' and  a.company_id=$companyID  and c.gmts_item_id='$gmts_item_id' and  c.gmts_color_id='$gmts_color_id' and c.order_no ='$order_no'  $buyerstyle_ref and a.party_id='$party_id' $subcon_date 
			AND a.is_deleted = 0
						...
						foreach($result as $row)
						{
							...
							<tr>
								<td width="30"><? echo $i;?></td>
 								...
 								<td width="80"><? echo $row[csf("delivery_date")];?></td>
								<td width="80"><? echo $row[csf("year")];?></td>
								<td width="100"><? echo $party_arr[$row[csf("party_id")]]; ?></td>
 ...
 if($action=="delivery_receive_popup")
{
	echo load_html_head_contents("Report Info", "../../../", 1, 1,'','','');
	extract($_REQUEST);
	//echo $po_id;
	...
	if($db_type==2)
	{
		...
		where a. entry_form = 296 and a.trans_type=1 and a.is_deleted = 0 and a.id=b.mst_id and  b.job_dtls_id=c.id  and c.mst_id=d.id and a.embl_job_no='$job_no' and  a.company_id=$companyID  and c.gmts_item_id='$gmts_item_id' and  c.gmts_color_id='$gmts_color_id' and c.order_no ='$order_no'  and c.buyer_style_ref='$buyer_style_ref' and a.party_id='$party_id'  order by a.subcon_date desc";*/
			...
			from subcon_delivery_mst a, subcon_delivery_dtls b,subcon_ord_dtls c,subcon_ord_mst d
			where a.entry_form = 303  and a.id=b.mst_id and  b.order_id=c.id and c.mst_id=d.id and a.job_no='$job_no' and  a.company_id=$companyID  and c.gmts_item_id='$gmts_item_id' and  c.gmts_color_id='$gmts_color_id' and c.order_no ='$order_no'  $buyerstyle_ref and a.party_id='$party_id'  $delivery_date 
			AND a.is_deleted = 0
			AND a.STATUS_ACTIVE = 1
			AND B.is_deleted = 0
			AND B.STATUS_ACTIVE = 1
					...
					<tbody>
						...
						foreach($result as $row)
						{
							...
							<tr>
								<td width="30"><? echo $i;?></td>
 								<td width="100" title="<? echo $row[csf("delivery_no")];?>"><? echo $row[csf("delivery_no")];?></td>
                                <td width="80" title="<? echo $row[csf("delivery_no")];?>"><? echo change_date_format($row[csf("delivery_date")]);?></td>
								<td width="80"><? echo $row[csf("year")];?></td>
								<td width="100"><? echo $party_arr[$row[csf("party_id")]]; ?></td>
								<td  align="right"><? echo number_format($row[csf("delivery_qty")],2);?></td>
							</tr>
