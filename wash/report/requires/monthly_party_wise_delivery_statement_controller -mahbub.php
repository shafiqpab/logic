...
{ 
	...
	$receive_sql = "select  c.rate,c.job_no_mst, c.gmts_item_id, c.gmts_color_id, c.order_no, c.buyer_po_no, c.buyer_style_ref, d.party_id, c.buyer_buyer, c.party_buyer_name,c.id as ord_dtls_id, c.order_uom, c.order_quantity as order_quantity,g.quantity as receive_qnty,c.remarks
  		from sub_material_mst f, sub_material_dtls g,subcon_ord_dtls c,subcon_ord_mst d
 		where  f.id=g.mst_id  and f.entry_form=296  and g.job_dtls_id=c.id  and c.mst_id = d.id $receive_company_name $search_com_cond $within_group $receive_date $party_con and  f.status_active=1 and f.is_deleted=0 and g.status_active=1 and g.is_deleted=0"; 
   	//echo $order_sql;die;
   ...
   $delivery_sql = "select a.id, a.job_no, sum(b.delivery_qty) as delivery_qty, b.order_id, c.job_no_mst, c.gmts_color_id, c.gmts_item_id, b.id as dtls_id, a.delivery_no, a.delivery_date, b.remarks
	...
	$order_con=where_con_using_array($order_id_arr,0,"c.id");
		...
		select  c.rate,c.job_no_mst, c.gmts_item_id, c.gmts_color_id, c.order_no, c.buyer_po_no, c.buyer_style_ref, d.party_id, c.buyer_buyer, c.party_buyer_name,c.id as ord_dtls_id, c.order_uom, c.order_quantity as order_quantity,d.within_group
 		...
 		where  a.id=b.mst_id  and a.entry_form=303  and b.order_id=c.id  and c.mst_id = d.id   $company_name $search_com_cond $within_group $delivery_date $party_con and  a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0"; 
				...
				{	
				   ...
				   // $ex_process=array_unique(explode(",",$row_data['process'])); 
					...
					$sub_process_name=implode(",",array_unique(explode(",",$sub_process_name)));
						...
						$receeive_qty=$receive_data_array[$row_data['job_dtls_id']]['receive_qnty'];
					?>
 ...
 if($action=="delivery_receive_popup")
{
	...
	if($db_type==2)
	{
		...
		where a. entry_form = 296 and a.trans_type=1 and a.is_deleted = 0 and a.id=b.mst_id and  b.job_dtls_id=c.id  and c.mst_id=d.id and a.embl_job_no='$job_no' and  a.company_id=$companyID  and c.gmts_item_id='$gmts_item_id' and  c.gmts_color_id='$gmts_color_id' and c.order_no ='$order_no'  and c.buyer_style_ref='$buyer_style_ref' and a.party_id='$party_id'  order by a.subcon_date desc";*/
		 ...
		 $do_sql="select a.id,a.delivery_no, a.delivery_date,a.party_id, a.job_no,$ins_year_cond as year, b.id as details_id ,b.delivery_qty, b.order_id,b.remarks,c.gmts_item_id,c.gmts_color_id,c.order_no,b.remarks
			from subcon_delivery_mst a, subcon_delivery_dtls b,subcon_ord_dtls c,subcon_ord_mst d
			where a.entry_form = 303 and a.is_deleted = 0  and a.id=b.mst_id and  b.order_id=c.id and c.mst_id=d.id and a.job_no='$job_no' and  a.company_id=$companyID  and c.gmts_item_id='$gmts_item_id' and  c.gmts_color_id='$gmts_color_id' and c.order_no ='$order_no'  $buyerstyle_ref and a.party_id='$party_id'  $delivery_date  order by a.delivery_date desc"; 
