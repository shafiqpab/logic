...
<script>
	...
	function fn_report_generated(operation)
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_party_name*txt_job_no*txt_date_from*txt_date_to*cbo_within_group*cbo_type*txt_search_string*cbo_job_year',"../../")+'&report_title='+report_title;
		freeze_window(operation);
		//freeze_window(3);
		http.open("POST","requires/wash_received_and_delivery_statement_controller.php",true);
	...
	function fn_report_generated_all(operation)
	{
		...
		{
			var report_title=$( "div.form_caption" ).html();
			var data="action=report_generate_all"+get_submitted_data_string('cbo_company_id*cbo_party_name*txt_job_no*txt_date_from*txt_date_to*cbo_within_group*cbo_type*txt_search_string*cbo_job_year',"../../")+'&report_title='+report_title;
			freeze_window(operation);
			//freeze_window(3);
			http.open("POST","requires/wash_received_and_delivery_statement_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	...
	{ 
		...
		var page_link="requires/wash_received_and_delivery_statement_controller.php?action=job_no_popup&company_id="+company_name+"&cbo_buyer_name="+cbo_buyer_name+"&cbo_within_group="+cbo_within_group;
	...
	function fnc_load_party(type,within_group)
	{
		...
		{
			load_drop_down( 'requires/wash_received_and_delivery_statement_controller', company+'_'+1, 'load_drop_down_buyer', 'buyer_td' );
		}
		else if(within_group==2 && type==1)
		{
			load_drop_down( 'requires/wash_received_and_delivery_statement_controller', company+'_'+2, 'load_drop_down_buyer', 'buyer_td' );
		}
	}
	...
	function print_report_button_setting(report_ids) 
    {
        ...
        report_id.forEach(function(items){
            ...
            else if(items==725){$('#search2').show();}
            });
    }
         ...
         <fieldset style="width:1000px;">
            <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                <thead>                    
                    ...
                    <th width="50">Year</th>
                    <th width="130" class="must_entry_caption">Job No</th>
                    <th width="100" style="display:none">Search By</th>
                    <th width="100" id="search_by_td" style="display:none">Buyer Po</th>
                ...
                <tbody>
                    <tr >
                        <td  align="center"> 
                            <?
								echo create_drop_down( "cbo_company_id", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "fnc_load_party(1,document.getElementById('cbo_within_group').value); get_php_form_data(this.value,'print_button_variable_setting','requires/wash_received_and_delivery_statement_controller' );");
                            ?>
                        </td>
                        ...
                        <td>
                        	<?php
                        		echo create_drop_down( 'cbo_job_year', 60, $year, '', 0, '', date('Y'), '', '', '');
                        	?>
                        </td>
                        <td>
						...
						<?
                            $search_by_arr=array(1=>"Buyer Po",2=>"Buyer Style");
                    ...
                    <td align="center" style="display:none">
                        ...
                        <td align="center">
                            <input type="button" id="search1" name="search1" class="formbutton" style="width:70px" value="Show" onClick="fn_report_generated(1)" />
                            <input type="button" id="search2" name="search2" class="formbutton" style="width:70px" value="All" onClick="fn_report_generated_all(1)" />
                        </td>
