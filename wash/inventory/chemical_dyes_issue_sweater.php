<?
/************************************************Comments************************************
Purpose			: 	This form will create Dyes Chemical Issue [Sweater]
Functionality	:	
JS Functions	:
Created by		:	Kausar 
...
echo load_html_head_contents("Dyes Chemical Issue [Sweater]","../../", 1, 1, $unicode,'',''); 
//--------------------------------------------------------------------------------------------------------------------
...
<script>
	...
	function fn_independent(val)
	{
		...
		if(val==4)
		{
			...
			$('#txt_batch_weight').val("");
			$('#txt_tot_liquor').val("");
			load_drop_down( 'requires/chemical_dyes_issue_sweater_controller', $('#cbo_company_name').val()+"**"+$('#cbo_issue_basis').val(), 'load_drop_down_color', 'sub_process_td');
	...
	function fnc_chemical_dyes_issue_entry(operation)
	{
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+$('#txt_mrr_no').val()+'*'+$('#cbo_template_id').val(),'chemical_dyes_issue_print','requires/chemical_dyes_issue_sweater_controller');
			return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			{
				if( form_validation('cbo_company_name*cbo_issue_basis*txt_issue_date*cbo_store_name','Company Name*Issue Basis*Issue Date*Store Name')==false )
				{
					return;
				}
			}
			...
			var dataString = "txt_mrr_no*update_id*cbo_company_name*cbo_location_name*cbo_issue_basis*cbo_issue_purpose*cbo_loan_party*txt_req_no*txt_req_id*txt_issue_date*txt_recipe_no*txt_recipe_id*txt_job_no*txt_order_no*hidden_order_id*cbo_store_name*txt_buyer*cbo_sub_process*txt_buyer_style*variable_lot";
			...
			http.open("POST","requires/chemical_dyes_issue_sweater_controller.php",true);
	...
	{	
		if(http.readyState == 4) 
		{
			...
			if((reponse[0]==0 || reponse[0]==1))
			{
				...
				show_list_view(reponse[1], 'recipe_item_details', 'recipe_items_list_view','requires/chemical_dyes_issue_sweater_controller', '' );
	...
	function open_reqpopup()
	{
		...
		var store_id = 0;
		var page_link='requires/chemical_dyes_issue_sweater_controller.php?action=req_popup&company='+company+'&issue_purpose='+issue_purpose+'&issue_basis='+issue_basis+'&store_id='+store_id;
		...
		emailwindow.onclose=function()
		{
			...
			$("#txt_buyer").val(requ_data_ref[8]);
			
			$('#cbo_company_name').attr('disabled','disabled');
			document.getElementById("list_container_recipe_items").innerHTML = "";
			//load_drop_down( 'requires/chemical_dyes_issue_sweater_controller', company+"**"+requ_data_ref[1], 'load_drop_down_sub_process', 'sub_process_td');
			load_drop_down( 'requires/chemical_dyes_issue_sweater_controller', company+"**"+requ_data_ref[2], 'load_drop_down_sub_process', 'sub_process_td');
		}
	}
	...
	function fnc_item_details(sub_process_id,is_update,store_id)
	{
		...
		if(is_update!="")
		{
			load_drop_down( 'requires/chemical_dyes_issue_sweater_controller', sub_process_id+"**"+is_update, 'load_drop_down_sub_process_up', 'sub_process_td');
			
		}
		$('#cbo_sub_process').val(sub_process_id);
		var hidden_posted_account=$("#hidden_posted_account").val();
		var variable_lot=$("#variable_lot").val();
		show_list_view(cbo_company_id+'**'+sub_process_id+"**"+txt_recipe_id+"**"+cbo_issue_basis+"**"+is_update+"**"+txt_req_id+"**"+cbo_store_name+"**"+hidden_posted_account+"**"+variable_lot, 'item_details', 'list_container_recipe_items', 'requires/chemical_dyes_issue_sweater_controller', '');
		if(is_update !="")
		{
	...
	function open_mrrpopup()
	{
		...
		var company = $("#cbo_company_name").val();	
		var page_link='requires/chemical_dyes_issue_sweater_controller.php?action=mrr_popup&company='+company; 
		var title="Search Issue Number Popup";
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=850px,height=460px,center=1,resize=0,scrolling=0','../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0]; 
			...
			document.getElementById("list_container_recipe_items").innerHTML = "";
			show_list_view(mrrNumber[1], 'recipe_item_details', 'recipe_items_list_view', 'requires/chemical_dyes_issue_sweater_controller', '' ) ;
			get_php_form_data(mrrNumber[1], "populate_data_from_data", "requires/chemical_dyes_issue_sweater_controller");
        ...
        <div style="width:100%;">
            ...
            <legend>Dyes And Chemical Issue</legend>
                ...
                <fieldset style="width:850px;">                                       
                    <table  width="850" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                        ...
                        <tr>
                            ...
                            <td width="170"><input type="text" name="txt_issue_date" id="txt_issue_date" class="datepicker" style="width:160px;" value="<? echo date("d-m-Y"); ?>" placeholder="Select Date" /></td>
                            <td width="130" align="right" class="must_entry_caption">Company</td>
                            <td width="170"><? echo create_drop_down( "cbo_company_name", 170, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond $company_credential_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "reset_form('chemicaldyesissue_1','list_container_yarn*list_container_recipe_items','','','','txt_issue_date*cbo_company_name'); load_drop_down( 'requires/chemical_dyes_issue_sweater_controller', this.value, 'load_drop_down_location', 'location_td'); load_drop_down( 'requires/chemical_dyes_issue_sweater_controller', this.value, 'load_drop_down_loan_party', 'loan_party_td'); get_php_form_data( this.value, 'company_wise_report_button_setting','requires/chemical_dyes_issue_sweater_controller'); location_select();"); ?>
                            <input type="hidden" id="variable_lot" name="variable_lot" />
                            </td>
                            <td width="130" align="right">Location</td>
                            <td id="location_td"><? echo create_drop_down( "cbo_location_name", 170, $blank_array,"", 1, "-- Select Location --", 0, "" ); ?></td>
                        </tr>
                        <tr>
                            <td align="right" class="must_entry_caption">Issue Basis</td>
                            <td><? echo create_drop_down( "cbo_issue_basis", 170, $receive_basis_arr,"", 1, "- Select Receive Basis -", $selected, "fn_independent(this.value),load_drop_down('requires/chemical_dyes_issue_sweater_controller',this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_store', 'store_td' );","","4,7" ); ?></td>
                            <td align="right">Issue Purpose </td>
                            <td><? echo create_drop_down( "cbo_issue_purpose", 170, $general_issue_purpose,"", 1, "-- Select Purpose --", "", "", "","1,5,15,61,63,64,69"); ?></td>
                            <td align="right">Loan/Sales Party</td>
                            <td id="loan_party_td"><? echo create_drop_down( "cbo_loan_party", 170, $blank_array,"", 1, "- Select Loan Party -", $selected, "","","" ); ?></td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <input class="text_boxes" type="text" name="txt_recipe_no" id="txt_recipe_no" style="width:160px;" disabled />
                            ...
                            <td><input class="text_boxes"  type="text" name="txt_job_no" id="txt_job_no" style="width:160px;" placeholder="Display" readonly /></td>
                        </tr>
                        <tr>
                            ...
                            <td id="store_td"><? 
							echo create_drop_down( "cbo_store_name", 170, "select a.id,a.store_name from lib_store_location a,lib_store_location_category b where a.id=b.store_location_id and a.status_active=1 and a.is_deleted=0 and b.category_type in(5,6,7,23) $store_location_credential_cond group by a.id,a.store_name order by a.store_name","id,store_name", 1, "-- Select Store --", "", "fn_sub_process_enable(this.value);", "",""); ?></td>
                        	<td align="right">Buyer</td>
                            <td>
                                <input class="text_boxes"  type="text" name="txt_buyer" id="txt_buyer" placeholder="Display" style="width:160px;" readonly />
                            </td>
                        </tr>
                        <tr>
                            <td align="right" class="must_entry_caption" id="sub_process_caption">Color</td>
                            <td id="sub_process_td"><? echo create_drop_down( "cbo_sub_process", 170, $blank_array,"", 1, "-- Select--", "", "",1); ?></td>
                            <td align="right">Style Ref</td>
                            <td><input class="text_boxes" type="text" name="txt_buyer_style" id="txt_buyer_style" placeholder="Display" style="width:160px;" readonly /></td>
                        </tr>
                    </table>
                </fieldset>
                <table cellpadding="0" cellspacing="1" width="100%">
                    ...
                    <tr>
                        <td align="center" colspan="6" valign="middle" class="button_container"><? echo create_drop_down( "cbo_template_id", 100, $report_template_list,'', 0, '', 0, "");?>&nbsp;
