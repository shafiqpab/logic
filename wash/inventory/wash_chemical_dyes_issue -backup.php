...
<script>
	...
	function fn_independent(val)
	{
		...
		if(val==4)
		{
			...
			$('#txt_batch_weight').val("");
			$('#txt_tot_liquor').val("");
			load_drop_down( 'requires/wash_chemical_dyes_issue_controller', $('#cbo_company_name').val(), 'load_drop_down_color', 'sub_process_td');
	...
	function fnc_chemical_dyes_issue_entry(operation)
	{
		if(operation==4)
		{
			...
			generate_report_file( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+$('#txt_mrr_no').val(),'chemical_dyes_issue_print','requires/wash_chemical_dyes_issue_controller');
			return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			var dataString = "txt_mrr_no*update_id*cbo_company_name*cbo_location_name*cbo_issue_basis*cbo_issue_purpose*cbo_loan_party*txt_req_no*txt_req_id*txt_issue_date*txt_recipe_no*txt_recipe_id*txt_job_no*txt_order_no*hidden_order_id*cbo_store_name*txt_buyer_po*hidden_buyer_po_id*cbo_sub_process*txt_pest_weight*txt_buyer_style";
			...
			for (var i=1; i<row_num; i++)
			{
				if(($('#txt_reqn_qnty_edit_'+i).val()*1)>0)
				{
					dtls_data=dtls_data+get_submitted_data_string('txt_prod_id_'+i+'*txt_item_cat_'+i+'*txt_recipe_qnty_'+i+'*txt_reqn_qnty_'+i+'*txt_reqn_qnty_edit_'+i+'*updateIdDtls_'+i+'*transId_'+i+'*hidtxt_reqn_qnty_edit_'+i,"../../",2);
				}
			}
	...
	function open_reqpopup()
	{
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/wash_chemical_dyes_issue_controller', company+"**"+requ_data_ref[2], 'load_drop_down_sub_process', 'sub_process_td');
		}
	}
	...
	function fnc_item_details(sub_process_id,is_update,store_id)
	{
		...
		show_list_view(cbo_company_id+'**'+sub_process_id+"**"+txt_recipe_id+"**"+cbo_issue_basis+"**"+is_update+"**"+txt_req_id+"**"+cbo_store_name+"**"+hidden_posted_account, 'item_details', 'list_container_recipe_items', 'requires/wash_chemical_dyes_issue_controller', '');
		if(is_update !="")
		{
                ...
                <fieldset style="width:850px;">                                       
                    <table  width="850" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                        ...
                        <tr>
                            ...
                            <td width="170">
                            <? 
                            echo create_drop_down( "cbo_company_name", 170, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond $company_credential_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "reset_form('chemicaldyesissue_1','list_container_yarn*list_container_recipe_items','','','','txt_issue_date*cbo_company_name');load_drop_down( 'requires/wash_chemical_dyes_issue_controller', this.value, 'load_drop_down_location', 'location_td');load_drop_down( 'requires/wash_chemical_dyes_issue_controller', this.value, 'load_drop_down_store', 'store_td' );load_drop_down( 'requires/wash_chemical_dyes_issue_controller', this.value, 'load_drop_down_loan_party', 'loan_party_td');" );
                            ?>
                            </td>
                         ...
                         <tr>
                            ...
                            <td>
                            <? 
                            echo create_drop_down( "cbo_issue_basis", 170, $receive_basis_arr,"", 1, "- Select Receive Basis -", $selected, "fn_independent(this.value)","","4,7" );//fn_independent(this.value)
                            ?>
                            </td>
                            <td align="right">Issue Purpose </td>
                            <td>
                            <? 
                            echo create_drop_down( "cbo_issue_purpose", 170, $yarn_issue_purpose,"", 1, "-- Select Purpose --", "", "", "","3,5,26,30,32,33,40");
                            ?>
                            </td>
