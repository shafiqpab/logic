...
if ($action=="load_drop_down_location")
{
	...
	echo create_drop_down( "cbo_location", 140, $sql,"id,location_name", 1, "-- Select Location --", $selected, " load_drop_down( 'requires/wash_chemical_dyes_issue_return_controller', this.value+'_'+document.getElementById('cbo_company_id').value, 'load_drop_down_floor', 'floor_id_td');",0 );     	 
	exit();
}
...
{
	load_room_rack_self_bin("requires/wash_chemical_dyes_issue_return_controller",$data);
}
...
if($action=="create_item_search_list_view")
{
	...
	where a.id=b.mst_id and a.status_active=1 and b.prod_id=c.id and c.status_active=1  and b.transaction_type=2
	$sql_cond $item_category $item_descrition $product_cond order by a.id ";
	
	//echo $sql;
		  
	$item_group_arr=return_library_array( "select id, item_name from lib_item_group where item_category in(5,6,7,22,23)",'id','item_name');
...
{  
	...
	$sql = "select  c.id as prod_id,product_name_details,c.unit_of_measure,c.item_group_id,b.company_id,b.item_category,b.cons_amount,b.cons_rate,a.challan_no,
			b.cons_quantity,a.issue_number,b.store_id, a.floor_id, a.room, a.rack,a.self
			from inv_issue_master a, inv_transaction b, product_details_master c
  ...
  // echo $sql;
	...
	foreach($res as $row)
	{
		...
		echo "load_drop_down( 'requires/wash_chemical_dyes_issue_return_controller', '".$row[csf('company_id')]."', 'load_drop_down_store', 'store_td' );\n";
		echo "$('#cbo_store_name').val(".$row[csf("store_id")].");\n";
		echo "load_room_rack_self_bin('requires/wash_chemical_dyes_issue_return_controller', 'floor','floor_td', '".$row[csf('company_id')]."','"."','".$row[csf('store_id')]."',this.value);\n";
		echo "$('#cbo_floor').val('".$row[csf("floor_id")]."');\n";
		echo "load_room_rack_self_bin('requires/wash_chemical_dyes_issue_return_controller', 'room','room_td', '".$row[csf('company_id')]."','"."','".$row[csf('store_id')]."','".$row[csf('floor_id')]."',this.value);\n";
		echo "$('#cbo_room').val('".$row[csf("room")]."');\n";
		echo "load_room_rack_self_bin('requires/wash_chemical_dyes_issue_return_controller', 'rack','rack_td', '".$row[csf('company_id')]."','"."','".$row[csf('store_id')]."','".$row[csf('floor_id')]."','".$row[csf('room')]."',this.value);\n";
		echo "$('#txt_rack').val('".$row[csf("rack")]."');\n";
		echo "load_room_rack_self_bin('requires/wash_chemical_dyes_issue_return_controller', 'shelf','shelf_td', '".$row[csf('company_id')]."','"."','".$row[csf('store_id')]."','".$row[csf('floor_id')]."','".$row[csf('room')]."','".$row[csf('rack')]."',this.value);\n";	
...
if($action=="batch_popup")
{
	...
	{ 
		 //alert(data) 
		var data=data.split("**");
		document.getElementById('hidden_issue_no').value=data[0];
		document.getElementById('hidden_issue_id').value=data[1];
		document.getElementById('hidden_location_id').value=data[2];
		document.getElementById('hidden_store_id').value=data[3];
		parent.emailwindow.hide();
	}
				...
				<table cellpadding="0" cellspacing="0" width="500" class="rpt_table">
					<thead>
						...
						<th>
							...
							<input type="hidden" name="hidden_issue_id" id="hidden_issue_id" value="">
							<input type="hidden" name="hidden_issue_no" id="hidden_issue_no" value="">
                            <input type="hidden" name="hidden_location_id" id="hidden_location_id" value="">
					...
					<tr class="general">
						<td align="center">	
							<?
								$search_by_arr=array(1=>"Issue No",2=>"Requsign No");
						...
						<td align="center">
							<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+<? echo $cbo_company_id; ?>, 'create_batch_search_list_view', 'search_div', 'wash_chemical_dyes_issue_return_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
						</td>
					</tr>
				</table>
...
if($action=="create_batch_search_list_view")
{
	...
	if($search_string!='')
	{
		if($search_by==1) $search_field=" and a.issue_number like '%".$search_string."'";
		else $search_field="a.req_no like '%".$search_string."'";
	}
	// ,b.id as batch_id,b.requ_no,b.id as requ_id
	$sql ="select a.id as mst_id,a.issue_number,a.issue_basis,a.issue_purpose,a.location_id,a.req_no,a.order_id,a.challan_no,a.store_id 
	from inv_issue_master a where a.company_id=$company_id and a.status_active=1 and a.is_deleted=0 and a.entry_form in(298,392) $search_field"; 	
	//echo $sql;die;
				...
				foreach($sql_data as $row)
				  {					
					...
					// }
					 ...
					 ?>
                     <tr bgcolor="<? echo $bgcolor; ?>" onClick='js_set_value("<? echo $row[csf("issue_number")].'**'.$row[csf("mst_id")].'**'.$row[csf("location_id")].'**'.$row[csf("store_id")];?>")' style="cursor:pointer" >
                        <td width="30"><? echo $i; ?></td>
                        <td width="130"  align="center"><p><? echo $row[csf("issue_number")]; ?></p></td>
						<td width="100" align="center"><p><? echo $receive_basis_arr[$row[csf("issue_basis")]]; ?></p></td>
                        <td width="120"  align="center"><p><? echo $row[csf("req_no")]; ?></p></td>
                        <td width="120"  align="center"><p><? echo $general_issue_purpose[$row[csf("issue_purpose")]]; ?></p></td>
...
if($action=="populate_issue_no_data")
{ 
	 $data=explode("**",$data);
	 $company_name=$data[0];
	 $mst_id=$data[1];
	 ...
	 where b.id=$mst_id and  b.id=a.mst_id and a.prod_id=c.id and a.transaction_type=2 and b.entry_form in(298,392) and b.company_id=$company_name ";
		 ...
		 <table width="430" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
			...
			{		
				...
				{	
					...
					?>
                 	<tr bgcolor="<? echo $bgcolor; ?>" onClick='get_php_form_data("<? echo $row[csf("prod_id")]."_".$row[csf("id")]."_".$mst_id;?>","child_form_item_data","requires/wash_chemical_dyes_issue_return_controller")'  style="cursor:pointer" >
...
if($action=="child_form_item_data")
{
	...
	foreach($res as $row)
	{
		...
		echo "$('#txt_issue_qty').val(".$row[csf("cons_quantity")].");\n";
		echo "$('#txt_net_used').val(".$net_used.");\n";
		echo "$('#hidden_trans_id').val('');\n";
		echo "$('#txt_return_qnty').val('');\n";
		echo "$('#cbo_store_name').val(".$row[csf("store_id")].");\n";
		echo "$('#txt_reject_qnty').val('');\n";
		echo "$('#txt_remarks').val('');\n";
		echo "set_button_status(0, permission, 'fnc_chemical_issue_return_entry',1);\n";
   	}	
	exit();
}
...
{	 
	...
	{
		$store_item_rate=str_replace("'","",$txt_rate);
	}
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		$presentStock=$presentStockValue=$presentAvgRate=$available_qnty=$avg_rate_perunit=0;
		...
		$txt_return_value=$txt_return_qnty*str_replace("'","",$txt_rate);
		...
		$field_array="last_purchased_qnty*current_stock*avg_rate_per_unit*stock_value*updated_by*update_date";
		$data_array="".$txt_return_qnty."*".$nowStock."*".$avg_rate_perunit."*".$nowStockValue."*'".$user_id."'*'".$pc_date_time."'";
		// --Store Wise Stock----//
		...
		$store_return_value=$txt_return_qnty*str_replace("'","",$txt_rate);
		$store_nowStock 		= $store_presentStock+$txt_return_qnty;
		$store_nowStockValue 	= $store_presentStockValue+$store_return_value;
		$store_avg_rate_perunit=$store_nowStockValue/$store_nowStock;
			
		
		//Issue qnty and Issue return qnty chk END  -------------------------------------//
		$totalIssuedQnty = return_field_value("sum(cons_quantity) as cons_quantity","inv_transaction"," prod_id=$txt_prod_id and mst_id=$txt_issue_id and transaction_type=2 and status_active=1","cons_quantity");
			
		$prvTotalIssuedReturn = return_field_value("sum(b.cons_quantity+ NVL(b.cons_reject_qnty, 0) ) as to_issue_return","inv_transaction b","b.prod_id=$txt_prod_id and b.item_category  in(5,6,7,23) and b.transaction_type=4 and b.issue_id=$txt_issue_id and b.status_active=1","to_issue_return");
		
		$iss_rtn_bal=$totalIssuedQnty-$prvTotalIssuedReturn-$txt_return_qnty;
		if($iss_rtn_bal<0)
		{
			echo "20** Returned Not Allow. Returned Qnty can not over Issue Qnty $iss_rtn_bal = $totalIssuedQnty=$prvTotalIssuedReturn=$txt_return_qnty";disconnect($con);die;
		}
		...
		if(str_replace("'","",$txt_return_no)=="")
		{
			...
			$new_recv_number=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '', 'DCIRN', date("Y",time()), 5, "select recv_number_prefix,recv_number_prefix_num from inv_receive_master where company_id=$cbo_company_id and entry_form='373' and $year_id".date('Y',time())." order by recv_number_prefix_num DESC ", "recv_number_prefix", "recv_number_prefix_num" ));*/
			
			$id = return_next_id_by_sequence("INV_RECEIVE_MASTER_PK_SEQ", "inv_receive_master", $con);
			$new_recv_number = explode("*", return_next_id_by_sequence("INV_RECEIVE_MASTER_PK_SEQ", "inv_receive_master",$con,1,$cbo_company_id,'WDCIRN',373,date("Y",time()),0 ));

			
			$field_array1="id, recv_number_prefix, recv_number_prefix_num, recv_number, entry_form,receive_basis, company_id,receive_date,challan_no, shift, store_id,floor,room,rack,shelf, location_id,issue_id, exchange_rate, currency_id,remarks, inserted_by,insert_date";
			$data_array1="(".$id.",'".$new_recv_number[1]."','".$new_recv_number[2]."','".$new_recv_number[0]."',373,".$cbo_issue_basis.",".$cbo_company_id.",".$txt_return_date.",".$txt_return_challan_no.",".$cbo_shift.",".$cbo_store_name.",".$cbo_floor_name.",".$cbo_room.",".$txt_rack.",".$txt_shelf.",".$cbo_location.",".$txt_issue_id.",1,1,".$txt_remarks.",'".$user_id."','".$pc_date_time."')";
		...
		else
		{
			...
			$field_array1="company_id*receive_date*challan_no*shift*store_id*floor*room*rack*shelf*location_id*exchange_rate*currency_id*remarks*updated_by*update_date";
			$data_array1="".$cbo_company_id."*".$txt_return_date."*".$txt_return_challan_no."*".$cbo_shift."*".$cbo_store_name."*".$cbo_floor_name."*".$cbo_room."*".$txt_rack."*".$txt_shelf."*".$cbo_location."*1*1*".$txt_remarks."*'".$user_id."'*'".$pc_date_time."'";
		...
		$field_array2 = "id, mst_id, company_id, prod_id, item_category, batch_lot, transaction_type, transaction_date, store_id, floor_id, room, rack, self, cons_uom, cons_quantity, cons_reject_qnty, cons_rate, cons_amount, balance_qnty, balance_amount, issue_challan_no, issue_id, remarks, inserted_by, insert_date, store_rate, store_amount";
 		$data_array2 = "(".$transactionID.",".$id.",".$cbo_company_id.",".$txt_prod_id.",".$cbo_item_category.",'".str_replace("'","",$txt_lot)."',4,".$txt_return_date.",".$cbo_store_name.",".$cbo_floor.",".$cbo_room.",".$txt_rack.",".$txt_shelf.",".$cbo_uom.",".$txt_return_qnty.",".$txt_reject_qnty.",".$txt_rate.",".$txt_return_value.",".$txt_return_qnty.",".$txt_return_value.",".$txt_return_challan_no.",".$txt_issue_id.",".$txt_remarks.",'".$user_id."','".$pc_date_time."',".number_format($store_item_rate,10,'.','').",".number_format($issue_store_value,8,'.','').")";
		...
		$data_array_store_up="".$txt_return_qnty."*".$store_nowStock."*".$store_avg_rate_perunit."*".$store_nowStockValue."*'".$user_id."'*'".$pc_date_time."'";
		...
		$data_array_store_insert= "(".$sdtlsid.",".$cbo_company_id.",".$cbo_store_name.",".$cbo_floor.",".$cbo_room.",".$txt_rack.",".$txt_shelf.",".$item_category_id.",".$txt_product_id.",".$txt_return_qnty.",".$txt_rate.",".$txt_return_value.",".$txt_return_qnty.",'".$user_id."','".$pc_date_time."','".$dyes_lot."')";
		...
		//echo "10** insert into inv_receive_master ($field_array1) values  $data_array1";oci_rollback($con);disconnect($con);die;
		if(str_replace("'","",$txt_return_no)=="")
		{
		...
		if($store_update_id!='')
		 {
			 ...
			 //if($flag==1) { if($storeUpdate) $flag=1; else $flag=0; } 
		 }
		 else
		 {
			$storeUpdate = sql_insert("inv_store_wise_qty_dtls",$field_array_store_insert,$data_array_store_insert,1); 
			// if($flag==1) { if($sdtlsrID) $flag=1; else $flag=0; } 
		 }
		//echo "10**".$prodUpdate.'='.$rID.'='.$dtlsrID.'='.$dtlsrID.'='.$store_update_id;oci_rollback($con);disconnect($con);die;
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		if($db_type==0)	{ mysql_query("BEGIN"); }
		$flag=0;
		$sql = sql_select("select a.cons_quantity,a.cons_amount,b.current_stock,b.stock_value from inv_transaction a, product_details_master b where a.id=$hidden_trans_id and a.prod_id=b.id");
	    ...
	    $adj_beforeStockValue_store		=$store_presentStockValue-$beforeReturnValue;
	    $adj_beforeAvgRate_Store		=number_format(abs($adj_beforeStockValue_store/$adj_beforeStock_store),$dec_place[3],'.','');
		...
		$update_array="last_purchased_qnty*current_stock*avg_rate_per_unit*stock_value*updated_by*update_date";
		...
		if(str_replace("'","",$txt_prod_id) == str_replace("'","",$before_prod_id))
		{
			...
			$present_avgrate=$presentStockValue/$presentStockQnty;
			$data_array_prod	= "".$txt_return_qnty."*".$presentStockQnty."*".$present_avgrate."*".$presentStockValue."*'".$user_id."'*'".$pc_date_time."'";
		...
		else
		{
			...
			$present_avgrate=$presentStockValue/$presentStockQnty;
			$update_data[$before_prod_id]=explode("*",("".$txt_return_qnty."*".$presentStockQnty."*".$present_avgrate."*".$presentStockValue."*'".$user_id."'*'".$pc_date_time."'"));
			...
			$present_avgrateAfter=$presentStockValueAfter/$presentStockQntyAfter; 

			$update_data[$txt_prod_id]=explode("*",("".$txt_return_qnty."*".$presentStockQntyAfter."*".$present_avgrateAfter."*".$presentStockValueAfter."*'".$user_id."'*'".$pc_date_time."'"));
		...
		if($db_type==0)
		{
			$prvTotalIssuedReturn = return_field_value("sum(b.cons_quantity+ IFNULL(b.cons_reject_qnty, 0) ) as to_issue_return","inv_transaction b","b.prod_id=$txt_prod_id and b.item_category  in(5,6,7,23) and b.transaction_type=4 and b.issue_id=$txt_issue_id and b.id<>$hidden_trans_id and b.status_active=1","to_issue_return");
		}
		else
		{
			$prvTotalIssuedReturn = return_field_value("sum(b.cons_quantity+ NVL(b.cons_reject_qnty, 0) ) as to_issue_return","inv_transaction b","b.prod_id=$txt_prod_id and b.item_category  in(5,6,7,23) and b.transaction_type=4 and b.issue_id=$txt_issue_id and b.id<>$hidden_trans_id and b.status_active=1","to_issue_return");

		}
		$iss_rtn_bal=$totalIssuedQnty-$prvTotalIssuedReturn-$txt_return_qnty;
		if($iss_rtn_bal<0)
		{
			echo "20**Update Not Allow. Returned Qnty can not over Issue Qnty";disconnect($con);die;
		}
		//adjust product master table END  -------------------------------------//
		$field_array="company_id*receive_basis*receive_date*challan_no*shift*location_id*floor*updated_by*update_date";
		$data_array="".$cbo_company_id."*".$cbo_issue_basis."*".$txt_return_date."*".$txt_return_challan_no."*".$cbo_shift."*".$cbo_location."*".$cbo_floor_name."*'".$user_id."'*'".$pc_date_time."'";
	
		if($txt_reject_qnty=="")$txt_reject_qnty=0;
		$txt_amount=((str_replace("'","",$txt_return_qnty))*str_replace("'","",$txt_avrage_rate));
		$field_array1 = "company_id*prod_id*item_category*batch_lot*transaction_type*transaction_date*store_id*floor_id*room*rack*self*cons_uom*cons_quantity*cons_reject_qnty*cons_amount*balance_qnty*balance_amount*issue_id*remarks*updated_by*update_date*store_rate*store_amount";
		$data_array1 = "".$cbo_company_id."*".$txt_prod_id."*".$cbo_item_category."*'".str_replace("'","",$txt_lot)."'*4*".$txt_return_date."*".$cbo_store_name."*".$cbo_floor."*".$cbo_room."*".$txt_rack."*".$txt_shelf."*".$cbo_uom."*".$txt_return_qnty."*".$txt_reject_qnty."*".$txt_amount."*".$txt_return_qnty."*".$txt_amount."*".$txt_issue_id."*".$txt_remarks."*'".$user_id."'*'".$pc_date_time."'*".number_format($store_item_rate,10,'.','')."*".number_format($issue_store_value,8,'.','')."";
		...
		if(str_replace("'","",$txt_prod_id) == str_replace("'","",$before_prod_id))
		{
			...
			$store_presentStockValue  = $store_presentStockValue-$beforeReturnValue+$txt_return_value; 
			$store_present_avgrate=$store_presentStockValue/$store_presentStockQnty;
			$data_array_store_up= "".$txt_return_qnty."*".$store_presentStockQnty."*".$store_present_avgrate."*".$store_presentStockValue."*'".$user_id."'*'".$pc_date_time."'";
		...
		else
		{
			...
			$store_presentStockValue  = $store_presentStockValue-$beforeReturnValue+$txt_return_value; 
			$store_present_avgrate=$store_presentStockValue/$store_presentStockQnty;
			$data_array_store_up[$store_update_id]=explode("*",("".$txt_return_qnty."*".$store_presentStockQnty."*".$store_present_avgrate."*".$store_presentStockValue."*'".$user_id."'*'".$pc_date_time."'"));
			//$data_array_store_up= "".$txt_return_qnty."*".$store_presentStockQnty."*".$store_present_avgrate."*".$store_presentStockValue."*'".$user_id."'*'".$pc_date_time."'";
		}
 		$field_array_store_up="last_purchased_qnty*cons_qty*rate*amount*updated_by*update_date";
		$field_array_store_insert="id,company_id,store_id,floor,room,rack,shelf,category_id,prod_id,cons_qty,rate,amount,last_purchased_qnty,inserted_by,insert_date,lot"; 
        ...
        if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");  
				echo "1**".str_replace("'","",$txt_return_no)."**".$update_id."**".$txt_prod_id;
			}
			else
			{
				mysql_query("ROLLBACK"); 
		...
		<table width="880" cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
				...
				<tbody>
					<tr>
						...
						<td align="center">
							<input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view (document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $company; ?>, 'create_return_search_list_view', 'search_div', 'wash_chemical_dyes_issue_return_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />				
						</td>
				</tr>
				<tr>                  
					<td align="center" height="40" valign="middle" colspan="5">
...
if($action=="create_return_search_list_view")
{
	...
	,a.batch_id as batch
			...
			where a.entry_form=373 and a.company_id=$company $sql_cond order by a.id DESC"; 
 //echo $sql;
            ...
            <tbody>
				...
				{					
					...
					// if($row[csf("receive_basis")]==7) { $batch_no=$req_arr[$row[csf("batch")]];}
 				    ...
 				    ?>
                 	<tr bgcolor="<? echo $bgcolor; ?>" onClick='js_set_value("<? echo $row[csf("recv_number")]."**".$row[csf("mst_id")];?>")' style="cursor:pointer" >
                        <td width="30"><? echo $i; ?></td>
...
if($action=="populate_issue_no_data_update")
{ 
	 $data=explode("**",$data);
	 $company_name=$data[0];
	 $receive_id=$data[1];
     ...
     <table width="400" cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
			...
			{		
		        ...
		        if($i%2==0) $bgcolor="#E9F3FF";
 				...
 				?>
                 	<tr bgcolor="<? echo $bgcolor; ?>" onClick='get_php_form_data("<? echo $row[csf("prod_id")]."_".$row[csf("id")]."_".$issue_sql[0][csf("issue_id")];?>","child_form_item_data","requires/wash_chemical_dyes_issue_return_controller")'  style="cursor:pointer" >
                        <td  align="center"><? echo $i; ?></td>
...
{
	$iss_arr=return_library_array( "select id, issue_number from inv_issue_master where entry_form in(298,392)",'id','issue_number');  
 	$sql = "SELECT b.id,b.recv_number,b.company_id,b.receive_date,b.challan_no,b.receive_basis,b.location_id,b.floor,b.batch_id,b.is_posted_account,b.shift,b.issue_id from inv_receive_master b where b.recv_number='$data' and b.entry_form=373";
	//echo $sql;
	...
	foreach($res as $row)
	{
		...
		echo "load_drop_down( 'requires/wash_chemical_dyes_issue_return_controller', document.getElementById('cbo_location').value+'_'+document.getElementById('cbo_company_id').value, 'load_drop_down_floor', 'floor_id_td');\n";
		
		echo "$('#cbo_floor_name').val(".$row[csf("floor")].");\n";
		echo "$('#cbo_shift').val(".$row[csf("shift")].");\n";
		...
		echo "$('#txt_issue_name').val('".$iss_arr[$row[csf("issue_id")]]."');\n";
...
if($action=="show_dtls_list_view")	
{
	...
	$sql = "select b.id as tr_id, a.location_id, a.recv_number, a.company_id, a.receive_date, a.store_id, a.receive_basis, b.id, b.cons_quantity, b.cons_reject_qnty, b.cons_uom, b.cons_rate, b.cons_amount, c.product_name_details, c.id as prod_id   
	       ...
	       where a.id=b.mst_id and a.id='$return_number' and b.transaction_type=4 and a.entry_form=373 and a.status_active=1 and b.status_active=1 $cond";	
	  //echo $sql; 	
	$result = sql_select($sql);
	$i=1;
	$rettotalQnty=0;
     	...
     	<table class="rpt_table" border="1" cellpadding="2" cellspacing="0" style="width:850px" >
            ...
            <tbody>
				...
				foreach($result as $row){					
 				...
 				?>
                 	<tr bgcolor="<? echo $bgcolor; ?>" onClick='get_php_form_data("<? echo $row[csf("recv_number")]."**".$row[csf("prod_id")]."**".$row[csf("tr_id")]."**".$hidden_posted_account;?>","child_form_input_data_update","requires/wash_chemical_dyes_issue_return_controller")' style="cursor:pointer" >
                        <td width="30"><? echo $i; ?></td>
...
{ 
	...
	and c.entry_form=373 and a.prod_id=b.id and b.status_active=1";
	$result = sql_select($sql);
	foreach($result as $row)
	{   
		echo "$('#txt_item_description').val('".$row[csf("product_name_details")]."');\n";
...
{ 
    $data=explode('**',$data);
  	...
  	$sql = "select b.id as prod_id, b.product_name_details, b.lot, a.id as tr_id, a.store_id, a.cons_uom, a.cons_rate, a.cons_quantity,a.cons_reject_qnty, a.cons_amount,    a.issue_challan_no,a.remarks,a.item_category,a.machine_id,b.item_group_id,c.receive_basis,c.location_id,c.receive_date,c.receive_basis,c.challan_no,c.company_id,c.batch_id
			from inv_transaction a, product_details_master b, inv_receive_master c
 			where c.recv_number='".$data[0]."' and b.id=".$data[1]." and a.status_active=1 and a.id=".$data[3]."  and c.id=a.mst_id and transaction_type=4 and c.entry_form=373 and a.prod_id=b.id and b.status_active=1";
   //echo $sql;
	$result = sql_select($sql);
	foreach($result as $row)
	   {   
...
if ($action=="issue_return_print")
{
	...
	$sql=" select id, recv_number, receive_basis, batch_id, knitting_source, knitting_company, challan_no, receive_date, floor from  inv_receive_master where id='$data[3]' and entry_form=373";
	...
	if($dataArray[0][csf('receive_basis')]==5) $batch_req=$batch_arr[$dataArray[0][csf('batch_id')]];
	else if($dataArray[0][csf('receive_basis')]==7) $batch_req=$req_arr[$dataArray[0][csf('batch_id')]];
	else $batch_req="";
    ...
    <div style="width:930px;">
        <table width="900" cellspacing="0" align="right">
            ...
            <tr>
                ...
                <td><strong>Floor/Unit:</strong></td> <td><? echo $floor_library[$dataArray[0][csf('floor')]]; ?></td>
            </tr>
        </table>
     <br>
        <div style="width:100%;">
        ...
        <br>
         <?
            echo signature_table(331, $data[0], "900px",1, 50, $user_id);
         ?>
        </div>
	</div>
	<?
