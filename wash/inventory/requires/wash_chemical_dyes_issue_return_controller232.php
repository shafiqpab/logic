...
{  
  ...
  // echo $sql;
	...
	foreach($res as $row)
	{
		...
		echo "load_room_rack_self_bin('requires/wash_chemical_dyes_issue_return_controller*5_6_7_23', 'store','store_td', '".$row[csf('company_id')]."','"."',this.value);\n";
...
<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            <table cellpadding="0" cellspacing="0" width="500" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td align="center">
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+<? echo $cbo_company_id; ?>+'_'+<? echo $cbo_issue_basis; ?>, 'create_batch_search_list_view', 'search_div', 'wash_chemical_dyes_issue_return_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:100px;" />
                    </td>
                </tr>
            </table>
...
if($action=="create_batch_search_list_view")
{
	...
	if($issue_basis==5)
	{
		...
		a.entry_form=298 $batch_relation $search_field"); 
	}
	else
	{
		...
		and a.entry_form=298  $search_field"); */
		
		 $sql = sql_select("select a.id as mst_id,a.issue_number,b.id as batch_id,b.requ_no,b.id as requ_id,a.issue_purpose,a.location_id,a.order_id,a.challan_no 
		from inv_issue_master a, dyes_chem_issue_requ_mst b where a.company_id=$company_id and a.req_id=b.id  and a.status_active=1 and a.is_deleted=0
		and a.entry_form=298 and b.entry_form=299  $search_field"); 	
	}
	?>
...
{ 
	 ...
	 where b.id=$mst_id and  b.id=a.mst_id and a.prod_id=c.id and a.transaction_type=2 and b.entry_form=298 and b.company_id=$company_name ";
...
{ 
	 ...
	 $sql="select a.id, a.prod_id, a.cons_quantity as issue_qty, c.product_name_details, c.item_category_id, c.item_group_id, a.batch_lot
	 from inv_transaction a, inv_issue_master b, product_details_master c 
	 where b.id=$mst_id and b.id=a.mst_id and a.prod_id=c.id and a.transaction_type=2 and b.entry_form=298";
	//echo $sql;
	$item_result=sql_select($sql);
	if(count($item_result)>0)
	{
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$txt_return_no)=="")
		{
			...
			$new_recv_number=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '', 'DCIRN', date("Y",time()), 5, "select recv_number_prefix,recv_number_prefix_num from inv_receive_master where company_id=$cbo_company_id and entry_form='29' and $year_id".date('Y',time())." order by recv_number_prefix_num DESC ", "recv_number_prefix", "recv_number_prefix_num" ));*/
			...
			$data_array1="(".$id.",'".$new_recv_number[1]."','".$new_recv_number[2]."','".$new_recv_number[0]."',373,".$cbo_issue_basis.",".$cbo_company_id.",".$txt_return_date.",".$txt_return_challan_no.",".$cbo_store_name.",".$cbo_floor.",".$cbo_room.",".$txt_rack.",".$txt_shelf.",".$cbo_location.",".$txt_batch_id.",1,1,".$txt_remarks.",'".$user_id."','".$pc_date_time."')";
		...
		$field_array2 = "id,mst_id,company_id,prod_id,item_category,batch_lot,transaction_type,transaction_date,store_id,floor_id,room, rack,self,cons_uom,cons_quantity,
		cons_reject_qnty,cons_rate,cons_amount,balance_qnty,balance_amount,issue_challan_no,issue_id,remarks,inserted_by,insert_date";
 		$data_array2 = "(".$transactionID.",".$id.",".$cbo_company_id.",".$txt_prod_id.",".$cbo_item_category.",'".str_replace("'","",$txt_lot)."',4,".$txt_return_date.",".$cbo_store_name.",".$cbo_floor.",".$cbo_room.",".$txt_rack.",".$txt_shelf.",".$cbo_uom.",".$txt_return_qnty.",".$txt_reject_qnty.",".$txt_rate.",".$txt_return_value.",".$txt_return_qnty.",".$txt_return_value.",".$txt_return_challan_no.",".$hidden_issue_id.",".$txt_remarks.",'".$user_id."','".$pc_date_time."')";
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		if($issue_id_check!="")
		{
			echo "20**Update Not Allow, This Item Found In Issue Entry.";die;
		}
		...
		$field_array="company_id*receive_basis*receive_date*challan_no*location_id*updated_by*update_date";
		$data_array="".$cbo_company_id."*".$cbo_issue_basis."*".$txt_return_date."*".$txt_return_challan_no."*".$cbo_location."*'".$user_id."'*'".$pc_date_time."'";
	
		if($txt_reject_qnty=="")$txt_reject_qnty=0;
		$txt_amount=((str_replace("'","",$txt_return_qnty))*str_replace("'","",$txt_avrage_rate));
		$field_array1 = "company_id*prod_id*item_category*batch_lot*transaction_type*transaction_date*store_id*floor_id*room*rack*self*cons_uom*cons_quantity*cons_reject_qnty*cons_amount*balance_qnty*balance_amount*issue_id*remarks*updated_by*update_date";
		$data_array1 = "".$cbo_company_id."*".$txt_prod_id."*".$cbo_item_category."*'".str_replace("'","",$txt_lot)."'*4*".$txt_return_date."*".$cbo_store_name."*".$cbo_floor."*".$cbo_room."*".$txt_rack."*".$txt_shelf."*".$cbo_uom."*".$txt_return_qnty."*".$txt_reject_qnty."*".$txt_amount."*".$txt_return_qnty."*".$txt_amount."*".$hidden_issue_id."*".$txt_remarks."*'".$user_id."'*'".$pc_date_time."'";
...
{  
 	$sql = "select b.id,b.recv_number,b.company_id,b.receive_date,b.challan_no,b.receive_basis,b.location_id,b.batch_id,b.is_posted_account
			...
			where b.recv_number='$data' and b.entry_form=373";
	//echo $sql;
...
if ($action=="issue_return_print")
{
	...
	$sql=" select id, recv_number, receive_basis, batch_id, knitting_source, knitting_company, challan_no, receive_date from  inv_receive_master where id='$data[3]' and entry_form=373";
...
?>
    <div style="width:930px;">
     ...
     <br>
        ...
        <br>
         <?
            echo signature_table(89, $data[0], "900px");
         ?>
        </div>
	</div>
	<?
