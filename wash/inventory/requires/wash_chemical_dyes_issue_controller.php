...
{
	$sql="select id,location_name from lib_location where company_id='$data' and status_active =1 and is_deleted=0 $company_location_credential_cond  order by location_name";
	...
	echo create_drop_down( "cbo_location_name", 170, $sql,"id,location_name", 1, "-- Select Location --", $selected, "load_drop_down( 'requires/wash_chemical_dyes_issue_controller', this.value+'_'+document.getElementById('cbo_company_name').value, 'load_drop_down_floor', 'floor_td');" );
	exit();
}
...
if ($action=="load_drop_down_loan_party_ini")
{
...
if($action=="req_popup")
{
	...
	if($issue_purpose==56){  $disabled="disabled";}
	...
	<table width="1030" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
            ...
            <tbody>
                <tr class="general">
                    ...
                    <td>				
                        <input type="text" style="width:100px" class="text_boxes_numeric"  name="txt_emb_job" id="txt_emb_job" placeholder="Number Field" <? echo $disabled;?> />	
                    </td>
                    <td>				
                        <input type="text" style="width:100px" class="text_boxes"  name="txt_emb_order" id="txt_emb_order" placeholder="Character Field" <? echo $disabled;?> />	
                    </td>
                    <td>				
                        <input type="text" style="width:100px" class="text_boxes"  name="txt_buyer_po" id="txt_buyer_po" placeholder="Character Field" <? echo $disabled;?> />	
                    </td>
                    <td>				
                        <input type="text" style="width:100px" class="text_boxes"  name="txt_buyer_style" id="txt_buyer_style" placeholder="Character Field" <? echo $disabled;?> />	
                    </td>
                    <td>
...
if($action=="create_reqisition_search_list_view")
{
	...
	$issue_purpose= trim($ex_data[9])*1;
	...
	if($emb_job!="" || $emb_order!="")
	{
		...
		$emb_job_sql="select b.id, a.job_no_prefix_num, b.job_no_mst, b.order_no, b.buyer_style_ref, a.within_group from subcon_ord_mst a, subcon_ord_dtls b where a.id=b.mst_id and a.entry_form=295 and a.status_active=1 and b.status_active=1 and a.company_id=$company $emb_job_cond";
		...
		foreach($emb_job_result as $row)
		{
			...
			$emb_data[$row[csf("id")]]["within_group"]=$row[csf("within_group")];
	...
	{
		$emb_job_sql="select b.id, a.job_no_prefix_num, b.job_no_mst, b.order_no, b.buyer_style_ref, a.within_group from subcon_ord_mst a, subcon_ord_dtls b where a.id=b.mst_id and a.entry_form=295 and a.status_active=1 and b.status_active=1 and a.company_id=$company";
		//echo $emb_job_sql;die;
		$emb_job_result=sql_select($emb_job_sql);
		$emb_data=array();
		foreach($emb_job_result as $row)
	...
	if($issue_purpose==83)//83 garments wash
	{
	...
	else if($issue_purpose==56)//56 machine wash
	{
		
		
			 $sql ="select a.requ_no, a.requ_prefix_num, a.company_id, a.requisition_date, a.requisition_basis, a.recipe_id, a.id, a.order_id, a.buyer_po_id,a.machine_id,a.method,a.store_id,a.tot_liquor
		from dyes_chem_issue_requ_mst a where a.company_id=$company and a.is_apply_last_update!=2  and a.method=177 and a.entry_form=259 $req_cond";
	}
	/*else
	{
			$sql ="select a.requ_no, a.requ_prefix_num, a.company_id, a.requisition_date, a.requisition_basis, a.recipe_id, a.id, a.order_id, a.buyer_po_id
   ...
   <div style="width:820px; overflow-y:scroll; max-height:230px; cursor:pointer" id="buyer_list_view" align="center">
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="790" class="rpt_table" id="tbl_list_search" >
        <tbody>
            ...
            foreach($result_sql as $row)
            {
            	$buyer_stye = ''; $within_group = '';
				if ($i%2==0)$bgcolor="#E9F3FF";						
				else $bgcolor="#FFFFFF";
				//echo $row[csf('recipe_id')];
				$recipi_no=$receipe_data_arr[$row[csf('recipe_id')]]['id'];

				$within_group = $emb_data[$row[csf("order_id")]]["within_group"];

				if($within_group==2)
				{
					$buyer_stye = $emb_data[$row[csf("order_id")]]["buyer_style_ref"];
				}
				else
				{
					$buyer_stye = $buyer_order_data[$row[csf("buyer_po_id")]]["style_ref_no"];
				}
				
				?>
				<tr bgcolor="<? echo $bgcolor; ?>" onClick="js_set_value('<? echo $row[csf('requ_no')]."_".$row[csf('id')]."_".$row[csf('recipe_id')]."_".$recipi_no."_".$emb_data[$row[csf("order_id")]]["job_no_mst"]."_".$emb_data[$row[csf("order_id")]]["order_no"]."_".$row[csf("order_id")]."_".$buyer_stye."_".$buyer_order_data[$row[csf("buyer_po_id")]]["po_number"]."_".$row[csf("buyer_po_id")]; ?>')">
                    ...
                    <td width="120" align="center" ><? echo $machine_arr[$row[csf("machine_id")]]; ?></td>
                    <td  width="120" align="center"><? echo $dyeing_method[$row[csf("method")]];  ?></td>
                    <td width="120" align="center"><? echo $store_arr[$row[csf("store_id")]]; ?></td>
                    <td align="right"><? echo $row[csf("tot_liquor")]; ?></td>
				</tr>
				<? 
				$i++; 
            }
        ...
        <tbody>
            ...
            foreach($result_sql as $row)
            {
				...
				<tr bgcolor="<? echo $bgcolor; ?>" onClick="js_set_value('<? echo $row[csf('requ_no')]."_".$row[csf('id')]."_".$row[csf('recipe_id')]."_".$recipi_no."_".$emb_data[$row[csf("order_id")]]["job_no_mst"]."_".$emb_data[$row[csf("order_id")]]["order_no"]."_".$row[csf("order_id")]."_".$buyer_stye."_".$buyer_order_data[$row[csf("buyer_po_id")]]["po_number"]."_".$row[csf("buyer_po_id")]; ?>')">
                    ...
                    <td width="120" align="center"><? echo $buyer_stye; ?></td>
            ...
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="880" class="rpt_table" id="tbl_list_search">
                ...
                if($is_update=="")
                {
					if($issue_basis==4)
					{
						...
						foreach ($nameArray as $selectResult)
						{
							...
							<tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>">
                                ...
                                <input type="text" name="reqn_qnty_edit[]" id="txt_reqn_qnty_edit_<? echo $i; ?>" class="text_boxes_numeric" style="width:80%" onKeyUp="check_data('#txt_reqn_qnty_edit_<? echo $i; ?>',<? echo number_format($stock_qty_org,6,'.', ''); ?>)"  value="<? //echo def_number_format($recipe_qnty,5,""); ?>" >
					...
					if($issue_basis==7)
					{
						...
						//echo "select a.issue_number, b.product_id, b.req_qny_edit as req_qny_edit,b.sub_req_process_id from inv_issue_master a, dyes_chem_issue_dtls b where a.id=b.mst_id and a.entry_form=298 and a.issue_basis=7 and a.req_id='".$req_id."' and b.sub_process=$sub_process_id"; die;
						
						//echo "10**select a.issue_number, b.product_id, b.req_qny_edit as req_qny_edit,b.sub_req_process_id from inv_issue_master a, dyes_chem_issue_dtls b where a.id=b.mst_id and a.entry_form=298 and a.issue_basis=7 and a.req_id='".$req_id."' and b.sub_process=$sub_process_id"; die;
						...
						where a.id=b.product_id and b.mst_id='".$req_id."' and b.status_active=1 and b.is_deleted=0 and a.company_id='$company_id' and a.item_category_id in(5,6,7,23) and a.status_active=1 and a.is_deleted=0 
						order by a.item_category_id ";
						...
						foreach ($nameArray as $selectResult)
						{
							...
							//$req_qnty=$selectResult[csf('required_qnty')]-($total_preveious_issue- $selectResult[csf('req_qny_edit')]);
							//echo "10**   ".$selectResult[csf('req_qny_edit')]."==".$total_preveious_issue."issue== ".$selectResult[csf("id")]."multi==  ".$selectResult[csf("multicolor_id")]; 
							//$req_qnty=$selectResult[csf('req_qny_edit')]-$total_preveious_issue;
							$req_qnty=def_number_format(($selectResult[csf('req_qny_edit')]-$total_preveious_issue),5,"");
							//echo $req_qnty."<br>";
							...
							$stock_qty=$current_stock=number_format($stock_qty_arr[$company_id][$cbo_store][$selectResult[csf('item_category_id')]][$selectResult[csf('id')]][$dyes_lot]['stock'],6, '.', '');
							...
							if(number_format($req_qnty,6, '.', '')>0)
							{
                                ...
                                <tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>"> 
                                    ...
                                    <td width="100" align="center"><p><? echo $selectResult[csf('item_lot')]; ?>
                                    ...
                                    <input type="text" name="reqn_qnty_edit[]" id="txt_reqn_qnty_edit_<? echo $i; ?>" class="text_boxes_numeric" style="width:80%"  value="<? echo number_format($req_qnty,6, '.', ''); ?>" onKeyUp="check_data('#txt_reqn_qnty_edit_<? echo $i; ?>',<? echo number_format($stock_qty,6, '.', '');//$selectResult[csf('current_stock')]; ?>)"  / >
                ...
                else
                {
					...
					$sql="select a.id,t.store_id,a.item_category_id, a.item_group_id, a.sub_group_name, a.item_description, a.item_size, a.unit_of_measure,a.current_stock, b.id as dtls_id,b.trans_id, b.dose_base, b.ratio,recipe_qnty,adjust_percent,adjust_type,required_qnty,req_qny_edit,b.product_id ,b.sub_req_process_id, t.batch_lot, t.remarks 
					...
					//echo "select a.issue_number, b.product_id, b.req_qny_edit as req_qny_edit,b.sub_req_process_id from inv_issue_master a, dyes_chem_issue_dtls b where a.id=b.mst_id and a.entry_form=298 and a.issue_basis=7 and a.req_id='".$req_id."' and b.sub_process=$sub_process_id and b.mst_id<>$is_update"; die;
					...
					if($issue_purpose==56)
					{
						...
						from product_details_master a, dyes_chem_issue_requ_dtls b 
						where a.id=b.product_id and b.mst_id='".$req_id."' and b.status_active=1 and b.is_deleted=0 and a.company_id='$company_id' and a.item_category_id in(5,6,7,23) and a.status_active=1 and a.is_deleted=0 order by a.item_category_id ";
					...
					foreach ($nameArray as $selectResult)
					{
						...
						//echo "10**   ".$total_requisition_qty."==".$total_preveious_issue;
						
						//echo "10**   ".$selectResult[csf('req_qny_edit')]."==".$total_preveious_issue."issue== ".$selectResult[csf("id")]."multi==  ".$selectResult[csf("sub_req_process_id")];
						
						//$req_qnty=$selectResult[csf('required_qnty')]-($total_issue_arr[$selectResult[csf("product_id")]]- $selectResult[csf('req_qny_edit')]);
						
						//30 -20=10 
						//$req_qnty=$selectResult[csf('required_qnty')]-($total_preveious_issue+ $selectResult[csf('req_qny_edit')]);
						//$req_qnty=$selectResult[csf('required_qnty')]-($total_preveious_issue);
						//$req_qnty=($total_requisition_qty-$total_preveious_issue);
						...
						if($issue_basis==7)
						{
							...
							<tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>"> 
                                ...
                                <td width="70"  title="<? echo $stock_qty?>" align="center" id="td_stock_qty_<? echo $i; ?>"><input type="text" name="stock_qty[]" id="stock_qty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px"  value="<? echo number_format($stock_qty,6,'.', '');//$selectResult[csf('current_stock')]; ?>"  disabled></td>
                                <td width="70" align="center" id="recipe_qnty_<? echo $i; ?>"><input type="text" name="txt_recipe_qnty[]" id="txt_recipe_qnty_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px"  value="<? echo number_format($selectResult[csf('recipe_qnty')],6,'.', ''); ?>" onClick="check_data('#txt_reqn_qnty_edit_<? echo $i; ?>',<? echo number_format($stock_qty,6,'.', '');//$selectResult[csf('current_stock')]; ?>)" readonly></td>
                                ...
                                <input type="text" name="reqn_qnty_edit[]" id="txt_reqn_qnty_edit_<? echo $i; ?>" class="text_boxes_numeric" style="width:80%"  value="<? echo number_format($selectResult[csf('req_qny_edit')],6,'.', ''); ?>"  onKeyUp="check_data('#txt_reqn_qnty_edit_<? echo $i; ?>',<? echo number_format(($stock_qty+$selectResult[csf('req_qny_edit')]),6,'.', '');//$selectResult[csf('current_stock')]+$selectResult[csf('req_qny_edit')]; ?>)"   <? echo $desable_cond; ?> / >
                                <input type="hidden" name="hidreqn_qnty_edit[]" id="hidtxt_reqn_qnty_edit_<? echo $i; ?>" class="text_boxes_numeric" style="width:80%"  value="<? echo number_format($req_qnty,6,'.', ''); ?>" readonly  / >
                                <input type="hidden" name="updateIdDtls[]" id="updateIdDtls_<? echo $i; ?>" value="<? echo $selectResult[csf('dtls_id')]; ?>">	
                                <input type="hidden" name="transId[]" id="transId_<? echo $i; ?>" value="<? echo $selectResult[csf('trans_id')]; ?>">	
...
if($action=="create_mrr_search_list_view")
{
	...
	//select recipe_no,batch_id from pro_recipe_entry_mst","recipe_no
		 ...
		 $recipe_sql ="Select b.recipe_no, b.id from pro_batch_create_mst a, pro_recipe_entry_mst b where a.id=b.batch_id and a.batch_no like '$batch_no'";
		$recipe_sql_res=sql_select($recipe_sql);
		$recipe_arr=array();
		foreach ($recipe_sql_res as $row)
		{
			$recipe_arr[$row[csf('recipe_no')]]="'".$row[csf("recipe_no")]."'";
		}
		$recipe_no=implode(',', $recipe_arr);
		if ($recipe_no != '') $recipe_cond = " and a.batch_no in ($recipe_no) "; else $recipe_cond = "";
	...
	$batch_id_arr = return_library_array("select recipe_no,batch_id from pro_recipe_entry_mst","recipe_no","batch_id");
...
//batch_id
	$sql = sql_select("select a.issue_number_prefix_num,a.req_no, a.issue_number, a.company_id, a.issue_date, a.issue_basis, a.issue_purpose, a.batch_no, a.issue_date, a.id, $year_id,
	 a.is_posted_account 
	 from inv_issue_master a where a.company_id=$company and a.entry_form=298 $sql_cond  $recipe_cond $year_cond and a.status_active=1 and a.is_deleted=0  order by a.issue_date desc");
	//echo "select a.issue_number_prefix_num,a.req_no, a.issue_number, a.company_id, a.issue_date, a.issue_basis, a.issue_purpose, a.batch_no, a.issue_date, a.id, $year_id, a.is_posted_account from inv_issue_master a where a.company_id=$company and a.entry_form=298 $sql_cond  $recipe_cond $year_cond and a.status_active=1 and a.is_deleted=0  order by a.id";
	?>
	<br/>
 	...
 	<table align="center" cellspacing="0" width="820"  border="1" rules="all" class="rpt_table"  id="list_view" >
		...
		?>
         	<tr bgcolor="<? echo $bgcolor; ?>" onClick='js_set_value("<? echo $inf[csf("issue_number")]."_".$inf[csf("id")]."_".$inf[csf("issue_basis")]."_".$inf[csf("issue_purpose")]."_".$inf[csf("is_posted_account")];?>")' style="cursor:pointer">
                ...
                <td width="150" align="center"><?php echo $batch_arr[$batch_id_arr[$inf[csf("batch_no")]]];//$batch_no_all; ?></td>
...
{
	$sql = sql_select("select location_id, floor_id, issue_date, issue_basis, req_no, req_id, issue_purpose, company_id, loan_party, lap_dip_no, batch_no, order_id, sub_order_id, style_ref, store_id, buyer_job_no, is_posted_account, lc_company, challan_no, remarks, shift from inv_issue_master where id=$data and entry_form=298");
	...
	foreach($sql as $row)
	{
		...
		echo  "load_drop_down( 'requires/wash_chemical_dyes_issue_controller', '".$row[csf("issue_basis")]."'+'**'+'".$row[csf("company_id")]."', 'load_drop_down_store', 'store_td');\n";
		$sql_variable = sql_select("select auto_transfer_rcv, id from variable_settings_inventory where company_name = '".$row[csf("company_id")]."' and variable_list = 29 and is_deleted = 0 and status_active = 1");
		echo "$('#variable_lot').val('".$sql_variable[0][csf("auto_transfer_rcv")]."');\n";	
		...
		echo "load_drop_down( 'requires/wash_chemical_dyes_issue_controller', document.getElementById('cbo_location_name').value+'_'+document.getElementById('cbo_company_name').value, 'load_drop_down_floor', 'floor_td');\n";
		...
		if($row[csf("issue_basis")]==7)
		{
			echo "$('#cbo_company_name').attr('disabled',true);\n"; 
			echo "$('#cbo_issue_basis').attr('disabled',true);\n"; 
		...
		else
		{
			echo "$('#cbo_company_name').attr('disabled',true);\n"; 
			echo "$('#cbo_issue_basis').attr('disabled',true);\n"; 
			echo "$('#cbo_issue_purpose').attr('disabled',false);\n"; 
	}
...
{	  
	...
	{
		if(str_replace("'","",$cbo_issue_purpose)==56)
		{
			$all_dtls_id="";
			for ($i=1;$i<$total_row;$i++)
			{
		...
		/*	echo "10**select a.issue_number, b.product_id, b.req_qny_edit as req_qny_edit, b.sub_req_process_id 
			from inv_issue_master a, dyes_chem_issue_dtls b 
			where a.id=b.mst_id and a.entry_form=298 and a.issue_basis=7 and a.req_id='".str_replace("'","",$txt_req_id)."' $dtls_cond"; die;
			*/
			$total_issue_sql=sql_select("select a.issue_number, b.product_id, b.req_qny_edit as req_qny_edit, b.sub_req_process_id 
			from inv_issue_master a, dyes_chem_issue_dtls b 
			where a.id=b.mst_id and a.entry_form=298 and a.issue_basis=7 and a.req_id='".str_replace("'","",$txt_req_id)."' $dtls_cond");
			$prev_issue=array();
			foreach($total_issue_sql as $row)
			{
		...
		/*	echo "10**select a.id, b.required_qnty as required_qnty, b.req_qny_edit, b.multicolor_id
			from product_details_master a, dyes_chem_issue_requ_dtls b
			where a.id=b.product_id and b.mst_id='".str_replace("'","",$txt_req_id)."'  and b.status_active=1 and b.is_deleted=0 and a.company_id='".str_replace("'","",$cbo_company_name)."' and a.item_category_id in(5,6,7,23) and a.status_active=1 and a.is_deleted=0 $prod_cond"; die;*/
			
			$sql_req=sql_select("select a.id, b.required_qnty as required_qnty, b.req_qny_edit, b.multicolor_id
			from product_details_master a, dyes_chem_issue_requ_dtls b
			where a.id=b.product_id  and b.mst_id='".str_replace("'","",$txt_req_id)."' and b.status_active=1 and b.is_deleted=0 and a.company_id='".str_replace("'","",$cbo_company_name)."' and a.item_category_id in(5,6,7,23) and a.status_active=1 and a.is_deleted=0 $prod_cond");
			$req_data=array();
			...
			for ($i=1;$i<$total_row;$i++)
			{
				...
				$subreqprocessId="subreqprocessId_".$i;
				$hidtxt_reqn_qnty_edit="hidtxt_reqn_qnty_edit_".$i;
				//$subreqprocessId=str_replace("'","",$$subreqprocessId);
				...
				$cu_req_qnty= number_format(($total_req_qnty-$total_prev_issue),6,".","")*1;
			   ...
			   // echo "10**"
				...
				//1.256200==1.260000
			//	echo "10**".number_format($current_issue,6,".","")."==".number_format($cu_req_qnty,6,".","");  
				
				if(number_format($current_issue,6,".","")>number_format($cu_req_qnty,6,".",""))
				{
		...
		else
		{
			...
			for ($i=1;$i<$total_row;$i++)
			{
				...
				//$current_issue_qnty[str_replace("'",'',$$txt_prod_id)]+=str_replace("'",'',$$txt_reqn_qnty_edit);
				//$req_qnty_arr[str_replace("'",'',$$txt_prod_id)]+=str_replace("'",'',$$hidtxt_reqn_qnty_edit);
			}
			$all_dtls_id=chop($all_dtls_id,",");
			$all_prod_id=chop($all_prod_id,",");
			$dtls_cond="";
			...
			$total_issue_sql=sql_select("select a.issue_number, b.product_id, b.req_qny_edit as req_qny_edit, b.sub_req_process_id 
			from inv_issue_master a, dyes_chem_issue_dtls b 
			where a.id=b.mst_id and a.entry_form=298 and a.issue_basis=7 and a.req_id='".str_replace("'","",$txt_req_id)."' and b.sub_process='".str_replace("'","",$cbo_sub_process)."' $dtls_cond");
			$prev_issue=array();
			foreach($total_issue_sql as $row)
			{
				$prev_issue[$row[csf("product_id")]][$row[csf("sub_req_process_id")]]+=$row[csf("req_qny_edit")];
			...
			for ($i=1;$i<$total_row;$i++)
			{
			   ...
			   // echo "10**"
				...
				$current_issue= str_replace("'","",$$txt_reqn_qnty_edit)*1;
				//1.256200==1.260000
				//echo "10**".number_format($current_issue,6,".","")."==".number_format($cu_req_qnty,6,".",""); die;
...
//	echo "10**";
	...
	//echo "select ID, PROD_ID, STORE_ID, BATCH_LOT, TRANSACTION_TYPE, ITEM_CATEGORY, CONS_QUANTITY, CONS_AMOUNT, BALANCE_QNTY, BALANCE_AMOUNT, CONS_RATE
 	//from inv_transaction where status_active=1 and is_deleted=0 and prod_id in($prod_ids) and store_id = $issue_store_id order by PROD_ID, ID"; die;
	...
	$trans_sql=sql_select("select ID, PROD_ID, STORE_ID, BATCH_LOT, TRANSACTION_TYPE, ITEM_CATEGORY, CONS_QUANTITY, CONS_AMOUNT, BALANCE_QNTY, BALANCE_AMOUNT, CONS_RATE, STORE_AMOUNT
 	from inv_transaction where status_active=1 and is_deleted=0 and prod_id in($prod_ids) and store_id = $issue_store_id order by PROD_ID, ID");
	...
	foreach($trans_sql as $val)
	{
		...
		if(($val["TRANSACTION_TYPE"]==1 || $val["TRANSACTION_TYPE"]==4 || $val["TRANSACTION_TYPE"]==5) && ($val["ITEM_CATEGORY"]==7 || $val["ITEM_CATEGORY"]==8) && $val["BALANCE_QNTY"]>0)
		{
	...
	//echo "10**<pre>";print_r($trans_data_arr);die;
	$product_arr=array();
	...
	foreach($dataArray as $row)
	{
		...
		$product_cur_arr[$row[csf("id")]]['qty']=$row[csf("current_stock")];
	}
	...
	foreach($sql_store as $row)
	{
		...
		$store_arr[$row[csf("id")]][$lot_no]['qty']=$row[csf("current_stock")];
		$store_arr[$row[csf("id")]][$lot_no]['avg_rate']=$row[csf("avg_rate")];
		//$store_arr[$row[csf("id")]]['val']=$row[csf("stock_value")];
		//$store_arr[$row[csf("id")]]['last_qty']=$row[csf("last_qty")];
		$store_idarr[$row[csf("prod_id")]][$row[csf("store_id")]][$row[csf("cat_id")]][$lot_no]['id']=$row[csf("id")];
		$store_wise_stock_qnty_arr[$row[csf("prod_id")]][$lot_no]= $row[csf("current_stock")];
	}
	
	//echo "10**";
	//echo "<pre>";
	//print_r($store_wise_stock_qnty_arr);disconnect($con); die;
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$update_id)=="")
		{
			...
			$field_array="id, issue_number_prefix, issue_number_prefix_num, issue_number,issue_basis, issue_purpose, entry_form, company_id, location_id, floor_id, store_id, buyer_job_no, style_ref, req_no, req_id, issue_date, loan_party, lap_dip_no, batch_no, order_id, sub_order_id, inserted_by, insert_date, lc_company, challan_no, remarks, shift";
			$data_array="(".$id.",'".$new_system_id[1]."',".$new_system_id[2].",'".$new_system_id[0]."',".$cbo_issue_basis.",".$cbo_issue_purpose.",298,".$cbo_company_name.",".$cbo_location_name.",".$cbo_floor_name.",".$cbo_store_name.",".$txt_order_no.",".$txt_buyer_style.",".$txt_req_no.",".$txt_req_id.",".$txt_issue_date.",".$cbo_loan_party.",".$txt_recipe_id.",".$txt_recipe_no.",".$hidden_buyer_po_id.",".$hidden_order_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_company_name.",".$txt_challan_no.",".$txt_remarks.",".$cbo_shift.")";
		...
		{
			$field_array_update="issue_purpose*location_id*floor_id*buyer_job_no*style_ref*req_no*req_id*issue_date*loan_party*lap_dip_no*batch_no*order_id*sub_order_id*updated_by*update_date*lc_company*challan_no*remarks*shift";
			$data_array_update="".$cbo_issue_purpose."*".$cbo_location_name."*".$cbo_floor_name."*".$txt_order_no."*".$txt_buyer_style."*".$txt_req_no."*".$txt_req_id."*".$txt_issue_date."*".$cbo_loan_party."*".$txt_recipe_id."*".$txt_recipe_no."*".$hidden_buyer_po_id."*".$hidden_order_id."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$cbo_company_name."*".$txt_challan_no."*".$txt_remarks."*".$cbo_shift."";
		...
		for ($i=1;$i<$total_row;$i++)
		{
			...
			$prod_stock=$product_cur_arr[str_replace("'",'',$$txt_prod_id)]['qty'];
			$trans_stock=$trans_data_arr[str_replace("'",'',$$txt_prod_id)][$dyes_lot]["STOCK"];
			//2=250.325=20=32
			//echo "10**".str_replace("'",'',$$txt_reqn_qnty_edit)."=".$prod_stock."=".$trans_stock."=".$store_prod_qnty;disconnect($con); die;
			//echo "10**".$trans_stock;disconnect($con); die;
			if(number_format(str_replace("'",'',$$txt_reqn_qnty_edit),6,'.','')>0)
			{
				if(str_replace("'","",$cbo_issue_purpose)==56)
				{
					...
					{
						//0.700000=-47.063=-92.063=7.937
						
						echo "10**".str_replace("'",'',$$txt_reqn_qnty_edit)."=".$prod_stock."=".$trans_stock."=".$store_prod_qnty;disconnect($con); die;
						
						//echo "20**Issue Quantity Not Allow Over Stock Quantity".str_replace("'",'',$$txt_prod_id)."=".$dyes_lot."=".$trans_stock;disconnect($con); die;
						echo "20**Issue Quantity Not Allow Over Stock Quantity".str_replace("'",'',$$txt_reqn_qnty_edit)."=".$prod_stock."=".$trans_stock."=".$store_prod_qnty;disconnect($con); die;
					}
				}
				else
				{
					...
					//if(number_format(str_replace("'",'',$$txt_reqn_qnty_edit)*1,6,".","") > number_format($store_prod_qnty,6,".","") )
					{
						//1=248.325==12
						//echo "10**".str_replace("'",'',$$txt_reqn_qnty_edit)."=".$prod_stock."=".$trans_stock."=".$store_prod_qnty;disconnect($con); die;
				...
				$product_cur_arr[str_replace("'",'',$$txt_prod_id)]['qty']-=number_format(str_replace("'",'',$$txt_reqn_qnty_edit)*1,6,".","");
			}
			...
			if(str_replace("'",'',$$txt_reqn_qnty_edit)>0)
			{
				...
				$storeid=$store_idarr[$prod_id][$issue_store_id][$cat_id][$dyes_lot]['id'];
		...
		if($all_prod_id_ac!="")
		{
			$auxChemData = sql_select("select prod_id, id, cons_rate, balance_qnty, balance_amount from inv_transaction where prod_id in ($all_prod_id_ac) and balance_qnty>0 and transaction_type in (1,4,5) and item_category in(7,23) order by transaction_date,id $cond_lifofifoA");
		...
		//echo "10**".bulk_update_sql_statement( "product_details_master", "id", $field_array_prod, $data_array_prod, $id_arr );oci_rollback($con);disconnect($con); die;
		if(str_replace("'","",$update_id)=="")
		{
		...
		//echo "10**".$rID.'='.$rID2.'='.$rID3.'='.$rID4.'='.$mrrWiseIssueID.'='.$upTrID.'='.$store_ID;disconnect($con); die;
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		for ($i=1;$i<$total_row;$i++)
		{
			...
			if(str_replace("'",'',$$transId)!="") $all_transId.=str_replace("'",'',$$transId).",";
			...
			$prod_stock=($product_cur_arr[str_replace("'",'',$$txt_prod_id)]['qty']+str_replace("'",'',$$hidtxt_reqn_qnty_edit))*1;
			...
			if(number_format(str_replace("'",'',$$txt_reqn_qnty_edit),6,'.','')>0)
			{
				...
				//if(number_format(str_replace("'",'',$$txt_reqn_qnty_edit)*1,6,".","") > number_format($store_c_stock,6,".",""))
				{
					echo "20**Issue Quantity Not Allow Over Stock Quantity";disconnect($con); oci_rollback($con); die;
				}
				$product_cur_arr[str_replace("'",'',$$txt_prod_id)]['qty']-=number_format(str_replace("'",'',$$txt_reqn_qnty_edit)*1,6,".","");
			}
			//store wise
			if(str_replace("'",'',$$txt_reqn_qnty_edit)>0)
			{
				...
				$cat_id=str_replace("'",'',$$txt_item_cat);
				$storeid=$store_idarr[$prod_id][$issue_store_id][$cat_id][$dyes_lot]['id'];
				//echo  '10**'.$txt_reqn_qnty_edit.'=='.$storeid.'=='.$prod_id.'=='.$issue_store_id.'=='.$cat_id; disconnect($con); die;
				//echo  '10**'.$prod_id.'=='.$issue_store_id.'=='.$cat_id; disconnect($con); die;
				
				//echo '10**'.$storeid.'saaz';disconnect($con); die;
				$store_stock_arr[$storeid][$prod_id][$issue_store_id][$cat_id]['issue_qty']=$txt_reqn_qnty_edit;
		...
		if($all_prod_id_ac!="")
		{
			$auxChemData = sql_select("select prod_id, id, cons_rate, balance_qnty, balance_amount from inv_transaction where prod_id in ($all_prod_id_ac) and balance_qnty>0 and transaction_type in (1,4,5) and item_category in(7,23) order by transaction_date $cond_lifofifoA");
			foreach($auxChemData as $row)
			{
				$auxChemDataArr[$row[csf("prod_id")]].=$row[csf("id")]."**".$row[csf("cons_rate")]."**".$row[csf("balance_qnty")]."**".$row[csf("balance_amount")].",";
			}
		...
		for ($i=1;$i<$total_row;$i++)
		{
			...
			if(str_replace("'",'',$$transId)!="")
			{
				...
				$data_array_trans[str_replace("'",'',$$transId)] =explode("*",("".$$txt_prod_id."*".$$txt_item_cat."*".$txt_issue_date."*12*".$txt_reqn_qnty_e."*".number_format($avg_rate,10,'.','')."*".number_format($issue_stock_value,8,'.','')."*".$cbo_store_name."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$$txt_lot."*".number_format($store_item_rate,10,'.','')."*".number_format($issue_store_value,8,'.','').""));
			}
			...
			$prod_id=str_replace("'",'',$$txt_prod_id);
			$cat_id=str_replace("'",'',$$txt_item_cat);
			//echo  '10**'.$prod_id.'=='.$issue_store_id.'=='.$cat_id; die;
			$storeId=$store_idarr[$prod_id][$issue_store_id][$cat_id][$dyes_lot]['id'];
			 //echo '10**'.$storeId.'aaz';die;
			if(str_replace("'",'',$txt_reqn_qnty_e)>0)
			{
				...
				if(in_array($storeId,$storeArrId))
				{
					if(str_replace("'",'',$storeId)!="")
					{
						//echo '10**'.$store_arr[$storeId]['qty'].'=='.$reg_issue_qty.'=='.$req_hidden_qty; die;
						//10**9494====
						//echo  '10**'.$storeId.'=='.$prod_id.'=='.$issue_store_id.'=='.$cat_id; die;
...
if($action=="chemical_dyes_issue_print")
{
 ...
 $sql_dtls = "SELECT a.inserted_by,b.id,a.issue_number,b.store_id,
	  ...
	  $i=1;
	if(trim($dataArray[0][csf('issue_basis')])==7 && trim($dataArray[0][csf('issue_purpose')])==56) $colspan=5; else $colspan=6;
