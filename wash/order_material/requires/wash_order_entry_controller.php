...
if ($action=="load_drop_down_location")
{
	...
	$party_location_count=return_library_array("select id,location_name from lib_location where company_id='$data[0]' and status_active =1 and is_deleted=0 order by location_name","id", "location_name");
	

	if (count($party_location_count)==1) {
		$selected= implode(",",array_keys($party_location_count));
	}
	else
	{
		$selected=0;
...
if($action=="check_popup_rate_mandatory")
{
	//echo $data; die;
	//$data=explode("**",$data);
	$is_mandatory=return_field_value( "is_mandatory", "mandatory_field"," page_id=295 and field_id=1 and status_active=1 and is_deleted=0");
	echo $is_mandatory;
	exit();
}

if($action=="check_popup_rate_disabled")
{
	//echo $data; die;
	//$data=explode("**",$data);
	$user_id=$_SESSION['logic_erp']['user_id'];
	$is_disabled=return_field_value( "is_disable", "field_level_access"," page_id=295 and field_id=1 and user_id=$user_id and status_active=1 and is_deleted=0");
	echo $is_disabled;
	exit();
}
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Start Here=================================================================================
	{
		...
		$new_job_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'WOE', date("Y",time()), 5, "select job_no_prefix, job_no_prefix_num from subcon_ord_mst where entry_form=295 and company_id=$cbo_company_name $insert_date_con order by id desc ", "job_no_prefix", "job_no_prefix_num" ));

		$id=return_next_id("id","subcon_ord_mst",1);
		$id1=return_next_id( "id", "subcon_ord_dtls",1);
		$id3=return_next_id( "id", "subcon_ord_breakdown", 1) ;
		$rID3=true;
		$field_array="id, entry_form, subcon_job, job_no_prefix, job_no_prefix_num, company_id, location_id, floor_id, within_group, party_id, party_location, currency_id, exchange_rate, receive_date, delivery_date,gmts_type, rec_start_date, rec_end_date, order_id, order_no, conv_factor, inserted_by, insert_date, status_active, is_deleted";
		...
		$field_array3="id, mst_id, order_id, job_no_mst, description, process, embellishment_type, rate,prod_sequence_no, inserted_by, insert_date, status_active, is_deleted";
		...
		{			
			...
			for($j=0; $j<count($dtls_data); $j++)
			{
				...
				$data_array3.="(".$id3.",".$id1.",".$hid_order_id.",'".$new_job_no[0]."',".$description.",".$process.",".$washtype.",".$rate.",".$prodsequenceno.",'".$user_id."','".$pc_date_time."',1,0)";
	...
	else if ($operation==1)   // Update Here================================================================
	{
		...
		*/
		...
		//echo "10**";$total_row; 
		//echo "<pre>";
		//print_r($prev_data_arr); die;
		$field_array="location_id*floor_id*within_group*party_id*party_location*currency_id*exchange_rate*receive_date*delivery_date*gmts_type*rec_start_date*rec_end_date*order_id*order_no*conv_factor*updated_by*update_date";		
		$data_array="".$cbo_location_name."*".$cbo_floor_name."*".$cbo_within_group."*".$cbo_party_name."*".$cbo_party_location."*".$cbo_currency."*".$txt_ex_rate."*".$txt_order_receive_date."*".$txt_delivery_date."*".$cbo_gmts_type."*".$txt_rec_start_date."*".$txt_rec_end_date."*".$hid_order_id."*".$txt_order_no."*".$txt_converstion_factor."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		$field_array4="order_id*description*process*embellishment_type*rate*prod_sequence_no*updated_by*update_date";
		...
		{			
			...
			$txtremarks 			= "txtremarks_".$i;
			$hdnDtlsdata 			= "hdnDtlsdata_".$i;
			$hdnDtlsdataIds 		= "hdnDtlsdataIds_".$i;
			$hdnDtlsUpdateId 		= "hdnDtlsUpdateId_".$i;
			$hdnbookingDtlsId 		= "hdnbookingDtlsId_".$i;
			$txtpartybuyername 		= "txtpartybuyername_".$i;
			...
			$dtls_data=explode("__",str_replace("'",'',$$hdnDtlsdata));
			$dtls_data_ids=explode("__",str_replace("'",'',$$hdnDtlsdataIds));
			...
			for($j=0; $j<count($dtls_data); $j++)
			{
				...
				if($bb==0 || $bb=='')
				{
					...
					if ($add_commadtls!=0) $data_array3 .=",";
					$data_array3.="(".$id3.",".$dtlsIdForBreak.",".$hid_order_id.",".$txt_job_no.",".$description.",".$process.",".$washtype.",".$rate.",".$prodsequenceno.",'".$user_id."','".$pc_date_time."',1,0)";
					$id3=$id3+1; 
					$add_commadtls++;
				}
				else
				{
					$data_array4[$bb]=explode("*",("".$hid_order_id."*".$description."*".$process."*".$washtype."*".$rate."*".$prodsequenceno."*".$user_id."*'".$pc_date_time."'"));
					$hdn_break_id_arr[]		=$bb;
				}
			}
			//die;			
		...
		if($txt_deleted_id_dtls!="" && $flag==1)
		{
			...
			$rID7=sql_multirow_update("subcon_ord_breakdown",$field_array_status,$data_array_status,"mst_id",$txt_deleted_id_dtls,0);
		 ...
		 if($data_array4!="")
		{
			...
			for ($m = 0; $m < $count_up_trans; $m++) 
			{
 			   ...
 			   // bulk_update_sql_statement("subcon_ord_breakdown", "id", $field_array4, $data_array_update_trans_chunk[$m], array_values($transId_up_arr[$m]));  
				
  			 $rID3 = execute_query(bulk_update_sql_statement("subcon_ord_breakdown", "id", $field_array4, $data_array_update_trans_chunk[$m], array_values($transId_up_arr[$m])), 1);
		...
		if($deleteIds!='')
		{
			...
			$field_array_status="updated_by*update_date*status_active*is_deleted";
			$data_array_status="'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'*0*1";
			$rID5=sql_multirow_update("subcon_ord_breakdown",$field_array_status,$data_array_status,"id",$deleteIds,0);
	...
	else if ($operation==2)   // delete here=======================================================================
	{
		...
		//echo "10**select pi_number from com_export_pi_mst a,com_export_pi_dtls b where a.id=b.pi_id and b.work_order_no=$txt_job_no and b.status_active=1 and b.is_deleted=0"; die;
		
		$pi_number=return_field_value( "pi_number", "com_export_pi_mst a,com_export_pi_dtls b","a.id=b.pi_id and b.work_order_no=$txt_job_no and b.status_active=1 and b.is_deleted=0");
		if($pi_number){
			echo "pinumber**".str_replace("'","",$txt_job_no)."**".$pi_number;
			disconnect($con); die;
		}
		
		
        $rec_number=return_field_value( "sys_no", "sub_material_mst"," embl_job_no=$txt_job_no and status_active=1 and is_deleted=0 and trans_type=1 and entry_form=296");
		if($rec_number){
			echo "washRec**".str_replace("'","",$txt_job_no)."**".$rec_number;
			disconnect($con); die;
		}
...
<body onLoad="fnc_load_party_popup(1,document.getElementById('cbo_within_group').value)">
<div align="center" style="width:100%;" >
        ...
        <table width="940" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
            ...
            <tbody>
                <tr class="general">
                    ...
                    <td>
                        <?php echo create_drop_down( "cbo_within_group", 100, $yes_no,"", 1, "-- Select --",$data[3], "fnc_load_party_popup(1,this.value);" ); ?>
                    </td>
                    <td id="buyer_td">
                        <?
    ...
    <script>
    	...
    	function showList() {
    		...
    		show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_party_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_type').value+'_'+document.getElementById('txt_search_string').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+document.getElementById('cbo_within_group').value+'_'+document.getElementById('cbo_year_selection').value, 'create_job_search_list_view', 'search_div', 'wash_order_entry_controller', 'setFilterGrid(\'tbl_po_list\',-1)');
    		isValidated = false;
    	}
    </script>
    </html>
...
{	
	...
	else if($search_type==4 || $search_type==0)
	{
		...
		if($search_str!="")
		{
			...
			else if ($search_by==3) $search_com_cond=" and d.JOB_NO_MST like '%$search_str%'";
	...
	if($within_group==1)
	{
		 ...
		 where a.entry_form=295 and a.subcon_job=b.job_no_mst and a.id=b.mst_id and  d.id=b.buyer_po_id and a.status_active=1 and b.status_active=1 $inter_ref $order_rcv_date $company $buyer $withinGroup $party_id_cond $search_com_cond $year_cond and b.id=c.mst_id and  c.status_active=1 and c.is_deleted=0
	...
	else
	{
		$sql= "select a.id, a.subcon_job, a.job_no_prefix_num, $ins_year_cond as year, a.company_id, a.location_id, a.party_id, a.receive_date, a.order_no, a.within_group,b.buyer_po_no,b.buyer_style_ref, $color_id_str as color_id, $buyer_po_id_str as buyer_po_id , $delivery_date_str as delivery_date
		 ...
		 where a.entry_form=295 and a.subcon_job=b.job_no_mst and a.id=b.mst_id and a.status_active=1 and b.status_active=1 $order_rcv_date $company $buyer $withinGroup $party_id_cond $search_com_cond $year_cond and b.id=c.mst_id and  c.status_active=1 and c.is_deleted=0
		 group by a.id, a.subcon_job, a.job_no_prefix_num, a.insert_date, a.company_id, a.location_id, a.party_id, a.receive_date, a.order_no, a.within_group,b.buyer_po_no,b.buyer_style_ref
		 order by a.id DESC";
	}
	// echo $sql;
        ...
        <tbody>
            ...
            {  
                ...
                <tr bgcolor="<? echo $bgcolor; ?>" onClick='js_set_value("<? echo $row[csf('id')].'_'.$row[csf('subcon_job')]; ?>")' style="cursor:pointer" >
                    ...
                    <td width="130" style="word-break:break-all"><? echo $row[csf('buyer_po_no')];//if ($within_group==1)echo $buyer_po; else echo $row[csf('buyer_po_no')];//echo $buyer_po; ?></td>
                    <td width="130" style="word-break:break-all"><?  echo $row[csf('buyer_style_ref')];// if ($within_group==1)echo $buyer_style; echo $row[csf('buyer_style_ref')];//echo $buyer_style; ?></td>
...
if ($action=="load_php_data_to_form")
{
	...
	$nameArray=sql_select( "select id, subcon_job, company_id, location_id, floor_id, party_id, currency_id, exchange_rate, party_location, delivery_date, rec_start_date, rec_end_date, receive_date, within_group, party_location, order_id, order_no, conv_factor,gmts_type from subcon_ord_mst where subcon_job='$data' and status_active=1" );
	foreach ($nameArray as $row)
	{
		...
		echo "load_drop_down( 'requires/wash_order_entry_controller', document.getElementById('cbo_location_name').value+'_'+document.getElementById('cbo_company_name').value, 'load_drop_down_floor', 'floor_td');\n";
...
{	
	...
	function search_by(val,type)
	{
		if(type==1)
		{
			...
			else if(val==5) $('#search_by_td').html('IR/IB');
		}
	}
</script>
</head>
...
{	
	...
	if($data[5]==1)
	{
		if ($data[1]!="")
		{
			...
			if ($search_type==5) $inter_ref=" and b.grouping = '$data[1]' ";
		}
	}
	if($data[5]==2)
	{
		if ($data[1]!="")
		{
			...
			if ($search_type==5) $inter_ref=" and b.grouping like '$data[1]%' ";
		}
	}
	if($data[5]==3)
	{
	...
	if(($job_cond!="" && $search_type==2) || ($style_cond!="" && $search_type==3)|| ($po_cond!="" && $search_type==4) || ($inter_ref!="" && $search_type==5))
	{
		
		//echo "select $id_cond as id from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst $job_cond $style_cond $po_cond $inter_ref"; die;
		
		$po_ids = return_field_value("$id_cond as id", "wo_po_details_master a, wo_po_break_down b", "a.job_no=b.job_no_mst $job_cond $style_cond $po_cond $inter_ref $year_cond", "id");
		if ($po_ids=="")
		{
			$po_idsCond="";
			echo "Not Found."; die;
	...
	else if($db_type==2)
	{
		...
		$gmts_color_id_cond="rtrim(xmlagg(xmlelement(e,d.color_number_id,',').extract('//text()') order by d.color_number_id).GetClobVal(),',')";
		$pre_cost_dtls_id_cond="rtrim(xmlagg(xmlelement(e,b.pre_cost_fabric_cost_dtls_id,',').extract('//text()') order by b.pre_cost_fabric_cost_dtls_id).GetClobVal(),',')";
		$pre_cost_trims_cond="rtrim(xmlagg(xmlelement(e,b.pre_cost_fabric_cost_dtls_id,',').extract('//text()') order by b.pre_cost_fabric_cost_dtls_id).GetClobVal(),',')";
		$gmts_item_cond="rtrim(xmlagg(xmlelement(e,b.gmt_item,',').extract('//text()') order by b.gmt_item).GetClobVal(),',')";
		$po_id_cond="rtrim(xmlagg(xmlelement(e,b.po_break_down_id,',').extract('//text()') order by b.po_break_down_id).GetClobVal(),',')";
	} 
	...
	<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="940" >
        ...
        <tbody>
            ...
            {  
				...
				$po_ids = $row[csf('po_id')]->load();
 				$expo_id=array_unique(explode(",",$po_ids));
				...
				$gmts_color_ids = $row[csf('gmts_color_id')]->load();
				$gmts_color_id=array_unique(explode(",",$gmts_color_ids));
				...
				$pre_cost_dtls_ids = $row[csf('pre_cost_dtls_id')]->load();
				$expre_cost_trims_id=array_unique(explode(",",$pre_cost_dtls_ids));
				...
				$gmts_item_name="";
				$gmts_items = $row[csf('gmts_item')]->load();
				$exgmts_item_id=explode(",",$gmts_items);
...
if( $action=='order_dtls_list_view' )
{
	...
	$receive_arr=array();
	//echo "select a.id, a.mst_id,a.quantity,a.job_dtls_id  from  sub_material_dtls  a , sub_material_mst b where a.mst_id=b.id and  embl_job_no='$data[1]'"; die;
	$receive_sql = sql_select("select a.id, a.mst_id,a.quantity,a.job_dtls_id  from  sub_material_dtls  a , sub_material_mst b where a.mst_id=b.id and  embl_job_no='$data[1]' and  a.status_active=1 and a.is_deleted=0 and  b.status_active=1 and b.is_deleted=0");
	foreach($receive_sql as $row)
	{
		$receive_arr[$row[csf('job_dtls_id')]]['quantity']=$row[csf('quantity')];
	}
	unset($receive_sql);
	...
	if($data[0]==2)
	{
		$qry_result=sql_select( "select id, mst_id, description, process, embellishment_type, rate,prod_sequence_no from subcon_ord_breakdown where job_no_mst='$data[1]' and  status_active=1 and is_deleted=0 ");	
		...
		$data_break_arr=array(); $data_break_id_arr=array(); $data_break=''; $add_comma=0; $k=1;
		foreach ($qry_result as $row)
		{
			...
			$data_break_id_arr[$row[csf('mst_id')]].=$row[csf('id')].',';
			$add_comma++;
		}
	}
	//die;
	...
	if($data[2]==1)
	{
		...
		foreach($data_ons_per as $row)
		{
		  ...
		  $sql = "select a.id as embe_cost_dtls_id, a.job_no, a.emb_name, a.emb_type, a.body_part_id, c.id as booking_dtls_id, b.booking_no, b.po_break_down_id, b.gmt_item, b.delivery_date, b.uom as order_uom, c.color_number_id, c.gmts_sizes, sum(c.requirment) as wo_qnty, sum(c.rate) as rate, sum(c.amount) as amount,d.buyer_id as party_buyer_name from wo_pre_cost_embe_cost_dtls a, wo_booking_dtls b, wo_emb_book_con_dtls c , wo_booking_mst d where b.status_active=1 and b.is_deleted=0 and a.status_active=1 and b.pre_cost_fabric_cost_dtls_id=a.id and b.id=c.wo_booking_dtls_id and b.booking_type=6 and a.emb_name=3 and a.job_no=b.job_no and b.booking_no=trim('$data[1]') and d.booking_type=6  and b.booking_no=d.booking_no  and c.is_deleted=0 and c.status_active=1 and c.requirment !=0 and c.cons!=0  group by a.id, a.job_no, b.job_no, a.emb_name, a.emb_type, a.body_part_id , b.uom, c.id, b.booking_no, b.po_break_down_id, b.gmt_item, b.delivery_date, c.color_number_id, c.gmts_sizes,d.buyer_id order by c.id ASC";
	}
	else
	{
		...
		foreach($data_array as $row)
		{
			...
			if($data[2]==1)
			{
				...
				else 
				{
					...
					/********** MAPPING WITH $COSTING_PER*****************/
					$costing_per=$cons_per_arr[$row[csf('embe_cost_dtls_id')]]['costing_per'];
					if($costing_per==1)
					{
						$order_uom=2;
					}
			...
			if($data[2]==1){ $party_buyer_name=$buyer_arr[$row[csf('party_buyer_name')]];};
			if($data[2]==2){ $party_buyer_name=$row[csf('party_buyer_name')];};
			$receive_quantity=$receive_arr[$dtls_id]['quantity'];
			if($data[2]==1)
			{ 
				$disabled='1';	
			}
			...
			{ 
				if($receive_quantity!=""){$disabled='1';}else{$disabled='0';}
					//$disabled='0';
			}
            ...
            <td><input name="txtpartybuyername_<? echo $tblRow; ?>" id="txtpartybuyername_<? echo $tblRow; ?>" value="<? echo $party_buyer_name; ?>" class="text_boxes" type="text"  style="width:80px"  <? echo $readonly; ?>  /></td>
				...
				<td><? echo create_drop_down( "cboGmtsItem_".$tblRow, 90, $garments_item,"", 1, "-- Select --",$row[csf('gmt_item')], "",$disabled,"" ); ?></td>			
				<td>
				...
				<td title="<? echo $receive_quantity ; ?>"><? if($data[2]==1){$readonly="readonly";} ?>
                <input name="txtOrderQuantity_<? echo $tblRow; ?>"  id="txtOrderQuantity_<? echo $tblRow; ?>" value="<? echo number_format($wo_qnty,4,'.',''); ?>" class="text_boxes_numeric" type="text"  style="width:60px" <? echo $readonly; ?>  onBlur="calculate_amount(<?php echo $tblRow; ?>)"  placeholder="<? echo number_format($receive_quantity,4,'.',''); ?>"/>
                <input name="hiddenOrderQuantity_<? echo $tblRow; ?>" id="hiddenOrderQuantity_<? echo $tblRow; ?>" value="<? echo number_format($wo_qnty,4,'.',''); ?>" class="text_boxes_numeric" type="hidden"  style="width:60px"  />
                
                 <input name="receiveQuantity_<? echo $tblRow; ?>" id="receiveQuantity_<? echo $tblRow; ?>" value="<? echo number_format($receive_quantity,4,'.',''); ?>" class="text_boxes_numeric" type="hidden"  style="width:60px"  />
                </td>
                
                
				<td><? echo create_drop_down( "cboUom_".$tblRow, 60, $unit_of_measurement,"", 1, "-- Select --",$order_uom,"fnc_load_uom($order_uom,this.value);", 1,"2,1" ); ?></td>
                ...
                <td><input name="txtDomAmount_<? echo $tblRow; ?>" id="txtDomAmount_<? echo $tblRow; ?>" value="<? echo number_format($amount_domestic,4,'.',''); ?>" type="text" style="width:70px"  class="text_boxes_numeric" readonly /></td>
                  ...
                  <td><input name="txtQtyPcs_<? echo $tblRow; ?>" id="txtQtyPcs_<? echo $tblRow; ?>" type="text" class="text_boxes_numeric" value="<? echo number_format($qty_pcs,4,'.',''); ?>" style="width:67px" readonly />
				...
				<td><input type="text" name="txtOrderDeliveryDate_<? echo $tblRow; ?>"  id="txtOrderDeliveryDate_<? echo $tblRow; ?>" value="<? echo change_date_format($row[csf("delivery_date")]);?>" class="datepicker" style="width:50px"  onChange="calculate_date(<?php echo $tblRow; ?>)" readonly /></td>
				<td><input name="txtWastage_<? echo $tblRow; ?>" id="txtWastage_<? echo $tblRow; ?>" type="text" value="<? echo $wastage; ?>" class="text_boxes_numeric" style="width:40px" />
					...
					<input type="hidden" name="hdnDtlsdataIds_<? echo $tblRow; ?>" id="hdnDtlsdataIds_<? echo $tblRow; ?>" value="<? echo implode("__",array_filter(explode(',',$data_break_id_arr[$dtls_id]))); ?>">
	...
	else
	{
		...
		<tr>
            ...
            <td><?
				echo create_drop_down( "cboUom_1", 60, $unit_of_measurement,"", 0, "-- Select --",2,"fnc_load_uom(2,this.value);", 1,"2,1" );
            ...
            <td><input type="text" name="txtOrderDeliveryDate_1" id="txtOrderDeliveryDate_1" class="datepicker" style="width:50px" readonly/></td>
            <td>
                ...
                <input type="hidden" name="hdnDtlsdataIds_1" id="hdnDtlsdataIds_1">
...
if($action=="order_rate_popup")
{
    ...
    <script>
		...
		function fnc_close()
		{
			...
			var data_break_down=""; var data_break_down_ids="";
			for(var i=1; i<=tot_row; i++)
			{
				if($("#txt_is_rate_mandatory").val()==1){
					...
					if (form_validation('txtprodsequenceno_'+i+'*cboProcess_'+i+'*cboProcessType_'+i,'Prod. Sequence no*Process*Wash Type')==false)
					{
						return;
					}
				}
				...
				if(data_break_down_ids=="")
				{
					data_break_down_ids+=$('#hiddenid_'+i).val();
				}
				else
				{
					data_break_down_ids+="__"+$('#hiddenid_'+i).val();
				}
			}
			$('#hidden_break_tot_row').val( data_break_down );
			$('#hidden_break_tot_row_ids').val( data_break_down_ids );
		...
		function fnc_spo_data(type,is_checked)
		{
			if(is_checked==true)
			{
				$("#txt_copy_for_all_po").val(type);
			}
			else
			{
				$("#txt_copy_for_all_po").val(0);
			}
		}
	</script>
</head>
		...
		<form name="ratepopup_1"  id="ratepopup_1" autocomplete="off">
			<table class="rpt_table" width="530px" cellspacing="0" cellpadding="0" rules="all" id="tbl_share_details_entry">
				...
				<tbody>
					...
					<input type="hidden" name="hidden_break_tot_row_ids" id="hidden_break_tot_row_ids" class="text_boxes" style="width:90px" />
					<input type="hidden" id="txt_copy_for_all_po" value="0">
					<input type="hidden" id="txt_is_rate_mandatory" value="<? echo $is_rate_mandatory; ?>">
					<?
					...
					if($hdnDtlsUpdateId !='')
					{
						$sql_break_down="select id, description, process, embellishment_type, rate,prod_sequence_no from subcon_ord_breakdown where mst_id='$hdnDtlsUpdateId' and  status_active=1 and is_deleted=0 ";
					...
					if($is_rate_disabled==1) $rate_disabled="disabled"; else $rate_disabled="";
					if(count($data_array)>0)
					{
						...
						foreach($data_array as $row)
						{
							...
							<tr>
                                ...
                                <td><input type="text" id="txtorderrate_<? echo $k;?>" name="txtorderrate_<? echo $k;?>"  class="text_boxes_numeric" style="width:60px" onKeyUp="sum_total_rate(<? echo $k;?>)" value="<?

                                 if($data[3]>0){
                                	echo number_format($data[3],4,'.','');
                                }
                                 ?>" <? echo $rate_disabled; ?> />
								</td>
								<td>
					...
					else
					{
                        ...
                        <tr>
                            ...
                            <td><input type="text" id="txtorderrate_1" name="txtorderrate_1"  class="text_boxes_numeric" style="width:35px" onKeyUp="sum_total_rate(1)" value="" <? echo $rate_disabled; ?> />
                            </td>
                            <td>
			...
			<table>
				<tr>
					<td align="left"><input type="checkbox" name="allow_spo" id="allow_spo" onClick="fnc_spo_data(2,this.checked)" />Copy to Color wise</td>
					<td align="left"><input type="checkbox" name="allow_spo" id="allow_spo" onClick="fnc_spo_data(3,this.checked)" />Copy to Size wise</td>
					<td align="left"><input type="checkbox" name="allow_spo" id="allow_spo" onClick="fnc_spo_data(1,this.checked)" />Copy to all below PO</td>
                    <td align="left"><input type="checkbox" name="allow_spo" id="allow_spo" onClick="fnc_spo_data(4,this.checked)" />Copy to Buyer Style</td>
