...
if($action=="create_receive_search_list_view")
{
	...
	if($db_type==0)
	{
		...
		$receiveNo_cond="group_concat(a.prefix_no_num)";
		$receiveId_cond="group_concat(a.id)";
	...
	else if($db_type==2)
	{
		...
		$receiveNo_cond="listagg(a.prefix_no_num,',') within group (order by a.prefix_no_num)";
		$receiveId_cond="listagg(a.id,',') within group (order by a.id)";
	...
	$rec_issue_return_data_arr=array();
	 $sql_rec_return="select a.sys_no, b.quantity, b.job_dtls_id,a.embl_job_no, b.buyer_po_id,a.trans_type from sub_material_mst a, sub_material_dtls b where a.id=b.mst_id   and a.trans_type in (1,2,3,4) and a.entry_form in (372,436,297,296) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
     ...
     <div style="width:820px; max-height:270px;overflow-y:scroll;" >	 
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="800" class="rpt_table" id="tbl_po_list">
            ...
            foreach($result as $row)
            {
				...
				$multi_receive_Id='';  
				$receive_Id=array_unique(explode(",",$row[csf("receiveId")]));
				foreach($receive_Id as $val)
				{
					if($multi_receive_Id=="") $multi_receive_Id=$val; else $multi_receive_Id.=",".$val;
				}
				$multi_receive_Id=implode(",",array_unique(explode(",",$multi_receive_Id)));
				$multi_receive_no='';
				$receive_NO=array_unique(explode(",",$row[csf("receiveNo")]));
				foreach($receive_NO as $val)
				{
					if($multi_receive_no=="") $multi_receive_no=$val; else $multi_receive_no.=",".$val;
				}
				$multi_receive_no=implode(",",array_unique(explode(",",$multi_receive_no)));
				...
				$rec_qty=$rec_issue_return_data_arr[$row[csf("embl_job_no")]][1]['qty'];
				$issue_qty=$rec_issue_return_data_arr[$row[csf("embl_job_no")]][2]['qty'];
				$rec_return_qty=$rec_issue_return_data_arr[$row[csf("embl_job_no")]][3]['qty'];
				$issue_return_qty=$rec_issue_return_data_arr[$row[csf("embl_job_no")]][4]['qty'];
				
				
				//echo $rec_qty."fg".$issue_return_qty."fg".$rec_return_qty."fg".$issue_qty; die;
				$total_balance_qty=($rec_qty+$issue_return_qty)-($rec_return_qty+$issue_qty);
				
				//echo $total_balance_qty; die;
				
				if($total_balance_qty>0){
				
				?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value('<? echo $multi_receive_Id."_".$row[csf("embl_job_no")]."_".$multi_receive_no."_".$party_buyer_name;?>');" > 
...
if ($action=="load_php_data_to_form")
{
	...
	{	
		...
		echo "load_drop_down( 'requires/wash_metarial_receive_return_controller', '".$row[csf("location_id")]."'+'_'+'".$row[csf("company_id")]."', 'load_drop_down_floor', 'floor_td');\n"; 
...
if($action=="load_php_dtls_form")
{
	...
	where b.id=a.mst_id and b.embl_job_no='$jobno' and b.trans_type in (1,2,3,4) and b.entry_form in (372,436,297,296)  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0"; 
	...
	if($db_type==0)
	{
		$id_cond="group_concat(c.id)";
		$mst_id_cond="group_concat(c.mst_id)";
		$sys_no_cond="group_concat(d.sys_no)";
		 
	}
	else if($db_type==2)
	{
		$id_cond="listagg(c.id,',') within group (order by c.id)";
		$mst_id_cond="listagg(c.mst_id,',') within group (order by c.mst_id)";
		//$sys_no_cond="listagg(d.sys_no,',') within group (order by d.sys_no)";
		$sys_no_cond="LISTAGG(cast(d.sys_no as  varchar2(4000)) , ',') WITHIN GROUP (ORDER BY d.sys_no) as  sys_no";
		// $sys_no_cond="RTRIM(XMLAGG(XMLELEMENT(E,d.sys_no,',').EXTRACT('//text()') ORDER BY d.sys_no).GetClobVal(),',') AS LIST";
...
/*	$sql_job="select a.id, a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id as po_id, b.buyer_po_id, b.gmts_item_id, b.order_uom, b.gmts_color_id as color_id, b.gmts_size_id, c.quantity as qnty, b.buyer_po_no, b.buyer_style_ref,c.mst_id as receive_id ,c.id as receive_dtls_id,d.sys_no
	...
	where a.entry_form='295' and d.trans_type=1 and d.entry_form='296' and a.subcon_job=b.job_no_mst and a.id=b.mst_id and b.id=c.job_dtls_id and c.mst_id=d.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and a.subcon_job='$jobno' and d.id in ($receive_id)
	order by b.id ASC";*/
	
	//,c.mst_id as receive_id ,c.id as receive_dtls_id,d.sys_no
	
	$sql_job="select a.id, a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id as po_id, b.buyer_po_id, b.gmts_item_id, b.order_uom, b.gmts_color_id as color_id, b.gmts_size_id, sum(c.quantity) as qnty, b.buyer_po_no, b.buyer_style_ref ,$id_cond as  receive_dtls_id ,$mst_id_cond as  receive_id ,$sys_no_cond
	from subcon_ord_mst a, subcon_ord_dtls b,sub_material_dtls c,sub_material_mst d
	where a.entry_form='295' and d.trans_type=1 and d.entry_form='296' and a.subcon_job=b.job_no_mst and a.id=b.mst_id and b.id=c.job_dtls_id and c.mst_id=d.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and a.subcon_job='$jobno'   group by a.id, a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id, b.buyer_po_id, b.gmts_item_id, b.order_uom, b.gmts_color_id, b.gmts_size_id, b.buyer_po_no, b.buyer_style_ref
	order by b.id ASC";
	...
	//	echo "<pre>";
	   // print_r($pre_qty_arr);
		...
		$issueQuantity=$pre_qty_arr[$row[csf("po_id")]][2]['qty'];//issue
		
		
		
		$receiveQty=$row[csf("qnty")];
		
		//600preissue_return_qty200issueQuantity1000prerecre_return_qty
		
		//echo "10**".$receiveQty."preissue_return_qty".$preissue_return_qty."issueQuantity".$issueQuantity."prerecre_return_qty".$prerecre_return_qty."<br>";  
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   
	{
		...
		$field_array="id, entry_form, prefix_no, prefix_no_num, sys_no, trans_type, company_id, location_id, floor_id, party_id, chalan_no, subcon_date, within_group, embl_job_no, receive_no,multi_receive_id,remarks, inserted_by, insert_date, status_active, is_deleted";
		$data_array="(".$id.",'372','".$new_receive_no[1]."','".$new_receive_no[2]."','".$new_receive_no[0]."','".$trans_Type."',".$cbo_company_name.",".$cbo_location_name.",".$cbo_floor_name.",".$cbo_party_name.",".$txt_return_challan.",".$txt_return_date.",".$cbo_within_group.",".$txtJob_no.",".$txt_receive_no.",".$receive_id.",".$txt_remarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";  
		...
		$field_array2="id, mst_id, quantity, uom, job_dtls_id, buyer_po_id, multi_receive_dtls_id, multi_receive_id, receive_qty, job_id, rec_challan, remarks, inserted_by, insert_date, status_active, is_deleted";
	...
	else if ($operation==1)   // Update Here
	{
		...
		$data_array="".$cbo_location_name."*".$cbo_floor_name."*".$cbo_party_name."*".$txt_return_challan."*".$txt_return_date."*".$txtJob_no."*".$txt_remarks."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"; 
