...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   
	{
		...
		$field_array="id, entry_form, prefix_no, prefix_no_num, sys_no, trans_type, company_id, location_id, party_id, chalan_no, prod_source, issue_to, subcon_date, within_group, embl_job_no, remarks, inserted_by, insert_date, status_active, is_deleted";
		$data_array="(".$id.",'297','".$new_issue_no[1]."','".$new_issue_no[2]."','".$new_issue_no[0]."','".$trans_Type."',".$cbo_company_name.",".$cbo_location_name.",".$cbo_party_name.",".$txt_issue_challan.",1,".$cbo_company_name.",".$txt_issue_date.",".$cbo_within_group.",".$txt_job_no.",".$txt_remarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		...
		for($i=1; $i<=$total_row; $i++)
		{
			...
			$data_array2.="(".$id1.",'".$id."',".$$txtissueqty.",".$$cbouom.",".$$ordernoid.",".$$txtbuyerPoId.",".$$hidrecsyschallan.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
	...
	else if ($operation==1)   // Update Here
	{
		...
		$data_array="".$cbo_location_name."*".$cbo_party_name."*".$txt_issue_challan."*".$txt_issue_date."*".$txt_job_no."*".$txt_remarks."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"; 
...
if($action=="create_issue_search_list_view")
{
	...
	*/
	...
	$sql= "select a.id, a.sys_no, a.prefix_no_num, $insert_date_cond as year, a.location_id, a.party_id, a.subcon_date, a.within_group, a.chalan_no, a.remarks, a.embl_job_no, $wo_cond as order_id, $buyer_po_id_cond as buyer_po_id ,b.job_dtls_id from sub_material_mst a, sub_material_dtls b where a.id=b.mst_id and a.trans_type=2 and a.entry_form=297 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $issue_date $company $buyer_cond $withinGroup $issue_id_cond $challan_no_cond $order_order_buyer_poCond  group by a.id, a.sys_no, a.prefix_no_num, a.insert_date, a.location_id, a.party_id, a.subcon_date, a.within_group, a.chalan_no, a.remarks, a.embl_job_no,b.job_dtls_id order by a.id DESC ";
	//echo $sql; 
                ...
                <table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_party_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_type').value+'_'+document.getElementById('txt_search_string').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+<? echo $data[3];?>, 'create_job_search_list_view', 'search_div', 'wash_material_issue_controller', 'setFilterGrid(\'tbl_po_list\',-1)')" style="width:70px;" />
                            </td>
                        </tr>
                        <tr>
...
{	
	...
	if($db_type==0)
	{
		$ins_year_cond="year(a.insert_date)"; $color_id_str="group_concat(b.gmts_color_id)";
	}
	else if($db_type==2)
	{
        ...
        <tbody>
            ...
            {  
                ...
                <tr bgcolor="<? echo $bgcolor; ?>" onClick='js_set_value("<? echo $row[csf('subcon_job')]; ?>")' style="cursor:pointer" >
                    ...
                    <td width="100"><? if ($within_group==1) echo $buyer_po_arr[$row[csf('buyer_po_id')]]['po']; else echo $row[csf('buyer_po_no')];//echo $buyer_po;  echo $buyer_po_arr[$row[csf('buyer_po_id')]]['po']; ?></td>
                    <td width="100"><? if ($within_group==1)echo $buyer_po_arr[$row[csf('buyer_po_id')]]['style']; echo $row[csf('buyer_style_ref')];//echo $buyer_style;  echo $buyer_po_arr[$row[csf('buyer_po_id')]]['style']; ?></td>
...
if($action=="load_php_dtls_form")
{
	...
	foreach ($sql_result as $row)
	{
		...
		{
			$order_qnty_pcs=number_format($row[csf("qnty")],4,'.','');
		}
