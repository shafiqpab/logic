...
<script>
	...
	$(document).ready(function(){
		load_drop_down( 'requires/pi_garments_service_controller',document.getElementById('cbo_importer_id').value, 'load_supplier_dropdown', 'supplier_td' );
		show_list_view(document.getElementById('cbo_pi_basis_id').value+'_'+'30'+'_'+'1', 'pi_details', 'pi_details_container', 'requires/pi_garments_service_controller', '' ) ;
	})
	...
	function fnc_pi_mst( operation )
	{
		...
		if(operation==4)
		{ 
			 print_report( $('#cbo_importer_id').val()+'*'+$('#update_id').val()+'*'+'197', "print", "requires/pi_print_urmi" );
			 return;
		}
		...
		{ 
			...
			http.open("POST","requires/pi_garments_service_controller.php",true);
	...
	function fnc_pi_mst_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			{
				reset_form('pimasterform_1','pi_details_container','','cbo_currency_id,2*cbo_pi_basis_id,2',"disable_enable_fields('cbo_pi_basis_id',0)");
			}
			...
			if(cbo_pi_basis_id==1)
			{
				for (var j=1; j<=row_num; j++)
				{
					...
					if($('#workOrderChkbox_'+j).is(':checked') || updateIdDtls!="")
					{
						...
						i++;
						data_all+="&workOrderNo_" + i + "='" + $('#workOrderNo_'+j).val()+"'"+"&hideWoId_" + i + "='" + $('#hideWoId_'+j).val()+"'"+"&hideWoDtlsId_" + i + "='" + $('#hideWoDtlsId_'+j).val()+"'"+"&hideWoQtyDtlsId_"+ i + "='" + $('#hideWoQtyDtlsId_'+j).val() + "'"+ "&gmtsitem_" + i + "='" + $('#gmtsitem_'+j).val()+"'"+"&colorName_" + i + "='" + $('#colorName_'+j).val()+"'"+ "&itemSize_" + i + "='" + $('#itemSize_'+j).val()+"'"+"&rateVariable_" + i + "='" + $('#rateVariable_'+j).val()+"'"+"&uom_" + i + "='" + $('#uom_'+j).val()+"'"+"&quantity_" + i + "='" + $('#quantity_'+j).val()+"'"+"&rate_" + i + "='" + $('#rate_'+j).val()+"'"+"&amount_" + i + "='" + $('#amount_'+j).val()+"'"+"&remarks_" + i + "='" + $('#remarks_'+j).val()+"'"+"&updateIdDtls_" + i + "='" + $('#updateIdDtls_'+j).val()+"'"+"&hideTransData_" + i + "='" + $('#hideTransData_'+j).val()+"'"+"&txtOrderNo_" + i + "='" + $('#txtOrderNo_'+j).val()+"'"+"&txtOrderSource_" + i + "='" + $('#txtOrderSource_'+j).val()+"'";
	...
	function fnc_pi_item_details_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			{	
				...
				show_list_view(cbo_pi_basis_id+'_'+''+'_'+'2'+'_'+reponse[1]+'_'+goods_rcv_status, 'pi_details', 'pi_details_container', 'requires/pi_garments_service_controller', '' ) ;
				var numRow = $('table#tbl_pi_item tbody tr').length; 
				if(reponse[0]==2)
				{
	...
	function openmypage()
	{
		...
		{ 	
			...
			var page_link = 'requires/pi_garments_service_controller.php?item_category_id='+item_category_id+'&importer_id='+importer_id+'&supplier_id='+supplier_id+'&action=pi_popup';
			...
			emailwindow.onclose=function()
			{
				...
				{
					freeze_window(5);
					get_php_form_data( theemail.value, "populate_data_from_search_popup", "requires/pi_garments_service_controller" );
					//alert(cbo_pi_basis_id);
					show_list_view($('#cbo_pi_basis_id').val()+'_'+''+'_'+'2'+'_'+theemail.value+'_'+$('#cbo_goods_rcv_status').val(),'pi_details','pi_details_container', 'requires/pi_garments_service_controller','');
	...
	/*function openmypage_fabricDescription(row_num)
	{
		...
		var page_link = 'requires/pi_garments_service_controller.php?action=fabricDescription_popup&fabricNature='+fabricNature+"&prev_attached_id="+prev_attached_id;
	...
	function openmypage_test_item(row_num)
	{
		...
		var page_link = 'requires/pi_garments_service_controller.php?action=testItem_popup&cboTestFor='+cboTestFor+"&prev_attached_id="+prev_attached_id;
	...
	function openmypage_wo(row_num)
	{
		...
		if($('#txt_system_id').val()=="")
		{
			...
			var page_link = 'requires/pi_garments_service_controller.php?item_category_id='+item_category_id+'&importer_id='+importer_id+'&supplier_id='+supplier_id+'&goods_rcv_status='+goods_rcv_status+'&prev_wo_ids='+prev_wo_ids+'&prev_wo_feb_datas='+prev_wo_feb_datas+'&action=wo_popup';
			//alert(page_link);
			...
			emailwindow.onclose=function()
			{
				...
				var order_type=this.contentDoc.getElementById("order_non_order_type").value; 
				//$('#txt_order_type').val(order_type); 
				var cbo_based_on=this.contentDoc.getElementById("cbo_based_on").value; 

				if (theemail.value!="")
				{
					var tot_row=$('#txt_tot_row').val();
					var data=theemail.value+"**"+item_category_id+"**"+tot_row+"**"+order_type+"**"+goods_rcv_status+"**"+importer_id+"**"+cbo_pi_basis_id+"**"+cbo_based_on;
	  
					//freeze_window(5);
					var list_view_wo =trim(return_global_ajax_value( data, 'populate_data_wo_form', '', 'requires/pi_garments_service_controller'));
	...
	{ 	
			...
			var page_link = 'requires/pi_garments_service_controller.php?item_category='+''+'&action=itemDesc_popup';
			...
			emailwindow.onclose=function()
			{
				...
				if (theemail.value!="")
				{
					...
					var list_view_wo =return_global_ajax_value( data, 'populate_data_item_form', '', 'requires/pi_garments_service_controller');
	...
	function openmypage_exportPi()
	{
		...
		var page_link = 'requires/pi_garments_service_controller.php?action=export_pi_popup';
		...
		emailwindow.onclose=function()
		{
			...
			if(theemail.value!="")
			{
				...
				if(datas[18]*1>0)
				{
					show_list_view(datas[18]+"**"+datas[14]+"**"+datas[15]+"**"+datas[16]+"**"+datas[17],'export_pi_details_update','pi_details_container','requires/pi_garments_service_controller','');
					set_button_status(1, permission, 'fnc_pi_mst',1);
				}
				else
				{
					show_list_view(datas[0]+"**"+datas[14]+"**"+datas[15]+"**"+datas[16]+"**"+datas[17], 'export_pi_details', 'pi_details_container', 'requires/pi_garments_service_controller','');
	...
	function openmypage_user()
	{
		...
		var page_link = 'requires/pi_garments_service_controller.php?action=approvalUser_popup&menu_id='+menu_id;
            ...
            <legend>PI Details</legend>
                <table width="100%" border="0" cellpadding="0" cellspacing="2">
                    ...
                    <tr>
                        ...
                        <td id="importer_td"><?php echo create_drop_down( "cbo_importer_id", 151,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business in(1,3) $company_cond order by comp.company_name",'id,company_name', 1, 'Select',0,"load_drop_down( 'requires/pi_garments_service_controller',this.value, 'load_supplier_dropdown', 'supplier_td' );",0); ?>       
                        </td>
                        ...
                        <td><?php echo create_drop_down( "cbo_pi_basis_id", 151, $pi_basis,'', 0, '',1,"",1,"1"); ?>  
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td><?php echo create_drop_down( "cbo_goods_rcv_status", 151, $acceptance_time,'', 0, '',2,"",1); ?></td>
                        <td>LC Group No.</td>
