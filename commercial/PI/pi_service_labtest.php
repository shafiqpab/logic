...
<script>
	...
	$(document).ready(function(){
		...
		load_drop_down( 'requires/pi_service_labtest_controller',document.getElementById('cbo_importer_id').value+'_'+cbo_item_category_id, 'load_supplier_dropdown', 'supplier_td' );
		show_list_view(document.getElementById('cbo_pi_basis_id').value+'_'+cbo_item_category_id+'_'+'1', 'pi_details', 'pi_details_container', 'requires/pi_service_labtest_controller', '' ) ;
	});
	...
	function fnc_pi_mst( operation )
	{
		...
		if(operation==4)
		{ 
			 print_report( $('#cbo_importer_id').val()+'*'+$('#update_id').val()+'*'+'171', "print", "requires/pi_print_urmi" );
			 return;
		}
		...
		{ 
			...
			http.open("POST","requires/pi_service_labtest_controller.php",true);
	...
	{ 
		...
		{ 
			...
			{
				$('#itemgroupid_'+i).removeAttr("onchange").attr("onchange","get_php_form_data(this.value+'**'+'uom_"+i+"','get_uom', 'requires/pi_service_labtest_controller')");
			}
			...
			{
				$('#embellname_'+i).removeAttr("onchange").attr("onchange","load_drop_down('requires/pi_service_labtest_controller',this.value+'**'+0+'**'+'embelltype_"+i+"', 'load_drop_down_embelltype','embelltypeTd_"+i+"')");
	...
	function fnc_pi_item_details( operation )
	{
		...
		else
		{
			...
			else
			{
				for (var j=1; j<=row_num; j++)
				{
					if (form_validation('cboTestFor_'+j+'*amount_'+j,'Test For*Amount')==false)
					{
						return;
					}
					i++;
					data_all+=get_submitted_data_string('cboTestFor_'+i+'*remarks_'+i+'*colorName_'+i+'*testItemId_'+i+'*amount_'+i+'*updateIdDtls_'+i,"../../",i);
	...
	function fnc_pi_item_details_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			{	
				...
				show_list_view(cbo_pi_basis_id+'_'+item_category+'_'+'2'+'_'+reponse[1]+'_'+goods_rcv_status, 'pi_details', 'pi_details_container', 'requires/pi_service_labtest_controller', '' ) ;
				var numRow = $('table#tbl_pi_item tbody tr').length; 
				if(reponse[0]==2)
				{
	...
	function openmypage()
	{
		...
		{ 	
			...
			var page_link = 'requires/pi_service_labtest_controller.php?item_category='+item_category+'&importer_id='+importer_id+'&supplier_id='+supplier_id+'&action=pi_popup';
			...
			emailwindow.onclose=function()
			{
				...
				{
					freeze_window(5);
					get_php_form_data( theemail.value, "populate_data_from_search_popup", "requires/pi_service_labtest_controller" );
					//alert(cbo_pi_basis_id);
					show_list_view($('#cbo_pi_basis_id').val()+'_'+item_category+'_'+'2'+'_'+theemail.value+'_'+$('#cbo_goods_rcv_status').val(),'pi_details','pi_details_container', 'requires/pi_service_labtest_controller','');
	...
	function openmypage_fabricDescription(row_num)
	{
		...
		var page_link = 'requires/pi_service_labtest_controller.php?action=fabricDescription_popup&fabricNature='+fabricNature+"&prev_attached_id="+prev_attached_id;
	...
	function openmypage_test_item(row_num)
	{
		...
		var page_link = 'requires/pi_service_labtest_controller.php?action=testItem_popup&cboTestFor='+cboTestFor+"&prev_attached_id="+prev_attached_id;
	...
	function openmypage_wo(row_num)
	{
		...
		else
		{
			...
			var page_link = 'requires/pi_service_labtest_controller.php?item_category_id='+item_category+'&importer_id='+importer_id+'&supplier_id='+supplier_id+'&goods_rcv_status='+goods_rcv_status+'&prev_wo_ids='+prev_wo_ids+'&prev_wo_feb_datas='+prev_wo_feb_datas+'&action=wo_popup';
			//alert(page_link);
			...
			emailwindow.onclose=function()
			{
				...
				if (theemail.value!="")
				{
					...
					//var list_view_wo =trim(return_global_ajax_value( data, 'populate_data_wo_form', '', 'requires/pi_service_labtest_controller'));
	...
	function openmypage_item_desc(row_num)
	{
		...
		{ 	
			...
			var page_link = 'requires/pi_service_labtest_controller.php?item_category='+item_category+'&action=itemDesc_popup';
			...
			emailwindow.onclose=function()
			{
				...
				if (theemail.value!="")
				{
					...
					var list_view_wo =return_global_ajax_value( data, 'populate_data_item_form', '', 'requires/pi_service_labtest_controller');
	...
	function openmypage_exportPi()
	{
		...
		var page_link = 'requires/pi_service_labtest_controller.php?action=export_pi_popup';
		...
		emailwindow.onclose=function()
		{
			...
			if(theemail.value!="")
			{
				...
				if(datas[18]*1>0)
				{
					show_list_view(datas[18]+"**"+datas[14]+"**"+datas[15]+"**"+datas[16]+"**"+datas[17],'export_pi_details_update','pi_details_container','requires/pi_service_labtest_controller','');
					set_button_status(1, permission, 'fnc_pi_mst',1);
				}
				else
				{
					show_list_view(datas[0]+"**"+datas[14]+"**"+datas[15]+"**"+datas[16]+"**"+datas[17], 'export_pi_details', 'pi_details_container', 'requires/pi_service_labtest_controller','');
	...
	function openmypage_user()
	{
		...
		var page_link = 'requires/pi_service_labtest_controller.php?action=approvalUser_popup&menu_id='+menu_id;
            ...
            <legend>PI Details</legend>
                <table width="100%" border="0" cellpadding="0" cellspacing="2">
                    ...
                    <tr>
                        ...
                        <?php echo create_drop_down( "cbo_item_category_id", 151, $item_category,'', 1, '--Select--',31,"",1,'31','','','74,72,79,73,71,77,78,75,76'); ?>  
                        
                        <td class="must_entry_caption"  width="110">Importer</td>
                        <td id="importer_td"><?php echo create_drop_down( "cbo_importer_id", 151,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business in(1,3) $company_cond order by comp.company_name",'id,company_name', 1, 'Select',0,"load_drop_down( 'requires/pi_service_labtest_controller',this.value+'_'+document.getElementById('cbo_item_category_id').value, 'load_supplier_dropdown', 'supplier_td' );",0); ?>       
                        </td>
                        ...
                        <td><?php echo create_drop_down( "cbo_pi_basis_id", 151, $pi_basis,'', 0, '',1,"show_list_view(this.value+'_'+document.getElementById('cbo_item_category_id').value+'_'+'1', 'pi_details', 'pi_details_container', 'requires/pi_service_labtest_controller', '' ) ;",0); ?>  
                        </td>				
                    </tr>
                    <tr>
