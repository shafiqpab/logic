...
<script>
	...
	$(document).ready(function(){
		$('#cbo_item_category_id_for_export').css("display","none");
		load_drop_down( 'requires/pi_fabric_controller',document.getElementById('cbo_importer_id').value, 'load_supplier_dropdown', 'supplier_td' );
		show_list_view(document.getElementById('cbo_pi_basis_id').value+'_'+''+'_'+'1','pi_details', 'pi_details_container', 'requires/pi_fabric_controller','');
	});
...
/*		if((cbo_item_category_id==12 || cbo_item_category_id==25 || cbo_item_category_id==31) && cbo_goods_rcv_status==1)
		...
		if(operation==4)
		{ 
			 print_report( $('#cbo_importer_id').val()+'*'+$('#update_id').val()+'*'+'166', "print", "requires/pi_print_urmi" );
			 return;
		}
		...
		{ 
			...
			if(export_pi_id>0)
			{
				...
				var data="action=save_update_delete&operation="+operation+'&total_row='+row_num+get_submitted_data_string('cbo_item_category_id_for_export*cbo_importer_id*cbo_supplier_id*pi_number*pi_date*last_shipment_date*pi_validity_date*cbo_currency_id*cbo_source_id*hs_code*txt_internal_file_no*intendor_name*cbo_pi_basis_id*txt_remarks*hide_approved_status*update_id*cbo_goods_rcv_status*export_pi_id*within_group*txt_total_amount*txt_upcharge*txt_discount*txt_total_amount_net*cbo_ready_to_approved*txt_lc_group_no*hiddn_user_id',"../../")+data_all;
			}
			else
			{
				var data="action=save_update_delete&operation="+operation+get_submitted_data_string('cbo_item_category_id_for_export*cbo_importer_id*cbo_supplier_id*pi_number*pi_date*last_shipment_date*pi_validity_date*cbo_currency_id*cbo_source_id*hs_code*txt_internal_file_no*intendor_name*cbo_pi_basis_id*txt_remarks*hide_approved_status*update_id*cbo_goods_rcv_status*export_pi_id*within_group*txt_total_amount*txt_upcharge*txt_discount*txt_total_amount_net*cbo_ready_to_approved*txt_lc_group_no*hiddn_user_id',"../../");	
			}
			
			freeze_window(operation);
			http.open("POST","requires/pi_fabric_controller.php",true);
	...
	{ 
		...
		{ 
			...
			{
				$('#itemgroupid_'+i).removeAttr("onchange").attr("onchange","get_php_form_data(this.value+'**'+'uom_"+i+"','get_uom', 'requires/pi_fabric_controller')");
			}
			...
			{
				$('#embellname_'+i).removeAttr("onchange").attr("onchange","load_drop_down('requires/pi_fabric_controller',this.value+'**'+0+'**'+'embelltype_"+i+"', 'load_drop_down_embelltype','embelltypeTd_"+i+"')");
	...
	function fnc_pi_item_details( operation )
	{
		...
		else
		{
			...
			/*
			if(cbo_item_category_id==2 || cbo_item_category_id==13)
			{
				...
				else
				{
					for(var j=1; j<=row_num; j++)
					{
						...
						if(quantity>0)
						{
							if (form_validation('construction_'+j+'*colorName_'+j+'*diawidth_'+j+'*uom_'+j,'Construction*Color*Width*UOM')==false)
							{
								return;
							}
						}
				...
				if(cbo_pi_basis_id==1)
				{
					for (var j=1; j<=row_num; j++)
					{
						...
						if($('#workOrderChkbox_'+j).is(':checked') || updateIdDtls!="")
						{
							...
							data_all+="&workOrderNo_" + i + "='" + $('#workOrderNo_'+j).val()+"'"+"&hideWoId_" + i + "='" + $('#hideWoId_'+j).val()+"'"+"&hideWoDtlsId_" + i + "='" + $('#hideWoDtlsId_'+j).val()+"'" + "&itemCategoryId_" + i + "='" + $('#itemCategoryId_'+j).val() + "'" + "&construction_" + i + "='" + $('#construction_'+j).val()+"'"+"&composition_" + i + "='" + $('#composition_'+j).val()+"'"+"&colorName_" + i + "='" + $('#colorName_'+j).val()+"'"+"&weight_" + i + "='" + $('#weight_'+j).val()+"'"+"&gsm_" + i + "='" + $('#gsm_'+j).val()+"'"+"&diawidth_" + i + "='" + $('#diawidth_'+j).val()+"'"+"&uom_" + i + "='" + $('#uom_'+j).attr('placeholder')+"'"+"&quantity_" + i + "='" + $('#quantity_'+j).val()+"'"+"&rate_" + i + "='" + $('#rate_'+j).val()+"'"+"&amount_" + i + "='" + $('#amount_'+j).val()+"'"+"&updateIdDtls_" + i + "='" + $('#updateIdDtls_'+j).val()+"'"+"&hideDeterminationId_" + i + "='" + $('#hideDeterminationId_'+j).val()+"'"+"&bookingWithoutOrder_" + i + "='" + $('#bookingWithoutOrder_'+j).val()+"'"+     "&hideTransData_" + i + "='" + $('#hideTransData_'+j).val()+"'";
				...
				else
				{
					for (var j=1; j<=row_num; j++)
					{
						...
						if(quantity>0 || updateIdDtls!="")
						{
							...
							data_all+="&itemCategoryId_" + i + "='" + $('#itemCategoryId_'+j).val()+"'"+"&construction_" + i + "='" + $('#construction_'+j).val()+"'"+"&composition_" + i + "='" + $('#composition_'+j).val()+"'"+"&colorName_" + i + "='" + $('#colorName_'+j).val()+"'"+"&weight_" + i + "='" + $('#weight_'+j).val()+"'"+"&gsm_" + i + "='" + $('#gsm_'+j).val()+"'"+"&diawidth_" + i + "='" + $('#diawidth_'+j).val()+"'"+"&uom_" + i + "='" + $('#uom_'+j).val()+"'"+"&quantity_" + i + "='" + $('#quantity_'+j).val()+"'"+"&rate_" + i + "='" + $('#rate_'+j).val()+"'"+"&amount_" + i + "='" + $('#amount_'+j).val()+"'"+"&updateIdDtls_" + i + "='" + $('#updateIdDtls_'+j).val()+"'"+"&hideDeterminationId_" + i + "='" + $('#hideDeterminationId_'+j).val()+"'";
	...
	function fnc_pi_item_details_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			{	
				...
				show_list_view(cbo_pi_basis_id+'_'+''+'_'+'2'+'_'+reponse[1]+'_'+goods_rcv_status, 'pi_details', 'pi_details_container', 'requires/pi_fabric_controller', '' ) ;
				var numRow = $('table#tbl_pi_item tbody tr').length; 
				if(reponse[0]==2)
				{
	...
	function openmypage()
	{
		...
		if (form_validation('cbo_importer_id','Item Category')==false)
		{
			return;
		}
		else
		{ 	
			...
			var page_link = 'requires/pi_fabric_controller.php?item_category_id='+''+'&importer_id='+importer_id+'&supplier_id='+supplier_id+'&action=pi_popup';
			...
			emailwindow.onclose=function()
			{
				...
				{
					freeze_window(5);
					get_php_form_data( theemail.value, "populate_data_from_search_popup", "requires/pi_fabric_controller" );
					//alert(cbo_pi_basis_id);
					show_list_view($('#cbo_pi_basis_id').val()+'_'+''+'_'+'2'+'_'+theemail.value+'_'+$('#cbo_goods_rcv_status').val(),'pi_details','pi_details_container', 'requires/pi_fabric_controller','');
	...
	function openmypage_fabricDescription(row_num)
	{
		...
		var page_link = 'requires/pi_fabric_controller.php?action=fabricDescription_popup&fabricNature='+fabricNature+"&prev_attached_id="+prev_attached_id;
	...
	function openmypage_test_item(row_num)
	{
		...
		var page_link = 'requires/pi_fabric_controller.php?action=testItem_popup&cboTestFor='+cboTestFor+"&prev_attached_id="+prev_attached_id;
	...
	function openmypage_wo(row_num)
	{
		...
		{*/
			...
			var page_link = 'requires/pi_fabric_controller.php?item_category_id='+item_category_id+'&importer_id='+importer_id+'&supplier_id='+supplier_id+'&goods_rcv_status='+goods_rcv_status+'&prev_wo_ids='+prev_wo_ids+'&prev_wo_feb_datas='+prev_wo_feb_datas+'&action=wo_popup';
			//alert(page_link);
			...
			emailwindow.onclose=function()
			{
				...
				var cbo_item_category_id=this.contentDoc.getElementById("cbo_item_category_id").value; //Access form field with id="emailfield"
				...
				if (theemail.value!="")
				{
					...
					var data=theemail.value+"**"+cbo_item_category_id+"**"+tot_row+"**"+order_type+"**"+goods_rcv_status+"**"+importer_id+"**"+cbo_pi_basis_id;
	  
					freeze_window(5);
					var list_view_wo =trim(return_global_ajax_value( data, 'populate_data_wo_form', '', 'requires/pi_fabric_controller'));
	...
	function openmypage_item_desc(row_num)
	{
		...
		//{ 	
			...
			var page_link = 'requires/pi_fabric_controller.php?item_category_id='+''+'&action=itemDesc_popup';
			...
			emailwindow.onclose=function()
			{
				...
				if (theemail.value!="")
				{
					...
					var list_view_wo =return_global_ajax_value( data, 'populate_data_item_form', '', 'requires/pi_fabric_controller');
	...
	function openmypage_exportPi()
	{
		...
		var page_link = 'requires/pi_fabric_controller.php?action=export_pi_popup';
		...
		emailwindow.onclose=function()
		{
			...
			if(theemail.value!="")
			{
				...
				//$('#cbo_item_category_id_for_export').css("display","none");
				$("#cbo_item_category_id_for_export option[value!='0']").remove();
				$("#cbo_item_category_id_for_export").append("<option selected value='"+datas[22]+"'>"+datas[23]+"</option>");
				...
				disable_enable_fields('cbo_item_category_id_for_export*cbo_importer_id*cbo_supplier_id*pi_number*pi_date*last_shipment_date*pi_validity_date*cbo_currency_id*hs_code*txt_internal_file_no*cbo_pi_basis_id',1,'','');
				
				if(datas[18]*1>0)
				{
					show_list_view(datas[18]+"**"+datas[14]+"**"+datas[15]+"**"+datas[16]+"**"+datas[17],'export_pi_details_update','pi_details_container','requires/pi_fabric_controller','');
					set_button_status(1, permission, 'fnc_pi_mst',1);
				}
				else
				{
					show_list_view(datas[0]+"**"+datas[14]+"**"+datas[15]+"**"+datas[16]+"**"+datas[17], 'export_pi_details', 'pi_details_container', 'requires/pi_fabric_controller','');
	...
	function openmypage_user()
	{
		...
		var page_link = 'requires/pi_fabric_controller.php?action=approvalUser_popup&menu_id='+menu_id;
            ...
            <legend>PI Details</legend>
                <table width="100%" border="0" cellpadding="0" cellspacing="2">
                    ...
                    <tr>
                        ...
                        <?php echo create_drop_down( "cbo_item_category_id_for_export", 151, $blank_array,'', 1, '--Select--',0,"",0,'','','','74,72,79,73,71,77,78,75,76'); ?>  
                        
                        <td class="must_entry_caption"  width="110">Importer</td>
                        <td id="importer_td"><?php echo create_drop_down( "cbo_importer_id", 151,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business in(1,3) $company_cond order by comp.company_name",'id,company_name', 1, 'Select',0,"load_drop_down( 'requires/pi_fabric_controller',this.value, 'load_supplier_dropdown', 'supplier_td' );",0); ?>       
                        </td>
                        ...
                        <td><?php echo create_drop_down( "cbo_pi_basis_id", 151, $pi_basis,'', 0, '',1,"show_list_view(this.value+'_'+''+'_'+'1', 'pi_details', 'pi_details_container', 'requires/pi_fabric_controller', '' ) ;",0); ?>  
                        </td>				
                    </tr>
                    <tr>
