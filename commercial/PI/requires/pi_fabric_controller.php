...
if ($action=="load_supplier_dropdown")
{
	...
	echo create_drop_down( "cbo_supplier_id", 151,"select DISTINCT(c.supplier_name),c.id from  lib_supplier_tag_company a,lib_supplier_party_type b, lib_supplier c where c.id=b.supplier_id and a.supplier_id = b.supplier_id and a.tag_company='$data' and b.party_type =9 and c.status_active=1 and c.is_deleted=0 order by c.supplier_name",'id,supplier_name', 1, '-- Select Supplier --',0,'',0);
...
{ 
	...
	{ 
		...
		else
		{
			...
			{
				$field_array_dtls="id, pi_id, work_order_no, work_order_id, work_order_dtls_id, determination_id, fabric_construction, fabric_composition, color_id, gsm, dia_width, uom, quantity, rate, amount, net_pi_rate, net_pi_amount, item_category_id, entry_form, inserted_by, insert_date";
	...
	<form name="searchpifrm"  id="searchpifrm">
		...
		<legend>Enter search words</legend>           
            <table cellpadding="0" cellspacing="0" border="1" rules="all" width="800" class="rpt_table">
                ...
                <tr class="general">
                    <td>
						<?php echo create_drop_down( "cbo_within_group", 151, $yes_no,"", 1, "-- Select --", 0, "load_drop_down( 'pi_fabric_controller',this.value, 'load_drop_down_importer', 'importer_td' );" ); ?>
                    </td>
                    <td id="importer_td"> 
...
{ 
	...
	if ($operation==0)  // Insert Here
	{
		...
		if(str_replace("'", '',$cbo_goods_rcv_status)==1)
		{
			...
			$previous_pi_qty=return_library_array("select b.work_order_dtls_id as id, sum(b.quantity) as qnty from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and b.item_category_id in ($all_itemCategoryId) and a.goods_rcv_status=1 and b.status_active=1 and b.work_order_dtls_id in($all_dtls_id) group by b.work_order_dtls_id","id","qnty");
		...
		else
		{
			...
			$previous_pi_qty=return_library_array("select b.work_order_dtls_id as id, sum(b.quantity) as qnty from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and b.item_category_id in ($all_itemCategoryId ) and a.goods_rcv_status<>1 and b.status_active=1 $booking_id_cond group by b.work_order_dtls_id","id","qnty");
			
			if(str_replace("'", '',$txt_order_type)==2)
			{
				$wo_qnty_arr=return_library_array("select b.id, b.fin_fab_qnty as qnty from wo_non_ord_samp_booking_mst a, wo_non_ord_samp_booking_dtls b where a.booking_no=b.booking_no and b.status_active=1 and a.id in($all_WoId)","id","qnty");
				$prev_pi_qty_arr=return_library_array( "select b.work_order_dtls_id,sum(b.quantity) as qty from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and b.item_category_id in ($all_itemCategoryId) and a.pi_basis_id=1 and a.goods_rcv_status=$cbo_goods_rcv_status and b.status_active=1 and b.is_deleted=0 and b.work_order_id in($all_WoId) group by b.work_order_dtls_id", "work_order_dtls_id", "qty");
			}
			else
			{
				
				$wo_qnty_sql=sql_select("select a.id, a.booking_no, b.fabric_color_id, c.construction as construction, c.composition as copmposition, b.gsm_weight, b.dia_width, sum(b.fin_fab_qnty) as fin_fab_qnty, sum(b.fin_fab_qnty*b.rate) as amount, c.lib_yarn_count_deter_id, c.uom, 0 as dtls_id 
				from wo_booking_mst a, wo_booking_dtls b, wo_pre_cost_fabric_cost_dtls c 
				where a.booking_no=b.booking_no and b.pre_cost_fabric_cost_dtls_id=c.id and a.id in($all_WoId) 
				group by a.id, a.booking_no, b.fabric_color_id, c.construction, c.composition, b.gsm_weight, b.dia_width,c.lib_yarn_count_deter_id,c.uom");
				
				foreach($wo_qnty_sql as $row)
				{
					$wo_qnty_arr[$row[csf('booking_no')]][$row[csf('lib_yarn_count_deter_id')]][$row[csf('fabric_color_id')]][$row[csf('construction')]][$row[csf('copmposition')]][$row[csf('gsm_weight')]][$row[csf('dia_width')]][$row[csf('uom')]]=$row[csf('fin_fab_qnty')];
				}
				
				$sql_prev="select b.work_order_no, b.determination_id, b.color_id, b.fabric_construction, b.fabric_composition, b.gsm, b.dia_width, b.uom, sum(b.quantity) as qty from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and b.item_category_id ($all_itemCategoryId) and a.pi_basis_id=1 and a.goods_rcv_status=$cbo_goods_rcv_status and b.status_active=1 and b.is_deleted=0 and b.work_order_id in($all_WoId) group by b.work_order_no, b.determination_id, b.color_id, b.fabric_construction, b.fabric_composition, b.gsm, b.dia_width, b.uom";// and b.work_order_dtls_id in($wo_dtls_id)
			...
			if(str_replace("'", '',$cbo_pi_basis_id)==1 && str_replace("'", '',$cbo_goods_rcv_status)==1 )
			{
				...
				foreach($hideTransData_arr as $hide_trans_val)
				{
					...
					{ 
						...
						{
							$color_id = return_id( str_replace("'","",$$colorName), $color_library, "lib_color", "id,color_name","166");  
				...
				if(str_replace("'", '',$cbo_pi_basis_id)==1)
				{
					...
					else
					{
						if( str_replace("'", '',$txt_order_type)==1)
						{
							...
							$wo_prev_qnty=$wo_qnty_arr[str_replace("'","",$$workOrderNo)][str_replace("'","",$$determinationId)][str_replace("'","",$color_id)][str_replace("'","",$$construction)][str_replace("'","",$$composition)][str_replace("'","",$$gsm)][str_replace("'","",$$diawidth)][str_replace("'","",$$uom)];
							$pi_prev_qnty=$prev_pi_qty_arr[str_replace("'","",$$workOrderNo)][str_replace("'","",$$determinationId)][str_replace("'","",$color_id)][str_replace("'","",$$construction)][str_replace("'","",$$composition)][str_replace("'","",$$gsm)][str_replace("'","",$$diawidth)][str_replace("'","",$$uom)];
							$bal_wo_qnty=($wo_prev_qnty-$pi_prev_qnty);		
							/*echo "11**".$$quantity."=".$wo_prev_qnty."=".$pi_prev_qnty;
							print_r($wo_qnty_arr)."<br>============";
						echo str_replace("'","",$$workOrderNo)."=".str_replace("'","",$$determinationId)."=".str_replace("'","",$color_id)."=".str_replace("'","",$$construction)."=".str_replace("'","",$$composition)."=".str_replace("'","",$$gsm)."=".str_replace("'","",$$diawidth)."=".str_replace("'","",$$uom);
							die;*/						
						}
						else
						{
	...
	{ 
		...
		$all_dtls_id="";$all_itemCategoryId = "";$all_bookingWithoutOrder= "";
		for($i=1;$i<=$total_row;$i++)
		{
			$workOrderId="hideWoId_".$i;
			$workOrderDtlsId="hideWoDtlsId_".$i;
		...
		//echo "11**".$all_itemCategoryId."=".$all_bookingWithoutOrder;die;
		if($all_bookingWithoutOrder == 0)
		{
			$txt_order_type = 1;
		}else if($all_bookingWithoutOrder == 1){
			$txt_order_type = 2;
		}
		...
		if(str_replace("'", '',$cbo_goods_rcv_status)==2)
		{
			...
			where a.id=b.pi_id and b.item_category_id in ($all_itemCategoryId) and a.goods_rcv_status<>1 and b.status_active=1 $booking_id_cond and a.id not in($update_id) group by b.work_order_dtls_id","id","qnty");
			
		
			if(str_replace("'", '',$txt_order_type)==2)
			{
				$wo_qnty_arr=return_library_array("select b.id, b.fin_fab_qnty as qnty from wo_non_ord_samp_booking_mst a, wo_non_ord_samp_booking_dtls b where a.booking_no=b.booking_no and b.status_active=1 and a.id in($all_WoId)","id","qnty");
				$prev_pi_qty_arr=return_library_array( "select b.work_order_dtls_id,sum(b.quantity) as qty from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and b.item_category_id in ($all_itemCategoryId) and a.pi_basis_id=1 and a.goods_rcv_status=$goods_rcv_status and b.status_active=1 and b.is_deleted=0 and b.work_order_id in($all_WoId) and a.id not in($update_id) group by b.work_order_dtls_id", "work_order_dtls_id", "qty");
			...
			else
			{
				...
				//echo "11**";print_r($wo_qnty_arr);die;
				
				//$all_WoId
				
				$sql_prev="select b.work_order_no, b.determination_id, b.color_id, b.fabric_construction, b.fabric_composition, b.gsm, b.dia_width, b.uom, sum(b.quantity) as qty from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and b.item_category_id in ($all_itemCategoryId) and a.pi_basis_id=1 and a.goods_rcv_status=$goods_rcv_status and b.status_active=1 and b.is_deleted=0 and b.work_order_id in($all_WoId) and a.id not in($update_id) group by b.work_order_no, b.determination_id, b.color_id, b.fabric_construction, b.fabric_composition, b.gsm, b.dia_width, b.uom";// and b.work_order_dtls_id in($wo_dtls_id)
				$prevData=sql_select($sql_prev);
				foreach($prevData as $rowP)
				{
					$prev_pi_qty_arr[$rowP[csf('work_order_no')]][$rowP[csf('determination_id')]][$rowP[csf('color_id')]][$rowP[csf('fabric_construction')]][$rowP[csf('fabric_composition')]][$rowP[csf('gsm')]][$rowP[csf('dia_width')]][$rowP[csf('uom')]]=$rowP[csf('qty')];
...
{	
    ...
    <table class="rpt_table" width="100%" cellspacing="0" cellpadding="0" border="1" rules="all" id="tbl_pi_item">
		...
		?>
            ...
            {
				$nameArray=sql_select( "select id, work_order_no, work_order_id, work_order_dtls_id, determination_id, color_id, fabric_construction, fabric_composition, gsm, dia_width, uom, booking_without_order, quantity, rate, amount,item_category_id from com_pi_item_details where pi_id='$pi_id' and quantity>0 and status_active=1 and is_deleted=0" );
            }
            else
            {
				...
				if($db_type==0)
				{
					...
					$nameArray=sql_select( "select group_concat(id) as id, work_order_no, work_order_id, group_concat(work_order_dtls_id) as work_order_dtls_id, determination_id, color_id, fabric_construction,item_category_id, fabric_composition, gsm, dia_width, uom, booking_without_order, sum(quantity) as quantity, avg(rate) as rate, sum(amount) as amount
					from com_pi_item_details 
					where pi_id='$pi_id' and quantity>0 and status_active=1 and is_deleted=0
					group by work_order_no, work_order_id, determination_id, color_id,item_category_id, fabric_construction, fabric_composition, gsm, dia_width, uom, booking_without_order" );
				...
				else
				{
					...
					$nameArray=sql_select( "select listagg(cast(id as varchar(4000)), ',') within group(order by id) as id, work_order_no, work_order_id, listagg(cast(work_order_dtls_id as varchar(4000)), ',') within group(order by work_order_dtls_id) as work_order_dtls_id, determination_id, color_id, fabric_construction,item_category_id, fabric_composition, gsm, dia_width, uom, booking_without_order, sum(quantity) as quantity, avg(rate) as rate, sum(amount) as amount 
					from com_pi_item_details 
					where pi_id='$pi_id' and quantity>0 and status_active=1 and is_deleted=0
					group by work_order_no, work_order_id, determination_id, color_id,item_category_id, fabric_construction, fabric_composition, gsm, dia_width, uom, booking_without_order" );
            ...
            ?>
                <tr class="general" id="row_1">
                    ...
                    ?>
                    <td>
                    	<?php echo create_drop_down( "itemCategoryId_1", 151, $item_category,'', 1, '--Select--',0,"",0,'2,3,13,14','','','74,72,79,73,71,77,78,75,76'); ?> 
                    </td>
                    <td> 
			...
			else
			{
				...
				?>
                    <tr class="general" id="row_<? echo $i; ?>">
                        ...
                        ?>
                        <td>
                    		<?php echo create_drop_down( "itemCategoryId_".$i, 151, $item_category,'', 1, '--Select--',$row[csf('item_category_id')],"",1,'2,3,13,14','','','74,72,79,73,71,77,78,75,76'); ?> 
                    	</td>
                        <td> 
            ...
            <table cellpadding="0" cellspacing="0" width="870" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
                    <td class="must_entry_caption">
						 <? echo create_drop_down( "cbo_importer_id", 120,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, '--Select--',$importer_id,"load_drop_down( 'pi_fabric_controller',this.value, 'load_supplier_dropdown', 'supplier_td' );",1); ?>       
                    </td>
                    <td id="supplier_td">	
                        <?
                        	if($importer_id > 0)
                        	{
                        		echo create_drop_down( "cbo_supplier_id", 151,"select DISTINCT(c.supplier_name),c.id from  lib_supplier_tag_company a,lib_supplier_party_type b, lib_supplier c where c.id=b.supplier_id and a.supplier_id = b.supplier_id and a.tag_company='$importer_id' and b.party_type =9 and c.status_active=1 and c.is_deleted=0 order by c.supplier_name",'id,supplier_name', 1, '-- Select Supplier --',0,'',0);
                        	}
                        	else{
                    ...
                    <td>	
						<? 
							echo create_drop_down( "cbo_item_category_id", 110, $item_category,'', 1, '--Select--',0,"",0,'2,3,13,14','','','74,72,79,73,71,77,78,75,76'); 
						?>
                    </td>               
                    <td> 
...
if($action=="create_pi_search_list_view")
{
	...
	$item_category_cond = ($item_category_id == 0)? "" :" and b.item_category_id = $item_category_id";
	$importer_id =$data[4];
	...
	//echo $sql= "select id,pi_number,pi_date,item_category_id,importer_id,supplier_id,last_shipment_date,hs_code,pi_basis_id from com_pi_master_details where supplier_id like '$supplier_id' and importer_id=$importer_id and item_category_id=$item_category_id and entry_form=1 and status_active=1 and is_deleted=0 $pi_number $pi_date $system_id_cond order by pi_number";


	$sql= "select distinct(a.id),a.pi_number,a.pi_date,a.importer_id,a.supplier_id,a.last_shipment_date,a.hs_code,a.pi_basis_id 
        from com_pi_master_details a left join  com_pi_item_details b on a.id = b.pi_id and b.item_category_id  in (2,3,13,14) and b.status_active=1 and b.is_deleted=0 $item_category_cond
        where a.supplier_id like '$supplier_id' and importer_id=$importer_id and a.entry_form=166 and a.version = 2 and a.status_active = 1 and a.is_deleted = 0 $pi_number $pi_date $system_id_cond
...
if ($action=="populate_data_from_search_popup")
{
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_drop_down('requires/pi_fabric_controller',".$row[csf("importer_id")].", 'load_supplier_dropdown', 'supplier_td' );\n";
            ...
            <table cellpadding="0" cellspacing="0" width="850" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
					...
					<?
                    	...
                    	echo create_drop_down( "cbo_importer_id", 101,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, 'Select',$importer_id,"load_drop_down( 'pi_fabric_controller',this.value+'_'+$item_category_id, 'load_supplier_dropdown', 'supplier_td' );",$dis_ana); 
                    ?>       
                    </td>
                    <td id="supplier_td">	
					<?
                    ...
                    <td>
                    	<? echo create_drop_down( "cbo_item_category_id", 101, $item_category,'', 1, '--Select--',0,"",0,'2,3,13,14','','','74,72,79,73,71,77,78,75,76');?>
                    </td>          
                    <td> 
                        <input type="text" name="txt_wo_no" id="txt_wo_no" class="text_boxes" style="width:90px">
                    </td>						
                    <td align="center">
					...
					?>
            		 ...
            		 <td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_wo_no').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_item_category_id').value+'_'+document.getElementById('cbo_importer_id').value+'_'+document.getElementById('cbo_supplier_id').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('cbo_based_on').value+'_'+document.getElementById('txt_goods_rcv_status').value+'_'+'<? echo $prev_wo_ids; ?>'+'_'+'<? echo $previous_wo_ids; ?>'+'_'+'<? echo $prev_deters; ?>'+'_'+'<? echo $prev_colors; ?>'+'_'+'<? echo $prev_constructs; ?>'+'_'+'<? echo $prev_compositions; ?>'+'_'+'<? echo $prev_gsms; ?>'+'_'+'<? echo $prev_widths; ?>'+'_'+'<? echo $prev_uoms; ?>', 'create_wo_search_list_view', 'search_div', 'pi_fabric_controller', 'setFilterGrid(\'tbl_list_search\',-1);reset_hide_field();')" style="width:100px;" />
                     </td>
                </tr>
                <tr>
...
if($action=="create_wo_search_list_view")
{
	...
	{
		echo "Please Select Item Category First";die;
	}
...
if( $action == 'populate_data_wo_form' ) 
{
	...
	if($goods_rcv_status==2 && $order_type==1)
	{
		...
		$prev_pi_qty_arr=array();
		$sql_prev="select b.work_order_no, b.determination_id, b.color_id, b.fabric_construction, b.fabric_composition, b.gsm, b.dia_width, b.uom, sum(b.quantity) as qty from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and b.item_category_id=$item_category_id and a.pi_basis_id=1 and a.goods_rcv_status=$goods_rcv_status and b.status_active=1 and b.is_deleted=0 and b.work_order_no in($wo_nos) group by b.work_order_no, b.determination_id, b.color_id, b.fabric_construction, b.fabric_composition, b.gsm, b.dia_width, b.uom";// and b.work_order_dtls_id in($wo_dtls_id)
		//echo $sql_prev;die;
		$prevData=sql_select($sql_prev);
		foreach($prevData as $rowP)
		{
		...
		foreach($data_array as $row)
		{
			...
			?>
				<tr class="general" id="row_<? echo $tblRow; ?>">
					...
					<td>
						<? echo create_drop_down( "itemCategoryId_".$tblRow, 151, $item_category,'', 1, '--Select--',$row[csf('item_category')],"",1,'2,3,13,14','','','74,72,79,73,71,77,78,75,76');?>
					</td>
					<td> 
	...
	else
	{
		...
		if($order_type==1)
		{
			$data_array=sql_select("select a.id, a.booking_no,a.item_category, d.color as fabric_color_id, d.gsm as gsm_weight, d.dia_width, c.order_qnty as fin_fab_qnty, c.order_amount as amount, d.detarmination_id as lib_yarn_count_deter_id, c.id as dtls_id, c.order_uom as uom, c.order_rate as rate, d.id as prod_id
			from wo_booking_mst a, inv_receive_master b, inv_transaction c, product_details_master d
		...
		{
			$data_array=sql_select("select a.id, a.booking_no,a.item_category, d.color as fabric_color_id, d.detarmination_id as lib_yarn_count_deter_id, d.gsm as gsm_weight, d.dia_width, c.order_amount as amount, c.order_qnty as fin_fab_qnty, c.id as dtls_id, c.order_uom as uom, c.order_rate as rate, d.id as prod_id 
			from wo_non_ord_samp_booking_mst a, inv_receive_master b, inv_transaction c, product_details_master d 
...
?> 
	...
	var item_category=<? echo $item_category_id; ?>;
	...
	<form name="searchwofrm"  id="searchwofrm">
		...
		<legend>Enter search words</legend>           
            <table cellpadding="0" cellspacing="0" width="700px" class="rpt_table">
                ...
                <tr class="general">
                    <td>
						 <? echo create_drop_down( "cbo_importer_id", 151,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, 'Select',0,"load_drop_down( 'pi_fabric_controller',this.value+'_'+$item_category, 'load_supplier_dropdown', 'supplier_td' );",0); ?>       
                    </td>
                    <td id="supplier_td">	
                        <?
							echo create_drop_down( "cbo_supplier_id", 151, $blank_array,"", 1, "-- Select Supplier --", 0, "" );
