	...
	<form name="searchpifrm"  id="searchpifrm">
		...
		<legend>Enter search words</legend>           
            <table cellpadding="0" cellspacing="0" border="1" rules="all" width="800" class="rpt_table">
                ...
                <tr class="general">
                    <td>
						<?php echo create_drop_down( "cbo_within_group", 151, $yes_no,"", 1, "-- Select --", 0, "load_drop_down( 'pi_general_controller',this.value, 'load_drop_down_importer', 'importer_td' );" ); ?>
                    </td>
                    <td id="importer_td"> 
			...
			if(str_replace("'", '',$cbo_pi_basis_id)==1 && str_replace("'", '',$cbo_goods_rcv_status)==1)
			{
				...
				foreach($hideTransData_arr as $hide_trans_val)
				{
					...
					{ 
						...
						{
							$color_id = return_id( str_replace("'","",$$colorName), $color_library, "lib_color", "id,color_name","172");  
       	...
       	<?
            ...
            ?>
                <tr class="general" id="row_1">
                    ...
                    <td> 
						 <?
                            echo create_drop_down( "itemgroupid_1", 130, $item_group_arr,'', 1, '-Select-',0,"get_php_form_data( this.value+'**'+'uom_1', 'get_uom', 'requires/pi_general_controller' );",1); 
                         ?>  
                    </td>

                    <td>
                    	<? echo create_drop_down( "itemCategoryId_1", 151, $item_category,'', 1, '--Select--',0,"",1,'','','','74,72,79,73,71,77,78,75,76,1,2,3,4,12,13,14,24,25,31'); ?>
                    </td>
                    <td>
			...
			else
			{
				...
				?>
                    <tr class="general" id="row_<? echo $i; ?>">
                        ...
                        <td> 
						 <?
                            echo create_drop_down( "itemgroupid_".$i, 130, $item_group_arr,'', 1, '-Select-',$row[csf('item_group')],"get_php_form_data( this.value+'**'+'uom_$i', 'get_uom', 'requires/pi_general_controller' );",1); 
                         ?>  
                    	</td>
                    	<td> 
						 <?php echo create_drop_down( "itemCategoryId_".$i, 151, $item_category,'', 1, '--Select--',$row[csf('item_category_id')],"",1,'','','','74,72,79,73,71,77,78,75,76,1,2,3,4,12,13,14,24,25,31'); ?>  
                    	</td>
                        <td>
            ...
            <table cellpadding="0" cellspacing="0" width="970" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
                    <td class="must_entry_caption">
						 <? echo create_drop_down( "cbo_importer_id", 151,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, '--Select--',$importer_id,"load_drop_down( 'pi_general_controller',this.value, 'load_supplier_dropdown', 'supplier_td' );",0); ?>       
                    </td>
                    <td id="supplier_td">	
                        <?
...
if($action=="create_pi_search_list_view")
{
	...
	$sql= "select distinct(a.id),a.pi_number,a.pi_date,a.importer_id,a.supplier_id,a.last_shipment_date,a.hs_code,a.pi_basis_id 
		...
		where a.supplier_id like '$supplier_id' and a.importer_id=$importer_id $pi_number $pi_date $system_id_cond and a.entry_form=172 and a.version = 2 and a.status_active = 1 and a.is_deleted = 0
...
if ($action=="populate_data_from_search_popup")
{
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_drop_down('requires/pi_general_controller',".$row[csf("importer_id")].", 'load_supplier_dropdown', 'supplier_td' );\n";
            ...
            <table cellpadding="0" cellspacing="0" width="980" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
					...
					<?
                    	...
                    	echo create_drop_down( "cbo_importer_id", 151,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, 'Select',$importer_id,"load_drop_down( 'pi_general_controller',this.value+'_'+$item_category_id, 'load_supplier_dropdown', 'supplier_td' );",$dis_ana); 
                    ?>       
                    </td>
                    <td id="supplier_td">	
					<?
					...
					?>
            		 ...
            		 <td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_wo_no').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_item_category_id').value+'_'+document.getElementById('cbo_importer_id').value+'_'+document.getElementById('cbo_supplier_id').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('cbo_based_on').value+'_'+document.getElementById('txt_goods_rcv_status').value+'_'+'<? echo $prev_wo_ids; ?>'+'_'+'<? echo $previous_wo_ids; ?>'+'_'+'<? echo $prev_deters; ?>'+'_'+'<? echo $prev_colors; ?>'+'_'+'<? echo $prev_constructs; ?>'+'_'+'<? echo $prev_compositions; ?>'+'_'+'<? echo $prev_gsms; ?>'+'_'+'<? echo $prev_widths; ?>'+'_'+'<? echo $prev_uoms; ?>', 'create_wo_search_list_view', 'search_div', 'pi_general_controller', 'setFilterGrid(\'tbl_list_search\',-1);reset_hide_field();')" style="width:100px;" />
                     </td>
                </tr>
                <tr>
...
if($action=="create_wo_search_list_view")
{
	...
	if($selected_based_on==1)
	{
		if($goods_rcv_status==2)
		{
			...
			if($db_type==0)
			{
				...
				where a.id=b.mst_id and b.item_id=c.id and a.company_name=$company_id and a.supplier_id like '$supplier_id' and a.pay_mode=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and b.item_category_id not in(74,72,79,73,71,77,78,75,76,1,2,3,12,13,14,24,25,31) and c.item_category_id not in(74,72,79,73,71,77,78,75,76,1,2,3,12,13,14,24,25,31) and c.entry_form = 20 $wo_number $wo_date_cond $prev_wo_ids_cond $item_cond
	...
	else
	{
		if($goods_rcv_status==2)
		{
			...
			from wo_non_order_info_mst a, wo_non_order_info_dtls b, product_details_master c 
			where a.id=b.mst_id and b.item_id=c.id and a.company_name=$company_id and a.supplier_id like '$supplier_id' and a.pay_mode=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.item_category_id not in(74,72,79,73,71,77,78,75,76,1,2,3,12,13,14,24,25,31) and c.item_category_id not in(74,72,79,73,71,77,78,75,76,1,2,3,12,13,14,24,25,31) $wo_number $wo_date_cond $prev_wo_ids_cond $item_cond"; 
		...
		else
		{
			...
			from wo_non_order_info_mst a, wo_non_order_info_dtls b, inv_receive_master c, inv_transaction d, product_details_master e 
			where a.id=b.mst_id and b.mst_id=c.booking_id and c.id=d.mst_id and d.prod_id=e.id and d.prod_id=b.item_id and c.entry_form=20 and e.entry_form=20 and a.company_name=$company_id and a.supplier_id like '$supplier_id' and a.pay_mode=1 and d.pi_is_lock!=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.item_category_id not in(74,72,79,73,71,77,78,75,76,1,2,3,12,13,14,24,25,31) and c.item_category_id not in(74,72,79,73,71,77,78,75,76,1,2,3,12,13,14,24,25,31) $wo_number $wo_date_cond $prev_wo_ids_cond3 $item_cond "; 
		}
	}
...
if( $action == 'populate_data_wo_form' ) 
{
	...
	$prev_pi_qty_arr=return_library_array( "select b.work_order_dtls_id,sum(b.quantity) as qty from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and b.item_category_id not in (74,72,79,73,71,77,78,75,76,1,2,3,12,13,14,24,25,31) and a.pi_basis_id=1 and a.goods_rcv_status=$goods_rcv_status and b.status_active=1 and b.is_deleted=0 and b.work_order_dtls_id in($wo_dtls_id) group by b.work_order_dtls_id", "work_order_dtls_id", "qty");  //,4
		...
		foreach($data_array as $row)
		{
			...
			if($bl_qty>0)
			{
				...
				<tr class="general" id="row_<? echo $tblRow; ?>">
					...
					<td> 
					<?
						echo create_drop_down( "itemgroupid_".$tblRow, 130, $item_group_arr,'', 1, '- Select -',$row[csf('item_group_id')],"get_php_form_data( this.value+'**'+'uom_$tblRow', 'get_uom', 'requires/pi_general_controller' );",1); 
					?>  
					</td>
					<td>
						<? echo create_drop_down( "itemCategoryId_".$tblRow, 151, $item_category,'', 1, '--Select--',$row[csf('item_category_id')],"",1,'','','','74,72,79,73,71,77,78,75,76,1,2,3,12,13,14,24,25,31'); ?>
					</td>
					<td>
			...
			foreach($book_data as $prod_id=>$row)
			{
				...
				if($bl_qty>0)
				{
					...
					<tr class="general" id="row_<? echo $tblRow; ?>">
						...
						<td> 
						<?
							echo create_drop_down( "itemgroupid_".$tblRow, 130, $item_group_arr,'', 1, '- Select -',$row[('item_group_id')],"get_php_form_data( this.value+'**'+'uom_$tblRow', 'get_uom', 'requires/pi_general_controller' );",1); 
						?>  
						</td>
						<td>
	...
	<form name="searchwofrm"  id="searchwofrm">
		...
		<legend>Enter search words</legend>           
            <table cellpadding="0" cellspacing="0" width="700px" class="rpt_table">
                ...
                <tr class="general">
                    <td>
						 <? echo create_drop_down( "cbo_importer_id", 151,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, 'Select',0,"load_drop_down( 'pi_general_controller',this.value, 'load_supplier_dropdown', 'supplier_td' );",0); ?>       
                    </td>
                    <td id="supplier_td">	
                        <?
							echo create_drop_down( "cbo_supplier_id", 151, $blank_array,"", 1, "-- Select Supplier --", 0, "" );
                    ...
                    <td> 
            		 ...
            		 <td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_item_desc').value+'_'+''+'_'+document.getElementById('cbo_importer_id').value+'_'+document.getElementById('cbo_supplier_id').value, 'create_item_search_list_view', 'search_div', 'pi_general_controller', 'setFilterGrid(\'tbl_list_search\',-1);reset_hide_field();')" style="width:100px;" />
                     </td>
                </tr>
           </table>
           <table width="100%" style="margin-top:5px">
...
if( $action == 'populate_data_item_form' ) 
{
	...
	foreach($data_array as $row)
	{
		...
		<tr class="general" id="row_<? echo $tblRow; ?>">
			<td> 
				<?
					echo create_drop_down( "itemgroupid_".$tblRow, 130, "SELECT id,item_name FROM lib_item_group WHERE  status_active = 1 AND is_deleted = 0 ORDER BY item_name ASC",'id,item_name', 1, '-Select-',$row[csf('item_group_id')],"get_php_form_data( this.value+'**'+'uom_$tblRow', 'get_uom', 'requires/pi_general_controller' );",1); 
