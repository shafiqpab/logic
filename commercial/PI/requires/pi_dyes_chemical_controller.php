...
{ 
	...
	{ 
		...
		if (is_duplicate_field( "pi_number", "com_pi_master_details", "pi_number=$pi_number and importer_id=$cbo_importer_id and supplier_id=$cbo_supplier_id  $year_cond and status_active=1 and is_deleted=0 and entry_form=227" ) == 1)
		{
			echo "11**0"; die;
		}
		else
	...
	else if ($operation==1)   // Update Here
	{
		...
		$pi_year =  date("Y",strtotime(str_replace("'","", $pi_date)));
		if($db_type==0) $year_cond=" and year(pi_date)=".trim($pi_year); else $year_cond=" and to_char(pi_date,'YYYY')=".trim($pi_year);
		if(is_duplicate_field("pi_number", "com_pi_master_details","pi_number=$pi_number and importer_id=$cbo_importer_id and supplier_id=$cbo_supplier_id $year_cond and id!=$update_id and status_active=1 and is_deleted=0 and entry_form=227")==1)
		{
			echo "11**0"; die;
		}
		else
	...
	<form name="searchpifrm"  id="searchpifrm">
		...
		<legend>Enter search words</legend>           
            <table cellpadding="0" cellspacing="0" border="1" rules="all" width="800" class="rpt_table">
                ...
                <tr class="general">
                    <td>
						<?php echo create_drop_down( "cbo_within_group", 151, $yes_no,"", 1, "-- Select --", 0, "load_drop_down( 'pi_dyes_chemical_controller',this.value, 'load_drop_down_importer', 'importer_td' );" ); ?>
                    </td>
                    <td id="importer_td"> 
			...
			if(str_replace("'", '',$cbo_pi_basis_id)==1 && str_replace("'", '',$cbo_goods_rcv_status)==1)
			{
				...
				foreach($hideTransData_arr as $hide_trans_val)
				{
					...
					{ 
						...
						{
							$color_id = return_id( str_replace("'","",$$colorName), $color_library, "lib_color", "id,color_name","407");  
	...
	{ 
		...
		for($i=1;$i<=$total_row;$i++)
		{
			...
			if(str_replace("'","",$$updateIdDtls)!="")
			{
				...
				$id_arr[]=str_replace("'",'',$$updateIdDtls);
				$data_array_update[str_replace("'",'',$$updateIdDtls)] = explode("*",($update_id."*'".str_replace("'","",$$workOrderNo)."'*'".str_replace("'","",$$workOrderId)."'*'".str_replace("'","",$$workOrderDtlsId)."'*'".str_replace("'","",$$bookingWithoutOrder)."'*'".str_replace("'","",$$determinationId)."'*'".str_replace("'","",$$itemProdId)."'*'".str_replace("'","",$$itemgroupid)."'*'".str_replace("'","",$$itemdescription)."'*'".str_replace("'","",$color_id)."'*'".str_replace("'","",$size_id)."'*'".str_replace("'","",$$itemSize)."'*'".str_replace("'","",$$countName)."'*'".str_replace("'","",$$yarnCompositionItem1)."'*'".str_replace("'","",$$yarnCompositionPercentage1)."'*'".str_replace("'","",$$yarnCompositionItem2)."'*'".str_replace("'","",$$yarnCompositionPercentage2)."'*'".str_replace("'","",$$composition)."'*'".str_replace("'","",$$construction)."'*'".str_replace("'","",$$yarnType)."'*'".str_replace("'","",$$gsm)."'*'".str_replace("'","",$$diawidth)."'*'".str_replace("'","",$$weight)."'*'".str_replace("'","",$item_color_id)."'*'".str_replace("'","",$$uom)."'*'".str_replace("'","",$$quantity)."'*'".str_replace("'","",$$rate)."'*".$$amount."*'".$net_pi_rate."'*'".$net_pi_amount."'*'".str_replace("'","",$$servicetype)."'*'".str_replace("'","",$$brandSupRef)."'*'".str_replace("'","",$$gmtsitem)."'*'".str_replace("'","",$$embellname)."'*'".str_replace("'","",$$embelltype)."'*'".str_replace("'","",$$lot)."'*'".str_replace("'","",$$yarnColor)."'*'".str_replace("'","",$$colorRange)."'*'".str_replace("'","",$$cboTestFor)."'*'".str_replace("'","",$$testItemId)."'*'".str_replace("'","",$$remarks)."'*'".str_replace("'","",$$itemCategoryId)."'*".$user_id."*'".$pc_date_time."'"));
			}
			else
			{
				if($data_array!="") $data_array.=","; 		
		...
		<?
		 ...
		 ?>
       	...
       	<?
            ...
            {
				$nameArray=sql_select( "select id, work_order_no, work_order_id, work_order_dtls_id, item_prod_id, item_group,item_category_id, item_description, item_size, uom, quantity, rate, amount from com_pi_item_details where pi_id='$pi_id' and status_active=1 and is_deleted=0" );
            }
            else
            {
            ...
            ?>
                <tr class="general" id="row_1">
                    ...
                    <td> 
						 <?
                            echo create_drop_down( "itemgroupid_1", 130, $item_group_arr,'', 1, '-Select-',0,"get_php_form_data( this.value+'**'+'uom_1', 'get_uom', 'requires/pi_dyes_chemical_controller' );",1); 
                         ?>  
                    </td>

                    <td>
                    	<? echo create_drop_down( "itemCategoryId_1", 151, $item_category,'', 1, '--Select--',0,"",1,'5,6,7,23','','',''); ?>
                    </td>
                    <td>
			...
			else
			{
				...
				?>
                    <tr class="general" id="row_<? echo $i; ?>">
                        ...
                        <td> 
						 <?
                            echo create_drop_down( "itemgroupid_".$i, 130, $item_group_arr,'', 1, '-Select-',$row[csf('item_group')],"get_php_form_data( this.value+'**'+'uom_$i', 'get_uom', 'requires/pi_dyes_chemical_controller' );",1); 
                         ?>  
                    	</td>
                    	<td> 
						 <?php echo create_drop_down( "itemCategoryId_".$i, 151, $item_category,'', 1, '--Select--',$row[csf('item_category_id')],"",1,'','5,6,7,23','',''); ?>  
                    	</td>
                        <td>
            ...
            <table cellpadding="0" cellspacing="0" width="970" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
                    <td class="must_entry_caption">
						 <? echo create_drop_down( "cbo_importer_id", 151,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, '--Select--',$importer_id,"load_drop_down( 'pi_dyes_chemical_controller',this.value, 'load_supplier_dropdown', 'supplier_td' );",0); ?>       
                    </td>
                    <td id="supplier_td">	
                        <?
                    ...
                    <td>
                    	<? echo create_drop_down( "cbo_item_category_id", 151, $item_category,'', 1, '--Select--',0,"",0,'5,6,7,23','','',''); ?>
                    </td>                 
                    <td> 
...
if($action=="create_pi_search_list_view")
{
	...
	//echo $sql= "select a.id,a.pi_number,a.pi_date,a.importer_id,a.supplier_id,a.last_shipment_date,a.hs_code,a.pi_basis_id from com_pi_master_details a, com_pi_item_details b where a.id = b.pi_id and  a.supplier_id like '$supplier_id' and importer_id=$importer_id $item_cond and b.item_category_id not in (74,72,79,73,71,77,78,75,76,1,2,3,4,12,13,14,24,25,31) and a.entry_form=227 and b.status_active=1 and b.is_deleted=0 and a.status_active = 1 and a.is_deleted = 0 $pi_number $pi_date $system_id_cond order by a.pi_number";

	$sql= "select distinct(a.id),a.pi_number,a.pi_date,a.importer_id,a.supplier_id,a.last_shipment_date,a.hs_code,a.pi_basis_id 
		from com_pi_master_details a left join  com_pi_item_details b on a.id = b.pi_id $item_cond and b.item_category_id in (5,6,7,23) and b.status_active=1 and b.is_deleted=0
		where a.supplier_id like '$supplier_id' and a.importer_id=$importer_id $pi_number $pi_date $system_id_cond and a.entry_form=227 and a.version = 2 and a.status_active = 1 and a.is_deleted = 0
...
if ($action=="populate_data_from_search_popup")
{
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_drop_down('requires/pi_dyes_chemical_controller',".$row[csf("importer_id")].", 'load_supplier_dropdown', 'supplier_td' );\n";
            ...
            <table cellpadding="0" cellspacing="0" width="980" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
					...
					<?
                    	...
                    	echo create_drop_down( "cbo_importer_id", 151,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, 'Select',$importer_id,"load_drop_down( 'pi_dyes_chemical_controller',this.value+'_'+$item_category_id, 'load_supplier_dropdown', 'supplier_td' );",$dis_ana); 
                    ?>       
                    </td>
                    <td id="supplier_td">	
					<?
					...
					?>
            		 ...
            		 <td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_wo_no').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_item_category_id').value+'_'+document.getElementById('cbo_importer_id').value+'_'+document.getElementById('cbo_supplier_id').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('cbo_based_on').value+'_'+document.getElementById('txt_goods_rcv_status').value+'_'+'<? echo $prev_wo_ids; ?>'+'_'+'<? echo $previous_wo_ids; ?>'+'_'+'<? echo $prev_deters; ?>'+'_'+'<? echo $prev_colors; ?>'+'_'+'<? echo $prev_constructs; ?>'+'_'+'<? echo $prev_compositions; ?>'+'_'+'<? echo $prev_gsms; ?>'+'_'+'<? echo $prev_widths; ?>'+'_'+'<? echo $prev_uoms; ?>', 'create_wo_search_list_view', 'search_div', 'pi_dyes_chemical_controller', 'setFilterGrid(\'tbl_list_search\',-1);reset_hide_field();')" style="width:100px;" />
                     </td>
                </tr>
                <tr>
...
if($action=="create_wo_search_list_view")
{
	...
	{
		$item_cond = " and b.item_category_id in (4,11)" ;
	}
	*/
	if($selected_based_on==1)
	{
		if($goods_rcv_status==2)
		{
			...
			$prev_pi_qnty_arr=return_library_array("select b.work_order_id, sum(b.quantity) as quantity from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and b.item_category_id in (5,6,7,23) and a.goods_rcv_status<>1 and b.work_order_dtls_id>0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.work_order_id",'work_order_id','quantity');
		...
		else
		{
			...
			if($db_type==0)
			{
				...
				where a.id=b.mst_id and b.mst_id=c.booking_id and c.id=d.mst_id and d.prod_id=e.id and d.prod_id=b.item_id and a.entry_form = 145  and a.company_name=$company_id and a.supplier_id like '$supplier_id' and a.pay_mode=1 and d.pi_is_lock!=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.item_category_id in(5,6,7,23) and e.item_category_id in(5,6,7,23) $wo_number $wo_date_cond $prev_wo_ids_cond3 $item_cond
				group by a.id, a.wo_number_prefix_num, a.wo_number, a.wo_date, a.item_category, a.supplier_id";
			}
			else
			{
	...
	else
	{
		if($goods_rcv_status==2)
		{
			...
			$sql="select a.id as mst_id, a.wo_number_prefix_num, a.wo_number, a.wo_date, c.item_category_id as item_category, a.supplier_id, b.id as dtls_id, b.item_id, b.uom, b.supplier_order_quantity, c.item_group_id, c.item_description, c.item_size, b.supplier_order_quantity as wo_qnty  
			from wo_non_order_info_mst a, wo_non_order_info_dtls b, product_details_master c 
			where a.id=b.mst_id and b.item_id=c.id and a.company_name=$company_id and a.supplier_id like '$supplier_id' and a.pay_mode=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.item_category_id in(5,6,7,23) and c.item_category_id in(5,6,7,23) $wo_number $wo_date_cond $prev_wo_ids_cond $item_cond"; 
			
			$prev_pi_qnty_arr=return_library_array("select b.work_order_dtls_id, sum(b.quantity) as quantity from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id $item_cond and b.item_category_id in (5,6,7,23) and a.goods_rcv_status<>1 and b.work_order_dtls_id>0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.work_order_dtls_id",'work_order_dtls_id','quantity');
			
		}
		else
		{
			$sql="select a.id as mst_id, a.wo_number_prefix_num, a.wo_number, a.wo_date, e.item_category_id as item_category, a.supplier_id, d.id as dtls_id, b.item_id, b.uom, d.cons_quantity as supplier_order_quantity, e.item_group_id, e.item_description, e.item_size
			from wo_non_order_info_mst a, wo_non_order_info_dtls b, inv_receive_master c, inv_transaction d, product_details_master e 
			where a.id=b.mst_id and b.mst_id=c.booking_id and c.id=d.mst_id and d.prod_id=e.id and d.prod_id=b.item_id and a.entry_form = 145 and a.company_name=$company_id and a.supplier_id like '$supplier_id' and a.pay_mode=1 and d.pi_is_lock!=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.item_category_id  in(5,6,7,23) and c.item_category_id  in(5,6,7,23) $wo_number $wo_date_cond $prev_wo_ids_cond3 $item_cond "; 
		}
	}
...
if( $action == 'populate_data_wo_form' ) 
{
	...
	$prev_pi_qty_arr=return_library_array( "select b.work_order_dtls_id,sum(b.quantity) as qty from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and b.item_category_id  in (5,6,7,23) and a.pi_basis_id=1 and a.goods_rcv_status=$goods_rcv_status and b.status_active=1 and b.is_deleted=0 and b.work_order_dtls_id in($wo_dtls_id) group by b.work_order_dtls_id", "work_order_dtls_id", "qty");  //,4
	...
	if($goods_rcv_status==2)
	{
		$sql_data="select a.id, a.wo_number, a.wo_date, a.supplier_id, b.id as dtls_id, b.item_id, b.uom, b.supplier_order_quantity, b.rate, b.amount, b.item_category_id, c.item_group_id, c.item_description, c.item_size, 0 as trans_id 
		from wo_non_order_info_mst a, wo_non_order_info_dtls b, product_details_master c 
		...
		foreach($data_array as $row)
		{
			...
			if($bl_qty>0)
			{
				...
				<tr class="general" id="row_<? echo $tblRow; ?>">
					...
					<td> 
					<?
						echo create_drop_down( "itemgroupid_".$tblRow, 130, $item_group_arr,'', 1, '- Select -',$row[csf('item_group_id')],"get_php_form_data( this.value+'**'+'uom_$tblRow', 'get_uom', 'requires/pi_dyes_chemical_controller' );",1); 
					?>  
					</td>
					<td>
						<? echo create_drop_down( "itemCategoryId_".$tblRow, 151, $item_category,'', 1, '--Select--',$row[csf('item_category_id')],"",1,'5,6,7,23','','',''); ?>
					</td>
					<td>
	...
	else
	{
		...
		$rcv_rtn_qnty=return_library_array("select recv_trans_id, sum(issue_qnty) as qnty from  inv_mrr_wise_issue_details where entry_form=28 and status_active=1 group by recv_trans_id","recv_trans_id","qnty");
		...
		where a.id=b.booking_id and b.id=c.mst_id and c.prod_id=d.id and a.entry_form = 145 and b.receive_basis=2 and a.is_deleted=0 and b.is_deleted=0 and b.is_deleted=0 and c.is_deleted=0 and c.id in($wo_dtls_id)";
		$data_array=sql_select($sql_data);
			...
			foreach($book_data as $prod_id=>$row)
			{
				...
				if($bl_qty>0)
				{
					...
					<tr class="general" id="row_<? echo $tblRow; ?>">
						...
						<td> 
						<?
							echo create_drop_down( "itemgroupid_".$tblRow, 130, $item_group_arr,'', 1, '- Select -',$row[('item_group_id')],"get_php_form_data( this.value+'**'+'uom_$tblRow', 'get_uom', 'requires/pi_dyes_chemical_controller' );",1); 
						?>  
						</td>
						<td>
						<? echo create_drop_down( "itemCategoryId_".$tblRow, 151, $item_category,'', 1, '--Select--',$row[csf('item_category_id')],"",1,'5,6,7,23','','',''); ?>
	...
	<form name="searchwofrm"  id="searchwofrm">
		...
		<legend>Enter search words</legend>           
            <table cellpadding="0" cellspacing="0" width="700px" class="rpt_table">
                ...
                <tr class="general">
                    <td>
						 <? echo create_drop_down( "cbo_importer_id", 151,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, 'Select',0,"",0); ?>       
                    </td>
                    <td>
                    	<? echo create_drop_down( "cbo_item_category_id", 151, $item_category,'', 1, '--Select--',0,"",0,'5,6,7,23','','',''); ?>
                    </td> 
                    <td> 
            		 ...
            		 <td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_item_desc').value+'_'+''+'_'+document.getElementById('cbo_importer_id').value+'_'+document.getElementById('cbo_item_category_id').value, 'create_item_search_list_view', 'search_div', 'pi_dyes_chemical_controller', 'setFilterGrid(\'tbl_list_search\',-1);reset_hide_field();')" style="width:100px;" />
                     </td>
                </tr>
           </table>
           <table width="100%" style="margin-top:5px">
...
if($action=="create_item_search_list_view")
{
	...
	if($company_id==0) { echo "Please Select Company First."; die; }
	if($data[3]==0) $item_category_id="%%"; else $item_category_id =$data[3];
	
	$supplier_arr=return_library_array( "select id, supplier_name from lib_supplier",'id','supplier_name');

	$sql = "select id, item_category_id, supplier_id, item_group_id, item_description, item_size, unit_of_measure from product_details_master where company_id=$company_id and item_category_id in (5,6,7,23) and item_category_id like '$item_category_id' and item_description like '%".trim($data[0])."%' and status_active=1 and is_deleted=0";
...
if( $action == 'populate_data_item_form' ) 
{
	...
	foreach($data_array as $row)
	{
		...
		<tr class="general" id="row_<? echo $tblRow; ?>">
			<td> 
				<?
					echo create_drop_down( "itemgroupid_".$tblRow, 130, "SELECT id,item_name FROM lib_item_group WHERE  status_active = 1 AND is_deleted = 0 ORDER BY item_name ASC",'id,item_name', 1, '-Select-',$row[csf('item_group_id')],"get_php_form_data( this.value+'**'+'uom_$tblRow', 'get_uom', 'requires/pi_dyes_chemical_controller' );",1); 
