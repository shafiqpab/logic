	...
	<form name="searchpifrm"  id="searchpifrm">
		...
		<legend>Enter search words</legend>           
            <table cellpadding="0" cellspacing="0" border="1" rules="all" width="800" class="rpt_table">
                ...
                <tr class="general">
                    <td>
						<?php echo create_drop_down( "cbo_within_group", 151, $yes_no,"", 1, "-- Select --", 0, "load_drop_down( 'pi_service_labtest_controller',this.value, 'load_drop_down_importer', 'importer_td' );" ); ?>
                    </td>
                    <td id="importer_td"> 
...
{ 
	...
	if ($operation==0)  // Insert Here
	{
		...
		if(str_replace("'", '',$cbo_goods_rcv_status)==1)
		{
			...
			$prev_pi_amnt_arr=return_library_array( "select b.work_order_dtls_id,sum(b.amount) as amnt from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and b.item_category_id=$cbo_item_category_id and a.pi_basis_id=1 and b.status_active=1 and b.is_deleted=0 and b.work_order_dtls_id in($all_dtls_id) group by b.work_order_dtls_id", "work_order_dtls_id", "amnt");
		...
		for($i=1;$i<=$total_row;$i++)
		{
			...
			{ 
				...
				{
					$color_id = return_id( str_replace("'","",$$colorName), $color_library, "lib_color", "id,color_name","171");  
	...
	{ 
		...
		for($i=1;$i<=$total_row;$i++)
		{
			...
			if(str_replace("'", '',$cbo_goods_rcv_status)==2 && str_replace("'", '',$cbo_pi_basis_id)==1)
			{
				...
				{
					echo "11**PI Amount Not Allow Over Balance Amount=".$trans_amount."=".$bal_wo_amt."=".$wo_amnt_arr[str_replace("'","",$$workOrderDtlsId)]."=".$prev_pi_amnt_arr[str_replace("'","",$$workOrderDtlsId)]."==".$all_dtls_id;die;
				}
				*/


				$bal_wo_amt=$wo_amnt_arr[str_replace("'","",$$workOrderDtlsId)];
				$bal_wo_amt=number_format($bal_wo_amt,2,'.','');
				$trans_amount=str_replace("'","",$$amount);

				if($trans_amount>$bal_wo_amt)
				{
					echo "11**PI Amount Not Allow Over Balance Amount=".$trans_amount."=".$bal_wo_amt;die;
				}
            ...
            <table cellpadding="0" cellspacing="0" width="870" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
                    <td class="must_entry_caption">
						 <? echo create_drop_down( "cbo_importer_id", 151,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, '--Select--',$importer_id,"load_drop_down( 'pi_service_labtest_controller',this.value+'_'+$item_category, 'load_supplier_dropdown', 'supplier_td' );",0); ?>       
                    </td>
                    <td id="supplier_td">	
                        <?
...
if($action=="create_pi_search_list_view")
{
	...
	$sql= "select distinct(a.id),a.pi_number,a.pi_date,a.importer_id,a.supplier_id,a.last_shipment_date,a.hs_code,a.pi_basis_id 
		...
		where a.supplier_id like '$supplier_id' and a.importer_id=$importer_id $pi_number $pi_date $system_id_cond and a.entry_form=171 and a.version = 2 and a.status_active = 1 and a.is_deleted = 0
...
if ($action=="populate_data_from_search_popup")
{
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_drop_down('requires/pi_service_labtest_controller',".$row[csf("importer_id")]."+'_'+".$row[csf("item_category_id")].", 'load_supplier_dropdown', 'supplier_td' );\n";
...
?> 
	...
	<script>
		...
		/*$(document).ready(function(e) {
            load_drop_down( 'pi_service_labtest_controller',<?echo $importer_id; ?>+'_'+item_category, 'load_supplier_dropdown', 'supplier_td' );
			$('#cbo_supplier_id').val( <?echo $supplier_id; ?> );
        });*/
            ...
            <table cellpadding="0" cellspacing="0" width="850" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
					...
					?>
            		 ...
            		 <td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_wo_no').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_item_category').value+'_'+document.getElementById('cbo_importer_id').value+'_'+document.getElementById('cbo_supplier_id').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('cbo_based_on').value+'_'+document.getElementById('txt_goods_rcv_status').value+'_'+'<? echo $prev_wo_ids; ?>'+'_'+'<? echo $previous_wo_ids; ?>'+'_'+'<? echo $prev_deters; ?>'+'_'+'<? echo $prev_colors; ?>'+'_'+'<? echo $prev_constructs; ?>'+'_'+'<? echo $prev_compositions; ?>'+'_'+'<? echo $prev_gsms; ?>'+'_'+'<? echo $prev_widths; ?>'+'_'+'<? echo $prev_uoms; ?>', 'create_wo_search_list_view', 'search_div', 'pi_service_labtest_controller', 'setFilterGrid(\'tbl_list_search\',-1);reset_hide_field();')" style="width:100px;" />
                     </td>
                </tr>
                <tr>
			...
			foreach($data_array as $row)
			{
				...
				if($bl_qty>0)
				{
					...
					<tr class="general" id="row_<? echo $tblRow; ?>">
						...
						<td> 
							<?
								//echo create_drop_down( "itemgroupid_".$tblRow, 110, $item_group_arr,'', 1, '-Select-',$row[csf('trim_group')],"get_php_form_data( this.value+'**'+'uom_$tblRow', 'get_uom', 'requires/pi_service_labtest_controller' );",1); 
							?>
				...
				foreach($row_val as $pord_id=>$row)
				{
					...
					if($bl_qty>0)
					{
						...
						<tr class="general" id="row_<? echo $tblRow; ?>">
							...
							<td> 
								<?
									echo create_drop_down( "itemgroupid_".$tblRow, 110, $item_group_arr,'', 1, '-Select-',$row[('trim_group')],"get_php_form_data( this.value+'**'+'uom_$tblRow', 'get_uom', 'requires/pi_service_labtest_controller' );",1); 
								?>  
							</td>
							<td>
		...
		foreach($data_array as $row)
		{
			...
			?>
                <tr class="general" id="row_<? echo $tblRow; ?>">
                    ...
                    <td>
                        <? echo create_drop_down( "embellname_".$tblRow, 130, $emblishment_name_array,'', 1, '-Select-', $row[csf('emb_name')],"load_drop_down( 'requires/pi_service_labtest_controller', this.value+'**'+1+'**'+'embelltype_$tblRow', 'load_drop_down_embelltype', 'embelltypeTd_$tblRow');",1); ?>
                    </td>
			...
			foreach($data_array as $row)
			{
				...
				if($bl_qty>0)
				{
					...
					<tr class="general" id="row_<? echo $tblRow; ?>">
						...
						<td> 
						<?
							echo create_drop_down( "itemgroupid_".$tblRow, 130, $item_group_arr,'', 1, '- Select -',$row[csf('item_group_id')],"get_php_form_data( this.value+'**'+'uom_$tblRow', 'get_uom', 'requires/pi_service_labtest_controller' );",1); 
						?>  
						</td>
						<td>
				...
				foreach($book_data as $prod_id=>$row)
				{
					...
					if($bl_qty>0)
					{
						...
						<tr class="general" id="row_<? echo $tblRow; ?>">
							...
							<td> 
							<?
								echo create_drop_down( "itemgroupid_".$tblRow, 130, $item_group_arr,'', 1, '- Select -',$row[('item_group_id')],"get_php_form_data( this.value+'**'+'uom_$tblRow', 'get_uom', 'requires/pi_service_labtest_controller' );",1); 
							?>  
							</td>
							<td>
	...
	<form name="searchwofrm"  id="searchwofrm">
		...
		<legend>Enter search words</legend>           
            <table cellpadding="0" cellspacing="0" width="700px" class="rpt_table">
                ...
                <tr class="general">
                    <td>
						 <? echo create_drop_down( "cbo_importer_id", 151,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, 'Select',0,"load_drop_down( 'pi_service_labtest_controller',this.value+'_'+$item_category, 'load_supplier_dropdown', 'supplier_td' );",0); ?>       
                    </td>
                    <td id="supplier_td">	
                        <?
							echo create_drop_down( "cbo_supplier_id", 151, $blank_array,"", 1, "-- Select Supplier --", 0, "" );
                    ...
                    <td> 
            		 ...
            		 <td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_item_desc').value+'_'+document.getElementById('txt_item_category').value+'_'+document.getElementById('cbo_importer_id').value+'_'+document.getElementById('cbo_supplier_id').value, 'create_item_search_list_view', 'search_div', 'pi_service_labtest_controller', 'setFilterGrid(\'tbl_list_search\',-1);reset_hide_field();')" style="width:100px;" />
                     </td>
                </tr>
           </table>
           <table width="100%" style="margin-top:5px">
