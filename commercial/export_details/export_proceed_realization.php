...
function openmypage_realizationUpdate()
{
	...
	emailwindow.onclose=function()
	{
		...
		if(trim(realization_id)!="")
		{
			...
			show_list_view( realization_id+"**1**"+submit_type+"**"+variable_distribution, 'details_list_view', 'distribution_list_view', 'requires/export_proceed_realization_controller', '' ) ;	
...
function openmypage_InvoiceBill()
{
	...
	var page_link='requires/export_proceed_realization_controller.php?action=invoice_bill_popup_search&beneficiary_name='+beneficiary_name+'&buyerID='+buyerID+'&import_btb='+import_btb;
	var title='Export Proceeds Realization Entry Form';
	...
	emailwindow.onclose=function()
	{
		...
		if(trim(invoice_bill_id)!="")
		{
			...
			$("#lc_id").val(lc_id);
			$("#lc_type").val(lc_type);
			get_php_form_data(invoice_bill_id+"**"+is_invoiceBill+"****"+lc_id+"**"+lc_type, "populate_data_from_invoice_bill", "requires/export_proceed_realization_controller" );
...
function fnc_invoiceDetails()
{
	...
	if (invoice_bill_id)
	{
		var page_link='requires/export_proceed_realization_controller.php?action=invoice_details_popup&invoice_bill_id='+invoice_bill_id+'&beneficiary_name='+beneficiary_name+'&buyerID='+buyerID;
...
{ 
	...
	else
	{
		...
		{
			if(number_format(imv_amt,4,'','')!=number_format(grand_document_amt,4,'',''))
			{
				alert("Realized Amount Must Be Equal Bill Amount");return;
			}
		}
...
function fnc_export_proceed_realization_Reply_info()
{
	if(http.readyState == 4) 
	{
		...
		else if((reponse[0]==0 || reponse[0]==1))
		{
			...
			show_list_view( reponse[1]+"**1**"+submit_type+"**"+variable_distribution, 'details_list_view', 'distribution_list_view', 'requires/export_proceed_realization_controller', '' ) ;	
...
function calculate(i,field_id,table_id,prefix)
{
	...
	if(field_id=="DocumentCurrency_")
	{
		if(ConversionRate!="" && DomesticCurrency!="")
		{
			...
			$('#'+prefix+'DomesticCurrency_'+i).val(DomsCurr.toFixed(4));
		}
...
function calculate_total(table_id,prefix)
{
	...
	$("#"+table_id).find('tbody tr').each(function()
	{
		...
		$('#total_'+prefix+'_document_currency').val(total_document_currency.toFixed(4));
		$('#total_'+prefix+'_domestic_currency').val(total_domestic_currency.toFixed(4));
	});
...
function calculate_grand_total()
{
	...
	var bill_ballance=(($('#txt_bill_invoice_amnt').val()*1)-grand_total_document_currency.toFixed(4));
	
	$('#grand_total_document_currency').val(grand_total_document_currency.toFixed(4));
	$('#grand_total_domestic_currency').val(grand_total_domestic_currency.toFixed(4));
...
{ 
	...
	if(prefix=="distribution")
	{
		//$('#cbodistributionHead_'+row_num).removeAttr("onChange").attr("onChange","get_php_form_data(this.value+'**"+row_num+"**'+$('#cbo_beneficiary_name').val()+'**'+$('#is_invoice_bill_lien_bank').val(), 'populate_acc_loan_no_data','requires/export_proceed_realization_controller');check_duplication("+row_num+");fn_loan_type_change(this.value,"+row_num+");");
		//onFocus="add_auto_complete( 1 )" onDblClick="fn_commercial_head_display(1,2,'cbodistributionHead')"
		$('#cbodistributionHead_'+row_num).removeAttr("onBlur").attr("onBlur","setTimeout(fn_value_check,1000,"+row_num+",2,'cbodistributionHead');clearTimeout(1500);").removeAttr("onFocus").attr("onFocus","add_auto_complete("+row_num+")").removeAttr("onDblClick").attr("onDblClick","fn_commercial_head_display("+row_num+",2,'cbodistributionHead')");
		...
		$('#acLoanNo_'+row_num).removeAttr("disabled").removeAttr("readonly").removeAttr("placeholder").removeAttr("onDblClick");
	...
	{
		$('#cbodeductionHead_'+row_num).removeAttr("onBlur").attr("onBlur","setTimeout(fn_value_check,1000,"+row_num+",1,'cbodeductionHead');clearTimeout(1500);").removeAttr("onFocus").attr("onFocus","add_auto_complete("+row_num+")").removeAttr("onDblClick").attr("onDblClick","fn_commercial_head_display("+row_num+",1,'cbodeductionHead')");
...
function check_duplication(rowNo)
{
	...
	else
	{
		...
		$("#tbl_distribution").find('tbody tr').each(function()
		{
			var distributionHead=$(this).find('input[name="cbodistributionHead[]"]').attr('acHeadVal');
			var acLoanNo=$(this).find('input[name="acLoanNo[]"]').val();
			if(distributionHead!=1 && acLoanNo!='')
			{
				var uniqueAccNo = 'Account Head: ' + distributionHead + ', Account No: ' + acLoanNo;
				if (uniqueAcc.indexOf(uniqueAccNo) != -1) {
					$("#cbodistributionHead_"+rowNo).val("").attr("acHeadVal","");
					$("#acLoanNo_"+rowNo).val('');
					$("#distributionDocumentCurrency_"+rowNo).val('');
					$("#distributionConversionRate_"+rowNo).val('');
					$("#distributionDomesticCurrency_"+rowNo).val('');
					alert("Duplicate Account Head For AC / Loan No "+acLoanNo);
					return;
				}
				uniqueAcc.push(uniqueAccNo);
			}
		});	
	}
...
function open_loan_number_popup (element_id) {
	...
	var page_link='requires/export_proceed_realization_controller.php?action=loan_number_popup&beneficiary_name='+beneficiary+'&loan_type='+loan_type+'&pre_export_dtls_id='+pre_export_dtls_id+'&lc_id='+lc_id+'&lc_type='+lc_type;
...
function fn_value_check(seq_no,source_from,fld_names)
{
	if (form_validation('cbo_beneficiary_name','Company')==false )
	{
	...
	fld_value=$('#'+fld_names+'_'+seq_no).val().toUpperCase();
	...
	var lc_id=$("#lc_id").val();
	var is_invoice_bill=$("#is_invoice_bill").val();
	if(source_from==2)
	{
		get_php_form_data( commercial_head_reverse[fld_value]+'**'+seq_no+'**'+$('#cbo_beneficiary_name').val()+'**'+$('#is_invoice_bill_lien_bank').val()+'**'+lc_id+'**'+is_invoice_bill+'**'+commercial_head_reverse[fld_value], 'populate_acc_loan_no_data', 'requires/export_proceed_realization_controller' );
...
{
	if (form_validation('cbo_beneficiary_name*txt_invoice_bill_no','Beneficiary*Bill No')==false )
	{
		return;
	}
	var is_invoice_bill=$("#is_invoice_bill").val();
	$('#'+fld_name+"_"+seq_no).removeAttr('onblur');
	var page_link='requires/export_proceed_realization_controller.php?action=commercial_head_popup&head_from='+head_from+'&is_invoice_bill='+is_invoice_bill;
	...
	emailwindow.onclose=function()
	{
		...
		var lc_id=$("#lc_id").val();
		var is_invoice_bill=$("#is_invoice_bill").val();
		alert(head_from+"="+hdn_head_id+"="+seq_no);
		if(head_from==2)
		{
			get_php_form_data( hdn_head_id+'**'+seq_no+'**'+$('#cbo_beneficiary_name').val()+'**'+$('#is_invoice_bill_lien_bank').val()+'**'+lc_id+'**'+is_invoice_bill+'**'+hdn_head_id, 'populate_acc_loan_no_data', 'requires/export_proceed_realization_controller' );
			check_duplication(seq_no);
			fn_loan_type_change(hdn_head_id,seq_no);
		}
		$('#'+fld_name+"_"+seq_no).val(hdn_head_val).attr("onblur","setTimeout(fn_value_check,1000,"+seq_no+","+head_from+","+"'"+fld_name+"');clearTimeout(1500);");
	}
}
...
function fn_set_variable(com_id,bank_id)
{
	...
	var variable_distribution = trim(return_global_ajax_value(com_id+"**"+bank_id, 'populate_data_form_lib', '', 'requires/export_proceed_realization_controller'));
	...
	//get_php_form_data( this.value, 'set_variable', 'requires/export_proceed_realization_controller' );
}
...
function fn_distribution_generate()
{
	...
	var cbo_currency_name=$("#cbo_currency_name").val()*1;
	var com_id=$("#cbo_beneficiary_name").val()*1;
	//alert(tot_row);return;
	
	//var net_bill_amount=(bill_amount-(deduction_total+distribution_total) );
	var net_bill_amount=(bill_amount-deduction_total )*1;
	//var bill_amount=$("#txt_bill_invoice_amnt").val();
	//alert(net_bill_amount);
	if(net_bill_amount>0)
	{
		var prev_data=trim($("#cbodistributionHead_1").val());
		if(prev_data!="") tot_row=tot_row; else tot_row=0;
		var variable_distribution = return_global_ajax_value(variable_distribution+"**"+net_bill_amount+"**"+tot_row+"**"+cbo_currency_name+"**"+com_id, 'populate_data_form_lib_dis', '', 'requires/export_proceed_realization_controller');
		//alert(variable_distribution);return;
		...
		$("#distribution_list_view").html(variable_distribution);
		var row_num=$('#tbl_distribution tbody tr').length;
		$("#distribution_tot_row").val(row_num);
		calculate_total('tbl_distribution','distribution');
		calculate_grand_total();
	}
}
...
function fn_print_report(action_type)
{
	...
	else
	{
		if(action_type==1)
		{
			var action="realization_report_print";
		}
		if(action_type==2)
		{
			var action="realization_report_print2";
		}
		print_report( $('#update_id').val(), action, "requires/export_proceed_realization_controller" ) ;
	}
}

function fnc_account_no(row_id,account_no)
{
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/export_proceed_realization_controller.php?action=account_no_popup&account_no='+account_no,'Account No Popup', 'width=250px,height=300px,center=1,resize=0','../')
	emailwindow.onclose=function()
	{
		var theemail=this.contentDoc.getElementById("hdn_acc_info");
		if (theemail.value!="")
		{
			$('#acLoanNo_'+row_id).val(theemail.value);
			check_duplication(row_id)
		}
        ...
        <form name="exportProceedRealizationFrm_1" id="exportProceedRealizationFrm_1" autocomplete="off" method="POST"  >
            ...
            <legend>Export Proceeds Realization Entry</legend>
                <table cellpadding="2" cellspacing="1" width="100%">
                    ...
                    <tr height="10"><td></td></tr>
                  	<tr>
                    	<td width="100" class="must_entry_caption">Beneficiary</td>
                       	<td> 
                        	<?
							  echo create_drop_down( "cbo_beneficiary_name", 152, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business in(1,3) $company_cond order by comp.company_name","id,company_name", 1, "-- Select Beneficiary --", 0, "load_drop_down( 'requires/export_proceed_realization_controller', this.value, 'load_drop_down_buyer', 'buyer_td_id' );get_php_form_data(this.value,'print_button_variable_setting','requires/export_proceed_realization_controller' );",0 );
 							?>
                        ...
                        <td width="100" class="must_entry_caption">Bill/Invoice No</td>
                    	<td>
                        	...
                        	<input type="button" onClick="fnc_invoiceDetails();" style="width: 65px; background: #8DAFDA; border-radius: 7px;" value="Invoice Dtls">
                    ...
                    <tr>
                        <td>Remarks</td>
                        <td colspan="5"><input name="txt_remarks" id="txt_remarks" class="text_boxes" style="width:680px"></td>
                    </tr>
                     <tr>
					...
					<tbody id="deduction_list_view">
						<tr class="general" id='deduction_1' align="center">
							<td>
                                ...
                                <input type="text" name="cbodeductionHead[]" id="cbodeductionHead_1" class="text_boxes" style="width:170px;" onFocus="add_auto_complete( 1 )" onDblClick="fn_commercial_head_display(1,1,'cbodeductionHead')" onBlur="setTimeout(fn_value_check,1000,1,1,'cbodeductionHead');clearTimeout(1500);" placeholder="Browse Or Write" />
							</td>
							<td>
				...
				<table id="tbl_distribution" width="100%" border="0" cellpadding="0" cellspacing="0" class="rpt_table" rules="all">
					...
					<tbody id="distribution_list_view">
						<tr class="general" id='distribution_1' align="center">
							<td> 
                            	<!--<select class="combo_boxes" name="cbodistributionHead[]" id="cbodistributionHead_1" onChange="get_php_form_data( this.value+'**1**'+$('#cbo_beneficiary_name').val()+'**'+$('#is_invoice_bill_lien_bank').val(), 'populate_acc_loan_no_data', 'requires/export_proceed_realization_controller' );check_duplication(1);fn_loan_type_change(this.value,1);" style="width:172px">
                                	<option value="0">-- Select Account Head --</option>
										<?
											/*foreach($commercial_head as $key=>$value)
											{
