<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create for Export Proceed Realization Service			
Functionality	:	
JS Functions	:
Created by		:	Nayem 
...
function openmypage_realizationUpdate()
{
	...
	//alert(buyerID);
	var page_link='requires/export_proceed_realization_service_controller.php?action=proceed_realization_popup_search&beneficiary_name='+beneficiary_name+'&buyerID='+buyerID;
	...
	emailwindow.onclose=function()
	{
		...
		{
			freeze_window(5);
			reset_form('exportProceedRealizationFrm_1','','','distribution_tot_row,1','$(\'#tbl_distribution tbody tr:not(:first)\').remove();','cbo_beneficiary_name*cbo_buyer_name');
			get_php_form_data(invoice_bill_id+"**"+is_invoiceBill+"**"+realization_id, "populate_data_from_invoice_bill", "requires/export_proceed_realization_service_controller" );
			...
			show_list_view( realization_id+"**1**"+submit_type+"**"+variable_distribution, 'details_list_view', 'distribution_list_view', 'requires/export_proceed_realization_service_controller', '' ) ;	
...
function openmypage_InvoiceBill()
{
	...
	if (form_validation('cbo_beneficiary_name*cbo_service_type*cbo_within_group','Beneficiary*Service Type*Within Group')==false )
	{
		return;
	}
	
	var page_link='requires/export_proceed_realization_service_controller.php?action=invoice_bill_popup_search&beneficiary_name='+beneficiary_name+'&buyerID='+buyerID+'&import_btb='+import_btb+'&cbo_service_type='+cbo_service_type+'&cbo_within_group='+cbo_within_group;
	var title='Export Proceeds Realization Entry Form';
	...
	emailwindow.onclose=function()
	{
		...
		{
			freeze_window(5);
			reset_form('exportProceedRealizationFrm_1','','','distribution_tot_row,1','$(\'#tbl_distribution tbody tr:not(:first)\').remove();','cbo_service_type*cbo_beneficiary_name*cbo_within_group*cbo_buyer_name');
			$("#lc_id").val(lc_id);
			$("#lc_type").val(lc_type);
			get_php_form_data(invoice_bill_id+"**"+is_invoiceBill+"****"+lc_id+"**"+lc_type, "populate_data_from_invoice_bill", "requires/export_proceed_realization_service_controller" );
...
function fnc_invoiceDetails()
{
	...
	if (invoice_bill_id)
	{
		var page_link='requires/export_proceed_realization_service_controller.php?action=invoice_details_popup&invoice_bill_id='+invoice_bill_id+'&beneficiary_name='+beneficiary_name+'&buyerID='+buyerID;
...
{ 
	...
	if ( form_validation('cbo_service_type*cbo_beneficiary_name*cbo_buyer_name*txt_invoice_bill_no*txt_received_date*cbo_currency_name','Beneficiary*Buyer*Bill/Invoice No*Received Date*Currency')==false )
	{
		return;
	}
	else
	{
		...
		var dataString_distribution="";
		var z=0; var tot_row=0;
		...
		if(tot_row<1)
		{
			alert("No Distributions Data Insert");	
			return;
		}
		
		var data="action=save_update_delete&operation="+operation+'&distribution_tot_row='+z+get_submitted_data_string('cbo_service_type*cbo_beneficiary_name*cbo_within_group*cbo_buyer_name*txt_invoice_bill_no*invoice_bill_id*is_invoice_bill*txt_received_date*txt_remarks*update_id*hdn_variable_distribution*buyer_partial_rlz*grand_total_document_currency',"../../")+dataString_distribution;
		//alert(data);return;
		freeze_window(operation);
		
		http.open("POST","requires/export_proceed_realization_service_controller.php",true);
...
function fnc_export_proceed_realization_Reply_info()
{
	if(http.readyState == 4) 
	{
		...
		else if((reponse[0]==0 || reponse[0]==1))
		{
			...
			var submit_type=$('#submit_type').val();
			var variable_distribution=$("#hdn_variable_distribution").val();
			show_list_view( reponse[1]+"**1**"+submit_type+"**"+variable_distribution, 'details_list_view', 'distribution_list_view', 'requires/export_proceed_realization_service_controller', '' ) ;	
...
function calculate_grand_total()
{
	...
	var grand_total_document_currency=total_distribution_document_currency;
	var grand_total_domestic_currency=total_distribution_domestic_currency;
...
{ 
	...
	if(prefix=="distribution")
	{
		//$('#cbodistributionHead_'+row_num).removeAttr("onChange").attr("onChange","get_php_form_data(this.value+'**"+row_num+"**'+$('#cbo_beneficiary_name').val()+'**'+$('#is_invoice_bill_lien_bank').val(), 'populate_acc_loan_no_data','requires/export_proceed_realization_service_controller');check_duplication("+row_num+");fn_loan_type_change(this.value,"+row_num+");");
...
function open_loan_number_popup (element_id) {
	...
	var page_link='requires/export_proceed_realization_service_controller.php?action=loan_number_popup&beneficiary_name='+beneficiary+'&loan_type='+loan_type+'&pre_export_dtls_id='+pre_export_dtls_id+'&lc_id='+lc_id+'&lc_type='+lc_type;
...
function fn_value_check(seq_no,source_from,fld_value,fld_names)
{
	...
	if(source_from==2)
	{
		get_php_form_data( commercial_head_reverse[fld_value]+'**'+seq_no+'**'+$('#cbo_beneficiary_name').val()+'**'+$('#is_invoice_bill_lien_bank').val(), 'populate_acc_loan_no_data', 'requires/export_proceed_realization_service_controller' );
...
function fn_commercial_head_display(seq_no,head_from,fld_name)
{
	...
	var page_link='requires/export_proceed_realization_service_controller.php?action=commercial_head_popup';
	...
	emailwindow.onclose=function()
	{
		...
		if(head_from==2)
		{
			get_php_form_data( hdn_head_id+'**'+seq_no+'**'+$('#cbo_beneficiary_name').val()+'**'+$('#is_invoice_bill_lien_bank').val(), 'populate_acc_loan_no_data', 'requires/export_proceed_realization_service_controller' );
			check_duplication(seq_no);
...
function fn_set_variable(com_id,bank_id)
{
	...
	var variable_distribution = trim(return_global_ajax_value(com_id+"**"+bank_id, 'populate_data_form_lib', '', 'requires/export_proceed_realization_service_controller'));
	...
	//get_php_form_data( this.value, 'set_variable', 'requires/export_proceed_realization_service_controller' );
}
...
function fn_distribution_generate()
{
	...
	var net_bill_amount=(bill_amount-(distribution_total) );
	...
	if(net_bill_amount>0)
	{
		...
		var variable_distribution = return_global_ajax_value(variable_distribution+"**"+net_bill_amount+"**"+tot_row, 'populate_data_form_lib_dis', '', 'requires/export_proceed_realization_service_controller');
...
function fn_print_report(action)
{
	...
	{
		print_report( $('#update_id').val(), action, "requires/export_proceed_realization_service_controller" ) ;
	}
}
        ...
        <form name="exportProceedRealizationFrm_1" id="exportProceedRealizationFrm_1" autocomplete="off" method="POST"  >
            ...
            <legend>Export Proceeds Realization Entry</legend>
                <table cellpadding="2" cellspacing="1" width="100%">
                	<tr>
                    	<td colspan="8" align="center"><b>Previous Entry Search: </b> 
                    ...
                    <tr height="10"><td></td></tr>
                  	<tr>
                    	<td width="100" class="must_entry_caption">Service Type</td>
                       	<td> 
						   <? echo create_drop_down("cbo_service_type", 162, $export_item_category, "", 1, "--- Select ---", 1, "", "",'10,23,35,36,37,38,45,67'); ?>
                        </td>
                    	<td width="100" class="must_entry_caption">Beneficiary</td>
                       	<td> 
                        	<?
							  echo create_drop_down( "cbo_beneficiary_name", 152, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business in(1,3) $company_cond order by comp.company_name","id,company_name", 1, "-- Select Beneficiary --", 0, "load_drop_down( 'requires/export_proceed_realization_service_controller', this.value, 'load_drop_down_buyer', 'buyer_td_id' );",0 );
 							?>
                    ...
                    <tr>
                    	...
                    	<td>
                        	<input type="text" name="txt_invoice_bill_no" id="txt_invoice_bill_no" style="width:150px" class="text_boxes" placeholder="Double Click to Search" onDblClick="openmypage_InvoiceBill()" readonly />
