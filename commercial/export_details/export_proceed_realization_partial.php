...
function openmypage_realizationUpdate()
{
	...
	//alert(buyerID);
	var page_link='requires/export_proceed_realization_partial_controller.php?action=proceed_realization_popup_search&beneficiary_name='+beneficiary_name+'&buyerID='+buyerID;
	...
	emailwindow.onclose=function()
	{
		...
		if(trim(realization_id)!="")
		{
			...
			get_php_form_data(invoice_bill_id+"**"+is_invoiceBill+"**"+realization_id, "populate_data_from_invoice_bill", "requires/export_proceed_realization_partial_controller" );
			
			var submit_type=$('#submit_type').val();
			$("#hidden_posted_account").val(is_posted_accounts);
			show_list_view( realization_id+"**0", 'details_list_view', 'deduction_list_view', 'requires/export_proceed_realization_partial_controller', '' ) ;
			...
			show_list_view( realization_id+"**1**"+submit_type+"**"+variable_distribution, 'details_list_view', 'distribution_list_view', 'requires/export_proceed_realization_partial_controller', '' ) ;	
...
function openmypage_InvoiceBill()
{
	...
	var page_link='requires/export_proceed_realization_partial_controller.php?action=invoice_bill_popup_search&beneficiary_name='+beneficiary_name+'&buyerID='+buyerID+'&import_btb='+import_btb;
	var title='Export Proceeds Realization Entry Form';
	...
	emailwindow.onclose=function()
	{
		...
		if(trim(invoice_bill_id)!="")
		{
			...
			$("#lc_id").val(lc_id);
			$("#lc_type").val(lc_type);
			get_php_form_data(invoice_bill_id+"**"+is_invoiceBill+"****"+lc_id+"**"+lc_type, "populate_data_from_invoice_bill", "requires/export_proceed_realization_partial_controller" );
...
function fnc_invoiceDetails()
{
	...
	if (invoice_bill_id)
	{
		var page_link='requires/export_proceed_realization_partial_controller.php?action=invoice_details_popup&invoice_bill_id='+invoice_bill_id+'&beneficiary_name='+beneficiary_name+'&buyerID='+buyerID;
...
{ 
	...
	else
	{
		...
		http.open("POST","requires/export_proceed_realization_partial_controller.php",true);
...
function fnc_export_proceed_realization_Reply_info()
{
	if(http.readyState == 4) 
	{
		...
		else if((reponse[0]==0 || reponse[0]==1))
		{
			...
			var submit_type=$('#submit_type').val();
			var variable_distribution=$("#hdn_variable_distribution").val();
			show_list_view( reponse[1]+"**0", 'details_list_view', 'deduction_list_view', 'requires/export_proceed_realization_partial_controller', '' ) ;
			show_list_view( reponse[1]+"**1**"+submit_type+"**"+variable_distribution, 'details_list_view', 'distribution_list_view', 'requires/export_proceed_realization_partial_controller', '' ) ;	
...
{ 
	...
	if(prefix=="distribution")
	{
		//$('#cbodistributionHead_'+row_num).removeAttr("onChange").attr("onChange","get_php_form_data(this.value+'**"+row_num+"**'+$('#cbo_beneficiary_name').val()+'**'+$('#is_invoice_bill_lien_bank').val(), 'populate_acc_loan_no_data','requires/export_proceed_realization_partial_controller');check_duplication("+row_num+");fn_loan_type_change(this.value,"+row_num+");");
...
function open_loan_number_popup (element_id) {
	...
	var page_link='requires/export_proceed_realization_partial_controller.php?action=loan_number_popup&beneficiary_name='+beneficiary+'&loan_type='+loan_type+'&pre_export_dtls_id='+pre_export_dtls_id+'&lc_id='+lc_id+'&lc_type='+lc_type;
...
function fn_value_check(seq_no,source_from,fld_value,fld_names)
{
	...
	if(source_from==2)
	{
		get_php_form_data( commercial_head_reverse[fld_value]+'**'+seq_no+'**'+$('#cbo_beneficiary_name').val()+'**'+$('#is_invoice_bill_lien_bank').val(), 'populate_acc_loan_no_data', 'requires/export_proceed_realization_partial_controller' );
...
function fn_commercial_head_display(seq_no,head_from,fld_name)
{
	...
	var page_link='requires/export_proceed_realization_partial_controller.php?action=commercial_head_popup';
	...
	emailwindow.onclose=function()
	{
		...
		if(head_from==2)
		{
			get_php_form_data( hdn_head_id+'**'+seq_no+'**'+$('#cbo_beneficiary_name').val()+'**'+$('#is_invoice_bill_lien_bank').val(), 'populate_acc_loan_no_data', 'requires/export_proceed_realization_partial_controller' );
			check_duplication(seq_no);
...
function fn_set_variable(com_id,bank_id)
{
	...
	var variable_distribution = trim(return_global_ajax_value(com_id+"**"+bank_id, 'populate_data_form_lib', '', 'requires/export_proceed_realization_partial_controller'));
	...
	//get_php_form_data( this.value, 'set_variable', 'requires/export_proceed_realization_partial_controller' );
}
...
function fn_distribution_generate()
{
	...
	if(net_bill_amount>0)
	{
		...
		var variable_distribution = return_global_ajax_value(variable_distribution+"**"+net_bill_amount+"**"+tot_row+"**"+cbo_currency_name+"**"+com_id, 'populate_data_form_lib_dis', '', 'requires/export_proceed_realization_partial_controller');
...
function fn_print_report(action)
{
	...
	{
		print_report( $('#update_id').val(), action, "requires/export_proceed_realization_partial_controller" ) ;
	}
}
                    ...
                    <tr height="10"><td></td></tr>
                  	<tr>
                    	<td width="100" class="must_entry_caption">Beneficiary</td>
                       	<td> 
                        	<?
							  echo create_drop_down( "cbo_beneficiary_name", 152, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business in(1,3) $company_cond order by comp.company_name","id,company_name", 1, "-- Select Beneficiary --", 0, "load_drop_down( 'requires/export_proceed_realization_partial_controller', this.value, 'load_drop_down_buyer', 'buyer_td_id' );",0 );
 							?>
				...
				<table id="tbl_distribution" width="100%" border="0" cellpadding="0" cellspacing="0" class="rpt_table" rules="all">
					...
					<tbody id="distribution_list_view">
						<tr class="general" id='distribution_1' align="center">
							<td> 
                            	<!--<select class="combo_boxes" name="cbodistributionHead[]" id="cbodistributionHead_1" onChange="get_php_form_data( this.value+'**1**'+$('#cbo_beneficiary_name').val()+'**'+$('#is_invoice_bill_lien_bank').val(), 'populate_acc_loan_no_data', 'requires/export_proceed_realization_partial_controller' );check_duplication(1);fn_loan_type_change(this.value,1);" style="width:172px">
                                	<option value="0">-- Select Account Head --</option>
										<?
											/*foreach($commercial_head as $key=>$value)
											{
											...
											//echo create_drop_down( "cbodistributionHead_1", 172, $commercial_head,"", 1, "-- Select Account Head --", 0, "get_php_form_data( this.value+'**1**'+$('#cbo_beneficiary_name').val(), 'populate_acc_loan_no_data', 'requires/export_proceed_realization_partial_controller' );" );
                                        ?>
                                </select>-->
