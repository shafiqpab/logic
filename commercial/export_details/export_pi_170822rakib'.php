...
function fnc_pi_mst( operation )
{
	...
	var data="action=save_update_delete&operation="+operation+get_submitted_data_string('cbo_item_category_id*cbo_exporter_id*cbo_within_group*cbo_buyer_name*pi_number*pi_date*last_shipment_date*pi_validity_date*cbo_currency_id*hs_code*txt_swift*txt_internal_file_no*txt_remarks*txt_attention*cbo_advising_bank*update_id*hidden_variable_setting*pi_revised_date',"../../");
	//alert(data);return;	  
...
function fnc_pi_item_details( operation )
{
	...
	for (var j=1; j<=row_num; j++)
	{
		...
		if($('#workOrderChkbox_'+j).is(':checked') || updateIdDtls!="")
		{
			...
			{
				data_all+="&workOrderNo_" + i + "='" + $('#workOrderNo_'+j).val()+"'"+"&hideWoId_" + i + "='" + $('#hideWoId_'+j).val()+"'"+"&hideWoDtlsId_" + i + "='" + $('#hideWoDtlsId_'+j).val()+"'"+"&salesbooking_" + i + "='" + $('#salesbooking_'+j).val()+"'"+"&colorId_" + i + "='" + $('#colorId_'+j).val()+"'"+"&hideAopColor_" + i + "='" + $('#hideAopColor_'+j).val()+"'"+"&gsm_" + i + "='" + $('#gsm_'+j).val()+"'"+"&hideBodypart_" + i + "='" + $('#hideBodypart_'+j).val()+"'"+"&uom_" + i + "='" + $('#uom_'+j).val()+"'"+"&quantity_" + i + "='" + $('#quantity_'+j).val()+"'"+"&rate_" + i + "='" + $('#rate_'+j).val()+"'"+"&amount_" + i + "='" + $('#amount_'+j).val()+"'"+"&updateIdDtls_" + i + "='" + $('#updateIdDtls_'+j).val()+"'";
			}
	...
	var data="action=save_update_delete_dtls&operation="+operation+'&cbo_item_category='+cbo_item_category+'&total_row='+i+'&update_id='+update_id+'&txt_total_amount='+txt_total_amount+'&txt_upcharge='+txt_upcharge+'&txt_discount='+txt_discount+'&txt_total_amount_net='+txt_total_amount_net+'&cbo_currency_id='+cbo_currency_id+data_all;
                    ...
                    <table width="100%" border="0" cellpadding="0" cellspacing="2">
                        ...
                        <tr>
                            ...
                            <td width="150"> 
                                ...
                                echo create_drop_down( "cbo_item_category_id", 151, $export_item_category,'', 1, ' --Select-- ',0,"show_list_view(document.getElementById('cbo_exporter_id').value+'_'+this.value+'_'+'1', 'catagory_wise_pi_details', 'export_pi_details_container', 'requires/export_pi_controller', '' ) ;",0,'1,2,3,4,5,10,11,20,22,23,24,30,31,35,36,37,45,67,68,69');
                                ?>  
                            </td>
                            <td width="110" align="right" class="must_entry_caption">Exporter</td>
                            <td width="150">
                                 <?php echo create_drop_down( "cbo_exporter_id", 151,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, 'Select',0,"load_drop_down( 'requires/export_pi_controller',this.value+'_'+document.getElementById('cbo_within_group').value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/export_pi_controller',this.value, 'load_drop_down_avising_bank', 'avising_bank_td' );check_variable_setting(this.value);load_drop_down( 'requires/export_pi_controller',this.value, 'company_wise_report_button_setting', 'buttion_list' );set_field_level_access(this.value);",0); ?>
						...
						<tr>
							...
							<td><input type="text" name="test_advance_job" id="test_advance_job" class="text_boxes" style="width:140px" placeholder="Browse" onDblClick="openmypage_advance_job()" maxlength="30" /></td>              	
						...
						<tr>
							...
							<td>
								<? 
								echo create_drop_down( "cbo_pay_term", 151, $pay_term,"", 1, "-- Select --", 0, "",0,"1,2");
								?> 
							</td>
                        ...
                        <tr>
                            <td colspan="8" height="50" valign="middle" align="center" class="button_container">
                                ...
                                <? 
									   ...
									   echo load_submit_buttons( $permission, "fnc_pi_mst", 0,0 ,"reset_form('pimasterform_1','','','cbo_currency_id,2','$(\'#tbl_pi_item tbody tr:not(:first)\').remove();')",1);
							    ?>
                             </td> 
