...
if($action=="create_po_search_list_view")
{
	...
	{
		//$attached_po_id=return_field_value("LISTAGG(wo_po_break_down_id, ',') WITHIN GROUP (ORDER BY wo_po_break_down_id) as po_id","com_sales_contract_order_info","com_sales_contract_id=$sales_contractID and status_active=1 and is_deleted=0","po_id");
                    ...
                    foreach ($nameArray as $selectResult)
                    {
                    	...
                    	?>
                            <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value(<? echo $i;?>)">
                                ...
                                <td width="130"><p><? echo $actual_po_arr[$selectResult[csf('id')]]; ?>&nbsp;</p></td>
                                <td width="130">
                                    <p>
                                        <?
                                            $gmts_item='';
...
if ($action=="save_update_delete_mst")
{
	...
	else if ($operation==1)   // Update Here
	{
		...
		{
			echo "31** Contact Value Not Allow Less Then Attach Value";die;
		}
	...
	else if ($operation==2)   // Delete Here
	{
		...
		if(str_replace("'","",$txt_system_id)=="") { echo "10**";die; }
		...
		if($invoice_id>0)
		{
			echo "35**Delete Not Allow. This SC No Found in Invoice"; die;
		}
		else
		{
			if($id>0)
...
if ($action=="save_update_delete_contract_order_info")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$pre_tot_attached =  sql_select("select sum(b.attached_value) as attached_value
			...
			where a.id = b.com_sales_contract_id and a.id=".$txt_system_id."
		...
		{
			echo "11** Attached Value Exceeds Contract Value ";die;
		}
