...
if($action=="print_button_variable_setting")
{
	...
	foreach($printButton as $id){
		...
		if($id==884)$buttonHtml.='<input type="button" style="width:80px;" id="btn_print_letter9"  onClick="print_to_html_report(9)"   class="formbutton_disabled printReport" name="btn_print_letter9" value="Bill of Exchng2" />';
...
{
	$rowNo = $data+1;
	//create_drop_down( "cbo_account_head_".$rowNo, 200, $commercial_head,"", 1, "-- Select --", $selected, "get_php_form_data(this.value+'**'+".$rowNo."+'**'+document.getElementById('cbo_company_name').value+'**'+document.getElementById('cbo_lien_bank').value, 'populate_acc_loan_no_data', 'requires/export_doc_sub_entry_controller' );",0,"" )
	echo '<tr id="tr'.$rowNo.'">
			<td>
			<input type="text" name="cbo_account_head[]" id="cbo_account_head_'.$rowNo.'" class="text_boxes" style="width:170px;" onFocus="add_auto_complete( '.$rowNo.' )"  onBlur="fn_value_check('.$rowNo.',this.value,\'cbo_account_head\')" onDblClick="fn_commercial_head_display('.$rowNo.',\'cbo_account_head\')" placeholder="Browse Or Write" />
			</td>							
            ...
            <table cellpadding="0" cellspacing="0" border="1" rules="all" width="930" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td>
                     ...
                     <td>
                        <input type="button" name="btn_show"  class="formbutton" value="Show" onClick="show_list_view(document.getElementById('cbo_company_name').value+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('cbo_buyer_sub').value+'**'+document.getElementById('txt_lc_no').value+'**'+document.getElementById('txt_invoice_no').value+'**'+'<? echo $invoice_id_all; ?>'+'**'+'<? echo $mst_tbl_id; ?>'+'**'+document.getElementById('txt_date_from').value+'**'+document.getElementById('txt_date_to').value+'**'+document.getElementById('cbo_lc_for').value, 'lcSc_search_list_view', 'search_divs', 'export_doc_sub_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1)');set_all();" style="width:80px;" />
                     </td>
					

						<!-- <td align="center">
                        <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<?// echo $company_name; ?>+'_'+document.getElementById('cbo_buyer_name').value, 'create_system_id_search_list_view', 'search_div', 'export_doc_sub_entry_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />				
                    </td> -->
...
if($action=="lcSc_search_list_view")
{
	...
	if($search_by_buy_sub==2)
	{
		...
		if ($search_lc_sc != "")
		{
			...
			SELECT m.is_lc, m.id, m.invoice_no, m.invoice_date, m.lc_sc_id, m.bl_no, m.bl_date, m.invoice_value, m.net_invo_value, m.buyer_id, b.short_name, c.id as sub_dtls_id, c.doc_submission_mst_id as sub_mst_id, 2 as type 
			...
			$sql = "SELECT m.is_lc, m.id, m.invoice_no, m.invoice_date, m.lc_sc_id, m.bl_no, m.bl_date, m.invoice_value, m.net_invo_value, m.buyer_id, b.short_name, c.id as sub_dtls_id, c.doc_submission_mst_id as sub_mst_id, 1 as type 
			FROM com_export_invoice_ship_mst m, lib_buyer b, com_export_doc_submission_invo c, com_export_doc_submission_mst d 
		...
		else
		{
			...
			$sql = "SELECT m.is_lc,m.id,m.invoice_no,m.invoice_date,m.lc_sc_id, m.bl_no, m.bl_date, m.invoice_value,m.net_invo_value,m.buyer_id,b.short_name,c.id as sub_dtls_id,c.doc_submission_mst_id as sub_mst_id, 1 as type FROM com_export_invoice_ship_mst m, lib_buyer b , com_export_doc_submission_invo c, com_export_doc_submission_mst d  
	...
	else
	{
		...
		{ 
			...
			$sql = "SELECT m.is_lc, m.id, m.invoice_no, m.invoice_date, m.bl_no, m.bl_date, m.lc_sc_id, m.invoice_value, m.net_invo_value, m.buyer_id, b.short_name, m.import_btb
			...
			UNION ALL 
			SELECT m.is_lc, m.id, m.invoice_no, m.invoice_date, m.bl_no, m.bl_date, m.lc_sc_id, m.invoice_value, m.net_invo_value, m.buyer_id, b.company_short_name as short_name, m.import_btb 
	...
	?>
       ...
       <div style="width:930px; overflow-y:scroll; max-height:200px" id="scroll_body">                
			...
			<?
				$nameArray_lib=sql_select( "select id, pi_source_btb_lc from variable_settings_commercial where company_name='$company_id' and variable_list=38 and STATUS_ACTIVE=1 order by id");
				$pi_source_btb_lc= $nameArray_lib[0]["PI_SOURCE_BTB_LC"];
                ...
                {  
					...
					if($pay_term_cond!=3 && $sub_invoice_data[$row[csf("id")]]=="")//pay term 3 means cash in advanced sales contact not come in document submission form		
					{
						...
						?>   	
                        <tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $i;?>', '<? echo $pi_source_btb_lc; ?>','<?=$row[csf("bl_no")]?>','<?=change_date_format($row[csf("bl_date")])?>')" > 
                            <td width="40"><? echo $i;?> 
...
if ($action=="show_invoice_list_view")
{
	...
	if($data[1]!="")
	{
		$sql_prev_dtls=sql_select("select a.invoice_id, a.id, a.is_lc, b.lc_for from com_export_doc_submission_invo a, com_export_invoice_ship_mst b 
		where a.invoice_id=b.id and a.doc_submission_mst_id in($data[1]) and a.status_active=1");
		foreach($sql_prev_dtls as $row)
		{
			$sub_dtls_id_arr[$row[csf("invoice_id")]]=$row[csf("id")];
			$prev_lc_sc=$row[csf("is_lc")];
			$lc_for=$row[csf("lc_for")];
			//$prev_inv_id.=$row[csf("invoice_id")].",";
		}
	}
	else
	{
		$sql_prev_dtls=sql_select("select b.lc_for, max(b.is_lc) as is_lc from com_export_invoice_ship_mst b where b.id in (".$data[0].") group by b.lc_for");
		$prev_lc_sc=$sql_prev_dtls[0][csf("is_lc")];
		$lc_for=$sql_prev_dtls[0][csf("lc_for")];
	}
	
	//$all_inv_id=$prev_inv_id.$data[0];
	//$sub_dtls_id_arr = return_library_array( "select invoice_id, id from com_export_doc_submission_invo where doc_submission_mst_id in($data[1]) and status_active=1",'invoice_id','id');
	$item_category_id=$lc_import_btb=0;
	if($prev_lc_sc==1)
	{
		$poSQL=sql_select("select max(b.lc_sc_id) as lc_sc_id, max(a.export_lc_no) as export_lc_no, max(a.import_btb) as import_btb, max(a.export_item_category) as export_item_category 
		from com_export_lc a, com_export_invoice_ship_mst b 
		where a.id=b.lc_sc_id and b.id in (".$data[0].")");
		foreach($poSQL as $poR)
		{
			$lc_sc_no=$poR[csf("export_lc_no")];
			$lc_import_btb=$poR[csf("import_btb")];
			$item_category_id=$poR[csf("export_item_category")];
			$all_lc_sc_id=$poR[csf("lc_sc_id")];
		} 
	}
	else
	{
		$scSQL=sql_select("select max(b.lc_sc_id) as lc_sc_id, max(contract_no) as contract_no from com_sales_contract a, com_export_invoice_ship_mst b where a.id=b.lc_sc_id and b.id in (".$data[0].")");
		foreach($scSQL as $poR)
		{
			$lc_sc_no=$poR[csf("contract_no")];
			$all_lc_sc_id=$poR[csf("lc_sc_id")];
		} 
	}
	//echo $all_lc_sc_id;die;;
	...
	if($lc_for==2)
	{
		...
		from sample_development_mst a, com_export_invoice_ship_dtls b, com_export_invoice_ship_mst c
		 where a.id=b.po_breakdown_id and b.mst_id=c.id and c.lc_sc_id in(".$all_lc_sc_id.") and c.is_lc=$prev_lc_sc and c.lc_for=2");
	...
	else if($lc_import_btb == 1 && $item_category_id == 2)
	{
		...
		where a.com_export_lc_id=b.lc_sc_id and b.id=c.mst_id and b.is_lc=$prev_lc_sc and a.com_export_lc_id in($all_lc_sc_id)");
	...
	else if($item_category_id == 10)
	{
		$poSQL=sql_select("select a.job_no_mst as PO_NUMBER, c.id as INV_ID from fabric_sales_order_dtls a, com_export_invoice_ship_dtls b, com_export_invoice_ship_mst c
		 where a.id=b.po_breakdown_id and b.mst_id=c.id and c.lc_sc_id in(".$all_lc_sc_id.") and c.is_lc=$prev_lc_sc");
		foreach($poSQL as $value)
		{
			$po_numbers_arr[$value["INV_ID"]].=$value["PO_NUMBER"].",";
		}
	...
	//print_r($po_numbers_arr);die;
	$lcscNoString="";$lcscNoID="";$lienBank="";$currencyID="";$invoice_tr="";$tot_invoice_val=0; 
	$i=1;
	//echo $prev_lc_sc.'system';die;
	if($prev_lc_sc == "1") // if LC
	{
		if($db_type==0)
		{
			$sqlDtls="SELECT invo.id, invo.invoice_no, invo.buyer_id, invo.invoice_date, invo.lc_sc_id, invo.is_lc,invo.bl_no,invo.bl_date, invo.net_invo_value, lc.lien_bank, lc.currency_name, lc.export_lc_no as lcsc_no, lc.tenor, group_concat(dtl.po_breakdown_id) as po_breakdown_id,invo.import_btb,group_concat(lc.export_item_category) as item_category, lc.pay_term 
			FROM  com_export_invoice_ship_mst invo, com_export_lc lc, com_export_invoice_ship_dtls dtl 
			WHERE invo.id in($data[0]) and invo.lc_sc_id = lc.id and invo.is_lc=1 and invo.id=dtl.mst_id and dtl.current_invoice_qnty>0 
			group by invo.id, invo.invoice_no, invo.buyer_id, invo.invoice_date, invo.lc_sc_id, invo.is_lc, invo.bl_no, invo.bl_date, invo.net_invo_value,invo.import_btb, lc.lien_bank, lc.currency_name, lc.export_lc_no, lc.tenor, lc.pay_term";
		}
		else
		{
			$sqlDtls="SELECT invo.id, invo.invoice_no, invo.buyer_id, invo.invoice_date, invo.lc_sc_id, invo.is_lc,invo.bl_no,invo.bl_date, invo.net_invo_value, lc.lien_bank, lc.currency_name, lc.export_lc_no as lcsc_no, lc.tenor, invo.import_btb, invo.lc_for ,LISTAGG(dtl.po_breakdown_id, ',') WITHIN GROUP (ORDER BY dtl.po_breakdown_id) as po_breakdown_id, LISTAGG(lc.export_item_category, ',') WITHIN GROUP (ORDER BY lc.export_item_category) as item_category, lc.pay_term 
			FROM  com_export_invoice_ship_mst invo, com_export_lc lc, com_export_invoice_ship_dtls dtl 
			WHERE invo.id in($data[0]) and invo.lc_sc_id = lc.id and invo.is_lc=1 and invo.id=dtl.mst_id and dtl.current_invoice_qnty>0 
			group by invo.id, invo.invoice_no, invo.buyer_id, invo.invoice_date, invo.lc_sc_id, invo.is_lc, invo.bl_no, invo.bl_date, invo.net_invo_value,invo.import_btb, lc.lien_bank, lc.currency_name, lc.export_lc_no, lc.tenor, lc.pay_term, invo.lc_for";
		}
		/*$sqlDtls="SELECT invo.id, invo.invoice_no, invo.buyer_id, invo.invoice_date, invo.lc_sc_id, invo.is_lc,invo.bl_no,invo.bl_date, invo.net_invo_value, lc.lien_bank, lc.currency_name, lc.export_lc_no as lcsc_no , invo.import_btb ,
		listagg(cast(dtl.po_breakdown_id as varchar(4000)),',') within group(order by dtl.po_breakdown_id) as po_breakdown_id,
		listagg(cast(lc.export_item_category as varchar(4000)),',') within group(order by lc.export_item_category) as item_category
		FROM  com_export_invoice_ship_mst invo, com_export_lc lc, com_export_invoice_ship_dtls dtl 
		WHERE invo.id in($data[0]) and invo.lc_sc_id = lc.id and invo.is_lc=1 and invo.id=dtl.mst_id and dtl.current_invoice_qnty>0 group by invo.id, invo.invoice_no, invo.buyer_id, invo.invoice_date, invo.lc_sc_id, invo.is_lc, invo.bl_no, invo.bl_date, invo.net_invo_value,invo.import_btb, lc.lien_bank, lc.currency_name, lc.export_lc_no";*/
	}
	else
	{
		if($db_type==0)
		{
			$sqlDtls="SELECT invo.id, invo.invoice_no, invo.buyer_id, invo.invoice_date, invo.lc_sc_id, invo.is_lc,invo.bl_no,invo.bl_date, invo.net_invo_value, sc.lien_bank,sc.currency_name,sc.contract_no as lcsc_no, sc.tenor, group_concat(dtl.po_breakdown_id) as po_breakdown_id,invo.import_btb,group_concat(sc.export_item_category) as item_category, sc.pay_term
			FROM  com_export_invoice_ship_mst invo, com_sales_contract sc, com_export_invoice_ship_dtls dtl 
			WHERE invo.id in($data[0]) and invo.lc_sc_id = sc.id and invo.is_lc=2 and invo.id=dtl.mst_id and dtl.current_invoice_qnty>0 
			group by invo.id, invo.invoice_no, invo.buyer_id, invo.invoice_date, invo.lc_sc_id, invo.is_lc, invo.bl_no, invo.bl_date, invo.net_invo_value,invo.import_btb, sc.lien_bank, sc.currency_name, sc.contract_no, sc.tenor, sc.pay_term";
		}
		else
		{
			$sqlDtls="SELECT invo.id, invo.invoice_no, invo.buyer_id, invo.invoice_date, invo.lc_sc_id, invo.is_lc, invo.bl_no, invo.bl_date, invo.net_invo_value, sc.lien_bank, sc.currency_name, sc.contract_no as lcsc_no, sc.tenor, invo.lc_for, LISTAGG(dtl.po_breakdown_id, ',') WITHIN GROUP (ORDER BY dtl.po_breakdown_id) as po_breakdown_id, LISTAGG(sc.export_item_category, ',') WITHIN GROUP (ORDER BY sc.export_item_category) as item_category, sc.pay_term 
			FROM  com_export_invoice_ship_mst invo, com_sales_contract sc, com_export_invoice_ship_dtls dtl  
			WHERE invo.id in($data[0]) and invo.lc_sc_id = sc.id and invo.is_lc=2 and invo.id=dtl.mst_id and dtl.current_invoice_qnty>0 
			group by invo.id, invo.invoice_no, invo.buyer_id, invo.invoice_date, invo.lc_sc_id, invo.is_lc, invo.bl_no, invo.bl_date, invo.net_invo_value,invo.import_btb, sc.lien_bank, sc.currency_name, sc.contract_no, sc.tenor, sc.pay_term, invo.lc_for";
		}
	}
	...
	foreach($resArray as $res)
	{
		...
		//if($db_type==2) $res[csf("po_breakdown_id")] = $res[csf("po_breakdown_id")]->load();
		//if($db_type==2) $res[csf("item_category")] = $res[csf("item_category")]->load();
		//echo $res[csf("po_breakdown_id")]."=".$res[csf("item_category")];die;
		$lcscNoString = $res[csf("lcsc_no")];
		$lcscNoID = $res[csf("lc_sc_id")];
		$lienBank=$res[csf("lien_bank")];
		$currencyID=$res[csf("currency_name")];
		$buyerID=$res[csf("buyer_id")];
		$import_btb=$res[csf("import_btb")];
		$lc_sc_tenor=$res[csf("tenor")];
		$pay_term=$res[csf("pay_term")];
		
		$item_category_id = implode(",",array_filter(array_unique(explode(",",$res[csf("item_category")]))));

		//list view data arrange-----------------// 
		$po_numbers="";
		$po_numbers=chop($po_numbers_arr[$res[csf("id")]],",");
		/*$sales_order_arr = array();
		if($import_btb == 1 && $item_category_id == 2)
		{
			
			$sales_order_arr =  sql_select("select work_order_no from com_export_lc_order_info where com_export_lc_id =". $res[csf("lc_sc_id")]);
			foreach ($sales_order_arr as $value) 
			{
				if($po_numbers=="") $po_numbers=$value[csf("work_order_no")]; else $po_numbers.=",".$value[csf("work_order_no")];
			}
			$po_numbers = implode(",",array_filter(array_unique(explode(",",$po_numbers))));
		}
		else
		{
		...
		/*$invoice_tr .= "<tr>".
							...
							"<td><input type=\"text\" id=\"txt_bl_date$i\" name=\"txt_bl_date[]\" class=\"text_boxes\" style=\"width:100px\" value=\"".change_date_format($res[csf("bl_date")])."\" /></td>".
						...
						"</tr>";*/
		$invoice_tr .= '<tr><td align="center">'.$i.'</td><td id="txt_invoice_no'.$i.'" title="'.$res[csf("id")].'" subdtls="'.$sub_dtls_id_arr[$res[csf("id")]].'">'.$res[csf("invoice_no")].'</td><td id="txt_lcsc_no'.$i.'" title="'.$res[csf("lc_sc_id")].'" islc="'.$res[csf("is_lc")].'">'.$res[csf("lcsc_no")].'</td><td id="txt_bl_no'.$i.'">'.$res[csf("bl_no")].'</td><td align="center" id="txt_bl_date'.$i.'">'.change_date_format($res[csf("bl_date")]).'</td><td align="center" id="txt_invoice_date'.$i.'">'.change_date_format($res[csf("invoice_date")]).'</td><td align="right" id="txt_net_invo_value'.$i.'">'.$res[csf("net_invo_value")].'</td><td id="txt_po_numbers'.$i.'" title="'.$res[csf("po_breakdown_id")].'" style="word-break:break-all">&nbsp;'.$po_numbers.'</td></tr>'; 
	...
	echo "$('#txt_import_btb').val('".$import_btb."');\n";
	echo "$('#txt_day_to_realize').val('".$lc_sc_tenor."');\n";
	echo "$('#cbo_pay_term').val('".$pay_term."');\n";
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$duplicate = is_duplicate_field("id"," com_export_doc_submission_mst","bank_ref_no=$txt_bank_ref and company_id=$cbo_company_name and entry_form=40 and buyer_id=$cbo_buyer_name and status_active=1 and is_deleted=0 $null_cond"); 
		if($duplicate==1) 
		{			 
		...
		$field_array="id,doc_submission_mst_id,invoice_id,is_lc,lc_sc_id,submission_dtls_id,bl_no,bl_date,invoice_date,net_invo_value,all_order_no,inserted_by,insert_date";
		$update_sub_dtls_arr="is_converted";
		$data_array="";$update_sub_dtlsID_array=array();
		for($i=1;$i<=$invoiceRow;$i++)
		{
			...
			$txt_bl_date 			= 'txt_bl_date'.$i;
			$txt_invoice_date 		= 'txt_invoice_date'.$i;
			$txt_net_invo_value 	= 'txt_net_invo_value'.$i;
			$hidden_po_numbers_id 	= 'hidden_po_numbers_id'.$i;
			
			if($buyer_submission_inv[$$hidden_invoice_id]=="") $sub_dtls_id=0; else  $sub_dtls_id=$buyer_submission_inv[$$hidden_invoice_id];
			
			if($db_type==0) $invoice_date=$$txt_invoice_date;
			else $invoice_date=change_date_format($$txt_invoice_date,'','',1);
			if($db_type==0) $bl_date=$$txt_bl_date;
			else $bl_date=change_date_format($$txt_bl_date,'','',1);
			
  			$data_array .= "(".$dtlsid.",".$id.",'".$$hidden_invoice_id."','".$$hidden_is_lc."','".$$txt_lcsc_id."','".$sub_dtls_id."','".$$txt_bl_no."','".$bl_date."','".$invoice_date."','".$$txt_net_invo_value."','".$$hidden_po_numbers_id."','".$user_id."','".$pc_date_time."')";	
			$dtlsid=$dtlsid+1;
			if($sub_dtls_id)
			{
				$update_sub_dtlsID_array[]=$sub_dtls_id; 
				$update_sub_dtls_data[$sub_dtls_id]=explode("*",("1"));
			}
		}
		//echo $$hidden_sub_dtls_id;die;
		//echo "20**insert into com_export_doc_submission_mst (".$field_array_mst.") values ".$data_array_mst;die;
		/*if($data_array!="")
		{
		...
		$field_array_trans="id,doc_submission_mst_id,acc_head,acc_loan,loan_acc_no,dom_curr,conver_rate,lc_sc_curr,inserted_by,insert_date";
		...
		for($i=1;$i<=$transRow;$i++)
		{
			...
			$txt_lcsc_currency 		= 'txt_lcsc_currency_'.$i;
  			...
  			$data_array_trans .= "(".$trid.",".$id.",'".$$cbo_account_head."','".$$txt_ac_loan_no."','".$$txt_loan_no."','".$$txt_domestic_curr."','".$$txt_conversion_rate."','".$$txt_lcsc_currency."','".$user_id."','".$pc_date_time."')";	
			$trid=$trid+1;
		}
		...
		$upsubDtlsID=true;
		if(count($update_sub_dtlsID_array)>0)
		{
		...
		//echo "10** $rID && $dtlsrID && $tsrID && $upsubDtlsID";die;
		if($db_type==0)
		{
	...
	else if ($operation==1)   // Update Here
	{
		...
		$rID=$dtlsrID=$tsrID=$deleteDtls=$deletetrans=$upsubDtlsID=$update_buyer_convart=true;
		if($all_buyer_sub_dtls_id!="")
		{
			$update_buyer_convart=execute_query("Update com_export_doc_submission_invo set is_converted=0 where id in($all_buyer_sub_dtls_id)");
		}
		...
		if(count($update_sub_dtlsID_array)>0)
		{
			$upsubDtlsID=execute_query(bulk_update_sql_statement("com_export_doc_submission_invo","id",$update_sub_dtls_arr,$update_sub_dtls_data,$update_sub_dtlsID_array));
		}
		//echo "10**$upsubDtlsID";die;execute_query
		//echo "10**".$rID." && ".$dtlsrID." && ".$tsrID." && ".$deleteDtls." && ".$deletetrans." && ".$upsubDtlsID." && ".$update_buyer_convart;oci_rollback($con);die;
		
		if($db_type==0)
		{
			if($rID && $dtlsrID && $tsrID && $deleteDtls && $deletetrans && $upsubDtlsID && $update_buyer_convart)
			{
				mysql_query("COMMIT");  
				echo "1**".$id;
			}
	...
	else if ($operation==2)   // Delete Here
	{
		...
		$sub_rlz_id = return_field_value("invoice_bill_id","com_export_proceed_realization","invoice_bill_id=".$id." and is_invoice_bill=1 and status_active=1 and is_deleted=0","invoice_bill_id");
		if($sub_rlz_id>0)
		{
		...
		else
		{
			if($id>0)
			{
				...
				$upsubMst=sql_update("com_export_doc_submission_mst",$update_field_arr,$update_data_arr,"id",$id,1);
			...
			if($db_type==0)
			{
				if($upsubMst && $upsubDtlsID && $upsubTransid && $update_buyer_convart)
				{
					mysql_query("COMMIT");  
					echo "2**".$id;
				}
			...
			{
				if($upsubDtlsID && $upsubTransid && $update_buyer_convart)
				{
					oci_commit($con);  
					echo "2**".$id;
				}
            ...
            <tbody>
                <tr>                    
                    ...
                    <td align="center">
                        ...
                        //echo create_drop_down( "cbo_buyer_name", 180, "select buy.id, buy.buyer_name from lib_buyer buy where buy.status_active =1 and buy.is_deleted=0 $buyer_cond order by buy.buyer_name","id,buyer_name", 1, "Select", $buyer_name, "",0 );
                        echo create_drop_down( "cbo_buyer_name", 180, "select a.id,a.buyer_name from lib_buyer a, lib_buyer_party_type b where a.id=b.buyer_id and b.party_type in(1) and a.status_active=1 group by a.id, a.buyer_name order by a.buyer_name","id,buyer_name", 1, "Select", $buyer_name, "",0 );
                        ?>
                    </td>   
                    <td align="center">
                    ...
                    <td align="center">
                        <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $company_name; ?>+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('cbo_year_selection').value, 'create_system_id_search_list_view', 'search_div', 'export_doc_sub_entry_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />				
                    </td>
            </tr>
        	<tr>                  
...
if($action=="create_system_id_search_list_view")
{
	...
	if($db_type==0)
	{
		$sql ="select a.id,a.bank_ref_no,a.submit_date,a.submit_to,a.lien_bank,a.submit_type, a.buyer_id, group_concat(b.is_lc) as is_lc_string, group_concat(b.invoice_id) as invoice_id_string, group_concat(b.lc_sc_id) as lc_sc_id_string , group_concat(b.id) as submission_dtls_id, a.is_posted_account, a.import_btb
			from com_export_doc_submission_mst a, com_export_doc_submission_invo b
			where a.id=b.doc_submission_mst_id and a.status_active=1 and a.entry_form=40 $sql_cond $invoice_idsCond group by a.id,a.bank_ref_no,a.submit_date, a.submit_to, a.lien_bank, a.submit_type,a.buyer_id, a.is_posted_account, a.import_btb";
	}
	else
	{
...
{  
	if($db_type==0)
	{
		$sql = "select a.id,a.company_id, a.buyer_id,a.submit_date, a.submit_to,a.bank_ref_no, a.bank_ref_date, a.days_to_realize,a.possible_reali_date, a.courier_receipt_no,a.courier_company,a.courier_date, a.bnk_to_bnk_cour_no, a.bnk_to_bnk_cour_dt, a.lien_bank,a.lc_currency,a.pay_term,a.submit_type,a.negotiation_date,a.remarks,group_concat(b.id) as inv_id, group_concat(b.is_lc) as is_lc_string, group_concat(b.invoice_id) as invoice_id_string, group_concat(b.lc_sc_id) as lc_sc_id_string,a.import_btb, a.issue_bank_dtls 
		from com_export_doc_submission_mst a, com_export_doc_submission_invo b
	...
	foreach($res as $row)
	{
		...
		if($db_type==2) $row[csf("invoice_id_string")] = $row[csf("invoice_id_string")]->load();
		if($db_type==2) $row[csf("lc_sc_id_string")] = $row[csf("lc_sc_id_string")]->load();
		if($db_type==2) $row[csf("inv_id")] = $row[csf("inv_id")]->load();*/
		if($row[csf("import_btb")] == 1)
		{
			$company_arr= return_library_array( "select id, company_name from  lib_company",'id','company_name');
		...
		//echo $expisLC.test.$all_lc_sc_id;die;
		...
		//invoice list view -----------------------------------------------------------------------//
		//start -----------------------------------------------------------------------//
		$invSQL="select a.id,a.doc_submission_mst_id,a.invoice_id,a.submission_dtls_id,a.is_lc,a.lc_sc_id,b.bl_no,b.bl_date,a.invoice_date,a.net_invo_value,a.all_order_no,b.invoice_no,b.id as dtls_id,b.import_btb from com_export_doc_submission_invo a, com_export_invoice_ship_mst b where a.status_active=1 and a.is_deleted=0 and a.invoice_id=b.id and a.doc_submission_mst_id=".$row[csf("id")]."";
 		// echo $invSQL;die;
		...
		{ 
 			//ls or sc number
			$invoice_id.=$invRow[csf("invoice_id")].",";
			...
			$item_category_id = 0;
			if($invRow[csf("is_lc")] == 1 && $invRow[csf("import_btb")] == 1)
			{
				$item_category_id = return_field_value("export_item_category","com_export_lc","id=".$invRow[csf("lc_sc_id")].""); 
			}
			if($invRow[csf("import_btb")] == 1 && $item_category_id == 2)
			{
				$sales_order_arr =  sql_select("select work_order_no from com_export_lc_order_info where com_export_lc_id =". $invRow[csf("lc_sc_id")]);
			...
			$invoice_tr .= "<tr>".
								"<td align=\"center\">".$i."</td>".
								"<td><input type=\"text\" id=\"txt_invoice_no$i\" name=\"txt_invoice_no[]\" class=\"text_boxes\" style=\"width:150px\" value=\"".$invRow[csf("invoice_no")]."\" /><input type=\"hidden\" id=\"hidden_invoice_id$i\" value=\"".$invRow[csf("invoice_id")]."\" /><input type=\"hidden\" id=\"hidden_sub_dtls_id$i\" value=\"".$invRow[csf("submission_dtls_id")]."\" /></td>".
								...
								"<td><input type=\"text\" id=\"txt_bl_date$i\" name=\"txt_bl_date[]\" class=\"text_boxes\" style=\"width:100px\" value=\"".change_date_format($invRow[csf("bl_date")])."\" /></td>".
			...
			$po_numbers=chop($po_numbers_arr[$invRow[csf("invoice_id")]],", ");
			$invoice_tr .= '<tr><td align="center">'.$i.'</td><td id="txt_invoice_no'.$i.'" title="'.$invRow[csf("invoice_id")].'" subdtls="'.$invRow[csf("submission_dtls_id")].'">'.$invRow[csf("invoice_no")].'</td><td id="txt_lcsc_no'.$i.'" title="'.$invRow[csf("lc_sc_id")].'" islc="'.$invRow[csf("is_lc")].'">'.$lc_sc_no.'</td><td id="txt_bl_no'.$i.'">'.$invRow[csf("bl_no")].'</td><td align="center" id="txt_bl_date'.$i.'">'.change_date_format($invRow[csf("bl_date")]).'</td><td align="center" id="txt_invoice_date'.$i.'">'.change_date_format($invRow[csf("invoice_date")]).'</td><td align="right" id="txt_net_invo_value'.$i.'">'.$invRow[csf("net_invo_value")].'</td><td id="txt_po_numbers'.$i.'" title="'.$invRow[csf("all_order_no")].'" style="word-break:break-all">&nbsp;'.$po_numbers.'</td></tr>';
		...
		$invSQL =  "select a.id,a.doc_submission_mst_id,a.acc_head,a.acc_loan,a.loan_acc_no,a.dom_curr,a.conver_rate,a.lc_sc_curr 
					from com_export_doc_sub_trans a, com_export_doc_submission_mst b 
					where a.doc_submission_mst_id=b.id and a.doc_submission_mst_id=".$row[csf("id")]." and a.status_active=1 and a.is_deleted=0";
 		//echo $invSQL;die;
		$resArray=sql_select($invSQL);
		//create_drop_down( "cbo_account_head_".$rowNo, 200, $commercial_head,"", 1, "-- Select --", $invRow[csf("acc_head")], "get_php_form_data(this.value+\'**\'+".$rowNo."+\'**\'+document.getElementById(\'cbo_company_name\').value+\'**\'+document.getElementById(\'cbo_lien_bank\').value, \'populate_acc_loan_no_data\', \'requires/export_doc_sub_entry_controller\' )",0, "" )
...
if($action=="bank_submit_letter")
{
	...
	$msg=str_replace("'","",$msg);
	$bank_ref=str_replace("'","",$txt_bank_ref);
	$txt_submit_date=str_replace("'","",$txt_submit_date);
	$cbo_lien_bank= str_replace("'","",$cbo_lien_bank);
	$cbo_pay_term= str_replace("'","",$cbo_pay_term);
	$cbo_buyer_name=str_replace("'","",$cbo_buyer_name);
	            	...
	            	<table cellpadding="0" align="left" cellspacing="0" border="1" width="650" style="font-size:13px;">
						...
						foreach($result as $row)
						{
	                    	...
	                    	<tr>
	                            ...
	                            <td width="40" valign="top" align="right"><div style="width:40px;; word-wrap:break-word;"><? echo number_format($row[csf("invoice_quantity")]); $tot_qty+=$row[csf("invoice_quantity")];?></div></td>
	...
	if($type==9) // Bill of Exchange 2
	{
		$company_library=return_library_array( "select id, company_name from lib_company", "id", "company_name"  );
		//$company_address=sql_select("select plot_no,level_no,road_no,block_no,province,city,zip_code from lib_company where id=$company_id");
		$company_address=return_library_array("select COMPANY_ID,address from lib_location where COMPANY_ID=$company_id AND INSERT_DATE=(select min(insert_date) from lib_location where COMPANY_ID=$company_id)",'COMPANY_ID','address');
		 
		$country_arr=return_library_array( "select id,country_name from lib_country",'id','country_name');
		$lien_bank_name=return_library_array("select id,bank_name from lib_bank",'id','bank_name');
		
		$lien_bank_address=return_library_array("select id,address from lib_bank",'id','address');
		//$compny_address=return_library_array("select COMPANY_ID,ADDRESS from lib_location",'COMPANY_ID','ADDRESS');
		// echo "<pre>";
		// 	print_r($compny_address);
		// echo "</pre>";die;
		...
		foreach($result as $row)
		{
			...
			$sql1="select a.id,a.doc_submission_mst_id,a.invoice_id,a.submission_dtls_id,a.is_lc,a.lc_sc_id,b.bl_no,b.bl_date,a.invoice_date,a.net_invo_value,a.all_order_no,b.invoice_no,b.id as
			dtls_id,b.import_btb, c.bank_ref_no from com_export_doc_submission_invo a, com_export_invoice_ship_mst b, com_export_doc_submission_mst c where a.status_active=1 and a.is_deleted=0 and 
			a.invoice_id=b.id and c.bank_ref_no='$bank_ref' and c.company_id=$company_id and c.entry_form=40 and c.id= a.doc_submission_mst_id";
			$data_array=sql_select($sql1);
			
			$sql2="select distinct ITEM_NUMBER_ID  a from wo_po_color_size_breakdown a, wo_po_break_down b where b.id=83065 and a.JOB_NO_MST=b.JOB_NO_MST";
			$item_number_id=sql_select($sql2);

		
			// echo "<pre>";
			//   print_r($garments_item);
			//   print_r($item_number_id);
			// echo "</pre>";
			//echo $item_number_id[0]['ITEM_NUMBER_ID'];
		...
		<?
		$sales_contract_issue_bank_sql="select ISSUING_BANK from com_sales_contract where CONTRACT_NO='$lc_sc_no'";
		 
		$sales_contract_issue_bank= sql_select($sales_contract_issue_bank_sql);
		if($sales_contract_issue_bank != null){
			$issuing_bank=$sales_contract_issue_bank[0]['ISSUING_BANK'];
		}
	 ...
	 foreach($copy_no as $cid)
	 {
		?>
	  	...
	  	<div class="page-break"> 
	    	<table width="650" cellpadding="0" align="center" cellspacing="0" border="0" id="table_<? echo $cid;?>">
                <tr>
                    <td colspan="5" align="center" style="font-size:15px; font-weight:bold; font-family: Old English Text MT; color: yellowgreen;"><? echo strtoupper($company_library[$company_id]); ?></td>										
		        </tr>
				<tr>
					<!--<td colspan="5" align="center" style="font-size:15px; font-weight:bold; font-family: Old English Text MT; color: black;"><? //echo strtoupper($company_address[0]['CITY']); ?></td> -->
					<td colspan="5" align="center" style="font-size:15px; font-weight:bold; font-family: Old English Text MT; color: black;"><? echo strtoupper($company_address[$company_id]); ?></td>
				</tr>
				<tr>
               		<td colspan="5" align="center">&nbsp;</td>
           		</tr>
				<tr>
					<td colspan="5" align="center" style="font-size:15px; font-weight:bold; font-family: Old English Text MT; color: grey;"><span style="border:1px dotted grey;background-color:rgb(214, 214, 214);padding:5px 30px;border-radius:35px ; "><? echo "BILL OF EXCHANGE "; ?></span></td>
				</tr>
				<tr>
               		<td colspan="5" align="center">&nbsp;</td>
           		</tr>		         
				...
				<tr border="0" style="font-size: 13px;">
					...
					<td width="42%" colspan="5" style="border: 2px solid black;height: 30px; word-break: break-all;" valign="center">&nbsp;INVOICE NO <strong><? echo strtoupper( $data_array[0]['INVOICE_NO'])." ON ".strtoupper($data_array[0]['INVOICE_DATE']); ?></strong></td> 
           		</tr>
				<tr>
               		<td colspan="5" align="center">&nbsp;</td>
           		</tr>
				<tr border="0" style="font-size: 13px;">
					...
					<td width="42%" colspan="5" style="border: 2px solid black;height: 30px; word-break: break-all;" valign="center">&nbsp;HBL NO: <strong> <? echo strtoupper($data_array[0]['BL_NO'])." DATE ".strtoupper($data_array[0]['BL_DATE']); ?></strong></td> 
           		</tr>
				<tr>
               		<td colspan="5" align="center">&nbsp;</td>
           		</tr>
				<tr border="0" style="font-size: 13px;">
					...
					<td width="25%" colspan="3" style="border: 0px dotted;height: 30px; word-break: break-all;" valign="top">&nbsp;<? echo "EXCHANGE FOR: <strong>US$ ".strtoupper($data_array[0]['NET_INVO_VALUE'])."</strong>"; ?></td> 
					<td width="25%" colspan="3" style="border: 0px dotted;height: 30px; word-break: break-all;text-align:right;" valign="top">&nbsp;<span><? echo "DATE: <strong>".strtoupper($txt_submit_date)."</strong>"; ?></span></td> 
           		</tr>

				<tr border="0" style="font-size: 12px;">					 
					<td colspan="5">THIS FRIST OF EXCHANGE (SECOND OF THE SAME TENOR AND DATE BEING UNPAID) PAY TO <strong><? echo strtoupper($lien_bank_name[$cbo_lien_bank]).", ".strtoupper($lien_bank_address[$cbo_lien_bank])." ";?></strong>  UNDER PAYMENT TERM LC <strong><? if($cbo_pay_term==1)echo "AT SIGHT";else echo "USANCE";?></strong>.</td>
				</tr>
				<tr>
               		<td colspan="5" align="center" style="font-size:23px;color:yellowgreen;padding-top:10px;padding-bottom:8px;"><? echo $cid;?></td>
           		</tr>
				<tr style="font-size: 11px;padding:2px;">
					<td colspan="5" style="font-size: 11px;padding-top:4px">THE SAME OF USD: <span style="border: 2px solid black;padding:3px;"><strong><? echo strtoupper(number_to_words($data_array[0]['NET_INVO_VALUE'])); ?></strong></span></td>
				</tr>
				<tr style="font-size: 11px;padding:10px">
					<td colspan="5" style="font-size: 11px;padding-top:10px">VALUE RECEIVED FOR SHIPMENT OF: <strong><? echo strtoupper($garments_item [$item_number_id[0]['A']]); ?></strong></td>
				</tr>
				<tr style="font-size: 11px;padding:10px">
					<td colspan="5" style="font-size: 11px;padding-top:4px">DRAWN UNDER LC NO: <strong><? echo strtoupper($lc_sc_no); ?></strong>, DATE: <strong><? echo $lc_sc_date;?></strong>, APPLICANT: <strong><? echo strtoupper($buyer_name_arr[$cbo_buyer_name]);?></strong></td>
				</tr>
				<tr style="font-size: 11px;"><!--plot_no,level_no,road_no,block_no,province,city,zip_code -->
					<td colspan="5" style="font-size: 11px;padding-top:10px;">TO:

					<br><div style="border:2px solid black;padding:5px;"><strong ><? echo strtoupper($lien_bank_name[$issuing_bank]);?></strong> <br><strong><? echo strtoupper($lien_bank_address[$issuing_bank]);?></strong></div></td>
				</tr>
				<tr style="font-size: 11px;">
					<td colspan="5" style="font-size: 11px;padding-top:6px">FROM:<br><div style="border:2px solid black;padding:5px;"><strong><? echo strtoupper($company_library[$company_id]); ?></strong><br>
					<strong><? echo  strtoupper($company_address[$company_id]); ?></strong></div></td>
				</tr>

				<tr style="font-size: 11px;">
					<td colspan="5" style="font-size: 11px;padding-top:160px;"><span style="text-align:right;border-top:1px dotted black;float:right;">AUTHORIZED SIGNATURE</span></td>
				</tr>
				<tr>
               		<td colspan="5" align="center" style="padding-top:50px;">&nbsp;</td>
           		</tr>
...
if($action=="acc_popup")
{
	...
	function fnc_trims_acc( operation )
	{
		...
		http.open("POST","export_doc_sub_entry_controller.php",true);
