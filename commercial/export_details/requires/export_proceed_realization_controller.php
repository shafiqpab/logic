...
if($action=="populate_acc_loan_no_data")
{
	...
	if(count($nameArray)>1){
		...
		foreach($nameArray as $row)
		{
			$account_no.=$row[csf("account_no")].'_';
		}
		$account_no=chop($account_no,'_');
		echo "$('#acLoanNo_".$rowID."').attr('readonly','readonly').attr('placeholder','Double Click to Show').attr('onDblClick','fnc_account_no(".$rowID.",\'".$account_no."\')');\n";
	}else{
		foreach($nameArray as $row)
		{
			echo "$('#acLoanNo_".$rowID."').attr('readonly','readonly');\n";
	...
	//echo $is_invoice_bill."=".$ac_head_id;die;
	if($is_invoice_bill==2 && $ac_head_id==177)
	{
		...
		where b.lc_sc_id = c.id and b.is_lc=2 and c.pay_term = 3 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.lc_sc_id=$lc_id";
		//echo $sql_inv;die;
		$sql_inv_result=sql_select($sql_inv);
		$sc_inv_id=$sql_inv_result[0]["INV_ID"];
		$prev_cash_ad_inv_amt=0;
		if($sc_inv_id!="")
		{
			$cash_adv_inv="select sum(b.DOCUMENT_CURRENCY) as DOCUMENT_CURRENCY from com_export_proceed_realization a, com_export_proceed_rlzn_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.IS_INVOICE_BILL=2 and b.ACCOUNT_HEAD=177 and a.INVOICE_BILL_ID in($sc_inv_id) ";
			$cash_adv_inv_result=sql_select($cash_adv_inv);
			//echo $cash_adv_inv;die;
			$prev_cash_ad_inv_amt=$cash_adv_inv_result[0]["DOCUMENT_CURRENCY"];
		}
		
		$cash_adv_sc="select sum(b.DOCUMENT_CURRENCY) as DOCUMENT_CURRENCY from com_export_proceed_realization a, com_export_proceed_rlzn_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.IS_INVOICE_BILL=3 and a.INVOICE_BILL_ID=$lc_id ";
		$cash_adv_sc_result=sql_select($cash_adv_sc);
		$cash_ad_amt=$cash_adv_sc_result[0]["DOCUMENT_CURRENCY"]-$prev_cash_ad_inv_amt;
		echo "$('#distributionDocumentCurrency_".$rowID."').val(".$cash_ad_amt.");\n";
	}
	
	exit();
}

if($action=="account_no_popup")
{
	//echo load_html_head_contents("Account No Popup", "../../../", 1, 1,'','1','');
	echo load_html_head_contents("Account No Popup", "../../../", 1, 1,'','','');

	extract($_REQUEST);
	$account_no=explode('_',$account_no);
	?>
	    <script>
			function js_set_value(str)
			{
				$("#hdn_acc_info").val(str); 
				parent.emailwindow.hide();
			}  
		</script>
		<input type="hidden" id="hdn_acc_info" />
		<table width="250"  cellpadding="0" cellspacing="0" border="1" class="rpt_table" rules="all">
			<thead>
				<tr>
					<th width="50">SL</th>
			...
			<tbody>
				...
				foreach($account_no as $row)
				{
					...
					?>
						<tr bgcolor="<? echo $bgcolor;?>" style="cursor:pointer;" onClick="js_set_value('<? echo $row;?>')">
...
if($action=="commercial_head_popup")
{
	...
	extract($_REQUEST);
	//echo $head_from."==".$is_invoice_bill;die;
	?>
	<script>
    ...
    <div align="center" style="width:360px;">
        ...
        <div style="width:360px; max-height:350px; overflow-y:scroll">
     	<table width="340" cellpadding="0" cellspacing="0" border="1" class="rpt_table" rules="all" id="list_view">
            ...
            <?
            $i=1;
			if($head_from==1 || $is_invoice_bill==1)
			{
            ...
            <table cellpadding="0" cellspacing="0" width="630" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td><input name="txt_date_to" id="txt_date_to" class="datepicker" style="width:50px" placeholder="To Date"></td>
                     <td>
                        <input type="button" id="search_button" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_buyer_name').value+'**'+<? echo $beneficiary_name; ?>+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('import_btb').value+'**'+document.getElementById('txt_date_from').value+'**'+document.getElementById('txt_date_to').value, 'invoice_bill_search_list_view', 'search_div', 'export_proceed_realization_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />
                     </td>
                </tr>
           </table>
...
if($action=="invoice_bill_search_list_view")
{
	...
	if($db_type==0){
		...
		if ($data[5]!="" &&  $data[6]!="") $date_con  = "and d.received_date  between '".change_date_format($data[5], "yyyy-mm-dd", "-")."' and '".change_date_format($data[6], "yyyy-mm-dd", "-")."'"; else $date_con ="";
	}
	else if($db_type==2){
		
		if ($data[5]!="" &&  $data[6]!="") $date_con  = "and d.received_date  between '".change_date_format($data[5], "yyyy-mm-dd", "-",1)."' and '".change_date_format($data[6], "yyyy-mm-dd", "-",1)."'"; else $date_con ="";
	}
	$prev_realized=sql_select("select invoice_bill_id, is_invoice_bill from com_export_proceed_realization where status_active=1 and is_deleted=0");
	$prev_rlz_data=array();
	foreach($prev_realized as $row)
	{
		$prev_rlz_data[$row[csf("is_invoice_bill")]][$row[csf("invoice_bill_id")]]=$row[csf("invoice_bill_id")];
	}
	...
	if($db_type==0)
	{
		...
		if($search_by==1 || $search_by==2)
		{
			$sql= " select a.id, a.company_id, a.buyer_id, a.bank_ref_no, a.bank_ref_date, a.submit_type, group_concat(distinct(b.invoice_id)) as invoice_id, a.import_btb, c.is_lc, c.lc_sc_id, 1 as type, a.lien_bank
			...
			group by a.id, a.company_id, a.buyer_id, a.bank_ref_no, a.bank_ref_date, a.submit_type,a.import_btb,a.lien_bank, c.is_lc, c.lc_sc_id
	...
	else
	{
		...
		if($search_by==1 || $search_by==2)
		{
			$sql= " select a.id, a.company_id, a.buyer_id, a.bank_ref_no, a.bank_ref_date, a.submit_type,rtrim(xmlagg(xmlelement(e,b.invoice_id,',').extract('//text()') order by b.invoice_id).GetClobVal(),',') as invoice_id, a.import_btb, 1 as type,a.lien_bank, b.is_lc, b.lc_sc_id,b.net_invo_value,d.received_date
			...
			where a.id=b.doc_submission_mst_id and c.id = b.invoice_id and d.invoice_bill_id=a.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.company_id=$company_id and a.buyer_id like '$buyer_id' and a.entry_form=40 $search_cond $import_btb_cond  $date_con 
			group by a.id, a.company_id, a.buyer_id, a.bank_ref_no, a.bank_ref_date, a.submit_type,a.import_btb,a.lien_bank, b.is_lc, b.lc_sc_id,b.net_invo_value,d.received_date 
			order by a.bank_ref_no";
		}
		else
		{
			...
			from com_export_invoice_ship_mst c, com_sales_contract d 
			where c.lc_sc_id = d.id and d.pay_term = 3 and c.status_active=1 and c.is_deleted=0 and c.benificiary_id=$company_id and c.buyer_id like '$buyer_id' $search_cond  $import_btb_cond $date_con  order by c.invoice_no";
		}
	}
	...
	<table width="700" cellpadding="0" cellspacing="0" border="1" class="rpt_table" rules="all">
        <thead>
            ...
            <th width="100">Negotiated Amnt</th>
    ...
    <div style="width:720px; max-height:270px; overflow-y:scroll">
		...
		<?
            ...
            foreach($data_array as $row)
            {
				...
				if($print_cond==1 && $prev_rlz_data[$row[csf('type')]][$row[csf("id")]]!=0)
				{
					$lc_sc_no=implode(",",array_unique(explode(",",$lc_sc_no)));
					?>
                    <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer;" id="search<? echo $i;?>" onClick="js_set_value(<? echo $row[csf('id')].','.$row[csf('type')].','.$row[csf('lien_bank')].','.$row[csf('lc_sc_id')].','.$row[csf('is_lc')]; ?>);" >
                        ...
                        <td align="right" width="100"><? echo number_format($negotiated_amnt,2); ?>&nbsp;</td>
						<td align="right" width="100"><? echo $row[csf('net_invo_value')]; ?>&nbsp;</td>
                    </tr>
					<?
                    $i++;
				}
...
if($action=="populate_data_from_invoice_bill")
{
	...
	$lc_id=$data[3];
	$lc_type=$data[4];
	//echo $lc_id."__".$lc_type;die;
	if($is_invoiceBill==1)
	{
		if($db_type==0)
		{
			$sql = "select a.bank_ref_no, a.bank_ref_date, a.buyer_id, a.submit_type, group_concat(distinct(b.invoice_id)) as invoice_id, sum(b.net_invo_value) as bill_amnt,a.import_btb,a.lien_bank,b.is_lc,b.lc_sc_id from com_export_doc_submission_mst a, com_export_doc_submission_invo b where a.id=b.doc_submission_mst_id and a.id=$invoice_bill_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id, a.bank_ref_no, a.bank_ref_date, a.buyer_id, a.submit_type,a.import_btb,a.lien_bank,b.is_lc,b.lc_sc_id";
		}
		else
		{
			$sql = "select a.bank_ref_no, a.bank_ref_date, a.buyer_id, a.submit_type, rtrim(xmlagg(xmlelement(e,b.invoice_id,',').extract('//text()') order by b.invoice_id).GetClobVal(),',') as invoice_id, sum(b.net_invo_value) as bill_amnt,a.import_btb,a.lien_bank,b.is_lc,b.lc_sc_id from com_export_doc_submission_mst a, com_export_doc_submission_invo b where a.id=b.doc_submission_mst_id and a.id=$invoice_bill_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id, a.bank_ref_no, a.bank_ref_date, a.buyer_id,a.lien_bank, a.submit_type,a.import_btb,b.is_lc,b.lc_sc_id";
		}
	}
	else
	{
	...
	{
		$currency='';
		if($db_type==2 && $is_invoiceBill==1) $row[csf('invoice_id')] = $row[csf('invoice_id')]->load();
		if($is_invoiceBill==1)
		{
            ...
            <table cellpadding="0" cellspacing="0" width="750" class="rpt_table">
                ...
                <tr class="general">
                    <td>
						...
						{
						 	echo create_drop_down( "cbo_buyer_name", 152, "select buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b,lib_buyer_party_type c where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company=$beneficiary_name $buyer_cond and c.buyer_id=buy.id and c.party_type in (1,3,21,90) group by buy.id, buy.buyer_name order by buy.buyer_name","id,buyer_name", 1, "-- All Buyer --", $selected, "",0 );
                    ...
                    <td>
                     ...
                     <td>
                        <input type="button" id="search_button" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_buyer_name').value+'**'+<? echo $beneficiary_name; ?>+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('txt_date_from').value+'**'+document.getElementById('txt_date_to').value, 'proceed_realization_search_list_view', 'search_div', 'export_proceed_realization_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" />
                     </td>
                </tr>
                <tr>
...
if($action=="proceed_realization_search_list_view")
{
	...
	*/
	...
	if($search_by==1 || $search_by==2){
		...
		where a.invoice_bill_id=e.id and a.status_active=1 and a.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and a.is_partial=0 and a.benificiary_id=$company_id $byer_cond $date_cond $search_cond 
			...
			?>
				<tr class="general" id="deduction_<? echo $i; ?>">
					<td>
                        ...
                        <input type="text" name="cbodeductionHead[]" id="cbodeductionHead_<?=$i;?>" class="text_boxes" style="width:170px;" onFocus="add_auto_complete( <?=$i;?> )"  onDblClick="fn_commercial_head_display(<?=$i;?>,1,'cbodeductionHead')" onBlur="setTimeout(fn_value_check,1000,<?=$i;?>,1,'cbodeductionHead');clearTimeout(1500);" placeholder="Browse Or Write" acHeadVal="<?= $row[csf('account_head')];?>" value="<?= $commercial_head[$row[csf('account_head')]];?>" />
					</td>
					<td>
					...
					<td>
						<input type="text" name="deductionDomesticCurrency[]" id="deductionDomesticCurrency_<? echo $i; ?>" class="text_boxes_numeric" style="width:177px;" onKeyUp="calculate(<? echo $i; ?>,'DomesticCurrency_','tbl_deduction','deduction')" value="<? echo number_format($row[csf('domestic_currency')],4,'.',''); ?>" />
					</td>
					<td width="65">
		...
		?>
        	<tr class="general" id='deduction_1' align="center">
                <td>
                    ...
                    <input type="text" name="cbodeductionHead[]" id="cbodeductionHead_1" class="text_boxes" style="width:170px;" onFocus="add_auto_complete( 1 )"  onBlur="setTimeout(fn_value_check,1000,1,1,'cbodeductionHead');clearTimeout(1500);" onDblClick="fn_commercial_head_display(1,1,'cbodeductionHead')" placeholder="Browse Or Write" />
                </td>
                <td>
	...
	else
	{
		if($num_row>0)
		{
			...
			?>
				<tr class="general" id="distribution_<? echo $i; ?>">
					<td>
                    	<!--<select class="combo_boxes" id="cbodistributionHead_<?// echo $i; ?>" name="cbodistributionHead[]" onChange="get_php_form_data( this.value+'**<?// echo $i; ?>**'+$('#cbo_beneficiary_name').val()+'**'+$('#is_invoice_bill_lien_bank').val(), 'populate_acc_loan_no_data', 'requires/export_proceed_realization_controller' );check_duplication(<?// echo $i; ?>)" <?// echo $disable; ?>>
                            <option value="0">-- Select Account Head --</option>
                                <?
                                    /*foreach($commercial_head as $key=>$value)
                                    {
                        ...
                        <input type="text" name="cbodistributionHead[]" id="cbodistributionHead_<?=$i;?>" class="text_boxes" style="width:170px;" onFocus="add_auto_complete( <?=$i;?> )"  onDblClick="fn_commercial_head_display(<?=$i;?>,2,'cbodistributionHead')"  onBlur="setTimeout(fn_value_check,1000,<?=$i;?>,2,'cbodistributionHead');clearTimeout(1500);"  placeholder="Browse Or Write" acHeadVal="<?= $row[csf('account_head')];?>" value="<?= $commercial_head[$row[csf('account_head')]];?>" />
					</td>
					<td>
					...
					<td>
						<input type="text" name="distributionDomesticCurrency[]" id="distributionDomesticCurrency_<? echo $i; ?>" class="text_boxes_numeric" style="width:130px;" onKeyUp="calculate(<? echo $i; ?>,'DomesticCurrency_','tbl_distribution','distribution')" value="<? echo number_format($row[csf('domestic_currency')],4,'.',''); ?>" <? echo $disable; ?>/>
					</td>
					<td width="65">
		...
		?>
            <tr class="general" id='distribution_1' align="center">
                <td>
					<!--<select class="combo_boxes" id="cbodistributionHead_1" name="cbodistributionHead[]" onChange="get_php_form_data( this.value+'**1**'+$('#cbo_beneficiary_name').val()+'**'+$('#is_invoice_bill_lien_bank').val(), 'populate_acc_loan_no_data', 'requires/export_proceed_realization_controller' );check_duplication(1)">
                        <option value="0">-- Select Account Head --</option>
                            <?
                                /*foreach($commercial_head as $key=>$value)
                                {
                    ...
                    <input type="text" name="cbodistributionHead[]" id="cbodistributionHead_1" class="text_boxes" style="width:170px;" onFocus="add_auto_complete( 1 )"  onBlur="setTimeout(fn_value_check,1000,1,2,'cbodistributionHead');clearTimeout(1500);"  onDblClick="fn_commercial_head_display(1,2,'cbodistributionHead')"  placeholder="Browse Or Write" />
                </td>
                <td>
...
if ($action=="save_update_delete")
{
	...
	for($j=1;$j<=$distribution_tot_row;$j++)
	{
		...
		//echo str_replace("'","",$$account_head)."=".str_replace("'","",$is_invoice_bill);die;
		if(str_replace("'","",$$account_head)==177 && str_replace("'","",$is_invoice_bill)==2 )
		{
			$sql_inv = "select listagg(cast(b.id as varchar(4000)),',') within group (order by b.id) as INV_ID
			from com_export_invoice_ship_mst b, com_sales_contract c 
			where b.lc_sc_id = c.id and b.is_lc=2 and c.pay_term = 3 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.lc_sc_id=$lc_id";
			...
			if($sc_inv_id!="")
			{
				$up_id=str_replace("'","",$update_id);
				$up_cond="";
				if($up_id) $up_cond=" and a.id <> $up_id";
				$cash_adv_inv="select sum(b.DOCUMENT_CURRENCY) as DOCUMENT_CURRENCY from com_export_proceed_realization a, com_export_proceed_rlzn_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.IS_INVOICE_BILL=2 and b.ACCOUNT_HEAD=177 and a.INVOICE_BILL_ID in($sc_inv_id) $up_cond";
				$cash_adv_inv_result=sql_select($cash_adv_inv);
				//echo $cash_adv_inv;die;
				$prev_cash_ad_inv_amt=$cash_adv_inv_result[0]["DOCUMENT_CURRENCY"];
			}
			...
			if($$document_currency>$cash_ad_amt)
			{
				echo "20** Cash In Advance Value Not Allow More then Advance Payment Receive".$$document_currency."=".$cash_ad_amt;die;
			}
		}
	}
	//echo "10**".$sql_inv;die;
	if ($operation==0)  // Insert Here
	{
		...
		$field_array="id, invoice_bill_id, is_invoice_bill, buyer_id, benificiary_id, received_date, remarks, lib_distribution_string, inserted_by, insert_date";

		$data_array="(".$id.",".$invoice_bill_id.",".$is_invoice_bill.",".$cbo_buyer_name.",".$cbo_beneficiary_name.",".$txt_received_date.",".$txt_remarks.",".$hdn_variable_distribution.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
	...
	if($data_ref[0]!="")
	{
		...
		foreach($ac_head_arr as $val)
		{
            ...
            <tr class="general" id='distribution_<?=$i?>' align="center">
                <td> 
                    <input type="text" name="cbodistributionHead[]" id="cbodistributionHead_<?=$i?>" class="text_boxes" style="width:170px;" onFocus="add_auto_complete( <?=$i?> )"  onBlur="setTimeout(fn_value_check,1000,<?=$i?>,2,'cbodistributionHead');clearTimeout(1500);"  onDblClick="fn_commercial_head_display(<?=$i?>,2,'cbodistributionHead')"  placeholder="Browse Or Write" acHeadVal="<?= $ref_val[0];?>" value="<?= $commercial_head[$ref_val[0]];?>" />
                </td>
                <td>
...
if ($action=="realization_report_print2")
{
	//echo load_html_head_contents("Report Info", "../../../", 1, 1,'','','');	
	extract($_REQUEST);

	if($db_type==0){$sql_clm=", group_concat(distinct(d.INVOICE_NO)) as INVOICE_NO";}
	else{ $sql_clm=", listagg(cast(d.INVOICE_NO as varchar(4000)),',') within group(order by d.id) as INVOICE_NO";}

	$mst_sql="SELECT a.id as rlz_id, a.benificiary_id, a.received_date,a.REMARKS,a.INSERTED_BY, b.bank_ref_no, b.submit_date, b.bank_ref_date, b.buyer_id, c.is_lc, C.lc_sc_id, sum(c.net_invo_value) as bill_value $sql_clm
	from com_export_proceed_realization a, com_export_doc_submission_mst b , com_export_doc_submission_invo c, COM_EXPORT_INVOICE_SHIP_MST d
	where a.invoice_bill_id=b.id and b.id=c.doc_submission_mst_id and b.entry_form=40 and c.INVOICE_ID=d.id and a.status_active=1 and b.status_active=1 and c.status_active=1 and d.status_active=1 and a.id=$data
	group by a.id,a.benificiary_id, a.received_date,a.REMARKS,a.INSERTED_BY, b.bank_ref_no, b.submit_date, b.bank_ref_date, b.buyer_id, c.is_lc, c.lc_sc_id";
	// echo $mst_sql;
	$mst_sql_result = sql_select($mst_sql);
	$rlz_id=$mst_sql_result[0][csf("rlz_id")];
	$received_date=$mst_sql_result[0][csf("received_date")];
	...
	$lc_sc_id=$mst_sql_result[0][csf("lc_sc_id")];
	$bill_value=$mst_sql_result[0][csf("bill_value")];
	$company_id=$mst_sql_result[0]["BENIFICIARY_ID"];
	$invoice_no=$mst_sql_result[0]["INVOICE_NO"];
	$remarks=$mst_sql_result[0]["REMARKS"];
	$inserted_by=$mst_sql_result[0]["INSERTED_BY"];

	$buyer_arr=return_library_array( "select id, SHORT_NAME from lib_buyer",'id','SHORT_NAME');
	$bank_arr=return_library_array( "select id, bank_name from lib_bank",'id','bank_name');

	$nameArray=sql_select( "SELECT COMPANY_NAME,plot_no,level_no,road_no,block_no,country_id,province,city,zip_code,email,website from lib_company where id=$company_id");
	$adderess='';
	foreach ($nameArray as $company_add)
	{
		$country_name= $company_add["COMPANY_NAME"];
		if($company_add[csf('plot_no')]!=''){ $plot_no= $company_add[csf('plot_no')]; }
		if($company_add[csf('road_no')]!=''){ $road_no=$company_add[csf('road_no')]; }
		if($company_add[csf('block_no')]!=''){ $block_no= $company_add[csf('block_no')];}
		if($company_add[csf('city')]!=''){ $city= $company_add[csf('city')];}
		if($company_add[csf('zip_code')]!=''){ $zip_code= $company_add[csf('zip_code')]; }
		if($company_add[csf('country_id')]!=''){ $country= $country_arr[$company_add[csf('country_id')]]; }
	}
	if($plot_no){$adderess.=$plot_no;}
	if($road_no && $adderess){$adderess.=', '.$road_no;}else{$adderess.=$road_no;}
	if($block_no && $adderess){$adderess.=', '.$block_no;}else{$adderess.=$block_no;}
	if($city && $adderess){$adderess.=', '.$city;}else{$adderess.=$city;}
	if($zip_code && $adderess){$adderess.=', '.$zip_code;}else{$adderess.=$zip_code;}
	if($country && $adderess){$adderess.=', '.$country;}else{$country.=$zip_code;}
	$com_logo=sql_select("SELECT image_location  from common_photo_library  where master_tble_id='$company_id' and form_name='company_details' and is_deleted=0 and file_type=1");

	// $buyer_names=return_field_value("buyer_name","lib_buyer","id=$buyer_id","buyer_name");
	if($is_lc==1)
	{
		$lc_sc_no=return_field_value("export_lc_no","com_export_lc","id=$lc_sc_id","export_lc_no");
		$lein_bank_id=return_field_value("LIEN_BANK","com_export_lc","id=$lc_sc_id","LIEN_BANK");
	}
	else
	{
		$lc_sc_no=return_field_value("contract_no","com_sales_contract","id=$lc_sc_id","contract_no");
		$lein_bank_id=return_field_value("LIEN_BANK","com_sales_contract","id=$lc_sc_id","LIEN_BANK");
	}

	$dtls_sql_distribute="SELECT id, type, account_head, document_currency, conversion_rate, domestic_currency from com_export_proceed_rlzn_dtls where status_active=1 and mst_id=$data and type=1 order by type";
	//echo $dtls_sql_distribute;
	$dtls_sql_distribute_result = sql_select($dtls_sql_distribute);
	$i=1;$rate_con='';
	$total_ded=$total_domestic=$total_docu=0;
	$bedit_loop_arr=array();
	foreach($dtls_sql_distribute_result as $row)
	{
		if($i==1){$rate_con=$row[csf('conversion_rate')];$i++;}
		$bedit_loop_arr[$row[csf('account_head')]]['document_currency']+=$row[csf('document_currency')];
		$bedit_loop_arr[$row[csf('account_head')]]['domestic_currency']+=$row[csf('domestic_currency')];
		$total_domestic+=$row[csf('domestic_currency')];
		$total_document+=$row[csf('document_currency')];
	}
	
	$dtls_sql_deduct="SELECT id, type, account_head, ac_loan_no, document_currency, conversion_rate, domestic_currency, distribute_percent from com_export_proceed_rlzn_dtls where status_active=1 and mst_id=$data and type=0 order by type";
	//echo $dtls_sql_deduct;
	$dtls_sql_deduct_result = sql_select($dtls_sql_deduct);
	foreach($dtls_sql_deduct_result as $row)
	{
		$bedit_loop_arr[$row[csf('account_head')]]['document_currency']+=$row[csf('document_currency')];
		$bedit_loop_arr[$row[csf('account_head')]]['domestic_currency']+=$row[csf('domestic_currency')];
		$total_ded+=$row[csf('document_currency')];
		$total_domestic+=$row[csf('domestic_currency')];
		$total_document+=$row[csf('document_currency')];
	}
	...
	?>
	<!-- <style>
		#tbl_dtls thead tr th,#tbl_dtls tbody tr td,#tbl_dtls tfoot tr td{
    ...
    <table width="700" cellpadding="0" align="left" cellspacing="0" border="0">
        <tr>
            <td rowspan="2" width="150"><img src='../../<? echo $com_logo[0][csf('image_location')]; ?>' height='70' width='140' align="center" /></td>
            <td align="center" style="font-size:x-large;"><strong> <?=$country_name;?> </strong></td>
        </tr>
        <tr>
            <td align="center" style="font-size:large;"><?=$adderess;?> </td>
        </tr>   
		<tr><td colspan="2" height="20"></td></tr>        
    </table>
    <table width="700" cellpadding="0" align="left" cellspacing="0" border="0">     
		<tr><td colspan="3" align="center" style="font-size:large;"><strong><u>Journal Voucher</u></strong></td></tr>     
		<tr>
			<td width="150">J.V No :<?=date("M",strtotime($received_date))."-".$data;?></td>
			<td width="400"></td>
			<td width="150" >Date:<?=$received_date;?></td>
		</tr>     
		<tr>
			<td colspan="3"><strong><u>Narration</u></strong> : </td>
		</tr>     
		<tr>
			<td colspan="3">Being the amount received from export for  US$ <?=number_format($bill_value,2);?> @ tk. <?=$rate_con;?>,  Under Inv. # <?=$invoice_no;?> (<?=$buyer_arr[$buyer_id];?>) , ITT # <?=$remarks;?>, FDBC # <?=$bank_ref_no;?> against L/C # <?=$lc_sc_no;?>  amount transfer for the following A/C . Realised Amount US$ <?=number_format($bill_value-$total_ded,2);?></td>
		</tr>     
    </table>
    <table width="700" cellpadding="0" align="left" cellspacing="0" border="1" id="tbl_dtls">     
		<thead>
			<tr>
				<th colspan="3" style="border: 2px solid black;">DEBIT</th>
				<th colspan="2" style="border: 2px solid black;">CREDIT</th>
			</tr>
			<tr>
				<th width="150" style="border: 2px solid black;">ACCOUNT NAME</th>
				<th width="100" style="border: 2px solid black;">AMOUNT IN US$</th>
				<th width="100" style="border: 2px solid black;">CONVERTED IN BDTK.</th>
				<th width="150" style="border: 2px solid black;">ACCOUNT NAME</th>
				<th style="border: 2px solid black;">AMOUNT IN TK.</th>
			</tr>
		</thead>
		<tbody>
			<?
				$i=1;
				foreach($bedit_loop_arr as $key=>$val)
				{
					?>
						<tr>
							<td style="border-left: 2px solid black;border-right: 2px solid black;">
								<?
									if($key==5 || $key==6 || $key==87 || $key==129)
									{
										if($key==5){ echo "FBPAR A/C ( ".$bank_arr[$lein_bank_id]." )";}
										if($key==6){ echo "F/C A/C";}
										if($key==87){ echo "FREE FUND";}
										if($key==129){ echo "CENTRAL FUND (RMG SECTOR)";}
									}
									else
									{
										echo $commercial_head[$key];
									}
								?>
							</td>
							<td align="right" style="border-left: 2px solid black;border-right: 2px solid black;">$<?=$val["document_currency"];?></td>
							<td align="right" style="border-left: 2px solid black;border-right: 2px solid black;"><?=$val["domestic_currency"];?></td>
							<td style="border-left: 2px solid black;border-right: 2px solid black;">
								<?
									if($i==1)
									{
										?>
											EXPORT-<?=strtoupper($buyer_arr[$buyer_id]);?>
										<?
									}
								?>
							</td>
							<td align="right" style="border-left: 2px solid black;border-right: 2px solid black;">
								<?
									if($i==1){ echo number_format($total_domestic,2);}
								?>
							</td>
						</tr>
					<?
		...
		<tfoot>
			<tr>
				<td align="right" style="border: 2px solid black;">TOTAL</td>
				<td align="right" style="text-decoration-line: underline;text-decoration-style: double;border: 2px solid black;">$<?=number_format($total_document,2);?></td>
				<td align="right" style="text-decoration-line: underline;text-decoration-style: double;border: 2px solid black;"><?=number_format($total_domestic,2);?></td>
				<td align="right" style="border: 2px solid black;">TOTAL</td>
				<td align="right" style="text-decoration-line: underline;text-decoration-style: double;border: 2px solid black;"><?=number_format($total_domestic,2);?></td>
			</tr>		
		</tfoot>
	...
	<table width="700" cellpadding="0" align="left" cellspacing="0" border="1">
		<tr>
			<td width="250" align="right" style="border: 2px solid black;">Taka in words : </td>
			<td style="border: 2px solid black;"><? echo number_to_words(number_format($total_domestic,2,'.',''),'BDT','Paisa');?></td>
		</tr>
	</table>
    <?
	$template_id=1;
	echo signature_table(299,$company_id,"700px",$template_id,40,$inserted_by); 
	exit();
}
...
if($action=="print_button_variable_setting")
{
    ...
    $print_report_format=return_field_value("format_id","lib_report_template","template_name ='".$data."' and module_id=5 and report_id=249 and is_deleted=0 and status_active=1");
   	$printButton=explode(',',$print_report_format);

	foreach($printButton as $id){
		if($id==115)$buttonHtml.='<input type="button" class="formbutton" id="btn_rlz_rpt" value="Print" style="width:100px;" onClick="fn_print_report(1)" >&nbsp;';
		if($id==116)$buttonHtml.='<input type="button" class="formbutton" id="btn_rlz_rpt" value="Print 2" style="width:100px;" onClick="fn_print_report(2)" >&nbsp;';
	}
