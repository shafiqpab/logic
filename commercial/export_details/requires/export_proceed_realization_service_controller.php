					...
					<tr class="general">
						...
						<td>
							<?
								...
								//$arr=array(,3=>'Cash In Adv. Invoice No');
						...
						<td>
							<input type="button" id="search_button" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_buyer_name').value+'**'+<? echo $beneficiary_name; ?>+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('import_btb').value+'**'+<? echo $cbo_service_type; ?>+'**'+<? echo $cbo_within_group; ?>, 'invoice_bill_search_list_view', 'search_div', 'export_proceed_realization_service_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />
						</td>
					</tr>
			</table>
...
if($action=="invoice_bill_search_list_view")
{
	...
	// There are opportunity group filter by import btb
	if($service_type==10)
	{
		if($within_group==1){$import_btb_cond .= " and d.is_sales=0";}
		else{$import_btb_cond .= "  and d.is_sales=1";}
	}
	else
	{
		if($within_group==1){$import_btb_cond .= " and d.is_service=0";}
		else{$import_btb_cond .= " and d.is_service=1";}
	}
	...
	if($db_type==0)
	{
		$sql= "SELECT a.id, a.company_id, a.buyer_id, a.bank_ref_no, a.bank_ref_date, a.submit_type, group_concat(distinct(b.invoice_id)) as invoice_id, a.import_btb, 2 as type, a.lien_bank, sum(b.net_invo_value) as bill_value
		from com_export_doc_submission_mst a, com_export_doc_submission_invo b ,com_export_invoice_ship_mst c, com_export_invoice_ship_dtls d, com_export_lc e
		where a.id=b.doc_submission_mst_id and c.id = b.invoice_id and c.id=d.mst_id and b.lc_sc_id=e.id and b.is_lc=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and a.company_id=$company_id and e.export_item_category=$service_type and d.is_service=1 and a.buyer_id like '$buyer_id' and a.entry_form=40 $search_condand  $import_btb_cond
	...
	else
	{
		...
		from com_export_doc_submission_mst a, com_export_doc_submission_invo b ,com_export_invoice_ship_mst c, com_export_invoice_ship_dtls d, com_export_lc e
		where a.id=b.doc_submission_mst_id and c.id = b.invoice_id and c.id=d.mst_id and b.lc_sc_id=e.id and b.is_lc=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and a.company_id=$company_id and e.export_item_category=$service_type and a.buyer_id like '$buyer_id' and a.entry_form=40 $search_cond $import_btb_cond
    ...
    <div style="width:720px; max-height:270px; overflow-y:scroll">
		...
		<?
            ...
            foreach($data_array as $row)
            {
				...
				// echo $pending_value.'**'.$buyer_partial_rlz_permision.'<br>';
				//if($print_cond==1 && $prev_rlz_data[$row[csf('type')]][$row[csf("id")]]=="")
				// && $buyer_partial_rlz_permision==1
				if($print_cond==1 && $pending_value>0)
				{
...
if($action=="populate_data_from_invoice_bill")
{
 	...
 	foreach ($data_array as $row)
	{
		...
		if($realization_id!="")
		{
			$sql_realization="select service_type, benificiary_id, within_group, buyer_id, received_date, remarks, lib_distribution_string from com_export_proceed_realization where id=$realization_id";
			$data_array_realization=sql_select($sql_realization);

			echo "document.getElementById('cbo_service_type').value 	= '".$data_array_realization[0][csf("service_type")]."';\n";
			echo "document.getElementById('cbo_beneficiary_name').value 	= '".$data_array_realization[0][csf("benificiary_id")]."';\n";
			echo "document.getElementById('cbo_within_group').value 	= '".$data_array_realization[0][csf("within_group")]."';\n";
			...
			<legend>Enter search words</legend>
				<table cellpadding="0" cellspacing="0" width="750" class="rpt_table">
					...
					<tr class="general">
						...
						<td>
							<input type="button" id="search_button" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_buyer_name').value+'**'+<? echo $beneficiary_name; ?>+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('txt_date_from').value+'**'+document.getElementById('txt_date_to').value, 'proceed_realization_search_list_view', 'search_div', 'export_proceed_realization_service_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" />
						</td>
					</tr>
					<tr>
...
if($action=="proceed_realization_search_list_view")
{
	...
	else
	{
		...
		where a.invoice_bill_id=e.id and a.status_active=1 and a.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and a.is_partial=2 and a.benificiary_id=$company_id $byer_cond $date_cond $search_cond
...
if ($action=="save_update_delete")
{
	...
	if($tot_bill_value>$inv_bill_value)
	{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$field_array="id, invoice_bill_id, is_invoice_bill, buyer_id, service_type, benificiary_id, within_group, received_date, remarks, lib_distribution_string, is_partial, buyer_partial_rlz, inserted_by, insert_date";

		$data_array="(".$id.",".$invoice_bill_id.",".$is_invoice_bill.",".$cbo_buyer_name.",".$cbo_service_type.",".$cbo_beneficiary_name.",".$cbo_within_group.",".$txt_received_date.",".$txt_remarks.",".$hdn_variable_distribution.",2,".$buyer_partial_rlz.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
	...
	else if ($operation==1)   // Update Here
	{
		...
		$field_array="invoice_bill_id*is_invoice_bill*buyer_id*service_type*benificiary_id*within_group*received_date*remarks*updated_by*update_date";

		$data_array=$invoice_bill_id."*".$is_invoice_bill."*".$cbo_buyer_name."*".$cbo_service_type."*".$cbo_beneficiary_name."*".$cbo_within_group."*".$txt_received_date."*".$txt_remarks."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
