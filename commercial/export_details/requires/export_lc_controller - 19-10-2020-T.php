...
if ($action == "load_drop_down_buyer") {
    ...
    if ($import_btb == 1) {
        echo create_drop_down("cbo_buyer_name", 162, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "-- Select Buyer --", $selected, "");
    } else {
        if($company_id != 0){
...
if ($action == "get_btb_limit") {
    ...
    /*if ($nameArray) {
        foreach ($nameArray as $row) {
			...
			echo "$('#txt_max_btb_limit').val(" . $row[csf("max_btb_limit")] . ");\n";
        }
    } else {
...
if ($action == 'populate_data_from_export_lc') {
    ...
    foreach ($data_array as $row) {
        ...
        echo "load_drop_down( 'requires/export_lc_controller', document.getElementById('cbo_beneficiary_name').value+'**'+'" . $row[csf("import_btb")] . "', 'load_drop_down_buyer', 'buyer_td_id' );\n";
        ...
        if ($lc_amnd > 0) {
            ...
            if ($row[csf("import_btb")] == 1) {
                ...
                echo "disable_enable_fields('txt_lc_value*txt_last_shipment_date*txt_expiry_date*cbo_shipping_mode*cbo_inco_term*txt_inco_term_place*txt_port_of_entry*txt_port_of_loading*txt_port_of_discharge*cbo_pay_term*txt_tenor*txt_claim_adjustment*txt_discount_clauses*txt_remarks*cbo_beneficiary_name*txt_lc_number*cbo_buyer_name*cbo_currency_name*txt_issuing_bank*cbo_export_item_category*txt_tolerance*txt_doc_presentation_days',0);\n";
            }
        }
                ...
                <fieldset style="width:1028px; margin-left:3px">
                    ...
                    <table cellpadding="0" cellspacing="0" width="80%" class="rpt_table" border="1" rules="all">
                        ...
                        <tr class="general">
                            <td>
                                <?
                                echo create_drop_down("cbo_company_name", 162, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "--- Select ---", $cbo_company_id, "load_drop_down( 'export_lc_controller', this.value, 'load_drop_down_buyer', 'buyer_td_id' );");
                                ?>                        
                            </td>
                            <td id="buyer_td_id">
                                <?
...
if ($action == "export_lc_search_list_view") {
    ...
    $sql = "select id,export_lc_no, internal_file_no $select_field, export_lc_prefix_number, export_lc_system_id, beneficiary_name, buyer_name, applicant_name, lc_value, lien_bank, pay_term, last_shipment_date, lc_date,import_btb from com_export_lc where status_active=1 and is_deleted=0 $company_id $buyer_id $search_condition order by lc_date desc";
    ...
    */
     ?>
     <table width="1020" cellpadding="0" cellspacing="0" border="1" class="rpt_table" rules="all">
        <thead> 
        <th width="30">SL</th>
    ...
    <div style="width:1020px; overflow-y:scroll; max-height:320px">  
        <table width="1000" cellpadding="0" cellspacing="0" border="1" class="rpt_table" rules="all" id="list_view"> 
            ...
            foreach ($data_array as $row) 
            {
                ...
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer;" id="search<? echo $i; ?>" onClick="js_set_value('<? echo $row[csf('id')]; ?>');">
                    ...
                    <td width="100"><? echo $row[csf('lc_value')]; ?></td>
                    ...
                    <table width="950" cellspacing="0" cellpadding="0" class="rpt_table">
                        ...
                        <tr class="general">
                            ...
                            <td align="center">
								...
								if($import_btb==1) $is_sales=1; else $is_sales=0;
                                ?>
                            </td>
                            <td align="center">
...
if ($action == "create_po_search_list_view") {
    ...
    if ($data[5] != "" )
	{
		if($cbo_export_item_category==10 || $cbo_export_item_category==23 || $cbo_export_item_category==35 || $cbo_export_item_category==36 || $cbo_export_item_category==37 || $cbo_export_item_category==45) $selected_order_id = "and wm.id not in (" . $data[5] . ")"; else $selected_order_id = "and wb.id not in (" . $data[5] . ")";
	}
	...
	if ($action_types == 'attached_po_status' && $import_btb != 1) 
	{
		...
		{
			$sql = "SELECT wb.id, wb.po_number, wb.po_total_price, wb.po_quantity, wb.pub_shipment_date as shipment_date, wb.job_no_mst, wb.file_no, wm.job_no_prefix_num, $select_field wm.style_ref_no, wm.gmts_item_id, wb.unit_price,wb.sc_lc 
			FROM wo_po_break_down wb, wo_po_details_master wm 
			WHERE wb.job_no_mst = wm.job_no and wm.buyer_name like '$buyer_id' $company $search_text $file_no_cond $txt_sc_lc_cond and wb.is_deleted = 0 AND wb.status_active = 1 and wb.is_confirmed=1 $date group by wb.id, wb.po_number, wb.po_total_price, wb.po_quantity, wb.pub_shipment_date, wb.job_no_mst, wb.file_no, wm.job_no_prefix_num, wm.insert_date, wm.style_ref_no, wm.gmts_item_id, wb.unit_price, wb.sc_lc";
		}
        
        //echo $sql;
        ?>
        <div>
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1060" style="margin-left:1px" class="rpt_table" >
                <thead>
                <th width="30">SL</th>
    ...
    if ($action_types == 'order_select_popup') 
	{
		...
		elseif($cbo_export_item_category==23 || $cbo_export_item_category==37 || $cbo_export_item_category==45)
		{
			$sql = "SELECT wm.id, wm.subcon_job as po_number, sum(wb.amount) as po_total_price, sum(wb.order_quantity) as po_quantity, wm.delivery_date as shipment_date, wb.job_no_mst, 0 as file_no, wm.job_no_prefix_num, $select_field wb.cust_style_ref as style_ref_no, 0 as gmts_item_id, avg(wb.rate) as unit_price, 0 as sc_lc 
			FROM subcon_ord_mst wm, subcon_ord_dtls wb  
			WHERE wm.subcon_job = wb.job_no_mst and wm.party_id like '$buyer_id' and wm.within_group=2 and wm.entry_form = ".$cat_wise_entry_form[$cbo_export_item_category]." and wb.is_deleted = 0 AND wb.status_active = 1 and wm.is_deleted = 0 AND wm.status_active = 1 $selected_order_id $company $search_text $file_no_cond $txt_sc_lc_cond $date 
			group by  wm.id, wm.subcon_job, wm.delivery_date, wb.job_no_mst, wm.job_no_prefix_num, wm.insert_date, wb.cust_style_ref";
        ...
        <div>
            ...
            <div style="width:1080px; overflow-y:scroll; max-height:220px;" id="buyer_list_view" align="center">
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1060" class="rpt_table" id="tbl_list_search" align="left" >
                    <?
                    $i = 1;
					if($import_btb == 1)
					{
                    ...
                    $lc_attached_qnty_arr = return_library_array("select wo_po_break_down_id, sum(attached_qnty) as qty from com_export_lc_order_info where status_active = 1 and is_deleted=0 and com_export_lc_id>0 and $cbo_export_item_category=$cbo_export_item_category $is_sales_cond group by wo_po_break_down_id", "wo_po_break_down_id", "qty");
...
if ($action == "show_po_active_listview") {
	...
	$is_sales=return_field_value("is_sales","com_export_lc_order_info","com_export_lc_id=$data and status_active=1","is_sales");
    ...
    <div>
        ...
        <div style="width:1200px; overflow-y:scroll; max-height:200px;" id="buyer_list_view" align="center">
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1182" class="rpt_table" id="po_active_list" >
                <?
                $i = 1;
                $total_attc_value = 0;
                ...
                foreach ($nameArray as $selectResult) {
                    ...
                    ?>
                    <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i; ?>" onClick="get_php_form_data('<? echo $selectResult[csf('idd')]."_".$is_sales."_".$data_ref[1]; ?>', 'populate_order_details_form_data', 'requires/export_lc_controller')"> 
                        <td width="100"><p><? echo $selectResult[csf('po_number')]; ?></p></td>
                        <td width="100"><p><? echo $actual_po_arr[$selectResult[csf('id')]]; ?></p></td>
                        <td width="80" align="right"><? echo $selectResult[csf('po_quantity')]; ?></td>
...
if ($action == "populate_order_details_form_data") {
	...
	if($is_sales==0)
	{
		if($export_item_cateogry==23 || $export_item_cateogry==37 || $export_item_cateogry==45)
		{
			...
			/*$sql = "select wm.id, ci.id as idd, 0 as gmts_item_id, wm.subcon_job as po_number, sum(wb.amount) as po_total_price, sum(wb.order_quantity) as po_quantity, wm.delivery_date as shipment_date, wb.job_no_mst, wb.cust_style_ref as style_ref_no, wb.order_uom, 1 as ratio, ci.attached_qnty, ci.attached_rate, ci.attached_value, ci.status_active 
				...
				group by  wm.id, ci.id, wm.subcon_job, wm.delivery_date, wb.job_no_mst, wb.cust_style_ref, wb.order_uom, ci.attached_qnty, ci.attached_rate, ci.attached_value, ci.status_active 
				order by ci.id";*/
			$data_array = sql_select("select wm.id, ci.id as idd, 0 as style_description, wm.subcon_job as po_number, sum(wb.amount) as po_total_price, sum(wb.order_quantity) as po_quantity, wm.delivery_date as shipment_date, wb.job_no_mst, wb.cust_style_ref as style_ref_no, 0 as gmts_item_id, avg(wb.rate) as unit_price, ci.attached_qnty, ci.attached_rate, ci.attached_value, ci.com_export_lc_id, ci.fabric_description, ci.category_no, ci.hs_code, ci.status_active 
			from subcon_ord_dtls wb, subcon_ord_mst wm, com_export_lc_order_info ci 
			where wb.job_no_mst = wm.subcon_job and wm.id=ci.wo_po_break_down_id and ci.id='$lc_attch_id' and ci.status_active = '1' and ci.is_deleted = '0'
			group by  wm.id, ci.id, wm.subcon_job, wm.delivery_date, wb.job_no_mst, wb.cust_style_ref, wb.order_uom, ci.attached_qnty, ci.attached_rate, ci.attached_value, ci.com_export_lc_id, ci.fabric_description, ci.category_no, ci.hs_code, ci.status_active 
			order by ci.id");
		}
		elseif($export_item_cateogry==35 || $export_item_cateogry==36)
		{
			/*$sql = "select wm.id, ci.id as idd, 0 as gmts_item_id, wm.embellishment_job as po_number, sum(wb.amount) as po_total_price, sum(wb.order_quantity) as po_quantity, wm.delivery_date as shipment_date, wb.job_no_mst, wb.cust_style_ref as style_ref_no, wb.order_uom, 1 as ratio, ci.attached_qnty, ci.attached_rate, ci.attached_value, ci.status_active 
				...
				group by  wm.id, ci.id, wm.embellishment_job, wm.delivery_date, wb.job_no_mst, wb.cust_style_ref, wb.order_uom, ci.attached_qnty, ci.attached_rate, ci.attached_value, ci.status_active 
				order by ci.id";*/
			$data_array = sql_select("select wm.id, ci.id as idd, 0 as style_description, wm.embellishment_job as po_number, sum(wb.amount) as po_total_price, sum(wb.order_quantity) as po_quantity, wm.delivery_date as shipment_date, wb.job_no_mst, wb.cust_style_ref as style_ref_no, 0 as gmts_item_id, avg(wb.rate) as unit_price, ci.attached_qnty, ci.attached_rate, ci.attached_value, ci.com_export_lc_id, ci.fabric_description, ci.category_no, ci.hs_code, ci.status_active 
			from fabric_sales_order_dtls wb, fabric_sales_order_mst wm, com_export_lc_order_info ci 
			wherewb.job_no_mst = wm.embellishment_job and wb.mst_id=ci.wo_po_break_down_id and ci.id='$lc_attch_id' and ci.status_active = '1' and ci.is_deleted = '0'
			group by  wm.id, ci.id, wm.embellishment_job, wm.delivery_date, wb.job_no_mst, wb.cust_style_ref, wb.order_uom, ci.attached_qnty, ci.attached_rate, ci.attached_value, ci.com_export_lc_id, ci.fabric_description, ci.category_no, ci.hs_code, ci.status_active 
			order by ci.id");
		}
		else
		{
	...
	else
	{
		...
		$data_array = sql_select("select wm.id, ci.id as idd, 0 as style_description, wm.job_no as po_number, sum(wb.amount) as po_total_price, sum(wb.finish_qty) as po_quantity, wm.delivery_date as shipment_date, wb.job_no_mst, wm.style_ref_no, 0 as gmts_item_id, avg(wb.avg_rate) as unit_price, ci.attached_qnty, ci.attached_rate, ci.attached_value, ci.com_export_lc_id, ci.fabric_description, ci.category_no, ci.hs_code, ci.status_active 
		from fabric_sales_order_dtls wb, fabric_sales_order_mst wm, com_export_lc_order_info ci 
		...
		group by  wm.id, ci.id, wm.job_no, wm.delivery_date, wb.job_no_mst, wm.style_ref_no, wb.order_uom, ci.attached_qnty, ci.attached_rate, ci.attached_value, ci.com_export_lc_id, ci.fabric_description, ci.category_no, ci.hs_code, ci.status_active 
		order by ci.id");
	}
...
if ($action == "order_list_for_attach") {
    ...
    if ($data != "") 
	{
		...
		else
		{
			...
			elseif($export_item_category==35 || $export_item_category==36)
			{
				$data_array = "SELECT wm.id, wm.embellishment_job as po_number, sum(wb.amount) as po_total_price, sum(wb.order_quantity) as po_quantity, wm.delivery_date as shipment_date, wb.job_no_mst, wb.cust_style_ref as style_ref_no, 0 as style_description, avg(wb.rate) as unit_price 
				FROM subcon_ord_mst wm, subcon_ord_dtls wb  
				WHERE wm.embellishment_job = wb.job_no_mst and wm.within_group=2 and wm.entry_form = ".$cat_wise_entry_form[$export_item_category]." and wb.is_deleted = 0 AND wb.status_active = 1 and wm.is_deleted = 0 AND wm.status_active = 1 and wm.within_group=2 and wm.id in ($data)
				group by wm.id, wm.subcon_job, wm.delivery_date, wb.job_no_mst, wb.cust_style_ref";
			}
			else
			{
				$data_array = "SELECT wm.id, wm.subcon_job as po_number, sum(wb.amount) as po_total_price, sum(wb.order_quantity) as po_quantity, wm.delivery_date as shipment_date, wb.job_no_mst, wb.cust_style_ref as style_ref_no, 0 as style_description, avg(wb.rate) as unit_price 
				FROM subcon_ord_mst wm, subcon_ord_dtls wb  
				WHERE wm.subcon_job = wb.job_no_mst and wm.within_group=2 and wm.entry_form = ".$cat_wise_entry_form[$export_item_category]." and wb.is_deleted = 0 AND wb.status_active = 1 and wm.is_deleted = 0 AND wm.status_active = 1 and wm.within_group=2 and wm.id in ($data)
				group by wm.id, wm.subcon_job, wm.delivery_date, wb.job_no_mst, wb.cust_style_ref";
			}
		}
...
if ($action == "sc_search_list_view") {
    ...
    if ($data[4] == "")
        ...
        $sc_selectedID = " and id not in ($data[4])";
            ...
            foreach ($data_array as $row) {
                ...
                if ($row[csf('contract_value')] > $replaced_result) {
                    ...
                    <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer;" id="search<? echo $i; ?>" onClick="js_set_value(<? echo $i; ?>)" >                		<td width="50"><? echo $i; ?>  
                            <input type="hidden" name="txt_individual_id" id="txt_individual_id<? echo $i ?>" value="<? echo $row[csf('id')]; ?>"/></td>
...
if ($action == "save_update_delete_mst") {
	...
	{   
        ...
        //update code here			
        $field_array = "export_lc_no*lc_date*beneficiary_name*buyer_name*applicant_name*notifying_party*consignee*issuing_bank_name*replacement_lc*lien_bank*lien_date*lc_value*currency_name*tolerance*maximum_tolarence*minimum_tolarence*last_shipment_date*expiry_date*shipping_mode*pay_term*inco_term*inco_term_place*lc_source*port_of_entry*port_of_loading*port_of_discharge*internal_file_no*doc_presentation_days*max_btb_limit*max_btb_limit_value*foreign_comn*foreign_comn_value*local_comn*local_comn_value*remarks*tenor*transfering_bank_ref*bl_clause*reimbursement_clauses*discount_clauses*is_lc_transfarrable*transfer_bank*negotiating_bank*nominated_shipp_line*re_imbursing_bank*claim_adjustment*expiry_place*bank_file_no*lc_year*reason*export_item_category*import_btb*import_btb_id*updated_by*update_date";
	...
	else if ($operation==2)   // Delete Here
	{
		...
		if($invoice_id>0)
		{
			echo "35**Delete Not Allow. This LC No Found in Invoice";disconnect($con); die;
		}
		else
		{
			if($id>0)
...
if ($action == "save_update_delete_lc_order_info") {
    ...
    if ($operation == 0) {  // Insert Here
        ...
        $field_array = "id,com_export_lc_id,wo_po_break_down_id,attached_qnty,attached_rate,attached_value,fabric_description,category_no,hs_code,status_active,inserted_by,insert_date,lc_amendment_id,is_sales,export_item_category";
        ...
        for ($j = 1; $j <= $noRow; $j++) {
            ...
            if ($$hiddenwopobreakdownid != "") {
                ...
                $data_array .= "(" . $id . "," . $txt_system_id . "," . $$hiddenwopobreakdownid . "," . $$txtattachedqnty . "," . $$hiddenunitprice . "," . $$txtattachedvalue . "," . $$txtfabdescrip . "," . $$txtcategory . "," . $$txthscode . "," . $$cbopostatus . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "','" . $lc_ammendment_id . "','" . $is_Sales . "'," . $cbo_export_item_category . ")";
        ...
        if(($tot_attached + $currentattachedvalue) > $lc_value)
        {
            echo "11** Attached Value Exceeds LC Value ";disconnect($con);die;
        }
        ...
        //update code here
        $field_array = "id,com_export_lc_id,wo_po_break_down_id,attached_qnty,attached_rate,attached_value,fabric_description,category_no,hs_code,status_active,inserted_by,insert_date,is_sales,export_item_category";
        $field_array_update = "wo_po_break_down_id*attached_qnty*attached_rate*attached_value*fabric_description*category_no*hs_code*status_active*updated_by*update_date*is_sales*export_item_category";
        ...
        for ($j = 1; $j <= $noRow; $j++) {
            ...
            if ($j == 1) {
                if ($hiddenexportlcorderid != "") {
                    ...
                    $data_array_update = "" . $$hiddenwopobreakdownid . "*" . $$txtattachedqnty . "*" . $$hiddenunitprice . "*" . $$txtattachedvalue . "*" . $$txtfabdescrip . "*" . $$txtcategory . "*" . $$txthscode . "*" . $$cbopostatus . "*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'*'" . $is_Sales . "'*" . $cbo_export_item_category . "";
                    ...
                    $data_array = "(" . $id . "," . $txt_system_id . "," . $$hiddenwopobreakdownid . "," . $$txtattachedqnty . "," . $$hiddenunitprice . "," . $$txtattachedvalue . "," . $$txtfabdescrip . "," . $$txtcategory . "," . $$txthscode . "," . $$cbopostatus . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "','" . $is_Sales . "'," . $cbo_export_item_category . ")";
                    $id = $id + 1;
        ...
        <table class="rpt_table" width="100%" cellspacing="0" cellpadding="0" border="1" rules="all" id="tbl_pi_item">
            ...
            <tbody id="pi_details_container">
				...
				if ($exportLcId > 0) 
				{
					...
					$sql = "SELECT id, pi_id, pi_dtls_id, work_order_no, wo_po_break_down_id as work_order_id, booking_no, gmts_item_id, color_id, fabric_description as item_desc, embell_name as process_id, embell_type as wash_type, uom, attached_qnty as quantity, attached_rate as rate, attached_value as amount, attached_rate as net_pi_rate, attached_value as net_pi_amount, is_sales 
					from com_export_lc_order_info where com_export_lc_id=$exportLcId and status_active=1 and is_deleted=0 and is_sales=1";
				} 
				else 
				{
					$sql = "select a.id as pi_dtls_id, a.pi_id, a.work_order_no, a.work_order_id, a.work_order_dtls_id, a.booking_no, a.gmts_item_id, a.color_id as color_id, a.item_description as item_desc, a.embell_name as process_id, a.embell_type as wash_type, a.uom, a.quantity, a.rate, a.amount, a.net_pi_rate, a.net_pi_amount, a.is_sales 
					from com_pi_item_details a, com_btb_lc_pi b
					where a.pi_id=b.pi_id and b.com_btb_lc_master_details_id='$btbId' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
				}
...
if ($action == "save_update_delete_pi") {
    ...
    if ($operation == 0) {  // Insert Here
        ...
        $field_array = "id,com_export_lc_id,wo_po_break_down_id,attached_qnty,attached_rate,attached_value,import_btb,import_btb_id,work_order_no,pi_id,pi_dtls_id,determination_id,construction,composition,color_id,gsm,dia_width,uom,is_sales,inserted_by,insert_date,booking_no,item_group_id,size_id,aop_color,body_part_id,gmts_item_id,embell_name,embell_type,fabric_description";
