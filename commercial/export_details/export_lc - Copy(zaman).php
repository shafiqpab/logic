...
{ 
	...
	if ( form_validation('cbo_beneficiary_name*txt_internal_file_no*txt_year*txt_lc_number*txt_lc_value*txt_lc_date*cbo_buyer_name*txt_last_shipment_date*cbo_export_item_category','Beneficiary Name*Internal File No*Year*LC Number*LC Value*LC Date*Buyer Name*Shipment Date*Export Item Category')==false )
	{
		return;
	}
	else
...
{
	var page_link='requires/export_lc_controller.php?action=export_lc_popup_search';
	...
	emailwindow.onclose=function()
	{
		...
		if(trim(export_lc_id)!="")
		{
			...
			show_list_view(export_lc_id,'show_po_active_listview','po_list_view','requires/export_lc_controller','');
			release_freezing();
		}
...
function add_sales_contract(row_num)
{
	...
	emailwindow.onclose=function()
	{
		...
		if(trim(scID_string)!="")
		{
			...
			if(trim(pre_selectID)=="") $("#hidden_sc_selectedID").val(scID_string);else $("#hidden_sc_selectedID").val(pre_selectID+","+scID_string);  
...
function fnc_po_selection_save(operation)
{
	...
	var data="action=save_update_delete_lc_order_info&noRow="+row_num+"&operation="+operation+get_submitted_data_string('txt_system_id*hiddenexportlcorderid'+submit_data,"../../");
...
function fnc_po_selection_save_Reply_info()
{
	if(http.readyState == 4) 
	{
		...
		{	
			...
			show_list_view(response[1],'show_po_active_listview','po_list_view','requires/export_lc_controller','');
...
function openmypage(page_link,title,row_num)
{
	...
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=980px,height=360px,center=1,resize=1,scrolling=0','../');
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0];
                ...
                <table cellpadding="0" cellspacing="1" width="100%" id="exportlc_tbl">
                	<tr> 
                    	<td colspan="6" align="center" ><b>System ID</b> 
                            ...
                            <input type="text" name="export_lc_system_id" id="export_lc_system_id"  placeholder="Double Click" onDblClick="export_lc_popup()" readonly class="text_boxes">
                        </td>
                    </tr>
                    <tr><td height="5" colspan="6"></td></tr>
                  	<tr>
                        ...
                        <td>
                        	<?
							   	echo create_drop_down( "cbo_beneficiary_name", 162, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Beneficiary --", 0, "load_drop_down( 'requires/export_lc_controller', this.value, 'load_drop_down_buyer', 'buyer_td_id' );load_drop_down( 'requires/export_lc_controller', this.value, 'load_drop_down_applicant_name', 'applicant_name_td' );load_drop_down( 'requires/export_lc_controller', this.value, 'load_drop_down_notifying_party', 'notifying_party_td' );load_drop_down( 'requires/export_lc_controller', this.value, 'load_drop_down_consignee', 'consignee_td' );get_php_form_data( this.value, 'get_btb_limit', 'requires/export_lc_controller' );get_php_form_data( this.value, 'eval_multi_select', 'requires/export_lc_controller' );" );
							?>                          
                        </td>
                    ...
                    <tr>
                        ...
                        <td width="181"><input type="text" name="txt_lc_value" id="txt_lc_value" style="width:150px" class="text_boxes_numeric" ></td>
                    </tr>
                    <tr>
                        <td class="must_entry_caption">LC Date</td>
                        <td><input type="text" name="txt_lc_date" id="txt_lc_date" style="width:150px" class="datepicker" readonly /></td>
                        <td>Currency</td>
                        <td>
							<?
                    ...
                    <tr>
                        ...
                        <td><input type="text" name="txt_doc_presentation_days" style="width:150px" id="txt_doc_presentation_days" class="text_boxes" maxlength="50" title="Maximum Character 50"/></td>
                        <td>BTB Limit %</td>
                    ...
                    <tr>
                      ...
                      <td><input type="text" name="txt_nominated_shipp_line" style="width:150px" id="txt_nominated_shipp_line" class="text_boxes" maxlength="50" title="Maximum Character 50"/></td>
                    ...
                    <tr>
                    	...
                    	<td>Remarks</td>
               			<td colspan="3"><input name="txt_remarks" id="txt_remarks" class="text_boxes" style="width:456px" maxlength="255" title="Maximum Character 255"></td>
                    </tr>
                    <tr>
                        <td colspan="6" height="50" valign="middle" align="center" class="button_container">
							<? 
								echo load_submit_buttons( $permission, "fnc_export_lc_entry", 0,0 ,"reset_form('exportLcFrm_1*exportLcFrm_2*exportLcFrm_3','po_list_view*sc_list_view','','txt_port_of_entry,Ctg*txt_tot_row,0*txt_tolerance,5*cbo_currency_name,2*cbo_replacement_lc,2*txt_expiry_place,Bangladesh','disable_enable_fields(\'cbo_beneficiary_name*txt_lc_value*txt_last_shipment_date*txt_expiry_date*cbo_shipping_mode*cbo_inco_term*txt_inco_term_place*txt_port_of_entry*txt_port_of_loading*txt_port_of_discharge*cbo_pay_term*txt_tenor*txt_claim_adjustment*txt_discount_clauses*txt_remarks\',0)');replacement_lc_diplay(2);$('#tbl_order_list tbody tr:not(:first)').remove();",1) ; 
							?>
                        </td>
                    </tr>
                </table>
