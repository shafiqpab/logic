...
$mendatory_field_main = array();
$mendatory_field_dtls = array();
if(count($_SESSION['logic_erp']['mandatory_field'][270]) > 0){
    foreach ($_SESSION['logic_erp']['mandatory_field'][270] as $key => $val){
        if(trim($val) == 'shipping_bill_no'){
            $mendatory_field_dtls[$key] = $val;
        }elseif(trim($val) == 'ship_bl_date'){
            $mendatory_field_dtls[$key] = $val;
        }else{
            $mendatory_field_main[$key] = $val;
        }
    }
}
?>
...
<?
if($_SESSION['logic_erp']['data_arr'][270]!="")
{
	$data_arr= json_encode( $_SESSION['logic_erp']['data_arr'][270] );
	echo "var field_level_data= ". $data_arr . ";\n";
}

echo "var mandatory_field = '". implode('*',$mendatory_field_main) . "';\n";
echo "var mandatory_field_dtls = '". implode('*',$mendatory_field_dtls) . "';\n";
echo "var field_message = '". implode('*',$_SESSION['logic_erp']['field_message'][270]) . "';\n";
?>
var port_of_loading = [<? echo substr(return_library_autocomplete( "select port_of_loading  from com_export_invoice_ship_mst where  status_active=1 and is_deleted=0 group by port_of_loading", "port_of_loading" ), 0, -1); ?>];
...
var port_of_discharge = [<? echo substr(return_library_autocomplete( "select port_of_discharge  from com_export_invoice_ship_mst where  status_active=1 and is_deleted=0 group by port_of_discharge", "port_of_discharge" ), 0, -1); ?>];
	$(document).ready(function(e)
	{
...
var port_of_entry = [<? echo substr(return_library_autocomplete( "select port_of_entry  from com_export_invoice_ship_mst where  status_active=1 and is_deleted=0 group by port_of_entry", "port_of_entry" ), 0, -1); ?>];
	$(document).ready(function(e)
	{
		$("#port_of_entry").autocomplete({
...
function openmypage_LcSc()
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1040px,height=400px,center=1,resize=1,scrolling=0','../');
	emailwindow.onclose=function()
	{
		...
		var lc_for=this.contentDoc.getElementById("lc_for").value;
		...
		if(trim(lcSc_id)!="")
		{
			...
			change_caption(import_btb,export_item_category,lc_for);
			get_php_form_data(lcSc_id+"**"+is_lcSc+"**"+'0'+"**"+company_id+"**"+import_btb+"**"+export_item_category, "populate_data_from_lcSc", "requires/export_information_entry_controller");
			...
			//alert(commission_source_at_export_invoice);
			$("#cbo_lc_for").val(lc_for);
			print_button_setting(company_id);
			load_drop_down( 'requires/export_information_entry_controller', company_id, 'load_drop_down_location', 'location_td' );
			release_freezing();
		}
	}
}

function change_caption(import_btb,export_item_category,lc_for)
{
	if(import_btb==1)
	{
	...
	else
	{
		if(lc_for==2)
		{
			$('#order_th').text('Requisition No');
			$('#ord_dtls_cap').text('Requisition Details');
			$('#po_bal_cap').text('Req. Balance Qnty');
		}
		else
		{
			$('#order_th').text('Order No');
			$('#ord_dtls_cap').text('Order Details');
			$('#po_bal_cap').text('PO Balance Qnty');
		}
...
function reset_page()
{
	...
	reset_form('','','txt_invoice_no*txt_invoice_date*txt_exp_form_no*txt_exp_form_date*cbo_location*cbo_country*cbo_country_code*txt_remarks*update_id*total_current_invoice_qty*total_current_invoice_val*ex_factory_date','','','');
	$('#txt_invoice_no').attr('disabled',false);
	var lcSc_id=$('#lc_sc_id').val();
	var is_lcSc=$('#is_lc_sc').val();
	var import_btb=$('#import_btb').val();
	var export_item_category=$('#export_item_category').val();
	var company_id=$('#cbo_beneficiary_name').val();
	get_php_form_data(lcSc_id+"**"+is_lcSc+"**"+'0'+"**"+company_id+"**"+import_btb+"**"+export_item_category+"****0", "populate_data_from_lcSc", "requires/export_information_entry_controller");
	//exportInformationFrm_1
...
function openmypage_Invoice()
{
	...
	emailwindow.onclose=function()
	{
		...
		var is_load_all_po=this.contentDoc.getElementById("is_load_all_po").value;
		...
		if(trim(invoice_id)!="")
		{
			...
			var import_btb=$("#import_btb").val();
			var export_item_category=$("#export_item_category").val();
			var lc_for=$("#cbo_lc_for").val();
			
			change_caption(import_btb,export_item_category,lc_for);
			
			get_php_form_data(lcSc_id+"**"+is_lcSc+"**"+invoice_id+"**"+company_id+"**"+import_btb+"**"+export_item_category+"**"+is_load_all_po, "populate_data_from_lcSc", "requires/export_information_entry_controller" );
...
function fnc_export_information_entry(operation)
{
	...
	if ( form_validation('cbo_buyer_name*txt_invoice_no*txt_lc_sc_no*txt_invoice_date*cbo_location*txt_invoice_val','Buyer*Invoice No*LC/SC No*Invoice Date*Location*Invoice Value')==false )
	{
		return;
	}	
	else
	{
		...
		var buyer_lcsc_level=trim(return_global_ajax_value(cbo_buyer_name, 'populate_buyer_lcsc_level_data', '', 'requires/export_information_entry_controller'));
		//alert(buyer_lcsc_level);return;
		if(tot_row!=0)
		{
			for(var j=1;j<=tot_row;j++)
			{
				...
				{
					if(buyer_lcsc_level !=1 )
					{
						if($('#td_cum_invo_qty_'+j).text()*1 > $('#td_order_qty_'+j).attr('title')*1)
						{
							alert("Cumulative Invoice Qnty Exceded Tolerate Attach Quantity");return;
						}
					}
					...
					var is_service=$('#td_order_no_'+j).attr('isservice');
					...
					var carton_qty=$('#carton_qty_'+j).val();
					var production_source_=$('#td_cbo_production_source_'+j).attr('title');
					var actual_po_infos=$('#actual_po_infos_'+j).val();
					//alert(actual_po_infos);
					var colorSize_infos=$('#td_dealing_merchant_'+j).attr('title');
					//alert($('#td_order_no_'+j).attr('orderid'));return;
					
					submit_data += "&order_id_"+j+"="+order_id+"&order_type_"+j+"="+order_type+"&is_service_"+j+"="+is_service+"&order_no_"+j+"="+order_no+"&order_qty_"+j+"="+order_qnty+"&order_rate_"+j+"="+order_rate+"&curr_invo_qty_"+j+"="+curr_invo_qty+"&curr_invo_val_"+j+"="+curr_invo_val+"&carton_qty_"+j+"="+carton_qty+"&cbo_production_source_"+j+"="+production_source_+"&actual_po_infos_"+j+"="+actual_po_infos+"&colorSize_infos_"+j+"="+colorSize_infos;
				}
			}
		...
		//alert(data_row+"="+tot_row);return;

		var data="action=save_update_delete_mst&operation="+operation+'&tot_row='+tot_row+'&color_size_rate='+color_size_rate+get_submitted_data_string('cbo_buyer_name*txt_invoice_no*lc_sc_id*is_lc_sc*import_btb*txt_invoice_date*txt_exp_form_no*txt_exp_form_date*cbo_lien_bank*cbo_beneficiary_name*cbo_location*txt_remarks*txt_invoice_val*txt_discount*txt_discount_ammount*txt_bonus*txt_bonus_ammount*txt_claim*txt_claim_ammount*txt_invo_qnty*txt_commission*txt_commission_amt*txt_other_discount*txt_other_discount_amt*txt_upcharge*txt_net_invo_val*cbo_country*cbo_country_code*update_id*additional_info*gmts_delv_id*gate_out_id*export_invoice_qty_source*commission_source_at_export_invoice*cbo_lc_for*cbo_payment_method*txt_cForm_qnty*txt_cForm_rate*txt_cForm_amt*txt_atsite_discount_percent*txt_atsite_discount_amt*txt_advance_payment_adjustment_amount*txt_advance_payment_adjustment*txt_buyer_supplied_goods_amount*txt_buyer_supplied_goods',"../../")+submit_data;
...
function set_discount(field_id)
{
	...
	else
	{
		if(commission_source_at_export_invoice==1 ||  commission_source_at_export_invoice==0)
		{
			...
			else if(field_id=='txt_atsite_discount_percent')
			{
				var field_val=$('#txt_atsite_discount_percent').val();
				other_commission = (total_invoice_val*field_val)/100;
				other_commission=other_commission.toFixed(2);
				$('#txt_atsite_discount_amt').val(other_commission);
			}
			else if(field_id=='txt_atsite_discount_amt')
			{
				var field_val=$('#txt_atsite_discount_amt').val();
				other_commission_percent = (field_val*100)/(total_invoice_val*1);
				other_commission_percent=other_commission_percent.toFixed(2);
				$('#txt_atsite_discount_percent').val(other_commission_percent);
			}
			
			else if(field_id=='txt_buyer_supplied_goods')
			{
				var field_val=$('#txt_buyer_supplied_goods').val();
				tot_supply = (total_invoice_val*field_val)/100;
				tot_supply=tot_supply.toFixed(2);
				$('#txt_buyer_supplied_goods_amount').val(tot_supply);
			}
			else if(field_id=='txt_buyer_supplied_goods_amount')
			{
				
				var field_val=$('#txt_buyer_supplied_goods_amount').val();
				
				supplied = (field_val*100)/(total_invoice_val*1);
				supplied=supplied.toFixed(2);
				$('#txt_buyer_supplied_goods').val(supplied);
			}
			else if(field_id=='txt_advance_payment_adjustment')
			{
				var field_val=$('#txt_advance_payment_adjustment').val();
				total_adjust = (total_invoice_val*field_val)/100;
				total_adjust=total_adjust.toFixed(2);
				$('#txt_advance_payment_adjustment_amount').val(total_adjust);
			}
			else if(field_id=='txt_advance_payment_adjustment_amount')
			{
				var field_val=$('#txt_advance_payment_adjustment_amount').val();
				adjust = (field_val*100)/(total_invoice_val*1);
				adjust=adjust.toFixed(2);
				$('#txt_advance_payment_adjustment').val(adjust);
			}
			...
			var atsite_discount_amt  =  $('#txt_atsite_discount_amt').val();
			var upcharge  =  $('#txt_upcharge').val();
			var txt_buyer_supplied_goods_amount  =  $('#txt_buyer_supplied_goods_amount').val();	
			var txt_advance_payment_adjustment_amount  =  $('#txt_advance_payment_adjustment_amount').val();
			var total_discount  = (((discount*1)+(bonus*1)+(claim*1)+(commission*1)+(other_commission*1)+(atsite_discount_amt*1)+(txt_buyer_supplied_goods_amount*1)+(txt_advance_payment_adjustment_amount*1))-(upcharge*1));
			var net_invo_val    = (total_invoice_val*1)-(total_discount*1);
			net_invo_val=net_invo_val.toFixed(2);
			$('#txt_net_invo_val').val(net_invo_val);
		}
...
{
	var actual_po_infos = $('#td_order_no_'+row_id).attr('title');
	//alert(actual_po_infos);
...
function openpage_colorSize(row_id)
{
	...
	if($('#chk_color_size_rate').is(':checked'))
	{
		...
		var page_link='requires/export_information_entry_controller.php?action=colorSize_infos_popup&colorSize_infos='+colorSize_infos+'&order_id='+order_id+'&country_id='+country_id+'&company_id='+company_id+'&invoice_id='+invoice_id;
	...
	else
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1050px,height=300px,center=1,resize=1,scrolling=0','../');
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0]
			var actual_infos=this.contentDoc.getElementById("actual_po_infos").value;
			var tot_actual_po_qnty=this.contentDoc.getElementById("tot_actual_po_qnty").value;
			$('#actual_po_infos_'+row_id).val(actual_infos);
			//alert(tot_actual_po_qnty+"=="+row_id);
			$('#curr_invo_qty_'+row_id).val(tot_actual_po_qnty);
			calculate_value_rate(row_id);
		}
...
function active_inactive(variable_setting)
{
	...
	else
	{
		for(var j=1;j<=tot_row;j++)
		{
			//$('#order_rate_'+j).removeAttr('disabled','disabled');
			//$('#order_rate_'+j).attr('disabled','disabled');
			var inv_qnty_td=trim($('#td_curr_invo_qty_'+j).attr("readonly_field"));
			if(inv_qnty_td !="readonly")
			{
				$('#curr_invo_qty_'+j).removeAttr('readonly','readonly');
				$('#curr_invo_qty_'+j).removeAttr('placeholder');
			}
...
function openmypage()
{
	...
	var page_link = 'requires/export_information_entry_controller.php?data='+additional_info+'&action=additional_info_popup&consignee='+consignee+'&notifying_party='+notifying_party;
	...
	emailwindow.onclose=function()
	{
		...
		var data=additional_infos.split('_');
		$("#txt_net_weight").val(data[4]);
		$("#txt_gross_weight").val(data[5]);
	}
}
...
function fn_print_report(action)
{
	...
	{	
		...
		if(action == "invoice_report_print"){
			...
			print_report( data, action, "requires/export_information_entry_controller" ) ;
		}else{
			print_report( $('#update_id').val(), action, "requires/export_information_entry_controller" ) ;
		}
	}
}

function fnc_print_invoice_CIHnM(type)
{
	if (form_validation('update_id','Save Data First')==false)
	{
		alert("Please Save Invoice First");
	...
	var action="print_invoice_CIHnM";
	//freeze_window();
	var data="data="+$('#update_id').val()+"&action="+action;
	http.open("POST","requires/export_information_entry_controller.php",true);
	http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	http.send(data);
	http.onreadystatechange = fnc_print_invoice_CIHnM_reponse;
}

function fnc_print_invoice_CIHnM_reponse()
{
	if(http.readyState == 4)
	{
       // release_freezing();
...
function fnc_print_invoice(type)
{
	if (form_validation('update_id','Save Data First')==false)
	{
		alert("Please Save Invoice First");
	...
	var export_item_category=$("#export_item_category").val();
	var import_btb=$("#import_btb").val();
	if(import_btb==1 && export_item_category==37)
	{
		if(type==1 || type==2 || type==3 || type==4 || type==5)
		{
			var data = $('#update_id').val()+"**"+type;
			print_report( data, "print_generate", "requires/export_information_entry_controller" ) ;
		}
		if(type==6)
		{
			...
			var data = $('#update_id').val()+"**"+type+"**"+show_item;
			print_report( data, "print_generate", "requires/export_information_entry_controller" ) ;
...
function print_pdf(action){
	...
	window.open('requires/export_information_entry_controller.php?action='+action+'&update_id='+update_id+'&cbo_buyer_name='+cbo_buyer_name);
	return;
}
...
/*$('#txt_ac_po_search').live('keydown', function(e) {
	if (e.keyCode === 13)
	{
		...
		if(ac_order_no!="")
		{
			var ac_order_data=trim(return_global_ajax_value(ac_order_no, 'populate_ac_order_data', '', 'requires/export_information_entry_controller'));
			if(ac_order_data!="")
			{
				for(var i=1; i<=tbl_lenth; i++)
				{
					var order_no 	= $('#order_no_'+i).html();
					var orderid 	= $('#td_order_no_'+i).attr('orderid');
					var order_no_id =order_no+'_'+orderid;
					if(order_no_id==ac_order_data){
						$('#tr_'+i).show();
					}else{
						$('#tr_'+i).hide();
					}
...
$('#txt_ac_po_search').live('keydown', function(e) {
	if (e.keyCode === 13)
	{
		...
		var tbl_lenth=(($('#tbl_order_list tbody tr').length*1)-2);
		var ac_order_no=trim($('#txt_ac_po_search').val());
		var company=$('#cbo_beneficiary_name').val();
		var data=company+'**'+ac_order_no;
		if(ac_order_no!="")
		{
			var ac_order_data=trim(return_global_ajax_value(data, 'populate_ac_order_data', '', 'requires/export_information_entry_controller')).split("__");
			//alert(ac_order_data);return;
			if(ac_order_data!="")
			{
				for(var i=1; i<=tbl_lenth; i++)
				{
					//var order_no=trim($('#order_no_'+i).html());
					var order_no=trim($('#td_order_no_'+i).attr("orderid"));
					
					ac_order_data.includes(order_no);
					//if(order_no==ac_order_data)
					if(ac_order_data.includes(order_no))
					{
						$('#tr_'+i).show();
					}
					else
...
$('#txt_internal_ref_search').live('keydown', function(e) {
	if (e.keyCode === 13)
	{
		if(form_validation('txt_lc_sc_no','LC/SC No')==false )
		{
		...
		var tbl_lenth=(($('#tbl_order_list tbody tr').length*1)-2);
		var internal_ref=trim($('#txt_internal_ref_search').val());
		if(internal_ref!="")
		{
			var ac_order_data=trim(return_global_ajax_value(internal_ref, 'populate_internal_ref_data', '', 'requires/export_information_entry_controller')).split("__");
			//alert(ac_order_data);//return;
			if(ac_order_data!="")
			{
				for(var i=1; i<=tbl_lenth; i++)
				{
					var order_no=trim($('#td_order_no_'+i).attr("orderid"));
					//alert(order_no+"="+i+"="+ac_order_data.includes(order_no));
					//ac_order_data.includes(order_no);
					//if(order_no==ac_order_data)
					if(ac_order_data.includes(order_no))
					{
...
function exportInvoiceQtySource(page_link,title,is_attach)
{
	...
	if(tot_row!=0)
	{
		for(var j=1;j<=tot_row;j++)
		{
			...
			order_id[j]=$('#td_order_no_'+j).attr('orderid');
		}
	}
...
function fn_freight_amt()
{
	
	var invoice_id = document.getElementById("update_id").value;
	var invoice_no = document.getElementById("txt_invoice_no").value;
	if(invoice_id=="")
	{
		alert("Save Data First");return;
	}
	var title = 'Freight Info';
	var page_link = 'requires/export_information_entry_controller.php?action=freight_info_popup&invoice_id='+invoice_id+'&invoice_no='+invoice_no;
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=920px,height=360px,center=1,resize=1,scrolling=0','../');
	emailwindow.onclose=function()
	{
		/*var theform=this.contentDoc.forms[0];
...
function fnc_print_report2(type)
{
	if($('#update_id').val()=="")
	{
		alert("Please Save Invoice First");
	...
	{
		var data = $('#update_id').val()+"|"+$('#additional_info').val();
		if(type==1){ var action="invoice_report_print_2"; }
		else if (type==4){ var action="invoice_report_print_4";}
		else if (type==5){ var action="invoice_report_print_5";}
		else if (type==6){ var action="invoice_report_print_6";}
		else if (type==7){ var action="invoice_report_print_7";}
		else if (type==8){ var action="invoice_report_print_8";}
		else if (type==9){ var action="invoice_report_print_9";}
		else if (type==10){ var action="invoice_report_print_10";}
		else{ var action="invoice_report_print_ci_ny"; }
        freeze_window();
		var data="update_id="+$('#update_id').val()+"&action="+action;
		http.open("POST","requires/export_information_entry_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
		http.onreadystatechange = show_print_report_2_reponse;

	}
}

function show_print_report_2_reponse(){
	if(http.readyState == 4){
        release_freezing();
		var file_data=http.responseText.split("####");
		$('#data_panel').html(file_data[1]);
...
function print_button_setting(company)
	{
		...
		// alert(company);
		get_php_form_data(company,'print_button_variable_setting','requires/export_information_entry_controller' );
	}

	function fnc_print_dem_export(id) 
	{
		var page_link='requires/export_information_entry_controller.php?action=openpopup_dem_export&data='+'button_id='+id;
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, 'User List','width=450px,height=300px,center=1,resize=1,scrolling=0','../../');
		emailwindow.onclose=function()
		{
			var selected_print_report = this.contentDoc.getElementById("selected_print_report").value;
			var selected_seq_id = this.contentDoc.getElementById("selected_seq_id").value;

			var action="fro_top_sheet_print"; 

			freeze_window();
			var data="update_id="+$('#update_id').val()+"&action="+action+"&selected_seq_id="+selected_seq_id+ "&selected_print_report=" + encodeURIComponent(selected_print_report);
			http.open("POST","requires/export_information_entry_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
			http.onreadystatechange = show_dem_export;
		}

	}

	function show_dem_export(){
	if(http.readyState == 4){
        release_freezing();
		var file_data=http.responseText.split("####");
		//alert(file_data);
...
function print_report_button_setting(report_ids)
	{
		...
		for (var k=0; k<report_id.length; k++)
		{
			...
			{
				$('#button_data_panel')
					.append( '<td align="right"><input type="button" class="formbutton" id="btn_print_rpt" value="Print Report" style="width:100px;" onClick="fn_print_report(print_invoice)" ></td>&nbsp;&nbsp;&nbsp;' );
			}
			if(report_id[k]==788)
			{
				var action = "'pdf'";
				$('#button_data_panel').append( '<td align="right"><input type="button" class="formbutton" id="btn_print_rpt" value="CI" style="width:100px;" onClick="print_pdf('+action+')" ></td>&nbsp;&nbsp;&nbsp;' );
			}
			if(report_id[k]==789)
			{
				$('#button_data_panel').append( '<td align="right"><input type="button" class="formbutton" id="btn_invoice_rpt" value="Invoice Rrport" style="width:100px;" onClick="fn_print_report(invoice_report_print)" > </td>&nbsp;&nbsp;&nbsp;' );
			}
			if(report_id[k]==790)
			{
				$('#button_data_panel').append( '<td align="right"><input type="button" name="CI2" class="formbutton" value="CI 2" id="CI2" onClick="fnc_print_invoice(1);" style="width:80px;"/></td>&nbsp;&nbsp;&nbsp;' );
			}if(report_id[k]==791)
			{
				$('#button_data_panel').append( '<td align="right"><input type="button" name="CI3" class="formbutton" value="CI 3" id="CI3" onClick="fn_print_report(invoice_report_print_ci3)" style="width:80px;"/></td>&nbsp;&nbsp;&nbsp;' );
			}if(report_id[k]==792)
			{
				$('#button_data_panel').append( '<td align="right"><input type="button" name="PL" class="formbutton" value="PL" id="PL" onClick="fnc_print_invoice(2);" style="width:80px;"/></td>&nbsp;&nbsp;&nbsp;' );
			}if(report_id[k]==793)
			{
				$('#button_data_panel').append( '<td align="right"><input type="button" name="DC" class="formbutton" value="DC" id="DC" onClick="fnc_print_invoice(3);" style="width:80px;"/></td>&nbsp;&nbsp;&nbsp;' );
			}if(report_id[k]==794)
			{
				$('#button_data_panel').append( '<td align="right"><input type="button" name="TC" class="formbutton" value="TC" id="TC" onClick="fnc_print_invoice(4);" style="width:80px;"/></td>&nbsp;&nbsp;&nbsp;' );
			}if(report_id[k]==795)
			{
				$('#button_data_panel').append( '<td align="right"><input type="button" name="CO" class="formbutton" value="CO" id="CO" onClick="fnc_print_invoice(5);" style="width:80px;"/></td>&nbsp;&nbsp;&nbsp;' );
			}if(report_id[k]==796)
			{
				$('#button_data_panel').append( '<td align="right"><input type="button" name="BE" class="formbutton" value="BE" id="BE" onClick="fnc_print_invoice(6);" style="width:80px;"/></td>&nbsp;&nbsp;&nbsp;' );
			}if(report_id[k]==797)
			{
				$('#button_data_panel').append( '<td align="right"><input type="button" name="ci_hnm" id="ci_hnm" class="formbutton" value="CI-HnM" onClick="fnc_print_invoice_CIHnM(1);" style="width:80px;"/></td>&nbsp;&nbsp;&nbsp;' );
			}if(report_id[k]==84)
			{
				$('#button_data_panel').append( '<td align="right"><input type="button" name="print_2" class="formbutton" value="Print 2" id="print_2" onClick="fnc_print_report2(1);" style="width:80px;"/></td>&nbsp;&nbsp;&nbsp;' );
			}if(report_id[k]==85) //print 3
			{
				var action = "'print_invoice3'";
				$('#button_data_panel').append( '<td align="right"><input type="button" class="formbutton" id="btn_print_rpt" value="Print 3" style="width:100px;" onClick="fn_print_report('+action+')" ></td>&nbsp;&nbsp;&nbsp;' );
			}
			
			if(report_id[k]==137) //print 4
			{
				$('#button_data_panel').append( '<td align="right"><input type="button" name="print_4" class="formbutton" value="Print 4" id="print_4" onClick="fnc_print_report2(4);" style="width:80px;"/></td>&nbsp;&nbsp;&nbsp;' );
			}
			if(report_id[k]==129) //print 5
			{
				$('#button_data_panel').append( '<td align="right"><input type="button" name="print_5" class="formbutton" value="Print 5" id="print_5" onClick="fnc_print_report2(5);" style="width:80px;"/></td>&nbsp;&nbsp;&nbsp;' );
			}
			if(report_id[k]==72) //print 6
			{
			
				$('#button_data_panel').append( '<td align="right"><input type="button" class="formbutton" id="Print_6" value="Print 6" style="width:100px;" onClick="fnc_print_report2(6);" ></td>&nbsp;&nbsp;&nbsp;' );
			}
			if(report_id[k]==584) //print 7
			{
			
				$('#button_data_panel').append( '<td align="right"><input type="button" class="formbutton" id="Print_7" value="CI-CnA" style="width:100px;" onClick="fnc_print_report2(7);" ></td>&nbsp;&nbsp;&nbsp;' );
			}
			if(report_id[k]==777) //print 8
			{
				$('#button_data_panel').append( '<td align="right"><input type="button" class="formbutton" id="Print_7" value="CI FSO" style="width:100px;" onClick="fnc_print_report2(8);" ></td>&nbsp;&nbsp;&nbsp;' );
			}
			if(report_id[k]==798)
			{
				$('#button_data_panel').append( '<td align="right"><input type="button" name="print_ny" class="formbutton" value="CI-NY" id="print_ny" onClick="fnc_print_report2(2);" style="width:80px;"/></td>&nbsp;&nbsp;&nbsp;' );
			}	
			if(report_id[k]==853)
			{
				$('#button_data_panel').append( '<td align="right"><input type="button" name="dem export" class="formbutton" value="Dem Export" id="dem_export" onClick="fnc_print_dem_export(2);" style="width:80px;"/></td>&nbsp;&nbsp;&nbsp;' );
			}	
			if(report_id[k]==904)
			{
				$('#button_data_panel').append( '<td align="right"><input type="button" name="Exp_Doc_4H" class="formbutton" value="Exp Doc-4H" id="Exp_Doc_4H" onClick="fnc_print_report2(9);" style="width:80px;"/></td>&nbsp;&nbsp;&nbsp;' );
			}
			if(report_id[k]==907)
			{
				$('#button_data_panel').append( '<td align="right"><input type="button" name="PVH_BTN" class="formbutton" value="PVH" id="Exp_Doc_4H" onClick="fnc_print_report2(10);" style="width:80px;"/></td>&nbsp;&nbsp;&nbsp;' );
			}
					
		}
	}

	function fn_calculate_cform()
	{
		var cform_qnty=$("#txt_cForm_qnty").val()*1;
		var cform_rate=$("#txt_cForm_rate").val()*1;
		var cform_amt=cform_qnty*cform_rate;
		$("#txt_cForm_amt").val(cform_amt);
		...
		var tot_inv_val=(net_invo_val+cform_amt)*1;
		$("#txt_net_invo_val").val(tot_inv_val);
	}

	function openmypage_invoice_number()
	{
		if (form_validation('txt_lc_sc_no','lc sc no')==false)
            {
                return;
		...
		var page_link='requires/export_information_entry_controller.php?action=pinumber_popup&companyID='+companyID+'&buyer_name='+buyer_name;
		var title='Invoice Popup Info';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=800px,height=390px,center=1,resize=1,scrolling=0','../');
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0];
			var invoice_id=this.contentDoc.getElementById("invoice_id").value;
...
function fn_fill_qnty() 
{
    ...
    if ($('#check_qnty').is(':checked')) {
        $("#tbl_order_list").find('tbody tr').not(':first').each(function(index, element) {		
            var bl_qnty = parseFloat($("#td_po_bl_qty_" + i).html());
			var inv_qnty_td=trim($('#td_curr_invo_qty_'+i).attr("readonly_field"));
            if (bl_qnty>0 && inv_qnty_td !="readonly") 
			{
                $("#curr_invo_qty_" + i).val(bl_qnty);
                calculate_value_rate(i);
                i++;
            }else{
				calculate_value_rate(i);
                i++;
	...
	{
        $("#tbl_order_list").find('tbody tr').not(':first').each(function(index, element) {
            var bl_qnty = parseFloat($("#curr_invo_qty_" + i).val());
            if (isNaN(bl_qnty)) {
                $("#td_po_bl_qty_" + i).val(bl_qnty);
                $("#curr_invo_qty_" + i).val();
                calculate_value_rate(i);
                i++;
            }
       ...
       <form name="exportInformationFrm_1" id="exportInformationFrm_1" autocomplete="off" method="POST"  >
        <fieldset style="width:1220px; margin-bottom:10px;">
            <legend>Export Information Entry</legend>
                ...
                <table cellpadding="0" cellspacing="1" width="1100">
                  	<tr>
                        ...
                        <td>
                            ...
                            <input type="hidden" name="cbo_lc_for" id="cbo_lc_for"/>

                        </td>
                        <td class="must_entry_caption">Invoice No <input type="button" style="width:15px" onDblClick="openmypage_invoice_number();" value="BT"></td>
                    	<td>
                    ...
                    <tr>
                        <td class="must_entry_caption">Ex-Fac Location</td>
                        <td id="location_td">
                            <?
								echo create_drop_down("cbo_location", 165, $blank_array, "", 1, "-- Select Location --", 0, "");
							?>
                        </td>
                        <td>Country</td>
                        <td>
                            <?
							   	echo create_drop_down( "cbo_country", 165, "select id,country_name from  lib_country where status_active =1 and is_deleted=0 order by country_name","id,country_name", 1, "-- Select Country --", 0, "load_drop_down( 'requires/export_information_entry_controller', this.value, 'load_drop_down_country_code', 'country_code_td' );",0 );
							?>
                        </td>
                        <td>Country Code</td>
                    ...
                    <tr>
                        <td>Remarks</td>
                        <td><input name="txt_remarks" id="txt_remarks" class="text_boxes" style="width:160px"></td>
                        ...
                        <td>Color & Size Rate &nbsp; <input type="checkbox" name="chk_color_size_rate" id="chk_color_size_rate" onClick="active_inactive();"></td>
                        <td>Payment Method</td>
                        <td>
                        	<?
								$payment_method_arr=array(1=>"Bank Submission",2=>"TT");
							   	echo create_drop_down( "cbo_payment_method", 165, $payment_method_arr,"", 1, "-- Select--", 0, "",0 );
							?>
                        </td>
                    </tr>
					<tr>
                ...
                <fieldset style="width:1335px; margin-bottom:10px;">


                <br/>
                <table width="1395" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" id="tbl_order_list_top">
                    <thead>
                    	<tr>
                        	<th colspan="4">
                            <input type="text" id="txt_ac_po_search" name="txt_ac_po_search" class="text_boxes" style="width:150px;" placeholder="Write Actual PO No Then Enter" >
                            <input type="text" id="txt_internal_ref_search" name="txt_internal_ref_search" class="text_boxes" style="width:150px;" placeholder="Write Internal Ref. Then Enter" >
                            </th>
                            <th colspan="3" ><input type="button" id="image_button" class="image_uploader" style="width:160px" value="CLICK TO Attach Invoice Qty" onClick="exportInvoiceQtySource('requires/export_information_entry_controller.php?action=invoice_qty_popup','Invoice Qty',1)" /></th>
                            <th colspan="2" ><input type="button" id="image_button" class="image_uploader" style="width:160px" value="CLICK TO Detach  Invoice Qty" onClick="exportInvoiceQtySource('requires/export_information_entry_controller.php?action=invoice_qty_popup','Invoice Qty',2)" /></th>
                            <th colspan="9" id="ord_dtls_cap">Order Details</th>
                        </tr>
                    	<tr>
                            <th width="80" id="job_th">Job No</th>
                            <th width="90" id="order_th">Order No</th>
                            <th width="105">Style Ref.</th>
                            <th width="80" id="art_desc">Article No</th>
                            <th width="60" id="ship_gsm">Shipment Date</th>
                            <th width="80">Attached Order Qnty</th>
                            <th width="60">UOM</th>
                            <th width="70">Rate</th>
                            <th width="80" class="must_entry_caption"><input type="checkbox" id="check_qnty" name="check_qnty" onChange="fn_fill_qnty()" /><br>Curr. Invoice Qnty</th>
                            <th width="95">Curr. Invoice Value</th>
                            <th width="65">Cumu Invoice Qnty</th>
                            <th width="65" id="po_bal_cap">PO Balance Qnty</th>
                            <th width="75">Cumu Invoice Value</th>
                            <th width="65">Ex-Factory Qnty</th>
                            <th width="90" id="color_mrch">Merchandiser</th>
                            <th>Production Source</th>
                            <th width="60">Brand</th>
                            <th width="60">Carton/Roll Qty</th>
                      	</tr>
                    </thead>
                </table>
                <div style="width:1395px; max-height:300px; overflow-y:scroll" id="scroll_body">
                    <table width="1395" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" id="tbl_order_list">
                        <tbody id="order_details">
                            <tr class="general">
                                <td colspan="16"><strong>Please select a LC/SC no to view order details</strong></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <br>
                 <table cellpadding="0" cellspacing="1" width="1400" >
                 	<tr>
                        ...
                        <td>
                            Up Charge for SM / C-From.
                        </td>
                        <td>
                            <input type="text" id="txt_cForm_qnty" name="txt_cForm_qnty" onKeyUp="fn_calculate_cform()" class="text_boxes_numeric" style="width:40px" placeholder="Qnty" />
                            <input type="text" id="txt_cForm_rate" name="txt_cForm_rate" onKeyUp="fn_calculate_cform()" class="text_boxes_numeric" style="width:35px" placeholder="Rate" />
                            <input type="text" id="txt_cForm_amt" name="txt_cForm_amt" class="text_boxes_numeric" style="width:45px" placeholder="Amt" readonly />
                        </td>
						<td>
                            Discount %
                        </td>
                  ...
                  <tr>
                        ...
                        <td >
                            Discount For At Sight Payment %
                        </td>
                        <td>
                            <input type="text" name="txt_atsite_discount_percent" id="txt_atsite_discount_percent" class="text_boxes_numeric" style="width:150px;" onKeyUp="set_discount(this.id)"/>
                        </td>
                  </tr>
					<tr>
							<td >Discount For At Sight Payment Amount</td>
							<td>
								<input type="text" name="txt_atsite_discount_amt" id="txt_atsite_discount_amt" class="text_boxes_numeric" style="width:150px;  " onKeyUp="set_discount(this.id)"/>
							</td>
							<td>Buyer Supplied Goods %</td>
							<td><input type="text" name="txt_buyer_supplied_goods" id="txt_buyer_supplied_goods" class="text_boxes_numeric" style="width:150px;" onKeyUp="set_discount(this.id)"/></td>
							<td>Buyer Supplied Goods Amount</td>
							<td><input type="text" name="txt_buyer_supplied_goods_amount" id="txt_buyer_supplied_goods_amount" class="text_boxes_numeric" style="width:150px;" onKeyUp="set_discount(this.id)" /></td>
							<td>Advance Payment Adjustment %</td>
							<td><input type="text" name="txt_advance_payment_adjustment" id="txt_advance_payment_adjustment" class="text_boxes_numeric" style="width:150px;" onKeyUp="set_discount(this.id)"/></td>
					</tr>
					 <tr>
					     <td>Advance Payment Adjustment Amount</td>
						<td><input type="text" name="txt_advance_payment_adjustment_amount" id="txt_advance_payment_adjustment_amount" class="text_boxes_numeric" style="width:150px;" onKeyUp="set_discount(this.id)"/></td>
						<td> Net Invoice Value</td>
                        <td>
                            <input type="text" name="txt_net_invo_val" id="txt_net_invo_val" class="text_boxes_numeric" style="width:150px;" disabled="disabled" onKeyUp="set_discount(this.id)"/>
                        </td>
                ...
                <table width="100%">
                	<tr>
                        <td width="60%" colspan="4" valign="middle" align="right" class="button_container">
							<?
								echo load_submit_buttons( $permission, "fnc_export_information_entry", 0,0 ,"reset_form('exportInformationFrm_1*exportInformationFrm_2','posted_status_td','','','reset_table();')",1) ;
							?>
        ...
        <fieldset style="width:1220px; margin-bottom:10px;">
            ...
            <form name="exportInformationFrm_2" id="exportInformationFrm_2" autocomplete="off" method="POST"  >
                <table cellpadding="0" cellspacing="1" width="1100">
					<tr>
						<td class="must_entry_caption">BL/Cargo/AWB No</td>
                        <td>
                            <input type="text" name="bl_no"  id="bl_no" class="text_boxes" style="width:150px"/>
                        </td>
                        <td class="must_entry_caption">BL/Cargo/AWB Date</td>
                        <td>
                    ...
                    <tr>
                        ...
                        <td>
                        	<?
							   	echo create_drop_down( "inco_term", 162, $incoterm,"", 1, "-- Select --", 0, "" );
							?>
                        </td>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" name="freight_amnt_supplier" id="freight_amnt_supplier" class="text_boxes_numeric" style="width:150px" onDblClick="fn_freight_amt();"/>
                        </td>
                    ...
                    <tr>
                        ...
                        <td>Stamp/Non-Judicial Stamp Value</td>
                        <td>
							<? //echo create_drop_down( "cbo_stamp_value", 162, $stamp_value_array,"", 1, "-- Select --", 0, "" ); ?>
                            <input type="text" name="txt_stamp_value" id="txt_stamp_value" class="text_boxes_numeric" style="width:150px;" />
						</td>
                    </tr>
                    <tr>
                        <td>Adhesive Stamp Value</td>
                        <td>
                            <input type="text" name="txt_adhesive_stamp_value" id="txt_adhesive_stamp_value" class="text_boxes_numeric" style="width:150px;" />
						</td>
						<td>Seal No</td>
                        <td>
                            <input type="text" name="txt_seal_no" id="txt_seal_no" class="text_boxes" style="width:150px;" />
						</td>
                    </tr>
                    <tr>
