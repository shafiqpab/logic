...
//--------------lib
	...
	if(count($pi_number_arr)>1){
		foreach($pi_number_arr as $key=>$row){
			$pi_numbers .= $row.'&nbsp;DT: '.date('d.m.Y',strtotime($pi_date[$key])).", ";
		}
...
<style>
    body{
		...
		background: url("../application_form/form_image/image/sibl_lca.jpg");
		background-size:21.59cm 30.56cm;
		background-repeat: no-repeat;
    }

	header{margin-top: 26.5%;padding-bottom: 10%;}
	#head-a{position: relative;}
	#head-a b{left: 37%;position:absolute;}
	#head-a{margin: 0;padding: 0;}
	#p-1{position: relative;top: 49%;}
	#p-1 b{position: absolute;left:50%;} 
	#p-2 b{margin-top: 5%;}
	#p-2 #p2a{font-size: 10px;position: absolute;left: 25.5%;}
	#p-2 #p2b{font-size: 10px;position: absolute;left: 69%;}
	#p-3 b{margin-top: 8%;}
	#p-3 b{position: absolute;left: 30%;}
	#p-4 b{margin-top: 10.5%;}
	#p-4 b{position: absolute;left: 50%;}
	#p-7 #p7a{margin-top: 29.5%;position: absolute;left: 50%;}
	#p-7 #p7b{margin-top: 30%; padding-top: 2%;position: absolute;left: 30%;}
	#p-9 #p9a{margin-top: 49%;padding-bottom: 5%;position: absolute;left:17%;}
	#p-9 #p9b{margin-top: 49%;position: absolute;left: 48%;}
	/* #position22{ transform: rotate(90deg) translate(120%, 0);transform-origin: top right; } */
</style>

<!-------------------------------------------- Comments
Purpose			: 	Shahjalal Islamic  Bank  form value formate
Functionality	:	
JS Functions	:
...
<body>
<header>
        <div id="head-a"><b></b></div>
</header>
   
<section id="part-a">
    <div id="p-1">
        <b><? echo $company_name;?> </br> <? echo "Plot# ".$company_add[$data_array[0][csf("importer_id")]]['plot_no'].", Road# ".$company_add[$data_array[0][csf("importer_id")]]['road_no'].",<br/> ".$company_add[$data_array[0][csf("importer_id")]]['city'].", ".$country_array[$company_add[$data_array[0][csf("importer_id")]]['country_id']]; ?> </b>   
    </div>
    <div id="p-2">
        <b id="p2a"><?= $irc_no;?></b> <span><b id="p2b"><? echo $renewal_year ;?></b></span>
    </div>
    <div id="p-3">
        <b id="p3">100% Export Oriented <? echo $business_nature; ?> Ready made Garments Industry</b>
    </div>
    <div id="p-4">
        <b id="p4">Regular</b>
    </div>
    ...
    <div id="p-9">
        <b id="p9a"><? echo $pi_category." for 100% Export Oriented / Ready made Garments Industry";?></b>
		<span>  <b id="p9b"><? echo $hs_code;?></b></span>
    </div>
