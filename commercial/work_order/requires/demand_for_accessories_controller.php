...
if ($action=="order_popup")
{
	...
	function js_set_value( strCon ) 
	{
		...
		//alert(str+"**"+selectID+"**"+selectStyle);
		...
		var id = ''; var name = ''; var style = ''; var style_name = ''; var style_ref_name='';
		for( var i = 0; i < selected_id.length; i++ )
		{
			id += selected_id[i] + ',';
		...
		style_ref_name=style;
		style_name_arr=style.split(',');
		style_name = $.unique(style_name_arr);

		$('#txt_job_id').val( id );
		$('#txt_job_sl_no').val( name );
		$('#txt_style_ref').val( style_name );
		$('#txt_style_ref_no').val( style_ref_name );
		//alert(id)
	}
	...
	function fn_set_prev_data()
	{
		...
		var prev_style_arr=$('#txt_style_ref_no').val().split(",");
		var id_ref="";
		for( var i = 0; i < prev_id_arr.length; i++ )
		{
			id_ref = prev_sl_arr[i]+'_'+prev_id_arr[i]+'_'+prev_style_arr[i]; 
			js_set_value(id_ref);
		}
	}
	</script>
    <?
    //echo $txt_style_no.'style';
    //echo $hidd_job_id.'job';
    //echo $hidd_job_sl_no.'sl';
	...
	$hidd_txt_style_no=trim(str_replace("'","",$hidd_txt_style_no));
	...
	echo "<input type='hidden' id='txt_style_ref_no' value='".$hidd_txt_style_no."' />";
...
{
	$data_array=sql_select("select id, sys_number, sys_number_prefix, sys_number_prefix_num, entry_form, company_id, buyer_id, season_id, season_year, brand_id, team_leader_id, deling_merchant_id, job_id, style_ref_no, demand_date,CS_REQ_DATE, remarks from scm_demand_mst where id='$data' and is_deleted=0 and status_active=1");
	$supp_mult_arr='';
	$basis='';
	foreach ($data_array as $row)
	{
		...
		echo "document.getElementById('txt_cs_req_date').value = '".change_date_format($row[csf("CS_REQ_DATE")])."';\n";
	...
	//'&preCostDtlsId_' + i + '=' + $('#preCostDtlsId_'+i).val() + '&itemGroupId_' + i + '=' + $('#itemGroupId_'+i).val() + '&mainGroupId_' + i + '=' + $('#mainGroupId_'+i).val() + '&nominatedSup_' + i + '=' + $('#nominatedSup_'+i).val()+ '&uom_' + i + '=' + $('#uom_'+i).val()+ '&brandSup_' + i + '=' + $('#brandSup_'+i).text()+ '&description_' + i + '=' + $('#description_'+i).text()+ '&txtReqQty_' + i + '=' + $('#txtReqQty_'+i).val() + '&txtStockQty_' + i + '=' + $('#txtStockQty_'+i).val() + '&txtRate_' + i + '=' + $('#txtRate_'+i).val()+ '&txtAmount_' + i + '=' + $('#txtAmount_'+i).val();
		...
		for($i=1; $i<=$row_num; $i++)
		{
			...
			if($$dtlsUpdateId==""){
				...
				$data_array_dtls_update[$$dtlsUpdateId]=explode("*",("'".$$txtDate."'*'".$$txtRemarks."'*"."".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
		...
		*{
		...
		<table cellpadding="0" align="left" cellspacing="5" border="0" >
			<tr>
				...
				<td><?= $buyer_arr[$data_array[0]['BUYER_ID']]; ?></td>
				<td align="right"><strong>Demand No.: &nbsp;</strong></td>
				<td><?= $data_array[0]['SYS_NUMBER']; ?></td>
				<td align="right"><strong>Merchandiser: &nbsp;</strong></td>
				<td><?= $merchant_arr[$data_array[0]['DELING_MERCHANT_ID']]; ?></td>
				<td align="right"><strong>Date: &nbsp;</strong></td>
				<td><?= change_date_format($data_array[0]['DEMAND_DATE']); ?></td>
				<td align="right"><strong>CS Required Date:</strong></td>
				<td><?=change_date_format($data_array[0][CS_REQ_DATE]);?></td>

            </tr>
			<tr>
				<td align="right"><strong>Remarks: &nbsp;</strong></td>
				<td colspan="9"><?= $data_array[0]['REMARKS']; ?></td>
            </tr>
		</table>
		<?
