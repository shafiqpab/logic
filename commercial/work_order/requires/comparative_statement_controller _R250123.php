...
if ($action=="populate_data_from_search_popup")
{
	...
	$supp_dtls_arr=sql_select("select id, mst_id,dtls_id, supp_id,prod_id, item_group_id,quoted_price, neg_price, con_price, brand,model,origin,pay_term,tenor from req_comparative_supp_dtls where mst_id='$data' and is_deleted=0 and status_active=1 order by id asc");
	...
	$data_tbl.='<th rowspan="2" width="100">Item Category</th>';
	$data_tbl.='<th rowspan="2" width="100">Items Group</th>';
	$data_tbl.='<th rowspan="2" width="80">Items Code</th>';
	$data_tbl.='<th rowspan="2" width="150">Items Description</th>';
	$data_tbl.='<th rowspan="2" width="80">Req. Qty.</th>';
	$data_tbl.='<th rowspan="2" width="50">UOM</th>';
	$data_tbl.='<th rowspan="2" width="80">Brand</th>';
	$data_tbl.='<th rowspan="2" width="80">Model</th>';
	$data_tbl.='<th rowspan="2" width="80">Origin</th>';
	foreach($supp_mult_arr as $row)
	{
		$data_tbl.='<th colspan="8" width="660">'.$supplier_arr[$row].'</th>';
	}
	$data_tbl.='</tr><tr>';
	foreach($supp_mult_arr as $row)
	{
		...
		$data_tbl.='<th width="80">Brand</th>';
		$data_tbl.='<th width="80">Model</th>';
		$data_tbl.='<th width="80">Pay Term</th>';
		$data_tbl.='<th width="80">Tenor</th>';
		$data_tbl.='<th width="80">Origin</th>';
	}
	...
	foreach($dtls_sql_result as $row)
	{
		...
		$data_tbl.='<td align="center" >'.create_drop_down( "txtcategory_$i", 110,$item_category,"", 1, "-- Select --", $item_des_sql_arr[$pro_id_one]['ITEM_CATEGORY_ID'], "",1).'</td>';
		$data_tbl.='<td align="center" ><input type="text" style="width:70px" name="" id="txtgroup_'.$i.'" class="text_boxes" title="'.$row['ITEM_GROUP_ID'].'" value="'.$item_group_arr[$row['ITEM_GROUP_ID']].'" readonly></td>';
		$data_tbl.='<td align="center" ><input type="text" style="width:70px" name="" id="" class="text_boxes" value="'.$item_des_sql_arr[$pro_id_one]['ITEM_CODE'].'" readonly></td>';
		$data_tbl.='<td align="center" ><input type="text" style="width:110px" name="" id="" class="text_boxes" value="'.$item.'" readonly></td>';
		if($basis_id==1)
		{
		...
		{
			$data_tbl.='<td align="center" ><input type="text" style="width:70px" name="" id="txtqty_'.$i.'" class="text_boxes_numeric" value="'.$row['REQ_QTY'].'" ></td>';
		}
		...
		$data_tbl.='<td align="center" ><input type="text" style="width:70px" name="" id="txtcsbrand_'.$i.'" class="text_boxes" value="'.$row['BRAND'].'" readonly></td>';
		$data_tbl.='<td align="center" ><input type="text" style="width:70px" name="" id="txtcsmodel_'.$i.'" class="text_boxes" value="'.$row['MODEL'].'" readonly></td>';
		$data_tbl.='<td align="center" ><input type="text" style="width:70px" name="" id="txtcsorigin_'.$i.'" class="text_boxes" value="'.$row['ORIGIN'].'" readonly></td>';
		$row_id=$row['ID'];
		foreach($supp_mult_arr as $supp)
		{
			foreach($supp_dtls_arr as $rows)
			{
				...
				if($row_id==$dtls_row_id)
				{
					if($supp==$supp_id)
					{
						...
						$data_tbl.='<td align="center" ><input type="text" style="width:60px" name="" id="txtbrand_'.$i.'_'.$supp_id.'" class="text_boxes" value="'.$rows[csf('brand')].'"></td>';
						$data_tbl.='<td align="center" ><input type="text" style="width:60px" name="" id="txtmodel_'.$i.'_'.$supp_id.'" class="text_boxes" value="'.$rows[csf('model')].'"></td>';
						...
						$data_tbl.='<td align="center" ><input type="text" style="width:60px" name="" id="txtorigin_'.$i.'_'.$supp_id.'" class="text_boxes" value="'.$rows[csf('origin')].'"></td>';
					}
				}
			}
		}
...
if ($action=="save_update_delete")
{
	...
	if($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$field_array_supp_dtls="id, mst_id, dtls_id, supp_id, prod_id, item_group_id, quoted_price, neg_price, con_price, last_approval_rate, brand, model, origin, pay_term, tenor, inserted_by, insert_date, is_deleted, status_active";
		...
		for($i=1; $i<=$row_num; $i++)
		{
			...
			$txtcsorigin="txtcsorigin_".$i;
				...
				for($m=0; $m<$col_num; $m++)
				{
					...
					if($supp_data!=''){
						$supp_data.= "*".$$txtsuppier."_".$$txtquoted."_".$$txtneg."_".$$txtcon."_".$$txtbrand."_".$$txtmodel."_".$$txtorigin."_".$$txtPayTerm."_".$$txtTenor;
					}else{
						$supp_data = $$txtsuppier."_".$$txtquoted."_".$$txtneg."_".$$txtcon."_".$$txtbrand."_".$$txtmodel."_".$$txtorigin."_".$$txtPayTerm."_".$$txtTenor;
					}

					if ($data_array_supp_dtls!='') {$data_array_supp_dtls .=",";}
					$data_array_supp_dtls .="(".$id_supp_dtls.",".$mst_id.",".$id_dtls.",'".$$txtsuppier."','".$$txtprod."','".$$txtGroup."','".$$txtquoted."','".$$txtneg."','".$$txtcon."','".$$txtcon."','".$$txtbrand."','".$$txtmodel."','".$$txtorigin."','".$$txtPayTerm."','".$$txtTenor."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',0,1)";
					$id_supp_dtls++;
				}
	...
	elseif($operation==1) // Update Here----------------------------------------------------------
	{
		...
		for($i=1; $i<=$row_num; $i++)
		{
			...
			$txtcsorigin="txtcsorigin_".$i;
				...
				for($m=0; $m<$col_num; $m++)
				{
					...
					if ($data_array_supp_dtls!='') {$data_array_supp_dtls .=",";}
					$data_array_supp_dtls .="(".$id_supp_dtls.",".$update_id.",".$id_dtls.",'".$$txtsuppier."','".$$txtprod."','".$$txtgroup."','".$$txtquoted."','".$$txtneg."','".$$txtcon."','".$$txtcon."','".$$txtbrand."','".$$txtmodel."','".$$txtorigin."','".$$txtPayTerm."','".$$txtTenor."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',0,1)";
					$id_supp_dtls++;
				}
		...
		//echo "10**INSERT INTO req_comparative_dtls (".$field_array_dtls.") VALUES ".$data_array_dtls; 
...
if($action=="load_cs_table")
{
	...
	if($basis==1)
	{
		...
		{
			$sql = "select listagg(cast(b.id as varchar(4000)),',') within group(order by b.id) as PROD_IDS ,listagg(cast(a.brand_name as varchar(4000)),',') within group(order by a.brand_name) as BRAND_NAME ,listagg(cast(a.model as varchar(4000)),',') within group(order by a.model) as MODEL ,listagg(cast(a.origin as varchar(4000)),',') within group(order by a.origin) as ORIGIN ,sum(a.quantity) as REQ_QTY, b.order_uom as ORDER_UOM, b.item_category_id as ITEM_CATEGORY_ID, b.item_description as ITEM_DESCRIPTION, b.item_group_id as ITEM_GROUP_ID, b.item_size as ITEM_SIZE, c.item_name as ITEM_NAME, b.item_code as ITEM_CODE, max(a.product_id) as PROD_ID 
			from inv_purchase_requisition_dtls a, product_details_master b, lib_item_group c 
			where a.product_id=b.id and b.item_group_id=c.id and a.id in($req_dtls) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 
	...
	<table width="<?=$tbl_width;?>" cellpadding="0" cellspacing="0" border="1" class="rpt_table" rules="all"  id="tbl_details">
		<thead>
			<tr>
				...
				<th rowspan="2" width="100">Items Group</th>
				<th rowspan="2" width="80">Items Code</th>
				<th rowspan="2" width="150">Items Description</th>
				<th rowspan="2" width="80">Req. Qty.</th>
				<th rowspan="2" width="50">UOM</th>
				<th rowspan="2" width="80">Brand</th>
				<th rowspan="2" width="80">Model</th>
				<th rowspan="2" width="80">Origin</th>
				<?
					foreach($supplier as $row)
					{
						?>
							<th colspan="8" width="660"><? echo $supplier_arr[$row]; ?></th>
						<?
					}
				?>
			</tr>
		...
		foreach($data_array as $row)
		{
			...
			<tr class="general" id="<? echo $i;?>">
				...
				<td align="center" >
					<? echo create_drop_down( "txtcategory_$i", 110,$item_category,"", 1, "-- Select --", $row['ITEM_CATEGORY_ID'], "",1);?>
				</td>
				<td align="center" >
					<input type="text" style="width:70px" name="" id="txtgroup_<?= $i;?>" class="text_boxes" title="<? echo $row['ITEM_GROUP_ID'] ;?>" value="<? echo $row['ITEM_NAME'] ;?>" readonly>
				</td>
				<td align="center" >
					<input type="text" style="width:70px" name="" id="" class="text_boxes" value="<? echo $row['ITEM_CODE'] ;?>" readonly>
				</td>
				<td align="center" >
					<input type="text" style="width:110px" name="" id="" class="text_boxes" value="<?
				...
				<td align="center" >
					...
					{
					?>
						<input type="text" style="width:70px" name="" id="txtqty_<?= $i;?>" class="text_boxes_numeric" value="" >
					<?
					}?>
					
				</td>
				<td align="center" >
					<input type="text" style="width:50px" name="" id="" class="text_boxes" value="<? echo $unit_of_measurement[$row['ORDER_UOM']] ;?>" readonly>
				</td>
				<td align="center" >
					<input type="text" style="width:70px" name="" id="txtcsbrand_<?= $i;?>" class="text_boxes" value="<? echo $row['BRAND_NAME'] ;?>" readonly>
				</td>
				<td align="center" >
					<input type="text" style="width:70px" name="" id="txtcsmodel_<?= $i;?>" class="text_boxes" value="<? echo $row['MODEL'] ;?>" readonly>
				</td>
				<td align="center" >
					<? 
					...
					?>
					<input type="text" style="width:70px" name="" id="txtcsorigin_<?= $i;?>" class="text_boxes" value="<? echo $origin_name;?>" readonly>
				</td>
			    <?
				foreach($supplier as $sup_id)
				{
					?>
						...
						<td align="center" ><input type="text" style="width:60px" name="" id="txtbrand_<?= $i.'_'.$sup_id;?>"  value="<?= $supp_prev_entry_data[$row["PROD_ID"]][$sup_id]["brand"];?>" class="text_boxes" ></td>
						<td align="center" ><input type="text" style="width:60px" name="" id="txtmodel_<?= $i.'_'.$sup_id;?>"  value="<?= $supp_prev_entry_data[$row["PROD_ID"]][$sup_id]["model"];?>" class="text_boxes" ></td>
						<td align="center" >
						...
						<td align="center" ><input type="text" style="width:60px" name="" id="txtorigin_<?= $i.'_'.$sup_id;?>"  value="<?= $supp_prev_entry_data[$row["PROD_ID"]][$sup_id]["origin"];?>" class="text_boxes" ></td>
					<? 
				}
			?>
			</tr>
...
if($action=="comparative_statement_print")
{
	...
	$supp_dtls_arr=sql_select("select id, mst_id,dtls_id, supp_id,prod_id, quoted_price, neg_price, con_price, brand,model,origin from req_comparative_supp_dtls where mst_id='$mst_id' and is_deleted=0 and status_active=1 order by id asc");
	$supplier_count=count($supp_mult_arr);
	...
	<table width="<?=$tbl_width;?>" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table" >
		<thead bgcolor="#dddddd" align="center">
			<tr>
				...
				<?
					...
					{
						?>
						<th colspan="3" width="260"><?= $supplier_contact_arr[$row];?></th>
						<?
					}
				?>
			...
			<tr>
				<?
					...
					{
						?>
						<th colspan="3" width="260"><?= $supplier_arr[$row];?></th>
						<?
					}
				?>
			</tr>
		...
		<tbody>
			...
			<tr>
				...
				{
					?>
					<td colspan="3" align="right" ><? echo number_format($grand_amount[$supp],2); ?></td>
					<?
				}
				?>
				<td></td>
...
if($action=="print_report_generate")
{
	...
	$supplier_arr=return_library_array( "select id, supplier_name from lib_supplier",'id','supplier_name');
	$lib_company_arr=return_library_array( "select id, company_name from lib_company",'id','company_name');
	$data_array=sql_select("SELECT id, sys_number, sys_number_prefix, sys_number_prefix_num, basis_id, req_item_no,req_item_mst_id, rec_date, cs_date, supp_id, currency_id, cs_valid_date, source, approved, company_id, comments from req_comparative_mst where id='$mst_id' and is_deleted=0 and status_active=1");
...
if($action=="print_report_estimated_price")
{
	...
	{
		$supplier_wise_rate_arr[$row['SUPP_ID']]+=$rate_supplier_arr[$row['MINIMUM_APPROVAL_RATE']][$row['DTLS_ID']];
	}
	...
	<table width="900" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table" >
		...
		<tbody>
			...
			$i=1;
			$grand_tot_amount=0;
			foreach($supplier_wise_rate_arr as $supp_id => $value)
			{
				if ($value > 0)
				{
					if ($i%2==0){$bgcolor="#E9F3FF";}else{$bgcolor="#FFFFFF";}
	...
	<table width="900" cellpadding="0" cellspacing="0" border="1" rules="all" class="rpt_table" >
		<tr>
			<td align="left" ><b><? echo "In World: ".$currency[$currency_id]." ".number_to_words($grand_tot_amount); ?> Only</b></td>
		</tr>
	</table>
	<?
	echo signature_table(25, $req_company_id, "900px", '', 20);
	?>
	<?
	exit();
}
