...
if($action=="show_req_dtls_listview")
{
    ...
    <table cellspacing="0" width="1350" class="rpt_table" id="tbl_details" border="1" rules="all">
        <thead>
            <tr>
...
if($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$field_array_mst="id, garments_nature, wo_number_prefix, wo_number_prefix_num, wo_number, company_name, buyer_po, wo_date, supplier_id, attention, buyer_name, style, wo_basis_id, entry_form, currency_id, delivery_date, source, pay_mode, do_no, remarks,payterm_id,tenor,ship_mode_id, inserted_by, insert_date,inco_term,pi_issue_to,ready_to_approved";
		$data_array_mst="(".$id.",".$garments_nature.",'".$new_wo_number[1]."','".$new_wo_number[2]."','".$new_wo_number[0]."',".$cbo_company_name.",".$txt_buyer_po.",".$txt_wo_date.",".$cbo_supplier.",".$txt_attention.",".$txt_buyer_name.",".$txt_style.",".$cbo_wo_basis.",144,".$cbo_currency.",".$txt_delivery_date.",".$cbo_source.",".$cbo_pay_mode.",".$txt_do_no.",".$txt_remarks.",".$cbo_payterm_id.",".$txt_tenor.",".$cbo_ship_mode.",'".$user_id."','".$pc_date_time."',".$cbo_inco_term.",".$cbo_pi_issue_to.",".$ready_to_approved.")";
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		if($mst_id!="")
		{
			$field_array_mst="buyer_po*wo_date*supplier_id*attention*buyer_name*style*wo_basis_id*entry_form*currency_id*delivery_date*source*pay_mode*do_no*remarks*payterm_id*tenor*ship_mode_id*inco_term*pi_issue_to*updated_by*update_date*ready_to_approved";
			$data_array_mst="".$txt_buyer_po."*".$txt_wo_date."*".$cbo_supplier."*".$txt_attention."*".$txt_buyer_name."*".$txt_style."*".$cbo_wo_basis."*144*".$cbo_currency."*".$txt_delivery_date."*".$cbo_source."*".$cbo_pay_mode."*".$txt_do_no."*".$txt_remarks."*".$cbo_payterm_id."*".$txt_tenor."*".$cbo_ship_mode."*".$cbo_inco_term."*".$cbo_pi_issue_to."*'".$user_id."'*'".$pc_date_time."'*".$ready_to_approved."";
			...
			for($i=1;$i<=$total_row;$i++)
			{
				...
				if($$txt_quantity!="" || $$txt_rate!="") //check blank row
				{
					if($dtlsID>0) //update
					{
						...
						$data_array[$dtlsID]=explode("*",("".$$txt_req_dtls_id."*".$$txt_job_id."*".$$txt_job."*".$$txt_buyer_id."*".$$txt_style."*".$$txt_req."*".$$po_breakdown_id."*0*1*".$$cbocount."*".$$cbocompone."*".$$percentone."*".$$cbotype."*".$$txt_color."*".$$cbo_uom."*".$$txt_req_qnty."*".$$txt_quantity."*".$$txt_rate."*".$$txt_amount."*'".$user_id."'*'".$pc_date_time."'*'".$txt_inhouse_date."'*'".$txt_delivery_end_date."'*".$$number_of_lot."*".$$txt_remarks."*".$$txt_sample_date.""));
					}
					else // new insert
					{
...
{
	 $sql = "SELECT id, wo_number, company_name, buyer_po, wo_date, supplier_id, attention, buyer_name, style, wo_basis_id, item_category, currency_id, delivery_date, source, pay_mode,payterm_id, is_approved, do_no, remarks,tenor,pi_issue_to,ref_closing_status, ready_to_approved, inco_term,ship_mode_id
...
if($action=="show_dtls_listview_update")
{
	...
	if($data[1]==1)
	{
		...
		<table cellspacing="0" width="1340" class="rpt_table" id="tbl_details" border="1" rules="all" >
			<thead>
				<tr>
					<th width="40">Req. No</th>
					<th width="100">Job No</th>
	    ...
	    foreach ($dataArr as $key => $row) {
	        ...
	        ?>
	                        <tr bgcolor="<? echo $bgcolor; ?>">
								...
								<td align="center"><p><? echo $row["internal_ref"]; ?></p></td>
	                            <td align="center"><p><? echo $color_arr[$row["color_name"]]; ?></p></td>
	                            <td align="center"><p><? echo $count_arr[$row["yarn_count"]]; ?></p></td>
	                            <td align="center"><p><? echo $row[feb_des]; ?></p></td>
	                            <td align="center"><p><? echo $unit_of_measurement[$row["uom"]]; ?></p></td>
...
if ($action == "print_to_html_report7") 
{
   ...
   foreach ($sql_res as $val) 
   {
        ...
        // if ($val['BUYER_ID'] != 0 && $check_buyer_id[$val['BUYER_ID']]==""){
        //     $check_buyer_id[$val['BUYER_ID']]=$val['BUYER_ID'];
        //     $buyer_names.=$buyer_arr[$val['BUYER_ID']].', ';
        // }
   }
    ...
    <div style="width:1000px;">
        ...
        <table width="1200" cellspacing="0" align="center" border="1" rules="all" class="rpt_table">
            ...
            <tr>
                ...
                <td width="80" style="font-size:16px"><strong>Pay Mode</strong></td>
                <td width="80" style="font-size:16px"><? echo $payMode; ?></td>
                <td width="80" style="font-size:16px"><strong>Source</strong></td>
                <td width="80" style="font-size:16px"><? echo $sourceData; ?></td>
                <td width="80" style="font-size:16px"><strong>Pay Term</strong></td>
                ...
                <td width="60" style="font-size:16px"><strong>Buyer</strong></td>
                <td style="font-size:16px"><? echo rtrim($buyer_nam); ?></td>
            </tr>            
            <tr>
                ...
                <td style="font-size:16px"><strong>WO Basis</strong></td>
                <td style="font-size:16px"><? echo $woBasis; ?></td>
                ...
                <td colspan="5" style="font-size:16px"><? echo $remarks; ?></td>
            </tr>       
        </table>
		<br>
            ...
            <tbody>
                ...
                foreach ($sql_res as $row) 
                {
                    ...
                    <tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                        ...
                        <td ><? echo $item_des; ?></td>
                        <td ><? echo $count_arr[$row['YARN_COUNT']]; ?></td>
        ...
        <br>
        <!-- <table width="1000" cellspacing="0" align="center" border="1" rules="all" class="rpt_table">
            <tr>
                <td align="left" colspan="10" style="font-size:20px"><? echo $email; ?></td>
            </tr>
            <tr>
                <td align="left" colspan="10" style="font-size:20px">Remarks:<? echo $remarks; ?></td>
            </tr>
        </table> -->
        <br>
        <?
        ...
        <?
        /*
        $sql = sql_select("select designation,name,activities,prepared_by from variable_settings_signature where report_id=42 and company_id=$company_id order by sequence_no");
        ...
        $prepared_by=$user_arr[$user_id];
            ...
            foreach ($sql as $row) {
                ...
                <strong>' . $row[csf("activities")] . '</strong><br>' . $row[csf("name")] . '<br>
            ...
            echo '</tr></table>';
            */
        echo signature_table(42, $company_id, "1200px", "", "70", $user_arr[$user_id]);
