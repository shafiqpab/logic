...
<script>
	...
	<?
		$data_arr= json_encode( $_SESSION['logic_erp']['data_arr'][104] );
    ...
    function fnc_pi_mst(operation)
    {
        if (operation == 4)
        {
            print_report($('#cbo_importer_id').val() + '*' + $('#update_id').val() + '*' + $('#cbo_item_category_id').val(), "print", "requires/pi_print");
            return;
        }
        ...
        } else
        {
            ...
            if (export_pi_id > 0)
            {
                ...
                var data = "action=save_update_delete&operation=" + operation + '&total_row=' + row_num + get_submitted_data_string('cbo_item_category_id*cbo_importer_id*cbo_supplier_id*pi_number*pi_date*last_shipment_date*pi_validity_date*cbo_currency_id*cbo_source_id*hs_code*txt_internal_file_no*intendor_name*cbo_pi_basis_id*txt_remarks*hide_approved_status*update_id*cbo_goods_rcv_status*export_pi_id*within_group*txt_total_amount*txt_upcharge*txt_discount*txt_total_amount_net*cbo_ready_to_approved*txt_lc_group_no*txt_requested_by', "../../") + data_all;
            } else
            {
                var data = "action=save_update_delete&operation=" + operation + get_submitted_data_string('cbo_item_category_id*cbo_importer_id*cbo_supplier_id*pi_number*pi_date*last_shipment_date*pi_validity_date*cbo_currency_id*cbo_source_id*hs_code*txt_internal_file_no*intendor_name*cbo_pi_basis_id*txt_remarks*hide_approved_status*update_id*cbo_goods_rcv_status*export_pi_id*within_group*cbo_ready_to_approved*txt_lc_group_no*txt_requested_by', "../../");
            }

            freeze_window(operation);
            http.open("POST", "requires/pi_controller.php", true);
    ...
    function fnc_pi_mst_reponse()
    {
        if (http.readyState == 4)
        {
            ...
            {
                alert("This PI is Attached With BTB LC No- '" + reponse[1] + "'. So You can't change it.");
            } else if (reponse[0] == 16)
            {
                alert("This PI is already Approved. So You can't change it.");
            }
            release_freezing();
        }
    }
    ...
    function add_break_down_tr(i)
    {
        ...
        } else
        {
            ...
            if (category == 1)
            {
                $('#yarnCompositionItem1_' + i).removeAttr("onchange").attr("onchange", "control_composition(" + i + ",'comp_one')");
            ...
            {
                $('#itemgroupid_' + i).removeAttr("onchange").attr("onchange", "get_php_form_data(this.value+'**'+'uom_" + i + "','get_uom', 'requires/pi_controller')");
            ...
            {
                $('#embellname_' + i).removeAttr("onchange").attr("onchange", "load_drop_down('requires/pi_controller',this.value+'**'+0+'**'+'embelltype_" + i + "', 'load_drop_down_embelltype','embelltypeTd_" + i + "')");
    ...
    function fnc_pi_item_details(operation)
    {
        ...
        if (operation == 2 && cbo_pi_basis_id == 1)
        {
            ...
            for (var j = 1; j <= row_num; j++)
            {
                if (cbo_item_category_id == 2 || cbo_item_category_id == 3 || cbo_item_category_id == 4) {
                    fabric_source = $('#hidden_fabric_source_' + j).val();
                }
                ...
                {
                    i++;
                    data_all += "&quantity_" + i + "='" + $('#quantity_' + j).val() + "'" + "&rate_" + i + "='" + $('#rate_' + j).val() + "'" + "&amount_" + i + "='" + $('#amount_' + j).val() + "'" + "&updateIdDtls_" + i + "='" + $('#updateIdDtls_' + j).val() + "'" + "&hidden_fabric_source_" + i + "='" + fabric_source + "'";
		...
		else
        {
            if(cbo_item_category_id==1)
			{
				if(cbo_pi_basis_id==1)
				{
					for (var j=1; j<=row_num; j++)
					{
						...
						if($('#workOrderChkbox_'+j).is(':checked') || updateIdDtls!="")
						{
							...
							data_all+="&workOrderNo_" + i + "='" + $('#workOrderNo_'+j).val()+"'"+"&hideWoId_" + i + "='" + $('#hideWoId_'+j).val()+"'"+"&hideWoDtlsId_" + i + "='" + $('#hideWoDtlsId_'+j).val()+"'"+"&colorName_" + i + "='" + $('#colorName_'+j).val()+"'"+"&countName_" + i + "='" + $('#countName_'+j).val()+"'"+"&yarnCompositionItem1_" + i + "='" + $('#yarnCompositionItem1_'+j).val()+"'"+"&yarnCompositionPercentage1_" + i + "='" + $('#yarnCompositionPercentage1_'+j).val()+"'"+"&yarnCompositionItem2_" + i + "='" + $('#yarnCompositionItem2_'+j).val()+"'"+"&yarnCompositionPercentage2_" + i + "='" + $('#yarnCompositionPercentage2_'+j).val()+"'"+"&yarnType_" + i + "='" + $('#yarnType_'+j).val()+"'"+"&uom_" + i + "='" + $('#uom_'+j).val()+"'"+"&quantity_" + i + "='" + $('#quantity_'+j).val()+"'"+"&rate_" + i + "='" + $('#rate_'+j).val()+"'"+"&amount_" + i + "='" + $('#amount_'+j).val()+"'"+"&updateIdDtls_" + i + "='" + $('#updateIdDtls_'+j).val()+"'";
				...
				else
				{
					for (var j=1; j<=row_num; j++)
					{
						if (form_validation('colorName_'+j+'*countName_'+j+'*yarnCompositionItem1_'+j+'*yarnCompositionPercentage1_'+j+'*quantity_'+j+'*rate_'+j,'Color*Count*Composition Type First*Composition Percentage First*Qunatity*Rate')==false)
						{
							return;
						}
						i++;
						data_all+=get_submitted_data_string('colorName_'+i+'*countName_'+i+'*yarnCompositionItem1_'+i+'*yarnCompositionPercentage1_'+i+'*yarnCompositionItem2_'+i+'*yarnCompositionPercentage2_'+i+'*yarnType_'+i+'*uom_'+i+'*quantity_'+i+'*rate_'+i+'*amount_'+i+'*updateIdDtls_'+i,"../../",i);
			...
			else if (cbo_item_category_id == 2 || cbo_item_category_id == 13)
            {
                if (cbo_pi_basis_id == 1)
                {
                    for (var j = 1; j <= row_num; j++)
                    {
                        ...
                        if ($('#workOrderChkbox_' + j).is(':checked') || updateIdDtls != "")
                        {
                            if ($('#hidden_fabric_source_' + j).val() == 3 && cbo_item_category_id == 2) {
                                if (form_validation('workOrderNo_' + j + '*quantity_' + j, 'WO*Qunatity') == false)
                                {
                                    return;
                                }
                            } else {
                            ...
                            data_all += "&workOrderNo_" + i + "='" + $('#workOrderNo_' + j).val() + "'" + "&hideWoId_" + i + "='" + $('#hideWoId_' + j).val() + "'" + "&hideWoDtlsId_" + i + "='" + $('#hideWoDtlsId_' + j).val() + "'" + "&construction_" + i + "='" + $('#construction_' + j).val() + "'" + "&composition_" + i + "='" + $('#composition_' + j).val() + "'" + "&colorName_" + i + "='" + $('#colorName_' + j).val() + "'" + "&gsm_" + i + "='" + $('#gsm_' + j).val() + "'" + "&diawidth_" + i + "='" + $('#diawidth_' + j).val() + "'" + "&uom_" + i + "='" + $('#uom_' + j).val() + "'" + "&quantity_" + i + "='" + $('#quantity_' + j).val() + "'" + "&rate_" + i + "='" + $('#rate_' + j).val() + "'" + "&amount_" + i + "='" + $('#amount_' + j).val() + "'" + "&updateIdDtls_" + i + "='" + $('#updateIdDtls_' + j).val() + "'" + "&hideDeterminationId_" + i + "='" + $('#hideDeterminationId_' + j).val() + "'" + "&hidden_fabric_source_" + i + "='" + $('#hidden_fabric_source_' + j).val() + "'";  //$('#hidden_fabric_source_'+j).val()
				...
				else
                {
                    for (var j = 1; j <= row_num; j++)
                    {
                        ...
                        var updateIdDtls = $('#updateIdDtls_' + j).val();
                        var quantity = $('#quantity_' + j).val() * 1;

                        if (quantity > 0)
                        {
                            if (form_validation('construction_' + j + '*colorName_' + j + '*diawidth_' + j, 'Construction*Color*Width') == false)
                            {
                                return;
                            }
                        }
                        ...
                        if (quantity > 0 || updateIdDtls != "")
                        {
                            ...
                            data_all += "&construction_" + i + "='" + $('#construction_' + j).val() + "'" + "&composition_" + i + "='" + $('#composition_' + j).val() + "'" + "&colorName_" + i + "='" + $('#colorName_' + j).val() + "'" + "&gsm_" + i + "='" + $('#gsm_' + j).val() + "'" + "&diawidth_" + i + "='" + $('#diawidth_' + j).val() + "'" + "&uom_" + i + "='" + $('#uom_' + j).val() + "'" + "&quantity_" + i + "='" + $('#quantity_' + j).val() + "'" + "&rate_" + i + "='" + $('#rate_' + j).val() + "'" + "&amount_" + i + "='" + $('#amount_' + j).val() + "'" + "&updateIdDtls_" + i + "='" + $('#updateIdDtls_' + j).val() + "'" + "&hideDeterminationId_" + i + "='" + $('#hideDeterminationId_' + j).val() + "'";
                            /*data_all+=get_submitted_data_string('construction_'+i+'*composition_'+i+'*colorName_'+i+'*gsm_'+i+'*diawidth_'+i+'*uom_'+i+'*quantity_'+i+'*rate_'+i+'*amount_'+i+'*updateIdDtls_'+i+'*hideDeterminationId_'+i,"../../");*/
			...
			else if (cbo_item_category_id == 3 || cbo_item_category_id == 14)
            {
                if (cbo_pi_basis_id == 1)
                {
                    for (var j = 1; j <= row_num; j++)
                    {
                        ...
                        if ($('#workOrderChkbox_' + j).is(':checked') || updateIdDtls != "")
                        {
                            if ($('#hidden_fabric_source_' + j).val() == 3 && cbo_item_category_id == 3) {
                                if (form_validation('workOrderNo_' + j + '*quantity_' + j, 'WO*Qunatity') == false)
                                {
                                    return;
                                }
                            ...
                            data_all += "&workOrderNo_" + i + "='" + $('#workOrderNo_' + j).val() + "'" + "&hideWoId_" + i + "='" + $('#hideWoId_' + j).val() + "'" + "&hideWoDtlsId_" + i + "='" + $('#hideWoDtlsId_' + j).val() + "'" + "&construction_" + i + "='" + $('#construction_' + j).val() + "'" + "&composition_" + i + "='" + $('#composition_' + j).val() + "'" + "&colorName_" + i + "='" + $('#colorName_' + j).val() + "'" + "&weight_" + i + "='" + $('#weight_' + j).val() + "'" + "&diawidth_" + i + "='" + $('#diawidth_' + j).val() + "'" + "&uom_" + i + "='" + $('#uom_' + j).val() + "'" + "&quantity_" + i + "='" + $('#quantity_' + j).val() + "'" + "&rate_" + i + "='" + $('#rate_' + j).val() + "'" + "&amount_" + i + "='" + $('#amount_' + j).val() + "'" + "&updateIdDtls_" + i + "='" + $('#updateIdDtls_' + j).val() + "'" + "&hideDeterminationId_" + i + "='" + $('#hideDeterminationId_' + j).val() + "'" + "&hidden_fabric_source_" + i + "='" + $('#hidden_fabric_source_' + j).val() + "'";
			...
			else if (cbo_item_category_id == 4)
            {
                ...
                if (cbo_pi_basis_id == 1)
                {
                    for (var j = 1; j <= row_num; j++)
                    {
                        ...
                        if ($('#workOrderChkbox_' + j).is(':checked') || updateIdDtls != "")
                        {
                            if ($('#hidden_fabric_source_' + j).val() == 3) {
                                if (form_validation('workOrderNo_' + j + '*quantity_' + j, 'WO*Qunatity') == false)
                                {
                                    return;
                                }
                            ...
                            i++;
                            //data_all+=get_submitted_data_string('workOrderNo_'+i+'*hideWoId_'+i+'*hideWoDtlsId_'+i+'*itemgroupid_'+i+'*itemdescription_'+i+'*colorName_'+i+'*sizeName_'+i+'*itemColor_'+i+'*itemSize_'+i+'*uom_'+i+'*quantity_'+i+'*rate_'+i+'*amount_'+i+'*updateIdDtls_'+i,"../../");

                            data_all += "&workOrderNo_" + i + "='" + $('#workOrderNo_' + j).val() + "'" + "&hideWoId_" + i + "='" + $('#hideWoId_' + j).val() + "'" + "&hideWoDtlsId_" + i + "='" + $('#hideWoDtlsId_' + j).val() + "'" + "&itemgroupid_" + i + "='" + $('#itemgroupid_' + j).val() + "'" + "&itemdescription_" + i + "='" + $('#itemdescription_' + j).val() + "'" + "&colorName_" + i + "='" + $('#colorName_' + j).val() + "'" + "&sizeName_" + i + "='" + $('#sizeName_' + j).val() + "'" + "&itemColor_" + i + "='" + $('#itemColor_' + j).val() + "'" + "&itemSize_" + i + "='" + $('#itemSize_' + j).val() + "'" + "&uom_" + i + "='" + $('#uom_' + j).val() + "'" + "&quantity_" + i + "='" + $('#quantity_' + j).val() + "'" + "&rate_" + i + "='" + $('#rate_' + j).val() + "'" + "&amount_" + i + "='" + $('#amount_' + j).val() + "'" + "&updateIdDtls_" + i + "='" + $('#updateIdDtls_' + j).val() + "'" + "&bookingWithoutOrder_" + i + "='" + $('#bookingWithoutOrder_' + j).val() + "'" + "&brandSupRef_" + i + "='" + $('#brandSupRef_' + j).val() + "'" + "&hidden_fabric_source_" + i + "='" + $('#hidden_fabric_source_' + j).val() + "'";
				...
				else
                {
                    ...
                    if (cbo_goods_rcv_status == 2)
                    {

                        for (var j = 1; j <= row_num; j++)
                        {
                            if (form_validation('itemgroupid_' + j + '*itemdescription_' + j + '*quantity_' + j + '*rate_' + j, 'Item Group*Item Description*Qunatity*Rate') == false)
                            {
                                //+'*colorName_'+j*Gmts Color
                                return;
                            }
                            i++;
                            data_all += get_submitted_data_string('itemgroupid_' + i + '*itemdescription_' + i + '*colorName_' + i + '*sizeName_' + i + '*itemColor_' + i + '*itemSize_' + i + '*uom_' + i + '*quantity_' + i + '*rate_' + i + '*amount_' + i + '*updateIdDtls_' + i + '*brandSupRef_' + i, "../../", i);
                    ...
                    } else
                    {
                        for (var j = 1; j <= row_num; j++)
                        {
                            if (form_validation('itemgroupid_' + j + '*quantity_' + j + '*rate_' + j, 'Item Group*Qunatity*Rate') == false)
                            {
                                //+'*colorName_'+j*Gmts Color
                                return;
                            }
			...
			else if (cbo_item_category_id == 12)
            {
                if (cbo_pi_basis_id == 1)
                {
                    for (var j = 1; j <= row_num; j++)
                    {
                        ...
                        if ($('#workOrderChkbox_' + j).is(':checked') || updateIdDtls != "")
                        {
                            ...
                            data_all += "&workOrderNo_" + i + "='" + $('#workOrderNo_' + j).val() + "'" + "&hideWoId_" + i + "='" + $('#hideWoId_' + j).val() + "'" + "&hideWoDtlsId_" + i + "='" + $('#hideWoDtlsId_' + j).val() + "'" + "&servicetype_" + i + "='" + $('#servicetype_' + j).val() + "'" + "&itemdescription_" + i + "='" + $('#itemdescription_' + j).val() + "'" + "&colorName_" + i + "='" + $('#colorName_' + j).val() + "'" + "&sizeName_" + i + "='" + $('#sizeName_' + j).val() + "'" + "&itemColor_" + i + "='" + $('#itemColor_' + j).val() + "'" + "&itemSize_" + i + "='" + $('#itemSize_' + j).val() + "'" + "&uom_" + i + "='" + $('#uom_' + j).val() + "'" + "&quantity_" + i + "='" + $('#quantity_' + j).val() + "'" + "&rate_" + i + "='" + $('#rate_' + j).val() + "'" + "&amount_" + i + "='" + $('#amount_' + j).val() + "'" + "&updateIdDtls_" + i + "='" + $('#updateIdDtls_' + j).val() + "'";
				...
				else
                {
                    for (var j = 1; j <= row_num; j++)
                    {
                        if (form_validation('servicetype_' + j + '*itemdescription_' + j + '*quantity_' + j + '*rate_' + j, 'Item Group*Item Description*Qunatity*Rate') == false)
                        {
                            //+'*colorName_'+j*Gmts Color
                            return;
                        }
                        i++;
                        data_all += get_submitted_data_string('servicetype_' + i + '*itemdescription_' + i + '*colorName_' + i + '*sizeName_' + i + '*itemColor_' + i + '*itemSize_' + i + '*uom_' + i + '*quantity_' + i + '*rate_' + i + '*amount_' + i + '*updateIdDtls_' + i, "../../", i);
            ...
            } else if (cbo_item_category_id == 24)
            {
                if (cbo_pi_basis_id == 1)
                {
                    for (var j = 1; j <= row_num; j++)
                    {
                        ...
                        if ($('#workOrderChkbox_' + j).is(':checked') || updateIdDtls != "")
                        {
                            ...
                            data_all += "&workOrderNo_" + i + "='" + $('#workOrderNo_' + j).val() + "'" + "&hideWoId_" + i + "='" + $('#hideWoId_' + j).val() + "'" + "&hideWoDtlsId_" + i + "='" + $('#hideWoDtlsId_' + j).val() + "'" + "&lot_" + i + "='" + $('#lot_' + j).val() + "'" + "&itemProdId_" + i + "='" + $('#itemProdId_' + j).val() + "'" + "&countName_" + i + "='" + $('#countName_' + j).val() + "'" + "&itemdescription_" + i + "='" + $('#itemdescription_' + j).val() + "'" + "&yarnColor_" + i + "='" + $('#yarnColor_' + j).val() + "'" + "&colorRange_" + i + "='" + $('#colorRange_' + j).val() + "'" + "&uom_" + i + "='" + $('#uom_' + j).val() + "'" + "&quantity_" + i + "='" + $('#quantity_' + j).val() + "'" + "&rate_" + i + "='" + $('#rate_' + j).val() + "'" + "&amount_" + i + "='" + $('#amount_' + j).val() + "'" + "&updateIdDtls_" + i + "='" + $('#updateIdDtls_' + j).val() + "'";
                ...
                } else
                {
                    for (var j = 1; j <= row_num; j++)
                    {
                        if (form_validation('lot_' + j + '*quantity_' + j + '*rate_' + j, 'Lot No*Qunatity*Rate') == false)
                        {
                            return;
                        }
                        i++;
                        data_all += get_submitted_data_string('lot_' + i + '*itemProdId_' + i + '*countName_' + i + '*itemdescription_' + i + '*yarnColor_' + i + '*colorRange_' + i + '*uom_' + i + '*quantity_' + i + '*rate_' + i + '*amount_' + i + '*updateIdDtls_' + i, "../../", i);

                        txt_total_amount += $('#amount_' + j).val() * 1;
                        selected_row++;
                    }
                }
            } else if (cbo_item_category_id == 25)
            {
                if (cbo_pi_basis_id == 1)
                {
                    for (var j = 1; j <= row_num; j++)
                ...
                } else
                {
                    for (var j = 1; j <= row_num; j++)
                    {
                        if (form_validation('gmtsitem_' + j + '*embellname_' + j + '*embelltype_' + j + '*quantity_' + j + '*rate_' + j, 'Gmts. Item*Embell Name*Embell Type*Qunatity*Rate') == false)
                        {
                            return;
                        }
                        i++;
                        data_all += get_submitted_data_string('gmtsitem_' + i + '*embellname_' + i + '*embelltype_' + i + '*colorName_' + i + '*uom_' + i + '*quantity_' + i + '*rate_' + i + '*amount_' + i + '*updateIdDtls_' + i, "../../", i);
			...
			else
            {
                ...
                } else
                {
                    for (var j = 1; j <= row_num; j++)
                    {
                        ...
                        i++;
                        data_all += get_submitted_data_string('itemgroupid_' + i + '*itemdescription_' + i + '*itemSize_' + i + '*uom_' + i + '*quantity_' + i + '*rate_' + i + '*amount_' + i + '*itemProdId_' + i + '*updateIdDtls_' + i, "../../", i);
        ...
        var data = "action=save_update_delete_dtls&operation=" + operation + '&cbo_item_category_id=' + cbo_item_category_id + '&cbo_pi_basis_id=' + cbo_pi_basis_id + '&total_row=' + i + '&update_id=' + update_id + '&txt_total_amount=' + txt_total_amount + '&txt_upcharge=' + txt_upcharge + '&txt_discount=' + txt_discount + '&txt_total_amount_net=' + txt_total_amount_net + '&txt_deleted_id=' + txt_deleted_id + '&cbo_currency_id=' + cbo_currency_id  + '&cbo_goods_rcv_status=' + cbo_goods_rcv_status + data_all;
		//alert(data);//return;
    ...
    function fnc_pi_item_details_reponse()
    {
        if (http.readyState == 4)
        {
            ...
            if ((reponse[0] == 0 || reponse[0] == 1 || reponse[0] == 2))
            {
                ...
                show_list_view(cbo_pi_basis_id + '_' + item_category + '_' + '2' + '_' + reponse[1], 'pi_details', 'pi_details_container', 'requires/pi_controller', '');
                var numRow = $('table#tbl_pi_item tbody tr').length;
                if (reponse[0] == 2)
                {
                    $('#txt_tot_row').val(0);
    ...
    function openmypage()
    {
		...
		else
        {
            ...
            var page_link = 'requires/pi_controller.php?item_category=' + item_category + '&importer_id=' + importer_id + '&supplier_id=' + supplier_id + '&action=pi_popup';
            ...
            emailwindow.onclose = function ()
            {
                ...
                {
                    freeze_window(5);
                    get_php_form_data(theemail.value, "populate_data_from_search_popup", "requires/pi_controller");
                    //alert(cbo_pi_basis_id);
					show_list_view($('#cbo_pi_basis_id').val()+'_'+item_category+'_'+'2'+'_'+theemail.value+'_'+$('#cbo_goods_rcv_status').val(),'pi_details','pi_details_container', 'requires/pi_controller','');
    ...
    function openmypage_fabricDescription(row_num)
    {
        ...
        var page_link = 'requires/pi_controller.php?action=fabricDescription_popup&fabricNature=' + fabricNature + "&prev_attached_id=" + prev_attached_id;
    ...
    function openmypage_wo(row_num)
    {
		...
		else
        {
            ...
            var page_link = 'requires/pi_controller.php?item_category=' + item_category + '&importer_id=' + importer_id + '&supplier_id=' + supplier_id +'&goods_rcv_status='+goods_rcv_status+'&prev_wo_ids='+prev_wo_ids+'&action=wo_popup';
            ...
            emailwindow.onclose = function ()
            {
                ...
                if (theemail.value != "")
                {
					...
					var data=theemail.value+"**"+item_category+"**"+tot_row+"**"+order_type+"**"+goods_rcv_status+"**"+importer_id;
					
					
					freeze_window(5);
					var list_view_wo =trim(return_global_ajax_value( data, 'populate_data_wo_form', '', 'requires/pi_controller'));
					if(list_view_wo!="")
					{
    ...
    function openmypage_item_desc(row_num)
    {
        ...
        } else
        {
            ...
            var page_link = 'requires/pi_controller.php?item_category=' + item_category + '&importer_id=' + importer_id + '&action=itemDesc_popup';
            ...
            emailwindow.onclose = function ()
            {
                ...
                if (theemail.value != "")
                {
                    ...
                    var list_view_wo = return_global_ajax_value(data, 'populate_data_item_form', '', 'requires/pi_controller');
                    if (item_category == 1)
                        var item_desc = $('#lot_' + row_num).val();
                    else
    ...
    function openmypage_exportPi()
    {
        ...
        var page_link = 'requires/pi_controller.php?action=export_pi_popup';
        ...
        emailwindow.onclose = function ()
        {
            ...
            if (theemail.value != "")
            {
                ...
                if (datas[18] * 1 > 0)
                {
                    show_list_view(datas[18] + "**" + datas[14] + "**" + datas[15] + "**" + datas[16] + "**" + datas[17], 'export_pi_details_update', 'pi_details_container', 'requires/pi_controller', '');
                    set_button_status(1, permission, 'fnc_pi_mst', 1);
                } else
                {
                    show_list_view(datas[0] + "**" + datas[14] + "**" + datas[15] + "**" + datas[16] + "**" + datas[17], 'export_pi_details', 'pi_details_container', 'requires/pi_controller', '');
                    ...
                    <table width="100%" border="0" cellpadding="0" cellspacing="2">
                        <tr height="25">
                            <td colspan="6" valign="middle" align="center" style="border-bottom:0px solid #666">
                                ...
                                <input type="text" name="txt_system_id" id="txt_system_id" style="width:140px" class="text_boxes" readonly/>
                        ...
                        <tr>
                            <td align="right" class="must_entry_caption" id="category_td">Item Category</td><input type="hidden" name="is_approved" id="is_approved" value="">
                        <td> 
                            <?php echo create_drop_down("cbo_item_category_id", 151, $item_category, '', 1, '--Select--', 0, "load_drop_down( 'requires/pi_controller',document.getElementById('cbo_importer_id').value+'_'+this.value, 'load_supplier_dropdown', 'supplier_td' );show_list_view(document.getElementById('cbo_pi_basis_id').value+'_'+this.value+'_'+'1', 'pi_details', 'pi_details_container', 'requires/pi_controller', '' ) ;", 0); ?>  
                        </td>
                        <td align="right" class="must_entry_caption">Importer</td>
                        <td id="importer_td">
                            <?php echo create_drop_down("cbo_importer_id", 151, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business in(1,3) $company_cond order by comp.company_name", 'id,company_name', 1, 'Select', 0, "load_drop_down( 'requires/pi_controller',this.value+'_'+document.getElementById('cbo_item_category_id').value, 'load_supplier_dropdown', 'supplier_td' );set_field_level_access(this.value);", 0); ?>       
                        </td>
                        ...
                        <tr>
                            ...
                            <td><input type="text" name="pi_number" id="pi_number" class="text_boxes" style="width:140px" placeholder="Double click for PI" onDblClick="openmypage()" maxlength="50" /></td>
                        ...
                        <tr>
                            ...
                            <td> 
                                <?php echo create_drop_down("cbo_source_id", 151, $source, '', 0, '', 0, 0); ?>                          
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td align="right">Indentor Name</td>
                            <td>
                                <?php echo create_drop_down("intendor_name", 151, "select a.id,a.supplier_name from lib_supplier a,lib_supplier_party_type b where a.id = b.supplier_id and b.party_type = 40 group by  a.id,a.supplier_name order by a.supplier_name", 'id,supplier_name', 1, 'Select', 0, 0, 0); ?>       
                            </td>
                        </tr>
                        <tr>
                            <td align="right" class="must_entry_caption">PI Basis</td>
                            <td>
                                <?php echo create_drop_down("cbo_pi_basis_id", 151, $pi_basis, '', 0, '', 2, "show_list_view(this.value+'_'+document.getElementById('cbo_item_category_id').value+'_'+'1', 'pi_details', 'pi_details_container', 'requires/pi_controller', '' ) ;", 0); ?>  
                            </td>
                            <td align="right" class="must_entry_caption">Goods Rcv Status</td>
                            <td>
                                <?php //echo create_drop_down( "cbo_goods_rcv_status",152,$acceptance_time,'',0,'',"show_list_view(this.value+'_'+document.getElementById('cbo_item_category_id').value+'_'+'1', 'pi_details', 'pi_details_container', 'requires/pi_controller', '' ) ;",0,0);  ?>  

                                <?php echo create_drop_down("cbo_goods_rcv_status", 151, $acceptance_time, '', 0, '', 2, "", 0); ?>  

                            </td>
                            <td align="right">LC Group No.</td>
                            <td>
                                <input type="text" id="txt_lc_group_no" name="txt_lc_group_no" style="width:140px" class="text_boxes_numeric" />
                            </td> 
                        </tr>
                        <tr>
                            <td align="right">Remarks</td>
                            <td colspan="3">
                                <input type="text" name="txt_remarks" id="txt_remarks"  style="width:88%;"  class="text_boxes" />
                            </td>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" name="txt_requested_by" id="txt_requested_by"  style="width:140px;"  class="text_boxes" />
                            </td>
                            <td align="right" colspan="4">&nbsp;</td>  
                        </tr>
                        <tr>
                            <td align="right" colspan="2">
                                <input type="button" id="image_button" class="image_uploader" style="width:152px; float:left; margin-left:120px;" value="CLICK TO ADD FILE" onClick="file_uploader('../../', document.getElementById('update_id').value, '', 'proforma_invoice', 2, 1)" />
                            </td>
                        </tr>
                        <tr>
