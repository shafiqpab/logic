...
<script>
	...
	function open_invoice_popup(page_link,title)
	{
		...
		emailwindow.onclose=function()
		{
			...
			{
				freeze_window(5);
			    reset_form('importpayment_1','check_posted_account_td*grid_show','','','');
	...
	function open_system_popup(page_link,title)
	{
		...
		emailwindow.onclose=function()
		{
			...
			if (system_no!="")
			{
				...
				get_php_form_data(lc_id+'_'+invoice_id, "populate_data_from_btb_lc", "requires/import_payment_controller" );
				get_php_form_data(system_id, "populate_data_from_mst", "requires/import_payment_controller" );
				show_list_view(system_id,'create_payment_list_view','grid_show','requires/import_payment_controller','');
				release_freezing();
			}
		}
	}
...
function check_conversion_rate(){
	...
	else
	{
		...
		var response=return_global_ajax_value( cbo_currercy+"**"+payment_date+"**"+company_id, 'check_conversion_rate', '', 'requires/import_payment_controller');
                ...
                <fieldset style="width:1024px;">
                    ...
                    <table width="100%" border="0" cellpadding="0" cellspacing="2" id="tbl_importdocumentacceptance">
                        <tr height="10">
                            ...
                            <td width="" colspan="3"><input type="text" name="txt_system_no" id="txt_system_no" class="text_boxes" placeholder="Double Click for Invoice No" onDblClick="open_system_popup( 'requires/import_payment_controller.php?action=open_system_popup','System No' );" readonly style="width:140px" /></td>
                        </tr>
                        <tr> 
                ...
                <fieldset style="width:1024px;">
                    ...
                    <table width="100%" border="0" cellpadding="0" cellspacing="2" id="tbl_importdocumentacceptance">
                        ...
                        <tr> 
                            ...
                            <td>	
                                <?php echo create_drop_down( "cbo_adj_source", 152,$commercial_head,'', 1, '-Select-','','adjustRef_validation(this.value)','','5,6,10,11,15,16,30,31,32,33,34,35,36,71,75,76,80,81,82,83,188,170,171'); ?>  
                            </td>
                        ...
                        <tr> 
                            ...
                            <td>
							<input type="button" id="image_button" class="image_uploader" style="width:152px;" value="CLICK TO ADD FILE" onClick="file_uploader( '../../', document.getElementById('update_id').value,'', 'import_payment',2,1)" />
                        ...
                        <tr>
                    		<td colspan="6" valign="middle" align="center" class="button_container">						
								<? echo load_submit_buttons( $permission, "fnc_import_payment", 0,0 ,"reset_form('importpayment_1','check_posted_account_td*grid_show','','','')",1) ; ?>
