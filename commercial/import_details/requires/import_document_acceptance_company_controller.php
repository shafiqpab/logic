				...
				<legend>Enter search words</legend>
					<table cellpadding="0" cellspacing="0" width="1040" class="rpt_table" border="1" rules="all">
						...
						<tr class="general">
							...
							<?
							//
									echo create_drop_down( "txt_company_id",150,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, 'Select',0,"load_drop_down( 'import_document_acceptance_company_controller',this.value+'_'+document.getElementById('cbo_item_category_id').value,'load_supplier_dropdown','supplier_td' );",0);
								?>
							</td>
							<td>
							...
							<td>
								<? echo create_drop_down( "cbo_item_category_id", 150, $item_category_with_gen,'', 1, '--Select--',0,"load_drop_down( 'import_document_acceptance_company_controller',document.getElementById('txt_company_id').value+'_'+this.value,'load_supplier_dropdown','supplier_td' );",0); ?>
							</td>
							...
							<td>
								<input type="button" id="search_button" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_company_id').value+'**'+document.getElementById('cbo_item_category_id').value+'**'+document.getElementById('cbo_supplier_id').value+'**'+document.getElementById('btb_start_date').value+'**'+document.getElementById('btb_end_date').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('cbo_bank_name').value, 'create_btb_search_list_view', 'search_div', 'import_document_acceptance_company_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" />
							</td>
						</tr>
				</table>
				<br>
...
if($action=="create_btb_search_list_view")
{
	...
	$query = "SELECT a.id,a.btb_prefix_number, a.btb_system_id, a.issuing_bank_id, a.lc_number, a.supplier_id, a.application_date, a.last_shipment_date, a.lc_date, a.lc_value,a.supplier_id, a.importer_id, a.payterm_id, a.ref_closing_status, $year_field, rtrim(xmlagg(xmlelement(e,c.item_category_id,',').extract('//text()') order by c.item_category_id).GetClobVal(),',') AS item_category_id 
	...
	Group By a.id,a.btb_prefix_number, a.btb_system_id, a.issuing_bank_id, a.lc_number, a.supplier_id, a.application_date, a.last_shipment_date, a.lc_date, a.lc_value, a.supplier_id, a.importer_id, a.payterm_id,a.ref_closing_status,a.insert_date  
	order by a.id";
	//echo $query;die;
            ...
            foreach($sql as $row)
            {
                ...
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value('<? echo $row[csf('id')]."_".$row[csf('importer_id')]."_".$row[csf('ref_closing_status')];?>')">
                    ...
                    <td width="130"><p><?
                    					$itemCategory="";
										$l=1;
										$cat_id_arr=array_unique(explode(",",$row[csf('item_category_id')]->load()));
				...
				<legend>Enter search words</legend>
					<table cellpadding="0" cellspacing="0" width="950" class="rpt_table">
						...
						<tr class="general">
							<td>
							<?
									echo create_drop_down( "txt_company_id",120,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, 'Select',0,"load_drop_down( 'import_document_acceptance_company_controller',this.value+'_'+document.getElementById('cbo_item_category_id').value,'load_supplier_dropdown','supplier_td' );",0);
								?>
							</td>
							<td>
							...
							<td>
								<? echo create_drop_down( "cbo_item_category_id", 100, $item_category_with_gen,'', 1, '--Select--',0,"load_drop_down( 'import_document_acceptance_company_controller',document.getElementById('txt_company_id').value+'_'+this.value,'load_supplier_dropdown','supplier_td' );",0); ?>
							</td>
							<td align="center" id="supplier_td">
							<? echo create_drop_down( "cbo_supplier_id", 165,$blank_array,'', 1, '--Select--',0,0,0); ?>
...
//	WHERE a.id=b.btb_lc_id and  a.importer_id = '".$company."' and a.supplier_id like '".$supplier."' and issuing_bank_id like '".$bnk_name."' and is_lc=1 $date and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 $invoice_cond $lc_cond";
...
if($action=='populate_data_from_btb_lc')
{
	...
	foreach ($data_array as $row)
	{
		if($data[1]==1) //Lc popup
		{
			...
			{
				echo "load_drop_down( 'requires/import_document_acceptance_company_controller', '".$row[csf('importer_id')]."'+'_'+'".$row[csf('item_category_id')]."', 'load_supplier_dropdown', 'supplier_td');\n";
...
{
	$data_array=sql_select("select id,btb_lc_id,invoice_no,invoice_date,document_value,shipment_date,company_acc_date,bank_acc_date,bank_ref,acceptance_time,retire_source,remarks,edf_tenor,bill_no,bill_date,shipment_mode,document_status,forwarder_name,copy_doc_receive_date,original_doc_receive_date,doc_to_cnf,feeder_vessel,mother_vessel,eta_date,ic_receive_date,shipping_bill_no,inco_term,inco_term_place,port_of_loading,port_of_discharge,bill_of_entry_no,bill_of_entry_date,psi_reference_no,maturity_date,container_no,pkg_quantity, pkg_quantity_breakdown, nagotiate_date, edf_paid_date, exchange_rate,loan_ref,etd_actual,pkg_uom,eta_advice,eta_actual,container_status,container_size,release_date, doc_rcv_date, local_doc_send_date, ready_to_approved, approved, bill_entry_value, submission_date,courier_no,courier_date,source from com_import_invoice_mst where id='$data'");
	//nagotiate_date
	foreach ($data_array as $row)
	{
			...
			echo "document.getElementById('txt_courier_no').value 	= '".$row[csf("courier_no")]."';\n";
			echo "document.getElementById('txt_courier_date').value 	= '".change_date_format($row[csf('courier_date')])."';\n";
...
if ($action=="save_update_delete")
{
	...
	else if ($operation==1)   // Update Here
	{
		...
		if($cbo_payterm_id!=1)
		{
			...
			{
				$field_array_up="bill_no*bill_date*shipment_mode*document_status*forwarder_name*copy_doc_receive_date*original_doc_receive_date*doc_to_cnf*feeder_vessel*mother_vessel*eta_date*ic_receive_date*shipping_bill_no*inco_term*inco_term_place*port_of_loading*port_of_discharge*bill_of_entry_no*bill_of_entry_date*psi_reference_no*maturity_date*edf_paid_date*container_no*pkg_quantity*pkg_quantity_breakdown*exchange_rate*loan_ref*etd_actual*pkg_uom*eta_advice*eta_actual*container_status*container_size*release_date*updated_by*update_date*doc_rcv_date*local_doc_send_date*bill_entry_value*submission_date*courier_no*courier_date*source";
				$data_array_up ="".$bill_no."*".$bill_date."*".$cbo_shipment_mode."*".$cbo_document_status."*".$cbo_forwarder_name."*".$copy_doc_receive_date."*".$original_doc_receive_date."*".$doc_to_cnf."*".$feeder_vessel."*".$mother_vessel."*".$eta_date."*".$ic_receive_date."*".$shipping_bill_no."*".$cbo_inco_term."*".$inco_term_place."*".$port_of_loading."*".$port_of_discharge."*".$bill_of_entry_no."*".$boe_date."*".$psi_reference_no."*".$maturity_date."*".$edf_paid_date."*".$container_no."*".$pkg_quantity."*".$pkg_quantity_breakdown."*".$txt_exchange_rate."*".$txt_loan_reff."*".$etd_actual."*".$pakg_uom."*".$eta_advice."*".$eta_actual."*".$cbo_container_status."*".$cbo_container_size."*".$release_date."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$txt_doc_rcv_date."*".$txt_local_date."*".$txt_bill_entry_value."*".$txt_submit_date."*".$txt_courier_no."*".$txt_courier_date."*".$cbo_source_id."";
			}
			else
			{
				$field_array_up="btb_lc_id*is_lc*invoice_no*invoice_date*document_value*shipment_date*company_acc_date*bank_acc_date*bank_ref*acceptance_time*retire_source*nagotiate_date*remarks*edf_tenor*bill_no*bill_date*shipment_mode*document_status*forwarder_name*copy_doc_receive_date*original_doc_receive_date*doc_to_cnf*feeder_vessel*mother_vessel*eta_date*ic_receive_date*shipping_bill_no*inco_term*inco_term_place*port_of_loading*port_of_discharge*bill_of_entry_no*bill_of_entry_date*psi_reference_no*maturity_date*edf_paid_date*container_no*pkg_quantity*pkg_quantity_breakdown*exchange_rate*loan_ref*etd_actual*pkg_uom*eta_advice*eta_actual*container_status*container_size*release_date*updated_by*update_date*doc_rcv_date*local_doc_send_date*ready_to_approved*bill_entry_value*submission_date*courier_no*courier_date*source";
		...
		else
		{
			...
			{
				$field_array_up="bill_no*bill_date*shipment_mode*document_status*forwarder_name*copy_doc_receive_date*original_doc_receive_date*doc_to_cnf*feeder_vessel*mother_vessel*eta_date*ic_receive_date*shipping_bill_no*inco_term*inco_term_place*port_of_loading*port_of_discharge*bill_of_entry_no*bill_of_entry_date*psi_reference_no*maturity_date*edf_paid_date*container_no*pkg_quantity*pkg_quantity_breakdown*exchange_rate*loan_ref*etd_actual*pkg_uom*eta_advice*eta_actual*container_status*container_size*release_date*updated_by*update_date*bill_entry_value*submission_date*courier_no*courier_date*source";
				$data_array_up ="".$bill_no."*".$bill_date."*".$cbo_shipment_mode."*".$cbo_document_status."*".$cbo_forwarder_name."*".$copy_doc_receive_date."*".$original_doc_receive_date."*".$doc_to_cnf."*".$feeder_vessel."*".$mother_vessel."*".$eta_date."*".$ic_receive_date."*".$shipping_bill_no."*".$cbo_inco_term."*".$inco_term_place."*".$port_of_loading."*".$port_of_discharge."*".$bill_of_entry_no."*".$boe_date."*".$psi_reference_no."*".$maturity_date."*".$edf_paid_date."*".$container_no."*".$pkg_quantity."*".$pkg_quantity_breakdown."*".$txt_exchange_rate."*".$txt_loan_reff."*".$etd_actual."*".$pakg_uom."*".$eta_advice."*".$eta_actual."*".$cbo_container_status."*".$cbo_container_size."*".$release_date."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$txt_bill_entry_value."*".$txt_submit_date."*".$txt_courier_no."*".$txt_courier_date.",".$cbo_source_id."";
			}
			else
			{
				$field_array_up="btb_lc_id*is_lc*invoice_no*invoice_date*document_value*shipment_date*company_acc_date*bank_acc_date*bank_ref*acceptance_time*retire_source*nagotiate_date*remarks*edf_tenor*bill_no*bill_date*shipment_mode*document_status*forwarder_name*copy_doc_receive_date*original_doc_receive_date*doc_to_cnf*feeder_vessel*mother_vessel*eta_date*ic_receive_date*shipping_bill_no*inco_term*inco_term_place*port_of_loading*port_of_discharge*bill_of_entry_no*bill_of_entry_date*psi_reference_no*maturity_date*edf_paid_date*container_no*pkg_quantity*pkg_quantity_breakdown*exchange_rate*loan_ref*etd_actual*pkg_uom*eta_advice*eta_actual*container_status*container_size*release_date*updated_by*update_date*ready_to_approved*bill_entry_value*submission_date*courier_no*courier_date*source";
				$data_array_up ="".$btb_lc_id."*1*".$txt_invoice_number."*".$txt_invoice_date."*".$txt_document_value."*".$txt_shipment_date."*".$txt_company_acc_date."*".$txt_bank_acceptance_date."*".$txt_bank_ref."*".$cbo_acceptance_time."*".$cbo_retire_source."*".$nagotiate_date."*".$txt_remarks."*".$txt_edf_tenor."*".$bill_no."*".$bill_date."*".$cbo_shipment_mode."*".$cbo_document_status."*".$cbo_forwarder_name."*".$copy_doc_receive_date."*".$original_doc_receive_date."*".$doc_to_cnf."*".$feeder_vessel."*".$mother_vessel."*".$eta_date."*".$ic_receive_date."*".$shipping_bill_no."*".$cbo_inco_term."*".$inco_term_place."*".$port_of_loading."*".$port_of_discharge."*".$bill_of_entry_no."*".$boe_date."*".$psi_reference_no."*".$maturity_date."*".$edf_paid_date."*".$container_no."*".$pkg_quantity."*".$pkg_quantity_breakdown."*".$txt_exchange_rate."*".$txt_loan_reff."*".$etd_actual."*".$pakg_uom."*".$eta_advice."*".$eta_actual."*".$cbo_container_status."*".$cbo_container_size."*".$release_date."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$cbo_ready_to_approved."*".$txt_bill_entry_value."*".$txt_submit_date."*".$txt_courier_no."*".$txt_courier_date."*".$cbo_source_id."";
			}
		}
		...
		else
		{
			...
			$rID=sql_update("com_import_invoice_mst",$field_array_up,$data_array_up,"id","".$invoice_id."",0);
			//echo "10**test = $field_array_up = $data_array_up = $invoice_id".$rID;oci_rollback($con);disconnect($con);die;
...
if($action=="forwarding_letter4")
{
	...
	$sql="SELECT b.id as accep_dtls_id, b.current_acceptance_value, c.id as btb_id, c.lc_number as btb_lc_no, c.importer_id, c.issuing_bank_id,c.currency_id, c.supplier_id, c.lc_date as btb_lc_date, c.lc_value as btb_lc_value, a.company_acc_date
	from com_import_invoice_mst a, com_import_invoice_dtls b, com_btb_lc_master_details c 
	where a.id=$data and a.id=b.import_invoice_id and b.btb_lc_id=c.id and a.status_active=1 and b.status_active=1 and c.status_active=1 ";
	 //echo $sql;die;
	$btb_sql=sql_select( $sql);
	foreach($btb_sql as $row)
	{
		...
		$importer   	= $company_arr[$row[csf("importer_id")]];
		$currency_name  = $currency[$row[csf("currency_id")]];
		$currency_sign  = $currency_arr[$row[csf("currency_id")]];
		$btb_lc_no		=$row[csf("btb_lc_no")];
		$btb_lc_date	=change_date_format($row[csf("btb_lc_date")]);
		$btb_lc_value	=$row[csf("btb_lc_value")];
		$supplier_id	=$row[csf("supplier_id")];
		if($exportPiSuppArr[$row[csf('btb_id')]]==1) $supplier_name=$company_arr[$supplier_id]; else $supplier_name=$suplier_arr[$supplier_id];
		if($row[csf("company_acc_date")]!="")$company_acc_date=change_date_format($row[csf("company_acc_date")]);	

		$bank_name		=$bank_dtls_arr[$row[csf('issuing_bank_id')]]['BANK_NAME'];
		$bank_branch	=$bank_dtls_arr[$row[csf('issuing_bank_id')]]['BRANCH_NAME'];
		$bank_address	=$bank_dtls_arr[$row[csf('issuing_bank_id')]]['ADDRESS'];
		$bank_designation=$designation_arr[$bank_dtls_arr[$row[csf('issuing_bank_id')]]['DESIGNATION']];
    ...
    <style>
	 ...
	 <table width="750" cellpadding="0" align="left" cellspacing="0" border="0" style="font-size: 20px;">
		...
		<tr>
			<td width="25"></td>
			<td colspan="4" >Date: <? echo $company_acc_date;?></td>
		</tr>
		<tr>
			<td width="25"> </td>
		...
		<tr>
			<td width="25"></td>
			<td width="50" valign="top">Sub: </td>
			<td width="625" colspan="2">Request to confirm Maturity date under L/C No. <?=$btb_lc_no;?> Date: <?=$btb_lc_date;?> For <?=$currency_name."".$currency_sign." ".number_format($current_acceptance_value,2)." ".$supplier_name;?></td>
			<td></td>
		</tr>
		<tr>
			<td width="25"></td>
			<td width="50"></td>
			<td width="675" colspan="2"><hr><hr></td>
			<td></td>
		</tr>
        <tr><td colspan="4">&nbsp;</td></tr>
		<tr>
		...
		<tr>
			<td width="25"></td>
			<td width="675" colspan="3" style="text-align: justify;">With reference to the above, we are requesting to you please confirm the maturity date against BB L/C No. <?=$btb_lc_no;?> Date: <?=$btb_lc_date;?> For Value of <?=$currency_name."".$currency_sign." ".number_format($current_acceptance_value,2);?>
			</td>
			<td></td>
		</tr>
        ...
        <tr>
			<td width="25"></td>
			<td width="675" colspan="3">At your end to the beneficiary. <b><i>" We accept the documents despite any discrepancy "</i></b></td>
			<td></td>
		</tr>
		<tr><td colspan="4"  height="25">&nbsp;</td></tr>
        <tr>
			<td width="25"></td>
			<td width="675" colspan="3">Your kind co-operation & necessary action will highly be appreciated. </td>
		</tr>
		<tr><td colspan="4"  height="75">&nbsp;</td></tr>
        <tr>
			<td width="25"></td>
			<td width="675" colspan="3">Thanking You,</br>Yours faithfully.</td>
		</tr>
	</table>
	<?
	exit();
...
if($action=="forwarding_letter5")
{
    ...
    <style>
	 ...
	 <table width="750" cellpadding="0" align="left" cellspacing="0" border="0" style="font-size: 20px;">
		...
		<tr>
			<td width="25"></td>
			<td width="675" >Subject: Acceptance of Document </td>
			<td></td>
		</tr>
        <tr><td colspan="4">&nbsp;</td></tr>
		<tr>
		...
		<tr>
			<td width="25"></td>
			<td width="675" colspan="3" style="text-align: justify;">
			Referring to the captioned subject we have agreed to accept the documents under L/C No: <?=$btb_lc_no;?> Dated: <?=$btb_lc_date;?> for <?=$currency_name."".$currency_sign." ".number_format($current_acceptance_value,2);?> and would request you to kindly process the documents & accordingly arrange to hand over the accompanying documents with a necessary endorsement to clear the respective consignment. This is Noteworthy that we have agreed to accept the documents despite having discrepancies. Please intimate the acceptance to the presenting bank at the earliest.
			</td>
			<td></td>
		</tr>
		<tr><td colspan="4"  height="75">&nbsp;</td></tr>
        <tr>
			<td width="25"></td>
			<td width="675" colspan="3">Thanking You,</br></br>Yours faithfully</td>
		</tr>
	</table>
	<?
	exit();
