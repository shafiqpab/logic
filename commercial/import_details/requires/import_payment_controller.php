            ...
            <legend>Enter search words</legend>           
            	<table cellpadding="0" cellspacing="0" width="1030" class="rpt_table">
                    ...
                    <tr class="general">
                        ...
                        <td>
                           <? 
								echo create_drop_down( "txt_company_id",130,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name",'id,company_name', 1, 'Select',0,"load_drop_down( 'import_payment_controller',this.value+'_'+document.getElementById('cbo_item_category_id').value,'load_supplier_dropdown','supplier_td' );",0); 
							?>  
                         </td>
...
if($action=="create_invoice_list_view")
{
	...
	$accepted_ammount_arr =return_library_array("select invoice_id, sum(accepted_ammount) as accepted_ammount from com_import_payment where status_active = 1 and is_deleted = 0 and payment_head=40 group by invoice_id","invoice_id","accepted_ammount");
            	...
            	<table cellpadding="0" cellspacing="0" width="1000" class="rpt_table">
                    ...
                    <tr class="general">
						...
						<td> 
                         ...
                         <td>
                 		  	<input type="button" id="search_button" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_company_id').value+'**'+document.getElementById('cbo_item_category_id').value+'**'+document.getElementById('cbo_supplier_id').value+'**'+document.getElementById('btb_start_date').value+'**'+document.getElementById('btb_end_date').value+'**'+document.getElementById('txt_bank_ref').value+'**'+document.getElementById('cbo_issue_bank').value+'**'+document.getElementById('txt_systen_no').value, 'create_system_list_view', 'search_div', 'import_payment_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />
                         </td>
					</tr>
               </table>
               <table width="100%" style="margin-top:5px" align="center">
...
if($action=='populate_data_from_btb_lc')
{
	...
	{ 
		...
		$accepted_ammount =return_field_value("sum(accepted_ammount) as accepted_ammount","com_import_payment"," status_active = 1 and is_deleted = 0 and payment_head=40 and invoice_id=$data[1] ","accepted_ammount");
...
if($action=='populate_data_from_mst')
{
	...
	$data_array=sql_select("select payment_date from com_import_payment_mst where id=$data");
	foreach ($data_array as $row)
	{ 
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$dtls_id=return_next_id( "id", "com_import_payment", 1 ) ;
		...
		if(str_replace("'","",$txt_system_no)=="")
		{
			...
			//echo "10**$rID**insert into com_import_payment_mst($field_array)values".$data_array;die;
		}
		else
		{
			$rID=sql_update("com_import_payment_mst",$field_array,$data_array,"id",$id,0);	
		}
		
		//	echo "<br/>10**insert into com_import_payment($field_array1)values".$data_array1;die;
		$rID1=sql_insert("com_import_payment",$field_array1,$data_array1,1);
	...
	else if ($operation==1)   // Update Here
	{
		...
		$rID1=sql_update("com_import_payment_mst",$field_array_mst,$data_array_mst,"id","".$txt_system_id."",1);
		
		$rID2=execute_query("update com_import_payment set payment_date=".$import_payment_date." where mst_id=".$txt_system_id." ",0);
...
if($action=="set_head_cum_value")
{
	...
	$accepted_ammount =return_field_value("sum(accepted_ammount) as accepted_ammount","com_import_payment"," status_active = 1 and is_deleted = 0 and invoice_id=$data[0] and payment_head =$data[1]","accepted_ammount");
