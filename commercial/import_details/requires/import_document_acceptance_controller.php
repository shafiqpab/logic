					...
					<table cellpadding="0" cellspacing="0" width="1040" class="rpt_table" border="1" rules="all">
						...
						<tr class="general">
							...
							// and comp.core_business not in(3)
									echo create_drop_down( "txt_company_id",150,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, 'Select',0,"load_drop_down( 'import_document_acceptance_controller',this.value+'_'+document.getElementById('cbo_item_category_id').value,'load_supplier_dropdown','supplier_td' );",0);
								?>
							</td>
							<td>
							...
							<td>
								<? echo create_drop_down( "cbo_item_category_id", 150, $item_category_with_gen,'', 1, '--Select--',0,"load_drop_down( 'import_document_acceptance_controller',document.getElementById('txt_company_id').value+'_'+this.value,'load_supplier_dropdown','supplier_td' );",0); ?>
							</td>
							...
							<td>
								<input type="button" id="search_button" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_company_id').value+'**'+document.getElementById('cbo_item_category_id').value+'**'+document.getElementById('cbo_supplier_id').value+'**'+document.getElementById('btb_start_date').value+'**'+document.getElementById('btb_end_date').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('cbo_bank_name').value, 'create_btb_search_list_view', 'search_div', 'import_document_acceptance_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" />
							</td>
						</tr>
				</table>
				<br>
...
if($action=="create_btb_search_list_view")
{
	...
	$exportPiSupp = sql_select("select c.import_pi, a.id, c.item_category_id from com_btb_lc_master_details a, com_btb_lc_pi b, com_pi_master_details c where a.id = b.com_btb_lc_master_details_id and b.pi_id = c.id and a.importer_id like '".$company."'");
	foreach ($exportPiSupp as $row)
	{
		...
		$exportPiSuppArr[$row[csf("id")]] = $row[csf("import_pi")];
	}
	//echo $query;die;
	//$arr=array(2=>$bank_array,3=>$supplier_lib,4=>$item_category,10=>$pay_term);
	//echo  create_list_view("list_view", "Sys Id,Year,Bank Name,Supplier,Category,L/C Number,L/C Date,L/C Value,Application Date,Last Ship Date,Pay Term", "50,50,110,130,100,110,70,100,70,70","1045","300",0, $sql , "js_set_value", "id,importer_id", "", 1, "0,0,issuing_bank_id,supplier_id,item_category_id,0,0,0,0,0,payterm_id", $arr , "btb_prefix_number,year,issuing_bank_id,supplier_id,item_category_id,lc_number,lc_date,lc_value,application_date,last_shipment_date,payterm_id", "",'','0,0,0,0,0,0,3,2,3,3,0') ;
				...
				<legend>Enter search words</legend>
					<table cellpadding="0" cellspacing="0" width="1000" class="rpt_table">
						...
						<tr class="general">
							...
							<?
								//and comp.core_business not in(3)
								echo create_drop_down( "txt_company_id",120,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0  $company_cond order by comp.company_name",'id,company_name', 1, 'Select',0,"load_drop_down( 'import_document_acceptance_controller',this.value+'_'+document.getElementById('cbo_item_category_id').value,'load_supplier_dropdown','supplier_td' );",0);
								?>
							</td>
							<td>
							...
							<td>
								<? echo create_drop_down( "cbo_item_category_id", 100, $item_category_with_gen,'', 1, '--Select--',0,"load_drop_down( 'import_document_acceptance_controller',document.getElementById('txt_company_id').value+'_'+this.value,'load_supplier_dropdown','supplier_td' );",0); ?>
							</td>
							<td align="center" id="supplier_td">
							<? echo create_drop_down( "cbo_supplier_id", 165,$blank_array,'', 1, '--Select--',0,0,0); ?>
							...
							<td>
								<input type="text" name="txt_pi_no" id="txt_pi_no" class="text_boxes" style="width:65px;" />
							</td>
							<td>
							...
							<td>
								<input type="button" id="search_button" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_company_id').value+'**'+document.getElementById('cbo_item_category_id').value+'**'+document.getElementById('cbo_supplier_id').value+'**'+document.getElementById('btb_start_date').value+'**'+document.getElementById('btb_end_date').value+'**'+document.getElementById('cbo_bank_name').value+'**'+document.getElementById('txt_inv_no').value+'**'+document.getElementById('txt_lc_no').value+'**'+document.getElementById('txt_pi_no').value, 'create_invoice_list_view', 'search_div', 'import_document_acceptance_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:80px;" />
							</td>
						</tr>
				</table>
...
if($action=="create_invoice_list_view")
{
	...
	if($txt_pi_no!="") $pi_con=" and c.pi_number like '%$txt_pi_no%'"; else $pi_con='';
	...
	$exportPiSupp = sql_select("select c.import_pi, a.id from com_btb_lc_master_details a , com_btb_lc_pi b , com_pi_master_details c where a.id = b.com_btb_lc_master_details_id and b.pi_id = c.id $pi_con");
...
if($action=='populate_data_from_btb_lc')
{
	...
	// $file_no_chk_sql = sql_select("SELECT a.id,a.lc_number,b.internal_file_no from com_btb_lc_master_details a,com_export_lc b WHERE a.id = '$data[0]' and a.lc_number = b.export_lc_no
	// and a.STATUS_ACTIVE =1 and a.is_deleted = 0 and b.STATUS_ACTIVE =1 and b.is_deleted = 0 ");
	// foreach($file_no_chk_sql as $row){
	// 	$file_no_arr[$row[csf('lc_number')]]= $row[csf('internal_file_no')];
	// }
	...
	$print_report_format=return_field_value("format_id","lib_report_template","template_name ='".$data_array[0][csf('importer_id')]."' and module_id=5 and report_id=189 and is_deleted=0 and status_active=1");
	$printButton=explode(',',$print_report_format);
		echo "document.getElementById('letter_print').style.display = 'none';\n";
		echo "document.getElementById('forwarding_letter2').style.display = 'none';\n";
		echo "document.getElementById('forwarding_letter3').style.display = 'none';\n";
		echo "document.getElementById('advance_payment_letter').style.display = 'none';\n";
		echo "document.getElementById('advance_acceptance').style.display = 'none';\n";
		echo "document.getElementById('regular_acceptance_letter').style.display = 'none';\n";
		echo "document.getElementById('btb_acceptance_letter').style.display = 'none';\n";
		echo "document.getElementById('undertaking_ific').style.display = 'none';\n";
		echo "document.getElementById('payment_letter').style.display = 'none';\n";
	foreach($printButton as $id){
		if($id==678){echo "document.getElementById('letter_print').style.display = 'inline';\n";}
		if($id==395){echo "document.getElementById('forwarding_letter2').style.display = 'inline';\n";}
		if($id==396){echo "document.getElementById('forwarding_letter3').style.display = 'inline';\n";}
		if($id==397){echo "document.getElementById('advance_payment_letter').style.display = 'inline';\n";}
		if($id==398){echo "document.getElementById('advance_acceptance').style.display = 'inline';\n";}
		if($id==399){echo "document.getElementById('regular_acceptance_letter').style.display = 'inline';\n";}
		if($id==400){echo "document.getElementById('btb_acceptance_letter').style.display = 'inline';\n";}
		if($id==401){echo "document.getElementById('undertaking_ific').style.display = 'inline';\n";}
		if($id==402){echo "document.getElementById('payment_letter').style.display = 'inline';\n";}
	}
	foreach ($data_array as $row)
	{
		//$intr_file_no = $file_no_arr[$row[csf("lc_number")]];
		if($data[1]==1) //Lc popup
		{
			...
			{
				echo "load_drop_down( 'requires/import_document_acceptance_controller', '".$row[csf('importer_id')]."'+'_'+'".$row[csf('item_category_id')]."', 'load_supplier_dropdown', 'supplier_td');\n";
			...
			echo "$('#txt_file_no').val( '$internal_file_no' );\n";
...
{
	$data_array=sql_select("select id,btb_lc_id,invoice_no,invoice_date,document_value,shipment_date,company_acc_date,bank_acc_date,bank_ref,acceptance_time,retire_source,remarks,edf_tenor,bill_no,bill_date,shipment_mode,document_status,forwarder_name,copy_doc_receive_date,original_doc_receive_date,doc_to_cnf,feeder_vessel,mother_vessel,eta_date,ic_receive_date,shipping_bill_no,inco_term,inco_term_place,port_of_loading,port_of_discharge,bill_of_entry_no,bill_of_entry_date,psi_reference_no,maturity_date,container_no,pkg_quantity, nagotiate_date, edf_paid_date, exchange_rate,loan_ref,etd_actual,pkg_uom,eta_advice,eta_actual,container_status,container_size,release_date, doc_rcv_date, ready_to_approved, approved, bill_entry_value, submission_date, is_posted_account, btmea_no, btmea_date, btmea_qnty, source from com_import_invoice_mst where id='$data'");
	//nagotiate_date
	foreach ($data_array as $row)
	{
		...
		echo "document.getElementById('txt_btmea_no').value 	= '".$row[csf("btmea_no")]."';\n";
		echo "document.getElementById('txt_btmea_date').value 	= '".($row[csf('btmea_date')] == '0000-00-00' || $row[csf('btmea_date')] == '' ? '' : change_date_format($row[csf('btmea_date')]))."';\n";
		echo "document.getElementById('txt_btmea_qnty').value 	= '".$row[csf("btmea_qnty")]."';\n";
...
if ($action=="save_update_delete")
{
	...
	$sql_btb="select IMPORTER_ID, LC_VALUE, TOLERANCE from com_btb_lc_master_details where id=$btb_lc_id";
	$sql_btb_result=sql_select($sql_btb);
	$importer_id=$sql_btb_result[0]["IMPORTER_ID"];
	$btb_lc_values=$sql_btb_result[0]["LC_VALUE"];
	$btb_tolerance=$sql_btb_result[0]["TOLERANCE"];
	$tolaret_lc_value=$btb_lc_values+($btb_lc_values*($btb_tolerance/100));
	//echo "30**$tolaret_lc_value";die;	
	
	$inv_cond="";
	if(str_replace("'","",$invoice_id)!="") $inv_cond=" and IMPORT_INVOICE_ID<>$invoice_id";
	$pre_acc_sql="select sum(CURRENT_ACCEPTANCE_VALUE) as CURRENT_ACCEPTANCE_VALUE from COM_IMPORT_INVOICE_DTLS where status_active=1 and is_deleted=0 and BTB_LC_ID=$btb_lc_id $inv_cond";
	$pre_acc_sql_result=sql_select($pre_acc_sql);
	$prev_acc_amt=$pre_acc_sql_result[0]["CURRENT_ACCEPTANCE_VALUE"];
	
	
	$tot_current_acceptance_value=str_replace("'","",$tot_current_acceptance_value);
	$tot_cu_acceptance_value=0;
	for ($i=1;$i<=$total_row;$i++)
	{
		$pi_id="pi_id_".$i;
		$current_acceptance_value="current_acceptance_value_".$i;
		$tot_cu_acceptance_value +=str_replace("'",'',$$current_acceptance_value);
	}
	$cu_lc_value=$prev_acc_amt+$tot_current_acceptance_value;
	$cu_lc_inv_value=$prev_acc_amt+$tot_cu_acceptance_value;
	if($cu_lc_inv_value>$tolaret_lc_value || $cu_lc_value>$tolaret_lc_value)
	{
		echo "30**Cumulative Invoice Value Not Allow More Then Tolerate LC Value\n Cumulative Invoice Value = $cu_lc_inv_value \n  Tolerate LC Value = $tolaret_lc_value";die;
	}
	...
	//is_posted_account
	//if(str_replace
	$pay_id=return_field_value("id","com_import_payment","invoice_id=$invoice_id and status_active=1 and is_deleted=0","id");
	...
	if ($operation==0)  // Insert Here
	{
		...
		if($cbo_payterm_id==1 && $payment_date!="")
		{
			...
			else if($db_type==0) $payment_date=change_date_format($payment_date,'yyyy-mm-dd');
	...
	else if ($operation==1)   // Update Here
	{
		...
		if($cbo_payterm_id!=1)
		{
			...
			if($hide_approved_status!=0 || $is_posted_account == 1)
			{
				//*exchange_rate
				//*".$txt_exchange_rate."
				$field_array_up="bank_acc_date*bill_no*bill_date*shipment_mode*document_status*forwarder_name*copy_doc_receive_date*original_doc_receive_date*doc_to_cnf*feeder_vessel*mother_vessel*eta_date*ic_receive_date*shipping_bill_no*inco_term*inco_term_place*port_of_loading*port_of_discharge*bill_of_entry_no*bill_of_entry_date*psi_reference_no*maturity_date*edf_paid_date*container_no*pkg_quantity*loan_ref*etd_actual*pkg_uom*eta_advice*eta_actual*container_status*container_size*release_date*updated_by*update_date*doc_rcv_date*bill_entry_value*submission_date*btmea_no*btmea_date*btmea_qnty*source";
				$data_array_up ="".$txt_bank_acceptance_date."*".$bill_no."*".$bill_date."*".$cbo_shipment_mode."*".$cbo_document_status."*".$cbo_forwarder_name."*".$copy_doc_receive_date."*".$original_doc_receive_date."*".$doc_to_cnf."*".$feeder_vessel."*".$mother_vessel."*".$eta_date."*".$ic_receive_date."*".$shipping_bill_no."*".$cbo_inco_term."*".$inco_term_place."*".$port_of_loading."*".$port_of_discharge."*".$bill_of_entry_no."*".$boe_date."*".$psi_reference_no."*".$maturity_date."*".$edf_paid_date."*".$container_no."*".$pkg_quantity."*".$txt_loan_reff."*".$etd_actual."*".$pakg_uom."*".$eta_advice."*".$eta_actual."*".$cbo_container_status."*".$cbo_container_size."*".$release_date."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$txt_doc_rcv_date."*".$txt_bill_entry_value."*".$txt_submit_date."*".$txt_btmea_no."*".$txt_btmea_date."*".$txt_btmea_qnty."*".$cbo_source_id."";
			}
			else
			{
				$field_array_up="btb_lc_id*is_lc*invoice_no*invoice_date*document_value*shipment_date*company_acc_date*bank_acc_date*bank_ref*acceptance_time*retire_source*nagotiate_date*remarks*edf_tenor*bill_no*bill_date*shipment_mode*document_status*forwarder_name*copy_doc_receive_date*original_doc_receive_date*doc_to_cnf*feeder_vessel*mother_vessel*eta_date*ic_receive_date*shipping_bill_no*inco_term*inco_term_place*port_of_loading*port_of_discharge*bill_of_entry_no*bill_of_entry_date*psi_reference_no*maturity_date*edf_paid_date*container_no*pkg_quantity*exchange_rate*loan_ref*etd_actual*pkg_uom*eta_advice*eta_actual*container_status*container_size*release_date*updated_by*update_date*doc_rcv_date*ready_to_approved*bill_entry_value*submission_date*btmea_no*btmea_date*btmea_qnty*source";
				$data_array_up ="".$btb_lc_id."*1*".$txt_invoice_number."*".$txt_invoice_date."*".$txt_document_value."*".$txt_shipment_date."*".$txt_company_acc_date."*".$txt_bank_acceptance_date."*".$txt_bank_ref."*".$cbo_acceptance_time."*".$cbo_retire_source."*".$nagotiate_date."*".$txt_remarks."*".$txt_edf_tenor."*".$bill_no."*".$bill_date."*".$cbo_shipment_mode."*".$cbo_document_status."*".$cbo_forwarder_name."*".$copy_doc_receive_date."*".$original_doc_receive_date."*".$doc_to_cnf."*".$feeder_vessel."*".$mother_vessel."*".$eta_date."*".$ic_receive_date."*".$shipping_bill_no."*".$cbo_inco_term."*".$inco_term_place."*".$port_of_loading."*".$port_of_discharge."*".$bill_of_entry_no."*".$boe_date."*".$psi_reference_no."*".$maturity_date."*".$edf_paid_date."*".$container_no."*".$pkg_quantity."*".$txt_exchange_rate."*".$txt_loan_reff."*".$etd_actual."*".$pakg_uom."*".$eta_advice."*".$eta_actual."*".$cbo_container_status."*".$cbo_container_size."*".$release_date."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$txt_doc_rcv_date."*".$cbo_ready_to_approved."*".$txt_bill_entry_value."*".$txt_submit_date."*".$txt_btmea_no."*".$txt_btmea_date."*".$txt_btmea_qnty."*".$cbo_source_id."";
			}
		...
		else
		{
			...
			{
				$field_array_up="bank_acc_date*bill_no*bill_date*shipment_mode*document_status*forwarder_name*copy_doc_receive_date*original_doc_receive_date*doc_to_cnf*feeder_vessel*mother_vessel*eta_date*ic_receive_date*shipping_bill_no*inco_term*inco_term_place*port_of_loading*port_of_discharge*bill_of_entry_no*bill_of_entry_date*psi_reference_no*maturity_date*edf_paid_date*container_no*pkg_quantity*loan_ref*etd_actual*pkg_uom*eta_advice*eta_actual*container_status*container_size*release_date*updated_by*update_date*bill_entry_value*submission_date*btmea_no*btmea_date*btmea_qnty*source";
				$data_array_up ="".$txt_bank_acceptance_date."*".$bill_no."*".$bill_date."*".$cbo_shipment_mode."*".$cbo_document_status."*".$cbo_forwarder_name."*".$copy_doc_receive_date."*".$original_doc_receive_date."*".$doc_to_cnf."*".$feeder_vessel."*".$mother_vessel."*".$eta_date."*".$ic_receive_date."*".$shipping_bill_no."*".$cbo_inco_term."*".$inco_term_place."*".$port_of_loading."*".$port_of_discharge."*".$bill_of_entry_no."*".$boe_date."*".$psi_reference_no."*".$maturity_date."*".$edf_paid_date."*".$container_no."*".$pkg_quantity."*".$txt_loan_reff."*".$etd_actual."*".$pakg_uom."*".$eta_advice."*".$eta_actual."*".$cbo_container_status."*".$cbo_container_size."*".$release_date."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$txt_bill_entry_value."*".$txt_submit_date."*".$txt_btmea_no."*".$txt_btmea_date."*".$txt_btmea_qnty."*".$cbo_source_id."";
			}
			else
			{
		...
		if($cbo_payterm_id==1 && $payment_date!="" && $hide_approved_status==0 && $is_posted_account!=1)
		{
		...
		//echo "10**".$rID."**".$rID2."**".$rID3."**".$PayID."**".$rID4;oci_rollback($con);disconnect($con);die;
		if($db_type==0)
		{
...
if ($action=="document_advance_payment_letter2")
{
    extract($_REQUEST);
	//echo $data;die;
	$lib_supplier=return_library_array("select id, supplier_name from lib_supplier","id", "supplier_name");
	...
	$sql="SELECT a.id as accep_dtls_id, a.pi_id, a.current_acceptance_value, b.id as btb_id, b.lc_number as btb_lc_no, b.importer_id, b.supplier_id, b.payterm_id, b.lc_date as btb_lc_date, b.lc_value as btb_lc_value, b.currency_id, d.id as lc_sc_id, d.buyer_name, d.export_lc_no as lc_sc_no, d.lc_date as lc_sc_date, d.expiry_date as lc_sc_expire_date, d.last_shipment_date as lc_sc_last_ship_date, d.lc_value as lc_sc_value, d.internal_file_no, 0 as type
	from com_import_invoice_dtls a, com_btb_lc_master_details b 
	left join com_btb_export_lc_attachment c on b.id=c.import_mst_id and c.is_lc_sc=0 and c.status_active=1 and c.is_deleted=0 
	left join com_export_lc d on c.lc_sc_id=d.id and c.is_lc_sc=0
	where a.btb_lc_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.import_invoice_id=$data
	union all
	select a.id as accep_dtls_id, a.pi_id, a.current_acceptance_value, b.id as btb_id, b.lc_number as btb_lc_no, b.importer_id, b.supplier_id, b.payterm_id, b.lc_date as btb_lc_date, b.lc_value as btb_lc_value, b.currency_id, d.id as lc_sc_id, d.buyer_name, d.contract_no as lc_sc_no, d.contract_date as lc_sc_date, d.expiry_date as lc_sc_expire_date, d.last_shipment_date as lc_sc_last_ship_date, d.contract_value as lc_sc_value, d.internal_file_no, 1 as type
	from com_import_invoice_dtls a, com_btb_lc_master_details b 
	left join com_btb_export_lc_attachment c on b.id=c.import_mst_id and c.is_lc_sc=1 and c.status_active=1 and c.is_deleted=0 
	left join com_sales_contract d on c.lc_sc_id=d.id and c.is_lc_sc=1
	where a.btb_lc_id=b.id and b.id=c.import_mst_id  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.import_invoice_id=$data";
	...
	foreach($btb_sql as $row)
	{
		...
		$lc_sc_no=$row[csf("lc_sc_no")];
		$lc_sc_file_no=$row[csf("internal_file_no")];
		$lc_sc_buyer=$row[csf("buyer_name")];
		$lc_sc_date=$row[csf("lc_sc_date")];
		$lc_sc_expire_date=$row[csf("lc_sc_expire_date")];
		$lc_sc_last_ship_date=$row[csf("lc_sc_last_ship_date")];
		$lc_sc_value=$row[csf("lc_sc_value")];
	...
	//echo '<pre>';print_r($btb_lc_arr);
	$accep_sql=sql_select("SELECT pi_id, import_invoice_id, current_acceptance_value, id from com_import_invoice_dtls where status_active=1 and is_deleted=0 and pi_id in($pi_ids) and import_invoice_id<$data");
	foreach($accep_sql as $row)
	{
		$cumulative_value+=$row[csf("current_acceptance_value")];
	}

	$exportPiSupp = sql_select("select c.import_pi, a.id from com_btb_lc_master_details a , com_btb_lc_pi b , com_pi_master_details c where a.id = b.com_btb_lc_master_details_id and b.pi_id = c.id and c.id in($pi_ids)");
	foreach ($exportPiSupp as $value)
	{
		$exportPiSuppArr[$value[csf("id")]] = $value[csf("import_pi")];
	}
	...
	/*if ($pi_ids != '')
	{
		$sql_pi = "SELECT a.id as pi_id, a.goods_rcv_status, a.item_category_id,a.pi_number, b.work_order_id, b.work_order_no, b.item_group, b.net_pi_amount as pi_value, b.color_id, b.count_name, b.yarn_composition_item1, b.yarn_composition_percentage1, b.yarn_composition_item2, b.yarn_composition_percentage2, b.yarn_type, b.body_part_id, b.fab_type, b.fabric_construction, b.fab_design, b.fabric_composition, b.item_description, b.determination_id, b.item_prod_id 
		from com_pi_master_details a, com_pi_item_details b 
		where a.id=b.pi_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.id in ($pi_ids)";
		//echo $sql_pi;
		$sql_pi_res=sql_select($sql_pi);
		...
		foreach ($sql_pi_res as $row) 
		{
			...
			if($pi_wo_check[$row[csf("pi_id")]][$item_desc][$row[csf("work_order_id")]]=="")
			{
				$pi_wo_check[$row[csf("pi_id")]][$item_desc][$row[csf("work_order_id")]]=$row[csf("work_order_id")];
				$all_pi_data_arr[$row[csf("pi_id")]][$item_desc]["work_order_id"].=$row[csf("work_order_id")].",";
				$jjj[$row[csf("pi_id")]][$item_desc]["work_order_id"].=$row[csf("work_order_id")].",";
			}
		...
		$rcv_pi_sql="SELECT a.id as mst_id, a.booking_id, a.receive_basis, a.recv_number, b.order_uom, b.order_rate, sum(b.order_qnty) as rcv_qnty, sum(b.order_amount) as rcv_amount, sum(b.cons_amount) as rcv_amount_tk, c.item_category_id, c.item_group_id, c.yarn_count_id, c.yarn_type, c.yarn_comp_type1st, c.color, c.detarmination_id, c.item_description, c.id as item_prod_id 
		from com_import_invoice_dtls_mrr p, inv_receive_master a, inv_transaction b, product_details_master c 
		where p.mrr_id=a.id and a.id=b.mst_id and b.prod_id=c.id and p.import_invoice_id=$data and a.receive_basis=1 and b.receive_basis =1 and b.transaction_type=1 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.payment_over_recv=0
		group by a.id, a.booking_id, a.receive_basis, a.recv_number, b.order_uom, b.order_rate, c.item_category_id, c.item_group_id, c.yarn_count_id, c.yarn_type, c.yarn_comp_type1st, c.color, c.detarmination_id, c.item_description, c.id
		union all
		select a.id as mst_id, a.booking_id, a.receive_basis, a.recv_number, b.order_uom, b.order_rate, sum(b.order_qnty) as rcv_qnty, sum(b.order_amount) as rcv_amount, sum(b.cons_amount) as rcv_amount_tk, c.item_category_id, c.item_group_id, c.yarn_count_id, c.yarn_type, c.yarn_comp_type1st, c.color, c.detarmination_id, c.item_description, c.id as item_prod_id
		from com_import_invoice_dtls_mrr p, inv_receive_master a, inv_transaction b, product_details_master c 
		where p.mrr_id=a.id and a.id=b.mst_id and b.prod_id=c.id and p.import_invoice_id=$data and a.receive_basis=2 and b.receive_basis =2 and b.transaction_type=1 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.payment_over_recv=0
		group by a.id, a.booking_id, a.receive_basis, a.recv_number,b.order_uom, b.order_rate, c.item_category_id, c.item_group_id, c.yarn_count_id, c.yarn_type, c.yarn_comp_type1st, c.color, c.detarmination_id, c.item_description, c.id";
		// echo $rcv_pi_sql;
		...
		foreach ($rcv_pi_sql_result as $row) 
		{
			...
			$test_datsss[$row[csf("booking_id")]][$item_des][$row[csf("receive_basis")]]+=$row[csf("rcv_amount")];
		}
		//echo '<pre>';print_r($receive_data_arr);//die;;
	}*/
	...
	<table cellspacing="0" cellpadding="0" width="650" border="0">
        ...
        <tr>
            <td class="trborders" colspan="2" align="center" style="font-size:16px; font-weight:bold">BTB L/C INFORMATION</td>
            <td class="trborders" colspan="2" align="center" style="font-size:16px; font-weight:bold">MASTER LC/SC INFORMATION</td>
        </tr>
        <tr>
            <td class="trborders" width="150">Company Name</td>
            <td class="trborders" width="150"><? $company_name=return_field_value("company_name","lib_company","id=$importer_id","company_name"); echo $company_name;?></td>
            <td class="trborders" width="150">File No</td>
            <td class="trborders" width="150"><? echo $lc_sc_file_no; ?></td>
        </tr>
        <tr>
            <td class="trborders" >Buyer name</td>
            <td class="trborders" ><? echo $lib_buyer[$lc_sc_buyer];?></td>
            <td class="trborders" >Master LC/SC No</td>
            <td class="trborders" class="wrd_brk"><? echo $lc_sc_no; ?></td>
        </tr>
        <tr>
            <td class="trborders" >Supplier Name</td>
            <td class="trborders" ><? if ($exportPiSuppArr[$btb_id]==1) echo $lib_company[$supplier_id]; else echo  $lib_supplier[$supplier_id]; ?></td>
            <td class="trborders" >LC/SC date</td>
            <td class="trborders" ><? echo $invoice_no; ?></td>
        ...
        <tr>
            ...
            <td class="trborders">LC/SC expire date</td>
            <td class="trborders"><? echo change_date_format($lc_sc_expire_date); ?></td>
        </tr>
        <tr>
			<td class="trborders">Pay Terms</td>
            <td class="trborders"><? echo $pay_term[$payterm_id];?></td>
            <td class="trborders">Last Ship Date</td>
            <td class="trborders"><? echo change_date_format($lc_sc_last_ship_date); ?></td>
        </tr>
        <tr>
            <td class="trborders">BTB LC Date</td>
            <td class="trborders"><? echo change_date_format($btb_lc_date); ?></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="trborders"><? echo $currency_sign_arr[$currency_id]." ".number_format($btb_lc_value,2);?></td>
			<td></td>
            <td></td>
        </tr>
        <tr>
            <td class="trborders">Previous Acceptance Done</td>
            <td class="trborders"><? echo number_format($cumulative_value,2);?></td>
			<td></td>
            <td></td>
        </tr>
        <tr>
            <td class="trborders">Present Accep. Value</td>
            <td class="trborders"><? echo number_format($current_acceptance_value,2);?></td>
			<td></td>
            <td></td>
        </tr>
        <tr>
            <td class="trborders">Outstanding</td>
            <td class="trborders"><? echo number_format($btb_lc_value-$cumulative_value-$current_acceptance_value,2);?></td>
			<td></td>
            <td></td>
        </tr>
        <tr>
        ...
        <tr>
            <td  colspan="4" align="center" style="font-size:18px; font-weight:bold">&nbsp; Advance Acceptance</td>
        </tr>
        <tr>
            <td colspan="4">&nbsp;I would like to inform you that we need to sign advance in the bill of exchange.<br>
				&nbsp;As per our company policy we need to advance acceptance to make the goods delivery smoothly.<br>
				&nbsp;Therefore, I would request you to allow this procedure.
			</td>
        </tr>
    </table>
     <!-- <table id="" cellspacing="0" cellpadding="0" width="650" border="1">
     	<thead>
	     	<tr bgcolor="#ddd">
				<th width="100">PI Number</th>
				<th width="150">Iteam Name</th>
				<th width="60">UOM</th>
				<th width="80">Receive Qty</th>
				<th width="70">Rate</th>
		...
		foreach ($all_pi_data_arr as $pi_id => $pi_data) 
		{
			foreach ($pi_data as $item_desc => $row) 
			{
				$rcv_qnty=$rcv_amount=0;
				$rcv_amount_tk=0;
				$rcv_qnty=$rcv_amount=$rcv_amount_tk=0;
				$order_uom="";
				if ($row['goods_rcv_status'] == 1) 
				{
					$receive_basis = 2;
					...
					foreach($wo_id_arr as $wo_id)
					{
						if($wo_id)
						{
							$order_uom.=rtrim($receive_data_arr[$wo_id][$item_desc][$receive_basis]['order_uom'],',').',';
							// $order_rate=implode(',',array_unique(explode(',', rtrim($receive_data_arr[$wo_id][$item_desc][$receive_basis]['order_rate'],','))));
							$rcv_qnty+=$receive_data_arr[$wo_id][$item_desc][$receive_basis]['rcv_qnty'];				
							$rcv_amount+=$receive_data_arr[$wo_id][$item_desc][$receive_basis]['rcv_amount'];				
							$rcv_amount_tk+=$receive_data_arr[$wo_id][$item_desc][$receive_basis]['rcv_amount_tk'];		
						}
					}
				...
				{
					$receive_basis = 1;
					$order_uom.=rtrim($receive_data_arr[$row['pi_id']][$item_desc][$receive_basis]['order_uom'],',').',';
				...
				$order_uom_all=implode(',',array_unique( explode(',', rtrim($order_uom,','))));
				if(count($item_des_data)>0) $item_desc=$item_des_data[$item_desc];
				...
				<tbody>
					<tr>
						...
						<td width="60" class="wrd_brk"><? echo $order_uom_all; ?></td>
						<td width="80" class="wrd_brk" align="right"><? echo number_format($rcv_qnty,2); ?></td>
						<td width="70" class="wrd_brk" align="right"><? echo number_format($rcv_amount/$rcv_qnty,4);//number_format($order_rate,4);?></td>
						<td width="90" class="wrd_brk" align="right"><? echo number_format($rcv_amount,2); $tot_rcv_amount += $rcv_amount; ?></td>
						<td width="90" class="wrd_brk" align="right"><? echo number_format($rcv_amount_tk,2); $tot_rcv_amount_tk += $rcv_amount_tk; ?></td>
					</tr>
				</tbody>
...
if ($action=="regular_acceptance_letter")
{
    extract($_REQUEST);
	//echo $data;die;
	$lib_company=return_library_array("select id, company_name from lib_company","id", "company_name");
	...
	if ($pi_ids != '')
	{
		...
		foreach ($sql_pi_res as $row) 
		{
			...
			else if ($row[csf("item_category_id")] == 2) $item_desc=$construction_arr[$row[csf('determination_id')]].' '.$composition_arr[$row[csf('determination_id')]];
			else if ($row[csf("item_category_id")] == 3) $item_desc=$construction_arr[$row[csf('determination_id')]].' '.$composition_arr[$row[csf('determination_id')]];
			else if ($row[csf("item_category_id")] == 4) $item_desc=$lib_item[$row[csf("item_group")]];
			else 
			{
				$item_desc=$row[csf('item_prod_id')];
		...
		$rcv_pi_sql="SELECT a.id as mst_id, b.pi_wo_batch_no as booking_id, a.receive_basis, a.recv_number, b.order_uom, b.order_rate, sum(b.order_qnty) as rcv_qnty, sum(b.ORDER_QNTY*b.ORDER_RATE) as rcv_amount, sum(b.cons_amount) as rcv_amount_tk, c.item_category_id, c.item_group_id, c.yarn_count_id, c.yarn_type, c.yarn_comp_type1st, c.color, c.detarmination_id, c.item_description, c.id as item_prod_id, p.pi_id 
		from com_import_invoice_dtls_mrr p, inv_receive_master a, inv_transaction b, product_details_master c 
		where p.mrr_id=a.id and a.id=b.mst_id and b.prod_id=c.id and p.import_invoice_id=$data and a.receive_basis=1 and b.receive_basis =1 and b.transaction_type=1 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.payment_over_recv=0
		group by a.id, b.pi_wo_batch_no, a.receive_basis, a.recv_number, b.order_uom, b.order_rate, c.item_category_id, c.item_group_id, c.yarn_count_id, c.yarn_type, c.yarn_comp_type1st, c.color, c.detarmination_id, c.item_description, c.id, p.pi_id
		union all
		select a.id as mst_id, b.pi_wo_batch_no as booking_id, a.receive_basis, a.recv_number, b.order_uom, b.order_rate, sum(b.order_qnty) as rcv_qnty, sum(b.ORDER_QNTY*b.ORDER_RATE) as rcv_amount, sum(b.cons_amount) as rcv_amount_tk, c.item_category_id, c.item_group_id, c.yarn_count_id, c.yarn_type, c.yarn_comp_type1st, c.color, c.detarmination_id, c.item_description, c.id as item_prod_id, p.pi_id
		from com_import_invoice_dtls_mrr p, inv_receive_master a, inv_transaction b, product_details_master c 
		where p.mrr_id=a.id and a.id=b.mst_id and b.prod_id=c.id and p.import_invoice_id=$data and a.receive_basis=2 and b.receive_basis =2 and b.transaction_type=1 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.payment_over_recv=0
		group by a.id, b.pi_wo_batch_no, a.receive_basis, a.recv_number,b.order_uom, b.order_rate, c.item_category_id, c.item_group_id, c.yarn_count_id, c.yarn_type, c.yarn_comp_type1st, c.color, c.detarmination_id, c.item_description, c.id, p.pi_id";
		//echo $rcv_pi_sql;die;
		...
		foreach ($rcv_pi_sql_result as $row) 
		{
			...
			$receive_data_arr[$row[csf("pi_id")]][$item_des][$row[csf("receive_basis")]]['order_uom'].=$unit_of_measurement[$row[csf("order_uom")]].',';
			$receive_data_arr[$row[csf("pi_id")]][$item_des][$row[csf("receive_basis")]]['order_rate'].=$row[csf("order_rate")].',';
			$receive_data_arr[$row[csf("pi_id")]][$item_des][$row[csf("receive_basis")]]['rcv_qnty']+=$row[csf("rcv_qnty")];
			$receive_data_arr[$row[csf("pi_id")]][$item_des][$row[csf("receive_basis")]]['rcv_amount']+=$row[csf("rcv_amount")];
			$receive_data_arr[$row[csf("pi_id")]][$item_des][$row[csf("receive_basis")]]['rcv_amount_tk']+=$row[csf("rcv_amount_tk")];
			$test_datsss[$row[csf("pi_id")]][$item_des][$row[csf("receive_basis")]]+=$row[csf("rcv_amount")];

		}
		//echo '<pre>';print_r($receive_data_arr);//die;;
	}
	// echo "<pre>";
	// 	print_r($receive_data_arr);
	// echo "</pre>";
	?>
    <style>
	.trborders
	...
	<table cellspacing="0" cellpadding="0" width="650" border="0">
        ...
        <tr>
            <td  colspan="4" >&nbsp; Against goods received acceptance,</td>
        </tr>
        <tr>
            <td colspan="4">&nbsp;We here by confirm that we have received all goods,against the aforesaid documents,which are 'OK' in terms of quality and quantity. We also confirm that the reffered goods have been properly reflected in the stock register maintained by store after completion of required formalities and we have no clain against such.
			</td>
        </tr>
    </table>
	<br>
    <table cellspacing="0" cellpadding="0" width="650" border="1" class="rpt_table" >
     	<thead>
	     	<tr bgcolor="#ddd">
				<th width="100">PI Number</th>
				<th width="150">Iteam Name</th>
		...
		foreach ($all_pi_data_arr as $pi_id => $pi_data) 
		{
			...
			foreach ($pi_data as $item_desc => $row) 
			{
				...
				?>
					<tr>
						<?
							if(!in_array($row["pi_number"],$array_check_arr))
							{
								$array_check_arr[]=$row["pi_number"];
								$row_count=count($pi_data);
								?>
									<td width="80" rowspan="<?=$row_count;?>" class="wrd_brk"><? echo $row["pi_number"]; ?></td>
								<?
							}
						?>
						<td width="80" class="wrd_brk"><? echo $item_desc; ?></td>
	...
	$rcv_return_sql="select cc.issue_number, cc.issue_date, bb.CONS_UOM, bb.CONS_QUANTITY, bb.CONS_AMOUNT from  inv_receive_master aa, inv_transaction bb, inv_issue_master cc where 
	cc.received_mrr_no=aa.recv_number and cc.id=bb.mst_id and bb.transaction_type=3 and cc.received_mrr_no in ( SELECT a.recv_number from com_import_invoice_dtls_mrr p, inv_receive_master a, inv_transaction b, product_details_master c where p.mrr_id=a.id and a.id=b.mst_id and b.prod_id=c.id and p.import_invoice_id=$data and a.receive_basis in (1,2) and b.receive_basis in(1,2) and b.transaction_type=1 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.payment_over_recv=0
	group by a.id, b.pi_wo_batch_no, a.receive_basis, a.recv_number, b.order_uom, b.order_rate, c.item_category_id, c.item_group_id, c.yarn_count_id, c.yarn_type, c.yarn_comp_type1st, c.color, c.detarmination_id, c.item_description, c.id, p.pi_id )";
	//echo $rcv_return_sql;
	$rcv_return_data=sql_select($rcv_return_sql);
	// echo "<pre>";
	// 	print_r($rcv_return_data);
	// echo "</pre>";

	$rcv_return_data_final=array();
	foreach($rcv_return_data as $val)
	{
		$rcv_return_data_final[$val['ISSUE_NUMBER']]['issue_number']=$val['ISSUE_NUMBER'];
		$rcv_return_data_final[$val['ISSUE_NUMBER']]['issue_date']=$val['ISSUE_DATE'];
		$rcv_return_data_final[$val['ISSUE_NUMBER']]['cons_uom']=$val['CONS_UOM'];
		$rcv_return_data_final[$val['ISSUE_NUMBER']]['cons_qty']=$rcv_return_data_final[$val['ISSUE_NUMBER']]['cons_qty']+$val['CONS_QUANTITY'];
		$rcv_return_data_final[$val['ISSUE_NUMBER']]['cons_amount']=$rcv_return_data_final[$val['ISSUE_NUMBER']]['cons_amount']+$val['CONS_AMOUNT'];

	}

	$dollar_rate_sql="select MARKETING_RATE from currency_conversion_rate where ID = (select max(ID) from currency_conversion_rate where company_id=$importer_id )";
	$dollar_rate_result=sql_select($dollar_rate_sql);
	$dollar_rate_result=$dollar_rate_result[0]['MARKETING_RATE'];
	?>
	<br><br>
	<table cellspacing="0" cellpadding="0" width="650" border="1" class="rpt_table" style="margin-bottom:-50px;" >
     	<thead>
	     	<tr bgcolor="#ddd">
		...
		<tbody>		
		<?
			foreach($rcv_return_data_final as $row){
		?>
				<tr>
					<td width="100" class="wrd_brk" align="center"><? echo $row['issue_number']; ?></td>
					<td width="80" class="wrd_brk" align="center"><? echo $row['issue_date']; ?></td>
					<td width="100" class="wrd_brk" align="center"><? echo $unit_of_measurement[$row['cons_uom']]; ?></td>
					<td width="100" class="wrd_brk" align="center"><? echo $row['cons_qty']; ?></td>
					<td width="100" class="wrd_brk" align="center"><? echo number_format($row['cons_amount']/$dollar_rate_result,2); ?></td>
				</tr>
		<?
			}
...
if ($action=="import_document_acceptance_forwarding2_old")
{
    extract($_REQUEST);
	//echo $data;die;
	$lib_supplier=return_library_array("select id, supplier_name from lib_supplier","id", "supplier_name");
	...
	if ($pi_ids != '')
	{
		...
		foreach ($sql_pi_res as $row) 
		{
			...
			/*else if ($row[csf("item_category_id")] == 3) $item_desc=$lib_body_part_arr[$row[csf('body_part_id')]]." ".$row[csf('fab_type')]." ".$row[csf('fabric_construction')]." ".$row[csf('fab_design')]." ".$row[csf('fabric_composition')];*/
			else if ($row[csf("item_category_id")] == 3) $item_desc=$row[csf('fabric_construction')]." ".$row[csf('fabric_composition')];
			else if ($row[csf("item_category_id")] == 4) $item_desc=$lib_item[$row[csf("item_group")]];
		...
		/*if($all_pi_ids !="" && $all_wo_ids !="")
		{
			...
			where a.id=b.mst_id and b.prod_id=c.id and a.booking_id in($all_pi_ids) and a.receive_basis=1 and b.receive_basis =1 and b.transaction_type=1 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
			...
			from inv_receive_master a, inv_transaction b, product_details_master c 
			where a.id=b.mst_id and b.prod_id=c.id and a.booking_id in($all_wo_ids) and a.receive_basis=2 and b.receive_basis =2 and b.transaction_type=1 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
		...
		else
		{
			if($all_pi_ids !="")
			{
				$rcv_pi_sql="select a.id as mst_id, a.booking_id, a.receive_basis, a.recv_number_prefix_num, a.recv_number, a.receive_date, a.challan_no, b.order_uom, b.order_rate, sum(b.order_qnty) as rcv_qnty, sum(b.order_amount) as rcv_amount, sum(b.cons_amount) as rcv_amount_tk, c.item_category_id, c.item_group_id, c.yarn_count_id, c.yarn_type, c.yarn_comp_type1st, c.color, c.detarmination_id 
				from inv_receive_master a, inv_transaction b, product_details_master c 
				where a.id=b.mst_id and b.prod_id=c.id and a.booking_id in($all_pi_ids) and a.receive_basis=1 and b.receive_basis =1 and b.transaction_type=1 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
				group by a.id, a.booking_id, a.receive_basis, a.recv_number_prefix_num, a.recv_number, a.receive_date, a.challan_no, b.order_uom, b.order_rate, c.item_category_id, c.item_group_id, c.yarn_count_id, c.yarn_type, c.yarn_comp_type1st, c.color, c.detarmination_id";
			}
		...
		from com_import_invoice_dtls_mrr p, inv_receive_master a, inv_transaction b, product_details_master c 
		where p.mrr_id=a.id and a.id=b.mst_id and b.prod_id=c.id and p.import_invoice_id=$data and a.receive_basis=1 and b.receive_basis =1 and b.transaction_type=1 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.payment_over_recv=0
		group by a.id, b.pi_wo_batch_no, a.receive_basis, a.recv_number_prefix_num, a.recv_number, a.receive_date, a.challan_no, b.order_uom, b.order_rate, c.item_category_id, c.item_group_id, c.yarn_count_id, c.yarn_type, c.yarn_comp_type1st, c.color, c.detarmination_id
		union all
		select a.id as mst_id, b.pi_wo_batch_no as booking_id, a.receive_basis, a.recv_number, a.recv_number_prefix_num, a.receive_date, a.challan_no, b.order_uom, b.order_rate, sum(b.order_qnty) as rcv_qnty, sum(b.ORDER_QNTY*b.ORDER_RATE) as rcv_amount, sum(b.cons_amount) as rcv_amount_tk, c.item_category_id, c.item_group_id, c.yarn_count_id, c.yarn_type, c.yarn_comp_type1st, c.color, c.detarmination_id 
		from com_import_invoice_dtls_mrr p, inv_receive_master a, inv_transaction b, product_details_master c 
		where p.mrr_id=a.id and a.id=b.mst_id and b.prod_id=c.id and p.import_invoice_id=$data and a.receive_basis=2 and b.receive_basis =2 and b.transaction_type=1 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.payment_over_recv=0
		group by a.id, b.pi_wo_batch_no, a.receive_basis, a.recv_number, a.recv_number_prefix_num, a.receive_date, a.challan_no, b.order_uom, b.order_rate, c.item_category_id, c.item_group_id, c.yarn_count_id, c.yarn_type, c.yarn_comp_type1st, c.color, c.detarmination_id";
		//echo $rcv_pi_sql;
		...
		//echo '<pre>';print_r($test_datsss);die;;
	}
			...
			foreach ($pi_data as $item_desc => $row) 
			{
				...
				if ($row['goods_rcv_status'] == 1) 
				{
					...
					foreach($wo_id_arr as $wo_id)
					{
						if($wo_id)
						{
							$challan_no=implode(',',array_unique( explode(',', rtrim($receive_data_arr[$wo_id][$item_desc][$receive_basis]['challan_no'],','))));
							$order_uom=implode(',',array_unique( explode(',', rtrim($receive_data_arr[$wo_id][$item_desc][$receive_basis]['order_uom'],','))));
							$recv_number_prefix_num=implode(',',array_unique( explode(',', rtrim($receive_data_arr[$wo_id][$item_desc][$receive_basis]['recv_number_prefix_num'],','))));
							$receive_date=implode(',',array_unique(explode(',', rtrim($receive_data_arr[$wo_id][$item_desc][$receive_basis]['receive_date'],','))));
							$order_rate=implode(',',array_unique(explode(',', rtrim($receive_data_arr[$wo_id][$item_desc][$receive_basis]['order_rate'],','))));
							$rcv_qnty+=$receive_data_arr[$wo_id][$item_desc][$receive_basis]['rcv_qnty'];						
							$rcv_amount+=$receive_data_arr[$wo_id][$item_desc][$receive_basis]['rcv_amount'];						
							$rcv_amount_tk+=$receive_data_arr[$wo_id][$item_desc][$receive_basis]['rcv_amount_tk'];
				...
				<tbody>
					<tr>
						...
						<td width="80" class="wrd_brk" align="right"><? echo number_format($rcv_qnty,2); ?></td>
						<td width="60" class="wrd_brk" align="right"><? echo number_format($rcv_amount/$rcv_qnty,4);//number_format($order_rate,4); ?></td>
...
if ($action=="forwarding_letter3")
{
	...
	$internal_file_no = implode(", ",array_unique(explode(",",chop($internal_file_no,','))));
	...
	.trborders
	{
	 ...
	 <table width="750" cellpadding="0" align="left" cellspacing="0" border="0" style="font-size: 20px;">
		...
		<tr>
		<td width="25"></td>
		<td  colspan="2" >Ref. No.: <? echo $ref."-".$internal_file_no."/".$lc_sc_year;?></td>
		...
		<tr>
            ...
            <p ><b>L/C NO#<? echo $btb_lc_no; ?>&nbsp;&nbsp;&nbsp;&nbsp;<? echo change_date_format($btb_lc_date); ?>&nbsp;&nbsp;&nbsp;&nbsp; <?  echo $currency_name .' '.number_format($current_acceptance_value,2); ?> &nbsp;&nbsp;&nbsp;&nbsp;<?  echo $supplier_name ;?></b></p>
			</td>
		</tr>
		<tr><td colspan="4"  height="25">&nbsp;</td></tr>
        <tr>
			...
			<td width="675" colspan="3">So, we request you to kindly settle the acceptance letter as soon as possible. </td>
		</tr>
		<tr><td colspan="4"  height="25">&nbsp;</td></tr>
        <tr><td colspan="4"  height="50">&nbsp;</td></tr>
        <tr>
...
if ($action=="payment_letter")
{
	...
	foreach($btb_sql as $row)
	{
		...
		$currency_name      = $currency[$row[csf("currency_id")]];
		$btb_id=$row[csf('btb_id')];
		$bank_account_type = $row[csf('retire_source')];
		$company_id=$row[csf("importer_id")];
		$issuing_bank_id=$row[csf("issuing_bank_id")];
	}
	
	$sql_bank_info=sql_select("SELECT a.ID, b.ACCOUNT_TYPE, b.ACCOUNT_NO from LIB_BANK a, LIB_BANK_ACCOUNT b where a.id=b.account_id and b.ACCOUNT_TYPE=$bank_account_type and b.COMPANY_ID=$company_id and a.id=$issuing_bank_id");
	$bank_dtls_arr_source='';
	if(count($sql_bank_info)>0)
	{
		$bank_account=$sql_bank_info[0]['ACCOUNT_NO'];
	}
	else
	{
		$sql_bank_info=sql_select("SELECT a.ID, b.ACCOUNT_TYPE, b.ACCOUNT_NO from LIB_BANK a, LIB_BANK_ACCOUNT b where a.id=b.account_id and b.ACCOUNT_TYPE=$bank_account_type and a.id=$issuing_bank_id ");
		$bank_account=$sql_bank_info[0]['ACCOUNT_NO'];
	}
	...
	.trborders
	{
	 ...
	 <table width="750" cellpadding="0" align="left" cellspacing="0" border="0" style="font-size: 20px;">
		...
		<tr>
			<td width="25"></td>
			<td width="675" colspan="3" style="text-align: justify;">We would like to request you to kindly settle the below mentioned BTB LC (Usance / EDF / Sight / Upass) payment will be as early as possible subject to available of fund.<b> A/C No.<? echo $bank_account;?>.</b>
		...
		<tr>
			<td width="25"></td>
			<td width="675" colspan="3" style="text-align: justify;"><b>LC NO-<? echo $btb_lc_no; ?>&nbsp;&nbsp;&nbsp;&nbsp; Date: <? echo $btb_lc_date; ?>&nbsp;&nbsp;&nbsp;&nbsp; for <?  echo $currency_name .' '.number_format($current_acceptance_value,2); ?> &nbsp;&nbsp;&nbsp;&nbsp;OF <? echo $supplier_name ;?>. </b>   
...
if($action=="save_update_delete_mrr")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$field_array="id, import_invoice_id, btb_lc_id, pi_id, invoice_dtls_id, is_service, mrr_id, inserted_by, insert_date,  status_active, is_deleted";
		$data_array="";
		for($i=1;$i<=$total_row;$i++)
		{
			$hdnMrrId="hdnMrrId_".$i;
			if($data_array!="") $data_array.=", ";
			$data_array .="(".$id.",".$inv_id.",".$btb_id.",".$pi_id.",".$inv_dtls_id.",'".$is_service_category."',".$$hdnMrrId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			$id++;
		}
