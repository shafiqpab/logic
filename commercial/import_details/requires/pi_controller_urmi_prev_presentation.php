...
if ($action=="load_supplier_dropdown")
{
	...
	{
		echo create_drop_down( "cbo_supplier_id", 151,"select DISTINCT(c.supplier_name),c.id from lib_supplier_tag_company a,lib_supplier_party_type b, lib_supplier c where c.id=b.supplier_id and a.supplier_id = b.supplier_id and a.tag_company='$data[0]' and b.party_type in(26) and c.status_active=1 and c.is_deleted=0 order by c.supplier_name",'id,supplier_name', 1, '-- Select Supplier --',0,'',0);
	}
	else if($data[1]==32)
	{
...
{ 
	...
	if ($operation==0)  // Insert Here
	{
		...
		else
		{
			...
			else if(str_replace("'", '',$cbo_item_category_id)==2 || str_replace("'", '',$cbo_item_category_id)==3)
			{
				if(str_replace("'", '',$txt_order_type)==2)
				{
					...
					$prev_pi_qty_arr=return_library_array( "select b.work_order_dtls_id,sum(b.quantity) as qty from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and a.item_category_id=$cbo_item_category_id and a.pi_basis_id=1 and a.goods_rcv_status=$cbo_goods_rcv_status and b.status_active=1 and b.is_deleted=0 and b.work_order_id in($all_WoId) group by b.work_order_dtls_id", "work_order_dtls_id", "qty");
					
					
				}
				else
				{
					//$wo_qnty_arr=return_library_array("select id, fin_fab_qnty as qnty from wo_booking_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and b.status_active=1 and a.id in($all_WoId)","id","qnty");
			...
			else
			{
				...
				if(str_replace("'", '',$cbo_pi_basis_id)==1)
				{
					...
					else
					{
						...
						{
							if((str_replace("'", '',$cbo_item_category_id)==2 || str_replace("'", '',$cbo_item_category_id)==3) && str_replace("'", '',$txt_order_type)==1)
							{
					...
					if(str_replace("'", '',$cbo_item_category_id)==31)
					{
						...
						{
							echo "11** PI Amount Not Allow Over Balance Amount";die;
						}
					}
					else
					{
	...
	{ 
		...
		if($cbo_item_category_id==1 && $cbo_goods_rcv_status==2)
		{
			$sql_attach_mrr=sql_select("select b.pi_wo_batch_no, c.color, c.yarn_count_id, c.yarn_comp_type1st, c.yarn_type, sum(b.cons_quantity) as qnty 
			from inv_receive_master a, inv_transaction b, product_details_master c 
			where a.id=b.mst_id and b.prod_id=c.id and b.pi_wo_batch_no=$update_id and b.receive_basis=1 and b.transaction_type=1 and b.item_category=$cbo_item_category_id and c.item_category_id=$cbo_item_category_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  group by b.pi_wo_batch_no, c.color, c.yarn_count_id, c.yarn_comp_type1st, c.yarn_type");
			if(count($sql_attach_mrr)>0)
			{
				$mrr_data=array();
				foreach($sql_attach_mrr as $row)
				...
				$sql_attach_mrr_rtn=sql_select("select a.pi_id as pi_wo_batch_no, c.color, c.yarn_count_id, c.yarn_comp_type1st, c.yarn_type, sum(b.cons_quantity) as qnty 
			from inv_issue_master a, inv_transaction b, product_details_master c 
			where a.id=b.mst_id and b.prod_id=c.id and a.pi_id=$update_id and a.pi_id>0 and a.entry_form=8 and b.transaction_type=3 and b.item_category=$cbo_item_category_id and c.item_category_id=$cbo_item_category_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  group by a.pi_id, c.color, c.yarn_count_id, c.yarn_comp_type1st, c.yarn_type");
				...
				for($i=1;$i<=$total_row;$i++)
				{
					...
					$mrr_qnty=$pi_item_qnty=$mrr_rtn_qnty=0;
					$mrr_qnty=number_format($mrr_data[str_replace("'","",$update_id)][str_replace("'","",$color_id)][str_replace("'","",$$countName)][str_replace("'","",$$yarnCompositionItem1)][str_replace("'","",$$yarnType)],2,".","");
					$mrr_rtn_qnty=number_format($mrr_rtn_data[str_replace("'","",$update_id)][str_replace("'","",$color_id)][str_replace("'","",$$countName)][str_replace("'","",$$yarnCompositionItem1)][str_replace("'","",$$yarnType)],2,".","");
					$pi_item_qnty=number_format(str_replace("'","",$$quantity),2,".","");
					
					$mrr_qnty=$mrr_qnty-$mrr_rtn_qnty;
					
					if($pi_item_qnty < $mrr_qnty)
					{
						echo "14** PI Quantity Not Allow Less Then Mrr Quantity.**1"; 
						die;
					}
		...
		if(str_replace("'", '',$cbo_goods_rcv_status)==2)
		{
			...
			{
				//$wo_qnty_arr=return_library_array("select wo_trim_booking_dtls_id, sum(cons) as qnty from wo_trim_book_con_dtls where status_active=1 and wo_trim_booking_dtls_id in($all_dtls_id) and status_active=1 and cons>0 group by wo_trim_booking_dtls_id","wo_trim_booking_dtls_id","qnty");
				if(str_replace("'", '',$txt_order_type)==1)
				{
...
;
            ...
            ?>
                <tr class="general" id="row_1">
                    ...
                    <td> 
						 <?
                            echo create_drop_down( "itemgroupid_1", 110, "SELECT id,item_name FROM lib_item_group WHERE item_category =$item_category_id AND status_active = 1 AND is_deleted = 0 ORDER BY item_name ASC",'id,item_name', 1, '-Select-',0,"get_php_form_data( this.value+'**'+'uom_1', 'get_uom', 'requires/pi_controller_urmi' );",$disable_status); 
                         ?>  
                    </td>
                    <td>
			...
			else
			{
				...
				?>
                    <tr class="general" id="row_<? echo $i; ?>">
                        ...
                        <td> 
						 <?
                            echo create_drop_down( "itemgroupid_".$i, 110, $item_group_arr,'', 1, '-Select-',$row[csf('item_group')],"get_php_form_data( this.value+'**'+'uom_$i', 'get_uom', 'requires/pi_controller_urmi' );",$disable_status); 
                         ?>  
                    	</td>
                        <td>
       	...
       	<?
            ...
            ?>
                <tr class="general" id="row_1">
                    ...
                    <td>
                    	<? echo create_drop_down( "embellname_1", 130, $emblishment_name_array,'', 1, '-Select-', 0,"load_drop_down( 'requires/pi_controller_urmi', this.value+'**'+".$disable_status."+'**'+'embelltype_1', 'load_drop_down_embelltype', 'embelltypeTd_1');",$disable_status); ?>
                    </td>
			...
			else
			{
				...
				?>
                    <tr class="general" id="row_<? echo $i; ?>">
                        ...
                        <td>
                            <? echo create_drop_down( "embellname_".$i, 130, $emblishment_name_array,'', 1, '-Select-', $row[csf('embell_name')],"load_drop_down( 'requires/pi_controller_urmi', this.value+'**'+".$disable_status."+'**'+'embelltype_$i', 'load_drop_down_embelltype', 'embelltypeTd_$i');",$disable_status); ?>
                        </td>
       	...
       	<?
            ...
            ?>
                <tr class="general" id="row_1">
                    ...
                    <td> 
						 <?
                            echo create_drop_down( "itemgroupid_1", 130, $item_group_arr,'', 1, '-Select-',0,"get_php_form_data( this.value+'**'+'uom_1', 'get_uom', 'requires/pi_controller_urmi' );",1); 
                         ?>  
                    </td>
                    <td>
			...
			else
			{
				...
				?>
                    <tr class="general" id="row_<? echo $i; ?>">
                        ...
                        <td> 
						 <?
                            echo create_drop_down( "itemgroupid_".$i, 130, $item_group_arr,'', 1, '-Select-',$row[csf('item_group')],"get_php_form_data( this.value+'**'+'uom_$i', 'get_uom', 'requires/pi_controller_urmi' );",1); 
                         ?>  
                    	</td>
                        <td>
            ...
            <table cellpadding="0" cellspacing="0" width="870" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
                    <td class="must_entry_caption">
						 <? echo create_drop_down( "cbo_importer_id", 151,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, '--Select--',$importer_id,"load_drop_down( 'pi_controller_urmi',this.value+'_'+$item_category, 'load_supplier_dropdown', 'supplier_td' );",0); ?>       
                    </td>
                    <td id="supplier_td">	
                        <?
...
if($action=="create_pi_search_list_view")
{
	...
	if (trim($data[0])!="") $pi_number=" and pi_number like '%".trim($data[0])."%'"; else { $pi_number = ''; }
	if ($data[1]!="" &&  $data[2]!="")
	{
		if($db_type==0)
		{
	...
	if (trim($data[6])=="") $system_id_cond=''; else $system_id_cond=" and id=".trim($data[6]);
            ...
            <table cellpadding="0" cellspacing="0" width="850" border="1" rules="all" class="rpt_table">
                ...
                <tr class="general">
					...
					<?
                    	...
                    	echo create_drop_down( "cbo_importer_id", 151,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, 'Select',$importer_id,"load_drop_down( 'pi_controller_urmi',this.value+'_'+$item_category_id, 'load_supplier_dropdown', 'supplier_td' );",$dis_ana); 
                    ?>       
                    </td>
                    <td id="supplier_td">	
					<?
					...
					?>
            		 ...
            		 <td align="center">
                     	<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_wo_no').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_item_category').value+'_'+document.getElementById('cbo_importer_id').value+'_'+document.getElementById('cbo_supplier_id').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('cbo_based_on').value+'_'+document.getElementById('txt_goods_rcv_status').value+'_'+'<? echo $prev_wo_ids; ?>'+'_'+'<? echo $previous_wo_ids; ?>'+'_'+'<? echo $prev_deters; ?>'+'_'+'<? echo $prev_colors; ?>'+'_'+'<? echo $prev_constructs; ?>'+'_'+'<? echo $prev_compositions; ?>'+'_'+'<? echo $prev_gsms; ?>'+'_'+'<? echo $prev_widths; ?>'+'_'+'<? echo $prev_uoms; ?>', 'create_wo_search_list_view', 'search_div', 'pi_controller_urmi', 'setFilterGrid(\'tbl_list_search\',-1);reset_hide_field();')" style="width:100px;" />
                     </td>
                </tr>
                <tr>
				...
				?>
					<tr class="general" id="row_<? echo $tblRow; ?>">
						...
						<td>
						 ...
						 <td>
							<? 
								echo create_drop_down( "uom_".$tblRow, 60, $unit_of_measurement,'', 1, 'Select',$row[csf('uom')],'','1');            
							?>						 
						</td>
						<td>
			...
			foreach($data_array as $row)
			{
				...
				if($bl_qty>0)
				{
					...
					<tr class="general" id="row_<? echo $tblRow; ?>">
						...
						<td> 
							<?
								echo create_drop_down( "itemgroupid_".$tblRow, 110, $item_group_arr,'', 1, '-Select-',$row[csf('trim_group')],"get_php_form_data( this.value+'**'+'uom_$tblRow', 'get_uom', 'requires/pi_controller_urmi' );",1); 
							?>  
						</td>
						<td>
				...
				foreach($row_val as $pord_id=>$row)
				{
					...
					if($bl_qty>0)
					{
						...
						<tr class="general" id="row_<? echo $tblRow; ?>">
							...
							<td> 
								<?
									echo create_drop_down( "itemgroupid_".$tblRow, 110, $item_group_arr,'', 1, '-Select-',$row[('trim_group')],"get_php_form_data( this.value+'**'+'uom_$tblRow', 'get_uom', 'requires/pi_controller_urmi' );",1); 
								?>  
							</td>
							<td>
		...
		foreach($data_array as $row)
		{
			...
			?>
                <tr class="general" id="row_<? echo $tblRow; ?>">
                    ...
                    <td>
                        <? echo create_drop_down( "embellname_".$tblRow, 130, $emblishment_name_array,'', 1, '-Select-', $row[csf('emb_name')],"load_drop_down( 'requires/pi_controller_urmi', this.value+'**'+1+'**'+'embelltype_$tblRow', 'load_drop_down_embelltype', 'embelltypeTd_$tblRow');",1); ?>
                    </td>
			...
			foreach($data_array as $row)
			{
				...
				if($bl_qty>0)
				{
					...
					<tr class="general" id="row_<? echo $tblRow; ?>">
						...
						<td> 
						<?
							echo create_drop_down( "itemgroupid_".$tblRow, 130, $item_group_arr,'', 1, '- Select -',$row[csf('item_group_id')],"get_php_form_data( this.value+'**'+'uom_$tblRow', 'get_uom', 'requires/pi_controller_urmi' );",1); 
						?>  
						</td>
						<td>
				...
				foreach($book_data as $prod_id=>$row)
				{
					...
					if($bl_qty>0)
					{
						...
						<tr class="general" id="row_<? echo $tblRow; ?>">
							...
							<td> 
							<?
								echo create_drop_down( "itemgroupid_".$tblRow, 130, $item_group_arr,'', 1, '- Select -',$row[('item_group_id')],"get_php_form_data( this.value+'**'+'uom_$tblRow', 'get_uom', 'requires/pi_controller_urmi' );",1); 
							?>  
							</td>
							<td>
	...
	<form name="searchwofrm"  id="searchwofrm">
		...
		<legend>Enter search words</legend>           
            <table cellpadding="0" cellspacing="0" width="700px" class="rpt_table">
                ...
                <tr class="general">
                    <td>
						 <? echo create_drop_down( "cbo_importer_id", 151,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name",'id,company_name', 1, 'Select',0,"load_drop_down( 'pi_controller_urmi',this.value+'_'+$item_category, 'load_supplier_dropdown', 'supplier_td' );",0); ?>       
                    </td>
                    <td id="supplier_td">	
                        <?
							echo create_drop_down( "cbo_supplier_id", 151, $blank_array,"", 1, "-- Select Supplier --", 0, "" );
