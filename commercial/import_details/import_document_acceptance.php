...
<script>
	...
	function check_duplicate_invoice(invoice_no)
	{
	  if(invoice_no!="")
	  {
	   ...
	   if(invoice_no==invoice_no_found)
	   {
		  ...
		  show_list_view(btb_lc_id+'_'+1+'___'+impoter_id,'pi_listview','pi_list','requires/import_document_acceptance_controller','');
	...
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1020px,height=450px,center=1,resize=0,scrolling=0','../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0];
			var theemail=this.contentDoc.getElementById("hidden_btb_id");
			...
			if (theemail.value!="")
			{
			    ...
			    reset_form('importdocumentacceptance_1','','','','');
				var invoice_popup=2;
				get_php_form_data(theemail.value+'_'+invoice_popup, "populate_data_from_btb_lc", "requires/import_document_acceptance_controller" );
				...
				show_list_view(theemail.value+'_'+invoice_popup+'_'+theemail_invoice.value+'_'+posted_in_account+'_'+impoter_id+'_'+ref_closing_staus+'_'+is_posted_accounts,'pi_listview','pi_list','requires/import_document_acceptance_controller','');
	...
	function fn_pi_print(pi_id,cbo_item_category_id)
	{
		var entry_form=0;
		...
		//alert(entry_form);
		 ...
		 print_report( $('#cbo_importer_id').val()+'*'+pi_id+'*'+entry_form+'*'+cbo_item_category_id, "print", "requires/pi_print_urmi" );
		 return;
	}
	...
	function fnc_import_document_acceptance( operation )
	{
		...
		let approved_status = $('#hide_approved_status').val();
		if(approved_status == 1 || approved_status == 3){
			alert("This PI is Approved. Update Restricted.");
			return;
		}
		...
		if($('#txt_document_value').val()=="") $('#txt_document_value').val($("#tot_current_acceptance_value").val());
		...
		if(http.readyState == 4)
		{
			//alert(http.responseText);release_freezing();return;
			 ...
			 else if(reponse[0]==0 || reponse[0]==1)
			 {
				...
				show_list_view(reponse[1]+'_'+2+'_'+reponse[2]+'__'+inpoter_id,'pi_listview','pi_list','requires/import_document_acceptance_controller','');
	...
	function fnc_letter_print(type)
	{
		...
		if(type==2){
			print_report($('#invoice_id').val(),'import_document_acceptance_forwarding2','requires/import_document_acceptance_controller');
		
		}
		if(type==3){
			print_report($('#invoice_id').val(),'document_advance_payment_letter','requires/import_document_acceptance_controller');
		
		}
		if(type==4){
			print_report($('#invoice_id').val(),'btb_acceptance_letter','requires/import_document_acceptance_controller');
		
		}
		if(type==5){
			print_report($('#invoice_id').val(),'forwarding_letter3','requires/import_document_acceptance_controller');
		
		}
		if(type==6){
			print_report($('#invoice_id').val(),'undertaking_ific','requires/import_document_acceptance_controller');
		
		}
		if(type==7){
			print_report($('#invoice_id').val(),'payment_letter','requires/import_document_acceptance_controller');
		
		}
		if(type==8){
			print_report($('#invoice_id').val(),'document_advance_payment_letter2','requires/import_document_acceptance_controller');
		}
		if(type==9){
			print_report($('#invoice_id').val(),'regular_acceptance_letter','requires/import_document_acceptance_controller');
		}
            ...
            <form name="importdocumentacceptance_1" id="importdocumentacceptance_1" autocomplete="off" data-entry_form="">
                <fieldset style="width:1024px;">
                    ...
                    <table width="100%" border="0" cellpadding="0" cellspacing="1" id="tbl_importdocumentacceptance">
                        ...
                        <tr>
                            ...
                            <td>
                            <input type="text" name="txt_document_value" id="txt_document_value" class="text_boxes_numeric" style="width:139px" title="If blank while save/update then autofill from total current acceptance value field" /></td>
                        ...
                        <tr>
                        	...
                        	<td class="must_entry_caption">Company Acc. Date</td>
                            <td>
                        ...
                        <tr>
                          ...
                          <td><?php echo create_drop_down( "cbo_acceptance_time",152,$acceptance_time,'',0,'',"",0,0); ?> </td>
                            <td class="must_entry_caption">Retire Source</td>
                            <td>
                        ...
                        <tr>
							...
							<td>File No.</td>
                            <td ><input type="text" name="txt_file_no" id="txt_file_no" style="width:150px" class="text_boxes" placeholder="Display" disabled /></td>
                        </tr>
						<tr>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="1">
					...
					<tr>
						...
						<td>
							<input type="text" name="pkg_quantity" id="pkg_quantity" style="width:80px" class="text_boxes" value="" />
                    ...
                    <tr>
                    	<td>BTMEA No</td>
						<td><input type="text" name="txt_btmea_no" id="txt_btmea_no" class="text_boxes" style="width:140px" /></td>
						<td>BTMEA Date</td>
						<td><input type="text" name="txt_btmea_date" id="txt_btmea_date" style="width:140px" class="datepicker" value="" /></td>
                        <td>BTMEA Qty.</td>
						<td><input type="text" name="txt_btmea_qnty" id="txt_btmea_qnty" class="text_boxes_numeric" style="width:140px" /></td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
					...
					<tr>
						<td colspan="6" height="50" valign="middle" align="center" class="button_container">
							...
							<input type="button" value="Letter Print" id="letter_print" name="letter_print" class="formbutton" style="width:100px;display:none" onClick="fnc_letter_print(1)" />
                            <input type="button" value="Forwarding letter2" id="forwarding_letter2" name="forwarding_letter2" class="formbutton" style="width:120px;display:none" onClick="fnc_letter_print(2)" />
							<input type="button" value="Forwarding letter3" id="forwarding_letter3" name="forwarding_letter3" class="formbutton" style="width:150px;display:none" onClick="fnc_letter_print(5)" />
                            <input type="button" value="Advance Payment letter" id="advance_payment_letter" name="advance_payment_letter" class="formbutton" style="width:150px;display:none" onClick="fnc_letter_print(3)" />
							<input type="button" value="Advance Acceptance" id="advance_acceptance" name="advance_acceptance" class="formbutton" style="width:150px;display:none" onClick="fnc_letter_print(8)" />
							<input type="button" value="Regular Acceptance letter" id="regular_acceptance_letter" name="regular_acceptance_letter" class="formbutton" style="width:150px;display:none" onClick="fnc_letter_print(9)" />
                            <input type="button" value="BTB Acceptance Letter" id="btb_acceptance_letter" name="btb_acceptance_letter" class="formbutton" style="width:150px;display:none" onClick="fnc_letter_print(4)" />
							<input type="button" value="UNDERTAKING(IFIC)" id="undertaking_ific" name="undertaking_ific" class="formbutton" style="width:150px;display:none" onClick="fnc_letter_print(6)" />
							<input type="button" value="Payment letter" id="payment_letter" name="payment_letter" class="formbutton" style="width:150px;display:none" onClick="fnc_letter_print(7)" />
