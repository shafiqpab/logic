...
$mendatory_field_main = $_SESSION['logic_erp']['mandatory_field'][532];
...
<script>
    ...
    echo "var field_message = '". implode('*',$_SESSION['logic_erp']['field_message'][532]) . "';\n";
    ?>
	...
	function open_import_lc_popup(page_link,title)
	{
		...
		emailwindow.onclose=function()
		{
			...
			if (theemail.value!="")
			{
				...
				var lc_popup=1;
				get_php_form_data(theemail.value+'_'+lc_popup, "populate_data_from_btb_lc", "requires/import_document_acceptance_bank_controller" );
				show_list_view(theemail.value+'_'+lc_popup+'___'+impoter_id,'pi_listview','pi_list','requires/import_document_acceptance_bank_controller','');
	...
	function check_duplicate_invoice(invoice_no)
	{
	  if(invoice_no!="")
	  {
	   ...
	   var invoice_no_found=return_global_ajax_value(invoice_no+'_'+btb_lc_id, 'check_duplicate_invoice', '', 'requires/import_document_acceptance_bank_controller');
	   if(invoice_no==invoice_no_found)
	   {
		  ...
		  reset_form('importdocumentacceptance_1','','','','');
		  get_php_form_data(btb_lc_id+'_'+1, "populate_data_from_btb_lc", "requires/import_document_acceptance_bank_controller" );
		  show_list_view(btb_lc_id+'_'+1+'___'+impoter_id,'pi_listview','pi_list','requires/import_document_acceptance_bank_controller','');
	...
	function open_invoice_popup(page_link,title)
	{
		...
		emailwindow.onclose=function()
		{
			...
			if (theemail.value!="")
			{
			    ...
			    reset_form('importdocumentacceptance_1','','','','');
				var invoice_popup=2;
				get_php_form_data(theemail.value+'_'+invoice_popup, "populate_data_from_btb_lc", "requires/import_document_acceptance_bank_controller" );
				show_list_view(theemail.value+'_'+invoice_popup+'_'+theemail_invoice.value+'_'+posted_in_account+'_'+impoter_id+'_'+ref_closing_staus,'pi_listview','pi_list','requires/import_document_acceptance_bank_controller','');
				get_php_form_data(theemail_invoice.value, "populate_data_from_invoice", "requires/import_document_acceptance_bank_controller" );
				//show_list_view(theemail.value,'pi_listview','pi_list','requires/import_document_acceptance_bank_controller','');
	...
	function fnc_import_document_acceptance( operation )
	{
		...
		if(pay_term==1 || pay_term==2)
		{
			...
			var txt_doc_rcv_date=$('#txt_doc_rcv_date').val();
			if(txt_doc_rcv_date!="")
			{
			...
			{
				if (form_validation('txt_lc_number*txt_invoice_number*txt_invoice_date*txt_company_acc_date*cbo_retire_source','Lc Number*Invoice Number*Invoice Date*Company Acceptance Date*Retire Source')==false)
				{
					return;
				}
			}
		...
		if (form_validation('txt_bank_ref*txt_bank_acceptance_date*txt_exchange_rate','Bank Ref No*Bank Ac Date*Exchange Rate')==false)
		{
			return;
		}
		...
		//alert(bill_entry_value);return;
		if(document_value!=bill_entry_value && bill_entry_value>0)
		{
			alert("Document Value and Bill of Entry Value Must Be Same");return;
		}

		data_all=data_all+get_submitted_data_string('btb_lc_id*txt_invoice_number*invoice_id*txt_invoice_date*txt_document_value*txt_shipment_date*txt_company_acc_date*txt_bank_acceptance_date*txt_bank_ref*cbo_acceptance_time*cbo_retire_source*txt_remarks*txt_edf_tenor*cbo_lc_type_id*bill_no*bill_date*cbo_shipment_mode*cbo_document_status*cbo_forwarder_name*copy_doc_receive_date*original_doc_receive_date*doc_to_cnf*feeder_vessel*mother_vessel*eta_date*ic_receive_date*shipping_bill_no*cbo_inco_term*inco_term_place*port_of_loading*port_of_discharge*bill_of_entry_no*psi_reference_no*maturity_date*container_no*pkg_quantity*tot_current_acceptance_value*cbo_payterm_id*hid_maturity_from*nagotiate_date*edf_paid_date*etd_actual*pakg_uom*eta_advice*eta_actual*cbo_container_status*cbo_container_size*release_date*boe_date*txt_exchange_rate*txt_loan_reff*txt_doc_rcv_date*txt_local_date*cbo_ready_to_approved*hide_approved_status*txt_bill_entry_value*txt_submit_date*cbo_source_id',"../../");
		...
		http.open("POST","requires/import_document_acceptance_bank_controller.php",true);
		...
		if(http.readyState == 4)
		{
			//alert(http.responseText);release_freezing();return;
			 ...
			 else if(reponse[0]==0 || reponse[0]==1)
			 {
				...
				show_msg(reponse[0]);
				get_php_form_data(reponse[1]+'_'+2, "populate_data_from_btb_lc", "requires/import_document_acceptance_bank_controller" );
				show_list_view(reponse[1]+'_'+2+'_'+reponse[2]+'__'+inpoter_id,'pi_listview','pi_list','requires/import_document_acceptance_bank_controller','');
				get_php_form_data(reponse[2], "populate_data_from_invoice", "requires/import_document_acceptance_bank_controller" );
				set_button_status(1, permission, 'fnc_import_document_acceptance',1);
				fnc_com_acc_view(4);
				release_freezing();
			 }
		...
		/* if(payterm_id==2)
		 {
			 ...
			 {
				 var maturity_date=return_global_ajax_value(date+'_'+hid_tenor, 'set_maturity_date', '', 'requires/import_document_acceptance_bank_controller');
	...
	{
			var page_link='requires/import_document_acceptance_bank_controller.php?action=cumulative_details_popup&pi_id='+pi_id
	...
	function fnc_ship_grnt_______off()
	{
		...
		print_report(1+'**'+$('#invoice_id').val()+'**'+$('#txt_lc_number').val()+'**'+$('#cbo_importer_id').val(),'import_document_acceptance_letter','requires/import_document_acceptance_bank_controller');
	}
	...
	function fnc_consignment_________off()
	{
		...
		print_report(2+'**'+$('#invoice_id').val()+'**'+$('#txt_lc_number').val()+'**'+$('#cbo_importer_id').val(),'import_document_acceptance_letter','requires/import_document_acceptance_bank_controller');
	}
	...
	function fnc_letter_print(type)
	{
		...
		if(type==1){
		print_report(letter_type+'**'+$('#invoice_id').val()+'**'+$('#txt_lc_number').val()+'**'+$('#cbo_importer_id').val(),'import_document_acceptance_letter','requires/import_document_acceptance_bank_controller');
		}
		if(type==2){
			print_report($('#invoice_id').val(),'import_document_acceptance_forwarding2','requires/import_document_acceptance_bank_controller');
		
		}
		if(type==3){
			print_report($('#invoice_id').val(),'document_advance_payment_letter','requires/import_document_acceptance_bank_controller');
		
		}
		if(type==4){
			print_report($('#invoice_id').val(),'btb_acceptance_letter','requires/import_document_acceptance_bank_controller');
		
		}
		if(type==5){
			print_report($('#invoice_id').val(),'forwarding_letter3','requires/import_document_acceptance_bank_controller');
		
		}
		if(type==6){
			print_report($('#invoice_id').val(),'undertaking_ific','requires/import_document_acceptance_bank_controller');
		
		}
		if(type==7){
			print_report($('#invoice_id').val(),'payment_letter','requires/import_document_acceptance_bank_controller');
	...
	function check_exchange_rate()
	{
		...
		var response=return_global_ajax_value( cbo_currercy+"**"+booking_date, 'check_conversion_rate', '', 'requires/import_document_acceptance_bank_controller');
	...
	function fnc_com_acc_view(str)
	{
		show_list_view(str+'_'+str,'com_acc_listview','com_acc_list','requires/import_document_acceptance_bank_controller','setFilterGrid(\'list_view\',-1)');
	}
	
	function fn_set_inv_data(lc_id,inv_id,is_posted_account,impoter_id,ref_closing_staus)
	{
		var invoice_popup=2;
		get_php_form_data(lc_id+'_'+invoice_popup, "populate_data_from_btb_lc", "requires/import_document_acceptance_company_controller" );
		show_list_view(lc_id+'_'+invoice_popup+'_'+inv_id+'_'+is_posted_account+'_'+impoter_id+'_'+ref_closing_staus,'pi_listview','pi_list','requires/import_document_acceptance_company_controller','');
		get_php_form_data(inv_id, "populate_data_from_invoice", "requires/import_document_acceptance_company_controller" );
		set_button_status(0, permission, 'fnc_import_document_acceptance',1);
	}
...
<body onLoad="set_hotkey()">
    <div align="center" style="width:100%;">
        ...
        <div style="width:1000px; float:left;">
            <form name="importdocumentacceptance_1" id="importdocumentacceptance_1" autocomplete="off" data-entry_form="">
                <fieldset style="width:1000px;">
                    <legend>Company Acceptance Details :</legend>
                    <table width="100%" border="0" cellpadding="0" cellspacing="1" id="tbl_importdocumentacceptance">
                        ...
                        <tr>
                            ...
                            <td width="150">
                            <input type="text" name="txt_lc_number" id="txt_lc_number" class="text_boxes" placeholder="Double Click for LC No" onDblClick="open_import_lc_popup( 'requires/import_document_acceptance_bank_controller.php?action=open_import_lc_popup','BTB / Import LC List' );" style="width:140px" readonly disabled/>
                             <input type="hidden" name="btb_lc_id" id="btb_lc_id" value="" />

                            </td>
                            <td width="150" class="must_entry_caption">Invoice Number</td>
                            <td width="150"><input type="text" name="txt_invoice_number" id="txt_invoice_number" class="text_boxes" placeholder="Double Click for Invoice No" onDblClick="open_invoice_popup( 'requires/import_document_acceptance_bank_controller.php?action=open_invoice_popup','Import Invoice List' );" onChange="check_duplicate_invoice(this.value)" style="width:139px" readonly />
                        ...
                        <tr>
                            ...
                            <td>
                                 <input type="text" name="txt_invoice_date" id="txt_invoice_date" class="datepicker" value="<? echo date('d-m-Y') ?>" style="width:140px" disabled />
                            </td>
                            <td>Company Acc. Date</td>
                            <td>
                                 <input type="text" name="txt_company_acc_date" id="txt_company_acc_date" class="datepicker" style="width:139px" disabled />
                            </td>
						...
						<tr>
							...
							<td><input type="text" name="txt_doc_rcv_date" id="txt_doc_rcv_date" style="width:140px" class="datepicker" value="" disabled /></td>
                            <td>Local Doc. Send Date</td>
							<td><input type="text" name="txt_local_date" id="txt_local_date" style="width:140px" class="datepicker" value="" disabled /></td>
                            <td>L/C Type</td>
                             <td>
                        ...
                        <tr>
                            ...
                            <td>
                            <input type="text" name="txt_document_value" id="txt_document_value" class="text_boxes_numeric" style="width:139px" disabled readonly /></td>
							<td class="must_entry_caption">Source</td>
                            <td><? echo create_drop_down( "cbo_source_id", 151, $source,'', 1, 'Display',0,0,1); ?></td>
                        </tr>
                    </table>
                    <legend>Bank Acceptance Details :</legend>
                   	<table width="100%" border="0" cellpadding="0" cellspacing="1" id="tbl_importdocumentacceptance">     
                        <tr>
                            <td width="150" class="must_entry_caption">Bank Ref</td>
                            <td width="150"><input type="text" name="txt_bank_ref" id="txt_bank_ref" class="text_boxes" style="width:139px"/></td>
                            ...
                            <td width="150" class="must_entry_caption">Bank Acc. Date</td>
                            <td>
                                 <input type="text" name="txt_bank_acceptance_date" id="txt_bank_acceptance_date" class="datepicker" style="width:150px"  onChange="set_maturity_date(this.value,'bank_acc_date');  calculate_maturity_date();check_exchange_rate();"/>
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                    		...
                    		<td><? echo create_drop_down( "cbo_ready_to_approved", 165, $yes_no,"", 1, "-- Select--", 2, "","","" ); ?>
                ...
                <fieldset style="width:1000px; margin-bottom:10px;">
				<legend>BTB / Import LC Shipment Details</legend>
				<table width="100%" border="0" cellpadding="0" cellspacing="1">
					<tr>
						<td width="150">BL/Cargo No</td>
						<td width="150"><input type="text" name="bill_no" id="bill_no" class="text_boxes" style="width:140px" /></td>
						<td width="150">BL/Cargo Date</td>
						<td width="150"><input type="text" name="bill_date" id="bill_date" class="datepicker" value="" style="width:140px" onChange="set_maturity_date(this.value,'bl_date');  calculate_maturity_date();" /></td>
                    	<td width="150">Shipment Mode</td>
						<td id="shipment_td">
					...
					<tr>
						...
						<td>
							<input type="text" name="pkg_quantity" id="pkg_quantity" style="width:70px" class="text_boxes" value="" />
        ...
        <div style="width:380px; overflow:auto; float:left; position:relative;">
        <input type="button" value="VIEW" id="btn_view" name="btn_view" class="formbutton" style="width:150px;" onClick="fnc_com_acc_view(4)" />
        <div id="com_acc_list" style="margin: 5px 0;"></div>
        </div>
    </div>
</body>
