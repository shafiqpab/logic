...
$con=connect();
$sqlbom="SELECT ID, FABRIC_COST_DTLS_ID, JOB_NO, COLOR, COUNT_ID, COPM_ONE_ID, PERCENT_ONE, TYPE_ID FROM WO_PRE_COST_FAB_YARN_COST_DTLS WHERE STATUS_ACTIVE=1 and IS_DELETED=0";

$sql_result=sql_select($sqlbom); $bomyarnarr=array();
foreach($sql_result as $row)
{
	$bomyarnarr[$row['JOB_NO']][$row['COLOR']][$row['COUNT_ID']][$row['COPM_ONE_ID']][$row['PERCENT_ONE']][$row['TYPE_ID']]=$row['FABRIC_COST_DTLS_ID'];
}
unset($sql_result);

/*$sqlreq="SELECT B.JOB_NO, B.COLOR_ID, B.COUNT_ID, B.COMPOSITION_ID, B.COM_PERCENT, B.YARN_TYPE_ID FROM INV_PURCHASE_REQUISITION_MST A, INV_PURCHASE_REQUISITION_DTLS B WHERE A.ID=B.MST_ID AND A.STATUS_ACTIVE=1 AND B.STATUS_ACTIVE=1 AND A.ENTRY_FORM=70 and a.BASIS=5 order by b.id desc";
$sqlreqRes=sql_select($sqlreq);
$z=0;
foreach($sqlreqRes as $rrow)
{
	$fabid=0;
	$fabid=$bomyarnarr[$rrow['JOB_NO']][$rrow['COLOR_ID']][$rrow['COUNT_ID']][$rrow['COMPOSITION_ID']][$rrow['COM_PERCENT']][$rrow['YARN_TYPE_ID']];
	
	$uptxt="update INV_PURCHASE_REQUISITION_DTLS set bom_fabric_id='".$fabid."' where JOB_NO ='".$rrow['JOB_NO']."' and COLOR_ID ='".$rrow['COLOR_ID']."' and  COUNT_ID='".$rrow['COUNT_ID']."' and COMPOSITION_ID='".$rrow['COMPOSITION_ID']."' and COM_PERCENT='".$rrow['COM_PERCENT']."' and YARN_TYPE_ID='".$rrow['YARN_TYPE_ID']."'";
	//echo $uptxt.'<br>';
	$rIDMst=execute_query($uptxt);
	$z++;
}
unset($sqlreqRes);

echo $z; oci_commit($con); disconnect($con); die;*/
?>
<script>
	var permission = '<?=$permission; ?>';
	if ($('#index_page', window.parent.document).val() != 1) window.location.href = "../../logout.php";

	var str_color = [<?=substr(return_library_autocomplete("select color_name from lib_color group by color_name", "color_name"), 0, -1); ?>];
	...
	function openmypage_req() {
		...
		emailwindow.onclose = function() {
            ...
            // alert(hidden_wo_number)
			$("#txt_wo_number").val(hidden_wo_number[2]);
	...
	function openmypage_job(row_no) {
		if (form_validation('cbo_company_name', 'Company Name') == false) {
			...
			if (basis == 4) {
				...
				page_link = 'requires/yarn_requisition_entry_controller.php?action=order_search_popup&company=' + company + '&buyer_name=' + buyer_name + '&cbo_basis=' + basis;
				var width = "620px";
			}
			...
			emailwindow.onclose = function() {
				...
				//alert(basis);
				//freeze_window(5);
				var txt_selected_id_val = this.contentDoc.getElementById("txt_selected_id_val").value ;
				var txt_selected = this.contentDoc.getElementById("txt_selected_val").value ;
				var arr = [];
				var job_noArr = [];
				arr.push(txt_selected_id_val);
				job_noArr.push(txt_selected);
				...
				if (basis == 4) {
					...
					var dtls_html = return_global_ajax_value(arr + '_' + job_noArr  + '_' + row_no + '_' + company + '_' + basis, 'dtls_part_html_row', '', 'requires/yarn_requisition_entry_controller');
				}
	...
	function openmypage_yarn_color() {
		...
		var company = $("#cbo_company_name").val();
		var title = "Yarn Color Selection";
		page_link = 'requires/yarn_requisition_entry_controller.php?action=color_selection&company=' + company;
		var width = '450px';

		emailwindow = dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=' + width + ', height=390px,center=1,resize=1,scrolling=0', '../')
		emailwindow.onclose = function() {
			var theform = this.contentDoc.forms[0];
			var data = this.contentDoc.getElementById("hidden_color_id").value.split("_");
			//var num_of_tr = $("#details_part_list tr").length;
			//alert(num_of_tr);
			//$(this).val(data[1]);
			$("#hidden_txtyarncolor_id").val(data[0]);
		}
		//var color_settings_array = return_global_ajax_value(company, action, '', 'requires/yarn_requisition_entry_controller');
	}

	function load_yarn_color_settings(company, action) {
		var color_settings = return_global_ajax_value(company, action, '', 'requires/yarn_requisition_entry_controller');
		if (color_settings == 1) {
			$("#txtyarncolor_1").attr({
				placeholder: "Browse",
				onDblClick: "openmypage_yarn_color()",
				readonly: "readonly"
			});
		}
	...
	function openmypage_wo(row_no) {
		if (form_validation('cbo_company_name', 'Company Name') == false) {
			...
			page_link = 'requires/yarn_requisition_entry_controller.php?action=order_search_with_wo_popup&company=' + company;
			var width = "795px";
			...
			emailwindow.onclose = function() {
				...
				var dtls_html = return_global_ajax_value(data[0] + '_' + data[1] + '_' + row_no + '_' + company + '_' + data[2], 'dtls_part_html_row_with_wo', '', 'requires/yarn_requisition_entry_controller');
	...
	function fnc_yarn_req_entry(operation) {
		if (operation == 4) {
			...
			print_report($('#cbo_company_name').val() + '*' + $('#update_id').val() + '*' + report_title, "yarn_requisition_print", "requires/yarn_requisition_entry_controller")
			return;
		} else if (operation == 6) {
			...
			if (form_validation('cbo_company_name*cbo_item_category*txt_delivery_date*txt_wo_date*txtjobno_1', 'Company Name*Item Category*Delivery Date*WO Date*Job No') == false) {
				return;
			}
			...
			for (var i = 1; i <= total_row; i++) {
				try {
					if ($('#reqqnty_' + i).val() != "" && $('#tr_' + i).css('display') != 'none') {
						...
						{
							alert("Rate Can not be 0 or less than 0");
							$("#txtrate_"+i).focus();
							return;
						}*/
						detailsData += '*txtjobno_' + i + '*txtjobid_' + i + '*txtwono_' + i + '*txtwoid_' + i + '*cbobuyername_' + i + '*txtstyleno_' + i +'*txtyarncolor_' + i + '*cbocount_' + i + '*cbocompone_' + i + '*txtpacent_' + i + '*cbotype_' + i + '*cbouom_' + i + '*reqqnty_' + i + '*txtrate_' + i + '*txtamount_' + i + '*txtyarndate_' + i + '*txtremarks_' + i + '*txtrowid_' + i + '*hiderow_' + i + '*hidbomfabid_' + i;
		...
		if ($("#cbo_basis").val() == 7) {
			...
			if ($("#txtjobno_" + row_id).val() != "" || $("#txtwono_" + row_id).val() != "") {
				if (parseFloat($("#hiddenreqqnty_" + row_id).val()) >= parseFloat($('#reqqnty_' + row_id).val())) {
					...
					//alert("Requisition Qty Should Less than Booking antu");
	...
	function add_break_down_tr(i) {
		...
		if (row_num != i) {
			...
			$("#tbl_purchase_item tbody tr:last").css({
				"height": "10px",
	...
	function fn_deletebreak_down_tr(rowNo, woNum) {
		var row_num = $('#tbl_purchase_item tbody tr').length;
		...
		//var dd = check_work_order_existance(req_dtls_id);
		//alert(dd);
		else {
			if (row_num != rowNo) {
				$('#tr_' + rowNo).hide();
	...
	function fnc_booking_td_change() {
		...
		if (basis == 4) {
			...
			if ($("#tbl_purchase_item").find("thead th:contains(Requisition Qty.)").attr("class") == "must_entry_caption") {
				$("#tbl_purchase_item").find("thead th.must_entry_caption").eq(1).removeClass("must_entry_caption");
			}
			...
			if ($("#tbl_purchase_item").find("thead th:contains(Requisition Qty.)").attr("class") == "") {
				$("#tbl_purchase_item").find("thead th:contains(Requisition Qty.)").eq(0).addClass("must_entry_caption");
			}
		}
	}
	...
	<div align="center">
		...
		<fieldset style="width:1350px">
			<form name="yarnRequisition_1" id="yarnRequisition_1" method="">
				<table cellspacing="2" width="1350px" border="1" rules="all">
					...
					<tr>
						...
						<td><input type="text" name="txt_wo_number" id="txt_wo_number" class="text_boxes" style="width:158px" placeholder="Double Click to Search" onDblClick="openmypage_req('x','WO Number Search');" readonly />
					...
					<tr>
						...
						<td width="100" style="display:none" align="right">Supplier&nbsp;</td>
						<td width="170" id="supplier_td" style="display:none">
							<?
							echo create_drop_down("cbo_supplier", 170, $blank_array, "", 1, "-- Select --", 0, "", 0);
							?>
						</td>

						<td width="80" align="right">Pay Mode&nbsp;</td>
						<td style="width: 100px;"><?
													echo create_drop_down("cbo_pay_mode", 170, $pay_mode, "", 1, "-- Select --", 0, "", 0);
													?></td>
						<td width="120px" class="must_entry_caption" align="right">Requisition Date&nbsp;</td>
						<td><input type="text" name="txt_wo_date" value="<? echo date('d-m-Y'); ?>" id="txt_wo_date" class="datepicker" style="width:158px" disabled /></td>
						<!--this field temporary hidden-->
						<!-- <td  class="must_entry_caption" align="right">Required Date&nbsp;</td>
						<td ><input type="text" name="txt_delivery_date"  id="txt_delivery_date" class="datepicker"  style="width:159px" /></td> -->
					</tr>
					...
					<tr>
						...
						<td>
							<?
							echo create_drop_down("cbo_basis", 170, $issue_basis, "", 0, "-- Select--", 5, "fnc_booking_td_change()", "", "", "", "", "3");
							?>
						</td>
						...
						<td colspan="3"><input type="text" name="txt_remarks" id="txt_remarks" style="width:410px " class="text_boxes" maxlength="140" /></td>
						<td class="must_entry_caption" align="right">Required Date&nbsp;</td>
						<td><input type="text" name="txt_delivery_date" id="txt_delivery_date" class="datepicker" style="width:158px" /></td>
					</tr>
					<tr>
						<td>&nbsp;</td>
					</tr>
					<tr>
						...
						<td>&nbsp;</td>
						<td>&nbsp;</td>
						<td style="padding-right: 140px;" align="center" height="10" colspan="6">
							<?
				...
				<br />
				<table class="rpt_table" width="1255" cellspacing="0" cellpadding="0" id="tbl_purchase_item" border="1" rules="all">
					<thead>
						<th width="100" id="job_no_td" class="must_entry_caption">Job No</th>
						<th width="80" style="display:none">Fab.Booking</th>
						...
						<th width="60">UOM</th>
						<th width="80" class="must_entry_caption">Requisition Qty.</th>
						<th width="60">Rate</th>
						<th width="90">Amount</th>
						<th width="100">Yarn Inhouse Date</th>
					...
					<tbody id="details_part_list">
						<tr class="general" id="tr_1">
							<td align="center">
								<input type="text" name="txtjobno_1" id="txtjobno_1" class="text_boxes" value="" style="width:100px;" onDblClick="openmypage_job(1)" placeholder="Doble Click For Job" readonly />

								<input type="hidden" id="txtjobid_1" name="txtjobid_1" style="width:100px;">
								<input type="hidden" name="txtrowid_1" id="txtrowid_1" class="text_boxes" value="" style="width:100px;" />
								<input type="hidden" name="hiderow_1" id="hiderow_1" class="text_boxes" value="0" style="width:100px;" />
                                <input type="hidden" name="hidbomfabid_1" id="hidbomfabid_1" class="text_boxes" value="" style="width:70px;" />
							</td>
							<td style="display:none">
								<input type="text" name="txtwono_1" id="txtwono_1" class="text_boxes" value="" style="width:80px;" onDblClick="openmypage_wo(1)" placeholder="Doble Click For WO Number" readonly />
							...
							<td id="buy_td">
								<?
								echo create_drop_down("cbobuyername_1", 100, "select id, buyer_name from lib_buyer where status_active=1 and is_deleted=0 order by buyer_name", "id,buyer_name", 1, "-- Select --", 0, "");
								?>
							</td>
							<td align="center"><input type="text" name="txtstyleno_1" id="txtstyleno_1" class="text_boxes" value="" style="width:120px;" /></td>
							<td align="center"><input type="text" name="txtyarncolor[]" id="txtyarncolor_1" class="text_boxes" value="" style="width:80px;" onFocus="add_auto_complete( 1 )" />
								<input type="hidden" id="hidden_txtyarncolor_id" readonly />
							</td>
							<td align="center">
								<?
								echo create_drop_down("cbocount_1", 100, "select id,yarn_count from  lib_yarn_count where is_deleted=0 and status_active=1 order by yarn_count", "id,yarn_count", 1, "-- Select --", 0, "", 0, "");
								?>
							</td>
							<td align="center">
								<?
								echo create_drop_down("cbocompone_1", 100, $composition, "", 1, "-- Select --", 0, "", 0, "");
								?>
							</td>
							<td><input type="text" name="txtpacent_1" id="txtpacent_1" class="text_boxes" value="100" style="width:50px;" /></td>
							<td>
								<?
								echo create_drop_down("cbotype_1", 100, $yarn_type, "", 1, "-- Select --", $row[csf("type_id")], "", $disabled, "");
								?>
							</td>
							<td>
								<?
								echo create_drop_down("cbouom_1", 60, $unit_of_measurement, "", 1, "-- Select--", 15, "", 1);
								?>
							</td>
							<td>
								<input type="text" id="reqqnty_1" name="reqqnty_1" class="text_boxes_numeric" style="width:80px" onKeyUp="calculate_amount(1)" />
								<input type="hidden" id="hiddenreqqnty_1" name="hiddenreqqnty_1" value="" />
							</td>
							<td>
								<input type="text" name="txtrate_1" id="txtrate_1" class="text_boxes_numeric" value="" style="width:60px;" onKeyUp="calculate_amount(1)" />
								<input type="hidden" id="hiddentxtrate_1" name="hiddentxtrate_1" value="" />
							</td>
							<td><input type="text" name="txtamount_1" id="txtamount_1" class="text_boxes_numeric" value="" style="width:90px;" readonly /></td>
							<td><input class="datepicker" type="text" style="width:100px;" name="txtyarndate_1" id="txtyarndate_1" placeholder="Select Date" /></td>
				...
				<table cellpadding="0" cellspacing="2" width="100%">
					<tr>
						<td align="center" colspan="6" valign="middle" class="button_container">
							...
							<?
							echo load_submit_buttons($permission, "fnc_yarn_req_entry", 0, 1, "reset_form('yarnRequisition_1','','','','','');", 1);
							?>
