...
<script>
    ...
    function print_to_html_report(type)
	{
		var report_title=$( "div.form_caption" ).html();
					//alert(report_title);
		if(type == 1){	
        	window.open("requires/yarn_work_order_controller.php?data=" + $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+type+'*'+$('#ref_closed_sts').val()+'&action='+"print_to_html_report", true );
        }
		else if(type == 4)
		{
			var rate_amount='';
        	...
        	window.open("requires/yarn_work_order_controller.php?data=" + $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+type+'*'+$('#ref_closed_sts').val()+'*'+rate_amount+'&action='+"print_to_html_report4", true );    
        }
        else
        {
        	window.open("requires/yarn_work_order_controller.php?data=" + $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+type+'*'+$('#ref_closed_sts').val()+'&action='+"print_to_html_report2", true );    
        }
    }
	...
	function fnc_yarn_order_entry(operation)
	{
		if(operation==4)
		 {
			var report_title=$( "div.form_caption" ).html();
			print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+$('#ref_closed_sts').val(),"yarn_work_order_print", "requires/yarn_work_order_controller")
			return;
		 }
			...
			else
			{
				...
				for(var i=1;i<=row;i++)
				{
					...
					{ 
						...
						detailsData+='*txt_po_brakdown_id_'+i+'*txt_color_'+i+'*hidden_colorID_'+i+'*cbocount_'+i+'*cbocompone_'+i+'*percentone_'+i+'*cbocomptwo_'+i+'*percenttwo_'+i+'*cbotype_'+i+'*cbo_uom_'+i+'*txt_quantity_'+i+'*txt_rate_'+i+'*txt_amount_'+i+'*txt_inhouse_date_'+i+'*txt_delivery_end_date_'+i+'*txt_row_id_'+i+'*txt_remarks_'+i+'*txt_number_of_lot_'+i;
					}
					catch(err){}
				}
			}
    ...
    <div style="width:1150px;"><? echo load_freeze_divs ("../../",$permission);  ?><br /></div>
        <fieldset style="width:1370px">
            <form name="yarnWorkOrder_1" id="yarnWorkOrder_1" method="" >
                ...
                <br/>
                <div style="width:1350px" id="details_container" align="left"></div>
                <table cellpadding="0" cellspacing="2" width="100%">
                    <tr>
                        ...
                        <input type="button" style="width:80px;" id="id_print_to_button"  onClick="print_to_html_report(4)"   class="formbutton_disabled printReport" name="id_print_to_button" value="Print4" />
                        </td>
                        <td>&nbsp;</td>				
                    </tr>
                </table> 
