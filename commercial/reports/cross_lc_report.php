<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Report Will about Create Cross LC Report.
Functionality	:
JS Functions	:
Created by		:	Safa
...
//--------------------------------------------------------------------------------
echo load_html_head_contents("Cross LC Report", "../../", 1, 1,'','','');
?>
...
function generate_report(operation)
{
    ...
    {	
        ...
        var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_lien_bank*txt_file_no*cbo_year',"../../")+'&report_title='+report_title+'&report_type='+operation;
        // alert(data);return;
		freeze_window(3);
		http.open("POST","requires/cross_lc_report_controller.php",true);
...
function openFilePopup() {
	...
	var page_link='requires/cross_lc_report_controller.php?action=file_search_popup&company='+cbo_company_name+'&buyer='+cbo_buyer_name+'&file_year='+cbo_year;
				...
				<fieldset style="width:100%" >
					<table class="rpt_table" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" width="800">
						...
						<tr class="general">
							<td align="center">
								<?
								echo create_drop_down( "cbo_company_name", 200, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 and comp.core_business in(1,3) $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/cross_lc_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );get_php_form_data(this.value,'print_button_variable_setting','requires/cross_lc_report_controller' );" );
							...
							<td align="center">
                            <input type="button" name="search2" id="search2" value="Show2" onClick="generate_report(2)" style="width:100px" class="formbutton" />
							</td> -->
							</td>
