...
function generate_report(rptType)
	{
		...
		var data="action=report_generate&rptType="+rptType+get_submitted_data_string("cbo_company_name*cbo_buyer_name*cbo_lien_bank*cbo_search_by*txt_exchange_rate","../../")+'&report_title='+report_title;
		freeze_window(3);
		http.open("POST","requires/export_statement_report_controller.php",true);
    ...
    <fieldset style="width:1000px;">
        <table class="rpt_table" cellspacing="0" cellpadding="0" width="1000">
            <thead>
                ...
                <th width="120">Echg. Rate</th>
                <th width="220"><input type="reset" name="res" id="res" value="Reset" style="width:100px" class="formbutton" onClick="reset_form('frm_export_statement','report_container*report_container2','','','')" /></th>
            </thead>
            <tbody>
                <tr >
                    <td>
                    <?
                    echo create_drop_down( "cbo_company_name", 155, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 and comp.core_business in(1,3) $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/export_statement_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                    ?>
                    </td>
                    <td id="buyer_td"><? 
                    ...
                    <td>
                    <input type="text" id="txt_exchange_rate" name="txt_exchange_rate" style="width:130px" class="text_boxes_numeric">
                    </td>
                    <td align="center">
                    <input type="button" name="search" id="search" value="Show" onClick="generate_report(1)" style="width:100px" class="formbutton" />
                    <input type="button" name="search" id="search" value="Show Excell" onClick="generate_report(2)" style="width:100px" class="formbutton" />
                    </td>
                </tr>
            </tbody>
        </table>
