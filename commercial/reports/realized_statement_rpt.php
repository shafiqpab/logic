<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Realized Statement Report.
Functionality	:
JS Functions	:
Created by		:	Nayem
Creation date 	: 	27-7-2022
Updated by 		:
Update date		:
QC Performed BY	:
QC Date			:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Realized Statement Report", "../../", 1, 1,'','','');
?>
<script>
	...
	function generate_report()
	{
		...
		//alert(data);return;
		freeze_window(3);
		http.open("POST","requires/realized_statement_rpt_controller.php",true);
...
<div style="width:100%" align="center">
    <form id="report_form" name="report_form" action="" autocomplete="off" method="post">
				...
				<table class="rpt_table" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" width="900">
					<thead>
						<tr>
							...
							<th align="center"><input type="reset" name="reset" id="reset" value="Reset" class="formbutton" style="width:90px" onClick="reset_form('report_form','report_container*report_container2','','','')" /></th>
						</tr>
					</thead>
					<tbody>
						<tr class="general">
							<td align="center">
								<? echo create_drop_down( "cbo_company_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 and comp.core_business in(1,3) $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/realized_statement_rpt_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" );?>
							</td>
							<td align="center">
							...
							<td>
								<input type="text" id="txt_bill_no" name="txt_bill_no" class="text_boxes" style="width:90px;" placeholder="Write">
							</td>
							<td>
								<input name="txt_date_from" id="txt_date_from" class="datepicker" style="width:60px">&nbsp;To&nbsp;<input name="txt_date_to" id="txt_date_to" class="datepicker" style="width:60px">
							</td>
							<td align="center">
								<input type="button" name="show" id="show" onClick="generate_report();" class="formbutton" style="width:90px" value="Show" />
							</td>
						</tr>
						<tr>
