...
<script>
    ...
    function openmypage(company,file_no,bank_id,text_year,lc_sc,lc_sc_id,action)
    {
        ...
        emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/export_lc_and_sales_contract_report_controller.php?file_no='+file_no+'&company_name='+company+'&bank_id='+bank_id+'&text_year='+text_year+'&lc_sc_type='+lc_sc +'&lc_sc_id='+lc_sc_id+'&action='+action, title, 'width='+popup_width+',height=350px,center=1,resize=0,scrolling=0','../');
    }
    function openmypage_file(action,lc_sc_id,type)
	{
		var page_link='requires/export_lc_and_sales_contract_report_controller.php?action='+action+'&lc_sc_id='+lc_sc_id+'&type='+type
		var title="File View";
                ...
                <table class="rpt_table" cellspacing="0" cellpadding="0" width="1170">
                    ...
                    <tbody>
                        <tr >
                            <td>
                            <?
                                echo create_drop_down( "cbo_company_name", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 and comp.core_business in(1,3)  $company_cond order by comp.company_name","id,company_name", 1, "- Select Company -", $selected, "load_drop_down( 'requires/export_lc_and_sales_contract_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/export_lc_and_sales_contract_report_controller',this.value, 'load_drop_down_applicant', 'applicat_td' );" );
                            ?>
                            </td>
                            <td>
                            ...
                            <td id="date_type_td">
                                <? 
                                    $date_type_array=array(1=>"LC Date", 2=>"Lien Date", 3=>"LC/SC Insert Date");
