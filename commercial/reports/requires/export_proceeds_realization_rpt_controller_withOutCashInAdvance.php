...
{ 
	...
	$sql="select a.id as bill_id, a.company_id, a.lien_bank, a.buyer_id, a.bank_ref_no, b.id as rlz_id, b.received_date as rlz_date, c.type, c.account_head, c.document_currency, c.domestic_currency from com_export_doc_submission_mst a, com_export_proceed_realization b, com_export_proceed_rlzn_dtls c where a.id=b.invoice_bill_id and b.id=c.mst_id and b.is_invoice_bill=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.company_id=$cbo_company_name $sql_cond";
	...
	{
		$result_data[$row[csf("bill_id")]]["company_id"]=$row[csf("company_id")];
		$result_data[$row[csf("bill_id")]]["lien_bank"]=$row[csf("lien_bank")];
		$result_data[$row[csf("bill_id")]]["buyer_id"]=$row[csf("buyer_id")];
		$result_data[$row[csf("bill_id")]]["bank_ref_no"]=$row[csf("bank_ref_no")];
		$result_data[$row[csf("bill_id")]]["rlz_id"]=$row[csf("rlz_id")];
		$result_data[$row[csf("bill_id")]]["rlz_date"]=$row[csf("rlz_date")];
		$result_data[$row[csf("bill_id")]]["document_currency"]+=$row[csf("document_currency")];
		$result_data[$row[csf("bill_id")]]["domestic_currency"]+=$row[csf("domestic_currency")];
		...
		$rlz_data[$row[csf("bill_id")]][$row[csf("type")]][$row[csf("account_head")]]["document_currency"]+=$row[csf("document_currency")];
		$rlz_data[$row[csf("bill_id")]][$row[csf("type")]][$row[csf("account_head")]]["domestic_currency"]+=$row[csf("domestic_currency")];
	}
	//echo "bill<pre>"; print_r($result_data);echo " deduct <pre>"; print_r($count_collspan_deduction); echo " distri <pre>"; print_r($count_collspan_distribution);echo " rlz <pre>"; print_r($rlz_data);die;
	$table_width=950+(240*count($count_collspan_deduction))+(240*count($count_collspan_distribution));
	$dib_width=$table_width+20;
	ob_start();
?>
<div style="width:<? echo $dib_width?>px;" id="scroll_body">
    ...
    <table width="<? echo $table_width?>" cellpadding="0" cellspacing="0" class="rpt_table" border="1" rules="all" id="" align="left">
        <thead>
            <tr>
                ...
                <th width="100" rowspan="3">Bill No</th>
		...
		foreach($result_data as $bill_id=>$b_row)
		{
			if ($i%2==0)
				$bgcolor="#E9F3FF";
			else
            ...
            <tr bgcolor="<? echo $bgcolor?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                ...
                <td><p><? echo $company_arr[$b_row["company_id"]]; ?></p></td>
                <td><p><? echo $bank_arr[$b_row["lien_bank"]]; ?>&nbsp;</p></td>
                <td><p><? echo $buyer_arr[$b_row["buyer_id"]]; ?>&nbsp;</p></td>
                <td align="center"><p><a href="##" onclick="openmypage_inv('<? echo $bill_id; ?>','inv_details')"><? echo $b_row["bank_ref_no"]; ?></a>&nbsp;</p></td>
                <td align="center"><p><? if($b_row["rlz_date"]!="" && $b_row["rlz_date"]!="0000-00-00") echo change_date_format($b_row["rlz_date"]); ?>&nbsp;</p></td>
                <td align="right"><? echo number_format($b_row["document_currency"],4); $tot_document_currency+=$b_row["document_currency"]; ?></td>
                <td align="right"><? echo number_format($conver_rate,4); ?></td>
                <?
				foreach($count_collspan_deduction as $head_id)
				{
					?>
					<td align="right"><? echo number_format($rlz_data[$bill_id][0][$head_id]["document_currency"],4); ?></td>
                    <td align="right"><? if($rlz_data[$bill_id][0][$head_id]["domestic_currency"]>0 && $rlz_data[$bill_id][0][$head_id]["document_currency"]>0) echo number_format($rlz_data[$bill_id][0][$head_id]["domestic_currency"]/$rlz_data[$bill_id][0][$head_id]["document_currency"],4); ?></td>
                    <td align="right"><? echo number_format($rlz_data[$bill_id][0][$head_id]["domestic_currency"],4); ?></td>
					<?
					$tot_deduction[$head_id]["document_currency"]+=$rlz_data[$bill_id][0][$head_id]["document_currency"];
					$tot_deduction[$head_id]["domestic_currency"]+=$rlz_data[$bill_id][0][$head_id]["domestic_currency"];
				}
				foreach($count_collspan_distribution as $head_id)
				{
					?>
					<td align="right"><? echo number_format($rlz_data[$bill_id][1][$head_id]["document_currency"],4); ?></td>
                    <td align="right"><? if($rlz_data[$bill_id][1][$head_id]["domestic_currency"]>0 && $rlz_data[$bill_id][1][$head_id]["document_currency"]>0) echo number_format($rlz_data[$bill_id][1][$head_id]["domestic_currency"]/$rlz_data[$bill_id][1][$head_id]["document_currency"],4); ?></td>
                    <td align="right"><? echo number_format($rlz_data[$bill_id][1][$head_id]["domestic_currency"],4); ?></td>
					<?
					$tot_distribution[$head_id]["document_currency"]+=$rlz_data[$bill_id][1][$head_id]["document_currency"];
					$tot_distribution[$head_id]["domestic_currency"]+=$rlz_data[$bill_id][1][$head_id]["domestic_currency"];
				}
				?>
                <td align="right"><? echo number_format($b_row["domestic_currency"],4); $tot_domestric_currency+=$b_row["domestic_currency"]; ?></td>
            </tr>
            <?
			$i++;
		}
