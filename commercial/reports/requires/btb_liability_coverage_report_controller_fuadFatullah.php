            ...
            <table width="550" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr>
                        ...
                        <td align="center">
                        	<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view ('<? echo $companyID; ?>'+'**'+'<? echo $lien_bank; ?>'+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('txt_internal_file_no').value, 'create_file_no_search_list_view', 'search_div', 'btb_liability_coverage_report_controller', 'setFilterGrid(\'tbl_list_search\',-1)');" style="width:100px;" />
                    </td>
                    </tr>
            	</tbody>
           	</table>
...
if($action=="create_file_no_search_list_view")
{
	...
	if($db_type==0)
	{
		...
		(
			select a.internal_file_no, a.lc_year, a.lien_bank, a.buyer_name from com_export_lc a where a.beneficiary_name='$company_id' and a.buyer_name like '$buyer_name' and a.lien_bank like '$lien_bank' and a.internal_file_no like '$internal_file_no' and a.status_active=1 and a.is_deleted=0 group by a.internal_file_no, a.lc_year
		union 
	...
	echo create_list_view("tbl_list_search", "File No,Year,Buyer,Lien Bank", "120,100,120","580","230",0, $sql , "js_set_value", "internal_file_no", "", 1, "0,0,buyer_name,lien_bank", $arr , "internal_file_no,lc_year,buyer_name,lien_bank", "","",'0,0,0,0','',1) ;
...
{ 
	...
	{
		$file_no_cond_lc="and a.internal_file_no in(".str_replace("'","",$txt_internal_file_no).")";
		$file_no_cond_sc="and b.internal_file_no in(".str_replace("'","",$txt_internal_file_no).")";	
	}
	...
	$yarn_cons_cost_arr=return_library_array( "select job_no, sum(avg_cons_qnty*rate) as amount from wo_pre_cost_fab_yarn_cost_dtls where status_active=1 and is_deleted=0 group by job_no","job_no","amount");
			...
			<div style="width:5000px;">
				<fieldset style="width:100%;">	 
					<table cellpadding="0" cellspacing="0" width="100%">
						<tr>
						   <td colspan="33" align="center" width="100%" style="font-size:18px"><b>BTB Liability Coverage Report</b></td>
						</tr>
						<tr>
						   <td colspan="33" align="center" width="100%" style="font-size:18px"><b><? echo $company_library[$company_name]; ?></b></td>
						</tr>
						<tr><td colspan="33" height="15"></td></tr>
					</table>
					<table width="100%" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all">
						<thead>
							<tr>
								...
								<th width="120" rowspan="2">SC Amount</th>
					...
					<div style="width:5000px; max-height:400px; overflow-y:scroll" id="dfc_liability">
						<table width="100%" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all">
						<?
							$internal_file_no_arr=array(); $lc_arr=array(); $sc_arr=array(); $lc_claim_comn_arr=array(); $sc_claim_comn_arr=array(); 
							$i=1; $lc_ids=''; $sc_ids='';
							
							$sql="select a.internal_file_no, a.id as lc_sc_id, a.bank_file_no, a.export_lc_no as no, a.replacement_lc as repl_conv, a.lc_value as value, a.pay_term, a.tenor, a.last_shipment_date, a.remarks, a.foreign_comn, a.lien_bank, a.currency_name, a.buyer_name, a.applicant_name, a.claim_adjustment, 0 as converted_from, 1 as type from com_export_lc a where a.beneficiary_name='$company_name' and a.lien_bank like '$lien_bank_id' and a.lc_year like '$file_year' and a.status_active=1 and a.is_deleted=0 $file_no_cond_lc 
								union all 
							...
							if($lc_ids!="")
							{
								$invoiceData=sql_select("select id, is_lc, lc_sc_id, net_invo_value, (discount_ammount+bonus_ammount+claim_ammount) as deduct_amnt from com_export_invoice_ship_mst where status_active=1 and is_deleted=0 and is_lc=1 and lc_sc_id in($lc_ids)");
								...
								$sql_opened_lc="select b.lc_sc_id, c.id, c.lc_value, c.payterm_id from com_btb_export_lc_attachment b, com_btb_lc_master_details c where b.is_lc_sc=0 and b.import_mst_id=c.id and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.lc_sc_id in($lc_ids) group by b.lc_sc_id, c.id, c.lc_value, c.payterm_id";
								...
								$pc_drawn_lc_arr=return_library_array( "select lc_sc_id, sum(amount) as amount from com_pre_export_lc_wise_dtls where export_type=1 and lc_sc_id in($lc_ids)","lc_sc_id","amount");
								
								if($db_type==0)
								{
									$subm_id_lc_arr=return_library_array( "select lc_sc_id, group_concat(doc_submission_mst_id) as sub_id from com_export_doc_submission_invo where is_lc=1 and lc_sc_id in($lc_ids)","lc_sc_id","sub_id");
								}
								else
								{
									$subm_id_lc_arr=return_library_array( "select lc_sc_id, LISTAGG(doc_submission_mst_id, ',') WITHIN GROUP (ORDER BY doc_submission_mst_id) as sub_id from com_export_doc_submission_invo where is_lc=1 and lc_sc_id in($lc_ids)","lc_sc_id","sub_id");
								}
							...
							if($sc_ids!="")
							{
								$invoiceData=sql_select("select id, is_lc, lc_sc_id, net_invo_value, (discount_ammount+bonus_ammount+claim_ammount) as deduct_amnt from com_export_invoice_ship_mst where status_active=1 and is_deleted=0 and is_lc=2 and lc_sc_id in($sc_ids)");
								foreach($invoiceData as $invRow)
								{
									$net_inv_val_arr[$invRow[csf('is_lc')]][$invRow[csf('lc_sc_id')]]['net_invo_value']+=$invRow[csf('net_invo_value')];
									$net_inv_val_arr[$invRow[csf('is_lc')]][$invRow[csf('lc_sc_id')]]['deduct_amnt']+=$invRow[csf('deduct_amnt')];
								...
								$sql_opened_sc="select b.lc_sc_id, c.id, c.lc_value, c.payterm_id from com_btb_export_lc_attachment b, com_btb_lc_master_details c where b.is_lc_sc=1 and b.import_mst_id=c.id and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.lc_sc_id in($sc_ids) group by b.lc_sc_id, c.id, c.lc_value, c.payterm_id";
								...
								$pc_drawn_sc_arr=return_library_array( "select lc_sc_id, sum(amount) as amount from com_pre_export_lc_wise_dtls where export_type=2 and lc_sc_id in($sc_ids)","lc_sc_id","amount");
								
								if($db_type==0)
								{
									$subm_id_sc_arr=return_library_array( "select lc_sc_id, group_concat(doc_submission_mst_id) as sub_id from com_export_doc_submission_invo where is_lc=2 and lc_sc_id in($sc_ids)","lc_sc_id","sub_id");
								}
								else
								{
									$subm_id_sc_arr=return_library_array( "select lc_sc_id, LISTAGG(doc_submission_mst_id, ',') WITHIN GROUP (ORDER BY doc_submission_mst_id) as sub_id from com_export_doc_submission_invo where is_lc=2 and lc_sc_id in($sc_ids)","lc_sc_id","sub_id");
								}
							...
							$proc_real_data=sql_select("select d.is_invoice_bill, d.invoice_bill_id,
														...
														sum(CASE WHEN e.type=0 THEN e.document_currency END) AS shortrealized,
														sum(CASE WHEN e.type=1 THEN e.document_currency END) AS realized
							...
							$btb_paid_arr=array();
							$paid_sql="select a.btb_lc_id, b.adj_source, sum(b.accepted_ammount) as accepted_ammount from com_import_invoice_mst a,  b where a.id=b.invoice_id and a.is_lc=1 and b.payment_head=40 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.btb_lc_id, b.adj_source";
							...
							foreach ($internal_file_no_arr as $internal_file_no=>$data)
							{
								...
								foreach($lcScData as $value)
								{
									...
									foreach($lcScOrArray as $val)
									{
										...
										$yarn_cons_cost=$yarn_cons_cost_arr[$job_no_mst];
								...
								$bill_id_lc=implode(",",array_unique(explode(",",chop($bill_id_lc,','))));
								foreach($btb_lc_data as $billId)
								{
								...
								$bill_id_sc=implode(",",array_unique(explode(",",chop($bill_id_sc,','))));
								...
								$bl=number_format($unrealized_val+$ship_balance,2,'.','');
								...
								foreach($lcScData as $value)
								{
									...
									?>
                                        <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                                            ...
                                            <td width="100">
                                                <? 
                                                    echo "<a href='#report_details' style='color:#000' onclick= \"generate_report_file('".$internal_file_no."**".$buyer_name."**".$applicant_name."**".$company_name."**".$lien_bank_id."**".$file_year."','internal_file_no_info','requires/internal_file_info_report');\">".$internal_file_no."</a>"; 
                                                ?>
                                            </td>
											...
											?>
                                                ...
                                                <td width="80" align="center"><? echo $convertible_to_lc[$repl_conv].$suffix; ?></td>
                                                <td width="120" align="right"><? echo number_format($sc_val,2); ?></td>
                                                <td width="120" align="right"><? echo number_format($lc_val_replaced,2); ?></td>
                                                <td width="120" align="right">&nbsp;</td>
                                                <td width="120" align="right">&nbsp;</td>
                                            <?
											}
                                            ...
                                            <td width="120" align="right"><? 
                                                echo "<a href='#report_details' style='color:#000' onclick= \"openmypage('".$internal_file_no."',$company_name,'$lien_bank_id','$file_year','order_info','Order Info');\">".number_format($att_total_value,2)."</a>";
                                                $tot_order_value+=$att_total_value; 
                                                $file_order_value+=$att_total_value; 
                                                ?>
                                            </td>
                                            <td width="100" align="right"><? echo number_format($foreign_comn_cost,2); $tot_fore_comn+=$foreign_comn_cost; ?></td>
                                            <td width="60" align="right"><? echo number_format($foreign_comm_per,2); ?></td>
                                            <td width="100" align="right"><? echo number_format($local_comm,2); $tot_local_comn+=$local_comm; ?></td>
                                            <td width="60" align="right"><? echo number_format($local_comm_per,2); ?></th>
                                            <td width="100" align="right"><? echo number_format($carring_cost,2); $tot_carring_cost+=$carring_cost; ?></td>
                                            <td width="100" align="right"><? echo number_format($file_deduction,2); $tot_deducd+=$file_deduction; ?></td>
                                            <td width="60" align="right"><? echo number_format($file_deduction_per,2); ?></td>
                                            <td width="120" align="right"><? echo number_format($net_lc_value,2); $tot_net_lc_val+=$net_lc_value; ?></td>
                                            <td width="80" align="center"><? echo $currency[$currency_name]; ?></td>
                                            <td width="120" align="right"><? echo number_format($bbb_lc_entitle,2); $tot_bblc_entitle+=$bbb_lc_entitle; ?></td>
                                            <td width="120" align="right"><? echo "<a href='#report_details' style='color:#000' onclick= \"openmypage('".$internal_file_no."',$company_name,'$lien_bank_id','$file_year','btb_open_info','BTB Info');\">".number_format($btb_opened_val,2)."</a>"; $tot_opening+=$btb_opened_val; ?></td>
                                            <td width="60" align="right"><? echo number_format($bblc_per,2); ?></td>
                                            <td width="120" align="right"><? echo number_format($fund_available,2); $tot_fund_avail+=$fund_available; ?> </td>
                                            <td width="120" align="right"><? echo number_format($pc_limit,2); $tot_pc_limit+=$pc_limit; ?></td>
                                            <td width="120" align="right"><? echo "<a href='#report_details' style='color:#000' onclick= \"openmypage('".$internal_file_no."',$company_name,'$lien_bank_id','$file_year','pc_drawn_info','PC Drawn Info');\">".number_format($pc_drawn,2)."</a>"; $tot_pc_drawn+=$pc_drawn; ?></td>
                                            <td width="120" align="right"><? echo number_format($fund_available_packing,2); $tot_fund_avail_pack+=$fund_available_packing; ?></td>
                                            <td width="120" align="right"><? echo number_format($pc_adjusted,2); $tot_pc_adjust+=$pc_adjusted; ?></td>
                                            <td width="120" align="right"><? echo number_format($out_bdt,2); $tot_out_bdt+=$out_bdt; ?></td>
                                            <td width="100" align="right"><? echo number_format($pc_liability_per,2); ?></td>
                                            <td width="120" align="right"><? echo "<a href='#report_details' style='color:#000' onclick= \"openmypage('".$internal_file_no."',$company_name,'$lien_bank_id','$file_year','gross_bill_info','Export Invoice Info');\">".number_format($gross_bill_amnt,2)."</a>"; $grand_bill_amnt+=$gross_bill_amnt; ?></td>
                                            <td width="120" align="right"><? echo "<a href='#report_details' style='color:#000' onclick= \"openmypage('".$internal_file_no."',$company_name,'$lien_bank_id','$file_year','net_bill_info','Export Invoice Info');\">".number_format($net_bill_amnt,2)."</a>"; $grand_net_bill_amnt+=$net_bill_amnt; ?></td>
                                            <td width="120" align="right"><? echo "<a href='#report_details' style='color:#000' onclick= \"openmypage('".$internal_file_no."',$company_name,'$lien_bank_id','$file_year','export_proceed_info','Export Proceed Realization Info');\">".number_format($realized_val,2)."</a>"; $tot_realized+=$realized_val; ?></td>
                                            <td width="120" align="right"><? echo "<a href='#report_details' style='color:#000' onclick= \"openmypage('".$internal_file_no."',$company_name,'$lien_bank_id','$file_year','short_realized','Short Realization Info');\">".number_format($short_realized,2)."</a>"; $tot_short_realized+=$short_realized; ?></td>
                                            <td width="120" align="right"><? echo number_format($unrealized_val,2); $tot_unrealized+=$unrealized_val; ?></td>
                                            <td width="120" align="right"><? echo number_format($ship_balance,2); $tot_ship_bal+=$ship_balance; ?></td>
                                            <td width="120" align="right"><? echo number_format($margin_build,2); $tot_margin_build+=$margin_build; ?></td>
                                            <td width="120" align="right"><? echo "<a href='#report_details' style='color:#000' onclick= \"openmypage('".$internal_file_no."',$company_name,'$lien_bank_id','$file_year','dfc_paid_info','DFC Paid Info');\">".number_format($paid_val,2)."</a>"; $tot_paid+=$paid_val; ?></td>
                                            <td width="120" align="right"><? echo number_format($balance,2); $tot_balance+=$balance; ?></td>
                                            <td width="110" align="right"><? echo "<a href='#report_details' style='color:#000' onclick= \"openmypage('".$internal_file_no."',$company_name,'$lien_bank_id','$file_year','dfc_paid_info_adjust','DFC Paid Info');\">".number_format($btb_liability_adjust,2)."</a>"; $tot_btb_liability_adjust+=$btb_liability_adjust; ?></td>
                                            <td width="110" align="right" title="<? echo "(btb opened value-(paid value+btb liability adjust))"; ?>"><? echo number_format($btb_liability,2); $tot_btb_liability+=$btb_liability; ?></td>
                                            <td width="100" align="right"><? echo number_format($liability_reso_per,2); ?></td>
								...
								<tr bgcolor="#CCCCCC">
									...
									<td align="right"><? echo number_format($file_sc_amnt,2); ?></td>
									<td align="right"><? echo number_format($file_lc_amnt,2); ?></td>
									<td align="right"><? $bal=$file_sc_amnt-$file_lc_amnt; $bal=abs($bal); echo number_format($bal,2); $tot_bal+=$bal; ?></td>
									<td align="right">
										<?
											$balance2=$file_sc_amnt-$file_lc_amnt; 
											if($balance2>0)
											{
												$file_value=$balance2+$file_lc_amnt; 
											}
											else
											{
												$file_value=$file_lc_amnt; 
											}
							...
							<tfoot>
								...
								<th align="right"><? echo number_format($tot_sc_amnt,2); ?></th>
								<th align="right"><? echo number_format($tot_lc_amnt,2); ?></th>
								<th align="right"><? echo number_format($tot_bal,2); ?></th>
								<th align="right"><? echo number_format($tot_file_value,2); ?></th>
								<th>&nbsp;</th>
								<th>&nbsp;</th>
								<th>&nbsp;</th>
								<th>&nbsp;</th>
								<th>&nbsp;</th>
								<th align="right"><? echo number_format($tot_order_value,2); ?></th>
								<th align="right"><? echo number_format($tot_fore_comn,2); ?></th>
								<th width="60">&nbsp;</th>
								<th align="right"><? echo number_format($tot_local_comn,2); ?></th>
								<th>&nbsp;</th>
								<th align="right"><? echo number_format($tot_carring_cost,2); ?></th>
								<th align="right"><? echo number_format($tot_deducd,2); ?></th>
								<th>&nbsp;</th>
								<th align="right"><? echo number_format($tot_net_lc_val,2); ?></th>
								<th>&nbsp;</th>
								<th align="right"><? echo number_format($tot_bblc_entitle,2); ?></th>
								<th align="right"><? echo number_format($tot_opening,2); ?></th>
								<th>&nbsp;</th>
								<th align="right"><? echo number_format($tot_fund_avail,2); ?></th>
								<th align="right"><? echo number_format($tot_pc_limit,2); ?></th>
								<th align="right"><? echo number_format($tot_pc_drawn,2); ?></th>
								<th align="right"><? echo number_format($tot_fund_avail_pack,2); ?></th>
								<th align="right"><? echo number_format($tot_pc_adjust,2); ?></th>
								<th align="right"><? echo number_format($tot_out_bdt,2); ?></th>
								<th>&nbsp;</th>
								<th align="right"><? echo number_format($grand_bill_amnt,2); ?></th>
                                <th align="right"><? echo number_format($grand_net_bill_amnt,2); ?></th>
								<th align="right"><? echo number_format($tot_realized,2); ?></th>
								<th align="right"><? echo number_format($tot_short_realized,2); ?></th>
								<th align="right"><? echo number_format($tot_unrealized,2); ?></th>
								<th align="right"><? echo number_format($tot_ship_bal,2); ?></th>
								<th align="right"><? echo number_format($tot_margin_build,2); ?></th>
								<th align="right"><? echo number_format($tot_paid,2); ?></th>
								<th align="right"><? echo number_format($tot_balance,2); ?></th>
								<th align="right"><? echo number_format($tot_btb_liability_adjust,2); ?></th>
								<th align="right"><? echo number_format($tot_btb_liability,2); ?></th>
								<th>&nbsp;</th>
								<th>&nbsp;</th>
							</tfoot>
						</table>
					...
					<div align="left">
						...
						<table width="900" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all">
							...
							<tfoot>
								<tr class="tbl_bottom">
									<td align="right"><? echo number_format($tot_unrealized,2); ?></td>
									<td align="right"><? echo number_format($tot_ship_bal,2); ?></td>
									<td align="right"><? echo number_format($tot_balance,2); ?></td>
									<td align="right"><? $total_res=$tot_unrealized+$tot_ship_bal+$tot_balance; echo number_format($total_res,2); ?></td>
									<td align="right"><? echo number_format($tot_btb_liability,2); ?></td>
									<td align="right"><? echo number_format(($tot_btb_liability/number_format($total_res,2,'.',''))*100,2); ?></td>
									<td align="right"><? echo number_format($tot_out_bdt,2); ?></td>
								</tr>
							</tfoot>
						</table>
					</div>
		...
		?>
			<div style="width:2300px;">
				<fieldset style="width:100%;">	 
					<table cellpadding="0" cellspacing="0" width="100%">
						<tr>
						   <td colspan="18" align="center" width="100%" style="font-size:18px"><b>BTB Liability Coverage Report</b></td>
						</tr>
						<tr>
						   <td colspan="18" align="center" width="100%" style="font-size:18px"><b><? echo $company_library[$company_name]; ?></b></td>
						</tr>
					...
					<table width="100%" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all">
						<thead>
							<tr>
								...
								<th width="120" rowspan="2">PC Drowen</th>
                                <th width="100" rowspan="2">PC Adjusted</th>
                                <th width="100" rowspan="2">PC Liability</th>
								<th colspan="5" width="600">Shipment Status</th>
								<th colspan="2" width="220">BTB Liability</th>
								<th width="200" colspan="2"></th>
								<th rowspan="2" width="150">Surplus (TK)</th>
                                <th rowspan="2">Remarks</th>
							</tr>
							<tr>
								<th width="120">Opening</th>
					...
					<div style="width:2300px; max-height:400px; overflow-y:scroll" id="dfc_liability">
						<table width="100%" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all">
						<?
							$i=1;
							if($db_type==0)
							{
								$sql="select internal_file_no, lien_bank, currency_name, buyer_name, applicant_name from
								(
									select a.internal_file_no, a.lien_bank, a.currency_name, a.buyer_name, a.applicant_name from com_export_lc a where a.beneficiary_name='$company_name' and a.lien_bank like '$lien_bank_id' and a.lc_year like '$file_year' and a.status_active=1 and a.is_deleted=0 $file_no_cond_lc group by a.internal_file_no 
								union 
									select b.internal_file_no, b.lien_bank, b.currency_name, b.buyer_name, b.applicant_name from com_sales_contract b where b.beneficiary_name='$company_name' and b.lien_bank like '$lien_bank_id' and b.sc_year like '$file_year' and b.status_active=1 and b.is_deleted=0 $file_no_cond_sc group by b.internal_file_no
								)  	
								com_export_lc group by internal_file_no order by internal_file_no ASC";
							}
							else
							{
								$sql="select a.internal_file_no, min(a.lien_bank) as lien_bank, min(a.currency_name) as currency_name, min(a.buyer_name) as buyer_name, min(a.applicant_name) as applicant_name from com_export_lc a where a.beneficiary_name='$company_name' and a.lien_bank like '$lien_bank_id' and a.lc_year like '$file_year' and a.status_active=1 and a.is_deleted=0 $file_no_cond_lc group by a.internal_file_no 
								union 
									select b.internal_file_no,min( b.lien_bank) as lien_bank, min( b.currency_name) as currency_name, min(b.buyer_name) as buyer_name, min(b.applicant_name) as applicant_name from com_sales_contract b where b.beneficiary_name='$company_name' and b.lien_bank like '$lien_bank_id' and b.sc_year like '$file_year' and b.status_active=1 and b.is_deleted=0 $file_no_cond_sc group by b.internal_file_no
								order by internal_file_no ASC";
							}
							//echo $sql;die;
							$nameArray=sql_select( $sql );
							...
							?>		
								 <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
									<td width="40"><? echo $i; ?></td>
									<td width="120"><?	echo $row[csf('internal_file_no')]; ?></td>
									<?
									$file_sc_amnt=0; $file_lc_amnt=0; $file_balance=0; $file_value=0; $file_order_value=0; $file_fore_comn=0; $file_carring_cost=0; $file_deducd=0; $file_net_lc_val=0; $file_local_comn=0; $file_bblc_entitle=0; $file_tot_opening=0; $file_fund_avail=0; $file_pc_limit=0; $file_pc_drawn=0;$file_fund_avail_pack=0; $file_pc_adjust=0; $file_out_bdt=0; $file_bill_amnt=0; $file_realized=0; $file_short_realized=0; $file_unrealized=0; $file_ship_bal=0; $file_margin_build=0; $file_paid=0; $file_balance=0; $file_btb_liability=0;
						
									$s=1; $lc_id_array=array(); $sc_id_array=array(); $foreign_comn=0; $local_comm=0; $local_comm_per=0; $carring_cost=0; $foreign_comn_cost=0; $foreign_comm_per=0; $file_deduction=0; $file_deduction_per=0; $net_lc_value=0; $bbb_lc_entitle=0; $btb_opened_val=0; $fund_available=0; $bblc_per=0; $pc_limit=0; $pc_drawn=0; $fund_available_packing=0; $pc_adjusted=0; $out_bdt=0; $margin_build=0; $gross_bill_amnt=0; $realized_val=0; $unrealized_val=0; $short_realized=0; $ship_balance=0; $pc_liability_per=0; $paid_val=0; $balance=0; $btb_liability=0; $btb_liability_adjust=0; $liability_reso_per=0; $cost_heads_fabric_order=0; $cost_heads_embellish_order=0; $trims_cost=0; $yarn_cost=0; $att_total_value=0; $deduc_on_claim_adjustment=0;
									
									$sql_lcSc="select a.id, a.foreign_comn, b.attached_value, b.attached_qnty, b.attached_rate, c.id as po_id, c.job_no_mst, 1 as type from com_export_lc a, com_export_lc_order_info b, wo_po_break_down c where a.id=b.com_export_lc_id and b.wo_po_break_down_id=c.id and a.beneficiary_name='$company_name' and a.lien_bank like '$lien_bank_id' and a.lc_year like '$file_year' and a.internal_file_no='".$row[csf('internal_file_no')]."' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
										union all
												select d.id, d.foreign_comn, e.attached_value, e.attached_qnty, e.attached_rate, f.id as po_id, f.job_no_mst, 2 as type from com_sales_contract d, com_sales_contract_order_info e, wo_po_break_down f where d.id=e.com_sales_contract_id and e.wo_po_break_down_id=f.id and d.beneficiary_name='$company_name' and d.lien_bank like '$lien_bank_id' and d.sc_year like '$file_year' and d.internal_file_no='".$row[csf('internal_file_no')]."' and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0
									";
									$lcScOrArray=sql_select( $sql_lcSc );
									foreach($lcScOrArray as $row_lcSc)
									{
										$att_total_value+=$row_lcSc[csf('attached_value')];
										
										$fabriccostArray=sql_select("select costing_per_id, freight, comm_cost from wo_pre_cost_dtls where job_no='".$row_lcSc[csf('job_no_mst')]."' and status_active=1 and is_deleted=0");
										...
										$freight_cost=($row_lcSc[csf('attached_qnty')]/$dzn_qnty)*$fabriccostArray[0][csf('freight')];
										$commercial_cost=($row_lcSc[csf('attached_qnty')]/$dzn_qnty)*$fabriccostArray[0][csf('comm_cost')];
										
										$carring_cost+=$freight_cost+$commercial_cost;
										
										$trimscostArray=sql_select("select sum(b.cons*a.rate) as trims_cost_total from wo_pre_cost_trim_cost_dtls a, wo_pre_cost_trim_co_cons_dtls b where a.id=b.wo_pre_cost_trim_cost_dtls_id and b.po_break_down_id='".$row_lcSc[csf('po_id')]."' and a.job_no='".$row_lcSc[csf('job_no_mst')]."' and a.status_active=1 and a.is_deleted=0");
										$trims_cost+=($row_lcSc[csf('attached_qnty')]/$dzn_qnty)*$trimscostArray[0][csf('trims_cost_total')];
										
										$yarn_cons_cost=return_field_value("sum(amount) as amount","wo_pre_cost_fab_yarn_cost_dtls","job_no='".$row_lcSc[csf('job_no_mst')]."' and status_active=1 and is_deleted=0","amount");
										$yarn_cost+=($row_lcSc[csf('attached_qnty')]/$dzn_qnty)*$yarn_cons_cost;
										
										$l_cost=return_field_value("commission_amount","wo_pre_cost_commiss_cost_dtls","job_no='".$row_lcSc[csf('job_no_mst')]."' and particulars_id=2 and status_active=1 and is_deleted=0");
										$local_comm+=($row_lcSc[csf('attached_qnty')]/$dzn_qnty)*$l_cost;
										
										if($foreign_comn==0 && $row_lcSc[csf('foreign_comn')]>0)
										{ 
											$foreign_comn=$row_lcSc[csf('foreign_comn')];
										}
										
										$cost_heads_fabric=$cost_heads_fabric_array[$job_company_library[$row_lcSc[csf('job_no_mst')]]];
										if($cost_heads_fabric!="")
										{
											$fabric_cost=return_field_value("sum(amount) as amount", "wo_pre_cost_fab_conv_cost_dtls","job_no='".$row_lcSc[csf('job_no_mst')]."' and cons_process in ($cost_heads_fabric) and status_active=1 and is_deleted=0","amount");
											$cost_heads_fabric_order+=($row_lcSc[csf('attached_qnty')]/$dzn_qnty)*$fabric_cost;
										}
										
										$cost_heads_embellish=$cost_heads_embellish_array[$job_company_library[$row_lcSc[csf('job_no_mst')]]];
										if($cost_heads_embellish!="")
										{
											$embellish_cost=return_field_value("sum(amount) as amount", "wo_pre_cost_embe_cost_dtls","job_no='".$row_lcSc[csf('job_no_mst')]."' and emb_name in ($cost_heads_embellish) and status_active=1 and is_deleted=0","amount");
											$cost_heads_embellish_order+=($row_lcSc[csf('attached_qnty')]/$dzn_qnty)*$embellish_cost;
										}
									}
									...
									$foreign_comn_cost=($att_total_value*$foreign_comn)/100;
									$foreign_comm_per=($foreign_comn_cost/$att_total_value)*100;
									$bbb_lc_entitle=$cost_heads_fabric_order+$cost_heads_embellish_order+$yarn_cost+$trims_cost;
									//echo $cost_heads_fabric_order."F ".$cost_heads_embellish_order."E ".$yarn_cost."Y ".$trims_cost."T ";
									$dataArray=sql_select("select a.id as id, a.claim_adjustment as claim_adjustment, a.pay_term, 1 as type from com_export_lc a where a.beneficiary_name='$company_name' and a.lien_bank like '$lien_bank_id' and a.lc_year like '$file_year' and a.status_active=1 and a.is_deleted=0 and a.internal_file_no='".$row[csf('internal_file_no')]."'
														union 
															select b.id as id, b.claim_adjustment as claim_adjustment, b.pay_term, 2 as type  from com_sales_contract b where b.beneficiary_name='$company_name' and b.lien_bank like '$lien_bank_id' and b.sc_year like '$file_year' and b.status_active=1 and b.is_deleted=0 and b.internal_file_no='".$row[csf('internal_file_no')]."'");
									$deduc_on_claim_adjustment=0; $lc_id=''; $sc_id='';	$sc_id_of_cash_in_advance=''; 					
									foreach($dataArray as $selectResult)
									{
										if($selectResult[csf('type')]==1)
										{
											if($lc_id=="") $lc_id=$selectResult[csf('id')]; else $lc_id.=",".$selectResult[csf('id')];
										}
										else 
										{
											if($selectResult[csf('pay_term')]==3)
											{
												if($sc_id_of_cash_in_advance=="") $sc_id_of_cash_in_advance=$selectResult[csf('id')]; else $sc_id_of_cash_in_advance.=",".$selectResult[csf('id')];
											}
											
											if($sc_id=="") $sc_id=$selectResult[csf('id')]; else $sc_id.=",".$selectResult[csf('id')];
											
										}
										
										$deduc_on_claim_adjustment+=$selectResult[csf('claim_adjustment')];
									}
	
									if($lc_id!="")
									{
										$deduc_on_lc=return_field_value("sum(discount_ammount+bonus_ammount+claim_ammount) as deduct_amnt","com_export_invoice_ship_mst","is_lc=1 and status_active=1 and is_deleted=0 and lc_sc_id in($lc_id)","deduct_amnt");
									}
									else
									{
										$deduc_on_lc=0;
									...
									if($sc_id!="")
									{
										$deduc_on_sc=return_field_value("sum(discount_ammount+bonus_ammount+claim_ammount) as deduct_amnt","com_export_invoice_ship_mst","is_lc=2 and status_active=1 and is_deleted=0 and lc_sc_id in($sc_id)","deduct_amnt");
									}
									else
									{
										$deduc_on_sc=0;
									...
									$btb_lc_id_array=array(); $btb_lc_id_atSight_array=array(); $n=0; $opened_lc=0; $opened_sc=0;
									if($lc_id!="")
									{
										$sql_opened_lc="select c.id, c.lc_value, c.payterm_id from com_btb_export_lc_attachment b, com_btb_lc_master_details c where b.is_lc_sc=0 and b.import_mst_id=c.id and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.lc_sc_id in($lc_id) group by c.id, c.lc_value, c.payterm_id";
										$res_open_lc=sql_select($sql_opened_lc);
										foreach($res_open_lc as $row_open_lc)
										{
											$opened_lc+=$row_open_lc[csf('lc_value')];
											$btb_lc_id_array[$n]=$row_open_lc[csf('id')];
											
											if($row_open_lc[csf('payterm_id')]==1)
											{
												$btb_lc_id_atSight_array[$n]=$row_open_lc[csf('id')];
											}
									...
									if($sc_id!="")
									{
										$sql_opened_sc="select c.id, c.lc_value, c.payterm_id from com_btb_export_lc_attachment b, com_btb_lc_master_details c where b.is_lc_sc=1 and b.import_mst_id=c.id and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.lc_sc_id in($sc_id) group by c.id, c.lc_value, c.payterm_id";
										$res_open_sc=sql_select($sql_opened_sc);
										foreach($res_open_sc as $row_open_sc)
										{
											if(in_array($row_open_sc[csf('id')],$btb_lc_id_array))
											{
												$opened_sc=$opened_sc;	
											}
											else
											{
												$opened_sc+=$row_open_sc[csf('lc_value')];
												$btb_lc_id_array[$n]=$row_open_sc[csf('id')];
											}
											
											if($row_open_sc[csf('payterm_id')]==1)
											{
												if(!in_array($row_open_sc[csf('id')],$btb_lc_id_atSight_array))
												{
													$btb_lc_id_atSight_array[$n]=$row_open_sc[csf('id')];
												}
											}
									...
									if($conversion_details[$row[csf('currency_name')]]=="") $conversion_rate=1; else $conversion_rate=$conversion_details[$row[csf('currency_name')]];
						
									$pc_limit=(($net_lc_value*$percent_details[$row[csf('lien_bank')]][$company_name])/100)*$conversion_rate;
									
									if($lc_id!="")
									{
										$pc_drawn_lc=return_field_value("sum(amount) as amount","com_pre_export_lc_wise_dtls","export_type=1 and lc_sc_id in($lc_id)","amount");
										
										if($db_type==0)
										{
											$bill_id_lc=return_field_value("group_concat(distinct(doc_submission_mst_id)) as sub_id","com_export_doc_submission_invo","is_lc=1 and lc_sc_id in($lc_id) and status_active=1 and is_deleted=0","sub_id");
										}
										else
										{
											$bill_id_lc=return_field_value("LISTAGG(doc_submission_mst_id, ',') WITHIN GROUP (ORDER BY doc_submission_mst_id) as sub_id","com_export_doc_submission_invo","is_lc=1 and lc_sc_id in($lc_id) and status_active=1 and is_deleted=0","sub_id");
											$bill_id_lc=implode(",",array_unique(explode(",",$bill_id_lc)));
										}
										
										$doc_inv_lc=return_field_value("sum(c.current_invoice_value) as current_invoice_value","com_export_invoice_ship_mst b, com_export_invoice_ship_dtls c","b.is_lc=1 and b.id=c.mst_id and b.lc_sc_id in($lc_id) and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0","current_invoice_value");
									}
									else
									{
										$pc_drawn_lc=0; $bill_id_lc=''; $doc_inv_lc=0;
									}
									
									if($sc_id!="")
									{
										$pc_drawn_sc=return_field_value("sum(amount) as amount","com_pre_export_lc_wise_dtls","export_type=2 and lc_sc_id in($sc_id)","amount");
										
										if($db_type==0)
										{
											$bill_id_sc=return_field_value("group_concat(distinct(doc_submission_mst_id)) as sub_id","com_export_doc_submission_invo","is_lc=2 and lc_sc_id in($sc_id) and status_active=1 and is_deleted=0","sub_id");
										}
										else
										{
											$bill_id_sc=return_field_value("LISTAGG(doc_submission_mst_id, ',') WITHIN GROUP (ORDER BY doc_submission_mst_id) as sub_id","com_export_doc_submission_invo","is_lc=2 and lc_sc_id in($sc_id) and status_active=1 and is_deleted=0","sub_id");
											$bill_id_sc=implode(",",array_unique(explode(",",$bill_id_sc)));
										}
										
										if($sc_id_of_cash_in_advance!="")
										{
											if($db_type==0)
											{
												$invoice_id_sc_of_cash_in_advance=return_field_value("group_concat(id) as invoice_id","com_export_invoice_ship_mst","is_lc=2 and lc_sc_id in($sc_id_of_cash_in_advance) and status_active=1 and is_deleted=0","invoice_id");
											}
											else
											{
												$invoice_id_sc_of_cash_in_advance=return_field_value("LISTAGG(id, ',') WITHIN GROUP (ORDER BY id) as invoice_id","com_export_invoice_ship_mst","is_lc=2 and lc_sc_id in($sc_id_of_cash_in_advance) and status_active=1 and is_deleted=0","invoice_id");
											}
										}
										else
										{
											$invoice_id_sc_of_cash_in_advance='';
										}
										
										$doc_inv_sc=return_field_value("sum(c.current_invoice_value) as current_invoice_value","com_export_invoice_ship_mst b, com_export_invoice_ship_dtls c","b.is_lc=2 and b.id=c.mst_id and b.lc_sc_id in($sc_id) and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0","current_invoice_value");
										
									}
									else
									{
										$pc_drawn_sc=0; $bill_id_sc=''; $doc_inv_sc=0;
									}
									...
									print_r($bill_id_lc); echo "<br>".count($bill_id_lc);	
									print_r($bill_id_sc); echo "<br>";	 echo "<br>".count($bill_id_sc);	
									$result = array_merge( $bill_id_lc,  $bill_id_sc);	
										print_r($result); echo count($result);echo "<br>";
										echo implode(",",$result);
											die;*/
									
									if($bill_id_lc!="")
									{
										$sql_doc_sub_lc=sql_select("select
										sum(CASE WHEN acc_head=20 THEN dom_curr END) AS packing,
										sum(CASE WHEN acc_head=5 THEN lc_sc_curr END) AS margin_build
										from com_export_doc_sub_trans where doc_submission_mst_id in($bill_id_lc) and status_active=1 and is_deleted=0");
										
										$doc_sub_lc_packing=$sql_doc_sub_lc[0][csf('packing')]; 
										$doc_sub_lc_margin_build=$sql_doc_sub_lc[0][csf('margin_build')]; 
										
										$sql_proc_real_lc=sql_select("select
										sum(CASE WHEN e.account_head=20 THEN e.domestic_currency END) AS packing,
										sum(CASE WHEN e.account_head=5 THEN e.document_currency END) AS margin_build,
										sum(CASE WHEN e.type=0 THEN e.document_currency END) AS shortrealized,
										sum(CASE WHEN e.type=1 THEN e.document_currency END) AS realized
										from com_export_proceed_realization d, com_export_proceed_rlzn_dtls e where d.id=e.mst_id and d.invoice_bill_id in($bill_id_lc) and d.is_invoice_bill=1 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0");
										
										$proc_real_lc_packing=$sql_proc_real_lc[0][csf('packing')]; 
										$proc_real_lc_margin_build=$sql_proc_real_lc[0][csf('margin_build')];
										$proc_real_lc_realized=$sql_proc_real_lc[0][csf('realized')];
										$proc_real_lc_shortrealized=$sql_proc_real_lc[0][csf('shortrealized')]; 
									}
									else
									{
										$doc_sub_lc_packing=0; $doc_sub_lc_margin_build=0;
										$proc_real_lc_packing=0; $proc_real_lc_margin_build=0; $proc_real_lc_realized=0; $proc_real_lc_shortrealized=0;
									}
									
									if($bill_id_sc!="")
									{
										$sql_doc_sub_sc=sql_select("select
										sum(CASE WHEN acc_head=20 THEN dom_curr END) AS packing,
										sum(CASE WHEN acc_head=5 THEN lc_sc_curr END) AS margin_build
										from com_export_doc_sub_trans where doc_submission_mst_id in($bill_id_sc) and status_active=1 and is_deleted=0");
										
										$doc_sub_sc_packing=$sql_doc_sub_sc[0][csf('packing')]; 
										$doc_sub_sc_margin_build=$sql_doc_sub_sc[0][csf('margin_build')]; 
										
										$sql_proc_real_sc=sql_select("select
										sum(CASE WHEN e.account_head=20 THEN e.domestic_currency END) AS packing,
										sum(CASE WHEN e.account_head=5 THEN e.document_currency END) AS margin_build,
										sum(CASE WHEN e.type=0 THEN e.document_currency END) AS shortrealized,
										sum(CASE WHEN e.type=1 THEN e.document_currency END) AS realized
										from com_export_proceed_realization d, com_export_proceed_rlzn_dtls e where d.id=e.mst_id and d.invoice_bill_id in($bill_id_sc) and d.is_invoice_bill=1 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0");
										
										$proc_real_sc_packing=$sql_proc_real_sc[0][csf('packing')]; 
										$proc_real_sc_margin_build=$sql_proc_real_sc[0][csf('margin_build')]; 
										$proc_real_sc_realized=$sql_proc_real_sc[0][csf('realized')];
										$proc_real_sc_shortrealized=$sql_proc_real_sc[0][csf('shortrealized')];
										
									}
									else
									{
										$doc_sub_sc_packing=0; $doc_sub_sc_margin_build=0; 
										$proc_real_sc_packing=0; $proc_real_sc_margin_build=0; $proc_real_sc_realized=0; $proc_real_sc_shortrealized=0;
									}
									
									if($invoice_id_sc_of_cash_in_advance!="")
									{
										$sql_proc_real_sc_invoice=sql_select("select
										sum(CASE WHEN e.account_head=20 THEN e.domestic_currency END) AS packing,
										sum(CASE WHEN e.account_head=5 THEN e.document_currency END) AS margin_build,
										sum(CASE WHEN e.type=0 THEN e.document_currency END) AS shortrealized,
										sum(CASE WHEN e.type=1 THEN e.document_currency END) AS realized
										from com_export_proceed_realization d, com_export_proceed_rlzn_dtls e where d.id=e.mst_id and d.invoice_bill_id in($invoice_id_sc_of_cash_in_advance) and d.is_invoice_bill=2 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0");
										
										$proc_real_sc_packing_invoice=$sql_proc_real_sc_invoice[0][csf('packing')]; 
										$proc_real_sc_margin_build_invoice=$sql_proc_real_sc_invoice[0][csf('margin_build')]; 
										$proc_real_sc_realized_invoice=$sql_proc_real_sc_invoice[0][csf('realized')];
										$proc_real_sc_shortrealized_invoice=$sql_proc_real_sc_invoice[0][csf('shortrealized')];
									}
									else
									{
										$proc_real_sc_packing_invoice=0; $proc_real_sc_margin_build_invoice=0; 
										$proc_real_sc_realized_invoice=0; $proc_real_sc_shortrealized_invoice=0;
									}
									...
									$ship_balance=$att_total_value-$gross_bill_amnt;
									
									$pc_liability_per=(($out_bdt/$conversion_details[$row[csf('currency_name')]])/($unrealized_val+$ship_balance))*100;
									
									$btb_lc_id=implode(",",$btb_lc_id_array);
								
									if($btb_lc_id!="")
									{
										$paid_sql="select b.accepted_ammount, b.adj_source from com_import_invoice_mst a,  b where a.id=b.invoice_id and a.btb_lc_id in($btb_lc_id) and a.is_lc=1 and b.payment_head=40 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
										$paidArray=sql_select( $paid_sql );
										foreach($paidArray as $row_paid)
										{
											if($row_paid[csf('adj_source')]==5)
											{
												$paid_val+=$row_paid[csf('accepted_ammount')];
											}
											else
											{
												$btb_liability_adjust+=$row_paid[csf('accepted_ammount')];
											}
										}
									}
									
									$btb_lc_id_atSight=implode(",",$btb_lc_id_atSight_array);
									if($btb_lc_id_atSight!="")
									{
										$paid_sql_adj="select sum(case when a.retire_source=5 then b.current_acceptance_value end) as paid_val,
														sum(case when a.retire_source!=5 then b.current_acceptance_value end) as paid_adj_val
														from com_import_invoice_mst a, com_import_invoice_dtls b where a.id=b.import_invoice_id and a.btb_lc_id in($btb_lc_id_atSight) and a.is_lc=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
										$paidArrayAdj=sql_select( $paid_sql_adj );
										
										$paid_val+=$paidArrayAdj[0][csf('paid_val')];
										$btb_liability_adjust+=$paidArrayAdj[0][csf('paid_adj_val')];
									}
									
									$balance=$margin_build-$paid_val;
									
									//$btb_liability=$margin_build-$btb_opened_val;
									$btb_liability=$btb_opened_val-($paid_val+$btb_liability_adjust);
									
									$bl=number_format($unrealized_val+$ship_balance,2,'.','');
									
									//$liability_reso_per=($btb_liability/($bl))*100;
									$liability_reso_per=($btb_liability/($bl+$balance))*100;
									
												
									$sql_lc="select id, bank_file_no, export_lc_no, replacement_lc, lc_value, pay_term, tenor, lien_bank, last_shipment_date, buyer_name, currency_name, remarks, foreign_comn from com_export_lc where beneficiary_name='$company_name' and internal_file_no='".$row[csf('internal_file_no')]."' and lien_bank like '$lien_bank_id' and lc_year like '$file_year' and status_active=1 and is_deleted=0 order by export_lc_no";
									$lcArray=sql_select( $sql_lc );
									$file_rows=count($lcArray);
									foreach($lcArray as $row_lc)
									{
										if($s==1)
										{
										?>
											<td width="120"><p><? echo $buyer_details[$row_lc[csf('buyer_name')]];?></p></td>
											<td width="120"><p>LC-<? echo $row_lc[csf('export_lc_no')]; ?></p></td>
											<td width="120" align="right"><? echo number_format($net_lc_value,2); $tot_net_lc_val+=$net_lc_value; ?></td>
											<td width="120" align="right"><? echo "<a href='#report_details' style='color:#000' onclick= \"openmypage('".$row[csf('internal_file_no')]."',$company_name,'$lien_bank_id','$file_year','btb_open_info','BTB Info');\">".number_format($btb_opened_val,2)."</a>"; $tot_opening+=$btb_opened_val; ?></td>
											<td width="120" align="right"><p><? echo number_format($pc_drawn,2); $tot_pc_drawn+=$pc_drawn; ?></p></td>
                                            <td width="100" align="right"><p><? echo number_format($pc_adjusted,2); $tot_pc_drawn_adjusted+=$pc_adjusted; ?></p></td>
                                            <td width="100" align="right"><p><? echo number_format($out_bdt,2); $tot_pc_liability+=$out_bdt; ?></p></td>
											
											<td width="120" align="right"><? echo "<a href='#report_details' style='color:#000' onclick= \"openmypage('".$row[csf('internal_file_no')]."',$company_name,'$lien_bank_id','$file_year','gross_bill_info','Export Invoice Info');\">".number_format($gross_bill_amnt,2)."</a>"; $grand_bill_amnt+=$gross_bill_amnt; ?></td>
											<td width="120" align="right"><? echo "<a href='#report_details' style='color:#000' onclick= \"openmypage('".$row[csf('internal_file_no')]."',$company_name,'$lien_bank_id','$file_year','export_proceed_info','Export Proceed Realization Info');\">".number_format($realized_val,2)."</a>"; $tot_realized+=$realized_val; ?></td>
											<td width="120" align="right"><? echo "<a href='#report_details' style='color:#000' onclick= \"openmypage('".$row[csf('internal_file_no')]."',$company_name,'$lien_bank_id','$file_year','short_realized','Short Realization Info');\">".number_format($short_realized,2)."</a>"; $tot_short_realized+=$short_realized; ?></td>
											<td width="120" align="right"><? echo number_format($unrealized_val,2); $tot_unrealized+=$unrealized_val; ?></td>
											<td width="120" align="right"><? echo number_format($ship_balance,2); $tot_ship_bal+=$ship_balance; ?></td>
											<td width="110" align="right"><? echo "<a href='#report_details' style='color:#000' onclick= \"openmypage('".$row[csf('internal_file_no')]."',$company_name,'$lien_bank_id','$file_year','dfc_paid_info_adjust','DFC Paid Info');\">".number_format($btb_liability_adjust,2)."</a>"; $tot_btb_liability_adjust+=$btb_liability_adjust; ?></td>
											<td width="110" align="right" title="<? echo "(btb opened value-(paid value+btb liability adjust))"; ?>"><? echo number_format($btb_liability,2); $tot_btb_liability+=$btb_liability; ?></td>
                                            
											<td width="100" align="right"><? $btb_lib_tk=($btb_liability*$conversion_details[2]); echo number_format($btb_lib_tk,2); $total_btb_liabilit += ($btb_liability*$conversion_details[2]); ?></td>
                                            <td width="100" align="right"><? echo number_format(($realized_val*$conversion_details[2]),2); $total_realized_val +=($realized_val*$conversion_details[2]); ?></td>
                                            <td width="150" align="right" title="<? echo "((Unrealized Value+Ship balance)*Rate-(BTB Liability TK+PC Liability))"; ?>"><p><? $ar_val=(($unrealized_val+$ship_balance)*$rate-($btb_lib_tk+$out_bdt)); echo number_format($ar_val,2); $total_ar_val+=$ar_val; ?></p></td>
                                            
											<td ><p><? echo $row_lc[csf('remarks')]; ?></p></td>
										</tr>
										<?
										}
										else
								...
								$s=1;
								$sql_sc="select id, bank_file_no, contract_no, convertible_to_lc, contract_value, pay_term, tenor, lien_bank, last_shipment_date, buyer_name, currency_name, remarks, converted_from, foreign_comn from com_sales_contract where beneficiary_name='$company_name' and internal_file_no='".$row[csf('internal_file_no')]."' and lien_bank like '$lien_bank_id' and sc_year like '$file_year' and status_active=1 and is_deleted=0 order by contract_no";
								$scArray=sql_select( $sql_sc );
								foreach($scArray as $row_sc)
								{
									if($row_sc[csf('converted_from')]==0)
									{
										$sc_val=$row_sc[csf('contract_value')];
										$file_sc_amnt+=$row_sc[csf('contract_value')]; $tot_sc_amnt+=$row_sc[csf('contract_value')];
										$suffix="";
										$lc_val_replaced="";
									}
									else
									{
										$lc_val_replaced=$row_sc[csf('contract_value')];
										$file_lc_amnt+=$row_sc[csf('contract_value')]; $tot_lc_amnt+=$row_sc[csf('contract_value')];
										$suffix=" (F)";
										$sc_val="";
									}
									...
									{
									?>
										<td width="120"><p><? echo $buyer_details[$row_sc[csf('buyer_name')]];?></p></td>
										<td width="120"><p>SC-<? echo $row_sc[csf('contract_no')];?></p></td>
										...
										<td width="100" align="right"><? echo number_format(($btb_liability*$conversion_details[2]),2); $total_btb_liabilit += ($btb_liability*$conversion_details[2]); ?></td>
                                        <td width="100" align="right"><? echo number_format(($realized_val*$conversion_details[2]),2); $total_realized_val +=($realized_val*$conversion_details[2]);  ?></td>
                                        <td width="150" align="right" title="<? echo "((Unrealized Value+Ship balance)*Rate-(BTB Liability TK+PC Liability))"; ?>"><p><? $ar_val=(($unrealized_val+$ship_balance)*$rate-($btb_lib_tk+$out_bdt)); echo number_format($ar_val,2); $total_ar_val+=$ar_val; ?></p></td>
										<td><p><? echo $row_sc[csf('remarks')]; ?></p></td>
									</tr>
									<?
									}
									else
									{
									?>
										<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_3rd<? echo $i.'_'.$s; ?>','<? echo $bgcolor; ?>')" id="tr_3rd<? echo $i.'_'.$s;?>">
											<td width="40">&nbsp;</td>
											<td width="120">&nbsp;</td>
											<td width="120">&nbsp;</td>
											<td width="120"><p>SC-<? echo $row_sc[csf('contract_no')];?></p></td>
							...
							<tfoot>
								...
								<th align="right"><? echo number_format($tot_opening,2); ?></th>
								<th align="right"><? echo number_format($tot_pc_drawn,2); ?></th>
                                <th align="right"><? echo number_format($tot_pc_drawn_adjusted,2); ?></th>
                                <th align="right"><? echo number_format($tot_pc_liability,2); ?></th>
								<th align="right"><? echo number_format($grand_bill_amnt,2); ?></th>
								<th align="right"><? echo number_format($tot_realized,2); ?></th>
								<th align="right"><? echo number_format($tot_short_realized,2); ?></th>
								<th align="right"><? echo number_format($tot_unrealized,2); ?></th>
								...
								<th align="right"><? echo number_format($tot_btb_liability_adjust,2); ?></th>
								<th align="right"><? echo number_format($tot_btb_liability,2); ?></th>
								<th align="right"><? echo number_format($total_btb_liabilit,2); ?></th>
								<th align="right"><? echo number_format($total_realized_val,2); ?></th>
                                <th align="right"><? echo number_format($total_ar_val,2); ?></th>
								<th>&nbsp;</th>
							</tfoot>
						</table>
					</div>
			...
			<div style="width:945px; overflow-y:scroll; max-height:230px;" id="scroll_body" align="left" >
                ...
                <? 
                    ...
                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                        ...
                        <td width="100" align="right"><? echo number_format($row[csf('po_quantity')],2); ?></td>
                        ...
                        <td width="110" align="right"><? echo number_format($row[csf('po_quantity')]*$row[csf('unit_price')],2); ?>&nbsp;</td>
                        <td width="105" align="right">
                            <?
                                if($row[csf('type')]==1)
                                { 
                                    echo number_format($row[csf('attached_value')],2); 
                    ...
                    ?>
                     <tfoot class="tbl_bottom">
                        <tr>
                            ...
                            <td align="right"><? echo number_format($total_order_qnty,2); ?></td>
                            <td></th>
                            <td></th>
                            <td align="right"><? echo number_format($total_order_amnt,2); ?></td>
                            <td align="right"><? echo number_format($total_att_lc_value,2); ?></td>
                            <td align="right"><? echo number_format($total_att_sc_value,2); ?></td>
                            <td></td>
                        </tr>
                        <tr>
	...
	<fieldset style="width:100%; margin-left:10px">
        <div id="report_container" align="center" style="width:100%">
           ...
           <div style="width:810px; overflow-y:scroll; max-height:230px" id="scroll_body" align="left" >
                ...
                <? 
                    $i=1; $btb_lc_id=array();
                    $sql_opened_lc="select c.id, c.lc_number, c.lc_date, c.supplier_id, c.item_category_id, c.lc_value, c.last_shipment_date from com_export_lc a, com_btb_export_lc_attachment b, com_btb_lc_master_details c where  a.id=b.lc_sc_id and b.is_lc_sc=0 and b.import_mst_id=c.id and a.internal_file_no='$file_no' and a.beneficiary_name='$company_name' and a.lien_bank like '$bank_id' and a.lc_year like '$text_year' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by c.id, c.lc_number, c.lc_date, c.supplier_id, c.item_category_id, c.lc_value, c.last_shipment_date";
                    ...
                    $sql_opened_sc="select c.id, c.lc_number, c.lc_date, c.supplier_id, c.item_category_id, c.lc_value, c.last_shipment_date from com_sales_contract a, com_btb_export_lc_attachment b, com_btb_lc_master_details c where  a.id=b.lc_sc_id and b.is_lc_sc=1 and b.import_mst_id=c.id and a.internal_file_no='$file_no' and a.beneficiary_name='$company_name' and a.lien_bank like '$bank_id' and a.sc_year like '$text_year' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by c.id, c.lc_number, c.lc_date, c.supplier_id, c.item_category_id, c.lc_value, c.last_shipment_date";
        ...
        <div id="report_container" align="center" style="width:100%">
           ...
           <div style="width:800px; overflow-y:scroll; max-height:290px" id="scroll_body" align="left" >
                ...
                <? 
                    ...
                    $sql_lc="select b.invoice_no, b.invoice_date, b.buyer_id, b.invoice_value, b.net_invo_value, b.discount_ammount+b.bonus_ammount+b.claim_ammount as deduct_amnt from com_export_lc a, com_export_invoice_ship_mst b where a.id=b.lc_sc_id and b.is_lc=1 and a.beneficiary_name='$company_name' and a.internal_file_no='$file_no' and a.lien_bank like '$bank_id' and a.lc_year like '$text_year' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
                    $result=sql_select($sql_lc);
                    foreach($result as $row)  
                    {
                    ...
                    $sql_sc="select b.invoice_no, b.invoice_date, b.buyer_id, b.invoice_value, b.net_invo_value, b.discount_ammount+b.bonus_ammount+b.claim_ammount as deduct_amnt from com_sales_contract a, com_export_invoice_ship_mst b where a.id=b.lc_sc_id and b.is_lc=2 and a.beneficiary_name='$company_name' and a.internal_file_no='$file_no' and a.lien_bank like '$bank_id' and a.sc_year like '$text_year' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
                    $result_sc=sql_select($sql_sc);
                    foreach($result_sc as $row_sc)  
                    {
        ...
        <div id="report_container" align="center" style="width:100%">
            <div style="width:800px">
                <table border="1" cellpadding="0" cellspacing="0" class="rpt_table" rules="all" width="500" align="center">
                   ...
                   <thead>
                        <tr>
                            <th colspan="8">Deduction at Export Invoice</th>
                        </tr>
                        <tr>
                            <th width="40">SL</th>
            ...
            <div style="width:800px; overflow-y:scroll; max-height:175px" id="scroll_body" align="left" >
                <table class="rpt_table" border="1" rules="all" width="100%" cellpadding="0" cellspacing="0">
					...
					$sql="(select a.export_lc_no as sc_lc_no, b.is_lc, b.lc_sc_id, b.invoice_no, b.invoice_date, b.discount_ammount, b.bonus_ammount, b.claim_ammount from com_export_lc a, com_export_invoice_ship_mst b where a.id=b.lc_sc_id and b.is_lc=1 and a.beneficiary_name='$company_name' and a.internal_file_no='$file_no' and a.lien_bank like '$bank_id' and a.lc_year like '$text_year' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0)
					union all
					(
						select c.contract_no as sc_lc_no, d.is_lc, d.lc_sc_id, d.invoice_no, d.invoice_date, d.discount_ammount, d.bonus_ammount, d.claim_ammount from com_sales_contract c, com_export_invoice_ship_mst d where c.id=d.lc_sc_id and d.is_lc=2 and c.beneficiary_name='$company_name' and c.internal_file_no='$file_no' and c.lien_bank like '$bank_id' and c.sc_year like '$text_year' and d.status_active=1 and d.is_deleted=0 and c.status_active=1 and c.is_deleted=0
					)
					";
					$result=sql_select($sql);
					foreach($result as $row)
					{
						...
						$deduct_amnt=$row[csf('discount_ammount')]+$row[csf('bonus_ammount')]+$row[csf('claim_ammount')];
						$tot_deduct_amnt+=$deduct_amnt;
						?>
						<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
							<td width="40"><? echo $i; ?></td>
							<td width="120"><p><? echo $row[csf('sc_lc_no')]; ?></p></td>
        ...
        <div id="report_container" align="center" style="width:100%">
           ...
           <div style="width:700px; overflow-y:scroll; max-height:230px" id="scroll_body" align="left" >
                ...
                <? 
                    ...
                    $sql_paid_lc="select b.import_mst_id as id, c.id as import_inovice_id, c.bank_ref, d.payment_date, d.accepted_ammount as amnt from com_export_lc a, com_btb_export_lc_attachment b, com_import_invoice_mst c,  d where a.id=b.lc_sc_id and b.is_lc_sc=0 and b.import_mst_id=c.btb_lc_id and c.is_lc=1 and c.id=d.invoice_id and d.payment_head=40 and d.adj_source=5 and a.internal_file_no='$file_no' and a.beneficiary_name='$company_name' and a.lien_bank like '$bank_id' and a.lc_year like '$text_year' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 group by b.import_mst_id, d.id, d.payment_date, d.accepted_ammount, c.id, c.bank_ref";
                    ...
                    $sql_paid_sc="select b.import_mst_id as id, c.id as import_inovice_id, c.bank_ref, d.payment_date, d.accepted_ammount as amnt from com_sales_contract a, com_btb_export_lc_attachment b, com_import_invoice_mst c,  d where a.id=b.lc_sc_id and b.is_lc_sc=1 and b.import_mst_id=c.btb_lc_id and c.is_lc=1 and c.id=d.invoice_id and d.payment_head=40 and d.adj_source=5 and a.internal_file_no='$file_no' and a.beneficiary_name='$company_name' and a.lien_bank like '$bank_id' and a.sc_year like '$text_year' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 group by b.import_mst_id, d.id, d.payment_date, d.accepted_ammount, c.id, c.bank_ref";
