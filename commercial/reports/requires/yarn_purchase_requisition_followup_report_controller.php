...
{
    $print_report_format_arr = return_library_array("select format_id,format_id from lib_report_template where template_name =$data and module_id=19 and report_id=157 and is_deleted=0 and status_active=1","format_id","format_id");
...
$report_format_arr=array(1=>"show_fabric_booking_report_gr",2=>"show_fabric_booking_report",3=>"show_fabric_booking_report3",4=>"show_fabric_booking_report1",5=>"show_fabric_booking_report2",6=>"show_fabric_booking_report4",7=>"show_fabric_booking_report5",8=>"show_fabric_booking_report",9=>"show_fabric_booking_report3",10=>"show_fabric_booking_report4",28=>"show_fabric_booking_report_akh",45=>"show_fabric_booking_report_urmi",53=>"show_fabric_booking_report_jk",73=>"show_fabric_booking_report_mf",93=>"show_fabric_booking_report_libas");//,78=>"Print",84=>"Print2",85=>"Print3"
...
{ 
 	...
 	if($template==1)
	{
		...
		//echo $sql;
		$nameArray=sql_select($sql); $po_data_arr=array(); $job_data_arr=array(); $job_allData_arr=array(); $tot_rows=0; $poIds='';
		if(count($nameArray)>0)
		{
			foreach($nameArray as $row)
			{
				...
				if($type==2)
				{
					...
					//echo $row[csf("grouping")]."**".$row[csf("file_no")]."**";//.$row[csf("po_number")]."##".$row[csf("po_qnty")]."##".$row[csf("pub_shipment_date")]."##".$row[csf("shiping_status")]."##".$row[csf("insert_date")]."##".$row[csf("po_received_date")]."##".$row[csf("plan_cut")]."##".$row[csf("is_confirmed")]."##".$row[csf("po_id")];
				}
		...
		//die;
		$poIds=chop($poIds,','); $poIds_country_cond=""; $yarn_iss_po_cond=""; $yarn_allo_po_cond=""; $grey_purchase_po_cond=""; $grey_delivery_po_cond=""; $fin_delivery_po_cond=""; $trans_po_cond=""; $fin_purchase_po_cond=""; $po_color_po_cond=""; $batch_po_cond=""; $dye_po_cond=""; $wo_po_cond=""; $cons_po_cond=""; $tna_po_cond="";
		if($db_type==2 && $tot_rows>1000)
		{
			$poIds_country_cond=" and (";
			$yarn_iss_po_cond=" and (";
			$yarn_allo_po_cond=" and (";
			$grey_purchase_po_cond=" and (";
			$grey_delivery_po_cond=" and (";
			$fin_delivery_po_cond=" and (";
			$trans_po_cond=" and (";
			$fin_purchase_po_cond=" and (";
			$po_color_po_cond=" and (";
			$batch_po_cond=" and (";
			$dye_po_cond=" and (";
			$wo_po_cond=" and (";
			...
			foreach($poIdsArr as $ids)
			{
				...
				$grey_purchase_po_cond.=" c.po_breakdown_id in($ids) or ";
				$grey_delivery_po_cond.=" order_id in($ids) or ";
				$fin_delivery_po_cond.=" a.order_id in($ids) or ";
				$trans_po_cond.=" po_breakdown_id in($ids) or ";
				$fin_purchase_po_cond.=" c.po_breakdown_id in($ids) or ";
				$po_color_po_cond.=" po_breakdown_id in($ids) or ";
				...
				$tna_po_cond.=" po_number_id in($ids) or ";
				$receive_by_batch_cond.=" po_breakdown_id in($ids) or ";
			}
			...
			$grey_purchase_po_cond=chop($grey_purchase_po_cond,'or ');
			$grey_purchase_po_cond.=")";
			
			$grey_delivery_po_cond=chop($grey_delivery_po_cond,'or ');
			$grey_delivery_po_cond.=")";
			
			$fin_delivery_po_cond=chop($fin_delivery_po_cond,'or ');
			$fin_delivery_po_cond.=")";
			
			$trans_po_cond=chop($trans_po_cond,'or ');
			$trans_po_cond.=")";
			
			$fin_purchase_po_cond=chop($fin_purchase_po_cond,'or ');
			$fin_purchase_po_cond.=")";
			
			$po_color_po_cond=chop($po_color_po_cond,'or ');
			$po_color_po_cond.=")";
			...
			$receive_by_batch_cond=chop($receive_by_batch_cond,'or ');
			$receive_by_batch_cond.=")";
		}
		else
		{
			...
			$grey_purchase_po_cond=" and c.po_breakdown_id in ($poIds)";
			$grey_delivery_po_cond=" and order_id in ($poIds)";
			$fin_delivery_po_cond=" and a.order_id in ($poIds)";
			$trans_po_cond=" and po_breakdown_id in ($poIds)";
			$fin_purchase_po_cond=" and c.po_breakdown_id in ($poIds)";
			$po_color_po_cond=" and po_breakdown_id in ($poIds)";
			...
			$tna_po_cond=" and po_number_id in ( $poIds )";
			$receive_by_batch_cond=" and po_breakdown_id in ( $poIds )";
		}
		
		$contry_ship_qty_arr=array();
		$country_ship_qty_sql="select po_break_down_id, country_ship_date, order_quantity as ship_qty from wo_po_color_size_breakdown where status_active=1 and is_deleted=0 $date_search_cond_country $poIds_country_cond";
		...
		$lapdipDataEc=sql_select("select job_no_mst, po_break_down_id, color_name_id, lapdip_no from wo_po_lapdip_approval_info where is_deleted=0 and status_active=1 $poIds_country_cond");
		...
		$sql_grey_purchase="select a.id, c.po_breakdown_id, a.receive_basis, c.quantity from inv_receive_master a,pro_grey_prod_entry_dtls b, order_wise_pro_details c where a.id=b.mst_id and b.id=c.dtls_id and a.entry_form in (22,58) and c.entry_form in (22,58) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $grey_purchase_po_cond";//and a.receive_basis<>9 sum(c.quantity) as grey_purchase_qnty
		...
		$sql_fin_purchase="select a.id, c.po_breakdown_id, c.color_id, a.receive_basis, c.quantity
		 ...
		 from inv_receive_master a, pro_finish_fabric_rcv_dtls b, order_wise_pro_details c where a.id=b.mst_id and b.id=c.dtls_id  and a.entry_form in(37,58,68) and c.entry_form in(37,58,68) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $fin_purchase_po_cond";//and a.receive_basis<>9 sum(c.quantity) as finish_purchase
		...
		$dataArrayTrans = sql_select("select trans_id, po_breakdown_id, color_id, entry_form, trans_type, quantity from order_wise_pro_details where status_active=1 and is_deleted=0 and entry_form in (2,7,11,14,13,15,16,18,37,51,52,61,66,68,71,80,81,82,83,84,126,134,110,183) $trans_po_cond");			
		...
		foreach($dataArrayTrans as $row)
		{
			...
			if($row[csf('entry_form')]==7 || $row[csf('entry_form')]==14 || $row[csf('entry_form')]==15 || $row[csf('entry_form')]==18 || $row[csf('entry_form')]==37 || $row[csf('entry_form')]==52 || $row[csf('entry_form')]==66 || $row[csf('entry_form')]==71 || $row[csf('entry_form')]==126 || $row[csf('entry_form')]==134)
			{
				...
				if(($row[csf('entry_form')]==52 || $row[csf('entry_form')]==126) && $row[csf('trans_type')]==4) $finish_issue_rtn_qnty_arr[$row[csf('po_breakdown_id')]][$row[csf('color_id')]] +=$row[csf('quantity')];
		...
		$sql_batch="select a.color_id, b.po_id, sum(b.batch_qnty) as batch_qnty from pro_batch_create_mst a, pro_batch_create_dtls b where a.id=b.mst_id and a.entry_form!=36 and  a.batch_against not in(2) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $batch_po_cond group by a.color_id, b.po_id";
		...
		unset($resultBatch);
		$grey_receive_by_batch=sql_select("select po_breakdown_id, qnty from pro_roll_details where entry_form=62 and status_active=1 and is_deleted=0 $receive_by_batch_cond");
		...
		$grey_receive_by_batch_withoutroll=sql_select("select b.po_id, b.batch_qnty as qnty from pro_batch_create_mst a, pro_batch_create_dtls b where a.id=b.mst_id and a.page_without_roll=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
        ...
        <fieldset style="width:100%">	
			...
			if(trim($txt_search_string)=="" && $start_date!="")
				...
				if($type==1 || $type==3)
				{
					//$po_data_arr[$row[csf("po_id")]]=
					//$row[csf("company_name")]."##".$row[csf("buyer_name")]."##".$row[csf("job_no_prefix_num")]."##".$row[csf("job_no")]."##".$row[csf("style_ref_no")]."##".$row[csf("gmts_item_id")]."##".$row[csf("order_uom")]."##".$row[csf("ratio")]."##".$row[csf("grouping")]."##".$row[csf("file_no")]."##".$row[csf("po_number")]."##".$row[csf("po_qnty")]."##".$row[csf("pub_shipment_date")]."##".$row[csf("shiping_status")]."##".$row[csf("insert_date")]."##".$row[csf("po_received_date")]."##".$row[csf("plan_cut")]."##".$row[csf("is_confirmed")];
								...
								if(($cbo_discrepancy==1 && $grey_recv_qnty>$yarn_issued) || ($cbo_discrepancy==0))
								{
										...
										foreach($dataArray as $woRow)
										{
											...
											if(!in_array($id,$booking_array))
											{
												...
												if($booking_type==4)
												{
													...
													$sample_booking.="<a href='##' style='color:#000' onclick=\"generate_worder_report('3','".$booking_no."','".$company_id."','".$wo_po_id."','".$item_category."','".$fabric_source."','".$job_no."','".$is_approved."','".$action_name."','".$print_report_format."')\"><font style='font-weight:bold' color='$wo_color;'>".$booking_no."(".$system_date.")"."</font></a><br>";
												...
												else
												{
													...
													$main_booking.="<a href='##' style='color:#000' onclick=\"generate_worder_report('".$is_short."','".$booking_no."','".$company_id."','".$wo_po_id."','".$item_category."','".$fabric_source."','".$job_no."','".$is_approved."','".$action_name."','".$print_report_format."')\"><font style='font-weight:bold' color='$wo_color;'>".$booking_no."(".$system_date.")".$pre."</font></a><br>";
									...
									if($tot_color>0)
									{
										...
										?>
											<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $k; ?>">
                                                ...
                                                <td width="40" onClick="openmypage_image('requires/yarn_purchase_requisition_followup_report_controller.php?action=show_image&job_no=<?=$job_no; ?>','Image View')"><?=$display_font_color; ?><img  src="../../<?=$imge_arr[$job_no]; ?>" height='25' width='30' /><?=$font_end; ?></td>
												<td width="120">
													<p>
												...
												<? if($country_ship_qty>0)
											   {
												...
												<td width="100" align="right">
													...
													<? 
														...
														{
														?>
															<a  href="##" onClick="open_febric_receive_status_order_wise_popup('<? echo $po_id; ?>','grey_issue','')"><? echo number_format($grey_fabric_issue,2,'.',''); ?></a>
														<?
									...
									?>
										<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $k; ?>">
                                            ...
                                            <td width="40" onClick="openmypage_image('requires/yarn_purchase_requisition_followup_report_controller.php?action=show_image&job_no=<?=$job_no; ?>','Image View')"><img  src="../../<?=$imge_arr[$job_no]; ?>" height='25' width='30' /></td>
											<td width="120">
												<p>
											...
											<? $html.="</td><td>"; ?>
											<td width="100" align="right"><a  href="##" onClick="open_febric_receive_status_order_wise_popup('<? echo $po_id; ?>','grey_issue','')"><? echo number_format($grey_fabric_issue,2,'.',''); $html.=number_format($grey_fabric_issue,2); ?></a>
											</td>
											<? $html.="</td><td>"; ?>
									...
									?>
										<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $k; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $k; ?>">
										   ...
										   <td width="100" align="right">
											...
											<? $html.="</td><td>"; $html_medium.="</td><td>"; ?>
											<td width="100" align="right" bgcolor="<? echo $fin_prod_color_td; ?>">
												<a  href="##" onClick="open_febric_receive_status_order_wise_popup('<? echo $po_id; ?>','fabric_receive','<? echo $key; ?>')"><? echo number_format($fab_recv_qnty,2,'.',''); ?>kk</a>
												<?
													$html.=number_format($fab_recv_qnty,2);
													$html_short.=number_format($fab_recv_qnty,2);
													$html_medium.=number_format($fab_recv_qnty,2);
                                ...
                                <tfoot>
                                    <tr>
                                        <th width="40" colspan="2" align="right">Total</th>
                                         <?php if($type==1){?>
                                        <th width="100" align="right"><?php echo $order_qty_array_sum_without_country;?></td>
                                   		 <?php }?>
	...
	<fieldset style="width:865px; margin-left:3px">
		<div id="report_container">
			...
			<table border="1" class="rpt_table" rules="all" width="860" cellpadding="0" cellspacing="0">
				...
				group by a.id, c.id, a.issue_number, a.issue_date, a.challan_no, a.knit_dye_source, a.knit_dye_company, a.booking_no, c.lot, c.yarn_type, c.product_name_details, d.brand_id, a.issue_basis, d.requisition_no order by a.issue_date DESC";
                $result=sql_select($sql);
                foreach($result as $row)
				{
                ...
                group by a.id, c.id, a.recv_number, a.receive_date, a.challan_no, a.knitting_source, a.knitting_company, a.booking_no, c.lot, c.yarn_type, c.product_name_details, d.brand_id, a.receive_basis order by a.receive_date DESC";
                // echo $sql;
                $result=sql_select($sql);
				foreach($result as $row)
				{
...
if($action=="yarn_issue_not_not_used") //Old==Not used it.
{
	echo load_html_head_contents("Report Info", "../../../", 1, 1,'','','');
	extract($_REQUEST);
	$brand_array=return_library_array( "select id, brand_name from lib_brand", "id", "brand_name"  );
	...
	<fieldset style="width:970px; margin-left:3px">
		<div id="report_container">
			...
			<table border="1" class="rpt_table" rules="all" width="960" cellpadding="0" cellspacing="0">
                ...
                <?
				$i=1; $total_yarn_issue_qnty=0; $total_yarn_issue_qnty_out=0; $yarn_desc_array_for_return=array(); $yarn_issue_not_arr=array(); $dtls_data_arr=array();
				...
				foreach($dataArrayIssue as $row_yarn_iss)
				{
					...
					if(!in_array($desc,$yarn_desc_array))
					{
						$sql="select a.issue_number, a.issue_date, a.challan_no, a.knit_dye_source, a.knit_dye_company, a.booking_no, sum(b.quantity) as issue_qnty, c.lot, c.yarn_type, c.id as prod_id, c.product_name_details, d.brand_id from inv_issue_master a, order_wise_pro_details b, product_details_master c, inv_transaction d where a.id=d.mst_id and d.transaction_type=2 and d.item_category=1 and d.id=b.trans_id and b.trans_type=2 and b.entry_form=3 and b.po_breakdown_id in ($order_id) and b.prod_id=c.id and c.yarn_count_id='".$row_yarn_iss[csf('yarn_count_id')]."' and c.yarn_comp_type1st='".$row_yarn_iss[csf('yarn_comp_type1st')]."' and c.yarn_comp_percent1st='".$row_yarn_iss[csf('yarn_comp_percent1st')]."' and c.yarn_comp_type2nd='".$row_yarn_iss[csf('yarn_comp_type2nd')]."' and c.yarn_comp_percent2nd='".$row_yarn_iss[csf('yarn_comp_percent2nd')]."' and c.yarn_type='".$row_yarn_iss[csf('yarn_type')]."' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.issue_purpose!=2 group by a.id, c.id, a.issue_number, a.issue_date, a.challan_no, a.knit_dye_source, a.knit_dye_company, a.booking_no, c.lot, c.yarn_type, c.product_name_details, d.brand_id order by a.id DESC";
						$result=sql_select($sql);
						foreach($result as $row)
						{
							$yarn_issue_not_arr[$row[csf('issue_number')]][$row[csf('prod_id')]][$row[csf('brand_id')]]=$row[csf('issue_date')];
							$dtls_data_arr[$row[csf('issue_number')]][$row[csf('prod_id')]][$row[csf('brand_id')]]['mst_data']=$row[csf('challan_no')].'***'.$row[csf('knit_dye_source')].'***'.$row[csf('knit_dye_company')].'***'.$row[csf('booking_no')].'***'.$row[csf('issue_qnty')];
							$dtls_data_arr[$row[csf('issue_number')]][$row[csf('prod_id')]][$row[csf('brand_id')]]['prod']=$row[csf('lot')].'***'.$row[csf('yarn_type')].'***'.$row[csf('product_name_details')];
						}
					}
				}
				arsort($yarn_issue_not_arr);
				unset($result);
				unset($dataArrayIssue);
				//print_r($yarn_issue_not_arr);
				//die;
				foreach($yarn_issue_not_arr as $iss_no=>$iss_val)
				{
					foreach($iss_val as $prod_id=>$prod_val)
					{
						foreach($prod_val as $brand_id=>$issue_date)
						{
							if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";	
							$mst_data=explode("***",$dtls_data_arr[$iss_no][$prod_id][$brand_id]['mst_data']);
							$challan_no=$knit_dye_source=$knit_dye_company=$booking_no=''; $issue_qnty=0;
							$challan_no=$mst_data[0];
							$knit_dye_source=$mst_data[1];
							$knit_dye_company=$mst_data[2];
							$booking_no=$mst_data[3]; 
							$issue_qnty=$mst_data[4];
							
							$prod_data=explode("***",$dtls_data_arr[$iss_no][$prod_id][$brand_id]['prod']);
							$lot=$yarn_type=$product_name_details='';
							$lot=$prod_data[0];
							$yarn_type=$prod_data[1];
							$product_name_details=$prod_data[2];
						
							$issue_to="";
							if($knit_dye_source==1) $issue_to=$company_library[$knit_dye_company]; 
							else $issue_to=$supplier_details[$knit_dye_company];
							
							foreach($fab_source_id as $fsid)
							{
								if($fsid==1) $yarn_issued=$issue_qnty;
							}
							
							?>
							<tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
								<td width="105"><p><? echo $iss_no; ?></p></td>
								<td width="90"><p><? echo $issue_to; ?></p></td>
								<td width="105"><p><? echo $booking_no; ?>&nbsp;</p></td>
								<td width="70"><p><? echo $challan_no; ?>&nbsp;</p></td>
								<td width="75" align="center"><? echo change_date_format($issue_date); ?></td>
								<td width="70"><p><? echo $brand_array[$brand_id]; ?>&nbsp;</p></td>
                ...
                <?
				$total_yarn_return_qnty=0; $total_yarn_return_qnty_out=0; $yarn_issue_ret_arr=array();
				foreach($yarn_desc_array_for_return as $key=>$value)
				{
					if(!in_array($key,$yarn_desc_array))
					{
						...
						foreach($result as $row)
						{
							...
							if($row[csf('knitting_source')]!=3)
							{
								$return_qnty_in= number_format($yarn_returned,2,'.','');
								$total_yarn_return_qnty+=$return_qnty_in;
							}else{
								$return_qnty_in = "";
							}
							if($row[csf('knitting_source')]==3)
							{ 
								$return_qnty_out =  number_format($yarn_returned,2,'.',''); 
								$total_yarn_return_qnty_out+=$return_qnty_out;
							}else{
								$return_qnty_out = "";
							}
							$yarn_issue_ret_arr[$row[csf('recv_number')]][$row[csf('prod_id')]][$brand_array[$row[csf('brand')]]]=change_date_format($row[csf('receive_date')]);
							$yarn_return_result_array[$row[csf('recv_number')]][$row[csf('prod_id')]][$brand_array[$row[csf('brand')]]]=$return_from."#".$row[csf('booking_no')]."#".$row[csf('challan_no')]."#".$row[csf('lot')]."#".$row[csf('product_name_details')]."#".$return_qnty_in."#".$return_qnty_out;
							ksort($yarn_return_result_array);
							$return_qnty = $total_yarn_return_qnty+$total_yarn_return_qnty_out;
						}
					}
				}
                arsort($yarn_issue_ret_arr);  
				unset($result);             
				foreach($yarn_issue_ret_arr as $recv_number=> $rec_data)
				{
					foreach($rec_data as $prodid=>$prod_data)
					{
						foreach($prod_data as $brand=>$receive_date)
						if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
											
						$val = explode("#",$yarn_return_result_array[$rec_no][$prodid][$brand]);
						$return_from = $val[0];
						$booking_no = $val[1];
						$challan_no = $val[2];
						$lot = $val[3];
						$product_name_details = $val[4];
						$ret_in = $val[5];
						$ret_out = $val[6];
						//$return_qnty = $ret_in + $ret_out;
						?>
						<tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
							<td width="105" title="<? echo $row[csf('id')];?>"><p><? echo $recv_number; ?></p></td>
                            <td width="90"><p><? echo $return_from; ?></p></td>
                            <td width="105"><p><? echo $booking_no; ?>&nbsp;</p></td>
                            <td width="70"><p><? echo $challan_no; ?>&nbsp;</p></td>
                            <td width="75" align="center"><? echo $receive_date; ?></td>
                            <td width="70"><p><? echo $brand; ?>&nbsp;</p></td>
                            <td width="60"><p><? echo $lot; ?></p></td>
                            <td width="180"><p><? echo $product_name_details; ?></p></td>
                            <td align="right" width="90"><? echo $ret_in; ?></td>
                            <td align="right"><? echo $ret_out; ?></td>
                        </tr>
						<?   
						$i++;
					}
                ...
                <tr style="font-weight:bold">
                    ...
                    <td align="right" colspan="2"><? echo number_format($return_qnty,2); ?></td>
                </tr>
                <tfoot>    
                    <tr>
                        <th align="right" colspan="9">Total Balance</th>
                    ...
                    if (!in_array($key, $yarn_desc_array)) 
                    {
                        ...
                        foreach ($result_data as $row) 
                        {
                            ...
                            if ($row[csf('knitting_source')] == 1) {
                                $reject_return_from = $company_library[$row[csf('knitting_company')]];
                            } else if ($row[csf('knitting_source')] == 3) {
                                $reject_return_from = $supplier_details[$row[csf('knitting_company')]];
                            } else
                                $reject_return_from = "&nbsp;";
			...
			<table border="1" class="rpt_table" rules="all" width="650" cellpadding="0" cellspacing="0">
             ...
             <div style="width:667px; max-height:320px; overflow-y:scroll" id="scroll_body">
                 <table border="1" class="rpt_table" rules="all" width="650" cellpadding="0" cellspacing="0">
                   ...
                   $sql="select a.batch_no, a.extention_no as ext_no, a.batch_date, a.color_id, sum(b.batch_qnty) as quantity from pro_batch_create_mst a, pro_batch_create_dtls b where a.id=b.mst_id and b.po_id in($ex_data[0]) and a.color_id='$ex_data[1]' and a.status_active=1 and a.entry_form!=36  and a.batch_against not in(2) and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id, a.batch_no, a.extention_no, a.batch_date, a.color_id";
                    $result=sql_select($sql);
        			foreach($result as $row)
                    {
                        if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";	
	...
	<fieldset style="width:970px; margin-left:3px">
		<div id="report_container">
			<table border="1" class="rpt_table" rules="all" width="950" cellpadding="0" cellspacing="0">
                <thead>
                	<tr>
                        <th colspan="10"><b>Grey Receive For Batch</b></th>
                    </tr>
                    <tr>
                        <th width="30">SL</th>
                 ...
                 <table border="1" class="rpt_table" rules="all" width="950" cellpadding="0" cellspacing="0">
                    ...
                    ?>
                        <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
                            ...
                            <td align="right"><? echo number_format($row[csf('qnty')],2); $total_rec_qnty+=$row[csf('qnty')]; ?></td>
                        </tr>
                    <?
                    $i++;
                    }
             ...
             <div style="width:920px; max-height:320px; overflow-y:scroll" id="scroll_body">
                 <table border="1" class="rpt_table" rules="all" width="900" cellpadding="0" cellspacing="0">
					...
					if($order_id[1]==9)
					{
					   ...
					   union all
					   (
						select a.recv_number, a.receive_date, a.receive_basis, a.knitting_source, a.knitting_company, b.batch_id, b.prod_id, sum(c.quantity) as quantity from inv_receive_master a, pro_finish_fabric_rcv_dtls b, order_wise_pro_details c where a.id=b.mst_id and b.id=c.dtls_id and a.entry_form in(7) and c.entry_form in(7) and c.trans_id!=0 and c.po_breakdown_id in($order_id[0]) and c.color_id='$color' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by b.id, a.recv_number, a.receive_date, a.receive_basis, a.knitting_source, a.knitting_company, b.batch_id, b.prod_id   
					   )";
					}
					else
					{
				  ...
				  // echo $sql;
        			...
        			foreach($result as $row)
                    {
                        ...
                        else if(csf($row['knitting_source'])==3) $dye_company=$supplier_details[$row[csf('knitting_company')]];
                        else $dye_company="&nbsp;";
				...
				$sql="select a.issue_number, a.issue_date, a.received_mrr_no, b.store_id, b.pi_wo_batch_no, b.prod_id, c.po_breakdown_id, sum(c.quantity) as quantity, d.product_name_details from inv_issue_master a, inv_transaction b, order_wise_pro_details c, product_details_master d, inv_receive_master e where a.id=b.mst_id and b.id=c.trans_id and b.prod_id=d.id and e.id=a.received_id and a.entry_form in (46) and c.entry_form in (46) and c.po_breakdown_id in($order_id[0]) and c.color_id='$color' $ret_rec_basis_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by a.issue_number, a.issue_date, a.received_mrr_no, b.store_id, b.pi_wo_batch_no, b.prod_id, c.po_breakdown_id, d.product_name_details";
                $result=sql_select($sql);
				foreach($result as $row)
				{
					if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";	
		...
		<div id="report_container">
			<table border="1" class="rpt_table" rules="all" width="730" cellpadding="0" cellspacing="0">
             ...
             <div style="width:738px; max-height:320px; overflow-y:scroll" id="scroll_body">
                 <table border="1" class="rpt_table" rules="all" width="720" cellpadding="0" cellspacing="0">
                    ...
                    $sql="SELECT a.id as issue_id,a.issue_number, a.issue_date, b.batch_id, b.prod_id, sum(c.quantity) as quantity, a.challan_no, a.entry_form 
                    ...
                    ?>
                        <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
                            ...
                            <td width="80" align="center"><? echo change_date_format($row[csf('issue_date')]); ?></td>
                            <td width="120"><p><? echo $batch_id; ?></p></td>
            ...
            <table border="1" class="rpt_table" rules="all" width="730" cellpadding="0" cellspacing="0">
             ...
             <div style="width:738px; max-height:320px; overflow-y:scroll" id="scroll_body">
                 <table border="1" class="rpt_table" rules="all" width="720" cellpadding="0" cellspacing="0">
                    <?
					/*select po_breakdown_id, color_id, 
	...
	<fieldset style="width:670px; margin-left:7px">
		<div id="report_container">
			<table border="1" class="rpt_table" rules="all" width="650" cellpadding="0" cellspacing="0">
				...
				where a.id=b.mst_id and b.id=c.dtls_id and c.prod_id=d.id and a.item_category=13 and c.trans_type=5 and c.entry_form in (13,81,82,83,183) and c.po_breakdown_id in ($order_id) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 
				group by a.id, c.prod_id, a.transfer_system_id, a.transfer_date, a.challan_no, b.from_order_id, b.from_prod_id, d.product_name_details,a.from_samp_dtls_id ";// and a.transfer_criteria=4
				 //echo $sql;
                $result=sql_select($sql);
				...
				foreach($result as $row)
				{
                    ...
                    <tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
                        ...
                        {
                        	echo $po_arr[$row[csf('from_order_id')]]; 
                        }
                        ?>                        	
                        </p></td>
                        <td width="170"><p><? echo $row[csf('product_name_details')]; ?></p></td>
                ...
                <?
                $total_trans_out_qnty=0;
				/*$sql="SELECT a.transfer_system_id, a.transfer_date, a.challan_no, a.to_order_id, b.from_prod_id, d.product_name_details, sum(c.quantity) as transfer_qnty 
				from inv_item_transfer_mst a, inv_item_transfer_dtls b, order_wise_pro_details c, product_details_master d 
				where a.id=b.mst_id and b.id=c.dtls_id and c.prod_id=d.id and a.item_category=13 and c.trans_type=6 and c.entry_form in (13,80,82,83) and c.po_breakdown_id in ($order_id) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 
				group by a.id, c.prod_id, a.transfer_system_id, a.transfer_date, a.challan_no, a.to_order_id, b.from_prod_id, d.product_name_details";// and a.transfer_criteria=4*/
				...
				where a.id=b.mst_id and b.id=c.dtls_id and a.item_category=13 and c.trans_type=6 and c.entry_form in (13,80,82,83,110) and c.po_breakdown_id in ($order_id) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 
				group by a.id, c.prod_id, a.transfer_system_id, a.transfer_date, a.challan_no, b.to_order_id, b.from_prod_id, b.to_prod_id,a.to_samp_dtls_id";// and a.transfer_criteria=4
				 //echo $sql;
                $result=sql_select($sql);
				...
				foreach($result as $row)
				{
                    ...
                    <tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
                        ...
                        {
                        	echo $po_arr[$row[csf('to_order_id')]]; 
                        }
                        ?>
