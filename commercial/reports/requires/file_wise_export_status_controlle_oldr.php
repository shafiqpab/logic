...
$pi_no_arr=return_library_array( "select id,po_number from  wo_po_break_down where status_active=1",'id','po_number');


if ($action=="load_drop_down_buyer")
{
	echo create_drop_down( "cbo_buyer_name", 170, "select a.id,a.buyer_name from lib_buyer a, lib_buyer_tag_company b where a.status_active =1 and a.is_deleted=0 and b.buyer_id=a.id and b.tag_company='$data' and a.id in (select buyer_id from lib_buyer_party_type where party_type in (1,3,21,90))  $buyer_cond order by buyer_name","id,buyer_name", 1, "-- All Buyer --", $selected, "" ,0); 
	exit();
}
...
<fieldset style="width:100%">
    ...
    <table width="1500" align="left">
        <tr>
            <td width="500">
                <table width="450"> 
                    ...
                    <tr>
                        ...
                        ?>
                        &nbsp;Buyer: <?php echo $buyer_name_arr[$sal_ref[csf('buyer_name')]];?>
                        </td>
                        <td align="right">
                        <? 
            ...
            <?
		$i = 1;$realize_lc_arr=array();$submit_id_lc=0;$sub_lc_sc_id_arr=array();//$relize_arr=array();$distributed_arr=array();$submit_inv_arr=array();
		//var_dump($lc_id_arr);var_dump($sc_id_arr);die;
		$sub_inv_id_lc=0;$sub_inv_id_sc=0;$payment_realized=0;$payment_realized_deduction=0;
		//var_dump($lc_id_arr);
		if(!empty($lc_id_arr))
		{
			//echo "select $group_concat(distinct doc_submission_mst_id) as id from com_export_doc_submission_invo where is_lc=1 and lc_sc_id in(".implode(',',$lc_id_arr).")";die;
			...
			{
				/*echo "select a.id as relz_id, $group_concat(distinct a.invoice_bill_id) as sub_id , a.received_date,sum(case when b.type=1 then b.document_currency else 0 end) as distribute_realize, sum(case when b.type=0 then b.document_currency else 0 end) as deduct_realize 
				from 
						com_export_proceed_realization a, com_export_proceed_rlzn_dtls b
				where   
						a.id=b.mst_id  AND a.invoice_bill_id in($submision_id_lc_rlz) and a.is_invoice_bill=1 and a.status_active=1 and a.is_deleted=0 group by  a.id , a.received_date";*/
				if($db_type==0)
				{
					$sql=sql_select("select a.id as relz_id, $group_concat(distinct a.invoice_bill_id) as sub_id , a.received_date,sum(case when b.type=1 then b.document_currency else 0 end) as distribute_realize, sum(case when b.type=0 then b.document_currency else 0 end) as deduct_realize 
				from 
						com_export_proceed_realization a, com_export_proceed_rlzn_dtls b
				where   
						a.id=b.mst_id  AND a.invoice_bill_id in($submision_id_lc_rlz) and a.is_invoice_bill=1 and a.status_active=1 and a.is_deleted=0 group by  a.id , a.received_date");
				...
				{
					$sql=sql_select("select a.id as relz_id, a.invoice_bill_id  as sub_id , a.received_date,sum(case when b.type=1 then b.document_currency else 0 end) as distribute_realize, sum(case when b.type=0 then b.document_currency else 0 end) as deduct_realize 
				from 
						com_export_proceed_realization a, com_export_proceed_rlzn_dtls b
				where   
						a.id=b.mst_id  AND a.invoice_bill_id in($submision_id_lc_rlz) and a.is_invoice_bill=1 and a.status_active=1 and a.is_deleted=0 
				group by  a.id ,a.invoice_bill_id, a.received_date");
				}
				
				foreach($sql as $row)
				{
					if($submit_id_lc==0) $submit_id_lc=$row[csf("sub_id")]; else $submit_id_lc= $submit_id_lc.",".$row[csf("sub_id")];
					$sub_lc_sc_id_arr[]=$row[csf("sub_id")];
					...
					$realize_lc_arr[$row[csf('relz_id')]]['distribute_realize']=$row[csf('distribute_realize')];
					$realize_lc_arr[$row[csf('relz_id')]]['deduct_realize']=$row[csf('deduct_realize')];
		...
		WHERE 
				b.doc_submission_mst_id=a.id and b.invoice_id=c.id and d.invoice_bill_id=a.id and d.is_invoice_bill='1'AND a.id in($submit_id_lc) and  a.status_active=1 and a.is_deleted=0  group by d.id , c.is_lc");
		}
		else
		{
		...
		FROM 
				com_export_doc_submission_mst a, com_export_doc_submission_invo b,  com_export_invoice_ship_mst c,com_export_proceed_realization d
		WHERE 
				b.doc_submission_mst_id=a.id and b.invoice_id=c.id and d.invoice_bill_id=a.id and d.is_invoice_bill='1'AND a.id in($submit_id_lc) and  a.status_active=1 and a.is_deleted=0  group by d.id , b.doc_submission_mst_id,a.bank_ref_no ,a.submit_date,a.negotiation_date,b.lc_sc_id,c.is_lc");
		}
		...
		sum(CASE WHEN account_head =11 THEN document_currency END) AS std
		
		FROM  
				com_export_proceed_rlzn_dtls a, com_export_proceed_realization b 
		WHERE 
				a.mst_id = b.id AND a.type = '1' AND b.is_invoice_bill = 1 AND b.invoice_bill_id  in($submit_id_lc) and  b.status_active=1 and b.is_deleted=0 group by b.id ");
		
		
		foreach($document_sub as $row)
		{
			$realize_lc_arr[$row[csf("rlzid")]]["erq"]=$row[csf("erq")];
			$realize_lc_arr[$row[csf("rlzid")]]["btb"]=$row[csf("btb")];
			$realize_lc_arr[$row[csf("rlzid")]]["cd"]=$row[csf("cd")];
			$realize_lc_arr[$row[csf("rlzid")]]["cc"]=$row[csf("cc")];
			$realize_lc_arr[$row[csf("rlzid")]]["fdbc"]=$row[csf("fdbc")];
			$realize_lc_arr[$row[csf("rlzid")]]["sun_ac"]=$row[csf("sun_ac")];
			$realize_lc_arr[$row[csf("rlzid")]]["std"]=$row[csf("std")];
		...
		if(!empty($sc_id_arr))
		{
			...
			if($submision_id_sc_rlz)
			{
				...
				from 
						com_export_proceed_realization a, com_export_proceed_rlzn_dtls b
				where 
						a.id=b.mst_id  AND a.invoice_bill_id in($submision_id_sc_rlz) and a.is_invoice_bill=1 and a.status_active=1 and a.is_deleted=0 group by  a.id , a.received_date ";*/
						
				if($db_type==0)
				{
				$sql_rlz_sc=sql_select("select a.id as relz_id, $group_concat(distinct a.invoice_bill_id) as sub_id, a.received_date,sum(case when b.type=1 then b.document_currency else 0 end) as distribute_realize, sum(case when b.type=0 then b.document_currency else 0 end) as deduct_realize 
				from 
						com_export_proceed_realization a, com_export_proceed_rlzn_dtls b
				where 
						a.id=b.mst_id  AND a.invoice_bill_id in($submision_id_sc_rlz) and a.is_invoice_bill=1 and a.status_active=1 and a.is_deleted=0 group by  a.id , a.received_date ");
				...
				{
				$sql_rlz_sc=sql_select("select a.id as relz_id, a.invoice_bill_id as sub_id, a.received_date,sum(case when b.type=1 then b.document_currency else 0 end) as distribute_realize, sum(case when b.type=0 then b.document_currency else 0 end) as deduct_realize 
				from 
						com_export_proceed_realization a, com_export_proceed_rlzn_dtls b
				where 
						a.id=b.mst_id  AND a.invoice_bill_id in($submision_id_sc_rlz) and a.is_invoice_bill=1 and a.status_active=1 and a.is_deleted=0 group by  a.id, a.invoice_bill_id , a.received_date ");
				}
			
				foreach($sql_rlz_sc as $row)
				{
					...
					$realize_sc_arr[$row[csf('relz_id')]]['distribute_realize']=$row[csf('distribute_realize')];
					$realize_sc_arr[$row[csf('relz_id')]]['deduct_realize']=$row[csf('deduct_realize')];
		...
		//var_dump($realize_sc_arr);die;
			...
			FROM 
					com_export_doc_submission_mst a, com_export_doc_submission_invo b,  com_export_invoice_ship_mst c,com_export_proceed_realization d
			WHERE 
					b.doc_submission_mst_id=a.id and b.invoice_id=c.id and d.invoice_bill_id=a.id and d.is_invoice_bill='1'AND a.id in($submit_id_sc) and a.status_active=1 and a.is_deleted=0 group by d.id, c.is_lc");
			}
			else if($db_type==2)
			{
			...
			FROM 
					com_export_doc_submission_mst a, com_export_doc_submission_invo b,  com_export_invoice_ship_mst c,com_export_proceed_realization d
			WHERE 
					b.doc_submission_mst_id=a.id and b.invoice_id=c.id and d.invoice_bill_id=a.id and d.is_invoice_bill='1'AND a.id in($submit_id_sc) and a.status_active=1 and a.is_deleted=0 
		...
		//var_dump($relize_arr);
		$document_sub_sc=  sql_select("SELECT 
		b.id as rlzid, 
		sum(CASE WHEN account_head =6 THEN document_currency END) AS erq,
		sum(CASE WHEN account_head =5 THEN document_currency END) AS btb,
		sum(CASE WHEN account_head =10 THEN document_currency END) AS cd,
		...
		FROM  
				com_export_proceed_rlzn_dtls a, com_export_proceed_realization b 
		WHERE 
				a.mst_id = b.id AND a.type = '1' AND b.is_invoice_bill = 1 AND b.invoice_bill_id  in($submit_id_sc)  AND b.status_active='1' and b.is_deleted=0 group by b.id ");
		foreach($document_sub_sc as $row)
		{
			$realize_sc_arr[$row[csf('rlzid')]]['rlzid']=$row[csf('rlzid')];
			$realize_sc_arr[$row[csf('rlzid')]]['erq']=$row[csf('erq')];
			$realize_sc_arr[$row[csf('rlzid')]]['btb']=$row[csf('btb')];
			$realize_sc_arr[$row[csf('rlzid')]]['cd']=$row[csf('cd')];
			$realize_sc_arr[$row[csf('rlzid')]]['cc']=$row[csf('cc')];
			$realize_sc_arr[$row[csf('rlzid')]]['fdbc']=$row[csf('fdbc')];
			$realize_sc_arr[$row[csf('rlzid')]]['sun_ac']=$row[csf('sun_ac')];
			$realize_sc_arr[$row[csf('rlzid')]]['std']=$row[csf('std')];
		...
		if(!empty($lc_id_arr))
		{
			...
			WHERE 
			b.doc_submission_mst_id=a.id and b.invoice_id=c.id and b.is_lc='1' and a.id not in($submit_id_lc) AND b.lc_sc_id in(".implode(',',$lc_id_arr).")  AND a.status_active='1' and a.is_deleted=0 group by a.id, a.bank_ref_no, a.bank_ref_date, a.negotiation_date, c.is_lc");
			}
			else if($db_type==2)
			{
		...
		if(!empty($sc_id_arr))
		{
			...
			WHERE b.doc_submission_mst_id=a.id and b.invoice_id=c.id  and b.is_lc='2' and a.id not in($submit_id_sc) AND b.lc_sc_id in(".implode(',',$sc_id_arr).") and a.company_id='$cbo_company_name' AND a.status_active='1' and a.is_deleted=0 group by a.id a.bank_ref_no, a.bank_ref_date, a.negotiation_date, c.is_lc");
			}
			else if($db_type==2)
			{
		...
		//var_dump($inv_arr);die;
                ...
                <tr>
                 ...
                 <tr>
					...
					if(!empty($lc_id_arr))
					{
						if($db_type==0)
						{
							$sql_lc=sql_select("SELECT a.id as id, a.invoice_no, a.invoice_quantity as current_invoice_qnty, a.invoice_value as current_invoice_value, a.net_invo_value 
							from 
									com_export_invoice_ship_mst a
							where 
									a.lc_sc_id in(". implode(',',$lc_id_arr).") and a.is_lc=1 and a.status_active=1 and benificiary_id like '$cbo_company_name' and  buyer_id like '$cbo_buyer_name'");
						}
						else if($db_type==2)
						{
							$sql_lc=sql_select("SELECT a.id as id, a.invoice_no, a.invoice_quantity as current_invoice_qnty, a.invoice_value as current_invoice_value, a.net_invo_value 
					...
					if(!empty($sc_id_arr))
					{
						if($db_type==0)
						{
							...
							from 
									com_export_invoice_ship_mst a
							where 
									a.lc_sc_id in(".implode(',',$sc_id_arr).") and a.is_lc=2 and a.status_active=1 and benificiary_id like '$cbo_company_name' and  buyer_id like '$cbo_buyer_name'");
						}
						else if($db_type==2)
						{
							$sql_lc=sql_select("SELECT a.id as id, a.invoice_no, a.invoice_quantity as current_invoice_qnty, a.invoice_value as current_invoice_value, a.net_invo_value
            ...
            <table width="400" style="vertical-align:top">	     	
                <tr>
                    <td width="120">Total BTB Opened</td>
                    <td width="100" align="right" style="font-weight:bold;">
                        <? 
						if(!empty($lc_id_arr)) $lc_id_arr=implode(',',$lc_id_arr); else $lc_id_arr=0;
						if(!empty($sc_id_arr)) $sc_id_arr=implode(',',$sc_id_arr); else $sc_id_arr=0;
						//if(empty($sc_id_arr)) $sc_id_arr=0;
						//print_r($sc_id_arr);die; 
						/*echo "select 
						$group_concat(distinct a.id) as id,
						$group_concat(distinct b.id) as dtls_id, 
						(a.lc_value) as lc_value
						from
							 com_btb_lc_master_details a, com_btb_export_lc_attachment b
						where
							a.id=b.import_mst_id and b.is_lc_sc=0 and b.lc_sc_id in($lc_id_arr)
						union all
						select 
						$group_concat(distinct a.id) as id,
						$group_concat(distinct b.id) as dtls_id,
						...
						from
							 com_btb_lc_master_details a, com_btb_export_lc_attachment b
						where
							a.id=b.import_mst_id and b.is_lc_sc=1 and b.lc_sc_id in($sc_id_arr)";die;*/
							/*echo "select 
						$group_concat(a.id) as id, 
						sum(distinct a.lc_value) as lc_value, 
						sum(case when a.payterm_id=1 THEN a.lc_value else 0 end) as at_sight,
						sum(case when a.payterm_id=3 THEN a.lc_value else 0 end) as cash_in_advance
						from
								com_btb_lc_master_details a, com_btb_export_lc_attachment b
						where
								a.id=b.import_mst_id and b.is_lc_sc=0 and b.lc_sc_id in($lc_id_arr) group by a.id
						...
						from
								com_btb_lc_master_details a, com_btb_export_lc_attachment b
						where
								a.id=b.import_mst_id and b.is_lc_sc=1 and b.lc_sc_id in($sc_id_arr) group by a.id";die;*/
						if($db_type==0)
						{
						$btb_mst_lc_id=return_field_value("$group_concat(distinct import_mst_id) as import_mst_id"," com_btb_export_lc_attachment","lc_sc_id in($lc_id_arr) and is_lc_sc=0","import_mst_id");
						$btb_mst_sc_id=return_field_value("$group_concat(distinct import_mst_id) as import_mst_id","com_btb_export_lc_attachment","lc_sc_id in($sc_id_arr) and is_lc_sc=1","import_mst_id");
						}
						else if($db_type==2)
						{
						$btb_mst_lc_id=return_field_value("LISTAGG(CAST(import_mst_id AS VARCHAR(4000)), ',') WITHIN GROUP (ORDER BY import_mst_id) as import_mst_id"," com_btb_export_lc_attachment","lc_sc_id in($lc_id_arr) and is_lc_sc=0","import_mst_id");
						$btb_mst_sc_id=return_field_value("LISTAGG(CAST(import_mst_id AS VARCHAR(4000)), ',') WITHIN GROUP (ORDER BY import_mst_id) as import_mst_id","com_btb_export_lc_attachment","lc_sc_id in($sc_id_arr) and is_lc_sc=1","import_mst_id");
						}
						...
						<a href="#" onClick="btb_open('btb_open','<? echo $btb_id; ?>')"><? echo number_format($btb_open_value,2); ?> </a>
                    </td> 
                    <td>&nbsp; </td>
                </tr>
                <tr>     		
                    ...
                    <td align="right" style="font-weight:bold;">
						...
						$bill_accepted=return_field_value("sum(current_acceptance_value) as current_acceptance_value","com_import_invoice_dtls","btb_lc_id in($btb_id)","current_acceptance_value");
                        ...
                        <a href="#" onClick="btb_open('btb_accep','<? echo $btb_id; ?>')">
                        <?
                ...
                <tr>     		
					...
					$paid2=return_field_value("sum(accepted_ammount) as paid"," ","lc_id in($usance_paid_lc_id) and  payment_head=40 and status_active='1'","paid");
					$paid=($paid2+$atsite_accepted+$cash_in_advance);
					
					$paid_all_id=$cash_lc_id."_".$atsite_accep_id."_".$usance_paid_lc_id;
					
                     ?>
                    <a href="#" onClick="btb_open('btb_paid','<? echo $paid_all_id; ?>')">
					<?
    ...
    <? //var_dump($sc_id_arr);echo "<br>___________<br>"; var_dump($lc_id_arr);echo "<br>___________<br>"; var_dump($sc_lc_id_arr); ?>
    ...
    <table width="1900" cellpadding="0" cellspacing="0" border="0" rules="all" id="" align="left">
        <tr>
        <td><strong style="font:Arial, Helvetica, sans-serif; font-size:16px;">Document Submited:</strong></td>
        </tr>
    </table>
    ...
    <table width="1900" cellpadding="0" cellspacing="0" border="0" rules="all" id="" align="left">
        <tr>
        <td><strong style="font:Arial, Helvetica, sans-serif; font-size:16px;">Un-Submited Invoice:</strong></td>
        </tr>
    </table>
    ...
    <table width="700" cellpadding="0" cellspacing="0" border="1" rules="all" align="left" class="rpt_table">
        ...
        foreach($inv_arr as $row)
        {
			...
			<tr bgcolor="<? echo "#FFFFFF"; ?>">
				<td width="200"><? if($row[('is_lc')]==1) echo $lc_no_arr[$row[('lc_sc_id')]]; else echo  $sc_no_arr[$row[('lc_sc_id')]]?></td>
				<td width="200"><? echo $row[('invoice_no')]?></td>
    ...
    <table width="700" cellpadding="0" cellspacing="0" border="1" rules="all" align="left" class="rpt_table">
        ...
        foreach($adjustment_arr as $row)
        {
			...
			<tr bgcolor="<? echo $bgcolor; ?>">
				...
				<td width="200"><? echo $row['invoice_no']?></td>
				<td width="150" align="right"><? echo number_format($row['current_invoice_value'],2); $to_adjst_gross+=$row['current_invoice_value'];?></td>
				<td width="150" align="right"><?  $adjust_amt=$row['current_invoice_value']-$row['net_invo_value']; echo number_format($adjust_amt,2); $to_adjust+=$adjust_amt;?></td>
                <td width="150" align="right"><? echo number_format($row['net_invo_value'],2); $to_adjust_net+=$row['net_invo_value'];?></td>
			</tr>
			<?
			$i++;
        }
...
if ($action=="search_file_info")
{
	...
	if($txt_search_common!="" && $cbo_search_by==1)
			...
			else if($txt_search_common!="" && $cbo_search_by==2)
			{
				...
				$sql="select beneficiary_name, internal_file_no, lien_bank, buyer_name, lc_year as lc_sc_year from com_export_lc where beneficiary_name='$company_id' and buyer_name like'%$txt_search_common%' and status_active=1 and is_deleted=0 $lien_bank_id  group by internal_file_no, lc_year, beneficiary_name, buyer_name , lien_bank
				union all
				select beneficiary_name, internal_file_no, lien_bank, buyer_name, sc_year as lc_sc_year from com_sales_contract where internal_file_no not in(select internal_file_no from com_export_lc where beneficiary_name='$company_id' and status_active=1 and is_deleted=0 and buyer_name='$txt_search_common') and beneficiary_name='$company_id' and status_active=1 and is_deleted=0 and buyer_name like'%$txt_search_common%' $lien_bank_id group by internal_file_no, sc_year, beneficiary_name, buyer_name , lien_bank";
			}
			else if($txt_search_common!="" && $cbo_search_by==3)
			{
				//echo $txt_search_common; die;
				
				$sql="select beneficiary_name, internal_file_no, lien_bank, buyer_name, lc_year as lc_sc_year from com_export_lc where beneficiary_name='$company_id' and lien_bank='$txt_search_common' and status_active=1 and is_deleted=0 $buy_query  group by internal_file_no, lc_year, beneficiary_name, buyer_name , lien_bank
				union all
				select beneficiary_name, internal_file_no, lien_bank, buyer_name, sc_year as lc_sc_year from com_sales_contract where internal_file_no not in(select internal_file_no from com_export_lc where beneficiary_name='$company_id' and status_active=1 and is_deleted=0 and lien_bank='$txt_search_common') and beneficiary_name='$company_id' and status_active=1 and is_deleted=0 and lien_bank='$txt_search_common' $buy_query  group by internal_file_no, sc_year, beneficiary_name, buyer_name , lien_bank";
			}
			else
			{
				$sql="select beneficiary_name, internal_file_no, lien_bank, buyer_name , lc_year as lc_sc_year from com_export_lc where beneficiary_name='$company_id' and status_active=1 and is_deleted=0 $buy_query $lien_bank_id group by internal_file_no, lc_year, beneficiary_name, buyer_name , lien_bank
				union all
				select beneficiary_name, internal_file_no, lien_bank, buyer_name, sc_year as lc_sc_year from com_sales_contract where internal_file_no not in(select internal_file_no from com_export_lc where beneficiary_name='$company_id' and status_active=1 and is_deleted=0) and beneficiary_name='$company_id' and status_active=1 and is_deleted=0 $buy_query $lien_bank_id group by internal_file_no, sc_year, beneficiary_name, buyer_name , lien_bank";
			}
			//echo $sql;die;
		?>
...
if ($action=="btb_accep")
{
	...
	$sql= "select 
			...
			where
				a.id=b.import_invoice_id and a.btb_lc_id=c.id and a.btb_lc_id in($hidden_btb_id) group by a.id,a.invoice_no,a.maturity_date,a.invoice_date,c.supplier_id,c.item_category_id";
	}
	else if($db_type==2)
	{
	$sql= "select 
...
if ($action=="btb_paid")
{
	...
	$sql= "select 
				...
				c.supplier_id,
				c.item_category_id,
				sum(d.accepted_ammount) as accepted_ammount,
				max(d.payment_date) as payment_date
			from
				  com_import_invoice_mst a, com_import_invoice_dtls b, com_btb_lc_master_details c,  d
			where
				a.id=b.import_invoice_id and a.btb_lc_id=c.id and a.id=d.invoice_id and d.lc_id in(".$paid_id_arr[2].") group by a.id,a.invoice_no,a.maturity_date,a.invoice_date,c.supplier_id,c.item_category_id";
	}
	else if($db_type==2)
	{
	$sql= "select 
	...
	<table width="810" cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center" rules="all">
        ...
        <tbody>
        	<tr bgcolor="<? echo $bgcolor; ?>">
                ...
                <td  ><?  echo $item_category[$row[csf("item_category_id")]]; ?></td>
                <td ><?  echo $suplier_name_arr[$row[csf("supplier_id")]]; ?></td>
                <td  align="right"><?  echo number_format($row[csf("accepted_ammount")],2); $total_accept_amount+=$row[csf("accepted_ammount")] ;?></td>
                <td align="center"><?  if ($row[csf("payment_date")]!='0000-00-00') echo change_date_format($row[csf("payment_date")]); else echo ""; ?></td>	
            </tr>
        <?
		$i++;
		}
	...
	$sql= "select 
			...
			from
				  com_import_invoice_mst a, com_import_invoice_dtls b, com_btb_lc_master_details c
			where
				a.id=b.import_invoice_id and a.btb_lc_id=c.id and a.btb_lc_id in(".$paid_id_arr[1].") 
			group by 
