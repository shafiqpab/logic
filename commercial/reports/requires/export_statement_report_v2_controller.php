...
{ 
	...
	{
		$lc_date_cond = " and b.lc_date between  '$txt_date_from' and '$txt_date_to'";
		$sc_date_cond = " and b.contract_date between  '$txt_date_from' and '$txt_date_to'";
	}
	...
	$sql=sql_select("SELECT b.id as lc_id,  b.export_lc_no as export_lc_no, b.lc_value as lc_value,b.buyer_name, b.replacement_lc, 1 as type
				FROM  com_export_lc b
				WHERE b.status_active=1 and b.is_deleted=0 $cbo_company $cbo_buyer_name $cbo_lien_bank $lc_date_cond
				union all
				SELECT b.id as lc_id,  b.contract_no  as export_lc_no, b.contract_value  as lc_value,b.buyer_name, null as replacement_lc, 2 as type
				FROM com_sales_contract b 
				WHERE  b.status_active=1 and b.is_deleted=0 $cbo_company $cbo_buyer_name $cbo_lien_bank $sc_date_cond order by buyer_name asc");
	...
	if($db_type==0)
	{
		...
		";
		//union allSELECT group_concat(a.id) as invoice_id, group_concat(a.invoice_no) as invoice_no, sum(a.invoice_quantity) as invoice_quantity, sum(a.net_invo_value) as invoice_value, sum(a.freight_amnt_by_supllier) as freight_amnt_by_supllier, sum(a.paid_amount) as paid_amount, sum(a.advice_amount) as advice_amount, max(a.lc_sc_id) as lc_id, max(a.is_lc) as is_lc, 0 as sub_id, null as bank_ref_no, 0 as submit_type, 0 as buyer_id, 0 as as sub_collection, 0 as sub_purchase, d.id as rlz_id from com_export_invoice_ship_mst a, com_sales_contract g , com_export_proceed_realization d where a.lc_sc_id in(".$sc_id_group.") and  a.lc_sc_id = g.id  and d.invoice_bill_id=a.id  and g.pay_term = 3 and d.is_invoice_bill=2 and a.status_active=1 and a.is_deleted=0 group by d.id
	...
	else
	{
		...
		//union all SELECT LISTAGG(CAST(a.id AS VARCHAR(4000)), ',') WITHIN GROUP (ORDER BY a.id) as invoice_id, LISTAGG(CAST( a.invoice_no AS VARCHAR(4000)), ',') WITHIN GROUP (ORDER BY a.invoice_no) as invoice_no,  sum(a.invoice_quantity) as invoice_quantity, sum(a.net_invo_value) as invoice_value, sum(a.freight_amnt_by_supllier) as freight_amnt_by_supllier, sum(a.paid_amount) as paid_amount, sum(a.advice_amount) as advice_amount, max(a.lc_sc_id) as lc_id, max(a.is_lc) as is_lc, 0 as sub_id, null as bank_ref_no, 0 as submit_type, 0 as buyer_id, 0 as  sub_collection, 0 as sub_purchase, d.id as rlz_id from com_export_invoice_ship_mst a, com_sales_contract g , com_export_proceed_realization dwhere a.lc_sc_id in(".$sc_id_group.") and  a.lc_sc_id = g.id  and d.invoice_bill_id=a.id  and g.pay_term = 3 and d.is_invoice_bill=2 and a.status_active=1 and a.is_deleted=0 group by d.id
	}
	...
	foreach($rlz_lc_result as $row)
	{
		...
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]][$row[csf("rlz_id")]]['rlz_val']=$rlz_dtls_arr[$row[csf("rlz_id")]]["deducation"]+$rlz_dtls_arr[$row[csf("rlz_id")]]["distribution"];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]['deducation']+=$rlz_dtls_arr[$row[csf("rlz_id")]]["deducation"];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]['distribution']+=$rlz_dtls_arr[$row[csf("rlz_id")]]["distribution"];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]['nagotiate_distribution']+=$rlz_dtls_arr[$row[csf("rlz_id")]]["nagotiate_distribution"];
		
		
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]["is_lc"]=$row[csf("is_lc")];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]["lc_id"]=$row[csf("lc_id")];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]["sub_id"]=$row[csf("sub_id")];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]["buyer_id"]=$row[csf("buyer_id")];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]["bank_ref_no"]=$row[csf("bank_ref_no")];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]["invoice_no"]=$row[csf("invoice_no")];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]["invoice_quantity"]+=$row[csf("invoice_quantity")];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]["invoice_value"]+=$row[csf("invoice_value")];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]["advice_amount"]+=$row[csf("advice_amount")];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]["freight_amnt_by_supllier"]+=$row[csf("freight_amnt_by_supllier")];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]["paid_amount"]+=$row[csf("paid_amount")];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]["sub_collection"]+=$row[csf("sub_collection")];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]["sub_purchase"]+=$row[csf("sub_purchase")];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]["submit_type"]+=$row[csf("submit_type")];
	}
	...
	if($db_type==0)
	{
		...
		group by c.id, c.bank_ref_no, c.submit_type, c.buyer_id, d.id
		union all
		SELECT group_concat(a.id) as invoice_id, group_concat(a.invoice_no) as invoice_no, sum(a.invoice_quantity) as invoice_quantity, sum(a.net_invo_value) as invoice_value, sum(a.freight_amnt_by_supllier) as freight_amnt_by_supllier, sum(a.paid_amount) as paid_amount, sum(a.advice_amount) as advice_amount, max(a.lc_sc_id) as lc_id, max(a.is_lc) as is_lc, 0 as sub_id, null as bank_ref_no, 0 as submit_type, 0 as buyer_id, 0 as as sub_collection, 0 as sub_purchase, d.id as rlz_id
		from com_export_invoice_ship_mst a, com_sales_contract g , com_export_proceed_realization d
        where a.lc_sc_id in(".$sc_id_group.") and  a.lc_sc_id = g.id  and d.invoice_bill_id=a.id  and g.pay_term = 3 and d.is_invoice_bill=2 and a.status_active=1 and a.is_deleted=0 group by d.id
		";	
	}
	else
	{
		...
		group by c.id, c.bank_ref_no, c.submit_type, c.buyer_id, d.id
		union all
       SELECT LISTAGG(CAST(a.id AS VARCHAR(4000)), ',') WITHIN GROUP (ORDER BY a.id) as invoice_id, LISTAGG(CAST( a.invoice_no AS VARCHAR(4000)), ',') WITHIN GROUP (ORDER BY a.invoice_no) as invoice_no, 
    sum(a.invoice_quantity) as invoice_quantity, sum(a.net_invo_value) as invoice_value, sum(a.freight_amnt_by_supllier) as freight_amnt_by_supllier, sum(a.paid_amount) as paid_amount, 
    sum(a.advice_amount) as advice_amount, max(a.lc_sc_id) as lc_id, max(a.is_lc) as is_lc, 0 as sub_id, null as bank_ref_no, 0 as submit_type, 0 as buyer_id, 0 as  sub_collection, 0 as sub_purchase, d.id as rlz_id 
        from com_export_invoice_ship_mst a, com_sales_contract g , com_export_proceed_realization d
        where a.lc_sc_id in(".$sc_id_group.") and  a.lc_sc_id = g.id  and d.invoice_bill_id=a.id  and g.pay_term = 3 and d.is_invoice_bill=2 and a.status_active=1 and a.is_deleted=0 group by d.id
		";
	}
	...
	foreach($rlz_sc_result as $row)
	{
		...
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]['rlz_val']=$rlz_dtls_arr[$row[csf("rlz_id")]]["deducation"]+$rlz_dtls_arr[$row[csf("rlz_id")]]["distribution"];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]['deducation']+=$rlz_dtls_arr[$row[csf("rlz_id")]]["deducation"];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]['distribution']+=$rlz_dtls_arr[$row[csf("rlz_id")]]["distribution"];
		$total_realize_value[$row[csf("sub_id")]][$row[csf("rlz_id")]]['nagotiate_distribution']+=$rlz_dtls_arr[$row[csf("rlz_id")]]["nagotiate_distribution"];
	...
	$sql_no_inv_lc=sql_select("select b.id as lc_sc_id, b.buyer_name as buyer_id, b.export_lc_no as lc_sc_no, b.lc_value as lc_sc_val, 1 as type from  com_export_lc b where  b.status_active=1 and b.is_deleted=0 and b.id not in(".implode(",",$lc_id_arr_unique).") $cbo_company $cbo_buyer_name $cbo_lien_bank $lc_date_cond
	union all
	select b.id as lc_sc_id, b.buyer_name as buyer_id, b.contract_no as lc_sc_no, b.contract_value as lc_sc_val, 2 as type  from  com_sales_contract b where  b.status_active=1 and b.is_deleted=0 and b.convertible_to_lc=2 and  b.id not in(".implode(",",$sc_id_arr_unique).") $cbo_company $cbo_buyer_name $cbo_lien_bank $sc_date_cond
	");
	...
	$submission_value_array=return_library_array("select a.id, sum(b.net_invo_value) as invoice_value from com_export_doc_submission_mst a, com_export_doc_submission_invo b where a.id=b.doc_submission_mst_id and a.entry_form=40 and b.status_active=1 and b.is_deleted=0 group by a.id","id","invoice_value");
		...
		foreach($total_realize_value as $rlz_sub_id=>$row_results)
		{
			foreach($row_results as $rlz_id=>$row_result)
			{
		...
		foreach($sub_bank_arr as $sub_id=>$row_result)
		{
			...
			if($row_result[('bank_ref_date')]!="" && $row_result[('bank_ref_date')]!='0000-00-00') $bank_ref_date=change_date_format($row_result[('bank_ref_date')]);
			...
			$html_sub_bank_data.='<td  width="30"><p>'.$m.'</p></td>
								...
								<td align="center" width="70"><p>&nbsp;'. $bank_ref_date.'&nbsp;</p></td>
