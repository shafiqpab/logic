...
{ 
	...
	else
	{
		...
		//and b.invoice_date <='$txt_lc_sc_date'
		$bank_cond="";
		if($cbo_lein_bank>0) $bank_cond=" and a.issuing_bank_id=$cbo_lein_bank";
		$btb_sql="SELECT a.id as btb_lc_id, a.lc_date, a.tenor, a.payterm_id, a.lc_category, a.lc_value, a.maturity_from_id, b.id as import_inv_id, b.maturity_date, b.edf_paid_date, b.bank_acc_date, b.nagotiate_date, b.shipment_date, b.bill_date, a.maturity_from_id, a.issuing_bank_id, sum(c.current_acceptance_value) as edf_loan_value, 1 as type 
		from lib_bank p, com_btb_lc_master_details a, com_import_invoice_mst b, com_import_invoice_dtls c
		where p.id=a.issuing_bank_id and a.id=c.btb_lc_id and c.import_invoice_id=b.id and p.lien_bank=1 and a.lc_category>0 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and a.ref_closing_status<>1 and a.payterm_id <> 3 and a.importer_id=$cbo_company_name and a.lc_date <='$txt_lc_sc_date' $bank_cond
		group by a.id, a.lc_date, a.tenor, a.payterm_id, a.lc_category, a.lc_value, a.maturity_from_id, b.id, b.maturity_date, b.edf_paid_date, b.bank_acc_date, b.nagotiate_date, b.shipment_date, b.bill_date, a.maturity_from_id, a.issuing_bank_id
		...
		{
			$lc_wise_aeecp[$row[csf("btb_lc_id")]]+=$row[csf("edf_loan_value")];
			if($row[csf("edf_paid_date")]!="" && $row[csf("edf_paid_date")]!="0000-00-00" && (strtotime($row[csf("edf_paid_date")])<=strtotime($txt_lc_sc_date)) && (abs($row[csf("lc_category")])==3 || abs($row[csf("lc_category")])==5 || abs($row[csf("lc_category")])==11))
			{
				$lc_wise_edf_paid[$row[csf("btb_lc_id")]]+=$row[csf("edf_loan_value")];
			}
			if($row[csf("maturity_from_id")]==1)
			...
			if(strtotime($maturity_date)<=strtotime($txt_lc_sc_date))
			{
				...
				{
					$paid_values=0;
					if($row[csf("edf_paid_date")]!="" && $row[csf("edf_paid_date")]!="0000-00-00" && (strtotime($row[csf("edf_paid_date")])<=strtotime($txt_lc_sc_date)))
					{
						$paid_values=$row[csf("edf_loan_value")];
					}
					...
					if(abs($row[csf("lc_category")])==3 || abs($row[csf("lc_category")])==5 || abs($row[csf("lc_category")])==11)
					{
						if(($row[csf("edf_loan_value")]-$paid_values)>0)
						{
							$actual_edf_ids[date('M-Y',strtotime($row[csf("maturity_date")]))][$row[csf("issuing_bank_id")]].=$row[csf("btb_lc_id")].",";
							$actual_edf_data[date('M-Y',strtotime($row[csf("maturity_date")]))][$row[csf("issuing_bank_id")]]+=$row[csf("edf_loan_value")]-$paid_values;
							$ac_edf_grand_total+=$row[csf("edf_loan_value")]-$paid_values;
							
							$edf_test_data[date('M-Y',strtotime($row[csf("maturity_date")]))][$row[csf("issuing_bank_id")]]["btb_lc_id"].=$row[csf("btb_lc_id")].",";
							$edf_test_data[date('M-Y',strtotime($row[csf("maturity_date")]))][$row[csf("issuing_bank_id")]]["edf_val"]+=$row[csf("edf_loan_value")]-$paid_values;
							$edf_test_data[date('M-Y',strtotime($row[csf("maturity_date")]))][$row[csf("issuing_bank_id")]]["acc_val"]+=$row[csf("edf_loan_value")];
						}
					}
				}
				...
				if($row[csf("payterm_id")]!=1 && $row[csf("maturity_date")]!="" && $row[csf("maturity_date")]!="0000-00-00")
				{
					if(abs($row[csf("lc_category")])!=3 && abs($row[csf("lc_category")])!=5 && abs($row[csf("lc_category")])!=11)
					{
						if(($row[csf("edf_loan_value")]-$payment_data_array[$row[csf("import_inv_id")]])>0)
						{
							$actual_btb_ids[date('M-Y',strtotime($row[csf("maturity_date")]))][$row[csf("issuing_bank_id")]].=$row[csf("btb_lc_id")].",";
						}						
						$actual_btb_data[date('M-Y',strtotime($row[csf("maturity_date")]))][$row[csf("issuing_bank_id")]]+=$row[csf("edf_loan_value")]-$payment_data_array[$row[csf("import_inv_id")]];
					}
				}
				else if($row[csf("payterm_id")]==1 && $row[csf("edf_paid_date")]=="" && $row[csf("maturity_date")]!="" && abs($row[csf("lc_category")])!=3 && abs($row[csf("lc_category")])!=5 && abs($row[csf("lc_category")])!=11)
				{
					if(($row[csf("edf_loan_value")])>0)
					{
						$actual_upass_ids[date('M-Y',strtotime($row[csf("maturity_date")]))][$row[csf("issuing_bank_id")]].=$row[csf("btb_lc_id")].",";
					}						
					$actual_upass_data[date('M-Y',strtotime($row[csf("maturity_date")]))][$row[csf("issuing_bank_id")]]+=$row[csf("edf_loan_value")];
				}
		...
		//echo "<pre>";print_r($edf_test_data);die;
		//ksort($actual_edf_data);ksort($actual_btb_data);
		//echo "<pre>";print_r($actual_edf_data);
		//echo "<pre>";print_r($actual_edf_ids);die;
		////
		
		
		$bank_cond="";
		if($cbo_lein_bank>0) $bank_cond=" and b.issuing_bank_id=$cbo_lein_bank";
		$btb_id_sql="SELECT b.id as btb_id, b.lc_date, b.tenor, b.payterm_id, b.lc_category, b.lc_value, b.issuing_bank_id
		from lib_bank p, com_btb_lc_master_details b 
		where p.id=b.issuing_bank_id and p.lien_bank=1 and b.is_deleted=0 and b.status_active=1 and b.ref_closing_status<>1 and b.payterm_id <> 3 and b.importer_id=$cbo_company_name and b.lc_date <='$txt_lc_sc_date' and (b.lc_category<>0 or b.lc_category is not null) $bank_cond";
		//echo $btb_id_sql;
		$bank_wise_edf_lc=$bank_wise_btb_lc=$all_bank_arr=array();
		$btb_id_result=sql_select($btb_id_sql);
		foreach($btb_id_result as $row)
		{
			$all_bank_arr[$row[csf("issuing_bank_id")]]=$bank_arr[$row[csf("issuing_bank_id")]];
			if($row[csf("payterm_id")]==1 && (abs($row[csf("lc_category")])==3 || abs($row[csf("lc_category")])==5 || abs($row[csf("lc_category")])==11))
			{
				$bank_wise_edf_lc[$row[csf("issuing_bank_id")]]+=$row[csf("lc_value")]-$lc_wise_edf_paid[$row[csf("btb_id")]];
			}
			if(abs($row[csf("lc_category")])!=3 && abs($row[csf("lc_category")])!=5 && abs($row[csf("lc_category")])!=11)
			{
				if($row[csf("payterm_id")]==1)
				{
					...
					foreach($invoce_data_arr[$row[csf("btb_id")]] as $inv_id=>$value)
					{
						if($value["edf_paid_date"]!="")
						{
							$btb_paid+=$payment_data_array[$inv_id];
						}						
					}
					$bank_wise_upass_lc[$row[csf("issuing_bank_id")]]+=$row[csf("lc_value")]-$btb_paid;
				}
				else
				{
					$btb_paid=0;
					...
					$bank_wise_btb_lc[$row[csf("issuing_bank_id")]]+=$row[csf("lc_value")]-$btb_paid;
				}

			}
		}
		
		foreach($actual_edf_data as $month_year=>$edf_val)
		{
			foreach($all_bank_arr as $bank_id=>$bank_val)
			{
				if($edf_val[$bank_id]>1)
				{
					$actual_edf_valid_data[$month_year][$bank_id]+=$edf_val[$bank_id];
				}
			}
		}
		foreach($actual_btb_data as $month_year=>$btb_val)
		{
			foreach($all_bank_arr as $bank_id=>$bank_val)
			{
				if($btb_val[$bank_id]>1)
				{
					$actual_btb_valid_data[$month_year][$bank_id]+=$btb_val[$bank_id];
				}
			}
		}
		foreach($actual_upass_data as $month_year=>$btb_val)
		{
			foreach($all_bank_arr as $bank_id=>$bank_val)
			{
				if($btb_val[$bank_id]>1)
				{
					$actual_upass_valid_data[$month_year][$bank_id]+=$btb_val[$bank_id];
				}
			}
		}
		//echo "<pre>";print_r($actual_edf_valid_data);die;
		//echo "<pre>";print_r($actual_btb_valid_data);
		
		//echo count($test_count);echo "<pre>";print_r($test_count); die;
		//echo "<pre>";print_r($lc_total_edf_data_arr);die;
		ob_start();
		$table_width=240+(110*count($all_bank_arr));
		$div_width=$table_width+20;
		?>
		<div style="width:<?= $table_width;?>px;" id="scroll_body">
		<fieldset style="width:100%">
			<p style="font-size:18px; font-weight:bold"><? echo $report_title;?></p>
			<table width="<?= $table_width;?>" cellpadding="0" cellspacing="0" class="rpt_table" border="1" rules="all" align="left">
				<thead>
					<tr>
						<th width="30" rowspan="2">SL</th>
						<th width="100" rowspan="2">Facility</th>
						<th colspan="<?= count($all_bank_arr);?>">Bank Name</th>
                        <th rowspan="2">Total</th>
					</tr>
                    <tr>
                    <?
					...
					{
						?>
                        <th width="110" title="<?= $bank_id;?>"><? echo $bank_val; ?></th>
                        <?
					}
					?>
                    </tr>
				...
				<tbody>
					<tr bgcolor="#FFFFFF" onclick="change_color('tr_1','FFFFFF')" id="tr_1" style="cursor:pointer">
						...
						{
							?>
                            <td align="right"><? echo number_format($bank_wise_edf_lc[$bank_id],2); ?></td>
                            <?
							$edf_lc_total+=$bank_wise_edf_lc[$bank_id];
							$bank_wise_lc_total[$bank_id]+=$bank_wise_edf_lc[$bank_id];
						}
						?>
						<td align="right"><? echo number_format($edf_lc_total,2); ?></td>
					</tr>
                    <tr bgcolor="#E9F3FF" onclick="change_color('tr_2','E9F3FF')" id="tr_2" style="cursor:pointer">
						...
						{
							?>
                            <td align="right"><? echo number_format($bank_wise_btb_lc[$bank_id],2); ?></td>
                            <?
							$btb_lc_total+=$bank_wise_btb_lc[$bank_id];
							$bank_wise_lc_total[$bank_id]+=$bank_wise_btb_lc[$bank_id];
						}
						?>
						<td align="right"><? echo number_format($btb_lc_total,2); ?></td>
					</tr>
                    <tr bgcolor="#E9F3FF" onclick="change_color('tr_2','E9F3FF')" id="tr_2" style="cursor:pointer">
						...
						{
							?>
                            <td align="right"><? echo number_format($bank_wise_upass_lc[$bank_id],2); ?></td>
                            <?
							$upass_lc_total+=$bank_wise_upass_lc[$bank_id];
							$bank_wise_lc_total[$bank_id]+=$bank_wise_upass_lc[$bank_id];
						}
						?>
						<td align="right"><? echo number_format($upass_lc_total,2); ?></td>
					</tr>
                    <tr bgcolor="#FFFF99">
                        <td colspan="2" align="right">TTL LC</td>
                        <?
                        foreach($all_bank_arr as $bank_id=>$bank_val)
						{
							?>
                            <td align="right"><? echo number_format($bank_wise_lc_total[$bank_id],2); ?></td>
                            <?
							$lc_total+=$bank_wise_lc_total[$bank_id];
						}
						$i++;
						?>
						<td align="right"><? echo number_format($lc_total,2); ?></td>
					</tr>
                    <tr bgcolor="#CCCCCC"><td align="center" style="font-size:16px; font-weight:bold;" colspan="<?= 3+count($all_bank_arr);?>">EDF Liability Month Wise Actual</td></tr>
                    <?
					$i=4;
					foreach($actual_edf_valid_data as $month_year=>$edf_val)
                    {
						$bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF";
						$month_year_ref=explode("-",$month_year);
						$month_id=date('m',strtotime($month_year_ref[0]));
						$year=$month_year_ref[1];
						?>
						<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i?>','<? echo $bgcolor?>')" id="tr_<? echo $i; ?>" style="cursor:pointer">
							<td align="center" valign="middle"><? echo $i;?></td>
							<td><? echo $month_year; ?></td>
							<?
							$actual_edf_total=0;
							foreach($all_bank_arr as $bank_id=>$bank_val)
							{
								?>
								<td align="right" title="<?= $month_year;?>"><a href='#report_detals'  onclick= "openmypage_btb_ac_dtls('<? echo implode(",",array_unique(explode(",",chop($actual_edf_ids[$month_year][$bank_id],",")))); ?>','<? echo $year; ?>','<? echo $month_id; ?>','btb_ac_popup','BTB Info','1','<? echo $txt_lc_sc_date; ?>')"><? echo number_format($edf_val[$bank_id],2); ?></a></td>
								<?
								$actual_edf_total+=$edf_val[$bank_id];
								$bank_wise_edf_actual[$bank_id]+=$edf_val[$bank_id];
							}
							?>
							<td align="right"><? echo number_format($actual_edf_total,2); ?></td>
						</tr>
						<?
						$i++;
                    }
					?>
                    <tr bgcolor="#FFFF99">
                    	<td colspan="2" align="right">Total EDF Actual Liability</td>
                        <?
                        foreach($all_bank_arr as $bank_id=>$bank_val)
						{
							?>
                            <td align="right"><? echo number_format($bank_wise_edf_actual[$bank_id],2); ?></td>
                            <?
							$actual_edf_gTotal+=$bank_wise_edf_actual[$bank_id];
							$bank_wise_liability[$bank_id]+=$bank_wise_edf_actual[$bank_id];
						}
						$i++;
						?>
						<td align="right"><? echo number_format($actual_edf_gTotal,2); ?></td>
                    </tr>
                    <tr bgcolor="#CCCCCC"><td align="center" style="font-size:16px; font-weight:bold;" colspan="<?= 3+count($all_bank_arr);?>">BTB Liability Month Wise Actual</td></tr>
                    <?
					foreach($actual_btb_valid_data as $month_year=>$btb_val)
                    {
						$bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF";
						$month_year_ref=explode("-",$month_year);
						$month_id=date('m',strtotime($month_year_ref[0]));
						...
						<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i?>','<? echo $bgcolor?>')" id="tr_<? echo $i; ?>" style="cursor:pointer">
							...
							{
								?>
								<td align="right" title="<?= $month_year;?>"><a href='#report_detals'  onclick= "openmypage_btb_ac_dtls('<? echo implode(",",array_unique(explode(",",chop($actual_btb_ids[$month_year][$bank_id],",")))); ?>','<? echo $year; ?>','<? echo $month_id; ?>','btb_ac_popup','BTB Info','3','')"><? echo number_format($btb_val[$bank_id],2); ?></a></td>
								<?
								$actual_btb_total+=$btb_val[$bank_id];
								$bank_wise_btb_actual[$bank_id]+=$btb_val[$bank_id];
							}
							?>
							<td align="right"><? echo number_format($actual_btb_total,2); ?></td>
						</tr>
						<?
						$i++;
                    }
					?>
                    <tr bgcolor="#FFFF99">
                    	<td colspan="2" align="right">Total BTB Actual Liability</td>
                        <?
                        foreach($all_bank_arr as $bank_id=>$bank_val)
						{
							?>
                            <td align="right"><? echo number_format($bank_wise_btb_actual[$bank_id],2); ?></td>
                            <?
							$actual_btb_gTotal+=$bank_wise_btb_actual[$bank_id];
							$bank_wise_liability[$bank_id]+=$bank_wise_btb_actual[$bank_id];
						}
						$i++;
						?>
						<td align="right"><? echo number_format($actual_btb_gTotal,2); ?></td>
                    </tr>
                    <tr bgcolor="#CCCCCC">
						<td align="center" style="font-size:16px; font-weight:bold;" colspan="<?= 3+count($all_bank_arr);?>">UPASS Liability Month Wise Actual</td>
					</tr>
                    <?
						foreach($actual_upass_valid_data as $month_year=>$upass_val)
						{
							$bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF";
							$month_year_ref=explode("-",$month_year);
							$month_id=date('m',strtotime($month_year_ref[0]));
							...
							<tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i?>','<? echo $bgcolor?>')" id="tr_<? echo $i; ?>" style="cursor:pointer">
								...
								{
									?>
									<td align="right" title="<?= $month_year;?>"><a href='#report_detals'  onclick= "openmypage_btb_ac_dtls('<? echo implode(",",array_unique(explode(",",chop($actual_upass_ids[$month_year][$bank_id],",")))); ?>','<? echo $year; ?>','<? echo $month_id; ?>','btb_ac_popup','BTB Info','3','')"><? echo number_format($upass_val[$bank_id],2); ?></a></td>
									<?
									$actual_upass_total+=$upass_val[$bank_id];
									$bank_wise_upass_actual[$bank_id]+=$upass_val[$bank_id];
								}
								?>
								<td align="right"><? echo number_format($actual_upass_total,2); ?></td>
							</tr>
							<?
							$i++;
						}
					?>
                    <tr bgcolor="#FFFF99">
                    	<td colspan="2" align="right">Total UPASS Actual Liability</td>
                        <?
                        foreach($all_bank_arr as $bank_id=>$bank_val)
						{
							?>
                            <td align="right"><? echo number_format($bank_wise_upass_actual[$bank_id],2); ?></td>
                            <?
							$actual_upass_gTotal+=$bank_wise_upass_actual[$bank_id];
							$bank_wise_liability[$bank_id]+=$bank_wise_upass_actual[$bank_id];
						}
						$i++;
						?>
						<td align="right"><? echo number_format($actual_upass_gTotal,2); ?></td>
                    </tr>
                    <tr bgcolor="#FFCC99">
                    	<td colspan="2" align="right">Total Accepted Liability</td>
                        <?
                        foreach($all_bank_arr as $bank_id=>$bank_val)
						{
							?>
                            <td align="right"><? echo number_format($bank_wise_liability[$bank_id],2); ?></td>
                            <?
							$tot_accep_liability+=$bank_wise_liability[$bank_id];
						}
						$i++;
						?>
						<td align="right"><? echo number_format($tot_accep_liability,2); ?></td>
                    </tr>
                    <tr bgcolor="#CCCCCC"><td align="center" style="font-size:16px; font-weight:bold;" colspan="<?= 3+count($all_bank_arr);?>">Pending Liability</td></tr>
                    <tr bgcolor="#FFFFFF" onclick="change_color('tr_<? echo $i?>','FFFFFF')" id="tr_<? echo $i; ?>" style="cursor:pointer">
                    	<td align="center" valign="middle"><?= $i;?></td>
                        <td>EDF LC</td>
                        <?
                        foreach($all_bank_arr as $bank_id=>$bank_val)
						{
							$bank_pending_edf_liability=$bank_wise_edf_lc[$bank_id]-$bank_wise_edf_actual[$bank_id];
							?>
                            <td align="right"><a href='#report_detals' onclick= "openmypage_btb_dtls('<? echo $cbo_company_name; ?>','<? echo $bank_id; ?>','<? echo $txt_lc_sc_date; ?>','btb_popup','BTB Info','11')"><? echo number_format($bank_pending_edf_liability,2); ?></a></td>
                            <?
							$edf_pending_total+=$bank_pending_edf_liability;
							$tot_pending_liability[$bank_id]+=$bank_pending_edf_liability;
						}
						$i++;
						?>
						<td align="right"><? echo number_format($edf_pending_total,2); ?></td>
					</tr>
                    <tr bgcolor="#E9F3FF" onclick="change_color('tr_<? echo $i?>','E9F3FF')" id="tr_<? echo $i; ?>" style="cursor:pointer">
                    	<td align="center" valign="middle"><?= $i;?></td>
                        <td>BTB LC</td>
                        <?
                        foreach($all_bank_arr as $bank_id=>$bank_val)
						{
							$bank_pending_btb_liability=$bank_wise_btb_lc[$bank_id]-$bank_wise_btb_actual[$bank_id];
							?>
                            <td align="right"><a href='#report_detals' onclick= "openmypage_btb_dtls('<? echo $cbo_company_name; ?>','<? echo $bank_id; ?>','<? echo $txt_lc_sc_date; ?>','btb_popup','BTB Info','12')"><? echo number_format($bank_pending_btb_liability,2); ?></a></td>
                            <?
							$btb_pending_total+=$bank_pending_btb_liability;
							$tot_pending_liability[$bank_id]+=$bank_pending_btb_liability;
						}
						$i++;
						?>
						<td align="right"><? echo number_format($btb_pending_total,2); ?></td>
					</tr>
                    <tr bgcolor="#E9F3FF" onclick="change_color('tr_<? echo $i?>','E9F3FF')" id="tr_<? echo $i; ?>" style="cursor:pointer">
                    	<td align="center" valign="middle"><?= $i;?></td>
                        <td>UPASS</td>
                        ...
                        foreach($all_bank_arr as $bank_id=>$bank_val)
						{
							$bank_pending_upass_liability=$bank_wise_upass_lc[$bank_id]-$bank_wise_upass_actual[$bank_id];
							?>
                            <td align="right"><a href='#report_detals' onclick= "openmypage_btb_dtls('<? echo $cbo_company_name; ?>','<? echo $bank_id; ?>','<? echo $txt_lc_sc_date; ?>','btb_popup','BTB Info','13')"><? echo number_format($bank_pending_upass_liability,2); ?></a></td>
                            <?
							$upass_pending_total+=$bank_pending_upass_liability;
							$tot_pending_liability[$bank_id]+=$bank_pending_upass_liability;
						}
						?>
						<td align="right"><? echo number_format($upass_pending_total,2); ?></td>
					</tr>
                    <tr bgcolor="#FFCC99">
                    	<td colspan="2" align="right">TTL Pending Liability</td>
                        <?
                        foreach($all_bank_arr as $bank_id=>$bank_val)
						{
							?>
                            <td align="right"><? echo number_format($tot_pending_liability[$bank_id],2); ?></td>
                            <?
							$tot_pending+=$tot_pending_liability[$bank_id];
						}
						$i++;
						?>
						<td align="right"><? echo number_format($tot_pending,2); ?></td>
                    </tr>
				</tbody>
			</table>
		</fieldset>
...
if($action=="btb_ac_popup")
{
	...
	extract($_REQUEST);
	//echo $btb_id."==".$type."==".$surch_date;die;
	...
	$btb_sql="select a.id, a.importer_id, a.issuing_bank_id, a.supplier_id, a.lc_number, a.lc_category, a.lc_date, a.lc_expiry_date, a.lc_value, a.tenor, a.payterm_id, b.id as inv_id, b.invoice_no, b.bank_acc_date, b.maturity_date, b.edf_paid_date, sum(c.current_acceptance_value) as accep_value
	from com_btb_lc_master_details a, com_import_invoice_mst b, com_import_invoice_dtls c
	...
	group by a.id, a.importer_id, a.issuing_bank_id, a.supplier_id, a.lc_number, a.lc_category, a.lc_date, a.lc_expiry_date, a.lc_value, a.tenor, a.payterm_id, b.id, b.invoice_no, b.bank_acc_date, b.maturity_date, b.edf_paid_date
	...
	ob_start();
	?>
	<div id="" align="center" style="width:1430px">
	<fieldset style="width:1430px;">
		<table class="rpt_table" border="1" rules="all" width="1420" cellpadding="0" cellspacing="0">
			<thead>
				<th width="50">SL</th>
			...
			foreach($btb_sql_result as $row)  
			{
				...
				if($row[csf("payterm_id")]==1)
				{
					if(abs($row[csf("lc_category")])==3 || abs($row[csf("lc_category")])==5 || abs($row[csf("lc_category")])==11)
					{
						if($row[csf("edf_paid_date")]!="" && $row[csf("edf_paid_date")]!="0000-00-00"  && (strtotime($row[csf("edf_paid_date")])<=strtotime($surch_date)))
						{
							$paid_values=$row[csf("accep_value")];
						}
					}
				}
				else
				{
					if(abs($row[csf("lc_category")])!=3 && abs($row[csf("lc_category")])!=5 && abs($row[csf("lc_category")])!=11)
					{
						$paid_values=$invoice_wise_payment[$row[csf("inv_id")]]["accepted_ammount"];
					}
				}
				$pending_value =$row[csf("accep_value")]-$paid_values;
				if($pending_value>0)
				{
                    ...
                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                        ...
                        <td align="right"><? echo number_format($paid_values,2);  ?></td>
                        <td align="right"><p><? echo number_format($pending_value,2); ?>&nbsp;</p></td>
                        <td title="<?= $row[csf('lc_category')];?>"><p><? echo $supply_source[$row[csf('lc_category')]*1]; ?>&nbsp;</p></td>
...
if($action=="btb_popup")
{
	...
	{
		$lc_cond=" and b.payterm_id=1 and abs(b.lc_category) in(3,5,11)";
	}
	else if($type==12)
	{
		$lc_cond=" and b.payterm_id<>1 and abs(b.lc_category) not in(3,5,11)";
	}
	else if($type==13)
	{
		$lc_cond=" and b.payterm_id=1 and abs(b.lc_category) not in(3,5,11)";
	...
	ob_start();
	?>
	<div id="" align="center" style="width:1490px">
	<fieldset style="width:1490px;">
		<table class="rpt_table" border="1" rules="all" width="1490" cellpadding="0" cellspacing="0">
			<thead>
				<th width="30">SL</th>
				<th width="110">BTB LC No</th>
				<th width="80">BTB LC Date</th>
				...
				<th width="80">Paid Date</th>
				<th width="100">Paid Value</th>
				<th width="100">Loan Liability</th>
			...
			foreach($btb_sql_result as $row)  
			{
				...
				if(number_format($loan_liability,2) != 0.00)
				{
                    ...
                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                        ...
                        <td><p><? echo $supply_source[$row[csf('lc_category')]*1]; ?>&nbsp;</p></td>
                        <td><p><? echo $bank_arr[$row[csf('issuing_bank_id')]]; ?>&nbsp;</p></td>
                    </tr>
                    <?
