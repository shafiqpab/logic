            ...
            <table cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr>
                        ...
                        <td align="center">
                        	<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view ('<? echo $company; ?>'+'**'+'<? echo $buyer; ?>'+'**'+document.getElementById('txt_style_ref_no').value+'**'+document.getElementById('txt_job_no').value, 'style_refarence_surch_list_view', 'search_div', 'po_and_date_wise_invoice_rpt_controller', 'setFilterGrid(\'list_view\',-1)');fn_selected();" style="width:100px;" />
                    	</td>
                    </tr>
            	</tbody>
           	</table>
...
if($action=="order_surch_list_view")
{
	...
	$sql = "SELECT a.id, a.po_number, a.job_no_mst, b.style_ref_no, b.job_no_prefix_num, $select_date as year, a.grouping, $group_con  
	from  wo_po_details_master b, wo_po_break_down a
	left join wo_po_acc_po_info c on a.id=c.po_break_down_id and c.is_deleted=0 
	where a.job_id=b.id and b.company_name=$company $search_con $buyer_cond  $style_cond and a.status_active=1 and b.status_active=1 
	group by a.id, a.po_number, a.job_no_mst, b.style_ref_no, b.job_no_prefix_num, a.grouping, $select_date"; 
	// echo $sql; die;
...
{ 
	...
	if($rpt_type==1)
	{
		...
		$str_cond = '';
		if($cbo_buyer_name) $str_cond.=" and b.buyer_name=$cbo_buyer_name";

		if($txt_style_ref != '')
		{
			if($txt_style_ref_id != '') $str_cond.=" and b.id in($txt_style_ref_id)";
			else $str_cond.=" and b.style_ref_no='$txt_style_ref'";
		}

		if($txt_order != '') 
		{
			if($txt_order_id != '') $str_cond.="and a.id in($txt_order_id)";
			else $str_cond.="and a.po_number='$txt_order'";
		}
		...
		if($txt_date_from!="" && $txt_date_to!="")
		{
			...
			$str_cond.= " and d.invoice_date between '$date_from' and '$date_to'";
		}
		
		$sql_invoice="SELECT a.id as ORDER_ID, a.JOB_NO_MST, a.PO_NUMBER, a.STATUS_ACTIVE, b.BUYER_NAME,b.STYLE_REF_NO, b.DEALING_MARCHANT, c.PO_BREAKDOWN_ID, c.mst_id as INVOICE_ID, c.id as INV_DTLS_ID, c.CURRENT_INVOICE_QNTY, c.CURRENT_INVOICE_VALUE, c.ACTUAL_PO_INFOS, d.IS_LC, d.LC_SC_ID, d.INVOICE_NO, d.EXP_FORM_NO
		from wo_po_break_down a, wo_po_details_master b, com_export_invoice_ship_dtls c, com_export_invoice_ship_mst d
		where a.job_id=b.id and a.id=c.po_breakdown_id and c.mst_id=d.id and c.current_invoice_value >0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and a.status_active in(1,3) and a.is_deleted=0 and b.is_deleted=0 and b.company_name=$cbo_company_name $str_cond order by a.id";
		// echo $sql_invoice;
		$invoice_result=sql_select($sql_invoice);
		$invoice_data_arr=array();$order_data_arr=array();
		...
		<div style="width:<?=$div_width;?>"> 
			<table width="<?=$div_width;?>" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" id="table_header_1" align="center"> 
					...
					<tr style="border:none;">
						<td colspan="8" align="center" style="border:none; font-size:14px;">
							<?if($txt_date_from!="" && $txt_date_to!=""){echo "Date: ".$txt_date_from." to ".$txt_date_to;}?>                    
						</td>
					</tr>
			</table>
			<br />
				...
				foreach($order_data_arr as $order_id=>$val)
				{
					...
					foreach($invoice_data_arr[$order_id] as $invoice_id=>$row)
					{
						...
						<tr bgcolor="<? echo $bgcolor;?>"  onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
							...
							<td >
								<?
									if($row['IS_LC']==1){ echo $lc_arr[$row['LC_SC_ID']]; }
									else{ echo $sc_arr[$row['LC_SC_ID']]; }
								?>
							</td>
	...
	else if($rpt_type==2)
	{
		...
		$str_cond = '';
		if($cbo_buyer_name) $str_cond.=" and b.buyer_name=$cbo_buyer_name and d.buyer_id=$cbo_buyer_name";
		...
		if($txt_date_from!="" && $txt_date_to!="")
		{
			...
			$str_cond.= " and d.ex_factory_date between '$date_from' and '$date_to'";
		}
		
		$sql_invoice="SELECT a.id as ORDER_ID, a.JOB_NO_MST, a.PO_NUMBER, b.BUYER_NAME,b.STYLE_REF_NO, b.DEALING_MARCHANT, a.STATUS_ACTIVE, c.PO_BREAKDOWN_ID, c.mst_id as INVOICE_ID, c.id as INV_DTLS_ID, c.CURRENT_INVOICE_QNTY, c.CURRENT_INVOICE_VALUE, d.IS_LC, d.LC_SC_ID, d.INVOICE_NO, d.EXP_FORM_NO, c.ACTUAL_PO_INFOS
		from  wo_po_break_down a, wo_po_details_master b,com_export_invoice_ship_dtls c, com_export_invoice_ship_mst d
		where a.job_id=b.id and b.company_name=$cbo_company_name and a.id=c.po_breakdown_id and c.mst_id=d.id and c.current_invoice_value >0 and d.ex_factory_date is not null and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and a.status_active in(1,3) and a.is_deleted=0 and b.is_deleted=0 and d.BENIFICIARY_ID=$cbo_company_name $str_cond";
		// echo $sql_invoice;
		
		$invoice_result=sql_select($sql_invoice);
		$invoice_data_arr=array();$order_data_arr=array();$po_id=0;$job_no_all='';$job_arr=array();
		foreach($invoice_result as $row)
		{
			$invoice_data_arr[$row['PO_BREAKDOWN_ID']][$row['INVOICE_ID']]['INVOICE_ID']=$row['INVOICE_ID'];
			$invoice_data_arr[$row['PO_BREAKDOWN_ID']][$row['INVOICE_ID']]['INV_DTLS_ID']=$row['INV_DTLS_ID'];
