...
function generate_report(rpt_type)
{
	...
	var data="action=report_generate"+get_submitted_data_string("cbo_company_name*cbo_buyer_name*cbo_lein_bank*txt_file_no*hide_year","../../")+'&report_title='+report_title+'&rpt_type='+rpt_type;
	freeze_window(3);
...
function openmypage_v2(sub_id)
{
	page_link='requires/file_wise_export_status_controller.php?action=acount_head_details_v2'+'&sub_id='+sub_id;
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe',page_link,'Account Head Details', 'width=600px,height=350px,center=1,resize=0,scrolling=0','../');
	emailwindow.onclose=function()
	{
		//alert("Jahid");
...
{	
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, "Select File", 'width=780px,height=200px,center=1,resize=0,scrolling=0','../')
	emailwindow.onclose=function()
	{
		//alert("Jahid");
	}		
...
function btb_open_category(action,btb_id,file_buyer,buyer,cat_wise_budge_val,item_cat)
{	
	page_link='requires/file_wise_export_status_controller.php?action='+action+'&hidden_btb_id='+btb_id+'&file_buyer='+file_buyer+'&item_cat='+item_cat+'&buyer='+buyer+'&cat_wise_budge_val='+cat_wise_budge_val;
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, "Select File", 'width=810px,height=300px,center=1,resize=0,scrolling=0','../')
	emailwindow.onclose=function()
	{
		//alert("Jahid");
	}		
...
function fnc_amount_detail(company,lc_ids,sc_ids,action,buyer)
{
	//alert(company+"**"+lc_ids+"**"+sc_ids+"**"+action);
	var popup_width='1100px';
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/file_wise_export_status_controller.php?company='+company+'&lc_ids='+lc_ids+'&sc_ids='+sc_ids+'&action='+action+'&action='+action+'&cbo_buyer_name='+buyer, 'Details Veiw', 'width='+popup_width+', height=300px,center=1,resize=0,scrolling=0','../');
}

function fnc_attach_order_details(company,txt_file_no,attach_order_id,action)
{
	var popup_width='1000px';
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/file_wise_export_status_controller.php?company='+company+'&txt_file_no='+txt_file_no+'&attach_order_id='+attach_order_id+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=300px,center=1,resize=0,scrolling=0','../');
}

function fnc_invoice_show(inv_id)
{
	var action ="print_invoice";
	print_report( inv_id, action, "../export_details/requires/export_information_entry_controller" ) ;
}

function print_report_button_setting(com_id)
{
	var report_ids=return_global_ajax_value( com_id, 'check_report_button', '', 'requires/file_wise_export_status_controller');
	//alert(report_ids);
	if(trim(report_ids)!="")
	{
		$("#show").hide();
		...
		{
			if(report_id[k]==86) $("#show").show();
			if(report_id[k]==96) $("#summary").show();
...
{	
	page_link='requires/file_wise_export_status_controller.php?action='+action+'&btb_id='+btb_id+'&file_no='+file_no+'&buyer_id='+buyer_id+'&cat_wise_budge_val='+cat_wise_budge_val+'&dtls_type='+dtls_type+'&category_type='+category_type;
		...
		<form id="file_wise_explort_import_status" action="" autocomplete="off" method="post">
			...
			<div id="content_search_panel" style="width:1020px;">
				...
				<fieldset style="width:100%" >
					<table class="rpt_table" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" width="1020">
						<thead>
						...
						<tr class="general">
							...
							<td  align="center">
								<input type="button" name="show" id="show" onClick="generate_report(1);" class="formbutton" style="width:70px; display:none" value="Show" />
								<input type="button" name="summary" id="summary" onClick="generate_report(2);" class="formbutton" style="width:70px; display:none" value="Summary" />
								<input type="button" name="show2" id="show2" onClick="generate_report(3);" class="formbutton" style="width:70px; display:none" value="Show 2" />
