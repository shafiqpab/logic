<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create BH Commission Report.
Functionality	:
JS Functions	:
Created by		:	Nayem
Creation date 	: 	28-7-2021
Updated by 		:
Update date		:
QC Performed BY	:
QC Date			:
...
//--------------------------------------------------------------------------------
echo load_html_head_contents('BH Commission Report', '../../', 1, 1, $unicode, 1, '', '');
?>
...
<script>
	...
	{		
        ...
        var data="action=report_generate"+get_submitted_data_string("cbo_company_name*cbo_bank_name*cbo_buyer_name*txt_job_no*txt_style_no*txt_date_from*txt_date_to*cbo_year_selection","../../")+'&report_title='+report_title;

		// alert(data);return;
		freeze_window(3);
		http.open("POST","requires/bh_commission_report_controller.php",true);
					...
					<fieldset style="width:960px;">
                        <table width="1080" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                            ...
                            <tbody>
                                <tr class="general">
                                    <td> 
                                        <? echo create_drop_down( "cbo_company_name", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and comp.core_business in(1,3) $company_cond order by company_name","id,company_name",1, "-- Select Company --", $cbo_company_name,"load_drop_down( 'requires/bh_commission_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );",'' ); ?>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                    <input type="text" name="txt_job_no" id="txt_job_no" style="width:120px" class="text_boxes"/>
                                    </td>
                                    <td>
