...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Yarn Purchase Requisition Follow Up Report", "../../", 1, 1,'',1,1);
?>	
<script>
	...
	function fn_report_generated(type)
	{
		...
		else
		{
			...
			freeze_window(3);
			http.open("POST","requires/yarn_purchase_requisition_followup_report_controller.php",true);
	...
	function open_febric_receive_status_order_wise_popup(order_id,type,color)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/yarn_purchase_requisition_followup_report_controller.php?order_id='+order_id+'&action='+type+'&color='+color, 'Detail Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../');
	}
	...
	{  
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/yarn_purchase_requisition_followup_report_controller.php?po_id='+po_id+'&start_date='+start_date+'&end_date='+end_date+'&buyer_id='+buyer_id+'&job_no='+job_no+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../');
	}
	...
	function openmypage_booking()
    {
        ...
        var page_link='requires/yarn_purchase_requisition_followup_report_controller.php?action='+action+'&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year='+cbo_year;
             ...
             <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr class="general">
                        <td> 
                            <?
                                echo create_drop_down( "cbo_company_name", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business in(1,3) $company_cond order by comp.company_name","id,company_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/yarn_purchase_requisition_followup_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' ); get_php_form_data(this.value,'load_variable_settings','requires/yarn_purchase_requisition_followup_report_controller');get_php_form_data(this.value,'print_button_variable_setting','requires/yarn_purchase_requisition_followup_report_controller' );" );
                            ?>
                        </td>
                        <td id="buyer_td">
                            <? 
                       ...
                       <td>
                         	<? 
                                ...
                                echo create_drop_down( "cbo_date_type", 120, $date_type_arr,"", 1, "-Select-", 0, "search_by(this.value,1);",0,"" );
                            ?>
                       </td>
                        <td>
