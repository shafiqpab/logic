...
if ($action == "generate_report")
{
	...
	else
	{
        ...
        //echo "<pre>"; print_r($rcv_trans_buyer_arr);
		...
		else if ($type == 5 || $type == 10)
		{
			...
			SUM(case when transaction_type in (1,5) then cons_quantity else 0 end) as mrr_total_recieved_quantity,
			...
			MAX(case when transaction_type in (1) then transaction_date else null end) as direct_mrr_recieved_last_date,
			MAX(case when transaction_type in (2) then transaction_date else null end) as direct_mrr_issue_last_date
	    	FROM inv_transaction
			...
			foreach ($mrr_info_sql as $row)
			{
				...
				$mrr_info_arr[$row[csf("prod_id")]]['direct_mrr_recieved_last_date']=$row[csf("direct_mrr_recieved_last_date")];
				$mrr_info_arr[$row[csf("prod_id")]]['mrr_total_recieved_quantity']=$row[csf("mrr_total_recieved_quantity")];
				$mrr_info_arr[$row[csf("prod_id")]]['direct_mrr_issue_last_date']=$row[csf("direct_mrr_issue_last_date")];
			}
			...
			<div>
				...
				<div style="width:2050px; overflow-y:scroll; max-height:220px" id="scroll_body" >
									...
									if (number_format($stockInHand, 2) > 0.00)
									{
										if (!in_array($check_string, $checkArr))
										{
											...
											if ($i > 1) {
												...
												$sub_stock_value_usd = 0;
												  ...
												  <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
													...
													<td width="100"><p><? echo $mrr_info_arr[$row[csf("id")]]['direct_mrr_recieved_last_date']; ?></p></td>
													<td width="100"><p><? echo $mrr_info_arr[$row[csf("id")]]['direct_mrr_issue_last_date']; ?></p></td>
													<td width="100"><p><? echo $mrr_info_arr[$row[csf("id")]]['mrr_total_recieved_quantity']; ?></p></td>
										  ...
										  else
										  {
											...
											$stock_value_usd = $stock_value / $exchange_rate;
											$stock_value_usd = (is_nan($stock_value_usd)==1 || is_nan($stock_value_usd)==1)?0:$stock_value_usd;
	...
	$subject = "Daily yarn stock summary Report";
	...
	if($_REQUEST['isview']==1){
		if($to){
			echo 'Mail Item:'.$form_list_for_mail[$mail_item].'=>'.$to."<br>";
		}else{
