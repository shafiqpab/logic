...
$action = $_REQUEST['action'];
	...
	$get_upto_qnty 		= 1;
	$txt_qnty 			= 1;
	$report_title 		= "Fabric Stock Ageing Report";
...
cbo_get_upto_qnty=1
txt_qnty=1
txt_date_from=06-Sep-2022
txt_date_to=06-Sep-2022*/
...
if ($action == "report_generate")
{
	...
	<div style="text-align:center;">
		<p style="font-size:20px;">
			Fabric Stock Ageing Report <br>
			<span style="font-size:12px;">Date: <?= change_date_format($txt_date_to); ?></span>
		</p>
	</div>
	...
	fnc_tempenginepo("GBL_TEMP_ENGINE", $user_id, 878, 1,$all_batch_id, $empty_arr);
	fnc_tempenginepo("GBL_TEMP_ENGINE", $user_id, 879, 1,$fso_id_arr, $empty_arr);
	oci_commit($con);
	...
	$batch_sql="SELECT a.mst_id,  a.item_description,  a.batch_qnty, a.body_part_id FROM pro_batch_create_dtls a, GBL_TEMP_ENGINE b Where status_active=1 and a.mst_id=b.ref_val and b.user_id=$user_id and b.entry_form=878 "; //$batch_conds 
	...
	if(!empty($fso_id_arr))
	{
		...
		$delivery_qnty_sql = sql_select("SELECT b.body_part_id bodypart_id,b.uom,b.width_type,sum(c.quantity) delivery_qnty, sum(a.order_amount) as amount, c.is_sales, c.po_breakdown_id, b.batch_id,d.detarmination_id determination_id,d.gsm,d.dia_width dia, $selectRcvStore d.color color_id, a.transaction_date
			...
			where a.company_id=$company_name and b.id=c.dtls_id and c.prod_id=d.id and c.trans_id = a.id  $rcvStoreCond and b.status_active=1 and c.entry_form=224 and c.status_active=1 and a.status_active=1 $to_trans_date_cond2 and c.po_breakdown_id=e.ref_val and e.user_id=$user_id and e.entry_form=879 group by b.body_part_id,b.uom, b.width_type, c.is_sales, c.po_breakdown_id, b.batch_id,d.detarmination_id,d.gsm, $groupByRcvStore d.dia_width,d.color,a.transaction_date"); //$all_fso_cond2 
		...
		$transfered_fabric_sql = sql_select("SELECT a.from_order_id as po_breakdown_id,b.batch_id, b.from_prod_id as prod_id, b.body_part_id, b.feb_description_id as fabric_description_id, $selectTransOutStore b.uom, d.color as color_id,b.fabric_shade,b.dia_width_type, b.dia_width as width, b.gsm, sum(b.transfer_qnty) as quantity, sum(c.order_amount) as amount, c.transaction_date
			...
			where a.id=b.mst_id and b.trans_id = c.id and c.prod_id=d.id and c.transaction_type=6 and a.entry_form in(230) and a.company_id = $company_name and a.from_order_id=e.ref_val and e.user_id=$user_id and e.entry_form=879 $to_trans_date_cond3 and a.status_active =1 and a.is_deleted =0 and b.status_active = 1 and b.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0
		...
		$rcv_return_sql = sql_select("SELECT c.po_breakdown_id, c.entry_form , c.quantity, c.is_sales, d.store_id, d.batch_id, e.detarmination_id, e.gsm, e.dia_width, d.body_part_id, d.width_type, e.color,d.uom, f.order_amount as amount, f.transaction_date
			...
			where c.dtls_id = d.id and d.prod_id = e.id and c.trans_id=f.id and c.entry_form = 287 and c.po_breakdown_id=b.ref_val and b.entry_form=879 and b.user_id=$user_id $to_trans_date_cond4 and c.is_sales=1 and e.item_category_id =2 and c.status_active =1 and c.is_deleted = 0 and d.status_active =1 and d.is_deleted = 0 and f.status_active =1 and f.is_deleted = 0"); //$all_fso_cond2
		...
		and b.pi_wo_batch_no=d.ref_val and d.user_id=$user_id and d.entry_form=878
	...
	//echo "Here";die;
		...
		foreach ($mainArrTex2 as $companyIDsTex => $companyDatasTex) 
		{
	 		...
	 		<div style="height: 250px;width: auto;float: left;">
	            <table border="1" rules="all" cellpadding="0" cellspacing="0" width="600" class="rpt_table" style="float: left; margin-right: 10px;margin-bottom: 10px;">
	...
	if(!empty($all_po_id_arr))
	{
		...
		fnc_tempenginepo("GBL_TEMP_ENGINE", $user_id, 910, 1,$all_po_id_arr, $empty_arr);//PO ID
	...
	if(!empty($batch_id_arr))
	{
		...
		*/
		...
		fnc_tempenginepo("GBL_TEMP_ENGINE", $user_id, 911, 1,$batch_id_arr, $empty_arr);//PO ID
    ...
    if(!empty($all_prod_id))
    {
		...
		*/
		...
		fnc_tempenginepo("GBL_TEMP_ENGINE", $user_id, 912, 1,$all_prod_id, $empty_arr);
				...
				foreach ($book_data as $prodStr => $row)
				{
					...
					if ((($get_upto == 1 && $ageOfDays > $txt_days) || ($get_upto == 2 && $ageOfDays < $txt_days) || ($get_upto == 3 && $ageOfDays >= $txt_days) || ($get_upto == 4 && $ageOfDays <= $txt_days) || ($get_upto == 5 && $ageOfDays == $txt_days) || $get_upto == 0) && (($get_upto_qnty == 1 && $stock_qnty > $txt_qnty) || ($get_upto_qnty == 2 && $stock_qnty < $txt_qnty) || ($get_upto_qnty == 3 && $stock_qnty >= $txt_qnty) || ($get_upto_qnty == 4 && $stock_qnty <= $txt_qnty) || ($get_upto_qnty == 5 && $stock_qnty == $txt_qnty) || $get_upto_qnty == 0))
					{
						...
						//{
							...
							else if($prodStrArr[1]==7 || $prodStrArr[1]==64 || $prodStrArr[1]==87 || $prodStrArr[1]==89)
							{
								$dataArr[4][$prodStrArr[7]][$ageOfDays]['stock_qntys']+=$stock_qnty;
								$dataArr[4][$prodStrArr[7]][$ageOfDays]['stock_amounts']+=$stock_amount;
							}

							if($prodStrArr[1]==133 || $prodStrArr[1]==98 || $prodStrArr[1]==97 || $prodStrArr[1]==95 || $prodStrArr[1]==96)
							{
	...
	echo "</pre>";
			*/
	$ageRange=array(1=> "1-30", 2=> "31-60", 3=> "61-90", 4=> "91-120", 5=> "121-150", 6=> "151-180", 7=> "Above 180");
	...
	foreach ($mainArr2 as $companyIDs => $companyDatas) 
	{
	   ...
	   /*foreach ($companyDatas as $ageKeyRange => $ageData) 
		{*/
	    foreach ($ageRange as $ageKeyRange => $ageData) 
		{
					...
					foreach ($comUomArr[$companyIDs] as $uomKey => $uomName) 
	 			 	{
	 			 		...
	 			 		$totalArrUomQnty[$companyIDs][$uomKey]+=ceil($mainArr[$companyIDs][$ageKeyRange][$uomKey]);
		...
		?>
	 		...
	 		<tfoot>
		 		<tr>
		 			...
		 			<th></th>
		 			<?
						foreach ($comUomArr[$companyIDs] as $uomKeys => $uomName) 
     			 		{
     			 			?>
		 					<th align="right"><? echo number_format($totalArrUomQnty[$companyIDs][$uomKeys]); ?></th>
		 					<?
		 				}
		 			
		 			?>
		 			<th align="right"><? echo number_format($totalArrAmount[$companyIDs],0); ?></th>
		...
		echo "</pre>";*/
	 	...
	 	<div style="height: 250px;width: auto; float: left;">
	        <table border="1" rules="all" cellpadding="0" cellspacing="0" width="500" class="rpt_table" style="float: left;margin-right: 10px;margin-bottom: 10px;">
	           <caption style="background-color:#f9f9f9; font-size: 16px; font-weight: bold;">Fabric Stock (Urmi-FTML)</caption>
	           <thead>
	              <th width="30">SL</th>
	              <th width="100">Age Days</th>
	              <?
	          ...
	          <?
				...
				{*/
				foreach ($ageRange as $ageKeyRangeStore => $ageDataStore) 
				{
				 		...
				 		<tr>
				 			...
				 			<?
								foreach ($comUomArrStore as $uomKeyStore => $uomNameStore) 
		     			 		{
		     			 			?>
				 					<th align="right"><? echo number_format($totalArrStore[$uomKeyStore],0); ?></th>
				 					<?
				 				}
				 			
				 			?>
				 			<th align="right"><? echo number_format($totalArrAmountStore,0); ?></th>
	...
	$subject="Fabric Stock Ageing Report";
	$header=mailHeader();
