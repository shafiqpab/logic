...
if ($action == "generate_report")
{
	...
	else
	{
		...
		else if ($type == 14)
		{
			...
			// from  product_details_master a, inv_receive_master b, inv_transaction c,lib_composition_array d  
			// where a.yarn_comp_type1st=d.id and a.id=c.prod_id and c.mst_id=b.id and a.item_category_id=1 and d.yarn_category_type >0 and b.source<>0 and c.status_active=1 and c.is_deleted=0 $company_cond $search_cond 
			// group by b.source, b.remarks, a.id, a.yarn_count_id, a.unit_of_measure, d.yarn_category_type 
			// order by d.yarn_category_type ";
	...
	$REAL_FILE_NAME = 'daily_yarn_stock_auto_mail_' . date('d-M-Y_h-iA',strtotime($previous_date)) . '.pdf';
