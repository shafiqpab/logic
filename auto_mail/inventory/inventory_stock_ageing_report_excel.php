...
$action = $_REQUEST['action'];
	...
	$job_year 			= 0;
	$cbo_company_id 	= $companyIDS;
	$company_name 		= 1; // only for textile
	$cbo_pay_mode 		= 0;
	$cbo_supplier_id 	= 0;
	$cbo_store_name 	= "48,82,83,86,125,126,127,128,88,85,84,49,18,47,80,81,116,117,110,111,7,64,87,89,96,95,97,98,133";
	...
	$get_upto_qnty 		= 0;
	$txt_qnty 			= "";
	$report_title 		= "Inventory Stock Ageing Report";
...
if ($action == "report_generate")
{
	...
	$con = connect();
	$r_id6=execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_id and entry_form in (878,879)");
    oci_commit($con);

	$sql = "SELECT b.id,d.within_group, f.id as batch_id, f.batch_no,f.extention_no,1 as type, sum(g.batch_qnty) as batch_qty, min(a.receive_date) as mrr_date, a.company_id, c.po_breakdown_id,  b.body_part_id, b.fabric_description_id, $selectRcvStore b.uom, f.color_id,b.dia_width_type, b.width, b.gsm, (c.quantity) as quantity , sum(e.cons_amount) as amount,0 as is_transfered,0 as from_order_id, a.receive_basis, (e.order_amount) as order_amount, e.transaction_date
	...
	group by  b.id,d.within_group,f.id, f.batch_no,f.extention_no,a.company_id,c.po_breakdown_id, b.body_part_id,b.fabric_description_id, $groupByRcvStore b.uom, e.order_amount, f.color_id, b.dia_width_type, b.width, a.item_category, b.gsm,c.quantity, a.receive_basis, e.transaction_date
	union all
	select  b.id,d.within_group,f.id as batch_id, f.batch_no,f.extention_no,2 as type, sum(g.batch_qnty) as batch_qty, min(a.transfer_date) as mrr_date, a.company_id,a.to_order_id as po_breakdown_id, g.body_part_id,b.feb_description_id as fabric_description_id, $selectTransStore b.uom, f.color_id,b.dia_width_type, b.dia_width as width, b.gsm, (b.transfer_qnty) as quantity , sum(e.cons_amount) as amount,1 as is_transfered,a.from_order_id , 0 as receive_basis,  (e.order_amount) as order_amount, e.transaction_date
	...
	group by  b.id,d.within_group,f.id,f.batch_no,f.extention_no,a.company_id,a.to_order_id, e.order_amount,b.from_prod_id, g.body_part_id, b.feb_description_id, $groupByTransStore b.uom, f.color_id, b.dia_width_type, b.dia_width, b.gsm,b.transfer_qnty,a.from_order_id, e.transaction_date
	...
	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 878, 1,$all_batch_id, $empty_arr);
	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 879, 1,$fso_id_arr, $empty_arr);
	oci_commit($con);
	...
	if(!empty($fso_id_arr))
	{
		...
		$delivery_qnty_sql = sql_select("SELECT b.body_part_id bodypart_id,b.uom,b.width_type,c.quantity delivery_qnty, a.order_amount as amount, c.is_sales, c.po_breakdown_id, b.batch_id,d.detarmination_id determination_id,d.gsm,d.dia_width dia, $selectRcvStore f.color_id, a.transaction_date
			from GBL_TEMP_ENGINE e,order_wise_pro_details c,inv_finish_fabric_issue_dtls b, inv_transaction a, product_details_master d, pro_batch_create_mst f
			where e.ref_val=c.po_breakdown_id and c.dtls_id=b.id and b.trans_id=a.id and a.prod_id=d.id and a.company_id=$company_name $rcvStoreCond and b.status_active=1 and c.entry_form=224 and c.status_active=1 and a.status_active=1 $to_trans_date_cond2 and c.po_breakdown_id=e.ref_val and e.user_id=$user_id and e.entry_form=889 and a.pi_wo_batch_no=f.id"); //$all_fso_cond2 
		...
		$issue_return_sql = sql_select("SELECT a.company_id, c.po_breakdown_id, b.prod_id, b.batch_id, b.body_part_id, b.fabric_description_id,  b.uom, f.color_id,b.dia_width_type, b.width, b.gsm, $selectRcvStore sum(c.quantity) as quantity , sum(e.order_amount) as amount, e.transaction_date from inv_receive_master a, pro_finish_fabric_rcv_dtls b, order_wise_pro_details c, inv_transaction e, pro_batch_create_mst f, GBL_TEMP_ENGINE d where a.id=b.mst_id and b.id=c.dtls_id and c.trans_id=e.id and e.pi_wo_batch_no=f.id and a.entry_form=233 and c.entry_form=233 and b.is_sales=1 and c.is_sales=1 and a.company_id=$company_name  $rcvStoreCond $to_trans_date_cond and c.po_breakdown_id=d.ref_val and d.user_id=$user_id and d.entry_form=889 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by a.company_id,c.po_breakdown_id, b.prod_id, b.batch_id, b.body_part_id, b.fabric_description_id,b.uom, f.color_id, $groupByRcvStore b.dia_width_type, b.width, a.item_category, b.gsm, e.transaction_date"); //$all_fso_cond2
		...
		$transfered_fabric_sql = sql_select("SELECT a.from_order_id as po_breakdown_id,b.batch_id, b.from_prod_id as prod_id, b.body_part_id, b.feb_description_id as fabric_description_id, $selectTransOutStore b.uom, d.color_id,b.fabric_shade,b.dia_width_type, b.dia_width as width, b.gsm, b.transfer_qnty as quantity, c.order_amount as amount, c.transaction_date
			from inv_item_transfer_mst a, inv_item_transfer_dtls b, inv_transaction c, pro_batch_create_mst d, GBL_TEMP_ENGINE e
			where a.id=b.mst_id and b.trans_id = c.id and c.pi_wo_batch_no=d.id and c.transaction_type=6 and a.entry_form in(230) and a.company_id = $company_name and a.from_order_id=e.ref_val and e.user_id=$user_id and e.entry_form=889 $to_trans_date_cond3 and a.status_active =1 and a.is_deleted =0 and b.status_active = 1 and b.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0"); //$all_fso_cond3
		...
		$rcv_return_sql = sql_select("SELECT c.po_breakdown_id, c.entry_form , c.quantity, c.is_sales, d.store_id, d.batch_id, e.detarmination_id, e.gsm, e.dia_width, d.body_part_id, d.width_type, g.color_id as color,d.uom, f.order_amount as amount, f.transaction_date
			from order_wise_pro_details c, inv_finish_fabric_issue_dtls d, product_details_master e, inv_transaction f, GBL_TEMP_ENGINE b, pro_batch_create_mst g
			where c.dtls_id = d.id and d.prod_id = e.id and c.trans_id=f.id and c.entry_form = 287 and c.po_breakdown_id=b.ref_val and b.entry_form=889 and b.user_id=$user_id $to_trans_date_cond4 and c.is_sales=1 and e.item_category_id =2 and c.status_active =1 and c.is_deleted = 0 and d.status_active =1 and d.is_deleted = 0 and f.status_active =1 and f.is_deleted = 0 and d.batch_id=g.id"); //$all_fso_cond2
		...
		from GBL_TEMP_ENGINE d ,order_wise_pro_details c,inv_transaction b, product_details_master a
		where d.entry_form=888 and d.user_id=1  and d.ref_val=b.pi_wo_batch_no and c.trans_id=b.id and b.prod_id=a.id and b.is_deleted=0 and b.status_active=1 
		and b.item_category=2 and b.transaction_type in (2,6) and c.trans_type in (2,6) and b.pi_wo_batch_no=d.ref_val
	...
	execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_id and entry_form in (878,879)");
	oci_commit($con);
	//echo "Here";die;
	...
	ob_start();
			...
			foreach ($source_arr as $uom_id => $uom_data)
			{
				...
				foreach ($uom_data as $po_breakdown_id => $po_breakdown_data)
				{
					...
					foreach ($po_breakdown_data as $prod_ref => $row)
					{
						...
						if ((($cbo_get_upto_qnty == 1 && $total_stock_qty > $txt_qnty_val) || ($cbo_get_upto_qnty == 2 && $total_stock_qty < $txt_qnty_val) || ($cbo_get_upto_qnty == 3 && $total_stock_qty >= $txt_qnty_val) || ($cbo_get_upto_qnty == 4 && $total_stock_qty <= $txt_qnty_val) || ($cbo_get_upto_qnty == 5 && $total_stock_qty == $txt_qnty_val) || $cbo_get_upto_qnty == 0) && (($cbo_get_upto == 1 && $daysOnHand > $txt_days) || ($cbo_get_upto == 2 && $daysOnHand < $txt_days) || ($cbo_get_upto == 3 && $daysOnHand >= $txt_days) || ($cbo_get_upto == 4 && $daysOnHand <= $txt_days) || ($cbo_get_upto == 5 && $daysOnHand == $txt_days) || $cbo_get_upto == 0))
						{
							...
							if($uom_id!=1)
							{
								$dataArrTex[$company_id][$uom_id][$ageOfDays]['stock_qntys']+=$total_stock_qty;
								$dataArrTex[$company_id][$uom_id][$ageOfDays]['stock_amounts']+=$total_stock_amount;
							}
						}
					}
				}
		...
		foreach ($mainArrTex2 as $companyIDsTex => $companyDatasTex) 
		{
	 		...
	 		<div style="height: 250px;width: auto;float: left;">
	            <table border="1" rules="all" cellpadding="0" cellspacing="0" width="500" class="rpt_table" style="float: left; margin-right: 10px;margin-bottom: 10px;">
	               	<caption style="background-color:#f9f9f9; font-size: 16px; font-weight: bold;">Fabric Stock(<? if($companyIDsTex==0 || $companyIDsTex==""){ echo "Unknown Company";}else{echo $short_company_arr[$companyIDsTex];} ?>-Textile)</caption>
	               <thead>
	                  <th width="30">SL</th>
								...
								foreach ($comUomArrTex[$companyIDsTex] as $uomKeyTex => $uomNameTex) 
		     			 		{
				 					...
				 					<th align="right"><? echo number_format($totalArr[$companyIDsTex][$uomKeyTex],2); ?></th>

				 					<?
				 				}
				 			
				 		?>
				 			<th align="right"><? echo number_format($totalArrAmountTex[$companyIDsTex],2); ?></th>
				 		</tr>
			 		</tfoot>
	   			</table>
			</div>
	...
	if(!empty($data_array))	
	{
		...
		$r_id6=execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_id and entry_form in (910,911,912)");
		if($r_id3 && $r_id6)
		{
			oci_commit($con);
		}
	...
	if(!empty($all_po_id_arr))
	{
		...
		fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 910, 1,$all_po_id_arr, $empty_arr);//PO ID
	...
	if(!empty($batch_id_arr))
	{
		...
		*/
		...
		fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 911, 1,$batch_id_arr, $empty_arr);//PO ID
	...
	$issue_sql = sql_select("select a.knit_dye_source, a.issue_purpose, c.prod_id, b.body_part_id, c.cons_uom, c.store_id, $issue_select c.cons_quantity, c.id as trans_id,c.transaction_date, d.detarmination_id, d.gsm, d.dia_width, d.color, e.booking_no, e.booking_without_order, round(c.order_rate,2) as order_rate from inv_issue_master a, inv_finish_fabric_issue_dtls b, inv_transaction c, product_details_master d, pro_batch_create_mst e, GBL_TEMP_ENGINE g  where a.id = b.mst_id and b.trans_id = c.id and c.prod_id = d.id and c.pi_wo_batch_no= e.id and c.company_id in ($cbo_company_id) $store_cond_2 $date_cond_2 and e.id=g.ref_val and g.user_id=$user_id and g.entry_form=911 and a.entry_form=18 and c.status_active=1 and b.status_active=1 and a.status_active=1 and c.item_category =2 and c.transaction_type =2 group by a.knit_dye_source, a.issue_purpose, c.prod_id, b.body_part_id, c.cons_uom, c.store_id, $issue_group c.cons_quantity, c.id, c.transaction_date, d.detarmination_id, d.gsm, d.dia_width, d.color, e.booking_no, e.booking_without_order, round(c.order_rate,2)");
	...
	$rcvRtnSql = sql_select("select c.transaction_date, c.company_id, c.prod_id, c.store_id, $rcv_return_select c.cons_quantity, c.cons_uom, d.detarmination_id, d.gsm, d.dia_width, d.color, e.booking_no, e.booking_without_order, b.body_part_id from inv_issue_master a, inv_finish_fabric_issue_dtls b, inv_transaction c, product_details_master d, pro_batch_create_mst e, GBL_TEMP_ENGINE g where a.id = b.mst_id and b.trans_id=c.id and a.entry_form =46 and c.company_id in ($cbo_company_id) $store_cond_2 $date_cond_2 and e.id=g.ref_val and g.user_id=$user_id and g.entry_form=911 and c.prod_id=d.id and c.pi_wo_batch_no=e.id and a.status_active =1 and b.status_active =1 and c.status_active =1");
	...
	$transOutSql = sql_select("select c.transaction_date,c.pi_wo_batch_no, e.batch_no,e.booking_no, e.booking_no_id, e.booking_without_order, c.body_part_id, c.prod_id, c.store_id, $trans_out_select d.detarmination_id, d.gsm, d.dia_width, d.color, c.cons_uom, c.cons_quantity,c.order_rate from inv_item_transfer_mst a, inv_item_transfer_dtls b, inv_transaction c,product_details_master d, pro_batch_create_mst e, GBL_TEMP_ENGINE g where a.id=b.mst_id and b.trans_id=c.id and c.prod_id=d.id and c.pi_wo_batch_no=e.id and c.company_id in ($cbo_company_id) $store_cond_2 $date_cond_2  and e.id=g.ref_val and g.user_id=$user_id and g.entry_form=911 and c.item_category=2 and c.transaction_type=6 and a.status_active=1 and b.status_active=1 and c.status_active=1 and a.entry_form in (14,15,306)");
	...
	if(!empty($all_po_id_arr))
	{
		...
		where a.job_no = c.job_no and b.job_no=c.job_no and c.id = b.pre_cost_fabric_cost_dtls_id and c.fab_nature_id=2 and c.status_active =1 and b.status_active=1 and c.color_size_sensitive !=3 and b.po_break_down_id=g.ref_val and g.user_id=$user_id and g.entry_form=910 
    ...
    if(!empty($all_prod_id))
    {
		...
		*/
		...
		fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 912, 1,$all_prod_id, $empty_arr);
    	...
    	//if($all_prod_id_cond!=""){
		if(!empty($all_prod_id)){
    		$sql_date="SELECT c.booking_no, a.prod_id, min(a.transaction_date) as min_date, max(a.transaction_date) as max_date from inv_transaction a,pro_batch_create_mst c, GBL_TEMP_ENGINE g where a.pi_wo_batch_no=c.id and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.item_category=2 and a.prod_id=g.ref_val and g.user_id=$user_id and g.entry_form=912 group by c.booking_no,a.prod_id"; //$all_prod_id_cond
				...
				foreach ($book_data as $prodStr => $row)
				{
					...
					if ((($get_upto == 1 && $ageOfDays > $txt_days) || ($get_upto == 2 && $ageOfDays < $txt_days) || ($get_upto == 3 && $ageOfDays >= $txt_days) || ($get_upto == 4 && $ageOfDays <= $txt_days) || ($get_upto == 5 && $ageOfDays == $txt_days) || $get_upto == 0) && (($get_upto_qnty == 1 && $stock_qnty > $txt_qnty) || ($get_upto_qnty == 2 && $stock_qnty < $txt_qnty) || ($get_upto_qnty == 3 && $stock_qnty >= $txt_qnty) || ($get_upto_qnty == 4 && $stock_qnty <= $txt_qnty) || ($get_upto_qnty == 5 && $stock_qnty == $txt_qnty) || $get_upto_qnty == 0))
					{
						...
						//{
							...
							//$dataArrAmount[$company_name][$prodStrArr[7]][$ageOfDays]+=$stock_amount;


							//$dataArr[$company_name][$prodStrArr[7]][$ageOfDays]['stock_qntys']+=$stock_qnty;
							//$dataArr[$company_name][$prodStrArr[7]][$ageOfDays]['stock_amounts']+=$stock_amount;

							if($prodStrArr[1]==48 || $prodStrArr[1]==82 || $prodStrArr[1]==83 || $prodStrArr[1]==86 )
							{
								$dataArr[1][$prodStrArr[7]][$ageOfDays]['stock_qntys']+=$stock_qnty;
								$dataArr[1][$prodStrArr[7]][$ageOfDays]['stock_amounts']+=$stock_amount;
							}

							else if($prodStrArr[1]==125 || $prodStrArr[1]==126 || $prodStrArr[1]==127 || $prodStrArr[1]==128 || $prodStrArr[1]==88 || $prodStrArr[1]==85 || $prodStrArr[1]==84 || $prodStrArr[1]==49)
							{
								$dataArr[2][$prodStrArr[7]][$ageOfDays]['stock_qntys']+=$stock_qnty;
								$dataArr[2][$prodStrArr[7]][$ageOfDays]['stock_amounts']+=$stock_amount;
							}
							else if($prodStrArr[1]==18 || $prodStrArr[1]==47 || $prodStrArr[1]==80 || $prodStrArr[1]==81 || $prodStrArr[1]==116 || $prodStrArr[1]==117 || $prodStrArr[1]==110 || $prodStrArr[1]==111)
							{
								$dataArr[3][$prodStrArr[7]][$ageOfDays]['stock_qntys']+=$stock_qnty;
								$dataArr[3][$prodStrArr[7]][$ageOfDays]['stock_amounts']+=$stock_amount;
							}
							else if($prodStrArr[1]==7 || $prodStrArr[1]==64 || $prodStrArr[1]==87 || $prodStrArr[1]==89 || $prodStrArr[1]==96 )
							{
								$dataArr[4][$prodStrArr[7]][$ageOfDays]['stock_qntys']+=$stock_qnty;
								$dataArr[4][$prodStrArr[7]][$ageOfDays]['stock_amounts']+=$stock_amount;
							}

							if($prodStrArr[1]==133 || $prodStrArr[1]==98 || $prodStrArr[1]==97 || $prodStrArr[1]==95 )
							{
								$dataArrStore[$prodStrArr[7]][$ageOfDays]['store_wise_stock_qntys']+=$stock_qnty;
								$dataArrStore[$prodStrArr[7]][$ageOfDays]['store_wise_stock_amounts']+=$stock_amount;	
							}
						//}
					}
				}
	...
	foreach ($dataArr as $companyID => $companyData) 
	{
		foreach ($companyData as $uomID => $uomData) 
		{
			foreach ($uomData as $ageKey => $value) 
			{
				...
				if($ageKey<=30)
				{
					$mainArr[$companyID][1][$uomID]+=$value['stock_qntys'];
					$mainArr2[$companyID][1]+=$value['stock_qntys'];
					$mainArr3[$companyID][1]+=$value['stock_amounts'];
				}
				else if($ageKey>30 && $ageKey<=60)
				{
					$mainArr[$companyID][2][$uomID]+=$value['stock_qntys'];
					$mainArr2[$companyID][2]+=$value['stock_qntys'];
					$mainArr3[$companyID][2]+=$value['stock_amounts'];
				}
				else if($ageKey>60 && $ageKey<=90)
				{
					$mainArr[$companyID][3][$uomID]+=$value['stock_qntys'];
					$mainArr2[$companyID][3]+=$value['stock_qntys'];
					$mainArr3[$companyID][3]+=$value['stock_amounts'];
				}
				else if($ageKey>90 && $ageKey<=120)
				{
					$mainArr[$companyID][4][$uomID]+=$value['stock_qntys'];
					$mainArr2[$companyID][4]+=$value['stock_qntys'];
					$mainArr3[$companyID][4]+=$value['stock_amounts'];
				}
				else if($ageKey>120 && $ageKey<=150)
				{
					$mainArr[$companyID][5][$uomID]+=$value['stock_qntys'];
					$mainArr2[$companyID][5]+=$value['stock_qntys'];
					$mainArr3[$companyID][5]+=$value['stock_amounts'];
				}

				else if($ageKey>150 && $ageKey<=180)
				{
					$mainArr[$companyID][6][$uomID]+=$value['stock_qntys'];
					$mainArr2[$companyID][6]+=$value['stock_qntys'];
					$mainArr3[$companyID][6]+=$value['stock_amounts'];
				}
				else if($ageKey>180)
				{
					$mainArr[$companyID][7][$uomID]+=$value['stock_qntys'];
					$mainArr2[$companyID][7]+=$value['stock_qntys'];
					$mainArr3[$companyID][7]+=$value['stock_amounts'];
				}
			}
		}
	}
	...
	foreach ($mainArr2 as $companyIDs => $companyDatas) 
	{
		 ...
		 ?>		<div style="height: 250px;width: auto; float: left;">
	            <table border="1" rules="all" cellpadding="0" cellspacing="0" width="500" class="rpt_table" style="float: left;margin-right: 10px;margin-bottom: 10px;">
	               <caption style="background-color:#f9f9f9; font-size: 16px; font-weight: bold;">Fabric Stock(<? if($companyIDs==0 || $companyIDs==""){ echo "Unknown Company";}else{echo $short_company_arr[$companyIDs];} ?>-Garments)</caption>
	               <thead>
	                  <th width="30">SL</th>
	                  <th width="100">Age Days</th>
	                  <?
				...
				foreach ($comUomArr[$companyIDs] as $uomKey => $uomName) 
 			 	{
 			 		...
 			 		$totalArr[$companyIDs][$uomKey]+=ceil($mainArr[$companyIDs][$ageKeyRange][$uomKey]);
 			 		
					?>
				 		<td align="right"><? 
				 			$ageWiseQnty=ceil($mainArr[$companyIDs][$ageKeyRange][$uomKey]);
				 			echo number_format($ageWiseQnty); 
				 			//number_format($mainArr[$companyIDs][$ageKeyRange][$uomKey],2); ?>
				 		</td>
					<?
				}
			?>
				<td align="right">
					<? 
					$ageWiseAmount= ceil($mainArr3[$companyIDs][$ageKeyRange]); 
					echo number_format($ageWiseAmount);
					$totalArrAmount[$companyIDs]+=ceil($mainArr3[$companyIDs][$ageKeyRange]);
					?>
		 		...
		 		<tr>
		 			...
		 			<?
						foreach ($comUomArr[$companyIDs] as $uomKey => $uomName) 
     			 		{
     			 			?>
		 					<th align="right"><? echo number_format($totalArr[$companyIDs][$uomKey]); ?></th>
		 					<?
		 				}
		 			
		 			?>
		 			<th align="right"><? echo number_format($totalArrAmount[$companyIDs],2); ?></th>
		...
		// This part only for 4 store wise all company data

		foreach ($dataArrStore as $uomIDstore => $uomDataStore) 
		{
			foreach ($uomDataStore as $ageKeyStore => $valueStore) 
			{

				$comUomArrStore[$uomIDstore]=$uomIDstore;
				if($ageKeyStore<=30)
				{
					$mainArrStore[1][$uomIDstore]+=$valueStore['store_wise_stock_qntys'];
					$mainArr2Store[1]+=$valueStore['store_wise_stock_qntys'];
					$mainArr3Store[1]+=$valueStore['store_wise_stock_amounts'];
				}
				else if($ageKeyStore>30 && $ageKeyStore<=60)
				{
					$mainArrStore[2][$uomIDstore]+=$valueStore['store_wise_stock_qntys'];
					$mainArr2Store[2]+=$valueStore['store_wise_stock_qntys'];
					$mainArr3Store[2]+=$valueStore['store_wise_stock_amounts'];
				}
				else if($ageKeyStore>60 && $ageKeyStore<=90)
				{
					$mainArrStore[3][$uomIDstore]+=$valueStore['store_wise_stock_qntys'];
					$mainArr2Store[3]+=$valueStore['store_wise_stock_qntys'];
					$mainArr3Store[3]+=$valueStore['store_wise_stock_amounts'];
				}
				else if($ageKeyStore>90 && $ageKeyStore<=120)
				{
					$mainArrStore[4][$uomIDstore]+=$valueStore['store_wise_stock_qntys'];
					$mainArr2Store[4]+=$valueStore['store_wise_stock_qntys'];
					$mainArr3Store[4]+=$valueStore['store_wise_stock_amounts'];
				}
				else if($ageKeyStore>120 && $ageKeyStore<=150)
				{
					$mainArrStore[5][$uomIDstore]+=$valueStore['store_wise_stock_qntys'];
					$mainArr2Store[5]+=$valueStore['store_wise_stock_qntys'];
					$mainArr3Store[5]+=$valueStore['store_wise_stock_amounts'];
				}

				else if($ageKeyStore>150 && $ageKeyStore<=180)
				{
					$mainArrStore[6][$uomIDstore]+=$valueStore['store_wise_stock_qntys'];
					$mainArr2Store[6]+=$valueStore['store_wise_stock_qntys'];
					$mainArr3Store[6]+=$valueStore['store_wise_stock_amounts'];
				}
				else if($ageKeyStore>180)
				{
					$mainArrStore[7][$uomIDstore]+=$valueStore['store_wise_stock_qntys'];
					$mainArr2Store[7]+=$valueStore['store_wise_stock_qntys'];
					$mainArr3Store[7]+=$valueStore['store_wise_stock_amounts'];
				}
			}
		}
		...
		echo "</pre>";*/
	 	...
	 	<div style="height: 250px;width: auto; float: left;">
	        <table border="1" rules="all" cellpadding="0" cellspacing="0" width="500" class="rpt_table" style="float: left;margin-right: 10px;margin-bottom: 10px;">
	           <caption style="background-color:#f9f9f9; font-size: 16px; font-weight: bold;">Fabric Stock(Urmi-FTML)Stock</caption>
	           <thead>
	              <th width="30">SL</th>
	              <th width="100">Age Days</th>
	              <?
	             foreach ($comUomArrStore as $uomKeyStore => $uomNameStore) 
	 			 {
	                     ?>
	                     <th width="100">Stock(<? echo $unit_of_measurement[$uomNameStore]; ?>)</th>
	                     <?
	              }
	              ?>
	              <th width="100">Total Value</th>
	          ...
	          <?
				...
				foreach ($mainArr2Store as $ageKeyRangeStore => $ageDataStore) 
				{

					?>
					<tr>
		                 <td><? echo $i; ?></td>
		                 <td><? echo $ageRange[$ageKeyRangeStore]; ?></td>


		                 <?
						foreach ($comUomArrStore as $uomKeyStore => $uomNameStore) 
		 			 	{
		 			 		$totalArrStore[$uomKeyStore]+=ceil($mainArrStore[$ageKeyRangeStore][$uomKeyStore]);
		 			 		
							?>
						 		<td align="right">

						 			<? 
						 			$ageWiseExQnty=ceil($mainArrStore[$ageKeyRangeStore][$uomKeyStore]);
						 			echo  number_format($ageWiseExQnty);
						 			//echo number_format($mainArrStore[$ageKeyRangeStore][$uomKeyStore],2); 
					...
					?>
						<td align="right">
							<? 
							$ageWiseExAmount=ceil($mainArr3Store[$ageKeyRangeStore]);
							echo number_format($ageWiseExAmount);
							//echo number_format($mainArr3Store[$ageKeyRangeStore],2); 
							$totalArrAmountStore+=ceil($mainArr3Store[$ageKeyRangeStore]);
							?>
				 		...
				 		<tr>
				 			...
				 			<?
								foreach ($comUomArrStore as $uomKeyStore => $uomNameStore) 
		     			 		{
		     			 			?>
				 					<th align="right"><? echo number_format($totalArrStore[$uomKeyStore],2); ?></th>
				 					<?
				 				}
				 			
				 			?>
				 			<th align="right"><? echo number_format($totalArrAmountStore,2); ?></th>
		...
		//END This part only for 4 store wise all company data
	...
	$to='';
	$sql = "SELECT a.BRAND_IDS,a.BUYER_IDS,c.email_address FROM mail_group_mst a, mail_group_child b, user_mail_address c where b.mail_group_mst_id=a.id  and a.mail_item=b.MAIL_ITEM_MST and a.mail_item=105 and b.mail_user_setup_id=c.id and a.company_id in($cbo_company_id)  and A.IS_DELETED=0 and A.STATUS_ACTIVE=1 AND a.MAIL_TYPE=1 and b.IS_DELETED=0 and b.STATUS_ACTIVE=1  and c.IS_DELETED=0 and c.STATUS_ACTIVE=1";
	// echo $sql;die;
	...
	$subject="Inventory Stock Ageing Report";
	$header=mailHeader();
