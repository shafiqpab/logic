...
extract($_REQUEST);
	...
	$sql="  SELECT a.WO_NUMBER, a.SUPPLIER_ID,a.INSERTED_BY, c.APPROVAL_CAUSE FROM wo_non_order_info_mst a,  wo_non_order_info_dtls b,  FABRIC_BOOKING_APPROVAL_CAUSE c WHERE a.id = b.mst_id  AND a.id = c.booking_id AND b.item_category_id = 1  AND a.status_active = 1 AND a.is_deleted = 0 AND b.status_active = 1 AND b.is_deleted = 0  AND c.entry_form = 2  AND a.entry_form = 144 AND a.id IN ($sysId) GROUP BY a.WO_NUMBER, a.SUPPLIER_ID,a.INSERTED_BY, c.APPROVAL_CAUSE";

	$sql = "SELECT c.EMAIL_ADDRESS FROM mail_group_mst a, mail_group_child b, user_mail_address c where b.mail_group_mst_id=a.id and a.mail_item=66 and a.MAIL_TYPE in(2,0) and b.mail_user_setup_id=c.id and a.company_id=$company_name AND a.MAIL_TYPE=1 and c.IS_DELETED=0 and c.STATUS_ACTIVE=1";
