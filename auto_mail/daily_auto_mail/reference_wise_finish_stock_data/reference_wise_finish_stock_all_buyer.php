...
{ 	
	...
	$cbo_store_name= '49,85,88,95,97,7,87,89,64,84,96,98,18,47,80,81,48,82,83,86';
	...
	foreach ($rcv_data as  $val)
	{
		...
		if($val['SALES_ORDER_ID'])$salse_order_booking_arr[$val['SALES_ORDER_ID']]=$val['SALES_ORDER_ID'];
	...
	if ($hdn_pi_id=="")
	{
		...
		group by c.transaction_date, c.pi_wo_batch_no, e.batch_no,e.booking_no, e.booking_no_id, e.booking_without_order, c.company_id, c.body_part_id, c.prod_id,c.store_id, $trans_in_group d.detarmination_id, d.gsm, d.dia_width, d.color, c.cons_uom, c.order_rate, c.order_amount ,e.SALES_ORDER_ID  order by c.company_id";
	...
	if(!empty($all_po_id_arr))
	{
		...
		fnc_tempenginepo("GBL_TEMP_ENGINE", $user_id, 990, 1,$all_po_id_arr, $empty_arr);//PO ID
  ...
  //  $floor_room_rack_arr = return_library_array("select floor_room_rack_id, floor_room_rack_name from lib_floor_room_rack_mst where status_active =1 and is_deleted=0","floor_room_rack_id","floor_room_rack_name");
	...
	$salseSql = "select SALES_BOOKING_NO,BUYER_ID from FABRIC_SALES_ORDER_MST where WITHIN_GROUP=2". where_con_using_array($salse_order_booking_arr,0,'id');
	echo $salseSql;
	$salseSqlRes=sql_select($salseSql);
	foreach( $salseSqlRes as $row )
	{
		$book_po_ref[$row['SALES_BOOKING_NO']]["buyer_name"]=$row['BUYER_ID'];
	}
	unset($salseSqlRes);
				...
				foreach ($data_array as $uom => $uom_data)
				{
					...
					foreach ($uom_data as $booking_no => $book_data)
					{
						foreach ($book_data as $prodStr => $row)
						{
							...
							// if($book_po_ref[$booking_no]["buyer_name"]==''){
							// 	echo $booking_no."', '";
							// }
	...
	foreach($ftml_data_arr->qty as $buyer_id => $buyer_data_arr){
		foreach($buyer_data_arr as $uom => $qty){
			$buyerTotalStockQtyArr[$buyer_arr[$buyer_id]][$uom]=$qty;
			$umQtyArr[$uom]+=$qty;
		}
	}

	echo "<pre>";
	print_r($buyerTotalStockQtyArr);
