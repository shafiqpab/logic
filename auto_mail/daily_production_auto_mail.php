...
//Kniting Production--------------------------------------                            
	$str_cond_f	=" and a.receive_date between '".$prev_date."' and '".$prev_date."'";
	 $sql_qty="select sum(c.quantity) as qtyinhouse,1 as TYPE 
  ...
  union all
     select sum(b.PRODUCT_QNTY) as qtyinhouse,2 as type  from subcon_production_mst a, subcon_production_dtls b where a.id=b.mst_id  and a.product_type in(2) and b.product_type in(2)  and a.product_date between '".$prev_date."' and '".$prev_date."'   and a.is_deleted=0 and a.status_active=1  and b.is_deleted=0 and b.status_active=1	 
  union all
	SELECT sum(case when a.booking_without_order=1 then b.grey_receive_qnty end ) as qtyinhouse,2 as TYPE from inv_receive_master a, pro_grey_prod_entry_dtls b where a.entry_form=2 and a.item_category=13 and a.id=b.mst_id and a.receive_basis!=4 and a.buyer_id>0    and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.booking_without_order=1  AND a.receive_date BETWEEN  '".$prev_date."' and '".$prev_date."'";//and a.ENTRY_FORM=159 
	     //echo $sql_qty;die;
	 
	$sql_qty="SELECT a.receive_date, a.KNITTING_SOURCE, sum(b.grey_receive_qnty) as PRODUCTION_QTY from inv_receive_master a, pro_grey_prod_entry_dtls b where a.id=b.mst_id and a.entry_form=2 and a.status_active=1 and b.status_active=1 $str_cond_f group by a.receive_date, a.knitting_source";
	 
	$sql_result=sql_select( $sql_qty, '', '', '', $con);
	$kniting_pro_qty=0;
	foreach($sql_result as $row)
	{
		$kniting_pro_qty += $row['PRODUCTION_QTY'];
		$kniting_pro_qty_arr[$row['KNITTING_SOURCE']] += $row['PRODUCTION_QTY'];
	}				
	unset($sql_result);
...
?>
	...
	<table cellspacing="0" cellpadding="1" border="1" rules="all">
        ...
        <table cellspacing="0" cellpadding="1" border="1" rules="all">
            ...
            <tr><td>Knitting (Kg) In House</td><td align="right"><?= number_format($kniting_pro_qty_arr[1],2);?></td></tr>
            <tr><td>Knitting (Kg) Out Bound</td><td align="right"><?= number_format($kniting_pro_qty_arr[3],2);?></td></tr>
