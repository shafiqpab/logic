...
//$company_library=array(2=>$company_library[2]);
        ...
        <tr>
            <td valign="top" align="left">
                 <table width="80%" cellpadding="0" cellspacing="0" rules="all" border="1">
                 	<tr>
                 		<td colspan="7" height="30" align="center"><strong>Dyeing Machine Loaded</strong></td>
                 	</tr>
                    ...
                    <?
						...
						if($is_insert_date==0){
							$str_cond_a=" and a.process_end_date between '$previous_date' and '$current_date'";
						}
						
						$i=0;
						
                    	$sql_dyeing="select e.buyer_name,a.service_source,a.load_unload_id,
						b.batch_weight from pro_fab_subprocess a, pro_batch_create_mst b,pro_batch_create_dtls c,wo_po_break_down d, wo_po_details_master e	 where b.id=a.batch_id and b.id=c.mst_id and c.po_id=d.id and e.job_no=d.job_no_mst and a.company_id=$compid and a.entry_form=35 and a.load_unload_id in(2,1) and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $str_cond_a group by b.batch_weight,e.buyer_name,a.service_source,a.load_unload_id order by a.load_unload_id";
						
						$dayDataArr=array();$daying_data_arr=array();
						$nameArray_dyeing=sql_select($sql_dyeing);	
						$tot_rows23=count($nameArray_dyeing);		
						foreach($nameArray_dyeing as $row)
						{
							
							$dayDataArr[$row[csf('load_unload_id')]][$row[csf('buyer_name')]][$row[csf('service_source')]]+=$row[csf('batch_weight')];
							$daying_data_arr[$row[csf('load_unload_id')]][$row[csf('buyer_name')]]=$row[csf('buyer_name')];
						...
						if($is_insert_date==0){
							$str_cond_a=" and a.process_end_date between '$previous_date' and '$current_date'";
							$str_cond_b=" and f.process_end_date between '$previous_date' and '$current_date'";
						}
						
						 $sql_dyeing_sub="select e.buyer_name,a.service_source,a.load_unload_id,
						 b.batch_weight from  pro_fab_subprocess a, pro_batch_create_mst b,pro_batch_create_dtls c,wo_po_break_down d, wo_po_details_master e  where b.id=a.batch_id and b.id=c.mst_id and c.po_id=d.id and e.job_no=d.job_no_mst and a.company_id=$compid and a.entry_form=35 and a.load_unload_id in(2,1) and a.service_source=1 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $str_cond_a group by b.batch_weight,e.buyer_name,a.service_source,a.load_unload_id order by a.load_unload_id";	

						

						// echo $sql_dyeing_sub;				
						$nameArray_dyeing=sql_select($sql_dyeing_sub);
						$tot_rows24=count($nameArray_dyeing);
						$subDayDataArr=array();$daying_data_arr=array();
						foreach($nameArray_dyeing as $row)
						{
							
							$subDayDataArr[$row[csf('load_unload_id')]][$row[csf('buyer_name')]][$row[csf('service_source')]]+=$row[csf('batch_weight')];
							$daying_data_arr[$row[csf('load_unload_id')]][$row[csf('buyer_name')]]=$row[csf('buyer_name')];

						}

						 $subcon_sql="select  SUM(distinct b.batch_qnty) AS sub_batch_qnty,d.party_id as buyer_name,f.load_unload_id,a.company_id  from pro_batch_create_dtls b,subcon_ord_dtls c, subcon_ord_mst d, pro_batch_create_mst a, pro_fab_subprocess f,lib_machine_name g where a.batch_against in(1,2) and a.company_id=$compid and f.batch_id=a.id and f.batch_id=b.mst_id and a.entry_form=36 and g.id=f.machine_id and a.id=b.mst_id and f.entry_form=38 and f.load_unload_id=2 and b.po_id=c.id and d.subcon_job=c.job_no_mst and b.status_active=1  and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0 $str_cond_b and f.service_source in(1)  GROUP BY  b.prod_id,d.party_id ,f.load_unload_id,a.company_id";
						 $subcon_data=sql_select($subcon_sql);

						 	foreach($subcon_data as $val){

						 			$subcon_buyer_wise_qty[$val[csf('company_id')]][$val[csf('buyer_name')]][$val[csf('load_unload_id')]]+=$val[csf('sub_batch_qnty')];
									 $daying_data_arr[$val[csf('load_unload_id')]][$val[csf('buyer_name')]]=$val[csf('buyer_name')];
						 	}

							//  echo "<pre>";
							//   print_r($subcon_buyer_wise_qty);  


							$self_con_sql="select f.load_unload_id,sum(distinct t.production_qty) as production_qty,d.buyer_name,a.company_id,a.batch_against from pro_batch_create_mst a,pro_batch_create_dtls b,wo_po_break_down c, wo_po_details_master d,pro_fab_subprocess f,lib_machine_name g,pro_fab_subprocess_dtls t where a.id=f.batch_id and f.id=t.mst_id and g.id=f.machine_id and a.id=b.mst_id and b.po_id=c.id and d.job_no=c.job_no_mst and f.entry_form=35 and a.batch_against in(1,2,3) and t.entry_page=35  and f.status_active=1 and f.is_deleted=0 and a.status_active=1   $str_cond_b  and a.company_id=$compid group by f.load_unload_id,d.buyer_name,a.company_id,a.batch_against" ;
							$self_con_data=sql_select($self_con_sql);

							foreach($self_con_data as $val){
								if($val[csf('batch_against')]==1){

									$self_buyer_wise_qty[$val[csf('company_id')]][$val[csf('buyer_name')]][$val[csf('load_unload_id')]]+=$val[csf('production_qty')];
								}
									$daying_data_arr[$val[csf('load_unload_id')]][$val[csf('buyer_name')]]=$val[csf('buyer_name')];
							}

							//   echo "<pre>"; 
							//   print_r($self_buyer_wise_qty);  

					$tot_InhouseSelfOrder=0;
					$tot_InhouseSubconOrder=0;
					$tot_OutSideSelfOrder=0;
					$tot_TotalInhouse=0;
					$tot_TotalSelfOrder=0;
					$lu_id=1;
					foreach($daying_data_arr[$lu_id] as $buyer_id)
					{
						$InhouseSelfOrder=$self_buyer_wise_qty[$compid][$buyer_id][1]+$dayDataArr[$lu_id][$buyer_id][3];

						// $InhouseSubconOrder=$dayDataArr[$lu_id][$buyer_id][1]+$subDayDataArr[$lu_id][$buyer_id][1];
						$InhouseSubconOrder=$self_buyer_wise_qty[$compid][$buyer_id][1]+$subcon_buyer_wise_qty[$compid][$buyer_id][1];
                        $tot_InhouseSelfOrder+=$self_buyer_wise_qty[$compid][$buyer_id][1];
                        $tot_InhouseSubconOrder+=$subcon_buyer_wise_qty[$compid][$lu_id][$buyer_id][1];
                        $tot_OutSideSelfOrder+=$dayDataArr[$lu_id][$buyer_id][3];
                        $tot_TotalInhouse+=$InhouseSubconOrder;
                        $tot_TotalSelfOrder+=$InhouseSelfOrder;
                    ...
                    <tr>
                    	<td align="center"><? echo $i; ?></td>
                        <td><? echo $buyer_library[$buyer_id]; ?></td>
                        <td align="right"><? echo number_format($self_buyer_wise_qty[$compid][$buyer_id][1]);?></td>
                        <td align="right"><? echo number_format($subcon_buyer_wise_qty[$compid][$buyer_id][1]);?></td>
                        <td align="right"><? echo number_format($dayDataArr[$lu_id][$buyer_id][3]);?></td>
                        <td align="right"><? echo number_format($InhouseSubconOrder);?></td>
                        <td align="right"><? echo number_format($InhouseSelfOrder);?></td>
                    </tr>
                    <?	
					$flag=1;
					}
                    ...
                    <tr>
                    	<tfoot bgcolor="#EEE">
                    		<th>&nbsp;</th>
                            <th>Total</th>
                            <th align="right"><? echo number_format($tot_InhouseSelfOrder); ?></th>
                            <th align="right"><? echo number_format($tot_InhouseSubconOrder); ?></th>
                            <th align="right"><? echo number_format($tot_OutSideSelfOrder); ?></th>
                            <th align="right"><? echo number_format($tot_TotalInhouse); ?></th>
                            <th align="right"><? echo number_format($tot_TotalSelfOrder); ?></th>
                        </tfoot>
                    </tr>
                 </table>&nbsp;
                 ...
                 <table width="80%" cellpadding="0" cellspacing="0" rules="all" border="1">
                 	<tr>
                 		<td colspan="7" height="30" align="center"><strong>Dyeing Machine Un-Loaded</strong></td>
                 	</tr>
					...
					$tot_InhouseSelfOrder=0;$tot_InhouseSubconOrder=0;$tot_OutSideSelfOrder=0;
					$tot_TotalInhouse=0;$tot_TotalSelfOrder=0;
					$lu_id=2;
					foreach($daying_data_arr[$lu_id] as $buyer_id)
					{
						$InhouseSelfOrder=$self_buyer_wise_qty[$compid][$buyer_id][2]+$dayDataArr[$lu_id][$buyer_id][3];

						// $InhouseSubconOrder=$dayDataArr[$lu_id][$buyer_id][1]+$subDayDataArr[$lu_id][$buyer_id][1];
						$InhouseSubconOrder=$self_buyer_wise_qty[$compid][$buyer_id][2]+$subcon_buyer_wise_qty[$compid][$buyer_id][2];
                        $tot_InhouseSelfOrder+=$self_buyer_wise_qty[$compid][$buyer_id][2];
                        $tot_InhouseSubconOrder+=$subcon_buyer_wise_qty[$compid][$buyer_id][2];
                        $tot_OutSideSelfOrder+=$dayDataArr[$lu_id][$buyer_id][3];
                        $tot_TotalInhouse+=$InhouseSubconOrder;
                        $tot_TotalSelfOrder+=$InhouseSelfOrder;
                    ...
                    <tr>
                    	<td align="center"><? echo $i; ?></td>
                        <td><? echo $buyer_library[$buyer_id]; ?></td>
                        <td align="right"><? echo number_format($self_buyer_wise_qty[$compid][$buyer_id][2]);?></td>
                        <td align="right"><? echo number_format($subcon_buyer_wise_qty[$compid][$buyer_id][2]);?></td>
                        <td align="right"><? echo number_format($dayDataArr[$lu_id][$buyer_id][3]);?></td>
                        <td align="right"><? echo number_format($InhouseSubconOrder);?></td>
                        <td align="right"><? echo number_format($InhouseSelfOrder);?></td>
                    </tr>
        ...
        <tr>
            <td valign="top" align="left">
                 <table width="60%" cellpadding="0" cellspacing="0" rules="all" border="1">
                 	<tr>
                 		<td colspan="3" height="30" align="center"><strong>Finish Fabric Production</strong></td>
                 	</tr>
                    ...
                    <?
						...
						if($is_insert_date==0){
							$str_cond_a_receive_date="and a.receive_date between '$previous_date' and '$current_date'";
							$str_cond_b='';
						}
						
						
						$sql_finish="select a.knitting_source,sum(b.receive_qnty) as receive_qnty from inv_receive_master a, pro_finish_fabric_rcv_dtls b where a.id=b.mst_id and a.company_id=$compid and a.entry_form=7 and a.knitting_source in(1,3) and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $str_cond_b $str_cond_a_receive_date group by a.knitting_source";				
...
<?
		...
		$mail_item=2;
		$to="";
		$sql2 = "SELECT c.email_address FROM mail_group_mst a, mail_group_child b, user_mail_address c where b.mail_group_mst_id=a.id and a.mail_item=2 and b.mail_user_setup_id=c.id and a.company_id=$compid and a.IS_DELETED=0 and a.STATUS_ACTIVE=1 AND a.MAIL_TYPE=1 and b.IS_DELETED=0 and b.STATUS_ACTIVE=1   and c.IS_DELETED=0 and c.STATUS_ACTIVE=1";
