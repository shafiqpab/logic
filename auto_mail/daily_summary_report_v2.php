...
//Sewing.......................
	...
	foreach($productionSqlResult as $row)
	{
		if($row[PRODUCTION_TYPE]==5){// sewing
			...
			$sewing_production_po_data_arr[$row[SERVING_COMPANY]][$row[FLOOR_ID]]+=$row[GOOD_QNTY];
			$floor_by_po[$row[PO_BREAK_DOWN_ID]]=$row[FLOOR_ID];
			$company_by_po[$row[PO_BREAK_DOWN_ID]]=$row[SERVING_COMPANY];
		}
...
//Exfactory--------------------
	...
	$exFactorySql="select a.DELIVERY_COMPANY_ID,a.DELIVERY_FLOOR_ID,b.PO_BREAK_DOWN_ID,sum(b.ex_factory_qnty) EX_FACTORY_QNTY  from pro_ex_factory_delivery_mst a,pro_ex_factory_mst b where b.delivery_mst_id=a.id $ex_factory_date_con and a.is_deleted=0 and a.status_active=1 group by a.delivery_company_id,a.delivery_floor_id,b.PO_BREAK_DOWN_ID";
	
	$exFactorySqlResult = sql_select($exFactorySql);
	$ex_fac_qty_arr=array();
	foreach($exFactorySqlResult as $rows)
	{	$rows[DELIVERY_FLOOR_ID]=$floor_by_po[$row[PO_BREAK_DOWN_ID]];
		$rows[DELIVERY_COMPANY_ID]=$company_by_po[$row[PO_BREAK_DOWN_ID]];
...
GROUP BY  b.uom";//
	...
	$fabric_receive_sql="SELECT b.uom ,b.receive_qnty, b.reject_qty, c.order_rate rate,c.order_amount amount  from inv_receive_master a, inv_transaction c,pro_finish_fabric_rcv_dtls b where a.id=c.mst_id and c.id=b.trans_id and a.item_category=2 and a.entry_form=37 and a.status_active=1 and c.status_active=1 and b.status_active=1 and a.RECEIVE_DATE BETWEEN '" . $prev_date . "' and '" . $prev_date . "'";
		
		$fabric_receive_result = sql_select($fabric_receive_sql);
		foreach ($fabric_receive_result as $row) 
		{
			$fabric_receive_qty[$row[csf('uom')]]+=$row[csf('receive_qnty')];

		}			
		//echo $fabric_receive_sql;die;
...
where a.id = b.mst_id and a.batch_id=c.id and a.load_unload_id = 2 and a.result=1 and b.load_unload_id=2 and b.entry_page=35 and a.status_active = 1 and b.status_active=1  and c.status_active=1 and a.company_id in($companyStr) $dye_date_con  group by c.total_trims_weight";*/
 ...
 $dye_date_con=" and f.process_end_date between '$prev_date' and '$prev_date'";
$dye_sql=" (select a.batch_no, a.batch_weight,a.id, a.color_id, a.extention_no,a.total_trims_weight,SUM(b.batch_qnty) AS batch_qnty, b.item_description, b.po_id, b.prod_id, b.width_dia_type, LISTAGG(CAST(c.po_number AS VARCHAR2(4000)),',') WITHIN GROUP ( ORDER BY c.po_number) AS po_number,c.grouping,c.file_no, d.job_no_prefix_num, d.buyer_name,f.remarks, f.shift_name, f.production_date as process_end_date,f.process_end_date as production_date, f.end_hours, f.floor_id, f.hour_unload_meter,f.water_flow_meter, f.end_minutes, f.machine_id, f.load_unload_id,f.fabric_type, f.result,a.booking_no,a.booking_without_order from pro_batch_create_dtls b, wo_po_break_down c, wo_po_details_master d, pro_fab_subprocess f, lib_color g, pro_batch_create_mst a where f.batch_id=a.id and f.batch_id=b.mst_id and a.company_id in($companyStr)  and f.process_end_date BETWEEN '$prev_date' and '$prev_date' and f.service_source=1 and a.entry_form=0 and g.id=a.color_id and a.id=b.mst_id and f.entry_form=35 and f.load_unload_id=2 and f.result=1 and a.batch_against in(1,2) and b.po_id=c.id and d.job_no=c.job_no_mst and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0 GROUP BY a.batch_no, a.batch_weight,a.id, a.color_id,a.total_trims_weight, a.extention_no, b.item_description, b.po_id, b.prod_id, b.width_dia_type,c.grouping,c.file_no,d.job_no_prefix_num, d.buyer_name, f.shift_name, f.production_date,f.process_end_date, f.end_hours, f.floor_id,f.remarks,f.hour_unload_meter, f.water_flow_meter, f.end_minutes, f.machine_id, f.load_unload_id,f.fabric_type,f.result,a.booking_no,a.booking_without_order) union ( select a.batch_no, a.batch_weight,a.id, a.color_id, a.extention_no,a.total_trims_weight, SUM(b.batch_qnty) AS batch_qnty,b.item_description, b.po_id, b.prod_id, b.width_dia_type,null, null,null, null, h.buyer_id as buyer_name,f.remarks,f.shift_name, f.production_date as process_end_date,f.process_end_date as production_date, f.end_hours, f.floor_id, f.hour_unload_meter,f.water_flow_meter, f.end_minutes, f.machine_id, f.load_unload_id, f.fabric_type,f.result,a.booking_no,a.booking_without_order from pro_batch_create_dtls b, pro_batch_create_mst a, pro_fab_subprocess f, lib_color g,wo_non_ord_samp_booking_mst h where h.booking_no=a.booking_no and a.company_id in($companyStr)  and f.batch_id=a.id and f.batch_id=b.mst_id and a.entry_form=0 and g.id=a.color_id and a.id=b.mst_id and f.entry_form=35 and f.result=1 and f.load_unload_id=2 and a.batch_against in(1,2) and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and f.process_end_date BETWEEN '$prev_date' and '$prev_date' and f.service_source=1 GROUP BY a.batch_no, a.batch_weight,a.id,h.buyer_id, a.color_id, a.extention_no, b.item_description, b.po_id, b.prod_id, b.width_dia_type, f.shift_name,a.total_trims_weight, f.production_date,f.process_end_date, f.end_hours, f.floor_id,f.hour_unload_meter, f.water_flow_meter, f.end_minutes, f.machine_id, f.load_unload_id,fabric_type,a.booking_without_order,a.booking_no, f.result,f.remarks ) order by buyer_name"; 
	...
	foreach($dye_sql_result as $row)
	{
		...
		$dyeing_qty+=($row[csf('batch_qnty')]+$row[csf('total_trims_weight')]);
	}
...
?>
	...
	<table cellspacing="0" cellpadding="1" border="1" rules="all">
        ...
        <table cellspacing="0" cellpadding="1" border="1" rules="all">
            ...
            <tr><td>Finish Fabric Receive by Store</td><td align="right"><?= number_format($fabric_receive_qty[12],2);?></td></tr>
...
<?
	...
	$to="";
	$sql = "SELECT c.email_address FROM mail_group_mst a, mail_group_child b, user_mail_address c where b.mail_group_mst_id=a.id and a.mail_item=18 and b.mail_user_setup_id=c.id AND a.MAIL_TYPE=1 and c.IS_DELETED=0 and c.STATUS_ACTIVE=1 group by c.email_address";
