...
$company_library=return_library_array("select id,company_name from lib_company where  id in(1,2,3,4,5,6) status_active=1 and is_deleted=0","id","company_name");
...
$select_fill="to_char(b.update_date,'DD-MM-YYYY HH12:MI:SS')";
	...
	foreach($company_library as $compid=>$compname)
	{
        ...
        <tr>
            <td align="center"><strong>Update Order Code List Of ( Date : <? echo date('d-m-Y');?> )</strong></td>
        </tr>
        <tr>
            <td>
                 ...
                 <table width="100%" cellpadding="0" cellspacing="0" border="1" rules="all">
                   <thead>
					...
					WHERE
							j.company_name IN ($compid)
							AND p.update_date BETWEEN trunc(p.update_date-1) AND trunc(sysdate)
							AND trunc(p.insert_date) <> trunc(sysdate-1)
							AND j.company_name=j.working_company_id
							--and (j.working_company_id=0  or  j.company_name=j.working_company_id)
					GROUP BY
							j.company_name,
							j.job_quantity,
							lb.buyer_name,
...
<? 
	...
	<table>
        <tr>
            <td align="center"><strong>Update Product Code List Of ( Date : <? echo date('d-m-Y');?> )</strong></td>
        </tr>
        <tr>
            <td>
                 <table width="100%" cellpadding="0" cellspacing="0" border="1" rules="all">
                   <thead>
					...
					$sql_pcode = "SELECT A.*, B.* FROM
							(
								...
								c.status_active
							FROM
									 teamerp.wo_po_break_down a
								INNER JOIN teamerp.wo_po_color_size_breakdown      b ON a.job_no_mst = b.job_no_mst
								INNER JOIN teamerp.pro_garments_production_dtls    c ON b.id = c.color_size_break_down_id
							WHERE
											c.production_type = 5 
											AND a.update_date BETWEEN trunc(a.update_date-300) AND trunc(sysdate)
											AND c.status_active = 1
											--AND trunc(a.insert_date) = trunc(sysdate -3)
							GROUP BY
								a.job_no_mst,
								c.production_type,
								c.status_active
							...
							(
								...
								FROM
									...
									WHERE
										j.company_name IN ($compid) 
										AND WP.update_date BETWEEN trunc(WP.update_date-1) AND trunc(sysdate)
										AND trunc(WP.insert_date) <> trunc(sysdate-1)									
	...
	<?
		$subject = "Update Order List for First React of ".$company_arr[$compid];
		$mail_body = "Please see the attached file for Update Order List for First React of ".$company_arr[$compid];
		...
		elseif($compid==4){
			...
			$to=$to.", ".'raihan.uddin@team.com.bd, sohel@4ajacket.com, zillur.frp@4ajacket.com';
			if($to!=""){echo sendMailMailer( $to, $subject, $mail_body, $from_mail, $att_file_arr );}
		}
		elseif($compid==5){
			...
			$to=$to.", ".'al-amin@team.com.bd, pavel@brothersfashion-bd.com, tanveer.hasan@team.com.bd';
			if($to!=""){echo sendMailMailer( $to, $subject, $mail_body, $from_mail, $att_file_arr );}
		}
		...
		if($compid==3){
			...
			$to=$to.", ".'al-amin@team.com.bd, pavel@brothersfashion-bd.com, emdad@brothersfashion-bd.com, tuhin.Rasul@team.com.bd, bfl_scm@brothersfashion-bd.com, abir@brothersfashion-bd.com';
			if($to!=""){echo sendMailMailer( $to, $subject, $mail_body, $from_mail,$att_file_arr );}
		}
		else{
				//$to=$to.", ".'al-amin@team.com.bd';
