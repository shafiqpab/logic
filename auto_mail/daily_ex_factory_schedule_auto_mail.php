...
foreach($company_library as $compid=>$compname)
{
	...
	$sql="select a.buyer_name,a.job_no, c.cartoon_qty,c.approx_cbm,c.approx_ship_mode,c.gross_weight,c.net_weight,b.id as po_id,b.po_number,b.pub_shipment_date,b.shipment_date,b.po_quantity,b.unit_price,b.po_total_price,a.ship_mode,a.factory_marchant,a.set_break_down,a.order_uom,a.style_ref_no,a.company_name,b.rfi_date,a.quotation_id from wo_po_details_master a,wo_po_break_down b, wo_po_color_size_breakdown c where   a.job_no=b.job_no_mst  and a.id = c.job_id
	AND b.id = c.po_break_down_id and to_date(to_char(b.rfi_date, 'DD-MON-YYYY')) BETWEEN '$current_date' AND '$next_date' and a.company_name=$compid  and b.IS_CONFIRMED=1  and b.status_active=1 and  a.status_active=1 and b.status_active=1 order by b.shipment_date,a.job_no, b.rfi_date asc";
   ...
   // echo $sql;die;
	...
	foreach($dataArr as $row)
	{
		...
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['job_no']=$row[csf('job_no')];
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['buyer_name']=$row[csf('buyer_name')];
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['po_number']=$row[csf('po_number')];
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['shipment_date']=$row[csf('shipment_date')];
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['po_quantity']=$row[csf('po_quantity')];
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['unit_price']=$row[csf('unit_price')];
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['po_total_price']=$row[csf('po_total_price')];
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['ship_mode']=$row[csf('ship_mode')];
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['order_uom']=$row[csf('order_uom')];
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['style_ref_no']=$row[csf('style_ref_no')];
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['factory_marchant']=$row[csf('factory_marchant')];
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['po_id']=$row[csf('po_id')];
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['quotation_id']=$row[csf('quotation_id')];
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['rfi_date']=$row[csf('rfi_date')];
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['cartoon_qty']=$row[csf('cartoon_qty')];
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['approx_cbm']=$row[csf('approx_cbm')];
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['approx_ship_mode']=$row[csf('approx_ship_mode')];

		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['gross_weight']=$row[csf('gross_weight')];
		$date_wise_data_arr[$row[csf('company_name')]][$row[csf('rfi_date')]][$row[csf('job_no')]][$row[csf('po_id')]]['net_weight']=$row[csf('net_weight')];
	}
...
th{
  ...
  font-size: 12px;
	...
	?>
		...
		?>
				<br>
				<h4 align="center" ><b>Final inspection offer schedule of Date : &nbsp;<? echo $date;?></b></h4>
                <table cellspacing="0" cellpadding="0" border="1" rules="all" style=" background-color:#ccebff;" class="rpt_table">
					<thead>
						...
						<th width="90">Inspection Date</th>				
						...
						<th width="70">Packing and Finishing (Qty Pcs)</th>
						<th width="50"><p style="color:red">Balance</b></th>
						...
						<th width="100">Approx CBM</th>
						<th width="80">Gross Weight</th>
						<th width="80">Net Weight</th>
						<th width="100">Approx Ship Mode</th>
					</thead>
				...
				foreach ($job_data as $jobId=>$po_data) {
					...
					foreach ($po_data as $poId=>$row) {
					    ...
					    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')"  id="tr_<? echo $i; ?>">
							...
							<td align="center">
								<?
								$image_location=return_field_value("image_location","common_photo_library","file_type=1 and form_name='knit_order_entry' and master_tble_id='$job_id'","image_location");
								?>
								<img  src='../<? echo $image_location; ?>' height='100' align="center" />
						...
						?></p></td>
							...
							<td align="center"><p><? echo $row['cartoon_qty']; ?></p></td>
							<td align="center"><p><? echo $row['approx_cbm']; ?></p></td>
							<td align="center"><p><? echo $row['gross_weight']; ?></p></td>
							<td align="center"><p><? echo $row['net_weight']; ?></p></td>
							<td align="center"><p><? echo $shipment_mode[$row['approx_ship_mode']]; ?></p></td>
							</tr>
                       <?
				...
				NB: In case of any type of mismatch with actual schedule, please immediate update your Order.<br>
			 	প্রকৃত সময়সূচীর সাথে কোন ধরণের অমিল হলে, অনুগ্রহ করে আপনার অর্ডার আপডেট করুন।
				</p>
			<?
		   }
			?>
	...
	<p align='left'><b>Please check today's & tomorrow's Final Inspection Offer and take necessary steps immediately :	</b></p>";	
	...
	else
	{
		...
		$sql = "SELECT c.email_address FROM mail_group_mst a, mail_group_child b, user_mail_address c where b.mail_group_mst_id=a.id and a.mail_item=74 AND a.MAIL_TYPE=1 and b.mail_user_setup_id=c.id and a.company_id=$compid  and c.IS_DELETED=0 and c.STATUS_ACTIVE=1";
...
if($is_all_company==1){
		...
		$sql = "SELECT c.email_address FROM mail_group_mst a, mail_group_child b, user_mail_address c where b.mail_group_mst_id=a.id and a.mail_item=74 and b.mail_user_setup_id=c.id and c.IS_DELETED=0 and c.STATUS_ACTIVE=1";
		$mail_sql=sql_select($sql);
		foreach($mail_sql as $row)
		{
			$toArr[$row[csf('email_address')]]=$row[csf('email_address')]; 
