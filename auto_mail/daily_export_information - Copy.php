...
$company_lib=return_library_array("select id,company_short_name from lib_company where status_active=1 and is_deleted=0 and CORE_BUSINESS=1 ","id","company_short_name");
$del_company_lib=return_library_array("select id,company_short_name from lib_company where status_active=1 and is_deleted=0 and CORE_BUSINESS=1 ","id","company_short_name");
...
// print_r($poDataArr); 
//   echo "</pre>";die();
$sql= "select id, BOOKING_NO,PO_BREAK_DOWN_ID as PO_ID  from wo_booking_mst  where status_active=1 and  is_deleted=0 ".where_con_using_array($orderArr,1,'po_break_down_id')."";
//echo $sql;die();
$bookiong_sql_res=sql_select($sql);
//print_r($bookiong_sql_res);
	$poDataArr2=array();
	foreach($bookiong_sql_res as $row){
		$poDataArr2[$row[PO_ID]]['BOOKING_NO']=$row[BOOKING_NO];
...
//  and b.is_deleted=0 and (a.order_id in('72581')) group by a.order_id,c.cutting_no,c.WORKING_COMPANY_ID 
  ...
  foreach($pro_sql_res as $row){
	 ...
	 $proDataArr2[$row[PO_ID]][$row[WORKING_COMPANY_ID]]['COM_CUTT_QTY']+=$row[CUTT_QTY];
	 
  }

	foreach($proDataArr2 as $poId=>$comData){
		foreach($comData as $cId=>$val){
			$str=$del_company_lib[$cId]."-".$val['COM_CUTT_QTY'];
			$proDataArr[$poId]['SER_COMPANY_CUT_QTY'].=$str.",";
			$proDataArr[$poId]['COM_CUTT_QTY']+=$val['COM_CUTT_QTY'];
		}
	}
	...
	//  		print_r($proDataArr); 
	//    echo "</pre>";die();
	$sql = "select id, embel_name,po_break_down_id as PO_ID,SERVING_COMPANY, PRODUCTION_QUANTITY from pro_garments_production_mst where production_type='3' and embel_name='1' and status_active=1 and is_deleted=0 ".where_con_using_array($orderArr,1,'po_break_down_id')." order by id  ";
    //echo $sql;die();
	...
	foreach($pri_sql_res as $row){
	   ...
	   $proDataArr3[$row[PO_ID]][$row[SERVING_COMPANY]]['COM_PRINT_QTY']+=$row[PRODUCTION_QUANTITY];
	   
	}
    
	  foreach($proDataArr3 as $poId=>$comData){
		  foreach($comData as $cId=>$val){
			  $str=$del_company_lib[$cId]."-".$val['COM_PRINT_QTY'];
			  $proDataArr[$poId]['SER_COMPANY_PRINT_QTY'].=$str.",";
			  $proDataArr[$poId]['COM_PRINT_QTY']+=$val['COM_PRINT_QTY'];
		  }
	  }
	...
	foreach($pri_sql_res as $row){
	   ...
	   $proDataArr4[$row[PO_ID]][$row[SERVING_COMPANY]]['COM_EMB_QTY']+=$row[PRODUCTION_QUANTITY];
	...
	//    echo "</pre>";die();
	  ...
	  foreach($proDataArr4 as $poId=>$comData){
		  foreach($comData as $cId=>$val){
			  $str=$del_company_lib[$cId]."-".$val['COM_EMB_QTY'];
			  $proDataArr[$poId]['SER_COMPANY_EMB_QTY'].=$str.",";
			  $proDataArr[$poId]['COM_EMB_QTY']+=$val['COM_EMB_QTY'];
		  }
	  }
	...
	//    echo "</pre>";die();
	  ...
	  foreach($proDataArr5 as $poId=>$comData){
		  foreach($comData as $cId=>$val){
			  $str=$del_company_lib[$cId]."-".$val['COM_SEW_QTY'];
			  $proDataArr[$poId]['SER_COMPANY_SEW_QTY'].=$str.",";
			  $proDataArr[$poId]['COM_SEW_QTY']+=$val['COM_SEW_QTY'];
		  }
	  }
  ...
  $pri_sql_res=sql_select($sql);
	...
	foreach($pri_sql_res as $row){
	   ...
	   $proDataArr6[$row[PO_ID]][$row[SERVING_COMPANY]]['COM_FIN_QTY']+=$row[PRODUCTION_QUANTITY];
	   
	}
	
    
	  foreach($proDataArr6 as $poId=>$comData){
		  foreach($comData as $cId=>$val){
			  $str=$del_company_lib[$cId]."-".$val['COM_FIN_QTY'];
			  $proDataArr[$poId]['SER_COMPANY_FIN_QTY'].=$str.",";
			  $proDataArr[$poId]['COM_FIN_QTY']+=$val['COM_FIN_QTY'];
		  }
	  }
	...
	foreach($ex_sql_res as $row){
	   ...
	   $proDataArr7[$row[PO_ID]][$row[DELIVERY_COMPANY_ID]]['EX_FACTORY_QNTY']+=$row[EX_FACTORY_QNTY];
...
// 	     echo "</pre>";die();
	  ...
	  foreach($proDataArr7 as $poId=>$comData){
		  foreach($comData as $cId=>$val){
			  $str=$del_company_lib[$cId]."-".$val['EX_FACTORY_QNTY'];
			  $proDataArr[$poId]['SER_COMPANY_EX_QTY'].=$str.",";
			  $proDataArr[$poId]['EX_FACTORY_QNTY']+=$val['EX_FACTORY_QNTY'];
		  }
	  }
	    // echo "<pre>";
	   	// 	print_r($proDataArr); 
	...
	$emb_sql_res=sql_select($sql);
	 foreach($emb_sql_res as $row){
		...
		elseif($row[ENTRY_FORM]==315){$proDataArr[$row[PO_ID]]['EMB_QTY']+=$row[EMB_QTY];
		$proDataArr[$row[PO_ID]]['COMPANY_ID']=$row[COMPANY_ID];        
		}
	 }
// 		 echo "<pre>";
// 		print_r($proDataArr); 
//   echo "</pre>";die();
foreach($proDataArr as $val){
	$str=$del_company_lib[$val['COMPANY_ID']]."-".$val['PRINT_QTY'];
	$proDataArr[$row[PO_ID]]['SER_COMPANY_PRI_QTY']=$str;

	$str2=$del_company_lib[$val['COMPANY_ID']]."-".$val['EMB_QTY'];
	//$proDataArr[$row[PO_ID]]['SER_COMPANY_EMB_QTY']=$str2;
	...
	// $sql="SELECT a.id,a.job_no,b.WORKING_COMPANY_ID,b.ORDER_ID
	// from ppl_cut_lay_mst a,ppl_cut_lay_dtls b 
	// where a.id=b.mst_id  and a.status_active=1 and b.status_active=1 ".where_con_using_array($orderArr,1,'a.ORDER_ID')."";
...
<table border="1" rules="all" width="2500">
	...
	foreach($buyerRows as $po_id=>$rows){
        ...
        <tr bgcolor="<? echo $bgcolor; ?>">
            ...
            <td><?=$poDataArr2[$po_id][BOOKING_NO];?></td>
			...
			echo $challan;?></td>
            <td><?=$proDataArr[$po_id][SER_COMPANY_EX_QTY];?></td>
            ...
            <td align="center"><?=$proDataArr[$po_id][SER_COMPANY_CUT_QTY];?></td>
            ...
            <td align="right"><?=$proDataArr[$po_id][SER_COMPANY_PRINT_QTY];?></td>
			...
			<td align="right"><?=$proDataArr[$po_id][SER_COMPANY_EMB_QTY];?></td>
			...
			<td align="right"><?=$proDataArr[$po_id][SER_COMPANY_SEW_QTY];?></td>
			...
			<td align="right"><?=$proDataArr[$po_id][SER_COMPANY_FIN_QTY];?></td>
