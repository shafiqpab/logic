...
$floor_library = return_library_array( "select id,floor_name from lib_prod_floor where is_deleted=0 and status_active=1", "id", "floor_name");
 ...
 $previous_3month_date = change_date_format(date('Y-m-d H:i:s', strtotime('-180 day', strtotime($current_date))),'','',1); 
	...
	//$previous_date='15-Oct-2020';$current_date='15-Oct-2020';
...
/*echo "tot_opening=".$tot_opening."<br>";
	echo "rcv=".$tot_recv_qty."<br>";
	echo "issue=".$tot_iss_qty."<br>";
	echo "tr_in=".$tot_trans_in_qty."<br>";
	echo "tr_out=".$tot_trans_out_qty."<br>";*/

//echo $grand_stock_qty;die;
...
//Production ---------------------------                           
	...
	//echo $sql_query;die;
		...
		foreach($production_sql_result as $val)
		{
			...
			$sewing_production_po_data_arr[$val[csf('serving_company')]][$val[csf('floor_id')]][$val[csf('sewing_line')]][$val[csf('po_break_down_id')]]+=$val[csf('sewing_output')];
			...
			if($val[csf('good_qnty')]){
				...
				{
					$production_data_arr[$val[csf('serving_company')]][$val[csf('floor_id')]][$val[csf('sewing_line')]]['item_number_id_poly'].="****".$val[csf('po_break_down_id')]."**".$val[csf('item_number_id')]; 
				}
				else
				{
					$production_data_arr[$val[csf('serving_company')]][$val[csf('floor_id')]][$val[csf('sewing_line')]]['item_number_id_poly']=$val[csf('po_break_down_id')]."**".$val[csf('item_number_id')]; 
		...
		foreach($all_style_arr as $com_id=>$styleArr){
			...
			if($smv_source==3)
			{
				...
				$gsdSql="select a.id,a.APPLICABLE_PERIOD,a.BULLETIN_TYPE,A.TOTAL_SMV,A.STYLE_REF,a.GMTS_ITEM_ID  from PPL_GSD_ENTRY_MST a where a.APPLICABLE_PERIOD <= '$previous_date' and A.IS_DELETED=0 and A.STATUS_ACTIVE=1 and a.BULLETIN_TYPE in(3,4)  ".where_con_using_array($all_style_arr[$com_id],1,'A.STYLE_REF')." 
		...
		foreach($company_library as $companyId=>$compname)/// Total Activities
		{
			foreach($production_data_arr[$companyId] as $f_id=>$fname)
			{
				...
				foreach($fname as $l_id=>$ldata)
				{
					...
					$germents_item_poly=array_unique(explode('****',$ldata['item_number_id_poly']));
					foreach($germents_item as $g_val)
					{
						$po_garment_item=explode('**',$g_val);
						
						if($temp_data_arr[$companyId][$f_id][$l_id][$po_garment_item[0]]==''){
							$sewing_produce_arr[$newCompany][$rows[csf("floor_id")]]+=($sewing_production_po_data_arr[$companyId][$f_id][$l_id][$po_garment_item[0]]*$item_smv_array[$po_garment_item[0]][$po_garment_item[1]]);
							$temp_data_arr[$companyId][$f_id][$l_id][$po_garment_item[0]]=1;
						}
						
					}
					
					foreach($germents_item_poly as $g_val)
					{
						$po_garment_item=explode('**',$g_val);
						if($temp_data_arr2[$companyId][$f_id][$l_id][$po_garment_item[0]]==''){
							$poly_produce_arr[$newCompany][$rows[csf("floor_id")]]+=($production_po_data_arr[$companyId][$f_id][$l_id][$po_garment_item[0]]*$item_smv_array[$po_garment_item[0]][$po_garment_item[1]]);
							$temp_data_arr2[$companyId][$f_id][$l_id][$po_garment_item[0]]=1;
						}
	...
	sum((case when a.production_type=5 and d.production_type=5 then (c.smv*d.prod_qnty) else 0 end) as sewing_produce, 
...
//Kniting Production--------------------------------------                            
  ...
  union all
     select sum(b.PRODUCT_QNTY) as qtyinhouse  from subcon_production_mst a, subcon_production_dtls b where a.id=b.mst_id  and a.product_type in(2) and b.product_type in(2) and a.company_id in($companyStr) and a.product_date between '".$previous_date."' and '".$current_date."'   and a.is_deleted=0 and a.status_active=1  and b.is_deleted=0 and b.status_active=1	 
...
//Dyes/Chemical...........................................
	...
	foreach($mrr_rate_sql as $row)
	{
        ...
        from inv_transaction a , product_details_master b
         where  a.prod_id=b.id and a.item_category in(5,6,7,23) and b.company_id=1 and b.item_category_id in (5,6,7,23)  and a.order_id=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
...
<table cellpadding="3" cellspacing="0" border="1" rules="all">
    <tr><th colspan="5">Daily Report</th></tr>		
    <tr><th colspan="5">Urmi Group</th></tr>		
    <tr><th colspan="5">Date-<? echo date('d.m.Y',strtotime($previous_date)); ?></th></tr>		
    <tr bgcolor="#DDD">
        <th width="100">Garments</th>
	...
	foreach($poly_qty_arr as $comapny_id=>$rows){
		...
		$totalPolyQty+=$polyQty;
		$totalExFactoryQty+=$ex_fac_qty_arr[$comapny_id][$floor_id];	
		$totalProduceQty+=$sewing_produce_arr[$comapny_id][$floor_id];
		
		
		$sewingQty=$sewing_qty_arr[$comapny_id][$floor_id];
		$totalSewingQty+=$sewingQty;
			
	?>
    <tr>
        <td><? echo $company_library[$comapny_id].' '.$floor_library[$floor_id];?></td>
        <td align="right"><? echo number_format($sewingQty);?></td>
        <td align="right"><? echo number_format($polyQty);?></td>
        <td align="right"><? echo number_format($sewing_produce_arr[$comapny_id][$floor_id]);?></td>
        <td align="right"><? echo number_format($ex_fac_qty_arr[$comapny_id][$floor_id]);?></td>
    </tr>		
    <? $i++;}} ?>
    <tr bgcolor="#DDD">
        <td align="right"><strong>Total</strong></td>
        <td align="right"><strong><? echo number_format($totalSewingQty);?></strong></td>
        <td align="right"><strong><? echo number_format($totalPolyQty);?></strong></td>
        <td align="right"><strong><? echo number_format($totalProduceQty);?></strong></td>
        <td align="right"><strong><? echo number_format($totalExFactoryQty);?></strong></td>
    </tr>		
</table>
...
//end 1 part.........................................
	...
	$cbo_company_id='4,1,3,2';
	$cbo_store_name='48,82,83,86, 18,47,80,81, 49,84,85,88,95,97,98,7,64,87,89,96';
	$cbo_buyer_id='0';
	$txt_book_no='';
	$txt_book_id='';
	$cbo_year='0';
	...
	WHERE a.company_id in ($cbo_company_id) and a.id = b.mst_id and b.id=c.trans_id and b.transaction_type=1 and a.entry_form=37 and a.status_active =1 and b.status_active =1 and c.status_active =1 and b.pi_wo_batch_no=e.id $store_cond $date_cond  $all_book_nos_cond $pi_no_cond
	...
	if(!empty($all_po_id_arr))
	{
		...
		foreach ($booking_sql as  $val)
		{
			...
			$book_po_ref[$val[csf("booking_no")]][$val[csf("body_part_id")]][$val[csf("lib_yarn_count_deter_id")]][$val[csf("gmts_color_id")]]["qnty"] += $val[csf("fin_fab_qnty")];
			$book_po_ref[$val[csf("booking_no")]][$val[csf("body_part_id")]][$val[csf("lib_yarn_count_deter_id")]][$val[csf("gmts_color_id")]]["color_type"] .= $color_type[$val[csf("color_type")]].",";

			$book_po_ref[$val[csf("booking_no")]][$val[csf("body_part_id")]][$val[csf("lib_yarn_count_deter_id")]][$val[csf("gmts_color_id")]]["amount"] += $val[csf("fin_fab_qnty")]*$val[csf("rate")];
	...
	$issRtnSql = "select c.transaction_date, d.knit_dye_source, b.body_part_id, b.prod_id,c.store_id, $issue_return_select b.fabric_description_id, b.gsm, b.width, b.color_id,c.cons_uom, c.cons_quantity as quantity, c.order_rate, b.batch_id, e.batch_no, e.booking_no, e.booking_without_order from inv_receive_master a, pro_finish_fabric_rcv_dtls b,inv_transaction c, inv_issue_master d, pro_batch_create_mst e where a.id = b.mst_id and b.trans_id = c.id and c.issue_id = d.id and a.entry_form = 52 and a.item_category = 2 and c.pi_wo_batch_no = e.id and a.status_active =1 and b.status_active=1 and c.status_active =1 and c.company_id in  ($cbo_company_id) $store_cond_2 $date_cond_2 $all_batch_ids_cond";
	...
	foreach ($data_array as $uom => $uom_data)
	{
			...
			foreach ($book_data as $prodStr => $row)
			{
				...
				if ((($get_upto == 1 && $ageOfDays > $txt_days) || ($get_upto == 2 && $ageOfDays < $txt_days) || ($get_upto == 3 && $ageOfDays >= $txt_days) || ($get_upto == 4 && $ageOfDays <= $txt_days) || ($get_upto == 5 && $ageOfDays == $txt_days) || $get_upto == 0) && (($get_upto_qnty == 1 && $stock_qnty > $txt_qnty) || ($get_upto_qnty == 2 && $stock_qnty < $txt_qnty) || ($get_upto_qnty == 3 && $stock_qnty >= $txt_qnty) || ($get_upto_qnty == 4 && $stock_qnty <= $txt_qnty) || ($get_upto_qnty == 5 && $stock_qnty == $txt_qnty) || $get_upto_qnty == 0))
				{
					if($stock_qnty!=0 && $cbo_value_with==2)
					{
						?>
						<!--<tr>
							<td>< ? echo $store_arr[$prodStrArr[1]];?></td>
							<td>< ? echo number_format($stock_rate,2,".","");?></td>
							<td>< ? echo number_format($stock_amount,2,".","");?></td>
						</tr>-->
						<?
						$i++;
					...
					else if($cbo_value_with==1)
					{
						...
						$urmiStorArr=array(49=>49,84=>84,85=>85,88=>88,95=>95,97=>97,98=>98,7=>7,64=>64,87=>87,89=>89,96=>96);	
						...
						?>
						<!--<tr>
							<td>< ? echo $store_arr[$prodStrArr[1]].'='.$unit_of_measurement[$prodStrArr[7]];?></td>
							<td>< ? echo number_format($stock_qnty,2,".","");?></td>
							<td>< ? echo number_format($stock_rate,2,".","");?></p></td>
							<td>< ? echo number_format($stock_amount,2,".","");?></td>
						</tr>-->
						<?
						/*$i++;
		...
		<!--<tr>
			<td>&nbsp;</td>
			<td>< ? echo number_format($uom_total_stock_qnty,2,".",""); ?></td>
			<td></td>
			<td>< ? echo number_format($uom_total_stock_amount,2,".",""); ?></td>
		</tr>-->
		<?
	}
	unset($data_array);
...
//textile stock.....................................................................
	...
	$sql = "select 1 as type, min(a.receive_date) as mrr_date, a.company_id, c.po_breakdown_id, b.prod_id, b.body_part_id, b.fabric_description_id, $selectRcvStore b.uom, b.color_id,b.dia_width_type, b.width, b.gsm, sum(c.quantity) as quantity , sum(e.cons_amount) as amount,0 as is_transfered,0 as from_order_id, a.receive_basis, sum(e.order_amount) as order_amount, e.transaction_date
	from inv_receive_master a, pro_finish_fabric_rcv_dtls b, order_wise_pro_details c, fabric_sales_order_mst d, inv_transaction e
	where a.id=b.mst_id and b.id=c.dtls_id and c.po_breakdown_id=d.id and c.trans_id=e.id and a.entry_form=225 and c.entry_form=225 and b.is_sales=1 and c.is_sales=1 and a.company_id = $company_name $withinGroupCond $order_no_cond $booking_no_cond $buyer_id_cond $rcvStoreCond $year_search_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.receive_basis in (10,14) $to_trans_date_cond
	group by a.company_id,c.po_breakdown_id, b.prod_id,b.body_part_id,b.fabric_description_id, $groupByRcvStore b.uom, b.color_id, b.dia_width_type, b.width, a.item_category, b.gsm, a.receive_basis, e.transaction_date
	union all
	select 2 as type, min(a.transfer_date) as mrr_date, a.company_id,a.to_order_id as po_breakdown_id, b.from_prod_id as prod_id, b.body_part_id,b.feb_description_id as fabric_description_id, $selectTransStore b.uom,b.color_id,b.dia_width_type, b.dia_width as width, b.gsm, sum(b.transfer_qnty) as quantity , sum(e.cons_amount) as amount,1 as is_transfered,a.from_order_id , 0 as receive_basis, sum(e.order_amount) as order_amount, e.transaction_date
	from inv_item_transfer_mst a, inv_item_transfer_dtls b, fabric_sales_order_mst d , inv_transaction e
	where a.id=b.mst_id and a.to_order_id=d.id and b.to_trans_id=e.id and a.company_id=$company_name $withinGroupCond $order_no_cond $booking_no_cond $buyer_id_cond $TransStoreCond $year_search_cond and a.entry_form in(230) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $to_trans_date_cond
	group by a.company_id,a.to_order_id, b.from_prod_id, b.body_part_id, b.feb_description_id, $groupByTransStore b.uom, b.color_id,b.dia_width_type, b.dia_width, b.gsm,a.from_order_id, e.transaction_date
	...
	if(!empty($fso_id_arr))
	{
		...
		$delivery_qnty_sql = sql_select("select b.body_part_id bodypart_id,b.uom,b.width_type,sum(c.quantity) delivery_qnty, sum(a.order_amount) as amount, c.is_sales, c.po_breakdown_id, c.prod_id,d.detarmination_id determination_id,d.gsm,d.dia_width dia, $selectRcvStore d.color color_id, a.transaction_date
			from inv_finish_fabric_issue_dtls b,order_wise_pro_details c,product_details_master d, inv_transaction a
			where a.company_id=$company_name and b.id=c.dtls_id and c.prod_id=d.id and c.trans_id = a.id $all_fso_cond2 $rcvStoreCond and b.status_active=1 and c.entry_form=224 and c.status_active=1 and a.status_active=1 $to_trans_date_cond2 group by b.body_part_id,b.uom, b.width_type, c.is_sales, c.po_breakdown_id, c.prod_id,d.detarmination_id,d.gsm, $groupByRcvStore d.dia_width,d.color,a.transaction_date");
		...
		$issue_return_sql = sql_select("select a.company_id, c.po_breakdown_id, b.prod_id, b.body_part_id, b.fabric_description_id,  b.uom, b.color_id,b.dia_width_type, b.width, b.gsm, $selectRcvStore sum(c.quantity) as quantity , sum(e.order_amount) as amount, e.transaction_date from inv_receive_master a, pro_finish_fabric_rcv_dtls b, order_wise_pro_details c, inv_transaction e where a.id=b.mst_id and b.id=c.dtls_id and c.trans_id=e.id and a.entry_form=233 and c.entry_form=233and b.is_sales=1 and c.is_sales=1 and a.company_id=$company_name $all_fso_cond2 $rcvStoreCond $to_trans_date_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by a.company_id,c.po_breakdown_id, b.prod_id, b.body_part_id, b.fabric_description_id,b.uom, b.color_id, $groupByRcvStore b.dia_width_type, b.width, a.item_category, b.gsm, e.transaction_date");
		...
		$transfered_fabric_sql = sql_select("select a.from_order_id as po_breakdown_id,b.batch_id, b.from_prod_id as prod_id, b.body_part_id, b.feb_description_id as fabric_description_id, $selectTransOutStore b.uom, b.color_id,b.fabric_shade,b.dia_width_type, b.dia_width as width, b.gsm, sum(b.transfer_qnty) as quantity, sum(c.order_amount) as amount, c.transaction_date
			from inv_item_transfer_mst a, inv_item_transfer_dtls b, inv_transaction c
			where a.id=b.mst_id and b.trans_id = c.id and c.transaction_type=6 and a.entry_form in(230) and a.company_id = $company_name $all_fso_cond3 $to_trans_date_cond3 and a.status_active =1 and a.is_deleted =0 and b.status_active = 1 and b.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0
			group by a.from_order_id, b.batch_id, b.from_prod_id, b.body_part_id, b.feb_description_id, $groupByTransOutStore b.uom,  b.color_id,b.fabric_shade,b.dia_width_type, b.dia_width, b.gsm, c.transaction_date");
		...
		foreach ($uom_data as $po_breakdown_id => $po_breakdown_data)
		{
			...
			foreach ($po_breakdown_data as $prod_ref => $row)
			{
				...
				if($cbo_store_wise ==1)
				{
					...
					//$opening_arr[$row[csf("po_breakdown_id")]][$row[csf("prod_id")]][$row[csf("fabric_description_id")]][$row[csf("gsm")]][$row[csf("width")]][$row[csf("body_part_id")]][$row[csf("dia_width_type")]][$row[csf("color_id")]][$row[csf("uom")]]["open_rcv_qnty"]
				...
				$total_stock_qty = $total_rcv_qnty-$total_issue_qnty + $opening_balance_qnty;
			...
			$details_row_span_arr[$uom_id."##".$po_breakdown_id] =$details_row_span;
		}
	}*/
...
<table border="1" rules="all">
	...
	<tr>
	    <td>FTML Textile</td>
	    <td align="right"><? echo number_format($textile_stock_qty_arr[12][1],2);?></td>
	    <td align="right"><? echo number_format($textile_stock_qty_arr[27][1],2);?></td>
	    <td align="right"><? echo number_format($textile_stock_qty_arr[23][1],2);?></td>
	    <td align="right"><? echo number_format(($textile_stock_amount_arr[12][1]+$textile_stock_amount_arr[27][1]+$textile_stock_amount_arr[23][1]),2);?></td>
	</tr>
	...
	<tr>
		<td>FTML Garments</td>
        <td align="right"><? echo number_format($ftmlTotalArr[12],2);?></td>
		<td align="right"><? echo number_format($ftmlTotalArr[27],2);?></td>
		<td align="right"><? echo number_format($ftmlTotalArr[23],2);?></td>
        <td align="right"><? echo number_format($ftmlTotalArr['amount'],2);?></td>
	</tr>
	<?
	$comKey=3;
	$uhmTotalArr=array();
	...
	<tr>
		<td>UHM</td>
		<td align="right"><? echo number_format($uhmTotalArr[12],2);?></td>
		<td align="right"><? echo number_format($uhmTotalArr[27],2);?></td>
		<td align="right"><? echo number_format($uhmTotalArr[23],2);?></td>
        <td align="right"><? echo number_format($uhmTotalArr['amount'],2);?></td>
	</tr>
	...
	<tr>
		<td>URMI</td>
		<td align="right"><? echo number_format($urmiTotalArr[12],2);?></td>
		<td align="right"><? echo number_format($urmiTotalArr[27],2);?></td>
		<td align="right"><? echo number_format($urmiTotalArr[23],2);?></td>
        <td align="right"><? echo number_format($urmiTotalArr['amount'],2);?></td>
	</tr>
    <tr bgcolor="#CCCCCC">
        <td align="right"><b>Total</b></td>
        <td align="right"><b><? echo number_format($ftmlTotalArr[12]+$uhmTotalArr[12]+$urmiTotalArr[12]+$textile_stock_qty_arr[12][1],2);?></b></td>
        <td align="right"><b><? echo number_format($ftmlTotalArr[27]+$uhmTotalArr[27]+$urmiTotalArr[27]+$textile_stock_qty_arr[27][1],2);?></b></td>
        <td align="right"><b><? echo number_format($ftmlTotalArr[23]+$uhmTotalArr[23]+$urmiTotalArr[23]+$textile_stock_qty_arr[23][1],2);?></b></td>
        <td align="right"><b><? echo number_format($ftmlTotalArr['amount']+$uhmTotalArr['amount']+$urmiTotalArr['amount']+($textile_stock_amount_arr[12][1]+$textile_stock_amount_arr[27][1]+$textile_stock_amount_arr[23][1]),2);?></b></td>
    </tr>
</table>
...
<table cellspacing="0" border="1" rules="all">
    <tr>
        <td colspan="7" align="center"><b>Buyer wise Segregation (RMG)</b></td>
    </tr>
    <tr bgcolor="#CCCCCC">
	...
	foreach($buyerTotalStockQtyArr as $buyer_id=>$uomRow){
		if($poly_qty_arr[$buyer_id] || $uomRow[12] || $uomRow[27] || $uomRow[23]){
	?>
    <tr>
        <td><? echo $buyer_arr[$buyer_id];?></td>
        <td align="right"><? echo array_sum($poly_line_arr[$buyer_id]);?></td>
        <td align="right"><? echo number_format($poly_qty_arr[$buyer_id],2);?></td>
        <td align="right"><? echo number_format($uomRow[12],2);?></td>
        <td align="right"><? echo number_format($uomRow[27],2);?></td>
        <td align="right"><? echo number_format($uomRow[23],2);?></td>
        <td align="right"><? echo number_format($buyerTStockValArr[$buyer_id],2);?></td>
    </tr>
    <? } }?>
    <tr bgcolor="#CCCCCC">
        <td align="right"><b>Total</b></td>
        <td align="right"><? //echo array_sum($poly_line_arr);?></td>
        <td align="right"><b><? echo number_format(array_sum($poly_qty_arr),2);?></b></td>
        <td align="right"><b><? echo number_format($umQtyArr[12],2);?></b></td>
        <td align="right"><b><? echo number_format($umQtyArr[27],2);?></b></td>
        <td align="right"><b><? echo number_format($umQtyArr[23],2);?></b></td>
        <td align="right"><b><? echo number_format(array_sum($comTotalStockValArr),2);?></b></td>
    </tr>
</table>
...
$companyStr = implode(',',array_keys($company_library));
	...
	foreach($sql_result as $val)
	{
		...
		$key=$val[csf('floor_id')].'**'.$val[csf('sewing_line')].'**'.$val[csf('po_break_down_id')].'**'.$val[csf('item_number_id')].'**'.$val[csf('production_date')].'**'.$val[csf('serving_company')];
...
$company_cond=" and a.company_id in(".$companyStr.")";
 ...
 $sql_subcon="select  a.company_id,a.production_type, a.floor_id, a.production_date, a.line_id as sewing_line, a.order_id as po_break_down_id, a.gmts_item_id as item_number_id,sum(d.prod_qnty) as good_qnty,sum(c.smv*d.prod_qnty) as produce,c.smv,c.cust_style_ref as style_ref_no
	...
	foreach($sql_subcon_result as $val)
	{
		...
		$key=$val[csf('floor_id')].'**'.$val[csf('sewing_line')].'**'.$val[csf('po_break_down_id')].'**'.$val[csf('item_number_id')].'**'.$val[csf('production_date')].'**'.$val[csf('company_id')];
...
foreach($company_library as $company_id=>$comapny_name){
	...
	if($smv_source==3)
	{
		$gsdSql="select a.id,a.APPLICABLE_PERIOD,a.BULLETIN_TYPE,A.TOTAL_SMV,A.STYLE_REF,a.GMTS_ITEM_ID  from PPL_GSD_ENTRY_MST a where a.APPLICABLE_PERIOD <= '$from_date' and A.IS_DELETED=0 and A.STATUS_ACTIVE=1 and a.BULLETIN_TYPE in(3,4)  ".where_con_using_array($all_style_arr[$company_id],1,'A.STYLE_REF')." 
			group by a.id,a.APPLICABLE_PERIOD,a.BULLETIN_TYPE,A.TOTAL_SMV,A.STYLE_REF,a.GMTS_ITEM_ID
			 ORDER BY a.GMTS_ITEM_ID ,a.APPLICABLE_PERIOD  DESC , a.id DESC";
			     //echo $gsdSql.'======'; 
			$gsdSqlResult = sql_select($gsdSql); 
	...
	foreach($sewing_production_po_data_arr[$company_id] as $key=>$good_qnty){
		...
		if(($floor_id==14 || $floor_id==20 || $floor_id==24 || $floor_id==37) and $company_id==1){$company_id_str='unite_1';}
			...
			$sewing_produce_arr[$company_id_str]+=$item_smv_array[$po_id][$item_id]*$good_qnty;
...
//echo $current_date.'=='.$search_prod_date;
	...
	foreach($poly_production_po_data_arr[$company_id] as $key=>$good_qnty){
		...
		$poly_produce_arr[$company_id_str]+=$item_smv_array[$po_id][$item_id]*$good_qnty;
...
$urmi=(($poly_achieved_arr[2]+$poly_achieved_arr[4])*100)/($poly_efficiency_min_arr[2]+$poly_efficiency_min_arr[4]);
...
//------------
//$poly_achieve_per_ftml_unite_1=($poly_produce_arr2['unite_1']/$poly_SMV_svailable_arr['unite_1'])*100;
...
//$achieve_per_urmi=(($poly_produce_arr2[2]+$poly_produce_arr2[4])/($poly_SMV_svailable_arr[2]+$poly_SMV_svailable_arr[4]))*100;
$achieve_per_urmi=(($sewing_produce_arr[2]+$sewing_produce_arr[4])/($sewing_SMV_svailable_arr[2]+$sewing_SMV_svailable_arr[4]))*100;





//$achieve_per_uhm=($poly_produce_arr2[3]/$poly_SMV_svailable_arr[3])*100;
$achieve_per_uhm=($sewing_produce_arr[3]/$sewing_SMV_svailable_arr[3])*100;
...
<table cellspacing="0" border="1" rules="all">
    ...
    <tr>
    	...
    	<td align="right"><? echo number_format($ftml_unite_1,2);?></td>
    	<td align="right"><? echo fn_number_format($sewing_achieve_per_ftml_unite_1,2);?></td>
	</tr>
    <tr>
    	...
    	<td align="right"><? echo number_format($ftml_unite_2,2);?></td>
    	<td align="right"><? echo fn_number_format($sewing_achieve_per_ftml_unite_2,2);?></td>
	</tr>
    <tr>
    	<td>URMI (Poly)</td>
    	<td align="right"><? echo number_format($poly_efficiency_min_arr[2]+$poly_efficiency_min_arr[4]);?></td>
    	<td align="right"><? echo number_format($poly_achieved_arr[2]+$poly_achieved_arr[4]);?></td>
    	<td align="right"><? echo number_format($urmi,2);?></td>
    	<td align="right"><? echo fn_number_format($achieve_per_urmi,2);?></td>
	</tr>
    <tr>
    	<td>UHM (Poly)</td>
    	<td align="right"><? echo number_format($poly_efficiency_min_arr[3]);?></td>
    	<td align="right"><? echo number_format($poly_achieved_arr[3]);?></td>
    	<td align="right"><? echo number_format($uhm,2);?></td>
    	<td align="right" title="<?= $poly_produce_arr[3].'/'.$poly_SMV_svailable_arr[3]; ?>"><? echo fn_number_format($achieve_per_uhm,2);?></td>
	</tr>
    <tr bgcolor="#DDD">
    	<td align="right"><b>Total</b></td>
