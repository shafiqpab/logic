...
/*	function fn_number_format($val,$after_point=0){
...
//Order...................................................................................................
    $dateCon="and ((c.TASK_FINISH_DATE between '$current_date' and '$next_date') or (c.TASK_START_DATE  between '$current_date' and '$next_date' ))";
   $capacity_sql="select a.company_name,a.job_no,a.total_set_qnty,a.set_smv,b.id,b.po_number,b.is_confirmed,b.pub_shipment_date,b.po_quantity,b.po_total_price,c.TASK_FINISH_DATE  from wo_po_details_master a,wo_po_break_down b,TNA_PROCESS_MST c where a.job_no=b.job_no_mst and b.id=c.PO_NUMBER_ID and c.TASK_NUMBER=86 and c.TASK_TYPE=1  and  b.status_active=1 and b.is_deleted=0    $dateCon and a.company_name in(".implode(',',$company_id_arr).")";
...
//   echo  $capacity_sql;
	...
	foreach($capacity_sql_result as $row)
	{
		$my=date("M-Y",strtotime($row[csf('task_finish_date')]));
...
<table rules="all" border="1">
   ...
   <tr>
       <td colspan="8" align="center">
           ...
           <strong>Monthly Capacity Vs Booked As TNA Date Wise</strong>
       </td>
   </tr>
	<?   
...
<?
	...
	$to="";$message="";
	$sql = "SELECT c.email_address FROM mail_group_mst a, mail_group_child b, user_mail_address c where b.mail_group_mst_id=a.id and a.mail_item=69 and b.mail_user_setup_id=c.id AND a.MAIL_TYPE=1 and c.IS_DELETED=0 and c.STATUS_ACTIVE=1 and a.company_id in(".implode(',',$company_id_arr).")";
	...
	$subject="Monthly Capacity Vs Booked As TNA Date Wise";
