...
{ 
	...
	$company_library 	=return_library_array( "select id, company_name from lib_company where status_active=1 and is_deleted=0 and id=3", "id", "company_name");
	...
	//$previous_date='1-Sep-2017';

	$str_cond	=" and approved_date between '".$previous_date."' and '".$current_date." 11:59:59 PM'";
...
$user_sql = "select a.id,a.user_full_name,b.custom_designation from user_passwd a,lib_designation b where a.valid=1 and b.id=a.designation";
	...
	foreach($user_data_array as $row){
		$user_arr[$row[csf('id')]]=$row[csf('user_full_name')];
	}
	
	$sql="select bypass,buyer_id, sequence_no, user_id FROM electronic_approval_setup where company_id=$compid and ENTRY_FORM=15 and is_deleted=0 order by sequence_no asc";
	//724
	...
	//$sql="select min(b.current_approval_status) as cstatus,b.approved_by,b.job_no FROM co_com_pre_costing_approval b where b.approved_by =".$electronic_data[$endSequence]." $str_cond group by b.job_no,b.approved_by";
	
	
	$sql="select min(b.current_approval_status) as cstatus,a.approved_by,b.job_no FROM approval_history a,co_com_pre_costing_approval b where a.mst_id=b.mst_id and a.approved_by =".$electronic_data[$endSequence]." $str_cond_b group by b.job_no,a.approved_by";
	$data_array=sql_select($sql);
	foreach($data_array as $row){
		if($row[csf('cstatus')]!=0){
			$last_app_data[$row[csf('job_no')]] = $row[csf('approved_by')];
			$app_job_arr[$row[csf('job_no')]] = "'".$row[csf('job_no')]."'";
		}
	}
	$app_job_str = implode(',',$app_job_arr);

	
	$sql="select min(b.current_approval_status) as cstatus,b.approved_by,b.mst_id,b.job_no FROM co_com_pre_costing_approval b where b.approved_by <> ".$electronic_data[$endSequence]." and b.higher_othorized_approved=1 $str_cond group by b.mst_id,b.job_no,b.approved_by";
	$data_array=sql_select($sql);
	foreach($data_array as $row){
		$higher_auth_id_arr[$row[csf('job_no')]] = $row[csf('approved_by')];//for Higher Auth Check;
	}
