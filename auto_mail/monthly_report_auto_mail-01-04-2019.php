...
//if(date('d')!=2){exit('This mail will be send only date of 2 every month');}
...
$ex_factory_sql= "SELECT d.delivery_company_id,d.delivery_floor_id,
			...
			where a.po_break_down_id=b.id and b.job_no_mst=c.job_no  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and d.delivery_company_id=$compid $ex_factory_date_con and c.status_active=1 and a.delivery_mst_id=d.id 
			and a.entry_form<>85 
			group by d.delivery_company_id,d.delivery_floor_id ";	 
	...
	foreach($ex_factory_sql_result as $rows)
	{
		...
		*/
		...
		//$ex_fac_fob_arr[$rows[csf("delivery_company_id")]][$rows[csf("delivery_floor_id")]]=$rows[csf("fob")];
	}
...
//Production ---------------------------                           
	...
	/*$production_sql="select a.po_break_down_id,a.serving_company as company_id,a.floor_id,sum(a.production_quantity)  production_quantity  from pro_garments_production_mst a where a.production_type=11 and a.serving_company=$compid and a.is_deleted=0 and a.status_active=1 $production_date_con group by a.serving_company,a.po_break_down_id,a.floor_id order by a.floor_id desc";*/
	
	$production_sql="select a.production_type,a.po_break_down_id,a.serving_company as company_id,a.floor_id,sum(b.production_qnty)  production_quantity  from pro_garments_production_mst a,pro_garments_production_dtls b,wo_po_color_size_breakdown c  where a.id=b.mst_id and a.production_type in(8,11) and b.production_type in(8,11) and a.serving_company=$compid and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and c.is_deleted=0 and b.status_active=1 and c.id=b.color_size_break_down_id $production_date_con group by a.serving_company,a.production_type,a.po_break_down_id,a.floor_id order by a.floor_id desc";
...
//Kniting Production--------------------------------------                            
	...
	$str_cond_f	=" and a.receive_date between '".$previous_date."' and '".$current_date."'";
	 $sql_qty="select sum(case when a.knitting_source in(1,3) then c.quantity end ) as qtyinhouse 
...
//Yearn stock-------------------------------------- 
		...
		//stock qty= (total receive+total trans in+issue Return)-(total issue+total trans out + Receive Return);
          	//$yearn_stock_qty_sql_result = sql_select($yearn_stock_qty_sql, '', '', '', $con);
          	foreach ($yearn_stock_qty_sql_result as $row) {
          		$total_stock_qty += $row[csf("stock_qty")];
				//$total_stock_amu +=$row[csf("stock_qty")]*($row[csf("amu")] / $row[csf("qty")]);
				//$total_stock_amu +=$row[csf("stock_amount")]/$conversion_rate;
 ...
 $sql_subcon="select  a.production_type, a.floor_id, a.production_date, a.line_id as sewing_line, a.order_id as po_break_down_id, a.gmts_item_id as item_number_id,sum(d.prod_qnty) as good_qnty
	...
	//$to="";
	$sql2 = "SELECT c.email_address FROM mail_group_mst a, mail_group_child b, user_mail_address c where b.mail_group_mst_id=a.id and a.mail_item=18 and b.mail_user_setup_id=c.id and a.company_id=$compid AND a.MAIL_TYPE=1";
...
/*	$dhu_sql = "SELECT d.serving_company,d.floor_id,sum(a.defect_qty) as defect_qty,sum(d.reject_qnty) as reject_qnty 
 ...
 WHERE   d.id=a.mst_id and a.defect_type_id in (3,4) and a.production_type=5 and a.is_deleted=0 and d.is_deleted=0 and d.status_active=1 and a.status_active=1 $company_cond $date_cond  group by d.serving_company,d.floor_id
";*/
	...
	foreach($dhu_deft_sql_result as $row){
			...
			*/
			...
			if($row[csf('serving_company')]==4){$row[csf('floor_id')]=0;$row[csf('serving_company')]=2;}//
	...
	$dhu_sql = "SELECT d.serving_company,d.floor_id,sum(f.production_qnty) as qc_pass_qty,sum(f.alter_qty) as alter_qnty, 
...
//Man Machine Ratio----------------------------------------------start;
		...
		//$company_cond=" and unit_id in($companyStr)";
...
/*		foreach($machine_data_array as $company_id=>$val){
			$man_machine_ratio=($val/array_sum($day_data_array[$company_id]))*1;
			$man_machine_ratio_array[$company_id]=number_format($man_machine_ratio,2,".","");
		}
*/		
...
//Air freight in Qty (lakh/pcs)--------------------------start;
	...
	foreach($air_exfactory_sql_result as $row){
			...
			if($row[csf('delivery_company_id')]==4){$row[csf('delivery_company_id')]=2;$row[csf('delivery_floor_id')]=0;}//
			if($row[csf('delivery_company_id')]==3 || $row[csf('delivery_company_id')]==2){$row[csf('delivery_floor_id')]=0;}
				
				
				//if($row[csf('delivery_company_id')]==4){$row[csf('delivery_company_id')]=2;}
				//this is for urmi group.........................end;
...
//Air freight in Qty (lakh/pcs)--------------------------end;
	...
	foreach($re_process_sql_result as $row){
		...
		$batch_qty_data_array+=($row[csf('production_qty')]+$row[csf('total_trims_weight')]);
	...
	$dates_com=" and  f.process_end_date BETWEEN '$previous_date' AND '$current_date' ";	
			...
			$workingCompany_name_cond2  $dates_com

			and a.entry_form=0  and a.id=b.mst_id and f.batch_id=b.mst_id and f.entry_form=35 and f.load_unload_id=2 and a.batch_against in(1,2,3,11) and b.po_id=d.id and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and a.is_sales=1 and b.is_sales=1
			";
			//echo $sql;die;
	...
	foreach($total_reprocess_qty as $company_id=>$floorDataARr){
		foreach($floorDataARr as $floor_id=>$qty){
			$re_process_qty_array+=($qty*100)/($total_rft_qnty[$company_id][$floor_id] + $total_adding_qnty[$company_id][$floor_id] + $total_reprocess_qty[$company_id][$floor_id]);
		}
	}
		
	
	 //echo number_format(($total_reprocess_qty*100)/($total_rft_qnty + $total_adding_qnty + $total_reprocess_qty),2);
	...
	$parcent_of_compensation_on_production_qty_array+=($short_fb_qty_compensative_data_array/$batch_qty_data_array)*100;
	...
	$company_cond=" and a.company_id in($companyStr)";
	//$dys_issue_sql="select a.transaction_date,sum(case when a.transaction_type in(2,3,6) then a.cons_amount else 0 end) as cons_amount from inv_transaction a where a.item_category in(5,6,7,23) and a.status_active=1 $company_cond $sql_cond group by a.transaction_date";	

 
	$dys_issue_sql="select sum(case when a.transaction_type in(2,3,6) then a.cons_amount else 0 end) as cons_amount from inv_transaction a ,inv_issue_master b where b.id=a.mst_id and b.issue_purpose not in(3,5,26,27,28,29,30) and a.item_category in(5,6,7,23) and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $company_cond $sql_cond";	
 
 
		$dys_issue_sql_result = sql_select($dys_issue_sql);
		foreach($dys_issue_sql_result as $val){
			$dyc_amount_data_array+=$val[csf('cons_amount')];
		}
...
//Dyes/Chemical Avg Cost Per Kg.....................................end;
  ...
  $companyStr = implode(',',array_keys($company_library));
   ...
   $to_date=$current_date;
	...
	$finishigSql="select b.receive_qnty,b.uom from inv_receive_master a,pro_finish_fabric_rcv_dtls b where a.id=b.mst_id and a.entry_form=7 and a.item_category=2 and a.receive_basis=5 and a.knitting_company in($companyStr) and a.receive_date between '$from_date' and '$to_date' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0"; //and a.recv_number = 'FTML-FFPE-18-03926'
...
<table cellpadding="3" cellspacing="0" border="1" rules="all">
    ...
    <tr>
		...
		?>
        <td align="right"><? echo number_format($tot_poly_qty);?></td>
     </tr>  
    ...
    <tr>
		...
		?>
        <td align="right"><? echo number_format($tot_ex_fac_qty);?></td>
     </tr>  
    ...
    <tr>
		...
		foreach($DataArr as $compId=>$valArr){
			foreach($valArr as $folId=>$val){
			 if($ex_fac_val_arr[$compId][$folId]>0){
				 echo "<td align='right'>".number_format($ex_fac_val_arr[$compId][$folId],2)."</td>";
			}
			else{echo "<td align='right'></td>";}
		...
		?>
        <td align="right"><? echo number_format($tot_ex_fac_val,2);?></td>
     </tr>
     <!-- 
    <tr>
    	<td><? //echo $i++;?></td>
        ...
        <?
		/*foreach($DataArr as $compId=>$valArr){
			 echo "<td colspan=".count($valArr)." align='right'>".$ex_fac_fob_arr[$compId]."</td>";	
			 $tot_avg_ex_fac_fob+=$ex_fac_fob_arr[$compId];
		}*/
		?>
        <td align="right"><? //echo number_format($tot_avg_ex_fac_fob/4,2);?></td>
     </tr>
   ...
   <tr>
		...
		/*foreach($DataArr as $compId=>$valArr){
			...
			if($folId==19){echo "<td align='right'>".$sewing_efficiency_array[$compId][$folId]."</td>";}
			else{echo "<td align='right'>".$poly_efficiency_array[$compId][$folId]."</td>";}
			}
			$tot_poly_efficiency+=$poly_efficiency_array[$compId][$folId];
		}*/
		?>
        <td align="right"><? //echo number_format($tot_poly_efficiency/4,2);?></td>
     </tr>
    ...
    <tr>
        ...
        <?
	/*	foreach($DataArr as $compId=>$valArr){
			 echo "<td colspan=".count($valArr)." align='right'>".$avg_smv_qty_array[$compId]."</td>";	
			 $tot_avg_smv_qty+=$avg_smv_qty_array[$compId];
		}*/
		?>
        <td align="right"><? //echo number_format($tot_avg_smv_qty/4,2);?></td>
     </tr>
     -->
    <tr>
    	<td><? echo $i++;?></td>
		...
		foreach($DataArr as $compId=>$valArr){
			foreach($valArr as $folId=>$val){
			 echo "<td align='right'>".$dhu_qty_array[$compId][$folId]."</td>";	
			 $total_dhu_qty+=$dhu_qty_array[$compId][$folId];
			}
		}
		?>
        <td align="right"><? echo number_format($total_dhu_qty/4,2);?></td>
     </tr>
    ...
    <tr>
		...
		?>
        <td align="right"><? echo number_format($tot_cut_panel_rejection/4,2);?></td>
     </tr>
    ...
    <tr>
		...
		foreach($DataArr as $compId=>$valArr){
			foreach($valArr as $folId=>$val){
			 echo "<td align='right'>".$re_check_qty_array[$compId][$folId]."</td>";
			 $tot_re_check_qty+=$re_check_qty_array[$compId][$folId];	
			}
		}
		?>
        <td align="right"><? echo number_format($tot_re_check_qty/4,2);?></td>
     </tr>
     ...
     <tr>
		...
		?>
        <td align="right"><? echo number_format($tot_re_reject_parcentage_qty/4,2);?></td>
     </tr>
...
*/		
		...
		?>
        <td align="right"><? echo number_format($totManMachineRatio/3,2);?></td>
     </tr>
    ...
    <tr>
		...
		?>
        <td align="right"><? echo number_format($totAirFreightQty,3);?></td>
     </tr>
    ...
    <tr>
		...
		?>
        <td align="right"><? echo number_format($tot_packing_finishing_qty);?></td>
     </tr>
...
-->   
    ...
    <tr><td>1</td><td>Knitting – Inhouse (Ton)</td><td align="right"><? echo number_format($knite/1000);?></td></tr> 
   
   
    <tr><td>2</td><td>Knitting – Subcon. (Ton)</td><td align="right"><? echo number_format($knite_sub_qty/1000);?></td></tr> 
   
   
   
    <tr><td>3</td><td>Dyeing (Ton)</td><td align="right"><? echo number_format($batch_qty_data_array/1000);?></td></tr> 
  <tr><td>4</td><td>Finishing (Kg/Yds)</td><td align="right"><? echo number_format($finishQty[12]).' / '.number_format($finishQty[27]);?></td></tr>
    
    <tr><td>5</td><td>Ex-Lot (Re-Process) %</td><td align="right"><? echo number_format($re_process_qty_array,2);?></td></tr> 
    <tr><td>6</td><td>% of Compensation on Production</td><td align="right"><? echo number_format($parcent_of_compensation_on_production_qty_array,2);?></td></tr> 
    <tr><td>7</td><td>Dyes/Chemical Avg Cost Per Kg</td><td align="right"><? echo number_format($dyc_amount_data_array  / $batch_weight_data,2);?></td></tr> 
...
<?
 ...
 //$to='fakhrul@urmigroup.net,ehtesham@urmigroup.net';
