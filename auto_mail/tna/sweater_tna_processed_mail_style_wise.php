...
$res=sql_select("select id,user_id,tna_task_id,mail_type from tna_mail_setup where status_active=1 and is_deleted=0 AND TASK_TYPE=3");
foreach($res as $row)
{
...
$sql_dtls="select b.dealing_marchant,a.id,a.task_number,b.buyer_name,a.job_no,b.style_ref_no,min(a.task_start_date) as task_start_date,  min(a.shipment_date) as shipment_date,LISTAGG(a.po_number_id, ',') WITHIN GROUP (ORDER BY a.po_number_id) as PO_NUMBER_ID,LISTAGG(d.po_number, ',') WITHIN GROUP (ORDER BY d.po_number) as PO_NUMBER,sum((d.po_quantity * b.total_set_qnty)) AS PO_QTY_PCS from tna_process_mst a, wo_po_details_master b, wo_po_break_down d where a.job_no=b.job_no and a.po_number_id=d.id  and b.job_no=d.job_no_mst and a.task_number in(".$task_id_string.") and a.task_start_date between '".$current_date."' and '".$day_after_tomorrow."' and a.actual_start_date $actual_date   and b.company_name=$compid $user_company_con $user_buyer_con and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and d.status_active!=3 and d.shiping_status !=3 and d.is_confirmed=1 group by b.dealing_marchant,a.id,a.task_number,b.buyer_name,a.job_no,b.style_ref_no";	
				...
				<tr>
					<td valign="top" align="left">
						<table width="100%" cellpadding="0" cellspacing="0" border="1" rules="all">
							<tr>
								<td colspan="11"><span><? echo $company_name; ?></span><br /><strong>Task Starting Reminder</strong></td>
							</tr>
							<?  
							...
							{ 
								...
								<tr bgcolor="#DDDDDD">
                                    ...
                                    <td width="130"><strong>Order Qty (Pcs)</strong></td>
...
foreach($company_library as $compid=>$company_name)
{
	...
	$sql_dtls="select b.dealing_marchant,a.id,a.task_number,b.buyer_name,a.job_no,b.style_ref_no
,min(a.task_finish_date) as task_finish_date,  min(a.shipment_date) as shipment_date,LISTAGG(a.po_number_id, ',') WITHIN GROUP (ORDER BY a.po_number_id) as PO_NUMBER_ID,LISTAGG(d.po_number, ',') WITHIN GROUP (ORDER BY d.po_number) as PO_NUMBER,sum((d.po_quantity * b.total_set_qnty)) AS PO_QTY_PCS	
	
	from tna_process_mst a, wo_po_details_master b,  wo_po_break_down d where a.job_no=b.job_no and a.po_number_id=d.id and b.job_no=d.job_no_mst  and a.task_number in(".$task_id_string.") and a.task_finish_date between '".$current_date."' and '".$day_after_tomorrow."' and b.company_name=$compid $user_company_con $user_buyer_con and a.actual_finish_date $actual_date and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and d.status_active !=3 and d.shiping_status !=3 and d.is_confirmed=1 group by b.dealing_marchant,a.id,a.task_number,b.buyer_name,a.job_no,b.style_ref_no";
	//echo $sql_dtls;die;
	$nameArray_dtls=sql_select($sql_dtls);
			...
			<table>
				...
				<tr>
					<td valign="top" align="left">
						<table width="100%" cellpadding="0" cellspacing="0" border="1" rules="all">
							<tr>
								<td colspan="11" height="40"><span><? echo $company_name; ?></span><br /><strong>Task Complition Reminder</strong></td>
							</tr>
							<?
							$flag=0;
...
foreach($company_library as $compid=>$company_name)
{
	...
	$sql_dtls="select b.dealing_marchant,a.id,a.task_number,b.buyer_name,a.job_no,a.po_number_id,b.style_ref_no,
	min(a.shipment_date) as shipment_date,LISTAGG(a.po_number_id, ',') WITHIN GROUP (ORDER BY a.po_number_id) as PO_NUMBER_ID,LISTAGG(d.po_number, ',') WITHIN GROUP (ORDER BY d.po_number) as PO_NUMBER,sum((d.po_quantity * b.total_set_qnty)) AS PO_QTY_PCS		
	from tna_process_mst a, wo_po_details_master b, wo_po_break_down d where a.job_no=b.job_no and a.po_number_id=d.id  and b.job_no=d.job_no_mst and a.task_number in(".$task_id_string.") and a.task_start_date='".$prev_date."' and a.actual_start_date $actual_date and b.company_name=$compid $user_company_con $user_buyer_con and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and d.status_active !=3 and d.shiping_status !=3 and d.is_confirmed=1 group by b.dealing_marchant,a.id,a.task_number,b.buyer_name,a.job_no,a.po_number_id,b.style_ref_no";
	//echo $sql_dtls;die;
	$nameArray_dtls=sql_select($sql_dtls);
	foreach($nameArray_dtls as $rows)
	{
...
foreach($company_library as $compid=>$company_name)
{
	...
	$sql_dtls="select b.dealing_marchant,a.id,a.task_number,b.buyer_name,a.job_no,a.po_number_id,b.style_ref_no, 
	 min(a.shipment_date) as shipment_date,LISTAGG(a.po_number_id, ',') WITHIN GROUP (ORDER BY a.po_number_id) as PO_NUMBER_ID,LISTAGG(d.po_number, ',') WITHIN GROUP (ORDER BY d.po_number) as PO_NUMBER,sum((d.po_quantity * b.total_set_qnty)) AS PO_QTY_PCS		
	from tna_process_mst a, wo_po_details_master b, wo_po_break_down d where a.job_no=b.job_no and a.po_number_id=d.id  and   b.job_no=d.job_no_mst and a.task_number in(".$task_id_string.") and a.task_finish_date='".$prev_date."'  and a.actual_finish_date $actual_date and b.company_name=$compid $user_company_con $user_buyer_con and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1  and d.status_active !=3 and d.shiping_status !=3 and d.is_confirmed=1 group by b.dealing_marchant,a.id,a.task_number,b.buyer_name,a.job_no,a.po_number_id,b.style_ref_no";
	//echo $sql_dtls;die;
	$nameArray_dtls=sql_select($sql_dtls);
			...
			<table width="1000">
				...
				<tr>
					<td valign="top" align="left">
						<table width="100%" cellpadding="0" cellspacing="0" border="1" rules="all">
							<tr>
								<td colspan="9" height="40"><strong><span><? echo $company_name; ?></span><br />
...
foreach($company_library as $compid=>$company_name)
{
	...
	$sql_dtls="select b.dealing_marchant,a.id,a.task_number,b.buyer_name,a.job_no,b.style_ref_no, 
min(a.task_start_date) as task_start_date,  min(a.shipment_date) as shipment_date,LISTAGG(a.po_number_id, ',') WITHIN GROUP (ORDER BY a.po_number_id) as PO_NUMBER_ID,LISTAGG(d.po_number, ',') WITHIN GROUP (ORDER BY d.po_number) as PO_NUMBER,sum((d.po_quantity * b.total_set_qnty)) AS PO_QTY_PCS		
	from tna_process_mst a, wo_po_details_master b,  wo_po_break_down d where a.job_no=b.job_no  and a.po_number_id=d.id and b.job_no=d.job_no_mst and a.task_number in(".$task_id_string.") and a.task_start_date <='".$current_date."' and a.actual_start_date $actual_date and b.company_name=$compid $user_company_con $user_buyer_con and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and d.status_active=1 and d.shiping_status !=3 and d.is_confirmed=1 group by b.dealing_marchant,a.id,a.task_number,b.buyer_name,a.job_no,b.style_ref_no";
	 //echo $sql_dtls;die;
	$nameArray_dtls=sql_select($sql_dtls);
			...
			<table width="1000">
				...
				<tr>
					<td valign="top" align="left">
						<table width="100%" cellpadding="0" cellspacing="0" border="1" rules="all">
							<tr>
								<td colspan="11" height="40"><span><? echo $company_name; ?></span><br /><strong>Total Task Start Pending As On Today</strong></td>
							</tr>
							<?
							$flag=0;
...
$sql_dtls="select b.dealing_marchant,a.id,a.task_number,b.buyer_name,a.job_no, b.style_ref_no,
min(a.task_finish_date) as task_finish_date,  min(a.shipment_date) as shipment_date,LISTAGG(a.po_number_id, ',') WITHIN GROUP (ORDER BY a.po_number_id) as PO_NUMBER_ID,LISTAGG(d.po_number, ',') WITHIN GROUP (ORDER BY d.po_number) as PO_NUMBER,sum((d.po_quantity * b.total_set_qnty)) AS PO_QTY_PCS	
from tna_process_mst a, wo_po_details_master b,  wo_po_break_down d where a.job_no=b.job_no  and a.po_number_id=d.id  and b.job_no=d.job_no_mst and a.task_number in(".$task_id_string.") and a.task_finish_date <='".$current_date."' and a.actual_finish_date $actual_date and b.company_name=$compid $user_company_con $user_buyer_con and a.is_deleted=0 and a.status_active=1 and d.status_active=1 and b.is_deleted=0 and b.status_active=1 and d.shiping_status !=3 and d.status_active !=3  and d.is_confirmed=1 group by b.dealing_marchant,a.id,a.task_number,b.buyer_name,a.job_no, b.style_ref_no";
//echo $sql_dtls;die;
$nameArray_dtls=sql_select($sql_dtls);
foreach($nameArray_dtls as $rows)
{
