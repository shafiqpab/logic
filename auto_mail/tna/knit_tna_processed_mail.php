...
$task_short_name = return_library_array("select task_name,task_short_name from lib_tna_task where status_active=1 and is_deleted=0 and task_type=1", "task_name", "task_short_name");
...
$res = sql_select("select id,user_id,tna_task_id,mail_type from tna_mail_setup where status_active=1 and is_deleted=0 AND TASK_TYPE=1");
foreach ($res as $row) {
...
foreach ($company_library as $compid => $company_name) {
	...
	foreach ($mail_type_wise_data[1] as $user_id => $task_id) {
		if ($user_company_arr[$user_id] == '') {
			$user_company_arr[$user_id] = implode(',', array_keys($company_library));
		}
				...
				<tr>
					<td valign="top" align="left">
						<table width="100%" cellpadding="0" cellspacing="0" border="1" rules="all">
							<tr>
								<td colspan="12" height="40"><span><? echo $company_name; ?></span><br /><strong>Task Starting Reminder [Next Three Days Plan Start Date  <span style="color:#f00;"> <?= $current_date.', '.$tomorrow.', '.$day_after_tomorrow;?></span>]</strong></td>
							</tr>
							<?
							$flag = 0;
							...
							?>
								...
								<tr bgcolor="#DDDDDD">
									...
									<td width="93"><strong style="color:#f00;"><? echo change_date_format($current_date); ?></strong></td>
									<td width="86"><strong style="color:#f00;"><? echo change_date_format($tomorrow); ?></strong></td>
									<td width="90"><strong style="color:#f00;"><? echo change_date_format($day_after_tomorrow); ?></strong></td>
								</tr>
								<?
								$i = 0;
			...
			<table width="1020">
				...
				<tr>
					<td valign="top" align="left">
						<table width="100%" cellpadding="0" cellspacing="0" border="1" rules="all">
							<tr>
								<td colspan="12" height="40"><span><? echo $company_name; ?></span><br /><strong>Task Complition Reminder [Next Three Days Plan Finish Date  <span style="color:#f00;"> <?= $current_date.', '.$tomorrow.', '.$day_after_tomorrow;?></span>]</strong></td>
							</tr>
							<?
							$flag = 0;
							...
							?>
								...
								<tr>
									...
									<td width="100"><strong>Order Qty (Pcs)</strong></td>
									<td width="80"><strong>Ship Date</strong></td>
									<td width="80"><strong style="color:#f00;"><? echo change_date_format($current_date); ?></strong></td>
									<td width="80"><strong style="color:#f00;"><? echo change_date_format($tomorrow); ?></strong></td>
									<td width="90"><strong style="color:#f00;"><? echo change_date_format($day_after_tomorrow); ?></strong></td>
								</tr>
								<?
								$i = 0;
								...
								?>
										<tr>
											...
											<td>
												<?
												if (change_date_format($current_date) == change_date_format($row_dtls[csf('task_finish_date')]))
													echo change_date_format($row_dtls[csf('task_finish_date')]);
												?>
											</td>
											<td>
												<?
												if (change_date_format($tomorrow) == change_date_format($row_dtls[csf('task_finish_date')]))
													echo change_date_format($row_dtls[csf('task_finish_date')]);
												?>
											</td>
											<td>
												<?
												if (change_date_format($day_after_tomorrow) == change_date_format($row_dtls[csf('task_finish_date')]))
													echo change_date_format($row_dtls[csf('task_finish_date')]);
												?>
											</td>
										</tr>
						...
						<table width="100%" cellpadding="0" cellspacing="0" border="1" rules="all">
							<tr>
								<td colspan="9" height="40"><span><? echo $company_name; ?></span><br />
									<strong>Yesterday Start Pending Of &nbsp;(&nbsp;Date :&nbsp;<? echo change_date_format($prev_date); ?>&nbsp;)</strong>
								</td>
							</tr>
							<?
...
// echo "YES";die;
//print_r($company_library);
//die;
// problem
foreach ($company_library as $compid => $company_name) {
	//print_r($mail_type_wise_data[5]);die;
	foreach ($mail_type_wise_data[5] as $user_id => $task_id) {
		...
		if (in_array($compid, $companyArr)) {
			...
			// print_r($TotalTaskStartPendingAsOnTodayArr);die;
