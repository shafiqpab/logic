...
if($action=="job_no_popup")
{
    ...
    {
        $year_field="year(a.insert_date) as year"; 
        $year_orderby="year(a.insert_date)";
    }
    else if($db_type==2) 
    {
        $year_field="to_char(a.insert_date,'YYYY') as year";
        $year_orderby="to_char(a.insert_date,'YYYY')";
    }
    else 
    {
        $year_field="";
    ...
    $sql="select a.id,a.job_no_prefix_num,a.subcon_job as job_no ,a.party_id as buyer_name, $year_field from subcon_ord_mst a where  a.company_id=$company_id $sub_buyer_name_cond $sub_location_name_cond and a.is_deleted =0 and a.job_no_prefix_num is not null order by a.id DESC";
    //echo $sql;
    ...
    <div style="max-height:340px; overflow:auto;">
        <table id="table_body2" width="500" border="1" rules="all" class="rpt_table">
            ...
            foreach($rows as $data)
            {
                if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
                ?>
                <tr bgcolor="<? echo $bgcolor; ?>" onclick="js_set_value('<? echo $data[csf('id')].'_'.$data[csf('job_no_prefix_num')]; ?>')" style="cursor:pointer;">
                    <td width="30"><? echo $i; ?></td>
                    
                    <td width="50"><p><? echo $data[csf('job_no_prefix_num')]; ?></p></td>
                    <td width="80"><p><? echo $buyer[$data[csf('buyer_name')]]; ?></p></td>
                    <td width="40" align="center"><p><? echo $data[csf('year')]; ?></p></td>
                </tr>
                <? $i++; 
...
{ 
    ...
    if (!empty($txt_job_id)) $job_id_cond=" and a.id = $txt_job_id"; else $job_id_cond="";
    ...
    WHERE    a.id=c.mst_id AND c.id = b.order_id AND a.is_deleted = 0 AND a.status_active = 1 AND b.is_deleted = 0 AND b.status_active = 1 AND c.is_deleted=0 
        $company_name $buyer_name $location $floor $txt_date $order_no_cond $job_no_cond $job_id_cond";
    //echo  $sql;
    ...
    foreach ($sql_res as $key => $row) 
    {
       ...
       //$order_id = implode(",", $order_id_array);
	    ...
	    $con = connect();
		$r_id=execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_id and entry_form in (44)");
		if($r_id)
		{
			oci_commit($con);
		}
		...
		{
			fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 44, 2,$order_id_array, $empty_arr);
		}
  ...
  /*  $sql_prod = "SELECT a.party_id,b.gmts_item_id, c.id,
        ...
        SUM (CASE WHEN production_type in(1,2,3,4,7) THEN reject_qnty ELSE 0 END) AS reject_qnty
    FROM subcon_ord_mst a, subcon_gmts_prod_dtls b, subcon_ord_dtls c
    WHERE    c.job_no_mst = a.subcon_job AND c.id = b.order_id AND a.is_deleted = 0 AND a.status_active = 1 AND b.is_deleted = 0
        AND b.status_active = 1 and c.is_deleted=0 and b.order_id in($order_id)
    ...
    b.gmts_item_id,
    c.id,
    SUM(CASE WHEN d.production_type = '1' THEN d.prod_qnty ELSE 0 END) AS cutting_qnty,
    SUM(CASE WHEN d.production_type = '7' THEN d.prod_qnty ELSE 0 END) AS sewing_input_qnty,
    SUM(CASE WHEN d.production_type = '2' THEN d.prod_qnty ELSE 0 END) AS sewingout_qnty,
    SUM(CASE WHEN d.production_type = '3' THEN d.prod_qnty ELSE 0 END) AS iron_qnty,
    SUM(CASE WHEN d.production_type = '4' THEN d.prod_qnty ELSE 0 END) AS finish_qnty,
    SUM(CASE WHEN d.production_type IN (1,2,3,4,7) THEN b.reject_qnty ELSE 0 END) AS reject_qnty
FROM
    subcon_ord_mst a
JOIN
    subcon_ord_dtls c ON c.mst_id = a.id
JOIN
    subcon_gmts_prod_dtls b ON c.id = b.order_id
JOIN
    subcon_gmts_prod_col_sz d ON b.id = d.dtls_id
JOIN
    GBL_TEMP_ENGINE e ON c.id = e.ref_val
WHERE
    e.entry_form = 44
...
GROUP BY
    ...
    c.id"; */ 
        ...
        SUM (CASE WHEN d.production_type = '4' THEN d.prod_qnty ELSE 0 END) AS finish_qnty,
        SUM (CASE WHEN d.production_type in(1,2,3,4,7) THEN b.reject_qnty ELSE 0 END) AS reject_qnty
    FROM subcon_ord_mst a, subcon_gmts_prod_dtls b, subcon_ord_dtls c,subcon_gmts_prod_col_sz d, GBL_TEMP_ENGINE e
    WHERE  c.mst_id = a.id AND c.id = b.order_id and  b.id=d.dtls_id  AND a.is_deleted = 0 AND a.status_active = 1 AND b.is_deleted = 0 AND b.status_active = 1 and c.is_deleted=0  and c.id=e.ref_val and e.entry_form=44  and e.user_id=$user_id and e.ref_from=2 
    GROUP BY a.party_id,b.gmts_item_id, c.id";   
    ...
    // echo "<pre>";
    // print_r($party_data_array);
    // echo "</pre>";
    // ======================================== FOR ORDER QNTY =================================
    //$order_id_cond=where_con_using_array(array_filter(array_unique(explode(",", $order_id))),0,"d.order_id");
	
	 
	
   $order_sql = "SELECT a.party_id, sum(d.qnty) as po_quantity, sum(d.amount) as po_total_price,c.id,d.item_id 
    from subcon_ord_mst a, subcon_ord_dtls c, subcon_ord_breakdown d , GBL_TEMP_ENGINE e
    where c.mst_id = a.id  and a.id=d.mst_id and c.id=d.order_id and a.status_active=1 and c.is_deleted=0 $company_name $buyer_name $location $floor   and c.id=e.ref_val and e.entry_form=44  and e.user_id=$user_id and e.ref_from=2 
    ...
    // echo "<pre>";
    // print_r($main_order_data);
    // echo "</pre>";
    // ======================================== FOR EXFACTORY ===================================
    //$order_id_cond=where_con_using_array(array_filter(array_unique(explode(",", $order_id))),0,"c.id");
	
	
	 
	
      $exfactory_sql="SELECT a.party_id,d.order_id, (CASE WHEN d.order_id=c.id THEN d.delivery_qty ELSE 0 END) as delivery_quantity 
    from subcon_ord_mst a, subcon_ord_dtls c, subcon_delivery_dtls d, GBL_TEMP_ENGINE e 
    where c.id=d.order_id and c.mst_id = a.id  and a.is_deleted=0 and a.status_active=1 and c.is_deleted=0 and c.status_active=1 $company_name $buyer_name $location $floor   and c.id=e.ref_val and e.entry_form=44  and e.user_id=$user_id and e.ref_from=2";  
    ...
    foreach($exfactory_sql_result as $resRow)
    {
        //$exfactory_arr[$resRow[csf("party_id")]] = $resRow[csf("delivery_quantity")];
		   if($resRow[csf("delivery_quantity")])
		   {
			$exfactory_party_arr[$resRow[csf("order_id")]] = $resRow[csf("party_id")];
		   }
    }
    // echo "<pre>";
    // print_r($exfactory_arr);
    // echo "<pre>";
    // ========================================== FOR EXFACT DEL. DATE ===================================
   // $order_id_cond=where_con_using_array(array_filter(array_unique(explode(",", $order_id))),0,"b.order_id");
   
     $sqlEx = "SELECT  b.order_id,b.item_id,MAX(a.delivery_date) AS ex_fac_date, sum(c.delivery_qty) AS ex_fac_qnty from subcon_delivery_mst a, subcon_delivery_dtls b, subcon_gmts_delivery_dtls c, GBL_TEMP_ENGINE e where a.id=b.mst_id and b.id = c.dtls_mst_id and a.id = c.mst_id and b.order_id=e.ref_val and e.entry_form=44  and e.user_id=$user_id and e.ref_from=2 group by  b.order_id,b.item_id";    
    ...
    foreach ($sqlEx_res as $key => $val) 
    {
        ...
        $sqlEx_date_data[$val[csf('order_id')]][$val[csf('item_id')]]['ex_fac_qnty'] = $val[csf('ex_fac_qnty')];
		
		$party_id=$exfactory_party_arr[$val[csf("order_id")]];
		$exfactory_arr[$party_id]+=$val[csf('ex_fac_qnty')];
    }
    ...
    from subcon_inbound_bill_mst a, subcon_inbound_bill_dtls b, GBL_TEMP_ENGINE e
    where a.id=b.mst_id and b.order_id=e.ref_val and e.entry_form=44  and e.user_id=$user_id and e.ref_from=2 and a.status_active=1 and b.status_active=1";  
    ...
    subcon_inbound_bill_dtls d, GBL_TEMP_ENGINE e
     WHERE a.id = b.master_id and b.bill_no = c.BILL_NO and c.id=d.mst_id and d.order_id=e.ref_val and e.entry_form=44  and e.user_id=$user_id and e.ref_from=2 and a.status_active=1 and b.status_active=1";   
        ...
        <div style="float:left; width:750px">
            ...
            <div style="max-height:425px; width:1150px" >
                ...
                <div>                   
                    ...
                    <table width="2230" cellspacing="0" border="1" class="rpt_table" rules="all" id="table_header_1">
                        <thead>
                            <tr>
                                <th width="40">SL</th>    
                    ...
                    <div style="max-height:425px; width:2250px" id="scroll_body">
                        <table border="1" class="rpt_table"  width="2230" rules="all" id="table_body" >
                            <?
                           $i=0; $k=0;
                           // initialize 
                                ...
                                foreach($orderData as $item_id=>$orderRes)
                                {
                                   ...
                                   if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";?>                                   
                                    ...
                                    <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_2nd<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_2nd<? echo $i; ?>" style="height:20px">
                                        ...
                                        <td width="80" align="right">
                                            <?php $company_name=str_replace("'","",$cbo_company_id); ?>
                                            <a href="##" onclick="openmypage_order_2('<?=$order_id?>','<?=$company_name?>','<?=$item_id?>',0,'OrderPopup')">
                                                <? 
                                        ...
                                        <td width="80" align="right">
                                            <a href="javascript:void()" onclick="prodPoppup(<? echo $order_id.",".$item_id;?>,1,'Cutting Quantity','production_popup');">
                                        ...
                                        <td width="80" align="right">
                                            <a href="javascript:void()" onclick="prodPoppup(<? echo $order_id.",".$item_id;?>,7,'Sewing Input Quantity','production_popup');">
                                        ...
                                        <td width="80" align="right">
                                            <a href="javascript:void()" onclick="prodPoppup(<? echo $order_id.",".$item_id;?>,2,'Sewing Output Quantity','production_popup');">
                                        ...
                                        <td width="80" align="right">
                                            <a href="javascript:void()" onclick="prodPoppup(<? echo $order_id.",".$item_id;?>,3,'Iron Quantity','production_popup');">
                                                <? echo number_format($prod_data_array[$order_id][$item_id]["iron_qnty"]); $total_iron+=$prod_data_array[$order_id][$item_id]["iron_qnty"]; ?>
                                            </a>    
                                        </td>
                                        <td width="80" align="right">
                                            <a href="javascript:void()" onclick="prodPoppup(<? echo $order_id.",".$item_id;?>,4,'Finishing Quantity','production_popup');">
                                        ...
                                        ?>
                                        <td width="80" align="right">
                                            <a href="javascript:void()" onclick="prodPoppup(<? echo $order_id.",".$item_id;?>,1,'Delivery Quantity','ex_factory_popup');">
                                                <? echo number_format($tot_delivery); ?>
                                            </a>  
                                        </td>
                                        <td width="80" align="right">
                                            <a href="javascript:void()" onclick="prodPoppup(<? echo $order_id.",".$item_id;?>,1,'Reject Quantity','reject_popup');">
                        ...
                        <table border="1" class="tbl_bottom"  width="2230" rules="all" id="report_table_footer_1" >
                            <tr>
                                <td width="40"></td>
                                <td width="130"></td>
                                <td width="100"></td>
...
if($action=="ex_factory_popup")
{
    ...
    $sizearr_order=return_library_array("select size_id from subcon_ord_breakdown where order_id in($order_id)","size_id","size_id");
    // print_r($sizearr_order);
    $sql="SELECT d.color_id,d.size_id,d.item_id,sum (c.delivery_qty) as delivery_qty
    from subcon_delivery_mst a, subcon_delivery_dtls b,subcon_gmts_delivery_dtls c,subcon_ord_breakdown d
    where a.id = b.mst_id and b.id = c.dtls_mst_id and a.id = c.mst_id and d.id = c.breakdown_color_size_id and b.order_id = b.order_id and b.order_id =$order_id and b.item_id=$item_id
    group by d.color_id, d.size_id, d.item_id";
    $sql_res = sql_select($sql)    ;
    $job_size_array         = array();
    $job_size_qnty_array    = array();
    $job_color_array        = array();
    ...
    $job_sql="SELECT a.party_id,a.subcon_job,b.cust_style_ref, b.order_no
            from subcon_ord_mst a, subcon_ord_dtls b 
            where a.id= b.mst_id and b.id=$order_id and a.status_active=1 and b.status_active=1";
    $job_sql_res = sql_select($job_sql);
    ?>
    <fieldset>
        <div class="main">
...
if($action=="production_popup")
{
    ...
    where a.id=b.mst_id and a.id=c.mst_id and b.id=c.order_id and c.order_id=b.id  and b.id=$order_id and c.item_id='$item_id' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0   
...
if ($action=='OrderPopup')
{
    ...
    $sql_color_size="select order_id,color_id,size_id,qnty from subcon_ord_breakdown where order_id=$po_break_down_id and is_deleted=0 and status_active=1";
    //echo $sql_color_size;
    $color_size=sql_select($sql_color_size);
    $size_arr=array();
    $color_arr=array();
    $color_size_wise_order_qnty=array();
    foreach ($color_size as $row) {
       // array_push($size_arr, $row[csf('size_id')]);
       // array_push($color_arr, $row[csf('color_id')]);
        $size_arr[$row[csf('size_id')]]= $row[csf('size_id')];
        $color_arr[$row[csf('color_id')]]=$row[csf('color_id')];
        $color_size_wise_qnty[$row[csf('color_id')]][$row[csf('size_id')]]['qnty']+=$row[csf('qnty')];
    }
    ...
    <div style="width:700px" align="center" id="details_reports"> 
        ...
        <table id="tbl_id" class="rpt_table" width="" border="1" rules="all" >
            ...
            <?
                ...
                $sql = "SELECT a.party_id,a.job_no_prefix_num,c.order_no,b.gmts_item_id, c.id, c.cust_style_ref,c.order_id,c.delivery_date, b.remarks,a.subcon_job,c.order_quantity
                FROM subcon_ord_mst a, subcon_gmts_prod_dtls b, subcon_ord_dtls c
                WHERE    c.job_no_mst = a.subcon_job AND c.id = b.order_id AND a.is_deleted = 0 AND a.status_active = 1 AND b.is_deleted = 0 AND b.status_active = 1 AND c.is_deleted=0  and c.id in ($po_break_down_id) and b.gmts_item_id=$item_id
                     ";
                //echo $sql;die;
            ...
            ?> 
            <tr>
                <td><? echo $buyer_short_library[$resultRow[0][csf("party_id")]]; ?></td>
                <td><p><? echo $resultRow[0][csf("subcon_job")]; ?></p></td>
                <td><p><? echo implode(",",array_unique(explode(",",$resultRow[0][csf("order_no")]))); ?></p></td>
                <td><? echo change_date_format($resultRow[0][csf("delivery_date")]); ?></td>
                <td><? echo $garments_item[$item_id]; ?></td>
                <td><? echo $resultRow[0][csf("order_quantity")]; ?></td>
            </tr>
        ...
        <?
          ...
          $sql_gmt="SELECT c.color_id, c.size_id,a.order_id, 
                    NVL(sum(CASE WHEN a.production_type ='1' THEN b.prod_qnty  ELSE 0 END),0) AS cutting_qnty,
                    NVL(sum(CASE WHEN a.production_type ='2' THEN b.prod_qnty ELSE 0 END),0) AS sewingout_qnty,
                    NVL(sum(CASE WHEN a.production_type ='3' THEN b.prod_qnty ELSE 0 END),0) AS iron_qnty,
                    NVL(sum(CASE WHEN a.production_type ='4' THEN b.prod_qnty ELSE 0 END),0) AS pack_qnty,
                    NVL(sum(CASE WHEN a.production_type ='5' THEN b.prod_qnty ELSE 0 END),0) AS poly_qnty,
                    NVL(sum(CASE WHEN a.production_type ='7' THEN b.prod_qnty ELSE 0 END),0) AS sewingin_qnty
                from 
                    subcon_gmts_prod_dtls a, subcon_gmts_prod_col_sz b,subcon_ord_breakdown c
                where  
                   a.id = b.dtls_id   AND a.order_id in ($po_break_down_id)
                   and c.id=b.ord_color_size_id
                   and a.status_active = 1
                   and a.is_deleted = 0
                   and a.production_type in (1,2,3,4,5,7)
            group by    c.color_id, c.size_id,a.order_id";
        // echo $sql_gmt;
          $result_gmt=sql_select($sql_gmt);
          foreach ($result_gmt as  $row) {
              $color_size_wise_qnty[$row[csf('color_id')]][$row[csf('size_id')]]['cutting_qnty']+=$row[csf('cutting_qnty')];
              $color_size_wise_qnty[$row[csf('color_id')]][$row[csf('size_id')]]['sewingin_qnty']+=$row[csf('sewingin_qnty')];
              $color_size_wise_qnty[$row[csf('color_id')]][$row[csf('size_id')]]['sewingout_qnty']+=$row[csf('sewingout_qnty')];
              $color_size_wise_qnty[$row[csf('color_id')]][$row[csf('size_id')]]['iron_qnty']+=$row[csf('iron_qnty')];
              $color_size_wise_qnty[$row[csf('color_id')]][$row[csf('size_id')]]['poly_qnty']+=$row[csf('poly_qnty')];
              $color_size_wise_qnty[$row[csf('color_id')]][$row[csf('size_id')]]['pack_qnty']+=$row[csf('pack_qnty')];
          ...
          $width= $count*70+350;        
        ?>
        <div style="color:#FF0000; font-size:14px; font-weight:bold; float:left; width:700px"></div>
        <table id="tblDtls_id" class="rpt_table" width="<? echo $width; ?>" border="1" rules="all" >
            <thead>
                <tr>
                    ...
                    <?
                    foreach($size_arr as $sizeId=>$val)
                    {
                    ?>
            ...
            <?
              ...
              foreach($color_arr as $colorId=>$totalorderqnty)
              {
                  $row_span=7;  
                ?>    
                 <?
                ...
                <tr bgcolor="<? echo $bgcolor1; ?>">
                    ...
                    <? 
                    $color_size_qty=0;
                    foreach($size_arr as $sizeId=>$sizeRes)
                    {
                        $color_size_qty+=$color_size_wise_qnty[$colorId][$sizeId]['qnty'];
                    ?>  
                        <td><? echo $color_size_wise_qnty[$colorId][$sizeId]['qnty']; ?></td>
                    <? 
                    } 
                    ?>
                    <td><? echo $color_size_qty; ?></td> 
                </tr>
               
                <?
                    $total_cutting=0;$total_sew_in=0;$total_sew_out=0;$total_iron_out=0;$total_pack=0;$total_poly_qnty=0;
                    $cutting_html='';$sewin_html='';$sewout_html='';$iron_html='';  $poly_qnty_html=''; $pack_html=''; 
                    foreach($size_arr as $sizeId=>$sizeRes)
                    { 
                      
                        $cutting_qnty=$color_size_wise_qnty[$colorId][$sizeId]['cutting_qnty'];
                        $sewingin_qnty=$color_size_wise_qnty[$colorId][$sizeId]['sewingin_qnty'];
                        $sewingout_qnty=$color_size_wise_qnty[$colorId][$sizeId]['sewingout_qnty'];
                        $iron_qnty=$color_size_wise_qnty[$colorId][$sizeId]['iron_qnty'];
                        $poly_qnty=$color_size_wise_qnty[$colorId][$sizeId]['poly_qnty'];
                        $pack_qnty=$color_size_wise_qnty[$colorId][$sizeId]['pack_qnty'];


                        
                        
                        
                        $cutting_html.='<td>'.$cutting_qnty.'</td>';
                        $total_cutting+=$cutting_qnty;
                        
                        
                    
                        $sewin_html .='<td >'.$sewingin_qnty.'</td>';
                        $total_sew_in+=$sewingin_qnty;
                        
                       
                        $sewout_html .='<td >'.$sewingout_qnty.'</td>';
                        ...
                        $pack_html .='<td >'.$pack_qnty.'</td>';
                        $total_pack+=$pack_qnty;


                        $total_poly_qnty+=$poly_qnty; 
                        $poly_qnty_html .='<td >'.$poly_qnty.'</td>';
                       


                      
                        $iron_html .='<td >'.$iron_qnty.'</td>';
                    ...
                    ?>
                        ...
                        <tr bgcolor="<? echo $bgcolor2; ?>">
                            <td><b>Iron Entry</b></td>
                            <? echo $iron_html; ?> 
                            <td><? echo $total_iron_out; ?></td> 
                        </tr>

                        <tr bgcolor="<? echo $bgcolor1; ?>">
                            <td><b>Poly Entry</b></td>
                            <? echo $poly_qnty_html; ?> 
                            <td><? echo $total_poly_qnty; ?></td> 
                        </tr>

                        <tr bgcolor="<? echo $bgcolor2; ?>">
                            <td><b>Packing And Finishing Entry.</b></td>
                            <? echo $pack_html; ?> 
                            <td><? echo $total_pack; ?></td> 
                        </tr>
