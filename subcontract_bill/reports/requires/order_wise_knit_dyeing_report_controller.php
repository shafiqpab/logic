...
if($action=="report_generate")
{
	...
	$job_number = str_replace("'","",$txt_job_no);
	$job_number_id = str_replace("'","",$hid_job_id);
	...
	$StyleCond=($txt_style_no )? " and d.style_ref_no='".$txt_style_no ."'" : '';
...
if($txt_date_from && $txt_date_to)
	...
	if($cbo_type==1)//Date Wise Report
		...
		$sql="
			select a.batch_no, a.batch_date,a.batch_weight,a.color_id,a.booking_no_id,a.extention_no,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.po_id,b.prod_id,b.width_dia_type,listagg(c.po_number,',' ) within group (order by c.po_number) AS po_number,c.job_no_mst,d.job_no_prefix_num,d.buyer_name from pro_batch_create_dtls b,wo_po_break_down c,wo_po_details_master d,pro_batch_create_mst a where a.company_id=$company $dates_com $jobdata $batch_num $buyerdata $StyleCond $order_no $ext_no $year_cond and a.entry_form=0 and a.id=b.mst_id and b.po_id=c.id and d.job_no=c.job_no_mst and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 GROUP BY a.batch_no, b.item_description,b.prod_id,a.batch_date, a.batch_weight, a.color_id, a.booking_no_id, a.extention_no,b.po_id, b.prod_id, b.width_dia_type,c.job_no_mst, d.job_no_prefix_num, d.buyer_name order by a.batch_no
			";
		}
	else if($cbo_type==2) //wait for Heat Setting
	{
	if($row_h!=0)
			{
		echo $sql="
			select a.batch_no,a.batch_date,a.batch_weight,a.color_id,a.booking_no_id,a.extention_no,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.po_id,b.prod_id,b.width_dia_type,listagg(c.po_number,',' ) within group (order by c.po_number) AS po_number,c.job_no_mst,d.job_no_prefix_num,d.buyer_name from pro_batch_create_dtls b,wo_po_break_down c,wo_po_details_master d,pro_batch_create_mst a where a.company_id=$company $dates_com $jobdata $batch_num $buyerdata $order_no $StyleCond $ext_no $year_cond and a.entry_form=0 and a.id=b.mst_id and b.po_id=c.id and d.job_no=c.job_no_mst and a.batch_against=2 $find_inset and a.id not in($row_heat) and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 GROUP BY a.batch_no, b.item_description,a.batch_date, a.batch_weight, a.color_id, a.booking_no_id, a.extention_no,b.po_id, b.prod_id, b.width_dia_type,c.job_no_mst, d.job_no_prefix_num, d.buyer_name order by a.batch_no ";
			}
		}
	else if($cbo_type==3)// Wait For Dyeing
	{
	...
	if($row_d!=0)
			{
		$sql="
			select a.batch_no,a.batch_date,a.batch_weight,a.color_id,a.booking_no_id,a.extention_no,SUM(b.batch_qnty) AS batch_qnty,b.item_description,b.po_id,b.prod_id,b.width_dia_type,listagg(c.po_number,',' ) within group (order by c.po_number) AS po_number,c.job_no_mst,d.job_no_prefix_num,d.buyer_name
	from pro_batch_create_dtls b,wo_po_break_down c,wo_po_details_master d,pro_batch_create_mst a where a.company_id=$company $dates_com $jobdata $batch_num $buyerdata $order_no $ext_no $year_cond  $StyleCond and a.entry_form=0 and a.id=b.mst_id and b.po_id=c.id and d.job_no=c.job_no_mst and a.batch_against=2 and a.id not in($row_d) and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 GROUP BY a.batch_no, b.item_description,a.batch_date, a.batch_weight, a.color_id, a.booking_no_id, a.extention_no,b.po_id, b.prod_id, b.width_dia_type,c.job_no_mst, d.job_no_prefix_num, d.buyer_name order by a.batch_no";
			}
		 }	
	}
	//echo $sql;//die;
...
?>
            ...
            <? if (!in_array($batch[csf('batch_no')],$batch_chk_arr) )
                ...
                <td align="center" width="50" title="<? echo $desc[3];  ?>"><p><? echo $desc[3]; ?></p></td>
                <td align="center" width="50" title="<? echo $desc[2]; ?>"><p><? echo $desc[2]; ?></p></td>
                 <td align="left" width="60" title="<? echo $yarn_lot_arr[$batch[csf('prod_id')]]; ?>"><p><? echo $yarn_lot_arr[$batch[csf('prod_id')]][$order_id]; ?></p></td>
        ...
        <tfoot>
            ...
            <tr>
             ...
             <tr>
                ...
                <td colspan="11" align="left">&nbsp; <? echo number_format($btq,2); ?> </td>
            </tr>
             <tr>
