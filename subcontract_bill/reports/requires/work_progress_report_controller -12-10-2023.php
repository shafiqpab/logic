...
{ 
	...
	else
	{
		...
		//===================================== main query =================================
		 $job_sql = "SELECT a.id as pic_id,a.job_no_prefix_num, a.subcon_job, a.party_id, b.order_no, b.order_quantity, b.amount, b.order_rcv_date, b.delivery_date, b.main_process_id, b.order_uom, b.id, b.cust_style_ref 
		from subcon_ord_mst a, subcon_ord_dtls b
		where  a.id=b.mst_id and a.company_id=$cbo_company_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $order_no_cond $style_ref_cond $date_cond $job_no_cond $process_id_cond $buyer_id_cond $date_cond_recv $year_cond
			group by a.job_no_prefix_num, a.subcon_job, a.company_id, a.party_id, b.order_no, b.order_quantity, b.amount, b.order_rcv_date, b.delivery_date, b.main_process_id, b.order_uom, b.id, b.cust_style_ref,a.id  order by b.main_process_id, a.job_no_prefix_num, b.order_no, b.delivery_date ";
			//echo $job_sql;
		$job_sql_result=sql_select($job_sql);
		if(count($job_sql_result)==0)
		...
		//================================================================
		 $inventory_array=array();
 		 $inventory_sql="SELECT b.order_id, sum(b.quantity) as quantity from  sub_material_mst a, sub_material_dtls b, GBL_TEMP_ENGINE e where a.id=b.mst_id and a.trans_type=1 and a.status_active=1 and b.is_deleted=0 and b.status_active=2 and b.is_deleted=0   and a.entry_form=288  and b.order_id=e.ref_val and e.entry_form=23 and e.user_id=$user_id and e.ref_from=1 group by b.order_id"; 
		// echo $inventory_sql;
		$inventory_sql_result=sql_select($inventory_sql);
		foreach ($inventory_sql_result as $row)
		{
			$recv_inventory_array[$row[csf('order_id')]]['quantity']=$row[csf('quantity')];
		}
		// print_r($inventory_array);
		//=========================================================================
		 $inv_iss_array=array();
 		 $inv_iss_sql="SELECT b.order_id, sum(b.quantity) as quantity from  sub_material_mst a, sub_material_dtls b, GBL_TEMP_ENGINE e where a.id=b.mst_id and a.trans_type=2 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and a.entry_form=343 and a.entry_form=288  and b.order_id=e.ref_val and e.entry_form=23 and e.user_id=$user_id and e.ref_from=1 group by b.order_id";
		$inv_iss_sql_result=sql_select($inv_iss_sql);
		foreach ($inv_iss_sql_result as $row)
		{
			$inv_iss_array[$row[csf('order_id')]]['quantity']=$row[csf('quantity')];
		...
		//===================================================================
		$inventory_ret_array=array();
 		$inv_ret_sql="SELECT b.order_id, sum(b.quantity) as quantity from  sub_material_return_mst a, sub_material_return_dtls b , GBL_TEMP_ENGINE e where a.id=b.mst_id  and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and b.order_id=e.ref_val and e.entry_form=23 and e.user_id=$user_id and e.ref_from=1 group by b.order_id";
		//echo $inv_ret_sql;die;
		$inv_ret_sql_result=sql_select($inv_ret_sql);
		foreach ($inv_ret_sql_result as $row)
		{
			$recv_inventory_ret_array[$row[csf('order_id')]]['quantity']=$row[csf('quantity')];
		}	//var_dump($inventory_array);

		//===================================================================
		$inventory_iss_ret_array=array();
 		$inv_iss_ret_sql="SELECT b.order_id, sum(b.quantity) as quantity from  sub_material_mst a, sub_material_dtls b, GBL_TEMP_ENGINE e where a.id=b.mst_id and a.trans_type=3 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form=344 and b.order_id=e.ref_val and e.entry_form=23 and e.user_id=$user_id and e.ref_from=1 group by b.order_id";
		
		$inv_iss_ret_sql_result=sql_select($inv_iss_ret_sql);
		foreach ($inv_iss_ret_sql_result as $row)
		{
			$inventory_iss_ret_array[$row[csf('order_id')]]['quantity']=$row[csf('quantity')];
		}	//var_dump($inventory_array);
		//=============================================================
		$delivery_array=array();
		 $delivery_sql="SELECT b.order_id,
		...
		from subcon_delivery_mst a, subcon_delivery_dtls b , GBL_TEMP_ENGINE e where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.order_id=e.ref_val and e.entry_form=23 and e.user_id=$user_id and e.ref_from=1 group by b.order_id";
		// echo $delivery_sql;
		$delivery_sql_result=sql_select($delivery_sql);
		foreach ($delivery_sql_result as $row)
		{
		...
		//=========================================================
		$fab_production_array=array();
 		$fab_production_sql="SELECT c.order_id, sum(CASE WHEN c.product_type='4' THEN  c.quantity END) AS finishing, sum(CASE WHEN c.product_type='8' THEN  c.quantity END) AS printing from  subcon_production_mst a, subcon_production_dtls b, subcon_production_qnty c, GBL_TEMP_ENGINE e where a.id=b.mst_id and b.id=c.dtls_id and a.status_active=1  and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.order_id=e.ref_val and e.entry_form=23 and e.user_id=$user_id and e.ref_from=1 group by c.order_id"; 
		...
		//===============================================================
		 ...
		 $knit_production_array=array();
 		 $knit_production_sql="SELECT b.order_id, sum(b.product_qnty) AS kniting from  subcon_production_mst a, subcon_production_dtls b , GBL_TEMP_ENGINE e where a.id=b.mst_id and b.product_type=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and b.order_id=e.ref_val and e.entry_form=23 and e.user_id=$user_id and e.ref_from=1 group by b.order_id";  
		$knit_production_sql_result=sql_select($knit_production_sql);
		foreach ($knit_production_sql_result as $row)
		{
			$knit_production_array[$row[csf('order_id')]]['kniting']=$row[csf('kniting')];
		...
		//var_dump($dying_data_array);
		 ...
		 $gmt_production_array=array();
 		 $gmt_production_sql="SELECT a.order_id, sum(CASE WHEN a.production_type='1' THEN  a.production_qnty END) AS cutting, sum(CASE WHEN a.production_type='2' THEN  a.production_qnty END) AS sewing, sum(CASE WHEN a.production_type='4' THEN  a.production_qnty END) AS Gmts_Finishing from subcon_gmts_prod_dtls a , GBL_TEMP_ENGINE e where a.status_active=1 and a.is_deleted=0 and a.order_id=e.ref_val and e.entry_form=23 and e.user_id=$user_id and e.ref_from=1 group by a.order_id";
 		$gmt_production_sql_result=sql_select($gmt_production_sql);
		foreach ($gmt_production_sql_result as $row)
		{
			$gmt_production_array[$row[csf('order_id')]]['cutting']=$row[csf('cutting')];
		...
		from subcon_inbound_bill_dtls, GBL_TEMP_ENGINE e where status_active=1 and is_deleted=0 and order_id=e.ref_val and e.entry_form=23 and e.user_id=$user_id and e.ref_from=1  group by order_id, mst_id"; 
		$in_bill_sql_result=sql_select($in_bill_sql);
		foreach ($in_bill_sql_result as $row)
		{
			$in_bill_qty_array[$row[csf('order_id')]]['cutting']+=$row[csf('cutting')];
	    ...
	    <div>
		    ...
		    <div style="max-height:400px; overflow-y:scroll; width:<? echo $tbl_width+20; ?>px" id="scroll_body">
				...
				foreach ($job_sql_result as $row)
				{
					if (!in_array($row[csf("main_process_id")],$process_array) )
					{
						if($k!=1)
						{
							...
							<tr class="tbl_bottom">
								...
								<td width="100" align="right"><? echo number_format($tot_rec_return); ?></td>
								<td width="120" align="right"><? echo number_format($tot_total_issue_qty); ?></td>
								<td width="100" align="right"><? echo number_format($tot_issue_return); ?></td>
							...
							unset($tot_order_qty);
							unset($tot_rec_qty);
							unset($tot_rec_return);
							unset($tot_total_issue_qty);
							unset($tot_issue_return);
							unset($tot_material_blce);
							unset($tot_batch_qty);
					...
					$rec_qty=$recv_inventory_array[$row[csf('id')]]['quantity'];//-$recv_inventory_ret_array[$row[csf('id')]]['quantity']
					$issue_qty=$inv_iss_array[$row[csf('id')]]['quantity'];//-$inventory_iss_ret_array[$row[csf('id')]]['quantity']
					$issue_return=$inventory_iss_ret_array[$row[csf('id')]]['quantity'];
					$receive_return=$recv_inventory_ret_array[$row[csf('id')]]['quantity'];
					...
					<tr bgcolor="<? echo $bgcolor; ?>" style="vertical-align:middle" height="25" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>">	
			            ...
			            <td width="120" align="right"><p><a href="##" onclick="show_progress_report_details('material_desc_popup_summary','<? echo $row[csf("id")]; ?>','1150px')"><? echo number_format($rec_qty); ?></a></p></td>
			            <td width="100" align="right"><p><a href="##" onclick="show_progress_report_details('material_desc_rcv_rtn_popup_summary','<? echo $row[csf("id")]; ?>','1150px','0')"><? echo number_format($receive_return); ?></a></p></td>
			            <td width="120" align="right"><p><a href="##" onclick="show_progress_report_details('material_desc_iss_popup_summary','<? echo $row[csf("id")]; ?>','1150px','0')"><? echo number_format($issue_qty); ?></a></p></td>
			            <td width="100" align="right"><p><a href="##" onclick="show_progress_report_details('material_desc_iss_rtn_popup_summary','<? echo $row[csf("id")]; ?>','1150px','0')"><? echo number_format($issue_return); ?></a></p></td>
			            <td width="120" align="right">
			            	<?
			            		$mat_blnce_qty=($rec_qty+$issue_return)-($issue_qty+$receive_return);
	                    ...
	                    <td width="80" align="right"><p>
	                    	<? $batch_balance = ($issue_qty-$issue_return) - $batch_qty;echo number_format($batch_balance); ?>	                   		
	                    	</p></td>
			                    
			            <td width="80" align="right"><p><a href="##" onclick="show_progress_report_details('product_qty_pop_up','<? echo $row[csf("id")]; ?>_<? echo $row[csf('main_process_id')]; ?>_summary','1150px')"><? echo number_format($prod_qty); ?></a></p></td>
					...
					$tot_total_order_val+=$row[csf('amount')];
					$tot_total_rec_qty+=$rec_qty;
					$tot_total_rec_return_qty+=$receive_return;
					$tot_total_issue_qty+=$issue_qty;
					$tot_total_iss_return_qty+=$issue_return;
					$tot_total_material_blce+=$mat_blnce_qty;
					$tot_total_prod_qty+=$prod_qty;
				...
				?>
			        ...
			        <tr class="tbl_bottom">
			            ...
			            <td width="100"><? echo number_format($tot_total_rec_return_qty); ?></td>
			            <td width="120"><? echo number_format($tot_total_issue_qty); ?></td>
			            <td width="100"><? echo number_format($tot_total_iss_return_qty); ?></td>
