...
{ 
	...
	<div style="max-height:300px; overflow-y:scroll; width:1330px" id="scroll_body">
		...
		<?
			...
			$sql_iss_ret= "select a.id, a.trans_type, b.order_id, b.item_category_id, b.material_description, b.subcon_uom, sum(b.quantity) as quantity from  sub_material_mst a, sub_material_dtls b where a.id=b.mst_id and a.is_deleted=0 and a.trans_type in (2,3) $order_no_cond $receive_date_cond $challan_no_cond $item_category_cond group by a.id, a.trans_type, b.order_id, b.item_category_id, b.material_description, b.subcon_uom";
			$sql_iss_ret_arr=sql_select($sql_iss_ret); $iss_ret_qty_arr=array();
			foreach ($sql_iss_ret_arr as $row)
			{
				//if($row[csf('trans_type')]==2)
				$iss_ret_qty_arr[$row[csf('trans_type')]][$row[csf('order_id')]][$row[csf('item_category_id')]][$row[csf('material_description')]][$row[csf('subcon_uom')]]+=$row[csf('quantity')];
			}
		  ...
		  $sql= "select a.id, a.chalan_no, a.subcon_date, a.party_id, b.order_id, b.item_category_id, b.material_description, b.subcon_uom, sum(b.quantity) as quantity from  sub_material_mst a, sub_material_dtls b where a.id=b.mst_id and a.is_deleted=0 and a.trans_type=1 $order_no_cond $party_rec_cond $receive_date_cond $challan_no_cond $item_category_cond group by a.id, a.chalan_no, a.subcon_date, a.party_id, b.order_id, b.item_category_id, b.material_description, b.subcon_uom";
			...
			foreach ($sql_result as $row)
			{
				...
				$issue_qty=$iss_ret_qty_arr[2][$row[csf('order_id')]][$row[csf('item_category_id')]][$row[csf('material_description')]][$row[csf('subcon_uom')]];
				$return_qty=$iss_ret_qty_arr[3][$row[csf('order_id')]][$row[csf('item_category_id')]][$row[csf('material_description')]][$row[csf('subcon_uom')]];
				$balance_qty=$receive_qty-$issue_qty-$return_qty;
