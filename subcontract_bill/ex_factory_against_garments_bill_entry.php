...
<script>
	...
	function copy_value(value,i)
	{
		...
		var cboitemname=document.getElementById('cboitemname_'+i).value;
		var pocolorid=document.getElementById('txtColorId_'+i).value;
		var ordernoid=document.getElementById('ordernoid_'+i).value;
		var cbocountry=document.getElementById('cbocountry_'+i).value;
		var rowCount = $('#bill_issue_table tr').length;
		var copy_basis=$('input[name="copy_basis"]:checked').val();
		//alert(copy_basis);
		for(var j=i; j<=rowCount; j++)
		{
			if(copy_val==true) { document.getElementById('txtrate_'+j).value=value; }
			else
			{
				if(copy_basis==1)
				{
					if( cboitemname==document.getElementById('cboitemname_'+j).value)
					{
						document.getElementById('txtrate_'+j).value=value;
						qnty_caluculation(j);
					}
				}
				else if(copy_basis==2)
				{
					if( pocolorid==document.getElementById('txtColorId_'+j).value) 
					{
						document.getElementById('txtrate_'+j).value=value;
						qnty_caluculation(j);
					}
				...
				{
					if( ordernoid==document.getElementById('ordernoid_'+j).value) 
					{
						document.getElementById('txtrate_'+j).value=value;
						qnty_caluculation(j);
					}
				...
				{
					if( cbocountry==document.getElementById('cbocountry_'+j).value) 
					{
						document.getElementById('txtrate_'+j).value=value;
						qnty_caluculation(j);
					}
	...
	function fnc_all_copy()
	{
		var copy_basis=$('input[name="copy_basis"]:checked').val();
		if(copy_basis==1 || copy_basis==2 || copy_basis==3 || copy_basis==4)
		{
			document.getElementById('copy_val').checked=false;
		}
		else document.getElementById('copy_val').checked=true;
	}
...
<body onLoad="set_hotkey()">
    ...
    <legend>Garments Derivery For Bill Info</legend>
   		 <form name="garmentsbillissue_2">
         	 Copy: <input type="checkbox" id="copy_val" name="copy_val" checked/>
            <input type="radio" name="copy_basis" value="1" onClick="fnc_all_copy();">GMT Item
            <input type="radio" name="copy_basis" value="2" onClick="fnc_all_copy();">Color Wise
            <input type="radio" name="copy_basis" value="3" onClick="fnc_all_copy();">Po Wise
            <input type="radio" name="copy_basis" value="4" onClick="fnc_all_copy();">Country Wise
        </form>
