...
*/			$('#txtBalance_'+i).removeAttr("value").attr("value","");
	...
	function fnc_batch_creation(operation)
	{
		...
		//alert(data_all);
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_batch_sl_no*cbo_company_id*cbo_batch_against*batch_no_creation*txt_batch_number*cbo_dyeing_floor*txt_machine_no*txt_batch_date*txt_batch_weight*txt_tot_trims_weight*txt_ext_no*txt_batch_color*cbo_color_range*txt_process_id*txt_du_req_hr*txt_exp_load_hr*txt_exp_load_min*txt_du_req_min*txt_batch_delivery_date*update_id*hide_update_id*txt_remarks*cbo_location_name*unloaded_batch*ext_from*cbo_double_dyeing*txt_collar_qty*txt_cuff_qty*collarAndCuffStr',"../")+data_all+'&total_row='+row_num+'&txt_deleted_id='+txt_deleted_id;
		//alert (data);return;
	...
	{
		reset_form('','','txt_ext_no*cbo_color_range*txt_process_name*txt_process_id*txt_du_req_hr*txt_du_req_min*txt_exp_load_hr*txt_exp_load_min*txt_batch_delivery_date*txt_batch_color*txtPoNo_1*poId_1*txtItemDesc_1*txtItemDescid_1*txtRollNo_1*txtBatchQnty_1*txtrecChallan_1*txt_total_batch_qnty*hide_job_no','','$(\'#tbl_item_details tbody tr:not(:first)\').remove();','');//
		//var batch_against= $('#cbo_batch_against').val();
	...
	function openmypage_collar_and_cuff()
	{
		...
		hiddin_job_no= $('#hidden_job_number_for_po_popup').val();
		var title = 'Coller & Cuff Mesurement Info';	
		var page_link = 'requires/subcon_batch_creation_controller.php?hiddin_job_no='+hiddin_job_no+'&hidden_serial_id='+hidden_serial_id+'&action=collar_and_cuff_popup&collarAndCuffStr='+collarAndCuffStr;
