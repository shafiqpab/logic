...
<script>
	...
	function window_close( uid )
	{
		...
		var cbo_party_source=$('#cbo_party_source').val()*1;
		
		var hidd_rate_from=$('#hidd_rate_from').val()*1;
		var hidd_rate_readonly=$('#hidd_rate_readonly').val()*1; 
		if(hidd_rate_readonly==1)  is_rate_readonly="";else is_rate_readonly="readonly";  
		//alert(hidd_rate_readonly);
		var hidd_rate_disable=$('#hidd_rate_disable').val()*1; 
		if(hidd_rate_disable==1)  is_rate_disable="";else is_rate_disable="disabled";  
			...
			for (var k=1; k<=list_tot_row; k++)
			{
				...
				if(check_id==1)
				{
					...
					if( jQuery.inArray(  trid , selected_id_listed ) == -1) 
					{
						...
						var unitCharge=split_str[24]*1;
						var bdt_rate=split_str[34]*1;
						//alert(bdt_rate+'='+split_str[34]);
						if(bdt_rate=="") bdt_rate=0;
						
						var amount=rec_qty*bdt_rate;
						if(amount) var amount=amount;else amount="";
						//alert(amount+'='+bdt_rate);
		...
		else
		{
			...
			for (var n=1; n<=split_list_view.length; n++)
			{
				...
				var rate=split_list_str[23]*1;
			//	alert(rate+'='+split_str[34]);
					//	if(rate=="") rate=0;
	...
	function fnc_rate_copy( trid )
	{
		...
		{		
			...
			for(var j=trid; j<=row_nums; j++)
			{
				...
				document.getElementById('txtRate_'+j).value=rate;
				qnty_caluculation(j);
			}
		}
	}
	...
	function generate_report(type)
	{
		...
		else if(type=="dyeingFinishinBillPrint5")
		{
			var report_title=$( "div.form_caption" ).html();
			generate_report_file( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_bill_no').val()+'*'+report_title+'*'+$('#hidd_inhouse_bill_from').val(), type, "requires/sub_fabric_finishing_bill_issue_controller");
			//return;
	...
	function fnc_list_search(type)
	{
		...
		if($('#cbo_party_source').val()==1)
		{
			...
			if(txt_bill_form_date=="" && txt_bill_to_date=="" && txt_manual_challan=="" && txt_sys_challan=="" && txt_batch_no=="" && txt_booking_no==""){
				var divData="txt_bill_form_date*txt_bill_to_date*txt_manual_challan*txt_sys_challan*txt_batch_no*txt_booking_no";	
		...
		else
		{
			...
			//alert(all_value);
			show_list_view($('#cbo_company_id').val()+'***'+location_cond+'***'+$('#cbo_party_source').val()+'***'+$('#cbo_party_name').val()+'***'+$('#cbo_bill_for').val()+'***'+$('#txt_bill_form_date').val()+'***'+$('#txt_bill_to_date').val()+'***'+$('#txt_manual_challan').val()+'***'+$('#variable_check').val()+'***'+$('#hidd_inhouse_bill_from').val()+'***'+$('#txt_sys_challan').val()+'***'+$('#txt_batch_no').val()+'***'+$('#txt_booking_no').val()+'***'+type+'***'+all_value,'dyingfinishing_delivery_list_view','dyeingfinishing_info_list','requires/sub_fabric_finishing_bill_issue_controller','setFilterGrid("tbl_list_search",-1); reset_hide_field();','','');
		}
	}
...
function openmypage_booking()
	{
		...
		var txt_booking_no = $("#txt_booking_no").val(); 
		//var page_link='requires/sub_fabric_finishing_bill_issue_controller.php?action=booking_no_popup&companyID='+companyID;
		var page_link='requires/sub_fabric_finishing_bill_issue_controller.php?action=booking_no_popup&companyID='+companyID+'&location_name='+location_name+'&cbo_booking_year='+cbo_booking_year+'&txt_booking_no='+txt_booking_no;
    ...
    <legend>Dyeing & Finishing Bill Info</legend>
                    ...
                    <table cellpadding="0" cellspacing="2" width="100%">
                        <tr>
                            ...
                            <td width="140" align="justify">
								...
								<input type="hidden" name="hidd_rate_readonly" id="hidd_rate_readonly" />
                        ...
                        <tr>
                            <td width="110" class="must_entry_caption">Dyeing Company</td>
                            <td width="150">
                                <? 
                        ...
                        <tr>
                        	<td>Bill For</td>
                            <td><? echo create_drop_down( "cbo_bill_for", 150, $bill_for,"", 0, "-- Bill Type --", 1, "fnc_head_change(this.value)",1,"","","","",7); ?></td> 
							<td>Booking No</td>
							<td>
                                <input type="text" id="txt_booking_no" name="txt_booking_no" class="text_boxes_numeric" style="width:140px" onDblClick="openmypage_booking();" placeholder="Write/Browse" />
        ...
        <table  style="border:none; width:1060px;" cellpadding="0" cellspacing="1" border="0" id="">
            <thead class="form_table_header">
                ...
                <th width="70" id="td_order">Order </th>
            ...
            <tbody id="bill_issue_table">
                <tr align="center">				
                    ...
                    <td>
                     ...
                     <td>
                        <input type="text" name="txtshadeper_1" id="txtshadeper_1"  class="text_boxes" style="width:60px"  />							 
                    </td>
                    <td>
            ...
            <tfoot>
                ...
                <tr>
                    <td colspan="17" align="center" class="button_container">
						...
						<input type="button" name="print5" id="print5" value="Print B5" onClick="generate_report('dyeingFinishinBillPrint5')" style="width:70px" class="formbutton" />
