...
<script>
	...
	function openmypage(page_link,title)
	{
		...
		emailwindow.onclose=function()
		{
			...
			if (po_id!="")
			{
				...
				get_php_form_data(po_id+'**'+item_id, "populate_data_from_search_popup", "requires/subcon_sewing_input_controller" );
				var variableSettings=$('#sewing_production_variable').val();
				//alert (variableSettings);
				if(variableSettings!=1)
					get_php_form_data(po_id+'**'+item_id+'**'+variableSettings, "color_and_size_level", "requires/subcon_sewing_input_controller" ); 
				else
					$("#txt_sewing_qty").removeAttr("readonly");
				show_list_view(po_id+'**'+item_id+'**'+variableSettings,'show_dtls_listview','sewing_output_dtls_list_view','requires/subcon_sewing_input_controller','');			
				set_button_status(0, permission, 'fnc_subcon_sewing_input_entry',1);
				load_drop_down( 'requires/subcon_sewing_input_controller', document.getElementById('cbo_location').value, 'load_drop_down_floor', 'floor_td' );
				release_freezing();
			}
		}
	}
	
	function fnc_subcon_sewing_input_entry(operation)
	{
		if(operation==5)
		{
			...
			print_report( $('#cbo_company_name').val()+'*'+master_id+'*'+report_title, "sewing_input_print2", "requires/subcon_sewing_input_controller" );
			return;
		}
		else
		{
		...
		//alert (data);
		http.open("POST","requires/subcon_sewing_input_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
		http.onreadystatechange = fnc_subcon_sewing_input_entry_respone;

		}
		
	}
	
	function fnc_subcon_sewing_input_entry_respone()
	{
		if(http.readyState == 4)
		{
			...
			{
			//reset_form('','','cbo_sewing_line*txt_hours*txt_sewing_qty*txt_reject_qnty*txt_spot_qnty*txt_alter_qnty*txt_cumul_sewing_qty*txt_yet_to_sewing*txt_super_visor*txt_challan*txt_manual_cut_no*txt_remark*hidden_break_down_html','txt_sewing_date,<? //echo date("d-m-Y"); ?>',''); 	
			//window.location.reload();
			//	release_freezing();
				//return;
			...
			get_php_form_data(po_id+'**'+item_id+'**'+variableSettings, "populate_data_from_search_popup", "requires/subcon_sewing_input_controller" );
			if(variableSettings!=1)
			{ 
				get_php_form_data(po_id+'**'+item_id+'**'+variableSettings, "color_and_size_level", "requires/subcon_sewing_input_controller" ); 
			}
	...
	{
		reset_form('','','cbo_sewing_line*txt_hours*txt_sewing_qty*txt_reject_qnty*txt_alter_qnty*txt_cumul_sewing_qty*txt_yet_to_sewing*txt_super_visor*txt_challan*txt_manual_cut_no*txt_remark','txt_sewing_date,<? echo date("d-m-Y"); ?>',''); 	
...
function fn_chk_next_process_qty(color,index,sizeId) // for color and size level
{
 ...
 //	alert(sewing_production_variable);
	if(sewing_production_variable==2)
	{
		...
		var totCutQty = $("#colSizeCutQty"+index).val()*1; 
		var prev_input_qty = $("#colSizeUpQty_"+index).val()*1; ;
	}
	else
	{
		...
		var totCutQty = $("#colSizeCutQty"+color+index).val()*1; 
		var prev_input_qty = $("#colSizeUpQty_"+color+index).val()*1; ;
	}
	...
	{
			alert('Sorry! Input qnty will not greater than Cutting qnty');
				if(prev_input_qty>0)
				{			
					if(sewing_production_variable==2)
					{
                ...
                <fieldset>
                    <table width="100%" border="0">
                        <tr>
                            ...
                            <td width="140" >                                
								<? 
									 echo create_drop_down( "cbo_company_name", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/subcon_sewing_input_controller', this.value, 'load_drop_down_location', 'location_td' );get_php_form_data(this.value,'load_variable_settings','requires/subcon_sewing_input_controller');" );
                                ?>
                            ...
                            <td width="140">
								<input name="txt_order_no" placeholder="Double Click to Search" id="txt_order_no" onDblClick="openmypage('requires/subcon_sewing_input_controller.php?action=order_popup&company='+document.getElementById('cbo_company_name').value+'&garments_nature='+document.getElementById('garments_nature').value,'Order Search')"  class="text_boxes" style="width:130px " readonly>
                    ...
                    <tr>
                        <td width="35%" valign="top">
                                ...
                                <legend>New Entry</legend>
                                    <table cellpadding="0" cellspacing="1" width="100%">
                                        ...
                                        <tr>
                                            <td class="must_entry_caption">Sew. Input  Qty</td> 
                                            <td>
                                        ...
                                        <tr>
                                            <td>Manual Cut No</td> 
                                            <td>
                                                <input type="text" name="txt_manual_cut_no" id="txt_manual_cut_no" class="text_boxes" style="width:100px" />
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>Remarks</td>
                     ...
                     <tr>
		   				<td align="center" colspan="9" valign="middle" class="button_container">
							<?
								$date=date('d-m-Y');
								echo load_submit_buttons( $permission, "fnc_subcon_sewing_input_entry", 0,0 ,"reset_form('sewingoutput_1','','','txt_sewing_date,".$date."','childFormReset()');$('#txt_sewing_qty').attr('placeholder','');$('#cbo_company_name').attr('disabled',false);$('#cbo_buyer_name').attr('disabled',false);",1); 
                            ?>
                            <input type="hidden" name="txt_mst_id" id="txt_mst_id" readonly />
                            <input value="Print" name="print2" onClick="fnc_subcon_sewing_input_entry(5)" style="width:80px" id="Print2" class="formbutton" type="button">
           				</td>
           				<td>&nbsp;</td>					
		  			</tr>
                </table>
