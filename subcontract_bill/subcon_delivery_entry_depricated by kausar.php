<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Sub-contract Delivery Entry
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Delivery Info","../", 1, 1, '','','');
?>
<script>
	...
	function openmypage_order()
	{
		...
		else
		{
			...
			var page_link = 'requires/subcon_delivery_entry_controller.php?data='+data+'&action=order_number_popup';
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link,'Order Number Form', 'width=850px,height=400px,center=1,resize=0,scrolling=0','')
			emailwindow.onclose=function()
			{
				...
				{
					//freeze_window(5);
					get_php_form_data(ret_value[0], "populate_data_from_search_popup", "requires/subcon_delivery_entry_controller" );
					...
					show_list_view(document.getElementById('txt_order_id').value+"_"+document.getElementById('cbo_process_name').value,'show_fabric_desc_listview','list_fabric_desc_container','requires/subcon_delivery_entry_controller','');
					reset_form('','','txt_item_id*txt_delivery_qnty*txt_carton_roll_no*txt_remarks');
					release_freezing();
				}
			}
		}
	...
	function fnc_subcon_delivery_entry(operation)
	{
		...
		if(operation==4)
		{
			 ...
			 print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+$('#txt_sys_id').val()+'*'+report_title, "subcon_delivery_entry_print", "requires/subcon_delivery_entry_controller" ) 
			 return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			if( form_validation('cbo_company_name*cbo_party_name*txt_order_no*txt_delivery_date*txt_dalivery_item*txt_delivery_qnty','Company Name*Party*Order Number*Delivery Date*Dalivery Item*Delivery Qnty')==false )
			{
				return;
			} 
			...
			else
			{
				...
				var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_sys_id*cbo_company_name*cbo_location*txt_order_id*cbo_party_name*cbo_process_name*txt_delivery_date*txt_item_id*txt_delivery_qnty*txt_carton_roll_no*txt_challan_no*txt_transport_company*cbo_forwarder*txt_remarks*update_id*update_id_dtls*txt_gsm*txt_dia*txt_color_id*txt_batch_no*txt_ext_no*hidden_dia_type*hid_sub_process',"../");
				//alert (data);return;
				freeze_window(operation);
				http.open("POST","requires/subcon_delivery_entry_controller.php",true);
	...
	function fnc_subcon_delivery_entry_respone()
	{
		if(http.readyState == 4) 
		{
			...
			show_list_view(reponse[1],'delivery_entry_list_view','delivery_entry_list_view','requires/subcon_delivery_entry_controller','setFilterGrid("list_view",-1)');
			reset_form('','','txt_item_id*txt_delivery_qnty*txt_carton_roll_no*txt_remarks*update_id_dtls*txt_production_qnty*txt_color*txt_color_id*txt_batch_no*txt_ext_no*txt_gsm*hid_sub_process*txt_dia*hidden_dia_type*bill_info*txt_dalivery_item*txt_pre_delivery_qnty');
			//$('#list_fabric_desc_container').html('');
			show_list_view(document.getElementById('txt_order_id').value+'_'+document.getElementById('cbo_process_name').value, 'show_fabric_desc_listview','list_fabric_desc_container','requires/subcon_delivery_entry_controller','');
			set_button_status(0, permission, 'fnc_subcon_delivery_entry',1,1);	
			release_freezing();	
		}
	}

	function openmypage_dalivery_item()
	{
		var data=document.getElementById('txt_order_id').value+"_"+document.getElementById('cbo_process_name').value;
		if( form_validation('txt_delivery_date','Delivery Date')==false )
		{
			return;
		}
		var page_link = 'requires/subcon_delivery_entry_controller.php?data='+data+'&action=dalivery_item_popup';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link,'Dalivery Item Popup', 'width=700px,height=300px,center=1,resize=0,scrolling=0','')
		emailwindow.onclose=function()
		{
			var const_composition=this.contentDoc.getElementById("hidd_item_id");
			var response=const_composition.value.split('_');
			if (const_composition!="")
			{
				freeze_window(5);
				document.getElementById('delivery_item_id').value = response[0];
				document.getElementById('txt_item_id').value = response[1];
				release_freezing();
			}
		}
	}
	...
	{ 
		...
		var page_link='requires/subcon_delivery_entry_controller.php?action=delivery_id_popup&data='+data
		...
		emailwindow.onclose=function()
		{
			...
			if (theemail.value!="")
			{
				...
				freeze_window(5);
				get_php_form_data( theemail.value, "load_php_data_to_form", "requires/subcon_delivery_entry_controller" );
				show_list_view(theemail.value,'delivery_entry_list_view','delivery_entry_list_view','requires/subcon_delivery_entry_controller','setFilterGrid("list_view",-1)');
				reset_form('','','txt_order_id*txt_order_no*cbo_process_name*txt_order_date*txt_ordr_qnty*txt_uom*txt_style*txt_item_id*txt_delivery_qnty*txt_carton_roll_no*txt_remarks*update_id_dtls*txt_gsm*txt_dia*txt_color_id*txt_batch_no*txt_ext_no*hid_sub_process*hidden_dia_type*txt_production_qnty*txt_color*txt_color_id*hid_sub_process*bill_info*txt_dalivery_item');
				set_button_status(0, permission, 'fnc_subcon_delivery_entry',1,1);
	...
	function qty_validation(val,field_id)
	{
		...
		var response=return_global_ajax_value(data, 'delivery_qty_check', '', 'requires/subcon_delivery_entry_controller');
	...
	{
		//alert (val)
		var response=return_global_ajax_value( document.getElementById('txt_order_id').value+"_"+document.getElementById('cbo_process_name').value+"_"+val, 'populate_data_production_qty', '', 'requires/subcon_delivery_entry_controller');
                ...
                <table width="100%">
                    <tr>
                     ...
                     <tr>
                        ...
                        <td width="140">
                            <?
                                echo create_drop_down( "cbo_company_name", 152, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/subcon_delivery_entry_controller', this.value, 'load_drop_down_location', 'location_td' ); load_drop_down( 'requires/subcon_delivery_entry_controller', this.value, 'load_drop_down_party_name', 'party_td' );",0 );	
                            ?>
                        </td>
            ...
            <table cellpadding="0" cellspacing="1" width="800" align="left">
                        ...
                        <legend>New Entry</legend>
                        	 <table  cellpadding="0" cellspacing="2" width="100%">
                                ...
                                <tr>
                                    ...
                                    <td>
                                    	<input type="text" name="txt_delivery_qnty" id="txt_delivery_qnty" class="text_boxes_numeric" style="width:120px;" />
                                ...
                                <tr>
                                    <td>Order Date</td>
                                    <td>
                                        <input type="date" name="txt_order_date" id="txt_order_date" class="datepicker" style="width:120px;" disabled />
                                    </td>
                                 ...
                                 <tr>
                                    ...
                                    <td>
                                        <input type="text" name="txt_ordr_qnty" id="txt_ordr_qnty" class="text_boxes_numeric" style="width:70px;" disabled />
                                         <? //echo create_drop_down( "txt_uom", 40, $production_process,"", 1, "--Select--",$selected,"", "1","" );?>
                ...
                <tr>
                	<td align="center" colspan="4" valign="middle" class="button_container">
						<?
                        echo load_submit_buttons( $permission, "fnc_subcon_delivery_entry", 0,1 ,"reset_form('deliveryentry_1','delivery_entry_list_view','','','')",1); 
                        ?><input type="hidden" id="update_id_dtls"><!--<input type="button" value="Delivery Challan" name="generate" id="generate" class="formbutton" style="width:150px" onClick="generate_delivery_challan_report()"/>-->
                    </td>
