...
<script>
	...
	{ 
		...
		emailwindow.onclose=function()
		{
			...
			if (theemail.value!="")
			{
				...
				$('#sewing_info_list tr').each(function(index, element) {
	...
	function fnc_list_search(type)
	{
		...
		{
			if( form_validation('cbo_company_id*cbo_location_name*cbo_party_source*cbo_party_name*txt_bill_form_date*txt_bill_to_date','Company Name*Location*Party Source*Party Name*Form Date*To Date')==false)
			{
				return;
			}
		...
		if (type==0 && ($('#update_id').val()*1)==0)
		{
			...
			//show_list_view(document.getElementById('cbo_party_source').value+'_'+this.value,'sewing_delivery_list_view','sewing_info_list','requires/sewing_bill_issue_controller','setFilterGrid(\'list_view_issue\',-1)');
			
			show_list_view(document.getElementById('cbo_company_id').value+'***'+location_cond+'***'+document.getElementById('cbo_party_source').value+'***'+document.getElementById('cbo_party_name').value+'***'+document.getElementById('cbo_bill_for').value+'***'+document.getElementById('txt_bill_form_date').value+'***'+document.getElementById('txt_bill_to_date').value,'sewing_delivery_list_view','sewing_info_list','requires/sewing_bill_issue_controller', 'setFilterGrid("list_view_issue",-1)');
		}
		else
		{
			var tot_row=$('#bill_issue_table tr').length;
			...
			if($('#cbo_party_source').val()==1)
			{
				for (var n=1; n<=tot_row; n++)
				{
					...
					// Lot of Data can not carry on via URL. It is replaced by action in knitting_delivery_list_view. If any change? Talk With Kausar/Aziz. Issue= 18- 12362.
				}
			}
			else
			{
			...
			//alert(all_value);
			show_list_view(document.getElementById('cbo_company_id').value+'***'+location_cond+'***'+document.getElementById('cbo_party_source').value+'***'+document.getElementById('cbo_party_name').value+'***'+document.getElementById('cbo_bill_for').value+'***'+document.getElementById('txt_bill_form_date').value+'***'+document.getElementById('txt_bill_to_date').value+'***'+type+'***'+all_value+'***'+document.getElementById('update_id').value,'sewing_delivery_list_view','sewing_info_list','requires/sewing_bill_issue_controller','setFilterGrid("list_view_issue",-1)','','');
		}
	}
</script>
</head>
