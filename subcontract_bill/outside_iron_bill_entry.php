<?
/*--- ----------------------------------------- Comments
Purpose			: 	This form will create Out Side Iron Bill Entry						
Functionality	:	
JS Functions	:
Created by		:	Kausar
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Iron Bill Entry", "../", 1,1, $unicode,1,'');
?>
...
<script>
	...
	{ 
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/outside_iron_bill_entry_controller.php?data='+data+'&action=bill_no_popup','Bill Popup', 'width=700px,height=400px,center=1,resize=1,scrolling=0','')
		...
		emailwindow.onclose=function()
		{
			...
			{
				freeze_window(5);
				get_php_form_data( theemail.value, "load_php_data_to_form_issue", "requires/outside_iron_bill_entry_controller" );
				window_close( theemail.value );
				show_list_view(document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_supplier_company').value+'_'+document.getElementById('update_id').value,'iron_entry_list_view','iron_info_list','requires/outside_iron_bill_entry_controller','set_all();');
				set_button_status(1, permission, 'fnc_iron_bill_entry',1);
				set_all_onclick();
				release_freezing();
			}
		}
	...
	function window_close( frm )
	{
		...
		//alert (data);
		var list_view_orders = return_global_ajax_value( data, 'load_php_dtls_form', '', 'requires/outside_iron_bill_entry_controller');
		if(list_view_orders!='')
		{
	...
	function fnc_iron_bill_entry( operation )
	{
		 if(operation==4)
		 {
			print_report( $('#txt_bill_no').val()+'*'+$('#cbo_company_id').val()+'*'+$('#cbo_party_name').val()+'*'+$('#issue_id_all').val()+'*'+$('#txt_bill_date').val()+'*'+$('#selected_order_id').val(), "iron_bill_entry_print", "requires/outside_iron_bill_entry_controller") 
			//return;
			show_msg("3");
		 }
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			{	
				...
				http.open("POST","requires/outside_iron_bill_entry_controller.php",true);
				http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
				http.send(data);
				http.onreadystatechange = fnc_iron_bill_entry_reponse;
			}
		}
	}

	function fnc_iron_bill_entry_reponse()
	{
...
<body onLoad="set_hotkey()">
    ...
    <form name="outironbill_1" id="outironbill_1"  autocomplete="off"  >
        ...
        <table cellpadding="0" cellspacing="2" width="100%">
            <tr>
             ...
             <tr>
                ...
                <td width="150">
                    <? 
                        echo create_drop_down( "cbo_company_id",150,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name","id,company_name", 1, "--Select Company--", $selected, "load_drop_down( 'requires/outside_iron_bill_entry_controller', this.value, 'load_drop_down_location', 'location_td');load_drop_down( 'requires/outside_iron_bill_entry_controller', this.value, 'load_drop_down_supplier_name', 'supplier_td');","","","","","",2);	
                    ?>
                </td>
    ...
    <legend>Iron Bill Details </legend>
            ...
            <tbody id="bill_issue_table">
                <tr align="center">				
                    <td>
                        ...
                        <input type="date" name="txt_deliverydate_1" id="txt_deliverydate_1"  class="text_boxes" style="width:65px" readonly />									
                    </td>
                    <td>
                    ...
                    <td>
                        <input type="text" name="text_wo_num_1" id="text_wo_num_1"  class="text_boxes" style="width:60px"  readonly/> 
                    </td>
                    <td>
            ...
            <tfoot>
                ...
                <tr>
                    <td colspan="12" align="center" class="button_container">
						<? 
							echo load_submit_buttons($permission,"fnc_iron_bill_entry",0,0,"reset_form('outironbill_1','iron_info_list','','','$(\'#bill_issue_table tr:not(:first)\').remove();')",1);
                        ?> 
                    </td>
                </tr>  
                <tr>
