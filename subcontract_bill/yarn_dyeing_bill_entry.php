...
<script>
	...
	function fnc_outside_finishing_bill_entry( operation ) // For Save and Update
	{
		var isFileMandatory = "";
		<?php 
			
			if(!empty($_SESSION['logic_erp']['mandatory_field'][256][1])) echo " isFileMandatory = ". $_SESSION['logic_erp']['mandatory_field'][256][1] . ";\n";
		?>
		...
		var tot_row=$('#outside_yarndyeingbill_table tr').length;
		var data1="action=save_update_delete&operation="+operation+"&tot_row="+tot_row+get_submitted_data_string('txt_bill_no*cbo_company_id*cbo_location_name*txt_bill_date*cbo_supplier_company*cbo_bill_for*cbo_bill_on*txt_party_bill*txt_manual_challan*update_id',"../");
		//alert (data1);
		var data2='';
		for(var i=1; i<=tot_row; i++)
		{
			...
			{
				data2+=get_submitted_data_string('txtReceiveDate_'+i+'*txtMrrNo_'+i+'*txtChallenNo_'+i+'*txtOrderNo_'+i+'*txtNumberBag_'+i+'*txtNumberCone_'+i+'*itemid_'+i+'*colorId_'+i+'*bodyPartId_'+i+'*textWoNumId_'+i+'*cboUom_'+i+'*txtYarnQty_'+i+'*txtRate_'+i+'*txtAmount_'+i+'*txtRemarks_'+i+'*reciveId_'+i+'*updateIdDtls_'+i+'*curanci_'+i+'*txtDomesticAmount_'+i,"../");+'*txtStyleName_'+i+'*txtPartyName_'+i
		...
		http.open("POST","requires/yarn_dyeing_bill_entry_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
		http.onreadystatechange = fnc_outside_finishing_bill_entry_response;
	}

	function fnc_outside_finishing_bill_entry_response() // For Save and Update
	{
	...
	function uploadFile(mst_id)
	{
		$(document).ready(function() { 
			...
			for( var i = 0 ; i < $("#multiple_file_field")[0].files.length ; i++)
			{
				...
				$.ajax({
					url: 'requires/yarn_dyeing_bill_entry_controller.php?action=file_upload&mst_id='+ mst_id, 
					type: 'post', 
					data:fd, 
					contentType: false, 
					processData: false, 
	...
	function window_close( uid )
	{
		...
		if(uid==0)
		{
			...
			for (var k=1; k<=list_tot_row; k++)
			{
				...
				if(check_id==1)
				{
					...
					{
						$("#outside_yarndyeingbill_table tr").remove();
					}
					//alert(selected_id_listed);
					if( jQuery.inArray(  trid , selected_id_listed ) == -1) 
					{
                        ...
                        var grey_quantity = split_str[24];
                        var available_qty_recv = avilable_qty;

                        if($('#cbo_bill_on').val() == 2){
                            if(grey_quantity > 0){
                                avilable_qty = grey_quantity;
                            }else{
                                errors.push(i);
                            }
                        }
						...
						+'" readonly/></td><td><input type="hidden" name="colorId_'+i+'" id="colorId_'+i+'" value="'+color_id
						+'"><input type="text" name="textColor_'+i+'" id="textColor_'+i+'" class="text_boxes" style="width:60px" value="'+color_name
						...
						+'"><input type="text" name="textWoNum_'+i+'" id="textWoNum_'+i+'" class="text_boxes" style="width:60px" placeholder="Display"  value="'+booking_no+'"/></td><td><select name="cboUom_'+i+'" id="cboUom_'+i+'" class="text_boxes" style="width:55px"><option value="'+order_uom+'">-UOM-</option><option value="1">Pcs</option><option value="2">Dzn</option><option value="12">Kg</option></select></td><td><input type="text" name="txtYarnQty_'+i+'" id="txtYarnQty_'+i+'" class="text_boxes_numeric qtyCalc" data-receiveqty = "'+available_qty_recv+'" data-greyqty = "'+grey_quantity+'" style="width:50px" value="'+avilable_qty
						+'" disabled /></td><td><input type="text" name="txtRate_'+i+'" id="txtRate_'+i+'" class="text_boxes_numeric" style="width:40px" value="'+dye_charge
						+'" onBlur="amount_caculation('+i+');" /></td><td><input type="text" name="txtAmount_'+i+'" id="txtAmount_'+i+'" style="width:70px" class="text_boxes_numeric" value="'+amount
		...
		{
			$("#outside_yarndyeingbill_table tr").remove();
			var list_view_str = return_global_ajax_value( uid, 'load_dtls_data', '', 'requires/yarn_dyeing_bill_entry_controller');
			...
			for (var n=0; n<=split_list_view.length-1; n++)
			{
				...
				trid=trim(trid);
				html+='<tr align="center" id="trent_'+trid+'"><td><input type="hidden" name="updateIdDtls_'+i+'" id="updateIdDtls_'+i+'" value="'+upd_id+'"><input type="hidden" name="updaterate_'+i+'" id="updaterate_'+i+'" value="'+dye_charge+'"><input type="hidden" name="reciveId_'+i+'" id="reciveId_'+i+'" value="'+rec_id
						+'"><input type="text" name="txtReceiveDate_'+i+'" id="txtReceiveDate_'+i+'" class="datepicker" style="width:60px" value="'+recdate
						+'" readonly /></td><td><input type="text" name="txtMrrNo_'+i+'" id="txtMrrNo_'+i+'" value="'+rec_number
						+'" class="text_boxes" style="width:100px" readonly /></td><td><input type="text" name="txtChallenNo_'+i+'" id="txtChallenNo_'+i+'" class="text_boxes" style="width:70px" value="'+rec_challan
						+'" readonly /></td><td><input type="text" name="txtOrderNo_'+i+'" id="txtOrderNo_'+i+'" class="text_boxes" style="width:80px" value="'+job_no
		...
		$("#outside_yarndyeingbill_table").append( html );
		
		var counter =$('#outside_yarndyeingbill_table tr').length;
        if(errors.length > 0){
            alert("Grey quantity 0 (zero) for selected MRR!");
        }
		for(var q=1; q<=counter; q++)
		{
			var index=q-1;
			$("#outside_yarndyeingbill_table tr:eq("+index+")").find("input,select").each(function() {
				$(this).attr({
            ...
            if(errors.length > 0){
                $('#cbo_bill_on').val(1);
                var recv_qty = parseFloat($('#txtYarnQty_'+q).data('receiveqty'));
                $('#txtYarnQty_'+q).val(recv_qty);
                $('#txtAmount_'+q).val(recv_qty*$('#txtRate_'+q).val());
            }
		}
		var tot_row=$('#outside_yarndyeingbill_table tr').length;
	...
	{ 
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/yarn_dyeing_bill_entry_controller.php?data='+data+'&action=outside_bill_popup','Outside Bill Popup', 'width=700px,height=400px,center=1,resize=1,scrolling=0','')
		...
		emailwindow.onclose=function()
		{
			...
			{
				freeze_window(5);
				get_php_form_data( theemail.value, "load_php_data_to_form_outside_bill", "requires/yarn_dyeing_bill_entry_controller" );
				//$("#outside_yarndyeingbill_table tr").remove();
	...
	function fnc_list_search(type) // for populate data
	{
		if( form_validation('cbo_company_id*cbo_supplier_company*txt_bill_from_date*txt_bill_to_date*cbo_bill_for','Company Name*Supplier*Party Source*From Date*To Date*Bill For')==false)
		{
			return;
		}
		...
		{
			show_list_view(document.getElementById('cbo_company_id').value+'***'+document.getElementById('cbo_supplier_company').value+'***'+document.getElementById('cbo_bill_for').value+'***'+document.getElementById('txt_bill_from_date').value+'***'+document.getElementById('txt_bill_to_date').value+'***'+document.getElementById('txt_manual_challan').value+'***'+$('#variable_check').val(),'outside_yarn_dyeing_info_list_view','outside_yarn_dyeing_info_list','requires/yarn_dyeing_bill_entry_controller', 'setFilterGrid("tbl_list_search",-1)','','');
		}
		else
		{
			var tot_row=$('#outside_yarndyeingbill_table tr').length;
			...
			//alert(all_value);
			show_list_view(document.getElementById('cbo_company_id').value+'***'+document.getElementById('cbo_supplier_company').value+'***'+document.getElementById('cbo_bill_for').value+'***'+document.getElementById('txt_bill_from_date').value+'***'+document.getElementById('txt_bill_to_date').value+'***'+document.getElementById('txt_manual_challan').value+'***'+$('#variable_check').val()+'***'+type+'***'+all_value,'outside_yarn_dyeing_info_list_view','outside_yarn_dyeing_info_list','requires/yarn_dyeing_bill_entry_controller','setFilterGrid("tbl_list_search",-1)','','');
		}
	}

	function amount_caculation(id)
	{
        var getTrID = $('#reciveId_'+id).val();
		var getDyeCharge = $('#tr_'+(getTrID*1)).find('.strcls').val();
        var oldRate = getDyeCharge.split('_');
		if(parseFloat($("#txtRate_"+id).val()) > parseFloat(oldRate[18])){
            alert("Increase Bill Rate From W.O. Dye Charge Is Not Allowed! [Maximum Allowed Rate: "+oldRate[18]+"]");
            if($('#updaterate_'+id).length > 0){
                $("#txtRate_"+id).val($('#updaterate_'+id).val());
            }else{
                $("#txtRate_"+id).val(oldRate[18]);
            }
        }
		var tot_row=$('#outside_yarndyeingbill_table tr').length;
		...
		if(curanci==1){
			...
			get_php_form_data( id+"_"+curanci+"_"+$("#txtAmount_"+id).val()*1, "load_domestic_amount", "requires/yarn_dyeing_bill_entry_controller" );
		}else{
	...
	function openmypage_remarks(id)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/yarn_dyeing_bill_entry_controller.php?data='+data+'&action=remarks_popup','Remarks Popup', 'width=420px,height=320px,center=1,resize=1,scrolling=0','')
	...
	function enableDisableFields()
	{
		...
		$('#cbo_supplier_company').removeAttr('disabled','disabled');	
		$('#cbo_bill_for').removeAttr('disabled','disabled');
		$('#txt_bill_date').val('<? echo date('d-m-Y'); ?>');	
	}
	function fnc_print_report(report_type)
	{
		...
		//	alert(report_type);
			generate_report_file( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_bill_no').val()+'*'+report_title+'*'+show_val_column,'outbound_yarn_dyeing_bill_print','requires/yarn_dyeing_bill_entry_controller');
			//return;
			show_msg("3");
		}
	}
	...
	{
		window.open("requires/yarn_dyeing_bill_entry_controller.php?data=" + data+'&action='+action, true );
	}

    function bill_on_fn(type){
        var errors = [];
        if($('.qtyCalc').length > 0) {
            $('#outside_yarndyeingbill_table').find('tr').each(function (index) {
                var recv_qty = parseFloat($(this).find('.qtyCalc').data('receiveqty'));
                var grey_qty = parseFloat($(this).find('.qtyCalc').data('greyqty'));
                if (type == 1) {
                    $(this).find('.qtyCalc').val(recv_qty);
                } else if (type == 2) {
                    if (grey_qty > 0) {
                        $(this).find('.qtyCalc').val(grey_qty);
                    } else {
                        errors.push(index + 1);
                    }
                }
                amount_caculation(index + 1);
            });
        }
        if(errors.length > 0){
            alert("Grey quantity zero for row number "+errors.join(", ")+" !");
            $('#cbo_bill_on').val(1);
            $('#outside_yarndyeingbill_table').find('tr').each(function (index){
                var recv_qty = parseFloat($(this).find('.qtyCalc').data('receiveqty'));
                $(this).find('.qtyCalc').val(recv_qty);
...
<body onLoad="set_hotkey()">
   ...
   <? echo load_freeze_divs ("../",$permission);  ?>
    ...
    <legend>Yarn Dyeing Bill Info </legend>
        <table cellpadding="0" cellspacing="2" width="100%">
            <tr>
             ...
             <tr>
                ...
                <td width="150">
					<?php 
						echo create_drop_down( "cbo_company_id",150,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name","id,company_name", 1, "--Select Company--", $selected, "load_drop_down( 'requires/yarn_dyeing_bill_entry_controller', this.value, 'load_drop_down_location', 'location_td'); load_drop_down( 'requires/yarn_dyeing_bill_entry_controller', this.value, 'load_drop_down_supplier_name', 'supplier_td');","","","","","",2);
                    ?>
                </td>
            ...
            <tr>
                ...
                <td>
					<?
						...
						echo create_drop_down( "cbo_bill_for", 150, $bill_for_yarn,"", 1, "-- Select --", 1, "",0,"","","","",7);
                    ?> 
                </td>
                <td>Party Bill No</td>
            ...
            <tr>
                ...
                <td>
                    <?
                    $bill_on_yarn=array(1=>'Receive Qty.', 2=>'Grey Qty.');
                    echo create_drop_down( "cbo_bill_on", 150, $bill_on_yarn,"", 0, "-- Select --", 1, "bill_on_fn(this.value)",0,"","","","");
                    ?>
                </td>
            </tr>
            <tr>
                <td colspan="4" align="center"><input class="formbutton" type="button" onClick="fnc_list_search(0);" style="width:130px" name="btn_populate" value="Populate" id="btn_populate" /></td>
				<td><input type="button" id="file_uploaded" class="image_uploader" style="width:130px" value="ADD FILE" onClick="file_uploader ( '../', document.getElementById('update_id').value,'', 'yarn_dying_bill_entry', 2 ,1)"></td>
        ...
        <table  style="border:none; width:950px;" cellpadding="0" cellspacing="1" border="0" id="">
            ...
            <tbody id="outside_yarndyeingbill_table">
                <tr align="center">				
                    <td>
                    ...
                    <td>
                        ...
                        <input type="text" name="textColor_1" id="textColor_1"  class="text_boxes" style="width:60px" readonly  placeholder="Display"/>
                    </td>
                    <td>
                    ...
                    <td>
                        <input type="text" name="txtYarnQty_1" id="txtYarnQty_1"  class="text_boxes_numeric" style="width:50px" readonly  placeholder="Display"/>
                    </td>
                    <td>
            ...
            <tfoot>
                ...
                <tr>
                    <td colspan="17" align="center" class="button_container">
						<? 
							echo load_submit_buttons($permission,"fnc_outside_finishing_bill_entry",0,0,"reset_form('outfinishingbill_1','outside_yarn_dyeing_info_list','','','enableDisableFields();$(\'#bill_issue_table tr:not(:first)\').remove();')",1);
                        ?> 
        ...
        <br>
        <div id="outside_yarn_dyeing_info_list"></div>  
        </form>
