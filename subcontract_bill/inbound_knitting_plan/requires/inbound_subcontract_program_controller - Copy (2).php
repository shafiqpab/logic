	...
	*/
	...
	foreach ($resultSet as $row)
	{
		...
		if(($cbo_planning_status == 2 && $balanceQty <= 0) || ($cbo_planning_status == 1 && $balanceQty > 0))
		{
			$i++;
			$isRequisition = 0;
			$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('order_id')]]['partyId'] = $row[csf('party_id')];
			$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('order_id')]]['partyDtls'] = $buyer_details[$row[csf('party_id')]];
			$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('order_id')]]['orderRcvDate'] = date('d-m-Y', strtotime($row[csf('order_rcv_date')]));
			$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('order_id')]]['orderNo'] = $row[csf('order_no')];
			$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('order_id')]]['fabricId'] = $row[csf('item_id')];
			$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('order_id')]]['fabricDtls'] = $fabric_details[$row[csf('item_id')]];
			
			$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('order_id')]]['machine_gg'] = $machine_gg;
			$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('order_id')]]['grey_dia'] = $row[csf('grey_dia')];
			$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('order_id')]]['gsm'] = $row[csf('gsm')];
			$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('order_id')]]['dia'] = $row[csf('finish_dia')];
			$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('order_id')]]['diaWidthTypeId'] = $row[csf('dia_width_type')];
			$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('order_id')]]['diaWidthTypeDtls'] = $fabric_typee[$row[csf('dia_width_type')]];
			$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('order_id')]]['orderQty'] = $orderQty;
			$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('order_id')]]['programQty'] = $programQty;
			$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('order_id')]]['balanceQty'] = $balanceQty;
			$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('order_id')]]['isRequisition'] = $isRequisition;
			$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('order_id')]]['jobNo'] = $row[csf('subcon_job')];
	...
	foreach ($rptData as $ordId=>$ordArr)
	{
		foreach ($ordArr as $febId=>$febArr)
		{
			...
			{
				foreach ($gsmArr as $diaNo=>$diaData)
				{
					if($diaNo != '')
					{
						//subTotal
						$subTotal[$diaNo]['orderQty'] += $diaData['orderQty'];
						$subTotal[$diaNo]['programQty'] += $diaData['programQty'];
						$subTotal[$diaNo]['balanceQty'] += $diaData['balanceQty'];
						
						//grandTotal
						$grandTotal['orderQty'] += $diaData['orderQty'];
						$grandTotal['programQty'] += $diaData['programQty'];
						$grandTotal['balanceQty'] += $diaData['balanceQty'];
					}


					foreach ($diaData as $orderID=>$row)
					{
						$sl++;
						$rptPData[$diaNo][$orderID][$sl] = $row;
					}
    ...
    <form name="palnningEntry_2" id="palnningEntry_2">
        <fieldset style="width:1100px;">
            ...
            <table width="1070" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" id="tbl_list_search">
				...
				{
					foreach ($diaArr as $orderIDs=>$orderArr)
					{
						foreach ($orderArr as $sl=>$row)
						{
							$rowNo++;
