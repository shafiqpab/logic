	...
	*/
	...
	foreach ($resultSet as $row)
	{
		...
		if(($cbo_planning_status == 2 && $balanceQty <= 0) || ($cbo_planning_status == 1 && $balanceQty > 0))
		{
			...
			{
				$rptData[$row[csf('sdid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]]['progNo'] = $progNoArr[$row[csf('smid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('sdid')]];
			}
		}
	}
	unset($resultSet);
				...
				foreach ($rptPData as $diaNo=>$diaArr)
				{
					foreach ($diaArr as $sl=>$row)
					{
						...
						<tr bgcolor="<?php echo $rowColor; ?>" style="text-decoration:none; cursor:pointer; vertical-align:middle;" onClick="fnc_selected_row('<?php echo $rowNo; ?>', '')" id="tr_<?php echo $rowNo; ?>">
							...
							$print_program_no = "";							
							if($row['progNo'] !=''){
								$print_program_no = "<a href='##' onclick=\"generate_report2(".$company_name.",".$row['progNo'].",".$program_info_format_id.")\">".$row['progNo']."</a>";
							}							
...
if ($action == "actn_program")
{
	...
	<script>
		...
		function fnc_program_entry(operation)
		{
			...
			var data = "action=save_update_delete&operation=" + operation + get_submitted_data_string('cbo_knitting_source*cbo_knitting_party*txt_color*txt_machine_dia*txt_machine_gg*txt_program_qnty*txt_stitch_length*txt_spandex_stitch_length*txt_draft_ratio*machine_id*txt_machine_capacity*txt_distribution_qnty*cbo_knitting_status*txt_start_date*txt_end_date*txt_program_date*cbo_feeder*txt_remarks*save_data*updateId*update_dtls_id*cbo_color_range*cbo_dia_width_type*hidden_color_id*txt_fabric_dia*cbo_location_name*hidden_advice_data*hidden_no_of_feeder_data*hidden_collarCuff_data*hidden_count_feeding_data*hidden_came_dsign_string_data*hdn_fab_desc*hidden_color_wise_prog_data', "../../../")
			+ '&planId='+planId
			+ '&orderMstId='+orderMstId
			+ '&companyId='+companyId
			+ '&gsm='+gsm 
...
if ($action == "actn_set_planning_data")
{
	...
	*/
	...
	foreach ($data_array as $row)
	{
		...
		$str_collar = '';
		$data_collar_cuff = sql_select("SELECT id, mst_id, dtls_id, body_part_id, grey_size,finish_size, qty_pcs FROM subcon_planning_collar_cuff_dtls WHERE dtls_id='".$data."' AND status_active=1 AND is_deleted=0");
	...
	if($program_id!="")
	{
		$sqlPlan = "
			SELECT 
				...
				sppd.id, sppd.mst_id, sppd.dtls_id, sppd.determination_id, sppd.gsm_weight, sppd.dia, sppd.buyer_id, sppd.fabric_desc,
				sod.order_no
			FROM 
