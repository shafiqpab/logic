...
if ($action == "actn_show_details")
{
	...
	*/
	...
	$poIdsArr = array_filter($poIdsArr);
	$all_po_ids = implode(",", $poIdsArr);
	$all_prod_id_cond=""; $poIdCond=""; 
	if($db_type==2 && count($poIdsArr)>999)
	{
		$all_poIdsArr_chunk=array_chunk($poIdsArr,999) ;
		foreach($all_poIdsArr_chunk as $chunk_arr)
		{
			$chunk_arr_value=implode(",",$chunk_arr);	
			$poIdCond.="  c.po_id in($chunk_arr_value) or ";
		}
		$all_prod_id_cond.=" and (".chop($poIdCond,'or ').")";
	}
	else
	{
		$all_prod_id_cond=" and c.po_id in($all_po_ids)";
	}

	$mstIdArr = array_filter($mstIdArr);
	$all_mst_ids = implode(",", $mstIdArr);
	$all_mst_id_cond=""; $mstIdCond=""; 
	if($db_type==2 && count($mstIdArr)>999)
	{
		$all_mstIdsArr_chunk=array_chunk($mstIdArr,999) ;
		foreach($all_mstIdsArr_chunk as $array_chunk)
		{
			$chunk_arr_values=implode(",",$array_chunk);	
			$mstIdCond.="  a.subcon_order_id in($chunk_arr_values) or ";
		}
		$all_mst_id_cond.=" and (".chop($mstIdCond,'or ').")";
	}
	else
	{
		$all_mst_id_cond=" and a.subcon_order_id in($all_mst_ids)";
	}
	...
	$sqlPlan = "SELECT a.subcon_order_id, c.id, c.mst_id, c.determination_id, c.gsm_weight, c.dia, ".$queryProgNo." SUM(c.program_qnty) AS program_qnty, c.status_active, c.po_id,b.machine_gg FROM subcon_planning_mst a, subcon_planning_dtls b, subcon_planning_plan_dtls c WHERE a.id = b.mst_id AND b.id = c.dtls_id $all_mst_id_cond $all_prod_id_cond  AND b.status_active = 1 AND b.is_deleted = 0 AND c.is_revised=0 GROUP BY a.subcon_order_id, c.id, c.mst_id, c.yarn_desc, c.body_part_id, c.determination_id, c.gsm_weight, c.dia, c.status_active, c.po_id,b.machine_gg";
	...
	foreach ($resultPlan as $rowPlan)
	{
		...
		$program_data_array[$rowPlan[csf('subcon_order_id')]][$rowPlan[csf('determination_id')]][$rowPlan[csf('gsm_weight')]][$rowPlan[csf('dia')]][$rowPlan[csf('po_id')]]['program_qnty'] += $rowPlan[csf('program_qnty')];
		$program_data_array[$rowPlan[csf('subcon_order_id')]][$rowPlan[csf('determination_id')]][$rowPlan[csf('gsm_weight')]][$rowPlan[csf('dia')]][$rowPlan[csf('po_id')]]['machine_gg'] = $rowPlan[csf('machine_gg')];
	...
	*/
	...
	{
		//program Qty
		if(empty($program_data_array[$row[csf('smid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('sdid')]]['program_qnty']))
		{
			$programQty = 0;
		}
		else
		{
			$programQty = $program_data_array[$row[csf('smid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('sdid')]]['program_qnty'];
		}
		$machine_gg = $program_data_array[$row[csf('smid')]][$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('finish_dia')]][$row[csf('sdid')]]['machine_gg'];
...
if ($action == "actn_program")
{
	...
	<script>
		...
		{
			if (form_validation('cbo_knitting_party', 'Knitting Company') == false)
			{
				return;
			}
			...
			var knittingParty = $('#cbo_knitting_party').val();

			var title = 'Machine Info';
			var page_link = 'inbound_subcontract_program_controller.php?action=actn_machine&save_string=' + save_string + '&companyId=' + knittingParty + '&txt_machine_dia=' + txt_machine_dia + '&update_dtls_id=' + update_dtls_id + '&txt_program_qnty='+txt_program_qnty + '&txt_machine_gg='+txt_machine_gg;
		...
		{
			if (form_validation('cbo_knitting_party*txt_machine_dia*txt_machine_gg*txt_fabric_dia*txt_program_qnty*txt_machine_no', 'Knitting Company*Machine Dia*Machine GG*Finish Fabric Dia*Program Qnty*Machine No') == false)
			{
				return;
			}
                ...
                <table width="900" align="center" border="0">
                    <tr>
                        ...
                        <td id="knitting_party">
                            <?
							echo create_drop_down("cbo_knitting_party", 177, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "--Select Knit Party--", $companyId, "company_onchange();", "");
	...
	*/
	if ($operation == 0)
	{
		...
		*/
		if ($flag == 1)
		{
			//echo "10**insert into subcon_planning_dtls (".$field_array_dtls.") values ".$data_array_dtls;die;
		...
		*/
		...
		if ($save_data != "")
		{
			if ($data_array_machine_dtls != "")
			{
				//echo "10**insert into subcon_planning_machine_dtls (".$field_array_machine_dtls.") values ".$data_array_machine_dtls;die;
		...
		<form name="searchwofrm" id="searchwofrm">
			<fieldset style="width:820px; margin-top:10px; margin-left:5px">
				...
				<div style="width:818px; overflow-y:scroll; max-height:220px;" id="buyer_list_view">
                    ...
                    //echo $companyId.'=';die;
                    $sql = "SELECT id, machine_no, dia_width, gauge, machine_group, prod_capacity, floor_id from lib_machine_name where company_id=$companyId and category_id=1 and is_subcon=2 and status_active=1 and is_deleted=0 $machinCond order by seq_no";// and dia_width='$txt_machine_dia'
                    // echo $sql;
					$result = sql_select($sql);
	...
	if($program_id!="")
	{
		$sqlPlan = "
			SELECT
				...
				spd.knitting_source, spd.knitting_party, spd.color_id, spd.color_range, spd.machine_dia, spd.width_dia_type, spd.machine_gg, spd.fabric_dia,spd.width_dia_type, spd.program_qnty, spd.stitch_length, spd.spandex_stitch_length, spd.draft_ratio, spd.machine_id, spd.machine_capacity, spd.distribution_qnty, spd.status, spd.start_date, spd.end_date, spd.program_date, spd.feeder, spd.remarks, spd.save_data, spd.location_id, spd.advice,
				...
				<tbody>
					...
					foreach ($machine_idarr as $machineid)
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>">
							...
							<td align="center"><? echo $fabric_typee[$planData[0][csf('width_dia_type')]];?></td>
							...
							<td align="center"><? echo $feeder[$planData[0][csf('feeder')]]; ?></td>
