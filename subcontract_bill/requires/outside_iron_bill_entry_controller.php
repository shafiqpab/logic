...
if ($action=="bill_no_popup")
{
	...
	<body>
        <div align="center" style="width:100%;" >
            <form name="ironbill_1"  id="ironbill_1" autocomplete="off">
                    ...
                    <tbody>
                        <tr>
                            <td> 
                                ...
                                <?   
									echo create_drop_down( "cbo_company_id", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", $ex_data[0],"load_drop_down( 'outside_iron_bill_entry_controller', this.value, 'load_drop_down_supplier', 'supplier_td' );",0 );
                                ?>
                            </td>
...
if ($action=="iron_bill_list_view")
{
	...
	$sql= "select id, bill_no, prefix_no_num, $year_cond, location_id, bill_date, supplier_id, bill_for from subcon_outbound_bill_mst where process_id=7 and status_active=1 $company_name $party_name $return_date $bill_id_cond";
	
	echo  create_list_view("list_view", "Bill No,Year,Location Name,Bill Date,Party Name,Bill For", "70,70,100,100,120,100","600","250",0, $sql , "js_set_value", "id", "", 1, "0,0,location_id,0,supplier_id,bill_for", $arr , "prefix_no_num,year,location_id,bill_date,supplier_id,bill_for", "outside_iron_bill_entry_controller","",'0,0,0,3,0,0') ;
	exit(); 
}
...
if ($action=="load_php_data_to_form_issue")
{
	...
	{	
		...
		echo "load_drop_down( 'requires/outside_iron_bill_entry_controller', document.getElementById('cbo_company_id').value, 'load_drop_down_location', 'location_td' );\n";
		echo "load_drop_down( 'requires/outside_iron_bill_entry_controller', document.getElementById('cbo_company_id').value, 'load_drop_down_supplier', 'supplier_td' );\n";
...
if ($action=="iron_entry_list_view")
{
	...
	<body>
        ...
        <div style="width:800px;max-height:180px; overflow-y:scroll" id="iron_list_view" align="center">
            ...
            if($data[2]=="") // Insert
            {
                $sql="select id, challan_no, production_date, po_break_down_id, item_number_id, serving_company, production_quantity from pro_garments_production_mst where company_id=$data[0] and serving_company=$data[1] and production_source=3 and production_type=7 and status_active=1 and is_deleted=0";			
            }
            else
            {
                $sql="(select id, challan_no, production_date, po_break_down_id, item_number_id, serving_company, production_quantity from pro_garments_production_mst where company_id=$data[0] and serving_company=$data[1] and id NOT IN (SELECT receive_id FROM subcon_outbound_bill_dtls) and production_source=3  and production_type=7 and status_active=1 and is_deleted=0) 
...
if ($action=="load_php_dtls_form") 
{
	...
	if( $data[2]!="" )//update===========
	{
		$sql="SELECT id as upd_id, receive_id, receive_date, challan_no, order_id, item_id as prod_id, receive_qty as order_qnty, uom, rate, amount, remarks FROM subcon_outbound_bill_dtls  WHERE mst_id=$data[2] and process_id=7";
	}
	else //insert=================
	{
		if($bill_id!="" && $del_id!="")
			$sql="(SELECT id as upd_id, receive_id, receive_date, challan_no, order_id, item_id as prod_id, receive_qty as order_qnty, uom, rate, amount, remarks  FROM subcon_outbound_bill_dtls  WHERE receive_id in ($bill_id) and process_id='7' )
			 union 
			 (SELECT 0 as upd_id, id as receive_id, production_date as receive_date, challan_no, po_break_down_id as order_id, item_number_id as prod_id, sum(production_quantity) as order_qnty, 0 as uom, 0 as rate, 0 as amount, null as remarks FROM pro_garments_production_mst WHERE id in ($del_id) and production_source=3 and production_type=7 and status_active=1 and is_deleted=0 group by id, production_date, challan_no, po_break_down_id, item_number_id)";
	...
	?>
       <tr align="center">				
            <td>
                ...
                <input type="date" name="txtreceivedate_<? echo $k; ?>" id="txtreceivedate_<? echo $k; ?>"  class="datepicker" style="width:65px" value="<? echo change_date_format($row[csf("receive_date")]); ?>" readonly />									
            </td>
            <td>
            ...
            <td>
                <input type="text" name="text_wo_num_<? echo $k; ?>" id="text_wo_num_<? echo $k; ?>"  class="text_boxes" style="width:60px" value="<? echo $row[csf("")]; ?>"  readonly />   <!--placeholder="Browse" onDblClick="openmypage_wonum();" -->
            </td>
            <td>
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   // Insert Here 
	{
		...
		$new_bill_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '', 'IRN', date("Y",time()), 5, "select prefix_no,prefix_no_num from  subcon_outbound_bill_mst where company_id=$cbo_company_id and process_id=$bill_process_id $year_cond=".date('Y',time())." order by id desc ", "prefix_no", "prefix_no_num" ));
		...
		$field_array1 ="id, mst_id, receive_id, receive_date, challan_no, order_id, item_id, receive_qty, uom, rate, amount, remarks, process_id, inserted_by, insert_date";
		$field_array_up ="receive_id*receive_date*challan_no*order_id*item_id*receive_qty*uom*rate*amount*remarks*updated_by*update_date";
		$add_comma=0;
		for($i=1; $i<=$tot_row; $i++)
		{
			...
			if(str_replace("'",'',$$updateid_dtls)=="")  
			{
				...
				$data_array1 .="(".$id1.",".$id.",".$$reciveid.",".$$receive_date.",".$$challen_no.",".$$orderid.",".$$item_id.",".$$quantity.",".$$cbouom.",".$$rate.",".$$amount.",".$$remarks.",'".$bill_process_id."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				$id1=$id1+1;
				$add_comma++;
			}
			else
			{
				...
				$data_array_up[str_replace("'",'',$$updateid_dtls)] =explode("*",("".$$reciveid."*".$$receive_date."*".$$challen_no."*".$$orderid."*".$$item_id."*".$$quantity."*".$$cbouom."*".$$rate."*".$$amount."*".$$remarks."*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'"));
