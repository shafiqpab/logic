	...
	*/	
	...
	if($operation==0)// Insert Here
	{
		...
		if(str_replace("'","",$update_id)=="")
		{
			...
			$field_array="id, batch_against, batch_no, floor_id, machine_no, location_id, batch_date, entry_form, company_id, extention_no, color_id, batch_weight, total_trims_weight, color_range_id, process_id, dur_req_hr, dur_req_min,remarks,double_dyeing,collar_qty,cuff_qty, inserted_by, insert_date";

			$data_array="(".$id.",".$cbo_batch_against.",".$txt_batch_number.",".$cbo_dyeing_floor.",".$txt_machine_no.",".$cbo_location_name.",".$txt_batch_date.",36,".$cbo_company_id.",".$txt_ext_no.",".$color_id.",".$txt_batch_weight.",".$txt_tot_trims_weight.",".$cbo_color_range.",".$txt_process_id.",".$txt_du_req_hr.",".$txt_du_req_min.",".$txt_remarks.",".$cbo_double_dyeing.",".$txt_collar_qty.",".$txt_cuff_qty.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		}
		else
		{
			...
			$field_array_update="batch_against*batch_no*floor_id*machine_no*location_id*batch_date*company_id*extention_no*color_id*batch_weight*total_trims_weight*color_range_id*process_id*dur_req_hr*dur_req_min*remarks*double_dyeing*collar_qty*cuff_qty*updated_by*update_date";
	...
	else if ($operation==1)   // Update Here
	{
		...
		if(str_replace("'","",$cbo_batch_against)==2 && str_replace("'", "", $unloaded_batch) != "" && str_replace("'", "", $ext_from) == 0)
		{
			...
			$batch_update_id=$id;
			$serial_no=date("y",strtotime($pc_date_time))."-".$id;
			$field_array="id, batch_against, batch_no, floor_id, machine_no,location_id, batch_date, entry_form, company_id, extention_no, re_dyeing_from, color_id, batch_weight, total_trims_weight, color_range_id, process_id, dur_req_hr, dur_req_min,remarks,double_dyeing,collar_qty,cuff_qty, inserted_by, insert_date";
		...
		else
		{
			...
			$field_array_update="batch_against*batch_no*floor_id*machine_no*location_id*batch_date*extention_no*color_id*batch_weight*total_trims_weight*color_range_id*process_id*dur_req_hr*dur_req_min*remarks*double_dyeing*collar_qty*cuff_qty*updated_by*update_date";
...
if ($action=="populate_data_from_search_popup")
{
	...
	$data_array=sql_select("select id, company_id, location_id, floor_id, machine_no, batch_against, batch_no, extention_no, batch_weight, total_trims_weight, batch_date, re_dyeing_from, color_id, color_range_id, process_id,double_dyeing,collar_qty,cuff_qty,dur_req_hr, dur_req_min,remarks, $year_cond from pro_batch_create_mst where id='$batch_id' and entry_form=36");
