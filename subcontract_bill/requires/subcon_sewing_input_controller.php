...
if($action=="populate_data_from_search_popup")
{
 	...
 	foreach($sql_res as $result)
	{
		...
		// echo "document.getElementById('txt_manual_cut_no').value 		= '".$result[csf("manual_cut_no")]."';\n";
		// echo "$('#txt_cutting_qty').attr('placeholder','');\n";
		
		$total_produced = return_field_value("sum(production_qnty)","subcon_gmts_prod_dtls","order_id='$po_id' and gmts_item_id='$item_id' and production_type=7 and is_deleted=0");
		$total_cutting = return_field_value("sum(production_qnty)","subcon_gmts_prod_dtls","order_id='$po_id' and gmts_item_id='$item_id' and production_type=1 and is_deleted=0");
		echo "$('#txt_cumul_sewing_qty').attr('placeholder','".$total_produced."');\n";
		echo "$('#txt_cumul_sewing_qty').val('".$total_produced."');\n";
		$yet_to_produced = $total_cutting  - $total_produced;
...
if($action=="chk_next_process_qty")
{
	...
	{
		$size_cond = " and c.size_id=$sizeId";
	}
	// $col_size_id = explode("*", str_replace("'", "", $hidden_colorSizeID));
	$sql_cutting = "SELECT sum(case when b.production_type=7 then b.prod_qnty else 0 end) as input_qty,
	sum(case when b.production_type=1 then b.prod_qnty else 0 end) as cutting_qty 
	from subcon_gmts_prod_dtls a,subcon_gmts_prod_col_sz b, subcon_ord_breakdown c where a.id=b.dtls_id and c.id=b.ord_color_size_id  and a.status_active=1 and a.is_deleted=0 and a.gmts_item_id=$cbo_item_name and c.color_id=$colorId  and c.status_active=1 and c.is_deleted=0 and a.order_id=$hidden_po_break_down_id and b.production_type in(7,1) $size_cond ";
	  //echo $sql_cutting;
	$sql_res_cuttting = sql_select($sql_cutting);
	echo $sql_res_cuttting[0][csf('cutting_qty')]."****".$sql_res_cuttting[0][csf('INPUT_QTY')];
...
{ 
	...
	//#############################################################################################//
	/*if($variableSettings!=1 ) // color n Size level
	{
	$sql_dtls = sql_select("select color_size_break_down_id,production_qnty,size_number_id, color_number_id from  subcon_gmts_prod_col_sz a,subcon_ord_breakdown b where a.mst_id=$data and a.status_active=1 and a.ord_color_size_id=b.id and b.order_id='$po_id' and b.item_number_id='$item_id' and b.country_id='$country_id'");
	...
	$sqlProd =sql_select(" SELECT b.ord_color_size_id,  (b.prod_qnty) AS prod_qnty FROM subcon_gmts_prod_dtls a, subcon_gmts_prod_col_sz b WHERE a.id = b.dtls_id AND a.production_type = b.production_type AND b.production_type = 1 and a.order_id in($po_id)  and a.gmts_item_id in($item_id) ");
	//echo " SELECT b.ord_color_size_id,  (b.prod_qnty) AS prod_qnty FROM subcon_gmts_prod_dtls a, subcon_gmts_prod_col_sz b WHERE a.id = b.dtls_id AND a.production_type = b.production_type AND b.production_type = 1 and a.order_id in($po_id)  and a.gmts_item_id in($item_id) ";
	 
 	$prod_cur_qntyArr=array();
	foreach($sqlProd as $result)
	{
		$prod_cur_qntyArr[$result[csf("ord_color_size_id")]]+=$result[csf("prod_qnty")];
	}
	...
	//echo $sql;
		...
		$sql_data_order=sql_select("SELECT  a.main_process_id,a.process_id, a.order_no, c.item_id   from  subcon_ord_dtls a, subcon_ord_mst b, subcon_ord_breakdown c 
		where b.id=c.mst_id and a.status_active=1 and a.is_deleted=0 and a.job_no_mst=b.subcon_job and a.main_process_id=11 and a.id=c.order_id and a.id='$po_id' and c.item_id='$item_id'");

		foreach($sql_data_order as $row){
			$process_id_arr=explode(",",$row[csf('process_id')]);
			foreach($process_id_arr as $process_id){
				$processIdArr[$process_id]=$process_id;
			}
		}
	...
	foreach($colorResult as $color)
	{
		...
		if(in_array(120,$processIdArr)){
			$prod_cur_qnty=$prod_cur_qntyArr[$color[csf("id")]];
			$color[csf("qnty")]='';
			$color[csf("qnty")]=$prod_cur_qnty;
		}else{
			$prod_cur_qnty=$color[csf("qnty")];
		}
		
		//echo $color[csf("id")].'='.$prod_cur_qnty.'<BR>'; 
		...
		{ 
			...
			$colorHTML .='<tr><td>'.$color_library[$color[csf("color_id")]].'</td><td><input type="text" name="colSize" id="colSize_'.$i.'" style="width:80px"  class="text_boxes_numeric" placeholder="'.($color[csf("qnty")]-$color[csf("production_qnty")]).'" onBlur="fn_chk_next_process_qty('.$color[csf("color_id")].','.($i).','.$color[csf("size_id")].');fn_colorlevel_total('.$i.');"><input type="hidden" id="txt_colo_size_mst_id_'.$i.'" name="txt_colo_size_mst_id_'.$i.'" value="'.$colSizeID_arr[$color[csf("color_id")]].'"><input type="hidden" name="colSizeUpQty" id="colSizeUpQty_'.$i.'" value="'.$prevQty.'" ><input type="hidden" name="colSizeCutQty" id="colSizeCutQty'.$i.'" value="'.$prod_cur_qnty.'" ></td></tr>';				
			$totalQnty += $color[csf("qnty")]-$color[csf("production_qnty")];
		...
		else //color and size level
		{
			...
			$colorHTML .='<tr><td>'.$size_library[$color[csf("size_id")]].'</td><td><input type="text" name="colSize" id="colSize'.$k.'___'.$z.'"  class="text_boxes" style="text-align:right;width:100px;"  placeholder="'.($color[csf("qnty")]-$color[csf("production_qnty")]).'" onBlur="fn_chk_next_process_qty('.$k.','.$z.','.$color[csf("size_id")].');sum_qnty('.$k.','.$z.' );"><input type="hidden" id="txt_colo_size_mst_id'.$k.'___'.$z.'" name="txt_colo_size_mst_id'.$k.'___'.$z.'" value="'.$color[csf("id")].'"><input type="hidden" name="colSizeUpQty" id="colSizeUpQty_'.$k.$z.'" value="'.$prevQty.'"><input type="hidden" name="colSizeCutQty" id="colSizeCutQty'.$k.$z.'" value="'.$prod_cur_qnty.'" ></td></tr>';		//fn_total('.$i.')
		}
		$z++;
		$j++; 
	}
	...
	<div style="width:100%;">
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="920" class="rpt_table">
            <thead>
                ...
                <th width="100" align="center">Level</th> 
                <th width="" align="center">Manual Cut No</th>                   
            </thead>
		<? 		 
		...
		if($db_type==0)  $hour_field=" TIME_FORMAT(hour, '%H:%i' ) as hour"; 
			...
			$i=1;
			 $sql="SELECT id, production_date, production_qnty, reject_qnty, floor_id, prod_reso_allo,alter_qnty, spot_qnty, supervisor, challan_no, manual_cut_no, remarks, line_id, location_id, entry_break_down_type, $hour_field, minute from  subcon_gmts_prod_dtls where production_type=7 and order_id='$po_id' and gmts_item_id='$item_id' and status_active=1 and is_deleted=0 and entry_break_down_type='$sewing_production_variable'  order by production_date desc ";
			...
			?>
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"> 
                    ...
                    <td onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_sub_sewing_form_data','requires/subcon_sewing_input_controller');"  width="30" align="center"><? echo $i; ?></td>
                    <td onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_sub_sewing_form_data','requires/subcon_sewing_input_controller');"  width="80" align="center"><?php echo change_date_format($selectResult[csf('production_date')]); ?></td>
                    <td onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_sub_sewing_form_data','requires/subcon_sewing_input_controller');"  width="80" align="center"><?php echo $selectResult[csf('hour')]; ?> </td> 
                    <td onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_sub_sewing_form_data','requires/subcon_sewing_input_controller');"  width="80" align="right"><?php echo $selectResult[csf('production_qnty')]; ?>&nbsp;</td>
                    <td onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_sub_sewing_form_data','requires/subcon_sewing_input_controller');"  width="80" align="right"><?php echo $line_name; ?>&nbsp;</td>
                    <td onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_sub_sewing_form_data','requires/subcon_sewing_input_controller');"  width="80" align="right"><?php echo $selectResult[csf('reject_qnty')]; ?>&nbsp;</td>
                    <td onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_sub_sewing_form_data','requires/subcon_sewing_input_controller');"  width="80" align="right"><?php echo $selectResult[csf('alter_qnty')]; ?>&nbsp;</td>
                        <?php 
                            $location_name= return_field_value("location_name","lib_location","id='".$selectResult[csf('location_id')]."'");
                        ?>
                    <td onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_sub_sewing_form_data','requires/subcon_sewing_input_controller');"  width="120" align="center"><p><? echo $location_name; ?></p></td>
                        <?php
                            $floor_name = return_field_value( "floor_name","lib_prod_floor","id='".$selectResult[csf('floor_id')]."'");
                        ?>                
                    <td onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_sub_sewing_form_data','requires/subcon_sewing_input_controller');"  width="80" align="center"><p><?php echo $floor_name; ?></p></td>  
                                        
                    <td onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_sub_sewing_form_data','requires/subcon_sewing_input_controller');"  width="100" align="center"><p><?php echo $production_update_areas[$selectResult[csf('entry_break_down_type')]]; ?></p></td>                    
                    <td onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_sub_sewing_form_data','requires/subcon_sewing_input_controller');"  width="" align="center"><p><?php echo $selectResult[csf('manual_cut_no')]; ?></p></td>                    
                </tr>
			<?php
			$i++;
			}
...
if($action=="populate_sub_sewing_form_data")
{
	...
	//echo $order_id.'='.$gmts_item_id;

	// ========================== getting production qnty ==============================

	
	$sqlProd =sql_select(" SELECT b.ord_color_size_id, 
	 (b.prod_qnty) AS prod_qnty FROM subcon_gmts_prod_dtls a, subcon_gmts_prod_col_sz b WHERE a.id = b.dtls_id AND a.production_type = b.production_type AND b.production_type = 1 and a.order_id in($order_id)  and a.gmts_item_id in($gmts_item_id)");
	...
	$sqlResult =sql_select("SELECT c.color_id, c.size_id,c.id as break_id, a.id, a.company_id,a.manual_cut_no, a.location_id, a.floor_id, a.line_id, a.prod_reso_allo, a.order_id, a.production_date, a.production_qnty, a.reject_qnty, a.alter_qnty, a.spot_qnty, a.supervisor, a.production_type, a.entry_break_down_type, a.table_no, a.challan_no, a.remarks, $hour_field, a.minute, b.production_type, b.gmts_item_id, b.production_date, b.ord_color_size_id, b.prod_qnty from subcon_gmts_prod_dtls a,subcon_gmts_prod_col_sz b,subcon_ord_breakdown c 
		where a.id=b.dtls_id and a.production_date=b.production_date and a.status_active=1 and a.is_deleted=0 and a.id='$data' and c.id=b.ord_color_size_id 	
 order by c.color_id,c.id");
  	...
  	foreach($sqlResult as $result)
	{
		if($i==0)
		{
			...
			echo "load_drop_down( 'requires/subcon_sewing_input_controller', document.getElementById('cbo_location').value, 'load_drop_down_floor', 'floor_td' );\n";			
			...
			echo "load_drop_down( 'requires/subcon_sewing_input_controller', document.getElementById('cbo_floor').value+'_'+document.getElementById('cbo_location').value+'_'+document.getElementById('prod_reso_allo').value+'_'+document.getElementById('txt_sewing_date').value, 'load_drop_down_sewing_line_floor', 'sewing_line_td' );\n";
			...
			echo "$('#txt_manual_cut_no').val('".$result[csf('manual_cut_no')]."');\n";
			...
			echo "set_button_status(1, permission, 'fnc_subcon_sewing_input_entry',1);\n";
	...
	$i++;
		...
		$prod_cur_qnty=$prod_cur_qntyArr[$result[csf("ord_color_size_id")]]; 
		
		$break_plan_cut_library[$result[csf("break_id")]]='';
		$break_plan_cut_library[$result[csf("break_id")]]=$prod_cur_qnty;
		
		if( $variableSettings==1 ) // gross level
		{
			die; 
		} //$prod_cur_qnty[$result[csf("ord_color_size_id")]]
		else if( $variableSettings==2 ) // Color level
		{
			$colorHTML .='<tr><td>'.$color_library[$break_color_library[$result[csf("ord_color_size_id")]]].'</td><td><input type="text" name="colorSize" id="colSize_'.$i.'" style="width:80px"  class="text_boxes_numeric" placeholder="'.($break_plan_cut_library[$result[csf("break_id")]]-$result[csf("prod_qnty")]).'" title="'.($break_plan_cut_library[$result[csf("break_id")]]-$result[csf("prod_qnty")]).'" value="'.$result[csf("prod_qnty")].'" onblur="fn_chk_next_process_qty('.$result[csf("color_id")].','.$i.','.$result[csf("size_id")].');fn_colorlevel_total('.$i.');"><input type="hidden" id="txt_colo_size_mst_id_'.$i.'" name="txt_colo_size_mst_id_'.$i.'" value="'.$result[csf("ord_color_size_id")].'"><input type="hidden" name="colSizeUpQty" id="colSizeUpQty_'.$i.'" value="'.$prevQty.'" ><input type="hidden" name="colSizeCutQty" id="colSizeCutQty'.$i.'" value="'.$prod_cur_qnty.'" ></td></tr>';				
		...
		{	
			...
			$colorHTML .='<tr><td>'.$size_library[$break_size_library[$result[csf("ord_color_size_id")]]].'</td><td><input type="text" name="colSize" id="colSize'.$k.'___'.$z.'"  class="text_boxes_numeric" style="text-align:right;width:100px;" placeholder="'.($break_plan_cut_library[$result[csf("break_id")]]-$result[csf("prod_qnty")]).'"  onblur="fn_chk_next_process_qty('.$k.','.$z.','.$result[csf("size_id")].');sum_qnty('.$k.','.$z.' );" title="'.($break_plan_cut_library[$result[csf("break_id")]]-$result[csf("prod_qnty")]).'" value="'.$result[csf("prod_qnty")].'"><input type="hidden" id="txt_colo_size_mst_id'.$k.'___'.$z.'" name="txt_colo_size_mst_id'.$k.'___'.$z.'" value="'.$result[csf("ord_color_size_id")].'"><input type="hidden" name="colSizeUpQty" id="colSizeUpQty_'.$k.$z.'" value="'.$prevQty.'" ><input type="hidden" name="colSizeCutQty" id="colSizeCutQty'.$k.$z.'" value="'.$prod_cur_qnty.'" ></td></tr>';	
			$z++;
		}
		$j++; 
	}
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)//Insert Here=================		
	{
		...
		$field_array="id, company_id, location_id, floor_id, line_id, prod_reso_allo, order_id, gmts_item_id, production_date, production_qnty, reject_qnty, alter_qnty, spot_qnty, hour, supervisor, production_type, entry_break_down_type, challan_no,manual_cut_no, remarks, is_deleted, status_active, inserted_by, insert_date";
		if($db_type==0)
		{
			$data_array="(".$id.",".$cbo_company_name.",".$cbo_location.",".$cbo_floor.",".$cbo_sewing_line.",".$prod_reso_allo.",".$hidden_po_break_down_id.",".$cbo_item_name.",".$txt_sewing_date.",".$txt_sewing_qty.",".$txt_reject_qnty.",".$txt_alter_qnty.",".$txt_spot_qnty.",".$txt_hours.",".$txt_super_visor.",7,".$sewing_production_variable.",".$txt_challan.",".$txt_manual_cut_no.",".$txt_remark.",0,1,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		}
		else
		{
		...
		{		
			for($i=1; $i<=$total_row; $i++)
			{
				...
				$colSize="colSize_".$i;
				if ($i!=1) $data_array1 .=",";
				$data_array1 .= "(".$dtls_id.",".$id.",7,".$cbo_item_name.",".$txt_sewing_date.",".$$txt_colo_size_mst_id.",".$$colSize.")";
	...
	else if ($operation==1)
	{
		...
		$field_array="location_id*floor_id*line_id*prod_reso_allo*production_qnty*production_date*reject_qnty*alter_qnty*spot_qnty*hour*supervisor*challan_no*manual_cut_no*remarks*is_deleted*status_active*updated_by*update_date";
		if($db_type==2)
		{
			$txt_hours=str_replace("'","",$txt_sewing_date)." ".str_replace("'","",$txt_hours);
			$txt_hours="to_date('".$txt_hours."','DD MONTH YYYY HH24:MI:SS')";
		}			
		$data_array="".$cbo_location."*".$cbo_floor."*".$cbo_sewing_line."*".$prod_reso_allo."*".$txt_sewing_qty."*".$txt_sewing_date."*".$txt_reject_qnty."*".$txt_alter_qnty."*".$txt_spot_qnty."*".$txt_hours."*".$txt_super_visor."*".$txt_challan."*".$txt_manual_cut_no."*".$txt_remark."*'0'*'1'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
	...
	else if ($operation==2)  // Delete Here---------------------------------------------------------- 
	{
		...
		$next_process_sql=sql_select("SELECT order_id from subcon_gmts_prod_dtls where status_active=1 and is_deleted=0 and production_type not in (1,7) and order_id=$hidden_po_break_down_id and gmts_item_id=$cbo_item_name "); 
...
$tot_next_process_sql=sql_select("SELECT order_id from subcon_gmts_prod_dtls where status_active=1 and is_deleted=0 and production_type in (7) and order_id=$hidden_po_break_down_id and gmts_item_id=$cbo_item_name ");
//echo "10**=SELECT order_id from subcon_gmts_prod_dtls where status_active=1 and is_deleted=0 and production_type in (7) and order_id=$hidden_po_break_down_id and gmts_item_id=$cbo_item_name ";die; 
...
if($action=="sewing_input_print2")
{
    ...
    <table width="900" cellspacing="0" align="right">
       <!--  <tr>
        ...
        <tr>
            <td colspan="4" align="center" style="font-size:18px"><strong>Sewing Input Challan</strong></td>
            <td colspan="2">&nbsp;</td>
