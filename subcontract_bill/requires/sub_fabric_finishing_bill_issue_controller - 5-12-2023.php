...
if ($action=="load_variable_settings")
{
	...
	// 		}
	$sql_result = sql_select("select variable_list, dyeing_fin_bill,allow_per from  variable_settings_subcon where company_id='$data' and variable_list in (1,3,8,11) order by id");
...
//echo $batch_no.'DSDD';
 ...
 //echo $data[12].'='.$inhouse_bill_from;die;
	...
	else if($data[2]==1)
	{
		...
		?>
            ...
            <div style="width:1090px;max-height:180px; overflow-y:scroll" id="sewing_production_list_view" align="center">
				...
				//echo $booking_no_cond;die;
				//if($ex_bill_for!=3)
			 	  //echo $inhouse_bill_from.'='.$inhouse_bill_from;
				if($ex_bill_for!=3) 
				{
					if($db_type==0)
					{
					...
					{		
						...
						if($data[2]==1){
						   ...
						   $sql="SELECT a.id, a.entry_form,c.recv_number,c.receive_basis,a.sys_number as sys_number, a.sys_number_prefix_num as recv_number_prefix_num, '' as challan_no, a.delevery_date as receive_date, d.booking_no_id as bookingno, $year_cond as year, b.batch_id, b.product_id as prod_id, b.bodypart_id as body_part_id, b.determination_id as fabric_description_id, b.color_id, b.width_type as dia_width_type,sum(b.current_delivery) as rec_qnty,sum(e.production_qty) as production_qty, sum((b.current_delivery+(f.process_loss_perc*b.current_delivery)/100)) as grey_used_qty, sum(e.roll_no) as carton_roll, f.po_breakdown_id as po_breakdown_id, d.booking_no_id, d.booking_no FROM pro_grey_prod_delivery_mst a, pro_grey_prod_delivery_dtls b, inv_receive_master c, pro_finish_fabric_rcv_dtls e, order_wise_pro_details f,pro_batch_create_mst d,wo_booking_mst w WHERE a.id=b.mst_id and b.grey_sys_id=c.id and d.id=b.batch_id and c.id=e.mst_id and e.id=f.dtls_id and e.id=b.sys_dtls_id and b.order_id=f.po_breakdown_id and d.booking_no_id=w.id and a.entry_form in (54,67) AND c.knitting_source=1 AND a.company_id=$data[3] $localtion_cond AND c.knitting_company=$data[0] and c.receive_basis in (0,2,4,5,9,11) and c.item_category=2 and c.entry_form in (7,66) and b.current_delivery>0  and f.entry_form in (7,66) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.batch_against in (0,1,2,3) and $booking_without_order=0 $date_cond $manual_challan_cond $sys_challan_cond $batchIds_cond $booking_no_cond group by a.id, a.entry_form,c.recv_number, c.receive_basis,a.sys_number, a.sys_number_prefix_num, a.delevery_date, c.booking_id, a.insert_date, b.batch_id, b.product_id, b.bodypart_id, b.determination_id, b.color_id, b.width_type, f.po_breakdown_id, d.booking_no_id, d.booking_no order by a.sys_number_prefix_num DESC";
					}
					else //Bill from receive
					{
						 $sql="SELECT a.id, a.entry_form, a.recv_number,a.receive_basis,a.recv_number as sys_number, a.recv_number_prefix_num, a.challan_no, a.receive_date, a.booking_id as bookingno, $year_cond as year, b.batch_id, b.prod_id, b.body_part_id,b.process_id, b.fabric_description_id, b.color_id, b.dia_width_type, sum(b.receive_qnty) as grey_receive_qnty,sum(c.quantity) as rec_qnty, sum(b.grey_used_qty) as grey_used_qty, sum(b.no_of_roll) as carton_roll, c.po_breakdown_id, d.booking_no_id, d.booking_no FROM inv_receive_master a, pro_finish_fabric_rcv_dtls b, order_wise_pro_details c, pro_batch_create_mst d,wo_booking_mst e WHERE a.id=b.mst_id and b.id=c.dtls_id and d.id=b.batch_id and a.booking_id=e.id and c.trans_type=1 and c.entry_form in (7,37,66,68) and c.trans_id!=0 and a.entry_form in (7,37,66,68) AND a.knitting_source=1 AND a.company_id=$data[3] AND a.location_id=$data[1] and c.quantity>0  AND a.knitting_company=$data[0] and a.receive_basis in (2,4,5,9,11) and a.item_category=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.batch_against in (0,1,3) and $booking_without_order=0 $date_cond $manual_challan_cond $sys_challan_cond $batchIds_cond $booking_no_cond group by a.id, a.entry_form,a.recv_number, a.receive_basis, a.recv_number,a.recv_number_prefix_num, a.challan_no, a.receive_date, a.booking_id, a.insert_date, b.batch_id, b.prod_id, b.body_part_id, b.fabric_description_id, b.color_id, b.dia_width_type, c.po_breakdown_id,b.process_id, d.booking_no_id, d.booking_no order by a.recv_number_prefix_num DESC";
					}
					//echo $sql; die;
				}
				else
				...
				foreach($sql_result as $row) // for update row
				{
					...
					if(in_array($all_value,$str_arr))
					{
						...
						else
						{
							...
							//$recv_Qty_prop=$row[csf('grey_used_qty')]*$row[csf('rec_qnty')]/$tot_recv_Qty;
							 //echo $row[csf('grey_used_qty')].'*'.$row[csf('grey_used_qty')].'/'.$tot_recv_Qty.'<br>';
			...
			//--------------------------------------------------------------------------------------------------
				...
				//	echo $inhouse_bill_from.'=A';
					...
					{ //grey_used_qty
						...
						$tot_recv_Qty=$recv_Qty_mrr_wiseArr[$row[csf('id')]][$row[csf('batch_id')]][$row[csf('body_part_id')]];
						//$recv_Qty_prop=$row[csf('grey_receive_qnty')]*$row[csf('rec_qnty')]/$row[csf('grey_used_qty')];
						//if($row[csf('color_id')]==89)
						//{
							
							//grey_receive_qnty
							$recv_Qty_prop_per=$row[csf('rec_qnty')]/$row[csf('grey_receive_qnty')]*100;
							$recv_Qty_prop=($recv_Qty_prop_per/100)*$row[csf('grey_used_qty')];
							//echo $recv_Qty_prop.'='.$row[csf('rec_qnty')].'='.$row[csf('grey_receive_qnty')].'<br>';
						//}
						
						
						//echo $row[csf('grey_receive_qnty')];//grey_receive_qnty
					
						if ($variable_check==1) $on_bill_qty=$recv_Qty_prop;
						else $on_bill_qty=$row[csf('rec_qnty')];
						//echo $row[csf('grey_used_qty')].'*'.$row[csf('rec_qnty')].'/'.$tot_recv_Qty.'<br>';
					}
					$color_range_id=$batch_array[$row[csf('batch_id')]]['color_range_id'];
					$BatchTotal 				= $batchTotal_arr[$row[csf('batch_id')]]['batch_ratio_total'];
					$chemicalsTotal 			= $batch_item_ratio_total_arr[$row[csf('batch_id')]][5]['ratio_total'];
				   ...
				   $conversion_date=change_date_format($receive_date, "d-M-y", "-",1);
					...
					$str_val=$row[csf('id')].'_'.change_date_format($row[csf('receive_date')]).'_'.$row[csf('recv_number_prefix_num')].'_'.$row[csf('po_breakdown_id')].'_'.$po_array[$row[csf('po_breakdown_id')]]['order'].'_'.$po_array[$row[csf('po_breakdown_id')]]['style'].'_'.$buyer_arr[$po_array[$row[csf('po_breakdown_id')]]['buyer']].'_'.$po_array[$row[csf('po_breakdown_id')]]['job'].'_'.$row[csf('carton_roll')].'_'.$row[csf('body_part_id')].'_'.$body_part[$row[csf("body_part_id")]].'_'.$row[csf('fabric_description_id')].'_'.$row[csf('prod_id')].'_'.$des_composition.'_'.$row[csf('batch_id')].'_'.$row[csf('color_id')].'_'.$color_name[$row[csf("color_id")]].'_'.$fin_processId_array[$row[csf('batch_id')]][$row[csf('body_part_id')]]['process_id'].'_'.$process_name.'_'.$row[csf('dia_width_type')].'_'.$fabric_typee[$row[csf('dia_width_type')]].'_'.number_format($on_bill_qty, 2, '.', '').'_'.$color_range_id.'_'.number_format($percentageOfColor,2, '.', '').'_'.number_format($unitCharge,2, '.', '').'_________'.'_'.number_format($bdt_rate,2, '.', '');
					//echo $str_val.'<br>';
					if($independent==4)
					{
						if($avilable_qty>0)
					...
					{
						//echo strtolower($ex_booking[1]).'='.strtolower($bill_for_id);die;
						//echo $re_batch_chk_array[$row[csf('batch_id')]].'DDSS';
