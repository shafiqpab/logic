...
if ($action=="load_drop_down_party_name")
{
	...
	if($data[1]==2)
	{
		//echo create_drop_down( "cbo_party_name", 150, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id  and b.tag_company='$data[0]' $buyer_cond  and buy.id in (select buyer_id from lib_buyer_party_type where party_type in (2,3)) order by buyer_name","id,buyer_name", 1, "-- Select Party --", $selected, "show_list_view(document.getElementById('cbo_party_source').value+'_'+this.value,'sewing_delivery_list_view','sewing_info_list','requires/sewing_bill_issue_controller','setFilterGrid(\'list_view_issue\',-1)');","","","","","",5 ); 
                    ...
                    <tbody>
                        <tr>
                            <td> 
                                ...
                                <?   
									echo create_drop_down( "cbo_company_id", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", $ex_data[0],"load_drop_down( 'sewing_bill_issue_controller', this.value, 'load_drop_down_party_name_popup', 'party_td_pop' );",0 );
                                ?>
                            </td>
                            <td id="party_td_pop">
								<?
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_party_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_search_common').value, 'sewing_list_view', 'search_div', 'sewing_bill_issue_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" />
                            </td>
                        </tr>
                        <tr>
...
if ($action=="sewing_list_view")
{
	...
	else if($db_type==2)
	{
	 ...
	 $sql= "select id, bill_no, prefix_no_num, $year_cond, location_id, bill_date, party_id, party_source, bill_for from subcon_inbound_bill_mst where status_active=1 and process_id=5 $company_name $party_name $return_date $bill_id_cond order by id DESC";
	
	echo  create_list_view("list_view", "Bill No,Year,Location,Bill Date,Party,Source,Bill For", "50,40,80,70,100,120,140","630","250",0, $sql , "js_set_value", "id", "", 1, "0,0,location_id,0,party_id,party_source,bill_for", $arr , "prefix_no_num,year,location_id,bill_date,party_id,party_source,bill_for", "sewing_bill_issue_controller","",'0,0,0,3,0,0,0') ;
	exit();
}
...
if ($action=="load_php_data_to_form_issue")
{
	...
	{	
		...
		echo "load_drop_down( 'requires/sewing_bill_issue_controller', document.getElementById('cbo_company_id').value, 'load_drop_down_location', 'location_td' );\n";
		...
		echo "load_drop_down( 'requires/sewing_bill_issue_controller', document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_party_source').value, 'load_drop_down_party_name', 'party_td' );\n";
...
if ($action=="sewing_delivery_list_view")
{
	...
	if($data[0]==2)
	{
        ...
        <div style="width:820px;max-height:180px; overflow-y:scroll" id="sewing_production_list_view">
			...
			//echo $data[0]."==".$data[2]; die;
				...
				if(!$data[2])
				{
					 $sql="select b.id, a.challan_no, a.delivery_date, b.process_id, b.item_id, b.delivery_qty, b.order_id from  subcon_delivery_mst a, subcon_delivery_dtls b where b.bill_status=0 and a.id=b.mst_id  and a.party_id='$data[1]' and b.process_id='5' and a.status_active=1 and a.is_deleted=0 $date_cond"; 
				}
				else
				{
					
				 $sql="(select b.id, a.challan_no, a.delivery_date, b.process_id, b.item_id, b.delivery_qty, b.order_id from subcon_delivery_mst a, subcon_delivery_dtls b where a.id=b.mst_id  and a.party_id='$data[1]' and b.process_id='5' and a.status_active=1 and b.bill_status='0' $date_cond)
					 union 
					 	(select b.id, a.challan_no, a.delivery_date, b.process_id, b.item_id, b.delivery_qty, b.order_id from subcon_delivery_mst a, subcon_delivery_dtls b where a.id=b.mst_id  and a.party_id='$data[1]' and b.process_id='5' and b.id in ( $data[3] ) and a.status_active=1 and b.bill_status=1 $date_cond)";
				}
	...
	else if($data[0]==1)
	{
		...
		<body>
			<div style="width:100%;">
				<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1010px" class="rpt_table">
					<thead>
						<th width="30">&nbsp;</th>
						<th width="30">SL</th>
        ...
        <div style="width:1010px;max-height:180px; overflow-y:scroll" id="sewing_production_list_view">
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="992px" class="rpt_table" id="list_view_issue">
            <?
				$buyer_arr=return_library_array("select id, buyer_name from lib_buyer",'id','buyer_name');
				$country_arr=return_library_array("select id, country_name from lib_country",'id','country_name');
			 	//echo $data[3];
			 	$job_order_arr=array();
				$sql_job="Select a.job_no_prefix_num, a.buyer_name, a.style_ref_no, b.id, b.po_number,b.grouping from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
				...
				if(!$data[2])
				{
					$sql="select id, challan_no, production_date, po_break_down_id, item_number_id, serving_company, country_id, production_quantity from pro_garments_production_mst where serving_company=$data[7] and company_id=$data[1] $date_cond and production_source=1	 and produced_by in (0,1) and production_type=5  and status_active=1 and is_deleted=0 order by id DESC"; 
				}
				else
				{
				...
				if(count($data_all_ex)>0)
				{
					...
					?>
						<tr id="tr_<?  echo $ex_val[0]; ?>" bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $ex_val[0]."_".'1'; ?>');" > 
							...
							<td width="100"><p><? echo $job_order_arr[$ex_val[1]]['grouping']; ?></p></td>
                ...
                foreach($sql_result as $row)
				{
					...
					?>
						...
						<td width="30" align="center"><input type="checkbox" name="checkid<? echo $i; ?>" id="checkid<? echo $i; ?>" onClick="fnc_check(<? echo $i; ?>)" value="2" ></td>
							...
							<td width="100"><p><? echo $job_order_arr[$row[csf('po_break_down_id')]]['grouping'];; ?></p></td>
...
if ($action=="load_php_dtls_form")  //new issue 
{
	...
	if($data[3]==2)
	{
		...
		else
		{
			...
			if($bill_id!="" && $del_id!="")
				$sql="(select id as upd_id, delivery_id, delivery_date, challan_no, item_id, packing_qnty as carton_roll, delivery_qty, rate, amount, remarks, order_id, currency_id from subcon_inbound_bill_dtls where delivery_id in ($bill_id) and status_active=1 and is_deleted=0 and process_id=5)
				 union
				 (select 0, CAST (b.id AS VARCHAR(100))  as delivery_id, a.delivery_date, a.challan_no,  CAST (b.item_id AS VARCHAR(100))  as item_id, b.carton_roll, b.delivery_qty, 0, 0, null, b.order_id,0 from  subcon_delivery_mst a, subcon_delivery_dtls b where a.id=b.mst_id and b.id in ($del_id) and a.status_active=1 and b.process_id=5)";
			else if($bill_id!="" && $del_id=="")
				$sql="select id as upd_id, delivery_id, delivery_date, challan_no, item_id, packing_qnty as carton_roll, delivery_qty, rate, amount, remarks, order_id, currency_id from subcon_inbound_bill_dtls where delivery_id in ($bill_id) and status_active=1 and is_deleted=0 and process_id=5";
			else  if($bill_id=="" && $del_id!="")
				$sql="select 0, b.id as delivery_id, a.delivery_date, a.challan_no, b.item_id, b.carton_roll, b.delivery_qty, 0, 0, 0, b.order_id from  subcon_delivery_mst a, subcon_delivery_dtls b where a.id=b.mst_id and b.id in ($del_id) and a.status_active=1 and a.is_deleted=0 and b.process_id=5"; 
		}
		//echo $sql; 
		...
		foreach ($sql_result as $row)
		{
			...
			<tr align="center" id="dtls_form_delete">				
				<td>
					...
					<input type="text" name="txt_deleverydate_<? echo $k; ?>" id="txt_deleverydate_<? echo $k; ?>"  class="datepicker" style="width:60px" value="<? echo change_date_format($row[csf("delivery_date")]); ?>" readonly />									
				</td>
				<td>
				...
				<td>
					<input type="text" name="txt_rate_<? echo $k; ?>" id="txt_rate_<? echo $k; ?>"  class="text_boxes_numeric" style="width:40px" value="<? if($row[csf("rate")]) echo $row[csf("rate")];else echo $order_array[$row[csf("order_id")]]['rate']; ?>" onBlur="qnty_caluculation(<? echo $k; ?>);" />
				</td>
				<td>
					<?
						$total_amount=$row[csf("delivery_qty")]*$order_array[$row[csf("order_id")]]['rate'];
					?>
					<input type="text" name="txt_amount_<? echo $k; ?>" id="txt_amount_<? echo $k; ?>" style="width:60px"  class="text_boxes_numeric"  value="<? if($row[csf("amount")]) echo  $row[csf("amount")];else echo $total_amount; ?>" readonly />
				</td>
                <td>
				<? 
	...
	else if ($data[3]==1)
	{
		...
		foreach ($sql_result as $row)
		{
			...
			<tr align="center" id="dtls_form_delete">				
				...
				<td>
				<input type="text" name="txt_rate_<? echo $k; ?>" id="txt_rate_<? echo $k; ?>"  class="text_boxes_numeric" style="width:40px" value="<? echo $row[csf("rate")]; ?>" onBlur="qnty_caluculation(<? echo $k; ?>);copy_rate_currency(<? echo $k; ?>);"  />
				</td>
				<td>
					<?
                ...
                <? 
                    //$order_array[$row[csf("order_id")]]['currency_id']
                    echo create_drop_down( "cbo_curanci_$k", 60, $currency,"", 1, "-Select Currency-",$row[csf("currency_id")],"copy_rate_currency($k);",0,"" );
                ?>
                </td>
				<td>
					<input type="text" name="txt_remarks_<? echo $k; ?>" id="txt_remarks_<? echo $k; ?>"  class="text_boxes" style="width:80px" value="<? echo $row[csf("remarks")]; ?>" />
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   // Insert Here========================================================================================
	{
		...
		$new_bill_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '', 'SEW', date("Y",time()), 5, "select prefix_no,prefix_no_num from  subcon_inbound_bill_mst where company_id=$cbo_company_id and process_id=$process_id  $year_cond=".date('Y',time())." order by id desc ", "prefix_no", "prefix_no_num" ));
	...
	else if ($operation==1)   // Update Here=============================================================================
	{
		...
		//echo "10**".bulk_update_sql_statement( "subcon_inbound_bill_dtls", "id",$field_array_up,$data_array_up,$id_arr );die;
		$rID1=execute_query(bulk_update_sql_statement("subcon_inbound_bill_dtls", "id",$field_array_up,$data_array_up,$id_arr ));
		if($rID1) $flag=1; else $flag=0;
            ...
            <tr>
             ...
             <?
			 	if($dataArray[0][csf('party_source')]==2)
				{
					...
					$party_dtls=$party_library[$dataArray[0][csf('party_id')]].' : Address :- '.$address;
				}
				else if($dataArray[0][csf('party_source')]==1)
				{
					$party_dtls=$company_library[$dataArray[0][csf('party_id')]];
				}
			 ?>
                <td><strong>Party Name : </strong></td><td colspan="5"> <? echo $party_dtls; ?></td>
            </tr>
        </table>
         <br>
	<div style="width:100%;">
		<table align="right" cellspacing="0" width="930"  border="1" rules="all" class="rpt_table" >
		 ...
		 <?
			...
			$sql_result =sql_select("select id, delivery_id, delivery_date, challan_no, order_id, item_id, packing_qnty, delivery_qty, rate, amount, remarks, currency_id, process_id, currency_id from subcon_inbound_bill_dtls  where mst_id='$mst_id' and process_id='5' and status_active=1 and is_deleted=0"); 
			foreach($sql_result as $row)
			{
				if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
			?>
				<tr bgcolor="<? echo $bgcolor; ?>"> 
                    ...
                    <td><div style="word-wrap:break-word; width:70px"><? echo $order_array[$row[csf('order_id')]]['order_no']; ?></div></td>
                    <td><div style="word-wrap:break-word; width:70px"><? echo $order_array[$row[csf('order_id')]]['cust_buyer']; ?></div></td>
                    <td><div style="word-wrap:break-word; width:70px"><? echo $order_array[$row[csf('order_id')]]['cust_style_ref']; ?></div></td>
                    ...
                    <td align=""><p><? echo $currency[$row[csf('currency_id')]]; ?>&nbsp;</p></td>
        ...
        <br>
		 <?
            echo signature_table(182, $data[0], "930px");
         ?>
   </div>
   </div>
<?
