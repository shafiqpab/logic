...
if ($action=="knitting_delivery_list_view")
{
	...
	{
		if($ex_bill_for==3) $tbl_wight="820"; else $tbl_wight="1080";
		?>
		</head> 
		<body>
        ...
        <div style="width:<? echo $tbl_wight; ?>px;max-height:180px; overflow-y:scroll" id="sewing_production_list_view">
            ...
            <?
				...
				$color_type_sql="select a.color_type_id, a.lib_yarn_count_deter_id, b.po_break_down_id from  wo_pre_cost_fabric_cost_dtls a, wo_pre_cos_fab_co_avg_con_dtls b where a.id=b.pre_cost_fabric_cost_dtls_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.company_id='$data[3]'";
				 ...
				 if($in_house_knit_bill_from!=2)   //delevery_date
				 {
					...
					else // ***=====Subcon Variable-In House Knit Bill From===***
					{
						...
						if($job_id)
						{
						    ...
						    $sql=" select a.id, a.entry_form, a.sys_number_prefix_num as recv_number_prefix_num, b.grey_sys_number as challan_no, a.delevery_date as receive_date, b.product_id as prod_id, 0 as body_part_id, b.determination_id as febric_description_id, sum(b.roll) as roll_qty, b.order_id as po_breakdown_id, sum(b.current_delivery) as quantity,sum(b.current_delivery_qnty_in_pcs) as quantity_pcs, 0 as rate, 0 as amount, c.receive_basis, c.booking_no, c.booking_id,b.size_coller_cuff from pro_grey_prod_delivery_mst a, pro_grey_prod_delivery_dtls b,inv_receive_master c
						where a.id=b.mst_id and b.grey_sys_id=c.id and c.knitting_source=1 and c.company_id='$data[3]' and c.knitting_company='$data[0]' and c.entry_form=2  and c.knitting_source=1 and a.company_id='$data[3]' and c.knitting_company='$data[0]' and c.location_id='$data[1]'   and a.entry_form in(56,53) and c.entry_form=2 and c.receive_basis in (2,4,11) 
						...
						group by a.id, a.entry_form, a.sys_number_prefix_num, c.receive_basis,b.grey_sys_number, a.delevery_date, b.product_id, b.determination_id, b.order_id, c.booking_no, c.booking_id,b.size_coller_cuff
						order by a.sys_number_prefix_num ASC";	


						$sqlRecvData=sql_select("select  b.prod_id, b.body_part_id, b.febric_description_id, c.po_breakdown_id, a.booking_no, a.booking_id,b.coller_cuff_size from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c
						where a.id=b.mst_id and $booking_without_order=0 and b.id=c.dtls_id and a.knitting_source=1 and a.company_id='$data[3]' and a.knitting_company='$data[0]' and a.location_id='$data[1]' and c.trans_type=1 and a.entry_form=2 and c.entry_form=2 and a.receive_basis in (1,2) and a.item_category=13 
						and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  $man_challan_cond $po_breakdown_id_conds2 $sys_challan_cond
						group by b.prod_id, b.body_part_id, b.febric_description_id, c.po_breakdown_id, a.booking_no, a.booking_id,b.coller_cuff_size");
						$recvInfo_arr=array();
						...
						/*$sql="(
						 select a.id, a.entry_form, a.recv_number_prefix_num, a.challan_no, a.receive_date, b.prod_id, b.body_part_id, b.febric_description_id, sum(b.no_of_roll) as roll_qty, c.po_breakdown_id, sum(c.quantity) as quantity,sum(b.quantity_pcs) as quantity_pcs, avg(b.rate) as rate, sum(b.amount) as amount, a.receive_basis, a.booking_no, a.booking_id from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c
						...
						and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $date_cond $man_challan_cond $po_breakdown_id_conds $sys_challan_cond
						group by a.id, a.entry_form, a.recv_number_prefix_num, a.challan_no, a.receive_date, b.prod_id, b.body_part_id, b.febric_description_id, c.po_breakdown_id, a.receive_basis, a.booking_no, a.booking_id order by a.recv_number_prefix_num ASC)";*/
					   //echo $sql;
					...
					foreach($sql_result as $row) // for update row
					{
						...
						if(in_array($all_value,$str_arr))
						{
							...
							//	echo $row[csf('entry_form')].'SSSASA';
					...
					foreach($sql_result as $row) // for new row
					{
						...
						$all_value=$row[csf('recv_number_prefix_num')].'_'.$row[csf('po_breakdown_id')].'_'.$row[csf('prod_id')].'_'.$row[csf('body_part_id')].'_'.$row[csf('febric_description_id')].'_'.$row[csf('challan_no')].'_'.$row[csf('size_coller_cuff')];
						$all_value=$row[csf('recv_number_prefix_num')].'_'.$row[csf('po_breakdown_id')].'_'.$row[csf('prod_id')].'_'.$row[csf('body_part_id')].'_'.$row[csf('febric_description_id')].'_'.$row[csf('challan_no')].'_'.$row[csf('size_coller_cuff')].'_'.$row[csf('booking_no')];
						 // echo $all_value.'<br>';
						if($row[csf('roll_qty')]=='') $row[csf('roll_qty')]=0;
						if($row[csf('body_part_id')]=='') $row[csf('body_part_id')]=0;
						if($body_part[$row[csf("body_part_id")]]=='') $body_part[$row[csf("body_part_id")]]=0;
				...
				else if($ex_bill_for==3)// sample without order
				{
				 ...
				 else
				 {
						...
						if($job_id)
						{
						  ...
						  $sql=" select a.id, a.entry_form, a.sys_number_prefix_num as recv_number_prefix_num, b.grey_sys_number as challan_no, a.delevery_date as receive_date, b.product_id as prod_id, 0 as body_part_id, b.determination_id as febric_description_id, sum(b.roll) as roll_qty, b.order_id as po_breakdown_id, sum(b.current_delivery) as quantity,sum(b.current_delivery) as quantity_pcs, 0 as rate, 0 as amount, c.receive_basis, c.buyer_id,c.booking_no, c.booking_id from pro_grey_prod_delivery_mst a, pro_grey_prod_delivery_dtls b,inv_receive_master c
						...
						and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $date_cond2 $man_challan_cond $po_breakdown_id_conds $sys_challan_cond
						group by a.id, a.entry_form, a.sys_number_prefix_num, c.receive_basis,b.grey_sys_number, a.delevery_date, b.product_id, b.determination_id, b.order_id, c.booking_no, c.booking_id,c.buyer_id
						order by a.sys_number_prefix_num ASC";	
				 }
					...
					foreach($sql_result as $row) // for update row
					{
						...
						if(in_array($all_value,$str_arr))
						{
							...
							$bill_qty=$bill_qty_array[$row[csf('recv_number_prefix_num')]][$row[csf('challan_no')]][$row[csf('po_breakdown_id')]][$row[csf('prod_id')]][$row[csf('body_part_id')]][$row[csf('febric_description_id')]]['qty'];
					...
					foreach($sql_result as $row) // for new row
					{
						...
						$all_value=$row[csf('recv_number_prefix_num')].'_'.$row[csf('order_id')].'_'.$row[csf('prod_id')].'_'.$row[csf('body_part_id')].'_'.$row[csf('febric_description_id')].'_'.$row[csf('challan_no')];
						$str_val=$row[csf('id')].'_'.change_date_format($row[csf('receive_date')]).'_'.$row[csf('recv_number_prefix_num')].'_'.$row[csf('order_id')].'_'.$job_order_arr[$row[csf('order_id')]]['po'].'_'.$job_order_arr[$row[csf('order_id')]]['style'].'_'.$buyer_arr[$row[csf('buyer_id')]].'_'.$row[csf('roll_qty')].'_'.$row[csf('febric_description_id')].'_'.$composition_arr[$determ_arr[$row[csf('febric_description_id')]]].'_'.$row[csf('body_part_id')].'_'.$body_part[$row[csf("body_part_id")]].'_'.$row[csf('prod_id')].'_'.$product_dtls_arr[$row[csf('prod_id')]].'_'.$row[csf('quantity')].'_'.$row[csf('rate')].'_'.$row[csf('amount')].'_0____1_'.$bodyPartTypeArr[$row[csf('body_part_id')]].'_'.$row[csf('quantity_pcs')].'_'.$row[csf('challan_no')];
						if($independent==4)
						{
							if($avilable_qty>0)
							{
...
if ($action=="load_dtls_data") 
{
	...
	$sql="select id as upd_id, delivery_id,rec_challan_no, delivery_date, challan_no, item_id, body_part_id, febric_description_id, uom, packing_qnty as carton_roll, delivery_qty, delivery_qtypcs, lib_rate_id, rate, amount, remarks, order_id, coller_cuff_measurement,currency_id from subcon_inbound_bill_dtls where mst_id=$upid and process_id=2 and status_active=1 and is_deleted=0 and process_id=2 order by upd_id ASC";
...
if ($action=="save_update_delete")
{
	...
	if($cbo_bill_sectionId==1)
	{
		//$process_id=1;	
	}
	//echo "10**=".$control_with.'='.$cbo_bill_sectionId.'='.$party_sourceId;die;
	if($control_with==1 && $cbo_bill_sectionId==1)
	{
		if($bill_forId!=3)
		{
			if($party_sourceId==1)
	...
	if ($operation==0)   // Insert Here========================================================================================delivery_id
	{
		...
		$field_array1 ="id, mst_id, delivery_id,rec_challan_no, delivery_date, challan_no, order_id, item_id, febric_description_id, body_part_id, uom, packing_qnty, delivery_qty, delivery_qtypcs, lib_rate_id, rate, amount, remarks, currency_id, process_id, inserted_by, insert_date, coller_cuff_measurement";
		$field_array_up ="delivery_id*rec_challan_no*delivery_date*challan_no*order_id*item_id* febric_description_id*body_part_id*uom*packing_qnty*delivery_qty*delivery_qtypcs*lib_rate_id*rate*amount*remarks*currency_id*updated_by*update_date*coller_cuff_measurement";
		...
		for($i=1; $i<=$tot_row; $i++)
		{
			...
			if(str_replace("'",'',$$updateid_dtls)=="")  
			{
				if($$amount!="")
				{
					...
					$data_array1 .="(".$id1.",".$id.",".$$delivery_id.",".$$recchallanno.",'".$delevery_date."',".$$challen_no.",".$$orderid.",".$$item_id.",".$$compoid.",".$$bodypartid.",".$$cbouom.",".$$number_roll.",".$$quantity.",".$$deliveryqntypcs.",".$$libRateId.",".$$rate.",".$$amount.",".$$remarks.",".$$curanci.",'".$process_id."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$$collarcuff.")";
					$id1=$id1+1;
					$add_comma++;
				}
			...
			else
			{
				...
				$data_array_up[str_replace("'",'',$$updateid_dtls)] =explode("*",("".$$delivery_id."*".$$recchallanno."*'".$delevery_date."'*".$$challen_no."*".$$orderid."*".$$item_id."*".$$compoid."*".$$bodypartid."*".$$cbouom."*".$$number_roll."*".$$quantity."*".$$deliveryqntypcs."*".$$libRateId."*".$$rate."*".$$amount."*".$$remarks."*".$$curanci."*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'*".$$collarcuff.""));
				$id_arr_delivery[]=str_replace("'",'',$$delivery_id);
				$data_array_delivery[str_replace("'",'',$$delivery_id)] =explode("*",("1"));
			}
			//order table insert====================================================================================================
