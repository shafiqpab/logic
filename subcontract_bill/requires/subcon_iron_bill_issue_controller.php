...
if ($action=="load_drop_down_party_name")
{
	...
	if($data[1]==2)
	{
		/*echo create_drop_down( "cbo_party_name", 150, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id  and b.tag_company='$data[0]' $buyer_cond  and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (2,3)) order by buyer_name","id,buyer_name", 1, "-- Select Party --", $selected, "show_list_view(document.getElementById('cbo_party_source').value+'_'+this.value,'iron_delivery_list_view','iron_info_list','requires/subcon_iron_bill_issue_controller','');","","","","","",5 ); 
		*/
                    ...
                    <tbody>
                        <tr>
                            <td> 
                                ...
                                <?   
									echo create_drop_down( "cbo_company_id", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", $ex_data[0],"load_drop_down( 'subcon_iron_bill_issue_controller', this.value, 'load_drop_down_party_name_popup', 'party_td_pop' );",0 );
                                ?>
                            </td>
                            <td id="party_td_pop">
								<?
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_party_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_search_common').value, 'iron_list_view', 'search_div', 'subcon_iron_bill_issue_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" />
                            </td>
                        </tr>
                        <tr>
...
if ($action=="iron_list_view")
{
	...
	//if ($data[1]!=0) $party_name=" and party_id='$data[1]'"; else { echo "Please Select Party First."; die; }
	...
	$sql= "select id, bill_no, prefix_no_num, $year_cond, location_id, bill_date, party_id, party_source, bill_for from subcon_inbound_bill_mst where status_active=1 and process_id=10 $company_name $party_name $return_date $bill_id_cond order by id DESC";
	
	echo  create_list_view("list_view", "Bill No,Year,Location,Bill Date,Party,Source,Bill For", "50,40,80,70,100,120,140","630","250",0, $sql , "js_set_value", "id", "", 1, "0,0,location_id,0,party_id,party_source,bill_for", $arr , "prefix_no_num,year,location_id,bill_date,party_id,party_source,bill_for", "subcon_iron_bill_issue_controller","",'0,0,0,3,0,0,0') ;
	exit();
}
...
if ($action=="load_php_data_to_form_issue")
{
	...
	{	
		...
		echo "load_drop_down( 'requires/subcon_iron_bill_issue_controller', document.getElementById('cbo_company_id').value, 'load_drop_down_location', 'location_td' );\n";
		...
		echo "load_drop_down( 'requires/subcon_iron_bill_issue_controller', document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_party_source').value, 'load_drop_down_party_name', 'party_td' );\n";
...
if ($action=="iron_delivery_list_view")
{
	...
	if($data[0]==2)
	{
        ...
        <div style="width:820px;max-height:180px; overflow-y:scroll" id="iron_production_list_view">
            ...
            <?
				...
				if(!$data[2])
				{
					$sql="select b.id, a.challan_no, a.delivery_date, b.process_id, b.item_id, b.delivery_qty, b.order_id from  subcon_delivery_mst a, subcon_delivery_dtls b where b.bill_status=0 and a.id=b.mst_id and a.company_id='$company_id' and a.party_id='$data[1]' and b.process_id='10' and a.status_active=1 and a.is_deleted=0 $date_cond"; 
				}
				else
				{
					$sql="(select b.id, a.challan_no, a.delivery_date, b.process_id, b.item_id, b.delivery_qty, b.order_id from subcon_delivery_mst a, subcon_delivery_dtls b where a.id=b.mst_id and a.party_id='$data[1]' and b.process_id='10' and a.status_active=1 and b.bill_status='0' $date_cond)
					 union 
					 	(select b.id, a.challan_no, a.delivery_date, b.process_id, b.item_id, b.delivery_qty, b.order_id from subcon_delivery_mst a, subcon_delivery_dtls b where a.id=b.mst_id and a.company_id='$company_id' and a.party_id='$data[1]' and b.process_id='10' and b.id in ( $data[3] ) and a.status_active=1 $date_cond)";
				}
	...
	else if($data[0]==1)
	{
        ...
        <div style="width:910px;max-height:180px; overflow-y:scroll" id="sewing_production_list_view">
            ...
            <?
				...
				{
					$sql_data="select id, challan_no, production_date, po_break_down_id, item_number_id, serving_company, country_id, production_quantity from pro_garments_production_mst where id in ($data[3]) and serving_company=$data[1] and company_id='$company_id' $date_cond and production_source=1 and produced_by=1 and production_type=5 and status_active=1 and is_deleted=0 order by id DESC";
...
if ($action=="load_php_dtls_form")  //new issue 
{
	...
	if($data[3]==2)
	{
		...
		if( $data[2]!="" )
		{
			$sql="select id as upd_id, delivery_id, delivery_date, challan_no, item_id, packing_qnty as carton_roll, delivery_qty, rate, amount, remarks, order_id, currency_id from subcon_inbound_bill_dtls where mst_id in ($data[2]) and status_active=1 and process_id=10"; 
		}
		else
		{
			if($bill_id!="" && $del_id!="")
				$sql="(select id as upd_id, delivery_id, delivery_date, challan_no, item_id, packing_qnty as carton_roll, delivery_qty, rate, amount, remarks, order_id, currency_id from subcon_inbound_bill_dtls where delivery_id in ($bill_id) and status_active=1 and is_deleted=0 and process_id=10)
				 union
				 (select 0, b.id as delivery_id, a.delivery_date, a.challan_no, b.item_id, b.carton_roll, b.delivery_qty, 0, 0, null, b.order_id from  subcon_delivery_mst a, subcon_delivery_dtls b where a.id=b.mst_id and b.id in ($del_id) and a.status_active=1 and b.process_id=10)";
			else if($bill_id!="" && $del_id=="")
				$sql="select id as upd_id, delivery_id, delivery_date, challan_no, item_id, packing_qnty as carton_roll, delivery_qty, rate, amount, remarks, order_id, currency_id from subcon_inbound_bill_dtls where delivery_id in ($bill_id) and status_active=1 and is_deleted=0 and process_id=10";
			else  if($bill_id=="" && $del_id!="")
				$sql="select 0, b.id as delivery_id, a.delivery_date, a.challan_no, b.item_id, b.carton_roll, b.delivery_qty, 0, 0, 0, b.order_id from  subcon_delivery_mst a, subcon_delivery_dtls b where a.id=b.mst_id and b.id in ($del_id) and a.status_active=1 and a.is_deleted=0 and b.process_id=10"; 
		}
		$sql_result =sql_select($sql);	
		$k=0;
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   // Insert Here========================================================================================
	{
		...
		$new_bill_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '', 'IBI', date("Y",time()), 5, "select prefix_no,prefix_no_num from  subcon_inbound_bill_mst where company_id=$cbo_company_id and process_id=$process_id  $year_cond=".date('Y',time())." order by id desc ", "prefix_no", "prefix_no_num" ));
		...
		if(str_replace("'",'',$update_id)=="")
		{
			...
			//There is no  input field $cbo_bill_for//
			$field_array="id, prefix_no, prefix_no_num, bill_no, company_id, location_id, bill_date, party_id, party_source, process_id, inserted_by, insert_date";
			$data_array="(".$id.",'".$new_bill_no[1]."','".$new_bill_no[2]."','".$new_bill_no[0]."',".$cbo_company_id.",".$cbo_location_name.",".$txt_bill_date.",".$cbo_party_name.",".$cbo_party_source.",".$process_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		//echo "10**".$rID."**".$rID1."**".$rID2."**".$rID4;die;
	...
	else if ($operation==1)   // Update Here=============================================================================
	{
		...
		$id=str_replace("'",'',$update_id);
		$posted_account = return_field_value("is_posted_account","subcon_inbound_bill_mst","id='$id'");
		...
		$field_array="bill_no*company_id*location_id*bill_date*party_id*party_source*updated_by*update_date";
		$data_array="".$txt_bill_no."*".$cbo_company_id."*".$cbo_location_name."*".$txt_bill_date."*".$cbo_party_name."*".$cbo_party_source."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"; 
	...
	<div style="width:100%;">
		<table align="right" cellspacing="0" width="930"  border="1" rules="all" class="rpt_table" >
		 ...
		 <?
			...
			$sql_result =sql_select("select id, delivery_id, delivery_date, challan_no, order_id, item_id, packing_qnty, delivery_qty, rate, amount, remarks, currency_id, process_id from subcon_inbound_bill_dtls  where mst_id='$mst_id' and process_id='5' and status_active=1 and is_deleted=0"); 
			foreach($sql_result as $row)
			{
				if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
			?>
        ...
        <table width="930" align="left" > 
            <tr>
                ...
                <td colspan=13 align=left>&bull; Receiver should be aware of the quantity &amp; specification of the Product(s) at the time of taking delivery.</td>
            </tr>
            <tr>
                <td colspan=13 align=left>&bull; No claim will be entertained after delivery of goods.</td>

            </tr>
            <tr>
                <td colspan=13 align=left>&bull; Delivery Challan have been attached.</td>
            </tr>
            <tr>
                <td colspan=13 align=left>&bull; Payment should be made within seven days from the bill date.</td>
            </tr> 
        </table>
        <br>
		 <?
