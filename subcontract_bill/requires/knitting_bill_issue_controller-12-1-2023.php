...
if ($action=="load_php_data_to_form_issue")
{
	...
	$nameArray= sql_select("select id, bill_no, company_id, location_id, party_location_id, bill_date, party_id, party_source, attention, bill_for, bill_section, is_posted_account, post_integration_unlock from subcon_inbound_bill_mst where id='$data'");
				 ...
				 if($in_house_knit_bill_from!=2)   //delevery_date
				 {
					...
					else // ***=====Subcon Variable-In House Knit Bill From===***
					{
						...
						if($job_id)
						{
						   ...
						   $sql=" select a.id, a.entry_form, a.sys_number_prefix_num as recv_number_prefix_num, b.grey_sys_number as challan_no, a.delevery_date as receive_date, b.product_id as prod_id, 0 as body_part_id, b.determination_id as febric_description_id, sum(b.roll) as roll_qty, b.order_id as po_breakdown_id, sum(b.current_delivery) as quantity,sum(b.current_delivery) as quantity_pcs, 0 as rate, 0 as amount, c.receive_basis, c.booking_no, c.booking_id from pro_grey_prod_delivery_mst a, pro_grey_prod_delivery_dtls b,inv_receive_master c
						...
						group by a.id, a.entry_form, a.sys_number_prefix_num, c.receive_basis,b.grey_sys_number, a.delevery_date, b.product_id, b.determination_id, b.order_id, c.booking_no, c.booking_id
					...
					$sql_result=sql_select($sql);
					//echo $independen.'DDS';die;
					foreach($sql_result as $row) // for update row
					{
						 if($row[csf('challan_no')]=='') $row[csf('challan_no')]=0;
				      // if($row[csf('rec_challan_no')]=='') $row[csf('rec_challan_no')]=0;
						$all_value=$row[csf('recv_number_prefix_num')].'_'.$row[csf('po_breakdown_id')].'_'.$row[csf('prod_id')].'_'.$row[csf('body_part_id')].'_'.$row[csf('febric_description_id')].'_'.$row[csf('challan_no')];
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   // Insert Here========================================================================================delivery_id
	{
		...
		if(str_replace("'",'',$update_id)=="")
		{
			...
			$field_array="id, prefix_no, prefix_no_num, bill_no, company_id, location_id,party_location_id, bill_date, party_id, party_source, attention, bill_for, process_id, bill_section, inserted_by, insert_date";
			$data_array="(".$id.",'".$new_bill_no[1]."','".$new_bill_no[2]."','".$new_bill_no[0]."',".$cbo_company_id.",".$cbo_location_name.",".$cbo_party_location.",".$txt_bill_date.",".$cbo_party_name.",".$cbo_party_source.",".$txt_attention.",".$cbo_bill_for.",'".$bill_process_id."',".$cbo_bill_section.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')"; 
		...
		else
		{
			...
			$data_array="".$cbo_location_name."*".$txt_bill_date."*".$cbo_party_name."*".$cbo_party_source."*".$txt_attention."*".$cbo_bill_for."*".$cbo_bill_section."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"; 
	...
	else if ($operation==1)   // Update Here=============================================================================
	{
		...
		$field_array="bill_date*bill_section*attention*updated_by*update_date";
		$data_array="".$txt_bill_date."*".$cbo_bill_section."*".$txt_attention."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
	...
	else if ($operation==2)  //Delete here======================================================================================
	{
		...
		for($i=1; $i<=$tot_row; $i++)
		{
			...
			{
				$id_delivery[]=$delivery_id[$i];
				$data_delivery[str_replace("'",'',$delivery_id[$i])] =explode(",",("0"));
			}
		}
		$flag=1;
