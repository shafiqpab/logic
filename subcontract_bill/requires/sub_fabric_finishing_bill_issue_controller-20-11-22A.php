...
if ($action=="bill_no_popup")
{
	...
	<script>
		...
		{
			if(val==1) $('#txt_search_challan').removeAttr('disabled','disabled');
			else $('#txt_search_challan').attr('disabled','disabled');
		}
	</script>
	</head>
	<body>
...
if ($action=="load_php_data_to_form_issue")
{
	...
	$nameArray= sql_select("select id, bill_no, company_id,upcharge,discount, location_id, bill_date, party_id, party_source, party_location_id, inhouse_bill_from, bill_for, is_posted_account,post_integration_unlock from subcon_inbound_bill_mst where id='$data'");
	...
	if($data[2]==2)
	{
		...
		?>
			...
			unset($nameArray);
				...
				$order_arr=array();
				$sql_ord=sql_select("select a.subcon_job, a.job_no_prefix_num, a.currency_id, b.id, b.order_no, b.cust_style_ref, b.cust_buyer,b.rate   from subcon_ord_mst a, subcon_ord_dtls b where a.subcon_job=b.job_no_mst ");
		...
		?>
			...
			//	echo $inhouse_bill_from.'='.$inhouse_bill_from;
				if($ex_bill_for!=3) 
				{
					...
					{		
						$roll_field_con="LISTAGG(e.roll_id, ',') WITHIN GROUP (ORDER BY e.roll_id desc) as roll_id";
						      ...
						      $sql="SELECT a.id, a.entry_form, c.receive_basis,a.sys_number as sys_number, a.sys_number_prefix_num as recv_number_prefix_num, '' as challan_no, a.delevery_date as receive_date, c.booking_id as bookingno, $year_cond as year, b.batch_id,e.process_id, b.product_id as prod_id, b.bodypart_id as body_part_id, b.determination_id as fabric_description_id, b.color_id, b.width_type as dia_width_type, sum(b.current_delivery) as rec_qnty, sum(f.grey_used_qty) as grey_used_qty,$roll_field_con, sum(e.roll_no) as carton_roll, f.po_breakdown_id as po_breakdown_id, d.booking_no_id, d.booking_no
							...
							and a.entry_form in (54,67) AND c.knitting_source=1 AND a.company_id=$data[3] AND c.location_id=$data[1] AND c.knitting_company=$data[0] and c.receive_basis in (0,2,4,5,9,11) and c.item_category=2 and c.entry_form in (7,66) and b.current_delivery>0 and f.entry_form in (7,66)
							...
							group by a.id, a.entry_form, c.receive_basis,a.sys_number, a.sys_number_prefix_num, a.delevery_date, c.booking_id, a.insert_date, b.batch_id, b.product_id, b.bodypart_id, b.determination_id, b.color_id, b.width_type,e.process_id, f.po_breakdown_id, d.booking_no_id, d.booking_no order by a.sys_number_prefix_num DESC";
					}
					else //Bill from receive
					{
						$sql="SELECT a.id, a.entry_form, a.receive_basis,a.recv_number as sys_number, a.recv_number_prefix_num, a.challan_no, a.receive_date, a.booking_id as bookingno, $year_cond as year, b.batch_id, b.prod_id, b.body_part_id,b.process_id, b.fabric_description_id, b.color_id, b.dia_width_type, sum(c.quantity) as rec_qnty, sum(b.no_of_roll) as carton_roll, c.po_breakdown_id, d.booking_no_id, d.booking_no
				...
				else
				{
					...
					if($inhouse_bill_from==2) //Bill from delivery
					{
						$sql="SELECT a.id, a.entry_form, c.receive_basis, a.sys_number as sys_number,a.sys_number_prefix_num , '' as challan_no, a.delevery_date as receive_date,  $year_cond as year, b.batch_id, b.product_id as prod_id, b.bodypart_id as body_part_id, b.determination_id as fabric_description_id, b.color_id, b.width_type as dia_width_type, sum(b.current_delivery) as rec_qnty, sum(e.grey_used_qty) as grey_used_qty, sum(b.roll) as carton_roll, null as po_breakdown_id, null as booking_no_id, d.booking_no as booking_no
							...
							group by a.id, a.entry_form, c.receive_basis,a.sys_number, a.sys_number_prefix_num, a.delevery_date, d.booking_no, a.insert_date, b.batch_id, b.product_id, b.bodypart_id, b.determination_id, b.color_id, b.width_type, b.order_id order by a.sys_number_prefix_num DESC";
					}
					else
					{
						$sql="SELECT a.id, a.entry_form, a.receive_basis, a.recv_number as sys_number, a.recv_number_prefix_num, a.challan_no, a.receive_date, null as bookingno, $year_cond as year, b.batch_id, b.prod_id, b.body_part_id,b.process_id, b.fabric_description_id, b.color_id, b.dia_width_type, sum(b.receive_qnty) as rec_qnty, sum(b.no_of_roll) as carton_roll, 0 as po_breakdown_id, null as booking_no_id,d.booking_no 
							...
							group by a.id, a.entry_form, a.receive_basis,  a.recv_number,a.recv_number_prefix_num, a.challan_no, a.receive_date, a.insert_date, b.batch_id, b.prod_id, b.body_part_id, b.fabric_description_id, b.color_id,b.process_id, b.dia_width_type,d.booking_no order by a.recv_number_prefix_num DESC";
					}
				}
			 //	echo $sql; //die;
				$sql_result =sql_select($sql);
				...
				$poid_cond=where_con_using_array(array_unique($RollIdArr),0,"d.roll_id");
				     $sql_grey="SELECT a.id as mst_id, a.entry_form, a.issue_basis as receive_basis, a.issue_number_prefix_num as recv_number_prefix_num, a.issue_number as challan_no, a.issue_date as receive_date, a.booking_id as bookingno, $year_cond as year, a.batch_id as batch_id, b.prod_id, b.body_part_id, e.detarmination_id as fabric_description_id, to_number(b.color_id) as color_id, 0 as dia_width_type,  sum(c.quantity) as rec_qty, sum(d.qnty) as grey_qty, sum(c.quantity) as order_qnty, sum(b.no_of_roll) as carton_roll, b.id as dtls_id, c.id, c.po_breakdown_id as po_id, a.booking_id as booking_no_id, d.booking_no
					FROM inv_issue_master a, inv_grey_fabric_issue_dtls b, order_wise_pro_details c, pro_roll_details d,product_details_master e
					...
					group by a.id, a.entry_form, a.issue_basis, a.issue_number_prefix_num, a.issue_number,a.batch_id, a.issue_date, a.booking_id, a.insert_date, a.batch_id, b.prod_id, b.body_part_id, b.color_id,b.id, c.id, c.po_breakdown_id, a.booking_id,e.detarmination_id, d.booking_no order by a.issue_number_prefix_num DESC";
					$sql_result_grey_issueroll =sql_select($sql_grey);
					foreach($sql_result_grey_issueroll as $row)
					{
						$grey_issue_array[$row[csf('po_id')]][$row[csf('fabric_description_id')]][$row[csf('body_part_id')]]['grey_qty']=$row[csf('grey_qty')];
					}
			...
			$sql="select id, item_category_id,product_name_details from product_details_master where status_active=1 and is_deleted=0 $pordidCond"; 
				...
				foreach($sql_result as $row) // for update row
				{
					...
					if(in_array($all_value,$str_arr))
					{
						...
						{
							if ($variable_check==1) $on_bill_qty=$batch_qty_arr[$row[csf('batch_id')]][$row[csf('po_breakdown_id')]][$row[csf('body_part_id')]]['batch_qnty'];
							else $on_bill_qty=$row[csf('rec_qnty')];
						}
						
						$grey_qty=0;
						if($inhouse_bill_from==2) $grey_qty=$row[csf('grey_used_qty')]; else $grey_qty=$batch_qty_arr[$row[csf('batch_id')]][$row[csf('po_breakdown_id')]][$row[csf('body_part_id')]]['batch_qnty'];
						...
						if($independent==4)
						{
                            ...
                            <tr id="tr_<? echo $all_value; ?>" bgcolor="yellow" bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $all_value."***".'1'; ?>');" >
								...
								<td width="100" align="center"><? echo $row[csf('sys_number')]; ?></td>
				...
				foreach($sql_result as $row) // for new row
				{
					...
					{ 
						...
						if($grey_qty) $grey_qty=$grey_qty;else $grey_qty=$grey_issue;
						 ...
						 $grey_qty=$batch_qty_arr[$row[csf('batch_id')]][$row[csf('po_breakdown_id')]][$row[csf('body_part_id')]]['batch_qnty'];
					 }
					$avilable_qty=$grey_qty-$bill_qty;
					 //echo $grey_issue.'='.$avilable_qty.'D';
...
if ($action=="save_update_delete")
{
	...
	$fabric_dyeingCost_arr=array(25,26,31,32,34,38,39,60,61,62,63,74,75,76,78,79,80,81,83,84,85,86,87,88,89,90,92,129,137,138,139,141,144,145,146,147,149,158,162,163,165,169,170,171,172,173,174,176,203,232,234,238,266,267,280,281,282,283,284,285,286,287);
	...
	if ($operation==0)   // Insert Here========================================================================================
	{
		...
		$field_array="id, prefix_no, prefix_no_num, bill_no, company_id, location_id, bill_date, party_id, party_source, party_location_id, inhouse_bill_from, bill_for,upcharge,discount,process_id, inserted_by, insert_date";
		$data_array="(".$id.",'".$new_bill_no[1]."','".$new_bill_no[2]."','".$new_bill_no[0]."',".$cbo_company_id.",".$cbo_location_name.",".$txt_bill_date.",".$cbo_party_name.",".$cbo_party_source.",".$cbo_party_location.",".$hidd_inhouse_bill_from.",".$cbo_bill_for.",".$txt_upcharge.",".$txt_discount.",4,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')"; 
		...
		for($i=1; $i<=$tot_row; $i++)
		{
			...
			{ 
				...
				$data_array1.="(".$id1.",".$id.",".$$delivery_id.",".$$delevery_date.",".$$challen_no.",".$$orderid.",".$$item_id.",".$$compoid.",".$$diaType.",".$$batchid.",".$$bodypartid.",".$$add_process.",".$$txt_add_process.",".$$number_roll.",".$$quantity.",".$$libRateId.",".$$rateDataString.",".$$rate.",".$$libAddRateId.",".$$addRate.",".$tot_amount.",".$$remarks.",".$$cbocolorrangeid.",".$$txtshadeper.",".$$curanci.",".$$color_process.",".$$color_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				$id1=$id1+1;
				$add_comma++;
	...
	else if ($operation==1)   // Update Here=============================================================================
	{
		...
		$field_array="bill_date*upcharge*discount*updated_by*update_date";
		$data_array="".$txt_bill_date."*".$txt_upcharge."*".$txt_discount."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"; 
