...
if ($action=="load_drop_down_party_name")
{
	...
	*/
	...
	{	
		...
		echo create_drop_down( "cbo_party_name", 150, $party_arr,"",$value, "-- Select Party --", $selected, "load_drop_down( 'requires/knitting_bill_issue_controller', this.value, 'load_drop_down_party_location', 'party_location_td');","","","","","",5 ); 
	}
	else
	{
		echo create_drop_down( "cbo_party_name", 150, $blank_array,"", 1, "-- Select Party --", $selected, "",0,"","","","",5);
...
if ($action=="bill_no_popup")
{
    ...
    <script> load_drop_down( 'knitting_bill_issue_controller', document.getElementById('cbo_party_name').value, 'load_drop_down_location_pop', 'location_td');</script>
	</html>
	<?
	exit();
}
...
if ($action=="load_php_data_to_form_issue")
{
	...
	$nameArray= sql_select("select id, bill_no, company_id, location_id,party_location_id, bill_date, party_id, party_source, attention, bill_for, is_posted_account,post_integration_unlock from subcon_inbound_bill_mst where id='$data'");
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   // Insert Here========================================================================================delivery_id
	{
		...
		if(str_replace("'",'',$update_id)=="")
		{
			...
			$field_array="id, prefix_no, prefix_no_num, bill_no, company_id, location_id,party_location_id, bill_date, party_id, party_source, attention, bill_for, process_id, inserted_by, insert_date";
			$data_array="(".$id.",'".$new_bill_no[1]."','".$new_bill_no[2]."','".$new_bill_no[0]."',".$cbo_company_id.",".$cbo_location_name.",".$cbo_party_location.",".$txt_bill_date.",".$cbo_party_name.",".$cbo_party_source.",".$txt_attention.",".$cbo_bill_for.",'".$bill_process_id."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')"; 
	...
	else if ($operation==1)   // Update Here=============================================================================
	{
		...
		$field_array="location_id*party_location_id*bill_date*party_id*party_source*attention*bill_for*updated_by*update_date";
		$data_array="".$cbo_location_name."*".$cbo_party_location."*".$txt_bill_date."*".$cbo_party_name."*".$cbo_party_source."*".$txt_attention."*".$cbo_bill_for."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
...
/*				$res_sql="select a.id, a.recv_number_prefix_num, a.receive_date, a.entry_form, a.challan_no, a.receive_basis, a.booking_id, a.booking_no, b.prod_id, b.body_part_id, b.febric_description_id, b.yarn_count, b.machine_no_id, b.color_id, b.machine_dia, b.machine_gg from inv_receive_master a, pro_grey_prod_entry_dtls b where a.company_id='".$dataArray[0][csf('party_id')]."' and a.knitting_company='$data[0]' and a.location_id='".$dataArray[0][csf('location_id')]."' and a.id=b.mst_id and b.trans_id > 0 and a.entry_form in (2,22,58)  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.id in(".implode(',',$receive_id_arr).")";
*/
			...
			?>
				<tr bgcolor="<? echo $bgcolor; ?>" style="font-size:16px"> 
                    ...
                    <td><div style="word-wrap:break-word; width:40px"><? echo $fab_color; ?></div></td>
...
*/				$bill_no=$dataArray[0][csf('bill_no')];
        ...
        <br>
		 <?
            echo signature_table(47, $data[0], "980px");
         ?>
   </div>
   </div>
<?
