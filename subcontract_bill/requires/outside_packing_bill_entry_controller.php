                ...
                <table width="650" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_supplier_company').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_search_common').value, 'packing_bill_list_view', 'search_div', 'outside_packing_bill_entry_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" />
                            </td>
                        </tr>
                        <tr>
...
if ($action=="packing_bill_list_view")
{
	...
	$sql= "select id, bill_no, prefix_no_num, $year_cond, location_id, bill_date, supplier_id, bill_for from subcon_outbound_bill_mst where process_id=8 and status_active=1 $company_name $party_name $return_date $bill_id_cond";
	
	echo  create_list_view("list_view", "Bill No,Year,Location Name,Bill Date,Party Name,Bill For", "70,70,100,100,120,100","600","250",0, $sql , "js_set_value", "id", "", 1, "0,0,location_id,0,supplier_id,bill_for", $arr , "prefix_no_num,year,location_id,bill_date,supplier_id,bill_for", "outside_packing_bill_entry_controller","",'0,0,0,3,0,0') ;
	exit(); 
}
...
if ($action=="load_php_data_to_form_issue")
{
	...
	{	
		...
		echo "load_drop_down( 'requires/outside_packing_bill_entry_controller', document.getElementById('cbo_company_id').value, 'load_drop_down_location', 'location_td' );\n";
		echo "load_drop_down( 'requires/outside_packing_bill_entry_controller', document.getElementById('cbo_company_id').value, 'load_drop_down_supplier', 'supplier_td' );\n";
...
if ($action=="packing_entry_list_view")
{
	...
	<body>
        ...
        <div style="width:800px;max-height:180px; overflow-y:scroll" id="packing_list_view" align="center">
            ...
            if($data[2]=="") // Insert
            {
                $sql="select id, challan_no, production_date, po_break_down_id, item_number_id, serving_company, production_quantity from pro_garments_production_mst where company_id=$data[0] and serving_company=$data[1] and production_source=3 and production_type=8 and status_active=1 and is_deleted=0";			
            }
            else
            {
            ...
            foreach($sql_result as $row)
            {
                ...
                <tr id="tr_<? echo $row[csf('id')]; ?>" bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $row[csf('id')]; ?>');" >
                    ...
                    <input type="text" id="currid<? echo $row[csf('id')]; ?>" value="<? echo $row[csf('id')]; ?>"></td>
                </tr>
                <?php
                $i++;
            }
...
if ($action=="load_php_dtls_form") 
{
	...
	if( $data[2]!="" )//update===========
	{
		$sql="SELECT id as upd_id, receive_id, receive_date, challan_no, order_id, item_id as prod_id, receive_qty as order_qnty, uom, rate, amount, remarks FROM subcon_outbound_bill_dtls  WHERE mst_id=$data[2] and process_id=8";
	}
	else //insert=================
	{
		if($bill_id!="" && $del_id!="")
			$sql="(SELECT id as upd_id, receive_id, receive_date, challan_no, order_id, item_id as prod_id, receive_qty as order_qnty, uom, rate, amount, remarks  FROM subcon_outbound_bill_dtls  WHERE receive_id in ($bill_id) and process_id='8' )
			 union 
			 (SELECT 0 as upd_id, id as receive_id, production_date as receive_date, challan_no, po_break_down_id as order_id, item_number_id as prod_id, sum(production_quantity) as order_qnty, 0 as uom, 0 as rate, 0 as amount, null as remarks FROM pro_garments_production_mst WHERE id in ($del_id) and production_source=3 and production_type=8 and status_active=1 and is_deleted=0 group by id, production_date, challan_no, po_break_down_id, item_number_id)";
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   // Insert Here 
	{
		...
		$new_bill_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '', 'PNF', date("Y",time()), 5, "select prefix_no,prefix_no_num from  subcon_outbound_bill_mst where company_id=$cbo_company_id and process_id=$bill_process_id $year_cond=".date('Y',time())." order by id desc ", "prefix_no", "prefix_no_num" ));
