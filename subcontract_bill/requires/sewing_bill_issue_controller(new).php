...
if ($action=="sewing_delivery_list_view")
{
	...
	//Array ( [0] => 1 [1] => 1 [2] => 1 [3] => 1 [4] => 0 [5] => 08-07-2019 [6] => 08-07-2019 ) 
	...
	if($data[2]==2)
	{
        ...
        <div style="width:820px;max-height:180px; overflow-y:scroll" id="sewing_production_list_view">
            ...
            <?
				...
				if(!$data[9])
				{
					$sql="select b.id, a.challan_no, a.delivery_date, b.process_id, b.item_id, b.delivery_qty, b.order_id from  subcon_delivery_mst a, subcon_delivery_dtls b where b.bill_status=0 and a.id=b.mst_id and a.party_id='$data[3]' and b.process_id='5' and a.status_active=1 and a.is_deleted=0"; 
				}
				else
				{
					$sql="(select b.id, a.challan_no, a.delivery_date, b.process_id, b.item_id, b.delivery_qty, b.order_id from subcon_delivery_mst a, subcon_delivery_dtls b where a.id=b.mst_id and a.party_id='$data[3]' and b.process_id='5' and a.status_active=1 and b.bill_status='0')
					 union 
					 	(select b.id, a.challan_no, a.delivery_date, b.process_id, b.item_id, b.delivery_qty, b.order_id from subcon_delivery_mst a, subcon_delivery_dtls b where a.id=b.mst_id and a.party_id='$data[3]' and b.process_id='5' and b.id in ( $data[9] ) and a.status_active=1)";
				}
	...
	else if($data[2]==1)
	{
        ...
        <div style="width:910px;max-height:180px; overflow-y:scroll" id="sewing_production_list_view">
            ...
            <?
				...
				if(!$data[9])
				{
					$sql="select id, challan_no, production_date, po_break_down_id, item_number_id, serving_company, country_id, production_quantity from pro_garments_production_mst where  serving_company=$data[3] $date_cond and production_source=1 and produced_by=1 and production_type=5 and status_active=1 and is_deleted=0 order by id DESC"; 
				}
				else
				{
					$sql_data="select id, challan_no, production_date, po_break_down_id, item_number_id, serving_company, country_id, production_quantity from pro_garments_production_mst where id in ($data[9]) and serving_company=$data[3] $date_cond  and production_source=1 and produced_by=1 and production_type=5 and status_active=1 and is_deleted=0 order by id DESC";
		...
		else
		{
			if($bill_id!="" && $del_id!="")
				$sql="(select id as upd_id, delivery_id, delivery_date, challan_no, item_id, packing_qnty as carton_roll, delivery_qty, rate, amount, remarks, order_id, currency_id from subcon_inbound_bill_dtls where delivery_id in ($bill_id) and status_active=1 and is_deleted=0 and process_id=5)
				 union
				 (select 0, a.id as delivery_id, a.delivery_date, a.challan_no, b.item_id, b.carton_roll, b.delivery_qty, 0, 0, null, b.order_id from  subcon_delivery_mst a, subcon_delivery_dtls b where a.id=b.mst_id and b.id in ($del_id) and a.status_active=1 and b.process_id=5)";
			else if($bill_id!="" && $del_id=="")
				$sql="select id as upd_id, delivery_id, delivery_date, challan_no, item_id, packing_qnty as carton_roll, delivery_qty, rate, amount, remarks, order_id, currency_id from subcon_inbound_bill_dtls where delivery_id in ($bill_id) and status_active=1 and is_deleted=0 and process_id=5";
			else  if($bill_id=="" && $del_id!="")
				$sql="select 0, a.id as delivery_id, a.delivery_date, a.challan_no, b.item_id, b.carton_roll, b.delivery_qty, 0, 0, 0, b.order_id from  subcon_delivery_mst a, subcon_delivery_dtls b where a.id=b.mst_id and b.id in ($del_id) and a.status_active=1 and a.is_deleted=0 and b.process_id=5"; 
		}
		$sql_result =sql_select($sql);	
		$k=0;
