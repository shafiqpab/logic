        ...
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="100%" class="rpt_table">
            ...
            foreach($sql_result as $row)
            {
                ...
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value('<? echo $row[csf('id')]."_".$row[csf('booking_no')]."_".$row[csf('rate')]; ?>');" > 
                    ...
                    <td width="100" align="center"><? echo $row[csf('color_size_table_id')]; ?></td>
...
if ($action=="knitting_entry_list_view")
{
	...
	//print_r ($ex_bill_for);
	if($ex_bill_for==3) $tbl_wight="720"; else $tbl_wight="1100";
	?>
    </head>
    <body>
    <div align="center" style="width:100%;" >	
        ...
        <?  
			...
			$po_array=array();
			$sql_po="Select a.job_no, a.job_no_prefix_num, a.style_ref_no, a.buyer_name, b.id, b.po_number from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.is_deleted=0 and a.status_active=1 ";//and b.status_active=1 and b.is_deleted=0
			...
			if($ex_bill_for!=3)
			{
				...
				/*$sql1="(select a.id, a.recv_number_prefix_num, a.challan_no, a.challan_no as pre_challan, a.receive_date, a.entry_form, b.prod_id, b.body_part_id, b.febric_description_id, sum(b.no_of_roll) as roll_qty, c.po_breakdown_id, sum(c.quantity) as quantity, a.receive_basis, a.booking_no, a.booking_id, '' as dtls_id from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c
					...
					$sql="(select a.id, 0 as grey_sys_id, a.recv_number_prefix_num, a.challan_no, a.receive_date, a.entry_form, b.prod_id, b.body_part_id, b.febric_description_id, sum(b.no_of_roll) as roll_qty, c.po_breakdown_id, sum(c.quantity) as quantity, a.receive_basis, a.booking_no, a.booking_id, null as dtls_id from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c
					where a.id=b.mst_id and b.id=c.dtls_id and a.knitting_source=3 and a.company_id='$data[0]' and a.knitting_company='$data[3]' and a.location_id='$data[1]' and c.trans_type=1 and a.entry_form in (2,22) and c.entry_form in (2,22) and a.item_category=13 and a.receive_basis in (0,1,2,4,9,11) and c.trans_id!=0 and $booking_without_order=0
					and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $date_cond $po_breakdown_id_conds
					group by a.id, a.recv_number_prefix_num, a.challan_no, a.receive_date, a.entry_form, b.prod_id, b.body_part_id, b.febric_description_id, c.po_breakdown_id, a.receive_basis, a.booking_no, a.booking_id)
					union all
					(select a.id, e.grey_sys_id, a.recv_number_prefix_num, a.challan_no, a.receive_date, a.entry_form, b.prod_id, b.body_part_id, b.febric_description_id, sum(b.no_of_roll) as roll_qty, c.po_breakdown_id, sum(c.quantity) as quantity, a.receive_basis, a.booking_no, a.booking_id, $dtls_id_cond as dtls_id from 
					...
					and a.knitting_source=3 and a.company_id='$data[0]' and a.knitting_company='$data[3]' and a.location_id='$data[1]' and c.trans_type=1 and a.item_category=13 and a.receive_basis in (10) and c.trans_id!=0 and $booking_without_order=0
					and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $date_cond $po_breakdown_id_conds
					group by a.id, a.recv_number_prefix_num, a.challan_no, a.receive_date, a.entry_form, b.prod_id, b.body_part_id, b.febric_description_id, c.po_breakdown_id, a.receive_basis, a.booking_no, a.booking_id, e.grey_sys_id) order by recv_number_prefix_num Desc";
 ...
 a.booking_no, a.booking_id, e.grey_sys_id*/
				...
				foreach($sql_result as $row) // for new row
				{
					...
					$all_value=$row[csf('recv_number_prefix_num')].'_'.$row[csf('po_breakdown_id')].'_'.$row[csf('prod_id')].'_'.$row[csf('body_part_id')].'_'.$row[csf('febric_description_id')];
					$str_val=$row[csf('id')].'_'.change_date_format($row[csf('receive_date')]).'_'.$row[csf('recv_number_prefix_num')].'_'.$row[csf('po_breakdown_id')].'_'.$po_array[$row[csf('po_breakdown_id')]]['po_number'].'_'.$po_array[$row[csf('po_breakdown_id')]]['style_ref_no'].'_'.$buyer_arr[$po_array[$row[csf('po_breakdown_id')]]['buyer']].'_'.$row[csf('no_of_roll')].'_'.$row[csf('body_part_id')].'_'.$row[csf('febric_description_id')].'_'.$row[csf('prod_id')].'_'.$item_id_arr[$row[csf('prod_id')]].'_'.$avilable_qty.'_'.$body_part[$row[csf('body_part_id')]];
					if($independent==4)
					{
						if($avilable_qty>0)
						{
			...
			else if($ex_bill_for==3)// sample without order
			{
				$sql1="(select a.id, a.recv_number_prefix_num, a.challan_no, a.receive_date, c.entry_form, b.prod_id, b.body_part_id, b.febric_description_id, sum(b.no_of_roll) as roll_qty, sum(b.grey_receive_qnty) as quantity, c.receive_basis, c.booking_no, c.booking_id from inv_receive_master a, pro_grey_prod_entry_dtls b, inv_receive_master c
					...
					$sql="select a.id, a.recv_number_prefix_num, a.challan_no, a.receive_date, a.entry_form, b.prod_id, b.body_part_id, b.febric_description_id, sum(b.no_of_roll) as roll_qty, sum(b.grey_receive_qnty) as quantity, a.receive_basis, a.booking_no, a.booking_id from inv_receive_master a, pro_grey_prod_entry_dtls b
					where a.id=b.mst_id and a.knitting_source=3 and a.company_id='$data[0]' and a.knitting_company='$data[3]' and a.location_id='$data[1]' and a.entry_form in (2,22,58) and a.item_category=13 and a.receive_basis in (0,1,2,4,9,10,11) and b.trans_id!=0 and $booking_without_order=0
					and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $date_cond 
					group by a.id, a.recv_number_prefix_num, a.challan_no, a.receive_date, a.entry_form, b.prod_id, b.body_part_id, b.febric_description_id, a.receive_basis, a.booking_no, a.booking_id order by a.recv_number_prefix_num Desc";
				...
				foreach($sql_result as $row) // for update row
				{
					...
					if(in_array($all_value,$str_arr))
					{
						...
						$str_val=$row[csf('id')].'_'.change_date_format($row[csf('receive_date')]).'_'.$row[csf('recv_number_prefix_num')].'_'.$row[csf('po_breakdown_id')].'_'.$po_array[$row[csf('po_breakdown_id')]]['po_number'].'_'.$po_array[$row[csf('po_breakdown_id')]]['style_ref_no'].'_'.$buyer_arr[$po_array[$row[csf('po_breakdown_id')]]['buyer']].'_'.$row[csf('roll_qty')].'_'.$row[csf('body_part_id')].'_'.$row[csf('febric_description_id')].'_'.$row[csf('prod_id')].'_'.$item_id_arr[$row[csf('prod_id')]].'_'.$row[csf('quantity')].'_'.$body_part[$row[csf('body_part_id')]];
						if($independent==4)
						{
							if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
							?>
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   // Insert Here========================================================================================receive_id
	{
		...
		if(str_replace("'",'',$update_id)=="")
		{
			...
			$field_array="id, prefix_no, prefix_no_num, bill_no, company_id, location_id, bill_date, supplier_id, bill_for, party_bill_no, process_id, inserted_by, insert_date";
			$data_array="(".$id.",'".$new_bill_no[1]."','".$new_bill_no[2]."','".$new_bill_no[0]."',".$cbo_company_id.",".$cbo_location_name.",".$txt_bill_date.",".$cbo_supplier_company.",".$cbo_bill_for.",".$txt_party_bill_no.",'".$bill_process_id."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')"; 
	...
	else if ($operation==1)   // Update Here=============================================================================
	{
		...
		$id=str_replace("'",'',$update_id);
		$field_array="bill_date*party_bill_no*updated_by*update_date";
		$data_array="".$txt_bill_date."*".$txt_party_bill_no."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"; 
		$return_no=str_replace("'",'',$txt_bill_no);
