...
if ($action == "load_drop_down_floor") {
	...
	if ($location_id == 0 || $location_id == "") {
		...
		$location_cond = " and b.location_id=$location_id";
	}
...
if ($action=="batch_search_list_view")
{
	...
	if ($party_id>0) $party_cond=" and d.party_id=$party_id"; else $party_cond="";
	 $sql="select a.id, a.batch_no, a.extention_no, a.color_id,a.process_id, a.batch_weight, a.total_liquor, sum(b.batch_qnty) as batch_qnty, b.po_id as po_id from pro_batch_create_mst a,pro_batch_create_dtls b,subcon_ord_dtls c,subcon_ord_mst d where a.id=b.mst_id and c.id=b.po_id and d.subcon_job=c.job_no_mst and a.status_active=1 and a.entry_form=36   $company_con $batch_date_cond $batch_no_cond $party_cond group by a.id,a.process_id, a.batch_no, a.extention_no, a.color_id, a.batch_weight, a.total_liquor, b.po_id order by a.id DESC";// and a.batch_against=1
	 //echo $sql;
	 ...
	 foreach( $result as $row )
		{
			...
			{
				$batch_data_arr[$row[csf("id")]]['id']=$row[csf("id")];
				$batch_data_arr[$row[csf("id")]]['total_liquor']=$row[csf("total_liquor")];
				$batch_data_arr[$row[csf("id")]]['batch_weight']=$row[csf("batch_weight")];
				$batch_data_arr[$row[csf("id")]]['color_id']=$row[csf("color_id")];
				$batch_data_arr[$row[csf("id")]]['extention_no']=$row[csf("extention_no")];
				$batch_data_arr[$row[csf("id")]]['batch_no']=$row[csf("batch_no")];
				$batch_data_arr[$row[csf("id")]]['po_id']=$row[csf("po_id")];
				$batch_data_arr[$row[csf("id")]]['process_id']=$row[csf("process_id")];
			}
			 $process_idArr=explode(",",$row[csf("process_id")]);
			if(in_array(440,$process_idArr)) // Finishing Process
			{
				$batch_data_arr[$row[csf("id")]]['id']=$row[csf("id")];
				$batch_data_arr[$row[csf("id")]]['total_liquor']=$row[csf("total_liquor")];
				$batch_data_arr[$row[csf("id")]]['batch_weight']=$row[csf("batch_weight")];
        ...
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="730" class="rpt_table" id="list_view">
            ...
            foreach( $batch_data_arr as $batch_id=>$row )
            {
				...
				//{
					...
					$order_no='';
					$order_id=array_unique(explode(",",$row[("po_id")]));
					foreach($order_id as $val)
					{
						if($order_no=="") $order_no=$po_arr[$val]; else $order_no.=",".$po_arr[$val];
					}
					?>
						<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value('<? echo $batch_id.'_'.$row[("po_id")];?>');" >
							<td width="30" align="center"><? echo $i; ?></td>
     ...
     <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="400">
        ...
        <tbody>
            ...
            foreach($data_array as $row)
            {
                if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
             ?>
                <tr bgcolor="<? echo $bgcolor; ?>" onClick='set_form_data("<? echo $row[csf('id')]."**".$row[csf('item_description')].','.$row[csf('fin_dia')]."**".$row[csf('gsm')]."**".$row[csf('color_id')]."**".$color_arr[$row[csf('color_id')]]."**".$row[csf('po_id')]."**".$row[csf('width_dia_type')]; ?>")' style="cursor:pointer" >
                    <td><? echo $i; ?></td>
