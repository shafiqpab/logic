...
{
	echo "$('#finishing_production_variable').val(0);\n";
	$sql_result = sql_select("select finishing_update from  variable_settings_production where company_name=$data and variable_list=1 and status_active=1");
 	foreach($sql_result as $result)
	{
		if($result[csf("finishing_update")]==4){$result[csf("finishing_update")]=3;}
		echo "$('#finishing_production_variable').val(".$result[csf("finishing_update")].");\n";
		//echo "$('#styleOrOrderWisw').val(".$result[csf("finishing_update")].");\n";
	}
 	exit();
}
...
{
	echo create_drop_down( "cbo_location", 140, "SELECT id, location_name from lib_location WHERE status_active =1 and is_deleted=0 and company_id='$data' order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down( 'requires/subcon_packing_finishing_entry_controller', this.value, 'load_drop_down_floor', 'floor_td' );load_drop_down( 'requires/subcon_packing_finishing_entry_controller', document.getElementById('cbo_location').value, 'load_drop_down_finishing_line_for_location', 'finishing_line_td' );" );
	exit();
}

if ($action=="load_drop_down_floor") 
{
	echo create_drop_down( "cbo_floor", 140, "select id, floor_name from lib_prod_floor where status_active=1 and production_process=11 and is_deleted=0 and location_id='$data' order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/subcon_packing_finishing_entry_controller', document.getElementById('cbo_company_name').value, 'load_drop_down_finishing_line_for_floor', 'finishing_line_td' );",0 );  
	exit();   	 
}

if($action=="load_drop_down_finishing_line_for_company")
{
	echo create_drop_down( "cbo_finishing_line", 110, "select id,line_name from lib_sewing_line where  is_deleted=0 and status_active=1 and company_name='$data' ","id,line_name", 1, "Select Line", $selected, "" );
}

if($action=="load_drop_down_finishing_line_for_location")
{
	echo create_drop_down( "cbo_finishing_line", 110, "select id,line_name from lib_sewing_line where  is_deleted=0 and status_active=1 and location_name='$data' ","id,line_name", 1, "Select Line", $selected, "" );
}

if($action=="load_drop_down_finishing_line_for_floor")
{
	$data=explode("_",$data);
	//$company_id=$data[0];

	echo create_drop_down( "cbo_finishing_line", 110, "select id,line_name from lib_sewing_line where is_deleted=0 and status_active=1 and company_name='$data[0]' ","id,line_name", 1, "Select Line", $selected, "" );//and floor_name='$data[1]' 
}
        ...
        <div style="width:815px; overflow-y:scroll; max-height:280px;" id="buyer_list_view" align="center">
            ...
            <?
				...
				foreach($sql_sew_result as $row)
				{
					...
					$pack_fin_array=array(131);
					if(array_intersect($pack_fin_array,$processid))
					{
				?>
...
if($action=="populate_data_from_search_popup")
{
 	...
 	foreach($sql_res as $result)
	{
		...
		// echo "load_drop_down( 'requires/subcon_packing_finishing_entry_controller', document.getElementById('cbo_location').value, 'load_drop_down_floor', 'floor_td' );\n";
	   // echo "$('#cbo_floor').val('".$result[csf('floor_id')]."');\n";  
		...
		$total_produced = return_field_value("sum(production_qnty)","subcon_gmts_prod_dtls","order_id='$po_id' and gmts_item_id='$item_id' and production_type=4 and is_deleted=0");
		echo "$('#txt_cumul_finishing_qty').attr('placeholder','".$total_produced."');\n";
		echo "$('#txt_cumul_finishing_qty').val('".$total_produced."');\n";
		$yet_to_produced = $result[csf('qnty')]*1 - $total_produced;
		echo "$('#txt_yet_to_finishing').attr('placeholder','".$yet_to_produced."');\n";
		echo "$('#txt_yet_to_finishing').val('".$yet_to_produced."');\n";
...
{ 
	...
	//#############################################################################################//
	$sqlProd =sql_select(" SELECT b.ord_color_size_id,  (b.prod_qnty) AS prod_qnty FROM subcon_gmts_prod_dtls a, subcon_gmts_prod_col_sz b WHERE a.id = b.dtls_id AND a.production_type = b.production_type AND b.production_type = 2 and a.order_id in($po_id)  and a.gmts_item_id in($item_id) ");

 	$prod_out_qntyArr=array();
	foreach($sqlProd as $result)
	{
		$prod_out_qntyArr[$result[csf("ord_color_size_id")]]+=$result[csf("prod_qnty")];
	}
	...
	{
		$i++;
		$prod_out_qnty=$prod_out_qntyArr[$color[csf("id")]];
		$prevQty = $color[csf("production_qnty")];
		$color[csf("qnty")]='';
		$color[csf("qnty")]=$prod_out_qnty;
		if( $variableSettings==2 ) // color level
		{ 
			$colorHTML .='<tr><td>'.$color_library[$color[csf("color_id")]].'</td><td><input type="text" name="colSize" id="colSize_'.$i.'" style="width:80px"  class="text_boxes_numeric" placeholder="'.($color[csf("qnty")]-$color[csf("production_qnty")]).'" onblur="fn_chk_next_process_qty('.$color[csf("color_id")].','.($i).','.$color[csf("size_id")].');fn_colorlevel_total('.$i.')"><input type="hidden" id="txt_colo_size_mst_id_'.$i.'" name="txt_colo_size_mst_id_'.$i.'" value="'.$color[csf("id")].'"><input type="hidden" name="colSizeUpQty" id="colSizeUpQty_'.$i.'" value="'.$prevQty.'" ><input type="hidden" name="colSizeoutQty" id="colSizeoutQty_'.$i.'" value="'.$prod_out_qnty.'" ></td></tr>';				
		...
		else //color and size level
		{
			...
			$colorHTML .='<tr><td>'.$size_library[$color[csf("size_id")]].'</td><td><input type="text" name="colSize" id="colSize'.$k.'___'.$z.'"  class="text_boxes_numeric" style="text-align:right;width:100px;"  placeholder="'.($color[csf("qnty")]-$color[csf("production_qnty")]).'" onblur="fn_chk_next_process_qty('.$k.','.$z.','.$color[csf("size_id")].');sum_qnty('.$k.','.$z.' );sum_qnty('.$k.','.$z.' )" ><input type="hidden" id="txt_colo_size_mst_id'.$k.'___'.$z.'" name="txt_colo_size_mst_id'.$k.'___'.$z.'" value="'.$color[csf("id")].'" ><input type="hidden" name="colSizeUpQty" id="colSizeUpQty_'.$k.$z.'" value="'.$prevQty.'"><input type="hidden" name="colSizeoutQty" id="colSizeoutQty_'.$k.$z.'" value="'.$prod_out_qnty.'" ></td></tr>';		//fn_total('.$i.')
		}
		$z++;
		$j++;
	}
...
if($action=="show_dtls_listview")
{
	...
	$po_id = $dataArr[0];
	$item_id = $dataArr[1];
	$finishing_production_variable=$dataArr[2];
	?>	
	<div style="width:100%;">
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="780" class="rpt_table">
            <thead>
                ...
                <th width="30">SL</th>
                <th width="80" align="center">Finishing Date</th>
                <th width="80" align="center">Rep. Hour</th>
                <th width="80" align="center">Finishing Qty</th>
                <th width="80" align="center">Reject Qty</th>                    
                <th width="80" align="center">Alter Qty</th>
			...
			foreach($sqlResult as $selectResult)
			{
				if ($i%2==0)  
                	$bgcolor="#E9F3FF";
               	 	...
               	 	$bgcolor="#FFFFFF";
			?>
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>,'populate_sub_finishing_form_data','requires/subcon_packing_finishing_entry_controller');" >
                    ...
                    <td width="80" align="center"><? echo $selectResult[csf('hour')]; ?> </td> 
                    <td width="80" align="right"><? echo $selectResult[csf('production_qnty')]; ?>&nbsp;</td>
                    <td width="80" align="right"><? echo $selectResult[csf('reject_qnty')]; ?>&nbsp;</td>
                    <td width="80" align="right"><? echo $selectResult[csf('alter_qnty')]; ?>&nbsp;</td>
                        <? 
                    ...
                    <td width="120" align="center"><p><? echo $location_name; ?></p></td>
                        ...
                        ?>                
                    <td width="80" align="center"><p><? echo $floor_name; ?></p></td>  
                                        
                    <td width="" align="center"><p><? echo $production_update_areas[$selectResult[csf('entry_break_down_type')]]; ?></p></td>                    
                </tr>
			<?php
			$i++;
			}
...
if($action=="packing_finishing_entry_print")
{
	extract($_REQUEST);
	echo load_html_head_contents("Sewing Packing and Finishing Entry Challan Print", "../", 1, 1,'','','');
	$data=explode('*',$data);
	/*print_r($data);*/
    ...
    <table width="900" cellspacing="0" align="right">
        ...
        <tr>
            <td colspan="4" align="center" style="font-size:20px"><strong>Packing and Finishing Entry</strong></td>
            <td colspan="2">&nbsp;</td>
	...
	<div style="width:100%;">
	    <table align="right" cellspacing="0" width="960"  border="1" rules="all" class="rpt_table" >
	        <thead bgcolor="#dddddd" align="center">
	            ...
	            <th width="80" align="center">Total Carton Qty.</th>
	            <th width="120">Remarks</th>
	        </thead>
	        <tbody>
	        	<?
	        		 
	        		$sql_prod="SELECT a.id, a.production_date,  a.challan_no,a.prod_reso_allo, a.floor_id,a.line_id,a.manual_cut_no, a.remarks,c.color_id as color_number_id,  sum(b.prod_qnty) as prod_qnty, a.carton_qty  from subcon_gmts_prod_dtls a, subcon_gmts_prod_col_sz b,subcon_ord_breakdown c where a.id=b.dtls_id and b.ord_color_size_id=c.id and  a.production_type=4 and a.id in($mst_id)  and a.status_active=1 and a.is_deleted=0 and b.prod_qnty>0 group by   a.id, a.production_date,  a.challan_no, a.floor_id, a.remarks,c.color_id,a.line_id,a.manual_cut_no, a.prod_reso_allo, a.carton_qty";
					...
					?>
						<tr>
	                        ...
	                        <td align="right">
	                          ...
	                          <td align="right"> <? echo $val[csf("carton_qty")]?> </td>
...
if($action=="populate_sub_finishing_form_data")
{
	
	$sqlResult =sql_select("select c.color_id, (b.prod_qnty) as prod_qnty,a.gmts_item_id,a.order_id from  subcon_gmts_prod_dtls a, subcon_gmts_prod_col_sz b, subcon_ord_breakdown c where a.id=b.dtls_id and a.production_date=b.production_date and a.status_active=1 and a.is_deleted=0 and a.production_type='4' and a.id='$data' and c.id=b.ord_color_size_id");
 	$prod_qnty_sub=array();
	...
	$sqlProd =sql_select(" SELECT b.ord_color_size_id, 
	 (b.prod_qnty) AS prod_qnty FROM subcon_gmts_prod_dtls a, subcon_gmts_prod_col_sz b WHERE a.id = b.dtls_id AND a.production_type = b.production_type AND b.production_type =2 and a.order_id in($order_id)  and a.gmts_item_id in($gmts_item_id)");
	 
 	$prod_packingFin_qntyArr=array();
	foreach($sqlProd as $result)
	{
		$prod_out_qntyArr[$result[csf("ord_color_size_id")]]+=$result[csf("prod_qnty")];
	}
	//select id, floor_name from lib_prod_floor where status_active=1 and production_process=11 and is_deleted=0 and location_id='$data' order by floor_name
	
	if($db_type==0)  $hour_field=" TIME_FORMAT(a.hour, '%H:%i' ) as hour"; 
    else             $hour_field=" TO_CHAR(a.hour,'HH24:MI')  as hour";
	$sqlResult =sql_select("select c.color_id, c.size_id, c.qnty, c.plan_cut, a.id, a.company_id, a.location_id, a.floor_id, a.line_id, a.order_id, a.production_date, a.production_qnty, a.reject_qnty, a.alter_qnty, a.spot_qnty, a.supervisor, a.production_type, a.entry_break_down_type, a.table_no, a.challan_no, a.remarks, $hour_field, a.minute, b.production_type, b.gmts_item_id, b.production_date, b.ord_color_size_id, b.prod_qnty from subcon_gmts_prod_dtls a,subcon_gmts_prod_col_sz b,subcon_ord_breakdown c where a.id=b.dtls_id and a.production_date=b.production_date and a.status_active=1 and a.is_deleted=0 and a.production_type='4' and a.id='$data' and c.id=b.ord_color_size_id order by c.color_id,c.id");
 
	//$sqlResult =sql_select("select a.id, a.location_id, a.floor_id, a.line_id, a.production_date, a.production_qnty, a.reject_qnty, a.alter_qnty, a.spot_qnty, a.supervisor, a.hour, a.minute, a.challan_no, a.remarks, c.color_id, c.size_id, c.qnty, c.plan_cut, a.order_id, a.production_type, a.entry_break_down_type, a.table_no, b.gmts_item_id, b.prod_qnty from subcon_gmts_prod_dtls a, subcon_gmts_prod_col_sz b,subcon_ord_breakdown c where a.id=b.dtls_id and a.production_date=b.production_date and a.status_active=1 and a.is_deleted=0 and a.production_type='4' and a.id='$data' and c.id=b.ord_color_size_id order by c.color_id");
  	...
  	foreach($sqlResult as $result)
	{
		if($i==0)
		{
			...
			//echo "load_drop_down( 'requires/subcon_packing_finishing_entry_controller', document.getElementById('cbo_company_name').value, 'load_drop_down_floor', 'floor_td' );\n";			
			
			echo "load_drop_down( 'requires/subcon_packing_finishing_entry_controller', document.getElementById('cbo_location').value, 'load_drop_down_floor', 'floor_td' );\n";
			echo "$('#cbo_floor').val('".$result[csf('floor_id')]."');\n";  
			echo "$('#cbo_finishing_line').val('".$result[csf('line_id')]."');\n"; 
			//echo "load_drop_down( 'requires/subcon_packing_finishing_entry_controller', document.getElementById('cbo_company_name').value, 'load_drop_down_finishing_line_for_company', 'finishing_line_td' );\n";
			//echo "load_drop_down( 'requires/subcon_packing_finishing_entry_controller', document.getElementById('cbo_location').value, 'load_drop_down_finishing_line_for_location', 'finishing_line_td' );\n";
			//echo "load_drop_down( 'requires/subcon_packing_finishing_entry_controller', document.getElementById('cbo_floor').value, 'load_drop_down_finishing_line_for_floor', 'finishing_line_td' );\n";
			echo "$('#txt_finishing_date').val('".change_date_format($result[csf('production_date')])."');\n";
			echo "$('#txt_finishing_qty').val('".$result[csf('production_qnty')]."');\n";
			...
			echo "$('#txt_cumul_finishing_qty').val('".$result[csf('total_produced')]."');\n";
			echo "$('#txt_mst_id').val('".$result[csf('id')]."');\n";
			echo "set_button_status(1, permission, 'fnc_subcon_finishing_entry',1);\n";
	...
	$i++;
		...
		$prod_out_qnty=$prod_out_qntyArr[$result[csf("ord_color_size_id")]];
		$result[csf("plan_cut")]='';
		$result[csf("plan_cut")]=$prod_out_qnty;
		...
		{
			$colorHTML .='<tr><td>'.$color_library[$break_color_library[$result[csf("ord_color_size_id")]]].'</td><td><input type="text" name="colorSize" id="colSize_'.$i.'" style="width:80px"  class="text_boxes_numeric" placeholder="'.($result[csf("plan_cut")]-$result[csf("prod_qnty")]).'" title="'.($result[csf("plan_cut")]-$result[csf("prod_qnty")]).'" value="'.$result[csf("prod_qnty")].'" onblur="fn_chk_next_process_qty('.$result[csf("color_id")].','.$i.','.$result[csf("size_id")].');fn_colorlevel_total('.$i.')"><input type="hidden" id="txt_colo_size_mst_id_'.$i.'" name="txt_colo_size_mst_id_'.$i.'" value="'.$result[csf("ord_color_size_id")].'"><input type="hidden" name="colSizeUpQty" id="colSizeUpQty_'.$i.'" value="'.$prevQty.'" ><input type="hidden" name="colSizeoutQty" id="colSizeoutQty_'.$i.'" value="'.$prod_out_qnty.'"></td></tr>';				
		...
		{	
			...
			$colorHTML .='<tr><td>'.$size_library[$break_size_library[$result[csf("ord_color_size_id")]]].'</td><td><input type="text" name="colSize" id="colSize'.$k.'___'.$z.'"  class="text_boxes_numeric" style="text-align:right;width:100px;" placeholder="'.($result[csf("plan_cut")]-$result[csf("prod_qnty")]).'"  onblur="fn_chk_next_process_qty('.$k.','.$z.','.$result[csf("size_id")].');sum_qnty('.$k.','.$z.' )" title="'.($result[csf("plan_cut")]-$result[csf("prod_qnty")]).'" value="'.$result[csf("prod_qnty")].'"><input type="hidden" id="txt_colo_size_mst_id'.$k.'___'.$z.'" name="txt_colo_size_mst_id'.$k.'___'.$z.'" value="'.$result[csf("ord_color_size_id")].'"><input type="hidden" name="colSizeUpQty" id="colSizeUpQty_'.$k.$z.'" value="'.$prevQty.'" ><input type="hidden" name="colSizeoutQty" id="colSizeoutQty_'.$k.$z.'" value="'.$prod_out_qnty.'" ></td></tr>';	
			$z++;
		}
		$j++; 
	}
...
if ($action=="save_update_delete")
{
	...
	if(str_replace("'","",$finishing_production_variable)==4) { $finishing_production_variable = 3;}
	if ($operation==0)//Insert Here=================		
	{
	    ...
	    $field_array="id, company_id, location_id, floor_id, line_id, order_id, gmts_item_id, production_date, production_qnty, reject_qnty, alter_qnty, spot_qnty, hour, supervisor, production_type, entry_break_down_type, challan_no, remarks,carton_qty, is_deleted, status_active, inserted_by, insert_date";
        if($db_type==0)
        {
		
		$data_array="(".$id.",".$cbo_company_name.",".$cbo_location.",".$cbo_floor.",".$cbo_finishing_line.",".$hidden_po_break_down_id.",".$cbo_item_name.",".$txt_finishing_date.",".$txt_finishing_qty.",".$txt_reject_qnty.",".$txt_alter_qnty.",".$txt_spot_qnty.",".$txt_hours.",".$txt_super_visor.",4,".$finishing_production_variable.",".$txt_challan.",".$txt_remark.",".$txt_carton_qty.",0,1,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		
 		$rID=sql_insert("subcon_gmts_prod_dtls",$field_array,$data_array,0);
       }
	   else
	   {
		$txt_hours=str_replace("'","",$txt_finishing_date)." ".str_replace("'","",$txt_hours);
	   ...
	   $data_array="insert into subcon_gmts_prod_dtls (".$field_array.") values(".$id.",".$cbo_company_name.",".$cbo_location.",".$cbo_floor.",".$cbo_finishing_line.",".$hidden_po_break_down_id.",".$cbo_item_name.",".$txt_finishing_date.",".$txt_finishing_qty.",".$txt_reject_qnty.",".$txt_alter_qnty.",".$txt_spot_qnty.",".$txt_hours.",".$txt_super_visor.",4,".$finishing_production_variable.",".$txt_challan.",".$txt_remark.",".$txt_carton_qty.",0,1,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')"; 
		...
		if(str_replace("'","",$finishing_production_variable)==2)//color level wise 
		{		
			for($i=1; $i<=$total_row; $i++)
			{
				...
				$colSize="colSize_".$i;
				if ($i!=1) $data_array1 .=",";
				$data_array1 .= "(".$dtls_id.",".$id.",4,".$cbo_item_name.",".$txt_finishing_date.",".$$txt_colo_size_mst_id.",".$$colSize.")";
				
				$dtls_id=$dtls_id+1;
			}
		}
		if(str_replace("'","",$finishing_production_variable)==3)//color and size wise 
		{
			$k=1;$j=1;$v=0;$tt=0;
		...
		if(str_replace("'","",$finishing_production_variable)==2 || str_replace("'","",$finishing_production_variable)==3)
		{ 
		...
		if($db_type==0)
		{
			if(str_replace("'","",$finishing_production_variable)!=1)
			{
				if($rID && $dtlsrID)
				{
					mysql_query("COMMIT");  
	...
	else if ($operation==1)
	{
		...
		// =======================================Issue Id 16823=====================================//
        $deliveryqty=return_field_value("sum(b.delivery_qty) as deliveryQty","subcon_delivery_dtls b","b.order_id=$hidden_po_break_down_id","deliveryQty");

		//   echo "10**".$deliveryqty;die();

		if($txt_finishing_qty< $deliveryqty)
		{
		
			echo "1000**Packing And Finishing qty not reduce after Garments Delivery Qty";
			disconnect($con);
			die;
		}
		//=================================END====================================//	
		...
		$field_array="location_id*floor_id*line_id*production_qnty*production_date*reject_qnty*alter_qnty*spot_qnty*hour*supervisor*challan_no*remarks*carton_qty*is_deleted*status_active*updated_by*update_date";
		if($db_type==2)
		{
			$txt_hours=str_replace("'","",$txt_finishing_date)." ".str_replace("'","",$txt_hours);
			$txt_hours="to_date('".$txt_hours."','DD MONTH YYYY HH24:MI:SS')";
		}			
		$data_array="".$cbo_location."*".$cbo_floor."*".$cbo_finishing_line."*".$txt_finishing_qty."*".$txt_finishing_date."*".$txt_reject_qnty."*".$txt_alter_qnty."*".$txt_spot_qnty."*".$txt_hours."*".$txt_super_visor."*".$txt_challan."*".$txt_remark."*".$txt_carton_qty."*'0'*'1'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
