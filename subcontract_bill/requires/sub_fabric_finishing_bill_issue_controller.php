...
if ($action=="load_variable_settings")
{
	...
	echo "$('#bill_on').text('');\n"; 
	// //$vari_Sql=sql_select("select dyeing_fin_bill,allow_per from variable_settings_subcon where company_id=".$data[3]."  and variable_list=3 and dyeing_fin_bill=5  and status_active=1 and is_deleted=0");
	// 	foreach($vari_Sql as $row) // for new row 
	// 		{
	// 			$rate_from_budget=$row[csf('allow_per')];
	// 			$dyeing_fin_vari=$row[csf('dyeing_fin_bill')];
	// 		}
	$sql_result = sql_select("select variable_list, dyeing_fin_bill,allow_per from  variable_settings_subcon where company_id='$data' and variable_list in (1,3,8,11,21) order by id");
 	foreach($sql_result as $result)
	{
		...
		else if($result[csf("variable_list")]==3)//rate from
		{
			...
			if($rate_from==5 && $result[csf("allow_per")]==1)
			{
				echo "$('#hidd_rate_readonly').val(".$result[csf("allow_per")].");\n";
			}
			else if($rate_from==1)
			{
				echo "$('#hidd_rate_readonly').val(".$rate_from.");\n";
			}
			
		}
		else if($result[csf("variable_list")]==21)//rate from
		{
			$rate_from=$result[csf("dyeing_fin_bill")];
			if($rate_from=="") $rate_from=3; else if ($rate_from==0) $rate_from=3;
			else $rate_from=$rate_from;
			...
			if($rate_from==5 && $result[csf("allow_per")]==1)
			{
				echo "$('#hidd_rate_disable').val(".$result[csf("allow_per")].");\n";
			}
			else if($rate_from==1)
			{
				echo "$('#hidd_rate_disable').val(".$rate_from.");\n";
			}
...
if($action=="print_button_variable_setting")
{
	...
	foreach($printButton as $id){				
		...
		else if($id==160){echo "$('#print4').show();";}	
		else if($id==129){echo "$('#print5').show();";}	
	}
            ...
            <table width="570" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
            	<thead>
                    ...
                    <th>
						...
						<input type="hidden" name="hide_sys_no" id="hide_sys_no" value="" /> 
                    </th> 
                </thead>
                <tbody>
                	<tr>
...
if($action=="create_booking_search_list_view")
{
	...
	//$sql= "select a.booking_no,a.recv_number_prefix_num,b.job_no,a.id,b.booking_no_prefix_num from inv_receive_master a,wo_booking_mst b where a.BOOKING_ID=b.id and b.booking_type=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form in (7,37,66,68) AND a.knitting_source=1 and a.receive_basis in (2,4,5,9,11) and a.item_category=2 $company $location $booking_cond $booking_date $year_cond order by  a.booking_no";
	


	$sql="select x.booking_no ,x.recv_number_prefix_num,x.job_no,x.id,x.booking_no_prefix_num 
	from (select a.booking_no,a.recv_number_prefix_num,b.job_no,a.id,b.booking_no_prefix_num from inv_receive_master a,wo_booking_mst b where a.BOOKING_ID=b.id and b.booking_type=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form in (7,37,66,68) AND a.knitting_source=1 and a.receive_basis in (2,4,5,9,11) and a.item_category=2 $company $location $booking_cond $booking_date $year_cond union all select a.booking_no,a.issue_number_prefix_num as recv_number_prefix_num,c.job_no,a.id,b.booking_no_prefix_num from inv_issue_master a,wo_booking_mst b, fabric_sales_order_mst c where a.BOOKING_ID=b.id and b.booking_no=c.sales_booking_no and b.booking_type=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form in (318) and a.item_category=2 $company $location $booking_cond $booking_date $year_cond) x order by x.booking_no";


	echo create_list_view("tbl_list_search", "Booking_no,Job No,Sys. Challan", "120,130,140","600","240",0, $sql , "js_set_value", "id,booking_no_prefix_num,recv_number_prefix_num", "", 1, "0,0,0", $arr , "booking_no,job_no,recv_number_prefix_num", "",'','0,0,0','',1) ;
	echo "<input type='hidden' id='txt_selected_id' />";
	echo "<input type='hidden' id='txt_selected' />";
	echo "<input type='hidden' id='txt_sys_id' />";
	?>
	<script language="javascript" type="text/javascript">
	var booking_no='<? echo $txt_booking_no;?>';
	var booking_id='<? echo $txt_booking_id;?>';
	var sys_ids='<? echo $txt_sys_id;?>';
	if(booking_no!="")
	{
		booking_no_arr=booking_no.split(",");
		booking_id_arr=booking_id.split(",");
		sys_id_arr=sys_ids.split(",");
		var str_ref="";
		for(var k=0;k<booking_no_arr.length; k++)
		{
			str_ref=booking_no_arr[k]+'_'+booking_id_arr[k]+'_'+sys_id_arr[k];
...
if ($action=="dyingfinishing_delivery_list_view") 
{
	...
	$booking_no=$data[12];
	$update_id=$data[13];
	$str_data=trim($data[14]);
	//print_r($str_data);die;
//echo $batch_no.'DSDD';
	
  //echo $bill_on_batchorfab.'='.$inhouse_bill_from; 
 $composition_arr=array(); $constructtion_arr=array();
		...
		?>
			...
			unset($nameArray);
				...
				foreach($grey_sql_result as $row)
				{
					...
					//if($row[csf('batch_against')]==2) issue id=18791,For FPG, Assignnee-Emon
					//{
				...
				//$old_delivery_id=implode(',',explode('_',$data[3]));
				   //echo $bill_on_batchorfab.'DDDDDDDD=';die;
				...
				if($bill_on_batchorfab==1)
				{
					if($db_type==0)
					{
						if(!$update_id)
						{
							$sql="select a.delivery_prefix_num as challan_no, b.width_dia_type, a.delivery_date, b.sub_process_id, sum(b.carton_roll) as carton_roll,b.color_id, b.batch_id, b.order_id, group_concat(b.id SEPARATOR '_') as id, group_concat(b.process_id SEPARATOR '_') as process_id, group_concat(b.item_id SEPARATOR '_') as item_id, sum(b.gray_qty) as gray_qty, sum(b.delivery_qty) as delivery_qty, 0 as type from  subcon_delivery_mst a, subcon_delivery_dtls b where a.company_id='$data[0]' and b.bill_status=0 and a.id=b.mst_id and a.party_id='$data[3]' and b.process_id in (3,4,26) and a.status_active=1 and a.is_deleted=0 $sys_challan_cond $manual_challan_no_cond $batchIds_cond $date_cond group by a.delivery_prefix_num, b.width_dia_type, a.delivery_date, b.sub_process_id, b.color_id, b.batch_id, b.order_id"; 
						}
						else
						{
							$sql="(select a.delivery_prefix_num as challan_no, b.width_dia_type, a.delivery_date, b.sub_process_id,sum(b.carton_roll) as carton_roll, b.color_id, b.batch_id, b.order_id, group_concat(b.id SEPARATOR '_') as id, group_concat(b.process_id SEPARATOR '_') as process_id, group_concat(b.item_id SEPARATOR '_') as item_id,sum(b.gray_qty) as gray_qty, sum(b.delivery_qty) as delivery_qty, 0 as type from subcon_delivery_mst a, subcon_delivery_dtls b where a.company_id='$data[0]' and a.id=b.mst_id and a.party_id='$data[3]' and b.process_id in (3,4,26) and a.status_active=1 and b.bill_status=0 $sys_challan_cond $manual_challan_no_cond $batchIds_cond $date_cond group by a.delivery_prefix_num, b.width_dia_type, a.delivery_date,b.sub_process_id, b.color_id, b.batch_id, b.order_id)
							 union 
							 (select a.delivery_prefix_num as challan_no, b.width_dia_type, a.delivery_date,b.sub_process_id,sum(b.carton_roll) as carton_roll, b.color_id, b.batch_id, b.order_id, group_concat(b.id SEPARATOR '_') as id, group_concat(b.process_id SEPARATOR '_') as process_id, group_concat(b.item_id SEPARATOR '_') as item_id,sum(b.gray_qty) as gray_qty, sum(b.delivery_qty) as delivery_qty, 1 as type from subcon_delivery_mst a, subcon_delivery_dtls b where  a.company_id='$data[0]' and a.id=b.mst_id and a.party_id='$data[3]' and b.process_id in (3,4,26) and b.id in ($delv_id) and a.status_active=1 group by a.delivery_prefix_num, b.width_dia_type, a.delivery_date,b.sub_process_id, b.color_id, b.batch_id, b.order_id) order by type DESC";
						}
					}
					else if ($db_type==2)
					{
					...
					from  subcon_delivery_mst a, subcon_delivery_dtls b where a.company_id='$data[0]' and b.bill_status=0 and a.id=b.mst_id and a.party_id='$data[3]' and a.process_id in(4,26) and b.process_id in (3,4,26) and a.status_active=1 and a.is_deleted=0 $sys_challan_cond $manual_challan_no_cond $batchIds_cond $date_cond group by a.delivery_prefix_num, b.width_dia_type, a.delivery_date, b.sub_process_id, b.color_id, b.batch_id, b.order_id order by a.delivery_prefix_num"; 
						}
						else
						{
							$sql="(select a.delivery_prefix_num as challan_no, b.width_dia_type, a.delivery_date, b.sub_process_id,sum(b.carton_roll) as carton_roll, b.color_id, b.batch_id, b.order_id,
							...
							listagg(b.item_id,'_') within group (order by b.item_id) as item_id,sum(b.gray_qty) as gray_qty,
							sum(b.delivery_qty) as delivery_qty, 0 as type 
							from subcon_delivery_mst a, subcon_delivery_dtls b where a.company_id='$data[0]' and a.id=b.mst_id and a.party_id='$data[3]' and a.process_id in(26,4) and b.process_id in (3,4,26) and a.status_active=1 and b.bill_status=0 $sys_challan_cond $manual_challan_no_cond $batchIds_cond $date_cond group by a.delivery_prefix_num, b.width_dia_type, a.delivery_date,b.sub_process_id, b.color_id, b.batch_id, b.order_id)
							 union 
							...
							sum(b.delivery_qty) as delivery_qty, 1 as type 
							  from subcon_delivery_mst a, subcon_delivery_dtls b where a.company_id='$data[0]' and a.id=b.mst_id and a.party_id='$data[3]' and a.process_id in(4,26) and b.process_id in (3,4,26) and b.id in ($delv_id) and a.status_active=1 and b.bill_status=1 group by a.delivery_prefix_num, b.width_dia_type, a.delivery_date,b.sub_process_id, b.color_id, b.batch_id, b.order_id) order by type DESC";
						}
					}
				}
				else
				{
					...
					else if ($db_type==2)
					{
						...
						else
						{
							...
							from subcon_delivery_mst a, subcon_delivery_dtls b where a.company_id='$data[0]' and a.id=b.mst_id and a.party_id='$data[3]' and a.process_id in(4,26) and b.process_id in (3,4,26) and a.status_active=1 and b.bill_status=0 $sys_challan_cond $manual_challan_no_cond $batchIds_cond $date_cond group by a.delivery_prefix_num, b.width_dia_type, a.delivery_date,b.sub_process_id, b.color_id, b.batch_id, b.order_id, b.id, b.process_id, b.item_id)
							 union 
							 (select a.delivery_prefix_num as challan_no, b.width_dia_type, a.delivery_date,b.sub_process_id,sum(b.carton_roll) as carton_roll, b.color_id, b.batch_id, b.order_id,
	...
	else if($data[2]==1)
	{
		...
		{ 
			...
			if ($date_from!="" &&  $date_to!="") $date_cond_issue= "and a.issue_date between '".change_date_format($date_from,'yyyy-mm-dd')."' and '".change_date_format($date_to,'yyyy-mm-dd')."'"; else $date_cond_issue= "";
			$year_cond= "year(b.insert_date)as year";
		}
		else if ($db_type==2)
		{
			...
			if ($date_from!="" &&  $date_to!="") $date_cond_issue= "and a.issue_date between '".change_date_format($date_from, "", "",1)."' and '".change_date_format($date_to, "", "",1)."'";  else $date_cond_issue= "";
			$year_cond= "TO_CHAR(b.insert_date,'YYYY') as year";
		}
		if($ex_bill_for==4)
		{
		...
		?>
            ...
            <div>
                <table cellspacing="0" cellpadding="0" border="1" rules="all"  align="center" width="1130px" class="rpt_table">
                    <thead>
                    	<th width="30">&nbsp;</th>
                        <th width="30">SL</th>
                        ...
                        <th width="80"><?=$td_head;?>Order No</th>
            ...
            <div style="width:1130px;max-height:180px; overflow-y:scroll" id="sewing_production_list_view" align="center">
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1110px" class="rpt_table" id="tbl_list_search">
				<?
				//$inhouse_bill_from=2;
				//  echo $inhouse_bill_from.'=A';
				...
				else
				{
					...
					if($sys_challan!="") $sys_challan_cond_issue=" and a.issue_number_prefix_num in ($sys_challan)";
				}
				$booking_no_cond="";
				if($inhouse_bill_from==2) //Bill from delivery
				{
					if($booking_no!="") $booking_no_cond=" and w.booking_no_prefix_num in ($booking_no)";
				}
				else
				{
					if($booking_no!="") $booking_no_cond=" and e.booking_no_prefix_num in ($booking_no)";
				}
				...
				if($ex_bill_for!=3) 
				{
					...
					{		
						...
						if($data[2]==1){
							...
							if($ex_bill_for==4)
							{
								$sql="SELECT a.id, a.entry_form, a.issue_number as recv_number,a.issue_basis as receive_basis,a.issue_number as sys_number, a.issue_number_prefix_num as recv_number_prefix_num, a.challan_no, a.issue_date as receive_date, a.booking_id as bookingno, $year_cond as year,b.batch_id, b.prod_id, b.body_part_id,d.process_id as process_id, f.detarmination_id as fabric_description_id,d.color_id, b.width_type as dia_width_type, 
								sum(b.issue_qnty) as grey_receive_qnty,sum(c.quantity) as rec_qnty, sum(c.quantity) as grey_used_qty, sum(b.no_of_roll) as carton_roll, c.po_breakdown_id, d.booking_no_id, d.booking_no,1 as is_sales
								FROM inv_issue_master a, inv_finish_fabric_issue_dtls b, order_wise_pro_details c,product_details_master f, pro_batch_create_mst d,wo_booking_mst e 
								WHERE a.id=b.mst_id and b.id=c.dtls_id and f.id=c.prod_id and d.id=b.batch_id and a.booking_id=e.id and c.trans_type=2 and c.entry_form in (318)
								and c.trans_id!=0 and a.entry_form in (318) AND a.company_id=$data[3] AND a.location_id=$data[1] and c.quantity>0 and a.item_category=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.batch_against in (0,1,3) and $booking_without_order=0 $date_cond_issue $manual_challan_cond $sys_challan_cond_issue  $batchIds_cond $booking_no_cond 
								group by a.id, a.entry_form,a.issue_number, a.issue_basis, a.issue_number,a.issue_number_prefix_num, a.challan_no, a.issue_date, a.booking_id, a.insert_date, b.batch_id, b.prod_id, b.body_part_id, f.detarmination_id, d.color_id, b.width_type, c.po_breakdown_id, d.process_id,d.booking_no_id, d.booking_no order by a.issue_number_prefix_num DESC";
							}
							else
							{
								$sql="SELECT a.id, a.entry_form,c.recv_number,c.receive_basis,a.sys_number as sys_number, a.sys_number_prefix_num as recv_number_prefix_num, '' as challan_no, a.delevery_date as receive_date, d.booking_no_id as bookingno, $year_cond as year, b.batch_id, b.product_id as prod_id, b.bodypart_id as body_part_id, b.determination_id as fabric_description_id, b.color_id, b.width_type as dia_width_type,sum(f.grey_used_qty) as po_grey_used_qty,sum(b.current_delivery) as rec_qnty,sum(e.production_qty) as production_qty, sum((b.current_delivery+(f.process_loss_perc*b.current_delivery)/100)) as grey_used_qty, sum(e.roll_no) as carton_roll, f.po_breakdown_id as po_breakdown_id, d.booking_no_id, d.booking_no FROM pro_grey_prod_delivery_mst a, pro_grey_prod_delivery_dtls b, inv_receive_master c, pro_finish_fabric_rcv_dtls e, order_wise_pro_details f,pro_batch_create_mst d,wo_booking_mst w WHERE a.id=b.mst_id and b.grey_sys_id=c.id and d.id=b.batch_id and c.id=e.mst_id and e.id=f.dtls_id and e.id=b.sys_dtls_id and b.order_id=f.po_breakdown_id and d.booking_no_id=w.id and a.entry_form in (54,67) AND c.knitting_source=1 AND a.company_id=$data[3] $localtion_cond AND c.knitting_company=$data[0] and c.receive_basis in (0,2,4,5,9,11) and c.item_category=2 and c.entry_form in (7,66) and b.current_delivery>0  and f.entry_form in (7,66) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.batch_against in (0,1,2,3) and $booking_without_order=0 $date_cond $manual_challan_cond $sys_challan_cond $batchIds_cond $booking_no_cond group by a.id, a.entry_form,c.recv_number, c.receive_basis,a.sys_number, a.sys_number_prefix_num, a.delevery_date, c.booking_id, a.insert_date, b.batch_id, b.product_id, b.bodypart_id, b.determination_id, b.color_id, b.width_type, f.po_breakdown_id, d.booking_no_id, d.booking_no order by a.sys_number_prefix_num DESC";
						  	}
					}
					else //Bill from receive
					{
						if($ex_bill_for==4)
							{
								$sql="SELECT a.id, a.entry_form, a.recv_number as recv_number,a.receive_basis as receive_basis,a.recv_number as sys_number, a.recv_number_prefix_num as recv_number_prefix_num, a.challan_no, a.receive_date as receive_date, a.booking_id as bookingno, $year_cond as year,b.batch_id, b.prod_id, b.body_part_id,d.process_id as process_id, f.detarmination_id as fabric_description_id,d.color_id, b.dia_width_type as dia_width_type, 
								sum(b.receive_qnty) as grey_receive_qnty,sum(c.quantity) as rec_qnty, sum(c.grey_used_qty) as grey_used_qty, sum(b.no_of_roll) as carton_roll, c.po_breakdown_id, d.booking_no_id, d.booking_no,b.is_sales as is_sales
								FROM inv_receive_master a, pro_finish_fabric_rcv_dtls b, order_wise_pro_details c,product_details_master f, pro_batch_create_mst d,wo_booking_mst e 
								WHERE a.id=b.mst_id and b.id=c.dtls_id and f.id=c.prod_id and d.id=b.batch_id and d.booking_no=e.booking_no and c.trans_type=1 and c.entry_form in (7)
								 and a.entry_form in (7)  and c.entry_form in (7) AND a.company_id=$data[3] AND a.location_id=$data[1] and c.quantity>0 and a.item_category=2 and b.is_sales=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.batch_against in (0,1,2,3) and $booking_without_order=0 $date_cond $manual_challan_cond $manual_challan_cond  $batchIds_cond $booking_no_cond 
								group by a.id, a.entry_form,a.recv_number, a.receive_basis,a.recv_number_prefix_num, a.challan_no, a.receive_date, a.booking_id, a.insert_date, b.batch_id, b.prod_id, b.body_part_id, f.detarmination_id, d.color_id, b.dia_width_type, c.po_breakdown_id, d.process_id,d.booking_no_id, d.booking_no,b.is_sales order by a.recv_number_prefix_num DESC";
							}
							else
							{
								$sql="SELECT a.id, a.entry_form, a.recv_number,a.receive_basis,a.recv_number as sys_number, a.recv_number_prefix_num, a.challan_no, a.receive_date, a.booking_id as bookingno, $year_cond as year, b.batch_id, b.prod_id, b.body_part_id,b.process_id, b.fabric_description_id, b.color_id, b.dia_width_type, sum(b.receive_qnty) as grey_receive_qnty,sum(c.quantity) as rec_qnty, sum(b.grey_used_qty) as grey_used_qty, sum(c.grey_used_qty) as po_grey_used_qty,sum(b.no_of_roll) as carton_roll, c.po_breakdown_id, d.booking_no_id, d.booking_no FROM inv_receive_master a, pro_finish_fabric_rcv_dtls b, order_wise_pro_details c, pro_batch_create_mst d,wo_booking_mst e WHERE a.id=b.mst_id and b.id=c.dtls_id and d.id=b.batch_id and ( a.booking_id=e.id or d.booking_no_id=e.id) and c.trans_type=1 and c.entry_form in (7,37,66,68) and c.trans_id!=0 and a.entry_form in (7,37,66,68) AND a.knitting_source=1 AND a.company_id=$data[3] AND a.location_id=$data[1] and c.quantity>0  AND a.knitting_company=$data[0] and a.receive_basis in (2,4,5,9,11) and a.item_category=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.batch_against in (0,1,2,3) and $booking_without_order=0 $date_cond $manual_challan_cond $sys_challan_cond $batchIds_cond $booking_no_cond group by a.id, a.entry_form,a.recv_number, a.receive_basis, a.recv_number,a.recv_number_prefix_num, a.challan_no, a.receive_date, a.booking_id, a.insert_date, b.batch_id, b.prod_id, b.body_part_id, b.fabric_description_id, b.color_id, b.dia_width_type, c.po_breakdown_id,b.process_id, d.booking_no_id, d.booking_no order by a.recv_number_prefix_num DESC ";	
							}
					}
					  // echo $sql;  
				}
				else
				{
					...
					if($inhouse_bill_from==2) //Bill from delivery
					{
						 $sql="SELECT a.id, a.entry_form, c.receive_basis,c.recv_number, a.sys_number as sys_number,a.sys_number_prefix_num as recv_number_prefix_num , '' as challan_no, a.delevery_date as receive_date,  $year_cond as year, b.batch_id, b.product_id as prod_id, b.bodypart_id as body_part_id, b.determination_id as fabric_description_id, b.color_id, b.width_type as dia_width_type, sum(b.current_delivery) as rec_qnty, sum(e.grey_used_qty) as grey_used_qty,sum(e.production_qty) as production_qty, sum(b.roll) as carton_roll, null as po_breakdown_id, null as booking_no_id, d.booking_no as booking_no FROM pro_grey_prod_delivery_mst a, pro_grey_prod_delivery_dtls b, inv_receive_master c, pro_batch_create_mst d, pro_finish_fabric_rcv_dtls e WHERE a.id=b.mst_id and b.grey_sys_id=c.id and d.id=b.batch_id and c.id=e.mst_id and e.id=b.sys_dtls_id and a.entry_form in (54,67) AND c.knitting_source=1 AND a.company_id=$data[3] AND a.location_id=$data[1] AND c.knitting_company=$data[0] and c.receive_basis in (0,2,4,5,9,11) and c.item_category=2 and c.entry_form in (7,66) and b.current_delivery>0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.batch_against in (0,1,2,3) and $booking_without_order=1 $date_cond $manual_challan_cond $sys_challan_cond $batchIds_cond group by a.id, a.entry_form,c.recv_number, c.receive_basis,a.sys_number, a.sys_number_prefix_num, a.delevery_date, d.booking_no, a.insert_date, b.batch_id, b.product_id, b.bodypart_id, b.determination_id, b.color_id, b.width_type, b.order_id order by a.sys_number_prefix_num DESC";
					}
					else
					{
						$sql="SELECT a.id, a.entry_form, a.recv_number,a.receive_basis, a.recv_number as sys_number, a.recv_number_prefix_num, a.challan_no, a.receive_date, null as bookingno, $year_cond as year, b.batch_id, b.prod_id, b.body_part_id,b.process_id, b.fabric_description_id, b.color_id, b.dia_width_type, sum(b.receive_qnty) as rec_qnty,sum(b.grey_used_qty) as grey_used_qty, sum(b.no_of_roll) as carton_roll, 0 as po_breakdown_id, null as booking_no_id,d.booking_no FROM inv_receive_master a, pro_finish_fabric_rcv_dtls b, pro_batch_create_mst d WHERE a.id=b.mst_id and d.id=b.batch_id and a.entry_form in (7,37,66,68) and b.receive_qnty>0 AND a.knitting_source=1 AND a.company_id=$data[3] AND a.location_id=$data[1] AND a.knitting_company=$data[0] and a.receive_basis in(2,4,5,9,11) and d.batch_against in (3,5) and $booking_without_order=1 and a.item_category=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $date_cond $manual_challan_cond $sys_challan_cond $batchIds_cond group by a.id, a.entry_form,a.recv_number, a.receive_basis,  a.recv_number,a.recv_number_prefix_num, a.challan_no, a.receive_date, a.insert_date, b.batch_id, b.prod_id, b.body_part_id, b.fabric_description_id, b.color_id,b.process_id, b.dia_width_type,d.booking_no order by a.recv_number_prefix_num DESC";
					}
				}
			 	 // echo $sql; //die;
				$sql_result =sql_select($sql);
				...
				//print_r($sql_result);
				$batchidstr=""; $poid="";$salesid=""; $pordid="";
				foreach($sql_result as $row)
				{
					...
					$RecvIdArr[$row[csf('bookingno')]]=$row[csf('bookingno')];
					$pordid.=$row[csf('prod_id')].',';
					if($row[csf('is_sales')]==1)
					{
						$salesid.=$row[csf('po_breakdown_id')].',';
					}
					
					$recv_Qty_mrr_wiseArr[$row[csf('id')]][$row[csf('batch_id')]][$row[csf('body_part_id')]]+=$row[csf('rec_qnty')];
				}
				$poId_cond=where_con_using_array($poidArr,0,'b.po_break_down_id');
				$RecvId_cond=where_con_using_array($RecvIdArr,0,'id');
				$recive_basis_arr=return_library_array( "select id, receive_basis from inv_receive_master where status_active=1 $RecvId_cond",'id','receive_basis');
				
				$color_type_array=array();//wo_pre_cost_fab_conv_cost_dtls
				...
				$po_array=array();$sales_array=array();
				if($ex_bill_for!=3)// Po Filter
				{
					...
					if($ex_bill_for==4) 
					{
						$salesid=chop($salesid,",");
						$poSales_sql=sql_select( "select a.style_ref_no, a.job_no_prefix_num, a.po_buyer, a.id,a.po_job_no, b.job_no_mst from  fabric_sales_order_mst a,fabric_sales_order_dtls b where a.job_no=b.job_no_mst and a.id=b.mst_id and a.status_active!=0 and a.is_deleted=0 and b.status_active!=0 and b.is_deleted=0 and a.id in($salesid)");
						foreach($poSales_sql as $row)
						{
							$po_array[$row[csf('id')]]['job']=$row[csf('po_job_no')];
							$po_array[$row[csf('id')]]['style']=$row[csf('style_ref_no')];
							$po_array[$row[csf('id')]]['order']=$row[csf('job_no_mst')];
							$po_array[$row[csf('id')]]['buyer']=$row[csf('po_buyer')];
				...
				if($ex_bill_for==4)
					{
						$rate_array=array();
						 $rate_sql="select a.body_part_id,a.lib_yarn_count_deter_id,b.charge_unit from wo_pre_cost_fabric_cost_dtls a,wo_pre_cost_fab_conv_cost_dtls b where a.id=b.fabric_description  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.cons_process=31 $jobId_cond ";
					...
					//echo '<pre>';print_r($rate_array);die;
			...
			$sql="select id, item_category_id,product_name_details from product_details_master where status_active=1 and is_deleted=0 $pordidCond"; 
				...
				foreach($sql_result as $row) // for update row
				{
					...
					//	echo $row[csf('batch_id')].'=='.$dyesTotal.'TTTTTTTT'.$BatchTotal;
					// $percentageOfColor=($dyesTotal/$BatchTotal)*100;
					 $percentageOfColor=$dyesTotal;//Issue id=18350
					}
					
					if(in_array($all_value,$str_arr))
					{
						$booking_no=0; $independent=''; $bill_for_id=0; $bill_for_sb=0;$bill_for_id2=0;
						if ($row[csf('entry_form')]==7)
						{
							...
							if($ex_bill_for==1 || $ex_bill_for==4) { $bill_for_id="Fb"; $bill_for_id2="FB"; }
						...
						else if ($row[csf('entry_form')]==37)
						{
							if ($row[csf('receive_basis')]==9) 
							{
								...
								else $booking_no=$row[csf('booking_no')];
							}
							...
							if($row[csf('receive_basis')]==2 || $row[csf('receive_basis')]==11) 
								...
								$booking_no=$row[csf('booking_no')];
							else if($row[csf('receive_basis')]==9 && $recive_basis_arr[$row[csf('bookingno')]]=='') 
							$booking_no=$row[csf('booking_no')];
							else 
								$booking_no=0;
							if($ex_bill_for==1) { $bill_for_id="Fb";$bill_for_id2="FB"; $bill_for_sb="SB"; } else if($ex_bill_for==2 && $row[csf('receive_basis')]!=4) $bill_for_id="SM";  else if($ex_bill_for==3) $bill_for_id="SMN";
						}
						...
						else if ($row[csf('entry_form')]==318)
						{
						...
						if($inhouse_bill_from==2)
						{
							...
							if($row[csf('grey_used_qty')]) $row[csf('grey_used_qty')]=$row[csf('grey_used_qty')];else $row[csf('grey_used_qty')]=$row[csf('po_grey_used_qty')];
						...
						else
						{
							 ...
							 $recv_Qty_prop_per=$row[csf('rec_qnty')]/$row[csf('grey_receive_qnty')]*100;
							$recv_Qty_prop=($recv_Qty_prop_per/100)*$row[csf('grey_used_qty')];

							if ($variable_check==1) $on_bill_qty=$recv_Qty_prop;//$batch_qty_arr[$row[csf('batch_id')]][$row[csf('po_breakdown_id')]][$row[csf('prod_id')]]['batch_qnty'];
						...
						if($inhouse_bill_from==2) $grey_qty=$row[csf('grey_used_qty')]; else $grey_qty=$row[csf('grey_used_qty')];//$batch_qty_arr[$row[csf('batch_id')]][$row[csf('po_breakdown_id')]][$row[csf('prod_id')]]['batch_qnty'];
						 ...
						 if($ex_bill_for==4)
						{
							$po_no=$po_array[$row[csf('po_breakdown_id')]]['order'];
							$job_no=$po_array[$row[csf('po_breakdown_id')]]['job'];
							$buyer=$po_array[$row[csf('po_breakdown_id')]]['buyer'];
						...
						$str_val=$row[csf('id')].'_'.change_date_format($row[csf('receive_date')]).'_'.$row[csf('recv_number_prefix_num')].'_'.$row[csf('po_breakdown_id')].'_'.$po_no.'_'.$style.'_'.$buyer_arr[$buyer].'_'.$job_no.'_'.$row[csf('carton_roll')].'_'.$row[csf('body_part_id')].'_'.$body_part[$row[csf("body_part_id")]].'_'.$row[csf('fabric_description_id')].'_'.$row[csf('prod_id')].'_'.$des_composition.'_'.$row[csf('batch_id')].'_'.$row[csf('color_id')].'_'.$color_name[$row[csf("color_id")]].'_'.$batch_array[$row[csf('batch_id')]]['process_id'].'_'.$process_name.'_'.$row[csf('dia_width_type')].'_'.$fabric_typee[$row[csf('dia_width_type')]].'_'.number_format($on_bill_qty, 2, '.', '').'_'.$color_range_id.'_'.number_format($percentageOfColor,2, '.', '').'________';
						...
						if($independent==4)
						{
                            ...
                            <tr id="tr_<? echo $all_value; ?>" bgcolor="yellow" bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $all_value."***".'1'; ?>');" >
								...
								<td width="90" style="word-break:break-all"><? echo $style; ?></td>
								<td width="80" style="word-break:break-all"><p><? echo $po_no; ?></p></td>
						...
						{ 
							if( strtolower($ex_booking[1])==strtolower($bill_for_id) || strtolower($ex_booking[1])==strtolower($bill_for_id2)  || strtolower($ex_booking[1])==strtolower($bill_for_sb)) 
							{
				...
				//where_con_using_array($job_id_arr,0,'a.job_id')
				foreach($sql_result as $row) // for new row
				{
					$poIdArr[$row[csf('po_breakdown_id')]]=$row[csf('po_breakdown_id')];
				}
				//dyeing_fin_bill,allow_per
			$vari_Sql=sql_select("select dyeing_fin_bill,allow_per from variable_settings_subcon where company_id=".$data[3]."  and variable_list=3 and dyeing_fin_bill=5  and status_active=1 and is_deleted=0");
			foreach($vari_Sql as $row) // for new row 
			{
				$rate_from_budget=$row[csf('allow_per')];
		...
		//  echo $dyeing_fin_vari.'DDD';
			if($dyeing_fin_vari==5)
			{
				$process_id_all=" and f.cons_process in(31)";
				$poidCond=where_con_using_array($poIdArr,0,'e.po_break_down_id');
				$sql_conv="SELECT d.color_size_sensitive as COLOR_SIZE_SENSITIVE, d.BODY_PART_ID,d.lib_yarn_count_deter_id as DETER_ID, e.po_break_down_id as POID, e.color_number_id as COLOR_ID, f.id as CONV_ID, f.cons_process as CONS_PROCESS, f.charge_unit as CHARGE_UNIT, f.color_break_down as COLOR_BREAK_DOWN 
				
				from wo_pre_cost_fabric_cost_dtls d, wo_pre_cos_fab_co_avg_con_dtls e, wo_pre_cost_fab_conv_cost_dtls f  
				
				where d.id=e.pre_cost_fabric_cost_dtls_id and d.id=f.fabric_description    and e.cons !=0 and d.is_deleted=0 and d.status_active=1 and e.is_deleted=0 and e.status_active=1 and f.is_deleted=0 and f.status_active=1 $poidCond $process_id_all order by f.id, e.po_break_down_id asc"; 
				...
				//color_breck_down=$('#gmtscolorid_'+i).val()+'_'+trim(unitcharge)+'_'+coversionchargelibraryid+'_'+$('#fabriccolorid_'+i).val()+'_'+$('#cons_'+i).val();
						...
						for($ci=0;$ci<count($arr_1);$ci++)
						{
							...
							//$this->_rateArray[$id][$arr_2[0]][$arr_2[3]]=$arr_2[1];//BODY_PART_ID
							if($arr_2[1])
							{
								$po_color_fab_array[$val['POID']][$arr_2[3]][$val['DETER_ID']][$val['BODY_PART_ID']]['rate'] =$arr_2[1];
							}
					...
					{
						$po_color_fab_array[$val['POID']][$val['COLOR_ID']][$val['DETER_ID']][$val['BODY_PART_ID']]['rate'] =$val['CHARGE_UNIT'];
					}
					else
					{
						$po_color_fab_array[$val['POID']][$val['COLOR_ID']][$val['DETER_ID']][$val['BODY_PART_ID']]['rate'] = $val['CHARGE_UNIT'];
					...
					$poidCond=where_con_using_array($poIdArr,0,'b.id');
					//--------------------------------------------------------------------------------------------------
					...
					WHERE     1 = 1
							...
							and g.fabric_description=d.id AND a.id = b.job_id  AND a.id = c.job_id AND a.id = d.job_id AND a.id = e.job_id
							...
							AND b.status_active = 1  AND c.is_deleted = 0 AND c.status_active = 1 AND d.status_active = 1 AND d.is_deleted = 0 $poidCond
				...
				foreach($sql_result as $row) // for new row
				{
					...
					else if ($row[csf('entry_form')]==37)
					{
						if ($row[csf('receive_basis')]==9)  
						{
							...
							//echo $recive_basis_arr[$row[csf('bookingno')]].'=AAAAA';die;
						}
						//echo $booking_no.'=A';die;
						if($ex_bill_for==1) { if($row[csf('receive_basis')]==4) $independent=4; }// else $independent='';
						if($row[csf('receive_basis')]==2 || $row[csf('receive_basis')]==11) 
							$booking_no=$row[csf('booking_no')];
						else if($row[csf('receive_basis')]==9 && $recive_basis_arr[$row[csf('bookingno')]]==5) 
						...
						if($ex_bill_for==1) { $bill_for_id="Fb";$bill_for_id2="FB"; $bill_for_sb="SB"; } 
						else if($ex_bill_for==2 && $row[csf('receive_basis')]!=4) $bill_for_id="SM"; 
						else if($ex_bill_for==3) $bill_for_id="SMN";
						 // echo $booking_no.'='.$bill_for_id.'='.$row[csf('receive_basis')].'<br>';
					...
					$on_bill_qty=0;
					 //	echo $inhouse_bill_from.'=A';die;
					if($inhouse_bill_from==2)
					{
						$grey_issue=$row[csf('production_qty')];
						$grey_qty=$row[csf('grey_used_qty')];
						if($grey_qty) $grey_qty=$grey_qty;else $grey_qty=$row[csf('po_grey_used_qty')];
					...
					{
						$tot_recv_Qty=$recv_Qty_mrr_wiseArr[$row[csf('id')]][$row[csf('batch_id')]][$row[csf('body_part_id')]];
						$recv_Qty_prop_per=$row[csf('rec_qnty')]/$row[csf('grey_receive_qnty')]*100;
						$recv_Qty_prop=($recv_Qty_prop_per/100)*$row[csf('grey_used_qty')];
						if ($variable_check==1) $on_bill_qty=$recv_Qty_prop;
						else $on_bill_qty=$row[csf('rec_qnty')];
					}
					$color_range_id=$batch_array[$row[csf('batch_id')]]['color_range_id'];
					$BatchTotal 				= $batchTotal_arr[$row[csf('batch_id')]]['batch_ratio_total'];
					...
					$avilable_qty=$on_bill_qty-$bill_qty;
					
					$receive_date=$row[csf('receive_date')];
				   $conversion_date=change_date_format($receive_date, "d-M-y", "-",1);
			 		$currency_rate=set_conversion_rate(2,$conversion_date,$data[3]);
					 $conv_rate=$po_color_fab_array[$row[csf('po_breakdown_id')]][$row[csf("color_id")]][$row[csf("fabric_description_id")]][$row[csf('body_part_id')]]['rate'];
					 $fso_rate=$rate_array[$row[csf('body_part_id')]][$row[csf('fabric_description_id')]]['rate'];
					// echo $conversion_date.'='.$currency_rate.', ';
					...
					if($ex_bill_for==4)
					{
						...
						if($conv_rate)
						{
							$bdt_rate=$conv_rate*$currency_rate;
						}
					}
					...
					if($ex_bill_for==4)
					{
					 ...
					 $des_composition=$composition_arr[$row[csf('fabric_description_id')]];
					//  echo $bdt_rate.'='.$currency_rate.'<br>';
					/*if($ex_bill_for==4 && $row[csf('is_sales')]==1)
					{
						$str_val=$row[csf('id')].'_'.change_date_format($row[csf('receive_date')]).'_'.$row[csf('recv_number_prefix_num')].'_'.$row[csf('po_breakdown_id')].'_'.$sales_array[$row[csf('po_breakdown_id')]]['order'].'_'.$sales_array[$row[csf('po_breakdown_id')]]['style'].'_'.$buyer_arr[$sales_array[$row[csf('po_breakdown_id')]]['buyer']].'_'.$sales_array[$row[csf('po_breakdown_id')]]['job'].'_'.$row[csf('carton_roll')].'_'.$row[csf('body_part_id')].'_'.$body_part[$row[csf("body_part_id")]].'_'.$row[csf('fabric_description_id')].'_'.$row[csf('prod_id')].'_'.$des_composition.'_'.$row[csf('batch_id')].'_'.$row[csf('color_id')].'_'.$color_name[$row[csf("color_id")]].'_'.$fin_processId_array[$row[csf('batch_id')]][$row[csf('body_part_id')]]['process_id'].'_'.$process_name.'_'.$row[csf('dia_width_type')].'_'.$fabric_typee[$row[csf('dia_width_type')]].'_'.number_format($on_bill_qty, 2, '.', '').'_'.$color_range_id.'_'.number_format($percentageOfColor,2, '.', '').'_'.number_format($unitCharge,2, '.', '').'_________'.'_'.number_format($bdt_rate,2, '.', '');
					}
					else
					{*/
						$str_val=$row[csf('id')].'_'.change_date_format($row[csf('receive_date')]).'_'.$row[csf('recv_number_prefix_num')].'_'.$row[csf('po_breakdown_id')].'_'.$po_no.'_'.$style.'_'.$buyer_arr[$buyer].'_'.$job_no.'_'.$row[csf('carton_roll')].'_'.$row[csf('body_part_id')].'_'.$body_part[$row[csf("body_part_id")]].'_'.$row[csf('fabric_description_id')].'_'.$row[csf('prod_id')].'_'.$des_composition.'_'.$row[csf('batch_id')].'_'.$row[csf('color_id')].'_'.$color_name[$row[csf("color_id")]].'_'.$fin_processId_array[$row[csf('batch_id')]][$row[csf('body_part_id')]]['process_id'].'_'.$process_name.'_'.$row[csf('dia_width_type')].'_'.$fabric_typee[$row[csf('dia_width_type')]].'_'.number_format($on_bill_qty, 2, '.', '').'_'.$color_range_id.'_'.number_format($percentageOfColor,2, '.', '').'_'.number_format($unitCharge,2, '.', '').'_________'.'_'.number_format($bdt_rate,2, '.', '');
					//}
					//echo $str_val.'<br>';
					if($independent==4)
					{
						if($avilable_qty>0)
						{
                            ...
                            <tr id="tr_<? echo $all_value; ?>" bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $all_value."***".'1'; ?>');" >
								...
								<td width="120" style="word-break:break-all"><? echo $des_composition; ?></td>
								<td width="70" style="word-break:break-all"><? echo $fabric_typee[$row[csf('dia_width_type')]]; ?></td>
								<td width="60" align="right"  title="Per%=<?=$row[csf('rec_qnty')].'/'.$row[csf('grey_receive_qnty')].'*100';?>"><? echo number_format($on_bill_qty,2,'.',''); ?></td>
								<td align="right"><? echo number_format($row[csf('rec_qnty')],2,'.',''); ?>
                                <input type="hidden" id="strid<? echo $i; ?>" value="<? echo $str_val; ?>">
								<input type="hidden" id="currid<? echo $all_value; ?>" style="width:50px" value="<? echo '1'; ?>"></td>
							</tr>
					...
					{
						// echo $avilable_qty.'='.strtolower($ex_booking[1]).'='.strtolower($bill_for_id).'='.strtolower($bill_for_id2);die;
						//echo $re_batch_chk_array[$row[csf('batch_id')]].'DDSS';bill_for_id2
							...
							if($avilable_qty>0)
							{
								if($re_batch_chk_array[$row[csf('batch_id')]])
								{
									...
									<tr id="tr_<? echo $all_value; ?>" bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $all_value."***".'1'; ?>');" >
	                                    ...
	                                    <td width="30"><? echo $batch_array[$row[csf('batch_id')]]['extention_no']; ?></td>
	                                    <td width="100" style="word-break:break-all"><? echo $process_name; ?>&nbsp;</td>
	                                    <td width="120" style="word-break:break-all" title="<?=$body_part[$row[csf('body_part_id')]];?>"><? echo  $des_composition; ?></td>
	                                    <td width="70" style="word-break:break-all"><? echo $fabric_typee[$row[csf('dia_width_type')]]; ?></td>
	                                    <td width="60" align="right"  title="Per%=<?=$row[csf('rec_qnty')].'/'.$row[csf('grey_receive_qnty')].'*100';?>"><? echo number_format($on_bill_qty,2,'.',''); //$recv_Qty_prop_per=$row[csf('rec_qnty')]/$row[csf('grey_receive_qnty')]*100;
										//$recv_Qty_prop=($recv_Qty_prop_per/100)*$row[csf('grey_used_qty')];?></td>
	                                    <td align="right"><? echo number_format($row[csf('rec_qnty')],2,'.',''); ?>
...
if ($action=="load_dtls_data") 
{
	...
	if($source==1)
	{
		...
		if($salesid!="") 
		{
			$poSales_sql=sql_select( "select a.style_ref_no, a.job_no_prefix_num, a.po_buyer, a.id, b.job_no_mst from  fabric_sales_order_mst a,fabric_sales_order_dtls b where a.job_no=b.job_no_mst and a.id=b.mst_id and a.status_active!=0 and a.is_deleted=0 and b.status_active!=0 and b.is_deleted=0 and a.id in($salesid)");
			foreach($poSales_sql as $row)
			{
				$job_order_arr[$row[csf('id')]]['job']=$row[csf('job_no_prefix_num')];
				$job_order_arr[$row[csf('id')]]['style']=$row[csf('style_ref_no')];
				$job_order_arr[$row[csf('id')]]['po']=$row[csf('job_no_mst')];
				$job_order_arr[$row[csf('id')]]['buyer']=$row[csf('po_buyer')];
			}
			unset($poSales_sql);
		}
		else
		...
		if($control_with==1)
		{
			if($bill_forId!=3)
			{
				if($party_sourceId==1)
				{
					/*if($bill_forId==4)
					{
						$sql_po=sql_select("select b.id, b.job_no_mst,a.exchange_rate from fabric_sales_order_mst c,wo_po_break_down b,wo_pre_cost_mst a where  c.po_job_no=b.job_no_mst and b.job_id= a.job_id and a.status_active=1 and b.status_active=1 and  c.id in($order_ids) ");
						$fsoToPoIds="";
						foreach($sql_po as $row)
						{
							$job_arr[$row[csf('id')]]=$row[csf('job_no_mst')];
							$job_exchnage_arr[$row[csf('job_no_mst')]]=$row[csf('exchange_rate')];
							$fsoToPoIds.=$row[csf('id')].",";
						}
						$fsoToPoIds=chop($fsoToPoIds,",");
						$condition = new condition();
						if($fsoToPoIds!='' || $fsoToPoIds!=0)
						{
							$condition->po_id_in($fsoToPoIds); 
						}
					}
					else
					{*/
	...
	if ($operation==0)   // Insert Here========================================================================================
	{
		...
		for($i=1; $i<=$tot_row; $i++)
		{
			...
			{ 
				...
				$data_array1.="(".$id1.",".$id.",".$$delivery_id.",".$$delevery_date.",".$$challen_no.",".$$orderid.",".$$item_id.",".$$compoid.",".$$diaType.",".$$batchid.",".$$bodypartid.",".$$add_process.",".$$txt_add_process.",".$$number_roll.",".$$quantity.",".$$libRateId.",".$$rateDataString.",".$$rate.",".$$libAddRateId.",".$$addRate.",".$tot_amount.",".$$remarks.",".$$cbocolorrangeid.",".$$txtshadeper.",".$$curanci.",".$processId.",".$$color_id.",".$is_sales.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				$id1=$id1+1;
				$add_comma++;
            ...
            <tr bgcolor="#FFFFAA" style="font-size:12px"> 
             ...
             <tr bgcolor="#FFFFAA" style="font-size:12px"> 
				...
				$net_total_amount=($total_amount+$upcharge)-$discount;
				 echo  number_format($net_total_amount,2,'.',','); ?>&nbsp;</strong></td>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
			</tr>
           <tr>
               <td colspan="20" align="left"><b>In Word: <? echo number_to_words(number_format($net_total_amount,2),$currency[$carrency_id],$paysa_sent); ?></b></td>
           </tr>
        </table>
		 ...
		 <?
            ...
            <tr bgcolor="#FFFFAA" style="font-size:12px"> 
             ...
             <tr bgcolor="#FFFFAA" style="font-size:12px"> 
                ...
                <td align="right"><strong><?
				$net_total_amount=($total_amount+$upcharge)-$discount;
				$netAmount=number_format($net_total_amount,4,'.',',');
           ...
           <tr>
               <td colspan="19" align="left"><b>In Word: <? echo number_to_words($netAmount,$currency[$carrency_id],$paysa_sent); ?></b></td>
           </tr>
        </table>
        <table width="930" align="center" > 
        	<tr><td colspan="2">&nbsp;</td> </tr>
