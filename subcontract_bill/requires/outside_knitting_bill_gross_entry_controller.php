...
if($action=="create_booking_search_list_view")
{
	...
	$sql= "select a.booking_no,a.recv_number_prefix_num,b.job_no,a.id,b.booking_no_prefix_num from inv_receive_master a,pro_grey_prod_entry_dtls c,wo_booking_mst b where a.id=c.mst_id  and a.booking_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  and a.knitting_source=3 and a.entry_form in (2,22) and a.item_category=13 and a.receive_basis in (0,1,2,4,9,11) and c.trans_id!=0 and b.booking_type=1 $company $location $booking_cond $booking_date $year_cond order by  a.booking_no";
...
if ($action == "knitting_entry_list_view") {
	...
	//print_r ($ex_bill_for);
	if ($ex_bill_for == 3) $tbl_wight = "820";
	else $tbl_wight = "1200";
	?>
 ...
 */
					...
					order by recv_number_prefix_num Desc";*/
						...
						$booking_id_conds = "";
						if ($booking_id) {
							$booking_id_conds = " and e.booking_no_prefix_num in ($booking_id)";
						}
...
//echo $ex_bill_for.'=A';die;
			if($ex_bill_for==4) //========Fso======
			{
				$sql = "select a.id, 0 as grey_sys_id, a.recv_number_prefix_num, a.challan_no, a.receive_date, a.entry_form, b.prod_id, b.body_part_id,b.uom, b.febric_description_id, sum(b.no_of_roll) as roll_qty, c.po_breakdown_id, sum(c.quantity) as quantity,sum(c.quantity_pcs) as quantity_pcs, a.receive_basis, a.booking_no, a.booking_id, null as dtls_id,c.is_sales,e.po_job_no,e.job_no,e.sales_booking_no,e.style_ref_no,e.po_buyer from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c,fabric_sales_order_mst e
					where a.id=b.mst_id and b.id=c.dtls_id and e.id=c.po_breakdown_id and b.prod_id=c.prod_id  and c.po_breakdown_id=b.order_id and a.knitting_source=3 and a.company_id='$data[0]' and a.knitting_company='$data[3]' and a.location_id='$data[1]' and c.trans_type=1 and a.entry_form in (2,22) and c.entry_form in (2,22) and a.item_category=13 and a.receive_basis in (0,1,2,4,9,11) and c.trans_id!=0 and $booking_without_order=0
					and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $date_cond $po_breakdown_id_conds $booking_id_conds
					group by a.id, a.recv_number_prefix_num, a.challan_no, a.receive_date, a.entry_form, b.prod_id, b.body_part_id,b.uom, b.febric_description_id, c.po_breakdown_id, c.is_sales,a.receive_basis, a.booking_no, a.booking_id,e.style_ref_no,e.po_job_no,e.job_no,e.sales_booking_no,e.po_buyer order by a.recv_number_prefix_num Desc";
					//echo $sql;die;
			}
			else{
				$sql = "select a.id, 0 as grey_sys_id, a.recv_number_prefix_num, a.challan_no, a.receive_date, a.entry_form, b.prod_id, b.body_part_id,b.uom, b.febric_description_id, sum(b.no_of_roll) as roll_qty, c.po_breakdown_id, sum(c.quantity) as quantity,sum(c.quantity_pcs) as quantity_pcs, a.receive_basis, a.booking_no, a.booking_id, null as dtls_id from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c,wo_booking_mst e
					where a.id=b.mst_id and b.id=c.dtls_id and a.booking_id=e.id and a.knitting_source=3 and a.company_id='$data[0]' and a.knitting_company='$data[3]' and a.location_id='$data[1]' and c.trans_type=1 and a.entry_form in (2,22) and c.entry_form in (2,22) and a.item_category=13 and a.receive_basis in (0,1,2,4,9,11) and c.trans_id!=0 and $booking_without_order=0
					and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $date_cond $po_breakdown_id_conds $booking_id_conds
					...
					a.booking_no, a.booking_id, e.grey_sys_id*/
						...
						foreach ($sql_result as $row) // for update row
						{
							...
							$roll_idArr[$row[csf('dtls_id')]]=$row[csf('dtls_id')];
						}
						$prodId_Cond= where_con_using_array($prod_idArr,0,' id');
						$RollId_Cond=where_con_using_array($prod_idArr,0,' id');
						$item_id_arr = return_library_array("select id,product_name_details from  product_details_master where status_active=1 $prodId_Cond", 'id', 'product_name_details');
				     	$roll_no_arr = return_library_array("select id, no_of_roll from pro_grey_prod_entry_dtls where status_active=1 $RollId_Cond", 'id', 'no_of_roll');
							...
							if (in_array($all_value, $str_arr)) {
								 ...
								 else if ($row[csf('entry_form')] == 22) //Receive
								{
									...
									else if ($row[csf('receive_basis')] == 9) //Production Receive
									{
										...
										else if ($rec_basis == 2 || $is_sales==1) //plan
										{
									...
									if ($ex_bill_for == 1) {
										$bill_for_id = "FB";
										$bill_for_sb = "SB";$bill_for_sb2 = "KSB";
									} 
									else if ($ex_bill_for == 2 && $row[csf('receive_basis')] != 4) $bill_for_id = "SM";
									else if ($ex_bill_for == 4 ) $booking_no = $row[csf('sales_booking_no')];$bill_for_id = "Fb";
								...
								if ($is_sales == 1) //========If Sales ============
								{
									...
									$int_ref_no=$po_array[$row[csf('po_breakdown_id')]]['grouping'];
					...
					?>
									<tr id="tr_<? echo $all_value; ?>" bgcolor="yellow" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $all_value; ?>','<? echo $i; ?>');">
										...
										<td width="100" style="word-break:break-all"><? echo $int_ref_no; ?></td>
										<td style="word-break:break-all" align="center"><? echo $buyer_arr[$buyerId]; ?>
											<input type="hidden" id="strid<? echo $i; ?>" value="<? echo $str_val; ?>">
											<input type="hidden" id="currid<? echo $all_value; ?>" value="<? echo '1'; ?>">
										</td>
									...
									//echo "A,";
									if (strtolower($ex_booking[1]) == strtolower($bill_for_id) || strtolower($ex_booking[1]) == strtolower($bill_for_sb) || strtolower($ex_booking[1]) == strtolower($bill_for_sb2)) {
										if ($i % 2 == 0) $bgcolor = "#E9F3FF";
										else $bgcolor = "#FFFFFF";
									?>
										<tr id="tr_<? echo $all_value; ?>" bgcolor="yellow" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $all_value; ?>','<? echo $i; ?>');">
						...
						foreach ($sql_result as $row) // for new row
						{
							...
							$bill_for_sb = 0;	$bill_for_sb2 = 0;
							$is_sales=$row[csf('is_sales')];
						//	echo $row[csf('entry_form')].'='.$is_sales.'=A';die;
							if ($row[csf('entry_form')] == 2) //Production
							{
								if ($ex_bill_for == 1) {
									if ($row[csf('receive_basis')] == 0) $independent = 4;
							...
							else if ($row[csf('entry_form')] == 22) //Receive
							{
								...
								else if ($ex_bill_for == 2 && $independent != 4) $bill_for_id = "SM";
								else if ($ex_bill_for == 4 && $is_sales == 1) $booking_no = $row[csf('sales_booking_no')];$bill_for_id = "Fb";
								...
								else if ($ex_bill_for == 4) $bill_for_id = "Fb";
							...
							if ($independent == 4) {
								...
								// echo "B=".$ex_booking[1].'='.$bill_for_id.'='.$avilable_qty.', ';
								if (strtolower($ex_booking[1]) == strtolower($bill_for_id) || strtolower($ex_booking[1]) == strtolower($bill_for_sb) || strtolower($ex_booking[1]) == strtolower($bill_for_sb2)) {
									if ($avilable_qty > 0) {
										if ($i % 2 == 0) $bgcolor = "#E9F3FF";
										else $bgcolor = "#FFFFFF";
									?>
										<tr id="tr_<? echo $all_value; ?>" bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $all_value; ?>','<? echo $i; ?>');">
											...
											<td width="100"><? echo $job_no; ?></td>
											<td width="100" style="word-break:break-all"><? echo $int_ref_no; ?></td>
											<td style="word-break:break-all"><? echo $buyer_arr[$buyerId]; ?>
												<input type="hidden" id="strid<? echo $i; ?>" value="<? echo $str_val; ?>">
												<input type="hidden" id="currid<? echo $all_value; ?>" value="<? echo '1'; ?>">
											</td>
											<input type="hidden" id="checkAllId<? echo $i; ?>" value="<? echo $all_value; ?>">
						...
						foreach ($sql_result as $row) // for update row
						{
							...
							if (in_array($all_value, $str_arr)) {
								...
								if ($row[csf('entry_form')] == 2) {
									...
									if ($row[csf('receive_basis')] == 2 || $row[csf('receive_basis')] == 11) $booking_no = $row[csf('booking_no')];
									else $booking_no = 0;
									if ($ex_bill_for == 1) {
										$bill_for_id = "Fb";
										$bill_for_sb = "SB";
								...
								if ($independent == 4) {
									...
									<tr id="tr_<? echo $all_value; ?>" bgcolor="yellow" style="text-decoration:none; cursor:pointer" onClick="js_set_value('<? echo $all_value; ?>','<? echo $i; ?>');">
										...
										<td width="160">
											<p><? echo $item_id_arr[$row[csf('prod_id')]]; ?></p>
										</td>
										<td width="60">
											<p><? //echo $color_type[$color_type_array[$row[csf('po_breakdown_id')]][$row[csf('febric_description_id')]]['color_type']]; 
												?></p>
										</td>
			...
			<table width="800">
                ...
                <td colspan="6" align="center">
                    <input type="checkbox" name="checkall" id="checkall" class="formbutton" value="2" onClick="check_all_data();" style="width:100px" />Check All
                </td>
				<td colspan="6" align="center">
				...
				<tr style="border:none">
					<td colspan="3" align="center" align="center"><span class="formbutton" style="width:200px"> <input type="checkbox" name="checkall" id="checkall" class="formbutton" value="2" onClick="check_all_data_view();" /><b>Check all</b> </span></td>
					<td colspan="9" align="center"><input type="button" id="show_button" align="middle" class="formbutton" style="width:100px" value="Close" onClick="window_close(0);" /></td>
					</tr>
			</table>
		</div>
	</body>
...
if ($action=="load_dtls_data")  
{
   ...
   // $item_id_arr=return_library_array( "select id,product_name_details from  product_details_master",'id','product_name_details');
	...
	if ($bill_for_id == 4) //========FSO================ subcon_inbound_bill_dtls
		{
			 $sql_order_sales = "Select b.id, b.job_no as po_number,b.po_buyer as buyer_name, b.po_job_no,b.style_ref_no from subcon_outbound_bill_dtls a, fabric_sales_order_mst b where a.order_id=b.id  and a.mst_id='$data' and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1";
			...
			foreach ($sql_order_sales_result as $row) {
				...
				$po_array[$row[csf('id')]]['grouping']='';
			}
			unset($sql_order_result);
			//print_r($po_array);
		}
		if ($bill_for_id == 2 || $bill_for_id == 1)
		{
			$po_array=array();
	...
	foreach ($sql_result_arr as $row)
	{
		...
		$prod_idArr[$row[csf('prod_id')]]=$row[csf('prod_id')];
		 //$roll_idArr[$row[csf('prod_id')]]=$row[csf('dtls_id')];
	}

	 
	$prodId_Cond= where_con_using_array($prod_idArr,0,'id');
	 $RollId_Cond=where_con_using_array($prod_idArr,0,'prod_id');
	$item_id_arr = return_library_array("select id,product_name_details from  product_details_master where status_active=1 $prodId_Cond", 'id', 'product_name_details');
	$roll_no_arr = return_library_array("select id, no_of_roll from pro_grey_prod_entry_dtls where status_active=1 $prodId_Cond", 'id', 'no_of_roll');
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   // Insert Here========================================================================================receive_id
	{
		...
		$field_array_up ="receive_id*receive_date*challan_no*order_id*item_id*body_part_id*febric_description_id*roll_no*receive_qty*rec_qty_pcs*rate*amount*remarks*currency_id*is_sales*updated_by*update_date";
		$add_comma=0;
		for($i=1; $i<=$tot_row; $i++)
		{
			...
			else
			{
				...
				$data_array_up[str_replace("'",'',$$update_id_dtls)] =explode("*",("".$$receive_id."*".$$receive_date."*".$$challen_no."*".$$orderid."*".$$item_id."*".$$bodyPartId."*".$$febDescId."*".$$number_roll."*".$$wo_number."*".$$quantity."*".$$txtqntypcs."*".$$cbo_uom."*".$$rate."*".$$amount."*".$$remarks."*".$bill_process_id."*".$is_sales."*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'"));
	...
	else if ($operation==1)   // Update Here=============================================================================
	{
		...
		$id1=return_next_id( "id","subcon_outbound_bill_dtls",1);
		$field_array1 ="id, mst_id, receive_id, receive_date, challan_no, order_id, item_id, body_part_id, febric_description_id, roll_no, wo_num_id, receive_qty, rec_qty_pcs, uom, rate, amount, remarks, process_id,is_sales, inserted_by, insert_date, currency_id";
		$field_array_up ="receive_id*receive_date*challan_no*order_id*item_id*body_part_id*febric_description_id*roll_no*wo_num_id*receive_qty*rec_qty_pcs*uom*rate*amount*remarks*process_id*is_sales*currency_id";
	  	$add_comma=0;
		for($i=1; $i<=$tot_row; $i++)
	  	{
			$receive_date="txtReceiveDate_".$i;
			...
			else
			{
				...
				$data_array_up[str_replace("'",'',$$update_id_dtls)] =explode("*",("".$$reciveid."*'".$change_date."'*".$$challen_no."*".$$orderid."*".$$item_id."*".$$bodyPartId."*".$$febDescId."*".$$number_roll."*".$$wo_number."*".$$quantity."*".$$txtqntypcs."*".$$cbo_uom."*".$$rate."*".$$amount."*".$$remarks."*".$bill_process_id."*".$is_sales."*".$$curanci.""));
			}
		}
			...
			and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $barcode_nosCond";
				...
				?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="font-size:16px">
						...
						<td align="center" style="word-break:break-all"><? echo $order_array[$row[csf('po_job_no')]]['job'];?></td>
