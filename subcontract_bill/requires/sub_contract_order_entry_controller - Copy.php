...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)
	{
		...
		$field_array3="id, mst_id, order_id, job_no_mst, item_mst_id, color_mst_id, size_mst_id, item_id, color_id, size_id, qnty, rate, amount, excess_cut, plan_cut, process_loss, gsm, grey_dia, finish_dia, embellishment_type, description, dia_width_type, status_active, is_deleted";
		...
		$hidden_color=explode("*",str_replace("'",'',$hidden_color));           $hidden_gsm=explode("*",str_replace("'",'',$hidden_gsm));
		...
		$hidden_grey_dia=explode("*",str_replace("'",'',$hidden_grey_dia));     $hidden_finish_dia=explode("*",str_replace("'",'',$hidden_finish_dia));
	...
	else if ($operation==1)
	{
		...
		$size_library_arr=return_library_array( "select id,size_name from lib_size where status_active=1 and is_deleted=0", "id", "size_name");
		$field_array3="id, mst_id, order_id, job_no_mst, item_mst_id, color_mst_id, size_mst_id, item_id, color_id, size_id, qnty, rate, amount, excess_cut, plan_cut, process_loss, gsm, grey_dia, finish_dia, embellishment_type, description, dia_width_type, status_active, is_deleted";
		$field_array_up="item_mst_id*color_mst_id*size_mst_id*item_id*color_id*size_id*qnty*rate*amount*excess_cut*plan_cut*process_loss*gsm*grey_dia*finish_dia*embellishment_type*description*dia_width_type*updated_by*update_date";
		...
		for ($i=0;$i<count($hidden_item);$i++)
		{
			...
			if($hidden_tbl_id_break[$i]!="")
			{
				...
				//$data_array_up[str_replace("'",'',$hidden_tbl_id_break[$i])] =explode(",",("'".$hidden_item[$i]."','".$color_tbl_id."','".$size_tbl_id."','".$hidden_qnty[$i]."','".$hidden_rate[$i]."','".$hidden_amount[$i]."','".$hidden_excess_cut[$i]."','".ceil($hidden_plan_cut[$i])."','".$hidden_loss[$i]."','".$hidden_gsm[$i]."','".$hidden_grey_dia[$i]."','".$hidden_finish_dia[$i]."','".$hidden_embelishment_type[$i]."','".$hidden_description[$i]."','".$hidden_diawidth_type[$i]."'"));
		...
		{
			//$brk_data_array_up="";
			$brk_data_array_up="'".$val['item_mst_id']."'*'".$val['color_mst_id']."'*'".$val['size_mst_id']."'*'".$val['hidden_item']."'*'".$val['color_tbl_id']."'*'".$val['size_tbl_id']."'*'".$val['hidden_qnty']."'*'".$val['hidden_rate']."'*'".$val['hidden_amount']."'*'".$val['hidden_excess_cut']."'*'".$val['hidden_plan_cut']."'*'".$val['hidden_loss']."'*'".$val['hidden_gsm']."'*'".$val['hidden_grey_dia']."'*'".$val['hidden_finishdia']."'*'".$val['hidden_embelishment_type']."'*'".$val['hidden_description']."'*'".$val['hidden_diawidth_type']."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
...
if ($action=="load_php_data_to_form_dtls")
{
	...
	$qry_result=sql_select( "select id,mst_id,order_id,item_id,color_id,size_id,qnty,rate,amount,excess_cut,plan_cut,process_loss,grey_dia,finish_dia,gsm,dia_width_type from subcon_ord_breakdown where order_id='$data'");
...
if($action=="order_qnty_popup")
{
	...
	<script>
		...
		function add_share_row( i )
		{
			...
			if(mst_id > 0){
			$('#txtorderquantity_'+i).removeAttr("onKeyUp").attr("onClick","openmypage_qnty("+val+");");
			}
		...
		function fnc_close()
		{
			...
			var descriptions=""; var emblishment_type="";
			...
			var process_loss="";var hidden_id=""; var txt_gsm = "";
			var total_qnty="";var total_rate="";var total_amount="";var txt_greydia =""; var txt_finishdia = "";
			var txtdiawidth = ""; var colorbreakdown = ""; 
			var process_id=document.getElementById('hidden_process_id').value;
            ...
            <form name="qntypopup_1"  id="qntypopup_1" autocomplete="off">
                ...
                <tbody>
						...
						for($i=0; $i<count($break_item); $i++)
						{
							...
							<tr>
                                ...
                                <td  class="widthDiaType_show">
		                        	...
		                        	<?
		                        	echo create_drop_down( "txtdiawidth_".$k, 100, $fabric_typee,"", 1, "-- Select --",$dia_width_type[$break_item[$i]],"", "","" );
		                        	?>
                                </td>
								...
								<td><input type="text" id="txtcolor_<? echo $k;?>" name="txtcolor_<? echo $k;?>" class="text_boxes txt_color" style="width:80px" value="<? echo $break_color[$i]; ?>" /></td>
								...
								<td><input type="text" id="txtorderrate_<? echo $k;?>" name="txtorderrate_<? echo $k;?>"  class="text_boxes_numeric" style="width:60px" onKeyUp="sum_total_qnty(<? echo $k;?>)" value="<? echo $break_rate[$i]; ?>"/>
								</td>
					...
					else
					{
						...
						$qry_result=sql_select( "select id, mst_id, order_id, item_id, color_id, size_id, qnty, rate, amount, excess_cut, plan_cut, process_loss,gsm,grey_dia,finish_dia,dia_width_type,embellishment_type,description from subcon_ord_breakdown where order_id='$order_id' and mst_id='$mst_id' order by id");
						...
						foreach ($qry_result as $row)
						{
							...
							<tr class="general" >
                                ...
                                <?
		                        	echo create_drop_down( "txtdiawidth_".$k, 100, $fabric_typee,"", 1, "-- Select --",$dia_width_type,"", $disabled_type,"" );
		                        ?>
								...
								<td>
								<input type="text" id="txtorderquantity_<? echo $k; ?>" onClick="openmypage_qnty('<?= $id.'_'.$k;?>');" name="txtorderquantity_<? echo $k; ?>" value="<? echo $qnty_val; ?>" class="text_boxes_numeric" style="width:70px" onKeyUp="sum_total_qnty(<? echo $k; ?>);" onBlur="check_receive_qnty2(<? echo $k; ?>);"/>
								...
								<td><input type="text" id="txtorderrate_<? echo $k; ?>" name="txtorderrate_<? echo $k; ?>" value="<? echo $rate_val; ?>"  class="text_boxes_numeric" style="width:60px" onKeyUp="sum_total_qnty(<? echo $k; ?>)" <? echo $disabled_cond;?>/></td>
...
if ($action == "sub_order_print") 
{
   ...
   <div style="width:930px;">
         ...
         $mst_id=$dataArray[0][csf('id')];
			$sql_result =sql_select("select order_id,job_no_mst,item_id,color_id,size_id,qnty,rate,amount,excess_cut,plan_cut,process_loss,gsm,grey_dia,finish_dia,embellishment_type,description,dia_width_type from subcon_ord_breakdown  where job_no_mst='$job_no'  and status_active=1 and is_deleted=0");
		 ?>
