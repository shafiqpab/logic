<?
/*--- ----------------------------------------- Comments
Purpose			: 	This form will create Sub-contract Iron Bill Issue				
Functionality	:	
JS Functions	:
Created by		:	Kausar 
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Iron Bill Issue", "../", 1,1, $unicode,1,'');
?>
<script>
	...
	{ 
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/subcon_iron_bill_issue_controller.php?data='+data+'&action=bill_no_popup','Bill Popup', 'width=680px,height=400px,center=1,resize=1,scrolling=0','')
		...
		emailwindow.onclose=function()
		{
			...
			{
				freeze_window(5);
				get_php_form_data( theemail.value, "load_php_data_to_form_issue", "requires/subcon_iron_bill_issue_controller" );
				 
				window_close( theemail.value );
				//show_list_view(document.getElementById('cbo_party_source').value+'_'+document.getElementById('cbo_party_name').value+'_'+document.getElementById('update_id').value+'_'+document.getElementById('issue_id_all').value+'_'+document.getElementById('cbo_company_id').value,'iron_delivery_list_view','iron_info_list','requires/subcon_iron_bill_issue_controller','set_all();');
	 
	 			fnc_list_search(theemail.value);
				set_button_status(1, permission, 'fnc_iron_bill_issue',1);
				set_all_onclick();
				release_freezing();
			}
		}
	...
	function window_close( frm )
	{
		...
		var list_view_orders = return_global_ajax_value( data, 'load_php_dtls_form', '', 'requires/subcon_iron_bill_issue_controller');
	...
	function fnc_iron_bill_issue( operation )
	{
		if(operation==4)
		{
			var report_title=$( "div.form_caption" ).html();
			print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+$('#txt_bill_no').val()+'*'+report_title, "iron_bill_print", "requires/subcon_iron_bill_issue_controller") 
			//return;
			show_msg("3");
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			else
			{
				...
				var data1="action=save_update_delete&operation="+operation+"&tot_row="+tot_row+get_submitted_data_string('txt_bill_no*cbo_company_id*cbo_location_name*txt_bill_date*cbo_party_name*cbo_party_source*update_id',"../");
				var data2='';
				...
				http.open("POST","requires/subcon_iron_bill_issue_controller.php",true);
				http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
				http.send(data);
				http.onreadystatechange = fnc_iron_bill_issue_reponse;
			}
		}
	}

	function fnc_iron_bill_issue_reponse()
	{
	...
	function fnc_bill_for(val)
	{
		...
		else
		{
			...
			//$('#txt_bill_form_date').attr('disabled','disabled'); //remove by issue no 4446
			//$('#txt_bill_to_date').attr('disabled','disabled'); //remove by issue no 4446

			$('#txt_bill_form_date').removeAttr('disabled','disabled'); //adding by issue no 4446
			$('#txt_bill_to_date').removeAttr('disabled','disabled'); //adding by issue no 4446
	...
	function fnc_list_search(type)
	{
		...
		if (type==0 && ($('#update_id').val()*1)==0)
		{
				...
				show_list_view($('#cbo_party_source').val()+'***'+$('#cbo_party_name').val()+'***'+$('#update_id').val()+'***'+location_cond+'***'+$('#issue_id_all').val()+'***'+$('#txt_bill_form_date').val()+'***'+$('#txt_bill_to_date').val()+'***'+$('#cbo_company_id').val(),'iron_delivery_list_view','iron_info_list','requires/subcon_iron_bill_issue_controller', 'setFilterGrid("list_view_issue",-1)','','');
		...
		else
		{
			...
			show_list_view($('#cbo_party_source').val()+'***'+$('#cbo_party_name').val()+'***'+$('#update_id').val()+'***'+$('#issue_id_all').val()+'***'+$('#cbo_company_id').val()+'***'+$('#txt_bill_form_date').val()+'***'+$('#txt_bill_to_date').val()+'***'+location_cond+'***'+$('#txt_bill_no').val()+'***'+type+'***'+all_value,'iron_delivery_list_view','iron_info_list','requires/subcon_iron_bill_issue_controller','set_all();setFilterGrid("tbl_list_search",-1)','','');
...
<body onLoad="set_hotkey()">
    ...
    <form name="ironbillissue_1" id="ironbillissue_1"  autocomplete="off"  >
    <fieldset style="width:800px;">
    <legend>Iron Bill Info </legend>
        <table cellpadding="0" cellspacing="2" width="790">
            <tr>
                <td align="right" colspan="3"><strong>Bill No </strong></td>
                <td width="140" align="justify">
                    <input type="hidden" name="hidden_tot" id="hidden_tot" value="0" />
             ...
             <tr>
                ...
                <td width="150">
                    <?php 
                        echo create_drop_down( "cbo_company_id",150,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name","id,company_name", 1, "--Select Company--", $selected, "load_drop_down( 'requires/subcon_iron_bill_issue_controller', this.value, 'load_drop_down_location', 'location_td');","","","","","",2);	
                    ?>
                </td>
            ...
            <tr>
                ...
                <td>
                    <?
                        echo create_drop_down( "cbo_party_source", 150, $knitting_source,"", 1, "-- Select Party --", $selected, "load_drop_down( 'requires/subcon_iron_bill_issue_controller',document.getElementById('cbo_company_id').value+'_'+this.value, 'load_drop_down_party_name', 'party_td' );fnc_bill_for(this.value)",0,"1,2","","","",4);
                    ?> 
                </td>
        ...
        <tfoot>
            ...
            <tr>
                <td colspan="13" align="center" class="button_container">
                    <? 
                        echo load_submit_buttons($permission,"fnc_iron_bill_issue",0,1,"reset_form('ironbillissue_1','iron_info_list','','','$(\'#bill_issue_table tr:not(:first)\').remove();')",1);
                    ?> 
                </td>
            </tr>  
            <tr>
