<?
/*--- ----------------------------------------- Comments
Purpose			: 	This form will create Out Side Cutting Bill Entry						
Functionality	:	
JS Functions	:
Created by		:	Kausar
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Cutting Bill Entry", "../", 1,1, $unicode,1,'');
?>
...
<script>
	...
	{ 
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/outside_cutting_bill_entry_controller.php?data='+data+'&action=bill_no_popup','Bill Popup', 'width=700px,height=400px,center=1,resize=1,scrolling=0','')
		...
		emailwindow.onclose=function()
		{
			...
			{
				freeze_window(5);
				get_php_form_data( theemail.value, "load_php_data_to_form_issue", "requires/outside_cutting_bill_entry_controller" );
				window_close( theemail.value );
				show_list_view(document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_supplier_company').value+'_'+document.getElementById('update_id').value,'cutting_entry_list_view','cutting_info_list','requires/outside_cutting_bill_entry_controller','set_all();');
				set_button_status(1, permission, 'fnc_cutting_bill_entry',1);
				set_all_onclick();
				release_freezing();
			}
		}
	...
	function window_close( frm )
	{
		...
		//alert (data);
		var list_view_orders = return_global_ajax_value( data, 'load_php_dtls_form', '', 'requires/outside_cutting_bill_entry_controller');
		if(list_view_orders!='')
		{
	...
	function fnc_cutting_bill_entry( operation )
	{
		 if(operation==4)
		 {
			print_report( $('#txt_bill_no').val()+'*'+$('#cbo_company_id').val()+'*'+$('#cbo_party_name').val()+'*'+$('#issue_id_all').val()+'*'+$('#txt_bill_date').val()+'*'+$('#selected_order_id').val(), "fabric_finishing_print", "requires/outside_cutting_bill_entry_controller") 
			//return;
			show_msg("3");
		 }
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			{	
				...
				{
					data2+=get_submitted_data_string('reciveid_'+i+'*txtreceivedate_'+i+'*txt_challenno_'+i+'*ordernoid_'+i+'*itemid_'+i+'*text_wo_num_'+i+'*cbouom_'+i+'*txt_qnty_'+i+'*txt_rate_'+i+'*txt_amount_'+i+'*txt_remarks_'+i+'*updateiddtls_'+i+'*delete_id',"../");
				}
				...
				http.open("POST","requires/outside_cutting_bill_entry_controller.php",true);
				http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
				http.send(data);
				http.onreadystatechange = fnc_cutting_bill_entry_reponse;
			}
		}
	}

	function fnc_cutting_bill_entry_reponse()
	{
	...
	{ 
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/outside_cutting_bill_entry_controller.php?action=wonum_popup&data='+data,'Wo Popup', 'width=950px,height=400px,center=1,resize=1,scrolling=0','')
...
<body onLoad="set_hotkey()">
    ...
    <form name="outcuttingbill_1" id="outcuttingbill_1"  autocomplete="off"  >
        ...
        <table cellpadding="0" cellspacing="2" width="100%">
            <tr>
             ...
             <tr>
                ...
                <td width="150">
                    <? 
                        echo create_drop_down( "cbo_company_id",150,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name","id,company_name", 1, "--Select Company--", $selected, "load_drop_down( 'requires/outside_cutting_bill_entry_controller', this.value, 'load_drop_down_location', 'location_td');load_drop_down( 'requires/outside_cutting_bill_entry_controller', this.value, 'load_drop_down_supplier_name', 'supplier_td');","","","","","",2);	
                    ?>
                </td>
            ...
            <tfoot>
                ...
                <tr>
                    <td colspan="12" align="center" class="button_container">
						<? 
							echo load_submit_buttons($permission,"fnc_cutting_bill_entry",0,0,"reset_form('outcuttingbill_1','cutting_info_list','','','$(\'#bill_issue_table tr:not(:first)\').remove();')",1);
                        ?> 
                    </td>
                </tr>  
                <tr>
