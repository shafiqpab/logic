...
<script>
		...
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			else
			{
				...
				for(var i=1; i<=tot_row; i++)
				{
					if(source==1)
					{
						if($('#bodypartid_'+i).val()==2 || $('#bodypartid_'+i).val()==3 || $('#bodypartid_'+i).val()==40 || $('#bodypartid_'+i).val()==172 || $('#bodypartid_'+i).val()==203 || $('#bodypartid_'+i).val()==214)
						{
	...
	function fnc_list_search(type)
	{
		if($('#cbo_party_source').val()==1)
		{
			...
			if($("#txt_job_id").val()=="")
			{
				if( form_validation('txt_bill_form_date*txt_bill_to_date','Form Date*To Date')==false)
				{
					return;
				}
			}
		...
		{
			show_list_view(document.getElementById('cbo_company_id').value+'***'+document.getElementById('cbo_location_name').value+'***'+document.getElementById('cbo_party_source').value+'***'+document.getElementById('cbo_party_name').value+'***'+document.getElementById('cbo_bill_for').value+'***'+document.getElementById('txt_bill_form_date').value+'***'+document.getElementById('txt_bill_to_date').value+'***'+document.getElementById('txt_manual_challan').value+'*********'+document.getElementById('txt_job_id').value,'knitting_delivery_list_view','knitting_info_list','requires/knitting_bill_issue_controller', 'setFilterGrid("tbl_list_search",-1)');
		}
		else
		{
			var tot_row=$('#bill_issue_table tr').length;
			...
			if($('#cbo_party_source').val()==1)
			{
				...
				{
					if(all_value=="") all_value+=$('#challenno_'+n).val()+'_'+$('#ordernoid_'+n).val()+'_'+$('#itemid_'+n).val()+'_'+$('#bodypartid_'+n).val()+'_'+$('#compoid_'+n).val(); else all_value+='!!!!'+$('#challenno_'+n).val()+'_'+$('#ordernoid_'+n).val()+'_'+$('#itemid_'+n).val()+'_'+$('#bodypartid_'+n).val()+'_'+$('#compoid_'+n).val();
				}
			}
			else
			{
			...
			//alert(all_value);
			show_list_view(document.getElementById('cbo_company_id').value+'***'+document.getElementById('cbo_location_name').value+'***'+document.getElementById('cbo_party_source').value+'***'+document.getElementById('cbo_party_name').value+'***'+document.getElementById('cbo_bill_for').value+'***'+document.getElementById('txt_bill_form_date').value+'***'+document.getElementById('txt_bill_to_date').value+'***'+document.getElementById('txt_manual_challan').value+'***'+type+'***'+all_value,'knitting_delivery_list_view','knitting_info_list','requires/knitting_bill_issue_controller','setFilterGrid("tbl_list_search",-1)','','');
		}
	}
    ...
    <legend>Knitting Bill Info </legend>
        <table cellpadding="0" cellspacing="2" width="850">
            ...
            <tr>
                ...
                <td>
                    <input class="text_boxes" type="text" style="width:130px" name="txt_manual_challan" id="txt_manual_challan" disabled />
                </td>
        ...
        <table align="right" cellspacing="0" cellpadding="0" width="970"  border="1" rules="all" class="rpt_table" >
            ...
            <tbody id="bill_issue_table">
                <tr align="center">				
                    ...
                    <td>
						<? echo create_drop_down( "curanci_1", 60, $currency,"", 1, "-Select Currency-",1,"",1,"" );?>
                    </td>
            ...
            <tfoot>
                ...
                <tr>
                    ...
                    echo load_submit_buttons($permission,"fnc_knitting_bill_issue",0,1,"reset_form('knitigbillissue_1','knitting_info_list','','txt_bill_date,".$date."','$(\'#bill_issue_table tr:not(:first)\').remove();')",1); ?>&nbsp;
                       		...
                       		<input type="button" name="search_2" id="search_2" value="Print 2" onClick="fnc_print_report_2(2)" style="width:80px" class="formbutton" />
