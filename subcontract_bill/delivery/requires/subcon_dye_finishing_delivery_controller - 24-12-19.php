                ...
                <table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="ganeral">
                            ...
                            <td  align="center">
                                <input type="text" style="width:100px" class="text_boxes"  name="txt_batch_no" id="txt_batch_no" placeholder="Search Batch"/>
                            </td>
                            <td  align="center">
...
if($action=="create_order_search_list_view")
{
	...
	if($search_type==1)
	{
		...
		if ($batch_no!='') $batch_cond="join pro_batch_create_mst d on d.id=c.batch_id and d.batch_no = '$batch_no'"; else $batch_cond="";
	}
	else if($search_type==4 || $search_type==0)
	{
		...
		if ($batch_no!='') $batch_cond="join pro_batch_create_mst d on d.id=c.batch_id and d.batch_no like '%$batch_no%'"; else $batch_cond="";
	}
	...
	else if ($db_type==2)
	{
		...
		$order_id_cond="to_char(a.id)=c.order_id";
	}

	$sql = "SELECT a.id, a.order_rcv_date, a.order_no, a.order_uom, a.main_process_id, a.order_quantity, b.party_id, a.cust_style_ref, b.subcon_job, b.job_no_prefix_num, $year_cond from subcon_ord_dtls a join subcon_ord_mst b on a.job_no_mst=b.subcon_job join subcon_production_dtls c on $order_id_cond $batch_cond where a.main_process_id in (3,4) and a.status_active=1 and a.is_deleted=0 and b.company_id='$company' $party_cond $job_cond $style_cond $order_cond $date_cond  group by a.id, a.order_rcv_date, a.order_no, a.order_uom, a.main_process_id, a.order_quantity, b.party_id, a.cust_style_ref, b.subcon_job, b.job_no_prefix_num , b.insert_date";
	//echo $sql;
	//$sql = "SELECT a.id, a.order_rcv_date, a.order_no, a.order_uom, a.main_process_id, a.order_quantity, b.party_id, a.cust_style_ref, b.subcon_job, b.job_no_prefix_num, $year_cond from subcon_ord_dtls a, subcon_ord_mst b, subcon_production_dtls c	where a.main_process_id in (3,4) and a.job_no_mst=b.subcon_job $order_id_cond and a.status_active=1 and a.is_deleted=0 and b.company_id='$company' $party_cond $job_cond $style_cond $order_cond $date_cond group by a.id, a.order_rcv_date, a.order_no, a.order_uom, a.main_process_id, a.order_quantity, b.party_id, a.cust_style_ref, b.subcon_job, b.job_no_prefix_num , b.insert_date ";
...
if($action=="show_fabric_desc_listview")
{
	...
	{
		$sql = "SELECT a.batch_id, a.gsm, a.dia_width, a.width_dia_type, a.cons_comp_id, a.color_id, a.fabric_description, a.process, sum(b.quantity) as production_qnty
		from subcon_production_dtls a, subcon_production_qnty b, pro_batch_create_dtls c
		where b.order_id in ($order_id) and a.id=b.dtls_id and a.cons_comp_id=c.id and a.product_type='$process_id'
		group by a.batch_id, a.gsm, a.dia_width, a.cons_comp_id, a.color_id, a.fabric_description, a.process, a.width_dia_type
		order by a.batch_id DESC";
	}
...
if($action=="load_php_data_for_batch")
{
	...
	$delivery_qty_array=array();
	$delivery_sql="select item_id, gsm, dia, color_id,sum(reject_qty) as reject_qty, sum(delivery_qty) as delivery_qty from subcon_delivery_dtls where order_id='$order_id' and item_id='$prod_id' and gsm='$gsm' and dia='$dia' and process_id in (3,4) $color_id_cond group by item_id, gsm, dia, color_id";
	$delivery_data_sql=sql_select($delivery_sql);
	foreach($delivery_data_sql as $row)
	{
		$delivery_qty_array[$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('color_id')]]['delivery_qty']=$row[csf('delivery_qty')];
		$delivery_qty_array[$row[csf('item_id')]][$row[csf('gsm')]][$row[csf('dia')]][$row[csf('color_id')]]['reject_qty']=$row[csf('reject_qty')];
	}
	$batch_color_arr=array();
	...
	foreach ($sql_result as $row)
	{
		...
		$availabe_delivery_qty=$row[csf('production_qnty')]-$delivery_qty_array[$row[csf('cons_comp_id')]][$row[csf('gsm')]][$row[csf('dia_width')]][$row[csf('color_id')]]['delivery_qty'];
		$reject_qty=$delivery_qty_array[$row[csf('cons_comp_id')]][$row[csf('gsm')]][$row[csf('dia_width')]][$row[csf('color_id')]]['reject_qty'];
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Start Here========================================================================================================================
	{
		...
		$field_array_dtls="id, mst_id, order_id, process_id, sub_process_id, item_id, gsm, dia, batch_id, color_id, width_dia_type, delivery_qty, carton_roll, remarks, gray_qty,reject_qty";

		$data_array_dtls="(".$dtlsid.",".$id.",".$txt_order_id.",".$cbo_process_name.",".$hid_sub_process.",".$txt_item_id.",".$txt_gsm.",".$txt_dia.",".$txt_batch_id.",".$txt_color_id.",".$hidden_dia_type.",".$txt_delivery_qnty.",".$txt_carton_roll_no.",".$txt_remarks.",".$txt_gray_qnty.",".$txt_reject_qty.")";
	...
	else if ($operation==1)   // Update Here==============================================================================================
	{
		...
		$return_no=$txt_sys_id;
		$field_array_dtls="order_id*process_id*sub_process_id*item_id*gsm*dia*batch_id*color_id*width_dia_type*delivery_qty*carton_roll*remarks*gray_qty*reject_qty";

		$data_array_dtls="".$txt_order_id."*".$cbo_process_name."*".$hid_sub_process."*".$txt_item_id."*".$txt_gsm."*".$txt_dia."*".$txt_batch_id."*".$txt_color_id."*".$hidden_dia_type."*".$txt_delivery_qnty."*".$txt_carton_roll_no."*".$txt_remarks."*".$txt_gray_qnty."*".$txt_reject_qty."";
					...
					foreach($dtls_value as $row)
					{
						...
						if(!in_array($chack_string,$sub_process_array))
						{
							...
							<tr class="tbl_bottom" bgcolor="#CCCCCC" style="font-size:14px">
								...
								<th width="70" align="right"><? echo number_format($tot_proces_loss,2); ?>&nbsp;</th>
								<th>&nbsp;</th>
							</tr>
						<?
								unset($tot_roll);
