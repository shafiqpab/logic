...
if($action=="load_php_data_to_form_delivery")
{
	...
	$nameArray =sql_select("select id, order_id, process_id, sub_process_id, item_id, gsm, dia, color_id, yarn_lot, width_dia_type, delivery_qty, delivery_pcs, carton_roll, collar_cuff, remarks from  subcon_delivery_dtls where id='$ex_data[0]' and process_id=2");
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Start Here======================================================================================================================== 
	{
		...
		$field_array_dtls="id, mst_id, order_id, process_id, item_id, gsm, dia, width_dia_type, color_id, yarn_lot, delivery_qty, carton_roll, collar_cuff, remarks, delivery_pcs";
		
		$data_array_dtls="(".$dtlsid.",".$id.",".$txt_order_id.",2,".$txt_item_id.",".$txt_gsm.",".$txt_dia.",".$txt_dia_type.",".$txt_color_id.",".$txt_lot.",".$txt_delivery_qnty.",".$txt_carton_roll_no.",".$txt_collar_cuff_mgt.",".$txt_remarks.",".$txt_delivery_pcs.")"; 
	...
	else if ($operation==1)   // Update Here==============================================================================================
	{
		...
		$rID=sql_update("subcon_delivery_mst",$field_array,$data_array,"id",$update_id,0);
		$return_no=$txt_sys_id;
		$field_array_dtls="order_id*item_id*gsm*dia*width_dia_type*color_id*yarn_lot*delivery_qty*carton_roll*collar_cuff*remarks*delivery_pcs";
		
		$data_array_dtls="".$txt_order_id."*".$txt_item_id."*".$txt_gsm."*".$txt_dia."*".$txt_dia_type."*".$txt_color_id."*".$txt_lot."*".$txt_delivery_qnty."*".$txt_carton_roll_no."*".$txt_collar_cuff_mgt."*".$txt_remarks."*".$txt_delivery_pcs.""; 
...
?>
   ...
   <table width="100%" cellpadding="0" cellspacing="0" >
    ...
    <div style="width:100%; height:870px">
        ...
        $sql_kniting="select order_id, item_id, dia, width_dia_type, gsm, color_id, yarn_lot, delivery_qty, delivery_pcs, carton_roll, remarks, collar_cuff from subcon_delivery_dtls where mst_id='$mst_id' and process_id=2 ";
        $sql_kniting_result=sql_select($sql_kniting);
		?>
        <div>
        <table align="right" cellspacing="0" width="930"  border="1" rules="all" class="rpt_table" style="margin-top: 10px;">
            ...
            foreach($sql_kniting_result as $row)
            {
				...
				{
				?>
                <thead bgcolor="#dddddd" align="center" style="font-size:13px">
                    <th width="25">SL</th>
...
?>
   ...
   <table width="100%" cellpadding="0" cellspacing="0" >
    ...
    <div style="width:100%; height:870px">
        ...
        {
            $prod_dia_sql="select order_id, cons_comp_id, fabric_description, gsm, dia_width, dia_width_type, color_id,yarn_lot, 
            group_concat(machine_id) as machine_id,group_concat(machine_gg) as machine_gg,group_concat(machine_dia) as machine_dia,
            group_concat(yrn_count_id) as yrn_count_id,
            group_concat(stitch_len) as stitch_len,
			group_concat(brand) as brand,
            sum(product_qnty) as production_qnty from subcon_production_dtls where product_type=2 and status_active=1 and is_deleted=0 group by order_id, cons_comp_id, fabric_description, gsm, dia_width, dia_width_type, color_id, yarn_lot";
