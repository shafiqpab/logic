...
if ($action=="delivery_id_popup")
{
	...
	<script>
		...
		function search_by(){
			...
			if(searchby==1){
				$('#searchId').html('Job No');
			}else if(searchby==2){
				$('#searchId').html('Style No');
			}else if(searchby==3){
				$('#searchId').html('Po No');
			}
	...
	<body>
        <div align="center" style="width:100%;" >
            <form name="deliverysearch_1"  id="deliverysearch_1" autocomplete="off">
                <table width="820" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    <thead>                	 
                        <th width="150">Company Name</th>
						<th width="70">Search by</th>
						<th width="100" id="searchId">Job NO</th>
                    ...
                    <tbody>
                        <tr>
							...
							<td> 
                                <?
                                    $selected=array(1=>"Job No",2=>"Style No",3=>"Po No");
                                    echo create_drop_down( "cbo_search_by", 70, $selected,"", 1, "--Select--", 1, "search_by()",0 );
                                ?>
                            </td>
							<td>
                                <input type="text" name="txt_job_style_po" id="txt_job_style_po" class="text_boxes" style="width:95px" />
                            </td>
                            <td>
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_year').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_job_style_po').value, 'create_delivery_search_list_view', 'search_div', 'subcon_gmts_delivery_controller', 'setFilterGrid(\'tbl_po_list\',-1)')" style="width:100px;" /></td>
                        </tr>
                        <tr>
...
if($action=="create_delivery_search_list_view")
{
	...
	if ($data[5]==1 && $data[6]!=""){
		$jobCond="and d.job_no_prefix_num='$job_style_po'";
	}elseif ($data[5]==2 && $data[6]!=""){
		$jobCond="and c.cust_style_ref='$job_style_po'";
	}elseif ($data[5]==3 && $data[6]!=""){
		$jobCond="and c.order_no='$job_style_po'";
	} 

	 $sql= "select a.id, a.delivery_no, a.company_id, a.delivery_prefix_num, $year_cond, a.location_id, a.party_id, a.challan_no, a.delivery_date, a.forwarder, a.transport_company,a.job_no from subcon_delivery_mst a,subcon_delivery_dtls b,subcon_ord_dtls c, subcon_ord_mst d where a.id=b.mst_id  and c.job_no_mst=d.subcon_job and c.id=b.order_id and a.status_active=1 and b.status_active=1 and c.status_active=1  and a.is_deleted=0 and a.process_id=3 $company $delivery_date $delivery_id_cond $jobCond order by a.id DESC";
    ...
    <div>
     ...
     <div style="width:670px; max-height:270px;overflow-y:scroll;" >	 
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="650" class="rpt_table" id="tbl_po_list">
			<?
			$i=1;
...
?>	
	...
	<div style="width:830px;max-height:180px; overflow:y-scroll" id="" align="left">
		...
		<?  
			$i=1;
			$sqlResult =sql_select("select a.id,b.id as dtls_id,b.order_id,b.item_id,a.delivery_date,b.process_id,b.delivery_qty,a.location_id,a.challan_no from  subcon_delivery_mst a,  subcon_delivery_dtls b where a.id=b.mst_id and  a.id=$data and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.id");
...
if($action=="populate_data_from_search_popup")
{
	...
	$res = sql_select("select a.id, a.delivery_date, a.order_no, a.main_process_id, b.subcon_job, c.item_id, sum(c.qnty) as order_quantity,a.cust_style_ref  from  subcon_ord_dtls a, subcon_ord_mst b, subcon_ord_breakdown c where a.main_process_id in (1,5,8,9,10,11) and a.job_no_mst=b.subcon_job and a.status_active=1 and a.is_deleted=0 and a.id='$po_id' and c.item_id='$item_id' and a.id=c.order_id and b.id=c.mst_id group by a.id, a.delivery_date, a.order_no, a.main_process_id, b.subcon_job, c.item_id,a.cust_style_ref"); 
 	...
 	foreach($res as $result)
	{
		...
		echo "$('#txt_style_no').val('".$result[csf('cust_style_ref')]."');\n";
		...
		//echo $result[csf('id')].'='.$item_id;die;
		 $total_delivery = return_field_value("sum(delivery_qty) as delivery_qty ","subcon_delivery_dtls","order_id=".$result[csf('id')]." and item_id='$item_id' and status_active=1 and is_deleted=0","delivery_qty");
...
if($action=="color_and_size_level")
{
	...
	else if( $variableSettings==3 ) //color and size level
	{
			...
			$prodData = sql_select("select a.ord_color_size_id, sum(a.prod_qnty) as production_qnty from  subcon_gmts_prod_col_sz a, subcon_gmts_prod_dtls b where a.dtls_id=b.id and b.order_id='$po_id' and b.gmts_item_id='$item_id' and a.production_type='$prod_type' and b.status_active=1 and b.is_deleted=0  and a.ord_color_size_id!=0 group by a.ord_color_size_id");
			...
			//subcon_ord_breakdown a left join subcon_gmts_delivery_dtls b on b.breakdown_color_size_id=a.id and a.status_active=1 and a.is_deleted=0  where a.order_id='$po_id' and a.item_id='$item_id' group by a.item_id, a.color_id, a.size_id");
			$sql_del=sql_select("select  a.id as color_size_id,a.item_id,a.color_id,a.size_id,sum(b.delivery_qty) as production_qnty from 
			subcon_ord_breakdown a, subcon_gmts_delivery_dtls b,subcon_delivery_dtls c where   b.breakdown_color_size_id=a.id and  c.id=b.dtls_mst_id  and a.order_id='$po_id' and a.item_id='$item_id' and c.status_active=1 and c.is_deleted=0 group by a.id,a.item_id, a.color_id, a.size_id");
			foreach($sql_del as $row_exfac)
			{
				if($row_exfac[csf("production_qnty")]>0)
				{
				//$delivery_value[$row_exfac[csf("item_id")]][$row_exfac[csf("color_id")]][$row_exfac[csf("size_id")]]+=$row_exfac[csf("production_qnty")];
				$delivery_value[$row_exfac[csf("color_size_id")]]=$row_exfac[csf("production_qnty")];
				}
	...
	foreach($colorResult as $color)
	{
		...
		else //color and size level
		{
			...
			//$exfac_qnty=$delivery_value[$color[csf('item_id')]][$color[csf('color_id')]][$color[csf('size_id')]];
				$exfac_qnty=$delivery_value[$color[csf('id')]];
			
			$colorHTML .='<tr><td title="'.$color[csf('id')].'='.$pro_qnty.'='.$exfac_qnty.'">'.$size_library[$color[csf("size_id")]].'</td><td><input type="text" name="colorSize" id="colSize_'.$color[csf("color_id")].($i+1).'"  class="text_boxes_numeric" style="width:100px" placeholder="'.($pro_qnty-$exfac_qnty).'" onblur="fn_total('.$color[csf("color_id")].','.($i+1).')"><input type="hidden" name="hidden_ord_breakdown_id" id="hidden_ord_breakdown_id_'.$color[csf("id")].($i+1).'"  class="text_boxes_numeric" style="width:100px" value="'.$color[csf("id")].'" ></td></tr>';				
		}
		$i++; 
	}
	//echo $colorHTML;die; 
...
if($action=="populate_delivery_form_data")
{
 	...
 	foreach($sqlResult as $result)
	{
		...
		echo "$('#shipping_status').val('".$result[csf('shipping_status')]."');\n";
		echo "$('#txt_remarks1').val('".$result[csf('remarks1')]."');\n";
		...
		{ 
			...
			$sql_dtls = sql_select("select a.id,b.delivery_qty,a.size_id, a.color_id from  subcon_ord_breakdown a,subcon_gmts_delivery_dtls b where   a.id=b.breakdown_color_size_id and a.order_id='$po_id' and a.item_id='$item_id'  and b.mst_id='$data' and b.dtls_mst_id='$dtls_id' and a.status_active=1 and a.is_deleted=0 ");
			...
			if( $variableSettings==2 ) // color level
			{
				...
				//{
					...
					$sql_del=sql_select("select a.item_id,a.color_id,sum(b.delivery_qty) as delivery_qty from subcon_ord_breakdown a
							left join subcon_gmts_delivery_dtls b on b.breakdown_color_size_id=a.id and b.status_active=1 and b.is_deleted=0 
				...
				/*$sql = "select id, item_number_id, size_number_id, color_number_id, order_quantity, plan_cut_qnty, (select sum(CASE WHEN pdtls.color_size_break_down_id=wo_po_color_size_breakdown.id then pdtls.production_qnty ELSE 0 END) from pro_garments_production_dtls pdtls where pdtls.production_type=8 and pdtls.is_deleted=0 ) as production_qnty, (select sum(CASE WHEN ex.color_size_break_down_id=wo_po_color_size_breakdown.id then ex.production_qnty ELSE 0 END) from pro_ex_factory_dtls ex where ex.is_deleted=0 ) as ex_production_qnty  
					...
					if($process_id==1)
					{
					 $prodData = "select b.ord_color_size_id,sum(a.production_qnty) as production_qnty
										from  subcon_gmts_prod_dtls a,subcon_gmts_prod_col_sz b where  b.dtls_id=a.id and a.order_id='$po_id' and a.status_active=1 and a.is_deleted=0  and a.gmts_item_id='$item_id' and b.production_type=1 group by b.ord_color_size_id";
					}
					else if($process_id==5)
					{
					 $prodData = "select b.ord_color_size_id,sum(a.production_qnty) as production_qnty
										from  subcon_gmts_prod_dtls a,subcon_gmts_prod_col_sz b where  b.dtls_id=a.id and a.order_id='$po_id' and a.status_active=1 and a.is_deleted=0  and a.gmts_item_id='$item_id' and b.production_type=2 group by b.ord_color_size_id";
					}
					else if($process_id==10)
					{
					 $prodData = "select b.ord_color_size_id,sum(a.production_qnty) as production_qnty
										from  subcon_gmts_prod_dtls a,subcon_gmts_prod_col_sz b where  b.dtls_id=a.id and a.order_id='$po_id'  and a.status_active=1 and a.is_deleted=0 and a.gmts_item_id='$item_id' and b.production_type=3 group by b.ord_color_size_id";
					}
					else if($process_id==11)
					{
					 $prodData = "select b.ord_color_size_id,sum(a.production_qnty) as production_qnty
										from  subcon_gmts_prod_dtls a,subcon_gmts_prod_col_sz b where  b.dtls_id=a.id  and a.status_active=1 and a.is_deleted=0 and a.order_id='$po_id' and a.gmts_item_id='$item_id' and b.production_type=4 group by b.ord_color_size_id";
					}
			...
			$sql_del=sql_select("select a.item_id,a.color_id,a.size_id,sum(b.delivery_qty) as production_qnty from subcon_ord_breakdown a
                    left join subcon_gmts_delivery_dtls b on b.breakdown_color_size_id=a.id  and b.status_active=1 and b.is_deleted=0 where a.order_id='$po_id' and a.item_id='$item_id' and b.dtls_mst_id='$dtls_id'   group by a.item_id, a.color_id, a.size_id");
			foreach($sql_del as $row_exfac)
			{
				$delivery_value[$row_exfac[csf("item_id")]][$row_exfac[csf("color_id")]][$row_exfac[csf("size_id")]]=$row_exfac[csf("production_qnty")];
			...
			foreach($colorResult as $color)
			{
				...
				{  
					...
					$colorHTML .='<tr><td>'.$color_library[$color[csf("color_id")]].'</td><td><input type="text" name="txt_color" id="colSize_'.($i+1).'" style="width:80px"  class="text_boxes_numeric" placeholder="'.($color[csf("production_qnty")]-$delivery_value[$color[csf('item_id')]][$color[csf('color_id')]]+$amount).'" value="'.$amount.'" onblur="fn_colorlevel_total('.($i+1).')"><input type="text" name="txt_color_size_id" id="txt_color_size_id_'.($i+1).'" style="width:80px"  class="text_boxes_numeric"  value="'.$color[csf("id")].'" ></td></tr>';				
					$totalQnty += $amount;
					$colorID .= $color[csf("color_id")].",";
				}
				else //color and size level
  ...
  //      	 echo "pre";
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$id_dtls=return_next_id("id", "subcon_delivery_dtls", 1);
  		...
  		$field_array1="id, mst_id, order_id, process_id, item_id, delivery_qty,entry_break_down_type, total_carton_qnty, carton_roll,shipping_status,remarks1";

		$data_array1="(".$id_dtls.",".$id.",".$hidden_po_break_down_id.",".$cbo_process_name.",".$cbo_item_name.",".$txt_delivery_qty.",".$sewing_production_variable.",".$txt_total_carton_qnty.",".$txt_ctn_qnty.",".$shipping_status.",".$txt_remarks1.")";

		// $data_array1="(".$id_dtls.",".$mst_id.",".$hidden_po_break_down_id.",".$cbo_process_name.",".$cbo_item_name.",".$txt_delivery_qty.",".$sewing_production_variable.",".$txt_total_carton_qnty.",".$txt_ctn_qnty.",".$txt_remarks.")";
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$gate_pass = sql_select("SELECT id,SYS_NUMBER from inv_gate_pass_mst where challan_no=$txt_sys_id and company_id=$cbo_company_id and basis=27 and status_active=1 and is_deleted=0");
		...
		$field_array1="order_id*process_id*item_id*delivery_qty*entry_break_down_type*total_carton_qnty*carton_roll*shipping_status*remarks1";
		$data_array1="".$hidden_po_break_down_id."*".$cbo_process_name."*".$cbo_item_name."*".$txt_delivery_qty."*".$sewing_production_variable."*".$txt_total_carton_qnty."*".$txt_ctn_qnty."*".$shipping_status."*".$txt_remarks1."";
		//print_r($data_array1);
	...
	else if ($operation==2)  // Delete Here---------------------------------------------------------- 
	{
		...
		if($flag==1)
		{
			$rID1=sql_delete("subcon_delivery_dtls",$field_array,$data_array,"id","".$txt_dtls_id."",1);
			if($rID1==1) 
			{
				$dtlsrDelete = execute_query("DELETE from subcon_gmts_delivery_dtls where mst_id='$delivery_mst_id' and dtls_mst_id=$txt_dtls_id ",1);
				if($dtlsrDelete==1) $flag=1; else $flag=0; 
			}
			else $flag=0;
		}

		//echo "10**".$rID.'='.$rID1.'='.$dtlsrDelete.'='.$flag; die;
...
<div style="width:710px;">
    <table width="700" cellspacing="0" align="right" style="margin-bottom:20px;">
        <tr>
            <td rowspan="2" align="center"><img src="../../<? echo $com_dtls[2]; ?>" height="50" width="60"></td>
            <td colspan="5" align="center"  style="font-size:xx-large; "><strong><? echo $com_dtls[0]; ?></strong></td>
        </tr>
        <tr class="form_caption">
		...
		$sql_master ="SELECT a.id,a.party_id,b.order_id,a.challan_no,b.item_id,a.delivery_date,a.transport_company,a.vehical_no,a.location_id,a.company_id,a.final_destination,a.lock_no,a.dl_no,a.do_no,a.attention,a.driver_name,a.mobile_no,a.remarks,b.process_id,b.total_carton_qnty,b.delivery_qty,b.remarks1  from  subcon_delivery_mst a,  subcon_delivery_dtls b where a.id=b.mst_id and  a.delivery_no='".$data[1]."' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.id";
		...
		$sql_wo_po_break =sql_select("select a.id,a.order_no as po_number,a.job_no_mst,a.cust_style_ref from subcon_ord_dtls a where a.id in($all_po_id)");
		//echo "select a.id,a.order_no as po_number,a.job_no_mst from subcon_ord_dtls a where a.id in($all_po_id)";
		foreach($sql_wo_po_break as $poData)
		{
			$po_data_arr[$poData[csf("id")]]["po_number"]=$poData[csf("po_number")];
			$po_data_arr[$poData[csf("id")]]["job_no_mst"]=$poData[csf("job_no_mst")];
			$po_data_arr[$poData[csf("id")]]["cust_style_ref"]=$poData[csf("cust_style_ref")];
		}
    ...
    <table align="right" cellspacing="0" width="1000"  border="1" rules="all" class="rpt_table" >
        ...
        foreach($result_master as $row)
        {
            ...
            <tr bgcolor="<? echo $bgcolor; ?>">
				...
				<td><? echo $po_data_arr[$row[csf("order_id")]]["cust_style_ref"]; ?></td>
