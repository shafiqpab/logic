...
if($action=="create_order_search_list_view")
{
	...
	$sql ="select a.product_date,b.batch_against, b.po_no, b.po_id from subcon_embel_production_mst  a, subcon_embel_production_dtls b where a.id=b.mst_id and a.company_id='$company' $party_cond $order_cond $date_cond $BatchAgainstCond $poIDCond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.product_date,b.batch_against, b.po_no, b.po_id order by b.po_id";
...
if( $action == 'populate_order_data') 
{
	...
	//$totDeliveryArr=return_library_array( " select b.batch_id,  sum(b.delivery_qty) as tot_delivery   from subcon_delivery_mst a, subcon_delivery_dtls b where a.id=b.mst_id  and a.entry_form='4' and a.status_active=1 and a.is_deleted=0 group by b.batch_id ",'batch_id','tot_delivery');
	$totDeliveryArr=return_library_array( " select b.embl_prod_dtls_id,  sum(b.delivery_qty) as tot_delivery   from subcon_delivery_mst a, subcon_delivery_dtls b where a.id=b.mst_id  and a.entry_form='4' and a.status_active=1 and a.is_deleted=0 group by b.embl_prod_dtls_id ",'embl_prod_dtls_id','tot_delivery');
	$totBatchDeliveryArr=return_library_array( " select b.batch_id,  sum(b.delivery_qty) as batch_tot   from subcon_delivery_mst a, subcon_delivery_dtls b where a.id=b.mst_id  and a.entry_form='4' and a.status_active=1 and a.is_deleted=0 group by b.batch_id ",'batch_id','batch_tot');
	...
	*/
	...
	foreach($data_array as $row)
	{
		...
		$totDelivery 	= $totDeliveryArr[$row[csf('id')]]*1;
        ...
        <tr class="general" name="tr[]" id="tr_<? echo $tblRow; ?>">
            ...
            <td>
                <input type="text" name="txtPreDelivQty[]"  id="txtPreDelivQty_<? echo $tblRow; ?>" value="<? echo $prevDelivery; ?>" class="text_boxes_numeric" style="width:50px"   placeholder="Display" readonly />
            </td>
            <td>
...
//and a.company_id='$data[1]'
	...
	*/
	...
	foreach($data_array as $row)
	{
		...
		$totDelivery 	= $totDeliveryArr[$row[csf('embl_prod_dtls_id')]]*1;
		//$totDelivery 	= $totDeliveryArr[$row[csf('batch_id')]]*1;
		$currDelivery 	= $row[csf('delivery_qty')]*1;
		$prevDelivery	= ($totDelivery - $currDelivery);
		$balance	 	= $QcPassQty - $prevDelivery;
        ...
        <tr class="general" name="tr[]" id="tr_<? echo $tblRow; ?>">
        	<td>
               ...
               <input type="hidden" name="txtOrderId[]"  id="txtOrderId_<? echo $tblRow; ?>" value="<? echo $row[csf('order_id')]; ?>" class="" style="width:50px" />
                <input type="hidden" name="updateIdDtls[]"  id="updateIdDtls_<? echo $tblRow; ?>" value="<? echo $row[csf('dtls_id')]; ?>" class="" style="width:50px" />
            </td>
            <td>
                <input type="text" name="EmblType[]"  id="EmblType_<? echo $tblRow; ?>" value="<? echo $batch_against[$row[csf('batch_against')]]; ?>" class="text_boxes" style="width:80px" placeholder="Display"disabled />
                <input type="hidden" name="txtEmblTypeId[]"  id="txtEmblTypeId_<? echo $tblRow; ?>" value="<? echo $row[csf('batch_against')]; ?>" class="text_boxes_numeric" style="width:50px" disabled />
...
if ($action=="load_php_data_to_form")
{
	...
	//select b.mst_id, listagg(b.order_id,',') within group(order by b.order_id) order_id from subcon_delivery_mst a,  subcon_delivery_dtls b where a.id=b.mst_id and status_active =1 and is_deleted=0 and entry_form=4    group by b.mst_id 
	if($db_type == 0){
		$order_IdArr=return_library_array( "select b.mst_id, group_concat(b.order_id,',') as order_id from subcon_delivery_mst a,  subcon_delivery_dtls b where a.id=b.mst_id and a.status_active =1 and a.is_deleted=0 and a.entry_form=4 group by b.mst_id",'mst_id','order_id');
	}else{
		$order_IdArr=return_library_array( "select b.mst_id, listagg(b.order_id,',') within group(order by b.order_id) order_id from subcon_delivery_mst a,  subcon_delivery_dtls b where a.id=b.mst_id and status_active =1 and is_deleted=0 and entry_form=4 group by b.mst_id",'mst_id','order_id');
	}
	...
	{	
		...
		echo "document.getElementById('txtPoId').value		= '".$order_IdArr[$row[csf("id")]]."';\n"; 
...
?>	
	...
	<div style="width:300px;max-height:180px; overflow:y-scroll" id="" align="left">
		...
		<?php  
			$i=1;
			$sqlResult =sql_select("select a.id,a.delivery_date,a.location_id,a.challan_no,b.id as dtls_id,b.order_id,b.item_id,b.process_id,b.delivery_qty from  subcon_delivery_mst a,  subcon_delivery_dtls b where a.id=b.mst_id and  a.id='$data' and a.status_active=1 and a.is_deleted=0 order by a.id");
 			foreach($sqlResult as $selectResult)
			{
 			...
 			?>
                <tr bgcolor="<?php echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="fnc_embl_delivery_dtls('<?php echo $selectResult[csf('id')]; ?>_<?php echo $selectResult[csf('dtls_id')]; ?>','populate_delivery_form_data','requires/subcon_embellishment_delivery_controller');" > 
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$txt_update_id)=="")
		{
			...
			$new_sys_number=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '', 'SED', date("Y",time()), 5, "select delivery_prefix, delivery_prefix_num from subcon_delivery_mst where company_id=$cbo_company_id $mrr_cond order by id DESC ", "delivery_prefix", "delivery_prefix_num" ));
			...
			for($i=1; $i<= str_replace("'","",$txt_tot_row); $i++)
			{
				...
				if( $QcPassQtyArr[$$emblProdDtlsId] < $$txtQcPassQty )
				{
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");  
				echo "0**".str_replace("'","",$id)."**".$mrr_no."**".$mrr_no_challan;
			}
			else
			{
