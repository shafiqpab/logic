...
<script>
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				if (ret_value[0]!="")
				{
					...
					show_list_view(document.getElementById('txt_order_id').value+"_"+document.getElementById('cbo_process_name').value+'_'+document.getElementById('txt_item_id').value+"_"+document.getElementById('cbo_company_name').value,'show_fabric_desc_listview','list_fabric_desc_container','requires/subcon_knitting_delivery_controller','');
	...
	function fnc_subcon_delivery_entry(operation)
	{
		...
		if (prod_qty==reject_qty){
			if( form_validation('cbo_company_name*cbo_location*cbo_party_name*txt_order_no*txt_delivery_date*txt_dalivery_item','Company Name*Location*Party*Order Number*Delivery Date*Dalivery Item')==false ){
				return;
			}
		}else{
			if( form_validation('cbo_company_name*cbo_location*cbo_party_name*txt_order_no*txt_delivery_date*txt_dalivery_item*txt_delivery_qnty','Company Name*Location*Party*Order Number*Delivery Date*Dalivery Item*Delivery Qty')==false ){
				return;
			}
		}
	...
	function generate_report(type)
	{
		...
		else
		{
			 ...
			 if(type==4)
			 {
				 print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+$('#txt_sys_id').val()+'*'+report_title+'*'+type+'*'+$('#cbo_location').val(), "delivery_entry_without_gp2_print", "requires/subcon_knitting_delivery_controller" )
			 }
			 else if(type==5)
			 {
				 print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+$('#txt_sys_id').val()+'*'+report_title+'*'+type+'*'+$('#cbo_location').val()+'*'+$('#cbo_template_id').val(), "subcon_delivery_entry_print5", "requires/subcon_knitting_delivery_controller" )
			 }
			 else if(type==7)
			 {
				//var report_title= ;
				 print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+$('#txt_sys_id').val()+'*'+"Knitting Job Work Delivery Challan & Bill"+'*'+type+'*'+$('#cbo_location').val()+'*'+$('#cbo_template_id').val(), "subcon_delivery_entry_print_seven", "requires/subcon_knitting_delivery_controller" )
			 }
			 else
			 {
				 print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+$('#txt_sys_id').val()+'*'+report_title+'*'+type+'*'+$('#cbo_location').val(), "subcon_delivery_entry_print", "requires/subcon_knitting_delivery_controller" )
			 }
			//return;
			show_msg("3");
		}
	...
	function generate_report_short(type)
	{
		...
		{
			 var report_title=$( "div.form_caption" ).html();
			 print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+$('#txt_sys_id').val()+'*'+report_title+'*'+type+'*'+$('#cbo_location').val(), "knitting_delivery_print", "requires/subcon_knitting_delivery_controller" )
			//return;
			show_msg("3");
		}
		...
		{
			 var report_title=$( "div.form_caption" ).html();
			 print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+$('#txt_sys_id').val()+'*'+report_title+'*'+type+'*'+$('#cbo_location').val(), "knitting_delivery_print1", "requires/subcon_knitting_delivery_controller" )
			//return;
			show_msg("3");
		}
	}
                ...
                <table width="100%">
                    <tr>
                     ...
                     <tr>
                        ...
                        <td width="140">
                            <?
                                echo create_drop_down( "cbo_company_name", 152, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/subcon_knitting_delivery_controller', this.value, 'load_drop_down_location', 'location_td' ); load_drop_down( 'requires/subcon_knitting_delivery_controller', this.value, 'load_drop_down_party_name', 'party_td' );",0 );
                            ?>
                        </td>
            ...
            <table cellpadding="0" cellspacing="1" width="800" align="left">
            	...
            	<tr>
                	<td width="60%" valign="top" >
                        ...
                        <legend>New Entry</legend>
                        	 <table  cellpadding="0" cellspacing="2" width="100%">
                                ...
                                <tr>
                                    <td>Process</td>
                                    <td><? echo create_drop_down( "cbo_process_name", 130, $production_process,"", 1, "--Select Process--",$selected,"", "1","" );?></td>
                                	<td class="must_entry_caption">Deli. Qty Wgt / Pcs</td>
                                    <td>
                                ...
                                <tr>
                                    <td>Order Date</td>
                                    <td><input type="text" name="txt_order_date" id="txt_order_date" class="datepicker" style="width:120px;" disabled /></td>
                                 ...
                                 <tr>
                                    ...
                                    <td>Style</td>
                                    <td><input type="text" name="txt_style" id="txt_style" class="text_boxes" style="width:120px;" disabled /></td>
                                 </tr>
                                <tr>
                                    <td>Collar Cuff Meas.</td>
                                    <td><input type="text" name="txt_collar_cuff_mgt" id="txt_collar_cuff_mgt" class="text_boxes" style="width:120px;" disabled/></td>
                                    <td>Reject Qty</td>
                                    <td><input type="text" name="txt_reject_qty" id="txt_reject_qty" class="text_boxes_numeric" style="width:120px;" /></td>
                                </tr>
                                <tr>
                                    <td>Moisture Gain</td>
                                ...
                                <tr>
                                	<td>Remarks</td>
                                    <td colspan="3"><input type="text" name="txt_remarks" id="txt_remarks" class="text_boxes" style="width:355px;" maxlength="150" title="Maximum 150 Character" /></td>
                                </tr>
                             </table>
                        </fieldset>
                    </td>
                    ...
                    <td width="35%" valign="top">
                        ...
                        <legend>Display</legend>
                        	<table cellpadding="0" cellspacing="2" width="100%">
                                <tr>
                                	<td width="120" colspan="2">Delivery Balance</td>
                ...
                <tr>
                	<td align="center" colspan="4" valign="middle" class="button_container">
						...
						<input type="button" name="search" id="search" value="Print" onClick="generate_report_short(4)" style="width:100px" class="formbuttonplasminus" />
                        <input type="button" name="search" id="search" value="WithOut G.P 2" onClick="generate_report(5)" style="width:100px" class="formbuttonplasminus" />
                        <input type="button" name="search" id="search" value="With Gate Pass2" onClick="generate_report(6)" style="width:100px" class="formbuttonplasminus" />
						<input type="button" name="search" id="search" value="C&B With Gate Pass" onClick="generate_report(7)" style="width:100px" class="formbuttonplasminus" />
                    </td>
                </tr>
                <tr>
                	<td colspan="4">&nbsp;</td>
