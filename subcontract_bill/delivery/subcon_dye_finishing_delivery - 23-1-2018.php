...
<script>
	...
	function fnc_subcon_delivery_entry(operation)
	{
		...
		{
			var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_sys_id*cbo_company_name*cbo_location*txt_order_id*cbo_party_name*cbo_process_name*txt_delivery_date*txt_item_id*txt_delivery_qnty*txt_carton_roll_no*txt_challan_no*txt_transport_company*cbo_forwarder*txt_remarks*update_id*update_id_dtls*txt_gsm*txt_dia*txt_color_id*txt_batch_id*hidden_dia_type*hid_sub_process*txt_gray_qnty',"../");
			//alert (data);return;
	...
	function fnc_subcon_delivery_entry_respone()
	{
		if(http.readyState == 4) 
		{
			...
			if(reponse[0]==0 || reponse[0]==1)
			{
				...
				reset_form('','','txt_item_id*txt_delivery_qnty*txt_carton_roll_no*txt_remarks*update_id_dtls*txt_production_qnty*txt_color*txt_color_id*txt_batch_no*txt_ext_no*txt_batch_id*txt_gsm*txt_sub_process*hid_sub_process*txt_dia*hidden_dia_type*bill_info*txt_dalivery_item*txt_pre_delivery_qnty*txt_gray_qnty');
	...
	function generate_report(type)
	{
		...
		{		
			 ...
			 print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+$('#txt_sys_id').val()+'*'+report_title+'*'+type, "subcon_delivery_entry_print", "requires/subcon_dye_finishing_delivery_controller" ) 
			//return;
			show_msg("3");
		}
	}	
	...
	function validate_gray_qty()
	{
		...
		var txt_gray_qnty=$('#txt_gray_qnty').val();
                ...
                <table width="100%">
                    <tr>
                     ...
                     <tr>
                        ...
                        <td width="140">
                            <?
                                echo create_drop_down( "cbo_company_name", 152, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1  order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/subcon_dye_finishing_delivery_controller', this.value, 'load_drop_down_location', 'location_td');load_drop_down( 'requires/subcon_dye_finishing_delivery_controller', this.value, 'load_drop_down_party_name', 'party_td' )",0 );	
