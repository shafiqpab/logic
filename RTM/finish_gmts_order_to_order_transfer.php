...
function openmypage_orderNo(type)
{
	...
	emailwindow.onclose=function()
	{
		...
		$('#txt_'+type+'_plan_cut_qty').val(data[5]);
		$('#txt_'+type+'_ref_no').val(data[9]);
		$('#txt_'+type+'_hid_po_id').val(data[10]);
...
function fnc_transfer_entry(operation)
{
	if(operation==0 || operation==1 || operation==2)
	{
		...
		else
		{
			...
			var data="action=save_update_delete&operation="+operation+"&colorIDvalue="+colorIDvalue+get_submitted_data_string('txt_system_id*update_id*cbo_company_id*sewing_production_variable*styleOrOrderWisw*txt_transfer_date*cbo_transfer_criteria*txt_challan_no*txt_from_hid_po_id*txt_from_order_id*cbo_from_gmts_item*cbo_from_country_name*txt_to_hid_po_id*txt_to_order_id*cbo_to_gmts_item*cbo_to_country_name*txt_transfer_qty*txt_remark*hidden_break_down_html*hidden_colorSizeID*update_dtls_id*update_dtls_issue_id*update_dtls_recv_id*txt_finish_quantity*txt_cumul_quantity*txt_yet_quantity*garments_nature*cbo_po_ok',"../");
