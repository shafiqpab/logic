<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Sample production Report.
Functionality	:	
JS Functions	:
...
<script>
	...
	function fn_report_generated(excel_type)
	{
		...
		{
			var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_sample_stage*cbo_buyer_name*cbo_dealing_merchant*cbo_year*txt_booking_no*txt_req_no*txt_style_ref*txt_internal_ref*txt_date_from*txt_date_to',"../")+'&excel_type='+excel_type;
			//alert(data);return;
			freeze_window(3);
			http.open("POST","requires/sample_production_report_controller.php",true);
	...
	function open_popup(arg,page_title,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/sample_production_report_controller.php?action='+action+'&data='+arg+'&garments_nature='+garments_nature, page_title, 'width='+width_pop+'px,height=400px,center=1,resize=0,scrolling=0','../');
	}	
    ...
    {
        $('#data_panel').html('');
        get_php_form_data($('#cbo_company_name').val(),'print_button_variable_setting','requires/sample_production_report_controller' ); 
    }
    function print_report_button_setting(report_ids) 
    {
        ...
        for (var k=0; k<report_id.length; k++)
        {
            if(report_id[k]==108)
            {
                $('#data_panel').append( '<input type="button"  id="show_button" class="formbutton" style="width:90px;" value="Show"  name="show"  onClick="fn_report_generated(0)" />&nbsp;&nbsp;&nbsp;' );
            }
            if(report_id[k]==259)
            {
                $('#data_panel').append( '<input type="button"  id="show_button2" class="formbutton" style="width:90px;" value="Show2"  name="show_button2"  onClick="fn_report_generated(1)" />&nbsp;&nbsp;&nbsp;' );
            }
        }
    }
</script>
    ...
    <div style="width:100%;" align="center">    
        <? echo load_freeze_divs ("../",'');  ?>
         <fieldset style="width:1270px;">
            ...
            <table class="rpt_table" width="1270px" cellpadding="0" cellspacing="0" align="center" border="1" rules="all">
               <thead>                    
                    <tr>
						<th width="130" class="must_entry_caption">Company Name</th>
						<th width="130">Sample Stage</th>
						<th width="130">Buyer Name</th>
                        <th width="130">Dealing Merchant</th>
                        <th width="60">Year</th>
                        <th width="70">Booking No</th>
                        <th width="70">Requisition No</th>
                        <th width="70">Style Ref.</th>
                        <th width="70">Int.Ref.No</th>
                        <th width="140" colspan="2" id="search_text_td">Transaction Date Range</th>
                        <th><input type="reset" id="reset_btn" class="formbutton" style="width:70px" value="Reset" /></th>
                    </tr>    
                 </thead>
                <tbody>
                    <tr class="general">
                        <td><? echo create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/sample_production_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );print_button_setting();" ); ?></td>
                        <td><? echo create_drop_down( "cbo_sample_stage", 130, $sample_stage,"", 1, "-- Select Stage --", $selected, "sample_stage_change(this.value);",0,"" ); ?></td>
                        <td id="buyer_td"><? echo create_drop_down( "cbo_buyer_name", 130, $blank_array,"", 1, "-- Select Buyer --", $selected, "",0,"" ); ?></td>
                        <td id="dealing_merchant_td"><? echo create_drop_down( "cbo_dealing_merchant", 130, "select id,team_member_name from lib_mkt_team_member_info where status_active =1 and is_deleted=0 order by team_member_name","id,team_member_name", 1, "-- Select Team Member --", $selected, "" ); ?></td>
