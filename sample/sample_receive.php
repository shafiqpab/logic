...
<script>
	...
	function sample_receive_pop()
    {
        ...
        emailwindow = dhtmlmodal.open('EmailBox', 'iframe', page_link, 'Item', 'width=1050px, height=280px, center=1, resize=0, scrolling=0', '');
        emailwindow.onclose = function()
        {
			...
			var totNumber = this.contentDoc.getElementById("totNumber").value
				for(var i=1;i<=totNumber;i++){
					var file = this.contentDoc.forms['uploadForm']['samplepic_'+i].files[0]; 
					
					try {
					  if(file.name){fileArr.push(file);}
					}
					catch (e) {
					  fileArr.push('');
					}
    ...
    function fnc_sample_receive(operation)
    {
		...
		var sup=$('#cbo_fabric_source').val();
		//alert(sup);
		if(operation==0 && sup==2)
		{
		...
		else
		{
			...
			//=======================================
				...
				for(var i=0;i<fileArr.length; i++){
					if(fileArr[i].name!=''){formdata.append("appendFile"+i, fileArr[i]);}
					if(uploadFileName==''){uploadFileName='<? echo time();?>'+i+fileArr[i].name;}else{uploadFileName+='***'+'<? echo time();?>'+i+fileArr[i].name;}
					if(fileName==''){fileName="appendFile"+i;}else{fileName+=",appendFile"+i;}
				}
				var ajax = new XMLHttpRequest();
				ajax.open("POST", "requires/sample_receive_controller.php?action=smp_image_upload&fileName="+fileName+"&imageName="+uploadFileName);
				ajax.send(formdata);
			//=====================
			var data = "action=save_update_delete&uploadFileName="+uploadFileName+"&operation=" + operation + get_submitted_data_string('receive_date*cbo_fabric_cat*txt_item*txt_style*cbo_fabric_source*cbo_supplier_source*txt_designer*txt_qty*cbo_fabric_natu*txt_construction*txt_Composition*txt_gsm*cbo_yarn_count*cbo_yarn_type*cbo_status*update_id*color_qty_breakdown', "../");
			//alert(data);
	...
	function sample_list_pop()
    {
        ...
        emailwindow = dhtmlmodal.open('EmailBox', 'iframe', page_link, 'sample List View', 'width=1050px, height=280px, center=1, resize=0, scrolling=0', '');
        emailwindow.onclose = function()
        {
                            ...
                            <tr>
                                ...
                                <td valign="top">
                                    <input type="text" name="txt_qty" id="txt_qty" onDblClick="sample_receive_pop()" placeholder="Double Click" class="text_boxes_numeric" style="width: 200px" readonly >
                                     <input type="hidden" name="color_qty_breakdown" id="color_qty_breakdown"  />
                                </td>			
                            </tr>
                            <tr>
                            <td></td>
                    ...
                    <fieldset style="height:200px; float:left;" >
                        ...
                        <table cellpadding="0" cellspacing="1" width="300"  align="center">
                            ...
                            <tr>
                                ...
                                <td valign="top">
                                    <? 
                                    echo create_drop_down( "cbo_status", 212, $row_status,"", 1, "-- Select --","","", "", "");		
                                    ?>
                                </td>	
                            </tr>
                            ...
                            <tr>
                                ...
                                <td valign="top">
                                    ...
                                    <input type="text" id="txt_sys_id" name="txt_sys_id" class="text_boxes" style="width:200px" placeholder="Browse" value=""  onDblClick="sample_list_pop();" readonly >
                                </td>	
                            </tr>
                        </table>
                    </fieldset>
