...
function put_sample_item_data(sample_dtls_part_tbl_id,smp_id,gmts,color_id)
{
	...
	//lert(sample_dtls_part_tbl_id+' '+smp_id+' '+gmts+' '+req_id);return;
	//freeze_window(5);
 	get_php_form_data(sample_dtls_part_tbl_id+'**'+smp_id+'**'+req_id+'**'+gmts+'**'+color_id, "color_and_size_level", "requires/sample_sewing_input_controller" ); 
    set_button_status(0, permission, 'fnc_sample_sewing_entry',1,0);
	//release_freezing();
}
...
function fnc_sample_sewing_entry(operation)
{
	...
	{	
		...
		else
		{
			...
			var data="action=save_update_delete&operation="+operation+"&colorIDvalue="+colorIDvalue+get_submitted_data_string('mst_update_id*dtls_update_id*cbo_company_name*txt_sample_requisition_id*cbo_buyer_name*txt_style_no*cbo_item_name*txt_sample_qty*cbo_source*cbo_sewing_company*cbo_location*cbo_floor*cbo_sample_name*txt_sewing_date*txt_reporting_hour*txt_sewing_qty*txt_remark*hidden_sample_dtls_tbl_id*hidden_requisition_id*cbo_sample_team*txt_total_cutting_qty',"../");
...
function childFormReset(type)
{
	if(type==1)
	{
		reset_form('samplesewinginput_1','','','','');
	}
	else
	{
                            ...
                            <fieldset>
                                ...
                                <table cellpadding="0" cellspacing="1" width="100%">
                                    ...
                                    <tr>
                                        ...
                                        <td id="team_td" ><? //echo create_drop_down( "cbo_sample_team", 140,"select id,team_name from lib_sample_production_team where is_deleted=0 and status_active=1 order by team_name","id,team_name", 1, "Select Team", $selected ); ?>
										<? echo create_drop_down( "cbo_sample_team", 140,$blank_array,"", 1, "Select Team", $selected, "",1 ); ?>
										</td> 
                                    </tr>
                                    <tr>
                                       <td class="must_entry_caption">Reporting Hour</td>
                                       <td colspan="2">
                                        <input name="txt_reporting_hour" id="txt_reporting_hour" class="text_boxes" style="width:130px" placeholder="24 Hour Format" onBlur="fnc_valid_time(this.value,'txt_reporting_hour');" onKeyUp="fnc_valid_time(this.value,'txt_reporting_hour');" onKeyPress="return numOnly(this,event,this.id);" maxlength="8" />
                                       </td>
                                    </tr>
                                    <tr>
