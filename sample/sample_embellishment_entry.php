<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Sample Embellishment Entry
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Embellishment Info","../", 1, 1, $unicode,'','');
...
function openmypage(page_link,title)
{
	...
	emailwindow.onclose=function()
	{
		...
		if (smp_id!="")
		{
			...
			get_php_form_data(smp_id, "populate_data_from_search_popup", "requires/sample_embellishment_entry_controller" );
			$("#cbo_company_name").attr('disabled','disabled');
			$('#mst_update_id').val('');
			
			show_list_view(smp_id,'show_sample_item_listview','list_view_country','requires/sample_embellishment_entry_controller','');		
			
			show_list_view(smp_id,'show_dtls_listview','list_view_container','requires/sample_embellishment_entry_controller','');
...
function check_requisition_embellishment_process(data)
{
	...
	var emb_type = return_global_ajax_value( res[0]+'__'+$('#hidden_requisition_id').val()+'__'+$('#cbo_sample_name').val()+'__'+$('#cbo_item_name').val(), 'embellishment_type_as_per_req', '', 'requires/sample_embellishment_entry_controller');
	var emb_id = return_global_ajax_value( $('#hidden_requisition_id').val()+'**'+$('#cbo_sample_name').val()+'**'+$('#cbo_item_name').val(), 'embellishment_id_as_per_req', '', 'requires/sample_embellishment_entry_controller');

   if(trim(emb_id).length==1)
   {
		get_php_form_data($('#hidden_sample_dtls_tbl_id').val()+'**'+$('#cbo_sample_name').val()+'**'+$('#hidden_requisition_id').val()+'**'+$('#cbo_item_name').val()+'**'+res[0]+'**'+'single','color_and_size_level', 'requires/sample_embellishment_entry_controller' );
   ...
   else 
   {
   	  ...
   	  var emb = return_global_ajax_value( $('#hidden_requisition_id').val()+'**'+$('#cbo_sample_name').val()+'**'+$('#cbo_item_name').val(), 'embellishment_name_as_per_req', '', 'requires/sample_embellishment_entry_controller');

      var position=r.indexOf(res[0]); 
   	  if(position==0)
   	  {
   	  	get_php_form_data($('#hidden_sample_dtls_tbl_id').val()+'**'+$('#cbo_sample_name').val()+'**'+$('#hidden_requisition_id').val()+'**'+$('#cbo_item_name').val()+'**'+res[0]+'**'+'single'+'**'+r[0],'color_and_size_level', 'requires/sample_embellishment_entry_controller' );
   	  	if($('#breakdown_td_id').html()=='')
   	  	{
    	  		 alert("No Cutting info found for this sample and garments item");
   	  	}
   	  ...
   	  {
   	  	get_php_form_data($('#hidden_sample_dtls_tbl_id').val()+'**'+$('#cbo_sample_name').val()+'**'+$('#hidden_requisition_id').val()+'**'+$('#cbo_item_name').val()+'**'+res[0]+'**'+'position_one'+'**'+r[0],'color_and_size_level', 'requires/sample_embellishment_entry_controller' );
   	  ...
   	  {
   	  	get_php_form_data($('#hidden_sample_dtls_tbl_id').val()+'**'+$('#cbo_sample_name').val()+'**'+$('#hidden_requisition_id').val()+'**'+$('#cbo_item_name').val()+'**'+res[0]+'**'+'position_two'+'**'+r[1],'color_and_size_level', 'requires/sample_embellishment_entry_controller' );
...
function put_sample_item_data(sample_dtls_part_tbl_id,smp_id,gmts)
{
	...
	var cbo_embel_type=$("#cbo_embel_type").val()*1;
	...
	freeze_window(5);
 	get_php_form_data(sample_dtls_part_tbl_id+'**'+smp_id+'**'+req_id+'**'+gmts+'**'+embel_name+'**'+'single'+'**'+'0'+'**'+status_id+'**'+cbo_embel_type, "color_and_size_level", "requires/sample_embellishment_entry_controller" ); 
    set_button_status(0, permission, 'fnc_sample_embellishment_entry',1,0);
	release_freezing();
}
...
function fn_total(tableName,index) // for color and size level
{
	...
	//alert(filed_value+'='+colorSizeinput);
...
function fnc_sample_embellishment_entry(operation)
{
	...
	{	
		...
		{		
		    ...
		    print_report( $('#cbo_company_name').val()+'*'+$('#mst_update_id').val()+'*'+$("#cbo_sample_name").val()+'*'+$("#cbo_item_name").val()+'*'+check_print_ids+'*'+$("#dtls_update_id").val()+'*'+$("#hidden_requisition_id").val()+'*'+$("#hidden_sample_dtls_tbl_id").val(), "embellishment_print", "requires/sample_embellishment_entry_controller" ) 
		    return;
		}
	}
	else if(operation==5)
	{	
		...
		{		
		    //var report_title=$( "div.form_caption" ).html();
		    print_report( $('#cbo_company_name').val()+'*'+$('#mst_update_id').val()+'*'+$("#cbo_sample_name").val()+'*'+$("#cbo_item_name").val()+'*'+check_print_ids+'*'+$("#dtls_update_id").val()+'*'+$("#hidden_requisition_id").val()+'*'+$("#hidden_sample_dtls_tbl_id").val()+'*'+embel_name, "embellishment_print2", "requires/sample_embellishment_entry_controller" ) 
		    return;
		}
	}
	else if(operation==0 || operation==1 || operation==2)
	{	
 		if (form_validation('cbo_company_name*txt_sample_requisition_id*txt_embellishment_date*cbo_embellishment_company*cbo_sample_name*txt_embellishment_qty*cbo_embel_name*cbo_embel_type','Company Name*Sample Requisition ID*Embellishment Company*Embellish Date*Sample Name*Embellish Quantity*Embel. Name*Embel. Type')==false)
		{
			return;
		}		
		else
		{
			...
			if('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][128]);?>')
		{
		    if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][128]);?>','<? echo implode('*',$_SESSION['logic_erp']['mandatory_message'][128]);?>')==false)
		    {
		      release_freezing();
		      return;
		    }
			...
			var data="action=save_update_delete&operation="+operation+"&colorIDvalue="+colorIDvalue+"&colorIDvalueRej="+colorIDvalueRej+get_submitted_data_string('mst_update_id*dtls_update_id*cbo_company_name*txt_sample_requisition_id*cbo_buyer_name*txt_style_no*cbo_item_name*txt_sample_qty*cbo_source*cbo_embellishment_company*cbo_location*cbo_floor*cbo_sample_name*txt_embellishment_date*txt_reporting_hour*txt_embellishment_qty*txt_reject_qnty*txt_remark*hidden_sample_dtls_tbl_id*hidden_requisition_id*cbo_embel_name*cbo_embel_type*txt_total_cutting_qty*cbo_status_id*mst_update_id',"../"); 
			 // alert(data);return;
			freeze_window(operation);
  			http.open("POST","requires/sample_embellishment_entry_controller.php",true);
...
function fnc_sample_embellishment_entry_Reply_info()
{
 	if(http.readyState == 4) 
	{
		...
		if(response[0]==0)//insert response;
		{
			...
			show_list_view(response[2],'show_dtls_listview','list_view_container','requires/sample_embellishment_entry_controller','');
			$('#mst_update_id').val(response[1]);
			$('#txt_mst_issue_id').val(response[6]);
  			$('#breakdown_td_id').html('');
  			var val =return_global_ajax_value( response[5]+"__"+response[1]+"__"+$('#cbo_sample_name').val()+"__"+$('#cbo_item_name').val()+"__"+$('#cbo_embel_name').val(), 'populate_data_yet_to_cut', '', 'requires/sample_embellishment_entry_controller');
...
// function fnc_load_from_dtls(dtls_data)
// {
// 	//alert(dtls_data);
// 	get_php_form_data(dtls_data,'populate_input_form_data','requires/sample_embellishment_entry_controller');
// }
...
{
	reset_form('','','txt_embellishment_date*txt_reporting_hour*txt_embellishment_qty*txt_reject_qnty*txt_remark','','');
}
...
<div style="width:100%;">
                ...
                <fieldset>
                    <table width="100%" border="0">
                        <tr>
                            ...
                            <td width="170">                                
								<? 
                                 echo create_drop_down( "cbo_company_name", 170, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/sample_embellishment_entry_controller', this.value, 'load_drop_down_location', 'location_td' );" );
                                ?>
							</td>
							 <td align="left"> &nbsp; Source</td>
                             <td>
								 <?
                                 echo create_drop_down( "cbo_source", 170, $knitting_source,"", 1, "-- Select Source --", $selected, "load_drop_down( 'requires/sample_embellishment_entry_controller', this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_sewing_output', 'sew_company_td' );", 0, '1,3' );
                                 ?>
                             </td>
                         	 <td align="left" class="must_entry_caption"> &nbsp; Embel. Company</td>
                             <td id="sew_company_td" >
								 <?
                                 echo create_drop_down( "cbo_embellishment_company", 170, $blank_array,"", 1, "--- Select ---", $selected, "" );
                                 ?>
						     </td>
                        ...
                        <td align="left" width="102" class="must_entry_caption">Sample Req. No</td>
                            <td width="170">
								<input name="txt_sample_requisition_id" placeholder="Double Click to Search" id="txt_sample_requisition_id" onDblClick="openmypage('requires/sample_embellishment_entry_controller.php?action=sample_requisition_popup&company='+document.getElementById('cbo_company_name').value,'Sample Requisition ID')"  class="text_boxes" style="width:155px " readonly>
                        ...
                        <td width="" class="must_entry_caption" align="left">Embel.Name</td>
                                 <td width="" id="embel_name_td">
                                     <? 
                                     echo create_drop_down( "cbo_embel_name", 170, $emblishment_name_array,"", 1, "-- Select Embel.Name --", $selected, "",'','','','','99' );
                                     ?>
                                 </td>
                    	...
                    	<td width="35%" valign="top">
                            ...
                            <legend>New Entry</legend>
                                <table  cellpadding="0" cellspacing="1" width="100%">
                                    <tr>
                                        <td>Status</td>
                                        <td title="Embl Issue Wise"><? echo create_drop_down( "cbo_status_id", 150, $sample_statusArr,"", 1, "-- Select Status --", $selected, "",0,0 );?></td> 
                                </tr>
                                   ...
                                   <tr>
                                        <td align="left" class="must_entry_caption">Embel. Quantity</td> 
                                        <td width="" valign="top"><input name="txt_embellishment_qty" id="txt_embellishment_qty" class="text_boxes_numeric"  style="width:70px" readonly >
                                            <input type="hidden" id="hidden_colorSizeID"  value=""/>
											Recive Id&nbsp;<input name="txt_mst_issue_id" id="txt_mst_issue_id" class="text_boxes_numeric" style="width:50px" readonly >
                                        </td>
                        ...
                        <td width="1%" valign="top">
                         ...
                         <td width="22%" valign="top">
                            ...
                            <legend>Display</legend>
                                ...
                                <tr>
                                        <td align="left" width="110" id="dynamic_cut_qty">Total Cutting Qty</td>
                                        <td>
                                    ...
                                    <tr>
                                        <td align="left" width="110">Cumul. Embel. Qty</td>
                                        <td>
                                            <input type="text" name="txt_cumul_embel_qty" id="txt_cumul_embel_qty" class="text_boxes_numeric" style="width:80px" readonly disabled />
                                            <input type="hidden" name="txt_cumul_rej_qty" id="txt_cumul_rej_qty" class="text_boxes_numeric" style="width:80px" readonly disabled />
  <input type="hidden" name="txt_cumul_previ_qty" id="txt_cumul_previ_qty" class="text_boxes_numeric" style="width:80px" readonly disabled />
                                     ...
                                     <tr>
                                        <td align="left" width="110">Yet to Embel.</td>
                                        <td>
                                            <input type="text" name="txt_yet_to_embel" id="txt_yet_to_embel" class="text_boxes_numeric" style="width:80px" / readonly disabled >
                                        </td>
                                    </tr>
                                </table>
                            </fieldset>	
                     ...
                     <tr>
		   				<td align="center" colspan="9" valign="middle" class="button_container">
							<?
							$date=date('d-m-Y');
                            echo load_submit_buttons( $permission, "fnc_sample_embellishment_entry", 0,1,"reset_form('sampleEmbellishment_1','list_view_country','','txt_embellishment_date,".$date."*txt_challan,0','childFormReset()')",1); 
                            ?>
