<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create sample Sewing output Entry
...
function openmypage(page_link,title)
{
		...
		emailwindow.onclose=function()
		{
			...
			if (smp_id!="")
			{
				...
				get_php_form_data(smp_id, "populate_data_from_search_popup", "requires/sample_sewing_output_entry_controller" );
				
				show_list_view(smp_id,'show_sample_item_listview','list_view_country','requires/sample_sewing_output_entry_controller','');		
				
				show_list_view(smp_id,'show_dtls_listview','list_view_container','requires/sample_sewing_output_entry_controller','');
...
function put_sample_item_data(sample_dtls_part_tbl_id,smp_id,gmts)
{
	...
	//freeze_window(5);
 	get_php_form_data(sample_dtls_part_tbl_id+'**'+smp_id+'**'+req_id+'**'+gmts, "color_and_size_level", "requires/sample_sewing_output_entry_controller" ); 
    set_button_status(0, permission, 'fnc_sample_sewing_entry',1,0);
	//release_freezing();
}
...
function fn_total(tableName,index) // for color and size level
{
	...
	//var colorSizeinput = $("#colorSizeinput_"+tableName+index).val()*1;
	 ...
	 var previ_rej_qty = $("#txt_cumul_rej_qty").val()*1;
	 var cumul_sewing_qty=$("#txt_cumul_sewing_qty").val()*1;
	...
	//var placeholder_value = (placeholder_value_chk*1);
	
	//alert(placeholder_value+'=='+filed_value);
...
function fnc_sample_sewing_entry(operation)
{
	...
	if(operation==4)
	{
		...
		if(check_print_ids==""){
			...
			print_report( $('#cbo_company_name').val()+'*'+$('#mst_update_id').val()+'*'+$("#cbo_sample_name").val()+'*'+$("#cbo_item_name").val()+'*'+check_print_ids+'*'+$("#hidden_requisition_id").val()+'*'+$("#hidden_sample_dtls_tbl_id").val(), "sewing_print", "requires/sample_sewing_output_entry_controller" ) 
		 return;
		}
	}
	...
	{	
		...
		if (form_validation('cbo_company_name*txt_sample_requisition_id*cbo_sewing_company*txt_sewing_date*cbo_sample_name*cbo_sample_team*txt_sewing_qty','Company Name*Sample Requisition ID*Sewing Company*Sewing Date*Sample Name*Sample Team*Cutting Quantity')==false)
		{
			return;
		...
		else
		{
			...
			var data="action=save_update_delete&operation="+operation+"&colorIDvalue="+colorIDvalue+"&colorIDvalueRej="+colorIDvalueRej+get_submitted_data_string('mst_update_id*dtls_update_id*cbo_company_name*txt_sample_requisition_id*cbo_buyer_name*txt_style_no*cbo_item_name*txt_sample_qty*cbo_source*cbo_sewing_company*cbo_location*cbo_floor*cbo_sample_name*txt_sewing_date*txt_reporting_hour*txt_sewing_qty*txt_reject_qnty*txt_remark*hidden_sample_dtls_tbl_id*hidden_requisition_id*cbo_sample_team*txt_total_cutting_qty',"../");
			// alert(data);return;
			freeze_window(operation);
  			http.open("POST","requires/sample_sewing_output_entry_controller.php",true);
...
function fnc_sample_sewing_entry_Reply_info()
{
 	if(http.readyState == 4) 
	{
		...
		if(response[0]==0)//insert response;
		{
			...
			show_list_view(response[2],'show_dtls_listview','list_view_container','requires/sample_sewing_output_entry_controller','');
			$('#mst_update_id').val(response[1]);
  			$('#breakdown_td_id').html('');
  			var val =return_global_ajax_value( response[5]+"__"+response[1]+"__"+$('#cbo_sample_name').val()+"__"+$('#cbo_item_name').val(), 'populate_data_yet_to_cut', '', 'requires/sample_sewing_output_entry_controller');
...
{	if(type==1)
	...
	reset_form('','','txt_sewing_date*txt_reporting_hour*txt_sewing_qty*txt_reject_qnty*txt_remark','','');
	}
}
...
<div style="width:100%;">
	...
	<div style="width:930px; float:left;">
        ...
        <legend>Sample Sewing Output</legend>  
                ...
                <fieldset>
                    <table width="100%" border="0">
                        <tr>
                            ...
                            <td width="170"><?=create_drop_down( "cbo_company_name", 170, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/sample_sewing_output_entry_controller', this.value, 'load_drop_down_location', 'location_td' );" ); ?></td>
                            <td width="102"> &nbsp; Source</td>
                            <td width="170"><?=create_drop_down( "cbo_source", 170, $knitting_source,"", 1, "-- Select Source --", $selected, "load_drop_down( 'requires/sample_sewing_output_entry_controller', this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_sewing_output', 'sew_company_td' );", 0, '1,3' ); ?></td>
                            <td width="102" class="must_entry_caption">Sewing Company</td>
                            <td id="sew_company_td"><?=create_drop_down( "cbo_sewing_company", 170, $blank_array,"", 1, "--- Select ---", $selected, "" ); ?></td>
                        </tr>
                        <tr>  
                            <td class="must_entry_caption">Sample Req. No</td>
                            <td><input name="txt_sample_requisition_id" placeholder="Double Click to Search" id="txt_sample_requisition_id" onDblClick="openmypage('requires/sample_sewing_output_entry_controller.php?action=sample_requisition_popup&company='+document.getElementById('cbo_company_name').value,'Sample Requisition ID');"  class="text_boxes" style="width:155px " readonly>
                                <input type="hidden" id="mst_update_id" />	 
                                <input type="hidden" id="hidden_requisition_id" />	 
                            </td>
                            <td> &nbsp; Buyer</td>
                            <td><?=create_drop_down( "cbo_buyer_name", 170, "select id,buyer_name from lib_buyer where is_deleted=0 and status_active=1 order by buyer_name","id,buyer_name", 1, "-- Select Buyer --", $selected, "",1,0 ); ?></td>  
                            <td>Style</td>
                            <td><input name="txt_style_no" id="txt_style_no" class="text_boxes"  style="width:158px " disabled readonly></td>
                        </tr>
                        <tr> 
                            <td> Prod Qty</td>
                            <td><input name="txt_sample_qty" id="txt_sample_qty" class="text_boxes"  style="width:158px " disabled readonly></td>
                            <td>&nbsp;Location</td>
                            <td id="location_td"><?=create_drop_down( "cbo_location", 170,$blank_array,"", 1, "-- Select Location --", $selected, "" ); ?></td>
                            <td>Floor</td>
                            <td id="floor_td">
								<? 
                            ...
                            <legend>New Entry</legend>
                                <table  cellpadding="0" cellspacing="1" width="100%">
                                   ...
                                   <tr>
                                    ...
                                    <tr>
                                    	<td align="left" class="must_entry_caption">Sewing Date</td>
                                         <td width=""> 
                                         <input name="txt_sewing_date" id="txt_sewing_date" class="datepicker" type="text" value="<? echo date("d-m-Y")?>" style="width:138px;"  onChange="load_drop_down( 'requires/sample_sewing_output_entry_controller',document.getElementById('cbo_floor').value+'_'+document.getElementById('cbo_location').value+'_'+document.getElementById('prod_reso_allo').value+'_'+this.value, 'load_drop_down_sewing_line_floor', 'sewing_line_td' );" />
                                        </td>
                                     </tr>

                                     <tr>
                                        <td align="left" class="must_entry_caption">Sample Team</td> 
                                        <td id="team_td">            
                              			<?
                              			//echo create_drop_down( "cbo_sample_team", 150,"select id,team_name from lib_sample_production_team where is_deleted=0 and status_active=1 order by team_name","id,team_name", 1, "--Select Team--", $selected );		
							  			?>	
										<? echo create_drop_down( "cbo_sample_team", 150,$blank_array,"", 1, "Select Team", $selected, "",1 ); ?>
                              			</td> 
                              			<input type="hidden" name="hidden_sample_dtls_tbl_id" id="hidden_sample_dtls_tbl_id" value="">
                                   </tr>
                                   ...
                                   <tr>
                                        <td align="left" class="must_entry_caption">Sewing Quantity</td> 
                                        <td width="" valign="top"><input name="txt_sewing_qty" id="txt_sewing_qty" class="text_boxes_numeric"  style="width:138px" readonly >
                        ...
                        <td width="1%" valign="top">
                         ...
                         <td width="22%" valign="top">
                            ...
                            <legend>Display</legend>
                                ...
                                <tr>
                                        <td align="left" width="110" id="dynamic_cut_qty">Total Sew Qty</td>
                                        <td>
                                    ...
                                    <tr>
                                        <td align="left" width="110">Cumul. Sewing. Qty</td>
                                        <td>
                                            <input type="text" name="txt_cumul_sewing_qty" id="txt_cumul_sewing_qty" class="text_boxes_numeric" style="width:80px" readonly disabled /><input type="hidden" name="txt_cumul_rej_qty" id="txt_cumul_rej_qty" class="text_boxes_numeric" style="width:80px" readonly disabled />
  <input type="hidden" name="txt_cumul_previ_qty" id="txt_cumul_previ_qty" class="text_boxes_numeric" style="width:80px" readonly disabled />
                                     ...
                                     <tr>
                                        <td align="left" width="110">Yet to Sewing</td>
                                        <td>
                     ...
                     <tr>
		   				<td align="center" colspan="9" valign="middle" class="button_container">
							<?
							$date=date('d-m-Y');
                            echo load_submit_buttons( $permission, "fnc_sample_sewing_entry", 0,0,"reset_form('samplesewingoutput_1','list_view_country','','txt_sewing_date,".$date."*txt_challan,0','childFormReset()')",1); 
                            ?>
							<input type="button" class="formbutton" name="btn_ids" id="btn_ids" value="Print" onClick="fnc_sample_sewing_entry(4)"  style="width:80px"/>
