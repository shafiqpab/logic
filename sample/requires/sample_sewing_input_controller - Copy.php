...
if($action=="create_po_search_list_view")
{
	...
	if(count($delv_arr)>0)
	{
	$reqIdCond=wherenot_con_using_array($req_id,0,"sample_development_id");
	}
	
	$arr=array (1=>$buyer_arr,3=>$product_dept,5=>$dealing_marchant,6=>$requistion_qnty,7=>$output_qnty,8=>$bal_arr);

	if($data[7] == 1)
	{
		$sql= "SELECT id, requisition_number_prefix_num, company_id, buyer_name, style_ref_no, product_dept, dealing_marchant,internal_ref from sample_development_mst where id in(select sample_development_id from sample_sewing_output_mst where entry_form_id=127 and status_active=1 and is_deleted=0 $reqIdCond ) and entry_form_id in (117,203,449)  and status_active=1 $ref_cond and is_deleted=0 $company $style_id_in_cond  $buyer $style_id_cond $style_cond $estimated_shipdate order by id DESC";
	}
	else
	{
		$sql= "SELECT id, requisition_number_prefix_num, company_id, buyer_name, style_ref_no, product_dept, dealing_marchant,internal_ref from sample_development_mst where id in(select sample_development_id from sample_sewing_output_mst where entry_form_id=127 and status_active=1 and is_deleted=0 $reqIdCond ) and entry_form_id in (117,203,449) and status_active=1 $ref_cond and is_deleted=0 $company $style_id_in_cond   $buyer $style_id_cond $style_cond $estimated_shipdate order by id DESC";
	...
	//echo $sql;
	echo  create_list_view("list_view", "Req No,Buyer Name,Style Name,Product Department,Int. Ref. No ,Dealing Merchant,Req. Qty,Prod. Qty.,Bal.Qty", "60,120,120,90,80,120,90,90,90,90","900","240",0, $sql , "js_set_value", "id", "", 1, "0,buyer_name,0,product_dept,0,dealing_marchant,id,id,id", $arr , "requisition_number_prefix_num,buyer_name,style_ref_no,product_dept,internal_ref,dealing_marchant,id,id,id", "",'',"0,0,0,0,0,0,0,0") ;
	exit();
}
...
if($action=="color_and_size_level")
{
 	...
 	if(trim($is_exist_emb)=='' && $val_req_embel=='')
	{
		...
		$total_cuml=return_field_value("sum(b.qc_pass_qty) as qc_pass_qty","sample_sewing_output_dtls b,sample_sewing_output_mst a","a.id=b.sample_sewing_output_mst_id and  b.sample_name=$smp_id and b.item_number_id=$gmts and a.sample_development_id=$req_id  and b.status_active=1 and b.is_deleted=0 and b.entry_form_id=337  and a.entry_form_id=337 and b.sample_dtls_row_id=$sample_dtls_part_tbl_id and  a.sample_development_id=$req_id ");
	...
	else
	{
		...
		if($serial==0 || $serial=='')
		{
  		...
  		//$total_cut=return_field_value("sum(qc_pass_qty)","sample_sewing_output_dtls","sample_name=$smp_id and item_number_id=$gmts and status_active=1 and is_deleted=0 and entry_form_id=128 and sample_dtls_row_id=$sample_dtls_part_tbl_id and embel_name=$last_emb");
			$total_cut=return_field_value("sum(b.qc_pass_qty)","sample_sewing_output_dtls b,sample_sewing_output_mst a"," a.id=b.sample_sewing_output_mst_id and b.sample_name=$smp_id and b.item_number_id=$gmts and b.status_active=1 and b.is_deleted=0 and b.entry_form_id=128 and b.sample_dtls_row_id=$sample_dtls_part_tbl_id and b.embel_name=$last_emb");

  		//$total_cuml=return_field_value("sum(qc_pass_qty)","sample_sewing_output_dtls","sample_name=$smp_id and item_number_id=$gmts and status_active=1 and is_deleted=0 and entry_form_id=337 and sample_dtls_row_id=$sample_dtls_part_tbl_id");
		$total_cuml=return_field_value("sum(b.qc_pass_qty)","sample_sewing_output_dtls b,sample_sewing_output_mst a "," a.id=b.sample_sewing_output_mst_id and b.sample_name=$smp_id and b.item_number_id=$gmts and b.status_active=1 and b.is_deleted=0 and b.entry_form_id=337 and a.sample_development_id=$req_id  and b.sample_dtls_row_id=$sample_dtls_part_tbl_id");
...
if($action=="populate_input_form_data")
{
	...
	else
	{
		...
		//echo "select c.color_id,c.size_id,c.size_pass_qty from sample_sewing_output_mst a, sample_sewing_output_dtls b, sample_sewing_output_colorsize c  where a.id=b.sample_sewing_output_mst_id and b.id=c.sample_sewing_output_dtls_id and b.sample_name=".$dtlsArr[$dtls_id]['sample_name']." and b.item_number_id=".$dtlsArr[$dtls_id]['item_number_id']." and a.entry_form_id=128 and b.entry_form_id=128 and c.entry_form_id=128 and b.sample_dtls_row_id=$sample_dtls_row_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.embel_name=$last_emb";
		$total_cut=return_field_value("sum(b.qc_pass_qty)","sample_sewing_output_dtls b,sample_sewing_output_mst a"," a.id=b.sample_sewing_output_mst_id and b.sample_name=$sample_name and b.item_number_id=$gmts and b.status_active=1 and b.is_deleted=0 and b.entry_form_id=128 and b.sample_dtls_row_id=$sample_dtls_row_id and b.embel_name=$last_emb");
		$total_cuml=return_field_value("sum(b.qc_pass_qty)","sample_sewing_output_dtls b,sample_sewing_output_mst a","b.sample_name=$sample_name and b.item_number_id=$gmts and b.status_active=1 and b.is_deleted=0 and b.entry_form_id=337 and  a.sample_development_id=$smp_id and b.sample_dtls_row_id=$sample_dtls_row_id");
	}
