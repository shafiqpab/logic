...
if($action=="report_generate")
{
	...
	//echo $master_id_1;
 	$sql_file_location_1="select master_tble_id,image_location from common_photo_library where form_name='sample_requisition_1' and master_tble_id in ($master_id_1)";
	//echo $sql_file_location_1;
	$sql_file_location_res_1= sql_select($sql_file_location_1);
	$file_location_1=array();
	foreach($sql_file_location_res_1 as $val){
		$file_location_1[$val['MASTER_TBLE_ID']]=$val['IMAGE_LOCATION'];
	}
	 ...
	 $sql_req_res_2=sql_select($sql_req_2);
	 $master_id_2="";
 
	 foreach($sql_req_res_2 as $val){		 
		 $master_id_2 .= "'".$val['ID']."'";
	 }
	  $sql_file_location_2="select master_tble_id,image_location from common_photo_library where form_name='sample_requisition_1' and master_tble_id in ($master_id_2)";
	 $sql_file_location_res_2= sql_select($sql_file_location_2);
	 $file_location_2=array();
	 foreach($sql_file_location_res_2 as $val){
		 $file_location_2[$val['MASTER_TBLE_ID']]=$val['IMAGE_LOCATION'];
	 }
    ...
    <form name="requisitionApproval_2" id="requisitionApproval_2">
        ...
        <legend>Sample Requisition Acknowledge</legend>	
            ...
            <div style="width:1490px; overflow-y:scroll; max-height:330px;" id="buyer_list_view" align="center">
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1470" class="rpt_table" id="tbl_list_search" align="left">
                    <tbody>
                        <? 
                            $i=1;
                            $nameArray="";
							...
							$sampChecklist=sql_select("SELECT b.checklist_number,a.id from sample_development_mst a join  sample_checklist_mst b on a.id=b.requisition_id where  a.status_active=1 and a.is_deleted=0 and a.company_id=$company_name and b.status_active=1 and b.is_deleted=0 and a.entry_form_id in (117,203,449)");
 							foreach ($sampChecklist as $val)
							{
								$samplCheckArr[$val[csf('id')]]=$val[csf('checklist_number')];
							}
                            ...
                            { 
								...
								<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>"> 
									...
									<? 
										if($file_location[$row['ID']]==null){
											
									?>
										<td width="70" align="center" style="color:blue;text-decoration:underline;">No File</td>
									<?
										}
										else{
											
									?>
										<td width="70" align="center"><a href="<?=base_url($file_location[$row['ID']]) ?>">File</a></td>
									<?
										}
									?>
									...
									<td width="100" align="center"><p><? $chk_req=$row[csf('id')];$chk_list=$samplCheckArr[$row[csf('id')]];echo "<p><a href='##'  onclick=\"openmypage_checklist( '$chk_req' ,'checklist_popup','$chk_list');\" >$chk_list </a></p>"; ?></p></td>
            ...
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1490" class="rpt_table" style="margin-left: -18px;">
				<tfoot>
                    <td width="50" align="center" ><input type="checkbox" id="all_check" onClick="check_all('all_check')" /></td>
                    <td colspan="17" align="left"><input type="button" value="<? if($approval_type==1) echo "Un-Acknowledge"; else echo "Acknowledge"; ?>" class="formbutton" style="width:100px" onClick="submit_approved(<? echo $i; ?>,<? echo $approval_type; ?>)"/></td>
				</tfoot>
			</table>
        </fieldset>
    </form>  
 ...
 ?>
    ...
    <legend> Checklist Info</legend>
     <div style="width:350px; margin-top:10px">
        ...
        <?
             $sql= sql_select("select checklist_id,requisition_id from sample_checklist_dtls where status_active=1 and is_deleted=0 and requisition_id='$chk_req'");
              $i=1;
             foreach($sql as $row)
             {
                ?>
                <tr>
                    ...
                    <td width="" align="center"><strong><? echo  $sample_checklist_set[$row[csf('checklist_id')]]; ?> </strong></td>
