...
<?	
	...
	$arr=array (3=>$sample_category,5=>$sample_source,6=>$supplier_name);
	
	echo  create_list_view ( "list_view", "System Id,Receive Date,Style Ref,Category,Item,Source,Supplier,Designer,Quantity,Barcode", "100,100,100,50,100,100,100,160","1000","220",0, "select a.id,a.receive_date,a.category_id,a.item_id,a.source_id,a.produced_by_id,a.designer,a.quantity,a.style_ref,b.barcode,b.mst_id from sample_receive_mst a,sample_receive_dtls b where a.id=b.mst_id and a.is_deleted=0 and b.is_deleted=0 order by a.id", "js_set_value", "id", "'load_php_data_to_form'", 1, "0,0,0,category_id,0,source_id,produced_by_id", $arr , "mst_id,receive_date,style_ref,category_id,item_id,source_id,produced_by_id,designer,quantity,barcode", "requires/sample_receive_controller", 'setFilterGrid("list_view",-1);','0,3,0,0,0,0,0,0' ) ;         
}
...
if ($action=="load_php_data_to_form")
{
	...
	$nameArray=sql_select( "select id,receive_date,category_id,item_id,source_id,produced_by_id,designer,quantity,fabric_nature,construction,composition,gsm,yarn_count_id,yarn_type_id,style_ref,status_active from sample_receive_mst where id='$data'" );
	$dtls_data="";
...
if ($action=="save_update_delete")
{
		...
		$color_library=return_library_array( "select id,color_name from lib_color where status_active=1", "id", "color_name"  );
		$size_name_array=return_library_array( "select id,size_name from lib_size where status_active=1", "id", "size_name"  );
	...
	$uploadFileNameArr=explode("***",$uploadFileName);		
	$index=0;
	foreach($color_break_down_tr as $tr_value)
			{
				...
				$sample_pic=$uploadFileNameArr[$index];
				...
				{ 
					...
					{
						$color_id = return_id( str_replace("'","",$colorName), $color_library, "lib_color", "id,color_name","410");  
				...
				{ 
					...
					{
						$size = return_id( str_replace("'","",$sizeName), $size_name_array, "lib_size", "id,size_name","410");  
						$new_array_color[$color_id]=str_replace("'","",$sizeName);
					}
					else $size =  array_search(str_replace("'","",$sizeName), $new_array_color); 
				}
				else $size=0;
		...
		{   // Update Here
			...
			$dtls_id=return_next_id( "id","sample_receive_dtls", 1 ) ; 
			$field_array_dtls_insert="id,mst_id,color_id,size_id,quantity,expected_price,amount,barcode,sample_pic,inserted_by,insert_date,status_active,is_deleted,prod_year";
			...
			foreach($color_break_down_tr as $tr_value)
			{
				...
				$array = explode('.',$sample_pic);
				$extension = end($array);
				if($extension!='') {$sample_pic_file="*sample_pic";}else{$sample_pic_file="";}
				 ...
				 if($dtls_update_id!='')
				 {
					if($extension!='') {$sample_pic_val=",'".$sample_pic."'";}else{$sample_pic_val="";}
					 $dtls_update_id_arr[]=$dtls_update_id;
					 $dtls_update_id_arr2[]=$dtls_update_id;
					 $data_array_dtls_update[$dtls_update_id]=explode(",",($color_id.",".$size.",".$qty.",".$rate.",".$amount.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."'$sample_pic_val"));
				 ...
				 {
					 if($i==0) $comma_set=''; else $comma_set='*';
					 $data_array_dtls_insert.="$comma_set (".$dtls_id.",".$update_id.",'".$color_id."','".$size."','".$qty."','".$rate."','".$amount."','".$barcode."','".$sample_pic."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_status.",0,".date("y",strtotime(str_replace("'","",$receive_date))).")";
					$i++;
				...
				$field_array_dtls_update="color_id*size_id*quantity*expected_price*amount*updated_by*update_date $sample_pic_file";
...
<script type="text/javascript">
	...
	var str_color = [<? echo substr(return_library_autocomplete( "select distinct(color_name) from lib_color where status_active=1","color_name" ), 0, -1); ?>];
	var str_size = [<? echo substr(return_library_autocomplete( "select distinct(size_name) from lib_size where status_active=1","size_name" ), 0, -1); ?>];
	//alert(str_color);
...
<body>
    <div align="center"  style="padding:0px; margin:0">
        ...
        <input type="hidden" name="totNumber" id="totNumber" value="" />
                ...
                <table width="60%" border="0" cellspacing="0" cellpadding="0" class="rpt_table" id="tbl_qty" rules="all">
                    <thead>
                        <tr>
                            ...
                            <th width="60" colspan="4">Sample Picture</th>
                        </tr>
                    </thead>
                    <tbody>
                    <?php 
						 ...
						 ?>
							<tr id="break_<?php echo $j;?>"><!--onClick="tr_index(this)"-->
	                           ...
	                           <input type="hidden" name="updatedtlsid_<?php echo $j;?>" id="updatedtlsid_<?php echo $j;?>" value="<?php echo $rows[csf('id')];?>" />      
	                            ...
	                            <input type="file" id="samplepic_<?php echo $j;?>"  name="samplepic_<?php echo $j;?>" style="width:192px" value="<? $rows['sample_pic']?>" src="">
	                    		</td>
	                            <td><? echo $rows['sample_pic']?></td>
                                <td height="25" valign="middle">
	                            <!--<input type="text" id="display_<?php //echo $j;?>"  name="display_<?php echo $j;?>" style="width:192px" value="<? $rows['show_hide_img']?>" >-->
                                 <select name="display_<?php echo $j;?>" id="display_<?php echo $j;?>" style="width:50px" >
					...
					?>
                        <tr id="break_1"><!--onClick="tr_index(this)"-->
                           ...
                           <td>
                            ...
                            <td height="25" valign="middle">
                    	<input type="file" id="samplepic_1" name="samplepic_1"  style="width:192px" value="" src="">
                    		</td>
                            <td height="25" valign="middle">
                                <select name="display_1" id="display_1" style="width:50px">
                                  <option value="0">select</option>
                                  <option value="1">Show</option>
                                </select>
	                    		</td>
                   ...
                   <?php 
                    ...
                    <tfoot>
                    <tr>
                    <td id="hdn_pic"><input type="hidden" name=""></td>
                    <td></td>
...
if($action=="smp_image_upload")
{
	...
	$i=0;
	foreach($fileName as $appendFile){
		$newFileName = $_FILES[$appendFile]["name"]; // The file name
		$fileTmpLoc = $_FILES[$appendFile]["tmp_name"]; // File in the PHP tmp folder
		$fileType = $_FILES[$appendFile]["type"]; // The type of file it is
		$fileSize = $_FILES[$appendFile]["size"]; // File size in bytes
		$fileErrorMsg = $_FILES[$appendFile]["error"]; // 0 for false... and 1 for true
		
		move_uploaded_file($fileTmpLoc, "uploads/".$imageName[$i]);
		$i++;
	}
exit();
}
...
if($action=="sample_receive_print")
{
          ...
          foreach($barcode_arr as $barcode=>$qnty)
          {
              for( $m=0; $m<$qnty; $m++ )
              {
                  $mn=$i%2;
                   ...
                   if( $mn==0 ){
					$pdf->AddPage();
					$style_ref="Style Ref:".$row[csf('style_ref')];
                    $item_des=$row[csf('composition')].','.$row[csf('gsm')];
					
                    $pdf->Code39(10, 1, "$barcode", $ext = '', $cks = '', $w = 0.22, $h = 7);
                    $pdf->Code39(10, 1,$style_ref, $ext = true, $cks = false, $w = 0.2, $h = 10, $wide = true, true,9) ;  
                    $pdf->Code39(10, 5,$item_des, $ext = true, $cks = false, $w = 0.2, $h = 10, $wide = true, true,9) ;
					}
					else
					{
                    $style_ref="Style Ref:".$row[csf('style_ref')];
                    ...
                    $pdf->Code39(51, 1, "$barcode", $ext = '', $cks = '', $w = 0.22, $h = 7);
                    $pdf->Code39(51, 1,$style_ref, $ext = true, $cks = false, $w = 0.2, $h = 10, $wide = true, true,9) ;  
                    $pdf->Code39(51, 5,$item_des, $ext = true, $cks = false, $w = 0.2, $h = 10, $wide = true, true,9) ; 
					}
	...
	$name = 'barcode_' . date('j-M-Y_h-iA') . '.pdf';
	$pdf->Output( "".$name, 'F');
	$url='requires/'.$name;
	
	?>
	<script> window.open("<? echo $url;?>", "_self");</script>
	<?php
	exit();
