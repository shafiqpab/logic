...
if ($action=="load_php_data_to_form")
{
	...
	foreach ($nameArray as $inf)
	{
		...
		$save_string='';
		$sql_dels=sql_select("select id,color_id,size_id,quantity,expected_price,amount,barcode from sample_receive_dtls where mst_id='$data' and is_deleted=0" );
	...
	foreach($color_break_down_tr as $tr_value)
			{
				...
				$data_array_dtls.="$comma_set (".$dtls_id.",".$id.",'".$color_id."','".$size."','".$qty."','".$rate."','".$amount."','".$barcode."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$cbo_status.",0,".date("y",strtotime(str_replace("'","",$receive_date))).")";
				$dtls_id++;
				$i++;
				$barcode=$barcode+1;
			}
...
<body>
    <div align="center"  style="padding:0px; margin:0">
        ...
        <fieldset style="width:90%;">
            <legend></legend>
            <form name="smapleqty_1" id="smapleqty_1">
                <table width="60%" border="0" cellspacing="0" cellpadding="0" class="rpt_table" id="tbl_qty" rules="all">
                    ...
                    <?php 
				  	...
				  	$sql_dels=sql_select( "select id,mst_id,color_id,size_id,quantity,expected_price,amount,barcode from sample_receive_dtls where mst_id='$update_id' and is_deleted=0" );
					if( count($sql_dels))
					{
					 $color_break_down_tr=explode("_",$color_qty_breakdown);
					 $j=0;
...
{
	//echo "select id,mst_id,color_id,size_id,quantity,expected_price,amount,barcode from sample_receive_dtls where mst_id='$data'";
	...
	$sql_dels=sql_select( "select id,mst_id,color_id,size_id,quantity,expected_price,amount,barcode from sample_receive_dtls where is_deleted=0 and mst_id='$data'" );
     ...
     <style>
		...
		@media print and (width: 5cm) and (height: 5cm) {
			 @page {
				margin: 1cm;
			 }
		}		
          ...
          foreach($barcode_arr as $barcode=>$qnty)
          {
              for( $m=0; $m<$qnty; $m++ )
              {
                  ...
                  ?>
                      ...
                      <td>
                          <table cellpadding="0" cellspacing="0" style=" width:136.062992126px; height:93px; <? echo $wid; ?> line-height: 1.8;  ">
                              <tr>
                              ...
                              <tr>
                              	<td style="float:left;margin-left:8px; margin-top:0px;"><strong >Style Ref:&nbsp;</strong></td>
                              	<td style="float:left;" ><? echo $row[csf('style_ref')]; ?></td>
                              </tr>
                              <tr>
				...
				/*if( $i % 2 == 0)
				{
				   echo "<tr class=\"breakAfter\"></tr><tr>";
				} 
				 */
              }
