...
if($action=="populate_data_yet_to_cut")
{
	...
	$val=return_field_value("sum(qc_pass_qty)","sample_sewing_output_dtls","sample_sewing_output_mst_id=$data[1] and sample_dtls_row_id=$data[0] and entry_form_id=673");
	echo $val;
	exit();
}
...
{
	echo create_drop_down( "cbo_location", 150, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id='$data' order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down( 'requires/sample_packing_finishing_entry_controller', this.value, 'load_drop_down_floor', 'floor_td' );" );
	exit();
}

if ($action=="load_drop_down_floor")
{
	echo create_drop_down( "cbo_floor", 150, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process in (11) order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "",0 );
	exit();
}
...
if($action=="load_drop_down_finishing")
{
	...
	if($data==3)
	{
		if($db_type==0)
		{
			echo create_drop_down( "cbo_finishing_company", 150, "select id,supplier_name from lib_supplier where status_active=1 and is_deleted=0 and find_in_set(22,party_type) order by supplier_name","id,supplier_name", 1, "-Fin. Company-", $selected, "get_php_form_data(this.value+'**'+$data+'**'+$('#hidden_po_break_down_id').val()+'**'+$('#cbo_item_name').val()+'**'+$('#cbo_country_name').val(),'display_bl_qnty','requires/sample_packing_finishing_entry_controller');",0,0 );
		}
		else
		{
			echo create_drop_down( "cbo_finishing_company", 150, "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=22 and a.status_active=1 group by a.id,a.supplier_name order by a.supplier_name","id,supplier_name", 1, "-Fin. Company-", $selected, "get_php_form_data(this.value+'**'+$data+'**'+$('#hidden_po_break_down_id').val()+'**'+$('#cbo_item_name').val()+'**'+$('#cbo_country_name').val(),'display_bl_qnty','requires/sample_packing_finishing_entry_controller');",0,0 );
		}
	}
	else if($data==1)
	{
 		echo create_drop_down( "cbo_finishing_company", 150, "select id,company_name from lib_company comp where is_deleted=0 and status_active=1 $company_cond order by company_name","id,company_name", 1, "-Fin. Company-", $selected_company,  "load_drop_down( 'requires/sample_packing_finishing_entry_controller', this.value, 'load_drop_down_location', 'location_td' );",0,0 );
	}
 	else
	{
		echo create_drop_down( "cbo_finishing_company", 150, $blank_array,"", 1, "-Fin. Company-", $selected, "",0,0 );
	}
	exit();
}
...
<body>
            ...
            <table width="1000" cellspacing="0" cellpadding="0" class="rpt_table" align="center" border="1" rules="all">
                ...
                <tr class="general">
                    <td>
                        ...
                        <?=create_drop_down( "cbo_company_mst", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", $company,"load_drop_down( 'sample_packing_finishing_entry_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" ); ?></td>
    ...
    <script>
        load_drop_down( 'sample_packing_finishing_entry_controller',<? echo $company; ?>, 'load_drop_down_buyer', 'buyer_td' );
    </script>
    </html>
    <?
    exit();
...
if($action=="create_po_search_list_view")
{
	...
	$output_qnty=return_library_array( "SELECT sample_development_id , sum(qc_pass_qty) as qc_pass_qty from sample_sewing_output_mst a,sample_sewing_output_dtls b  where a.id=b.sample_sewing_output_mst_id and a.entry_form_id=673 and b.entry_form_id=673 and  b.status_active=1 and  a.status_active=1 group by sample_development_id ",'sample_development_id','qc_pass_qty');
	$bal_arr=array();
	...
	if($data[7] == 1){
		$sql= "SELECT id,requisition_number_prefix_num,to_char(insert_date,'YYYY') as year,company_id,buyer_name,style_ref_no,product_dept,dealing_marchant,internal_ref from sample_development_mst where id in(select sample_development_id from sample_sewing_output_mst where status_active=1 and is_deleted=0 $reqCond2) and entry_form_id in (117,203,449,341)  and status_active=1 and is_deleted=0 $ref_cond $company $style_id_in_cond $buyer $style_id_cond $style_cond $estimated_shipdate order by id asc";
	}
	else{
		$sql= "SELECT id,requisition_number_prefix_num,to_char(insert_date,'YYYY') as year,company_id,buyer_name,style_ref_no,product_dept,dealing_marchant,internal_ref from sample_development_mst where id in(select sample_development_id from sample_sewing_output_mst where  status_active=1 and is_deleted=0 $reqCond) and entry_form_id in (117,203,449,341)  $ref_cond and status_active=1 and is_deleted=0 $company $style_id_in_cond $buyer $style_id_cond $style_cond $estimated_shipdate order by id asc";
	}
 // echo $sql;
...
if($action=="populate_data_from_search_popup")
{
  	...
  	foreach($res as $result)
	{
	    ...
	    echo "load_drop_down( 'requires/sample_packing_finishing_entry_controller', '".$result[csf("company_id")]."', 'load_drop_down_location', 'location_td' );\n";
	...
	$smp_mst_id = sql_select("select id,company_id,production_source,sewing_company,location,floor_id from sample_sewing_output_mst where sample_development_id=$data and entry_form_id=673 and status_active=1 and is_deleted=0");
	if(count($smp_mst_id)>0)
	{
		echo "load_drop_down('requires/sample_packing_finishing_entry_controller', '".$smp_mst_id[0][csf('production_source')].'**'.$smp_mst_id[0][csf('company_id')]."', 'load_drop_down_finishing', 'sew_company_td' );";
		
		echo "$('#mst_update_id').val('".$smp_mst_id[0][csf('id')]."');\n";
		echo "$('#cbo_source').val('".$smp_mst_id[0][csf('production_source')]."');\n";
		echo "$('#cbo_finishing_company').val('".$smp_mst_id[0][csf('sewing_company')]."');\n";
		echo "$('#cbo_location').val('".$smp_mst_id[0][csf('location')]."');\n";
		echo "load_drop_down( 'requires/sample_packing_finishing_entry_controller', '".$smp_mst_id[0][csf("location")]."', 'load_drop_down_floor', 'floor_td' );\n";
		echo "$('#cbo_floor').val('".$smp_mst_id[0][csf('floor_id')]."');\n";
	}

	$smp_team = sql_select("SELECT b.SAMPLE_TEAM from sample_sewing_output_mst a, sample_sewing_output_dtls b where a.sample_development_id=$data and a.id=b.sample_sewing_output_mst_id and a.entry_form_id=130 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 ");
	$teamIds = '';
	...
	echo "load_drop_down('requires/sample_packing_finishing_entry_controller', '".$teamIds."', 'load_drop_down_sample_team', 'sample_team_td' );";
...
if($action=="show_sample_item_listview")
{
 ...
 c where a.id=b.sample_mst_id and b.id=c.dtls_id and a.id=$data and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by b.sample_name,b.sample_color,b.gmts_item_id,b.id order by b.id asc");
		...
		?>
			<tr bgcolor="<?=$bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="put_sample_item_data(<?=$row[csf('id')];?>,<?=$row[csf('sample_name')]; ?>,<?=$row[csf('gmts_item_id')]; ?>);">
				<td><?=$i; ?></td>
				<td style="word-break:break-all"><?=$sample_name_library[$row[csf('sample_name')]]; ?></td>
				<td style="word-break:break-all"><?=$item_arrs[$row[csf('gmts_item_id')]]; ?></td>
				<td style="word-break:break-all"><?=$color_library[$row[csf('sample_color')]]; ?></td>
				<td align="right"><?=$row[csf('size_qty')]; ?></td>
			</tr>
		<?
			$i++;
		}
...
if($action=="color_and_size_level")
{
	...
	$is_exist_emb=return_field_value("id","sample_sewing_output_dtls","sample_name=$smp_id and item_number_id=$gmts and sample_dtls_row_id=$sample_dtls_part_tbl_id and status_active=1 and is_deleted=0 and entry_form_id=130");
	...
	//$last_emb=return_field_value("embel_name","sample_sewing_output_dtls","sample_name=$smp_id and item_number_id=$gmts and sample_dtls_row_id=$sample_dtls_part_tbl_id and status_active=1 and is_deleted=0 and entry_form_id=130 and rownum=1 order by id desc ");
	if($db_type==0)
	{
 	...
 	if(trim($is_exist_emb)=='' && $val_req_embel=='')
	{
  		...
  		$colorResult_qc_pass = sql_select("select b.sample_name, c.color_id, c.size_id, c.size_pass_qty, c.size_rej_qty from sample_sewing_output_mst a, sample_sewing_output_dtls b, sample_sewing_output_colorsize c
		where a.id=b.sample_sewing_output_mst_id and b.id=c.sample_sewing_output_dtls_id and b.sample_dtls_row_id=$sample_dtls_part_tbl_id and  a.sample_development_id=$req_id and b.sample_name=$smp_id and b.item_number_id=$gmts and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form_id=673 and b.entry_form_id=673 and c.entry_form_id=673 order by c.id asc");
		...
		if($serial==0 || $serial=='')
		{
			echo "alert('Sewing data not available for this sample and item');\n";
		}
		...
		$total_cuml=return_field_value("sum(qc_pass_qty)","sample_sewing_output_dtls","sample_name=$smp_id and item_number_id=$gmts and status_active=1 and is_deleted=0 and entry_form_id=673 and sample_dtls_row_id=$sample_dtls_part_tbl_id");
		 echo "$('#dynamic_cut_qty').html('Tot Finishing Qty');\n";
	}
	else
	{
		// echo "$('#txt_sample_requisition_id').val('33');\n"; die;
		$colorResult_qc_pass = sql_select("select b.sample_name, c.color_id, c.size_id, c.size_pass_qty, c.size_rej_qty from sample_sewing_output_mst a, sample_sewing_output_dtls b, sample_sewing_output_colorsize c
		where a.id=b.sample_sewing_output_mst_id and b.id=c.sample_sewing_output_dtls_id and b.sample_dtls_row_id=$sample_dtls_part_tbl_id and  a.sample_development_id=$req_id and b.sample_name=$smp_id and b.item_number_id=$gmts and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form_id=673 and b.entry_form_id=673 and c.entry_form_id=673 ");
		...
		if($serial==0 || $serial=='')
		{
			echo "alert('as per requisitions you have to Sewing Output first then Packing And Finishing.');\n";
		}

  		$total_cut=return_field_value("sum(qc_pass_qty)","sample_sewing_output_dtls","sample_name=$smp_id and item_number_id=$gmts and status_active=1 and is_deleted=0 and entry_form_id=130 and sample_dtls_row_id=$sample_dtls_part_tbl_id");

  		$total_cuml=return_field_value("sum(qc_pass_qty) as qc_pass_qty","sample_sewing_output_dtls","sample_name=$smp_id and item_number_id=$gmts and status_active=1 and is_deleted=0 and entry_form_id=673 and sample_dtls_row_id=$sample_dtls_part_tbl_id","qc_pass_qty");
  		echo "$('#dynamic_cut_qty').html('Tot Finishing Qty');\n";
	}
	...
	echo "$('#txt_cumul_finishing_qty').val('');\n";
	echo "$('#txt_total_cutting_qty').val('');\n";
	echo "$('#txt_yet_to_finishing').val('');\n";
	echo "$('#txt_reporting_hour').val('');\n";
	echo "$('#txt_finishing_qty').val('');\n";
	...
	$qty=return_field_value("sum(total_qty)","sample_development_size","mst_id=$req_id and dtls_id=$sample_dtls_part_tbl_id and status_active=1 and is_deleted=0");
  	...
  	//$total_cuml=return_field_value("sum(qc_pass_qty)","sample_sewing_output_dtls","sample_name=$smp_id and item_number_id=$gmts and status_active=1 and is_deleted=0 and entry_form_id=130 and sample_dtls_row_id=$sample_dtls_part_tbl_id and embel_name=$embel_name");
	...
	echo "$('#txt_cumul_finishing_qty').val(".$total_cuml.");\n";
	...
	echo "var qcqty=$('#txt_cumul_finishing_qty').val();\n";
	echo "$('#txt_yet_to_finishing').val(total_cuts-qcqty);\n";
	...
	<div style="width:610px;">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="600px" class="rpt_table">
            <thead>
				...
				<th width="40">Rep. Hour</th>
				<th>Remarks</th>
            </thead>
		</table>
	</div>
	<div style="width:600px; max-height:180px; overflow-y:scroll" id="sewing_production_list_view">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="580px" class="rpt_table" id="sample_detail_tbl">
		<?php
		$i = 1;
		...
		foreach ($sqlResult as $row) {
			...
			$click_val = "get_php_form_data($data_val,'populate_input_form_data','requires/sample_packing_finishing_entry_controller')";
			...
			<tr bgcolor="<?=$bgcolor; ?>" style="text-decoration:none; cursor:pointer">
                <td width="30" align="center">
                	<input type="checkbox" id="check_for_print_<?=$i; ?>" name="check_for_print" value="<?=$row[csf('dtls_id')]; ?>"/>
                </td>
                <td onClick="<? echo $click_val;?>" width="20" align="center"><?=$i; ?></td>
                <td onClick="<? echo $click_val;?>" width="100" style="word-break:break-all"><?=$sample_name_library[$row[csf('sample_name')]]; ?></td>
                <td onClick="<? echo $click_val;?>" align="center" width="80" style="word-break:break-all"><?=$color_library[$color_id]; ?></td>
                <td onClick="<? echo $click_val;?>" width="60" align="center"><?=change_date_format($row[csf('sewing_date')]); ?></td>
                <td onClick="<? echo $click_val;?>" width="60" align="right"><?=$row[csf('qc_pass_qty')]; ?></td>
                <td onClick="<? echo $click_val;?>" width="40" align="right"><?=$row[csf('reject_qty')]; ?></td>
                <td onClick="<? echo $click_val;?>" width="40"><?=$row[csf('reporting_hour')]; ?></td>
                <td onClick="<? echo $click_val;?>" style="word-break:break-all"><?=$row[csf('remarks')]; ?></td>
			</tr>
			<?php
			$i++;
		}
...
if($action=="populate_input_form_data")
{
 	...
 	$is_exist_emb=return_field_value("id","sample_sewing_output_dtls","sample_name=$sample_name and item_number_id=$gmts and sample_dtls_row_id=$sample_dtls_row_id and status_active=1 and is_deleted=0 and entry_form_id=130");
	...
	echo "$('#txt_finishing_date').val('".change_date_format($dtlsArr[$dtls_id]['sewing_date'])."');\n";
	echo "$('#txt_reporting_hour').val('".$dtlsArr[$dtls_id]['reporting_hour']."');\n";
	echo "$('#txt_finishing_qty').val('".$dtlsArr[$dtls_id]['qc_pass_qty']."');\n";
	...
	else
	{
		...
		$total_cut=return_field_value("sum(qc_pass_qty) as qc_pass_qty","sample_sewing_output_dtls","sample_name=$sample_name and item_number_id=$gmts and status_active=1 and is_deleted=0 and entry_form_id=130 and sample_dtls_row_id=$sample_dtls_row_id and embel_name=$last_emb","qc_pass_qty");
		$total_cuml=return_field_value("sum(b.qc_pass_qty) as qc_pass_qty","sample_sewing_output_mst a,sample_sewing_output_dtls b","a.id=b.sample_sewing_output_mst_id and a.sample_development_id=$smp_id and b.sample_name=$sample_name and b.item_number_id=$gmts and b.status_active=1 and b.is_deleted=0 and b.entry_form_id=673 and b.sample_dtls_row_id=$sample_dtls_row_id","qc_pass_qty");
		$total_cuml_rej=return_field_value("sum(b.reject_qty) as reject_qty","sample_sewing_output_mst a,sample_sewing_output_dtls b","a.id=b.sample_sewing_output_mst_id and a.sample_development_id=$smp_id and b.sample_name=$sample_name and b.item_number_id=$gmts and b.status_active=1 and b.is_deleted=0 and b.entry_form_id=673 and b.sample_dtls_row_id=$sample_dtls_row_id","reject_qty");
	}
	...
	echo "set_button_status(1, permission, 'fnc_sample_finishing_entry',1,0);\n";
	echo "$('#breakdown_td_id').html('".addslashes($colorHTML)."');\n";
	$colorList = substr($colorID,0,-1);
	echo "$('#hidden_colorSizeID').val('".$colorList."');\n";
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert part----------------------------------------------------------
	{
		...
		{
			$msg="Sample Packing And Finishing($previous_sam_qty) entry should not allow to save more than quantity($previous_sample_qty)";
		...
		if($mst_update_id=='')
		{
			...
			$data_array_mst="(".$mst_id.",".$cbo_company_name.",".$hidden_requisition_id.",".$cbo_source.",".$cbo_finishing_company.",".$cbo_location.",".$cbo_floor.",".$user_id.",'".$pc_date_time."','1','0',673)";
			...
			$data_array_dtls="(".$dtls_id.",".$mst_id.",".$cbo_sample_name.",".$hidden_sample_dtls_tbl_id.",".$cbo_item_name.",".$txt_finishing_date.",".$txt_reporting_hour.",".$txt_finishing_qty.",".$txt_reject_qnty.",".$txt_remark.",".$user_id.",'".$pc_date_time."','1','0',673,".$txt_sample_qty.",".$cbo_sample_team.",".$txt_total_cutting_qty.")";
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				if($j==0)$data_array_brk = "(".$colorsize_brk_id.",".$mst_id.",".$dtls_id.",'".$colorID."','".$sizeID."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$user_id.",'".$pc_date_time."','1','0',673)";
				else $data_array_brk .= ",(".$colorsize_brk_id.",".$mst_id.",".$dtls_id.",'".$colorID."','".$sizeID."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$user_id.",'".$pc_date_time."','1','0',673)";
				$colorsize_brk_id+=1;
				$j++;
			}
		...
		else
		{
			...
			$dtls_id=return_next_id("id", "sample_sewing_output_dtls", 1);
  			$field_array_dtls="id, sample_sewing_output_mst_id, sample_name,sample_dtls_row_id,item_number_id, sewing_date,reporting_hour, qc_pass_qty,reject_qty, remarks, inserted_by, insert_date, status_active, is_deleted,entry_form_id,prod_qty,sample_team,total_cut_qty";
			$data_array_dtls="(".$dtls_id.",".$mst_update_id.",".$cbo_sample_name.",".$hidden_sample_dtls_tbl_id.",".$cbo_item_name.",".$txt_finishing_date.",".$txt_reporting_hour.",".$txt_finishing_qty.",".$txt_reject_qnty.",".$txt_remark.",".$user_id.",'".$pc_date_time."','1','0',673,".$txt_sample_qty.",".$cbo_sample_team.",".$txt_total_cutting_qty.")";
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				if($j==0)$data_array_brk = "(".$colorsize_brk_id.",".$mst_update_id.",".$dtls_id.",'".$colorID."','".$sizeID."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$user_id.",'".$pc_date_time."','1','0',673)";
				else $data_array_brk .= ",(".$colorsize_brk_id.",".$mst_update_id.",".$dtls_id.",'".$colorID."','".$sizeID."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$user_id.",'".$pc_date_time."','1','0',673)";
				$colorsize_brk_id+=1;
				$j++;
			}
  	...
  	else if ($operation==1) // Update part ------------------------------------------------------
	{
		...
		if($mst_update_id!='')
		{
			...
			$data_array_mst="".$cbo_company_name."*".$cbo_source."*".$cbo_finishing_company."*".$cbo_location."*".$cbo_floor."*".$user_id."*'".$pc_date_time."'";
			...
			$data_array_dtls="".$txt_finishing_date."*".$txt_reporting_hour."*".$txt_finishing_qty."*".$txt_reject_qnty."*".$txt_remark."*".$user_id."*'".$pc_date_time."'*".$cbo_sample_team."";
			...
			foreach($rowEx as $rowE=>$valE)
			{
				...
				if($j==0)$data_array_brk = "(".$colorsize_brk_id.",".$mst_update_id.",".$dtls_update_id.",'".$colorID."','".$sizeID."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$user_id.",'".$pc_date_time."','1','0',673)";
				else $data_array_brk .= ",(".$colorsize_brk_id.",".$mst_update_id.",".$dtls_update_id.",'".$colorID."','".$sizeID."','".$colorSizeValue."','".$rejQtyArr[$index]."',".$user_id.",'".$pc_date_time."','1','0',673)";
				$colorsize_brk_id+=1;
				$j++;
			}
...
if($action=="packing_finishing_print")
{
	extract($_REQUEST);
	...
	$sql="select a.sample_name, a.item_number_id, a.sewing_date, $reporting_hour_fill, sum(a.qc_pass_qty) as qc_pass_qty, a.remarks, b.color_id, b.size_id, sum(b.size_pass_qty) as size_pass_qty, sum(b.size_rej_qty) as size_rej_qty
	from sample_sewing_output_dtls a,sample_sewing_output_colorsize b
	where a.id=b.sample_sewing_output_dtls_id and a.sample_sewing_output_mst_id=$mst_id and a.id in($dtls_id) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.entry_form_id=673 group by a.sample_name, a.item_number_id,a.sewing_date,a.reporting_hour,a.remarks, b.color_id, b.size_id";
	//echo $sql;//die;

	$sql_result=sql_select($sql);
	$gmts_item_arr=array(); $sewing_date_arr=array(); $sample_name_arr=array(); $reporting_hour_arr=array();
...
<div style="width:<? echo $width_2;?>px;">
    <table width="100%" cellspacing="0">
        ...
        <tr>
            <td colspan="3" align="center"><strong>Sample Packing and Finishing </strong></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td><strong>Finishing Date : </strong><? echo implode(",", $sewing_date_arr);  ?></td>
        </tr>
        <tr>
