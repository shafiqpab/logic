<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Sample RMG Production Report V2.
Functionality	:	
JS Functions	:
Created by		:	md mamun ahmed sagor
Creation date 	: 	18-05-2022
Updated by 		: 	Aziz	
Update date		: 	17-12-2022	   
QC Performed BY	:		
QC Date			:	
Comments		:
*/
...
//---------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Sample RMG Production Report V2", "../../", 1, 1,$unicode,'','');
?>	
<script>
	...
	function fn_report_generated(excel_type)
	{
		...
		{
			var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_sample_stage*cbo_buyer_name*cbo_dealing_merchant*cbo_year*txt_req_no*txt_booking_no*txt_internal_ref*cbo_report_category*txt_date_from*txt_date_to',"../../")+'&excel_type='+excel_type;
			//alert(data);return;
			freeze_window(3);
			http.open("POST","requires/sample_rmg_production_report_v2_controller.php",true);
	...
	{
  		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/sample_rmg_production_report_v2_controller.php?sample_req_mst_id='+sample_req_mst_id+'&sample_name='+sample_name+'&action='+action, 'MnM Ex-Factory Date  View', 'width=500px,height=450px,center=1,resize=0,scrolling=0','');
	}
	function openmypage_sample_req(data,action)
	{
  		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/sample_rmg_production_report_v2_controller.php?action='+action+'&data='+data, 'Sample Req. Qty  View', 'width=500px,height=450px,center=1,resize=0,scrolling=0','');
	}
	
	function openmypage_remark(sample_dtls_id,smp_dev_mst_id,color_id,category,date_form,date_to,action)
	{
  		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/sample_rmg_production_report_v2_controller.php?sample_dtls_id='+sample_dtls_id+'&smp_dev_mst_id='+smp_dev_mst_id+'&color_id='+color_id+'&category='+category+'&date_form='+date_form+'&date_to='+date_to+'&action='+action, 'Remarks View', 'width=500px,height=450px,center=1,resize=0,scrolling=0','');
	}

	function openmypage_details(sample_id,entry_form,embl_id,category,date_form,date_to,sub_action)
	{
  		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/sample_rmg_production_report_v2_controller.php?data='+sample_id+'&sub_action='+sub_action+'&entry_form='+entry_form+'&embl_id='+embl_id+'&category='+category+'&date_form='+date_form+'&date_to='+date_to+'&action=details_view', 'Details View', 'width=500px,height=450px,center=1,resize=0,scrolling=0','');
	}
	

	function openmypage_reject_qty(sample_dtls_id,color_id,action)
	{
  		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/sample_rmg_production_report_v2_controller.php?sample_dtls_id='+sample_dtls_id+'&color_id='+color_id+'&action='+action, 'Remarks View', 'width=500px,height=450px,center=1,resize=0,scrolling=0','');
	}
	...
	function openmypage_order(po_break_down_id,gmts_item_id,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/sample_rmg_production_report_v2_controller.php?po_break_down_id='+po_break_down_id+'&gmts_item_id='+gmts_item_id+'&action='+action+'&garments_nature='+garments_nature, 'Order Quantity', 'width=550px,height=250px,center=1,resize=0,scrolling=0','');
	}
    ...
    <div style="width:100%;" align="center">    
        <? echo load_freeze_divs ("../../",'');  ?>
         <fieldset style="width:1100px;">
            ...
            <table class="rpt_table" width="1100px" cellpadding="0" cellspacing="0" align="center" border="1" rules="all">
               <thead>                    
                    <tr>
						...
						<th width="100">Report Category</th>
                        <th width="130" colspan="2" id="search_text_td">Date Range</th>
                        <th><input type="reset" id="reset_btn" class="formbutton" style="width:70px" value="Reset" /></th>
                    </tr>    
                 </thead>
                <tbody>
                    <tr class="general">
                        <td><? echo create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/sample_rmg_production_report_v2_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" ); ?></td>
						...
						<td ><?
						$type_arr=array(1=>'Req. Date',2=>'Transaction Date');
						 echo create_drop_down( "cbo_report_category", 100, $type_arr,"", 1, "-- Select Buyer --", 1, "",0,"" );
						  ?></td>
