...
class Planning_model extends CI_Model {
	...
	function get_plan_data_info($company_id, $location_id, $floor_id, $txt_date_from, $user_id, $auto_balancing = 1) {
		...
		$sql_line = "select ID,USER_IDS,LOCATION_NAME,FLOOR_NAME,LINE_NAME from lib_sewing_line where STATUS_ACTIVE=1 and IS_DELETED=0 $comCon3 $loc_cond $floor_cond_line order by sewing_line_serial";
		// echo $sql_line;die;
		...
		// if ($company_id > 0) {$comCon3 = " and company_name=$company_id";}
		// $line_names_ids = array();
		// $sql_line = "select ID,USER_IDS,LOCATION_NAME,FLOOR_NAME,LINE_NAME from lib_sewing_line where 1=1 $comCon3 $loc_cond $floor_cond_line order by sewing_line_serial";

		// $user_wise_line_data_arr=array();
		// $new_line_resource = $this->db->query($sql_line)->result();
		// foreach ($new_line_resource as $ids => $vals) {
		// 	foreach(explode(',',$vals->USER_IDS) as $uid){
		// 		$user_wise_line_data_arr[$uid][$vals->FLOOR_NAME] = $vals->FLOOR_NAME;
		// 	}
		// 	$line_names_ids[$vals->ID] = $vals->ID;
		// }
		// $null_arr = array();
		...
		$is_plan = 0; $insert_plan_temp_data=array();
		foreach ($plan_dtls_data as $rows) {
 			// $po_arr_by_plan[$rows->PLAN_ID][$rows->PO_BREAK_DOWN_ID]=$rows->PO_BREAK_DOWN_ID;
 			// $item_arr_by_plan[$rows->PLAN_ID][$rows->ITEM_NUMBER_ID]=$rows->ITEM_NUMBER_ID;
 			// $size_arr_by_plan[$rows->PLAN_ID][$rows->SIZE_NUMBER_ID]=$rows->SIZE_NUMBER_ID;
 			// $color_arr_by_plan[$rows->PLAN_ID][$rows->COLOR_NUMBER_ID]=$rows->COLOR_NUMBER_ID;
 			// $country_arr_by_plan[$rows->PLAN_ID][$rows->COUNTRY_ID]=$rows->COUNTRY_ID;

			$po_arr_by_plan[$rows->PLAN_ID][]=$rows->PO_BREAK_DOWN_ID;
 			$item_arr_by_plan[$rows->PLAN_ID][]=$rows->ITEM_NUMBER_ID;
		...
		$marge_plan_sql = "SELECT  a.MERGED_PLAN_ID,a.set_dtls_id,a.color_size_id,a.PO_BREAK_DOWN_ID as multi_po,a.learing_iterator, a.half, a.id,a.line_id,a.plan_id,a.start_date,a.start_hour,a.end_date,a.end_hour,a.duration,a.plan_qnty,a.comp_level,a.first_day_output, a.next_first_day_output,
		a.next_increment, a.next_terget, a.increment_qty,a.terget,a.day_wise_plan,a.company_id,a.location_id,a.off_day_plan,a.order_complexity,a.ship_date, a.extra_param,a.plan_level,a.first_day_capacity,a.last_day_capacity,a.seq_no,a.po_company_id,1 as use_learning_curve,a.current_production_date,a.production_percent, a.top_border_color,a.bottom_border_color,a.left_color,a.right_color,1 as job_no,a.LOCATION_X,a.LOCATION_Y,a.SMV, 0 as PO_BREAK_DOWN_ID,  0 as ITEM_NUMBER_ID,  0 as SIZE_NUMBER_ID, 0 as COLOR_NUMBER_ID, 0 as COUNTRY_ID, a.MERGE_COMMENTS,a.MERGE_TYPE,a.INSERT_DATE  ,a.NOTES,a.CLOSING_STATUS,a.CLOSED_BY,a.CLOSING_DATE,a.CLOSING_NOTE,a.RE_OPEN_DATE,a.RE_OPENED_BY,a.RE_OPEN_NOTE,a.ALLOCATED_MP,a.BYPASS_MP,a.remaining_work_hour,a.AUTO_TARGET from ppl_sewing_plan_board a, gbl_temp_engine gbl where a.MERGED_PLAN_ID=gbl.ref_val and gbl.entry_form=5001 and gbl.REF_FROM=55 and gbl.user_id=$user_id and a.MERGE_TYPE is not null and a.status_active=1 ";
		...
		if ($is_plan == 1) {
			$sql=where_con_using_array($npos,0,'PO_NUMBER_ID');	
		}
		$sql4=where_con_using_array($line_id_arr,0,'a.SEWING_LINE');
		$sql2=where_con_using_array($npos,0,'c.id');
		$sql3=where_con_using_array($npos,0,'a.PO_BREAK_DOWN_ID');
		...
		$sqls = $this->db->query("select min(a.task_start_date) as TASK_START_DATE,max(a.task_finish_date) as TASK_FINISH_DATE,a.PO_NUMBER_ID from tna_process_mst a, gbl_temp_engine gbl where a.is_deleted=0 and a.status_active=1  and a.task_number in(86,190,191) and a.PO_NUMBER_ID=gbl.ref_val and gbl.entry_form=5001 and gbl.REF_FROM=56 and gbl.user_id=$user_id  group by a.po_number_id");//$sql
		...
		foreach ($plan_data as $rows) {
			...
			if (1 == 1) {
				...
				$size_name_arr = array();
				foreach ($ROWS_SIZE_NUMBER_ID_ARR as $sid) {
				...
				$data_array[$i]["PO_BREAK_DOWN_ID"] = implode(',',$ROWS_PO_BREAK_DOWN_ID_ARR) .'';
				...
				/*if($wo_po_details[end($ROWS_PO_BREAK_DOWN_ID_ARR)][IS_CONFIRMED]==2){
	...
	if($plan_lavel_data_arr[0]->BULLETIN_TYPE!=2){//not color lavel
		...
		// echo $timediff = $endtime - $starttime;die;
 ...
 */
	...
	function create_plan($plan_obj) {
		...
		if ($response_obj->Status == 1) {
			...
			foreach ($response_obj->SewingPlanBoard as $sewing_plan_row) {
				...
				if ($sewing_plan_row->RowState == "add" || $sewing_plan_row->MERGE_TYPE=='Merged') {
					...
					//$ppl_sewing_plan_board_data['PLAN_ID'] = $sewing_plan_row->PLAN_ID;
					//$update_plan_data_arr[$plan_id]=$ppl_sewing_plan_board_data;
			...
			//$this->db->trans_complete();die;
			
			//echo implode(',',$delete_plan_id_arr);die;
			//--------------------------------end;
		...
		// if(count($update_line_capacity_data_arr)){
		// 	$this->db->update_batch('LINE_CAPACITY', $update_line_capacity_data_arr, 'ID');
		// }		
