...
class Yarn_parking_model extends CI_Model
{
    ...
    public function grn_wise_yarn_data_for_issue($req_no, $basis_id)
    {
        ...
        if ($basis_id == 3) {
            ...
            // print_r(5);
            // die;
            $demand_query = "SELECT a.PROD_ID,a.REQUISITION_NO,a.YARN_QNTY,c.COMPANY_ID, 8 as RCV_BASIS,c.BOOKING_NO,b.KNITTING_SOURCE,c.BUYER_ID,c.PO_ID,c.IS_SALES,d.WITHIN_GROUP,d.PO_BUYER,b.KNITTING_PARTY,f.DEMAND_SYSTEM_NO
    ...
    public function grn_wise_yarn_data_for_issue_save($response_arr)
    {
        ...
        if($rfid_arr)
        {
            ...
            $rfid_table_max_id = return_next_id("ID","RFID_YARN_DTLS");
            for($i=0;$i<$count;$i++)
            {
                $table_rfid_dtls[$i]["ID"] = $rfid_table_max_id;
                $table_rfid_dtls[$i]["UPDATED_BY"] = $response_arr->USER_ID;
                $table_rfid_dtls[$i]["TRANS_TYPE"] = 2;
                $rfid_table_max_id++;
            }            
         //print_r($table_rfid_dtls);die;
        }

         //end RFID SAVE
        //print_r($cbo_store_name);die;           
        ...
        if (($variable_set_allocation != 1) && ($over_issue_status == 1) && (str_replace("'", "", $cbo_issue_purpose) == 1)) {
            ...
            if ($operation == 1) {
                $update_id_cond = " and b.id <> $update_id";
            }
        ...
        if ($operation == 0) // Insert Here----------------------------------------------------------
        {
            ...
            //start RFID insert with trans_type = 2
            if($rfid_arr)
            {
                $this->db->insert_batch("RFID_YARN_DTLS", $$table_rfid_dtls);
            }
            //end RFID insert with trans_type = 2
    ...
    public function createObj_grn_wise_yarn_data_for_issue_save($obj)
    {
        ...
        if ($basis_id == 3) {
            ...
            $demand_query = "SELECT a.COMPANY_ID,3 as RCV_BASIS,a.BOOKING_NO,b.KNITTING_PARTY as KNITTING_COMPANY,a.BUYER_ID,c.REQUISITION_NO,c.PROD_ID,c.REQUISITION_DATE,sum(c.YARN_QNTY) as YARN_QNTY,c.ID as REQUISI_ID,b.KNITTING_SOURCE,b.KNITTING_PARTY,e.DEMAND_SYSTEM_NO,e.ID as DEMAND_ID,e.KNITTING_COMPANY from PPL_PLANNING_INFO_ENTRY_MST a, PPL_PLANNING_INFO_ENTRY_DTLS b, PPL_YARN_REQUISITION_ENTRY c,PPL_YARN_DEMAND_ENTRY_DTLS d,PPL_YARN_DEMAND_ENTRY_MST e where a.ID=b.MST_ID and b.ID=c.KNIT_ID and d.REQUISITION_NO = c.requisition_no and e.ID = d.MST_ID and e.DEMAND_SYSTEM_NO = '$req_no' and c.status_active=1 and c.is_deleted=0 group by a.company_id,a.booking_no,c.requisition_no,c.prod_id,c.REQUISITION_DATE,a.buyer_id,b.KNITTING_PARTY,c.ID,b.KNITTING_SOURCE,b.KNITTING_PARTY,e.DEMAND_SYSTEM_NO,e.ID,e.KNITTING_COMPANY";
            ...
            $requisition_query = "SELECT a.COMPANY_ID, 1 as RCV_BASIS,a.BOOKING_DATE,a.SUPPLIER_ID,b.PRODUCT_ID,e.PO_BUYER,b.YARN_COLOR,a.SUPPLIER_ID,a.YDW_NO,a.ENTRY_FORM,b.JOB_NO,c.BUYER_NAME,a.SOURCE from wo_yarn_dyeing_mst a, wo_yarn_dyeing_dtls b left join wo_po_details_master c on b.job_no_id=c.id left join wo_non_ord_samp_booking_mst d on b.booking_no=d.booking_no left join fabric_sales_order_mst e on e.job_no=b.job_no where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.ydw_no = '$req_no' and a.entry_form in(41,42,114,125,135)";
            //print_r($requisition_query);die;
            $table = $this->db->query($requisition_query)->result();
