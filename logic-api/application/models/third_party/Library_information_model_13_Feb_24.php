...
class Library_information_model extends CI_Model
{
	...
	{
		if ($model_id) {
			$where_con = "and a.STYLE_REF_NO like '%$model_id%'";
		}
		//print_r($model_id);die;
		$query = "SELECT a.JOB_NO,a.STYLE_REF_NO,a.REMARKS,a.BUYER_NAME AS BUYER_ID,d.BUYER_NAME,c.GMTS_ITEM_ID,e.ITEM_NAME from WO_PO_DETAILS_MASTER a, WO_PO_BREAK_DOWN b, WO_PO_DETAILS_MAS_SET_DETAILS c, LIB_BUYER d,LIB_GARMENT_ITEM e WHERE b.JOB_ID = a.ID AND d.ID = a.BUYER_NAME AND c.JOB_ID = b.JOB_ID AND c.GMTS_ITEM_ID = e.ID AND b.SHIPING_STATUS !=3 AND a.IS_DELETED = 0 AND a.STATUS_ACTIVE=1 $where_con ORDER BY a.ID DESC";
		//echo $query; die;
	...
	function knit_order_image()
	{
		$query_image_path = "SELECT MASTER_TBLE_ID,IMAGE_LOCATION FROM COMMON_PHOTO_LIBRARY where FORM_NAME = 'knit_order_entry'";	
